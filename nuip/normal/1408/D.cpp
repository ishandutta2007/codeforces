...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	cauto n=read();
	cauto m=read();
	cauto vs=readV<pii>(n);
	cauto ws=readV<pii>(m);
	vector<int> lb(1123456);
	for(cauto &[a,b]:vs)for(cauto &[c,d]:ws)if(a<=c && b<=d) MX(lb[c-a],d-b+1);
	rreps(i,1,lb.size()) MX(lb[i-1],lb[i]);
	rep(i,10) out(i,lb[i],1);
	int re=MOD;
	rep(i,lb.size()) MN(re,i+lb[i]);
	cout<<re NL;
  return 0;
}