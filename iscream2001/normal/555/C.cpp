...
void Change(int &id,int l,int r,int ql,int qr,int K){
	...
	if(ql<=l&&qr>=r){
		if(val[id]<K) val[id]=K;
		return;
	}
	int mid=(l+r)>>1;
	Change(ls[id],l,mid,ql,qr,K);
	Change(rs[id],mid+1,r,ql,qr,K);
	return;
}
int ask(int id,int l,int r,int ql){
	if(ql>r||ql<l||id==0) return 0;
	int mid=(l+r)>>1,res=val[id];
	res=max(res,ask(ls[id],l,mid,ql));
	res=max(res,ask(rs[id],mid+1,r,ql));
	return res;
}
void MAIN(){
	int x,y,z;
	...
	while(m--){
		...
		if(op[0]=='U'){
			z=ask(1,1,n,x);
			cout<<max(y-z,0)<<"\n";
			Change(rt1,1,n,x,x,y);
			if(z+1<=y) Change(rt2,1,n,z+1,y,x);
		}
		else{
			z=ask(2,1,n,y);
			cout<<max(x-z,0)<<"\n";
			Change(rt2,1,n,y,y,x);
			if(z+1<=x) Change(rt1,1,n,z+1,x,y);
		}
	}
	return;
}
