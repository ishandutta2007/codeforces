...
int main(void)
{
	...
	stack<P> stk, stk2;
	stk.push(P(0, inf)), stk2.push(P(0, -inf));
	rep(i, 1, n){
		while(stk.top().snd <= a[i]){
			ll r = stk.top().fst, h = stk.top().snd; stk.pop();
			ll l = stk.top().fst;
			seg.add(l, r-1, -h);
		}
		seg.add(stk.top().fst, i-1, a[i]);
		stk.push(P(i, a[i]));
		
		while(stk2.top().snd >= a[i]){
			ll r = stk2.top().fst, h = stk2.top().snd; stk2.pop();
			ll l = stk2.top().fst;
			seg.add(l, r-1, h);
		}
		seg.add(stk2.top().fst, i-1, -a[i]);
		stk2.push(P(i, a[i]));
