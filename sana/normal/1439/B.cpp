...
typedef gp_hash_table<int, int, chash> table;
...
void solve() {
    ...
    while (!degs.empty()) {
        if (degs.begin()->first >= k) {
            cout << 1 << " " << sz(degs) << '\n';
            for (auto p : degs) cout << p.second+1 << " ";
            cout << '\n';
            return;
        }

        if (degs.begin()->first == k-1) {
            ...
            for (int i = 0; i < sz(th) && ok; i++) {
                for (int j = i+1; j < sz(th) && ok; j++) {
                    if (ar[th[i]].find(th[j]) == ar[th[i]].end()) ok = false;
                }
            }
