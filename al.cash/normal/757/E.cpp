...
// Least prime divisor of all the numbers <= N: O(N).
// Factorization: O(number of prime divisors).
struct Divisors {
  int size;
  vector<int> leastDivisor, primes;

  Divisors(int n = 0) { init(n); }

  void init(int n) {
    size = n;
    leastDivisor.assign(n + 1, 0);
    for (int i = 2; i <= n; ++i) {
      if (leastDivisor[i] == 0) {
        leastDivisor[i] = i;
        primes.push_back(i);
  ...
  vector<pair<T, int>> factorize(T n) {
    assert(0 < n && n / primes.back() <= primes.back() + 1);
    vector<pair<T, int>> res;
    for (int p : primes) {
      if (n <= size || n < sqr(static_cast<T>(p))) break;
      if (n % p == 0) {
        int power = 0;
        do { n /= p; ++power; } while (n % p == 0);
        res.emplace_back(p, power);
      }
    }
    if (size < n) res.emplace_back(n, 1);
    else while (n != 1) {
  ...
  void enumerate(T value, int level, const vector<pair<T, int>>& factors,
                 Visitor visitor) {
    if (level == static_cast<int>(factors.size())) visitor(value);
    else for (int i = 0; i <= factors[level].second; ++i) {
      if (i != 0) value *= factors[level].first;
      enumerate(value, level + 1, factors, visitor);
    }
  }

  template <class T, class Visitor>
  void processDivisors(T n, Visitor visitor) {
    enumerate(1, 0, factorize(n), visitor);
  }
};
...
public:
  ...
  F operator () (const F& x) const {  // O(degree)
    F res = 0;
  ...
  void addPoint(const F& x, F y) {  // O(degree)
    F w = 1;
...
int main() {
  ...
  FOR(i, 1, M) {
    ...
    if (i == x) {
      ...
      f[kg][0] = 1 << sz(fact);
      g[i] = kg;
      d.enumerate(1, 0, fact, [kg](int x) { e[kg].pb(g[x]); });
      ++kg;
    } else {
      g[i] = g[x];
    }
