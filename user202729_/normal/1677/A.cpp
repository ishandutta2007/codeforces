// that was unintended?
// Okay...
// making it linear space is a bit more difficult though.
...
void up(){
	...
	std::vector<int> belowBeforeB(number);
	int64_t result {};
	for(int const b: range(1, number)){
		for(int const j: range(p[b-1], number)){
			belowBeforeB[j]++;
		}

		int afterCBelowPB=0;
		for(int c=number-2; c>b; c--){
			afterCBelowPB+=p[c+1]<p[b];
			result+=int64_t(afterCBelowPB)*belowBeforeB[p[c]];
		}
	}
	cout<<result<<'\n';
