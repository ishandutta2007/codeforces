...
void solve() {
    ...
    int ans = 0;
    // time, check, num
    pqg<tuple<int, int, int>> pque;
    rep(i, n) {
        pque.push(make_tuple(0, check[i], i));
    }
    rep(i, m) {
        auto [ti, che, id] = pque.top(); pque.pop();
        if(che == 0) {
            g[id].ppb();
            pque.push(make_tuple(ti + 1, !(g[id].size() > 0), id));
            chmax(ans, ti + 1);
        } else {
            pque.push(make_tuple(ti + 2, 1, id));
            chmax(ans, ti + 2);
        }
    }
    out(ans);
