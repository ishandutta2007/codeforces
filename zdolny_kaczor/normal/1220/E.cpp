...
ll ans(int x, int y = -1) {
	ll ret = 0;
	for (int v : g2[x]) {
		if (v != y) maxi(ret, ans(v, x));
	}
	ret += w[x];
	return ret;
}
int main() {
	...
	scanf("%d", &s);
	FOR(i, 1, n) if (i != s) cut(i);
	FOR(i, 1, n) if (!out[i]) w[0] += w[i];
	FOR(i, 1, n) for (int v : graf[i]) {
		int vv = get(v), ii = get(i);
		if (ii != vv) {
			debug(imie(vv), imie(ii));
			g2[vv].push_back(ii);
		}
	}
	REP(i, n + 1) {
