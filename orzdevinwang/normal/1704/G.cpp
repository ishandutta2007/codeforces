...
ll a[N], b[N];
ll x[N], y[N], tx[N];
bool slv(vi qwq, int p) {
	L(i, 3, m) tx[i] = a[p + i - 1] + a[p + i - 2] * 2 + a[p + i - 3];
	L(i, 3, m) if(tx[i] != y[i] && tx[i] + 1 != y[i]) return false;
	L(i, 3, m) if(tx[i] != y[i]) qwq.emplace_back(i + p - 1);
...
void Main() {
	cin >> n;
	L(i, 1, n) cin >> a[i];
	L(i, 3, n) x[i] = a[i] + a[i - 1] * 2 + a[i - 2];
	cin >> m;
	L(i, 1, m) cin >> b[i];
	L(i, 3, m) y[i] = b[i] + b[i - 1] * 2 + b[i - 2];
	
	xsum = 0;
	L(i, 3, m) y[i] = min(y[i], (ll) 1e6);
	L(i, 3, m) xsum += y[i] * y[i] - y[i];
	L(i, 1, n) pre[i] = pre[i - 1] + x[i] * x[i] + x[i];
	
	poly f(n + 1), g(m + 1);
	L(i, 3, n) f[i] = (x[i] + mod) % mod;
	L(i, 3, m) g[m - i] = (y[i] + mod) % mod;
	f *= g;
	
	L(i, 1, n - m + 1) {
		ll sum = f[i + m - 1];
		sum = -sum * 2;
		sum += xsum + pre[i + m - 1] - pre[i + 1];
		if(sum % mod == 0) { 
			ll wa = b[1] - a[i], wb = b[2] - a[i + 1];
			wb = wa + wb, swap(wa, wb), wb = -wb;
			
			ll len1 = i / 2 + 1, len2 = (i + 1) / 2;
			ll ql = max(wa, 0LL), qr = min(len1, len2 + wa);
			ll mi = (len2 + wa + 1) / 2;
			
			L(j, max(mi - 5, ql), min(mi + 5, qr)) {
				ll k = j - wa;
				ll val = wb + k;
				if(val & 1) continue;
				val /= 2;
				ll wl = S(0, j - 1) - S(len2 - j + wa, len2 - 1), wr = S(len1 - j, len1 - 1) - S(0, j - wa - 1);
				if(wl <= val && val <= wr) {
					vi qwq;
					ll tmp = val - wl;
					R(i, j - 1, 0) {
						ll move = min(tmp, len1 - j);
						tmp -= move;
						qwq.emplace_back(2 * (i + move));
					} 
					L(i, len2 - k, len2 - 1) {
						ll move = min(tmp, len2 - k);
						tmp -= move;
						qwq.emplace_back(2 * (i - move) + 1);
					} 
					L(j, 0, sz(qwq) - 1) qwq[j] = i - qwq[j] + 1;
					if(!slv(qwq, i)) break ;
					return ;
				}
			}
