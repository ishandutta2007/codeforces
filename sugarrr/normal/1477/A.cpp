...
/*
 ...
 rep(z,1,1000){
     ...
     for(auto x:v)v.pb(x);
     for(auto x:v){
         l_l nx={t.fi*2-x.fi,t.se*2-x.se};
         if(s.find(nx)==s.end()){
             s.insert(nx);
             q.push(nx);
         }
         nx={x.fi-t.fi*2,x.se-2*t.se};
         if(s.find(nx)==s.end()){
             s.insert(nx);
             q.push(nx);
         }
 ...
 ll N=100;
 bool f[N*2][N*2];memset(f,false,sizeof(f));
 for(auto x:s){
     l_l y={x.fi+N,x.se+N};
     if(0<=y.fi&&y.fi<=2*N-1&&0<=y.se&&y.se<=2*N-1){
         f[y.fi][y.se]=true;
     }
 }
 rep(i,-10,10){
