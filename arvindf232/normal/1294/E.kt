...
fun shifts(a:IntArray,tar:IntArray):Int {
    val n = a.size
    val tars = mutableMapOf<Int,Int>()
    for((i,v) in tar.withIndex()){
        tars[v] = i
    }
    ...
    return (0 until n).minOf{n - profits[it] + it}
}
...
fun main(){
    solve.cases{
        val n = getint()
         val m = getint()
        val all = List(n){getline(m)}
        var cost = 0
        for(i in 0 until m){
            val here = IntArray(n){all[it][i]}
            val to = IntArray(n){it * m + (i + 1)}
            cost += shifts(here,to)
        }
        put(cost)
