...
int main(void)
{
	...
	cin >> T;
	while(T--){
		cin >> sx >> sy >> gx >> gy >> fx >> fy;
		if(sx > gx) sx *= -1, gx *= -1, fx *= -1;
		if(sy > gy) sy *= -1, gy *= -1, fy *= -1;
		
		ll ans = abs(sx-gx) + abs(sy-gy);
		if(sx == gx && sx == fx && sy <= fy && fy <= gy) ans += 2;
		if(sy == gy && sy == fy && sx <= fx && fx <= gx) ans += 2;
		outl(ans);
	}
