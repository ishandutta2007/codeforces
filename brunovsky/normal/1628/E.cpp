...
auto edge_line_tree(int N, vector<array<int, 2>> tree, vector<int> order) {
    disjoint_set dsu(N);
...
// min edge line array for less<T> (i.e. array retains the minimum edge on path)
template <typename T, typename Compare = less<T>>
auto edge_line_array(int N, const vector<array<int, 2>>& edges, const vector<T>& weight,
    ...
    auto line = edge_line_tree(N, edges, order);
...
struct Segnode {
    ...
    static constexpr int ACTIVATE = 1, DEACTIVATE = 0;
    int low = INT_MAX, low_all = INT_MAX;
    int high = INT_MIN, high_all = INT_MIN;
    int lazy = -1;

    Segnode() = default;
    Segnode(int pos) : low_all(pos), high_all(pos) {}

    void pushup(const Segnode& lhs, const Segnode& rhs) {
        low = min(lhs.low, rhs.low);
        high = max(lhs.high, rhs.high);
        low_all = min(lhs.low_all, rhs.low_all);
        high_all = max(lhs.high_all, rhs.high_all);
    }
    ...
    void apply(int modify) {
        if (modify == DEACTIVATE) {
            low = INT_MAX, high = INT_MIN;
        } else if (modify == ACTIVATE) {
            low = low_all, high = high_all;
        }
        lazy = modify;
    }
};
...
int main() {
    ...
    for (int e = 0; e < N - 1; e++) {
        auto& [u, v] = tree[e];
        cin >> u >> v >> weight[e], u--, v--;
    }

    auto [index, edge, value] = edge_line_array(N, tree, weight, greater<int>{});
    sparse_table rmq(value, &maxop);
    segtree<Segnode> seg(N, index);

    while (Q--) {
        if (int t; cin >> t, t == 1) {
            int l, r;
            cin >> l >> r, l--;
            seg.update_range(l, r, Segnode::ACTIVATE);
        } else if (t == 2) {
            int l, r;
            cin >> l >> r, l--;
            seg.update_range(l, r, Segnode::DEACTIVATE);
        } else if (t == 3) {
            int x;
            cin >> x, x--;
            auto state = seg.query_all();
            int a = state.low;
            int b = state.high;
            int ans = -1;
            if (a < index[x]) {
                ans = max(ans, rmq.query(a, index[x]));
            }
            if (index[x] < b) {
                ans = max(ans, rmq.query(index[x], b));
            }
            cout << ans << '\n';
        } else {
            assert(false);
