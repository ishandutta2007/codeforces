...
int main() {
    ...
    vector<array<array<int, 2>, 2>> cnt_r(n, { 0, 0, 0, 0 }), cnt_c(m, { 0, 0, 0, 0 });
    int ng_r = 0, ng_c = 0;
    int fr_r = n, fr_c = m;

    array<array<int, 2>, 2> cw { 0, 0, 0, 0 };

    auto is_free = [&](array<array<int, 2>, 2> &cnt) {
        return cnt == array<array<int, 2>, 2>{ 0, 0, 0, 0 };
    };

    auto is_ng = [&](array<array<int, 2>, 2> &cnt) {
        array<int, 2> c { 0, 0 };
        rep(i, 2) rep(j, 2) {
            c[i ^ j] += cnt[i][j];
        }
    ...
    auto unset = [&](int i, int j) {
        ...
        fr_r += is_free(cnt_r[i]);
        fr_c += is_free(cnt_c[j]);
    };
    ...
    vector<mint> pow2(M);
    pow2[0] = 1;
    rep(i, 1, M) pow2[i] = pow2[i - 1] + pow2[i - 1];
    ...
    loop(k) {
        ...
        mint ans = 0;
        if (not ng_r) ans += pow2[fr_r];
        if (not ng_c) ans += pow2[fr_c];

        if (cw[0][0] == 0 and cw[1][1] == 0) --ans;
        if (cw[0][1] == 0 and cw[1][0] == 0) --ans;
