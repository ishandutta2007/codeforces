...
ll solve(ll n, ll m) {
    ...
    for (ll i = 1; i <= t; ++i) {
        ...
        //        DBN(from, to);
        ll first = n % to;
        ll step = n % (to - 1) - first;
        //        DBN(first, step);
        ll k = to - from + 1;
        ll tk = k & 1 ? (k - 1) / 2 % mod * (k % mod) : k / 2 % mod * ((k - 1) % mod);
        tk %= mod;
        ans += first % mod * (k % mod) + step % mod * tk % mod;
        //        DBN(ans, tk, first % mod * k % mod + step % mod * tk % mod);
        ans %= mod;
    }
...
int main() {
    ...
    return 0;
    //    while(true)
    for (ll n = 1e13; n >= 100; n--) {
        for (int m = 1; m <= 100; ++m) {
            if (solve(n, m) != snaive(n, m)) {
                DBN(n, m);
            }
        }
    }
