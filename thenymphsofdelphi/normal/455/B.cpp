...
void add(string &s){
	int cur = 0;
	Fora(ch, s){
		if (trie[cur].next[ch - 'a'] == -1){
			trie[cur].next[ch - 'a'] = trie.size();
			trie.eb(cur, ch);
		}
...
void dfs(int cur){
    ...
    For(i, 0, 26){
        if (trie[cur].next[i] != -1){
            leaf = 0;
            dfs(trie[cur].next[i]);
            if (!dp1[trie[cur].next[i]]){
                dp1[cur] = 1;
            }
            if (!dp2[trie[cur].next[i]]){
                dp2[cur] = 1;
            }
        }
    }
...
signed main(){
    ...
    dp1.resize(trie.size());
    dp2.resize(trie.size());
    dfs(0);
    if (!dp1[0]) Ptest("Second");
    if (dp2[0]) Ptest("First");
    if (k % 2 == 1) Ptest("First");
    cout << "Second";
}
