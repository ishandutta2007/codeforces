...
void solve() {
    ...
    int req_length = s.size() - first_zero;
    for (int i = first_zero; i < s.size(); i++)
        if (s[i] == '0') pos_zero.push_back(i - first_zero);
    int max_start = s.size() - req_length;
    vector<int> candidate;
    for (int i = 0; i < max_start; i++)
        if (s[i] == '1') candidate.push_back(i);
    for (int i = 1; i < pos_zero.size(); i++) {
        static vector<int> new_candidate;
        new_candidate.clear();
        for (auto&& c : candidate) {
            if (s[c + pos_zero[i]] == '1') new_candidate.push_back(c);
        }
    ...
    for (auto&& c : candidate) {
        ...
        for (int i = 0; i < req_length; i++)
            if ((res[first_zero + i] == '0') && (s[c + i] == '1')) res[first_zero + i] = '1';
        ans = max(ans, res);
    }
    cout << ans << '\n';
}
