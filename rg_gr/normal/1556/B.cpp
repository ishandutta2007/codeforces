...
{
	int T=read();
	rg(T)int n=read(),sum=0;
	rg(n)a[i]=read()&1;sum+=a[i];gr
	if(sum*2!=n&&sum*2+1!=n&&sum*2-1!=n){puts("-1");continue;}
	//start with 1
	int ans=0x3f3f3f3f3f3f3f3fll;
	if(sum*2==n||sum*2-1==n)
	{
		int S=1,b=0;
		rg(n)if(a[i])b+=abs(i-S),S+=2;gr
		ans=min(ans,b);
	}
	if(sum*2==n||sum*2+1==n)
	{
		int S=1,b=0;
		rg(n)if(a[i]==0)b+=abs(i-S),S+=2;gr
		ans=min(ans,b);
	}oldl(ans);
	gr
	return 0;
