...
bool _try(int x) {
	...
	REP(i, L, R) b[i] = a[i] ^ x;
	sort(b + L, b + R + 1);
	REP(i, L, R) if(b[i] != i) return false;
	prt(x << alr), pc('\n');
	return true;
}
...
void mian() {
	...
	while(true) {
		if(R - L + 1 & 1) {
			...
			static bool hav[N];
			REP(i, L, R) hav[a[i] - 1] = hav[a[i] + 1] = false;
			REP(i, L, R) hav[a[i]] = true;
			if(L & 1) {
				REP(i, L, R) if(!hav[a[i] ^ 1]) {
					if(_try(L ^ a[i])) return;
				}
			} else {
				static int b[N];
				int n = L >> 1;
				REP(i, L, R) if(a[i] & 1) b[n++] = a[i] >> 1;
				L >>= 1, R >>= 1;
				REP(i, L, R) a[i] = b[i];
				++alr;
			}
