...
void main_() {
	...
	vector dp(n + 1, vector(n + 1, vector(10001, inf)));
	dp[0][0][0] = 0;
	REP(i, n) {
		REP(j, i + 1) {
			REP(k, 10001) {
				const auto &now = dp[i][j][k];
				if(now == inf) continue;
				if(k + a[i] <= 10000) chmin(dp[i + 1][j + 1][k + a[i]], now);
				chmin(dp[i + 1][j][k], now + b[i]);
			}
		}
	}
	REPS(j, n) {
		ld res = 0;
		REP(k, 10001) {
			const auto &now = dp.back()[j][k];
			if(now == inf) continue;
			chmax(res, min((ld)k, sum - (ld)now / 2));
		}
		cout << res << " ";
	}
	cout << endl;
