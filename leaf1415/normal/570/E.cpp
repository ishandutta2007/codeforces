...
int main(void)
{
	...
	dp[0][0][0] = 1;
	rep(i, 0, n/2-1){
		rep(j, 0, i+1) rep(k, 0, i+1) dp[(i+1)&1][j][k] = 0;
		rep(j, 0, i) rep(k, 0, i){
			ll ax = j+1, ay = i-j+1, bx = w-k, by = h-(i-k);
			if(ax+1 <= w && bx-1 >= 1 && c[ax+1][ay] == c[bx-1][by]){
				dp[(i+1)&1][j+1][k+1] += dp[i&1][j][k];
			}
			if(ax+1 <= w && by-1 >= 1 && c[ax+1][ay] == c[bx][by-1]){
				dp[(i+1)&1][j+1][k] += dp[i&1][j][k];
			}
			if(ay+1 <= h && bx-1 >= 1 && c[ax][ay+1] == c[bx-1][by]){
				dp[(i+1)&1][j][k+1] += dp[i&1][j][k];
			}
			if(ay+1 <= h && by-1 >= 1 && c[ax][ay+1] == c[bx][by-1]){
	...
	mint ans = 0;
	if(n % 2 == 0){
		rep(j, 0, n/2) rep(k, 0, n/2) if(j+k == w-1) ans += dp[n/2%2][j][k];
	}
	else{
		rep(j, 0, n/2) rep(k, 0, n/2){
			if(j+k == w-1 && c[j+1][n/2-j+1] == c[w-k][h-(n/2-k)]) ans += dp[n/2%2][j][k];
			if(j+k == w-2 && c[j+1][n/2-j+1] == c[w-k][h-(n/2-k)]) ans += dp[n/2%2][j][k];
		}
	}
	outl(ans);
