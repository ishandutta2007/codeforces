...
int main() {
  ...
  rep(t) {
    int n = IN;
    auto a = IN.vi(n*2);
    rep(i, n*2) a[i] = a[i] == 2 ? 1 : -1;
    VI left(n+1), right(n+1);
    rep(i, n) left[i+1] = a[n-1-i];
    rep(i, n) right[i+1] = a[n+i];
    rep(i, n) left[i+1]+=left[i], right[i+1]+=right[i];
    map<int, int> l, r;
    rep(i, n+1) if (not l.count(left[i])) l[left[i]] = i;
    rep(i, n+1) if (not r.count(right[i])) r[right[i]] = i;
    int res = INF;
    int s = right[n] + left[n];
    for (auto [p, k] : l) if (r.count(s-p)) {
      debug(p);
      debug(l[p]);
      debug(r[s-p]);
      chmin(res, k+r[s-p]);
