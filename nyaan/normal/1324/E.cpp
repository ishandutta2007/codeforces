...
void solve() {
  ...
  vi a(N);
  in(a);
  rep(i, 2040) rep(j, 2040) dp[i][j] = -inf;
  auto ok = [&](int n) { return l <= n && n <= r; };
  dp[0][0] = 0;
  rep(i, N) rep(j, h) {
    trc(i, j, dp[i][j]);
    if (dp[i][j] == -inf) continue;
    amax(dp[i + 1][(j + a[i] - 1) % h], dp[i][j] + ok((j + a[i] - 1) % h));
    amax(dp[i + 1][(j + a[i]) % h], dp[i][j] + ok((j + a[i]) % h));
  }
  int ans = 0;
  rep(i, h) amax(ans, dp[N][i]);
  out(ans);
}