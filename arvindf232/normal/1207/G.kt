...
class query(val song_id:Int, val str:String, val query_id:Int){
    var ans:Int = nlarge
...
fun main(){
    solve.cases{
        val n = getint
        val trie = Graph(n+1,n,true)
        repeat(n){
            val v = it + 1
            val t = getint
            if(t == 1){
                val c = getstr
                trie.addWeighted(0,v,c[0].code)
            }else{
                val j = getint
                val c = getstr
                trie.addWeighted(j,v,c[0].code)
            }
        }
        val m = getint
        val Qs = mutableListOf<query>()
        repeat(m){
            Qs.add(query(getint , getstr,it))
        }
        val qbin = Array(n+1){ mutableListOf<query>() }
        for(q in Qs){
            qbin[q.song_id].add(q)
        }
        val allstrings = Qs.map{it.str}
        val ac = AC(allstrings)
        val s = ac.nodes.size + 1
        val root = s - 1
        val G = Graph(s,s-1,false)
        for(i in 0 until s-1){
            val p = ac.nodes[i].link
            if(p == -1 || p == i){
                G.add(i,root)
            }else{
                G.add(i,p)
        ...
        val intime = G.intime()
        val outitme = G.outtime()
        val strnode = IntArray(m)
        for((i,str) in allstrings.withIndex()){
            val reached = ac.endstate(str)
            strnode[i] = reached
        }
        val st = simpleSegTree(s)
        fun query(stid:Int):Int {
            val node = strnode[stid]
            return st.query(intime[node],outitme[node])
        }
        fun addstate(v:Int){
            st.addValue(intime[v],1)
        }
        fun removestate(v:Int){
            st.addValue(intime[v],-1)
        }

        fun dfs(v:Int, p:Int, s:Int){
            addstate(s)
            for(q in qbin[v]){
                q.ans = query(q.query_id)
            }
            trie.NS_E(v){
                e,w->
                if(w == v) return@NS_E
                val c = trie.weights[e]
                val ns = ac.go(s,(c + 'a'.toInt()).toChar())
                dfs(w,v,ns)
            }
