...
int get_t(int v, int l, int r, int pos) {
    if (l == pos) {
        return dp[v];
    }
    if (l > pos || r <= pos) {
    ...
    return max({ dp[v], get_t(v << 1, l, (l + r) >> 1, pos), get_t(v << 1 | 1, (l + r) >> 1, r, pos) });
}

void upd_t(int v, int l, int r, int L, int R, int V) {
    if (R <= l || L >= r) {
        return;
    }
    if (L <= l && r <= R) {
    ...
    upd_t(v << 1, l, (l + r) >> 1, L, R, V);
    upd_t(v << 1 | 1, (l + r) >> 1, r, L, R, V);
...
inline void solve() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        if (v[i] <= i + 1) fst[v[i]].push_back(i + 1);
    }
    ...
    for (int k = 1; k <= n; ++k) {
        ...
        //}
        for (auto pos : fst[k]) {
            upd_t(1, 0, n + 1, pos - k, n + 1, get_t(1, 0, n + 1, pos - k) + 1);
        }
        //for (int i = k; i <= n; ++i) {
        //    dp[i] = ndp[i];
        ans = max(ans, get_t(1, 0, n + 1, n - k));
        //}
    }
    cout << ans << endl;
}
