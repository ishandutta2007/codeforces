...
int main(int /* argc */, const char** /* argv */)
{
    ...
    while (cin >> n && n) {
        auto p = ivec2d(ll, n, n, 0);
        {
            ...
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < n; ++j) {
                    // p[i][j] = a[i] / (ld)(a[i] + a[j]);
                    p[i][j] = divmod(a[i], (a[i] + a[j]));
                    // dbg(i, j, p[i][j]);
                }
            }
        }
        ...
        for (int msk = 1; msk < (1 << n); ++msk) {
            ...
            for (int x = msk; (x = (msk & (x - 1))); ) {
                ...
                val = (val - prob + MOD) % MOD;
            }
            f[msk] = val;

            {
                ll t = f[msk] * b.size() % MOD;
                for (auto& u : b) {
                    for (int i = 0; i < n; ++i) {
                        if ((msk & (1 << i)) == 0) {
                            t = (t * p[u][i]) % MOD;
