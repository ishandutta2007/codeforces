...
inline Z &operator*=(Z &x1, const Z x2) { return x1 = x1 * x2; }
inline Z &operator/=(Z &x1, const Z x2) { return x1 = x1 / x2; }
const int i2=(p+1)>>1,i3=(p+1)/3;
Z f[1000005],s[1000005],g[1000005];
Z C2(Z n){
	return n*(n+1)*i2;
}
Z C3(Z n){
	return n*(n+1)*(n+2)*i2*i3;
}
Z pre[1000005];
signed main(){
#ifdef QAQAutoMaton 
	...
	for(int i=2;i<=n;++i){
		f[i]=f[i-1]*s[i-2];
		f[i]+=f[i-1]*(f[i-1]+1)*i2;
		s[i]=f[i]+s[i-1];
		g[i]=f[i]-f[i-1];
	}
	Z ans=f[n]*C2(s[n-1])+C2(f[n])*s[n-1]+C3(f[n]);
	ans=ans+ans-1;
	for(int i=1;i<=n+1;++i)pre[i]=pre[i-1]+g[i];
	for(int i=1;i<=n;++i)ans+=g[i]*pre[n+1-i];
	write(ans.x,'\n');
	return 0;
}