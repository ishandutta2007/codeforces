...
void solve() {
	...
	vector<pair<int, int>> big_boys;
	for (int i = 0; i < m; ++i) {
		for (int x : a[i]) {
			big_boys.push_back({x, i});
		}
	}
	sort(all(big_boys));
	reverse(all(big_boys));
	vector<char> is_good(m);
	for (int i = 0; i < min(100, (int)big_boys.size()); ++i) {
		is_good[big_boys[i].second] = 1;
	}
	vector<int> repr(N);
	vector<char> used(N);
