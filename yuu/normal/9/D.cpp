...
uint64_t f[36][36];
bool done[36][36];
uint64_t F(int n, int h) {
    if (n == 0) return h == 0;
    if (h == 0) return 0;
    if (done[n][h]) return f[n][h];
    done[n][h] = 1;
    if (h > n) return f[n][h] = 0;
    if (n == 1) return f[n][h] = (h == 1);
    for (int i = 1; i <= n; i++) {
        for (int h_r = 0; h_r < h; h_r++) f[n][h] += F(i - 1, h - 1) * F(n - i, h_r);
        for (int h_l = 0; h_l < h - 1; h_l++) f[n][h] += F(i - 1, h_l) * F(n - i, h - 1);
    }
    return f[n][h];
}
