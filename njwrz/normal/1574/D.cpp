...
int a[12][200005],n,len[12];
priority_queue<P<int,V<int> > > q;
map<V<int>,int> mp,vis;
signed main(){
...
//	cin.tie(0);
	...
	while(!q.empty()){
		P<int,V<int> > x=q.top();q.pop();
		if(!mp.count(x.S)){
			for(auto u:x.S)cout<<u<<' ';
			break;
		}
		FOR(i,1,n)if(x.S[i-1]!=1){
			P<int,V<int> > to=x;
			to.F-=a[i][x.S[i-1]]-a[i][x.S[i-1]-1];
			to.S[i-1]--;
			if(!vis.count(to.S)){
				vis[to.S]=1;
				q.emplace(to);
