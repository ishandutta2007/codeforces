...
int main() {
    ...
    sort(A.begin(), A.end());
    vector<vector<mod_int>> dp(N + 1);

    for (int prefix = 0; prefix <= N; prefix++)
        dp[prefix].assign(prefix + 1, 0);
    ...
    for (int prefix = 1; prefix <= N; prefix++) {
        ...
        for (int remain = 1; remain < prefix; remain++)
            dp[prefix][remain] += dp[prefix - 1][remain];

        int bad = int(lower_bound(A.begin(), A.end(), A[prefix - 1] / 2 + 1) - A.begin());
        assert(bad < prefix);
        mod_int sum = 0;

        for (int remain = 1; remain - 1 <= bad; remain++) {
            sum += factorial[bad - (remain - 1)] * dp[bad][remain - 1];
            dp[prefix][remain] += sum * inv_factorial[bad - (remain - 1)];
        }
    }
