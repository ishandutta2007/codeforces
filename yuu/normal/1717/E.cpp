...
void solve() {
    cin >> n;
    // lcm(c, gcd(a, b))
    // lcm(p ^ x, gcd(p ^ y, p ^ z)) = p ^ (max(x, min(y, z))) = p ^ min(max(x, y), max(x, z))
    // lcm(g, gcd(a, b)) = gcd(lcm(a, c), lcm(b, c))
    // gcd(lcm(a, c), lcm(b, c)) = d then d | lcm(a, c), d | lcm(b, c)
    // d | c or (d | a) && (d | b)
    // or d | n - c
    // for c, for d, PIE
    for (int i = 1; i <= n; i++)
        for (int j = i; j <= n; j += i) divisors[j].push_back(i);
    for (int i = 1; i <= n; i++) reverse(divisors[i].begin(), divisors[i].end());
    int64_t ans = 0;
    for (int c = 1; c < n; c++) {
        auto&& current = divisors[n - c];
        for (auto&& d : current) {
            if (c % d) {
                assert((n - c) % d == 0);
                cnt[d] += (n - c) / d - 1;  // this much choice for a
            }
        }
        int remain = n - c - 1;
        for (auto&& d : current) {
            if (c % d == 0) continue;
            for (auto&& x : divisors[d])
                if (x != d) [[likely]]
                    cnt[x] -= cnt[d];
            assert(cnt[d] >= 0);
            if (c % d) {
                (ans += (cnt[d] % base) * (lcm(c, d) % base)) %= base;
                remain -= cnt[d];
            }
        }
        for (auto&& d : current) cnt[d] = 0;  // clean up
        (ans += static_cast<int64_t>(c) * remain) %= base;
    }
    cout << ans << '\n';
}
