...
void solve(){
  ...
  vector<int> b(N+1,0);
  REP2(i,N) REP(j,i) if(a[j]*2 <= a[i]) b[i]++;

  vector<int> id(N+1,N+1);
  REP2(i,N) FOR2(j,i+1,N) if(a[i]*2 <= a[j]) {id[i] = j; break;}
  
  mat dp(N+5,vec(N+5,0)), dp2 = dp;
  dp[0][0] = 1;

  REP2(i,N) REP2(j,N){
    if(i) dp2[i][j] += dp2[i-1][j];
    dp[i][j] +=  dp2[i][j];
    dp[i][j+1] += dp[i][j] * max(0LL,(b[i]-j)); // sad
    dp2[id[i]][j+1] += dp[i][j];
  }

  // REP2(i,N){
  //   REP2(j,N) cout << dp[i][j] << " ";
  //   cout << endl;
  // }
