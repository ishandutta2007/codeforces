...
void solve() {
  ...
  auto check = [&](Re t) -> bool {
    Intervals<bool, ll> I1(0), I2(0);
    ll INF = 1 << 20;
    FOR(i, N) {
      auto [x, v, d] = dat[i];
      Re dx = (d == 1 ? x : LIM - x);
      if (dx <= v * t) {
        if (d == 1) I1.set(0, LIM, 1);
        if (d == 2) I2.set(0, LIM, 1);
        continue;
      }
      Re t2 = (dx - v * t) / s;
      Re t1 = t - t2;
      ...
      if (d == 1) {
        Re l = dx, r = dx2;
        I1.set(ceil(l - eps), ceil(r + eps), 1);
      }
      if (d == 2) {
        Re r = LIM - dx, l = LIM - dx2;
        I2.set(ceil(l - eps), ceil(r + eps), 1);
      }
    }
    /*
    I1.debug();
    ...
    */
    vc<Re> X;
    for (auto&& [l, r, t]: I1.get(0, LIM)) { X.eb(l), X.eb(r); }
    for (auto&& [l, r, t]: I2.get(0, LIM)) { X.eb(l), X.eb(r); }
    UNIQUE(X);
    vi DP(len(X));
    for (auto&& [l, r, t]: I1.get(0, LIM))
      if (t) {
        DP[LB(X, l)]++;
        DP[LB(X, r)]--;
      }
    for (auto&& [l, r, t]: I2.get(0, LIM))
      if (t) {
        DP[LB(X, l)]++;
        DP[LB(X, r)]--;
      }
