...
int n,start,ans,from[3001],dist[3001][3001];
vi okil,reb[3001];
bool ok,visit[3001];
...
void dfs(int now,int pred)
{
    if (ok) return;
    from[now]=pred;
    if (visit[now]) start=now, ok=true;
    if (ok) return;
    visit[now]=true;
    for (int i=0;i<reb[now].size();i++)
        if (reb[now][i]!=pred) dfs(reb[now][i],now);
}
...
void dfs(int now,int start,int len)
{
    ...
    visit[now]=true;
    for (int i=0;i<reb[now].size();i++)
        if (!visit[reb[now][i]]) dfs(reb[now][i],start,len+1);
}
...
int main()
{
    ...
    for (int i=0;i<okil.size();i++)
        write(okil[i]);
    endl;
    */

    for (int i=1;i<=n;i++)
        memset(visit,false,sizeof(visit)), dfs(i,i,0);
    for (int i=1;i<=n;i++)
    {
        ans=maxint;
        for (int j=0;j<okil.size();j++)
            ans=min(ans,dist[i][okil[j]]);
        write(ans);
    }
}