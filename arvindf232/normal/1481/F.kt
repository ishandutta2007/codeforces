...
class sqrtpacker(val all:IntArray, val want:Int){
    val s = all.size
    val taken = BooleanArray(all.size)
    var ok = false
    init{
        val count = IntArray(want+1)
        ...
        val store = Array(distinct+1){bitset(want+1)}
        store[0][0] = true
        var phase = -1
        for(i in 1..want){
            if(count[i] == 0) continue
            phase++
            val new = IntArray(want+1){1_000_000_000}
            val ava = count[i]
            val last = store[phase]
            for(p in 0..want){
                if(last[p]){
                    new[p] = 0
                }else{
            ...
            for(i in 0..want){
                store[phase+1][i] = new[i] < 1_000_000_000
            }
        }
        if(!store.last()[want]){
            ok = false
            ...
            ok = true
            var now = want
            val takensize = IntArray(want+1)
            for(step in want downTo 1){
                if(count[step] == 0){
                    continue
                }
                while(!store[phase][now]){
                    now -= step
                    takensize[step] ++
                }
                phase--
...
fun main(){
    solve.cases{
        ...
        if(box.ok){
            ...
            put(depth.maxOrNull()!! + 1)
            put(ret.map { if(it) 'a' else 'b'}.conca())
        }else{
            val degree = G.degree()
            val real = Array(n+1){mint}
            val leaves = Array(n+1){mint}
            for(i in 0 until n){
                val d = depth[i]
                val isleaf = degree[i] == 1 && i != 0
                if(isleaf){
                    leaves[d].add(i)
                }else{
                    real[d].add(i)
...
//            val fakedd = (0 until n).map { Pair(it,depth[it]) }
//            val ret = BooleanArray(n)
//            val realdd = fakedd.sortedBy { it.second }
//            var aleft = x
//            for(i in n-1 downTo 0){
//                if(aleft > 0){
//                    ret[realdd[i].first] = true
//                    aleft --
//                }else{
//                    ret[realdd[i].first] = false
//                }
//            }
            put(depth.maxOrNull()!! +2)
            put(ret.map { if(it) 'a' else 'b'}.conca())
        }
