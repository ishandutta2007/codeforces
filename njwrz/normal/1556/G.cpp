...
void dfs2(int x,ll l,ll r,ll tl,ll tr,int num){
	...
	ll mid=(l+r)>>1;
	if(tr<=mid||tl==-1)dfs2(lc[x],l,mid,tl,tr,num);
	if(tl>mid||tl==-1)dfs2(rc[x],mid+1,r,tl,tr,num);
}
...
void update(ll l,ll r){
	...
	for(auto u:nowv){
		ll l=u.F.F,r=u.F.S;
		for(int i=n-1;i>=0;i--)if((1ll<<i)>=r-l+1&&check(root,0,(1ll<<n)-1,l^(1ll<<i),r^(1ll<<i))){
