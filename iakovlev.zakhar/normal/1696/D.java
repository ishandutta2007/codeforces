...
public class D {
	...
	public int floorRoot1(long x) {
		int root = (int) Math.sqrt(x);
		while ((root + 1L) * (root + 1) <= x) {
			root++;
		}
		while (1L * root * root > x) {
			root--;
		}
		return root;
	}
	...
	public boolean check(long x) {
		int root = floorRoot1(x);
		return x - root * 1L * root <= root;
	}
	
	int counter = 0;
	
	public int find(long d) {
		++counter;
	...
	public long find(int i, int j, long d) {
		int cur = j;
		int root = floorRoot1(a[i] + d);
		if (a[i] + d > root * (root + 1L)) {
			d += (root + 1L) * (root + 1L) - a[i] - d;
		}
		while (true) {
			root = floorRoot1(a[cur] + d);
			if (a[cur] + d > root * (root + 1L)) {
				d += (root + 1L) * (root + 1L) - a[cur] - d;
				cur ^= i ^ j;
			} else {
				return d;
			}
	...
	TreeMap<Long, Integer> sq = new TreeMap<>();
	int n;
	int[] a;
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; --qwerty) {
			...
			prefMin[1] = prefMax[1] = 0;
			prefMin[0] = prefMax[0] = -1;
			sufMin[n - 2] = sufMax[n - 2] = n - 1;
			sufMin[n - 1] = sufMax[n - 1] = -1;
			for (int i = 2; i < n; i++) {
				if (a[i - 1] < a[prefMin[i - 1]]) {
					prefMin[i] = i - 1;
				} else {
					prefMin[i] = prefMin[i - 1];
				}
				if (a[i - 1] > a[prefMax[i - 1]]) {
					prefMax[i] = i - 1;
				} else {
			...
			for (int i = n - 3; i >= 0; --i) {
				if (a[i + 1] < a[sufMin[i + 1]]) {
					sufMin[i] = i + 1;
				} else {
					sufMin[i] = sufMin[i + 1];
				}
				if (a[i + 1] > a[sufMax[i + 1]]) {
					sufMax[i] = i + 1;
				} else {
					sufMax[i] = sufMax[i + 1];
				}
