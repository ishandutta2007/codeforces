...
public class TaskB {
	...
	void solve() {
		...
		int[] roadStart = new int[roadCount];
		int[] roadEnd = new int[roadCount];
		int[] startHeight = new int[roadCount];
		in.readIntArrays(roadStart, roadEnd, startHeight);
		long totalStones = 0;
		for (int i = 0; i < queryCount; i++) {
			int query = in.readInt();
			for (int j = 0; j < roadCount; j++) {
				if (roadStart[j] <= query && roadEnd[j] >= query)
					totalStones += startHeight[j] + query - roadStart[j];
			}
		}
		out.println(totalStones);
	}
