...
int main() {
    ...
    vvvmint dp(N + 1, vvmint(M + 1, vmint(2,0)));
    ...
    rep(i, N + 1)rep(j, M + 1) {
        ...
        if (i > 1 && x[i - 1] != x[i - 2])dp[i][j][0] += dp[i - 1][j][0];
        if (j > 1 && y[j - 1] != y[j - 2])dp[i][j][1] += dp[i][j - 1][1];
    ...
    rep(i, N + 1)rep(j, M + 1) {
        if (i>0&&j>0)ans += dp[i][j][0]+dp[i][j][1];
        //cout << i<<" "<<j<<" "<<dp[i][j][0].val() << " " << dp[i][j][1].val() << endl;
    }
