...
int64_t swaps(const string& a, const string& b) {
    F0R (i, 256) {
        pos[i].clear();
        used[i] = 0;
    }
    ...
    F0R (i, sz(a)) {
        char c = a[i];
        int put = pos[c][used[c]++];
        inv += bit.accumulate_prefix(N) - bit.accumulate_prefix(put);
        bit.add(put, 1);
    }
...
void solve() {
    ...
    pair<int64_t, string> res = { 0, s };

    string t = s;
    string order = "ANOT";
    do {
        sort(all(t), [&order](char a, char b) {
            return order.find(a) < order.find(b);
        });

        int64_t sw = swaps(s, t);
        if (sw > res.f) {
            res = { sw, t };
        }
