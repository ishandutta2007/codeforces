...
int fact[1000005],ifact[1000005],fv[1000005];
...
void solve() {
	...
	ifact[1000004]=powm(fact[1000004],mod-2);
	for(int i=1000003; i>=0; i--)
		ifact[i]=(ifact[i+1]*(i+1))%mod;
	fr(i,1,k+2)
		fv[i]=(fv[i-1]+powm(i,k))%mod;
	if(n<=k+2) {
		cout<<fv[n]<<endl;
		return;
	}
	...
	fr(i,1,k+2) {
		trace(fv[i]);
		int te=((fv[i]*ifact[i-1])%mod)*((ifact[k+2-i]*powm(n-i,mod-2))%mod);
		trace(te);
		if((k-i)&1)
			ans2=(ans2-te)%mod;
		else
