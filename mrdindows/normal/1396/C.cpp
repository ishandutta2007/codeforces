...
int solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (i + 2 <= n) {
            dp[i + 2] = dp[i] + d * 2 + min(r2 + r1, a[i] * r1 + r1 * 2) + min(r1 + r2, a[i + 1] * r1 + r1 * 2) + 2 * d;
        }
        if (i + 2 == n) {
            dp[i + 2] = min(dp[i + 2], dp[i] + min(r1 + r2, a[i] * r1 + r1 * 2) + d + a[n - 1] * r1 + r3 + d * 2);
        }
        dp[i + 1] = min({dp[i + 1], dp[i] + r1 * a[i] + r3 + d, dp[i] + min(r2 + r1, a[i] * r1 + r1 * 2) + 2 * d + d});
    }
//    DBN(r1, r2, r3, d);
//    printarray(a, n);
//    FI(n + 1) {
