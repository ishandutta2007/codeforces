...
int m,a[MN],id[MN]; pii b[MN]; 
vector<vi>ans; 
void solve(int x,vector<int>now) {
//	cerr<<"? "<<x<<' '<<m<<endl;
	if(x==m+1) {
//		fprint(now);
		for(auto it:now) {
	...
	if(len==1) return solve(x+1,{1,3,2}); 
	vi tmp(len*2+1);
	For(i,0,len-2) {
		tmp[i]=tmp[2*len-2-i]=now[i]; 
		i&1?tmp[i]+=len+1:tmp[2*len-2-i]+=len+1; 
	}
	tmp[len-1]=now[len-1],tmp[len*2-1]=len+1,tmp[len*2]=now[len-1]+len+1;
	solve(x+1,tmp);
}
void works() {
	m=read(); 
	For(i,1,m) a[i]=read(),b[i]=mkp(a[i],i);
	sort(b+1,b+1+m,greater<pii>()); 
	ans.clear(); 
	For(i,1,m) a[i]=b[i].first,id[b[i].second]=i; 
	if(a[1]>=4) Fail; 
	if(a[1]==3) {
		if(m>1&&a[2]>1) Fail; 
		solve(2,{2,1,3}); 
	...
	if(a[1]==2) {
		if(m>2&&a[3]>1) Fail; 
		if(m==1||a[2]==1) solve(2,{2,1}); 
		else solve(3,{8,1,7,3,5,6,4,2}); 
	}	
	if(a[1]==1) {
		solve(2,{1}); 
	}
	for(auto it:ans) {
		For(i,0,m-1) printf("%d ",it[id[i+1]-1]); 
		printf("\n"); 
	}
}
signed main() {
