...
int GetMinF(){
	bool haveOddDis = false;
	function<void(int,int,int)> Dfs = [&]( int pos , int fa , int dis ){
		if(isLeaf[pos])
			haveOddDis |= dis&1;
		for( auto v : e[pos] ){
			if( v == fa ) continue;
	...
	int anyLeaf = 0;
	For(i,n) if(isLeaf[i]){ anyLeaf = i; break; }
	Dfs(anyLeaf,0,0);
	return haveOddDis ? 3 : 1;
}

int GetMaxF(){
	static vc<int> ne[MAXN];
	For(i,n){
