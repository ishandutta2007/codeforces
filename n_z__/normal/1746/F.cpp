...
cin>>n>>q;
for(int x=0;x<sl;x++)
bas[x]=x+100000,mo[x]=mt()%100000000+bas[x]+100000000,del[x]=mt()%100000000000+100000;
for(int x=1;x<=n;x++)
{
    cin>>a[x];
    for(int y=0;y<sl;y++)
...
while(q--)
{
    ...
    if(op==2)
    {
        ...
        cin>>l>>r>>k;
        for(int x=0;x<sl;x++)
        if(sum[x].querylr(l,r)%k!=0){cout<<"NO"<<endl;nowno++;goto nxt;}
        if(nowyes*500<=nowno)
        {
            ...
            mp[a[x]]++;
            for(auto q:mp)
            if(q.second%k!=0){cout<<"NO"<<endl;nowno++;goto nxt;}
        }
        nowyes++;
        cout<<"YES"<<endl;
        nxt:;
