...
circuit parse(string inp) {
    ...
    trav (c, inp) {
        switch(c) {
            ...
            case 'P':
            case 'S':
                res.back().type = c == 'P' ? PARALLEL : SERIES;
                break;
            case '*':
                res.back().elts.eb();
                break;
            case ')':
                if (sz(res) > 1) {
                    res[sz(res) - 2].elts.push_back(move(res.back()));
                    res.pop_back();
                }
            default:
                break;
...
ll calc_coeff(circuit& c) {
    if (c.type == SERIES) {
        c.coeff = LLONG_MAX;
        trav (e, c.elts) ckmin(c.coeff, calc_coeff(e));
    }
    if (c.type == PARALLEL) {
        c.coeff = 0;
        trav (e, c.elts) c.coeff += calc_coeff(e);
    }
    return c.coeff;
}

vll output;
void assign(circuit& c, frac<ll> R) {
    if (c.type == LEAF) {
        assert(R.n % R.d == 0);
        output.pb(R.n/R.d);
        return;
    ...
    if (c.type == SERIES) {
        int mi = 0;
        FOR (i, 1, sz(c.elts)) if (c.elts[i].coeff < c.elts[mi].coeff) mi = i;
        F0R (i, sz(c.elts)) assign(c.elts[i], i != mi ? 0ll : R);
    }
    if (c.type == PARALLEL) {
        trav (e, c.elts) assign(e, R * c.coeff / e.coeff);
    }
}
...
void solve() {
    ...
    int sp = inp.find(' ');
    int R = atoi(inp.substr(0, sp).c_str());

    while (inp[sp] == ' ') sp++;
    circuit c = parse(inp.substr(sp));

    calc_coeff(c);
    assign(c, R);
    pr("REVOLTING "), pc(output);
    output.clear();
}
