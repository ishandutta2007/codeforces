...
int main(){
    ...
    vl xt;
    vl yt;
    f0r(i, m) re(go[i].f, go[i].s), xt.eb(go[i].f), yt.eb(go[i].s);
    auto dist = [&](pl a, pl b) ->ll{
        return abs(a.f-b.f)+abs(a.s-b.s);
    };
    auto jump = [&](pl a, pl b) -> ll {
        return min(abs(a.f-b.f), abs(a.s-b.s));
    };
    map<ll, vi> xs;
    map<ll, vi> ys;
    f0r(i, m) xs[go[i].f].eb(i);
    f0r(i, m) ys[go[i].s].eb(i);
    DSU D;
    Dijkstra<ll, false> paths;
    paths.init(m+7);
    D.init(m);
    //st is m
    //ed is m+1
    ...
    f0r(i, m){
        for(int j: comps[i]){
            ckmin(cdist[i], jump(go[j], st));
            ckmin(edist[i], dist(go[j], ed));
        }
        if (sz(comps[i]) != 0){
            paths.ae(m, i, cdist[i]);
            paths.ae(i, m+1, edist[i]);
        }
    }
//    ps(comps, cdist, edist);
    sort(all(xt));
    xt.erase(unique(all(xt)), xt.end());
    sort(all(yt));
    yt.erase(unique(all(yt)), yt.end());
    f0r(i, sz(xt)-1){
        paths.ae(xm[xt[i]], xm[xt[i+1]], xt[i+1]-xt[i]);
    }
    f0r(i, sz(yt)-1){
        paths.ae(ym[yt[i]], ym[yt[i+1]], yt[i+1]-yt[i]);
//        ps(ym[yt[i]], ym[yt[i+1]], yt[i+1]-yt[i]);
    }
//    ps(xt, yt);
    paths.gen(m);
    ckmin(ans, paths.dist[m+1]);
    ps(ans);
    return 0;
}