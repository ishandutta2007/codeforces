...
void solve()
{
    ...
    dp.a[0][0] = 1;
    int cover = (1 << m) - 1;
    Matrix go((k + 1) * (1 << m), (k + 1) * (1 << m));

    for (int h = 0; h <= k; h++)
    {
        for (int mask = 0; mask < (1 << m); mask++)
        {
            int nmask = (mask << 1) & cover;
            int kek = __builtin_popcount(mask) + 1;
            //dp[i][h][nmask] = (dp[i][h][nmask] + dp[i + 1][h][mask]) % MOD;
            go.a[h * (1 << m) + mask][h * (1 << m) + nmask] = 1;
            //if (h + 1 <= k)
            //    dp[i][h + 1][nmask + 1] = (dp[i][h + 1][nmask + 1] + dp[i + 1][h][mask] * kek) % MOD;
            if (h + 1 <= k)
                go.a[h * (1 << m) + mask][(h + 1) * (1 << m) + nmask + 1] = kek;
        }
    }
    go = binpow(go, n);
    dp = dp * go;
