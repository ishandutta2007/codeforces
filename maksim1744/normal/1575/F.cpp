...
int main() {
    ...
    for (int ci = 1; ci < n; ++ci) {
        Mint A = (-Mint(k) * ci - n * 2 + 2 * ci + n) * f[ci];
        Mint B = Mint(ci) * (k - 1) * f[ci - 1];
        Mint C = n - ci;
        // A + B + C*x = -n
        f.pb((-n - A - B) / C);
    }
    ...
    for (int i = 0; i <= c1; ++i)
        prob[i] = C(c1, i) * pow(Mint(k - 1), c1 - i) / pow(Mint(k), c1);
    ...
    map<int, Mint> mem;
    auto calc = [&](int cnt) {
        if (mem.count(cnt)) return mem[cnt];
        Mint res = 0;
    ...
    Mint target = f[n] + Mint(k - 1) * f[0];
    Mint res = 0;
    for (auto [_, cnt] : mm) {
        res += calc(cnt);
    }
    res += calc(0) * (k - mm.size());

    cout << res - target << '\n';

    // show(f);

    // auto calc = y_combinator([&](auto calc, vector<int> a) -> Mint {
    //     int ind = find(a.begin(), a.end(), -1) - a.begin();
    //     if (ind == a.size()) {
    //         map<int, int> mc;
    //         for (int x : a)
    //             mc[x]++;
