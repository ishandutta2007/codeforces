...
int n;
int64_t a[200001];
int64_t get_previous(int u, int md) {
    u--;
    if ((u + md) % 2) u--;
    if (u >= 0) return a[u];
    else return 0;
}
int64_t get_this_or_previous(int u, int md) {
    if ((u + md) % 2) u--;
    if (u >= 0) return a[u];
    else return 0;
}
void solve() {
    ...
    // only reverse when r % 2 != l % 2
    // reverse l->r then answer is
    // get_previous(l, 0) + get_this_or_previous(r, 1) - get_previous(l, 1) +
    // get_this_or_previous(n, 0) - get_this_or_previous(r, 0)
    n--;
    int64_t ans = get_this_or_previous(n, 0);
    int64_t max_val[2];
    max_val[0] = -1e18;
    max_val[1] = -1e18;
    for (int i = 0; i <= n; i++) {
        int64_t r_val = get_this_or_previous(i, 1) - get_this_or_previous(i, 0);
        ans = max(ans, r_val + max_val[1 - i % 2] + get_this_or_previous(n, 0));

        int64_t l_val = get_previous(i, 0) - get_previous(i, 1);
        max_val[i % 2] = max(max_val[i % 2], l_val);
    }
    cout << ans << '\n';
}
