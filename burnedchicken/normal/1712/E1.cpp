...
void orzck(){
    rep2(i,1,maxn) for(int j=i; j<maxn; j+=i) factor[j].pb(i);
    int q;
    cin >> q;
    ...
    int cur=0;
    rep3(i,maxn-3,1){
        for(int j=i; j<maxn; j+=i) bit.modify(j,max(cnt[j]-1,0ll)),cnt[j]++;
        if(i%3==0){
            if(i*2<maxn) bit.modify(i*2,1);
            if(i%2==0&&i*5/2<maxn) bit.modify(i*5/2,1);
        }
        while(cur<q&&arr[cur][0]==i) ans[arr[cur][2]]-=bit.query(0,arr[cur][1]),cur++;
    }
    rep(q) print(ans[i]);
}
