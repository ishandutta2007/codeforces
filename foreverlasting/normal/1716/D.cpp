...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		f[0][0]=1;
		int cur=0;
		for(int t=1;s<=n;s+=k+t-1,t++){
			int p=k+t-1;
			for(int i=s;i<=n&&i<s+p;i++){
				g[i%p]=f[cur][i],ans[i]+=f[cur][i],f[cur][i]=0;
			}
			for(int i=s+p;i<=n;i++){
				f[cur^1][i]=g[i%p],g[i%p]+=f[cur][i],ans[i]+=f[cur][i],f[cur][i]=0;
			}
			cur^=1;
		}
		for(int i=0;i<=n;i++)ans[i]+=f[cur][i];
