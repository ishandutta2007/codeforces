...
int main() {
    ...
    int n;
    re(n);
    array<union_find, 2> uf{union_find{n}, {n}};
    ...
    vvi components_index(n);
    for (int i = 0; i < n; i++) {
        components_index[target.find(i)].push_back(i);
    }

    vi comps;
    for (auto i = 0; i < n; i++) {
        if (!components_index[i].empty())
            comps.push_back(i);
    }

    auto attach = [&](int u, int v) -> bool {
        if (uf[0].find(u) == uf[0].find(v))
            return false;
        if (uf[1].find(u) == uf[1].find(v))
            return false;
        uf[0].unite(u, v);
        uf[1].unite(u, v);
        ans.push_back({u, v});
    ...
    while (comps.size() != 1) {
        shuffle(all(comps), rng);
        vi next_comps;
        for (int i = 0; i + 1 < sz(comps); i += 2) {
            auto cur = comps[i];
            auto next = comps[i + 1];
            ...
            for (int j : components_index[cur])
                first[other.find(j)] = j;
            for (int j : components_index[next])
                second[other.find(j)] = j;
            pd(first, second);

            vpii fc(all(first)), sc(all(second));
            pd(sz(first), sz(second));
            for (int a = 0; a < 2 && a < sz(fc); a++) {
                for (int b = 0; b < 2 && b < sz(sc); b++) {
                    if (attach(fc[a].second, sc[b].second))
                        goto merge;
                }
            }
            next_comps.push_back(cur);
            next_comps.push_back(next);
            continue;

        merge:;
            if (components_index[cur].size() < components_index[next].size())
                swap(cur, next);
            components_index[cur].insert(components_index[cur].end(), all(components_index[next]));
            next_comps.push_back(cur);
        }
        if (sz(comps) % 2 == 1)
            next_comps.push_back(comps.back());

        swap(next_comps, comps);
        pd(comps);
    }
