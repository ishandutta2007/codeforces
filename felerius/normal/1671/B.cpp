// Consecutive Points Segment (https://codeforces.com/contest/1671/problem/B)
...
int main() {
    ...
    while (t--) {
        ...
        array base = {x[0] - 1, x[0], x[0] + 1};
        array pos = {true, true, true};
        rep(i, n)
            rep(j, 3)
                pos[j] &= abs(base[j] + i - x[i]) < 2;

        println(pos[0] || pos[1] || pos[2] ? "YES" : "NO");
    }
}