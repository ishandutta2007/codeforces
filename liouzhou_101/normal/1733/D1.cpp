...
void solve()
{
	...
	if (cnt % 2) return writeln(-1);

	vector<int> p;
	for (int i = 0; i < n; ++i)
		if (s[i]) p.push_back(i);
	int m = p.size();
	if (m == 0) return writeln(0);

	vector dp(m, vector<ll>(m));
	for (int i = m - 1; i >= 0; --i)
		for (int j = i + 1; j < m; ++j)
		{
			...
			if (j == i + 1)
			{
				if (p[i] + 1 == p[j])
					...
					dp[i][j] = min(y, (p[j] - p[i]) * x);
				continue;
			}
			dp[i][j] = (j - i + 1) / 2 * y;
			freshmin(dp[i][j], dp[i + 1][j - 1] + y);
			freshmin(dp[i][j], dp[i][i + 1] + dp[i + 2][j]);
			freshmin(dp[i][j], dp[i][j - 2] + dp[j - 1][j]);
		}
	writeln(dp[0][m - 1]);
}
