...
fn run() {
    ...
    for i in 0..h {
        for j in 0..w {
            ...
            if i + 1 < h && s[i + 1][j] == b'#' {
                g.add_edge(pos(i, j), pos(i + 1, j), 1);
            }
            if j + 1 < w && s[i][j + 1] == b'#' {
                g.add_edge(pos(i, j + 1), pos(i, j), 1);
            }
            if i == 0 || s[i - 1][j] == b'.' {
                g.add_edge(src, pos(i, j), 1);
            }
            if j == 0 || s[i][j - 1] == b'.' {
                g.add_edge(pos(i, j), dst, 1);
            }
        }
    }
    let ans = g.flow(src, dst);
    println!("{}", ans);
}
