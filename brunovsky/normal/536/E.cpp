...
struct Segnode {
    ...
    static inline vector<int> f;
    int length = 0, prefix = 0, suffix = 0, sum = 0;

    Segnode() = default;
    Segnode(int v) : length(1), prefix(v), suffix(v) {}

    bool full() const { return length == prefix && length == suffix; }

    void reverse() { swap(prefix, suffix); }

    int value() const { return full() ? f[length] : sum + f[prefix] + f[suffix]; }
    ...
    void pushup(const Segnode& lhs, const Segnode& rhs) {
        ...
        if (lhs.full() && rhs.full()) {
            ...
            prefix = lhs.length + rhs.prefix;
            suffix = rhs.suffix;
        } else if (rhs.full()) {
            prefix = lhs.prefix;
            suffix = lhs.suffix + rhs.length;
        } else {
            prefix = lhs.prefix;
            suffix = rhs.suffix;
            sum += f[lhs.suffix + rhs.prefix];
        }
    }

    void apply(int set) {
        assert(length == 1 && sum == 0 && prefix == 1 && suffix == 1);
        prefix = suffix = set;
    }

    static auto combine(const Segnode& a, const Segnode& b) {
        Segnode c;
        c.pushup(a, b);
        return c;
    }
...
int main() {
    ...
    map<array<int, 2>, int> edge_index;
    for (int u = 1; u < N; u++) {
        int v = hld.parent[u];
        edge_index[{u, v}] = edge_index[{v, u}] = hld.time[u];
    }

    vector<array<int, 4>> queries(Q);
    for (int q = 0; q < Q; q++) {
        auto& [l, u, v, i] = queries[q];
        cin >> u >> v >> l, u--, v--, i = q;
    }
    ...
    for (int q = 0, e = 0; q < Q; q++) {
        auto [l, u, v, qi] = queries[q];
        while (e < M && graph[e][0] < l) {
            auto [_, a, b] = graph[e++];
            seg.update_point(edge_index.at({a, b}), 0);
        }
        vector<Segnode> path;
        auto [up, down] = hld.oriented_edge_segments(u, v);
        for (auto [l, r] : up) {
            auto node = seg.query_range(l, r);
            node.reverse();
            path.push_back(node);
        }
        for (auto [l, r] : down) {
        ...
        int P = path.size();
        for (int i = 1; i < P; i++) {
            agg = Segnode::combine(agg, path[i]);
        }
        ans[qi] = agg.value();
    }
