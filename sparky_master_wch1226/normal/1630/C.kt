...
fun main(){
    solve.cases{
        ...
        val lastoccurence = IntArray(n+1){-1}
        for((pos,i) in L.withIndex()){
            counts[i] ++
            lastoccurence[i] = pos
        }
        var current:Int? = null
        var currentBest:Int? = null
        var rightBestEnd:Int? = null
        var off = 0
        for(i in 0 until n){
            if(counts[L[i]] == 1){
                if(current == null) off++
            ...
            if(current == null){
                current = L[i]
                rightBestEnd = lastoccurence[L[i]]
                off++
            }else if(i == lastoccurence[current]){
                if(currentBest != null && rightBestEnd != null && rightBestEnd != i){
                    current = currentBest
                    currentBest = null
                    off++
                }else{
                ...
                if(rightBestEnd == null || lastoccurence[L[i]] > rightBestEnd){
                    rightBestEnd = lastoccurence[L[i]]
                    currentBest = L[i]
                }
            }
