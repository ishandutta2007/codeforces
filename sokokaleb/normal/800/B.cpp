...
int main(int argc, char** argv) {
	...
	vc.pb(vc[1]);
	FORN (i, 1, n) {
		ll ax = vc[i + 1].ff - vc[i - 1].ff,
		   ay = vc[i + 1].ss - vc[i - 1].ss,
		   bx = vc[i].ff - vc[i - 1].ff,
		   by = vc[i].ss - vc[i - 1].ss;
		double ang = atan2(ax * by - ay * bx, ax * bx + ay * by);
		double jarak = abs(sin(ang)) * hypot(bx, by);
		vec.pb(jarak);
		vec.pb(hypot(bx, by));
	}
	cout << fixed << setprecision(12) << (*min_element(ALL(vec)) / 2.0) << "\n";
}