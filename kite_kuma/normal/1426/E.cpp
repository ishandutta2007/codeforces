...
int solve(int a, int b, int c, int d, int e, int f, int n) {
	int k = min(a, f);
	a -= k;
	f -= k;
...
int main() {
	...
	INT(n);
	INT(ag, ac, ap, bg, bc, bp);

	int mini, maxi;

	maxi = min(ag, bc) + min(ac, bp) + min(ap, bg);

	mini = n;

	chmin(mini, solve(ag, ac, ap, bc, bp, bg, n));
	chmin(mini, solve(ag, ap, ac, bc, bg, bp, n));
	chmin(mini, solve(ac, ap, ag, bp, bg, bc, n));
