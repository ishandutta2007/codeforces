...
int main() {
    ...
    VEC(string, s, n);
    map<pii, int> mp;
    rep(i, n) { rep2(j, 1, si(s[i])) mp[pii(i, j)] = si(mp); }

    int N = si(mp);
    Matrix<mint> mat(si(mp) * 5);

    rep(i, n) rep2(j, 1, si(s[i])) {
        string t = s[i].substr(si(s[i]) - j, j);
        int now = mp[pii(i, j)];
        rep(k, n) {
            if(si(s[k]) >= j) {
                if(s[k].substr(0, j) == t) {
                    if(si(s[k]) == j) {
                        rep(p, n) mat[mp[pii(p, si(s[p]))] + N * (j - 1)][now] += 1;
                    } else {
                        mat[mp[pii(k, si(s[k]) - j)] + N * (j - 1)][now] += 1;
                    }
                }
            } else if(t.substr(0, si(s[k])) == s[k]) {
                mat[mp[pii(i, j - si(s[k]))] + N * (si(s[k]) - 1)][now] += 1;
            }
        }
    }
    ...
    rep(i, n) rep2(j, 1, si(s[i])) {
        int now = mp[pii(i, j)];
        rep2(k, 1, 4) { mat[now + N * (k - 1)][now + N * k] += 1; }
    }
    ...
    // cout << mat << endl;
    Matrix<mint> v(N * 5, 1);
    rep(i, n) v[mp[pii(i, si(s[i]))]][0] = 1;
    auto f = mat * v;
    mint ans;
    rep(i, n) ans += f[mp[pii(i, si(s[i]))]][0];
    cout << ans / n << endl;
}