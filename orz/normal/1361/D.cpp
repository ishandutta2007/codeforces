...
int another_main()
{
	...
	if (k >= 2 * except - 1)
	{
		for (I = 0; I < sz(g); ++I)
		{
			...
			{
				int right = k / 2;
				int left = k - except - right;
				for (int i = 0; i < left; ++i)
					ans1 += sqrtl(g[I][i]) * (k - 1 - 2 * (right + left - i - 1));
				for (int i = 0; i < right; ++i)
					ans1 += sqrtl(g[I][sz(g[I]) - 1 - i]) * (k - 1 - 2 * i);
			}
			else
			{
				for (int i = 0; i < sz(g[I]); ++i)
					ans1 += sqrtl(g[I][sz(g[I]) - 1 - i]) * (k - 1 - 2 * i);
			}
		}
	}
	...
	for (I = 0; I < sz(g); ++I)
		for (int i = 0; i < sz(g[I]); ++i)
			v.pb(sqrtl(g[I][sz(g[I]) - 1 - i]) * (k - 1 - 2 * i));
	sort(whole(v));
	for (int i = n - 1; i >= n - k; --i)
		ans += v[i];
	cout << setprecision(30) << fixed << max(ans, ans1) << '\n';
	return 0;
}
