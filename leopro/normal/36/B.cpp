...
template<typename VVC>
void fill(VVC &result, VVC &pattern, VVC &black, int size, int n, int x = 0, int y = 0) {
    if (size == 1) {
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                result[i + x][j + y] = pattern[i][j];
            }
        }
        return;
    }
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            if (pattern[i][j] == '*') fill(result, black, black, size / n, n, x + i * size, y + j * size);
            if (pattern[i][j] == '.') fill(result, pattern, black, size / n, n, x + i * size, y + j * size);
        }
    }
}
...
void solve() {
    ...
    cin >> pattern;
    vector<vector<char>> black(n, vector<char>(n, '*'));
    vector<vector<char>> result(size, vector<char>(size, 'x'));
    fill(result, pattern, black, size / n, n);
    answer(result);
}