...
int main() {
  ...
  while (t--) {
    ...
    REP(_, n - 1) {
      ...
      const int g = gcd(x, y);
      x /= g; y /= g;
      graph[i].emplace_back(j, x, y);
      graph[j].emplace_back(i, y, x);
    }
    ModInt ans = 0;
    vector<int> l(n + 1, 0), cur(n + 1, 0);
    auto f = [&](auto&& f, int par, int ver, ModInt rat) -> void {
      ans += rat;
      for (const auto [dst, x, y] : graph[ver]) {
        if (dst == par) continue;
        const auto xs = osa_k.query(x), ys = osa_k.query(y);
        for (const auto [p, ex] : ys) cur[p] -= ex;
        for (const auto [p, ex] : xs) {
          cur[p] += ex;
          chmax(l[p], cur[p]);
        }
        f(f, ver, dst, rat * y / x);
        for (const auto [p, ex] : ys) cur[p] += ex;
        for (const auto [p, ex] : xs) cur[p] -= ex;
      }
    };
    f(f, -1, 0, 1);
    FOR(i, 1, n + 1) {
      if (l[i]) ans *= ModInt(i).pow(l[i]);
    }
    cout << ans << '\n';
  }
  return 0;
