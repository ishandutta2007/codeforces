...
void main(int) {
  ...
  string s;
  scan(s);
  if (count(all(s), '1') * m % n) { return println(-1); }
  int x = int(count(all(s), '1') * m / n);
  DUMP(x);
  vector<int> suff(n + 1);
  for (int i : rev(rep(n))) { suff[i] = (s[i] == '1') + suff[i + 1]; }
  for (int i : rep1(0, n - m)) {
    if (suff[i] - suff[i + m] == x) {
      println(1);
      println(i + 1, i + m);
      return;
    }
  ...
  println(2);
  for (int i : rep1(0, m)) {
    if (suff[0] - suff[i] + suff[i + (n - m)] - suff[n] == x) {
      println(1, i);
      println(i + (n - m) + 1, n);
      break;
    }
