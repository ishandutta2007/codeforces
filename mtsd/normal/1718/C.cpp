...
public:
    ...
    T query(int a,int b){
        ...
        a += n, b += n;
        while(a != b){
            if(a & 1) res1 = max(res1, node[a++]);
            if(b & 1) res2 = max(node[--b], res2);
            a >>= 1, b >>= 1;
        }
...
int main(){
    ...
    rep(zz,T){
        ...
        rep(i,q){
            ...
            rep(z,m){
                P[z][p%d[z]] += (x-a[p])*d[z];
                seg[z].update(p%d[z],P[z][p%d[z]]);
                chmax(res,seg[z].query(0,d[z]));
            }
            a[p] = x;
            cout << res << "\n";
        }
