...
template<typename _Tp>inline poly operator / (poly a,const _Tp &b){return a/=b;}
int a[N];
struct comp{inline bool operator ()(const poly &a,const poly &b)const{return a.size()>b.size();};};
std::priority_queue<poly,std::vector<poly>,comp> q;
int main(){
	setup();
	int n;read(n);
