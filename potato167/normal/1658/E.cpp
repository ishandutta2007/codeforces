...
void solve(){
	int n,k;
	cin>>n>>k;
	segment_tree<F,op,e> seg1(n*2),seg2(n*2);
	vector<vector<char>> ans(n,vector<char>(n));
	vector<tuple<int,int,int>> p;
	rep(i,n) rep(j,n){
		int a;
		cin>>a;
		p.push_back({a,i+j,i-j+n});
	}
	Sore(p);
	int S=0;
	rep(i,n*n){
		...
		tie(a,x,y)=p[i];
		//cout<<a<<" "<<x<<" "<<y<<endl;
		if(op(seg1.query(max(0,x-k),min(2*n,x+k+1)),seg2.query(max(0,y-k),min(2*n,y+k+1)))==S*2){
			ans[(x+y-n)/2][(x-y+n)/2]='M';
			seg1.addl(x,1);
			seg2.addl(y,1);
			S++;
		}else{
			ans[(x+y-n)/2][(x-y+n)/2]='G';
		}
	}
	rep(i,n){
