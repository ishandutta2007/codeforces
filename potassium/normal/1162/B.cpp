...
int main() {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (matrix[0][i][j] > matrix[1][i][j]) swap(matrix[0][i][j],matrix[1][i][j]);
		}
	}
	...
	for (int k = 0; k < 2; k++) {
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m-1; j++)
				if (matrix[k][i][j+1] <= matrix[k][i][j]) isPossible = 0;
		
		for (int j = 0; j < m; j++)
			for (int i = 0; i < n-1; i++)
			if (matrix[k][i+1][j] <= matrix[k][i][j]) isPossible = 0;
	}
	
	if (isPossible) W("Possible"); else W("Impossible");
