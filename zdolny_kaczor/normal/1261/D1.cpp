...
const int mod = 998244353, nax = 2e5 + 44;
ll powe(ll b, ll e) {
	assert(e >= 0);
	ll r = 1;
	while (e) {
...
int a[nax];
ll k2pows[nax], kpows[nax];
void run(ll tab[], ll b) {
	tab[0] = 1;
	for (int i = 1; i < nax; ++i) tab[i] = tab[i - 1] * b % mod;
}
int main() {
	fac[0] = 1;
	...
	debug(imie(diffs));
	ll ans = 0;
	for (int good = 0; good <= diffs; ++good)
		for (int bad = 0; bad < good && bad + good <= diffs; ++bad) {
			debug(imie(good), imie(bad));
			ans += cho(diffs, good) * cho(diffs - good, bad) % mod * k2pows[diffs - good - bad] % mod * kpows[n - diffs] % mod;
		}
	ans %= mod;
	if (ans < 0) ans += mod;
	printf("%lld\n", ans);
