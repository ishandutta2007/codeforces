    ...
    while (t --)
    {
        ...
        if (sz(bad_boy) == 1) {
            if (ans[bad_boy[0]] != 0) {
                ...
                for (int i = 1; i <= n; ++i) {
                    if (ans[i] == v[bad_boy[0]]) {
                        ans[i] = bad_boy[0];
                        ans[bad_boy[0]] = v[bad_boy[0]];
                        bad_boy.clear();
                        break;
                    }
                }
        ...
        if (sz(bad_boy)) {
            ...
            vector<int> givers(sz(bad_boy));
            assert(sz(to_give) == sz(bad_boy));
            for (int i = 0; i < sz(bad_boy); ++i) {
                int boy = bad_boy[i];
                if (to_give.find(boy) != to_give.end()) {
                    to_give.erase(boy);
                    givers[i] = boy;
                }
            }
            for (int i = 0; i < sz(bad_boy); ++i) {
                if (givers[i] == 0) {
                    givers[i] = *to_give.begin();
                    to_give.erase(to_give.begin());
                }
            }
            rotate(givers.begin(), givers.begin() + 1, givers.end());
            for (int i = 0; i < sz(bad_boy); ++i) {
                ans[givers[i]] = bad_boy[i];
            }
        }
        cout << granted << endl;
        for (int i = 1; i <= n; ++i) {
