...
int main() {
    ...
    while (true) {
        int count_remaining = accumulate(remaining.begin(), remaining.end(), 0);
        ...
        for (int i = 0; i < N; i++)
            if (remaining[i]) {
                ...
                compressed[i] = c_index++;
            }

        LCA lca(count_remaining);

        for (pair<int, int> &edge : edges)
            if (remaining[edge.first] && remaining[edge.second])
                lca.add_edge(compressed[edge.first], compressed[edge.second]);

        pair<int, int> diameter = lca.get_diameter().second;
        dbg(diameter);
        cout << "? " << node_list[diameter.first] + 1 << ' ' << node_list[diameter.second] + 1 << endl;
        lca.build();
        int answer;
        cin >> answer;
        ...
        assert(answer >= 0);
        int dist = lca.get_dist(diameter.first, answer);
        int subtree = lca.get_kth_node_on_path(diameter.first, diameter.second, dist);

        for (int i = 0; i < N; i++)
            if (remaining[i]) {
                dbg(i + 1, lca.on_path(subtree, compressed[i], diameter.first), lca.on_path(subtree, compressed[i], diameter.second));
                remaining[i] = lca.on_path(subtree, compressed[i], diameter.first) && lca.on_path(subtree, compressed[i], diameter.second);
            }
