...
void dfs(int x, int y = 0) {
	// if (y == 0) debug("START OVER!!!!!!!!!!!!!!!", imie(x));
	size[x] = 1;
	...
	int rest = n - size[x];
	ll t1 = size[x] * (size[x] - 1ll) / 2;
	// debug(arr(size, x), imie(t1));
	for (int v : graf[x]) if (v != y)
		t1 -= size[v] * (size[v] - 1ll) / 2;
	// debug(imie(t1));
	t1 *= rest * (rest - 1ll) / 2;
	// debug(imie(t1));
	ans += t1 * 2;
	ll c1 = 0, c2 = 0;
	for (int v : graf[x]) if (v != y) {
		c1 += size[v] * (size[v] - 1ll) / 2;
		c2 += K(size[v] * (size[v] - 1ll) / 2);
	}
	// debug(2 * (imie(c1) * imie(c1) - imie(c2)));
	ans -= 1 * (c1 * c1 - c2);
}
int main() {
	scanf("%d", &n);
