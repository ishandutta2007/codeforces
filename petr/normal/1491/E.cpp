...
class EFibTree {
 public:
  ...
  multiset<Result>* merge(multiset<Result>* a, multiset<Result>* b) {
    if (a->size() > b->size()) {
      for (Result x : *b) {
        a->insert(x);
      }
  ...
  Result doit(int root, int skip) {
    Result r;
    r.root = 1;
    r.children = new multiset<Result>;
    for (int x : adj[root]) if (x != skip) {
      r.children->insert(doit(x, root));
    }
    while (!r.children->empty()) {
      Result first = *r.children->begin();
      if (abs(r.root - first.root) == 1 || (r.root == 1 && first.root == 1)) {
        r.children->erase(r.children->begin());
        r.root = max(r.root, first.root) + 1;
        r.children = merge(r.children, first.children);
      } else {
        break;
      }
    }
