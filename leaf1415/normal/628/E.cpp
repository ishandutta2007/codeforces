...
ll h, w;
char c[3005][3005];
ll l[3005][3005], r[3005][3005], d[3005][3005];
BIT bit(6005);
vector<ll> vec[6005];

ll calc(ll x, ll y)
{
	vector<ll> lvec, rvec, dvec;
	while(x <= w && y >= 1){
		lvec.push_back(l[x][y]);
		rvec.push_back(r[x][y]);
		dvec.push_back(d[x][y]);
		x++, y--;
	}
	reps(i, lvec) lvec[i] -= i;
	...
	bit.init();
	rep(i, 0, 6000) vec[i].clear();
	
	reps(i, lvec){
		if(lvec[i] > 0) bit.add(i, 1);
		else vec[-lvec[i]].push_back(i);
	}
	
	ll ret = 0;
	reps(i, lvec){
		//outl(i);
		ll x = min(rvec[i], dvec[i]);
		ret += bit.query(i, i+x-1);
...
int main(void)
{
	...
	rep(y, 1, h){
		rep(x, 1, w){
			if(c[x][y] == 'z') l[x][y] = l[x-1][y] + 1;
			else l[x][y] = 0;
		}
		per(x, w, 1){
			if(c[x][y] == 'z') r[x][y] = r[x+1][y] + 1;
			else r[x][y] = 0;
		}
		per(x, w, 1){
			if(c[x][y] == 'z') d[x][y] = d[x+1][y-1] + 1;
			else d[x][y] = 0;
		}
	}
	
	ll ans = 0;
	rep(y, 1, h) ans += calc(1, y);
	rep(x, 2, w) ans += calc(x, h);
	outl(ans);
