...
bool judge(vector<int> &vec) {
	...
	SegmentTree<ll> RMQ(n, 0, f, update);
	REP(i, n) { RMQ.set(i, vec[i]); }
	V<> pos(n);
	REP(i, n) { pos[vec[i]] = i; }

	REP(i, n - 1) {
		if(pos[i + 1] < pos[i]) {
			if(RMQ.query(pos[i + 1], pos[i]) > i + 1) return false;
		}
	}
	ADD(vec, 1);
	return true;
...
void main_() {
	...
	SUB(p);
	V<> exi(n);
	for(auto &v : p) exi[v] = 1;
	int now = 0, M = *max_element(all(p));
	stack<int> st;
	REP(i, M + 1) {
		if(exi[i]) {
			while(!st.empty()) {
