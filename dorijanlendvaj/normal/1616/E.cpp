...
int main()
{
	...
	while (t--)
	{
		cin>>n>>a>>b;
		ll an=LLINF,cs=0;
		for (int i='a';i<='z';++i) gd[i].clear();
		for (int i=n-1;i>=0;--i) gd[a[i]].pb(i),upd(i,0);
		for (int i=0;i<n;++i)
		{
			for (int j='a';j<b[i];++j) if (gd[j].size())
			{
				//cout<<i<<' '<<char(j)<<' '<<gd[j].back()<<' '<<ge(0,gd[j].back())<<en;
				an=min(an,cs+gd[j].back()-ge(0,gd[j].back()));
			}
			if (gd[b[i]].size())
			{
				cs+=gd[b[i]].back()-ge(0,gd[b[i]].back());
				upd(gd[b[i]].back(),1);
				gd[b[i]].pop_back();
			}
			else break;
		}
		if (an==LLINF) cout<<-1<<en;
		else cout<<an<<en;
	}
}