...
int main() {
    TEST {
        ...
        // RMT<int> rmt(vi(n, -1));
        // fore(e, v) rep(i, 1, si(e)) rmt.set(e[i], e[i]);

        int R = 0;
        lazy_segtree<int, f, e, int, f, f, e> seg(n);
        // vector<LS> seg(si(v));
        // rep(i, si(v)) seg[i] = LS(si(v[i]));

        // LS seg(vi(n));
        bool flag = false;
        rep(l, n) {
            fore(r, x[l]) { chmax(R, r); }
            if(R <= l) continue;
            int k = lb(v[a[l]], R) - 1;
            if(k == si(v[a[l]])) continue;
            int t = v[a[l]][k];
            if(t == l) continue;
            flag = true;
            int p = lb(v[a[l]], l);
            ...
            int q2 = v[a[l]][k - 1];
            dump(l, l2, q, q2);
            seg.apply(0, l + 1, q2 + 1);
            seg.apply(l + 1, l2 + 1, q + 1);
            seg.apply(l2 + 1, n, inf<int>);
        }
        if(flag) {
            int mi = inf<int>;
            rep(i, n) chmin(mi, seg[i] - i);
            OUT(mi);
        } else
            OUT(0);
    }
