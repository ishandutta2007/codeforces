...
void dfs(const int& v, const vvi& g, const vvpll& mem, vb& nv, vl& ans, const int& h, vl& quest, ll sum)
{
	nv[v] = false;
	sum += quest[h];
	for (int i = 0; i < sz(mem[v]); ++i)
	{
		sum += mem[v][i].X;
		if (mem[v][i].Y + h + 1 < sz(quest))
			quest[mem[v][i].Y + h + 1] -= mem[v][i].X;
	}
	ans[v] = sum;
	for (int i = 0; i < sz(g[v]); ++i)
		if (nv[g[v][i]])
			dfs(g[v][i], g, mem, nv, ans, h + 1, quest, sum);
	for (int i = 0; i < sz(mem[v]); ++i)
	{
		if (mem[v][i].Y + h + 1 < sz(quest))
			quest[mem[v][i].Y + h + 1] += mem[v][i].X;
