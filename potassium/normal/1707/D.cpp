...
int main() {
	...
	vector<vector<Mint>> dp(n + 1, vector<Mint>(n + 1, Mint(0, p)));
	function<void(int, int)> dfs = [&](int u, int p) {
		...
		if (u != 1) {
			const int sz = (int)ch.size();
			if (sz == 0) {
				for (int maxv = 0; maxv <= n; maxv++)
					dp[u][maxv] = ONE;
			}
			else {
				vector<vector<Mint>> ps_low(sz, vector<Mint>(n + 1, Mint(0, p)));
				for (int root = 0; root <= n; root++) {
					vector<Mint> low;
					for (auto& v : ch)
						low.emplace_back(dp[v][root]);

					vector<Mint> ps = low, ss = low;
					for (int i = 1; i < sz; i++)
						ps[i] *= ps[i - 1];
					...
					for (int i = 0; i < sz; i++)
						ps_low[i][root] = (i - 1 >= 0 ? ps[i - 1] : ONE) * (i + 1 < sz ? ss[i + 1] : ONE);
				...
				for (int i = 0; i < sz; i++)
					for (int root = 1; root <= n; root++)
						ps_low[i][root] += ps_low[i][root - 1];

				for (int i = 0; i < sz; i++) {
					int v = ch[i];
					for (int maxv = 1; maxv <= n; maxv++)
						dp[u][maxv] += (dp[v][maxv] - dp[v][maxv - 1]) * ps_low[i][maxv - 1];
				}
			}
			for (int maxv = 1; maxv <= n; maxv++)
				dp[u][maxv] += dp[u][maxv - 1];
		}
		else {
			for (int maxv = 1; maxv <= n - 1; maxv++)
				dp[u][maxv] = ONE;
	...
	vector<Mint> fac(n + 7, ONE), ifac(n + 7, ONE);
	for (int i = 2; i <= n; i++) fac[i] = fac[i - 1] * i;
	ifac[n] = fac[n].inv();
	for (int i = n; i >= 3; i--) ifac[i - 1] = ifac[i] * i;
