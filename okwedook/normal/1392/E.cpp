...
signed main() {
    ...
    while (q--) {
        ...
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                if (need & a[i][j]) {
                    if (x == i && j - y > 1) continue;
                    if (x < i && a[x][y] > 0) ++x, st.pb({x, y}), need -= a[x][y];
                    while (y < j) ++y, st.pb({x, y}), need -= a[x][y];
                    while (x < i) ++x, st.pb({x, y}), need -= a[x][y];
                }
        while (x < n - 1) ++x, st.pb({x, y});
        while (y < n - 1) ++y, st.pb({x, y});
        for (auto i : st) println(i.f + 1, ' ', i.s + 1);
        cout << flush;
    }
    #ifdef DEBUG
