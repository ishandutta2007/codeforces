...
{
    if (l==r) return vecs[l];
    else
    {
        int mid = (l+r)/2;
        return ntt.product(ans(l, mid), ans(mid+1, r));
    }
}
...
int main()
{
    ...
    auto vec = ans(0, vecs.size()-1);

    ll res = 0;
    for (int i = 0; i<vec.size(); i++)
    {
        if (i%2==0) res = add(res, mul(facs[n-i], vec[i]));
        else res = sub(res, mul(facs[n-i], vec[i]));
    }

    cout<<res;

    //Need to multiply (1+xcnt[i]) now
}