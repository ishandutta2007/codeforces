...
vector<int> get_stirling()
{
	...
	for(int i = 0; i <= n; i++)
	{
		int cur = invfact[i] * pow(i, n, MOD);
		cur %= MOD;
		b.push_back(cur);
	}
	vector<int> v = FFT::mod_multiply(a, b, MOD, 1);
	return v;
}
...
int32_t main()
{
	...
	vector<int> stirling = get_stirling();
	int ans = nCr(n, n - k) * stirling[n - k];
	ans %= MOD;
	if(k != 0)
	{
		ans *= 2;
