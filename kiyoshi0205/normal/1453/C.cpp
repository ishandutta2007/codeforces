...
void solve(vector<string>& g){
  debug(g);
  ll i,j,d;
  mat mi(10,vec(N,inf)),ma(10,vec(N,-inf));
  vec firstindex(10,inf),lastindex(10,-inf);
  rep(i,N){
    rep(j,N){
      d=g[i][j]-'0';
      chmin(mi[d][i],j);
      chmax(ma[d][i],j);
      chmin(firstindex[d],i);
      chmax(lastindex[d],i);
    }
    rep(d,10){
      if(mi[d][i]>ma[d][i])continue;
      chmax(ans[d],(ma[d][i]-mi[d][i])*max(i,N-1-i));
      if(firstindex[d]<i){
        chmax(ans[d],(i-firstindex[d])*max(ma[d][i],N-1-mi[d][i]));
      }
    }
  }
  brep(i,N){
    rep(d,10){
      if(mi[d][i]>ma[d][i])continue;
      if(lastindex[d]>i){
        chmax(ans[d],(lastindex[d]-i)*max(ma[d][i],N-1-mi[d][i]));
      }
    }
  }
}
