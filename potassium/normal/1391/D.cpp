...
int main() {
	...
	const int FULL = 1 << n;
	const int INF = 1e9;
	vector<vector<int>> dp(m, vector<int>(FULL, INF));
	
	for (int mask = 0; mask < FULL; mask++)
		dp[0][mask] = __builtin_popcount(mask ^ v[0]);
		
	vector<vector<bool>> ok(FULL, vector<bool>(FULL, false));
	...
	for (int i = 1; i < m; i++) {
		for (int mask = 0; mask < FULL; mask++) {
			for (int lastmask = 0; lastmask < FULL; lastmask++) {
				if (ok[mask][lastmask]) {
					chkmin(dp[i][mask], dp[i - 1][lastmask] + __builtin_popcount(mask ^ v[i]));
				}
			}
		}
	}
