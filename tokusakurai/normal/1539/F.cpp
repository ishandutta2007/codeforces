...
int main(){
    ...
    Lazy_Segment_Tree<int, int> seg1(v, f1, g, h, -inf, 0), seg2(v, f2, g, h, inf, 0);

    vector<int> ans(N, 0);

    rep(i, N){
        each(e, ids[i]){
            int d1 = seg1.query(e+1, N+1)-seg2.query(0, e+1)-1;
            if(d1 > 0) chmax(ans[e], (d1+1)/2);
        }

        each(e, ids[i]){
            seg1.apply(e+1, N+1, -2);
            seg2.apply(e+1, N+1, -2);
        }

        each(e, ids[i]){
            int d2 = seg1.query(0, e+1)-seg2.query(e+1, N+1)-1;
            if(d2 > 0) chmax(ans[e], d2/2);
        }
    }
