...
int main() {
    ...
    while (1) {
        bool b = 1;
        for (ll w : g[cent]) {
            if (son[w] * 2 > N && w != ps) { b = 0; ps = cent; cent = w; break; }
        }
        if (b) { break; }
    }
    ...
    rep(v, N) {
        for (ll w : g[v]) {
            if (dep[w] > dep[v]) { tr[v].push_back(w); }
        }
    }
    ...
    map<ll,ll> mp;
    for (ll w : tr[cent]) {
        while (tr[w].size() == 1) { w = tr[w][0]; }
        if (tr[w].size() == 0) { mp[dep[w]] = w; }
    }

    vll deg(N, -1);
    ll ans = -2;
    rep(v, N) {
        if (deg[dep[v]] == -1) { deg[dep[v]] = g[v].size(); }
        else if (deg[dep[v]] != g[v].size()) { break; }
        if (v == N - 1) { ans = cent; }
    }
