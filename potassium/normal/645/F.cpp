...
int main() {
	...
	vector<Mint> tot(MAXN, Mint(1, MOD107));
	for (int i = 2; i < MAXN; i++) {
		int last = i / mpf[i];
		tot[i] = tot[last] * (mpf[i] == mpf[last] ? mpf[i] : mpf[i] - 1);
	}
	
	vector<Mint> fact(MAXN, Mint(1, MOD107)), inv = fact;
	for (int i = 2; i < MAXN; i++)
		fact[i] = fact[i - 1] * i;
	...
	vector<Mint> cntk(MAXN, Mint(0, MOD107));

	auto add = [&](int v) {
		cnt[v]++;
		Mint newCountK = C(cnt[v], k);
		ans += (newCountK - cntk[v]) * tot[v];
		cntk[v] = newCountK;
	};
	
	auto push = [&](int v) {
		for (auto& d : factors[v])
			add(d);
	};
