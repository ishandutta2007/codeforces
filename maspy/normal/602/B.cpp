...
struct MaxMin {
  using value_type = pi;
  static pi op(pi x, pi y) { return {max(x.fi, y.fi), min(x.se, y.se)}; }
  static constexpr pi unit = {-INF, INF};
  bool commute = true;
};
...
void solve() {
  ...
  VEC(ll, A, N);
  vc<pi> seg_raw(N);
  FOR(i, N) seg_raw[i] = mp(A[i], A[i]);

  SegTree<MaxMin> seg(seg_raw);

  ll ANS = 0;
  FOR(L, N) {
    auto check = [&](pi a) -> bool { return a.fi - a.se <= 1; };
    ll R = seg.max_right(check, L);
    chmax(ANS, R - L);
  }
  print(ANS);
}
