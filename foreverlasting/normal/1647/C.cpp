...
#define tup tuple<int,int,int,int>
    inline void MAIN(const res &Case){
        ...
        for(res i=1;i<=n;i++){
            scanf("%s",str+1);
            for(res j=1;j<=m;j++)a[i][j]=str[j]-'0';
        }
        if(a[1][1]==1){puts("-1");return;}
        vector<tup> ans;
        for(res i=n;i;i--){
            for(res j=m;j;j--){
                if(a[i][j]==1){
                    a[i][j]^=1;
                    if(i>1&&a[i-1][j]==1)ans.emplace_back(i-1,j,i,j);
                    else if(j>1&&a[i][j-1]==1)ans.emplace_back(i,j-1,i,j);
                    else a[i][j]^=1;
                }
            }
        }
        for(res i=1;i<=n;i++)for(res j=1;j<=m;j++){
            if(a[i][j]==1){
                if(i>1)ans.emplace_back(i-1,j,i,j);
                else ans.emplace_back(i,j-1,i,j);
            }
        }
        printf("%d\n",(int)ans.size());
        for(auto [x,y,u,v]:ans)printf("%d %d %d %d\n",x,y,u,v);
    }
}
int main(){
//    srand(time(0));
