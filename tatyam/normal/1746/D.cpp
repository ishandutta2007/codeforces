...
ll solve(){
    ...
    auto dfs=[&](auto dfs,ll i,ll k)->ll{
        auto[p,f]=mem.try_emplace({i,k});
        ll&ans=p->second;
        if(!f)return ans;
        ans=k*s[i];
        if(child[i].empty())return ans;
        if(child[i].size()==1)return ans+=dfs(dfs,child[i][0],k);
        const ll k2=k/child[i].size(),rem=k%child[i].size();
        vector<ll>add;
        each(j,child[i]){
            const ll a=dfs(dfs,j,k2);
            const ll b=dfs(dfs,j,k2+1);
            ans+=a;
            add.push_back(b-a);
        }
        nth_element(add.begin(),add.end()-rem,add.end());
        ans+=accumulate(add.end()-rem,add.end(),0LL);
        return ans;
    };
    return dfs(dfs,0,k);
}
