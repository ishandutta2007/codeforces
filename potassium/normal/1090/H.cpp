...
int main() {
	...
	vector<vector<int>> ss(n, vector<int>(lg, 0));
	for (int i = n - 1; i >= 0; i--)
		for (int j = 0, jump = 1; j < lg && i + jump < n; j++, jump <<= 1)
			ss[i][j] = (i + jump * 2 < n ? ss[i + jump * 2][j] : 0) + d[i + jump];
	...
	while (q--) {
		...
		for (int j = 0; j < p; j++) {
			int sum = ss[l][j] - (r + 1 < n ? ss[r + 1][j] : 0);
			ans += min(sum, (1 << (p - j - 1)) - sum);
		}
		W((ans + 1) / 2);
	}
