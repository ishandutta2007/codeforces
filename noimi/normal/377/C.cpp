...
int main() {
    ...
    vv(int, dp, m + 1, 1 << m, -inf<int>);
    dp[m][allbit(m)] = 0;
    a.resize(m);
    cout << REC([&](auto &&f, int i, int b) -> int {
        if(dp[i][b] != -inf<int>) return dp[i][b];
        int &res = dp[i][b];
        if(v[i].se == 1) {
            res = -inf<int>;
            rep(j, m) {
                if(~b & 1 << j) { chmax(res, f(i + 1, b | 1 << j) + (v[i].fi == 'p' ? a[j] : 0)); }
            }
            return res;
        } else {
            res = inf<int>;
            rep(j, m) {
                if(~b & 1 << j) chmin(res, f(i + 1, b | 1 << j) - (v[i].fi == 'p' ? a[j] : 0));
            }
            return res;
        }
    })(0, 0)
