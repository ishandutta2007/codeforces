...
int main() {
    ...
    auto update = [&](int i, int x) {
        auto pf = sv.factor(x);
        for (auto [p, f] : pf) {
            int prev_min = sz(freq[p]) < n ? 0 : *freq[p].begin();

            int &count = factors[i][p];
            if (count != 0) {
                auto it = freq[p].find(count);
                if (it != freq[p].end())
                    freq[p].erase(it);
            }
            ...
            int new_min = sz(freq[p]) < n ? 0 : *freq[p].begin();
            if (new_min - prev_min > 0)
                ans *= mn(p).pow(new_min - prev_min);
        }
    };
