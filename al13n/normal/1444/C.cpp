...
void dfs1(int v,int c,int i,int p){
  if(C[v]!=c) return;
  if(I[v]!=-1){
    assert(I[v]==i);
    if(P[v]!=p) bad[c]=true;
  ...
  I[v]=i;
  P[v]=p;
  for(int vv:G[v]) dfs1(vv,c,i,1-p);
}

map<int,vector<pii>> G2;
map<int,int> P2;

bool dfs2(int v,int p){
  if(P2.count(v)) return P2[v]==p;
  P2[v]=p;
  for(pii vv:G2[v]) if(!dfs2(vv.X,p^vv.Y)) return false;
  return true;
}
...
bool solve(vector<pii> E){
  ...
  P2.clear();
  for(pii e:E){
    G2[I[e.X]].pb(mp(I[e.Y],P[e.X]!=P[e.Y]));
    G2[I[e.Y]].pb(mp(I[e.X],P[e.X]!=P[e.Y]));
  }
  for(auto& it:G2) if(!P2.count(it.X) && !dfs2(it.X,0)) return false;
  return true;
}

void gen(){
  int n=500000,m=500000,k=500000;
  cout<<n<<' '<<m<<' '<<k<<'\n';
  forn(i,n) cout<<i+1<<' ';
  cout<<'\n';
  forn(i,m) cout<<rand()%n+1<<' '<<rand()%n+1<<'\n';
}
...
int main(int argc, char **argv){
  ...
  int cnt=0;
  forn(i,n) if(I[i]==-1) dfs1(i,C[i],cnt++,0);

  map<pii,vector<pii>> M;
  forv(v,G){
    for(int p:G[v]){
      if(C[p]<=C[v]||bad[C[p]]||bad[C[v]]) continue;
      M[mp(C[v],C[p])].pb(mp(v,p));
    }
  }

  LL ans=0;
  for(auto&it:M) ans+=solve(it.Y);
  LL nb=accumulate(all(bad),0);
  dbg(ans,nb);
  ans+=(k-nb)*(k-nb-1)/2-sz(M);
  cout<<ans;
