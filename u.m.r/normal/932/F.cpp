...
namespace DynamicHull {
    ...
    struct DynamicHull : public multiset<Line> { // for maximum
        bool bad(iterator y) {
            iterator z = next(y);
            if (y == begin()) {
                if (z == end())
        ...
        void merge(const DynamicHull &rhs) {
            for (auto &v : rhs) {
                add(v.m, v.b);
            }
        }
...
int size_ = 0;
DynamicHull::DynamicHull f[MAXN];
ll dp[MAXN];
...
void dfs(int u, int fa) {
    ...
    if (sz[u] == 1) {
        ...
        idx[u] = size_;
        dp[u] = 0;
        f[idx[u]].add(-B[u], -dp[u]);
    }
    else {
        idx[u] = idx[maxson];
        for (auto &v : E[u]) {
            if (v != fa && v != maxson) {
                f[idx[u]].merge(f[idx[v]]);
            }
        }
        dp[u] = -f[idx[u]].eval(A[u]);
        f[idx[u]].add(-B[u], -dp[u]);
    }
}
