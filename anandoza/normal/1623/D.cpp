...
void solve() {
    ...
    auto period = 2 * lcm(n[0] - 1, n[1] - 1);

    vector<ll> times;

    for (int i = 0; i < 2; i++) {
        auto init = goal[i] - robot[i];
        auto step = 2 * (n[i] - 1);
        auto reflect = 2 * (n[i] - goal[i]);

        for (int t = init; t < period; t += step) {
            times.push_back(t);
            times.push_back(t + reflect);
        }
    }

    times.erase(remove_if(all(times), L1(u, u < 0 || u >= period)), times.end());
    dedup(times);
