...
void tc() {
	...
	fori (i, 0, m-1) {
		assert(a[i] != a[i+1]);
		origCost     += abs(a[i] - a[i+1]);
		save[a[i]  ] += abs(a[i] - a[i+1]);
		save[a[i+1]] += abs(a[i] - a[i+1]);
		accs[a[i]].pushb(a[i+1]);
		accs[a[i+1]].pushb(a[i]);
	}
	ll best = origCost;
	fori (i, 1, n+1) {
		if (accs[i].size()) {
			sort(allof(accs[i]));
			ll median = accs[i][accs[i].size() / 2];
			ll mcost = 0;
			foruin (jj, accs[i]) {
				mcost += abs(median - accs[i][jj]);
			}
			minify(best, origCost - save[i] + mcost);
		}
	}
	printf("%" PRId64 "\n", best);
}
