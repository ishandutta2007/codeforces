...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<array<int, 3>> elements(n * m);
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
            int x;
            cin >> x;
            elements[i * m + j] = {x, i, j};
        }
    ...
    sort(all(elements));
    mint squared_sum = 0, sum_i = 0, sum_j = 0, sum_dp = 0;
    vector<mint> dp(n * m);

    for (int i = 0; i < len(elements);) {
        int j = i + 1;
        while (j < len(elements) && elements[j][0] == elements[i][0])
            j++;

        for (int k = i; k < j; k++) {
            dp[k] += sum_dp;
            dp[k] += squared_sum + i * (mint(elements[k][1]).power(2) + mint(elements[k][2]).power(2));
            dp[k] -= mint(2) * (sum_i * mint(elements[k][1]) + sum_j * mint(elements[k][2]));
            dp[k] /= i;
        }

        for (; i < j; i++) {
            squared_sum += mint(elements[i][1]).power(2) + mint(elements[i][2]).power(2);
            sum_i += elements[i][1];
            sum_j += elements[i][2];
            sum_dp += dp[i];
        }
    ...
    for (int i = 0; i < n * m; i++)
        if (elements[i][1] == r && elements[i][2] == c)
            cout << dp[i] << '\n';
}