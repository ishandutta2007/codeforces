...
int dpl[N][N], dpr[N][N], dpu[N][N], dpb[N][N];
char mat[N][N];
...
void solve(){
	...
	for(int i = 0 ; i< n; ++i){
		for(int j = 0; j < m; ++j){
			...
			if ((i == 0) || (j == 0) || (mat[i - 1][j] != mat[i][j]) || (mat[i][j - 1] != mat[i][j]) ){
				dpl[i][j] = 1;
			}
			else {
				dpl[i][j] = min(dpl[i - 1][j], dpl[i][j - 1]) + 1;
			}
		}
	}
	...
	for(int i = 0 ; i< n; ++i){
		for(int j = m; j >= 0; --j){
			...
			if ((i == 0) || (j + 1== m) || (mat[i - 1][j] != mat[i][j]) || (mat[i][j + 1] != mat[i][j])){
				dpu[i][j] = 1;
			}
			else {
				dpu[i][j] = min(dpu[i - 1][j], dpu[i][j + 1]) + 1;
			}
		}
	}
	...
	for(int i = n - 1 ; i >= 0; --i){
		for(int j = m - 1; j >= 0; --j){
			if (((i + 1) == n) || ((j + 1) == m) || (mat[i][j] != mat[i + 1][j]) || (mat[i][j] != mat[i][j + 1])){
				dpr[i][j] = 1;
			}
			else{
				dpr[i][j] = min(dpr[i + 1][j], dpr[i][j + 1]) + 1;
			}
		}
	}
	...
	for(int i = n - 1 ; i >= 0; --i){
		for(int j = 0; j < m; ++j){
			if (((i + 1) == n) || (j == 0) || (mat[i][j] != mat[i + 1][j]) || (mat[i][j] != mat[i][j - 1])){
				dpb[i][j] = 1;
			}
			else{
				dpb[i][j] = min(dpb[i + 1][j], dpb[i][j - 1]) + 1;
			}
		}
	}
	long long ans = 0;
	...
	for(int i = 0; i < n; ++i){
		for(int j = 0; j < m; ++j){
			ans += min({dpl[i][j], dpr[i][j], dpb[i][j], dpu[i][j]});
			// cout << min(dpl[i][j], dpr[i][j]) << " ";
		}
		// cout << endl;
	}
	assert(ans >= (n * 1LL *m));
