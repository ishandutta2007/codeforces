...
ll get(const vector<ll> &vs){
	ll re=0;
	reps(i,1,vs.size()) re+=abs(vs[i]-vs[i-1]);
	return re;
}
...
int main(){
	...
	TT{
		...
		cauto x=read();
		cauto vs=readV(n);
		cauto mn=*min_element(all(vs));
		cauto mx=*max_element(all(vs));
		vector<ll> sub{vs[0],mn,mx,vs.back()};
		ll re=MOD;
		rep(i,sub.size()+1){
			auto sub2=sub;
			sub2.insert(sub2.begin()+i,1);
			rep(i,sub2.size()+1){
				auto sub3=sub2;
				sub3.insert(sub3.begin()+i,x);
				MN(re,get(sub3));
			}
		}
		re-=get(sub);
