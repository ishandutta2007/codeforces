...
ll n, m, C;
ll c[300005], d[300005], h[300005];
ll D[300005], H[300005];
ll a[1000005], g[1000005];
...
int main(void)
{
	...
	rep(i, 1, C) for(int j = 1; i*j <= C; j++) chmax(g[i*j], a[i]*j);
	rep(i, 1, C) chmax(g[i], g[i-1]);
	

	cin >> m;
	rep(i, 1, m){
		cin >> D[i] >> H[i];
		ll ans = upper_bound(g, g+C+1, D[i]*H[i]) - g;
		if(ans > C) ans = -1;
		cout << ans << " ";
	}
	cout << endl;
