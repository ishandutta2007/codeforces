...
signed main(){
    ULL(d);
    vector<ull>factor;
    for(uint i=2;ull(i)*i<=d;i++)if(d%i==0){
        factor.push_back(i);
        do{
            d/=i;
        }while(d%i==0);
    }
    if(d!=1)factor.push_back(d);
    LL(q);
    rep(q){
        ULL(u,v);
        vector<ll>x(factor.size()),y(factor.size());
        rep(factor.size()){
            ull p=factor[i];
            while(u%p==0){
                x[i]++;
        ...
        ll a=0,b=0;
        rep(factor.size()){
            if(x[i]>y[i])a+=x[i]-y[i];
            else b+=y[i]-x[i];
        }
        Modint ans=fact(a)*fact(b);
        rep(factor.size())ans*=inv[abs(x[i]-y[i])];
        out(ans);
    }
}