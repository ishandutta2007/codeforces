...
int main(void)
{
	...
	while(T--){
		...
		set<P> S;
		rep(i, 0, 2) S.insert(P(r[i], c[i]));
		
		bool ans = false;
		rep(i, 0, 2) if(r[i] == r[3] && c[i] == c[3]) ans = true;
		if(ans){
			yes();
			continue;
		}
		if(S.count(P(1, 1)) && S.count(P(2, 2)) == 0){
			if(r[3] == 1 || c[3] == 1) yes();
			else no();
			continue;
		}
		if(S.count(P(1, n)) && S.count(P(2, n-1)) == 0){
			if(r[3] == 1 || c[3] == n) yes();
			else no();
			continue;
		}
		if(S.count(P(n, 1)) && S.count(P(n-1, 2)) == 0){
			if(r[3] == n || c[3] == 1) yes();
			else no();
			continue;
		}
		if(S.count(P(n, n)) && S.count(P(n-1, n-1)) == 0){
			if(r[3] == n || c[3] == n) yes();
			else no();
			continue;
		}
		...
		ll cnt = 0;
		rep(i, 0, 2) if(r[i] % 2 == r[3] % 2 && c[i] % 2 == c[3] % 2) cnt++;
		
		//if(r[3] == 1 && c[3] == 1 || r[3] == 1 && c[3] == n || r[3] == n && c[3] == 1 || r[3] == n && c[3] == n) no();
		if(cnt) yes();
		else no();
	}
