...
signed main(){
    ...
    while (q--){
        ...
        int idxxx = 0;
        ForE(i, 1, n){
            if (idxxx != idx.size() - 1 && idx[idxxx + 1] == i){
                idxxx++;
            }
            int tmp = 0, idx2 = idxxx;
            //cout << idxxx << endl;
            while (idx2 > 0){
                //cout << i << ' ' << idx[idx2] << ' ' << i - idx[idx2 - 1] << ' ';
                if (i - idx[idx2] >= 20){
                    break;
                }
                tmp += (1 << (i - idx[idx2]));
                //cout << tmp << endl;
                if (i - idx[idx2 - 1] >= tmp) ans++;
                idx2--;
            }
            if (i >= idx[0] && i - idx[0] <= 20){
                tmp += (1 << (i - idx[0]));
                if (i >= tmp){
                    ans++;
                    //cout << "IDX0 " << i << ' ' << tmp << ' ' << idx[0] << endl;
                }
            }
            //cout << "ANST " << ans << endl;
            //cout << "TMP " << tmp << endl;
