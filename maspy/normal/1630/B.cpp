...
void solve() {
  LL(N, K);
  VEC(ll, A, N);
  auto CNT = bincount(A, N + 1);
  ll cnt_xy = 0;
  ll y = 0;
  ll best_diff = N + 1;
  pi best_xy = {0, N + 1};
  FOR(x, N + 1) {
    while (y <= N && cnt_xy - (N - cnt_xy) < K) { cnt_xy += CNT[y++]; }
    if (cnt_xy - (N - cnt_xy) < K) break;
    if (chmin(best_diff, y - x)) best_xy = {x, y};
    cnt_xy -= CNT[x];
  }
  ll x;
  tie(x, y) = best_xy;
  --y;
  for (auto&& a: A) a = (x <= a && a <= y ? 1 : -1);
  assert(SUM(A) >= K);
  auto Ac = cumsum(A);
  vi T(K, -1);
  FOR(i, len(Ac)) {
    ll k = Ac[i];
    if (0 <= k && k < K && T[k] == -1) T[k] = i;
  }
  T.eb(N);
