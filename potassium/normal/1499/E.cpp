...
int main() {
	...
	for (int i = n; i >= 0; i--) {
		for (int j = m; j >= 0; j--) {
			for (int c = 0; c <= 1; c++) {
				if ((c == 0 && i == 0) || (c == 1 && j == 0)) continue;
				char last = (c == 0 ? s[i - 1] : t[j - 1]);
				if (i + 1 <= n && last != s[i]) dp[i][j][c] += dp[i + 1][j][0];
				if (j + 1 <= m && last != t[j]) dp[i][j][c] += dp[i][j + 1][1];
				dp[i][j][c] %= MOD998;
			}
		}
	}
	...
	auto go = [&](string& s) {
		ll ans = 0;
		for (int r = 0, l = 0; r < (int)s.length(); r++) {
