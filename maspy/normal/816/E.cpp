...
void solve() {
  ...
  */
  ...
  auto conv = [&](vi& A, vi& B) -> vi {
    ll NA = A.size();
    ll NB = B.size();
    vi C(NA + NB - 1, INF);
    FOR(a, NA) FOR(b, NB) chmin(C[a + b], A[a] + B[b]);
    return C;
  };
  ...
  FOR_R(v, N) {
    ...
    vi& dp0 = DP0[v];
    vi& dp1 = DP1[v];
    dp0.assign(A.size() + 1, INF);
    dp1.assign(B.size() + 1, INF);
    FOR(i, A.size()) {
      chmin(dp0[i], A[i]);
      chmin(dp0[i + 1], A[i] + C[v]);
      chmin(dp1[i + 1], B[i] + C[v] - D[v]);
    }
    FOR(i, dp0.size()) chmin(dp1[i], dp0[i]);
  }
  auto& dp = DP1[0];
  ll ANS = 0;
  FOR(n, dp.size()) if (dp[n] <= HAVE) ANS = n;
  print(ANS);
}
