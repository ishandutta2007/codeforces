						...
						reps(i,1,n){
							...
							if(prv==cur){
								...
								if(rs.size() && *rs.begin()==cur){
									rs.erase(rs.begin());
									rs.emplace(MOD); rs.emplace(MOD);
								}else{
									if(rs.size() && *rs.begin()<cur) return false;
									rs.emplace(MOD);
								}
							}else{
								rs.emplace(prv);
								if(ls.size() && *ls.rbegin()==cur){
									ls.erase(prev(ls.end()));
									ls.emplace(-MOD); ls.emplace(-MOD);
								}else{
									if(ls.size() && *ls.rbegin()>cur) return false;
									ls.emplace(-MOD);
								}
							}
						}
