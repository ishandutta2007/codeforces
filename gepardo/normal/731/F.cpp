/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 376 (Div. 2)         |
|       Task: F                                     |
|       Date: 16. 10. 2016                          |
\--------------------------------------------------*/
// Includes
...
const int divpoint = 500;
const int maxN = 500000;

int n = in;
vector <int> v(n);
int cnt[maxN];
long long ans[divpoint+1] = {};
...
main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            opt = 0;
            for (int j = 1; j * v[i] <= 200000; j++)
                opt += 1LL * v[i] * j * (cnt[(j+1) * v[i] - 1] - cnt[j * v[i] - 1]);
        }
        res = max(res, opt);
    }
    out = res;
