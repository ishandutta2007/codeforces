...
{
    int x,y,z;
    gyq(int x_=0,int y_=0,int z_=0){x=x_;y=y_;z=z_;}
}a[N];
struct zjj
{
    int first,second;
    zjj(int x=0,int y=0){first=x;second=y;}
    inline bool operator <(zjj ath){return first==ath.first?second<ath.second:first<ath.first;}
};

int du[N];
int num[N];
vector<zjj>nxt[N];
inline bool myru_v(gyq x,gyq y){return x.z>y.z;}
inline bool myru_num(int x,int y){return du[x]<du[y];}
inline bool myru_zjj(zjj x,zjj y){return du[x.first]>du[y.first];}

void add(int x,int y,int z){du[x]++;du[y]++;nxt[x].push_back(zjj(y,z));nxt[y].push_back(zjj(x,z));return;}
...
{
    LL siz,s;
    priority_queue<LL>z,g;//z g 
    inline void clear(){siz=s=0;for(;!z.empty();z.pop());for(;!g.empty();g.pop());return;}
    inline void erase(LL val){g.push(val);siz--;s-=val;return;}
    inline void push(LL val){z.push(val);siz++;s+=val;return;}
    inline void del(){for(;!z.empty()&&!g.empty()&&z.top()==g.top();z.pop())g.pop();return;}
    inline void pop(){del();siz--;s-=z.top();z.pop();return;}
    inline LL top(){del();return z.empty()?-INF:z.top();}
}D[N];
inline void delete_(int now){for(auto i:nxt[now])if(du[i.first]>du[now])D[i.first].push(i.second);else break;return;}

vector<LL>yea,bcy;
inline void remark(int now,int fa,int Time_)
{
    // printf("now:%d fa:%d T:%d\n",now,fa,Time_);
    int les=du[now]-Time_;LL ans=0;t[now]=Time_;
	for(;D[now].siz>les;D[now].pop());
	for(auto i:nxt[now])if(i.first!=fa){if(du[i.first]<=Time_)break;remark(i.first,now,Time_);}
	yea.clear(),bcy.clear();
    for(auto i:nxt[now])if(i.first!=fa)
    {
        if(du[i.first]<=Time_)break;
		LL x=f[i.first][1]+i.second-f[i.first][0];
		if(x<=0){ans+=f[i.first][1]+i.second;les--;}
		else {ans+=f[i.first][0];D[now].push(x);bcy.push_back(x);}
	}
	for(;D[now].siz>les&&D[now].siz!=0;D[now].pop())yea.push_back(D[now].top());f[now][0]=ans+D[now].s;
	for(;D[now].siz>=les&&D[now].siz!=0;D[now].pop())yea.push_back(D[now].top());f[now][1]=ans+D[now].s;
    for(auto i:yea)D[now].push(i);for(auto i:bcy)D[now].erase(i);return;
}
vector<LL> minimum_closure_costs(int n,vector<int>u,vector<int>v,vector<int>w)
{
    for(int i=1;i<=n;i++)D[i].clear();
    for(int i=1;i<n;i++)a[i]=gyq(u[i-1],v[i-1],w[i-1]),sum+=w[i-1];
    ans.clear();ans.push_back(sum);for(int i=1;i<n;i++)add(a[i].x,a[i].y,a[i].z);
    for(int i=1;i<=n;i++)num[i]=i,sort(nxt[i].begin(),nxt[i].end(),myru_zjj);sort(num+1,num+n+1,myru_num);
    for(int i=1,j=1;i<n;i++)
    {
        for(;j<=n&&du[num[j]]==i;j++)delete_(num[j]);LL sum=0;
        for(int k=j,now;k<=n;k++)if(t[now=num[k]]!=i)remark(now,0,i),sum+=f[now][0];ans.push_back(sum);
    }
    return ans;
}
...
void Read()
{
    rin>>n;for(int i=1,x,y,z;i<n;i++){rin>>x>>y>>z;u.push_back(x);v.push_back(y);w.push_back(z);}
    return;
}
int main()
{
    Read();
    pri=minimum_closure_costs(n,u,v,w);
    for(auto i:pri)printf("%lld ",i);puts("");
    return 0;
}