...
int main() {
    ...
    Points xy(3);
    rep(i,3) cin >> xy[i];
    Real d1 = distance(xy[0],xy[1]);
    Real d2 = distance(xy[0],xy[2]);
    Real d3 = distance(xy[1],xy[2]);
    debug(d1,d2,d3,t1,t2);
    if(d1 + t1 + d3 - EPS <= d2 + t2) {
        debug(1);
        cout << d1 + t1 << '\n';
    }
    else if(d2 + t2 + d3 - EPS <= d1 + t1) {
        cout << d2 + t2 + d3 << '\n';
    }
    else if(d2 + d3 - EPS <= d1 + t1) cout << d1 + t1 << '\n';
    else {
        Real ok = 0;
        Real ng = 1e5;
        while(ng - ok > 1e-7) {
            Real mid = (ok + ng) / 2;
            Circle c1(xy[0],mid);
            Circle c2(xy[1],d1 + t1 - mid);
            Circle c3(xy[2],d2 + t2 - mid);
            if(intersect(c1,c2) == 4 || intersect(c2,c3) == 4 || intersect(c1,c3) == 4) {
                ng = mid;
                continue;
            }
            auto p1 = crosspoint(c1,c2);
            if(distance(xy[2],p1.first) <= d2 + t2 - mid || distance(xy[2],p1.second) <= d2 + t2 - mid) {
                ok = mid;
                continue;
            }
            auto p2 = crosspoint(c1,c3);
            if(distance(xy[1],p2.first) <= d1 + t1 - mid || distance(xy[1],p2.second) <= d1 + t1 - mid) {
                ok = mid;
                continue;
            }
            auto p3 = crosspoint(c3,c2);
            if(distance(xy[0],p3.first) <= mid || distance(xy[0],p3.second) <= mid) {
                ok = mid;
                continue;
            }
            ng = mid;
