...
{ _
    ...
    {
        r = min(r, m - 1);
        return a[i][r] - (l > 0 ? a[i][l - 1] : 0);
    };

    vector<int> dp(m, 0), dpnxt(m, 0);

    for (int x = 0; x < m; ++x)
    {
        dp[x] = query(0, x, x + k - 1) + query(1, x, x + k - 1);
    }
    ...
    for (int i = 1; i < n; ++i)
    {
        ...
        for (int x = 0; x < k; ++x)
        {
            st.update(0, x, F(-query(i, x, x)));
        }

        for (int x = 0; x < m; ++x)
        {
            int cur = query(i, x, x + k - 1) + query(i + 1, x, x + k - 1);

            dpnxt[x] = cur + st.query(0, m - 1);

            st.update(max(0, x - k + 1), x, F(+query(i, x, x)));
            if (x + k < m)
            {
                st.update(x + 1, x + k, F(-query(i, x + k, x + k)));
            }
        }
