...
void dfs(int x){
	...
	for(int i=0;i<e[x].size();i++){
		int to=e[x][i];fa[to]=x;
		dfs(to),sz[x]+=sz[to];
		if(vis[to])son[x].push_back(to);
		cnt+=vis[to];L[to]=cnt;
	}
	CNT[x]=cnt;
...
//	printf("%.5lf",(&pppp-&ppp)/1024.0/1024.0);
	...
	while(m--){
		...
		if(op==1){
			...
			if(!vis[l])continue;
			int f=fa[l];
			add(dfn[f],-(CNT[f]*(CNT[f]-1)/2+CNT[f]));
			CNT[f]--;
			add(dfn[f],(CNT[f]*(CNT[f]-1)/2+CNT[f]));
			son[f].erase(lower_bound(son[f].begin(),son[f].end(),l));
		}else{
			int l=id[read()],r=id[read()];
			int f=fa[l];
			int res=get(dfn[r]+sz[r]-1)-get(dfn[l]-1);
			int rr=lower_bound(son[f].begin(),son[f].end(),r)-son[f].begin();
			int ll=lower_bound(son[f].begin(),son[f].end(),l)-son[f].begin();
			int cnt=rr-ll+1;
