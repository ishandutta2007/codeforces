...
fun main(){
    solve.cases{
		...
		for(i in 1..n){
			if(targetTime > T[i]){
				val dt = T[i] - nowtime
				val newpos = nowpos + dt * (if(targetPos > nowpos) 1 else -1 )
				retPos[i] = newpos
			}else{
				nowtime = T[i]
				nowpos = targetPos
				retPos[i] = nowpos
				val dx = ((X[i]) - nowpos ).absoluteValue
				targetPos = X[i]
				targetTime = T[i] + dx
			}
		}
		val finalpos = targetPos
		var total = 0
		for(i in 1..n){
			val want = X[i]
			val before = retPos[i]
			val after = if(i == n) finalpos else retPos[i+1]
			if(want in before..after || want in after..before){
				total ++
			}
		}
//        just dei retPos
...
/*
boolean mistakes .... why

1 single simulation mistakes
