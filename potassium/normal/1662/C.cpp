...
template<class T>
vector<vector<T>> operator * (vector<vector<T>>& x, vector<vector<T>>& y) {
	const int a = (int)x.size();
	const int b = (int)y[0].size();
	const int c = (int)y.size();
	vector<vector<T>> ans(a, vector<T>(b, 0));
	for (int i = 0; i < a; i++)
		for (int k = 0; k < c; k++)
			for (int j = 0; j < b; j++)
				ans[i][j] = (ans[i][j] + 1LL * x[i][k] * y[k][j]) % MOD998;
	return ans;
};
...
int main() {
	...
	for (int i = 0; i < n; i++)
		M[i][i + n] = MOD998 + 1 - D[i];

	for (int i = 0; i < n; i++)
		M[i + n][i] = 1;

	vector<vector<int>> pwM(2 * n, vector<int>(2 * n, 0));
	for (int i = 0; i < 2 * n; i++)
		pwM[i][i] = 1;
	...
	vector<vector<int>> ans = pwM * col;

	int trace = 0;
	for (int i = 0; i < n; i++)
		trace = (trace + ans[i + n][i]) % MOD998;
