...
public class TaskA {
	...
	InputReader in;
	PrintWriter out;
	String[] alcohol = {"ABSINTH", "BEER", "BRANDY", "CHAMPAGNE", "GIN", "RUM", "SAKE", "TEQUILA", "VODKA", "WHISKEY",
		"WINE"};

	void solve() {
		int visitorCount = in.readInt();
		int checkRequired = 0;
		for (int i = 0; i < visitorCount; i++) {
			String visitor = in.readString();
			try {
				int age = Integer.parseInt(visitor);
				if (age < 18)
					checkRequired++;
			} catch (NumberFormatException e) {
				if (Arrays.binarySearch(alcohol, visitor) >= 0)
					checkRequired++;
			}
		}
		out.println(checkRequired);
	}
