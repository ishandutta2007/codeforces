...
void check(int x1,int y1,int x2,int y2)
{
    int cnt=0;
    bool on=true;
    for(int x=x1,x0=1;x2>x1?x<=x2:x2<x1?x>=x2:x0;x2>x1?x++:x2<x1?x--:x,x0=0)
        for(int y=y1,y0=1;y2>y1?y<=y2:y2<y1?y>=y2:y0;y2>y1?y++:y2<y1?y--:y,y0=0)
        {
            if(s[x][y]=='.')
                on=false;
            if(on)
...
signed main()
{
    ...
    for0(i,n)
        for0(j,m)
            if(s[i][j]=='*'&&i!=c_x&&j!=c_y)
                halt("NO");
    check(c_x,c_y,0,c_y);
    check(c_x,c_y,c_x,0);
    check(c_x,c_y,n-1,c_y);
    check(c_x,c_y,c_x,m-1);
    cout<<"YES";
