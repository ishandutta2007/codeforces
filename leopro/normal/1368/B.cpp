...
void solve() {
   ...
   while (r - l > 1){
       int m = (l + r) / 2;
       vector<int> app(10, m / 10);
       int s = accumulate(app.begin(), app.end(), 0ll);
       for (int i = 0; i < m - s; ++i) app[i]++;
       int ans = 1;
       for (int x : app) ans *= x;
       (ans >= k ? r : l) = m;
   }
       vector<int> app(10, r / 10);
       int s = accumulate(app.begin(), app.end(), 0ll);
       for (int i = 0; i < r - s; ++i) app[i]++;
   for (int i = 0; i < 10; ++i){
       for (int j = 0; j < app[i]; ++j) cout << word[i];
   }
   cout << '\n';
}