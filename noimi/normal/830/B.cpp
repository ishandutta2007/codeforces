...
signed main(){
    ...
    vec b = a;
    sort(all(b));
    SegmentTree<ll> seg(n*2,plus<ll>(),0LL);
    rep(i,n*2)seg.set(i,1);
    seg.build();
    int now = 0;
    int pl = 0;
    set<pii> se;
    rep(i,n)se.emplace(a[i],i);
    ll ans = 0;
    while(now!=n){
        auto it = se.lower_bound({b[now],pl});
        if(it != se.end() and (*it).fi == b[now]){
            ans += seg.query(pl,it->se + 1);
            seg.update(it->se,0);
            seg.update(it->se + n,0);
            pl = (it->se + 1)%n;
        }
        else{
            it = se.lower_bound({b[now],0});
            ans += seg.query(pl,it->se + n + 1);
            seg.update(it->se,0);
            seg.update(it->se + n,0);
            pl = (it-> se + 1)%n;
        }
