...
ll xd(ll a[], ll b[])
{
    ll t = dist(a[0],a[1],b[0],b[1]);
    if(t > sqq(a[2] + b[2])) return 0;
    else if(t == sqq(a[2] + b[2])) return 1;
    else if(t > sqq(abs(a[2] - b[2]))) return 2;
    else if(t == sqq(abs(a[2] - b[2]))) return 1;
    else return 0;
}

ll xd3(ll a[], ll b[], ll c[])
{
    ll dis1 = dist(a[0],a[1],b[0],b[1]);
    ll dis2 = dist(a[0],a[1],c[0],c[1]);
    ll dis3 = dist(b[0],b[1],c[0],c[1]);
    ld h1 = (dis1 + sqq(a[2]) - sqq(b[2]))/(2*sqrt(abs((ld)dis1)));
    ld v1 = sqrt(abs(sqq(a[2])-h1*h1));
    ld h2 = (dis1 + dis2 - dis3)/(2*sqrt(abs((ld)dis1)));
    ld v2 = sqrt(abs(dis2-h2*h2));
    ll ans = 0;
    if(abs((v2-v1)*(v2-v1) + (h2-h1)*(h2-h1) - sqq(c[2])) <= 1e-6) ans++;
    if(abs((v2+v1)*(v2+v1) + (h2-h1)*(h2-h1) - sqq(c[2])) <= 1e-6) ans++;
    return ans;
}
...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        else if(n==3)
        {
            ...
            if(t1==0 || t2==0 || t3 == 0)
            {
                cout<<4 + (t1==2?1:0) + (t2==2?1:0) + (t3==2?1:0);
            }
            else if(t1>0 && t2>0 && t3>0)
            {
                ll tt = xd3(a[0], a[1], a[2]);
                if(t1==1 || t2==1 || t3 == 1)
                {
                    cout<<4 + (t1==2?1:0) + (t2==2?1:0) + (t3==2?1:0) + (tt==0?1:0);
                }
                else
                {
                    cout<<8-tt;
