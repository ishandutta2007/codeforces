...
void solve(){
  ...
  if(k == n-1) {
    ...
    li y2 = A[y];
    li z2 = A[z];
    do { cerr << "a.cpp" ":" "18" "  {" << "x,y,z,x2,y2,z2" << "} = {"; debug_impl_seq(x,y,z,x2,y2,z2); cerr << endl; } while(0);
    A[x]=y; A[y]=x;
    A[z]=y2; A[y2]=z;
    A[z2]=x2; A[x2]=z2;
  } else if(k != 0) {
  ...
  do { cerr << "a.cpp" ":" "45" "  {" << "s" << "} = {"; debug_impl_seq(s); cerr << endl; } while(0);
  cout << flush;
}
