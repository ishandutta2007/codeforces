...
pair<Mat,bool> Inv(Mat a){
	int n=a.size();
	Mat re(n,vector<mint>(n));
	rep(i,n) re[i][i]=1;
	rep(i,n){
		reps(j,i,n)if(a[j][i].v){
			swap(a[j],a[i]); swap(re[j],re[i]);
			break;
		}
		if(a[i][i].v==0) return make_pair(re,false);
		mint t=mint(1)/a[i][i];
		rep(k,n) a[i][k]*=t, re[i][k]*=t;
		rep(j,n)if(i!=j){
			mint t=a[j][i];
			if(t.v==0) continue;
			rep(k,n){
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	sort(all(b));
	int bad=0;
	rep(i,n)if(a[i]!=b[i]) ++bad;
	bad/=2;
	int n1=accumulate(all(a),0);
	int n0=n-n1;
	int n01=min(n0,n1)+1;
	Mat A(n01,vector<mint>(n01));
	mint q=mint(1)/comb(n,2);
	rep(i,n01){
		mint c0=n0-i;
		mint c1=n1-i;
		A[i][i]=1;
		if(i) A[i][i]-=A[i][i-1]=i*i*q;
		if(i+1<n01) A[i][i]-=A[i][i+1]=c0*c1*q;
	}
	out(A,1);
	A=modpow(A,t);
	cout<<A[bad][0]<<endl;
