...
void main_() {
	...
	REP(bit, 1 << n) {
		...
		REP(i, k - 1) {
			V<> np;
			REP(j, 1 << (k - 2 - i)) {
				int u = player[j << 1], v = player[j << 1 | 1];
				if(bit >> now & 1) {
					res[v] = ((1 << (k - 1 - i)) + 1);
					np.pb(u);
				} else {
	...
	REP(bit, 1 << n) {
		...
		mint hash = 0;
		REP(i, 1 << (k - 1)) {
			hash += mint(a).pow(res[i]) * (i + 1 + (1 << (k - 1)));
		}
		if(bit >> now & 1) {
			mint oth = h - hash;
			if(loses.count(oth.val())) {
				V<> ans = loses[oth.val()];
				foa(v, res) ans.pb(v);
				print(ans);
				return;
