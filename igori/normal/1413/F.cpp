...
void bfs(int v, vi &dist, vector<vector<pii> > &g)
{
    vector<int> q = {v};
    fill(all(dist), -1);
    dist[v] = 0;
...
void bfs01(int v, vi &dist, vector<vector<pii> > &g)
{
    vector<int> q = {v};
    fill(all(dist), -1);
    dist[v] = 0;
...
void dfs(int v, int p, vector<vector<pii> > &g, vi &L, vi &R, vi &C, vi &D, int &T, int c, int d)
{
    C[v] = c;
    D[T] = d;
    L[v] = R[v] = T++;
    ...
    {
        int u = e.fi;
        dfs(u, v, g, L, R, C, D, T, c ^ e.se, d + 1);
        R[v] = R[u];
    }
}
...
signed main()
{
    ...
    bfs(0, d2, g);
    int x = 0, y = 0;
    forn(i, n) if (d2[i] > d2[x]) x = i;
    bfs(x, d0, g);
    forn(i, n) if (d0[i] > d0[y]) y = i;
    bfs(y, d1, g);
    #ifdef LOCAL
    cout << "possible diameter: " << x << " " << y << endl;
    #endif // LOCAL
    vi L0(n), R0(n), C0(n), D0(n);
    vi L1(n), R1(n), C1(n), D1(n);
    int t = 0;
