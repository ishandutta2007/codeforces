...
int get(Node* cur, int l, int r, int x, int y) {
    ...
    int m = (l + r) / 2;
    return get(cur->lp, l, m, x, y) + get(cur->rp, m + 1, r, x, y);
}
int eval(Node* ptr) {
    return ptr ? ptr->val : 0;
}
void upd(Node*& cur, int l, int r, int x) {
    if (cur == nullptr) {
        cur = lastn++;
    }
    if (l == r) {
        ...
        cur->val = eval(cur->lp) + eval(cur->rp);
    }
}
int solve() {
    int n, t;
    ...
    for (int x : v) {
        ...
        while (cur + x < t && cur <= n) {
//            DBN(a[cur]);
            upd(root, 0, INF, a[cur]);
            ++cur;
        }
//        DBN(t - x - 1);
        ans = max(ans, get(root, 0, INF, 0, x));
    }
    cout << ans << endl;
