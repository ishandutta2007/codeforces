...
ff(x,1,n)
{
	auto calc=[&](auto y,auto x,auto z)
	{
		int res=a.query(n)-a.query(x);
		ff(u,0,min(k,n-z+1)-1)
		if((y&(1<<u))&&id[u+z]>x)res++;
		return res;
	};
	me(f[x],0x3f);
	for(int y=1;y<(1<<min(k,n-x+2));y+=2)
	f[x][y/2]=f[x-1][y];
	ff(y,0,(1<<min(k,n-x+1))-1)
	ff(z,0,k-1&&z+x<=n)
	if(!((y>>z)&1))
	f[x][y^(1<<z)]=min(f[x][y^(1<<z)],f[x][y]+calc(y,id[x+z],x));
	a.add(id[x],1);
}
cout<<f[n][1]<<endl;
#endif
