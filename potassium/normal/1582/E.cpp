...
int main() {
	CASE(t) {
		...
		deque<pair<ll, int>> last = { { LLONG_MAX, -1 } };
		deque<pair<ll, int>> cur;

		auto push = [&](ll sum, int idx) {
			if (!cur.empty() && cur.back().first >= sum) return;
			cur.push_back({ sum, idx });
		};

		int ans = -1;
		for (int i = 1; ; i++) {
			for (int j = 0; j < n; j++) {
				if (j - i < last.front().second) continue;
				while ((int)last.size() >= 2 && j - i >= last[1].second)
					last.pop_front();

				ll sum = get(j - i + 1, j);
				if (sum < last.front().first)
					push(sum, j);
			}
