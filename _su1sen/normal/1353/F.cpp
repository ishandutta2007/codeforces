...
int main() {
    ...
    while (t --> 0) {
        ...
        vec<vec<int>> cmp_a(n, vec<int>(m));
        rep(i, n) rep(j, m) {
            cmp_a[i][j] = cmp[a[i][j]];
        }
        vec<vec<ll>> dp(m, vec<ll>(s, INF));
        dp[0][cmp_a[0][0]] = 0;
        rep(i, n) rep(j, m) {
            if (i == 0 and j == 0) continue;
            rep(k, s) {
                ll pi = i > 0 ? dp[j][k] : INF;
                ll pj = j > 0 ? dp[j - 1][k] : INF;
                int nk = min(k, cmp_a[i][j]);
                if (nk == k) dp[j][nk] = INF;
                if (pi != INF) {
                    chmin(dp[j][nk], pi + a[i][j]);
                }
                if (pj != INF) {
                    chmin(dp[j][nk], pj + a[i][j]);
                }
                if (nk != k) dp[j][k] = INF;
            }
        }
