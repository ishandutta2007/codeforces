    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                p[i] = new pair<P, int>(new P(b, a), i);
            }
            Array.Sort(p);
            var dp = new long[n + 1][];
            ...
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < k + 1; j++)
                {
                    if (dp[i + 1][j] < dp[i][j] + (k - 1) * (long)p[i].v1.v1) {
                        dp[i + 1][j] = dp[i][j] + (k - 1) * (long)p[i].v1.v1;
                        prev[i + 1][j] = j;
                    }
                    if (j + 1 < k + 1 && dp[i + 1][j + 1] < dp[i][j] + j * (long)p[i].v1.v1 + p[i].v1.v2) {
                        dp[i + 1][j + 1] = dp[i][j] + j * (long)p[i].v1.v1 + p[i].v1.v2;
                        prev[i + 1][j + 1] = j;
                    }
                }
            }
