...
int main(int argc, char **argv){
  ...
  vector<map<int,int>> iA(n);
  vector<vector<int>> B(41);
  forn(i,n){
    int k;
    cin>>k;
    A[i].resize(k);
  ...
  vector<vector<vector<int>>> E(41);
  for(int k=1;k<sz(B);++k){
    E[k].resize(sz(B[k]));
    for(int i=sz(B[k])-1;i>=0;--i){
      E[k][i].resize(k);
      forn(j,k){
        if(i+1==sz(B[k])){
          E[k][i][j]=n;
        ...
        E[k][i][j] = A[B[k][i+1]][j]==A[B[k][i]][j] ? E[k][i+1][j] : B[k][i+1];
      }
    }
  }
  vector<int> ans(m);
  vector<tuple<int,int,int>> T(sz(B));
  forn(i,n){
    forv(k,B){
      int t=lower_bound(all(B[k]),i)-B[k].begin();
      T[k]=MT(t==sz(B[k]) ? n : B[k][t], k, t);
    }
    sort(all(T));
    forv(j0,A[i]){
      int v=A[i][j0];
