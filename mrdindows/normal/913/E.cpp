...
Solution opAnd(Solution s1, Solution s2) {
    Solution res;
    res.s = s1.s + "&" + s2.s;
    res.bs = s1.bs & s2.bs;
    return res;
}
Solution opOr(Solution s1, Solution s2) {
    Solution res;
    res.s = s1.s + "|" + s2.s;
    res.bs = s1.bs | s2.bs;
    return res;
...
const int R = 300;
vector<Solution> allF[R];
vector<Solution> allE[R];
vector<Solution> allT[R];
Solution usedF[300];
Solution usedE[300];
Solution usedT[300];

Solution bestAns[1010];
bool operator<(const Solution& s1, const Solution& s2) {
    return s2.s.empty() || s1.s.size() < s2.s.size() || (s1.s.size() == s2.s.size() && s1.s < s2.s);
}
void addF(const Solution& s) {
    if (s < usedF[s.bs]) {
        usedF[s.bs] = s;
        allF[s.s.size()].push_back(s);
    }
}
void addT(const Solution& s) {
    if (s < usedT[s.bs]) {
        usedT[s.bs] = s;
        allT[s.s.size()].push_back(s);
    }
}
void addE(const Solution& s) {
    if (s < usedE[s.bs]) {
        usedE[s.bs] = s;
        allE[s.s.size()].push_back(s);
    }
}
int32_t solve() {
    FI(3) {
        addF(Solution::from(i));
    }
    allE[1] = allT[1] = allF[1];
    int tot = 1 << 8;
    for (int len = 2; len < R; ++len) {
//        DBN(len, tot);
        if (tot == 0) break;
        for (auto& p : allE[len - 2]) {
            Solution sol = p;
            sol.s = "(" + sol.s + ")";
            addF(sol);
        }
        for (auto& p : allF[len - 1]) {
            addF(p.otr());
        }
        for (int i = 1; i + 1 < len; ++i) {
            for (auto& p1 : allT[i]) {
                for (auto& p2 : allF[len - 1 - i]) {
                    addT(opAnd(p1, p2));
                }
            }
        }
