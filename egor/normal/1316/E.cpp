...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        auto s = in.readTable<int>(n, p);

        arr<ll> res(1 << p, numeric_limits<ll>::min());
        arr<ll> next(1 << p);
        ...
        for (int i : order) {
            ...
            for (int j : range(1 << p)) {
                for (int l : range(p)) {
                    if ((j >> l & 1) == 0) {
                        maxim(next[j + (1 << l)], res[j] + s(i, l));
                    }
                }
                maxim(next[j], res[j] + (id < k + bitCount(j) ? a[i] : 0));
            }
            swap(res, next);
            id++;
        }
        out.printLine(res[(1 << p) - 1]);
    }
};
