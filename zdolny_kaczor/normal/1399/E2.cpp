...
vector <tuple <int, int, int> > graf[nax];
int dfs(int x, int y, vpii e[]) {
	int siz = 0;
	debug(arr(graf, x), imie(y));
	for (auto [v, c, coi] : graf[x]) if (v != y) {
		int r = dfs(v, x, e);
		e[coi].emplace_back(c, r);
		siz += r;
		debug(imie(x), imie(v), imie(c));
	}
	if (siz == 0) siz = 1;
	return siz;
}
void solve() {
	...
	REP(_, n - 1) {
		...
		graf[a].emplace_back(b, w, c);
		graf[b].emplace_back(a, w, c);
	}
	vpii edges[3];
	vector <ll> val[3];
	dfs(1, -1, edges);
	debug(range(edges, 1, 3));
	for (int h = 1; h <= 2; ++h) {
		ll curr = 0;
		int ops = 0;
		priority_queue<pair <ll, int> > gains;
		for (auto [v, cou] : edges[h]) curr += v * 1ll * cou;
		int siz = edges[h].size();
		REP(i, siz) gains.push({edges[h][i].second * 1ll * ceil_div(edges[h][i].first, 2), i});
		val[h].push_back(curr);
		while (curr > 0) {
			auto [v, i] = gains.top();
			gains.pop();
			curr -= v;
			edges[h][i].first /= 2;
			gains.push({edges[h][i].second * 1ll * ceil_div(edges[h][i].first, 2), i});
			val[h].push_back(curr);
			ops++;
			debug(arr(edges, h));
		}
	...
	int i = 0, j = val[2].size() - 1;
	while (i < (int) val[1].size() && j >= 0) {
		if (val[1][i] + val[2][j] <= s) {
			int sc = i + 2 * j;
			mini(ans, sc);
			j--;
		}
