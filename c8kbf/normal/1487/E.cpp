...
constant maxn = 15E4+8;

_ n1, n2, n3, n4, m1, m2, m3, x, y;
set<_> ba[maxn], bb[maxn], bc[maxn];
struct dish {
    _ c, id;
    inline bool operator < (const dish &b) const {
        return c < b.c;
    }
...
inline void AC() {
    ...
    for(_ i = 1; i <= n3; ++i) {
        _ mn = 1E11;
        for(_ j = 1; j <= n4; ++j) if(!bc[i].count(d[j].id)) {
            mn = d[j].c;
            break;
        }
        c[i].c += mn;
    ...
    for(_ i = 1; i <= n2; ++i) {
        _ mn = 1E11;
        for(_ j = 1; j <= n3; ++j) if(!bb[i].count(c[j].id)) {
            mn = c[j].c;
            break;
        }
        b[i].c += mn;
    ...
    for(_ i = 1; i <= n1; ++i) {
        _ mn = 1E11;
        for(_ j = 1; j <= n2; ++j) if(!ba[i].count(b[j].id)) {
            mn = b[j].c;
            break;
        }
        a[i].c += mn;
    ...
    _ mn = 1E11;
    for(_ i = 1; i <= n1; ++i) mn = min(mn, a[i].c);
    if(mn >= 1E11) writeln(-1);
    else writeln(mn);
