...
void solve(){
  int n; cin>>n;
  string S[3];
  for(li i = 0; i < (li)(3); ++i) cin>>S[i];

  int n0[3] = {0,0,0};
  for(li i = 0; i < (li)(3); ++i) for(li j = 0; j < (li)(2*n); ++j) if(S[i][j] == '0') n0[i]++;
  for(li a = 0; a < (li)(3); ++a) for(li b = (a+1); b <= (li)(2); ++b) {
    if(n0[a] >= n && n0[b] >= n) {
      int x=0,y=0;
      string r;
      while(x<2*n&&y<2*n) {
      ...
      while(x<2*n) { r += S[a][x]; x++; }
      while(y<2*n) { r += S[b][y]; y++; }
      cout << r << endl;
      return;
    }
    if(n0[a] <= n && n0[b] <= n) {
