...
int st[200005][20],p[200005],l2[200005];
int get(int x,int y){
	int len=l2[y-x+1];
	RE __gcd(st[x][len],st[y-(1<<len)+1][len]);
}
void solve(){
	int n;
	cin>>n;
	...
	FOR(i,1,n)FILL(st[i],0);
	FOR(i,2,n)st[i][0]=abs(p[i]-p[i-1]);
	FOR(i,1,18)FOR(j,1,n-(1<<i)+1)st[j][i]=__gcd(st[j][i-1],st[j+(1<<(i-1))][i-1]);
	int ans=1;
	int l=1;
	FOR(i,2,n){
		while(l<i&&get(l+1,i)==1)l++;
		gmax(ans,i-l+1);
	}
	cout<<ans<<'\n';
}
signed main(){
	FOR(i,2,200000)l2[i]=l2[i/2]+1;
	io.init();
	int T;
	cin>>T;
	while(T--)solve();
