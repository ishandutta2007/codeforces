...
void MAIN(){
	...
	for(int i=n-1;i>4;--i){
		...
		if(i+1<n){
			...
			if(i+2<n&&dp[i+2][0]&&(s[i]!=s[i+2]||s[i+1]!=s[i+3])){
				dp[i][0]=1;
			}
			if(dp[i][0]){
				mp[re]=1;
		...
		if(i+2<n){
			...
			if(i+3<n&&dp[i+3][1]&&(s[i]!=s[i+3]||s[i+1]!=s[i+4]||s[i+2]!=s[i+5])){
				dp[i][1]=1;
			}
			if(dp[i][1]){
				mp[re]=1;
