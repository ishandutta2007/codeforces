#line 1 "Contests/Codeforces_1540/B/main.cpp"
using namespace std;
...
#endif
#line 3 "Contests/Codeforces_1540/B/main.cpp"
...
#line 7 "Contests/Codeforces_1540/B/main.cpp"
...
mint gyakus[1000];
mint winProb[300][300];
int dones[300][300];
mint tmps(int a,int b){
    if(dones[a][b]) return winProb[a][b];
    dones[a][b] = 1;
    return winProb[a][b] = (tmps(a-1,b) + tmps(a,b-1)) / 2;
}
void solve(){
    ...
    REP(i,n){
        ...
        for(int q = 1;q < 10;++q){
            REP(j,n){
                backs[q][j] = backs[q-1][backs[q-1][j]];
            }
        }
        REP(q,n){
            for(int j = 0;j < q;++j){
                
                if(lefts[q] <= lefts[j] and rights[j] <= rights[q]){
                    ans += 1;
                    continue;
                }
                if(lefts[j] <= lefts[q] and rights[q] <= rights[j]){
                    continue;
                }
                int points = lca(j,q);
                ans += tmps(depth[q] - depth[points],depth[j] - depth[points]);
            }
        }
    }
    ans /= n;
