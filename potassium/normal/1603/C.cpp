...
int main() {
	...
	CASE(t) {
		...
		vector<map<int, int>> ps(n);
		function<int(int, int)> get = [&](int idx, int maxv) {
			if (idx <= -1) return 0;
			if (ps[idx].count(maxv))
				return ps[idx][maxv];

			int p = (a[idx] + maxv - 1) / maxv;
			int newmaxv = a[idx] / p;
			int ans = (get(idx - 1, newmaxv) + 1LL * (p - 1) * (idx + 1)) % MOD998;
