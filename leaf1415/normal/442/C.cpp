...
ll get(ll p)
{
	seg.update(p, P(inf, p));
	ll ret = min(a[getnext(p)], a[getprev(p)]);
	S.erase(P(p, a[p]));
	return ret;
}
...
int main(void)
{
	...
	rep(i, 0, n+1) S.insert(P(i, a[i]));
	
	ll ans = 0;
	while(getnext(l) < r){
		P res = seg.query(l+1, r-1);
		ans += get(res.se);
		while(a[l] <= a[getnext(l)]) l = getnext(l);
		while(a[r] <= a[getprev(r)]) r = getprev(r);
	}
	
	/*set<P> T;
	for(auto p : S){
		if(p.fi >= 2 && p.fi <= n-1) T.insert(P(p.se, p.fi));
	}
	while(sz(T)){
		auto it = T.end(); it--;
		ans += get(it->se);
	...
	vector<ll> vec;
	for(auto p : S) if(p.fi >= 1 && p.fi <= n) vec.pb(p.se);
	sort(all(vec)), reverse(all(vec));
	
	rep(i, 2, sz(vec)-1) ans += vec[i];
	outl(ans);
