...
public class Main
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=1; i<n-1; i++)
			gra[i].inflow/=2;
		Queue<Integer> lala=new ArrayDeque<Integer>();
		lala.add(0);
		while(!lala.isEmpty())
		{
			int te=lala.poll();
			for(int i=0,j,k,l; i<gra[te].adj.size; i++)
			{
				j=gra[te].adj.a[i];
				if(gra[j].inflow!=0)
				{
					...
					l=gra[te].dire.a[i];
					gra[j].inflow-=k;
					dir[hola.get(min(te,j)*(n+0L)+max(te,j))]=l;
					if(gra[j].inflow==0)
						lala.add(j);
				}
			}
	...
	static class node
	{
		...
		vector wei=new vector();
		vector dire=new vector();
	}
