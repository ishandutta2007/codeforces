...
array<int, 10> s = {};
int ansv;
int value(const array<int, 10> &s) {
	int v = 0;
	repeat (i, 0, n) v += a[i][s[i]];
	return v;
}
bool work(const array<int, 10> &s) {
	if (!st.count(s) && ansv < value(s)) {
		ansv = value(s);
		ans = s;
	}
	return !st.count(s);
...
void Solve() {
	...
	repeat (i, 0, n) s[i] = a[i].size() - 1;
	dfs(0);
	repeat (i, 0, n) print(ans[i] + 1);
	puts("");
}
signed main() {
	int T = 1; // T = read();
