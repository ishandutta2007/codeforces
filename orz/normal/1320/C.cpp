...
struct st
{
	...
	void add(const int v, const int vx, const int vy, const int qx, const int qy, const ll val)
	{
		push(v, vx, vy);
		if ((vx >= qy) || (qx >= vy))
			return;
		...
		{
			int vz = (vx + vy) / 2;
			add(2 * v + 1, vx, vz, qx, qy, val);
			add(2 * v + 2, vz, vy, qx, qy, val);
			mx[v] = max(mx[2 * v + 1], mx[2 * v + 2]);
		}
	}
	ll find_max(const int v, const int vx, const int vy, const int qx, const int qy)
	{
		push(v, vx, vy);
		if ((vx >= qy) || (qx >= vy))
			return -1e9;
		...
		{
			int vz = (vx + vy) / 2;
			return max(find_max(2 * v + 1, vx, vz, qx, qy), find_max(2 * v + 2, vz, vy, qx, qy));
		}
	}
};
...
int another_main()
{
	...
	sort(a.begin(), a.end());
	for (int i = n - 1; i > 0; --i)
		a[i - 1].Y = min(a[i - 1].Y, a[i].Y);
	sort(b.begin(), b.end());
	for (int i = m - 1; i > 0; --i)
		b[i - 1].Y = min(b[i - 1].Y, b[i].Y);
	...
	for (int i = 0; i < m; ++i)
		if (i == 0)
			t.add(0, 0, M, 0, b[i].X, -b[i].Y);
		else
			t.add(0, 0, M, b[i - 1].X, b[i].X, -b[i].Y);
	int j = 0;
	for (int i = 0; i < n; ++i)
	{
		while (j < p && c[j].X.X < a[i].X)
		{
			t.add(0, 0, M, c[j].X.Y, M, c[j].Y);
			++j;
		}
		ans = max(ans, t.find_max(0, 0, M, 0, b.back().X) - a[i].Y);
	}
	cout << ans << '\n';
	return 0;
}
