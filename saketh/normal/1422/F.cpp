...
int main() {
    ...
    segment_tree_persistent<mn, decltype(prod)> st(N, mn(1), prod);

    vector<stack<pii>> locs(MAXV);
    ...
    F0R (i, N) {
        trav (pf, sv.factor(a[i])) {
            ...
            int tf = 0;
            while (!locs[p].empty() && tf + locs[p].top().s <= m) {
                auto [inx, inx_ct] = locs[p].top();
