...
// ----- library -------
int calc(int lim, vector<pair<int, int>> v) {
    int n = v.size();
    v.pb(1e9, 1e9);
    int LOG = 1;
    while ((1 << LOG) <= n)
        LOG++;
    vector<vector<int>> dp(n + 1, vector<int>(LOG, n));
    queue<int> que;
    rep(i, n) {
        while (!que.empty()) {
            int now = que.front();
    ...
    rep(t, LOG - 1) rep(i, n) dp[i][t + 1] = dp[dp[i][t]][t];
    int ret = 0;
    rep(i, n) {
        int now = i, val = 1;
        rep2(t, LOG) if (v[dp[now][t]].first - v[i].second <= lim) now = dp[now][t], val += (1 << t);
        chmax(ret, val);
    }
    return ret;
}
