...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let _n: usize = cin.read();
    let s: String = cin.read();

    let mut left = ('a'..='z').collect::<BTreeSet<char>>();
    let mut next = BTreeMap::<char, char>::new();
    ...
    for c in s.chars() {
        if next.get(&c).is_none() {
            let to: char = *left
                .iter()
                .find(|to| {
                    dsu.par(c as usize - 'a' as usize) != dsu.par((**to) as usize - 'a' as usize)
                        || left.len() == 1
                })
                .unwrap();
            left.remove(&to);
            next.insert(c, to);
            dsu.un(c as usize - 'a' as usize, to as usize - 'a' as usize);
        }
        t.push(next[&c]);
    }
    outln!(t);
...
mod __cargo_equip {
    pub(crate) mod crates {
        pub mod rlib_dsu {#[derive(Clone,Debug)]pub struct DSU{pub p:Vec<usize>,pub sz:Vec<usize>,}impl DSU{pub fn new(n:usize)->Self{Self{p:(0..n).collect(),sz:vec![1;n],}}pub fn reset(&mut self,n:usize){self.p.resize(n,0);for i in 0..n{self.p[i]=i;}self.sz.resize(n,0);for i in 0..n{self.sz[i]=1;}}pub fn par(&mut self,v:usize)->usize{if self.p[v]!=v{self.p[v]=self.par(self.p[v]);}self.p[v]}pub fn un(&mut self,mut u:usize,mut v:usize)->bool{u=self.par(u);v=self.par(v);if u==v{return false;}if self.sz[u]>self.sz[v]{std::mem::swap(&mut u,&mut v);}self.sz[v]+=self.sz[u];self.p[u]=v;true}}}
