...
int main() {
	...
	vector<Mint> fac(MAXN, Mint(1, MOD998)), inv(MAXN, Mint(1, MOD998));
	...
	auto gen = [&](int s, int p, int maxs) {
		if (p == 0)
			return Mint(s == 0, MOD998);
		...
		for (int take = 0; take <= p; take++) {
			...
			ans += (take % 2 ? -1 : 1) * C(p, take) * C(p + s - pw - 1, s - pw);
		}
	...
	for (int score = r; score <= s; score++) {
		for (int maxp = 1; maxp <= p; maxp++) {
			int rems = s - score * maxp;
			int remp = p - maxp;
			
			if (rems < 0) break;
			
			ans += C(p - 1, maxp - 1) * gen(rems, remp, score - 1) / maxp;
		}
	}
