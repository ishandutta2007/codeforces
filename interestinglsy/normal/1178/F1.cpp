...
ll Dfs( int l , int r ){
	if( l > r ) return 1;
	if(KNOW) return NOW;
	int mincol = INF; Forx(i,l,r) Mymin(mincol,col[i]);
	if( poses[mincol][0] < l or poses[mincol].back() > r ) return NOW = 0;
	// cout << l << " " << r << endl;

	ll ans = 1;
	int lastapp = -1;
	for( auto p : poses[mincol] ){
		if( lastapp != -1 ) ( ans *= Dfs(lastapp+1,p-1) ) %= MOD;
		lastapp = p;
	}
	ll lsum = 0, rsum = 0;
	fORx(i,l,poses[mincol][0]) ( lsum += Dfs(l,i-1) * Dfs(i,poses[mincol][0]-1) ) %= MOD;
	Forx(i,poses[mincol].back(),r) ( rsum += Dfs(poses[mincol].back()+1,i) * Dfs(i+1,r) ) %= MOD;
	ans = ans * lsum % MOD * rsum % MOD;
	return NOW = ans;
}
