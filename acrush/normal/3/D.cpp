...
const int64 oo=1000000000LL*100000000LL;

class Node
{
public:
	int s_delta,max_delta;
	int min_value,max_value;
};

int n,m;
int64 D[maxn],CA[maxn],CB[maxn];
char S[maxn];
int pos[maxn];
pair<int64,int> Q[maxn];
int A[maxn];
Node tree[maxn*8];
...
{
	node.s_delta=A[key];
	node.max_delta=max(0,A[key]);
	node.min_value=(A[key]>0)?key:-1;
	node.max_value=(A[key]<0)?key:-1;
}
...
{
	node.s_delta=left.s_delta+right.s_delta;
	node.max_delta=max(left.max_delta,left.s_delta+right.max_delta);
	node.min_value=min2(left.min_value,right.min_value);
	node.max_value=max2(left.max_value,right.max_value);
}
void initTree(int id,int H,int T)
{
	if (H==T)
	{
		build(tree[id],H);
	...
	int M=(H+T)/2;
	initTree(id*2+1,H,M);
	initTree(id*2+2,M+1,T);
	combine(tree[id],tree[id*2+1],tree[id*2+2]);
}
int findBad(int id,int H,int T,int p)
{
	if (p+tree[id].max_delta<=0) return -1;
	if (H==T) return H;
	int M=(H+T)/2;
	int t=findBad(id*2+1,H,M,p);
	if (t>=0) return t;
	return findBad(id*2+2,M+1,T,p+tree[id*2+1].s_delta);
}
int findMin(int id,int H,int T,int s,int t)
{
	if (H>=s && T<=t) return tree[id].min_value;
	int r=-1;
	int M=(H+T)/2;
	if (M>=s)
	{
		int t0=findMin(id*2+1,H,M,s,t);
		r=min2(r,t0);
	}
	if (M<t)
	{
		int t0=findMin(id*2+2,M+1,T,s,t);
		r=min2(r,t0);
	}
	return r;
}
int findMax(int id,int H,int T,int s,int t)
{
	if (H>=s && T<=t) return tree[id].max_value;
	int r=-1;
	int M=(H+T)/2;
	if (M>=s)
	{
		int t0=findMax(id*2+1,H,M,s,t);
		r=max2(r,t0);
	}
	if (M<t)
	{
		int t0=findMax(id*2+2,M+1,T,s,t);
		r=max2(r,t0);
	}
	return r;
}
void updateTree(int id,int H,int T,int key)
{
	if (H==T)
	{
		build(tree[id],H);
	...
	int M=(H+T)/2;
	if (key<=M)
		updateTree(id*2+1,H,M,key);
	else
		updateTree(id*2+2,M+1,T,key);
	combine(tree[id],tree[id*2+1],tree[id*2+2]);
}
int main()
{
	...
	for (int i=n/2;i<n;i++) A[Q[i].second]=1;
	initTree(0,0,n-1);
	while (1)
	{
		int p=findBad(0,0,n-1,0);
		if (p<0) break;
		int d=findMin(0,0,n-1,0,p);
		A[d]=-1;
		updateTree(0,0,n-1,d);
		int e=findMax(0,0,n-1,p+1,n-1);
		A[e]=1;
		updateTree(0,0,n-1,e);
	}
	int64 R=0;
	for (int i=0;i<n;i++)
		if (A[i]==-1)
