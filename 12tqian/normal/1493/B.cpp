...
int main() {
    ...
    f0r(i, 100) {
        nums[i] = to_string(i);
        while (sz(nums[i]) < 2) nums[i] = "0" + nums[i];
        conv[nums[i]] = i;
    }
    int tt; re(tt);
    while (tt--) {
        ...
        f0r(i, h) {
            f0r(j, m) {
                ...
                f0r(i, 2) {
                    ...
                    s1[i] = (char)(f[s1[i]-'0'] + '0');
                    s2[i] = (char)(f[s2[i]-'0'] + '0');
                }
                if (!ok) continue;
                int x = stoi(s2);
                int y = stoi(s1);
                ...
                if (x < h && y < m) {
                    ckmin(ans, {dist(ti(i, j) - ti(xx, yy)), i, j});
                }
            }
        }
        pr(nums[ans[1]], ":", nums[ans[2]]);
        ps();
