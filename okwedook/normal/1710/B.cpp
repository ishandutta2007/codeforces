...
struct Arithm {
    ...
    ll sumc = 0;
    ll cntopen = 0;
    void process(ll t) { // process everyting up to time t
        while (ptr < sz(p) && p[ptr].f <= t) {
            auto [c, open] = p[ptr].s;
            sumc += c * open;
            cntopen += open;
            ++ptr;
    ...
    ll evalL(ll t) const {
        return cntopen * t - sumc;
    }
    ll evalR(ll t) const {
        return sumc - cntopen * t;
    }
};
...
void solve() {
    ...
    for (int i = 0; i < sz(pt); ++i) {
        ...
        al.process(t);
        ar.process(t);
        val[i] = al.evalL(t) + ar.evalR(t);
    }
    vector<ll> valplust(sz(pt)), valminust(sz(pt));
    for (int i = 0; i < sz(pt); ++i) {
        valplust[i] = val[i] + pt[i];
        valminust[i] = val[i] - pt[i];
    }
    dbg(valminust);
    Tree<node> tval(val), tplus(valplust), tminus(valminust);
    vector<bool> ans;
    for (auto [xp, lr] : segm) {
        auto [x, p] = xp;
        auto [l, r] = lr;
        auto ql = lower_bound(all(pt), l) - pt.begin();
        auto qx = lower_bound(all(pt), x) - pt.begin();
        auto qr = upper_bound(all(pt), r) - pt.begin();
        dbg(ql, qx, qr);
        ll check = 0;
        chmax(check, tval.getup(0, ql).mx);
        chmax(check, tval.getup(qr, sz(pt)).mx);
        dbg(check);
        dbg(tminus.getup(ql, qx).mx);
        chmax(check, tminus.getup(ql, qx).mx - p + x);
        dbg(check);
        chmax(check, tplus.getup(qx, qr).mx - p - x);
        ans.pb(check <= m);
    }
