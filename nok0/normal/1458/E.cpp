...
struct line {
	//y=x+a
	int sx, gx, a;
	bool operator<(const line &oth) const { return sx < oth.sx; }
};
...
void main_() {
	...
	map<int, int> xminy, yminx;
	for(auto [x, y] : b) {
		if(!xminy.count(x)) xminy[x] = y;
		chmin(xminy[x], y);
		if(!yminx.count(y)) yminx[y] = x;
		chmin(yminx[y], x);
	}
	...
	V<line> lines;
	while(nx != inf * 2 and ny != inf * 2) {
		debug(nx, ny);
		if(xminy.count(nx) or yminx.count(ny)) {
			int dx = (xminy.count(nx) ? xminy[nx] <= ny : 0);
			int dy = (yminx.count(ny) ? yminx[ny] <= nx : 0);
			nx += dx;
			ny += dy;
			if(dx + dy == 0) {
				lines.pb(line{nx, nx + 1, ny - nx});
				nx++, ny++;
			}
		} else {
			auto new_x = *xp.upper_bound(nx);
			auto new_y = *yp.upper_bound(ny);
			if(new_x - nx <= new_y - ny) {
				lines.pb(line{nx, new_x, ny - nx});
				ny += new_x - nx, nx = new_x;
			} else {
				debug(nx, nx + new_y - ny, ny - nx);
				lines.pb(line{nx, nx + new_y - ny, ny - nx});
				nx += new_y - ny, ny = new_y;
			}
		}
	}
	...
	while(m--) {
		...
		if(out.count({x, y}))
			...
			auto itr = lower_bound(all(lines), line{x + 1, -inf, -inf});
			if(itr != lines.begin()) {
				auto [l, r, a] = *(prev(itr));
				if(l <= x and x < r and x + a == y) win = 0;
			}
		}
		print(win ? "WIN" : "LOSE");
	}
