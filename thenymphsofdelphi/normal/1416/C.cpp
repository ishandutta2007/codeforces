...
struct Vertex{
    ...
    Vertex (int p = -1, bool pch = 0): p(p), pch(pch) {
        next[0] = next[1] = -1;
    }
};
...
void add(int x, int pos){
    ...
    FordE(i, M, 0){
        bool cac = ((x & (1 << i)) >> i);
        if (trie[cur].next[cac] == -1){
            trie[cur].next[cac] = trie.size();
            trie.eb(cur, cac);
        }
        cur = trie[cur].next[cac];
    }
    finpos[cur].pb(pos);
}
...
unordered_map <int, ordered_set <int>> stt;

ll inv1[M + 2], inv2[M + 2];

void dfs(int u, int h){
    int mxsz = 0; bool idmxsz = 0;
    if (trie[u].next[0] != -1){
        dfs(trie[u].next[0], h - 1);
        sz[u] += sz[trie[u].next[0]];
        if (sz[trie[u].next[0]] > mxsz){
            mxsz = sz[trie[u].next[0]];
            idmxsz = 0;
        }
    }
    if (trie[u].next[1] != -1){
        dfs(trie[u].next[1], h - 1);
        sz[u] += sz[trie[u].next[1]];
        if (sz[trie[u].next[1]] > mxsz){
            mxsz = sz[trie[u].next[1]];
            idmxsz = 1;
        }
    }
    vidmxsz[u] = idmxsz;
    ...
    if (trie[u].next[0] != -1 && trie[u].next[1] != -1){
        int idxstt0 = trie[u].next[0], idxstt1 = trie[u].next[1];
        while (stt.find(idxstt0) == stt.end()){
            idxstt0 = trie[idxstt0].next[vidmxsz[idxstt0]];
        }
        while (stt.find(idxstt1) == stt.end()){
            idxstt1 = trie[idxstt1].next[vidmxsz[idxstt1]];
        }
        ll tinv = 0;
        if (idmxsz){
            stt[u].swap(stt[idxstt1]);
            Fora(&v, stt[idxstt0]){
                tinv += stt[u].order_of_key(v);
            }
            Fora(&v, stt[idxstt0]){
                stt[u].insert(v);
            }
            inv1[h] += tinv;
            inv2[h] += (ll)sz[trie[u].next[0]] * sz[trie[u].next[1]] - tinv;
        }
        else{
            stt[u].swap(stt[idxstt0]);
            Fora(&v, stt[idxstt1]){
                tinv += stt[u].order_of_key(v);
            }
            Fora(&v, stt[idxstt1]){
            ...
            inv2[h] += tinv;
            inv1[h] += (ll)sz[trie[u].next[0]] * sz[trie[u].next[1]] - tinv;
        }
        // cout << u << ' ' << idmxsz << ' ' << idxstt0 << ' ' << idxstt1 << ' ' <<
        //         tinv << ' ' << (ll)sz[trie[u].next[0]] * sz[trie[u].next[1]] - tinv << endl;
    }
}
