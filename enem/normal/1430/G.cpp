...
static const ll N=18;
static ll n,m;
static V(pll) ad[N],rad[N];
static bool dpf[1<<N];
static ll dp[1<<N];
static ll dpv[1<<N][N];
static ll dpp[1<<N];
...
int main()
{
    ...
    REP(t,1,(1<<n))
    {
        ...
        REP(i,0,n)
        {
            ...
            {
                ll up,dw,vl=0;
                bool upf=false,dwf=false;
                for(auto it:ad[i])
                {
                    if(t&(1<<it.F))
                    {
                        if(dwf==false||dw<=dpv[t^(1<<i)][it.F])
                        {
                            dwf=true;
                            dw=dpv[t^(1<<i)][it.F]+1;
                        }
                ...
                for(auto it:rad[i])
                {
                    if(t&(1<<it.F))
                    {
                        if(upf==false||up>=dpv[t^(1<<i)][it.F])
                        {
                            upf=true;
                            up=dpv[t^(1<<i)][it.F]-1;
                        }
                ...
                if(upf==false||dwf==false||up>=dw)
                {
                    ...
                    else
                    {
                        if(upf==false) 
                        {
                            if(dwf==false) ivl=100000;
                            else ivl=dw;
                        }
                        else ivl=up;
                    }
                    if(dpf[t]==false || dp[t^(1<<i)] + vl*ivl<dp[t])
                    {
                        dpf[t]=true;
                        dpp[t]=(t^(1<<i));
                        dp[t]=dp[t^(1<<i)]+vl*ivl;
                        REP(j,0,n) dpv[t][j]=dpv[t^(1<<i)][j];
                        dpv[t][i]=ivl;
                    }
                }
            }
