...
bool mem[MAXN][MAXN][MAXC];
// mem[i][j][k](Maxi,Lucasj,k,k+1,k+2,...,'z'),
// Max1,Lucas0
#define NOW mem[u][v][c]
#define KNOW vis[u][v][c]
bool Dfs( int u , int v , int c ){
	...
	For0(i,e[u].size()){
		Edge now = e[u][i];
		if( now.label < c ) continue;
		bool tans = Dfs(v,now.to,now.label);
		NOW |= !tans;
	}
	return NOW;
}
