...
void chemthan() {
    ...
    auto check = [&] (int u, int v, int w) {
        return tree.dist(u, v) + tree.dist(v, w) == tree.dist(u, w);
    };
    ...
    vi can(n, 1);
    while (1 < accumulate(all(can), 0)) {
        pi mx;
        function<void(int, int, int)> dfs = [&] (int u, int p, int lev) {
            chkmax(mx, mp(lev, u));
            for (int v : adj[u]) {
                if (v != p && can[v]) {
                    dfs(v, u, lev + 1);
