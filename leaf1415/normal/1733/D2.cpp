...
int main(void)
{
	...
	while(T--){
		...
		rep(j, 0, n) rep(k, 0, 1) rep(l, 0, 1) dp[0][j][k][l] = inf;
		dp[0][0][0][0] = 0;
		
		rep(i, 0, n-1){
			ll d = t[i+1]-'0';
			rep(j, 0, n) rep(k, 0, 1) rep(l, 0, 1) dp[(i+1)&1][j][k][l] = inf;
			rep(j, 0, n){
				rep(k, 0, 1){
					rep(l, 0, 1){
						ll c = s[i+1]-'0';
						c ^= k;
						if(c == d) chmin(dp[(i+1)&1][j][0][0], dp[i&1][j][k][l]);
						else{
							chmin(dp[(i+1)&1][j][1][0], dp[i&1][j][k][l]+min(x, 2*y));
							chmin(dp[(i+1)&1][j+1][0][1], dp[i&1][j][k][l]+y);
							if(j-1 >= 0 && !(j == 1 && l == 1)){
								chmin(dp[(i+1)&1][j-1][0][0], dp[i&1][j][k][l]);
							}
						}
					}
				}
