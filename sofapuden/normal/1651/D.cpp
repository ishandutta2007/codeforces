...
void solve(){
	...
	map<pair<int,int>,array<int,3>> M1, M2, M3, M4;

	{
		sort(v.begin(),v.end());
		for(int j = 0; j < n; ++j){
			if(M1.count({v[j].first+dx[0],v[j].second+dy[0]})){
				M1[v[j]] = M1[{v[j].first+dx[0],v[j].second+dy[0]}];
				M1[v[j]][0]++;
			}
			else{
				M1.erase({v[j].first+dx[0],v[j].second+dy[0]});
				M1[v[j]] = {1,v[j].first+dx[0],v[j].second+dy[0]};
			}
			if(M1.count({v[j].first+dx[1],v[j].second+dy[1]})){
				if(M1[{v[j].first+dx[1],v[j].second+dy[1]}][0]+1 < M1[v[j]][0]){
					M1[v[j]] = M1[{v[j].first+dx[1],v[j].second+dy[1]}];
					M1[v[j]][0]++;
				}
			}
			else{
				M1.erase({v[j].first+dx[1],v[j].second+dy[1]});
				M1[v[j]] = {1,v[j].first+dx[1],v[j].second+dy[1]};
			}
		}
	}
	{
		...
		for(int j = 0; j < n; ++j){
			if(M2.count({v[j].first+dx[2],v[j].second+dy[2]})){
				M2[v[j]] = M2[{v[j].first+dx[2],v[j].second+dy[2]}];
				M2[v[j]][0]++;
			}
			else{
				M2.erase({v[j].first+dx[2],v[j].second+dy[2]});
				M2[v[j]] = {1,v[j].first+dx[2],v[j].second+dy[2]};
			}
			if(M2.count({v[j].first+dx[3],v[j].second+dy[3]})){
				if(M2[{v[j].first+dx[3],v[j].second+dy[3]}][0]+1 < M2[v[j]][0]){
					M2[v[j]] = M2[{v[j].first+dx[3],v[j].second+dy[3]}];
					M2[v[j]][0]++;
				}
			}
			else{
				M2.erase({v[j].first+dx[3],v[j].second+dy[3]});
				M2[v[j]] = {1,v[j].first+dx[3],v[j].second+dy[3]};
			}
		}
	}
	{
		...
		for(int j = 0; j < n; ++j){
			if(M3.count({v[j].first+dx[3],v[j].second+dy[3]})){
				M3[v[j]] = M3[{v[j].first+dx[3],v[j].second+dy[3]}];
				M3[v[j]][0]++;
			}
			else{
				M3.erase({v[j].first+dx[3],v[j].second+dy[3]});
				M3[v[j]] = {1,v[j].first+dx[3],v[j].second+dy[3]};
			}
			if(M3.count({v[j].first+dx[0],v[j].second+dy[0]})){
				if(M3[{v[j].first+dx[0],v[j].second+dy[0]}][0]+1 < M3[v[j]][0]){
					M3[v[j]] = M3[{v[j].first+dx[0],v[j].second+dy[0]}];
					M3[v[j]][0]++;
				}
			}
			else{
				M3.erase({v[j].first+dx[0],v[j].second+dy[0]});
				M3[v[j]] = {1,v[j].first+dx[0],v[j].second+dy[0]};
			}
		}
	}
	{
		...
		for(int j = 0; j < n; ++j){
			if(M4.count({v[j].first+dx[1],v[j].second+dy[1]})){
				M4[v[j]] = M4[{v[j].first+dx[1],v[j].second+dy[1]}];
				M4[v[j]][0]++;
			}
			else{
				M4.erase({v[j].first+dx[1],v[j].second+dy[1]});
				M4[v[j]] = {1,v[j].first+dx[1],v[j].second+dy[1]};
			}
			if(M4.count({v[j].first+dx[2],v[j].second+dy[2]})){
				if(M4[{v[j].first+dx[2],v[j].second+dy[2]}][0]+1 < M4[v[j]][0]){
					M4[v[j]] = M4[{v[j].first+dx[2],v[j].second+dy[2]}];
					M4[v[j]][0]++;
				}
			}
			else{
				M4.erase({v[j].first+dx[2],v[j].second+dy[2]});
				M4[v[j]] = {1,v[j].first+dx[2],v[j].second+dy[2]};
			}
		}
	}
	for(int i = 0; i < n; ++i){
		array<int,3> a = M1[v2[i]];
		array<int,3> b = M2[v2[i]];
		array<int,3> c = M3[v2[i]];
		array<int,3> d = M4[v2[i]];
		if(a < b && a < c && a < d){
			cout << a[1] << ' ' << a[2] << '\n';
		}
		else if(b < c && b < d){
