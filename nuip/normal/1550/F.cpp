...
struct Min{
	...
	Min(pii mn):mn(mn){}
	Min():mn({MOD,-1}){}
	Min operator+(const Min &seg){
		return Min(min(mn,seg.mn));
	}
  static Min e;
};
Min Min::e=Min();
ostream&  operator<<(ostream &os, const Min &seg){ return os<<"("<<seg.mn<<")";}

struct Line{
	int pos,neg;
  Line(int pos=MOD,int neg=MOD):pos(pos),neg(neg){}
	void operator+=(const Line &lfun){
		MN(pos,lfun.pos);
		MN(neg,lfun.neg);
	}
	bool operator==(const Line &lfun){ return pos==lfun.pos && neg==lfun.neg;}

	void eval(Min &s,int l,int r){
		if(cauto it=rem.lower_bound(l); *it<r) MN(s.mn,{pos+vs[*it],*it});
		if(cauto it=prev(rem.lower_bound(r)); l<=*it) MN(s.mn,{neg-vs[*it],*it});
	}
	static Line e;
};
Line Line::e=Line();
ostream&  operator<<(ostream &os, const Line &lfun){ return os<<"("<<lfun.pos<<","<<lfun.neg<<")";}
...
struct LazySegTree{
	...
	void upd(int l,int r,int k,int a,int b,int val){
		lazy_eval(k,a,b);
		if(b<=l || r<=a) return; 
		if(l<=a && b<=r){
			segs[k]=Min({val,l});
		...
		int m=(a+b)/2;
		upd(l,r,2*k+1,a,m,val);
		upd(l,r,2*k+2,m,b,val);
		upd_node(k);
	}
...
int main(){
	...
	cauto d=read();
	vs=readV(n,0);
	cauto qs=readV(q,pii{1,0});
	reps(i,-1,n) rem.insert(i);
	rem.insert(MOD);
	vector<int> inds(q); iota(all(inds),0);
	sort(all(inds),[&](cauto &i,cauto &j){return qs[i].Y<qs[j].Y;});
	LazySegTree<Min,Line> st(n);
	vector<int> re(q);
	cauto upd=
		[&](cauto i){
			rem.erase(i);
			st.upd(i,i+1,0,0,st.segs.size()/2,MOD);
			st(i,i+1,{MOD,MOD});
			cauto l=lower_bound(all(vs),vs[i]-d)-vs.begin();
			cauto r=lower_bound(all(vs),vs[i]+d)-vs.begin();
			st(0,l,{MOD,vs[i]-d});
			st(l,i,{d-vs[i],MOD});
			st(i,r,{MOD,vs[i]+d});
			st(r,n,{-d-vs[i],MOD});
		};
	// rep(i,n) cout<<st(i,i+1)<<","; cout NL;
	upd(s);
	// rep(i,n) cout<<st(i,i+1)<<","; cout NL;
	int cur=0;
	for(cauto &qi:inds){
