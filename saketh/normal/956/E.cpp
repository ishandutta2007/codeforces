...
int main() {
    ...
    vi boxes(N); re(boxes);
    vi is_good(N); re(is_good);

    const int H = accumulate(all(boxes), 0);
    ...
    F0R (i, N) {
        if (is_good[i])
            ...
            prefix |= prefix << boxes[i];
    }

    vi dp(H + 1, -1e9);
    F0R (h, sz(dp)) if (prefix[h]) dp[h] = 0;

    auto score = [&](int top, int bsz, int sub) {
        int first = -sub + 1, last = 0;
        // make sure top + first * bsz >= H - R
        ckmax(first, ceil(H - R - top, bsz));

        // make sure top + last * bsz <= H - L
        ckmin(last, floor(H - L - top, bsz));

        int res = max(0, last - first + 1);
        //pd(top, bsz, sub, res);
        return res;
    };

    FORd (bsz, 1, H + 1) {
        int ct = good[bsz], sub = 1;
        while (ct) {
            int pair = (ct - 1) / 2;
            for (int rep = 0; rep < ct - 2 * pair; rep++) {
                F0Rd (h, sz(dp) - bsz * sub) {
                    int top = h + bsz * sub;
                    ckmax(dp[top], dp[h] + score(top, bsz, sub));
                }
            }
            ct = pair;
            sub *= 2;
