...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  Str S;
  in >> S;
  Vec<int> col_next(N * M, N * M);
  Vec<int> col_reg(M, N * M);
  for (int i = N * M - 1; i >= 0; i--) {
    col_next[i] = col_reg[i % M];
    if (S[i] == '1') col_reg[i % M] = i;
  }
  Vec<i64> delta_col_ans(N * M);
  Vec<i64> delta_row_ans(N * M);
  var recover = [&](Vec<i64> &delta) {
    for(int i = 1; i < Size(delta); i++) {
      delta[i] += delta[i - 1];
    }
  };
  var update = [&](Vec<i64>& delta_ans, int l, int r, int val) {
    Assert(l <= r);
    DebugRun({
      if(l <= 5 && r >= 5) {
        DebugFmtln("[%d, %d] => %d", l, r, val);
      }
    });
    if(l < Size(delta_ans)) {
      delta_ans[l] += val;
    }
    if (r + 1 < Size(delta_ans)) {
      delta_ans[r + 1] -= val;
    }
  };
  for (int i = 0; i < N * M; i++) {
    if (S[i] == '1') {
      int next_occur = col_next[i];
      update(delta_col_ans, i, next_occur - 1, 1);
    }
  }
  recover(delta_col_ans);
  if(M >= 100) {
    //N2
    int first = -1;
    for(int i = 0; i < N * M; i++) {
    ...
    if(first >= 0) {
      for(int i = 0; i < N; i++) {
        int l = first + i * M;
        update(delta_row_ans, l, N * M, 1);
      }
      Vec<Pair<int, int>> large_zero_blocks;
      for(int i = first; i < N * M; i++) {
        ...
        i = r;
        if(r - l + 1 >= M)
        large_zero_blocks.push_back(MakePair(l, r));
      }
      Line(calc row);
      for(int i = 0; i < N; i++) {
        Debug(i);
        for(var &b : large_zero_blocks) {
          int start = b.first + i * M + M - 1;
          int end = b.second + i * M;
          update(delta_row_ans, start, end, -1);
        }
      }
    }
    recover(delta_row_ans);
  } else {
    //NMM
    Vec<Vec<i64>> delta_cols(M, Vec<i64>(N));
    Vec<int> next_item(N * M);
    int next_item_loc = N * M;
    for(int i = N * M - 1; i >= 0; i--) {
      next_item[i] = next_item_loc;
      if(S[i] == '1') {
        next_item_loc = i;
      }
    }
    for(int i = 0; i < N * M; i++) {
      ...
      int final_row = N - 1 - i / M;
      int final_col = M - 1 - i % M;
      int dist = next_item[i] - i;
      //0 ~ dist - 1
      dist = Min(dist, M);
      for(int j = 0; j < dist; j++) {
        ...
        if(h >= 0) {
          ...
          //0 ~ h
          //time: j + i + km
          int step_forward = (j + i) / M;
          update(delta_cols[(j + i) % M], 0 + step_forward, h + step_forward, 1);
        }
      }
    }
    for(int i = 0; i < M; i++) {
      recover(delta_cols[i]);
    }
    for(int i = 0; i < N; i++) {
      for(int j = 0; j < M; j++) {
        delta_row_ans[i * M + j] = delta_cols[j][i];
      }
    }
  }
  Debug(delta_col_ans);
  Debug(delta_row_ans);
  for(int i = 0; i < N * M; i++) {
    var ans = delta_row_ans[i] + delta_col_ans[i];
    out << ans << ' ';
  }
  out << '\n';
}
