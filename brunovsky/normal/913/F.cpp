...
int main() {
    ...
    num p = num(A) / num(B);
    num q = 1 - p;
    vector<num> pow(N * N + 1), qow(N * N + 1);
    for (int i = 0; i <= N * N; i++) {
        pow[i] = i ? p * pow[i - 1] : 1;
        qow[i] = i ? q * qow[i - 1] : 1;
    }
    vector<vector<num>> select(N + 1, vector<num>(N + 1));
    for (int n = 1; n <= N; n++) {
        select[n][n] = select[n][0] = 1;
        for (int s = 1; s < n; s++) {
            select[n][s] = pow[n - s] * select[n - 1][s - 1] + qow[s] * select[n - 1][s];
        }
    }
    vector<num> full(N + 1, 1);
    for (int n = 2; n <= N; n++) {
        for (int s = 1; s < n; s++) {
            full[n] -= full[s] * select[n][s];
        }
    }
    vector<num> dp(N + 1);
    for (int n = 2; n <= N; n++) {
        for (int s = 1; s < n; s++) {
            num c = s * (s - 1) / 2 + s * (n - s);
            dp[n] += (c + dp[s] + dp[n - s]) * full[s] * select[n][s];
        }
        dp[n] = (full[n] * n * (n - 1) / 2 + dp[n]) / (1 - full[n]);
    }
    cout << dp[N] << '\n';
    return 0;
}