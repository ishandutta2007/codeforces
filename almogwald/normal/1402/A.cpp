...
lol costii(vecp &ord, lol slope) {
	if (ord.size() == 0) {
		return lmaxi;
	}
	int low = 0,high=ord.size()-1;
	while (low < high) {
		int mid = (low + high) / 2;
		lol cost1 = slope * ord[mid].ff + ord[mid].ss;
		lol cost2 = slope * ord[mid+1].ff + ord[mid+1].ss;
		if (cost1 < cost2) {
			high = mid;
		}
		else {
	...
	lol cost1 = slope * ord[low].ff + ord[low].ss;
	return cost1;
}
lol costi(vector<vector<vecp>>& segtre, int l, int r, lol slope) {
	lol mincost = lmaxi;
	fori(i, logn) {
		if (l >= r) {
			br
		...
		if (l % 2 == 1 ) {
			min2(mincost, costii(segtre[i][l++],slope))
		}
		if (r % 2 == 1) {
			min2(mincost, costii(segtre[i][--r], slope))
		}
		l /= 2;
		r /= 2;
	}
...
void add(vecp& c, point p) {
	...
	while (true) {
		...
		if (c.back().ff <= p.ff  && c.back().ss <= p.ss) {
			return;
		}
		if (c.size() == 1) {
			c.push_back(p);
		...
		point a = c[c.size() - 2];
		point b = c.back();
		lol difxab = a.ff - b.ff;
		lol difyab = b.ss - a.ss;
		ldb ab = difyab * 1.0 / difxab;
		lol difxap = b.ff - p.ff;
		lol difyap = p.ss - b.ss;
		ldb ap = difyap * 1.0 / difxap;
		if (ab >= ap) {
			c.pop_back();
			con
		}
