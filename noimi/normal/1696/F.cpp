...
int main() {
    TEST {
        ...
        //         if((d[i][j] == d[i][k]) and j != k) a[i][j][k] = true;
        //     }
        // }
        // OJ {
        rep(i, n - 1) {
        ...
        auto check = [&](Graph &g) {
            ...
            rep(i, n) rep(j, n) rep(k, j) {
                if(a[i][j][k]) flag &= (d[i][j] == d[i][k] and d[i][j] < n);
                if(d[i][j] == d[i][k] and d[i][j] < n) flag &= (a[i][j][k]);
            }
            return flag;
        };
        ...
        rep(x, 1, n) {
            ...
            if(check(g)) {
                Yes();
                rep(i, n) fore(e, g[i]) if(i < e) OUT(i + 1, e + 1);
                break;
            }
            if(x == n - 1) { No(); }
        }
