...
primeSieve<long long> sieve(MAXV);

struct Node {
	int ptr = MAXV;
	vector<int> pf = vector<int>(MAXV + 1, 0);

	int getMaxPf() {
		while (ptr >= 2 && pf[ptr] == 0)
			ptr--;
		return ptr;
	}
};
...
int main() {
	vector<int> dist(MAXV + 1, 0);
	for (int i = 2; i <= MAXV; i++)
		dist[i] = dist[i / sieve.mpf[i]] + 1;
	for (int i = 2; i <= MAXV; i++)
		dist[i] += dist[i - 1];
	...
	vector<Node> nodes(MAXV + 1);
	for (int i = 2; i <= MAXV; i++) {
		nodes[i] = nodes[i - 1];
		for (int cur = i; cur > 1; cur /= sieve.mpf[cur])
			nodes[i].pf[sieve.mpf[cur]]++;
	}

	const int thres = (n + 1) / 2;
	while (true) {
		vector<int> weight(MAXV + 1, 0);
		for (int i = 1; i <= MAXV; i++)
			weight[nodes[i].getMaxPf()] += cnt[i];

		const int w = max_element(weight.begin() + 2, weight.end()) - weight.begin();
		if (weight[w] < thres)
			break;
