...
pair<int,int> Fdfs(pair<int, int> now, vector<pair<int,int>>& fails) {
    visited[now.first][now.second] = 1;
    int next_x = now.first + dx[moves[now.first][now.second]];
    int next_y = now.second + dy[moves[now.first][now.second]];
    pair<int, int> values = mp(-1, -1);
    if (visited[next_x][next_y] == 0) {
        pair<int,int> hoge = Fdfs(mp(next_x, next_y), fails);
        if (hoge.first != -1) {
            fails.push_back(now);
            bads[now.first][now.second] = bads[next_x][next_y] + 1;
            if (hoge == now) {
                values = mp(-1, -1);
            }
            else {
        ...
        //happens!
        bads[now.first][now.second] = 1;
        fails.push_back(now);
        values = mp(next_x, next_y);
    }
...
void solve() {
    ...
    REP(i, loops.size()) {
        ...
        REP(q, loops[i].size()) {
            REP(j, 4) {
                int next_x = loops[i][q].first + dx[j];
                int next_y = loops[i][q].second + dy[j];
                if (next_x >= 0 && next_x < n && next_y >= 0 && next_y < m) {
                    if (bads[next_x][next_y] != 0) continue;
                    if (moves[next_x][next_y] != (j^1)) continue;
                    bads[next_x][next_y] = bads[loops[i][q].first][loops[i][q].second] + 1;
                    loops[i].push_back(mp(next_x, next_y));
                }
            }
        }
        REP(q, loops[i].size()) {
            if (grids[loops[i][q].first][loops[i][q].second] == '1') continue;
            int val = bads[loops[i][q].first][loops[i][q].second] - 1;
            val %= cnters.size();
            cnters[val] = 1;
        }
        REP(q, cnters.size()) {
