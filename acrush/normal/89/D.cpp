...
public:
	int64 x,y,z;
	Point(int64 _x=0,int64 _y=0,int64 _z=0)
	{
		x=_x;
		y=_y;
		z=_z;
...
double solve1(Point A,Point V,int64 R,Point O,int64 r)
{
	R+=r;
	A=A-O;
	int64 a=sqr(V.x)+sqr(V.y)+sqr(V.z);
	int64 b=2*A.x*V.x+2*A.y*V.y+2*A.z*V.z;
	int64 c=sqr(A.x)+sqr(A.y)+sqr(A.z)-sqr(R);
	int64 delta=b*b-4*a*c;
	if (delta<0) return 1e100;
	double v1=(-(double)b+sqrt((double)delta))/(2.0*(double)a);
	double v2=(-(double)b-sqrt((double)delta))/(2.0*(double)a);
...
int main()
{
	...
	for (scanf("%d",&n);n>0;n--)
	{
		...
		O.readin();
		scanf("%d%d",&r,&m);
		checkmin(result,solve1(A,V,R,O,r));
		for (;m>0;m--)
		{
			Point D;
			D.readin();
			D=D+O;
			checkmin(result,solve1(A,V,R,D,0));
		}
	}
	if (result>1e99)
		printf("-1\n");
