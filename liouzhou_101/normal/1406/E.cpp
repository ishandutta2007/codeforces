...
void solve()
{
	...
	//writeln<int>(primes.size()); 9592

	int res = 1;
	int cnt = 0;
	for (auto p : primes)
	{
		if ((ll)p * p <= n) B(p), cnt += 1;
	}
	//writeln(cnt); 65
	...
	else // big prime
	{
		vector<ll> go;
		for (auto p : primes)
			if ((ll)p * p > n) go.push_back(p);

		int baseline = go.size() + 1;

		function<int(int, int)> dfs = [&](int L, int R) -> int
		{
			if (L > R) return -1;
			int m = (L + R) / 2;
			for (int i = L; i <= m; ++i) B(go[i]);
			int tmp = A(1);
			if (tmp != baseline - (m - L + 1))
			{
				for (int i = L; i <= m; ++i)
					if (A(go[i]) == 1) return go[i];
				return -1;
			}
			else
			{
		...
		int L = 0, R = (int)go.size() - 1;
		int gogogo = dfs(L, R);
		if (gogogo == -1) res = 1; else res = gogogo;
	}

	/*
	int res = 1;
	auto a = arr<int>(n + 1);
	for (int i = primes[0]; i <= n; i += primes[0])
		a[i] = 1;
	assert(B(primes[0]) == n / 2);
	if (A(primes[0]) == 1) res *= primes[0];
