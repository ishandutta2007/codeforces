    ...
    while(T--)
    {
        ...
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                int id=getid(i,j);
                if(dir[i][j]=='R')
                    nxt[id][0]=getid(i,j+1);
                else if(dir[i][j]=='U') nxt[id][0]=getid(i-1,j);
                else if(dir[i][j]=='L') nxt[id][0]=getid(i,j-1);
                else nxt[id][0]=getid(i+1,j);
            }
        }
        for(int j=1;j<30;j++)
        {
        ...
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                ...
                if(cc==0)
                    c0[nxt[getid(i,j)][29]]++;
                else c1[nxt[getid(i,j)][29]]++;
            }
        }
        int ans1=0,ans2=0;
        for(int i=0;i<n*m;i++)
        {
            ans1+=min(1,c0[i]+c1[i]);
            ans2+=min(1,c0[i]);
            //dbg(i,c0[i],c1[i]);
        }
        cout<<ans1<<' '<<ans2<<'\n';
