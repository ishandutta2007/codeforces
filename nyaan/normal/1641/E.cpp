...
void q() {
  ...
  vl rui(S);
  each(x, p) rui[of + x]++;
  rep(i, S - 1) rui[i + 1] += rui[i];

  fps f(S), g(S);
  rep(i, S) {
    f[i] = mint{2}.pow(rui[i]);
    g[i] = mint{2}.pow(rui.back() - (i ? rui[i - 1] : 0));
  }
  trc(f);
  trc(g);
  fps h(2 * S - 1);

  auto dc = [&](auto rc, int L, int R) -> void {
    if (L + 1 == R) return;
    int m = (L + R) / 2;
    rc(rc, L, m);
    rc(rc, m, R);
    fps s{begin(f) + L, begin(f) + m};
    fps t{begin(g) + m, begin(g) + R};
    s *= t;
    rep(i, sz(s)) h[L + m + i] += s[i];
  };
  dc(dc, 0, S);
  ...
  rep(i, N) {
    trc(h[(of + i) * 2]-(of+i));
    ans += (h[(of + i) * 2]-(of+i)) * a[i];
  }
  out(ans / (mint{2}.pow(M) - 1));
}
