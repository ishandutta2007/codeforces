...
signed main() {
    ...
    auto make = [&](string s) {
        ...
        vi res(m + 1, -1);
        set<int> se;
        for(int i = 0, j = min(m, k); j >= 0; j--) {
            while(i < n and v[id[i]] >= j) {
                se.emplace(id[i]);
                i++;
            }
            auto it = se.lower_bound(k - j);
            if(it == se.end())
                continue;
            else {
                res[j] = *it;
    ...
    auto right = make(t + s);
    rep2(i, 0, m) {
        if(right[i] != -1) right[i] = n - right[i];
        if(left[i] != -1) left[i]++;
    }
    rep2(i, 0, min(m, k)) {
        if(left[i] != -1 and right[m - i] != -1) {
            if(left[i] + i - 1 < right[m - i] - (m - i) + 1) {
                cout << "Yes\n"
                     << left[i] + i - k << " " << right[m - i] - (m - i) + 1
                     << endl;
                return 0;
            }
        }
