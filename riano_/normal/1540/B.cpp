...
int main(){
    ...
    //L:max
    ll L = 1e6+1;
    fact.assign(L+1,mint(1));
    invf.assign(L+1,mint(1));
    rep(i,L) fact[i+1] = fact[i]*mint(i+1);
    invf[L] = fact[L].inv();
    rrep(i,L) invf[i] = invf[i+1]*mint(i+1); 

    auto prob = v2m(N+1,N+1,0);
    rep(i,N+1){
        mint b = 0;
        mint bb = (mint(2).pow(i)).inv();
        rep(j,N+1){
            if(j!=0) b += bb*((mint(2).pow(j-1)).inv())*comb(i+j-2,j-1);
            prob[i][j] = b;
        }
    }
    rep(j,N+1) prob[0][j] = 1;
    ...
    for(int i=1;i<=N;i++){
        for(int j=i+1;j<=N;j++){
            mint pb = 0;
            ll di = tr.depth[i],dj = tr.depth[j];
            ll pi = -1,pj = -1,ni = i,nj = j;
            vector<ll> fi,fj;
            while(di>dj){
                if(pi!=-1){
                    fi.push_back(tr.subtree_size[ni]-tr.subtree_size[pi]);
                }
                else{
                    fi.push_back(tr.subtree_size[ni]);
                }
                pi = ni; ni = tr.par[ni]; di--;
            }
            while(dj>di){
                if(pj!=-1){
                    fj.push_back(tr.subtree_size[nj]-tr.subtree_size[pj]);
                }
                else{
                    fj.push_back(tr.subtree_size[nj]);
                }
                pj = nj; nj = tr.par[nj]; dj--;
            }
            bool par = false;
            if(ni==nj){
                par = true;
                if(pi==-1){
                    fj.push_back(N-tr.subtree_size[pj]);
                }
                else{
                    fj.push_back(N-tr.subtree_size[pi]);
                }
            }
            while(ni!=nj){
                if(pi!=-1){
            ...
            if(!par){
                fj.push_back(N-tr.subtree_size[pj]-tr.subtree_size[pi]);
            }
            reverse(all(fj));
            for(ll x:fj) fi.push_back(x);
            ll M = fi.size();
            // cout << i << " " << j << endl;
            // out(fi);
            // cout << endl;
            rep(k,M){
                pb += prob[M-1-k][k]*mint(fi[k]);
                //cout << M-k-1 << " " << k << " " << prob[M-1-k][k] << endl;
            }
            ans += pb;
            //cout << ans << endl;
        }
    }
    cout << ans/mint(N) << endl;
}