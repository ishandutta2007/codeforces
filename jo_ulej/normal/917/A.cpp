...
void solve() {
	...
	int n = len(S), answ = 0;
	for(int l = 0; l < n; ++l) {
		int minBal = 0, toChange = 0, cntQue = 0;
		for(int r = l; r < n; ++r) {
			if(S[r] == '(') ++minBal;
			else if(S[r] == ')' || S[r] == '?') --minBal;
			else assert(false);
			if(S[r] == '?') ++cntQue;
			if(minBal < 0) toChange = max(toChange, (1 - minBal) / 2);
			if(toChange > cntQue) break;
			if((r - l) & 1) {
				if(minBal + 2 * toChange <= 0) {
					++answ;
					DBG(S.substr(l, r - l + 1));
				}
			}
		}
	}
