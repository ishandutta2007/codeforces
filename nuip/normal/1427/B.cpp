...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	while(T--){
		...
		auto rem=t;
		while(ls.size() && rem){
			cauto t=min<ll>(ls.back(),rem);
			ls.back()-=t;
			rem-=t;
			if(ls.back()==0) ls.pop_back();
		}
		while(ls_.size() && rem){
			cauto t=min<ll>(ls_.back(),rem);
			ls_.back()-=t;
			rem-=t;
			if(ls_.back()==0) ls_.pop_back();
		}
		out(ls,1);
		int re=n*2-1;
		for(auto x:ls) re-=x*2+1;
		for(auto x:ls_) re-=x*2;
		cout<<max(0,re) NL;
	}
  return 0;
}