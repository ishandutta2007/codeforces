...
public:
    void solve() {
        ...
        arr<string> ans(h, string(w, '0'));
        auto place = [&](int r, int c) -> void {
            for (int i : range(r - 1, r + 2)) {
                for (int j : range(c - 1, c + 2)) {
                    if (isValidCell(i, j, h, w) && ans[i][j] == '1') {
                        return;
                    }
                }
            }
        ...
        for (int i : RevRange(w)) {
            place(h - 1, i);
        }
        for (int i : RevRange(h)) {
            place(i, 0);
        }
        for (val v : ans) {
            out.printLine(v);
