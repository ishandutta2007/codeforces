#line 1 "codeforces-workspace\\contest\\1598\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1598\\b\\b.cpp"
...
void main() {
  ...
  input n;
  input<vector<array<int, 5>>> table(n);

  array<int, 5> att{0, 0, 0, 1, 1};
  do {
    bool fail = 0;
    vector<int> c;
    for (auto i : range(5)) {
      if (att[i]) c.emplace_back(i);
    }
    int f[2]{};
    for (auto &&v : table) {
      if (!v[c[0]] and !v[c[1]]) {
      ...
      if (v[c[0]] and v[c[1]]) continue;
      if (v[c[0]])
        f[0]++;
      else
        f[1]++;
    }
    if (fail or max(f[0], f[1]) > n / 2) continue;
    eject("YES");
  } while (next_permutation(begin(att), end(att)));
