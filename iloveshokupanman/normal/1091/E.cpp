...
int main() {
    ...
    vll rui(N + 1, 0), rev(N + 1, 0);
    repn(i, N)rui[i] = rui[i - 1] + a[i - 1];
    for (ll i = N - 1; i >= 0; i--)rev[i] = rev[i + 1] + a[i];

    vll mk(N + 1, 0), mkk(N + 1, 0);
    repn(i, N) {
        auto itr = lower_bound(a.begin(), a.end(), N - i);
        ll x = distance(a.begin(), itr);
        x = min(x, i);
        mk[i] = rui[x] + (N - i) * (i - x);

        itr = lower_bound(a.begin(), a.end(), N - i+1);
        x = distance(a.begin(), itr);
        x = min(x, i);
        mkk[i] = rui[x] + (N - i+1) * (i - x);
    }
    ...
    rep(i, N+1) {
        //cout << mk[i] << " "<<mkk[i]<<" " << rev[i] << endl;

        ll now = mkk[i] + (N-i+1) * (N-i) - rev[i];
        mx = min(mx, now);
        //cout << now << endl;

        now = rev[i] - (N - i) * (N - i - 1) - mk[i];
        mn = max(mn, now);
        //cout << now << endl;
        

        now = rev[i] - (N - i) * (N - i - 1) - mk[i] - (N - i);
