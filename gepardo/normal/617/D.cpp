...
main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            if (i != j)
            {
                if (p[i].ff == p[j].ff || p[i].ss == p[j].ss) lp[i] = true;
            }
        }
    }
    ...
    if (lp[0] && lp[1] && lp[2])
    {
        if ((p[0].ff == p[1].ff && p[1].ff == p[2].ff) ||
            (p[0].ss == p[1].ss && p[1].ss == p[2].ss)) out = 1; else out = 2;
        return 0;
    }
    if (!lp[0] && !lp[1] && !lp[2])
    {
        ...
        for (int j = 0; j < i; j++)
        {
            if (p[i].ss == p[j].ss)
            {
                for (int k = 0; k < 3; k++)
                    if (k != i && k != j)
                    {
                        if ((p[k].ff > p[i].ff && p[k].ff > p[j].ff) ||
                            (p[k].ff < p[i].ff && p[k].ff < p[j].ff))
                        {
                            out = 2;
                            return 0;
                        }
            ...
            else if (p[i].ff == p[j].ff)
            {
                for (int k = 0; k < 3; k++)
                    if (k != i && k != j)
                    {
                        if ((p[k].ss > p[i].ss && p[k].ss > p[j].ss) ||
                            (p[k].ss < p[i].ss && p[k].ss < p[j].ss))
                        {
                            out = 2;
                            return 0;
                        }
