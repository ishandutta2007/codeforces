...
const int maxn = 1e4 + 100, maxw = 5e6 + 100, inf = 1e9 + 100, sq = 450, LG = 20, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
int main() {
    ...
    for (int k = 1; k <= n; k++)
        for (int i = 0; i <= sa; i++)
            if (q[k][i] != -1) {
                ans[k - 1] = max(ans[k - 1], min((double)i, B / 2.0 + q[k][i] / 2.0));
            }
    for (int i = 0; i < n; i++) {
