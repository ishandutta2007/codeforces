...
int main()
{
	...
	rep(i,n){
		...
		rep(j,n){
			ll t = 0;
			rep(k, m) if (l[j] % p[k] != 0) t += (1 << k);
			s[t] = min(s[t], c[j]);
		}
		ll dp[(1 << m)][(1 << m)];
		rep(j, (1 << m)) rep(k, (1 << m)) dp[j][k] = 1e9;
		dp[0][0] = c[i];
		for (ll j = 1; j < (1 << m);j++)
			rep(k, (1 << m)) dp[j][k] = min(dp[j][k], dp[j - 1][k]), dp[j][k | j] = min(dp[j][k | j], dp[j - 1][k] + s[j]);
		ans = min(ans, dp[(1 << m) - 1][(1 << m) - 1]);
	}
	if(ans < 1e8)
		cout << ans << endl;
	else
