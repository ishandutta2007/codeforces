#line 1 "codeforces-workspace\\contest\\1620\\f\\f.cpp"
...
#line 13 "codeforces-workspace\\contest\\1620\\f\\f.cpp"
...
void main() {
  ...
  constexpr array<i32, 2> sign{1, -1};

  input n;

  array<i32, 2> dp{-inf, -inf};
  vector<array<i32, 2>> prev;
  ...
  for (auto t : range(n)) {
    input a;
    as.emplace_back(a);
    array<i32, 2> nx{inf, inf};
    auto& src = prev.emplace_back();
    src = {-1, -1};

    for (auto s : {0, 1}) {
      for (auto j : {0, 1}) {
        if (!t or last * sign[j] < a * sign[s]) {
          if (chle(nx[s], dp[j])) src[s] = j;
        }
        if (dp[j] < a * sign[s]) {
          if (chle(nx[s], t ? last * sign[j] : -inf)) src[s] = j;
        }
      }
    }
  ...
  for (auto j : range(2)) {
    if (dp[j] < inf) {
      print("YES");
      vector<i32> ans;
      for (auto [p, a] : reversed(zip(prev, as))) {
        ans.emplace_back(sign[j] * a);
        j = p[j];
      }
      eject(reversed(ans));
    }
