...
int n,i,r,b,x[N],y[N],xxb,yxb,ctx[N],cty[N],limx[N],limy[N];
char rr,bb;
...
inline void out(){for(int i=1;i<=::n;++i)cout<<(g.e[ide[i]].f?rr:bb);cout<<endl;}
}g;
int main(){
	...
	for(i=1;i<=xxb;++i)limx[i]=1;
	for(i=1;i<=yxb;++i)limy[i]=1;
	...
	g.add(g.n,g.n-1,0,n);g.work();g.out();
	return 0;
}