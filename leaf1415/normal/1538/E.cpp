...
Q merge(Q a, Q b)
{
	...
	string t = A.substr(max(0LL, sz(A)-3)) + B.substr(0, min(sz(B), 3LL));
	Q ret = get(t);
	ret.second += a.second + b.second;
	
	string s = A+B;
	if(sz(s) <= 6) ret.first = s;
	else ret.first = s.substr(0, 3) + s.substr(sz(s)-3, 3);
