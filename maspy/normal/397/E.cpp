...
void solve() {
  ...
  FenwickTree<mint> bit0(N + 10);
  FenwickTree<mint> bit1(N + 10);

  auto add = [&](ll v, mint x, mint k) -> void {
    mint d = dep[v];
    // x - k(dep - d) = (-k) dep + (x + kd) 
    bit0.add(LID[v], x + k * d);
    bit0.add(RID[v], -(x + k * d));
    bit1.add(LID[v], -k);
    bit1.add(RID[v], k);
  };

  auto calc = [&](ll v) -> mint {
    mint a = bit0.sum(LID[v] + 1);
    mint b = bit1.sum(LID[v] + 1);
    return a + b * mint(dep[v]);
  };
