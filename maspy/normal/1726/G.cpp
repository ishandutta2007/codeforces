...
void solve() {
  ...
  */
  ...
  vi CNT(N + N + 10);
  for (auto&& a: A) CNT[a]++;
  auto CNTc = cumsum(CNT);

  ll mi = MIN(A);

  if (mi == MAX(A)) { return print(fact<mint>(N)); }
  for (auto&& i: IDS[mi]) {
    if (B[i] == 0) return print(0);
  }
  if (len(IDS[mi]) >= 2) return print(0);
  ll TO = mi + (N - 1);
  if (MAX(A) > TO) return print(0);
  ...
  FOR(x, mi, TO + 1) {
    ...
    if (x == TO) {
      ...
      ANS *= fact<mint>(a);
      // 1 
      ANS *= C<mint>(n + a + b, b) * fact<mint>(b);
      break;
    }

    if (b > 1) return print(0);

    ll k = TO - x;
    //  k  a 
    if (a > 0 && k > n) return print(0);
    ANS *= fact<mint>(a);
    // 1 n+a  s 
    ll s = TO - (x + CNTc.back() - CNTc[x + 1]);
    if (b && s < 0) return print(0);
    if (b && s > n + a) return print(0);
    n += a + b;
  }
