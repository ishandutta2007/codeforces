...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto vs=readV(n);
	const ll INF=5e15;
	fill(dp[0][0],dp[212345][0],-INF);
	dp[0][0][0]=0;
	rep(i,n)rep(j,3)rep(f,2){
		MX(dp[i+1][j][f==1||i%2==1], dp[i][j][f]+vs[i]);
		MX(dp[i+1][(j+1)%3][f==1||i%2==0], dp[i][j][f]-vs[i]);
	}
	// rep(i,n+1){
	// 	rep(j,4){rep(k,4) cout<<pll(dp[i][j][k][0],dp[i][j][k][1]); cout NL;}
	// 	cout NL;
	// }
	ll re=-INF;
	rep(i,3)if((n+i)%3==1){
		MX(re,dp[n][i][1]);
		if(n==1) MX(re,dp[n][i][0]);
	}
	cout<<re NL;
	
	if(0){
		vector<set<string>> ok(20);
		ok[1].insert("0");
		reps(i,2,ok.size()){
			reps(l,1,i){
				cauto r=i-l;
				for(cauto a:ok[l])for(cauto b:ok[r]){
						auto tmp=a+b;
						for(char &c:tmp) c^=1;
						ok[i].emplace(tmp);
					}
		...
		rep(i,ok.size()){
			out(i,1);
			vector<int> cnt(i+1);
			for(auto s:ok[i]) ++cnt[count(all(s),'1')];
			out(cnt,1);
			rep(j,cnt.size())if(cnt[j] && cnt[j]!=comb(i,j).v) out(i,j,cnt[j],comb(i,j),1);
		}
	}
  return 0;
}