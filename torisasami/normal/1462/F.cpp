...
int main()
{
	...
	while(T--){
		...
		rep(i, n) cin >> l[i] >> r[i];
		vector<ll> v;
		rep(i, n) v.pb(l[i]), v.pb(r[i]);
		sort(all(v));
		ll p = 0;
		map<ll, ll> m;
		rep(i, 2 * n) if (m[v[i]] == 0) m[v[i]] = ++p;
		SegmentTree<ll> segl(p, [](ll a, ll b) { return a + b; }, 0);
		SegmentTree<ll> segr(p, [](ll a, ll b) { return a + b; }, 0);
		ll cntl[p] = {}, cntr[p] = {};
		rep(i, n) cntl[m[l[i]] - 1]++, cntr[m[r[i]] - 1]++;
		rep(i, p) segl.set(i, cntl[i]), segr.set(i, cntr[i]);
		segl.build(), segr.build();
		ll ans = 1e9;
		rep(i,n){
			ll s = 0;
			s += segl.query(m[r[i]], p);
			s += segr.query(0, m[l[i]] - 1);
			ans = min(ans, s);
		}
		cout << ans << endl;
	}
