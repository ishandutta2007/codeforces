...
public class TaskC {
	...
	private static class Folder {
		...
		private final Set<Folder> folders = new HashSet<Folder>();
		...
		public int countFolders() {
			...
			for (Folder folder : folders)
				folderCount += folder.countFolders() + 1;
			return folderCount;
		}
	...
	private void solve() {
		...
		for (String path : paths) {
			...
			for (int i = path.length() - 1; i > 2; i--) {
				...
				if (folder == null)
					folders.put(folderPath, folder = new Folder());
				if (lastSlash == -1)
					folder.addFile(path);
				else
					folder.addFolder(folders.get(path.substring(0, lastSlash)));
				lastSlash = i;
			}
		}
		int maxFolders = 0;
		...
		for (Folder folder : folders.values()) {
			maxFolders = Math.max(maxFolders, folder.countFolders());
			maxFiles = Math.max(maxFiles, folder.countFiles());
		}
		out.println(maxFolders + " " + maxFiles);
	}

	private static List<Test> createTests() {
		List<Test> tests = new ArrayList<Test>();
		tests.add(new Test("C:\\folder1\\file1.txt", "0 1"));
		tests.add(new Test("C:\\folder1\\folder2\\folder3\\file1.txt\n" +
			"C:\\folder1\\folder2\\folder4\\file1.txt\n" +
			"D:\\folder1\\file1.txt", "3 2"));
		tests.add(new Test("C:\\file\\file\\file\\file\\file.txt\n" +
			"C:\\file\\file\\file\\file2\\file.txt", "4 2"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
