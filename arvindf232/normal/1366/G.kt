...
fun main(){
    solve.cases{
        ...
        var DP = IntArray(n+1){big}
        DP[0] = 0

        val m1 = fastEmptyList(10002)
        val vs = fastEmptyList(10002)
        val mins = fastEmptyList(10002)
        fun addstack(b:Int, v:Int){
            val newmin = if(mins.isEmpty()) v else minOf(v,mins.last())
            m1.addLast(b)
            vs.addLast(v)
            mins.addLast(newmin)
        }

        val shift = balance[n-1]
        for(i in 0 until m){
            val here = t[i]
            val new = IntArray(n+1){big}
            ...
            for(l in 0 until n){
                ...
                val bhere = balance[l]
                var cheapest = plarge
                while(m1.isNotEmpty() && m1.last() >= bhere){
                    val b = m1.removeLast()
                    val v = vs.removeLast()
                    mins.removeLast()
                    val new = v + 2 * (b- bhere)
                    cheapest = minOf(cheapest, new)
                }
                if(cheapest != plarge){
                    addstack(bhere,cheapest)
                }
                if(DP[l] <= 50000){
                    addstack(balance[l], DP[l] - balance[l])
                }
                
                if(s[l] == here){
                    if(mins.isNotEmpty()) {
                        new[l+1] = mins.last() + balance[l]
                    }
                }
//                here dei stack
...
bruh uses real costs as much as possible
 */