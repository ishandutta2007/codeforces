...
inline void solve()
{
	cnt=read(),res=0,minn=inf,maxn=-inf;
	Forr(i,1,cnt)
	{
		num[i]=read();
	}
	Forr(i,1,cnt)
	{
		...
		{
			chkmin(minn,num[i]),chkmax(maxn,num[i]);
		}
	}
	v=(minn+maxn)>>1,num[1]=num[1]==-1?v:num[1];
	Forr(i,2,cnt)
	{
		num[i]=num[i]==-1?v:num[i];
