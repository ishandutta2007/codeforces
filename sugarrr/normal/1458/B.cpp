...
const ll N=102,M=10002;
vector<vl>dp(N,vl(M,-inf));
vector<vl>nx(N,vl(M,-inf));

int main(){fastio
    ll n;cin>>n;
    ll a[n],b[n];
    rep(i,0,n-1)cin>>a[i]>>b[i];
    ll S=0;rep(i,0,n-1)S+=b[i];
    
    dp[0][0]=0;
    rep(z,0,n-1){
        //rep(i,0,N-1)rep(j,0,M-1)nx[i][j]=-inf;
        
        ll A=a[z],B=b[z];
        rrep(i,z,0){
            rep(j,0,M-1){
                chmax(nx[i+1][j+A],dp[i][j]+B);
                chmax(nx[i][j],dp[i][j]);
            }
        }
        dp=nx;
  ...
  /*  rep(i,0,n){
    ...
    rep(i,1,n){
        dd ans=0;
        rep(j,0,M-1){
            dd ret=min((dd)dp[i][j]+(S-dp[i][j])/2.0,(dd)j);
            chmax(ans,ret);
        }
        cout<<ans<<" ";
    }cout<<endl;
