...
int main() {
	...
	for (int l = 1; l <= m; ++l) {
		for (int i = 1; i + l - 1 <= m; ++i) {
			int j = i + l - 1;
			REP(r, n) {
				int b = seg[r][i].first == i ? i : seg[r][i].second + 1;
				int e = seg[r][j].second == j ? j : seg[r][j].first - 1;
				for (int c = b; c <= e; ++c) q[c]++;
			}
			debug(range(q, i, j + 1));
			for (int x = i; x <= j; ++x) maxi(dp[i][j], dp[i][x - 1] + dp[x + 1][j] + q[x] * q[x]);
			for (int x = i; x <= j; ++x) q[x] = 0;
		}
	}
	printf("%d\n", dp[1][m]);
}
