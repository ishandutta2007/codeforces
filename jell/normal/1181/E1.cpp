...
bool solve(vector<int> in) {
    int m=in.size();
    if(m==1) {
        return true;
    }
    vector<tuple<int,int,int>> vert,hrzn;
    for(int i:in) {
        rect &r=cas[i];
        hrzn.emplace_back(r.min_x,r.max_x,i);
    }
    sort(all(hrzn));
    for(int i=0,tmp=0; i<m; ++i) {
        int a,b; tie(a,b,ignore)=hrzn[i];
        if(i && a>=tmp) {
            vector<int> in_1(i),in_2(m-i);
            for(int j=0; j<i; ++j) {
                in_1[j]=get<2>(hrzn[j]);
            }
            for(int j=i; j<m; ++j) {
                in_2[j-i]=get<2>(hrzn[j]);
            }
            return solve(in_1) && solve(in_2);
        }
        tmp=max(tmp,b);
    }
    for(int i:in) {
        rect &r=cas[i];
        vert.emplace_back(r.min_y,r.max_y,i);
    }
    sort(all(vert));
    for(int i=0,tmp=0; i<m; ++i) {
        int a,b; tie(a,b,ignore)=vert[i];
        if(i && a>=tmp) {
            vector<int> in_1(i),in_2(m-i);
            for(int j=0; j<i; ++j) {
                in_1[j]=get<2>(vert[j]);
...
signed main() {
    ...
    for(int i=0; i<n; ++i) {
        whole[i]=i;
        rect &r=cas[i];
        cin>>r.min_x>>r.min_y>>r.max_x>>r.max_y;
    }
    if(solve(whole)) {
        cout<<"YES\n";
    } else {
