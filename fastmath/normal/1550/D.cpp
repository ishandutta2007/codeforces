...
signed main() {
    ...
    rep (t) {
        ...
        auto make_eq = [&] (int a, int b, int have) {
            if (a >= 0 && b >= 0) {
                assert(a + b == have);
                return comb(have, a);                
            }
        ...
        auto solve_for_w = [&] (int w) {
            mi ans = 0;
            int pref = min(n, R - w);
            int suf = max(1ll, L + w);

            assert(pref >= 1 && suf <= n && pref >= suf - 1);
            ...
            if (n%2==0) {
                ans += make_eq(n/2-left, n/2-right, com);    
            }
            else {
                int half = n/2;
                ans += make_eq(half - left, half + 1 - right, com);
                ans += make_eq(half + 1 - left, half - right, com);
            }
            return ans;
        };
        ...
        if (mx - 2 * n > 1) {
            int k = mx - 2 * n - 1;
            ans += k * solve_for_w(mx - 2 * n);
            for (int w = mx - 2 * n; w <= mx; ++w) {
                ans += solve_for_w(w);
            }
        }
        else {
