...
{
	if(a[y*w+x] == 1) return true;
	rep(i, 0, 3){
		ll nx = x + dx[i], ny = y + dy[i];
		if(nx < 0 || ny < 0 || nx >= w || ny >= h) continue;
		if(a[y*w+x] > a[ny*w+nx]) return true;
	}
	return false;
}

void calc2(ll x, ll y, ll c)
{
	rep(i, 0, 4){
		ll nx = x + dx[i], ny = y + dy[i];
		if(nx < 0 || ny < 0 || nx >= w || ny >= h) continue;
		if(c == 1){
			if(b[ny*w+nx]) cnt--, b[ny*w+nx] = false;
		}
		else{
			if(!b[ny*w+nx] && check(nx, ny)) cnt++, b[ny*w+nx] = true;
		}
	}
}

void calc(ll x, ll y, ll nx, ll ny)
{
	calc2(x, y, 1), calc2(nx, ny, 1);
	swap(a[y*w+x], a[ny*w+nx]);
	calc2(x, y, 0), calc2(nx, ny, 0);
}
...
int main(void)
{
	...
	cin >> h >> w;
	rep(y, 0, h-1) rep(x, 0, w-1) cin >> a[y*w+x];
	
	P p = P(-1, -1);
	rep(y, 0, h-1) rep(x, 0, w-1){
		if(check(x, y)) b[y*w+x] = true, cnt++;
		else p = P(x, y);
	}
	...
	rep(i, 0, 4){
		...
		if(nx < 0 || ny < 0 || nx >= w || ny >= h) continue;
		rep(y, 0, h-1) rep(x, 0, w-1){
			if(P(x, y) == P(nx, ny)) continue;
			calc(x, y, nx, ny);
			if(cnt == h*w){
				ll f = a[y*w+x], g = a[ny*w+nx];
				if(f > g) swap(f, g);
				ans.insert(P(f, g));
			}
			calc(x, y, nx, ny);
