...
signed main(){
    ...
    while(T--){
        ...
        while(l<r){
            int mid = (l+r)/2;
            int N = 0;
            rep2(i,l,mid) N+=s[i].size();
            cout << "? "<<N<<" ";
            rep2(i,l,mid)for(auto &e:s[i])cout<<e<<" ";
            cout<<endl;
            cout.flush();
            int x = in();
            if(x == ma) r = mid;
        ...
        cout<<"? "<<n-s[l].size()<<" ";
        vi cnt(n+1);
        for(auto e:s[l])cnt[e]=1;
        rep2(i,1,n)if(!cnt[i]) cout<<i<<" ";cout<<endl;
        cout.flush();
        int x = in();
        cout<<"! ";
        rep(i,l)cout<<ma<<" ";
        cout<<x<<" ";
        rep2(i,l+1,k-1)cout<<ma<<" ";cout<<endl;cout.flush();
        stin();
    }
}