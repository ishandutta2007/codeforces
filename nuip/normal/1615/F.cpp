...
void exp(){
	const int n=10;
	fill(dst[0],dst[1<<10],MOD);
	rep(i,1<<n) dst[i][i]=0;
	rep(st,1<<n)rep(i,n-1){
		if((st>>i&3)==0) dst[st][st|(3<<i)]=1;
		else if((st>>i&3)==3) dst[st][st^(3<<i)]=1;
	}
	rep(k,1<<10)rep(i,1<<10)rep(j,1<<10) MN(dst[i][j],dst[i][k]+dst[k][j]);
	cauto dif=
		[&](int i){
			int re=0;
			rep(d,10)if(i>>d&1) if(d%2) --re; else ++re;
			return re;
		};
	rep(i,1<<n)rep(j,1<<n){
		cauto exp=dst[i][j];
...
void solve(const string &s,const string &t, mint dp[2123][o+o]){
	const int n=s.size();
	fill(dp[0],dp[n+10],0);
	dp[0][o]=1;
	rep(i,n){
		mint dec=0,eq=0,inc=0;
		rep(c,2)if(s[i]=='?' || s[i]-'0'==c){
			rep(d,2)if(t[i]=='?' || t[i]-'0'==d){
				if(c==d){
					eq+=1;
				}else if(c){
					(i%2?inc:dec)+=1;
		...
		reps(j,1,4120){
			dp[i+1][j-1]+=dp[i][j]*dec;
			dp[i+1][j+0]+=dp[i][j]*eq;
			dp[i+1][j+1]+=dp[i][j]*inc;
		}
	}
	out(s,t,1);
	rep(i,n+1)rep(j,o+o)if(dp[i][j].v) out(i,j-o,dp[i][j],1);
}
...
int main(){
	...
	TT{
		...
		solve(s,t,dpL);
		if(n%2){
			solve(s-reversed,t-reversed,dpR);
		}else{
			solve(t-reversed,s-reversed,dpR);
		}
		mint re=0;
		reps(i,1,n)reps(d,-2010,2010){
			re+=dpL[i][d+o]*dpR[n-i][-d+o]*abs(d);
		}
		cout<<re NL;
	}
	return 0;
