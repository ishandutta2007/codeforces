...
int main() {
  ...
  vector dp(n, vector(m + 1, ModInt(0)));
  dp[0][0] = 2;
  REP(i, n - 1) REP(j, m + 1) {
    dp[i + 1][j] += dp[i][j]; // back
    if (j == 0) dp[i + 1][j] += dp[i][j]; // front
    if (j + 1 <= m && s[i + 1] == t[m - (j + 1)]) dp[i + 1][j + 1] += dp[i][j]; // front
  }
  // REP(i, n) REP(j, m + 1) cout << dp[i][j] << " \n"[j == m];
  ModInt ans = 0;
  REP(i, n) ans += dp[i][m];
  // cout << ans << '\n';
  vector pat(n, ModInt(0));
  REP(i, m) {
    if (s[0] == t[i]) pat[i] = 2;
  }
  FOR(i, 1, n) {
    if (i >= m) ans += pat[0];
    vector nx(n, ModInt(0));
    REP(j, n) {
      if (j >= m || s[i] == t[j]) {
        if (j + 1 < n) nx[j] += pat[j + 1];
        if (j - i >= 0) nx[j - i] += pat[j - i];
      }
    }
    pat.swap(nx);
  }
