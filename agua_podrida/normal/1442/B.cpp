...
void b(){
    ...
    F0(i,k) b[i]--;
    F0(i,k) b[i] = a[b[i]];
    int l = 0;//primer no usado
    int r = n-1;//ultimo no usado
    set<int> tengo;
    F0(i,n) tengo.insert(i);
    ll res = 1;
    vb faltaPoner(n);
    F0(i,k) faltaPoner[b[i]] = true;
    F0(i,k){
        if(tengo.count(b[i]) == 0) {
            res = 0;
            break;
        }

        faltaPoner[b[i]] = false;
        bool izq = true;
        auto it = tengo.find(b[i]);
        if(it == tengo.begin()) izq = false;
        else if(faltaPoner[(*(--it))]) izq = false;
        it = tengo.find(b[i]);
        auto it2 = it;
        it2++;
        bool der = true;
        if(it2 == tengo.end() or faltaPoner[*it2]) der = false;
        it = tengo.find(b[i]);
        if(izq and der){
            res *= 2;
            res %= mod;
