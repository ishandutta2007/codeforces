...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let (n, k): (usize, usize) = cin.read();
    let a: Vec<i32> = cin.read_vec(n);
    let mut l = 0;
    let mut r = 1e9 as i32 + 1;
    while r - l > 1 {
        let m = l + (r - l) / 2;
        let small = a.iter().filter(|&&x| x * 2 < m).count();
        let mut here = usize::MAX;
        for i in 0..n - 1 {
            let cur = small - ((a[i] * 2 < m) as usize) - ((a[i + 1] * 2 < m) as usize) + ((a[i] < m) as usize) + ((a[i + 1] < m) as usize);
            here = here.min(cur);
        }
        if here <= k {
            l = m;
