...
vector<int> ans;
vector<int> lowDeg, highDeg;

void solve(int s, int e, vector<int> vtx, vector<pi> edg, vector<pi> fixed, vector<int> &v){
	if(s == e){
		for(auto &x : vtx){
		//	cout << "ans[" << x << "] = " << v[s] << "\n";
			ans[x] = v[s];
	...
	int m = (s + e) / 2;
	HLPP<int> mf(sz(vtx) + 2);
	for(auto &[idx, val] : fixed){
		int nidx = lower_bound(all(vtx), idx) - vtx.begin();
		if(val <= v[m]) mf.addEdge(sz(vtx), nidx, 1e9);
		else mf.addEdge(nidx, sz(vtx) + 1, 1e9);
	}
	for(auto &[u, v] : edg){
		int nu = lower_bound(all(vtx), u) - vtx.begin();
		int nv = lower_bound(all(vtx), v) - vtx.begin();
		mf.addEdge(nu, nv, 1, 1);
	}
	for(int i = 0; i < sz(vtx); i++){
		mf.addEdge(sz(vtx), i, lowDeg[vtx[i]]);
		mf.addEdge(i, sz(vtx) + 1, highDeg[vtx[i]]);
	}
	lint ans = 1ll * mf.flow(sz(vtx), sz(vtx) + 1) * (v[m + 1] - v[m]);
	vector<int> l, r;
	vector<pi> le, re;
	vector<pi> lf, rf;
	for(int i = 0; i < sz(vtx); i++){
		if(mf.cutSide(i)) l.push_back(vtx[i]);
		else r.push_back(vtx[i]);
	}
	for(auto &[idx, val] : fixed){
		if(val <= v[m]) lf.emplace_back(idx, val);
		else rf.emplace_back(idx, val);
	}
	for(auto &[u, v] : edg){
		int nu = lower_bound(all(vtx), u) - vtx.begin();
		int nv = lower_bound(all(vtx), v) - vtx.begin();
		if(mf.cutSide(nu) != mf.cutSide(nv)){
			if(!mf.cutSide(nu)){
				lowDeg[u]++;
				highDeg[v]++;
			}
		...
		if(mf.cutSide(nu)) le.emplace_back(u, v);
		else re.emplace_back(u, v);
	}
	solve(s, m, l, le, lf, v);
	solve(m+1, e, r, re, rf, v);
}
...
int main(){
	...
	for(int i = 0; i < n; i++){
		for(int j = 0; j < n; j++){
			if(j + 1 < n && idx[i][j] >= 0 && idx[i][j+1] >= 0){
				ed.push_back({idx[i][j], idx[i][j+1]});
			}
			if(i + 1 < n && idx[i][j] >= 0 && idx[i+1][j] >= 0){
				ed.push_back({idx[i][j], idx[i+1][j]});
			}
			if(a[i][j] > 0){
				fixed.emplace_back(idx[i][j], a[i][j]);
			}
		}
	}
	vector<int> vtx(piv); iota(all(vtx), 0);
	ans.resize(piv);
	lowDeg.resize(piv);
	highDeg.resize(piv);
	solve(0, sz(v) - 1, vtx, ed, fixed, v);
	lint ret = 0;
	for(auto &[x, y] : ed) ret += abs(ans[x] - ans[y]);
	cout << ret << "\n";
}