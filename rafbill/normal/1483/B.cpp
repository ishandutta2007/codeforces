...
void solve(){
  ...
  vi nxt(n), prv(n);
  for(li i = 0; i < (li)(n); ++i) nxt[i] = (i+1)%n;
  for(li i = 0; i < (li)(n); ++i) prv[nxt[i]] = i;

  set<int> del;
  for(li i = 0; i < (li)(n); ++i) if(__gcd(A[i],A[nxt[i]]) == 1) del.insert(nxt[i]);
  ...
  while(!del.empty()) {
    ...
    pos %= n;
    auto it = del.lower_bound(pos);
    if(it == end(del)) it = begin(del);
    ...
    del.erase(b);
    if((int)ans.size() < n) {
      if(__gcd(A[prv[a]],A[a]) == 1) del.insert(a);
      if(__gcd(A[a],A[b]) == 1) del.insert(b);
      pos = nxt[b];
    }
  }
