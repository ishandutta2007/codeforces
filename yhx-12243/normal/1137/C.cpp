...
void trans(int x, int tx) {
	if (x == tx) return;
	int i, Dist = (dep[tx] - dep[x]) % d; Dist += Dist >> 31 & d;
...
void dfs2(int x, int scc) {
	...
	for (i = first[x]; i; i = next[i])
		if (!used[y = to[i]] && top[y]) {
			...
			if (top[y] == scc) dfs2(y, scc);
			else if (trans(y, top[y]), !--deg[top[y]]) que[T++] = top[y];
		}
}
...
int main() {
	...
	for (i = 0; i < E; ++i) if (u[i][top] && v[i][top]) {
		if (u[i][top] == v[i][top]) cyc[ u[i][top] ] = gcd(cyc[ u[i][top] ], abs(1 + u[i][dep] - v[i][dep]));
		else {
