    ...
    solve.cases{
        ...
        for(width in 1 until n){
            for(left in 0 until n){
                ...
                if( right >= n ) continue
                for( mid in left until right){
                    val L = DP[left][mid]
                    val R = DP[mid+1][right]
                    if(L == -1 || R == -1 || L != R){
                        continue
                    }
                    DP[left][right] = L + 1
        ...
        val minreach = IntArray(n){Int.MAX_VALUE}
        for(i in -1 until n){
            val here = if(i == -1) 0 else minreach[i]
            for(right in i+1 until n){
                if(DP[i+1][right] != -1){
                    minreach[right] = minOf(minreach[right],here + 1 )
                }
            }
        }
        put(minreach.last())
...
//            }
//            if(i == n ) break
//            for(right in leftPointer until n){
//                if(DP[leftPointer][right] != -1){
//                    getTo = maxOf(getTo,right)
//                }
//            }
//        }
    }
...
/*
forgetting to update once again
variable swap on taking the min

incorrect greedy, of course...

final interval fro intreval cover problem
