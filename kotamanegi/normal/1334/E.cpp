...
#define int ll
modint<MOD> kaijo[100000];
modint<MOD> gyaku[100000];
void solve() {
    int d;
    cin >> d;
    kaijo[0] = 1;
    ...
    for (int i = 1; i < 100000; ++i) {
        kaijo[i] = modint<MOD>(i) * kaijo[i - 1];
        gyaku[i] = modint<MOD>(1) / kaijo[i];
    }
    ...
    for (int i = 2; i * i <= d; ++i) {
        if (d % i == 0) {
            ...
            inputs.push_back(mp(i, cnt + 1));
            geko *= (cnt + 1);
        }
    }
    if (d != 1) {
        inputs.push_back(mp(d, 2));
        d = 1;
        geko *= 2;
    }
    map<int, int> gogos;
    ...
    REP(i, query) {
        ...
        int TC = 0;
        REP(q, inputs.size()) {
            int hoge = (a % inputs[q].second) - (b % inputs[q].second);
            if (hoge >= 0) {
                cnt += hoge;
                ans *= gyaku[hoge];
            }
