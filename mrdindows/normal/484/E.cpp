...
void add(node *cur, int l, int r, int pos){
    ...
    else
    {
        ...
        int ls = (m - l + 1);
        int rs = (r - m);
        cur->lx = cur->l ? cur->l->lx : ls + cur->r->lx;
        cur->rx = cur->r ? cur->r->rx : rs + cur->l->rx;
        cur->mx = max(
                      cur->l ? cur->l->mx : ls,
                      cur->r ? cur->r->mx : rs,
                      (cur->l ? cur->l->rx : ls) + (cur->r ? cur->r->lx : rs)
                      );
    }
}
tri get(node *cur, int l, int r, int x, int y){
    if (x < l) x = l;
    if (y > r) y = r;
    int siz = y - x + 1;
    if (x == l && y == r || !cur){
        return cur ? cur->get_tri() : tri{siz, siz, siz};
    }
    else{
        int m = (l + r) >> 1;
        if (y <= m)
            return get(cur->l, l, m, x, y);
        else if (x > m)
            return get(cur->r, m + 1, r, x, y);
        else
        {
            tri lres = get(cur->l, l, m, x, m);
            tri rres = get(cur->r, m + 1, r, m + 1, y);
            int rs = y - m;
            int ls = m - x + 1;
            return{
                lres.a == ls ? lres.a + rres.a : lres.a,
                max(lres.b, rres.b, lres.c + rres.a),
                rres.c == rs ? rres.c + lres.c : rres.c
            };
        }
    }
}
...
int main()
{
    ...
    while (k--){
        ...
        while (lb < rb){
            int m = (lb + rb) >> 1;
            auto bp = lower_bound(b, b + n, pii(m, -1));
            if (bp == b){
                lb = m + 1;
            }
            else
            {
                --bp;
                int ind = bp - b;
                tri res = get(roots + ind, 0, n - 1, l, r);
                if (res.b >= w){
                    lb = m + 1;
                }
                else{
