...
void solve(){
    int n;
    cin >> n;
    vector<tuple<int,string,int>> inputs;
    REP(i, 2 * n - 2) {
        string a;
        cin >> a;
        inputs.push_back(make_tuple(a.length(), a, i));
    }
    sort(ALL(inputs));
    reverse(ALL(inputs));
    REP(tea, 2) {
        int ok = 1;
        string ans = get<1>(inputs[0]);
        ans.push_back(get<1>(inputs[1]).back());
        for (int i = 1; i < inputs.size(); i += 2) {
            if (get<1>(inputs[i]) == ans.substr(0, get<0>(inputs[i]))) {
                string now = get<1>(inputs[i - 1]);
                if (ans.substr(ans.length() - now.length(), now.length()) != now) {
                    ok = 0;
                    break;
                }
                finaling[get<2>(inputs[i])] = 0;
                finaling[get<2>(inputs[i - 1])] = 1;
            }
            else {
                string now = get<1>(inputs[i]);
                if (ans.substr(ans.length() - now.length(), now.length()) != now) {
                    ok = 0;
                    break;
                }
                if (get<1>(inputs[i-1]) != ans.substr(0, get<0>(inputs[i]))) {
                    ok = 0;
                    break;
                }
                finaling[get<2>(inputs[i])] = 1;
                finaling[get<2>(inputs[i - 1])] = 0;
            }
        }
