...
fn dfs(u: usize, e: &[Vec<usize>], t: &[i32], goal: i32, is_root: bool) -> (i32,Vec<usize>) {
	let mut sum = t[u];
	let mut	done = v!([2]);
	for &v in e[u].iter() {
		let (s,mut d) = dfs(v,e,t,goal,false);
		done.append(&mut d);
		if done.len() >= 2 {
			done.truncate(2);
			return (0,done);
...
fn main() {
	...
	let goal = sum / 3;
	let (_,res) = dfs(root, &e, &t, goal, true);
	if res.len() >= 2 {
		writeln!(rout, "{} {}", res[0]+1, res[1]+1).ok();
	} else {
		writeln!(rout, "-1").ok();
	}
}