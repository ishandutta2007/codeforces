...
NTT ntt;
vector<ll> multipoint_evaluation(vector<ll> c, vector<ll> p){
	vector<vector<ll>> segp, segc;
	int m=p.size(), sz=1;
	while(sz<m) sz<<=1;
	segp.resize(2*sz-1);
	segc.resize(2*sz-1);
	for(int i=0; i<m; i++){
		segp[i+sz-1].resize(2);
		segp[i+sz-1][1]=1;
		segp[i+sz-1][0]=MOD-p[i];
		if(segp[i+sz-1][0]>=MOD) segp[i+sz-1][0]-=MOD;
	}
	for(int i=sz-2; i>=0; i--){
		if(segp[2*i+1].empty() && segp[2*i+2].empty()) continue;
		if(segp[2*i+2].empty()) segp[i]=segp[2*i+1];
		else{
			segp[i]=ntt.multiply(segp[2*i+1], segp[2*i+2]);
			segp[i].resize(segp[2*i+1].size()+segp[2*i+2].size()-1);
		}
	}
	segc[0]=ntt.mod(c, segp[0]);
	for(int i=1; i<m+sz-1; i++){
		if(segp[i].empty()) continue;
		segc[i]=ntt.mod(segc[(i-1)>>1], segp[i]);
	}
	vector<ll> ans(m);
	for(int i=0; i<m; i++){
		ans[i]=segc[i+sz-1][0];
...
vector<ll> interpolation(vector<ll> x, vector<ll> y){
	int n=x.size();
	vector<vector<ll>> segp;
	int sz=1;
	while(sz<n) sz<<=1;
	segp.resize(2*sz-1);
	for(int i=0; i<n; i++){
	...
	vector<ll> df(n);
	for(int i=0; i<n; i++) df[i]=segp[0][i+1]*(i+1)%MOD;
	vector<ll> w=multipoint_evaluation(df, x);
	for(int i=0; i<n; i++) (y[i]*=inv(w[i]))%=MOD;
	auto solve=[&](auto solve, int l, int r, int k)->vector<ll>{
		if(r-l==1){
			vector<ll> c(1, y[l]);
			return c;
		}
		int m=(l+r)/2;
		if(segp[2*k+2].empty()){
			return solve(solve, l, m, 2*k+1);
		}
		int len=min(r, n)-l;
		vector<ll> c1=solve(solve, l, m, 2*k+1);
		vector<ll> c2=solve(solve, m, r, 2*k+2);
		vector<ll> ret=ntt.multiply(c1, segp[2*k+2]);
		vector<ll> w2=ntt.multiply(c2, segp[2*k+1]);
		for(int i=0; i<len; i++){
			ret[i]+=w2[i];
	...
	return solve(solve, 0, sz, 0);
}
vector<ll> stirling_first(int n){
	if(n==0){
		vector<ll> ret(1, 1);
		return ret;
	}
	if(n&1){
		vector<ll> ret(n+1);
		vector<ll> s=stirling_first(n-1);
		for(int i=0; i<n; i++){
			ret[i+1]+=s[i];
			ret[i]+=MOD-s[i]*(n-1)%MOD;
			if(ret[i]>=MOD) ret[i]-=MOD;
			if(ret[i+1]>=MOD) ret[i+1]-=MOD;
		}
		return ret;
	}
	vector<ll> s=stirling_first(n>>1);
	return ntt.multiply(s, ntt.shift(s, MOD-(n>>1)));
}
...
int C(int n, int k)
{
    ...
    return ll(fc[n]) * iv[k] % MOD * iv[n - k] % MOD;
}
...
int main()
{
	...
	scanf("%d %I64d", &n, &k);
	if (k >= n) { printf("0\n"); return 0; }
	vector<ll> s=stirling_second(n);
	k = n - k;
	int res = ll(s[k]) * C(n, k) % MOD * fc[k] % MOD;
	if (k < n) res = 2ll * res % MOD;
	printf("%d\n", res);
	return 0;
}