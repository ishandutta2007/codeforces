...
int main(void)
{
	...
	cin >> k;
	rep(i, 1, k) cin >> a[i];
	mat.at(1, 2) = mat.at(2, 2) = mint(1);
	mat.at(2, 1) = mint(2);
	
	ll mul = 1;
	rep(i, 1, k) mul *= a[i], chmin(mul, 10LL);
	if(mul == 1){
		cout << "0/1" << endl;
		return 0;
	}
	
	Matrix pmat = mat; mint bunbo = mint(2);
	rep(i, 1, k) pmat = pmat.pow(a[i]), bunbo = modpow(bunbo, a[i]);
	bunbo /= mint(2);
	cout << pmat.at(1, 1)/mint(2) << "/" << bunbo << endl;
