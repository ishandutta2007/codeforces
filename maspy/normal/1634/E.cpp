...
void solve() {
  ...
  FOR(k, K) if (len(IDS[k]) % 2 == 1) return NO();

  vi match(len(A));
  FOR(k, K) {
    auto& I = IDS[k];
    FOR(j, len(I) / 2) {
      match[I[2 * j + 0]] = I[2 * j + 1];
      match[I[2 * j + 1]] = I[2 * j + 0];
    }
  }
  vi grp(len(A));
  FOR(n, N) FOR3(i, indptr[n], indptr[n + 1]) grp[i] = n;

  FastSet ss(len(A));
  FOR(i, len(A)) ss.insert(i);

  vi ANS(len(A));

  while (1) {
    auto root = ss.prev(len(A));
    if (root == -1) break;
    ll v = root;
    while (1) {
      ll w = match[v];
  ...
  FOR(n, N) {
    string S;
    FOR3(i, indptr[n], indptr[n + 1]) { S += (ANS[i] == 0 ? 'L' : 'R'); }
    print(S);
  }
}
