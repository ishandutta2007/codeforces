...
int f2(int flen, int blen) {
	int& res = dp2[flen][blen];
	if (res != -1) return res;
	if (flen + blen == len) {
		return res = 1;
	}
	res = 0;
	int cleft = len - flen - blen;
	if (S[cleft - 1] == T[flen]) {
		res += f2(flen + 1, blen);
		if (res >= MOD) res -= MOD;
	}
	if (S[cleft - 1] == Trev[blen]) {
		res += f2(flen, blen + 1);
		if (res >= MOD) res -= MOD;
	}
	return res;
...
int f1(int pleft, int flen) {
	int& res = dp1[pleft][flen];
	if (res != -1) return res;
	res = 0;
	int cleft = len - flen;
	if (cleft == 0) {
