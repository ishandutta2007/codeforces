...
int solve() {
    ...
    for (int i = 0; i < n; ++i) {
        dp[i][1] = cost(i, i - 1) + cost(i, i + 1);
    }
    for (int i = 0; i < n; ++i) {
        for (int j = 1; j <= i / 2 + 1; ++j) {
            if (i + 2 < n) {
                dp[i + 2][j + 1] = min(dp[i + 2][j + 1], dp[i][j] + cost(i + 2, i + 3) + max(0, cost(i + 2, i + 1) - cost(i, i + 1)));
            }
            pr[j] = min(pr[j], dp[i][j]);
            if (i + 3 < n) {
                dp[i + 3][j + 1] = min(dp[i + 3][j + 1], pr[j] + cost(i + 3, i + 4) + cost(i + 3, i + 2));
            }
        }
    }
    for (int i = 1; i <= k; ++i) {
