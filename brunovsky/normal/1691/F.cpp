...
int main() {
    ...
    vector<int> subsize(N, 1), parent(N, -1);
    ...
    num ans = N * B::choose(N - 1, K - 1) * N; // (1)
    for (int u = 0; u < N; u++) {
        vector<int> subs;
        for (int v : tree[u]) {
            subs.push_back(v == parent[u] ? N - subsize[u] : subsize[v]);
        }
        num bad = 0;
        for (int S : subs) {
            bad += B::choose(S, K);
        }
        ans += (B::choose(N - 1, K) - bad) * N; // (2)
        for (int S : subs) {
            int R = N - S - 1;
            bad -= B::choose(S, K);
            ans += S * (B::choose(R, K) - bad) * (R + 1); // (3)
            ans += S * B::choose(R, K - 1) * (R + 1);     // (4)
            bad += B::choose(S, K);
        }
    }
    cout << ans << '\n';
