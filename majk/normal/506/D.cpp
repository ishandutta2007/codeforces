...
public:
    void solve(istream& cin, ostream& cout) {
        ui N, M; cin >> N >> M;
        vector<vector<pii>> E(M); // color -> edge*
        vector<map<int,int>> C(N); // vertex -> color*
        vector<map<int,int>> V(M); // color -> vertex*
        for (ui i = 0; i < M; ++i) {
            int a,b,c; cin >> a >> b >> c; --a; --b; --c;
            E[c].emplace_back(a,b);
            if (C[a].find(c) == C[a].end()) {
                auto j = (int)V[c].size();
                C[a][c] = j;
                V[c][a] = j;
            }
            if (C[b].find(c) == C[b].end()) {
                auto j = (int)V[c].size();
                C[b][c] = j;
                V[c][b] = j;
            }
        ...
        vector<UnionFind> U;
        for (int i = 0; i < M; ++i) {
            U.emplace_back(V[i].size());
            for (pii&e:E[i]) {
                U[i].unite(C[e.x][i], C[e.y][i]);
            }
        }
        ...
        for (int i = 0; i < Q; ++i) {
            ...
            if (C[u].size() == C[v].size() && u > v) swap(u,v);
            ...
            if (it != Z.end()) {
                ...
                for (auto&c:C[u]) {
                    int col = c.x;
                    int uIdx = c.y;
                    auto it2 = C[v].find(col);
                    if (it2 != C[v].end()) {
                        int vIdx = it2->y;
                        ans += U[col].united(uIdx, vIdx);
                    }
                }
