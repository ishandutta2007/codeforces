...
int main(void)
{
	...
	ll cnt = 0, pre; vector<P> ans;
	for(auto p : mp){
		cnt += p.second;
		if(cnt-p.second < k && cnt >= k) pre = p.first;
		if(cnt-p.second >= k && cnt < k) ans.push_back(P(pre, p.first-1));
	}
	
	outl(sz(ans));
	for(auto p : ans) cout << p.first/2 << " " << p.second/2 << "\n";
