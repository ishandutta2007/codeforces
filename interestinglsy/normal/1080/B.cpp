...
int main(){
	...
	For(qwq,q){
		lRead2(l,r);
		ll fua = l&1LL ? -l : -(l+1LL);
		ll fub = r&1LL ? -r : -(r-1LL);
		ll fucnt = (fua-fub) / 2LL + 1LL;
		ll ans = (fua+fub) * fucnt / 2LL;
		ll zha = l&1LL ? l+1LL : l;
		ll zhb = r&1LL ? r-1LL : r;
		ll zhcnt = (zhb-zha) / 2LL + 1LL;
		ans += (zha+zhb) * zhcnt / 2LL;
		cout << ans << endl;
	}
