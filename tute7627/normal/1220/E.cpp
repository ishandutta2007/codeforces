...
vector<ll>t(MAX_V,-1);//
vector<ll>w(MAX_V,-1);
pair<bool,P> dfs(ll k,ll d){
  t[k]=d;
  ll maxbuf=0,sumbuf=0;
  bool judge=false;
  for(ll i=0;i<G[k].size();i++){
    if(t[G[k][i]]==-1){
      auto x=dfs(G[k][i],d+1);
      if(x.fi){
        judge=true;
        sumbuf+=x.se.fi;
        chmax(maxbuf,x.se.se);
      }
      else chmax(maxbuf,x.se.se); 
    }
    else if(t[G[k][i]]!=d-1){
      judge=true;
    }
  }
  if(judge)return {judge,{sumbuf+w[k],maxbuf}};
  else return {judge,{0,maxbuf+w[k]}};
}
...
int main(){
  ...
  ll s;cin>>s;s--;
  auto x=dfs(s,0);
  //cout<<x.fi spa x.se.fi spa x.se.se<<endl;
