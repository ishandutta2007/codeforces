...
bool conclockwise(int i, int j, int k)
{
    cout<<2<<' '<<i<<' '<<j<<' '<<k<<endl;
    int c;
    cin>>c;
...
int main() {
    ...
    cin>>n;
    int idx = 2;
    for (int i = 3; i<=n; i++) if (conclockwise(1, i, idx)) idx = i;

    vector<pair<ll, int>> areas;
    for (int i = 2; i<=n; i++) if (i!=idx)
    {
        areas.push_back(mp(area(1, idx, i), i));
    }
    sort(areas.begin(), areas.end());
    int big = areas.back().second;
    ...
    {
        if (conclockwise(1, areas[i].second, big)) left.push_back(areas[i].second);
        else right.push_back(areas[i].second);
    }
    vector<int> res = {1, idx};
    for (auto it: left) res.push_back(it);
    res.push_back(big);
    reverse(right.begin(), right.end());
    for (auto it: right) res.push_back(it);
    cout<<0<<' ';
