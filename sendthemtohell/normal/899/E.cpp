...
struct seg {
    ...
    int wt;
    seg() {}
    seg(int a, int b, int c, int d) { l = a; r = b; val = c; wt = d; }
    
};

bool operator<(const seg &oper1, const seg &oper2) { if (oper1.wt != oper2.wt) { return oper1.wt > oper2.wt; } return oper1.l < oper2.l; }
...
inline void solve(ld tt) {
    ...
    for (int i = 1; i < n; ++i) {
        if (vc != v[i]) {
            seg tmp = seg(lc, i - 1, vc, i - lc);
            end_[i - 1] = st[lc] = tmp;
            lc = i; vc = v[i];
            cur.insert(tmp);
        }
    }
    seg tmp = seg(lc, n - 1, vc, n - lc);
    end_[n - 1] = st[lc] = tmp;
    cur.insert(tmp);
    int cnt = 0;
    for (; sz(cur); ++cnt) {
        auto tmp = *cur.begin();
        cur.erase(tmp);
        if (tmp.l != 0 && tmp.r != n - 1 && end_[tmp.l - 1].val == st[tmp.r + 1].val) {
            cur.erase(st[tmp.r + 1]);
            cur.erase(end_[tmp.l - 1]);
            auto tmp2 = end_[tmp.l - 1];
            end_.erase(tmp.l - 1);
            end_[st[tmp.r + 1].r].wt += tmp2.wt;
            end_[st[tmp.r + 1].r].l = tmp2.l;
            st[tmp2.l].wt = end_[st[tmp.r + 1].r].wt;
            st[tmp2.l].r = st[tmp.r + 1].r;
            st.erase(tmp.r + 1);
            st.erase(tmp.l);
            end_.erase(tmp.r);
            cur.insert(st[tmp2.l]);
        }
        else if (tmp.l != 0) {
            cur.erase(end_[tmp.l - 1]);
            auto tmp2 = end_[tmp.l - 1];
