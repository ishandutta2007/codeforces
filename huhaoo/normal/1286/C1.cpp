...
{
	printf("? %d %d\n",1,n); fflush(stdout);
	fr(i,0,n+2){ e[i].clear(); fr(j,0,25) a[i][j]=0; }
	fr(i,1,n*(n+1)/2)
	{
		scanf("%s",s[i]);
...
//			printf("%d %c\n",i,ans[i]);
		...
		if(!c2) c2=c1;
//		printf("%d %d %c %c\n",i,n-i+1,c1,c2);
		v[i].push_back(mp(n-i+1,mp(c1,c2)));
		v[n-i+1].push_back(mp(i,mp(c1,c2)));
	}
}
void dfs(int u)
{
