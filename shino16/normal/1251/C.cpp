...
int main() {
  ...
  rep(t) {
    ...
    int n = a.size();
    queue<char> q[2];
    for (char c : a) q[c%2].push(c);
    if (q[0].empty()) { OUT(a); continue; }
    string s;
    while (not q[0].empty()) {
      sort(q, q+2);
      if (not q[0].empty()) s.push_back(q[0].front()), q[0].pop();
      if (q[0].empty()) while (not q[1].empty()) s.push_back(q[1].front()), q[1].pop();
    }
    OUT(s);
  }
}