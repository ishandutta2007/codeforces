...
bool solve(vector<int> a) {
    ...
    int oneId = find(a.begin(), a.end(), 1) - a.begin();
    int oneCount = 0;
    FOR(i, n) if (a[i] == 1) oneCount++;
    if (oneCount != 1) return false;
    FOR(i, n - 1) {
        int pv = a[(oneId - i + n) % n];
        int nt = a[(oneId - i - 1 + n) % n];
        if (nt > pv + 1) return false;
    }
    return true;
}
...
int main() {
    ...
    while (t--) {
        int n; reader(n);
        vector<int> a = readerArray<int>(n);
        bool ans = solve(a);
        puts(ans ? "YES" : "NO");
    }
