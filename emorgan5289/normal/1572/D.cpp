...
int main() {
    ...
    for (ll i = 0; i < 1ll<<n; i++) {
        for (ll j = 1; j < 1ll<<n; j <<= 1)
            if (i&j) {
                s.insert({a[i]+a[i^j], i, i^j});
                if (s.size() > 2*k*(n+1)) s.erase(s.begin());
            }
        if (i%1000 == 0) debug(i);
    }
    debug(s);
    ...
    for (auto [x, i, j] : s) {
        if (__builtin_popcountll(i)&1) swap(i, j);
        f.edge(get(i), get(j), 1, -x);
    }
    int S = N-1, Sp = N-2, T = N-3;
    for (auto [i, j] : c) {
