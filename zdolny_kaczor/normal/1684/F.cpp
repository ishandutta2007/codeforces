...
void solve() {
	...
	for (int i = 1; i < n; ++i) maxi(reach[i], reach[i - 1]);
	debug(imie(a), imie(reach));
	vi best_paired(n);
	int min_r = inf, max_l = -1;
	REP(l, n) {
		int r = reach[l];
		int v = a[l];
		auto &occ = occs[v];
		auto last = upper_bound(occ.begin(), occ.end(), r);
		last--;
		auto first = lower_bound(occ.begin(), occ.end(), l);
		assert(*first == l);
		if (first != last) {
			if (last - first >= 2) {
				auto second = first;
				second++;
				auto second_to_last = last;
				second_to_last--;
				maxi(must_max, *second_to_last);
				mini(must_min, *second);
			}
			alts.emplace_back(*first, *last);
		}
	}
	debug(imie(alts), imie(must_max), imie(must_min));
	for (auto [l, r] : alts) {
		maxi(best_paired[l], r);
		mini(min_r, r);
		maxi(max_l, l);
	}
	int top = max(must_max, max_l);
	if (must_max < must_min && alts.empty()) {
		puts("0");
		return;
	}
	int ans = n;
	REP(l, n) {
		if (l <= must_min && l <= min_r) {
			debug(range(a, l, top + 1));
			mini(ans, top - l + 1);
		}
		maxi(top, best_paired[l]);
	}
	printf("%d\n", ans);
}
int main() {
