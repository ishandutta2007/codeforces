...
const int N=1005,M=2234567;
int n,a[N],res[N][N],m;
vector<int> g[N];
vector<vector<int>> ng;
int main(){
	scan(n),m=1;
	for(int i=1;i<=n+1;++i) g[1].pb(i);
	for(int i=1,x;i<=n;++i){
		scan(x);
		res[g[1][0]][i]=0;
		res[g[1][1]][i]=1;
		--x;
		vector<int> other;
		for(int j=1;j<=n+1&&x;++j) if(j!=g[1][0]&&j!=g[1][1]) {
			res[j][i]=1;
			--x;
		}
		for(int j=1;j<=m;++j){
			vector<int> lt,rt;
			for(int u:g[j]) if(res[u][i]==1) lt.pb(u); else rt.pb(u);
			if(lt.size()>1) ng.pb(lt);
			if(rt.size()>1) ng.pb(rt);
		}
		m=ng.size();
		for(int i=1;i<=m;++i) g[i]=ng[i-1];
		ng.clear();
	}
	print(n+1),putc('\n');
	for(int i=1;i<=n+1;++i){
