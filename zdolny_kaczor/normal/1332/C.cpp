...
int main() {
	...
	REP(_, t) {
		...
		scanf("%d%d%s", &n, &k, in);
		reset(n);
		for (int i = 0; i < n; ++i) onion(i, n - i - 1);
		for (int i = 0; i + k < n; ++i) onion(i, i + k);
		REP(i, n) occs[find(i)][in[i]]++;
		int ans = 0;
		REP(i, n) if (!occs[i].empty()) {
			int lar = 0, sum = 0;
			for (auto [_, v] : occs[i]) {
				maxi(lar, v);
				sum += v;
