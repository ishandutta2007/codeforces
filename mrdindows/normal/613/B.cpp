...
//    FREIN("in.txt");
    ...
    reverse(p, p + n);
    for (int i = 1; i <= n; ++i) {
        ss[i] = ss[i - 1] + p[i - 1].first;
//        DBN(i, ss[i]);
    }
//    printarray(p, n);
    int bestLp = 0, bestRp = 0, bestT = 0;
    for (int lp = 0; lp < n - 1; ++lp) {
        ...
        ll free;
        while (true) {
            free = m - ps - (p[rp].first * 1ll * (n - rp) - (ss[n] - ss[rp]));
            if (free < 0) {
                ++rp;
            } else {
                break;
...
//        DBN(res);
    ...
    cout << ans << endl;
    for (int i = 1; i <= bestLp; ++i) {
        p[i].first = A;
    }
    for (int i = bestRp; i < n; ++i) {
        p[i].first = bestT;
    }
    for (int i = 1; i < n; ++i) {
