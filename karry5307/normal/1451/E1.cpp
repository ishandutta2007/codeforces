...
{
	n=read();
	cout<<"XOR 1 2"<<endl,x=read();
	cout<<"XOR 2 3"<<endl,y=read();
	cout<<"OR 1 3"<<endl,z=read(),w=x^y;
	cout<<"AND 1 2"<<endl,qwq=read();
	cout<<"AND 2 3"<<endl,qaq=read();
	for(register int i=1;i<n;i<<=1)
	{
		if(z&i)
		{
			if(w&i)
			{
				if(y&i)
				{
					qwq&i?r[1]+=i,r[2]+=i:r[3]+=i;
				}
				else
				{
					qaq&i?r[2]+=i,r[3]+=i:r[1]+=i;
				}
			}
			else
			{
				r[1]+=i,r[3]+=i,!(x&i)?r[2]+=i:1;
			}
		}
		else
		{
	...
	{
		cout<<"XOR 1 "<<i<<endl,r[i]=r[1]^read();
	}
	cout<<"! ";
