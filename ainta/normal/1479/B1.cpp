...
int main(){
    ...
    rep(i,n){
        ...
        if(w[i]!=w[i-1]){
            ...
            int x = w[i-1];
            int g = max(IT.Max(1,0,SZ-1,0,w[i]-1),IT.Max(1,0,SZ-1,w[i]+1,SZ-1)) + 1;
            IT.Add(1,0,SZ-1,0,SZ-1,1);
            int t = IT.Max(1,0,SZ-1,x,x);
            if(t < g) IT.Add(1,0,SZ-1,x,x,g-t);
        }
        else{
            int x = w[i-1];
            int g = max(IT.Max(1,0,SZ-1,0,x-1),IT.Max(1,0,SZ-1,x+1,SZ-1)) + 1;
            //IT.Add(1,0,SZ-1,0,SZ-1,1);
            int t = IT.Max(1,0,SZ-1,x,x);
            if(t < g) IT.Add(1,0,SZ-1,x,x,g-t);
        }
       /* for(int j=0;j<=3;j++){
            printf("%d ",IT.Max(1,0,SZ-1,j,j));
        }
        puts("");*/
    }
    printf("%d\n",IT.Max(1,0,SZ-1,0,SZ-1));
}