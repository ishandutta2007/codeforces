...
vector<P> ans;
void swp(vector<int>& p, vector<int>& pos, int a, int b) {
	ans.pb({a, b});
...
int main() {
	...
	vector<pair<P, int>> p1;
	vector<pair<P, int>> p2;
	REP(i, n) {
		if(a[i] < b[i]) p1.pb({{a[i], b[i]}, i});
		else p2.pb({{a[i], b[i]}, i});
	}
	sort(ALL(p1)); reverse(ALL(p1));
	sort(ALL(p2));
	vector<int> ans;
	if((int)p1.size() > (int)p2.size()) {
		REP(i, p1.size()) ans.pb(p1[i].second);
	} else {
		REP(i, p2.size()) ans.pb(p2[i].second);
	}
