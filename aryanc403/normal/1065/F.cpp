/************************************************************************************************************************************************************************************
  *                                                                                                                                                                                  *
   *                                                                                                                                                                                  *
    *  ****************    **************    *               *    ****************    *           *    ****************    *              *    ****************    ****************    *
     *  *              *    *            *    *               *    *              *    **          *    *                   *              *    *              *                   *    *
      *  *              *    *            *    *               *    *              *    * *         *    *                   *              *    *              *                   *    *
       *  *              *    *            *    *               *    *              *    *  *        *    *                   *              *    *              *                   *    *
        *  *              *    *            *    *               *    *              *    *   *       *    *                   *              *    *              *                   *    *
         *  *              *    *            *    *               *    *              *    *    *      *    *                   *              *    *              *                   *    *
          *  ****************    **************    *****************    ****************    *     *     *    *                   ****************    *              *    ****************    *
           *  *              *    *  *                      *            *              *    *      *    *    *                                  *    *              *                   *    *
            *  *              *    *    *                    *            *              *    *       *   *    *                                  *    *              *                   *    *
             *  *              *    *      *                  *            *              *    *        *  *    *                                  *    *              *                   *    *
              *  *              *    *        *                *            *              *    *         * *    *                                  *    *              *                   *    *
               *  *              *    *          *              *            *              *    *          **    *                                  *    *              *                   *    *
                *  *              *    *            *            *            *              *    *           *    ****************                   *    ****************    ****************    *
                 *                                                                                                                                                                                  *
                  *                                                                                                                                                                                  *
                   ************************************************************************************************************************************************************************************/
/*
  Warn - Don't change next line else you will get WA verdict. Online Judge is configured to give WA if next line is not present.
  Author - Aryan Choudhary (@aryanc403)
 
  const short DEBUG { 0 };
  #define debug(x) if (DEBUG) cout << #x << " = " << x << '\n'
*/

#pragma warning(disable:4996)
#pragma comment(linker, "/stack:200000000")
#pragma GCC optimize ("Ofast")
#pragma GCC target ("sse,sse2,sse3,ssse3,sse4,popcnt,abm,mmx,avx,tune=native")
#pragma GCC optimize ("-ffloat-store")

#include<iostream>
#include<bits/stdc++.h>
#include<stdio.h>
using namespace std;
#define fo(i,n)   for(i=0;i<(n);++i)
#define repA(i,j,n)   for(i=(j);i<=(n);++i)
#define repD(i,j,n)   for(i=(j);i>=(n);--i)
#define pb push_back
#define mp make_pair
#define X first
#define Y second
#define endl "\n"
#define PI 3.1415926535897932384626433832795
typedef long long int lli;
typedef long double mytype;
typedef pair<lli,lli> ii;
typedef vector<ii> vii;
typedef vector<lli> vi;

//const lli [3] ={ 999119999L,1000000007L,1000992299L};
//const lli [3] ={ 97L,101L,103L};
//const lli  = chrono::high_resolution_clock::now().time_since_epoch().count();
clock_t time_p=clock();
void aryanc403()
{
    time_p=clock()-time_p;
    cerr<<"Time Taken : "<<(float)(time_p)/CLOCKS_PER_SEC<<"\n";
}

class CMP
{
public:
    bool operator()(ii a , ii b) //sum,hei//For min priority_queue .
    {
        return ! ( a.X < b.X || (a.X==b.X&&a.Y>b.Y));
    }
};

void add( map<lli,lli> &m, lli x)
{
    map<lli,lli> ::iterator jt;
    jt=m.find(x);
    if(jt==m.end())         m.insert(mp(x,1));
    else                    jt->Y++;
}

void del( map<lli,lli> &m, lli x)
{
    map<lli,lli> ::iterator jt;
    jt=m.find(x);
    if(jt->Y==1)            m.erase(jt);
    else                    jt->Y--;
}

bool cmp(const ii &a,const ii &b)//sum,hei
{
    return a.X>b.X||(a.X==b.X&&a.Y<b.Y);
}
const lli INF = 0xFFFFFFFFFFFFFFFL;
const lli mod = 1000000007L;

// lli tin[200005],tout[200005],T;
// bool isAnc(lli u,lli v)
// {
//     return tin[u]<=tin[v]&&tout[v]<=tout[u];
// }
// lli dfs(lli u,lli p)
// {
//     tin[u]=++T;
//     dp[u][0]=p;
//     h[u]=h[p]+1;
//     for(auto &v:a[u])
//     {
//         if(v==p)
//             continue;
//         dfs(v,u);
//     }
//     tout[u]=++T;
// }

    lli n,i,m,j,k,in,cnt,l,r;
    lli p[1000005],dp[1000005],dp2[1000005];
    vi a[1000005];
    vi :: iterator it;
    string s;

lli lvf[1000005];
lli dfs1(lli u)
{
    if(a[u].empty())
    {
        dp2[p[u]]+=1;
        dp[u]=dp2[u]=0;
        return k;
    }
    dp[u]=dp2[u]=0;
    lli mx=0;
    lli upto=0;
    for(auto &v:a[u])
    {
        lli val=dfs1(v);
        if(val>0)
        {
            dp2[u]+=dp2[v];
            mx=max(mx,dp[v]-dp2[v]);
        }
        else
        {
            mx=max(mx,dp[v]);
        }
        upto = max(val,upto);
    }
    dp[u]=dp2[u]+mx;
    return upto-1;
}

int main(void) {
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
    // freopen("txt.in", "r", stdin);
    // freopen("txt.out", "w", stdout);

    cin>>n>>k;
    repA(i,2,n)
    {
        cin>>in;
        p[i]=in;
        a[in].pb(i);
    }
    dfs1(1);
    cout<<dp[1]<<endl;
    //repA(i,1,n)     cout<<i<<" : "<<dp[i]<<" "<<dp2[i]<<endl;
    aryanc403();
    return 0;
}