...
void solve(int _) {
    ...
    vi o(n);
    iota(all(o), 0);
    sort(all(o), [&](int i, int j) {return b[i] < b[j] || (b[i] == b[j] && a[i] > a[j]);});

    vector <int> steps;


    vll dp(k + 1, LLONG_MIN);
    vvi chain(k + 1);
    ...
    for (int z : o) {
        ford(r, k + 1) {
            ...
            if (r > 0 && dp[r - 1] != LLONG_MIN) {
                if (uax(dp[r], dp[r - 1] + (r - 1) * b[z] + a[z])) {
                    chain[r] = chain[r - 1];
                    chain[r].push_back(z);
                }
            }
    ...
    for (int e : o) {
        if (find(all(chain[k]), e) != chain[k].end() || e == v - 1) {
            continue;
        }
