...
fun main(){
    solve.cases{
        ...
        fun inversoinStack(a:String):List<MutableList<IntArray>> {
            val before = IntArray(26)
            val stacks = List(26){ mutableListOf<IntArray>() }
//            val stacks = Array(a.length){IntArray(26)}
            for((i,c) in a.withIndex()){
                stacks[c.code].add(before.copyOf())
                before[c.code] ++
            }
            return stacks
        }
        ...
        val A = inversoinStack(str)
        val B = inversoinStack(str.reversed())
        ...
        for(i in 0 until 26){
            for(j in A[i].indices){
                for(k in 0 until 26){
                    ret += (A[i][j][k] - B[i][j][k]).absoluteValue
                }
            }
        }
        put(ret/2)
