...
void ma1n(){
	...
	for(int i=3;i>=1;i--){
		...
		fio<<j<<':'<<j-i<<endl;
		vector<pii> nums;
		int I=6-i,J=j,K=a,L=b;
		while(I||J||K||L){
			//cerr<<I<<' '<<J<<' '<<K<<' '<<L<<endl;
			int to=lst[I][J][K][L];
			if(to<0){
				to=-to-1;
				nums.pb(mp(to,max(to+2,I==5?15:25)));
				K-=to;
				L-=max(to+2,I==5?15:25);
				I--;
			}else{
				to=to-1;
				nums.pb(mp(max(to+2,I==5?15:25),to));
				J--;
				K-=max(to+2,I==5?15:25);
				I--;
				L-=to;
			}
		}
		for(int i=SZ(nums)-1;i>=0;i--){
			fio<<nums[i].first<<':'<<nums[i].second<<' ';
		}
		fio<<endl;
		return;
	}
...
int main(){
	...
	dp[0][0][0][0]=1;
	for(int i=1;i<=5;i++){
		for(int j=max(0,i-3);j<=min(i,3);j++){
			if(i-j>3){
				continue;
			}
			for(int k=0;k<=200;k++){
				for(int l=0;l<=200;l++){
					for(int a=0;a<=k;a++){
						if(l>=max(a+2,i==5?15:25)&&j!=3){
							if(dp[i-1][j][k-a][l-max(a+2,i==5?15:25)]){
								dp[i][j][k][l]=1;
								lst[i][j][k][l]=-a-1;
							}
						}
					...
					for(int a=0;a<=l;a++){
						if(k>=max(a+2,i==5?15:25)&&j&&i-j!=3){
							if(dp[i-1][j-1][k-max(a+2,i==5?15:25)][l-a]){
								dp[i][j][k][l]=1;
								lst[i][j][k][l]=a+1;
							}
						}
