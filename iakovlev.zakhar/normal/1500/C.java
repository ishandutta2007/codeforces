...
public class C {
	...
	public void solve() {
		...
		boolean[][] less = new boolean[n - 1][m];
		int[] free = new int[m];
		boolean[] usedr = new boolean[n - 1];
		boolean[] usedc = new boolean[m];
		for (int i = 0; i < n - 1; i++) {
			for (int j = 0; j < m; j++) {
				less[i][j] = b[i + 1][j] < b[i][j];
				if (less[i][j]) {
					free[j]++;
				}
			}
		...
		comp = new int[ans.length];
		for (int i = 0; i < ans.length; i++) {
			comp[i] = ans[ans.length - 1 - i];
		}
		Arr[] arr = new Arr[n];
