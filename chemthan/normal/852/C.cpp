...
void solve() {
    ...
    FOR(i, 0, n) cin >> x[i];
    FOR(i, 0, n + n) {
        p[i] = PT(cos(PI * i / n), sin(PI * i / n));
    }
    vi ver;
    FOR(i, 0, n) {
        int j = (i + 1) % n;
        PT a = p[i + i] + (p[i + i + 1] - p[i + i]) * ((double) x[i] / n);
        PT b = p[j + j] + (p[j + j + 1] - p[j + j]) * ((double) x[j] / n);
        f[i] = area(a, b, p[i + i + 1]) - area(a, b, p[j + j]);
        ver.pb(i);
    }
    sort(all(ver), cmp);
    static int ans[maxn];
    FOR(i, 0, n) ans[ver[i]] = i;
