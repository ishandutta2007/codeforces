...
// source: https://codeforces.com/contest/1709/submission/165160774
...
int main()
{
  ...
  for (int i = n - 1; i >= 0; i--)
  {
    auto prod = convolution(dp, dp);
    if (!i) swap(dp, prod);
    else
    {
      vector<long long> newDp(k + 1);
      long long suffixSum = 0;
      for (int j = k * 2; j >= 0; j--)
      {
        if (j <= k)
          newDp[j] = (prod[j] * (k - j + 1) + suffixSum) % MOD;
        suffixSum = (suffixSum + prod[j]) % MOD;
      }
      swap(dp, newDp);
    }
  }
