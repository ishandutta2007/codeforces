...
fun main(){
    solve.cases{
        ...
        val sortedorder = L.withIndex().sortedByDescending { it.value }

        val ret = Array(n+1){BooleanArray(n)}

        val taken = BooleanArray(n)

        for(i in 0 until n){
            val mainindex = sortedorder[i].index
            val v = L[mainindex]
            assert(v <= n-i)
            for(c in i + 1 .. i + v){
                ret[c][mainindex] = true
            }
            for(b in 0 until n){
                if(taken[b] || b == mainindex) continue
                if(L[b] > 1){
                    L[b] --
                    ret[i][b] = true
                }
            ...
            L[mainindex] = 0
            taken[mainindex] = true
        }

        put(n+1)
        for(a in ret){
            put(CharArray(n){if(a[it]) '1' else '0' }.conca)
        }
