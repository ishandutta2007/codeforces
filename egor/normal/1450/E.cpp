...
public:
    void solve() {
        ...
        int m = in.readInt();
        arri u, v, b;
        in.readArrays(m, u, v, b);
        decreaseByOne(u, v);

        Graph<WeightedEdge<int>> graph(n);
        for (int i : range(m)) {
            graph.addEdge(u[i], v[i], b[i]);
            graph.addEdge(v[i], u[i], -b[i]);
        }
        arri color(n, 0);
        color[0] = 1;
        que<int> q;
        ...
        for (int i : range(n)) {
            ...
            while (true) {
                bool update = false;
                for (int j : range(m)) {
                    if (b[j] == 0 && abs(lim[u[j]] - lim[v[j]]) > 1) {
                        if (lim[u[j]] < lim[v[j]]) {
                            lim[v[j]] = lim[u[j]] + 1;
                            update = true;
                        } else {
                        ...
                        if (lim[u[j]] > lim[v[j]] - 1) {
                            lim[u[j]] = lim[v[j]] - 1;
                            update = true;
                        } else if (lim[v[j]] > lim[u[j]] + 1) {
                            lim[v[j]] = lim[u[j]] + 1;
                            update = true;
                        }
                    }
            ...
            if (good && answer < *max_element(all(lim))) {
                answer = *max_element(all(lim));
                copy(all(lim), delta.begin());
            }
        }
        if (answer == -1) {
            out.printLine("NO");
        ...
        while (true) {
            ...
            for (auto& s : sets) {
                for (auto& t : sets) {
                    ...
                    int minDelta = numeric_limits<int>::min();
                    int maxDelta = numeric_limits<int>::max();
                    int deltaOddity = -1;
                    for (int i : s) {
                        for (int j : t) {
                            if (rel(i, j) == -2) {
                            ...
                            if (deltaOddity != -1 && deltaOddity != abs(delta[i] - delta[j]) % 2) {
                                out.printLine("NO");
                                return;
                            }
                            deltaOddity = abs(delta[i] - delta[j]) % 2;
                            if (rel(i, j) == 0) {
                                maxim(minDelta, delta[j] - delta[i] - 1);
                                minim(maxDelta, delta[j] - delta[i] + 1);
                            } else {
                                maxim(minDelta, delta[j] - delta[i] + rel(i, j));
                                minim(maxDelta, delta[j] - delta[i] + rel(i, j));
                            }
                        }
                    }
