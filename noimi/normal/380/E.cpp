...
using S = pair<long double, long double>;
S f(S x, S y) { return S(x.fi * y.fi, x.fi * y.se + x.se); }
S g(S x, S y) { return S(x.fi * y.fi, y.fi * x.se + y.se); }
S e() { return S(1, 0); }
...
int main() {
    ...
    auto v = iota(a);
    reverse(all(v));
    vector<S> setter(n + 1, S(1, 1));
    atcoder::segtree<S, g, e> lseg(setter);
    atcoder::segtree<S, f, e> rseg(setter);
    ld ans = 0;
    ld s = 1.0 / n / n;
    for(auto i : v) {
        // cout << lseg.prod(0, i + 1).se << " " << rseg.prod(i + 1, n + 1).se << endl;
        ans += lseg.prod(0, i + 1).se * rseg.prod(i + 1, n + 1).se * a[i] * s * 0.5;
        lseg.set(i, S(0.5, 0.5));
        rseg.set(i + 1, S(0.5, 0.5));
        // cout << i << " " << ans << endl;
    }
    cout << ans << endl;
}