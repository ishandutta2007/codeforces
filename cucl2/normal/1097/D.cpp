...
INT calcdp(INT x,INT y){
	...
	for(int i=0;i<m;i++){
		for(int j=0;j<=y;j++){
			for(int k=0;k<=j;k++){
				dp[i+1][k]=(dp[i+1][k]+(dp[i][j]*nee[j+1])%Mod)%Mod;
				//cerr<<dp[i+1][k]<<' '<<dp[i][j]<<' '<<nee[j+1]<<' '<<i<<' '<<j<<' '<<k<<endl;
			}
		}
	}
	INT sum=0;
	INT xy=1;
	for(int i=0;i<=y;i++){
		sum=(sum+(dp[m][i]*xy)%Mod)%Mod;
		xy=xy*x%Mod;
		//cerr<<sum<<' '<<dp[m][y]<<' '<<xy<<endl;
	}
	//cerr<<sum<<endl;
	return sum;
}
...
int main(){
	...
	for(INT i=2;i*i<=n;i++){
		if(nn%i==0){
			...
			ans=ans*calcdp(i,cnt)%Mod;
			//cerr<<ans<<endl;
		}
	}
	if(nn!=1){
		ans=ans*calcdp(nn,1)%Mod;
		//cerr<<ans<<endl;
	}
	fio<<ans<<endl;
	return 0;
