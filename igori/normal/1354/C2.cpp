...
struct mcf{
    ...
    vector<long long> fr, to, cap, cost, f;
    vector<vector<int> > g;
    mcf(int _n)
    {
        g = vector<vector<int> >(_n);
    }
    void add_edge(long long Fr, long long To, long long Cap, long long Cost)
    {
        g[Fr].push_back(fr.size());
        fr.push_back(Fr), to.push_back(To), cap.push_back(Cap), cost.push_back(Cost), f.push_back(0);

        g[To].push_back(fr.size());
        fr.push_back(To), to.push_back(Fr), cap.push_back(0), cost.push_back(-Cost), f.push_back(0);
    }
    pair<long long, long long> levit_1()
    {
        vector<long long> dist(g.size(), INFLL);
        dist[0] = 0;
        vector<int> gr(g.size(), 2);
        vector<int> p(g.size());
        ...
        while (q1.size() || q2.size())
        {
            ...
            for (auto id : g[x])
            {
                if (f[id] < cap[id] && dist[fr[id]] + cost[id] < dist[to[id]])
                {
                    dist[to[id]] = dist[fr[id]] + cost[id];
                    p[to[id]] = id;
        ...
        if (dist[g.size() - 1] == INFLL) return {0, 0};
        long long D = INFLL;
        int mark = g.size() - 1;
        while (mark != 0)
        {
            D = min(D, cap[p[mark]] - f[p[mark]]);
            mark = fr[p[mark]];
        }
        mark = g.size() - 1;
        while (mark != 0)
        ...
        return {D, dist[g.size() - 1]};
    }
    pair<long long, long long> levit_2()
    {
        vector<long long> dist(g.size(), INFLL);
        dist[0] = 0;
        vector<int> gr(g.size());
        vector<int> p(g.size());
    ...
    void run()
    {
        while (1)
        {
            pair<long long, long long> sl = levit_1(); // use levit_1 or levit_2 here (TODO: add Johnson's algorithm)
            // levit_1 (2 queues)
            // levit_2 (deque)

            if (sl.first == 0) return;
            blocks.push_back(sl);
            if (sl.first >= INFLL / 2) return;
        }
    }
};
...
void solve()
{
    ...
    double PI = acos(-1);
    double len = sin(PI * (2 * n - 1) / (2 * n) / 2) / cos(PI * (n - 1) / n / 2);
    printf("%.9f\n", len);
}
