...
void solve(){
	...
	for(int i=1;i<=n;i++){
		if(i==1)cout<<(d[i]=c[i])<<" ";
		else if(c[i]!=d[i-1]&&i<n)cout<<(d[i]=c[i])<<" ";
		else if(b[i]!=d[i-1]&&i<n)cout<<(d[i]=b[i])<<" ";
		else if(i<n)cout<<(d[i]=a[i])<<" ";
		else if(i==n){
			if(c[n]!=d[n-1]&&c[n]!=d[1])cout<<c[n]<<" ";
			else if(b[n]!=d[n-1]&&b[n]!=d[1])cout<<b[n]<<" ";
			else cout<<a[n]<<" ";
		}
	} cout<<endl;
}
