...
int another_main()
{
	...
	for (int I = 0; I < t; ++I) {
		...
		if (k == 1) {
			int max_pairs = 0;
			for (int i = 0; i < n - 1; ++i)
				max_pairs = max(max_pairs, min(a[i], a[i + 1]));
			...
			for (int i = 0; i < n; ++i) {
				...
				int new_pairs = 0;
				if (i)
					new_pairs = max(new_pairs, min(INF, a[i - 1]));
				if (i < n - 1)
					new_pairs = max(new_pairs, min(INF, a[i + 1]));
				if (a[i] == min_1)
					new_ans = min(min_2 * 2, max(new_pairs, max_pairs));
				else
					new_ans = min(min_1 * 2, max(new_pairs, max_pairs));
				ans = max(ans, new_ans);
			}
			cout << ans << '\n';
			continue;
		...
		sort(whole(pairs));
		int possible_ans_1 = min(2 * pairs[k - 1].X, INF);
		for (int i = 0; i < k; ++i)
			a[pairs[i].Y] = INF;
		int possible_ans_2 = 2 * pairs[k].X;
		int max_pairs = 0;
		for (int i = 0; i < n - 1; ++i)
			max_pairs = max(max_pairs, min(a[i], a[i + 1]));
		cout << max(possible_ans_1, min(possible_ans_2, max_pairs)) << '\n';
	}
