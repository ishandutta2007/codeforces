...
long long m,x,n,y,z,k,mx,one,gd,two,mid,l,r,pos,a[500003],b[500003],c[500000];string s,s2;vector<int>v,v2,v3;long long sum,mn=1e9,ans,num;char ch;
...
vector<int> vecs[100001];bool used[100001];
int main()
{
...
for(int i=0;i<n;i++)
{
    cin>>a[i];a[i]=n-a[i];b[a[i]]++;vecs[a[i]].push_back(i);
}
z=1;
for(int i=0;i<n;i++)
{
    ...
    if(b[a[i]]<a[i])return cout<<"Impossible",0;
    b[a[i]]-=a[i];
    int j=a[i];
    while(j--)
    {
        int ind=*vecs[a[i]].begin();
        c[ind]=z;used[ind]=true;
        vecs[a[i]].erase(vecs[a[i]].begin());
    }
    z++;
}
}
