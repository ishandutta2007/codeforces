...
fun solve() {
    val n = io.int()
    var strings = io.strs(n)
    strings = strings.sortedWith(Comparator<String>{x,y -> x.length.compareTo(y.length)})
    var bad = false
    for (i in 0..strings.size-2) {
        val (a,b) = arrayOf(strings[i],strings[i+1])
        var ok = false;
        for (j in 0..b.length-a.length) {
            if (b.substring(j,j+a.length).equals(a)) ok = true;
        }
        if (!ok) bad = true;
    }
    if (bad) {
