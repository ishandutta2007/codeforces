...
constexpr int C = 1e3 + 1, B = 12 + 1, N = C * B;
int cost[C];
int profit[N];
int b[C], c[C];
...
int main() {
  ...
  cost[1] = 0;
  for (int i = 1; i < C; ++i)
  for (int j = 1; j <= i; ++j) if (i + i / j < C) {
    mini(cost[i + i / j], cost[i] + 1);
  }
  for (int testcase = $; testcase; --testcase) {
    int n = $, k = $;
    mini(k, n * (B - 1));
    memset(profit, 0, sizeof *profit * (k + 1));
    cin >> range{b, n} >> range{c, n};
    vector<int> prof[B];
    for (int i = 0; i < n; ++i) {
      prof[cost[b[i]]].push_back(c[i]);
    }
    for (auto& v: range{prof, B}) {
      sort(v.rbegin(), v.rend());
    }
    for (auto& v: range{prof + 1, B - 1})
    for (int i = k; i > -1; --i) {
      int s = 0, p = 0;
      for (auto c: v) {
        s += &v - prof, p += c;
        if (i >= s) {
          maxi(profit[i], profit[i - s] + p);
        }
      }
    }
    print(*max_element(profit, profit + k + 1) + accumulate(prof[0].begin(), prof[0].end(), 0));
  }
}