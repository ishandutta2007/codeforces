...
int main() {
    ...
    F0R(i,ind) tmp[t[i]-'0'] ++;
    string res = t; co[0][0] -= co[1][0], co[0][1] -= co[1][1];
    while (co[0][0] >= tmp[0] && co[0][1] >= tmp[1]) {
        co[0][0] -= tmp[0];
        co[0][1] -= tmp[1];
        FOR(i,sz(t)-ind,sz(t)) res += t[i];
    }
    while (co[0][0]) res += '0', co[0][0] --;
    while (co[0][1]) res += '1', co[0][1] --;
    pr(res);
     // you should actually read the stuff at the bottom
}
