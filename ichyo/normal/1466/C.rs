...
fn run() {
    ...
    for _ in 0..t {
        let s: String = read_line().unwrap();
        let s: Vec<char> = s.chars().collect();
        let mut dp = vec![vec![usize::max_value(); 1 << 2]; s.len() + 1];
        dp[0][0] = 0;
        for i in 0..s.len() {
            for t in 0..(1 << 2) {
                let val = dp[i][t];
                ...
                let mut ok = true;
                if i > 0 && s[i - 1] == s[i] && (t >> 0) & 1 == 0 {
                    ok = false;
                }
                if i > 1 && s[i - 2] == s[i] && (t >> 1) & 1 == 0 {
                    ok = false;
                }
                if ok {
                    dp[i + 1][(t << 1) & 3].set_min(val);
                }
                dp[i + 1][((t << 1) & 3) | 1].set_min(val + 1);
            }
        }
        let mut ans = usize::max_value();
        for t in 0..(1 << 2) {
            ans.set_min(dp[s.len()][t]);
        }
        answer += &format!("{}\n", ans);
    }
    print!("{}", answer);
