...
void solve() {
  ...
  auto f = [&](auto dfs, vi A, ll lim, vc<pi> history) -> void {
    if (!ANS.empty()) return;
    vi I;
    FOR(i, N + 1) if (abs(A[i] - A[i + 1]) != 1) I.eb(i + 1);
    if (I.size() == 0) {
      ANS = history;
      return;
    }
    if (I.size() > 2 * lim) return;
    ll n = I.size();
    FOR(i, n) if (I[i] - 1 > 0) I.eb(I[i] - 1);
    UNIQUE(I);

    FORIN(l, I) FORIN(r, I) if (l + 1 < r) {
      vi B = A;
