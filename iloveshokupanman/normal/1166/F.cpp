...
int main() {
    ...
    vector<set<ll>> st(N + 2 * M + 2 * Q);//leadernode

    auto ad_edge = [&](ll a, ll b, ll c) {
        st[d.leader(a)].insert(b);
        st[d.leader(b)].insert(a);
        ...
        ll t = d.leader(b);
        ll u = d.leader(node[{a, c}]);
        d.merge(b, node[{a, c}]);
        ll v = d.leader(b);

        if (t != v)for (ll x : st[t]) { st[v].insert(x); }
        if (u != v)for (ll x : st[u]) { st[v].insert(x); }
    ...
    rep(i, Q) {
        ...
        if (c == '+') {
            ...
            else if (st[d.leader(a)].count(b)) { cout << "Yes" << endl; }
            else { cout << "No" << endl; }
        }
    }
}