...
int main(){
  ...
  while(t--){
    ...
    dp[idx[0]]=1;
    vector<bool>loop(sz);
    rep(i,0,n)for(auto to:g[i]){
      if(idx[i]==idx[to])loop[idx[i]]=true;
    }
    rep(i,0,sz){
      if(dp[i]>0&&loop[i])dp[i]=3;
      for(auto to:sg[i]){
        if(dp[to]==1&&dp[i]==1)dp[to]=2;
        else chmax(dp[to],dp[i]);
      }
    }
    vector<ll>ret(n);
    rep(i,0,n)ret[i]=(dp[idx[i]]==3?-1:dp[idx[i]]);
    debug(ret,n);
  }
  return 0;
}