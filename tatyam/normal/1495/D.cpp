...
int main(){
    ...
    rep(n)ans[i][i]=[&]{
        ...
        rep(n)if(i!=at){
            Modint cnt=0;
            each(e,g[i])if(c[e]==c[i]-1)cnt++;
            ans*=cnt;
        }
        return ans;
    }();
    rep(n)rep(j,i+1,n)ans[i][j]=ans[j][i]=[&]{
        const ll mn=cost[i][j];
        Modint ans=1;
        rep(k,n){
            if(cost[i][k]+cost[j][k]!=mn){
                const ll c2=cost[i][k]+cost[j][k]-2;
                Modint cnt=0;
                each(e,g[k])if(cost[i][e]+cost[j][e]==c2)cnt++;
                ans*=cnt;
            }
            else{
                ll cnt=0;
                each(e,g[k])if(cost[i][e]+cost[j][e]==mn)cnt++;
                if(cnt!=2-(k==i)-(k==j))return 0_M;
            }
        }
        return ans;
    }();
