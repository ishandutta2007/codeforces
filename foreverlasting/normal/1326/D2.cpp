...
namespace MAIN{
    ...
    inline bool check(const string &s){
        for(res i=0,sz=int(s.size());i<sz;i++)if(s[i]!=s[s.size()-i-1])return 0;
        return 1;
    }
    inline void MAIN(){
        t="";
        cin>>s;
        res len=int(s.size());
        for(res i=0;i<=2*len;i++)nxt1[i]=nxt2[i]=0;
        if(check(s)){cout<<s<<endl;return;}
        res p=0,q=len-1;
        while(s[p]==s[q])p++,q--;
        t=s.substr(0,p),s=s.substr(p,q-p+1),rev=s,reverse(rev.begin(),rev.end()),s1=s+"$"+rev,s2=rev+"$"+s;
        res j=0;
        for(res i=2,sz=int(s1.size());i<=sz;i++){
            while(j&&s1[i-1]!=s1[j])j=nxt1[j];
            if(s1[j]==s1[i-1])j++;
            nxt1[i]=j;
        }
        j=0;
        for(res i=2,sz=int(s2.size());i<=sz;i++){
            while(j&&s2[i-1]!=s2[j])j=nxt2[j];
            if(s2[j]==s2[i-1])j++;
            nxt2[i]=j;
        }
        if(nxt1[s1.size()]<nxt2[s2.size()])ans=s2.substr(0,nxt2[s2.size()]);
        else ans=s1.substr(0,nxt1[s1.size()]);
        cout<<t<<ans,reverse(t.begin(),t.end()),cout<<t<<endl;
    }
}
int main(){
//    srand(time(0));
