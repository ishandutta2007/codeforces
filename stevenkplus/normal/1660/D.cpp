...
int main() {
   ...
   while (T--) {
      ...
      vector<int> v(n);
      for (int &i: v) cin >> i;
      vector<int> nxt0(n), prv0(n);
      prv0[0] = -1;
      for (int i = 0; i < n; ++i) {
         if (v[i] == 0) prv0[i] = i;
         if (i + 1 < n) prv0[i + 1] = prv0[i];
      }
      nxt0[n - 1] = n;
      for (int i = n - 1; i >= 0; --i) {
         if (v[i] == 0) nxt0[i] = i;
         if (i - 1 >= 0) nxt0[i - 1] = nxt0[i];
      }
      pp(prv0);
      int ans = 0;
      pii ansp;
      vector<int> signs(n + 1), twos(n + 1);
      signs[0] = 1;
      for (int i = 0; i < n; ++i) {
         signs[i + 1] = signs[i] * (v[i] < 0 ? -1 : 1);
         twos[i + 1] = twos[i] + (abs(v[i]) == 2 ? 1 : 0);
      }
      pp(v);
      auto check = [&](int l, int r) -> void {
         pp("check", l, r);
         if (signs[r] / signs[l] == 1) {
            if (twos[r] - twos[l] > ans) {
               ans = twos[r] - twos[l];
               ansp = pii(l, r);
            }
         }
      ...
      for (int i = 0; i < n; ++i) {
         ...
         if (v[i] < 0) {
            if (i + 1 < n) check(i + 1, nxt0[i + 1]);
            check(prv0[i] + 1, i);
         }
      }
      check(prv0[0] + 1, nxt0[0]);
      pp(ans);
      pp(ansp);
      cout << ansp.fi << " " << n - ansp.se << "\n";
   }
}
