...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    val types = IntArray(if (graphWeighed) m else 0)
    ...
    fun calculateDepthWeighted(): LongArray {
        val ret = LongArray(n)
        rootFirstEdge{from,to,e -> ret[to] = ret[from] + types[e]}
        return ret
    }
...
fun main(){
    solve.cases{
        ...
        val retdist = dist[0]
        put(if(retdist == plarge) -1 else retdist)
        for(i in 0 until n){
            if(types[i] == -1){
                types[i] = 0
            }
        }
        put(types.joinToString(""))
...
/*
another boolean swap another day (types != -1 vs types == -1)
