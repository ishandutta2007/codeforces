...
int main() {
	...
	while (T--) {
		...
		rep(i, 0, n) cin >> a[i];
		rep(i, 0, n) if (!~a[i]) {
			if (i && ~a[i - 1]) b.pb(a[i - 1]);
			if (i != n - 1 && ~a[i + 1]) b.pb(a[i + 1]);
		}
		int k = b.empty() ? 0 : *max_element(range(b)) + *min_element(range(b)) >> 1;
		rep(i, 0, n) if (!~a[i]) a[i] = k;
		int m = 0;
		rep(i, 1, n) maxx(m, abs(a[i] - a[i - 1]));
		cout << m << " " << k << "\n";
	}
}