...
fun main(){
    solve.cases{
        ...
        val right = mutableListOf<Pair<Int,Long>>()
        val ret = mutableListOf<Triple<Int,Int,Long>>()
        for((i,v) in balance.withIndex()){
            if(v > 0){
                left.add(Pair(i,v))
            }else if(v < 0){
                right.add(Pair(i,-v))
        ...
        while(left.isNotEmpty() && right.isNotEmpty()){
            ...
            val md = minOf(l.second, r.second)
            ret.add(Triple(r.first,l.first,md))
            val lp = l.second - md
            val rp = r.second - md
            if(lp > 0){
                left.add(Pair(l.first, lp))
            }
            if(rp >0){
                right.add(Pair(r.first, rp))
            }
        }
