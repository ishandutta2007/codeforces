...
fn run() {
    ...
    let inf = 3000u32;
    let mut graph = maxflow::Graph::new(n + a + b + 2);
    let src = n + a + b;
    let dst = src + 1;
    for (i, pa) in pa.iter().enumerate() {
        graph.add_edge(n + i + 1, dst, 1);
        graph.add_edge(n + i + 1, n + *pa, inf);
    }
    for (i, &x) in x.iter().enumerate() {
        graph.add_edge(i, n + x, inf);
    }
    for (i, pa) in pb.iter().enumerate() {
        graph.add_edge(src, n + a + i + 1, 1);
        graph.add_edge(n + a + *pa, n + a + i + 1, inf);
    }
    for (i, &x) in y.iter().enumerate() {
        graph.add_edge(n + a + x, i, inf);
    }
    let ans = (a + b - 2) as u32 - graph.flow(src, dst);
    println!("{}", ans);
}
