...
void solve()
{
	...
	vector<vector<int>> f(n * 4 + 1, vector<int>(26, -1));
	function<int(int, int, int, int)> dp = [&](int k, int L, int R, int x) -> int
	{
		if (f[k][x] != -1) return f[k][x];
		if (L == R)
		{
			return f[k][x] = 1 - (s[R][x] - s[L - 1][x]);
		}
		int m = (L + R) / 2;
		int Lres = dp(k << 1, L, m, x + 1) + (R - m) - (s[R][x] - s[m][x]);
		int Rres = dp(k << 1 | 1, m + 1, R, x + 1) + (m - L + 1) - (s[m][x] - s[L - 1][x]);
		return f[k][x] = min(Lres, Rres);
	};
	int res = dp(1, 1, n, 0);
	writeln(res);
}
