...
void solve() {
    ...
    sort(ALL(inputs));
    vector<pair<int, int>> nekos;
    REP(q, inputs.size()) {
        nekos.push_back(mp(inputs[q].second, q));
    }
    sort(ALL(nekos));
    int itr = nekos[0].second;
    int prom = nekos[1].second;
    int ans = 0;
    if (itr == n - 1) {
        ans = max(ans, inputs[prom].second - inputs[n - 2].first);
    }
    else {
        ans = max(ans, inputs[itr].second - inputs[n - 2].first);
        ans = max(ans, inputs[prom].second - inputs[n - 1].first);
    }
    cout << ans << endl;
}
#undef int
