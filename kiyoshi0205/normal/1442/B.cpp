...
int main(){
  ...
  brep(i,K){
    fch[v[i]]=1;
    if(v[i]==0||fch[v[i]-1])if(v[i]==N-1||fch[v[i]+1])break;
  }
  if(i>=0){
    print(0);
    continue;
  ...
  rep(i,K){
    auto itr=now.upper_bound(v[i]);
    auto sitr=now.lower_bound(v[i]);
    int f=itr==now.end()||se.count(*itr);
    int sf=sitr==now.begin()||se.count(*(--sitr));
    ans=ans*(2-f-sf)%mod;
    if(f&&sf)break;
    if(f)now.erase(*sitr);
    else now.erase(*itr);
