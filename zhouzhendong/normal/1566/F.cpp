...
void solve(){
	...
	auto ita=a.begin();
	for (auto i : s){
		while (ita!=a.end()&&i.fi>*ita)
			ita++;
		if (ita!=a.end()&&i.se>=*ita)
			continue;
		while (!t.empty()&&t.back().se>=i.se)
			t.pop_back();
		t.pb(i);
	}
	s.swap(t);
	...
	For(i,0,(int)a.size()-2){
		for (R=L;R<s.size()&&s[R].se<a[i+1];R++);
		//[L,R)
		if (L==R){
			ckmin(f,h);
			h=INF;
			...
			LL nf=INF,nh=INF;
			//1. xiang you
			LL cost=llabs(s[R-1].fi-a[i]);
			ckmin(nf,min(f+cost,h+cost*2));
			//2. xiang zuo
			cost=llabs(a[i+1]-s[L].se);
			ckmin(nf,min(f+cost*2,h+cost*2));
			ckmin(nh,min(f+cost,h+cost));
			//3. zuo you
			For(j,L,R-2){
				LL lp=s[j].fi,rp=s[j+1].se;
				LL cl=llabs(a[i]-lp);
				LL cr=llabs(a[i+1]-rp);
				ckmin(nf,min(f+cl+2*cr,h+cl*2+cr*2));
				ckmin(nh,min(f+cl+cr,h+cl*2+cr));
			}
			f=nf,h=nh;
		}
		outvals(f,h);
