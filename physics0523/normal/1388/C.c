...
int main(void){
  ...
  long long h[524288],p[524288];
  long long hp[524288],tp[524288];
  int perm[524288];
  rs g[524288];
  mkj x[524288];
  sd dat[524288];
  ...
  while(t>0){
    t--;
    scanf("%lld%lld",&n,&m);
    for(i=1;i<=n;i++){scanf("%lld",&p[i]);x[i].kz=0;}
    ...
    for(i=0;i<n;i++){
      ...
      tp[w]+=p[w];
      if(h[w]<(2*hp[w]-tp[w]) || tp[w]<h[w]){goto FAIL;}
      if((tp[w]-h[w])%2!=0){goto FAIL;}
      hp[w]=tp[w]-(tp[w]-h[w])/2;
    }
    puts("YES");
    continue;
    FAIL:
