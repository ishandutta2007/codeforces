#line 1 "codeforces-workspace/contest/1469/e/e.cpp"
/**
 * @file template.cpp
 * @brief Template
 */
...
#line 16 "codeforces-workspace/contest/1469/e/e.cpp"
...
#line 32 "codeforces-workspace/contest/1469/e/e.cpp"
...
void main() {
  ...
  rolling_hash_table rt(s);
  unordered_set<u64> ls;
  for (auto i = 0, j = 0; i + r <= n; ++i) {
    if (j >= k) ls.emplace(rt.substr(i, r).value);
    if (s[i] == '1')
      j = 0;
    else
      ++j;
  }
  for (auto f = 0; f < 1 << r; ++f) {
    string g(r, '0');
    for (auto j = 0, ff = f; j < r; ++j) {
      g[r - 1 - j] += (ff & 1);
      ff >>= 1;
    }
    if (ls.count(rolling_hash_table(g).substr().value)) continue;
    cout << "YES\n";
    g = string(k, '0') + g;
    cout << g << eol;
    return;
