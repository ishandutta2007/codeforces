...
int go(int h, int w, int k) {
  int& res = dp[h][w][k];
  if (res != -1) return res;
  if (h*w == k or k == 0) return res = 0;
  if (h*w < k) return res = INF;
  res = INF;
  rep(r, h+1) rep(l, k+1) chmin(res, go(r, w, l) + go(h-r, w, k-l) + w*w);
  rep(c, w+1) rep(l, k+1) chmin(res, go(h, c, l) + go(h, w-c, k-l) + h*h);
  return res;
}
