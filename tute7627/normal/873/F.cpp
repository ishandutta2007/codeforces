...
int main(){
  ...
  rep(i,0,n){
    if(t[i]=='0')chmax(res,n-i);
  }
  vector<ll>cum(n+1);
  rep(i,0,n)cum[i+1]=cum[i]+(t[sa[i]]=='0');
  set<ll>st;
  st.insert(-1);st.insert(n-1);
  vector<ll>ord(n-1);
  iota(ALL(ord),0);
  sort(ALL(ord),[&](ll i,ll j){return lcp[i]<lcp[j];});
  //debug(cum,n+1);
  for(auto i:ord){
    ll l=*--st.lower_bound(i);
    ll r=*st.upper_bound(i);
    //l+1,...r
    //cout<<l spa r spa lcp[i] spa cum[r+1] spa cum[l+1]<<endl;
    chmax(res,lcp[i]*(cum[r+1]-cum[l+1]));
    st.insert(i);
  }
  cout<<res<<endl;
  return 0;
