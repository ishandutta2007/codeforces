...
fn main() {
	...
	if k == 0 {
		...
		{
			let sum1 = a[0..n-1].iter().sum::<i64>();
			let min1 = *d[0..n-1].iter().min().unwrap();
			max!(sol, sum1 - min1 + (a[n-1] - d[n-1]).max(0));
		}
		{
			let sum = a[1..n].iter().sum::<i64>();
			max!(sol, sum - d[1]);
		}
		{
			let mut pre = v!([n]);
			...
			for i in (1..n).rev() {
				sum += a[i];
				max!(max, sum - d[i]);
				max!(sol, pre[i-1].max(0) + max);
			}
		}
		{
			let sum = a.iter().sum::<i64>();
			let min = *a[1..n].iter().min().unwrap();
			max!(sol, sum - min - d[0]);
		}
	} else {
		let sum = a.iter().sum::<i64>();
		let min = *d[0..n-1].iter().min().unwrap();
		max!(sol, sum - min);
	}
	writeln!(rout, "{}", sol).ok();
}