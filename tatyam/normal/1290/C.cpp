...
signed main(){
    ...
    bitset<300000>a;
    rep(n)a[i]=s[i]&1;
    vector<pair<bit,int>>b[300000];
    rep(k){
        LL(n);
        VEC(int,a,n);
        each(i,a)i--;
        sort(all(a));
        b[a.front()].emplace_back(bit(all(a)),1);
    }
    vec(int,ans,n+1);
    rep(n){
        if(a[i]){
            ans[i+1]=ans[i];
            if(b[i].size()>1){
                xor_set(b[i][0].first,b[i][1].first);
                if(b[i][0].first.size())b[*b[i][0].first.begin()].emplace_back(move(b[i][0].first),b[i][0].second+b[i][1].second);
            }
        }
        else{
            auto p=min_element(all(b[i]),[](const auto&a,const auto&b){return a.second<b.second;});
            ans[i+1]=ans[i]+p->second;
            each(i,p->first)a[i]=!a[i];
            if(b[i].size()>1){
                xor_set(b[i][0].first,b[i][1].first);
                if(b[i][0].first.size())b[*b[i][0].first.begin()].emplace_back(move(b[i][0].first),abs(b[i][0].second-b[i][1].second));
            }
        }
        vector<pair<bit,int>>a;
        b[i].swap(a);
    }
    ans.erase(ans.begin());
    each(i,ans)out(i);
