// time-limit: 3000
// problem-url: https://codeforces.com/contest/1656/problem/D
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	int64_t const n=read<int64_t>();
	auto const check=[&](int k){
		auto tmp=(int64_t(k)*(k+1))>>1;
		return k>=2 and n>=tmp and (n-tmp)%k==0;
	};
	auto const t=__builtin_ctzll(n);
	auto const k=std::min(int64_t(2)<<t, n>>t);
	if((n>>t)==1){ // n is power of 2
		std::cout<<"-1\n";
	}else{
		assert(k==int(k) and check(int(k)));
		std::cout<<k<<'\n';
	}
}