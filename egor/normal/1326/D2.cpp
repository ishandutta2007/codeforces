...
public:
    void solve() {
        ...
        auto even = evenPalindromes(s);
        int at = -1;
        int len = 2 * same;
        for (int i : range(n)) {
            if (i - even[i] <= same || i + even[i] + same >= n) {
                int cur = 2 * even[i] + 2 * min(i - even[i], n - (i + even[i]));
                maxim(len, cur, at, i);
            }
        }
        string answer;
        if (at != -1) {
            answer = s.substr(0, (len - 2 * even[at]) / 2) + s.substr(at - even[at], 2 * even[at]) +
                     s.substr(n - (len - 2 * even[at]) / 2);
        } else {
            answer = s.substr(0, same) + s.substr(n - same);
        }
        auto odd = oddPalindromes(s);
        at = -1;
        for (int i : range(n)) {
            if (i - odd[i] < same || i + odd[i] + same >= n) {
                int cur = 2 * odd[i] - 1 + 2 * min(i - odd[i] + 1, n - (i + odd[i]));
                maxim(len, cur, at, i);
            }
        }
        if (at != -1) {
            answer = s.substr(0, (len - 2 * odd[at] + 1) / 2) + s.substr(at - odd[at] + 1, 2 * odd[at] - 1) +
                     s.substr(n - (len - 2 * odd[at] + 1) / 2);
        }
