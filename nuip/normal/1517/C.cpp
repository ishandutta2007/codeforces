...
void solve(const vector<ll> &vs){
	const int n=vs.size();
	vv<pii> stks(n);
	rep(i,n)rep(j,i+1) stks[j].eb(i,j);
	vv<int> re(n);
	rep(i,n) re[i].resize(i+1);
	rrep(i,n){
		for(int x=i,y=i,rem=vs[i];rem;--x){
			out(x,y,rem,1);
			assert(x>=0);
			assert(stks[x].size());
			cauto tmp=stks[x].back().X;
			out(tmp,stks[x],1);
			while(stks[x].size() && stks[x].back().X>=y){
				cauto [a,b]=stks[x].back(); stks[x].pop_back();
				re[a][b]=vs[i];
				--rem;
			}
			assert(rem>=0);
	...
	for(cauto &vs:re)rep(i,vs.size()) cout<<vs[i]<<" \n"[i+1==vs.size()];
}
