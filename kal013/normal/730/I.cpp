...
void solve(){
    ...
    For(i,n) c.pb(mp(mp(b[i],a[i]),i+1));
    sort(all(c));
    
    For(i,n){
        for(int j=min(i+1,p);j>=0;j--){
            dp[j][i+1]=dp[j][i];
            if (i+1-j<=s && i+1!=j){
                dp[j][i+1]+=c[n-1-i].F.F;
            }
            if(j>0){
                dp[j][i+1]=max(dp[j][i+1],dp[j-1][i]+c[n-1-i].F.S);
            }
    ...
    for(int i=n;i>0;i--){
        ...
        if (dp[p][i]==dp[p][i-1]) continue;
        if (p>0 && dp[p][i]==dp[p-1][i-1]+c[n-i].F.S){
            A.pb(c[n-i].S);
            --p;
        }
        else if (i==p+s && s>0){
