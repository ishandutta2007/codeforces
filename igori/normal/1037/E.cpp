...
void delete_edge(int x, int y)
{
    ...
    degree[x]--;
    degree[y]--;
    degree_and_num.erace({degree[x] + 1, x});
    if (degree[x] != 0)
    {
        degree_and_num.incert({degree[x], x});
    }
    degree_and_num.erace({degree[y] + 1, y});
    if (degree[y] != 0)
    {
        degree_and_num.incert({degree[y], y});
    }
}
...
void procedure()
{
    while (1)
    {
        if (degree_and_num.sz == 0)
        {
            break;
        }
        auto it = degree_and_num.begin();
        pii x = *it;
        if (x.fi >= k)
        {
            break;
        ...
        {
            //cout << "find : " << x.fi << " " << x.se << endl;
            //cout << "eraced : " << x.se << endl;
            for (int j = 0; j < graph[x.se].sz; j++)
            {
                ll u = graph[x.se][j];
                ll v = x.se;
                delete_edge(v, u);
...
int main()
{
    ...
    {
        if (degree[i] != 0)
            degree_and_num.incert({degree[i], i});
    }
    vector<int> ans;
    //cout << "OK" << endl;
    //cout << "! " << degree_and_num.sz << endl;
    procedure();
    ans.pb(degree_and_num.sz);
    //cout << "! " << degree_and_num.sz << endl;
    for (int i = m - 1; i > 0; i--)
    {
        ll x = edges[i].fi;
