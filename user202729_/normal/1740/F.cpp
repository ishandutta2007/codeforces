// time-limit: 2000
// problem-url: https://codeforces.com/contest/1740/problem/F

// ah right I also forgot *this* part
...
using modular=ModularComparable<998244353>;


int main(){
	std::ios::sync_with_stdio(0);std::cin.tie(0);
	int const count=read<int>();
	
	std::vector<int> countEach(count+1);
	for(int _=count; _--;){
		int const value=read<int>();
		countEach[value]++;
	}

	std::vector<int> limit(count+1); // meaningless names but okay
	for(auto x: countEach) limit[x]++;

	std::partial_sum(limit.rbegin(), limit.rend(), limit.rbegin());
	limit[0]=0;
	std::partial_sum(begin(limit), end(limit), limit.begin());

	std::vector<std::vector<modular>> f(count+1, std::vector<modular>(count+1));

	{
		modular const value=1;
		int const pos=0, last=-count;
		int const index=1;
		auto const right=std::max(0, std::min(limit[index]-pos, pos-last)+1);

		// do not use accumulated table here
		for(int const next_size: range(0, right)){
			f[pos+next_size][pos]+=value;
		}
	}


	std::vector<std::vector<modular>> old(count+1, std::vector<modular>(count+1));
	for(int const index: range(2, count+1)){
		std::swap(old, f);
		//for(auto& x: f) x.assign(count+1, modular {});

		for(int const pos: range(0, count+1)) for(int const last: range(pos-pos/(index-1), pos+1)){
			auto const value=old[pos][last];
			if(value==0) continue;
			auto const right=std::max(0, std::min(limit[index]-pos, pos-last)+1);
			f[pos][pos]+=value;
			if(pos+right!=count+1)
				f[pos+right][pos]-=value;

			old[pos][last]=0;
		}

		for(int const pos: range(0, count+1)) for(int const last: range(pos-pos/index, pos+1)){
			auto const value=f[pos][last];
			if(value==0) continue;
			if(pos+1!=count+1)
				f[pos+1][last]+=value;
		}
	...
	{
		int const pos=count;
		for(int const last: range(0, pos+1)){
			auto const value=f[pos][last];
			result+=value;
		}
	}
	...
	modular result1 {};
	{
		std::map<std::pair<int, int>, modular> f;
		f[{0, -count}]=1;
		for(int const index: range(1, count+1)){
			auto old=std::move(f);
			f.clear();
			for(auto [pos_last, value]: old){
				auto const [pos, last]=pos_last;
				for(int const next_size: range(0, pos-last+1)){
					if(pos+next_size>limit[index]) break;
					f[{pos+next_size, pos}]+=value;
				}
			}
		}

		for(auto [pos_last, value]: f){
			auto const [pos, last]=pos_last;
			if(pos==count) result1+=value;
		}
	}
