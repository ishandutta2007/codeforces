...
void dfs2 (int v) {
    ...
    for (int i=0; i<gr[v].size();i++)
        if (!used[gr[v][i].X]) dfs2(gr[v][i].X);
}

void dfs3(int v, ll L) {
    d[v] = L;
    used[v] = true;
    int c = col[v];
    for (int i=0; i<g[v].size();i++)
    {
        if (col[g[v][i].X]!=c) continue;
        if (used[g[v][i].X])
        {
            mn[c] = gcd(mn[c],L+g[v][i].Y-d[g[v][i].X]);
        }
        if (!used[g[v][i].X]) dfs3(g[v][i].X,L+g[v][i].Y);
    }
}
