...
int main() {
	...
	vector<Mint> fac(n + 1, Mint(1, m)), inv(n + 1, Mint(1, m));
	for (int i = 2; i <= n; i++) fac[i] = fac[i - 1] * i;
	inv[n] = fac[n].inv();
	for (int i = n; i >= 2; i--) inv[i - 1] = inv[i] * i;

	auto C = [&](int n, int r) {
		return fac[n] * inv[r] * inv[n - r];
	};

	vector<vector<Mint>> dp(n + 1, vector<Mint>(n + 1, Mint(0, m)));
	for (int i = 1; i <= n; i++) {
		dp[i][i] = 1;
		for (int j = 1; j <= i - 2; j++) {
			for (int k = 1; k <= j; k++) {
				dp[i][k + i - j - 1] += dp[j][k] * C(k + i - j - 1, k);
			}
		}
	}
