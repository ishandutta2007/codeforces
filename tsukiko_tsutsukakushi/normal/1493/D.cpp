...
void solve() {
    ...
    rep(i, n) {
        for(auto [p, exp]: er.prime_factors(a[i])) {
            int pid = lb(ps, p);
            g[pid].insert(P{exp, i});
            ap[i][pid] = exp;
        }
    }
    mint now = 1;
    rep(i, sz) {
        if(si(g[i]) < n) continue;
        auto ite = g[i].begin();
        now *= mint(ps[i]).pow((*ite).fr);
    }
    debug(now);
    rep(_, q) {
        INT(i, x); -- i;
        for(auto [p, exp]: er.prime_factors(x)) {
            int pid = lb(ps, p);
            int pre = (si(g[pid]) < n ? 0 : (*g[pid].begin()).fr);
            if(ap[i][pid] > 0) {
                auto ite = g[pid].lower_bound(P{ap[i][pid], i});
                g[pid].erase(ite);
            }
            ap[i][pid] += exp;
            g[pid].insert(P{ap[i][pid], i});
            debug(i, p, ap[i][pid]);
            debug(si(g[pid]));
            if(si(g[pid]) == n) {
                auto ite2 = g[pid].begin();
                debug(pre, (*ite2).fr);
                if((*ite2).fr > pre) {
                    now *= mint(p).pow((*ite2).fr - pre);
                }
            }
        }
        out(now);
