...
newe(1000000);
int n=read();
int dq[303][608],sz[303],*dp[303];
int c[303],d[303],cc,dd,tmp2[1000],*tmp=tmp2+500;
int dfs(int u,int fa,int r)
{
	// odp(u,fa);fflush(stdout);
	dp[u][0]=1;sz[u]=1;
	for(int i=-n;i<=n;i++)dp[u][i]=0;
	dp[u][r]=1;
	dp[u][-1]=1;
	fe(u)if(v==fa)continue;dfs(v,u,r);
	int cc=0,dd=0;
	for(int i=-n;i<=n;i++)if(dp[u][i])c[++cc]=i;
	for(int i=-n;i<=n;i++)if(dp[v][i])d[++dd]=i;
	memset(tmp2,0,sizeof(tmp2));
	for(int i=1;i<=cc;i++)
		for(int j=1;j<=dd;j++)
		{
			int a=c[i],b=d[j],p=me(dp[u][a],dp[v][b]);
			if(a<0&&b<0)mae(tmp[min(a,b-1)],p);
			else if(a<0)mae(tmp[a+b<0?a:b-1],p);
			else if(b<0)mae(tmp[a+b<0?b-1:a],p);
			else mae(tmp[max(a,b-1)],p);
		}
	for(int i=-n;i<=n;i++)dp[u][i]=tmp[i];
	gr
	// printf("(%d %d %d)\n",u,fa,r);
	// for(int i=-n;i<=n;i++)odb(dp[u][i]);puts("");
	return 0;
}
int ans[305];
int solve(int x)
{
	dfs(1,1,x);
	int ans=0;
	for(int i=-n;i<0;i++)mae(ans,dp[1][i]);
	// odl(ans);
	return ans;
}
signed main()
{
	...
	// return 0;
	int anss=0;
	rg(n-1)mae(anss,ans[i]=ae(0,solve(i)));gr
	// rg(n-1)odb(ans[i]);gr puts("");
	// for(int i=1;i<=n;i++)mae(ans,solve(i)*i);
	oldl(anss*qp(mod+1>>1,n)%mod);
	return 0;
}