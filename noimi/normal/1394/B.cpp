...
main() {
    ...
    rep(i, n + k) rep(j, K + 1) mat[i][j] = 0;
    rep(i, n) {
        sort(all(g[i]));
        int st = g[i].size() * (g[i].size() - 1) / 2;
        rep(j, g[i].size()) {
            mat[g[i][j].se][st + j] = 1;
            // cout << g[i][j].se << " " << st + j << endl;
        }
    }
    // rep(i, n) {
    //     rep(j, K) cout << mat[i][j];
    //     cout << endl;
    // }
    vll no(K);
    vi cnt(K);
    rep(i, n) {
        vi v;
        rep(j, K) {
            if(mat[i][j]) v.eb(j), cnt[j]++;
        }
        rep(j, v.size()) rep(k, v.size()) if(j != k) no[v[j]] |= 1LL << (ll)v[k];
    }
    vi id;
    int ans = 0;
    auto dfs = [&](auto &&f, vi &id) -> void {
        if(id.size() == k) {
            ll NO = 0;
            ll sum = 0;
            rep(i, k) {
