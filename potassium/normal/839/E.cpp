...
//	mt19937_64 rng64(chrono::steady_clock::now().time_since_epoch().count());
	...
	public:
		...
		Modular& operator /= (const Modular& other)       { val *= other.inv().val; return *this; }
		Modular  operator /  (const Modular& other) const { Modular tmp = *this; return tmp /= other; }
		template<class T> Modular& operator /= (const T& other)       { return *this /= initModular(other); }
		template<class T> Modular  operator /  (const T& other) const { return *this /  initModular(other); }
		template<class T> friend Modular operator / (const T& other, const Modular& m) { return m.initModular(other) / m; }
...
int main() {
	int n,k; R(n,k);
	vector<vector<int>> adj(40, vector<int>(40, 0));
	...
	const int HALF = 1 << 20;
	vector<vector<int>> cliqueL(HALF), cliqueR(HALF);

	auto buildDP = [&](vector<vector<int>>& clique, int shift) {
		for (int mask = 1; mask < HALF; mask++) {
			for (int j = 0; j < 20; j++) {
				if (mask & (1 << j)) {
					int last = mask ^ (1 << j);
					vector<int> now = clique[last];

					if ((int)now.size() == __builtin_popcount(last)) {
						bool can = 1;
						for (auto& k : now) {
							if (!adj[k][j + shift]) {
								can = 0;
						...
						if (can) {
							now.emplace_back(j + shift);
						}

						clique[mask] = move(now);
						break;
					}

					if (now.size() > clique[mask].size())
						clique[mask] = move(now);
				}
			}
		}
	...
	for (int mask = 0; mask < HALF; mask++) {
		if (cliqueR[mask].size() == __builtin_popcount(mask)) {
			int cmask = 0;
			for (int i = 0; i < 20; i++) {
				bool can = 1;
				for (auto& j : cliqueR[mask]) {
					if (!adj[i][j]) {
						can = 0;
						break;
					}
			...
			chkmax(ans, (int)(cliqueL[cmask].size() + cliqueR[mask].size()));
		}
	}

	double val = 0.5 * (1 - 1.0 / ans) * k * k;
	W(val);
