...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            if (cnt.Any(x => x == 1)) {
            ...
            else if (s.Length <= 2 || cnt[fir] == s.Length || cnt[fir] == 1) {
                for (int i = 0; i < 26; i++)
                {
                    for (int j = 0; j < cnt[i]; j++)
                    {
            ...
            else if (cnt[fir] <= s.Length / 2 + 1) {
                var la = new Queue<char>();
                var lb = new Queue<char>();
                {
                    for (int j = 0; j < cnt[fir] - 1; j++)
                    {
                        la.Enqueue((char)(fir + 'a'));
                    }
                }
                ...
                sb.Append((char)(fir + 'a'));
                // Assert(la.Count <= lb.Count + 1);
                while (la.Any() || lb.Any()) {
                    if (la.Any()) {
                        sb.Append(la.Dequeue());
                    }
                    if (lb.Any()) {
                        sb.Append(lb.Dequeue());
                    }
                }
            }
            else {
                ...
                int uq = cnt.Count(x => x > 0);
                if (uq > 2) {
                    var la = new Queue<char>();
                    var lb = new Queue<char>();
                    var lc = new Queue<char>();
                    {
                        for (int j = 0; j < cnt[fir]; j++)
                        {
                            la.Enqueue((char)(fir + 'a'));
                        }
                        for (int j = 0; j < cnt[sec]; j++)
                        {
                            lb.Enqueue((char)(sec + 'a'));
                        }
                    }
