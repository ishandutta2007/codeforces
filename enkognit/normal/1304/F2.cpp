...
struct segment_tree
{
    ...
    {
        if (l==r) {d[h]=dp[nm][l];leaf[h]=1;return;}
        int w=(l+r)/2;
        build(h*2,l,w);
        build(h*2+1,w+1,r);
    ...
    void update(int h,int l,int r,int x,int y,int k)
    {
        push(h);
        if (x>y) return;
        if (l==x && y==r) {d[h]+=k;tt[h]+=k;push(h);return;}
        int w=(l+r)/2;
...
int main()
{
    ...
    for (ll i = 1; i <= n; i++)
    {
        ...
        if (i>1) for (ll j = 1; j <= k; j++) g[i].update(1,1,m-k+1,1,min(j,m-k+1),-a[i][j]);

        for (ll j = 1; j <= m-k+1; j++)
        {
            dp[i+1][j]=pr[i][min(j+k-1,m)]-pr[i][j-1]+pr[i+1][min(j+k-1,m)]-pr[i+1][j-1]+g[i].get(1,1,m-k+1,1,m-k+1);
            if (i>1)
            {
                g[i].update(1,1,m-k+1,max(j-k+1,1ll),j,a[i][j]);
                g[i].update(1,1,m-k+1,j+1,min(j+k,m-k+1),-a[i][j+k]);
            }
            //cout << dp[i+1][j] << " ";
        }
        //cout << "\n";
