...
void solve() {
  ...
  FOR(x, H) FOR(y, W) FOR(d, 4) {
    ll nx = x + dx[d], ny = y + dy[d];
    if (!isin(nx, ny)) continue;
    char a = A[x][y], b = A[nx][ny];
    if (a == 'D' && b == 'I') G.add(W * x + y, W * nx + ny);
    if (a == 'I' && b == 'M') G.add(W * x + y, W * nx + ny);
    if (a == 'M' && b == 'A') G.add(W * x + y, W * nx + ny);
    if (a == 'A' && b == 'D') G.add(W * x + y, W * nx + ny);
  }

  vi indeg(H * W);
  FORIN(e, G.edges) indeg[e.to]++;
  vi que;
  FOR(v, H * W) if (indeg[v] == 0) que.eb(v);
  ...
  while (len(que)) {
    ...
    FORIN(e, G[v]) {
      ...
      indeg[e.to]--;
      if (indeg[e.to] == 0) que.eb(e.to);
    }
  }

  FOR(v, H * W) if (indeg[v]) return print("Poor Inna!");

  ll ANS = 0;
  FOR(x, H) FOR(y, W) if (A[x][y] == 'A') {
    chmax(ANS, (DP[W * x + y] + 1) / 4);
  }
  if (ANS == 0)
    print("Poor Dima!");
  else
