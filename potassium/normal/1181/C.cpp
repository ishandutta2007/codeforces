...
bool isAll(int r1, int r2, int c1, int c2) {
	if (isValid(r1, 1) && isValid(r2, 1) && isValid(c1, 0) && isValid(c2, 0)) {
		int val = table[r1][c1] - 'a';
		int sum = cnt[val][r2][c2];
		if (r1 > 0)
			sum -= cnt[val][r1-1][c2];
		if (c1 > 0)
			sum -= cnt[val][r2][c1-1];
		if ((r1 > 0) && (c1 > 0))
			sum += cnt[val][r1-1][c1-1];
...
int main() {
	...
	cnt.resize(26, vector<vector<int>>(n, vector<int>(m, 0)));
	table.resize(n);
	for (int i = 0; i < n; i++) {
		R(table[i]);
		for (int j = 0; j < m; j++)
			cnt[table[i][j] - 'a'][i][j] = 1;
	}
	...
	vector<vector<bool>> isCalc(n, vector<bool>(m, 0));
	ll ans = 0;
...
//			W(i, j, table[j][i]);
			if (j == n-1 || table[j][i] != table[j + 1][i]) {
				...
				last = j;
				if (isCalc[L][i]) continue;
				
				for (int c = i; c < m; c++) {
//					W(i, j, c, L, R, range);
					if (isAll(L, R, i, c) && isAll(L + range, R + range, i, c) && isAll(L - range, R - range, i, c)) {
						isCalc[L][c] = 1;
						ans += c - i + 1;
//						W(ans, "added", c - i + 1);
					} else break;
