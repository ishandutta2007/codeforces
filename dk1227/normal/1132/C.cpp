...
int main() {
	qread(n, q);
	rep(i, 1, q) qread(l[i], r[i]), ++dif[l[i]], --dif[r[i] + 1];
	rep(i, 1, n) dif[i] += dif[i - 1];
	rep(i, 1, n) rep(j, 1, 2) pre[i][j] = pre[i - 1][j] + (dif[i] == j);
	rep(i, 1, n) ans += (dif[i] != 0); 
	rep(i, 1, q) rep(j, i + 1, q) {
		int now = ans;
		now -= (pre[r[i]][1] - pre[l[i] - 1][1]);
		now -= (pre[r[j]][1] - pre[l[j] - 1][1]);
		if(!((r[i] < l[j]) || (r[j] < l[i]))) {
			int L = max(l[i], l[j]), R = min(r[i], r[j]);
			now -= (pre[R][2] - pre[L - 1][2]);	
		}
		To_max(sum, now);
	}
	printf("%d\n", sum);
