...
int main() {
  int n = IN;
  auto a = IN.v<bitset<60>>(n);

  vector<vector<int>> graph(n);
  rep(i, n) rep(j, n) if (i != j and (a[i] & ~a[j]).none()) graph[i].push_back(j);

  VB dead(n);
  bool found;
  do {
    found = false;
    rep(i, n) if (not dead[i])  {
      while (not graph[i].empty() and dead[graph[i].back()]) graph[i].pop_back();
      if (graph[i].empty()) dead[i] = found = true;
    }
  } while (found);


  if (accumulate(all(dead), 0) > n-2) EXIT(0);
  auto b = IN.vll(n);
  ll res = 0;
  rep(i, n) if (not dead[i]) res += b[i];
  OUT(res);
}