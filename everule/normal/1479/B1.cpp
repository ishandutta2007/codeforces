...
void solve(){
    ...
    st.updaterange(0,0,+INF);
    for(int i=1;i<n;i++){
        int l1 = st.query(0,seq[i]-1) + 1;
        int l2 = st.query(seq[i] + 1, n) + 1;
        int lm = st.query(seq[i], seq[i]);
        if(max({l1,l2,lm}) > st.query(seq[i-1], seq[i-1]) + (seq[i] != seq[i-1])){
            st.updaterange(seq[i-1], seq[i-1], max({l1,l2,lm}) - st.query(seq[i-1], seq[i-1]) - (seq[i] != seq[i-1]));
        }
        st.updaterange(0,n,(seq[i] != seq[i-1]));
    }
    int ans2 = st.query(0,n) + 1;
    cout<<max(ans1, ans2)<<"\n";
}
int main() {
