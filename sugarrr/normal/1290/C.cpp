...
public:
    ...
    ll merge(ll x, ll y) {
        ...
        if (siz[x].fi+siz[x].se < siz[y].fi+siz[y].se) swap(x, y);
        siz[x].fi += siz[y].fi;
        siz[x].se+=siz[y].se;
        par[y] = x;
...
signed main(){fastio
    ...
    UnionFind uf;
    {
        ...
        rep(i,0,n-1){
            if((int)v[i].size() ==2){
                if(s[i]=='0'){
                    d.merge(v[i][0],v[i][1]+k);
                    d.merge(v[i][1],v[i][0]+k);
                }else{
                    d.merge(v[i][0],v[i][1]);
                    d.merge(v[i][0]+k,v[i][1]+k);
                }
        ...
        rep(i,0,k-1){
            if(!d.same(S,i) && !d.same(S,i+k)){
                d.merge(S,i);
            }
        }
        vl a(k);
        rep(i,0,k-1)a[i]=d.same(S,i);
      //  DEB(a);
        uf.init(a);
    }
    ...
    ll ans=0;
    rep(i,0,n-1){
        while((int(v[i].size())<=1))v[i].pb(k);
        
        ans+=uf.merge(v[i][0],v[i][1]);
        cout<<ans<<endl;
    }
