// Problem: C. Familiar Operations
// Contest: Codeforces - Z Flip #2
// URL: https://codeforces.com/gym/374761/problem/C
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
std::map<std::vector<int>,int>dp[300];
void fix(std::vector<int>&v)
{
	while(v.size()<20)v.push_back(1);
	std::sort(v.begin(),v.end());
	std::reverse(v.begin(),v.end());
}
int sum(std::vector<int>&v)
{
	int p=0;for(auto g:v)p+=g;return p;
}
int mul(std::vector<int>&v)
{
	int p=1;for(auto g:v)p*=g;return p;
}
std::map<std::vector<int>,int>mp;
std::vector<int>g[800000];
int id[1800000];
int dist[999][999],cc;
// std::set<int>test;
int Dist[300][10028];
...
void solve(int s)
{
	...
	while(!q.empty())
	{
		vi u=q.front();q.pop();int D=d[u];
		if(mul(u)>1024)continue;
		mne(Dist[s][mul(u)],D);
		// test.insert(mul(u));
		// oldl(mul(u));
		mxe(SSs,mul(u));
		if(D>=10)break;
		// odb(D);odb(mul(u));putchar(' ');for(auto p:u)odb(p);puts("");
		// if(!res.count(mul(u)))res[mul(u)]=D;
		if(sum(u)<=43)
		{
			for(int j=0;j<20;j++)
			{
				if(j&&u[j]==u[j-1])continue;
				vi v=u;v[j]++;
				fix(v);
				if(!d.count(v))d[v]=D+1,q.push(v);
				if(u[j]==1)break;
			}
		}
		for(int j=0;j<20;j++)
...
signed main()
{
	...
	if(s>1)v.push_back(2);
	fix(v);
	if(!mp.count(v))mp[v]=++cc,
	g[cc]=v;
	// if(i==512000||i==810000){odb(i);for(auto p:v)odb(p);puts("");}
	id[i]=mp[v];
	gr
	// odl(cc);
	// return 0;
	...
	rg(cc)
	solve(i);
	// odl(Dist[id[810000]][52]);
	gr
	
	// odl(id[810000]);
	// odl(SSs);
	rg(cc)rg_(j,cc)
	rg_(k,SSs)mne(dist[i][j],Dist[i][k]+Dist[j][k]);gr
	gr gr
	
	// rg_(k,1024)if(Dist[id[512000]][k]+Dist[id[810000]][k]==dist[id[512000]][id[810000]])odl(k);gr
	// odl(Dist[id[420]][12]);
	// odl(Dist[id[512]][12]);
	// odl(test.size());
	int T=read();
	rg(T)
	int u=read(),v=read();
	odl(min(dist[id[u]][id[v]],dist[id[v]][id[u]]));
	gr
	return 0;
}