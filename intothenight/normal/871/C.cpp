...
int main(){
	...
	map<int, vector<array<int, 2>>> indx, indy;
	for(auto i = 0; i < n; ++ i){
		int x, y;
		cin >> x >> y;
		indx[x].push_back({y, i}), indy[y].push_back({x, i});
		a[i] = {x, y};
	}
	vector<vector<int>> adj(n);
	for(auto [ignore, a]: indx){
		sort(a.begin(), a.end());
		for(auto i = 1; i < (int)a.size(); ++ i){
			int u = a[i - 1][1], v = a[i][1];
			adj[u].push_back(v), adj[v].push_back(u);
		}
	}
	for(auto [ignore, a]: indy){
		sort(a.begin(), a.end());
		for(auto i = 1; i < (int)a.size(); ++ i){
			int u = a[i - 1][1], v = a[i][1];
			adj[u].push_back(v), adj[v].push_back(u);
	...
	for(auto u = 0; u < n; ++ u){
		if(!vis[u]){
			...
			dfs(u);
			sort(ox.begin(), ox.end()), ox.erase(unique(ox.begin(), ox.end()), ox.end());
			sort(oy.begin(), oy.end()), oy.erase(unique(oy.begin(), oy.end()), oy.end());
			int tot = (int)ox.size() + (int)oy.size();
			Zp cur = 0;
			for(auto x = 0; x <= min(cnt, tot); ++ x){
				cur += C.C(tot, x);
			}
			res *= cur;
		}
