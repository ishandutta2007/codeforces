...
void solve()
{
	...
	auto value_merge = [](const ValueType& a, const ValueType& b) -> ValueType
	{
		...
		for (int i = 0; i < 2; ++i)
			for (int j = 0; j < 2; ++j)
			{
				c.val[i][j] = INF;
				for (int k = 0; k < 2; ++k)
					freshmin(c.val[i][j], b.val[i][k] + a.val[k][j]);
			}
		return c;
	};
	...
	vector<ValueType> init(n - 1);
	for (int i = 1; i < n - 1; ++i)
	{
		// (i, vt[i]), (vr[i], i)
		init[i].val[0][0] = dis(i + 1, vt[i], i + 1, vt[i + 1]) + 1;
		init[i].val[0][1] = dis(vr[i], i + 1, i + 1, vt[i + 1]) + 1;
		init[i].val[1][0] = dis(i + 1, vt[i], vr[i + 1], i + 1) + 1;
		init[i].val[1][1] = dis(vr[i], i + 1, vr[i + 1], i + 1) + 1;
	}
	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		tree(init, value_init, value_merge, tag_init, tag_merge, tag_spread);
	...
	while (q--)
	{
		...
		if (z1 == z2)
		{
			writeln(dis(x1, y1, x2, y2));
			continue;
		}
		auto a = tree.Query(z1, z2 - 1);
		array<int, 2> d1 = { dis(x1, y1, z1, vt[z1]), dis(x1, y1, vr[z1], z1) };
		array<int, 2> d2 = { dis(x2, y2, z2, vt[z2 - 1]) + 1, dis(x2, y2, vr[z2 - 1], z2) + 1 };

		ll res = INF;
		for (int i = 0; i < 2; ++i)
			for (int j = 0; j < 2; ++j)
			{
				freshmin(res, d1[i] + a.val[j][i] + d2[j]);
			}
		writeln(res);
	}
}
