...
int group(int idx) {
	if(parent[idx] == -1)
		return idx;
	return group(parent[idx]);
}

int main() {
	in >> n >> m;
	memset(leader, -1, m * sizeof(int));
	...
	for(int i = 0; i < n; i++) {
		int g1 = group(i);
		for(unsigned int j = 0; j < langs[i].size(); j++) {
			if(leader[langs[i][j]] != -1) {
				int g2 = group(leader[langs[i][j]]);
				if(g1 != g2) {
					parent[g2] = g1;
				}
			}
