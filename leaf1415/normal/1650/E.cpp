...
bool check2(ll id, ll x)
{
	...
	rep(i, 1, sz(vec)-1) if(vec[i]-vec[i-1]-1 < x) return false;
	rep(i, 1, sz(vec)-1) if(vec[i]-vec[i-1] >= 2*x+2) return true;
	if(vec.back() + x+1 <= d) return true;
	return false;
}
...
bool check(ll x)
{
	rep(i, 1, n){
		if(a[i]-a[i-1]-1 < x){
			if(check2(i, x)) return true;
			if(i-1 >= 1 && check2(i-1, x)) return true;
			return false;
		}
	}
	return true;
