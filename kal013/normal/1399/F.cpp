...
// 			if (i == j){
// 				optl[i][j] = i ;
// 				dp[i][j] = (is_seg[i][j]) ? 1:0;
// 				continue;
// 			}
// 			pair<int,int> ansl = {-1,-1}, ansr = {-1, -1};
// 			for(int k = optl[i][j - 1] - 1 ; k <= min(optl[i + 1][j] + 1 , j - 1) ; ++k){
// 				ansl = max(ansl, {dp[i][k] + dp[k + 1][j], -k});
// 				// ansr = max(ansr, {dp[i][k] + dp[k + 1][j], k});
// 			}
// 			optl[i][j] = -ansl.second;
// 			// optr[i][j] = ansr.second;
// 			dp[i][j] = max({ansl.first,dp[i][j - 1], dp[i - 1][j]}) + (is_seg[i][j] ? 1: 0);
// 		}

// 	}	
// 	// trace(optl[7][9], optr[7][9]);
// 	// trace(is_seg[2][8], opt[2][8], opt[2][7], opt[3][8],opt[6][8], opt[5][8], opt[4][8], opt[5][7]);
// 	// trace(dp[3][4], dp[2][5],dp[6][8], dp[2][8]);
// 	cout << dp[1][n] << endl;
