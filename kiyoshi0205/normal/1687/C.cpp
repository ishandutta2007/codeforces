...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  set<ll> se;
  rep(i,N){
    if(a[i]==b[i])que.emplace(i);
    else se.emplace(i);
  }
  mat g(N);
  for(auto&[l,r]:lr){
    g[--l].emplace_back(r);
    if(r<N)g[r].emplace_back(l);
    if(se.count(l)+se.count(r)==0){
  ...
  while(!que.empty()){
    ...
    for(auto y:g[x]){
      ...
      auto itr=se.lower_bound(min(x,y));
      while(itr!=se.end()&&*itr<max(x,y)){
        que.emplace(*itr);
        itr=se.erase(itr);
      }
    }
  }
  if(se.empty())printYES;
  else printNO;
}}