...
fun main(){
    solve.cases{
        val n = getint()
        val Cs = Array(3){getline(n)}
        val G = Graph(n,false)
        repeat(n-1){
            G.add(getint() - 1 , getint() - 1 )
        }
        val bad = (0 until n).any{G.E[it].size >= 3}
        if(bad){
            put(-1)
            return@cases
        }
        ...
        val ret = LongArray(6)
        val ctype = Array(6){IntArray(n)}
        var order = 0
        G.DFS(root) {
            order ++
            for(i in 0 until 3){
                ret[i] += (Cs[(i+order) %3][it]).toLong()
                ret[3+i] += (Cs[(i+3 * n - order) %3][it]).toLong()
                ctype[i][it] = (i+order) %3
                ctype[3+i][it] = (i+3 * n - order) %3
            }
        }
        val best = ret.minOrNull()!!
        put(best)
        val besttype = ret.indexOfFirst { it == best }
        for(i in 0 until n){
            put(ctype[besttype][i] + 1)
        }
