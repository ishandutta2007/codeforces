...
int main() {
    ...
    auto do_it = [&](int i, int j, int k) {
        int v = a[i] ^ a[j] ^ a[k];
        ops.push_back({ i+1, j+1, k+1 });
        a[i] = a[j] = a[k] = v;
        return v;
    };

    auto solve = [&](auto &self, vi locs) -> void {
        if (sz(locs) < 3) {
            FOR (i, 1, sz(locs))
                assert(a[locs[i]] == a[locs[0]]);
            return;
        }
        ...
        int i = 0;
        for (; i + 3 <= sz(locs); i += 3) {
            do_it(locs[i], locs[i+1], locs[i+2]);
            nlocs.push_back(locs[i]);
        }
        for (; i < sz(locs); i++) {
            nlocs.pb(locs[i]);
        }
