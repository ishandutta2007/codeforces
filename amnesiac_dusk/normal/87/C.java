...
public class Main
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		Set<Integer>[]hola=new HashSet[100001];
		for(int i=0; i<=100000; i++)
			hola[i]=new HashSet<Integer>();
		int n=ni();
		int ans=100000000;
		for(int i=2; i<=100000; i++)
		{
			int g=0;
			//grundy's number is mex of set of grundy numbers of children
			while(hola[i].contains(g))
				g++;
			grundy[i]=g;
			int xorian=grundy[i],sum=i;
			for(int j=i-1; j>=1; j--)
			{
				sum+=j;
				xorian^=grundy[j];
