...
public class F {
	...
	public void solve() {
		...
		TreeMap<Integer, Integer> goodUnlock = new TreeMap<>();
		s[0] = "blue";
		...
		for (int i = n; i >= 0; i--) {
			if (s[i].equals("lock")) {
				time.put(i, cur);
				cur = 0;
			} else if (s[i].equals("unlock")) {
				time.put(i, cur);
				if (cur > 0) {
					goodUnlock.put(i, cur);
				}
				cur = 0;
			} else {
				cur++;
		...
		time.put(-1, cur);
		goodUnlock.put(-1, cur);
		out.println(s[goodUnlock.lastKey() + goodUnlock.get(goodUnlock.lastKey())]);
		int t = in.nextInt();
		for (int i = 0; i < t; i++) {
			int x = in.nextInt();
			String str = in.next();
			if (str.equals("lock")) {
				goodUnlock.remove(x);
				if (!s[x].equals("unlock") && !s[x].equals("lock")) {
					int prev = time.lowerKey(x);
					int value = time.get(prev);
					time.put(prev, x - prev - 1);
					if (goodUnlock.containsKey(prev)) {
						if (x - prev - 1 > 0) {
							goodUnlock.put(prev, x - 1 - prev);
						} else {
							goodUnlock.remove(prev);
						}
					}
					time.put(x, value - x + prev);
				}
				...
				if (!s[x].equals("unlock") && !s[x].equals("lock")) {
					...
					time.put(x, value - x + prev);
					if (value - x + prev > 0) {
						goodUnlock.put(x, value - x + prev);
					}
				} else if (s[x].equals("lock")) {
					if (time.get(x) > 0) {
						goodUnlock.put(x, time.get(x));
					}
				}
			} else {
				if (time.containsKey(x)) {
					int value = time.get(x);
					goodUnlock.remove(x);
					time.remove(x);
					int prev = time.lowerKey(x);
					time.put(prev, time.get(prev) + value + 1);
					if (prev < 0 || s[prev].equals("unlock")) {
						goodUnlock.put(prev, time.get(prev));
					}
				}
			}
			s[x] = str;
