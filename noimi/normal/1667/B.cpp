...
int main() {
    TEST {
        ...
        auto x = R;
        UNIQUE(x);
        RMT<int> seg(si(x)), seg2(si(x));
        seg.set(lb(x, 0), 0), seg2.set(lb(x, 0), 0);
        int pre = 0;
        rep(i, 1, n + 1) {
            int k = lb(x, R[i]);
            pre = max<int>({pre - 1, seg2[k], seg.prod(0, k) + (int)i});
            seg2.set(k, max(seg2[k], pre));
            seg.set(k, max(seg[k], pre - (int)i));
        }
        OUT(pre);
    }
}