...
int main() {
    ...
    sort(all(eg));
    eg=vector<tuple<i64,int,int>>(eg.begin(),eg.begin()+min(k,m));
    // cerr<<eg<<endl;
    fill(is,is+n,-1);
    init(dist,inf<i64>);
    for(auto &e:eg) {
        int u,v; i64 w; tie(w,u,v)=e;
        if(0>is[u]) {
            is[u]=vcnt++;
            // vs.emplace_back(u);
        }
        ...
        // e=tie(w,is[u],is[v]);
        dist[is[u]][is[v]]=dist[is[v]][is[u]]=w;
    }
    for(int k=0; k<vcnt; ++k) {
        for(int x=0; x<vcnt; ++x) {
            for(int y=0; y<vcnt; ++y) {
                chmin(dist[x][y],dist[x][k]+dist[k][y]);
            }
        }
    }
    // dump(vcnt);
    vector<i64> lens;
    for(int x=1; x<vcnt; ++x) {
        for(int y=0; y<x; ++y) {
            lens.emplace_back(dist[x][y]);
        }
    }
    sort(all(lens));
    assert(lens.size()>=k);
