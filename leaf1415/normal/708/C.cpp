...
void dfs(int v, int p)
{
	...
	sum.first++;
	if(sum.first*2 <= n) chmax(sum.second, sum.first);
	dp[v] = sum; //
}

P lsum[500005], rsum[500005];
void dfs2(int v, int p)
{
	...
	for(int i = 0; i < m; i++){
		if(G[v][i] == p) continue;
		P tmp = ope(lsum[i], rsum[i+2]);
		tmp.first++;
		if(tmp.first*2 <= n) chmax(tmp.second, tmp.first);
		dp2[G[v][i]] = tmp;
	}
...
int main(void)
{
	...
	rep(i, 1, n){
		...
		if(mx*2 <= n || 2*(mx - get(i, mx_u).second) <= n) cout << 1 << " ";
		else cout << 0 << " ";
	}
	cout << endl;
