...
//ll dp[201][210*501];
int dp[201][2001][101];
bool used[201][2001][101];
int a[10101];
int go(int cur, int bal, int z) {
//    DBN(cur, bal, z);
    if (z > cur) return 0;
    if (cur == 0) return bal == 0 && z == 0;
    int x = a[cur - 1];
    bal += x * z;
    if (bal < 0) return 0;
    if (used[cur][bal][z]) return dp[cur][bal][z];
    used[cur][bal][z] = true;
    ll res =
    go(cur - 1, bal - x * z - x, z + 1) +
    (z + 1LL) * go(cur - 1, bal - x * z, z) +
    (z == 0 ? 0 : z * 1ll * go(cur - 1, bal - x * z + x, z - 1));
    
    
    res %= mod;
    dp[cur][bal][z] = (int)res;
    return (int)res;
}
int main() {
    int n, k;
...
////    }
//    if (!e) {
//        cout << besti + besti - 1 << endl;
//        for (int i = 0; i < besti; ++i) {
//            cout << a[i] << ' ';
//        }
//        for (int i = n - besti + 1; i < n; ++i) {
//            cout << a[i] << ' ';
//        }
//        cout << endl;
//    } else {
