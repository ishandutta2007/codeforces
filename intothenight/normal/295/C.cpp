...
int main(){
	...
	combinatorics<modular> C(1000);
	vector dp(cnt[1] + 1, vector(cnt[2] + 1, vector(cnt[1] + 1, vector(cnt[2] + 1, array<pair<int, modular>, 2>{{{-1, 0}, {-1, 0}}}))));
	dp[0][0][cnt[1]][cnt[2]][1] = {0, 1};
	deque<array<int, 5>> dq{{0, 0, cnt[1], cnt[2], 1}};
	while(!dq.empty()){
		auto [l1, l2, r1, r2, side] = dq.front();
		dq.pop_front();
		auto &cur = dp[l1][l2][r1][r2][side];
		if(side){
			for(auto m1 = 0; m1 <= r1; ++ m1){
				for(auto m2 = (int)!m1; m2 <= r2 && m1 + m2 * 2 <= k; ++ m2){
					auto &prev = dp[l1 + m1][l2 + m2][r1 - m1][r2 - m2][!side];
					if(!~prev.first){
						prev = {cur.first + 1, cur.second * C.C(r1, m1) * C.C(r2, m2)};
						dq.push_back({l1 + m1, l2 + m2, r1 - m1, r2 - m2});
					}
					else if(prev.first == cur.first + 1){
						prev.second += cur.second * C.C(r1, m1) * C.C(r2, m2);
					}
				}
			}
		}
			...
			for(auto m1 = 0; m1 <= l1; ++ m1){
				for(auto m2 = (int)!m1; m2 <= l2 && m1 + m2 * 2 <= k; ++ m2){
					auto &prev = dp[l1 - m1][l2 - m2][r1 + m1][r2 + m2][!side];
					if(!~prev.first){
						prev = {cur.first + 1, cur.second * C.C(l1, m1) * C.C(l2, m2)};
						dq.push_back({l1 - m1, l2 - m2, r1 + m1, r2 + m2, !side});
					}
					else if(prev.first == cur.first + 1){
						prev.second += cur.second * C.C(l1, m1) * C.C(l2, m2);
					}
				}
			}
		}
	}
	auto res = dp[cnt[1]][cnt[2]][0][0][0];
	cout << res.first << "\n" << res.second << "\n";
	return 0;
}
