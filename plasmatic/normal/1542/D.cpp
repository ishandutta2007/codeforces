...
char op[MN];
pii A[MN];
MI dp[MN][MN]; // dp[index][# elements <= x], collect from dp[N][>0]
...
int main() {
    ...
    for (auto i = 1; i <= N; i++) {
        ...
        if (op[i] == '+') {
            cin >> A[i].fs;
            A[i].sn = i; // enforce strict ordering
        }
    }
            ...
            for (auto j = 1; j <= N; j++) {
                ...
                if (op[j] == '+') {
                    if (A[j] < A[i])
                        for (auto k = 1; k <= N; k++)
                            for (auto l = 0; l < 2; l++)
                                dp[j][k][l] += dp[j-1][k-1][l];
                    else if (A[j] == A[i])
                        for (auto k = 1; k <= N; k++)
                            for (auto l = 0; l < 2; l++)
                                dp[j][k][1] += dp[j-1][k-1][l];
                    else
                        for (auto k = 0; k <= N; k++)
                            for (auto l = 0; l < 2; l++)
                                dp[j][k][l] += dp[j-1][k][l];
                    ...
                    for (auto k = 0; k < 2; k++)
                        dp[j][0][0] += dp[j-1][1][k];
                    dp[j][0][0] += dp[j-1][0][0]; // if empty
                }

                // cout << "dp[j]=["; for (auto __x:dp[j])cout<<__x<<", "; cout<<"], "; cout << endl; // db I:dp[j]
            }

            // cout<<"i="<<(i)<<", "; cout<<"A[i].fs="<<(A[i].fs)<<", "; cout<<"A[i].sn="<<(A[i].sn)<<", "; cout << endl; // db i,A[i].fs,A[i].sn
            MI cur = 0;
            for (auto j = 1; j <= N; j++) cur += dp[N][j][1];
            // cout<<"cur="<<(cur)<<", "; cout << endl; // db cur
