// Hydro submission #61271aa6b189b900f590357d@1629952679020
...
namespace Solution {
	...
	iv Main() {
		MT {
			...
			if(a[1]) {
				write(n + 1);
				F(int, i, 1, n) space_print(i);
				puts(""); continue;
			}
			int st = n;
			F(int, i, 1, n) if(a[i]) {st = i - 1; break;}
			if(st == n) {
				F(int, i, 1, n + 1) print_space(i);
				puts(""); continue; 
			}
			F(int, i, 1, st) printf("%d ", i);
			printf("%d ", n + 1);
			F(int, i, st + 1, n) printf("%d ", i);
			puts("");
		}
		return;
	}
