...
bool gao(){
	...
	while(!q.empty()){
		...
		q.pop();
		inq[x] = false;
		if (push(x+2, dp[x]+1)) return true;
		if (push(x-2, dp[x]+1)) return true;
		if (push((x+2*k)^1, dp[x]+1)) return true;
	}
	return false;
}

int main(){
	for (int i=0; i<N*2; ++i) dp[i] = i / 2 + 1;
	cin >> n >> k >> a >> b;
	for (int i=0; i<n; ++i) ch[i*2] = a[i];
	for (int i=0; i<n; ++i) ch[i*2+1] = b[i];
	if (gao()) puts("YES");
	else puts("NO");
	return 0;
}