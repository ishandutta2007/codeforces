...
void solve() {
  ...
  ll K = len(X);
  vi W(K);
  FOR(i, K) W[i] = (i == 0 ? X[0] : X[i] - X[i - 1]);

  using Lazy = Lazy_CntSum_Affine<mint>;
  using P = pair<mint, mint>;
  vc<P> seg_raw(K);
  FOR(k, K) seg_raw[k] = {W[k], 0};
  LazySegTree<Lazy> seg(seg_raw);
  FOR(i, N) {
    ll a = LB(X, A[i]);
    if (i == 0) {
      FOR(j, a + 1) seg.set(j, {W[j], W[j]});
    } else {
      mint total = seg.prod_all().se;
      seg.apply(0, a + 1, {-1, total});
      seg.apply(a + 1, K, {0, 0});
    }
  }
