#line 1 "codeforces-workspace\\contest\\1572\\a\\a.cpp"
...
#line 13 "codeforces-workspace\\contest\\1572\\a\\a.cpp"
...
#line 29 "codeforces-workspace\\contest\\1572\\a\\a.cpp"
...
void main() {
  ...
  input n;
  vector<int> waits(n);
  vector<vector<int>> aft(n);

  heap<i32, greater<>> now;
  ...
  for (auto i : range(n)) {
    ...
    while (k--) {
      input a;
      --a;
      aft[a].emplace_back(i);
    }
  }

  for (auto t : range(n + 1)) {
    if (now.empty()) {
      if (count(begin(waits), end(waits), 0) == waits.size()) {
        eject(t);
      }
      eject(-1);
    }
