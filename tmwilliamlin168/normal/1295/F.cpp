...
void solve() {
	...
	vt<int> l2(n), r2(n);
	FOR(n) {
		l2[i]=lower_bound(all(v), l[i])-v.begin();
		r2[i]=lower_bound(all(v), r[i]+1)-v.begin();
	}
	int m=sz(v)-1;
	vt<vt<Mnt<M>>> dp;
	vti(dp, 0, n, 2*n);
	...
	FOR(i, 1, n) {
		FOR(j, 1, m) {
			for(int k=i-1; ~k&&l2[k]<=j-1&&j-1<r2[k]; --k)
				dp[i][j]+=dp[k][j-1]*c.nck2(i-k+v[j]-v[j-1]-1, i-k);
			dp[i][j]+=dp[i][j-1];
		}
	}
	Mnt<M> ans=0;
	FOR(j, l2[n-1], r2[n-1])
		for(int k=n-1; ~k&&l2[k]<=j&&j<r2[k]; --k)
			ans+=dp[k][j]*c.nck2(n-k+v[j+1]-v[j]-1, n-k);
	FOR(n)
		ans/=r[i]-l[i]+1;
	print(ans);
}
