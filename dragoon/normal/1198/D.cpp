...
int IsThere(int i1, int j1, int i2, int j2) {
	return black[i2][j2] - black[i1 - 1][j2]
		- black[i2][j1 - 1] + black[i1 - 1][j1 - 1];
}
			...
			for (int i = 1; i + w - 1 <= n; i++) {
				for (int j = 1; j + h - 1 <= n; j++) {
					...
					for (int k = 1; k < w; k++) {
						dp[i][j][w][h] = MIN(
							dp[i][j][w][h],
							dp[i][j][k][h] + dp[i + k][j][w - k][h]
						);
					}
					for (int k = 1; k < h; k++) {
						dp[i][j][w][h] = MIN(
							dp[i][j][w][h],
							dp[i][j][w][k] + dp[i][j + k][w][h - k]
						);
					}
				}
			}
