...
vector< int > get(int l, int r){
	...
	int m = (l + r) / 2;
	return fft::multiply_mod(get(l, m), get(m + 1, r), mod);
}
int main()
{
	scanf("%d%d", &n, &k);
    ...
    res[0] = get(1, w[0] - 1);
	for(int i = 1; i < k; i++)
	    res[i] = fft::multiply_mod(res[i - 1], get(w[i - 1], w[i] - 1), mod);
	for(int i = 0; i < k; i++)
		for(int j = 0; j < res[i].size() && w[i] + 1 + j < N; j++)	
			ans[w[i] + 1 + j] = (ans[w[i] + 1 + j] + res[i][j]) % mod;		
