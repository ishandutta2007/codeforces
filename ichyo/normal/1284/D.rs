...
fn check(segs: &[((i64, i64), (i64, i64))]) -> bool {
    // 0 - enter
    // 1 - remove
    let n = segs.len();
    ...
    for (_, ty, idx) in events {
        let ((_, _), (a, b)) = segs[idx];
        if ty == 0 {
            lefts.insert((a, idx));
            rights.insert((b, idx));
            if lefts.len() > 0 {
                let lmax = lefts.iter().rev().next().unwrap();
                let rmin = rights.iter().next().unwrap();
                if rmin.0 < lmax.0 {
                    return true;
                }
            }
...
fn run() {
    ...
    let rev: Vec<_> = segments.iter().cloned().map(|(x, y)| (y, x)).collect();
    if check(&segments) || check(&rev) {
        println!("NO");
    } else {
        println!("YES");
    }
