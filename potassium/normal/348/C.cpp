...
int main() {
	...
	vector<int> heavyIdx;
	vector<ll> heavyAns;
	vector<vector<int>> isect;

	for (int i = 0; i < m; i++) {
		if ((int)s[i].size() >= SQRT) {
			heavyIdx.emplace_back(i);

			heavyAns.emplace_back(0);
			for (auto& e : s[i])
				heavyAns.back() += a[e];
			...
			isect.emplace_back(vector<int>(m, 0));
			for (int j = 0; j < m; j++) {
				for (auto& e : s[j])
					isect.back()[j] += table[e];
			}
		}
	}

	const int HEAVY_COUNT = (int)heavyIdx.size();
	vector<ll> tag(HEAVY_COUNT, 0);

	while (q--) {
		char cmd; R(cmd);
		if (cmd == '?') {
			int k; R(k); k--;
			if ((int)s[k].size() >= SQRT) {
				int idx = lower_bound(heavyIdx.begin(), heavyIdx.end(), k) - heavyIdx.begin();
