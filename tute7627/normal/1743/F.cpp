...
int main(){
  ...
  ll res=0,buf=0;
  bool judge = true;
  using SQ=SquareMatrix<modint,2>;
  vector<SQ>mat(2);
  rep(i,0,2){
    rep(from,0,2){
      mat[i][from][from|i]+=1;
  ...
  ll n;cin>>n;
  auto f=[&](SQ x,SQ y){return x*y;};
  auto iden=SQ::I(2);
  ll sz=300005;
  SegmentTree seg(n,f,iden);
  vector<vector<ll>>in(sz),out(sz);
  rep(i,0,n){
    ll l,r;cin>>l>>r;r++;
    in[l].PB(i);
    out[r].PB(i);
  }
  rep(i,1,n)seg.set(i,mat[0]);
  seg.build();
  ll now=0;
  modint ret=0;
  rep(i,0,sz){
    for(auto z:in[i]){
      if(z==0)now=1;
      else seg.update(z,mat[1]);
    }
    for(auto z:out[i]){
      if(z==0)now=0;
      else seg.update(z,mat[0]);
    }    
    auto all=seg.seg[1];
    ret+=all[now][1];
    /*if(all[now][1]!=0){
      cout<<i spa now spa all[now][1]<<endl;
    }*/
  }
  cout<<ret<<endl;
  return 0;
