...
void solve(){
                ...
                REP(t, 4) {
                    REP(p, 4) {
                        REP(w, 4) {
                            ...
                            next.push_back(mp(min(tea,n), min(m,ge)));
                            next.push_back(mp(max(0LL,n - i), max(0LL,m - q)));
                            next.push_back(mp(min(n,j), min(m,t)));
                            next.push_back(mp(max(0LL,n - p),max(0LL, m - w)));
                            int ok = 1;
                            int dist = 0;
                            REP(q, next.size()) {
                                REP(j, next.size()) {
                                    if (q != j and next[q] == next[j]) {
                                        ok = 0;
                                        break;
                                    }
                                }
                                if (q == next.size() - 1) continue;
                                int itr = (q + 1) % next.size();
                                dist += (next[q].first - next[itr].first) * (next[q].first - next[itr].first) + (next[q].second - next[itr].second) * (next[q].second - next[itr].second);
                            }
                            if (ok == 0) continue;
                            ans.push_back(mp(dist, next));
                        }
                    }
                }
            }
    ...
    sort(ALL(ans));
    REP(q, 4) {
        cout << ans.back().second[q].first << " " << ans.back().second[q].second << endl;
    }
}
#undef int
int main() {
