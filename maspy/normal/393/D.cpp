  ...
  FOR3(n, 1, N + 1) {
    if (n == 1)
      ...
      FOR(i, 3) FOR(j, 3) if (i != j) {
        ll k = 3 - i - j;
        chmin(DP[n][i][j], DP[n - 1][i][k] + COST[i][j] + DP[n - 1][k][j]);
        chmin(DP[n][i][j], DP[n - 1][i][j] + COST[i][k] + DP[n - 1][j][i] +
                             COST[k][j] + DP[n - 1][i][j]);
      }
    }
    auto& dp = DP[n];
    FOR(k, 3) FOR(i, 3) FOR(j, 3) chmin(dp[i][j], dp[i][k] + dp[k][j]);
    // print(n);
    // print(dp);
  }
  ll ANS = DP[N][0][2];
