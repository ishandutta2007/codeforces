...
ll T;
ll n;
mint beki[2000005], beki2[2000005];
mint dp[1000005], dpsum[1000005];
...
int main(void)
{
	...
	while(T--){
		...
		per(i, n-1, 2){
			dp[i] = 0;
			if(n < 2*i) dp[i] += dp[n] * beki2[n-i-1];
			ll r = min(2*i-1, n-1);
			if(i+1 <= r) dp[i] += (dpsum[i+1] - dpsum[r+1]) * beki[i];
			dpsum[i] = dpsum[i+1] + dp[i] * beki2[i];
		}
