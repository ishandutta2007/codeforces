#line 1 "codeforces-workspace\\contest\\1161\\d\\d2.cpp"
...
#line 6 "codeforces-workspace\\contest\\1161\\d\\d2.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace\\contest\\1161\\d\\d2.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1161\\d\\d2.cpp"
...
#line 35 "codeforces-workspace\\contest\\1161\\d\\d2.cpp"
...
void main() {
  ...
  if (false) {  // when |A| > |S|
    ...
    while (t.back() != '1') {
      ...
      for (auto i = 0; i < t.size() - 1 - i; ++i) {
        if (t[i] == '?' && t[t.size() - i - 1] == '?') {
          co *= 2;
        } else if (t[i] != t[t.size() - 1 - i]) {
          co = 0;
        }
        co *= 2;
      }
  ...
  if (s.back() != '1') {  // when |A| < |S|
    s.back() = '0';
    reverse(begin(s), end(s));

    for (auto a : range(1, s.size())) {
      mint co{1};

      vector<i32> visited(s.size());
      for (auto i : rrange(s.size())) {
        if (visited[i]) {
          continue;
        }
