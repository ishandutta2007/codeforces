...
void solve() {
    ...
    for(int x=0; x<n; ++x) {
        ...
        int ymx=pos[x].back();
        vector<int> nex;
        auto itr=lower_bound(all(saf),ymi);
        if(itr!=begin(saf)) {
            nex.emplace_back(*prev(itr));
        }
        if(itr!=end(saf)) {
            nex.emplace_back(*itr);
        }
        itr=lower_bound(all(saf),ymx);
        if(itr!=begin(saf)) {
            nex.emplace_back(*prev(itr));
        }
        if(itr!=end(saf)) {
        ...
        nex.erase(unique(all(nex)),end(nex));
        if(x==xmx) {
            nex.clear();
            nex.emplace_back(ymi);
            nex.emplace_back(ymx);
            nex.erase(unique(all(nex)),end(nex));
            goals=nex.size();
        }
        vector<pii> nexx;
        for(int to:nex) {
            int tidx=vn++;
            for(pii &fro:now) {
                int dis=abs(fro.fir-to)+2*max(0,fro.fir-ymi)+2*max(0,ymx-to);
                if(fro.fir<=to) {
                    dis=to-fro.fir+2*max(0,fro.fir-ymi)+2*max(0,ymx-to);
                } else {
                    dis=fro.fir-to+2*max(0,to-ymi)+2*max(0,ymx-fro.fir);
                }
                g[fro.sec].emplace_back(tidx,dis);
            }
            nexx.emplace_back(to,tidx);
        }
        swap(nexx,now);
        prv=x;
    }
    ...
    while(!que.empty()) {
        i64 c,v; tie(c,v)=que.top();
        que.pop();
        if(dist[v]<c) continue;
        for(auto &e:g[v]) {
            if(chmin(dist[e.fir],e.sec+dist[v])) {
                que.emplace(dist[e.fir],e.fir);
            }
        }
    }
    for(auto &e:g[5]) {
