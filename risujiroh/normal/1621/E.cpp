...
int fcmp(ld x, ld y) {
  if (x < y && nextafter(nextafterl(x, y), y) < y) return -1;
  if (y < x && nextafter(nextafterl(y, x), x) < x) return 1;
  return 0;
}
...
void rj::main(int) {
  ...
  const auto m = input();
  const auto a = [&] {
    auto ret = input<vector<i64>>(n);
    ALL(sort, ret, greater());
    ret.resize(m);
    ALL(reverse, ret);
    return ret;
  }();
  const auto b = gen_view(m, [] { return input<vector<i64>>(input()); }) | to_vector;
  DUMP(n, m);
  DUMP(a);
  DUMP(b);
  auto avg = vector<ld>(m);
  for (const auto i : rep(m)) {
    avg[i] = ALL(accumulate, b[i], 0ll) / ld(size(b[i]));
  }
  auto order = rep(m) | to_vector;
  ALL(sort, order, lambda(avg[$1] < avg[$2]));
  auto rank = vector(m, -1);
  for (const auto i : rep(m)) {
    rank[order[i]] = i;
  }
  auto suff = vector<int>(m + 1);
  for (const auto i : per(m)) {
    suff[i] = (fcmp(a[i], avg[order[i]]) < 0) + suff[i + 1];
  }
  auto suff_l = vector<int>(m + 1);
  for (const auto i : per(m)) {
    suff_l[i] = (i ? fcmp(a[i], avg[order[i - 1]]) < 0 : 0) + suff_l[i + 1];
  }
  auto suff_r = vector<int>(m + 1);
  for (const auto i : per(m)) {
    suff_r[i] = (i + 1 < m ? fcmp(a[i], avg[order[i + 1]]) < 0 : 0) + suff_r[i + 1];
  }
  auto ans = string();
  for (const auto i : rep(m)) {
    for (const auto j : rep(size(b[i]))) {
      const auto x = (avg[i] * size(b[i]) - b[i][j]) / (size(b[i]) - 1);
      const auto pos = int(ALL(lower_bound, order, x, lambda(avg[$1] < $2)) - begin(order));
      auto ok = true;
      if (rank[i] < pos) {
        ok &= suff[0] == suff[rank[i]];
        ok &= suff_r[rank[i]] == suff_r[pos - 1];
        ok &= fcmp(x, a[pos - 1]) <= 0;
        ok &= suff[pos] == 0;
      } else {
        ok &= suff[0] == suff[pos];
        ok &= fcmp(x, a[pos]) <= 0;
        ok &= suff_l[pos + 1] == suff_l[rank[i] + 1];
        ok &= suff[rank[i] + 1] == 0;
      }
      ans += char('0' + ok);
    }
