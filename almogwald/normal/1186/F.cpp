...
int main() {
	...
	fori(i, n) {
		...
		forin(j, cons[i]) {
			...
			options.push_back({can[cons[i][j]],cons[i][j] });
		}
		sort(all(options));
		forinb(j, options) {
			if (can[i] > 0 && can[options[j].second] > 0) {
				can[i]--;
				can[options[j].second]--;
			}
			else {
				ans.push_back({ i, options[j].second });
			}
		}
	}
	co ans.size() el
	forin(i, ans) {
		out(ans[i].first+1) co ans[i].second + 1 el
	}
	ex
}