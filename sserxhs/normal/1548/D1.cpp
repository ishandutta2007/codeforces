...
int main()
{
	...
	T=1;
	read(n);
	for (i=1;i<=n;i++) read(a[i],b[i]),a[i]>>=1,b[i]>>=1;
	for (i=1;i<=n;i++)
	{
		memset(cnt,0,sizeof cnt);
		for (j=i+1;j<=n;j++) ++cnt[(a[i]^a[j])&1][(b[i]^b[j])&1];
		for (j=0;j<=1;j++) for (k=0;k<=1;k++) if (cnt[j][k]) for (x=0;x<=1;x++) for (y=0;y<=1;y++)
			if (cnt[x][y]&&0==(__gcd(x,y)^__gcd(j,k)^__gcd(x^j,y^k)))
			{
				//printf("%d %d %d %d %d\n",i,j,k,x,y);
				if (j!=x||y!=k) ans+=cnt[j][k]*cnt[x][y]; else ans+=cnt[x][y]*(cnt[x][y]-1);
			}
	}
	ans>>=1;enter(ans);
	//for (i=1;i<=n;i++) for (j=i+1;j<=n;j++) for (k=j+1;k<=n;k++) if ((__gcd(a[j]-a[i],b[j]-b[i])+__gcd(a[j]-a[k],b[j]-b[k])+__gcd(a[k]-a[i],b[k]-b[i]))%2==0) --ans;
	//assert(!ans);
}