...
void Main() {
	...
	poly a(n + 1), b(n + 1);
	L(i, 0, n / 2) 
		a[i * 2] = (ll) ifac[i * 2] * f[i] % mod;
	L(i, 0, n) 
		b[i] = ifac[i];
	a *= b;
	L(i, 0, n) a[i] = (ll) a[i] * fac[i] % mod;  
	int ns = 0, tmp = 1;
	L(c, 0, n / 4) {
		int ls = n - c * 4;
		int cur = C(n - c * 2, c * 2);
		cur = (ll) cur * qpow(2, c) % mod;
		cur = (ll) cur * f[c] % mod;
		cur = (ll) cur * tmp % mod;
		(ns += (ll) cur * a[ls] % mod) %= mod;
	} 
	cout << ns << '\n';
}
int main() {
	...
	Pinit(N - 7);
	f[0] = 1;
	L(i, 1, N - 7) f[i] = (ll) f[i - 1] * (i * 2 - 1) % mod;
	int t;
	cin >> t;
	while(t--) Main();
	return 0;
