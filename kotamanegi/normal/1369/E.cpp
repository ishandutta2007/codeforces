...
int cnt[300000];
int edge_used[300000];
int vertex_used[300000];
...
void solve() {
    ...
    while (nexts.empty() == false) {
        ...
        nexts.pop();
        for (auto& x : vertexs[now]) {
            if (edge_used[x.second] == 1) continue;
            edge_used[x.second] = 1;
            cnt[x.first]++;
            ans.push_back(x.second + 1);
            if (vertex_used[x.first] == 0 and cnt[x.first] >= 0) {
                nexts.push(x.first);
                vertex_used[x.first] = 1;
            }
        }
    }
    if (ans.size() != m) {
