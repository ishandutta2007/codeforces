...
void dfs(int v, int par){
    ...
    if (u != par){
        dfs(u, v);
        vector<vector<int> > p(11, vector<int>(3, 0));
        for (int x = bl; x >= 0; x--)
            for (int y = bl - x; y >= 0; y--){
                p[x + y][0] = (p[x + y][0] + (ll)q[v][x][0] * (ll)(q[u][y][0] + (ll)q[u][y][1] + (ll)q[u][y][2])) % mod;
                p[x + y][1] = (p[x + y][1] + (ll)q[v][x][1] * (ll)q[u][y][0]) % mod;
                p[x + y][2] = (p[x + y][2] + (ll)q[v][x][2] * (ll)(q[u][y][0] + (ll)q[u][y][2])) % mod;
            }
        for (int i = 0; i <= bl; i++)
            for (int j = 0; j < 3; j++)
                q[v][i][j] = p[i][j];
