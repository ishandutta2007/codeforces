...
int main() {
    ...
    const long long s = -accumulate(all(a), 0LL);

    const long long m = count(all(a), 0LL);
    ...
    long long lx = cld(s + m * k, 2 * k) - 1;
    long long rx = fld(s + m * k, 2 * k);
    ...
    long long ans = 0;
    for (const int x : { lx, rx }) {
        const int y = m - 1 - x;
        const long long d = s - (x - y) * k;
        assert(abs(d) <= k);

        rep(r, n + 1) rep(l, r + 1) {
            long long p = sum[r] - sum[l];
            int c = cnt[r] - cnt[l];

            if (x >= c) {
                chmax(ans, abs(p + c * k));
            } else {
                chmax(ans, abs(p + x * k + d - (c - 1 - x) * k));
            }
            if (y >= c) {
                chmax(ans, abs(p - c * k));
            } else {
                chmax(ans, abs(p - y * k + d + (c - 1 - y) * k));
            }
        }
    }
