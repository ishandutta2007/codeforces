...
int main(){
  ...
  while(t--){
    ll n;cin>>n;
    auto ask=[&](ll xl,ll xr,ll yl,ll yr){
      vector<ll>vl(xr-xl);
      iota(ALL(vl),xl+1);
      vector<ll>vr(yr-yl);
      iota(ALL(vr),yl+1);
      cout<<"?" spa xr-xl spa yr-yl<<endl;
      debug(vl,vl.size());
      debug(vr,vr.size());
      ll k;cin>>k;
      return k;
    ...
    rep(i,0,n-1){
      ll p=ask(i,i+1,i+1,n);
      if(p){
        vector<ll>ret;
        ll t=p;
        rep(j,i+1,n-1){
          ll nt=ask(i,i+1,j,j+1);
          t-=nt;
          //cout<<t spa nt<<endl;
          if(!nt)ret.PB(j+1);
        }
        ...
        cout<<"!" spa ret.size() << " ";debug(ret,ret.size());
        break;
      }
    }
  }
