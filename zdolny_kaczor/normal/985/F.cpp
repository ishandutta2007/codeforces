...
struct frac_mod {int o,m;int get(){return (o*1ll*mpow(m,MOD-2)%MOD+MOD)%MOD;}frac_mod(int x):o(x%MOD),m(1){}frac_mod(LL x):o(x%MOD),m(1){}
  frac_mod(LL a = 0,LL b = 1):o(a),m(b){}
...
const int MN = 2e5 + 44, A = 26;
const zet_p B = 1234567;
char in[MN];
zet_p pows[MN], hashes[MN];
SI occ[A];
#define prev __prev_X3v_k
#define hash __hash_jC6eq
int prev[MN];
zet_p hash(int start, int l) {
  debug(string(in + start, in + start + l));
  debug(range(prev, imie(start), imie(start) + imie(l) - 1));
  zet_p o = (hashes[start + l] - hashes[start]);
  REP(i, A) {
    auto it = occ[i].lower_bound(start);
    if (it == occ[i].end())
      continue;
    if (*it < start + l) {
      o -= pows[*it] * (prev[*it] + 1);
...
int main() {
  ...
  REP(i, n) {
    ...
    if (occ[l].empty())
      ...
      prev[i] = i - *occ[l].rbegin();
    occ[l].insert(i);
  }
  debug(range(occ, 0, A - 1));
  debug(range(prev, 0, n - 1));
  pows[0] = 1;
  REP(i, MN - 1)
    pows[i + 1] = pows[i] * B;
  hashes[0] = 0;
  REP(i, n)
    hashes[i + 1] = hashes[i] + pows[i] * prev[i];
  REP(_, m) {
    int a, b, l;
    scanf("%d%d%d", &a, &b, &l);
    a--;
    b--;
    if (hash(a, l) == hash(b, l))
      printf("YES\n");
    else
      printf("NO\n");
  }
