...
public class Task58A {
	...
	private void solve() {
		...
		int sampleIndex = 0;
		for (int i = 0; i < word.length && sampleIndex < 5; i++) {
			if (word[i] == sample[sampleIndex])
				sampleIndex++;
		}
		out.println(sampleIndex == 5 ? "YES" : "NO");
	}

	private static List<Test> createTests() {
		List<Test> tests = new ArrayList<Test>();
		tests.add(new Test("ahhellllloou", "YES"));
		tests.add(new Test("hlelo", "NO"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
...
//		return strictCheck(result, output);
	...
	private static void test() {
		...
		for (Test test : tests) {
			...
			try {
				new Task58A(in, new PrintWriter(out)).run();
	...
	private Task58A(InputReader in, PrintWriter out) {
		this.in = in;
		this.out = out;
		testMode = true;
	}
