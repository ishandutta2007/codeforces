...
const int N=2e5+5,M=121,K=7;
int n,k,m,p[N][K],nxt[N][M],coef[N],vis[M],cnt;
int st[M][K],u[K],f[M][M];
i64 sum;
int tr(int *a){
	int res=0,mul=1;
	for(int i=k-1;i>=1;--i){
		int cnt=0;
		for(int j=i+1;j<=k;++j) if(a[i]>a[j]) ++cnt;
		res=res+cnt*mul;
		mul*=k-i+1;
	}
	return res;
...
int main(){
	...
	for(int i=1;i<=n;++i){
		for(int j=1;j<=k;++j) scan(p[i][j]);
		coef[i]=tr(p[i]);
	}
	for(int i=1;i<=k;++i) u[i]=i;
	for(int i=0;i<m;++i) nxt[n+1][i]=n+1;
	for(int i=n;i>=1;--i){
		memcpy(nxt[i],nxt[i+1],sizeof(int)*m);
		nxt[i][coef[i]]=i;
	}
	do{
		memcpy(st[tr(u)],u,sizeof(u));
	}while(next_permutation(u+1,u+k+1));
	for(int i=0;i<m;++i) for(int j=0;j<m;++j) {
		int *a=st[i],*b=st[j];
		for(int c=1;c<=k;++c) u[c]=a[b[c]];
		f[i][j]=tr(u);
	}
	for(int l=1;l<=n;++l){
		for(int j=0;j<m;++j) vis[j]=0;
		vis[0]=cnt=1;
		for(int x=coef[l];x;x=f[x][coef[l]]) vis[x]=1,++cnt;
		int r=l;
		vector<int> lb;
		lb.pb(coef[l]);
		while(true){
			int cm=n+1,pos=-1;
			for(int j=0;j<m;++j) if(!vis[j]&&nxt[r+1][j]<cm) cm=nxt[r][j],pos=j;
			sum+=(i64)(cm-r)*cnt;
			//cerr<<l<<";"<<r<<","<<cm-1<<":"<<cnt<<endl;
			if(cm>n) break;
			lb.pb(pos);
			queue<int> q;
			for(int x=0;x<m;++x) if(vis[x]) q.push(x);
			while(q.size()){
				int b=q.front();
				q.pop();
				for(int k:lb) {
					if(!vis[f[b][k]]) ++cnt,q.push(f[b][k]),vis[f[b][k]]=1;
				}
			}
			r=cm;
		}
		...
		if(l==1){
			for(int j=0;j<m;++j) if(!vis[j]){
				for(int x=1;x<=k;++x) cerr<<st[j][x]<<' '; cerr<<endl;
			}
		}*/
	}
	print(sum),putc('\n');
