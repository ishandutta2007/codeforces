...
int Check( int l , int r , int cutinto , int maxlen ){
	int nowsum = 0, cuted = 0;
	Forx(i,l,r){
		if( nowsum + a[i] > maxlen ){
			cuted++;
			if( cuted+1 > cutinto ) return 0;
			nowsum = a[i];
		}else{
			nowsum += a[i];
		}
...
// XXXXXXOOOOOO
int Minimum_max( int L , int R , int cutinto ){
	if(mem[L][R].count(cutinto)) return mem[L][R][cutinto];
	int l = 1, r = sum[R]-sum[L-1], mid, ans = r;
	Forx(i,L,R) Mymax(l,a[i]);
	while( l <= r ){
		mid = l+r >> 1;
		if(Check(L,R,cutinto,mid)){
			r = mid-1;
			Mymin(ans,mid);
		}else{
			l = mid+1;
...
int main(){
	...
	For(i,m){
		...
		if( s == f ) continue;
		if( s > f ) swap(s,f);
		int mini_max = Minimum_max(s,f-1,r+1);
		ll need = 1LL*c*mini_max;
		// O(need);
		Mymax(ans,need);
	}
	printf("%lld\n",ans);
