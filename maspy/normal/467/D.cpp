...
void solve() {
  auto to_small = [&](string S) -> string {
    FORIN(x, S) if ('A' <= x && x <= 'Z') x = x - 'A' + 'a';
    return S;
  };
  ...
  ll C = MAX(comp) + 1;
  const ll INF = 1LL << 60;
  vc<pi> vals(C, {INF, INF});
  FOR(v, V) {
    chmin(vals[comp[v]], mp(ll(count(all(keys[v]), 'r')), len(keys[v])));
  }
  FOR_R(c, C) {
    FORIN(v, scc.comp[c]) FORIN(e, G[v]) {
      ll to = comp[e.to];
      chmin(vals[c], vals[to]);
    }
  }
