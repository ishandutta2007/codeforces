...
//    cout << endl;
    ...
    FI(q) {
        vq[qq[i][0] - 1].push_back({i, qq[i][1] - 1});
    }
    for (int i = 1; i <= n; ++i) {
        add(p[i]);
        for (auto p : vq[i]) {
            ans[p.first] += c2(sum(p.second));
        }
        vq[i].clear();
    }
    /****/
...
//    cout << endl;
    ...
    FI(q) {
        vq[qq[i][0] - 1].push_back({i, qq[i][3]});
    }
    for (int i = 1; i <= n; ++i) {
        add(p[i]);
        for (auto p : vq[i]) {
            ans[p.first] += c2(i - sum(p.second));
        }
        vq[i].clear();
    }
    /****/
...
//    cout << endl;
    memset(tr, 0, sizeof(tr));
    FI(q) {
        vq[qq[i][2] + 1].push_back({i, qq[i][1] - 1});
    }
    for (int i = n; i >= 1; --i) {
        add(p[i]);
        for (auto p : vq[i]) {
...
//    cout << endl;
    memset(tr, 0, sizeof(tr));
    FI(q) {
        vq[qq[i][2] + 1].push_back({i, qq[i][3]});
    }
    for (int i = n; i >= 1; --i) {
//        DBN(i, vq[i]);
        add(p[i]);
        for (auto p : vq[i]) {
//            DBN(n - i + 1 - sum(p.second));
            ans[p.first] += c2(n - i + 1 - sum(p.second));
        }
        vq[i].clear();
    }
    FI(q) {
