...
int main() {
    ...
    cin >> str;
    for (char c : str) {
        pts.erase(find(all(pts), st));
        int who = find(all(hull), st) - hull.begin();
        if (c == 'L') {
            st = hull[(who + 1) % hull.size()];
        } else {
            st = hull[(who - 1 + hull.size()) % hull.size()];
        }
        ans.push_back(inx[st]);
