...
void solve()
{
    ...
    vector<vector<ll> > dist(n, vector<ll>(51, INFLL));
    dist[0][0] = 0;
    set<pair<ll, pair<ll, ll> > > s;
    s.insert({0, {0, 0}});
    while (s.size())
    {
        pair<ll, pair<ll, ll> > S = *s.begin();
        s.erase(s.begin());
        ...
        else
        {
            ...
            {
                int u1 = e.first, u2 = e.second;
                if (dist[v][S.se.se] + (S.se.se + u2) * (S.se.se + u2) <= dist[u1][0])
                {
                    s.erase({dist[u1][0], {u1, 0}});
                    dist[u1][0] = dist[v][S.se.se] + (S.se.se + u2) * (S.se.se + u2);
                    s.insert({dist[u1][0], {u1, 0}});
                }
            }
        }
    }
