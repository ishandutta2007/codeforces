...
void solve() {
  ...
  auto calc = [&](ll p) -> vi {
    ...
    P[k] = p;
    vc<pi> event;
    FOR(i, N) event.eb(mp(T[i], i));
    sort(all(event));
    reverse(all(event));
    priority_queue<pi> que;

    ll now = 0;
    while (!que.empty() || !event.empty()) {
      if (!event.empty() && event.back().fi == now) {
        auto [t, i] = event.back();
        event.pop_back();
        que.push(mp(P[i], i));
        continue;
      ...
      auto [p, i] = que.top();
      ll nxt = now + rest[i];
      if (!event.empty()) chmin(nxt, event.back().fi);
      rest[i] -= nxt - now;
      if (rest[i] == 0) {
        ANS[i] = nxt;
        que.pop();
  ...
  set<ll> ss;
  FORIN(x, P) ss.insert(x / 2);

  auto p = binary_search(check, MAX(P) + 10, 0);
  while (ss.count(p)) ++p;
  print(p);
  print(calc(p));
}
