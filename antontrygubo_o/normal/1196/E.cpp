...
void solve()
{
    ...
    for (int i = 0; i<2*w-1; i++) a.push_back(mp(2, 2 + i));
    b-=(w-1);
    if (b>2*w+2) {cout<<"NO"<<endl; return;}
    for (int i = 0; i<w; i++)
    {
        if (b==0) break;
        a.push_back(mp(1, 2 + i*2));
        b--;
    }
    for (int i = 0; i<w; i++)
    {
        if (b==0) break;
        a.push_back(mp(3, 2 + i*2));
        b--;
    }
    if (b!=0)
    {
        a.push_back(mp(2, 1)); b--;
    }
    if (b!=0)
    {
        a.push_back(mp(2, 2*w+1)); b--;
    }
    if (check)
    {
        for (auto &it: a) it.second++;
    }
    cout<<"YES\n";
