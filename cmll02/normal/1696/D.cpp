...
int b[1000005],q[1000005];
int n;
// SegTreeAl t;
struct qwq{
	int st[21][1000005],ts[21][1000005];
	void build(int n,int *a)
	{
		...
		for(int i=1;i<=20;i++)
			for(int j=1;j+(1<<i)-1<=n;j++)
				ts[i][j]=min(ts[i-1][j],ts[i-1][j+(1<<i-1)]);
	}
	...
	{
		int g=std::__lg(r-l+1);
		return min(ts[g][l],ts[g][r-(1<<g)+1]);
	}
}t;
int solve(int *a,int *q)
{
	if(a[1]<a[2])return inf;
	// t.build(1,1,n);
	t.build(n,a);
	// rg(n)odb(a[i]);gr puts("");
	// rg(n)t.add(1,1,n,i,i,a[i]);gr
	int cur=1,ans=0;
	while(cur<n)
	{
		if(ans&1)
		{
			...
			{
				int m=l+r>>1;
				if(t.qmin(cur,m)==a[cur])tt=m,l=m+1;
				else r=m-1;
			}
			int d=t.qmax(cur,tt);
			d=q[d];
			ans++;
			// odp(d,cur);fflush(stdout);
			cur=d;
		}
		else
		{
			...
			{
				int m=l+r>>1;
				// printf("[%d %d] = %d\n",cur,m,t.qmax(1,1,n,cur,m));
				if(t.qmax(cur,m)==a[cur])tt=m,l=m+1;
				else r=m-1;
			}
	// printf("()");		odp(cur,tt);
			int d=t.qmin(cur,tt);
