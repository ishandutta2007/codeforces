...
int main(void)
{
	...
	rep(i, 0, n) rep(j, 0, n) rep(k, 0, n) dp[i][0][j][k] = dp[i][1][j][k] = -inf;
	dp[0][0][0][0] = 0;
	
	rep(i, 0, n){
		rep(j, 0, n){
			rep(k, 0, n){
				chmax(dp[i][1][j][k], dp[i][0][j][k]);
				chmax(dp[i+1][0][j][k], dp[i][0][j][k] + a[j]);
			}
		}
		rep(j, 0, n){
			rep(k, 0, n){
				...
				ll nk = k+1 - (j-i);
				if(i <= j && nk >= 0) chmax(dp[i+1][0][j+1][nk], dp[i][1][j][k] + a[j+1]);
			}
		}
	}
	
	int ans = 0;
	rep(j, 0, n+2) rep(k, 0, n+2) chmax(ans, dp[n][0][j][k]);
	outl(ans);
