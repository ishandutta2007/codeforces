...
int main() {
  rep(IN) {
    ...
    auto a = IN.vll(n);
    vector<PLL> b(n/2);
    rep(i, n/2) b[i] = minmax(a[i], a[n-i-1]);
    sort(all(b), [&](auto a, auto b){
      return get<1>(a) < get<1>(b) or (get<1>(a) == get<1>(b) and get<0>(a) < get<0>(b));
    });
    auto c = b;
    sort(all(c));
    map<ll, int> cnt;
    rep(i, n/2) cnt[b[i].first+b[i].second]++;
    ll res = INF_LL;
    for (auto [x, v] : cnt) {
      ll tmp = n/2 - v;
      tmp += lower_bound(all(b), PLL{0, x-k}, [&](auto a, auto b){
        return get<1>(a) < get<1>(b) or (get<1>(a) == get<1>(b) and get<0>(a) < get<0>(b));
      }) - b.begin();
      tmp += c.end() - lower_bound(all(c), PLL{x, 0});
      if (n == 8) {debug(x); debug(tmp);}
      chmin(res, tmp);
    }
    map<ll, int> cnt2;
    rep(i, n/2) cnt2[b[i].first]++, cnt2[b[i].second] += b[i].first != b[i].second;
    for (auto [x0, v] : cnt2) {
      auto x = x0+1;
      ll tmp = n/2 - cnt[x];
      tmp += c.end() - upper_bound(all(c), PLL{x, 0});
      if (n == 8) {debug(x); debug(v); debug(tmp);}
      chmin(res, tmp);
    }
    OUT(res);
  }
