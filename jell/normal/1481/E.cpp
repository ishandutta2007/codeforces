#line 1 "codeforces-workspace\\contest\\1481\\e\\e.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\1481\\e\\e.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1481\\e\\e.cpp"
...
void main() {
  ...
  cin >> n;
  vector<int> a(n);
  vector<int> cnt(n + 1), last(n + 1), first(n + 1);
  cin >> a;
  ...
  for (auto &&[i, e] : enumerate(a)) {
    ...
    chgr(dp[i + 1], dp[i]);
    for (auto f : add[i]) {
      chgr(dp[i + 1], dp[first[f]] + cnt[f]);
    }
    chgr(ans, dp[i + 1]);
    chgr(ans, dp[pre] + (i - pre) + aft[e]);
    aft[e]--;
  }
