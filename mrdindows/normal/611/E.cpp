...
//        p[i] /= 10;
    ...
    for (int i = 0; i < n; ++i) {
        int x = p[i];
        if (x > b + c) used[i] = true, ++tot;       // can only ABC
    }
    int ans = n;
    int powers[6] = {a, b, c, a + b, a + c, b + c};
    vector<int> s(64);
    for (int i = 0; i < n; ++i) {
        int x = p[i];
        int e = 0;
...
//    DBN(s);
    for (int oc = 0; oc <= n; ++oc) {
        ...
        int t = oc;
        for (int i = 0; i < 64; ++i) {
            if (v[i] && t && !checkbit(i, 3) && checkbit(i, 2)) {   // can C, can not AB
                int d = min(v[i], t);
                v[i] -= d;
                t -= d;
            }
        ...
        for (int i = 0; i < 64; ++i) {
            if (v[i] && t && !checkbit(i, 1) && checkbit(i, 2)) {   // can C, can not B
                int d = min(v[i], t);
                v[i] -= d;
                t -= d;
            }
        ...
        for (int i = 0; i < 64; ++i) {
            if (v[i] && t && !checkbit(i, 0) && checkbit(i, 2)) {   // can C, can not A
                int d = min(v[i], t);
                v[i] -= d;
                t -= d;
            }
        ...
        for (int i = 0; i < 64; ++i) {
            if (v[i] && t && checkbit(i, 2)) {                      // other can C
                int d = min(v[i], t);
                v[i] -= d;
                t -= d;
            }
...
//        }
        ...
        int workA = 0;
        for (int i = 0; i < 64; ++i) {
            if (v[i] && checkbit(i, 5) && !checkbit(i, 4)) {        // can BC, can not AC
                int d = v[i];
                v[i] -= d;
                res += d;
                workB += d;
        ...
        for (int i = 0; i < 64; ++i) {
            if (v[i] && checkbit(i, 4) && !checkbit(i, 3)) {        // can AC, can not AB
                int d = v[i];
                v[i] -= d;
                res += d;
                workA += d;
        ...
        int canAB = 0;
        for (int i = 0; i < 64; ++i) {
            if (v[i] && checkbit(i, 0)) {           // can A
                canA += v[i];
            } else if (v[i] && checkbit(i, 1)) {    // can B, can not A
                canB += v[i];
            } else if (v[i]) {                      // can only A + B
                canAB += v[i];
            }
        }
//        if (oc == 3) {
//            DBN(res);
//        }
        res = max(res, res - oc + canAB);
        res = max(res, canAB + canB + workB);
        int leftB = res - canAB - canB - workB;
        res = max(res, canAB + canA + workA - min(leftB, canA + workA));
//        if (oc == 3) {
//            DBN(leftB);
//            DBN(canA, canB, canAB, workA, workB);
//            DBN(v);
//        }
        ans = min(ans, res);
    }
