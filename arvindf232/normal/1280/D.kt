...
fun main(){
    solve.cases{
        ...
        val every = 1L shl 47
        val halfevery = 1L shl 46
        val everymask = every - 1

        val DP = Array(n){LongArray(0)}
        G.leafFirst {v ->
            var here = longArrayOf(0,real[v].toLong())
            G.subs(v){w ->
                val was = DP[w]
                val new = LongArray(here.lastIndex + was.lastIndex + 1){nhuge}
                for((i,s) in was.withIndex()){
                    if(i == 0) continue
                    for((j,t) in here.withIndex()){
                        if(j == 0) continue
                        new[i+j-1] = maxOf(new[i+j-1],s + t)
                        var raws = (s + halfevery) and (everymask.inv())
                        if((s + halfevery) and everymask > halfevery){
                            raws += every
                        }
                        new[i+j] = maxOf(new[i+j], t + raws)
                    }
                }
                here= new
            }
        ...
        val best = DP[G.root][m]
        var set = (every /2  + best) / every
        val remainign = best - set * every
        if(remainign > 0){
            set ++
        }
...
/*
mashing states have some conversions issues
 */