...
int tp[5];
int n;
int scores[6] = {500, 1000, 1500, 2000, 2500, 3000};
int getScore(int min, int score) {
    if (min == -1) return 0;
    return score * (250 - min);
}
int iter = 0;
void go(int cur) {
    if (cur == 5) {
        int diff = 0;
        for (int i = 0; i < 5; ++i) {
            diff += getScore(a[0][i], scores[tp[i]]);
            diff -= getScore(a[1][i], scores[tp[i]]);
        }
        ++iter;
        if (diff <= 0) return;
