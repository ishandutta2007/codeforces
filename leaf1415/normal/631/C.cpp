...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, n) cin >> a[i];
	rep(i, 1, m) cin >> t[i] >> r[i];

	vec.push_back(0);
	per(i, m, 1){
		if(r[vec.back()] < r[i]) vec.push_back(i);
	}
	reverse(all(vec));
	
	vector<ll> ans;
	per(i, n, r[vec[0]]+1) ans.push_back(a[i]);
	
	multiset<ll> S;
	rep(i, 1, r[vec[0]]) S.insert(a[i]);
	
	
	rep(i, 0, sz(vec)-2){
		ll id = vec[i], nid = vec[i+1];
		rep(j, 1, r[id]-r[nid]){
			if(t[id] == 1){
				auto it = S.end(); it--;
				ans.push_back(*it);
			...
			else ans.push_back(*S.begin()), S.erase(S.begin());
		}
	}
	reverse(all(ans));
	outl(ans);
