...
li solve(vi const& A) {
  vvi G(100);
  for(li x : A) {
    if(x == 0) G[0].push_back(x);
    else G[2+64-__builtin_clzll(x)].push_back(x - (1<<(63-__builtin_clzll(x))));
  }
  vvi H;
  for(li i = 0; i < (li)(100); ++i) if(!G[i].empty()) H.push_back(G[i]);
  if(H.size() == 1) {
    return solve(H[0]);
  }
  li ans = H.size();
  for(li i = 0; i < (li)(H.size()); ++i) if(H[i].size() >= 2) {
    smax<li>(ans,
             solve(H[i]) +
             (i == 0 ? 0 : 1) +
             (H.size()-1-i)
