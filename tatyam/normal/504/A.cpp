...
signed main(){
    ...
    vector<pll>ans;
    queue<ll>q;
    rep(n)if(a[i].first==1)q.push(i);
    while(q.size()){
        ll at=q.front();
        q.pop();
        if(a[at].first!=1)continue;
        ans.emplace_back(at,a[at].second);
        a[a[at].second].first--;
        a[a[at].second].second^=at;
        if(a[a[at].second].first==1)q.push(a[at].second);
    }
    out(ans.size());
    each(i,ans)out(i);
}