...
void solve() {
    ...
    rep(i, n) {
        ...
        rep(_, k) {
            INT(x);
            if(!cprs.count(x)) cprs[x] = si(cprs);
            v[i].eb(cprs[x]);
        }
        sort(all(v[i]));
    }
    ...
    rep(i, si(idx)) {
        ...
        for(auto e : idx[i])
            if(si(v[e]) <= T) {
                for(auto j : v[e]) w[j] = 0;
            }
    }

    rep(i, n) {
        if(si(v[i]) > T) {
            for(auto e : v[i]) w[e] = 1;
            rep(j, n) {
                if(j <= i and si(v[j]) > T) continue;
                int cnt = 0;
                for(auto e : v[j]) cnt += w[e];
                if(cnt >= 2) {
                    cout << i + 1 << " " << j + 1 << endl;
                    return;
                }
            }
            for(auto e : v[i]) w[e] = 0;
        }
    }
    cout << -1 << endl;
    return;
