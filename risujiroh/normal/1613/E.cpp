...
std::array di{1, 0, -1, 0};
std::array dj{0, 1, 0, -1};

void solve(int) {
  auto h = input<int>();
  auto w = input<int>();
  auto s = input<std::vector<std::string>>(h);
  auto valid = [&](int i, int j) { return 0 <= i && i < h && 0 <= j && j < w && s[i][j] != '#'; };
  auto c = std::vector(h, std::vector(w, 0));
  auto si = -1;
  auto sj = -1;
  for (int i : rep(h))
    for (int j : rep(w)) {
  ...
  auto d = std::vector(h, std::vector(w, -1));
  auto q = std::vector<std::pair<int, int>>();
  d[si][sj] = 0;
  q.emplace_back(si, sj);
  for (auto pos = 0; pos < int(q.size()); ++pos) {
    auto [i, j] = q[pos];
    for (int z : rep(4)) {
      int ni = i + di[z];
      int nj = j + dj[z];
    ...
    for (int z : rep(4)) {
      int ni = i + di[z];
      int nj = j + dj[z];
      if (valid(ni, nj) && d[ni][nj] == -1 && c[ni][nj] < 2) {
        d[ni][nj] = d[i][j] + 1;
