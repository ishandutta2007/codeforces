...
#define lowbit(x) ((x)&-(x))
#define ll int
ll w[1<<24],sum[1<<24],f[1<<24],a[25],b[3];
const ll mod=1e9+7;
void md(ll&a,const ll&b){
	a+=b;
	if(a>=mod)a-=mod;
}
int main(){
	//IO(P2396 yyy loves Maths VII);
	ll rill(n);
	rep(i,0,n-1)rill(w[1<<i]=a[i]);
	ll rill(m);
	rep(i,1,m)rill(b[i]);
	ll up=(1<<n)-1;
	f[0]=1;
	rep(i,1,up){
		{
			register ll li=lowbit(i);
			sum[i]=sum[i^li]+w[li];
//			writeln(i,' ',sum[i],' ',w[li]);
