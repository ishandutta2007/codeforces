...
void solvethetestcase(){
	...
	REP(i,m) sfl(a[i])
	REP(i,200005) nxt[i] = -1,dp[i][0] = -1;
	nxt[pos[a[m-1]]] = m-1;
	for(int i = m-2; i >= 0; i --){
		if(nxt[(pos[a[i]]+1)%n] != -1){
			dp[i][0] = nxt[(pos[a[i]]+1)%n];
		}
		nxt[pos[a[i]]] = i;
	}
	FOR(j,1,21){
	...
	for(int i = m-1; i > 0; i --) seg[i] = min(seg[2*i],seg[2*i+1]);
	...
	WL(q){
		...
		while(l < r){
			if(l%2){
				if(seg[l++] <= orig_r) ans = 1;
			}
			if(r%2){
				if(seg[--r] <= orig_r) ans = 1;
			}
			l /= 2; r /= 2;
		}
		printf("%lld",ans);
