...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    int l, r, t;
    cin >> l >> r >> t;
    events.push_back({l, ~i, t - 1});
    events.push_back({r, i, t - 1});
    right[i] = r;
  }
  sort(events.begin(), events.end(), [](auto& x, auto& y) {
    return tie(x.x, x.id) < tie(y.x, y.id);
  });
  vector<bool> matched(n);
  vector<set<pair<int, int>>> r_to_i(2);
  int sz = 0;
  for (auto[x, id, type] : events) {
    if (id < 0) {
      r_to_i[type].emplace(right[~id], ~id);
    } else {
      if (!matched[id]) {
        auto& st = r_to_i[type ^ 1];
        while (!st.empty()) {
          auto[r, i] = *st.begin();
          st.erase(st.begin());
