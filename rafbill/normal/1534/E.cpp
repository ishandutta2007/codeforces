...
int main(){
  ...
  vvi G(n+1);
  for(li i = (0); i <= (li)(n); ++i) {
    for(li da = (0); da <= (li)(k); ++da) if(li db = k-da; da <= i && db <= n-i) {
      G[i].push_back(i-da+db);
    }
  }
  ...
  while(!Q.empty()) {
    li d,i; tie(d,i) = Q.top(); Q.pop();
    if(d != D[i]) continue;
    for(int j : G[i]) {
      if(d+1 < D[j]) {
        D[j]=d+1;
        F[j]=i;
        Q.push(make_tuple(D[j],j));
      }
    }
  }
  ...
  vi A(n); for(li i = 0; i < (li)(n); ++i)A[i]=i;
  vi B;

  reverse(begin(path), end(path));

  for(li i = 0; i < (li)((li)path.size()-1); ++i) {
    li a=path[i],b=path[i+1];
    li db=(b-a+k)/2;
    li da=k-db;


    vi CA,CB;
    while(da) { CA.emplace_back(A.back()); A.pop_back(); da--; }
    while(db) { CB.emplace_back(B.back()); B.pop_back(); db--; }
    vi X;
    X.insert(end(X),begin(CA), end(CA));
    X.insert(end(X),begin(CB), end(CB));
    ans ^= ask(X);
    A.insert(end(A),begin(CB), end(CB));
    B.insert(end(B),begin(CA), end(CA));
