...
int query(const int v, const int w)
{
	cout << "? " << v + 1 << ' ' << w + 1 << endl;
	int a;
	cin >> a;
...
void dfs(const int v, const int p, const vvi& g, const bool father, vb& nv)
{
	if (nv[v] == false)
	{
		if (father)
	...
	if (!father)
	{
		auto it = find(h.begin(), h.end(), p);
		if (it != h.end())
			h.erase(it);
	}
	while (h.size() >= 2)
	{
		int q = query(h[sz(h) - 1], h[sz(h) - 2]);
		if (q != v)
			dfs(q, v, g, false, nv);
		else
		{
