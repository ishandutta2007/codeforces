...
void solve() {
    ...
    if (!connected()) {
        ...
        if (!good) {
            vector<int> max_ctz;
            for (int i = 1; i <= n; i++) {
                if ((!max_ctz.empty()) && (__builtin_ctz(a[i]) > __builtin_ctz(a[max_ctz.back()]))) max_ctz.clear();
                if ((max_ctz.empty()) || (__builtin_ctz(a[i]) == __builtin_ctz(a[max_ctz.back()]))) max_ctz.emplace_back(i);
            }
            assert(max_ctz.size() >= 2);
            a[max_ctz[0]]--;
            a[max_ctz[1]]++;
            ans += 2;
        } else {
