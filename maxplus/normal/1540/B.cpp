...
int main() {
  ...
  for (int i = 0; i < n; ++i)
  for (int j = 0; j < n; ++j) {
    pp[i][j] = !i? 1: !j? 0: (pp[i - 1][j] + pp[i][j - 1]) / 2;
  }
  ...
  for (int j = 0; j < n; ++j)
  for (int k = 0; k < n; ++k) {
    mini(dst[j][k], dst[j][i] + dst[i][k]);
  }
  ModNum ans{};
  for (int i = 0; i < n; ++i)
  for (int j = 0; j < i; ++j)
  for (int k = 0; k < n; ++k) {
    if (dst[i][k] + dst[i][j] == dst[j][k]) {
      ans += 1;
    } else if (dst[j][k] + dst[i][j] == dst[i][k]) {
    } else {
      int dtp = (dst[i][k] + dst[j][k] - dst[i][j]) / 2;
      ans += pp[dst[i][k] - dtp][dst[j][k] - dtp];
    }
  }
  print(ans / n);
}