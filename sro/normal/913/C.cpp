...
int main()
{
	...
	for(int i=1;i<n;i++)can[i]=min(can[i],can[i-1]*2);
	for(int i=n-2;i>=0;i--)can[i]=min(can[i],can[i+1]);
	INT sum=0,msum=0;
	for(int i=0;i<31;i++)
	{
		if(sum>can[i])sum=can[i];
		if(BIT(m,i))
		{
			if(i<n)
			{
			...
			else sum+=MASK(i-n+1)*can[n-1],msum=30;
		}
	}
	cout<<min(sum,can[msum+1])<<endl;
	return 0;
}