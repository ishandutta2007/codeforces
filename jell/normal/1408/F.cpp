#line 1 "codeforces-workspace\\contest\\1408\\f\\f.cpp"
...
#line 11 "codeforces-workspace\\contest\\1408\\f\\f.cpp"
...
void main() {
  ...
  for (auto &&p : piles) {
    for (int x = 1; x < p.size(); x <<= 1) {
      for (int j = 0; j < p.size(); j += x * 2) {
        for (int k = j; k < j + x; ++k) {
          ans.emplace_back(p[k], p[k + x]);
        }
      }
    }
  }
  ...
  while (piles.size() > 2) {
    auto &la = piles.back();
    auto &fr = piles[0];
    while (piles[0].size() < piles[1].size()) {
      auto sz = fr.size();
      for (auto i : range(sz)) {
        ans.emplace_back(la.back(), fr[i]);
        fr.emplace_back(la.back());
        la.pop_back();
      }
    }
    auto sz = fr.size();
    for (auto i : range(sz)) {
      ans.emplace_back(fr.back(), piles[1][i]);
      piles[1].emplace_back(fr.back());
      fr.pop_back();
    }
