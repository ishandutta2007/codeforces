...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		while (firstBig < n && firstSmall < n) {
			while (firstBig < n && d[firstBig] <= 0) {
				++firstBig;
			}
			while (firstSmall < n && d[firstSmall] >= 0) {
				++firstSmall;
			}
			if (firstBig == n || firstSmall == n) {
				break;
			}
			auto cur = min(d[firstBig], -d[firstSmall]);
			ans.push_back({firstBig + 1, firstSmall + 1, cur});
			d[firstBig] -= cur;
			d[firstSmall] += cur;
		}
