...
void solve() {
  ...
  ll s = ID(sx, sy), g = ID(gx, gy);

  UnionFind uf(H * W);
  VEC(string, A, H);
  auto isin = [&](ll x, ll y) -> bool {
    return (0 <= x && x < H && 0 <= y && y < W && A[x][y] == '0');
  };
  ll dx[] = {1, 0, -1, 0};
  ll dy[] = {0, 1, 0, -1};
  FOR(x, H) FOR(y, W) {
    FOR(d, 4) {
      ...
      if (isin(x, y) && isin(nx, ny)) { uf.merge(ID(x, y), ID(nx, ny)); }
    }
  }

  if (uf[s] == uf[g]) return print(0);
  const ll INF = 1LL << 60;
  ll ANS = INF;
  FOR(a, H * W) FOR(b, H * W) {
    if (uf[a] == uf[s] && uf[b] == uf[g]) {
      auto [ax, ay] = divmod(a, W);
      auto [bx, by] = divmod(b, W);
      ll dx = ax - bx;
      ll dy = ay - by;
