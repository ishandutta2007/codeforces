...
int main() {
    ...
    for (auto &v : a) read(v);
    vec<vec<vec<vec<int>>>> dp(n + 1, vec<vec<vec<int>>>(m + 1, vec<vec<int>>(m / 2 + 2, vec<int>(k, -IINF))));
    dp[0][m][0][0] = 0;
    rep(i, n) {
        rep(num, m / 2 + 1) rep(rem, k) {
            chmax(dp[i + 1][0][0][rem], dp[i][m][num][rem]);
        }
        rep(j, m) rep(num, m / 2 + 1) rep(rem, k) {
            chmax(dp[i + 1][j + 1][num][rem], dp[i + 1][j][num][rem]);
            chmax(dp[i + 1][j + 1][num + 1][(rem + a[i][j]) % k], dp[i + 1][j][num][rem] + a[i][j]);
        }
    }
    int ans = 0;
    rep(num, m / 2 + 1) {
        chmax(ans, dp[n][m][num][0]);
    }
    print(ans);
    return 0;
}