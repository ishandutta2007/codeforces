...
bool ok(int M, vector<int> &a, int n, int m, int k)
{
    vector<int> b(n);
    forn(i, n) b[i] = (a[i] >= M);
    vector<int> okay_lefttakers(m);
    for (int leta = 0; leta < m; leta++)
    {
        int rita = m - 1 - leta;
        if (b[leta] || b[n - 1 - rita])
            okay_lefttakers[leta] = 1;
    }
    for (int tells = 0; tells < m && tells <= k; tells++)
    {
        int t = 1;
        for (int j = tells; j <= tells + max(0, m - 1 - k); j++)
        {
            if (okay_lefttakers[j] == 0)
                t = 0;
        }
        //cout << M << " " << tells << " " << t << endl;
        if (t) return 1;
