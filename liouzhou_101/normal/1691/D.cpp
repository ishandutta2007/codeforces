...
void solve()
{
	...
	auto max = static_cast<const ll& (*)(const ll&, const ll&)>(&std::max<ll>);
	data_structure::SparseTable<ll, decltype(max)> stmax(s.begin(), s.end(), max);

	auto min = static_cast<const ll & (*)(const ll&, const ll&)>(&std::min<ll>);
	data_structure::SparseTable<ll, decltype(min)> stmin(s.begin(), s.end(), min);
	...
	for (auto i : p)
	{
		...
		int r = *it - 1;
		int l = *prev(it) + 1;
		ll tmp = stmax.Query(i + 1, r + 2) - stmin.Query(l, i + 1);
		if (a[i] >= tmp) continue;
		return writeln("NO");
	}
	writeln("YES");
