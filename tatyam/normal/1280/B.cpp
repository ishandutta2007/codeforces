...
void solve(){
    ...
    VEC(string,s,r);
    if([&]{
        each(i,s)each(j,i)if(j=='A')return 0;
        return 1;
    }())return(void)out("MORTAL");
    if([&]{
        each(i,s)each(j,i)if(j=='P')return 0;
        return 1;
    }())return(void)out(0);
    auto rotate=[&]{
        vec(string,t,c);
        each(i,t)i.resize(r);
        rep(r)rep(j,c)t.rbegin()[j][i]=s[i][j];
        s=move(t);
        swap(r,c);
    };
    ll ans=4;
    rep(4){
        chmin(ans,[&]{
            if(count(all(s[0]),'P')==0)return 1;
            if(s[0][0]=='A')return 2;
            rep(r)if(count(all(s[i]),'P')==0)return 2;
            if(count(all(s[0]),'A'))return 3;
            return 4;
        }());
        rotate();
    }
