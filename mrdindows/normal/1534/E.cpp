...
int n, k;
int cnt[20202];
int arr[20202] = {2, 1, 7, 5, 6};
int ans2;
void ask(const vector<int>& v) {
    assert(v.size() == k);
    cout << "?";
    for (int x : v) {
        cnt[x] ^= 1;
...
int solve() {
    ...
    while (true) {
        rebuild();
//        DBN(even.size(), odd.size());
        if (even.size() == k) {
            ask(even);
            break;
        }
        if (2 * k >= even.size() && even.size() % 2 == 0 && (k - even.size() / 2) > odd.size() && k % 2 == 1) {
//            DBN("here");
            int dif = n - k;
            if (even.size() > 2 * dif) {
                vector<int> v1;
                ...
                for (int i = 0; i < dif; ++i) {
                    v1.push_back(even[i]);
                    used[even[i]] = true;
                    v2.push_back(even[i + dif]);
                    used[even[i + dif]] = true;
                }
                vector<int> s;
                FI(n) {
                    if (s.size() == k - dif) break;
                    if (!used[i]) {
                        s.push_back(i);
                    }
                }
        ...
        if (2 * k >= even.size() && even.size() % 2 == 0 && (k - even.size() / 2) <= odd.size()) {
            vector<int> v;
            FI(even.size() / 2) {
                v.push_back(even[i]);
            }
            ...
            v.clear();
            FI(even.size() / 2) {
                v.push_back(even[i + even.size() / 2]);
            }
            FI(k - even.size() / 2) {
                v.push_back(odd[i]);
            }
        ...
        vector<int> v;
        while (v.size() < k && !even.empty()) {
            v.push_back(even.back());
            even.pop_back();
        }
        while (v.size() < k) {
            v.push_back(odd.back());
            odd.pop_back();
        }
        ask(v);
    }
