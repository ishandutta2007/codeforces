...
int main() {
	...
	while(t--){
		...
		re(s);
		int n = sz(s);
		int alsm = 0,exon = 0,exo,fndsm = 0,smid,smo;
		F0R(i,n){
			cnt[s[i]-'a']+=1;
		}
		vi apr;
		...
		vector<char> cs,cs2;
		F0R(i,smo)cs.pb('a'+smid);
		FOR(i,smid+1,26){
			F0R(j,cnt[i])cs2.pb('a'+i);
		}
		reverse(all(cs));
		reverse(all(cs2));
		if((n-smo)>=smo-2){
			pr(cs.back(),cs.back());cs.pop_back();cs.pop_back();
			while(cs2.size() || cs.size()){
				if(cs2.size()){
					pr(cs2.back());cs2.pop_back();
				}
				if(cs.size()){
					pr(cs.back());cs.pop_back();
				}
			}
			pr('\n');
			continue;
		...
		pr((char)('a'+apr[0]));cnt[apr[0]]-=1;
		pr((char)('a'+apr[1]));cnt[apr[1]]-=1;
		while(cnt[apr[0]]){
			pr((char)('a'+apr[0]));cnt[apr[0]]-=1;
		}
		pr((char)('a'+apr[2]));cnt[apr[2]]-=1;
		F0R(i,26){
			while(cnt[i]){
				pr((char)('a'+i));cnt[i]-=1;
			}
