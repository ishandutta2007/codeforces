...
signed main() {
    ...
    for (int i = 0; i < n; ++i) cnt[a[i]].pb(i);
    vector<vector<int>> b;
    vector<pii> p(5e6 + 1, {-1, -1});
    for (auto i : cnt)
        if (sz(i.s) >= 2) {
            p[i.f * 2] = {i.s[0], i.s[1]};
            b.pb(i.s);
    ...
    if (sz(b) >= 2) {
        println("YES");
        println(b[0][0] + 1, ' ', b[1][0] + 1, ' ', b[0][1] + 1, ' ', b[1][1] + 1);
        return 0;
    }
    if (sz(b) > 0 && sz(b[0]) >= 4) {
        println("YES");
        println(b[0][0] + 1, ' ', b[0][1] + 1, ' ', b[0][2] + 1, ' ', b[0][3] + 1);
        return 0;
    }
    set<int> st;
    set<int> can;
    ...
    for (auto i : can) {
        for (auto j : can)
            if (i < j) {
                ...
                if (was.f != -1) {
                    println("YES");
                    println(was.f + 1, ' ', was.s + 1, ' ', i + 1, ' ', j + 1);
                    return 0;
                }
                was = {i, j};
            }
