...
int n,m;
int l[50],r[50];
Z<mod>f[N],g[N]{1},pre[N]{1};
Z<mod>solve(int a){
    g[0]=1;
    for(int i=1;i<=m/a;i++)g[i]=0;
    for(int i=0;i<n;i++){
        int ll=(l[i]-1)/a+1,rr=r[i]/a;
        pre[0]=g[0];
        for(int j=1;j<=m/a;j++)pre[j]=pre[j-1]+g[j];
        for(int j=m/a;~j;j--){
            g[j]=0;
            if(j-ll>=0)g[j]+=pre[j-ll];
            if(j-rr-1>=0)g[j]-=pre[j-rr-1];
        }
    }
    Z<mod>ret;
    for(int i=0;i<=m/a;i++)ret+=g[i];
    return ret;
}
int main() {
    freopen("A.in","r",stdin);
    I(n,m);
    for(int i=0;i<n;i++)I(l[i],r[i]);
    for(int i=1;i<=m;i++){
        f[i]=solve(i);
    }
    for(int i=m;i;i--){
