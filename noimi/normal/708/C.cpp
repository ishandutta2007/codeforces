...
int main() {
    INT(n);
    Graph g = getG(n);
    vi sub(n), par(n), id(n), id2(n), ans(n, 1);
    {
        int num = 0;
    ...
    RMQ<int> seg(n);
    auto f = [&](int x) { return x <= n / 2 ? x : 0; };
    rep(i, n) seg.set(id[i], f(sub[i]));
    seg.build();
    {
        auto dfs = [&](auto &&self, int x, int p) -> void {
            for(auto e : g[x]) {
                if(sub[e] > n / 2) {
                    int y = 0;
                    if(e == p) {
                        y = max(seg.query(0, id[x]), seg.query(id2[x], n));
                    } else
                        y = seg.query(id[e], id2[e]);
                    ans[x] = sub[e] - y <= n / 2;
                }
            }
            for(auto e : g[x]) {
                if(e != p) {
                    sub[x] -= sub[e];
                    seg.update(id[x], f(sub[x]));
                    sub[e] += sub[x];
                    seg.update(id[e], f(sub[e]));
                    self(self, e, x);
                    sub[e] -= sub[x];
                    seg.update(id[e], f(sub[e]));
                    sub[x] += sub[e];
