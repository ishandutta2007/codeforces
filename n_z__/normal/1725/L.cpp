...
cin>>n;
vector<int>a(n+1);
int s[n+1],t[n+1],mx=-1145141919810;
s[0]=0;
bool b=1;
for(int x=1;x<=n;x++)
cin>>a[x],s[x]=a[x]+s[x-1],t[x]=s[x],b&=(s[x]>=0),mx=max(mx,s[x]);
b&=(s[n]>=mx);
if(!b)ex(-1);
sort(t+1,t+1+n);
int sz=unique(t+1,t+1+n)-t-1;
for(int x=1;x<=n;x++)
s[x]=lower_bound(t+1,t+1+sz,s[x])-t;
int ans=0;
BIT bb;
for(int x=1;x<=n;x++)
ans+=bb.query(s[x]+1,n+5),bb.add(s[x],1);
cout<<ans<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
