...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		int n=ni(),m=ni();
		pair[]bus=new pair[m];
		NavigableMap<Integer, Integer> arr=new TreeMap<Integer, Integer>();
		NavigableMap<Integer, Integer> prearr=new TreeMap<Integer, Integer>();
		//no. of ways to reach at this station
		for(int i=0; i<m; i++)
		{
			bus[i]=new pair(ni(),ni());
		}
		Arrays.sort(bus,new Comparator<pair>()
		{
			@Override
		...
		for(pair bu:bus)
		{
			int te=prearr.floorKey(bu.a-1);
			int allo=prearr.floorKey(bu.b-1);
			prearr.put(bu.b, ((prearr.floorEntry(bu.b).getValue()+prearr.get(allo))%mod-prearr.get(te))%mod);
		}
		if(prearr.containsKey(n))
			pr(((prearr.get(n)-prearr.floorEntry(n-1).getValue())%mod+mod)%mod);
		else
			pr(0);
