...
int main() {
    TEST {
        ...
        rep(i, n) {
            int k = n;
            rep(j, b[i]) {
                if(!empty(idx[j])) chmin(k, idx[j].back());
            }
            if(k != n) chmin(mi, now + (k + seg.prod(k, n) - i));
            if(empty(idx[b[i]])) { break; }
            k = idx[b[i]].back(), idx[b[i]].pop_back();
            now += k + seg.prod(k, n) - i;
            if(now >= mi) break;
            seg.set(k, 1);
            // dump(k, now, mi);
        }
