...
ld dst(const pair<ll, ll> &p1, const pair<ll, ll> &p2) {
...
inline void solve() {
	...
	sort(all(bt), 
		[&](const pair<ll, ll> &p1, const pair<ll, ll> &p2) {
			return dst(p1, u) - dst(p1, b) > dst(p2, u) - dst(p2, b);
		}
	);
	...
	for (int i = 0; i < n; ++i) {
		ans = min({ ans, st - dst(bt[i], u) + dst(bt[i], a), st - dst(bt[i], u) + dst(bt[i], b) });
		if (i == 0 && n > 1) {
			ans = min(ans, st - dst(bt[i], u) + dst(bt[i], a) - dst(bt[1], u) + dst(bt[1], b));
		}
		else if (i > 0) {
			ans = min(ans, st - dst(bt[i], u) + dst(bt[i], a) - dst(bt[0], u) + dst(bt[0], b));
		}
	}
