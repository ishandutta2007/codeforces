...
int query_mx(int v, int l, int r, int a, int b) {
    if (r < a or b < l) return 0;
    if (a <= l and r <= b) return mx[v];

    int m = (l+r)/2;
    return get_max(query_mx(2*v, l, m, a, b), query_mx(2*v+1, m+1, r, a, b));
}

int query_mn(int v, int l, int r, int a, int b) {
    if (r < a or b < l) return n+1;
    if (a <= l and r <= b) return mn[v];

    int m = (l+r)/2;
    return get_min(query_mn(2*v, l, m, a, b), query_mn(2*v+1, m+1, r, a, b));
}
...
void solve() {
    ...
    for (int i = MAXN-1; i >= 1; i--) {
        mn[i] = get_min(mn[2*i], mn[2*i+1]);
        mx[i] = get_max(mx[2*i], mx[2*i+1]);
    }
    
    vector<int> ans(n+1, 1e6);
    ans[n] = 0;

    // cout << query_mn(1, 0, MAXN-1, 4, 5) << endl;

    vector<int> dec = {n};
    vector<int> inc = {n};
    for (int i = n-1; i >= 1; i--) {
        while (!dec.empty() and a[dec.back()] < a[i]) dec.pop_back();
        while (!inc.empty() and a[inc.back()] > a[i]) inc.pop_back();
        
        if (a[i] > a[i+1]) {
            int l = i+1;
            int r = dec.empty() ? n : (dec.back()-1);
            int it = query_mn(1, 0, MAXN-1, l, r);
            ans[i] = 1 + ans[it];
            // dbg(1,  i, l, r, it);
        } else {
            int l = i+1;
            int r = inc.empty() ? n : (inc.back()-1);
            int it = query_mx(1, 0, MAXN-1, l, r);
            ans[i] = 1 + ans[it];
            // dbg(2, i, l, r, it);
        }
        dec.push_back(i);
