...
fun main(){
    solve.cases{
        ...
        val t1 = calculateZarray(target + "#" + A).sliceArray(m+1 until n+m+1)
        val t2 = calculateZarray(target.reversed() + "#" + B.reversed()).sliceArray(m+1 until n+m+1).reversedArray()
        ...
        val count = simpleSegTree(s+1)
        val sum = longSumSimple(s+1)


        var ret = 0L
        var rightpt = -1
        for(l in 0 until n){
            while(rightpt != n-1 && rightpt <= l+s - 3 ){
                rightpt++
                val b = t2[rightpt]
                count.addValue(b,1)
                sum.addValue(b,b.toLong())
            }

            val a = t1[l]
            val atleast = s - 1- a
            val gc = count.query(atleast,s)
            val gsum = sum.query(atleast, s)
            val extra = gsum + gc.toLong() * (a - s + 1)
            ret += extra
