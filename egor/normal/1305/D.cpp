    ...
    void solve() {
        ...
        while (true) {
            ...
            for (auto* e : graph[current]) {
                ...
                for (auto* f : graph[mid]) {
                    other = f->to;
                    if (notRoot[other] || other == current) {
                        other = -1;
                        continue;
                    }
                    break;
            ...
            if (other != -1) {
                out.printLine("?", current + 1, other + 1);
                out.flush();
                int res = in.readInt() - 1;
                for (int i : {current, mid, other}) {
                    if (i != res) {
                        notRoot[i] = true;
                    }
                }
        ...
        for (int i = 0; i < cand.size(); i += 2) {
            out.printLine("?", cand[i] + 1, cand[i + 1] + 1);
            out.flush();
            int res = in.readInt() - 1;
            if (res != current) {
                out.printLine("!", res + 1);
