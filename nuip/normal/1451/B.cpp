...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(_,T){
		...
		cauto q=read();
		cauto s=read<string>();
		cauto ps=readV(q,pii{1,0});
		cauto l0=s.find_first_of('0');
		cauto l1=s.find_first_of('1');
		cauto r0=s.find_last_of('0');
		cauto r1=s.find_last_of('1');
		for(cauto [l,r]:ps){
			cout<<((l0==l || l1==l) && (r0==r-1 || r1==r-1) ? "NO" : "YES") NL;
		}
		// vector<pii> nxt(n);
		// {
		// 	pii p(n,n);
