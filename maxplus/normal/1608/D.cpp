...
ModNum calc() {
  ...
  for (int wl = 0; wl < n; ++wl) {
    if (wl < w[0] || wl < b[1] || n - wl < b[0] || n - wl < w[1]) {
      continue;
    }
    ans += fact(n - w[0] - b[0], wl - w[0]) * fact(n - w[1] - b[1], n - wl - w[1]);
  }
  return ans;
}
...
ModNum calcnowwbb() {
  ...
  for (int wl = 0; wl < n; ++wl) {
    ...
    ans += fact(n - w[0] - b[0], wl - w[0]);
  }
  return ans;
}

ModNum calcnowbbw() {
  bool ok = 1, nowb = 1, nobw = 1;
  for (int i = 0; i < n && ok; ++i) {
    if (a[i][0] == a[i][1]) {
      ok &= a[i][0] == '?';
      continue;
...
int main() {
  ...
  cin >> n >> range{a, n};
  print(calc() - calcnowwbb() + calcnowbbw());
}