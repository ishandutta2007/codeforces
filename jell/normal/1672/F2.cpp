#line 1 "codeforces-workspace\\contest\\1672\\f2\\f2.cpp"
...
#line 13 "codeforces-workspace\\contest\\1672\\f2\\f2.cpp"
...
void main() {
  ...
  auto m = max_element(begin(freq), end(freq)) - begin(freq);

  vector<vector<i32>> g(n);
  for (auto &&[u, v] : zip(a, b)) {
    if (u != m and v != m) {
      g[u].emplace_back(v);
    }
  }

  vector<i32> on(n), visited(n);

  fixed_point dfs = [&](auto dfs, i32 now) -> void {
    if (on[now]) eject("WA");
