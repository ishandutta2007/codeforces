...
int main() {
    ...
    vector dist(2, vector(2, vector<long long>(n, inf)));
    pq_greater<tuple<long long, int, int, int>> pq;

    pq.emplace(dist[0][0][0] = 0, 0, 0, 0);
    while (pq.size()) {
        auto [du, fp, fs, u] = pq.top();
        pq.pop();
        if (dist[fp][fs][u] != du) continue;
        for (const auto &[v, w] : g[u]) {
            if (not (fp or fs)) {
                if (chmin(dist[1][1][v], du + w)) {
                    pq.emplace(dist[1][1][v], 1, 1, v);
                }
            }
            if (not fp) {
                if (chmin(dist[1][fs][v], du + 2 * w)) {
                    pq.emplace(dist[1][fs][v], 1, fs, v);
                }
            }
            if (not fs) {
                if (chmin(dist[fp][1][v], du)) {
                    pq.emplace(dist[fp][1][v], fp, 1, v);
                }
            }
            if (chmin(dist[fp][fs][v], du + w)) {
                pq.emplace(dist[fp][fs][v], fp, fs, v);
            }
        }
    }
