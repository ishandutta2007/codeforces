...
//  freopen("in.in", "w", stdout);
  ...
  int n;gn(n);
  for(int i=1; i<=n; i++) gn(p[i].x,p[i].y);
  n=unique(p,p+n+1)-p-1;
  if(!n){puts("1");return 0;}
  int m=0;
  for(int i=1; i<=n; i++){
    if(max(p[i-1].x,p[i-1].y)<=min(p[i].x,p[i].y)) q[m++]=pii(max(p[i-1].x,p[i-1].y),min(p[i].x,p[i].y));
  }
  n=0;
  for(int i=1; i<m; i++) {
    if(q[i].x<=q[n].y) smax(q[n].y,q[i].y);
    else q[++n]=q[i];
  }
  int ans=0;
  for(int i=0; i<=n; i++) ans+=q[i].y-q[i].x+1;
  println(ans);
