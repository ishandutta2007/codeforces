...
inline void init()
{
    for(int i=2;i<70;i++)
    {
        ...
        for(auto j:prime){int now=i*j;if(now>=70)break;pri[now]=true;if(i%j==0)break;}
    }
    return;
}
...
int n,m;
int gyq[20];
inline void insert(int x){for(int i=18;i>=0;i--){if(!(x>>i))continue;if(!gyq[i]){gyq[i]=x;break;}x^=gyq[i];}return;}
int main()
{
    init();rin>>n;
    ...
    {
        rin>>x,s=0;
        for(int j=0,y;j<19;j++){int cutt=0;for(y=prime[j];x%y==0;x/=y)cutt^=1;if(cutt)s|=(1<<j);}insert(s);
    }
    for(int i=18;i>=0;i--)m+=(gyq[i]>0);
    int ans=ksm(2,n-m)-1;if(ans<0)ans+=M;printf("%d\n",ans);
    return 0;
}