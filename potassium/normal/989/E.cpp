...
template<class T>
vector<T> operator * (vector<T>& x, matrix<T>& y) {
	int n = (int)x.size();
	vector<T> ans(n, 0);
	for (int k = 0; k < n; k++)
		for (int j = 0; j < n; j++)
...
int main() {
	...
	matrix<double> M(n, vector<double>(n, 0));
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < n; j++) {
			if (!trav[j]) {
				...
				pt v = a[j] - a[i];
				for (int k = 0; k < n; k++)
					if (cross(v, a[k] - a[i]) == 0) {
						trav[k] = true;
						idx.emplace_back(k);
					}

				for (auto& k : idx)
					M[k][i] += 1.0 / (int)idx.size();
	...
	const int NUM = (int)lines.size();

	const int MAXLOG = 15;
	vector<matrix<double>> powM(MAXLOG + 1);
	powM[0] = M;
	for (int i = 1; i <= MAXLOG; i++)
		powM[i] = powM[i - 1] * powM[i - 1];
