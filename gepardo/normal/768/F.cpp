...
inline int prefCount(ll stackCnt, ll maxItemCnt, ll minH) {
	maxItemCnt -= stackCnt * minH;
	if (maxItemCnt < 0) {
		return 0;
	} else {
		return divMod(fact[stackCnt + maxItemCnt], 
		              mulMod(fact[stackCnt], fact[maxItemCnt]));
	}
}

inline int count(ll stackCnt, ll itemCnt, ll minH) {
	return subMod(prefCount(stackCnt, itemCnt, minH), 
	              prefCount(stackCnt, itemCnt - 1, minH));
}

int foodStacks[300000], wineStacks[300000], wineStacksTotal[300000];

signed main() {
	precalc();
	int f = get<int>(), w = get<int>(), h = get<int>();
	for (int i = 0; i < 300000; i++) {
		foodStacks[i] = count(i, f, 1);
		wineStacks[i] = count(i, w, h + 1);
		wineStacksTotal[i] = count(i, w, 1);
	}
	int good = 0, total = 0;
	for (int tot = 1; tot < 300000; tot++) {
		int fCnt, wCnt, wCntT;
		// Case #1: food at first
		fCnt = foodStacks[(tot + 1) / 2];
		wCnt = wineStacks[tot / 2];
		wCntT = wineStacksTotal[tot / 2];
		good = addMod(good, mulMod(fCnt, wCnt));
		total = addMod(total, mulMod(fCnt, wCntT));
		// Case #2: wine at first
		fCnt = foodStacks[tot / 2];
		wCnt = wineStacks[(tot + 1) / 2];
		wCntT = wineStacksTotal[(tot + 1) / 2];
		good = addMod(good, mulMod(fCnt, wCnt));
		total = addMod(total, mulMod(fCnt, wCntT));
	}
	putLn(divMod(good, total));
	return 0;
}