...
vector<int> f(LL n, LL k, LL a) {
	//~ [&]() { auto& out = cout; put_stream(out, "n>", make_string_formatter(n), " k>", make_string_formatter(k), " a>", make_string_formatter(a), "\n");  }();
	if(n >= 62) {
		auto ans = f(n - 1, k, a + 1);
		ans.push_back(a);
		return ans;
	}
	if(k >= (1LL << (n - 1))) {
	...
	--i;
	//~ [&]() { auto& out = cout; put_stream(out, "i>", make_string_formatter(i), "\n");  }();
	p -= (1LL << (n - i - 1));
	auto ans = f(n - i, k - p, a + i);
	for(int j = 0; j < i; ++j) {
		ans.push_back(a + j);
	}
	return ans;
