...
const int bases[A] = {(int)31, (int) 37};
/*/-----------------------------Code begins----------------------------------/*/
	...
	for(int i = 1; i < N; ++i){
		for(int j = 0; j < A; ++j){
			...
			for(int k = 0; k < B; ++k){
				...
				cur[j][k] = mul(cur[j][k], bases[j], mod[k]);
				hsh[i][j][k] = cur[j][k];
				req[i][j][k] = add(hsh[i][j][k], req[i - 1][j][k], mod[k]);
			}
		}
	}
}
...
void do_bucket(int x, int c){
	...
	for(auto n: buck[x]){
		...
		for(int i = 0; i < A; ++i){
			for(int j = 0; j < B; ++j){
				val[x][c][i][j] = add(val[x][c][i][j], hsh[oth][i][j], mod[j]);
			}
		}
...
void back_track(int k, int n){
	if (k == 0){
		for(int i = 0; i < A; ++i){
			for(int j = 0; j < B; ++j){
				if (tmp[i][j] != req[n][i][j]){
					return;
				}
			}
		}
	...
	for(int i = 1; i <= k; ++i){
		...
		for(int j = 0; j < A; ++j){
			for(int l = 0; l < B; ++l){
				tmp[j][l] = add(tmp[j][l], val[k][i][j][l], mod[l]);
			}
		}
		...
		for(int j = 0; j < A; ++j){
			for(int l = 0; l < B; ++l){
				tmp[j][l] = sub(tmp[j][l], val[k][i][j][l], mod[l]);
			}
		}
	}
}
