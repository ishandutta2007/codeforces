...
ll N,nextvertex[100001];
ll ne(ll now){
  if(now==nextvertex[now])return now;
  return nextvertex[now]=ne(nextvertex[now]);
}
int main(){
  // startupcpp();
  ...
  if(cnt<N){
    ...
    while(se.size()){
      //
      i=ne(i);
      auto itr=se.upper_bound(i);
      if(itr==se.end())itr=se.begin();
      //
      j=*itr;
      ans.emplace_back(j+1);
      auto nextitr=se.upper_bound(j);
      if(nextitr==se.end())nextitr=se.begin();
      //
      ll k=*nextitr;
      //
      if(gcd(v[i],v[k])>1){
