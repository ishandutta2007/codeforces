...
const int N = 14;
using Darr = std::array<int, 1 << (N / 2 - 1)>;
map<int, Darr> dp[N];
bool e[N][N];
ll ans[1 << N];
int cnt[N][1 << N];
    ...
    for (int mask = 0; mask < (1 << n); ++mask) {
        ...
        do {
            ...
            for (int i = 0; i + 1 < as; ++i) {
                res = (res << 1) | int(e[a[i]][a[i + 1]]);
            }
            cnt[a[0]][res]++;
        } while (next_permutation(a, a + as));
        as = 0;
        FI(n) {
            if (!checkbit(mask, i)) {
                a[as++] = i;
        ...
        do {
            int res = 0;
            for (int i = 0; i + 1 < as; ++i) {
                res = res | (int(e[a[i]][a[i + 1]]) << i);
            }
            cnt[a[0]][res]++;
        } while (next_permutation(a, a + as));
        int h1 = n - h;
        for (int x = 0; x < n; ++x) {
            if (!checkbit(mask, x)) continue;
            for (int y = 0; y < n; ++y) {
                if (checkbit(mask, y)) continue;
                int exy = e[x][y] << (h - 1);
                for (int m = 0; m < (1 << (h - 1)); ++m) {
                    for (int m1 = 0; m1 < (1 << (h1 - 1)); ++m1) {
                        int res = m | (m1 << h) | exy;
                        ans[res] += cnt[x][m] * 1LL * cnt[y][m1];
                    }
                }
            }
        }
