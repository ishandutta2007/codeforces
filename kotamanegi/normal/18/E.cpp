...
void solve(){
                ...
                REP(t, 26) {
                    REP(p, 26) {
                        ...
                        if (i == 0) {
                            ...
                            if (t == q or j == p) continue;
                            if (dp[i][q][j] + cost[i][t][p] < dp[i + 1][t][p]) {
                                dp[i + 1][t][p] = min(dp[i + 1][t][p], dp[i][q][j] + cost[i][t][p]);
                                nexts[i+1][t][p] = mp(q, j);
                            }
                         }
                    }
                }
    ...
    for (int i = n; i >= 1; --i) {
        ...
        REP(q, m) {
            if (q % 2 == 0) {
                now.push_back(itr.first + 'a');
            }
            else {
                now.push_back(itr.second + 'a');
            }
        }
        anss.push_back(now);
        itr = nexts[i][itr.first][itr.second];
    }
    cout << ans << endl;
    reverse(ALL(anss));
    REP(i, anss.size()) {
