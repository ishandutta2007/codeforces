...
void _solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (char c = 'a'; c <= 'c'; ++c) {
            if (c != cur) {
                nxt = min(nxt, make_pair(*pos[c].lower_bound(i), c));
            }
        }

        if (cur + 1 == t[i] && nxt.y == t[i]) {
            assert(pos[cur].erase(i));
            assert(pos[nxt.y].erase(nxt.x));
            pos[cur].insert(nxt.x);
            pos[nxt.y].insert(i);
            continue;
        }
