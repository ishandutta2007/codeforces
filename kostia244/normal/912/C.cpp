...
ll n, m, bounty, increase, damage, mx[100100], reg[100100];
vector<tuple<int, int, int>> a;
vi ind[100100];
bitset<100100> roll = 0;
int main() {
	doin();
	cin >> n >> m >> bounty >> increase >> damage;
	for(int st, i = 0; i < n; i++) {
		cin >> mx[i] >> st >> reg[i];
		if(mx[i] <= damage && increase >0)
			return cout << -1, 0;
		a.pb({0, st, i});
	}
	for(int t, e, h, i = 0; i < m; i++) {
		cin >> t >> e >> h, e--;
		a.pb({t, h, e});
		ind[e].pb(t);
	}
	...
	for(int t, e, h, i = 0; i < a.size(); i++) {
		...
		if(h <= damage) {
			...
			if(reg[e] > 0) {
				int q = 1 + t + (damage - h) / reg[e];
				if(mx[e] > damage && upper_bound(all(ind[e]), q) == upper_bound(all(ind[e]), t)) {
					scan[q-1] += 0;
					scan[q]--;
					roll.set(e, 0);
				}
			} else if(upper_bound(all(ind[e]),t) == ind[e].end() && increase > 0) {
				return cout << -1, 0;
			}
		} else {
			if(roll.test(e)) {
	...
	for(auto i : scan) {
		f_t+=i.second;
		maxq(ans, f_t*(bounty+i.first*increase));
	}
	cout << ans;
}