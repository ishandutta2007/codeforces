...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> a[i];
        for (int j = 1; j <= min(int(q[a[i]-1].size()), 3); j++)
            f.edge(2*q[a[i]-1][q[a[i]-1].size()-j]+1, 2*i, 1, 0);
        for (int j = 1; j <= min(int(q[a[i]+1].size()), 3); j++)
            f.edge(2*q[a[i]+1][q[a[i]+1].size()-j]+1, 2*i, 1, 0);
        for (int j = 1; j <= min(int(q7[a[i]%7].size()), 3); j++)
            f.edge(2*q7[a[i]%7][q7[a[i]%7].size()-j]+1, 2*i, 1, 0);
        f.edge(N-2, 2*i, 1, 0);
        f.edge(2*i, 2*i+1, 1, -1);
        f.edge(2*i+1, N-1, 1, 0);
        q[a[i]].pb(i), q7[a[i]%7].pb(i);
    }
