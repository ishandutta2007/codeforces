...
pair<bool,bool> combine(pair<bool,bool> X, pair<bool,bool> Y){
	pair<bool,bool> ans = {false,false};

	if (X.first){
		ans.first |= (not Y.first);
		ans.second |= (not Y.second);
	}
	if (X.second){
		ans.first |= (Y.first);
		ans.second |= (Y.second);
...
void solve(){
	...
	for(int i = 0; i<n;++i){
		long long s,e;
		cin >> s>>e;
		V.push_back({win(s,e),lose(s,e)});
	}   

	pair<bool,bool> X = V[0];
	for(int i = 1; i< n ; ++i){
