...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) for(_ j = m; j >= 3; --j) {
        _ dx = a[i][j];
        a[i][j-1] += dx;
        a[i][j] -= dx;
        a[i][1] -= dx;
    ...
    map<_p, _> mp;
    for(_ i = 1; i <= n; ++i) ++mp[make_pair(a[i][1], a[i][2])];
    _ dx = 1, cx;
    for(_ i = 2; i <= n; ++i) if(mp[make_pair(a[i][1], a[i][2])] == 1) {
        dx = i;
        break;
    }
    cx = -1;
    for(_ i = 1; i <= n; ++i) if(i != dx) {
        cx = i;
        break;
    }
    writeln(mp(dx, abs(a[cx][2]-a[dx][2])));
