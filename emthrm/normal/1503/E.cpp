...
int main() {
  ...
  REP(c, m - 1) {
    ModInt sum = 0;
    for (int i = n - 1; i >= 0; --i) {
      ans += sum * ModInt::nCk(c + n - (i + 1), c) * ModInt::nCk(c + 1 + i, c + 1);
      sum += ModInt::nCk(m - (c + 2) + i, i) * ModInt::nCk(m - (c + 1) + (n - (i + 1)), n - (i + 1));
    }
  }
  REP(r, n - 1) {
    ModInt sum = 0;
    for (int j = m - 2; j >= 0; --j) {
      ans += sum * ModInt::nCk(r + m - (j + 1), r) * ModInt::nCk(r + 1 + j, r + 1);
      sum += ModInt::nCk(n - (r + 2) + (j + 1), j + 1) * ModInt::nCk(n - (r + 1) + (m - (j + 2)), m - (j + 2));
    }
  }
  cout << ans * 2 << '\n';
  return 0;
