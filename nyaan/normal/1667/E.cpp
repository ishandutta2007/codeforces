...
void q() {
  ...
  int M = N / 2;
  fps f(N + 1);
  rep1(i, M) f[i] = C.inv(i);
  f = f.exp();
  rep(i, N + 1) f[i] *= C.fac(i);
  ...
  reg(i, 2, N + 1) {
    ...
    rep(j, N + 1) {
      // 0 <= j <= M+1-i
      // i+j <= M+1
      // f[i] = C.finv(i-2) sum_j fac(i+j-2) * f[N-i-j] * fac(N-i) / fac(j) / fac(N-i-j) 
      // f[i] = C.finv(i-2) * fac(N-i) sum_{i+j<=M+1} ( fac(i+j-2) * f[N-i-j] / fac(N-i-j) ) / fac(j)
      if (j > M+1-i) break;
      int k = N - i - j;
      trc(i, j, f[k], C(N - i, j));
      cur += C.fac(i + j - 2) * C.finv(i - 2) * f[k] * C(N - i, j);
    }
    ans.push_back(cur * C.fac(i - 1));
  }
  out(ans);
  mint sm=0;
  each(x,ans)sm+=x;
  ...
  */
  ...
  fps t(M+2),u(M+1);
  reg(i,2,M+2)t[i]=C.fac(i-2)*f[N-i]*C.finv(N-i);
  rep(i,M+1)u[i]=C.finv(i);
  trc(t,u);
  // f[i] = C.finv(i-2) * fac(N-i) sum_{j} t(i+j) u(j)
  u=u.rev();
  fps s=t*u;
  
  trc(s);
  fps g(N+1);
  reg(i,2,M+2)g[i]=C.finv(i-2)*C.fac(N-i)*s[i+M]*C.fac(i-1);
  g[1]=f[N-1];
  g.erase(begin(g));
  out(g);
