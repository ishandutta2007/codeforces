...
void perform(vi ops,vi&v)
{
    ...
    {
        //cout<<last<<" "<<ops.back()<<'\n';
        for(int i=last-ops.back();i<last;i++)
        {
            x.pb(v[i]);
        }
        last=last-ops.back();
...
//int cnt=0;
  ...
  //      cnt++;
        ...
        for(int i=0;i<n;i++)
        {
            if(v[i]!=i+1) {firstindex=i;break;}
        }
        if(firstindex==-1) break;
        for(int i=0;i<n;i++)
        {
            if(v[i]==v[firstindex]-1)
            {
                secondindex=i;
                break;
            }
        ...
        {
            if(v[i]==v[i-1]+1) group2++;
            else break;
            //cout<<i<<endl;
        }
        group2++;
        group4=n-secondindex-1;
        group3=n-group1-group2-group4;
        //printvector(thisstep);
        //cout<<endl;
        vi temp;
        if(group1) thisstep.pb(group1);
        if(group2) thisstep.pb(group2);
        if(group3) thisstep.pb(group3);
        if(group4) thisstep.pb(group4);
        op.pb(thisstep);
        perform(thisstep,v);
        //printvector(v);
        //if(cnt==3)
