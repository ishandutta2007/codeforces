...
bool checkLine(int x1, int y1, int x2, int y2)
{
	...
	{
		if (y1 < y2) swap(y1, y2);
		return hor[x1+1][y1 + 1] == hor[x1+1][y2];
	}
	else if (y1 == y2)
	{
		if (x1 < x2) swap(x1, x2);
		return ver[x1 + 1][y1 + 1] == ver[x2][y1 + 1]; 
	}
	else 
	{
		if (x1 < x2) swap(x1, x2), swap(y1, y2);
		if (y1 > y2)
			return ddo[x1 + 1][y1 + 1] == ddo[x2][y2];
		else
		{
			return dup[x1 + 1][y1 + 1] == dup[x2 + 2 ][y2];
		}
	}
}
bool check(int x1, int y1, int x2, int y2, int x3, int y3)
{
	if (x1 < 0 || y1 < 0 || x2 < 0 || y2 < 0 || x3 < 0 || y3 < 0 ||
		x3 >= n || x1 >= n || x2 >= n || y3 >= m || y1 >= m || y2 >= m) return false;
	return checkLine(x1, y1, x2, y2) && checkLine(x1, y1, x3, y3) && checkLine(x2, y2, x3, y3);
}
int main()
{
	...
	FI(n)
	{
		FJ(m)
		{
			hor[i + 1][j + 1] = hor[i + 1][j] + (s[i][j] == '1');
			ver[i + 1][j + 1] = ver[i][j + 1] + (s[i][j] == '1');
			ddo[i + 1][j + 1] = ddo[i][j] + (s[i][j] == '1');
			dup[n - i][j + 1] = dup[n - i + 1][j] + (s[n - i - 1][j] == '1');
		}
	}
	int ans = 0;
	FI(n)
		...
		FJ(m)
		{
			if (s[i][j] == '0')
			{
				for (int k = i+1; k < n; ++k)
				{
					if (s[k][j] == '1') break;
					int d = k - i;
					if (j + d < m && checkLine(i, j, i, j + d) && checkLine(k, j, i, j + d)) ++ans;
					if (j - d >= 0 && checkLine(i, j, i, j - d) && checkLine(k, j, i, j - d)) ++ans;
					if (j + d < m && checkLine(i, j, k, j + d) && checkLine(k, j, k, j + d)) ++ans;
					if (j - d >= 0 && checkLine(i, j, k, j - d) && checkLine(k, j, k, j - d)) ++ans;
					if ((i+k) % 2 == 0)
					{
						ans += check(i, j, k, j, (i + k) / 2, j + d / 2);
						ans += check(i, j, k, j, (i + k) / 2, j - d / 2);
					}
				}
			}
