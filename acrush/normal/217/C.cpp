...
int ID[2][2]={{4,8},{16,32}};

int calc(int s1,char key,int s2)
{
	if (key=='&') return s1&s2;
	if (key=='|') return s1|s2;
	if (key=='^') return s1^s2;
...
int solve(int s1,char key,int s2)
{
	int R=0;
	for (int b1=0;b1<2;b1++) if (contain(s1,b1))
		for (int b2=0;b2<2;b2++) if (contain(s2,b2))
			R|=two(calc(b1,key,b2));
	for (int x1=0;x1<2;x1++) for (int y1=0;y1<2;y1++) if (s1&ID[x1][y1])
		for (int x2=0;x2<2;x2++) for (int y2=0;y2<2;y2++) if (s2&ID[x2][y2])
		{
			int old_val=calc(x1,key,x2);
			int new_val=calc(y1,key,y2);
			R|=ID[old_val][new_val];
		}
	return R;
}
int getres(int s)
{
	if ((s&3)!=3) return false;
	return (s&ID[0][1]) || (s&ID[1][0]);
}
int main()
{
#ifdef _MSC_VER
