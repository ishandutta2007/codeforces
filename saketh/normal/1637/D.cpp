...
void solve() {
    ...
    vector<int64_t> dp(MAXS, INF), DP = dp;
    ...
    F0R (i, N) {
        ...
        F0R (sum, MAXS) {
            ...
            ckmin(DP[sum + a[i]], dp[sum] + int64_t(sum) * a[i] + int64_t(psum - sum) * b[i]);
            ckmin(DP[sum + b[i]], dp[sum] + int64_t(sum) * b[i] + int64_t(psum - sum) * a[i]);
        }
    ...
    int64_t ans = 2 * *min_element(all(dp));

    F0R (i, N) {
        ans += int64_t(a[i]) * a[i] * (N - 1);
        ans += int64_t(b[i]) * b[i] * (N - 1);
    }
