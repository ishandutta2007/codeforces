...
vector<ll> dp(int l, int r) {
    ...
    int k = min_element(a + l, a + r + 1) - a;

    const auto &L = dp(l, k - 1), &R = dp(k + 1, r);

    vector<ll> f(r - l + 2, INT64_MIN);

    For(i, 0, SZ(L)) For(j, 0, SZ(R)) {

        cmax(f[i + j], L[i] + R[j] - 2ll * a[k] * i * j);

        cmax(f[i + j + 1], L[i] + R[j] + a[k] * (m - 2ll * (i * j + i + j) - 1));

    }

    // printf("%d %d\n", l, r);

    // rep(i, 0, r - l + 1) printf("%lld ", f[i]);
