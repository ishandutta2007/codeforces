#line 1 "codeforces-workspace\\contest\\1667\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1667\\b\\b.cpp"
...
#line 30 "codeforces-workspace\\contest\\1667\\b\\b.cpp"
...
void main() {
  ...
  compression<i64> comp(begin(a), end(a));

  constexpr i32 inf = 1 << 30;

  struct mono {
    int minus = -inf, plus = -inf, zero = -inf;
    mono operator+(mono x) const {
      return {max(minus, x.minus), max(plus, x.plus), max(zero, x.zero)};
    }
  };

  segment_tree<mono> sgt(comp.size());
  i32 dp = 0;
  sgt[comp.lower_bound(0)] = {0, 0, 0};

  for (auto pos : range(1, a.size())) {
    auto x = comp.lower_bound(a[pos]);
    auto minus = sgt.fold(0, x).minus;
    auto plus = sgt.fold(x + 1, sgt.size()).plus;
    auto zero = sgt[x].zero;
    dp = max({zero, minus + pos, plus - pos});
    sgt[x] = sgt[x] + mono{dp - pos, dp + pos, dp};
  }
