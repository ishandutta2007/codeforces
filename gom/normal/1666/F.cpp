...
using mi=modnum<mod>;
const int N=5005,M=5e5+5,K=2005;
int n,a[N],c;
mi dp[2][N],fact[N],finv[N],ans;
mi ncr(int n,int r){
...
void solve(){
	...
	c=0; dp[0][1]=1;
	for(int i=2;i<=n;i++,c=1-c){
		int j=i;
		while(j+1<=n&&a[i]==a[j+1]) j++;
		for(int j=0;j<=n/2;j++) dp[1-c][j]=0;
		int d=j-i+1;
		for(int j=(d+i)/2;j<n/2;j++) dp[1-c][j+1]+=dp[c][j]*ncr(2*j-i,d-1);
		for(int j=(d+i+1)/2;j<n/2;j++) dp[1-c][j]+=dp[c][j]*ncr(2*j-i,d);
		dp[1-c][n/2]+=dp[c][n/2]*ncr(n-i+1,d);
		i=j;
	}
	cout<<dp[c][n/2]<<"\n";
}
int main(){
	...
	for(int i=0;i<=5000;i++) finv[i]=1/fact[i];
	int t;
	cin>>t;
	while(t--) solve();
	return 0;
