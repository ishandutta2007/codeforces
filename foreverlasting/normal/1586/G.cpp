...
namespace MAIN{
    ...
    inline void modify(const res &x,const res &y){
        for(res i=x;i;i-=lowbit(i))add(tr[i],y);
    }
    inline int query(const res &x){
        res ret=0;
        for(res i=x;i<=N-10;i+=lowbit(i))add(ret,tr[i]);
        return ret;
    }
    int o[N];
    int vis[N];
    ...
    inline void MAIN(){
        ...
        sort(a+1,a+n+1);
        res T=read();
        for(res i=1;i<=T;i++)vis[lower_bound(a+1,a+n+1,mp(o[read()],0))-a]=1;
        modify(2*n+2,1);
        for(res i=1;i<=n;i++)f[i]=query(a[i].se),modify(a[i].se,f[i]);
        for(res i=n,j=1;i;i--){
            modify(a[i].se,kcz-f[i]);
            if(vis[i]&&a[i].se>=j)add(ans,query(j)),j=a[i].se;
        }
        printf("%d\n",ans);
    }
}
