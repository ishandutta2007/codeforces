...
void solve() {
  int n, q; string s, f; cin >> n >> q >> s >> f;
  vector<int> l(q), r(q); REP(i, q) cin >> l[i] >> r[i], --l[i], --r[i];
  LazySegmentTree<monoid::RangeSumAndUpdateQuery<int>> seg(monoid::RangeSumAndUpdateQuery<int>::init(n));
  REP(i, n) seg.set(i, {f[i] - '0', 1});
  for (int i = q - 1; i >= 0; --i) {
    int o = seg.get(l[i], r[i] + 1).sum, z = r[i] - l[i] + 1 - o;
    if (z == o) {
      cout << "NO\n";
      return;
    }
    seg.apply(l[i], r[i] + 1, o > z);
