    ...
    solve.cases{
        ...
        for(i in 0 until n){
            for(j in 0 until m){
                ...
                val now = mutableListOf<Pair<Int,Int>>()
                for(d in 0 until 4){
                    if(!arr[i+dx[d]][j + dy[d]]){
                        if(now.isEmpty()){
                            now.add(Pair(i+dx[d], j+dy[d]))
                        }else{
                            val (a,b) = now.removeLast()
                            val c1 = code(a,b)
                            val c2 = code(i + dx[d], j + dy[d])
                            assert(dsu.union(c1,c2,true))
                        }
                    }
                }
            }
        ...
        for(i in 0 until n){
            for(j in 0 until m){
                if(arr[i][j]) continue
                ret[i][j] = if(dsu.find(code(i,j)).second) 1 else 4
            }
        }
        for(i in 0 until n){
            for(j in 0 until m){
