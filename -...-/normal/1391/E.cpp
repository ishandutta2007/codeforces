...
#define N 500005
vector<lld> adj[N], levelwise[N];
lld level[N], path[N], n;
bool visited[N], printed;
...
void dfs(lld x)
{
	...
	visited[x]=1;
	levelwise[level[x]].pb(x);
	if(level[x]>=lld(ceil(n/2.0)))
	{
		printed=1;
		lld curr=x;
		cout<<"PATH\n"<<level[x]<<endl;
...
void swagWaalaFunction()
{
	...
	if(!printed)
	{
		...
		FORZ(i,n+1)
			if(levelwise[i].size()>1)
				FORZI(j,levelwise[i].size()-1,2)
						cout<<levelwise[i][j]<<" "<<levelwise[i][j+1]<<endl;
	}
	return;
}
