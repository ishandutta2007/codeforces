...
void dfs(graph &g, int s,int i=0){
    int N = g.G.size();
    if(i==0){
...
int main(){
    ...
    if(logN==0){
        ll kk=1;
        while(kk<=G.G.size()){ kk*=2; logN++; }
        logN++;
    }
    anc.assign(N+1,{}); depth_lca.assign(N+1,-1);
    anc[1].assign(logN,make_pair(-1,-1)); depth_lca[1] = 0; //s: 
    lcapre(G,1); //s: 
    //
