...
vector<pll> solve(const vector<int> &vs, const int m){
	vector<pll> re;
	for(cauto &v:vs){
		auto u=v;
		int cnt=1;
		while(u%m==0) u/=m, cnt*=m;
		if(re.empty() || re.back().X!=u) re.eb(u,0);
		re.back().Y+=cnt;
	}
	return re;
}
...
int main(){
	...
	TT{
		...
		cauto m=read(0);
		cauto vs=readV(n,0);
		cauto us=readV(read(0),0);
		cout<<(solve(vs,m)==solve(us,m)?"Yes":"No") NL;
	}
	return 0;
}