...
vector<int> onHeight[maxn];
vector<int> occurences[maxn][26];

#define prev afsjjsfa
int g[maxn], prev[maxn];
int last[maxn], sz;
...
void dfs(int v, int h = 0) {
    tin[v] = ++timer;
    onHeight[h].push_back(tin[v]);
    occurences[h][c[v]].push_back(tin[v]);
...
vector<int>::iterator itL, itR, b, e;
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        itL = lower_bound(onHeight[h].begin(), onHeight[h].end(),
                              tin[v]);
        itR = upper_bound(onHeight[h].begin(), onHeight[h].end(),
                              tout[v]);

        if (itL != onHeight[h].end() && itR != onHeight[h].begin()) {
            int l = itL - onHeight[h].begin();
            int r = itR - onHeight[h].begin() - 1;

            int cntodd = 0;
            for (int i = 0; i < 26; i++) {
                b = occurences[h][i].begin();
                e = occurences[h][i].end();
                itL = lower_bound(b, e,
                                  tin[v]);
                itR = upper_bound(b, e,
                                    tout[v]);
