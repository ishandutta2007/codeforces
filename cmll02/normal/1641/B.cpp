...
signed main()
{
	...
	gr
	int end=0;
	for(auto x:mp)if(x.second&1)end=1;
	if(end){puts("-1");continue;}
	// puts("?");
	int cur=1,D=0,cc=0;tt=0;
	for(;cur<=n;)
	{
		for(int j=cur+1;j<=n;j++)
		{
			if(a[cur]==a[j])
			{
				...
				// printf("get %d %d, len = %d\n",cur,j,len);fflush(stdout);
				for(int k=1;k<len;k++)f(j+k-1+D,a[cur+k]);
				std::reverse(a+cur+1,a+j+1);
				cur+=2;
				D+=len*2-2;
				t[++cc]=len;
				break;
	...
	odl(tt);
	rg(tt)odp(opx[i],opy[i]);gr
	odl(cc);
	rg(cc)odb(t[i]<<1);gr puts("");
	// puts("");
	gr
	return 0;
}