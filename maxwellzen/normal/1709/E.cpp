...
int dfs2(int i, int p) {
    ...
    for (int nb : adj[i]) if (nb!=p) {
        ans += dfs2(nb, i);
        if ((int)prefs[nb].size() > mx) {
            mx = prefs[nb].size();
            child = nb;
        } else debug(mx, prefs[nb].size());
    }
    debug(i, mx, child);
    if (child==-1) {
        prefs[i].insert(pre[i]);
        debug(i, "leaf");
        return 0;
    }
    if (prefs[child].find(pre[i]^a[i]) != prefs[child].end()) {
        debug(i, child);
        return ans+1;
    }
    swap(prefs[i], prefs[child]);
    prefs[i].insert(pre[i]);
    for (int nb : adj[i]) if (nb != p && nb != child) {
        for (int x : prefs[nb]) {
            if (prefs[i].find(x^a[i]) != prefs[i].end()) {
                debug(i, child, nb);
                prefs[i].clear();
                return ans+1;
            }
        }
        for (int x : prefs[nb]) prefs[i].insert(x);
    }
    debug(i, prefs[i]);
    return ans;
}
