...
main() {
...
#ifdef _LOCAL
        int res = 0;
        if(x <= x11 and x12 <= xx and y <= y11 and y12 <= yy) res++;
        if(x <= x21 and x22 <= xx and y <= y21 and y22 <= yy) res++;
        return res;
#endif
        cout << "? " << x << " " << y << " " << xx << " " << yy << endl;
        cout.flush();
    ...
    rep(i, 2) {
        ...
        a[1 ^ i] = l[1], b[1 ^ i] = r[1];
        if(l[1] > r[1]) {
            x[i].eb(l[0], r[1]), x[i].eb(l[1], r[0]);
        } else if(ask(a[0], a[1], b[0], b[1]) >= 1)
            x[i].eb(l[1], r[1]), x[i].eb(l[0], r[0]);
        else
            x[i].eb(l[1], r[0]), x[i].eb(l[0], r[1]);
    }
    swap(x[0], x[1]);
    sort(all(x[1]), [](pii a, pii b) { return a.se - a.fi < b.se - b.fi; });
    if(ask(x[0][0].fi, x[1][0].fi, x[0][0].se, x[1][0].se) == 1) {
        cout << "! " << x[0][0].fi << " " << x[1][0].fi << " " << x[0][0].se << " " << x[1][0].se << " ";
        cout << x[0][1].fi << " " << x[1][1].fi << " " << x[0][1].se << " " << x[1][1].se << endl;
        cout.flush();
    } else {
        cout << "! " << x[0][0].fi << " " << x[1][1].fi << " " << x[0][0].se << " " << x[1][1].se << " ";
        cout << x[0][1].fi << " " << x[1][0].fi << " " << x[0][1].se << " " << x[1][0].se << endl;
        cout.flush();
    }
}