	...
	void add(int v, int l, int r, int x, int val)
	{
		...
		else
		{
			...
			if (x <= m)
				...
				add(2 * v + 2, m + 1, r, x, val);
			sum[v] = sum[2 * v + 1] + sum[2 * v + 2];
			lm[v] = max(lm[2 * v + 2], sum[2 * v + 2] + lm[2 * v + 1]);
		}
	}
	int find_money(int v, int l, int r, int val)
	{
		if (lm[v] < val)
			return -1;
		if (val <= 0)
		...
		int m = (l + r) / 2;
		if (lm[2 * v + 2] >= val)
			return find_money(2 * v + 2, m + 1, r, val);
		else
			return find_money(2 * v + 1, l, m, val - sum[2 * v + 2]);
	}
};
