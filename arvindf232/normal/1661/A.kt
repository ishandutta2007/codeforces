...
fun main(){
    solve.cases{
        ...
        for(i in 1 until n){
            val now = (A[i] - A[i-1]).absoluteValue + (B[i] - B[i-1]).absoluteValue
            val other = (A[i] - B[i-1]).absoluteValue + (B[i] - A[i-1]).absoluteValue
            if(other < now){
        ...
        var ans = 0L
        for(i in 1 until n){
            ans += (A[i] - A[i-1]).absoluteValue
            ans += (B[i] - B[i-1]).absoluteValue
        }
        put(ans)
