...
li solve(){
  ...
  vi A(n); for(li i = 0; i < (li)(n); ++i) { cin>>A[i]; A[i] %= 2; }
  vi OS;
  for(li i = 0; i < (li)(n); ++i) if(A[i]) OS.emplace_back(i);

  li ans = -1;
  for(li d = 0; d < (li)(2); ++d) {
    vi GOAL;
    for(li i = 0; i < (li)(n); ++i) if((i&1) == d) GOAL.emplace_back(i);

    if(GOAL.size() == OS.size()) {
      li cur = 0;
      for(li i = 0; i < (li)(GOAL.size()); ++i) cur += abs(GOAL[i]-OS[i]);
...
int main(){
  ...
  int t; cin>>t; for(li i = 0; i < (li)(t); ++i) { li ans = solve(); cout << ans << endl; }
  return 0;
}