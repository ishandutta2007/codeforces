...
vector<ll> diameter(ll from){
    vector<ll>cost(n,LINF);
    rep(n)if(!exist[i])cost[i]=-LINF;
    cost[from]=0;
    ll d1;
    queue<ll>q;
    q.push(from);
    while(q.size()){
        d1=q.front();
        q.pop();
        each(i,g[d1])if(exist[i]&&chmin(cost[i],cost[d1]+1))q.push(i);
    }
    cost.assign(n,LINF);
    rep(n)if(!exist[i])cost[i]=-LINF;
    vec(ll,back,n);
    ...
    while(q.size()){
        d2=q.front();
        q.pop();
        each(i,g[d2])if(exist[i]&&chmin(cost[i],cost[d2]+1)){
            q.push(i);
            back[i]=d2;
        }
    }
...
signed main(){
    ...
    while(cnt>1){
        auto d=diameter(l);
        l=lca(d.front(),d.back());
        each(i,d)if(exist[i]&&i!=l){
            exist[i]=0;
            cnt--;
        }
        vector<ll>cost(n,LINF);
    ...
    answer(max_element(all(exist))-exist.begin());
}