...
int cal(int x,int y){
  ...
  while(1){
    assert(x>=res);
    assert(y>=res);
    assert(y+res<g[0].size());
    if(g[x-res][y-res]=='.'||g[x-res][y+res]=='.')break;
    ++res;
  }
  // debug(x,y,res);
  return res;
...
int codeforces;cin>>codeforces;while(codeforces--){
  ll i,j,N,M,K;
  cin>>N>>M>>K;
  g.assign(N+1,string(M+2,'.'));
  mat ch(N,vec(M,0));
  rep(i,N){
    string s;
    cin>>s;
    rep(j,M)g[i+1][j+1]=s[j];
  }
  bool f=true;
  brep(i,N)rep(j,M){
    if(g[i+1][j+1]=='*'){
      auto cnt=cal(i+1,j+1);
      if(!ch[i][j]&&cnt<=K)f=false;
      if(cnt>K){
        while(cnt--)ch[i-cnt][j-cnt]=ch[i-cnt][j+cnt]=1;
      }
    }
  }
  if(f)printYES;
