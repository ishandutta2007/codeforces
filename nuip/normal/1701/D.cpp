...
int inv(const int i, const int v){
	int l=0,r=MOD;
	while(r-l>1){
		cauto m=(l+r)/2;
		(i/m>=v?l:r)=m;
...
int main(){
	...
	TT{
		...
		cauto vs=readV(n,0);
		vector<pii> ints;
		rep(i,n) ints.eb((i+1)/(vs[i]+1),vs[i]?(i+1)/vs[i]:n);
		out(ints,1);
		vv<int> ads(n+1);
		rep(i,n) ads[ints[i].X].eb(i);
		set<pii> st;
		vector<int> re(n);
		rep(i,n+1){
			if(i){
				re[st.begin()->Y]=i-1; st.erase(st.begin());
			}
			for(cauto &j:ads[i]) st.emplace(ints[j].Y,j);
			while(st.size() && st.begin()->X==i) st.erase(st.begin());
		}
		out(re,1);
