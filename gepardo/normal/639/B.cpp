...
{
    int n = in, d = in, h = in;
    if (h > d && h >= n && d >= n) { out = -1; return 0; }
    if (h + h < d) { out = -1; return 0; }
    if (d == h && n != d + 1)
    {
        if (d == 1) { out = -1; return 0; }
        for (int i = 1; i <= h; i++) out.putLn(i, ' ', i + 1);
        for (int i = h + 2; i <= n; i++) out.putLn(2, ' ', i);
        return 0;
    }
    int p2 = d - h;
    for (int i = 1; i <= h; i++) out.putLn(i, ' ', i + 1);
    ...
    {
        out.putLn(1, ' ', h + 2);
        for (int i = 1; i < p2; i++) out.putLn(i + h + 1, ' ', i + h + 2);
        for (int i = h + p2 + 1; i < n; i++) out.putLn(1, ' ', i + 1);
    }
    return 0;
}