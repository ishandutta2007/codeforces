...
int f[1000005],tt[1000005];
int dp[1000005],way[1000005];
signed main()
{
	initprog();
	int T=read();
	rg(T)
	int n=read();rg(n)t[i]=tt[i]=dp[i]=way[i]=0;gr
	rg(n)a[i]=read();t[a[i]]++;gr
	int cc=0;
	rg(n)if(t[i])tt[t[i]]++;gr
	int g=0;
	rg(n)if(tt[i])f[++cc]=i,g=std::__gcd(g,i);gr
	if(g==n)
	{
		puts("1");
		// puts("");
	...
	for(int i=1;i<=n;i++)
	{
		...
		for(int j=1;j<=cc;j++)
		{
			//
			// odl(f[j]);
			way[i]=me(way[i],qp(inv[f[j]/(n/i)],tt[f[j]]));
		}
		for(int j=1;j<=cc;j++)
		{
			int q=f[j];
	...
	for(int i=1;i<=n;i++)
	{
		...
		{
			if(n%j||j%g)continue;
			// mde(way[j],me(inv[j/i],));
			mde(dp[j],me(rnv[j/i],dp[i]));
		}
	}
	//
	//
	// odl(me(ways,1));
	// odl(de(me(n,ways),ans));

	// ways=1;
	odl(de(n,ve(ans,ways)));
	// puts("");
	gr
	return 0;
}