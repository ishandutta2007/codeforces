...
void solve() {
    ...
    for (int i = 1; i <= k; i++) {
        for (int j = 1; j <= n; j++) (path[i][j] = path[i - 1][j - 1] + path[i - 1][j + 1]) %= base;
    }
    for (int i = 0; i <= k; i++) {
        if (i == 0) {
            for (int i = 1; i <= n; i++) f[i] = 1;
        } else {
            for (int i = 1; i <= n; i++) g[i] = (f[i - 1] + f[i + 1]) % base;
            for (int i = 1; i <= n; i++) f[i] = g[i];
        }
        for (int j = 1; j <= n; j++) (w[j] += f[j] * path[k - i][j]) %= base;
    }
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) (ans += w[i] * a[i]) %= base;
