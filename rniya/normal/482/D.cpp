...
int main() {
    ...
    auto dfs = [&](auto self, int v) -> void {
        vector<vector<vector<mint>>> cur(3, vector<vector<mint>>(2, vector<mint>(2, 0)));
        vector<vector<vector<mint>>> nxt(3, vector<vector<mint>>(2, vector<mint>(2, 0)));
        cur[2][1][0] = 1;
        for (int& u : G[v]) {
            self(self, u);
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 2; j++) {
                    for (int k = 0; k < 2; k++) {
                        nxt[i][j][k] += cur[i][j][k];
                        for (int l = 0; l < 2; l++) {
                            nxt[i == 2 ? l : i][j & (i == 2 || l == i)][k ^ l] += cur[i][j][k] * dp[u][l];
                        }
                    }
                }
            }
        ...
        cur[(G[v].size() ? 0 : 2)][1][0] -= 1;
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 2; j++) {
                for (int k = 0; k < 2; k++) {
                    dp[v][k ^ 1] += cur[i][j][k] * (i == 2 || (j && (i == 0 || k == 1)) ? 1 : 2);
                    cur[i][j][k] = 0;
                }
            }
        }
