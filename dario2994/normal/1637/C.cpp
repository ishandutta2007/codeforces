...
void solve() {
    ...
    for (int i = 1; i < n-1; i++) {
        ...
        odd += a[i]%2;
        ones += a[i] == 1;
        even_big += a[i] > 1 and (a[i]%2 == 0);
        odd_big += a[i] > 1 and (a[i]%2 == 1);
    }
    if (sum > 0 and even_big == 0 and odd_big == 0) cout << -1 << "\n";
    else if (sum == 3 and odd_big) cout << -1 << "\n";
    else if (even_big or ones or odd_big > 1) cout << (sum+odd)/2 << "\n";
    else cout << (sum + odd + 2) / 2 << "\n";
}
