...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		int[][]udig=new int[2*n+1][2];
		int[][]ddig=new int[2*n+1][2];
		for(int i=0; i<=n; i++)
		{
			rows[i][0]=10000000;
			rows[i][1]=-10000000;
		...
		for(int i=0; i<m; i++)
		{
			rows[pairs[i][0]][0]=Math.min(pairs[i][1], rows[pairs[i][0]][0]);
			rows[pairs[i][0]][1]=Math.max(pairs[i][1], rows[pairs[i][0]][1]);
			cols[pairs[i][1]][0]=Math.min(pairs[i][0], cols[pairs[i][1]][0]);
			cols[pairs[i][1]][1]=Math.max(pairs[i][0], cols[pairs[i][1]][1]);
			udig[pairs[i][0]+pairs[i][1]][0]=Math.min(pairs[i][0]-pairs[i][1], udig[pairs[i][0]+pairs[i][1]][0]);
			udig[pairs[i][0]+pairs[i][1]][1]=Math.max(pairs[i][0]-pairs[i][1], udig[pairs[i][0]+pairs[i][1]][1]);
			ddig[pairs[i][0]-pairs[i][1]+n][0]=Math.min(pairs[i][0]+pairs[i][1], ddig[pairs[i][0]-pairs[i][1]+n][0]);
			ddig[pairs[i][0]-pairs[i][1]+n][1]=Math.max(pairs[i][0]+pairs[i][1], ddig[pairs[i][0]-pairs[i][1]+n][1]);	
		}
		int[]ans=new int[9];
		for(int i=0; i<m; i++)
		{
			int te=0;
			if(rows[pairs[i][0]][0]<pairs[i][1]) te++;
			if(rows[pairs[i][0]][1]>pairs[i][1]) te++;
			if(cols[pairs[i][1]][0]<pairs[i][0]) te++;
			if(cols[pairs[i][1]][1]>pairs[i][0]) te++;
			if(udig[pairs[i][0]+pairs[i][1]][0]<pairs[i][0]-pairs[i][1]) te++;
			if(udig[pairs[i][0]+pairs[i][1]][1]>pairs[i][0]-pairs[i][1]) te++;
			if(ddig[pairs[i][0]-pairs[i][1]+n][0]<pairs[i][0]+pairs[i][1]) te++;
			if(ddig[pairs[i][0]-pairs[i][1]+n][1]>pairs[i][0]+pairs[i][1]) te++;
			ans[te]++;
		}
		pra(ans);
