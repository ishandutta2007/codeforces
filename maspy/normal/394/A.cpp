...
void solve() {
  ...
  auto make = [&](ll a, ll b, ll c) -> string {
    string S;
    FOR(_, a) S += "|";
    S += "+";
    FOR(_, b) S += "|";
  ...
  if (a + b == c) return print(S);
  if (a > 1 && a - 1 + b == c + 1) return print(make(a - 1, b, c + 1));
  if (b > 1 && a + b - 1 == c + 1) return print(make(a, b - 1, c + 1));
  if (c > 1 && a + 1 + b == c - 1) return print(make(a + 1, b, c - 1));
  print("Impossible");
}
