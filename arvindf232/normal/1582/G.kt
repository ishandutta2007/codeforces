...
fun main(){
    ...
    solve.cases{
        ...
        val L = getline(n)
        val str = getstr
        val have = BooleanArray(n){str[it] == '*'}

        var id = 0
        val primeID = IntArray(sieve.sieveMx)
        for(p in 0 until sieve.sieveMx){
            if(sieve.primeOf[p] == p){
                primeID[p] = id
                id ++
            }
        }
        val st = simpleMaxTree(IntArray(id){n})
        val now = IntArray(id)
        val all = Array(id){ArrayDeque<pii>()}
        for(a in all){
            a.add(pii(nlarge,n+1))
            a.add(pii(0,n))
        }
        var ret = 0L
        for(i in n-1 downTo 0 ){
            ...
            v.eachPrimePower { p, e ->
                now[primeID[p]] += if(adding) -e else e
                val S = all[primeID[p]]
                S.last().i = i + 1
                while(S.isNotEmpty() && S.last().v >= now[primeID[p]]){
                    S.removeLast()
                }
                val best = S.last().i - 1
                st.setValue(primeID[p],best)
                S.addLast(pii(now[primeID[p]],i))
            }
            val lasttill = st.arr[1]
//            i dei lasttill
            ret += lasttill - i
        }
        put(ret)
