...
data class edge (val x:Int, val y:Int, val id:Int){
}
...
//        val Q = fastDeque(0,n)
    ...
    inline fun BFS(distRoot:Int, tar:Int): Int {
        Q.reset()
...
fun main(){
    solve.cases{
        ...
        val bad = mutableListOf<edge>()
//        val head = Array<edge?>(n){null}
        var forbidden:Pair<Int,Int> = Pair(-1,-1)
        repeat(m){
            all.add(edge(getint - 1 , getint - 1,it))
        }
        for(e in all){
            val (a,b) = order(e.x,e.y)
            if(!dsu.union(a,b)) {
                bad.add(e)
            }else{
                G.addWeighted(a,b,e.id)
            }
//            val (a,b) = order(e.x,e.y)
//            if(a == forbidden.first && b== forbidden.second){
//                G.BFS(a)
////                assert(head[a] != null)
////                bad.add(head[a]!!)
//                break
...
//                if(bad.size == 2){
//                    if(a == bad[0].x){
//                        forbidden = order(b, bad[0].y)
//                    }else if(a == bad[0].y){
//                        forbidden = order(b,bad[0].x)
//                    }else if(b == bad[0].x){
//                        forbidden = order(a,bad[0].y)
//                    }else if(b == bad[0].y){
//                        forbidden = order(a,bad[0].x)
//                    }
//                }else if(bad.size == 3){
//                    break
//                }
...
//            }
        ...
        if(bad.size == 3){
            ...
            if(S.values.all { it == 2 }){
                val r = bad[0]
                val x = r.x
                val y = r.y
                val replace = G.BFS(x,y)
                ret[G.weights[replace shr 1]] = true
                ret[r.id] = false
            }
        }
