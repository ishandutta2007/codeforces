...
int main() {
  ...
  int k, n; cin >> k >> n;
  vector<vector<pair<ll, int>>> special(k);
  while (n--) {
    ll v; string s; cin >> v >> s;
    REP(i, k) {
      if ((1LL << i) <= v && v < (1LL << (i + 1))) {
        special[i].emplace_back(v - (1LL << i), m[s]);
      }
    }
  }
  vector dp2(k - 1, vector(C, ModInt(0)));
  if (k > 1) fill(ALL(dp2.front()), 1);
  FOR(i, 1, k - 1) REP(l, C) REP(r, C) {
    REP(x, C) {
      if (x != l && x % (C / 2) != l % (C / 2) && x != r && x % (C / 2) != r % (C / 2)) {
        dp2[i][x] += dp2[i - 1][l] * dp2[i - 1][r];
      }
    }
  }
  map<ll, vector<ModInt>> dp;
  for (auto [v, s] : special.back()) {
    dp[v].assign(C, 0);
    dp[v][s] = 1;
  }
  for (int i = k - 2; i >= 0; --i) {
    set<ll> exs;
    for (auto [v, _] : special[i]) exs.emplace(v);
    for (auto [node, val] : dp) {
      if (exs.count(node / 2) == 0) {
        exs.emplace(node / 2);
        special[i].emplace_back(node / 2, -1);
      }
    }
    sort(ALL(special[i]));
    map<ll, vector<ModInt>> nx;
    for (auto [v, s] : special[i]) {
      nx[v] = vector<ModInt>(C, 0);
      vector<ModInt> a = (dp.count(v * 2) == 0 ? dp2[k - 2 - i] : dp[v * 2]);
      vector<ModInt> b = (dp.count(v * 2 + 1) == 0 ? dp2[k - 2 - i] : dp[v * 2 + 1]);
      if (s == -1) {
        REP(l, C) REP(r, C) {
          REP(x, C) {
            if (x != l && x % (C / 2) != l % (C / 2) && x != r && x % (C / 2) != r % (C / 2)) {
        ...
        REP(l, C) REP(r, C) {
          if (s != l && s % (C / 2) != l % (C / 2) && s != r && s % (C / 2) != r % (C / 2)) {
            nx[v][s] += a[l] * b[r];
          }
        }
      }
  ...
  cout << accumulate(ALL(dp[0]), ModInt(0)) << '\n';
  return 0;
}