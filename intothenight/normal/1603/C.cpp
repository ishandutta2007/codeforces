...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		...
		vector<tuple<int, modular, modular>> dp;
		vector<tuple<int, modular, modular>> dp_next;
		modular res = 0;
		for(auto i = 0; i < n; ++ i){
			dp_next.clear();
			for(auto [last, cnt, sum]: dp){
				int x = (a[i] + last - 1) / last;
				dp_next.push_back({a[i] / x, cnt, sum + cnt * (x - 1)});
				res += get<2>(dp_next.back());
			}
			dp_next.push_back({a[i], 1, 0});
			swap(dp, dp_next);
			reverse(dp.begin(), dp.end());
			dp_next.clear();
			for(auto [last, cnt, sum]: dp){
				if(!dp_next.empty() && get<0>(dp_next.back()) == last){
					get<1>(dp_next.back()) += cnt;
					get<2>(dp_next.back()) += sum;
				}
				else{
					dp_next.push_back({last, cnt, sum});
				}
			}
			swap(dp, dp_next);
		}
