...
const int N=5e3+2,M=1e6+2,inf=1e9;
char s[N];
int a[N],f[N][N*2];
int T,n,m,c,i,j,k,t[2],w[2],w0,x,y,z,ans,la,ksiz,ks;
int main()
{
	//ios::sync_with_stdio(false);
	cout<<setiosflags(ios::fixed)<<setprecision(15);
	T=1;
	read(n);read(a,n);t[1]=t[0]=1;
	...
	else
	{
		for (j=-i;j<=0;j++)
		{
			f[i][j+n]=min(f[i-1][j+n],f[i-1][j+n-1]+i);
		}
		for (j=1;j<=i;j++)
		{
			f[i][j+n]=min(f[i-1][j+n],f[i-1][j+n-1]-i);
	...
	//for (i=1;i<=n;i++) for (j=0;j<=n*2;j++) printf("%d %d %d\n",i,j-n,f[i][j]);
	printf("%d",f[n][n]);
}