...
struct bit{
    aN f;
    static const ll n=1e6;
    void update(ll u,ll c){
        for(;u<=n;u+=u&-u)f[u]+=c;
    }
    ll kth(ll k){
        ll u=0;
        red(i,1<<20)if(u+i<=n&&k>i-f[u+i])k-=i-f[u+=i];
        return u+1;
    }
}t;
aN x,p,flag;
void calc(int cases){
    ll n=readll(),m=readll(),k=0;
    rep(i,1,m)x[i]=readll(),p[i]=readll();
    per(i,m,1){
        ll&g=flag[t.kth(p[i]+1)];
        if(g!=cases)k++;
        g=cases;
        x[i]=t.kth(p[i]);
        t.update(x[i],1);
    }
    rep(i,1,m)t.update(x[i],-1);
    writell(C(n,2*n+~k),10);
}
int main(){
    IO();
    static const ll N=1e6;
