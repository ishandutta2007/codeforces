...
struct node {
    ...
    node(key k, key v) { this->k = k; sum = this->v = v; }
};

ostream& operator<<(ostream& str, pnode n) {
    if (!n) return str;
    return str << n->l << "{" << n->k << ',' << n->v << "}, " << n->r; 
}
...
void insert(pnode &t, key k, key v) {
    pnode r;
    splitval(t, k, t, r);
    t = merge(t, new node(k, v));
    t = merge(t, r);
}

void insert(pnode &t, int ind, key k, key v) {
    pnode r;
    splitind(t, ind, t, r);
    t = merge(t, new node(k, v));
    t = merge(t, r);
