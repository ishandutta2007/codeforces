...
int main() {
    ...
    struct node { int left = 0, made = 0, right = 0; };
    auto nn = [](node a, node b){
        int join = min(a.right, b.left);
        return node{
            a.left + b.left - join,
            a.made + b.made + 2 * join,
            a.right + b.right - join
        };
    };

    string str; re(str);
    segment_tree<node> st(sz(str), {}, nn);
    st.set_leaves([&](int i)->node{ return {str[i] == ')', 0, str[i] == '('}; });
