...
int main() {
    TEST {
        ...
        queue<int> q;
        rep(i, n) if(deg[i] == n - 1) q.emplace(i);
        vi ans(n, -1);
        vi v = iota(deg);
        while(!empty(q)) {
            int t = q.front();
            ...
            for(auto e : g[t]) {
                deg[e]--;
                if(ans[e] == -1 and deg[e] == n - 1) q.emplace(e);
            }
        }
        vi ret;
        int N = si(ans);
        rep(i, N) if(ans[i] == -1) ret.eb(i), sort(all(g[i]));
        Graph G(N);
        UnionFind uf(N);
        for(auto i : ret) {
            for(auto j : ret) {
                if(i == j) continue;
                if(find(all(g[i]), j) == end(g[i])) {
                    if(uf.unite(i, j)) {
                        // dump(i, j);
                        G[i].eb(j), G[j].eb(i);
                    }
        ...
        auto dfs = [&](auto &&f, int x, int p) -> bool {
            ...
            rep(i, si(v)) ans[v[i]] = n - i, ans2[v[i]] = n - i - 1;
            ans[x] = n - si(v), ans2[x] = n;
            n -= si(v) + 1;
            return false;
        };
        for(auto i : ret) {
            if(ans[i] == -1 and si(G[i]) == 1) { dfs(dfs, G[i][0], -1); }
        }
        cout << ans << endl << ans2 << endl;
        // rep(i, n) {
        //     for(auto j : g[i]) { assert(!((ans[i] < ans[j]) ^ (ans2[i] < ans2[j]))); }
        // }
    }
}