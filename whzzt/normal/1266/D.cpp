...
struct node{
	int u,v; i64 d;
	node(){ d=u=v=0; }
	node(int u,int v,i64 d):u(u),v(v),d(d){}
	void output(){ print(u),putc(' '),print(v),putc(' '),print(d),putc('\n'); }
};
vector<node> res;
int main(){
	scan(n,m);
	for(int i=1,u,v,w;i<=m;++i) scan(u,v,w),s[v]+=w,s[u]-=w;
	int pos=1,neg=1;
	while(pos<=n){
		while(s[pos]<=0&&pos<=n) ++pos;
		while(s[neg]>=0&&neg<=n) ++neg;
		if(pos>n) break;
		i64 t=min(s[pos],-s[neg]);
		res.pb(neg,pos,t);
		s[pos]-=t,s[neg]+=t;
	}
	print(res.size()),putc('\n');
	for(auto p:res) p.output();
}