...
fun main(){
    solve.cases{
        ...
        val A = getline(n)
        val B = getline(n)
        val new = IntArray(4 * n){
            val v = if(it >= 2 * n) (it - 2* n) else it
            if(v and 1 == 0) B[(v shr 1).previous(n)] else -A[v shr 1]
        }
        
        if(A.sumOf { it.toLong() } > B.sumOf { it.toLong() }){
            put("NO")
            return@cases
        ...
        for(i in 0..4 * n){
            ...
            if(i % 2 == 0) {
                largestEven = maxOf(largestEven, sum)
            } else {
                if(sum < largestEven){
                    put("NO")
                    return@cases
...
11 minutes for first submission
