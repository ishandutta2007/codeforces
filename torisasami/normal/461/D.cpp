...
int main() {
    ...
    cin >> n >> k;
    UnionFind uf((n + 2) * 2);
    uf.unite(0, 1), uf.unite(n + 2, n + 3);
    rep(i, k) {
        int a, b;
        char c;
        cin >> a >> b >> c;
        a--, b--;
        int l = abs(b - a), r = (b + a >= n ? (n - 1) * 2 - (b + a) : b + a) + 2;
        // cout << l << ' ' << r << endl;
        if (c == 'o')
            uf.unite(l, r + n + 2), uf.unite(l + n + 2, r);
        else
            uf.unite(l, r), uf.unite(l + n + 2, r + n + 2);
    }
    rep(i, n + 2) {
        if (uf.same(i, i + n + 2)) {
            cout << 0 << endl;
