...
int main() {
    ...
    for (int i = 0; i < n; i++) sum[i + 1] = sum[i] + (r[i] - l[i] + 1);
    int sz = sum.back(), s = sz++, t = sz++;
    Dinic<ll, true> Di(sz);
    int cur = 0;

    for (int i = 0; i < n; i++) {
        Di.add_edge(s, cur, INF);
        for (int j = l[i]; j < r[i]; j++, cur++) Di.add_edge(cur, cur + 1, inf - f(i, j));
        Di.add_edge(cur++, t, INF);
    }
    ...
    for (; m--;) {
        ...
        for (int j = l[u]; j <= r[u]; j++) {
            int k = j - d;
            if (l[v] <= k && k <= r[v]) Di.add_edge(sum[u] + (j - l[u]), sum[v] + (k - l[v]), INF);
        }
    }

    int ans = inf * n - Di.max_flow(s, t);
    cout << ans << '\n';
    return 0;
}