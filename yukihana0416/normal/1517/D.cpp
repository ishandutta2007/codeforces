...
void work()
{
    ...
    for(int d = 1; d <= k / 2; d++)
        for(int i = 1; i <= n; i++)
            for(int j = 1; j <= m; j++)
            {
                if(i + 1 <= n) get_min(dp[i + 1][j][d], dp[i][j][d - 1] + v[i][j]);
                if(j + 1 <= m) get_min(dp[i][j + 1][d], dp[i][j][d - 1] + h[i][j]);
                if(i - 1 >= 1) get_min(dp[i - 1][j][d], dp[i][j][d - 1] + v[i - 1][j]);
                if(j - 1 >= 1) get_min(dp[i][j - 1][d], dp[i][j][d - 1] + h[i][j - 1]);
            }
