...
int main() {
    ...
    REP(i, N) {
        ...
        cin >> a;
        dp.push_back(0);
        IREP(i, dp.size() - 1) dp[i + 1] += dp[i] * a;
    }
    REP(i, dp.size()) dp[i] /= mint(N).nCr(i);
    mint ret = 0;
    vector<mint> minusnpowinv(N + 10, 1);
    const mint ninv = mint(N).inv();
    FOR(i, 1, minusnpowinv.size()) minusnpowinv[i] = minusnpowinv[i - 1] * (-ninv);

    vector<mint> kfacs(N + 10, K);
    FOR(i, 1, kfacs.size()) kfacs[i] = kfacs[i - 1] * (K - i);
    FOR(nx, 0, N) {
        int nb = N - 1 - nx;
        if (nb + 1 > K) continue;
        ret += dp[nx] * mint(nb + 1).inv() * mint(N - 1).nCr(nb) * kfacs[nb] * minusnpowinv[nb];
    }
    cout << ret << '\n';
}