...
const int N = 110000;
int pre[maxdeep][N], lft[N], rght[N], deep[N];
int curtime;
vector<int> v[N], g[N];
...
int getpre(int u, int n){
    int ans = u;
    for(int i=0; i<maxdeep; ++i){
        if(shl(i) & n) ans = pre[i][ans];
    }
    return ans;
}
...
int main(){
    ...
    for(int i=1; i<=n; ++i){
        scanf("%d", &pre[0][i]);
        v[pre[0][i]].push_back(i);
    }
    curtime = 0;
    dfs(0, 0);
    for(int i=1; i<=n; ++i){
        g[deep[i]].push_back(lft[i]);
    }
    for(int i=1; i<N; ++i){
    ...
    for(int i=0; i<q; ++i){
        ...
        if(anc == 0){
            ...
            ans = upper_bound(g[deep[a]].begin(), g[deep[a]].end(), rght[anc]) - lower_bound(g[deep[a]].begin(), g[deep[a]].end(), lft[anc]) - 1;
        }
        printf("%d", ans);
        if(i < q-1) putchar(' ');
        else putchar(10);
