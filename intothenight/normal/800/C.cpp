...
int main(){
	...
	cin >> n >> m;
	vector<int> fact;
	for(auto low = 1, d = 1; low <= m; low = d + 1){
		d = m / (m / low);
		if(m % d == 0){
			fact.push_back(d);
		}
	...
	debug(adj);
	vector<int> allowed(m, 1);
	vector<vector<int>> contains(k);
	for(auto i = 0; i < n; ++ i){
		int x;
		cin >> x;
		allowed[x] = false;
	...
	for(auto x = 0; x < m; ++ x){
		if(allowed[x]){
			int g = gcd(x, m);
			contains[lower_bound(fact.begin(), fact.end(), g) - fact.begin()].push_back(x);
		}
	}
	debug(contains);
	vector<int> dp(k), prev(k, -1);
	for(auto u = 0; u < k; ++ u){
		dp[u] += int(contains[u].size());
		for(auto v: adj[u]){
			if(dp[v] < dp[u]){
				dp[v] = dp[u];
				prev[v] = u;
	...
	vector<int> res;
	for(auto u: t){
		for(auto x: contains[u]){
			res.push_back(x);
		}
	}
	debug(res);
	cout << res[0] << " ";	
	for(auto i = 1; i < int(res.size()); ++ i){
		int g = gcd(res[i - 1], m);
		m /= g;
		debug(res[i - 1], res[i], g, m);
		cout << Zp(res[i] / g) / (res[i - 1] / g) << " ";
		m *= g;
	}
	cout << "\n";
	return 0;
