        ...
        while(!q.empty()) {
            ...
            for (const auto& e : graph[cur]) {
                ...
                if (dist[next].first > ncost) {
                    ...
                    if (v[next]) continue;
                    v[next] = 1;
                    if (q.empty() == false && ncost < dist[q.front()].first)
                        q.push_front(next);
...
int main()
{
    ...
    MinCostFlow mcf(3 + n * 2);

    mcf.addEdge(0, 1, 0, 4);

    for (int i = 1; i <= n; i++)
    {
        mcf.addEdge(1, i * 2, 0, 1);
        mcf.addEdge(i * 2, i * 2 + 1, -1, 1);
        mcf.addEdge(i * 2 + 1, n * 2 + 2, 0, 1);
    }
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = i + 1; j <= n; j++)
        {
            if (abs(dat[i] - dat[j]) == 1 || (dat[i] - dat[j]) % 7 == 0)
            {
                mcf.addEdge(i * 2 + 1, j * 2, 0, 1);
            }
        }
    }

    printf("%d\n", -mcf.solve(0, n * 2 + 2).first);
}