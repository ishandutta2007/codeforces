...
void my_main() {
  int n, m;
  cin >> n >> m;
  auto a = VII(n + 100, m + 100);
  rep(i, 1, n + 1) rep(j, 1, m + 1) {
    cin >> a[i][j];
  }
  auto should = VI(10 * n * m + 1);
  int ans = 0;
  rep(j, 1, m + 1) {
    auto b = VI(n + 100);
    auto cost = VI(n + 100);
    rep(i, 1, n + 1) cost[i] = i - 1 + n;
    b[1] = j;
    rep(i, 2, n + 1) b[i] = b[i - 1] + m;
    rep(i, 1, n + 1) should[b[i]] = i;
    rep(i, 1, n + 1) {
      int v = a[i][j];
      if (v > n * m) continue;
      if (should[v] == 0) continue;
      int start = i + 1 - should[v];
      if (start <= 0) start += n;
      if (start < 0 || start > n) return;
      --cost[start];
    }
    int cur_ans = INT_MAX;
    rep(i, 1, n + 1) cur_ans = min(cur_ans, cost[i]);
    ans += cur_ans;
    rep(i, 1, n + 1) should[b[i]] = 0;
  }
  cout << ans << endl;
}
