...
fun solve() {
    val (n,m) = io.ints(2)
    val a = Array(n) { IntArray(m, { io.int() })}
    ...
    for (j in 0..m-1) if (a[0][j] == 1) {
        flipCol(j)
    }
    var zero = true
    for (i in 1..n-1) {
        if (zero) {
            if (a[i][0] == 1) flipRow(i)
            if (a[i][m-1] == 1) zero = false
        } else {
    ...
    fun fin() {
        ...
        for (i in 0 until n) for (j in 0 until m) seq.add(a[i][j])
        for (i in 0..seq.size-2) if (seq[i] > seq[i+1]) return
        io.println("YES")
        for (i in 0 until n) io.print(row[i])
        io.println()
        for (j in 0 until m) io.print(col[j])
        io.println()
        io.close()
        exitProcess(0)
    }
    fin()
    for (j in 0..m-1) if (a[n-1][j] == 0) {
        flipCol(j)
    }
    var one = true
    for (i in n-2 downTo 0) {
        if (one) {
            if (a[i][m-1] == 0) flipRow(i)
            if (a[i][0] == 0) one = false
        } else {
            if (a[i][m-1] == 1) flipRow(i)
        }
    }
    fin()
}
