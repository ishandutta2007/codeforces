...
int main() {
    ...
    // (2 * area + 3-gcds) = 0 or 2 (mod 4)

    // gcd(a, b) % 4?

    auto check = [&](P a, P b, P c) {
        int sum = 0;
        sum += gcd(a.first - b.first, a.second - b.second);
        sum += gcd(b.first - c.first, b.second - c.second);
        sum += gcd(c.first - a.first, c.second - a.second);
        return (sum / 2 % 2 == 0);
    };
    ...
    ll ans = 0;
    for (auto p: cnt) {
        if (check(p.first, p.first, p.first)) ans += C3(p.second);
    }
    for (auto p : cnt) {
        for (auto q : cnt) {
            if (p.first == q.first) continue;
            if (check(p.first, p.first, q.first)) ans += C2(p.second) * q.second;
        }
    }
    for (auto p : cnt) {
        for (auto q : cnt) {
            for (auto w : cnt) {
                if (p.first == q.first || p.first == w.first || q.first == w.first) continue;
                if (check(p.first, q.first, w.first))
                    ans += p.second * q.second * w.second;
            }
        }
    }
