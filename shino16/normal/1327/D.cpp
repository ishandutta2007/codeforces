...
int main() {
  ...
  rep(t) {
    ...
    rep(i, n) if (not visited[i]) {
      ...
      ll loopN = trace.size();
      set<ll> div;
      rep(i, 1, sqrt(loopN)+1) if (loopN % i == 0) div.insert(i), div.insert(loopN/i);
      for (ll d : div) {
        VB done(loopN);
        rep(i, loopN) if (not done[i]) {
          bool fail = false;
          int i0 = i;
          do {
            done[i] = true;
            fail |= c[trace[i]] != c[trace[i0]];
            i = (i + d) % loopN;
          } while (i != i0);
          if (not fail) {
            debug(i0);
      ...
      // debugv(trace);
      // rep(i, trace.size()*2) {
      //   if (prev.count(c[trace[i%trace.size()]]))
      //     lcm = lcm / __gcd(lcm, i-prev[c[trace[i%trace.size()]]])
      //               * (i-prev[c[trace[i%trace.size()]]]);
      //   prev[c[trace[i%trace.size()]]] = i;
      // }
      // chmin(res, lcm);
    }
    OUT(res);
