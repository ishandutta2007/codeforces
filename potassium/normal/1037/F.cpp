...
int main() {
	...
	Mint ans(0, MOD107);
	for (int i = 1; i <= n; i++) {
		const int width = rb[i] - lb[i] - 1;

		ll total = 0;
		int tl = 1;
		int tr = 1 + div(width - 1, k - 1) * (k - 1);
		int tn = div(tr - tl, k - 1) + 1;
		total += 1LL * (tl + tr) * tn / 2;

		int minl = lb[i] - i + div(i - lb[i] - 1 + k - 1, k - 1) * (k - 1) + 1;
		int maxl = lb[i] - (i - tr + 1) + 1;
		int ln = (maxl - minl) / (k - 1) + 1;
		total -= 1LL * (minl + maxl) * ln / 2;

		int minr = i - rb[i] + div(rb[i] - i - 1 + k - 1, k - 1) * (k - 1) + 1;
		int maxr = (i + tr - 1) - rb[i] + 1;
		int rn = (maxr - minr) / (k - 1) + 1;
		total -= 1LL * (minr + maxr) * rn / 2;
