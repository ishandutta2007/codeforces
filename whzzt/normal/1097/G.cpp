...
int f[N][M],sz[N],coef[M],n,m,w[N],tf[M],ans;
int S[M][M],fac[M];
vector<int> adj[N];
void dfs(int u,int fa){
	...
	for (int v:adj[u]) if(v!=fa) {
		...
		for(i=0;i<=min(m,sz[u]);++i) if(f[u][i]) {
			for(j=0;j<=min(m-i,sz[v]);++j) if(f[v][j]) {
				inc(tf[i+j],(i64)f[u][i]*f[v][j]);
			}
		}
		for(i=0;i<=min(m,sz[v]);++i) dec(coef[i],f[v][i]);
		memcpy(f[u],tf,sizeof(tf));
		sz[u]+=sz[v];
	}
	for(i=0;i<=m;++i) inc(coef[i],f[u][i]);
	if(u!=1) {
		for(i=1;i<=m;++i) inc(coef[i],(i64)f[u][i-1]*w[n-sz[u]]%P*(i==1?w[sz[u]]:1));
		for(i=m;i>=2;--i) inc(f[u][i],f[u][i-1]);
		inc(f[u][1],(i64)f[u][0]*w[sz[u]]);
	}
}
int main(){
	int i,j,u,v;
	scan(n,m);
	for(i=1;i<n;++i)scan(u,v),adj[u].pb(v),adj[v].pb(u);
	w[0]=1;
	for(i=1;i<=n;++i)w[i]=i64(w[i-1])*(P/2+1)%P;
	for(i=0;i<=n;++i)w[i]=(P+1-w[i])%P;
	dfs(1,0);
	for(i=0;i<=m;++i)coef[i]=(i64)coef[i]*fpow(2,n)%P;
	for(i=0;i<=m;++i){
		S[i][0]=(i==0);
		for(j=1;j<=i;++j) S[i][j]=(S[i-1][j-1]+(i64)S[i-1][j]*j)%P;
	}
	for(i=fac[0]=1;i<=m;++i) fac[i]=(i64)fac[i-1]*i%P;
	for(i=0;i<=m;++i) inc(ans,(i64)coef[i]*fac[i]%P*S[m][i]);
	cout<<ans<<endl;
}