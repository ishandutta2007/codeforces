...
void solve() {
    ...
    for (int len = 4; len <= n; len += 2) {
        for (int i = 0; i + len <= n; i++) {
            ...
            f[i][j] = -1;
            {
                // choose i, other player is forced to choose i + 1 or j for the next turn
                int ans_i = -1;
                if (f[i + 2][j] == 1) ans_i = 1;
                else if (f[i + 2][j] == 0) {
                    if (s[i] < s[i + 1]) ans_i = 1;
                    else if (s[i] == s[i + 1]) ans_i = 0;
                }
                int ans_j = -1;
                if (f[i + 1][j - 1] == 1) ans_j = 1;
                else if (f[i + 1][j - 1] == 0) {
                    if (s[i] < s[j]) ans_j = 1;
                    else if (s[i] == s[j]) ans_j = 0;
                }
                f[i][j] = max(f[i][j], min(ans_i, ans_j));
            }
            {
                // choose j, other player is forced to choose i or j - 1 for the next turn
                int ans_i = -1;
                if (f[i + 1][j - 1] == 1) ans_i = 1;
                else if (f[i + 1][j - 1] == 0) {
                    if (s[j] < s[i]) ans_i = 1;
                    else if (s[j] == s[i]) ans_i = 0;
                }
                int ans_j = -1;
                if (f[i][j - 2] == 1) ans_j = 1;
                else if (f[i][j - 2] == 0) {
                    if (s[j] < s[j - 1]) ans_j = 1;
                    else if (s[j] == s[j - 1]) ans_j = 0;
                }
                f[i][j] = max(f[i][j], min(ans_i, ans_j));
            }
        }
    ...
    if (n % 2 == 0) {
        if (f[0][n - 1] == 1) cout << "Alice\n";
        else if (f[0][n - 1] == 0) cout << "Draw\n";
        else cout << "Bob\n";
    } else {
        // alice is forced to choose 0 or n - 1
        int ans_0 = -1;
        if (f[1][n - 1] == -1) ans_0 = 1;
        int ans_n = -1;
        if (f[0][n - 1] == -1) ans_n = 1;
        int ans = max(ans_0, ans_n);
        if (ans == 1) cout << "Alice\n";
