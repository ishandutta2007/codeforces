...
int main() {
    ...
    while(TT--) {
        ...
        VEC(int,s,n);
        vvi g(n);
        rep(i,n-1) g[p[i] - 1].emplace_back(i+1);
        map<pii,ll> mp;
        auto dfs = REC([&](auto f,int now,int c) -> ll {
            if(mp.find(make_pair(now,c)) != mp.end()) {
                return mp[make_pair(now,c)];
            }
            int sz = g[now].size();
            if(sz == 0) {
                mp[make_pair(now,c)] = 1LL * s[now] * c;
                return 1LL * s[now] * c;
            }
            int h = c / sz;
            if(c % sz == 0) {
