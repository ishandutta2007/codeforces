...
inline void calc()
{
	...
	{
		for(;u<i&&(x[i]&(p-1))+(x[u]&(p-1))<(p>>1);u++);
		for(;v<i&&(x[i]&(p-1))+(x[v]&(p-1))<p;v++);
		for(;w<i&&(x[i]&(p-1))+(x[w]&(p-1))<((p>>1)|p);w++);
		u=min(u,i),v=min(v,i),w=min(w,i),rr=rr^((i+v-w-u)%2);
	}
	res=res^(rr<<q);
}
int main()
{
	...
	for(;p<=4e7;p<<=1,q++)
	{
		sort(&x[1],&x[n+1],[](ll x,ll y){return (x&(p-1))<(y&(p-1));}),calc();
	}
	printf("%d\n",res);
}