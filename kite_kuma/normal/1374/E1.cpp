...
int main() {
	...
	int hs = h.size(), ls = l.size(), ms = m.size();

	vector<ll> p(hs + 1), q(ls + 1), r(ms + 1);
	rep(i, hs) p[i + 1] = p[i] + h[i];
	rep(i, ls) q[i + 1] = q[i] + l[i];
	rep(i, ms) r[i + 1] = r[i] + m[i];
	ll ans = dekai;
	rep(i, ms + 1) {
		ll j = max(k - i, 0LL);
		if(j > hs || j > ls) continue;
		chmin(ans, p[j] + q[j] + r[i]);
	}
	if(ans == dekai) ans = -1;
	cout<<ans<<endl;
}