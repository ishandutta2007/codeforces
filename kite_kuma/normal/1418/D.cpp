...
int main() {
	...
	int res = a.back() - a[0];
	int mini = 0;
	rep(i, n - 1) { chmax(mini, a[i + 1] - a[i]); }
	// res -= mini;
	vector<int> ans(q + 1);
	ans[0] = res - mini;

	multiset<int> ms;
	rep(i, n - 1) { ms.insert(a[i + 1] - a[i]); }
	...
	rep(i, q) {
		...
		if(!t) {
			...
			if(it != se.begin()) {
				auto pre = it;
				pre--;
				auto itms = ms.find((*it - *pre));
				ms.erase(itms);
			}
			auto lat = it;
			lat++;
			if(lat != se.end()) {
				auto itms = ms.find(*lat - *it);
				ms.erase(itms);
			}
			se.erase(x);
		} else {
