...
void solve() {
  ...
  vector<unordered_map<int, ll>> ma(n);
  function<void(int, int)> dfs = [&](int u, int left) {
    if (ma[u].count(left)) {
      return;
    }
    ma[u][left] = left * s[u];
    if (edg[u].empty()) return;
    int nxt_left = left / edg[u].size();
    int more = left % edg[u].size();
    for (auto v : edg[u]) {
      dfs(v, nxt_left);
      if (more) dfs(v, nxt_left + 1);
    }
    if (more) {
      sort(edg[u].begin(), edg[u].end(), [&](int a, int b) {
        return ma[a][nxt_left + 1] - ma[a][nxt_left] >
               ma[b][nxt_left + 1] - ma[b][nxt_left];
      });
    }
    for (int i = 0; i < edg[u].size(); i++) {
      ma[u][left] += ma[edg[u][i]][nxt_left + (i < more)];
    }
  };
  dfs(0, k);
  cout << ma[0][k] << endl;
