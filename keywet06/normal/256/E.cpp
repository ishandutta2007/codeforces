...
const int N = 160 * 1000 + 5;
Mat st[4 * N], w;

void buildst(int v, int tl, int tr, int n) {
    if (tl == tr - 1) {
        if (tl == 0) {
            for (int i = 0; i < 3; ++i) st[v].a[0][i] = 1;
        } else if (tr == n) {
            for (int i = 0; i < 3; ++i) st[v].a[i][0] = 1;
        } else if (tl & 1) {
            st[v] = w;
        } else {
            st[v] = Mat(-1);
    ...
    if (tl == tr - 1) {
        if (tl == 0) {
            if (t == -1) {
                ...
                for (int i = 0; i < 3; ++i) st[v].a[0][i] = (i == t);
            }
        } else if (tr == n) {
            if (t == -1) {
                for (int i = 0; i < 3; ++i) st[v].a[i][0] = 1;
            } else {
                for (int i = 0; i < 3; ++i) st[v].a[i][0] = (i == t);
            }
        } else {
            st[v] = Mat(t);
        }
