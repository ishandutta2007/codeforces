...
inline bool judge(int a, int b, int c, int d) {
    return a == 50 && b == 48 && c == 50 && d == 48;
}
inline void work() {
    read(n); read(s + 1);
    if(judge(s[1], s[2], s[3], s[4]) || judge(s[1], s[2], s[3], s[n]) || judge(s[1], s[2], s[n - 1], s[n]) || judge(s[1], s[n - 2], s[n - 1], s[n]) || judge(s[n - 3], s[n - 2], s[n - 1], s[n])) print("YES");
    else print("NO");
}
