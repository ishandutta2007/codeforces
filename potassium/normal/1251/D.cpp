...
int main() {
	CASE(t) {
		ll n,s; R(n,s);
		vector<pll> emp(n); R(emp);
		
		//sort(all(emp), [&](pll a, pll b) {if (a.S != b.S) return a.S < b.S; else return a.F < b.F;});
		//sort(all(emp));
		...
		while (r >= l) {
			...
			ll sum = 0;
			for (int i = 0; i < n; i++) {
				if (emp[i].S < mid) {sum += emp[i].F; cntl++; continue;}
				if (emp[i].F > mid) {sum += emp[i].F; cntr++; continue;}
				lv.pb(emp[i].F);
			}
			...
			sort(all(lv));
			int take = n/2 - cntl;
			int takemid = max((n+1)/2 - cntr, 0LL);
			
			for (int i = 0; i < min(take, (int)lv.size()); i++)
				sum += lv[i];
				
			sum += mid * takemid;
			//W(mid,sum,cntl,cntr,take,takemid);
			
			if (sum > s) r = mid-1; else l = mid+1;
		}
