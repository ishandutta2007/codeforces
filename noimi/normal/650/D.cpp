...
main() {
    ...
    vi ans(q, -1);
    {
        rep(j, 2) {
            vi v;
            rep(i, n) {
                for(auto [x, id] : que[i]) ans[id] += lb(v, x) + 1;
                auto it = lower_bound(all(v), a[i]);
                lis[j][i] = distance(v.begin(), it) + 1;
                if(it == end(v))
                    v.eb(a[i]);
                else
                    *it = a[i];
            ...
            reverse(all(que));
            for(auto &e : a) e = -e;
            rep(i, n) for(auto &e : que[i]) e.fi = -e.fi;
        }
    }
    reverse(all(lis[1]));
    int m = *max_element(all(lis[0]));
    rep(i, q) chmax(ans[i], m - 1);
    vi imos(n + 1);
    vector<map<int, int>> mp(m + 1);
    rep(i, n) chmax(mp[lis[1][i]][a[i]], i);
    mp[0][INT_MAX] = n;
    rep2(i, -1, n - 1) {
        int rem;
        if(i == -1)
            ...
            rem = m - lis[0][i];
        auto it = mp[rem].upper_bound((i == -1 ? 0 : a[i]));
        if(it != mp[rem].end() and (it->se) > i + 1) { imos[i + 1]++, imos[(it->se)]--; }
    }
    rep(i, n) imos[i + 1] += imos[i];
    rep(i, n) if(imos[i]) {
        for(auto [x, id] : que[i]) chmax(ans[id], m);
    }
