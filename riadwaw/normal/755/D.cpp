...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		k = min(k, n - k);
		int64_t pos = 0;
		BottomUpSumSegmentTree<int64_t> tree(n);
		for (int64_t i: range(n)) {
			int64_t to = pos + k;
			int64_t cur = tree.getResult(pos, min(to, n));
			if (to >= n) {
				to -= n;
				cur += tree.getResult(0, to);
			}
			//cerr << cur << ' ' << endl;
			int64_t from = pos - k + 1;
			cur += tree.getResult(max(from, (int64_t)0), pos);
			if (from < 0) {
				cur += tree.getResult(from + n, n);
			}
