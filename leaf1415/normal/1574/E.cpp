...
ll h, w, Q;
map<P, ll> mp;
ll xcnt[1000005][2], ycnt[1000005][2];
ll xnum[4], ynum[4], dnum[2];
...
int main(void)
{
	...
	rep(i, 1, Q){
		...
		if(mp.count(P(x, y))){
			ll b = mp[P(x, y)];
			xnum[get(xcnt[x])]--, ynum[get(ycnt[y])]--;
			xcnt[x][(b+y)%2]--, ycnt[y][(b+x)%2]--;
			xnum[get(xcnt[x])]++, ynum[get(ycnt[y])]++;
			mp.erase(P(x, y));
			dnum[(x+y+b)%2]--;
		}
		if(t != -1){
			mp[P(x, y)] = t;
			xnum[get(xcnt[x])]--, ynum[get(ycnt[y])]--;
			xcnt[x][(t+y)%2]++, ycnt[y][(t+x)%2]++;
			xnum[get(xcnt[x])]++, ynum[get(ycnt[y])]++;
			//outl(ycnt[y][0], ycnt[y][1]);
			dnum[(x+y+t)%2]++;
		}
		...
		mint ans = 0;
		if(xnum[3] == 0) ans += modpow(2, xnum[0]);
		if(ynum[3] == 0) ans += modpow(2, ynum[0]);
		if(dnum[0] == 0 || dnum[1] == 0) ans -= 1;
		if(dnum[0] == 0 && dnum[1] == 0) ans -= 1;
		cout << ans << "\n";
	}
