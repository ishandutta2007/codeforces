...
using namespace mem;
const int N=5e3+10;
int n,t[N],x[N],f[N],g[N],h[N];
bool can[N][N];
inline int dis(int i,int j){return abs(x[i]-x[j]);}
inline bool check(int i,int j){return abs(x[i]-x[j])<=abs(t[i]-t[j]);}
int main(){
  ...
  read(n);
	...
	for(int i=1;i<=n;i++){
		can[i][i]=1;
		for(int j=i+1;j<=n;j++)can[i][j]=can[i][j-1]&&check(j-1,j);
	}
	memset(g,-1,sizeof(g));
	memset(h,-1,sizeof(h));
	f[0]=1;
	for(int i=0;i<n;i++){
		if(f[i]){
			if(check(i,i+1)){
				f[i+1]=1;
				if(~g[i+1]){
          g[i+1]=min(g[i+1],t[i]+dis(i,i+1));
        }else{
          g[i+1]=t[i]+dis(i,i+1);
        }
			}
			for(int j=i+2;j<=n;j++)
				if(dis(i,j)+dis(j,i+1)<=t[i+1]-t[i]&&can[i+1][j-1]){
					h[j]=1;
        }
		}
		if(~g[i]){
			if(t[i+1]-g[i]>=dis(i,i+1)){
				f[i+1]=1;
				if(~g[i+1]){
          g[i+1]=min(g[i+1],max(t[i],g[i]+dis(i,i+1)));
        }else{
          g[i+1]=max(t[i],g[i]+dis(i,i+1));
        }
			}
			for(int j=i+2;j<=n;j++){
				if(max(t[i],g[i]+dis(i,j))+dis(j,i+1)<=t[i+1]&&can[i+1][j-1]){
					h[j]=1;
				}
			}
		}
		if(~h[i]){
			if(t[i+1]-t[i-1]>=dis(i-1,i+1)){
				f[i+1]=1;
				if(~g[i+1]){
          g[i+1]=min(g[i+1],max(t[i],t[i-1]+dis(i-1,i+1)));
        }else{
          g[i+1]=max(t[i],t[i-1]+dis(i-1,i+1));
        }
			}
			for(int j=i+2;j<=n;j++)
				if(max(t[i],t[i-1]+dis(i-1,j))+dis(j,i+1)<=t[i+1]&&can[i+1][j-1]){
					h[j]=1;
        }
		}
	}
	// for(int i=1;i<=n;i++)log("%d%c",f[i]," \n"[i==n]);
	// for(int i=1;i<=n;i++)log("%d%c",g[i]," \n"[i==n]);
	// for(int i=1;i<=n;i++)log("%d%c",h[i]," \n"[i==n]);
	if(f[n]||g[n]!=-1||h[n]!=-1){
    println("YES");
  }else{
    println("NO");
