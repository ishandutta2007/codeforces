...
public class G {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] field = sc.nextIntArray(N);
        int[] lmax = new int[M];
        int[] rmax = new int[M];
        for (int n = 0; n < M; n++) {
            int now = hp[n];
            for (int m = hero[n] - 1; m >= 0; m--) {
                now += field[m];
                if (now < 0) {
                    lmax[n] = m + 1;
                    break;
        ...
        // cp.println(Arrays.toString(lmax));
        // cp.println(Arrays.toString(rmax));
        boolean ok = false;
        for (int n = 0; n < N; n++) {
            int freemin = n;
            int freemax = n;
            ...
            for (int m = 0; m < M; m++) {
                ...
                for (int k = 0; k < M; k++) {
                    ...
                    if (hero[k] == n) {
                        ...
                        if (rmax[k] >= freemin) {
                            found = true;
                            freemin = Math.min(freemin, hero[k] - 1);
                            freemax = Math.max(freemax, n + 1);
                            order.add(k);
                            pass[k] = true;
                            break;
                        }
                        ...
                        if (lmax[k] <= freemax) {
                            found = true;
                            freemin = Math.min(freemin, n - 1);
                            freemax = Math.max(freemax, hero[k] + 1);
                            order.add(k);
                            pass[k] = true;
                            break;
                        }
