...
int main() {
    ...
    repn(i, M) {
        ...
        g[u].push_back({ i,u,v,0 });
        g[v].push_back({ -i,v,u,0 });
        d.merge(u, v);
    }
    ...
    auto dfs = [&](auto&& dfs, edge &f)->ll {

        ll sum = 0;
        ll v = f.to;
        for (edge e : g[v]) {
            if (f.fr == e.to) { continue; }
           
            sum += dfs(dfs, e);
        }

        ll now = s[v] - sum;
        if (f.id > 0) {ans[f.id] = now; }
        else { ans[-f.id] = -now; }
        return -now;

    };

    for (auto vec : d.groups()) {
        ll r = vec[0];
        edge f = { 0,r,0,0 };
        dfs(dfs, f);
    }
