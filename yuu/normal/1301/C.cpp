...
void solve() {
    int64_t n, m;
    cin >> n >> m;
    // minimize 0 only instead
    // m + 1 regions that can be only 0
    // each region with size x cost x * (x + 1) / 2
    // sum up to n - m
    // AM-GM something something each region should be as close as possible
    int64_t ans = n * (n + 1) / 2;
    n -= m;
    m++;
    // n / m
    int64_t x = n / m;
    int64_t count_x_1 = n % m;
    int64_t count_x = m - count_x_1;
    ans -= count_x * x * (x + 1) / 2;
    ans -= count_x_1 * (x + 1) * (x + 2) / 2;
    cout << ans << '\n';
}
