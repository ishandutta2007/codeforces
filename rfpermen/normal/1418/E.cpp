...
ll n,m,x[MAX],y[MAX],val,a,b,ans,cnt;
int id;
...
//	freopen("output.txt","w",stdout);
	...
	rep(i,1,n)cin>>x[i];
	sort(x+1,x+1+n);
	rep(i,1,n)y[i] = (x[i]+y[i-1]) % MOD;
	rep(i,1,m){
		cin>>a>>b;
		ans = 0;
		id = lower_bound(x+1,x+1+n,b) - x;
		cnt = (n+1)-id;
		if(cnt<a){
			cout<<"0\n";
			continue;
		}
		ans = fc[cnt] * (cnt+1-a) % MOD * fc[n] % MOD * pre[cnt+1] % MOD * y[id-1] % MOD;
		ans+= fc[cnt-1] * (cnt-a) % MOD * fc[n] % MOD * pre[cnt] % MOD * (y[n]-y[id-1]) % MOD;
		ans%= MOD;
		if(ans<0)ans+= MOD;
