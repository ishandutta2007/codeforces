...
struct SparseTable {
private:
	vector<vector<T>> sparseTable = vector<vector<T>>(1);
	function<T(T,T)> func;

public:
	vector<T>& operator [] (int level) {return sparseTable[level];}
	int size() {return sparseTable.size();}
	T query(int l, int r) {
		assert(l <= r);
		int level = __lg(r - l + 1);
		return func(sparseTable[level][l], sparseTable[level][r - (1 << level) + 1]);
	}
	void build() {
		int baseSize = sparseTable[0].size();
		for (int i = 1; ; i++) {
			int curSize = baseSize - (1 << i) + 1;
			if (curSize > 0) {
				if (sparseTable.size() >= i + 1)
					sparseTable[i].clear();
				else
					sparseTable.push_back(vector<T>());
				sparseTable[i].resize(curSize);
				for (int j = 0; j < curSize; j++)
					sparseTable[i][j] = func(sparseTable[i-1][j], sparseTable[i-1][j + (1 << i - 1)]);
			} else break;
		}
	}
	SparseTable(vector<T>& v, function<T(T,T)> func) : func(func) {sparseTable[0] = v; build();}
	SparseTable(int size, function<T(T,T)> func) : func(func) {sparseTable[0].resize(size);}
};
...
int main() {
	CASE(t) {
		...
		SparseTable<int> minST(a, [&](int x, int y) {
			return min(x, y);
		}), maxST(a, [&](int x, int y) {
			return max(x, y);
		});
		...
		for (int i = 0; i < n - 2; i++) {
			...
			int mr = min(l, last[pmax] - 1);
			if (mr <= i)
				continue;
				
			int x = maxST.query(0, i);
			int y = minST.query(i + 1, mr);
			int z = maxST.query(mr + 1, n - 1);
			if (x == y && y == z) {
				W("YES");
				W(i + 1, mr - i, n - mr - 1);
				is = 1;
