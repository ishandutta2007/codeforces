...
//
#define MAXN 200005
int n, a[MAXN], ans[MAXN], stk[MAXN], tp;
bool vis[MAXN];
inline void work(){
    read(n);read(a + 1, a + n + 1);memset(vis + 1, 0, n * sizeof(int));
    rep(i, 1, n) if(a[i] ^ a[i - 1]) ans[i] = a[i], vis[a[i]] = 1;else ans[i] = 0;
    int it = 1;
    rep(i, 1, n){
        if(!ans[i]) {
            while(vis[it]) ++ it;
    ...
    int pre = 0;tp = 0;
    rep(i, 1, n){
        if(!ans[i]) sprint(stk[tp --]);
        else {
            rep(j, pre + 1, ans[i] - 1) stk[++ tp] = j;
            pre = ans[i];sprint(ans[i]);
        }
    }
    putcha(10);
}
