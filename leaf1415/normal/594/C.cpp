...
int main(void)
{
	...
	rep(i, 1, n){
		xvec.push_back(P(x[i], i));
		yvec.push_back(P(y[i], i));
	}
	sort(all(xvec)), sort(all(yvec));
	
	//rep(i, 1, n) outl(x[i], y[i]);
	
	ll ans = inf;
	rep(l, 0, min(k, sz(xvec)-1)){
		rep(r, 0, min(k, sz(xvec)-1)){
			rep(d, 0, min(k, sz(yvec)-1)){
				rep(u, 0, min(k, sz(yvec)-1)){
					set<ll> S;
					rep(i, 0, l-1) S.insert(xvec.begin()[i].second);
					rep(i, 0, r-1) S.insert(xvec.rbegin()[i].second);
					rep(i, 0, d-1) S.insert(yvec.begin()[i].second);
					rep(i, 0, u-1) S.insert(yvec.rbegin()[i].second);
					if(sz(S) > k) continue;
					
					ll L = xvec.begin()[l].first, R = xvec.rbegin()[r].first;
					ll D = yvec.begin()[d].first, U = yvec.rbegin()[u].first;
					chmin(ans, max(1LL, ceil(R-L, 2))*max(1LL, ceil(U-D, 2)));
				}
			}
		}
	}
