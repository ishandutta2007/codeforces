...
int main() {
    ...
    ll N,M,K,T; cin >> T;
    rep(ii,T){
        ll N,M; cin >> N >> M; ans = 0;
        ll a[N];
        rep(i,N) cin >> a[i];
        ll g = 0;
        rep(i,M){
            ll b; cin >> b; g = gcd(g,b);
        }
        if(g==1){
            rep(i,N) ans += abs(a[i]);
            cout << ans << "\n"; continue;
        ...
        rep(i,g){
            ...
            K = p.size();
            auto dp = v2ll(K,2,-2e18);
            dp[0][0] = 0;
            rep(j,K-1){
                chmax(dp[j+1][0],dp[j][0]+p[j]);
                chmax(dp[j+1][0],dp[j][1]-p[j]);
                chmax(dp[j+1][1],dp[j][0]-p[j]);
                chmax(dp[j+1][1],dp[j][1]+p[j]);
            }
            tmp += max(dp[K-1][0]+p[K-1],dp[K-1][1]-p[K-1]);
        }
        chmax(ans,tmp);
        //another
        tmp = 0;
        rep(i,g){
            vector<ll> p;
            for(int j=i;j<N;j+=g){
                if(j==i) p.push_back((-1LL)*a[j]);
                else p.push_back(a[j]);
            }
            K = p.size();
            auto dp = v2ll(K,2,-2e18);
            dp[0][0] = 0;
