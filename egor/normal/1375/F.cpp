...
public:
    void solve() {
        ...
        while (true) {
            ...
            if (at == last) {
                ll small = *min_element(all(a));
                ll big = a[at];
                ll med = accumulate(all(a), 0ll) - small - big;
                ll B = big - small;
                ll A = med - small;
                ll move = 2 * B - A;
                out.printLine(move);
                ...
                a[pos] += move;
                big = *max_element(all(a));
                small = *min_element(all(a));
                ll y = (big - small) / 2;
                out.printLine(y);
                pos = in.readInt() - 1;
                if (pos == -1) {
