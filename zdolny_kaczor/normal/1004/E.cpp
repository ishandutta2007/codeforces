...
#define left __left_uR7XuG
VPII graf[MN];
priority_queue<PII, VPII, greater<PII>> kol;
int furthest[MN];
int left[MN];
bool done[MN];
int ans = 0;
void enque(int x) {
	...
	for (auto v : graf[x])
		if (!done[v.f]) {
			maxi(furthest[v.f], furthest[x] + v.s);
			kol.push(MP(furthest[x] + v.s, v.f));
			cou++;
			assert(cou <= 1);
		}
}
int main() {
	...
	while (kol.size()) {
		PII t = kol.top();
		debug(imie(t), imie(left_cou));
		if (kol.size() <= 2u && left_cou <= k) {
			printf("%d\n", ans);
			return 0;
		}
		maxi(ans, t.f);
