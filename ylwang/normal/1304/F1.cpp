...
struct Segment_tree {
	...
	inline void build(int i, int k, int l, int r) {
		if(l == r) return mx[k] = f[i-1][l], tag[k] = 0, void();
...
signed main()
{
	...
	For(i, 2, n) {
		...
		For(j, 1, m - k + 1) {
			f[i][j] = tr.mx[1];
//			cerr << tr.mx[1] << ' ';
			tr.update(1, 1, m, max(1ll, j - k + 1), j, a[i][j]);
			tr.update(1, 1, m, j + 1, j + k, -a[i][j + k]);
		}
//		cerr << endl;
		For(j, 1, m - k + 1)
			f[i][j] += calc(i, i+1, j, j+k-1);
	}
//	For(i, 1, n) {
