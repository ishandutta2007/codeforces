...
void solve(){
    ...
    int mx = 2*m;
    auto dp = vec2d(m+1, n, 0);
    vector<int> consumed(n);
    int t = ord.back();
    for(int i = 0; i < n; i++) dp[0][i] = min(mx, a[i]);
    for(int i = 0; i < m; i++){
        int dp_max = *max_element(dp[i].begin(), dp[i].end());
        if(dp_max == 0){
            cout << i << endl;
            return;
        }
        for(int j = 0; j < n; j++){
            if(dp[i][j] > 0){
                ...
                dp[i+1][j] += dp[i][j]-1;
                for(int to: g[j]){
                    dp[i+1][to] = min(mx, dp[i+1][to]+dp[i][j]);
                }
            }
        }
        // debug_value(dp[i][t])
    ...
    // debug_value(total[t])
    // debug_value(m)
    // debug_value(consumed[t])
    cout << total[t]+(m-consumed[t]) << endl;
}
