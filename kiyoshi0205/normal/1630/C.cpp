...
// int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep(i,N)init[i].first=r[a[i]];
  segtree<datas> maxtree(init,datas(-1,-1),[](datas x,datas y){return max(x,y);});
  vec ch(N,0);
  rep(i,N){
    if(i)ch[i]+=ch[i-1];
    assert(ch[i]==0||ch[i]==1);
    if(ch[i]>0||i==r[a[i]])continue;
    ch[i+1]++;ch[r[a[i]]]--;
    debug(i);
    auto [val,p]=maxtree.prod(i+1,r[a[i]]);
    debug(val,p);
    if(val<=r[a[i]])continue;
    assert(i+1<=l[a[p]]&&l[a[p]]<r[a[i]]);
    ch[l[a[p]]]--;ch[r[a[i]]]++;
  }
  debug(ch);
