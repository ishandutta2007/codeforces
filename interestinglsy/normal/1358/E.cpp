...
bool Check( int k ){
	...
	For(i,n-k+1){
		if( sum[i+k-1] - sum[i-1] <= 0 ) return false;
	}
	return true;
}
...
int main(){
	...
	Read(a,1,n-n/2);
	Read(x);
	Forx(i,(n-n/2)+1,n) a[i] = x;

	For(i,n) sum[i] = sum[i-1] + a[i];

	vc<int> l = {n/2,n-n/2,n-1,n,n/2+1,(n-n/2)+1};
	...
	if( x >= 0 ){
		...
		static int lim[MAXN];
		For(i,n-(n/2)){
			ll ts = sum[n-(n/2)] - sum[i-1];
			if( ts <= 0 or ts <= -x ){
				lim[n-i+1] = INF;
				Mymax(minK,n-i+2);
			}else{
				ll can = (ts-1) / (-x);
				int maxR = min( (ll)n , n-(n/2) + can );
				lim[n-i+1] = maxR-i+1;
			}
		}
		int shouldLeq = n;
		fOR(k,n){
			Mymin(shouldLeq,lim[k]);
			if( k <= shouldLeq ){
				l.Eb(k);
				if( l.size() > 50 ) break;
			}
