...
int main() {
    ...
    F0R(i, M) {
        pl changeNew[6011];
        F0R(j, 6011) changeNew[j] = mp(0, 1);
        F0R(j, 6011) {
            if (change[j].f == 0) continue;
            ll curSum = sum + j - 3005;
            ll countPos = i - (j - 3005); countPos = countPos / 2;
            countPos = i - countPos;
            ll curPos = inPos + countPos;
            ll curNeg = curSum - curPos;
            changeNew[j+1] = add(changeNew[j+1], mul(change[j], mp(curPos, curSum)));
            changeNew[j-1] = add(changeNew[j-1], mul(change[j], mp(curNeg, curSum)));
        }
        F0R(j, 6011) {
            change[j] = changeNew[j];
        }
    }
    ...
    F0R(i, 6011) {
        ...
        if (change[i].f == 0) continue;
        ll countNeg = M - (i - 3005); countNeg = countNeg / 2;
        ll countPos = M - countNeg;
        expPos = add(expPos, mul(mp(countPos, 1), change[i]));
        expNeg = add(expNeg, mul(mp(countNeg, 1), change[i]));
    }
