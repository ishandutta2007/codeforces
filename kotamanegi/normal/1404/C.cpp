...
void solve() {
    ...
    REP(i, n) {
        ...
        cin >> a;
        if (a - (i + 1) > 0) {
            inputs.push_back(n + 1);
        }
        else {
            inputs.push_back((i + 1) - a);
        }
     }
    ...
    REP(i, query) {
        int a, b;
        cin >> a >> b;
        queries.push_back(make_tuple(a, n - b, i));
    }
    sort(ALL(queries));
    reverse(ALL(queries));
    vector<tuple<int, int,int>> going[2]; // ans, ending, itr
    for (int i = 0; i < (n+DIV-1) / DIV; ++i) {
        int l = i * DIV;
        int r = min(n, (i + 1) * DIV);
        vector<tuple<int, int, int>> goryu;
        while (queries.empty() == false and get<0>(queries.back()) < r) {
            tuple<int, int, int> now = queries.back();
            queries.pop_back();
            if (get<1>(now) <= r) {
                int val = 0;
                for (int q = get<0>(now); q < get<1>(now); ++q) {
                    if (val >= inputs[q]) {
                        val++;
                    }
                }
                answer[get<2>(now)] = val;
                continue;
            }
            int val = 0;
            for (int q = get<0>(now); q < r; ++q) {
                if (val >= inputs[q]) {
                    val++;
                }
            }
            goryu.push_back(make_tuple(val, get<1>(now), get<2>(now)));
        }
        ...
        while (diff != 1e9) {
            ...
            for (int i = l; i < r; ++i) {
                if (val >= inputs[i]) {
                    ...
                    diff = min(diff, inputs[i] - val);
                }
            }
            upgrades.push_back(mp(now_go, val - now_go));
            now_go += diff;
        }
        int itr = 0;
        REP(i, going[0].size()) {
            if (get<1>(going[0][i]) <= r) {
                for (int q = l; q < get<1>(going[0][i]); ++q) {
                    if (get<0>(going[0][i]) >= inputs[q]) {
                        get<0>(going[0][i])++;
                    }
                }
                answer[get<2>(going[0][i])] = get<0>(going[0][i]);
                continue;
            }
            while (itr + 1 != upgrades.size() and get<0>(going[0][i]) >= upgrades[itr + 1].first) {
                itr++;
            }
            while (goryu.empty() == false and get<0>(going[0][i]) + upgrades[itr].second >= get<0>(goryu.back())) {
                going[1].push_back(goryu.back());
                goryu.pop_back();
            }
            going[1].push_back(make_tuple(get<0>(going[0][i]) + upgrades[itr].second,get<1>(going[0][i]),get<2>(going[0][i])));
        }
        while (goryu.empty() == false) {
            going[1].push_back(goryu.back());
            goryu.pop_back();
        }
        going[0].clear();
        swap(going[0], going[1]);
    }
    REP(i, query) {
        cout << answer[i] << "\n";
    }
