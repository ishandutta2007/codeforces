...
struct TRIE{
	...
	inline void insert(string s) {
		...
		for (int i = 0; i < s.length(); i++)
			if (Trie[cur].child[to_num(s[i])])
				cur = Trie[cur].child[to_num(s[i])];
			else {
				cur = Trie[cur].child[to_num(s[i])] = Trie.size();
				Trie.push_back(NewTrieNode);
			}

		Trie[cur].isWord = 1;
	}

	inline void remove(string s) {
		int cur = 0;
		for (int i = 0; i < s.length(); i++)
			if (Trie[cur].child[to_num(s[i])])
				cur = Trie[cur].child[to_num(s[i])];
...
int dfs(int pos, int dep) {
	TRIE::TrieNode& node = t.Trie[pos];
	int l = node.child[0];
	int r = node.child[1];
	
	if (l == 0 && r == 0) return 0;
	if (l == 0) return dfs(r, dep-1);
	if (r == 0) return dfs(l, dep-1);
	return ((1 << dep-1) + min(dfs(l, dep-1), dfs(r, dep-1)));
}
