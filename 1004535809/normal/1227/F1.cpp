...
int main() {
	scanf("%d%d", &n, &k); init_fac(MAXN); int cnt = n, res = 0;
	if (k == 1) return puts("0"), 0;
	for (int i = 1; i <= n; i++) scanf("%d", &a[i]);
	for (int i = 1; i <= n; i++) cnt -= (a[i] == a[i % n + 1]);
	for (int i = 0; i <= cnt; i++) for (int j = 0; j < i && i + j <= cnt; j++)
		res = (res + 1ll * fac[cnt] * ifac[i] % MOD * ifac[j] % MOD * ifac[cnt - i - j] % MOD * qpow(k - 2, cnt - i - j)) % MOD;
