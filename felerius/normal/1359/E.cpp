...
// Problem: E: Modular Stability (https://codeforces.com/contest/1359/problem/E)
...
// Number of ways to partition n elements into k non-empty partitions
// O(k log n)
MI S(int n, int k, bool ordered = false) {
    MI r = 0, cki = 1;
    rep(i, k + 1) {
        r += (i % 2 ? -1 : 1) * cki * MI(k - i).pow(n);
        cki *= (k - i) / MI(i + 1);
    }
    return r / (ordered ? 1 : f(k));
}
// endregion
