...
public:
    void solve(istream& cin, ostream& cout) {
        ...
        for (int i = 1; i <= N; ++i) C[i] = C[i-1] + A - B[N-i].x;
        for (int i = 2; i <= N; ++i) D[i] = D[i-1] + (i-1) * (B[i-1].x - B[i-2].x);
...
//        cerr << "C " << C << "D " << D;
        ...
        int lo = 1;
        for (int f = N-1; f >= 0; --f) {
            while (lo + f < N && D[lo+1] + C[f] <= M) ++lo;
            ll rem = M - (D[lo] + C[f]);
            if (rem >= 0) {
                ll add = rem / lo;
                ll mxx = min(A, B[lo-1].x + add);
                ll score = mxx * Cm + f * Cf;
//                cerr << f << ' ' << lo << ' ' << add << ' ' << score << endl;
                if (score > ans) {
                    ans = score;
                    k = f;
                }
        ...
        vector<int> E(N);
        for (int j = 1; j <= k; ++j) B[N-j].x = A;
        for (int i = 0; i < N; ++i) E[B[i].y] = B[i].x;
        cout << ans << endl;
        cout << E;
    }
};
