...
void solve() {
    ...
    cp = a;
    b.rsz(n);
    F0R(i, n) b[i] = mp(a[i],i);
    sort(all(b));
    F0R(i, k) a[b[i].S]=int(1e9);
    int mn = 1e9;
    if (k<n) mn = b[k].F;
    int mx = 0;
    F0R(i, n-1) mx = max(mx, min(a[i], a[i+1]));
    int ans = min(mx, 2*mn);
    if (mx < int(1e9) && k>1) ans = max(ans, min(int(1e9), 2*b[k-1].F));
    if (k==1) {
        mx = 0;
        F0R(i, n) mx = max(mx, cp[i]);
        debug(mx, b[0].F);
        ans = max(ans, min(mx, 2*b[0].F));
    }
    cout << ans << nl;
}
