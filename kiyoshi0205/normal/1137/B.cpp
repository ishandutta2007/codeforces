...
int main(){
  ...
  ll i,j;
  cin>>s>>t;
  stringalgorithm solve;
  auto v=solve.z_algorithm(t);
  rep1(A,t.size()){
    if(v[A]+A==(ll)t.size())break;
  }
  vec cnt(2,0),allcnt(2,0),loopcnt(2,0);
  for(auto x:s)cnt[x&1]++;
  for(auto x:t)allcnt[x&1]++;
  rep(i,A)loopcnt[t[t.size()-i-1]&1]++;
  if(cnt[0]<allcnt[0]||cnt[1]<allcnt[1]){
    print(s);
    return 0;
  }
  debug(cnt);
  cout<<t;
  cnt[0]-=allcnt[0];cnt[1]-=allcnt[1];
  // debug(cnt);
  t=t.substr(t.size()-A);
  while(cnt[0]>=loopcnt[0]&&cnt[1]>=loopcnt[1]){
    cout<<t;
    cnt[0]-=loopcnt[0];cnt[1]-=loopcnt[1];
  }
  rep(i,cnt[0])cout<<0;
  rep(i,cnt[1])cout<<1;
  cout<<endl;
