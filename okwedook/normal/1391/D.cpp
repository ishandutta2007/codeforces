...
signed main() {
    ...
    vector<string> s(n);
    read(s);
    vector<vector<int>> dp(m, vector<int>(1 << n, mod));
    function<int(int)> mask = [&](int i) {
        int ans = 0;
        for (int j = 0; j < n; ++j)
            ans += (s[j][i] - '0') << j;
        return ans;
    ...
    int start = mask(0);
    for (int i = 0; i < (1 << n); ++i)
        dp[0][i] = __builtin_popcount(start ^ i);
    function<bool(int, int)> check = [&](int a, int b) {
        for (int t = 0; t + 1 < n; ++t) {
            int x = (a & (1 << t)) ^ (a & (1 << (t + 1)));
            int y = (b & (1 << t)) ^ (b & (1 << (t + 1)));
            if (__builtin_popcount(x ^ y) % 2 == 0) return false;
        }
        return true;
    };
    vector<pii> upd;
    for (int was = 0; was < (1 << n); ++was)
        for (int now = 0; now < (1 << n); ++now)
            if (check(was, now)) upd.pb({now, was});
    for (int i = 1; i < m; ++i) {
        int x = mask(i);
        for (auto v : upd)
            chmin(dp[i][v.f], dp[i - 1][v.s] + __builtin_popcount(v.f ^ x));
    }
    println(*min_element(all(dp.back())));
    #ifdef DEBUG
