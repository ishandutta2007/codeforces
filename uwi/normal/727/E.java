...
public class E {
	...
	void solve()
	{
		...
		long[] qi = new long[Q];
		for(int i = 0;i < Q;i++){
			qi[i] = (long)isa[2*s.length()+i*K]<<32|i;
		}
		Arrays.sort(qi);
		...
		for(int r = 0;r < K;r++){
			...
			for(int i = 0;i < n;i++){
				while(j < Q){
					int llcp = 0;
					if(qi[j] < poss[i]){
						llcp = rmq(st, lcp, (int)(qi[j]>>>32)+1, (int)(poss[i]>>>32)+1);
					}else{
						llcp = rmq(st, lcp, (int)(poss[i]>>>32)+1, (int)(qi[j]>>>32)+1);
					}
					if(llcp >= K){
						mats[(int)poss[i]] = (int)qi[j];
						j++;
						continue inner;
					}
					j++;
