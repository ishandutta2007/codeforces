...
{
	mint ret = 1;
	rep(i, 1, k) ret /= mint(i), ret *= mint(n+1-i);
	return ret;
}
...
int main(void)
{
	...
	rep(i, 0, 400){
		rep(j, 0, m){
			rep(k, 2, 400){
				if(i+k > 400 || j+k-1 > m) break;
				dp[i+k][j+k-1] += dp[i][j] * comb(i+k-1, k-1) * fact[k-1];
			}
		}
	}
	rep(i, 0, 400) rep(j, 0, m) if(n >= i) ans[j] += dp[i][j] * comb2(n, i); 
	
	rep(i, 1, m){
		mint sum = mint(0);
		rep(j, 0, i) if(j % 2 == i % 2) sum += ans[j];
		cout << sum << " ";
	}
	cout << endl;
