...
int main() {
	CASE(t) {
		...
		const int IMP = 1e9;
		vector<int> dp(100 * n + 7, IMP);
		dp[0] = 0;
		for (int i = 0; i < n; i++) {
			vector<int> ndp(100 * n + 7, IMP);
			int x = a[i + 1];
			int y = b[i + 1];
			for (int acost = 0; acost <= min(100 * i, ps[i]); acost++) {
				int bcost = (i == 0 ? 0 : ps[i] - acost);
				chkmin(ndp[acost + x], dp[acost] + 2 * x * acost + 2 * y * bcost);
				chkmin(ndp[acost + y], dp[acost] + 2 * y * acost + 2 * x * bcost);
			}
			dp = move(ndp);
		}
