...
_ n, m, b[18][maxn], ca[18], cb[18], cc[18], s, o, c[18][maxn], f[18][maxn];
char a[maxn];
...
inline void AC() {
    ...
    n = read();
    read(a+1);
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= 12; ++j) {
        b[j][i] = rand();
        f[j][i] = rand();
    }
    for(_ j = 1; j <= 12; ++j) {
...
void dfs(_ x, _ d) {
    ...
    bool ok = true;
    for(_ j = 1; j <= 12; ++j) {
        if(c[j][x<<1] != c[j][x<<1|1]) ok = false;
        if(c[j][x<<1] > c[j][x<<1|1]) swap(c[j][x<<1], c[j][x<<1|1]);
        c[j][x] = (c[j][x<<1]^f[j][d])*c[j][x<<1|1]%mod;
        c[j][x] ^= (a[x] == 'A' ? ca[j] : cb[j])*b[j][d]%mod;
    }
    if(ok)
        --s;
    return;
