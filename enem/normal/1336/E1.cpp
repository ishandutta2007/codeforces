...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            ll k=powM(2,n-v[0].size(),M2);
            ll dp[v[0].size()];
            dp[v[0].size()-1]=f[v[0][v[0].size()-1]];
            REPI(i,0,(ll)(v[0].size())-1)
            {
                dp[i]=(dp[i+1]^f[v[0][i]]);
            }
            ll x=0;
            ans[0]+=k;
            REP(t,1,(1LL<<v[0].size()))
            {
                x^=dp[__builtin_ctzll(t)];
                ans[__builtin_popcountll(x)]+=k;
            }
        }
        else
        {
            ll dp[1LL<<v[1].size()][v[0].size()+1],tp[1LL<<v[1].size()][v[0].size()+1];
            REP(i,0,(1LL<<v[1].size()))
            {
                REP(j,0,v[0].size()+1)
                {
                    dp[i][j]=0;
            ...
            dp[0][0]=powM(2,n-v[0].size(),M2);
            REP(i,0,v[0].size())
            {
                REP(j,0,(1LL<<v[1].size()))
                {
                    REP(k,0,v[0].size()+1)
                    {
                        tp[j][k]=dp[j][k];
            ...
            REP(i,0,(1LL<<v[1].size()))
            {
                REP(j,0,v[0].size()+1)
                {
                    ans[__builtin_popcountll(i)+j]+=dp[i][j];
                }
            }
        }
