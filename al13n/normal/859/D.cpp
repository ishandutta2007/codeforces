...
int main(int argc, char **argv){
  ...
  vector<vector<pdd>> D(N,vector<pdd>(1,mp(1.,0.)));
  for(int k=1;k<=K;++k){
    vector<vector<pdd>> DD(1<<(K-k),vector<pdd>(1<<k,mp(0.,0.)));
    forv(i,DD){
      forv(a,D[i*2]){
        forv(b,D[i*2+1]){
          ld p1=D[i*2][a].X*D[i*2+1][b].X;
          ld p2=A[a+(i<<k)][b+(1<<(k-1))+(i<<k)];
          ld t=D[i*2][a].Y+D[i*2+1][b].Y;
          
          DD[i][a].X+=p1*p2;
          DD[i][a].Y=max(DD[i][a].Y,t);

          DD[i][b+(1<<(k-1))].X+=p1*(1-p2);
          DD[i][b+(1<<(k-1))].Y=max(DD[i][b+(1<<(k-1))].Y,t);
        }
      }
      forv(c,DD[i]) DD[i][c].Y+=DD[i][c].X*(1<<(k-1));
    }
    D=move(DD);
    dbg(k,D);
  }
