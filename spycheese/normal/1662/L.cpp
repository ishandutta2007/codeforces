...
struct Fen {
  vector<T> t;
  Fen() = default;
  explicit Fen(int n) : t(n, -LINF) {}
  void add(int p, T x) { for (; p < sz(t); p |= p + 1) updMax(t[p], x); }
  T get(int p) { T s = -LINF; for (p = min(p, sz(t)) - 1; p >= 0; p = (p&(p+1))-1) updMax(s, t[p]); return s; }
};
...
void solve() {
  ...
  vector<ll> allY = {0};
  for (int i = 0; i < n; ++i) {
    allY.push_back(ps[i].Y);
  }
  unique(allY);
  Fen<ll> fen(n + 3);
  int y0 = lowerBound(allY, 0LL);
  fen.add(y0, 0);

  ll ans = 0;
  for (auto p : ps) {
    if (p < lpair(0, 0)) continue;
    int y = lowerBound(allY, p.Y);
    ll d = 1 + fen.get(y + 1);
    fen.add(y, d);
    updMax(ans, d);
  }
