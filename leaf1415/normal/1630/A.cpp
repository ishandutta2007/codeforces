...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, n/2-1) ans.pb(P(i, n-1-i));
		
		if(k == n-1){
			swap(ans[0].snd, ans[n/2-1].snd);
			swap(ans[0].fst, ans[n/2-2].snd);
		}
		else if(k > 0){
			if(k < n/2) swap(ans[0].fst, ans[k].fst);
			else swap(ans[0].fst, ans[n-1-k].snd);
		}
