#line 1 "codeforces-workspace/contest/1458/b/b.cpp"
/**
 * @file template.cpp
 * @brief Template
 */
...
#line 15 "codeforces-workspace/contest/1458/b/b.cpp"
...
void main() {
  ...
  cin >> n;
  const int inf = 1e9;
  auto dp = make_vector<int>({n + 1, 100 * n + 1}, -inf);
  dp[0][0] = 0;
  auto bsum = 0;
  for (auto i = 0; i < n; ++i) {
    int a, b;
    ...
    bsum += b;
    for (auto j = n; j > 0; j--) {
      for (auto s = a; s < size(dp[j]); s++) {
        chge(dp[j][s], dp[j - 1][s - a] + b);
      }
    }
  }
  vector<int> ans(n);
  for (auto i = 0; i < n; ++i) {
    auto& c = dp[i + 1];
    int ans = -inf;
    for (auto s = 0; s < size(c); s++) {
      chge(ans, min(2 * s, bsum + c[s]));
    }
    cout << (double)(ans) / 2 << " ";
  }
  cout << eol;
