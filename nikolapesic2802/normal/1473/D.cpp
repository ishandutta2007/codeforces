...
int main()
{
	for(int t=ri();t--;){
		...
		cin >> s;
		s='#'+s;
		vector<int> pl(n+1),pr(n+1),sum(n+1),sl(n+2),sr(n+2);
		for(int i=1;i<=n;i++){
			if(s[i]=='-')
				sum[i]=sum[i-1]-1;
			else
				sum[i]=sum[i-1]+1;
			pl[i]=min(pl[i-1],sum[i]);
			pr[i]=max(pr[i-1],sum[i]);
		}
		for(int i=n;i>0;i--){
			if(s[i]=='-')
				sl[i]=sl[i+1]-1,sr[i]=max(0,sr[i+1]-1);
			else
				sl[i]=min(0,sl[i+1]+1),sr[i]=sr[i+1]+1;
		}
		for(int i=0;i<m;i++){
			int a=ri(),b=ri();
			int l=min(pl[a-1],sl[b+1]+sum[a-1]),r=max(pr[a-1],sr[b+1]+sum[a-1]);
			printf("%i\n",r-l+1);
		}
	}
	return 0;
