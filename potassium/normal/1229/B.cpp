...
inline void dfs(int node, int par, map<ll,int> m, ll lasttotal) {
	if (w[node] == 0) {
		ans += lasttotal;
		ans %= MOD;
		m[0]++;
		for (auto& i : g[node])
			if (i != par) dfs(i,node,m,lasttotal);
	} else {
		ll total = 0;
		map<ll,int> newm;
		for (auto& i : m) {
			ll key = i.F;
			int cnt = i.S;
			key = __gcd(key,w[node]);
			newm[key] += cnt;
		}
		newm[w[node]]++;
		for (auto& i : newm)
			total += ((i.F % MOD) * i.S) % MOD, total %= MOD;
		ans += total;
		ans %= MOD;
		for (auto& i : g[node])
			if (i != par) dfs(i,node,newm,total);
	}
}
