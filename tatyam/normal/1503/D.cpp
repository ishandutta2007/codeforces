...
int main(){
    ...
    VEC(pll,a,n);
    set<ll>remain;
    rep(n*2)remain.insert(remain.end(),i);
    unordered_map<ll,ll>rev;
    vec(bool,flip,n*2);
    each(x,y,a){
        x--; y--;
        if(x>y)flip[x]=flip[y]=1;
        rev[x]=y;
        rev[y]=x;
    }
    ll lo=0,hi=2*n-1,ans=0,c0=0,c1=0,lo1=-1,lo2=-1,hi1=2*n,hi2=2*n;
    while(remain.size()){
        if(*remain.begin()==lo&&*prev(remain.end())==hi){
            ans+=min(c0,c1);
            c0=0;
            c1=0;
            ll x=lo,y=rev[x];
        ...
        elif(*remain.begin()!=lo){
            ll x=*remain.begin(),y=rev[x];
            remain.erase(x);
            remain.erase(y);
            if(lo1<x&&hi1>y){
                lo1=x;
    ...
    if(lo1>=hi2||hi1<=lo2)return out(-1);
    ans+=min(c0,c1);
    out(ans);
}