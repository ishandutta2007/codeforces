...
int n, m, k;
vpii routes;
void dijkstra(int source, int res[]) {
	memset(res, 60, sizeof(int) * nax);
	set <pii> que;
	auto touch = [&](int x, int d) {
		if (res[x] > d) {
			que.erase(pii(res[x], x));
			res[x] = d;
			que.insert(pii(res[x], x));
		}
	};
	touch(source, 0);
	while (!que.empty()) {
		auto [d, x] = *que.begin();
		que.erase(que.begin());
		for (auto [to, w] : graph[x]) touch(to, d + w);
	}
}
void solve() {
	vpii tried;
	...
	for (auto [x, y] : tried) {
		int total = 0;
		for (auto [a, b] : routes)
			total += min(dist[a][b], min(dist[a][x] + dist[y][b], dist[a][y] + dist[x][b]));
		mini(ans, total);
	}
	printf("%d\n", ans);
}
