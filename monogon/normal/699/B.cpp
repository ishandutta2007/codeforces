...
int main() {
	...
	for(int i = 0; i < n; i++) {
		b = true;
		for(SPIII it = walls.begin(); it != walls.end() && b; it++) {
			if((*it).first != i && (*it).second != point.second) {
				b = false;
			}
		}
		if(b) {
			out << "YES" << endl << i + 1 << " " << point.second + 1;
			return 0;
		}
	}
	for(int j = 0; j < m; j++) {
		b = true;
		for(SPIII it = walls.begin(); it != walls.end() && b; it++) {
			if((*it).first != point.first && (*it).second != j) {
				b = false;
			}
		}
		if(b) {
			out << "YES" << endl << point.first + 1 << " " << j + 1;
			return 0;
		}
	}
	out << "NO";
