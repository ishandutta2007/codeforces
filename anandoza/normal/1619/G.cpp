...
struct M {
    ...
    friend void pr(const M &m) { pr("[", m.index, " (", m.x, " ", m.y, ") ", m.t, "]"); }
};
...
void solve() {
    ...
    union_find uf(n);
    {
        ...
        auto mines = og_mines;
        {
            sort(all(mines), L2(u, v, u.y < v.y));
            for (const auto &[i, x, y, t] : mines) {
                by_x[x].push_back(i);
            }
        }
        {
            sort(all(mines), L2(u, v, u.x < v.x));
            for (const auto &[i, x, y, t] : mines) {
                by_y[y].push_back(i);
            }
        }
        ...
        for (auto &[_, v] : by_x) {
            for (int i = 0; i + 1 < sz(v); i++) {
                if (og_mines[v[i + 1]].y - og_mines[v[i]].y <= k) {
                    uf.unite(v[i], v[i + 1]);
                }
            }
        }
        for (auto &[_, v] : by_y) {
            for (int i = 0; i + 1 < sz(v); i++) {
                if (og_mines[v[i + 1]].x - og_mines[v[i]].x <= k) {
                    uf.unite(v[i], v[i + 1]);
                }
            }
        }
    ...
    auto possible = [&](int threshold) -> bool {
        vector<bool> boomed(n, false);
        for (auto &[i, x, y, t] : og_mines) {
            if (t <= threshold)
                boomed[uf.find(i)] = true;
        }

        int manual = 0;
        for (auto &[i, x, y, t] : og_mines)
            manual += uf.is_root(i) && !boomed[i];

        return manual <= threshold + 1;
    };

    auto ans = binarysearch(possible, 0, n + 1);
    ps(ans);
}
