...
//        just dei all
        val coods = now + 1
        val st = tempIntervalSum(coods)
        val mul = tempIntervalSum(coods)
        for(i in 0 until n){
            val pos = xs[i]
            val power = p[i]
            val now = S[pos] !!
            val left = S.ceilingEntry(pos - power).value
            val right = S.floorEntry(pos + power).value
            mul.add(now +1 ,right , -1L )
            mul.add(left, now -1 , 1L )
            st.add(now,now,power.toLong())
            st.add(now+1, right,pos.toLong() + power )
            st.add(left, now-1, -pos.toLong() + power)
//
//            just dei st.compile()
//            just dei mul.compile()
        }

        val c1 = st.compile()
        val c2 = mul.compile()
        val final = LongArray(coods){c1[it]  + c2[it] * all[it]}
        val finalminust = LongArray(coods){final[it] - all[it]}
        val finalplust = LongArray(coods){final[it] + all[it]}

        val Q = rmq(final)
        val Qminus = rmq(finalminust)
        val Qplus = rmq(finalplust)
        ...
        for(i in 0 until n){
            ...
            if(Q.query(0,left - 1) > m || Q.query(right + 1, coods - 1) > m){
                continue
            }
            if(final[now] > m + power){
                continue
            }
            if(Qplus.query(now,right) > m.toLong() + power + pos){
                continue
            }
            if(Qminus.query(left,now) > m.toLong() + power - pos){
                continue
            }
            ret[i] = true
        }
