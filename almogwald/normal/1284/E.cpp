...
int main() {
	...
	fori(i, n) {
		...
		fori(j, n) {
			...
			if (a[j].y >= a[i].y) {
				up.push_back(a[j] - a[i]);
			}
			else {
				down.push_back(a[j] - a[i]);
			}
		}
		sort(all(up), comp);
		sort(all(down), comp);
		ans += triangle((lol)up.size() - 1) * triangle((lol)down.size() - 1);
		forin(i, up) {
			int low = 0;
			int high = (lol)down.size();
			while (low < high) {
				int mid = (low + high) / 2;
				if (!comp(up[i], down[mid])) {
					high = mid;
				}
				else {
					low = mid + 1;
			...
			ans += ((low * ((lol)down.size()-low))* ((lol)down.size()-2))/2;
			ans -= (triangle(low-1)* ((lol)up.size() - i - 1));
			ans -= (triangle((lol)down.size()-low - 1) * i);
		}
		forin(i, down) {
			int low = 0;
			int high = (lol)up.size();
			while (low < high) {
				int mid = (low + high) / 2;
				if (!comp(down[i], up[mid])) {
					high = mid;
				}
				else {
					low = mid + 1;
			...
			ans += ((low * ((lol)up.size() - low)) * ((lol)up.size() - 2)) / 2;
		}
	}
	if (ans < 0) {
		a[238239].x++;
