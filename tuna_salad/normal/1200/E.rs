...
fn main() {
	let mut rin = reader();
	let mut rout = writer();
	const L: usize = 1000002;
	let base = Z::new(62);
	let code = |c: u8| -> Z {
		let x = if b'A' <= c && c <= b'Z' {
			c - b'A'
		} else if b'a' <= c && c <= b'z' {
			c - b'a' + 26
		} else if b'0' <= c && c <= b'9' {
	...
	let hash = |s: &[u8], z0: Z| -> Vec<Z> {
		let mut z = z0;
		let mut res = v!([s.len() + 1]);
		res.push(z);
		for &c in s.iter() {
			z = z * base + code(c);
			res.push(z);
	...
	rep!{[n-1]
		...
		s.extend_from_slice(&t[res..]);
		let sh1 = hash(&t[res..], sh[sn]);
		sh.extend_from_slice(&sh1[1..]);
	}
