...
void solve() {
    ...
    REP(i, n) {
        REP(q, n) {
            int b;
            cin >> b;
            if (b == 0) zeros = 1, b = 10, inputs.push_back(mp(i, q));
            while (b % 2 == 0) {
                cnt[0][i][q]++;
                b /= 2;
            }
    ...
    REP(i, 2) {
        REP(q, n) {
            REP(j, n) {
                if (j != n - 1) {
                    if (dp[i][q][j] + cnt[i][q][j + 1] < dp[i][q][j + 1]) {
                        dp[i][q][j + 1] = dp[i][q][j] + cnt[i][q][j + 1];
                        cnts[i][q][j + 1] = 0;
                    }
                }
                if (q != n - 1) {
                    if (dp[i][q][j] + cnt[i][q + 1][j] < dp[i][q + 1][j]) {
                        dp[i][q + 1][j] = dp[i][q][j] + cnt[i][q + 1][j];
                        cnts[i][q + 1][j] = 1;
                    }
                }
            }
    ...
    if (inputs.size() >= 1 and dp[0][n - 1][n - 1] != 0) {
        cout << 1 << endl;
        REP(q, 2) {
            REP(i, inputs[0].first) {
                cout << "D";
            }
            REP(i, inputs[0].second) {
                cout << "R";
            }
            inputs[0].first = n - inputs[0].first - 1;
            inputs[0].second = n - inputs[0].second - 1;
        }
        cout << endl;
        return;
    }
