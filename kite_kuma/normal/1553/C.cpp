...
int solve_min(string s, bool win_is_first) {
	const int ten = 10;
	int first_nokori, second_nokori;
	int first_win, second_win;
	first_nokori = second_nokori = second_win = first_win = 0;
	first_nokori=second_nokori=5;
	rep(i, ten) {
		if(i & 1) {	 // first
			first_nokori--;
			if(s[i] == '1') {
			...
			if(s[i] == '1') {
				...
				if(!win_is_first) second_win++;
			}
		}
		if(first_win + first_nokori < second_win || second_win + second_nokori < first_win) {
			return i + 1;
		}
	}
...
int solve(string s) { return min(solve_min(s, true), solve_min(s, false)); }
