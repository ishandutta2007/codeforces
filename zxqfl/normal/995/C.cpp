...
int main() {
  ...
  VI indices(n);
  ji(n) indices[i] = i;
  sort(indices.begin(), indices.end(), [&angle](int i, int j) { return angle[i] < angle[j]; });
  pair<ll, ll> cpos = {0, 0};
  VI sign(n);
  for (int i : indices) {
    pair<ll, ll> a1(cpos.first + x[i], cpos.second + y[i]);
    pair<ll, ll> a2(cpos.first - x[i], cpos.second - y[i]);
    if (norm(a1) < norm(a2)) {
      sign[i] = 1;
      cpos = a1;
    } else {
