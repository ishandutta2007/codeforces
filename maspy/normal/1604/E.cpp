...
void solve() {
  ...
  FOR(i, N - 1) {
    ...
    for (auto&& a: key) {
      ...
      ll n = ceil(b, a);
      ll c = b / n;
      if (newdp[c].fi == mint(0)) newkey.eb(c);
      newdp[c].fi += dp[a].fi;
      newdp[c].se += dp[a].se + dp[a].fi * mint(n - 1);
    }
    for (auto&& k: key) dp[k] = {mint(0), mint(0)};
    swap(dp, newdp);
    swap(key, newkey);
    if (dp[b].fi == mint(0)) key.eb(b);
    dp[b].fi += mint(1);
    for (auto&& k: key) { ANS += dp[k].se; }
  }
  for (auto&& k: key) dp[k] = {mint(0), mint(0)};
  print(ANS);
}
