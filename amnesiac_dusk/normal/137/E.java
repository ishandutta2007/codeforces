...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		int[]pre=new int[n+1];
		for(int i=1; i<=n; i++)
			pre[i]=pre[i-1]+(vowel[s.charAt(i-1)]?-1:2);
		int[]shifted=new int[3*n+2];
		Arrays.fill(shifted, 1000000);
		shifted[3*n+1]=-1;
		int ans=0,ct=0;
		for(int i=0; i<=n; i++)
		{
			if(i-shifted[n+pre[i]]>ans)
			{
				ans=i-shifted[n+pre[i]];
				ct=1;
			}
			else if(i-shifted[n+pre[i]]==ans)
				ct++;
			for(int j=n+pre[i]; shifted[j]>i; j++)
				shifted[j]=i;
		}
		if(ans==0)
			pr("No solution");
