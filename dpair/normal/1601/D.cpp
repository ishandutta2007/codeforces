...
struct DPair {
    int x, y;
    inline bool operator< (const DPair &tmp) const { return mx(x, y) < mx(tmp.x, tmp.y) || (mx(x, y) == mx(tmp.x, tmp.y) && (x < tmp.x || (x == tmp.x && y < tmp.y))); }
}a[MAXN];

int n, d;
signed main() {
    read(n, d); rep(i, 1, n) read(a[i].x, a[i].y);
    std::sort(a + 1, a + n + 1);
    int ans = 0; rep(i, 1, n) {
        if(a[i].x >= d) chmax(d, a[i].y), ++ ans;
    }
    print(ans);
