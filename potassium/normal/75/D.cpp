...
struct smallArray {
	ll pre = -INF, suf = -INF, full = -INF, in = -INF;
};

int main() {
	int n,m; R(n,m);
	vector<smallArray> v(n + 1);
	
	for (int i = 1; i <= n; i++) {
		int l; R(l);
		vector<int> tmp(l); R(tmp);
		
		int tot = 0;
		int maxv = 0;
		for (int j = 0; j < l; j++) {
			tot += tmp[j];
			maxv = max(maxv, 0) + tmp[j];
			chkmax(v[i].pre, tot);
			chkmax(v[i].in, maxv);
		}
	...
	for (int i = 0; i < m; i++) {
		chkmax(ans, v[x[i]].in);
		chkmax(ans, premax + v[x[i]].pre);
		premax = max(premax + v[x[i]].full, v[x[i]].suf);
	}
