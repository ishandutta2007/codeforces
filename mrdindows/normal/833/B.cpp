...
struct Tree {
    int tr[35353 * 4];
    int pu[35353 * 4];
    void set(int cur, int l, int r, int x, int y) {
        if (l == r) {
//            DBN(cur, y);
            tr[cur] = y;
        } else {
...
//            }
//            dp[j][i] = t;
            tree[j].set(1, 0, n, i, tree[j - 1].getMax());
        }
//        DBN(i, dp[i][1], dp[i][2], dp[i][3]);
        lastPos[x] = i;
    }
//    cout << dp[k][n] << endl;
    cout << tree[k].getMax() << endl;
