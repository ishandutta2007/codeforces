...
struct minop {
  li operator()(li const& a, li const& b) const { return min(a,b); }
};
...
int main(){
  ...
  int n; cin>>n;
  for(li i = 0; i < (li)(n); ++i) cin>>A[i]>>C[i];

  vi I(n); iota(begin(I), end(I),0); sort(begin(I), end(I), [&](int i, int j) { return A[i]<A[j]; });

  set<li> S;
  for(li i = 0; i < (li)(n); ++i) S.insert(A[i]);
  for(li i = 0; i < (li)(n); ++i) S.insert(A[i]+C[i]);
  vi allS(begin(S), end(S));

  map<li, li> M;
  for(li i = 0; i < (li)(allS.size()); ++i) M[allS[i]] = i;

  BIT<li, minop> bit1(allS.size()+10, 1ll<<50);
  BIT<li, minop> bit2(allS.size()+10, 1ll<<50);
  ...
  for(int j : I) {
    ...
    li d = 1ll<<50;
    smin(d, bit1.get(allS.size()+1-pa));
    smin(d, bit2.get(pa+1)+A[j]);
    if(d == (1ll<<50)) d = 0;

    bit1.add(allS.size()+1-pac, d);
    bit2.add(pac+1, d-A[j]-C[j]);
