...
main()
{
	...
	while(true)
	{
		if(!parallel(Line(Q[(id+N-1)%N],Q[id]),Line(Q[id],Q[(id+1)%N])))break;
		id++;
	}
	Polygon P;
	P.push_back(Q[id]);
	P.push_back(Q[(id+1)%N]);
	for(int i=2;i<N;i++)
	{
		Point p=Q[(id+i)%N];
		if(parallel(Line(P[P.size()-2],P.back()),Line(P.back(),p)))P.back()=p;
		else P.push_back(p);
	}
	N=P.size();
	//for(Point p:P)cout<<p<<endl;cout<<endl;
	for(;M--;)
	{
		Line a;
		cin>>a.p1>>a.p2;
		...
		for(int i=0;i<N;i++)
		{
			...
			if(!parallel(s,a)&&intersect(s,a))
			{
				Point p=crosspoint(s,a);
				if(!(p==P[i]||p==P[(i+1)%N]))X.push_back(make_pair(p,p));
			}
		}
		set<pair<Point,Point> >S;
		int id=0;
		while(parallel(a,Segment(P[id],P[(id+1)%N])))id++;
		id++;
		//cout<<id<<endl;
		for(int i=0;i<N;i++)
		{
			...
			if(intersect(a,p))
			{
				Point l=P[(id+i+N-1)%N],r=P[(id+i+1)%N];
				if(parallel(a,Line(p,r)))
				{
					//cout<<i<<" "<<p<<endl;
					Point r2=P[(id+i+2)%N];
					if(intersect(Segment(l,r2),a))X.push_back(minmax(p,r));
					else
					{
						X.push_back(make_pair(p,p));
				...
				if(intersect(Segment(l,r),a))X.push_back(make_pair(p,p));
			}
		}
		sort(X.begin(),X.end());
		//for(pair<Point,Point>x:X)cout<<x.first<<", "<<x.second<<endl;
		double ans=0;
		for(int i=0;i<X.size();i+=2)
		{
			ans+=abs(X[i+1].second-X[i].first);
			S.erase(minmax(X[i].first,X[i+1].second));
		}
		for(pair<Point,Point>s:S)ans+=abs(s.first-s.second);
