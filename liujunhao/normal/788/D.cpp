...
inline void Divide_Conqure(int l,int r){
	if(l>r)
		return;
	int mid((l+r)>>1);
	int d=ask(mid,mid);
	if(!d){
		a.pb(mid);
		Divide_Conqure(l,mid-1);
		Divide_Conqure(mid+1,r);
	}
	else{
		Divide_Conqure(l,mid-d);
		Divide_Conqure(mid+d,r);
	}
}
int main()
{
	Divide_Conqure(-1e8,1e8);
	sort(all(a));
	int t;
	for(auto i=a.begin();i<a.end();i++)
		if(*i-1>=-1e8&&(i==a.begin()||*i-1!=*(i-1))){
			t=*i-1;
			break;
		}
		else if(*i+1<=1e8&&(i==a.end()-1||*i+1!=*(i+1))){
			t=*i+1;
			break;
		}
	for(auto i:a){
