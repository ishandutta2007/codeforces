...
void solve(){
  ...
  li base = 0;
  for(li i = 0; i < (li)(n); ++i) base += (n-1)*A[i]*A[i];
  for(li i = 0; i < (li)(n); ++i) base += (n-1)*B[i]*B[i];

  vvi dp(n+1,vi(n*101, infty));
  dp[0][0] = base;
  li tots = 0;
  for(li i = 0; i < (li)(n); ++i) {
    for(li sa = 0; sa < (li)(n*101); ++sa) if(dp[i][sa] < infty) {
      li sb = tots-sa;
      smin(dp[i+1][sa+A[i]], dp[i][sa] + 2*A[i]*sa + 2*B[i]*sb);
      smin(dp[i+1][sa+B[i]], dp[i][sa] + 2*B[i]*sa + 2*A[i]*sb);
    }
    tots += A[i]+B[i];
  }

  li ans = infty;
  for(li sa = 0; sa < (li)(n*101); ++sa) smin(ans, dp[n][sa]);
  cout << ans << endl;
}
