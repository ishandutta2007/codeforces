...
int n, m, limit, a[N], last[M];
ModT sum_all[N + 1], sum[N + 1][M];
...
int main() {
  ...
  if (m == 1) {
    ...
    std::vector<std::pair<int, int>> best2{{0, 0}, {1, 0}};
    sum_all[1] = ModT{1};
    for (int i = 1; i <= n; ++i) {
      if (~a[i]) {
        last[a[i]] = i;
        auto iterator = std::find_if(
            best2.begin(), best2.end(),
            [&](const std::pair<int, int> &p) { return p.first == a[i]; });
        if (iterator != best2.end()) {
          best2.erase(iterator);
        } else {
          best2.pop_back();
        }
        best2.insert(best2.begin(), {a[i], i});
      }
      //   std::cout << KV(i) << KV(a[i]) << KV(best2) << std::endl;
      sum_all[i + 1] = sum_all[i];
      for (int j = 0; j < m; ++j) {
        int bound = std::max(best2[best2[0].first == j].second, i - limit);
        auto dp = (sum_all[i] - sum_all[bound]) - (sum[i][j] - sum[bound][j]);
        // std::cout << KV(i) << KV(j) << KV(bound) << KV(dp) << std::endl;
        sum_all[i + 1] += dp;
        sum[i + 1][j] = sum[i][j] + dp;
      }
    }
    printf("%d\n", (sum_all[n + 1] - sum_all[n]).get());
  }
}