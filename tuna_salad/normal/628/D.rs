...
fn rec(p: usize, x: usize, y: usize, a: &[u8], dp: &mut [Vec<Vec<Option<Z>>>], m: usize, d: usize) -> Z {
	if let Some(ans) = dp[p][x][y] {
		return ans;
	}
	let mut res = Z0;
	let n = a.len();
	if p == n {
		...
		let x = x * 10;
		let dd = if y == 1 { (a[p]-b'0') as usize } else {9};
		if p & 1 == 0 {
			for i in 0..=dd {
				if i == d {
					continue;
				...
				let xx = (x + i) % m;
				let yy = if y == 1 && i == dd {1} else {0};
				res += rec(p+1, xx, yy, a, dp, m, d);
			}
		} else {
			if d <= dd {
				let i = d;
	...
	dp[p][x][y].replace(res);
	res
}

fn calc(a: &[u8], m: usize, d: usize) -> Z {
	let n = a.len();
	let mut dp = v!([n+1][m][2] = None);
	rec(0, 0, 1, a, &mut dp, m, d)
}

fn check_a(a: &[u8], m: usize, d: usize) -> bool {
	let mut x = 0;
...
fn main() {
	...
	l!(m,d = rin.u());
	l!(a,b = rin.vb());
	let mut sol = if check_a(&a, m, d) {Z1} else {Z0};
	if a != b {
		sol += calc(&b, m, d) - calc(&a, m, d);
	}
	writeln!(rout, "{}", sol).ok();
}