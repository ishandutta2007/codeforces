...
struct Dinic {
  ...
  vector<Pr> output(ll N) {
    ...
    vector<ll> covering_vertex(ll N){ // S:0-(1-N)-(N+1-2N)-T:2N+1 
        vector<int> red;
        for(auto &e : graph[0]) {
            if(e.isrev) continue;
            auto &rev_e = graph[e.to][e.rev];
...
int main(){
    ...
    ll F = G.max_flow(0,2*N+1);
    auto v = G.covering_vertex(N);
    //out(v);
    ...
    auto dp = v2ll(N+1,2*F+1,2e18); dp[0][0] = 0;
    auto pv = v2ll(N+1,2*F+1,-1);
    rep(i,N){
        rep(j,F+1){
            rep(k,F+1){
                if(dp[i+1][j+k]>dp[i][j]+min((ll)k*y[i],x[i])){
                    pv[i+1][j+k] = j;
                }
                chmin(dp[i+1][j+k],dp[i][j]+min((ll)k*y[i],x[i]));
            }
        }
        rep(j,F+1){
            if(N-F+j<=i+1) dp[i+1][j] = 2e18;
        }
    }
