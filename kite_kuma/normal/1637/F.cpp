...
int main() {
	...
	int root = 0;
	rep(i, n) if(h[i] > h[root]) root = i;

	debug(root);

	bool root_is_leaf = (g[root].size() == 1);

	auto dfs = [&](auto self, int now, int par) -> vector<ll>  // sum, first, second;
	{
		ll sum = 0;
		ll first = 0;
		ll second = 0;
		foa(e, g[now]) {
			...
			auto res = self(self, e.to, now);
			sum += res[0];
			rep(i, 1, 2) {
				ll value = res[i];
				if(chmax(second, value)) {
		...
		if(par == -1) {
			if(root_is_leaf) {
				...
				second = first;
				first = h[now];
			} else if(second < h[now]) {
				sum += h[now] - second;
				second = h[now];
			}
		}
		// debug(now, sum, first, second);
		return vector<ll>{sum, first, second};
	};

	// debug(dfs(dfs, root, -1));

	drop(dfs(dfs, root, -1).at(0));
