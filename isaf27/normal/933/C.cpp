...
{   //*
    ...
    for (int i = 0; i < n; i++)
    {
        p.push_back(point(c[i].p.x - c[i].r, c[i].p.y));
        p.push_back(point(c[i].p.x + c[i].r, c[i].p.y));
    }
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++)
        {
            int kol = kol_intersec_circle_circle(c[i], c[j]);
            if (kol == 0 || kol == 3)
                continue;
            pair<point, point> nw = intersection_circle_circle(c[i], c[j]);
            p.push_back(nw.first);
            if (kol == 2)
                p.push_back(nw.second);
        }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        for (int j = 0; j < (int)id.size() - 1; j++)
            w.merge_set(id[j], id[j + 1]);
        //cerr << id.size() << "\n";
    }
    for (int i = 0; i < w.parent.size(); i++)
        ans += (i == w.parent[i]);
    ans -= (int)p.size();
    cout << ans;
    return 0;
