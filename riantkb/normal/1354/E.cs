    ...
    public void solve() {
        ...
        for (int i = 0; i < s; i++)
        {
            for (int j = 0; j < b + 1; j++)
            {
                for (int k = 0; k < 2; k++)
                {
                    if (dp[i][j] && j + lis[i][k].Count < b + 1) {
                        dp[i + 1][j + lis[i][k].Count] = true;
                        used[i + 1][j + lis[i][k].Count] = k;
                    }
                }
            }
        }
        ...
        for (int i = s - 1; i >= 0 ; i--)
        {
            foreach (var item in lis[i][used[i + 1][pj]])
            {
                isb[item] = true;
            }
            pj -= lis[i][used[i + 1][pj]].Count;
        }
        Assert(pj == 0);
        var ans = new char[n];
        for (int i = 0; i < n; i++)
