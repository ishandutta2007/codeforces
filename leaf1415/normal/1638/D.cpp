...
ll check(ll x, ll y)
{
	...
	ll ret = max(max(a[x][y], a[x+1][y]), max(a[x][y+1], a[x+1][y+1]));
	
	if(a[x][y] != 0 && a[x][y] != ret) return 0;
	if(a[x][y+1] != 0 && a[x][y+1] != ret) return 0;
	if(a[x+1][y] != 0 && a[x+1][y] != ret) return 0;
	if(a[x+1][y+1] != 0 && a[x+1][y+1] != ret) return 0;
...
int main(void)
{
	...
	reverse(all(ans));
	outl(sz(ans));
	for(auto t : ans) cout << t.fi.se << " " << t.fi.fi << " " << t.se << "\n";
