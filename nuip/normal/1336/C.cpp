...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	const int n=s.size();
	const int m=t.size();
	rep(i,n)if(i>=m || s[0]==t[i]) dp[i][i]=2;
	reps(i,1,n){
		const char c=s[i];
		const int d=i+1;
		rep(l,n+1-d){
			const int r=l+d-1;
			if(l>=m || c==t[l]) dp[l][r]+=dp[l+1][r];
			if(r>=m || c==t[r]) dp[l][r]+=dp[l][r-1];
		}
	}
	// rep(i,n){rep(j,n) cout<<dp[i][j]<<","; cout NL;}
	cout<<accumulate(&dp[0][m-1],dp[1],mint(0)) NL;
  return 0;
}