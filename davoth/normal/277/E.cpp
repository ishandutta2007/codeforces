...
int main(){
    fast_io;
    cin >> n;
    Flow<ll,ld> flow(2*n+2);
    for(ll i=1; i<=n; i++){
        cin >> X[i] >> Y[i];
        flow.AddEdge(0,i,2,0);
        flow.AddEdge(i+n,n*2+1,1,0);
        for(ll j=1; j<i; j++){
            if(Y[j]<Y[i]){
                flow.AddEdge(i,j+n,1,di(i,j));
            }
            if(Y[j]>Y[i]){
                flow.AddEdge(j,i+n,1,di(i,j));
            }
        }
    }
    pair<ll,ld> ans=flow.MinCost(0,n*2+1);
    if(ans.F<n-1) cout << -1 << '\n';
    else cout << fixed << setprecision(7) << ans.S << '\n';
    return 0;
}