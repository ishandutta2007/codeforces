...
int main(){
    ...
    len.back()=1;
    rrep(i,1,n-1){
        g.add_directed_edge(i,n-1,1);
        len[i]=1;
        rrep(j,i+1,n-1){
            g.add_directed_edge(i,j,len[i]);
            len[i]+=len[j];
        }
    }
    if(l==1){
        g.add_directed_edge(0,n-1,1);
        l++;
    }
    while(r>=l){
        ll i=__lg(r-l+1);
        const ll at=n-2-i;
        g.add_directed_edge(0,n-2-i,l-1);
        l+=1LL<<__lg(r-l+1);
    }
    vector<tuplis>edge;
    rep(n)each(j,g[i])edge.push_back({i,j,j.cost});
    out(n,edge.size());
    each(i,j,k,edge)out(i+1,j+1,k);
}