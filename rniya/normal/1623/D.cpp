...
void solve() {
    ...
    cin >> n >> m;
    vector<int> r(2), c(2);
    for (int i = 0; i < 2; i++) cin >> r[i] >> c[i], r[i]--, c[i]--;
    int p;
    cin >> p;

    vector<tuple<int, int, int>> cycle;
    set<tuple<int, int, int>> s;
    int x = r[0], y = c[0], dir = 0;

    while (!s.count({x, y, dir})) {
        cycle.emplace_back(x, y, dir);
        s.emplace(x, y, dir);
        if (x + dx[dir] < 0 or n <= x + dx[dir]) dir ^= 2;
        if (y + dy[dir] < 0 or m <= y + dy[dir]) dir ^= 1;
        x += dx[dir];
        y += dy[dir];
    }
    ...
    while (cycle.back() != make_tuple(x, y, dir)) {
        path.emplace_back(cycle.back());
        cycle.pop_back();
    }
    assert(!cycle.empty());
    reverse(cycle.begin(), cycle.end());

    mint ans = 0, suc = mint(p) / 100, cur = 1;
    int time = 0;
    for (auto& p : path) {
        if (get<0>(p) == r[1] or get<1>(p) == c[1]) {
            ans += cur * suc * time;
            cur *= 1 - suc;
        }
        time++;
    ...
    sum = (sum + cur * int(cycle.size())) / (1 - cur);
    ans += (sum + time1) * cur1;
