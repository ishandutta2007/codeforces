...
{
	int n,A,R,M;
	scanf("%i %i %i %i",&n,&A,&R,&M);
	vector<int> h(n);
	int mx=0,mn=INT_MAX;;
	for(int i=0;i<n;i++)
		scanf("%i",&h[i]),mx=max(mx,h[i]),mn=min(mn,h[i]);
	auto getCost=[&](int d){
		ll over=0,under=0;
		for(int i=0;i<n;i++)
			if(h[i]>d)
				...
				under+=d-h[i];
		ll moze=min(over,under);
		return min(moze*M+(over-moze)*R+(under-moze)*A,over*R+under*A);
	};
	int l=mn,r=mx;
	while(l<r){
		int m=(l+r)>>1;
		ll a=getCost(m),b=getCost(m+1);
		if(a<b)
			r=m;
		else
			l=m+1;
