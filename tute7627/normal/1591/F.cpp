...
int main(){
  ...
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  auto [l,r]=val_range(a,true);
  auto dp=vec(n+2,2,modint(0));
  auto cum=vec(n+2,2,modint(0));
  dp[0][0]=1;
  dp[1][0]=-1;
  cum[1][0]=1;
  rep(i,0,n){
    if(i>0){
      rep(j,0,2){
        dp[i][j]+=dp[i-1][j];
        cum[i+1][j]=cum[i][j]+dp[i][j];
      }
    }
    rep(j,0,2){
      modint add=cum[i+1][j]-cum[l[i]+1][j];
      dp[i+1][j^1]+=add*a[i];
      dp[r[i]+1][j^1]-=add*a[i];
    }
  }
  rep(j,0,2)dp[n][j]+=dp[n-1][j];
  cout<<(dp[n][0]-dp[n][1])*(n%2==0?1:-1)<<endl;;
  return 0;
}