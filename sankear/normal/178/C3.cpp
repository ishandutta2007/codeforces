...
int sz, h, m, res, cnt;
bool used[200200];
int col[200200], pos[200200], maxv[200200], need[200200];
...
void dfs(vector <tr> &rmq, int v, int l, int r){
...
void dfs1(vector <tr> &rmq, int maxv, int v){
	if(v >= maxv){
		res = min(res, v - maxv);
		return;
	}
...
int main(){
	...
	for(int i = 0; i < n; i++){
		...
		if(ch == '+'){
			...
			res = inf;
			cnt = 0;
			dfs(rmq[col[hash]], 1, pos[hash], (int)lst[col[hash]].size() - 1);
			for(int j = 0; j < cnt; j++){
				if(rmq[col[hash]][need[j]].val == 0){
					dfs1(rmq[col[hash]], maxv[col[hash]], need[j]);
					break;
				}
			}
			if(res == inf){
				...
				cnt = 0;
				dfs(rmq[col[hash]], 1, 0, pos[hash] - 1);
				for(int j = 0; j < cnt; j++){
					if(rmq[col[hash]][need[j]].val == 0){
						dfs1(rmq[col[hash]], maxv[col[hash]], need[j]);
						break;
