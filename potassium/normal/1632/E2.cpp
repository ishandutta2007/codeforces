...
const int NINF = -1e7;
S e() {
	return S{ NINF, NINF, NINF, NINF, NINF };
}
...
int main() {
	CASE(t) {
		...
		function<void(int, int)> dfs_euler = [&](int u, int p) {
			pos[u].emplace_back((int)euler.size());
		...
		for (int x = 1, k = -1; x <= n; x++) {
			while (st.all_prod().a[4] > 2 * (k - x)) {
				k++;
				for (auto& u : dep[k])
					for (auto& p : pos[u])
						st.set(p, S{ NINF, -2 * d[u], NINF, NINF, NINF });
			}
			ans[x - 1] = k;
		}
