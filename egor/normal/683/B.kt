...
fun solve(inp: InputReader, out: OutputWriter, testNumber: Int) {
    var n = inp.readInt()
    var students = Array<Student>(n,
        { var name = inp.readString()
            var height = inp.readInt()
            return@Array Student(name, height)
        })
    Arrays.sort(students, Comparator { a, b -> a.height - b.height })
    for (student in students) {
        out.printLine(student.name)
    }
}
