...
int main() {
	...
	auto blocked = [&](int x, int y) {
		for (int i = 1; i <= 666; i++)
			if (pos[i] == make_pair(x, y)) return true;
		return false;
	};

	auto mov = [&](int nx, int ny) {
		assert(abs(nx - x) <= 1 && abs(ny - y) <= 1);
	...
	while (x != 500 || y != 500) {
		int dx = x == 500 ? 0 : (500 - x) / abs(500 - x);
		int dy = y == 500 ? 0 : (500 - y) / abs(500 - y);
		mov(x + dx, y + dy);
	}
	...
	for (int i = 1; i <= 666; i++) {
		int mask = 0;
		if (pos[i].first > 500) mask |= 1;
		if (pos[i].second > 500) mask |= 2;
		cnt[mask]++;
	}
	
	for (int quad = 0; quad < 4; quad++) {
		int sum = cnt[quad] + cnt[quad ^ 1] + cnt[quad ^ 2];
		if (sum <= 499) continue;
