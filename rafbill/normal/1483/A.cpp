...
void solve(){
  ...
  for(li i = 0; i < (li)(m); ++i) {
    ...
    for(li j = 0; j < (li)(k); ++j) { cin>>A[i][j]; --A[i][j]; }
  }
  vi S(m,0);
  vi C(n);
  for(li i = 0; i < (li)(m); ++i)C[A[i][0]]++;
  int ix = -1;
  for(li i = 0; i < (li)(n); ++i) if(C[i]>(m+1)/2) ix = i;
  if(ix != -1) {
    for(li i = 0; i < (li)(m); ++i) if(A[i].size() >= 2 && A[i][0] == ix && C[ix]>(m+1)/2) {
      S[i] = 1;
      C[ix]--;
    }
    if(C[ix] > (m+1)/2) {
  ...
  cout << "YES\n";
  for(li i = 0; i < (li)(m); ++i) cout << 1+A[i][S[i]] << ' ';
  cout << '\n';
}
