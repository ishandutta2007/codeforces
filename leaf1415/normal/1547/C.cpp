...
int main(void)
{
	...
	while(T--){
		cin >> k >> n >> m;
		queue<ll> A, B; ll x, y;
		rep(i, 1, n) cin >> x, A.push(x);
		rep(i, 1, m) cin >> y, B.push(y);
		
		vector<ll> vec;
		while(sz(A) || sz(B)){
			bool flag = false;
			if(sz(A) && A.front() <= k) flag = true;
			if(sz(B) && B.front() <= k) flag = true;
			if(!flag){
				outl(-1);
				goto end;
			}
			...
			if(sz(A)){
				if(A.front() == 0) k++;
				if(A.front() <= k) vec.push_back(A.front()), A.pop();
			}
			if(sz(B)){
				if(B.front() == 0) k++;
				if(B.front() <= k) vec.push_back(B.front()), B.pop();
			}
		}
		outl(vec);
		end:;
