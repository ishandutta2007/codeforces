...
const int maxn = 6e5 + 100, maxw = 3e3 + 10, inf = 1e9 + 100, LG = 18, SQ = 300;
...
//        t = 2;
    ...
    for (int it = 0; it < zaps; it++) {
        if (ques[it].size() == 2) {
            int id = ques[it][0], w = ques[it][1];
            w = low_bo(vals, w);
            for (int i = 0; i < B; i++) {
                int df = has[i][w] - has[i][a[id]];
                if (df != 0)
                    f[i].inc(id, df);
            }
            a[id] = w;
        } else {
            int l = ques[it][0], r = ques[it][1], k = ques[it][2];
            bool good = 1;
            for (int i = 0; i < B; i++)
                if (f[i].get(l, r) % k != 0) {
                    good = 0;
