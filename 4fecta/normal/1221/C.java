//package cfperfectteam;

import java.io.*;
import java.util.*;

public class Cfperfectteam {
    ...
    public static void main(String[] args) throws IOException {
        int q = readInt();
        for (; q > 0; q--) {
            int c = readInt(), m = readInt(), x = readInt();
            int cnt = 0;
            if (x > 0) {
                int min = Math.min(Math.min(c, m), x);
                c -= min;
                m -= min;
                x -= min;
                cnt += min;
            ...
            if (c != m) {
                int min = Math.min(Math.min(c, m), Math.abs(c - m));
                if (m > c) {
                    m -= 2 * min;
                    c -= min;
                } else {
