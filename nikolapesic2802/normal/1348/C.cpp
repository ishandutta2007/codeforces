...
// URL : http://codeforces.com/contest/1348/problem/C
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
void test(){
	...
	if(koliko[0].f==k){
		...
		if(koliko.size()>2){
			for(int i=1;i<koliko.size();i++)
				for(int j=0;j<koliko[i].f;j++)
					sol+=koliko[i].s;
		}
		else{
			for(int i=0;i<koliko[1].f/k;i++)
				sol+=koliko[1].s;
			if(koliko[1].f%k)
				sol+=koliko[1].s;	
		}
	...
	if(koliko[0].f>k){
		if(koliko.size()==1){
			...
			string sol;
			for(int i=0;i<koliko[0].f-k+1;i++)
				sol+=koliko[0].s;
			for(int i=1;i<koliko.size();i++)
					for(int j=0;j<koliko[i].f;j++)
						sol+=koliko[i].s;
