...
fn run() {
    ...
    let mut divisor = vec![];
    let mut dp = vec![];
    let mut map = InitArray::new(0u32, m + 1);
    for (g, memo) in memo.iter_mut().enumerate().skip(1) {
        map.init();
        for k in 1..=(n / g) {
            let mut a = a[k * g - 1];
            *memo += M::from(a);
            ...
            while a > 1 {
                ...
                step.push((len, divisor.len()));
            }
            for &d in divisor.iter() {
                dp.push(map[d]);
                map[d] += 1;
            }
            for (a, b) in step.drain(..) {
                for dp in dp.chunks_exact_mut(b) {
                    for i in a..b {
                        dp[i - a] -= dp[i];
                    }
                }
            }
            for (dp, d) in dp.drain(..).zip(divisor.drain(..)) {
                *memo += M::new(dp) * M::from(d) * M::new(2);
            }
        }
    }
    for i in 2..=m {
