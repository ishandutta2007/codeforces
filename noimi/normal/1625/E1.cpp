...
template <typename T = int> struct Doubling {
    ...
    Doubling(const vector<int> &_nxt, const vector<T> &_cost) {
        n = _nxt.size();
        LOG = 32 - __builtin_clz(n) + 1;
    ...
    Doubling(const vector<int> &_nxt) : Doubling(_nxt, vi(si(_nxt), 1)) {}
...
int main() {
    ...
    vl d(n + 1);
    per(i, n) if(to[i] != -1) { d[i] = d[to[i]] + 1; }
    ...
    while(q--) {
        INT(t, l, r);
        l--;
        int k = db.move_cnt(l, r);
        ll res = seg.prod(l, r) + 1LL * k * (k + 1) / 2;
        int p = d[l];
        dump(res, k);
        res -= 1LL * k * (p + p - k + 1) / 2;
        OUT(res);
