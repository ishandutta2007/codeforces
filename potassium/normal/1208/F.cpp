...
int main() {
	...
	vector<pii> pos(FULL, { -1, -1 });
	auto push = [&](int p, int idx) {
		if (idx > pos[p].first) {
			pos[p].second = pos[p].first;
			pos[p].first = idx;
		}
		else if (idx != pos[p].first && idx > pos[p].second) {
			pos[p].second = idx;
		}
	};
	...
	for (int b = 0; b < BITS; b++)
		for (int mask = 0; mask < FULL; mask++) {
			...
			push(mask, pos[mask | (1 << b)].first);
			push(mask, pos[mask | (1 << b)].second);
		}
	...
	for (int i = 0; i + 2 < n; i++) {
		...
		for (int b = BITS - 1; b >= 0; b--) {
			...
			int nxtMask = curMask | (1 << b);
			if (i < pos[nxtMask].second)
				curMask = nxtMask;
		}

		chkmax(ans, a[i] | curMask);
	}
