...
signed main(){
    ...
    Dinic cacflow(1 + nodeb + nodec + 1, 0, nodeb + nodec + 1);
    ForE(i, 1, nodeb){
        cacflow.add_edge(0, i, 1);
    }
    ForE(i, 1, nodec){
        cacflow.add_edge(nodeb + i, nodeb + nodec + 1, 1);
    }
    ForE(i, 2, n){
        ForE(j, 1, m){
            if (b[i][j]){
                if (c[i][j]){
                    cacflow.add_edge(idxb[i][j], nodeb + idxc[i][j], 1);
                }
                if (c[i][j + 1]){
                    cacflow.add_edge(idxb[i][j], nodeb + idxc[i][j + 1], 1);
                }
                if (c[i - 1][j]){
                    cacflow.add_edge(idxb[i][j], nodeb + idxc[i - 1][j], 1);
                }
                if (c[i - 1][j + 1]){
                    cacflow.add_edge(idxb[i][j], nodeb + idxc[i - 1][j + 1], 1);
                }
            }
        }
    }
    cout << cells - (nodeb + nodec - cacflow.flow());
}
