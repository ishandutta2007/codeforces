...
void main_() {
	...
	if(n <= 3) {
		...
		print(1, 3, 5, 7, 4, 8, 6, 2);
	} else {
		V<> res;
		if(n & 1) {
			REP(i, 1, n + 1, 2) { res.pb(i); }
			res.pb(n - 3);
			res.pb(n - 1);
			for(int i = n - 5; i >= 2; i -= 2) res.pb(i);
		} else {
			res = {2, 4, 1, 5, 3};
			REP(i, 6, n + 1, 2) { res.pb(i); }
			if(n >= 10) {
				res.pb(n - 3);
				res.pb(n - 1);
			}
			for(int i = n - 5; i >= 6; i -= 2) res.pb(i);
		}
		print(res);
	}
}
