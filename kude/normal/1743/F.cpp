...
M e() {
  ...
  t1[0] = {1, 2};
  t1[1] = {1, 2};
  constexpr int MX = 300010;
  vector<char> s1(MX);
  {
    int l, r;
    cin >> l >> r;
    for(int i = l; i <= r; i++) s1[i] = 1;
  }
  segtree<M, op, e> seg(vector<M>(n - 1, t0));
  vector<vector<pair<int, bool>>> qs(MX);
  rep(i, n - 1) {
    int l, r;
    cin >> l >> r;
    qs[l].emplace_back(i, true);
    qs[r+1].emplace_back(i, false);
  }
  mint ans;
  rep(i, MX) {
    for(auto [idx, type] : qs[i]) {
      seg.set(idx, type ? t1 : t0);
    }
    ans += seg.all_prod()[s1[i]][1];
  }
  cout << ans.val() << '\n';
}