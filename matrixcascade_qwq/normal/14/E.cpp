...
signed main()
{
	...
	{
		rdn;rdk;
		dp[2][2][1][1]=1;dp[2][3][1][1]=2;dp[2][4][1][1]=3;
		up(i,3,n)
		{
			up(j,1,4)
			{
				up(t,1,k)
				{
					pu(q,4,j+1)dp[i][j][t][0]+=dp[i-1][q][t][0]+dp[i-1][q][t][1];
					up(q,1,j-1)dp[i][j][t][1]+=dp[i-1][q][t][1]+dp[i-1][q][t-1][0];
				}
			}
		}
		int w=0;
		up_(20)w+=dp[n][i][k][0];opl(w);
	}
}
/*
