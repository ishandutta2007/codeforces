...
template<typename T, typename F> struct segment_tree {
	...
	void join_in_place(int i, T v) {
		i += SZ;
		table[i] = tt(table[i], v);
		rebuild(i);
	}
...
int main() {
    ...
    segment_tree live(sz(uniq), -1, [](int a, int b) { return max(a, b); });

	auto get_last = [&](int lo, int hi) {
		return live(lb(all(uniq), lo) - uniq.begin(), lb(all(uniq), hi) - uniq.begin());
	};

	segment_tree laugh(N, INT_MAX, [](int a, int b) { return min(a, b); });
    ...
    F0R (r, N) {
		{ // a[l] < a[r]
			int previous = r;
			while (true) {
				int next = get_last(previous < r ? (a[r] + a[previous] + 1) / 2 : INT_MIN, a[r] + 1);
				if (next == -1 || next == previous) break;
				laugh.join_in_place(next, a[r] - a[next]);
				previous = next;
			}
		}
		{ // a[l] > a[r]
			int previous = r;
			while (true) {
				int next = get_last(a[r], previous < r ? (a[r] + a[previous]) / 2 + 1 : INT_MAX);
				if (next == -1 || next == previous) break;
				laugh.join_in_place(next, a[next] - a[r]);
				previous = next;
			}
		}
		{
			live.replace(index(uniq, a[r]), r);

			for (auto [l, i] : query[r])
				love[i] = laugh(l, laugh.SZ);
		}
    }
