...
template<class T, int SZ> struct BIT {
    ...
    ll getFst(ll mx) {
        ...
        // pr("AH",cur,res,mx);
        if (cur+1 < MX) res.s += (mx-res.f)/(cur+1);
        return res.s;
    }
};
...
void dfs(int v, ll lef) {
    S.upd(t[v],{(ll)x[v]*t[v],x[v]});
    val[v] = S.getFst(lef);
    // pr(v,lef,val[v]);
    trav(w,child[v]) dfs(w.f,lef-2*w.s);
    S.upd(t[v],{-(ll)x[v]*t[v],-x[v]});
}
...
int main() {
    ...
    FORd(i,1,n+1) {
        vl v;
        trav(t,child[i]) v.pb(ans[t.f]);
        sort(all(v));
        if (i != 1 && sz(v)) v.pop_back();
        ans[i] = val[i];
        if (sz(v)) ckmax(ans[i],v.back());
    }
    pr(ans[1]);
    // you should actually read the stuff at the bottom
}
