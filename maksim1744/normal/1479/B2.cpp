...
int main() {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        tree.modify(0, tree.n, 1);
        int without = min({tree.ask(0, k - 1).mn + 1, tree.ask(k + 1, tree.n).mn + 1, tree.ask(k, k).mn}) - 1;
        tree.set(a[i - 1], without);
    }

    cout << tree.ask(0, tree.n).mn << '\n';
