...
void solve() {
    ...
    F0Rd (bit, 31) {
        const bool lv = (L >> bit)&1, rv = (R >> bit)&1;
        fill(all(DP), 0);
        F0R (s, sz(dp)) {
            bool al = s&1, ar = (s>>1)&1, bl = (s>>2)&1, br = (s>>3)&1;
            F0R (av, 2) F0R (bv, 2) {
                if ((av && bv)) continue;
                if ((al && av < lv) || (ar && av > rv)) continue;
                if ((bl && bv < lv) || (br && bv > rv)) continue;
                DP[(al && av == lv) * 0b0001
                 + (ar && av == rv) * 0b0010
                 + (bl && bv == lv) * 0b0100
                 + (br && bv == rv) * 0b1000] += dp[s];
            }
        }
        swap(dp, DP);
    }

    ps(accumulate(all(dp), 0ll));
}
