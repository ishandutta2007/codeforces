...
S e() {
	return S{ INT_MIN, INT_MAX, INT_MIN, INT_MAX };
}
...
int main() {
	...
	for (int i = 0; i < n; i++) {
		if (pref[i] == 'L') st_base[i].lmax = a[i];
		if (pref[i] == 'R') st_base[i].rmin = a[i];
		st_base[i].maxv = st_base[i].minv = a[i];
	}
