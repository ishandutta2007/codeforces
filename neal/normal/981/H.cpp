...
vector<mod_int> dp_sum;
mod_int total;

vector<mod_int> K_permute;

mod_int compute_dp(const vector<mod_int> &poly) {
    mod_int dp = 0;

    for (int i = 0; i < (int) poly.size(); i++)
        dp += K_permute[i] * poly[i];

    return dp;
}

// Divides `poly` by 1 + ax and computes DP on the new polynomial.
mod_int divide_and_compute_dp(const vector<mod_int> &poly, mod_int a) {
    assert(poly.size() > 1);
    mod_int carry = 0, dp = 0;

    for (int i = 0; i < (int) poly.size(); i++) {
        carry = poly[i] - a * carry;
        dp += K_permute[i] * carry;
    }
...
void dfs(int node, int parent) {
    ...
    for (int child : adj[node]) {
        ...
        // Compute all non-vertical paths with `node` as the LCA.
        total += dp_sum[node] * dp_sum[child];
        dp_sum[node] += dp_sum[child];

        sizes.emplace_back(subtree_size[child], child);
    }

    vector<vector<mod_int>> polynomials;
    sort(sizes.begin(), sizes.end());

    for (int i = 0, j = 0; i < children; i = j) {
        while (j < children && sizes[i].first == sizes[j].first)
            j++;

        polynomials.push_back(NTT::mod_power({1, sizes[i].first}, j - i));
    }

    vector<mod_int> product = NTT::mod_multiply_all(polynomials);
    mod_int dp_node = compute_dp(product);
    dp_sum[node] += dp_node;

    // Compute all vertical paths where `node` is the higher node.
    product = NTT::mod_multiply(product, {1, N - subtree_size[node]});

    for (int i = 0, j = 0; i < children; i = j) {
        mod_int sum_of_dp = 0;

        while (j < children && sizes[i].first == sizes[j].first) {
            sum_of_dp += dp_sum[sizes[j].second];
            j++;
        }

        // Don't repeat the division and DP computation for the same size twice. This results in at most sqrt(N)
        // computations since the sum of sizes is at most N.
        mod_int dp_up = divide_and_compute_dp(product, sizes[i].first);
        total += dp_up * sum_of_dp;
    }
}

int main() {
    IO::read_int(N, K);
    K_permute.assign(max(N, K) + 1, 0);
    K_permute[0] = 1;

    for (int i = 0; i < K; i++)
        K_permute[i + 1] = K_permute[i] * (K - i);
