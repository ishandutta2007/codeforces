...
int main(){
  ...
  lng hy,ay,dy,hm,am,dm,h,a,d;
  cin>>hy>>ay>>dy>>hm>>am>>dm>>h>>a>>d;
  lng ans=LINF;
  for(lng dd=0;dd<1000;++dd){
    for(lng aa=0;aa<1000;++aa){
      if(ay+aa<=dm)
        continue;
      lng t=(hm+ay+aa-dm-1)/(ay+aa-dm);
      lng hh=max(0ll,1+t*max(0ll,am-dy-dd)-hy);
      lng c=dd*d+aa*a+hh*h;
      ans=min(ans,c);
    }
  }
  cout<<ans;
