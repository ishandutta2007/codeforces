...
int n,m;
string s[maxn];
bool vis[maxn][maxn],v1[maxn][maxn],v2[maxn],v3[maxn];

void dfs(int u,int v){
    if (vis[u][v]) return;
    vis[u][v]=1;
    if (u>0&&s[u-1][v]=='#') dfs(u-1,v);
    if (v>0&&s[u][v-1]=='#') dfs(u,v-1);
    if (u<n&&s[u+1][v]=='#') dfs(u+1,v);
    if (v<m&&s[u][v+1]=='#') dfs(u,v+1);
}
signed main(){
