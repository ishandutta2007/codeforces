namespace Solution {
    ...
    operation Solve(inputs: Qubit[], output: Qubit): Unit is Adj+Ctl {
        body(...) {
            ...
            using(qs=Qubit[3]) {
                X(qs[0]);
                for(i in 0..2..n-1) { (Controlled INC)([inputs[i]],qs); }
                for(i in 1..2..n-1) { (Controlled Adjoint INC)([inputs[i]],qs); }
                CNOT(qs[0],output);
                for(i in 1..2..n-1) { (Controlled INC)([inputs[i]],qs); }
                for(i in 0..2..n-1) { (Controlled Adjoint INC)([inputs[i]],qs); }
                X(qs[0]);
            }
        }
    }
    ...
    operation Tester() : Unit {
        body(...) {
            Message("qqz");
            using(qs=Qubit[3]) { using(q=Qubit()) {
                for(i in 0..7) {
                    Solve(qs,q);
                    DumpMachine(());
                    Incr(LittleEndian(qs));
                }
                ResetAll(qs);
                Reset(q);
            }}
