...
long long calculate(const set<long long>& values, const multiset<long long>& differences){
	assert(values.size() == 0 || differences.size() + 1 == values.size());
	...
	long long lo = *values.begin(), hi = *values.rbegin();

	return hi - lo - (*differences.rbegin());
}

void erase(multiset<long long>& diff, long long val){
...
void solve(){
	...
	cout << calculate(values, diff) << endl;
