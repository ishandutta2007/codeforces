...
void solve() {
  ...
  dfs0(0,-1);
  li ans = 1ll<<60;
  smin(ans, cost[0] + rmax[0]);

  auto dfs1 = letrec([&](auto dfs1, int i, int p, li maxup, li costup) -> void {
    for(int j : G[i]) if(j != p) {
        li mx,co;
        if(j == max1at[i]) {
          mx = max(maxup, max2[i]);
          co = costup+scost[i]-cost[j];
        }else{
          mx = max(maxup, max1[i]);
          co = costup+scost[i]-cost[j];
        }
    ...
    if(i != 0) {
      li mx = max(maxup, max1[i]);
      li co = scost[i] + costup;
      if(mx < H[i]) {
        co += H[i]-mx;
        mx = H[i];
      }
