...
int main()
{
	...
	int risan = 0;
	FORD(i,r-1,0) FORD(j,c-1,0)
	{
		if (data[i][j] == 'W' && tot[i][j] == 1) continue;
		if (data[i][j] == 'B' && tot[i][j] == -1) continue;
		int ganti = 0;
		if (data[i][j] == 'W') ganti = 1 - tot[i][j];
		else ganti = -1 - tot[i][j];
		++risan;
		FORN(k,0,i) FORN(l,0,j) tot[k][l] += ganti;
	}
	printf("%d\n",risan);
	return 0;
}