...
namespace MAIN{
    ...
    inline void modify(const res &x,const res &y){
        for(res i=x;i<=N-10;i+=lowbit(i))tr[i]+=y;
    }
    ...
    inline void MAIN(const res &Case){
        n=read(),m=read(),fac[0]=fac[1]=1;
        for(res i=2;i<=n;i++)fac[i]=mul(fac[i-1],i);
        for(res i=1;i<=n;i++)s[i]=read(),b[s[i]]++,modify(s[i],1);
        for(res i=1;i<=m;i++)t[i]=read();
        res ans=0,ret=1;
        for(res i=1;i<=N-10;i++){
            ret=mul(ret,fac[b[i]]);
        }
        ret=qpow(ret);
        for(res i=1;i<=min(n,m);i++){
//            printf("%d %d %d\n",ret,fac[n-i],mul(ret,fac[n-i]));
            add(ans,mul(fac[n-i],mul(query(t[i]-1),ret)));
//            printf("%d\n",ans);
            ret=mul(ret,fac[b[t[i]]]);
            b[t[i]]--,modify(t[i],-1);
            if(b[t[i]]<0){ret=0;break;}
            ret=mul(ret,qpow(fac[b[t[i]]]));
        }
        if(n<m&&ret)add(ans,1);
        printf("%d\n",ans);
    }
}
int main(){
