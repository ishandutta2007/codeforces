...
class tempIntervalSumP(val n:Int){
...
fun main(){
    ...
    solve.cases{
        ...
        fun solve(k:Int):Int {
            ...
            DP[0] = 1
            for(i in 0 until n){
                val new = tempIntervalSumP(max+1)
                val lx = ll[i] divceil k
                val rx = rr[i] / k
...
//                just dei DP
        ...
        val atleast = IntArray(m+1){if(it == 0) 0 else solve(it)}
        val ret = IntArray(m+1)
        for(x in 1..m){
            for(y in x..m step x){
                ret[x] = ret[x] modPlus (atleast[y] modM sieve.mobius[y /x].adjust())
            }
        }
        put(ret[1])
