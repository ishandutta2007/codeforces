...
void solve() {
  auto primes = prime_table(1 << 15);
  vc<bool> is_prime(1 << 15);
  FORIN(p, primes) is_prime[p] = true;
  LL(N);
  VEC(ll, A, N);

  mf_graph<ll> G(N + 2);
  ll source = N, sink = N + 1;
  FOR(i, N) if (A[i] % 2 == 0) G.add(source, i, 2);
  FOR(i, N) if (A[i] % 2 == 1) G.add(i, sink, 2);
  FOR(i, N)
  FOR(j, N) if (A[i] % 2 == 0 && A[j] % 2 == 1 && is_prime[A[i] + A[j]]) {
    G.add(i, j, 1);
  }

  ll F = G.flow(source, sink);
  if (F < N) return print("Impossible");

  Graph<int> H(N);
  auto E = G.edges();
  FORIN(e, E) {
    if (e.frm == source) continue;
    if (e.to == sink) continue;
    if (e.flow == 0) continue;
    FOR(_, e.flow) H.add(e.frm, e.to);
  }
