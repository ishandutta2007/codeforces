...
void solve(){
    ...
    nexts.push(make_tuple(0, 0, n));
    int itr = 1;
    while (!nexts.empty()) {
        pair<int, int> now = mp(-get<1>(nexts.top()),get<2>(nexts.top()));
        nexts.pop();
        if (now.first >= now.second) continue;
        int hoge = (now.first + now.second - 1) / 2;
        dp[hoge] = itr;
        itr++;
        nexts.push(make_tuple(hoge - now.first, -now.first, hoge));
        nexts.push(make_tuple(now.second - (hoge + 1), -(hoge + 1), now.second));
    }
    REP(i, n) {
        cout << dp[i] << " ";
    }
