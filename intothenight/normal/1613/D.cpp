...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		int n;
		cin >> n;
		vector<modular> dp(n + 2), fixed(n + 2);
		dp[0] = 1;
		for(auto i = 0; i < n; ++ i){
			int x;
			cin >> x;
			fixed[x + 1] += fixed[x + 1];
			if(x){
				fixed[x - 1] += fixed[x - 1] + dp[x - 1];
			}
			dp[x + 1] += dp[x] + dp[x + 1];
		}
		cout << accumulate(dp.begin(), dp.end(), accumulate(fixed.begin(), fixed.end(), modular(-1))) << "\n";
		return 0;
	};
	int __tc_cnt;
	cin >> __tc_cnt;
...
/*
0...0 1...1 2...2 (k-1)...(k-1) (alternates between k-1 and k+1)
*/
