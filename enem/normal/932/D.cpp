...
static const ll N=400005;
static ll p[N],w[N],bl[N][20],wl[N][20],pt;
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,q)
        {
            ...
            if(typ==1)
            {
                ...
                wl[pt][0]=t2;
                bl[pt][0]=p[pt];
                while(bl[pt][0]!=0&&w[bl[pt][0]]<w[pt])
                {
                    bl[pt][0]=bl[bl[pt][0]][0];
                }
                REP(j,1,20)
                {
                    bl[pt][j]=bl[bl[pt][j-1]][j-1];
                    wl[pt][j]=wl[pt][j-1]+wl[bl[pt][j-1]][j-1];
                }
                pt++;
            }
            else
