...
int main() {
	...
	// auto has_friend = [&](int man) {
	// 	auto it = s.lower_bound(pair(man, -1));
	// 	if(it == s.end() || it->first != man) return false;
	// 	return true;
	// };

	int res = 0;
	rep(i, n) {
		// if(has_friend(i) && under[i]) {
		// 	res++;
		// }
		if(fri[i] && fri[i] != under[i]) {
			res++;
		}
	}

	auto cnt = [&](int i) { return fri[i] && fri[i] != under[i]; };
