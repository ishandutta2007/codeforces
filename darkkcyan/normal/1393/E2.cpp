...
function<bool(int, int)> gen_missing_char_cmp(const string& su, const string& sv) {
    ...
    return [&su, &sv](int u, int v) {
        auto cmp_char = [&](int x, int y) {
            // deb(&su); 
            char cu = x >= len(su) ? ' ' : su[x];
            char cv = y >= len(sv) ? ' ' : sv[y];
            return cu < cv;
        };
...
int main(void) {
    ...
    rep(i, n) {
        DB(""; deb(i));
        cin >> s[i];  
        // s[i] = string(1000000, 'a'); 
        s[i] += ' ';
        deb(s + i);
        rep(f, len(s[i])) mchs[i].push_back(f);
        auto cur_cmp = gen_missing_char_cmp(s[i], s[i]);
        sort(mchs[i].begin(), mchs[i].end(), cur_cmp);
        // for (auto f: mchs[i]) { 
