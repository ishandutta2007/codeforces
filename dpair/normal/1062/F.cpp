...
struct Queue{
    int buf[MAXN], *hd, *ed;
    Queue(){hd = ed = buf;}
    inline void push(int x){*ed ++ = x;}
    inline int pop(){return *hd ++;}
    inline void clear(){hd = ed = buf;}
    inline bool empty(){return hd == ed;}
    inline int size(){return ed - hd;}
    inline int history(){return ed - buf;}
    inline int front(){return *hd;}
}q;
int f[MAXN];
struct Graph{
    int head[MAXN], nxt[MAXN], e[MAXN], cnt, d[MAXN];
    ...
    inline void solve() {
        ...
        while(!q.empty()) {
            const int sz = q.size(), x = q.pop(); 
            if(sz == 1) f[x] += n - q.history();
            if(sz == 2) { const int y = q.front();
                bool ck = 1; for (int i = head[y];i;i = nxt[i]) {if(d[e[i]] == 1) {ck = 0; break; } }
                f[x] += ck * (n - q.history());
            }
...
signed main(){
    ...
    } g1.solve(); q.clear(); g2.solve();
    rep(i, 1, n) ans += (f[i] >= n - 2);
    print(ans);
}