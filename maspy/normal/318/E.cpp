...
void solve() {
  ...
  vi diff(N);
  FOR(i, N) diff[i] = B[i] - A[i];
  if (SUM(diff) != 0) return NO();
  ...
  auto ope = [&](ll frm, ll to, ll x) -> void {
    A[frm] -= x;
    A[to] += x;
    ANS.eb(vi({frm + 1, to + 1, x}));
  };

  while (1) {
    auto S = min_element(all(diff)) - diff.begin();
    if (diff[S] == 0) break;
    assert(diff[S] < 0);
    auto [dist, par] = bfs01(G, S);
    ll T = -1;
    FOR(v, N) {
    ...
    if (T == -1) return NO();
    ll val = min(-diff[S], diff[T]);
    diff[S] += val;
    diff[T] -= val;
    // S  T  val 
    vi PATH = {T};
    while (PATH.back() != S) PATH.eb(par[PATH.back()]);
    reverse(all(PATH));
    ...
    FOR(i, n - 1) {
      ...
      PUSH[i] = min(A[frm], MAX - A[to]);
      ope(frm, to, PUSH[i]);
    }
    FOR_R(i, n - 1) {
      ll frm = PATH[i], to = PATH[i + 1];
      if (PUSH[i] >= val)
        ope(to, frm, PUSH[i] - val);
      else
        ope(frm, to, val - PUSH[i]);
    }
  }
