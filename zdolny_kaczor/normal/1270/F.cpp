...
char in[nax];
ll tr_cou(int sum, int r, int m) {
	assert(sum >= 0 && m > 0);
	r %= m;
	if (r < 0) r += m;
	int full = (sum - r + m) / m;
	ll ans = (r + 1ll) * full + m * 1ll * (full * (full - 1ll) / 2);
	debug(imie(full));
#ifdef LOCAL
	ll brans = 0;
	FOR(i, 0, sum) FOR(j, 0, sum) if (i + j <= sum && (i + j) % m == r)
		brans++;
	debug(imie(sum), imie(r), imie(m), imie(ans), imie(brans));
	assert(ans == brans);
#endif
	rans;
}
ll rec_cou(int a, int b, int r, int m) {
	assert(a >= 0 && b >= 0 && m > 0);
	r %= m;
	if (r < 0) r += m;
	debug("calc");
	ll ans = tr_cou(a + b + 1, r, m) - tr_cou(b, r - a - 1, m) - tr_cou(a, r - b - 1, m);
#ifdef LOCAL
	ll brans = 0;
	FOR(i, 0, a) FOR(j, 0, b) if ((i + j) % m == r) brans++;
	debug(imie(a), imie(b), imie(r), imie(m), imie(ans), imie(brans));
	assert(ans == brans);
#endif
	rans;
}
ll foo(int a, int b, int l, int r, int c) {
	ll ans = 0;
	int bou = c * r - l;
	if (a + b > bou) {
	 	ans += rec_cou(a, b, -l, c);
		...
		if (bou > a) {
			ans += tr_cou(bou - a - 1, -l - a - 1, c);
			debug("add tr");
		}
		if (bou > b) {
			ans += tr_cou(bou - b - 1, -l - b - 1, c);
			debug("add tr");
		}
	}
#ifdef LOCAL
	ll brans = 0;
	FOR(i, 0, a) FOR(j, 0, b) if (i + j > c * r - l && (i + j + l) % c == 0) brans++;
	debug(imie(a), imie(b), imie(l), imie(r), imie(c), imie(ans), imie(brans), imie(bou));
	assert(ans == brans);
#endif
	rans;
}
int main() {
	while (true && false) {
		const int nax = 20;
		// tr_cou(rand() % nax, rand() % nax, rand() % nax + 1);
		// rec_cou(rand() % nax, rand() % nax, rand() % nax, 1 + rand() % nax);
		foo(rand() % nax, rand() % nax, rand() % nax, rand() % nax + 1, rand() % nax + 1);
	}
	scanf("%s", in);
	int n = strlen(in);
	vector <int> ones;
	REP(i, n) if (in[i] == '1') ones.push_back(i);
	int r = sqrt(n);
	debug(imie(ones));
	ll ans = 0;
	FOR(c, 1, n) {
		if (c * r >= n) break;
		for (int i = 0; i + c <= (int)ones.size(); ++i) {
			int l = ones[i + c - 1] - ones[i] + 1;
			int a = ones[i] - (i == 0 ? -1 : ones[i - 1]) - 1;
			int b = (i + c == (int) ones.size() ? n : ones[i + c]) - ones[i + c - 1] - 1;
			debug(imie(l), imie(a), imie(b), imie(c), imie(r));
			ans += foo(a, b, l, r, c);
		}
	}
	FOR(ratio, 1, r) {
