...
int main(void)
{
	...
	rep(i, 1, m){
		if(ccnt[i] >= B) bvec.pb(i);
		else cvec.pb(i);
	}
	
	for(auto c : bvec){
		uf[c] = UnionFind(n);
		for(auto p : vec[c]) uf[c].unite(p.fst, p.snd);
	}
	...
	for(auto c : cvec){
		...
		for(auto v : vvec){
			tmp.clear();
			if(!used[v]) dfs(v);
			reps(i, tmp) reps(j, tmp){
				ll s = tmp[i], t = tmp[j];
				if(s < t) mp[P(s, t)]++;
			}
		}
		for(auto v : vvec) G[v].clear(), used[v] = false;
	}
	...
	rep(i, 1, Q){
		...
		if(u > v) swap(u, v);
		ll ans = 0;
		for(auto c : bvec) if(uf[c].same(u, v)) ans++;
		ans += mp[P(u, v)];
		cout << ans << "\n";
	}
