...
int main() {
	...
	vector<int> numUsed = b;
	numUsed.insert(numUsed.end(), all(c));
	distinct(numUsed);
	for (auto& i : b)
		i = lower_bound(all(numUsed), i) - numUsed.begin();
	for (auto& i : c)
		i = lower_bound(all(numUsed), i) - numUsed.begin();
	
	const int maxN = (int)numUsed.size();
