...
int main() {
	...
	vector<int> x;
	for (int i = 1; i < m; i++) x.emplace_back(b[i]);
	x.emplace_back(INT_MIN);
	for (int i = 1; i < n; i++) x.emplace_back(a[i]);

	vector<int> z(x.size(), 0);
	for (int i = 1, l = 0, r = 0; i < (int)x.size(); i++) {
		if (i <= r)
			z[i] = min(z[i - l], r - i + 1);
		while (i + z[i] < (int)x.size() && x[i + z[i]] == x[z[i]])
			z[i]++;
		if (i + z[i] - 1 > r)
			l = i, r = i + z[i] - 1;
	}
