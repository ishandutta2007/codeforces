...
void solve(){
	ll n, q, mod; cin >> n >> q >> mod;
	vector<ll> A(n), B(n);
	read(A,B);
	vector<ll> fib(n+5);
	fib[0] = 0;
	fib[1] = fib[2] = 1;
	for(ll i = 3; i < n+5; ++i)fib[i] = (fib[i-1]+fib[i-2])%mod;
	vector<array<ll,2>> lazy(n+1,{0,0});
	set<int> S;
	for(int i = 0; i < n; ++i){
		if(A[i] != B[i])T.insert(i);
	}
	for(int i = 0; i < q; ++i){
		char c; cin >> c;
		int a, b; cin >> a >> b;
		...
		lazy[a-1][0] += fib[0]*(c == 'B' ? -1 : 1);
		lazy[a-1][1] += fib[1]*(c == 'B' ? -1 : 1);
		lazy[b][0] -= fib[b-a+1]*(c == 'B' ? -1 : 1);
		lazy[b][1] -= fib[b-a+2]*(c == 'B' ? -1 : 1);
		lazy[a-1][0] += mod;
		lazy[a-1][1] += mod;
		lazy[b][0] += mod;
		lazy[b][1] += mod;
		...
		S.insert(a-1);
		S.insert(b);
		if(!T.size() || T.order_of_key(*prev(S.end())+1)-T.order_of_key(*S.begin()) == T.size()){
			while(S.size()){
				auto x = *S.begin();
				S.erase(x);
				while(1){
					if(x >= n)break;
					if(lazy[x][1] == 0 && lazy[x][0] == 0)break;
					A[x]+=lazy[x][1];
					A[x]%=mod;
					lazy[x+1][1]+=lazy[x][0]+lazy[x][1];
					lazy[x+1][1]%=mod;
					lazy[x+1][0]+=lazy[x][1];
					lazy[x+1][0]%=mod;
					lazy[x][1] = lazy[x][0] = 0;
				...
				if(!S.size() || T.order_of_key(*prev(S.end())+1)-T.order_of_key(x) != T.size()){
					break;
				}
			}
		}
