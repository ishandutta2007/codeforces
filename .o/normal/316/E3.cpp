...
inline int PLUS (ll a, ll b) { int T = MD(a) + MD(b); while(T >= MOD) T -= MOD; return T; } 
...
namespace Query {
	...
	inline void mergeNodeA (int nd, int len) {
		if(nd <= LEAF) {
			...
			s0[nd] = PLUS(s0[nd], pl[nd]);
			s1[nd] = PLUS(s1[nd], pl[nd]);
			pl[nd] = 0;
		}
	}
	...
	inline void spreadNode (int nd, int nl, int nr) {
		...
		if(nd+nd <= LEAF) {
			tmp = pl[nd+nd]; pl[nd+nd] = pl[nd]; 
			mergeNodeB (nd+nd, len >> 1);
			pl[nd+nd] = PLUS(tmp, pl[nd]);

			tmp = pl[nd+nd+1]; pl[nd+nd+1] = pl[nd]; 
			mergeNodeB (nd+nd+1, len >> 1);
			pl[nd+nd+1] = PLUS(tmp, pl[nd]);
		}else {
			s0[nd+nd] = PLUS(s0[nd+nd], pl[nd]);
			s1[nd+nd] = PLUS(s1[nd+nd], pl[nd]);
			s0[nd+nd+1] = PLUS(s0[nd+nd+1], pl[nd]);
			s1[nd+nd+1] = PLUS(s1[nd+nd+1], pl[nd]);
		}
		pl[nd] = 0;
		//mergeNode(nd, nl, nr);
	}
