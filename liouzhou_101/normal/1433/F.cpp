	...
	for (int i = 0; i < n; ++i)
	{
		...
		for (int j = 0; j < m; ++j)
			for (int c = 0; c <= m; ++c)
				for (int x = 0; x < k; ++x)
				{
					if (f[j][c][x] == -INF) continue;
					freshmax(f[j + 1][c][x], f[j][c][x]);
					freshmax(f[j + 1][c + 1][(x + a[i][j]) % k], f[j][c][x] + a[i][j]);
				}
		for (int c = 0; c <= m / 2; ++c)
			for (int x = 0; x < k; ++x)
				for (int y = 0; y < k; ++y)
				{
					if (f[m][c][x] == -INF) continue;
					freshmax(g[i + 1][(y + x) % k], g[i][y] + f[m][c][x]);
				}
	}
	writeln(g[n][0]);
}
