...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        res ans=0;
        for(res i=1;i<=m;i++)add(ans,mul(i+1,mul(C(n-i,m-i),ipw[n-i+1])));
        printf("%d\n",mul(ans,k));
    }
}
int main(){
...
//    freopen("1.out","w",stdout);
    fac[0]=fac[1]=inv[0]=inv[1]=ipw[0]=1,inv2=(kcz+1)>>1,ipw[1]=inv2;
    for(res i=2;i<=N-10;i++)fac[i]=mul(fac[i-1],i),inv[i]=mul(inv[kcz%i],kcz-kcz/i),ipw[i]=mul(ipw[i-1],inv2);
