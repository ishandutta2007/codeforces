#line 1 "codeforces-workspace/contest/1499/f/f.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/1499/f/f.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1499/f/f.cpp"
...
#line 27 "codeforces-workspace/contest/1499/f/f.cpp"
...
void main() {
  ...
  auto dp = fixed_point([&](const auto& dfs, int now, int pre) -> vector<mint> {
    vector<mint> dp{1};
    ...
    for (auto&& to : tree[now]) {
      ...
      vector<mint> nx(max(dp.size(), 1 + cp.size()));
      for (auto i : range(dp.size())) {
        for (auto j : range(cp.size())) {
          nx[i] += dp[i] * cp[j];
          if (i + j >= k) continue;
          nx[max(i, j + 1)] += dp[i] * cp[j];
        }
      }
  ...
  cout << accumulate(begin(dp), end(dp), mint{}) << "\n";
}
