...
*/
bitset<1010>b[33],c[(1<<20)+10],g[(1<<20)+10],f;
signed main()
{
	int T=1;
	while(T--)
	{
		int s,d;rdn;rd(s);rd(d);
		up(i,0,s-1)
		{
			int x;rd(x);
			up(j,1,x)
		...
		unordered_map<bitset<1010>,int>mp;
		int m1=s-n1;
		up(i,0,(1<<m1)-1)
		{
			g[i].reset();
		...
		up(i,0,(1<<m1)-1)
		{
			if(mp.find(g[i])==mp.end())mp[g[i]]=__builtin_popcount(i);
			else chkmin(mp[g[i]],__builtin_popcount(i));
		//	cout<<mp[g[i]]<<" "<<__builtin_popcount(i);edl;
		}
		//if(s==30)return 0;
	//	cout<<mp[g[0]]<<endl;
	int w=0;
		...
		//	cout<<mp[f]<<endl;
			up(i,0,(1<<n1)-1)
			{
				bitset<1010>nw=f^c[i];
				if(mp.find(nw)==mp.end())continue;
		//		cout<<mp[nw];edl;
				chkmin(ans,mp[nw]+__builtin_popcount(i));
			}
			if(ans==1000000000)ans=-1;
			opd(ans);edl;
			w++;
		//	if(s==30&&w==10)return 0;
		}
	}
}
/*
