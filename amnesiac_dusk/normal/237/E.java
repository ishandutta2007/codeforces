...
public class Main
{
	...
	//augementing path finder using floyd warshall
	static boolean sendmultipleunitsofflowmyboy()
	{
		int[]holala=new int[n+28];
		Arrays.fill(holala, infi);
		holala[0]=0;
		int[]pred=new int[n+28];
		boolean improvement=true;
		for(int k=0; k<=n+27&&improvement; k++)
		{
			improvement=false;
			for(int i=0; i<=n+27; i++)
				for(int j=0; j<=n+27; j++)
					if(cap[i][j]>0&&holala[j]>holala[i]+cost[i][j])
					{
						holala[j]=holala[i]+cost[i][j];
						pred[j]=i;
						improvement=true;
					}
		}
		...
		{
			count+=max;
			fincost+=max*holala[n+27];
			return true;
		}
		return false;
	}
	static int fincost=0,count=0;
	static int[][]cap;
	static int[][]cost;
	static int n;
	public static void main(String[] args) throws Exception
	{
		String s=ns();
		n=ni();
		int[][]hola=new int[n+1][26];
		cap=new int[n+28][n+28];
		cost=new int[n+28][n+28];
		for(int i=1; i<=n; i++)
		{
			String te=ns();
			for(int j=0; j<te.length(); j++)
				hola[i][te.charAt(j)-'a']++;
			cap[0][i]=ni();
			cost[0][i]=0;
			for(int j=0; j<26; j++)
			{
				cap[i][n+j+1]=hola[i][j];
				cost[n+j+1][i]=-i;
				cost[i][n+j+1]=i;
			}
		}
		...
		while(Main.sendmultipleunitsofflowmyboy()&&count!=s.length());
		if(count==s.length())
			pr(fincost);
		else
			pr(-1);
