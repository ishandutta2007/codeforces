...
void solve() {
    cin >> n >> m;
    read(a, n);
    F0R(i, n) F0R(j, a[i]) b.pb(i);
    pa.rsz(n+1, 0); F0R(i, n) pa[i+1] = pa[i]+a[i];
    pb.rsz(m+1, 0); F0R(i, m) pb[i+1] = pb[i]+b[i];
    int dp[n][m+1][m+1]; // dp[i][sum][last]
    fill(*dp[0], *dp[n], (int)1e9);
    F0R(i, m+1) {
        int val = pb[i];
        if (pa[1]>i) val = pa[1]-i;
        F0R(j, i+1) dp[0][i][j] = val;
    }
    debug(pa, pb);
    FOR(i, 1, n) F0R(j, m+1) R0F(k, m+1) if ((i+1)*k <= j) {
        dp[i][j][k]=dp[i][j][k+1];
        int val = 0;
        if (pa[i+1] < j) {
            int add = min(k, j - pa[i+1]);
            val = pb[j] - pb[j-add] - add*i;
        }
        if (pa[i+1] > j) val = pa[i+1]-j;
        if (i==1 && j==4 && k==2) debug(pb[j], pb[j-k], pb[pa[i+1]]);
        ckmin(dp[i][j][k], dp[i-1][j-k][k] + val);
    }
    debug(dp[0][2][2], dp[1][4][2], dp[2][6][2]);
    cout << dp[n-1][m][0] << nl;
}
