...
bool comp1(pair<pair<int, int>, int> a1, pair<pair<int, int>, int> a2)
{
    return a1.first.first < a2.first.first;
}

bool comp2(pair<pair<int, int>, int> a1, pair<pair<int, int>, int> a2)
{
...
signed main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            Ba++;
            t1.push_back({{a[i], b[i]}, i + 1});
        }
        else
        {
            Bb++;
            t2.push_back({{a[i], b[i]}, i + 1});
        }
    }
    if (Ba >= Bb)
    {
