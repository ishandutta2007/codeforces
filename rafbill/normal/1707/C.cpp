...
void solve(){
  ...
  dfs(root,-1);
  auto dfs2 = letrec([&](auto dfs2, int i, int p, int delta) -> void {
    delta += cnt[i];
    base[i] += delta;
    for(int j : H[i]) if(j != p) {
        dfs2(j,i,delta);
  ...
  dfs2(root,-1,0);
  vi ok(n);
  for(li i = 0; i < (li)(n); ++i) ok[i] = base[i] >= 0;
  for(li i = 0; i < (li)(n); ++i) cout << ok[i];
  cout << endl;
}
int main(){
