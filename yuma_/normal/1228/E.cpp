...
int main() {
	...
	dp[0][0]=Mod(1);
	init();
	vector<Mod>ks(300),k1s(300);
	ks[0]=1;k1s[0]=1;
	for(int i=1;i<300;++i)ks[i]=ks[i-1]*K;
	for(int i=1;i<300;++i)k1s[i]=k1s[i-1]*(K-1);
	for(int h=0;h<N;++h){
		for(int one=0;one<=N;++one){
			for(int plus=0;plus<=N-one;++plus){
				Mod cnt=ks[one]*comb(N-one,plus)*k1s[N-one-plus];
				if(plus==0){
					cnt-=mod_pow(K-1,N);
				}
				dp[h+1][one+plus]+=dp[h][one]*cnt;
			}
		}
	}
