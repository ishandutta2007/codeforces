...
signed main() {
  ...
  using node_data = tuple<pi, pi, pi>;
  using edge_data = node_data;
  // 
  edge_data e_unit({mp(-1, -INF), mp(-1, -INF), mp(-1, -INF)});
  // 

  auto add = [&](edge_data x, pi y) -> edge_data {
    if (y.se == -INF) return x;
    pi a, b, c;
    tie(a, b, c) = x;
    if (a.se < y.se) return mt(y, a, b);
    if (b.se < y.se) return mt(a, y, b);
    if (c.se < y.se) return mt(a, b, y);
    return mt(a, b, c);
  };

  auto f_ev = [&](ll v, edge_data x) -> node_data { return add(x, mp(v, 0)); };
  // 
  auto f_ve = [&](ll v, node_data x) -> edge_data {
    pi a, b, c;
    tie(a, b, c) = x;
    return mt(mp(a.fi, a.se + 1), mp(-1, -INF), mp(-1, -INF));
  };
  // 
