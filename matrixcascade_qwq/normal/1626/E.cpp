...
void dfs2(int x,int fa)
{
	fp v[x] pf
	{
		...
		if(b[to]==1)dn[x]=1;
		dfs2(to,x);
		if(dn[to]==1&&sum[to]>=2)dn[x]=1,w[x]++;
	}
}
...
void dfs3(int x,int fa)
{
	if(x!=1)
	{
		if(b[fa])up[x]=1;
		int bb=0;
		int g1=sm[fa];int g2=w[fa];
		g1-=b[x];g2-=dn[x];
		if(g1||g2)bb=1;
		if(up[fa]&&(sum[1]-sum[x])>=2)up[x]=1;
		if(bb&&(sum[1]-sum[x])>=2)up[x]=1;
	}
	fp v[x] pf
	{
		if(to==fa)continue;
...
signed main()
{
	...
	while(T--)
	{
		...
		dfs(1,0);
		dfs2(1,0);dfs4(1,0);dfs3(1,0);
		upn
		{
			if(up[i]||dn[i]||b[i])cout<<1<<" ";
			else cout<<0<<" ";
		}
	}
}
