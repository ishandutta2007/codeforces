...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n){
		int t=min(8,n-i);
		int ok=t;;
		rep(c,t)rep(a,c)if(a%2==c%2){
			int b=(a+c)/2;
			if(s[i+a]==s[i+b] && s[i+b]==s[i+c]){ ok=c; goto BRK;}
		}
	BRK:
		out(i,ok,1);
		re+=n-ok-i;
	...
	cout<<re<<endl;
	// reps(d,8,20){ //i hate this
	// 	int fnd=0;
	// 	rep(st,1<<d){
	// 		int ok=0;
	// 		rep(i,d)for(int k=i+2;k<d;k+=2){
	// 			int j=(i+k)/2;
	// 			if((st>>i&1)==(st>>j&1) && (st>>i&1)==(st>>k&1)){ ok=1; break;}
	// 		}
	// 		if(!ok){ fnd=1; break;}
	// 	}
	// 	if(!fnd) out(d,1);
