...
int main(){
    ...
    REP(i,n){
        ...
        double v=(p-t).dis();
        an+=v*2;
        d[0].PB(MP((a-p).dis()-v,i));
        d[1].PB(MP((b-p).dis()-v,i));
    }
    REP(i,2)sort(ALL(d[i]));
    double ret=min(an+d[0][0].F,an+d[1][0].F);
    if(d[0][0].S!=d[1][0].S)ret=min(ret,an+d[0][0].F+d[1][0].F);
    else{
        if(n>1){
            ret=min({ret,an+d[0][0].F+d[1][1].F,an+d[0][1].F+d[1][0].F});
        }
    }
    W(ret);
    return 0;
