...
int main() {
    ...
    atcoder::fenwick_tree<int> ft_zero(n), ft_index(n);
    rep(i, n) ft_index.add(i, 1);
    map<int, int> memo;
    auto query = [&](int i) {
        if (memo.count(i)) {
            return memo[i];
            ...
            int response;
            read(response);
            int zero_count = (i - response) + ft_zero.sum(0, i);
            return memo[i] = zero_count;
        }
    };
    rep(_, t) {
        int k;
