...
struct pq_plus {
    ...
    priority_queue<int, vector<int>, greater<int>> to_remove;
    void push(int x) {
        // cout << "ADD: " << x << endl;
        if (!to_remove.empty() and to_remove.top() == x) to_remove.pop();
        else pq.push(x);
        // if (!pq.empty() and !to_remove.empty()) assert(pq.top() != to_remove.top());
    }
    void remove(int x) {
        // cout << "REMOVE: " << x << endl;
        if (pq.top() == x) pq.pop(), clean();
        else to_remove.push(x);
        // if (!pq.empty() and !to_remove.empty()) assert(pq.top() != to_remove.top());
    }
    bool empty() { return pq.empty(); }
    int top() { return pq.top(); }
    void pop() { pq.pop(); clean(); }
    void clean() {
        while (!pq.empty() and !to_remove.empty() and pq.top() == to_remove.top()) pq.pop(), to_remove.pop();
        // if (!pq.empty() and !to_remove.empty()) dbg(pq.top(), to_remove.top());
    }
    int size() {
        return pq.size() + to_remove.size();
    }
};

struct pq_plus_worng {
    multiset<int> pq;
    void push(int x) {
        pq.insert(x);
    }
    ...
    bool empty() { return pq.empty(); }
    int top() { return *(pq.begin()); }
    void pop() { pq.erase(*pq.begin()); }
    int size() { return pq.size(); }
};

pq_plus* solve(int v, vector<int>& ans) {
    pq_plus* curr = new pq_plus();
    for (int a : aa[v]) {
        if (a == fast_pp[v][0]) continue;
        pq_plus* son = solve(a, ans);
        if (son->size() > curr->size()) swap(son, curr);
        while (!son->empty()) curr->push(son->top()), son->pop();
    }
    for (int x : ss[v]) {
        if (x <= 0) curr->push(-x);
        else curr->remove(x-1);
    }
    if (v == 1) return curr;
    if (curr->empty()) ans[idp[v]] = 1e9;
    else ans[idp[v]] = curr->top();
    return curr;
}
