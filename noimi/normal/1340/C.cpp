...
signed main(){
    ...
    int g = in(),r=in();
    rep(i,n)dp[i][0]=1;
    rep(j,g+1)dp[n-1][j]=1;
    rep(j,g+1){
        rep(i,n){
            if(i and j>=a[i]-a[i-1]){
                dp[i][j]|=dp[i-1][j-a[i]+a[i-1]];
            }
            if(i!=n-1 and j>=a[i+1]-a[i]){
                dp[i][j]|=dp[i+1][j-a[i+1]+a[i]];
            }
        }
    }
    if(!dp[0][g]){
    ...
    q.emplace(0,g);
    int t = 0;
    vector<vector<bool>> used(n,vector<bool>(g+1));
    used[0][g]=1;
    ll ans = LONG_LONG_MAX;
    while(!q.empty()){
        queue<pii> nxt;
        while(!q.empty()){
            auto [pl,s] = q.front();q.pop();
            if(pl==n-1){
                chmin(ans,(ll)t*(r+g) + g-s);
            }
            if(pl){
                int D = a[pl]-a[pl-1];
                if(s>=D and !used[pl-1][s-D] and dp[pl-1][s-D]){
                    q.emplace(pl-1,s-D);
                    used[pl-1][s-D]=1;
                }
            }
            if(pl!=n-1){
                int D = a[pl+1]-a[pl];
                if(s>=D and !used[pl+1][s-D] and dp[pl+1][s-D]){
                    q.emplace(pl+1,s-(a[pl+1]-a[pl]));
                    used[pl+1][s-D]=1;
                }
            }
            if(dp[pl][g-s]) nxt.emplace(pl,g-s);
        }
        while(!nxt.empty()){
