...
// URL : http://codeforces.com/contest/1398/problem/F
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int main()
{
	int n;
	scanf("%i",&n);
	vector<int> cnt1(n),cnt0(n),prev1(n),prev0(n);
	string s;
	cin >> s;
	int jed=-1,nula=-1;
	for(int i=0;i<n;i++){
	...
	for(int i=1;i<=n;i++){
		int tr=0,cnt=0;
		while(tr+i<=n){
			int c1=cnt1[tr+i-1],c0=cnt0[tr+i-1];
			if(tr)
				c1-=cnt1[tr-1],c0-=cnt0[tr-1];
			if(c0==0||c1==0){
				cnt++;
				tr+=i;
				continue;
			}
			int pos=min(prev1[tr+i-1],prev0[tr+i-1]);
			tr=pos+1;
		}
		printf("%i ",cnt);
	}
