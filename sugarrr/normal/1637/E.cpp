...
void solve(){
    ...
    vl dsz;
    {
        ...
        rep(i,1,mx){
            ...
            sort(all(c[i]),[](ll x,ll y){return x>y;});
            d.pb(c[i]);
            dsz.pb(i);
        }
    }
    ll sz=d.size();
    assert((int)dsz.size()==sz);//assert(sz<=1000);
    ...
    for(auto TT:mp){
        ...
        ll cntx = TT.se;
        rep(i,0,sz-1){
            if(cntx>dsz[i])continue;
            ll p=0;
            while(p<=(int)d[i].size()-1){
                if(ok(x,d[i][p])){
                    ll y=d[i][p];
                    ll cnty=dsz[i];
                   // cerr<<x<<" "<<y<<" "<<cntx<<" "<<cnty<<endl;
                    chmax(ans,(x+y) * (cntx + cnty));
                    break;
                }else{
                    p++;
                }
