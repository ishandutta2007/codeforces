...
string dict[] = {"1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011"};

using M = map<char, int, greater<char>>;
...
M nxt(string s) {
  ...
  rep(i, 10) {
    ...
    int cost = 0;
    rep(j, 7)
      if (s[j] == '1' and dict[i][j] == '0') ok = false;
      else cost += s[j] != dict[i][j];
    if (ok) res['0'+i] = cost;
  }
  return res;
}
...
int main() {
  ...
  vector<M> costs(n);
  rep(i, n) costs[i] = nxt(s[i]);

  string res = "";
  auto done = make_v(n, k+1, false);
  auto dfs = fix([&](auto f, auto i, auto rest)->void{
    if (i == n and rest != 0) return;
    if (i == n and rest == 0) EXIT(res);
    if (done[i][rest]) return;
    done[i][rest] = true;
    for (auto [ch, c] : costs[i]) if (rest >= c) {
      res.push_back(ch);
      f(i+1, rest-c);
      res.pop_back();
    }
