...
struct N{
	...
	static N merge(const N&a,const N&b){
		N res;
		rep(i,2)rep(j,2)res.v[i][j]=inf;
		rep(i,2)rep(j,2)rep(k,2){
			chmin(res.v[i][k],a.v[i][j]+b.v[j][k]);
		}
		return res;
	}
};
...
void slv(){
	...
	rep(i,n-2){
		lf[a[i]]++;
		rt[a[i]]--;
		ini[a[i]]=(bool(lf[a[i]])|bool(rt[a[i]])<<1);
		seg.set(a[i],ini[a[i]]);
		auto w=seg.composite(0,a[i+1]);
		int mn=inf;
		rep(j,2)rep(k,2)chmin(mn,w.v[j][k]);
		ans+=mn;
	}
	print(ans);
}
