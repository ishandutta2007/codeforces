...
void solve() {
	...
	int di = w / g;
	int sm = s / di, bi = sm + 1;
	int sm_c = di - s % di, bi_c = di - sm_c;
	debug(imie(m), imie(d), imie(w), imie(g));
	debug(imie(s), imie(di), imie(sm), imie(bi), imie(sm_c), imie(bi_c));
	ll ans = sm_c * 1ll * sm * sm + bi_c * 1ll * bi * bi;
	ans -= s;
	assert(ans % 2 == 0 && ans >= 0);
	ans /= 2;
	printf("%lld\n", ans);
}
int main() {
