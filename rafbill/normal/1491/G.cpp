...
int main(){
  ...
  { vi B(n);
    for(li i = 0; i < (li)(n); ++i) B[A[i]] = i;
    A = B;
  }

  vvi cs;
  vi E(n,0);
  for(li i = 0; i < (li)(n); ++i) if(!E[i]) {
    vi C;
    C.push_back(i); E[i]=1;
    for(int j = A[i]; !E[j]; j = A[j]) {
      E[j] = 1;
      C.push_back(j);
    }
    if(C.size() != 1) cs.push_back(C);
  }
  ...
  while(cs.size() >= 2) {
    ...
    int nd = D.size();
    sw(C[0],D[0]);
    for(li i = (nc-1); i >= (li)(1); --i) sw(C[(i+1)%nc], C[i]);
    for(li i = (nd-1); i >= (li)(1); --i) sw(D[(i+1)%nd], D[i]);
    sw(C[1],D[1]);
  }

  if(cs.size() >= 1) {
    vi C = cs.back(); cs.pop_back();
    int nc = C.size();
    if(nc >= 3) {
      sw(C[0],C[1]);
      for(li i = (nc-1); i >= (li)(3); --i) {
        sw(C[(i+1)%nc], C[i]);
      }
      sw(C[1],C[2]);
      sw(C[3%nc],C[1]);
