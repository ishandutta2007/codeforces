...
const int N = 200'229;
const int INF = 1'000'000'228;
pair<int, int> dp[N][2];
vector<pair<int, int>> re[N][2];
vector<int> g[N];
...
void dfs(int v, int p) {
    ...
    for(int i : g[v]) {
        ...
        if(sz(g[i]) == 1) {
            dp[v][0].x += 1;
            dp[v][0].y += dp[i][0].y;
            re[v][0].push_back({i, 0});
        } else {
            auto zal = mx(dp[i][0], dp[i][1]);
            dp[v][0].x += zal.x;
            dp[v][0].y += zal.y;
            re[v][0].push_back({i, zal == dp[i][1]});
        }
    }
    ...
    for(int i : g[v]) {
        ...
        dp[v][1].x += dp[i][0].x;
        dp[v][1].y += dp[i][0].y;
        re[v][1].push_back({i, 0});
    }
}

int ans[N];

void dfs_re(int v, int id) {
    ans[v] = (id == 0 ? 1 : sz(g[v]));
...
void _solve() {
    ...
    if(n <= 2) {
        cout << n << " " << n << endl << vector<int>(n, 1) << endl;
        return;
    }
