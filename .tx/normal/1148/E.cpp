...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n; i++) {
        if (s[i].first < t[i]) {
            up.insert({s[i].first, t[i] - s[i].first, s[i].second});
        } else if (s[i].first > t[i]) {
            down.insert({s[i].first, s[i].first - t[i], s[i].second});
        }
    }

    vector<tuple<int, int, int>> ans;

    while (isz(up) && isz(down)) {
        auto [l, ld, li] = *begin(up);
        auto [r, rd, ri] = *begin(down);
