    ...
    for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) {
      if(A[i][j] == 0) {
        if((i+j)&1) {
          ...
          if(a != 2) {
            sset(i,j,2); goto l_end;
          }
        }else{

          if(a != 1) {
            sset(i,j,1); goto l_end;
          }
        }
        bool ok = 1;
        for(li d = 0; d < (li)(4); ++d) {
          int i_ = i+dx[d], j_ = j+dy[d];
          if(i_>=0&&i_<n&&j_>=0&&j_<n&&A[i_][j_] == 0) ok = 0;
        }
        if(ok) {
          if((i+j)&1) {

            sset(i,j,a==2?3:2); goto l_end;
          }else{

            sset(i,j,a==1?3:1); goto l_end;
          }
        }
      }
    }
