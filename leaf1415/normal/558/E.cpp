...
int main(void)
{
	...
	rep(q, 1, Q){
		...
		tmp = seg.query(l, r);
		if(t == 1){
			rep(i, 0, 25) seg.add(l, l+tmp[i]-1, i), l += tmp[i];
		}
		else{
			per(i, 25, 0) seg.add(l, l+tmp[i]-1, i), l += tmp[i];
		}
	}
