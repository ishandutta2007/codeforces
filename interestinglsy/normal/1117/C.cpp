...
#define Tr(c) (c=='U'?0:c=='D'?1:c=='L'?2:3)

int n;
string inp;
int seq[MAXN];
ll sumdx[MAXN], sumdy[MAXN];
ll stx,sty,edx,edy;

bool Check( ll lim ){
	ll totdx = 0, totdy = 0;
	totdx += sumdx[n] * (lim/n) + sumdx[lim%n];
	totdy += sumdy[n] * (lim/n) + sumdy[lim%n];
	ll x = totdx + stx, y = totdy + sty;
	ll totd = abs(x-edx) + abs(y-edy);
	return totd <= lim;
}
// XXXXOOOO
ll Binarysearch(){
	ll l = 0, r = 10000000000000000LL, ans = -1, mid;	// 1e16
	while( l <= r ){
		mid = l+r >> 1;
		if(Check(mid)){
			ans = mid;
...
int main(){
	...
	For0(i,n){
		seq[i+1] = Tr(inp[i]);
		sumdx[i+1] = sumdx[i] + dx[seq[i+1]];
		sumdy[i+1] = sumdy[i] + dy[seq[i+1]];
	}
