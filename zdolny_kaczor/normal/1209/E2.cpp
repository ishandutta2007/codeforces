...
void solve() {
	...
	REP(i, m) {
		int big = 0;
		REP(j, n) maxi(big, in[j][i]);
		go.emplace_back(big, i);
	}
	...
	REP(i, m) {
		...
		REP(mask, (1 << n)) {
			if (lowest[mask] != mask) {
				dp[(i + 1) & 1][mask] = dp[(i + 1) & 1][lowest[mask]];
				continue;
			}
			dp[(i + 1) & 1][mask] = 0;
			for (int sub = mask; ; sub = (sub - 1) & mask) {
				maxi(dp[(i + 1) & 1][mask], dp[i & 1][sub] + curr[mask ^ sub]);
				if (!sub) break;
			}
		}
	}
