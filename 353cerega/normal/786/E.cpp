...
void dfs0(int u, int p, int H)
{
    ...
    for (int i=1;i<K;i++)
    {
        ...
        if (up[u][i]!=-1)
        {
            add_edge(u+(i+1)*N,u+i*N,INF);
            add_edge(u+(i+1)*N,up[u][i-1]+i*N,INF);
        }
    }
...
int main()
{
    ...
    for (int i=1;i<=m;i++)
    {
        if (col[(K+1)*N+i-1]==0) L.push_back(i);
    }
    cout << L.size();
    for (int u: L) cout << " " << u;
    cout << endl;
