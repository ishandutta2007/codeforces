...
#define VALOP(op)                                                                                                                                              \
    friend Modular operator op(Modular a, const Modular &b) { return a op## = b; }
    VALOP(+);
    VALOP(-);
    VALOP(*);
    VALOP(/)
...
using Mint = library_checker::Modular<int, 1000000007>;
...
signed main() {
    ...
    vector<Mint> v(N);
    rep2(i, 1, N) {
        fib.eb(fib[i] + fib[i - 1]);
        v[i - 1] = fib[i];
        rep(_, k) v[i - 1] *= i;
        if(i > 1) v[i - 1] += v[i - 2];
    }
    auto rel = library_checker::linear_recurrence::BerlekampMassey(v);

    int T = rel.size();

    Matrix<Mint> mat(T, T);
    rep(i, T - 1) mat[i][i + 1] = 1;
    rep(i, T) mat[T - 1][T - 1 - i] = rel[i];
