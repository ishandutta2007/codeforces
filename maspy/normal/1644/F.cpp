...
#line 2 "/home/maspy/compro/library/poly/fps_inv.hpp"
...
void solve() {
  LL(N, K);
  if (N == 1 || K == 1) return print(1);

  auto mu = mobius_table<mint>(N);

  auto POW2 = powertable_1<mint>(2, N + 1);
  //  d 
  vc<mint> A(N + 1);
  FOR3(d, 1, N) {
    FOR3(i, 1, (N - 1) / d + 1) { A[i - 1] += mu[d]; }
  }
  A = poly_taylor_shift(A, mint(1));
  A.insert(A.begin(), 0);

  // n  K 
  vc<mint> f(K + 1);
  FOR3(k, 1, K + 1) f[k] = fact_inv<mint>(k);
  f = poly_taylor_shift(f, mint(-1));
  // sum f_ke^{kx} 
  // (a_k / (1-kx)) 
  using POLY = vc<mint>;
  deque<pair<POLY, POLY>> que;
  FOR3(k, 1, K + 1) que.eb(POLY({f[k]}), POLY({1, -k}));
  while (len(que) > 1) {
    auto [p1, q1] = que.front();
    que.pop_front();
    auto [p2, q2] = que.front();
    que.pop_front();
    auto q = convolution(q1, q2);
    p1 = convolution(p1, q2);
    p2 = convolution(p2, q1);
    FOR(i, len(p1)) p1[i] += p2[i];
    que.eb(p1, q);
  }
  auto [num, den] = que.front();
  num.resize(N + 1);
  den.resize(N + 1);
  f = convolution(num, fps_inv(den));
  f.resize(N + 1);
  f[0] = 0;
  ...
  // 
  f.erase(f.begin());
  FOR(n, N) f[n] *= fact_inv<mint>(n);
  vc<mint> exp_inv(N);
  FOR(i, N) exp_inv[i] = fact_inv<mint>(i);
  FOR(i, N) if (i & 1) exp_inv[i] *= mint(-1);
  f = convolution(f, exp_inv);
  f.resize(N);
  FOR(i, N) f[i] *= fact<mint>(i);
  // print("A", A);
  // print("f", f);
  mint ANS = 0;
  FOR(i, len(f)) ANS += A[i] * f[i];
  print(ANS);
}
