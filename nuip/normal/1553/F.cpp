...
void Add(ll *bit,int i,ll x){ ++i; ++i;
  while(i<=nn){
    bit[i]+=x;
    i+=i&-i;
  }
...
int main(){
	...
	// 	if(n<100){
	// 		out(re.size(),re,1);
	// 		rep(i,re.size()-1)reps(x,re[i].X,re[i+1].X)if(n/x!=re[i].Y){
	// 			out(n,re,i,1);
	// 		}
	// 	}
	// }
	...
	cauto n=read();
	// auto vs=vector<int>(n); iota(all(vs),100000);
	cauto vs=readV(n,0);
	vector<ll> re(n);
	vector<ll> smalls(SQ);
	vector<ll> cnt1(312345);
	rep(i,n){
		cauto &v=vs[i];
		if(v<SQ){
			re[i]=smalls[v];
			...
			reps(j,1,MOD){
				cauto l=j*v;
				cauto r=min((j+1)*v,302345);
				if(l>=r) break;
				cauto scr=Sum(cnts,r);
				cauto scl=(sc>=0?sc:Sum(cnts,l));
				sc=scr;
				cauto ssr=Sum(sums,r);
				cauto ssl=(ss>=0?ss:Sum(sums,l));
				ss=ssr;
				cauto cnt=(scr-scl);
				re[i]+=v*cnt+(ssr-ssl)-v*j*cnt;
				// out(i,cnt,v*cnt+(Sum(sums,r)-Sum(sums,l))-v*j*cnt,1);
			}
			re[i]-=v*cnt1[v];
			// out(i,re[i],1);
			// v>
			...
			sc=ss=-1;
			int j=1;
			for(j=1;j*j<=v;++j) re[i]+=cnt1[j]*(v%j);
			for(;j;--j){
				cauto l=v/(j+1)+1;
				cauto r=v/j+1;
				if(l<=j) continue;
				// out(l,r,(Sum(cnts,r)-Sum(cnts,l))*v-(Sum(sums,r)-Sum(sums,l))*j,1);
				cauto scr=Sum(cnts,r);
				cauto scl=(sc>=0?sc:Sum(cnts,l));
				sc=scr;
				cauto ssr=Sum(sums,r);
				cauto ssl=(ss>=0?ss:Sum(sums,l));
				ss=ssr;
				re[i]+=(scr-scl)*v - (ssr-ssl)*j;
			}
		}
		Add(cnts,v,1);
		Add(sums,v,v);
		reps(u,1,SQ) smalls[u]+=u%v+v%u;
		++cnt1[v];
	}
	// out(re,1);
	reps(i,1,n) re[i]+=re[i-1];
	for(cauto &v:re) cout<<v<<" ";
	cout NL;
	return 0;
}