...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep(i,N-1){
    cin>>A>>B>>C;
    --A;--B;
    if(C==-1)remedge.emplace_back(A,B);
    else{
  ...
  rep(i,N)if(dsu.parcheck(i,i+N))break;
  if(i<N){
    printNO;
    continue;
  }
  printYES;
  for(auto [A,B]:remedge){
    if(!dsu.parcheck(A,B)&&!dsu.parcheck(A,B+N)){
      dsu.unite(A,B);
      dsu.unite(A+N,B+N);
    }
    edge.emplace_back(A,B,dsu.parcheck(A,B+N));
  }
  for(auto[A,B,C]:edge)print(A+1<<" "<<B+1<<" "<<C);
}
}