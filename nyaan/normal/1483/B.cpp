...
void q() {
  ...
  auto erase = [&](int i, int j) -> void {
    ...
    exist[j]=0;
    er.push_back(j+1);
    if (__gcd(a[i], a[k]) == 1) buf.emplace_back(i, k);
  };
  
  rep(i, N) s.insert(i);
  rep(i,N) {
    if(__gcd(a[i],a[(i+1)%N])==1) buf.emplace_back(i,(i+1)%N);
  }
  while(!buf.empty()) {
    auto [u,v]=buf.front();
    buf.pop_front();
    if(!exist[u] or !exist[v]) continue;
    trc(s);
    trc(u,v);
    erase(u,v);
    /*
    if(!buf.empty()) {
      auto [_u,_v]=buf.front();
      buf.pop_front();
      buf.emplace_back(_u,_v);
    }
    */
  }
