    ...
    int a[500001];
        ...
        int main()
        {
         cin>>n>>k;vector<int> vec;
         for(int i=0;i<n;i++)
         {
             cin>>a[i];
             auto it=myst.lower_bound(a[i]);
             if(*it!=a[i]){vec.push_back(a[i]),x++;myst.insert(a[i]);}
             if(x>k)
             {
                 int ind=vec.size();
                 ind-=k+1;
                auto it2=myst.lower_bound(vec[ind]);
                myst.erase(it2);
                x--;
             }
         }
         int si=vec.size();
         cout<<myst.size()<<endl;
         //for(auto it=myst.begin();it!=myst.end();it++)cout<<*it<<" ";
         for(int i=si-1;i>=(max(0,si-k));i--)cout<<vec[i]<<" ";
