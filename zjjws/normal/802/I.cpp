...
struct Rin
{
    ...
    inline Rin&operator >>(char *x){n=0;for(c=gc();c>'z'||c<'a';c=gc());for(;c>='a'&&c<='z';c=gc())x[++n]=c;return *this;}
}rin;
...
namespace SAM
{
    ...
    int cutt[N];
    int nxt[N][26];
    inline int newnode(){int now=++nodecutt;lik[now]=len[now]=gyq[now]=0;memset(nxt[now],0,sizeof(nxt[now]));return now;}
    inline void init(){nodecutt=0;lst=newnode();return;}
    inline void insert(int c)
    {
        int ed=newnode();len[ed]=len[lst]+1;gyq[ed]=1;
        int now=lst;lst=ed;for(;now&&!nxt[now][c];now=lik[now])nxt[now][c]=ed;if(!now){lik[ed]=1;return;}
        int to=nxt[now][c];if(len[to]==len[now]+1){lik[ed]=to;return;}
        int cly=newnode();len[cly]=len[now]+1;lik[cly]=lik[to];for(int i=0;i<26;i++)nxt[cly][i]=nxt[to][i];
        for(;now&&nxt[now][c]==to;now=lik[now])nxt[now][c]=cly;lik[to]=lik[ed]=cly;return;
    }
    inline void sortlen()
    {
        int maxlen=0;
        for(int i=1;i<=nodecutt;i++)cutt[len[i]]++,maxlen=max(maxlen,len[i]);
        for(int i=1;i<=maxlen;i++)cutt[i]+=cutt[i-1];
        for(int i=1;i<=nodecutt;i++)num[cutt[len[i]]--]=i;
        for(int i=1;i<=nodecutt;i++)cutt[len[i]]=0;
        return;
    }
}
using namespace SAM;
...
inline void work()
{
    init();rin>>x;for(int i=1;i<=n;i++)insert(x[i]-'a');sortlen();
    LL ans=0;for(int i=nodecutt;i;i--){gyq[lik[num[i]]]+=gyq[num[i]];ans+=1LL*gyq[num[i]]*gyq[num[i]]*(len[num[i]]-len[lik[num[i]]]);}
    printf("%lld\n",ans);
    return;
}
int main()
