...
void q() {
  ...
  V<piv> vc;
  {
    ...
    trc(mp);
    map<int, vi> mp2;
    each2(k, v, mp) mp2[v].push_back(k);
    trc(mp2);
    each2(k, v, mp2) vc.emplace_back(k, v);
  }

  each2(k, v, vc) sort(all(v));

  ll ans = 0;
  rep(i, sz(vc)) rep(j, vc[i].second.size()) {
    int x = vc[i].second[j];
    int cx = vc[i].first;
    rep(s, i + 1) {
      int cy = vc[s].first;
      repr(t, sz(vc[s].second)) {
        int y = vc[s].second[t];
        if (x == y) continue;
        if (st.find({x, y}) != end(st)) continue;
        trc(x,cx,y,cy);
        amax(ans, 1LL * (x + y) * (cx + cy));
        break;
      }
    }
  }
