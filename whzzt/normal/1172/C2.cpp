...
int n,m,a[N],w[N],f[M][M],s[2],sv[M<<1],e,E,res;
int main(){
	scan(n,m);
	for(int i=1;i<=n;++i) scan(a[i]);
	for(int i=1;i<=n;++i) scan(w[i]),s[a[i]]+=w[i];
	f[0][0]=1;
	for(int i=-m;i<=m;++i) sv[i+M]=Inv(s[0]+s[1]+i,mod);
	for(int i=0;i<m;++i) for(int j=0;j<=i;++j) if(f[i][j]) {
		inc(f[i+1][j+1],i64(f[i][j])*(s[0]-j)%mod*sv[i-j-j+M]%mod);
		inc(f[i+1][j],i64(f[i][j])*(s[1]+i-j)%mod*sv[i-j-j+M]%mod);
	}
	for(int i=0;i<=m;++i) inc(e,i64(f[m][i])*i%mod),inc(E,i64(f[m][i])*(m-i)%mod);
	e=i64(e)*Inv(s[0],mod)%mod;
	E=i64(E)*Inv(s[1],mod)%mod;
	for(int i=1;i<=n;++i){
		res=w[i];
		if(a[i]) inc(res,i64(E)*w[i]%mod);
		else dec(res,i64(e)*w[i]%mod);
		print(res),putc('\n');
	}
}