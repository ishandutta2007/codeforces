...
int main(){
  ...
  for(ll i=0;i<x.size();i+=2){
    if(i+1!=x.size()){
      ret.EB(x[i][0]+1,x[i+1][0]+1);
      rep(j,1,x[i].size())ret.EB(x[i][j]+1,x[i+1][0]+1);
      rep(j,1,x[i+1].size())ret.EB(x[i][0]+1,x[i+1][j]+1);
      ret.EB(x[i][0]+1,x[i+1][0]+1);
    }
    else{
      if(i==0){
        ret.EB(x[0][0]+1,x[0][1]+1);
        ret.EB(x[0][1]+1,x[0][2]+1);
        ret.EB(x[0][0]+1,x[0][2]+1);
        rep(j,3,x[0].size())ret.EB(x[0][1]+1,x[0][j]+1);
        ret.EB(x[0][0]+1,x[0][1]+1);
      }
      else{
        ret.EB(x[0][0]+1,x[i][0]+1);
        rep(j,1,x[i].size())ret.EB(x[0][0]+1,x[i][j]+1);
        ret.EB(x[0][0]+1,x[i][0]+1);
      }
    }
  }
