...
inline void add(int x) { for (;x <= p;x += lowbit(x)) ++ c[x]; }
...
struct Segtree {
    ...
    void build(int rt, int l, int r) {
        if(l == r) { t[rt] = Dat(0, -1); return ; }
        int mid = (l + r) >> 1;
        build(LSON); build(RSON); pushup(rt);
    }
    void modify(int rt, int l, int r, int x, int y) {
        if(l == r) { t[rt] = (Dat){y, y}; return ; }
        int mid = (l + r) >> 1;
        x <= mid? modify(LSON, x, y) : modify(RSON, x, y);
        pushup(rt);
    }
}t;
...
inline void work() {
    read(n, m); p = 0; read(a + 1, a + n + 1); read(b + 1, b + m + 1);
    rep(i, 1, n) d[++ p] = a[i];
    rep(i, 1, m) d[++ p] = b[i];
    std::sort(d + 1, d + p + 1); std::sort(b + 1, b + m + 1);
    p = std::unique(d + 1, d + p + 1) - d - 1;
    rep(i, 1, n) a[i] = std::lower_bound(d + 1, d + p + 1, a[i]) - d, q[i] = i;
    rep(i, 1, m) b[i] = std::lower_bound(d + 1, d + p + 1, b[i]) - d;
    memset(c + 1, 0, p * sizeof(int));
    LL ans = 0;
    rep(i, 1, n) { add(a[i]); ans += i - ask(a[i]); }
    t.build(1, 1, n);
