...
private:
	struct edge {
		int to, cap; T cost; int rev; bool isrev;
	};
...
void solve() {
	...
	rep(i, m) {
		int u, v; cin >> u >> v; u--; v--;
		mc.add_edge(v, u, n+1, 0);
	}
	rep(i, n) {
		mc.add_edge(n, i, 1, a[i]);
		//mc.add_edge(i, n, 1, -a[i]);
		d[i] -= 1;
		d[n] += 1;
		val -= a[i];
		mc.add_edge(n, i, 1, a[i]);
	...
	val+=mc.minimum_cost_flow(s, t,n);
	vector<ll> ans = mc.ds(n); ans.resize(n);
	//cout << -val << "\n";
	coutarray(ans);
}
