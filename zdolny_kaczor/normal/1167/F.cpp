...
const int nax = 1 << 19, mod = 1e9 + 7;
pii tree[nax * 2];
void put(int x, pii val) {
...
int main() {
	...
	FOR(i, 1, n) {
		int x;
		scanf("%d", &x);
		ord.emplace_back(x, i);
	}
	sort(ALL(ord));
	ll ans = 0;
	for (pii c : ord) {
		int v = c.first, x = c.second;
		pii left = sum(0, x - 2), right = sum(x, n - 1);
		debug(imie(left), imie(right));
		ll mul1 = (n - x + 1ll) * left.first, mul2 = (n - x + 1ll) * x, mul3 = x * ((n + 1ll) * right.second % mod - right.first);
		debug(imie(v), imie(x), imie(mul1), imie(mul2), imie(mul3));
		ans = (ans + (mul1 + mul2 + mul3) % mod * v) % mod;
		put(x - 1, {x, 1});
	}
	ans %= mod;
	if (ans < 0) ans += mod;
