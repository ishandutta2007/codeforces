	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			...
			if(q >= 0){
				...
				int ind = Arrays.binarySearch(a, numin);
				if(ind < 0)ind = -ind-2;
				for(int j = ind;j >= 0;j--){
					int entry = findGFenwick(ft, a[j]-1)+1;
					addFenwick(ft, entry, -1);
					numin--;
				}
			}
		}
		
		int[] res = restoreFenwick(ft);
		if(numin == 0){
			out.println("Poor stack!");
		}else{
			char[] ret = new char[numin];
			int u = 0;
			for(int i = 0;i < p;i++){
				if(res[i] == 1){
					ret[u++] = ined[i] ? '1' : '0';
