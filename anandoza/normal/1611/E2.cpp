...
void solve() {
    ...
    auto vlad = BFS(adj, {0});
    auto friends = BFS(adj, x);

    for (int i = 1; i < n; i++) {
        if (sz(adj[i]) == 1 && vlad.distance[i] < friends.distance[i]) {
            ps(-1);
            return;
        }
    }
    ...
    for (int i = 1; i < n; i++) {
        if (friends.distance[i] <= vlad.distance[i])
            mark[i] = true;
    }
    pd(mark);
