...
const int maxn = 1e5 + 100, maxw = 5e6 + 100, inf = 1e9 + 100, sq = 450, LG = 20, mod = 1e9 + 933, mod1 = 1e9 + 993;

map<int, pair<ordered_set<pair<int, int>>, ordered_set<pair<int, int>>>> q;
...
void add(int x1, int y1, int y2) {
    //cerr << x1 << ' ' << y1 << ' ' << x1 + y2 - y1 << ' ' << y2 << '\n';
    int it = counter++;
    int id = x1 - y1;
    int l = y1, r = y2;
    if (l > r)
...
bool get(int x, int y) {
    ...
    if (q.find(id) != q.end()) {
        int sz = q[id].fr.size();
        int A = (sz - q[id].sc.order_of_key({w, -1}));
        int B = (sz - q[id].fr.order_of_key({w + 1, -1}));
        if (A > B)
            return 1;
    }
    return 0;
...
int main() {
    ...
    while (1) {
        ...
        while (1) {
            bool has = 0;
            while (!A.empty() && A.begin()->fr <= x) {
                has = 1;
                int X = A.begin()->fr, w = A.begin()->sc;
                A.erase(A.begin());
                if (w == -1) {
                    if (X == x)
                        rx = 1;
            ...
            while (!B.empty() && B.begin()->fr <= y) {
                has = 1;
                int X = B.begin()->fr, w = B.begin()->sc;
                B.erase(B.begin());
                if (w == -1) {
                    if (X == y)
                        ry = 1;
