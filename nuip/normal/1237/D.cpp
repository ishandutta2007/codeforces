...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<ll> a(n);
	rep(i,n) cin>>a[i];
	rep(_,2)rep(i,n) a.pb(a[i]);
	vector<pll> stk{{MOD,-1}};
	vector<int> hate(3*n);
	rep(i,3*n){
		ll mx=2*a[i];
		ll l=0,r=stk.size();
	...
	rep(i,n){
		...
		while(r-l>1){
			int m=(l+r)/2;
			(rmq.get(i+1,m)<i?l:r)=m;
		}
		out(i,l,r,rmq.get(i+1,l),rmq.get(i+1,r),hate[r],1);
		re[i]=r-i;
	}
	out(re,1);
	for(int x:re) cout<<x<<" "; cout NL;
  return 0;
}