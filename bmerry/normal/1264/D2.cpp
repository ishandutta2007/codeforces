...
static vector<mr> make_csums(int n)
{
    vector<mr> out(n + 2);
    for (int i = 0; i <= n; i++)
    {
        mr c = fact[n] * ifact[i] * ifact[n - i];
        out[i + 1] = out[i] + c;
    }
    return out;
}
...
int main(int argc, const char **argv)
{
    ...
    cin >> s;
    int N = SZ(s);
    vi prefL(N + 1), prefR(N + 1), prefQ(N + 1);
    ...
    for (int i = 0; i < N; i++)
    {
        prefL[i + 1] = prefL[i] + (s[i] == '(');
        prefR[i + 1] = prefR[i] + (s[i] == ')');
    ...
    vector<mr> csums[2];
    csums[0] = make_csums(q0);
    csums[1] = make_csums(q0 - 1);
    ...
    for (int i = 1; i < N; i++)
    {
        ...
        int b = prefR[N] - prefR[i];
        int y = prefQ[N] - prefQ[i];
        int q = x + y;
        int z = y + b - a;
        z = max(-1, min(q, z));
        ans += csums[q0 - q][z + 1];
