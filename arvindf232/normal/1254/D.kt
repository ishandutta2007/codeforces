...
class lazySegTree (nSuggest  :Int){
    ...
    val lazy = IntArray(n) // l // lazy : already applied to this node
    ...
    fun nodeAdd(x:Int,level:Int,inc:Int){
        ...
        arr[x] = arr[x] modPlus (inc modM (1 shl level))
        if(x < n){
...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    fun sumQuery(l:Int, r:Int):Int{
        return st.queryFunctional(l,r)
    }
    fun rootpathsum(v: Int): Int {
        val start = v
        var v = v
        var ret = 0
        while(true){
//            ret = ret modPlus sumQuery(intime[top[v]], intime[v])
            v = top[v]
            if (v == root) {
                break
            }
            ret = ret modMinus  (mark[parent[v]] modM size[v] )
            v = parent[v]
        }
        return ret modPlus (range.rangeQueryLazy(intime[start], intime[start]))
    }
    ...
    solve.cases{
        ...
        repeat(q){
            ...
            if(t == 1){
                ...
                val v = getint -1
                val d = getint
                global = global modPlus d
                if(v != 0){
                    val left = (n - G.size[v]) modM d
                    global = global modMinus (left modM ninverse)
                    G.range.rangeAdd(G.intime[v], G.outtime[v], left)
                }
                G.mark[v] = G.mark[v] modPlus d
//                G.st.addValue(G.intime[v],d)
                if(G.hson[v] != -1) {
                    val w = G.hson[v]
                    G.range.rangeAdd(G.intime[w], G.outtime[w],(d modM G.size[w]).additiveInverse())
                }
            }else{
                val v = getint -1
                val first = G.rootpathsum(v)
                val ans = (first modM ninverse) modPlus global
                put(ans)
            }
        }
