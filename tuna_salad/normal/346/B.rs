...
fn main() {
	...
	let mut dp = v!([sn+1][tn+1][vn+1] = (-1,0,0,0));
	dp[0][0][0] = (0,0,0,0);
	for i in 0..=sn {
		for j in 0..=tn {
			for k in 0..vn {
	...
	if best <= 0 {
		...
		let mut i = sn;
		let mut j = tn;
		let mut sol = v!([best as usize]);
		while i > 0 || j > 0 {
			let (_,ii,jj,kk) = dp[i][j][k];
			if ii != i && jj != j {
				sol.push(s[ii]);
			}
			i = ii;
		...
		sol.reverse();
		writeln!(rout, "{}", str::from_utf8(&sol).unwrap()).ok();
	}
}