...
void solve() {
	...
	for(x=1;x<=N;x++) {
		...
		if(x-1>=N/2+1) continue;
		ll del1=0,del2=0;
		for(y=0;y<=N-x;y++) {
			if(y>=N/2+1) {
				// new
				tot-=comb(N-x,y)*fact[y-1]%mo*fact[N-y-1]%mo;
				del1+=comb(N-x,y)*fact[y-1]%mo*fact[N-y-1]%mo;
			}
			if(x-1+y>=N/2+1) {
				// cur
				tot-=comb(N-x,y)*fact[x+y-2]%mo*(x-1)*fact[N-(x+y)]%mo;
				del2+=comb(N-x,y)*fact[x+y-2]%mo*(x-1)*fact[N-(x+y)]%mo;
			}
		}
		ret[x]=tot;
	}
