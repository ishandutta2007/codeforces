...
class FDvukhtsvetnieOtrezki {
public:
    void solve() {
        int n = in.readInt();
        arri l, r, t;
        in.readArrays(n, l, r, t);
        decreaseByOne(t);

        compress(l, r);
        arr<pair<pii, int>> segs(n);
        for (int i : range(n)) {
            segs[i] = {{l[i], r[i]}, t[i]};
        }
        sort(all(segs));
        vector<SegmentTree<int, int>> trees;
        for (int i : range(2)) {
            trees.emplace_back(2 * n, [&](int a, int b) -> int {
                return max(a, b);
            }, [&](int a, int b) -> int {
                return a + b;
            }, [&](int a, int b, int, int) -> int {
                return a + b;
            });
        }
        for (const auto& s : segs) {
            int best = max(trees[1 - s.second].query(0, s.first.first), trees[s.second].query(0, s.first.second));
            int current = trees[s.second].query(s.first.second, s.first.second + 1);
            if (best > current) {
                trees[s.second].update(s.first.second, s.first.second + 1, best - current);
            }
            trees[s.second].update(s.first.second, 2 * n, 1);
        }
        out.printLine(max(trees[0].query(0, 2 * n), trees[1].query(0, 2 * n)));
    }
};
