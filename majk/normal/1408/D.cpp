...
public:
    void solve(istream& cin, ostream& cout) {
        int N, M; cin >> N >> M;
        vector<pii> R(N); cin >> R;
        vector<pii> S(M); cin >> S;
        vector<int> Diff(MX, 0);
        for (int i = 0; i < N; ++i) {
            for (int j = 0; j < M; ++j) {
                int x = max(0, S[j].x - R[i].x + 1);
                int y = max(0, S[j].y - R[i].y + 1);
                if (x) {
                    Diff[x-1] = max(Diff[x-1], y);
                }
            }
        }
