...
int main(void)
{
	...
	ll sum = 0;
	rep(i, 0, 2) sum += x[i] * y[i];
	
	ll n = 0;
	rep(i, 1, 100000) if(i*i == sum){
		n = i;
		break;
	}
	if(n == 0){
	...
	do{
		rep(i, 0, 7){
			rep(j, 0, 2) if(i & (1<<j)) swap(x[j], y[j]);
			
			if(x[0] == n && x[1] == n && x[2] == n && y[0]+y[1]+y[2] == n){
				outl(n);
				rep(r, 1, n){
					rep(c, 1, n){
						if(r <= y[0]) cout << "A";
						else if(r <= y[0]+y[1]) cout << "B";
						else cout << "C";
					}
					cout << endl;
				}
			...
			if(x[p[0]] == n && y[p[0]]+y[p[1]] == n && y[p[0]]+y[p[2]] == n && x[p[1]]+x[p[2]] == n){
				outl(n);
				rep(r, 1, n){
					rep(c, 1, n){
						if(r <= y[p[0]]) cout << (char)(p[0]+'A');
						else if(c <= x[p[1]]) cout << (char)(p[1]+'A');
						else cout << (char)(p[2]+'A');
					}
					cout << endl;
				}
				return 0;
