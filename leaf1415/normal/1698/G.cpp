...
{
	F ret = F(0); ll deg = 0;
	rep(i, 0, 75) if(m[i]) deg = i;
	rep(i, 0, 40) if(f[i]) ret ^= g << i;
	per(i, 75, deg) if(ret[i]) ret ^= m << (i-deg);
	return ret;
}

void div(F f, F g, F &q, F&r)
{
	q = F(0); ll deg = 0;
	rep(i, 0, 75) if(g[i]) deg = i;
	per(i, 75, deg) if(f[i]) f ^= g << (i-deg), q[i-deg] = 1;
	r = f;
}
...
{
	if(n == 0) return F(1);
	if(n % 2) return mul(calc(f, n-1, m), f,  m);
	else return calc(mul(f, f, m), n/2, m);
}

F extgcd(F a, F b, F &x, F &y)
{
	if(b == F(0)){
		x = F(1), y = F(0);
		return a;
	...
	F xx, yy, q, r;
	div(a, b, q, r);
	F d = extgcd(b, r, xx, yy);
	x = yy, y = xx^mul(q, yy, F(1LL<<60));
	return d;
}
...
unordered_map<F, ll> mp;
ll BabyStep_GiantStep(F a, F y, F mod)
{
	mp.clear();
	ll m = 1LL<<18, ans = inf; F f = F(1);
	for(int j = 0; j <= m; j++){
		if(j > 0 && !mp.count(f)) mp[f] = j;
		f = mul(f, a, mod);
	}
	F g = inv(a, mod);
	g = calc(g, m, mod);
...
int main(void)
{
	...
	ll cnt = 0;
	while(sz(s) && s[0] == '0') cnt++, s.erase(s.begin());
	if(sz(s) == 0){
		outl(-1);
		return 0;
	}
	...
	ll ans = BabyStep_GiantStep(F(2), F(1), m);
	if(ans > inf/2){
		outl(-1);
		return 0;
	}
	outl(cnt+1, cnt+ans+1);
