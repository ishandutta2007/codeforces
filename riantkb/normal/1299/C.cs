...
class Solver {
    ...
    public void solve() {
        int n = sc.Int;
        var a = sc.IntArr;
        var st = new Stack<pair<double, int>>();
        for (int i = n - 1; i >= 0 ; i--)
        {
            var now = new pair<double, int>(a[i], 1);
            while (st.Count > 0) {
                if (st.Peek().v1 < now.v1) {
                    var p = st.Pop();
                    now = new pair<double, int>((now.v1 * now.v2 + p.v1 * p.v2) / (now.v2 + p.v2), now.v2 + p.v2);
                }
                else break;
            }
            st.Push(now);
