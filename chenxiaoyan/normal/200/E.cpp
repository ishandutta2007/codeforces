...
ti4 ans(inf, inf, inf, inf);
void chk(int k3, int k4) {
  int c3k3 = c[3] * k3, c4k4 = c[4] * k4, c5k5 = s - c3k3 - c4k4;
  int f = abs(c3k3 - c4k4) + abs(c4k4 - c5k5);
  chkmn(ans, mt(f, k3, k4, c5k5 / c[5]));
}

void mian() {
  n = read(), s = read();
  REP(i, 1, n) ++c[read()];
  REP(k3, 0, s) st[c[3] * k3 % c[5]].insert(k3);
  REP(k4, 0, s) {
    int c4k4 = c[4] * k4;
    int s0 = s - c4k4;
    if(s0 < 0) continue;
    ...
    PER(i, 20, 0) {
      int k3 = bd + (1 << i);
      int c3k3 = c[3] * k3, c5k5 = s0 - c3k3;
      int f = abs(c3k3 - c4k4) + abs(c4k4 - c5k5);
      ++k3;
      c3k3 = c[3] * k3, c5k5 = s0 - c3k3;
      int f0 = abs(c3k3 - c4k4) + abs(c4k4 - c5k5);
      if(f >= f0) bd += 1 << i;
    }
    int r = min(k4, (s0 - c[5] * k4) / c[3]);
    set<int> &st = ::st[s0 % c[5]];
    auto fd = st.upper_bound(min(r, bd));
    if(fd != st.end() && *fd <= r) chk(*fd, k4);
    if(fd != st.begin()) chk(*--fd, k4);
  }
  if(X(ans) == inf) puts("-1");
  else prt(Y(ans)), pc(' '), prt(Z(ans)), pc(' '), prt(W(ans)), pc('\n');
}
