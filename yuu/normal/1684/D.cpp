...
void solve() {
    ...
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) ans += a[i];    // pay for everything
    for (int i = 1; i <= n; i++) a[i] -= n - i;  // this amount is saved for each i
    sort(a + 1, a + n + 1);
    reverse(a + 1, a + n + 1);
    for (int i = 1; i <= k; i++) ans -= a[i];
    ans -= k * (k - 1) / 2;  // this amount of bonus is saved by pair of jumped over stuff
    cout << ans << '\n';
}
