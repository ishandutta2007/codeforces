...
ntt::Poly<ntt::NTT<ModT>> ntt(1 << LOG_N);

using Poly4 = std::array<std::array<std::vector<ModT>, 2>, 2>;

Poly4 multiply(const Poly4 &a, const Poly4 &b) {
  Poly4 out;
  for (int x = 0; x < 2; ++x) {
    for (int y = 0; y < 2; ++y) {
      ntt.multiply(out[x][y], a[x][0], b[0][y]);
      std::vector<ModT> r1;
      ntt.multiply(r1, a[x][1], b[1][y]);
      if (out[x][y].size() < r1.size() - 1) {
        out[x][y].resize(r1.size() - 1);
      }
      for (int i = 1; i < static_cast<int>(r1.size()); ++i) {
        out[x][y][i - 1] += r1[i];
      }
    }
  }
...
Poly4 recur(const std::vector<int> &s, int l, int r) {
  if (l + 1 == r) {
    Poly4 result;
    for (int a = 0; a < 2; ++a) {
      for (int b = 0; b < 2; ++b) {
        result[a][b].resize(a + b + 1);
        result[a][b][a + b] = a && b && s[l] == 1 ? ModT(2).inverse() : ModT(1);
      }
    }
    return result;
  }
  int m = (l + r) >> 1;
  return multiply(recur(s, l, m), recur(s, m, r));
}
...
int solve(int n, const std::vector<int> &a) {
  ...
  for (int i = 0; i < n;) {
    ...
    s.push_back(a[i] == 1 ? 1 : 2);
    i += a[i];
  }
  int m = s.size();
  std::vector<ModT> dp = recur(s, 0, m)[1][1];
  ModT result(0), fact(1);
  for (int i = 0; i < m; ++i) {
    fact *= ModT(i + 1) * ModT(2);
    if (m - 1 - i & 1) {
      result -= dp[i + 2] * fact;
    } else {
      result += dp[i + 2] * fact;
  ...
  return result.get();
  /*
  std::vector<std::vector<ModT>> dp(m + 1, std::vector<ModT>(m, ModT(0)));
  for (int i = 1; i <= m; ++i) {
    dp[i][0] = ModT(i == 1 && a[0] == 1 ? 1 : 2);
    for (int j = 1; j < i; ++j) {
      for (int k = 0; k < j; ++k) {
        dp[i][k + 1] += dp[j][k] * ModT(j + 1 == i && s[j] == 1 ? 1 : 2);
      }
    }
  }
  ModT result(0), fact(1);
