...
void solve() {
    ...
    auto rec = [&](auto&&rec, int v, int p) -> void {
        for(int nv: g[v]) {
            ...
            rec(rec, nv, v);
            sum[v] += dp[nv][0];
            if(chmax(ma[v], dp[nv][0] - dp[nv][1])) {
                id[v] = nv;
                cnt[v] = 1;
            } else if(ma[v] == dp[nv][0] - dp[nv][1]) {
                ++ cnt[v];
            }
        }
        if(ma[v] != -1) {
            chmin(dp[v][1], sum[v] - ma[v]);
            chmin(dp[v][0], sum[v] - ma[v] + c[v]);
            chmin(dp[v][0], sum[v]);
        } else {
            chmin(dp[v][1], 0LL);
            chmin(dp[v][0], c[v]);
    ...
    ll ans = dp[0][0];
    vc<ll> res;
    auto dfs = [&](auto&&dfs, int v, int p, int f) -> void {
        debug(v, f);
        if(dp[v][1] == dp[v][0]) f = 0;
        if(ma[v] == -1 && dp[v][f] == c[v]) res.eb(v);
        if(ma[v] != -1 && dp[v][f] == sum[v] - ma[v] + c[v] && f == 0) {
            res.eb(v);
        }
        for(int nv: g[v]) {
            if(nv == p) continue;
            int nf = (dp[v][f] != sum[v] && cnt[v] == 1 && id[v] == nv ? 1 : 0);
            dfs(dfs, nv, v, nf);
        }
    };
    dfs(dfs, 0, -1, 0);
