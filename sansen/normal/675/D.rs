...
fn run() {
    ...
    let mut a = x.clone().into_iter().enumerate().map(|p| (p.1, p.0)).collect::<Vec<_>>();
    a.sort();
    let mut seg = segment_tree::PURQ::new(n, (n, n), |a, b| std::cmp::min(*a, *b));
    for (i, a) in a.iter().enumerate() {
        seg.update_tmp(i, (a.1, i));
    }
    seg.update_all();
    let mut parent = vec![0; n];
    let mut dfs = vec![(0, n, n)];
    while let Some((l, r, p)) = dfs.pop() {
        ...
        parent[v.0] = p;
        dfs.push((l, v.1, v.0));
        dfs.push((v.1 + 1, r, v.0));
    }
    let out = std::io::stdout();
    let mut out = std::io::BufWriter::new(out.lock());
    for (i, &p) in parent[1..].iter().enumerate() {
        if i > 0 {
            write!(out, " ").ok();
        }
        write!(out, "{}", x[p]).ok();
