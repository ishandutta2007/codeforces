...
int main() {
    ...
    REP(i, N) {
        int cum = accumulate(C.begin(), C.begin() + i, 0);
        if (C[i] > cum + D) {
    ...
    bitset<100000 * 50 + 1> dp;
    dp.set(0);
    for (auto x : C) dp |= dp << x;
    const int ret = accumulate(C.begin(), C.end(), 0);
    ShortestPath<int> graph(ret + 1);
    REP(i, ret) graph.add_edge(i + 1, i, 0);
    REP(i, ret + 1) {
        if (dp[i]) {
            int j = min(ret, i + D);
