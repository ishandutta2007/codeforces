...
vll manacher(vvll s,vll od) {
    vll radius(s.size());
    int i = 0, j = 0;
    while (i < s.size()) {
        if (od[i] >= 2) { i++; continue; }
        while (i - j >= 0 && i + j < s.size() && s[i - j] == s[i + j]&&od[i-j]<2&&od[i+j]<2) {
            ++j;
        }
       radius[i] = j;
        ll k = 1;
...
int main() {
    ...
    rep(i, M) {
        ...
        for (ll j = i; j < M; j++) {
            ...
            rep(k, N) {
                cnt[2*k+1][s[k][j]]++;
                if (cnt[2*k+1][s[k][j]] % 2 == 1) { od[2*k+1]++; }
                else { od[2*k+1]--; }
            }
           
            
            //rep(k, 2 * N + 1) {
            //    rep(l, 3)cout << cnt[k][l];
            //    cout << endl;
            //}
            
            vll v = manacher(cnt,od);
            
            //rep(k,2*N+1)cout << v[k] << " ";
            //cout << endl;
            ll now = 0;
            rep(k, 2 * N + 1) { now += v[k]; }
            now = (now - N - 1) / 2;
