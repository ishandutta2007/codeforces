// time-limit: 3000
// problem-url: https://codeforces.com/contest/1710/problem/D

// ... can hope
// did not prove very carefully though
// although bugs are more likely than not "wrong algorithm" and unfixable
...
void up(){
	...
	resultEdges.reserve(number-1);

	for(int const len: range(2, number+1)){
		for(int const left: range(0, number-len+1)){
			assert(left<=number-len);
			auto const right=left+len;
			assert(right<=number);

			auto const connected=lines[left][len-1];
			if(connected){

				auto leftIter=bounds.upper_bound(left);
				auto rightIter=bounds.lower_bound(right);

				// the segment from leftIter-1..leftIter is the leftmost segment intersecting left..right
				// the segment from rightIter-1..rightIter is the rightmost segment intersecting left..right

				if(leftIter==rightIter) continue;

				std::vector<int> points{left};
				points.insert(points.end(), leftIter, std::prev(rightIter));
				points.push_back(right-1);

				bounds.erase(leftIter, rightIter);
				
				assert(points.size()!=3);

				if(points.size()%2==0){
					int last=points[1];
					for(int i=3; i<int(points.size()); i+=2){
						resultEdges.push_back({last, points[i]});
						last=points[i];
					}
					...
					resultEdges.push_back({points[2], points[0]});
					int last=points[0];
					for(int i=4; i<int(points.size()); i+=2){
						resultEdges.push_back({last, points[i]});
						last=points[i];
					}
	...
	assert(int(resultEdges.size())==number-1);
	for(auto [a, b]: resultEdges) std::cout<<b+1<<' '<<a+1<<'\n';
	assert(std::cerr<<'\n');
