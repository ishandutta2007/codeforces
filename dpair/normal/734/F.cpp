...
int n, a[MAXN], b[MAXN], c[MAXN]; LL s = 0;
int cnt[32];
signed main(){
    read(n); read(b + 1, b + n + 1); read(c + 1, c + n + 1);
    rep(i, 1, n) s += b[i] + c[i];
    if(s % (n << 1)) return print("-1"), 0;
    s /= (n << 1); rep(i, 1, n) {
        a[i] = b[i] + c[i] - s;
        if(a[i] % n) return print("-1"), 0;
        a[i] /= n; rep(j, 0, 31) cnt[j] += (a[i] >> j & 1);
    } 
    rep(i, 1, n) { LL x = 0, y = 0;
        rep(j, 0, 31) {
            if(a[i] >> j & 1) {
                x += (1ll << j) * cnt[j];
                y += (1ll << j) * n;
        ...
        if(x != b[i] || y != c[i]) return print("-1"), 0;
    }
    print(a + 1, a + n + 1);
}