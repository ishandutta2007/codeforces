...
void solve() {
  ...
  dp[0][0] = 0;
  FORIN(s, S) {
    Matrix<ll> newdp(N + 1, N + 1);
    newdp.fill(-INF);
    FOR(k, N + 1) {
      FOR(a, N + 1) {
        if (dp[k][a] == -INF) continue;
        // T[0]  T[1] 
        chmax(newdp[k][a], dp[k][a]);
        // T[0] == T[1]
        if (T[0] == T[1]) {
          ll cost = (s == T[0] ? 0 : 1);
          chmax(newdp[k + cost][a + 1], dp[k][a] + a);
        }
        // T[0]
        ll cost = (s == T[0] ? 0 : 1);
        chmax(newdp[k + cost][a + 1], dp[k][a]);
        // T[1]
        cost = (s == T[1] ? 0 : 1);
        chmax(newdp[k + cost][a], dp[k][a] + a);
      }
    }
    swap(dp, newdp);
  }
