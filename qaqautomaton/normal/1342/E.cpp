...
inline Z operator+(const Z x1, const Z x2) { return norm(x1.x+x2.x);}
inline Z operator-(const Z x1, const Z x2) { return norm(x1.x+p-x2.x);}
...
Z C(int a,int b){return fac[a]*invf[a-b]*invf[b];}
Z S(int n,int k){
	Z ans=0;
	for(int i=0;i<=k;++i){
		Z s=fpm(k-i,n)*C(k,i);
