...
void q() {
  ...
  rep(i, N - 1) {
    ini(u, v, x, y);
    --u, --v;
    g[u].push_back(vi{v, x, y});
    g[v].push_back(vi{u, y, x});
  }
  ...
  auto dfs = [&](auto rc, int c, int p = -1, mint dat = 1) -> void {
    sm += dat;
    for (auto& v : g[c]) {
      int d = v[0], x = v[1], y = v[2];
      if (d == p) continue;
      each(f, osak<MAX>(y)) cur[f]--;
      each(f, osak<MAX>(x)) {
        cur[f]++;
        amax(mx[f], cur[f]);
      }
      rc(rc, d, c, dat * y / x);
      each(f, osak<MAX>(y)) cur[f]++;
      each(f, osak<MAX>(x)) cur[f]--;
    }
  };
  dfs(dfs, 0);
  rep1(i, N + 5) if (mx[i]) sm *= mint{i}.pow(mx[i]);
  out(sm);
}
