...
inline bool read ()
{
    ...
    forn(i, cnt)
    {
        ...
        //forn(j, 50000) buf[j] = rand() % 26 + 'a';
        assert(scanf("%d%d", &occ[i].ft, &occ[i].sc) == 2);
        //occ[i].ft = rand(), occ[i].sc = occ[i].ft + 5000;
...
tree ins (tree root, tree t)
{
    ...
    if (root->val < t->val) root->rg = ins(root->rg, t);
    else root->lf = ins(root->lf, t);
