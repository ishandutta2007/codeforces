...
void solve() {
  ...
  auto div = [&](ll dy, ll dx) -> Re {
    ll g = gcd(dx, dy);
    dx /= g, dy /= g;
    return Re(dx) / dy;
  };

  vc<pair<Re, int>> change;

  FOR(j, N) FOR(i, j) {
    auto [ai, bi, ci] = dat[i];
    auto [aj, bj, cj] = dat[j];
    if (ci == cj) continue;
    Re l = div(cj - ci, aj - bi);
    Re r = div(cj - ci, bj - ai);
    change.eb(l, 1);
    change.eb(r, -1);
  }
  ...
  ll n = 0;
  Re x = -LIM;
  for (auto&& [nx, t]: change) {
    if (n == 0) cand.eb(x), cand.eb(nx);
    x = nx;
    n += t;
  }

  CHT_monotone<Re, 1> cht_l;
  CHT_monotone<Re, 0> cht_r;
  FOR(i, N) {
    auto [a, b, c] = dat[i];
    cht_l.add(-c, a);
    cht_r.add(-c, b);
  ...
  Re ANS = 1e18;
  for (auto&& x: cand) {
    Re y = cht_r.query_monotone_inc(x) - cht_l.query_monotone_inc(x);
    chmin(ANS, y);
  }
  print(ANS);
}
