...
int main() {
	...
	F0R(_,Q) {
		int K; re(K); str W; re(W);
		str cur = ori;
		int pos = 0;
		mi ans = 0;
		while (pos < K && sz(cur) < sz(W)) {
			cur = cur+T[pos]+cur;
			++ pos;
		}
		if (sz(cur) < sz(W)) {
		...
		vi z_fo = getPrefix(W,cur);
		vi z_bk = getPrefix(rev(W),rev(cur));
		// dbg("WUT",W,cur);
		int matches = 0;
		trav(t,z_fo) if (t == sz(W)) ++matches;
		ans += pow(mi(2),K-pos)*matches;
		vmi contrib(26);
		F0R(i,sz(W)) {
			if (z_fo[sz(cur)-i] == i)
				if (z_bk[sz(cur)-(sz(W)-1-i)] == sz(W)-1-i) {
					++ contrib[W[i]-'a'];
				}
		}
		// dbg(pos,K,contrib);
		...
		mi mul = pow(mi(2),K);
		F0R(i,26) {
			ans += mul*contrib[i]*(cum[K][i]-cum[pos][i]);
		}

		// 2^{K-i} occurrences
		// now deal with i+1 to K
		// 2^-i
		ps(ans);
	}
