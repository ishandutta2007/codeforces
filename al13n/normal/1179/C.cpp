...
struct node{
  ...
  void upd(){
    int ls=v.Y+(l?l->s:0);
    s=ls+(r?r->s:0);
    mx=max(0,max(l?l->mx:0,ls+(r?r->mx:0)));
  }
};
...
struct tree{
  ...
  void init(int a,int b,int k,const vector<int>&A){
    ...
    T[k].mx=max(T[k*2+1].mx,T[k*2+1].s+T[k*2+2].mx);
  }
  ...
  void add(int p,int v,int a,int b,int k){
    ...
    int c=(a+b)/2;
    if(p<c) add(p,v,a,c,k*2+1);
    else add(p,v,c,b,k*2+2);
    T[k].s=T[k*2+1].s+T[k*2+2].s;
    T[k].mx=max(T[k*2+1].mx,T[k*2+1].s+T[k*2+2].mx);
  }

  int doit(int a,int b,int k,int s){
    assert(s+T[k].mx>0);
    if(b==a+1) return a;
    int c=(a+b)/2;
    return s+T[k*2+1].mx>0 ? doit(a,c,k*2+1,s) : doit(c,b,k*2+2,s+T[k*2+1].s);
  }
};
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    T.add(B[a],+t,0,N,0);
    //dbg(T.T);
    cout<<(T.T[0].mx>0?N-T.doit(0,N,0,0):-1)<<'\n';
  }
