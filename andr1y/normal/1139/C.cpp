...
ll pd[N];
ll h[N];
void dfs(ll v = 1, ll p = -1, ll th = 1){
    pd[v] = 1;
    h[v] = th;
    for(auto i : d[v]) if(i!=p) {dfs(i, v, th+1);pd[v]+=pd[i];}
}
vector<pll> blacks;
ll binpowmod(ll a, ll b, ll m = M){
    if(b==0) return 1;
    if(b==1) return a;
    if(b%2) return (a*binpowmod((a*a)%m, b/2, m))%m;
    else return binpowmod((a*a)%m, b/2, m)%m;
}
main(){
    FAST;
    ll n, k, u, v, c;
    ...
    ll resmod = binpowmod(n, k);
    for(ll i =1;i<=n;i++){
        if(pd[i]==0){
            dfs(i);
            resmod = (resmod + M - binpowmod(pd[i], k))%M;
        }
    }
    cout << resmod;
}