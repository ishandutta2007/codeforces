...
public class CF965C {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\CF965C.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        long d = Long.parseLong(input.nextBlock());

        d = Math.min(d, (n + k - 1) / k);

        long max = 0;
        for (long i = Math.max(1, getTime(n, m, k)); i <= d; i++) {
            long last = binarySearch(i, m, n, k);
    ...
    public static long binarySearch(long target, long m, long n, long k) {
        long l = 1;
        long r = m;
        while (l < r) {
            long mid = (l + r + 1) >> 1;
            long time = getTime(n, mid, k);
    ...
    public static long getTime(long n, long x, long k) {
        return (n / x + k - 1) / k;
    }
