...
public class TaskD {
	...
	private static class State {
		...
		private State(int g, int h, int r, int s) {
			this.g = g;
			this.h = h;
			this.r = r;
			this.s = s;
		...
		public boolean equals(Object o) {
			...
			if (g != state.g) return false;
			if (h != state.h) return false;
	...
	private void solve() {
		in.readInt();
		char[] students = in.readString().toCharArray();
		Set<State> reachable = new HashSet<State>();
		int answer = go(0, 0, 0, 0, 0, students, reachable);
		if ((answer & 1) != 0)
	...
	private int go(int step, int g, int h, int r, int s, char[] students, Set<State> reachable) {
		State current = new State(g, h, r, s);
		if (reachable.contains(current))
			return 0;
		reachable.add(current);
		int min = Math.min(Math.min(g, h), Math.min(r, s));
		if (step == students.length) {
			...
			if (s == min)
				result += 8;
			return result;
		} else if (students[step] != '?') {
			if (students[step] == 'G')
				return go(step + 1, g + 1, h, r, s, students, reachable);
			if (students[step] == 'H')
				return go(step + 1, g, h + 1, r, s, students, reachable);
			if (students[step] == 'R')
				return go(step + 1, g, h, r + 1, s, students, reachable);
			if (students[step] == 'S')
				return go(step + 1, g, h, r, s + 1, students, reachable);
			throw new RuntimeException();
		} else {
			int result = 0;
			if (g == min)
				result |= go(step + 1, g + 1, h, r, s, students, reachable);
			if (h == min)
				result |= go(step + 1, g, h + 1, r, s, students, reachable);
			if (r == min)
				result |= go(step + 1, g, h, r + 1, s, students, reachable);
			if (s == min)
				result |= go(step + 1, g, h, r, s + 1, students, reachable);
			return result;
		}
	}
	...
	private static List<Test> createTests() {
		...
		tests.add(new Test("2\n" +
			...
			"Ravenclaw\n" +
			"Slytherin"));
		tests.add(new Test(generateTest(), "Gryffindor\n" +
			"Hufflepuff\n" +
			"Ravenclaw\n" +
			"Slytherin"));
//		tests.add(new Test("", ""));
