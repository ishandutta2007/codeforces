...
int main() {
    ...
    while (!st.empty()) {
        ...
        if (p.first < m1 && p.first < m2) { cout << -1 << endl; return 0; }

        
        if (p.first < m1) {
            swap(p.first, p.second);
            
            if (p.first > M2) { cout << -1 << endl; return 0; }
            now = p;
            M2 = p.first;
            m2 = p.second;
        }
            ...
            if (p.second > M1) { cout << -1 << endl; return 0; }
            now = p;
            M1 = p.second;
            m1 = p.first;
        }
            ...
            if (q.first > M1 && q.first > M2) { cout << -1 << endl; return 0; }

            if (q.first > M1) {
               
                if (q.second < m2) { cout << -1 << endl; return 0; }
                now = q;
                M2 = q.first;
                m2 = q.second;
            }
                ...
                if (q.first < m1) { cout << -1 << endl; return 0; }
                now = q;
                M1 = q.second;
                m1 = q.first;
            }
        ...
        if (nswap.count(now)) { x++; }
        else { y++; }

        st.erase(now);
        st.erase({ now.second,now.first });
    ...
    if (m1 > M2 || m2 > M1) { cout << -1 << endl; return 0; }
