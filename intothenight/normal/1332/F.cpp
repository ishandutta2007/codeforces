...
int main(){
	...
	vector<vector<Zp>> dp(n, vector<Zp>(3));
	auto sum = [&](int u){
		return accumulate(dp[u].begin(), dp[u].end(), Zp(0));
	};
	function<void(int, int)> dfs = [&](int u, int p){
		...
		for(auto v: adj[u]){
			if(v != p){
				...
				dp[u] = {
					dp[u][0] * sum(v)
					, dp[u][0] * (sum(v) + dp[v][0]) + dp[u][1] * (3 * dp[v][0] + 2 * dp[v][1] + 2 * dp[v][2])
					, dp[u][0] * (dp[v][0] + dp[v][1]) + dp[u][2] * (dp[v][0] * 2 + dp[v][1] * 2 + dp[v][2])
				};
			}
		}
	};
...
/*
dp: # of (E, V) in its subtree including empty, empty

dp[u][0]: u not in E, u not in V
dp[u][1]: u in E,     u not in V
dp[u][2]: u in E,     u in V
