...
int sub_to_errichto() {
   ...
   auto getmanhattan = [&](int x1, int y1, int x2, int y2) -> ll {
      return ll(abs(x1 - x2)) + abs(y1 - y2);
   };

   for (auto ord: vector<vector<pii>>({xord, yord})) {
      for (int i = 0; i + 1 < n; ++i) {
         int a = ord[i].se;
         int b = ord[i + 1].se;
         int d = getdist(X[a], Y[a], X[b], Y[b]);
         ed[a].push_back(pii(d, b));
         ed[b].push_back(pii(d, a));
      }
   }
   pp(ed);
   ...
   while (!pq.empty()) {
      ...
      for (pii nxt: ed[cur]) {
         pp(nxt);
         pp(dist[nxt.se], dist[cur] + nxt.fi);
         if (dist[cur] + nxt.fi < dist[nxt.se]) {
            dist[nxt.se] = dist[cur] + nxt.fi;
            pq.push(pii(dist[nxt.se], nxt.se));
         }
      }
   }
   pp(dist);

   ll ans = getmanhattan(sx, sy, fx, fy);;
   for (int i = 0; i < n; ++i) {
      ll d = getmanhattan(fx, fy, X[i], Y[i]);
      pp(i, d, dist[i]);
