...
const sieve<MAX + 1> sv;

mn count(vi l, vi r, int m, int p) {
    const int n = sz(l);
    for (auto &i : l)
        i = (i + p - 1) / p;
    for (auto &i : r)
    ...
    for (int i = 0; i < n; i++) {
        vmn diff(m + 2);
        auto add_range = [&](int L, int R, mn v) {
            if (L < sz(diff))
                diff[L] += v;
            if (R < sz(diff))
                diff[R] -= v;
        ...
        for (int j = 0; j <= m; j++) {
            add_range(j + l[i], j + r[i] + 1, dp[j]);
        }
        vmn next(m + 1);
        next[0] = diff[0];
        for (int j = 1; j <= m; j++) {
            next[j] = next[j - 1] + diff[j];
        }
        swap(dp, next);
    }

    return accumulate(all(dp), mn(0));
}
...
int main() {
    ...
    mn ans = 0;
    for (int i = 1; i <= MAX; i++) {
        ans += sv.nums[i].mu * count(l, r, m, i);
    }
