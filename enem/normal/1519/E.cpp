...
ll dfs(ll x, ll par, bool vis[], bool don[], V(pll) ad[], V(pll) &ans)
{
    vis[x] = true;
    V(ll) v;
    for(auto it:ad[x])
    {
        if(!don[it.F] && vis[it.F] && it.S!=par)
        {
            v.PB(it.S);
        }
        else if(!vis[it.F])
        {
            ll t = dfs(it.F, it.S, vis,don,ad,ans);
            if(t==-1) v.PB(it.S);
            else ans.PB(MP(it.S, t));
        }
    }
    don[x] = true;

    ll nv=v.size();
    for(ll i=0; i+1<nv; i+=2)
    {
        ans.PB(MP(v[i], v[i+1]));
    }
...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll n;
        cin>>n;
        map<pll, vector<pair<pll,ll>>> admp;
        REP(i,0,n)
        {
            ll a, b, c, d;
            cin>>a>>b>>c>>d;
            ...
            pll t1 = MP(c*b, d*(a+b));
            ll tt1 = __gcd(t1.F, t1.S);
            t1.F/=tt1; t1.S/=tt1;

            pll t2 = MP((c+d)*b, d*a);
            ll tt2 = __gcd(t2.F, t2.S);
            t2.F/=tt2; t2.S/=tt2;

            // cout<<i<<' '<<t1.F<<' '<<t1.S<<' '<<t2.F<<' '<<t2.S<<'\n';
        ...
        for(auto it:admp)
        {
            for(auto itt:it.S) ad[fracmp[it.F]].PB(MP(fracmp[itt.F], itt.S));
        }
        fracmp.clear();
        admp.clear();

        bool vis[nv], don[nv];
        REP(i,0,nv) vis[i] = don[i] = false;

        // REP(i,0,nv)
        // {
        //     cout<<i<<'\n';
        //     for(auto it:ad[i]) cout<<it.F<<' '<<it.S<<'\t';
        //     cout<<'\n';
        // }
