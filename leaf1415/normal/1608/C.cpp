...
int main(void)
{
	...
	while(T--){
		cin >> n;
		rep(i, 1, n) cin >> a[i], p[i] = P(a[i], i);
		rep(i, 1, n) cin >> b[i], q[i] = P(b[i], i);
		sort(p+1, p+n+1), sort(q+1, q+n+1);
		
		SCC scc(n);
		rep(i, 1, n-1){
			scc.add_edge(p[i+1].second, p[i].second);
			scc.add_edge(q[i+1].second, q[i].second);
		}
		scc.tpsort();
		ll c = scc.calcSCC();
		
		rep(i, 1, c) deg[i] = 0;
		rep(i, 1, n) for(auto u : scc.G[i]){
			if(scc.scc[i] == scc.scc[u]) continue;
			deg[scc.scc[u]]++;
		}
		vector<ll> vec;
		rep(i, 1, c) if(deg[i] == 0) vec.push_back(i);
		
		rep(i, 1, n) ans[i] = 0;
		if(sz(vec) == 1){
			ll r = vec[0];
			rep(i, 1, n) if(scc.scc[i] == r) ans[i] = 1;
		}
