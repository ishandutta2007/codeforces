...
ModInt comp(const ModInt& x) {
  return -x + 1;
}

void solve() {
  int n, m, rb, cb, rd, cd; ModInt p; cin >> n >> m >> rb >> cb >> rd >> cd >> p; --rb; --cb; --rd; --cd;
  p /= 100;
  int dr = 1, dc = 1, period = 0;
  vector<int> ts;
  for (int t = 0, y = rb, x = cb; ; ++t) {
    if (t > 0 && y == rb && x == cb && (dr == 1 || y == 0) && (dc == 1 || x == 0)) {
      period = t;
      break;
    }
    if (y == rd || x == cd) ts.emplace_back(t);
    if (y == 0 && dr == -1) dr = 1;
    if (y + 1 == n && dr == 1) dr = -1;
    if (x == 0 && dc == -1) dc = 1;
    if (x + 1 == m && dc == 1) dc = -1;
  ...
  // cerr << period << ": ";
  // REP(i, l) cerr << ts[i] << " \n"[i + 1 == l];
  const ModInt comp_p = comp(p);
  ModInt ans = 0;
  REP(i, l) ans += comp_p.pow(i) * ts[i] / comp(comp_p.pow(l));
  ans += comp_p.pow(l) / comp(comp_p.pow(l)).pow(2) * period * comp(comp_p.pow(l)) / p;
  ans *= p;
  cout << ans << '\n';
}
