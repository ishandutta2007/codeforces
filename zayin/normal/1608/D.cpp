...
int main()  {
    ...
    // cout<<B_<<" "<<W_<<" "<<_B<<" "<<_W<<" "<<__<<endl;

    int ans=0;

    //case 3: BB=WW

    poly B(_B+B_+1),W(_W+W_+1),_(2*__+1);
    for (int i=0;i<=_B+B_;++i) B[i]=C(B_+_B,i);
    for (int i=0;i<=_W+W_;++i) W[W_+_W-i]=C(W_+_W,i);
    for (int i=0;i<=2*__;++i) _[i]=C(2*__,i);
    ...
    // WW-BB+__+W_+_W
    int i=WW-BB+__+W_+_W;
    // cout<<"i="<<i<<endl;
    if (0<=i&&i<T.size())
        ans=(ans+T[i])%modu;
    // cout<<ans<<endl;
    
    //case 0: minus no BB,WW
    if (!BB&&!WW) ans=(ans-pw(2,__)+modu)%modu;
    //case 1: BW
    if (!BB&&!WW&&!WB&&!W_&&!_B) ++ans;
    //case 2: WB
    if (!BB&&!WW&&!BW&&!B_&&!_W) ++ans;
