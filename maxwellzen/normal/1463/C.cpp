...
ll step(ll start, ll goal, ll time) {
    if (abs(start-goal)<=time) return goal;
    if (goal<start) return start-time;
    return start+time;
}
...
void solve() {
    ...
    ll s=0, g=x[0], ans=0;
    FOR(i, 1, n) {
        ll nxt = step(s, g, t[i]-t[i-1]);
        if (btw(x[i-1], min(s, nxt), max(s, nxt))) ans++;
        s=nxt;
        if (nxt==g) g=x[i];
    }
    if (btw(x[n-1], min(s, g), max(s, g))) ans++;
    cout << ans << nl;
}
