...
int main() {
    ...
    loop(tc) {
        input(string, s, t);
        int bcnt = 0;
        for (char c : s) bcnt += c == 'B';
        for (char c : t) bcnt += c == 'B';
        if (bcnt % 2) {
            print("NO");
            continue;
        }
        s.erase(remove_if(all(s), [](char c) { return c == 'B'; }), s.end());
        t.erase(remove_if(all(t), [](char c) { return c == 'B'; }), t.end());

        deque<char> dq_s;
        for (char c : s) {
            if (dq_s.size() and dq_s.back() == c) {
                dq_s.pop_back();
            } else {
                dq_s.push_back(c);
            }
        ...
        deque<char> dq_t;
        for (char c : t) {
            if (dq_t.size() and dq_t.back() == c) {
                dq_t.pop_back();
            } else {
                dq_t.push_back(c);
            }
        }
        print(dq_s == dq_t ? "YES" : "NO");
    }
    return 0;
}