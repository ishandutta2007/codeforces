...
void solve() {
    ...
    FOR(i,1,n){
        AA[MP(x[i],y[i])]=i;
        an+=w[i];
        flow.add_edge(i*2-1,i*2,w[i]);
        ty[i]=get_type(MP(x[i],y[i]));
        if(ty[i]==0){
            flow.add_edge(0,i*2-1,INF);
        }else if(ty[i]==3){
            flow.add_edge(i*2,2*n+1,INF);
        }
    }
    FOR(i,1,n){
        REP(j,4){
            int nx=x[i]+dx[j];
            int ny=y[i]+dy[j];
            auto it=AA.find(MP(nx,ny));
            if(it!=AA.end()){
                int nid=it->S;
                if(ty[nid]==ty[i]+1){
                    flow.add_edge(2*i,2*nid-1,INF);
                }
            }
        }
    }
