...
public:
	...
	std::pair<T, T> min_range() const {
		return std::pair<T, T>{L.top() + add_L, R.top() + add_R};
	}

	void add_all(const T x) { min_f += x; }

	void add_x_minus_a(const T a) {
		if(L.top() >= a) min_f += L.top() - a;
		L.push(a);
		auto val = L.top();
		L.pop();
		R.push(val);
	...
	void add_abs_x_minus_a(const T a) {
		add_x_minus_a(a);
		add_a_minus_x(a);
	}
	...
	//g(x) = f(x-a)
	void translation(const T a) { add_L += a, add_R += a; }

	//g(x) = min(f(x-b) to f(x-a))
	void sliding_window_minimum(const T a, const T b) { add_L += a, add_R += b; }
...
void main_() {
	...
	//dp[i + 1][j] = abs(j - a[i]) + min(dp[i][1 ~ j])

	slope_trick<ll> trick;

	REP(i, n) {
		trick.right_set_min();
		trick.add_abs_x_minus_a(a[i]);
	}
