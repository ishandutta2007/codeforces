...
mint calc(ll g)
{
	...
	rep(i, 1, n){
		sum[0] = dp[i-1][0];
		rep(j, 1, M) sum[j] = sum[j-1] + dp[i-1][j];
		rep(j, 0, M){
			ll l = j - R[i], r = j - L[i];
			if(r < 0) continue;
			chmax(l, 0LL);
			dp[i][j] = sum[r];
			if(l-1 >= 0) dp[i][j] -= sum[l-1];
		}
	}
	
	mint ret = 0;
	rep(j, 0, M) ret += dp[n][j];
	return ret;
}
...
int main(void)
{
	...
	rep(i, 1, m/n) ans[i] = calc(i);
	per(i, 100000, 1){
		for(int j = 2*i; j <= 100000; j += i) ans[i] -= ans[j];
	}
	outl(ans[1]);
