...
#endif
	scanf("%d",&n);rep(i,1,n)scanf("%s%d",s,&a[i]),b[i]=s[0];
	t=0;
	rep(i,1,n)if(b[i]=='|')t|=a[i];else if(b[i]=='&')t&=a[i];else if(b[i]=='^')t^=a[i];
	int t1=t;
	t=1023;
	rep(i,1,n)if(b[i]=='|')t|=a[i];else if(b[i]=='&')t&=a[i];else if(b[i]=='^')t^=a[i];
	int t2=t;
	int a1=0,a2=1023,a3=0;
	rep(i,0,9)if((t1>>i&1)&&(t2>>i&1))a1+=(1<<i);
	rep(i,0,9)if(!(t1>>i&1)&&!(t2>>i&1))a2-=(1<<i);
	rep(i,0,9)if((t1>>i&1)&&!(t2>>i&1))a3+=(1<<i);
	puts("3");
	printf("| %d\n& %d\n^ %d\n",a1,a2,a3);
	return 0;
}