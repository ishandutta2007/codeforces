...
void solve() {
    ...
    F0R(i, n) if (! vis[i]) {
        vpi ord;
        F0R(j, n) if (j!=i) ord.pb(mp(dist(i, j), j));
        sort(all(ord));
        int val = ord[0].F, cnt=0;
        bool works = true;
        while (cnt<n-1 && ord[cnt].F==val) cnt++;
        F0R(j, cnt) {
            if (vis[ord[j].S]) works=false;
            F0R(k, cnt) if (k!=j && dist(ord[j].S, ord[k].S) != val) works=false;
            FOR(k, cnt, n-1) if (dist(ord[j].S, ord[k].S) <= val) works=false;
        }
        if (works) {
            g.pb(cnt+1);
            vis[i]=true;
            F0R(j, cnt) vis[ord[j].S]=true;
        } else {
            g.pb(1);
            vis[i]=true;
        }
    ...
    cnt[m]=1;
    for (int i : g) if (i>1) {
        ROF(j, m+i-1, n+1) cnt[j] = (cnt[j] + cnt[j-i+1])%MOD;
    }
    debug(g);
    debug(cnt);
    ll ans = 0;
    factinit(n);
    FOR(i, m, n+1) {
        ans = (ans + fact[n] * ifact[n-i]%MOD * cnt[i]) % MOD;
    }
    cout << ans << nl;
}
