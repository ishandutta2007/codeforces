#line 1 "codeforces-workspace/contest/1491/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1491/c/c.cpp"
  ...
  for (auto i : range(n)) {
    ...
    while (s[i] > 1) {
      ...
      while (pos < n) {
        int nx = pos + s[pos];
        s[pos]--;
        if (s[pos] == 1) yet.erase(pos);
        auto lb = yet.lower_bound(nx);
        if (lb == end(yet)) break;
        nx = *lb;
        pos = nx;
      }
