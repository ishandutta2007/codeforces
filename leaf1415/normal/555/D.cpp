...
ll calc(ll id, ll len, ll dir)
{
	if(len == 0) return id;
	ll x = pos[id];
	
	ll nid;
	if(dir == 1) nid = p[upper_bound(p+1, p+n+1, P(x+len, inf)) - p - 1].second;
	else nid = p[lower_bound(p+1, p+n+1, P(x-len, -inf)) - p].second;
	
	if(id == nid) return id;
	ll l = abs(pos[nid]-pos[id]);
	if(len/l%2 == 0) return calc(id, len%l, dir);
	else return calc(nid, len%l, 1-dir);
}
...
int main(void)
{
	...
	cin >> n >> Q;
	rep(i, 1, n) cin >> pos[i], p[i].first = pos[i], p[i].second = i;
	sort(p+1, p+n+1);
	
	ll id, l;
	rep(i, 1, Q){
		cin >> id >> l;
		ll nid = p[upper_bound(p+1, p+n+1, P(pos[id]+l, inf)) - p - 1].second;
		cout << calc(nid, l - (pos[nid]-pos[id]), 0) << "\n";
	}
