...
int main(){
    LL(n,k);
    vv(ll,ans,n,n);
    auto solve=[&](vector<ll>a,ll d,auto solve)->void{
        if(a.size()<=1)return;
        vector<vector<ll>>b(k);
        rep(a.size())b[i*k/a.size()].push_back(a[i]);
        rep(k)rep(j,i+1,k)each(x,b[i])each(y,b[j])ans[x][y]=d;
        each(i,b)solve(i,d+1,solve);
    };
    solve(iota(n),1,solve);
    ll x=0;
    each(i,ans)chmax(x,max(i));
    out(x);
    rep(n)rep(j,i+1,n)out(ans[i][j]);
}