...
int main(void)
{
	...
	rep(i, 1, n){
		if(i % 2){
			while(sz(lvec[i])){
			vector<ll> tmp;
				ll x = lvec[i].back(); lvec[i].pop_back();
				rep(j, 0, i-1) tmp.push_back(vec[x][(i+1)/2*j%i]);
				vec[x] = tmp;
			}
		}
		else{
			...
			while(sz(lvec[i])){
				ll x = lvec[i].back(); lvec[i].pop_back();
				ll y = lvec[i].back(); lvec[i].pop_back();
				vector<ll> tmp;
				rep(j, 0, i-1){
					tmp.push_back(vec[x][j]);
					tmp.push_back(vec[y][j]);
				}
				vec[x] = tmp;
				vec[y].clear();
			}
	...
	rep(i, 1, n){
		ll l = sz(vec[i]);
		reps(j, vec[i]) ans[vec[i][j]] = vec[i][(j+1)%l];
	}
	outa(ans, 1, n);
