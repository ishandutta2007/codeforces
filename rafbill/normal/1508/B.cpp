...
vi solve(li n, li k){
  k--;
  vi A(n);iota(begin(A), end(A),1);
  for(li i = 0; i < (li)(n-1); ++i) {
    if(k == 0) break;
    if(n-i > 62) continue;
    for(li j = (i+1); j <= (li)(n); ++j) {
      li d = 1ll<<(max(0ll,n-j-1));
      if(k < d) {
...
vi brute(li n, li k){
  vi I(n); iota(begin(I), end(I),1);
  vvi O;
  do {
    bool ok = 1;
    for(li i = 0; i < (li)(n-1); ++i) if(I[i+1] < I[i]-1) ok = 0;
    if(ok) O.emplace_back(I);
  } while(next_permutation(begin(I), end(I)));
  sort(begin(O), end(O));
  return (k-1 < (int)O.size()) ? O[k-1] : vi{{-1}};
}
