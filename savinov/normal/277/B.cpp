...
int main()
{
    ...
    while(1)
    {
        ...
        forn(i, m)
        {
            double phi = (double)i * 2.0 * PI / m + PI / 3.0;
            int x1 = (r) * cos(phi) + rand() % 10;
            int y1 = (r) * sin(phi) + rand() % 10;
            ans.pb(mp(x1, y1));
        }

        forn(i, n)
        {
            int x = ((ans[i].x + ans[(i + 1) % m].x) / 2);
            int y = (ans[i].y + ans[(i + 1) % m].y) / 2;
            x = (x + 100 * (ans[(i - 1 + m) % m].x)) / 101;
            y = (y + 100 * (ans[(i - 1 + m) % m].y)) / 101;
            ans.pb(mp(x, y));
        }
        bool ok = true;
        forn(i, n)
            forn(j, n)
                forn(k, n)
                {
                    if (i != j && j != k && i != k)
                    {
                        pair <ll, ll> a = mp(ans[k].x - ans[i].x, ans[k].y - ans[i].y);
                        pair <ll, ll> b = mp(ans[j].x - ans[i].x, ans[j].y - ans[i].y);
                        if (a.x * b.y == a.y * b.x)
                            ok = false;
                    }
                }
