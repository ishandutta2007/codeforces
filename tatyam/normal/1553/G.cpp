...
int main(){
    using u32 = uint32_t;
    const u32 MAX = 1e6 + 1;
    vector<u32> primes, factor(MAX + 1);
    for(u32 i = 2; i <= MAX; i++){
        if(!factor[i]){
            primes.push_back(i);
            factor[i] = i;
    ...
    rep(q){
        auto&[x,y]=query[i];
        if(uf.find(MAX+--x,MAX+--y))ans[i]=0;
    }
    set<pii>edge;
    rep(n)[&](u32 a){
        u32 x=a;
        ...
        Uniq(idx);
        rep(idx.size())rep(j,i+1,idx.size())edge.insert({idx[i],idx[j]});
    }(a[i]);
    rep(q)if(ans[i]==2){
        auto[x,y]=query[i];
        x=uf[MAX+x];
