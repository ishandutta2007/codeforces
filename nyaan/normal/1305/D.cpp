...
void solve() {
  ...
  while (1) {
    ...
    auto efs = [&](auto rec, int cur, int par = -1) -> int {
      if (cur == w) {
        unused[par][w] = unused[w][par] = 1;
        return true;
      }
      each(dst, g[cur]) {
        if (unused[cur][dst]) continue;
        if (dst == par) continue;
        if (rec(rec, dst, cur)) return true;
      }
      return false;
    };
