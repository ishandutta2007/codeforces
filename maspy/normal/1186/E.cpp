...
void solve() {
  ...
  auto dfs = [&](auto self, ll k, ll X, ll Y) -> ll {
    if (k == -1) { return Ac.sum(0, 0, X, Y); }

    ll H0 = H << k, W0 = W << k;
    if (X <= H0 && Y <= W0) return self(self, k - 1, X, Y);
    if (X <= H0 && Y > W0) return -self(self, k - 1, X, Y - W0);
    if (X > H0 && Y <= W0) return -self(self, k - 1, X - H0, Y);
    return self(self, k - 1, X - H0, Y - W0);
  };

  auto f = [&](ll X, ll Y) -> ll { return dfs(dfs, 30, X, Y); };

  FOR_(Q) {
    LL(a, b, c, d);
    --a, --b;
    ll diff = f(a, b) + f(c, d) - f(a, d) - f(c, b);
    ll total = (c - a) * (d - b);
    print((total + diff) / 2);
  }
}
