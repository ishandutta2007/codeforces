...
constexpr int m2=(mod+1)/2;

ll N;
mat g;
vec par,dep;
void dfs(ll now,ll p,ll d){
  par[now]=p;
  dep[now]=d;
  for(auto x:g[now]){
    if(x==p)continue;
...
vec makepath(ll a,ll b){
  vec v1,v2;
  while(a>=0&&b>=0&&dep[a]>dep[b]){
    v1.emplace_back(a);
    a=par[a];
  }
  while(a>=0&&b>=0&&dep[a]<dep[b]){
  ...
  v1.emplace_back(a);
  reverse(all(v2));
  for(auto x:v2)v1.emplace_back(x);
  return v1;
}
int main(){
  startupcpp();
  ...
  mat prob(N);
  {
    ...
    For(i,2,N){
      prob[i].resize(i+1);
      prob[i][0]=0;
      rep(j,prob[i-1].size()-1)prob[i][j+1]=(prob[i-1][j]+prob[i-1][j+1])*m2%mod;
      prob[i].back()=1;
    }
  }
  rep(i,N-1){
  ...
  rep(B,N){
    ...
    rep(A,B){
      //A to B path
      vec path=makepath(A,B);
      vec chid(N,-1);
      ll K=path.size();
      rep(i,K)chid[path[i]]=i;
      rep(i,N){
        j=i;
        while(chid[j]<0)j=par[j];
        ans+=prob[K-1][chid[j]];
      }
    }
  }
  print(moddevide(ans%mod,N));
}