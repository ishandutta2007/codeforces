...
void solve() {
    ...
    hogea.exec();
    REP(i, n) {
        inputs.push_back(hogea.fetch());
    }
    int hoge = 0;
    for (int q = 1; q < inputs.size(); ++q) {
        if (inputs[q - 1] != inputs[q]) hoge = 1;
    }
    if (hoge == 0) {
        cout << 1 << endl;
        REP(i, inputs.size()) {
    ...
    if (hoge == 1) {
        ...
        for (int q = 1; q < inputs.size();++q) {
            ...
            if (starter == 0 && inputs[q] == inputs[q - 1]) {
                cout << (now+1)%2+1;
                starter = 1;
            }
            else {
    ...
    cout << 1;
    int now = 1;
    for (int q = 1; q < (int)inputs.size() - 1; ++q) {
        cout << " " << q % 2 + 1;
    }
    cout << " 3" << endl;
}
