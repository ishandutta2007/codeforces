...
ll n, m, k, x, y, ans, a, b;
string s;
int aa[1020];
int dp[1020][1020][2];
//first i digths with j ones and a = 1 means failed number and a = 0 means not failed number
struct marray{
    ll x[500000];
    ll n;
    void read(int i)
    ...
    for (int i = 2; i <= n; i++)
    {
        ...
        for (int j = 0; j <= i; j++)
        {
            ...
            else
            {
                ...
                dp[i][j][1] = (dp[i - 1][j][0] + dp[i - 1][j - 1][1] + dp[i - 1][j][1]) % MOD;
            }
        }
    }
    for (int i = 1; i <= n; i++)
