...
ll cnt[100005], lev[100005], rlev[100005];
segtree<max_t> st;
...
int main() {
  ...
  st.init(m + 7, c);
  f0r(i, n) {
    ll v = st.query(0, cnt[i]).val;
    lev[i] = v + 1;
    v = st.query(cnt[i], cnt[i]).val;
    if (lev[i] > v) st.update(cnt[i], cnt[i], lev[i] - v);
  }
  ...
  f0rd(i, n - 1) {
    ...
    rlev[i] = v + 1;
    v = st.query(cnt[i], cnt[i]).val;
    if (rlev[i] > v) st.update(cnt[i], cnt[i], rlev[i] - v);
  }

  f0r(i, n) ans = max(ans, lev[i] + rlev[i] - 1);
  cout << ans << endl;
}