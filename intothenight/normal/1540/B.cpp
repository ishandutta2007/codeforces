...
int main(){
	...
	vector<vector<Zp>> prob(n, vector<Zp>(n));
	for(auto sum = 1; sum < n; ++ sum){
		for(auto l = 0; l <= sum; ++ l){
			int r = sum - l;
			if(!l){
				prob[l][r] = 1;
			}
			else if(l < sum){
				prob[l][r] = (prob[l - 1][r] + prob[l][r - 1]) / 2;
			}
		}
	}
	Zp res = 0, inv_n = Zp(1) / n;
	for(auto root = 0; root < n; ++ root){
		lca_solver ls(g, {root});
		for(auto u = 0; u < n; ++ u){
			for(auto v = 0; v < u; ++ v){
				int w = ls.lca(u, v);
				res += prob[ls.depth[u] - ls.depth[w]][ls.depth[v] - ls.depth[w]] * inv_n;
			}
		}
	}
	cout << res << "\n";
