// Permutation Addicts (https://codeforces.com/contest/1738/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        vector after(n + 2, vector<int>());
        rep(i, n)
            after[b[i]].push_back(i + 1);

        int i = sz(after[0]) ? 0 : n + 1;
        vector<int> a;
        while (sz(after[i])) {
            auto it = find_if(all(after[i]), [&](int j) { return sz(after[j]); });
            if (it != end(after[i]))
                iter_swap(it, prev(end(after[i])));
            a.insert(end(a), all(after[i]));
            i = a.back();
        }
