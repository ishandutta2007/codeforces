...
fun main(){
    solve.cases{
        ...
        val friend = a.indexOf(-1)
        for(i in 0 until friend){
            a[i] = 0
        }
        val finals = a.filter { it != -1 }

        val k = 31 - n.countLeadingZeroBits()
        val powers = mutableSetOf<Int>()
        for(i in 0 until k){
            powers.add((1 shl ( 1+ i)) -  2 )
        }
        val stuff = TreeMap<Int,Int>()
        var ret = 0L
        for( c in n-2 downTo 0){
            val at = finals[c]
            stuff[at] = stuff.getOrDefault(at,0) + 1
            if(powers.contains(c)){
                val best = stuff.ceilingEntry(-1)
                ret += best.key
                if(best.value == 1){
                    stuff.remove(best.key)
                }else{
                    stuff[best.key] = best.value - 1
                }
            }
        }
        put(ret)
...
/*
wrong index: index is for a differetn list
 */