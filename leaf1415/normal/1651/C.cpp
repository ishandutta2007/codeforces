...
ll get(ll x, ll a[])
{
	...
	if(p <= n) chmin(ret, abs(x-a[p]));
	if(p-1 >= 1) chmin(ret, abs(x-a[p-1]));
	return ret;
}
...
int main(void)
{
	...
	while(T--){
		...
		chmin(ans, abs(a[1]-b[n]) + abs(a[n]-b[1]));
		chmin(ans, abs(a[1]-b[1]) + get(a[n], B) + get(b[n], A));
		chmin(ans, abs(a[n]-b[n]) + get(a[1], B) +get(b[1], A));
		chmin(ans, abs(a[1]-b[n]) + get(a[n], B) +get(b[1], A));
		chmin(ans, abs(a[n]-b[1]) + get(a[1], B) +get(b[n], A));
		chmin(ans, get(a[1], B) + get(a[n], B) + get(b[1], A) + get(b[n], A));
