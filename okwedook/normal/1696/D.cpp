...
vector<int> rgt_big(vector<int> a) {
    vector<int> ans(sz(a), sz(a));
    vector<int> st;
    for (int i = 0; i < sz(a); ++i) {
        while (sz(st) && a[i] > a[st.back()]) {
            ans[st.back()] = i;
            st.popb();
        }
        st.pb(i);
...
void solve() {
    ...
    read(a);
    for (auto &i : a) --i;
    auto rgtup = rgt_big(a);
    for (auto &i : a) i = -i;
    auto rgtdown = rgt_big(a);
    for (auto &i : a) i = -i;
    reverse(a);
    auto lftup = rgt_big(a);
    for (auto &i : lftup) {
        i = n - i - 1;
    }
    reverse(lftup);
    for (auto &i : a) i = -i;
    auto lftdown = rgt_big(a);
    for (auto &i : lftdown) {
        i = n - i - 1;
    }
    reverse(lftdown);
    ...
    Tree<node> treeup(n), treedown(n);
    for (int i = 0; i < n; ++i) {
        treeup[i].val = treedown[i].val = mod;
    }
    vector<int> ans(n, mod);
    treeup[0].val = treedown[0].val = ans[0] = 0;
    treeup.build();
    treedown.build();
    vector<vector<int>> delup(n + 1), deldown(n + 1);
    for (int i = 0; i < n; ++i) {
        for (auto j : delup[i]) {
            treeup.updup(j, mod);
        }
        for (auto j : deldown[i]) {
            treedown.updup(j, mod);
        }
        if (i > 0) {
            int valup = treeup.getup(lftup[i], i + 1).val;
            chmin(ans[i], valup + 1);
            int valdown = treedown.getup(lftdown[i], i + 1).val;
            chmin(ans[i], valdown + 1);
            treeup.updup(i, ans[i]);
            treedown.updup(i, ans[i]);
        }
        delup[rgtdown[i]].pb(i);
        deldown[rgtup[i]].pb(i);
    }
    println(ans.back());
}
