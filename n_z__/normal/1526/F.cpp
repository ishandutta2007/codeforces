...
#undef cout
#undef endl
//constexpr int sample[]={0,9,10,19,7,16,18,11,14,15,6,20,8,17,4,5,3,12,2,13,1};
//auto query=[&](int a,int b,int c){cout<<"? "<<a<<' '<<b<<' '<<c<<endl;int t=abs(sample[a]-sample[b])^abs(sample[a]-sample[c])^abs(sample[b]-sample[c])^max({abs(sample[a]-sample[b]),abs(sample[a]-sample[c]),abs(sample[b]-sample[c])})^min({abs(sample[a]-sample[b]),abs(sample[a]-sample[c]),abs(sample[b]-sample[c])});cout<<t<<endl;return t;};
auto query=[&](int a,int b,int c){cout<<"? "<<a<<' '<<b<<' '<<c<<endl;int t;cin>>t;return t;};
int n;
cin>>n;
vector<int>p(n+1);
int a,b,c;
...
ff(y,x+1,13)
ff(z,y+1,13)
if(query(x,y,z)<=(n-4)/6){a=x,b=y,c=z;break;} 
map<int,vector<int>>m;
ff(x,1,n)
if(x!=a&&x!=b)m[query(a,b,x)].push_back(x);
int mx=m.rbegin()->first;
if(m[mx-1].size()>=2)if(query(m[mx][0],m[mx-1][0],a)>query(m[mx][0],m[mx-1][1],a))swap(m[mx-1][0],m[mx-1][1]);
a=m[mx][0],b=m[mx-1][0];
p[a]=1;p[b]=2;
for(int x=1;x<=n;x++)
if(x!=a&&x!=b)p[x]=query(a,b,x)+2;
if(p[1]>p[2])for(int x=1;x<=n;x++)p[x]=n-p[x]+1;
cout<<"! ";for(int x=1;x<=n;x++)cout<<p[x]<<' ';cout<<endl;
int res;
cin>>res; 
#endif
}
