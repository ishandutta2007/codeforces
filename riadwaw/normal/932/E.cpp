...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		vector<vector<Z>> polys(a + 1);
		polys[0] = {Z::rawValueOf(1)};
		for (int i = 1; i <= a; ++i) {
			polys[i].resize(i + 1);
			for (int j = 0; j < i; ++j) {
				polys[i][j + 1] += polys[i - 1][j];
			}
			for (int j = 0; j < i; ++j) {
				polys[i][j] -= polys[i - 1][j] * (i - 1);
			}
		}

		vector<Z> cur_poly(a + 1);
		...
		for (int i = 0; i <= a; ++i) {
			...
			ans += coef[i] * prod * binpow(Z::rawValueOf(2), n - i);
		}
