...
int main() {
    ...
    vvvll dpo(N + 1, vvll(N + 1, vll(N + 1, -INF)));
    vvvll dpz(N + 1, vvll(N + 1, vll(N + 1, -INF)));
    ...
    repn(x,N){
        ...
        rep(i,N+1) {
            ...
            repn(k, x) {
                if (S[j - 1] - '0' == 1)dpo[i][j][k] = dpo[i][j - 1][k - 1];
                else { dpz[i][j][k] = dpz[i][j - 1][k - 1]; }

                for (ll l = i + 1; l <= j; l++) {
                    dpo[i][j][k] = max(dpo[i][j][k], dpo[i][l][k] + dp[l][j]);
                    dpz[i][j][k] = max(dpz[i][j][k], dpz[i][l][k] + dp[l][j]);
                }

                //cout << i << " " << j << " " << k << " " << dpo[i][j][k] << " " << dpz[i][j][k] << endl;
            }

            repn(k, x) {
                dp[i][j] = max(dp[i][j], dpo[i][j][k] + a[k]);
                dp[i][j] = max(dp[i][j], dpz[i][j][k] + a[k]);
            }
