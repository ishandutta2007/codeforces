...
int main() {
    ...
    loop(t) {
        ...
        vector segs(4, atcoder::segtree<int, op, e>(n + 1));
        segs[0].set(0, 0);

        rep(i, 1, n + 1) {
            rep(d, 3) {
                segs[d].set(i - 1, segs[d].get(i - 1) + 1);
                for (int p = 1; i - p - 1 >= 0; p <<= 1) {
                    segs[d].set(i - p - 1, segs[d].get(i - p - 1) + p);
                }
            }
            if (i < n and a[i - 1] == a[i]) continue;
            rep(d, 3) {
                segs[d + 1].set(i, segs[d].prod(0, i));
            }
        }
        int ans = e();
        rep(d, 4) {
            chmin(ans, segs[d].get(n) - n + (3 - d));
        }
        print(ans);
    }
    return 0;
