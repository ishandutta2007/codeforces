...
void run_case() {
    ...
    for (edge &e : edges) {
        cin >> e.u >> e.v >> e.weight >> e.cost;
        e.u--; e.v--;
        lca.add_edge(e.u, e.v);
    }

    lca.build();
    vector<int64_t> options[3];
    int64_t current_cost = 0;
    vector<int64_t> leaves(N, 0);

    for (int x : lca.rev_tour_list) {
        leaves[x] = lca.adj[x].empty();

        for (int child : lca.adj[x])
            leaves[x] += leaves[child];
    }

    for (edge &e : edges) {
        int deeper = lca.depth[e.u] > lca.depth[e.v] ? e.u : e.v;
        current_cost += e.weight * leaves[deeper];

        for (int64_t w = e.weight; w > 0; w /= 2)
            options[e.cost].push_back((w - w / 2) * leaves[deeper]);
    }

    for (int c = 1; c <= 2; c++)
        sort(options[c].rbegin(), options[c].rend());
    ...
    int64_t need = current_cost - S;
    int64_t sum1 = 0, sum2 = accumulate(options[2].begin(), options[2].end(), 0LL);
    int n1 = int(options[1].size());
    int n2 = int(options[2].size());
    int best = n1 + 2 * n2;

    for (int i = 0, j = n2; i <= n1; i++) {
        while (j > 0 && sum1 + sum2 - options[2][j - 1] >= need) {
            j--;
            sum2 -= options[2][j];
        }
