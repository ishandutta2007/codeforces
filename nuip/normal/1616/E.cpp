...
int main(){
	...
	TT{
		...
		cauto s=read<string>();
		cauto t=read<string>();
		out(s-sorted, t,(s-sorted>=t), 1);
		if(s-sorted>=t){
			cout<<-1 NL;
			continue;
		}
		vv<int> inds(26);
		rrep(i,n) inds[s[i]-'a'].eb(i);
		rep(i,n) add(i,1);
		ll sum=0;
		ll re=1e18;
		rep(i,n){
			int mn=n;
			rep(c,t[i]-'a')if(inds[c].size()) MN(mn,inds[c].back());
			out(mn,1);
			if(mn<n) MN(re,sum+Sum(mn));
			auto &rm=inds[t[i]-'a'];
			if(rm.empty()) break;
			sum+=Sum(rm.back());
			add(rm.back(),-1);
		...
		rep(c,26)for(cauto &i:inds[c]) add(i,-1);
		cout<<re NL;
	}
	return 0;
}