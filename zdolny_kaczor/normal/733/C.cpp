...
int main() {
	...
	REP(i, k) {
		...
		if (sum != b[i]) err();
		debug << arr(b, i) << range(a + first, a + last);
		if (last - first == 1) {
			debug << "tutaj";
			first = last;
			continue;
		...
		int m = 0;
		FOR(j, first, last - 1) maxi(m, a[j]);
		bool found = false;
		FOR(j, first, last - 2) if (a[j] == m && a[j + 1] != m) {
			int p = j + i - first;
			debug << "case 1" imie(j) imie(p);
			REP(_, last - j - 1)
				ans.emplace_back(p, true);
			REP(_, j - first)
				ans.emplace_back(p--, false);
			found = true;
			break;
		}
		if (!found) FOR(j, first + 1, last - 1) if (a[j] == m && a[j - 1] != m) {
			int p = j + i - first;
			debug << "case 2" imie(j) imie(p);
			REP(_, j - first)
				ans.emplace_back(p--, false);
			REP(_, last - j - 1)
				ans.emplace_back(p, true);
