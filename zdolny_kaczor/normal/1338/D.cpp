...
vi graf[nax];
int ans = 0;
int with[nax], without[nax];
void inc(vi &here, int v) {
	for (int &x : here) if (x < v) swap(x, v);
}
void dfs(int x, int y = -1) {
	int deg_down = graf[x].size() - (y != -1);
	int deg = graf[x].size();
	for (int v : graf[x]) if (v != y) dfs(v, x);
	for (int v : graf[x]) if (v != y) maxi(without[x], max(with[v], without[v]));
	for (int v : graf[x]) if (v != y) maxi(with[x], without[v]);
	with[x]++;
	without[x] += max(0, deg_down - 1);
	debug(arr(with, x));
	maxi(ans, with[x]);
	debug(arr(without, x) + (y != -1));
	maxi(ans, without[x] + (y != -1));
	vi withs = {-inf, -inf};
	vi withouts = {-inf, -inf};
	for (int v : graf[x]) if (v != y) inc(withs, max(with[v], without[v]));
	for (int v : graf[x]) if (v != y) inc(withouts, without[v]);
	debug(imie(x), imie(withs), imie(withouts));
	maxi(ans, withs[0] + withs[1] + deg - 2);
	maxi(ans, withouts[0] + withouts[1] + 1);
}
int main() {
	int n;
	scanf("%d", &n);
	...
	dfs(1);
	debug(draw_tree(1, graf, [](int v){stringstream o; o << "[" << with[v] << "; " << without[v] << "]_" << v; ros.str();}));
	printf("%d\n", ans);
	exit(0);
}
//#STAY AT HOME