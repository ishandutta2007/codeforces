...
//                 res.allsame = true;
//                 res.l = res.r = x.l + y.r;
//                 res.ltype = res.rtype = x.ltype;
//                 return res;
//             } else {
//                 res.l = x.l, res.ltype = x.ltype;
//                 res.r = y.r, res.rtype = y.rtype;
//                 return res;
//             }
//         } else {
//             if(x.rtype == y.ltype) {
...
int main() {
    TEST {
        ...
        STR(s);
        vi v;
        for(auto e : s) v.eb(e - '0');

        vvvv(int, dp, n + 1, a, b, (n + 1) * 2);
        using T = tuple<int, int, int>;
        vvvv(T, pre, n + 1, a, b, (n + 1) * 2);
        dp[0][0][0][n] = 1;
        rep(i, n) rep(A, a) rep(B, b) rep(j, (n + 1) * 2) {
            if(dp[i][A][B][j] == 0) continue;
            dp[i + 1][(A * 10 + v[i]) % a][B][j + 1] = 1;
            pre[i + 1][(A * 10 + v[i]) % a][B][j + 1] = T(A, B, j);

            dp[i + 1][A][(B * 10 + v[i]) % b][j - 1] = 1;
            pre[i + 1][A][(B * 10 + v[i]) % b][j - 1] = T(A, B, j);
        }

        int mi = inf<int>;
        int K;
        rep(k, (n + 1) * 2) {
            if(dp[n][0][0][k] and chmin(mi, abs(n - k))) K = k;
        }
        if(mi == inf<int> or mi == n) {
            OUT(-1);
            continue;
        }
        int A = 0, B = 0, j = K;
