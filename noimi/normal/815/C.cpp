...
signed main(){
    ...
    auto mrg = [&](vll &x, vll y)->vll{
        vll res(x.size() + y.size() -1,INF);
        rep(i,x.size())rep(j,y.size()) chmin(res[i+j],x[i] + y[j]);
        return res;
    };
    auto mrg2 = [&](vll &x, vll y, vll z)->vll{
        vll res(x.size() + y.size() -1,INF);
        rep(i,x.size())rep(j,y.size()) chmin(res[i+j],x[i] + min(y[j],z[j]));
        return res;
    };
    auto dfs = [&](auto &&f,int x)->pair<vll,vll>{
        vll A{0},B{0};
        for(auto e:g[x]){
            auto [a,b] = f(f,e);
            // cout << e<<": ";
        ...
        A.insert(A.begin()+1,c[x]-d[x]);
        rep2(i,2,A.size()-1) A[i] += c[x]-d[x];
        vll C{0,c[x]};
        B = mrg(B,C);
        return {A,B};
    };
