...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
        char ch = getDg();
        if(ch == '1') b.push_back(mp(a[i], i));
        else c.push_back(mp(a[i], i));
    }
    sort(b.begin(), b.end(), greater<_p>());
    sort(c.begin(), c.end(), greater<_p>());
    _ p = n;
    for(_p i : b) o[i.second] = p--;
    for(_p i : c) o[i.second] = p--;
    wr(o, n);
