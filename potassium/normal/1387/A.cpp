...
int main() {
	int n,m; R(n,m);
	vector<vector<pair<pii, int>>> g(n + 1);
	for (int i = 0; i < m; i++) {
		int u,v,w; R(u,v,w); w *= 2;
		g[u].push_back({ { v, w }, i });
		g[v].push_back({ { u, w }, i });
	}

	vector<bool> vis(n + 1, false);
	vector<pii> line(n + 1, { 0, 0 });
	...
	for (int u = 1; u <= n; u++) {
		...
		vector<int> cc;		
		function<void(int, int)> dfs_solve = [&](int u, int pid) {
			vis[u] = true;
			cc.emplace_back(u);
			for (auto& e : g[u]) {
				...
				if (!vis[v]) {
					line[v] = { -line[u].first, w - line[u].second };
					dfs_solve(v, id);
				} else {
					int m = line[u].first + line[v].first;
					int c = line[u].second + line[v].second;

					if (m == 0) {
						if (c != w) isConsistent = false;
					}
					else {
						int sol = (w - c) / m;
						if (!fixed) {
		...
		if (!fixed) {
			...
			for (auto& u : cc)
				if (line[u].first != 0)
					slope.emplace_back( -line[u].second / line[u].first);

			sort(all(slope));
			fixed = true;
			val = slope[(int)slope.size() / 2];
		}

		for (auto& u : cc)
			ans[u] = line[u].first * val + line[u].second;
	}
