...
int N, M, K, Q;
int8_t A[NM_MAX][NM_MAX];
vector<pair<int16_t, int16_t>> color_list[K_MAX];
...
int query(int r1, int c1, int r2, int c2) {
    ...
    for (int r = max(r1 - goal, 0); r <= min(r1 + goal, N - 1); r++) {
        ...
        for (int c = max(c1 - (goal - r_diff), 0); c <= min(c1 + (goal - r_diff), M - 1); c++)
        // for (int c = max(c1 - goal, 0); c <= min(c1 + goal, M - 1); c++)
            closest[A[r][c]] = min(closest[A[r][c]], r_diff + abs(c - c1));
    }
