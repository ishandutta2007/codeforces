...
int main(){
    ...
    rep(k/2){
        vv(ll,dp2,n,m,LINF);
        rep(n)rep(j,m-1){
            chmin(dp2[i][j+1],dp[i][j]+yoko[i][j]);
            chmin(dp2[i][j],dp[i][j+1]+yoko[i][j]);
        }
        rep(n-1)rep(j,m){
            chmin(dp2[i+1][j],dp[i][j]+tate[i][j]);
            chmin(dp2[i][j],dp[i+1][j]+tate[i][j]);
        }
        swap(dp,dp2);
    }
    each(i,dp)each(j,i)j*=2;
    each(i,dp)out(i);
}