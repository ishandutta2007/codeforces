...
ll n, m;
ll u[100005], v[100005], c[100005];
vector<ll> avec, avec2;
UnionFind uf(200005);
map<ll, vector<ll>> mp;

bool calc(vector<ll> &avec)
{
	uf.init();
	rep(i, 1, m){
		if(c[i] == 0){
			uf.unite(u[i], v[i]);
			uf.unite(u[i]+n, v[i]+n);
		}
		else{
			uf.unite(u[i], v[i]+n);
			uf.unite(u[i]+n, v[i]);
	...
	//rep(i, 1, 2*n) cout << uf.root(i) << " "; cout << endl;
	rep(i, 1, n) if(uf.same(i, n+i)) return false;
	
	mp.clear();
	rep(i, 1, n) mp[uf.root(i)].push_back(i);
	
	ll ret = 0;
	rep(i, 1, n){
		if(uf.root(i) != i) continue;
		vector<ll> &vec = mp[i], &vec2 = mp[uf.root(n+i)];
		if(sz(vec) < sz(vec2)) avec.insert(avec.end(), all(vec));
		else avec.insert(avec.end(), all(vec2));
	}
	return true;
}
...
int main(void)
{
	...
	if(!calc(avec)) rep(i, 1, n+1) avec.push_back(-1);
	
	rep(i, 1, m) c[i] ^= 1;
	if(!calc(avec2)) rep(i, 1, n+1) avec2.push_back(-1);
	
	if(sz(avec) > sz(avec2)) swap(avec, avec2);
	if(sz(avec) > n){
		outl(-1);
		return 0;
	}
