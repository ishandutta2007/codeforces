...
const int mod = 998244353, maxn = 200200;
mt19937 rd(time(0));
...
int dp[maxn];
int bit[maxn];
inline void add(int x,int v) { ++ x; for(;x < maxn;x += x &-x)bit[x]=(bit[x]+v)%mod; }
inline int qry(int x) { int ret = 0; ++x; for(;x;x&=x-1)ret=(ret+bit[x])%mod; return ret; }
int main() {
