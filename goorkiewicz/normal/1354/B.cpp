...
void solve ()
	...
	for (i=0; i<n; i++)
		{
		if (vec[1].empty() || vec[1].back() < i) continue;
		if (vec[2].empty() || vec[2].back() < i) continue;
		if (vec[3].empty() || vec[3].back() < i) continue;
		int a = vec[1][lowb(vec[1], i)];
		int b = vec[2][lowb(vec[2], i)];
		int c = vec[3][lowb(vec[3], i)];
		int l = min(a, min(b, c));
		int r = max(a, max(b, c));
		umin(out, r - l + 1);
		}
