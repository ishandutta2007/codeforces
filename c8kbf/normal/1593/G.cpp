...
node query(_ L, _ R, _ l, _ r, _ pos);
...
node merge(node l, node r) {
    ...
    if(l.md == 1) {
        if(l.l & 1) {
            if(r.md == 1) return (node){1+(r.l>l.l), abs(r.l-l.l)};
            else return (node){1, r.l+l.l};
        } else {
            if(r.md == 1) return (node){1, r.l+l.l};
            else return (node){1+(r.l>l.l), abs(r.l-l.l)};
        }
    } else {
        if(l.l & 1) {
            if(r.md == 1) return (node){2, r.l+l.l};
            else return (node){2-(r.l>l.l), abs(r.l-l.l)};
        } else {
            if(r.md == 1) return (node){2-(r.l>l.l), abs(r.l-l.l)};
            else return (node){2, r.l+l.l};
        }
    }
}
...
node query(_ L, _ R, _ l, _ r, _ pos) {
    ...
    return query(L, R, md+1, r, pos<<1|1);
}