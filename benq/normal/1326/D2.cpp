...
str solve(str s) {
	vi v = manacher(s);
	vi ret(sz(s)); 
	for (int i = 0; i <= sz(v)/2; ++i) if (v[i]) {
		ckmax(ret[(i-v[i]+1)/2],v[i]);
	}
	F0R(i,sz(ret)-1) ckmax(ret[i+1],ret[i]-2);
	// ps("HUH",ret);
	pi bes = {-1,-1};
	// int ans = 0;
	for (int i = 0; 2*i <= sz(s); ++i) {
		if (i) {
			if (s[i-1] != s[sz(s)-i]) break;
		}
		ckmax(bes,mp(2*i+ret[i],i));
	}
	str tmp;
	int len = bes.f-bes.s;
	tmp += s.substr(0,len);
	tmp += s.substr(sz(s)-bes.s,bes.s);
	// ps("HUH",s,tmp);
	return tmp;
