...
void solve() {
    ...
    for (int i = 2; i <= n; ++i) {
        for (int q = 2; i * q <= n; ++q) {
            req_check[i * q].push_back(i);
            req_check[i * q].push_back(q);
            Die[i * q] = max({ Die[i * q],i,q });
        }
        int ok = 1;
        if (req_check[i].size()) ok = 0;
        if (ok == 1) {
            AP[i] = 1;
            for (int q = 2; i * q <= n; ++q) {
                Die[i * q] = max(Die[i * q], i);
            }
            cnt++;
        }
    }
    ...
    for (int k = 2; k <= n; ++k) {
        while (cnt < k) {
            ...
            for (int j = 1; j * ans <= n; ++j) {
                if (AP[j * ans] == 1) continue;
                if (Die[j * ans] > ans) continue;
                int ok = 1;
                for (int t = 0; t < req_check[j*ans].size(); ++t) {
                    if (req_check[j * ans][t] <= ans) continue;
                    if (AP[req_check[j * ans][t]] == 1) {
                        ok = 0;
                        break;
                    }
                }
                if (ok == 1) {
                    AP[j * ans] = 1;
                    for (int q = 2; j * ans * q <= n; ++q) {
                        Die[j * ans * q] = max(Die[j * ans * q], j * ans);
                    }
                    cnt++;
                }
            }
