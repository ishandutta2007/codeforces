...
void dfs(int i, int parent, int d) {
    ...
    st[i]=tim++;
    dep[i] = d;
    for (int nb : adj[i]) if (nb!=parent) dfs(nb, i, d+1);
    ed[i]=tim-1;
}
void init(int root) {
    ldep=tim=0;
    ldep = 32 - __builtin_clz(n);
    st.rsz(n), ed.rsz(n); up.rsz(n, vi(ldep)); 
    dep.rsz(n); mx.rsz(n, vi(ldep));
    dfs(root, root, 0);
    F0R(i, m) {
        if (dep[edges[i].F] < dep[edges[i].S]) swap(edges[i].F, edges[i].S);
        if (up[edges[i].F][0] == edges[i].S) mx[edges[i].F][0] = i+1;
    }
    FOR(i, 1, up[0].size()) F0R(j, n) up[j][i] = up[up[j][i-1]][i-1], mx[j][i] = max(mx[j][i-1], mx[up[j][i-1]][i-1]);
}
int goup(int i, int k) { int ans = 0; F0R(d, ldep) if (k&1<<d) ckmax(ans, mx[i][d]), i=up[i][d]; return ans; }
...
int mxpath(int a, int b) {
    int u = lca(a, b);
    return max(goup(a, dep[a]-dep[u]), goup(b, dep[b]-dep[u]));
}

void solve() {
    cin >> n >> m >> q;
    adj.clear(); adj.rsz(n);
    up.clear(); mx.clear(); rmq.clear(); edges.clear();
    DSU d(n);
    F0R(i, m) {
        int u, v; cin >> u >> v; u--; v--;
        edges.pb(mp(u, v));
    ...
    F0R(i, n) rmq[0][i] = 0;
    debug(ldep);
    FOR(i, 1, ldep) F0R(j, n - (1<<i) + 1) {
        rmq[i][j] = max(rmq[i-1][j], rmq[i-1][j + (1<<(i-1))]);
        ckmax(rmq[i][j], mxpath(j, j+(1<<(i-1))));
        debug(i, j, j+(1<<i), rmq[i][j]);
    }
    while (q--) {
        int l, r; cin >> l >> r; l--;
        int len = r-l, bit = 31 - __builtin_clz(len);
        debug(l, r, len, bit);
        if (len == 1<<bit) {
            cout << rmq[bit][l] << ' ';
        } else {
            cout << max(rmq[bit][l], rmq[bit][r - (1<<bit)]) << ' ';
        }
    }
    cout << nl;
}
