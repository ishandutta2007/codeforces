...
signed main() {
    ...
    rep(iiii,T) {
        ...
        bool suc=false;
        string ret;
        if(od.empty()||ev.empty()) ret=s,suc=true;
        else {
            vector<int> oddd={(int)od.size()-1,0};
            vector<int> evvv={(int)ev.size()-1,0};
            for(int i:oddd)for(int j:evvv) {
                if(suc) break;
                if(abs(od[i]-ev[j])>1) {
                    for(int k=0;k<od.size();++k) {
                        if(k==i) continue;
                        ret.push_back(od[k]+'a');
                    }
                    ret.push_back(od[i]+'a');
                    ret.push_back(ev[j]+'a');
                    for(int k=0;k<ev.size();++k) {
                        if(k==j) continue;
                        ret.push_back(ev[k]+'a');
                    }
                    suc=true;
                }
            }
