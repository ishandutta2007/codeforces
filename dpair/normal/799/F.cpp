...
std::map < LL, std::pair < int, LL > > vis;

int n, m;
const int MAXN = 2e5 + 5;
LL b[MAXN], ans = 0; int c[MAXN];
signed main(){
    read(n, m); while(n --) { int l, r; read(l, r); const LL val = rnd(); b[l + 1] ^= val; b[r + 1] ^= val; ++ c[l]; -- c[r + 1]; }
    rep(i, 1, m) b[i] ^= b[i - 1];
    int s = 0; rep(i, 1, m) { b[i] ^= b[i - 1]; !(c[i] += c[i - 1])? ++ s : s = 0; ans -= 1ull * s * (s + 1) >> 1; }
    rep(i, 1, m) {
        std::pair < int, LL > x = vis[b[i]];
        ++ x.first; x.second += i - 1;
        ans += 1ull * i * x.first - x.second;
        vis[b[i]] = x;
    } print(ans);
}
/*
