...
int main()
{
    ...
    for (int i = 1; i < n; i++)
    {
        while (waves[i].sz < cnt[i] - 1)
        {
            while (is[first_free] == 1)
            {
                first_free++;
            }
            if (first_free >= i)
    ...
    for (int i = 1; i < n; i++)
    {
        for (int j = 0; j < waves[i].sz; j++)
        {
            if (j == 0)
            {
                cout << waves[i][j] << " " << n << endl;
            }
            else
            {
                cout << waves[i][j] << " " << waves[i][j - 1] << endl;
            }
        }
    }
}