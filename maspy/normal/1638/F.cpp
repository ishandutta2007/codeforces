...
void solve() {
  ...
  FOR_R(L, N) FOR3(R, L + 1, N + 1) { DP[L][R] = min(A[L], DP[L + 1][R]); }
  // range max rectangle area
  ...
  FOR(M, N + 1) chmax(ANS, DP[0][M] + DP[M][N]);
  // print(ANS);
  ...
  auto shrink = [&](vc<pi>& A) -> vc<pi> {
    ...
    vc<pi> B;
    for (auto&& [h, area]: A) {
      if (len(B) == 0 || B.back().se < area) { B.eb(h, area); }
    }
    return B;
  };

  {
    ...
    FOR(i, N) FOR(j, N) {
      ...
      if (lx <= ly) continue;
      ll ax = A[i], ay = A[j];
      if (rx <= ry) { chmax(ANS, (ry - ly) * ay + (rx - lx) * (ax - ay)); }
    }
  }
  ...
  FOR(i, N) {
    ...
    ll H = A[i];
    // 
    vi B(N + 1, INF), C(N + 1, INF);
    B[L] = H;
    FOR_R(i, L) B[i] = min(A[i], B[i + 1]);
    C[R] = H;
    FOR3(i, R, N) C[i + 1] = min(C[i], A[i]);
    // 
    vc<pi> X, Y;
    FOR(i, L + 1) X.eb(B[i], B[i] * (R - i));
    FOR3_R(i, R, N + 1) Y.eb(C[i], C[i] * (i - L));
    X = shrink(X);
    Y = shrink(Y);
    /*
    print("LRH", L, R, H);
    ...
    */
    for (auto&& [hx, ax]: X) {
      while (len(Y) && hx + Y.back().fi > H) {
        ...
        ll h = H - hx;
        chmax(ANS, ax + h * wy);
        // print("ax, h, wy", ax + h * wy);
        h = H - hy;
        chmax(ANS, ay + h * wx);
        // print("ay, h, wx", ay + h * wx);
        Y.pop_back();
      }
      if (len(Y) == 0) break;
      chmax(ANS, ax + Y.back().se);
    }
  }
  print(ANS);
}
