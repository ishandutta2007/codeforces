...
void check(ll i){
	...
	auto it = S[1-a[p]].begin();
	rep(j, 1, 5){
		if(it == S[1-a[p]].end()) break;
		if(*it < p || *it >= p+3){
			q = *it;
			break;
		}
	...
	used[i] = true;
	ll res1 = query(q, p+1, p+2);
	ll res2 = query(p, q, p+2);
	
	if(res1 == a[p] && res2 == a[p]){
		S[a[p]].insert(p);
		S[a[p]].insert(p+1);
		S[a[p]].insert(p+2);
	}
...
int main(void)
{
	...
	while(T--){
		cin >> n;
		rep(i, 1, n/3) a[i*3-2] = query(i*3-2, i*3-1, i*3);
		
		S[0].clear(), S[1].clear();
		rep(i, 1, n/3-1){
			ll p = i*3-2;
			if(a[p] == a[p+3]) continue;
			rep(j, 1, 2) a[p+j] = query(p+j, p+j+1, p+j+2); 
			rep(j, 0, 2){
				if(a[p+j] != a[p+j+1]){
					S[a[p+j]].insert(p+j);
					S[a[p+j+1]].insert(p+j+3);
					break;
				}
			}
			break;
		...
		rep(i, 1, n/3) used[i] = false;
		rep(i, 1, n/3) if(!used[i]) check(i);
		rep(i, 1, n/3) if(!used[i]) check(i);
