...
int main() {
    ...
    rep(i, N) {
        ...
        if (T[sa[i]] == '0') { c.push_back(now); c.push_back(N - sa[i]); now = INF; }

        if (i < N - 1) { now = min(now,v[i]); }
    }
    ...
    rep(i, L) {
        while (st.back() != -1 && c[st.back()] >= c[i]) { st.pop_back(); }
        lef[i] = st.back();
        st.push_back(i);
    }
    ...
    st = vll(1, L);
    for (ll i = L - 1; i >= 0; i--) {
        while (st.back() != L && c[st.back()] >= c[i]) { st.pop_back(); }
        ri[i] = st.back();
        st.push_back(i);
    }

    //5rep(i, L)cout << lef[i] << " " << ri[i] << " "<<c[i]<<endl;

    ll ans = 0;
    rep(i, L) {
       ans = max(ans, (ri[i] - lef[i]) / 2 * c[i]);
