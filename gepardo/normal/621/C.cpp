...
main()
{
    ...
    in.getv(v, n);
    vector <int> pond(n), tot(n);
    for (int i = 0; i < n; i++)
        pond[i] = v[i].ss / p - (v[i].ff - 1) / p,
        tot[i] = v[i].ss - v[i].ff + 1;
    long double ans = 0;
    for (int i = 0; i < n; i++)
    {
        int a = i, b = i + 1;
        if (b == n) b = 0;
        long double ver1 = (long double)pond[a] / tot[a], ver2 = (long double)pond[b] / tot[b];
        ans += 2000 * (1 - (1 - ver1) * (1 - ver2));
