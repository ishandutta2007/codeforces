...
int main(){
  ...
  auto seg=range_min::make(n);
  auto ch=[&](ll i,ll v){
    if(seg[i]>v)seg.update(i,v);
  };
  rep(i,0,n){
    while(!st1.empty()&&w[st1.top()]>=w[i]){
      ch(st1.top(),f(st1.top(),i));
      st1.pop();
    }
    if(!st1.empty())ch(st1.top(),f(st1.top(),i));
    while(!st2.empty()&&w[st2.top()]<=w[i]){
      ch(st2.top(),f(st2.top(),i));
      st2.pop();
    }
    if(!st2.empty())ch(st2.top(),f(st2.top(),i));
    for(auto z:gl[i]){
      ret[z]=seg.query(l[z],r[z]+1);
    }
    st1.push(i);
    st2.push(i);
  }
