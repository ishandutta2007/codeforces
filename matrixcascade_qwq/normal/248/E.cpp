...
signed main()
{
	...
	while(T--)
	{
		C[0][0]=1;
		up_(500000)C[i][0]=1;
		up_(500000)up(j,1,min(i,5ll))C[i][j]=C[i-1][j-1]+C[i-1][j];
	//	opl(C[5][2]);
		rdn;double ans=0;
		upn
		{
			rd(a[i]);dp[i][a[i]]=1;if(a[i]==0)++ans;
		}
		int q;rd(q);
		
		while(q--)
		{
			int u,v;rd(u);rd(v);rdk;
			up(i,0,109)g[i]=0;
			int all=C[a[u]][k];
			up(i,0,k)
			{
				...
				up(j,i,100)
				{
					g[j-i]+=dp[u][j]/(double)(all)*C[j][i]*C[a[u]-j][k-i];
				}
			}
			ans-=dp[u][0];
			up(i,0,109)dp[u][i]=g[i];
			ans+=dp[u][0];
			a[v]+=k;a[u]-=k;
			printf("%.9lf\n",ans);
		}
