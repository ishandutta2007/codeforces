 ...
 int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000],c2[500000];string s2;vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
    ...
    while(t--){
    cin>>s1>>s2>>s3;
    map<char,int>mp,mp1;yes=true;
    for(int i=0;i<s1.size();i++)mp[s1[i]]++;
    for(int i=0;i<s2.size();i++)mp1[s2[i]]++;
    for(char i='a';i<='z';i++){if(mp1[i]<mp[i])yes=false;}
    for(int i=0;i<s3.size();i++)mp[s3[i]]++;

    for(char i='a';i<='z';i++){if(mp1[i]>mp[i])yes=false;}
    if(yes)
    {
       for(int i=0;i<s1.size();i++)mp[s1[i]]--;cnt=0;
       for(int i=0,j=0;i<s2.size();i++)
       {
           if(s2[i]==s1[j]){j++;continue;}
           else if(mp[s2[i]]>0)mp[s2[i]]--,cnt++;
           else yes=false;
       }
if(s1.size()+cnt>s2.size())yes=false;
    }
    if(yes)cout<<"YES\n";
    else cout<<"NO\n";
    }
