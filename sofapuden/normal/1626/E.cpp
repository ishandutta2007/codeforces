...
struct Mint{
	...
	Mint(int _value = 0, int _MOD = 1e9+7) : MOD(_MOD), value(((_value%MOD)+MOD)%MOD){}
...
const int fftmod = 7340033;
const Mint root(5,fftmod);
const Mint root_1(4404020,fftmod);
...
void dfs(int x, int p, vector<int> &in, vector<int> &po, vvi &gr){
	in[x] = cn++;
	for(auto y : gr[x]){
		if(y == p)continue;
		dfs(y,x,in,po,gr);
...
void dfs2(int x, vector<int> &in, vector<int> &po,vi &pre, vvi &gr, vi &ans, vi &v){
	if(ans[x])return;
	ans[x] = 1;
	for(auto &y : gr[x]){
		if(in[y] > in[x]){
			if(tot > pre[po[y]]-pre[in[y]-1]+1 || v[x]){
				dfs2(y,in,po,pre,gr,ans,v);
			}
		}
		else{
			if(pre[po[x]]-pre[in[x]-1] >= 2 || v[x]){
				dfs2(y,in,po,pre,gr,ans,v);
			}
		}
	}
...
void solve(){
	...
	tot = accumulate(all(v),0);
	vi ans(n,0);
	for(int i = 0; i < n; ++i){
		if(v[i])dfs2(i,in,po,pre,gr,ans,v);
	}
	print(ans);
}
