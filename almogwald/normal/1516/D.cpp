...
int main() {
	...
	defn defm
		veci a(n);
	intov(a,n)
	vector<veci> primer(100000);
	vector<veci> nekt(logn, veci(n+1, n));
	fori(i, n) {
		fort(j,2, 400) {
			if (a[i] % j == 0) {
				primer[j].push_back(i);
				while (a[i] % j == 0) {
					a[i] /= j;
				}
			}
		...
		if (a[i] > 1) {
			primer[a[i]].push_back(i);
		}
	}
	forin(i, primer) {
		fori(j, ((int)primer[i].size())-1) {
			min2(nekt[0][primer[i][j]], primer[i][j + 1]);
		}
	}
	forib(i, n-1) {
		min2(nekt[0][i], nekt[0][i+1]);
		forn(j, logn) {
			nekt[j][i] = nekt[j - 1][nekt[j - 1][i]];
		}
	}
	fori(i, m) {
		defa defb
