...
class G1SchastlivieChislaProstayaVersiya {
public:
    void solve() {
        int k = in.readInt();
        auto f = in.readLongArray(6);

        ll infty = numeric_limits<ll>::min() / 2;
        arr<ll> res(1000000, infty);
        arr<ll> next(res.size());
        res[0] = 0;
        int ten = 1;
        for (int i : range(6)) {
            copy(all(res), next.begin());
            for (int j : range(ten)) {
                for (int l : range(9)) {
                    if (l * ten + j >= res.size()) {
                        continue;
                    }
                    {
                        deque<pair<int, ll>> d;
                        ll add = 0;
                        for (int m = l * ten + j; m < res.size(); m += 9 * ten) {
                            if (!d.empty() && d.front().first < m - ll(9) * k * ten) {
                                d.pop_front();
                            }
                            while (!d.empty() && d.back().second + add <= res[m]) {
                                d.pop_back();
                            }
                            d.emplace_back(m, res[m] - add);
                            if (!d.empty()) {
                                maxim(next[m], d.front().second + add);
                            }
                            add += 3 * f[i];
                        }
                    }
                    {
                        ...
                        ll add = 0;
                        for (int m = l * ten + j; m < res.size(); m += 9 * ten) {
                            if (!d.empty() && d.front().first < m - ll(9) * (k - 1) * ten) {
                                d.pop_front();
                            }
                            while (!d.empty() && d.back().second + add <= res[m]) {
                                d.pop_back();
                            ...
                            d.emplace_back(m, res[m] - add);
                            for (int n : range(1, 3)) {
                                if (m + 3 * n * ten < res.size()) {
                                    maxim(next[m + 3 * n * ten], d.front().second + add + n * f[i]);
                                }
                            }
                            for (int n : range(1, 9)) {
                                if (m + n * ten < res.size()) {
                                    maxim(next[m + n * ten], d.front().second + add);
                                }
                            }
                            add += 3 * f[i];
                        }
                    ...
                    if (k > 1) {
                        ...
                        ll add = 0;
                        for (int m = l * ten + j; m < res.size(); m += 9 * ten) {
                            if (!d.empty() && d.front().first < m - ll(9) * (k - 2) * ten) {
                                d.pop_front();
                            }
                            while (!d.empty() && d.back().second + add <= res[m]) {
                                d.pop_back();
                            ...
                            for (int n : range(1, 3)) {
                                if (m + 3 * n * ten < res.size()) {
                                    for (int o : range(1, 3)) {
                                        if (m + 3 * n * ten + o * ten < res.size()) {
                                            maxim(next[m + 3 * n * ten + o * ten], d.front().second + add + n * f[i]);
                                        }
                                    }
                                }
                            }
...
int main() {
    ...
    auto time = clock();
#endif
    G1SchastlivieChislaProstayaVersiya solver;
