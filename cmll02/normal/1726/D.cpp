...
// URL: https://codeforces.ml/contest/1726/problem/D
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
signed main()
{
	...
	else
	{
		...
		gr
		std::set<int>s;
		rg(3)s.insert(U[a[i]]),s.insert(V[a[i]]);gr
		std::set<int>t;
		rg(3)t.insert(a[i]);gr
		if(s.size()==3)
		{
			for(auto x:s)b[x]=0;
			int k=0;
			rg(2)
			b[U[a[i]]]++;
			if(b[U[a[i]]]==2)k=U[a[i]];
			
			b[V[a[i]]]++;
			if(b[V[a[i]]]==2)k=V[a[i]];
			gr
			int tt=1;
			rg(m)
			if(!t.count(i))
		...
		{
			std::set<int>s;
			rg(3)s.insert(a[i]);gr
			rg(m)od(s.count(i));gr
		}
	}
