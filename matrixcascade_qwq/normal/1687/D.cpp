...
signed main()
{
	...
	while(T--)
	{
		...
		upn rd(a[i]);
		int nw=0;S(n);
		while(nw*nw+nw<a[1])++nw;
		upn pre[i]=i-1,nxt[i]=i+1;
		up(t,nw,maxn-10)
		{
			int L=max(0ll,t*t-a[1]);
			int R=t*t-a[1]+t;
			...
			upn
			{
				int gl=a[i],gr=a[find(i)];
				gl+=L;gr+=L;
				int sl=my_sqrt(gl),sr=my_sqrt(gr);
				if(sl==sr)
				{
					if(gr<=sl*sl+sl)
					{
						chkmax(al,L);chkmin(ar,L+sl*sl+sl-gr);
					}
					else
					{
						int ll=(sl+1)*(sl+1)-gl;
				...
				else
				{
					...
					{
						int ll=sr*sr-gl;int rr=sr*sr+sr-gr;
						chkmax(al,ll+L);chkmin(ar,rr+L);
					}
				}if(al>ar)break;
				i=find(i);
			}
