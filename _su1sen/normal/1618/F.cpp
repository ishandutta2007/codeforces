...
vector<int> to_digits(long long v) {
    vector<int> d;
    for (; v; v >>= 1) {
        d.push_back(v & 1);
    }
...
bool eq(const vector<int> &x1, const vector<int> &x2, int l1, int r1, int l2) {
    int w = r1 - l1, r2 = l2 + w;
    if (r2 > int(x2.size())) return false;
    rep(i, w) if (x1[l1 + i] != x2[l2 + i]) return false;
    return true;
}
...
int main() {
    ...
    vector<int> dx = to_digits(x), dy = to_digits(y);
    vector<int> dx2(find(all(dx), 1), dx.end());

    vector<int> dx1 { 1 };
    copy(all(dx), back_inserter(dx1));
    ...
    for (auto x : { dx1, dx2 }) {
        loop(2) {
            ...
            int n = x.size();
            rep(i, m - n + 1) {
                if (not eq(x, dy, 0, n, i)) continue;
                bool ok = true;
                rep(j, i)        ok &= dy[j] == 1;
                rep(j, i + n, m) ok &= dy[j] == 1;
                if (ok) {
                    print("YES");
                    return 0;
                }
