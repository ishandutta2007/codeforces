...
bool dfs(int v, int cc) {
    ...
    for (int x : g[v]) if (comp[x] == comp[v]) {
        if (!dfs(x, -cc)) return false;
    }
    return true;
}
...
int main() {
    ...
    long long ans = good_comps * 1ll * (good_comps-1) / 2;


    sort(all(edges));
    vi cc_idx(cc+1, -1);

    for (int firstEdge = 0; firstEdge < sz(edges); firstEdge++) {
        if (firstEdge != 0 && edges[firstEdge].first == edges[firstEdge-1].first) continue;
        if (!comp_ok[edges[firstEdge].first.first] || !comp_ok[edges[firstEdge].first.second]) continue;
        ...
        int seen_ccs = 0;
        for (int j = firstEdge; j < sz(edges) && edges[j].first == edges[firstEdge].first; j++) {
            auto [x,y] = edges[j].second;
            if (cc_idx[abs(seen[x])] == -1) {
                cc_idx[abs(seen[x])] = seen_ccs;
                seen_ccs += 2;

                par.push_back(-1);
                par.push_back(-1);
            }
            if (cc_idx[abs(seen[y])] == -1) {
                cc_idx[abs(seen[y])] = seen_ccs;
                seen_ccs += 2;
            ...
            int ix = cc_idx[abs(seen[x])] + (seen[x]<0);
            int iy = cc_idx[abs(seen[y])] + (seen[y]<0);
        ...
        bool is_ok = true;
        for (int i = 0; i < seen_ccs; i+=2) {
            if (find(i) == find(i+1)) is_ok = false;
        }
        ans += is_ok;
