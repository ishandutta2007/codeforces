...
struct Dsu {
  struct Node {
    int p, size;
    Node() = default;
    Node(int p, int size) : p(p), size(size) {} };

  Node *const p;
  Dsu(std::size_t size) : p(new Node[size]) {
    for (std::size_t i=0; i<size; ++i) {
      p[i] = Node(i,1); } }
  ~Dsu() { delete[] p; }

  int get(int u) {
    return p[u].p==u ? u : p[u].p = get(p[u].p); }
  bool join(int u, int v) {
    u = get(u); v = get(v);
    if (u == v) { return false; }
    if (p[u].size < p[v].size) { std::swap(u,v); }
    p[u].size += p[v].size; p[v].p = u; 
    return true; }
  Node& info(int u) { return p[get(u)]; }
};
