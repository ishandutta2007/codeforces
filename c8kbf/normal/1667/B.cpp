...
_ n, nn, p[maxn], a[maxn], t[2][maxn], b[maxn], dp[maxn];

_ query(_ x, _ nb);
void add(_ x, _ k, _ nb);
inline void AC() {
    
    n = read();
    rd(a, n);
    for(_ i = 1; i <= n; ++i) a[i] += a[i-1];
    for(_ i = 1; i <= n; ++i) p[i] = a[i];
    for(_ i = 1; i <= n+1; ++i) t[0][i] = t[1][i] = b[i] = -9E12;
    sort(p+1, p+2+n);
    ...
    for(_ i = 1; i <= n; ++i) {
        ...
        _ vl = lower_bound(p+1, p+1+nn, a[i])-p;
        dp[i] = max(dp[i], query(vl-1, 0)+i);
        dp[i] = max(dp[i], query(nn-vl, 1)-i);
        dp[i] = max(dp[i], b[vl]);
        add(vl, dp[i]-i, 0);
        add(nn-vl+1, dp[i]+i, 1);
        b[vl] = max(b[vl], dp[i]);
    }
    writeln(dp[n]);
...
void add(_ x, _ k, _ nb) {
//    printf("added x = %lld, k = %lld, at tree %lld\n", x, k, nb);
    for(; x <= nn; x += x & -x) t[nb][x] = max(t[nb][x], k);
    return;
}

_ query(_ x, _ nb) {
    _ rv = -9E12;
    for(; x; x -= x & -x) rv = max(rv, t[nb][x]);
    return rv;
}