...
void solve(){
  ...
  map<li,vi> at;
  for(li i = 0; i < (li)(n); ++i) at[A[i]].push_back(i);
  vi ans(n+1, 1'000'000'000);
  for(auto const& iv : at) {
    li i = iv.first;
    auto const& v = iv.second;
    li k = 1;
    li last = -1;
    for(li x : v) {
      k = max(k, x-last);
  ...
  for(li i = (1); i <= (li)(n-1); ++i) ans[i+1] = min(ans[i], ans[i+1]);
  for(li i = (1); i <= (li)(n); ++i) cout << (ans[i] == 1'000'000'000 ? -1 : ans[i]) << ' ';
  cout << endl;
}
