...
#line 3 "/home/nok0/documents/programming/library/math/osa_k.hpp"
...
struct osa_k {
   ...
   public:
	...
	std::vector<std::pair<int, int>> pair_factorize(int n) {
	...
	int smallest_prime_factor(const int n) const {
		return spf[n];
	}
...
void main_() {
	...
	auto dfs = [&](auto dfs, int now) -> void {
		for(auto e : h[now]) {
			...
			mt *= d * fac.inv[c];
			res += mt;
			for(auto [key, ct] : pfs[c]) {
				cnt[key] -= ct;
				chmin(mn[key], cnt[key]);
			}
			for(auto [key, ct] : pfs[d]) {
				cnt[key] += ct;
			}
			dfs(dfs, to);
			mt *= c * fac.inv[d];
