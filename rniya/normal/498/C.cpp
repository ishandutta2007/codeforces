...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        cin >> a;
        mp[i] = prime_factor(a);
        for (auto p : mp[i]) primes.emplace(p.first);
    }
    ...
    int s = n, t = n + 1, ans = 0;
    auto val = [&](int v, int x) { return mp[v].count(x) ? mp[v][x] : 0; };
    for (int x : primes) {
        Dinic<int, true> D(n + 2);
        for (int i = 1; i < n; i += 2) D.add_edge(s, i, val(i, x));
        for (int i = 0; i < n; i += 2) D.add_edge(i, t, val(i, x));
        for (int i = 0; i < m; i++) {
            int u = es[i].first, v = es[i].second;
            int a = val(u, x), b = val(v, x);
            a = min(a, b);
            if (a) D.add_edge(u, v, a);
        }
        ans += D.max_flow(s, t);
    }
