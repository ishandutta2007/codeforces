...
void solve(){
	...
	vpi v(n);
	int cn = 1;
	for(auto &x : v){cin >> x.f; x.s = cn++;}
	sort(v.begin(),v.end());
	vector<pair<int,int>> num(3e6,{0,0});
	for(int i = 0; i < min(4000,n); ++i){
		for(int j = i+1; j < min(4000,n); ++j){
			if(num[v[j].f-v[i].f] != make_pair(0,0)){
				if(num[v[j].f-v[i].f].s == v[j].s)break;
				if(num[v[j].f-v[i].f].s == v[i].s)continue;
				if(num[v[j].f-v[i].f].f == v[i].s){num[v[j].f-v[i].f].s = v[j].s;continue;}
				cout << "YES\n";
				cout << num[v[j].f-v[i].f].f << " " << v[j].s << " " << num[v[j].f-v[i].f].s << " " << v[i].s << "\n";
				return;
			}
			num[v[j].f-v[i].f] = {v[i].s,v[j].s};
		}
	}
	cout << "NO\n";
}
