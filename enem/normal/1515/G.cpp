...
static ll rn[N], sz[N], gc[N], hgt[N];
...
void dfs2(int v)
{
    ...
    for(size_t i=0; i<gr[v].size(); i++)
    {
        ...
        {
            un(v, gr[v][i].F, rn, sz);
            hgt[gr[v][i].F] = hgt[v] + gr[v][i].S;
            dfs2(gr[v][i].F);
        }
        else if(fn(v, rn) == fn(gr[v][i].F, rn))
        {
            if(gc[fn(v, rn)]==-1)
            {
                gc[fn(v, rn)] = abs(hgt[v] +gr[v][i].S - hgt[gr[v][i].F]);
            }
            else
            {
                gc[fn(v, rn)] = __gcd(gc[fn(v, rn)], abs(hgt[v]+gr[v][i].S - hgt[gr[v][i].F]));
            }
        }
    }
    // cout<<v<<' '<<hgt[v]<<' '<<gc[fn(v, rn)]<<endl;
}
...
int main()
{
    ...
    while(q--)
    {
        ...
        {
            ll x = __gcd(gc[fn(t1, rn)], t3);
            // cout<<x<<' ';
            if((t3-t2)%x==0) cout<<"YES\n";
            else cout<<"NO\n";
        }
    }
    return 0;
