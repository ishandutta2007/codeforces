...
class stuff( val type:Int,val aa:Int, val bb:Int, val cost:Int):Comparable<stuff>{
    var ca = - 1
    // 0 weapon 1 offences 2 costs
    override fun compareTo(other:stuff):Int {
        if(this.bb != other.bb){
...
fun main(){
    solve.cases{
        ...
        val p = getint()
         val weapons = List(n){
             stuff(0, getint(), -1 , getint())
         }
        val armors = List(m){
            stuff(1, -1, getint() , getint() )
        }
        val monsters = List(p){
            stuff(2,getint() ,getint() , getint() )
        }
        val all = mutableListOf<stuff>()
        all.addAll(weapons)
        all.addAll(armors)
        all.addAll(monsters)
        val allatt = mutableListOf<Int>()
        for(a in all){
            if(a.aa != -1) allatt.add(a.aa)
        }
        val cc = coordinateCompression(allatt)
        for(a in all){
            if(a.aa != -1){
                a.ca = cc.reverse[a.aa]!!
            }else{
                a.ca = -1
            }
        }
        val cheapest = LongArray(cc.forward.size){nhuge}
        for(a in weapons){
            cheapest[a.ca] = maxOf(cheapest[a.ca], - a.cost.toLong())
        }
        val st = longMaxLazy(cheapest)
        st.rangeAdd(cc.forward.size,st.n-1, nhuge)

        var ret = nhuge
        val events = (armors + monsters).sorted()
        for(e in events){
            if(e.type == 2){
                st.rangeAdd(e.ca + 1, st.n - 1 , e.cost.toLong())
            }else{
                val best = st.rangeQueryLazy(0,st.n - 1 )
                ret = maxOf(ret , best - e.cost )
            }
        }
        put(ret)
    }
