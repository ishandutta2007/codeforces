...
inline void solve() {
	...
	dp[0][0] = 0;
	for (int i = 0; i < n; ++i) {
		for (int pmsk = 0; pmsk < (1 << p); ++pmsk) {
			for (int put = 0; put < p; ++put) {
				if (pmsk >> put & 1) {
					continue;
				}
				dp[i + 1][pmsk | (1 << put)] = max(dp[i + 1][pmsk | (1 << put)], dp[i][pmsk] + v[i].y[put]);
			}
			if (i - ppcnt(pmsk) < k) {
				dp[i + 1][pmsk] = max(dp[i + 1][pmsk], dp[i][pmsk] + v[i].x);
			} else {
				dp[i + 1][pmsk] = max(dp[i + 1][pmsk], dp[i][pmsk]);
			}
		}
	}
