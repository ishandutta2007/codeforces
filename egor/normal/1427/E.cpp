...
class EKsumma {
public:
/*    void solve(int x, vector<string>& answer) const {
        set<ll> s;
        s.insert(x);
        map<ll, pair<pair<ll, ll>, char>> toAdd;
        toAdd[2 * x] = {make_pair(x, x), '+'};//to_string(x) + " + " + to_string(x);
        map<ll, pair<pair<ll, ll>, char>> how;
        while (*s.begin() != 1) {
            auto it = toAdd.begin();
            auto p = *it;
            toAdd.erase(it);
...
//            answer.push_back(p.second);
            how[p.first] = p.second;
            for (ll i : s) {
                if (s.count(i + p.first) == 0 && toAdd.count(i + p.first) == 0) {
                    toAdd[i + p.first] = {make_pair(i, p.first), '+'};
                }
                if (i != p.first && s.count(i ^ p.first) == 0 && toAdd.count(i ^ p.first) == 0) {
                    toAdd[i ^ p.first] = {make_pair(i, p.first), '^'};
                }
            }
        }
        ...
        while (!q.empty()) {
            ll cur = q.pop();
            auto h = how[cur];
            if (processed.count(h.first.first) == 0) {
                q.push(h.first.first);
                processed.insert(h.first.first);
            }
            if (processed.count(h.first.second) == 0) {
                q.push(h.first.second);
                processed.insert(h.first.second);
            }
            answer.push_back(to_string(h.first.first) + " " + h.second + " " + to_string(h.first.second));
        }
        reverse(all(answer));
    }*/
    ...
    void solve() {
        ...
        auto add = [&](ll a, char op, ll b) -> void {
            answer.push_back(to_string(a) + " " + op + " " + to_string(b));
        };
        while (x != 1) {
            ll d = x;
            ll one = 1;
