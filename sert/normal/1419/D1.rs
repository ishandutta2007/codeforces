...
fn solve(reader: &mut Reader) {
    let (n, mut a) = (reader.u(), reader.arr_i());
    a.sort();
    let (mut l, mut r) = (0usize, (n + 1) / 2);
    while r - l > 1 {
        let m = (l + r) / 2;
        let ok = (0..m).all(|x| a[n - 1 - m + x] > a[x] && a[x] < a[n - m + x]);
        if ok { l = m } else { r = m }
    }
    println!("{}", l);
    (0..l).for_each(|x| print!("{} {} ", a[n - l - 1 + x], a[x]));
    print!("{}", a[n - 1]);
    (l..n - l - 1).for_each(|x| print!(" {}", a[x]));
    println!();
}
