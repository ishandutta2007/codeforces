#line 1 "codeforces-workspace\\contest\\1543\\c\\c.cpp"
...
#line 11 "codeforces-workspace\\contest\\1543\\c\\c.cpp"
...
#line 35 "codeforces-workspace\\contest\\1543\\c\\c.cpp"
...
void main() {
  ...
  fixed_point dfs = [&](auto dfs, auto c, auto m, auto p) -> double {
    double ret = 0;
    if (c) {
      auto d = min(c, v);
      ret += (m ? dfs(c - d, m + d / 2, p + d / 2) : dfs(c - d, 0, p + d)) * c;
    }
    if (m) {
      auto d = min(m, v);
      ret += (c ? dfs(c + d / 2, m - d, p + d / 2) : dfs(0, m - d, p + d)) * m;
    }
    return ret / all + 1;
  };
