...
pii bruteforce(int n, int x, int y) {
	if (n == 1)
		return mp(1,1);
	...
	a[0] = (x == 1 ? 2 : 1);
	for (int i = 1; i < n-1; i++)
		a[i] = (x == i+1 ? a[i-1] + 2 : a[i-1] + 1);
		
	b[0] = (y == 1 ? 2 : 1);
	for (int i = 1; i < n-1; i++)
		b[i] = (y == i+1 ? b[i-1] + 2 : b[i-1] + 1);
...
pii solve(int n, int x, int y) {
	...
	int maxy = (y == n ? n - 1 : n);
	int need = max(1, x + y - maxy + 1);
	int disc = need - 1 - (need > x);
	int minv = disc + 1 - ((n == x) && (n == y));

	int maxv = min(n, x + y - 1);
