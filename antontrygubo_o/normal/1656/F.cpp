...
void solve()
{
    ...
    L = -a[0];
    sum = a[0]*get(1, n-1);
    coeff = a[0]*(n-1) + get(1, n-1);

    if (coeff>0) {cout<<"INF"<<endl; return;}
    maxx = max(maxx, sum + L*coeff);
    ...
    R = -a[n-1];
    sum = a[n-1]*get(0, n-2);
    coeff = a[n-1]*(n-1) + get(0, n-2);

    if (coeff<0) {cout<<"INF"<<endl; return;}
    maxx = max(maxx, sum + R*coeff);

    //cout<<maxx<<endl;

    for (int pos = 1; pos<n; pos++) // position of zero
    {
        sum = a[0]*a[n-1]; coeff = a[0]+a[n-1];
        sum+=a[0]*get(pos, n-2);
        coeff+=(a[0]*(n-1-pos) + get(pos, n-2));

        sum+=a[n-1]*get(1, pos-1);
        coeff+=(a[n-1]*(pos-1) + get(1, pos-1));

        R = -a[pos-1]; L = -a[pos];

        maxx = max(maxx, sum+coeff*R);
        maxx = max(maxx, sum+coeff*L);
