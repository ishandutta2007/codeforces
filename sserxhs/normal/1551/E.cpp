...
char s[N];
int a[N],b[N],c[N],f[N];
int T,n,m,q,i,j,k,x,y,z,ans,la,ksiz,ks;
int main()
{
	...
	{
		read(n,m,a,n);
		for (i=1;i<=n;i++) a[i]=i-a[i];q=0;
		for (i=1;i<=n;i++) if (a[i]>=0) b[++q]=a[i],c[q]=i-a[i];
		for (i=1;i<=q;i++) a[i]=b[i];n=q;ans=inf;
		//write(a,n);
		for (i=1;i<=n;i++) for (f[i]=1,j=1;j<i;j++) if (a[j]<=a[i]&&c[j]<c[i]) f[i]=max(f[i],f[j]+1);
		for (i=1;i<=n;i++) if (f[i]>=m) ans=min(ans,a[i]);
		enter(ans==inf?-1:ans);
	}
}