...
int main() {
    input(int, n);
    vector cnt(n, array<int, 26>{});
    vector s(1 << n, array<int, 26>{});
    rep(i, n) {
        input(string, si);
        for (char c : si) ++cnt[i][c - 'a'];
    }
    fill(all(s[0]), inf);
    vector<mint> f(1 << n);
    rep(t, 1, 1 << n) {
        ...
        int last = count_tz(x);
        s[t] = s[t ^ x];
        f[t] = __builtin_parity(t) ? +1 : -1;
        rep(j, 26) {
            chmin(s[t][j], cnt[last][j]);
            f[t] *= s[t][j] + 1;
        }
    }
    long long ans = 0;
    subset_transform::zeta(f);
    rep(t, 1 << n) {
        int k = 0, sum = 0;
        rep(i, n) {
            if (kth_bit(t, i)) {
