...
const int MAX_ROW = oj_local(4100, 100);      // to be set appropriately
const int MAX_COL = oj_local(4100 + 10, 100); // to be set appropriately
struct BitMatrix {
    int H, W;
...
int main() {
    ...
    if(~n & 1 and ~m & 1) {
        mint ans = 1;
        rep(i, n) rep(j, m) if(a[i][j] == 2) ans *= 2;
        OUT(ans);
    } else {
        int c = 0;
        rep(i, n) rep(j, m) if(a[i][j] == 2) c++;
        UnionFind uf(n + m);
        if(n & 1 and m & 1) {
            BitMatrix mat(n + m, min(4010, c + 1));
            int t = 0;
            vi res(n + m);
            rep(i, n) rep(j, m) {
                if(a[i][j] == 2) {
                    if(uf.unite(i, n + j)) mat[i][t] = mat[n + j][t] = 1, t++;
                } else
                    res[i] ^= a[i][j], res[n + j] ^= a[i][j];
            }
            rep(i, n + m) mat[i][t] = 1;
            mat.W = t + 1;
            vi v;
            ...
            dump(res);
            auto rank = linear_equation(mat, res, v);
            if(rank == -1)
                OUT(0);
            else
                OUT(mint(2).pow(c + 1 - rank));
        } else {
            if(n & 1) {
                vv(int, aa, m, n);
            ...
            dump(a);
            BitMatrix mat(n, min(4010, c + 1));
            int t = 0;
            vi res(n);
            vi used(n);
            rep(i, n) rep(j, m) {
                if(a[i][j] == 2 and !used[i]++)
                    mat[i][t] = 1, t++;
                else
                    res[i] ^= a[i][j];
            }
