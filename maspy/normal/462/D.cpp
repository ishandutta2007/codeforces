...
void solve() {
  ...
  using node_data = pair<mint, mint>;
  using edge_data = pair<mint, mint>;
  // 
  edge_data e_unit = mp(mint(1), mint(0));
  // 
  auto f_ev = [&](ll v, edge_data& x) -> node_data {
    if (A[v] == 1)
      return mp(0, x.fi);
    else
      return x;
  ...
  auto f_ve = [&](ll v, node_data& x) -> edge_data {
    return mp(x.fi + x.se, x.se);
  };
  // 
  auto mul_e = [&](edge_data& x, edge_data& y) -> edge_data {
    return mp(x.fi * y.fi, x.fi * y.se + x.se * y.fi);
  };
  EulerTour<Graph<int>> ET(G);

  auto tree_dp = [&]() {
    vc<node_data> dp1_v(N);
    vc<edge_data> dp1_e(N);
    for (auto&& v : ET.VR) {
      auto x = e_unit;
      for (auto&& e : G[v]) {
        auto w = e.to;
        if (ET.parent[w] == v) { x = mul_e(x, dp1_e[w]); }
      }
      dp1_v[v] = f_ev(v, x);
