...
_ n, m, x, y, lc, cnt, o, a[maxn], b[maxn], t[maxn<<2], lz[maxn<<2], f[maxn];
v_ h[maxn];
_p r[maxn];
bitset<maxn> v;
...
inline void AC() {
    ...
    if(m > lc) o = n;
        ...
        for(_ i = m; i--; ) {
            ...
            for(; x && !v[x]; x = f[x]) {
                v.set(x);
                update(r[x].first, r[x].second, 1, cnt, 1);
            }
        }
    }
    if(o == n) {
        _ p = -1E17;
        for(_ i = lc; i <= m; ++i) p = max(p, (n-i)*i);
        writeln(p);
    } else {
        _ p = 1E17;
        for(_ i = 0; i <= n-o; ++i) p = min(p, (n-i-m)*(m-i));
        writeln(p);
    }
...
void trev(_ x, _ fa, _ d) {
    ...
    for(_ i : h[x]) if(i != fa) {
        ...
        ++ch;
        trev(i, x, d+1);
        r[x].first = min(r[x].first, r[i].first);
        r[x].second = max(r[x].second, r[i].second);
    }
    if(!ch) {
        ++lc;
        r[x].first = r[x].second = ++cnt;
        a[cnt] = x;
        b[cnt] = d;
    }
    return;
...
_ query(_ l, _ r, _ pos) {
    ...
    _ md = (l+r)>>1;
    pushDown(pos);
    if(t[pos<<1] == t[pos]) return query(l, md, pos<<1);
    else return query(md+1, r, pos<<1|1);
}