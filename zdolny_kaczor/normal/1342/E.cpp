...
ll choose(int n, int k) {
	if (k < 0 || k > n) return 0;
	return fac[n] * fin[k] % mod * fin[n - k] % mod;
}
ll surjective(int dom, int opp) {
	ll ans = 0;
	for (int i = 0; i <= opp; ++i) {
		debug(fun(choose, opp, i) * powe(opp - i, dom), imie(i));
		ans += (i % 2 == 0 ? 1 : -1) * choose(opp, i) * powe(opp - i, dom);
		ans %= mod;
	}
	debug(imie(dom), imie(opp), imie(ans));
	return fix(ans, mod);
}
int main() {
	debug(imie(mod));
	...
	if (k0 > n) done("0\n");
	int k = k0;
	ll ans = choose(n, k) * surjective(n, n - k) * (k == 0 ? 1 : 2) % mod;
	printf("%lld\n", fix(ans, mod));
	exit(0);
}
//#STAY AT HOME