...
int main() {
    ...
    for (auto e: edges) {
        ...
        if (qf.same(i, j)) continue;
        i = qf.id[i];
        j = qf.id[j];
        int ma = max(cliq[i], cliq[j]);
        for (auto x: qf.groups[i]) {
            for (auto y: qf.groups[j]) {
                ma = max(ma, g[x][y]);
            }
        }
        bool ok = true;
        int sz = int(qf.groups[i].size() + qf.groups[j].size());
        for (auto x : qf.groups[i]) {
            if (g2[x][sz] <= ma) ok = false;
        }
        for (auto y : qf.groups[j]) {
            if (g2[y][sz] <= ma) ok = false;
        }
        ...
        auto x = pols[i], y = pols[j];
        int xs = int(x.size()), ys = int(y.size());
        auto z = V<mint>(xs + ys - 1);
