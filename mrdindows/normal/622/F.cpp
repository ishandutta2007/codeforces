...
//    DBN(solve(n, k));
    ...
    revfact[0] = n;
    for (int i = 1; i <= 1000001; ++i) {
        revfact[i] = revfact[i - 1] * (n - i) % mod;
    }
    long long revMinusOne = getRev(mod - 1);
    long long ans = 0;
    for (int i = 0; i <= k + 1; ++i) {
        ll y = ::y[i];
        ll foo = revfact[k + 1] * getRev(n - i) % mod;
        int a = i;
        if (a > 0) {
            foo = foo * getRev(fact[a]) % mod;
        }
        a = k + 1 - i;
        if (a >= 0) {
            foo = foo * getRev(fact[a]) % mod;
            if (a % 2 == 1) {
                foo = foo * revMinusOne % mod;
            }
        }
        ans = (ans + foo * y) % mod;
    }
