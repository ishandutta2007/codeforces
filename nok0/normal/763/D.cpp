...
public:
	...
	pii solve(int root = 0) {
		build(root);
		return {pre_ans, pre_id + 1};
	}
...
private:
	...
	void dfs2(int now, int per = -1, S add = e()) {
		if(chmax(pre_ans, now_ans)) pre_id = now;
		for(int i = 0; i < (int)g[now].size(); i++) {
			if(g[now][i] == per) {
				dp[now][i] = add;
				break;
		...
		for(int i = 0; i < g[now].size(); i++) {
			if(g[now][i] == per) continue;
			auto val = op2(merge(pL[i], pR[i + 1]), now);
			int old_ans = now_ans;
			auto &hm1 = hash_map[val.val()];
			hm1++;
			if(hm1 == 1)
				now_ans++;
			auto &hm2 = hash_map[dp[now][i].val()];
			hm2--;
			if(hm2 == 0)
				now_ans--;
			dfs2(g[now][i], now, val);
			hm1--;
			hm2++;
			now_ans = old_ans;
		}
...
S p, q;
S op(S x, int id) { return (x + p).pow(2); }
S merge(S x, S y) { return x + y; }
S op2(S x, int id) { return x + q; }
S e() { return 0; }

void main_() {
	xorshift rnd(clock());
	INT(n);
	int now_val = 0, now_id = 1;
	VEC(pii, uv, n - 1);
	REP(_, 5) {
		while(true) {
			int mod = 8 * POW(10, 8) + rnd() % 200000000;
			if(atcoder::internal::is_prime_constexpr(mod)) {
				p = rnd() % mod, q = rnd() % mod;
				S::set_mod(mod);
				break;
			}
		}
		re_rooting<S, op, merge, op2, e> g(n);

		for(auto [u, v] : uv) {
			g.add_edge(u, v, 1);
		}
		auto [c, id] = g.solve();
		if(chmax(now_val, c)) now_id = id;
	}
	print(now_id);
}