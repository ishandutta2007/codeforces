...
//    println(ans)
    ...
    val oc = HashMap<Int,Int>()
    for (i in a) oc.put(i,oc.getOrDefault(i,0)+1);
    for (i in 0..n-1) {
        val des_2 = sum-a[i]
        if (des_2%2 != 0L) continue
        val des = des_2/2
        if (des <= 1000000 && oc.getOrDefault(des.toInt(),0)-(if (a[i].toLong() == des) 1 else 0) > 0) ans.add(i)
    }
    io.println(ans.size)
    for (i in ans) io.print("${i+1} ")
}
