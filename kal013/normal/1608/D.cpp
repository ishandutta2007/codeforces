...
const int modular_int<mod>::inverse_limit = 500000;
...
void solve(){
    ...
    // vector<Mint> p(n + 3), q(2 * n + 6);
   	...
   	// 	// p = exponent(logarithm(p) * vector<Mint>{tot[0]});
   	// 	// q = exponent(logarithm(q) * vector<Mint>{tot[1]});
   	// 	p *= q;

   	// }

   	// cerr << wb() << " " << bw() << " " << uneq() << endl;
