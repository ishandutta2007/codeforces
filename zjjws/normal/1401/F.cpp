...
namespace gyqtree
{
    ...
    LL sum[N<<1];
    int nodecutt;
    inline void up(int i){sum[i]=sum[ls[i]]+sum[rs[i]];return;}
    inline void addtag(int i){jh(ls[i],rs[i]);tag[i]^=1;return;}
    inline void updata(int i){if(tag[i]!=gyq[dep[i]])addtag(i);return;}
    inline void maketree(int l,int r,int i,int dp)
    {
        dep[i]=dp;tag[i]=0;if(l==r){sum[i]=a[l];return;}
        int mid=l+r>>1;maketree(l,mid,ls[i]=++nodecutt,dp-1);maketree(mid+1,r,rs[i]=++nodecutt,dp-1);up(i);return;
    }
    inline void change(int l,int r,int i,int &qx,int &qk)
    {
        if(l==r){sum[i]=qk;return;}
        int mid=l+r>>1;updata(i);if(qx<=mid)change(l,mid,ls[i],qx,qk);else change(mid+1,r,rs[i],qx,qk);up(i);return;
    }
    inline LL check(int l,int r,int i,int &ql,int &qr)
    {
        if(ql<=l&&r<=qr)return sum[i];
        int mid=l+r>>1;updata(i);LL sum=0;if(ql<=mid)sum+=check(l,mid,ls[i],ql,qr);if(mid<qr)sum+=check(mid+1,r,rs[i],ql,qr);up(i);return sum;
    }
}
using namespace gyqtree;
inline void work()
{
    int typ,x,y;rin>>typ>>x;
    if(typ==1)rin>>y,change(1,n,1,x,y);
    if(typ==2)for(;x>=0;x--)gyq[x]^=1;
    if(typ==3)gyq[x+1]^=1;
    if(typ==4){rin>>y;printf("%lld\n",check(1,n,1,x,y));}
    return;
}
int main()
{
    rin>>k>>q;n=1<<k;for(int i=1;i<=n;i++)rin>>a[i];maketree(1,n,nodecutt=1,k);
    for(;q;q--)work();
    return 0;
}