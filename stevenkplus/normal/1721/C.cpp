...
int main() {
   ...
   cin >> T;
   while (T--) {
      // ans1[i] = b[i] - max(a[j]) where a[j] < b[i]
      // ans2[i] = b[i] - a[right[j]], where right[i] = largest j such that all x in [i + 1...j] satisfy b[x] >= a[x - 1]
      // i.e. right[j] is first j where b[j + 1] < a[j], or j = n - 1
      int n;
      cin >> n;
      vector<int> a(n), b(n);
      for (int &i: a) cin >> i;
