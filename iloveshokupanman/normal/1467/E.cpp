...
map<ll,ll> dfsmp(ll u, ll v) {
    ...
    for (ll w : g[v]) {
        if (u == w) { continue; }
        auto now = dfsmp(v, w);
        if (now[a[v]] > 0) {st.insert(w); }
        if(now.size()>ret.size())swap(now, ret);
        for (auto p : now) { ret[p.first] += p.second; }
    }

    
    if (mp[a[v]] > ret[a[v]] + 1) { b[v] = 1;}
    ret[a[v]]++;
...
void dfs(ll u, ll v) {
    ...
    if (nowst.count(v)) { nowst.erase(v); }
