...
void solve() {
	...
	REP(i, n) {
		for (int q = 0; q < n; ++q) {
			if (i == q or inputs[i] < inputs[q]) continue;
			nexts.push_back(make_tuple(inputs[i] - inputs[q], i + 1, q + 1));
		}
	}
	sort(ALL(nexts));
	while (nexts.empty() == false) {
		cout << "? " << get<1>(nexts.back()) << " " << get<2>(nexts.back()) << endl;
		string s;
		cin >> s;
		if (s == "Yes") {
			cout << "! " << get<1>(nexts.back()) << " " << get<2>(nexts.back()) << endl;
