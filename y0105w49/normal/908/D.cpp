...
const int N=1024;
pair<int,int> g(int as,int nd) {
	static bool seen[N][N];
	static pair<int,int> dp[N][N];
	bool &sn=seen[as][nd];
	pair<int,int> &ans=dp[as][nd];
	if(sn) return ans;
	sn=1;
	if(as>=nd) {
		int p=((k+(as-nd))*pb+pa)%P;
		int q=pb;
		return ans={p,q};
	}
	pair<int,int> A=g(as+1,nd);
	pair<int,int> B=g(as,nd-as);
	A.fi=int(1LL*A.fi*pa%P);
	A.se=int(1LL*A.se*(pa+pb)%P);
	B.fi=int(1LL*B.fi*pb%P);
	B.se=int(1LL*B.se*(pa+pb)%P);
	return ans={int((1LL*A.fi*B.se + 1LL*B.fi*A.se)%P), int(1LL*A.se*B.se%P)};
}
...
int32_t main() {
	cin>>k>>pa>>pb;
	int p,q; tie(p,q)=g(1,k);
	printf("%d\n",int(1LL*p*powq(q,P-2)%P));
}