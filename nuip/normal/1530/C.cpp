...
int main(){
	...
	TT{
		cauto n=read();
		auto as=readV(n); sort(rall(as));
		auto bs=readV(n); sort(rall(bs));
		vector<ll> sumas(n+1); rep(i,n) sumas[i+1]=sumas[i]+as[i];
		vector<ll> sumbs(n+1); rep(i,n) sumbs[i+1]=sumbs[i]+bs[i];
		reps(i,n,MOD){
			cauto sz=i-i/4;
			cauto a100=min<ll>(i-n,sz);
			cauto a=100*a100+sumas[min<int>(n,sz-a100)];
			cauto b=sumbs[min<int>(n,sz)];
			out(i,sz,a100,a,b,1);
			if(a>=b){
				cout<<i-n NL;
				break;
		...
		// vector<ll> sums{accumulate(all(as),0ll),accumulate(all(bs),0ll)};
		// vector<priority_queue<ll,vector<ll>,greater<ll>>> pqs(2);
		// for(cauto &a:as) pqs[0].push(a);
		// for(cauto &b:bs) pqs[1].push(b);
		// rep(i,2)while(n-n/4<pqs[i].size()){
		// 	sums[i]-=pqs[i].top();
		// 	pqs[i].pop();
		// }
		// rep(i,MOD){
		// 	rep(j,2){
		// 		if((n+i)-(n+i)/4<pqs[i].size()){
		// 			sums[i]-=pqs[i].top();
		// 			pqs[i].pop();
		// 		}
		// 	}
