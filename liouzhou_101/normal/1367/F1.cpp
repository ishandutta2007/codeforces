...
void solve()
{
	...
	for (int i = 0; i < m; ++i)
	{
		if (u[i]) continue;
		int j = i;
		while (j + 1 < m && v[j + 1].front() > v[j].back()) j++;
		for (int k = i; k <= j; ++k) u[k] = 1;
		int len = 0;
		for (int k = i; k <= j; ++k) len += v[k].size();
		if (i - 1 >= 0)
		{
			int L = i - 1;
			int id = lower_bound(v[L].begin(), v[L].end(), v[i].front()) - v[L].begin();
			len += id;
		}
		if (j + 1 < m)
		{
			int R = j + 1;
			int id = lower_bound(v[R].begin(), v[R].end(), v[j].back()) - v[R].begin();
			len += v[R].size() - id;
		}
		freshmax(res, len);
	}
