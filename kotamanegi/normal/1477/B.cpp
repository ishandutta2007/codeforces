...
pair<int, int> e() { return mp(0, 0); }
pair<int,int> mapping(pair<int,int> a,pair<int,int> b) {
	if (a.first == 1) {
		return mp(0, b.second);
	}
	else if (a.first == 2) {
		return mp(b.second, b.second);
	}
	return b;
}
pair<int,int>composition(pair<int,int> a, pair<int,int> b) {
	if (a.second > b.second) return a;
	return b;
}
pair<int,int> id() { return mp(0,-1); }
void solve() {
	int n, query;
	cin >> n >> query;
	string s, t;
	cin >> s >> t;
	lazy_segtree<pair<int,int>, op, e, pair<int,int>, mapping, composition, id> seg(n);
	for (int i = 0; i < s.length(); ++i) {
		seg.set(i, mp(t[i] - '0', 1));
	}
	vector<pair<int, int>> Queries;
	REP(i, query) {
		int a, b;
		cin >> a >> b;
		Queries.push_back(mp(a - 1, b));
	}
	reverse(ALL(Queries));
	REP(i, query) {
		auto tmp = seg.prod(Queries[i].first, Queries[i].second);
		if (tmp.first * 2 == tmp.second) {
			//bad
			cout << "NO" << endl;
			return;
		}
		if (tmp.first* 2 < tmp.second) {
			//fix it to 0
			seg.apply(Queries[i].first, Queries[i].second, mp(1, i));
		}
		if (tmp.first * 2 > tmp.second) {
			//fix it to 1
			seg.apply(Queries[i].first, Queries[i].second, mp(2, i));
		}
	}
	REP(i, n) {
		if (seg.get(i).first != s[i] - '0') {
			cout << "NO" << endl;
			return;
		}
	}
