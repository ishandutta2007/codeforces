...
//	}
	...
	public class Edge {
		...
		public Edge(int a, int b, int c, int type) {
			this.a = a;
			this.b = b;
			this.c = c;
			this.type = type;
	...
	public void solve() {
		...
		if (l > 1) {
			list.add(new Edge(free++, free, l - 1, 0));
			r -= l - 1;
		}
		list.add(new Edge(free, 0, 1, 1));
		r--;
		int already = 1;
		int maxpow = -1;
		for (int pow = 19; pow >= 0; pow--) {
			if (r >= (1 << pow)) {
				list.add(new Edge(free, pow + 1, already, 1));
				already += (1 << pow);
				r -= (1 << pow);
				maxpow = Math.max(maxpow, pow);
			}
		}
		int maxv = free + maxpow + 2;
		for (int i = maxpow; i >= 0; i--) {
			for (int j = 0; j < i; j++) {
				list.add(new Edge(maxv - i - 1, maxv - j - 1, (1 << j), 0));
			}
			list.add(new Edge(maxv - i - 1, maxv, 1, 0));
		}
		out.println("YES");
		out.println(maxv + " " + list.size());
		for (Edge e : list) {
			if (e.type == 0) {
				out.println(e.a + " " + e.b + " " + e.c);
			} else {
				out.println(e.a + " " + (maxv - e.b) + " " + e.c);
			}
		}
	}
