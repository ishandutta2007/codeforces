...
int main() {
    ...
    vector<int> x(q), y(q);
    vector<string> S(q);
    for (int i = 0; i < q; i++) cin >> x[i] >> y[i] >> S[i], x[i]--, y[i]--;
    ...
    auto PATH = [&](int u, int v) {
        ...
        while (u != p) {
            ascend.emplace_back(u);
            u = HLD.par[u];
        }
        while (v != p) {
            descend.emplace_back(v);
            v = HLD.par[v];
        }
        vector<int> res;
        for (int& w : ascend) res.emplace_back(w);
        res.emplace_back(p);
        reverse(descend.begin(), descend.end());
        for (int& w : descend) res.emplace_back(w);
        return res;
    };

    vector<vector<char>> cand(n);
    vector<vector<int>> path(q);
    for (int i = 0; i < q; i++) {
        path[i] = PATH(x[i], y[i]);
        assert(path[i].size() == S[i].size());
        for (size_t j = 0; j < path[i].size(); j++) {
            if (cand[path[i][j]].size() != 0) continue;
            cand[path[i][j]].emplace_back(S[i][j]);
        }
        reverse(path[i].begin(), path[i].end());
        for (size_t j = 0; j < path[i].size(); j++) {
            if (cand[path[i][j]].size() != 1) continue;
            cand[path[i][j]].emplace_back(S[i][j]);
        }
        reverse(path[i].begin(), path[i].end());
    }

    TwoSAT TS(n + q);  // vertex , path
    for (int i = 0; i < q; i++) {
        for (size_t j = 0; j < path[i].size(); j++) {
            int v = path[i][j];
            if (cand[v].size() < 2) continue;
            for (int k = 0; k < 2; k++) {
                if (cand[v][k] != S[i][j]) {
                    TS.add_if(n + i, 0, v, k ^ 1);
                }
            }
        }
        reverse(path[i].begin(), path[i].end());
