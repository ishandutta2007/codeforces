...
mint perm(int n, int k) {
  ...
  auto dfs = [&](auto self, int u, int p) -> int {
    int res = 1;
    for(auto& [v, sz]: to[u]) if (v != p) {
      res += sz = self(self, v, u);
    }
    for(auto& [v, sz]: to[u]) if (v == p) {
      sz = n - res;
    }
  ...
  rep(i, n) {
    mint des_cnt;
    for(auto [_, sz]: to[i]) des_cnt += comb(sz, k);
    ans += n * (comb(n, k) - des_cnt);
    for(auto [_, sz]: to[i]) ans += (n - sz) * (comb(n - sz, k) - (des_cnt - comb(sz, k))) * sz;
  }
  cout << ans.val() << '\n';
}