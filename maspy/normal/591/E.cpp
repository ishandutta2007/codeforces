...
void solve() {
  ...
  vvv(ll, dist, 3, H, W, INF);
  FOR(comp, 3) {
    deque<pi> que;
    auto& D = dist[comp];
    FOR(x, H) FOR(y, W) if (G[x][y] == '1' + comp) {
      que.eb(x, y);
      D[x][y] = 0;
    }
    while (len(que)) {
      ...
      FOR(d, 4) {
        ...
        if (chmin(D[nx][ny], D[x][y] + 1)) que.eb(nx, ny);
      }
    }
  }

  vv(ll, C, 3, 3, INF);
  FOR(a, 3) FOR(b, 3) FOR(x, H) FOR(y, W) {
    chmin(C[a][b], dist[a][x][y] + dist[b][x][y] - 1);
  }
  ll ANS = INF;
  chmin(ANS, C[0][1] + C[0][2]);
  chmin(ANS, C[1][2] + C[1][0]);
  chmin(ANS, C[2][0] + C[2][1]);
  FOR(x, H) FOR(y, W) {
    ll cost = -2;
    FOR(a, 3) cost += dist[a][x][y];
    chmin(ANS, cost);
  }
  if (ANS > INF / 2) ANS = -1;
  print(ANS);
