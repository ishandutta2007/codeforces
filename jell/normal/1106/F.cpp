#line 1 "codeforces-workspace/contest/1106/f/f.cpp"
/**
 * @file template.cpp
 * @brief Template
 */
...
#line 16 "codeforces-workspace/contest/1106/f/f.cpp"
...
template <auto Mod, unsigned Storage> struct modint_base {
 ...
 protected:
  static value_type _mem(value_type __x) {
    static std::vector<modint_base> __m{0, 1};
    static value_type __i = (__m.reserve(Storage), 2);
    while (__i <= __x) {
      __m.emplace_back(mod / __i * -__m[mod % __i]);
      ++__i;
    }
    return __m[__x];
  }
...
#line 33 "codeforces-workspace/contest/1106/f/f.cpp"

namespace workspace {

constexpr int mod = 998244353;
constexpr modint<mod> root = 3;
using mint = modint<mod - 1>;

int dlog(modint<mod> x, modint<mod> r = root) {
  unordered_map<modint<mod>, int> b;
  modint<mod> p{1};
  int i;
  for (i = 0; i * i < mod; ++i) {
    b.emplace(p, i);
...
void main() {
  ...
  cin >> b >> n >> m;
  --n;
  auto mul = [&](vector<mint> & p, vector<mint> const& q) -> auto {
    vector<mint> r(size(p) + size(q));
    for (auto i = 0; i < size(p); ++i)
      for (auto j = 0; j < size(q); ++j) r[i + j] += p[i] * q[j];
    for (auto i = size(r); i-- > k;)
      for (auto j = 0; j < k; ++j) r[i - 1 - j] += r[i] * b[j];
    r.resize(k);
    swap(p, r);
  };
  vector<mint> p{0, 1}, r{1};
  while (n) {
    if (n & 1) mul(r, p);
    mul(p, p);
    n /= 2;
  }
  auto x = dlog(m, pow(root, (int)r.back()));
  if (~x)
    cout << pow(root, x) << eol;
  else
    cout << -1 << '\n';
