...
template<class T> struct Seg { // comb(ID,b) = b
    bool ismin;
    T ID; T comb(T a, T b) { return ismin ? min(a, b) : max(a, b); }
    int n; vector<T> seg;
    void init(int _n, bool _ismin) { n = _n; ismin = _ismin; ID = ismin ? 1e9 : -1e9; seg.assign(2*n, ID); }
    ...
    F0R(query, q) {
        ...
        if (t==0) {
            ...
            if (x==0) {
                ...
                while (it != remain.end() && *it <= r) {
                    remove.insert(*it);
                    it++;
                }
                for (int i : remove) remain.erase(i);
            } else {
                minst.upd(l, min(minst.query(l, l), r));
                maxst.upd(r, max(maxst.query(r, r), l));
            }
        } else {
            cin >> j; j--;
            if (remain.find(j)==remain.end()) cout << "NO\n";
            else {
                auto it = remain.find(j);
                int left, right;
                if (it==remain.begin()) left=0;
                else {
                    it--;
                    left = *it + 1;
                    it++;
                ...
                it++;
                if (it == remain.end()) right=n-1;
                else right = *it - 1;
                
                if (minst.query(left, j) <= right || maxst.query(j, right) >= left) cout << "YES\n";
                else cout << "N/A\n";
            }
        }
    }
