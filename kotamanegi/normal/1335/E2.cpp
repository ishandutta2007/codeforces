...
void solve(){
    ...
    int ans = 0;
    for (int i = 1; i <= 200; ++i) {
        if (calcs[i].size() == 0) continue;
        ans = max(ans, (int)calcs[i].size());
        for (int q = 0; q < calcs[i].size() / 2; ++q) {
            int gekos = 0;
            REP(j, 201) {
                gekos = max(gekos, cnts[calcs[i][calcs[i].size() - 1 - q]][j] - cnts[calcs[i][q] + 1][j]);
            }
            ans = max(ans, (q + 1) * 2 + gekos);
        }
    }
    cout << ans << endl;
}
