...
#line 2 "/home/maspy/library/string/trie.hpp"
...
#line 4 "/home/maspy/library/string/trie.hpp"
...
void solve() {
  ...
  FOR_R(v, V) {
    // leaf node
    bool is_leaf = [&]() {
      FOR(s, 26) if (TO[v][s] != -1) return false;
      return true;
    }();
    if (is_leaf) {
      win[v] = 0;
    ...
    FOR(s, 26) {
      ...
      if (to == -1) continue;
      if (win[to] == 0) win[v] = 1;
      if (lose[to] == 0) lose[v] = 1;
    }
  }

  auto OUT = [&](bool b) -> void { print((b ? "First" : "Second")); };

  bool a = win[0], b = lose[0];
  if (a && b) {
    // 
    OUT(true);
  }
