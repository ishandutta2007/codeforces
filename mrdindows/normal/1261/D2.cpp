...
template <class Ch, class Tr, class Container>
basic_ostream <Ch, Tr> & operator << (basic_ostream <Ch, Tr> & os, const Container& x) {
    if constexpr (std::is_pointer<std::decay_t<Container>>::value) return os << (void*)x << endl; else
...
int solve() {
    ...
    for (int i = 0; i <= cnt / 2; ++i) {
        ll cur = cnk(cnt, i) * cnk(cnt - i, i) % fftmod * powmod(freeChoices, cnt - 2 * i, fftmod) % fftmod * powmod(k, n - cnt, fftmod) % fftmod;
        ans += cur;
    }
    ans %= fftmod;
    ll tot = powmod(k, n, fftmod) - ans;
    if (tot < 0) tot += fftmod;
    cout << tot * powmod(2, fftmod - 2, fftmod) % fftmod << endl;
