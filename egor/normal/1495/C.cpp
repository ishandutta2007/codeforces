...
public:
    void solve() {
        ...
        for (int i : range(ls.size() - 1)) {
            bool found = false;
            for (int j : range(m)) {
                if (c(ls[i] + 1, j) == 'X' || c(ls[i] + 2, j) == 'X') {
                    c(ls[i] + 1, j) = c(ls[i] + 2, j) = 'X';
                    found = true;
                    break;
                }
            }
            if (!found) {
                c(ls[i] + 1, 0) = c(ls[i] + 2, 0) = 'X';
            }
        }
        for (int i : range(n)) {
            out.printLine(string(all(c[i])));
        }
//        out.printLine();
    }
};
