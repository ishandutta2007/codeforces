...
int main() {
	CASE(t) {
		...
		const ll IMP = -1e15;
		vector<vector<ll>> dp(n + 1, vector<ll>(k + 1, IMP));
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j <= k; j++) {
				...
				if (chkmax(dp[i + 1][j], dp[i][j] + a[i].fi.se * (k - 1)))
					pre[i + 1][j] = j;
				if (j < k && chkmax(dp[i + 1][j + 1], dp[i][j] + a[i].fi.fi + a[i].fi.se * j))
					pre[i + 1][j + 1] = j;
			}

		vector<int> keep;
		vector<int> del;
		for (int i = n, take = k; i > 0; i--) {
			int last = pre[i][take];

			if (last < take) {
				take = last;
				keep.emplace_back(a[i - 1].se);
			}
			else {
				del.emplace_back(a[i - 1].se);
			}
		}
