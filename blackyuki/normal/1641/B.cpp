...
int main(){
    ...
    rep(tt,t){
        ...
        rep(i,n)mp[v[i]]++;
        bool ok=true;
        for(auto x:mp)if((x.se)%2)ok=false;
        if(!ok){
            out(-1);continue;
        }
        ll cnt=0;
        ...
        rep(i,n){
            ll idx=-1;
            rep(j,cur.size())if(v[i]==cur[j])idx=j;
            if(idx!=-1){
                for(int j=idx-1;j>=0;j--){
                    ans1.pb(cur[j],cnt);cnt++;
                }
                cnt+=idx+1;
                for(int j=idx+1;j<cur.size();j++){
                    ans1.pb(cur[j],cnt);cnt++;
                }
                for(int j=idx-1;j>=0;j--){
                    ans1.pb(cur[j],cnt);cnt++;
                ...
                cnt+=cur.size()-1;
                ans2.pb(idx*4+2+(cur.size()-idx-1)*2);
                vi ncur;
                rep(j,idx)ncur.pb(cur[j]);
                for(int j=cur.size()-1;j>idx;j--)ncur.pb(cur[j]);
                cur=ncur;
            }
            else{
                cur.pb(v[i]);cnt++;
        ...
        out(ans1.size());
        for(auto x:ans1)cout<<x.se<<' '<<x.fi<<endl;
        out(ans2.size());
        outv(ans2);
    }
}