...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto n1=read();
	cauto vs1=readV(n1);
	const vector vs{vs0,vs1};
	auto dp=table(2,n,0);
	dp[0][0]=dp[1][0]=-1;
	auto deg=table(2,n,0);
	rep(i,2)reps(j,1,n) deg[i][j]=vs[i].size();
	queue<pii> que;
	que.emplace(0,0);
	que.emplace(1,0);
	while(que.size()){
		cauto [p,v]=que.front(); que.pop();
		for(auto d:vs[1-p]){
			int w=(v+n-d)%n;
			if((dp[p][v]>0 && --deg[1-p][w]==0) ||
				(dp[p][v]<0 && MN(deg[1-p][w],0))){
				out(p,v,1-p,w,1);
				dp[1-p][w]=-dp[p][v];
				que.emplace(1-p,w);
			}
	...
	out(dp,1);
	vector tmp{"Lose","Loop","Win"};
	rep(i,2)reps(j,1,n) cout<<tmp[dp[i][j]+1]<<" \n"[j+1==n];
  return 0;
}