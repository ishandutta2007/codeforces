...
ld f(long double c, long double m, long double p, long double v, int k) {
    if(k == 23) return 0;
    ...
    if(c > eps) {
        if(c > v) {
            if(m > eps) ret += c * f(c - v, m + v / 2, p + v / 2, v, k + 1);
            else        ret += c * f(c - v, m        , p + v, v, k + 1);
        } else {
            if(m > eps) ret += c * f(0, m + c / 2, p + c / 2, v, k + 1);
            else        ret += c * f(0, m        , p + c, v, k + 1);
        }
    }

    if(m > eps) {
        if(m > v) {
            if(c > eps) ret += m * f(c + v / 2, m - v, p + v / 2, v, k + 1);
            else        ret += m * f(c        , m - v, p + v    , v, k + 1);
        } else {
            if(c > eps) ret += m * f(c + m / 2, 0    , p + m / 2, v, k + 1);
            else        ret += m * f(c        , 0    , p + m    , v, k + 1);
        }
    }
...
int main() {
    ...
    REP(i, t) {
        ...
        cout << fixed << setprecision(39) << f(c, m, p, v, 0) << endl;
    }
