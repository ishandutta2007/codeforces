...
int main(){
	scan(n,m);
	for(int i=1;i<=n;++i) read(A[i]+1,A[i]+m+1);
	for(int i=1;i<=n;++i) for(int j=1,x;j<=m;++j) gi(x),A[i][j]^=x;
	for(int i=1;i<=n;++i) for(int j=1;j<=m;++j) x[i]^=A[i][j],y[j]^=A[i][j];
	for(int i=1;i<=n;++i) if(x[i]) { cout<<"No"<<endl,exit(0); }
	for(int i=1;i<=m;++i) if(y[i]) { cout<<"No"<<endl,exit(0); }
	cout<<"Yes"<<endl;
}