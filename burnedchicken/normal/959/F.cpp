...
//i_am_noob
int n,q,a[maxn],x=0,cur=0;
bool vis[pow2(20)+5]={false};
...
struct query{
    int pos,l,x;
    modint res;
    bool operator <(const query& o){return l<o.l;}
}b[maxn];
...
signed main(){
    ...
    cin >> n >> q;
    rep(n) cin >> a[i];
    rep(q) cin >> b[i].l >> b[i].x;
    rep(q) b[i].pos=i;
    sort(b,b+q);
    vis[0]=true;
    rep(n){
        if(!vis[a[i]]) {rep1(j,pow2(20)) if(vis[j]) vis[j^a[i]]=true;}
        else x++;
        while(cur<q&&b[cur].l==i+1){
            b[cur].res=vis[b[cur].x]?modint(2).Pow(x):modint(0);
            cur++;
        }
    }
    rep(q) ans[b[i].pos]=b[i].res;
    rep(q) print1(ans[i]);
    return 0;
}