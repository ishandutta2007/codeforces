...
_ n, m, b[maxn], f[maxn], l, r, md, s;
bool p;
struct point {
    _ x, y, id;
    bool operator < (const point &b) const {
        if(p) return y == b.y ? x < b.x : y < b.y;
...
inline void AC() {
    ...
    p = false;
    sort(a+1, a+1+n);
    for(_ i : rg(n-1)) if(a[i].x == a[i+1].x && abs(a[i].y-a[i+1].y) <= m) {
        _ x = find(a[i].id), y = find(a[i+1].id);
        f[x] = y;
        b[y] = min(b[y], b[x]);
    }
    p = true;
    sort(a+1, a+1+n);
    for(_ i : rg(n-1)) if(a[i].y == a[i+1].y && abs(a[i].x-a[i+1].x) <= m) {
        _ x = find(a[i].id), y = find(a[i+1].id);
        f[x] = y;
        b[y] = min(b[y], b[x]);
    }
    st.clear();
    g.clear();
    for(_ i : rg(n)) st.insert(find(i));
    for(_ i : st) g.push_back(b[i]);
    sort(g.begin(), g.end());
    l = 0;
    r = g.size();
    for(; l <= r; ) {
