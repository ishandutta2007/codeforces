...
// URL : https://codeforces.com/contest/1369/problem/F
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
//1:48:30-
int solve(ll s,ll e,int w,int d){
	//printf("%lld %lld %i %i\n",s,e,w,d);
	ll same=e/2;
	if(d==0){
		if(same<s){
		...
		int flip=(e-same+1)&1;
		return solve(s,same,(w^flip)^1,(e&1)^w^1);
	}
	if(same<s)
		return 1;
	return solve(s,same,0,0);
}
int main()
{
	int n;
	...
	for(int i=0;i<n;i++){
		ll s,e;
		scanf("%lld %lld",&s,&e);
		int win=solve(s,e,0,0);
		int lose=solve(s,e,1,1);
		//printf("%i %i!\n",win,lose);
		int np=(prvi&&lose)||(drugi&&!lose);
		int nd=(prvi&&win)||(drugi&&!win);
		prvi=np;
		drugi=nd;
	}
	printf("%i %i\n",drugi,prvi);
