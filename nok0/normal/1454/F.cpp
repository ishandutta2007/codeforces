...
void solve() {
	...
	SegTree<ll> RmQ(n, INF, f, update);
	REP(i, n) { RmQ.set(i, a[i]); }

	map<int, set<int>> lc, rc;
	...
	REP(i, n) {
		auto itr = lc[a[i]].lower_bound(i);
		if(itr == lc[a[i]].begin()) continue;
		itr--;
		auto itr2 = rc[a[i]].upper_bound(i);
		if(itr2 == rc[a[i]].end()) continue;
		debug(i, *itr, *itr2);
		if(RmQ.query(*itr + 1, *itr2) == a[i]) {
			YES(1);
			cout << *itr + 1 << " " << *itr2 - *itr - 1 << " " << n - *itr2 << endl;
			return;
		}
	}
	YES(0);
