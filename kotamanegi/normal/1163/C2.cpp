...
void solve() {
    ...
    REP(i, n) {
        for (int q = i + 1; q < n; ++q) {
            Fraction A(inputs[q].second - inputs[i].second, inputs[q].first - inputs[i].first);
            Fraction B(inputs[i].second * (inputs[q].first - inputs[i].first) - inputs[i].first * (inputs[q].second - inputs[i].second), inputs[q].first - inputs[i].first);
            if (inputs[q].first == inputs[i].first) {
                A.a = 0;
                A.b = 0;
                B.a = inputs[i].first;
                B.b = 0;
            }
            Lines.push_back(mp(mp(A.a, A.b), mp(B.a, B.b)));
        }
    }
    sort(ALL(Lines));
