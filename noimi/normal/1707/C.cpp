...
int main() {
    ...
    fore(x, y, v) {
        ...
        if(l == x or l == y) {
            if(l == x) swap(x, y);
            int d = hld.d[x] - hld.d[y];
            int nxt = hld.la(x, d - 1);
            assert(hld.par[nxt] == y);
            seg.update(hld.in[x], hld.out[x], -1);
            seg.update(hld.in[nxt], hld.out[nxt], 1);
            // seg.update(0, n, -1);
            // seg.update(0, n, 1);
        } else {
            dump(x, y);
            seg.update(hld.in[x], hld.out[x], -1);
            seg.update(hld.in[y], hld.out[y], -1);
            seg.update(0, n, 1);
        }
        // rep(i, n) cout << seg[i] << " ";
        // OUT();
    ...
    // rep(i, n) dump(hld.in[i], hld.out[i]);

    rep(i, n) cout << (seg[hld.in[i]] == 0);
    OUT();
}