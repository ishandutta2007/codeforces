...
void work()
{
    ...
    {
        s[i] = s[i - 1] + a[i];
        for(int j = 0; j < 2; j++) ss[j][i] = ss[j][i - 1];
        ss[i & 1][i] += a[i];
    }
    ...
    for(int i = 1; i <= n + 1; i++)
    {
        ...
        while(l <= r)
        {
            ...
            if((ss[d][y] - ss[d][i - 1] + s[n] - s[y]) * 2 > s[n])
                res = mid, l = mid + 1;
            else
                r = mid - 1;
        }
//        cout <<i <<" C..|PC..PC|P.. " <<res <<endl;
        ans += res + 1;
...
//            if(i == 1)
//            {
//                cout <<y <<" " <<ss[d][y] - ss[d][i - 1] <<" " <<s[n - 1] - s[y] <<" " <<s[n] <<endl;
//            }

            if((ss[d][y] - ss[d][i - 1] + s[n - 1] - s[y]) * 2 > s[n])
                res = mid, l = mid + 1;
            else
                r = mid - 1;
...
//        cout <<i <<" C..|PC..PC|P..|C " <<res <<endl;
        ...
        if(i >= 4)
        {
            ...
            while(l <= r)
            {
                ...
                if((a[1] + ss[d][y] - ss[d][i - 1] + s[n] - s[y]) * 2 > s[n])
                    res = mid, l = mid + 1;
                else
                    r = mid - 1;
            }
//            cout <<i <<" P|CC.|PC..PC|P.. " <<res <<endl;
            ans += res + 1;
            ...
            while(l <= r)
            {
                ...
                if((a[1] + ss[d][y] - ss[d][i - 1] + s[n - 1] - s[y]) * 2 > s[n])
                    res = mid, l = mid + 1;
                else
                    r = mid - 1;
            }
//            cout <<i <<" P|CC.|PC..PC|P..|C " <<res <<endl;
            ans += res + 1;
        }
    }
