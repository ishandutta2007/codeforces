...
//     return std::forward<R>(r) | mapped([](auto&& e) { return std::tuple{std::get<Ns>(e)...}; });
//   }
// };

// template <int... Ns>
// inline constexpr elements_t<Ns...> elements;
...
void main() {
  input<int>();
  for (int n; scan(n);) {
    auto is_palindrome = [](auto v) { return v == (v | reversed); };

    auto a = input<std::vector<int>>(n);
    bool ok = is_palindrome(a | all);
    if (!ok) {
      auto a_rev = a | reversed;
      int i = (rep(n) | IF(a[_] != a_rev[_])).front();
      ok |= is_palindrome(a | IF(_ != a[i]));
      ok |= is_palindrome(a | IF(_ != a_rev[i]));
    }
    println(ok ? "YES" : "NO");
  }
}
