...
main() {
    INT(n, q);
    VEC(int, a, n);
    function<Node(Node, Node)> f = [](Node x, Node y) {
        Node res;
        res.sum = x.sum + y.sum;
        rep2(i, 0, 9) res.dig[i] = x.dig[i] + y.dig[i];
        return res;
    };
    function<Node(Node, A)> g = [](Node x, A y) {
        Node res;
        rep2(i, 0, 9) res.dig[y[i]] += x.dig[i];
        rep2(i, 0, 9) res.sum += res.dig[i] * i;
        return res;
    };
    function<A(A,A)> h = [](A x, A y) {
        rep2(i, 0, 9) x[i] = y[x[i]];
        return x;
    };
    A E0;
    rep2(i, 1, 9) E0[i] = i;
    LazySegmentTree<Node, A> seg(n, f, g, h, Node(), E0);
    vector<Node> data(n);
    rep(i, n) {
        ll x = a[i], d = 1;
        while(x) {
        ...
        rep2(j, 0, 9) data[i].sum += data[i].dig[j] * j;
    }
    seg.build(n, data);
    while(q--) {
        INT(id);
