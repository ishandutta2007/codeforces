...
int main(){
    ...
    SegmentTree<int> segh(MAX_H,f,0),segw(MAX_H,f,0);
    set<pair<int,int>> sh,sw;
    sh.emplace(h,0); sw.emplace(w,0);
    int curh=0,curw=0;
    segh.update(0,h); segw.update(0,w);
    auto add_h=[&](int x){
        auto itr=sh.lower_bound({x,-1});
        auto p=*itr; sh.erase(itr);
        int id=p.second,pre=segh[id];
        segh.update(id,pre-(p.first-x));
        sh.emplace(x,id);
        segh.update(++curh,p.first-x);
        sh.emplace(p.first,curh);
    };
    auto add_w=[&](int x){
        auto itr=sw.lower_bound({x,-1});
        auto p=*itr; sw.erase(itr);
        int id=p.second,pre=segw[id];
        segw.update(id,pre-(p.first-x));
        sw.emplace(x,id);
        segw.update(++curw,p.first-x);
        sw.emplace(p.first,curw);
    };
    for (;n--;){
        char c; int x; cin >> c >> x;
        if (c=='H') add_h(x);
        else add_w(x);
        int p=segh.query(0,MAX_H),q=segw.query(0,MAX_H);
        ll ans=ll(p)*q;
        // debug(p,q,segh[0],segh[1]);
        cout << ans << '\n';
    }
}