...
int dfs(const int a, const int m)
{
	int ans = 1;
	nv[a] = true;
	for (int to : g[a])
...
bool euler(const vpii& a, const int k)
{
	int m = (1 << k) - 1;
	for (pii b : a)
	{
	...
	fill(nv.begin(), nv.begin() + m + 1, false);
	return (c == 0);
}

int get(const vpii& a, const int n)
{
	if (n & 1)
		return a[n / 2].Y;
	else
...
int get(const vpii& a, const int n, const int m)
{
	return get(a, n) & m;
}

vi euler1(const vpii& a, const int k)
{
	int m = (1 << k) - 1;
	vvi h(m + 1);
	int n = sz(a);
