...
int res;
poly solve(int m) {
	if (m==1) { res=1; vector<int> F(n+1,1); return F; }
	poly F=solve(m/2);
	poly A(n+1,0),B(n+1,0);
	for (int i=0;i<=n;i++)
		if (i&1) A[i]=F[i]; else B[i]=F[i];
	A[1]=(m+1)/2;
	poly T=Inv(((poly){1})-A);
	F=A+B*B*T;
	if (n&1) res=0; else res=(ll)res*2%mod;
	add(res,T[n]);
	for (int i=1;i<=n;i+=2) add(res,(ll)A[i]*T[n-i]%mod*(i-1)%mod);
	F.resize(n+1);
	return F;
}
int main() {
