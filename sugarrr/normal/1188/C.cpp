...
/*
    int n=1000;
    int K=4;
    vector<int>a(n);rep(i,0,n-1)a[i]=abs((i*i*2324+i*231+13241)%100000);
 */
    int n,K;cin>>n>>K;
    ...
    if(K==2){
        mint ans=0;
        rep(i,0,n-1)rep(j,i+1,n-1)ans+=a[j]-a[i];
        cout<<ans.val()<<endl;
        return 0;
    }
    if(K==3){
        ...
        rep(M,1,n-3){
            ...
            rep(i,0,M-1)l.pb(a[M]-a[i]);
            {
                rep(i,M+1,n-1){
                    ...
                    if(n-P>=1)r.pb({mi,n-P});
                    rep(j,i+1,P-1)r.pb({a[j]-a[i],1});
                }
            }
        
            SORT(l);
            sort(all(r),[](pair<int,int>a,pair<int,int>b){return a.fi<b.fi;});
            ll sr=r.size();
            ll sl=l.size();
            ll pr=0;
            ll SUMR=0;for(auto x:r)SUMR+=x.se;
            rep(i,0,sl-1){
                while(pr<=sr-1 && r[pr].fi<=l[i]){
                    SUMR-=r[pr].se;
                    pr++;
                }
                if(pr==sr)break;
            ...
            ll pl=0;
            rep(i,0,sr-1){
                while(pl<=sl-1 && l[pl]<r[i].fi)pl++;
                if(pl==sl)break;
                ans+=r[i].fi*r[i].se*(sl-pl);
            }
        }
        mint a=ans;
        cout<<a.val()<<endl;
    ...
    ll mx=(a.back()-a.front())/(K-1) + 1;
    
    vvi e(n+1,vi(mx+2));
    rep(j,1,K){
        vvi f(n+1,vi(mx+2));
        if(j==1){
            rep(i,0,n-1)f[i+1][mx+1]=1;
        }else{
            rep(i,0,n-1){
                rep(x,0,mx){
                    ll pl=lower_bound(a.begin(),a.begin()+i,a[i]-x)-a.begin();
                    ll pr=upper_bound(a.begin(),a.begin()+i,a[i]-x)-a.begin();
                    f[i+1][x+1]+=e[pl][x+1]+e[0][x]-e[pl][x]-e[0][x+1];
                    f[i+1][x+1]+=e[pr][mx+1]+e[pl][x]-e[pr][x]-e[pl][mx+1];
                }
            }
        }
        rep(i,0,n-1)rep(x,0,mx){
            f[i+1][x+1]+=f[i+1][x];
        }
        rep(i,0,n-1)rep(x,0,mx+1){
            f[i+1][x]+=f[i][x];
        }
        e=f;
    }
