...
void solve() {
    ...
    vvc<int> dp(2, vc<int>(n + 1, 0));
    int ans = 0;
    auto calc = [&](int &val, int t) -> void {
        if(val <= 0) val += t;
        else val -= t;
    };
    rep(i, n) {
        rep(j, 2) {
            dp[j][i + 1] = dp[j][i] + a[j][i] - b[j][i];
        }
        if(dp[0][i + 1] * dp[1][i + 1] < 0) {
            int t = min(abs(dp[0][i + 1]), abs(dp[1][i + 1]));
            ans += t;
            calc(dp[0][i + 1], t);
            calc(dp[1][i + 1], t);
        }
        ans += abs(dp[0][i + 1] + dp[1][i + 1]);
    }
    out(ans);
}
