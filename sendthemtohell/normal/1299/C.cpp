...
vector<ld> stp(vector<int>& v, int n) {
	vector<ld> ans;
	int ptr = 0;
	while (ptr < n) {
		ll sm = v[ptr];
		...
		for (int i = ptr + 1; i < n; ++i) {
			pair<ll, int> nvl = { sm + v[i], i - ptr + 1 };
			if (vl.x * nvl.y > nvl.x * vl.y) {
				vl = nvl;
			}
			sm += v[i];
		}

		for (int i = 0; i < vl.y; ++i) {
			ans.push_back(ld(vl.x) / vl.y);
		}
		ptr += vl.y;
	}
	return ans;
...
bool check(const pair<ll, int>& p1, const pair<ll, int>& p2) {
	// p1.x / p1.y > (p1.x + p2.x) / (p1.y + p2.y)
	return p1.x * (p1.y + p2.y) > p1.y * (p1.x + p2.x);
}

vector<ld> slv(vector<int>& v, int n) {
	...
	for (int i = 0; i < n; ++i) {
		sm = v[i];
		cnt = 1;
		while (sz(st) && check(st.back(), { sm, cnt })) {
			auto b = st.back();
			st.pop_back();
			sm += b.x;
			cnt += b.y;
	...
	for (int i = 0; i < sz(st); ++i) {
		for (int j = 0; j < st[i].y; ++j) {
			ans.push_back(ld(st[i].x) / st[i].y);
		}
	}
	return ans;
}
