...
inline bool check(int n, LL k) {
    if(n >= 61) return 1;
    return (n? (1ll << (n - 1)) : 1) >= k;
}

void solve(int n, LL k, const int base = 0) {
    // debug("solve n = %d, k = %lld, base = %d\n", n, k, base);
    if(n == 1) return sprint(base + 1);
    if(n == 0) return ;
    per(m, n - 1, 0) {
        if(!check(m, k)) k -= (m? (1ll << (m - 1)) : 1);
        else {
            int tot = n - m + base;
            rep(i, m + 1, n) sprint(tot --);
            solve(m, k, n - m + base);
            break;
        }
    }
