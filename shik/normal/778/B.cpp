...
int n,m;
char lhs[N][L],rval[N][M];
int rt[N],rvar[N][2];
map<string,int> var_id;

int val[N];
int eval(int p, int x) {
    val[N-1]=x;
    REP(i,n) {
        if ( rt[i]==0 ) {
            val[i]=rval[i][p]-'0';
        } else if ( rt[i]==1 ) {
            val[i]=val[rvar[i][0]]&val[rvar[i][1]];
        } else if ( rt[i]==2 ) {
            val[i]=val[rvar[i][0]]|val[rvar[i][1]];
        } else if ( rt[i]==3 ) {
            val[i]=val[rvar[i][0]]^val[rvar[i][1]];
...
int main() {
    ...
    REP(i,n) {
        ...
        if ( isdigit(buf[0]) ) {
            ...
            rvar[i][0]=var_id.at(buf);
            R(buf);
            if ( strcmp(buf,"AND")==0 ) rt[i]=1;
            else if ( strcmp(buf,"OR")==0 ) rt[i]=2;
            else if ( strcmp(buf,"XOR")==0 ) rt[i]=3;
            else assert(0);
            R(buf);
            rvar[i][1]=var_id.at(buf);
        }
    }
    string smx="",smi="";
    REP(i,m) {
        int s[2];
        REP(j,2) s[j]=eval(i,j);
        if ( s[0]>s[1] ) {
            smx+="0";
            smi+="1";
        } else if ( s[0]<s[1] ) {
