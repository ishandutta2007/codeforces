...
signed main(){
    ...
    ForE(i, 1, n){
        if (isz(a[i]) == 1){
            dsu.ck[dsu.root(a[i][0])] = 1;
        }
    }
    ForE(i, 1, m){
        if (dsu.par[i] < 0){
            if (dsu.ck[i]){
                t = t * binpow(2, -dsu.par[i]) % mod;
            }
            else{
                t = t * binpow(2, -dsu.par[i] - 1) % mod;
            }
        }
    }
    cout << t << ' ';
    ...
    ForE(i, 1, n){
        if (isz(a[i]) == 1){
            if (!dsu.ck[dsu.root(a[i][0])]){
                dsu.ck[dsu.root(a[i][0])] = 1;
                vans.push_back(i);
            }
        }
        else{
            if (dsu.root(a[i][0]) == dsu.root(a[i][1])){
                continue;
            }
            if (dsu.ck[dsu.root(a[i][0])] && dsu.ck[dsu.root(a[i][1])]){
                continue;
            }
            dsu.merge(a[i][0], a[i][1]);
            vans.push_back(i);
