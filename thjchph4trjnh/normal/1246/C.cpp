...
void solve() {
    ...
    auto calc_right = [&] (int u, int v) {
        return row[u][m - 1] - row[u][v];
    };
    auto calc_down = [&] (int u, int v) {
        return col[n - 1][v] - col[u][v];
    };
    auto updfen = [&] (vi& fen, int p, int val) {
        p++;
        for (; p < sz(fen); p += p & -p) {
            addmod(fen[p], val);
        }
    };
    auto queryfen = [&] (vi& fen, int p) {
        p++;
        int res = 0;
        for (; 0 < p; p -= p & -p) {
            addmod(res, fen[p]);
