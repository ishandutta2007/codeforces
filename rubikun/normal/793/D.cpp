...
const int mod=1000000007,MAX=85,INF=1<<28;
bool seen[MAX][MAX][MAX][2];
int dp[MAX][MAX][MAX][2];
int cost[MAX][MAX];

int solve(int l,int r,int rem,int which){
    if(seen[l][r][rem][which]) return dp[l][r][rem][which];
    
    seen[l][r][rem][which]=1;
    if(rem==0) return dp[l][r][rem][which]=0;
    ...
    for(int k=l+1;k<r;k++){
        if(!which){
            if(cost[l][k]){
                chmin(res,cost[l][k]+solve(k,r,rem-1,0));
                chmin(res,cost[l][k]+solve(l,k,rem-1,1));
            }
        }else{
            if(cost[r][k]){
                chmin(res,cost[r][k]+solve(l,k,rem-1,1));
                chmin(res,cost[r][k]+solve(k,r,rem-1,0));
            }
        }
    }
    
    return dp[l][r][rem][which]=res;
}
...
int main(){
    ...
    for(int i=0;i<MAX;i++) for(int j=0;j<MAX;j++) for(int k=0;k<MAX;k++) for(int l=0;l<2;l++) dp[i][j][k][l]=INF;
    ...
    for(int i=1;i<=N;i++){
        for(int j=1;j<=N;j++){
            ...
            if(i<j) chmin(ans,cost[i][j]+solve(i,j,K-2,1));
            else chmin(ans,cost[i][j]+solve(j,i,K-2,0));
            
            if(i<j) chmin(ans,cost[i][j]+solve(j,N+1,K-2,0));
            else chmin(ans,cost[i][j]+solve(0,j,K-2,1));
        }
    }
