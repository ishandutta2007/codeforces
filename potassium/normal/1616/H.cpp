...
int main() {
	...
	vector<Mint> pw2(150007, Mint(1, MOD998));
	for (int i = 1; i < 150007; i++)
		pw2[i] = pw2[i - 1] * 2;

	function<Mint(vector<int>&, vector<int>&, int)> solve2 = [&](vector<int>& a, vector<int>& b, int bit) {
		if (bit < 0 || a.size() == 0 || b.size() == 0)
			return pw2[a.size()] * pw2[b.size()];

		vector<int> a0, a1, b0, b1;
		for (auto& i : a)
			(i >> bit & 1 ? a1 : a0).emplace_back(i);
		for (auto& i : b)
			(i >> bit & 1 ? b1 : b0).emplace_back(i);

		if (x >> bit & 1)
			return solve2(a0, b1, bit - 1) * solve2(a1, b0, bit - 1);
		else
			return pw2[a.size()] - 1 + pw2[b.size()] - 1
					+ solve2(a0, b0, bit - 1) + solve2(a1, b1, bit - 1) - 1
					- pw2[a0.size()] + 1 - pw2[a1.size()] + 1
					- pw2[b0.size()] + 1 - pw2[b1.size()] + 1;
	};

	function<Mint(vector<int>&, int)> solve = [&](vector<int>& a, int bit) {
		if (bit < 0 || a.size() == 0)
			return pw2[a.size()];
		...
		if (x >> bit & 1)
			return solve2(a0, a1, bit - 1);
		else
			return solve(a0, bit - 1) + solve(a1, bit - 1) - 1;
	};
