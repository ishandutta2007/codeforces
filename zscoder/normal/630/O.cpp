...
int main()
{
	...
	//ld vec = ld(1)/ld(sqrt(vx*vx + vy*vy));
	//cout << fixed << setprecision(20) << ld(1)/sqrt(vx*vx+vy*vy) << endl;
	//ld k = ld(b)/ld(sqrt(vx*vx + vy*vy));
	ld x1, y1;
	x1 = px + ld(b*vx)/ld(sqrt(vx*vx + vy*vy));
	y1 = py + ld(b*vy)/ld(sqrt(vx*vx + vy*vy));
	ld x2, y2, x3, y3, x6, y6, x7, y7;
	//ld l = a/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	//ld q = (a - c)/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	...
	/*x8 = px - ld(d)/ld(sqrt(vx*vx + vy*vy));
	y8 = py - ld(d)/ld(sqrt(vx*vx + vy*vy));
	cout << x8 << " " << y8 << endl;
	x4 = x8 - ld(c*vy)/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	y4 = y8 + ld(c*vx)/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	x5 = x8 + ld(c*vy)/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	y5 = y8 - ld(c*vx)/(ld(2)*ld(sqrt(vx*vx + vy*vy)));
	*/
	//cout << fixed << setprecision(20) << sqrt((x2-x7)*(x2-x7) + (y2-y7)*(y2-y7)) << endl;
	//cout << fixed << setprecision(20) << sqrt((x5-x4)*(x5-x4) + (y5-y4)*(y5-y4)) << endl;
