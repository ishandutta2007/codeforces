...
const int MN=6e2+5,lim=23;
mt19937 rnd(chrono::steady_clock::now().time_since_epoch().count());
int Rnd(int l,int r) { return rnd()%(r-l+1)+l; }
int n,m,mat[MN],vis[MN],tot,p[MN];
vector<int>e[MN];
int ext(int u) {
	if(vis[u]) return 0; vis[u]=1;
	shuffle(e[u].begin(),e[u].end(),rnd);
	for(auto v:e[u]) {
		if(!mat[v]) return mat[u]=v,mat[v]=u,1;
		else {
			int w=mat[v];
			mat[u]=v,mat[v]=u,mat[w]=0;
			if(ext(w)) return 1; 
			mat[u]=0,mat[w]=v,mat[v]=w;
		}
	}
	return 0; 
}
int solve(int n) {
	For(i,1,n) p[i]=i,mat[i]=vis[i]=0;  
	int ans=0;
	For(x,1,20) {
		shuffle(p+1,p+1+n,rnd); 
		For(i,1,n) {
			int u=p[i]; if(!mat[u]) mst(vis,0),ans+=ext(u);
		}
	}
	return ans; 
}
void works() {
	n=read(),m=read();
	For(i,1,2*n+m) e[i].clear(); 
	
	For(i,1,n) {
		e[i*2].pb(i*2-1),e[i*2-1].pb(i*2);
		char s[305]; FI(s); 
		For(j,0,m-1) {
			if(s[j]=='1') {
				e[i*2-1].pb(n*2+j+1),e[n*2+j+1].pb(i*2-1);	
				e[i*2].pb(n*2+j+1),e[n*2+j+1].pb(i*2);	
			}
		}
		
	}
	int tn=n,ans=0;	n=2*n+m; 
	ckmax(ans,solve(n));
	printf("%d\n",ans-tn);
