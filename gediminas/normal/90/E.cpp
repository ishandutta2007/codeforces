...
/*link https://codeforces.com/problemset/problem/90/E */
/*input
4 4
DRLD
U.UL
...
struct UnionFind {
	...
	int getMi(const int &x) {
		return mi[getParent(x)];
	}
	int getMa(const int &x) {
		return ma[getParent(x)];
	}
	int getParent(int x) {
		while (par[x] != x) {
			par[x] = par[par[x]];
	...
	bool merge(int x, int y) {
		if (x < 0 or y < 0 or x >= par.size() || y >= par.size())
			return false;
...
int main () {
	...
	vector<UnionFind> stul(m);
	vector<UnionFind> eil(n);
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (a[i][j] != '.') {
				...
				while (dab.x >= 0 and dab.y >= 0 and dab.x < n and dab.y < m) {
					if (b[dab.x][dab.y] == '.') {
						if (kryp.y > 0) {
							if (dab.y != eil[dab.x].getMa(dab.y)) {
								dab.y = eil[dab.x].getMa(dab.y);
							}

							eil[dab.x].merge(dab.y, dab.y + 1);
							dab += kryp;
						}
						else if (kryp.y < 0) {
							if (dab.y != eil[dab.x].getMi(dab.y)) {
								dab.y = eil[dab.x].getMi(dab.y);
							}
							...
							if (dab.x != stul[dab.y].getMa(dab.x)) {
								dab.x = stul[dab.y].getMa(dab.x);
							}

							stul[dab.y].merge(dab.x, dab.x + 1);
							dab += kryp;
						}
						else if (kryp.x < 0) {
							if (dab.x != stul[dab.y].getMi(dab.x)) {
								dab.x = stul[dab.y].getMi(dab.x);
							}
