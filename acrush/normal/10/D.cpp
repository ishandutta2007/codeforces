...
int n,A[maxn],ID[maxn],m,B[maxn];
int next[maxn][maxn];
int f[maxn][maxn];
int S[maxn];

void addS(int i,int p)
{
	for (;i<=n;i=(i|(i-1))+1) if (p<S[i]) S[i]=p;
}
int getS(int i)
{
	int p=m;
	for (;i>0;i&=(i-1)) if (S[i]<p) p=S[i];
	return p;
}
int main()
{
	...
	vector<int> MA(M.begin(),M.end());
	map<int,int> SA;
	for (int i=0;i<SIZE(MA);i++) SA[MA[i]]=i;
	for (int i=0;i<n;i++) ID[i]=SA[A[i]]+1;
	for (int i=0;i<n;i++)
	{
		next[i][m]=m;
		for (int j=m-1;j>=0;j--) next[i][j]=(A[i]==B[j])?j:next[i][j+1];
	}
	int L=1;
	for (int i=0;i<n;i++) f[1][i]=next[i][0];
	while (1)
	{
		bool exists=false;
		for (int i=0;i<n;i++) if (f[L][i]<m) exists=true;
		if (!exists) break;
		L++;
		...
		{
			f[L][i]=getS(ID[i]-1);
			if (f[L-1][i]<m) addS(ID[i],f[L-1][i]);
		}
		for (int i=0;i<n;i++) f[L][i]=next[i][f[L][i]];
	}
	L--;
	printf("%d\n",L);
	if (L==0) 
	...
	for (int i=0;i<n;i++)
		if (f[L][i]<m)
		{
			...
			while (1)
			{
				...
				if (L==1) break;
				for (int k=0;k<i;k++)
					if (A[k]<A[i] && next[i][f[L-1][k]]<=f[L][i])
					{
						i=k;
						L--;
						break;
