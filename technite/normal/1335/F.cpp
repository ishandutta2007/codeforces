...
int mv_x[]={0,0,1,-1};
int mv_y[]={1,-1,0,0};
map<char,int> mapy={{'R',0},{'U',3},{'D',2},{'L',1}}; 
//
...
void Testcase()
{
    ...
    for(int i=0;i<n*m;i++)
    {
        dp[i][0]=i+mv_x[mapy[s[(i-i%m)/m][i%m]]]*m+mv_y[mapy[s[(i-i%m)/m][i%m]]];
    }
    for(long long i=1;(1<<i)<=n*m;i++)
    {
        for(int j=0;j<n*m;j++)
        {
            if(dp[j][i-1]!=-1)
    ...
    for(int i=0;i<n*m;i++)
    {
        ...
        for(auto node:aa[i])
        {
            ...
            if(flag1){count1++;flag1=false;}
            if(col[(node-node%m)/m][node%m]=='0') {count2++;ans[node]=1;flag=true;break;}
        }
        if(!flag&&aa[i].size())
        {
            ans[aa[i][0]]=1;
