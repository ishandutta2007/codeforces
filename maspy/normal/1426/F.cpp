...
void solve() {
  auto N = in();
  auto S = sin();
  auto cnt = [&](string T) -> mint {
    mint x0 = 1, x1 = 0, x2 = 0, x3 = 0;
    FORIN(s, S) {
      if (s == T[2]) x3 += x2;
      if (s == T[1]) x2 += x1;
      if (s == T[0]) x1 += x0;
  ...
  mint ANS = 0;
  ANS += POW[k] * ooo;
  if (k >= 1) ANS += POW[k - 1] * (oox + oxo + xoo);
  if (k >= 2) ANS += POW[k - 2] * (oxx + xox + xxo);
  if (k >= 3) ANS += POW[k - 3] * xxx;
  print(ANS);
}
