...
int main()
{
	...
	sort(all(a));
	c[2*n]=2*a[n-1].second;
	c[2*n-a[n-1].first]=2*a[n-1].second-1;
	int now=2*n-a[n-1].first;
	int t=2*n;
	for(i=n-2;i>=0;i--){
		while(c[t])
			t--;
		if(t<now)
			break;
		c[t]=2*a[i].second;
		if(c[t-a[i].first]||t-a[i].first>=now)
			ans.pb({t-a[i].first+1,2*a[i].second-1});
		else{
			c[t-a[i].first]=2*a[i].second-1;
		}
	}
	for(;i>=0;i--){
		ans.pb({2*n,2*a[i].second});
		c[2*n+1]=2*a[i].second;
		int x=2*n+1-a[i].first+1;
		ans.pb({x,2*a[i].second-1});
	}
	for(i=2*n;c[i-1];i--)
		printf("%d %d\n",c[i-1],c[i]);
	for(auto x:ans){
