...
struct MinMax{
	...
	MinMax operator+(const MinMax &seg){
		return MinMax(max(mx,seg.mx));
	}
  static MinMax e;
};
template<class T> MinMax<T> MinMax<T>::e=MinMax();
template<class T>ostream&  operator<<(ostream &os, const MinMax<T> &seg){ return os<<"("<<seg.mx<<")";}
...
struct Add{
  ...
  Add(T ad=0):ad(ad){}
	...
	void eval(MinMax<pii> &s,int l,int r){
		s.mx.X+=ad;
	}
	static Add e;
};
...
int main(){
	...
	cauto g=readG(n);
	int leaf=0;
	reps(i,1,n)if(g[i].size()==1) ++leaf;
	if(leaf<=t){
		ll re=0;
		reps(r,leaf,t+1) MX(re,(n-r)*r);
		cout<<re NL;
		return 0;
	}
	vector<int> et,lb(n),rb(n),ps(n);
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p)->void{
				...
				lb[v]=et.size();
				et.eb(v);
				for(cauto &u:g[v])if(u!=p) dfs(dfs,u,v);
				rb[v]=et.size();
			};
		dfs(dfs,0,-1);
	}
	vector<pii> init(n); rep(i,n) init[i].Y=et[i];
	LazySegTree<MinMax<pii>,Add<int>> st(init);
	out(st(0,1),st(),1);
	rep(v,n) st(lb[v],rb[v],1);
	vector<int> ex(n,1);
	rep(_,t){
		cauto v=st().mx.Y;
		for(int i=v;i>=0;i=ps[i]){
			if(!ex[i]) break;
			ex[i]=0;
			st(lb[i],rb[i],-1);
		}
	...
	out(ex,1);
	cauto B=(ll)count(all(ex),1);
	cauto r=t;
	ll re=1e18;
	rep(b,B+1) MN(re,(r-b)*(n-r-b));
	cout<<re NL;
	return 0;
}