...
void solve() {
    cin >> n >> k;
    f.rsz(2*k+1, 1);
    FOR(i, 1, 2*k+1) f[i] = f[i-1] * (n-i+1);
    invf.rsz(2*k+1, 1);
    FOR(i, 1, 2*k+1) {
        if (f[i]==0) invf[i]=1;
        else invf[i] = mi(1) / f[i];
    }
    inverse.rsz(k+2, 1);
    FOR(i, 1, k+2) inverse[i] = mi(1) / i;
    debug(f);
    dp.rsz(k+1, vm(2*k+1, 0));
    dp[0][0] = 1;
    FOR(d, 2, k+2) {
        vvm nxt = dp;
        F0R(i, k+1) F0R(j, 2*k+1) if (dp[i][j] != 0) {
            mi mult = 1;
            for (int m = 1; i+m*(d-1)<=k; m++) {
                mult *= f[j+m*d] * invf[j+(m-1)*d] * inverse[d] * inverse[m];
                nxt[i+m*(d-1)][j+m*d] += dp[i][j] * mult;
            }
        }
        dp = nxt;
    }
    vm ans(k+1, 0);
    F0R(i, k+1) F0R(j, 2*k+1) {
        ans[i] += dp[i][j];
    }
    FOR(i, 2, k+1) ans[i] += ans[i-2];
