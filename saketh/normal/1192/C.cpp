...
int nperm(int a, int b, int c, int d) {
    int res = 24;
    if (b != c) {
        if (a == b) res >>= 1;
        if (c == d) res >>= 1;
...
const int CC = 62;
mn solve(set<string>& words) {
    vvmn ch(CC, vmn(CC, 0));
    trav (s, words) ch[ctoi(s.front())][ctoi(s.back())] += 1;

    vector<vvmn> p3(CC, vvmn(CC, vmn(CC, 0)));
    F0R (a, CC) FOR (b, a, CC) FOR (c, b, CC) F0R (z, CC)
        p3[a][b][c] += ch[z][a] * ch[z][b] * ch[z][c];

    mn ret = 0;
    F0R (a, CC) FOR (b, a, CC) FOR (c, b, CC) FOR (d, c, CC) {
        ret += p3[a][b][c] * p3[a][b][d] * p3[a][c][d] * p3[b][c][d] * nperm(a, b, c, d);
    }
    return ret;
}
...
int main() {
    ...
    F0R (i, N) {
        string str; re(str);
        words[sz(str)].insert(str);
        reverse(all(str));
        words[sz(str)].insert(str);
    }

    mn ans = 0;
    FOR (l, 3, 11) ans += solve(words[l]);
    ps(ans);
