...
void solve() {
    ...
    auto [SCC, scc] = strongly_connected_components(like);
    if (SCC == 1) { ps("No"); return; }

    F0R (i, N) trav (j, like[i]) assert(scc[j] <= scc[i]);

    vi cats, ppl;
    F0R (i, N) if (scc[i] == SCC - 1) cats.pb(i + 1);
    else ppl.pb(i + 1);

    ps("Yes"), ps(sz(ppl), sz(cats)), pc(ppl), pc(cats);
}
