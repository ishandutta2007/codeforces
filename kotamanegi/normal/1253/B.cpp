...
void solve() {
    ...
    REP(i, n) {
        if (inputs[i] > 0) {
            if (already.count(inputs[i]) or gekos.count(inputs[i])) {
                cout << -1 << endl;
                return;
            }
            gekos.insert(inputs[i]);
        }
        else {
            if (gekos.count(-inputs[i]) == 0) {
                cout << -1 << endl;
                return;
            }
            already.insert(-inputs[i]);
            gekos.erase(-inputs[i]);
            if (gekos.size() == 0) {
                ans.push_back(already.size() * 2);
                already.clear();
            }
        }
    }
