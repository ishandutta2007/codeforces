...
signed main(){
    ...
    while(t--){
        ...
        sort(all(s));
        vec cnt(26);
        rep(i,26)cnt[i] = count(all(s),'a'+i);
        bool flag = true;
        vec c;
        rep(i,26)if(cnt[i])c.pb(cnt[i]);
        if(c.size()==2 and c[0]==k){
            rep(i,26)rep(j,(k-1+cnt[i])/k)cout<<(char)('a'+i);
            cout<<endl;
        }
        else if(s==string(n,s[0])){
            rep(i,(n+k-1)/k)cout<<s[0];
            cout<<endl;
        }
        else if(s[k-1]==s[0]){
            for(auto it=s.begin()+k-1;it!=s.end();it++){
                cout<<*it;
            }
            cout<<endl;
        }
