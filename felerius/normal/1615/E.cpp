// Purple Crayon (https://codeforces.com/contest/1615/problem/E)
...
int main() {
    ...
    auto dfs = [&](int v, auto&& self) -> void {
        trav(v2, adj[v])
            if (v2 != p[v])
                d[v2] = d[v] + 1, p[v2] = v, self(v2, self);
    };
    dfs(0, dfs);
    ...
    trav(v, idx) {
        int v2 = v;
        while (v2 != -1 && !mark[v2])
            mark[v2] = true, ++block[v], v2 = p[v2];
    }
    sort(all(block), greater<>{});

    int blocked = 0;
    auto eval = [&](int r) {
        int b = min(n - blocked, n / 2);
        return r * ll(n - r) - b * ll(n - b);
    };
    ll mx = eval(0);
    rep(i, k)
        blocked += block[i], mx = max(mx, eval(i + 1));
