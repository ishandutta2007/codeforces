...
bool isWin(vector<int>& x, vector<int>& y) {
	int win = 0;
	for (int i = 0; i < 5; i++)
		if (x[i] < y[i]) win++;
	return win >= 3;
...
int main() {
	CASE(t) {
		int n; R(n);
		vector<vector<int>> a(n, vector<int>(5)); R(a);
		...
		while (winners.size() > 1) {
			int x = winners.back(); winners.pop_back();
			int y = winners.back(); winners.pop_back();
			if (isWin(a[x], a[y]))
				winners.emplace_back(x);
			else
				winners.emplace_back(y);
		}
