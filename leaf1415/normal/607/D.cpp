...
ll l[200005], r[200005], par[200005];
vector<P> evec;
...
int main(void)
{
	...
	rep(i, 1, Q){
		...
		if(t == 1){
			...
			n++;
			G[p].push_back(n);
			evec.push_back(P(1, n));
			a[n] = v, par[n] = p;
		}
		if(t == 2){
			cin >> v;
			evec.push_back(P(2, v));
		}
	}
	dfs(1);
	...
	seg.update(l[1], a[1]), deg[1] = 1;
	rep(i, 0, n) seg2.update(l[i], 1);
	
	//rep(i, 1, n) cout << seg2.query(l[i], l[i]) << " "; cout << endl;
	
	for(auto e : evec){
		ll v = e.second, p = par[v];
		//outl(v, p);
		if(e.first == 1){
			mint ratio = mint(deg[p]+1) / mint(deg[p]);
			deg[p]++;
			seg.add(l[p], r[p], ratio);
			seg2.add(l[p], r[p], ratio);
			seg.update(l[v], mint(a[v]) * seg2.query(l[v], l[v]));
			deg[v]++;
		}
		if(e.first == 2){
			mint div = seg2.query(l[p], l[p]);
			cout << seg.query(l[v], r[v]) / div << "\n";
		}
		//rep(i, 1, n) cout << seg.query(l[i], l[i]) << " "; cout << endl;
		//rep(i, 1, n) cout << seg2.query(l[i], l[i]) << " "; cout << endl;
	}
