...
int main() {
    ...
    for (auto rng: rngs) {
        ...
        int pre = (l ? a[l - 1] : -1);
        if (pre != -1 && !used[pre]) {
            edges.push_back({to_v0[l], pre});
        }
        int suc = (r < n ? a[r] : -1);
        if (suc != -1 && !used[suc]) {
            edges.push_back({to_v0[l] + to_sz[l] - 1, suc});
        }

        if (x == 2) {
            edges.push_back({to_v0[l], to_v0[l] + 1});
        }
    ...
    MaxMatching mt(vn);
    for (auto e : edges) {
        mt.add_edge(e.first, e.second);
    }
    auto matches = mt.max_matching();
    V<int> mate(vn, -1);
    for (auto p : matches) {
    ...
    for (auto rng : rngs) {
        const int l = rng.first, r = rng.second;
        ...
        if (x == 1) {
            ...
            // x == 2
            int m0 = mate[to_v0[l]], m1 = mate[to_v0[l] + 1];
            if (m0 == to_v0[l] + 1) {
                // do nothing
            } else {
                if (m0 != -1) {
        ...
        for (int i = s; i + 2 <= t; i += 2) {
            // match i, i + 1
            int y = *dummy.begin(); dummy.erase(y);
            a[i] = a[i + 1] = y;
        }
    }

    for (int i = 0; i < n; i++) {
        pr.write((a[i] == -1 ? 0 : a[i]) + 1, ' ');
    }
    pr.writeln();
    return 0;
}