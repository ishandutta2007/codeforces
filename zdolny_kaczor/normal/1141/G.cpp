...
vpii graf[nax];
int ans[nax];
void dfs(int num, int x, int y = 0, int c = -1) {
	debug(imie(num), imie(x), imie(y), imie(c));
	if ((int)graf[x].size() > num) {
		for (pii e : graf[x]) if (e.first != y) {
			ans[e.second] = 1;
			dfs(num, e.first, x, 1);
		}
	}
...
int main() {
	...
	REP(_, n - 1) {
		int a, b;
		scanf("%d%d", &a, &b);
		graf[a].emplace_back(b, _);
		graf[b].emplace_back(a, _);
		deg[a - 1]++;
		deg[b - 1]++;
	}
	sort(deg, deg + n);
