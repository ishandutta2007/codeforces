...
vi tin(N,0);  
vi low(N,0); //low[u] denotes the minimum tin of any node that can be reached through u by a connection of edges. 
vi visited(N,0);
...
void dfs2(int u, int p = -1)
{
    ...
    FA(v, gr[u])
    {
        ...
        if(visited[v])
        {
            if(st.count(mp(u,v))==0 && st.count(mp(v,u))==0)
            {
                st.insert(mp(u,v));
                st.insert(mp(v,u));
                cout<<u<<" "<<v<<endl;
...
void dfs(int u, int p=-1)
{
    ...
    {
        if(v==p)
            continue;    // Can't consider this edge u->v to update low[u] because 
                         // this edge will be removed when we try to consider it as bridge,     
        if(visited[v])
