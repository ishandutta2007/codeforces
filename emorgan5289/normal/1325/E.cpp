...
// works for n <= 10^18
bool isprime(ll n) {
    static vector<bool> sieve = primesieve(1000005);
...
vector<ll> primefactors(ll n) {
    static vector<ll> small = primesupto(1000005);
...
void bfs(int r) {
    ...
    vector<bool> v(100005, 0), ve(100005, 0);
    queue<int> q({r}); v[r] = 1;
    while (!q.empty()) {
        int i = q.front(); q.pop();
        debug(i);
...
int main() {
    ...
    for (ll i = 0; i < n; i++) {
        ...
        auto f = primefactors(a[i]);
        if (f.size() == 0)
            return cout << "1\n", 0;
        int u = lower_bound(all(p), f[0])-p.begin()+1;
        int v = f.size() > 1 ? lower_bound(all(p), f[1])-p.begin()+1 : 0;
        adj[u].pb({v, ++t}); adj[v].pb({u, t});
    }
    for (int i = 0; i < sqrt(p.size())+10; i++)
        bfs(i);
    cout << (ans == inf ? -1 : ans) << "\n";
}