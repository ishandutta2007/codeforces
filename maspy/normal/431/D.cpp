...
void solve() {
  ...
  auto f = [&](auto self, ll N) -> vi {
    ...
    */
    ...
    FOR(d, 2) {
      auto DP = self(self, (N - d) / 2);
      FOR(k, lim + 1) {
        ll to = k + d;
        if (to <= lim) ANS[to] += DP[k];
      }
    }
    return ANS;
  };

  auto check = [&](ll x) -> bool { return f(f, 2 * x)[K] - f(f, x)[K] >= M; };

  ll ANS = binary_search(check, 1LL << 60, 0LL);
  print(ANS);
}
