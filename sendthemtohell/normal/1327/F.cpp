...
int slv(vector<pair<pair<int, int>, int>>& v, int n) {
	vector<int> kek(n + 1);
	vector<int> end(n + 1, -1);
	int st = 0;
	for (auto &p : v) {
		if (p.y == 1) {
			for (int i = max(p.x.x, st); i < p.x.y; ++i) {
				kek[i] = 1;
			}
			st = max(p.x.y, st);
		} else {
	...
	for (int i = 1; i < sz(end); ++i) {
		end[i] = max(end[i], end[i - 1]);
	}
	vector<int> dp(n + 2); // cnt on prefix of size i
	vector<int> ps(n + 3);
	dp[0] = 1;
	ps[0] = 0;
	ps[1] = 1;
...
inline void solve() {
	...
	sort(all(v));
	for (int i = 0; i < k; ++i) {
		vector<pair<pair<int, int>, int>> vv = v;
		for (auto &pp : vv) {
			pp.y = ((pp.y >> i) & 1);
		}
		ans = ll(ans) * slv(vv, n) % FFT_MOD;
	}
	cout << ans << endl;
}
