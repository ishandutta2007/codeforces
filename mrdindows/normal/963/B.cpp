...
//int sz[202020];
//int cnt[202020];
//void go(int cur, int p = -1, int d = 1) {
//    cnt[cur]++;
//    for (int to : v[cur]) {
//        if (to != p) {
//            go(to, cur);
...
void dfs(int cur, int p = -1) {
    ...
    for (int to : v[cur]) {
        if (to == p) continue;
        int s = dp[to][0] * 2 + dp[to][1];
        cnt[s]++;
    }
//    DBN(cur, cnt[0], cnt[1], cnt[2]);
    if (cnt[3]) {
        dp[cur][0] = dp[cur][1] = true;
        return;
    }
    int vs = (int)v[cur].size() - cnt[1];
    int hp = p != -1;
    dp[cur][0] = (vs + hp) % 2 == 0;
    if (hp) {
        dp[cur][1] = vs % 2 == 0;
    }
//    DBN(cur, dp[cur][0], dp[cur][1]);
}
void restore(int cur, int p = -1, bool e = false) {
    for (int to : v[cur]) {
        if (to == p) continue;
        if (dp[to][1] == true) {
            restore(to, cur, true);
