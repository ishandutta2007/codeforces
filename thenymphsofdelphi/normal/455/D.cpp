...
int erase(int pos){
    ForE(i, 1, S){
        ...
        int ans = block[i][pos - 1];
        block[i].erase(block[i].begin() + pos - 1);
        cnt[i][ans]--;
        return ans;
    }
}
...
void insert(int pos, int val){
    ForE(i, 1, S){
        if (pos > block[i].size() + 1){
            pos -= block[i].size();
            continue;
        }
        block[i].insert(block[i].begin() + pos - 1, val);
        cnt[i][val]++;
        return;
    }
}
...
void redo(){
    ...
    ForE(i, 1, n){
        block[calblock(i)].pb(a[i - 1]);
        cnt[calblock(i)][a[i - 1]]++;
    }
}
...
signed main(){
    ...
    while (q--){
        ...
        if (que == 1){
            ...
            int l, r, k; cin >> l >> r >> k;
            decode(l); decode(r); decode(k); if (l > r) swap(l, r);
            lastans = count(r, k) - count(l - 1, k);
            cout << lastans << endl;
        }
        if (que1 == S){
            redo();
