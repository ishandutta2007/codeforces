...
vi solve0(bool& ok){
  ...
  li s = accumulate(begin(A), end(A),0ll);
  if(s == 0) {
    ok = 1;
    return vi();
  }
  ...
  auto proc=[&](int i, int j){
    for(li k = 0; k < (li)((j-i)/2); ++k) {
      ans.emplace_back(i+2*k);
    }
    for(li k = ((j-i)/2-2); k >= (li)(0); --k) {
      ans.emplace_back(i+2*k);
    }
    for(li k = (i); k <= (li)(j); ++k) A[k] = 0;
  };
  ...
  while(!pairs.empty()) {
    ...
    reverse(begin(newpairs), end(newpairs));
    pairs = move(newpairs);
  }
  ok = 1;
  return ans;
