...
struct S {
    ...
    S() : steps(0), pr(1) {}
    S(mn steps_, mn pr_) : steps(steps_), pr(pr_){};

    operator mn() { return steps / pr; }

    friend S operator*(const S &l, const S &r) { return {l.steps + l.pr * r.steps, l.pr * r.pr}; }
};
...
int main() {
    ...
    segment_tree st(n, S(), multiplies<S>());
    st.set_leaves(L1(i, p[i]));
    ...
    for (int query = 0; query < q; query++) {
        ...
        auto it = checkpoints.lower_bound(i);
        if (*it == i) {
