...
int dp[70][N*N],dp2[70][N*N][2];

#define X(s,e) (s+e*n)
int sol(int s,int e,int k, int cnt);
int sol2(int s,int e,int k,int cnt,int idx){
    if (dp2[cnt][X(s,e)][idx]!=-1) return dp2[cnt][X(s,e)][idx];
    int ans=0;
    if (idx){
        for(auto j:acc[e]) {
            ans+=sol(s,j,k,cnt);
...
int sol(int s,int e,int k,int cnt){
    ...
    if (dp[cnt][X(s,e)]!=-1) return dp[cnt][X(s,e)];
    int ans=0;
    if (k%2){
        
        For(i,n){
            int l=0,r=0;
            l=sol2(s,i,k/2,cnt+1,1);
            r=sol2(i,e,k/2,cnt+1,0);
            // for(auto j:acc[i]){
            //     l+=sol(s,j,k/2,cnt+1);
            //     if (l>=mod)l-=mod;
            //     r+=sol(j,e,k/2,cnt+1);
        ...
        For(i,n){
            ans+=(sol(s,i,k/2,cnt+1)*(sol2(i,e,k/2,cnt+1,0)))%mod;
            if (ans>=mod) ans-=mod;
            // for(auto j:acc[i]){
            //     ans+=(sol(s,i,k/2,cnt+1)*sol(j,e,k/2,cnt+1))%mod;
            //     if (ans>=mod)ans-=mod;
            // }
        }
    }
...
void solve(){
    ...
    For(i,n){
        ...
        For(j,i){
            ...
            val[i][j]=val[j][i]=pos(arr[i]^arr[j]);
            if (val[i][j]){
                acc[i].pb(j);
                acc[j].pb(i);
            }
