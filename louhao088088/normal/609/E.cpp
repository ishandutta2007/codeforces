// Problem: CF609E Minimum spanning tree for each edge
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/CF609E
// Memory Limit: 250 MB
// Time Limit: 2000 ms
// 2021-12-14 20:44:43
// Author : louhao088
...
int n,m,x,y,z,fa[maxn],f[maxn][22],s[maxn][22],d[maxn],flag[maxn],ans[maxn],sum;
struct node
{
	int x,y,z,id;
}a[maxn];
...
signed main()
{
	...
	for(int i=1;i<=m;i++)
	{
		...
		fa[getf(a[i].x)]=getf(a[i].y);sum+=a[i].z;
	}
	dfs(1,0);
	for(int i=1;i<=m;i++)
	{
		if(flag[i])ans[a[i].id]=sum;
		else ans[a[i].id]=a[i].z-Lca(a[i].x,a[i].y)+sum;
	}
