...
bool go() {
  int n=rint();
  map<pii,pii,greater<pii> > h;
  for (int k=0;k<2;k++) {
    for (int i=0;i<n;i++) c[k][i]=0;
    for (int i=0;i<n;i++) {
      int x=rint()-1; a[k][i]=MP(x,c[k][x]++);
      //printf("x:%d i:%d\n",x,i);
      if (k==0) h[a[k][i]].fst=i; else h[a[k][i]].snd=i;
    }
  }
  if (h.size()!=n) return false;

  seg.segn=n;
  for (int i=0;i<2*n;i++) seg.t[i]=seg.identity;
  for (auto &w:h) {
    int atop=w.snd.fst,abot=w.snd.snd;
    int bbot=seg.query(atop+1,n-1);
    //printf("w: (%d,%d) atop:%d abot:%d bbot:%d\n",w.fst.fst,w.fst.snd,atop,abot,bbot);
    if (bbot<abot) return false;
    seg.sset(atop,abot);
  }
  return true;
}
int main() 
{
  ...
  int T=rint(); for (int t=0;t<T;t++) {bool ans=go();
    if (ans) printf("YES\n");
    else printf("NO\n");
  }
}
