...
signed main(){
    ...
    // rep(i, 1, n) print(a[i], b[i]);

    rep(i, 1, m) v[t[i] - 97].push_back(i);
    rep(i, 1, n) {
        if(v[s[i] - 97].empty()) return print("No"), 0;
        auto it = std :: upper_bound(v[s[i] - 97].begin(), v[s[i] - 97].end(), a[i]);
        a[i] = (it == v[s[i] - 97].begin()? 0 : *(-- it));
    }
    per(i, n, 1) {
        auto it = std :: lower_bound(v[s[i] - 97].begin(), v[s[i] - 97].end(), b[i]);
        b[i] = (it == v[s[i] - 97].end()? 0 : (m - *it + 1));
    }
    // rep(i, 1, n) print(a[i], b[i]);
    rep(i, 1, n) if(a[i] + b[i] - 1 < m) return print("No"), 0;
    print("Yes");
}