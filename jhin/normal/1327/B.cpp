#include <bits/stdc++.h>
        ...
        string s1,s2,ss[5000];
            ...
            int main()
            {
                ...
                while(t--)
                {
                    cin>>n;set<int>myset;yes=true;set<int>notused;
                    for(int i=1;i<=n;i++)notused.insert(i);
                    for(int i=0;i<n;i++)
                    {
                        cin>>x;vector<int>tmp;
                        for(int j=0;j<x;j++)
                        {
                            cin>>y;
                            auto it=myset.lower_bound(y);
                            if(*it!=y)
                            {tmp.push_back(y);
                            }

                        }
                        if(tmp.empty()){z=i+1;yes=false;}
                        else{
                        myset.insert((*tmp.begin()));
                        auto it2=notused.lower_bound(*tmp.begin());
                            notused.erase(it2);}
