...
int main() {
	...
	function<pair<pii, ll>(int, int)> dfs = [&](int u, int p) {
		int max = 0, smax = 0;
		ll sum = 0;
		for (auto& v : g[u]) {
			if (v == p) continue;
		...
		return make_pair(make_pair(max, smax), sum);
	};
	int root = max_element(h.begin(), h.end()) - h.begin();
	auto res = dfs(root, -1);

	ll ans = res.second;
	if (res.first.first < h[root]) {
		ans -= res.first.first;
		ans += h[root];
	}
	if (res.first.second < h[root]) {
