...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/mod/comb_table.hpp"
...
void solve() {
  ...
  VEC(ll, A, N);
  ll LIM = SUM(A);
  auto C = comb_table<i128>(N + 1, N + 1);

  vv(i128, DP, N + 1, LIM + 1);
  DP[0][0] = 1;
  for (auto&& a: A) {
    FOR_R(n, N) FOR(x, LIM - a + 1) DP[n + 1][x + a] += DP[n][x];
  }
  map<ll, ll> CNT;
  for (auto&& a: A) CNT[a]++;
  if (len(CNT) <= 2) return print(N);
  ll ANS = 0;

  for (auto&& [k, v]: CNT) {
    FOR(n, 1, v + 1) {
      ll w = k * n;
      if (DP[n][w] == C[v][n]) { chmax(ANS, n); }
    }
  }
  print(ANS);
}
