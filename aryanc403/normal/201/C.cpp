...
// cin>>T;while(T--)
{
    ...
    repD(i,n,1)
    {
        ...
        {
            if(x>2)
                dp[j][i]=x-1+dp[j][i+1-j];
            dp[j+1][i]=max(x+dp[j][i+1-j],x+dp[j+1][i+1-j]);
        }
        else if(x>0)
        {
            dp[j][i]=x+dp[j][i+1-j];
            dp[j+1][i]=max(x+dp[j][i+1-j],x-1+dp[j+1][i+1-j]);
        }
        dbg(x,dp[j][i],dp[j+1][i]);
    }
    ...
    repA(i,1,n)
    {
        ans=max(ans,dp[0][i]+dp[2][i]);
        ans=max(ans,dp[0][i]+dp[3][i]);
