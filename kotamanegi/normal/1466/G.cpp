...
// from https://kopricky.github.io/code/String/kmp.html
class KMP
{
public:
	string pattern;
	...
	void search(const string& text, vector<int>& res) {
		...
		while (head + j < tlen) {
			...
			head += j - table[j], j = max(table[j], 0LL);
		}
	}
};
mint mae[200000];
...
void solve() {
	...
	gyaku[0] = 1;
	mae[1] = 2;
	gyaku[1] = mint(1) / mint(2);
	for (int q = 2; q < 200000; ++q) {
		mae[q] = mae[q - 1] * 2;
		gyaku[q] = gyaku[q - 1] * gyaku[1];
	}
	int n, query;
	cin >> n >> query;
	vector<string> S(1);
	...
	REP(i, t.length()) {
		...
		TMP[t[i] - 'a'][i] += mae[150000 - i];
	}
	for (int i = 1; i < min(100LL,(int)t.length()+1); ++i) {
		S.push_back(S[i - 1]);
		if (S[i - 1].length() <= 1e6) {
			S[i] += t[i - 1];
			S[i] += S[i - 1];
	...
	REP(i, query) {
		...
		string b;
		cin >> a >> b;
		if (b.length() > S.back().length()) {
			cout << 0 << endl;
			continue;
		}
		int itr = 0;
		while (S[itr].length() < b.length()) {
			itr++;
		}
		if (itr > a) {
			cout << 0 << endl;
		...
		if (itr == a) {
			KMP hoge(b);
			vector<int> ans;
			hoge.search(S[itr], ans);
			cout << ans.size() << endl;
			continue;
		}
		string Check = S[itr] + "a" + S[itr];
		int Checker[26] = {};
		int Base = 0;
		KMP hoge(b);
		for (int i = 0; i < 26; ++i) {
			Check[Check.length() / 2] = i + 'a';
			vector<int> ans;
			hoge.search(Check, ans);
			for (int q = 0; q < ans.size(); ++q) {
				if (ans[q] + b.length() <= S[itr].size() or ans[q] > S[itr].length()) {
					//outbound
					if (i == 0) Base++;
				}
				else {
		...
		mint ANS = Base * mae[a - itr - 1].val();
		REP(q, 26) {
			mint hoge = TMP[q][a - 1];
			if (itr != 0) {
				hoge -= TMP[q][itr - 1];
			}
			hoge *= gyaku[150000 - (a - 1)];
			hoge *= Checker[q];
			ANS += hoge;
		}
		cout << ANS.val()<< endl;
