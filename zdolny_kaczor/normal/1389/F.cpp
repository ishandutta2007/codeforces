...
struct Tree {
	...
	int read(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
		if (l >= rb || r <= lb) return numeric_limits<int>::min();
...
void solve() {
	...
	REP(i, n) m[l[i]], m[r[i]];
	int cou = 0;
	for (auto &[_, x] : m) x = cou++;
	REP(i, n) {
		l[i] = m[l[i]];
		r[i] = m[r[i]];
		back[r[i]].emplace_back(l[i], t[i]);
	}
	int ans = 0;
	REP(i, cou) {
		debug(arr(back, i));
		for (auto [ll, tt] : back[i])
			tree[tt].inc(0, ll + 1, 1);
		int re = 0;
		REP(wh, 2) maxi(re, tree[wh].read(0, i + 1));
		maxi(ans, re);
		REP(wh, 2)
			tree[wh].inc(i, i + 1, re);
	}
	printf("%d\n", ans);
}
int main() {
