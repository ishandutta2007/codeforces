...
zet_p fac[nax], fin[nax];
zet_p cho(int a, int b) {
	if (b < 0 || b > a) return 0;
	return fac[a] * fin[b] * fin[a - b];
}
int main() {
	fac[0] = 1;
	int n;
	...
	fin[n] = 1 / fac[n];
	for (int i = n - 1; i >= 0; --i) fin[i] = fin[i + 1] * (i + 1);
	zet_p ans = 0;
	ll sum = 0;
	zet_p c_sum = 1;
	int up_to = n;
	zet_p pow2 = 1;
	zet_p half = zet_p(1) / zet_p(2);
	for (int i = 1; i <= n; ++i) {
		pow2 *= half;
		int x;
		scanf("%d", &x);
		...
		c_sum -= cho(i - 1, up_to);
		ll bound = t - sum;
		while (up_to > bound && up_to >= 0) {
			zet_p dec = cho(i, up_to);
			debug(imie(c_sum), imie(dec), imie(up_to));
			c_sum -= dec;
			up_to--;
		}
		debug(imie(pow2) * imie(c_sum), imie(i), imie(up_to));
		ans += c_sum * pow2;
	}
	debug(imie(ans));
	printf("%d\n", ans.get());
