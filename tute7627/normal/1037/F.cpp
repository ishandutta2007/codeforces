...
//using modint=ld;
template<class T>
vector<T> slidemax(vector<T>x, ll len){
  ll n=x.size();
...
int main(){
  ...
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  auto mx=slidemax(a,k);
  modint ret=0;
  rep(i,0,k-1){
    vector<ll>v;
    for(ll j=i;j<mx.size();j+=k-1){
      v.PB(mx[j]);
    }
    ll sz=v.size();
    set<ll>st;
    vector<ll>ord(sz);
    rep(i,-1,sz+1)st.insert(i);
    iota(ALL(ord),0);
    sort(ALL(ord),[&](ll x,ll y){return v[x]<v[y];});
    for(auto i:ord){
      ll l=*(--st.find(i));
      ll r=*(++st.find(i));
      ret+=modint(i-l)*(r-i)*v[i];
      st.erase(i);
    }
    //debug(v,sz);
    //cout<<ret<<endl;
