...
struct Sub{
	...
	SA sa;
	seglazy<N> seg;
	segtree<MinNode> lcps;
	Sub(string ss):s(ss),n(si(s)),sa(s),seg(vi(n)),lcps(sa.lcp){}
	void upd(int i,int len){
		int j=sa.as[i];
		int l=lcps.min_left(j,&MinNode::ok,len).a;
		int r=min(lcps.max_right(j,&MinNode::ok,len).a,n-1);
		seg.ch(l,r+1,&N::upd,len);
	}
	int get(int i){
		int j=sa.as[i];
		return seg.point_get(j).mx;
	}
};
...
void slv(){
	...
	per(i,n+1){
		...
		if(i<n){
			chmax(dp[i],a.get(i)+1);
			if(i-dp[i]-1>=0)x[i-dp[i]-1].eb(i,dp[i]);
			b.upd(n-(i+dp[i]),dp[i]);
		}
		if(i>0){
			chmax(eq[i],b.get(n-i)+1);
			chmin(eq[i],i);
			chmax(dp[i-eq[i]],eq[i]);
			y[i-eq[i]].eb(i,eq[i]);
			if(i-eq[i]*2-1>=0)x[i-eq[i]*2-1].eb(i-eq[i],eq[i]);
		}
	}
	print(max(*max_element(all(dp)),*max_element(all(eq))));
}
