...
C f(C x, C y) { return x + y; }
C e() { return C(); }
struct S {
    C a, b;
};
C g(S f, C x) { return f.a * x + f.b; }
S h(S f, S g) { return S{g.a * f.a, g.b * f.a + f.b}; }
S id() { return S{C(1, 0), C()}; }

int main() {
    INT(n, m);
    vector<C> setter(n + 1);
    rep(i, n + 1) setter[i] = C(i, 0);
    lazy_segtree<C, f, e, S, g, h, id> seg(setter);
    rep(m) {
        INT(t, i, x);
        i--;
        if(t == 1) {
            auto l = seg[i], r = seg[i + 1];
            auto c = (r - l) / abs(r - l) * C(x, 0);
            seg.apply(i + 1, n + 1, S{C(1, 0), c});
        } else {
            auto c = seg[i];
            double theta = -1.0 * x / 180 * acos(-1);
            auto e = C(cosl(theta), sinl(theta));
            seg.apply(i + 1, n + 1, S{e, c * (C(1, 0) - e)});
        }
        auto res = seg[n];
