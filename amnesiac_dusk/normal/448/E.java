...
public class Main
{
	...
	static Map<Long,ArrayList<Long>>dp;
	static void concater(long a,long k)
	{
		if(ans.size()>1e5)
		{
			for(int i=0; i<1e5; i++)
	...
	public static void main(String[] args) throws Exception
	{
		...
		dp=new HashMap<Long,ArrayList<Long>>();
		for(long i=1; i*i<=x; i++)
			if(x%i==0)
			{
				dp.put(i, new ArrayList<Long>());
				dp.put(x/i, new ArrayList<Long>());
			}
		for(Map.Entry<Long,ArrayList<Long>> te:dp.entrySet())
		{
			long i=1;
			for(i=1; i*i<te.getKey(); i++)
				if(te.getKey()%i==0)
				{
					te.getValue().add(i);
					te.getValue().add(te.getKey()/i);
				}
			if(i*i==te.getKey())
				te.getValue().add(i);
			Collections.sort(te.getValue());
		}
		concater(x,k);
		for(int i=0; i<ans.size()&&i<1e5; i++)
			p(ans.get(i)+" ");
