	...
	while(T--){
		...
		while(sz(vec) > 1){
			...
			for(int i = 0; i+1 < sz(vec); i+=2){
				nvec.push_back(calc(vec[i], vec[i+1]));
			}
			if(sz(vec) % 2) nvec.push_back(vec.back());
			vec = nvec;
		}
