...
void solve() {
	...
	REP(i, n) {
		...
		inputs.push_back({});
		REP(q, s.length()) {
			inputs.back().push_back(s[q] - '0');
		}
	}
	vector<vector<int>> geko;
	for (int i = 0; i < n - 2; ++i) {
		REP(q, m - 1) {
			if (inputs[i][q] == 1 or inputs[i][q + 1] == 1) {
				//add it
				vector<int> hoge;
				if (inputs[i][q] == 1) {
					hoge.push_back(i);
	...
	for (int i = n - 2; i <= n - 2; ++i) {
		for (int q = 0; q < m - 1; ++q) {
			if (inputs[i][q] == 1 or inputs[i + 1][q] == 1) {
				vector<int> hoge;
				if (inputs[i][q] == 1) {
					hoge.push_back(i);
					hoge.push_back(q);
	...
	if (flag == 1) {
		geko.push_back({ n - 1,m - 1,n - 1,m - 2,n - 2,m - 2 });
	}
	if (flag == 2) {
		geko.push_back({ n - 2,m - 1,n - 1,m - 2,n - 2,m - 2 });
	}
	if (flag == 1 or flag == 2) {
		geko.push_back({ n - 1,m - 2,n - 1,m - 1,n - 2,m - 1 });
		geko.push_back({ n - 1,m - 1,n - 2,m - 1,n - 2,m - 2 });
	}
	if (flag == 3) {
		geko.push_back({ n - 1,m - 1,n - 1,m - 2,n - 2,m - 2 });
		geko.push_back({ n - 1,m - 2,n - 2,m - 2,n - 2,m - 1 });
	}
	cout << geko.size() << endl;
	REP(i, geko.size()) {
		REP(q, geko[i].size()) {
