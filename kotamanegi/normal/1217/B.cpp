...
int main() {
	...
	REP(i, test_case) {
		long long n, x;
		cin >> n >> x;
		long long now_ans = LONG_INF;
		long long max_blow = 0;
		long long max_diff_blow = -1;
		REP(i, n) {
			long long a, b;
			cin >> a >> b;
			max_blow = max(max_blow, a);
			max_diff_blow = max(max_diff_blow, a - b);
		}
		if (max_blow >= x) {
			now_ans = 1;
		}
			...
			if (max_diff_blow <= 0) {
				...
				long long geko = x - max_blow;
				now_ans = (geko + max_diff_blow - 1) / max_diff_blow + 1;
			}
		}
		if (now_ans == LONG_INF) {
			now_ans = -1;
