...
int main()
{
    ...
    ll n;
    cin >> n;
    vector<vector<ll>> d(n,vector<ll>(n,n));
    ...
    vector<vector<ll>> dp(n+1,vector<ll>(n+1)), dp2(n+1,vector<ll>(n+1));
    dp[0][0] = 1;
    ll p2 = pew(2,mod-2);
    for (int i=0;i<=n;i++)
    {
        for (int j=0;j<=n;j++)
        {
            ...
            if (j<n) dp[i][j+1] = (dp[i][j+1]+dp[i][j]*p2)%mod;
            if (i<n) dp[i+1][j] = (dp[i+1][j]+dp[i][j]*p2)%mod;
        }
    }
    ll ans = 0;
    for (int i=0;i<n;i++)
    {
        for (int j=i+1;j<n;j++)
        {
            for (int k=0;k<n;k++)
            {
                ...
                if (a==0) continue;
                ans = (ans+dp2[a-1][b-1]*p2)%mod;
            }
        }
    }
    ans = ans*pew(n,mod-2)%mod;
    cout << ans << endl;
}