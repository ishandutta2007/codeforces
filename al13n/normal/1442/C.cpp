...
int main(int argc, char **argv){
  ...
  {
    array<pii,2> a0;
    fill(all(a0),mp(INF,INF));
    vector<array<pii,2>> D(n,a0);
    D[0][0]=mp(0,0);
    D[0][1]=mp(1,0);
    priority_queue<pair<pii,pii>> qu;
    qu.push(mp(mp(-0,-0),mp(0,0)));
    ...
    while(!qu.empty()){
      ...
      pii v=qu.top().Y;
      qu.pop();
      if(D[v.X][v.Y]!=d) continue;
      for(pii p:G[v.X]){
        pii vv(p.X,p.Y);
        pii dd(d.X+(p.Y!=v.Y),d.Y+1);
        if(dd>=D[vv.X][vv.Y]) continue;
        D[vv.X][vv.Y]=dd;
        qu.push(mp(mp(-dd.X,-dd.Y),vv));
      }
    }
    pll w=min(D[n-1][0],D[n-1][1]);
    dbg(w);
    assert(w.X<INF);
    if(w.X>=B){
      LL r=(powmod(2,w.X,mod)-1+w.Y)%mod;
      cout<<r;
      return 0;
    }
  }

  {
    ...
    while(sz(qu)){
      ...
      if(d!=D[v][q]) continue;
      for(pii p:G[v]){
        int vv=p.X,qq=q+(q+p.Y)%2,dd=d+1;
        if(qq>=B || dd>=D[vv][qq]) continue;
        D[vv][qq]=dd;
        qu.push(mp(-dd,mp(vv,qq)));
      }
    }
    LL r=LINF;
    forn(q,B){
