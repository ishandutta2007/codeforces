...
cin>>T;while(T--)
{
    ...
    vii tinout(n,mp(0,-1));
    const lli logN = 22;
    vector<vii> pars(logN,vii(n,mp(0,-INF)));
    e[0].pb({1,0});
    lli tim=0;
    y_combinator([&](const auto &dfs,lli u,ii par)->void{
        tinout[u].X=tim++;
        pars[0][u]=par;
        for(lli i=1;i<logN;i++){
            const lli v = pars[i-1][u].X;
            pars[i][u].X = pars[i-1][v].X;
            pars[i][u].Y=max(pars[i-1][v].Y,pars[i-1][u].Y);
        }
        // dbg(u,pars[logN-1][u]);
        for(auto x:e[u]){
            if(x.X==par.X)
    ...
    auto runToTop = [&](const lli u,lli v)->lli{
        if(isAnc(v,u))
            return -INF;
        lli ans = -INF;
        for(lli i=logN-1;i>=0;i--){
            // dbg(i,sz(pars),u,pars[i][u]);
            const auto j = pars[i][v];
            if(isAnc(j.X,u))
                continue;
            ans=max(ans,j.Y);
            v=j.X;
    ...
    auto getDistanceLCA=[&](lli u,lli v)->lli{
        return max(runToTop(u,v),runToTop(v,u));
    };

    atcoder::segtree<int, op, e2> seg(n-1);
    for(lli i=0;i<n-2;i++){
        const lli d=getDistanceLCA(i+2,i+1);
        dbg(i+2,i+1,d);
        seg.set(i,d);
    }
    while(q--){
