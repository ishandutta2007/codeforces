...
pair<int, ll> solve(ll m) {
    ...
    auto s = decube(m);
    pair<int, ll> ans;
    for (ll firstMove = s; firstMove >= s - 1 && firstMove > 0; --firstMove) {
        ll c = cube(firstMove);
        for (ll secondMove = min(3200LL, firstMove); secondMove > 0; --secondMove) {
            ll b = cube(secondMove);
            if (c + b >= m) continue;
            if (c + b >= cube(firstMove + 1)) continue;
            ll left = m - c - b;
            if (left > b) left = b;
            if (left + b >= cube(secondMove + 1)) {
                left = cube(secondMove + 1) - b - 1;
            }
            if (c + b + left >= cube(firstMove + 1)) {
                left = cube(firstMove + 1) - c - b - 1;
            }
            if (left > N) break;
            pair<int, ll> res = dp[left];
            ll thirdMove = decube(res.second);
            if (thirdMove > secondMove) continue;
            res.first += 2;
            res.second += c + b;
            if (res > ans) {
                ans = res;
...
pair<int, ll> naive(ll m) {
    pair<int, ll> ans;
    for (ll i = m; i >= m - 1000000 && i > 0; --i) {
        int res = 0;
        ll x = i;
        while (x) {
            if (x <= N) {
...
//    CURTIME();
    while (cin >> m) {
//    while (true) {
//        auto m = 1 + (rand() * (1LL << 32) + rand()) % 1000000000000000LL;
        auto res = solve(m);
//        auto naiveRes = ssolve(m);
//        if (res != naiveRes) {
//            DBN(m, res, naiveRes);
//            auto v1 = moves(res.second);
//            auto v2 = moves(naiveRes.second);
//            DBN(v1, v2);
//            DBN(v1.size(), v2.size());
//        }
