...
impl RerootingOperator for State {
    ...
    type Edge = ();
    fn init(&mut self, v: usize) -> Self::Value {
        let mut a = vec![0; 2 * self.k];
        a[0] = self.a[v];
        a
    }
    fn merge(&mut self, p: &Self::Value, c: &Self::Value, _: &Self::Edge) -> Self::Value {
        let mut p = p.clone();
        let mut c = c.clone();
        c.rotate_right(1);
        for (p, c) in p.iter_mut().zip(c.iter()) {
            *p ^= *c;
        }
        p
    }
...
fn run() {
    ...
    let mut s = String::new();
    for dp in dp.iter() {
        let xor = dp[k..].iter().fold(0, |s, a| s ^ a);
        let v = (xor != 0) as usize;
        s.push_str(&format!("{} ", v));
    }
    s.pop();
    println!("{}", s);
