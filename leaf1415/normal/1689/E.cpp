...
ll calc()
{
	...
	rep(i, 1, n){
		rep(j, 0, 30) if(a[i] & (1<<j)) uf.unite(i, n+1+j);
	}
	
	ll odd = -1, ret = 0;
	rep(i, 1, n) if(a[i] % 2) odd = uf.root(i);
	
	map<ll, vector<ll>> mp;
	rep(i, 1, n) mp[uf.root(i)].pb(i);
	if(sz(mp) == 1) return 0;
	if(odd == -1) return inf;
	for(auto p : mp) if(p.fi != odd) b[p.se[0]]++, ret++;
