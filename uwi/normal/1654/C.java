...
public class C2 {
	...
	void go()
	{
		...
		for(int v : a){
			pq.merge((long)v, 1, Integer::sum);
		}

		PriorityQueue<Long> rems = new PriorityQueue<>(Comparator.comparingLong(x -> -x));
		rems.add(s);
		...
		while(!rems.isEmpty()){
			...
			if(pq.containsKey(cur)){
				pq.merge(cur, -1, Integer::sum);
				if(pq.get(cur) == 0)pq.remove(cur);
			}else{
				rems.add(cur/2);
				rems.add((cur+1)/2);
			}
