...
namespace Polys
  {
    ...
    namespace Helper
      {
        inline uint32 power(uint32 a, uint32 b, const uint32 _MOD = MOD)
          {
            uint32 res = 1;
            while (b)
              {
        ...
        inline uint32 inv(uint32 a, const uint32 _MOD = MOD)
            ...
            struct Num
              {
                ...
                Num operator * (const Num& B) const
                  {
                    return Num(
                      ( 1ull * x * B.x + 1ull * y * B.y % MOD * w ) % MOD,
              ...
              template <class _Num>
              inline _Num power(_Num a, uint32 b)
                {
                  _Num res(1,0);
                  while (b)
                    {
              ...
              inline uint32 euler_judge(uint32 x)
                {
                  return Helper::power(x, (MOD-1)/2);
                }

              struct NoResidueException {} ;
              uint32 solve(uint32 n)
                {
                  if (n == 0)
                    return 0;
                  if (euler_judge(n) != 1)
    ...
    struct Int
      {
        typedef int uint32;
        static inline uint32 _mod_red(uint32 a)
          { return a >= MOD ? a-MOD : a; }
        static inline void _mod_red_set(uint32& a)
          { a = a >= MOD ? a-MOD : a; }

        uint32 a;
        Int (uint32 a=0) : a(a) {}
        inline Int operator + (const Int B) const
          { return Int( _mod_red( a + B.a ) ); }
        inline Int& operator += (const Int B)
          { _mod_red_set( a += B.a ); return *this; }
        inline Int operator - (const Int B) const
          { return Int( _mod_red( MOD + a - B.a ) ); }
        inline Int& operator -= (const Int B)
          { _mod_red_set( a += MOD - B.a ); return *this; }
        inline Int operator * (const Int B) const
          { return Int( static_cast<uint64>(a) * B.a % MOD ); }
        inline Int& operator *= (const Int B)
          { a = static_cast<uint64>(a) * B.a % MOD; return *this; }
        inline Int operator / (const Int B) const
          { return Int( 1ull * a * Helper::inv( B.a ) % MOD ); }
        inline Int& operator /= (const Int B)
          { a = 1ull * a * Helper::inv( B.a ) % MOD; return *this; }
        inline Int inv() const
          { return Int( Helper::inv(a) ); }
    ...
    template <class _Item>
        struct _PolyBase
      {
        ...
        _PolyBase resize( uint32 _new_len )
          {
            ...
            if (origin != nullptr)
              {
                memcpy( a, origin, std::min( 1<<n, 1<<origin_n ) *
                        sizeof(_Item) );
                delete[] origin;
              }
            return *this;
        ...
        inline _Item& operator [] (const uint32 pos)
          { return a[ pos ]; }
        inline _Item operator [] (const uint32 pos) const
          { return a[ pos ]; }
            ...
            void require(uint32 _new_n)
              {
                ...
                for (uint32 _l = 1; _l <= _lim; _l <<= 1)
                  for (uint32 _i = 0; _i < _l; ++ _i)
                    w[_l + _i] = Complex( std::cos(Pi / _l * _i),
                                          std::sin(Pi / _l * _i) );
              }
    ...
    template <class _Item>
    Int _PolyBase<_Item>::eval(const Int x) const
      {
        Int pos = 1;
        Int res = 0;
        uint32 len = 1 << n;
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::deriv() const
      {
        uint32 len = 1 << n;
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::inter() const
      {
        uint32 len = 1 << n;
        _PolyBase<_Item> R(len);
        Pre::Inv.require(len);
    ...
    template <class _Item>
    void _PolyBase<_Item>::butterfly ()
      {
        Pre::Rev.require(n);
    ...
    template <class _Item>
    void mulpn(_PolyBase<_Item>& _A, _PolyBase<_Item>& _B)
      {
    ...
    template <class _Item>
    void mulpb(_PolyBase<_Item>& _C,
               const _PolyBase<_Item>& _A, const _PolyBase<_Item>& _B)
      {
        uint32 n = std::max(_A.n , _B.n) + 1;
        _C.resize( 1<<n );
    ...
    template <class _Item>
    void mulp(_PolyBase<_Item>& _A, const _PolyBase<_Item>& _B)
      {
    ...
    template <class _Item>
    _PolyBase<_Item> mulp_r(const _PolyBase<_Item>& _A,
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::operator+
        (const _PolyBase<_Item>& _B) const
      {
        uint32 len_a = 1 << n;
        uint32 len_b = 1 << _B.n;
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::operator* (_Item b) const
      {
        uint32 len = 1 << n;
    ...
    template <class _Item>
    _PolyBase<_Item>& _PolyBase<_Item>::operator*= (_Item b)
      {
        uint32 len = 1 << n;
        for (uint32 i = 0; i < len; ++ i)
          a[i] *= b;
    ...
    _PolyBase<_Item>& _PolyBase<_Item>::operator*=
        (const _PolyBase<_Item>& _B)
      { mulp(*this, _B); return *this; }

    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::inv() const
      {
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::sqrt() const
      {
        typedef _PolyBase<_Item> Poly;
        Poly R(2);
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::ln() const
      {
        typedef _PolyBase<_Item> Poly;
        uint32 len = 1 << n;
        Poly A = deriv();
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::exp() const
      {
        typedef _PolyBase<_Item> Poly;
        if (a[0] != 0)
          {
    ...
    template <class _Item>
    _PolyBase<_Item> _PolyBase<_Item>::pow(uint64 k, uint64 k2) const
      {
        typedef _PolyBase<_Item> Poly;
        uint32 len = 1 << n;
        uint32 _fir_id = len;
    ...
    template <class _Item>
    _PolyBase<_Item> divp(_PolyBase<_Item>& A, uint32 An,
                          _PolyBase<_Item>& B, uint32 Bn)
      {
        uint32 Cn = An - Bn + 1;
