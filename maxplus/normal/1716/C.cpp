...
int main() {
  for (int t = $; t--; ) {
    ...
    --a[0][0];
    for (int r: {0, 1})
    for (int i = n, sml = -inf, smr = -inf; i--; ) {
      smaxl[r][i] = maxi(sml, a[r][i] + i);
      smaxr[r][i] = maxi(smr, a[r][i] - i);
    }
    int ans = inf;
    for (int i = 0, pt = 0; i < n; ++i)
    for (auto r: {i & 1, ~i & 1}) {
      int f = r != i % 2, cans = maxi(pt, a[r][i] + 2 * n - 1 - 2 * i - f);
      if (i < n - 1)       maxi(cans, smaxr[r][i + 1] + 2 * n - 1 - i - f);
      if (i < n - 1 || !f) maxi(cans, smaxl[!r][i + f] - i - f);
      mini(ans, cans);
    }
    print(ans + 1);
  }
