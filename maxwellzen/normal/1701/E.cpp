...
void solve() {
    ...
    int cur = 0;
    vi pref(m), suf(m);
    F0R(i, n) if (cur<m && s[i]==t[cur]) pref[cur]=i, cur++;
    if (cur < m) {
        cout << -1 << nl;
        return;
    }
    cur = m-1;
    R0F(i, n) if (cur>=0 && s[i]==t[cur]) suf[cur]=i, cur--;
    int ans;
    F0R(i, m+1) {
        if (i==m || s[i]!=t[i]) {
            ans = n-i;
    ...
    vvi len(n, vi(m, 0));
    R0F(i, n) R0F(j, m) {
        if (s[i]==t[j]) len[i][j] = 1 + (i<n-1 && j<m-1 ? len[i+1][j+1] : 0);
        else len[i][j] = 0;
    }
    F0R(j, m-1) FOR(i, pref[j], suf[j+1]+1) if (s[i]==t[j]) {
        if (i==1 && j==0) debug(i, j, len[i][j]);
        int val = 1 + j + 2*(i-j) + n - i - len[i][j];
        ckmin(ans, val);
    }
    cout << ans << nl;
}
