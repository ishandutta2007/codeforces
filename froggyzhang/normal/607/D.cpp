...
int Q,inv[N],d[N],x[N],opt[N],f[N];
int w[N],n;
int dfn[N],dep[N],son[N],siz[N],top[N],id[N],num;
vector<int> G[N];
...
void update(int i){
	...
	tree[i].all=(tree[ls].all+tree[rs].all)%mod;
}
inline void pushdown(int i){
	if(tree[i].mul^1){
		tree[ls].Mul(tree[i].mul);
		tree[rs].Mul(tree[i].mul);
		tree[i].mul=1;
	}
}
...
void Change(int i,int L,int R,int l,int r,int d){
	if(L>=l&&R<=r){
		tree[i].Mul(d);
		return;
	}
...
int main(){
	...
	for(int i=1;i<=Q;++i){
		int u=x[i];
		if(opt[i]==1){
			int t=1LL*inv[d[f[u]]]*(d[f[u]]+1)%mod;
			++d[f[u]];
			Change(1,1,n,dfn[f[u]],dfn[f[u]]+siz[f[u]]-1,t);
			Add(1,1,n,dfn[u]);
		}
		else{
			int ans=Query(1,1,n,dfn[u],dfn[u]+siz[u]-1);
			ans=1LL*ans*qpow(Get(1,1,n,dfn[u]),mod-2)%mod;
			ans=1LL*ans*d[u]%mod;
			printf("%d\n",ans);
		}
	}
