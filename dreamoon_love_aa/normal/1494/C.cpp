...
int solve(VI &a, VI &b) {
    ...
    for(int i=SZ(b)-1;i>=0;i--){
        if(binary_search(ALL(a),b[i]))dp[i]=1;
        if(i+1<SZ(b))dp[i]+=dp[i+1];
    }
    int it=-1;
    int ret=0;
    REP(i,SZ(b)){
        while(it+1<SZ(a)&&a[it+1]<=b[i]+it+1)it++;
        if(it<0)ret=max(ret,dp[i]);
        else {
            int pos=upper_bound(ALL(b),b[i]+it)-b.begin();
            ret=max(ret,pos-i+(pos<SZ(b)?dp[pos]:0));
        }
    }
    return ret;
}
int a[SIZE], b[SIZE],n,m;
void solve() {
    VI aa[2];
    VI bb[2];
    REP(i,n){
        if(a[i]<0)aa[0].PB(-a[i]);
        else aa[1].PB(a[i]);
    }
    REP(i,m){
        if(b[i]<0)bb[0].PB(-b[i]);
        else bb[1].PB(b[i]);
    }
    int an=0;
    REP(i,2)an+=solve(aa[i],bb[i]);
    W(an);
}
void input() {
    R(n,m);
