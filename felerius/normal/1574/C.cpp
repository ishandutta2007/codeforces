// Slay the Dragon (https://codeforces.com/contest/1574/problem/C)
...
int main() {
    ...
    for (auto [x, y] : dr) {
        ...
        if (it != end(a))
            cost = min(cost, max(0ll, y - (asum - *it)));
        if (it != begin(a))
            cost = min(cost, x - *prev(it) + max(0ll, y - (asum - *prev(it))));
        println(cost);
    }
}