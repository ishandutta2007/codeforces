...
class TaskG {
public:
	// (prefix length * hasMaxDigit * is_smaller_than_N * is_larger_than_0) => amount

	ll solve(vector<int> &N, int maxDigit) {
		vector3<ll> D(LEN+1, 2, 4, 0);
		D[0][0][0] = 1;

		int failI = LEN - 1 - maxDigit / 4;
		int failMask = 1 << (maxDigit % 4);

		for (int i = 0; i < LEN; ++i) {
			for (int j = 0; j < 2; ++j) {
				for (int k = 0; k < 4; ++k) {
					if (!D[i][j][k]) continue;
					bool is_smaller_than_N = (k & 1) == 1;
					bool is_larger_than_0 = (k & 2) == 2;
					for (int d = 0; d <= maxDigit; d++) {
						if (d == 0 && !is_larger_than_0) continue;
						if (d > N[i] && !is_smaller_than_N) continue; // would be larger than N
						if (i == failI && (d & failMask) == 0) continue; // would be increasing
						int newK = k;
						if (d < N[i]) newK |= 1;
						if (d > 0) newK |= 2;
						D[i + 1][j | (d == maxDigit)][newK] += D[i][j][k];
					}

					if (!is_larger_than_0) {
						if (i == failI) continue; // would be increasing
						int newK = k;
						if (0 < N[i]) newK |= 1;
						D[i + 1][j][newK] += D[i][j][k];
					}
				}
			}
		}
		return D[LEN][1][2] + D[LEN][1][3];
	}
