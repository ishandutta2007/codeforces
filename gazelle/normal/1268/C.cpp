...
int main() {
    ...
    vector<ll> ans(n, 0);
    ll sum = 0;
    segment_tree<int> rsq(n, [] (int a, int b) { return a + b; }, 0);
    vector<int> hoge(n, 0);
    for(int i = n - 1; i >= 0; i--) {
        sum += rsq.query(0, dict[i]);
        rsq.update(dict[i], 1);
    }
    REP(i, n) rsq.update(dict[i], 0);
    REP(i, n) {
        hoge[i] = i - rsq.query(0, dict[i]);
        rsq.update(dict[i], 1);
    }
    segment_tree<ll> tol(n, [] (ll a, ll b) { return a + b; }, 0);
    segment_tree<ll> tor(n, [] (ll a, ll b) { return a + b; }, 0);
    for(int i = n - 1; i >= 0; i--) {
        if(i == n - 1) ans[i] = sum;
        else {
            ll l = -1, r = n;
            ...
            while(r - l > 1) {
                ll m = (l + r) / 2;
                if(rsq.query(0, m + 1) >= tgt) r = m;
                else l = m;
            }
            ll x = r - (tgt - 1), y = (n - 1) - i - x;
            
            ans[i] = sum;
            ans[i] += tol.query(0, r) - x * (x - 1) / 2;
            ans[i] += tor.query(r, n) - y * (y - 1) / 2;
        }
        rsq.update(dict[i], 0);
        sum -= hoge[i];
        tol.update(dict[i], dict[i]);
        tor.update(dict[i], n - 1 - dict[i]);
    }
    cout << ans << endl;
    return 0;
}