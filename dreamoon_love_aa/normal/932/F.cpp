...
void merge(int x,int y){
    for(auto line:H[x].lines){
        H[y].add_line(line.slope,line.h,line.id);
    }
}
void dfs(int x,int lt){
    ...
    for(int y:e[x]){
        ...
        dfs(y,x);
        son_num++;
        if(SZ(H[line_ptr[x]])<SZ(H[line_ptr[y]])){
            swap(line_ptr[x],line_ptr[y]);
        }
        merge(line_ptr[y],line_ptr[x]);
    }
    if(son_num){
        int me=line_ptr[x];
        int y=H[me].get_line_id(a[x]);
        ans[x]=ans[y]+(LL)a[x]*b[y];
    }
    H[line_ptr[x]].add_line(b[x],ans[x],x);
}
int main(){
    int n;R(n);
    FOR(i,1,n)R(a[i]);
