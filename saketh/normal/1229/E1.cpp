...
int main() {
    ...
    vvmn p(N, vmn(N));
    re(p);
    trav (r, p) trav (e, r) e /= 100;

    if (N < 6) {
        trav (r, p) r.resz(6, 0);
        while (sz(p) < 6) p.pb(vmn(6, 1));
        N = 6;
    }

    vi p3, inx(1 << 6, -1);
    vvi es;
    F0R (m, 1 << 6) if (__builtin_popcount(m) == 3) {
        inx[m] = sz(p3);
        p3.pb(m);
        es.pb({});
        vi ord{0, 1, 2};
    ...
    auto calc = [&p3, &inx, &es](vvmn p, bool c) {
        vmn res(1 << 20);
        F0R (em, 1 << 18) {
            int tr = 0;
            F0R (_i, 20) {
                int i = c ? inx[0b111111 ^ p3[_i]] : _i;
                bool ok = false;
                trav (eu, es[i]) if ((em | eu) == em) ok = true;
                if (ok) tr |= 1 << _i;
            }

            mn ch = 1;
            F0R (i, 3) F0R (j, 6) {
                ch *= (em & (1 << (i * 6 + j))) ? p[i][j] : 1 - p[i][j];
            }
            res[tr] += ch;
        }
        return res;
    ...
    vmn r1 = calc({p.begin(), p.begin() + 3}, false);
    vmn r2 = calc({p.begin() + 3, p.end()}, true);

    F0R (b, 20) F0R (m, 1 << 20) if (m & (1 << b)) {
        r2[m] += r2[m ^ (1 << b)];
    }

    mn ans = 0;
    F0R (m, 1 << 20) ans += r1[m] * r2[((1 << 20) - 1) ^ m];
    ps(1 - ans);
