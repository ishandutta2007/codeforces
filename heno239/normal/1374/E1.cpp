...
void solve() {
	...
	while (st.size() + sa.size() + sb.size() < k && rest.size()) {
		P val = *rest.begin();
		rest.erase(rest.begin());
		st.insert(val);
		sum += val.first;
	}
	//cout << "? " << st.size() << " " << sa.size() << " " << sb.size() << "\n";
	if (st.size()+sa.size()+sb.size() >=k && sa.size() == k && sb.size() == k) {
		ans = min(ans, sum);
		//cout << "wow " << sum << "\n";
	}
	rep1(i, k) {
		...
		while (st.size() + sa.size() + sb.size() + sab.size() < k && rest.size()) {
			P val = *rest.begin();
			sum += val.first;
			st.insert(val);
			rest.erase(rest.begin());
		}
		while (st.size() + sa.size() + sb.size() + sab.size() > k && st.size()) {
			P val = *--st.end();
			sum -= val.first;
			rest.insert(val);
			st.erase(--st.end());
		}
		if (st.size() + sa.size() + sb.size() + sab.size() >=k && sa.size() + sab.size() == k && sb.size() + sab.size() == k) {
			ans = min(ans, sum);
		}
	}
	if (ans == INF) {
