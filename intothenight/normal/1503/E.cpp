...
int main(){
	...
	for(auto rep = 0; rep < 2; ++ rep){
		for(auto x = 1; x < n; ++ x){
			Zp suff = 0;
			for(auto l = m - 1; l > 0; -- l){
				suff += C.C(n - x + m - l - 1, n - x) * C.C(n - x + l - 1, l);
				res += 2 * C.C(x + l - 1, x) * C.C(x + m - l - 1, m - l) * suff;
			}
		}
		swap(n, m);
	}
	for(auto x = 1; x < n; ++ x){
		for(auto y = 1; y < m; ++ y){
			res -= 2 * C.C(x + y - 1, x) * C.C(x + m - y - 1, m - y) * C.C(n - x + m - y - 1, n - x) * C.C(n - x + y - 1, y);
		}
	}
	cout << res << "\n";
	return 0;
