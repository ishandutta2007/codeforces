...
//    solve.usetc()
    ...
    solve.cases{
        ...
        val downCount = List(n){IntArray(m)}
        val rightCount = List(n){IntArray(m)}

        for(i in n-1 downTo 0 ){
            for(j in 0 until m){
                downCount[i][j] += if(T[i][j]) 1 else 0
                if(i < n-1)
        ...
        for(j in m-1 downTo 0 ){
            for(i in 0 until n){
                rightCount[i][j] += if(T[i][j]) 1 else 0
                if(j < m-1){
        ...
        val RA = List(n){IntArray(m)} // go down next
        val DA = List(n){IntArray(m)}
        RA[0][0] = 1
        DA[0][0] = 1
        if(m >= 2) RA[0][1] = -1
        if(n >= 2) DA[1][0] = -1
        ...
        for(d in 0 until (n+m-1)){
            for(i in 0 until n){
                ...
                if(j > 0) RA[i][j] += RA[i][j-1]
                if(i > 0) DA[i][j] += DA[i-1][j]

                RA[i][j] = RA[i][j].adjust()
                DA[i][j] = DA[i][j].adjust()

                val dnext = RA[i][j]
                val rnext = DA[i][j]

//                just dei "$i $j $dnext $rnext"

                val d1 = i + 1
                val d2 = n-1-if(i < n-1) downCount[i+1][j] else 0

                val r1 = j + 1
                val r2 = m-1-if(j < m-1)rightCount[i][j+1] else 0

                if(d1 < n)
                    DA[d1][j] = (DA[d1][j] + dnext).adjust()

                if(d2 + 1 < n){
                    DA[d2+1][j] = (DA[d2+1][j] - dnext).adjust()
                }

                if(r1 < m) {
                    RA[i][r1] = (RA[i][r1] + rnext).adjust()
                }
                if(r2 + 1 < m){
                    RA[i][r2+1] = (RA[i][r2+1] - rnext).adjust()
                }
            }
        }
        put((RA[n-1][m-1] + DA[n-1][m-1]).adjust())
