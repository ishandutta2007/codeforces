...
void dfs(int p, double q, double a, double b, double c)
{
	if(p == 21) return;
	
	ans += q*c * (p+1);//, outl(q*c, p+1);
	double na, nb, nc;
	if(a > eps){
		double x = min(a, v);
		na = a-x;
		if(b > eps) nb = b+x/2, nc = c+x/2;
		else nb = b, nc = c+x;
		dfs(p+1, q*a, na, nb, nc);
	}
	if(b > eps){
		double x = min(b, v);
		nb = b-x;
		if(a > eps) na = a+x/2, nc = c+x/2;
		else na = a, nc = c+x;
		dfs(p+1, q*b, na, nb, nc);
	}
}
