...
int main() {
    ...
    rep(i, N) {
        rep(j, s[i].size()) {
           ...
           mp[i][cst[i]].push_back(j);
        }
    }

    vvll dp(1ull << N, vll(2, -INF));
    dp[0][0] = 0;
    rep(x, 1ull << N) {
        ll now = 0;
        rep(j, N)if ((x >> j) & 1)now += cst[j];
        ...
        rep(j, N) {
            ...
            if (t >= 0) {
                if (mp[j].count(-t - 1)) {
                    if (mp[j].count(-t)) {
                        ll id = mp[j][-t - 1][0];
                        auto itr = lower_bound(mp[j][-t].begin(), mp[j][-t].end(), id);
                        ll d = distance(mp[j][-t].begin(), itr);
                ...
                else if(mp[j].count(-t)) {
                    dp[x][0] = max(dp[x][0], dp[y][0] + mp[j][-t].size());
                }
                else {
    ...
    ll ans = max(dp.back()[0], dp.back()[1]);
    cout << ans << endl;
}