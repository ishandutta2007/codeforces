...
public class C {
	...
	int solve2(int[] a, int len) {
		...
		for (int i = 0; i < n; i++) {
			int left = b[n - 1 - i] - 4 * i;
			if (left <= 0) {
				break;
			}
			ans += Math.max(left - 1, 1);
		}
		return len - ans;
	}
