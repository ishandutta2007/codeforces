...
public class Main
{
	...
	{
		int n=ni(),l=ni(),ini=ni();
		int[]pr=pnia(n);
		int[]ba=pnia(n);
		double[][][]dp=new double[n+1][n+1][2*n+1];
		if(n+ini<2*n)
			dp[0][0][n+ini]=1;
		else
			dp[0][0][2*n]=1;
		for(int i=1; i<=n; i++)
		{
			for(int j=0; j<=n; j++)
				for(int k=0; k<=2*n; k++)
					dp[i][j][k]+=dp[i-1][j][k]*(100-pr[i])/100.0;
			for(int j=1; j<=n; j++)
				for(int k=0; k<=2*n; k++)
					if(k+ba[i]>=2*n)
						dp[i][j][2*n]+=dp[i-1][j-1][k]*(pr[i])/100.0; 
					else if(k+ba[i]>=0)
						dp[i][j][k+ba[i]]+=dp[i-1][j-1][k]*(pr[i])/100.0;
		}
		double ans=0;
		for(int i=l; i<=n; i++)
			for(int j=n; j<=2*n; j++)
