...
int main(){
	...
	while(t--){
		...
		a[0]=mp(0,0);
		rep(n){
			if(str[i]=='U') a[i+1]=mp(a[i].first,a[i].second+1);
			if(str[i]=='D') a[i+1]=mp(a[i].first,a[i].second-1);
			if(str[i]=='R') a[i+1]=mp(a[i].first+1,a[i].second);
			if(str[i]=='L') a[i+1]=mp(a[i].first-1,a[i].second);
		}
		Min=1e9;
		m.clear();
		m[a[0]]=0;
