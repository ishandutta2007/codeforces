...
void solve() {
    autoint n;
    autoread<vector<point>> points(n);
    ...
    point twice_center = {minx + maxx, miny + maxy};
    stable_sort(points.begin(), points.end());
    for (point &p : points){
        if (binary_search(points.begin(), points.end(), point{twice_center.x - p.x, twice_center.y - p.y}) == false)answer("NO");
    }
    answer("YES");
}