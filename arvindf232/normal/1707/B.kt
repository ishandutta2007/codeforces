...
fun main(){
    solve.cases{
        ...
        fun solve(x:TreeMap<Int,Int>):TreeMap<Int,Int>{
            ...
            for((a,b) in x){
                if(previouscount >= 0){
                    val d = a - previousv
                    new[d] = new.getOrDefault(d,0) + 1
                }
                if(b > 1){
                    new[0] = new.getOrDefault(0,0) + b - 1
                }
        ...
        var start = TreeMap<Int,Int>()
        for(a in L){
            start[a] = start.getOrDefault(a,0) + 1
        }
        repeat(n-1){
            start = solve(start)
        }
