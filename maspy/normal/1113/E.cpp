...
void solve() {
  ...
  auto rm = [&](ll x) -> void {
    ...
    ll px = ss.prev(x);
    ll nx = ss.next(x);
    ll vx = (X[nx] - X[px]) * speed[px];
    seg.set(px, {min(0LL, vx), vx});
    seg.set(x, {0, 0});
  };

  auto add = [&](ll x, ll s) -> void {
    assert(!ss[x]);
    ll px = ss.prev(x);
    ll nx = ss.next(x);
    ss.insert(x);
    ...
    ll vx = (X[x] - X[px]) * speed[px];
    seg.set(px, {min(0LL, vx), vx});
    // print("set", X[px], vx);
    vx = (X[nx] - X[x]) * speed[x];
    seg.set(x, {min(0LL, vx), vx});
    // print("set", X[x], vx);
  };
  ...
  for (auto&& [t, a, b, c]: query) {
    ...
    if (t == 3) {
      ...
      a = LB(X, a);
      b = LB(X, b);
      auto check = [&](auto e) -> bool { return c + e.fi > 0; };
      ll k = seg.max_right(check, a);
      if (k >= b) {
        print(-1);
      } else {
        ll x = X[k];
        ll now = c + seg.prod(a, k).se;
        ll s = -speed[k];
        ll dt = X[b] - x;
        if (now - s * dt > 0) {
          print(-1);
