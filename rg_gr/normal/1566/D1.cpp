...
signed main()
{
	int T=read();
	rg(T)read();
	n=read();memset(q,0,sizeof(int)*(n+3));
	rg(n)aa[i].a=read(),aa[i].id=i;p[i]=i;gr
	std::sort(p+1,p+1+n,[&](int x,int y){return aa[x].a==aa[y].a?aa[x].id>aa[y].id:aa[x].a<aa[y].a;});
//	rg(n)odb(p[i]);gr
rg(n)pq[i]=p[i];gr
	long long ans=0;
	rg(n)ans+=sum(pq[i]);add(pq[i]);gr oldl(ans);
	gr
	return 0;
}