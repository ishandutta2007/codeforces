...
#define rank orzorzorzYJZ
int n,m,rank[105];
double dp[105][100005],pre[100005];
int main()
{
	get2(n,m);
	int sum=0;
	for(int i=1;i<=n;i++){get1(rank[i]);sum+=rank[i];}
	dp[0][0]=m*1.0-1;
	for(int i=1;i<=n;i++)
	{
		for(int j=1;j<=i*m+1;j++)pre[j]=pre[j-1]+dp[i-1][j-1];
		for(int j=0;j<=i*m;j++)
		{
			dp[i][j]=pre[j]-pre[max(m,j)-m];
			if(j>=rank[i])dp[i][j]-=dp[i-1][j-rank[i]];
			dp[i][j]/=(m*1.0-1);
		}
	}
	double ans=1.0;
	for(int i=n;i<sum;i++)ans+=dp[n][i];
	printf("%.10lf\n",ans);
	return 0;
}
/*
