...
void Nyaan::solve() {
  ...
  auto dd = [&](auto rc, int l, int r) -> D {
    ...
    auto sX = R.ss * L.ss + R.sf * L.fs;
    auto sY = R.ss * L.sf + R.sf * L.ff;
    auto fX = R.fs * L.ss + R.ff * L.fs;
    auto fY = R.fs * L.sf + R.ff * L.ff;
      trc(sX,sY,fX,fY);
    return {sX, sY, fX, fY};
  };

  auto res = dd(dd, 0, sz(b));

  auto f = res.fs;
  f += res.ff;
  trc(f);
  mint ans = 0;
  rep(i, sz(f)) { ans += f[i] * C.fac(i)  * (i%2==sz(b)%2?1:-1); }
  out(ans);
}