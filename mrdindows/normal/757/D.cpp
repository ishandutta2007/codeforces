...
string s;
map<int, int> dp[1 << 20];
int go(int pos, int mask, int maxval) {
    ...
    if (need > s.size() - pos) return 0;
    
//    int &res = dp[pos][maxval][mask];
    int &res = dp[mask][maxval * 128 + pos];
    if (res != 0) {
        return res - 1;
    }
    ll ans = 0;
    if (mask != 0 && (1 << maxval) == mask + 1) {
        ++ans;
    }
    int val = 0;
    for (int i = pos; i < s.size(); ++i) {
        ...
        if (val > 20) break;
        if (val != 0) {
            ans += go(i + 1, mask | (1 << (val - 1)), max(maxval, val));
        }
    }
    res = ans % mod + 1;
    return res - 1;
