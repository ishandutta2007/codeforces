...
val adj = Array<ArrayList<Int>>(300005,{ArrayList<Int>()})
val ans = BooleanArray(300005,{false})
val deg = IntArray(300005,{0})
var cur = 0
var n = 0
var k = 0
...
fun PrintWriter.solve() {
    ...
    for (i in 1..t) {
        val (n,m) = readInts(2)
        val p = PriorityQueue<Pair<Int,Int>>(compareBy {it.first})
        val z = ArrayList<Pair<Int,Int>>()
        val a = IntArray(n+1)
        val b = IntArray(n+1)
        val t = IntArray(n+1)
        for (j in 1..n) {
            val (_a,_b) = readInts(2)
            a[j] = _a
            b[j] = _b
            z.add(Pair<Int,Int>(a[j],j))
        }
        z.sortBy{it.first}
        var ind = 0
        var ti = 0
        var d = 0
        while (ind < n || p.size > 0) {
            if (p.size == 0) {
                ti = z[ind].first
            }
            while (ind < n && ti == z[ind].first) {
                val j = z[ind].second
                p.add(Pair<Int,Int>(b[j],j))
                ind ++
            }
            for (j in 1..m) {
                if (p.size == 0) break
