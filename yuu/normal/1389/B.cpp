...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 0; j <= z; j++) f[i][j] = -1e9;
    f[1][0] = a[1];
    int ans = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j <= z; j++) {
            int moved = i - 1 + j * 2;
            if (moved == k) ans = max(ans, f[i][j]);
            if ((i > 1) && (moved == k - 1) && (j < z)) ans = max(ans, f[i][j] + a[i - 1]);
            if (moved >= k) continue;
            if ((j < z) && (i > 1)) f[i][j + 1] = max(f[i][j + 1], f[i][j] + a[i - 1] + a[i]);
            if (i < n) f[i + 1][j] = max(f[i + 1][j], f[i][j] + a[i + 1]);
        }
    }
    cout << ans << '\n';
}
