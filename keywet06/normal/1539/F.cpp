...
#define rs u << 1 | 1
    ...
    int minv[maxn], maxv[maxn], tag[maxn], pos1[maxn], pos2[maxn];
    inline void build(int u, int l, int r) {
        tag[u] = 0, pos1[u] = 0, pos2[u] = 0;
        if (l == r)
            return pos1[u] = l, pos2[u] = l, maxv[u] = minv[u] = l, void();
        int mid = (l + r) >> 1;
    ...
    inline void pushup(int u) {
        if (minv[ls] < minv[rs])
            minv[u] = minv[ls], pos1[u] = pos1[ls];
        else
            minv[u] = minv[rs], pos1[u] = pos1[rs];
        if (maxv[ls] > maxv[rs])
            maxv[u] = maxv[ls], pos2[u] = pos2[ls];
        else
            maxv[u] = maxv[rs], pos2[u] = pos2[rs];
    }
    inline void update(int u, int l, int r, int x, int y, int v) {
        if (x <= l && y >= r)
            return maxv[u] += v, minv[u] += v, tag[u] += v, void();
        int mid = (l + r) >> 1;
        pushdown(u, l, r);
...
inline void solve() {
    ...
    FOR(i, 1, n) {
        int x = id[i];
        if (a[id[i]] != a[id[i - 1]])
            for (int x : v[a[id[i]]]) tree.update(1, 0, n, x, n, -2);
        tree.update(1, 0, n, x, n, 2);
        auto lp = tree.query2(1, 0, n, 0, x - 1),
             rp = tree.query1(1, 0, n, x, n);
    ...
    FOR(i, 1, n) {
        int x = id[i];
        if (a[id[i]] != a[id[i - 1]] && i != 1)
            for (int x : v[a[id[i - 1]]]) tree.update(1, 0, n, x, n, -2);
        tree.update(1, 0, n, x, n, -2);
        auto lp = tree.query1(1, 0, n, 0, x - 1),
             rp = tree.query2(1, 0, n, x, n);
        int val = rp.first - lp.first;
        assert(val >= -1);
