...
struct GraphKeeper{
	struct Dsu{
		static constexpr int DSUSZ = 131072;
		int dsu[DSUSZ];
	...
	void AddNode( int x , int y ){
		...
		For0(k,4){
			...
			if( vx < 1 or vy < 1 or vx > n or vy > m ) continue;
			if(!added[vx][vy]) continue;
			adjs.Eb(dsu.Findfa(id[vx][vy]));
			dsu.Unite(id[x][y],id[vx][vy]);
		}
		sort(All(adjs));
		adjs.resize( unique(All(adjs)) - adjs.begin() );
		componentCnt += 1 - (int)adjs.size();
		// P(x,y,componentCnt);
	}

}graphKeeper;

struct SpecificColorSolver{
	struct Event{ int x,y,t; };
	vc<Event> adds, dels;
	void Solve(){
		if( adds.empty() and dels.empty() ) return;

		auto Clear = [&]( const vc<Event> &es ){
			graphKeeper.componentCnt = 0;
			for( auto [x,y,t] : es ){
				graphKeeper.dsu.dsu[id[x][y]] = id[x][y];
				graphKeeper.added[x][y] = false;
			}
		};

		For0(i,adds.size()){
			auto [x,y,stT] = adds[i];
			int edT = i < (int)adds.size()-1 ? adds[i+1].t : !dels.empty() ? dels[0].t : q+1;
			graphKeeper.AddNode(x,y);
			sumKeeper.Add(stT,edT-1,graphKeeper.componentCnt);
		}
		Clear(adds);

		fOR0(i,dels.size()){
			auto [x,y,stT] = dels[i];
			int edT = i+1 == dels.size() ? q+1 : dels[i+1].t;
			sumKeeper.Add(stT,edT-1,graphKeeper.componentCnt);
			graphKeeper.AddNode(x,y);
		}
		Clear(dels);
...
void ReadQueries(){
	...
	For(i,n) For(j,m)
		solvers[0].adds.Pb({i,j,0});
	For(t,q){
		iRead3(x,y,c);
		if( cur[x][y] == c ) continue;
		solvers[cur[x][y]].dels.Pb({x,y,t});
		solvers[c].adds.Pb({x,y,t});
		cur[x][y] = c;
	}
	For(i,n) For(j,m)
		solvers[cur[i][j]].dels.Pb({i,j,q+1});
}
...
int main(){
	...
	Forx(col,0,2000000) solvers[col].Solve();
	// solvers[0].Solve();
