...
int main(void)
{
	...
	rep(i, 1, n) bfs(G, n, i, dist[i]);
	ll d1 = dist[s1][t1], d2 = dist[s2][t2];
	if(d1 > l1 || d2 > l2){
		outl(-1);
		return 0;
	}
	...
	rep(i, 1, n) rep(j, 1, n){
		if(dist[s1][i] + dist[i][j] + dist[j][t1] <= l1 && dist[s2][i] + dist[i][j] + dist[j][t2] <= l2){
			chmin(ans, dist[i][j]+dist[s1][i]+dist[j][t1]+dist[s2][i]+dist[j][t2]);
		}
		if(dist[t1][i] + dist[i][j] + dist[j][s1] <= l1 && dist[s2][i] + dist[i][j] + dist[j][t2] <= l2){
			chmin(ans, dist[i][j]+dist[t1][i]+dist[j][s1]+dist[s2][i]+dist[j][t2]);
		}
		if(dist[s1][i] + dist[i][j] + dist[j][t1] <= l1 && dist[t2][i] + dist[i][j] + dist[j][s2] <= l2){
			chmin(ans, dist[i][j]+dist[s1][i]+dist[j][t1]+dist[t2][i]+dist[j][s2]);
		}
		if(dist[t1][i] + dist[i][j] + dist[j][s1] <= l1 && dist[t2][i] + dist[i][j] + dist[j][s2] <= l2){
			chmin(ans, dist[i][j]+dist[t1][i]+dist[j][s1]+dist[t2][i]+dist[j][s2]);
		}
	}
	outl(m - ans);
