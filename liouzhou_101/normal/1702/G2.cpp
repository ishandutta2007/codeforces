...
void solve()
{
	...
	while (q--)
	{
		...
		auto between = [&](int x, int y, int e)
		{
			int z = g.GetLCA(x, y);
			if (g.GetLCA(x, e) == e && g.GetLCA(e, z) == z) return true;
			if (g.GetLCA(y, e) == e && g.GetLCA(e, z) == z) return true;
			return false;
		};

		for (int i = 1; i < a.size(); ++i)
		{
			if (g.depth[x] > g.depth[y]) swap(x, y);
			int z = g.GetLCA(x, y);
			int e = a[i];
			if (between(x, y, e)) continue;
			if (x != z && y != z && g.GetLCA(e, x) == x) { x = e; continue; }
			if (x != z && y != z && g.GetLCA(e, y) == y) { y = e; continue; }
			if (x == z)
			{
				if (g.GetLCA(y, e) == y) { y = e; continue; }
				if (g.depth[g.GetLCA(y, e)] <= g.depth[x]) { x = e; continue; }
			}
			flag = 1;
			break;
		}
