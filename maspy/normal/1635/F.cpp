...
void solve() {
  ...
  auto dist
      = [&](int i, int j) -> ll { return abs(X[i] - X[j]) * (W[i] + W[j]); };
  vi ANS(Q, INF);
  VEC(pi, query, Q);
  ...
  FOR_(2) {
    ...
    reverse(all(X));
    reverse(all(W));
    for (auto&& [l, r]: query) tie(l, r) = mp(N - r, N - l);

    vc<vi> QID(N + 1);
    FOR(q, Q) QID[query[q].se].eb(q);
    ...
    vi LEFT(N, -1);
    {
      SegTree<Mono> seg(W);
      FOR(i, N) {
        auto check = [&](auto e) -> bool { return e > W[i]; };
        LEFT[i] = seg.min_left(check, i) - 1;
      }
    }
    ...
    FOR(i, N + 1) {
      for (auto&& q: QID[i]) {
        auto [l, r] = query[q];
        chmin(ANS[q], seg.prod(l, i));
      }
      if (i == N) break;
      auto l = LEFT[i];
      if (l != -1) { seg.set(l, min(seg[l], dist(l, i))); }
    }
  }
