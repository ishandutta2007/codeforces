    ...
    function<void(int, int)> dfs = [&](int v, int p) {
        ...
        if (isz(us)) {
            ...
            for (int i = 1; i < isz(us); i++) {
                int u = us[i];
                for (auto [c, k] : ms[u]) {
                    ts[v].erase(pair(ms[v][c], c));
                    ms[v][c] += k;
                    ts[v].insert(pair(ms[v][c], c));
                }
            }
        }

        ts[v].erase(pair(ms[v][c[v]], c[v]));
        ms[v][c[v]]++;
        ts[v].insert(pair(ms[v][c[v]], c[v]));

        for (auto [k, id] : qs[v]) {
            ans[id] = isz(ts[v]) - ts[v].order_of_key(pair(k, -1));
        }
    };
