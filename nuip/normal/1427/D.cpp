  ...
  cout<<fixed<<setprecision(0);
	...
	cauto app=
		[&](vector<int> op){
			...
			vector<int> op_=op;
			op.clear();
			for(auto x:op_) op.eb(x);
			re.eb(op);
			vv<int> bkt;
			int s=0;
			for(auto x:op){
				bkt.eb();
				reps(i,s,s+x) bkt.back().eb(vs[i]);
				s+=x;
			}
			reverse(all(bkt));
			vs.clear();
			for(auto &v:bkt)for(auto x:v) vs.eb(x);
			out(vs,1);
		};
	while(1){
		{
			...
			rep(i,n)if(vs[i]==t){
				vector<int> op(t,1);
				op.eb(i+1-t);
				cauto sum=accumulate(all(op),0);
				if(sum!=n) op.eb(n-sum);
				app(op);
				break;
			}
		}
		{
			int t=0;
			while(t<n && vs[n-1-t]==t) ++t;
			if(t==n){
				app(vector(n,1));
				break;
			}
			rep(i,n)if(vs[n-1-i]==t){
				vector<int> op(t,1);
				op.eb(i+1-t);
				cauto sum=accumulate(all(op),0);
				if(sum!=n) op.eb(n-sum);
