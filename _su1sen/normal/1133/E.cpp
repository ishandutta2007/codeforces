...
int main() {
    ...
    for (int i = 0; i < n;) {
        ...
        c.emplace_back(a[i], j - i);
        i = j;
    }
    int m = c.size();
    vec<vec<int>> dp(m + 1, vec<int>(k + 1, 0));
    rep(i, m) {
        int p = i;
        while (p - 1 >= 0 and c[p - 1].first >= c[i].first - 5) --p;
        int s = 0;
        rep(j, p, i + 1) s += c[j].second;
        rep(j, k + 1) {
            if (j < k) {
                chmax(dp[i + 1][j + 1], dp[p][j] + s);
            }
            chmax(dp[i + 1][j], dp[i][j]);
        }
    }
    print(*max_element(all(dp[m])));
    return 0;
}