	...
	for (int i = 1; i <= n; ++i)
		for (int j = 0; j <= m; ++j)
			for (int k = 0; k <= 500; ++k)
				dp[i][j][k] = INF;
	for (int j = 0; j <= m; ++j)
		dp[1][j][250 + (a[1] - j)] = abs(a[1] - j);
	for (int i = 1; i < n; ++i)
		for (int j = 0; j <= m; ++j)
			for (int k = 0; k <= 500; ++k)
			{
				if (dp[i][j][k] == INF) continue;
				for (int jj = j; jj <= m / (n - i); ++jj)
				{
					if (0 <= k + (a[i + 1] - jj) && k + (a[i + 1] - jj) <= 500) 
						freshmin(dp[i + 1][jj][k + (a[i + 1] - jj)], dp[i][j][k] + abs(k + (a[i + 1] - jj) - 250));
				}
			}
	int res = INF;
	for (int j = 0; j <= m; ++j)
		freshmin(res, dp[n][j][250]);
	writeln(res);
}
