...
signed main(){
	...
	rep(_,q){
		...
		//round,idx,length
		vc<T> dist(s,T(inf,inf,inf));
		using ti=pair<T,int>;
		priority_queue<ti,vc<ti>,greater<ti>> pq;
		
		auto reach=[&](int v,T d){
			if(dist[v]>d){
				dist[v]=d;
				pq.emplace(d,v);
			}
		...
		while(si(pq)){
			int v;
			T t;
			tie(t,v)=pq.top();pq.pop();
			if(dist[v]!=t)continue;
			
			int rd,id,len;tie(rd,id,len)=t;
			for(auto e:g[v]){
				int nx=len+e.b;
				reach(e.a,T((nx+sp[id]-1)/sp[id],id,nx));
			}
		}
		
		rep(i,m){
			int j=hld.index[imp[i]];
			print(get<1>(dist[j])+1,i==m-1?1:2);
		}
	}
}