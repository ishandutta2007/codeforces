...
void dfs(int p,int now,int djg,int cnt)
{
	dist[djg][now]=cnt;
	for(int i:nei[now])
		if(i!=p)
...
int main()
{
	...
	int x,y,z;
	for(x=0;x<n;x++)if(f[x])break;
	dfs(-1,x,0,0);
	y=x;
	for(int i=0;i<n;i++)if(f[i]&&dist[0][i]>dist[0][y])y=i;
	dfs(-1,y,1,0);
	z=y;
	for(int i=0;i<n;i++)if(f[i]&&dist[1][i]>dist[1][z])z=i;
	dfs(-1,z,2,0);
	int cnt=0;
	for(int i=0;i<n;i++)if(dist[1][i]<=d&&dist[2][i]<=d)cnt++;
	printf("%d\n",cnt);
	return 0;
}