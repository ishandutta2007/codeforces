...
int main() {
	...
	fori(i, m) {
		defa defb defc
			a--; b--;
		cons[a].push_back({ b,2*c });
		cons[b].push_back({ a,2*c });
	}
	veci value(n);
	veci sign(n);
	veci visited(n);
	fori(i, n) {
		...
		wne(stk) {
			...
			desiredvalues.push_back( -value[cur] * sign[cur]);
			visited[cur] = 1;
			current.push_back(cur);
			forin(i, cons[cur]) {
				int oth = cons[cur][i].ff;
				int sum= cons[cur][i].ss;
				if (visited[oth]) {
					if (sign[oth] != sign[cur]) {
						if (value[cur] + value[oth] != sum) {
							co "NO" el
								ex
						}
					}
					else {
						int need = sign [cur]*((sum - value[cur] - value[oth]) / 2);
						if (forced) {
							if (must != need) {
								co "NO" el
									ex
					...
					sign[oth] = -sign[cur];
					value[oth] = sum - value[cur];
					stk.push_back(oth);
				}
			}
		}
		if (!forced) {
			sort(all(desiredvalues));
			must = desiredvalues[desiredvalues.size() / 2];
		}
		forin(j, current) {
			value[current[j]] += must * sign[current[j]];
		}
	}
	co "YES" el
	fori(i, n) {
