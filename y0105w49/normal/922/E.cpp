...
ll dp[1024][10<<10];
ll rr[10<<10];
int c[1024],cost[1024];

template<class T> inline void e1(multiset<T> &ms,const T &x) {
	auto it=ms.find(x);
	// assert(it!=ms.end() && *it==x);
	ms.erase(it);
}
...
int32_t main() {
	...
	for(int i=0;i<=n;i++) for(int j=0;j<=10000;j++) dp[i][j]=-linf;
	dp[0][0]=W;
	...
	for(int i=1;i<=n;i++) {
		...
		for(int j=0;j<=10000;j++) {
			rr[j]=min(dp[i-1][j]+X,1LL*j*B+W)+1LL*j*cost[i];
			ss.insert(rr[j]);

			ll z=*ss.rbegin();
			dp[i][j]=z-1LL*j*cost[i];
			if(dp[i][j]<0) {
				dp[i][j]=-linf;
				break;
			}
			// if(j<=10 && dp[i][j]>=0)
				// dbg(i,j,c[i],cost[i],W,B,X,dp[i][j],ss);

			if(j>=c[i]) e1(ss,rr[j-c[i]]);
		}
	}
	for(int j=10000;j>=0;j--) if(dp[n][j]>=0) return printf("%d\n",j),0;
}