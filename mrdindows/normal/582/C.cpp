...
int pi[1000100];
int prefix_function (int *s, int n, int k) {
    for (int i = 1; i < n; ++i) {
        int j = pi[i - 1];
        while (j > 0 && s[i] != s[j])
            j = pi[j - 1];
...
int di[201010];
vector<int> vd[3000];
ll getCount(int x, int d, int max) {
    ll res = 0;
    int id = di[d];
    for (int p : vd[id]) {
        if (p > x) break;
    ...
    for (int d : v) {
        ...
        for (int i = 0; i < n; ++i) {
            if (e[i]) {
                ...
                ans += getCount(len, d, n - i);
                i = j;
            }
        }
//        for (int len = d; len < n; len += d) {
//            if (gcd(len, n) != d) continue;
//            for (int i = len; i < n + len; ++i) {
//                if (tot[i] - tot[i - len] == i - (i - len)) {
//                    ++ans;
//                }
//            }
//        }
...
//            maxd[i + d + 1] = a[i];
//        }
//        int t = prefix_function(maxd, n + d + 1, d);
//        ans += t;
    }
    cout << ans << endl;
//    FI(n) {
