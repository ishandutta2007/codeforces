...
void solve() {
    ...
    read(grid, n);
    vector<int> loc[n][2];
    F0R(i, n) F0R(j, m) if (grid[i][j]=='1') loc[i][(i+j)%2].pb(j);
    int ans = 0;
    F0R(par, 2) {
        vpi q;
        F0R(i, n) if (!loc[i][par].empty()) {
            q.pb(mp(loc[i][par].back(), i));
        }
        while (!q.empty()) {
            sort(all(q));
            reverse(all(q));
            ...
            for (pi p : q) {
                pi r = mp(p.S, p.F);
                auto it = path.ub(r);
                if (it != path.end() && abs(it->F - r.F) < abs(it->S - r.S)) continue;
                if (it != path.begin()) {
                    it--;
                    if (abs(it->F - r.F) < abs(it->S - r.S)) continue;
                }
                path.insert(r);
                loc[r.F][par].pop_back();
            }
            debug(path);
            q.clear();
            F0R(i, n) if (!loc[i][par].empty()) {
