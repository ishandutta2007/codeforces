...
int main(){
  ...
  vector<ll>p;
  ll lim=59;
  rep(i,2,lim)if(isPrime(i))p.PB(i);
  ll sz=p.size();
  vector<ll>px(lim);
  rep(i,1,lim){
    auto mp=factoring(i);
    for(auto z:mp){
      rep(j,0,sz){
        if(z.fi==p[j])px[i]^=1<<j;
      }
    }
  }
  const int inf=1e9;
  auto dp=vec(1<<sz,n+1,inf);
  dp[0][0]=0;
  rep(i,0,1<<sz){
    rep(o,1,lim){
      if(i&px[o])continue;
      rep(j,0,n){
        chmin(dp[i^px[o]][j+1],dp[i][j]+abs(o-a[j]));
      }
    }
  }
  res=inf;
  rep(i,0,1<<sz)chmin(res,dp[i][n]);
  rep(i,0,1<<sz){
    if(res==dp[i][n]){
      vector<ll>ret(n);
      ll now=n,bit=i;
      while(now){
        rep(o,1,lim){
          if((bit&px[o])!=px[o])continue;
          if(dp[bit^px[o]][now-1]+abs(o-a[now-1])==dp[bit][now]){
            ret[now-1]=o;
            now--;
            bit^=px[o];
            break;
