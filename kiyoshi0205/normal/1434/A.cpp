...
int main(){
  ...
  set<tdata,greater<tdata>> se;
  rep(i,N)se.emplace(b[i]-a[0],datas(i,0));
  ans=-prev(se.end())->first+se.begin()->first;
  while(1){
    auto itr=se.begin();
    tie(i,j)=itr->second;
    if(j==5)break;
    se.erase(itr);
    se.emplace(b[i]-a[j+1],datas(i,j+1));
    chmin(ans,-prev(se.end())->first+se.begin()->first);
  }
  print(ans);
}