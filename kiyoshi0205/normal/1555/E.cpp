...
F id(){return 0;}
using minplussegtree=lazy_segtree<S,ope,e,F,mapping,composition,id>;
...
int main(){
  ...
  vector<tdata> v(N);
  for(auto&[c,x]:v){
    cin>>x.first>>x.second>>c;
    --x.first;
    --x.second;
  }
  vsort(v);
  v.emplace_back(inf,datas(0,M-1));
  debug(v);
  minplussegtree tree(vec(M-1,0));
  ll ans=inf;
  rep(i,N){
    while(!tree.all_prod()){
      tree.apply(v[j].second.first,v[j].second.second,1);
      ++j;
    }
    debug(i,j);
    chmin(ans,v[j-1].first-v[i].first);
    tree.apply(v[i].second.first,v[i].second.second,-1);
  }
  print(ans);
}