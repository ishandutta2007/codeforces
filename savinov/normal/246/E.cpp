...
inline vector<USI * > * merge(vector<USI * > * l, vector<USI * > * r)
{
    if (l->size() < r->size())
        swap(l, r);
    //cout << "***" << endl;
    ...
    forn(i, r->size())
        (*l)[l->size() - i - 1] = merge((*l)[l->size() - i - 1], (*r)[r->size() - i - 1]);
    //cout << l->size() << " " << r->size() << endl;
    //cout << "***" << endl;
    return l;
}
...
void dfs(int v, vector <USI * > * &t)
{
    ...
    {
        if (gg[v][i].x < t->size())
            ans[gg[v][i].y] = (*t)[t->size() - gg[v][i].x - 1]->size();
    }
//    cout << endl;
//    cout << v << endl;
//    forn(i, t->size())
