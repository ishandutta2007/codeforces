...
int main() {
    ...
    int N; re(N); vi a(N); re(a); trav (e, a) --e;

    int v1 = N;

    auto smoller = [](int a, int b) { return min(a, b); };
    segment_tree<int, decltype(smoller)> v2(N + 1, IMPOSSIBLE, smoller);
    ...
    F0R (i, N) {
        ...
        // v2 := a[i]
        int v2_cost = IMPOSSIBLE;
        {
            // v2 already a[i]
            ckmin(v2_cost, v2[a[i]] + adjust);

            // v2 not a[i]
            ckmin(v2_cost, 1 + adjust + min(v2.accumulate(0, a[i]), v2.accumulate(a[i] + 1, N + 1)));
        }
        ...
        // offer v2 := a[i] as v2 := v1_old
        {
            int entry = v2[v1_old] + adjust;
            ckmin(entry, v2_cost);
            v2.assign(v1_old, entry - adjust);
        }
    }

    ps(v2.accumulate(0, N + 1) + adjust);
