...
int main() {
    ...
    F0R (_, 2) {
        vi order = {0, 1, 2};
        sort_by(order, vals[a] < vals[b]);
        if (vals[order[0]] + vals[order[2]] == 2 * vals[order[1]])
            go(vals[order[2]] - vals[order[1]]);
        else
            go(3 * vals[r] - accumulate(all(vals), 0ll));
    }
