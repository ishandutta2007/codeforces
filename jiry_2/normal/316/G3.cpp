...
int main(){
    scanf("%s",ch+1); len=1; memset(t,0x00,sizeof t); last=1; scanf("%d",&n); int kk1=strlen(ch+1);
    for (int i=1;i<=kk1;i++) insert(ch[i]-'a',0); insert(26,n+1);
    for (int i=1;i<=n;i++){scanf("%s",ch+1); kk1=strlen(ch+1); for (int j=1;j<=kk1;j++) insert(ch[j]-'a',i); insert(26,n+1); scanf("%d%d",&l[i],&r[i]);}
