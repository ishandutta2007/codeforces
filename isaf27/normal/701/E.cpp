...
ll ans;
int n, k, fl[MAX_N], kol[MAX_N];
vector<int> st;
bool univ[MAX_N], used[MAX_N];
vector<vector<int> > v(MAX_N), t(MAX_N);
...
void go(int p, int now, int del)
{
    ...
    int d = pp[num] - del;
    ans -= (ll)(2 * now - 2 * d) * (ll)(fl[p]);
    go(t[p][num], 2 * d - now, del + now - d);
}
