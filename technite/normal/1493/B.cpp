...
int main()
{
    ...
    while(t--)
    {
        ...
        for(int i =HH; i<h;i++)
        {
            ...
            for(int j = min; j < m;j++)
            {
                if(maps[i%10]!=-1 and maps[i/10]!=-1 and maps[j%10]!=-1 and maps[j/10]!=-1)
                {
                    int temp_MM = maps[i%10]*10 + maps[i/10];
                    int temp_HH = maps[j%10]*10 + maps[j/10];
                    if(temp_HH < h and temp_MM < m )
                    {
                        found = true;
                        ans = make_pair(i,j);
                        break;
        ...
        {   
            string answer="";
            answer+= char(ans.F/10+'0');
            answer+= char(ans.F%10+'0');
            answer+= ":";
            answer+= char(ans.S/10+'0');
            answer+= char(ans.S%10+'0');
            cout<<answer<<'\n';
        }
        else
