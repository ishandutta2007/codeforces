...
struct Trie {
	...
	void insertString(char *s, int t) {
		Node* v = root;
		for (int i=0;i<t;i++)
		{
			char ch = s[i];
			if (v->child.find(ch)==v->child.end())
				v->child[ch] = new Node();
			v = v->child[ch];
			v->count++;
		}
...
int main()
{
	...
	for (int i=0;i<len;i++)
	{
		...
		for (int j=i;j<len;j++)
		{
			char ch = s[j];
			bool w = false;
			if (ver->child.find(ch)==ver->child.end())
			{
				ver->child[ch] = new Node();
				w = true;
			}
			ver = ver->child[ch];
			...
			{
				int c = 0;
				if (e[q] && node[q]->child.find(ch)!=node[q]->child.end())
				{
					node[q] = node[q]->child[ch];
					c = node[q]->count;
				}
				else
				{
