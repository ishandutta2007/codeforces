...
void solve() {
    ...
    int mx = ask(no_zero, k);
    int low = no_zero, high = k;
    while (low < high) {
        int mid = (low + high) / 2;
        if (ask(low, mid) == mx) {
            high = mid;
    ...
    int mx_group = low;
    int cnt = 0;
    for (int i = 1; i <= n; i++) cnt += (group[i] != mx_group);
    cout << "? " << cnt;
    for (int i = 1; i <= n; i++)
        if (group[i] != mx_group) cout << ' ' << i;
    cout << '\n';
    cout.flush();
    int ans_mx_group;
    cin >> ans_mx_group;
    assert(ans_mx_group != -1);
    cout << "! ";
    for (int i = 1; i <= k; i++) cout << ((i == mx_group) ? ans_mx_group : mx) << " \n"[i == k];
    cout.flush();
    string result;
    cin >> result;
