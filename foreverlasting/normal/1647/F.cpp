...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        auto solve=[&](const auto &a){
            res x=max_element(a+1,a+n+1)-a,mx1=0,mx2=0;
            for(res i=1;i<x;i++){
                if(a[i]>mx1)mx1=a[i];
            ...
            res lmx=mx2;
            vector<int> rmx(n+1,inf);
            mx1=mx2=0;
            for(res i=n;i>x;i--){
                if(a[i]>mx1)mx1=a[i],rmx[i]=mx2;
                else if(a[i]>mx2)mx2=a[i],rmx[i]=mx1;
                else break;
            }
            res ans=0;
            array<int,2> f{lmx,0};
            for(res i=x+1;i<=n;i++){
                array<int,2> g{inf,0};
                if(a[i]<a[i-1])g[0]=min(g[0],f[0]);
                if(a[i]>a[i-1])g[1]=max(g[1],f[1]);
                if(a[i]<f[1])g[0]=min(g[0],a[i-1]);
                if(a[i]>f[0])g[1]=max(g[1],a[i-1]);
                if(g[1]>rmx[i])ans++;
                f=g;
            }
            return ans;
        ...
        res ans=solve(a);
        reverse(a+1,a+n+1),ans+=solve(a),printf("%d\n",ans);
    }
}
int main(){
//    srand(time(0));
