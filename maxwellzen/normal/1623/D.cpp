...
int n, m, x, y, rd, cd, p;
vvi vis;
vector<vector<array<pair<mi, mi>, 4>>> dp;
int ind(int dx, int dy) {return (dx+1)/2 + dy+1;}
bool getvis(int x, int y, int dx, int dy) { return (vis[x][y] & 1<<ind(dx, dy)) > 0; }
void setvis(int x, int y, int dx, int dy) { vis[x][y] |= 1<<ind(dx, dy); }

void solve() {
    cin >> n >> m >> x >> y >> rd >> cd >> p;
    x--, y--, rd--, cd--;
    vis.assign(n, vi(m, 0));
    dp.assign(n, vector<array<pair<mi, mi>, 4>>(m));
    int dx=-1, dy=-1;
    mi a(0), b(1), k = mi(100-p)/mi(100);
    while (! getvis(x, y, dx, dy)) {
        setvis(x, y, dx, dy);
        dp[x][y][ind(dx, dy)] = mp(a, b);
        setvis(x, y, dx, dy);
        if (! btw(x+dx, 0, n-1)) dx *= -1;
        if (! btw(y+dy, 0, m-1)) dy *= -1;
        x += dx; y += dy; ++a;
        if (x==rd || y==cd) a *= k, b *= k;
    }
    pair<mi, mi> cur = dp[x][y][ind(dx, dy)];
    mi ans = (cur.F - a) / (b - cur.S);
    cout << ans << nl;
}
