...
struct treap {
  treap(int x_, int bsz_ = 1) : l(0), r(0), heap(rand()+rand()*RAND_MAX), x(x_), bsize(bsz_), size(bsz_) { }
  treap_ptr l, r;
  int heap;
  int x;
  int bsize, size;

  static int get_size(treap_ptr a){ return a?a->size:0; }
  ...
  static tuple<treap_ptr, treap_ptr> split_pos(treap_ptr a, li i){
    if(a == 0) return make_tuple(nullptr, nullptr);
    push(a);
    if(i <= get_size(a->l)){
      auto p = split_pos(a->l, i);
      a->l = p&yy;
      update(a);
      return make_tuple(p&xx, a);
    }else if(i < get_size(a->l) + a->bsize) {
      int szl = (i-get_size(a->l));
      int szr = a->bsize - szl;
      assert(szl>0 && szr>0);
      auto b = new treap(a->x + szl, szr);
      a->bsize = szl;
      b->r = a->r; a->r = 0;
      ...
      auto p = split_pos(a->r, i - get_size(a->l) - a->bsize);
      a->r = p&xx;
      update(a);
      return make_tuple(a, p&yy);
    }
  }

  template<class F>
  static void iterate(treap_ptr a, F f) {
    if(!a) return;
    iterate(a->l,f);
    f(a->x, a->bsize);
    iterate(a->r,f);
  ...
  static treap_ptr leftmost(treap_ptr a) {
    assert(a);
    while(a->l) a = a->l;
    return a;
  }

  static treap_ptr rightmost(treap_ptr a) {
    assert(a);
    while(a->r) a = a->r;
    return a;
  }
...
void solve(){
  ...
  for(li i = 0; i < (li)(m); ++i) {
    ...
    if(x > last+1) {
      li cnt = x-last-1;
      t = treap::merge(t, new treap(cur, cnt));
      cur += cnt;
    }
    last = x;
    treap_ptr l,r; tie(l,r) = treap::split_pos(t,y);
    if(r) isS.insert(treap::leftmost(r)->x);
    t = treap::merge(l, treap::merge(new treap(cur,1),r));
    cur += 1;
  }
  li nS = isS.size();
  li k = n-1-nS;
