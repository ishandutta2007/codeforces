...
void solve() {
  int d; cin >> d >> mod;
  vector<ModInt> cnt(N + 1, 0);
  int ub = 32 - __builtin_clz(d) - 1;
  for (int i = 0; i < ub; ++i) cnt[i] = ModInt(2).pow(i);
  cnt[ub] = ModInt(d) - accumulate(ALL(cnt), ModInt(0));
  vector dp(N + 1, vector(N + 2, ModInt(0)));
  dp[0][0] = 1;
  REP(i, N) REP(j, N + 2) {
    for (int k = j + 1; k <= N + 1; ++k) dp[i + 1][k] += dp[i][j] * cnt[k - 1];
  }
  ModInt ans = 0;
  FOR(i, 1, N + 1) ans += accumulate(ALL(dp[i]), ModInt(0));
  cout << ans << '\n';
}
