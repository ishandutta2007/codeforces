...
signed main() {
    TEST {
        ...
        // 
        vll R(n + 1);
        rep(i, n) R[i + 1] = R[i] + (i & 1 ? a[i] : -a[i]);
        auto get = [&](int x, int y) { return r[y] - r[x]; };
        auto alt = [&](int l, int r) {
            ll s = R[r] - R[l];
            if(l & 1)
                return s;
            else
        ...
        // 1 
        rep(i, n + 1) {
            if(get(0, i) > get(i, n)) ans += 1;
        }
        rep3(i, n - 1, 1) if(get(0, i) < get(i, n)) ans += 1;

        // cout << ans << endl;
        // 2 
        rep(i, n - 1) {
            // o...o xo...xo x...x
            if(get(0, i) - alt(i, i + 2) - get(i + 2, n) >= 0) continue;
            int ok = i + 2, ng = n + 1;
            while(ok < ng - 1) {
                int mid = ok + ng >> 1;
                (get(0, i) - alt(i, mid) - get(mid, n) < 0 ? ok : ng) = mid;
            }
            ans += (ok - i) / 2;
            // cout << i << " " << ok << " " << (ok - i) / 2 << endl;
        }
        // cout << ans << endl;
        ...
        rep2(d, 3, n - 1) {
            // o...o xo...xo x...x o
            if((n - d) & 1) {
                if(alt(0, n - d) - get(n - d, n - 1) + a[n - 1] >= 0) continue;
            } else if(-alt(0, n - d) - get(n - d, n - 1) + a[n - 1] >= 0)
                continue;
            int ok = 0, ng = n - d;
            while(ok < ng - 1) {
                int mid = ok + ng >> 1;
                (get(0, mid) - ((n - d - mid) & 1 ? -alt(mid, n - d) : alt(mid, n - d)) - get(n - d, n - 1) + a[n - 1] < 0 ? ok : ng) = mid;
            }
            ans += 1;
            if((n - d) & 1)
                ans += ok / 2;
        ...
        rep2(d, 3, n - 1) {
            // x o...o xox...o x...x
            if(-a[0] + get(1, d) - get(d, n) >= 0) continue;
            int ok = d, ng = n + 1;
            while(ok < ng - 1) {
                int mid = ok + ng >> 1;
                (-a[0] + get(1, d) - alt(d, mid) - get(mid, n) < 0 ? ok : ng) = mid;
            }
            ans += 1 + (ok - d) / 2;
        }
        // cout << ans << endl;
        ...
        rep2(d, 3, n - 3) {
            // xoo...x oxox...ox ox...xo
            if(-a[0] + get(1, d) - get(d, n - 1) + a[n - 1] >= 0) continue;
            int ok = d, ng = n - 2;
            while(ok < ng - 1) {
                int mid = ok + ng >> 1;
                (-a[0] + get(1, d) - alt(d, mid) - get(mid, n - 1) + a[n - 1] < 0 ? ok : ng) = mid;
            }
            // cout << ok << endl;
            // cout << d << " " << 1 + (ok - d) / 2 << endl;
            ans += 1 + (ok - d) / 2;
        }
