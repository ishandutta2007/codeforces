...
int main()
{
    ...
    in(k2);
    in(k3);
    int z1 = k1, z2 = k2, z3 = k3;
    in(t1);
    in(t2);
    in(t3);
    in(n);
    int t[n];
    priority_queue<PLI, vector < PLI > , greater<PLI> > q1, q2, q3, r1, r2, r3;
    forn(i, n)
    {
        t[i] = nxt();
        q1.push(mp(t[i], i));
    }
    ll time1 = 0, time2 = 0, time3 = 0;
    while(!q1.empty())
    {
        remax(time1, q1.top().x);
        if (r1.size() == k1)
        {
            remax(time1, r1.top().x);
            q2.push(r1.top());
            r1.pop();
        }
        PLI v = q1.top();
    ...
    while(!q2.empty())
    {
        remax(time2, q2.top().x);
        if (r2.size() == k2)
        {
            remax(time2, r2.top().x);
            q3.push(r2.top());
            r2.pop();
        }
        PLI v = q2.top();
    ...
    while(!q3.empty())
    {
        remax(time3, q3.top().x);
        if (r3.size() == k3)
        {
            remax(time3, r3.top().x);
            remax(ans, r3.top().x - t[r3.top().y]);
            r3.pop();
        }
        PLI v = q3.top();
        q3.pop();
