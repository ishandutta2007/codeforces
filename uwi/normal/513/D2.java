...
//		tr(Arrays.copyOf(imap, q+1));
		...
		SegmentTreeRMQ st = new SegmentTreeRMQ(nn);
//		int[] reach = new int[nn];
		for(int i = 0;i < nn;i++){
			int v = i;
			if(g[i].length > 0){
		...
		int[] reach = Arrays.copyOfRange(st.st, st.H, st.H+nn);
		for(int i = 0;i < nn;i++){
			reach[i] = -reach[i];
		}
//		tr("reach", reach);
		
		int[] xfrom = new int[2*nn];
		int[] xto = new int[2*nn];
//		int[] xfrom = new int[nn-1];
//		int[] xto = new int[nn-1];
		
		int[] split = new int[nn];
		Arrays.fill(split, -1);
		if(!dfs(0, nn-1, reach, g, split, st)){
			out.println("IMPOSSIBLE");
			return;
		}
//		tr(split);
		p = 0;
		dfs2(0, nn-1, split, xfrom, xto);
//		tr(xfrom);
//		tr(xto);
		if(p != nn-1){
			out.println("IMPOSSIBLE");
...
//		tr(p);
		for(int i = 0;i < imap[0];i++){
			out.print(i+1 + " ");
		}
		dfs3(0, nn-1, split, imap);
	...
	void dfs3(int cur, int r, int[] split, int[] imap)
	{
		if(cur > r)return;
		if(split[cur] == -1){
			for(int i = imap[cur+1]-1;i >= imap[cur];i--){
				out.print(i+1 + " ");
			}
			return;
		}
		
		dfs3(cur+1, split[cur]-1, split, imap);
		for(int i = imap[cur+1]-1;i >= imap[cur];i--){
			out.print(i+1 + " ");
		}
		dfs3(split[cur], r, split, imap);
	}
