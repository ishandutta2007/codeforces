...
int main() {
	...
	int taille = read<int>();
	string chaine = read<string>();

	for(char car : chaine) {
		cars[car]++;
	}

	int nbZeros = min(min(cars['z'], cars['e']), min(cars['r'], cars['o']));
	...
	int nbUns = min(min(cars['o'], cars['n']), cars['e']);

	writeln(concat(vector<int>(nbUns, 1), vector<int>(nbZeros, 0)));
	return 0;
}