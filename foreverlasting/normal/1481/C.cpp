...
namespace MAIN{
    ...
    vector<int> A[N],ans;
    inline void MAIN(){
        n=read(),m=read(),vector<int>().swap(ans);
        for(res i=1;i<=n;i++)a[i]=read(),vector<int>().swap(A[i]);
        for(res i=1;i<=n;i++){
            b[i]=read();
            if(a[i]!=b[i])A[b[i]].pb(i);
        }
        for(res i=1;i<=m;i++)c[i]=read();
        res id=0;
        if(!A[c[m]].empty())id=A[c[m]].front();
        else for(res i=1;i<=n;i++)if(b[i]==c[m])id=i;
        if(!id){puts("NO");return;}
        for(res i=1;i<=m;i++){
            if(!A[c[i]].empty())ans.pb(A[c[i]].back()),A[c[i]].pop_back();
            else ans.pb(id);
        }
        for(res i=1;i<=n;i++)if(!A[i].empty()){id=0;break;}
        if(!id){puts("NO");return;}
        puts("YES");
