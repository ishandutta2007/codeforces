...
bool judge(int i, int j){
	return (ch[i] == '(' && ch[j] == ')') || (ch[i] == '[' && ch[j] == ']');
}

int gao(int i){
	return sum[i] - sum[pre[i]-1];
}
...
int main(){
	...
	int n = strlen(ch+1);
	sum[0] = 0;
	for (int i=1; i<=n; ++i) sum[i] = sum[i-1] + (ch[i] == '[');
	pre[0] = 1;
	for (int i=1; i<=n; ++i){
		if (judge(pre[i-1]-1, i)){
			pre[i] = pre[i-1]-1;
			int t = pre[i];
			while(pre[t-1] != t) t = pre[t-1];
			pre[i] = t;
		}else {
			pre[i] = i+1;
		}
	...
	cout << gao(p) << endl;
	for (int i=pre[p]; i<=p; ++i) putchar(ch[i]);
	puts("");
	return 0;
}