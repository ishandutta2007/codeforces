...
int main() {
	...
	while (q--) {
		...
		for (int i = 1; i <= n; i++) {
			req[i] = max(0, i * x + b[i]);
			if (req[i] > 0) isAll = false;
			if (req[i] > 100 * i) isNone = true;
		}
		...
		const int MAXSUMA = 100 * 100;
		vector<vector<ll>> dp(n + 1, vector<ll>(MAXSUMA + 1, 0));
		dp[0][0] = 1;
		for (int i = 1; i <= n; i++) {
			vector<ll> ps = dp[i - 1];
			...
			for (int j = req[i]; j <= 100 * i; j++)
				dp[i][j] = ps[j] - (j - a[i] - 1 >= 0 ? ps[j - a[i] - 1] : 0);
		}
