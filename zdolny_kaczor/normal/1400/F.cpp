...
int x;
int div_mask;
int insert(int m, int v) {
	int leave = (1 << (x - v + 1)) - 1;
	return (m & leave) << v | 1;
}
void add_min(map <int, int> &m, int k, int v) {
	auto [it, added] = m.insert({k, v});
	if (!added) mini(it->second, v);
}
int solve(string s) {
	mask_set(true);
	if (s.empty()) return 0;
	...
	for (char c : s) {
		int dig = c - '0';
		map <int, int> sub = dp;
		for (auto &x : sub) x.second++;
		for (auto [m, v] : dp) {
			int get = insert(m, dig);
			debug(imask(m), imask(get), imask(v));
			int good = get & div_mask;
			if (good) {
				int low = ctz(good);
				get &= (1 << low) - 1;
			}
	...
	int ans = inf;
	for (auto [_, c] : dp) mini(ans, c);
	debug(imie(s), imie(ans));
	rans;
}
void solve() {
	scanf("%s%d", s, &x);
	for (int i = 1; i < x; ++i) if (x % i == 0) div_mask |= 1 << i;
	int n = strlen(s);
	int start = 0;
	int ans = 0;
	for (int i = 0; i < n; ++i) if (x % (s[i] - '0') == 0 && x != s[i] - '0') {
		ans += solve(string(s + start, s + i));
		start = i + 1;
	}
	ans += solve(string(s + start, s + n));
	printf("%d\n", ans);
}
int main() {
