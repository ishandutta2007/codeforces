...
int main(void)
{
	...
	rep(i, 0, k-1){
		mat[0][i] = Matrix::Ident(k+1);
		rep(j, 1, k+1) if(j != i+1) mat[0][i].at(i+1, j) += 1;
	}
	
	rep(i, 1, 61){
		lmul[0] = mat[i-1][0];
		rep(j, 1, k-1) lmul[j] = mat[i-1][j] * lmul[j-1];
		rmul[k-1] = mat[i-1][k-1];
		per(j, k-2, 0) rmul[j] = rmul[j+1] * mat[i-1][j];
		
		rep(j, 0, k-1){
			mat[i][j] = Matrix::Ident(k+1);
			mat[i][j] = lmul[(j+k-1)%k] * mat[i][j];
			if(j > 0) mat[i][j] = mat[i][j] * rmul[j];
		}
	}
	...
	Matrix pmat = Matrix::Ident(k+1); ll sum = 0;
	per(i, 60, 0){
		rep(j, 0, vec[i]-1) pmat = mat[i][(sum+j)%k] * pmat;
		sum += vec[i], sum %= k;
	}
	
	mint ans = 0;
	rep(i, 1, k+1) ans += pmat.at(i, k+1);
	outl(ans);
