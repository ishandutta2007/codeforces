...
int main(){
  ...
  while(t--){
    ...
    vector<ll>a(n);
    rep(i,0,n)cin>>a[i];
    auto [l0,r0]=val_range(a,false);
    auto [l1,r1]=val_range(a,true);
    auto seg0=range_min::make(n);
    auto seg1=range_min::make(n);
    vector<vector<ll>>g0(n+1),g1(n+1);
    rep(i,0,n){
      g0[r0[i]].PB(i);
      g1[r1[i]].PB(i);
    }
    rep(i,0,n){
      for(auto z:g0[i])seg0.update(z,seg0.M1);
      for(auto z:g1[i])seg1.update(z,seg0.M1);
      if(i==0){
        seg0.update(i,0);
        seg1.update(i,0);
      }
      else{
        ll mi=min(seg0.query(l1[i]+1,i),seg1.query(l0[i]+1,i))+1;
        seg0.update(i,mi);
        seg1.update(i,mi);
      }
    }
