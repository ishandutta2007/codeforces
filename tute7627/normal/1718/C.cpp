...
int main(){
  ...
  bool judge = true;
  ll t;cin>>t;
  using S=decltype(range_max::make(1));
  while(t--){
    ll n,q;cin>>n>>q;
    vector<int>a(n);
    rep(i,0,n)cin>>a[i];
    auto mp=factoring(n);
    vector<int>p;
    for(auto z:mp)p.PB(n/z.fi);
    vector<S>seg;
    rep(i,0,p.size()){
      seg.push_back(range_max::make(p[i]));
      vector<ll>sum(p[i]);
      rep(j,0,n){
        sum[j%p[i]]+=a[j];
      }
    ...
    while(q--){
      ...
      ll diff=x-a[b];
      rep(i,0,p.size()){
        seg[i].update(b%p[i],seg[i][b%p[i]]+diff);
      }
      a[b]=x;
      //debug(a);
      cout<<get()<<endl;
