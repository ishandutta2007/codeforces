...
void dfs(int x) {
	...
	for (int pos = 0; pos < SZ; pos++) {
		...
		if (v >= m) {
			v -= m;
			if (o[ful-v] != -1 && !vis.test(o[ful-v]))
				vis.set(o[ful-v]), st[SZ++] = o[ful-v];
			for (int k = 0; k < n; k++) {
				if ((v | (1 << k)) != v && !vis.test(m + (v | (1 << k))))
					vis.set(m + (v | (1 << k))), st[SZ++] = m + (v | (1 << k));
			}
		} else {
			if (!vis.test(a[v] + m))
				vis.set(a[v] + m), st[SZ++] = a[v] + m;
		}
	}
}
