...
bool solve(vector<int> count, int beg, bool print) {
    while (count[beg] > 0) {
        if (print) cout << beg << " ";
        count[beg]--;
        beg ^= 1;
    ...
    if (beg == 1 || beg == 2) return false;
    beg ^= 2;
    if (count[beg] == 0) return false;
    return solve(count, beg, print);
}
