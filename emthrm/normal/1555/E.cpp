...
int main() {
  int n, m; cin >> n >> m;
  vector<int> l(n), r(n), w(n); REP(i, n) cin >> l[i] >> r[i] >> w[i], --l[i], --r[i];
  vector<int> ord(n);
  iota(ALL(ord), 0);
  sort(ALL(ord), [&](int a, int b) -> bool { return w[a] < w[b]; });
  int ans = *max_element(ALL(w)) - *min_element(ALL(w)), j = 0;
  LazySegmentTree<monoid::RangeMinimumAndAddQuery<int, INF>> seg(vector<int>(m - 1, 0));
  REP(i, n) {
    for (; j < n && seg.get(0, m) == 0; ++j) {
      seg.apply(l[ord[j]], r[ord[j]], 1);
    }
    if (seg.get(0, m) > 0) chmin(ans, w[ord[j - 1]] - w[ord[i]]);
    seg.apply(l[ord[i]], r[ord[i]], -1);
  }
  cout << ans << '\n';
  return 0;
}