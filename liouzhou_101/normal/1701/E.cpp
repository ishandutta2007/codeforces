...
void solve()
{
	...
	{
		if (j < m && s[i] == t[j]) j += 1, c += 1;
		L[i] = c;
	}
	for (int i = n - 1, j = m - 1, c = 0; i >= 0; --i)
	{
		if (j >= 0 && s[i] == t[j]) j -= 1, c += 1;
		R[i] = c;
	}
	...
	for (int i = 0; i < n; ++i)
		...
		{
			if (s[i] == t[j])
				f[i][j] = (i - 1 >= 0 && j - 1 >= 0 ? f[i - 1][j - 1] : 0) + 1;
			else
				f[i][j] = 0;
		}
	...
	for (int x = 0; x <= n; ++x)
	{
		for (int y = 0; y <= R[n - x]; ++y)
		{
			int len = n - x - 1 >= 0 && m - y - 1 >= 0 ? f[n - x - 1][m - y - 1] : 0;
			int lx = n - x - 1 - len, ly = m - y - 1 - len;
			int tmp = x;
			if (ly < 0)
			{
				freshmin(res, tmp + (lx + 1) * 2 + (lx >= 0));
			}
			else if (lx >= 0)
			{
				if (L[lx] >= ly + 1)
