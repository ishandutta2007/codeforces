  ...
  void solveOne(istream &in, ostream &out) {
    ...
    for (int step = 0; step < 1000; ++step) {
      ...
      for (int i = 0; i < (n + 1) / 2; ++i) {
        if (i > 0 && i - 1 <= step && i != n / 2)
          assert(b[i] > k);
        else
          assert(b[i] >= k);
      }
      for (int i = (n + 1) / 2; i < n; ++i) {
        if (i != (n + 1) / 2 && n - 1 - i <= step) {
          assert(b[i] < k);
        } else
          assert(b[i] <= k);
      }
    ...
    int nextCandidate = 0;
    int areaSize = 1;
    int maxAreaSize = n / 2 - 1;
    while (true) {
      nextCandidate = (nextCandidate + areaSize) % n;
      out << "? " << (nextCandidate + 1) << "\n";
      out.flush();
      in >> got;
      if (got > k) {
      ...
      ++areaSize;
      if (areaSize > maxAreaSize) areaSize = maxAreaSize;
    }
    while (true) {
      nextCandidate = (nextCandidate + n - 1) % n;
      out << "? " << (nextCandidate + 1) << "\n";
      out.flush();
      in >> got;
      if (got == k) break;
