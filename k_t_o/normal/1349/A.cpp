...
#define N 200400
#define MO 
int a[N],n,g,p[N],tt,pr[N],u,v,ss;
ll Ans;
...
inline void _Sol_()
{
	...
	g=a[1];F(i,2,n)g=__gcd(g,a[i]);
	F(i,1,n)a[i]/=g;
	F(i,2,200000){
		if(!pr[i])pr[i]=i,p[++tt]=i;
		F(j,1,tt){
	...
	Ans=g;
	F(i,1,tt){
		if(V[p[i]].size()<=n-2)continue;
		
//		cerr<<p[i]<<endl;
		
		sort(V[p[i]].begin(),V[p[i]].end());
		if(V[p[i]].size()==n-1){
			F(j,1,V[p[i]][0])Ans*=p[i];
		} else F(j,1,V[p[i]][0]+V[p[i]][1])Ans*=p[i];
	}
	priE(Ans);
}
