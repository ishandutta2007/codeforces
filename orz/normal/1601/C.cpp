...
struct st
{
	vi v, p;
	int N;
	st(const int n) : v(n * 4), p(n * 4), N(n)
	{
		fl(0, 0, n);
	}
	void fl(int u, int lu, int ru)
	{
		v[u] = lu;
		if (lu != ru - 1)
		{
	...
	void push(int u, int lu, int ru)
	{
		v[u] += p[u];
		if (ru != lu + 1)
			p[2 * u + 1] += p[u], p[2 * u + 2] += p[u];
		p[u] = 0;
	}
	int mn(int u, int lu, int ru, int l, int r)
	{
		if (l >= ru || lu >= r)
			return 1e9;
		push(u, lu, ru);
		if (lu >= l && ru <= r)
			return v[u];
		return min(mn(2 * u + 1, lu, (lu + ru) / 2, l, r),
			mn(2 * u + 2, (lu + ru) / 2, ru, l, r));
	}
	void add(int u, int lu, int ru, int l, int r, int a)
	{
		if (l >= ru || lu >= r)
			return;
		if (lu >= l && ru <= r)
		...
		{
			push(u, lu, ru);
			add(2 * u + 1, lu, (lu + ru) / 2, l, r, a);
			add(2 * u + 2, (lu + ru) / 2, ru, l, r, a);
			push(2 * u + 1, lu, (lu + ru) / 2);
			push(2 * u + 2, (lu + ru) / 2, ru);
			v[u] = min(v[2 * u + 1], v[2 * u + 2]);
		}
	}
	...
	for (int I = 0; I < t; ++I)
	{
		...
		for (int x = 0; x < n; ++x)
		{
			...
			while (z < m && b[z] < A[x].X)
				++z;
			if (zz != z)
				ans += s.mn(0, n + 1) * 2 * (ll)(z - zz);
			zz = z;
			int yy = y;
			while (y < n && A[y].X <= A[x].X)
			{
				s.add(A[y].Y + 1, n + 1, -1);
				++y;
			}
			for (int i = yy; i < y; ++i)
				ans += s.mn(A[i].Y + 1, A[i].Y + 2);
			while (z < m && b[z] <= A[x].X)
				++z;
			if (zz != z)
				ans += s.mn(0, n + 1) * 2 * (ll)(z - zz);
