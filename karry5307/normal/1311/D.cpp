...
ll test,p,q,r,pp,qq,rr,res,cur,px,qx,rx,u,v;
// Fast IO
inline ll read()
{
...
inline void solve()
{
	p=read(),q=read(),r=read(),res=0x3f3f3f3f;
	for(register int i=1;i<=16000;i++)
	{
		qx=i,u=0x3f3f3f3f,cur=abs(i-q);
		for(register int j=1;j<=sqrt(qx);j++)
		{
			if(i%j==0)
			{
				if(abs(p-j)<u)
		...
		cur+=u,u=0x3f3f3f3f;
		for(register int j=i;j<=32000;j+=i)
		{
			if(abs(r-j)<u)
			{
				rx=j,u=abs(r-j);
		...
		if(cur<res)
		{
			res=cur,pp=px,qq=qx,rr=rx;
		}
	}
	writeln(res),write(pp),write(qq),writeln(rr);
