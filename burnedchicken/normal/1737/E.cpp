...
#define wiwihorz  
int n;
modint dp[maxn],pw[maxn];
const modint inv2=modint(2).inv();
void balbitorz(){
    pw[0]=1;
    rep2(i,1,maxn) pw[i]=pw[i-1]*inv2;
}
...
void orzck(){
    ...
    //dp[i]=exists good pos >=i
    dp[n]=0;
    rep3(i,n-1,0){
        dp[i]=pw[i/2+1-(i==n-1)]+dp[i+1];
        if(2*i+1<=n) dp[i]-=pw[i/2+1-(i==n-1)]*dp[2*i+1];
        bug(i,dp[i]);
    }
    rep(n) print(dp[i]-dp[i+1]);
}
