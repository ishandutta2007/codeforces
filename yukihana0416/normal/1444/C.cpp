...
void work()
{
    ...
    for(int i = 1; i <= k; i++)
    {
        for(const auto &pr : mp[mk(i, i)])
        {
            int x = pr.fi, y = pr.se;
            mer(x, y + n);
            mer(x + n, y);
            if(ck(x, x + n) || ck(y, y + n))
            {
                flag[i] = 1;
                break;
            }
    ...
    LL tot = (LL)cnt * (cnt - 1) / 2;

    for(int i = 1; i <= n * 2; i++) find_f(i);
    while(stk.size()) stk.pop();


    for(const auto &pr : mp)
    {
        if(flag[pr.fi.fi] || flag[pr.fi.se]) continue;
        int top = stk.size();
        int f = 1;
        for(const auto &pr2 : pr.se)
        {
            int x = pr2.fi, y = pr2.se;
            mer(x, y + n);
            mer(x + n, y);
