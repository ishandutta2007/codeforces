...
ll mult(ll a, ll b) {
    return INF/a <= b ? INF : min(INF, a * b);
}

ll good(const vi& pref, int fi) {
    if (sz(pref) == fi || pref[fi] == -1) return good_ct[sz(pref) - fi];

    vi cyc0 = {fi};
    for (int i = pref[fi]; ~i; i = pref[i]) {
        if (i == fi) {
            if (*max_element(all(cyc0)) != pref[fi]) return 0;
            if (pref[fi] + 1 != fi + sz(cyc0)) return 0;
            return good(pref, fi + sz(cyc0));
        } else cyc0.pb(i);
    }
    ...
    vi tail(MS, -1);
    trav (e, cyc0) tail[e] = cyc0.back();

    FOR (i, fi, MS) if (!~tail[i]) {
        int ti = i; while (!~tail[ti] && ~pref[ti]) {
            ti = pref[ti];
            if (ti == i) return 0;
        }
    ...
    vi is_tail(MS);
    trav (e, tail) if (~e) is_tail[e] = 1;
    int tails = accumulate(all(is_tail), 0);

    return mult(fact[tails - 1], good_ct[sz(pref) - MS]);
}
...
void solve() {
    ...
    set<int> avail; F0R (i, N) avail.insert(i);
    F0R (i, N) {
        bool done = 0;
        trav (e, avail) {
            ans[i] = e;
    ...
    if (K != 0 || good(ans, 0) == 0) ps(-1);
    else { trav (e, ans) ++e; pc(ans); }
}
...
int main() {
    ...
    fact[0] = 1;
    FOR (i, 1, MAXL) fact[i] = mult(i, fact[i - 1]);
