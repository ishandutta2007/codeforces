    ...
    solve.cases{
        ...
        for(i in 1..n){
            for(prev in 1..1003){
                val j = i - prev
                if(j < 0) break
                val dist = (X[i] - X[j]).absoluteValue + (Y[i] - Y[j]).absoluteValue
...
//                    just dei "$i $j $dist"
            ...
            DP[i] = maxOf(DP[i], if(i >= 1003) max[i-1001] + 1 else nlarge )

            max[i] = maxOf(max[i-1],DP[i])
        }
        put(DP.maxOrNull()!!)
    }
    done()
