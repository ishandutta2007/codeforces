...
bool check(ll l, ll r, ll L, ll R, ll id)
{
	bool lt = false, gt = false, eq = false;
	if(y[id] < l || y[id] > R) gt = true;
	else if(y[id] > r && y[id] < L) lt = true;
	else eq = true;
	
	if(c[id] == "<") return lt;
	if(c[id] == ">") return gt;
	if(c[id] == "=") return eq;
	if(c[id] == "<=") return lt || eq;
	if(c[id] == ">=") return gt || eq;
}
...
int main(void)
{
	...
	cin >> n >> k;
	rep(i, 1, k) cin >> x[i] >> c[i] >> y[i];
	
	rep(i, 1, k){
		if(x[i] == y[i]){
			if(c[i].back() == '=') continue;
			else{
				outl(0);
				return 0;
			}
	...
	rep(i, 1, n){
		rep(j, 0, 2*n){
			ll L = j, R = 2*n - (2*(i-1)-j) + 1;
			if(dp[i][j] == 0) continue;
			rep(k, 0, 2){
				if(i == n && k < 2 || j+k > 2*n) continue;
				bool flag = true;
				rep(l, 1, k){
					ll p = L+l;
					for(auto id : vec[p]) if(!check(L+1, L+k, R-2+k, R-1, id)) flag = false;
				}
				rep(l, 1, 2-k){
					ll p = R-l;
					for(auto id : vec[p]) if(!check(L+1, L+k, R-2+k, R-1, id)) flag = false;
				}
				if(flag) dp[i+1][j+k] += dp[i][j];
			}
		}
	}
	
	ll ans = 0;
	rep(i, 0, 2*n) ans += dp[n+1][i];
	outl(ans);
