...
void dfs(int u, int coeff = 1, int par = -1)
{
    tot = add(tot, coeff);
    for (auto it: G[u]) if (it.first!=par)
    {
        int v = it.first;
        int x = it.second.first;
        int y = it.second.second;

        int coeff2 = mul(coeff, mul(y, inv(x)));

        for (auto pr: primes[x])
        {
            cnt[pr.first]-=pr.second;
            cnt[pr.first] = max(cnt[pr.first], 0);
        }
        for (auto pr: primes[y])
        {
            cnt[pr.first]+=pr.second;
        }
        dfs(v, coeff2, u);
...
void solve()
{
    ...
    for (int i = 0; i<n-1; i++)
    {
        ...
        G[u].push_back(mp(v, mp(x, y)));
        G[v].push_back(mp(u, mp(y, x)));
    }
    ...
    for (auto it: cnt)
    {
        ans = mul(ans, po(it.first, it.second));
    }

    cout<<mul(tot, ans)<<endl;
...
int main()
{
    ...
    {
        int i1 = i;
        for (int d = 2; d*d<=i1; d++) if (i1%d==0)
        {
            primes[i][d] = 0;
            while (i1%d==0)
            {
