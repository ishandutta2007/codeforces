...
int main(){
  ...
  rep1(i,N-1){
    if(v[i]>v[i-1]&&v[i]>v[i+1]){
      A=B=0;
      while(i-A-1>=0&&v[i-A-1]<v[i-A])++A;
      while(i+B+1<N&&v[i+B+1]<v[i+B])++B;
      if(chmax(C,max(A,B))){
        se.clear();
        se.emplace(i);
      }else if(C==max(A,B))se.emplace(i);
      assert(A&&B);
      // if(A==B&&A%2==0)++ans;
    }
  }
  rep1(i,N-1){
    if(v[i]<v[i-1]&&v[i]<v[i+1]){
      ...
      if(chmax(C,max(A,B))){
        se.clear();
        if(A<=B)se.emplace(i+B);
        if(A>=B)se.emplace(i-A);
      }else if(C==max(A,B)){
        if(A<=B)se.emplace(i+B);
        if(A>=B)se.emplace(i-A);
      }
