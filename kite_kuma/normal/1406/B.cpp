...
int main() {
	...
	int t;
	cin >> t;
	vector<long long> answ(t);
	for(int i = 0; i < t; i++) {
		long long res = numeric_limits<ll>::min();
		INT(n);
		VEC(ll, a, n);
		sort(all(a));
		...
		ll b = 1;
		b = a[0] * a[1] * a[2] * a[3] * a.back();
		chmax(res, b);
		b = a[0] * a[1] * a[n - 1] * a[n - 2] * a[n - 3];
		chmax(res, b);
		b = a[n - 5] * a[n - 4] * a[n - 1] * a[n - 2] * a[n - 3];
		chmax(res, b);
