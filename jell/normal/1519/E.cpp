#line 1 "codeforces-workspace/contest/1519/e/e.cpp"
...
#line 11 "codeforces-workspace/contest/1519/e/e.cpp"
...
void main() {
  ...
  for (int i = 1; i <= n; i++) {
    ...
    pair s2 = {b * (c + d), a * d};
    {
      auto g1 = gcd(s1.first, s1.second);
      s1.first /= g1;
      s1.second /= g1;
      auto g2 = gcd(s2.first, s2.second);
      s2.first /= g2;
      s2.second /= g2;
    }
    G[s1].emplace_back(s2, i);
    G[s2].emplace_back(s1, i);
  }

  vector<pair<int, int>> ans;
  set<p64> used;
  vector<int> checked(1 + n);

  auto dfs = fixed_point([&](const auto& dfs, p64 now) -> int {
    int rem = -1;
    used.emplace(now);
