...
public:
    ...
    int64_t value, lazy;  // value = cost left + cost right
    int l, r, m;

    segment_tree_t(int l, int r) : l(l), r(r), m((l + r) / 2), lazy(0) {
        if (l == r) {
            value = init[l];
        } else {
    ...
    void update(const int& u, const int& v, const int64_t& d) {
        if (l > v || r < u) return;
        if (u <= l && v >= r) {
            lazy += d;
            value += d;
...
void solve() {
    ...
    int64_t ans = min(a[p[1]], a[p[n]]);  // make 1 set empty
    for (int i = 1; i <= n; i++) init[i] = init[i - 1] + a[i];
    tree = new segment_tree_t(1, n);

    for (int i = 1; i < n; i++) {
        // first set has to have from 1 to i
        if (p[i] >= 1) tree->update(1, p[i] - 1, a[p[i]]);
        tree->update(p[i], n, -a[p[i]]);
        ans = min(ans, tree->value);
    }
    cout << ans << '\n';
}
