...
int solve() {
    ...
    FI(n) {
        ...
        ll h = a[i][1];
        t = gcd(t, a[i][2]);
        allw.push_back(a[i][0]);
        allh.push_back(a[i][1]);
    }
    FI(n) {
        a[i][2] /= t;
    }
    ...
    sort(all(allh));
    reunique(allh);
    ll cnt = allw.size() * (ll)allh.size();
    if (cnt > n) {
        cout << 0 << endl;
        return 0;
    }
    if (allw.size() < allh.size()) {
        allw.swap(allh);
        wg.swap(hg);
        FI(n) swap(a[i][0], a[i][1]);
    }
    FI(n) {
        b[i][0] = int(lower_bound(all(allw), a[i][0]) - allw.begin());
        b[i][1] = int(lower_bound(all(allh), a[i][1]) - allh.begin());
    }
    int idx = (rand() ^ (rand() << 10)) % allw.size();
    if (idx < 0) idx += allw.size();
    ll x = wg[allw[idx]];
    vector<pll> v;
    for (int i = 0; i < n; ++i) {
        if (a[i][0] == allw[idx]) {
            v.push_back({b[i][1], a[i][2]});
        }
    }
    ll ans = 0;
    for (auto p : v) {
    ...
    for (int i = 0; i < n; ++i) {
        if (b[i][1] == 0) {
            respa[b[i][0]] = a[i][2] / respb[0];
        }
    }
    bool ok = true;
    FI(n) {
        double c = respa[b[i][0]] * 1.0 * respb[b[i][1]];
        if (c > 1e15) {
            ok = false;
            break;
        }
        ll t = respa[b[i][0]] * respb[b[i][1]];;
        if (t != a[i][2]) {
            ok = false;
        }
    }
