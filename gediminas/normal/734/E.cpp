...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/248912/problem/E */
/*input
11
	...
	for (int i = 0; i < n; i++) {
		...
		for (auto && i : cur) {
			for (auto && j : gra[i]) {
				...
				graf.back().insert(dal[j]);
				graf[dal[j]].insert(dal[i]);
			}
		}

		graf.back().erase(dal[i]);
	}
