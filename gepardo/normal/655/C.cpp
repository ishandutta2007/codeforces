...
inline int ps(int l, int r)
{
    //cout << "ps " << l << " " << r << " " << v[r + 1] - v[l] << endl;
    if (l > r) return 0;
    return v[r + 1] - v[l];
}
...
bool ok(int md)
{
    ...
    {
        if (s[i] == '1') continue;
        if (ps(max(0, i - md), i - 1) +
            ps(i + 1, min(n - 1, i + md)) >= k) return true;
    }
    //cout << "no :(" << endl;
    return false;
}
...
{
    v[0] = 0;
    for (int i = 0; i < n; i++) v[i + 1] = v[i] + ((s[i] == '0') ? 1 : 0);
    //cout << v << endl;
    cout << binSearch(1, n);
    return 0;
}