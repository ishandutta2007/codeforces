...
void dfs(ll u,ll fa){
    ...
    rev(i,u,v)if(v^fa){
        if(!deep[v])dfs(v,u);
        else if(deep[u]-deep[v]+1>=sz){
            myputs("2");
            const ll s=deep[u]-deep[v]+1;
            writell(s,10);
            rep(i,1,s)writell(u,i==s?10:32),u=f[u];
            exit(0);
        }
    }
    if(!g[u]){
...
int main(){
    ...
    dfs(1,0);
    myputs("1");
    rep(i,1,sz)writell(ans[i],i==sz?10:32);
    return 0;
}