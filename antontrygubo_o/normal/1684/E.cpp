...
void solve()
{
    ...
    for (auto it: cnt) cntcnts[it.second]++;

    int diffleft = cnt.size();
    ...
    for (int mex = n; mex>=0; mex--)
    {
        ...
        int need = mex - have[mex];
        if (need>k) continue;

        //cout<<need<<endl;
        //for (auto it: cntcnts) cout<<it.first<<' '<<it.second<<endl;

        int destroyed = 0;
        int k1 = k;
        for (auto it: cntcnts)
        {
            int sz = it.first; int num = it.second;

            if (k1<=num*sz)
            {
                destroyed+=(k1/sz); break;
            }
        ...
        int tot_diff = mex + (diffleft - destroyed);

        //cout<<mex<<": "<<tot_diff<<' '<<tot_diff - mex<<endl;

        ans = min(ans, tot_diff - mex);
    }
    cout<<ans<<endl;
}
