...
void orzck(){
    ...
    dp[0]=0;
    rep(n){
        while(sz(vec)>1&&a[vec.back()]>a[i]) vec.pop_back();
        bug(i,vec.back());
        if(vec.back()==-1) dp[i+1]=segtree.query(1,0,n,0,i)+b[i];
        else dp[i+1]=max(dp[vec.back()+1],segtree.query(1,0,n,vec.back()+1,i)+b[i]);
        segtree.modify(1,0,n,i+1,i+1,dp[i+1]);
        vec.pb(i);
        bug(dp[i+1]);
    }
    print(dp[n]);
