...
bool contains(ll st, ll dif, ll len, ll x)
{
    if (x<st) return false;
    if ((x - st)%dif) return false;
    ll rat = (x - st)/dif;
    if (rat>=len) return false;
    return true;
}

void solve()
{
    ll st1, dif1, len1, st2, dif2, len2;
    cin>>st1>>dif1>>len1>>st2>>dif2>>len2;

    if (dif2%dif1) {cout<<0<<endl; return;}
    for (auto x: {st2, st2 + dif2*(len2-1)})
    {
        if (!contains(st1, dif1, len1, x)) {cout<<0<<endl; return;}
    }

    if (!contains(st1, dif1, len1, st2 - dif2)) {cout<<-1<<endl; return;}
    if (!contains(st1, dif1, len1, st2 + dif2 * len2)) {cout<<-1<<endl; return;}

    vector<ll> divs;
    for (int i = 1; i*i<=dif2; i++) if (dif2%i == 0)
    {
        divs.push_back(i);
        if (i*i<dif2) divs.push_back(dif2/i);
    }
    ...
    for (auto dif: divs)
    {
        if (dif * dif1 != dif2 * __gcd(dif, dif1)) continue;

        int options = dif2/dif;
        ans = add(ans, mul(options, options));
    }
