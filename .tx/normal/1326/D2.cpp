...
void solve() {
    ...
    pair<llong, llong> p = {31, 239};
    pair<llong, llong> M = {int(1e9) + 7, int(1e9) + 9};
    
    int n = isz(s);
    vector<pair<llong, llong>> hs(n + 1), ht(n + 1);
    vector<pair<llong, llong>> ps(n + 1);
    ps[0].first = ps[0].second = 1;
    for (int i = 0; i < n; i++) {
        hs[i + 1].first = (hs[i].first * p.first + s[i] - 'a' + 1) % M.first;
        hs[i + 1].second = (hs[i].second * p.second + s[i] - 'a' + 1) % M.second;
        ht[i + 1].first = (ht[i].first * p.first + t[i] - 'a' + 1) % M.first;
        ht[i + 1].second = (ht[i].second * p.second + t[i] - 'a' + 1) % M.second;
        ps[i + 1].first = ps[i].first * p.first % M.first;
        ps[i + 1].second = ps[i].second * p.second % M.second;
    }

    auto get = [&](vector<pair<llong, llong>> &h, int l, int r) {
        pair<llong, llong> res;
        res.first = (h[r].first - h[l].first * ps[r - l].first + M.first * M.first) % M.first;
        res.second = (h[r].second - h[l].second * ps[r - l].second + M.second * M.second) % M.second;
        return res;
    };

    auto is_paly = [&](int l, int r) {
        auto zs = get(hs, l, r + 1);
        auto zt = get(ht, n - r - 1, n - l);
        return zs == zt;
    };
    // cout << get(hs, 1, 2).first << "\n";
    // cout << get(ht, 1, 2).first << "\n";
    // return;
    ...
    for (int it = 0; it < 2; it++) {
        for (int i = 0; i * 2 <= n; i++) {
            ...
            while (l + 1 < r) {
                ...
                if (is_paly(i - m, i + m - 1)) {
                    l = m;
                } else {
                    r = m;
                }
            }
            pl[i - l] = max(pl[i - l], 2 * l);
        }
        for (int i = 1; i < n; i++) {
            pl[i] = max(pl[i], pl[i - 1] - 2);
        }
        // cout << pl << "\n";
        swap(s, t);
        swap(hs, ht);
    ...
    int ar = 0;
    for (int i = 0; i < n; i++) {
        if (is_paly(0, i) && i + 1 > alen) {
            alen = i + 1;
            al = i + 1;
            ar = 0;
        }
        if (is_paly(n - i - 1, n - 1) && i + 1 > alen) {
            alen = i + 1;
            al = 0;
            ar = i + 1;
        }
