...
ll tree[nax * 2];
int reindex = 0;
ll read(int l, int r, int wh = 1, int lb = 0, int rb = nax, int dep = 17) {
	if (l >= rb || r <= lb) return 0;
	if (l <= lb && r >= rb) return tree[wh];
	int sb = (lb + rb) / 2;
	bool flip = reindex >> dep & 1;
	return read(l, r, wh * 2 + flip, lb, sb, dep - 1) + read(l, r, wh * 2 + !flip, sb, rb, dep - 1);
}
int a[nax];
void solve() {
	int n, q;
