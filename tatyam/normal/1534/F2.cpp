...
int main(){
    LL(n,m);
    VEC(string,s,n);
    UnWeightedGraph g(n*m);
    rep(j,m){
        ll at=-1;
        rep(n){
            if(s[i][j]=='#'){
                if(at!=-1)g.add_directed_edge(at,i*m+j);
                at=i*m+j;
            }
            if(j&&s[i][j-1]=='#'){
                if(at!=-1)g.add_directed_edge(at,i*m+j-1);
            }
            if(j+1<m&&s[i][j+1]=='#'){
                if(at!=-1)g.add_directed_edge(at,i*m+j+1);
            }
            if(i&&s[i-1][j]=='#'){
                g.add_directed_edge(at,i*m+j-m);
            }
        }
    }
    ...
    vec(ll,mn,N,LINF);
    vec(ll,mx,N,-LINF);
    rep(j,m)rep(n)if(s[i][j]=='#'){
        chmin(mn[scc[i*m+j]],j);
        chmax(mx[scc[i*m+j]],j);
    }
    rep(N)each(j,g2[i]){
        chmin(mn[j],mn[i]);
        chmax(mx[j],mx[i]);
    ...
    vector<pll>q;
    VEC(ll,a,m);
    if(sum(a)==0)return out(0);
    rep(j,m)rrep(n)if(s[i][j]=='#'){
        if(!a[j]--)break;
        q.emplace_back(mn[scc[i*m+j]],mx[scc[i*m+j]]);
    }
    Sort(q);
    ll ans=1,min=LINF;
    each(l,r,q){
