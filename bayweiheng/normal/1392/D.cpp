...
struct sol {
	...
	int f(int i) {
		...
		int ans=inf;
		if(i+2 <= n) {
			ans = min(ans, f(i+2) + (s[i]!='R') + (s[i+1]!='L'));
		}
		if(i+3 <= n) {
			ans = min(ans, f(i+3) + (s[i]!='R') + (s[i+1]!='L') + (s[i+2]!='L'));
			ans = min(ans, f(i+3) + (s[i]!='R') + (s[i+1]!='R') + (s[i+2]!='L'));
		}
		if(i+4 <= n) {
			ans = min(ans, f(i+4) + (s[i]!='R') + (s[i+1]!='R') + (s[i+2]!='L') + (s[i+3]!='L'));
		}
		return dp[i] = ans;
	}
	void solve() {
