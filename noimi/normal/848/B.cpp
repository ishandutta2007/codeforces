...
signed main(){
    ...
    vector<pii> ans(n);
    for(auto &e:mp1)sort(all(e.se));
    for(auto &e:mp2)sort(all(e.se));
    for(auto [x,v]:mp1){
            auto w = mp2[x];
            reverse(all(w));
            vector<pii> a;
            for(auto e:v)a.eb(1,e);
            for(auto e:w)a.eb(2,e);
            rep(i,w.size()) ans[rev[{2,{w[i],x}}]] = a[i];
            rep(i,v.size()) ans[rev[{1,{v[i],x}}]] = a[i + w.size()];
    }
    for(auto [x,v]:mp2){
