...
void solve() {
    cin >> n >> k >> s;
    F0R(i, n) s[i] = '0' + '1' - s[i];
    vm hash(n+1, 0), three(n+1, 1);
    FOR(i, 1, n+1) three[i]=three[i-1]*p;
    set<mi> ex;
    FOR(i, 1, n+1) {
        hash[i] = hash[i-1]*p + (int)(s[i-1] - '0');
        if (i>=k) ex.insert(hash[i] - hash[i-k]*three[k]);
    }
    ex.insert(hash[k]);
    str ans = ""; F0R(i, k) ans += '0'; mi h = 0;
    while (true) {
        if (ex.find(h)==ex.end()) {
            YES;
            cout << ans << nl;
