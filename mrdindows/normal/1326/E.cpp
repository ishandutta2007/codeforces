...
void update(int cur, int l, int r, int x, int val) {
    if (l == r) {
        ...
        int dcur = cur + cur;
        if (x <= m) {
            update(dcur, l, m, x, val);
        } else {
            update(dcur + 1, m + 1, r, x, val);
        }
        tr[cur].prefNeg = tr[dcur].prefNeg + max(0, tr[dcur + 1].prefNeg - tr[dcur].sufPos);
        tr[cur].sufPos = tr[dcur + 1].sufPos + max(0, tr[dcur].sufPos - tr[dcur + 1].prefNeg);
    }
}
//void addRange(int cur, int l, int r, int x, int y, int val) {
//    if (x > r || y < l) return;
//    if (x <= l && r <= y) {
//        pu[cur] += val;
//    } else {
//        int dcur = cur + cur;
//        int m = (l + r) / 2;
//        for (int to : {dcur, dcur + 1}) {
//            pu[to] += pu[cur];
//            tr[to] += pu[cur];
//        }
//        pu[cur] = 0;
//        addRange(dcur, l, m, x, y, val);
//        addRange(dcur + 1, m + 1, r, x, y, val);
