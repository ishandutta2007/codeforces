...
const int NN = (1 << 12) + 57;
pair<short,short> scpr[NN][NN];
int prsm[NN][NN];
int cnt[NN];

void precalc(vector<int>& w) {
	for (int i = 0; i < (1 << sz(w)); ++i) {
		for (int j = i; j < (1 << sz(w)); ++j) {
			int msk = (i ^ j);
			for (int k = 1; k <= sz(w); ++k) {
				scpr[i][j].x += w[sz(w) - k] * !(msk & 1);
				msk >>= 1;
			}
			scpr[j][i].x = scpr[i][j].x;
			scpr[i][j].y = j;
			scpr[j][i].y = i;
		}
	}
	for (int i = 0; i < (1 << sz(w)); ++i) {
		sort(scpr[i], scpr[i] + (1<<sz(w)));
	}
}
...
inline void solve() {
	...
	for (int i = 0; i < (1 << n); ++i) {
		for (int j = 1; j <= (1 << n); ++j) {
			prsm[i][j] = prsm[i][j - 1] + cnt[scpr[i][j - 1].y];
		}
	}
	for (int i = 0; i < q; ++i) {
		int t;
