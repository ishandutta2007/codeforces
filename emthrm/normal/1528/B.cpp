...
int main() {
  ...
  // int ans = 0;
  // auto fn = [&](auto &&fn, vector<pair<int, int>> &pr, vector<int> point) -> void {
  //   if (point.empty()) {
  //     REP(i, n) FOR(j, i + 1, n) {
  //       int x = i, y = j;
  //       if (pr[i].first > pr[j].first) swap(x, y);
  //       if (pr[x].second < pr[y].second && pr[i].second - pr[i].first != pr[j].second - pr[j].first) return;
  //     }
  //     ++ans;
  //     REP(i, n) cout << '(' << pr[i].first << ',' << pr[i].second << ") ";
  //     cout << '\n';
  //     return;
  //   }
  //   int a = point.front();
  ...
  ModInt sum = 2;
  FOR(i, 2, n + 1) {
    f[i] = sum + (divisor.query(i).size() - 1);
    sum += f[i];
  }
  cout << f[n] << '\n';
  return 0;
