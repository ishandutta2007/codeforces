...
struct Stairs{
	set<pair<int, pi>> blocks; //starting x - 0, y range - [0, L]
	mi cursum = 0;
	void init(){
		blocks.ins(mp(0, mp(0, L)));
	}
	void upd(pi corner){ //X1 >= x1
		assert(sz(blocks) > 0);
...
int y[mx];
int c[mx];
vpi cols[mx]; //coordinates in decreasing order
...
mi solve(int y){ //Y1 = y, Y2 goes from y+1 to L
	//dbg("SOLVE", y);
	vector<pair<int, pi>> upds; //x coordinate of when we update, upper right corner of the update
