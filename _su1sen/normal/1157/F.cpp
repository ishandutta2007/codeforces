...
int main() {
    ...
    int ans = 0;
    for (int i = 0; i <= M;) {
        while (i <= M and cnt[i] <= 1) ++i;
        if (i > M) break;
        int j = i;
        while (j <= M and cnt[j] >= 2) ++j;
        int p = i, q = j - 1;
        if (p > 0 and cnt[p - 1] > 0) --p;
        if (q < M and cnt[q + 1] > 0) ++q;
        if (chmax(ans, cum[q + 1] - cum[p])) {
            ml = p, mr = q;
        }
        i = j;
    }
