...
public class TaskB {
	...
	private void solve() {
		int dateCount = in.readInt();
		char[][] dates = new char[dateCount][];
		for (int i = 0; i < dateCount; i++)
			dates[i] = in.readString().toCharArray();
		int currentDate = 1000;
		List<Integer> correctDates = new ArrayList<Integer>();
		for (int i = 0; i < dateCount; i++) {
			int nextDate = 10000;
			for (int j = 0; j < 4; j++) {
				int original = dates[i][j] - '0';
				for (int k = 0; k < 10; k++) {
					dates[i][j] = (char) ('0' + k);
					int currentCandidate = Integer.parseInt(new String(dates[i]));
					if (currentCandidate >= currentDate)
						nextDate = Math.min(currentCandidate, nextDate);
				}
				dates[i][j] = (char) (original + '0');
			}
			correctDates.add(nextDate);
			currentDate = nextDate;
		}
		if (currentDate > 2011)
			out.println("No solution");
		else {
			for (int date : correctDates)
				out.println(date);
		}
	}
	...
	private static List<Test> createTests() {
		...
		tests.add(new Test("3\n" +
			...
			"5055\n" +
			"2000", "No solution"));
		tests.add(new Test(generateTest(), "No solution"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
