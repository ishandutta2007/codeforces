...
{
    if (popularity[a.fi] == popularity[b.fi])
        return a.fi < b.fi;
    return popularity[a.fi] < popularity[b.fi];
}

void solve()
{
    int n, x, y, x0, y0;
    cin >> n >> x >> y; y -= x, x0 = x, y0 = y;
    vector<int> a(n);
    forn(i, n) cin >> a[i], a[i]--;
    vector<vi> pos(n + 1);
    forn(i, n) pos[a[i]].push_back(i);
    set<pii, greater<pii> > cnts;
    forn(i, n + 1) cnts.insert({pos[i].size(), i});
    vector<int> ans(n, -1);
    int other = 0;
    forn(i, n + 1) if (pos[i].size() == 0) other = i;
    while (x--)
    {
        pii c = *(cnts.begin());
        cnts.erase(c);
        c.fi--;
        ans[pos[c.se].back()] = c.se;
        pos[c.se].pop_back();
        cnts.insert(c);
    }
    vector<pii> untouched;
    forn(i, n)
    {
        if (ans[i] == -1)
        {
            untouched.push_back({a[i], i});
        }
    }
    forn(i, n + 1)
    {
        popularity[i] = pos[i].size();
    }
    sort(all(untouched), comp);
    int plusic = untouched.size() / 2;
    int sel = untouched.size() - y;
    //forn(i, untouched.size()) cout << untouched[i].first << " " << untouched[i].second << "\n";
    for (int i = 0; i < untouched.size(); i++)
    {
        ans[untouched[i].second] = untouched[(i + plusic) % untouched.size()].first;
        if (ans[untouched[i].second] == a[untouched[i].second])
        {
            if (sel)
            {
                ans[untouched[i].second] = other;
                sel--;
            }
            else
            {
    ...
    while (sel)
    {
        if (ans[untouched.back().second] != other)
            ans[untouched.back().second] = other, sel--;
        untouched.pop_back();
    }
    cout << "YES\n";
    forn(i, n) cout << ans[i] + 1 << " "; cout << '\n';
}
