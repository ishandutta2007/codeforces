...
ll d(pll x, pll y) { return (x.fi - y.fi) * (x.fi - y.fi) + (x.se - y.se) * (x.se - y.se); }

vpll beet;
const ll INF = 1e18;
auto compare_y = [](const pll &a, const pll &b) { return a.se < b.se; };
vpll ps;
ll rec(int left, int right) {
    ...
    int ptr = 0;
    rep2(i, left, right - 1) {
        if((ps[i].fi - x) * (ps[i].fi - x) >= ret) continue;
        rep(j, ptr) {
            pll luz(ps[i].fi - beet[ptr - j - 1].fi, ps[i].se - beet[ptr - j - 1].se);
            if(luz.se * luz.se >= ret) break;
            chmin(ret, d(luz, pll()));
        }
        beet[ptr++] = ps[i];
    }
...
int main() {
    ...
    rep2(i, 1, n) ps.eb(i + r[i], i - r[i]);
    cout << closest_pair() / 2 << endl;
}