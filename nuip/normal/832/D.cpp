...
int main(){
  ...
  while(m--){
    ...
    int anc=-1;
    int done=0;
    for(pii p:vector<pii>{{a,b},{b,c},{a,c}}){
      int x=h.LCA(p.X,p.Y);
      int ok=1;
      for(pii p:vector<pii>{{a,b},{b,c},{a,c}})if(h.dist(p.X,x)+h.dist(p.Y,x)!=h.dist(p.X,p.Y)) ok=0;
      if(ok) {anc=x; break;}
    }
    if(done) continue;
    vector<int> abc{a,b,c}; sort(all(abc));
    do{
      a=abc[0]; b=abc[1]; c=abc[2];
    if(h.dist(a,b)+h.dist(b,c)==h.dist(a,c)){
      cout<<max(h.dist(a,b),h.dist(b,c))+1<<endl;
      done=1;
      break;
    }
    ...
    if(done) continue;
    int re=0;
    for(int x:vector<int>{a,b,c}) MX<int>(re,h.dist(x,anc));
    cout<<re+1<<endl;
  }
  return 0;
}