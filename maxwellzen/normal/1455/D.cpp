...
void solve() {
    cin >> n >> x;
    read(a, n);
    // a[i] = j and x=a[i], or x=j and a[i]=a[i]
    // int dp1[n][501], dp2[n][501];
    // fill(dp1[0], dp1[n], 1e9); fill(dp2[0], dp2[n], 1e9);
    // dp1[0][x]=1; dp2[0][x]=0;
    // FOR(i, 1, n) {
    //     F0R(j, a[i]+1) ckmin(dp2[i][a[i-1]], dp1[i-1][j]);
    //     F0R(j, 501) {
    //         if (j>=x) {
    //             // x=j and a[i]=a[i]
    //             if (a[i]>=a[i-1]) ckmin(dp2[i][j], dp2[i-1][j]);
    //         }
    //         if (j<=a[i]) {
    //             // a[i] = j and x = a[i], there was a swap, either j=a[i-1] or it's anything else 
    //             ckmin(dp1[i][j], dp2[i-1][j]+1);
    //             if (j>=a[i-1]) ckmin(dp1[i][j], dp2[i][j]+1);
    //         }
    //     }
    // }
    // F0R(i, n) {
    //     F0R(j, 6) cout << dp1[i][j] << ' '; cout << nl;
    // }
    // cout << nl;
    // F0R(i, n) {
    //     F0R(j, 6) cout << dp2[i][j] << ' '; cout << nl;
    // }
    // int ans = 1e9;
    // F0R(i, 501) ans = min({ans, dp1[n-1][i], dp2[n-1][i]});
