...
namespace MAIN{
    ...
    inline void MAIN(){
        n=read();
        for(res i=1;i<=n;i++)x[i]=0;
        for(res i=1;i<=n;i++)a[i]=mp(read(),i);
        for(res i=1;i<=n;i++)b[i]=mp(read(),i);
        sort(a+1,a+n+1),sort(b+1,b+n+1);
        reverse(a+1,a+n+1),reverse(b+1,b+n+1);
        for(res i=1;i<=n;i++)p[a[i].se]=i,q[b[i].se]=i;
        for(res i=1;i<=n;i++)mxp[i]=max(mxp[i-1],q[a[i].se]);
        for(res i=1;i<=n;i++)mxq[i]=max(mxq[i-1],p[b[i].se]);
        res flp=0,flq=0,now=1;
        for(res i=1;i<=2*n;i++){
            if(i&1){
                flq=max(flq,mxp[now]),now=q[a[now].se];
            }
            else flp=max(flp,mxq[now]),now=p[b[now].se];
        }
        now=1;
        for(res i=1;i<=2*n;i++){
            if(!(i&1)){
        ...
        for(res i=1;i<=n;i++){
            flq=max(flq,mxp[flp]),flp=max(flp,mxq[flq]);
        }
        for(res i=1;i<=flp;i++)x[a[i].se]=1;
        for(res i=1;i<=flq;i++)x[b[i].se]=1;
        for(res i=1;i<=n;i++)if(x[i])putchar('1');else putchar('0');
        puts("");
    }
}
int main(){
