...
void Process(int mask) {
	if (which[mask].second) {
		LL now = which[mask].first;
		LL temp = now;
		int prv = whr[temp];
		do {
			LL nxt = out[temp];
			ans[whr[temp]] = { temp, prv };
			prv = whr[temp];
			temp = out[temp];
		} while (temp != now);
		ans[whr[temp]] = { temp, prv };
...
void solve(int ks) {
	...
	for (int i = 0; i < k; i++) {
		...
		for (int j : V[i]) {
			LL rem = sum - (now - j);
			out[j] = rem;
			if (S.find(rem) != S.end()) INs[rem]++;
		}
	}

	queue<int> Q;
	for (int i : S) if (INs[i] == 0) Q.push(i);
	while (!Q.empty()) {
		int i = Q.front();
		Q.pop();
		LL o = out[i];
		if (S.find(o) == S.end()) continue;
		INs[o]--;
		if (INs[o] == 0) {
			Q.push(o);
		}
