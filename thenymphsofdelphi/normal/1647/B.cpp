...
int tests; cin >> tests; while (tests--){
    ...
    ForE(i, 1, n){
        ForE(j, 1, m){
            if (i > 1 and j > 1 and a[i][j] == 0 and a[i][j - 1] == 1 and a[i - 1][j] == 1 and a[i - 1][j - 1] == 1){
                ans = 0;
            }
            if (i > 1 and j < m and a[i][j] == 0 and a[i][j + 1] == 1 and a[i - 1][j] == 1 and a[i - 1][j + 1] == 1){
                ans = 0;
            }
            if (i < n and j > 1 and a[i][j] == 0 and a[i][j - 1] == 1 and a[i + 1][j] == 1 and a[i + 1][j - 1] == 1){
                ans = 0;
            }
            if (i < n and j < m and a[i][j] == 0 and a[i][j + 1] == 1 and a[i + 1][j] == 1 and a[i + 1][j + 1] == 1){
                ans = 0;
            }
        }
    }
