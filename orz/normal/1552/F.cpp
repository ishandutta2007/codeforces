...
bool lesss(const vi& a, const vi& b)
{
	int n = sz(a);
	int x = 0;
	for (int i = 0; i < n; ++i)
...
int another_main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		dp[i + 1] = sum(dp[i], way, M);
		int z = lower_bound(whole(x), y[i]) - x.begin();
		dp[i + 1] = raz(dp[i + 1], yy, M);
		if (z)
			dp[i + 1] = sum(dp[i + 1], x[z - 1], M);
		dp[i + 1] = raz(dp[i + 1], dp[z], M);
		dp[i + 1] = sum(dp[i + 1], dp[i], M);
		dp[i + 1] = sum(dp[i + 1], way, M);
