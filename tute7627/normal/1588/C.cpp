...
int main(){
  ...
  bool judge = true;
  ll t;cin>>t;
  auto sdec=add_min::make(1);
  using S=decltype(sdec);
  while(t--){
    ll n;cin>>n;
    vector<ll>a(n);
    vector<ll>b(n+1);
    rep(i,0,n){
      cin>>a[i];
      b[i+1]=b[i]+(i%2==1?-a[i]:a[i]);
    }
    map<ll,vector<ll>>idx;
    rep(i,0,n+1)idx[b[i]].PB(i);
    vector<S>seg;
    rep(i,0,2){
      seg.PB(add_min::make(n+2));
      rep(j,0,n+2){
        if(j%2==i){
          if(j<n)seg[i].set(j,0);
          else seg[i].set(j,-INF/2);
        }
        else{
          seg[i].set(j,INF/2);
        }
    ...
    rrep(i,0,n){
      rep(j,0,2){
        if(i%2==j%2)seg[j].update(i,n,a[i]);
        else seg[j].update(i,n,-a[i]);
      }
      ll mi=n;
      rep(j,0,2){
        auto check=[&](ll v){
        ...
        ll lim=seg[j].find_first(i,check);
        chmin(mi,lim);
      }
      ret+=upper_bound(ALL(idx[b[i]]),mi)-lower_bound(ALL(idx[b[i]]),i+1);
    }
    cout<<ret<<endl;
  }
  return 0;
