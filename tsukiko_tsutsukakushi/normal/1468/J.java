    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int t = 0; t < T; t++) {
            ...
            if (edge[0][2] >= K) {
                ...
                cp.println(ans);
            } else if (edge[M - 1][2] <= K) {
                cp.println(K - edge[M - 1][2]);
            } else {
                int small = 0;
                int big = 0;
                for (int n = 0; n < M - 1; n++) {
                    if (edge[n][2] <= K && K <= edge[n + 1][2]) {
                        small = n;
                        big = n + 1;
                    }
                }
                ...
                for (int n = 0; n <= small; n++) {
                    dsu.merge(edge[n][0], edge[n][1]);
                }
                if (dsu.groups().size() == 1) {
                    sp = K - edge[small][2];
                }
                long bp = 0;
                for (int n = small + 1; n < M; n++) {
                    if (!dsu.same(edge[n][0], edge[n][1])) {
                        dsu.merge(edge[n][0], edge[n][1]);
                        bp += edge[n][2] - K;
                    }
                }
