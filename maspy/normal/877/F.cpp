...
void solve() {
  ...
  VEC(ll, C, N);
  VEC(ll, A, N);
  FOR(i, N) if (C[i] == 2) A[i] *= -1;
  auto S = cumsum(A);
  auto key = S;
  UNIQUE(key);
  for (auto&& s: S) s = LB(key, s);
  ll K = len(key);
  vi TO(K, -1);
  vi FRM(K, -1);
  FOR(i, K) {
  ...
  auto ADD_L = [&](int i) -> void {
    ll s = S[i];
    if (TO[s] != -1) ans += CNT[TO[s]];
    CNT[s]++;
  };
  auto ADD_R = [&](int i) -> void {
    ll s = S[i + 1];
    if (FRM[s] != -1) ans += CNT[FRM[s]];
    CNT[s]++;
  };
  ...
  auto CALC = [&](int q) -> void { ANS[q] = ans; };
