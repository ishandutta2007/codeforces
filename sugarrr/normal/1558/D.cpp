...
struct S{ll val;ll sz;};
S op(S a,S b){return S{a.val+b.val,a.sz+b.sz};}
S e(){return S{0,0};}
struct F{ll val;bool f;};//ff=falseval
S mapping(F f,S x){
    if(f.f)return {f.val*x.sz,x.sz};
    else return x;
}
F composition(F f,F g){
    if(f.f)return F{f.val,true};
    else return g;
}
F id(){return F{3,false};}//3
//vector<S>init(n,{0,1});//0
...
ll target;
bool f(S s){
    return s.sz-s.val < target;
}

void solve(){
    seg.apply(0,N,{0,true});
    
    ll n,m;cin>>n>>m;
    vl_l a(m);rep(i,0,m-1)cin>>a[i].fi>>a[i].se;
    reverse(all(a));
    ...
    for(auto X:a){
        ll x=X.se;
        target=x;
        ll pos=seg.max_right<f>(0);
        if(pos-1>=0 && mp.find(pos-1)==mp.end()){
            mp[pos-1]=0;
        }
  ...
  //  for(auto x:mp)cerr<<x.fi<<" "<<x.se<<endl;cerr<<endl;
    ll A=0,B=0;
    for(auto x:mp){
        if(x.se==1){
            B++;
        }
        assert(0<=x.fi && x.fi<=n-2);
    }
    A=n-1-B;
    assert(0<=A && A<=n-1);
    cout<<comb(n+A,A).val()<<endl;
}
