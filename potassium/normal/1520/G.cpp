...
int main() {
    int n,m; ll w; R(n,m,w);
    vector<vector<ll>> a(n, vector<ll>(m)); R(a);

    vector<vector<int>> distX(n, vector<int>(m, INT_MAX)), distY = distX;
    const int dr[] = { -1, 1, 0, 0 };
    const int dc[] = { 0, 0, -1, 1 };
    auto bfs = [&](vector<vector<int>>& dist, int r, int c) {
        queue<pii> q;
        dist[r][c] = 0;
        q.push({ r, c });
        ...
        while (!q.empty()) {
            ...
            for (int dir = 0; dir < 4; dir++) {
                int toR = rr + dr[dir];
                int toC = cc + dc[dir];
                if (0 <= toR && toR < n && 0 <= toC && toC < m && a[toR][toC] != -1 && chkmin(dist[toR][toC], dist[rr][cc] + 1))
                    q.push({ toR, toC });
            }
        }
    };
    ...
    ll ans = LLONG_MAX;
    if (distY[0][0] != INT_MAX)
        chkmin(ans, distY[0][0] * w);

    ll bestX = LLONG_MAX;
    ll bestY = LLONG_MAX;
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
            if (distX[i][j] != INT_MAX && a[i][j] > 0)
                chkmin(bestX, distX[i][j] * w + a[i][j]);
            if (distY[i][j] != INT_MAX && a[i][j] > 0)
                chkmin(bestY, distY[i][j] * w + a[i][j]);
        }

    if (bestX != LLONG_MAX && bestY != LLONG_MAX)
        chkmin(ans, bestX + bestY);

    W(ans == LLONG_MAX ? -1 : ans);
