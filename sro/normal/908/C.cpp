...
typedef priority_queue<PII,VPII,greater<PII> > XQP;
//Circle with r=1: x^2+y^2=1

int n;
double r,x[1007],y[1007];

double yof(int now,int down)
{
	double xwz=(x[now]+x[down])/2;
	double yy=sqrt(r*r-(xwz-x[down])*(xwz-x[down]));
	yy+=y[down];
	yy+=yy-y[down];
	return yy;
}
...
void choose(int now)
{
	double maxv=r;
	for(int i=0;i<now;i++)
		if(x[now]+r>=x[i]-r||x[now]-r<=x[i]+r)
			maxv=max(maxv,yof(now,i));
	y[now]=maxv;
}
...
int main()
{
	...
	for(int i=0;i<n;i++)printf("%.10f%c",y[i],i==n-1?'\n':' ');
	return 0;
}