...
using ARR = PersistentArray<ll>;
using np = typename ARR::np;
...
void solve() {
  ...
  vi ANS = {0};
  vc<np> A = {PA.get_root()};
  vc<np> FLIP = {PA.get_root()};
  vc<np> CNT = {PA.get_root()};
  ...
  FOR3(q, 1, Q + 1) {
    ...
    if (t == 1 || t == 2) {
      LL(x, y);
      --x, --y;
      ll now = PA.get(A.back(), ID(x, y));
      ll flip = PA.get(FLIP.back(), x);
      if ((t == 1 && (now ^ flip) == 1) || (t == 2 && (now ^ flip) == 0)) {
        ANS.eb(ANS.back());
        A.eb(A.back());
        FLIP.eb(FLIP.back());
        CNT.eb(CNT.back());
        ...
        ll cnt = PA.get(CNT.back(), x);
        ANS.eb(ANS.back() + 1);
        A.eb(PA.set(A.back(), ID(x, y), (flip ? 0 : 1)));
        FLIP.eb(FLIP.back());
        CNT.eb(PA.set(CNT.back(), x, cnt + 1));
      }
      elif (t == 2) {
        ll cnt = PA.get(CNT.back(), x);
        ANS.eb(ANS.back() - 1);
        A.eb(PA.set(A.back(), ID(x, y), (flip ? 1 : 0)));
        FLIP.eb(FLIP.back());
        CNT.eb(PA.set(CNT.back(), x, cnt - 1));
      }
    }
      ...
      ll flip = PA.get(FLIP.back(), x);
      ll new_cnt = W - cnt;
      ANS.eb(ANS.back() - cnt + new_cnt);
      A.eb(A.back());
      FLIP.eb(PA.set(FLIP.back(), x, flip ^ 1));
      CNT.eb(PA.set(CNT.back(), x, new_cnt));
    }
    elif (t == 4) {
      LL(k);
      ANS.eb(ANS[k]);
