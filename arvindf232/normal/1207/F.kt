...
fun main(){
    solve.cases{
        ...
        val arr = IntArray(c)
        val store = Array(cut+1){IntArray(it){0} }
        ...
        repeat(n){
            ...
            if(type == 1){
                ...
                if(y <= cut){
                    ...
                    var ret = 0
                    val start = if(x == 0) y else x
                    for(i in start..c step y){
                        ret += arr[i-1]
                    }
                    put(ret)
                }
...
inconsistent indices: array and store
    better to use 1 based if any is 1 based
    then off by 1 on final bounds
 */