...
int main() {
	...
	vector<int> evenCycles;
	vector<int> oddCycles;

	vector<int> cycleSize(n, -1);
	...
	for (int i = 0; i < n; i++) {
		if (!trav[i]) {
			...
			(sz % 2 ? oddCycles : evenCycles).emplace_back(i);
	...
	sort(evenCycles.begin(), evenCycles.end(), [&](int u, int v) { return cycleSize[u] < cycleSize[v]; });

	vector<int> ans(n, -1);
	bool can = true;
	while (evenCycles.size() >= 2) {
		int u = evenCycles.back(); evenCycles.pop_back();
		int v = evenCycles.back(); evenCycles.pop_back();

		if (cycleSize[u] != cycleSize[v]) {
			can = false;
			break;
		}
	...
	while (!oddCycles.empty()) {
		int u = oddCycles.back(); oddCycles.pop_back();

		vector<int> cycle = { u };
		while (a[cycle.back()] != u) {
			cycle.emplace_back( a[cycle.back()] );
		}
		
		assert((int)cycle.size() == cycleSize[u]);

		const int JUMP = (cycleSize[u] + 1) / 2;
		for (int i = 0; i < cycleSize[u]; i++)
			ans[cycle[i]] = cycle[(i + JUMP) % cycleSize[u]];
	}
