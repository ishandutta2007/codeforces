...
void link(int a, int b, int idx) {
    ...
    dbg(comps[y]);
    split_tgt[idx] = y;
    for (int elem : comps[y]) {
        comps[x].push_back(elem);
        split[idx].push_back(elem);
    }
    dbg(idx, split[idx]);
}
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (queries[i].first == 2) {
            edges[queries[i].second].first = i;
        }
    }
    ...
    split = vector<vi>(q+2);
    split_tgt = vi(q+2);
    for (int i = 0; i < n; i++) comps[i].push_back(i);

    for (int i = 0; i < m; i++) {
        link(edges[i].second.first, edges[i].second.second, edges[i].first);
    }

    vector< set<int> > vals(n);
    for (int i = 0; i < n; i++) vals[find(i)].insert(-val[i]);
    ...
    for (int _ = 0; _ < q; _++) {
        ...
        if (t == 1) {
            int comp_idx = find(x);
            dbg(x, comp_idx);
            if (vals[comp_idx].empty()) {
                cout << 0 << '\n';
            }
            else {
                int f = -(*vals[comp_idx].begin());
                cout << f << '\n';
                val[rev[f]] = 0;
                vals[comp_idx].erase(vals[comp_idx].begin());
            }
        }
        else if (!split[_].empty()) {
            int old_comp = find(split[_].front());
            int new_comp = split_tgt[_];

            dbg(_, split[_]);
            for (int elem : split[_]) {
                if (val[elem] != 0) {
                    vals[new_comp].insert(-val[elem]);
                    vals[old_comp].erase(-val[elem]);
                }

                dbg(elem);
                if (elem == new_comp) par[elem] = -sz(split[_]);
                else par[elem] = new_comp;
            }
        }
    }
