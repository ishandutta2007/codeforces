...
int main() {
	int a,b,h,w,n; R(a,b,h,w,n);
	vector<int> ex(n); R(ex);
	sort(ex.rbegin(), ex.rend());

	int cnt2 = 0, cnt3 = 0;
	while (!ex.empty() && ex.back() == 2) {
		ex.pop_back();
		cnt2++;
	}
	while (!ex.empty() && ex.back() == 3) {
		ex.pop_back();
		cnt3++;
	}

	const int MAXN = 100000;
	vector<int> need2(MAXN + 1, 0), need3(MAXN + 1, 0);
	need2[1] = 0;
	for (int i = 1; i <= MAXN; i *= 2) need2[i + 1] = need2[i / 2 + 1] + 1;
	for (int i = 2; i <= MAXN; i++) chkmax(need2[i], need2[i - 1]);
	need3[1] = 0;
	for (int i = 1; i <= MAXN; i *= 3) need3[i + 1] = need3[i / 3 + 1] + 1;
	for (int i = 2; i <= MAXN; i++) chkmax(need3[i], need3[i - 1]);

	vector<int> pw3(cnt3 + 1, 1);
	for (int i = 1; i <= cnt3; i++)
		pw3[i] = min(MAXN, pw3[i - 1] * 3);

	const int MAXSZ = 16;
	if ((int)ex.size() > MAXSZ)
		ex.resize(MAXSZ);
	
	n = (int)ex.size();
	const int FULL = 1 << n;
	vector<int> val(FULL, 1);
	for (int mask = 1; mask < FULL; mask++) {
		int lb = mask & -mask;
		int b = __lg(lb);
		val[mask] = min(MAXN, 1LL * val[mask ^ lb] * ex[b]);
	}

	vector<vector<int>> maxv(n + 1, vector<int>(FULL, 1));
	for (int mask = 0; mask < FULL; mask++) {
		maxv[__builtin_popcount(mask)][mask] = val[mask];
		for (int b = 0; b < n; b++) {
			if (mask >> b & 1) {
				for (int c = 0; c <= n; c++) {
					chkmax(maxv[c][mask], maxv[c][mask ^ (1 << b)]);
				}
			}
		}

		for (int c = 1; c <= n; c++)
			chkmax(maxv[c][mask], maxv[c - 1][mask]);
	}

	int ans = INT_MAX;
	for (int i = 0; i < 2; i++) {
		for (int mask = 0; mask < FULL; mask++) {
			int x = min(MAXN, 1LL * h * val[mask]);
			int na = (a + x - 1) / x;
			for (int c = 0; c <= n; c++) {
				int y = min(MAXN, 1LL * w * maxv[c][(FULL - 1) ^ mask]);
				int nb = (b + y - 1) / y;

				if (need3[na] + need3[nb] <= cnt3) {
					chkmin(ans, __builtin_popcount(mask) + c + need3[na] + need3[nb]);
				}
				else {
					for (int p3 = 0; p3 <= cnt3; p3++) {
						int na3 = (na + pw3[p3] - 1) / pw3[p3];
						int nb3 = (nb + pw3[cnt3 - p3] - 1) / pw3[cnt3 - p3];
						if (need2[na3] + need2[nb3] <= cnt2) {
							chkmin(ans, __builtin_popcount(mask) + c + cnt3 + need2[na3] + need2[nb3]);
						}
					}
				}
			}
