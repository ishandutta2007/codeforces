...
int main() {
    ...
    loop(t) {
        ...
        vector<map<long long, long long>> vs(n);
        auto dfs = [&](auto dfs, int u, long long x, long long acc) -> long long {
            acc += s[u];

            if (auto it = vs[u].find(x); it != vs[u].end()) return it->second;

            int c = g[u].size();

            if (c) {
                long long q = x / c, r = x % c;

                long long res = 0;
                vector<long long> as, bs;
                for (int v : g[u]) {
                    long long base = dfs(dfs, v, q, acc);
                    res += base;
                    if (r) {
                        bs.push_back(dfs(dfs, v, q + 1, acc) - base);
                    }
                }
                sort(all(bs), greater<>());
                return vs[u][x] = res + accumulate(bs.begin(), bs.begin() + r, 0LL);
            } else {
                return vs[u][x] = acc * x;
            }
        };
