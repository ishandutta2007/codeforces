...
int main(){
  ...
  rep(i,0,n){
    ll k;cin>>k;
    x[i].resize(k);
    rep(j,0,k)cin>>x[i][j],x[i][j]--;
  }
  UnionFind uf(m);
  rep(i,0,n){
    if(x[i].size()==1){
      auto r=uf.root(x[i][0]);
      if(t[r])continue;
      t[r]=true;
      ret.PB(i+1);
    }
    else{
      if(uf.find(x[i][0],x[i][1]))continue;
      ll r1=uf.root(x[i][0]),r2=uf.root(x[i][1]);
      if(t[r1]&&t[r2])continue;
      bool tr=t[r1]||t[r2];
      ret.PB(i+1);
      uf.unite(r1,r2);
      t[uf.root(r1)]=tr;
  ...
  cout<<pow(2LL,(ll)ret.size()) spa ret.size()<<endl;
  debug(ret,ret.size());
  return 0;
}