// Hidden Permutations (https://codeforces.com/contest/1621/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        rep(i, n) {
            if (p[i] != -1)
                continue;
            vector<int> cyc = {ask(i), ask(i)};
            while (cyc[0] != cyc.back())
                cyc.push_back(ask(i));
            cyc.pop_back();

            // cyc[0] = (p^{off})_i
            int m = sz(cyc);
            int shift = ((1 - off) % m + m) % m;
            rotate(begin(cyc), begin(cyc) + shift, end(cyc));
