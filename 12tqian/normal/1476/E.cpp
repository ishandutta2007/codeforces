...
int main() {
    ...
    vector<string> s(m);
    vi mt(m);
    f0r(i, m) re(s[i], mt[i]), mt[i]--;

    f0r(i, m) {
        f0r(j, k) {
            if (p[mt[i]][j] == '_') continue;
            else if (p[mt[i]][j] != s[i][j]) {
                ps("NO");
                return 0;
            }
        }
    ...
    auto ID = [&](string s) -> int {
        return lower_bound(all(tot), s) - tot.begin();
    };

    int sz = sz(tot);
    vector<vi> adj(sz(tot));
    f0r(i, m) {
        int cnt = 0;
        f0r(mask, (1<<k)) {
            string use = s[i];
            ...
            if (use != p[mt[i]]) {
                adj[ID(p[mt[i]])].eb(ID(use));
            }
            if (binary_search(all(ot), use)) cnt++;
        }
        if (cnt == 0) {
            ps("NO");
            return 0;
