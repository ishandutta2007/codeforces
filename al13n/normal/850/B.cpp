...
int main(int argc, char **argv){
  ...
  LL n,x,y;
  cin>>n>>x>>y;
  const LL lim=1001000;
  vector<LL> C(lim),S(lim);
  forn(i,n){
    int a;
    cin>>a;
  ...
  for(LL g=2;g<lim;++g){
    LL k=min(x/y+1,g);
    LL r=0;
    for(LL i=g;i-g+1<lim;i+=g){
      LL c2=C[min(i,lim-1)]-C[min(i-k,lim-1)];
      LL s2=S[min(i,lim-1)]-S[min(i-k,lim-1)];
      LL c1=C[min(i-k,lim-1)]-C[i-g];
      LL t=(c2*i-s2);
      assert(t>=0);
      if(t>LINF/y) {
        r=LINF;
