...
void solve()
{
    ...
    vector<map<int, vector<int>>> b(30);
    for (int iter = 29; iter>=0; iter--)
    {
        int mask = 0;
        for (int i = iter; i<=29; i++) mask+=(1<<i);
        for (int i = 0; i<n; i++) b[iter][a[i]&mask].push_back(i);
    }
    ...
    while (q--)
    {
        ...
        {
            //cout<<iter<<": ";
            int tmp = cnt(b[iter][base], l, r);
            int fromx = 0;
            vector<int> tmpx;

            for (auto it: x) if ((it&(1<<iter))==0)
            {
                fromx++;
                tmpx.push_back(it);
            }
            //cout<<x<<' '<<base<<' '<<ans<<' '<<tmp<<' '<<fromx<<endl;
            ...
            else
            {
                if (tmp==1)
                {
                    int pos = lower_bound(b[iter][base].begin(), b[iter][base].end(), l) - b[iter][base].begin();
                    x.push_back(a[b[iter][base][pos]]);
                }
                base+=(1<<iter);
                ans+=(1<<iter);
            }
