...
ll n,q,a,b,va,x[MAX],ans[MAX],sm[MAX],val[MAX];
vii qu[MAX];
deque<pii> dq;
ll tree[1<<19],lz[1<<19];
...
//	freopen("output.txt","w",stdout);
	...
	rap(i,n,1){
		for(auto j:qu[i]){
			le = max(val[i-1], (ll)i-j.fi);
			it = lower_bound(all(dq), (pii){le,0});
			res = sm[i-1] + le + tree[1];
		...
		le = i-x[i];
		while(dq.front().fi<=le)dq.pof();
		a = i, b = dq.front().se-1, va = le;
		upd(1,1,n);
		dq.pf({le,i});
	}
	res = n*(n+1)/2;
	rep(i,1,q)cout<<res-ans[i]<<endl;
	return 0;
}