...
bool solve() {
	if (*std::max_element(b+1, b+1+n) == 0) return true;
	int mx_pos = std::max_element(b+1, b+1+n) - b;
	int should_dec = a[mx_pos] - b[mx_pos];
	if (should_dec < 0) return false;
	For(i,n) {
		int aft = std::max(a[i]-should_dec, 0);
		if (aft != b[i]) return false;
	}
	return true;
}
