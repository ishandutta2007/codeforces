...
int main() {
    ...
    loop(t) {
        ...
        const int c = gs.size();
        rep(i, c) {
            for (auto v : gs[i]) ids[v] = i;
        }
        vector<vector<tuple<int, int, int>>> g2(c);
        for (auto &[u, v] : es) {
            int ui = ids[u], vi = ids[v];
            g2[ui].emplace_back(vi, u, v);
            g2[vi].emplace_back(ui, u, v);
        }

        atcoder::dsu uf2(2 * c + 2);
        bool ok = true;
        rep(i, m) {
            input(int, u, v, w);
            --u, --v;
            ...
            if (dif) {
                ...
                ok &= not uf2.same(u, v + c);
                uf2.merge(u, v);
                uf2.merge(u + c, v + c);
            }
        }
        rep(i, c) {
            ok &= not uf2.same(i, i + c);
        }
        if (not ok) {
            print("NO");
            continue;
        }
        const int W = 2 * c, B = W + 1;
        rep(i, c) {
            if (uf2.same(i, W) or uf2.same(i, B)) continue;
            uf2.merge(i, W);
            uf2.merge(i + c, B);
        }
        ...
        auto dfs2 = [&](auto dfs2, int u, int p) -> void {
            int ux = uf2.same(u, B);
            for (auto &[v, ur, vr] : g2[u]) {
                if (v == p) continue;
                int vx = uf2.same(v, B);
                ans.emplace_back(ur, vr, ux ^ vx);
                dfs2(dfs2, v, u);
            }
        };
        dfs2(dfs2, 0, -1);
