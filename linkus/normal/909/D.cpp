...
int main(){
    ...
    vector<list<pair<char,int>>::iterator> vec;
    int cnt = 1;
    FOR(i,1,s.size()){
        if(s[i] != s[i-1]){
            ls.push_back(mp(s[i-1],cnt));
            cnt = 1;
        }
        else{
            ++cnt;
    ...
    ls.push_back(mp(s.back(),cnt));
    int ans = 0;
    while(ls.size() > 1){
        ++ans;
        /* whatis(ls) */
        auto till = prev(ls.end());
        for(auto it = ls.begin(); it != till; ++it){
            /* if(it->first != next(it)->first){ */
                if(!--it->second)
                    vec.push_back(it);
                if(!--next(it)->second)
                    vec.push_back(next(it));
            /* } */
        }
        FORR(i,vec){
            ls.erase(i);
        ...
        till = prev(ls.end());
        /* whatis(ls) */
        for(auto it = ls.begin(); it != ls.end() && next(it) != ls.end();){
            if(it->first == next(it)->first){
                ls.insert(it,{it->first,it->second+next(it)->second});
                --it;
                ls.erase(next(it));
                ls.erase(next(it));
            }
