...
struct T{
    ...
    T& operator+=(const T& a){
        ll val=a.first+1;
        if(first<val)swap(first,val);
        if(second<val)swap(second,val);
        if(third<val)swap(third,val);
        return *this;
    }
    T& operator-=(const T& a){
        ll val=a.first+1;
        if(first==val)swap(first,second);
        return *this;
    }
...
int solve(){
    ...
    vec(bool,junction,n);
    vec(bool,issnake,n);
    each(i,snake)issnake[i]=1;
    rep(n)junction[i]=dp[i].third>=length;
    ll l=depth[b].first,r=length-down[snake[1]].first;
    rep(length*2){
        if(l>=r+1)break;
        if(i&1){
            ll at=snake[r];
            l=depth[at].first-(length-r);
        }
        elif(l+1<=length){
            r=length-(down[snake[l+1]].first-l);
        }
    }
    if(l>=r+1)l=r=0;
    rep(i,0,l+1){
        ll at=snake[i];
        auto dfs=[&](ll from,ll at,const auto&dfs)->bool{
            if(junction[at])return 1;
            each(i,g[at])if(i!=from&&!issnake[i]){
                if(dfs(at,i,dfs))return 1;
            }
            return 0;
        };
        if(dfs(-1,at,dfs))return YES();
    }
    rep(i,r,length+1){
        ll at=snake[i];
        auto dfs=[&](ll from,ll at,const auto&dfs)->bool{
