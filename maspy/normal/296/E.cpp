...
void solve() {
  ...
  vvv(ll, COST, N1 + 1, N2 + 1, 2, INF);
  vvv(mint, DP, N1 + 1, N2 + 1, 2, 0);
  COST[N1][N2][0] = 0;
  DP[N1][N2][0] = 1;
  deque<tuple<ll, ll, ll>> que;
  que.push_back(mt(N1, N2, 0));

  while (!que.empty()) {
    auto [n1, n2, t] = que.front();
    que.pop_front();
    ...
    FOR(a, n1 + 1) FOR(b, n2 + 1) {
      ...
      ll m1 = N1 - (n1 - a);
      ll m2 = N2 - (n2 - b);
      mint coef = MC.C(n1, a) * MC.C(n2, b);
      if (chmin(COST[m1][m2][t ^ 1], COST[n1][n2][t] + 1)) {
        que.push_back(mt(m1, m2, t ^ 1));
        DP[m1][m2][t ^ 1] = coef * DP[n1][n2][t];
      }
      elif (COST[m1][m2][t ^ 1] == COST[n1][n2][t] + 1) {
        DP[m1][m2][t ^ 1] += coef * DP[n1][n2][t];
      }
    }
  }
