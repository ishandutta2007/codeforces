...
S f(S x, S y) {
    S s;
    rep(i, 2) rep(j, 2) s[i][j] = inf<ll>;
    rep(i, 2) rep(j, 2) rep(k, 2) { chmin(s[i][k], x[i][j] + y[j][k]); }
    return s;
}
S e() {
    S s;
    rep(i, 2) rep(j, 2) s[i][j] = (i == j ? 0 : inf<ll>);
    return s;
}
...
int main() {
    ...
    vector<S> setter(n - 1);
    auto d = [](pii x, pii y) { return abs(x.fi - y.fi) + abs(x.se - y.se); };
    rep(i, n - 1) {
        S s;
        rep(x, 2) rep(y, 2) { s[x][y] = 1 + d(a[i][x] + pii(x ? 0 : 1, x ? 1 : 0), a[i + 1][y]); }
        setter[i] = s;
    }
    segtree<S, f, e> seg(setter);
    ...
    rep(q) {
        ...
        if(max(x1, y1) == max(x2, y2)) {
            OUT(d(pii(x1, y1), pii(x2, y2)));
            continue;
        }
        if(max(x1, y1) > max(x2, y2)) swap(x1, x2), swap(y1, y2);
        auto s = seg.prod(max(x1, y1), max(x2, y2) - 1);
        ll mi = inf<ll>;
        // dump(a[max(x1, y1)][0], x1, y1);
        rep(i, 2) rep(j, 2) { chmin(mi, 1 + s[i][j] + d(pii(x1, y1), a[max(x1, y1)][i]) + d(pii(x2, y2), a[max(x2, y2) - 1][j] + pii(j ^ 1, j))); }
        OUT(mi);
    }
}