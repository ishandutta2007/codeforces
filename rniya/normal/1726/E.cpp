...
mint solve(int n) {
    mint ans = 0;
    vector<mint> dp1(n + 1);  // i  2 
    dp1[0] = 1;
    for (int i = 1; i <= n; i++) dp1[i] = dp1[i - 1] + (i - 2 >= 0 ? dp1[i - 2] * (i - 1) : 0);
    vector<mint> dp2(n + 1);
    dp2[0] = 1;
    for (int i = 2; i <= n; i += 2) dp2[i] = dp2[i - 2] * (i - 1);
    for (int i = 0; i * 4 <= n; i++) {  // 2 * 2  i 
        ans += mint(2).pow(i) * BINOM.C(n - 2 * i, 2 * i) * dp1[n - 4 * i] * dp2[2 * i];
    }
