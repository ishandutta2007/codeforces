...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (coma.find(a[i]) != coma.end())
        {
            coma.erase(coma.find(a[i]));
        }
        else
        {
            cnta[a[i].size()]++;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (comb.find(b[i]) != comb.end())
        {
            comb.erase(comb.find(b[i]));
        }
        else
        {
            cntb[b[i].size()]++;
    ...
    for (int i = 2; i < 10; i++)
    {
        ans += max(cnta[i], cntb[i]) - min(cnta[i], cntb[i]);
    }
    cout << ans << "\n";
}
