...
void solve() {
  ...
  FOR(x, lim + 1) {
    ...
    vi nbd;
    FORIN(i, I) {
      if (i > 0 && A[i - 1] != x) nbd.eb(A[i - 1]);
      if (i + 1 < N && A[i + 1] != x) nbd.eb(A[i + 1]);
    }
    if (len(nbd) == 0) continue;
    sort(all(nbd));
    ll mid = nbd[len(nbd) / 2];
    ll before = 0, after = 0;
    FORIN(a, nbd) {
      before += abs(x - a);
      after += abs(mid - a);
    }
    chmin(ANS, base - before + after);
  }
  print(ANS);
}
