...
void add_or(int a, int b) {
	debug(imie(a) ^ 1, imie(b));
	graf[a ^ 1].push_back(b);
	debug(imie(b) ^ 1, imie(a));
...
void dfs2(int x, int nr) {
	...
	com_nr[x] = nr;
	debug(arr(com_nr, x));
	for (int y : graf[x]) dfs2(y, nr);
}
int make_true(int a) {
	return 2 * a;
}
bool check_true(int a) {
	return a % 2 == 0;
}
bool check_false(int a) {
	return a % 2 == 1;
}
int make_false(int a) {
	return 2 * a + 1;
}
bool err[nax];
int main() {
	...
	while (t--) {
		...
		for (int i = 0; i < m; ++i)
			scanf("%d%d", FS(&in[i]));
		bool found = false;
		const int arr_siz = 2 * (n + 2);
		for (int b = 0; b < 2; ++b) {
			for (int i = 0; i <= arr_siz; ++i) graf[i].clear();
			order.clear();
			memset(vis, 0, sizeof(vis[0]) * arr_siz);
			memset(com_nr, 0, sizeof(com_nr[0]) * arr_siz);
			memset(err, 0, sizeof(err[0]) * arr_siz);
			for (pii x : in) {
				if (b) swap(FS(x));
				add_or(make_false(x.first), make_true(x.second));
			}
			add_or(make_true(1), make_true(1));
			...
			for (int x : order) {
				if (com_nr[x] == 0) {
					dfs2(x, ++com_cou);
					is_true[com_cou] = (x & 1) == 0;
				}
			}
			for (int i = 2; i <= 2 * n + 1; ++i) {
				if (check_false(i))
					for (int x : graf[i])
						if ((com_nr[x] != com_nr[i]) && (check_false(x) || com_nr[x] == com_nr[i ^ 1])) {
							debug(imie(x), imie(i), "err on ", arr(com_nr, i));
							err[com_nr[i]] = 1;
						}
			}
			bool flipped = false;
			for (int i = com_cou; i >= 1; --i) {
				if (!is_true[i] && !err[i]) {
					debug("flip", imie(i));
					flipped = true;
					is_true[i] = 1;
			...
			if (flipped) {
				...
				for (int i = 1; i <= n; ++i) {
					debug(imie(i), arr(is_true, arr(com_nr, make_false(i))));
					if (!is_true[com_nr[make_false(i)]])
						trues.push_back(i);
					else
						falses.push_back(i);
				}
				if (b) swap(trues, falses);
				debug(imie(trues), imie(falses));
				assert(!trues.empty() && !falses.empty());
				printf("Yes\n%d %d\n", (int)trues.size(), (int)falses.size());
				for (int x : trues) printf("%d ", x);
				printf("\n");
				for (int x : falses) printf("%d ", x);
				printf("\n");
				found = true;
				break;
			}
