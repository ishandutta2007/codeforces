#line 1 "codeforces-workspace\\contest\\543\\a\\a.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\543\\a\\a.cpp"
...
#line 21 "codeforces-workspace\\contest\\543\\a\\a.cpp"
...
void main() {
  ...
  int n, m, b;
  cin >> n >> m >> b >> mint::mod;
  auto dp = make_vector<mint>({m + 1, b + 1});
  dp[0][0] = 1;
  for (auto i = 0; i != n; ++i) {
    int a;
    cin >> a;
    for (auto j = 0; j < m; j++) {
      for (auto k = 0; k <= b; k++) {
        if (k + a <= b) dp[j + 1][k + a] += dp[j][k];
      }
    }
  }
  cout << accumulate(begin(dp[m]), end(dp[m]), mint()) << eol;
}
}