...
struct trie_node {
	bool is_solo = true, is_end = false;
	trie_node *nxt[26];
	trie_node() { memset(nxt, 0, sizeof nxt); }
};

vector<trie_node *> upd;
int ok;

struct trie {
	trie_node *root;
	trie() { root = new trie_node; root->is_solo = false; }
	void proceed(string &s, int st, int end) {
		auto cur = root;
		upd.clear();
		ok = 0;
		for (int i = st; i < end; ++i) {
			if (cur->nxt[s[i] - 'a']) {
				upd.pb(cur->nxt[s[i] - 'a']);
				cur = cur->nxt[s[i] - 'a'];
			}
			else {
				if (cur->is_end == true) ok = 1;
				cur = cur->nxt[s[i] - 'a'] = new trie_node;
			}
		}
		if (cur->is_end) ok = 2;
		cur->is_end = true;
...
inline void solve(ld tt) {
	...
	while (getline(cin, s)) {
		...
		for (int i = 0; i < sz(s); ++i) {
			...
			int j;
			for (j = i; j < sz(s) && !(s[j] == ' ' || s[j] == '-' || s[j] == '\'' || s[j] == ',' || s[j] == '.' || s[j] == '?' || s[j] == '!'); ++j);
			tr.proceed(s, i, j);
			int lol = -1;
			for (auto &ptr : upd) {
				if (ok != 0) lol += ptr->is_solo;
				if (ok != 2) {
					ptr->is_end = 0;
					ptr->is_solo = 0;
				}
