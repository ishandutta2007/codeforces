...
void solve() {
  LL(N, sigma);
  STR(S);
  if (N == 1) return print(sigma - 1);

  auto A = s_to_vi(S, 'a');
  A.eb(0);
  /*
  LCS dp dp[k][k-1], dp[k][k], dp[k][k+1] 
  (k-2,k-1) / (k-1,k) / (k-1,k)
   8 
  */
  vvv(ll, DP, 2, 2, 2);
    ...
    if (k == 1) {
      ...
      FOR(s, sigma) {
        FOR(a, 2) FOR(b, 2) FOR(c, 2) {
          ...
          ll x = max(k - 2 + b, k - 3 + a + (s == A[k - 2]));
          ll y = max({x, k - 2 + c, k - 2 + b + (s == A[k - 1])});
          ll z = max(y, k - 2 + c + (s == A[k]));
          x -= (k - 2);
          y -= (k - 1);
          z -= (k - 1);
          if (x < 0 || y < 0 || z < 0) continue;
          assert(x < 2 && y < 2 && z < 2);
          newDP[x][y][z] += DP[a][b][c];
        }
      }
      swap(DP, newDP);
    }
  ...
  ll ANS = 0;
  FOR(a, 2) FOR(b, 2) FOR(c, 2) if (b == 0) ANS += DP[a][b][c];
  print(ANS);
}
