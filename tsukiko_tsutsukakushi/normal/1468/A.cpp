...
int solve(vector<int> a) {
    ...
    int ans = 0;
    vector<vector<pair<int, int>>> dp(n + 1, { { inf, -1 } });
    dp[0].emplace_back(-inf, -1);
    rep(i, n) {
        int x;
        {
            int l = 0, r = n + 1;
        ...
        dp[x].emplace_back(a[i], i);
        chmax(ans, x);
        int y;
        {
            int l = 0, r = dp[x - 1].size();
            while (r - l > 1) {
                int m = (l + r) >> 1;
                if (dp[x - 1][m].first > a[i]) {
                    l = m;
                } else {
                    r = m;
                }
            ...
            y = r;
            if (y < int(dp[x - 1].size()) and seg.prod(dp[x - 1][y].second + 1, i) > a[i]) {
                dp[x + 1].emplace_back(a[i], i);
                chmax(ans, x + 1);
                // debug(i, x + 1);
            } else {
                // debug(i, x);
...
int naive(vector<int> a) {
    ...
    rep(s, 1 << n) {
        vector<int> b;
        rep(i, n) if (kth_bit(s, i)) b.push_back(a[i]);

        bool ok = true;
        int k = b.size();
        rep(i, k - 2) {
            if (min(b[i], b[i + 1]) <= min(b[i + 1], b[i + 2])) {
