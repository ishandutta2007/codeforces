...
primeSieve<long long> sieve(200007);
...
int main() {
	...
	map<int, LazyPQ<int>> pw;
	for (int i = 0; i < n; i++) {
		int x; R(x);
		auto div = sieve.factorize(x);
		d[i] = { all(div) };

		for (auto& j : div)
			pw[j.fi].emplace(j.se);
	}
	
	Mint ans(1, MOD107);
	for (auto& i : pw)
		if (i.se.size() == n)
			ans *= Mint(i.fi, MOD107).pow(i.se.top());
	...
	while (q--) {
		...
		for (auto& p : div) {
			...
			if (pw[val].size() == n)
				ans /= Mint(val, MOD107).pow(pw[val].top());

			if (d[idx].count(val))
				pw[val].remove(d[idx][val]);

			d[idx][val] += mul;
			pw[val].emplace(d[idx][val]);
