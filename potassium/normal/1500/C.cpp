...
int main() {
	int n,m; R(n,m);
	vector<vector<int>> a(n, vector<int>(m)), b = a; R(a, b);

	vector<int> chain(m, 0);
	for (int i = 0; i + 1 < n; i++)
		for (int j = 0; j < m; j++)
			if (b[i][j] > b[i + 1][j])
				chain[j]++;
	...
	vector<int> col;
	auto unchain = [&](int row) {
		if (done[row]) return;
		done[row] = 1;
		for (int j = 0; j < m; j++)
			if (b[row][j] > b[row + 1][j])
				if (--chain[j] == 0)
					col.emplace_back(j);
	};
	...
	reverse(all(col));
	for (auto& j : col)
		stable_sort(all(a), [&](const vector<int>& r1, const vector<int>& r2) {
			return r1[j] < r2[j];
		});
