    ...
    void solve() {
        ...
        for (int i : range(n)) {
            ...
            if (k == 1) {
                ...
                int j = in.readInt() - 1;
                int l = in.readInt() - 1;
                if (dsu[j] != dsu[l] && (!ones[dsu[j]] || !ones[dsu[l]])) {
                    bool o = ones[dsu[j]] || ones[dsu[l]];
                    dsu.join(j, l);
                    ones[dsu[j]] = o;
                    id.push_back(i + 1);
                }
        ...
        out.printLine(power(modint(2), id.size()), id.size());
        out.printLine(id);
    }
};
