...
void solve() {
  ...
  struct OP {
    mint mul;
    ll pf;
    bool operator==(OP rhs) { return mul == rhs.mul && pf == rhs.pf; }
  };

  SegmentTree<E, OP> seg(
    [](E x, E y) {
      return E({x.prod * y.prod, x.cnt + y.cnt, x.pf | y.pf});
    },
    [](E x, OP y) {
      return E({x.prod * y.mul.pow(x.cnt), x.cnt, x.pf | y.pf});
    },
    [](OP x, OP y) {
      return OP({x.mul * y.mul, x.pf | y.pf});
    },
    E({mint(1), 0, 0}), OP({mint(1), 0}));
  seg.init(N);

  FOR(i, N) { seg.set_val(i, E({mint(A[i]), 1, PF[A[i]]})); }
  ...
  FOR(_, Q) {
    ...
    if (S == "TOTIENT") {
      ...
      auto t = seg.fold(L, R);
      mint ANS = t.prod;
      // print(L, R, t.cnt, t.prod, t.pf);

      FOR(k, K) if (t.pf & 1LL << k) {
        ll p = primes[k];
        ANS -= ANS / p;
      }
      print(ANS);
