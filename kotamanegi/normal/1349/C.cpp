...
void solve(){
    ...
    vector<tuple<ll, int, int,int>> queries;
    REP(i, t) {
        ll p;
        int a, b;
        cin >> a >> b >> p;
        a--; b--;
        queries.push_back(make_tuple(p, a, b, i));
    }
    REP(i, n) {
        REP(q, m) {
            StartFlip[i][q] = 2e18;
    ...
    while (!nextV.empty()) {
        ...
        REP(t, 4) {
            ...
            if (x >= 0 and x < n and y >= 0 and y < m) {
                if (StartFlip[x][y] > StartFlip[now.first][now.second] + 1) {
                    nextV.push(mp(x, y));
                    StartFlip[x][y] = StartFlip[now.first][now.second] + 1;
                }
            }
        }
    }
    REP(i, queries.size()) {
        int hoge = get<0>(queries[i]) - StartFlip[get<1>(queries[i])][get<2>(queries[i])];
        hoge = max(hoge, 0LL);
        cout << (grid[get<1>(queries[i])][get<2>(queries[i])] + hoge) % 2LL << "\n";
    }
}
#undef int
int main() {
