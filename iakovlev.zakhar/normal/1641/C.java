...
public class C {
	...
	public boolean[][] calc(int n, int k) {
	...
	public void solve() {
		int n = in.nextInt();
		int q = in.nextInt();
		TreeSet<Integer> unknown = new TreeSet<>();
		for (int i = 0; i <= n + 1; i++) {
			unknown.add(i);
		}
		TreeMap<Integer, Integer> segments = new TreeMap<>();
		segments.put(0, 0);
		segments.put(n + 1, n + 1);
		for (int qwerty = 0; qwerty < q; qwerty++) {
			if (in.nextInt() == 0) {
				...
				if (in.nextInt() == 0) {
					...
					int key = segments.ceilingKey(l);
					if (segments.get(key) <= r) {
						continue;
					}
					while (true) {
						key = segments.floorKey(l);
						if (segments.get(key) >= r) {
							segments.remove(key);
						} else {
							break;
				...
				if (!unknown.contains(id)) {
					...
					int prev = unknown.lower(id);
					int next = unknown.higher(id);
					int l = segments.higherKey(prev);
