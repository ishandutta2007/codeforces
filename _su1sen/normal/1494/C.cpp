...
int solve(const vector<int> a, const vector<int> b) {
    ...
    rrep(i, n + 1) {
        ...
        int l = a[i - 1] - (i - 1);
        int r = i == n ? inf : a[i] - (i - 1);

        {
            int cnt_l = lower_bound(all(b), l + i) - lower_bound(all(b), l);
            chmax(ans, cnt_l + cnt_r);
        }
        {
            int cnt_l = lower_bound(all(b), r - 1 + i) - lower_bound(all(b), r - 1);
            chmax(ans, cnt_l + cnt_r);
        }
        int fr = lower_bound(all(b), l) - b.begin();
        int to = lower_bound(all(b), r) - b.begin();
        rep(j, fr, to) {
            int p = b[j];
            int cnt_l = lower_bound(all(b), p + i) - lower_bound(all(b), p);
            chmax(ans, cnt_l + cnt_r);
        }
    }
    return ans;
...
int main() {
    ...
    loop(t) {
        ...
        rep(i, n) {
            input(int, a);
            (a < 0 ? a1 : a2).push_back(abs(a));
        }
        rep(j, m) {
            input(int, b);
            (b < 0 ? b1 : b2).push_back(abs(b));
        }
        reverse(all(a1));
        reverse(all(b1));
