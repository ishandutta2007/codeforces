...
int main()
{
    ...
    REP(_, M)
    {
        ...
        if (q == 1)
        {
            ...
            len += k;
            lazy_a = 0, lazy_b = 0;
            cht = ConvexHullTrick(true);
            cht.add_line(0, 0);
        }
        else if (q == 2)
        {
            cht.add_line(len, -lazy_a * len - lazy_b);
            lint k;
            cin >> k;
            len += k;
        }
        else if (q == 3)
        {
            lint b, s;
            cin >> b >> s;
            if (cht.lines.size() <= 1) s = 0;
            lazy_a += s;
            lazy_b += b;
        }
        plint ret = cht.get(lazy_a);

        printf("%lld %lld\n", ret.second + 1, ret.first + lazy_b);
    }
}