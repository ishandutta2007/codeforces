...
// cin>>T;while(T--)
    ...
    for(i=0;i<n;++i)
    {
        if(i>=2&&s[0][i]=='0'&&s[1][i]=='0'&&s[0][i-1]=='0'&&s[1][i-1]=='0'&&s[0][i-2]=='0'&&s[1][i-2]=='0')
        {
            dp[i+1]=dp[i-2]+2;
            //cout<<"YES";
        }
        if((i>=1&&s[0][i-1]=='0'&&s[1][i-1]=='0'&&(s[0][i]=='0'||s[1][i]=='0'))||(s[0][i]=='0'&&s[1][i]=='0'&&(s[0][i-1]=='0'||s[1][i-1]=='0')))
            dp[i+1]=max(dp[i-1]+1,dp[i+1]);
        else
            dp[i+1]=max(max(dp[i+1],dp[i]),max(dp[i-1],dp[i-2]));
        // cout<<dp[i];
    }
    // cout<<endl;
    if(n>=3)
        dp[n]=max(dp[n],dp[n-3]);
    if(n>=2)
        dp[n]=max(dp[n],dp[n-2]);
    if(n>=1)
