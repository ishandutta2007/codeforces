...
int main(){
	...
	cin.tie(0);
	MinCostFlow<102, 701> mcf;
	int n,m;
	cin>>n>>m;
	mcf.addEdge(n, 1, inf, 0);
	for (int i=0;i<m;i++){
		int a,b,c,d;
		cin>>a>>b>>c>>d;
		mcf.addEdge(a, b, inf, 2);
		if (d<=c){
			mcf.addEdge(a, b, c-d, 1);
			mcf.addEdge(101, b, d, 0);
			mcf.addEdge(a, 102, d, 0);
			mcf.addEdge(b, a, d, 1);
		}
		else{
			mcf.addEdge(101, b, c, 0);
			mcf.addEdge(a, 102, c, 0);
			mcf.addEdge(101, b, d-c, 1);
			mcf.addEdge(a, 102, d-c, 0);
			mcf.addEdge(b, a, d-c, 0);
			mcf.addEdge(b, a, c, 1);
		}
	}
	auto v=mcf.getKFlow(101, 102, inf);
	cout<<v.S<<endl;
}