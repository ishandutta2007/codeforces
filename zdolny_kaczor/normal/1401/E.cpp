...
int read(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
	if (l >= rb || r <= lb) return 0;
	if (l <= lb && r >= rb) return tree[wh];
	int sb = (lb + rb) / 2;
	return read(l, r, wh * 2, lb, sb) + read(l, r, wh * 2 + 1, sb, rb);
}
struct event {
	bool type;
	int time;
...
ostream &operator<<(ostream &o, event x) {
	if (x.type) ros << "[read: [" << x.a << ", " << x.b << ") at " << x.time << "]";
	else ros << "[put: " << x.a << "at point " << x.b << " at time " << x.time << "]";
}
...
void solve() {
	...
	REP(i, n) {
		int y, lx, rx;
		scanf("%d%d%d", &y, &lx, &rx);
		go.push_back({true, y, lx, rx + 1});
		if (lx == 0 && rx == M) {
			debug(go.back(), "is full");
			inter++;
		}
	}
	REP(i, m) {
		int x, ly, ry;
		scanf("%d%d%d", &x, &ly, &ry);
		go.push_back({false, ly, 1, x});
		go.push_back({false, ry + 1, -1, x});
		if (ly == 0 && ry == M) {
			debug(range(go.end() - 2, go.end()), "is full");
			inter++;
		}
	}
	sort(ALL(go));
	for (auto e : go) {
		auto [type, time, a, b] = e;
		debug(imie(e)); 
		if (type) {
			ll seen = read(a, b);
			debug(imie(seen));
