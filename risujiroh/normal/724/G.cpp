...
int main() {
  ...
  array<array<Mint, 2>, 60> cnt;
  auto dfs = [&](auto self, int v, int p) -> void {
    for (int i = 0; i < 60; ++i) ++cnt[i][a[v] >> i & 1];
    for (auto&& [u, w] : g[v]) {
      if (u == p) continue;
      if (a[u] == -1) {
        a[u] = a[v] ^ w;
        ...
        auto t = a[u] ^ a[v] ^ w;
        for (auto&& e : basis) t = min(t, t ^ e);
        if (t) basis.push_back(t);
      }
    }
  };
  Mint i2 = 1 / Mint(2);
  auto choose2 = [&](Mint x) { return x * (x - 1) * i2; };
  vector<Mint> p2(n + 1);
  p2[0] = 1;
  ...
  for (int v = 0; v < n; ++v) {
    ...
    a[v] = 0;
    basis.clear();
    for (auto&& e : cnt) e.fill(0);
    dfs(dfs, v, -1);
    int64_t mask = accumulate(begin(basis), end(basis), int64_t(0), bit_or{});
    Mint base = 1;
    for (int i = 0; i < 60; ++i) {
      Mint cur;
      if (mask >> i & 1) {
        cur += choose2(cnt[i][0] + cnt[i][1]) * p2[size(basis) - 1];
      } else {
        cur += cnt[i][0] * cnt[i][1] * p2[size(basis)];
      }
      cur *= base;
      ans += cur;
      base *= 2;
