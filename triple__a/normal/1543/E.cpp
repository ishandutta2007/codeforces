...
signed main(){
    ...
    while (t--){
        cin>>n;
        rep(i,1<<n) g[i].clear(), vis[i]=0, pos[i]=rev[i]=0;
        rep(i,n*1<<(n-1)){
            int u,v;
            cin>>u>>v;
            g[u].pb(v), g[v].pb(u);
        ...
        vi cont[17];
        rep(i,17) cont[i].clear();
        rep(i,1<<n){
            cont[__builtin_popcount(i)].pb(i);
        }
        auto fd=[&](int u,int v){
            for (auto c:g[u]){
                for (auto d:g[v]){
                    if (c==d&&!vis[c]) return c;
                }
            }
            assert(123);
            return 0ll;
        ...
        for (int i=2;i<=n;++i){
            for (auto c:cont[i]){
                vi hii;
                hii.clear();
                rep(j,n) if (c>>j&1) hii.pb(j);
                int idx=fd(rev[c^(1<<hii[0])],rev[c^(1<<hii[1])]);
                vis[idx]=1, pos[idx]=c, rev[c]=idx;
            }
        }
        rep(i,1<<n) cout<<rev[i]<<" ";
        cout<<"\n";
        if (n==1||n==2||n==4||n==8||n==16){
            rep(i,1<<n){
                int hi=0;
                rep(j,n) if (pos[i]>>j&1) hi^=j;
                cout<<hi<<" ";
            }
        }
        else{
