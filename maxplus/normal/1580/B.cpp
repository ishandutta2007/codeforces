...
void convolve(int i0, int i1, int jj, int p) {
  if (mxk[jj][i0] > mxk[jj][i1]) {
    swap(i0, i1);
  }
  for (int k0 = 0; k0 <= mxk[jj][i0]; ++k0)
  for (int k1 = 0; k1 <= mxk[jj][i1]; ++k1) {
    buf[k0 + k1] += a[jj][i0][k0] * (uint64_t)a[jj][i1][k1];
  }
  for (int k0 = mxk[jj][i0], k = 0; k <= k0 + mxk[jj][i1]; ++k) {
    buf[k] %= p;
    if (++k <= k0 + mxk[jj][i1]) buf[k] %= p;
    if (++k <= k0 + mxk[jj][i1]) buf[k] %= p;
    if (++k <= k0 + mxk[jj][i1]) buf[k] %= p;
  }
}
...
int main() {
  ...
  for (int j = 0; j <= m; ++j) {
    int J = j & 1;
    a[J][0][0] = 1;
    for (int i = 1, f = 1; f = fma(0, f, i), i <= n; ++i) if (j >= m - n + i) {
      if (j == 0 || j > i) {
        a[J][i][0] = f;
        mxk[J][i] = 0;
        continue;
      ...
      for (int i0 = 0; i0 * 2 < i; ++i0) {
        int i1 = i - i0 - 1, ccmk = mxk[!J][i0] + mxk[!J][i1];
        maxi(cmk, ccmk);
      ...
      int dcmk = j == 1;
      cmk += dcmk;
      for (int k = dcmk; k <= cmk; ++k) {
        a[J][i][k] = buf2[k - dcmk] % p;
        if (++k <= cmk) a[J][i][k] = buf2[k - dcmk] % p;
        if (++k <= cmk) a[J][i][k] = buf2[k - dcmk] % p;
        if (++k <= cmk) a[J][i][k] = buf2[k - dcmk] % p;
      }
      mxk[J][i] = cmk;
  ...
  print(a[m & 1][n][min(k, (int)size(**a) - 1)]);
}