...
class GPerevorotiStolbtsov {
public:
    void solve() {
        int n = in.readInt();
        auto a = in.readIntTable(2, n);
        ...
        auto go = [&](int pp, int el) -> vi {
            int start = pp;
            vi res;
            do {
                processed[pp] = true;
                int nPos = pos[el][0] + pos[el][1] - pp;
                if (a(0, nPos) == el) {
                    res.push_back(nPos + 1);
                }
                el = a(0, nPos) + a(1, nPos) - el;
                pp = nPos;
            } while (start != pp);
            return res;
        };
