...
#endif
char a[1000005][4];int l[1000005];
int g[26][26],h[26][26][26];
signed main()
{
	initprog();
	int T=read();rg(T)
	int n=read();
	rg(n)scanf("%s",a[i]);l[i]=strlen(a[i]);gr
	int gg=0;
	rg(n)
	if(l[i]==1)gg=1;
	else if(a[i][0]==a[i][l[i]])gg=1;
	gr
	if(gg){puts("YES");continue;}
	for(int i=0;i<26;i++)for(int j=0;j<26;j++)
	{
		g[i][j]=0;
		for(int k=0;k<26;k++)h[i][j][k]=0;
	}
	rg(n)
	for(int j=0;j<l[i];j++)a[i][j]-=97;;
	if(l[i]==2)
	{
		g[a[i][0]][a[i][1]]=1;
		if(g[a[i][1]][a[i][0]])gg=1;
		for(int k=0;k<26;k++)if(h[a[i][1]][a[i][0]][k])gg=1;
	}
	else
	{
		h[a[i][0]][a[i][1]][a[i][2]]=1;
		if(h[a[i][2]][a[i][1]][a[i][0]])gg=1;
		if(g[a[i][2]][a[i][1]])gg=1;
	}
	gr
	if(gg)puts("YES");
	else puts("NO");
	...
	// {
		// // 21 012 10
		// if(g[a[i][2]][a[i][1]]||g[a[i][1]][a[i][0]]){gg=1;}
	// }
	// gr
	// if(gg){puts("YES");continue;}
	// puts("unknow");
