...
int n,m;
char s[maxn];
int nextp[maxn][26],prevp[maxn][26];
...
int main()
{
	...
	for (int k=0;k<26;k++) 
	{
		nextp[n+1][k]=100000000;
		prevp[0][k]=-100000000;
	}
	for (int i=1;i<=n;i++) for (int k=0;k<26;k++) prevp[i][k]=(s[i]=='a'+k)?i:prevp[i-1][k];
	for (int i=n;i>=1;i--) for (int k=0;k<26;k++) nextp[i][k]=(s[i]=='a'+k)?i:nextp[i+1][k];
	for (;m>0;m--)
	{
		scanf("%s",s+1);
		int L=strlen(s+1);
		...
		for (int i=1;i<=L;i++)
		{
			int k=s[i]-'a';
			int p=min(i,n);
			int d=min(abs(i-prevp[p][k]),abs(i-nextp[p][k]));
			if (d>=10000000) d=L;
			result+=d;
		}
		printf("%I64d\n",result);
