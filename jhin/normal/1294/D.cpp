        ...
        cin>>t>>x;set<int>tmp;map<int,int>mapp,coun;
        mapp[0]=-1;
        auto it=tmp.end();
        while(t--)
        {
            cin>>y;
            int temper=y%x;
            if(coun[temper]==0){coun[temper]=1;mapp[temper]=temper;}
            else
            mapp[temper]+=x;
            tmp.insert(mapp[temper]);
            if(mapp[temper]==0&&temper==0)it=tmp.begin(),yes=true;
