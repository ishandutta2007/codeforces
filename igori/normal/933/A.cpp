...
ll n, m, ans1;
ll a[399499], b[484888], c[409399];

int zero_interval(int x, int y)
{
    return c[y] - c[x - 1];
}

int one_interval(int x, int y)
{
    return b[y] - b[x - 1];
}
...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        for (int j = 0; j <= i; j++)
        {
            ll aa = zero_interval(1, j) + one_interval(j + 1, i);
            x = max(aa, x);
        }
        for (int j = i; j <= n; j++)
        {
            ll aa = zero_interval(i + 1, j) + one_interval(j + 1, n);
            y = max(y, aa);
        }
        //cout << x << " " << y << endl;
        ans1 = max(ans1, x + y);
