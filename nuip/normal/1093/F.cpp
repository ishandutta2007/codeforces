...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> a(n);
	rep(i,n) cin>>a[i];
	for(int &x:a) if(x>0) --x;
	deque<pii> mx,mn;
	dp[0][0]=1;
	rep(i,n){
		while(mx.size() && mx.front().X<i+1-len) mx.pop_front();
		while(mn.size() && mn.front().X<i+1-len) mn.pop_front();
		if(a[i]!=-1){
			while(mx.size() && mx.back().Y<=a[i]) mx.pop_back();
			while(mn.size() && mn.back().Y>=a[i]) mn.pop_back();
			mx.eb(i,a[i]); mn.eb(i,a[i]);
		}
		out(i,mx,mn,1);
		mint s=accumulate(dp[i],dp[i+1],mint(0));
		mint sp=0;
		if(i+1>=len) sp=accumulate(dp[i+1-len],dp[i+1-len+1],mint(0));
		rep(j,t)if(a[i]==-1 || a[i]==j){
			dp[i+1][j]=s;
			if(i+1>=len){
				if(mx.size() && (mx.front().Y!=j || mn.front().Y!=j)) continue;
				out(i,j,1);
				dp[i+1][j]-=sp-(i+1-len==0?mint(0):dp[i+1-len][j]);
			}
		}		
	}
	// rep(i,n+1){rep(j,t) cout<<dp[i][j]<<",";cout<<endl;}
	cout<<accumulate(dp[n],dp[n+1],mint(0))<<endl;
  return 0;
}