...
int main() {
    ...
    for (int i = 2; i < dp.size(); ++i) {
        dp[i] = 1 + dp[i - 1] + dp[i - 1] * (dp[i - 1] + 1) / 2;
    }

    vector<Mint> rooted(dp.size());
    for (int n = 1; n < rooted.size(); ++n) {
        Mint ans = dp[n] + dp[n] * (dp[n] + 1) / 2 + dp[n] * (dp[n] - 1) * (dp[n] - 2) / 6 + dp[n] * (dp[n] - 1) + dp[n];
        ans *= 2;
        ans -= n;
        rooted[n] = ans;
    }
