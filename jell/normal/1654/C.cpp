#line 1 "codeforces-workspace\\contest\\1654\\c\\c.cpp"
...
#line 13 "codeforces-workspace\\contest\\1654\\c\\c.cpp"
...
void main() {
  ...
  auto sum = accumulate(begin(a), end(a), i64{});
  priority_queue<i64> q(begin(a), end(a)), p;
  p.emplace(sum);
  while (p.size() <= q.size()) {
    if (p.empty()) eject("YES");
    auto x = p.top();
    p.pop();
    if (q.top() == x)
      q.pop();
    else
      p.emplace(x / 2), p.emplace((x + 1) / 2);
  }
  eject("NO");
}
