...
void solveL(int l, int r, int ql, int qr) {
	vector<ll> disc;
	for (int i = l; i <= r; i++) disc.emplace_back(a[i] - v * t[i]);
	for (int i = ql; i <= qr; i++) disc.emplace_back(a[i] - v * t[i]);
	distinct(disc);
	BIT<ll, mx> bit(disc.size());

	vector<int> event;
	for (int i = l; i <= r; i++) event.push_back(i);
	for (int i = ql; i <= qr; i++) event.push_back(i);

	sort(event.begin(), event.end(), [&](int i, int j) {
		if (a[i] != a[j]) return a[i] < a[j];
		return i < j;
	});
	for (auto& i : event) {
		int idx = disc.end() - lower_bound(disc.begin(), disc.end(), a[i] - v * t[i]);
		if (i <= r) {
			bit.upd(idx, ans[i]);
		}
		else {
			chkmax(ans[i], bit.qry(idx) + 1);
		}
	}
}

void solveR(int l, int r, int ql, int qr) {
	vector<ll> disc;
	for (int i = l; i <= r; i++) disc.emplace_back(a[i] + v * t[i]);
	for (int i = ql; i <= qr; i++) disc.emplace_back(a[i] + v * t[i]);
	distinct(disc);
	...
	for (auto& i : event) {
		int idx = lower_bound(disc.begin(), disc.end(), a[i] + v * t[i]) - disc.begin() + 1;
		if (i <= r) {
			bit.upd(idx, ans[i]);
		}
		else {
...
void CDQ(int l, int r) {
	...
	int mid = (l + r) / 2;
	CDQ(l, mid);
	solveL(l, mid, mid + 1, r);
	solveR(l, mid, mid + 1, r);
	CDQ(mid + 1, r);
}
...
int main() {
	...
	t.resize(n + 1, 0);
	a.resize(n + 1, 0);
	ans.resize(n + 1, LLONG_MIN);
	ans[0] = 0;
	for (int i = 1; i <= n; i++) R(t[i]);
	for (int i = 1; i <= n; i++) R(a[i]);
	
	CDQ(0, n);
	W(*max_element(ans.begin(), ans.end()));
