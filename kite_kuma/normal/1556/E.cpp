...
int main() {
	...
	rep(i, n + 1) v[i] = make_pair(acc[i], acc[i]);

	ST<pair<ll, ll>> st_minmax(
		n + 1, [](pair<ll, ll> a, pair<ll, ll> b) { return make_pair(min(a.first, b.first), max(a.second, b.second)); }, make_pair(INF, -INF),
		v);

	auto query = [&](int l, int r) -> ll {
		if(acc[l] != acc[r]) return -1;
		ll minimum = st_minmax.get(l, r + 1).first;
		ll maximum = st_minmax.get(l, r + 1).second;

		if(maximum > acc[l]) return -1;
		return acc[l] - minimum;
	};
