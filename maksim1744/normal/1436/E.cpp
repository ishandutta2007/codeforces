...
int main() {
    ...
    segtree tree(n + 5);
    int iq = 0;
    sort(que.begin(), que.end(), [&](const pair<pair<int, int>, int>& a, const pair<pair<int, int>, int>& b) {
    ...
    for (int i = 0; i < n; ++i) {
        tree.set(a[i], i);
        while (iq < que.size() && que[iq].first.second == i) {
            int last = tree.ask(0, que[iq].second - 1).mn;
            if (last >= que[iq].first.first) has[que[iq].second] = true;
            ++iq;
        }
    }
    show(has);
