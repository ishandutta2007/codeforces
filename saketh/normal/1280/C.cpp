...
template<typename E> struct tree {
    ...
    const E& up_edge(int u) const { assert(u); return nbrs[u].front(); }
...
void solve() {
    using E = wedge<int>;
    int N; re(N); tree<E> tr(2 * N); re(tr);

    ll good = 0, bad = 0;

    FOR (i, 1, 2 * N) {
        if (tr.subt_sz[i]&1) good += tr.up_edge(i).wt();
        ll hi = min(tr.subt_sz[i], 2 * N - tr.subt_sz[i]);
        bad += hi * tr.up_edge(i).wt();
    }
