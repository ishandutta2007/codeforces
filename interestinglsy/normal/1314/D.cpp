...
int Solve(){
	static bool isWhite[MAXN];
	For(i,n) isWhite[i] = Rand()&1;
	...
	For(step,k){
		int last = cur; cur ^= 1;
		Msn(dp[cur],INF);
		auto bdp = dp[last], udp = dp[cur];
		For(i,n){
			For(v,n){
				if( isWhite[i] == isWhite[v] ) continue;
				MyMin(udp[v],bdp[i]+cost[i][v]);
			}
		}
	}
	return dp[cur][1];
...
int main(){
	...
	Read(n,k);
	For(i,n) Read(cost[i],1,n);

	int ans = INF;
	constexpr time_t TIME_LIMIT = 2.8*CLOCKS_PER_SEC;
