...
void slv(){
	...
	auto dfs=[&](auto self,int v)->int{
		if(v==n)return 0;
		int lf=self(self,t[v][0]);
		int rt=self(self,t[v][1]);
		int s=1+lf+rt;
		rep(i,s+1)dp[v][i]=-inf;
		rep(i,lf+1)rep(j,rt+1){
			int val=dp[t[v][0]][i]+dp[t[v][1]][j]-i*j*a[v]*2;
			chmax(dp[v][i+j],val);
			chmax(dp[v][i+j+1],val+((m-1)-2*(i+j))*a[v]);
		}
		return s;
	};
	dfs(dfs,root);
