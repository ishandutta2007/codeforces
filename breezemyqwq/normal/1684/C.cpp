...
bool chk(int x, int y) {
	...
	rep(i, 1, n) rep(j, 1, m - 1) if(a[i][j] > a[i][j + 1]) return false;
	return true;
}
...
int main() {
	...
	W(T) {
		...
		rep(i, 1, n) {
			int f = 1;
			int px, py;
			rep(j, 1, m) if(a[i][j] < a[i][j - 1]) {
				--j; while(j > 1 && a[i][j] == a[i][j - 1]) --j;
				f = 0;
				px = j;
				break;
			}
			per(j, m - 1, 1) if(a[i][j] > a[i][j + 1]) {
				++j; while(j < m && a[i][j] == a[i][j + 1]) ++j; 
				py = j;
				break;
			}
			if(!f) {
