...
void dfs(int v,int pr,vector<int>&D,int d){
  D[v]=d;
  for(int p:G[v]) if(p!=pr) dfs(p,v,D,d+1);
}

vector<int> ask2(int v){
  vector<int> D(n,-1);
  dfs(v,v,D,0);
  return D;
}
...
vector<int> ask(int v){
  if(++qs>(n+1)/2) while(true) (void)malloc(1<<20);
  cout<<"? "<<v+1<<endl;
  vector<int> r(n);
  forn(i,n) cin>>r[i];
  assert(!r[v]);
...
vector<pii> solve(int n){
  ...
  vector<short> W(n);
  vector<tuple<int,int,int,int,int>> qu;
  W[0]=1;
  A[0]=ask(0);
  forn(i,n){
    if(A[0][i]==1){
      ...
      qu.emplace_back(i,0,0,0,0);
      //dbg("enqu",i);
    }
  }
  while(sz(qu)){
    auto [v,p1,d1,p2,d2]=qu.back();
    qu.pop_back();
    if(W[v]) continue;
    W[v]=1;
    A[v]=ask(v);
    int pr=-1;
    forn(i,n) if(A[v][i]==1&&A[p1][i]==d1+1&&A[p2][i]==d2+1) { assert(pr==-1); pr=i; }
    assert(pr!=-1);
    //dbg("dequ",v,p,pr);
    forn(i,n){
      if(A[v][i]==1){
        ...
        R.pb(mp(v,i));
        //dbg("B",i,v);
      }else if(A[v][i]==2&&A[p1][i]==d1+2&&A[p2][i]==d2+2){
        W[i]=1;
        R.pb(mp(pr,i));
      }else if(A[v][i]==3&&A[p1][i]==d1+3&&A[p2][i]==d2+3){
        qu.emplace_back(i,v,1,p1,d1+1);
      }else if(A[v][i]==2&&A[p1][i]==d1+4&&A[p2][i]==d2+4){
        qu.emplace_back(i,v,0,v,0);
      }
    }
  }
...
void stress(){
  for(n=2;n<100;++n){
    ...
    forn(q,10000){
      gen();
      auto R=solve(n);
      for(pii&e:R) if(e.X>e.Y) swap(e.X,e.Y);
      sort(all(R));
      assert(unique(all(R))-R.begin()==n-1);
      forn(i,n) for(int p:G[i]) assert(binary_search(all(R),mp(min(i,p),max(i,p))));
    }
  }
  dbg("ok");
}
