...
int main() {
  ...
  while(abs(ok-ng)>1){
    ll t=(ok+ng)/2;
    vector<vector<ll>> o(a,vector<ll>(b,inf));
    for(int j=0;j<b;j++){
      ll c=0;
      char p=char(int('a')+j);
      for(int i=0;i<t-1;i++){
      ...
      for(int i=t-1;i<a;i++){
        if(s[i]!='?'&&s[i]!=p) c++;
        if(!c) o[i-(t-1)][j]=i-(t-1);
        if(s[i-(t-1)]!='?'&&s[i-(t-1)]!=p) c--;
      }
    }
    for(int j=0;j<b;j++){
      for(int i=a-2;i>=0;i--) chmin(o[i][j],o[i+1][j]);
    }
    vector<ll> dp(1<<b,inf);
    dp[0]=0;
    for(int i=0;i<(1<<b);i++){
      if(dp[i]>=a) continue;
      for(int j=0;j<b;j++){
        if(!((i>>j)&1)){
      //   cout<<t<<" "<<i<<" "<<j<<" "<<dp[i]<<" "<<o[dp[i]][j]<<endl;
          if(o[dp[i]][j]<a) chmin(dp[i+(1<<j)],o[dp[i]][j]+t);
        //  cout<<(i+(1<<j))<<" "<<dp[i+(1<<j)]<<endl;
        }
      }
    }
    if(dp[(1<<b)-1]==inf) ng=t;
    else ok=t;
  }
  cout<<ok<<endl;
}