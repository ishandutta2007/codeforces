...
int a[MAXN << 1], t, dep[MAXN << 1];
signed main(){
    read(n, m); const int N = 1 << (n ++); read(a + N, a + (N << 1)); REP(i, 1, N << 1) dep[i] = dep[i >> 1] + 1;
    per(i, N - 1, 1) a[i] = ((dep[i] ^ n) & 1)? (a[i << 1] | a[i << 1 | 1]) : (a[i << 1] ^ a[i << 1 | 1]);
    while(m --) {
        int i, y; read(i, y);
        a[i += N - 1] = y;
        while(i >>= 1) a[i] = ((dep[i] ^ n) & 1)? (a[i << 1] | a[i << 1 | 1]) : (a[i << 1] ^ a[i << 1 | 1]);
        print(a[1]);
    }
}