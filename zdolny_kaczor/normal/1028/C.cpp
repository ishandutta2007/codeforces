...
QII merge(QII a, QII b) {
	return MQ(max(get1(a), get1(b)), max(get2(a), get2(b)), min(get3(a), get3(b)), min(get4(a), get4(b)));
}
const int inf = 2e9 + 44;
const QII neu = MQ(-inf, -inf, inf, inf);
void check(QII rec) {
	if (get1(rec) <= get3(rec) && get2(rec) <= get4(rec)) {
		printf("%d %d\n", get1(rec), get2(rec));
		exit(0);
	}
}
...
int main() {
	...
	scanf("%d", &n);
	REP(i, n)
		scanf("%d%d%d%d", &get1(in[i]), &get2(in[i]), &get3(in[i]), &get4(in[i]));
	pref[0] = neu;
	REP(i, n)
		pref[i + 1] = merge(pref[i], in[i]);
	suf[n - 1] = neu;
	FORD(i, n - 1, 1)
		suf[i - 1] = merge(suf[i], in[i]);
	REP(i, n)
		check(merge(pref[i], suf[i]));
	assert(false);
}