...
int main() {
    ll f[32][32] = {};
    map<pii, int> mp;
    auto solve = [&](auto &&f, int u, int v) -> int {
        if(u == v) return 1;
        if(u == 0) return 0;
        if(u > v) return 0;
        if(mp.count(pii(u, v))) return mp[pii(u, v)];
        auto &res = mp[pii(u, v)];
        int vt = 1 << topbit(v);
        v -= vt;
    ...
    TEST {
        ...
        if(u > v) {
            ...
            rep(i, a) rep(j, b) {
                f[i][j + 1] += f[i][j];
                if(!f[i][j]) continue;
                if(topbit(U[i]) == topbit(V[j])) {
                    f[i + 1][j + 1] += 1;
                    f[i + 1][j + 2] -= 1;
                } else if(topbit(U[i]) > topbit(V[j])) {
                    f[i + 1][j + 1] += 1;
                }
            }
            rep(j, b) f[a][j + 1] += f[a][j];
            ans = f[a][b];
        }
        YES(ans);
        // if(ans != solve(solve, memu, memv)) {
        //     cout << bitset<32>(memu) << " " << bitset<32>(memv) << endl;
        //     cout << ans << endl;
        //     dump(memu, memv);
        // }
    }
