...
int main() {
	...
	string s;
	cin >> s;
	ll m = 0, a = 0, dp[200100][3];
	for(int i = 0; i < 200100; i++) dp[i][0]= dp[i][1]= dp[i][2]=-10000000;
	dp[0][0] = 0;
	for(int i = 0; i < s.size(); i++) {
		m=(s[i]-'0');
		for(int j = 0; j < 3; j++)
			maxq(dp[i+1][j], dp[i][j]),
			maxq(dp[i+1][0], dp[i][j]);
		for(int j = 0; j < 3; j++)
			maxq(dp[i+1][(j+m)%3], dp[i][j]+((j+m)%3 == 0));
	}
	cout << max({dp[s.size()][0], dp[s.size()][1], dp[s.size()][2]});
}