...
int main() {
    ...
    vi order(T);
    rep(i,T)order[i] = i;
    sort(all(order),[&](int x,int y){return L[x] > L[y];});
    FenwickTree<ll> BIT(N);
    int now = 200000;
    for(auto i:order) {
        while(now >= L[i]) {
            rep(k,now*2,200001,now) {
                auto it = da[k].end() - upper_bound(all(da[k]),now) - 1;
                BIT.add(k,it);
            }
            if(now % 3 == 0 && 2 * now <= N) BIT.add(2 * now,1);
            if(now % 6 == 0 && now / 2 * 5 <= N) BIT.add(now / 2 * 5,1);
            now--;
        }
        ans[i] -= BIT.prod(0,R[i] + 1);
    }
    rep(i,T) cout << ans[i] << '\n';
}