...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) cin >> x[i] >> y[i], mp[P(x[i], y[i])] = i;
	
	set<P> S;
	rep(i, 1, n){
		rep(j, 0, 3){
			ll nx = x[i] + dx[j], ny = y[i] + dy[j];
			if(mp.count(P(nx, ny)) == 0) S.insert(P(nx, ny));
		}
	}
	
	queue<P> Q;
	for(auto p : S) Q.push(p);
	rep(i, 1, n) ans[i] = P(-inf, -inf);
	
	while(sz(Q)){
		ll x = Q.front().fi, y = Q.front().se; Q.pop();
		P p = P(x, y);
		if(mp.count(P(x, y))) p = ans[mp[P(x, y)]];
		
		rep(j, 0, 3){
			ll nx = x + dx[j], ny = y + dy[j];
			if(mp.count(P(nx, ny)) == 0) continue;
			ll nid = mp[P(nx, ny)];
			if(ans[nid].fi > -inf/2) continue;
			ans[nid] = p;
			Q.push(P(nx, ny));
		}
	}
