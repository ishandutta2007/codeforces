...
fun main(){
    solve.cases{
        ...
        val squeeze = mutableListOf<Boolean>()
        for(i in 0..n){
            if(L[i] < last){
                ...
                lastpos = i
                things.add(dif)
                val find = everything.ceiling(L[i] + 1 )
                squeeze.add(find != null && find  < last)
                last = L[i]
            }
        }
        val bad = things.filter { it > k }
        if(bad.size > 1){
            put("NO")
            return@cases
        }
        ...
        val badpos = things.indexOfFirst { it > k }
//        if(badpos > 0){
//            if(bad[badpos] + bad[badpos-1] <= 2 * k){
//                put("YES")
//                return@cases
//            }
//        }
        if(badpos != things.lastIndex){
            if(things[badpos] + things[badpos+1] <= 2 * k ){
                put("YES")
                return@cases
            }
        }
        if(squeeze[badpos] && things[badpos] <= 2 * k){
            put("YES")
            return@cases
        }
        put("NO")
