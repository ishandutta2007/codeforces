...
fenwick<mint> a;
int h[maxn], tin[maxn], tout[maxn], timer = 0, n;
mint val[maxn];
vector<int> g[maxn];
void dfs(int v) {
	//cout << v << " " << h[v] << " " << val[v].val() << endl;
	if(h[v]&1) val[v] *= -1;
	...
	tout[v] = timer;
	//cout << v << " " << val[v].val() << endl;
	a.update(tin[v], tout[v], val[v]); 
}
mint ans;
void solve(int v) {
	ans += a.query(1, timer) * (h[v]%2 ? -1 : 1);
	for(auto &i : g[v]) {
		a.update(tin[i], tout[i], -val[v]-val[i]); 
		a.update(1, timer, val[i]); 
		solve(i);
		a.update(tin[i], tout[i], val[v]+val[i]); 
		a.update(1, timer, -val[i]); 
	}
