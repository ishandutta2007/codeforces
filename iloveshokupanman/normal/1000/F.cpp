...
int main() {
    ...
    rep(i, Q) {
        ll l, r;
        cin >> l >> r;
        q[l - 1].push_back({ i,r-1 });
       
    }

    lazy_segtree<P, mx, e, P, mx, mx, e> sg(N+1);

    vll ans(Q);

    rep(i, M) {
        if (lis[i].size() > 1) {
           sg.apply(lis[i][0], lis[i][1], { lis[i][0],i });
    ...
    rep(i, N) {
        for (auto p : q[i]) {
            ...
            auto t = sg.get(r);
            if (t.second > 0 && t.first >= i) { ans[p.first] = t.second; }
        }
        
        ll x = a[i];
       auto itr = lower_bound(lis[x].begin(), lis[x].end(), i);
        ll d = distance(lis[x].begin(), itr);

        if (d+2<lis[x].size()) {
            sg.apply(lis[x][d+1], lis[x][d + 2], { lis[x][d+1], x });
        }

        //rep(i, N+1)cout << sg.get(i).second << " ";
        //cout << endl;
    }
