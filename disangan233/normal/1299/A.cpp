...
const int N=3e5+5;
int n,a[N],cnt[33],ans=-1,id; 
int main()
{
	n=read();for(int i=1;i<=n;i++) a[i]=read();
	for(int j=0;j<30;j++) for(int i=1;i<=n;i++) if((a[i]>>j)&1) cnt[j]++;
	for(int i=1;i<=n;i++)
	{
		for(int j=0;j<30;j++) if((a[i]>>j)&1) cnt[j]--;
		int res=0;for(int j=0;j<30;j++) if((a[i]>>j)&1&&!cnt[j]) res|=1<<j;
		if(res>ans) ans=res,id=i;
		for(int j=0;j<30;j++) if((a[i]>>j)&1) cnt[j]++;
	}
	write(a[id],' ');for(int i=1;i<=n;i++) if(i^id) write(a[i],' ');
	return ot(),0;
}
//Author: disangan233
