...
signed main()
{
	...
	for(int i=1;i<=n;i++)
	{
		int cc=i&1,d=cc^1;memset(dp[cc],0,sizeof(dp[cc]));
		for(int j=0;j<=1;j++)
		{
			for(int k=0;k<=1;k++)
			{
					...
					for(int s=0;s<8;s++)
					{
						for(int a=0;a<=1;a++)
						{
							for(int b=0;b<=1;b++)
							{
								for(int c=0;c<=1;c++)
								{
									int nj;if(j==0)nj=0;else if(a>t[i])continue;else nj=a==t[i];
									int nk;if(k==0)nk=0;else if(b>t[i])continue;else nk=b==t[i];
									int nl;if(l==0)nl=0;else if(c>t[i])continue;else nl=c==t[i];
									int A=a^b,B=a^c,C=b^c;
									int ns=s;
									if(A&&!B&&C)ns|=1;
									if(!A&&B&&C)ns|=2;
									if(A&&B&&!C)ns|=4;
									dp[cc][nj][nk][nl][ns]+=dp[d][j][k][l][s];
									if(dp[cc][nj][nk][nl][ns]>=mod)dp[cc][nj][nk][nl][ns]-=mod;
									// if(i==1&&j&&k&&l&&s==0)
									// puts("qwq?"),
									// printf("%d %d %d\n",nj,nk,nl),
									// odl(ns),
									// odl(dp[d][j][k][l][s]),
									// odl(dp[c][nj][nk][nl][ns]),
									// puts("q");
									// ;
								}
							}
