...
template <int N> struct Binom {
  ...
  ModT operator()(int n, int k) {
    return fact[n] * inv_fact[k] * inv_fact[n - k];
  }

  ModT fact[N + 1], inv_fact[N + 1];
};
...
int main() {
  ...
  while (T--) {
    ...
    ModT result = binom(n, n / 2);
    if (n & 1) {
      result += result;
    }
    result *= ModT(1 - l);
    for (int k = 2 - l; k <= n - l; ++k) {
      int b = l + k;
      int e = std::min(r - k, n);
      int cboth = std::max(e - b + 1, 0);
      if (b <= e + 1) {
        int cpos = e - cboth;
        int cneg = n - b + 1 - cboth;
        if (cpos <= n / 2 && cneg <= (n + 1) / 2) {
          result += binom(cboth, n / 2 - cpos);
        }
        if ((n & 1) && cpos <= (n + 1) / 2 && cneg <= n / 2) {
          result += binom(cboth, n / 2 - cneg);
        }
      }
    }
