...
const int dax = 30, nax = 1 << dax;
ll pow3[dax + 1];
void go(vector <pii> &ans, int l, int r, int lb = 0, int rb = nax) {
	if (l >= rb || r <= lb) return;
	if (l <= lb && r >= rb) {
		ans.emplace_back(lb, ctz(rb - lb));
		return;
	}
	int sb = (lb + rb) >> 1;
	go(ans, l, r, lb, sb);
...
int main() {
	pow3[0] = 1;
	for (int i = 1; i <= dax; ++i) pow3[i] = 3 * pow3[i - 1];
	int t;
	scanf("%d", &t);
	while (t--) {
		int l, r;
		...
		debug(imie(s));
		ll ans = 0;
		for (pii x : s) for (pii y : s) {
			int mul = (x == y ? 0 : 1);
			if ((x.first & y.first) == 0) {
				int l = min(x.second, y.second), h = max(x.second, y.second);
				ll curr = pow3[l] << (h - l - popc((x.first | y.first) & ((1 << h) - 1)));
				ans += curr << mul;
			}
			if (x == y) break;
		}
