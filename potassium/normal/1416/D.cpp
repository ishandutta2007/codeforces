...
int main() {
	...
	for (int i = q - 1; i >= 0; i--) {
		if (qry[i].first == 2) {
			...
			small.pb(dsu.uni(e[u].first, e[u].second));
		}
	}

	reverse(all(small));

	vector<priority_queue<pii>> gp(300007);
	vector<int> gpn(n + 1, 0);
	int gpptr = 0;
	for (int i = 1; i <= n; i++) {
		if (dsu.p[i] == i) {
			for (auto& j : dsu.sub[i]) {
				gp[gpptr].push({a[j], j});
				gpn[j] = gpptr;
			}
	...
	int op2cnt = 0;
	for (int qptr = 0; qptr < q; qptr++) {
		if (qry[qptr].first == 1) {
			int u = qry[qptr].second;
			int num = gpn[u];

			while (!gp[num].empty() && gpn[gp[num].top().second] != num) {
				gp[num].pop();
			}
			...
			int idx = gp[num].top().second;
			gp[num].pop();

			W(a[idx]);
			a[idx] = 0;
		} else {
			vector<int>& sm = small[op2cnt];
