...
int main() {
	...
	mo[1] = 1;
	for (int i = 2; i < MAXV; i++)
		mo[i] = ((i / mpf[i]) % mpf[i] == 0 ? 0 : mo[i / mpf[i]] * -1);
	...
	vector<ll> val(MAXV), logv(MAXV);
	val[2] = 1e18;
	for (int i = 3; i < MAXV; i++) {
		val[i] = pow(1e18, 1.0 / i) + 7;
		logv[i] = i * log(val[i]);
	}
	...
	for (auto& i : qry) {
		...
		for (int j = 3; j < MAXV; j++) {
			if (chkmin(val[j], val[j - 1]))
				logv[j] = j * log(val[j]);
				
			if (val[j] == 1 || mo[j] == 0) continue;
			
			while (logv[j] > logq + eps || pow(val[j], j, LLONG_MAX) > i.F) {
				val[j]--;
				logv[j] = j * log(val[j]);
			}
			
			ans[i.S] += (val[j] - 1) * mo[j];
		}
	}
