/*-------------------( alex256 )--------------------+
|    Contest: Codeforces Round 333 (Div. 2)         |
|       Task: C                                     |
|       Date: 13. 11. 2015                          |
+--------------------------------------------------*/
#include <algorithm>
...
int paths(bool ttype)
{
    for (int i = 0; i < n; i++) path[i] = INF, used[i] = true;
    path[0] = 0;
    for (int i = 0; i < n; i++)
    {
        int mn = INF, c = -1;
        for (int j = 0; j < n; j++)
            if (used[j] && path[j] < mn) mn = path[j], c = j;
        if (c < 0) return -1;
        if (c == n - 1) return mn;
        used[c] = false;
        for (int j = 0; j < n; j++)
            if ((q[c][j] == ttype) && (path[j] > (path[c] + 1))) path[j] = path[c] + 1;
    }
    return -256;
}
