...
int main() {
	...
	for (int i = 0; i < n; ++i) {
		...
		for (int ra = 0; ra < k; ++ra) {
			int rb = fix(sum - ra, k);
			ll here = dp[i][ra];
			if (here >= 0) debug(arr(in, i), imie(sum), imie(ra), imie(rb), imie(here));
			auto go = [&](int takea, int takeb) {
				assert(takea + takeb == 0 || takea + takeb == k);
				int taken = !!(takea + takeb);
				int leavea = a - takea, leaveb = b - takeb;
				maxi(dp[i + 1][(ra + leavea) % k], here + taken + (ra + leavea) / k + (rb + leaveb) / k);
			};
			go(0, 0);
			for (int ta = 0; ta <= k; ++ta) {
				int tb = k - ta;
				if (ta <= a && tb <= b) go(ta, tb);
			}
		}
		(sum += a + b) %= k;
	}
	ll ans = 0;
	REP(i, k) maxi(ans, dp[n][i]);
	printf("%lld\n", ans);
	exit(0);
}
//#STAY AT HOME