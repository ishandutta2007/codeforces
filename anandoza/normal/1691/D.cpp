...
struct T {
    ...
    friend T operator+(const T &l, const T &r) {
        ...
        if (r.null)
            return l;
        return T{false, l.sum + r.sum, max(l.left, l.sum + r.left), max(l.right + r.sum, r.right),
                 max({l.overall, r.overall, l.right + r.left})};
    }
};

constexpr T IDENTITY{true, 0, 0, 0, 0};

T create(int x) { return T{false, x, max(x, 0), max(x, 0), max(x, 0)}; }
...
void solve() {
    ...
    segment_tree st(n, IDENTITY, std::plus<T>());

    vi order(n);
    iota(all(order), 0);

    sort(all(order), L2(u, v, a[u] < a[v]));

    for (int i : order) {
        st.assign(i, create(a[i]));
        if (a[i] >= 0 && st.accumulate(0, n).overall > a[i]) {
            ps("NO");
            return;
        }
    }
