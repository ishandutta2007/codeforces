...
int main() {
    ...
    auto dfs = [&](auto dfs, int u, int p) -> vector<mint> {
        vector<mint> dp_u { 1 };
        for (int v : g[u]) {
            if (v == p) continue;
            auto dp_v = dfs(dfs, v, u);
            const int sz_u = dp_u.size(), sz_v = dp_v.size();
            dp_u.resize(min(k + 1, max(sz_u, sz_v)), 0);
            rrep(i, sz_u) {
                mint val = dp_u[i];
                dp_u[i] = 0;
                const int jmax = min(k - i + 1, sz_v);
                rep(j, jmax) dp_u[max(i, j)] += val * dp_v[j];
            }
        }
        dp_u.insert(dp_u.begin(), accumulate(dp_u.begin(), dp_u.end(), mint(0)));
        return dp_u;
    };
    print(dfs(dfs, 0, -1).front());
    return 0;
}