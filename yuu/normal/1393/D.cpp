...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            ul[i][j] = 1;
            if (c[i - 1][j] != c[i][j]) continue;
            if (c[i][j - 1] != c[i][j]) continue;
            ul[i][j] = min(ul[i][j - 1], ul[i - 1][j]) + 1;
        }
        for (int j = m; j >= 1; j--) {
            ur[i][j] = 1;
            if (c[i - 1][j] != c[i][j]) continue;
            if (c[i][j + 1] != c[i][j]) continue;
            ur[i][j] = min(ur[i][j + 1], ur[i - 1][j]) + 1;
        }
    }
    for (int i = n; i >= 1; i--) {
        for (int j = 1; j <= m; j++) {
            ...
            if (c[i + 1][j] != c[i][j]) continue;
            if (c[i][j - 1] != c[i][j]) continue;
            dl[i][j] = min(dl[i][j - 1], dl[i + 1][j]) + 1;
        }
        for (int j = m; j >= 1; j--) {
            dr[i][j] = 1;
            if (c[i + 1][j] != c[i][j]) continue;
            if (c[i][j + 1] != c[i][j]) continue;
            dr[i][j] = min(dr[i][j + 1], dr[i + 1][j]) + 1;
        }
    }
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) {
