...
struct cmp{
	bool operator()(const bits&x,const bits&y)const{
		if(x==y)return 0;
		else{
			bits z=x^y;
			int p=z._Find_first();
...
map<bits,int,cmp>mp[10];
int dfs(int k1,bits k2){
	if(k1==n){
		return k2.count()==1u<<n;
	}
	if(mp[k1].count(k2))return mp[k1][k2];
	int&res=mp[k1][k2];
	vector<bits>b(n);
	rep(i,0,(1<<n)-1)if(k2[i])rep(j,0,n-1)if(~i>>j&1)b[j].set(i^(1<<j));
	rep(i,0,(1<<n)-1){
		bits k3=k2;
		rep(j,0,n-1){
			if(i>>j&1)k3|=b[j];
