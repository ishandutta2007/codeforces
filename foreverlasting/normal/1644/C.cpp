...
namespace MAIN{
    ...
    inline void MAIN(){
        n=read();
        res x=read();
        for(res i=1;i<=n;i++)a[i]=read(),sum[i]=sum[i-1]+a[i],ans[i]=0;
        ans[0]=0;
        for(res len=1;len<=n;len++){
            res ret=-inf;
            for(res l=1;l<=n-len+1;l++){
                res r=l+len-1;
                ret=max(ret,sum[r]-sum[l-1]);
            }
            for(res k=0;k<=len;k++)ans[k]=max(ans[k],ret+k*x);
        }
        res ret=0;
        for(res i=0;i<=n;i++)ret=max(ret,ans[i]),printf("%d ",ret);
        puts("");
    }
}
int main(){
