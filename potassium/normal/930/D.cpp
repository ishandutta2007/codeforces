...
int main() {
	...
	for (int p = 0; p <= 1; p++) {
		...
		vector<pair<int, pii>> Events; // x, type, y
		for (auto& i : pts) {
			int lx = i.first >> 1;
			int ly = i.second >> 1;

			Events.push_back({ -INF, { 1, ly } });
			Events.push_back({ lx + 1, { -1, ly } });
			Events.push_back({ -INF, { 2, ly + 1 } });
			Events.push_back({ lx + 1, { -2, ly + 1 } });
			Events.push_back({ lx + 1, { 3, ly } });
			Events.push_back({ INF, { -3, ly } });
			Events.push_back({ lx + 1, { 4, ly + 1 } });
			Events.push_back({ INF, { -4, ly + 1 } });
		}

		sort(all(Events));
		vector<multiset<int>> plane(5);
		for (int i = 0; i < (int)Events.size(); i++) {
			int type = Events[i].second.first;
			int val = Events[i].second.second;

			if (type > 0)
				plane[type].insert(val);
			else
				plane[-type].erase(plane[-type].find(val));

			if (i + 1 < (int)Events.size() && Events[i].first != Events[i + 1].first &&
				!plane[1].empty() && !plane[2].empty() && !plane[3].empty() && !plane[4].empty()) {
				int ymax = min(*plane[1].rbegin(), *plane[3].rbegin());
				int ymin = max(*plane[2].begin(), *plane[4].begin());

				ans += 1LL * max(0, ymax - ymin + 1) * (Events[i + 1].first - Events[i].first);
			}
		}
	}
