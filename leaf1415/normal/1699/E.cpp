...
int main(void)
{
	...
	while(T--){
		cin >> n >> m;
		rep(i, 1, n) cin >> a[i];
		rep(i, 1, m) have[i] = false;
		rep(i, 1, n) have[a[i]] = true;
		
		ll ans = inf, mx = m+1;
		rep(i, 1, m+1) cnt[i] = 0;
		rep(i, 1, m) if(have[i]) cnt[m+1]++;
		
		rep(i, 1, m) dp[i] = m+1, used[i] = false;
		per(i, m, 1){
			dp[i] = i;
			if(have[i]) cnt[m+1]--, cnt[i]++;
			used[i] = true;
			
			for(int j = i; j <= m; j += i){
				if(!used[j]) continue;
				for(int k = 2*j; k <= m; k+=j){
					ll x = max(dp[j], dp[k/j]);
					if(dp[k] > x){
						if(have[k]) cnt[dp[k]]--, cnt[x]++;
						dp[k] = x, used[k] = true;
					}
				}
			}
			
			for(int j = i; j <= m; j += i) used[j] = false;
			while(cnt[mx] == 0) mx--;
			if(mx <= m) chmin(ans, mx-i);
