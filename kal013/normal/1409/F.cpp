	...
	for(int i = 1; i <= n; ++i){
		for(int x = 0; x <= i; ++x){
			for(int y = 0; y <= k; ++y){
				...
				if (y > 0 && dp[i - 1][x][y - 1] >= 0){
					dp[i][x][y] = max(dp[i][x][y], dp[i - 1][x][y - 1] + x);
				}
				if (y > 0 && x > 0 && dp[i - 1][x - 1][y - 1] >= 0){
					dp[i][x][y] = max(dp[i][x][y], dp[i - 1][x - 1][y - 1]);
				}
				if (s[i - 1] == t[0]){
					// no change
					if (x != 0){
						dp[i][x][y] = max(dp[i][x][y],dp[i - 1][x - 1][y]);
					}	
				}
				else if (s[i - 1] == t[1] && dp[i - 1][x][y] >= 0){
					dp[i][x][y] = max(dp[i][x][y], dp[i - 1][x][y] + x);
				}
				else{
					dp[i][x][y] = max(dp[i][x][y], dp[i - 1][x][y]);
				}
