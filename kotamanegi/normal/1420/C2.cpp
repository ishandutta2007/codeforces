...
int e() { return 0LL; }
segtree<int, op, e> calcs[2];
int itr[600000];
int n, query;
void eval() {
	int ans = calcs[0].prod(0, itr[n]);
	ans += n;
	ans += calcs[1].prod(itr[n] + 1, n);
	cout << ans << "\n";
	return;
}
void solve() {
	cin >> n >> query;
	REP(q, 2) {
		calcs[q] = segtree<int, op, e>(n + 1);
	}
	REP(i, n) {
		cin >> inputs[i];
		itr[inputs[i]] = i;
	}
	calcs[1].set(0, inputs[0]);
	for (int i = 1; i < n; ++i) {
		if (inputs[i - 1] < inputs[i]) {
			calcs[1].set(i, inputs[i] - inputs[i - 1]);
		}
		else {
			calcs[0].set(i, inputs[i - 1] - inputs[i]);
		}
	}
	eval();
	REP(i, query) {
		...
		cin >> a >> b;
		a--; b--;
		swap(inputs[a], inputs[b]);
		itr[inputs[a]] = a;
		itr[inputs[b]] = b;
		REP(q, 2) {
			for (int i = max(1LL, a); i < min(n, a + 2LL); ++i) {
				if (inputs[i - 1] < inputs[i]) {
					calcs[1].set(i, inputs[i] - inputs[i - 1]);
					calcs[0].set(i, 0);
				}
