...
Dinic<20000> D;
int n,m,g; // wow this problem is so unclear
...
int main() {
    ...
    ll res = 0;
    F0R(i,n) {
        if (s[i] == 1) D.addEdge(i+2,1,v[i]);
        else D.addEdge(0,i+2,v[i]);
    }
    F0R(i,m) {
        int t,w,k; re(t,w,k); res += w;
        vi z(k); re(z); trav(q,z) q --;
        int f; re(f); w += f*g;
        if (t == 0) {
            D.addEdge(0,n+2+i,w);
            trav(q,z) D.addEdge(n+2+i,q+2,MOD);
        } else {
            D.addEdge(n+2+i,1,w);
            trav(q,z) D.addEdge(q+2,n+2+i,MOD);
        }
    }
    ps(res-D.maxFlow(0,1));
}
