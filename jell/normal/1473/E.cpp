#line 1 "codeforces-workspace/contest/1473/e/e.cpp"
...
#line 11 "codeforces-workspace/contest/1473/e/e.cpp"
...
void main() {
  ...
  auto dist = make_vector<i64>({n, 2, 2}, inf);
  while (m--) {
    int a, b;
    cin >> a >> b;
    --a, --b;
    int w;
    cin >> w;
    grh[a].emplace_back(b, w);
    grh[b].emplace_back(a, w);
  }

  priority_queue<tuple<i64, int, int, int>, greater<>> pq;
  pq.emplace(0, 0, 0, 0);
  dist[0][0][0] = 0;
  while (!pq.empty()) {
    auto [d, v, x, y] = pq.top();
    pq.pop();
    if (d != dist[v][x][y]) continue;
    for (auto &&[u, w] : grh[v]) {
      if (!x) {
        if (chle(dist[u][1][y], d)) {
          pq.emplace(d, u, 1, y);
        }
      }
      if (!y) {
        if (chle(dist[u][x][1], d + w * 2)) {
          pq.emplace(d + w * 2, u, x, 1);
        }
      }
      if (!x && !y) {
        if (chle(dist[u][1][1], d + w)) {
          pq.emplace(d + w, u, 1, 1);
        }
      }
      if (chle(dist[u][x][y], d + w)) {
        pq.emplace(d + w, u, x, y);
      }
    }
  }
