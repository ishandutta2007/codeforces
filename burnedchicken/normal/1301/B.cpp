...
//i_am_noob
int t,n,a[maxn],maxx,minn,k,ans;
...
int main(){
	...
	while(t--){
		...
		rep(n){
			if(a[i]>=0){
				if(i!=n-1) if(a[i+1]==-1) maxx=max(maxx,a[i]),minn=min(minn,a[i]);
				if(i!=0) if(a[i-1]==-1) maxx=max(maxx,a[i]),minn=min(minn,a[i]);
			}
		}
		k=maxx+minn>>1;
		rep(n) if(a[i]==-1) a[i]=k;
		ans=-1;
		rep(n-1) ans=max(ans,abs(a[i]-a[i+1]));
		print2(ans,k);
	}
	return 0;
}