...
class intMaxLazy (nSuggest  :Int){
    ...
    val max = IntArray(n * 2){nlarge}
    val min = IntArray(n * 2){plarge}
    ...
    private fun updateNode(i:Int,level:Int){
        max[i] = maxOf(max[i shl 1 ], max[(i shl 1) + 1]) + lazy[i]
        min[i] = minOf(min[i shl 1] , min[(i shl 1) + 1 ]) + lazy[i]
    }
    ...
    fun rangeMaxQuery(l:Int, r:Int):Int {
        pushPath(l+n)
        pushPath(r+n)
    ...
    fun rangeMinQuery(l:Int, r:Int):Int {
        pushPath(l+n)
        pushPath(r+n)
...
fun main(){
    solve.cases{
        ...
        val have = Array(n+1){mint}
        for((i, v) in L.withIndex()) {
            have[v].add(i)
        }

        val st = intMaxLazy(IntArray(n+1){it})
        val strange = IntArray(n){0}

        for(pt in 0..n){
            for(c in have[pt]){
                val rbest = st.rangeMaxQuery(c+1,n)
                val lbest = st.rangeMinQuery(0,c)
                val max = rbest - lbest
                strange[c] = maxOf(strange[c], (max) shr 1)
            }
            for(c in have[pt]){
                st.rangeAdd(c+1,n,-2)
            }
            for(c in have[pt]){
                val rbest = st.rangeMinQuery(c+1,n)
                val lbest = st.rangeMaxQuery(0,c)
                val max = (rbest - lbest) + 2
                strange[c] = maxOf(strange[c], -((max) shr 1))
            }
