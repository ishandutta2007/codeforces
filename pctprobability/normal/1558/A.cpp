...
void solve(){
  ...
  map<ll,ll> ans;
  ll as=c/2,bs=(c+1)/2;
  for(int i=max(0ll,as-b);i<=min(a,as);i++) m[i]++;
  for(auto e:m) ans[e.fi*2+b-as]++;
  swap(as,bs);
  map<ll,ll> m2;
  for(int i=max(0ll,as-b);i<=min(a,as);i++) m2[i]++;
  for(auto e:m2) ans[e.fi*2+b-as]++;
  swap(as,bs);
  vector<ll> s;
  for(auto e:ans) s.pb(e.fi);
  cout<<s.size()<<endl;
  for(auto e:s) cout<<e<<" ";
  cout<<endl;
}
