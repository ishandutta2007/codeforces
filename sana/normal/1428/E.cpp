...
long long calc(int st, int parts) {
    int fl = st / parts;
    int rem = st - parts * fl;
    int ori = parts - rem;

    return rem * 1ll * (fl+1) * 1ll * (fl+1) + ori * 1ll * fl * 1ll * fl;
}

long long red(int st, int top) {
    return calc(st, top) - calc(st, top+1);
}

struct comp {
    bool operator()( pair<int, int> a, pair<int, int> b) const {
        return red(a.first, a.second) > red(b.first, b.second);
    }
};
