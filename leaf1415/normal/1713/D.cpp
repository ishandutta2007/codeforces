...
int main(void)
{
	...
	while(T--){
		cin >> n;
		vector<ll> vec;
		rep(i, 1, (1<<n)) vec.pb(i);
			
		
		while(sz(vec) >= 4){
			vector<ll> nvec; ll res;
			reps(i, vec){
				if(i % 4) continue;
				ll res = query(vec[i], vec[i+2]);
				if(res == 0){
					if(query(vec[i+1], vec[i+3]) == 1) nvec.pb(vec[i+1]);
					else nvec.pb(vec[i+3]);
				}
				else if(res == 1){
					if(query(vec[i], vec[i+3]) == 2) nvec.pb(vec[i+3]);
					else nvec.pb(vec[i]);
				}
				else{
					if(query(vec[i+2], vec[i+1]) == 2) nvec.pb(vec[i+1]);
		...
		if(sz(vec) == 2){
			if(query(vec[0], vec[1]) == 1) outl("!", vec[0]);
			else outl("!", vec[1]);
		}
		else outl("!", vec[0]);
	}
