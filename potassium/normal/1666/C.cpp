...
int main() {
	...
	auto push = [&](ll a, ll b, ll c, ll d) {
		ans.push_back({ { a, b }, { c, d } });
	};

	pll cen = { x[1], y[1] };
	auto con = [&](pll u, pll v) {
		pll now = u;
		if (now.first != v.first) {
			push(now.first, now.second, v.first, now.second);
			now.first = v.first;
		}
		if (now.second != v.second) {
			push(now.first, now.second, now.first, v.second);
			now.second = v.second;
		}
