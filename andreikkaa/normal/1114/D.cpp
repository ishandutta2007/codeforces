...
inline void _main_function() {
    ...
    for(int r = 0; r < n; ++r) {
        for(int l = r; l >= 0; --l) {
            if(l) {
                dp[l - 1][r][0] = min({dp[l - 1][r][0], dp[l][r][0] + (a[l - 1] != a[l]), dp[l][r][1] + (a[l - 1] != a[r])});
            }
            
            if(r + 1 < n) {
                dp[l][r + 1][1] = min({dp[l][r + 1][1], dp[l][r][0] + (a[l] != a[r + 1]), dp[l][r][1] + (a[r] != a[r + 1])});
            }
        }
    }
