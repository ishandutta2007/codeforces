...
int main() {
    ...
    int N; re(N); vector<pt> pts(N); re(pts);
    vpii seg; F0R (i, N) F0R (j, N) if (i != j) seg.eb(i, j);
    sort_by(seg, (pts[a.f] - pts[a.s]).ang() < (pts[b.f] - pts[b.s]).ang());

    F0R (i, N) dp[i][i][0] = 1;
    for (auto [j, k] : seg) {
        F0R (i, N) {
            F0R (s, 5) dp[i][k][s+1] += dp[i][j][s];
        }
    }

    ll ans = 0;
    F0R (i, N) ans += dp[i][i][5];
    ps(ans);
