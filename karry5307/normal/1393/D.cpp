...
const ll MAXN=3e3+51,MOD=998244353,inf=0x3f3f3f3f;
// Structures and variables
ll n,m;
li res;
char ch[MAXN][MAXN];
ll u[MAXN][MAXN],d[MAXN][MAXN],l[MAXN][MAXN],r[MAXN][MAXN];
ll lf[MAXN][MAXN],rf[MAXN][MAXN];
// Fast IO
inline ll read()
{
...
int main()
{
	...
	for(register int i=1;i<=n;i++)
	{
		...
		{
			u[i][j]=(ch[i][j]==ch[i-1][j])?u[i-1][j]+1:1;
		}
	}
	for(register int i=n;i;i--)
	{
		for(register int j=1;j<=m;j++)
		{
			d[i][j]=(ch[i][j]==ch[i+1][j])?d[i+1][j]+1:1;
		}
	}
	for(register int i=1;i<=n;i++)
	{
		for(register int j=1;j<=m;j++)
		{
			...
			{
				lf[i][j]=min(min(u[i][j],d[i][j]),lf[i][j-1]+1);
			}
		}
		for(register int j=m;j>=1;j--)
		{
			...
			{
				rf[i][j]=min(min(u[i][j],d[i][j]),rf[i][j+1]+1);
			}
		}
		for(register int j=1;j<=m;j++)
		{
			res+=min(lf[i][j],rf[i][j]);
		}
	}
	printf("%lld\n",res);
}