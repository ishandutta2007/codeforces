...
void q() {
  ...
  if(a > b){
    // 
    int n = a - b;
    if(x - x1 < n) { out("NO"); return;}
  }
  else if(a < b){
    int n = b - a;
    if(x2 - x < n) { out("NO"); return;}
  }
  else {
    if(a != 0 && x1 == x && x == x2) { out("NO"); return;}
  }

  if(c > d){
    int n = c - d;
    if(y - y1 < n) { out("NO"); return;}
  }
  else if(c < d){
    int n = d - c;
    if(y2 - y < n) { out("NO"); return;}
  }
  else {
    if(c != 0 && y1 == y && y == y2) { out("NO"); return;}
  }
  out("YES");
}
