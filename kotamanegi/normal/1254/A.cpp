...
void solve() {
    ...
    REP(i, r) {
        ...
        REP(q, c) {
            int itr = q;
            if (i % 2 == 1) itr = c - itr - 1;
            if (s[itr] == 'R') {
                inputs.push_back(mp(i, itr));
                grid[i][itr] = 1;
            }
        }
    }
    int itr = 0;
    int cnt = 0;
    while (itr != inputs.size()) {
        cnt++;
        int geko = inputs.size() - itr + k - 1;
        geko /= k;
        geko += itr;
        for (int q = itr; q < geko; ++q) {
            grid[inputs[q].first][inputs[q].second] = cnt;
            for (int j = inputs[q].second - 1; j >= 0; --j) {
                if (grid[inputs[q].first][j] != 0) break;
                grid[inputs[q].first][j] = cnt;
            }
            for (int j = inputs[q].second + 1; j < c; ++j) {
                if (grid[inputs[q].first][j] != 0) break;
                grid[inputs[q].first][j] = cnt;
            }
        }
    ...
    REP(i, r) {
        REP(q, c) {
            int itr = q;
            if (i % 2) itr = c - 1 - itr;
            gogo.push_back(mp(i, itr));
        }
    }
    REP(i, gogo.size()) {
        if (i != 0) {
            if (grid[gogo[i].first][gogo[i].second] == 0) {
                grid[gogo[i].first][gogo[i].second] = grid[gogo[i-1].first][gogo[i-1].second];
            }
        }
    }
    REP(tea, gogo.size()) {
        if (tea != 0) {
            int i = gogo.size() - 1 - tea;
            if (grid[gogo[i].first][gogo[i].second] == 0) {
                grid[gogo[i].first][gogo[i].second] = grid[gogo[i + 1].first][gogo[i + 1].second];
            }
        }
