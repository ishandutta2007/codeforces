...
struct matrix {
	...
	vector<vector<long long>> mas;
	matrix(unsigned n, unsigned m) {
		mas = vector<vector<long long>>(n, vector<long long>(m, 0));
	}
	matrix(vector<vector<long long>> a) {
		mas = a;
	}
	matrix operator*(const matrix &o)const {
		matrix ret(size(), o[0].size());
		assert(o.size() == mas[0].size());

		for (unsigned i = 0; i < size(); i++) {
			for (unsigned j = 0; j < o[0].size(); j++) {
				for (unsigned k = 0; k < o.size(); k++) {
					ret[i][j] += mas[i][k] * o[k][j];
					ret[i][j] %= mod;
				}
			}
		}
...
matrix pow(matrix x, unsigned n) {
	if (n == 0)
		return matrix::null(x.size());
	else if (n == 1)
		return x;
	else if (n == 2)
		return x * x;
