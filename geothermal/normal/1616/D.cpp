...
void solve() {
    ...
    F0Rd(i, N) {
        while (vals.ub({-pre[i], MX}) != vals.end()) {
            auto it = vals.ub({-pre[i], MX});
            lst[it->s] = i;
            vals.erase(it);
        }
        vals.ins({-pre[i+1], i+1});
    }
    FOR(i, 1, N+1) ckmax(lst[i], lst[i-1]);

    int ans = N;
    FOR(i, 1, N+1) {
        int cur = 0;
        if (lst[i] != -1) {
            cur = query(lst[i], i-1) + 1;
        }
        update(i, cur);
        //dbg(cur, lst[i]);
        ckmin(ans, cur + N - i);
