...
fn run<W: Write>(sc: &mut scanner::Scanner, out: &mut std::io::BufWriter<W>) {
    ...
    for _ in 0..t {
        ...
        let mut weight = vec![0usize; n + 1];
        for i in 0..n {
            let d = n / binary_gcd(n, i);
            if g % d == 0 {
                weight[d] += 1;
            }
        }
        let mut way = M::zero();
        let mut edge = M::zero();
        for (d, weight) in weight.iter().enumerate().filter(|p| *p.1 != 0) {
            let w = M::from(*weight);
            let c = &cnt;
            if c[0] / d == 1 {
                way += w * pc.fact(c.len());
            ...
            let mut dp = (M::one(), M::one());
            let mut len = c[0] / d;
            for &c in c[1..].iter() {
                let q = c / d;
                let mut next = (dp.0 * pc.comb(len + q, q), dp.1 * pc.comb(len - 2 + q, q));
                if q >= 2 {
                    next.1 += dp.0 * pc.comb(len + q - 2, q - 2);
                }
                len += q;
                dp = next;
            }
            way += w * dp.0;
            edge += w * dp.1 * M::from(n);
        }
        let ans = M::from(n) - edge * way.inv();
        writeln!(out, "{}", ans).ok();
    }
}