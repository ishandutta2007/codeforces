...
string decode(ll x) {
	string res;
	while (x) res.push_back('0'+x%2), x /= 2;
...
int main() {
	...
	int t; cin >> t;
	for (size_t iteration = 0; iteration < t; iteration++) {
		int n, m; cin >> n >> m;
		VLL a;
		...
		auto check = [&](ll x) {
			return x - (lower_bound(all(a), x) - a.begin());
		};
		...
		ll res = lb;
		dump(res);
		string dec = decode(res);
		reverse(all(dec));
		while (dec.size() < m) dec.push_back('0');
		reverse(all(dec));
		cout << dec << "\n";
	}
}