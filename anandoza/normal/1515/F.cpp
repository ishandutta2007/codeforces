...
int main() {
    ...
    union_find uf(n);
    set<pair<ll, int>, greater<pair<ll, int>>> components;
    for (int i = 0; i < n; i++) {
        components.insert({a[i], i});
    }

    vi ans;
    for (int step = 0; step < n - 1; step++) {
        int cur = components.begin()->second;

        int i, u, v;
        do {
            i = adj[cur].back();
            u = uf.find(edges[i].first);
            v = uf.find(edges[i].second);
            adj[cur].pop_back();
        } while (u == v);
        ...
        if (adj[child].size() > adj[root].size())
            swap(adj[root], adj[child]);
        adj[root].insert(adj[root].end(), all(adj[child]));

        components.erase({a[child], child});
        components.erase({a[root], root});
        a[root] += a[child] - x;
        a[child] = 0;
        components.insert({a[root], root});
    }
