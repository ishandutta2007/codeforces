...
ll n, k;
ll a[100005];
ll par[100005], s[100005], dep[100005];
vector<ll> G[100005];

void dfs(int v, int p, int d)
{
	par[v] = p, s[v] = a[v], dep[v] = d;
	for(auto u : G[v]){
		if(u == p) continue;
		dfs(u, v, d+1);
		s[v] ^= s[u];
...
int main(void)
{
	...
	while(T--){
		...
		vector<P> vec;
		rep(i, 1, n) if(s[i] == sum) vec.push_back(P(dep[i], i));
		sort(all(vec));
		
		set<ll> S;
		v = vec.back().second; bool ans = false;
		while(v != -1){
			S.insert(v);
			if(s[v] == 0) ans = true;
			v = par[v];
		...
		for(auto p : vec){
			if(S.count(p.second) == 0) ans = true;
		}
		if(ans) yes();
		else no();
	}
