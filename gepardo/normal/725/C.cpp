...
string s = in;
string s2;
vector <int> rep(s.length(), false);

int neigbour(int x1, int y1, int x2, int y2)
{
...
int neigbour(char a, char b)
{
    int x1, y1, x2, y2;
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 13; j++)
        {
            if (c[i][j] == a) x1 = i, y1 = j;
            if (c[i][j] == b) x2 = i, y2 = j;
        }
    return neigbour(x1, y1, x2, y2);
}

void check()
{
    for (int i = 1; i < s.length(); i++)
        if (!neigbour(s[i], s[i-1])) return;
    out = c[0];
    out = c[1];
    exit(0);
}
