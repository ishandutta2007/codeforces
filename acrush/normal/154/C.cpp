...
int64 solve(int *c,int n,int pos)
{
	if (pos<0) return (int64)n*(n-1)/2;
	for (int i=0;i<n;i++) q[i]=MP(a[c[i]][pos],c[i]);
	sort(q,q+n);
	for (int i=0;i<n;i++) c[i]=q[i].second;
	int64 ret=0;
	for (int i=0;i<n;i++) if (i==0 || a[c[i]][pos]!=a[c[i-1]][pos])
	{
		int L=0;
		for (int j=i;j<n && a[c[i]][pos]==a[c[j]][pos];j++) L++;
		ret+=solve(c+i,L,pos-1);
	}
	return ret;
}
int64 solve()
{
	...
	for (int i=0;i<n;i++) g[SIZE(a[i])].push_back(i);
	int64 ret=0;
	for (int k=0;k<=n;k++) if (SIZE(g[k])>0) ret+=solve(&g[k][0],SIZE(g[k]),k-1);
	return ret;
}
