...
int main(){
    ...
    VV<int> exdist(n, V<int>(n, inf));
    REP(i, n) REP(j, n) REP(k, m) chmin(dist[i][j], abs(a[i][k] - a[j][k]));
    REP(i, n) REP(j, n) REP(k, m - 1) chmin(exdist[i][j], abs(a[i][k + 1] - a[j][k]));
    //nowbitDP
    VV<V<int>> dp(pow(2, n), VV<int>(n, V<int>(n, 0)));
    int res = 0;
    REP(k, n){
        dp[(1 << k)][k][k] = inf;
        for(int i = 1; i < (1 << n); i++){
            REP(j, n) REP(l, n){
                if(j == l or i >> l & 1 or !dp[i][k][j]) continue;
                chmax(dp[i | (1 << l)][k][l] , min(dp[i][k][j], dist[j][l]));
            }
        }
        REP(j, n) if(dp[(1 << n) - 1][k][j] != INF) chmax(res, min(dp[(1 << n) - 1][k][j], exdist[j][k]));
    }
    print(res);
}