...
void Init(){
	...
	fai[N-1]=Pow(fac[N-1],mo-2);
	per(i,1,N-1)fai[i-1]=1ll*fai[i]*i%mo;
}
int C(int n,int m){
	return n<m?0:1ll*fac[n]*fai[m]%mo*fai[n-m]%mo;
}
int S(int n,int k){
	ll res=0,t=1;
	per(i,1,k){
		res+=t*C(k,i)*Pow(i,n)%mo;
		t*=-1;
	}
	res=(res%mo+mo)%mo;
	return res*fai[k]%mo;
...
int main(){
	...
	res%=mo;
	debug("%lld\n",res);
	res=res*(S(n,k)+1ll*(n-1)*S(n-1,k)%mo)%mo;
	out,res;
}