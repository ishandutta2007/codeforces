...
struct CentroidDecomposition{
  ...
  void build_dfs(int k,int par,int cent){
	...
	void dfs(int k,int par,vector<int>&ver){
		ver.push_back(k);
		for(auto e:g[k]){
			if(e.to==par||used[e.to])continue;
			dfs(e.to,k,ver);
...
int main(){
  ...
  auto cent=cd.centroid_enum();
  ll cnt=0;
  set<ll>cst;
  for(auto c:cent)cst.insert(c);
  stringstream out;
  auto ope=[&](ll x,ll y1,ll y2){
    out<<x+1 spa y1+1 spa y2+1<<endl;
    cnt++;
  };
  for(auto c:cent){
    for(auto st:g[c]){
      if(cst.count(st))continue;
      vector<ll>v;
      v.PB(c);
      P last(c,st);
