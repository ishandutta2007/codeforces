...
void add(vector<int> &fenw, int pos, int val)
{
    while (pos < fenw.size())
    {
        fenw[pos] += val;
...
int get(vector<int> &fenw, int pos)
{
    int res = 0;
    while (pos >= 0)
    {
...
void solve()
{
    ...
    ll sets_factorial = fact(n);
    vector<int> cnt(N);
    forn(i, n) cnt[s[i]]++;
    forn(i, N) sets_factorial = 1ll * sets_factorial * __ufact[cnt[i]] % MOD;

    vector<int> fenw(N);
    forn(i, N) add(fenw, i, cnt[i]);
    ...
    for (int i = 0; i < n; i++)
    {
        //cout << sets_factorial << " " << get(fenw, t[i] - 1) << "\n";
        if (t[i] == -1)
        {
            break;
        }
        ans = (ans + 1ll * sets_factorial * __rev[sz] % MOD * get(fenw, t[i] - 1)) % MOD;
        sets_factorial = 1ll * sets_factorial * __rev[sz] % MOD * cnt[t[i]] % MOD;
        cnt[t[i]]--, add(fenw, t[i], -1), sz--;
        if (cnt[t[i]] < 0)
        {
            fl = 1;
            break;
