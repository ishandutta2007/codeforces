...
int main() {
    ...
    if(!s) {
        ...
        for(auto [_, c] : mp) { ans += mint(c) * (c - 1); }
        ans += 1;
        OUT(ans);
    } else {
        vmint dp(k + 1);
        ...
        if(!f) {
            ...
            rep(i, n) {
                ...
                ans += mint(m) * (i * 2 + 1) * ALL;

                // OUT(ans);

                int l = id, r = id + REM;
                ans += mint(m) * seg.query(l, r);
                // dump(l, r);
                seg.add(id, 1), seg.add(id + k, 1);
                ...
                // if(inc(id + k, l, r) or inc(id, l, r)) ans += m;
            }
