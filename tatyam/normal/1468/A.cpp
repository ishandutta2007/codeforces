...
using S = pair<ll, ll>;
inline constexpr S op(S a, S b){
    return {max(a.first, b.first), max(a.second, b.second)};
}
inline constexpr S e(){
    return {0, 0};
}
using F = bool;
inline constexpr S mapping(F f, S x){
    return {x.first, max(x.second, x.first + f)};
}
inline constexpr F composition(F f, F g){
    return f || g;
}
inline constexpr F id(){
    return 0;
}
