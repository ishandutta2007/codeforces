...
void solve() {
    VEC(int, b, 3);
    VEC(int, c, 3);
    if((c[0] - b[0]) % b[1] != 0 or c[1] % b[1] != 0) {
        out(0);
        return;
    }
    int x = (c[0] - b[0]) / b[1], y = (c[0] + (c[2] - 1) * c[1] - b[0]) / b[1];
    unless(0 <= x and y < b[2]) {
        out(0);
        return;
    }
    if(b[0] == c[0] or b[0] + (b[2] - 1) * b[1] == c[0] + (c[2] - 1) * c[1]) {
        out(-1);
        return;
    }
    mint ans = 0;
    vc<int> ds;
    for(int d = 1; d * d <= c[1]; ++ d) {
        if(c[1] % d == 0) {
            ds.eb(d);
            if(d * d != c[1]) ds.eb(c[1] / d);
        }
    }
    for(int d: ds) {
        int step = d / __gcd(d, b[1]) * b[1];
        if(step == c[1] or c[2] == 1) {
            int can = step / d;
            ans += mint(can) * mint(can);
        }
        if(c[0] - step < b[0]) {
            out(-1);
            return;
        }
        if(b[0] + (b[2] - 1) * b[1] < c[0] + (c[2] - 1) * c[1] + step) {
            out(-1);
            return;
        }
    }
