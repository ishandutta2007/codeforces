...
main() {
    ...
    auto check = [&](int i, int j, int k) {
        ll sum = 0;
        rep(t, 5) sum += (a[j][t] - a[i][t]) * (a[k][t] - a[i][t]);
        return (sum <= 0);
    };
    rep2(i, 1, n - 1) {
        if(i != n - 1) {
            v.eb(i), v.eb(++i);
            vi w;
            if(check(v[0], v[1], v[2])) w.eb(v[0]);
            if(check(v[1], v[0], v[2])) w.eb(v[1]);
            if(check(v[2], v[0], v[1])) w.eb(v[2]);
            v = w;
        } else
            v.eb(i);
    }
    ...
    for(auto e : v) {
        auto f = [&]() -> void {
            rep(i, n) {
                rep(j, n) {
                    if(i == j or i == e or j == e) continue;
                    if(!check(e, i, j)) return;
                }
            }
            ans.eb(e);
            return;
