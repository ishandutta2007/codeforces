...
signed main(){
    ...
    vv(ll,mem,n,n,-1);
    rep(n)mem[i][i]=0;
    auto dp=[&](ll x,ll y,auto f)->ll{
        if(mem[x][y]!=-1)return mem[x][y];
        ll x2=parent[y][x],y2=parent[x][y];
        return mem[x][y]=max(f(x,y2,f),f(x2,y,f))+siz[x][y]*siz[y][x];
    };
    ll ans=0;
    rep(n)rep(j,n)chmax(ans,dp(i,j,dp));
    out(ans);
}