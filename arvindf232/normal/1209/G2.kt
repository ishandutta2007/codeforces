...
//                t.add(s.first(),s.last(),s.size)
        ...
        fun addDelete(deleting:Boolean, i:Int, v:Int){
            val s = things[v]
            if(s.isNotEmpty()){
                val oldroot = s.first()
                st.pointset(oldroot,0)
            }

            val previous = if(s.isEmpty()) Pair(0,0) else Pair(s.first(), s.last())
            if(deleting){
                s.remove(i)
            }else{
                s.add(i)
            }
            val now = if(s.isEmpty()) Pair(0,0) else Pair(s.first(), s.last())
            if(previous != now){
                val (l,r) = previous
                st.rangeAdd(l,r,-1)
                val (ll,rr) = now
        ...
        repeat(q){
            ...
            addDelete(true, i,old)
            addDelete(false, i, new)
            put(n - st.answer())
