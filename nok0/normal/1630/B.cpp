...
#line 2 "/Users/nok0/Documents/Programming/nok0/data_structure/binary_indexed_tree.hpp"
...
void main_() {
	...
	V<> cnt(n);
	foa(v, a) cnt[v]++;
	int need = divup(n + k, 2);
	binary_indexed_tree<int> bt(n);
	REP(i, n) { bt.add(i, cnt[i]); }
	int len = inf;
	int nl = -1, nr = -1;
	REP(l, n) {
		int r = bt.lower_bound(need + bt.sum(0, l)) + 1;
		if(r <= n and bt.sum(l, r) >= need) {
			if(chmin(len, r - l)) {
				nl = l, nr = r;
			}
		}
