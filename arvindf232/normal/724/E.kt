...
const val huge = Long.MAX_VALUE - 1_000_000_000_000_000L // used for seg tree things
...
fun main(){
    solve.cases{
        ...
        var DP = LongArray(n+1){huge}
        DP[0] = 0L
        var newDP = LongArray(n+1){huge}
        for(s in 0 until n){
            val d = D[s]
            val u = U[s]
            newDP[0] = DP[0] + u
            ...
            for(i in 0..s){
                res += c
                newDP[i+1] = minOf(DP[i+1] + res, DP[i] + d)

            }
//            var res = u.toLong()
//            for(i in 0..s){
//                newDP[i] = minOf(newDP[i], DP[i] + res)
//                res += c
//            }
            val temp = newDP
            newDP = DP
...
/*
forgot initial conditoons of having 0
bad variabln swap (newDP and DP)
up and down sawp ( started off thinkign numbre of up, then become numbre of down halfway through)
 forgetting taking min

 no single variable names!
    - only indices are allowed
