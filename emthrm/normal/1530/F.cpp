...
int main() {
  int n; cin >> n;
  vector a(n, vector(n, ModInt(1) / 10000));
  REP(i, n) REP(j, n) {
    int aij; cin >> aij;
    a[i][j] *= aij;
  }
  ...
  // ModInt ans = 0;
  // REP(b, 1 << (n * n)) {
  //   vector bingo(n, vector(n, 0));
  //   REP(i, n) REP(j, n) bingo[i][j] = b >> (i * n + j) & 1;
  //   ModInt p = 1;
  //   REP(i, n) REP(j, n) p *= (bingo[i][j] ? a[i][j] : -a[i][j] + 1);
  //   bool is_winning = false;
  //   REP(y, n) {
  //     bool is_true = true;
  //     REP(j, n) is_true &= bingo[y][j];
  //     is_winning |= is_true;
  //   }
  //   REP(x, n) {
  //     bool is_true = true;
  //     REP(i, n) is_true &= bingo[i][x];
  //     is_winning |= is_true;
  //   }
  //   {
  //     bool is_true = true;
  //     REP(i, n) is_true &= bingo[i][i];
  //     is_winning |= is_true;
  //   }
  //   {
  //     bool is_true = true;
  //     REP(i, n) is_true &= bingo[i][n - 1 - i];
  //     is_winning |= is_true;
  //   }
  //   if (is_winning) ans += p;
  // }
  ...
  REP(diag, 4) {
    ...
    vector<ModInt> dp(1 << n, 1);
    REP(row, n) {
      vector<ModInt> prob(1 << n, 1);
      FOR(i, 1, 1 << n) {
        const int lsb = i & -i;
        prob[i] = prob[i ^ lsb];
        const int col = __builtin_ctz(lsb);
        prob[i] *= b[row][col];
      }
      REP(i, 1 << n) prob[i] -= prob[(1 << n) - 1];
      REP(i, 1 << n) dp[i] *= prob[i];
    }
    dp = fast_mobius_transform(dp, true);
    ans += dp[0] * fixed * (__builtin_popcount(diag) & 1 ? -1 : 1);
  }
  cout << -ans + 1 << '\n';
  return 0;
}