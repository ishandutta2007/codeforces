...
void dfs(int v, const vector <vector <int> >&g, vector <int> &tin, vector <int> &tout, int &timer) {
    tin[v] = timer;
    for (int to : g[v]) {
        if (tin[to]) continue;
...
void solve(int test) {
    ...
    auto check = [&](int M, vector <int> &rev) {
        ...
        forn(i, n) {
            if (!tin[i])
                dfs(i, g, tin, tout, timer);
        }
        forn(i, m) {
            if (edges[i].cost > M) {
                if (tout[edges[i].from] < tout[edges[i].to]) return false;
                rev[i] = 0;
            } else {
                rev[i] = tout[edges[i].from] < tout[edges[i].to];
            }
        }
        return true;
    };
    ...
    check(r, rev);
    vector <int> ans;
    forn(i, m) if (rev[i]) ans.push_back(i);
    cout << r << " " << ans.size() << "\n";
    for (int x : ans) {
        cout << x + 1 << " ";
    }
