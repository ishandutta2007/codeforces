...
int q(int x)
{
	...
	for(int i=1;i<6;i++)
	{
		if(w+1+a[i]<=x)w+=a[i]+1;
		else h++,w=a[i];
	}
	return h;
}
int Q(int x){if(x<=MN)return 0;printf("? %lld\n",x);fflush(stdout);int g=read();if(g==0)MN=x;return g;}
signed main()
{
	initprog();
	int n=read();
	int l=1,r=n*2000+n,ans=inf;
	while(l<=r)
	{
		int m=l+r>>1;
		if(Q(m)==1)ans=m,r=m-1;
		else l=m+1;
	}
	ans-=n-1;
	int sum=ans+1;
	...
	for(int j=2;j<=n;j++)
	{
		...
		int l=r;
		// oldp(l,r);
		int p=(signed)inf,tmp;
		while(l<=r)
		{
			int m=l+r>>1;
			// printf("Q(%d) = %d\n",m,Q(m));
			if((tmp=Q(m))<=j){if(tmp)p=tmp,r=m-1,mne(ans,j*m);else l=m+1;}
			else l=m+1;
		}
		// odp(j,p);
		// mne(ans,j*p);
