...
typedef vector<vll> DV;
// [kaip toli yra artimiausias neuzdengtas baltas][kaip toli galima uzdengti]

DV dfs(int d, int p = -1) {
	DV ret(k + 2, vll(k + 2, 0));
			...
			for (int j = 0; j < k + 1; j++) {
				for (int ii = 0; ii < k + 1; ii++) {
					for (int jj = 0; jj < k + 1; jj++) {
						...
						int jjj = max(j, jj - 1);
						// debug(i, j, ii, jj, iii, jjj);
						ret[iii][jjj] = (ret[iii][jjj] + x[i][j] * y[ii][jj]) % mod;
					}
				}
			}
		}
