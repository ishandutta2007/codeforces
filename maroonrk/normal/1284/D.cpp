...
void check(int n,vc<pair<pi,pi>> z){
	vi vs;
	rep(i,n){
		vs.pb(z[i].b.a);
		vs.pb(++z[i].b.b);
	...
	mkuni(vs);
	rep(i,n){
		z[i].b.a=lwb(vs,z[i].b.a);
		z[i].b.b=lwb(vs,z[i].b.b);
	}
	...
	rep(_,2){
		...
		segbeats<N> seg(vi(vs.size(),0));
		
		for(auto q:qs){
			int i=q.b;
			if(q.a%2==0){
				if(seg.get(z[i].b.a,z[i].b.b,&N::getm,[](int x,int y){return max(x,y);},int(0))){
					dmp(z[i]);
					no();
				}
			}else{
				seg.ch(z[i].b.a,z[i].b.b,&N::add,1);
			}
		}
