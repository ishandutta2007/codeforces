...
int main() {
    ...
    cin >> n >> m;
    int c;
    vector<vector<ll>> x(100000), y(100000);
    rep(i, n) rep(j, m) cin >> c, x[--c].pb(i), y[c].pb(j);
    ll ans = 0;
    rep(c, 100000) {
        if (!x[c].empty()) {
            sort(all(x[c]));
            ll sum = x[c][0];
            REP(i, 1, siz(x[c])) ans += x[c][i] * i - sum, sum += x[c][i];
        }
        if (!y[c].empty()) {
            sort(all(y[c]));
            ll sum = y[c][0];
            REP(i, 1, siz(y[c])) ans += y[c][i] * i - sum, sum += y[c][i];
        }
    }
    cout << ans << endl;
}