...
int calc(vi p, vi q) {
    int res = 0;
    rep(i, si(p)) res += abs(p[q[(i + 1) % si(p)]] - q[i]);
    return res;
}
...
int greedy(vi p) {
    vi q = iota(si(p));
    int mi = inf<int>;
    do { chmin(mi, calc(p, q)); } while(next_permutation(all(q)));
    return mi;
}
...
int main() {
    TEST {
        ...
        UnionFind uf(n);
        rep(i, si(g)) rep(j, si(g[i]) - 1) uf.unite(g[i][j], g[i][j + 1]);
        Edges<int> E;
        rep(i, n) rep(j, i + 1, n) E.eb(i, j, j - i);
        SORT(E);
        Graph G(n);
        vi vis(n), used(n);
        fore(e, E) {
        ...
        set<pii> s;
        rep(i, n) {
            if(si(G[i]) == 1 and !vis[i]) {
                REC([&](auto &&f, int x, int p) -> void {
                    vis[x] = true;
                    if(si(G[x]) == 1 and x != i) { s.emplace(x, i); }
                    fore(e, G[x]) {
                        if(e == p) continue;
                        s.emplace(x, e);
                        f(e, x);
        ...
        vpi rv(n);
        rep(i, si(g)) rep(j, si(g[i])) rv[g[i][j]] = pii(i, j);

        CycleDecomposisiton q(n);

        dump(s);
        fore(l, r, s) {
            auto [i, j] = rv[l];
            int nxt = g[i][(j + 1) % si(g[i])];
            {
                q.add(nxt, r);
                dump(nxt, r);
            }
        ...
        rep(i, si(g)) rep(j, si(g[i])) {
            int l = g[i][j];
            int r = g[i][(j + 1) % si(g[i])];
            if(!used[l]) {
                q.add(r, l);
                dump(r, l);
            }
        ...
        // assert(si(res2) == 1);
        REV(res2[0]);
        // dump(calc(a, res2[0]), greedy(a));
        // assert(calc(a, res2[0]) == greedy(a));
        fore(e, res2[0]) e++;
        OUT(res2[0]);
    }
}