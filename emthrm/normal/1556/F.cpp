...
int main() {
  ...
  vector p(n, vector(n, ModInt(0)));
  REP(i, n) FOR(j, i + 1, n) {
    p[i][j] = ModInt(a[i]) / (a[i] + a[j]);
    p[j][i] = -p[i][j] + 1;
  }
  vector<unordered_map<int, ModInt>> q(1 << n);
  FOR(b, 2, 1 << n) {
    vector<int> bit;
    REP(i, n) {
      if (b >> i & 1) bit.emplace_back(i);
    }
    int sub = (b - 1) & b;
    for (int sub = (b - 1) & b; sub > 0; sub = (sub - 1) & b) {
      vector<int> x, y;
      for (int i : bit) (sub >> i & 1 ? x : y).emplace_back(i);
      q[b][sub] = 1;
      for (int xi : x) for (int yi : y) q[b][sub] *= p[xi][yi];
    }
  }
  ModInt ans = 0;
  vector<ModInt> dp(1 << n);
  REP(winner, n) {
    FOR(i, 1 << winner, 1 << n) {
      if (i >> winner & 1) {
        dp[i] = 1;
        int wo_winner = i ^ (1 << winner);
        for (int sub = wo_winner; sub > 0; sub = (sub - 1) & wo_winner) {
          dp[i] -= dp[i ^ sub] * q[i][sub];
        }
      }
    }
    ans += dp[(1 << n) - 1];
