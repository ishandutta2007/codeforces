#line 1 "codeforces-workspace\\contest\\1641\\b\\b.cpp"
...
template <class... T> class hash<std::tuple<T...>> {
  ...
  template <class Tuple> struct tuple_hash<Tuple, size_t(-1)> {
    static uint64_t apply([[maybe_unused]] const Tuple &t) { return 0; }
  };
...
template <class _Tp, class = void> struct is_hashable : std::false_type {};

template <class _Tp>
struct is_hashable<_Tp, std::__void_t<decltype(std::declval<hash<_Tp>>()(
                            std::declval<const _Tp &>()))>> : std::true_type {};
...
#endif
#line 11 "Library\\src\\utils\\cached.hpp"
...
template <class... _Args>
using as_tuple = decltype(std::tuple_cat(
    std::declval<std::tuple<std::conditional_t<
        std::is_convertible<std::decay_t<_Args>, _Args>::value,
        std::decay_t<_Args>, _Args>>>()...));

template <class _Tp, class _V> struct hash_map : unordered_map<_Tp, _V> {
  template <class... _Args>
  decltype(auto) lower_bound(_Args &&...__args) noexcept {
    return unordered_map<_Tp, _V>::find(std::forward<_Args>(__args)...);
  }
};

template <class _Tp> struct hash_set : unordered_set<_Tp> {
  template <class... _Args>
  decltype(auto) lower_bound(_Args &&...__args) noexcept {
    return unordered_set<_Tp>::find(std::forward<_Args>(__args)...);
  }
};

// Associative array.
template <class _Value, class... _Keys>
struct assoc : std::integral_constant<int, !std::is_void<_Value>::value>,
               std::conditional_t<
                   std::is_void<_Value>::value,
                   std::conditional_t<is_hashable<as_tuple<_Keys...>>::value,
                                      hash_set<as_tuple<_Keys...>>,
                                      std::set<as_tuple<_Keys...>>>,
                   std::conditional_t<is_hashable<as_tuple<_Keys...>>::value,
                                      hash_map<as_tuple<_Keys...>, _Value>,
                                      std::map<as_tuple<_Keys...>, _Value>>> {};
...
#line 13 "codeforces-workspace\\contest\\1641\\b\\b.cpp"
...
void main() {
  ...
  vector<pair<i32, i32>> ops, tmp;
  vector<i32> lens;

  for (auto left = a.begin(); left != a.end();) {
    auto right = find(next(left), end(a), *left);
    if (right == a.end()) eject(-1);

    auto p = right - begin(a) + 1;
    tmp.clear();
    for (auto i : range(next(left), right)) {
      tmp.emplace_back(p++, *i);
    }

    lens.emplace_back((right - left) * 2);

    for (auto [i, c] : tmp) {
      ops.emplace_back(i, c);
      a.insert(a.begin() + i, 2, c);
    }
