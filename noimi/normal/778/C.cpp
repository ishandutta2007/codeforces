...
main() {
    INT(n);
    auto g = getWeightedTree<char>(n);
    rep(i, 1000100) rep(j, 26) G[i][j] = -1;
    vi ans(n), sub(n), dep(n);
    {
        auto dfs = [&](auto &&f, int x, int p, int d) -> int {
            sub[x] = 1;
            dep[x] = d;
            for(auto &e : g[x]) {
                if(e == p) continue;
            ...
            for(auto &e : g[x]) {
                if(e == p) continue;
                if(sub[e] > sub[g[x].front()]) swap(g[x][0], e);
            }
            return sub[x];
        };
        dfs(dfs, 0, -1, 0);
    }
    {
        int cnt = 0, sz = n;
        queue<pii> proc;
        auto find = [&](auto &&f, int x, int p, int now) -> void {
            for(auto e : g[x]) {
                if(e == p) continue;
                if(G[now][e.cost] != -1) {
                    cnt++;
                    f(f, e, x, G[now][e.cost]);
                } else {
                    G[now][e.cost] = sz++;
                    proc.emplace(now, e.cost);
                    f(f, e, x, sz - 1);
                }
            }
        };
    ...
    int ma = -1, mem = 0;
    rep(i, n) {
        if(chmax(ma, ans[i])) mem = i + 1;
    }
    cout << n - ma << endl << mem << endl;
}