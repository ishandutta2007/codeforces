...
void solve(){
    ...
    array d={0,0,0};
    each(x,a){
        if(x==0)d={max({d[0],d[1],d[2]}),0,0};
        elif(x==1)d={max({d[0],d[1],d[2]}),max({d[0],d[2]})+1,0};
        elif(x==2)d={max({d[0],d[1],d[2]}),0,max({d[0],d[1]})+1};
        else d={max({d[0],d[1],d[2]}),max({d[0],d[2]})+1,max({d[0],d[1]})+1};
    }
    out(n-max({d[0],d[1],d[2]}));
}
int main(){
    const ll t=1;
    rep(t)solve();
