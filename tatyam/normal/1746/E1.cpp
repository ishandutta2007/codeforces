...
pair<vector<ll>,vector<ll>>split(vector<ll>a){
    const ll siz=a.size()/2;
    vector b(a.begin()+siz,a.end());
    a.resize(siz);
    return{a,b};
}
...
int main(){
    LL(n);
    vector<ll>a;
    rep(n)a.push_back(i+1);
    if(n<=2){
        answer(1);
        answer(2);
    }
    auto[yes,no]=split(a);
    if(query(no))swap(yes,no);
    while(yes.size() + no.size()>=3){
        if(no.size() && no.size() * 4 >= yes.size() && no.size() * 2 <= yes.size()){
            if(query(yes))no.clear();
            else swap(yes,no);
            continue;
        }
        auto[yes1,yes2]=split(yes);
        auto[no2,no1]=split(no);
        if(query(yes1+no1)){
            yes=yes1+no1;
            no=yes2;
        }
