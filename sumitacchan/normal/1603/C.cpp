    ...
    REP(_, t){
        ...
        IREP(i, n){
            ...
            for(int j = 1; j * j <= a[i]; j++){
                d1.pb(j); d2.pb((a[i] + j - 1) / j);
                if((a[i] + j) / (j + 1) == j + 1) d1.pb(j + 1);
            }
            IREP(j, SZ(d2)) if(d1.back() != d2[j]) d1.pb(d2[j]);
            l.resize(0); r.resize(0); num.resize(0);
            for(int j: d1){
                l.pb(a[i] / j);
                r.pb((a[i] + j - 1) / j);
                num.pb(j - 1);
            }
            Reverse(l); Reverse(r); Reverse(num);
