...
i64 calc(int x,int y) {
    ...
    ret+=(i64)rx*qy*m;
    ret+=(i64)ry*qx*n;
    bool pari=(popcnt(qx)+popcnt(qy))&1;
    ret+=pari?(i64)rx*ry-t[rx][ry]:t[rx][ry];
    return ret;
}
...
signed main() {
    ...
    for(int i=0; i<n; ++i) {
        for(int j=0; j<m; ++j) {
            t[i+1][j+1]=s[i][j]-'0';
            t[i+n+1][j+m+1]=t[1+i][1+j];
            t[i+n+1][j+1]=t[i+1][1+j+m]=1-t[1+i][1+j];
        }
    }
    ...
    rep(iii,qry) {
        ...
        cin>>x1>>y1>>x2>>y2;
        x1--,y1--,x2--,y2--;
        ans=calc(x2+1,y2+1)-calc(x1,y2+1)-calc(x2+1,y1)+calc(x1,y1);
        cout<<ans<<"\n";
    }
