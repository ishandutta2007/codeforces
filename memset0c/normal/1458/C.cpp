...
int T,n,m,res[3],pos[3],tag[3],a[N][N],f[N*N][3];
char s[M];
...
int main(){
  ...
  while(T--){
    ...
    // log("!! tag %d %d %d\n",tag[0],tag[1],tag[2]);
    for(int i=1;i<=n*n;i++){
      res[0]=f[i][pos[0]]+tag[pos[0]];
      res[1]=f[i][pos[1]]+tag[pos[1]];
      res[2]=f[i][pos[2]]+tag[pos[2]];
      // log("%d >> %d %d %d\n",i,res[0],res[1],res[2]);
      a[norm(res[0])][norm(res[1])]=norm(res[2]);
    }
    for(int i=1;i<=n;i++){
      for(int j=1;j<=n;j++){
        print(a[i][j]," \n"[j==n]);
