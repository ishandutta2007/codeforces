...
void solve() {
    ...
    freq.clear(); f.clear();
    for (int i : a) freq[i]++;
    for (pi p : freq) f.pb(p.S);

    m = f.size();
    p.rsz(m+1, 0);
    F0R(i, m) p[i+1] = p[i]+f[i];
    ...
    F0R(i, lg) {
        int r = 0, goal = 1<<i;
        F0R(j, m) {
            while (r<m && p[r+1]-p[j]<=goal) r++;
            nxt[i][j] = r;
            // debug(j, r, goal, p[j], p[r]);
        }
    }
    // debug(p);
    int ans = req(n)+2;
    F0R(i, lg) F0R(j, m) {
        int k = nxt[i][j];
        // debug(j, k);
        ans = min(ans, req(p[j]) + req(p[k]-p[j]) + req(n-p[k]));
    }
    cout << ans << nl;
}
