...
#endif
#line 2 "Contests/Codeforces_1610/D/main.cpp"
using namespace std;
...
#line 16 "Contests/Codeforces_1610/D/main.cpp"
using namespace atcoder;
using mint = modint1000000007;

#define int long long
/*
bool creates(vector<int> hoge, int curValue)
{
  if (hoge.size() == 0)
  {
    return curValue == 0;
  ...
  {
    int next = curValue + backs * (backs - 1) / 2LL + backs * a;
    if (creates(hoge, next))
    {
      return true;
    }
  }
...
void solve()
{
  ...
  ans -= hoge;
  for (int border = 1; border <= 30; border++)
  {
    int cnt_go = 0;
    int cnt_choice = 0;
    REP(i, n)
    {
      if (inputs[i] & (1 << border) and inputs[i] % 2 == 0)
      {
        cnt_go++;
        inputs[i] = 1;
      }
