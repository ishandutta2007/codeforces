...
void solve(){
    ...
    ll min=LINF;
    vector<string>ans;
    auto diff=[&](vector<string>t){
        ll cnt=0;
        rep(n)rep(j,n)cnt+=s[i][j]!=t[i][j];
        if(chmin(min,cnt))ans=t;
    };
    rep(3){
        auto t=s;
        rep(x,n)rep(y,n){
            if((x+y)%3==i)if(t[x][y]!='.')t[x][y]='O';
            if((x+y)%3==(i+1)%3)if(t[x][y]!='.')t[x][y]='X';
        }
        diff(t);
        t=s;
        rep(x,n)rep(y,n){
            if((x+y)%3==i)if(t[x][y]!='.')t[x][y]='X';
            if((x+y)%3==(i+1)%3)if(t[x][y]!='.')t[x][y]='O';
        }
        diff(t);
    }
    each(i,ans)out(i);
