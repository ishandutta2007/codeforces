// Book (https://codeforces.com/contest/1572/problem/A)
...
// end "cp-lib/io.hpp"
// begin "cp-lib/graph/topological-sort.hpp"
// begin "_detail.hpp"
struct Identity {
...
// end "cp-lib/graph/topological-sort.hpp"
...
int main() {
    ...
    while (t--) {
        ...
        rep(v, n) {
            ...
            rep(_, k) {
                int v2; read(v2); --v2;
                adj[v2].emplace_back(v, 0);
            }
        }

        auto topo = topological_sort(Graph(adj, [&](auto& p) { return p.first; }));
        if (!sz(topo)) {
            println(-1);
            continue;
        }
