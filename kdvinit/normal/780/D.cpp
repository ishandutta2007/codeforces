...
void solve()
{
    ...
    {
        string s, t; cin>>s>>t;
        name[i][0] = s.substr(0, 3);
        name[i][1] = s.substr(0, 2);
        name[i][1] += t[0];
    }

    //for(int i=1; i<=n; i++) cout<<name[i][0]<<" "<<name[i][1]<<endl;
    ...
    for(int i=1; i<=n; i++)
    {
        ...
        if(SCC_Topo_Index[x]>SCC_Topo_Index[y]) ans[i]=name[i][0];
        else ans[i]=name[i][1];
    }
