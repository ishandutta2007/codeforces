...
int main()
{
	...
	for(int i = 0; i <= n; i++)
	{
		x[i] = (a[i][1]/p) - ((a[i][0] - 1)/p);
		x[i] = a[i][1] - a[i][0] + 1 - x[i];
		range[i] = a[i][1] - a[i][0] + 1;
	}
	
	double exp = 0;
	for(int i = 0; i < n; i++)
	{
		exp = 2000 - 2000*double(x[i])/double(range[i])*double(x[i+1])/double(range[i+1]);
		ans += exp;
	}
