...
vvi conv(const vvi& a, const vvi &b) {
  int n = sz(a);
  V<fps> s(n, fps(n));
  V<fps> t(n, fps(n));
  rep(i, n) rep(j, n) s[i][j] = a[i][j], t[i][j] = b[i][j];

  rep(i, n) s[i].ntt(), t[i].ntt();
  rep(i, n) rep(j, i) swap(s[i][j], s[j][i]), swap(t[i][j], t[j][i]);
  rep(i, n) s[i].ntt(), t[i].ntt();
  ...
  rep(i, n) s[i].intt();
  //mint in = mint{n}.inverse();
  //rep(i, n) rep(j, n) s[i][j] *= in;
  ...
  vvi u(n, vi(n));
  rep(i, n) rep(j, n) u[i][j] = s[i][j].get() & 1;
  return u;
}
...
void Nyaan::solve() {
  ...
  rep(t,60){
    vvi a(n,vi(n));
    rep(i,n)rep(j,n)a[i][j]=gbit(A[i][j],t);
    vvi b=conv(a,invf);
    if(t<3)trc(t,a,b);
    rep(i,n)rep(j,n)res[i][j]|=b[i][j];
  }
  ll ans=0;
  rep(i,n)rep(j,n)ans+=!!res[i][j];
  out(ans);
}