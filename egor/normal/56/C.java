...
public class TaskC {
	...
	void solve() {
		out.println(readEmployees(new MultiSet<String>()));
	}

	private int readEmployees(MultiSet<String> superiors) {
		StringBuilder name = new StringBuilder();
		char ch;
		while ((ch = in.readCharacter()) != '.' && ch != ':')
			name.append(ch);
		int result = superiors.get(name.toString());
		if (ch == '.')
			return result;
		superiors.add(name.toString());
		do
			result += readEmployees(superiors);
		while (in.readCharacter() == ',');
		superiors.remove(name.toString());
		return result;
	}
