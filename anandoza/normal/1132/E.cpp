...
int main() {
    ...
    ll reserved = 0;
    for (int i = 0; i < Z; i++) {
        reserved += min(c[i], (ll) Z - 1) * v[i];
    }
    ll greedy = 0;
    vll used(Z);
    for (int i = 0; i < Z; i++) {
        ll limit = max(c[i] - (Z - 1), 0LL);

        if (greedy + limit * v[i] <= W - reserved) {
            greedy += limit * v[i];
            used[i] += limit;
        } else {
            ll amt = max(0LL, ((W - reserved - greedy) + v[i] - 1) / v[i]);
            greedy += amt * v[i];
            used[i] += amt;
        }

        /* for (int j = 0; j < c[i] - (Z - 1); j++) { */
        /*     if (greedy >= W - reserved) */
        /*         goto DONE; */
        /*     greedy += v[i]; */
        /*     used[i]++; */
        /* } */
...
DONE:;
    ...
    while (true) {
        inc(0);
        int r = 0;
        while (used[r] + extra[r] == c[r] + 1 || extra[r] == Z + 1) {
            if (r == Z - 1)
                goto DONE2;
            inc(r + 1);
            zero(r);
