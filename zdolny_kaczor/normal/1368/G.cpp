...
int blocked[2 * nax], cou[2 * nax];
void update(int wh, int lb, int rb) {
	if (blocked[wh]) cou[wh] = rb - lb;
	else if (wh < nax) cou[wh] = cou[wh * 2] + cou[wh * 2 + 1];
	else cou[wh] = 0;
}
void put(int l, int r, int val, int wh = 1, int lb = 0, int rb = nax) {
	if (l >= rb || r <= lb) return;
	if (l <= lb && r >= rb) {
		blocked[wh] += val;
		update(wh, lb, rb);
	...
	int sb = (lb + rb) / 2;
	put(l, r, val, wh * 2, lb, sb);
	put(l, r, val, wh * 2 + 1, sb, rb);
	update(wh, lb, rb);
}
int main() {
	scanf("%d%d", &n, &m);
	REP(i, n) board[i].resize(m + 1);
	REP(i, n) scanf("%s", board[i].c_str());
	...
	REP(i, n) REP(j, m) {
		...
		if (ii >= 0 && ii < n && jj >= 0 && jj < m)
			to[num(i, j)] = num(ii, jj);
	}
	memset(l, -1, sizeof(l));
	memset(r, -1, sizeof(r));
	int tot = n * m;
	REP(i, tot) if (to[i] != -1) {
		down[to[i]].push_back(i);
		debug(arr(to, i));
	}
	REP(i, tot) {
		if (l[i] == -1) {
	...
	REP(i, tot) debug(arr(l, i), arr(r, i));
	vector <pair <pii, pii> > incs;
	REP(i, n) REP(j, m) {
		auto add = [&](int x, int y) {
			if ((i + j) & 1) swap(x, y);
			debug(arr(l, x), arr(r, x), arr(l, y), arr(r, y));
			incs.push_back({{l[x], 1}, {l[y], r[y]}});
			incs.push_back({{r[x], -1}, {l[y], r[y]}});
		};
		if (board[i][j] == 'U') add(num(i, j), num(i + 1, j));
		if (board[i][j] == 'L') add(num(i, j), num(i, j + 1));
	}
	sort(ALL(incs));
	ll ans = 0;
	int prev = 0;
