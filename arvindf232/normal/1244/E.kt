...
fun main(){
    solve.cases{
        ...
        val pos = BinarySearchLastTrue(0,n-1){phrase ->
            val right = phrase/2
            val left = phrase - right
            val leftcost = 1L * L[left] * (left +1) - Q.sumQuery(0,left)
            val rightcost = Q.sumQuery(n-1-right,n-1) - 1L * L[n-1-right] * (right+1)
            (leftcost +rightcost) <= k
        }!!
        if(pos == n-1){
            put(0)
            return@cases
        ...
        val leftcost = 1L * L[left] * (left +1) - Q.sumQuery(0,left)
        val rightcost = Q.sumQuery(n-1-right,n-1) - 1L * L[n-1-right] * (right+1)
        val remainingcost = k - (leftcost + rightcost)
        val movecost = minOf(left, right)+1
        val extramoves = remainingcost / movecost
        var currentDif = L[n-1-right] - L[left]
        put(currentDif - extramoves)
    }
    done()
}
const val singleCase = true

/* always bigger - smaller */