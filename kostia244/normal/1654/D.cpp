// Problem:  
// Contest: Codeforces
// URL: https://m1.codeforces.com/contest/1654/problem/D
// Memory Limit: 256 MB
// Time Limit: 3000 ms
// 
...
void add(int v, int d) {
	while(v > 1) {
		pw[lp[v]] += d;
		mn[lp[v]] = min(mn[lp[v]], pw[lp[v]]);
		v /= lp[v];
	}
}
void dfs(int v, int p, mint cur, mint &ans) {
	ans += cur;
	for(auto [i, x, y] : g[v]) if(i != p) {
		add(y, 1), add(x, -1);
		cur *= y;
		cur /= x;
		dfs(i, v, cur, ans);
...
int main() {
	...
	multitest([&](){
		...
		fill(pw, pw+n+3, 0);
		fill(mn, mn+n+3, 0);
		g = vector<vector<array<int,3>>>(n+1);
