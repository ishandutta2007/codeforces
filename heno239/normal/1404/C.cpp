...
public:
	...
	vector<int> allzeros() {
		...
		int k = 0, l = 0, r = n;
		eval(k, l, r);
		vector<ste> v; v.push_back({ 0,0,n });
		while (!v.empty()) {
			ste s = v.back(); v.pop_back();
			if (s.l + 1 == s.r) {
				if (node[s.k] <= 0) {
					res.push_back(s.k - (n - 1));
				}
				continue;
			}
			else {
				...
				eval(s.k * 2 + 1, s.l, m);
				if (node[s.k * 2 + 1] <= 0) {
					v.push_back({ s.k * 2 + 1,s.l,m });
				}
				eval(s.k * 2 + 2, m, s.r);
				if (node[s.k * 2 + 2] <= 0) {
					v.push_back({ s.k * 2 + 2,m,s.r });
				}
			}
		}
		return res;
