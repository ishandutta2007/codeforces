...
int l,m,n,t,C;
int T[M];
void modi(int x,int k){++x;for(int i=x;i<M;i+=i&-i)T[i]+=k;}
int que(int x){++x;if(x<=0)return 0;int ans=0;for(int i=x;i;i-=i&-i)ans+=T[i];return ans;}
int main(){
#ifdef LOCAL
