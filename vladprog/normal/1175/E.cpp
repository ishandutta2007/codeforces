...
const int N=2e5+100,X=5e5+100,L=20;
//const int INF=;
...
signed main()
{
    ...
    for(int i=1;i<=n;i++)
        cin>>a[i].x>>a[i].y;
    sort(a+1,a+n+1);
    multiset<int,greater<int>> s_r;
    multiset<pii> s_lr;
    for(int i=0,pos=1;i<=X;i++)
    {
        while(pos<=n&&a[pos].x==i)
            s_lr.insert(a[pos]),
            s_r.insert(a[pos].y),
            pos++;
        rig[i][0]=(s_r.empty()?i:*s_r.begin());
        while(!s_lr.empty()&&s_lr.begin()->y==i)
            s_r.erase(s_r.find(i)),
            s_lr.erase(s_lr.begin());
    }
    for(int j=0;j<L;j++)
        for(int i=0;i<=X;i++)
            rig[i][j+1]=rig[rig[i][j]][j];
//    cout<<" \t  ";
//    for(int i=1;i<=10;i++)
//        cout<<i<<"\t";
//    cout<<"\n";
