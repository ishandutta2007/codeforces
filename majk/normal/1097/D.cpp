...
void solve(istream& cin, ostream& cout) {
    ll N, K; cin >> N >> K;
    vector<pair<FieldMod,int>> F;
    for (ll i = 2; i < 4e7 && N != 1; ++i) {
        if (N%i==0) {
            int c = 0;
            while (N%i==0) { N/=i; c++; }
            F.push_back({FieldMod{i}, c});
        }
    }
    if (N != 1) F.push_back({FieldMod{N%1000000007},1});

    vector<FieldMod> INV(70);
    for (int i = 1; i < 70; ++i) INV[i] = FieldMod{1} / i;

    FieldMod ans = 1;
    for (auto &f:F) {
        vector<FieldMod> Z(f.y+2, 0);
        Z[f.y] = 1;
        for (int k = 0; k < K; ++k) {
            for (int i = f.y; i >= 0; --i) {
                Z[i] = (Z[i] * INV[i+1]) + Z[i+1];
            }
        }
