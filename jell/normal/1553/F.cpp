#line 1 "codeforces-workspace\\contest\\1553\\f\\f.cpp"
...
#line 13 "codeforces-workspace\\contest\\1553\\f\\f.cpp"
// #include "lib/cxx20"
...
#line 16 "codeforces-workspace\\contest\\1553\\f\\f.cpp"
//*/
...
#line 1 "ac-library\\atcoder\\fenwicktree.hpp"



#line 6 "ac-library\\atcoder\\fenwicktree.hpp"
...
#line 8 "ac-library\\atcoder\\fenwicktree.hpp"
...
#line 34 "codeforces-workspace\\contest\\1553\\f\\f.cpp"
...
void main() {
  ...
  {
    // segment_tree<i64> s(max_val);
    atcoder::fenwick_tree<int> s(max_val);
    i64 sum = 0, now = 0, i = 0;

    for (auto x : a) {
      i64 cont = 0;
      for (auto m = x; m < max_val; m += x) {
        // cont += s.fold(m, s.size());
        cont += s.sum(m, max_val);
      }
      // s[x] += 1;
      s.add(x, 1);
      now += sum - cont * x;
  ...
  {
    // segment_tree<i64> s(max_val);
    atcoder::fenwick_tree<i64> s(max_val);
    i64 i = 0, now = 0;

    for (auto x : a) {
      now += x * i + s.sum(0, x + 1);
      for (auto m = x; m < max_val; m += x) s.add(m, -x);

      // for (i64 l = x, r, y; l > 0;) {
      //   y = x / l;
      //   r = l;
      //   l = x / (y + 1);
      //   // now -= s.fold(l + 1, r + 1) * y;
      //   now -= s.sum(l + 1, r + 1) * y;
      // }
      // s[x] += x;
      // s.add(x, x);
