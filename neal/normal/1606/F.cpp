...
int main() {
    ...
    for (int x : lca.rev_tour_list)
        if (x > 0)
            dp[lca.parent[x]] += max(dp[x], 1);

    set<int> alive;

    for (int i = 0; i < N; i++)
        if (dp[i] > 1)
            alive.insert(lca.tour_start[i]);
    ...
    auto increment_k = [&]() -> void {
        k++;
        vector<int> became_one;

        for (auto it = alive.rbegin(); it != alive.rend(); it++) {
            int index = *it;
            int node = lca.tour_list[index];

            if (dp[node] - k <= 1) {
                became_one.push_back(node);
                int adjust = 1 - (dp[node] - k);
                drop[node] -= adjust;
            }

            if (node > 0) {
                drop[lca.parent[node]] += 1 + drop[node];
                dp[lca.parent[node]] -= 1 + drop[node];
            }
        }
        ...
        for (int x : became_one)
            alive.erase(lca.tour_start[x]);
    };
