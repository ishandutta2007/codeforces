...
void run_case() {
    ...
    for (int x : lca.rev_tour_list) {
        vector<int> children = lca.adj[x];
        assert(children.size() <= 2);

        if (children.size() == 1)
            dp[x] = lca.subtree_size[children[0]] - 1;
        else if (children.size() == 2)
            dp[x] = max(dp[children[0]] + lca.subtree_size[children[1]] - 1, lca.subtree_size[children[0]] - 1 + dp[children[1]]);
    }
