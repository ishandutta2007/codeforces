...
int main() {
	...
	//dbg(basis,mul);
	vmi ans(m+1);
	if (sz(basis) <= 20) { // sz(basis) <= 20
		F0R(i,1<<sz(basis)) {
			ll res = 0;
			F0R(j,sz(basis)) if (i&(1<<j)) res ^= basis[j];
			ans[pct(res)] += 1;
		}
	} else {
		vi bits; F0R(i,m) if (!active[i]) bits.pb(i);
		// dbg(bits);
		vector<vmi> dp(1<<sz(bits));
		dp[0] = {1};
		trav(t,basis) {
			int cur = 0;
			trav(b,bits) {
		...
		//dbg(dp);
		F0R(i,sz(dp)) F0R(j,sz(dp[i])) ans[pct(i)+j] += dp[i][j];
	}
	trav(t,ans) pr(t*mul,' ');
	ps();
