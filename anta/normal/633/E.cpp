...
int main() {
	...
	while(~scanf("%d%d", &n, &k)) {
		...
		for(int i = 0; i < n; ++ i)
			scanf("%d", &c[i]);
		DirectRMQ<int, greater<int>> rMq_v;
		DirectRMQ<int, less<int>> rmq_r;
		rMq_v.build(&v[0], n);
		rmq_r.build(&c[0], n);
		vector<int> values(n);
		rep(i, n) {
			...
			while(u - l > 0) {
				int mid = (l + u) / 2;
				int x = rMq_v.queryVal(&v[0], i, mid) * 100;
				int y = rmq_r.queryVal(&c[0], i, mid);
				if(x >= y)
					u = mid;
				else
					l = mid + 1;
			}
			rer(j, l - 1, l + 1) if(i <= j && j < n) {
				int x = rMq_v.queryVal(&v[0], i, j) * 100;
				int y = rmq_r.queryVal(&c[0], i, j);
				amax(values[i], min(x, y));
			}
		}
		sort(all(values));
		vector<long double> logfact(n + 1);
		logfact[0] = 0;
		rep(i, n)
			logfact[i + 1] = logfact[i] + log(i + 1.L);
		long double ans = 0;
		rer(i, 0, n - k) {
			//C(a, k - 1) / C(n, k)
			//a!k!(n-k)!/((k-1)!(a-k+1)!n!)
			long double v[6] = {
				logfact[n - i - 1], - logfact[k - 1], - logfact[n - i - k],
				- logfact[n], logfact[k], logfact[n - k]
			};
			sort(v, v + 6, [](long double x, long double y) { return abs(x) < abs(y); });
			long double prob = exp(accumulate(v, v + 6, 0.L));
			ans += prob * values[i];
		}
		printf("%.10f\n", (double)ans);
	}
