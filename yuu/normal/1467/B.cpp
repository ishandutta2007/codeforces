...
int n;
int a[300001];
bool good(int a, int b, int c) { return ((b > a) && (b > c)) || ((b < a) && (b < c)); }
int good_pos(int i) {
    if ((i <= 1) || (i >= n)) return 0;
    return good(a[i - 1], a[i], a[i + 1]);
}
int test_good(int i) {
    int old = a[i];
    int res = good_pos(i) + good_pos(i - 1) + good_pos(i + 1);
    int orginal = res;
    a[i] = a[i - 1];
    res = min(res, good_pos(i) + good_pos(i - 1) + good_pos(i + 1));
    a[i] = a[i + 1];
    res = min(res, good_pos(i) + good_pos(i - 1) + good_pos(i + 1));
    a[i] = 1e9 + 1;
    res = min(res, good_pos(i) + good_pos(i - 1) + good_pos(i + 1));
...
void solve() {
    ...
    int ans = 0;
    for (int i = 1; i <= n; i++) ans += good_pos(i);
    int minus = 0;
    for (int i = 1; i <= n; i++) minus = max(minus, test_good(i));
    ans -= minus;
    cout << ans << '\n';
}
