...
void solve() {
  ...
  auto f = [&](ll n) -> ll {
    ll res = 0;
    FOR(a, 1, n + 1) FOR(b, 1, n + 1) if (a + b <= n) res += a + b;
    return res;
  };
  FOR(n, 10) print(n, f(n), (n - 1) * n * (n + 1) / 3);
  */
  ...
  multiplier_mobius(dp0);
  multiplier_mobius(dp1);

  mint ANS = 0;
  FOR(d, 1, N + 1) { ANS += (mint(N) * dp0[d] - dp1[d]) * mint(d / gcd(N, d)); }
  print(ANS);
}
