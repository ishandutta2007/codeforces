...
int main() {
    ...
    segment_tree<int, decltype(bigger)> v2(N + 1, IMPOSSIBLE, bigger);
    ...
    F0R (i, N) {
        ...
        {
            // v2 already a[i]
            ckmax(v2_cost, v2[a[i]] + adjust);

            // v2 not a[i]
            ckmax(v2_cost, 1 + adjust + max(v2.accumulate(0, a[i]), v2.accumulate(a[i] + 1, N + 1)));
        }
