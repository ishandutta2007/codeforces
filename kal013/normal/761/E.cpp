...
inline ll mid_point(ll l,ll r){
	return l + ((r - l) >> 1);
}

void dfs(int n,int p,ll x0,ll y0,ll lx, ll rx, ll ly,ll ry,int block = -1){
	...
	for(auto e: adj[n]){
		...
		ll clx = lx, cly = ly, crx = rx, cry = ry, cx0 = x0, cy0 = y0;

		if (dx[iter] == 1){
			clx = mid_point(x0,crx);
			cx0 = mid_point(clx,crx);
			rx = clx - 1;
		}
		else if (dx[iter] == -1){
			crx = mid_point(clx , x0);
			cx0 = mid_point(clx , crx);
			lx = crx + 1;
		}
		else if (dy[iter] == 1){
			cly = mid_point(y0,cry);
			cy0 = mid_point(cly,cry);
			ry = cly - 1;
		}
		else {
			assert(dy[iter] == -1);

			cry = mid_point(cly , y0);
			cy0 = mid_point(cly,cry);
			ly = cry + 1;
		}
		dfs(e,n,cx0,cy0,clx,crx,cly,cry, 3 - iter);
		++iter;
	}
}
