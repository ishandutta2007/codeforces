#line 1 "codeforces-workspace\\contest\\543\\d\\d.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\543\\d\\d.cpp"
...
#line 21 "codeforces-workspace\\contest\\543\\d\\d.cpp"
...
void main() {
  ...
  vector<mint> dp(n);
  fixed_point([&](auto dfs, int now, int pre) -> void {
    dp[now] = 1;
  ...
  vector<mint> ans(n);
  fixed_point([&](auto dfs, int now, mint anc) -> void {
    ans[now] = dp[now] * (1 + anc);
    anc += 1;
    vector<mint> lprod{1};
    for (auto &&ch : tree[now]) {
      lprod.emplace_back(lprod.back() * (1 + dp[ch]));
    }
    vector<mint> rprod{1};
    for (auto &&ch : reversed(tree[now])) {
      rprod.emplace_back(rprod.back() * (1 + dp[ch]));
    }
    reverse(begin(rprod), end(rprod));
    for (auto i = 0; i != tree[now].size(); ++i) {
      auto nanc = anc * lprod[i] * rprod[i + 1];
      dfs(tree[now][i], nanc);
    }
  })(0, 0);
  cout << ans << eol;
