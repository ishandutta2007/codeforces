...
main() {
    ...
    ll ans = 1e17;
    vll rui(n + 1);
    rui[n - 1] = r1 * a.back() + r3;
    rep3(i, n - 2, 0) { rui[i] = rui[i + 1] + min(r2 + r1, r1 * (a[i] + 2)) + d; }
    rep(i, n) {
        chmin(ans, dp[i][0] + rui[i]);
        chmin(dp[i + 1][0], dp[i][0] + r1 * a[i] + r3);
        chmin(dp[i + 1][1], dp[i][0] + min(r2 + r1, r1 * (a[i] + 2)) + d * 2);
        chmin(dp[i + 1][0], dp[i][1] + min(r2 + r1, r1 * (a[i] + 2)));
        chmin(dp[i + 1][0], dp[i][1] + r1 * a[i] + r3);
    }
    // print(dp);
    cout << min(ans + d * (n - 1), *min_element(all(dp.back())) + d * (n - 1)) << endl;
}