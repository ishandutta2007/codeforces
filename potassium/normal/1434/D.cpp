...
S op(S l, S r) {
	S ans;
	for (int i = 0; i <= 1; i++)
		ans.maxd[i] = max(l.maxd[i], r.maxd[i]);
	return ans;
}
...
int main() {
	...
	auto bfs = [&](int u, vector<int>& dist, vector<int>& wdist) {
		dist.resize(n + 1, INT_MAX);
		wdist.resize(n + 1, INT_MAX);
		queue<int> q;
		dist[u] = 0;
		wdist[u] = 0;
		q.push(u);
		while (!q.empty()) {
			...
			for (auto& e : g[u])
				if (chkmin(dist[e.first], dist[u] + 1)) {
					wdist[e.first] = wdist[u] ^ e.second;
					q.push(e.first);
				}
		}
	};
	...
	vector<int> root(2);
	for (int i = 0, now = 1; i <= 1; i++) {
		vector<int> dist, wdist;
		bfs(now, dist, wdist);
		now = root[i] = max_element(dist.begin() + 1, dist.end()) - dist.begin();
	}

	vector<vector<int>> dist(2), wdist(2);
	vector<vector<int>> in(2, vector<int>(n + 1)), out = in;
	function<void(int, int, vector<int>&, vector<int>&, int&)> dfs = [&](int u, int p, vector<int>& in, vector<int>& out, int& timer) {
		in[u] = ++timer;
		for (auto& e : g[u])
			if (e.first != p)
				dfs(e.first, u, in, out, timer);
		out[u] = timer;
	};
	vector<vector<S>> st_base(2, vector<S>(n, e()));
	for (int i = 0; i <= 1; i++) {
		bfs(root[i], dist[i], wdist[i]);
		int timer = -1;
		dfs(root[i], -1, in[i], out[i], timer);

		for (int u = 1; u <= n; u++)
			st_base[i][in[i][u]].maxd[wdist[i][u]] = dist[i][u];
	}

	vector<SegTree<S, op, e, F, mapping, composition, id>> st(2);
	for (int i = 0; i <= 1; i++)
		st[i] = st_base[i];
	...
	while (q--) {
		int id; R(id); id--;
		for (int i = 0; i <= 1; i++) {
			int u = dist[i][edges[id].first] < dist[i][edges[id].second] ? edges[id].second : edges[id].first;
			st[i].apply(in[i][u], out[i][u] + 1, F{ 1 });
		}

		int ans = 0;
		for (int i = 0; i <= 1; i++)
			ans = max(ans, st[i].all_prod().maxd[0]);
