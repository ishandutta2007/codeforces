...
/*
  ans
  = \sum_{n=0}^N A_n(x) / n!
  = \sum_{n=0}^N (1 - x)^(n+1) (\sum_{i>=0} i^n x^i) / n!
  = (1 - x) \sum_{i>=0} x^i \sum_{n=0}^N (i (1 - x))^n / n!)
  = (1 - x) \sum_{i>=0} x^i [y^N] (e^(i (1 - x) y) / (1 - y))
  = (1 - x) [y^N] (1 / ((1 - y) (1 - x e^((1 - x) y))))
  = (1 - x)^(N+1) [z^N] (1 / ((1 - z / (1 - x)) (1 - x e^z)))
  = (1 - x)^(N+2) [z^N] (1 / ((1 - x - z) (1 - x e^z)))
  
  1 / ((1 - z - x) (1 - x e^z)) = a / (1 - x - z) + b / (1 - x e^z)
      1 = a + b (1 - z), 0 = -a x e^z - b x
      a = 1 / (1 - e^z (1 - z)), b = -e^z / (1 - e^z (1 - z))
  
  [z^N] (1 / ((1 - e^z (1 - z)) (1 - x - z)))
  = [z^N] (1 / (1 - e^z (1 - z))) \sum_{i>=0} z^i / (1 - x)^(i+1)
      1 - e^z (1 - z) = z^2/2 + O(z^3)
  
  [z^N] (-e^z / ((1 - e^z (1 - z)) (1 - x e^z)))
  = (1/N) [w^(N-1)] f'(w) (w / log(1 + w))^N
      w := e^z - 1
      z = log(1 + w)
      f(w) := g(w) (1 / (1 - x (1 + w)))
      g(w) := -(1 + w) / (1 - (1 + w) (1 - log(1 + w)))
      f'(w) = g'(w) (1 / (1 - x (1 + w))) + g(w) (x / (1 - x (1 + w))^2)
            = g'(w) \sum_{i>=0} w^i x^i / (1 - x)^(i+1) + g(w) \sum_{i>=0} (i + 1) w^i x^(i+1) / (1 - x)^(i+2)
      1 - (1 + w) (1 - log(1 + w)) = w^2/2 + O(w^3)
*/
...
int main() {
  for (; ~scanf("%d", &N); ) {
    ...
    
    {
      ...
      Poly fs(N + 3);
      for (int i = 0; i <= N + 2; ++i) {
        fs[i] = -invFac[i + 2] + invFac[i + 1];
      }
      fs = fs.inv(N + 3);
// cerr<<"270 fs = "<<270*fs<<endl;
      
      /*
        [z^(N+2)] *
        = \sum_{i=0}^{N+2} f[N + 2 - i] (1 - x)^(N+2 - (i+1))
        = f[0] / (1 - x) + \sum_{i=0}^{N+1} f[i + 1] (1 - x)^i
        = f[0] / (1 - x) + \sum_{i=0}^{N+1} f[i + 1] \sum_{j=0}^i binom(i, j) (-1)^j x^j
      */
      for (int i = 0; i <= N; ++i) {
        ans[i] += fs[0];
      }
      ...
      fs *= coef;
      for (int i = 0; i <= N; ++i) {
        ans[i] += ((i & 1) ? -1 : +1) * invFac[i] * fs[N + 1 + i];
      }
// cerr<<10206*ans<<endl;//(N=4,[x^2])
    }
    
    {
      ...
      Poly hs(N + 3);
      for (int i = 0; i <= N + 2; ++i) {
        hs[i] = ((i & 1) ? -1 : +1) * inv[i + 1];
      }
      hs = hs.pow(Mint(-N), N + 3);
      
      // w^-2 g(w) = w^-2 (-(1 + w) / (1 - (1 + w) (1 - log(1 + w))))
      Poly gs(N + 3);
      for (int i = 0; i <= N + 2; ++i) {
        gs[i] = (((i + 1) & 1) ? -1 : +1) * inv[i + 2] + ((i & 1) ? -1 : +1) * inv[i + 1];
      }
      gs = -Poly{1, 1}.div(gs, N + 3);
// cerr<<"810 gs = "<<810*gs<<endl;
      
      /*
        [w^(N+1)] *
        = \sum_{i=0}^{N+1} k[N + 1 - i] (i + 1) x^(i+1) (1 - x)^(N-i)
        = k[0] (N + 2) x^(N+2) / (1 - x) + \sum_{i=0}^N k[N + 1 - i] (i + 1) \sum_{j=i}^N binom(N-i, j-i) (-1)^(j-i) x^(j+1)
      */
      {
        Poly ks = (hs * gs).mod(N + 2);
        ...
        Poly coef(N + 1);
        for (int i = 0; i <= N; ++i) {
          coef[i] = invFac[i] * ((i & 1) ? -1 : +1);
        }
        ks *= coef;
        for (int i = 0; i <= N - 1; ++i) {
          ans[i + 1] += inv[N] * (invFac[N - i] * ks[i]);
        }
// cerr<<"! ";for(int i=0;i<=N-1;++i){cerr<<-136080*(inv[N] * (invFac[N - i] * ks[i]))<<" ";}cerr<<endl;
      }
...
// cerr<<"3 gs = "<<3*gs<<endl;
      ...
      /*
        [w^(N+2)] *
        = \sum_{i=0}^{N+2} k[N + 2 - i] x^i / (1 - x)^(N+1-i)
        = k[0] x^(N+2) / (1 - x) + \sum_{i=0}^{N+1} k[N + 2 - i] \sum_{j=i}^{N+1} binom(N+1-i, j-i) (-1)^(j-i) x^j
      */
      {
        Poly ks = (hs * gs).mod(N + 3);
// cerr<<"3 ks = "<<3*ks<<endl;
        reverse(ks.begin(), ks.end());\
        ks.pop_back();
        for (int i = 0; i <= N + 1; ++i) {
        ...
        ks *= coef;
        for (int i = 0; i <= N; ++i) {
          ans[i] += inv[N] * (invFac[N + 1 - i] * ks[i]);
        }
// cerr<<"! ";for(int i=0;i<=N;++i){cerr<<408240*(inv[N] * (invFac[N + 1 - i] * ks[i]))<<" ";}cerr<<endl;
      }
    }
