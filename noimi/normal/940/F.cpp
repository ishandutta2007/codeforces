...
constexpr int T = 2000; // ~ N ^ (2/3)

struct Change{
    int p,x;
    int pre;
    Change(int p,int x):p(p),x(x){}
};
...
struct Query{
    ...
    int changecnt;
    int queryid;
    Query(int l,int r,int changecnt,int queryid):l(l),r(r),changecnt(changecnt),queryid(queryid){}
};
...
signed main(){
    ...
    while(q--){
        ...
        if(type == 1){
            ans.pb(0);
            int num = querys.size();
            querys.eb(p-1,x,changes.size(),num);
        }
        else{
            zip.eb(x);
            changes.eb(p-1,x);
    ...
    UNIQUE(zip);
    for(auto &e:a) e = lb(zip,e);
    rep(i,changes.size()) changes[i].x = lb(zip,changes[i].x);
    sort(all(querys),[](Query x,Query y){
        if(x.changecnt/T != y.changecnt/T) return x.changecnt < y.changecnt;
    ...
    freq[0] = zip.size()+1;
    for(auto [l,r,t,id]:querys){
        while(nt < t){
            changes[nt].pre = a[changes[nt].p];
            change(changes[nt].p,changes[nt].x);
            nt++;
        }
        while(nt > t){
            nt--;
            change(changes[nt].p,changes[nt].pre);
        }
        while(nl > l){
            nl--;
            int c = cnt[a[nl]]++;
