...
ll T(ll n,ll k,ll a){//nka
  return (COM(n-(k-1)*(a-1),a)*fac[a])%mod;
}
void solve(){
  ll n,k;
  cin>>n>>k;
  ...
  ll ans2=0;
  for(ll i=1;i<n;i++){
    ll tmp=T(n,k,i)*(n-i)-T(n,k,i+1);
    tmp%=mod;
    tmp+=mod;
    tmp%=mod;
    ans+=tmp*finv[n]%mod*fac[n-i-1]%mod*(i+1);
   // cout<<tmp<<" "<<finv[n]<<" "<<fac[i-1]<<endl;
    ans2+=tmp;
    ans2%=mod;
   // cout<<T(n,k,i)<<" "<<T(n,k,i+1)<<" "<<tmp<<endl;
    ans%=mod;
  }
  cout<<ans<<endl;
}
