...
mint binom(int a, int b) {
    if (0 <= b and b <= a) return fact(a) / (fact(b) * fact(a-b));
    return 0;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        string s;
        cin >> s;
        if (s[0] == 'B' or s[1] == 'W') only_wb = 0;
        if (s[0] == 'W' or s[1] == 'B') only_bw = 0;
        sort(s.begin(), s.end());
        if (s == "??") {
            only_mixed *= 2;
            exponent += 2;
    ...
    // dbg(shift, exponent);
    mint res = -only_mixed + only_wb + only_bw;
    res += binom(exponent, -shift);
    cout << res << "\n";
}