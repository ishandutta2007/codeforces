...
ll area(ll mnx, ll mny, ll mxx, ll mxy){
	return (mxx - mnx + 1) * (mxy - mny + 1);
}

ll area2(pair<ll,ll> X1, pair<ll,ll> Y1, pair<ll,ll> X2, pair<ll,ll> Y2){
	ll vx = min(X1.second , X2.second) - max(X1.first, X2.first) + 1;
	...
	ll vy = min(Y1.second, Y2.second) - max(Y1.first, Y2.first) + 1;
...
void solve(){
	...
	ll mnx1 = max(1ll, x1 - x2 + 1);
	ll mnx2 = max(1ll, x2 - x1 + 1);

	ll mny1 = max(1ll, y1 - y2 + 1);
	ll mny2 = max(1ll, y2 - y1 + 1);


	ll mxx1 = min(n, x1 + n - x2);
	ll mxx2 = min(n, x2 + n - x1);


	ll mxy1 = min(m, y1 + m - y2);
	ll mxy2 = min(m, y2 + m - y1);


	ll tot = area(mnx1, mny1, mxx1, mxy1) + area(mnx2, mny2, mxx2, mxy2);

	tot -= area2({mnx1, mxx1}, {mny1, mxy1},{mnx2, mxx2}, {mny2, mxy2} );
