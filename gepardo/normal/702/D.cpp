/*-------------------( alex256 )--------------------\
|    Contest: Education Codeforces Round 15         |
|       Task: D                                     |
|       Date: 29. 07. 2016                          |
\--------------------------------------------------*/
// Includes
...
main()
{
    int d = in, k = in, a = in, b = in, t = in;

    int tGo = d * b;

    int gCnt = (d + k - 1) / k, fixCnt = gCnt - 1;

    int tCar = d * a + fixCnt * t;

    int tCarGo;

    if (fixCnt == 0) tCarGo = tCar;
        else tCarGo = tCar - t - a * ( (d-1) % k + 1 ) + b * ( (d-1) % k + 1 );

    int tGoCar;
    if (d <= k) tGoCar = tGo;
        else tGoCar = k * a + (d - k) * b;

    //out.putLn(tGo, ' ', tCar, ' ', tCarGo, ' ', tGoCar);

    out = min(min(tGo, tCar), min(tGoCar, tCarGo));
