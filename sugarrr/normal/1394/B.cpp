...
int main(){fastio
    ...
    rep(i,0,n-1){
        if(i==0)val[i]=P;
        else val[i]=P*val[i-1];
    }
    mint goal=0;
    for(auto x:val)goal+=x;
    vvi c(k);
    rep(i,0,k-1)c[i].resize(i+1);
    rep(i,0,n-1){
        ll sz=a[i].size();
        rep(j,0,sz-1){
            c[sz-1][j]+=val[a[i][j].se];
        }
    }
    cout<<dfs(c,0,-goal)<<endl;
