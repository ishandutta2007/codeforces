...
int main() {
	...
	cin >> n; 
	Pinit(n * 2 + 1), init(n * 2 + 1);
	int mid = (n + 1) / 2;
	poly H(n + 1), A(n + 1);
	L(i, 0, n) A[i] = ifac[i];
	L(i, mid, n) H[i] = fac[n - i - 1];
	A *= H;
	L(i, 2, n) f[n - i + 1] = A[i];
	L(i, 2, n) f[i] = (ll) f[i] * fac[n - i] % mod;
	R(i, n, 2) g[i] = (g[i + 1] + f[i + 1]) % mod;
	L(i, 2, n) f[i] = ((ll) f[i] * (i - 1) % mod + mod - g[i]) % mod;
	f[1] = fac[n - 1];
	L(i, 2, n) (f[1] += mod - f[i]) %= mod;
	L(i, 1, n) cout << f[i] << ' ';
	cout << '\n';
	return 0;
}
