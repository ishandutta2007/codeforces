...
int main() {
    ...
    while(TT--) {
        ...
        else if(dif.size() == 2 && dif[0] == dif[1] - 1) {
            cout << min(2 * y,x) << '\n';
        }
        else if(x >= y) {
            cout << 1LL * y * dif.size() / 2 << '\n';
        }
        else {
            int m = dif.size();
            vvl dp(m+1,(vl(m+1,1e18)));
            rep(i,m+1) dp[i][i] = 0;
            rep(wid,2,m+1,2) {
                rep(l,m-wid+1) {
                    int r = l + wid;
                    chmin(dp[l][r],dp[l+1][r-1] + y);
                    chmin(dp[l][r],dp[l][r-2] + y);
                    chmin(dp[l][r],dp[l+2][r] + y);
                    chmin(dp[l][r],dp[l][r-2] + 1LL * x * (dif[r-1] - dif[r-2]));
                    chmin(dp[l][r],dp[l+2][r] + 1LL * x * (dif[l+1] - dif[l]));
                }
            }
            debug(dp);
            cout << dp[0][m] << '\n'; 
