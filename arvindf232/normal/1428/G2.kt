...
private typealias ei = Long
private typealias earr = LongArray
//NOT batle tested
...
class maxqueuefast(max:Int){
    val queue = LongArray(max)
    val actual = fastEmptyList(max)
    var frontpointer =0
    var endpointer = -1
    fun addLast(v:Long){
        while(endpointer +1 != frontpointer && queue[endpointer] < v) endpointer --
        endpointer ++
        queue[endpointer]  = v
        actual.addLast(v)
    }
    fun removeFirst(){
        val v = actual.removeFirst()
        if(endpointer +1 != frontpointer && queue[frontpointer] == v) frontpointer ++
    }
    val size:Int get() = actual.size
    fun isEmpty():Boolean = actual.isEmpty()
    fun min(): Long {
        return if(endpointer +1 != frontpointer)queue[frontpointer] else nhuge
    }

    override fun toString():String {
        return "${min()} ${actual.toList()}"
    }
}
fun debug(){}
const val singleCase = true
fun main(){
    val S = maxqueuefast(10)
    ...
    solve.cases{
        ...
        fun build(c:Int,previous:LongArray):LongArray {
            val ret = LongArray(max+1){nhuge}
            val actual = LongArray(max+1){i -> if(i % 10 == 0 && (i/10) in previous.indices) previous[i/10]- c.toLong() * (i /3) else nhuge}
            val t1max = (k-1) * 3
            val st = Array(3){maxqueuefast(ret.size / 3 + 5)}
            for(i in ret.indices){
                ...
                for(p in 0 until 3){
                    val new = (i - p).snap()
                    if( new >= 0){
                        best = maxOf(best,st[new %3].min() + (new /3) * c.toLong())
                    }
                }
                while(st[i %3].size > t1max){
                    st[i%3].removeFirst()
                }
                for(buff in 1..9){
                    val attempt = i - t1max * 3 - buff
                    if(attempt >= 0){
                        best = maxOf(best,actual[attempt] + c.toLong() * (attempt/3) + t1max * c.toLong() + if(buff % 3 == 0) (buff/3) * c.toLong() else 0 )
                    }
                }
                ret[i] = best
            }
