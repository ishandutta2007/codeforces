...
void solve(){
	...
	vector<set<int>> S(n);
	vector<vector<char>> ou(n);
	vector<map<int,char>> asdf(n);
	map<int,set<int>> M;
	for(int i = 0; i < n; ++i){
		int m; cin >> m;
		v[i].resize(m);
	...
	for(int i = 0; i < n; ++i){
		...
		for(int j = 0; j < m; ++j){
			...
			if(su.count(v[i][j])){ou[i][j] = (x ? 'R' : 'L');su.erase(v[i][j]);}
			else{ou[i][j] = (x ? 'L' : 'R');su.insert(v[i][j]);}
		}
	}
	cout << "YES\n";
	for(auto x : ou){for(auto y : x)cout << y; cout << '\n';}
}
