// if I'm not good enough, Codeforces will be just speedforces
// if I'm good enough I still need to solve the easy problems quickly so I have the time to solve the hard ones
...
void up(){
	...
	if(k>=n){
		std::cout<<
			((int64_t(k-n+k-1)*n)>>1) + std::accumulate(begin(a), end(a), int64_t(0))
			<<'\n';
	}

	else{
		std::partial_sum(begin(a), end(a), begin(a));
		int64_t result=a[k-1];
		for(int const i: range(0, n-k)){
			result=std::max(result, 
					a[i+k]-a[i]
					);
		}

		std::cout<<result+((k*int64_t(k-1))>>1)<<'\n';
	}
}
