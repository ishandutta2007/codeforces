...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        if (ok[i] && i + 2 <= n) {
            ...
            for (int j = i + 3; j <= n; ++j) {
                if (j >= i + 4 && t[j - 1] - t[j - 2] < std::abs(x[j - 1] - x[j - 2])) break;
                int64_t val = std::max(t[i] + std::abs(x[i] - x[j]), t[i + 1]) + std::abs(x[j] - x[i + 2]);
        ...
        if (dp[i] <= 1e9) {
            ...
            for (int j = i + 2; j <= n; ++j) {
                if (j >= i + 3 && t[j - 1] - t[j - 2] < std::abs(x[j - 1] - x[j - 2])) break;
                int64_t val = std::max(t[i], dp[i] + std::abs(x[i] - x[j])) + std::abs(x[j] - x[i + 1]);
                if (val <= t[i + 1]) ok[j - 1] = true;
            }
        }
    }
    
    if (ok[n - 1] || dp[n] <= 1e9) std::cout << "YES\n";
    else std::cout << "NO\n";
