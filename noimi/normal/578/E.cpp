...
template <typename T> vector<edge<T>> eulerian_path(Edges<T> es, int s = -1, bool directed = false) {
    ...
    if(s == -1) {
        if(directed) {
            ...
            for(int i = 0; i < V; i++) {
                if(abs(deg[i]) > 1) assert("not eulerian graph");
                if(deg[i] == 1) s = i;
            }
            if(s == -1) s = es[0].from;
        } else {
            ...
            if(cnt > 2) assert("not eulerian graph");
        }
    }
...
int main() {
    ...
    rep(i, n) {
        tmp += (s[i] == 'L' ? 1 : -1);
        chmax(res, max(tmp - mi, ma - tmp));
        chmax(ma, tmp), chmin(mi, tmp);
    }
    res--;
    OUT(res);
    int m = res + 1;
    ...
    if(!cnt[0][1] and !cnt[1][0] and cnt[0][0] and cnt[1][1]) {
        int a, b;
        rep(i, m) {
            if(st[i])
                b = i;
        ...
        if(la < lb) {
            v[a].eb(lb), v[b].pop_back();
        } else {
            v[b].eb(la), v[a].pop_back();
        }
        en[a] ^= 1, en[b] ^= 1;
    ...
    auto trail = eulerian_path(E, -1, true);

    vi ans;
    for(auto e : trail) {
        int i = e.cost;
        for(auto e : v[i]) ans.eb(e + 1);
    }
    OUT(ans);
}