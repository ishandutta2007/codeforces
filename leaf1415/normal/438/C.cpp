...
mint calc(ll l, ll r)
{
	...
	if((l+2)%n == r%n) return 1;
	
	mint ret = 0;
	if(can[l+1][r]) ret += calc(l+1, r);
	
	if(can[l][r-1]) ret += calc(l, r-1);
	per(i, r-2, l+2){
		if(can[l][i] && can[i][r]) ret += calc(l, i) * calc(i, r);
	}
	used[l][r] = true;
	//outl(l, r, ret);
	return memo[l][r] = ret;
...
bool check(ll i, ll j, ll k, ll nk)
{
	return sgn(cross(p[j]-p[i], p[k]-p[i])) != sgn(cross(p[j]-p[i], p[nk]-p[i])) &&
	sgn(cross(p[nk]-p[k], p[i]-p[k])) != sgn(cross(p[nk]-p[k], p[j]-p[k]));
}
bool check2(ll i, ll j, ll k)
{
	return cross(p[i]-p[k], p[j]-p[k]) == 0 && dot(p[i]-p[k], p[j]-p[k]) < 0;
}
...
int main(void)
{
	...
	ll s = 0;
	rep(i, 1, n-2) s += cross(p[i]-p[0], p[i+1]-p[0]);
	...
	rep(i, 0, n-1) rep(j, 0, n-1){
		...
		for(int k = (i+1)%n; k != j; k=(k+1)%n){
			s += cross(p[k]-p[i], p[(k+1)%n]-p[i]);
			if((k+1)%n != j && check(i, j, k, (k+1)%n) || check2(i, j, k)) can[i][j] = false;
		}
		if(s <= 0) can[i][j] = false;
		
		s = 0;
		for(int k = (j+1)%n; k != i; k=(k+1)%n){
			s += cross(p[k]-p[j], p[(k+1)%n]-p[j]);
			if((k+1)%n != i && check(i, j, k, (k+1)%n) || check2(i, j, k)) can[i][j] = false;
		}
		if(s <= 0) can[i][j] = false;
		
		can[j][i] = can[i][j];
	}
	//rep(i, 0, n-1) outa(can[i], 0, n-1);
	outl(calc(0, n-1));
