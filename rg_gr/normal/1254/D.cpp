...
#endif
newe(1000005);
arr hson,sz,dep,top,fa,dfn,tag,l,r;
int cc=0;
void dfs(int u,int faa)
{
	...
	dep[u]=dep[faa]+1;;
	fa[u]=faa;
	fe(u)if(v!=faa)dfs(v,u),sz[u]+=sz[v],(sz[hson[u]]<sz[v])&&(hson[u]=v);gr
}
void dfs2(int u,int t)
{
	...
	fe(u)if(v!=fa[u]&&v!=hson[u])dfs2(v,v);gr
	r[u]=cc;
}
signed main()
{
	...
	int n=read(),m=read();
	rg(n-1)
	int x=read(),y=read(),w=1;addedge(x,y,w),addedge(y,x,w);
	gr
	dfs(1,0);dfs2(1,1);
	// rg(n)odb(top[i]);gr puts("");
	// rg(n)odb(dfn[i]);gr puts("");
	// rg(n)odb(l[i]);gr puts("");
	// rg(n)odb(r[i]);gr puts("");
	// rg(n)odb(sz[i]);gr puts("");
	// rg(n)odb(hson[i]);gr puts("");
	rg(m)
	int op=read();if(op==1){int u=read(),v=read();
	mae(tag[u],me(v,1));
	if(fa[u])
	{
		int val=me(v,sz[u]);
		t.add(1,1,n,l[u],r[u],de(0,val));
		t.add(1,1,n,1,n,val);
	}
	if(hson[u])
	{
		int val=me(v,n-sz[hson[u]]);
		t.add(1,1,n,l[hson[u]],r[hson[u]],val);
	}
	}else{
		int u=read(),ans=me(tag[u],n);
		mae(ans,t.qsum(1,1,n,dfn[u],dfn[u]));
		while(u)
		{
			// mae(ans,t.qsum(1,1,n,dfn[top[u]],dfn[u]));
			u=top[u];
			// mae(ans,tag[u]);
			mae(ans,me(tag[fa[u]],n-sz[u]));
			u=fa[u];
		}
		// oldl(u);
		oldl(ive(ans,n));
