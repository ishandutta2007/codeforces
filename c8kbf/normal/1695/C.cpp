...
_ n, m, a[maxn][maxn], b[maxn][maxn], c[maxn][maxn];
...
inline void AC() {
    ...
    b[0][1] = b[1][0] = c[0][1] = c[1][0] = 0;
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) {
        b[i][j] = max(b[i-1][j], b[i][j-1])+(a[i][j] == 1);
        c[i][j] = min(c[i-1][j], c[i][j-1])+(a[i][j] == 1);
    }
    lt >>= 1;
    if(c[n][m] <= lt && lt <= b[n][m]) yes(7);
    else no(7);
