...
void run() {
	...
	REP(i,n-1) { scanf("%d%d",&e[i].u,&e[i].v); --e[i].u,--e[i].v; if(e[i].u>e[i].v) swap(e[i].u,e[i].v); } sort(e,e+n-1);
	...
	if(nnp==2) {
		if(np[0]==1&&np[1]==1) { printf("%d %d %d\n",min(p[0][0],p[1][0])+1,max(p[0][0],p[1][0])+1,ret+1); return; }
		printf("-1\n"); return;
	}
