...
void chemthan() {
    ...
    vector<vector<FF>> fen(2, vector<FF>(n + 1));
    auto upd = [&] (vector<FF>& fen, int p, FF val) {
        p++;
        for (; p < sz(fen); p += p & -p) {
            fen[p] += val;
        }
    };
    auto query = [&] (vector<FF>& fen, int p) {
        p++;
        FF res = 0;
        for (; p > 0; p -= p & -p) {
            res += fen[p];
    ...
    FOR(i, 0, n) {
        ...
        FF x = query(fen[0], u);
        FF y = query(fen[1], u);
        res += FF(n - u) * (x + y) * a[i].fi;
        x = i - x;
        y = sum - y;
        res += FF(u + 1) * (FF(n) * x - y) * a[i].fi;
        res += FF(a[i].fi) * (u + 1) * (n - u);
        upd(fen[0], u, 1);
        upd(fen[1], u, u);
        sum += u;
    }
