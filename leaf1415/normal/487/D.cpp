...
A get(ll y)
{
	...
	rep(i, 1, w){
		...
		while(1){
			...
			if(v <= 0){ ret[i] = 11; break; }
			if(v > w){ ret[i] = 12; break; }
			if(used[v]){ ret[i] = 0; break; }
		}
	}
	return ret;
}
...
int main(void)
{
	...
	seg.init();
	rep(i, 1, h) seg.update(i, get(i));
	
	//rep(i, 1, h) outl(i, get(i));
	
	char t; ll x, y;
	rep(i, 1, Q){
		cin >> t >> y >> x;
		if(t == 'A'){
			ll res = seg.query(1, y)[x];
			//outl(res);
			if(res == 0) cout << -1 << " " << -1 << "\n";
			else if(res <= w) cout << 0 << " " << res << "\n";
			else{
				A tmp;
				tmp[0] = x, tmp[1] = res;
				ll res2 = seg.left_find(y, tmp);
				if(res == 11) cout << res2 << " " << 0 << "\n";
				else cout << res2 << " " << w+1 << "\n";
			}
		}
		else{
