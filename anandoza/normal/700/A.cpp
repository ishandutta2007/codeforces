...
template <typename I, typename F> I find_min(const F &f, I l, I r, int iter = 100) {
    while (iter--) {
        I w = (r - l) / 3;
        I u = l + w, v = l + 2 * w;
        if (f(u) < f(v))
...
int main() {
    ...
    double trips = (n + k - 1) / k;
    auto compute = [&](double d) {
        double forward_time = d / bus;
        double meet_time = d * (1 - walk / bus) / (walk + bus);

        double ans_bus = forward_time * trips + meet_time * (trips - 1);
        double ans_walk = d / bus + (l - d) / walk;

        return max(ans_bus, ans_walk);
    };

    auto d = find_min<double>(compute, 0, l);
    pd(d);
