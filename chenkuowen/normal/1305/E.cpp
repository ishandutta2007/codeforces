...
//		n=rand()%100+2,m=rand()%(n*n/4);
	...
	int sum=0,top=0;
	vector<int> ans;
	for(int i=1;sum+(i-1)/2<=m;++i){
		sum+=(i-1)/2;
		ans.push_back(i);
	}
	if(sum!=m){
		int i=0;
		while(sum+(ans.size()-i)/2!=m) ++i;
		ans.push_back(ans.size()+i+1);
		sum+=(ans.size()-i)/2;
	}
	if(ans.size()>n){
		puts("-1");
...
//		continue;
	...
	int cnt=n-ans.size(),s=ans.size()>0?ans.back()+1:1;
	for(int i=1;i<=cnt;++i){
		if((ll)5e8+1ll*i*s>1e9) assert(0);
		ans.push_back((int)5e8+i*s);
	}
//	printf("%d\n",s);
