...
int main() {
    ...
    for (int q = 0; q < Q; q++) {
        ...
        auto possible = [&](double ans) {
            int64_t xx = int64_t(ceil(ans));
            int x = xx > INF ? INF : int(xx);
            splay_node *node = tree.query_prefix_key(x);
            int count = splay_node::get_size(node);
            int64_t sum = splay_node::get_sum(node);
            return ans * count - sum >= v;
        };

        double low = 0, high = 1e16;

        auto test = [&](double mid) {
            if (possible(mid))
                high = mid;
            else
                low = mid;
        ...
        for (int iter = 0; iter < 19; iter++)
            test(high <= 1 ? (low + high) / 2 : sqrt(low * high));
