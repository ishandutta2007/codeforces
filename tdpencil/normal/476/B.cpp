...
double dfs(int dir, int idx) {
    if(idx == n)
    {
        return (double)dir == spot;
    } else {
        if(s2[idx] == '-') {
            return dfs(dir - 1, idx + 1);
        } else if(s2[idx] == '+') {
            return dfs(dir + 1, idx+ 1);
        } else {
            return (dfs(dir + 1, idx + 1) + dfs(dir - 1, idx + 1)) / (double) 2;
        }
    }
...
void solve(int test_case = 0)
{
    ...
    for (int i = 0; i < n; i++)
        spot += (s1[i] == '+' ? 1 : -1);
    cout << fixed << setprecision(15) << dfs(0, 0) << "\n";
}
