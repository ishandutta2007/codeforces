...
void solve() {
    ...
    while (! q.empty()) {
        int cur = q.front();
        q.pop();
        for (int nb : adj[cur]) if (dist[nb] > dist[cur]+1) {
            dist[nb] = dist[cur]+1;
            q.push(nb);
        }
    }
    bydist.clear();
    F0R(i, n) {
        if (dist[i] >= bydist.size()) bydist.rsz(dist[i]+1);
        bydist[dist[i]].pb(i);
    }
    dp.assign(n, mp(0, 0));
    dp[s] = mp(1, 0);
    FOR(level, 1, bydist.size()) for (int i : bydist[level]) {
        for (int nb : adj[i]) if (dist[nb]==level-1) dp[i].F = (dp[i].F + dp[nb].F)%MOD;
    }
    FOR(level, 1, bydist.size()) for (int i : bydist[level]) {
        for (int nb : adj[i]) {
            if (dist[nb]==level-1) dp[i].S = (dp[i].S + dp[nb].S)%MOD;
            if (dist[nb]==level) dp[i].S = (dp[i].S+dp[nb].F)%MOD;
        }
    }
    cout << (dp[t].F + dp[t].S)%MOD << nl;
}
