...
vector<int> compute_starting_counts() {
    suffix_array SA(T, false);
    vector<int> change(L + 1, 0);
    ...
    for (string &str : S) {
        ...
        while (low < high) {
            int mid = (low + high) / 2;
            int position = SA.suffix[mid];

            if (T.compare(position, length, str) < 0)
                low = mid + 1;
            else
                high = mid;
        }
    ...
    vector<int> starting_counts(L, 0);
    int count = 0;

    for (int i = 0; i < L; i++) {
        count += change[i];
        starting_counts[SA.suffix[i]] = count;
    }
...
int main() {
    ...
    vector<int> starting_counts = compute_starting_counts();
    ...
    vector<int> ending_counts = compute_starting_counts();
    reverse(ending_counts.begin(), ending_counts.end());

    long long total = 0;

    for (int i = 0; i < L - 1; i++)
        total += (long long) ending_counts[i] * starting_counts[i + 1];
