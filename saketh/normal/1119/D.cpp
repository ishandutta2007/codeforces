...
int main() {
    ...
    int N; re(N); vll s(N); re(s);
    sort(all(s)); dedup(s); N = sz(s);

    vll diffs;
    FOR (i, 1, N) diffs.pb(s[i] - s[i-1]);
    sort(all(diffs));

    vll pref = prefixes(diffs, 0ll, plus<ll>());

    int Q; re(Q); F0R (q, Q) {
        ll l, r; re(l, r); r = r - l + 1;
        int f = lb(all(diffs), r) - diffs.begin();
        if (q) pr(" "); pr(r * (N - f) + pref[f]);
    }
    ps();
