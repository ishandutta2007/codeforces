...
//                    if (wa == 3 && wb == 0) cout << endl;
                    if (wa != 3 && wb != 3) continue;
                    if (wa + wb > 5) continue;
                    if (wa == 3 && who == 2) continue;
                    if (wb == 3 && who == 1) continue;
                    if (wa == 0 && who == 1) continue;
                    if (wb == 0 && who == 2) continue;
                    if (wa + wb == 5 && who == 0) continue;
                    if (wa + wb < 5 && who) continue;
                    int needa = wa * 25 - (who == 1) * 10;
                    int needb = wb * 25 - (who == 2) * 10;
                    if (needa > a || needb > b) continue;
                    int cana = wb * 23 - (who == 2) * 10;
                    int canb = wa * 23 - (who == 1) * 10;
                    bool ok = (needa + cana >= a) && (needb + canb >= b);
                    vector<pii> res;
                    res.resize(wa + wb);
                    int aa = a;
                    int bb = b;
                    ...
                    FI(waa) {res[i].first = 25; aa -= 25;}
                    FI(wbb) {res[i + waa].second = 25; bb -= 25;}
                    
                    if (ok) {
                        FI(res.size()) {
                            if (res[i].first == 0) {
                                res[i].first = min(23, aa);
                                aa -= res[i].first;
                            }
                            if (res[i].second == 0) {
                                res[i].second = min(23, bb);
                                bb -= res[i].second;
                            }
                        }
                        assert(aa == 0 && bb == 0);
...
//                    DBN(wa, wb, ok);
//                    DBN(a, b, needa, needb);
                    for (int sa = 0; sa <= wa; ++sa) {
                        for (int sb = 0; sb <= wb; ++sb) {
                            for (int tab = 0; tab < 2; ++tab) {
                                if (sa == 0 && sb == 0) continue;
                                if (tab && !who) continue;
                                if (who == 1 && sa == wa && !tab) continue;
                                if (who == 1 && sa == 0 && tab) continue;
                                if (who == 2 && sb == wb && !tab) continue;
                                if (who == 2 && sb == 0 && tab) continue;
                                int sneeda = needa + sb * 23 - (tab == 1 && who == 2) * 10;
                                int sneedb = needb + sa * 23 - (tab == 1 && who == 1) * 10;
                                if (sneeda > a || sneedb > b) continue;
                                int cana = (wb - sb) * 23 - (who == 2 && !tab) * 10;
                                int canb = (wa - sa) * 23 - (who == 1 && !tab) * 10;
                                int freea = a - sneeda;
                                int freeb = b - sneedb;
//                                DBN(sa, sb, tab);
//                                DBN(freea, freeb, cana, canb);
                                if ((freea <= freeb && freea + canb >= freeb) || (freeb <= freea && freeb + cana >= freea)) {
                                    ok = true;
                                    int saa = sa;
                                    int sbb = sb;
                                    if (tab == 1) {
                                    ...
                                    FI(res.size()) {
                                        if (i == 4) break;
                                        if (saa && res[i].second == 0) {
                                            --saa;
                                            res[i].second = 23;
                                            bb -= 23;
                                        }
                                        if (sbb && res[i].first == 0) {
                                            --sbb;
                                            res[i].first = 23;
                                            aa -= 23;
                                        }
                                    ...
                                    int e = min(aa, bb);
                                    FI(5) {
                                        if (abs(res[i].first - res[i].second) == 2) {
                                            res[i].first += e;
                                            res[i].second += e;
                                            aa -= e;
                                            bb -= e;
