...
void my_main() {
  ...
  auto check = [&](int x, int y) -> int {
    int cnt = 0;
    rep(i, 1, n + 1) {
      if (dis(p[i].x, p[i].y, x, y) + dis(x, y, sx, sy) == 
          dis(p[i].x, p[i].y, sx, sy)) {
        ++cnt;
      }
    }
    return cnt;
  ...
  Info ans{-1, -1, -1};
  if (sx > 0) ans = max(ans, {check(sx - 1, sy), sx - 1, sy});
  if (sy > 0) ans = max(ans, {check(sx, sy - 1), sx, sy - 1});
  if (sx < (int)1e9) ans = max(ans, {check(sx + 1, sy), sx + 1, sy});
  if (sy < (int)1e9) ans = max(ans, {check(sx, sy + 1), sx, sy + 1});
  cout << ans.cnt << "\n";
  cout << ans.x << " " << ans.y << "\n";
}
