...
bool dfs(vv<int> &g,int v,int i){
	vector<pii> es;
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p)->int{
				int sz=1;
				for(cauto &w:g[v])if(w!=p) sz+=dfs(dfs,w,v);
				if(sz==fib[i-1]){
					es.eb(p,v);
				}else if(sz==fib[i-2]){
					es.eb(v,p);
		...
		if(dfs(dfs,v,-1)!=fib[i]) return false;
	}
	if(i<=1) return true;
	out(g,v,i,es,1);
	for(cauto [a,b]:es){
		...
		out(g,v,i,a,b,1);
		if(dfs(g,a,i-2) && dfs(g,b,i-1)) return true;
		g[a].eb(b);
		g[b].eb(a);
	}
	return false;
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,30) fib.eb(fib[i]+fib[i+1]);
	out(fib,1);
	rep(i,fib.size())if(fib[i]==n){
		cout<<(dfs(g,0,i)?"YES":"NO") NL;
		return 0;
	}
	cout<<"NO" NL;
  return 0;
