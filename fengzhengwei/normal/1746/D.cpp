...
ll f[xx],fn[xx],dlt[xx],sum[xx];
void dfs(int x,int y)
{
//	cerr<<x<<" "<<y<<"%\n";
	f[x]=y;
	int son=0;
	for(int i=h[x];i;i=e[i].next)++son;
	sum[x]=dlt[x]=0;
	if(!son)
	{
		sum[x]=fn[x]*s[x];
	...
	sum[x]+=fn[x]*s[x];
	dlt[x]=s[x];
	sort(v.begin(),v.end(),[&](int a,int b){return dlt[a]>dlt[b];});
	for(int i=0;i<rem;i++)sum[x]+=dlt[v[i]];
	assert(rem<son);
	dlt[x]+=dlt[v[rem]];
}
int main(){
	...
	while(T--)
	{
		...
		fn[1]=m,dfs(1,0);
		cout<<sum[1]<<"\n";
		for(int i=1;i<=n;i++)fn[i]=f[i]=sum[i]=dlt[i]=0;
