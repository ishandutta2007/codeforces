...
int main() {
  ...
  set<int> s(ALL(a));
  for (auto it = prev(s.end()); ; it = prev(it)) {
    bool is_valid = true;
    for (int v = *it; v > 1;) {
      if (v % 2 == 0) {
        if (v % 4 > 0) break;
    ...
    if (!is_valid) it = s.erase(it);
    if (it == s.begin()) break;
  }
  vector<ModInt> fib(p, 1);
  FOR(i, 2, p) fib[i] = fib[i - 1] + fib[i - 2];
  FOR(i, 1, p) fib[i] += fib[i - 1];
  ModInt ans = 0;
  for (int x : s) {
    const int y = 32 - __builtin_clz(x);
    if (y > p) break;
    ans += fib[p - y];
  }
  cout << ans << '\n';
