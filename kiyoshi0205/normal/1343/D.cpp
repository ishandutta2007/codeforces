...
int main(){
  ...
  LazySegmentTree tree(seg,[](ll a,ll b){return min(a,b);},[](ll a,ll b){return a+b;},inf);
  rep(i,N/2){
    a=min(v[i],v[N-i-1]);
    b=max(v[i],v[N-i-1]);
    tree.update(1+a,K+b+1,1);
    tree.update(a+b,a+b+1,-1);
    tree.update(2,a+1,2);
    tree.update(K+b+1,K*2+1,2);
  }
  print(tree.get(0,K*2+1));
 }return 0;
}