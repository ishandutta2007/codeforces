...
vector<set<int>> v(2e5+1);

//dfs_dp
#define type pair<int,int>
type el = make_pair(0,0);
...
void dfs_dp_(graph &g, int s,int i){
    ...
    if(dp_cand[s].size()==0){
        ...
        bool ok = true;
        for(auto[ct,ch]:dp_cand[s]){
            if(v[ch].count(b[s]^a[s])){
                ok = false; break;
            }
            //cout << s << " " << ch << " " << (b[s]^a[s]) << "\n";
            // for(ll y:v[ch]){
        ...
        sort(all(dp_cand[s])); reverse(all(dp_cand[s]));
        int j = 0;
        for(auto[ct,ch]:dp_cand[s]){
            if(!ok) break;
            if(j==0) swap(v[s],v[ch]);
            else{
                for(int y:v[ch]){
                    if(v[s].count(y^a[s])){
                        ok = false;
        ...
        if(!ok){
            v[s].clear();
            dp[s] = make_pair(0,s); ans++;
        }
        else{
            v[s].insert(b[s]);
            dp[s] = make_pair((int)v[s].size(),s);
        }
        // cout << s << " " << b[s] << " " << ans << endl;
        // for(ll y:v[s]){
        //     cout << y << endl;
