...
// Problem: E. What Is It?
// Contest: Codeforces - Codeforces Round #696 (Div. 2)
// URL: https://codeforces.com/contest/1474/problem/E
// Memory Limit: 256 MB
// Time Limit: 1000 ms
...
ll getSol(vector<int> pp,vector<int> per){
	operations.clear();
	vector<int> ppos(pp.size());
	...
	for(int i=0;i<pp.size();i++){
		...
		if(ppos[tr]==tr)
			continue;
		trsol+=(ll)(tr-ppos[tr])*(tr-ppos[tr]);
		int sw=ppos[tr];
		swap(pp[sw],pp[tr]);
		ppos[pp[tr]]=tr;
		ppos[pp[sw]]=sw;
