...
void solve() {
  LL(m1, m2, m3, n1, n2, n3, s1, s2, s3);
  ll N = n1 * n2 * n3;
  ll M = m1 * m2 * m3;
  ll S = 2 * s1 * s2 * s3;

  ll ANS = 0;
  auto f = [&](ll d) -> ll { return N / d; };
  auto [div, cnt] = multiplier_mobius_on_divisors<ll>(M, f);
  FOR(i, len(div)) {
    if (S % div[i] == 0) ANS += cnt[i];
  }
  for (auto&& d: divisors(S)) { ANS += d <= M; }
  print(ANS);
}
