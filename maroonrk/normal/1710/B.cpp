...
void slv(){
	...
	rep(i,s){
		suma[i+1]+=suma[i];
		sumb[i+1]+=sumb[i];
		ini[i]=suma[i]*vs[i]+sumb[i];
	}
	segtree<MaxNode> iniseg(ini);
	vi add=ini;
	rep(i,s)add[i]+=vs[i];
	segtree<MaxNode> addseg(add);
	vi sub=ini;
	rep(i,s)sub[i]-=vs[i];
	segtree<MaxNode> subseg(sub);
	...
	for(auto [v,p]:xp){
		...
		{//f(x)=x-v+p
			int l=lwb(vs,v-p);
			int r=lwb(vs,v);
			if(iniseg.composite(0,l).v>m)ok=false;
			if(subseg.composite(l,r).v-(-v+p)>m)ok=false;
		}
		{//f(x)=p-(x-v)=-x+v+p
			int l=lwb(vs,v);
			int r=lwb(vs,v+p);
			if(iniseg.composite(r,s).v>m)ok=false;
			if(addseg.composite(l,r).v-(v+p)>m)ok=false;
		}
		cout<<ok;
	}
	cout<<"\n";
