...
void slv(){
	...
	rep(i,n)cnt[freq[i]]++;
	vc<pi> ls;
	rng(i,1,n+1)if(cnt[i])ls.eb(i,cnt[i]);
	if(ls==vc<pi>{pi(n,1)}){
		return print(1);
	}
	int g=0;
	for(auto [k,v]:ls)g=gcd(k,g);
	...
	for(int d=1;d<=n;d++)if(n%d==0){
		...
		if(g%e==0){
			...
			for(auto [k,v]:ls){
				...
				rep(_,v){
					dp[1]*=finv[num];
					if(num>=2)dp[1]+=dp[0]*d*finv[num-2];
					dp[0]*=finv[num];
				}
			}
			way+=phi[e]*dp[0]*fact[d];
			sum+=phi[e]*dp[0]*fact[d]*n-phi[e]*dp[1]*fact[d-2]*e;
		}
	}
