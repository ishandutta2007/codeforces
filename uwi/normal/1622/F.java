...
public class F2 {
	...
	void print(int n, int... rem)
	{
		Set<Integer> re = new HashSet<>();
		for(int v : rem)re.add(v);
		out.println(n - rem.length);
		for(int i = 1;i <= n;i++){
			if(!re.contains(i)){
				out.print(i + " ");
			}
	...
	void solve()
	{
		...
		long[] zob = new long[1000000 + 1];
		for (int i = 0; i <= 1000000; i++) zob[i] = gen.nextLong();

		int[] lpf = enumLowestPrimeFactors(1000000);
		long[] cum = new long[1000000 + 1];
		for (int i = 1; i <= 1000000; i++) {
			int[][] f = factorFast(i, lpf);
			cum[i] = cum[i - 1];
			for (int[] p : f) {
				if (p[1] % 2 == 1) {
					cum[i] ^= zob[p[0]];
