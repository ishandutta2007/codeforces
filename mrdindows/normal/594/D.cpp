...
int ql[200100];
int qr[200100];
vector<int> vq[200010];
int ans[200100];
int pred[1000110];
ll tr[1000010];
void upd(int cur, int l, int r, int pos, int vald, int valm) {
    if (l == r) {
        tr[cur] = tr[cur] / vald * valm;
    } else {
        int m = (l + r) >> 1;
        int dcur = cur + cur;
        if (pos <= m) {
            upd(dcur, l, m, pos, vald, valm);
        } else {
            upd(dcur + 1, m + 1, r, pos, vald, valm);
        }
...
ll get(int cur, int l, int r, int x, int y) {
    if (l == x && r == y) {
        ...
        int dcur = cur + cur;
        if (y <= m) {
            return get(dcur, l, m, x, y);
        } else if (x > m) {
            return get(dcur + 1, m + 1, r, x, y);
        } else {
            return get(dcur, l, m, x, m) * get(dcur + 1, m + 1, r, m + 1, y) % mod;
        }
    }
}
int main() {
