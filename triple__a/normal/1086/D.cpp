...
void update(int u,int v,int w){
    for (int i=u;i<=n+1;i+=lowbit(i)) C[i][v]+=w;
}
int query(int u,int v){
    int ans=0;
    for (int i=u;i>0;i-=lowbit(i)) ans+=C[i][v];
    return ans;
}
...
int main(){
    ...
    auto solve=[&](){
        int res=0;
        for (int i=0;i<3;++i){
            int ua=sz(s[(i+1)%3])?*s[(i+1)%3].begin():n+1, va=sz(s[(i+2)%3])?*s[(i+2)%3].begin():n+1;
            int ub=sz(s[(i+1)%3])?*prev(s[(i+1)%3].end()):0, vb=sz(s[(i+2)%3])?*prev(s[(i+2)%3].end()):0;
            int la=min(ua,va)-1, ra=ua-1, lb=ub, rb=max(ub,vb);
            if (la>lb){
                swap(la,lb), swap(ra,rb);
            }
            res+=query(ra,i)+query(rb,i)-query(la,i)-query(lb,i);
            if (lb<ra) res-=query(min(ra,rb),i)-query(lb,i);
        }
        cout<<n-res<<" ";
    };
    solve();
