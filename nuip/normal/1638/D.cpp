...
int main(){
	...
	cauto check=
		[&](const int i, const int j)->int{
			...
			rep(I,2)rep(J,2){
				if(cur[i+I][j+J]){
					if(c && c!=cur[i+I][j+J]) return 0;
					c=cur[i+I][j+J];
				}
			}
			return c?c:1;
		};
	auto usd=table(n,m,0);
	vector<tuple<int,int,int>> re;
	queue<pii> que;
	rep(i,n-1)rep(j,m-1)if(cauto c=check(i,j); c && MX(usd[i][j],1)){
		re.eb(i,j,c);
		que.emplace(i,j);
	}
	while(que.size()){
		cauto [i,j]=que.front(); que.pop();
		rep(I,2)rep(J,2) cur[i+I][j+J]=0;
		reps(I,-1,2)reps(J,-1,2)if(cauto c=check(i+I,j+J);c && MX(usd[i+I][j+J],1)){
			re.eb(i+I,j+J,c);
			que.emplace(i+I,j+J);
		}
	}
	int ok=1;
	rep(i,n)rep(j,m)if(cur[i][j]) ok=0;
	if(ok){
		reverse(all(re));
		cout<<re.size() NL;
