...
void solve() {
	...
	scanf("%d", &n);
	int s = good.size();
	REP(i, s) REP(j, i) REP(k, j) {
		int p = good[i], q = good[j], r = good[k];
		int re = n - p - q - r;
		if (re > 0 && re != p && re != q && re != r) {
			printf("YES\n%d %d %d %d\n", p, q, r, re);
			return;
		}
	}
...
int main() {
	...
	for (int p = 2; p < nax; ++p) if (div[p] == 0)
		for (int x = p; x < nax; x += p) div[x] = p;
	for (int i = 2; i <= 100; ++i) {
		int p = div[i];
		int q = div[i / p];
		if (p != q && i == p * q) good.push_back(i);
	}
	debug(imie(good));
	int t = 1;
	scanf("%d", &t);
