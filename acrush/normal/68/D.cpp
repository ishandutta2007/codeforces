...
{
	if (M.find(id)!=M.end()) return M[id];
	int size=SIZE(M);
	M[id]=size;
	a[size]=0;
	return size;
...
int get_s(int key)
{
	...
	if (it==M.end() || it->first!=key) return 0;
	return s[it->second];
}
void update(int key,int delta)
{
	...
	{
		int id=get_id(key);
		s[id]=a[id]+get_s(key*2+1)+get_s(key*2+2);
		if (key==0) break;
		key=(key-1)/2;
	}	
}
double calc()
{
	...
	{
		int id=get_id(key);
		int s_left=get_s(key*2+1);
		int s_right=get_s(key*2+2);
		if (s_left+s_right==0)
		{
			total+=(double)max(ps,a[id])*two(height-depth);
			break;
		}
		if (s_left<s_right)
		{
			total+=(double)max(ps,s_right+a[id])*two(height-depth-1);
			key=key*2+2;
			checkmax(ps,s_left+a[id]);
		}
		else
		{
			total+=(double)max(ps,s_left+a[id])*two(height-depth-1);
			key=key*2+1;
			checkmax(ps,s_right+a[id]);
		}
	}
	return total/two(height);
}
int main()
{
#ifdef _MSC_VER
...
#endif
	for (scanf("%d%d",&height,&cmds);cmds>0;cmds--)
	{
		char str[10];
		scanf("%s",str);
		if (strcmp(str,"add")==0)
