...
ll n,m,k,q,a[xx],b[xx];
char s[xx];
struct nod{ll next,to;}e[xx<<1];
ll cnt,h[xx];
void add(ll x,ll y){cnt++,e[cnt]={h[x],y},h[x]=cnt;}
ll l[xx],r[xx];
struct pr{ll x,y;}f[xx];
pr operator +(const pr&a,const pr&b){return {a.x+b.x,a.y+b.y};}
