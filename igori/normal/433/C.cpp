...
void solve()
{
    ...
    forn(i, m)
    {
        if (i) near[a[i]].push_back(a[i - 1]);
        if (i + 1 < m) near[a[i]].push_back(a[i + 1]);
    }

    ll cur = 0;
    for (int i = 0; i + 1 < m; i++) cur += abs(a[i + 1] - a[i]);

    ll ans = cur;
    for (int val = 0; val < n; val++)
    {
        if (near[val].size() == 0) continue;
        sort(all(near[val]));
        ll adds = 0;
        for (auto e : near[val])
            adds += abs(e - val);
        ll kek = 0;
        for (auto e : near[val])
            kek += abs(e - near[val][near[val].size() / 2]);
        ans = min(ans, cur - adds + kek);
    }
    cout << ans << "\n";
}
