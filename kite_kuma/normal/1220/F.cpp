...
int calc(int lef, int rig) {
	debug(lef, rig);
	ll id = ll(lef) * mod + rig;
	if(memo.count(id)) return memo[id];
	if(rig - lef < 2) return rig - lef;
	int nxt = st.get(lef, rig);
	debug(nxt);
	return memo[id] = 1 + max(calc(lef, place[nxt]), calc(place[nxt] + 1, rig));
}
...
int main() {
	...
	vi b;
	rep(i, place[0], n) b.push_back(v[i]);
	rep(i, place[0]) b.push_back(v[i]);
	int ans_shift = place[0];
	...
	ans_shift += ok;
	print(max(left, right) + 1, ans_shift % n);
