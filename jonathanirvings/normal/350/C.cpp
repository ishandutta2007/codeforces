//Codeforces Round #203 (Div. 2 Only)
//1 Oct 2013
...
int main()
{
	...
	REP(i,n) 
	{
		...
		risan += 2;
		if (data[i].fi != 0) risan += 2;
		if (data[i].se != 0) risan += 2;
	}
	sort(ALLA(data,n),f);
	printf("%d\n",risan);
	REP(i,n)
	{
		if (data[i].fi > 0) printf("1 %d R\n",data[i].fi);
		if (data[i].fi < 0) printf("1 %d L\n",abs(data[i].fi));
		if (data[i].se > 0) printf("1 %d U\n",data[i].se);
		if (data[i].se < 0) printf("1 %d D\n",abs(data[i].se));
		puts("2"); 
		if (data[i].fi > 0) printf("1 %d L\n",data[i].fi);
		if (data[i].fi < 0) printf("1 %d R\n",abs(data[i].fi));
		if (data[i].se > 0) printf("1 %d D\n",data[i].se);
