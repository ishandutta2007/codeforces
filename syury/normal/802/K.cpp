...
pll dfs(int v, int pr = -1) {
    ...
    for (auto x:g[v]) {
        ...
        {
            auto t=dfs(x.fi,v);
            vec.pb({x.se+t.fi,x.se+t.second});
        }
    }
    sort(all(vec));
    ll m=0;
    ford(i,sz(vec)-1,max(sz(vec)-k+1,0))
    {
        ans+=vec[i].first;
        m=max(m,vec[i].second-vec[i].first);
    }
    ll m1=m;
    f(i,sz(vec)-k+1)
    {
        m1=max({m1,vec[i].second,m+vec[i].first});
    }
    re {ans,ans+m1};
}
