...
void solve(int n) {
	...
	if (n <= 1000) {
		...
		for (int i = 0; i < n; i++)
			toshf[i] = i;
		shuffle(all(toshf), rng);
		int curBest = 48763;
		
		for (int i = 0; i < 40; i++) {
			int res = ask(i * 2, i * 2 + 1);
			if (chkmin(curBest, __builtin_popcount(res)))
				now = i * 2;
		}
	}
	...
	while (true) {
		...
		vector<int> res( cand.size() );
		for (int i = 0; i < cand.size(); i++) {
			if (now == cand[i]) continue;
			res[i] = ask(cand[i], now);
			curMask &= res[i];
		}
