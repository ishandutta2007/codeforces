...
void solve() {
  ...
  FOR3(n, 2, N + 1) {
    if (N - n <= 4) break;
    if (DP3[n - 2]) DP2[n] = 1;
    if (n >= 3 && DP2[n - 3]) DP3[n] = 1;
    if (DP2[n - 2] && S.substr(N - n, 2) != S.substr(N - n + 2, 2)) DP2[n] = 1;
    if (n >= 3 && DP3[n - 3] && S.substr(N - n, 3) != S.substr(N - n + 3, 3))
      DP3[n] = 1;
  }

  vc<string> ANS;
  FOR3(n, 1, N + 1) if (DP2[n]) ANS.eb(S.substr(N - n, 2));
  FOR3(n, 1, N + 1) if (DP3[n]) ANS.eb(S.substr(N - n, 3));
  UNIQUE(ANS);
  print(len(ANS));
