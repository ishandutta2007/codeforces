...
int main() {
	...
	vector<int> sg(MAXSG + 1, 0);
	for (int i = 2; i <= MAXSG; i++) {
		set<int> val;
		for (int j = 0; j + 2 <= i; j++)
			val.insert(sg[j] ^ sg[i - j - 2]);

		while (val.count(sg[i])) sg[i]++;
	}

	auto getSG = [&](int x) {
		if (x <= MAXSG) return sg[x];
		return sg[x % 34 + 340];
	};
