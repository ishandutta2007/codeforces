...
template<int SZ> struct palTree {
    ...
    int slink[SZ], diff[SZ], series_ans[SZ];
    int n, last, sz;
...
int main() {
    ...
    for(int i = 1; i <= sz(S); i++) {
        p.addChar(S[i-1]-'a');
        for(int v = p.last; p.len[v] > 0; v = p.slink[v])
        {
            p.series_ans[v] = ans[i - (p.len[p.slink[v]] + p.diff[v])];
            if(p.diff[v] == p.diff[p.link[v]])
                AD(p.series_ans[v], p.series_ans[p.link[v]]);
            AD(ans[i],p.series_ans[v]);
        }
        if (i&1) ans[i] = 0;
    }
    pr(ans[sz(S)]);
