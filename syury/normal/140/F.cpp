...
struct pt{
	...
	pt operator - (const pt & a)const{return pt(x - a.x, y - a.y);}
	bool operator < (const pt & a)const{return mp(x, y) < mp(a.x, a.y);}
...
void slow(){
	...
	F(i, 0, n){
		F(j, i, n){
			...
			F(c, 0, n){
				pt sym = curr - p[c];
				if(!has.count(sym))++failed;
			}
			if(failed <= k)ans.insert(curr);
		}
	}
}
...
void solve(){
	F(lp, 0, k + 1){
		F(rp, n - k - 1, n){
			...
			F(i, 0, n){
				pt sym = curr - p[i];
				while(ptr >= 0 && sym < p[ptr])--ptr;
				if(ptr < 0 || p[ptr] != sym)++failed;
			}
			if(failed <= k)ans.insert(curr);
		}
	}
...
signed main(){
	...
	in.readInteger(k);
	F(i, 0, n){
		in.readInteger(p[i].x);
		in.readInteger(p[i].y);
	}
	sort(p, p + n);
	if(k >= n){
		out.writeInteger(-1);
