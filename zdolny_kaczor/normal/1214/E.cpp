...
#define loc(r...)r
#define onl(r...)
#define azm(r)sim,class n,class...v>bool emz1(const r<c,n,v...>&o,ll b){ros.count(b);}sim,class n,class...v>const n&emz2(const r<c,n,v...>&o,ll b){ros.find(b)->second;}
...
int main() {
	...
	sort(in, in + n, greater<pii>());
	vector <int> seq(n);
	REP(i, n) seq[i] = in[i].second;
	REP(i, n - 1) ans.emplace_back(seq[i], seq[i + 1]);
	REP(i, n) {
		int x = in[i].second + 1;
		int p = i + in[i].first;
		ans.emplace_back(x, seq[p - 1]);
		if (p == (int) seq.size())
			seq.push_back(x);
	}
