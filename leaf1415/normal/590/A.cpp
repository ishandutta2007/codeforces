...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) cin >> a[i];
	a[0] = a[1], a[n+1] = a[n], a[n+2] = -1;
	
	ll cnt = 1;
	rep(i, 1, n+2){
		if(a[i] == a[i-1]) cnt++;
		else{
			if(cnt >= 2) rep(j, i-cnt, i-1) S.insert(P(j, a[j]));
			cnt = 1;
		}
	}
	...
	rep(i, 1, n){
		...
		it--;
		l = *it;
		if(abs(i-l.first) < abs(i-r.first)) a[i] = l.second, chmax(ans, abs(i-l.first));
		else a[i] = r.second, chmax(ans, abs(i-r.first));
	}
