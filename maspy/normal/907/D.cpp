...
void solve() {
  ...
  auto f = [&]() -> pair<bool, vvc<ll>> {
    ...
    FOR(x, H) FOR(y, W) { ANS[x][y] = W * x + C[y]; }
    if (H == 1) return {true, ANS};
    if (W == 4) {
      FOR(x, H) if (x % 2 == 1) reverse(all(ANS[x]));
    } else {
      FOR(x, H) if (x % 2 == 1) {
        ANS[x].eb(ANS[x][0]);
  ...
  if (transpose) {
    vvc<ll> tmp(W, vc<ll>(H));
    FOR(x, H) FOR(y, W) {
      ll a = ANS[x][y];
      auto [q, r] = divmod(a - 1, W);
      tmp[y][x] = r * H + q + 1;
    }
    ANS = tmp;
    swap(H, W);
