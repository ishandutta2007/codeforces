...
struct Input{ Input(){ io::input.reset(new io::InputFile(stdin, false)); } template<class T> auto& operator>>(T& t){ io::read(t); return *this; } } cin; struct Output{ Output(){ io::output.reset(new io::OutputFile()); } template<class T> auto& operator<<(T const& t){ io::write(t); return *this; } } cout;
...
void makeJump(){
	...
	while(true){
		...
		auto b=a; bool useful=false;
		for(auto& it: b) if(it>=0){
			it=a[it];
			if(it>=0) useful=true;
		}
		if(useful) jump.push_back(std::move(b));
		else break;
	}
}
...
void work(int node, int par, int depthNode){
	jump[0][node]=par;
	depth[node]=depthNode;
	for(auto it: first[node]) if(it!=par) work(it, node, depthNode+1);
}
...
std::vector<Component*> componentOf; // dsu-like data structure
int root(int node){ return componentOf[node]->root; }

int la(int first, int delta){
	for(int tmp=delta; tmp; tmp-=tmp&-tmp)
		first=jump[__builtin_ctz(tmp)][first];
	return first;
}

int lca(int first, int sec){
	if(depth[first]<depth[sec]) std::swap(first, sec);
	first=la(first, depth[first]-depth[sec]);
	assert(depth[first]==depth[sec]);

	if(first==sec) return first;
	std::for_each(jump.rbegin(), jump.rend(),[&](auto const& layer){
		if(layer[first]!=layer[sec]){first=layer[first]; sec=layer[sec];}
	});
	assert(first!=sec);
	return jump[0][first];
}

void process(int const par, int const child){
	// par and child are on the second tree
	auto x=root(child);
	assert(root(par)!=x);
	int y;
	//if(lca(x, par)==x){
	if(depth[par]>=depth[x] and la(par, depth[par]-depth[x])==x){
		y=par;
		std::for_each(jump.rbegin(), jump.rend(),[&, depthx=depth[x]](auto const& layer){
			if(layer[y]!=-1 and depth[layer[y]]>=depthx and root(layer[y])!=x) y=layer[y];
		});
		assert(root(y)!=x);
		assert(root(jump[0][y])==x);
		x=jump[0][y];
		std::swap(x, y);
	}else{
		assert(root(x)==x);
	...
	//link[x]=y;
	assert(componentOf[x]!=componentOf[y]);
	auto const newRoot=root(y);
	if(componentOf[x]->values.size()>componentOf[y]->values.size())
		std::swap(x, y);
	auto const smallComponent=componentOf[x];
	auto const largeComponent=componentOf[y];
	largeComponent->root=newRoot;
	for(auto& it: smallComponent->values){
		assert(componentOf[it]==smallComponent);
		componentOf[it]=largeComponent;
	}
	if(largeComponent->values.capacity()<smallComponent->values.size()+largeComponent->values.size())
		largeComponent->values.reserve(largeComponent->values.capacity()*2);
	largeComponent->values.insert(largeComponent->values.end(), begin(smallComponent->values), end(smallComponent->values));


}

std::vector<std::vector<int>> sec;
void workSec(int node, int par){
	for(auto it: sec[node]) if(it!=par) {
		workSec(it, node);
...
int main(){
	...
	cout<<number-1<<'\n';
	components.resize(number);
	componentOf.resize(number);
	for(int i=0; i<number; ++i){
		componentOf[i]=&components[i];
		components[i].values.push_back(i);
		components[i].root=i;
	}
