...
int main () {
	...
	if (p.find('*') == -1) {
		...
		int zv = p.find('*');
		string pr = p.substr(0, zv);
		string pa = p.substr(zv + 1);
		...
		for (int i = 0; i < n; i++) {
			...
			if (a.size() + 1 < p.size()) {
				...
				string pra = a.substr(0, zv);
				string vid = a.substr(zv, (int)a.size() - pa.size() - pr.size());
				string paa = a.substr(a.size() - pa.size());
				debug(pra, vid, paa);
