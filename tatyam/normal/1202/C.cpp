...
signed main(){
    ...
    rep(t){
        ...
        ll Mx=max(x),mx=min(x),My=max(y),my=min(y),h=Mx-mx+1,w=My-my+1,ans=h*w;
        if(h>2){
            if(!accumulate(find(range(x),Mx), x.end(), 0, [mx](ll a,ll b){return a||b==mx;}))chmin(ans,(h-1)*w);
            elif(!accumulate(find(range(x),mx), x.end(), 0, [Mx](ll a,ll b){return a||b==Mx;}))chmin(ans,(h-1)*w);
        }
        if(w>2){
            if(!accumulate(find(range(y),My), y.end(), 0, [my](ll a,ll b){return a||b==my;}))chmin(ans,h*(w-1));
            elif(!accumulate(find(range(y),my), y.end(), 0, [My](ll a,ll b){return a||b==My;}))chmin(ans,h*(w-1));
        }
        out(ans);
    }
}