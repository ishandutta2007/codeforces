...
int ask_sig(int id1, int id2, int id3) {
	cout << "2 " << id1 << " " << id2 << " " << id3 << endl;
	cout.flush();
	int ans;
	cin >> ans;
	return ans;
...
void solve() {
	...
	ans.push_back(1);
	{
		...
		vector<pair<ll, int>> llw, rlw;
		for (auto& p : lw) {
			if (p.x != idm) {
				if (ask_sig(idm, 1, p.x) > 0) {
					llw.push_back({ p.y, p.x });
				} else {
					rlw.push_back({ -p.y, p.x });
				}
			}
		}
		sort(all(llw));
