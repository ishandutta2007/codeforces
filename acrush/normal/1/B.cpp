...
int main()
{
	...
	for (scanf("%d",&testcase);testcase>0;testcase--)
	{
		...
		int L=strlen(s);
		bool isFirst=false;
		if (L>=4 && isalpha(s[0]) && !isalpha(s[1]))
			for (int i=2;i<L;i++)
				if (isalpha(s[i]))
					isFirst=true;
		if (isFirst)
		{
			int x=0,y=0,k=0;
			for (k++;s[k]>='0' && s[k]<='9';k++) x=x*10+(s[k]-'0');
			for (k++;s[k]>='0' && s[k]<='9';k++) y=y*10+(s[k]-'0');
			printf("%s%d\n",solve(y).c_str(),x);
		}
		else
		{
