...
int main() {
    ...
    auto dfs = [&](auto&& dfs, ll u, ll v) ->void {
        for (ll w : g[v]) {
            if (u == w) { continue; }
            rep(i,hash[v].size()) {
                ll now = hash[v][i] * base + (S[w] - 'a'+1);
                now %= mod[i];
                hash[w].push_back(now);
            }
            dfs(dfs, v, w);
        }
    };
    ...
    auto merge = [&](auto&& merge, ll u, ll v) ->set<vll> {
        set<vll> st;
        st.insert(hash[v]);

        for (ll w : g[v]) {
            if (u == w) { continue; }
            auto nowst = merge(merge, v, w);
            if (st.size() < nowst.size()) { swap(st, nowst); }
            for (vll v : nowst) { st.insert(v); }
        }
        
        val[v] = st.size() + c[v];
        //for (auto p : st)cout << p[0] << " ";
        //cout << endl;
        return st;
    };
    merge(merge, -1, 0);
    ...
    ll mx = 0;
    rep(i, N)mx = max(val[i], mx);
    ll cnt = 0;
    rep(i, N)if (val[i] == mx) { cnt++; }
