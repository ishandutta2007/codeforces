...
int main() {
    ...
    const int INF = 1e9;
    int N; re(N); vvi dist(N, vi(N, INF));

    F0R (i, N) {
        string s; re(s);
        F0R (j, N) if (s[j] == '1') dist[i][j] = 1;
    }
    ...
    int M; re(M); vi path(M); re(path); trav (e, path) --e;

    vpii dp(M, {INT_MAX, -1}); dp[0] = {1, -1};
    vi last(N, -1); last[path[0]] = 0;

    FOR (i, 1, M) {
        F0R (pv, N) if (pv != path[i] && last[pv] != -1 && i - last[pv] == dist[pv][path[i]]) {
            ckmin(dp[i], mp(dp[last[pv]].f + 1, last[pv]));
        }
        last[path[i]] = i;
    }

    vi trail;
    for (int i = M - 1; i != -1; i = dp[i].s) trail.pb(path[i] + 1);
    ps(sz(trail));
    reverse(all(trail));
    pc(trail);
