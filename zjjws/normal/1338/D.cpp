...
{
    if(fa&&nxt[x].size()==1){f[1][x]=1;return;}
    for(vector<int>::iterator i=nxt[x].begin();i!=nxt[x].end();i++)
    {
        if((*i)==fa)continue;dfs(*i,x);
        ans=max(ans,max(f[0][x]+nxt[x].size()-2,f[1][x]+1)+f[0][*i]);
        ans=max(ans,f[0][x]+nxt[x].size()-2+f[1][*i]);
        f[1][x]=max(f[1][x],f[0][*i]);
        f[0][x]=max(f[0][x],max(f[1][*i],f[0][*i]));
    }
    ans=max(ans,max(++f[1][x],f[0][x]+=nxt[x].size()-2));return;
}
int main()
{
