...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1672/problem/F1
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	...
	std::vector<std::vector<int>> indicesByValue(number+1);
	for(int const index: range(0, number)){
		std::cin>>a[index];
		indicesByValue[a[index]].push_back(index);
	}

	std::sort(begin(indicesByValue), end(indicesByValue), [&](auto const& first, auto const& sec){
		return first.size()>sec.size();
	});
	// no longer by-value anymore, but is okay
	

	for(int const pos: range(0, int(indicesByValue[1].size()))){
		for(int index=0;; index++){
			std::swap(a[indicesByValue[index][pos]], a[indicesByValue[index+1][pos]]);
			if(pos>=int(indicesByValue[index+2].size())) break; // cannot continue anymore
		}
	}
