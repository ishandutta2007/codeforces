...
template <typename T, typename F> void tuple_foreach(T&& t, F&& f) {
    ...
    #define BEGIN void __solve() {
    #define END } int main() { io_init(); int T; cin >> T; while(T--) { __solve(); } }
#elif defined(GOOGLE_CODEJAM)
    #define BEGIN(t0) void __solve(int t0) {
    #define END } int main() { io_init(); int T; cin >> T; for(int t = 1; t <= T; ++t) { __solve(t); } }
#else
...
pair<LL, LL> ans = y_combinator([&](auto rec, int n, int p) -> pair<LL, LL> {
	vector<int> u;
	vector<pair<LL, LL>> v;
	for(auto x : G[n]) {
		if(x != p) {
	...
	for(LL x : {L[n], R[n]}) {
		...
		for(int i = 0; i < int(v.size()); ++i) {
			a.back() += max(abs(x - L[u[i]]) + v[i].first, abs(x - R[u[i]]) + v[i].second);
		}
	}
	return { a[0], a[1] };
})(0, -1);

[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(max(ans.first, ans.second)), "\n");  }();
