...
int main() {
	...
	sort(all(s)); s.resize((n + 1) / 2);
	sort(rall(t)); t.resize(n / 2);
	...
	int sminptr = 0, smaxptr = 25;
	int tminptr = 0, tmaxptr = 25;

	auto shift = [&](vector<int>& cnt, int& ptr, int dir) {
		while (0 <= ptr && ptr <= 25 && cnt[ptr] == 0) ptr += dir;
	};

	shift(cnts, sminptr, 1); shift(cnts, smaxptr, -1);
	shift(cntt, tminptr, 1); shift(cntt, tmaxptr, -1);
	...
	while (cur < n) {
		if (currentTurn == 0) {
			if (sminptr < tmaxptr) {
				ans += 'a' + sminptr;
				cnts[sminptr]--;
				shift(cnts, sminptr, 1);
			} else {
				rev += 'a' + smaxptr;
				cnts[smaxptr]--;
				shift(cnts, smaxptr, -1);
			}
		} else {
			if (sminptr < tmaxptr) {
				ans += 'a' + tmaxptr;
				cntt[tmaxptr]--;
				shift(cntt, tmaxptr, -1);
			} else {
				rev += 'a' + tminptr;
				cntt[tminptr]--;
				shift(cntt, tminptr, 1);
			}
		}
