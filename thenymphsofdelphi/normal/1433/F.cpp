...
int n, m, md;
int a[N][N], amd[N][N];
int dp1[N][N][N][N]; // max sum in row i, first j elements, used k elements, sum mod md = l;
int dp2[N][N]; // max sum in row i, maximum m/2 element, sum mod md = j;
int dp3[N][N]; // max sum in first i row, sum mod md = j;
...
signed main(){
    ...
    memset(dp1, 195, sizeof(dp1));
    memset(dp2, 195, sizeof(dp2));
    ForE(i, 1, n){
        dp1[i][0][0][0] = 0;
        ForE(j, 1, m){
            ForE(k, 0, m / 2){
                For(l, 0, md){
                    dp1[i][j][k][l] = dp1[i][j - 1][k][l];
                    if (k){
                        dp1[i][j][k][l] = max(dp1[i][j][k][l],
                            dp1[i][j - 1][k - 1][(l + md - amd[i][j]) % md] + a[i][j]);
                    }
                    dp2[i][l] = max(dp2[i][l], dp1[i][j][k][l]);
                }
            }
        }
    }
    memset(dp3, 195, sizeof(dp3));
    dp3[0][0] = 0;
    ForE(i, 1, n){
        For(j, 0, md){
            For(k, 0, md){
                dp3[i][j] = max(dp3[i][j], dp3[i - 1][(j + md - k) % md] + dp2[i][k]);
            }
        }
    }
    cout << dp3[n][0];
