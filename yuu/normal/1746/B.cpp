...
void solve() {
    ...
    int one_count = a[n];
    int ans = n + 1;
    for (int end = one_count; end <= n; end++) {
        int has_one = a[end] - a[end - one_count];
        int one_after = a[n] - a[end];
        int zero_after = n - end - one_after;
        ans = min(ans, one_count - has_one + zero_after);
    }
    cout << ans << '\n';
}
