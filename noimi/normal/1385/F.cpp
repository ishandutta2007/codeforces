...
main() {
    ...
    while(T--) {
        ...
        rep(i, n - 1) {
            INT(a, b);
            g[a - 1].emplace(b - 1), g[b - 1].emplace(a - 1);
        }
        if(k == 1) {
            cout << n - 1 << endl;
            continue;
        ...
        vector<set<int>> G(n);
        rep(i, n) if(g[i].size() == 1) cnt[*g[i].begin()]++, G[*g[i].begin()].emplace(i);
        queue<int> que;
        vi used(n);
        rep(i, n) {
            if(cnt[i] >= k) { rep(_, cnt[i] / k) que.emplace(i); }
        }
        int ans = 0;
        while(!que.empty()) {
            ans++;
            ...
            if(g[now].size() == 1) {
                int par = *g[now].begin();
                cnt[par]++, G[par].emplace(now);
                if(cnt[par] % k == 0) que.emplace(par);
            }
        }
        cout << ans << endl;
    }
