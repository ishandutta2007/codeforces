...
int main() {
    ...
    for (int i = 1; i <= q; ++i) {
        ...
        if (t == 1) {
            int a, b, c; cin >> a >> b >> c; --a; --b;
            if (a > b) swap(a, b);
            edge_set[make_pair(a, b)] = make_pair(i, c);
        }
        if (t == 2) {
            int a, b; cin >> a >> b; --a; --b;
            if (a > b) swap(a, b);
            auto p = edge_set[make_pair(a, b)];
            Add(1, 0, q, p.first, i, a, b, p.second);
            edge_set.erase(make_pair(a, b));
        }
        if (t == 3) {
            int a, b; cin >> a >> b; --a; --b;
            queries[i].emplace_back(a, b, qc++);
        }
    }
