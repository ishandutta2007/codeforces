        ...
        REP(i,0,n)
        {
            while(pr.size()>0)
            {
                if(-pr.top().F<a[i].F.F)
                {
                    ...
                    sp.erase(sp.find(MP(a[pr.top().S].S.F,pr.top().S)));
                    ep.erase(ep.find(MP(a[pr.top().S].S.S,pr.top().S)));
                    if(sp.size()>0)
                    {
                        if((*sp.rbegin()).F>a[pr.top().S].S.S)
                        {
                            flag=1;
                        }
                    }
            ...
            st.insert(i);
            pr.push(MP(-a[i].F.S,i));
            sp.insert(MP(a[i].S.F,i));
            ep.insert(MP(a[i].S.S,i));
        }
        while(pr.size()>0)
        {
            st.erase(st.find(pr.top().S));
