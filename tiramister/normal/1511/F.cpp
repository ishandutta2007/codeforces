...
struct Trie {
    struct Node {
        std::array<int, K> nxt;
        bool end;
        explicit Node() : end(false) { nxt.fill(-1); }
    };
...
void solve() {
    ...
    vector<pair<int, int>> es;
    {
        ...
        while (!que.empty()) {
            ...
            for (int c = 0; c < 26; ++c) {
                if (trie[i].nxt[c] == -1 ||
                    trie[j].nxt[c] == -1) continue;

                int ni = trie[i].nxt[c],
                    nj = trie[j].nxt[c];

                add_edge(ni, nj);
                if (trie[ni].end) add_edge(0, nj);
                if (trie[nj].end) add_edge(ni, 0);
                if (trie[ni].end && trie[nj].end) add_edge(0, 0);
            }
        }
    }
