...
int main() {
    ...
    sort_by(items, a.f < b.f);

    vi64 psums(T + 1);

    F0R (i, T) {
        psums[i + 1] = psums[i] + items[i].f;
    }
    ...
    auto contribution = [&](int i) {
        assert(i == uf.find(i));
        return psums[max_elt[i] + 1] - psums[max_elt[i] + 1 - owned[i]];
    };
    ...
    auto merge = [&](int i) {
        ...
        max_elt[c] = max(max_elt[a], max_elt[b]);

        total += contribution(c);
    };

    vector<pair<int, int>> events(T - 1 + Q);

    F0R (i, T - 1) {
        events[i] = { items[i + 1].f - items[i].f, i };
    }

    F0R (q, Q) {
        int K; re(K);
        events[T - 1 + q] = { K, T - 1 + q };
    }
