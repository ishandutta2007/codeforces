...
const int N=5e2+5;
int n,m,K,p,a[N],f[N][N];
in void M(int &x) {x-=p;x+=x>>31&p;}
int main()
{
	n=read(),m=read(),K=read(),p=read();
	f[0][0]=1;for(int i=1;i<=n;i++) a[i]=read();
	for(int i=1;i<=n;i++) for(int j=1;j<=m;j++) for(int k=K;k>=a[i];k--) M(f[j][k]+=f[j-1][k-a[i]]);
	int ans=0;for(int i=0;i<=K;i++) M(ans+=f[m][i]);write(ans,'\n');
	return ot(),0;
}