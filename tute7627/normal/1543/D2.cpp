...
int main(){
  ...
  while(t--){
    ...
    auto kxor=[&](vector<ll>x,vector<ll>y){
      vector<ll>ret(m);
      rep(i,0,m)ret[i]=(x[i]+y[i])%k;
      return ret;
    };
    auto invkxor=[&](vector<ll>x,vector<ll>y){
      vector<ll>ret(m);
      rep(i,0,m)ret[i]=(x[i]-y[i]+k)%k;
      return ret;
    };
    auto ask=[&](ll x){
      cout<<x<<endl;
    ...
    rep(i,0,n){
      //debug(trans);
      auto now=convert_base(i,m,k);
      //debug(now);
      auto asknow=kxor(now,trans);
      if(i&1)asknow=invkxor(trans,now);
      //debug(asknow);
      if(ask(convert_base(asknow,k))){
        break;
      }
      trans=invkxor(asknow,trans);
    }
  }
  return 0;
}