...
char a[1000005];
int b[1000005];
int s[]={0,0,1,1,2,0,3,1,1,0,3,3,2,2,4,0,5,2,2,3,3,0,1,1,3,0,2,1,1,0,4,5,2,7,4,
...
signed main()
{
	...
	scanf("%s",a+1);
	int r=0,bb=0;
	rg(n)if(a[i]=='R')r++;else bb++;gr
	if(r!=bb)puts(r>bb?"Alice":"Bob");
	else
	{
		// puts("?");
		int c=1,N=0,cc=0,p=0,rw=0,bw=0,q=0;
		rg(n)if(i==1)continue;
		if(a[i]!=a[i-1])c++;else 
		{
			// if(c&1);else if(c>0)b[++cc]=c>>1;
			b[++cc]=c;
			c=1;
		}
		gr
...
b[++cc]=c;
		...
		// {
			// // printf("sure ");
			// puts(rw>bw?"Alice":"Bob");
			// continue;
		// }
	// odb(p);printf(":");	
	// rg(cc)odb(b[i]);gr puts("");
	// rg(n)sg[i]=(i&1)?1:2;gr
	int ns=0;
	rg(cc)ns^=sg[b[i]];gr
	// odl(ns);
	// odp(n,q);
