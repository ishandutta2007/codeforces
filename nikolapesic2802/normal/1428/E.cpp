...
// URL : http://codeforces.com/contest/1428/problem/E
// Memory Limit : 256 MB
// Time Limit : 1000 ms
...
ll get(int a,int c){
	...
	if(a%c)
		dole++,br=a%c;
	return (ll)dole*dole*br+(ll)(dole-1)*(dole-1)*(c-br);
}
int main()
{
	int n=ri(),k=ri();
	...
	set<pair<ll,int>> nxt;
	for(int i=0;i<n;i++){
		nxt.insert({get(a[i],1)-get(a[i],2),i});
		sol+=get(a[i],1);
	}
	for(int i=0;i<k-n;i++){
		auto tr=*nxt.rbegin();
		...
		int t=tr.s;
		cnt[t]++;
		nxt.insert({get(a[t],cnt[t])-get(a[t],cnt[t]+1),t});
	}
	printf("%lld\n",sol);
	return 0;
}