...
int main()
{
    ...
    for1(i,n-1)
        a[i]=a[i-1]+(s[i]=='a');
    #define ca(l,r) ((r<0||l>=n||l>r)?0:((l==0)?a[r]:(a[r]-a[l-1])))
    #define cb(l,r) ((r<0||l>=n||l>r)?0:((r)-(l)+1-ca(l,r)))
    int k=0;
    IFD( mstr.setinterval(); )
    fort1(l,-1,n-1)
        fort1(r,l+1,n)
            k=max<ll>(k,ca(0,l)+cb(l+1,r-1)+ca(r,n-1))
    IFD(
        , mstr<<l<<r<<"|"<<ca(0,l)<<cb(l+1,r-1)<<"("<<ca(l+1,r-1)<<")"<<ca(r,n-1)<<"|"<<ca(0,l)+cb(l+1,r-1)+ca(r+1,n-1)<<"\n";
    );
    mstr<<k;
}