...
void solve() {
    ...
    F0R(i, N) {
        din.add_edge(i*2, i*2+1, W[i]);
    }

    F0R(i, N) {
        if (X[i]%2 && Y[i]%2 == 0) {
            din.add_edge(2*N, i*2, 1e15);
            if (ind.count({X[i]-1, Y[i]})) {
                din.add_edge(i*2+1, 2*ind[{X[i]-1, Y[i]}], 1e15);
            }
            if (ind.count({X[i]+1, Y[i]})) {
                din.add_edge(i*2+1, 2*ind[{X[i]+1, Y[i]}], 1e15);
            }
        } else if (X[i]%2==0&&Y[i]%2==0) {
            if (ind.count({X[i], Y[i]-1})) {
                din.add_edge(i*2+1, 2*ind[{X[i], Y[i]-1}], 1e15);
            }
            if (ind.count({X[i], Y[i]+1})) {
                din.add_edge(i*2+1, 2*ind[{X[i], Y[i]+1}], 1e15);
            }
        } else if (X[i]%2==0&&Y[i]%2) {
            if (ind.count({X[i]-1, Y[i]})) {
                din.add_edge(i*2+1, 2*ind[{X[i]-1, Y[i]}], 1e15);
            }
            if (ind.count({X[i]+1, Y[i]})) {
            ...
            din.add_edge(i*2+1, 2*N+1, 1e15);
        }
    }
    ll sum = 0; F0R(i, N) sum += W[i];
