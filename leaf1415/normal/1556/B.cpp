...
struct mint{
	int x;
	mint(ll y = 0){if(x < 0 || x >= mod) x = (x%mod+mod)%mod; x = y;}
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) vec[a[i]%2].push_back(i);
		...
		if(n % 2 == 0){
			...
			if(sz(vec[0]) != sz(vec[1])+1){
				outl(-1);
				continue;
			}
		}
		
		reps(i, vec[0]) p[vec[0][i]] = i*2+1;
		reps(i, vec[1]) p[vec[1][i]] = i*2+2;
		
		BIT bit(n+5);
		ll ans = inf, tmp = 0;
		rep(i, 1, n){
			tmp += bit.query(p[i]+1, n);
			bit.add(p[i], 1);
		}
		chmin(ans, tmp);
		
		if(n % 2 == 0){
			reps(i, vec[0]) p[vec[0][i]] = i*2+2;
			reps(i, vec[1]) p[vec[1][i]] = i*2+1;
