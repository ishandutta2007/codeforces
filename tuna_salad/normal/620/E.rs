...
fn dfs(u: usize, par: usize, e: &[Vec<usize>], dfst: &mut [(usize,usize)], t: &mut usize) {
	dfst[u].0 = *t;
	*t += 1;
	for &v in e[u].iter() {
		if v != par {
...
fn main() {
	...
	dfs(0, n, &e, &mut dfst, &mut 0);
	let mut s = v!([n] = 0);
	for i in 0..n {
		s[dfst[i].0] = 1_i64 << a[i];
	}
	let mut st = LST1::new();
	st.init(s);
	rep!{[q]
		...
		let v = rin.u()-1;
		if t == 1 {
			let mut c = 1_i64 << rin.i();
			st.update(dfst[v].0, dfst[v].1, &mut c, true);
		} else {
			let res = st.query(dfst[v].0, dfst[v].1).count_ones();
			writeln!(rout, "{}", res).ok();
		}
	}
}