    ...
    f0r(root, n) {
        ...
        function<void(int)> dfs = [&](int src) {
            ...
            for (auto nxt : adj[src]) {
                int go = nxt.f;
                int we = nxt.s;
                pi p = {-label[src].f, we - label[src].s};
                if (vis[go]) {
                    pi a = p;
                    pi b = label[go];
                    if (a.f == b.f && a.s != b.s) {
        ...
        vi id;
        for (int i : comp) {
            if (label[i].f == 0) continue;
            srt.pb(label[i]);
            id.eb(i);
        }
        sort(all(srt), [&](pl a, pl b) {
            ld v1 = (ld) -a.s / a.f;
            ld v2 = (ld) -b.s / b.f;
            return (v1 < v2);
        });
        auto sgn = [&](pl x, ll y) -> int {
            ll val = x.f * y + x.s;
            if (val == 0) {
                val = x.f * (y + 1) + x.s;
                if (val > 0) return 1;
        ...
        auto eval = [&](pl x, ll y) -> ll {
            return x.f * y + x.s;
        };
        auto eval2 = [&](pl x, ld y) -> ld {
            return x.f * y + x.s;
        };
        pl tot;
        vi signs;
