...
int main(void)
{
	...
	rep(j, 0, m) rep(k, 0, 12000) dp[0][j][k] = inf;
	dp[0][0][0] = 0;
	
	rep(i, 0, n-1){
		rep(j, 0, m) rep(k, 0, 12000) dp[(i+1)&1][j][k] = inf;
		rep(j, 0, m){
			if(j > i) break;
			rep(k, 0, 12000){
				if(k+i-j <= 12000) chmin(dp[(i+1)&1][j+1][k+i-j], dp[i&1][j][k]+a[i+1]);
	...
	ll ans = inf;
	rep(i, 0, min(s, 12000LL)) chmin(ans, dp[n&1][m][i]);
	outl(ans);
