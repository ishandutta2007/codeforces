...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		if (2 * *min_element(a) >= *max_element(a)) {
			for (int i: range(n)) {
				out << -1 << " ";
			}
			return;
		...
		BottomUpMinSegmentTree<int> tree(values.size());
 
		for (int i: downrange(n)) {
			int pos = lower_bound(values, a[i]) - values.begin();
			tree.set(pos, i + n);
		}
		...
		for (int i: downrange(n)) {
			int pos = lower_bound(values, (a[i] + 1)/ 2) - values.begin();
			if (pos == 0) {
				//cerr << i << " inf" << endl;
				ans[i] =  1000000000;
			} else {
				auto cur = tree.getResult(0, pos);
				//cerr << i << ' ' << cur << endl;
				ans[i] = cur - i;
			}
			int rpos = lower_bound(values, a[i]) - values.begin();
			tree.set(rpos, i);
