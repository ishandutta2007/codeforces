...
int minl=-1000000001,maxl=1000000001,minr=-1000000001,maxr=1000000001;
int lastl=0,lastr=-1;
...
int main()
{
	...
	for(int i=4;i<n;i++)
	{
		int maxx=max(max(max(a[i],a[i-1]),a[i-2]),max(a[i-3],a[i-4]));
		int minx=min(min(min(a[i],a[i-1]),a[i-2]),min(a[i-3],a[i-4]));
		if(b[i]==0&&b[i-1]==1)
		{
			maxr=min(maxr,minx);
			maxl=min(maxl,minx);
		...
		else if(b[i]==b[i-1]&&b[i-1]==b[i-2]&&b[i-2]==b[i-3]&&b[i-3]==b[i-4])
		{
			minr=max(minr,minx-1);
			maxl=min(maxl,maxx+1);
		}
	}
	printf("%d %d\n",minl+1,maxr-1);
	return 0;
