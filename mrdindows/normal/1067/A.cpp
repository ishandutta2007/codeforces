...
int solve() {
    ...
    for (int i = 1; i < n; ++i) {
        ...
        if (x == -1) {
            ...
            for (int p = 200; p >= 1; --p) {
                sum += dp[i - 1][p][1];
                if (sum >= fftmod) sum -= fftmod;
                dp[i][p][1] = summodfast(sum, dp[i - 1][p][0], fftmod);
            }
            sum = 0;
            for (int p = 1; p <= 200; ++p) {
                dp[i][p][0] = sum;
                sum += dp[i - 1][p][0]; if (sum >= fftmod) sum -= fftmod;
                sum += dp[i - 1][p][1]; if (sum >= fftmod) sum -= fftmod;
            }
        } else {
            int sum = 0;
            for (int p = 200; p >= x; --p) {
            ...
            dp[i][x][1] = summodfast(sum, dp[i - 1][x][0], fftmod);
            sum = 0;
            for (int p = 1; p < x; ++p) {
                sum += dp[i - 1][p][0]; if (sum >= fftmod) sum -= fftmod;
                sum += dp[i - 1][p][1]; if (sum >= fftmod) sum -= fftmod;
