...
int main() {
    std::cin >> n >> k;
    if (n <= 3 || k > 3 || k == 1) return std::cout << -1 << std::endl, 0;
    if (n == 4 && k == 2) return std::cout << -1 << std::endl, 0;
    std::cout << n - 1 << std::endl;
    if (k == 2) {
        for (int i = 1; i < n; ++i) std::cout << i << ' ' << i + 1 << std::endl;
        return 0;
    }
    for (int i = 2; i < n; ++i) std::cout << 1 << ' ' << i << std::endl;
