...
void solve() {
  ...
  auto rot = [&]() -> void {
    vv(ll, B, W, H);
    FOR(x, H) FOR(y, W) B[y][x] = A[H - 1 - x][y];
    A = B;
    FOR(q, Q) {
      ll t = query[q][0], a = query[q][1], b = query[q][2];
      query[q] = {t, b, H - 1 - a};
    }
    swap(H, W);
  };
  ...
  FOR(_, 4) {
    rot();
    //  fastset
    vc<FastSet> ss(H, FastSet(W));
    FOR(x, H) FOR(y, W) if (A[x][y] == 0) ss[x].insert(y);
    // FOR(x, H) ss[x].print();

    auto B = A;

    FOR(q, Q) {
      ll t = query[q][0], x = query[q][1], y = query[q][2];
      if (t == 1) {
        if (B[x][y])
          ss[x].insert(y);
        else
        ...
        auto LR = maximal_rectangles(ht);
        FOR(i, H) {
          auto [l, r] = LR[i];
          auto h = ht[i];
          if (l <= x && x < r) { chmax(ANS[q], (r - l) * h); }
        }
      }
    }
  }
  FOR(q, Q) if (query[q][0] == 2) print(ANS[q]);
}
