...
void solve()
{
	...
	for (int x = 0; x < n; ++x)
	{
		...
		if (n - sum - 1 > 0) s.push_back(n - sum - 1);
		modint tmp = 0;
		for (auto e : s)
			tmp += modint::Binomial(e, k);
		for (auto e : s)
		{
			res += modint(e) * (n - e) * (modint::Binomial(n - e, k) - (tmp - modint::Binomial(e, k)));
		}
		res += n * (modint::Binomial(n, k) - tmp);
	}
