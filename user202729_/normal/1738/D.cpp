//(*still* a bit difficult to focus)
...
void up(){
	...
	std::vector<std::vector<int>> add(number+2);
	std::vector<int> b(number);
	int k=0;
	for(int const index: range(0, number)){
		std::cin>>b[index];
		if(index+1<b[index]) k=index+1;
		add[b[index]].push_back(index+1);
	}


	int root=add[0].empty() ? number+1: 0;
