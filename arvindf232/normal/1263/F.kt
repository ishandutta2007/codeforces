...
//        }
    ...
    fun topologicalSort(): List<Int> {
        ...
        DFS_Long_Exhaust({ret.add(it)}){a,b,c ->}
        return ret
    }
    fun DFSrecur(){
...
fun main(){
    solve.cases{
        ...
        val firstNodes = IntArray(n)
        repeat(n){
            firstNodes[it] = getint() - 1
        }
        ...
        val secondNodes = IntArray(b)
        repeat(n){
            secondNodes[it] = getint() - 1
        }
        ...
        fun gcost(a:Int,b:Int):Int {
            val L = G.LCA(a,b)
            return G.depth[b] - G.depth[L]
        }
        fun hcost(a:Int,b:Int):Int {
            val L = H.LCA(a,b)
            return H.depth[b] - H.depth[L]
        }

        val upDP = Array(n){IntArray(b){777222777} }
        val downDP = Array(n){IntArray(a){777222777} }

        upDP[0][0] = G.depth[firstNodes[0]]
        downDP[0][0] = H.depth[secondNodes[0]]

        for(i in 0 until n-1){
            for(ta in 0 until a){
                val here = downDP[i][ta]
                val downfrom = secondNodes[i]
                val downTo = secondNodes[i+1]
                val extra = hcost(downfrom,downTo)
                downDP[i+1][ta] = minOf(downDP[i+1][ta],here + extra)
                upDP[i+1][downfrom] = minOf(upDP[i+1][downfrom], here + gcost(ta,firstNodes[i+1]))
            }
            for(tb in 0 until b){
                val here = upDP[i][tb]
                upDP[i+1][tb] = minOf(upDP[i+1][tb], here + gcost(firstNodes[i],firstNodes[i+1]))
                downDP[i+1][firstNodes[i]] = minOf( downDP[i+1][firstNodes[i]], here + hcost(tb,secondNodes[i+1]))
            }
        }
        val ret = minOf(downDP.last().minOrNull()!!,upDP.last().minOrNull()!!)
        put((a-1 + b-1) - ret)
...
/*
pay attention to strange conditions
just state capturing'

forgottin about the final shifts
