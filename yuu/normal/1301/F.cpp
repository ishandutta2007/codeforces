...
public:
    ...
    void input() {
        cin >> x >> y >> u >> v;
        for (int j = 0; j < k; j++) f[j] = 1e9;
        for (int j = 0; j < k; j++) g[j] = 1e9;
    }
    ...
    void answer() {
        ...
        for (int i = 0; i < k; i++)
            for (int j = 0; j < k; j++) {
                ans = min(ans, f[i] + g[j] + floyd[i][j]);
                // cerr << i << ' ' << j << ' ' << f[i] + g[j] + floyd[i][j] << '\n';
            }
        cout << ans << '\n';
    }
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (dist[xx][yy] == 1e9) {
                ...
                if (floyd[c][a[xx][yy]] == 0)
                    floyd[c][a[xx][yy]] = dist[xx][yy] + 2;  // cost to go from any cell of c to any cell of a[xx][yy]
                q.emplace(xx, yy);
            }
        }
    }
...
void solve() {
    cin >> n >> m >> k;
    for (int i = 1; i <= n; i++) {
        a[i] | ARRAY | 1 | m | ARRAY_INPUT;
        for (int j = 1; j <= m; j++) a[i][j]--;
    }
    cin >> q;
    for (int i = 1; i <= q; i++) qr[i].input();
    for (int i = 0; i < k; i++) {
        make(i);
        for (int j = 1; j <= q; j++) qr[j].update(i);
    }
    for (int a = 0; a < k; a++)
        for (int b = 0; b < k; b++)
            for (int c = 0; c < k; c++) floyd[b][c] = min(floyd[b][c], floyd[a][b] + floyd[a][c] - 1);
    for (int i = 1; i <= q; i++) qr[i].answer();
}
