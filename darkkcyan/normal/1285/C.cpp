...
int main(void) {
    ...
    cin >> num;
    auto divs = get_divs(num);
    tuple<llong, llong, llong> ans(num, num, 1);
    rep(mask, (1ll << len(divs))) {
        llong a = 1;
        rep(i, len(divs)) {
            if ((mask >> i) & 1) a *= divs[i];
        }
        llong b = num / a;
        ans = min(ans, {max(a, b), a, b});
    }
