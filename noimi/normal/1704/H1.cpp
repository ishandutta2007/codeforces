...
class modint {
  ...
  public:
    ...
    template <typename T> modint pow(T n) noexcept {
        modint res = 1;
        modint x = a;
        while(n) {
            if(n & 1) res *= x;
    ...
    friend modint operator+(const modint &lhs, const modint &rhs) { return modint(lhs) += modint(rhs); }
    friend modint operator-(const modint &lhs, const modint &rhs) { return modint(lhs) -= modint(rhs); }
    friend modint operator*(const modint &lhs, const modint &rhs) { return modint(lhs) *= modint(rhs); }
    friend modint operator/(const modint &lhs, const modint &rhs) { return modint(lhs) /= modint(rhs); }
    friend bool operator==(const modint &lhs, const modint &rhs) { return lhs.a == rhs.a; }
    friend bool operator!=(const modint &lhs, const modint &rhs) { return lhs.a != rhs.a; }
};
#define mint modint
...
int main() {
    ...
    vv(mint, dp, n + 1, n + 1);
    dp[0][0] = 1;
    rep(i, 1, n + 1) rep(j, 2 * i, n + 1) {
        dp[i][j] = dp[i - 1][j - 2] * (n - (j - 2)) * (n - (j - 2) - 1);
        dp[i][j] += dp[i][j - 1] * (n - (j - 1));
    }
    ...
    rep(i, 1, n + 1) {
        pt[i][0] = 1;
        rep(j, 1, n + 1) pt[i][j] = pt[i][j - 1] * i;
    }
    rep(i, 1, n + 1) rep(j, n + 1) {
        int k = i, r = n - j;
        if(dp[i][j].a == 0) continue;
        // ans += mint(n - k - r).pow(r) * mint(n - 1).pow(k) * dp[i][j] * ifact(i);
        ans += pt[n - k - r][r] * pt[n - 1][k] * dp[i][j] * ifact(i);
        // if(dp[i][j].a) dump(i, j, dp[i][j], mint(n - k - r).pow(r) * mint(n - 1).pow(k) * dp[i][j]);
    }
    OUT(ans);
}