...
void solve() {
	...
	int len_tour = v * 2;
	ST<int> st(
		len_tour, [](int a, int b) { return max(a, b); }, 0);

	auto tour = [&](auto tour, int now, int par) -> void {
		debug(now, par);
		in[now] = euler_tour.size();
		euler_tour.push_back(now);
		if(now < n) {
			st.update(in[now], p[now]);
			rev[p[now]] = in[now];
		}
		foa(e, g[now]) {
		...
		out[now] = euler_tour.size();
		euler_tour.push_back(now);
		return;
	};
	tour(tour, root, -1);

	tree_doubling tr(g, root);
	...
	for(int i = 0; i < int(query.size()); i++) {
		...
		// for(int up = (1 << 20); up > 0; up >>= 1) {
		// 	if(int(tr.length_of_path(par, root)) < up) continue;
		// 	int grand = tr.level_ancestor(par, up);
		// 	if(time[grand] > i) par = grand;
		// }
		for(int up = tr.max_jump - 1; up >= 0; up--) {
			int &grand = tr.parent[up][par];
			if(grand != -1 && time[grand] > i) par = grand;
		}

		int lef = in[par], rig = out[par] + 1;
		// int idx, val;
		// tie(val, idx) = st.get(lef, rig);
		int val = st.get(lef, rig);
		ans.push_back(val);
		if(val) st.update(rev[val], 0);
	}
