...
template<typename type> inline type pow(type a,int n)
{
    if (n==0) return 1;
    if (n==1) return a;
...
int main()
{
    ...
    for (int i=1;i<=n;i++)
    {
        for (int j=1;j<=k;j++)
        {
            for (int l=j;l<=(n+1)*k;l++)
            {
                if (l>n) break;
                if (j>=d) dp[i+1][l][true]=(dp[i+1][l][true]+dp[i][l-j][true]+dp[i][l-j][false])%md;
                else dp[i+1][l][false]=(dp[i+1][l][false]+dp[i][l-j][false])%md,
                     dp[i+1][l][true]=(dp[i+1][l][true]+dp[i][l-j][true])%md;
            }
        }
    }
    //for (int i=1;i<=n;i++)
      //  for (int j=1;j<=n*k;j++)
        //    if (j==5)
          //      cout<<"dp["<<i<<"]["<<j<<"][true]=="<<dp[i][j][true]<<"\n",
            //    cout<<"dp["<<i<<"]["<<j<<"][false]=="<<dp[i][j][false]<<"\n";
    for (int i=1;i<=n+1;i++)
        ans=(ans+dp[i][n][true])%md;
    writeln(ans);
}