...
struct segtre {
	...
	inline void add(int start) {
		fori(i, curlogn) {
			arr[i][start >> i]++;
		}
	}
	inline void add(int start, int end) {
		fori(i, curlogn) {
			arr[i][start >> i]++;
		}
	}
	inline int serchlowest(int value) {
		int low = 0, high = n;
		while (low < high) {
			int mid = (low + high) / 2;
			if (val(mid) < value) {
	...
	inline int serchhigest(int value) {
		int low = -1, high = n - 1;
		while (low < high) {
	...
	inline void adder(int from, int sum) {
		int start = serchlowest(from);
		if (start + sum >= n) {
			add(start, n - 1);
			return;
		}
		int value = val(start + sum - 1);
		int x = serchlowest(value);
		int y = serchhigest(value);
		add(start, x - 1);
		add(y - (sum - (x - start) - 1), y);
	}
	inline int quert(int start, int end) {
		int x = serchlowest(start);
		int y = serchhigest(end);
		return 1 + y - x;
	}
};
string op(string s) {
