...
int main()
{
    ...
    double ans = 1e30;
    REP(i, n) {
        ans = min(ans, max(tar.x / a[i].x, tar.y / a[i].y));
    }
    int m = 0;
    make_convex_hull_pt_strict(a, n, b, m);
    REP(i, m) {
        pt u = b[i], v = b[(i + 1) % m];
        if (line_seg_cross(pt(0, 0), tar, u, v)) {
            pt cp = cross_point(u, v, pt(0, 0), tar);
            ans = min(ans, max(tar.x / cp.x, tar.y / cp.y));
        }
    }
    printf("%.12f\n", ans);
    return 0;
