...
typedef poly<modular<MOD>> polyn;
...
int main(){
    ...
    int mi = b-a+2*t*k+1;
    polyn v1;
    for(int i = 0; i<=(2*k+1)*(2*t); i++){
        if(i % (2*k+1) == 0){
            if(i%2 == 1) v1.a.push_back(-c[2*t][i/(2*k+1)]);
            else v1.a.push_back(c[2*t][i/(2*k+1)]);
        }
        else v1.a.push_back(0);
    }
    polyn v2;
    for(int i = 0; i<=(2*t); i++){
        if(i % 2 == 1) v2.a.push_back(MOD-c[2*t][i]);
        else v2.a.push_back(c[2*t][i]);
    }
    // for(int i = 0; i<v1.a.size(); i++) cout << v2.a[i] << endl;
    polyn v3 = v1.divmod(v2).first;
    for(int i = mi; i<v3.a.size(); i++){
        // cout << v3.a[i] << endl;
        ans += v3.a[i];
        ans %= MOD;
    }
