...
void solve(){
    ...
    For(i,z.size()){
        int l=-1,r=1;
        while (i+l>=0 && i+r<z.size()){
            if (z[i]-z[i+l]==z[i+r]-z[i]) {
                K[z[i]]=z[i+r]-z[i];
                break;
            }
            if (z[i]-z[i+l]<z[i+r]-z[i]) --l;
            else ++r;
        }
    }
    For(i,o.size()){
        int l=-1,r=1;
        while (i+l>=0 && i+r<o.size()){
            if (o[i]-o[i+l]==o[i+r]-o[i]) {
                K[o[i]]=o[i+r]-o[i];
                break;
            }
            if (o[i]-o[i+l]<o[i+r]-o[i]) --l;
            else ++r;
        }
    }
    For(i,n){
        if (K[i]!=n){
            r[i-K[i]]=min(r[i-K[i]],i+K[i]);
        }
    }
    for(int i=n-2;i>=0;i--){
        r[i]=min(r[i+1],r[i]);
