...
int main() {
  ...
  auto mul = [](i64 x, int y) {
    return (int) (x * y % MOD);
  };
  int const MAX = (int) 5e6 + 10;
  i64 const INF = (i64) 1e18;
  vector<i64> dp(MAX, INF);
  dp[0] = dp[1] = 0;
  for (int d = 2; d < MAX; ++d) {
    for (int i = d; i < MAX; i += d) {
      double cur = (double) dp[i / d] * d + ((i64) d * (d - 1) / 2);
      if (cur > INF) continue;
      set_min(dp[i], dp[i / d] * d + ((i64) d * (d - 1) / 2));
    }
  }
  int t, l, r;
  cin >> t >> l >> r;
