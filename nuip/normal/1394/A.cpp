...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	for(auto a:as) (a<=m?small:large).pb(a);
	sort(rall(large)); large.resize(n+1); large.insert(large.begin(),0);
	sort(rall(small)); small.resize(n+1); small.insert(small.begin(),0);
	reps(i,1,large.size()) large[i]+=large[i-1];
	reps(i,1,small.size()) small[i]+=small[i-1];
	ll re=small[n];
	out(large,small,1);
	reps(i,1,n+1){
		cauto rem=n-1-(i-1ll)*(d+1);
		if(rem<0) continue;
		out(i,rem,large[i],small[rem],1);
		MX(re,large[i]+small[rem]);
	}
	cout<<re NL;
  return 0;
}