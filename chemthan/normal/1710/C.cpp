...
void chemthan() {
  string s; cin >> s;
  int n = sz(s);
  vector<vi> dp(n, vi(1 << 6, -1));
  auto calc = [&] (auto&& calc, int p, int msk) -> int {
    if (p == n) return (msk & (1 << 3) - 1) == (1 << 3) - 1;
    int& res = dp[p][msk];
    if (~res) return res;
    res = 0;
    FOR(u, 0, 2) FOR(v, 0, 2) FOR(w, 0, 2) {
      ...
      if (!found) {
        addmod(res, calc(calc, p + 1, nmsk));
      }
    }
    return res;
  };
  int res = calc(calc, 0, 0);
  cout << res << "\n";
}
