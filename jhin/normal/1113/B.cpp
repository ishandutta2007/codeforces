...
int m,x,n,y,z,k,mx,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000];string s2,s1;vector<int>v,v2[100003];long long sum,ans,num;char ch;
...
for(int i=0;i<n;i++)
{
    cin>>a[i];ans+=a[i];mn=min(mn,a[i]);if(!hast[a[i]])hast[a[i]]=true,v.push_back(a[i]);
}
sum=ans;
for(int x=1;x<=100;x++)
{
    for(int i=0;i<v.size();i++)
        if(v[i]%x==0)sum=min(sum,ans-mn+x*mn+v[i]/x-v[i]);
