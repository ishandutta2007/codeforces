...
void solve() {
    cin >> n >> m;
    f[0] = 1;
    for (int i = 1; i <= m; i++) (f[i] = f[i - 1] * i) %= base;
    r[m] = power(f[m], base - 2);
    for (int i = m; i >= 1; i--) (r[i - 1] = r[i] * i) %= base;
    int64_t ans = 0;
    int64_t i_multiplier = 0;
    for (int i = 2; i < n; i++) {
        // for (int val = 1; val <= m; val++) {
        //     (ans += (C(val - 1, 1) * C(val - 2, n - 3)) % base * C(n - 3, i - 2)) %= base;
        // }
        (i_multiplier += C(n - 3, i - 2)) %= base;
    }
    int64_t val_multiplier = 0;
    for (int val = 1; val <= m; val++) {
        (val_multiplier += C(val - 1, 1) * C(val - 2, n - 3)) %= base;
    }

    (ans = i_multiplier * val_multiplier) %= base;
    cout << ans << '\n';
}
