#line 1 "codeforces-workspace\\contest\\1416\\d\\d.cpp"
#include <bits/extc++.h>
...
#line 9 "codeforces-workspace\\contest\\1416\\d\\d.cpp"
...
#line 22 "codeforces-workspace\\contest\\1416\\d\\d.cpp"
...
void main() {
  ...
  for (int i = 0; i < m; i++) {
    if (!erased[i]) {
      auto [s, d] = es[i];
      if (uf.same(s, d)) continue;
      s = uf.find(s), d = uf.find(d);
      if (uf.size(s) < uf.size(d)) {
        // s->d
        for (int e : cmp[s]) cmp[d].emplace_back(e);
      } else {
        // d->s
        for (int e : cmp[d]) cmp[s].emplace_back(e);
      }
      uf.unite(s, d);
    }
  }
  vector<tuple<int, int>> split;  // (large, small)
  for (auto &&[tp, idx, ref] : reversed(qs)) {
    if (tp == 2) {
      auto [src, dst] = es[idx];
      if (!uf.same(src, dst)) {
        ref = split.size();
        // bridge
        src = uf.find(src), dst = uf.find(dst);
        if (uf.size(src) < uf.size(dst)) {
          // src->dst
          for (int e : cmp[src]) cmp[dst].emplace_back(e);
          split.emplace_back(dst, src);
        } else {
          // dst->src
          for (int e : cmp[dst]) cmp[src].emplace_back(e);
          split.emplace_back(src, dst);
        }
        uf.unite(src, dst);
      }
    } else {
  ...
  vector<set<pair<int, int>>> sotd(n);
  for (int i = 0; i < n; i++) {
    int r = uf.find(i);
    sotd[r].emplace(p[i], i);
  }
  for (auto &&[tp, idx, ref] : qs) {
    if (tp == 2) {  // split
      if (~ref) {
        auto [lrg, sml] = split.back();
        split.pop_back();
        auto &to = sotd[sml];
        auto &from = sotd[lrg];
        for (int v : cmp[sml]) {
      ...
      auto [np, nv] = *rbegin(sotd[idx]);
      sotd[idx].erase({np, nv});
      sotd[idx].emplace(0, nv);
      cout << np << eol;
      p[nv] = 0;
    }
  }
