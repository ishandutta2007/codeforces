...
void solve() {
	...
	for (int x : a) if (x >> b & 1) with++;
	else without++;
	debug(imie(with), imie(without));
	assert(with % 2);
	if (with % 4 == 1 ? true : without % 2) puts("WIN"); else puts("LOSE");
}
int main() {
	// dp[0][0][0] = 1;
	// int n = 10;
	// REP(i, n) REP(j, n) if (i || j) REP(c, 2) {
		// if (i && !dp[i - 1][j][(c ^ i ^ 1) & 1]) dp[i][j][c] = true;
		// if (j && !dp[i][j - 1][(c ^ i) & 1]) dp[i][j][c] = true;
		// if (c == 1 && dp[i][j][c]) debug(imie(i), imie(j), "wins");
	// }
