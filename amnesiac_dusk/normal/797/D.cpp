...
void dfs(int at) {
	trace(at);
	if(((!lefts.empty())&&hola[at]->value>=(*lefts.begin()))||((!rights.empty())&&hola[at]->value<(*(--rights.end())))) {
		ans++;
	} else {
		findthis.insert(hola[at]->value);
	}
	if(hola[at]->l!=-1) {
		lefts.insert(hola[at]->value);
		dfs(hola[at]->l);
		lefts.erase(lefts.find(hola[at]->value));
	}
	if(hola[at]->r!=-1) {
		rights.insert(hola[at]->value);
		dfs(hola[at]->r);
		rights.erase(rights.find(hola[at]->value));
	}
}
void solve() {
	int n;
	...
	fr(i,1,n) {
		cin>>hola[i]->value>>hola[i]->l>>hola[i]->r;
		if(hola[i]->l!=-1)
			hola[hola[i]->l]->par=i;
		if(hola[i]->r!=-1)
			hola[hola[i]->r]->par=i;
	...
	int ans=0;
	fr(i,1,n) {
		if(!findthis.count(hola[i]->value)) {
			ans++;
		}
	}
	cout<<ans<<endl;
