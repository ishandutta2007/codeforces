...
uint64_t key(const string &s) {
    uint64_t res = 0;
    for (char c : s) {
        res = (res << 8) + ((unsigned char) c);
    }
    return res;
}
...
void solve() {
    ...
    umap<uint64_t, int> pinx;
    F0R (i, N) pinx[key(patterns[i])] = i;
    ...
    if (scc.components < N) {
        ...
        vi order(N);
        iota(all(order), 0);
        sort_by(order, scc.label[a] > scc.label[b]);
        trav (e, order) ++e;
        pc(order);
    }
}
