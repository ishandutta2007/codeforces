...
void solve(int bit)
{
    ll inversions_invert=0,inversions_notinvert=0;
    int appended=0;
    for(int i=l;i<=r;i++)
    {
        vi ninvertv,invertv;
        int notinverted=0,inverted=0;
        for(auto x:sets[i])
        {
            if(x&(1<<bit))
            {
                ninvertv.pb(x);
                notinverted++;
                inversions_invert+=inverted;
            }
            else
            {
                invertv.pb(x);
                inverted++;
                inversions_notinvert+=notinverted;
            }
        }
        if(ninvertv.size())
            sets.pb(ninvertv),appended++;
        if(invertv.size())
            sets.pb(invertv),appended++;
    }
    if(inversions_notinvert>inversions_invert)
    {
        ans^=(1<<bit);
    }
    total+=min(inversions_invert,inversions_notinvert);
    l=r+1;
    r+=appended;
