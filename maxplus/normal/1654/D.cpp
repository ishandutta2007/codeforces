  ...
  Eratosthenes() {
    for (int i = 2; i < N; ++i) {
      ...
      for (int j = i * i, cj = i; j < N; j += i, ++cj) if (dpr[j].d == 0) {
        dpr[j] = {i, dpr[cj].d == i? dpr[cj].p + 1: 1, dpr[cj].d == i? dpr[cj].r: cj};
      }
    }
  }

  auto pdivi(int n) const {
    vector<pair<int, int>> pdivi;
    while (n > 1) {
      pdivi.emplace_back(dpr[n].d, dpr[n].p);
      n = dpr[n].r;
    }
    return pdivi;
  }
...
vector<int> req, cur;
vector<array<int, 3>> nei[N];

void updcur(int x, int dir) {
  for (auto [d, p]: fact[x]) {
    maxi(req[d], cur[d] += p * dir);
  }
}

void dfs0(int v, int p) {
  for (auto [u, m, d]: nei[v]) if (u != p) {
    updcur(m, 1);
    updcur(d, -1);
    dfs0(u, v);
    updcur(m, -1);
...
ModNum dfs1(int v, int p, ModNum cur) {
  ModNum res = cur;
  for (auto [u, m, d]: nei[v]) if (u != p) {
    res += dfs1(u, v, cur * d * inv.inv(m));
  }
  nei[v].clear();
  return res;
}
...
int main() {
  ...
  for (int i = 1; i < N; ++i) {
    fact[i] = sieve.pdivi(i);
  }
  for (int testcase = $; testcase; --testcase) {
    int n = $;
    for (int i = 1; i < n; ++i) {
      int a = $ - 1, b = $ - 1, x = $, y = $, g = gcd(x, y);
      x /= g, y /= g;
      nei[a].push_back({b, x, y});
      nei[b].push_back({a, y, x});
    }
    req.clear(), req.resize(n + 1);
    cur.clear(), cur.resize(n + 1);
    dfs0(0, 0);
    ModNum root = 1;
    for (int i = 2; i <= n; ++i) {
      root *= fpow(ModNum{i}, req[i]);
    }
    cout << dfs1(0, 0, root) << '\n';
  }
}