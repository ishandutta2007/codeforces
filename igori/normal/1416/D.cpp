...
void build(int L = 0, int R = N, int VER = 0)
{
    if (L + 1 == R)
    {
        tree[VER] = ray[L];
    ...
    build(L, M, 2 * VER + 1);
    build(M, R, 2 * VER + 2);
    tree[VER] = max(tree[2 * VER + 1], tree[2 * VER + 2]);
}

int Get(int l, int r, int L = 0, int R = N, int VER = 0)
{
    if (R <= l || r <= L) return 0;
    if (l <= L && R <= r) return tree[VER];
    int M = (L + R) / 2;
    return max(Get(l, r, L, M, 2 * VER + 1), Get(l, r, M, R, 2 * VER + 2));
}

void Set(int pos, int L = 0, int R = N, int VER = 0)
{
    if (L + 1 == R)
    {
        tree[VER] = 0;
    ...
    int M = (L + R) / 2;
    if (pos < M) Set(pos, L, M, 2 * VER + 1);
    else Set(pos, M, R, 2 * VER + 2);
    tree[VER] = max(tree[2 * VER + 1], tree[2 * VER + 2]);
}
...
signed main()
{
    ...
    for (int i = k - 1; i >= 0; i--)
    {
        if (t[i] == 2)
        {
            ...
            int A = important[V], B = important[U];
            graph[em].push_back(A);
            graph[em].push_back(B);
            important[V] = em;
            important[U] = em;
            Merge(V, em);
            Merge(U, em);
