...
int main()
{
    ...
    For(j, 1, l - 1)
        forn(i, n)
        {
            ...
            {
                int p = i + (1 << (j - 1));
                mi[i][j] = min(mi[i][j - 1], p < n ? mi[p][j - 1] : INT_MAX);
                ma[i][j] = max(ma[i][j - 1], p < n ? ma[p][j - 1] : INT_MIN);
            }
        }
    vector <PII> ans;
    int max_len = 0;
    ...
    {
        while (j < n && max(ma[i][lg[j - i + 1]], ma[j - (1 << lg[j - i + 1]) + 1][lg[j - i + 1]]) - min(mi[i][lg[j - i + 1]], mi[j - (1 << lg[j - i + 1]) + 1][lg[j - i + 1]]) <= k)
            ++j;
        if (j - i > max_len)
        {
            max_len = j - i;
