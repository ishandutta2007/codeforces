...
void solve() {
	...
	rep(i, n)cnt[a[i]]++;
	vector<int> rcnt(cnt.size() + 1);
	rep(i, cnt.size()) {
		rcnt[i + 1] = rcnt[i] + cnt[i];
	}
	...
	rep(i, n) {
		int id = vp[i].second;
		val[i] = a[id];
		if (b[id] == 0)val[i] += rcnt[a[id]];
		else val[i] += n - rcnt[a[id] + 1];
	}
	int t = val[0];
	rep(i, n)if (t < a[i]) {
		cout << 0 << "\n"; return;
	...
	if (vs.size()) {
		vector<P> ori(vs.size());
		rep(i, vs.size())ori[i] = { vs[i].second,i };
		auto f = [&](P a, P b) {
			if (a.first != b.first)return min(a, b);
			else return max(a, b);
		};
		auto g = [&](P a, int b, int l, int r) {
			a.first -= b;
			return a;
		};
		auto h = [&](int a, int b) {
		...
		SegT<P, int> st(ori, P{ mod,-1 }, 0, f, g, h);
		rep(_, vs.size()) {
			P p = st.query(0, vs.size());
			int id = p.second;
			if (p.first != 0) {
				cout << 0 << "\n"; return;
			}
			int le = lower_bound(all(vs), pair<P, int>{ {vs[id].first.first, vs[id].first.second | 1}, -1 }) - vs.begin();
			st.add(1, le, vs.size());
			st.update(id, { mod,-1 });
		}
	}
