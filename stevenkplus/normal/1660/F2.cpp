...
int main() {
   ...
   while (T--) {
      ...
      vector<vector<int>> bit(3, vector<int>(2 * n + 2));
      auto ask = [&](vector<int> &v, int x) -> int {
         int res = 0;
         while (x) {
            res += v[x];
            x &= x - 1;
      ...
      auto inc = [&](vector<int> &v, int x) -> void {
         while (x < sz(v)) {
            ++v[x];
            x += x & -x;
         }
      ...
      ll ans = 0;
      for (int i = 0; i <= n; ++i) {
         // count pref[j] s.t. pref[j] >= pref[i], pref[j] = pref[i] % 3
         vector<int> &v = bit[pref[i] % 3];
         ans += ask(v, 2 * n + 1) - ask(v, pref[i] - 1);
         inc(v, pref[i]);
      }
      cout << ans << "\n";
   }
