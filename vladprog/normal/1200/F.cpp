...
#define y1 eroigheiorgnerib1

const int N=1010,MOD=2*2*2*3*3*5*7;
//const int INF=;

int k[N];
vector<int> e[N];
int nxt[N*MOD];
vector<int> rev[N*MOD];
bool used[N*MOD];
vector<int> ord;
...
signed main()
{
    ...
    for1(i,n)
        k[i]=(k[i]%MOD+MOD)%MOD;
    for1(i,n)
        e[i]=read<int>(read<int>());
    for1(x,n)
        for0(y,MOD)
        {
            int y1=(y+k[x])%MOD;
            int x1=e[x][y1%sz(e[x])];
            nxt[x+y*n]=x1+y1*n;
//            cout<<"rev["<<x1+y1*n<<"].phb("<<x+y*n<<")"<<endl;
            rev[x1+y1*n].phb(x+y*n);
        }
    for1(v,n*MOD)
        dfs1(v);
    mem0(used);
