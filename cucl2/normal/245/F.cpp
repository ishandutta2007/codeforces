...
int n,m,cnt=0,date[5000007];
char eater[5000007];
int daynum[]={0,31,29,31,30,31,30,31,31,30,31,30,31};

int input()
{
	int MM,DD,hh,mm,ss,val=0;
	if(scanf("2012-%d-%d %d:%d:%d:",&MM,&DD,&hh,&mm,&ss)==EOF)return 0;
	gets(eater);
	for(int i=1;i<MM;i++)val+=daynum[i];
	val+=DD-1;
	val=val*24+hh;
	val=val*60+mm;
	val=val*60+ss;
...
void output(int val)
{
	...
	for(MM=1;MM<=12;MM++)
	{
		if(val<daynum[MM])break;
		val-=daynum[MM];
	}
	printf("2012-%02d-%02d %02d:%02d:%02d",MM,val+1,hh,mm,ss);
}
