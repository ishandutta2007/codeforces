...
ll n;
vector<P> vec[100005];
mint sum[100005], beki[200005], beki2[200005];
map<ll, mint> mp;
...
int main(void)
{
	...
	mint inv26 = mint(26).inverse();
	beki[0] = mint(1);
	rep(i, 1, 200000) beki[i] = beki[i-1] * inv26;
	beki2[0] = mint(1);
	rep(i, 1, 200000) beki2[i] = beki2[i-1] * mint(25);
	...
	rep(i, 1, 100000){
		...
		rep(j, 0, 100000){
			sum[j] = comb(i-1+j, j)*beki[i+j]*beki2[j];
			if(j > 0) sum[j] += sum[j-1];
		}
		for(auto p : vec[i]){
			if(p.first-i < 0) mp[p.second] = mint(0);
			else mp[p.second] = sum[p.first-i] / beki[p.first];
		}
	}
	
	for(auto p : mp) outl(p.second);
