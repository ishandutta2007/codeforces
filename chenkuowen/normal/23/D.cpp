...
template<class T> struct P{
	...
	inline P<T> operator!(){ return P<T>(x,-y); }
	inline P<T> operator+(P<T> b)const { return P<T>(x+b.x,y+b.y); }
	inline P<T> operator-(P<T> b)const { return P<T>(x-b.x,y-b.y); }
	inline P<T> operator*(P<T> b)const { return P<T>(x*b.x-y*b.y,x*b.y+y*b.x); }
	inline P<T> operator/(T t)const { return P<T>(x/t,y/t); }
	inline P<T> div2(P<T> b)const { return (*this)*(!b); }
	inline P<T> operator/(P<T> b)const { return div2(b)/b.dis2(); }
	inline T cp(P<T> b)const { return x*b.y-y*b.x; }
	inline T dot(P<T> b)const { return x*b.x+y*b.y; }
	inline P<T> rot90()const { return P<T>(-y,x); }
	inline P<T> rot(T x)const { return (*this)*P<T>(cos(x),sin(x)); }
	inline T dis2(){ return x*x+y*y; }
...
template<class T>bool check_polygon(vector<P<T> > a){
	bool flag1=1,flag2=1;
	for(int i=0;i<a.size();++i){
		int s=sn((a[(i+1)%a.size()]-a[i]).cp(a[(i+2)%a.size()]-a[i]));
		if(s<=0) flag1=0;
		if(s>=0) flag2=0;
	}
	return flag1||flag2;
...
template<class T>struct Circle{
	P<T> x; T r2;
	Circle(P<T> _x=P<T>(),T _r=T()){ x=_x,r2=_r; }
	Circle(P<T> a,P<T> b,P<T> c){
		x=getIP(getPL(a,b),getPL(a,c));
		r2=(a-x).dis2();
	}
	int in(P<T> y){ return sn(r2-(y-x).dis2()); }
};
}
/*-------------------Data Structure----------------------*/
/*-----------------String Algorithms---------------------*/
/*-------------------Coding-----------------------------*/
using namespace GEO;
bool solve(P<db> x,P<db> y,P<db> z){
	if(para(getPL(y+y-x,y),getPL(y,z))) return 0;
	P<db> c=Circle<db>(y+y-x,y,z).x;
	P<db> b=y+y-c,a=x+x-b,d=z+z-c;
	vector<P<db> > ret({a,b,c,d});
	if(check_polygon(ret)) {
		puts("YES");
		for(auto x:ret) printf("%.9lf %.9lf ",x.x,x.y);
		puts("");
		return 1;
	}
	return 0;
