...
ll n;
vector<ll> G[2005];
mint dp[2005][2005], dpsum[2005][2005];
mint lmul[2005][2005], rmul[2005][2005], sum[2005];
mint ans[2005];
...
void dfs(int v, int p)
{
	...
	if(cnt == 0){
		rep(i, 1, n) dp[v][i] = 1, dpsum[v][i] = i;
		return;
	}
	
	ll d = sz(G[v]);
	rep(i, 1, n) lmul[i][0] = 1, rmul[i][d+1] = 1;
	rep(i, 1, n){
		rep(j, 1, d){
			ll u = G[v][j-1];
			if(u == p){
	...
	rep(i, 1, n){
		rep(j, 1, d){
			...
			if(u == p) continue;
			dp[v][i] += dp[u][i] * sum[u];
			sum[u] += lmul[i][j-1] * rmul[i][j+1];
		}
		dp[v][i] += rmul[i][1];
	}
	
	rep(i, 1, n) dpsum[v][i] = dpsum[v][i-1] + dp[v][i];
}
...
int main(void)
{
	...
	rep(i, 1, n) ans[i] = rmul[i][1];
	rep(i, 2, n) rep(j, 1, i-1) ans[i] -= ans[j] * comb(i, j);
	outa(ans, 1, n-1);
