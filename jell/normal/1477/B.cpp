#line 1 "codeforces-workspace/contest/1477/b/b.cpp"
...
#line 12 "codeforces-workspace/contest/1477/b/b.cpp"
...
#line 28 "codeforces-workspace/contest/1477/b/b.cpp"
...
void main() {
  ...
  struct mono {
    ...
    mono(int a, int b) : zero(a), one(b) {}
    mono operator+(mono r) { return {zero + r.zero, one + r.one}; }
    mono operator*(endo r) {
      mono res{*this};
      if (~r.asgn) {
        if (r.asgn) {
          res.one += res.zero;
  ...
  lazy_segment_tree<mono, endo> laz(begin(f), end(f));
  for (auto &&[l, r] : reversed(segs)) {
    --l;
    auto [z, o] = laz.fold(l, r);
    if (z == o) {
      cout << "NO"
           << "\n";
      return;
