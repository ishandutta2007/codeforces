...
int main() {
	...
	vector<vector<ll>> pre(n, vector<ll>(2, 0)), suf = pre;
	for (int i = 1; i < n; i++) {
		pre[i] = pre[i - 1];
		if (s[i - 1] != '?')
			pre[i][s[i - 1] - '0']++;
	...
	for (int i = 0; i < n; i++)
		if (s[i] != '?')
			base += (s[i] == '0' ? pre[i][1] * c10 : pre[i][0] * c01);
	...
	const int q = Qpos.size();
	
	vector<vector<ll>> preQ(q, vector<ll>(2, 0)), sufQ = preQ;
	for (int i = 0; i < q; i++) {
		preQ[i] = pre[Qpos[i]];
		sufQ[i] = suf[Qpos[i]];
	}
	...
	auto getPre = [&](int l, int r, int v) -> ll {
		if (l > r)
			return 0;
		return preQ[r][v] - (l == 0 ? 0 : preQ[l - 1][v]);
	};
	
	auto getSuf = [&](int l, int r, int v) -> ll {
		if (l > r)
			return 0;
		return sufQ[l][v] - (r == q - 1 ? 0 : sufQ[r + 1][v]);
	};
			
	ll ans = LLONG_MAX;
	for (int p = 0; p <= q; p++) {
		ll cnt01 = getPre(p, q - 1, 0) + getSuf(0, p - 1, 1);
		ll cnt10 = getPre(0, p - 1, 1) + getSuf(p, q - 1, 0);
		ll inter = p * (q - p);
		
		ll val = cnt01 * c01 + cnt10 * c10 + inter * c01;
		chkmin(ans, val);
	}
