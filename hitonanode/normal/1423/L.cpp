...
int main()
{
    int N, S, D;
    cin >> N >> S >> D;
    vector<lint> zobrist_hash(N + 1);
    for (auto &x : zobrist_hash) x = rnd(1LL << 62);
    ...
    int L = min(17, S);
    vector<plint> ls;
    gp_hash_table<lint, int> rs;
    vector<int> VR(1e8, 1e9);
    REP(S, 1 << L)
    {
        lint b = 0;
        REP(i, L) if ((S >> i) & 1)
        ...
        ls.emplace_back(b, __builtin_popcount(S));
    }
    REP(s, 1 << (S - L)) {
        lint b = 0;
        REP(i, S - L) if ((s >> i) & 1)
        ...
        if (rs.find(b) != rs.end()) {
            chmin(rs[b], __builtin_popcount(s));
        }
        else {
            rs[b] = __builtin_popcount(s);
        }
    }
    REP(d, D)
    {
        ...
        for (auto [bl, popl]: ls) {
            lint bc = bl ^ b;
            if (rs.find(bc) != rs.end()) {
                chmin<int>(ret, rs[bc] + popl);
            }
        }
        cout << (ret < 100 ? ret : -1) << '\n';
    }
