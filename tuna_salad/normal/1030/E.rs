...
mod i1 {
	...
	impl LSTNode for I {
		...
		fn update_value_from_lazy(n: &mut N<Self>) {
			if let Some(l) = n.l {
				if l {
					n.v = (n.len() as i32) - n.v;
				}
			}
		}
		fn augment_lazy(n: &mut N<Self>, l: &L, _fw: bool) {
			let x = n.l.get_or_insert(false);
			*x ^= *l;
		}
...
fn main() {
	...
	let mut st = v!([64]);
	st.push(0);
	let mut sgt = LST1::new();
	sgt.init_def(n+1);
	sgt.update(0, n, &mut true, true);
	for i in 1..=n {
		...
		for j in 1..m {
			...
			if st[j-1] < res {
				ans += sgt.query(st[j-1]+1, res) as i64;
			}
		}
	}
