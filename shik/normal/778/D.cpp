...
vector<PII> solve( vector<string> s ) {
    vector<PII> sol;
    ...
    while ( 1 ) {
        auto ts=s;
        REP(i,n-1) REP(j,m) if ( s[i][j]=='L' && s[i+1][j]=='L' ) rot(i,j);
        REP(i,n) REP(j,m-1) if ( s[i][j]=='U' && s[i][j+1]=='U' ) rot(i,j);
        if ( s==ts ) break;
    }
    return sol;
}
...
int main() {
    ...
    W(SZ(ans));
    for ( auto i:ans ) W(i.first+1,i.second+1);
    return 0;
}