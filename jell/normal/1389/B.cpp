...
struct solver
{
    ...
    solver()
    {
        const int n=read<int>(),k=read<int>(),z=read<int>();
        vector<int> a=read<vector<int>>(n);
        vector dp(z+2,vector<int>(n,INT_MIN));
        dp[0][0]=0;
        for(int i=0; i<=z; i++)
        {
            for(int j=0; j<n; j++)
            {
                if(j)
                {
                    chmax(dp[i+1][j],dp[i][j]+a[j]+a[j-1]);
                }
                if(j+1<n)
                {
                    chmax(dp[i][j+1],dp[i][j]+a[j]);
                }
            }
        }
        int ans=0;
        for(int i=0,kk=k; i<=z && kk>=0; i++,kk-=2)
        {
            chmax(ans,dp[i][kk]+a[kk]);
        }
        for(int i=0,kk=k-1; i<z && kk>0; i++,kk-=2)
        {
            chmax(ans,dp[i][kk]+a[kk]+a[kk-1]);
        }
        cout<<ans<<endl;
    }
};