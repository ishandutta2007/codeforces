...
bool can_move(pii u, pii v, int type) {
    int x = abs(u.f - v.f), y = abs(u.s - v.s);
    switch (type) {
    case 0:
        return x + y == 3 && x * y != 0;
    case 1:
...
using C = pii;
C operator+(const C &u, const C &v) { return {u.f + v.f, u.s + v.s}; }
...
int main() {
    ...
    auto ind = [&](int i, int t) { return 3 * i + t; };

    WeightedDirectedGraph<C> g(m * 3);
    for (int i = 0; i < m; i++) {
        for (int u = 0; u < 3; u++) {
            // move a piece
            for (int j = 0; j < m; j++) {
                if (can_move(squares[i], squares[j], u))
                    g.add_directed_edge(ind(i, u), ind(j, u), {1, 0});
            }
            // change pieces
            for (int v = 0; v < 3; v++) {
                g.add_directed_edge(ind(i, u), ind(i, v), {1, 1});
            }
        }
    }

    array<C, 3> cost{{{0, 0}, {0, 0}, {0, 0}}};
    for (int i = 0; i + 1 < m; i++) {
        array<C, 3> next{{{1e9, 1e9}, {1e9, 1e9}, {1e9, 1e9}}};

        for (int u = 0; u < 3; u++) {
            ShortestPathTree<C> spt(g, {ind(i, u)}, {0, 0}, L2(x, y, x + y), less<C>());
            for (int v = 0; v < 3; v++) {
                ckmin(next[v], cost[u] + spt.distance(ind(i + 1, v)));
            }
        }
