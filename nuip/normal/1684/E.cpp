...
int main(){
	...
	TT{
		...
		multiset<int> L,R;
		int lsum=0;
		rep(i,n)if(cnts[i]) R.emplace(cnts[i]);
		for(cauto &p:cntl) R.emplace(p.Y);
		int re=MOD;
		int need=0;
		rep(i,n+1){
			while(R.size() && lsum+*R.begin()<=need){
				lsum+=*R.begin();
				R.erase(R.begin());
			}
			if(need<=t) MN(re,(int)R.size());
			if(i==n) break;
			if(cnts[i]){
				if(cauto it=R.find(cnts[i]);it!=R.end()){
					R.erase(it);
				}else{
					lsum-=cnts[i];
					// L.erase(L.find(cnts[i]));
				}
			}else{
				++need;
			}
