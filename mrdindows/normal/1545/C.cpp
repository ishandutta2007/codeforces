...
int cnt[505][505];
int n;
bool anySame(const int *a, const int *b) {
    bool ok = false;
    for (int i = 0; i < n; ++i) {
        ok |= a[i] == b[i];
    }
...
bool used[2010];
vector<int> res;
void go(int cur, bool take) {
    used[cur] = true;
    if (take) {
        res.push_back(cur);
    }
    for (int i = 0; i < 2 * n; ++i) {
        if (g[cur][i] && !used[i] && !good[i] && !bad[i]) {
            go(i, !take);
        }
    }
}
...
int solve() {
    ...
    while (tc--) {
        ...
        res.clear();
        for (int i = 0; i < 2 * n; ++i) {
            if (good[i] || bad[i] || used[i]) continue;
            go(i, true);
            ans = ans * 1LL * 2 % fftmod;
        }
        FI(n) FJ(n) v[i][j].clear();
        FI(n) FJ(n) cnt[i][j] = 0;
        cout << ans << endl;
        FI(2 * n) {
            if (good[i]) {
