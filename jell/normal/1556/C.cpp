#line 1 "codeforces-workspace\\contest\\1556\\c\\c.cpp"
...
#line 16 "codeforces-workspace\\contest\\1556\\c\\c.cpp"
//*/
...
void main() {
  ...
  struct mono {
    ...
    mono operator+(mono x) const {
      return {std::min(min, sum + x.min), sum + x.sum};
    }
  };
  ...
  for (auto l : range(n - 1)) {
    if (l & 1) continue;
    mono acc;
    for (auto r : range(l + 1, n)) {
      if (r & 1) {
        i64 low = 1, up = runs[l].sum;
        chgr(low, -acc.min);
        chgr(low, 1 - acc.sum);
        chle(up, -runs[r].sum - acc.sum);
        ans += std::max(i64(0), up - low + 1);
      }
      acc = acc + runs[r];
    }
  }
