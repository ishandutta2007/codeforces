...
int Dfs( int pos , int fa ){
	...
	if( a[pos] == 0 and val.empty() ){
		...
		for( auto [c,v] : cur ){
			// c MUST be -1
			dp[pos][0] = 1LL*dp[pos][0]*(dp[v][1]+dp[v][0]) % MOD;
		}
		for( auto [c,v] : cur ){
			int tadd = dp[v][1];
			for( auto [c,w] : cur ){
				if( v == w ) continue;
				tadd = 1LL*tadd*(dp[w][0]+dp[w][1]) % MOD;
			}
			dp[pos][1] = (dp[pos][1]+tadd) % MOD;
		}
		return -1;
	}else{
		dp[pos][0] = 0;
		...
		for( auto [c,v] : cur ){
			if( c == -1 ){
				dp[pos][1] = 1LL*dp[pos][1]*(dp[v][0]+dp[v][1]) % MOD;
			}else{
				dp[pos][1] = 1LL*dp[pos][1]*dp[v][1] % MOD;
			}
		}
		if( a[pos] == 0 ) return val[0];
		if( pos == l[a[pos]] ) return -1;
		else return a[pos];
	}
}
