...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/248834/problem/G */
/*input
4 2 3 10
wwhw
*/
...
int main () {
	...
	for (int i = 0; i <= n; i++) {
		...
		while (it + 1 < i and r[it + 1] - r[0] + (it + 1) * a <= lik) {
			it++;
		}
	...
	for (int i = n - 1; i >= 0; i--) {
		...
		while (it - 1 > i and t[it - 1] + (n - it + 1) * a <= lik) {
			it--;
		}
