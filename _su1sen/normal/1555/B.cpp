...
int main() {
    ...
    loop(t) {
        ...
        auto valid = [&](int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
            if (x1 < 0 or y1 < 0 or x3 < 0 or y3 < 0) return false;
            if (x2 > w or y2 > h or x4 > w or y4 > h) return false;
            int cx = min(x4, x2) - max(x3, x1);
            int cy = min(y4, y2) - max(y3, y1);
            return cx <= 0 or cy <= 0;
        };

        int ans = numeric_limits<int>::max();
        for (int l : { 0, w - a }) for (int d : { 0, h - b }) {
            int r = l + a;
            int u = d + b;
            
            
            int rx = max(0, r - x1);
            if (valid(l, d, r, u, x1 + rx, y1, x2 + rx, y2)) {
                chmin(ans, rx);
            }
            int lx = max(0, x2 - l);
            if (valid(l, d, r, u, x1 - lx, y1, x2 - lx, y2)) {
                chmin(ans, lx);
            }
            int uy = max(0, u - y1);
            if (valid(l, d, r, u, x1, y1 + uy, x2, y2 + uy)) {
                chmin(ans, uy);
            }
            int dy = max(0, y2 - d);
            if (valid(l, d, r, u, x1, y1 - dy, x2, y2 - dy)) {
                chmin(ans, dy);
            }
        }
