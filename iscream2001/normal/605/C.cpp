...
void MAIN(){
	...
	for(int i=1;i<=n;++i){
		...
		a[i].x=x;
		a[i].y=y;
		ans=min(ans,max((LD)p/(LD)x,(LD)q/(LL)y));
	}
	...
	LD X,Y,Z;
	for(int i=1;i<=n;++i){
		while(tp>1&&(a[Q[tp]]-a[i])/(a[Q[tp-1]]-a[i])>=0) --tp;
		Q[++tp]=i;
		X=(LD)q/(LD)p;
		Y=a[Q[tp]].y/a[Q[tp]].x;
		Z=a[Q[tp-1]].y/a[Q[tp-1]].x;
		if(X>=min(Y,Z)&&X<=max(Y,Z)){
			Y=a[Q[tp-1]].x*a[Q[tp]].y-a[Q[tp-1]].y*a[Q[tp]].x;
			if(fabs(Y)>eps){
				Y=((LD)p*a[Q[tp]].y-(LD)q*a[Q[tp]].x)/Y;
				X=((LD)p-a[Q[tp-1]].x*Y)/a[Q[tp]].x;
				ans=min(ans,X+Y);
			}
		}
	}
