...
void main()
{
	...
	while(t--) {
		...
		ll ans=2e9,ts=0;
		for(int i=0;i<n;++i) {
			cin>>a[i]; tsq+=a[i]*a[i];ts+=a[i];
		}
		b.resize(n);
		for(int i=0;i<n;++i) {
			cin>>b[i]; tsq+=b[i]*b[i];ts+=b[i];
		}
		for(int i=0;i<=n;++i)
			memset(su[i],0,sizeof su[0]);
		su[0][0]=1;
		for(int i=0;i<n;++i) {
			for(int j=0;j<S;++j) if(su[i][j])
				su[i+1][j+a[i]]=1,
				su[i+1][j+b[i]]=1;
		}
		for(int i=0;i<S;++i)
			if(su[n][i]) ans=min(ans,i*i+(ts-i)*(ts-i)+tsq*(n-2));
		printf("%lld\n",ans);
	}
