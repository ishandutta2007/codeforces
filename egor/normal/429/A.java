...
class TaskA {
    ...
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		int[] init = IOUtils.readIntArray(in, count);
		int[] goal = IOUtils.readIntArray(in, count);
		for (int i = 0; i < count; i++)
			goal[i] ^= init[i];
		out.printLine(process(0, -1, 0, 0, graph, goal));
		answer.inPlaceSort();
		for (int i : answer.toArray())
			out.printLine(i);
    }

	private int process(int vertex, int last, int chCurrent, int chNext, Graph graph, int[] goal) {
		int result = 0;
		if ((goal[vertex] ^ chCurrent) != 0) {
			chCurrent ^= 1;
			result++;
			answer.add(vertex + 1);
		}
		for (int i = graph.firstOutbound(vertex); i != -1; i = graph.nextOutbound(i)) {
			...
			if (next == last)
				continue;
			result += process(next, vertex, chNext, chCurrent, graph, goal);
		}
		return result;
	}
}
