...
signed main(){
    ...
    vector<bools> f(n, bools(3, true));
    REP(i, k){
        if(f[x[i]][0]) f[x[i]][0] = false;
        if(x[i] > 0 && f[x[i] - 1][0] == false) f[x[i] - 1][1] = false;
        if(x[i] < n - 1 && f[x[i] + 1][0] == false) f[x[i] + 1][2] = false;
    }
    int ans = 0;
    REP(i, n) REP(j, 3) if(f[i][j]) ans++;
    ans -= 2;
    cout << ans << endl;
