...
int main()
{
	...
	{
		if(cc=='0')b[0].insert(iii);
		else if(cc=='1') b[1].insert(iii);
		iii++;
	}
	cnt=0;
	while(1)
	{
		if(!SZ(b[0])&&!SZ(b[1]))break;
		if(!SZ(b[0]))OVER("-1");
		if(SZ(b[1])&&*b[0].begin()>*b[1].begin())OVER("-1");
		int cur=0,curr=0;
		set<int>::iterator currr=b[0].lower_bound(cur);
		if(currr==b[0].end())OVER("-1");
		vs[cnt].pb(*currr+1);
		b[0].erase(currr);
		while(SZ(b[0])&&SZ(b[1]))
		{
			set<int>::iterator currr=b[1].lower_bound(cur);
			if(currr==b[1].end())break;
			cur=*currr;
			set<int>::iterator currr2=b[0].lower_bound(cur);
			if(currr2==b[0].end())break;
			vs[cnt].pb(*currr+1);
			b[1].erase(currr);
			vs[cnt].pb(*currr2+1);
			cur=*currr2;
