...
tuple<modint<MOD>, modint<MOD>, modint<MOD>> dfs(int now, int back) {
    vector< tuple<modint<MOD>, modint<MOD>, modint<MOD>> > nexts;
    for (auto x : vertexs[now]) {
        if (x == back) continue;
        nexts.push_back(dfs(x, now));
    }
    if (nexts.size() == 0) {
        return make_tuple(modint<MOD>(1), modint<MOD>(1), modint<MOD>(0));
    }
    tuple<modint<MOD>,modint<MOD>,modint<MOD>> ans = make_tuple(modint<MOD>(1), modint<MOD>(1), modint<MOD>(1));
    for (auto x : nexts) {
        get<0>(ans) *= modint<MOD>(2) * (get<0>(x) + get<2>(x)) + get<1>(x);
        get<1>(ans) *= get<0>(x) + get<2>(x);
        get<2>(ans) *= get<0>(x) * modint<MOD>(2) + get<2>(x);
    }
    get<2>(ans) -= get<1>(ans);
    return ans;
}
void solve(){
    ...
    tuple<modint<MOD>, modint<MOD>, modint<MOD>> ans = dfs(0, -1);
    get<0>(ans) += get<2>(ans);
    get<0>(ans) -= modint<MOD>(1);
    cout << get<0>(ans) << endl;
}
#undef int
int main() {
