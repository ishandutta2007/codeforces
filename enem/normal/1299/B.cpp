...
ll xd(ll l,ll r,pll a[],ll n)
{
    return (a[(r)%n].F-a[(l)%n].F)*(a[(r)%n].F-a[(l)%n].F)+(a[(r)%n].S-a[(l)%n].S)*(a[(r)%n].S-a[(l)%n].S);
}
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        else
        {
            ...
            REP(i,1,n/2)
            {
                if(!((a[i].S-a[i-1].S)==-(a[i+n/2].S-a[i+n/2-1].S)&&(a[i].F-a[i-1].F)==-(a[i+n/2].F-a[i+n/2-1].F))) flag=0;
            }
            if(flag) cout<<"YES";
            else cout<<"NO";
        }
