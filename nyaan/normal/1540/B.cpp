...
void Nyaan::solve() {
  ...
  vvl a(N, vl(N, inf));
  rep(i, N) each(j, g[i]) a[i][j] = a[j][i] = 1;
  warshall_floyd(a);
  ...
  rep(i, N) rep(j, i) {
    ...
    auto path = Path(g, i, j);
    unordered_set<int> st;
    each(x, path) st.insert(x);

    auto dfs = [&](auto rc, int c, int p, int on_path) -> int {
      trc(i, j, c, p, on_path);
      if (c == i or c == j) {
        each(d, g[c]) {
          if (d == p) continue;
          rc(rc, d, c, st.find(d) != st.end());
        }
        return 0;
      }
      int res = 0;
