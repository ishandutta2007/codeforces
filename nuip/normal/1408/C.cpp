...
int main(){
  ...
  cout<<fixed<<setprecision(10);
	...
	while(T--){
		...
		cauto l=read();
		cauto a=readV(n);
		deque<Double> vs(all(a));
		int lv=1,rv=1;
		Double lx=0,rx=l,re=0;
		while(vs.size()){
			out(vs,lv,rv,lx,rx,re,1);
			cauto lt=(vs[0]-lx)/lv;
			cauto rt=(rx-vs.back())/rv;
			cauto t=min(lt,rt);
			out(lt,rt,t,1);
			lx+=lv*t;
			rx-=rv*t;
			...
			re+=t;
			while(vs.size() && lx+EPS>=vs[0]){
				++lv;
				vs.pop_front();
			}
			// out(vs.size() , rx-EPS<=vs.back(),1);
			// cout<<rx NL;
			// cout<<vs NL;
			while(vs.size() && rx-EPS<=vs.back()){
				vs.pop_back();
				++rv;
			}
		}
		if(lx<rx) re+=(rx-lx)/(lv+rv);
		cout<<re NL;
	}
  return 0;
}