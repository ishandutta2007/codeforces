...
int main(){
    ...
    rep(zz,T){
        ...
        if(allzero){
            cout << mod_pow((mod)2,n-1) << "\n";
            continue;
        }
        auto calc2 = [&](int a,int b)->mod{
            mod X = 0;
            rep(i,min(a,b)+1){
                X += comb(a+1,i+1) * comb(b+1,i+1);
            }
            return X;
        };
        int c = 0,d = 0;
