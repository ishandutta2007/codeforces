...
#define reps(i, n, s)       for (decltype(n) i = 0; i < n; i += (s))
#define replr(i, l, r)      for (decltype(r) i = (l); i < (r); ++i)
#define replrs(i, l, r, s)  for (decltype(r) i = (l); i < (r); i += (s))
#define rrep(i, n)          for (decltype(n) i = (n) - 1; i >= 0; --i)
#define rreps(i, n, s)      for (decltype(n) i = (n) - 1; i >= 0; i -= (s))
#define rreplr(i, l, r)     for (decltype(r) i = (r) - 1; i >= (l); --i)
#define rreplrs(i, l, r, s) for (decltype(r) i = (r) - 1; i >= (l); i -= (s))
...
int main() {
    ...
    while (t --> 0) {
        ...
        replr(i, 1, n) {
            ...
            auto it = s.find(prv);
            if (b[i] > prv) {
                if (++it == s.end() or b[i] <= *it) {
                    s.insert(b[i]);
                } else {
                    ok = false;
                }
            } else {
                if (it == s.begin() or *--it <= b[i]) {
                    s.insert(b[i]);
                } else {
                    ok = false;
                }
