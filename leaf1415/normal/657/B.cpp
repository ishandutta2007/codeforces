...
mint beki[200005];
map<P, ll> mp;
ll p[] = {1053818881, 1051721729, 1045430273, 1012924417, 1007681537};
...
void calc(ll M)
{
	...
	beki[0] = 1;
	rep(i, 1, n) beki[i] = beki[i-1] * mint(2);
	
	mint sum = 0;
	rep(i, 0, n) sum += mint(a[i]) * beki[i];
	
	rep(i, 0, n){
		mint b = -(sum - mint(a[i]) * beki[i]) / beki[i];
		rep(j, -1, 1) mp[P(i, b.x + j*M)]++;
	}
}
...
int main(void)
{
	...
	ll ans = 0;
	for(auto p : mp) if(p.first != P(n, 0) && abs(p.first.second) <= k && p.second == 5) ans++;
	outl(ans);
