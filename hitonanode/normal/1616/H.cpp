...
mint guchoku(vector<int> A, int x) {
    const int N = A.size();
...
mint solve(vector<int> A, int x) {
    ...
    const int degthres = floor_lg(x);
    const int D = 1 << degthres;
    // dbg(x);
    // dbg(D);
    vector<int> dictkeys;
    ...
    vector<mint> pow2s(202020, 1);
    FOR(i, 1, pow2s.size()) pow2s[i] = pow2s[i - 1] * 2;
    ...
    for (const auto &vec : mp) {
        ...
        // ret += pow2s[upper.size()] - 1;
        // ret += pow2s[lower.size()] - 1;

        auto rec = [&](auto &&self, vector<int> &upper, vector<int> &lower, int deg) -> mint {
            // if (upper.empty() or lower.empty()) return 0;
            if (upper.empty() and lower.empty()) return 0;

            dbg(make_pair(upper, lower));
            if (deg < 0) {
                // return (pow2s[upper.size()] - 1) * (pow2s[lower.size()] - 1);
                return pow2s[upper.size() + lower.size()] - 1;
            }
            ...
            if ((x >> deg) & 1) {
                ...
                mint tmp = 0;
                tmp += (pow2s[upper.size()] - 1) * (pow2s[up1.size()] - 1);
                tmp += (pow2s[lower.size()] - 1) * (pow2s[lo1.size()] - 1);
                tmp += self(self, upper, lower, deg - 1);
                tmp += self(self, up1, lo1, deg - 1);
                return tmp;
            }
        };
        ret += rec(rec, upper, lower, degthres - 1);
    }
    return ret;
}
...
int main() {
    ...
    //     vector<int> A;
    //     int D = rnd(1, 10);
    //     REP(i, N) A.push_back(rnd(1 << D));
    //     int x = rnd(1 << D);
    //     auto sol = solve(A, x);
    //     auto guc = guchoku(A, x);
    //     if (sol != guc) {
    //         dbg(A);
    //         dbg(x);
    //         dbg(sol);
