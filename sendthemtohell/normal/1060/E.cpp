...
struct way {
	ll cnt_ev = 0, cnt_odd = 0;
	ll sum_ev = 0, sum_odd = 0;


	ll swp() {
		ll rt = (sum_ev + sum_odd - cnt_odd) / 2 + cnt_odd;
		swap(cnt_ev, cnt_odd);
		swap(sum_ev, sum_odd);
		++cnt_odd;
		sum_ev += cnt_ev;
	...
	ll mul(way &oth) {
		ll add = (cnt_ev) * oth.cnt_odd + (oth.cnt_ev) * cnt_odd;
		ll mn = (sum_ev + sum_odd) * (oth.cnt_ev + oth.cnt_odd) + (oth.sum_ev + oth.sum_odd) * (cnt_ev + cnt_odd);
		return (mn - add) / 2 + add;
	}
	void add(way &oth) {
		cnt_ev += oth.cnt_ev;
...
const int N = 200000 + 57;
vector<int> g[N];
ll ans;

way dfs(int v, int p = -1) {
	int i = (g[v][0] == p ? 1 : 0);
	if (i >= sz(g[v])) {
		way w;
		ans += w.swp();
