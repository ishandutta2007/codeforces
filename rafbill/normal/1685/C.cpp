...
vii solve2(string s) {
  ...
  vii ans;
  ans.emplace_back(1,bi);
  ans.emplace_back(bi+1,s.size());
  return ans;
}

vii solve(string s){
  vii ans;
  auto solve = letrec([&](auto solve, li l, li r, li al, li ar, li dir) {
    li bi = l, xi = al;
    for(li i = (l); i <= (li)(r); ++i) {
      if(i == r) return;
      if(s[i] == ")("[dir]) {
    ...
    ans.emplace_back(bi+1,bj);

    solve(bi,bj-1,xj,xi,dir^1);
  });
  solve(0,s.size()-1,0,0,1);
  reverse(begin(ans), end(ans));
  if(ans.size() > 2) ans = solve2(s);
  return ans;
}
...
li brute(string s) {
  ...
  while(!Q.empty()) {
    ...
    if(isok(s)) return dist[s];
    for(li i = (0); i <= (li)(s.size()-1); ++i) for(li j = (i+1); j <= (li)(s.size()); ++j) {
      string t = s;
      reverse(begin(t)+i,begin(t)+j);
      if(!dist.count(t)) {
        dist[t] = dist[s]+1;
        Q.push(t);
      }
