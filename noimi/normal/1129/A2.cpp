...
signed main(){
    ...
    ll ma = 0;
    rep(i,n)chmax(ma,(ll)g[i].size());
    vector<ll> d(n);
    rep(i,n){
        if(g[i].size() < ma - 1 or g[i].size() == 0) continue;
        d[i] = LONG_LONG_MAX;
        for(auto e:g[i]){
            chmin<ll>(d[i],(e+n-i)%n);
        }
    }
    rep(i,n){
        ll nxt = 0;
        rep(j,n){
            if(g[j].size() == ma)
            chmax(nxt,d[j] + (j-i+n)%n);
            else if(g[j].size() == ma-1)
            chmax(nxt,d[j] + (j+n-i)%n -n);
        }
        cout << (ma-1) * n + nxt << " ";
    }
