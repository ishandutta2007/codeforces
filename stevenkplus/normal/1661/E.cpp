...
struct node {
   ...
   int leftmsk;
   int rightmsk;
   bool left; // is left separated?
   bool right; // is right separated?
   bool connected; // are both left & right separated, but joining one would join the other?
};
ostream& operator<<(ostream &os, const node &node) {
   return os << "{" << vector<int>({node.ccs, node.leftmsk, node.rightmsk, node.left, node.right, node.connected}) << "}";
}
node zero = {0, 0, 0, 0, 0, 0};

node operator+ (node a, node b) {
   int overlap = a.rightmsk & b.leftmsk;
   node res = zero;
   res.leftmsk = a.leftmsk;
   res.rightmsk = b.rightmsk;
   res.left = a.left;
   res.right = b.right;
   res.ccs = a.ccs + b.ccs;
   if (overlap == 5 && (a.right || b.left)) {
      res.ccs -= 2;
      if (a.connected && b.connected) res.connected = true;
      if (a.connected && !b.left) res.left = false;
      if (b.connected && !a.right) res.right = false;
   } else if (overlap) {
      res.ccs -= 1;
   }
   return res;
...
int main() {
   int n;
   cin >> n;
   vector<string> matinp(3);
   for (string &s: matinp) cin >> s;
   vector<vector<int>> mat(3, vector<int>(n));
   for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < n; ++j) {
         mat[i][j] = matinp[i][j] - '0';
      }
   }
   int size = 1;
   while (size < n) size *= 2;
   vector<node> seg(size * 2, zero);
   for (int i = 0; i < n; ++i) {
      int msk = mat[0][i] + mat[1][i] * 2 + mat[2][i] * 4;
      int ccs = __builtin_popcount(msk);
      if ((msk & 3) == 3) --ccs;
      if ((msk & 6) == 6) --ccs;
      seg[i + size] = {
         ccs,
