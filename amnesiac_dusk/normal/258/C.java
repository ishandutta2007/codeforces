...
{static class segtree
	...
	public static void main(String[] args) throws Exception
	{
		...
		for(int i:a)
			b[i]++;
		int hey=max(a);
		List<Integer>[]hola=new ArrayList[1+hey];
		for(int i=0; i<=hey; i++)
			hola[i]=new ArrayList<Integer>();
		for(int i=2; i<=hey; i++)
			for(int j=i; j<=hey; j+=i)
				hola[j].add(i);
		segtree seg=new segtree(b, new combiner()
				{
		...
		for(int i=1; i<=hey; i++)
		{
			...
			{
				ct++;
				int hol=seg.query(j, k);
				j=k;
				te=(te*(powm(ct,hol,mod)))%mod;
			}
			ct++;
			te=(((te)%mod)*(powm(ct,seg.query(i, 100001),mod)-powm(ct-1,seg.query(i, 100001),mod)))%mod;
			ans=(ans+te)%mod;
		}
		sop((ans+mod)%mod);
