...
void reducir(vi& criba){
    vi primos;
    for(int i = 2; i < criba.size(); i++){
        if(criba[i] == i) primos.eb(i);
    }
    for(int i = 2; i < criba.size(); i++){
        criba[i] = bb(primos,criba[i]);
    }
}
...
void c(){
    int n;
    cin>>n;
    vi criba = hacerCriba(n);
    reducir(criba);
    for(int i = 2; i <= n; i++) cout<<criba[i]+1<<" ";
    cout<<endl;
}
...
void d(){
    ...
    set<int> permitidos;
    F1(i, lim-1) {
        if(i != x) permitidos.insert(i);
    }
    int xorHastaI = 0;
    vi res;
    while(!permitidos.empty()){
        auto it = permitidos.begin();
        int primerPermitido = *it;
        primerPermitido = primerPermitido xor xorHastaI;
        res.eb(primerPermitido);
        xorHastaI = xorHastaI xor primerPermitido;
        int nuevoProhibido1 = xorHastaI;
        int nuevoProhibido2 = xorHastaI xor x;
        permitidos.erase(nuevoProhibido1);
        permitidos.erase(nuevoProhibido2);
    }
    cout<<res.size()<<endl;
