...
int dfs(int i) {
    if (children[i].empty()) {
        ...
        for (int nb : children[i]) {
            dfs(nb); s.clear();
            if (dq[nb].size()<mn) mn = dq[nb].size();
        }
        dq[i].assign(mn, 0);
        for (int nb : children[i]) F0R(k, mn) dq[i][mn-1-k] ^= dq[nb][dq[nb].size()-1-k];
        F0R(k, mn) s.insert(dq[i][k]);
        int ans = 0;
        while (s.find(ans) != s.end()) ans++;
        s.insert(ans);
        dq[i].pb(ans);
...
void solve() {
    cin >> n;
    children.clear(); roots.clear();
    children.rsz(n);
    F0R(i, n) {
        int p; cin >> p;
        if (p==0) roots.pb(i);
        else children[p-1].pb(i);
    }
    dq.clear(); dq.rsz(n);
    // vals.clear(); vals.rsz(n);
    s.clear();
    int ans = 0;
    for (int i : roots) ans ^= dfs(i);
    cout << (ans==0 ? "NO" : "YES") << nl;
    debug(dq);
}
