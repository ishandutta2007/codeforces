...
void solve(){
  ...
  vi cntl(n), cntr(n);
  for(li i = (n-2); i >= (li)(0); --i) {
    cntl[i] = G[i].find(i+1)!=G[i].end() ? 1+cntl[i+1] : 0;
  }
  for(li i = (1); i <= (li)(n-1); ++i) {
    cntr[i] = RG[i].find(i-1)!=G[i].end() ? 1+cntr[i-1] : 0;
  }
  ...
  for(li i = (y); i >= (li)(0); --i) {
    if(dp0[i]) {
      for(int j : RG[i+1]) if(j != i && i+1-j <= cntr[i]+2) {
          dp1[j] = 1;
        }
    }
    if(dp1[i]) {
  ...
  for(li i = (y); i <= (li)(n-2); ++i) {
    if(dp0[i]) {
      for(int j : G[i]) if(j != i+1 && j-i <= cntl[i+1]+2) {
          dp1[j-1] = 1;
        }
    }
    if(dp1[i]) {
  ...
  { li xl=0,xr=0;
    for(li i = (0); i <= (li)(y); ++i) if(dp0[i] && i <= cntl[0]) { xl++; }
    for(li i = (0); i <= (li)(y-1); ++i) if(dp1[i] && i <= cntl[0]) { xl++; s1=1; break; }
    for(li i = (y); i <= (li)(n-2); ++i) if(dp0[i] && n-2-i <= cntr[n-1]) { xr++; }
    for(li i = (y+1); i <= (li)(n-2); ++i) if(dp1[i] && n-2-i <= cntr[n-1]) { xr++; e1=1; break; }

    ans += xl*xr;
    if(y <= cntl[0] && n-2-y <= cntr[n-1]) ans -= 1;
  }



  { li xl=0,xr=0;
    for(li i = (0); i <= (li)(y); ++i) if(dp1[i] && i <= cntl[0]) { xl++; }
    for(li i = (0); i <= (li)(y); ++i) if(dp0[i] && i <= cntl[0]) { xl++; s2=1; break; }
    for(li i = (y); i <= (li)(n-2); ++i) if(dp1[i] && n-2-i <= cntr[n-1]) { xr++; }
    for(li i = (y); i <= (li)(n-2); ++i) if(dp0[i] && n-2-i <= cntr[n-1]) { xr++; e2=1; break; }

    ans += xl*xr;
  }

  { li xl=0,xr=0;
    for(li i = (0); i <= (li)(y); ++i) if(dp0[i]&&dp1[i] && i <= cntl[0]) { xl++; }
    if(s1&&s2) xl++;
    for(li i = (y); i <= (li)(n-2); ++i) if(dp1[i]&&dp0[i] && n-2-i <= cntr[n-1]) { xr++; }
    if(e1&&e2) xr++;
