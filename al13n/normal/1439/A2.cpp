...
vector<vector<array<pii,3>>> pre(int n,int m){
  vector<int> D(1<<(n*m),INF);
  vector<vector<array<pii,3>>> P(sz(D));
  D[0]=0;
  queue<int> qu;
  qu.push(0);
  while(!qu.empty()){
    ...
    forn(a,n-1){
      forn(b,m-1){
        forn(i,2){
          forn(j,2){
            int vv=v^(1<<((a+i)*m+b+j));
            forn(p,2) forn(q,2) vv^=1<<((a+p)*m+b+q);
            //dbg(n,m,v,a,b,i,j,vv);
            if(D[vv]!=INF) continue;
            D[vv]=D[v]+1;
            P[vv]=P[v];
            P[vv].emplace_back();
            int c=0;
            forn(p,2) forn(q,2) if(mp(p,q)!=mp(i,j)) P[vv].back()[c++]=mp(a+p,b+q);
            qu.push(vv);
          }
        }
      }
  ...
  forv(i,D){
    //if(D[i]>n*m) dbg(n,m,i,D[i]);
    assert(D[i]<=n*m);
  }
  return P;
}

map<pii,vector<vector<array<pii,3>>>> P;

vector<array<pii,3>> solve(vector<string> A){
  int n=sz(A),m=sz(A[0]);
  vector<array<pii,3>> R;
    for(int a=0;a+1<n;a+=2){
      for(int b=0;b+1<m;b+=2){
        int sa=(a+3==n)?3:2;
        int sb=(b+3==m)?3:2;
        int v=0;
        forn(i,sa) forn(j,sb) if(A[a+i][b+j]=='1') v|=1<<(i*sb+j);
        const auto& V=P.at(mp(sa,sb)).at(v);
        for(auto x:V){
          for(pii&p:x) p=mp(p.X+a,p.Y+b);
          R.pb(x);
        }
      }
    }
  ...
  for(int n=2;n<=30;++n){
    for(int m=2;m<=30;++m){
      forn(qq,10000){
        ...
        forn(i,n) forn(j,m) A[i][j]='0'+rand()%2;
        auto A0=A;
        auto R=solve(A);
        for(auto r:R) for(pii p:r) A.at(p.X).at(p.Y)^=1;
        if(A!=vector<string>(n,string(m,'0'))){
          dbg(n,m,A0,R,A);
          return;
        }
      }
...
int main(int argc, char **argv){
  ...
  for(pii p:{mp(2,2),mp(2,3),mp(3,2),mp(3,3)}) P[p]=pre(p.X,p.Y);
