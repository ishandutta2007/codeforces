...
int check(int mid) {
	int mxl=-1e9,mnr=1e9; 
	For(i,1,n) {
		ckmax(mxl,x[i]-(mid-t[i])); 
		ckmin(mnr,x[i]+(mid-t[i])); 
	}
	if(mxl>mnr) return -1e9-10; 
	return mxl; 
} 
void works() {
	n=read();
	For(i,1,n) x[i]=read()*2; 
	int l=0,r=1e9; 
	For(i,1,n) t[i]=read()*2,ckmax(l,t[i]); 
	int ans=0; 
	while(l<=r) {
		int mid=(l+r)>>1; 
		if(check(mid)>=-1e9) r=mid-1,ans=mid; 
		else l=mid+1; 
	}
	printf("%.10lf\n",check(ans)*0.5); 
}
