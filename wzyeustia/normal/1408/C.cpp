...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ll n, l; cin >> n >> l;
        vector<ld> arr(n+2), pre(n+2), suf(n+2);
        vector<bool> reached(n+2);
        FOR(i, 1, n) {cin >> arr[i];}
        arr[0] = 0;
        arr[n+1] = l;
        ...
        ROF(i, n, 0) {
            suf[i] = suf[i+1] + (arr[i+1] - arr[i]) / (n+1-i);
        }

        FOR(i, 0, n+1) {
            reached[i] = pre[i] < suf[i];
        }
        ...
        FOR(i, 0, n) {
            if (reached[i] && (!reached[i+1])) {
                ...
                ld left = arr[i] + (i+1) * (done - pre[i]);
                ld right = arr[i+1];

                ld ans = done + (right - left) / (n+2);
