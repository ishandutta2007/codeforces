...
ll ge(int cnt,ll X,ll ng){
	ll ok=0;
	if(cnt>=10)ng=min(ng,20ll);
	while(ok+1!=ng){
		ll amid((ok+ng)/2);
		ld num=powl(amid,cnt);
		if(num>2*X){
			ng=amid;
		}else{
			ll val=1;
...
int main(){
	...
	int N;cin>>N;
	vector<map<ll,int>>mps(61);
	for(int i=2;i<60;++i){
			mps[i]=soinnsuu(i);
	}
	vector<vector<ll>>sqs(62);
	for(ll x=2;x<=1e6;++x){
		ll num=1;
		ld numd=1;
		for(int i=0;true;++i){
	...
	while(N--){
		...
		for(int i=2;i<60;++i){
			...
			ll xx=-1;
			if(i==2){	
				xx=ll(powl(X,1.0l/i)+1e-16);

			}else{
				xx=upper_bound(sqs[i].begin(),sqs[i].end(),X)-sqs[i].begin();
				xx++;
			}
			//ld yy=powl(X,1.0l/i);
			//WHATS(yy)
