...
main() {
    ...
    vv(int, s, min(n, m), max(n, m));
    if(n > m) {
        rep(i, n) rep(j, m) s[j][i] = a[i][j] - '0';
        swap(n, m);
    } else
        rep(i, n) rep(j, m) s[i][j] = a[i][j] - '0';
    vv(int, dp, m, 1 << n, 1e8);
    rep(i, 1 << n) {
        dp[0][i] = 0;
        rep(j, n) {
            if(((i & (1 << j)) > 0) ^ s[j][0]) dp[0][i]++;
        }
    }
    bool ok[1 << n][1 << n];
    rep(j, 1 << n) {
        rep(k, 1 << n) {
            vi p, q;
            bool flag = true;
            rep(x, n) p.eb((j & (1 << x)) > 0), q.eb((k & (1 << x)) > 0);
            rep(x, n - 1) {
                if(~(p[x] + p[x + 1] + q[x] + q[x + 1]) & 1) flag = false;
            }
            ok[j][k] = flag;
        }
    }
