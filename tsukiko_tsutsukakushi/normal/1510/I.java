...
public class I {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] wins = new int[N];
        int selfwin = 0;
        boolean[] yaochos = new boolean[N];
        Arrays.fill(yaochos, true);
        int yaochowin = 0;
        double yaochobetzero = 0;
        ...
        for (int n = 0; n < M; n++) {
            ...
            for (int m = 0; m < N; m++) {
                if (pred.charAt(m) == '0') {
                    zero++;
                    if (yaochos[m]) {
                        yaochozero += Math.pow(0.8, yaochowin - wins[m]);
                    }
                } else {
                    one++;
                    if (yaochos[m]) {
                        yaochoone += Math.pow(0.8, yaochowin - wins[m]);
                    }
                }
            }
            yaochobetzero = (double) (yaochozero) / (yaochozero + yaochoone);
            boolean bet = yaochowin - 10 > selfwin ? sig(yaochobetzero) < random.nextInt(1000000) / 1000000.0
                    : zero < one;
            if (bet) {
                cp.println(1);
            } else {
            ...
            cp.flush();
            int res = sc.nextInt();
            if (bet && res == 1 || !bet && res == 0) {
                selfwin++;
            }
            int max = -1;
            for (int m = 0; m < N; m++) {
                if (pred.charAt(m) == res + '0') {
                    wins[m]++;
                }
                if (wins[m] > max) {
                    max = wins[m];
            ...
            yaochowin = max;
            for (int m = 0; m < N; m++) {
                yaochos[m] = wins[m] > max * 0.95 - 5;
            }
        }
        if ((M - selfwin) > (M - yaochowin) * 1.3 + 100) {
            cp.println(1 / 0);
        }
        cp.close();
    }
    ...
    static double sig(double val) {
        return 1.0 / (1 + Math.pow(Math.E, -(val - 0.5) * 10));
    }

    static public final class Random {
