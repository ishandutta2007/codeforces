...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        vector<vector<ll>> dp(n+3, vector<ll>(2, INT32_MAX));
        dp[0][1] = 0; // next is friend -> 1

        REP(i, n) {
            chkmin(dp[i+1][0], dp[i][1]+arr[i]);
            chkmin(dp[i+2][0], dp[i][1]+arr[i]+arr[i+1]);
            
            chkmin(dp[i+1][1], dp[i][0]);
            chkmin(dp[i+2][1], dp[i][0]);
        }
