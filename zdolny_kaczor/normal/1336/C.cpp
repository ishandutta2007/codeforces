...
#endif
//#STAY AT HOME
const int mod = 119 << 23 | 1, nax = 6010;
int dp[nax][nax];
...
int tlen, slen;
char s[nax], t[nax];
bool matches(char c, int pos) {
	if (pos < 0 || pos >= slen) return false;
	return t[pos] == 0 || t[pos] == c;
}
int main() {
	scanf("%s%s", s, t);
	...
	REP(l, slen) {
		char now = s[l];
		REP(p, slen + 1) {
			debug(arr2(dp, l, p), range(s, 0, l), range(t, p, p + l));
			if (matches(now, p + l))
				add(dp[l + 1][p], dp[l][p]);
			if (matches(now, p - 1))
				add(dp[l + 1][p - 1], dp[l][p]);
		}
	}
	for (int len = tlen; len <= slen; ++len) {
		debug(arr(dp, len, 0));
		ans += dp[len][0];
	}
	printf("%lld\n", fix(ans, mod));
