...
int main(){
  ...
  while(t--){
    ...
    modint ret=0;
    auto f=[&](ll x)->modint{
      ll pl=clamp<ll>(r-x,0,n);
      ll pr=clamp<ll>(n-(x+l-1),0,n);
      if(pl+pr<n)return 0;
      modint tmp=comb.C(pl-(n-pr),n/2-(n-pr));
      if(n&1)tmp+=comb.C(pl-(n-pr),n/2+1-(n-pr));
      return tmp;
    };
    ll stl=max(1LL,min(-l,r)-5*n);
    ll str=max(1LL,min(-l,r)+5*n);
    ret+=f(1)*(stl-1);
    rep(x,stl,str){
      ret+=f(x);
    }
