...
int main() {
    ...
    }
    {
        ...
        if (size_int(deg1) == 2 and size_int(deg2) == n - 2) {
            print(n - 1);
            print(deg1[0] + 1, deg1[1] + 1, deg2[0] + 1);
            return 0;
        }
    }
    vec<int> dp(n, 0);
    ...
    auto dfs2 = [&](auto self, int u, int p) -> void {
        ...
        rrep(i, sz) {
            int v = g[u][i];
            if (v != p) {
                dp_up[v] = max(max(lmax[i], rmax) + 1, dp_up[u]) + 1;
                self(self, v, u);
                chmax(rmax, dp[v]);
            }
        }
        if (sz >= 3) {
            vec<pair<int, int>> dist(sz);
            rep(i, sz) {
                int v = g[u][i];
                if (v == p) {
                    dist[i] = {dp_up[u], v};
