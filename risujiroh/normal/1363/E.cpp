...
int main() {
  ...
  for (int v : g.vs) {
    if (v) {
      a[v] = min(a[v], a[g.pv[v]]);
    }
  }
  for (int v : g.vs) {
    if (v) {
      g.es[g.pe[v]].w = a[g.pv[v]] - a[v];
      DEBUG(v, a[g.pv[v]] - a[v]);
    }
  }
  g.build_hld();
  auto res = (long long)size(x) * a[0];
  vector<int> cumx(n + 1), cumy(n + 1);
  for (int v : x) {
    ++cumx[g.in[v]];
  }
  for (int v : y) {
  ...
  for (int v = 1; v < n; ++v) {
    long long t = min(cumx[g.in[v]] - cumx[g.out[v]], cumy[g.in[v]] - cumy[g.out[v]]);
    DEBUG(v, t);
    res -= t * g.es[g.pe[v]].w;
  }
  res *= 2;
