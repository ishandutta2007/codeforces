// Unusual Minesweeper (https://codeforces.com/contest/1619/problem/G)
...
int main() {
    ...
    while (t--) {
        int n, k; read(n, k);
        map<int, vector<pair<int, int>>> by_x, by_y;
        vector ti(n, 0);
        rep(i, n) {
            int x, y; read(x, y, ti[i]);
            by_x[x].emplace_back(y, i);
            by_y[y].emplace_back(x, i);
        }
        ...
        for (auto& [_, ys] : by_x) {
            sort(all(ys));
            rep(i, 1, sz(ys)) {
                if (ys[i].first - k <= ys[i - 1].first) {
                    int mn = min(ti[dsu.find(ys[i].second)], ti[dsu.find(ys[i - 1].second)]);
                    dsu.join(ys[i].second, ys[i - 1].second);
                    ti[dsu.find(ys[i].second)] = mn;
                }
            }
        }
        for (auto& [_, xs] : by_y) {
            sort(all(xs));
            rep(i, 1, sz(xs)) {
                if (xs[i].first - k <= xs[i - 1].first) {
                    int mn = min(ti[dsu.find(xs[i].second)], ti[dsu.find(xs[i - 1].second)]);
                    dsu.join(xs[i].second, xs[i - 1].second);
                    ti[dsu.find(xs[i].second)] = mn;
                }
            }
        }
