                ...
                long long m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500003],d[500003];vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
    ...
    //                }
                    ...
                    cin>>s;ans=1;n=s.size();if(n==1&&(s[0]=='m'||s[0]=='w'))return cout<<0,0;a[0]=1;
                    for(int i=0;i<n-1;i++)
                    {

                        if(i==0){if((s[i]=='n'&&s[i+1]=='n')||(s[i]=='u'&&s[i+1]=='u'))ans*=2;x++;a[x]=ans;}
                        else
                        {

                            if((s[i]=='n'&&s[i+1]=='n'&&s[i-1]=='n')||(s[i]=='u'&&s[i+1]=='u'&&s[i-1]=='u')){ans+=a[x-1];x++;a[x]=ans;}
                            else if((s[i]=='n'&&s[i+1]=='n')||(s[i]=='u'&&s[i+1]=='u')){ans*=2;x++;a[x]=ans;}
                        }
                       ans%=mod;
                        if(s[i]=='m'||s[i]=='w'||s[i+1]=='m'||s[i+1]=='w')return cout<<0,0;
                    }
