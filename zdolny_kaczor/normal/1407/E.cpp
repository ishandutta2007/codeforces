...
void solve() {
	...
	REP(_, m) {
		int a, b, t;
		scanf("%d%d%d", &a, &b, &t);
		graf[b].emplace_back(a, t);
	}
	memset(dist, -1, sizeof(dist));
	queue<tuple<int, int, int>> bfs_queue;
	bfs_queue.push(make_tuple(n, 0, 0));
	bfs_queue.push(make_tuple(n, 0, 1));
	while (!bfs_queue.empty()) {
		auto [x, d, t] = bfs_queue.front();
		bfs_queue.pop();
		touched[x][t] = true;
		if (touched[x][!t] && !done[x]) {
			done[x] = true;
			dist[x] = d;
			type[x] = t;
			for (auto [v, c] : graf[x]) {
				bfs_queue.emplace(v, d + 1, c);
			}
		}
	}
	printf("%d\n", dist[1]);
	for (int i = 1; i <= n; ++i) if (!done[i] && touched[i][0]) type[i] = 1;
