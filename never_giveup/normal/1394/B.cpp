...
const int maxn = 2e5 + 100, maxw = 1e4 + 100, inf = 1e9 + 100, sq = 450, LG = 18, mod = 1e9 + 933;
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        sort(e[i].begin(), e[i].end(), [&](int x, int y) {return edg[x].fr < edg[y].fr;});
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 1; j <= k; j++)
            for (int c = 1; c <= k; c++)
                if (q[j][c] > 1)
                    bad[j][c] = 1;
                else if (q[j][c] == 1 && !bad[j][c])
                    g.push_back({j, c});
        sort(g.begin(), g.end());
        for (int x = 0; x < g.size(); x++)
            for (int y = x + 1; y < g.size(); y++)
                bu[g[y].fr][g[y].sc][g[x].fr][g[x].sc] = 1;
    }
    go();
    cout << ans << '\n';
}