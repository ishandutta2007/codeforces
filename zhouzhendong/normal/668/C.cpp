...
int main(){
	...
	For(i,1,n-1){
		LD aa=1-x[i+1]+m[i];
		LD bb=m[i];
		LD cc=sqrt(max(aa*aa-4*bb,0.0l));
		LD pp=(aa+cc)/2,qq=(aa-cc)/2;
		if (pp<p[i-1]-Eps||qq<q[i-1]-Eps)
			swap(pp,qq);
//		assert(pp>p[i-1]-Eps&&qq>q[i-1]-Eps);
		p[i]=pp,q[i]=qq;
	}
	Fod(i,n,1){
		p[i]-=p[i-1];
	...
	For(i,1,n){
		printf("%.7lf ",(double)p[i]);
	}
	puts("");
	For(i,1,n){
		printf("%.7lf ",(double)q[i]);
	}
	return 0;
}