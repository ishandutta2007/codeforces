...
S f(S x, S y) { return S(max(x.fi, y.fi), max(x.se, y.se)); }
S g(F x, S y) {
    if(x == -1) return y;
    return S(-inf<ll>, -inf<ll>);
}
F h(F x, F y) {
    if(x == -1) return y;
    return x;
}
S e() { return S(-inf<ll>, -inf<ll>); }
F id() { return -1; }
...
int main() {
    ...
    VEC(ll, b, n);
    rep(i, n) a[i]--;
    atcoder::lazy_segtree<S, f, e, F, g, h, id> seg(n + 1);
    seg.set(n, S(0, 0));
    rep(i, n) {
        ll nxt = seg.all_prod().fi;
        seg.set(a[i], S(nxt + b[i], nxt));
        ll ma = seg.prod(a[i], n + 1).se;
        seg.set(a[i], S(ma + b[i], ma));
        seg.apply(a[i] + 1, n + 1, 0);
    }
    cout << seg.all_prod().fi << endl;
}