...
int Solve(){
	...
	static vc<int> order;
	For(i,n) order.Eb(i);
	Forx(i,1,order.size()-1)
		swap(order[i],order[Rand()%i]);
	// Print(order);

	auto ForEachPrimeFactors = [&]( ll x , function<void(ll)> Processor ){
		int sqt = sqrt(x);
		while( 1LL*sqt*sqt <= x ) ++sqt;
		while( 1LL*sqt*sqt >  x ) --sqt;
		Forx(i,2,sqt){
			if(x%i) continue;
			Processor(i);
	...
	constexpr int TIME_LIMIT = CLOCKS_PER_SEC*2.4;
	for( auto pos : order ){
		auto Func = [&]( ll x ){
			if( x == 1 ) return;
			ll tans = 0;
			For(i,n){
				// O(min(a[i]%x,x-a[i]%x));
				tans += a[i] <= x ? x-a[i] : min(a[i]%x,x-a[i]%x);
				if( tans >= ans ) return;
			}
			MyMin(ans,tans);
		};
		ForEachPrimeFactors(a[pos],Func);
		ForEachPrimeFactors(a[pos]+1,Func);
		if( a[pos] != 1 )
			ForEachPrimeFactors(a[pos]-1,Func);
		if( (~pos&7) and clock() > TIME_LIMIT ) break;
	}
