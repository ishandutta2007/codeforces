...
fun Graph.hasCycle():Int{
    //find directed cycle if graph is directed, undirected cycle if undirected
    val visited = BooleanArray(n)
    val onstack = BooleanArray(n)
    var comps = 0
    var havecycles = 0
    var hascycle = false
    fun visit(v:Int, p:Int){
        onstack[v] = true
        this.NS(v){
                w->
            if(!directed && w == p) return@NS
            if(onstack[w]){
                hascycle = true
                return
            }
...
fun main(){
    solve.cases{
        ...
        val A = getstr
        val B = getstr
        val have = Array(20){BooleanArray(20)}
        for(i in 0 until n){
            val from = A[i].code
            val to = B[i].code
            have[from][to] = true
        ...
        fun solve(arr:IntArray):Int {
            val n = arr.size
            val DP = IntArray( 1 shl n){nlarge}
            DP[0] = 0
            for(m in 0 until (1 shl n)){
                for(i in 0 until n){
                    if(!m.has(i)){
                        val new = m xor ( 1 shl i)
                        val newcost = DP[m] +
                        if(m.isSupersetOf(arr[i])){
                            1
                        }else{
                            0
                        }
                        DP[new] = maxOf(DP[new], newcost)
                    }
                }
            }
            return n + (n - DP.last()) -1
        }

        val dsu = DisjointUnionSets(20)
        for(i in 0 until 20){
            for(j in 0 until 20){
                if(have[i][j]){
                    dsu.union(i,j)
        ...
        dsu.eachComponent { r->
            ...
            val compress = IntArray(20)
            for((i,v) in who.withIndex()){
                compress[v] = i
            }
            val ask = IntArray(who.size)
            for(i in 0 until who.size){
                for(j in 0 until who.size){
                    if(i == j) continue
                    val a = who[i]
                     val b = who[j]
                    if(have[a][b]){
                        ask[i] = ask[i] xor ( 1 shl j)
                    }
                }
            }
            val extra = solve(ask)
...
smallest number of edges for which the resultant grahp is the same

definitely need some greedy?

middle pivot? no
clean invariant? probably not

greedy 1: need to be a path

- 1 2
2 3 /2 4

claim: must be a full path

proof not complete, but good enough


all occur once and the occur twice
