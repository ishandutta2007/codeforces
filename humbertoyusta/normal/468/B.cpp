...
int32_t main(){
    ...
    for(int i=1; i<=n; i++){
        if( id[a-arr[i]] )
            sat2.implication(sat2.neg(i-1),sat2.neg(id[a-arr[i]]-1));
        else
            sat2.implication(sat2.neg(i-1),i-1);
        if( id[b-arr[i]] )
            sat2.implication((i-1),(id[b-arr[i]]-1));
        else
            sat2.implication(i-1,sat2.neg(i-1));
    }
