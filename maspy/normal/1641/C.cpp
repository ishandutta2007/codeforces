...
struct Mono {
  ...
  static X op(X x, X y) {
    ...
    if (i1 == -1) return y;
    if (j1 == -1) return x;
    for (auto&& [y, j]: vc<pi>({{y1, j1}, {y2, j2}})) {
      if (x2 >= y) continue;
      tie(x2, i2) = mp(y, j);
      if (x1 >= y) continue;
      swap(x1, x2);
    ...
    return mt(x1, i1, x2, i2);
  }
  static constexpr X unit = mt(-1, -1, -1, -1);
  static constexpr bool commute = true;
};
...
void solve() {
  ...
  // 0 
  auto ZERO = seg.get_all();
  // print(ZERO);

  vc<tuple<ll, ll, ll, ll>> seg_raw(N);
  FOR(i, N) {
    ll x = ZERO[i];
    seg_raw[i] = mt(x, i, 0, -1);
  }
  SegTree<Mono> seg2(seg_raw);
  ...
  FOR(q, Q) {
    auto [t, l, r] = query[q];
    if (t != 1) continue;
    auto [x1, i1, x2, i2] = seg2.prod(l, r);
    assert(x1 == INF);
    if (x2 == INF) continue;
    chmax(x2, q);
    ONE[x2].eb(i1);
  ...
  vi A(N);
  FOR(q, Q) {
    for (auto&& i: ONE[q]) A[i] = 1;
    auto [t, l, r] = query[q];
    if (t != 2) continue;
    ll who = l;
    if (ZERO[who] <= q) print("NO");
    elif (A[who] == 1) print("YES");
    else print("N/A");
  }
}
