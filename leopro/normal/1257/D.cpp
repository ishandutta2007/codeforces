...
struct Hero {
    ...
    friend istream &operator>>(istream &is, Hero &h) {
        is >> h.p >> h.s;
        return is;
    }

    const bool operator<(const Hero &hero) const {
        return p < hero.p;
    }
};
...
void solve() {
    ...
    for (int i = 0; i < m; ++i) power[heroes[i].s] = max(power[heroes[i].s], heroes[i].p);
    int max_power = power[n];
    for (int i = n - 1; i >= 0; --i) {
        if (power[i] > max_power) max_power = power[i]; else power[i] = max_power;
    }
    int days = 0;
    for (int j = 0; j < n;) {
        int k = j;
        max_power = a[j];
        if (power[1] < max_power) answer(-1);
        for (; k < n; ++k) {
            max_power = max(max_power, a[k]);
            if (power[k - j + 1] < max_power) break;
        }
        j = k;
        ++days;
    }
