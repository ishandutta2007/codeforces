...
public class CFContest {
    ...
    public static class Segment implements Cloneable {
        ...
        public static int floor(int l, int r, Segment old, Segment now, int v) {
            if (l > v || now.cnt <= old.cnt) {
                return -1;
            }
            if (l == r) {
                return l;
            ...
            int m = (l + r) >> 1;
            int res = -1;
            if (now.right.cnt > old.right.cnt) {
                res = floor(m + 1, r, old.right, now.right, v);
            }

            if (res == -1) {
                res = floor(l, m, old.left, now.left, v);
            }
            return res;
        }

        public static int ceil(int l, int r, Segment old, Segment now, int v) {
            if (r < v || now.cnt <= old.cnt) {
                return -1;
            }
            if (l == r) {
                return l;
            ...
            int m = (l + r) >> 1;
            int res = -1;
            if (now.left.cnt > old.left.cnt) {
                res = ceil(l, m, old.left, now.left, v);
            }
            if (res == -1) {
                res = ceil(m + 1, r, old.right, now.right, v);
            }
            return res;
        }
