...
const li INFTY = 1ll<<50;

void solve(){
  li n,m; cin>>n>>m;
  vi A(n+2);
  A[0] = -INFTY;
  for(li i = 0; i < (li)(n); ++i) cin>>A[i+1];
  A[n+1] = INFTY;
  n += 2;
  sort(begin(A), end(A));
  ...
  vi L(m), R(m);
  for(li i = 0; i < (li)(m); ++i) cin>>L[i]>>R[i];

  vvi at(n);

  for(li i = 0; i < (li)(m); ++i) {
    auto it = SA.lower_bound(L[i]);
    li j = it->second;
    if(A[j] <= R[i]) continue;
    at[j].emplace_back(i);
  }
  ...
  for(li i = (1); i <= (li)(n-1); ++i) {
    int na = at[i].size();
    sort(begin(at[i]), end(at[i]), [&](int a, int b) { return make_tuple(L[a],R[a]) < make_tuple(L[b],R[b]); });
    vi to = at[i];
    sort(begin(to), end(to), [&](int a, int b) { return R[a] < R[b]; });

    vii at2;

    int k = 0;
    at2.emplace_back(A[i-1], k == na ? A[i] : R[to[k]]);
    for(int j : at[i]) {
      taken[j] = 1;
      while(k < na && taken[to[k]]) k++;
      at2.emplace_back(L[j], k == na ? A[i] : R[to[k]]);
    }

    vii pairs;
    for(auto j : at[i]) pairs.emplace_back(L[j],R[j]);

    for(auto lr : at2) {
      li l,r; tie(l,r) = lr;
      assert(A[i-1] <= l && l <= r && r <= A[i]);
      smin(dp0[i], (dp0[i-1] + 2*(l-A[i-1])) + (A[i]-r));
      smin(dp0[i], (dp1[i-1] + (l-A[i-1])) + (A[i]-r));
      smin(dp1[i], (dp0[i-1] + 2*(l-A[i-1])) + 2*(A[i]-r));
      smin(dp1[i], (dp1[i-1] + (l-A[i-1])) + 2*(A[i]-r));
    }
  }
