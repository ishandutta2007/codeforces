...
void dfs(int u,int fa){
	...
	f[u]=i64(f[u])*(u==1?sz[u]:cnt+1)%mod;
	f[u]=i64(f[u])*per[cnt]%mod;
}
int main(){
	scan(n);
	for(int i=1,u,v;i<n;++i) scan(u,v),adj[u].pb(v),adj[v].pb(u);
	per[0]=1;
	for(int i=1;i<=n;++i) per[i]=i64(per[i-1])*i%mod;
	dfs(1,0);
	print(f[1]),putc('\n');
}