...
void solve() {
  ...
  FOR(i, N) {
    STR(S);
    auto H = RH.build(S);
    ss[len(S)].eb(RH.query(H, 0, len(S)));
  }
  FOR(x, 1 << 20) sort(all(ss[x]));
  ...
  FOR(i, M) {
    ...
    bool ok = [&]() {
      FOR(i, n) {
        ll left = RH.query(H, 0, i);
        ll right = RH.query(H, i + 1, n);
        FOR(j, 3) {
          if (j == S[i] - 'a') continue;
          ll x = RH.combine(left, single[j], 1);
          x = RH.combine(x, right, n - (i + 1));
          if (binary_search(all(ss[n]), x)) return true;
        }
      }
      return false;
    }();
