...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int k = 0; k < 4; k++)
        {
            int nx = x[i] + cx[k], ny = y[i] + cy[k];
            if (mmid.find({nx, ny}) == mmid.end())
            {
                mmid[{nx, ny}] = x.size();
                x.push_back(nx);
                y.push_back(ny);
    ...
    vector<int> q;
    vector<int> source(g.size());
    for (int i = 0; i < x.size(); i++)
    {
        if (i >= n) dist[i] = 0, source[i] = i, q.push_back(i);
        for (int k = 0; k < 4; k++)
        {
            int nx = x[i] + cx[k], ny = y[i] + cy[k];
            if (mmid.find({nx, ny}) != mmid.end())
            {
                g[i].push_back(mmid[{nx, ny}]);
            }
        }
    }
