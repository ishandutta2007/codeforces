...
int main(){
	...
	for(int i = N - 2; i >= 0; i--) invfact[i] = (i + 1) * invfact[i + 1];
	assert(invfact[0] == 1);
	...
	while(t--){
		...
		for(int i = 1; i <= n; i++, pw *= 2){
			P[i] = mint(i + 1) * mint(k) * pw;
		}
		for(int i = 0; i <= n; i++){
			vec1[i] = invfact[i].val();
			vec2[n - i] = (P[n - i] * fact[i]).val();
		}
		// trace(vec1, vec2);
		mul_big_mod(vec1, vec2);
		mint ans = mint(vec1[m]) * invfact[n - m] * mint(2).inv().pow(n);
		cout << ans.val()<<endl;
	}
}