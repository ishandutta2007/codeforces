...
Mint way(int a, int b, int c, int d) {
    return nCk(abs(a-c) + abs(b-d), abs(a-c));
}
...
Mint solve(int N, int M) {
	...
	Mint sum = 0;
	for (int i=1; i+1<N; i++) {
	    sum += way(0, M, i-1, j) * way(i, j+1, N, M);
	    Mint tmp = sum * way(0, 0, i+1, j-1) * way(i+2, j, N, 0);
	    ans += tmp * 2;
	}
    }
    return ans;
