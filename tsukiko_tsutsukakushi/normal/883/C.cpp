...
void solve() {
    ...
    INT(a2, t2, p2);
    ll ans = INF;
    for(int c1 = 0; t1 * c1 <= T and c1 <= f ; ++ c1) {
        int resf = max(0LL, f - c1);
        int rest = T - t1 * c1;
        if(resf * t0 <= rest) {
            chmin(ans, (c1 + a1 - 1) / a1 * p1);
        }
        if(t2 >= t0) continue;
        ld need2 = (t0 * resf - rest) / (t0 - t2);
        debug(c1, need2);
        debug(c2);
        for(int i = max((int)need2 - 4, 0LL); i <= need2 + 4; ++ i) {
            debug(i, i * t2 + max(0LL, resf - i) * t0, rest);
            if(i * t2 + max(0LL, resf - i) * t0 > rest) continue;
            chmin(ans, (c1 + a1 - 1) / a1 * p1 + (i + a2 - 1) / a2 * p2);
        }
    }
    if(ans == INF) ans = -1;
    out(ans);
