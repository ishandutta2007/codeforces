...
void dfs(ll v, vll& mn, vll& mx, vll& num, vvll& g, vll& dev) {
    if (g[v].size() == 0) {
        mn[v] = dev[v]; mx[v] = dev[v];
        num[v] = 1;
        return;
    }
...
int main() {
    ...
    rep(i, 2) {
        ...
        mn[i].resize(a); mx[i].resize(a); num[i].resize(a);
        
        dfs(0, mn[i], mx[i], num[i], g[i], dev[i]);
    }
    ...
    repn(i, N) {
        dp[i] = dp[i - 1];
        rep(k, 2)rep(j,mx[k].size()) {
            if (mx[k][j] == i - 1) {
                dp[i] = max(dp[i], dp[mn[k][j]] + num[k][j]);
            }
        }
    }
