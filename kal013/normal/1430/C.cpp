...
void solve(){
	...
	while(odd.size() + even.size() > 1){
		int odda = -1, oddb = -1, suma = -1, evena = -1, evenb = -1, sumb = -1;
		int final = -1;
		if (odd.size() == 1 && even.size() == 1){
			...
			evena = even.top();
			even.pop();
			final = (odda + evena + 1) / 2;
			moves.push_back({odda, evena});
		}
		else if (odd.size() <= 1){
			evena = even.top();
			even.pop();
			evenb = even.top();
			even.pop();
			final = (evena + evenb + 1) / 2;
			moves.push_back({evenb, evena});
