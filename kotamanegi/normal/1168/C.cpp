...
int now[400000];
int ans[400000];
queue<int> thinks[20][20];
void adds(int itr, int nowa) {
    REP(q, 20) {
        if (now[itr] & (1 << q)) continue;
        thinks[nowa][q].push(itr);
    }
}
void fetch(int a,int b) {
    while (thinks[a][b].empty() == false) {
        int nows = thinks[a][b].front();
        thinks[a][b].pop();
        if (now[nows] & (1 << b)) continue;
        now[nows] |= (1 << b);
        adds(nows, b);
    }
}
void solve() {
    ...
    REP(i, query) {
        ...
        checks.push_back(make_pair(b, i));
    }
    sort(ALL(queries));
    reverse(ALL(queries));
    sort(ALL(checks));
    reverse(ALL(checks));
    REP(i, n) {
        while (queries.empty() == false and get<0>(queries.back()) == i) {
            now[get<2>(queries.back())] = inputs[i];
            REP(q, 20) {
                if ((1 << q) & inputs[i]) adds(get<2>(queries.back()), q);
            }
            queries.pop_back();
        }
        while (checks.empty() == false and checks.back().first == i) {
            if (now[checks.back().second] & inputs[i]) {
                ans[checks.back().second] = 1;
            }
            else {
                ans[checks.back().second] = 0;
            }
            checks.pop_back();
        }
        REP(q, 20) {
