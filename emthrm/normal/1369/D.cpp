...
int main() {
  const int X = 2000000;
  vector dp(X + 1, vector(2, ModInt(0)));
  dp[3][true] = 1;
  FOR(i, 4, X + 1) {
    dp[i][true] = dp[i - 1][false] + dp[i - 2][false] * 2 + 1;
    dp[i][false] = *max_element(ALL(dp[i - 1])) + *max_element(ALL(dp[i - 2])) * 2;
  }
  int t; cin >> t;
  while (t--) {
    int n; cin >> n;
    cout << *max_element(ALL(dp[n])) * 4 << '\n';
  }
  return 0;

  // vector<vector<int>> graph{{}};
  // FOR(i, 1, n) {
  //   int m = graph.size(), idx = m;
  //   REP(j, m) {
  //     if (graph[j].empty()) {
  //       graph[j].emplace_back(idx++);
  //       graph.emplace_back();
  ...
  //   }
  // }
  // REP(i, graph.size()) for (int e : graph[i]) cout << i << ' ' << e << '\n';
}