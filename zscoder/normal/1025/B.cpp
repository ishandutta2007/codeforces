...
int main()
{
	...
	NumberTheory nt; nt.Sieve(500111);
	int n; cin>>n;
	vector<ll> vec; vector<ll> poor;
	for(int i=0;i<n;i++)
	{
		ll x,y; cin>>x>>y; vec.pb(lcm(x,y));
		poor.pb(x); poor.pb(y);
	}
	ll ans=vec[0];
	for(int i=1;i<n;i++) ans=__gcd(ans,vec[i]);
	if(ans==1) ans=-1;
	else
	{
		for(int i=0;i<poor.size();i++)
		{
			ll tmp = __gcd(ans,poor[i]);
			if(tmp>1) ans = tmp; 
		}
		for(int p:nt.primes)
		{
			if(ans%p==0){ans=p; break;}
		}
		//ans is not divisble by any prime
	}
	cout<<ans<<'\n';
}