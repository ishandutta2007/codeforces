	...
	while(T--)
	{
		...
		up(i,1,n)
		{
			up(j,1,n)
			{
				int p;
				rd(p);
				chkmin(xl[p],i);chkmin(yl[p],i);
				chkmax(xr[p],j);chkmax(yr[p],i);
			}
		}
		int tot=0;
		up(i,1,n*n)if(yr[i])++tot;
		if(tot<=k)
		{
			cout<<k-tot;return 0;
		}
		...
		up(len,1,n)
		{
			...
			up(i,1,n*n)
			{
				if(!yr[i])continue;
				int XL,YL;
				XL=max(1,xr[i]-len+1);YL=max(1,yr[i]-len+1);
				int XR,YR;
				XR=min(xl[i],n-len+1),YR=min(xr[i],n-len+1);
//				cout<<XL<<" "<<YL<<" "<<XR<<" "<<xr[i];edl;
				if(XL<=XR&&YL<=YR)
				{
					++a[XL][YL];++a[XR+1][YR+1];
					--a[XL][YR+1];--a[XR+1][YL];
