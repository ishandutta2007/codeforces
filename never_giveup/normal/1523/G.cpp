...
namespace fen {
    ...
    const int N = 5e4 + 10;
    vector<int> ind[N], q[N];

    void add(int l, int r, int id) {
        l = n - 1 - l;
        for (; l < n; l = (l | (l + 1))) {
            for (int j = low_bo(ind[l], r); j < ind[l].size(); j = (j | (j + 1)))
                setmin(q[l][j], id);
        }
    }
    ...
    int get(int l, int r) {
        ...
        int ans = inf;
        for (; l >= 0; l = (l & (l + 1)) - 1)
            for (int j = up_bo(ind[l], r) - 1; j >= 0; j = (j & (j + 1)) - 1)
                setmin(ans, q[l][j]);
        return ans;
    }
}
...
void solve() {
    ...
    vector<pair<int, int>> sid(m);
    for (int i = 0; i < m; i++) {
        cin >> seg[i].fr >> seg[i].sc;
        seg[i].fr--;
        seg[i].sc--;
    }
    {
        ...
        for (int i = 0; i < m; i++) {
            auto [l, r] = seg[i];
            l = n - 1 - l;
            for (int j = l; j < n; j = (j | (j + 1)))
                ind[j].push_back(r);
        }
        for (int i = 0; i < n; i++) {
            sort(all(ind[i]));
            unique(ind[i]);
            q[i].resize(ind[i].size(), inf);
        }
    }
    vector<int> answer(n + 1);
    for (int x = 1; x <= min(n, 5); x++) {
        set<int> g;
        int s = 0;
        for (int i = 0; i < m; i++) if (seg[i].sc - seg[i].fr + 1 >= x) {
                auto [l, r] = seg[i];
    ...
    for (int i = 0; i < m; i++) {
        segs.push_back({seg[i], i});
    }
    sort(segs.begin(), segs.end(), [&](auto x, auto y) {return x.fr.sc - x.fr.fr + 1 > y.fr.sc - y.fr.fr + 1;});
    int iter = 0;
    for (int LEN = n; LEN > 5; LEN--) {
        bool changed = 0;
        while (iter < m && segs[iter].fr.sc - segs[iter].fr.fr + 1 == LEN) {
            changed = 1;
            fen::add(segs[iter].fr.fr, segs[iter].fr.sc, segs[iter].sc);
            iter++;
        }
        if (!changed) {
            answer[LEN - 1] = answer[LEN];
            continue;
        }
        int s = 0;
        ...
        while (!q.empty()) {
            ...
            q.pop();
            id = -id;
            s += seg[id].sc - seg[id].fr + 1;
            int l1 = seg[id].fr, r1 = seg[id].sc;
            add(l, l1 - 1);
            add(r1 + 1, r);
        }
