...
namespace vb{
	...
	inline int query(int x,int y){
		if(bg[x]==bg[y]){
			for(int i=x;i<=y;i++) if(a[i]) return val[i];
			return -1;
		}
		x--,y++;
		for(int i=x+1;i<=r[bg[x]];i++) if(a[i]) return val[i];
		for(int i=y-1;i>=l[bg[y]];i--) if(a[i]) return val[i];
		for(int i=bg[x]+1,j=bg[y];i<j;i++){
			while(tp[i]&&!a[st[i][tp[i]]]) in[st[i][tp[i]--]]=0;
			if(tp[i]) return val[st[i][tp[i]]];
		}
		return -1;
	}
}
