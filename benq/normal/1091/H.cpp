...
sieve<MX> S;
int n,f, ans, grundy[MX];
bitset<MX> ok, tmp[101];

void gen() {
    F0R(i,sz(S.pr)) {
        ok[S.pr[i]] = 1;
        FOR(j,i,sz(S.pr)) {
            if ((ll)S.pr[i]*S.pr[j] >= MX) break;
            ok[(ll)S.pr[i]*S.pr[j]] = 1;
        }
    }
    ok[f] = 0;
    // for (int i = 0; i < MX; ++i) pr(i,ok[i]);
    // exit(0);
    /*F0R(i,1000) {
        for ()
    }*/
    F0R(i,MX) {
        int x = 0; while (tmp[x][i]) x ++;
        tmp[x] |= ok<<i;
        grundy[i] = x;
        // if (i <= 1000) pr(i,grundy[i]);
    }
}
