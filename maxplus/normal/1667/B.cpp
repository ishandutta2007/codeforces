...
struct FT {
  ...
  T& op(T& val, const T& arg) {
    return maxi(val[0], arg[0]), maxi(val[1], arg[1]), val;
  }
  ...
  T query(int i) {
    T res = {(int64_t)-1e18, (int64_t)-1e18};
    for (; i; i &= i - 1) {
      op(res, ft[i - 1]);
    }
    return res;
...
int main() {
  ...
  for (int testcase = $; testcase; --testcase) {
    cin >> n >> range{a, n};
    for (int64_t ps = 0, i = 0; i <= n; ps += a[i++]) {
      b[i] = ps;
    }
    sort(b, b + n + 1);
    m = unique(b, b + n + 1) - b;
    memset(&ft, 192, m * 16);
    int pans = 0;
    ft.update(lower_bound(b, b + m, 0) - b, {0, 1});
    for (int64_t ps = 0, i = 0; i < n; ++i) {
      ps += a[i];
      int cidx = lower_bound(b, b + m, ps) - b;
      int cans = pans + sgn(a[i]);
      auto q = ft.query(cidx);
      maxi(cans, q[1] + i);
      maxi(cans, q[0]);
      maxi(cans, ft.ft[cidx][0]);
      ft.update(cidx, {cans, cans - i});
      pans = cans;
    }
    print(pans);
  }
