...
struct segment_op {
  ...
  static void op(node &ab, node const &a, node const &b) {
    ab.sum = a.sum + b.sum;
    ab.need = max(a.need, b.need - a.sum);
    ab.nop = max(a.nop, b.nop + a.sum);
  }
};
...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) cin>>B[i];

  segment_tree<segment_op> st(n);
  for(li i = 0; i < (li)(n); ++i) {
    st.A[st.n + i].reset(i, A[i], B[i]);
  }
  st.build();

  for(li q_ = 0; q_ < (li)(q); ++q_) {
    li l,r; cin>>l>>r; --l; --r;
    auto res = st.query_range(l,r);

    if(res.sum == 0 && res.need == 0) {
      cout << res.nop << endl;
    }else{
      cout << -1 << endl;
    }
