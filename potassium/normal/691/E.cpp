...
int main() {
	...
	vector<vector<Mint>> m(n, vector<Mint>(n, Mint(0, MOD107)));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			if (__builtin_popcountll(a[i] ^ a[j]) % 3 == 0)
				m[i][j] = Mint(1, MOD107);
				
	vector<vector<Mint>> ans(n, vector<Mint>(n, Mint(0, MOD107)));
	for (int i = 0; i < n; i++)
		ans[i][i] = Mint(1, MOD107);
		
	auto b = m;
		
	auto multiply = [&](const vector<vector<Mint>>& x, const vector<vector<Mint>>& y) {
		vector<vector<Mint>> z(n, vector<Mint>(n, Mint(0, MOD107)));
		for (int i = 0; i < n; i++)
			for (int k = 0; k < n; k++)
				for (int j = 0; j < n; j++)
					z[i][j] += x[i][k] * y[k][j];
