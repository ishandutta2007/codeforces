...
void dfs(int v, int p)
{
	for(auto e : G[v]){
		...
		if(u == p) continue;
		if(w[id] == -1){
			if(ts.assign[u] == ts.assign[v]) w[id] = 0;
			else w[id] = 1;
		}
		cout << u << " " << v << " " << w[id] << "\n";
		dfs(u, v);
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n-1){
			...
			G[v].pb(edge(u, i));
			if(w[i] != -1){
				if(popcount(w[i])%2 == 0) ts.addClause(-u, v), ts.addClause(-v, u);
				else ts.addClause(-u, -v), ts.addClause(v, u);
			}
		}
		ll p;
		rep(i, 1, m){
			cin >> u >> v >> p;
			if(p == 0) ts.addClause(-u, v), ts.addClause(-v, u);
			else ts.addClause(-u, -v), ts.addClause(v, u);
		}
		if(!ts.calc()){
			cout << "NO" << "\n";
