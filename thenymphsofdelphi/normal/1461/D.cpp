...
int q; cin >> q; while (q--){
    ...
    ranges.pb({1, n});
    while (!ranges.empty()){
        int l = ranges.back().fi, r = ranges.back().se; ranges.pob();
        sttSum.insert(b[r] - b[l - 1]);
        if (a[l] == a[r]){
            continue;
        }
        int valMid = (a[l] + a[r]) / 2;
        int idx = upb(a + 1, a + 1 + n, valMid) - a - 1;
        ranges.pb({l, idx});
        ranges.pb({idx + 1, r});
    }
    while (qq--){
        int x; cin >> x;
        if (sttSum.find(x) != sttSum.end()){
            cout << "Yes" << endl;
        }
        else{
            cout << "No" << endl;
