...
struct Trie {
	...
	void insert(int v) {
		...
		for (int b = BITS - 1; b >= 0; b--) {
			int to = v >> b & 1;
			if (trie[cur].child[to] == -1) {
				trie[cur].child[to] = (int)trie.size();
				trie.push_back(Node());
			}
			cur = trie[cur].child[to];
		}
	}

	int get(int v, bool isMin) {
		int ans = 0;
		int cur = 0;
		for (int b = BITS - 1; b >= 0; b--) {
			int to = v >> b & 1;
			int need = isMin ? to : !to;
			if (trie[cur].child[need] != -1) {
				if (!isMin) ans ^= 1 << b;
				cur = trie[cur].child[need];
			}
			else {
				if (isMin) ans ^= 1 << b;
				cur = trie[cur].child[!need];
...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < r - l + 1; i++) {
			int x = a[i] ^ l;
			if (trie.get(x, true) == l && trie.get(x, false) == r) {
				W(x);
				break;
			}
		}
