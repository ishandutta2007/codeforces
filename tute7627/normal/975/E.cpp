...
int main(){
  ...
  vector<Point>p(n);
  rep(i,0,n)cin>>p[i];
  Point g=convex_centroid(p);
  vector<Point>q(n);
  rep(i,0,n)q[i]=p[i]-g;
  ld rad=0;
  ll pin1=0,pin2=1;
  auto locate=[&](ll idx){
    return g+rotate(rad,q[idx]);
  };
  lfs;
  //rep(i,0,n)cout<<locate(i)<<endl;
  //cout<<g spa rad<<endl;
  while(que--){
    ll type;cin>>type;
    if(type==1){
      ll f,t;cin>>f>>t;f--;t--;
      if(f==pin1)swap(pin1,pin2);
      pin2=t;
      g=locate(pin1)-Point(0,abs(q[pin1]));
      rad=PI/2-arg(q[pin1]);
      //cout<<g spa rad spa locate(pin1)<<endl;
      //cout<<pin1 spa pin2<<endl;
    }
    else{
      ll v;cin>>v;v--;
      Point tmp=locate(v);
  ...
  //rep(i,0,3)rep(j,i+1,3)cout<<distance(locate(i),locate(j))<<endl;
  return 0;
}