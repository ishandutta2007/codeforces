...
void test(li ab, li ba, vi szs) {
  sort(begin(szs), end(szs));
  while(ab > 0 && !szs.empty()) {
    li sz = szs.back();
    szs.pop_back();
    ba += sz;
    sz -= 1;
  ...
  cout << (ab<=0 ? "YES" : "NO") << endl;
}

void solve(){
  li a,b,ab,ba; cin>>a>>b>>ab>>ba;
  string s; cin>>s;
  li na=0,nb=0;
  for(char c : s) if(c=='A') na++; else nb++;
