...
bool f(int nw,int mask,bool st){
	...
	if(ret!=-1)return ret;
	if(st){
		rep(i,0,9)if(f(nw+1,mask|(1<<i),1))return ret = true;
	}
	else {
		if(f(nw+1,mask|(1<<x[nw]),0))return ret = true;
		rep(i,x[nw]+1,9)if(f(nw+1,mask|(1<<i),1))return ret = true;
	}
	return ret = false;
}
...
void bt(int nw,int mask,bool st){
	if(nw==n)return;
	if(st){
		rep(i,0,9)if(f(nw+1,mask|(1<<i),1)){
			cout<<i;
			bt(nw+1,mask|(1<<i),1);
			return;
		}
		...
		rep(i,x[nw]+1,9)if(f(nw+1,mask|(1<<i),1)){
			cout<<i;
			bt(nw+1,mask|(1<<i),1);
			return;
		}
...
// freopen("output.txt","w",stdout);
	...
	rep(i,0,1023)pre[i] = __builtin_popcount(i);
	cin>>tc;
	rep(ntc,1,tc){
		cin>>s>>k;
		n = s.size();
