...
void work()
{
    ...
    rin>>h>>w;
    int ans=0;
    ans=((w+1>>1)<<1)+((h-3>>1)<<1);jh(w,h);
    ans=max(ans,((w+1>>1)<<1)+((h-3>>1)<<1));

    jh(w,h);
        a[1][1]=a[1][w]=a[h][1]=a[h][w]=1;
        for(int i=2;i<w;i++)if(!a[1][i-1]&&!a[1][i+1])a[1][i]=a[h][i]=1;
        for(int i=2;i<h;i++)if(!a[i-1][1]&&!a[i+1][1])a[i][1]=a[i][w]=1;
    for(int i=1;i<=h;i++){for(int j=1;j<=w;j++)printf("%d",a[i][j]);puts("");}
    // printf("%d\n",ans);
    return;
}
int main()
