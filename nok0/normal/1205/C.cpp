...
void main_() {
	...
	//b[0][1] = 1
	board[0][1] = 1;
	board[1][2] = query(0, 1, 1, 2);
	board[1][0] = (!query(1, 0, 1, 2)) ^ board[1][2];
	...
	REP(i, n) {
		REP(j, n) {
			...
			REP(k, 3) {
				int nx = i + dx[k];
				int ny = j + dy[k];
				if(nx < n and ny < n and board[nx][ny] == -1) {
					board[nx][ny] = board[i][j] ^ (!query(i, j, nx, ny));
				}
			}
		}
	}
	...
	REP(i, n) {
		REP(j, n) {
			if(i + 3 < n and board[i][j] == board[i + 3][j] and board[i + 1][j] == board[i + 2][j]) {
				rev = !query(i, j, i + 3, j);
				goto fin;
			}
			if(j + 3 < n and board[i][j] == board[i][j + 3] and board[i][j + 1] == board[i][j + 2]) {
				rev = !query(i, j, i, j + 3);
				goto fin;
			}
			if(i + 2 < n and j + 1 < n and board[i][j] == board[i + 2][j + 1] and (board[i + 1][j] == board[i + 2][j] or board[i + 1][j] == board[i + 1][j + 1] or board[i][j + 1] == board[i + 1][j + 1])) {
				rev = !query(i, j, i + 2, j + 1);
				goto fin;
			}
			if(i + 1 < n and j + 2 < n and board[i][j] == board[i + 1][j + 2] and (board[i][j + 1] == board[i][j + 2] or board[i][j + 1] == board[i + 1][j + 1] or board[i + 1][j] == board[i + 1][j + 1])) {
				rev = !query(i, j, i + 1, j + 2);
				goto fin;
			}
		}
	}
