...
const Res Neu = {false, -1, -1, -1, -1};
int max(int a, int b, int c) {
	return max(max(a, b), c);
}
def_op(Res, odd_len, first, last, best_odd, best_even)
Res merge(Res a, Res b) {
	if (a.first == -1) return b;
	if (b.first == -1) return a;
	Res ret = {a.odd_len ^ b.odd_len,
		a.first,
		b.last,
		a.odd_len ? max(a.best_odd, b.best_even) : max(a.best_odd, b.best_odd, a.last + b.first),
		a.odd_len ? max(a.best_even, b.best_odd, a.last + b.first) : max(a.best_even, b.best_even)
	};
	// debug(imie(a), imie(b), imie(ret));
	return ret;
}
struct MyOp {
	using T = Res;
	static T neu() {return Neu;}
	static T conv(pii x) {return make(x.second);}
	static T mer(T a, T b) {return merge(a, b);}
};
const int nax = 5e5 + 44;
char row[nax];
ordered_set <int> points;
seg_set<pii, MyOp> segments;
const char flip = '<' ^ '>';
void erase_seg(int a, int b) {
	assert(a < b);
	segments.erase(make_pair(a, b - a));
}
void insert_seg(int a, int b) {
	assert(a < b);
	segments.insert(make_pair(a, b - a));
}
void touch(int p) {
...
int main() {
	...
	points.insert(0);
	points.insert(n);
	REP(i, n - 1) if (row[i] != row[i + 1]) points.insert(i + 1);
	{
		int prev = -1;
		for (int x : points) {
			if (prev != -1) insert_seg(prev, x);
			prev = x;
		}
	}
	debug(imie(points), imie(segments));
	REP(_, q) {
		int l, r;
		scanf("%d%d", &l, &r);
		l--;
		...
		loc(for(int i = max(l, 1); i < r; ++i) row[i] ^= flip);
		if (r != n) touch(r);
		debug(imie(points), imie(segments), imie(row), imie(l), imie(r));
		if (*points.lower_bound(l) >= r) {
			printf("%d\n", r - l);
		}
		else {
			auto it1 = segments.lower_bound(pii(l, -1));
			auto it2 = segments.lower_bound(pii(r, inf));
			it2--;
			Res p = segments.read(it1, it2);
			debug(imie(points));
			debug(range(segments.begin(), it1), range(it1, it2), range(it2, segments.end()));
			if (it1->first > l)
				p = merge(make(it1->first - l), p);
			if (it2->first < r)
				p = merge(p, make(r - it2->first));
			bool dir = (row[0] == '<') ^ (points.order_of_key(l + 1) % 2);
			debug(imie(p), imie(dir));
			if (!dir) printf("%d\n", p.best_odd);
			else printf("%d\n", p.best_even);
		}
	}
