#line 1 "codeforces-workspace\\contest\\1601\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1601\\b\\b.cpp"
...
#line 13 "Library\\src\\graph\\base.h"
...
#line 16 "Library\\src\\graph\\base.h"
...
#line 9 "Library\\src\\graph\\digraph.h"
...
#line 29 "codeforces-workspace\\contest\\1601\\b\\b.cpp"
...
void main() {
  ...
  input n;
  weighted_digraph<int> g;
  auto in = g.add_nodes(n + 1);
  auto out = g.add_nodes(n + 1);

  auto is_in = [&](auto v) -> auto { return v < in.size(); };

  for (auto i : range(1, 1 + n)) {
    input a;
    g.add_edge(out[i], in[i - a], 1);
  }
  for (auto i : range(n)) {
    g.add_edge(in[i], in[i + 1], 0);
  }
  for (auto i : range(1, 1 + n)) {
    input b;
    g.add_edge(in[i], out[i + b], 0);
  }
  g.add_edge(in[0], out[0], 0);

  auto now = out[0];
  vector<i32> ans;

  for (auto &&e : reversed(g.dijkstra(out[n]))) {
    if (now == e.head) {
      if (not is_in(now) and is_in(e.tail)) {
        ans.emplace_back(e.tail);
      }
      now = e.tail;
    }
  }
