...
ll check(vl a,vl b,ll M){
    ...
    assert(c.size()==sz && d.size()==sz);
    vvl e(sz);
    rep(i,0,sz-1){
        sort(all(d[i]),[](ll x,ll y){return x>y;});
        e[i].resize(d[i].size()+1);
        rep(j,1,d[i].size()){
            e[i][j]=e[i][j-1]+d[i][j-1];
        }
    }
    /*
    for(auto x:e){
    ...
    vvl dp(sz,vl(n+1,-inf));
    dp[0][d[0].size()]=e[0][d[0].size()];
    rep(i,0,sz-2){
        rep(j,0,n){
            if(dp[i][j]<=-inf/2)continue;
            ll dsz=d[i+1].size();
            rep(ko,0,min(j,dsz)){
                assert(j-ko+dsz-ko<=n);
                chmax(dp[i+1][j-ko+(dsz-ko)],dp[i][j]+e[i+1][dsz-ko]);
            }
        }
    }
    /*
...
signed main(){fastio
    ll n;cin>>n;
    vl a(n),b(n);
    rep(i,0,n-1){cin>>a[i];a[i]*=1000;}
    rep(i,0,n-1)cin>>b[i];
    
    ll ok=2e11,ng=0;
    while(abs(ok-ng)>1){
        dd mid=(ok+ng)/2;
        if(check(a,b,mid)>=0)ok=mid;
        else ng=mid;
    }
    /*
    rep(i,ok-1,ok+1){
        cout<<i<<":"<<check(a,b,i)<<endl;
    }//*/
