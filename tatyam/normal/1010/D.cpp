...
int main(){
    ...
    auto dfs1 = [&](ll at, auto dfs) -> void {
        if(type[at]=="IN")return;
        if(type[at]=="NOT"){
            dfs(l[at],dfs);
            res[at]=!res[l[at]];
            return;
        ...
        dfs(l[at],dfs);
        dfs(r[at],dfs);
        if(type[at]=="AND")res[at]=res[l[at]]&&res[r[at]];
        elif(type[at]=="OR")res[at]=res[l[at]]||res[r[at]];
        else res[at]=res[l[at]]^res[r[at]];
    };
    dfs1(0,dfs1);
    nres[0]=!res[0];
    auto dfs2 = [&](ll at, auto dfs) -> void {
        if(type[at]=="IN")return;
        if(type[at]=="NOT"){
            res[l[at]]=res[at];
            nres[l[at]]=nres[at];
        ...
        if(type[at]=="AND"){
            nres[l[at]]=res[r[at]]?nres[at]:res[at];
            nres[r[at]]=res[l[at]]?nres[at]:res[at];
            res[l[at]]=res[r[at]]=res[at];
        }
        elif(type[at]=="OR"){
            nres[l[at]]=res[r[at]]?res[at]:nres[at];
            nres[r[at]]=res[l[at]]?res[at]:nres[at];
            res[l[at]]=res[r[at]]=res[at];
        }
        else{
            nres[l[at]]=nres[r[at]]=nres[at];
            res[l[at]]=res[r[at]]=res[at];
        }
        dfs(l[at],dfs);
        dfs(r[at],dfs);
    ...
    dfs2(0,dfs2);
    rep(n)if(type[i]=="IN")print(nres[i]);
    out();
}