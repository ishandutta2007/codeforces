...
void solve()
{
    ...
    for (int i = 0; i<m; i++)
    {
        vector<int> maxx(i+1, -1);
        maxx[0] = 0;

        for (int j = 0; j<n; j++)
        {
            vector<pair<int, int>> temp;
            for (int k = 0; k<=i; k++) if (maxx[k]!=-1)
            {
                //(k, maxx[k]
                if (k<i && s[j]==t[k]) temp.push_back(mp(k+1, maxx[k]));
                if (maxx[k]<m-i && s[j]==t[maxx[k] + i]) temp.push_back(mp(k, maxx[k]+1));
            }
            for (auto it: temp) maxx[it.first] = max(maxx[it.first], it.second);
        }
        if (maxx[i]==m-i) {cout<<"YES"<<endl; return;}
    }
    cout<<"NO"<<endl;
