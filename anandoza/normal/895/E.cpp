...
int main() {
    ...
    auto tt = [](T a, T b) -> T { return {a.sum + b.sum, a.size + b.size}; };
    auto uu = [](U a, U b) -> U {
        auto p = 1 - (1 - a.pr) * (1 - b.pr);
        if (p == 0.0)
            return {0, 0};
        auto val = (1 - a.pr) * b.pr * b.ev + a.pr * (1 - b.pr) * a.ev + a.pr * b.pr * a.ev;
        val /= p;
        return {val, p};
    };
    auto ut = [](U u, T t) -> T { return {t.sum + t.size * (u.ev - t.sum / t.size) * u.pr, t.size}; };
    segment_tree_lazy st(n, T{0, 0}, U{0, 0}, tt, uu, ut);
    st.set_leaves([&](int i) -> T { return T{(double) a[i], 1}; });
    ...
    F0R (query, q) {
        ...
        if (type == 1) {
            ...
            T t1 = st(l1, r1);
            T t2 = st(l2, r2);
            st(l1, r1, {t2.sum / t2.size, 1.0 / (r1 - l1)});
            st(l2, r2, {t1.sum / t1.size, 1.0 / (r2 - l2)});
        } else {
            int l, r;
            re(l, r);
            l--;
