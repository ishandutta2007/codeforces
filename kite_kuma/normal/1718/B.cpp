...
bool solve() {
	...
	rep(nxt, 2, 100) {
		if(fibsum == csum) break;
		if(fibsum > csum) return false;
		fib.push_back(fib[nxt - 1] + fib[nxt - 2]);
		fibsum += fib.back();
		if(fib.back() > pow_ll(10LL, 9)) break;
	}
