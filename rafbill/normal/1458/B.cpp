...
int main(){
  ...
  li sb = 0;
  for(li i = 0; i < (li)(n); ++i) sb += B[i];

  for(li i = 0; i < (li)(n+1); ++i) for(li j = 0; j < (li)(11111); ++j) dp[i][j] = -1e9;
  dp[0][0] = 0;
  for(li i = 0; i < (li)(n); ++i) for(li k = (n-1); k >= (li)(0); --k) for(li j = 0; j < (li)(11111); ++j) if(dp[k][j] > -1) {
    smax(dp[k+1][j + A[i]], dp[k][j] + B[i]);
  }
  for(li k = (1); k <= (li)(n); ++k) {
    li ans = 0;
    for(li i = 0; i < (li)(11111); ++i) smax(ans, min<li>(2*i, 2*dp[k][i] + (sb-dp[k][i])));
    cout << setprecision(10) << fixed << (double)ans/2.0 << ' ';
  }
  cout << endl;
