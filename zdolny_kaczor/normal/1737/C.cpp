...
//#STAY AT HOME
istream&operator>>(istream &i, pii &p) {
	i >> p.first >> p.second;
	return i;
}
...
void solve() {
	...
	cin >> s1 >> s2 >> s3 >> goal;
	pii vacant = s1 ^ s2 ^ s3;
	debug(imie(s1), imie(s2), imie(s3), imie(vacant), imie(goal));
	bool can = true;
	if (goal % 2 == vacant % 2) can = false;
	for (auto [corner, next] : vector <pair <pii, pii> > {{{1, 1}, {2, 2}}, {{1, n}, {2, n - 1}}, {{n, 1}, {n - 1, 2}}, {{n, n}, {n - 1, n - 1}}}) {
		if ((corner == s1 || corner == s2 || corner == s3) && vacant == next && (corner.first != goal.first && corner.second != goal.second))
			can = false;
	}
	if (!can) cout << "NO\n";
	else cout << "YES\n";
