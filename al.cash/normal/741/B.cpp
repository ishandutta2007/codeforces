...
class DSU {
  vector<int> parent;  // if vertex is a root, then parent = -tree_size < 0
...
int main() {
  ...
  REP(i, n) if (!g[i].empty()) {
    ...
    for (int j : g[i]) {
      bSum += b[j];
      wSum += w[j];
      REP(u, l-w[j]+1) smax(d1[u + w[j]], d0[u] + b[j]);
    }
    REP(u, l+1-wSum) smax(d1[u + wSum], d0[u] + bSum);
    swap(d0, d1);
  }
  int ans = 0;
  REP(u, l+1) smax(ans, d0[u]);
  writeln(ans);
