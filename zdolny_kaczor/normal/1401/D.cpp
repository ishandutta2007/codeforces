...
#endif
//#STAY AT HOME
const int nax = 1e5 + 44, mod = 1e9 + 7;
vi graf[nax];
vi sizes;
int dfs(int x, int y = -1) {
	int ans = 1;
	for (int v : graf[x]) if (v != y) ans += dfs(v, x);
	if (y != -1) sizes.push_back(ans);
	rans;
}
void solve() {
	int n;
	...
	scanf("%d", &m);
	vi ps(m);
	REP(i, m) scanf("%d", &ps[i]);
	debug(imie(sizes));
	vector <ll> coef;
	for (int x : sizes) coef.push_back(x * 1ll * (n - x));
	sort(ALL(coef));
	sort(ALL(ps));
	int start = ps.size() < coef.size() ? coef.size() - ps.size() : 0;
	for (ll &x : coef) x %= mod;
	REP(i, (int)ps.size()) {
		debug(arr(ps, i));
		int at = min(i + start, (int) coef.size() - 1);
		(coef[at] *= ps[i]) %= mod;
		debug(imie(coef));
	}
	ll ans = 0;
