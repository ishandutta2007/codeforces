...
int displeasure(int i, vector<int> &age, vector<int> &postfix_min){
    if (postfix_min[i] == age[i]) return -1;
    int L = i + 1, R = age.size() - 1;
    while (R - L > 1){
        int m = (L + R) / 2;
        (postfix_min[m] < age[i] ? L : R) = m;
    }
    return (postfix_min[R] < age[i] ? R : L) - i - 1;
}
...
void solve() {
    ...
    postfix_min.back() = age.back();
    for (int i = n - 2; i >= 0; --i) postfix_min[i] = min(postfix_min[i + 1], age[i]);
    for (int i = 0; i < n; ++i){
        cout << displeasure(i, age, postfix_min) << " ";
    }
}