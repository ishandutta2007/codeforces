...
struct solver {
	int n;
	vector<int> a;	// p[i] - p[n-1]
	vector<int> question;
	solver(int N) : a(N), question(N, 1), n(N) {}
	...
	void ans(vi &p) {
		cout << "! ";
		int smallest = *min_element(all(p));
		int up = 1 - smallest;
		foa(t, p) t += up;
	...
	void solve() {
		rep(sa, -n + 1, n) {
			if(sa > 0) {
				rep(i, n - 1) question[i] = 1;
				question[n - 1] = 1 + sa;
			} else if(sa < 0) {
				rep(i, n - 1) question[i] = 1 - sa;
				question[n - 1] = 1;
			} else {
				continue;
			}
			int res = query(question);
			if(res != -1) {
				a[res] = sa;
			}
		}
