...
map <pii, vi> dirs;
using LL = __int128;
sim, class d> vector <d> psum(const vector <c> &a) {
	int len = a.size();
	vector <d> b(len + 1);
	REP(i, len) b[i + 1] = b[i] + a[i];
	return b;
}
vector <LL> dists(const vi &a, int total) {
	int len = a.size();
	vector <ll> dists(len);
	ll su = 0;
	REP(i, len) su += a[i];
	REP(i, len) {
		dists[i] = su + a[i] * (2ll * i - len);
		su -= 2 * a[i];
	}
	debug(imie(a), imie(dists));
	vector <LL> distsums = psum<ll, LL>(dists);
	vector <ll> possums = psum<int, ll>(a);
	vector <LL> possumsq(len + 1);
	REP(i, len) possumsq[i + 1] = possumsq[i] + i * 1ll * a[i];
	auto calc_inside = [&](int l, int r) {
		int seg_len = r - l;
		return 2 * (possumsq[r] - possumsq[l]) + (possums[r] - possums[l]) * (LL)(-2 * l - seg_len + 1);
	};
	// for (int i = 0; i <= len; ++i) for (int j = i; j <= len; ++j) {
		// debug(range(a, i, j), fun(calc_inside, i, j));
	// }
	vector <LL> ans;
	ll tot_sum = calc_inside(0, len);
	for (int rem = 0; rem <= len; ++rem) {
		int rest = total - (len - rem);
		debug(imie(rem), imie(rest));
		auto calc_res = [&](int l) {
			int r = l + rem;
			assert(r <= len);
			auto ret = tot_sum + calc_inside(l, r) - (distsums[r] - distsums[l]) + rest * (LL) (possums[len] - possums[r] + possums[l]);
			debug(range(a, l, r), imie(ret));
			return ret;
		};
		int low = 0, high = len - rem;
		LL best = -1e36;
		while (low + 1 <= high) {
			int med = (low + high) / 2;
			LL v1 = calc_res(med), v2 = calc_res(med + 1);
			maxi(best, max(v1, v2));
			if (v1 > v2) {
				high = med - 1;
			}
		...
		for (int i = low; i <= high; ++i) maxi(best, calc_res(i));
		ans.push_back(best);
	}
	debug(imie(a), imie(total), imie(ans));
	rans;
}
const int nax = 5e5 + 44;
vector <LL> scores[nax];
ld coef[nax];
int pos[nax];
ld norm(pii x) {
	return sqrt(x.first * 1ll * x.first + x.second * 1ll * x.second);
}
void assert_concave(vector <LL> &a) {
	int len = a.size();
	bool wrong = false;
	REP(i, len - 2) {
...
int main() {
	debug(fun(numeric_limits<LL>::min));
	int n, k;
	scanf("%d%d", &n, &k);
	REP(_, n) {
		pii v;
	...
	for (auto &[x, v] : dirs) {
		sort(ALL(v));
		coef[cou] = norm(x);
		scores[cou] = dists(v, k);
		// assert_concave(scores[cou]);
		cou++;
	}
	debug(range(coef, 0, cou));
	priority_queue<pair <ld, int> > gain;
	REP(i, cou)
		gain.push(make_pair(coef[i] * (scores[i][1] - scores[i][0]), i));
	ld ans = 0;
	REP(turn, n - k + 1) {
		if (turn) {
			auto [_, i] = gain.top();
			gain.pop();
			pos[i]++;
			if (pos[i] + 1u != scores[i].size())
				gain.push(make_pair(coef[i] * (scores[i][pos[i] + 1] - scores[i][pos[i]]), i));
		}
		if (turn >= n - k - 1) {
			REP(i, cou) debug(arr(scores, i, arr(pos, i)) * arr(coef, i));
			ld curr = 0;
			REP(i, cou) curr += coef[i] * scores[i][pos[i]];
			maxi(ans, curr);
			debug(imie(curr));
		}
	}
