...
int main() {
    ...
    for(auto [a, b, c] : v) g[a].eb(b, c);
    vi w(R + 1);
    auto dfs = [&](auto &&f, int x, ll d) -> void {
        if(x == 32) { w[d] = 1; }
        for(auto e : g[x]) { f(f, e, d + e.cost); }
    };
    dfs(dfs, 1, 0);
    rep2(i, L, R) { assert(w[i] == 1); }
    rep2(i, 0, L - 1) assert(w[i] == 0);
}