...
struct S {
    ...
    friend S operator+(const S &l, const S &r) { return {l.sum + r.sum, std::min(l.sum + r.min, l.min)}; }
};

S make(int val) { return {val, std::min(0, val)}; }
...
int main() {
    ...
    segment_tree st(n, S{0, 0}, std::plus<S>());
