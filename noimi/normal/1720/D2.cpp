...
template <typename T, int MAX_LOG, typename D = int> struct BinaryTrie {
  public:
    struct Node {
        ...
        Node() : nxt{nullptr, nullptr}, exist(0) { dp[0] = dp[1] = 0; }
    };
    ...
    void add(const T &bit, int idx = -1, D delta = 1, T xor_val = 0) { root = add(root, bit, idx, MAX_LOG, delta, xor_val); }

    void erase(const T &bit, T xor_val = 0) { add(bit, -1, -1, xor_val); }

    Node *find(const T &bit, T xor_val = 0) { return find(root, bit, MAX_LOG, xor_val); }
    ...
    int solve(int bit, int idx) { return solve(root, bit, idx, MAX_LOG); }

  private:
    virtual Node *clone(Node *t) { return t; }

    int solve(Node *t, T bit, int idx, int depth) {
        dump(bit, idx, depth);
        // int res = t->dp[idx >> depth & 1];
        int res = 0;
        if(depth == -1) return res;
        auto &to = t->nxt[bit >> depth & 1];
        auto &p = t->nxt[(bit >> depth & 1) ^ 1];
        if(p) { chmax(res, p->dp[(idx >> depth) & 1]); }
        if(to) chmax(res, solve(to, bit, idx, depth - 1));
        return res;
    }
    ...
    Node *add(Node *t, T bit, int idx, int depth, D x, T xor_val, bool need = true) {
        ...
        // dump(idx, depth, (idx >> (depth + 1) & 1), x);
        chmax(t->dp[idx >> (depth + 1) & 1], x);
        return t;
    }
...
int main() {
    TEST {
        ...
        BinaryTrie<int, oj_local(31, 2)> trie;
        int ans = 0;
        rep(i, n) {
            int x = a[i] ^ i;
            int r = trie.solve(x, i) + 1;
            chmax(ans, r);
            dump(r);
            trie.add(x, a[i], r);
        }
