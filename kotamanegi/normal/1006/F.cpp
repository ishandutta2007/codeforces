...
void solve(){
    ...
    nexts.push(make_tuple(n - 1, m - 1, 2));
    cnts[0][0][grid[0][0]] = 1;
    cnts[n - 1][m - 1][grid[n - 1][m - 1]] = 1;
    Visited[0][0] |= 1;
    Visited[n - 1][m - 1] |= (1 << 2);
    while (!nexts.empty()) {
        tuple<int, int, int> now = nexts.front();
        nexts.pop();
        for (int i = get<2>(now); i < get<2>(now) + 2; ++i) {
            int next_x = get<0>(now) + dx[i];
            int next_y = get<1>(now) + dy[i];
            if (next_x >= 0 && next_x < n && next_y >= 0 && next_y < m) {
                if (Visited[next_x][next_y] == 0) {
                    nexts.push(make_tuple(next_x, next_y, get<2>(now)));
                }
                Visited[next_x][next_y] |= (1 << get<2>(now));
                if (Visited[next_x][next_y]  != (1 << get<2>(now))) {
                    //Die course
                    for (auto& x : cnts[get<0>(now)][get<1>(now)]) {
                        ans += x.second * cnts[next_x][next_y][x.first ^ k];
                    }
                }
                else {
                    for (auto& x : cnts[get<0>(now)][get<1>(now)]) {
                        cnts[next_x][next_y][x.first ^ grid[next_x][next_y]] += x.second;
                    }
                }
            }
        }
        cnts[get<0>(now)][get<1>(now)].clear();
    }
    cout << ans << endl;
}
#undef int
