...
#line 1 "/home/maspy/library/nt/extgcd.hpp"
...
void solve() {
  ...
  auto [x1, y1, x2, y2] = Farey_lr(a, b);
  ll g = a / (x1 + x2);
  if (g > 1) return print("Impossible");

  vc<string> ANS;
  // (x1/y1, x2/y2) 
  while (1) {
    if (x1 + y1 == 1 && x2 + y2 == 1) break;
    ll r = (x1 + y1) / (x2 + y2) - (x2 + y2 == 1);
    x1 -= r * x2, y1 -= r * y2;
    ll l = (x2 + y2) / (x1 + y1) - (x1 + y1 == 1);
    x2 -= l * x1, y2 -= l * y1;
    if (r) ANS.eb(to_string(r) + "A");
    if (l) ANS.eb(to_string(l) + "B");
  }
  string S;
  while (len(ANS)) {
    S += ANS.back();
