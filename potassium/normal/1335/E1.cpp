...
int main() {
	CASE(t) {
		...
		for (int out = 1; out <= 200; out++) {
			for (int in = 1; in <= 200; in++) {
				if (in == out)
					continue;
					
				int outCnt = cnt[n][out];
				for (int outUse = 1; outUse <= outCnt / 2; outUse++) {
					int L = pos[out][outUse - 1];
					int R = pos[out][outCnt - outUse];
					chkmax(ans, 2 * outUse + cnt[R - 1][in] - cnt[L][in]);
				}
			}
		}
