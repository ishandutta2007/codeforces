...
fn lower_bound(s: &mut BTreeSet<usize>, x: usize, rem: bool) -> Option<usize> {
	if let Some(&u) = s.range(x..).next() {
		if rem { s.remove(&u); }
		Some(u)
	} else {
		None
...
fn dfs(u: usize, e: &HashSet<(usize,usize)>, s: &mut BTreeSet<usize>) -> usize {
	let mut last = 0;
	let mut sz = 1;
	while let Some(v) = lower_bound(s, last, false) {
		if !e.contains( &(u,v) ) {
			s.remove(&v);
			sz += dfs(v, e, s);
		}
...
fn main() {
	...
	let mut sol = v!([]);
	while let Some(u) = lower_bound(&mut uv, 0, true) {
		let res = dfs(u, &e, &mut uv);
		sol.push(res);
	}
	sol.sort();
