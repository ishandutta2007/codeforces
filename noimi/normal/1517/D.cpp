...
int main() {
    ...
    // vv(int, a, n, m - 1);
    // vv(int, b, n - 1, m);
    // rep(i, n) rep(j, m - 1) a[i][j] = rnd(1e6);
    // rep(i, n - 1) rep(j, m) b[i][j] = rnd(1e6);
    VV(int, a, n, m - 1);
    VV(int, b, n - 1, m);
    if(k & 1) {
        rep(i, n) {
    ...
    auto ins = [&](int i, int j) { return 0 <= min(i, j) and i < n and j < m; };
    rep(t, 11) rep(i, n) rep(j, m) cost[t][i][j] = inf<int>;
    // vvv(int, cost, 11, n, m, inf<int>);
    int s = k / 2;
    rep(i, n) {
        rep(j, m) {
            cost[0][i][j] = 0;
            rep(t, s) {
                rep2(ii, max<int>(0, i - t), min<int>(n - 1, i + t)) rep2(jj, max<int>(0, j - t + abs(i - ii)), min<int>(m - 1, j + t - abs(i - ii))) {
                    if(cost[t][ii][jj] == inf<ll>) continue;
                    int now = cost[t][ii][jj];
                    if(ii) chmin(cost[t + 1][ii - 1][jj], now + b[ii - 1][jj]);
                    if(jj) chmin(cost[t + 1][ii][jj - 1], now + a[ii][jj - 1]);
                    if(ii < n - 1) chmin(cost[t + 1][ii + 1][jj], now + b[ii][jj]);
                    if(jj < m - 1) chmin(cost[t + 1][ii][jj + 1], now + a[ii][jj]);
                }
            }
            int ans = inf<int>;
            int t = s;
            rep2(ii, max<int>(0, i - t), min<int>(n - 1, i + t)) rep2(jj, max<int>(0, j - t + abs(i - ii)), min<int>(m - 1, j + t - abs(i - ii))) {
                chmin(ans, cost[t][ii][jj] + cost[t][ii][jj]);
            }
            cout << ans << " ";
            rep2(t, 0, s) {
                rep2(ii, max<int>(0, i - t), min<int>(n - 1, i + t)) rep2(jj, max<int>(0, j - t + abs(i - ii)), min<int>(m - 1, j + t - abs(i - ii))) {
                    cost[t][ii][jj] = inf<int>;
                }
            }
        }
        cout << endl;
