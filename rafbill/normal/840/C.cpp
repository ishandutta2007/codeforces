...
int main(){
  ...
  fact[0]=1; FOR(i,N-1) fact[i+1]=((i+1)*fact[i])%MOD;
  FOR(i,N) {
    cnk[i][0]=cnk[i][i]=1;
  ...
  int n; cin>>n;
  vi A(n);
  FOR(i,n) { lli x; cin>>x; A[i]=f(x); }
  set<lli> B; for(lli x:A) B.insert(x);
  vi C(all(B));
  vi D(C.size());
  FOR(i,C.size()) FOR(j,n) if(A[j] == C[i]) D[i]++;
  dp[0][D[0]-1] = 1;
  int sz=D[0];
  FORU(i,1,D.size()-1) {
    int a=D[i];
    FOR(k,n) if(dp[i-1][k]) {
      FORU(ii,0,min<int>(a,k)) {
        int rem = a-ii;
        int npos = sz+1-k;
        assert(npos>=0);
        FORU(j,0,min<int>(rem,npos)) {
          if(ii+j==0) continue;
          (dp[i][k-ii+D[i]-(ii+j)] += mulmod(dp[i-1][k],mulmod(cnk[k][ii],mulmod(cnk[npos][j],cnk[D[i]-1][ii+j-1])))) %= MOD;
        }
      }
    }
    sz+=D[i];
  }
  lli ans=dp[D.size()-1][0];
  FOR(i,D.size()) (ans *= fact[D[i]]) %= MOD;
  cout << ans << endl;
  return 0;
}