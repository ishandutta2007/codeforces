...
//template
#define MAXN 200000
int n,d[MAXN+10],rt,td[MAXN+10],cs[MAXN+10][2];
pii f[MAXN+10],f2[MAXN+10];
...
bool ban[MAXN+10];
void dfs1(int u,int len,int bg){
	if(d[u]>2){
		b[u].pb(pii(len,bg));
		return;
	}
...
int main()
{
	...
	if(!rt){
		...
		for(i=1;i<=n;i++)
			if(!ban[i])
				t.pb(i);
		printf("%d %d\n",b[t[0]][0].second,b[t[1]][0].second);
		printf("%d %d\n",b[t[0]][1].second,b[t[1]][1].second);
		return 0;
	}
	dfs(rt,0);
	int mx;
	...
	int t1=dfs2(cs[mx][0]);
	int t2=dfs2(cs[mx][1]);
	printf("%d %d\n",b[t1][0].second,b[t2][0].second);
	printf("%d %d\n",b[t1][1].second,b[t2][1].second);
