...
mi neg_even() {
	...
	R0F(i,N) {
		V<AR<mi,2>> ndp = dp;
		ndp.eb();
		F0R(cur_size, sz(dp)) F0R(p, 2) {
			if (cur_size < K) {
				ndp.at(cur_size + 1).at(p ^ is_neg(A[i]))
					+= dp.at(cur_size).at(p) * A.at(i);
			} else {
				ndp.at(cur_size + 1).at(p)
					+= dp.at(cur_size).at(p);
			}
		}
		swap(dp, ndp);
	}
	FOR(i,K,sz(dp)) ans += dp.at(i).at(0);
	return ans;
}
...
mi neg_odd() {
	...
	F0R(i,N) {
		int j = i-1;
		while (j >= 0 && is_neg(A[j]) != is_neg(A[i])) --j;
		if (j != -1) {
	...
	// dbg("HUH", num_below);
	vmi po2(N+1);
	po2[0] = 1; FOR(i,1,N+1) po2[i] = po2[i-1] * 2;
	...
	R0F(i,N) {
		{
			...
			vmi count(K);
			if (is_neg(A[i])) {
				F0R(j,K-1) count.at(j + 1) = dp.at(j).at(0);
			} else {
				F0R(j,K-1) {
			...
			R0F(i2,i) if (is_neg(A[i2]) != is_neg(A[i])) {
				R0F(j,K) {
					if (j == K-1) {
						// A.at(i)
						// A.at(i2)
						ans += count.at(j) * A.at(i) * A.at(i2);
						F0R(x,i2) if (is_neg(A.at(x)) == is_neg(A.at(i))) {
							ans += count.at(j) * A.at(x) * A.at(i) * po2[num_below.at(x)];
						}
						F0R(y,i2) if (is_neg(A.at(y)) == is_neg(A.at(i2))) {
	...
	F0R(i2,N) {
		vi all_y; F0R(y,i2) if (is_neg(A.at(y)) == is_neg(A.at(i2))) all_y.pb(y);
		for (auto [i, contrib] : tuples[i2]) {
			mi tot_sum_y = 0;
			for (int y: all_y) tot_sum_y += A.at(y) * po2[num_below.at(y)];
			mi tot_sum_po = 0;
			int iy = 0;
			F0R(x,i2) if (is_neg(A.at(x)) == is_neg(A.at(i))) { // SPEED UP
