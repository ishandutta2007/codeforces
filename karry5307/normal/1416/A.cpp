...
inline void solve()
{
	...
	{
		x[i]=read(),res[i]=-1,barrel[i].clear();
	}
	for(register int i=1;i<=n;i++)
	{
		barrel[x[i]].pb(i);
	...
	for(register int i=1;i<=n;i++)
	{
		barrel[i].pb(n+1),dis=0;
		for(register int j=0;j<barrel[i].size();j++)
		{
			prv=j?barrel[i][j-1]:0,dis=max(dis,barrel[i][j]-prv);
		}
		while(lst>dis)
		{
			res[--lst]=i;
