...
int calc(){
	...
	for(i=0;i<3;i++){
		...
		if(!ps[y].size())
			...
			ans+=ps[w].size();
			if(*ps[y].begin()<*ps[s].begin())
				ans-=get_sum(c[w],*ps[s].begin())-get_sum(c[w],*ps[y].begin());
			if(*prev(ps[s].end())<*prev(ps[y].end()))
				ans-=get_sum(c[w],*prev(ps[y].end()))-get_sum(c[w],*prev(ps[s].end()));
		}
	}
	return ans;
}
