...
void solve()
{
    ...
    for (auto c: s)
    {
        ...
        {
            //cout<<"Cand "<<cand<<endl;
            if (nxt.find(cand)!=nxt.end()) continue;
            if (dsu.find_set(cand-'a') == dsu.find_set(c-'a') && dsu.sz[dsu.find_set(c-'a')]!=26) continue;

            dsu.union_sets(cand-'a', c-'a');
            prv[c] = cand; nxt[cand] = c; ans+=cand; break;
        }
    }
