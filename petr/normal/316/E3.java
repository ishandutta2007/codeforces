...
class TaskE1 {
    ...
    static class Summator {
        ...
        public void rangeUpdate(int left, int right, int by) {
            internalRangeUpdate(0, 0, n - 1, left, right, by);
        }

        private void internalRangeUpdate(int root, int rl, int rr, int left, int right, int by) {
            if (left > right) return;
            if (rl == left && rr == right) {
                delta[root] = (delta[root] + by) % MODULO;
                sum[root] = (int) ((sum[root] + by * (long) unitSum[root]) % MODULO);
                return;
            }
            killDelta(root);
            int rm = (rl + rr) / 2;
            internalRangeUpdate(root * 2 + 1, rl, rm, left, Math.min(right, rm), by);
            internalRangeUpdate(root * 2 + 2, rm + 1, rr, Math.max(rm + 1, left), right, by);
            sum[root] = (sum[root * 2 + 1] + sum[root * 2 + 2]) % MODULO;
        }

        private void killDelta(int root) {
            if (delta[root] == 0) return;
            delta[root * 2 + 1] = (delta[root * 2 + 1] + delta[root]) % MODULO;
            delta[root * 2 + 2] = (delta[root * 2 + 2] + delta[root]) % MODULO;
            sum[root * 2 + 1] = (int) ((sum[root * 2 + 1] + delta[root] * (long) unitSum[root * 2 + 1]) % MODULO);
            sum[root * 2 + 2] = (int) ((sum[root * 2 + 2] + delta[root] * (long) unitSum[root * 2 + 2]) % MODULO);
            delta[root] = 0;
        }
    }
        ...
        for (int i = 0; i < m; ++i) {
            ...
            switch (t) {
                ...
                case 3: {
                    ...
                    int r = in.nextInt() - 1;
                    int d = in.nextInt();
                    s1.rangeUpdate(l, r, d);
                    s2.rangeUpdate(l, r, d);
                    break;
                }
            }
        }
