...
void solve(){
	...
	multiset<pair<long long,long long>> doub, sing;
	...
	for(int i = 0; i < n; ++i){
		...
		if (d < 0){
			...
			if (sing.find({d,t}) != sing.end()){
				auto it = sing.find({d, t});
				sing.erase(it);
			}
			else{
				auto it = doub.find({d,t});
				doub.erase(it);
				ext_sum -= d;
			}
			if (t == 0){
			...
			if (sing.size() == 0){
				...
				auto mx_sing = *(sing.rbegin());
				if (mx_sing.first < d){
					doub.insert({d, t});
					ext_sum += d;
				}
				else{
					auto it1 = sing.find(mx_sing);
					sing.erase(it1);
					doub.insert(mx_sing);
					ext_sum += mx_sing.first;
					sing.insert({d,t});
				}
		...
		if (cnt > 0){
			if (fire.size() == 0){
				ans += ext_sum - ((*(doub.begin())).first);
			}
			else{
				auto it = *fire.rbegin();
				pair<long long,long long> X = {it, 0ll};
				ans += ext_sum;
				if (doub.find(X) == doub.end()){
					ans -= ((*(doub.begin())).first);
					ans += it;
				}
			}
		}
