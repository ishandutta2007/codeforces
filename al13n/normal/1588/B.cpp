...
/*
LL ask(LL a,LL b){
  const LL n=INF,p=0,q=2,r=INF;
  LL ans=0;
  for(pll x:{mp(p,q),mp(q,r)}){
    LL c=min(b,x.Y)-max(a,x.X);
    if(c<=1) continue;
    ans+=c*(c-1)/2;
  }
  return ans;
...
array<LL,3> solve(LL n){
  ...
  LL q;
  {
    ...
    while(b>a+1){
      LL k=(a+b)/2;
      if(k*(k-1)+(v0-k)*(v0-k-1)<=s1) b=k; else a=k;
    }
    assert(b*(b-1)+(v0-b)*(v0-b-1)==s1);
    q=b;
  }
  dbg(q);

  LL s2=2*ask(p,p+v0-q);
  if(s2==(v0-q)*(v0-q-1)) q=v0-q;
  dbg(q);
  s0-=q*(q-1);
  LL w=isqrt(s0)+1;
  dbg(s0,w);
