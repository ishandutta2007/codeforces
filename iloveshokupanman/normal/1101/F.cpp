...
int main() {
    ...
    rep(i, N)for (ll j = i; j < N; j++){
        ...
        repn(k, j - i) {
            ...
            while (r - l > 1) {
                ll m = (r + l) / 2;
                if (a[j] - a[m] > dp[i][m][min(m-i,k-1)]) { l = m; }
                else { r = m; }
            }
            
            dp[i][j][k] = max(a[j] - a[l], dp[i][l][min(l-i,k-1)]);
            
            dp[i][j][k] = min(dp[i][j][k], max(a[j] - a[r], dp[i][r][min(r-i,k-1)]));
    ...
    ll ans = 0;
    rep(i, M) {
        ll now = dp[s[i]-1][f[i]-1][min(r[i]+1,f[i]-s[i])] * c[i];
        //cout << now << endl;
        ans = max(ans, now);
    }
