...
mint div2path(int n,int p){
	if(p==0)return n==0;
	return binom(n-p,p-1)*fact[n]*finv[p];
}
...
void slv(){
	...
	rep(ps,n+1){
		...
		rep(p,ps+1){
			int single=ps-p;
			mint w=div2path(n-single,p);
			ans+=parity(single)*mint(p+single-1).pow(single)*np*w*choose(n,single);
			np*=(n-1);
		}
	}
	print(ans);
