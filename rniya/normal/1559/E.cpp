...
int main() {
    ...
    vector<mint> dp(m + 1, 0), ndp(m + 1, 0);
    auto calc = [&](int x) -> mint {  // gcd(a) >= x 
        int M = m / x;
        dp[0] = 1;
        for (int i = 0; i < n; i++) {
            int L = ceil(l[i], x), R = floor(r[i], x);
            for (int j = 0; j <= M; j++) {
                if (j + L <= M) ndp[j + L] += dp[j];
                if (j + R + 1 <= M) ndp[j + R + 1] -= dp[j];
                dp[j] = 0;
            }
            for (int j = 0; j < M; j++) ndp[j + 1] += ndp[j];
            swap(dp, ndp);
        }
    ...
    vector<mint> sum(m + 1);  // gcd(a) = i 
    for (int i = m; i > 0; i--) {
        sum[i] = calc(i);
        for (int j = i + i; j <= m; j += i) sum[i] -= sum[j];
    }
    mint ans = sum[1];
