...
map<int, map<int, pair<int, int>>> mapka;

void ins(int cur, int nxt, int steps, int pos)
{
    if (mapka.find(nxt)==mapka.end()) mapka[nxt] = map<int, pair<int, int>>();

    if (mapka[nxt].find(cur)==mapka[nxt].end())
    {
        mapka[nxt][cur] = mp(steps, pos);
    }

    if (mapka[nxt][cur].first<steps) mapka[nxt][cur] = mp(steps, pos);
}
...
void solve()
{
    ...
    for (auto mapochka: mapka)
    {
        vector<pair<int, int>> who;
        for (auto it: mapochka.second) who.push_back(it.second);
        sort(who.begin(), who.end());
        if (who.size()>=2)
        {
            int k = who.size();
            if (who[k-2].first + who[k-1].first > m)
            {
                m = who[k-2].first + who[k-1].first;
                ans = mp(who[k-2].second, who[k-1].second);
            }
        }
    }

    cout<<ans.first+1<<' '<<ans.second+1<<' '<<m<<endl;
