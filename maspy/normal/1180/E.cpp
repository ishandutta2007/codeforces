...
void solve() {
  ...
  auto from_elem = [&](ll a) -> pi { return {min<ll>(0, a), a}; };

  vc<pi> seg_raw(LIM);
  FOR(x, LIM) seg_raw[x] = from_elem(C[x]);
  SegTree<Mono> seg(seg_raw);

  auto upd = [&](ll x) -> void { seg.set(x, from_elem(C[x])); };
  ...
  FOR(Q) {
    ...
    auto check = [&](auto e) -> bool { return e.fi >= 0; };
    ll l = seg.min_left(check, LIM);
    print(l - 1);
  }
}
