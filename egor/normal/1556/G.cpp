...
class GVrataVDrugoiMir {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        arr<string> type(m, string());
        arr<ll> f(m);
        arr<ll> s(m);
        for (int i : range(m)) {
            type[i] = in.readString();
            f[i] = in.readLong();
            s[i] = in.readLong();
        }
        ...
        vector<ll> npoi;
        vi sz;
        for (int i : range(poi.size() - 1)) {
            ll cur = poi[i];
            while (cur != poi[i + 1]) {
                npoi.push_back(cur);
                for (int j : RevRange(n)) {
                    if (cur + (1ll << j) <= poi[i + 1] && (cur & ((1ll << j) - 1)) == 0) {
                        cur += 1ll << j;
                        sz.push_back(j);
                        break;
                    }
        ...
        vi openAt(npoi.size(), m - 1);
        for (int i : range(m)) {
            if (type[i][0] == 'b') {
                int pos = lower_bound(all(npoi), f[i]) - npoi.begin();
                while (pos < npoi.size() && npoi[pos] <= s[i]) {
                    openAt[pos++] = i;
                }
            }
        }
        arr<vector<pii>> edges(m, vector<pii>());
        for (int i : range(npoi.size())) {
            for (int j : range(sz[i], n)) {
                ll pos = npoi[i] ^ (1ll << j);
                int p = upper_bound(all(npoi), pos) - npoi.begin() - 1;
                if (sz[p] > sz[i] || sz[p] == sz[i] && p > i) {
                    edges[min(openAt[i], openAt[p])].emplace_back(i, p);
                }
            }
        }
        vi answer;
        ...
        for (int i : RevRange(m)) {
            ...
            if (type[i][0] == 'a') {
                int p1 = upper_bound(all(npoi), f[i]) - npoi.begin() - 1;
                int p2 = upper_bound(all(npoi), s[i]) - npoi.begin() - 1;
                answer.push_back(dsu[p1] == dsu[p2] ? 1 : 0);
            }
        }
        reverse(all(answer));
        for (int i : answer) {
