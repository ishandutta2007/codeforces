...
int main(){
	...
	cauto n=read();
	cauto vs=readV(n,0);
	const int N=5000456; //20000567;
	vector<int> cnt(N);
	for(cauto &v:vs) ++cnt[v];
	vector<int> sum(N);
	reps(i,1,N)for(int j=i;j<N;j+=i) sum[i]+=cnt[j];
	vector<ll> re(N);
	rreps(i,1,N){
		MX(re[i], ll(sum[i])*i);
		for(int j=i+i;j<N;j+=i) MX(re[i],re[j]+ll(sum[i]-sum[j])*i);
	}
	rep(i,10) out(i,re[i],sum[i],1);
	cout<<*max_element(all(re)) NL;
	return 0;
}