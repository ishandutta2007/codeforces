...
int main()
{
	...
	for (int i=0;i<n;++i)
	{
		r1.erase(r1.find(a[i]));
		c1.erase(c1.find(b[i]));
		r2.erase(r2.find(c[i]));
		c2.erase(c2.find(d[i]));
		int l3=*(r1.rbegin()),r3=*(r2.begin());
		int u3=*(c1.rbegin()),d3=*(c2.begin());
		if (l3<=r3 && u3<=d3)
		{
			cout<<l3<<' '<<u3<<endl;
			return 0;
