...
struct SegTree{
	...
	SEG ope(SEG a, SEG b){ //operator
		return P(a.fi+b.fi, max(a.se, b.se));
	}
...
int main(void)
{
	...
	seg.init();
	rep(i, 1, n) seg.set(Q+i, P(1, i));
	seg.setup();
	
	ll x, y;
	rep(i, 1, Q){
		cin >> x >> y;
		ll pos = seg.right_find(0, P(y, -inf));
		ll id = seg.query(pos, pos).se;
		if(p[id] != 0 && p[id] != x){
			outl(-1);
			return 0;
		}
		p[id] = x;
		seg.update(Q+1-i, P(1, id));
		seg.update(pos, P(0, 0));
	}
	
	set<ll> S;
	rep(i, 1, n) S.insert(i);
	rep(i, 1, n) S.erase(p[i]);
	rep(i, 1, n) if(p[i] == 0) p[i] = *S.begin(), S.erase(S.begin());
	if(sz(S)) outl(-1);
	else outa(p, 1, n);
