...
fun main(){
    solve.cases{
        ...
        val items = multiset()
        val wantsize = 2 * n + m + 5
        val st = longMaxLazy(LongArray(wantsize){it - 1L})
        ...
        repeat(m){
            ...
            val delta = if(adding) 1 else -1
            


            st.rangeAdd(0,v,delta.toLong())
...
//            st.lazyPrint()
            ...
            val max = items.st.lastKey()
            val ans = st.rangeQueryLazy(0,max)
