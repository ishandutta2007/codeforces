...
public class B {
	...
	{
		int n = ni(), m = ni(), K = ni();
		int[] from = new int[m+K];
		int[] to = new int[m+K];
		int[] w = new int[m+K];
		int[] info = new int[m+K];
		for(int i = 0;i < m;i++){
			from[i] = ni()-1;
			to[i] = ni()-1;
			w[i] = ni();
	...
	public static long[] dijk(int[][][] g, int from)
	{
		int n = g.length;
		long[] td = new long[n];
		Arrays.fill(td, Long.MAX_VALUE / 100);
	...
	public static int[][][] packWU(int n, int[] from, int[] to, int[]... w) {
		...
		int wc = w.length;
		for(int i = 0;i < n;i++)
			g[i] = new int[p[i]][wc+1];
		for(int i = 0;i < from.length;i++){
			--p[from[i]];
			g[from[i]][p[from[i]]][0] = to[i];
			for(int j = 0;j < wc;j++)g[from[i]][p[from[i]]][1+j] = w[j][i];
			--p[to[i]];
			g[to[i]][p[to[i]]][0] = from[i];
			for(int j = 0;j < wc;j++)g[to[i]][p[to[i]]][1+j] = w[j][i];
		}
		return g;
	}
