...
void solve() {
    ...
    vector<int> bal(n);
    vector<int> cnt(n);
    for (auto i : edge) ++cnt[i.f], ++cnt[i.s];
    auto get = [&](pii p) {
        return abs(bal[p.f] - bal[p.s]);
    };
    vector<pii> ans;
    while (sz(edge)) {
        int ind = 0;
        int val = get(edge[0]);
        for (int i = 1; i < sz(edge); ++i)
            if (chmax(val, get(edge[i])))
                ind = i;
        pii add = edge[ind];
        edge.erase(edge.begin() + ind);
        if (bal[add.f] > bal[add.s]) swap(add.f, add.s);
        ans.pb({add});
        ++bal[add.f];
        --bal[add.s];
        for (int i = 0; i < n; ++i)
