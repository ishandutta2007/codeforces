...
int check(int d){
	...
	int l0=-N*2,l1=N*2;
	For(i,1,n){
		r=intersect(r,{cx[i]-d,cy[i]-d,cx[i]+d,cy[i]+d});
		if (r.empty())
			return 0;
		ckmax(l0,cx[i]-cy[i]-d);
		ckmin(l1,cx[i]-cy[i]+d);
	}
	outvals(l0,l1);
	outvals(r.x0,r.x1,r.y0,r.y1);
	if (l0>l1)
		return 0;
	if (max(l0,r.x0-r.y1)<=min(l1,r.x1-r.y0)){
		int v=max(l0,r.x0-r.y1);
		int x=max(r.x0,v+r.y0);
		int y=x-v;
		assert(r.x0<=x&&x<=r.x1);
		assert(r.y0<=y&&y<=r.y1);
		outvals(v,x,y);
		res[d]=mp(x,y);
		return 1;
	}
