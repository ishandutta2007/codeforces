...
int main() {
  ...
  f0r(i, n) {
    while (lpt < m && sleft[lpt].f == i) {
      st.update(sleft[lpt].f, sleft[lpt].s, 1);
      ++lpt;
    }

    c[i] = a[i] - st.query(0, n - 1).val;
    ans = max(ans, c[i]);

    while (rpt < m && sright[rpt].f == i) {
      st.update(sright[rpt].s, sright[rpt].f, -1);
      ++rpt;
    }
  }
