  ...
  rep(i,N*2+1){
    for(auto x:edge){
      ...
      if(x.first){
        ...
        chmin(ma[a],ma[b]-1);
        chmax(mi[a],mi[b]-1);
        if(ma[a]-mi[a]<0)return 0;
      }else{
        chmin(ma[b],ma[a]+1);
        chmax(mi[b],mi[a]-1);
        chmin(ma[a],ma[b]+1);
        chmax(mi[a],mi[b]-1);
        if(ma[a]-mi[a]<0||ma[b]-mi[b]<0)return 0;
      }
    }
  }
  if(chmax(ans,*max_element(all(ma))))anslis=ma;
  return 1;
}
int main(){
  startupcpp();
  ...
  rep(i,N)solve(i);
  rep(j,N)if(dsu.parcheck(j,j+N))break;
  if(j==N&&ans>=0){
    printYES;
    print(ans);
    output(anslis);
