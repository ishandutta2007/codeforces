...
int main() {
    LL(n, m);
    double s = 1.0 / (n * n);
    vv(double, mem, n + 1, n + 1);
    vv(bool, vis, n + 1, n + 1);
    auto rec = [&](auto &&f, int x, int y) -> double {
        if(x < 0 or y < 0) return 0;
        if(vis[x][y]) return mem[x][y];
        vis[x][y] = true;
        if(x == 0 and y == 0) return 0;
        double k = s * (n * (x + y) - x * y);
        return mem[x][y] = (s * (x * y * f(f, x - 1, y - 1) + x * (n - y) * f(f, x - 1, y) + (n - x) * y * f(f, x, y - 1))) / k + 1 / k;
    };
    vi x(n), y(n);
    while(m--) {
        INT(a, b);
    ...
    cout << rec(rec, count(all(x), 0), count(all(y), 0)) << endl;
}