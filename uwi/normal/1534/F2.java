...
public class F2 {
	...
	void solve()
	{
		...
		int[] from = new int[4*n*m];
		int[] to = new int[4*n*m];
		int p = 0;
		for(int i = 0;i < n;i++){
			for(int j = 0;j < m;j++){
				if(i < n-1){from[p] = i*m+j; to[p] = (i+1)*m+j; p++;}
				if(map[i][j] == '#'){
					if(i-1 >= 0){from[p] = i*m+j; to[p] = (i-1)*m+j; p++;}
					if(j+1 < m){from[p] = i*m+j; to[p] = i*m+j+1; p++;}
					if(j-1 >= 0){from[p] = i*m+j; to[p] = i*m+j-1; p++;}
				}
			}
		}
		int[] a = na(m);
		...
		int[] minsrc = new int[u];
		int[] maxsrc = new int[u];
		Arrays.fill(minsrc, 999999999);
		Arrays.fill(maxsrc, -1);
		for(int i = 0;i < m;i++){
			for(int j = 0;j < n;j++){
				if(map[j][i] == '#'){
					int c = clus[j*m+i];
					minsrc[c] = Math.min(minsrc[c], i);
					maxsrc[c] = Math.max(maxsrc[c], i);
					break;
				}
			}
		}
		for(int i = u-1;i >= 0;i--){
			for(int e : cg[i]){
				minsrc[e] = Math.min(minsrc[e], minsrc[i]);
				maxsrc[e] = Math.max(maxsrc[e], maxsrc[i]);
			}
		}
		...
		int q = 0;
		for(int i = 0;i < m;i++){
			if(sinks[i] == -1)continue;
			rs[q++] = new int[]{minsrc[clus[sinks[i]]], maxsrc[clus[sinks[i]]]};
			assert minsrc[clus[sinks[i]]] <=  maxsrc[clus[sinks[i]]];
		}
		rs = Arrays.copyOf(rs, q);
		Arrays.sort(rs, (x, y) -> x[1] - y[1]);
