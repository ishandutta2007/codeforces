	...
	public void precalc(int n) {
		...
		for (int len = 1; len <= n; len++) {
			for (int mask = 0; mask < 1 << len; mask++) {
				...
				for (int l = 0; l < len; l++) {
					f: for (int r = l; r < len; r++) {
						for (int x = l + 1; x <= r; x++) {
							if (s.charAt(x) == s.charAt(x - 1)) {
								continue f;
							}
						}
						String prev = s.substring(0, l) + s.substring(r + 1);
						if (!map.containsKey(s) || map.get(s) > map.get(prev) + 1) {
...
//		for (int l = 0; l < n; l++) {
//			for (int r = l; r < n; r++) {
			int n0 = pref0[r] - pref0[l];
			int n1 = pref1[r] - pref1[l];
//				if (Math.max(n0, n1) + 1 != map.get(s.substring(l, r + 1))) {
//					System.out.println(s + " " + l + " " + r + " " + (Math.max(n0, n1) + 1) + " " + n0 + " " + n1 + " " + map.get(s.substring(l, r + 1)));
//					return;
//				}
			out.println(Math.max(n0, n1) + 1);
//			}
		}
	}
