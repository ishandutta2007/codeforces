...
int main()
{
    ...
    fac[0]=1;
    inv[0]=1;
    for(int i=1;i<200;i++) fac[i]=fac[i-1]*i%mod,inv[i]=quick(fac[i],mod-2);
    ll D;
    cin>>D;
    vc<ll> iz;
    ...
    while(q--)
    {
        ll u,v;
        cin>>u>>v;
        auto getcnt=[&](ll x,ll y){
            ll ret=1;
            int cc=0;
            for(auto &u:ls[x])
            {
                if(ls[y].count(u.first))
                {
                    cc+=u.second-ls[y][u.first];
                    ret=ret*inv[u.second-ls[y][u.first]]%mod;
                }
                else{
                    cc+=u.second;
                    ret=ret*inv[u.second]%mod;
                }
            }
            ret=ret*fac[cc]%mod;
            return ret;
        ...
        ll g=__gcd(u,v);
        print(getcnt(u,g)*getcnt(v,g)%mod);
    }
}