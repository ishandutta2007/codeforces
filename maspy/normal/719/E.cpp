...
void solve() {
  ...
  vc<E> POW2(B + 1);
  vc<E> POW3(B + 1);
  POW1[0] = POW2[0] = POW3[0] = mp(mint(1), mint(0));
  POW1[1] = mp(mint(0), mint(1));
  FOR(i, B) POW1[i + 1] = mul(POW1[i], POW1[1]);
  FOR(i, B) POW2[i + 1] = mul(POW2[i], POW1[B]);
  FOR(i, B) POW3[i + 1] = mul(POW3[i], POW2[B]);
  auto calc_pow = [&](ll n) -> E {
    E x = POW3[n >> 32];
    E y = POW2[(n & ((1LL << 32) - 1)) >> 16];
    E z = POW1[n & ((1LL << 16) - 1)];
    return mul(mul(x, y), z);
  };

  using OP = ll;
  SegmentTree<E, OP> seg([](E x, E y) { return mp(x.fi + y.fi, x.se + y.se); },
                         [&](E x, OP y) { return mul(calc_pow(y), x); },
                         [](OP x, OP y) { return x + y; }, mp(mint(0), mint(0)),
                         0);
  LL(N, Q);
  VEC(ll, A, N);
  seg.init(N);
  FOR(i, N) seg.set_val(i, calc_pow(A[i]));
  FOR(_, Q) {
    LL(t, L, R);
    --L;
    if (t == 2) {
