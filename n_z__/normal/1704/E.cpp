...
vector<pair<int,int>>uu[n+1];
int t=0;
int ans=0;
for(auto q:tp)
{
    //cout<<q<<"::"<<endl;
    if(a[q]!=0)uu[q].push_back({1,a[q]});
    if(uu[q].size()==0)continue;
    sort(uu[q].begin(),uu[q].end());
    pair<int,int>now=uu[q][0];
    vector<pair<int,int>>pd;
    for(int x=1;x<uu[q].size();x++)
    if(uu[q][x].first<=now.second+1)now.second+=uu[q][x].second-uu[q][x].first+1;
    else pd.push_back(now),now=uu[q][x];
    if(now.second!=0)pd.push_back(now);
    uu[q].clear();
    /*cout<<q<<':';
    for(auto ww:pd)
    cout<<ww.first<<','<<ww.second<<';';cout<<endl;*/
    for(auto w:e[q])
    for(auto ww:pd)
    uu[w].push_back({ww.first+1,ww.second+1});
    if(e[q].size()==0)ans=pd[pd.size()-1].second;
}
ex(ans%998244353);
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
