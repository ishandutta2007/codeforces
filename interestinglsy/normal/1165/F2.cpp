...
#define Flip(mask,pos) ((mask)^(1<<(pos)>>1))
#define MOD (998244353LL)
///////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
#define MAXN 262144
...
bool Check( int lim ){
	// static int offcnt[MAXN];
	// Ms(offcnt);
	// for( auto [d,t] : off ){
	// 	if( d > lim ) break;
	// 	++offcnt[t];
	// }
	static int tneed[MAXN];
	For(i,n) tneed[i] = need[i];
	int money = lim, costed = 0;
	fOR(i,min(lim,200001)){
		Mymin(money,i);
		for( auto tp : canoff[i] ){
			while(1){
				if(!tneed[tp]) break;
				if(!money) break;
				--money;
	...
	int still = accumulate(tneed+1,tneed+1+n,0);
	return still*2 <= lim - costed;
}
int Binarysearch(){
	int l = 1, r = 600000, mid, ans = INF;
	while( l <= r ){
		mid = l+r >> 1;
		if(Check(mid)){
			ans = mid;
