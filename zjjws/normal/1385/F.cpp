...
{
    cut[i]=nxt[i].size();
    for(auto to:nxt[i]){if(to!=fa)dfs(to,i);if(nxt[to].size()==1)lar[i]++,son[i].push_back(to);}return;
}
priority_queue<pair<int,int> >d;
inline void work()
{
    rin>>n>>k;ans=0;
    for(int i=1;i<=n;i++)nxt[i].clear(),son[i].clear(),lar[i]=0;
    for(int i=1,x,y;i<n;i++){rin>>x>>y;nxt[x].push_back(y);nxt[y].push_back(x);}
    if(k==1){printf("%d\n",n-1);return;}
    if(n==2){printf("%d\n",0);return;}
    dfs(1,0);for(int i=1;i<=n;i++)if(lar[i])d.push(make_pair(lar[i],i));
    for(int now;!d.empty();)
    {
        for(;!d.empty();d.pop())if(lar[d.top().second]==d.top().first)break;if(d.empty())break;
        now=d.top().second;d.pop();if(lar[now]<k)continue;
        int cutt=lar[now]/k;ans+=cutt;lar[now]-=cutt*k;cut[now]-=cutt*k;
        for(;cutt;cutt--)for(int i=1;i<=k;i++)cut[son[now].back()]=0,son[now].pop_back();
        if(cut[now]==1){for(auto i:nxt[now])if(cut[i])son[i].push_back(now),lar[i]++,d.push(make_pair(lar[i],i));}
        else d.push(make_pair(lar[now],now));
    }
    for(;!d.empty();d.pop());printf("%d\n",ans);return;
}
int main()
{
