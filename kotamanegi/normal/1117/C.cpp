...
string s;
bool check(int itr) {
    pair<int, int> now = nows;
    REP(i, s.length()) {
        int moves = itr / (int)s.length();
        if (i < itr % (int)s.length())moves++;
        if (s[i] == 'R') {
            now.first += moves;
        }
        else if (s[i] == 'L') {
    ...
    int dist = abs(now.first - goal.first) + abs(now.second - goal.second);
    dist -= itr;
    dist *= -1;
    if (dist >= 0) {
        return 1;
