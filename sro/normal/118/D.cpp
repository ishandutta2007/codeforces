...
//Use dp. dp[i][j] means the number of ways when there are i footmen and j horsemen.
int dp[107][107][2];
int n1,n2,k1,k2;
	...
	for(int i=1;i<=n1;i++)
		for(int j=1;j<=n2;j++)
		{
			for(int k=1;k<=min(k1,i);k++)
				dp[i][j][0]=(dp[i][j][0]+dp[i-k][j][1])%100000000;
			for(int k=1;k<=min(k2,j);k++)
				dp[i][j][1]=(dp[i][j][1]+dp[i][j-k][0])%100000000;
		}
