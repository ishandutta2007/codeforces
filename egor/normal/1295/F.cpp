...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        arri zero(1, -1);
        vi pts = compress(l, r, zero);
        arr3d<modint> answer(n + 1, n + 1, pts.size() - 1, 0);
        arr2d<modint> lenInvPow(pts.size() - 1, n + 1);
        for (int i : range(lenInvPow.dim1())) {
            lenInvPow(i, 0) = 1;
            modint inv = modint(pts[i + 1] - pts[i]).inverse();
            for (int j : range(n)) {
                lenInvPow(i, j + 1) = lenInvPow(i, j) * inv;
            }
        }
        arr2d<modint> ways(pts.size() - 1, n + 1);
        for (int i : range(lenInvPow.dim1())) {
            ways(i, 0) = 1;
            modint mult = pts[i + 1] - pts[i];
            for (int j : range(n)) {
                ways(i, j + 1) = ways(i, j) * mult * modint(j + 1).inverse();
                mult += 1;
            }
        }
        answer(0, 0, 0) = 1;
        for (int i : range(n)) {
            int len = pts[r[i]] - pts[l[i]];
            modint invLen = modint(len).inverse();
            for (int j : range(i + 1)) {
                modint sum = 0;
                for (int k : range(r[i])) {
                    if (k >= l[i]) {
                        answer(i + 1, j + 1, k) += answer(i, j, k) * invLen;
                        answer(i + 1, 1, k) += sum * invLen;
                    }
                    sum += answer(i, j, k) * ways(k, j);// * lenInvPow(k, j);
                }
            }
        }
        modint ans = 0;
        for (int j : range(n + 1)) {
            for (int k : range(r[n - 1])) {
                ans += answer(n, j, k) * ways(k, j);// * lenInvPow(k, j);
            }
        }
        out.printLine(ans);
    }
