...
public:
    void solve() {
        ...
        int maxDeg = *max_element(all(deg));
        vector<pii> answer;
        Graph<BiEdge> graph(n);
        for (int i : range(n - 1)) {
            graph.addEdge(u[i], v[i]);
        }
        function<void(int, int, int)> dfs = [&](int vert, int last, int time) {
            int exitTime = time - 1;
            for (auto* e : graph[vert]) {
                ...
                answer.emplace_back(vert + 1, time);
                if (time == maxDeg) {
                    time = maxDeg - deg[vert];
                    answer.emplace_back(vert + 1, time);
                }
                dfs(next, vert, ++time);
            }
            answer.emplace_back(vert + 1, time);
            if (time != exitTime && vert) {
                time = exitTime;
                answer.emplace_back(vert + 1, time);
            }
        };
