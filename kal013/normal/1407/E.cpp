...
struct Node{
	...
	friend ostream& operator << (ostream& os, const Node& p) {return os<<"( "<<p.n << ", " << p.dist <<" )";}
...
void solve(){
	...
	for(int i = 0;  i < m; ++i){
		...
		adj[(u << 1) + w].push_back((v << 1));
		adj[(u << 1) + w].push_back((v << 1) | 1);

		rev[v].push_back((u << 1) + w);
	}
	...
	for(int i = 0;  i < bfs.size(); ++i){
		int cur = bfs[i].n, dis = bfs[i].dist;
