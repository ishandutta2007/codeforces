...
int main() {
    C.init();
    setIO(); re(k); a.rsz(k); re(a); sort(all(a));
    int ind = 0;
    while (ind < k && a[ind] >= ind) ind ++;
    mi ans = 0;
    if (ind == k) {
        int sum = 0; 
        trav(t,a) {
        ...
        F0R(j,k) {
            sum -= k;
            sum += k*cnt[k-1-j];
            ans += C.comb(sum/k+k-1,k-1);
            // those equal to k-1-j
        }
    } else {
        int cur = 0;
        F0R(j,ind) {
            while (cur < sz(a) && a[cur] < j) cur ++;
            ans += C.comb(j-cur+k-1,k-1);
            // add[0]+add[1]+...+add[k-1] = j-(#<j)
            // j-cur
        }
    }
    ps(ans);
