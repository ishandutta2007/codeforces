...
void fill_divisors(int c, const vpui& v, const int index, vi& ans) {
	if (index >= v.size()) {
		ans.push_back(c);
		return;
	}

	for (int pw = 0; pw <= v[index].Y; ++pw) {
		fill_divisors(c, v, index + 1, ans);
		c *= v[index].X;
	}
}

vi find_divisors(const vpui& v) {
	vi ans;
	fill_divisors(1, v, 0, ans);
	return ans;
}
...
int another_main()
{
	...
	for (int I = 0; I < t; ++I) {
		...
		ll ans = 0;
		for (int i = l; i <= r; ++i) {
			ll number_of_divisors = lower_bound(whole(div[i]), i) - lower_bound(whole(div[i]), l);
			ans += number_of_divisors * (number_of_divisors - 1) / 2;
		}
		int first_type = 0, second_type = 0;
		if (r >= 2 * l)
			first_type = r / 6 - (l - 1) / 3;
		int l_prime = (l + 1) / 2;
		if (r >= 5 * l_prime)
			second_type = r / 15 - (l_prime - 1) / 3;
		//cout << ans << '\n';
		cout << (ll(r - l + 1) * ll(r - l) * ll(r - l - 1)) / 6 - ans - first_type - second_type << '\n';
	}
	return 0;
}
