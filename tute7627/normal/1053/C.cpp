...
int main(){
  ...
  rep(i,0,n)cin>>w[i];
  BIT<ll>ws(n);
  rep(i,0,n)ws.add(i,w[i]);
  BIT<modint>aws(n);
  rep(i,0,n)aws.add(i,a[i]*w[i]);
  while(q--){
    ll x,y;cin>>x>>y;
    if(x<0){
      x*=-1;
      x--;
      ws.add(x,y-ws[x]);
      aws.add(x,a[x]*y-aws[x]);
    }
    else{
      x--;
      ll idx=ws.lower_bound(ws.sum(x-1)+ws.sum(x,y-1)/2+1);
      //cout<<x spa idx spa y<<endl;
      //ws.print();
      //aws.print();
      modint ls=a[idx]*ws.sum(x,idx-1)-aws.sum(x,idx-1);
      modint rs=aws.sum(idx,y-1)-a[idx]*ws.sum(idx,y-1);
      cout<<rs+ls<<endl;
    }
  }
  return 0;
