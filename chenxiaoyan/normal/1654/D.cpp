...
void dfs(int x = 1, int fa = 0) {
	for(auto e : nei[x]) {
		int y = X(e), a = Y(e), b = Z(e);
		if(y == fa) continue;
		vii add;
		for(int i = 2; i * i <= b; ++i) if(b % i == 0) {
			add.pb(i, 0);
			while(b % i == 0) ++Y(add.back()), b /= i;
		} if(b > 1) add.pb(b, 1);
		for(int i = 2; i * i <= a; ++i) if(a % i == 0) {
			add.pb(i, 0);
			while(a % i == 0) --Y(add.back()), a /= i;
		} if(a > 1) add.pb(a, -1);
		for(tii t : add) {
			cnt[X(t)] += Y(t);
			chkmx(shd[X(t)], -cnt[X(t)]);
		}
		dfs(y, x);
		for(tii t : add) cnt[X(t)] -= Y(t);
	}
}

int ans, a0;
void dfs0(int x = 1, int now = a0, int fa = 0) {
	addto(ans, now);
	for(auto e : nei[x]) {
		int y = X(e), a = Y(e), b = Z(e);
		if(y == fa) continue;
		dfs0(y, (ll)now * inv(a) % P * b % P, x);
	}
}

void mian() {
	n = read();
	REP(i, 1, n) nei[i].clear(), shd[i] = cnt[i] = 0;
	REP(i, 1, n - 1) {
		int x = read(), y = read(), a = read(), b = read();
		nei[x].pb(y, a, b), nei[y].pb(x, b, a);
	}
	dfs();
	a0 = 1; ans = 0;
	REP(i, 1, n) a0 = (ll)a0 * qpow(i, shd[i]) % P;
	dfs0();
	prt(ans), pc('\n');
}
