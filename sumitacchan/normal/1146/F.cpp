...
public:
    ...
    void dfs(int v){
        if(SZ(G[v]) == 0) dp[v][2] = 1;
        else{
            dp[v][0] = 1;
            for(auto e: G[v]){
                dfs(e.to);
                vec dq(3, 0);
                dq[0] = (dp[v][0] * (dp[e.to][0] + dp[e.to][2])) % mod;
                dq[1] = (dp[v][1] * (dp[e.to][0] + dp[e.to][2]) + dp[v][0] * (dp[e.to][1] + dp[e.to][2])) % mod;
                dq[2] = (dp[v][2] * (dp[e.to][0] + dp[e.to][1] + 2 * dp[e.to][2]) + dp[v][1] * (dp[e.to][1] + dp[e.to][2])) % mod;
                dp[v] = dq;
            }
        }
    }
