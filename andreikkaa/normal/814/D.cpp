...
//	cout << "d " << vv << endl;
	...
	for(int p1 = 0; p1 < 2; ++p1)
	{
		for(int p2 = 0; p2 < 2; ++p2)
		{
			...
			// 1
		//	cout << (acos(-1) * v[vv].y) * v[vv].y << endl;
	//		cout << r1 << endl;
			if(p1 == 0)
			{
				r1 += (acos(-1) * v[vv].y) * v[vv].y;
			}
			else
			{
				r1 -= (acos(-1) * v[vv].y) * v[vv].y;
	...
	//		cout << v[vv] << endl;
	//		cout << vv << " " << p1 << " " << p2 << " = " << r1 << ", " << r2 << endl;
			dp[vv][p1][p2] = max(r1, r2);
		}
	}
}
	...
	for(int i = 0; i < n; ++i)
	{
		...
		for(int j = 0; j < n; ++j)
		{
			if(i == j || v[j].y <= v[i].y)
		   	...
		   	if((v[j].x.x - v[i].x.x) * (v[j].x.x - v[i].x.x) + (v[j].x.y - v[i].x.y) * (v[j].x.y - v[i].x.y) <= v[j].y * v[j].y && v[j].y < r)
		   	{
				ind = j;
				r = v[j].y;
			}		   			
