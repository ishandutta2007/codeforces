...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		int n, m, k;
		cin >> n >> m >> k;
		combinatorics<modular> C(n << 1);
		auto power = modular::precalc_power(2, n);
		auto invpower = modular::precalc_power(1 / modular(2), n);
		modular res = 0;
		for(auto i = 1; i <= m; ++ i){
			auto init = power[i] * i * k - power[i - 1] * (i - 1) * k;
			res += C.C(n - i, m - i) * init;
		}
		cout << res * invpower[n] << "\n";
		return 0;
	};
...
/*
n >= m
dp[n][m]: max score when bob adds at least m out of n

dp[n][n] = n*k
dp[n][0] = 0

if alice choose x, she gains
min(dp[n-1][m-1]+x, dp[n-1][m]-x)
-> she must choose x = (dp[n-1][m] - dp[n-1][m-1]) / 2

dp[n][m] = (dp[n-1][m-1] + dp[n-1][m]) / 2

2^n * dp[n][m] = 2^(n-1) * (dp[n-1][m-1] + dp[n-1][m])
*/
