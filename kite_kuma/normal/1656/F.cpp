...
ll mst(const vector<ll> &a, ll t) {
	ll ret = -POW(t, 2LL) * (ll(a.size()) - 1LL);
	ll mx = *max_element(all(a));
	ll mn = *min_element(all(a));
	rep(i, int(a.size()) - 1) { ret += min((a[i] + t) * (mx + t), (a[i] + t) * (mn + t)); }
	return ret;
}

ll solve(const vector<ll> &a) {
	ll tmax = -*min_element(all(a));
	ll tmin = -*max_element(all(a));
	while(tmax - tmin > 10LL) {
		ll left = tmin + (tmax - tmin) / 2;
		ll right = left + 1;
		ll resleft = mst(a, left);
		ll resrig = mst(a, right);
		debug(left, right);
		debug(resleft, resrig);
		if(resleft > resrig) {
			tmax = right;
		} else {
			tmin = left;
	...
	ll ret = std::numeric_limits<ll>::min();
	for(ll t = tmin; t <= tmax; t++) {
		if(chmax(ret, mst(a, t))) debug(t, ret);
	}
	cerr << ret << dl;
	return ret;
}
...
int main() {
	...
	rep(t) {
		...
		sort(all(a));
		if(-amax * (n - 2) <= asum && asum <= -amin * (n - 2))
			cout << solve(a) << dl;
		else
			cout << "INF\n";
	}
