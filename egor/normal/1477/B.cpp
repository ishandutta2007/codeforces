...
class BNezzarIBinarnayaStroka {
public:
    void solve() {
        int n = in.readInt();
        int q = in.readInt();
        auto s = in.readString();
        auto f = in.readString();
        arri l, r;
        in.readArrays(q, l, r);
        decreaseByOne(l);
        ...
        init(0, 0, n);
        RecursiveFunction query = [&](const auto& self, int root, int left, int right, int from, int to) -> ll {
            ...
            if (delta[root] != -1) {
                ...
                qty[2 * root + 1] = delta[root] * (mid - left);
                delta[2 * root + 2] = delta[root];
                qty[2 * root + 2] = delta[root] * (right - mid);
                delta[root] = -1;
            }
            ll lRes = self(2 * root + 1, left, mid, from, to);
            ll rRes = self(2 * root + 2, mid, right, from, to);
            return lRes + rRes;
        };
        ...
        for (int i : RevRange(q)) {
            int qq = query(0, 0, n, l[i], r[i]);
            if (qq * 2 < r[i] - l[i]) {
                update(0, 0, n, l[i], r[i], 0);
            } else if (qq * 2 > r[i] - l[i]) {
...
int main() {
    ...
    auto time = clock();
#endif
    BNezzarIBinarnayaStroka solver;
