...
void solve() {
    ...
    REP(i, n) {
        ...
        cin >> a >> b;
        if (b >= 0) {
            plus.push_back(mp(a, b));
        }
        else {
            b *= -1;
            minus.push_back(mp(a - b, b));
        }
    }
    if (plus.size() != 0) {
        sort(ALL(plus));
    ...
    reverse(ALL(minus));
    REP(i, minus.size()) {
        for (int q = minus[i].first + minus[i].second; q <= 60000; ++q) {
            if(q - minus[i].second >= 0)
            dp[q - minus[i].second] = max(dp[q - minus[i].second], dp[q] + 1LL);
        }
    }
    REP(i, 60001) {
        ans = max(ans, dp[i]);
