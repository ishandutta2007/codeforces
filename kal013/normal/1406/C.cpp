...
void centre(int n,int p, vector<int>& centroid, int tot){
	...
	if (pos){
		centroid.push_back(n);
	}
}

pair<int,int> leaf_parent(int n,int p){

	for(auto e: adj[n]){
		if (e != p){
			return leaf_parent(e, n);
		}
	}
...
void solve(){
	...
	auto X = leaf_parent(centroid[0], centroid[1]);

	cout << X.first << " " << X.second << endl;

	cout << X.first << " " << centroid[1] << endl; 
