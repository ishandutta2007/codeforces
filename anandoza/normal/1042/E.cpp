...
int main() {
    ...
    for (int coord = 0; coord < 2; coord++) {
        vmn dp;
        vmn dp_prefix(1);
        vmn x_squared_prefix(1);
        vmn x_prefix(1);
        for (auto &[v, p] : cells) {
            mn x = p[coord];
            mn ev = 0;
            int i = lower_bound(all(cells), mp(v, P{-1, -1})) - cells.begin();
            ev += dp_prefix[i];
            ev += x_squared_prefix[i];
            ev -= 2 * x_prefix[i] * x;
            ev += i * x * x;
            if (i > 0)
                ev /= i;
            ...
            dp.push_back(ev);
            dp_prefix.push_back(dp_prefix.back() + dp.back());
            x_prefix.push_back(x_prefix.back() + x);
            x_squared_prefix.push_back(x_squared_prefix.back() + x * x);
