...
int main() {
    ...
    vi x(n),y(n);
    rep(i,n) cin >> x[i] >> y[i];
    vi dp(1<<n,1e9),pre1(1<<n,-1),pre2(1<<n,-1);
    dp[0] = 0;
    vvi dist(n,vi(n));
    rep(i,n) rep(j,n) {
        if(i == j) {
            dist[i][j] = dis(x[j],y[j],xs,ys) * 2;
        }
        else {
            dist[i][j] = dis(x[j],y[j],xs,ys) + dis(x[i],y[i],xs,ys) + dis(x[j],y[j],x[i],y[i]);
        }
    }
    rep(i,1,1<<n) {
        rep(j,n) {
            if(i >> j & 1) {
                if(chmin(dp[i],dp[i ^ (1 << j)] + dist[j][j])) {pre1[i] = j;pre2[i] = -1;}
                rep(k,j+1,n) {
                    if(i >> k & 1) {
                        if(chmin(dp[i],dp[i ^ (1 << j) ^ (1 << k)] + dist[j][k])) {pre1[i] = j,pre2[i] = k;}
                    }
                }
                break;
            }
