...
void main_() {
	...
	debug(e, o);
	vector dp(n + 1, vector(o + 1, vector(2, inf)));
	dp[0][0][0] = dp[0][0][1] = 0;
	REP(i, n) {
		if(p[i]) {
			REP(j, o + 1) {
				chmin(dp[i + 1][j][p[i] & 1], dp[i][j][p[i] & 1]);
				chmin(dp[i + 1][j][p[i] & 1], dp[i][j][!(p[i] & 1)] + !!i);
			}
		} else {
			REP(j, o + 1) {
				if(j + 1 <= o) {
					chmin(dp[i + 1][j + 1][1], dp[i][j][0] + !!i);
					chmin(dp[i + 1][j + 1][1], dp[i][j][1]);
				}
				chmin(dp[i + 1][j][0], dp[i][j][1] + !!i);
				chmin(dp[i + 1][j][0], dp[i][j][0]);
			}
		}
	}
	debug(dp);
	print(min(dp.back()[o][0], dp.back()[o][1]));
}
