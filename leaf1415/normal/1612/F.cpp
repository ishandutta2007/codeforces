...
int main(void)
{
	...
	cin >> n >> m >> Q;
	rep(i, 1, Q) cin >> a[i] >> b[i], S.insert(P(a[i], b[i]));
	...
	rep(i, 0, N) rep(j, 1, n) dp[i][j] = -inf;
	dp[0][1] = 1;
	...
	rep(i, 0, N){
		rep(j, 1, n){
			...
			ll d = j + dp[i][j];
			if(S.count(P(j, dp[i][j]))) d++;
			chmax(dp[i+1][j], min(d, m));
			chmax(dp[i+1][min(d, n)], dp[i][j]);
		}
	}
	...
	rep(i, 0, N) if(dp[i][n] >= 1) chmin(ans, i + dist2[dp[i][n]]);
	outl(ans);
