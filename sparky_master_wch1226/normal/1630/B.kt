...
fun main(){
    solve.cases{
        ...
        val leqcount = counts.copyOf()
        for(i in 0 until n){
            leqcount[i+1] += leqcount[i]
        }
        fun rangecount(l:Int, r:Int):Int {
            val rr = minOf(r,n)
            return leqcount[rr] - leqcount[l-1]
        }
        val need = (n+k+1)/2
        var bestNow = Int.MAX_VALUE
        var bestleft = -1
        var bestright = -1
        for(left in 1..n){
            val right = BinarySearchFirstTrue(left,n){
                rangecount(left,it) >= need
            }
            if(right != null ){
                if(right - left < bestNow){
                    bestNow = right - left
                    bestleft = left
                    bestright = right
                }
