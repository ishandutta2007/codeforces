...
int f[MAXN][MAXN][MAXN]; // pos, #replace, #current_t0
char s[MAXN], t[MAXN];
	...
	for (int i = 0; i < n; ++i)
		for (int j = 0; j <= k; ++j)
			for (int c = 0; c <= i; ++c)
			{
				...
				freshmax(f[i + 1][j + (s[i] != t[0])][c + 1], f[i][j][c]);
				freshmax(f[i + 1][j + (s[i] != t[1])][c + (t[0] == t[1])], f[i][j][c] + c);
				freshmax(f[i + 1][j][c], f[i][j][c]);
			}
	int res = -INF;
	for (int j = 0; j <= k; ++j)
		for (int c = 0; c <= n; ++c)
			freshmax(res, f[n][j][c]);
	writeln(res);
}
