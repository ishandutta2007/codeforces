...
bool ok(vector<int> main, vector<int> other, int64_t me) {
    ...
    int used = 0;
    auto grow = [&]() {
        while (used < sz(runs) && (me + runs[used].s >= 0)) {
            me += runs[used].f;
            used++;
        }
    };
...
void solve() {
    ...
    vi left(a.begin(), a.begin() + K);
    vi right(a.begin() + K + 1, a.end());

    reverse(all(left));

    {
        if (ok(left, right, a[K])) {
            ps("YES");
            return;
        }
    }
    {
        if (ok(right, left, a[K])) {
            ps("YES");
            return;
        }
    }
