...
//		cerr<<"@#$#$\n";
	...
	poly operator%(const poly&w)const{if(A.size()<w.A.size())return *this;poly B=*this-*this/w*w;B.re(min(A.size(),w.A.size()-1));return B;}//
...
int main(){
	F.re(1e6);
	n=read(),k=read();
	for(int i=1;i<=n;i++)F.A[read()]=1;
	F=F.Ksm(k);
	for(int i=1;i<=1e6;i++)if(F.A[i])cout<<i<<" ";
	puts("");
	pc('1',1);
	return 0;
}