...
int main(){
	...
	while(T--){
		...
		ll x[m], y[m];
		rep(i, m) cin >> x[i] >> y[i], y[i]--;
		seg.update(n, -1e9);
		int flag = 0;
		rep2(i, m){
			y[i] = seg.find_first(0, [&](ll x){ return y[i] + x < -1; });
			if (y[i] >= n)
				flag = 1;
			else
				seg.update(y[i], 0);
		}
		rep(i, m) seg.update(y[i], -1);
		seg.update(n, -1);
		if (flag){
			cout << 0 << endl;
			continue;
		...
		ll cnt = 0;
		vector<pair<ll, ll>> v;
		rep(i, m) v.push_back({y[i], i});
		sort(all(v));
		rep(i, m - 1) if (v[i].first + 1 == v[i + 1].first && v[i].second < v[i + 1].second) cnt++;
		rep(i, cnt + 1) ans += comb(2 * n - 1 - m, n - i) * comb(cnt, i);
		cout << ans << endl;
		// rep(i, m) cout << y[i] << (i + 1 == m ? '\n' : ' ');
	}
}