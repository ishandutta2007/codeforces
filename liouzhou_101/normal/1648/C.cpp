...
namespace data_structure
{
	...
	public:
		...
		template<typename InputIt>
		explicit FenwickTree(InputIt first, InputIt last, const Operation& operation = Operation(), const ValueType& init_value = 0) :
...
void solve()
{
	...
	input(t);
	sort(s.begin(), s.end());
	data_structure::FenwickTree<int> g(200010);
	data_structure::FenwickTree<modint> h(200010);
	for (auto x : s)
	{
		g.Add(x, 1);
		int cnt = g.Get(x) - g.Get(x - 1);
		h.Add(x, -modint(cnt - 1));
		h.Add(x, modint(cnt));
	}
	...
	{
		int cnt = g.Get(i) - g.Get(i - 1);
		factor *= modint::InvFactorial(cnt);
	}

	modint res = 0;
	for (int i = 0; i < min(m, n); ++i)
	{
		modint tmp = h.Get(t[i] - 1);
		res += tmp * factor * modint::Factorial(n - i - 1);
		if (g.Get(t[i]) - g.Get(t[i] - 1) == 0) break;
		g.Add(t[i], -1);
		int cnt = g.Get(t[i]) - g.Get(t[i] - 1);
		h.Add(t[i], -modint(cnt + 1));
		h.Add(t[i], modint(cnt));
		factor *= cnt + 1;
	}
	if (m > n)
	{
		if (g.Get(200000) == 0) res += 1;
	}
