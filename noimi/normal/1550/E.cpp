...
int main() {
    ...
    vv(int, rui, n + 1, k + 1);
    rep(i, n) {
        rep(j, k + 1) rui[i + 1][j] += rui[i][j];
        if(s[i] == '?')
            rui[i + 1][k]++;
        else
            rui[i + 1][s[i] - 'a']++;
    }
    auto check = [&](int l, int r, int c) { return rui[r][c] - rui[l][c] + rui[r][k] - rui[l][k] == r - l; };
    OUT(bin_search(0, floor(n, k) + 1, [&](int x) {
        vv(int, ok, k);
        rep(i, n - x + 1) {
            rep(j, k) {
                if(check(i, i + x, j)) ok[j].eb(i);
            }
        }
        vv(int, nxt, n + 1, k, n + 2);
        rep(j, k) {
            int pre = 0;
            for(auto e : ok[j]) {
                while(pre <= e) {
        ...
        rep(b, 1 << k) {
            if(dp[b] > n) continue;
            rep(j, k) {
                if(~b & 1 << j) { chmin(dp[b | 1 << j], nxt[dp[b]][j] + x); }
            }
        }
        return dp.back() <= n;
    }));
