...
int N;
vector<vector<pair<int, pi>>> graph;
int curFac[MX];
int hiFac[MX];
mi invs[MX];
mi ans;

void dfs(int v, int p, mi cur) {
    ans += cur;
    trav(a, graph[v]) {
        if (a.f == p) continue;
        trav(b, facs[a.s.s]) {
...
void solve() {
    cin >> N;
    graph = vector<vector<pair<int, pi>>>(N);
    F0R(i, N+1) {
        curFac[i] = 0; hiFac[i] = 0;
    }
    ans = 0;
    F0R(i, N-1) {
...
int main() {
    ...
    compPrimes(MX);
    FOR(i, 2, MX) {
        facs[i] = facs[i/leastFac[i]];
        facs[i].pb(leastFac[i]);
        invs[i] = inv(mi(i));
    }
    int T = 1;
    cin >> T;
