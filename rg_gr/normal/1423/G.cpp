...
#ifdef COMB
	fac[0]=inv[0]=inv[1]=1;
	rg(N0)fac[i]=fac[i-1]*i%mod;gr
	rg(N0)if(i>1)inv[i]=inv[mod%i]*(mod-mod/i)%mod;gr
	rg(N0)rnv[i]=inv[i];gr
	rg(N0)inv[i]=inv[i]*inv[i-1]%mod;gr
#endif
}
#ifdef COMB
int C(int n,int m)
...
std::set<Node>b[100005];
int cc;
std::map<int,int>mp;
void add(int x,int p)
{
	// odp(x,p);fflush(stdout);
	A.add(1,1,N,x,x,p*x);
	B.add(1,1,N,x,x,p);
	// rg(N)odb(A.qsum(1,1,N,,i));gr puts("");
	// rg(N)odb(B.qsum(1,1,N,i,i));gr puts("");
	// fflush(stdout);
}
qwq erase(Node x)
{
	int l=x.l,r=x.r,v=x.v;
	auto p=b[v].lower_bound(x);
	assert(p!=b[v].end());
	int F=-1,T=-1;
	if(p!=b[v].begin())
	{
...
inline void setval(int l,int r,int v)
{
	...
	qwq it1=split(l),j;
	// erase(it1,it2);
	for(auto p=it1;p!=it2;p=erase(*p));
	insert(Node(l,r,v));
}
int F(int x)
{
...
signed main()
{
	...
	a[i]=F(read());
	gr
	rg(N)b[i].insert(Node(0,0,-1)),b[i].insert(Node(n+1,n+1,-2));gr
	add(n+1,N);
	s.insert(Node(0,0,-1));s.insert(Node(n+1,n+1,-2));
	rg(n)insert(Node(i,i,a[i]));gr;
	rg(m)
	int p=read();
	if(p==1)
	{
		int l=read(),r=read(),v=F(read());
		setval(l,r,v);
	}
	else 
	{
		int k=read();
		oldl(N*(n-k+1)-(A.qsum(1,1,N,k,N)-(k)*B.qsum(1,1,N,k,N)));
	}
	gr
	return 0;
}