...
fn solve(
    ...
    for _testcase in 0..inp.np::<usize>() {
        let n = inp.np::<usize>();
        let candies = (0..n).map(|_| inp.np::<i32>()).collect::<Vec<_>>();
        let (mut a_ans, mut b_ans) = (0, 0);
        let (mut a_move, mut b_move): (i32, i32);
        b_move = 0;

        let mut can_it = candies.iter();
        let mut n_move = 0;
        let mut gameover = false;
        while !gameover {
            a_move = 0;
            while a_move <= b_move {
                if let Some(x) = can_it.next() {
                    n_move += (a_move == 0) as usize;
                    a_move += x;
                } else {
                    gameover = true;
                    break;
            ...
            b_move = 0;
            while b_move <= a_move {
                if let Some(x) = can_it.next_back() {
                    n_move += (b_move == 0) as usize;
                    b_move += x;
                } else {
                    gameover = true;
                    break;
        ...
        writeln!(out, "{} {} {}", n_move, a_ans, b_ans).ok();
    }
}
