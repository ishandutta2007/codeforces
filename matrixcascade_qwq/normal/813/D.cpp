...
signed main()
{
	...
	while(T--)
	{
		...
		up(i,0,n)
		{
			...
			up(j,1,i)
			{
				chkmax(f2[a[j]%7],dp[i][j]);
				chkmax(f1[a[j]],dp[i][j]);
			}
			up(j,i+1,n)
			{
				chkmax(dp[i][j],Max(dp[i][0],f1[a[j]-1],f1[a[j]+1],f2[a[j]%7])+1);
				dp[j][i]=dp[i][j];chkmax(ans,dp[i][j]);chkmax(f2[a[j]%7],dp[i][j]);
				chkmax(f1[a[j]],dp[i][j]);
			}
		}
		opl(ans);
