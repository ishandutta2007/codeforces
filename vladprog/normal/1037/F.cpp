...
signed main()
{
    ...
    for1(i,n)
        res[i]=(res[i-1]+(i-1)/(k-1))%MOD;
    int ans=0;
    for0(i,n)
    {
        int tl=i-l[i]-1,tr=r[i]-i-1;
        int tres=res[tl+tr+1]-res[tl]-res[tr];
        tres=tres%MOD;
        tres+=MOD;
        tres=tres%MOD;
        IFD( cout<<i<<" -> "<<tl<<" "<<tr<<" "<<tres<<"\n"; )
        ans=(ans+a[i]*tres%MOD)%MOD;
    }
    PRT(rng(l,l+n));
    PRT(rng(r,r+n));
    PRT(rng(res+1,res+n+1));
    cout<<ans;
