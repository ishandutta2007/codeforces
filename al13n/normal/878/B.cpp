...
int main(int argc, char **argv){
  ...
  forn(i,n){
    LL a;
    cin>>a;
    if(sz(A)&&A.back().X==a) ++A.back().Y;
    else A.pb(mp(a,1ll));
    if(A.back().Y>=k){
      s0-=k;
      A.back().Y-=k;
      if(!A.back().Y) A.pop_back();
    }
  }
  if(!sz(A)||m==1){
    cout<<s0;
  ...
  LL s1=s0;
  dbg(s0,A);
  while(sz(A)>1 && A[0].X==A.back().X && A[0].Y+A.back().Y>=k){
    LL c=A[0].Y+A.back().Y-k;
    s1-=k;
    Q.pop_front();
    A.pop_front();
    if(c){
  ...
  if(sz(A)==1){
    if(P.back().X==A[0].X){
      ...
      r-=P.back().Y/k*k;
      P.back().Y%=k;
      if(!P.back().Y)P.pop_back();
    }
    dbg(r,P,Q);
    for(pll p:Q){
      if(sz(P)&&P.back().X==p.X)P.back().Y+=p.Y;
      else P.pb(p);
      if(P.back().Y>=k){
        r-=k;
        P.back().Y-=k;
