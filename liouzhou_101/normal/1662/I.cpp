...
void solve()
{
	...
	for (int i = 0; i < n; ++i)
	{
		int j = lower_bound(a.begin(), a.end(), i * 100) - a.begin();
		if (a[j] == i * 100) continue;
		v[j].push_back(i);
	}
	...
	for (int k = 1; k < m; ++k)
	{
		...
		ll len = a[k] - a[k - 1];
		vector<ll> s(v[k].size() + 1);
		for (int i = 0; i < v[k].size(); ++i)
			s[i + 1] = s[i] + p[v[k][i]];
		for (int i = 0, j = 0; i < v[k].size(); ++i)
		{
			while (j < v[k].size() && (v[k][j] - v[k][i]) * 100 * 2 < len) j += 1;
			freshmax(res, s[j] - s[i]);
		}
	}
	writeln(res);
}
