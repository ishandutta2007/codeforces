...
ll solve(ll mi, ll ma){
	if(mi > ma)
		return 0;
	ll lv = ma + 1, rv = mi - 1, nmi = mi + 1, nma = ma;
	ll cnt = 0, sc = 0, flag = 0;
	queue<ll> que;
	que.push(mi);	
	while (!que.empty()){
		ll q = que.front();
	...
	if(flag)
		...
		ll res = solve(nmi, nma);
		if(res == -1)
			ans = -1;
		else
			ans = min(sc, cnt - sc) + res;
	}
	return ans;
}
...
int main(){
	...
	ll a[n], b[n];
	rep(i, n) cin >> a[i] >> b[i];
	rep(i, n) p[a[i]] = b[i], p[b[i]] = a[i], f[a[i]] = 0, f[b[i]] = 1;
