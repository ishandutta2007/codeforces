...
void TryAnswer(int s, LongInteger m) {
	if (m < LongInteger(1)) return;
	if (m % 2 == 0) return;
	LongInteger res = (m * (m - LongInteger(1))/2) + m * LongInteger((1LL<<s) - 1);
	if (res != n) return;
	ans.insert(LongInteger(1LL<<s) * m);
}
...
int main() {
	...
	for (int s = 0; s<=61; s++) {
		LongInteger qb = (1LL<<(s+1)) - 3;
		LongInteger qc = n * (-2);
		LongInteger qd = qb*qb - qc*4;
		if (qd < LongInteger(0))
			continue;
		qd = qd.SquareRoot();
		TryAnswer(s, (-qb + qd) / 2);
