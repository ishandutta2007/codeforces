...
bool cmp(pair<pair<int,int>,int> &a, pair<pair<int,int>,int> &b){
	if(a.f.f/bsz == b.f.f/bsz)return a.f.s < b.f.s;
	return a.f.f/bsz < b.f.f/bsz;
}
...
void solve(){
	...
	for(int i = 0; i < q; ++i){
		...
		if(cu <= (mr-ml)/2+1){
			...
			ou[que[i].s] = (mr-ml+1)-(mr-ml+1-cu)*2;
		}
	}
	for(auto i : ou)cout << i << "\n";
}
