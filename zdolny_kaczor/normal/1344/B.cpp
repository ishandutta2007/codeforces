...
void test(string s) {
	int first = -1, last = -1, cou = 0;
	REP(i, s.size()) if (s[i] == '#') {
		cou++;
		last = i;
		if (first == -1) first = i;
	}
	if (cou && cou != last - first + 1) nope();
}
void dfs(int x, int y) {
	if (x < 0 || y < 0 || in[x][y] != '#') return;
	in[x][y] = '.';
	dfs(x + 1, y);
	dfs(x - 1, y);
	dfs(x, y - 1);
...
int main() {
	...
	REP(i, n) {
		bool is_empty = true;
		REP(j, m) if (in[i][j] == '#') is_empty = false;
		if (is_empty) empty_row = true;
	}
	REP(j, m) {
		bool is_empty = true;
		REP(i, n) if (in[i][j] == '#') is_empty = false;
		if (is_empty) empty_col = true;
	}
	if (empty_row != empty_col) nope();
	int ans = 0;
