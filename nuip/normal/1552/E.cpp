...
vector<pii> solve(const int n,const vector<int> &vs){
	const int t=vs.size()/n;
	cauto ub=(n+t-2)/(t-1);
	vector<pii> re(n,{-1,-1});
	...
	rep(i,vs.size()){
		cauto &v=vs[i];
		if(re[v].X>=0) continue;
		cauto &l=prv[v];
		cauto &r=i;
		if(l>=0){
			if(*max_element(sum.begin()+l,sum.begin()+r+1)<ub){
				re[v]={l,r};
				reps(j,l,r+1) sum[j]++;
			}
		}
	...
	if(count(all(re),pii(-1,-1))) return {};
	return re;
}
...
int main(){
	...
	// reps(n,1,6)reps(k,2,6){
	// 	out(n,k,1);
	// 	vector<int> vs;
	// 	rep(i,n)rep(j,k) vs.eb(i);
	// 	do{
	// 		if(solve(n,vs).empty()){
	// 			out(n,k,vs,1);
	// 			return 0;
