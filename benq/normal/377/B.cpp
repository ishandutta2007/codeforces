...
int main() {
	...
	F0R(i, N) re(stu[i][0]);
	F0R(i, N) re(stu[i][1]);
	bugs.ins(end(bugs), all(stu));
	sor(bugs);
	reverse(all(bugs));
	dbg(bugs);
	auto get_min_ans = [&](int days) -> vi {
		priority_queue<pi, vpi, greater<pi>> passes;
		vi use;
		vi assigned(M);
		ll ans = 0;
		for (auto [a, b, _] : bugs) {
			if (b < 0) {
				...
				assigned.at(_) = use.bk + 1;
				use.pop_back();
			} else {
				passes.push({b, _});
			}
		}
		if (ans <= S) return assigned;
		return {};
	};
	int days = fstTrue(1, M, [&](int x) { return sz(get_min_ans(x)) > 0; });
	dbg(M, days);
	if (days > M) {
		ps("NO");
		exit(0);
	...
	ps("YES");
	ps(ts_sep(get_min_ans(days), " "));

	// fassert(ans <= S);
	// ps("YES");
	// each(t, assigned) pr(t, ' ');
