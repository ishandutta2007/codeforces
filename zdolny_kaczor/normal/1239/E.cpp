...
const int vax = (5e4 + 1) * 25, nax = 26;
bitset<vax> dp[nax * 2][nax];
int a[nax * 2];
#define tab _M_w
using word = urs(bitset<vax>().tab[0]);
const int SIZE = 8 * sizeof(word);
const int LOG = __builtin_popcount(SIZE - 1);
template <size_t N> int find_prev(const bitset<N> &x, int v) {
	int start = v >> LOG;
	int first_bits = v & (SIZE - 1);
	if (first_bits) {
		word curr = x.tab[start];
...
int main() {
	...
	for (int i = 2 * n - 1; i > 0; --i) {
		dp[i][0] = 1;
		for (int s = 0; s < min(n - 1, 2 * n - i); ++s)
			dp[i][s + 1] = (dp[i + 1][s] << a[i]) | dp[i + 1][s + 1];
	}
	int all_sum = 0;
	for (int i = 0; i < 2 * n; ++i) all_sum += a[i];
	pair <int, pii> best = {inf, {inf, inf}};
	for (int i = 1; i < 2 * n; ++i) {
		int rest = all_sum - a[i] - a[0];
		int x = find_prev(dp[i + 1][n - 1], rest / 2 + 1);
		int y = dp[i + 1][n - 1]._Find_next(rest / 2);
		debug(arr(a, i), imie(rest), imie(x), imie(y));
		for (int s : vector <int>{x, y})
			if (s != -1 && s != vax)
				mini(best, make_pair(a[i] + a[0] + max(s, rest - s), pii(i, s)));
	}
	debug(imie(best));
	loc(return 0 * printf("%d\n", best.first));
	int sum = best.second.second;
	int i = best.second.first + 1;
	int cou = n - 1;
	used[best.second.first] = 1;
	while (cou) {
		debug(arr2(dp, i + 1, cou)[imie(sum)]);
		if (dp[i + 1][cou][sum]) i++;
		else {
			used[i] = 1;
			sum -= a[i];
	...
	for (int i = 0; i < 2 * n; ++i) if (used[i]) bottom.push_back(a[i]);
	else top.push_back(a[i]);
	assert(top.size() == bottom.size());
	reverse(bottom.begin(), bottom.end());
	for (int x : top) printf("%d ", x);
	printf("\n");
	for (int x : bottom) printf("%d ", x);
	printf("\n");
	assert(sum == 0);
}