...
int main() 
{
	...
	while(true){
		while(!que.empty()){
			...
			vector<int>ers;
			for(auto x:not_connects){
				if(nums[x]!=sum){
					ers.push_back(x);
				}
			}
			for(auto er:ers){
				not_connects.erase(er);
				que.emplace(er);
			}
		}
		if(!not_connects.empty()){
			que.push(*not_connects.begin());
			not_connects.erase(not_connects.begin());
			answer++;
		}else{
			break;
		}
