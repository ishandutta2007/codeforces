...
void solve() {
  ...
  
  {
    ...
    // }
    mx.second.first = i - 1;
    mx.second.second = j + 1;
    // if (mx.second.first + 1 >= mx.second.second) {
    //   cout << s << "\n";
    //   return;
    // }
    mx.first = mx.second.first + 1 + n - mx.second.second;
  }

  // check prefix
  int pv1 = mx.second.first + 1;
  int pv2 = mx.second.second - 1;
  for (int i = pv1; i <= pv2; i++) {
    int l = pv1;
    int r = i;

    lint fr = getpref(l, r);
    lint bk = getsuff(n - r - 1, n - l - 1);
    if (fr == bk) {
      mx = max(mx, {r + 1 + n - (pv2 + 1), {r, pv2 + 1}});
    }
  }

  // suffix
  for (int i = pv2; i >= pv1; i--) {
    int l = i;
    int r = pv2;
