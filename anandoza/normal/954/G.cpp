...
int main() {
    ...
    vll diff(n + 1);
    auto add_range = [&](int left, int right, ll v) {
        diff[max(0, left)] += v;
        diff[min(n, right)] -= v;
    };
    for (int i = 0; i < n; i++) {
        add_range(i - r, i + r + 1, a[i]);
    }

    auto impossible = [&](ll level) {
        vll copy = diff;
    ...
    auto ans = -1 + binarysearch(impossible, 0LL, accumulate(all(a), 0LL) + k + 1);
