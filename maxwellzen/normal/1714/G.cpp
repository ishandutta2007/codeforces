...
void dfs(int i) {
    ...
    for (int nb : children[i]) {
        tmp.pb(tmp.back()+b[nb]);
        cur += a[nb];
        for (int step = 1<<20; step > 0; step /= 2) if (pos+step < tmp.size() && tmp[pos+step]<=cur) pos += step;
        dfs(nb);
        tmp.pop_back();
        cur -= a[nb];
        pos = r[i];
...
void solve() {
    cin >> n;
    p.rsz(n); a.rsz(n); b.rsz(n);
    children.clear(); children.rsz(n);
    FOR(i, 1, n) {
        cin >> p[i] >> a[i] >> b[i];
        p[i]--;
        children[p[i]].pb(i);
