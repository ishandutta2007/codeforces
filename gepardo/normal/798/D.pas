...
type
  ...
  TSorter = specialize TOrderingArrayUtils<TArrayPairIntInt, TPairIntInt, TLessPairIntInt>;
...
var
  ...
  P: TArrayPairIntInt;
  LeftN: Integer;
  SumOdd, SumEven: Int64;
  BegWith: Integer;
begin
  ReadLn(N);
  SetLength(A, N); SetLength(B, N); SetLength(P, N);
  ...
  BegWith := IfThen(SumOdd > SumEven, 1, 0);
  WriteLn(N div 2 + 1);
  for I := 0 to LeftN div 2 - 1 do
  begin
    Write(P[I * 2 + BegWith].Y + 1, ' ');
  end;
  for I := LeftN to N-1 do
  begin
    Write(P[I].Y + 1, ' ');
  end;
  WriteLn;
