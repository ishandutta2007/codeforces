...
void dfs(int x,vvi&adj,vl&l,vl&r,vl&leftval,vl&rightval,vector<bool>&vis)
{
    ll left=0, right=0;
    bool leaf = true;
    for( auto node : adj[x])
    {
        ...
        vis[node]=1;
        leaf= false;
        dfs(node,adj,l,r,leftval,rightval,vis);
        left += max(abs(l[x]-l[node])+leftval[node],abs(l[x]-r[node])+rightval[node]);
        right += max(abs(r[x]-l[node])+leftval[node],abs(r[x]-r[node])+rightval[node]); 
        //cout<<x<<" "<<node<<" "<<" "<<leftval[node]<<" "<<rightval[node]<<" "<<left<<" "<<right<<endl;
    }
...
int main()
{
    ...
    TT
    {
        ...
        cin>>n;
        vvi adj(n);
        vl l(n),r(n),leftval(n),rightval(n);
        vector<bool> vis(n,0);
        for(int i = 0 ; i < n ; i++)
        {
            cin>>l[i]>>r[i];
        ...
        vis[0]=1;
        dfs(0,adj,l,r,leftval,rightval,vis);
        cout<<max(leftval[0],rightval[0])<<'\n';
    }   
}