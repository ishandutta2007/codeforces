...
const int MN=2e5+5; 
int r1,c1,r2,c2,r3,c3,n;
map<int,int>cntr,cntc;  

void works() {
	n=read();
	r1=read(),c1=read(),r2=read(),c2=read(),r3=read(),c3=read();
	cntr.clear(),cntc.clear(); 
	cntr[r1]++; cntr[r2]++; cntr[r3]++; 
	cntc[c1]++; cntc[c2]++; cntc[c3]++;
	int x=0,y=0; 
	for(auto it:cntr) if(it.second==2) x=it.first; 
	for(auto it:cntc) if(it.second==2) y=it.first; 
	int r=read(),c=read();
	if((r&1)!=(x&1)&&(c&1)!=(y&1)) printf("NO\n");
	else {
		if((x!=1||y!=1)&&(x!=1||y!=n)&&(x!=n||y!=1)&&(x!=n||y!=n)) printf("YES\n"); 
		else {
