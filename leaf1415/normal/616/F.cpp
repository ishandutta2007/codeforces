...
ll MaxRectangle(vector<ll> hvec, vector<ll> wvec, ll &l, ll &r, ll &h)
{
	int n = sz(hvec);
	vector<ll> lvec(n), rvec(n), wsum(n+1);
	...
	rep(i, 0, n-1){
		while(stk.top().first >= hvec[i]) stk.pop();
...
ll MaxRectangle(vector<ll> hvec)
{
	ll l, r, h;
	vector<ll> wvec;
	reps(i, hvec) wvec.push_back(1);
	return MaxRectangle(hvec, wvec, l, r, h);
}
...
int main(void)
{
	...
	ll sum = 0;
	rep(i, 1, n) sum += sz(s[i])+1, qvec.push_back(sum-1);
	
	rep(i, 1, n){
		for(auto c : s[i]) vec.push_back(c-'a');
		vec.push_back(30+i);
	}
	sa.init(vec);
	...
	reps(i, sa.sa){
		if(i == 0) continue;
		ll p = lower_bound(all(qvec), sa.sa[i]) - qvec.begin();
		ll len = qvec[p] - sa.sa[i];
		hvec.push_back(len), wvec.push_back(c[p+1]);
		if(i < sz(sa.lcp)) hvec.push_back(sa.lcp[i]), wvec.push_back(0);
	}
	
	ll l, r, h;
	outl(MaxRectangle(hvec, wvec, l, r, h));
