...
const int MN=105+10;
int n,f[MN][MN][2],a[MN]; 
void works() {
	n=read(); 
	For(i,1,n) a[i]=read(); 
	For(i,0,n) For(j,0,n) For(op,0,1) f[i][j][op]=INF; 
	f[0][0][0]=f[0][0][1]=0; 
	For(i,1,n) {
		For(j,0,i) {
			For(op,0,1) {
				f[i][j][op]=INF; 
				if(a[i]&&(a[i]&1)!=op) continue; 
				if(j>=op) ckmin(f[i][j][op],f[i-1][j-op][op]),ckmin(f[i][j][op],f[i-1][j-op][op^1]+1); 
//				cerr<<i<<' '<<j<<' '<<op<<' '<<f[i][j][op]<<endl;
			}
		}
	}
	cout<<min(f[n][(n+1)/2][0],f[n][(n+1)/2][1])<<endl;
}
signed main() {
	#ifdef wasa855
