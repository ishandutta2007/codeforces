...
#define se second
const int N=1e2+5;
int n,ans,f[N][N][2];pair<int,int>a[N];
int main()
{
	n=read();for(int i=1;i<=n;i++) a[i].fi=read(),a[i].se=read();
	sort(a+1,a+n+1);a[0].fi=-1e9;
	for(int i=0;i<=n;i++) for(int j=0;j<=i;j++) for(int p=0;p<2;p++)
	{
		ans=max(ans,f[i][j][p]);int o=a[j].fi+p*a[j].se,mx=-1e9,x,y;
