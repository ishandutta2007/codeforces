...
fun main(){
    solve.cases{
        ...
        val isoutput = BooleanArray(n)
        fun solve(x:Int,output:Boolean):Int?{
            var qnow  = q
            var ret = 0
            for(i in 0 until n){
                if(i <= x){
        ...
        val last = BinarySearchFirstTrue(-1,n){solve(it,false) != null}!!
        solve(last,true)
        put(isoutput.map { if(it) '1' else '0'}.conca())
