...
void dfs3(int v)
{
	for(auto u : G[v]) dfs3(u);
	rep(i, -Q+1, Q){
		double mul = 1-p[v];
		for(auto u : G[v]) mul *= dp[u][zero+i];
		if(i < h[v]) mul *= 0;
		dp[v][zero+i] += mul;
		
		mul = p[v];
		for(auto u : G[v]) mul *= dp[u][zero+i-1];
		if(i-1 < h[v]) mul *= 0;
		dp[v][zero+i] += mul;
	}
}
...
int main(void)
{
	...
	cin >> n >> Q;
	rep(i, 1, n) cin >> a[i];
	rep(i, 1, Q) cin >> l[i] >> r[i] >> p[i];
	l[0] = 1, r[0] = n;
	
	rep(i, 1, Q) mp[l[i]].pb(P(r[i], i));
	for(auto &q : mp) sort(all(q.snd));
	dfs(0);
	
	seg.init();
	dfs2(0);
	rep(i, 1, n) leaf[i] = seg.query(i, i);
	
	ll mx = 0;
	rep(i, 1, n) chmax(mx, a[i]);
	rep(i, 1, n) a[i] = max(-Q-1, a[i]-mx);
	
	rep(i, 0, Q) h[i] = -Q-1;
	rep(i, 1, n) chmax(h[leaf[i]], a[i]);
	
	dfs3(0);
	
	double ans = 0;
	rep(i, 0, Q) ans += 1-dp[0][zero+i];
	ans += mx;
	outf(ans);
