...
// https://github.com/noshi91/Library/blob/master/data_structure/w_ary_tree_set.cpp
namespace n91 {
...
public:
  ...
  usize succ(const usize key) const {
    const usize tp = child[key / C].succ(key % C);
    if (tp != -1) {
      return key / C * C + tp;
    }
    const usize masked = (map >> 1) & (~static_cast<usize>(0) << key / C);
    if (masked == 0) {
      return -1;
    }
    const usize pos = bsf(masked) + 1;
    return pos * C + child[pos].min();
  }

  usize pred(const usize key) const {
    const usize tp = child[key / C].pred(key % C);
    if (tp != -1) {
      return key / C * C + tp;
    }
    const usize masked = map & ~(~static_cast<usize>(0) << key / C);
    if (masked == 0) {
      return -1;
    }
    const usize pos = bsr(masked);
    return pos * C + child[pos].max();
  }
...
public:
  ...
  usize succ(const usize key) const {
    const usize masked = (map >> 1) & (~static_cast<usize>(0) << key);
    if (masked == 0) {
      return -1;
    }
    return bsf(masked) + 1;
  ...
  usize pred(const usize key) const {
    const usize masked = map & ~(~static_cast<usize>(0) << key);
    if (masked == 0) {
      return -1;
    }
    return bsr(masked);
...
int main(){
    ...
    {
        auto b=iota(n);
        sort(all(b),[&](int i,int j){return x[i]<x[j];});
        rep(n)a[b[i]]=i;
        Sort(x);
    }
    LL(q);
    const int sq = sqrt(q);
    vec(vector<tuplis>,bucket,sq-1);
    vec(int,ans,q);
    rep(q){
        LL(l,r);
        l--;
        if(r-l<=n/sq+1){
            ...
            bitset<100000>bit;
            rep(i,l,r)bit[a[i]]=1;
            ll at=bit._Find_first();
            while(true){
                ll at2=bit._Find_next(at);
                if(at2==bit.size())break;
                chmin(ans[i],x[at2]-x[at]);
                at=at2;
            }
        }
        else bucket[(l*sq)/n].push_back({l,r,i});
    }
    each(b,bucket)sort(all(b),[](tuplis&a,tuplis&b){return a[1]<b[1];});
    rep(sq-1){
        const ll L=(n*(i+1))/sq;
        n91::w_ary_tree_set<100000> s;
        int mn=INF,snap=0;
        ll at=L;
        auto add=[&](int i){
            auto r=s.succ(i);
            auto l=s.pred(i);
            s.insert(i);
            if(l!=-1)chmin(mn,x[i]-x[l]);
            if(r!=-1)chmin(mn,x[r]-x[i]);
        };
        each(l,r,id,bucket[i]){
            while(at<r)add(a[at++]);
            snap=mn;
            rep(i,l,L)add(a[i]);
            ans[id]=mn;
            rep(i,l,L)s.erase(a[i]);
            mn=snap;
        }
    }
    each(i,ans)out(i);
