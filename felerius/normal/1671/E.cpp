// Preorder (https://codeforces.com/contest/1671/problem/E)
...
int main() {
    ...
    auto dfs = [&](int v, auto&& self) -> pair<MI, string> {
        string c(1, s[v]);
        if (v >= (1 << (n - 1)))
            return {1, c};

        auto [lcnt, lgrp] = self(2 * v, self);
        auto [rcnt, rgrp] = self(2 * v + 1, self);
        if (lgrp == rgrp)
            return {lcnt * lcnt, c + lgrp + lgrp};
        return {2 * lcnt * rcnt, c + min(lgrp + rgrp, rgrp + lgrp)};
    };
    println(dfs(1, dfs).first);
}