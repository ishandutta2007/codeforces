...
pii findfar(int v, int p = -1) {
    pii ans = {0, v};
    for (auto i : G[v])
        if (i != p) {
            auto check = findfar(i, v);
            ++check.f;
            if (check.f > ans.f)
                ans = check;
        }
...
bool findpath(int v, int f, vector<int> &path, int p = -1) {
    if (v == f) {
        path.pb(v);
        return true;
    }
...
int count2(int v, int p = -1, int h = 0) {
    int ans = 0;
    if (h % 2 == 0) ++ans;
    for (auto i : G[v])
        if (i != p)
