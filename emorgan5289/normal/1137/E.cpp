...
deque<point<ll>> a = {{0, 0}};
ll gb = 0, gm = 0;

point<ll> get(point<ll> p) {
    return {p.x, p.y+gb+gm*p.x};
}
...
int main() {
    ...
    while (m--) {
        ...
        if (t == 1) {
            ll k; cin >> k;
            l -= k;
            point<ll> p = {l, 0-gb-gm*l};
            while (a.size() > 0 && get(a.front()).y >= 0)
                a.pop_front();
            a.push_front(p);
        }
        if (t == 2) {
            ll k; cin >> k;
            point<ll> p = {r, 0-gb-gm*r};
            r += k;
            while (a.size() > 1 && cross(get(a[a.size()-2]), get(a.back()), get(p)) < 0)
                a.pop_back();
            if (get(a.back()).y > 0)
                a.push_back(p);
        }
        if (t == 3) {
            ll b, s; cin >> b >> s;
            gb += b-l*s, gm += s;
            while (a.size() > 1 && get(a.back()).y >= get(a[a.size()-2]).y)
                a.pop_back();
        }
        auto p = get(a.back());
        cout << (long long)(p.x-l+1) << " " << (long long)(p.y) << "\n";
    }
}