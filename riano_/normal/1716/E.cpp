...
int main(){
    ...
    ll N; cin >> N;
    ll L = (1<<N);
    vector<ll> sm(2*L,0),mx(2*L,0),lm(2*L,0),rm(2*L,0);
    vector<bool> fl(2*L,false);
    rep(i,L){
        cin >> sm[L+i];
        mx[L+i] = max(0LL,sm[L+i]); lm[L+i] = max(0LL,sm[L+i]); rm[L+i] = max(0LL,sm[L+i]);
    }
    rrep(i,L){
        sm[i] = sm[2*i]+sm[2*i+1];
        lm[i] = max(lm[2*i],sm[2*i]+lm[2*i+1]);
        rm[i] = max(rm[2*i+1],sm[2*i+1]+rm[2*i]);
        mx[i] = max({mx[2*i],mx[2*i+1],rm[2*i]+lm[2*i+1]});
    }
    ll pv = 0;
    vector<ll> ans(L,0);
    chmax(ans[0],mx[1]);
    for(int i=1;i<L;i++){
        vector<int> f;
        rep(j,N){
            if(((pv)^(i))&(1<<j)) f.emplace_back(j);
        }
        int mid = 0;
        for(int k:f){
            ll s = (1<<k);
            for(int t=s;t<2*s;t++){
                fl[t] = (1-fl[t]); chmax(mid,t+1);
            }
            //cout << k << " ";
        }
        //cout << "\n";
        rrep(j,mid){
            if(fl[j]){
                sm[j] = sm[2*j]+sm[2*j+1];
                lm[j] = max(lm[2*j+1],sm[2*j+1]+lm[2*j]);
                rm[j] = max(rm[2*j],sm[2*j]+rm[2*j+1]);
                mx[j] = max({mx[2*j],mx[2*j+1],rm[2*j+1]+lm[2*j]});
            }
            else{
                sm[j] = sm[2*j]+sm[2*j+1];
                lm[j] = max(lm[2*j],sm[2*j]+lm[2*j+1]);
                rm[j] = max(rm[2*j+1],sm[2*j+1]+rm[2*j]);
                mx[j] = max({mx[2*j],mx[2*j+1],rm[2*j]+lm[2*j+1]});
            }
        }
        chmax(ans[i],mx[1]);
        pv = i;
