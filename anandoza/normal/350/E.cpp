...
int main() {
    ...
    add_edge(unmarked, u);
    add_edge(unmarked, v);
    for (int i = 0; sz(ans) < m && i < n; i++)
        add_edge(unmarked, i);

    for (int i = 0; sz(ans) < m && i < n; i++)
        for (int j = 0; sz(ans) < m && j < n; j++) {
            if (i == j)
                continue;
            if ((i == u) && marked.count(j))
                continue;
            if ((j == u) && marked.count(i))
                continue;
    ...
    [&]() {
        ...
        for (auto v : a)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    ckmin(dp[i][j], dp[i][v] + dp[v][j]);

        bool bad = false;
        for (int v = 0; v < n; v++)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    bad |= dp[i][j] > dp[i][v] + dp[v][j];
        assert(bad);
    }();
#endif
