...
class lazySegTree (nSuggest  :Int){
    ...
    val lazy = LongArray(n) // l // lazy : already applied to this node

    private fun updateNode(i:Int,level:Int){
        arr[i] = arr[i shl 1 ] + arr[(i shl 1) + 1] + lazy[i] * ( 1L shl level)
    }
    ...
    solve.cases{
        ...
        fun addone(v:Int){
            if(!st.contains(v)){
                ...
                val until = st.st.floorEntry(v)!!.value
                st.delete(v,until)
                st.add(until + 1 , until + 1)
            }
        }
        fun removeone(v:Int){
            if(st.contains(v)){
                ...
                val next = st.st.ceilingEntry(v)!!.key
                st.delete(next,next)
                st.add(v,next- 1 )
            }
        }
