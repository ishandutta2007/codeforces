...
/*
 *  * Add to range, find max
 *   */
using n = int;
using u = int;
auto nn = [](n a, n b) { return max(a, b); };
auto uu = plus<n>();
auto un = [](u U, n N) { N += U; return N; };
...
int main() {
    ...
    segment_tree_lazy st(MAXN, 0, 0, nn, uu, un);
    st.set_leaves([&](int i){ return i; });

    int N, K, M; re(N, K, M); --K;

    set<pii> pawns;
    multiset<int> entering;

    F0R (_, M) {
        int x, y; re(x, y); --x, --y;

        auto it = pawns.find({x, y});
        if (it != pawns.end()) {
            pawns.erase(it);
            int enters = y + abs(x - K);
            entering.erase(entering.find(enters));
            st(0, enters + 1, -1);
        } else {
            pawns.insert({x, y});
            int enters = y + abs(x - K);
            entering.insert(enters);
            st(0, enters + 1, 1);
        }
        ...
        if (entering.empty())
            ...
            ps(max(0, st(0, *entering.rbegin() + 1) - N));
    }
