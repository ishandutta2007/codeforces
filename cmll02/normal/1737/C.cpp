...
signed main()
{
	...
	int F=x+y&1,G=a+b&1,H=c+d&1;
	int xx=read(),yy=read();
	int f=xx+yy&1;
	int less,op;
	int TEST=-1;
	
	if(F==H)less=G,op=a&1;
	if(F==G)less=H,op=c&1;
	if(G==H)less=F,op=x&1;
	if(less==F&&(x==1||x==n)&&(y==1||y==n))TEST=(x==1)<<1|(y==1);
	if(less==G&&(a==1||a==n)&&(b==1||b==n))TEST=(a==1)<<1|(b==1);
	if(less==H&&(c==1||c==n)&&(d==1||d==n))TEST=(c==1)<<1|(d==1);
	if(TEST>=0)
	{
		int x=(TEST&2)?1:n;
		int y=(TEST&1)?1:n;
		if(y==yy||x==xx)puts("YES");
		else puts("NO");
		// int dist=std::abs(x-xx)+std::abs(y-yy);
		// if(std::abs(x-xx)==1&&1==std::abs(y-yy))
		// {
			// puts("NO");
		// }
		// else if()
