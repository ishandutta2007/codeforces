...
public class Main
{
	public static void main(String[] args)
	{
		...
		NavigableMap<Integer, Integer> ho=new TreeMap<Integer, Integer>();
		ho.put(0, n-1);
		Map<Integer, Integer> hola=new HashMap<Integer, Integer>();
		hola.put(n,1);
		int ans=1,val=a[pos[n-1]]+1;
		for(int i=n-2; i>=0; i--)
		{
			int po=pos[i+1];
			int lol=ho.floorKey(po);
			int temp=ho.get(lol);
			if(lol==po&&po==temp)
			{
				ho.remove(lol);
				if(!hola.remove(1,1))
					hola.put(1, hola.get(1)-1);
			}
			else if(lol==po)
			{
				ho.remove(lol);
				ho.put(lol+1,temp);
				if(!hola.remove(temp-lol+1,1))
					hola.put(temp-lol+1, hola.get(temp-lol+1)-1);
				hola.put(temp-lol, hola.getOrDefault(temp-lol, 0)+1);
			}
			else if(po==temp)
			{
				ho.remove(lol,temp);
			...
			else
			{
				...
				ho.put(lol, po-1);
				ho.put(po+1,temp);
				hola.put(po-lol, hola.getOrDefault(po-lol, 0)+1);
				hola.put(temp-po, hola.getOrDefault(temp-po, 0)+1);
			}
			if(hola.size()==1)
			{
				if(ho.size()>=ans)
