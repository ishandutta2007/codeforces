...
vi ans;
void gao3(int x,int d){
	if (!d){
		ans.pb(id[x]);
		return;
	}
	if (son[x][0])
		return gao3(son[x][0],d-1);
	else
		return gao3(son[x][1],d-1);
}
int gao2(int x,int y,int d){
	if (!x||!y)
		return 0;
	if (d==0){
		ans.pb(id[x]);
	...
	if (~k>>(d-1)&1){
		if (son[x][0]&&son[y][1])
			return gao3(son[x][0],d-1),gao3(son[y][1],d-1),1;
		if (son[x][1]&&son[y][0])
			return gao3(son[x][1],d-1),gao3(son[y][0],d-1),1;
		return gao2(son[x][0],son[y][0],d-1)||gao2(son[x][1],son[y][1],d-1);
	}
	return gao2(son[x][0],son[y][1],d-1)||gao2(son[x][1],son[y][0],d-1);
}
