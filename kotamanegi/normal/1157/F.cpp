...
void solve() {
    ...
    for (int i = 1; i <= 2e5 + 1; ++i) {
        if (cnt[i] == 0) {
            if (back != -1) {
                judges.push_back(mp(back, i - 1));
                back = -1;
            }
        }
        else if (cnt[i] == 1) {
            if (back != -1) {
                judges.push_back(mp(back, i));
            }
            back = i;
        }
        else {
    ...
    REP(i, judges.size()) {
        vector<int> tmps;
{
            for (int q = judges[i].first; q <= judges[i].second; ++q) {
                tmps.push_back(q);
            }
            for (int q = judges[i].second; q >= judges[i].first; --q) {
                for (int j = 0; j < cnt[q] - 1; ++j) {
                    tmps.push_back(q);
                }
            }
        }
        if (ans.size() < tmps.size()) {
            ans = tmps;
        }
    }
    cout << ans.size() << endl;
