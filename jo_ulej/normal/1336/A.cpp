...
vector<int> G[MAXN];
int n, k, depth[MAXN], take[MAXN], dist[MAXN], change[MAXN], sz[MAXN];
void dfs(int v, int prv) {
	...
	change[v] = depth[v] + 1 - sz[v];
}
void dfs2(int v, int prv) {
	dist[v] = 1 - take[v];
	if(prv != NIL) dist[v] += dist[prv];
	for(auto& u: G[v]) if(u != prv) dfs2(u, v);
}
void solve() {
	cin >> n >> k;
	...
	ll answ = 0;
	for(int i = 0; i < k; ++i) answ += change[i];
	cout << answ << '\n';
}	
