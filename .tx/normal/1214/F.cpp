...
int main(int argc, char** argv) {
    ...
    vector<pair<llong, int>> work(3 * n);
    ...
    sort(begin(work) + n, begin(work) + 2 * n);
    sort(begin(person), end(person));

    for (int i = 0; i < n; i++) {
        work[i].first = work[i + n].first - m;
        work[i].second = work[i + n].second;
        work[i + 2 * n].first = work[i + n].first + m;
        work[i + 2 * n].second = work[i + n].second;
    }

    auto get = [&](int st) {
        llong res = 0;
        for (int i = 0; i < n; i++) {
            res += abs(person[i].first - work[i + st].first);
        }
        return res;
    };
    ...
    vector<int> ans(n);
    for (int i = 0; i < n; i++) {
        ans[work[b + i].second] = person[i].second;
    }
