...
int main() {
    ...
    MULT {
        ...
        vector<pl> movs;
        F(i,0,n)
            if (s[i] == 'U') movs.push_back(dir[0]);
            else if (s[i] == 'D') movs.push_back(dir[1]);
            else if (s[i] == 'R') movs.push_back(dir[2]);
            else if (s[i] == 'L') movs.push_back(dir[3]);
        vector<pl> pos;
        pl cur{0,0};
        F(i,0,n) {
            cur = add(cur, movs[i]);
        ...
        F(i,0,n) {
            ...
            cur = {0, 0};
            F(j,0,n) {
                if (block == add(cur, movs[j])) continue;
                cur = add(cur, movs[j]);
            }
            if (cur.K == 0 and cur.V == 0) ans = block;
        }
        if (ans.K != -1e18) cout << ans.K << " " << ans.V << '\n';
        else cout << "0 0\n";
    };
}