...
mint cb[mxN][mxN][mxN][3], fact[mxN*2];

mint solve(const state& st, int i) {
    auto& c = cb[st[0]][st[1]][st[2]][i];
    if(c.d!=-1) return c;
    c=0;
    if(!st[i]) return 0;
    if(accumulate(all(st),0)==1) {
        return c=1;
    }
    state nw=st;
    nw[i]--;
...
int main() {
    ...
    rep(i,nh) rep(j,nh) rep(k,nh) rep(last,3) cb[i][j][k][last].d=-1;
    fact[0]=1;
    rep(i,nh) fact[i+1]=fact[i]*(i+1);
    mint ans=0;
    for(auto& [st, cnt] : dp[T]) {
        mint res=0;
        for(int i=0;i<3;++i) res+=solve(st,i);
        res *= cnt;
