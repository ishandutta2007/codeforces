...
void solve() {
    ...
    REP(i, m - 1) {
        ...
        inputs.push_back(mp(min(x[i], x[i + 1]), max(x[i], x[i + 1])));
        
        inputs.back().first--;
        inputs.back().second--;

        imos[inputs.back().first + 1]++;
        imos[inputs.back().second]--;
        vertexs[inputs.back().first].push_back(inputs.back().second);
        vertexs[inputs.back().second].push_back(inputs.back().first);
    }
    for (int i = 0; i < n + 1; ++i) {
        imos[i + 1] += imos[i];
    }
