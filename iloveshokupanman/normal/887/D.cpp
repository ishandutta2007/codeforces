...
int main() {
    ...
    lazy_segtree<ll, min, inf,ll,ad,ad,e> sg(N);

    ll now = start;
    rep(i, N) {
        if (q[i] == 0) { sg.set(i, now-b); }
        else { sg.set(i, now+a); }
        now = sg.get(i);
    }
    ...
    rep(i, N) {
        mp[t[i]].push_back(i);
        mp[t[i] - len].push_back(N + i);
    }
    if (!mp.count(-1)) { mp[-1] = vll(0, 0); }
    ...
    for (auto p : mp) {
        ...
        for (ll x : v) {
            if (x < N) {
                if (q[x] == 0) { sg.apply(x,N,d-b); }
                else {sg.apply(x, N, a - c); }
            }
            else {
                if (q[x-N] == 0) { sg.apply(x-N,N, b-d); }
                else { sg.apply(x - N, N, c - a); }
            }
        }

        //rep(i, N)cout << sg.get(i) << " ";
        //cout << endl;

        auto itr = lower_bound(t.begin(), t.end(), time + len);
        ll x = distance(t.begin(), itr);
        if (sg.prod(0, x) >= 0&&time>=0) { cout << time << endl; return 0; }
    }
