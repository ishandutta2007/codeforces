...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        g[i] = readInt(), p[i] = readInt(), t[i] = readInt();
        a[i] = make_pair(p[i] - t[i], i);
    }
    sort(a, a + n);
    for (int l = 0, r = 0; l < n; l = r) {
        ...
        sort(finish + l, finish + r);
        for (int i = l; i < r; i++) {
            ans[start[i].second] = make_pair(finish[i].first, -finish[i].second);
        }
    }
    for (int i = 0; i < n; i++) {
