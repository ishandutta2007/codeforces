...
fn rec(k: usize, l: usize, r: usize, ql: usize, qr: usize, a: &[Vec<i32>], dp: &mut [Vec<i32>]) {
	let m = (l+r) / 2;
	let mut qm = ql;
	let mut best = 1000000000;
	let mm = if m == 0 {0} else {m-1};
	for i in ql..=qr.min(mm) {
		if min!(best, dp[k-1][i] + a[i+1][m]) {
			qm = i;
		}
	}
	dp[k][m] = best;
	if l < m {
		rec(k, l, m-1, ql, qm, a, dp);
	}
	if m < r {
		rec(k, m+1, r, qm, qr, a, dp);
	}
}
...
fn main() {
	...
	let a = {
		...
		let mut a = v!([n+1][n+1] = 0);
		for i in 0..n {
			for j in i..n {
				a[i+1][j+1] = (b[i][i] + b[j+1][j+1] - b[i][j+1] - b[j+1][i]) / 2;
			}
		}
		a
	};
	let mut dp = v!([k+1][n+1] = 1000000000);
	dp[0][0] = 0;
	for i in 1..=k {
		rec(i, 1, n, 0, n, &a, &mut dp);
	}
	let sol = dp[k][n];
	writeln!(rout, "{}", sol).ok();
}