...
int main() {
    ...
    map<ll, ll> mp;
    rep(i, N) {
        if (mp.count(a[i])) { mae[i] = mp[a[i]]; }
        mp[a[i]] = i;
    }

    vector<lazy_segtree<ll, max, e, ll, ad, ad, e>>lsg(K + 1, lazy_segtree<ll, max, e, ll, ad, ad, e>(N+1));

    rep(i, K + 1)rep(j, N + 1)lsg[i].set(j, -INF);
    lsg[0].set(0, 0);

    rep(i, N)rep(j, K) {
        ll x = mae[i];
        lsg[j].apply(x + 1, i + 1, i - x);

        ll val = lsg[j].prod(0, i+1);
        lsg[j + 1].set(i + 1, val);
        //rep(k, N)cout << lsg[j].get(k) << " ";
        //cout << endl;
    }
   
    ll now = lsg[K].prod(0, N + 1);
    //cout << now << endl;
