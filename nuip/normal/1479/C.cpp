...
vector<tuple<int,int,int>> solve(int l,int r){
	fill(g[0],g[32],0);
	cauto n=32;
	int t;
	rep(i,20){
	...
	rep(i,n)rep(j,i)if(g[j][i]) re.eb(j,i,g[j][i]);
	return re;
}

void verify(vector<tuple<int,int,int>> re,int l,int r){
	auto dp=table(32,r+1,0);
	dp[0][0]=1;
	vv<pii> g(32);
	for(cauto [a,b,c]:re) g[a].eb(b,c);
	rep(v,32)for(cauto [w,d]:g[v])rep(i,r) dp[w][min(r,i+d)]+=dp[v][i];
	rep(i,r+1)if(dp[31][i]!=(l<=i && i<r)){
		out(i,l,r,re,dp[31][i],1);
		exit(0);
	}
}
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	//out(solve(1,8),1); return 0;
	if(0){
		rep(r,1000)reps(l,1,r){
			cauto re=solve(l,r);
			if(re.empty()){
				out(l,r,1);
				exit(0);
	...
	cout<<"YES" NL;
	cout<<32<<" "<<re.size() NL;
	for(cauto [a,b,c]:re) cout<<a+1<<" "<<b+1<<" "<<c NL;
	// rep(i,n)rep(j,i)if(g[j][i]) cout<<j+1<<" "<<i+1<<" "<<g[j][i] NL;
  return 0;
}