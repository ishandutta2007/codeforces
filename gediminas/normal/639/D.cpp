...
int main () {
	...
	sort(a.begin(), a.end());
	ll ats = 1000000000000000000ll;
	mini(b, 5 * c);

	for (int liek = 0; liek < 5; liek++) {
		vector<pair<ll, int>> tv;

		for (int i = 0; i < n; i++) {
			if (a[i] % 5 == liek) {
				tv.emplace_back(a[i] * b, i);
			}
			else {
				tv.emplace_back((a[i] + reik(a[i], liek))*b - 5 * reik(a[i], liek) * c, i);
			}
		}
		...
		for (int i = 0; i < n; i++) {
			...
			if ((int)jau.size() == k) {
				ll reikia = (k * (a[i] + reik(a[i], liek)) * b - suma) / 5;
				debug(i, reikia, liek, (a[i] + reik(a[i], liek)));
				mini(ats, reikia);
			}
		}
	}
