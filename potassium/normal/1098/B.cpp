...
int main() {
	int n,m; R(n,m);
	vector<string> b(n); R(b);
	
	map<char, int> mp = {{'A', 0}, {'G', 1}, {'C', 2}, {'T', 3}};
	vector<char> rmp = {'A', 'G', 'C', 'T'};
	...
	auto buildCnt = [&](int n, int m, vector<string>& b) {
		cnt = vector<vector<vector<int>>>(2, vector<vector<int>>(m, vector<int>(4, 0)));
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				cnt[i % 2][j][mp[b[i][j]]]++;
			}
	...
	auto solve = [&](int n, int m, vector<string>& b, int typ) {
		buildCnt(n, m, b);
		
		auto go = [&](vector<int>& fir, vector<int>& sec) {
			ll maxv = 0;
			
			for (int j = 0; j < m; j++) {
				vector<int>& now = (j % 2 == 0 ? fir : sec);
				
				ll v1 = cnt[0][j][now[0]] + cnt[1][j][now[1]];
				ll v2 = cnt[0][j][now[1]] + cnt[1][j][now[0]];
