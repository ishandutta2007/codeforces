...
int main()
{
    ...
    TC(t)
    {
    		scanf("%d %d",&a,&b);
		int difx,dify;
		if (a < 0) difx = (posx - 1); else difx = n - posx;
		if (b < 0) dify = (posy - 1); else dify = m - posy;
		if (a == 0) jarak = dify / abs(b); else
		if (b == 0) jarak = difx / abs(a); else
		jarak = min(difx / abs(a),dify / abs(b));
		ans = ans + jarak;
		posx = posx + jarak * a;
		posy = posy + jarak * b;
		//printf("%d %d\n",posx,posy);
