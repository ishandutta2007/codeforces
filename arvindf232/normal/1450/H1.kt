...
fun main(){
    ...
    solve.cases{
        ...
        val oddq = qcount - evenq
        var options = intPow(2,qcount - 1 , pI)
        var ret = 0
        val start = evenb * 2 - bcount - oddq
        for(take in 0..qcount){
            if((start + take) %2 != 0) continue
            val real = (take + start) / 2
            ret = ret modPlus (real.absoluteValue modM FACT.choose(qcount,take))
        }
//        for(extrab in 0..qcount){
//            if((extrab + bcount) % 2 != 0) continue
//            val realbcount = (extrab + bcount)
//            for(infused in 0..evenq){
//                val counts = FACT.choose(evenq,infused) modM FACT.choose(oddq,extrab - infused)
//                ret = ret modPlus ((infused + evenb - (realbcount /2)).absoluteValue modM counts)
//            }
//        }
        put(ret modDivide options)
        //every even: yes += 1/2, no : 0  otherwise even -1/2 no: 0  ((evenb - bcount/2 - oddq/2) start )
