...
int main () {
	...
	for (int i = 0; i < n; i++) {
		...
		if (a[1] == 'D') {
			if (buy.size() > 0 and x < (*buy.begin()))
				buy.insert(x);
			else if (sell.size() > 0 and x > (*sell.begin()))
				sell.insert(x);
			else
				nez.insert(x);
		}
		...
		if (sell.size() > 0 and buy.size() > 0 and (*sell.begin()) < (*buy.begin())) {
			exit(1);
		}
