...
void query(Node *t,int l,int r,int x,int &v){
	if(t == nullptr || r<=x || t->mn>v) return;
	if(l>=x){
		v = t->mn;
		return;
	}
...
void add(Node *&t,int l,int r,int x,int v){
	...
	if(r-l == 1)return;
	int m = (l+r)/2;
	if(x<m) add(t->lc,l,m,x,v);
	else add(t->rc,m,r,x,v);
}
...
int main() {
	...
	ROF(x,1,n+1){
		//dbg(byl[x],x);
		each(i,byl[x])add(l[i],r[i],i);
		function<void(int, int)> Solve = [&](int fr,int to){
			if(to-fr<x)return ;
			int u = query(fr,to);
			//dbg(fr,to,u);
			if(u >=m )return;
