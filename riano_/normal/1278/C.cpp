...
vector<int> mineat(vector<int> j,int s){
    vector<int> res(s+1,1e9);
    int N = j.size(); ll p = 0;
    rep(i,N+1){
        if(p>=0&&p<=s) chmin(res[p],i);
        if(i==N) break;
        if(j[i]==1) p++;
        else p--;
    }
...
int main(){
    ...
    rep(ii,T){
        ...
        if(s1<0){
            s1 *= -1;
            rep(i,N){
                j1[i] = 3-j1[i]; j2[i] = 3-j2[i];
            }
        }
        auto v1 = mineat(j1,s1);
        auto v2 = mineat(j2,s1);
        rep(i,s1+1){
            chmin(ans,v1[i]+v2[s1-i]);
        }
        cout << ans << "\n";
    }
    //cout << ans << endl;
