...
int main(int argc, char** argv) {
    ...
    vector<vector<int>> up(22, vector<int>(n, -1));
    vector<vector<int>> mx(22, vector<int>(n, -2e9));
    ...
    function<void(int, int, int)> dfs = [&](int v, int p, int w) {
        ...
        up[0][v] = p;
        mx[0][v] = w;
        for (int i = 0; up[i][v] != -1; i++) {
            up[i + 1][v] = up[i][up[i][v]];
            mx[i + 1][v] = max(mx[i][v], mx[i][up[i][v]]);
        }
    ...
    auto max_path = [&](int a, int b) {
        int c = lca(a, b);
        return max(get_mx(a, c), get_mx(b, c));
    };
