...
void solve() {
    cin >> n >> C;
    c.rsz(n); d.rsz(n); h.rsz(n);
    F0R(i, n) cin >> c[i] >> d[i] >> h[i];
    F0R(i, n) score[c[i]] = max(score[c[i]], (ll)d[i]*h[i]);
    mx.rsz(C+1, 0);
    for (auto p : score) {
        for (ll i = 1; i*p.F <= C; i++) ckmax(mx[i*p.F], i*p.S - 1);
    }
    FOR(i, 1, C+1) ckmax(mx[i], mx[i-1]);
    cin >> m;
    F0R(query, m) {
        cin >> D >> H;
        ll prod = (ll)D*H;
