...
int main(void)
{
	...
	cin >> e >> s >> n >> Q;
	rep(i, 1, n) cin >> p[i].second >> p[i].first;
	sort(p+1, p+n+1);
	
	while(n > 0 && p[n].first >= e) n--;
	n++, p[n] = P(e, 3);
	
	ll l = e - s;
	per(i, n-1, 1){
		if(p[i].first < l) break;
		l = p[i].first - s;
	}
	...
	rep(i, 1, n){
		if(p[i].second >= 2) vec2.push_back(P(p[i].first, i));
		if(p[i].second >= 3) vec3.push_back(P(p[i].first, i));
	}
	...
	per(i, n-1, 1){
		if(p[i].second >= 2){
			ll j = (upper_bound(all(vec2), P(p[i].first, inf)))->second;
			dp2[i] = min(s, p[j].first - p[i].first) + dp2[j];
		}
		if(p[i].second >= 3){
			ll j = (upper_bound(all(vec3), P(p[i].first, inf)))->second;
			dp3[i] = min(s, p[j].first - p[i].first) + dp3[j];
		}
	}
	...
	rep(q, 1, Q){
		...
		ll j = (upper_bound(all(vec2), P(f, inf)))->second;
		ll use2 = min(s, p[j].first - f) + dp2[j];
		
		j = (upper_bound(all(vec3), P(f, inf)))->second;
		ll use3 = min(s, p[j].first - f) + dp3[j];
		
		cout << use1 - use2 << " " << use2 - use3 << "\n";
	}
