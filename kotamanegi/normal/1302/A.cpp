...
void solve() {
    ...
    vector<int> gekos;
    REP(i, inputs.size()) {
        REP(q, min(inputs[i].second,h)) {
            gekos.push_back(inputs[i].first);
        }
    }
    vector<vector<int>> ans(h, vector<int>(w, 0LL));
    REP(i, h) {
        REP(q, w*h) {
            if (ans[(i + q) % h][q % w] != 0) break;
            ans[(i + q) % h][q%w] = gekos.back();
            gekos.pop_back();
        }
    }
    REP(i, h) {
