...
#endif
int c[505][505],b[505],d[505],e[505],cc,f[505];
const double eps=1e-10;
double a[505][1005]; 
int eqaq(double x){return -eps<x&&x<eps;}
void swap(double &a,double &b){int t=a;a=b;b=t;}
int L;
struct xjoi{
	double a[105][105];
	void fi(int x)
	...
	xjoi operator*(const xjoi& b)const{
		xjoi c;c.fi(0);
		rg(L)rg_(j,L)rg_(k,L)
	...
	void print()
	{
		rg(L)rg_(j,L)printf("%.4lf ",a[i][j]);gr puts("");gr
	}
}x,res;
signed main()
{
	...
	rg(n)b[i]=re1d();if(b[i])e[cc]=i,f[i]=cc++;gr
	rg(m)
	int u=read(),v=read();
	c[u][v]++,c[v][u]++;
	d[u]++;d[v]++;
	...
	rg(n)if(b[i]){a[i-1][i-1]=1,a[i-1][n+f[i]]=1;}else{
	a[i-1][i-1]=1;
	rg_(j,n)if(c[i][j])a[i-1][j-1]=-1.0*c[i][j]/d[i];gr
	}gr
	// for(int i=0;i<n;i++,puts(""))for(int j=0;j<n+cc;j++)printf("%.4lf ",a[i][j]);puts("");
	for(int i=0;i<n;i++)
	{
		int amxn=i;
		// for(int i=0;i<n;i++,puts(""))for(int j=0;j<n+cc;j++)printf("%.4lf ",a[i][j]);puts("");
	
		for(int j=i+1;j<n;j++)if(fabs(a[j][i])>fabs(a[amxn][i]))amxn=j;
		if(i!=amxn)for(int j=0;j<n+cc;j++)swap(a[i][j],a[amxn][j]);
		// for(int i=0;i<n;i++,puts(""))for(int j=0;j<n+cc;j++)printf("%.4lf ",a[i][j]);puts("");
	
		if(eqaq(a[i][i]))return puts("No Solution")&0;
	//	for(int i=0;i<n;i++,puts(""))
	//		for(int j=0;j<=n;j++)printf("%.2lf ",a[i][j]);puts("");
	
		for(int j=i+1;j<n+cc;j++)a[i][j]/=a[i][i];
		a[i][i]=1.0;
		for(int j=0;j<n;j++)
		{
			if(i^j)
			{
				for(int k=i+1;k<n+cc;k++)a[j][k]-=a[j][i]*a[i][k];
				a[j][i]=0;
			}
		}
	}
	...
	for(int i=0;i<cc;i++)
		for(int j=0;j<cc;j++)
		{
			...
			for(int k=1;k<=n;k++)
			{
				if(c[u][k])
				{
					x.a[f[u]+2][f[v]+2]+=1.0*c[u][k]/d[u]*a[k-1][n+f[v]];
				}
			}
		}
	// for(int i=0;i<n;i++)
	// {
		...
		// {
			...
			// {
				// odp(i,j);
				// x.a[f[i+1]+2][f[j+1]+2]=a[i][n+f[j+1]];
			// }
		// }
	// }
	for(int i=0;i<1;i++)
