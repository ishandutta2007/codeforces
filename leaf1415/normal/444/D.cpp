...
int Q;
int a[50005][5];
vector<int> vec[110005];
ll len[110005];

int B = 230;
vector<int> bvec;
unsigned short pre[110005][1005];
int succ[1005];

unsigned short get(ll i, ll k, ll j, ll l)
{
	return max(i+k, j+l) - min(i, j);
}
...
int main(void)
{
	...
	ll id = 0;
	for(auto &p : mp) p.se = ++id;
	
	rep(l, 1, 4){
		rep(i, 1, n){
			if(i+l-1 > n) break;
			a[i][l] = mp[s.substr(i, l)];
			len[a[i][l]] = l;
			vec[a[i][l]].pb(i);
		}
	}
	rep(i, 1, id) if(sz(vec[i]) >= B) bvec.pb(i);
	
	rep(i, 1, id) reps(j, bvec) pre[i][j] = n+1;
	rep(i, 1, n){
		reps(j, bvec){
			ll x = bvec[j];
			if(a[i][len[x]] == x) succ[j] = i;
		}
		reps(j, bvec){
			ll x = bvec[j];
			if(succ[j] > 0){
				rep(k, 1, 4){
					chmin(pre[a[i][k]][j], get(succ[j], len[x], i, k));
				}
			}
		}
	}
	reps(j, bvec) succ[j] = n+1;
	per(i, n, 1){
		reps(j, bvec){
			ll x = bvec[j];
			if(a[i+1][len[x]] == x) succ[j] = i+1;
			if(succ[j] <= n){
				rep(k, 1, 4) chmin(pre[a[i][k]][j], get(i, k, succ[j], len[x]));
			}
		}
	}
	...
	rep(q, 1, Q){
		...
		if(sz(vec[y]) >= B){
			ll p = lower_bound(all(bvec), y) - bvec.begin();
			cout << pre[x][p] << "\n";
		}
		else{
			ll q = 0; unsigned short ans = n+1;
			for(auto p : vec[x]){
				while(q+1 < sz(vec[y]) && vec[y][q+1] <= p) q++;
				chmin(ans, get(vec[y][q], len[y], p, len[x]));
				if(q+1 < sz(vec[y])) chmin(ans, get(p, len[x], vec[y][q+1], len[y]));
			}
			cout << ans << "\n";
		}
