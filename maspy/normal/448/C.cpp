...
void solve() {
  ...
  VEC(ll, A, N);
  const ll INF = 1LL << 60;
  SegTree<pi> seg([](pi x, pi y) { return min(x, y); }, mp(INF, -1));
  seg.init(N);
  FOR(i, N) seg.set(i, mp(A[i], i));

  auto dfs = [&](auto self, ll L, ll R, ll h) -> ll {
    if (L == R) return 0;
    // if (R - L == 1) return (h == A[L] ? 0 : 1);
    auto [x, M] = seg.fold(L, R);
    ll res = R - L;
    chmin(res, (x - h) + self(self, L, M, x) + self(self, M + 1, R, x));
    // print(L, R, h, res);
    return res;
  };
  ll ANS = dfs(dfs, 0, N, 0);
  print(ANS);
}
