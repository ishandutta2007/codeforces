...
void up(){
	...
	std::vector<int> missing(number+1); //[value]->length deficiency in a. Must be nonnegative.
...
cannot:
	...
	for(int index=number-1, last=number; index>=0; index--){
		if(index==0 or b[index-1]!=b[index]){
			// consider run [index..last]
			int const value=b[index];
			while(true){
				if(iter==a.begin()){
					// somehow cannot find value!? this should not happen but... "just in case"...
					assert(false);
					goto cannot;
				}
				--iter;
				...
				if(*iter==value){
					// okay here it is, use it
					int countA=1;
					while(iter!=a.begin() and iter[-1]==value){
						++countA;
						--iter;
					}

					auto const countB=last-index;
					if(countA>countB){
						// have to throw some (value)
						if(countA-countB>missing[value]) goto cannot;
						missing[value]-=countA-countB;
						countA=countB;
					}

					assert(countA<=countB);
					missing[value]+=countB-countA;
					break;
				}else{
					// not found yet...
					// throw the value at *iter
					if(missing[*iter]==0) goto cannot;
					--missing[*iter];
				}
			}
...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1672/problem/D