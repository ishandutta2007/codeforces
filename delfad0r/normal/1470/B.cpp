...
void solve() {
	int N;
	cin >> N;
	map<vector<int>, int> M0, M1;
	int ans0 = 0, ans1 = 0;
	while(N--) {
		int a;
		cin >> a;
		ans0 = max(ans0, ++M0[factors[a]]);
	}
	for(const auto& [v, i] : M0) {
		if(i % 2) {
			ans1 = max(ans1, M1[v] += i);
		} else {
			ans1 = max(ans1, M1[{}] += i);
		}
	}
	...
	while(Q--) {
		...
		cin >> w;
		if(w == 0) {
			[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(ans0), "\n");  }();
		} else {
			[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(ans1), "\n");  }();
		}
	}
}
