...
int main() {
    ...
    set<H> dict;
    for (auto &i : s) {
        dict.insert(H::get_prefixes(all(i)).back());
    }

    while (m--) {
        string t;
        re(t);
        auto prefix = H::get_prefixes(all(t));
        bool ans = false;
        for (int i = 0; i < sz(t); i++) {
            for (char c = 'a'; c <= 'c'; c++) {
                ...
                auto hash = H::get_substring(prefix, 0, i);
                hash = H::concatenate(hash, H{c});
                hash = H::concatenate(hash, H::get_substring(prefix, i + 1, sz(t)));
