    ...
    TEST {
        ...
        REC([&](auto &&f, int x, int p) -> void {
            ...
            rep(t, 2) { // (x, p) 
                ...
                if((s + 1) / 2 <= c[2] + c[3] and s / 2 <= c[1] + c[3]) ok[x][t] = true;
            }
        })
        (0, -1);
        // dump(ok);
        ...
        deque<pii> ans;
        REC([&](auto &&f, int x, int p, int t) -> void {
            dump(x, p, t);
            vv(int, v, 4);
            int s = 0;
            fore(e, g[x]) {
            ...
            rep(i, s) {
                ...
                int t;
                if(!empty(v[k]))
                    t = v[k].back(), v[k].pop_back();
                else
                    t = v[3].back(), v[3].pop_back();
                if(t == x)
                    flag = true;
                else {
                    if(flag)
                        ans.emplace_front(x, t);
                    else
