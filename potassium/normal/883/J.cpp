...
int main() {
	...
	vector<int> time(m);
	for (int i = 0; i < m; i++) {
		if (b[i] > a[MonoStack[0]]) {
			time[i] = -1;
			continue;
		}

		int now = 0;
		for (int jump = 131072; jump >= 1; jump >>= 1) {
			if (now + jump < (int)MonoStack.size() && a[MonoStack[now + jump]] >= b[i])
				now += jump;
		}
		time[i] = MonoStack[now];
	}

	vector<pii> sorted_time;
	for (int i = 0; i < m; i++)
		if (time[i] != -1)
			sorted_time.push_back({ time[i], i });
	sort(rall(sorted_time));
	...
	ll spent = 0;
	maxheap<int> taken;
	while (!sorted_time.empty()) {
		int curTime = sorted_time.back().fi;
		int curIdx = sorted_time.back().se;
		ll curTotal = pa[curTime];

		if (curTotal - spent >= p[curIdx]) {
			spent += p[curIdx];
			taken.push(p[curIdx]);
		}
		else if (!taken.empty() && taken.top() >= p[curIdx]) {
			spent -= taken.top();
			taken.pop();
			spent += p[curIdx];
			taken.push(p[curIdx]);
