// Double Sort (https://codeforces.com/contest/1681/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        sort(all(idx), [&](int i, int j) {
            return pair(a[i], b[i]) < pair(a[j], b[j]);
        });

        bool good = true;
        rep(i, 1, n)
            good &= (a[idx[i - 1]] <= a[idx[i]] && b[idx[i - 1]] <= b[idx[i]]);
        if (good) {
            vector cur(n, 0);
            iota(all(cur), 0);
            ...
            rep(i, n) {
                auto it = find(all(cur), idx[i]);
                int j = int(it - begin(cur));
                if (i != j)
                    swaps.emplace_back(i + 1, j + 1), swap(cur[i], cur[j]);
            }
