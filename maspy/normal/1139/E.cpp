...
void solve() {
  ...
  auto right = [&](int i) -> int { return 1 + M + i; };

  Incremental_MaxFlow<ll> G(N + LIM + 2, source, sink, INF);
  FOR(i, M) G.add(source, left(i), 1);
  FOR(i, LIM) G.add(right(i), sink, 1);

  vi ANS(Q);
  ll mex = 0;
  vvc<int> edges(LIM + 10);
  FOR_R(q, Q) {
    FOR(i, N) if (day[i] == q + 1) { edges[A[i]].eb(C[i]); }
    while (1) {
      // mex 
      FOR(p, mex + 1) {
        for (auto&& to: edges[p]) { G.add(left(to), right(p), 1); }
        edges[p].clear();
      }
      ll x = G.flow();
      if (mex == x) break;
