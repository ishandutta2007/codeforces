...
int main() {
    ...
    scanf("%d", &n);
    FI(n) {
        scanf("%d%d%d%d", &cards[i].a, &cards[i].b, &cards[i].c, &cards[i].d);
        ids[i] = i;
    }
    sort(ids, ids + n, [&](int x, int y) {
        return cards[x].a < cards[y].a;
    });
    for (int i = 0; i < n; ++i) {
        d[i] = -1;
    }
    ...
    for (int i = 0; i < v.size(); ++i) {
        auto& w = v[i];
        minx[i] = cards[w.front()].a;
        maxx[i] = cards[w.back()].a;
        sort(w.begin(), w.end(), [&](int x, int y){
            return cards[x].b > cards[y].b;
        });
    }
    vector<int> q = {n};
    int qc = 0;
