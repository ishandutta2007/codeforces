...
int d[N<<1];
int tail;
int qst(int x,int y){printf("0 %d %d\n",x,y);fflush(stdout);int ans;rin>>ans;return ans;}
void work(int l,int r)
{
    int mid=l+r>>1,ans;if(l>r)return;
    if((ans=qst(mid,mid))==0)d[++tail]=mid,work(l,mid-1),work(mid+1,r);else work(l,mid-ans),work(mid+ans,r);return;
}
...
{
    srand(time(0));
    int l=-1e8,r=1e8,zjj;work(l,r);
    for(int i=1;i<=tail;i++)gyq.insert(d[i]);
    for(;true;){zjj=(rand()-1)%r+1;if(gyq.find(zjj)==gyq.end())break;}
    for(int i=1;i<=tail;i++)
    {
        if(qst(d[i],zjj)==0)X[++n]=d[i];
        if(qst(zjj,d[i])==0)Y[++m]=d[i];
    }
    printf("1 %d %d\n",n,m);
