...
string s;
ll comp[5005][5005];
mint dp[5005][5005], sum[5005][5005];
...
int main(void)
{
	...
	rep(l, 1, n-1){
		if(s[l] < s[n]) comp[l][n] = -1;
		if(s[l] > s[n]) comp[l][n] = 1;
	}
	
	per(r, n-1, 1) rep(l, 1, r-1){
		if(s[l] < s[r]) comp[l][r] = -1;
		else if(s[l] > s[r]) comp[l][r] = 1;
		else comp[l][r] = sgn(comp[l+1][r+1]) * (abs(comp[l+1][r+1])+1);
	}
	
	//rep(i, 1, n) outa(comp[i], 1, n);
	...
	rep(i, 1, n){
		...
		rep(j, 1, i-1){
			if(s[j+1] != '0'){
				ll len = i-j;
				dp[i][j] = sum[j][j-1];
				if(j-len >= 0) dp[i][j] -= sum[j][j-len];
				if(j-len >= 0 && comp[j-len+1][i-len+1] < 0 && comp[j-len+1][i-len+1] >= -len) dp[i][j] += dp[j][j-len];
			}
			sum[i][j] = sum[i][j-1] + dp[i][j];
		}
	}
	
	//rep(i, 1, n) outa(dp[i], 0, n);
	
	mint ans = 0;
	rep(i, 0, n-1) ans += dp[n][i];
	outl(ans);
