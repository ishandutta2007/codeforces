...
int main(){
  ...
  while(t--){
    ll n,m;cin>>n>>m;
    auto g1=readGraph<ll>(n,m,1,false,true);
    Graph<ll>g2(n);
    rep(i,0,n){
      for(auto e:g1[i]){
        g2[i].EB(e.to,1);
    ...
    auto d=WarshallFloyd(g2);
    ll ret=INF;
    vector<ll>mi(n,INF);
    rep(i,0,n){
      rep(j,0,n){
        chmin(mi[i],d[i][j]+d[j][0]+d[j][n-1]+1);
      }
    }
    rep(i,0,n){
      for(auto e:g1[i]){
        //cout<<i spa e.to spa d[i][0] spa d[e.to][n-1] spa mi[i] spa e.cost<<endl;
        chmin(ret,e.cost+e.cost*min(d[i][0]+d[e.to][n-1],mi[i]));
      }
    }
    cout<<ret<<endl;
  }
