...
namespace MAIN{
    ...
    int he,ta;
    inline bool bfs(){
        res L=m+1,R=0,U=n+1,D=0;
        while(he<=ta){
            auto [x,y]=Q[he++];
            L=min(L,y),R=max(R,y),U=min(U,x),D=max(D,x);
            for(res k=0;k<4;k++){
                res nx=x+dx[k],ny=y+dy[k];
                if(nx<1||ny<1||nx>n||ny>m||vis[nx][ny]||!a[nx][ny])continue;
                vis[nx][ny]=1,Q[++ta]=mp(nx,ny);
            }
        }
        return (R-L+1)*(D-U+1)!=ta;
    }
    inline void MAIN(const res &Case){
        n=read(),m=read();
        for(res i=1;i<=n;i++){
            scanf("%s",str+1);
            for(res j=1;j<=m;j++)a[i][j]=str[j]-'0',vis[i][j]=0;
        }
        for(res i=1;i<=n;i++){
            for(res j=1;j<=m;j++){
                if(vis[i][j]||!a[i][j])continue;
                vis[i][j]=1,Q[he=ta=1]=mp(i,j);
                if(bfs()){puts("NO");return;}
            }
        }
        puts("YES");
    }
