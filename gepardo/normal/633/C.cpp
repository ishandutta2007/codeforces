...
inline int findHere(pii el)
{
    vector < pair<pii, int> > :: iterator p = upper_bound(q.begin(), q.end(), (pair<pii, int>){el, -123456789});
    if (p == q.end()) return 0;
    return (p -> first == el) ? p -> second : 0;
}
...
main()
{
    ...
    vector<string> o;
    in.getv(o, k);
    vector < pair<string, string> > r(k);
    for (int i = 0; i < k; i++)
    {
        r[i].ff = o[i];
        r[i].ss = o[i];
        reverse(r[i].ss.begin(), r[i].ss.end());
        int l = r[i].ss.length();
        for (int j = 0; j < l; j++) r[i].ss[j] = r[i].ss[j] | 32;
    }  
    vector <int> p(n + 1);
    for (int i = 0; i < k; i++)
    {
        ...
        for (int j = 0; j < l; j++)
        {
            sh = (sh * 1030992887 + r[i].ss[j]) % 1973988871;
            qh = (qh * 1427936681 + r[i].ss[j]) % 1932013429;
        }
        q.push_back({{sh, qh}, i + 1});
    }
    sort(q.begin(), q.end());

    for (int i = 0; i < n; i++) p[i] = 0; p[0] = -1;
    for (int i = 0; i < n; i++)
    {
        if (!p[i]) continue;
        long long sh = s[i], qh = s[i];
        for (int j = i + 1; j <= min(i + 1001, n); j++)
        {
            if (!p[j])
                p[j] = max(p[j], findHere({sh, qh}));
            if (j != n)
            {
                sh = (sh * 1030992887 + s[j]) % 1973988871;
                qh = (qh * 1427936681 + s[j]) % 1932013429;
            }
        }
    }
    vector <string> ans;
