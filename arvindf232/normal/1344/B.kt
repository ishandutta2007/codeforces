...
fun ok(a:BooleanArray):Boolean {
//    just dei a.contentToString()
    var state = 0
    for(i in a){
        if(!i){
            if(state == 1) state = 2
...
fun main(){
    solve.cases{
        val n = getint()
        val m = getint()
         val all = Array(n){BooleanArray(m)}
        ...
        val tranposed = Array(m){x -> BooleanArray(n){y -> all[y][x]} }
        val ok1 = (0 until n).all { ok(all[it]) }
        val ok2 = (0 until m).all{ ok(tranposed[it])}
        if(!ok1 || !ok2){
            put(-1)
            return@cases
        }
        val allempty = (0 until n).any { all[it].all { it== false } }
        val allempty2 = (0 until m).any { tranposed[it].all { it== false }}
        if(allempty xor allempty2){
            put(-1)
            return@cases
        }
        val visited = Array(n){BooleanArray(m)}

        val dsu = DisjointUnionSets(n * m)
        fun code(a:Int, b:Int):Int {
            return a * m + b
        }
        var no = 0
        for(i in 0 until n){
            for(j in 0 until m){
                ...
                for(d in 0 until 4){
                    val ii = i + dx[d ]
                    val jj = j + dy[d]
                    if(ii !in 0 until n || jj !in 0 until m) continue
                    if(!all[ii][jj]) continue
                    val new = code(ii,jj)
                    dsu.union(here,new)
                }
            }
        }
        put(dsu.components - no)
