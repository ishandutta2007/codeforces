...
vector<int> tbl[ten(5) * 2 + 1];

void init() {
	init_prime();
	for (auto x : prs) {
		for (int i = x; i <= ten(5) * 2; i += x) {
			tbl[i].push_back(x);
		}
	}
}
...
unordered_map<int, int> memo[ten(5) * 2];
int pans[ten(5) * 2];
...
void dfs(int v, int par) {
	...
	for (auto d : tbl[a[v]]) {
		...
		for (auto to : e[v]) {
			if (par == to) continue;
			cur.update(memo[to][d]);
		}
		memo[v][d] = cur.f + 1;
		pans[v] = max(pans[v], cur.f + cur.s + 1);
	}
}
