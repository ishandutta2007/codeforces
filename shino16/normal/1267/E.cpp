...
int main() {
  ...
  auto a = make_v(m, n, 0);
  rep(r, m) a[r] = IN.vi(n);
  vector<pair<VI, int>> b(m);
  rep(r, m) b[r] = {a[r], r};
  int res = 0; VI x(m); iota(all(x), 1);
  rep(above, n-1) {
    sort(all(b), [&](auto& u, auto& v){
      return u._1[above] - u._1[n-1] > v._1[above] - v._1[n-1];
    });
    rep(r, m) { debugv(b[r]._1); debug(b[r]._2);}
    int s = 0, cnt = 0; VI t;
    for (auto& v : b) {
      if (s + v._1[above] - v._1[n-1] < 0) break;
      s += v._1[above] - v._1[n-1];
      cnt++;
      t.push_back(v._2);
    }
    if(chmax(res, cnt)) {
      ...
      debugv(t);
      VB k(m);
      for (int p : t) k[p] = true;
      x.clear();
      rep(r, m) if (not k[r]) x.push_back(r+1);
    }
  }
  OUT(m-res);
  OUT.range(x);
