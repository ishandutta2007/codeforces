...
int main() {
    ...
    int N; re(N); vi _a(N); re(_a);
    vpii a; F0R (i, 3 * N) a.eb(_a[i % N], i);
    sparse_table st(a, [](pii x, pii y){ return min(x,y); });

    auto solve = [&](auto& self, int i, int j) -> int {
        if (i >= j) return 0;
        if (j - i == 1) return 1;
        int k = st(i, j).s;
        return 1 + max(self(self, i, k), self(self, k+1, j));
    };

    int j = N; while (a[j].f != 1) j++;
    int i = j - N + 1;
    const int i1 = j;
    ...
    while (i <= j) {
        int s = (i + j) / 2;
        int ls = (N - (i1-s) + (i1-N)) % N;
        int l = solve(solve, s, i1);
        int r = solve(solve, i1+1, s + N);
        ckmin(ans, pii{1 + max(l, r), ls});
        if (l > r) i = s + 1;
        else j = s - 1;
    }
