...
bool brute(li n, vi A, vi B){
  set<vi> seen;
  vvi Q;
  seen.insert(A); Q.push_back(A);
  while(!Q.empty()) {
    vi X = Q.back(); Q.pop_back();
    for(li i = 0; i < (li)(n); ++i) for(li j = (i+2); j <= (li)(n-1); ++j) if(X[i]==X[j]) {
      vi Y = X;
      rotate(begin(Y)+i,begin(Y)+i+1,begin(Y)+j+1);
      if(!seen.count(Y)) {
        seen.insert(Y); Q.emplace_back(Y);
      }
    }
  }
  return seen.count(B);
...
bool solve(li n, vi A, vi B){
  vi E(n,1);
  for(li i = 0; i < (li)(n-1); ++i) if(B[i] == B[i+1]) E[i]=0;

  vi seenb(n,0);
  li j = n-1;
  for(li i = (n-1); i >= (li)(0); --i) if(E[i]) {
    while(j >= 0 && A[j] != B[i]) {
      if(!seenb[A[j]]) return false;
      seenb[A[j]]--;
      j -= 1;
    }
    if(j < 0) return false;
...
int main(){
  ...
  li t; cin>>t; for(li t_ = 0; t_ < (li)(t); ++t_) {
    ...
    for(li i = 0; i < (li)(n); ++i) { cin>>B[i]; --B[i]; }
