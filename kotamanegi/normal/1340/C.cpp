...
void solve() {
    ...
    while (!a.empty()) {
        while (!a.empty()) {
            pair<int, int> now = a.front();
            a.pop();
            if (now.second == 0) {
                if (dist[now.first][g] > dist[now.first][0] + r) {
                    dist[now.first][g] = dist[now.first][0] + r;
                    b.push(mp(now.first, g));
                }
                continue;
            }
            if (now.first < m - 1) {
                int nexts = now.second - (safety[now.first + 1] - safety[now.first]);
                if (nexts >= 0) {
                    if (dist[now.first + 1][nexts] > dist[now.first][now.second] + (safety[now.first + 1] - safety[now.first])) {
                        dist[now.first + 1][nexts] = dist[now.first][now.second] + (safety[now.first + 1] - safety[now.first]);
                        a.push(mp(now.first + 1, nexts));
                    }
                }
            }
            if (now.first > 0) {
                int nexts = now.second - (safety[now.first] - safety[now.first - 1]);
                if (nexts >= 0) {
                    if (dist[now.first - 1][nexts] > dist[now.first][now.second] + (safety[now.first] - safety[now.first - 1])) {
                        dist[now.first - 1][nexts] = dist[now.first][now.second] + (safety[now.first] - safety[now.first - 1]);
                        a.push(mp(now.first - 1, nexts));
                    }
                }
            }
