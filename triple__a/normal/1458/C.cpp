...
/*struct mat{
    ...
    friend mat operator*(mat x,mat y){
        vector<vi> ret;
        ret.clear();
        ret.resize(x.n,vi(y.m,0));
        assert(x.m == y.n);
        rep(i,x.n) rep(j,y.m) rep(k,x.m) ret[i][j]+=x.b[i][k]*y.b[k][j];
        return mat(ret);
    }
};
*/
...
signed main(){
    ...
    while (t--){
        ...
        for (auto c:s){
            ...
            if (c=='U') rep(i,4) x[i]--;
            if (c=='D') rep(i,4) x[i]++;
            if (c=='I') rep(i,4) swap(y[i],z[i]);
            if (c=='C') rep(i,4) swap(x[i],z[i]);
        }
        rep1(i,3) x[i]-=x[0],y[i]-=y[0], z[i]-=z[0];
//        rep(i,4) cout<<x[i]<<" "<<y[i]<<" "<<z[i]<<endl;
        rep(i,n) rep(j,n){
            int tx,ty,tz;
            tx=x[0]+i*x[1]+j*x[2]+a[i][j]*x[3];
            ty=y[0]+i*y[1]+j*y[2]+a[i][j]*y[3];
            tz=z[0]+i*z[1]+j*z[2]+a[i][j]*z[3];
            tx%=n, tx+=n, tx%=n;
            ty%=n, ty+=n, ty%=n;
            tz%=n, tz+=n, tz%=n;
//            cout<<i<<" "<<j<<" "<<a[i][j]<<" "<<tx<<" "<<ty<<" "<<tz<<endl;
            b[tx][ty]=tz;
        }
