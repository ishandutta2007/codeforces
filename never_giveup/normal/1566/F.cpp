...
void solve() {
    ...
    sort(all(a));
    {
        ...
        for (int i = 0; i < a.size(); i++) {
            while (it < n && pt[it] < a[i].fr)
                it++;
            if (it < n && pt[it] <= a[i].sc);
    ...
    for (int i = 1; i < a.size(); i++) {
        assert(a[i - 1].fr < a[i].fr);
        assert(a[i - 1].sc < a[i].sc);
    }
    for (int i = 0; i < n; i++)
        a.push_back({pt[i], -llinf});
    a.push_back({(ll)1e10, -llinf});
    a.push_back({(ll)-1e10, -llinf});
    sort(all(a));
    n = a.size();
    ...
    dp[0] = 0;
    int last_p = -1;
    ll bdl = llinf, bdr = llinf;
    for (int it = 0; it < n; it++)
        if (a[it].sc == -llinf) {
            if (last_p != -1) {
                for (int j = last_p + 1; j < it; j++) {
                    assert(a[j].fr > a[last_p].fr);
                    assert(a[j].sc < a[it].fr);
                    if (dp[last_p] != llinf)
                        setmin(dp[j + 1], a[j].fr - a[last_p].fr + dp[last_p]);
                    if (bdl != llinf)
                        setmin(dp[j + 1], bdl + a[j].fr + a[j].fr - a[last_p].fr);
                    if (bdr != llinf)
                        setmin(dp[j + 1], bdr + a[j].fr);
                }
            }
            for (int j = it - 1; j > last_p; j--)
                if (dp[j] != llinf)
                    setmin(dp[it + 1], dp[j] + a[it].fr - a[j].sc);
            for (int j = last_p + 1; j < it; j++)
            if (dp[j] != llinf) {
                setmin(bdl, -a[j].sc + dp[j]);
                setmin(bdr, -a[j].sc + dp[j] + a[it].fr - a[j].sc);
            }
            setmin(dp[it + 1], dp[it]);
            last_p = it;
        }
    cout << dp.back() << '\n';
}
