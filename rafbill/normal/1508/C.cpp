...
struct union_find {
  ...
  union_find(li n = 0) : A(n), S(n) {
    ...
    for(li i = 0; i < (li)(n); ++i) roots.insert(i);
...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) {
    vi mergeWith;
    for(int j : uf.roots) if(j != uf.find(i)) {
        for(int k : uf.S[j]) {
          if(G[i].find(k) == G[i].end()) {
            mergeWith.push_back(j);
            break;
          }
        }
      }
    for(int j : mergeWith) if(uf.unite(i,j)) ne++;
  }
  ...
  sort(begin(E), end(E));
  for(auto e : E) {
    li i,j,w; tie(w,i,j) = e;
    if(uf.unite(i,j)) { uf2.unite(i,j); cost += w; }
  }
