...
inline void work()
{
    rin>>n;int cutt=0;for(LL i=n;i;i>>=2)cutt++;
    LL les=n-(sk[cutt-1]-1);
    // printf("n:%lld cutt:%d les:%lld\n",n,cutt,les);
    LL x=(1LL<<(cutt-1<<1))+(les-1)/3,y=(1LL<<(cutt<<1)-1);if(les%3==1){printf("%lld\n",x);return;}
    for(LL i=(les-1)/3,s=1;(--cutt)>0;i>>=2,s<<=2)
    {
        if((i&3)==1)y+=s<<1;
        if((i&3)==2)y+=s+(s<<1);
        if((i&3)==3)y+=s;
...
int main()
{
    sk[0]=1;for(int i=1;i<30;i++)sk[i]=sk[i-1]<<2;
