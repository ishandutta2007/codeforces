...
ll distancia(ll a1, ll b1, ll a2, ll b2){
    if((a1-a2)*(b1-b2) < 0) return abs(a1-a2) + abs(b1-b2);
    return max(abs(a1-a2),abs(b1-b2));
}

int n;

ll distanciaMaxima(ll x,ll y,vll& a,vll& b){
    ll res = 0;
    F0(i,n) maxi(res,distancia(x,y,a[i],b[i]));
    return res;
}

void minimizar(ll& x, ll& y, vll& a, vll& b, ll salto){
    while(true){
        ll d = distanciaMaxima(x,y,a,b);
        REP(i,-2,2){
            REP(j,-2,2){
                if(x + i*salto >= 0 and y + j*salto >= 0 and distanciaMaxima(x+i*salto,y+j*salto,a,b) < d){
                    x += i*salto;
                    y += j*salto;
                    goto end;
                }
...
void c(){
    ...
    ll salto = 100000;
    while(salto > 0){
        minimizar(x,y,a,b,salto);
        salto /= 2;
    }
    cout<<distanciaMaxima(x,y,a,b)<<'\n';
    F0(i,x) cout<<'B';
    F0(i,y) cout<<'N';
    cout<<'\n';
}
