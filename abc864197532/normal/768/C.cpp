...
const int mod = 1e9 + 7, x = 864197532, N = 5001, logN = 17;
...
int main () {
    ...
    int now = 0;
    while (k--) {
        vector <int> ncnt(1024, 0);
        fop (i,0,1024) {
            if (cnt[i] & 1) {
                if (now) ncnt[i ^ x] += cnt[i] / 2, ncnt[i] += cnt[i] - cnt[i] / 2;
                else ncnt[i ^ x] += cnt[i] - cnt[i] / 2, ncnt[i] += cnt[i] / 2;
                now ^= 1;
            } else {
                ncnt[i] += cnt[i] / 2;
                ncnt[i ^ x] += cnt[i] / 2;
