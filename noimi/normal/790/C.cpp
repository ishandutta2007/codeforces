...
signed main(){
    ...
    int A = n-V-K;
    int mem[75][75][75][2];
    bool visited[75][75][75][2]={};
    auto dfs=[&](auto &f,int v,int k,int a,int pre,int x,string &t)->int{
        if(x==s.size()) return 0;
        if(visited[v][k][a][pre])return mem[v][k][a][pre];
        int res=1000000;
        if(v<V){
            int nxt;
            rep2(i,x,n-1){
                if(t[i]=='V'){nxt = i;break;}
            }
            int cnt = 0;
            rep3(i,nxt,x+1)swap(t[i],t[i-1]),cnt++;
            chmin(res,cnt+f(f,v+1,k,a,1,x+1,t));
            rep2(i,x,nxt-1)swap(t[i],t[i+1]);
        }
        if(k<K and !pre){
            int nxt;
            rep2(i,x,n-1){if(t[i]=='K'){nxt=i;break;}}
            int cnt = 0;
            rep3(i,nxt,x+1)swap(t[i],t[i-1]),cnt++;
            chmin(res,cnt+f(f,v,k+1,a,0,x+1,t));
            rep2(i,x,nxt-1)swap(t[i],t[i+1]);
        }
        if(a<A){
            int nxt;
            rep2(i,x,n-1){if(t[i]!='V' and t[i]!='K'){nxt = i;break;}}
            int cnt = 0;
            rep3(i,nxt,x+1)swap(t[i],t[i-1]),cnt++;
            chmin(res,cnt+f(f,v,k,a+1,0,x+1,t));
            rep2(i,x,nxt-1)swap(t[i],t[i+1]);
        }
        visited[v][k][a][pre]=1;
        return mem[v][k][a][pre] = res;
    };
    cout << dfs(dfs,0,0,0,0,0,s) << endl;
