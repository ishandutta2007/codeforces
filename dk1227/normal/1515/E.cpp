...
int main() {
	...
	int n, p;
	qread(n, p);
	rep(i, 1, n) Pow[i] = Pow[i - 1] * 2 % p;
	rep(i, 0, n) C[i][0] = C[i][i] = 1;
	rep(i, 1, n) rep(j, 1, i - 1) C[i][j] = (C[i - 1][j] + C[i - 1][j - 1]) % p;
	dp[1][1] = 1, dp[2][1] = 2;
	rep(i, 3, n) rep(j, 1, (i + 1) / 2) {
		if(j != 1) rep(k, 1, i - 2) (dp[i][j] += dp[k][j - 1] * C[i - j + 1][i - k - 1] % p * Pow[i - k - 2] % p) %= p;
		else dp[i][j] = Pow[i - 1];	
		if(i == n) (res += dp[i][j]) %= p;
	}
	cout << res << endl;
