...
public class Main
{
	...
	{
		int n=ni();
		NavigableMap<Long,Integer> start=new TreeMap<Long,Integer>();
		NavigableMap<Long,Integer> end=new TreeMap<Long,Integer>();
		long[]anses=new long[n+1];
		for(int i=0; i<n; i++)
		{
			long te=nl(),te1=nl();
			start.put(te, start.getOrDefault(te, 0)+1);
			end.put(te1, end.getOrDefault(te1, 0)+1);
		}
		int open=0;
		long lastval=0;
		while(!end.isEmpty())
		{
			...
			if(!start.isEmpty())
			 mins=start.firstKey();
			long mine=end.firstKey();
			if(mins<=mine)
			{
				anses[open]+=mins-lastval;
				lastval=mins;
				open+=start.pollFirstEntry().getValue();
			}
			else
			{
				anses[open]+=mine-lastval+1;
				lastval=mine+1;
				open-=end.pollFirstEntry().getValue();
			}
		}
		for(int i=1; i<=n; i++)
			p(anses[i]+" ");
