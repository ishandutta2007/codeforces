...
int main(){
	...
	int m = int(a.size());
	vector<vector<Zp>> dp(k + 1, vector<Zp>(m)); // dp[i][j]: cnt of len i, ending at j
	vector<vector<Zp>> sum(k + 1, vector<Zp>(m + 1)); // sum[i][j]: sum of dp[i][0] + ... + dp[i][j - 1]
	sum[0] = vector<Zp>(m + 1, 1);
	for(int i = 0; i < m; ++ i){
		dp[1][i] = a[i].second;
		sum[1][i + 1] = sum[1][i] + a[i].second;
	}
	for(int i = 2; i <= k; ++ i){
		for(int j = 0; j < m; ++ j){
			dp[i][j] = sum[i - 1][j + 1] * a[j].second;
			sum[i][j + 1] = sum[i][j] + dp[i][j];
		}
	}
	Zp res = 0;
	for(int i = 1; i <= min<long long>((l + n - 1) / n, k); ++ i){
		if(i <= l / n){
			res += sum[i][m] * (l / n - i + 1);
		}
		for(auto x: endval){
			int p = lower_bound(a.begin(), a.end(), pair<int, int>{x, numeric_limits<int>::max()}) - a.begin();
			res += sum[i - 1][p];
		}
	}
	cout << res;
