...
bool clockwise (pair<ll, ll> x, pair<ll, ll> y) {
...
int main() {
    ...
    {
        vector<pair<long double, pair<ll, ll>>> a;
        for (int j = 0; j<n; j++) if (j!=pt) a.push_back(mp(atan2(x[j]-x[pt], y[j]-y[pt]), mp(x[j]-x[pt], y[j]-y[pt])));
        ...
        for (int i = 0; i<n-1; i++)
        {
            while (clockwise(a[i].second, a[(cur+1)%(n-1)].second)) cur = (cur+1)%(n-1);
            ll ans = (cur+(n-1) - i)%(n-1);
            total-=ans*(ans-1)*(ans-2)/6;
            if (cur==i) cur = (cur+1)%(n-1);
        }
    }
