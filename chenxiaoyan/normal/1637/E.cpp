...
void disc() {
	...
	REP(i, 1, n) a[i] = lower_bound(ALL(nums), a[i]) - nums.begin() + 1;
}
vi nei[N];
int cnt[N];
vi buc[N];
...
void mian() {
	n = read(), m = read();
	REP(i, 1, n) nei[i].clear(), cnt[i] = 0, buc[i].clear();
	REP(i, 1, n) a[i] = read();
	disc();
	while(m--) {
		int x = read(), y = read();
		x = lower_bound(ALL(nums), x) - nums.begin() + 1;
		y = lower_bound(ALL(nums), y) - nums.begin() + 1;
		nei[x].pb(y), nei[y].pb(x);
	}
	REP(i, 1, n) ++cnt[a[i]];
	vi small, large;
	int B = sqrt(n);
	REP(i, 1, SZ(nums)) if(cnt[i] > B) large.pb(i); else small.pb(i);
	int ans = 0;
	static bool isbad[N];
	for(int x : large) {
		for(int y : nei[x]) isbad[y] = true;
		REP(i, 1, SZ(nums)) if(i != x && !isbad[i]) chkmx(ans, (nums[i - 1] + nums[x - 1]) * (cnt[i] + cnt[x]));
		for(int y : nei[x]) isbad[y] = false;
	}
	for(int x : small) buc[cnt[x]].pb(x);
	REP(i, 1, B) sort(ALL(buc[i]), greater<int>());
	for(int x : small) {
		for(int y : nei[x]) isbad[y] = true; isbad[x] = true;
		REP(i, 1, B) {
			int now = 0;
			while(now < SZ(buc[i]) && isbad[buc[i][now]]) ++now;
			if(now < SZ(buc[i])) chkmx(ans, (nums[x - 1] + nums[buc[i][now] - 1]) * (cnt[x] + i));
		}
		for(int y : nei[x]) isbad[y] = false; isbad[x] = false;
	}
	prt(ans), pc('\n');
}
