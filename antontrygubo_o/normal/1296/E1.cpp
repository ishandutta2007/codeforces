...
int main()
{
    ...
    {
        if (b.empty()) swap(a, b);
        if (!a.empty() && !b.empty() && s[a.back()]>s[b.back()]) swap(a, b);

        if (b.empty() || (!b.empty() && s[b.back()] <= s[i])) b.push_back(i);
        else if (a.empty() || (!a.empty() && s[a.back()] <= s[i])) a.push_back(i);
        else
        {
            //print(a); print(b);
    ...
    vector<char> ans(n);
    for (auto it: a) ans[it] = '0';
    for (auto it: b) ans[it] = '1';
