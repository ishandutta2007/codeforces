...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll n;
        cin>>n;
        tuple<ll,ll,ll,ll,ll> a[n];
        REP(i,0,n)
        {
            ll t1,t2,t3;
            cin>>t1>>t2>>t3;
            a[i]=MT(t1*t1+t2*t2+t3*t3,t1,t2,t3,i);
        }
        ll f[n]={};
        sort(a,a+n);
        V(pll) ans;
        REP(i,0,n)
        {
            ...
            else
            {
                ...
                REP(j,x+1,n)
                {
                    if(min(G(1,a[i]),G(1,a[x]))<=G(1,a[j])&&max(G(1,a[i]),G(1,a[x]))>=G(1,a[j]))
                    {
                        if(min(G(2,a[i]),G(2,a[x]))<=G(2,a[j])&&max(G(2,a[i]),G(2,a[x]))>=G(2,a[j]))
                        {
                            if(min(G(3,a[i]),G(3,a[x]))<=G(3,a[j])&&max(G(3,a[i]),G(3,a[x]))>=G(3,a[j]))
                            {
                                if(f[j]==0) x=j;
                            }
                        }
                ...
                f[i]=1;
                f[x]=1;
                ans.PB(MP(G(4,a[i]),G(4,a[x])));
            }
        }
        REP(i,0,ans.size()) cout<<ans[i].F+1<<' '<<ans[i].S+1<<'\n';
