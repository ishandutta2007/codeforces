...
void solve() {
	...
	scanf("%d", &n);
	vll with(bits);
	vector <vll> both(bits, vll(bits));
	REP(_, n) {
		ll x;
		scanf("%lld", &x);
		debug(imask(x));
		vi here;
		forbits(i, x) here.push_back(i);
		for (int i : here) with[i]++;
		for (int i : here) for (int j : here)
			both[i][j]++;
	}
	REP(i, bits) both[i][i] = with[i];
	ll ans = 0;
	REP(b1, bits) REP(b2, bits) {
		ll coef = 0;
		(coef += with[b1] * 1ll * both[b1][b2] % mod * n) %= mod;
		(coef += with[b1] * 1ll * with[b1] % mod * with[b2]) %= mod;
		(coef -= with[b1] * 1ll * both[b1][b2] % mod * with[b2]) %= mod;
		// if (coef) debug(imie(b1), imie(b2), imie(coef));
		ll val = p(b1) * p(b2) % mod;
		(ans += val * coef) %= mod;
	}
	printf("%lld\n", fix(ans, mod));
}
