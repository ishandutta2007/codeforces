// BA-String (https://codeforces.com/contest/1620/problem/C)
...
int main() {
    int t; read(t);
    while (t--) {
        int n, k; ll x; string s; read(n, k, x, s);
        --x;
        vector<pair<int, ll>> compr;
        ...
        while (i >= 0) {
            ...
            while (i >= 0 && s[i] == '*')
                ++s_cnt, --i;
            compr.emplace_back(a_cnt, poss);
            poss = ll(min(__int128_t(INFL), __int128_t(s_cnt * k + 1) * poss));
        }

        reverse(all(compr));
        string ans;
        for (auto [a_cnt, poss_after] : compr) {
            while (x >= poss_after)
                ans.push_back('b'), x -= poss_after;
            rep(_, a_cnt)
                ans.push_back('a');
        }
