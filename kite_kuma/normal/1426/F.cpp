...
int main() {
	...
	rep(i, n) {
		if(v[i] == 2) {
			ll lef = as[i] - as[0];
			ll rig = cs[n] - cs[i + 1];
			if(lef && rig) ans += lef * rig * modpow(3, hatena[n], mod);

			lef = hatena[i] - hatena[0];
			rig = hatena[n] - hatena[i + 1];
			if(lef && rig) ans += lef * rig * modpow(3, hatena[n] - 2, mod);

			lef = as[i] - as[0];
			rig = hatena[n] - hatena[i + 1];
			if(lef && rig) ans += lef * rig * modpow(3, hatena[n] - 1, mod);
		...
		if(v[i] == 0) {
			...
			lef = hatena[i] - hatena[0];
			rig = hatena[n] - hatena[i + 1];
			if(lef && rig) ans += lef * rig * modpow(3, hatena[n] - 1 - 2, mod);

			lef = as[i] - as[0];
			rig = hatena[n] - hatena[i + 1];
			if(lef && rig) ans += lef * rig * modpow(3, hatena[n] - 1 - 1, mod);
