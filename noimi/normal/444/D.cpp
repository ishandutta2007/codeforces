...
int main() {
    ...
    map<string, vi> mp;
    rep(i, si(s)) {
        for(int j = 1; j <= 4 and i + j <= si(s); j++) mp[s.substr(i, j)].eb(i);
    }
    ...
    while(Q--) {
        ...
        else if(!mp.count(x) or !mp.count(y))
            cout << -1 << endl;
        else {
            const auto &v = mp[x];
            const auto &w = mp[y];
            int &mi = ans[make_pair(x, y)];
            mi = inf<int>;
            int i = 0, j = 0;
            for(; i < si(v); i++) {
                while(j < si(w) and w[j] < v[i]) j++;
                if(j) chmin(mi, max(v[i] + si(x), w[j - 1] + si(y)) - w[j - 1]);
                if(j < si(w)) chmin(mi, max(v[i] + si(x), w[j] + si(y)) - v[i]);
            }
            cout << mi << endl;
        }
    }
