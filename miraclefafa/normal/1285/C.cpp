...
int main() {
	Factor::init(100000);
	scanf("%lld",&x);
	vector<PLL> z=Factor::factorG(x);
	pair<ll,ll> ans(x,x);
	function<void(int,ll,ll)> dfs=[&](int dep,ll a,ll b) {
		//printf("%d %lld %lld\n",dep,a,b);
		if (dep==SZ(z)) {
			if (max(a,b)<max(ans.fi,ans.se)) ans=mp(a,b);
		} else {
			ll w=1;
			rep(i,0,z[dep].se) w=w*z[dep].fi;
			rep(i,0,z[dep].se+1) {
				dfs(dep+1,a,b*(i==z[dep].se?1:w));
				a=a*z[dep].fi;
			}
		}
	};
