...
void solve() {
	...
	int killo = -1; REP(i, 3) if (killo == -1 || ocnt[i] < ocnt[killo]) killo = i;
	if (killx == killo) {
		int cheap = killx;
		killx = killo = -1; int best = INT_MAX;
		REP(i, 3) if (i != cheap && xcnt[cheap] + ocnt[i] < best) killx = cheap, killo = i, best = xcnt[cheap] + ocnt[i];
		REP(i, 3) if (i != cheap && xcnt[i] + ocnt[cheap] < best) killx = i, killo = cheap, best = xcnt[i] + ocnt[cheap];
	}
