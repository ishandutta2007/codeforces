...
mint solve() {
    ...
    mint ret = 0;
    int n1 = N / 2, n2 = N - n1;
    int th = max(1, min(abs(L) - N, R - N * 2));
    mint d = th - 1;
    ret += d * mint(N).nCr(n1) * (n1 == n2 ? 1 : 2);

    vector<pint> pm{{n1, n2}, {n2, n1}};
    pm = sort_unique(pm);

    FOR(x, th, min(abs(L), abs(R)) + N * 2) {
        int thresr = min(N, max(0, R - x)), thresl = min(N + 1, max(1, L + x));
        const int minus_fixed = N - thresr, plus_fixed = thresl - 1;
        for (auto [m, p] : pm) {
            int rm = m - minus_fixed, pm = p - plus_fixed;
            if (rm >= 0 and pm >= 0) ret += mint(rm + pm).nCr(rm);
        }
    }
    return ret;
}
