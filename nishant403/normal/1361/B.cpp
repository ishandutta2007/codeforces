...
bool check(int id,int need)
{
    ...
    else if(check(id-1,(need-mp[id])*p)) 
    {
          mp[id] = 0;
          return 1;
    }
...
void solve()
{
   ...
   f(i,n)
   {
       ...
       int ad = mod_pow(p,a[i]);
       int ad1 = ((cnt/2)*ad)%MOD;
       int ad2 = ((cnt-(cnt/2))*ad)%MOD;
       ...
       if(cnt % 2 == 1)
       {
           if(!check(a[i]-1,p)) ff = 0;
           else resA = (resA + ad)%MOD; 
       }
           
       }
       else resA = (resA + ad1 + ad2)%MOD;
   }
    
   int ans = (resB - resA + MOD)%MOD;
