...
int main(){
    ...
    Modint ans=0;
    dp[0][n][0]=2_M .pow(count(all(s),'?'));
    rep(l,n)rrep(r,l+1,n+1){
        if(s[l]=='(')dp[l+1][r][1]+=dp[l][r][0];
        elif(s[l]==')')dp[l+1][r][0]+=dp[l][r][0];
        else{
            const Modint x=dp[l][r][0]*Modint(2).inv();
            dp[l+1][r][0]+=x;
            dp[l+1][r][1]+=x;
        }
        if(s[r-1]=='(')dp[l][r-1][1]+=dp[l][r][1];
        elif(s[r-1]==')'){
            dp[l][r-1][0]+=dp[l][r][1];
            ans+=dp[l][r][1];
        }
        else{
            const Modint x=dp[l][r][1]*Modint(2).inv();
            dp[l][r-1][0]+=x;
            dp[l][r-1][1]+=x;
            ans+=x;
        }
