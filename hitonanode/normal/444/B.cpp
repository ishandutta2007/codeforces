...
vector<int> a, b;
void initAB(int n, int d) {
    int i = 0;
    for (i = 0; i < n; i = i + 1) { a[i] = i + 1; }
    for (i = 0; i < n; i = i + 1) { swap(a[i], a[getNextX().val % (i + 1)]); }
    for (i = 0; i < n; i = i + 1) {
        if (i < d)
            b[i] = 1;
        else
    ...
    for (i = 0; i < n; i = i + 1) { swap(b[i], b[getNextX().val % (i + 1)]); }
}
...
int main()
{
    ...
    vector<int> ainv(N);
    REP(i, N) ainv[a[i] - 1] = i;

    vector<int> q;
    REP(i, N) if (b[i]) q.emplace_back(i);

    IREP(a, N) {
        int i = ainv[a];
        for (auto j : q) {
            if (i + j >= N) break;
            if (!ret[i + j]) ret[i + j] = a + 1, nrem--;
        }
        if (nrem * q.size() <= 1e7) break;
    }
    REP(i, N) if (!ret[i]) {
        for (auto j : q) {
            if (j > i) break;
