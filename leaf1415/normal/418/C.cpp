...
void make(ll n, vector<ll> &hvec)
{
	vector<ll> tmp;
	ll N = 1<<5;
	if(n%3 == 1) tmp.pb(1*N);
	if(n%3 == 2) tmp.pb(3*N), tmp.pb(4*N);
	if(n%3 == 0) tmp.pb(2*N), tmp.pb(3*N), tmp.pb(6*N);
	
	while(sz(tmp) + sz(hvec) < n){
		while(tmp.back() % 2) hvec.pb(tmp.back()), tmp.pop_back();
		ll x = tmp.back(); tmp.pop_back();
		rep(i, 1, 4) tmp.pb(x/2);
	}
	hvec.insert(hvec.end(), all(tmp));
}
