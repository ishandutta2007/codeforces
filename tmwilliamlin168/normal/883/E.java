...
public class E implements Runnable{
    ...
    private void solve() {
        int n = readInt();
        char[] mistery = readCharArray();

        int m = readInt();
        char[][] words = readCharField(m);

        int possibleWords = 0;
        int[] counts = new int[26];

        boolean[] alreadyUsed = new boolean[26];
        for (char letter : mistery) {
            if (letter != '*') alreadyUsed[letter - 'a'] = true;
        }

        for (char[] word : words) {
            boolean correct = true;
            for (int j = 0; j < word.length; ++j) {
                correct &= (!alreadyUsed[word[j] - 'a'] == (mistery[j] == '*'));
                correct &= (mistery[j] == '*' || mistery[j] == word[j]);
            }
        ...
        int answer = 0;
        for (int letter = 0; letter < 26; ++letter) {
            if (!alreadyUsed[letter] && counts[letter] == possibleWords && counts[letter] > 0) {
                ++answer;
            }
        }
