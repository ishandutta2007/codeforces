...
int main()
{
	...
	for(int i=0;i<n;i++)
		scanf("%i",&a[i].f);
	for(int i=0;i<n;i++)
		scanf("%i",&a[i].s),vVals.pb(a[i].s);
	sort(all(vVals));
	vVals.erase(unique(all(vVals)),vVals.end());  //Compress v
	map<int,int> mapa;
	for(int i=0;i<(int)vVals.size();i++)
		mapa[vVals[i]]=i;
	for(auto &p:a)
		p.s=mapa[p.s];
	
	int v=vVals.size(); //Number of v values
	
	ll ans=0;
	cordSum.init(v);
	cnt.init(v);
	sort(a.begin(),a.end());  //Sweep in ascending order of points
	for(auto p:a){
		int broj=cnt.getSum(0,p.s);
		ll suma=cordSum.getSum(0,p.s);
		ans+=(ll)broj*p.f-suma;
		cnt.singleAdd(p.s,1);
		cordSum.singleAdd(p.s,p.f);
	}
	printf("%lld\n",ans);
	return 0;
}