...
LL val[MAXN]; int root;
inline LL calc(int x) {return 1ll * x * (x - 1);}
void dfs2(int x, int pre) {
    bool ck = 0; if(pre) val[x] = val[pre] + calc(sz[pre] - sz[x]);
    else val[x] = 0;
    for (int i = head[x];i;i = nxt[i]) {
        if(e[i] == pre) continue;
        dfs2(e[i], x); ck = 1;
    } if(!ck && (val[x] < val[root])) root = x;
}

signed main(){
    read(n); REP(i, 1, n) link(read(), read());
    val[n + 1] = INFll; root = n + 1; dfs1(1, 0); dfs2(1, 0);
    const int rt = root; root = n + 1; dfs1(rt, 0); dfs2(rt, 0);
    print(calc(n) - (val[root] >> 1));
}