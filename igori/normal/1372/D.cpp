...
void solve()
{
    ...
    forn(i, n) cin >> a[i];
    ll ans = 0;
    vll peven(n), podd(n), seven(n), sodd(n);
    peven[0] = a[0];
    for (int i = 1; i < n; i++)
    {
        peven[i] = peven[i - 1] + (i % 2 == 0 ? a[i] : 0);
        podd[i] = podd[i - 1] + (i % 2 == 1 ? a[i] : 0);
    }
    seven[n - 1] = a[n - 1];
    for (int i = n - 2; i >= 0; i--)
    {
        seven[i] = seven[i + 1] + (i % 2 == 0 ? a[i] : 0);
        sodd[i] = sodd[i + 1] + (i % 2 == 1 ? a[i] : 0);
    }
    ans = peven[n - 1];
    for (int i = 0; i + 1 < n; i++)
    {
        if (i % 2 == 0) ans = max(ans, peven[i] + sodd[i + 1]);
        if (i % 2 == 1) ans = max(ans, podd[i] + seven[i + 1]);
    }
    cout << ans << "\n";
}
