...
void reset() {
	...
	memset(l_matched, 0, sizeof(l_matched));
}
bool dfs(int x) {
...
int run_matching() {
	...
	while (any) {
		...
		for (int i = 1; i <= n; ++i) if (!l_matched[i] && !visited[i] && dfs(i)) {
			l_matched[i] = true;
			any = true;
			size++;
		}
...
void solve() {
	...
	while (low <= high) {
		...
		int siz = run_matching();
		debug(range(edges, 0, med), imie(siz));
		if (siz == n) {
			ans = med;
			high = med - 1;
		}
	...
	if (ans != -1) debug(range(edges, 0, ans));
	if (ans != -1) ans = get<0>(edges[ans - 1]);
	printf("%d\n", ans);
}
int main() {
