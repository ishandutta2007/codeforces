...
int main(void)
{
	...
	while(T--){
		cin >> n;
		cin >> d[0][1] >> d[1][2] >> d[2][0];
		d[1][0] = d[0][1], d[2][1] = d[1][2], d[0][2] = d[2][0];
		
		bool flag = true;
		rep(i, 0, 2) if(d[i][(i+1)%3]+d[(i+1)%3][(i+2)%3] < d[(i+2)%3][i]) flag = false;
		if((d[0][1] + d[1][2] + d[2][0]) % 2) flag = false;
		if(!flag){
			no();
			continue;
		}
		...
		ll mx = -inf, mx_i;
		rep(i, 0, 2) if(mx < d[i][(i+1)%3]) mx = d[i][(i+1)%3], mx_i = i;
		ll a = mx_i, b = (mx_i+1)%3, c = (mx_i+2)%3;
		
		vector<P> ans;
		
		vector<ll> vec;
		vec.pb(a);
		rep(i, 1, d[a][b]-1) vec.pb(2+i);
		vec.pb(b);
		
		ll p;
		reps(i, vec){
			if(sz(vec)-1-2*i == d[b][c] - d[a][c]){
				p = i;
				break;
			}
		}
		if(d[a][c]+d[b][c] == d[a][b]){
			vec[p] = c;
			rep(i, p+1, sz(vec)-2) vec[i]--;
		}
		else{
			vector<ll> vec2; vec2.pb(vec[p]);
			ll x = (d[a][c]+d[b][c] - d[a][b])/2;
			rep(i, 1, x-1) vec2.pb(2+d[a][b]-1+i);
			vec2.pb(c);
			rep(i, 1, sz(vec2)-1) ans.pb(P(vec2[i-1], vec2[i]));
		}
		rep(i, 1, sz(vec)-1) ans.pb(P(vec[i-1], vec[i]));
		...
		vector<ll> vec3;
		vec3.pb(0);
		rep(i, 0, n-1) if(!used[i]) vec3.pb(i);
		rep(i, 1, sz(vec3)-1) ans.pb(P(vec3[i-1], vec3[i]));
