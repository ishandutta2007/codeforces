...
int main() {
  ...
  if (int badv = accumulate(qdg + 1, qdg + 1 + n, 0)) {
    ...
    for (const auto& c: range{queries, q}) {
      pref = {c[0]}, suf = {c[1]};
      for (int i = d[c[0]] - d[c[1]], pb = pref.back(); i > 0; --i) {
        pref.push_back(pb = p[pb]);
      }
      for (int i = d[c[1]] - d[c[0]], sb = suf.back(); i > 0; --i) {
        suf.push_back(sb = p[sb]);
      }
      for (int pb = pref.back(), sb = suf.back(); pb != sb; ) {
        pref.push_back(pb = p[pb]);
        suf.push_back(sb = p[sb]);
      }
