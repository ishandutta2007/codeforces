...
pii pp[2111];
pair<double,int> p[4111];
int n,t1[2111],t2[2111],pre[4111];
...
int main()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		{
			p[++nn]=mp(atan2(pp[j].ss-pp[i].ss,pp[j].ff-pp[i].ff),j);
			p[++nn]=mp(atan2(pp[i].ss-pp[j].ss,pp[i].ff-pp[j].ff),-j);
		}
		sort(p+1,p+nn+1);
