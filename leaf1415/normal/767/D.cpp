...
int main(void)
{
	...
	cin >> n >> m >> k;
	ll N = 2000001;
	rep(i, 1, n) cin >> a[i], a[i]++, chmin(a[i], N), cnt[a[i]]++;
	rep(i, 1, m) cin >> b[i].first, b[i].first++, chmin(b[i].first, N), b[i].second = i;
	sort(a+1, a+n+1), sort(b+1, b+m+1);
	reverse(b+1, b+m+1);
	
	seg.init(); ll sum = 0;
	rep(i, 1, N) sum += cnt[i], seg.update(i, -k*i + sum);
	if(seg.query(1, N) > 0){
		outl(-1);
		return 0;
	}
	...
	rep(i, 1, m){
		seg.add(b[i].first, N, 1);
		if(seg.query(1, N) > 0){
			outl(i-1);
			rep(j, 1, i-1) cout << b[j].second << " "; cout << endl;
			return 0;
		}
	}
	outl(m);
	rep(j, 1, m) cout << b[j].second << " "; cout << endl;
