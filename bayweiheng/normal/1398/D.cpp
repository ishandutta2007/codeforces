...
ll f(int i,int j,int k) {
	...
	if(i==sz[0] && j==sz[1] && k==sz[2]) return dp[i][j][k]=0;
	ll ans=0;
	if(i!=sz[0]) ans = max(ans, f(i+1,j,k));
	if(j!=sz[1]) ans = max(ans, f(i,j+1,k));
	if(k!=sz[2]) ans = max(ans, f(i,j,k+1));
	if(i!=sz[0] && j!=sz[1]) ans = max(ans, v[0][i] * v[1][j] + f(i+1,j+1,k));
	if(i!=sz[0] && k!=sz[2]) ans = max(ans, v[0][i] * v[2][k] + f(i+1,j,k+1));
	if(j!=sz[1] && k!=sz[2]) ans = max(ans, v[1][j] * v[2][k] + f(i,j+1,k+1));
	return dp[i][j][k]=ans;
}
