...
int another_main()
{
		...
		for (int i = 0; i < N; ++i)
			for (int j = 0; j < 4; ++j)
				for (int l = 0; l < 4; ++l)
				{
					...
					int base1 = (((j & 1) | (l & 1)) != 0);
					int base2 = (((j & 2) | (l & 2)) != 0);
					ll base3 = dp[2 * i][j] + dp[2 * i + 1][l];
					ll base = base1 + base2 + base3;
					int p = -3;
					if (base2 || base11)
						p = 2 | base1;
					else if (base2 == 0 && base1 == 0)
						p = 0;
					if (p != -3)
					{
						relax(dq[i][p], base + (p != 0));
						if (dq[i][p] == 5)
						{
	...
	ll base2 = dp[0][2] + 1;
	ll base3 = dp[0][3] + 1;
	cout << max(max(base0, base1), max(base2, base3)) << '\n';
	return 0;
}
