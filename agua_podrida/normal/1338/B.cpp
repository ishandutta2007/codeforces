...
bool hayCaminoImpar(vvi& grafo, vi& padre, vb& caminosPares, int nodo){
    bool hayPar = false;
    bool hayImpar = false;
    F0(i,grafo[nodo].size()){
        int hijo = grafo[nodo][i];
        if(padre[nodo] == hijo) continue;
        if(hayCaminoImpar(grafo,padre,caminosPares,hijo)) return true;
        hayPar |= caminosPares[hijo];
        hayImpar |= !caminosPares[hijo];
        if(caminosPares[hijo]) caminosPares[nodo] = false;
    }
    return hayImpar and hayPar;
}
...
void b(){
    ...
    vi padre(n+1);
    int raiz = 1;
    while(grafo[raiz].size() == 1) raiz++;
    padre[raiz] = -1;
    dfs(grafo,padre,raiz);

    int fMin = 1;
    vb caminosPares(n+1,true);
    if(hayCaminoImpar(grafo,padre,caminosPares,raiz)) fMin = 3;
    int fMax = n-1;
    resolverMax(grafo,padre,fMax,raiz);
    mini(fMin,3);
    cout<<fMin<<' '<<fMax<<'\n';
}
