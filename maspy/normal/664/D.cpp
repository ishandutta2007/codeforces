...
void solve() {
  ...
  auto calc = [&](ll t) -> void {
    ...
    for (auto&& v: hld.V) {
      for (auto&& e: G[v]) {
        if (dep[e.to] <= dep[v]) { push[v] = push[e.to] ^ e.cost ^ t; }
      }
    }
    for (auto&& e: G.edges) {
      if (e.cost ^ push[e.frm] ^ push[e.to] != t) return;
    }
    vi res(N);
    vi rev(N);
    FOR(v, N) res[hld.root[v]] += push[v];
    FOR(v, N) if (hld.root[v] == v) {
      ll n = hld.subtree_size(v);
      rev[v] = res[v] > n - res[v];
    }
    FOR(v, N) if (rev[hld.root[v]]) push[v] ^= 1;
    ll cost = SUM(push);
    if (chmin(ANS, cost)) {
      ans.clear();
      FOR(v, N) if (push[v]) ans.eb(v + 1);
    }
  };
