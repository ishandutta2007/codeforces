...
using Z=ModInt<kcz>;
const int N=2e6+10;
int a[N],id[N],fa[N],cnt;
set<Pair> S;
inline void solve(){
	int n=read();
	cnt=n;
	for(int i=1;i<=n;i++)a[i]=read(),id[i]=i,fa[i]=i,S.insert({a[i],i});
	auto find=[&](int x){
	...
	while(q--){
		...
		if(opt==1){
			...
			int l=read(),r=read();
			auto L=S.lower_bound({l,0}),R=L;
			int f=++cnt,g=++cnt;
			fa[f]=f,a[f]=l-1,fa[g]=g,a[g]=r+1,id[f]=f,id[g]=g;
			for(;R!=S.end()&&(*R).fi<=r;R++){
				auto [x,y]=*R;
				if(x<=(l+r-1)/2){
					fa[find(y)]=f;
				}
