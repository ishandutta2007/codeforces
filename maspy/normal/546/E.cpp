...
void solve() {
  ...
  FOR(v, N) G.add(v, v + N, INF);
  FOR(v, N) G.add(source, v, A[v]);
  FOR(v, N) G.add(v + N, sink, B[v]);

  ll F = G.flow(source, sink);
  if (F != SUM(A)) return NO();
  ...
  FORIN(e, E) {
    ll a = e.frm, b = e.to - N;
    if (0 <= a && a < N && 0 <= b && b < N) { ANS[a][b] += e.flow; }
  }

  YES();
  FORIN(row, ANS) print(row);
}
