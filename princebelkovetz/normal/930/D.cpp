...
int solve(vector <vector <int>> p) {
    ...
    for (int i = 0; i < mx; ++i) {
        if (i & 1) {
            ...
            while (prefId < p.size() and p[prefId][0] == i) {
                prefUp[i + 1] = max(prefUp[i + 1], p[prefId][1] - 1);
                prefDn[i + 1] = min(prefDn[i + 1], p[prefId][1] + 1);
                prefId++;
            }
        }
    }
    ...
    for (int i = mx - 1; i >= 1; --i) {
        if (i & 1) {
            ...
            while (sufId >= 0 and p[sufId][0] == i) {
                sufUp[i - 1] = max(sufUp[i - 1], p[sufId][1] - 1);
                sufDn[i - 1] = min(sufDn[i - 1], p[sufId][1] + 1);
                sufId--;
            }
        }
    }
    ...
    for (int i = 1; i < mx; i += 2) {
        ans += max(0ll, (min(sufUp[i], prefUp[i]) - max(sufDn[i], prefDn[i]) + 2) / 2);
    }
    return ans;
}
