#line 1 "codeforces-workspace\\contest\\848\\b\\b.cpp"
#include <bits/extc++.h>
...
#line 13 "codeforces-workspace\\contest\\848\\b\\b.cpp"
...
struct workspace::solver {
  ...
  solver() {
    ...
    sort(begin(onx), end(onx), [](auto x, auto y) {
      if (x.pos != y.pos) return x.pos < y.pos;
      return x.time < y.time;
    });
    sort(begin(ony), end(ony), [](auto x, auto y) {
      if (x.pos != y.pos) return x.pos < y.pos;
      return x.time < y.time;
    });
    calc(1);
  ...
  void calc(bool vert) {
    set<danc> dancs;
    for (auto d : onx) {
      dancs.insert({d.time - d.pos, d.id, d.pos});
    }

    for (auto [t, i, p] : ony) {
      auto lb = dancs.lower_bound({t - p, i, -inf});
      auto ub = dancs.lower_bound({t - p, i, inf});
      if (ub != begin(dancs)) {
        ub--;
        if (ub->time == t - p) {  // collision
          auto add = *lb;
          add.id = i;
          add.pos--;
          if (vert)
  ...
  // for (auto d : dancs) {
  //   ans[d.id] = !dire ? p32{d.pos, h} : p32{w, d.pos};
  // }
}
