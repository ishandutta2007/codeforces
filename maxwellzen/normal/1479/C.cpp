...
void solve() {
    cin >> L >> R; R++;
    len = R-L;
    l.pb(0); r.pb(1); adj.rsz(1); sz = 1;
    while (2*(r.back()-l.back()) <= len) {
        l.pb(1); r.pb(1); adj.pb(vpi());
        F0R(i, sz) {
            adj[sz].pb(mp(i, r[sz]-l[i]));
            r[sz] += r[i]-l[i];
        }
        sz++;
    }
    l.pb(L); r.pb(L); adj.pb(vpi());
    if (len%2==0) {
        adj[sz].pb(mp(0, L));
        adj[sz].pb(mp(1, L));
        r[sz] += 2;
    ...
    sz++;
    F0R(i, sz) m += adj[i].size();
    cout << "YES\n" << sz << ' ' << m << nl;
    F0R(i, sz) for (pi p : adj[i]) cout << sz-i << ' ' << sz-p.F << ' ' << p.S << nl;
}
