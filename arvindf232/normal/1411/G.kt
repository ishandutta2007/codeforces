...
fun debug(){}
fun Graph.toplogicalSort(): IntArray {
    val ans = fastEmptyList(n)
    this.trueOrderDFS(null, {},{},{
        ans.addLast(it)
    })
    return ans.st.reversedArray()
    //ancestors first
}
...
fun main(){
    solve.cases{
        val n = getint
        val m = getint
        val G = Graph(n,m,true)
        repeat(m){
            G.add(getint - 1 , getint - 1 )
        }
        val tsort = G.toplogicalSort()
        val xgrundy = IntArray(n)
        for(v in tsort.reversed()){
            val have = mint
            G.NS(v){
                have.add(xgrundy[it])
            }
            xgrundy[v] = mexOf(have.toIntArray())
        }
        val maxn = xgrundy.maxOrNull()!!
        val p2 = TreeSet((0..20).map { 1 shl it })

        val next = p2.higher(maxn)
        val counts = IntArray(next)
        for(c in xgrundy){
            counts[c] ++
        }

        val invn1 = (n+1).inverse()
        val construct = matrixP(next+1, next+2)
        construct[next][next+1] = 1
        construct[next][next] = 1
        for(i in 0 until next){
            if(i == 0){
                construct[i][next] = invn1
            }

            for(j in 0 until next){
                construct[i][i xor j] = counts[j] modM invn1
            }
        }
        for(i in 0 until next){
            construct[i][i] = construct[i][i] modMinus 1
        }
//        construct.print()
        val ans = construct.elimination()!!
//        just dei ans
        put(1 modMinus ans[0])
...
find the number of terminations
