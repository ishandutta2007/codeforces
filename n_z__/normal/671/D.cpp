...
function<void(int,int)>dfs=[&](int x,int f){
	...
	if(q!=f)
	for(auto r:s[q])
	w[r]-=sb[q]-sb[x],s[x].insert(r);
	for(auto q:ww[x])
	if(q<0)s[x].erase(-q);
	else w[q]+=sb[x],s[x].insert(q);
	if(!s[x].empty())ans+=w[*s[x].begin()]-sb[x],sb[x]=w[*s[x].begin()];
	else if(x==1){cout<<ans<<endl;exit(0);}
