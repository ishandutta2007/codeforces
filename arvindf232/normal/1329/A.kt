...
fun main(){
    solve.cases{
        ...
        val maxreturn = IntArray(n){ m - L[it] - it}
        val maxinrcease = IntArray(n){L[it] - 1 }
        for(i in n-1 downTo 1){
            maxreturn[i-1] = minOf(maxreturn[i-1],maxreturn[i])
        }
        if(maxreturn[0] < 0){
            put(-1)
            return@cases
        ...
        ret[0] = 0
        for(i in 1 until n){
            val new = minOf(maxinrcease[i-1] + ret[i-1], maxreturn[i].toLong())
            ret[i] = new
        }
        for(i in 0 until n){
            ret[i] += (i+1).toLong()
