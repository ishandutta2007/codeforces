...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		//k = 100;
		//q = 100000;
		fact = factorials<Z>(MAGIC);
		invfact.resize(fact.size());
		for (auto i: range(fact.size())) {
			invfact[i] = Z::rawValueOf(1) / fact[i];
		}
		vector<int> divisible_by(MAGIC + 1);
		vector<Z> times(MAGIC + 1);
		vector<vector<int>> divisors(MAGIC);
		for (int i: inclusiveRange(1, MAGIC)) {
			for (int j = i; j <= MAGIC; j += i) {
				divisors[j].push_back(i);
			}
		}
		for (int d: inclusiveRange(MAGIC)) {
			times[d] = d;
		}
		for (int d: inclusiveRange(1, MAGIC)) {
			for (int j = d * 2; j <= MAGIC; j += d) {
				times[j] -= times[d];
			}
		}
		Z ans = Z::valueOf(0);
		for (int i: range(n + q)) {
			int c = 720720;
			in >> c;
			//cerr << divisors[c].size();
			//return;
			for (int d: divisors[c]) {
				Z delta = cc(divisible_by[d] + 1, k) - cc(divisible_by[d], k);
				++divisible_by[d];
				//out << delta << ' ' << times[d] << endl;
				ans += delta * times[d];
			}
			if (i >= n) {
				out << ans << "\n";
