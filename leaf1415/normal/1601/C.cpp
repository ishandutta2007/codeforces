...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, m) comp.push_back(b[i]);
		uniq(comp);
		rep(i, 1, n) a[i] = lower_bound(all(comp), a[i]) - comp.begin() + 1;
		rep(i, 1, m) b[i] = lower_bound(all(comp), b[i]) - comp.begin() + 1;
		ll C = sz(comp);
		
		rep(i, 1, C) vec[i].clear(), acnt[i] = bcnt[i] = 0;
		rep(i, 1, n) vec[a[i]].push_back(i), acnt[a[i]]++;
		rep(i, 1, m) bcnt[b[i]]++;
		
		ll ans = 0, sum = 0;
		rep(i, 0, n) seg.update(i, i);
		rep(i, 1, C){
			for(auto x : vec[i]) seg.add(x, n, -1);
			ans += bcnt[i] * (sum + seg.query(0, n));
