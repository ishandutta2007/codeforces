...
vector<pii> vin[1010101];
vector<pii> vout[1010101];
ll ans[1010101];
ll ansOnP[1010101];
ll ansOnS[1010101];
int32_t solve() {
    ...
    for (int d = 1000000; d >= 0; --d) {
        ...
        ansOnS[d] = cnt == n ? curAns : LLINF;
    }
    ll ans = LLINF;
    for (int i = 0; i + k + 1 <= 1000000; ++i) {
        ans = min(ans, ansOnP[i] + ansOnS[i + k + 1]);
    }
    if (ans == LLINF) {
        ans = -1;
    }
