...
int main() {
    ...
    vector<point<ll>> v;
    for (int i = 0; i < n; i++) {
        while (!v.empty() && v.back().y <= a[i].y) v.pop_back();
        while (v.size() > 1 && cross(a[i]-v.back(), v.back()-v[v.size()-2]) < 0) v.pop_back();
        v.pb(a[i]);
    }
    v.insert(v.begin(), {0, v[0].y});
    v.push_back({v.back().x, 0});
    debug(v);
    ld ans = inf_ll;
    for (int i = 0; i < v.size()-1; i++)
        if (intersects({0, 0}, q*1e7, v[i], v[i+1]))
            ans = min(ans, dist(q)/dist({0, 0}, intersection(v[i], v[i+1], {0, 0}, q)));
    for (auto& x : v) {
        if (cross(q, x) == 0)
            ans = min(ans, dist(q)/dist(x));
    }
