...
struct solver
{
    ...
    solver()
    {
        ...
        for(int i=0; i<n; ++i)
        {
            ...
            int mi; cin>>mi;
            mi%=m;
            cnt[mi].emplace_back(i);
            cnt[mi+m].emplace_back(i);
        }
        int now=0;
        int ans=1+n;
        int at=-1;
        ...
        for(auto& [t,c]: cnt)
        {
            if(t>=m) break;
            now-=c.size();
            while(it!=end(cnt) and it->first<t+k)
            {
                now+=it->second.size();
                it++;
            }
            if(chmin(ans,now)) at=t;
        }
        vector<int> tra;
        for(auto& [t,c]: cnt)
        {
            if(t>at and at+k>t)
            {
                tra.insert(tra.end(),begin(c), end(c));
            }
        }
        assert(tra.size()==ans);
        for(int &x: tra) x++;
        cout << tra.size() << " " << (at+k)%m << endl << tra << endl;
    }
};