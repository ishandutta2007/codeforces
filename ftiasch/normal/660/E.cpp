...
int main() {
  int n, m;
  scanf("%d%d", &n, &m);
  std::vector<ModT> power_m(n + 1), power_2m_minus_1(n + 1);
  power_m[0] = power_2m_minus_1[0] = ModT{1};
  for (int i = 1; i <= n; ++i) {
    power_m[i] = power_m[i - 1] * ModT(m);
    power_2m_minus_1[i] = power_2m_minus_1[i - 1] * ModT(2 * m - 1);
  }
  ModT result = power_m[n];
  for (int i = 1; i <= n; ++i) {
    result += power_m[n + 1 - i] * power_2m_minus_1[i - 1];
  }
  printf("%d\n", result.get());
}