...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n){
			if(i == mn_i) continue;
			if(i % 2 == mn_i % 2) ans.push_back(Q(P(i, mn_i), P(mn, mn)));
			else ans.push_back(Q(P(i, mn_i), P(mn+1, mn)));
		}
		cout << sz(ans) << "\n";
		for(auto q : ans) cout << q.first.first << " " << q.first.second << " " << q.second.first << " " << q.second.second << "\n";
	}
