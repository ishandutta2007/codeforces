...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    cin >> s;
    for (int j = 1; j <= m; ++j) {
      passable[i * row + j] = s[j - 1] == '.' || s[j - 1] == 'E';
      if (s[j - 1] == 'E') exit = i * (row) + j;
    }
  }
  string moves;
  cin >> moves;
  ...
  for (int idx = 0; idx <= k; ++idx) {
    ...
    int offset;
    if (ch == 'U') offset = -row;
    else if (ch == 'D') offset = row;
    else if (ch == 'L') offset = -1;
    else offset = 1;

    BS move = sh(cur, offset);
    BS nxt = (move & passable) | sh(move & ~passable, -offset);
    cur = nxt;
  }
  cur = passable;
  for (int idx = 0; idx <= k; ++idx) {
