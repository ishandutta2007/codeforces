...
bool check(int ai, int aj, int bi, int bj){
    int ci = ai, cj = bj;
    for(int i=ci; i<=bi; ++i) if(mat[i][cj] == 'W') return false;
    for(int i=bi; i<=ci; ++i) if(mat[i][cj] == 'W') return false;

    for(int j=aj; j<=cj; ++j) if(mat[ci][j] == 'W') return false;
    for(int j=cj; j<=aj; ++j) if(mat[ci][j] == 'W') return false;
...
bool check(){
    REP(ai, h) REP(aj, w){
        if(mat[ai][aj] == 'W') continue;
        REP(bi, h) REP(bj, w){
            if(mat[bi][bj] == 'W') continue;
            if(!check(ai, aj, bi, bj) && !check(bi, bj, ai, aj)) return false;
        }
    }
    return true;
}
