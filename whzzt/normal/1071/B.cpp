...
int main(){
	...
	dp[0][1]=dp[1][0]=0,in[0][1]=in[1][0]=true,m=1; 
	for(i=1;i<=n;i++)for(j=1;j<=n;j++){
		dp[i][j]=min(dp[i-1][j],dp[i][j-1])+(mm[i][j]!='a');
		if(dp[i][j]<=k)chkmax(m,i+j);
	}
	for(i=2;i<=m;i++)S+='a';
	for(i=1;i<=n;i++)if(m-i>=1&&m-i<=n&&dp[i][j=m-i]<=k)in[i][j]=true;
	for(s=m+1;s<=2*n;s++){
		char c='z';
		for(i=1;i<=n;i++)if(s-i>=1&&s-i<=n){
			j=s-i;
			if(in[i-1][j]||in[i][j-1])chkmin(c,mm[i][j]);
		}
		S+=c;
		for(i=1;i<=n;i++)if(s-i>=1&&s-i<=n){
			j=s-i;
			if((in[i-1][j]||in[i][j-1])&&mm[i][j]==c){
				in[i][j]=true;
			}
		}
	}
