...
int main(){
    riano_; ll ans = 0;
    ll N,M,K,H,W,Q,L,R,T,C,A,B,X;
    cin >> N;
    ll a[N];
    rep(i,N) cin >> a[i];
    std::vector<S> v(N+2);
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(v);
    rep(i,N+2) seg.set(i,2e9);
    ll last = 0;
    rep(i,N){
        if(i==0){
            seg.set(0,1); last = a[i]; continue;
        }
        ll rp = min({seg.prod(0,a[i])+1,seg.prod(a[i]+1,N+1)+1,seg.prod(a[i],a[i]+1)});
        //cout << rp << endl;
        if(last!=a[i]){
            seg.apply(0,N+1,1);
        }
        if(seg.prod(last,last+1)>rp){
            seg.set(last,rp);
        }
        last = a[i];
