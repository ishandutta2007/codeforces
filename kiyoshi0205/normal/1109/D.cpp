...
int main(){
  ...
  For(K,2,N){
    debug(K);
    // debug(modpow(N,N-K-1)*modncrlistp[K]*modpow(M,N-K));
    j=modpow(N,N-K-1)*K%mod*modpow(M,N-K)%mod*modncrlistp[N-2]%mod*modncrlistm[N-K]%mod*modncr(M-1,K-2)%mod;
    ans+=j;
    debug(j);
  }
  ans+=modncrlistp[N-2]*modncr(M-1,N-2)%mod;
  print(ans%mod);
}