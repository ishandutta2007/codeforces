...
int main(){
    ...
    rep(ii,T){
        ...
        pot.push_back(-1);
        ll mx = -1; vector<Pr> pm; ll mi = -1; ll allmx = -1; ll allmi = -1;
        rep(i,N+2){
            if(pot[i]>=0){
                if(pot[i]>mx) mi = i;
                chmax(mx,pot[i]);
                if(pot[i]>allmx) allmi = i;
                chmax(allmx,pot[i]);
            }
            else{
                if(mx>=0){
                    pm.emplace_back(mx,mi);
                    mx = -1; mi = -1;
                }
            }
        }
        if(pm.size()==1){
            ...
            ll K = pm.size();
            ll w = pm[0].first + pm[K-1].first;
            if(w>=allmx){
                cout << 1 << "\n";
                cout << pm[0].second+1 << " " << pm[K-1].second << "\n";
            }
            else{
                cout << 2 << "\n";
                cout << 1 << " " << allmi << "\n";
