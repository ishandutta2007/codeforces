...
int main(){
  ...
  while(n--){
    ...
    if(s=="ADD"){
      ...
      auto itl=mp.lower_bound(p);
      auto itr=mp.upper_bound(p);
      itr->se+=itl->se;
      for(auto it=nonzero.begin();it!=nonzero.end();){
        if(itr->fi!=*it)mp[*it]=0;
        it=nonzero.erase(it);
      }
      mp.erase(itl);
      nonzero.insert(itr->fi);
    }
  }
  modint ret=0;
  for(auto z:mp)ret+=z.se;
  cout<<ret<<endl;
  return 0;
}