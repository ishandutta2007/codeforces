...
void __solve()
{
    ...
    dp[0]=0;
    int R=n;
    for(auto i=vot.begin(),j=i; i!=vot.end(); i=j)
    {
        int M=i->first;
        vector<i64> s(1,0);
        while(j!=vot.end() and i->first==j->first)
        {
            i64 tmp=s.back();
            s.emplace_back(tmp+j->second);
            ++j;
        }
        int now=j-i;
        R-=now;
        ...
        for(int j=n-now; j>=0; j--)
        {
            sbmin(ndp[j+now],dp[j]+s[now]);
            for(int k=0; k<now; k++)
            {
                if(k+j>=D)
                {
                    sbmin(ndp[j+k],dp[j]+s[k]);
                }
            }
        }
        swap(ndp,dp);
