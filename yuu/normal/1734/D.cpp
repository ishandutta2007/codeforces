...
void solve() {
    ...
    q[0].clear();
    q[1].clear();
    for (int i = k - 1; i >= 1; i--) q[0].push_back(a[i]);
    for (int i = k + 1; i <= n; i++) q[1].push_back(a[i]);
    int64_t hp = a[k];
    while ((!q[0].empty()) && (!q[1].empty())) {
        // cerr << q[0].size() << ' ' << q[1].size() << ' ' << hp << '\n';
        if (q[0].front() >= 0) {
            hp += q[0].front();
            q[0].pop_front();
            continue;
        ...
        bool moved[2]{0, 0};
        for (int i = 0; i <= 1; i++) {
            while ((!q[i].empty()) && (f[i] + q[i].front() >= 0)) {
                f[i] += q[i].front();
                q[i].pop_front();
                moved[i] = true;
                if (f[i] >= hp) break;
        ...
        if ((f[0] >= hp) && moved[0]) {
            if (moved[1]) q[1].push_front(f[1] - hp);
            hp = f[0];
        } else if ((f[1] >= hp) && moved[1]) {
            if (moved[0]) q[0].push_front(f[0] - hp);
            hp = f[1];
        } else {
            cout << "NO\n";
            return;
