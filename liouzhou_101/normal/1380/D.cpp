...
void solve()
{
	...
	vector<int> b(m);
	input(b);
	set<int> H(b.begin(), b.end());
	vector<int> sub;
	for (auto x : a)
		if (H.count(x)) sub.push_back(x);
	if (sub != b)
	{
		return writeln(-1);
	}
	...
	for (int i = 0; i < n; ++i)
	{
		if (H.count(a[i])) continue;
		int j = i;
		while (j < n && H.count(a[j]) == 0) ++j;

		int cnt = 0;
		int tmp = max(i - 1 >= 0 ? a[i - 1] : 0, j < n ? a[j] : 0);
	...
	ll res = INFll;
	for (int need = mink; need <= maxk; ++need)
	{
		int remain = n - m - need * k;
		freshmin(res, (ll)need * x + (ll)remain * y);
	}
	writeln(res);
}
