...
long long check(int l, int r) {
    if (l == r || a[l] == 1 || a[l] > 32) return a[l];
    long long t = check(l + 1, r);
    if (t > 32) return t;
    long long res = 1;
    while (t--) {
        res *= a[l];
...
int getProd(int l, int r, int modIdx, long long& t, bool& isBig) {
    int curmod = mods[modIdx];
    if (a[l] == 1 || l == r || modIdx == mods.size() - 1) {
        isBig = check(l, r) > 32;
        t = a[l];
        return a[l] % curmod;
    }
    int p = getProd(l + 1, r, modIdx + 1, t, isBig);
    int res = 1;
    if (!isBig) {
        ll tt = t;
        while (tt--) {
            res = res * (ll)a[l] % curmod;
        }
        ll c = 1;
        while (t--) {
            c *= a[l];
        ...
        res = int(powmod(a[l], p + mods[modIdx + 1], curmod));
//        res = int(powmod(a[l], p, tmod) * g % rms[modIdx]);
    }
    return res;
}
bool sieve[101010];
...
//        rms.push_back(val);
    ...
    while (q--) {
        ...
        bool isBig = false;
        long long t = 1;
        cout << getProd(l, r, 0, t, isBig) % oldM << '\n';
    }
