...
bool check(ll d,ll h,ll w) {
	if (d==1) return 1;
	if (h%d!=0&&h%d!=1) return 0;
	int v=w-(h%d==0?1:0);
	if (v%d!=0&&v%d!=1) return 0;
	int v2=h-(v%d==0?1:0);
	if (v2%d!=0&&v2%d!=1) return 0;
	int v3=(w-1)-(v2%d==0?1:0);
	return v3%d==0;
}
...
int main() {
	...
	for (scanf("%d",&_);_;_--) {
		ll h,w;
		scanf("%lld%lld",&h,&w);
		auto g=Factor::factor(2*(h+w-2));
		vector<ll> ans;
		sort(all(g));
		for (auto d:g) if (check(d,h,w)||check(d,w,h)) {
			ans.pb(d);
		}
		printf("%d ",SZ(ans));
