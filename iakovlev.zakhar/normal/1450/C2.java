...
public class E {
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			...
			int maxSumX = 0;
			int maxSumV = numberX[0] + numberO[0];
			for (int i = 0; i < numberX.length; i++) {
				if (numberX[i] + numberO[i] > maxSumV) {
					maxSumX = i;
					maxSumV = numberX[i] + numberO[i];
				}
			}
			int x = -1;
			int o = -1;
			...
			if (numberO[x] + numberX[o] < numberO[o] + numberX[x]) {
				x = o;
				o = 3 - x - maxSumX;
			}
			for (int i = 0; i < s.length; i++) {
				for (int j = 0; j < s.length; j++) {
					if (s[i].charAt(j) == 'X' && ((i - j) % 3 + 3) % 3 == x) {
						out.print("O");
					} else if (s[i].charAt(j) == 'O' && ((i - j) % 3 + 3) % 3 == o) {
						out.print("X");
					} else {
