...
int main(){
    LL(n);
    VEC(pll,a,n);
    auto p=partition(all(a),[](pll a){return(a.first^a.second)&1;});
    auto b=vector(p,a.end());
    a.erase(p,a.end());
    each(x,y,a)tie(x,y)=pair{(x+y+1)/2,(x-y+1)/2};
    each(x,y,b)tie(x,y)=pair{(x+y)/2,(x-y)/2};
    ll ans=0;
    auto solve=[&](vector<pll>&a){
        if(a.empty())return;
        Sort(a);
        multiset<ll>y1;
        ll ymin=LINF,ymax=-LINF;
        each(x,y,a)y1.insert(y);
        const ll x1=a.front().first,x2=a.back().first;
        Rev(a);
        rep(X,x1,x2){
            while(a.back().first==X){
                const ll y=a.back().second;
                a.pop_back();
                y1.erase(y1.find(y));
                chmin(ymin,y);
                chmax(ymax,y);
            }
            ans+=max(min(*y1.rbegin(),ymax)-max(*y1.begin(),ymin),0LL);
        }
    };
    solve(a);
    solve(b);
