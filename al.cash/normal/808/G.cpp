...
public:
    int size() const { return static_cast<int>(prefixF.size()) - 1; }
...
void ac() {
    ...
    writeln(*max_element(dp, dp + n + 1));
}

int main(int argc, const char* argv[]) {
    input.reset(argc > 1 ? new InputFile(argv[1]) : new InputFile(stdin, false));
    output.reset(argc > 2 ? new OutputFile(argv[2]) : new OutputFile());
    //input.reset(new InputFile());
    //output.reset(new OutputFile());
    ac();
    if (argc > 1) {
        writeln("\n\nTotal time: ", setPrecision(6), static_cast<double>(clock()) / CLOCKS_PER_SEC);
    }
    return 0;
}