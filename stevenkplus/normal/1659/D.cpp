...
// ============= Solution ============= //
// 0 3 4 2 3 2 7
// first, figure out popcount of A; = sum(C) / len(C) = 3 in example
// then, you know the last config: 0 0 ... 1 1 1. subtract it from C.
// this tells you A[n - 1] = C[n - 1] / (n - 1) = 1
// now recurse
// this gives you the second to last config as well; 0 0 0 .. 1 1
int main() {
   int T;
   cin >> T;
   while (T--) {
      ...
      for (int i = n - 1; i >= 0; --i) {
         ...
         pp(v);
         subtract += 1;
         // for (int j = i; j > i - popcount; --j) {
            // v[j] -= 1;
         // }
         if (i - popcount >= 0) diffs[i - popcount] += 1;
         subtract -= diffs[i];
         sum -= popcount;
         if (v[i] - subtract || (i == 0 && popcount > 0)) {
            // v[i] must be equal to i
            A[i] = 1;
            sum -= v[i] - subtract;
         }
      }
