...
ll rev(ll n) {
    n %= fftmod;
    return powmod(n, fftmod - 2, fftmod);
}
int cnt[530];
int whichRow[530];
int solve() {
...
//        DBN(i, g[i]);
    ...
    for (int i = N - 1; i >= 0; --i) {
        ...
        whichRow[i] = goodRow;
        if (goodRow == -1) continue;
        used[goodRow] = true;
        ll agi = rev(a[goodRow][i]);
        for (int row = 0; row < N; ++row) {
            if (row == goodRow) continue;
            if (a[row][i] != 0) {
                ll factor = agi * a[row][i] % fftmod;
                for (int j = 0; j < N; ++j) {
                    a[row][j] -= a[goodRow][j] * factor;
                    a[row][j] %= fftmod;
                    if (a[row][j] < 0) a[row][j] += fftmod;
                }
                b[row] -= b[goodRow] * factor;
                b[row] %= fftmod;
                if (b[row] < 0) b[row] += fftmod;
            }
            // a[row][i] -= a[goorRow][i] * (a[row][i] / a[goorRow][i])
            assert(a[row][i] == 0);
        }
        if (i == 0) {
            cout << b[goodRow] * agi % fftmod << endl;
            return 0;
        }
    }
    assert(false);
    // w[i] * (n + 1) - sum(w[i ^ g[j]]) = ((i != 0))
    return 0;
}