...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        auto value = [&](ll val, ll p) -> ll {
            if (val < p) {
                return p - val;
            }
            return min(val % p, p - val % p);
        };
        vec<ll> vals;
        ll start = max(1ll, a[0] - n + 1);
        for (ll i = start; i < a[0] + n; i++) {
            vals.push_back(i);
        }
        auto p = primes(1'000'002);
        vec<ll> vars;
        for (int i : p) {
            int st = (i - start % i) % i;
            if (st >= vals.size()) {
        ...
        reverse(all(a));
        sort(all(vars), [&](ll x, ll y) -> bool {
            return value(a[0], x) < value(a[0], y);
        });
        ll answer = n + 1;
        for (ll i : vars) {
            ll res = 0;
