...
class DKhranitelnitsaBeskonechnogoZooparka {
public:
    void solve() {
        /*int sz = 64;
        arr2d<bool> r(sz, sz, false);
        for (int i : range(sz)) {
            for (int j : range(sz - i)) {
                if ((i & j) == j) {
                    r(i, i + j) = true;
                }
            }
        ...
        auto bits = [](unsigned s) -> vi {
            vi res;
            for (int i : RevRange(30)) {
                if (isSet(s, i)) {
                    res.push_back(i);
                }
            }
...
//            }
        ...
        int at = 0;
        bool canSkip = false;
        for (int i : range(vb.size())) {
            if (at < ub.size() && vb[i] > ub[at]) {
                canSkip = true;
            }
            if (canSkip) {
                while (at < ub.size() && ub[at] > vb[i]) {
                    at++;
                }
            }
            if (at == ub.size() || ub[at] > vb[i]) {
                out.printLine("NO");
//                if (r(u, v)) {
//                    throw "Jopa";
//                }
...
int main() {
    ...
    auto time = clock();
#endif
    DKhranitelnitsaBeskonechnogoZooparka solver;
