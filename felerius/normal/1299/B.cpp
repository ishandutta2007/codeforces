...
// Problem: B: Aerodynamic (https://codeforces.com/contest/1299/problem/B)
...
int main() {
    ...
    vpi d(n);
    rep(i, n)
        d[i] = {p[(i+1)%n].first - p[i].first, p[(i+1)%n].second - p[i].second};
    bool all = true;
    rep(i, n/2)
        all &= d[i] == pair(-d[i+n/2].first, -d[i+n/2].second);
