...
struct node{
    ...
    bool ed[2];
    bool f[2];
}*rt,tree[MAXN*32+10],*tcnt=tree;
void insert(uint val,int len,bool f){
    node *p=rt;
    int i;
    for(i=0;i<len;i++){
        p->f[f]=1;
...
void dfs(node *p,int val,int len){
    if(!p)
        return;
    if(!p->f[0])
        return;
    if(!p->f[1]){
        ans.pb({val<<(32-len),len});
        return;
    }
    if((p->ed[0]&&p->f[1])||(p->ed[1]&&p->f[0])){
        puts("-1");
        exit(0);
    }
    dfs(p->ch[0],(val<<1),len+1);
    dfs(p->ch[1],(val<<1)|1,len+1);
}
void print(pii x){
