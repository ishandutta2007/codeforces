...
const ll INF = 1e18;
int main() {
	ll n,r1,r2,r3,d; R(n,r1,r2,r3,d);
	vector<ll> a(n + 1);
	for (int i = 1; i <= n; i++)
		R(a[i]);
	...
	for (int i = 1; i <= n; i++) {
		dp[i + 1][0] = dp[i][0] + r1 * a[i] + r3 + d;
		
		ll min1 = min(r1 * (a[i] + 1), r2);
		dp[i + 1][1] = dp[i][0] + min1 + d;
		
		chkmin(dp[i + 1][1], dp[i][1] + min1 + d + r1 + d + d);
		chkmin(dp[i + 1][0], dp[i][1] + min1 + d + r1 + d + r1 + d);
		chkmin(dp[i + 1][0], dp[i][1] + r1 * a[i] + r3 + d + r1 + d + d);
	}
	
	ll ans = min({
		dp[n][1] + min(r1 * (a[n] + 1), r2) + d + r1 + d + r1,
		dp[n][1] + r1 * a[n] + r3 + d + r1,
		dp[n][0] + min(r1 * (a[n] + 1), r2) + d + d + r1,
		dp[n][0] + r1 * a[n] + r3,
	});
