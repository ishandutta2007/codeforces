...
array<char, 20202020> buf;
char *getNew(int n) {
    ++n;
    static char* bptr = buf.data();
    return (bptr += n) - n;
}
char *s[1010101];
vector<vector<pii>> rounds;
int n, m;
int solve() {
    vector<pii> nextRound;
    vector<pii> curRound;
    cin >> n >> m;
    FI(n + 2) {
        s[i] = getNew(m + 2);
