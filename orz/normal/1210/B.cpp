...
void take(int idx, const vpll& p, set<int>& taken, vl& took, ll& ans)
{
	if (taken.count(idx) == 0)
	{
		taken.insert(idx);
		took.pb(p[idx].X);
		ans += p[idx].Y;
...
int another_main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		if (i && p[i].X == p[i - 1].X)
		{
			take(i - 1, p, taken, took, ans);
			take(i, p, taken, took, ans);
		}
		else if (good(p[i].X, took))
			take(i, p, taken, took, ans);
	}
	cout << ans << '\n';
	return 0;
