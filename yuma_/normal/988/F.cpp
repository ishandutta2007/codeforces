...
{
	int A,N,K;cin>>A>>N>>K;
	vector<vector<long long int>>dp(A+1,vector<long long int>(K+1,1e12));
	dp[0][K]=0;
	vector<int>rains(A);
	for (int i = 0; i < N; ++i) {
		int l,r;cin>>l>>r;
	...
	for (int x = 0; x < A; ++x) {
		for (int nu = 0; nu <= K; ++nu) {
			...
			if (rains[x] && nu == K) {
				...
				dp[x+1][nu]=min(dp[x+1][nu],dp[x][nu]+ws[nu]);
			}

			if (!rains[x] && nu != K) {
				dp[x+1][K]=min(dp[x+1][K],dp[x][nu]);
			}

			for (auto umb : umbs[x]) {
				dp[x+1][umb]=min(dp[x+1][umb],dp[x][nu]+ws[umb]);
			}
		}
	}
