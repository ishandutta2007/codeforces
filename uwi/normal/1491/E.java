...
public class E2 {
	...
	boolean isfib(int[][] g, int[] vs)
	{
		int n = g.length;
		if(n <= 3)return true;
		int tar = Arrays.binarySearch(F, n);
		if(tar < 0)return false;

		int[] vv = Arrays.copyOf(vs, vs.length);
		Arrays.sort(vv);
		long h = 0;
		for(int o : vv){
			h = add(h, o);
		...
		outer:
		for(int i = n-1;i >= 0;i--){
			if(des[i] == F[tar-2] || des[i] == F[tar-1]){
				...
				Arrays.fill(map, -1);
				{
					...
					map[i] = 0;
					int gen = 1;
					int[] from = new int[des[i]-1];
					int[] to = new int[des[i]-1];
					while (!q.isEmpty()) {
						int cur = q.poll();
						for (int e : g[cur]) {
					...
					if(!isfib(packU(gen, from, to), nvs))continue outer;
				}
				int xgen = 0;
				for(int j = 0;j < n;j++){
					map[j] = map[j] == -1 ? xgen++ : -1;
				}
				{
					Queue<Integer> q = new ArrayDeque<>();
					q.add(0);
					map[0] = 0;
					int gen = 1;
					int[] from = new int[n-des[i]-1];
					int[] to = new int[n-des[i]-1];
					while (!q.isEmpty()) {
						int cur = q.poll();
						for (int e : g[cur]) {
							if (par[cur] == e) continue;
