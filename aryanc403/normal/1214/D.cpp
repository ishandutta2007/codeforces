...
const lli maxN = 1000000L;
    ...
    lli m;
    string s[maxN+5];
    lli vis[maxN+5],vis2[maxN+5];
    vi a;
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

bool dfs(lli x,lli y)
{
    if(x>=n||y>=m)
        return false;
    if(vis[x*m+y]!=-1)
        return vis[x*m+y];
    if(s[x][y]=='#')
        return vis[x*m+y]=vis2[x*m+y]=false;
    dfs(x,y+1);
    dfs(x+1,y);
    return vis[x*m+y]=vis2[x*m+y]=(dfs(x+1,y)||dfs(x,y+1));
}
...
// cin>>T;while(T--)
{
    ...
    fo(i,n)
        cin>>s[i];
    memset(vis,-1,(maxN+5)*sizeof(lli));
    vis[n*m-1]=1;
    dfs(0,0);
    dbg(vis[0]);
    if(vis[0]==0)
    ...
    {
        repD(i,n-2,0)
            vis[i*m+j]|=vis[(i+1)*m+j];
    }

    fo(i,n)
    {
        repD(j,m-2,0)
            vis2[i*m+j]|=vis2[i*m+j+1];
    }
