...
int main() {
    ...
    loop(t) {
        ...
        map<long long, vector<int>> mpp, mps;
        rep(i, n + 1) {
            if (i != 0) mpp[s[i]].push_back(i);
            if (i != n) mps[s[n] - s[i]].push_back(i);
        }

        vector<int> ls;
        rep(i, n + 1) {
            if (0 < i and i < n and a[i - 1] == 0 and a[i] == 0) continue;
            ls.push_back(i);
        }
        mint ans = 1;
        for (auto [sum, ids] : mpp) {
            int w = ids.size();
            if (sum > s[n] >> 1) break;
            int h = mps[sum].size();
            if (s[n] == sum * 2 and h > 1) {
                debug(w, h);
                ans *= mint(2).pow(h);
            } else {
                mint c = 0;
                rep(q, h + 1) {
                    c += fac.binom(w, q) * fac.binom(h, q);
                }
                ans *= c;
            }
        }
        ...
        //     rep(t, 1 << (n - 1)) {
        //         vector<int> sep{ 0 };
        //         rep(i, n - 1) if (kth_bit(t, i)) sep.push_back(i + 1);
        //         sep.push_back(n);

        //         bool ok = true;
        //         int siz = sep.size();
        //         rep(i, siz - 1) {
        //             int l1 = sep[i], r1 = sep[i + 1];
        //             int l2 = sep[siz - i - 2], r2 = sep[siz - i - 1];
        //             ok &= s[r1] - s[l1] == s[r2] - s[l2];
        //         }
        //         ans2 += ok;
        //     }
        //     debug(ans, ans2);
