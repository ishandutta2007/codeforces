...
vector<pii> v[55];
bool used[55];
void goDist(int cur, int d, int *dp) {
    dp[cur] = d;
    used[cur] = true;
    for (pii p : v[cur]) {
        int to = p.first;
...
int solve() {
    ...
    FI(n) {
        int maxd = *max_element(dist[i], dist[i] + n);
        dp[i][1] = maxd;
    }
    int s;
    cin >> s;
    ...
    for (int j = 1; j <= k; ++j) {
        for (int root : leaves) {
            ...
            while (l < r) {
                ...
                for (int i : leaves) {
                    int k = 0;
                    while (k + 1 <= j && dist[root][i] + dp[i][j - k - 1] >= m) {
                        ++k;
                    }
                    cnt += k;
                }
    ...
    while (l < r) {
        ...
        for (auto& s : subtrees) {
            ...
            for (auto i : s) {
                int k = 0;
                while (k + 1 <= tot && dist[root][i] + dp[i][tot - k - 1] >= m) {
                    ++k;
                }
                cnt -= k;
            }
