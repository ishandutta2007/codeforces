...
long long mod=1000000007,n=3000000,m,k,fac[3000002],ifac[3000002],inv[3000002];
...
int main() {
	...
	F(iakioi,1,read()){
		read(n);read(m);read(k);ll ans=0;
		if(n!=m){
		F(i,1,m){
			ans+=C(n-i-1,m-i)*pw(2,i)%mod*i%mod;
		}write(ans%mod*pw(mod/2+1,n)%mod*k%mod,'\n');	
		}else{
			write(n*k%mod,'\n');
		}
