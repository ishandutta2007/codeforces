...
// URL: http://codeforces.com/contest/1504/problem/D
// Memory Limit: 256 MB
// Time Limit: 3000 ms
...
{
	int n=ri();
	for(int i=0;i<n;i++)for(int j=0;j<n;j++)if((i+j)&1)a.pb({i,j});else b.pb({i,j});
	for(int i=0;i<n*n;i++){
		c=ri();
		if(a.empty()){
			placeB();
