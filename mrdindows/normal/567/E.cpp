...
ll dout[100010];
bool used[100010];
void dej(int s, ll *d, vector<int> *v, int *to) {
    for (int i = 0; i < n; ++i) {
        d[i] = LLINF;
        used[i] = false;
    }
...
int main() {
    ...
    FI(m) {
        ...
        v[x].push_back(i);
        revv[y].push_back(i);
        mv[{x, y, len[i]}].push_back(i);
    }
    dej(s, din, v, to);
    dej(t, dout, revv, from);
    long long sd = LLINF;
    FI(n) {
        if (din[i] + dout[i] < sd) {
            sd = din[i] + dout[i];
...
//            DBN(i);
    ...
    find_bridges();
    FI(m) {
        if (ok[i] && isb[i] && !proc[i]) {
            auto& pv = mv[{from[i], to[i], len[i]}];
            for (int r : mv[{from[i], to[i], len[i]}]){
                ok[r] = true;
                isb[r] = (pv.size() == 1);
                proc[r] = true;
            }
    ...
    FI(m) {
        if (ok[i] && isb[i]) {
            ...
            ll tot = din[from[i]] + len[i] + dout[to[i]];
            ll d = tot - (sd - 1);
            if (d < len[i]) {
                cout << "CAN " << d << '\n';
            } else {
