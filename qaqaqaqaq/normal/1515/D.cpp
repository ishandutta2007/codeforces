...
signed main(){
    ...
    while (t--){
        cin>>n>>l>>r;
        rep1(i,n) cnt[i][0]=cnt[i][1]=0;
        rep1(i,n) cin>>c[i];
        rep1(i,l) cnt[c[i]][0]++;
        for (int i=l+1;i<=n;++i) cnt[c[i]][1]++;
        int hi=0;
        rep1(i,n) hi=min(cnt[i][0],cnt[i][1]), cnt[i][0]-=hi, cnt[i][1]-=hi;
        int cost=0,ll=0,rr=0;
        int pil=0,pir=0;
        rep1(i,n){
            pil+=cnt[i][0]/2;
            pir+=cnt[i][1]/2;
        ...
        hi=min(ll,rr);
        ll-=hi,rr-=hi;
        if (rr>0) swap(rr,ll), swap(pil,pir);
        while (ll>=2&&pir>0) cost+=2, ll-=2, pir--;
        cost+=pir+pil+ll+rr+hi;
        cout<<cost<<"\n";
    }
    return 0;
}