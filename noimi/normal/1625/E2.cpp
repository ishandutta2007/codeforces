...
int main() {
    ...
    g.resize(num);
    vl dp(n + 1);
    rep(i, num) { dp[i] = 1LL * si(g[i]) * (si(g[i]) + 1) / 2; }
    vi idx2(num);
    int c = 0;
    rep(i, num) { fore(e, g[i]) idx2[e] = c++; }

    RST<ll> seg(dp), seg2(vl(num, 1));
    while(q--) {
        INT(t, l, r);
        l--, r--;
        if(t == 1) {
            seg2.set(idx2[idx[l]], 0);
            int p = par[idx[l]];
            g[p].ppb();
            seg.set(p, 1LL * si(g[p]) * (si(g[p]) + 1) / 2);
        } else {
            l = idx[l], r = idx[r];
            ll res = seg2.prod(idx2[l], idx2[r] + 1);
            OUT(res * (res + 1) / 2 + seg.prod(l, out[r]));
        }
    }
}