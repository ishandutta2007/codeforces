...
void solve() {
    ...
    vector<int> a(n);
    cin >> a;
    vector<int> powers(63, 1);int i = 1;
    for (; i < 63; ++i) {
        powers[i] = powers[i - 1] * k;
        if (powers[i] > 1e18 / k) break;
    }
    powers.resize(i);
    vector<bool> used(63);
    for (int i = 0; i < n; ++i) {
        while (a[i]) {
            auto it = upper_bound(powers.begin(), powers.end(), a[i]);
            if (used[( it- powers.begin() - 1)]) answer("NO");
            used[it - powers.begin() - 1] = true;
            a[i] -= *(it - 1);
        }
    }
    answer("YES");
