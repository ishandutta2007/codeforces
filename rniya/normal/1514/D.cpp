...
int main() {
    ...
    vector<int> cnt(n, 0);
    vector<int> cnt_cnt(n + 1, 0);
    int cur = -1;
    auto add = [&](int idx) {
        int val = a[idx];
        // seg.update(val, seg[val] + 1);
        cnt_cnt[cnt[val]]--;
        cnt[val]++;
        cnt_cnt[cnt[val]]++;
        if (cur < cnt[val]) cur = cnt[val];
    };
    auto del = [&](int idx) {
        int val = a[idx];
        // seg.update(val, seg[val] - 1);
        cnt_cnt[cnt[val]]--;
        if (cur == cnt[val] && cnt_cnt[cnt[val]] == 0) cur--;
        cnt[val]--;
        cnt_cnt[cnt[val]]++;
    };
    vector<int> Max(q);
    auto rem = [&](int idx) {
        // Max[idx] = seg.query(0, n);
        Max[idx] = cur;
    };
