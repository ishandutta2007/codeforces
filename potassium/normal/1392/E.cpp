...
int main() {
		...
		for (int i = n - 2; i >= 0; i--) {
			for (int j = n - 2; j >= 0; j--) {
				ll minv = getMinv(i, j + 1);
				ll maxv = getMaxv(i + 1, j);
				
				table[i][j + 1] = maxv - minv + 1;
			}
		}
		...
		while (q--) {
			...
			while (now.F < n - 1 && now.S < n - 1) {
				ll maxv = getMaxv(now.F + 1, now.S);
				if (dist > maxv) {
					now.S++;
				} else {
					now.F++;
				}
				dist -= table[now.F][now.S];
				ans.pb(now);
			}
