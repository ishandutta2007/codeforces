...
void do_op(int x, int y) {
    vector<int> v1, v2, v3;

    for (int j = 0; j < x; j++) v1.push_back(a[j]);
    for (int j = x; j < y; j++) v2.push_back(a[j]);
    for (int j = y; j < n; j++) v3.push_back(a[j]);

    vi op;
    if (sz(v1)) op.push_back(sz(v1));
    if (sz(v2)) op.push_back(sz(v2));
    if (sz(v3)) op.push_back(sz(v3));

    if (sz(op) > 1) {
        ans.push_back(op);
        a.clear();
        a.insert(a.end(), all(v3));
        a.insert(a.end(), all(v2));
        a.insert(a.end(), all(v1));
    }
}
