...
int main() {
	...
	vector<Mint> fac(MAXN, 1), rev(MAXN, 1);
	for (int i = 2; i < MAXN; i++)
		fac[i] = fac[i - 1] * i;
	rev[MAXN - 1] = fac[MAXN - 1].inv();
	for (int i = MAXN - 1; i >= 2; i--)
		rev[i - 1] = rev[i] * i;
		
	vector<Mint> knap(total + 1, 0);
	knap[0] = 1;
	for (auto& i : m) {
		if (i == 0) continue;
	...
	Mint ways = fac[total / 2] * fac[total / 2];
	for (auto& i : m)
		ways *= rev[i];
		
	vector<vector<Mint>> ans(52, vector<Mint>(52));
	for (int i = 0; i < 52; i++)
		ans[i][i] = knap[total / 2] * ways;
		
	for (int i = 0; i < 52; i++) {
		if (m[i] == 0) continue;
		
		for (int k = m[i]; k <= total; k++)
			knap[k] -= knap[k - m[i]];
				
		for (int j = i + 1; j < 52; j++) {
			if (m[j] == 0) continue;
			
			for (int k = m[j]; k <= total; k++)
				knap[k] -= knap[k - m[j]];
				
			ans[i][j] = ans[j][i] = knap[total / 2] * ways * 2;
			
			for (int k = total; k >= m[j]; k--)
				knap[k] += knap[k - m[j]];
		}
		
		for (int k = total; k >= m[i]; k--)
			knap[k] += knap[k - m[i]];
	}
