...
int mul(int a,int b) {
	__int128 ans=0; 
	For(i,0,n-1) if(a>>i&1) ans^=(__int128)b<<i;
	foR(i,2*n-2,n) if(ans>>i&1) ans^=(__int128)mod<<(i-n); 
	return ans; 
}
map<int,int>t; 
void works() {
	FI(s); n=strlen(s); 
	st=1;
	For(i,0,n-1) if(s[i]=='1') mod|=(1ll<<i); 
	while(mod&&mod%2==0) mod>>=1,st++; 
	if(!mod) return printf("-1\n"),void(); 
	if(mod==1) return printf("%lld %lld\n",st,st+1),void(); 
	n=63-__builtin_clzll(mod);
	int bs=1,gs=1;
	For(i,0,(1<<17)-1) t[bs]=i,bs=mul(bs,2);
	For(i,1,(1<<18)) {
		gs=mul(gs,bs); 
//		cerr<<i<<endl;
		if(t.count(gs)) {
