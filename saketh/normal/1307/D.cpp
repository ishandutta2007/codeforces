...
int main() {
    ...
    vi spec(K); re(spec); trav (e, spec) --e;
    ...
    sort_by(spec, to_dest[a] > to_dest[b]);

    int worst_add = 0;
    int worst_from = INT_MIN;

    trav (e, spec) {
        ckmax(worst_add, worst_from + 1 + to_dest[e]);
        ckmax(worst_from, from_src[e]);
    }

    int current = from_src[N - 1];
    cerr << current << " " << worst_add << endl;

    ps(min(current, worst_add));
