...
void solve(){
    ...
    REP(i, n / 2) {
        REP(q, n / 2) {
            bads.push_back(mp(i, q));
        }
    }
    for (auto i = inputs.begin(); i != inputs.end(); ++i) {
        while (bads.empty() == false && i->second >= 4) {
            i->second -= 4;
            ans[bads.back().first][bads.back().second] = i->first;
            ans[n - bads.back().first - 1][bads.back().second] = i->first;
            ans[bads.back().first][n - bads.back().second - 1] = i->first;
            ans[n - bads.back().first - 1][n - bads.back().second - 1] = i->first;
            bads.pop_back();
        }
    }
    if (bads.empty() == false) {
    ...
    if (n % 2 == 1) {
        REP(i, n / 2) {
            bads.push_back(mp(n / 2, i));
            bads.push_back(mp(i, n / 2));
        }
        for (auto i = inputs.begin(); i != inputs.end(); ++i) {
            while (bads.empty() == false && i->second >= 2) {
                i->second -= 2;
                ans[bads.back().first][bads.back().second] = i->first;
                ans[n - bads.back().first - 1][n - bads.back().second - 1] = i->first;
                bads.pop_back();
