...
public class Main
{
	...
	static long[]cost;
	static final Comparator<pair> paic=new Comparator<pair>()
		{
	@Override
	...
	static PriorityQueue<pair> heap=new PriorityQueue<pair>(paic);
	static class node
	{
		List<pair> adj=new ArrayList<pair>();
		Set<Integer> trav=new HashSet<Integer>();
	}
	static void dijkstrianbfs()
	{
		if(!heap.isEmpty())
		{
		pair te=heap.poll();
		while(!heap.isEmpty()&&visited[te.a])
			te=heap.poll();
		if(!visited[te.a])
		{
		visited[te.a]=true;
		cost[te.a]=te.b;
		while(gra[te.a].trav.contains(te.b))
			te.b++;
		for(pair ho:gra[te.a].adj)
			heap.add(new pair(ho.a, ho.b+te.b));
		dijkstrianbfs();
		}
		}
	}
