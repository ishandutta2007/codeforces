...
int main() {
    ...
    mint sum = dp[0] + dp[1];
    FOR(i, 2, dp.size()) {
        dp[i] = dp[i - 1] + dp[i - 1] * cum[i - 2] + dp[i - 1] * (dp[i - 1] + 1) / 2;
        sum += dp[i];
        cum[i] = sum;
    }
    dbgif(N <= 10, dp);
    ...
    REP(u, N) {
        int d = N - u - 2;
        if (d < 0) continue;
        mint tmp = dp[d] * (d ? cum[d - 1] : 0) + dp[d] * (dp[d] + 1) / 2;
        ret += dp[u] * tmp;
    }
    dbg(ret);
    // 2
    ...
    // 3, 5
    {
        mint tmp = dp[N - 1] * cum[N - 2] + dp[N - 1] * (dp[N - 1] + 1) / 2;
        ret += tmp * 2;
    }
    dbg(ret);
    ...
    // 4, 6
    {
        mint tmp = 0;
        // oxx
        tmp += dp[N - 1] * (cum[N - 2] * (cum[N - 2] + 1) / 2);
        // oox
        tmp += dp[N - 1] * (dp[N - 1] + 1) / 2 * cum[N - 2];
        // ooo
        tmp += dp[N - 1]; // aaa
        tmp += dp[N - 1] * (dp[N - 1] - 1); // aab
        tmp += dp[N - 1] * (dp[N - 1] - 1) * (dp[N - 1] - 2) / 6; // abc
        ret += tmp * 2;
    }
    dbg(ret);
