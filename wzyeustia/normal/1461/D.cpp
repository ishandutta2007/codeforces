...
void split(vector<ll> &arr, int l, int r,
            vector<ll> &pre, unordered_set<ll> &good) {
    
    // cout << "split " << l << " " << r << endl;
    if (l == 0)
        good.emplace(pre[r]);
    else
        good.emplace(pre[r] - pre[l-1]);
    ...
    auto iter = upper_bound(arr.begin()+l, arr.begin()+r+1, mid);

    // cout << "find " << iter-arr.begin() << endl;
    if (r == -1) getchar();

    split(arr, l, iter-arr.begin()-1, pre, good);
    split(arr, iter-arr.begin(), r, pre, good);
}
