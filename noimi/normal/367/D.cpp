...
template <class T, class Op> class SWAG {
  ...
  public:
    SWAG(const Op &op = Op(), T t = T()) : op(op), front_stack(), back_stack() {}
...
int main() {
    ...
    SWAG swag([](int x, int y) { return x | y; }, 0);

    vi dp(1 << m);
    rep(i, d - 1) swag.push(1 << inv[i]);
    rep2(i, d - 1, n - 1) {
        swag.push(1 << inv[i]);
        dp[swag.fold_all()] = 1;
        swag.pop();
    ...
    rep(j, m) rep(i, 1 << m) if(~i & 1 << j) dp[i | 1 << j] |= dp[i];
    int mi = m;
    rep(i, 1 << m) if(!dp[i]) chmin(mi, m - popcount(i));
    cout << mi << endl;
}