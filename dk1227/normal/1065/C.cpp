...
int main() {
	qread(n, k);
	rep(i, 1, n) qread(a[i]), ++cnt[a[i]], To_min(mn, a[i]), To_max(mx, a[i]);
	rep(i, mn, mx) pre[i] = pre[i - 1] + cnt[i];
	int l = mn, r = mx;
	ll cost = 0, cn = 0;
	while(l < r) {
		int now = r;
		cn = cnt[now]; 
		while(cost <= k && l <= now) --now, cost += pre[r] - pre[now], cn += cnt[now];
		if(cost > k) cn -= cnt[now], ++now;
		++ans;
		r = now;
		pre[r] += cn - cnt[r];
		cnt[r] = cn;
		//cout << now << ' ' << pre[r] << ' ' << cnt[r] << endl;
		cost = cn = 0;
	}
	cout << ans << endl;
 	return 0;
