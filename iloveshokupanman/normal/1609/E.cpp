...
vll merge(vll x, vll y) {
    vll r(6);
    rep(j, 3)r[j] = x[j] + y[j];
    r[3] = min(x[3] + y[1],x[0] + y[3]);
    r[4] = min(x[4] + y[2],x[1] + y[4] );
    r[5] = min({ x[0] + y[5],x[3] + y[4],x[5] + y[2] });
    return r;
}
...
int main() {
    ...
    string S;
    cin >> S;
    segtree<vll, merge, e> sg(N);
    rep(i, N) {
        vll v(6);
        v[S[i] - 'a']++;
        sg.set(i, v);
    }
    //rep(j, 6)cout << sg.all_prod()[j] << " ";
    //cout << endl;
    ...
    rep(q, Q) {
        ...
        v[c - 'a']++;
        sg.set(x, v);
        //rep(j, 5)cout << sg.all_prod()[j] << " ";
        cout << sg.all_prod()[5] << endl;
    }
}