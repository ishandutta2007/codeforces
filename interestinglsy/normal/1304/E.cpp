...
bool Solve( int e1 , int e2 , int st , int ed , int k ){
	int dis1 = Dis(st,e1) + 1 + Dis(e2,ed);
	int dis2 = Dis(st,e2) + 1 + Dis(e1,ed);
	int minDis = min({
		Dis(st,ed),
		dis1,
		dis2
	...
	int oDis = Dis(st,ed);
	if( oDis <= k and k%2 == oDis%2 ) return true;

	if( dis1 <= k and dis1%2 == k%2 ) return true;
	if( dis2 <= k and dis2%2 == k%2 ) return true;
	return false;
}
...
int main(){
	...
	iRead(q);
	For(qwq,q){
		iRead4(e1,e2,st,ed); iRead(len);
		bool ans = Solve(e1,e2,st,ed,len);
		puts(ans?"YES":"NO");
	}
