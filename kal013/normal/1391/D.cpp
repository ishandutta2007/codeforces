...
inline int get_dist(int first_mask,int second_mask){
	return __builtin_popcount(first_mask ^ second_mask);
}

inline bool correct(int first_mask, int second_mask, int n){
	for(int i = 0; i + 1 < n; ++i){
		int curr = ((first_mask >> i) & 1) + ((first_mask >> (i + 1)) & 1);
		curr += ((second_mask >> i) & 1) + ((second_mask >> (i + 1)) & 1);
		if (~ curr & 1){
			return false;
		}
	}
