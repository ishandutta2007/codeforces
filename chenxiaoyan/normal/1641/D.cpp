...
int n, m;
int ord[N];
int _a[N][M], _w[N], a[N][M], w[N];
void disc() {
	vi nums;
	REP(i, 1, n) REP(j, 1, m) nums.pb(a[i][j]);
	sort(ALL(nums)), nums.resize(unique(ALL(nums)) - nums.begin());
	REP(i, 1, n) REP(j, 1, m) a[i][j] = lower_bound(ALL(nums), a[i][j]) - nums.begin() + 1;
}
...
void disc0() {
	vector<i128> nums;
	REP(i, 1, n) REP(j, 1, (1 << m) - 1) nums.pb(hsh[i][j]);
	sort(ALL(nums)), nums.resize(unique(ALL(nums)) - nums.begin());
	REP(i, 1, n) REP(j, 1, (1 << m) - 1) h[i][j] = lower_bound(ALL(nums), hsh[i][j]) - nums.begin() + 1;
	REP(i, 1, n) REP(j, 1, (1 << m) - 1) v[i].pb(h[i][j]);
}

int ans[N];
void solve(int l, int r, vi se) {
	if(l == r) {
		for(int x : se) ans[x] = l;
		return;
	}
	int mid = l + r >> 1;
	static int tmp[N * 32];
	REP(i, l, mid) for(int x : v[i]) ++tmp[x];
	vi sel, ser;
	for(int i : se) {
		int cnt = 0;
		REP(j, 1, (1 << m) - 1) cnt += (__builtin_popcount(j) & 1 ? 1 : -1) * tmp[h[i][j]];
		if(cnt == mid - l + 1) ser.pb(i);
		else sel.pb(i);
	}
	REP(i, l, mid) for(int x : v[i]) tmp[x] = 0;
	solve(l, mid, sel), solve(mid + 1, r, ser);
}

void mian() {
	n = read(), m = read();
	REP(i, 1, n) {
		REP(j, 1, m) _a[i][j] = read();
		_w[i] = read();
	}
	REP(i, 1, n) ord[i] = i;
	sort(ord + 1, ord + n + 1, [&](int x, int y) { return _w[x] < _w[y]; });
	REP(i, 1, n) {
		REP(j, 1, m) a[i][j] = _a[ord[i]][j];
		w[i] = _w[ord[i]];
	}
//	debug(a, 1, n, 1, m);
	disc();
	REP(i, 1, n) {
		sort(a[i] + 1, a[i] + m + 1);
		REP(j, 1, (1 << m) - 1) {
			REP(k, 1, m) if(j >> k - 1 & 1) hsh[i][j] = hsh[i][j] * 4000000 + a[i][k];
		}
	}
	disc0();
	vi se; REP(i, 1, n) se.pb(i);
	solve(1, n + 1, se);
	ll A = lnf;
	REP(i, 1, n) if(ans[i] <= n) chkmn(A, w[i] + w[ans[i]]);
	prt(A == lnf ? -1 : A), pc('\n');
}
