...
int x[2005],y[2005],w[2005];
bitset<2000>bt[2000];
int cnt[2005];
Z c[2005][2005];
int n,m,r;
Z solve(int x,int y,int z){
	return c[n][m]-c[n-z-x][m]-c[n-z-y][m]+c[n-z-x-y][m];
}
signed main(){
#ifdef QAQAutoMaton 
	...
	for(int i=0;i<n;++i){
		for(int j=0;j<n;++j){
			int z=(bt[i]&bt[j]).count(),x=cnt[i]-z,y=cnt[j]-z;
			ans+=solve(x,y,z)*w[i]*w[j];
		}
	}
	write(ans.x,'\n');
	return 0;
