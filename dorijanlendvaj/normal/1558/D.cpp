...
int t,n,m,fac[N],inf[N],x[N],y[N],seg[M*2+10];
vector<pii> updl;
...
int main()
{
	...
	while (t--)
	{
		cin>>n>>m;
		int cn=2*n-m-1;
		for (int i=0;i<m;++i) cin>>x[i]>>y[i],--y[i];
		upd(n,-1);
		for (int i=m-1;i>=0;--i)
		{
			int po=prvj(y[i]);
			//cout<<n<<' '<<m<<' '<<i<<' '<<y[i]<<' '<<po<<en;
			if (seg[po+M+1]==0) ++cn;
			upd(po,-1);
		}
		auto cu=updl;
		for (auto x: cu) upd(x.x,-x.y);
		updl.clear();
		cout<<mult(fac[cn],mult(inf[n],inf[cn-n]))<<en;
	}
}