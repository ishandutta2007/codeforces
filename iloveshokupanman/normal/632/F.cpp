...
int main() {
    ...
    vector<vector<P>> lis;
    rep(i, N* N) {
        if (i == 0 || c[i].first > c[i - 1].first) { lis.push_back({}); }
        lis.back().push_back(c[i].second);
    }
    
    
    vector<bitset<2500>> row(N);
   
    for (auto v: lis) {
       
        for (P pr : v) {        
            if (!(row[pr.first] & row[pr.second]).none()) {
                cout << "NOT MAGIC" << endl;
                return 0;
             }          
        }
