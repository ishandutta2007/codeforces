  ...
  for(ll i=0;i<n;i++){
    ...
    while(!que.empty()){
      ...
      for(auto to:g[p]){
        ...
        else if(t[p]==t[to])return vector<int>();
      }
    }
  }
  return t;
  ...
  for(ll i=0;i<n;i++){
    ...
    while(!que.empty()){
      ...
      for(auto to:g[p]){
        ...
        else if(t[p]==t[to])v.back().first=false;
      }
    }
  }
  return v;
...
int main(){
  ...
  cout<<"YES"<<endl;
  auto ret=vec(n,m,0LL);
  rep(i,0,n)rep(j,0,m){
    if(s[i][j]=='.'){
      ret[i][j]=1;
      if(t[i*m+j])ret[i][j]=4;
    }
  }
  rep(i,0,n)rep(j,0,m){
    if(s[i][j]=='X'){
      rep(o,0,4){
        ll ni=i+dx[o],nj=j+dy[o];
        if(s[ni][nj]!='X')ret[i][j]+=ret[ni][nj];
      }
    }
  }
  debug(ret,n,m);
