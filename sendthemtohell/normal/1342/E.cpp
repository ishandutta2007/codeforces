...
ll c(ll n, ll k) {
	return ll(fct[n]) * ifct[k] % FFT_MOD * ifct[n - k] % FFT_MOD;
}
...
inline void solve() {
	...
	ll ans = 0;//c(n, k);
	int sg = 1;
	for (int free = 0; free < tk; ++free) {
		ans += sg * ll(c(tk, free)) * pwm(tk - free, n, FFT_MOD) % FFT_MOD;
		ans = (ans % FFT_MOD + FFT_MOD) % FFT_MOD;
		sg = -sg;
	}
	cout << ll(ans) * 2 * c(n, k) % FFT_MOD << endl;
	/*
	int n, k;
	cin >> n >> k;
	...
	for (int s = k - 1; s >= 0; --s) {
		can += c[s];
		int ptr = 0;
		for (int i = 0; i < lol[s]; ++i) {
			while (sz(ans[ptr]) == can) {
				++ptr;
				assert(ptr != sz(ans));
			}
			ans[ptr].push_back(s + 1);
		}
	}
	for (int i = 0; i < r; ++i) {
		cout << sz(ans[i]) << " " << ans[i] << endl;
	}
	*/
}
