...
void q() {
  ...
  each(v, dat1) {
    ...
    if (c == -1) continue;
    if (uf.merge(a, b, popcnt(c) & 1) == false) die("NO");
  }
  each(v, dat2) {
    int a = v[0], b = v[1], c = v[2];
    if (c == -1) continue;
    trc(a,b,c);
    if (uf.merge(a, b, popcnt(c) & 1) == false) die("NO");
    trc(a,b,c,uf.diff(a,b));
  }

  rep(i, N) {
    if (!uf.same(0, i)) uf.merge(0, i, 0);
  }

  each(v, dat1) {
    
    int a = v[0], b = v[1], c = v[2];
    if (c == -1) v[2] = uf.diff(a, b);
    
    v[0]++, v[1]++;
  }

  out("YES");
  rep(i, N - 1) out(dat1[i]);
}
