...
vector <int> solve(int l, int r) {
    if(l == r) {
        vector <int> ans;
        ans.push_back(l - 1); ans.push_back(1);
        return ans;
    }
    int mid = (l + r) >> 1;
    vector <int> ansl = solve(l, mid);
    vector <int> ansr = solve(mid + 1, r);
    return pmul(ansl, ansr);
}
...
int main() {
    ...
    cout << mul(fac[a + b - 2], mul(inv[a - 1], mul(inv[b - 1], ans[a + b - 2]))) << endl;
    return 0;
}