...
db operator*(const pkt&a,const pkt&b){
	return db(a.x)*b.y-db(a.y)*b.x;
}

int suan(const pkt&a){
	return abs(gcd(a.x,a.y));
}
...
int main()
{
	...
	rep(i,1,n+1)p[i+n]=p[i];

	db su=0.0;
	rep(i,0,n+1){
		f[i]=pow(2,-i-1)-pow(2,-n);
	}
	rep(i,1,n+1)
		rep(j,i+1,i+n-1){
			if(j-i>100)break;
			db gong=0.5*(p[i]*p[j])-0.5*suan(p[j]-p[i]);
			su+=gong*f[j-i];
		}
	su/=(1-(1+n+0.5*n*(n-1))*pow(2,-n));
	su+=1.0;
