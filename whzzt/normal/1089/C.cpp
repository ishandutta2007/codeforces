...
const int N=505;
int n,m,k,p[N],d[N][N],cnt[N];
bool is[N];
char os[55];
vector<int>adj[N],cur;
int main(){
	int i,j,k,s,w,t,cw,cp,T;
	scanf("%d%d",&n,&m);
	for(i=1;i<=n;i++)for(j=1;j<=n;j++)if(i!=j)d[i][j]=n;
	for(i=1;i<=m;i++){
		scanf("%d",&k);
		for(j=1;j<=k;j++)scanf("%d",p+j);
		for(j=1;j<k;j++){
			d[p[j]][p[j+1]]=d[p[j+1]][p[j]]=1;
			adj[p[j]].pb(p[j+1]),adj[p[j+1]].pb(p[j]);
		}
	}
	for(k=1;k<=n;k++)for(i=1;i<=n;i++)for(j=1;j<=n;j++)chkmin(d[i][j],d[i][k]+d[k][j]);
	for(T=1;T<=n;T++){
	  cur.clear();
		...
		do{
			...
			for(k=1;k<=n;k++){
				...
				for(int o:adj[k]){
					s=0;
					for(int e:cur)if(d[o][e]<d[k][e])++s;
					chkmax(w,s);
				}
				if(chkmin(cw,w))cp=k;
			}
			...
			if(*os=='G'){
				scanf("%d",&t);
				vector<int>ne;
				for(int e:cur)if(d[t][e]<d[cp][e])ne.pb(e);
				cur.swap(ne);
			}
		}while(*os=='G');
	}
