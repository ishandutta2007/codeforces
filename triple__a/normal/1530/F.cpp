...
signed main(){
    ...
    cin.tie(0);
    cin>>n;
    rep1(i,mod-1) inv[i]=basics::power(i,mod-2,mod);
    rep(i,n) rep(j,n) cin>>a[i][j], a[i][j]=a[i][j]*inv[10000]%mod;
    int ret=0;
    rep(i,1<<n){
        if (!i) {
            rep(j,n) val[i][j]=1;
    ...
    rep(i,1<<n){
        rep(s,2){
            rep(t,2){
                auto solve=[&](int mask,int s,int t){
                    int sum=1;
                    rep(i,n) {
                        int ret=val[((1<<n)-1)^mask][i];
                        int bonus=val[mask][i];
                        bool ok=0;
                        if (s && ((mask>>i&1)^1)) {ret=ret*inv[a[i][i]]%mod; ok=1; bonus=bonus*a[i][i]%mod;}
                        if (t && ((mask>>(n-1-i)&1)^1) && (!ok || n-1-i!=i)) ret=ret*inv[a[n-1-i][i]]%mod, bonus=bonus*a[n-1-i][i]%mod; 
                        sum=sum*bonus*(mod+1-ret)%mod;
                    }
                    // cout<<mask<<" "<<s<<" "<<t<<" "<<sum<<endl;
                    return sum;
                };
                // cout<<i<<" "<<s<<" "<<t<<": ";
                if ((__builtin_popcount(i)^s^t)&1) ret-=solve(i,s,t);
                else ret+=solve(i,s,t);
            }
        }
    }
    ret=1-ret;
