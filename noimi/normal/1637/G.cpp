...
int main() {
    TEST {
        ...
        multiset<int> s;
        vpi ans;
        int N = 1 << (topbit(n - 1) + 1);
        rep(i, 1, n + 1) if(i != N) s.emplace(i);
        auto f = [&](int x, int y) {
            ans.eb(x, y);
            if(x != N) s.erase(s.lower_bound(x));
            if(y != N) s.erase(s.lower_bound(y));
            if(x + y != N) s.emplace(x + y);
            if(abs(x - y) != N) s.emplace(abs(x - y));
        };
        vi v;
        for(int t = N; t >= 2; t >>= 1) {
            int pre = t;
            while(!empty(s) and *s.rbegin() > t / 2) {
        ...
        while(!s.empty() and *s.rbegin()) {
            auto x = *s.rbegin();
            auto y = *next(s.rbegin());
            f(x, y);
            f(x + y, x - y);
        }
        rep(s.size()) { f(0, N); }
