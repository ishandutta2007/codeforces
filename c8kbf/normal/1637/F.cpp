...
inline void AC() {
    ...
    rt = 1;
    for(_ i = 2; i <= n; ++i) if(a[i] > a[rt]) rt = i;
    prc(rt, -1);
    _ s = 0;
    for(_ i : g[rt]) {
        _ cr = dfs(i, rt, 0);
        s += cr;
        h.push_back(dfs(i, rt, a[rt])-cr);
    }
    h.push_back(a[rt]);
    h.push_back(a[rt]);
    sort(h.begin(), h.end());
...
_ dfs(_ x, _ fa, _ br) {
    br = max(br, a[x]);
    _ cc = 0;
    for(_ i : g[x]) if(i != fa) ++cc;
    if(!cc) return br;
    v_ ga, gb;
    _ mn = -1E11, dx = -1;
    for(_ i = 0; i < g[x].size(); ++i) if(g[x][i] != fa && dp[g[x][i]] > mn) {
        mn = dp[g[x][i]];
        dx = i;
    }
    _ rv = 0;
    for(_ i : g[x]) if(i != fa && i != g[x][dx]) rv += dfs(i, x, 0);
    rv += dfs(g[x][dx], x, br);
    return rv;
}