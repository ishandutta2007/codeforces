...
int qand(int i, int j) {
    if (i>j) swap(i, j);
    if (a.find(mp(i, j))!=a.end()) return a[mp(i, j)];
...
int qor(int i, int j) {
    if (i>j) swap(i, j);
    if (b.find(mp(i, j))!=b.end()) return b[mp(i, j)];
...
int qxor(int i, int j) {
    return qor(i, j) & (~qand(i, j));
}

void solve() {
    cin >> n >> k;
    int all = qand(0, 1) & qand(0, 2), a01 = qand(0,1) ^ all, a02 = qand(0, 2) ^ all, a12 = qand(1, 2) ^ all;
    int un = qor(0, 1)|qor(0, 2), a0 = (un ^ qor(1, 2)) | all | a01 | a02;
    nums.pb(a0);
    FOR(i, 1, n) nums.pb(qxor(0, i) ^ nums[0]);
    sort(all(nums));
