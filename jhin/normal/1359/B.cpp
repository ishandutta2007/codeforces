        ...
        long long m,n,x,y,z,k,mn=3e9,mx,ans,b[500003];vector<long long>v;char ch;
        ...
        while(t--)
        {
            cin>>n>>m>>x>>y;string st[n+2],ss;
            ans=0;
            for(int i=1;i<=n;i++){cin>>st[i];st[i]='*'+st[i]+'*';}
            for(int i=0;i<m+2;i++)ss+='*';
            st[0]=ss;st[n+1]=ss;
            for(int i=1;i<=n;i++)
            {
                ...
                for(int j=1;j<m+1;j++)
                {
                    ...
                    else if(st[i][j]=='.'&&st[i][j+1]=='.'&&2*x>y)ans+=y,j++;
                    else if(st[i][j]=='.'&&st[i][j+1]=='*'&&2*x>y)ans+=x,j++;
