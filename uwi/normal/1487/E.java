...
public class E {
	...
	void solve()
	{
		...
		for(int i = 0;i < 3;i++){
			...
			int[][] g = packD(ns[i+1], from, to);
			conds[i] = g;
		}

		int[] dp = Arrays.copyOf(a[0], ns[0]);
		for(int i = 1;i < 4;i++){
			SegmentTreeRMQ st = new SegmentTreeRMQ(dp);
			int[] ndp = new int[ns[i]];
			for(int j = 0;j < ns[i];j++){
				for(int e : conds[i-1][j]){
					st.update(e, Integer.MAX_VALUE/2);
				}
				ndp[j] = st.vals[1] + a[i][j];
				for(int e : conds[i-1][j]){
					st.update(e, dp[e]);
				}
			}
