...
bool hassub(string text, string pattern) {
    int ptr = -1;
    for (auto c : pattern) {
        ++ptr;
        while (ptr < sz(text) && text[ptr] != c) {
            ++ptr;
        }
    }
    return ptr < sz(text);
...
node merge(const node &a, const node &b) {
    ...
    for (auto tp : poss) {
        ...
        auto tb = tp.f.s;
        auto tab = tp.s;
        chmin(ans.val[tab], a.val[ta] + b.val[tb]);
    }
    return ans;
}
...
void rec(vector<string> &s, string curr, int maxd) {
    if (sz(curr) > 0) {
        s.pb(curr);
    }
    if (sz(curr) == maxd) {
...
signed main() {
    ...
    dbg(gettype("bacb"));
    dbg(gettype("aaab"));
    dbg(t.getup(0, 4).val[gettype("bacb")]);
    dbg(t.getup(4, 8).val[gettype("aaab")]);
    dbg(t.getup(0, 8).val[gettype("bacbaaab")]);
    for (int i = 0; i < q; ++i) {
        int x;
        char c;
        read(x, c);
