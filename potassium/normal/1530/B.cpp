...
int main() {
	CASE(t) {
		...
		vector<pii> order;
		for (int j = 0; j < w; j++) order.push_back({ 0, j });
		for (int i = 0; i < h; i++) order.push_back({ i, w - 1 });
		for (int j = w - 1; j >= 0; j--) order.push_back({ h - 1, j });
		for (int i = h - 1; i >= 0; i--) order.push_back({ i, 0 });
