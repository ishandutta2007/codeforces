...
struct Undo_Union_Find_Tree{
    ...
    Undo_Union_Find_Tree(int n) : n(n){
        data.assign(n, -1);
    }
...
int main(){
    ...
    VEC(int, c, N);
    each(e, c) e--;
    Undo_Union_Find_Tree uf(2*N);
    vector<bool> ok(K, true);
    map<pii, vector<pii>> mp;
    rep(i, M){
        INT(u, v); u--, v--;
        if(c[u] == c[v]){
            uf.unite(u, N+v), uf.unite(N+u, v);
            if(uf.same(u, v)) ok[c[u]] = false;
        }
        else{
            mp[minmax(c[u], c[v])].eb(u, v);
        }
    }
    ll n = 0;
    rep(i, K) n += ok[i];
    ...
    each(e, mp){
        ...
        each(u, e.second){
            auto [p, q] = u;
            uf.unite(p, N+q), uf.unite(N+p, q);
            if(uf.same(p, q)) flag = true;
        }
        if(flag) ans--;
        uf.undo(2*sz(e.second));
    }
    out(ans);
}