...
// Problem: D: Memory and Scores (https://codeforces.com/contest/712/problem/D)
...
int main() {
    ...
    const int ZERO = k * t + 5;
    vMI dp(2 * k * t + 10), dpp(sz(dp));
    dpp[ZERO] = 1;
    rep(_, t) {
        reps(i, 1, sz(dpp))
            dpp[i] += dpp[i-1];
        rep(i, sz(dp))
            dp[i] = dpp[min(sz(dp) - 1, i + k)] - (i - k - 1 >= 0 ? dpp[i - k - 1] : 0_mi);
        swap(dpp, dp);
    }
    ...
    rep(i, sz(dpp)) {
        int x = i + a - b;
        if (x > 0)
            sum += dpp[i] * pref[min(sz(pref) - 1, x - 1)];
    }
    cout << sum << '\n';
