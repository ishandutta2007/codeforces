...
int main() {
    ...
    if (n & 1) {
        ...
        rep(v, M) {
            if (cnt[v] == 0) continue;
            vs[cnt[v] % 4].push_back(v);
        }
        if (vs[1].size() + vs[3].size() != 1) {
            print("NO");
            return 0;
        }
        if (vs[1].size()) {
        ...
        if (int(vs[2].size()) > n - 1) {
            print("NO");
            return 0;
        }
        int rem = n - 1 - int(vs[2].size());
        assert(rem % 2 == 0);
        loop(rem / 2) {
            int v = vs[0].back();
            if (cnt[v] == 0) {
        ...
        rep(i, n / 2) {
            ans[c][i] = ans[c][n - i - 1] = vs[2][i];
            if (i < n - 1 - rem) vs[0].push_back(vs[2][i]);
        }
        rep(i, n / 2) {
            ans[i][c] = ans[n - i - 1][c] = vs[2][n / 2 + i];
            if (n / 2 + i < n - 1 - rem) vs[0].push_back(vs[2][n / 2 + i]);
        }
        int rc = 0;
        for (int v : vs[0]) {
            loop(cnt[v] / 4) {
                int r = rc / (n / 2), c = rc % (n / 2);
                ans[r][c] = ans[r][n - c - 1] = ans[n - r - 1][c] = ans[n - r - 1][n - c - 1] = v;
                ++rc;
            }
        }
        print("YES");
