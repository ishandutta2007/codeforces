...
int main() {
	...
	if (n % 2) {
		printf("YES\n");
		vector <int> x;
		REP(i, n) x.push_back(2 * i + i % 2 + 1);
		REP(i, n) x.push_back(2 * i + 1 - i % 2 + 1);
	...
	loc({
		vector <int> p;
		for (int i = 1; i < 2 * n; i += 2) p.push_back(i);
		PER(ALL(p)) {
			REP(m, (1 << n)) {
				vector <int> r;
				REP(i, n) r.push_back(p[i] + (m >> i & 1));
				REP(i, n) r.push_back(p[i] + 1 - (m >> i & 1));
				vector <int> sums;
				REP(i, 2 * n) {
					int s = 0;
					REP(j, n) s += r[(i + j) % (2 * n)];
