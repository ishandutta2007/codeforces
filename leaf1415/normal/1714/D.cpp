...
int main(void)
{
	...
	while(T--){
		...
		cin >> n;
		rep(i, 1, n) cin >> s[i];
		rep(i, 1, m) rep(j, 1, m) cover[i][j] = 0;
		
		rep(i, 1, m){
			rep(j, 1, n){
				ll r = i+sz(s[j])-1;
				if(r > m) continue;
				if(t.substr(i, sz(s[j])) == s[j]) cover[i][r] = j;
			}
		}
		
		vector<P> ans; ll pos = 0;
		while(pos < m){
			ll maxI = 0, maxJ = 0;
			rep(i, 1, m) rep(j, 1, m){
				if(i > j || pos+1 < i || j < pos+1 || cover[i][j] == 0) continue;
				if(maxJ < j) maxI = i, maxJ = j;
			}
			if(maxJ == 0){
				ans.clear();
				break;
			}
			ans.pb(P(cover[maxI][maxJ], maxI));
			pos = maxJ;
		}
