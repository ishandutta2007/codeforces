...
ll modncr(const ll n,const ll r,const ll m) noexcept{
  if(n<r)return 0;
  ...
  if(size<=n){
    ...
    For(i,size,n+1)modncrlistp[i]=modncrlistp[i-1]*i%m;
    modncrlistm[n]=modinv(modncrlistp[n],m);
    for(i=n;i>size;--i)modncrlistm[i-1]=modncrlistm[i]*i%m;
  }
  return modncrlistp[n]*modncrlistm[r]%m*modncrlistm[n-r]%m;
}
...
int main(){
  ...
  vec pow2(N+1);
  pow2[0]=1;
  rep(i,N)pow2[i+1]=(pow2[i]+pow2[i])%M;
  rep(i,N+1){
    dp[i].resize(i+1,0);
    if(i)dp[i][i]=pow2[i-1];
  }
  modncr(401,0,M);
  For(i,3,N+1){
    rep1(j,i-1){
      //j++i-j-1
      rep(k,j+1){
        (dp[i][k+i-j-1]+=dp[j][k]*modncr(k+i-j-1,k,M)%M*pow2[i-j-2]%M)%=M;
      }
    }
  }
  // debug(dp);
  print(accumulate(all(dp[N]),0LL)%M);
}