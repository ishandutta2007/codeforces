...
void solve(){
  ...
  while(n != 1) {
    ...
    int j = distance(begin(A),find(begin(A), end(A),n-1));
    if(j%2 != 1) {
      cout << -1 << endl;
      return;
    }
    ans.emplace_back(j/2+1);
    reverse(begin(A),begin(A)+j);

    ans.emplace_back(n/2+1);
    reverse(begin(A),begin(A)+n);

    i = distance(begin(A),find(begin(A), end(A),n));
    ans.emplace_back(i/2+1);
    reverse(begin(A),begin(A)+i+1);
