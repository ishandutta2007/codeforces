...
/////////////////////////////////////////////////////////////////////////////////////////
template<class S>
pair<S,S> ADD(pair<S,S>a,pair<S,S>b){
    a.first += b.first;
    a.second += b.second;
    return a;
}
...
signed main(){fastio
    ...
    rep(len,2,n){
        rep(l,0,n-1){
            ...
            if(r>n)break;
            if(s[r-1]!=')'){
                if(r-1>=0)dp[l][r] += dp[l][r-1];
            }
            if(s[l]!='('){
                if(l+1<=n-1)dp[l][r] += dp[l+1][r];
            }
            if(s[r-1]!=')' && s[l]!='('){
                if(l+1<=n-1 && r-1>=0)dp[l][r] -= dp[l+1][r-1];
            }
            if(s[l] != ')' && s[r-1] !='('){
                ll cnt=0;
               // rep(i,l+1,r-2)if(s[i]=='?')cnt++;
                cnt = COUNT[r-1] - COUNT[l+1];
                if(l+1<=n-1 && r-1>=0)dp[l][r] += dp[l+1][r-1];
                dp[l][r] += mint(2).pow(cnt);
            }
        }
    }
    /*
