...
void solve()
{
	...
	if (!zero.empty()) freshmax(res, 0LL);

	for (int k = 0; k <= 5; k++)
	{
		if (neg.size() >= k && pos.size() >= 5 - k)
		{
			ll neg1 = 1, neg2 = 1;
			for (int i = 0; i < k; ++i) neg1 *= neg[i], neg2 *= neg[neg.size() - i - 1];
			ll pos1 = 1, pos2 = 1;
			for (int i = 0; i < 5 - k; ++i) pos1 *= pos[i], pos2 *= pos[pos.size() - i - 1];
			freshmax(res, neg1 * pos1);
			freshmax(res, neg1 * pos2);
			freshmax(res, neg2 * pos1);
			freshmax(res, neg2 * pos2);
		}
	}
	writeln(res);
}
