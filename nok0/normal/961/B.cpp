...
void main_() {
	...
	VEC(int, a, n);
	VEC(int, t, n);
	V<ll> allcum(n + 1), ncum(n + 1);
	REP(i, n) {
		allcum[i + 1] = allcum[i] + a[i];
		ncum[i + 1] = ncum[i] + (t[i] ? a[i] : 0);
	}
	REP(i, n - k + 1) {
		chmax(res, ncum[i] + allcum[i + k] - allcum[i] + ncum[n] - ncum[i + k]);
	}
	print(res);
}
