...
void upd(int cur, int l, int r, int pos, ll val) {
    if (l == r) {
        tr[cur] = val;
    } else {
        int m = (l + r) / 2;
...
ll getMin(int cur, int l, int r, int x, int y) {
    if (x > r || y < l) return LLINF;
    if (x <= l && y >= r) return tr[cur];
    int m = (l + r) / 2;
    int dcur = cur + cur;
...
ll solve() {
    ...
    ll ans = 0;
    for (int i = n - 2; i >= 0; --i) {
        ll res = getMin(1, 0, n - 1, i + 1, a[i] - 1) + (n - 1 - a[i]) - i + 1;
//        DBN(i, res);
        upd(1, 0, n - 1, i, res + i);
        ans += res;
    }
