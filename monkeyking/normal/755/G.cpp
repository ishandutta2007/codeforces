...
//	for(int i=0;i<(int)b.size();i++) chadd(a[i],b[i]);
//}
//
//map<int,vector<int> > ans;
//
//inline vector<int> calc(int x)
//{
//	if(ans.count(x)) return ans[x];
//	if(x==0) return ans[x]={1};
//	if(x==1) return ans[x]={1,1};
//	vector<int> &v=ans[x];
//	if(ans.count(x-1) && ans.count(x-2))
//	{
//		add(v,mult(ans[x-1],{1,1}));
//		add(v,mult(ans[x-2],{0,1}));
//		return v;
//	}
//	if(x%2==0)
//	{
//		vector<int> v1=calc(x/2-1),v2=calc(x/2);
//		ntt.sqr(v1);ntt.sqr(v2);
//		if(v2.size()>k+1) v2.resize(k+1,0);
//		add(v,mult(v1,{0,1}));add(v,v2);
//		return v;
//	}
//	else
//	{
//		vector<int> v1=calc(x/2-1),v2=calc(x/2),v3=calc(x/2+1);
//		add(v,mult(mult(v1,v2),{0,1}));
//		add(v,mult(v2,v3));
//		return v;
//	}
//}
