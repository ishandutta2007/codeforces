...
const int SQ=334;
const int BK=112345/SQ;

unordered_map<int,mint> mp[BK];
array<int,BK> ad;
array<mint,BK> sum;
...
int main(){
  ...
  int n,t;
	...
	auto addBK=
		[&](int i){
			++ad[i];
			if(mp[i].count(t+1-ad[i])) sum[i]-=mp[i][t+1-ad[i]];
			// rep(j,SQ) ++num[i*SQ+j];
		};
	auto subBK=
		[&](int i){
			--ad[i];
			if(mp[i].count(t-ad[i])) sum[i]+=mp[i][t-ad[i]];
			// rep(j,SQ) --num[i*SQ+j];
		};
	auto range_add=
		[&](int l,int r,int d){
			for(int i=l;i<r;++i){
				if(i%SQ==0 && i+SQ<r){
					...
					mp[i/SQ][num[i]]-=dp[i];
					if(num[i]+ad[i/SQ]<=t) sum[i/SQ]-=dp[i];
					num[i]+=d;
					mp[i/SQ][num[i]]+=dp[i];
					if(num[i]+ad[i/SQ]<=t) sum[i/SQ]+=dp[i];
				}
	...
	rep(i,n){
		...
		range_add(prv[i]+1,i+1,1);
		if(prv[i]>=0) range_add(prv[prv[i]]+1,prv[i]+1,-1);
		rep(j,(i+1)/SQ) dp[i+1]+=sum[j];
		reps(j,(i+1)/SQ*SQ,i+1)if(num[j]<=t) dp[i+1]+=dp[j];
		out(dp,num,1);
		// rep(i,3) out(i,ad[i],1);
	}
	cout<<dp[n]<<endl;
  return 0;
}