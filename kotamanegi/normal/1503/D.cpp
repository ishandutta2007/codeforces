...
void solve() {
	...
	int cnt = 0;
	int now_point = 0;
	pair<int, int> now = mp(1e9, 1e9);
	for(int i = 0;i < n;++i) {
		int choice = 0;
		if (now.first < get<1>(inputs[i]) and now.second < get<1>(inputs[i])) {
			cout << "-1" << endl;
			return;
		}
		if (now.first < get<1>(inputs[i])) {
			choice = 1;
		}
		else if (now.second < get<1>(inputs[i])) {
			choice = 0;
		}
		else {
			int hoge = min(now.first,now.second);
			if (i == 0 or 2*n - hoge + 1 == i) {
				choice = get<2>(inputs[i]);
				ans += min(now_point, cnt - now_point);
				now_point = 0;
				cnt = 0;
			}
			else {
		...
		if (get<2>(inputs[i]) != choice) now_point++;
		if (choice == 0) {
			now.first = get<1>(inputs[i]);
		}
		else {
			now.second = get<1>(inputs[i]);
		}
		cnt++;
	}
	ans += min(now_point, cnt - now_point);
