...
void solve(){
  ...
  vi IB(n); iota(begin(IB), end(IB),0); sort(begin(IB), end(IB), [&](int i, int j){ return B[i]<B[j]; });

  vvi G(n);
  for(li i = 0; i < (li)(n-1); ++i) G[IA[i+1]].emplace_back(IA[i]);
  for(li i = 0; i < (li)(n-1); ++i) G[IB[i+1]].emplace_back(IB[i]);

  vvi cs; vi C;
  scc(G,cs,C);

  int w = C[IA[n-1]];
  for(li i = 0; i < (li)(n); ++i) cout << (C[i]==w?1:0);
  cout << endl;
}
