...
// }
ll ans[17];
void solve(ll L,ll R,vec v,int nowbit){
  if(v.empty()||nowbit<0)return;
  ll nowval=1LL<<nowbit;
  vec ch[2];
  for(auto x:v)ch[x>>nowbit&1].emplace_back(x&(nowval-1));
  if(~L&R&nowval){
    if(nowval-L==R-nowval+1){
      solve(0,R^nowval,ch[1],nowbit-1);
      return;
    }
    if(ll(ch[0].size())==nowval-L){
      assert(ans[nowbit]!=2);
      ans[nowbit]=1;
      solve(L,nowval-1,ch[0],nowbit-1);
      solve(0,R^nowval,ch[1],nowbit-1);
    }else if(ll(ch[1].size())==nowval-L){
      assert(ans[nowbit]!=1);
      ans[nowbit]=2;
      solve(L,nowval-1,ch[1],nowbit-1);
      solve(0,R^nowval,ch[0],nowbit-1);
    }
  }else if(R<nowval){
    if(ch[0].size()){
      assert(ans[nowbit]!=2);
      ans[nowbit]=1;
      solve(L,R,ch[0],nowbit-1);
    }else{
      assert(ans[nowbit]!=1);
      ans[nowbit]=2;
      solve(L,R,ch[1],nowbit-1);
    }
  }else{
    assert(nowval<=L);
    if(ch[0].size()){
      assert(ans[nowbit]!=1);
      ans[nowbit]=2;
      solve(L^nowval,R^nowval,ch[0],nowbit-1);
    }else{
      assert(ans[nowbit]!=2);
      ans[nowbit]=1;
      solve(L^nowval,R^nowval,ch[1],nowbit-1);
    }
  }
}
bool judge(ll L,ll R,vec v,ll pr){
  for(auto x:v){
    x^=pr;
...
int codeforces;cin>>codeforces;while(codeforces--){
  ll L,R;
  cin>>L>>R;
  // L=rand()%100;R=L+rand()%100;
  // ll ret=rand()%3000;
  vec v(R-L+1);
  // for(int i=L;i<=R;++i)v[i-L]=i^ret;
  for(auto& x:v)cin>>x;
