...
template <typename T> struct RmQ : SegmentTree<T> {
    RmQ(int n)
        : SegmentTree<T>(
              n, [](T i, T j) { return min(i, j); }, (ll)(1e15)) {}
    RmQ(vector<T> &a)
        : SegmentTree<T>(
              a, [](T i, T j) { return min(i, j); }, (ll)(1e15)) {}
};
...
int main() {
    TEST {
        ...
        rep(i, n - 1) {
            ...
            REV(X), REV(Y);
            RmQ<ll> seg(Y), seg2(si(Y));
            rep(i, si(Y)) seg2.set(i, Y[i] + X[i]);
            // dump(X, Y);
            seg2.build();
            ...
            dp[i + 1][r.begin()->fi] = mi + (a[i + 1] - r.begin()->fi);
            rep2(k, L, R - 1) {
                r[v[k].se]--;
                if(r[v[k].se] == 0) r.erase(v[k].se);

                // 
                ll t = (k == R - 1 ? a[i + 1] : r.begin()->fi);

                int s = lb(X, v[k].fi - a[i]);
                ll cost = (v[k].fi - a[i]) + min(seg2.query(0, s), seg.query(s, si(X)) + (v[k].fi - a[i]));
                cost += (a[i + 1] - t);
                if(dp[i + 1].count(t))
                    chmin(dp[i + 1][t], cost);
                else
        ...
        // rep(i, 3) for(auto [x, y] : dp[i]) { dump(x, y); }
        ll mi = inf<ll>;
        for(auto [x, y] : dp.back()) chmin(mi, y);
        OUT(mi);
    }
}