...
int main(int argc, char **argv){
  ...
  int r=0;
  vector<int> C(n);
  auto ask=[&](vector<pii> A){
             vector<int> V;
             for(pii p:A) for(int i=p.X;i<p.X+p.Y;++i) {
                 C[i%n]^=1;
                 V.pb(i%n);
               }
             assert(sz(V)==k);
             ...
             cout<<"?";
             for(pii p:A) for(int i=p.X;i<p.X+p.Y;++i) cout<<' '<<i%n+1;
             cout<<endl;
  ...
  if(!w){
    assert(k*2>n);
    assert(n%2==0);
    for(int b=0;b<n;b+=(n-k)*2){
      int d=min((n-b)/2,n-k);
      ask({{b+0,d},{0,min(b,k-d)},{b+d*2,k-d-min(b,k-d)}});
      ask({{b+d,d},{0,min(b,k-d)},{b+d*2,k-d-min(b,k-d)}});
    }
  }else{
    if(k%2==0 && n%k && w>1) --w;
    int v=n-k*w;
    ...
    if(v){
      ask({{n-v  ,v/2},{0,k-v/2}});
      ask({{n-v/2,v/2},{0,k-v/2}});
    }
  }
  assert(accumulate(all(C),0)==n);
  cout<<"! "<<r;
