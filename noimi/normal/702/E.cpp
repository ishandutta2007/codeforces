...
signed main(){
    ...
    VEC(ll,w,n);
    using P = pair<ll,ll> ;
    vector<vector<P>> dp(40,vector<P>(n));
    vector<vec> nxt(40,vector<int>(n));
    rep(i,n){
        dp[0][i] = {w[i],w[i]};
        nxt[0][i] = f[i];
    }
    rep2(i,1,39){
        rep(j,n){
            auto p = dp[i-1][nxt[i-1][j]];
            dp[i][j] = P(dp[i-1][j].fi + p.fi, min(dp[i-1][j].se,p.se));
