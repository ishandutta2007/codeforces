...
int main(int argc, char **argv){
  ...
  while(sz(C)&(sz(C)-1)) C.pb(0);

  const LL mod=998244353;
  vector<vector<LL>> M(sz(C),vector<LL>(sz(C)+1));
  LL inp1=inv(n+1,mod);
  forv(x,C){
    M[x][x]=mod-1;
    if(x) M[x].back()=mod-inp1;
    forv(i,C) (M[x][x^i]+=inp1*C[i])%=mod;
  }
  forv(i,M){
    assert(M[i][i]);
    LL in=inv(M[i][i],mod);
    forv(j,M[i]) (M[i][j]*=in)%=mod;
    forv(k,M) if(k!=i) {
      LL c=mod-M[k][i];
      forv(j,M[k]) (M[k][j]+=M[i][j]*c)%=mod;
    }
  }
  dbg(M);
  cout<<M[0].back();
