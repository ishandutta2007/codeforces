...
class registry<T>(val st:MutableMap<T,Int> = mutableMapOf()):MutableMap<T,Int> by st{
    fun register(a:T):Int {
        val new = st[a]
        if(new == null){
            val ret = st.size
            st[a] = ret
...
//        return st.computeIfAbsent(a){st.size}
    }
}
...
class DisjointUnionSets(val n: Int) {
    ...
    fun union(x: Int, y: Int):Boolean {
        ...
        edgecount[xRoot] += edgecount[yRoot]
        edgecount[xRoot] ++
        return true
    }
...
class rational(xsus:Long, ysus:Long){
    var x :Long
    var y :Long
    init{
        val g  = gcd(xsus,ysus)
    ...
    fun addone():rational{
        return rational(x + y, y )
    }
    fun divide(other:rational):rational{
        return rational(this.x  * other.y, this.y * other.x)
    }
...
fun Graph.twomatch():MutableList<Pair<Int, Int>> {
    val taken = BooleanArray(this.edgecount shr 1)
    ...
    this.leafFirst {
        ...
        NS_E(v){e,w ->
            ...
            if(e shr 1 == parentEdge[v]) return@NS_E
            if(now.size >= 1 ){
                val a = e shr 1
                val b = now.removeLast() shr 1
                taken[a] = true
                taken[b] = true
                ret.add(Pair(a,b))
            }else{
...
fun main(){
    solve.cases{
        ...
        repeat(q){
            ...
            val c1 = (q1.addone().divide(q2))
            val c2 = (q1.divide(q2.addone()))
            val v1 = reg.register(c1)
            val v2= reg.register(c2)
            edge.add(Pair(v1,v2))
        }
        val G = Graph(reg.size + 1,edge.size + reg.size + 1, false)

        val dsu  = DisjointUnionSets(reg.size)
        for(e in edge){
        ...
        fun usableedge(e:Int):Boolean {
            return G.from[e shl 1 ] != reg.size && G.to[e shl  1] != reg.size
        }
        G.root = reg.size
        G.treeOrderDFS(true)
        val fact = G.twomatch().filter { usableedge(it.first) && usableedge(it.second) }
        put(fact.size)
        for((a,b) in fact){
            put("${a + 1 } ${b+1}")
        }
