...
int main() {
	const int MAXK = 2000;
	vector<vector<int>> s(MAXK + 1, vector<int>(MAXK + 1, 0));
	s[0][0] = 1;
	for (int n = 1; n <= MAXK; n++)
		for (int k = 1; k <= MAXK; k++)
			s[n][k] = (1LL * s[n - 1][k] * k + s[n - 1][k - 1]) % MOD998;

	CASE(t) {
		int n,m,k; R(n,m,k);

		int p = 1LL * (m + 1) / 2 * binpow(m, MOD998 - 2, MOD998) % MOD998;
		vector<int> pw(min(n, k) + 1, 1);
		for (int i = 1; i <= min(n, k); i++)
			pw[i] = 1LL * pw[i - 1] * p % MOD998;

		int ans = 0;
		int falling = 1;
		for (int j = 1; j <= min(n, k); j++) {
			falling = 1LL * falling * (n - j + 1) % MOD998;
			ans += 1LL * s[k][j] * falling % MOD998 * pw[j] % MOD998;
			ans %= MOD998;
		}

		ans = 1LL * ans * binpow(m, n, MOD998) % MOD998;
		ans %= MOD998;
		ans += MOD998;
		ans %= MOD998;
