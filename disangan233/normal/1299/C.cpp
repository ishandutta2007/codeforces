...
const int N=1e6+5;
int n,a[N],l[N],r[N],k=1;ll s[N];db b[N];
int main()
{
	n=read();for(int i=1;i<=n;i++) a[i]=read();
	l[0]=n+1;l[1]=r[1]=n;s[1]=a[n];
	for(int i=n-1;i;i--)
	{
		int tot=1;ll sum=a[i];
		for(;k&&s[k]*tot<=sum*(r[k]-l[k]+1);k--) sum+=s[k],tot+=r[k]-l[k]+1;
		k++;r[k]=l[k-1]-1;l[k]=i;s[k]=sum;
	}
	for(int i=1;i<=k;i++) for(int j=l[i];j<=r[i];j++) b[j]=(long db)s[i]/(r[i]-l[i]+1);
