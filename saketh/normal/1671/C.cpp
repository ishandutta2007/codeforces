...
void solve() {
    ...
    int pref = N;
    int64_t sum = accumulate(all(a), int64_t(0));

    while (true) {
        while (pref > 0 && sum + day * pref > X) {
            pref--;
            sum -= a[pref];
        }
        ...
        int64_t rep = (X - (sum + day * pref)) / pref + 1;

        assert(rep > 0);
        assert(sum + (day + rep - 1) * pref <= X);
        assert(sum + (day + rep) * pref > X);
