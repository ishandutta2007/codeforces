...
void solve()
{
	...
	ValueType value_init = { 0, 0, 0, 0, 0, 0 };
	auto value_merge = [](const ValueType& a, const ValueType& b) -> ValueType
	{
		return ValueType{ a.as + b.as, a.asd + b.asd, a.a + b.a, a.s + b.s, a.c + b.c };
	};
	...
	{
		value.as += value.a * tag.ds;
		value.s += value.c * tag.ds;
		value.asd += value.a * tag.dsd;
		value.sd += value.c * tag.dsd;
	};

	int maxn = 200000;
	vector<ValueType> init(maxn + 1);
	for (int i = 0; i <= maxn; ++i)
		init[i] = { 0, 0, 0, 0, 0, 1 };

	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		tree(init.begin(), init.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);
	...
	while (q--)
	{
		...
		{
			H.insert(x);
			if (int L = max(x - d, 1), R = x - 1; L <= R)
			{
				ll cnt = tree.Query(L, R + 1).a;
				res += (cnt - 1) * cnt / 2;
			}
			if (int L = x + 1, R = min(x + d, maxn); L <= R)
			{
				ll cnt = tree.Query(L, R + 1).a;
				res += (cnt - 1) * cnt / 2;
			}
			auto [as, asd, a, s, sd, c] = tree.Query(x);
			tree.ModifyValue(x, { s, sd, 1, s, sd, c });
			tree.ModifyTag(x, maxn + 1, { 1, 0 });
			tree.ModifyTag(max(x - d, 1), maxn + 1, { 0, 1 });
			if (int L = max(x - d + 1, 1), R = x - 1; L <= R)
			{
				auto [as, asd, a, s, sd, c] = tree.Query(L, R + 1);
				ll sj = tree.Query(x).s;
				res += asd - a * sj;
			}
		}
		else
		{
			...
			auto [as, asd, a, s, sd, c] = tree.Query(x);
			tree.ModifyValue(x, { 0, 0, 0, s, sd, c });
			tree.ModifyTag(x, maxn + 1, { -1, 0 });
			tree.ModifyTag(max(x - d, 1), maxn + 1, { 0, -1 });
			if (int L = max(x - d, 1), R = x - 1; L <= R)
			{
