...
mint a[100005], b[100005];

void moebius(mint b[], ll n){
	for(auto p : pvec){
		if(p > n) break;
		rep(i, 1, n/p) b[i] -= b[i*p];
	}
}
...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) for(int j = i; j <= n; j += i) vec[j].pb(i);

	for(auto d : vec[n]){
		ll m = n/d;
		rep(i, 1, m){
			ll h = m/i;
			b[i] = mint(arith(h-1))*mint(m) - mint(i)*(mint(h*(h+1)*(2*h+1)/6) - arith(h));
		}
		moebius(b, m);
		rep(i, 1, m) a[d] += b[i] * mint(i) * mint(d*d);
	}

	moebius(a, n);

	mint ans = 0;
	for(auto d : vec[n]) ans += a[d] / mint(d);
	outl(ans);
