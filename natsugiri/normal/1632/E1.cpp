...
const int MAXN = 3011;
const int MAXLOGN = 13; // == __lg(MAXN) + 2;
...
void MAIN() {
    ...
    int ma = 0;
    REP (v, N) amax(ma, depth(v));
    REP (i, N+1) ans[i] = ma;

    REP (k, N) {
	REP (i, N+1) X0[i] = X1[i] = 0;
	...
	REP (i, N+1) amax(X0[i+1], X0[i]);
	for (int i=N-1; i>=0; i--) amax(X1[i], X1[i+1] - 1);
	REP (i, N+1) amin(ans[i], max(X0[i], X1[i]));
    }
