    ...
    /*************************************/
    string s,ans[209];
    ll dp[209][209][209],n,a,b,k;
    int Dp(int i,int num,int taken){
        if(i==n){
            return num==k-1 && taken>=a && taken<=b;
        }
        ll &ret=dp[i][num][taken];
        if(ret+1) return ret;
        ret=0;
        if(taken<a){
            ret |= Dp(i+1,num,taken+1);
        }
        if(taken>=a && taken<=b){
            ret |= Dp(i+1,num,taken+1);
            ret |= Dp(i,num+1,0);
    ...
    void path(int i,int num,int taken){
        if(i==n){
            return ;
        }
        if(taken<a){
            cout << s[i];
            path(i+1,num,taken+1);
        }
        else
        {
            if(taken>=a && taken<=b){
                if(Dp(i+1,num,taken+1)){
                    cout << s[i];
                    path(i+1,num,taken+1);
                }
                else{
    ...
    int main()
    {
        ...
        if(Dp(0,0,0)==0) return cout << "No solution" << endl,0;
        path(0,0,0);
        return 0;
    }