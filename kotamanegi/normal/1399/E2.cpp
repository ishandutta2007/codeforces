...
priority_queue<tuple<int, int, int>> nexts[2];
vector<tuple<int,int,int>> vertexs[100010];
int dist[100010];
int ans = 0;
int dfs(int now, int back) {
    int cnter = 0;
    for (auto& x : vertexs[now]) {
        if (get<0>(x) == back) continue;
        dist[get<0>(x)] = dist[now] + get<1>(x);
        int hoge = dfs(get<0>(x), now);
        cnter += hoge;
        nexts[get<2>(x)].push(make_tuple(get<1>(x) * hoge - (get<1>(x) / 2LL) * hoge, get<1>(x), hoge));
    }
    if (cnter == 0) ans += dist[now],cnter=1;
    return cnter;
}
void solve(){
    int n, S;
    ...
    REP(q,2)
    while (!nexts[q].empty()) nexts[q].pop();
    REP(i, n - 1) {
        int a, b, c,d;
        cin >> a >> b >> c >> d;
        a--; b--; d--;
    ...
    REP(q, 2) {
        go[q].push_back(0);
        while (nexts[q].empty() == false) {
            tuple<int, int, int> now = nexts[q].top();
            nexts[q].pop();
            go[q].push_back(get<0>(now) + go[q].back());
            get<1>(now) /= 2;
            if (get<1>(now) != 0) {
                nexts[q].push(make_tuple((get<1>(now) - get<1>(now) / 2) * get<2>(now), get<1>(now), get<2>(now)));
            }
        }
    }
    ans = 1e18;
    int back = go[1].size()-1;
    REP(i, go[0].size()) {
        while (back != 0 and go[1][back - 1] + go[0][i] + S >= 0) {
            back--;
        }
        if (go[1][back] + go[0][i] + S >= 0) {
            ans = min(ans, i + back * 2LL);
        }
    }
    cout << ans << "\n";
}
