...
vector<int> pl, pr;
int cmpL(int pos) {
    int f = pl[l.size() + 1 + pos];
    if (f >= l.size()) {
        return 0;
    }
    assert(a[pos + f] != l[f]);
    return a[pos + f] < l[f] ? -1 : 1;
}
int cmpR(int pos) {
    int f = pr[r.size() + 1 + pos];
    if (f >= r.size()) {
        return 0;
    }
    assert(a[pos + f] != r[f]);
    return a[pos + f] < r[f] ? -1 : 1;
}
int prefF[1010101];
...
int solve() {
    ...
    for (int i = 1; i <= as; ++i) {
        if (i >= ls) {
            if (ls == rs) {
                if (cmpL(i - ls) >= 0 && cmpR(i - ls) <= 0) {
                    addmodfast(dp[i], dp[i - ls], fftmod);
                }
            } else {
                addmodfast(dp[i], getSum(i - (rs - 1), i - (ls + 1)), fftmod);
                if (cmpL(i - ls) >= 0) {
                   addmodfast(dp[i], dp[i - ls], fftmod);
                }
                if (i >= rs && cmpR(i - rs) <= 0 && a[i - rs] != '0') {
                    addmodfast(dp[i], dp[i - rs], fftmod);
                }
            }
        }
        prefF[i] = prefF[i - 1];
        if (a[i] != '0') {
            addmodfast(prefF[i], dp[i], fftmod);
        }
//        DBN(i, dp[i]);
    }
    cout << dp[as] << endl;
