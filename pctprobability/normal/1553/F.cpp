...
int main() {
 ...
 ll n;
  ...
  vector<ll> a(n);
  vcin(a);
 // for(int i=0;i<n;i++) a[i]=300000-i;
  constexpr int T=300010;
  BIT<ll> dp(T),dp2(T);
  ll ans=0;
  ll rui=0;
  vector<ll> dp3(T);
  for(ll i=0;i<n;i++){
    ans+=rui;
    ans+=a[i]*(i);
    //if(i==63) cout<<"63"<<" "<<i<<" "<<ans<<" "<<a[i]<<endl;
   // cout<<ans<<endl;
    for(ll j=0;j<T;j+=a[i]){
      ll x=j,y=min(j+a[i],ll(T));
     // if(j<10) cout<<x<<" "<<y<<" "<<dp.query_sum(x,y)<<" "<<j<<endl;
     
      ans-=dp.query(x,y)*(j);
     //  if(i==63&&ans>1000000) cout<<"U"<<" "<<dp.query(x,y)<<" "<<ans<<endl;
    }
  //  if(i==63) cout<<"63"<<" "<<i<<" "<<ans<<" "<<a[i]<<endl;
   // cout<<ans<<endl;
    ll M;
    ...
    ll now=1;
  for(M = 1; M * M <= a[i]; M++) {
  //  ret.emplace_back(make_pair(M, M), a[i] / M);
    ans-=dp3[M]*(a[i]/M);
    X=M;
  }
  for(int ii = M; ii >= 1; ii--) {
    ...
    int R = a[i] / ii;
    if(L <= R && X< L){
  //   ret.emplace_back(make_pair(L, R), a[i] / L);
      ans-=dp2.query(L,R+1)*(a[i]/L);
      X=R;
    }
  }
   /* for(auto e:d){
      ll x=e.fi.fi,y=e.fi.se+1;
      ll z=dp2.query(x,y);
      ans-=z*e.se;
     // cout<<i<<" "<<e.fi.fi<<" "<<e.fi.se<<" "<<e.se<<" "<<z<<endl;
    }*/
   // if(i==63) cout<<"63"<<" "<<i<<" "<<ans<<" "<<a[i]<<endl;
    cout<<ans<<" ";
    rui+=a[i];
    ...
    dp2.add(a[i],a[i]);
    dp3[a[i]]+=a[i];
   // cout<<dp.query_sum(a[i],a[i+1])<<" "<<dp2.query_sum(a[i],a[i+1])<<endl;
  }
}