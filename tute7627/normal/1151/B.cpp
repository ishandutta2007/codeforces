...
int main(){
  ...
  ll idx,x,y;
  rep(i,0,n){
    if(*max_element(ALL(v[i]))!=*min_element(ALL(v[i]))){
      judge=true;
      idx=i;
      x=max_element(ALL(v[i]))-v[i].begin();
      y=min_element(ALL(v[i]))-v[i].begin();
      break;
    }
    res^=v[i][0];
  }
  ans(res!=0||judge,"TAK","NIE");
  if(judge){
    vector<ll> ret(n);
    rep(i,0,n)if(i!=idx){
      buf^=v[i][0];
