...
using modint = ModInt< 31607 >;modint pow(ll n, ll x){return modint(n).pow(x);}modint pow(modint n, ll x){return n.pow(x);}
//using modint=ld;
...
int main(){
  ...
  bool judge = true;
  ll n;cin>>n;
  auto a=vec(n,n,modint(0));
  rep(i,0,n)rep(j,0,n){
    ll x;cin>>x;
    a[i][j]=modint(x)/10000;
  }
  ll sz=n+2;
  vector<modint>dp(1<<sz,1);
  auto print=[&](vector<modint>v){
    auto tmp=BitConvolution::and_meb(dp);
    debug(tmp);
  };
  vector<modint>mv(1<<n);
  rep(i,0,n){
    //debug(dp);
    //print(dp);
    modint all=1;
    rep(j,0,n)all*=a[i][j];
    auto dfs=[&](auto &&f,int k,int bit,modint mult)->void{
      if(k==n){
        ll nbit=bit^((1<<n)-1);
        //if(nbit&1<<i)nbit|=1<<n;
        //if(nbit&1<<n-i-1)nbit|=1<<n+1;
        mv[nbit]=mult-all;
      }
      else{
        f(f,k+1,bit,mult*a[i][k]);
        f(f,k+1,bit^1<<k,mult);
      }
    };
    dfs(dfs,0,0,1);
    //debug(mv);
    rep(j,0,1<<sz){
      ll bit=j&((1<<n)-1);
      if(j>>n&1)bit|=1<<i;
      if(j>>n+1&1)bit|=1<<n-i-1;
      dp[j]*=mv[bit];
    }
  }
  auto ret=BitConvolution::and_meb(dp);
  cout<<1-ret[0]<<endl;
  return 0;
}