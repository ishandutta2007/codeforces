...
int main() {
    ...
    vi root;
    F0R (i, N) if (uf.rep(i) == i) root.pb(i);

    int C = sz(root);

    vvi elts(C);
    F0R (i, N) elts[index(root, uf.rep(i))].pb(i);

    vector<tree<edge>> tr;
    F0R (c, C) tr.pb(tree<edge>(sz(elts[c]), 0));

    trav (e, span) {
        int c = index(root, uf.rep(e.f));
        tr[c].add_edge(index(elts[c], e.f), index(elts[c], e.s));
    }

    F0R (c, C) tr[c].init();

    vector<heavy_path_decomposition<edge>> hld;
    F0R (c, C) hld.pb(heavy_path_decomposition(tr[c]));

    trav (e, rest) {
        int c = index(root, uf.rep(e.f));
        int u = index(elts[c], e.f);
        int v = index(elts[c], e.s);

        int dist = hld[c].dist(u, v);
        if (dist <= K - 1) {
            vi cyc;
            hld[c].for_each(u, v, true,
                    [&](index_t i, index_t j) {
                        FOR (k, i.i, j.i)
                            cyc.pb(elts[c][hld[c].at_index({k})]);
                    },
                    [&](index_t i, index_t j) {
                        FORd (k, i.i, j.i)
                            cyc.pb(elts[c][hld[c].at_index({k})]);
    ...
    // if we got here, no cycles of length <= K

    vvi odd(C), even(C);
    F0R (i, N) {
        int c = index(root, uf.rep(i));
        int u = index(elts[c], i);
