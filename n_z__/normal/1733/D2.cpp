...
vector<int>qwq;
for(int x=0;x<n;x++)
if(a[x]!=b[x])qwq.push_back(x);
if(qwq.size()%2==1)ex(-1);
if(qwq.size()==0)ex(0);
if(X>=Y)if(qwq.size()==2&&qwq[1]-qwq[0]==1)ex(X);else ex(qwq.size()/2*Y);
int f[qwq.size()+5][qwq.size()+5];
memset(f,0,sizeof(f));
for(int l=2;l<=qwq.size();l+=2)
for(int x=1,y=l;y<=qwq.size();x++,y++)
f[x][y]=min({f[x+1][y-1]+Y,f[x][y-2]+(qwq[y-1]-qwq[y-2])*X,f[x+2][y]+(qwq[x]-qwq[x-1])*X});
cout<<f[1][qwq.size()]<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
/*Main-end----------------------------------------------------------------------------------------*/
