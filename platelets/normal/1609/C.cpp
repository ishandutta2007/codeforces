...
const int N = 2e5 + 5;
int n, K, pid, prm[N], a[N], pre[N], suf[N];
bool vis[N * 5];
void solve() {
    scanf("%d%d", &n, &K);
    rep(i, 1, n) scanf("%d", &a[i]);
    rep(i, 1, n) pre[i] = i > K && a[i - K] == 1 ? pre[i - K] + 1 : 1;
    per(i, n, 1) suf[i] = i + K <= n && a[i + K] == 1 ? suf[i + K] + 1 : 1;
    ll ans = 0;
    rep(i, 1, n) if(a[i] > 1 && !vis[a[i]]) ans += (ll)pre[i] * suf[i] - 1;
    printf("%lld\n", ans);
}
int main() {
