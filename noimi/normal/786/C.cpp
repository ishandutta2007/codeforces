...
template <class T, int MAXLOG> struct WaveletMatrix {
    ...
    int select(T val, int k, int l) { return select(val, k + rank(val, l)); }
...
main() {
    ...
    vi nxt(n, n + 1);
    rep(i, n) {
        if(pre[a[i]] != -1) { nxt[pre[a[i]]] = i; }
        pre[a[i]] = i;
    }
    WaveletMatrix<int, 17> mat(nxt);
    rep2(i, 1, n) {
        int now = 0;
        int cnt = 0;
        while(now < n) {
            ...
            while(l < r - 1) {
                int mid = (l + r) / 2;
                int res = mid - now - mat.rangefreq(now,mid,now,mid) - i;
                if(res > 0){
                    r = mid - (res-1);
                }
                else l = mid - res;
