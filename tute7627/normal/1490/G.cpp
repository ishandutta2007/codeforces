...
int main(){
  ...
  while(t--){
    ...
    vector<ll>b(n+1);
    rep(i,0,n)b[i+1]=max(b[i],p[i+1]);
    vector<ll>ret;
    //debug(b,n+1);
    //debug(p,n+1);
    while(m--){
      ll x;cin>>x;
      if(p[n]<=0&&b[n]<x)ret.PB(-1);
      else{
        if(p[n]<=0){
          ret.PB(lower_bound(ALL(b),x)-b.begin()-1);
        }
        else{
          ll tmp=x-b[n];
          ll k=max(0LL,CEIL(tmp,p[n]));
          ll sum=k*n;
          sum+=lower_bound(ALL(b),x-p[n]*k)-b.begin()-1;
          ret.PB(sum);
        }
      }
    }
