...
#define let auto const
  ...
  auto mex = vla<int, 2>(array{n + 1, n + 1});
  for (let r : rep(n + 1)) {
    auto ex = vla(n + 1, false);
    for (let l : rep(r).rev()) {
      if (a[l] <= n) { ex[a[l]] = true; }
      mex[{l, r}] = all(ex).find(!_1);
    }
  }
  auto ans = i64(0);
  auto f = vla<int, 2>(array{n + 1, n + 1});
  for (let r : rep(n + 1)) {
    for (let l : rep(r).rev()) {
      f[{l, r}] = mex[{l, r}] + 1;
      for (let i : rep(l + 1, r)) { chmax(f[{l, r}], f[{l, i}] + f[{i, r}]); }
      ans += f[{l, r}];
    }
  }
  println(ans);
