...
void solve(__attribute__((unused)) bool read) {
  ...
  string s;
  cin >> s;
  vector<int> pref_sums(s.length() + 1, 0);
  for (int i = 0; i < s.length(); ++i) {
    pref_sums[i + 1] = pref_sums[i] + (s[i] - '0');
  }
  vector<char> good_column(m, false);
  int cols = 0;
  vector<int> cnt_good(m, 0);
  for (int pos = 0; pos < s.length(); ++pos) {
    if (s[pos] == '1') {
      if (!good_column[pos % m]) {
        good_column[pos % m] = true;
        ++cols;
      }
    }
    if (pref_sums[pos + 1] - pref_sums[max(0, pos + 1 - m)] > 0) {
      ++cnt_good[pos % m];
    }
    cout << cols + cnt_good[pos % m] << " ";
  }
  cout << "\n";
}