...
int main() {
	CASE(t) {
		ll n,m; R(n,m);
		ll rb,cb,rd,cd; R(rb,cb,rd,cd);
		ll p; R(p);

		vector<ll> t;
		for (int i = rb, time = 0, dr = 1; time < 2 * n - 2; i += dr, time++) {
			if (i == rd)
				for (int j = 0; j < 2 * m - 2; j++)
					t.emplace_back(time + (2 * n - 2) * j);
			if ((i == 1 && dr == -1) || (i == n && dr == 1)) dr *= -1;
		}
		for (int i = cb, time = 0, dc = 1; time < 2 * m - 2; i += dc, time++) {
			if (i == cd)
				for (int j = 0; j < 2 * n - 2; j++)
					t.emplace_back(time + (2 * m - 2) * j);
			if ((i == 1 && dc == -1) || (i == m && dc == 1)) dc *= -1;
		}

		sort(t.begin(), t.end());
		t.resize(unique(t.begin(), t.end()) - t.begin());

		const ll len = (2 * n - 2) * (2 * m - 2);
		const Mint success = 1 - (Mint(100 - p, MOD107) / 100).pow(t.size());

		Mint ans(0, MOD107);
		Mint prob(1, MOD107);
		for (int i = 0; i < (int)t.size(); i++) {
			ans += prob * p / 100 / success * t[i];
			prob = prob * (100 - p) / 100;
		}
		ans += (1 / success - 1) * len;
		W(ans);
