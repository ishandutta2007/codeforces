...
bool used[1057][1057];
int n, m;
vector<pair<pair<int, int>, int>> que[10];
int ptr[10];
int cnt[10];
vector<int> v;
int dx[] = { 0,0,1,-1 };
...
void bfs(int num) {
	if (ptr[num] == sz(que[num])) {
		return;
	}
	int st = que[num][ptr[num]].y;
	while (ptr[num] != sz(que[num]) && que[num][ptr[num]].y != st + v[num - 1]) {
		auto p = que[num][ptr[num]].x;
		auto d = que[num][ptr[num]].y;
		++ptr[num];
		++cnt[num];
...
inline void solve() {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) {
			cin >> fld[i][j];
			if (fld[i][j] >= '1' && fld[i][j] <= '9') {
				que[fld[i][j] - '0'].push_back({ { i, j }, 0 });
			}
			if (fld[i][j] != '.') {
				used[i][j] = true;
			}
