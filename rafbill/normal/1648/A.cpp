...
int main(){
  ...
  li n,m; cin>>n>>m;
  vvi C(n,vi(m));
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(m); ++j) cin>>C[i][j];

  map<int, map<int, li>> A,B;
  ...
  for(auto const& p : A) proc(p.second);
  for(auto const& p : B) proc(p.second);
