...
int main(){
	...
	TT{
		...
		cauto m=read();
		cauto xs=readV(n);
		cauto lrs=readV<char>(n);
		vector<int> inds(n); iota(all(inds),0);
		sort(all(inds),[&](cauto &i,cauto &j){ return xs[i]<xs[j];});
		vector<int> even,odd;
		vector<int> re(n,-1);
		for(cauto &i:inds){
			auto &stk=xs[i]%2?odd:even;
			if(stk.size() && lrs[stk.back()]=='R' && lrs[i]=='L'){
				cauto j=stk.back(); stk.pop_back();
				cauto d=abs(xs[i]-xs[j]);
				re[i]=re[j]=d/2;
				out(i,j,1);
			}else{
				stk.eb(i);
		...
		cauto aaa=
			[&](vector<int> &vs){
				...
				rep(_,2){
					while(vs.size()>=2){
						cauto i=vs[vs.size()-1];
						cauto j=vs[vs.size()-2];
						if(lrs[i]!=D || lrs[j]!=D) break;
						vs.pop_back(); vs.pop_back();
						cauto d=abs(wall-xs[i])+abs(wall-xs[j]);
						re[i]=re[j]=d/2;
					}
					out(vs,1);
					wall=m-wall;
