...
fun main(){
    solve.cases{
		...
		val n = getint()
		 val L = getlineL(n)
		val bittypes = IntArray(66)
		for(a in L){
			for(i in 0 until 64){
				if(a and ( 1L shl i ) != 0L){
					bittypes[i] ++
				}
			}
		}
		...
		for(j in 0 until n){
			val here = L[j]
			val ored = bittypes.copyOf()
			val anded = bittypes.copyOf()
			for(i in 0 until 64){
				if(here and ( 1L shl i ) != 0L){
					ored[i] = n
				}else{
					anded[i] = 0
				}
			...
			var right = 0
			for(i in 0 until 64){
				left = left modPlus (ored[i] modM p2[i])
				right = right modPlus (anded[i] modM p2[i])
			}
			total = total modPlus ( left modM right)
		}
