...
void solve() {
    ...
    if (a[0]==1 && a.back()==1) {
        ...
        if (m==1) {
            ...
            for (int i = g[0] - 2; i >= 0; i -= 2) add(i);
        } else if (g.back()%2==0) {
            for (int i = n-g.back(); i < n; i += 2) add(i-1);
        } else {
            bool bad = true;
            for (int i = 1; i+1 < m; i++) if (g[i]%2 != 0) bad = false;
            if (bad) {
                NO; return;
            }
