    ...
    for (int i = 0; i < n; i++) {
        for (auto& key1 : keys)
            for (auto& key2 : keys) {
                auto& [g1, x1, y1] = key1;
                auto& [g2, x2, y2] = key2;
                auto f1 = f[i][g1][x1][y1];
                auto f2 = f[i][g2][x2][y2];

                if (abs(x1*y2-x2*y1)%2 != 0) continue;
                if (g1%2 != g2%2) continue;
                
                if (((g1+g2+abs(x1*y2-x2*y1))%4 == 2) == (x1 != x2 || y1 != y2))
                    ans += (g1%2 == 0 ? 1 : 3)*(key1 == key2 ? f1*(f1-1) : f1*f2);
            }
    }
    assert(ans%6 == 0);
    cout << ans/6 << "\n";
