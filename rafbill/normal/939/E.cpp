...
struct LiChao {
  ...
  long double qu(lli x) {
    if(lo == hi) return f(x);
    if(x > mi) { return r?max(f(x),right().qu(x)):f(x); }
    else { return l?max(f(x),left().qu(x)):f(x); }
  }
};
...
int main(){
  ...
  lli sz = 0;
  long double ans = 0;
  LiChao root(-(1ll<<40), (1ll<<40), fn());
  FOR(q_,q) {
    lli ty; cin>>ty;
    if(ty == 1) {
      lli x; cin>>x;
      ans = max(ans, root.qu(x));
      S[sz+1] = S[sz]+x;
      sz += 1;
      root.add(fn(1.0-1.0/(1.0+(long double)sz), -(long double)S[sz]/(1.0+(long double)sz)));
    }else{
