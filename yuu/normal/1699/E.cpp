...
void solve() {
    ...
    int ans = mx - mi;
    for (int i = 1; i <= m; i++) min_max[i] = i;
    for (int i = 1; i <= m; i++) cnt[min_max[i]] = have[i];
    for (int r = sqrt(m) + 1; r >= 2; r--) {  // only allowed to use factor >= r, what is the minimum max achievable
        for (int i = r * r; i <= m; i += r) {
            if (have[i]) cnt[min_max[i]]--;
            min_max[i] = min(min_max[i], min_max[i / r]);
            if (have[i]) cnt[min_max[i]]++;
        }
        while (cnt[mx] == 0) mx--;
        ans = min(ans, mx - min(mi, r));
    }
    cout << ans << '\n';
}
