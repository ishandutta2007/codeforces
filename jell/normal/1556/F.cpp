#line 1 "codeforces-workspace\\contest\\1556\\f\\f.cpp"
...
#line 16 "codeforces-workspace\\contest\\1556\\f\\f.cpp"
//*/
...
#line 2 "Library\\src\\utils\\subsets.hpp"
...
#line 9 "Library\\src\\utils\\subsets.hpp"
...
template <class _Int, bool = std::is_integral<_Int>::value> class iterator {
 ...
 public:
  ...
  iterator() = default;
  iterator(_Int __x, bool __y) noexcept : __a(__x), __b(__x), __stop(__y) {}
...
class iterator<_Container, false> : public iterator<int> {
 ...
 public:
  ...
  iterator(const _Container &__c, bool __stop) noexcept
      : base_type((1 << __c.size()) - 1, __stop), __p(std::begin(__c)) {}
...
#line 34 "codeforces-workspace\\contest\\1556\\f\\f.cpp"
...
void main() {
  ...
  input n;
  input<vector<mint>> a(n);

  vector<mint> dp(1 << n, 0);
  mint ans;

  for (auto s : range(1, dp.size())) {
    vector<i32> out, in;
    for (auto i : range(n)) {
      if (s >> i & 1)
        in.emplace_back(i);
