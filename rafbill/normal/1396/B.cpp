...
static map<vector<tuple<int, bool>>, bool> memo;
bool calc(vector<tuple<int, bool>> a) {
  sort(begin(a), end(a));
  if(!memo.count(a)) {
    bool w = 0;
    for(li i = 0; i < (li)(a.size()); ++i) if(get<0>(a[i]) > 0 && get<1>(a[i]) == 0) {
      auto b = a;
      for(li j = 0; j < (li)(a.size()); ++j) get<1>(b[j]) = 0;
      get<0>(b[i]) -= 1;
      get<1>(b[i]) = 1;
      if(!calc(b)) {
        cerr << "B.cpp" ":" "17" "  {" << "a, b" << "} = {"; debug_impl_seq(a, b); cerr << endl;;
        w = 1;
        break;
      }
    }
...
bool brute() {
  vector<tuple<int, bool>> b(n); for(li i = 0; i < (li)(n); ++i) b[i] = make_tuple(A[i], 0);
  return !calc(b);
}
...
bool solve(){
  sort(begin(A), end(A));
  li s = 0; for(li i = 0; i < (li)(n); ++i) s += A[i];
  if(n == 1) {
    return 0;
  }else if(n == 2) {
    if(A[0] == A[1]) {
