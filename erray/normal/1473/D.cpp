...
int main () {
  ...
  while (T--) {
    ...
    debug(pref);
    SparseTable mx(pref, [&](int x, int y) { return max(x, y); });
    SparseTable mn(pref, [&](int x, int y) { return min(x, y); });
    ...
    while (m--) {
      ...
      
      {
        ...
        int next = 0;
        if (r < n - 1) {
          next = mx.get(r + 2, n) - (pref[r + 1] - pref[l]); 
        }
...

      {
        ...
        int next = 0;
        if (r < n - 1) {
          next = mn.get(r + 2, n) - (pref[r + 1] - pref[l]); 
        }
