...
void solve() {
    ...
    perm.clear();
    int l_k = 0, r_k = n;
    while (!pos[current_value].empty()) {
        for (int i = 0; i < pos[current_value].size(); i++) {
            auto&& x = pos[current_value][i];
            if (pos[x].size()) {
                swap(x, pos[current_value][0]);
            }
        }

        while (pos[current_value].size() > 1) {
            auto val = pos[current_value].back();
            pos[current_value].pop_back();
            perm.push_back(val);
            if (val < current_value) l_k = max(l_k, val);
            else r_k = min(r_k, val - 1);
            assert(pos[val].empty());
        }
        auto val = pos[current_value].back();
        pos[current_value].pop_back();
        perm.push_back(val);
