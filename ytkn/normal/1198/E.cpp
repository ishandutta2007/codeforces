...
int main(){
    ...
    auto cpx = Compress<int>(x);
    auto cpy = Compress<int>(y);
    int nx = cpx.size()-1;
    int ny = cpy.size()-1;
    auto is_black = vec2d(nx, ny, false);
    ...
    atcoder::mf_graph<ll> mf(2+nx+ny);
    auto x_index = [&](int i){
        return i+1;
    };
    auto y_index = [&](int j){
    ...
    int s = 0, t = nx+ny+1;
    for(int i = 0; i < nx; i++){
        int height = cpx.data[i+1]-cpx.data[i];
        mf.add_edge(s, x_index(i), height);
        for(int j = 0; j < ny; j++){
            if(is_black[i][j]){
                mf.add_edge(x_index(i), y_index(j), inf);
            }
        }
    }
    for(int j = 0; j < ny; j++){
        int width = cpy.data[j+1]-cpy.data[j];
        mf.add_edge(y_index(j), t, width);
    }
