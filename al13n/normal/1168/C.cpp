...
struct sol{
  int n;
  vector<int> A;
  vector<array<int,B>> W;

  sol(vector<int> _A):n(sz(_A)),A(move(_A)){
    array<array<int,B>,B> D;
    forn(i,B) forn(j,B) D[i][j]=n;
    W.resize(n);
    for(int i=n-1;i>=0;--i){
      fill(all(W[i]),n);
      forn(j,B) if(A[i]&(1<<j)) forn(k,B) W[i][k]=min(W[i][k],j==k?i:D[j][k]);
      forn(j,B) if(A[i]&(1<<j)) forn(k,B) D[j][k]=min(D[j][k],W[i][k]);
    }
    //dbg(W);
  }

  bool get(int L,int R){
    forn(i,B) if(A[R]&(1<<i)) return W[L][i]<=R;
    return false;
  }
};
