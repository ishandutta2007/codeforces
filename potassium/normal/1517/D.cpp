...
int main() {
	...
	vector<vector<ll>> h(n, vector<ll>(m - 1)); R(h);
	vector<vector<ll>> v(n - 1, vector<ll>(m)); R(v);

	vector<vector<ll>> dp(n, vector<ll>(m, 0)), ndp = dp;
	for (int t = 0; t < k / 2; t++) {
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++)
				ndp[i][j] = LLONG_MAX;
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++) {
				if (i > 0    ) chkmin(ndp[i - 1][j], dp[i][j] + v[i - 1][j]);
				if (i + 1 < n) chkmin(ndp[i + 1][j], dp[i][j] + v[i][j]);
				if (j > 0    ) chkmin(ndp[i][j - 1], dp[i][j] + h[i][j - 1]);
				if (j + 1 < m) chkmin(ndp[i][j + 1], dp[i][j] + h[i][j]);
			}
