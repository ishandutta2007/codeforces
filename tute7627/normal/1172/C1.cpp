...
int main(){
  ...
  rep(i,0,n)ws+=w[i];
  vector<modint>gs(2);
  rep(i,0,n)gs[a[i]]+=w[i];
  rep(x,0,n){
    auto dp=make_v(m+1,m+1,m+1,modint(0));
    dp[0][0][0]=1;
    rep(i,0,m)rep(j,0,m+1)rep(o,0,m+1){
      if(dp[i][j][o]==0)continue;
      modint np=w[x]+o;
      if(a[x]==0)np=w[x]-o;
      modint nw=ws+j-(i-j);
      dp[i+1][j+a[x]][o+1]+=dp[i][j][o]*np/nw;
      rep(t,0,2){
        modint tp=gs[t];
        if(t==a[x])tp-=np;
        if(t==0)tp-=i-j;
        else tp+=j;
        dp[i+1][j+t][o]+=dp[i][j][o]*tp/nw;
      }
    }
    modint ret=0;
    //cout<<dp<<endl;
    rep(i,0,m+1)rep(j,0,m+1)ret+=(w[x]+(2*a[x]-1)*i)*dp[m][j][i];
    cout<<ret<<endl;
  }
  return 0;
}