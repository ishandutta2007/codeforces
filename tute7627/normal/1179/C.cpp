...
int main(){
  ...
  ll sz=1000005;
  //ll sz=105;cout<<"naosu"<<endl;
  LazySegmentTree<ll,ll,decltype(ff),decltype(gg),decltype(hh)>seg(sz,ff,gg,hh,INF,0);
  ll n,m;cin>>n>>m;
  vector<ll>a(n),b(m);
  vector<ll>c(sz);
  rep(i,0,n){
  ...
  rep(i,0,sz)seg.set(i,c[i]);
  seg.build();
  auto ch=[&](ll k){return k<0;};
  ll q;cin>>q;
  while(q--){
