...
vector<ll> solve(string s, char want, ll x, ll y) {
    vector<ll> ans(sz(s));
    vector<ll> cnt(2);
    ll curr = 0;
...
signed main() {
    ...
    ll x, y;
    read(x, y);
    auto zeropref = solve(s, '0', x, y);
    auto onepref = solve(s, '1', x, y);
    reverse(s);
    auto zerosuff = solve(s, '0', y, x);
    auto onesuff = solve(s, '1', y, x);
    reverse(s);
    reverse(zerosuff);
    reverse(onesuff);
    dbg(zeropref);
    ...
    dbg(onesuff);
    ll ans = inf;
    chmin(ans, zeropref.back());
    chmin(ans, onepref.back());
    chmin(ans, zerosuff.front());
    chmin(ans, onesuff.front());
    vector<ll> pq(sz(s)), p0(sz(s)), p1(sz(s));
    for (int i = 0; i < sz(s); ++i) {
        if (s[i] == '?') ++pq[i];
        if (s[i] == '0') ++p0[i];
        if (s[i] == '1') ++p1[i];
    ...
    auto get = [](const vector<ll> &p, int l, int r) {
        if (l > r) return 0ll;
        return l == 0 ? p[r] : p[r] - p[l - 1];
    };
    auto sum = [&](const vector<ll> &a, const vector<ll> &b, int l, int r) {
        return get(a, l, r) + get(b, l, r);
    };
    dbg(ans);

    for (int i = 0; i + 1 < sz(s); ++i) {
        chmin(ans, zeropref[i] + onesuff[i + 1] +
            sum(pq, p0, 0, i) * sum(pq, p1, i + 1, sz(s) - 1) * x +
            get(p1, 0, i) * get(p0, i + 1, sz(s) - 1) * y);
        chmin(ans, onepref[i] + zerosuff[i + 1] +
            sum(pq, p1, 0, i) * sum(pq, p0, i + 1, sz(s) - 1) * y +
            get(p0, 0, i) * get(p1, i + 1, sz(s) - 1) * x);
    }
    println(ans);
    #ifdef DEBUG
