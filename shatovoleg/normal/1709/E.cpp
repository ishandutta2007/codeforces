...
void solve() {
  ...
  int ans = 0;
  auto dfs = YPredicate([&g, &a, &ans](auto& dfs, int v, int p = -1, int cur_xor = 0) -> unordered_set<int> {
    cur_xor ^= a[v];

    unordered_set<int> occured = {cur_xor};
    bool clear = false;
    ...
    for (auto x : g[v]) {
      ...
      auto ret = dfs(x, v, cur_xor);

      if (!clear) {
        if (len(ret) > len(occured)) {
          swap(ret, occured);
        }
        for (auto x : ret) {
          if (occured.count(x ^ a[v])) {
            clear = true;
            break;
          }
        }
