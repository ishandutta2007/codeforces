...
int main () {
	...
	for (int i = 0; i < n; i++) {
		for (int j = i + 1; j < n; j++) {
			a.emplace_back((x[i] - x[j]) * (x[i] - x[j]) + (y[i] - y[j]) * (y[i] - y[j]), makep(i, j));
		}
	}
	...
	for (auto && x : a) {
		bitset<3000> t = b[x.y.x] & b[x.y.y];

		if (t.any()) {
			cout << fixed << setprecision(11) << sqrtl(x.x) / 2;
			return 0;
		}

		b[x.y.x][x.y.y] = b[x.y.y][x.y.x] = true;
	}
}