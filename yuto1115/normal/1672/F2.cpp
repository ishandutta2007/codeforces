...
vi solve_f1(const vi &a) {
    int n = SZ(a);
    vvi v;
    map<int, int> id;
    rep(i, n) {
        if (id.count(a[i])) v[id[a[i]]].pb(i);
        else {
            id[a[i]] = SZ(v);
            v.pb({});
            v.back().pb(i);
    ...
    vi rot = ls;
    rotate(rot.begin(), rot.begin() + mx, rot.end());
    vi ans(n);
    rep(i, n) ans[ls[i]] = a[rot[i]];
    return ans;
}
...
void solve() {
    ...
    vvi G(n);
    rep(i, n) {
        if (a[i] != mx and b[i] != mx) G[a[i]].pb(b[i]);
    }
    print(SZ(topological_sort(G)) == n ? "AC" : "WA");
}
