#line 1 "codeforces-workspace/contest/1508/c/c.cpp"
...
#line 6 "codeforces-workspace/contest/1508/c/c.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1508/c/c.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1508/c/c.cpp"
...
#line 27 "codeforces-workspace/contest/1508/c/c.cpp"
...
void main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<gp_hash_table<int, int>> grh(n);
  int wx = 0;
  ...
  if (n * i64(n - 1) / 2 - m < n) {  // maybe forest
    i64 ans = 1e18;
    auto mat = make_vector<int>({n, n});

    auto calc_mst = [&]() -> i64 {
      vector<int> dist(n, 1 << 30), used(n);
      dist[0] = 0;
      i64 ans = 0;
      auto nx = 0;
      for (auto $ : range(n)) {
    ...
    vector<int> cmp(n);
    {
      ...
      for (auto v : range(n)) {
        ...
        while (!q.empty()) {
          ...
          q.pop();
          cmp[now] = cnt;
          gp_hash_table<int> nxt;
          for (auto &&to : yet) {
            if (grh[now].count(to)) {
              if (!chkd[to]) nxt.emplace(to);
              continue;
            }
            chkd[to] = 1;
            q.push(to);
