...
bool cannot(vl_l &Q,vl &c,vl &g){ //a(c)
    assert(Q.size()!=0);
    vl gs;
    for(auto x:Q){
        gs.pb(g[x.fi]);gs.pb(g[x.se]);
    }
    ERASE(gs);
    ll gss=gs.size();
    dsu d(2*gss);
        ...
        while (!q.empty()){
            ...
            for(auto x:v[t]){
                ...
                if(a[x]==0){
                    ...
                    if(a[x]+a[t]!=3)ng[c[x]]=true;
                }
            }
        }
    }
  //  rep(i,1,k)cout<<ng[i]<<" ";cout<<endl;
    ...
    rep(i,0,(int)G.size()-1){
        for(auto x:G[i])g[x]=i;
    }
    ll ans=0;
    for(auto Q:es){
        if(cannot(Q.se,a,g))ans++;
      //  cout<<"ans is "<<ans<<endl;
    }
    {
        ll cnt=0;rep(i,1,k)if(!ng[i])cnt++;
        ans=cnt*(cnt-1)/2-ans;
     //   cout<<"cnt :"<<cnt<<endl;
    }
    cout<<ans<<endl;
