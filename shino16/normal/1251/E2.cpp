...
int main() {
  ...
  rep(t) {
    ...
    sort(all(mp));
    int M = -1;
    rep(i, n) chmax(M, mp[i]._1);
    VI small(M+10);
    rep(i, n) small[mp[i]._1+1]++;
    rep(i, M) small[i+1] += small[i];
    minheap<int> pm;
    ll res = 0; int cnt = 0;
    repr(i, n) {
      int x = mp[i]._1;
      while (i >= 0 and mp[i]._1 == x) pm.push(mp[i]._2), i--;
      int todo = x - small[x] - cnt;
      rep(todo) if (not pm.empty()) res += pm.top(), pm.pop(), cnt++;
      i++;
    }
    OUT(res);
  }
