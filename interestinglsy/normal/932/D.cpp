...
ll maxx[MAXN][30];
int cnt = 1;
int Findfa( int u , ll w ){
    fOR0(i,MAXD+2)
        if( maxx[u][i] < w )
            u = up[u][i];
    return u;
}
il void Add( int f , ll w ){
    cnt++;
    int u = cnt;
    up[u][0] = Findfa(f,w);
    wei[u] = sum[u][0] = maxx[u][0] = w;

    For(j,MAXD+1){
        up[u][j] = up[up[u][j-1]][j-1];
        maxx[u][j] = max( maxx[u][j-1] , maxx[up[u][j-1]][j-1] );
        sum[u][j] = sum[up[u][j-1]][j-1] + sum[u][j-1];
    }
}

il ll Query( int u , ll lim ){
    //cout << "-------------------------- " << u << " " << lim << endl;
    ll ans = 0;
    //O(up[2][0]);
    fOR0(i,MAXD){
        //cout << u << " " << sum[u][i] << " " << lim << endl;
        if( sum[u][i] <= lim ){
            lim -= sum[u][i];
            u = up[u][i];
            ans += (1LL<<i);
...
int main(){
	FILEIO();
    Ms(up); Ms(sum); Ms(maxx);

    wei[0] = LLINF;
    For(j,27)
        maxx[1][j] = sum[1][j] = LLINF;
    For0(j,28)
        maxx[0][j] = sum[0][j] = LLINF;
    //Add(0,0);
