#line 1 "codeforces-workspace\\contest\\1566\\c\\c.cpp"
...
#line 13 "codeforces-workspace\\contest\\1566\\c\\c.cpp"
...
void main() {
  ...
  int last[2]{-1, -1};
  for (auto i : range(n)) {
    for (auto j : {0, 1}) last[s[j][i] - '0'] = i;
    dp[i + 1] = dp[i];
    if (~last[0]) chgr(dp[i + 1], 1 + dp[last[0]]);
    if (auto p = min(last[0], last[1]); ~p) chgr(dp[i + 1], dp[p] + 2);
  }
  print(dp.back());
}
