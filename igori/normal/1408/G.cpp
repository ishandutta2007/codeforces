...
void Merge(int x, int y)
{
    ...
    int iscorrect = (edgesin[y] == sz[y] * (sz[y] - 1) / 2) % MOD;

    dp[y][1] = (dp[y][1] + iscorrect) % MOD;
    //cout << "Correct subset:\n";
    //for (auto e : f[y]) cout << e << " "; cout << "\n";
    //cout << "Dp result:\n";
    //for (auto e : dp[y]) cout << e << " "; cout << "\n";
}
...
signed main()
{
    ...
    vector<edge> e;
    forn(i, n) forn(j, n) if (i < j) e.push_back({i, j, a[i][j]});
    forn(i, n) dp[i] = {0, 1};
    sort(all(e), comp);
    forn(i, n) root[i] = i, sz[i] = 1, f[i] = {i};
    for (auto ed : e)
    {
        if (Root(ed.a) != Root(ed.b))
        {
            //cout << "MERGE : " << ed.a << " " << ed.b << "\n";
            Merge(ed.a, ed.b);
        }
        else
        {
