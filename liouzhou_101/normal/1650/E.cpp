...
void solve()
{
	...
	auto add = [&](int x)
	{
		...
		if ((*prev <= d) && (*next <= d))
		{
			gap.erase({ *next - *prev, *prev, *next });
		}
		if (*prev <= d)
		{
			gap.insert({ *cur - *prev, *prev, *cur });
		}
		if (*next <= d)
		{
			gap.insert({ *next - *cur, *cur, *next });
		}
	};
	...
	auto del = [&](int x)
	{
		...
		if ((*prev <= d) && (*next <= d))
		{
			gap.insert({ *next - *prev, *prev, *next });
		}
		if (*prev <= d)
		{
			gap.erase({ *cur - *prev, *prev, *cur });
		}
		if (*next <= d)
		{
			gap.erase({ *next - *cur, *cur, *next });
		}
		H.erase(cur);
	};
	...
	int res = get<0>(*gap.begin());
	auto go = [&](int x)
	{
		if (!H.count(x))
		{
			add(x);
			freshmax(res, get<0>(*gap.begin()));
			del(x);
		}
	};
