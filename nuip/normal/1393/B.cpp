...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto vs=readV(n);
	cauto q=read();
	cauto qs=readV<pair<char,int>>(q);
	vector<int> cnt0(112345);
	vector<int> cnt1(112345);
	vector<int> cnt12(2);
	int cnt=0;
	auto add=
		[&](int x,int d){
	...
	auto check=
		[&](){
			if(cnt-cnt12[1]<8) return false;
			return cnt1[1]+2*cnt1[2]+3*cnt1[3]!=cnt;
		};
	for(auto v:vs) add(v,1);
	for(auto [c,x]:qs){
		add(x,c=='+'?1:-1);
