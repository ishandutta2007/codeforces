...
#endif
int a[173660],pos[173660];
int ansl[173660],ansr[173660];
int f[17360][173],id[174],p[174];
signed main()
{
	initprog();
	int n=read(),k=read();
	rg(n*k)a[i]=read();f[a[i]][++id[a[i]]]=i;gr
	int zak=(n-1)/(k-1)+1;
	rg(k)
	if(i==1)continue;
	rg(n)p[i]=i;gr
	std::sort(p+1,p+1+n,[&](int x,int y){return f[x][i]<f[y][i];});
	// rg(n)odb(p[i]);gr puts("");
	rg_(j,zak)if(f[p[j]][i-1]==inf)break;ansl[p[j]]=f[p[j]][i-1],ansr[p[j]]=f[p[j]][i];
	memset(f[p[j]],inf,sizeof(f[p[j]]));
	gr
	gr
	rg(n)odp(ansl[i],ansr[i]);gr
	return 0;
}