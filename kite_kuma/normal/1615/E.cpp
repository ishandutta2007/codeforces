...
ll solve() {
	...
	auto dfs = [&](auto self, int now, int par) -> void {
		foa(e, g[now]) {
			...
			// foa(c, res[e.to]) { tmp.insert(c); }

			if(res[e.to].size() > res[now].size()) {
				swap(res[e.to], res[now]);
			}
			// for(auto it = res[e.to].begin(); it != res[e.to])
			foa(c, res[e.to]) res[now].insert(c);
		}
		auto &tmp = res[now];
		tmp.insert(0);
		auto it = tmp.end();
		...
		// debug(now);
		// {
		// 	foa(component, tmp) { debug(component); }
		// }
		return;
	};
	dfs(dfs, 0, -1);
	const auto &result = res.front();
	int erase_cnt = 0;
	auto it = result.end();
	// debug(result);
	ll ans = -INF;
	rep(erase, k) {
		...
		int blue = min(n / 2, n - erase_cnt);
		int red = erase + 1;
		int white = n - blue - red;
		// debug(blue, red, white);
		chmax(ans, white * ll(red - blue));
	}

	// return white * ll(blue - red);
	return ans;
}
