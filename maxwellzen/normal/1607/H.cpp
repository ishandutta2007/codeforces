...
void solve() {
    cin >> n;
    a.rsz(n); b.rsz(n); m.rsz(n); l.rsz(n); r.rsz(n); bymass.clear();
    F0R(i, n) {
        cin >> a[i] >> b[i] >> m[i];
        r[i] = min(a[i], a[i]+b[i]-m[i]);
        l[i] = max(0, a[i]-m[i]);
        bymass[a[i]+b[i]-m[i]].pb(i);
    }
    val.rsz(n);
    int ans = 0;
    for (auto &p : bymass) {
        ...
        int cur = -1;
        for (int i : p.S) {
            if (l[i]>cur) cur=r[i], ans++;
            val[i]=cur;
        }
    }
    cout << ans << nl;
    F0R(i, n) {
        cout << a[i]-val[i] << ' ' << m[i]-a[i]+val[i] << nl;
    }
}
