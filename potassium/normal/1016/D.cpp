...
int main() {
	int n,m; R(n,m);
	vector<int> r(n), c(m); R(r, c);

	const int rsum = accumulate(r.begin(), r.end(), 0, [](int x, int y) { return x ^ y; });
	const int csum = accumulate(c.begin(), c.end(), 0, [](int x, int y) { return x ^ y; });
	...
	if (rsum != csum) {
		...
		W("YES");
		ans[0][0] = r[0] ^ c[0] ^ rsum;
		for (int i = 1; i < n; i++) ans[i][0] = r[i];
		for (int j = 1; j < m; j++) ans[0][j] = c[j];
