...
int main()
{
	...
	for (i=1;i<=n;i++) {
		vec.push_back({brr[i]+arr[i]*t,brr[i]-arr[i]*t});
	}
	...
	vec.push_back({0,0});
		...
		// 	cout<<g.fi<<' '<<g.se<<endl;
		// }
		for (auto g : vec) sus[g.se]=1;
		t1=0;
		for (auto& g : sus) {
			t1++;
			g.se=t1;
		}
		for (auto& g : vec) g.se=sus[g.se]; 
		st.merg=cmpg;
		st.reset(1,1,n);
		for (i=0;i<=n;i++) {
			dp[i]=st.get(1,1,n,1,vec[i].se)+1;
			if (vec[i].fi==0 && sus[0]==vec[i].se) res=dp[i];
			// cout<<"dp "<<vec[i].fi<<' '<<vec[i].se<<' '<<dp[i]<<endl;
			st.upd(1,1,n,vec[i].se,dp[i]);
		}
	cout<<res-1;
	
}

// aj-vtj>=ai-vti && aj+vtj<=ai+vti