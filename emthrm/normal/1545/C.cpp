...
void solve() {
  int n; cin >> n;
  vector<vector<int>> a(n * 2, vector<int>(n)); REP(i, n * 2) REP(j, n) cin >> a[i][j], --a[i][j];
  vector col(n, vector(n, set<int>{}));
  REP(i, n * 2) REP(j, n) col[j][a[i][j]].emplace(i);
  ModInt ans = 1;
  vector<int> is_alive(n * 2, true), good;
  while (good.size() < n) {
    int row = -1;
    for (int j = 0; j < n && row == -1; ++j) for (int i = 0; i < n && row == -1; ++i) {
      if (col[j][i].size() == 1) row = *col[j][i].begin();
    }
    if (row == -1) {
      ans *= 2;
      REP(i, n * 2) {
    ...
    is_alive[row] = false;
    good.emplace_back(row);
    vector<int> era{row};
    REP(j, n) for (int i : col[j][a[row][j]]) {
      if (is_alive[i]) {
        is_alive[i] = false;
        era.emplace_back(i);
      }
    }
    for (int i : era) REP(j, n) col[j][a[i][j]].erase(i);
  }
  cout << ans << '\n';
  REP(i, n) cout << good[i] + 1 << " \n"[i + 1 == n];
}
