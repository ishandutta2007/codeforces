...
int main(){
	...
	cauto qs=readV(q,tuple{1,1,1});
	cauto s=string(s0.size(),'(')+s0;
	const int n=s.size();
	vv<int> g(1);
	vector<pii> ps{{-1,-1}};
	vector<int> vs(n);
	...
	cauto dfs=
		[&](auto &&dfs,int v)->ll{
			ll re=1+g[v].size()*(g[v].size()-1)/2;
			for(cauto &u:g[v]) re+=dfs(dfs,u);
			return cnts[v]=re;
		};
	dfs(dfs,0);
	out(cnts,1);
	vv<int> sums(N);
	rep(v,N){
		sums[v].resize(g[v].size()+1);
		rep(i,g[v].size()) sums[v][i+1]=sums[v][i]+cnts[g[v][i]];
	}
	for(cauto &[t,l0,r0]:qs){
		if(!t) continue;
		cauto l=vs[n/2+l0];
		cauto r=vs[n/2+r0];
		cauto [p,li]=ps[l];
		cauto ri=ps[r].Y;
		ll re=0;
		// reps(i,li,ri+1) re+=cnts[g[p][i]];
		re+=sums[p][ri+1]-sums[p][li];
		const ll d=ri-li+1;
		re+=d*(d-1)/2;
		cout<<re NL;
	}
