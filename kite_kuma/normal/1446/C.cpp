...
int main() {
	...
	// 	now |= 1 << i;
	// 	up = now + (1 << i);
	// 	int k = lower_bound(all(a), up) - lower_bound(all(a), now);

	// 	if(majority < k)
	// }

	function<int(int, int)> dfs = [&](int bits, int i) {
		if(i == -1) return 0;

		auto lef = lower_bound(all(a), bits);
		auto mid = lower_bound(all(a), bits + (1 << (i)));
		auto rig = lower_bound(all(a), bits + (1 << (i + 1)));
		...
		if(h == 0 && m == 0) return 0;
		if(h == 0) {
			return dfs(bits + (1 << i), i - 1);
		} else if(m == 0) {
			return dfs(bits, i - 1);
		}

		return min(dfs(bits, i - 1) + m - 1, dfs(bits + (1 << i), i - 1) + h - 1);
		// return;
	};
