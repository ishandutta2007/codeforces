...
bool check(int mid){
	...
	for(j=1;j<=k;j++)
		vis[j]=0;
	for(i=j=1;i<=n&&a[i]<=p;i++){
		while(j<=k&&abs(a[i]-b[j])+abs(p-b[j])>mid)
			j++;
		if(j>k)
			break;
		vis[j++]=1;
	...
	if(i>n)
		return 1;
	for(i=n,j=k;i&&a[i]>p;i--){
		while(j&&(vis[j]||abs(a[i]-b[j])+abs(p-b[j])>mid))
			j--;
		if(!j)
			break;
		j--;
