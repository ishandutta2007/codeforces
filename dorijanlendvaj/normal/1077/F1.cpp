...
int main()
{
	...
	for (int i=1;i<=x;++i) dp[0][i]=-LLINF,ms[i].insert(-LLINF);
	ms[0].insert(0);
	for (int i=1;i<=n;++i)
	{
		for (int j=1;j<=x;++j)
		...
		for (int j=1;j<=x;++j) ms[j].insert(dp[i][j]);
		if (i>=k) for (int j=1;j<=x;++j) ms[j].erase(ms[j].find(dp[i-k][j]));
	}
	ll ma=0;
	for (int i=n-k+1;i<=n;++i) ma=max(ma,dp[i][x]);
	/*for (int j=x;j>0;--j)
	{
		for (int i=1;i<=n;++i) cout<<dis(dp[i][j])<<' ';
		cout<<endl;
	}*/
	if (ma) cout<<ma;
	else cout<<-1;
