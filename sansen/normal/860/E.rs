...
fn run() {
    ...
    for i in 0..n {
        hld.path(root, i, &mut up, &mut down);
        let d = up.iter().chain(&down).fold(0, |s, p| s + p.1 - p.0);
        depth[i] = d - 1;
        g[d].push(i);
    }
    let mut bit0 = Fenwick::new(n + 2, 0i64);
    let mut bit1 = Fenwick::new(n + 2, 0i64);
    let mut ans = vec![0; n];
    for g in g {
        for &v in g.iter() {
            hld.path(root, v, &mut up, &mut down);
            for &(l, r) in up.iter().chain(&down) {
                let l = l + 1;
                bit0.add(l, -(l as i64 - 1));
                bit0.add(r + 1, r as i64);
                bit1.add(l, 1);
                bit1.add(r + 1, -1);
            }
        }
        for v in g {
            hld.path(root, v, &mut up, &mut down);
            for &(l, r) in up.iter().chain(&down) {
                ans[v] += bit0.sum(r) + bit1.sum(r) * r as i64;
                if l > 0 {
                    ans[v] -= bit0.sum(l) + bit1.sum(l) * l as i64;
                }
            }
            ans[v] -= depth[v] as i64 + 1;
        }
    }
    let s = ans
        .iter()
