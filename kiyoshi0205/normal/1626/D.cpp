...
int main(){
  ...
  rep(i,N){
    ll res=calc(v[i]);
    ll ch=int(i==0)+1+int(i==N-1);
    chmin(ans,res+calc(v.back()-v[i])+ch);
    debug(i,ans);
    ll p=1;
    do{
      auto itr=prev(upper_bound(all(v),v[i]+p));
      ch=int(i==0)+int(*itr==v[i])+int(*itr==v.back());
      chmin(ans,res+calc(*itr-v[i])+calc(v.back()-*itr)+ch);
      debug(ans);
      itr=lower_bound(all(v),v.back()-p);
      if(v[i]<*itr){
        ch=int(i==0)+int(*itr==v[i])+int(*itr==v.back());
        chmin(ans,res+calc(*itr-v[i])+calc(v.back()-*itr)+ch);
      }
