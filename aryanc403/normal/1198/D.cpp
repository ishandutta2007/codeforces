...
const lli N = 50L;
    ...
    string s[N+5];
    vi a;
    lli dp[N+5][N+5][N+5][N+5];
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

lli solve(lli lx,lli ly,lli rx,lli ry)
{
    lli &ans=dp[lx][ly][rx][ry];
    // dbg(lx,ly,rx,ry,ans);
    if(ans!=-1)
        return ans;
    ...
    ans=1+max(ry-ly,rx-lx);
    repA(i,lx,rx-1)
        ans=min(ans,solve(lx,ly,i,ry)+solve(i+1,ly,rx,ry));
    repA(i,ly,ry-1)
        ans=min(ans,solve(lx,ly,rx,i)+solve(lx,i+1,rx,ry));
    dbg(lx,ly,rx,ry,ans);
    return ans;
}
...
// cin>>T;while(T--)
{
    ...
    memset(dp,-1,(N+5)*(N+5)*(N+5)*(N+5)*sizeof(lli));
