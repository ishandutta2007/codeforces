// time-limit: 1000
// problem-url: https://codeforces.com/contest/1684/problem/D
...
void up(){
	...
	for(int const index: range(0, number)){
		std::cin>>a[index];
		baseDamage+=a[index];
		a[index]=number-1-index - a[index];
	}

	std::nth_element(begin(a), begin(a)+(k-1), end(a));
	std::cout<<std::accumulate(a.begin(), a.begin()+k, baseDamage-((k*int64_t(k-1))>>1))
	<<'\n';
