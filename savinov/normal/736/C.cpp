            ...
            for (int down = -1; down <= k; ++down) {
                for (int u = -1; u <= k; ++u) {
                    for (int d = -1; d <= k; ++d) {
                        ...
                        int nu = max(pu, up);
                        int nd = max(up, pu) >= 0 ? -1 : 0;

                        if (pu < down && down != -1) nd = max(nd, down);
                        if (up < pd && d != -1) nd = max(nd, pd);

                        if (nd <= k) {
                            tmp[nu + 1][nd + 1] += dp[v][up + 1][down + 1] * dp[to][u + 1][d + 1];
                            tmp[nu + 1][nd + 1] %= mod;
                        }
                    }
                }
                ...
                tmp[uu + 1][dd + 1] += dp[v][up + 1][down + 1] * qp[to];
                tmp[uu + 1][dd + 1] %= mod;
            }
        }
