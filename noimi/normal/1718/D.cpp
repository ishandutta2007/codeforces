...
int main() {
    TEST {
        ...
        bool flag = true;
        vpi v;
        REC([&](auto &&f, int l, int r, int ma) -> void {
            if(l == r) return;
            int m = rmt.prod(l, r).se;
        ...
        // fore(l, r, v) imos[lb(val, r)]++;
        // rep(i, si(cnt)) imos[i + 1] += imos[i];
        // rep(i, si(cnt)) cnt[i] -= imos[i];
        rep(i, si(cnt) - 1) cnt[i + 1] += cnt[i];
        ...
        {
            vpi setter(si(cnt));
            rep(i, si(cnt)) setter[i] = {cnt[i], i};
            RARm<pii> seg(setter);
            SORT(v);
            fore(l, r, v) {
                dump(l, r, s);
                while(idx < si(s) and s[idx] < l) seg.apply(s[idx], si(cnt), pii(-1, 0)), idx++;
                dump(idx, seg);
                auto [x, i] = seg.prod(l, si(cnt));
                if(x < -1) flag = false;
                if(x == -1) { res = (res & pii(l, i)); }
                seg.apply(r, si(cnt), pii(1, 0));
                dump(seg);
            }
        }
        dump(val, res, flag);
        if(res.fi > res.se) flag = false;

        rep(q) {
            INT(x);
            YES(flag && inc(x, val[res.fi], val[min(res.se + 1, si(val) - 1)]));
        }
    }
}