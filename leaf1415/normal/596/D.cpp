...
double p;
ll x[2005];
double dp[2005][2][2][3];
double predp[2005][2][2][3];
...
int main(void)
{
	...
	rep(len, 1, n){
		rep(i, 1, n){
			...
			rep(k, 0, 1){
				rep(l, 0, 1){
					ll L = min(x[i], x[i-1] + k*h), R = max(x[j], x[j+1] - l*h);
					rep(m, 0, 2){
						double lsum = 0, rsum = 0;
						if(x[i+1] >= x[i]+h) lsum += (min(h, min(R,x[i+1])-x[i]) + predp[i+1][1][l][0]);
						else lsum += (min(h, min(R, x[i+1])-x[i]) + predp[i+1][1][l][1]);
							
						if(x[j-1] <= x[j]-h) rsum += (min(h, x[j]-max(L, x[j-1])) + predp[i][k][1][0]);
						else rsum += (min(h, x[j]-max(L, x[j-1])) + predp[i][k][1][2]);
						
						if(m == 0){
							lsum = (1-p) * lsum + p * (min(h, x[i]-L) + predp[i+1][0][l][0]);
							rsum = p * rsum + (1-p) * (min(h, R-x[j]) + predp[i][k][0][0]);
							dp[i][k][l][m] = (lsum+rsum)/2;
						}
						if(m == 1) dp[i][k][l][m] = lsum;
						if(m == 2) dp[i][k][l][m] = rsum;
						//if(dp[i][j][k][l][m] > eps) outl(i, j, k, l, m, lsum, rsum, dp[i][j][k][l][m]);
					}
					
				}
			}
		}
		rep(i, 1, n)rep(k, 0, 1)rep(l, 0, 1)rep(m, 0, 2) predp[i][k][l][m] = dp[i][k][l][m];
	}
	outf(dp[1][0][0][0]);
