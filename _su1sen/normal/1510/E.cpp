...
void solve() {
    ...
    int m = n / 2;
    LD(x, y);
    vc<tuple<int, int, ld, ld, ld>> left;
    // left
    {
        auto dfs = [&](auto&&dfs, int i, int h, int bit, ld p, ld qx, ld qy) -> void {
            if(i == m) {
                left.eb(h, bit, p, qx, qy);
                return;
            } 
            {
                ld area = det(P{(ld)i, (ld)h}, P{(ld)(i + 1), (ld)(h + 1)});
                ld gx = (ld)(i + (i + 1)) / 3.0;
                ld gy = (ld)(h + (h + 1)) / 3.0;
                ld np = p + area;
                ld nqx = qx + area * gx;
                ld nqy = qy + area * gy;
                dfs(dfs, i + 1, h + 1, bit | (1<<i), np, nqx, nqy);
            }
            if(h > 0) {
                ld area = det(P{(ld)i, (ld)h}, P{(ld)(i + 1), (ld)(h - 1)});
                ld gx = (ld)(i + (i + 1)) / 3.0;
                ...
                ld nqx = qx + area * gx;
                ld nqy = qy + area * gy;
                dfs(dfs, i + 1, h - 1, bit, np, nqx, nqy);
            }  
        };
        dfs(dfs, 0, 0, 0, 0.0, 0.0, 0.0);
    }
    // right
    vc<tuple<int, int, ld, ld>> right;
    {
        auto dfs2 = [&](auto&&dfs2, int ci, int h, int bit, ld p, ld qx, ld qy) -> void {
            if(ci == m) {
                ld vx = x * p - qx;
                ld vy = y * p - qy;
                right.eb(h, bit, vx, vy);
                return;
            } 
            int i = n - ci;
            {
                ld area = det(P{(ld)(i - 1), (ld)(h + 1)}, P{(ld)i, (ld)h});
                ld gx = (ld)(i + (i - 1)) / 3.0;
                ld gy = (ld)(h + (h + 1)) / 3.0;
                ld np = p + area;
                ld nqx = qx + area * gx;
                ld nqy = qy + area * gy;
                dfs2(dfs2, ci + 1, h + 1, bit | (1<<ci), np, nqx, nqy);
            }
            if(h > 0) {
                ld area = det(P{(ld)i - 1, (ld)(h - 1)}, P{(ld)i, (ld)h});
                ld gx = (ld)(i + (i - 1)) / 3.0;
                ...
                ld nqx = qx + area * gx;
                ld nqy = qy + area * gy;
                dfs2(dfs2, ci + 1, h - 1, bit, np, nqx, nqy);
            }  
        };
        dfs2(dfs2, 0, 0, 0, 0.0, 0.0, 0.0);
    }
    vc<ld> vxs, vys;
    map<tuple<int, int, int>, int> mp;
    for(auto [h, bit, vx, vy]: right) {
        debug(h, bit, vx, vy);
        vxs.eb(vx);
        vys.eb(vy);
    }
    otera::Compressor<ld> cvx(vxs), cvy(vys);
    for(auto [h, bit, vx, vy]: right) {
        mp[make_tuple(h, cvx.get(vx), cvy.get(vy))] = bit;
    }
    int szx = si(cvx), szy = si(cvy);
    auto get = [&](int h, ld vx, ld vy) -> int {
        int lx = cvx.min_gt_index(vx - eps), rx = cvx.max_lt_index(vx + eps);
        int ly = cvy.min_gt_index(vy - eps), ry = cvy.max_lt_index(vy + eps);
        for(int i = lx; i <= rx; ++ i) {
            for(int j = ly; j <= ry; ++ j) {
                if(mp.count(make_tuple(h, i, j))) {
                    return mp[make_tuple(h, i, j)];
                }
            }
        }
        return -1;
