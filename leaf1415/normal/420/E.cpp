...
int main(void)
{
	...
	typedef pair<double, ll> E;
	vector<E> evec;
	rep(i, 1, n){
		rep(j, 1, 5000){
			if(x[i]*x[i]+y[i]*y[i] > (j*d+r[i])*(j*d+r[i])) continue;
			if(x[i]*x[i]+y[i]*y[i] < (j*d-r[i])*(j*d-r[i])) break;
			
			vec2d p, q;
			getCrossPointCC(vec2d(0, 0), j*d, vec2d(x[i], y[i]), r[i], p, q);
			double l = atan2(p.y, p.x), r = atan2(q.y, q.x);
			if(l > r) r += 2*PI;
			evec.pb(E(l-eps, 1)), evec.pb(E(r+eps, -1));
			evec.pb(E(l-eps+2*PI, 1)), evec.pb(E(r+eps+2*PI, -1));
		}
	}
	sort(all(evec));
	
	ll ans = 0, cnt = 0;
	for(auto p : evec) cnt += p.se, chmax(ans, cnt);
	outl(ans);
