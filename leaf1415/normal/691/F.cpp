...
ll n, Q;
ll cnt[3000005], sum[3000005];
ll mcnt[3000005], msum[3000005];
ll M = 3000000;
...
int main(void)
{
	...
	rep(i, 1, M){
		ll b = M / i;
		rep(j, 1, b){
			if(i != j) mcnt[i*j] += cnt[i] * cnt[j];
			else mcnt[i*j] += cnt[i] * (cnt[j] - 1);
		}
	}
	rep(i, 1, M) msum[i] = msum[i-1] + mcnt[i];
	
	cin >> Q;
	ll p;
	rep(q, 1, Q){
		cin >> p;
		cout << lnum+msum[M]-msum[p-1] << "\n";
	}
