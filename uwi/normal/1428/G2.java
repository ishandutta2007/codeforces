...
public class G2 {
	...
	void dfs(int cur, int d, long[] dp, int all, long[] F, int B, int K)
	{
		if(d == 6){
			dps[cur] = dp[0];
			return;
		...
		for(int dd = 0;dd <= 9;dd++) {
			...
			for (int c = 0; c < ndp.length; c++) {
				...
				for (int rem = 0; rem < 3; rem++) {
					...
					long v = -st[rem].min(inf, sup + 1) + (val - rem) / 3 * F[d];
				...
				for (int t = Math.max(val - 9 * K, 0); t <= val - 9 * (K - 1) && t < dp.length; t++) {
					...
					ndp[c] = Math.max(ndp[c], got * F[d] + dp[t]);
				}
			}
			dfs(cur + dd * B, d + 1, ndp, all / 10, F, B * 10, K);
		}
	}
	...
	void solve()
	{
		int K = ni();
		long[] F = nal(6);
		dps = new long[1000000];
		dfs(0, 0, new long[]{0L}, 1000000, F, 1, K);
		long I = Long.MIN_VALUE / 2;
		for(int Q = ni();Q > 0;Q--){
			int n = ni();
			out.println(dps[n]);
