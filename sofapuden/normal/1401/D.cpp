...
ll dfs(ll x, ll p){
	ll retu = 0;
	for(int i = 0; i < sz(tre[x]); ++i){
		if(tre[x][i] == p)continue;
		ll z = dfs(tre[x][i],x);
...
void solve(){
	...
	tre.resize(n);
	for(int i = 0; i < n-1; ++i){
		ll tm1, tm2; cin >> tm1 >> tm2; tm1--; tm2--;
		tre[tm1].pb(tm2);
		tre[tm2].pb(tm1);
	}
	for(int i = 0; i < sz(tre[0]); ++i){
		ll x = dfs(tre[0][i], 0);
		path.pb(min(x, n-x));
	}
	...
	sort(rall(path));
	ll m; cin >> m;
	vector<ll> pri(n-1,1), se(m);
	EACH(x, se)cin >> x;
	sort(all(se));
	for(ll i = 0; i < m; ++i){
		pri[min(i,n-2LL)]*=se[i];
		pri[min(i,n-2LL)]%=MOD;
	}
	sort(rall(pri));
	ll ans = 0;
	for(int i = 0; i < n-1; ++i){
		ans+=path[i]*(n-path[i])*pri[i];
		ans%=MOD;
	}
	print(ans);
