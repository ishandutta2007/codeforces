...
void solve() {
    ...
    vvi blocks;
    for (int i = 0; i < n; i++)
        blocks.push_back(vi{i});

    vi prefix{0};
    vi ans;
    for (int i = 1; i < n; i++) {
        int res = query(prefix, vi{i});
        if (res != 0) {
            ans.push_back(i);
        }
        if (ans.empty())
    ...
    pd(ans);
    vi oracle{ans.front()};
    auto enough = [&](int count) {
        vi test(prefix.begin(), prefix.begin() + count);
        int res = query(test, oracle);
        return res != 0;
    };

    int count = binarysearch(enough, 1, sz(prefix));
    ans.push_back(prefix[count - 1]);
    pd(ans, queries);
    sort(all(ans));
