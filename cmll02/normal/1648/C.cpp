...
void add(int x,int p){
	...
	if(x==0)return;
	while(x<=n)mae(a[x],p),x+=x&-x;}
int operator[](int x){int res=0;while(x)mae(res,a[x]),x-=x&-x;return res;}
...
#define s a
#define t b
int s[1000005],t[1000005],ta[1000005],tb[1000005];
BIT A;
#define ifac inv
signed main()
{
	...
	const int V=200000;
	int n=read(),m=read();
	rg(n)s[i]=read();ta[s[i]]++;gr
	rg(m)t[i]=read();tb[t[i]]++;gr
	// return 0l;
	int ans=fac[n];
	rg(V)
	mle(ans,ifac[ta[i]]);
	...
	int G=min(n+1,m);
	rg(V)
	if(ta[i]!=0)A.Add(i,ta[i]);
	gr
	// idk
	// odl(ans);
	rg(G)
	// odp(ans,res);
	if(i==n+1)mae(res,ans);
	else
	{
		int x=b[i];
		// odl(A[x-1]);
		int L=n-i+1;
		mae(res,me(rnv[L],me(ans,A[x-1])));
		mle(ans,rnv[L]);
		A.Add(x,de(0,ta[x]));
		if(ta[x]==0)break;
		mle(ans,ta[x]);
