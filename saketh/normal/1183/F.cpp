...
void solve() {
    int N; re(N); vi a(N); re(a); dedup(a); N = sz(a);

    int ans = 0;

    int M = a.back();
    if (M % 30 == 0) {
        bool m2 = find(a, M/2) != a.end();
        bool m3 = find(a, M/3) != a.end();
        bool m5 = find(a, M/5) != a.end();
        if (m2 && m3 && m5) ans = M/2 + M/3 + M/5;
    }

    int j = N-2; while (j >= 0 && M % a[j] == 0) j--;
    int k = j-1; while (k >= 0 && (M % a[k] == 0 || a[j] % a[k] == 0)) k--;
    ckmax(ans, M + (j>=0 ? a[j] : 0) + (k>=0 ? a[k] : 0));
