...
#define int ll
vector<int> FV[300000];
vector<int> BV[300000];
int Visited[300000];

void dfs(int now,stack<int> &cnter) {
    Visited[now] = 1;
    for (auto x : FV[now]) {
...
void solve() {
    ...
    REP(q, 2) {
        REP(i, n) {
            ...
            cin >> a;
            a--;
            inputs[q].push_back(a);
        }
    }
    REP(i, 2) {
        for (int q = 0; q < n - 1; ++q) {
            FV[inputs[i][q+1]].push_back(inputs[i][q]);
            BV[inputs[i][q]].push_back(inputs[i][q + 1]);
        }
    }
    stack<int> goes;
    REP(i, n) {
