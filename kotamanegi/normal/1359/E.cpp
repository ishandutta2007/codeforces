...
modint<MOD> comb(int a, int b) {
    if (a - b < 0 or a < 0 or b < 0) return 0;
    modint<MOD> ans = mae[a];
    ans *= gyaku[b];
    ans *= gyaku[a - b];
    return ans;
}
modint<MOD> Calc(int a, int b) {
    modint<MOD> ans = 0;
    b -= a - 1;
    ans = comb(a + b - 1, b);
    return ans;
}
modint<MOD> muls[600000];
void solve() { 
    int n, k;
    cin >> n >> k;
    mae[0] = 1;
    ...
    gyaku[n + k + 1] = modint<MOD>(1) / mae[n + k + 1];
    for (int q = n + k + 1; q >= 1; --q) {
        gyaku[q - 1] = gyaku[q] * q;
    }
    modint<MOD> ans = 0;
    for (int q = 2; q <= n; ++q) {
        ans += muls[q] * Calc(k, n / q - 1);
    }
    modint<MOD> hoge = comb(n - 1, k - 1);
    ans += hoge;
    cout << ans << endl;
}
#undef int
