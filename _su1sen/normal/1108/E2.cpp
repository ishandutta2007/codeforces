...
constexpr int e() { return IINF; }
constexpr int mapping(int f, int x) { return f + x; }
constexpr int composition(int f, int g) { return f + g; }
...
int main() {
    ...
    vec<pair<int, int>> ps(m);
    vec<vec<int>> in(n), out(n + 1);
    rep(i, m) {
        int l, r;
        read(l, r);
        --l;
