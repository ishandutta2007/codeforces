#line 1 "codeforces-workspace/contest/1499/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1499/c/c.cpp"
...
void main() {
  ...
  cin >> c;
  i64 ans = 1e18;
  i64 ev = c[0], od = c[1], sum = od + ev;
  for (auto k : range(2, n + 1)) {
    chle(ans, sum + ev * (n - (k + 1) / 2) + od * (n - k / 2));
    if (k < n) {
      chle((k % 2 ? od : ev), c[k]);
      sum += c[k];
    }
