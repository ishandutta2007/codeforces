...
#line 4 "/home/maspy/compro/library/alg/monoid_rollinghash.hpp"
...
void solve() {
  ...
  ll LIM = 1 << 20;
  ll p = 0;
  SegTree<Monoid_Rolling_Hash> seg(LIM);
  string ANS;

  FOR(N) {
    STR(S);
    SegTree<Monoid_Rolling_Hash> segS(
        len(S), [&](int i) { return Mono::from_element(S[i]); });
    ll a = 0;
    FOR(i, len(S) + 1) {
      if (p < i) break;
      auto x = seg.prod(p - i, p);
      auto y = segS.prod(0, i);
      if (x == y) a = i;
    }
    FOR(j, a, len(S)) {
      seg.set(p++, Mono::from_element(S[j]));
      ANS += S[j];
    }
  }
  print(ANS);
