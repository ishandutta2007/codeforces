...
string solve() {
	...
	debug(g);
	vector<int> pars(n, -1);
	vector<int> pars_id(n, -1);
	vector<int> depth(n, -1);
	constexpr int root = 0;
	pars[root] = root;
	depth[root] = 0;

	vector<tuple<int, int, int>> upwards;  // under, top, edge_id
	auto dfs = [&](auto self, int now, int par) -> void {
		foa(e, g[now]) {
			...
			if(pars[e.to] == -1) {
				...
				self(self, e.to, now);
			} else if(depth[now] > depth[e.to]) {
				upwards.emplace_back(now, e.to, e.id);
			}
		}
		return;
	};
	...
	if(m == n + 2) {
		set<int> s;
		vector<int> from_to(n, 0);
		debug(upwards);
		for(auto [bottom, top, id] : upwards) {
			s.insert(bottom);
			s.insert(top);
			from_to[bottom]++;
			from_to[top]--;
		...
		if(s.size() == 3) {
			...
			rep(m) ans += '0';
			debug(from_to);
			int bottom = find(all(from_to), 2) - from_to.begin();
			int top = find(all(from_to), -2) - from_to.begin();
			debug(bottom, top);
			for(auto [b, t, id] : upwards) {
				if(b == bottom && t == top) {
					ans[id] = '1';
				}
			}
