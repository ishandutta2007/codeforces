...
int main()
{
    ...
    {
        ll t[6][6]={
            {2, (2*i)%M2, (i*i)%M2, 1, (2*i)%M2, (i*i)%M2},
            {0, 2, (i)%M2, 0, 1, (i)%M2},
            {0, 0, 2, 0, 0, 1},
            {0, 0, 0, 2, (2*i)%M2, (i*i)%M2},
            {0, 0, 0, 0, 2, (i)%M2},
            {0, 0, 0, 0, 0, 2}
        };
        ll tt[6][6];
        REP(i1,0,6) REP(j1,i1,6) m[i][i1][j1]=0;
        REP(i1,0,6) m[i][i1][i1]=1;
        ll r=1;
        while(r<=f[i])
        {
            if(r&f[i])
            {
                REP(i1,0,6) REP(j1,i1,6) tt[i1][j1]=m[i][i1][j1];
                REP(i1,0,6) REP(j1,i1,6) m[i][i1][j1]=0;
                REP(i1,0,6) REP(j1,i1,6) REP(k1,i1,j1+1) m[i][i1][j1]+=((tt[i1][k1]*t[k1][j1])%M2);
                REP(i1,0,6) REP(j1,i1,6) m[i][i1][j1]%=M2;
            }
            REP(i1,0,6) REP(j1,i1,6) tt[i1][j1]=t[i1][j1];
            REP(i1,0,6) REP(j1,i1,6) t[i1][j1]=0;
            REP(i1,0,6) REP(j1,i1,6) REP(k1,i1,j1+1) t[i1][j1]+=((tt[i1][k1]*tt[k1][j1])%M2);
            REP(i1,0,6) REP(j1,i1,6) t[i1][j1]%=M2;
            r<<=1;
        }
    }
    REP(i,1,N)
    {
        ll t[6][6];
        REP(i1,0,6) REP(j1,i1,6) t[i1][j1]=0;
        REP(i1,0,6) t[i1][i1]=1;
        for(ll j=i;j<N;j+=i)
        {
            ll tt[6][6];
            REP(i1,0,6) REP(j1,i1,6) tt[i1][j1]=t[i1][j1];
            REP(i1,0,6) REP(j1,i1,6) t[i1][j1]=0;
            REP(i1,0,6) REP(j1,i1,6) REP(k1,i1,j1+1) t[i1][j1]+=((tt[i1][k1]*m[j][k1][j1])%M2);
            REP(i1,0,6) REP(j1,i1,6) t[i1][j1]%=M2;
        }
        d[i]=(t[0][5]+M2-t[0][2])%M2;
    }
    REPI(i,1,N)
    {
        g[i]=d[i];
