...
struct DPair {
    int val, id;
    DPair(int val, int id) : val(val), id(id) {}
    inline bool operator< (const DPair &tmp) const { return val > tmp.val || (val == tmp.val && id > tmp.id); }
};


inline void work() {
    read(n); rep(i, 1, n) v[i].clear(), d[i] = 0;
    rep(i, 1, n) {
        int k = read(); d[i] = k;
    ...
    rep(i, 1, n) if(d[i] == 0) q.push(DPair(1, i));
    int tot = 0, ans = 0;
    while(!q.empty()) {
        const DPair x = q.top(); q.pop(); ++ tot;
        chmax(ans, x.val);
        for (auto it : v[x.id]) if(!(-- d[it])) q.push(DPair(x.val + (it <= x.id), it));
    }
    if(tot == n) print(ans);
    else print("-1");
}
