...
int main(int argc, char** argv) {
    ...
    auto sum = [](const string& a, const string& b) {
        string res = "";
        int c = 0;
        for (int i = 0; i < max(isz(a), isz(b)); i++) {
            int x = i < isz(a) ? a[isz(a) - 1 - i] - '0' : 0;
            int y = i < isz(b) ? b[isz(b) - 1 - i] - '0' : 0;
            res.push_back((x + y + c) % 10 + '0');
            c = (x + y + c) / 10;
        }
        if (c) {
            res.push_back(c + '0');
    ...
    if (x == 0) {
        cout << sum(s.substr(0, y), s.substr(y, n - y));
        return 0;
    }

    if (y == n) {
        cout << sum(s.substr(0, x), s.substr(x, n - x));
        return 0;
    }

    string a = sum(s.substr(0, x), s.substr(x, n - x));
    string b = sum(s.substr(0, y), s.substr(y, n - y));
