...
int chk(int n, int m, vi p) {
	debug(n, m, p);
	UF uf(n);
	rep(i, n) uf.unite(p[i], i);
	return int(m >= n - uf.components);
}

vi shift(vi a, int rig) {
	int n = a.size();
	vi b;
	rep(i, n - rig, n) b.push_back(a[i]);
	rep(i, 0, n - rig) b.push_back(a[i]);
	return b;
}

vi solve(int n, int m, vi p) {
	map<int, int> ma;
	rep(i, n) { ma[(n + p[i] - i) % n]++; }
