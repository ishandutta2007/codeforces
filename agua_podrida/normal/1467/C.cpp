...
void c(){
    ...
    ll sumaa = 0, sumab = 0, sumac = 0;
    for(auto ai : a) sumaa += ai;
    for(auto ai : b) sumab += ai;
    for(auto ai : c) sumac += ai;

    ll tot = sumaa + sumab + sumac;
    ll res = tot - 2*min(min(sumaa,sumab),sumac);

    maxi(res,tot - 2*a[0] - 2*b[0] - 2*c[0] + 2*max(max(a[0],b[0]),c[0]));
