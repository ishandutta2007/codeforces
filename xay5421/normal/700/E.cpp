...
struct SAM{
	...
	int query(int u,int l,int r,int lx,int rx){
		if(!u||lx>r||rx<l)return 0;
		if(lx<=l&&r<=rx)return 1;
		int mid=(l+r)>>1;
		if(rx<=mid)return query(lc[u],l,mid,lx,rx);else
		if(mid<lx)return query(rc[u],mid+1,r,lx,rx);else
