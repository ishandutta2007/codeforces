...
int main() {
	...
	CASE(t) {
		...
		for (int last = 0; last <= 9; last++) {
			for (int t9 = 0; last + t9 * 9 <= n; t9++) {
				int bef = min(10 - last, k);
				int aft = k - bef;

				int bsum = last + t9 * 9;
				int asum = 1;

				int totalSum = bef * bsum + bef * (bef - 1) / 2 + aft * asum + aft * (aft - 1) / 2;
				if (totalSum <= n && (n - totalSum) % k == 0) {
					int rem = (n - totalSum) / k;

					ll pre = 0;
					for (int i = 0; rem > 0; i++) {
						int take = min(rem, (i == 0 ? 8 : 9));
						rem -= take;
						pre += take * pow10[i];
					}

					ll val = pre * pow10[t9 + 1] + tval[t9] + last;
					chkmin(ans, val);
				}
			}
		}
