...
void solve() {
	...
	rep(i, q) {
		...
		int c =bt.query(l[i]+1,r[i],a[r[i]]);
		//cout << "? " << c << "\n";
		sum += c; sum -= (r[i] - l[i] - 1 - c);
		
		c = bt.query(l[i] + 1, r[i],a[l[i]]);
		//cout << "? " << c << "\n";
		sum += r[i] - l[i] - 1 - c; sum -= c;
		
		if (a[l[i]] > a[r[i]])sum--;
		else sum++;

		bt.update(l[i], a[l[i]], -1);
		bt.update(r[i], a[r[i]], -1);
		swap(a[l[i]], a[r[i]]);
		bt.update(l[i], a[l[i]], 1);
		bt.update(r[i], a[r[i]], 1);
