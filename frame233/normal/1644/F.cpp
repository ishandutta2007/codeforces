...
template<typename _Tp>inline poly operator / (poly a,const _Tp &b){return a/=b;}
int mu[N],p[N],pos,c[N];bool pr[N];ll fac[N],inv[N],ifac[N];
...
int main(){
	sieve(),setup(),init();
	int n,k;read(n,k);int ans=0;if(n==1||k==1)return puts("1"),0;
	for(int i=1;i<=n;++i)if(mu[i])c[(n+i-1)/i]+=mu[i];
	for(int i=1;i<=n;++i)if(c[i]){
		poly f(i+1),g(i+1);
		for(int j=0;j<=i;++j)f[j]=ifac[j]*ksm(j,i)%mod,g[j]=j%2?mod-ifac[j]:ifac[j];
		f*=g;int res=0;for(int j=1;j<=i&&j<=k;++j)add(res,f[j]);
		add(ans,1LL*(c[i]+mod)*res%mod);
	}
	for(int i=1;i<=n;++i)ans-=mu[i];
	printf("%d\n",(ans+mod)%mod);
	return 0;
}