...
ll min1[314514],min2[314514],f[314514],in[314514];
ll n,m,cnt=0,to[314514],next[314514],head[314514];
void add(ll u,ll v){
	++cnt;
	to[cnt]=v;
	in[v]++;
...
ll q[314514],nq[314514],vis[314514];//,nq[314514];
ll check(void){
	ll h=1,t=0;
	rep(i,1,n)
	if(!in[i]){
...
ll x[314514],y[314514],c[314514];
ll cmp1(ll u,ll v){
	return nq[x[u]]<nq[x[v]];
}
ll cmp2(ll u,ll v){
	return nq[y[u]]>nq[y[v]];
}
int main(){
//IO(CF1344C Quantifier Question);
	rill(n,m);
	rep(i,1,n)min1[i]=min2[i]=i;
	rep(i,1,m){
		rill(x[i],y[i]);
		add(x[i],y[i]);c[i]=i;
//		if(x<y){
//			min1[y]=min(min1[y],min1[x]);
//		}
//		else {
//			min2[x]=min(min2[x],min2[y]);
//		}
	}
	if(check()){
		write(-1);
	...
	std::sort(c+1,c+m+1,cmp1);
	rep(i,1,m){
		min1[y[c[i]]]=min(min1[y[c[i]]],min1[x[c[i]]]);
	}
	std::sort(c+1,c+m+1,cmp2);
	rep(i,1,m){
		min2[x[c[i]]]=min(min2[x[c[i]]],min2[y[c[i]]]);
	}
	ll tot=0;
	rep(i,1,n){
//		writeln(i,' ',min1[i],' ',min2[i]);
		if(min1[i]>=i&&min2[i]>=i)
		f[i]=1,tot++;
	}
	writeln(tot);
	rep(i,1,n)write(f[i]?'A':'E');
	return 0;
}
//