...
void solve(){
	...
	RMQ<long long,20,std::max> device_max;
	RMQ<long long,20,std::min> device_min;

	device_max.init(pref);
	device_min.init(pref);

    for(int i = 0; i < q; ++i){
    	int l, r;
    	cin >> l >> r;
    	if (pref[r] != pref[l - 1] || device_max.query(l, r) > pref[l - 1]){
    		cout << -1 << endl;
    	}
    	else{
    		cout << pref[l - 1] - device_min.query(l, r) << endl;
    	}
    }
}
int main(){
