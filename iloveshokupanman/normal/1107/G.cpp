...
int main() {
    ...
    rep(i, N-1)lis.push_back({ d[i + 1] - d[i],i });
    ...
    vll rui(N + 1);
    repn(i, N)rui[i] = rui[i - 1] + a - c[i - 1];

    ll ans = 0;
    rep(i, N)ans = max(ans, a - c[i]);
    ...
    rep(i, N - 1) {
        ...
        if (itr != --cut.end()) {
            ll y = (++itr)->first;
            cut[y].first = max(cut[y].first, rui[y + 1] - rui[x + 1] + cut[x].first);
        }
        
        itr = cut.find(x);
        if (itr != cut.begin()) {
            ll z = (--itr)->first;
            cut[z].second = max(cut[z].second, rui[x + 1] - rui[z + 1] + cut[x].second);
        }
        
        ll now = cut[x].first + cut[x].second - (d[x + 1] - d[x]) * (d[x + 1] - d[x]);
        ans = max(now, ans);
        cut.erase(x);
    }
