...
void solve()
{
	...
	vector<ll> b(n - 1);
	input(b);
	ll res = c / a[0] + (c % a[0] > 0);
	
	ll remain = 0, extra_days = 0;
	for (int i = 0; i < n; ++i)
	{
		int need = max(c - remain, 0LL) / a[i] + (max(c - remain, 0LL) % a[i] > 0);
		freshmin(res, need + extra_days);
		if (i != n - 1)
		{
			if (remain < b[i])
			{
				int need = (b[i] - remain) / a[i] + ((b[i] - remain) % a[i] > 0);
				extra_days += need;
				remain += need * a[i];
			}
			remain -= b[i];
			extra_days += 1;
			if (extra_days >= res) break;
		}
	}
	writeln(res);
}
