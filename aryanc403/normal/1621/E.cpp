...
ii op(ii a, ii b) { return {a.X+b.X,min(b.Y,a.Y+b.X)}; }
ii e() { return {0,0}; }
...
atcoder::segtree<ii, op, e> seg(N);
cin>>T;while(T--)
{

    cin>>n>>m;
    vi b(n);
    auto change=[&](lli pos,lli c){
        auto g=seg.get(pos);
        g.X+=c;g.Y+=c;
        seg.set(pos,g);
    };
    ...
    fo(i,m){
        const lli k=sz(a[i]);
        const lli avg=(sums[i]+k-1)/k;
        change(avg,1);

        for(auto x:a[i]){
            const lli ag=(sums[i]-x+k-2)/(k-1);
            change(ag,-1);
            cout<<(seg.all_prod().Y>=0);
            change(ag,1);
        }
