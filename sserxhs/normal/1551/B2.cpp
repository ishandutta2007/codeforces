...
char s[N];
int a[N],b[N],cnt[N],ans[N],st[N];
vector<int> pos[N];
int T,n,m,c,i,j,k,x,y,z,la,ksiz,ks,tp;
int main()
{
	...
	{
		read(n,m,a,n);
		for (i=1;i<=n;i++) pos[i].clear(),ans[i]=0;tp=0;
		for (i=1;i<=n;i++) if (pos[a[i]].size()<m) pos[a[i]].push_back(i);
		for (i=1;i<=n;i++) for (auto u:pos[i]) st[++tp]=u;
		for (i=1;i+m-1<=tp;i+=m) for (j=i;j<=min(i+m-1,tp);j++) ans[st[j]]=j-i+1;
		write(ans,n);
	}
}