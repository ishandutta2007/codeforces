...
string s,t;
int s0[maxn],s1[maxn];
int prv0[maxn],prv1[maxn];
int q;
signed main(){
    ios::sync_with_stdio(false);
    cin.tie(0);
    cin>>s>>t;
    rep1(i,sz(s)) s0[i]=s0[i-1]+(s[i-1]!='A');
    rep1(i,sz(t)) s1[i]=s1[i-1]+(t[i-1]!='A');
    int prv=0;
    rep1(i,sz(s)){
        if (s[i-1]!='A') prv=i;
        prv0[i]=prv;
    ...
    while (q--){
        int u,v,x,y;
        cin>>u>>v>>x>>y;
        int l=s0[v]-s0[u-1], r=s1[y]-s1[x-1];
        int consl=v-max(u-1,prv0[v]), consr=y-max(x-1,prv1[y]);

        if (l==r){
            if (consl>=consr&&consl%3==consr%3) cout<<1;
            else cout<<0;
            continue;
        }
        if (l<r&&l%2==r%2&&consl>=consr){
            if (consl==consr&&!l) cout<<0;
            else cout<<1; 
        }
        else cout<<0;
    }
