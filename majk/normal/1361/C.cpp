...
public:
    ...
    bool cycle(vector<set<pii>> &E, vector<pii>& X, int u, int t) {
        while (true) {
            if (E[u].empty()) return u == t;
    ...
    bool tour(vector<set<pii>> &E, vector<pii>&ans, int u) {
        vector<pii> S;
        if (!cycle(E, S, u, u)) return false;

        for (pii s: S) {
            ans.push_back(s);
            if (!tour(E, ans, s.x)) return false;
        }

        return true;
    }

    vector<int> build(vector<pii> A, int N, int ans) {
        if (ans == 0) {
            vector<int> Ans(2*N);
            for (int i = 1; i <= 2*N; ++i) Ans[i-1] = i;
            return Ans;
        }

        int mask = (1<<ans)-1;
        vector<set<pii>> E(1<<ans);
        for (int i = 0; i < N; ++i) {
            int a = A[i].x & mask;
            int b = A[i].y & mask;
            E[a].insert({b,2*i});
        ...
        vector<pii> T;
        bool res = tour(E, T, lo);
        vector<int> Ans;
        for (pii t: T) {
            Ans.push_back(1 + t.y);
            Ans.push_back(1 + (t.y ^ 1));
        }
        return Ans;
    }

    bool solve(vector<pii> A, int N, int ans) {
        if (ans == 0) {
            return true;
        }

        int mask = (1<<ans)-1;
        vector<int> C(1<<ans, 0);
        UnionFind UF(1<<ans);
        for (int i = 0; i < N; ++i) {
            int a = A[i].x & mask;
            int b = A[i].y & mask;
