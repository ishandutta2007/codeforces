#line 1 "codeforces-workspace\\contest\\1528\\d\\d.cpp"
...
#line 6 "codeforces-workspace\\contest\\1528\\d\\d.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace\\contest\\1528\\d\\d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1528\\d\\d.cpp"
...
void main() {
  ...
  auto ans = make_vector({n, n}, inf);
  for (auto src : range(n)) {
    auto &dist = ans[src];
    // dist[src] = 0;
    vector<int> used(n);
    // used[src] = 1;
    ...
    while (true) {
      ...
      used[src] = 1;
      for (auto &&[to, c] : G[src]) {
        chle(dist[(to + dist[src]) % n], dist[src] + c);
      }
      for (auto t : range(src)) {
        chle(dist[t], dist[src] + t + n - src);
      }
      for (auto t : range(src, n)) {
        chle(dist[t], dist[src] + t - src);
      }
    }
  }

  for (auto &&[s, v] : enumerate(ans)) {
    v[s] = 0;
    cout << v << "\n";
  }
}
