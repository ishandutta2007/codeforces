...
bool inthefuckrange(int x,int y){
...
int main(){
	...
	queue<int> changed;
	FILLBIG(f13);
	q.push(mp(mp(0,1),mp(fuxk,fuyk)));
	q.push(mp(mp(1,1),mp(fuxk,fuyk)));
	q.push(mp(mp(2,1),mp(fuxk,fuyk)));
	bs.push(0);
	changed.push(0);
	bs.push(0);
	changed.push(0);
	...
	f13[1][1][fuxk][fuyk]=0;
	f13[2][1][fuxk][fuyk]=0;
	int minccc,minboo=Inf;
	while(SZ(q)){
		pair<pii,pii> ppp=q.front();
		q.pop();
		int boo=bs.front();
		bs.pop();
		int ccc=changed.front();
		changed.pop();
		/*if(ccc!=f13[ppp.first.first][ppp.first.second][ppp.second.first][ppp.second.second]){
			continue;
		}*/
		if(ppp.first.second==n*n){
			cerr<<boo<<' '<<ccc<<endl;
			if(minboo==Inf){
				minboo=boo;
				minccc=ccc;
			}else if(minboo==boo){
				minccc=min(minccc,ccc);
			}else{
				break;
			}
			continue;
		}
		if(ppp.first.first==1){
			for(int i=0;i<8;i++){
				if(inthefuckrange(ppp.second.first+dx[i],ppp.second.second+dy[i])){
					pair<pii,pii> qqq=ppp;
					qqq.second.first+=dx[i];
					qqq.second.second+=dy[i];
					if(A[qqq.second.first][qqq.second.second]==qqq.first.second+1){
						qqq.first.second++;
					}
					if(f13[qqq.first.first][qqq.first.second][qqq.second.first][qqq.second.second]>ccc){
						q.push(qqq);
						bs.push(boo+1);
						changed.push(ccc);
						f13[qqq.first.first][qqq.first.second][qqq.second.first][qqq.second.second]=ccc;
					}
				}
			}
		}else if(ppp.first.first==2){
			for(int i=1;1;i++){
				if(inthefuckrange(ppp.second.first+i,ppp.second.second+i)){
					pair<pii,pii> qqq=ppp;
					qqq.second.first+=i;
					qqq.second.second+=i;
					if(A[qqq.second.first][qqq.second.second]==qqq.first.second+1){
			...
			for(int i=1;1;i++){
				if(inthefuckrange(ppp.second.first-i,ppp.second.second)){
					pair<pii,pii> qqq=ppp;
					qqq.second.first-=i;
					if(A[qqq.second.first][qqq.second.second]==qqq.first.second+1){
						qqq.first.second++;
			...
			for(int i=1;1;i++){
				if(inthefuckrange(ppp.second.first,ppp.second.second-i)){
					pair<pii,pii> qqq=ppp;
					qqq.second.second-=i;
					if(A[qqq.second.first][qqq.second.second]==qqq.first.second+1){
						qqq.first.second++;
		...
		for(int i=0;i<3;i++){
			if(i!=ppp.first.first){
				pair<pii,pii> qqq=ppp;
				qqq.first.first=i;
				if(f13[qqq.first.first][qqq.first.second][qqq.second.first][qqq.second.second]>ccc+1){
					q.push(qqq);
					bs.push(boo+1);
					changed.push(ccc+1);
					f13[qqq.first.first][qqq.first.second][qqq.second.first][qqq.second.second]=ccc+1;
				}
			}
		}
	}
	fio<<minboo<<' '<<minccc<<endl;
	return 0;
}