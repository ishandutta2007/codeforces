...
_ n, m, a[maxn], p[maxn], b[maxn], op, x, y;
deque<_> q;
...
void fix(_ x) {
    q.clear();
    for(_ i = x, j = big; j--; i = p[i]) q.push_front(i);
    for(_ i = a[x], j = big; j--; i = a[i]) q.push_back(i);
    for(_ i = 0; i+big < q.size(); ++i) b[q[i]] = q[i+big];
    return;
}