...
int main() {
	...
	fori(i, n) {
		...
		wne(stk) {
			int cur = stk.back();
			if (cons[cur].size() == a[cur]) {
				stk.pop_back();
				con
			}
			if (depth[cons[cur][a[cur]]] == -1) {
				depth[cons[cur][a[cur]]] = depth[cur] + 1;
				f[cons[cur][a[cur]]] = cur;
				stk.push_back(cons[cur][a[cur]]);
				ord.push_back(cons[cur][a[cur]]);
			}
			else {
				if (depth[cur] - depth[cons[cur][a[cur]]] >= serch-1 && cycle.size()==0) {
					co 2 el
					int b = cons[cur][a[cur]];
					int num = depth[cur] - depth[cons[cur][a[cur]]] + 1;
					co num el
					fori(j, num-1) {
						out(cur + 1)
						cur = f[cur];
	...
	forib(i, n) {
		int cur = ord[i];
		if (colored[cur]) con
			indep.push_back(cur + 1);
		forin(j, cons[cur]) {
			colored[cons[cur][j]] = 1;
		}
	}
	if (indep.size() >= serch) {
		co 1 el
