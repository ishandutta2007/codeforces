...
int main()
{
    ...
    forn(i, n)
    {
        if (!used[i])
        {
            int cnt = 0;
            ll cur = a[i];
            while (cur <= a[n - 1] && binary_search(a, a + n, cur))
            {
                used[lower_bound(a, a + n, cur) - a] = 1;
                ++cnt;
                cur *= k;
            }
            ans += (cnt + 1) / 2;
