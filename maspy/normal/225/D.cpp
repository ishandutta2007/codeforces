...
void solve() {
  ...
  */
  ...
  FOR(s, OK.size()) {
    ...
    FOR(i, K) FOR(j, i) if (P[i] == P[j]) OK[s] = 0;
  }
  ll init = 0;
  FOR_R(i, K - 1) {
    auto [a, b] = pos[i];
    auto [c, d] = pos[i + 1];
    FOR(k, 4) {
      if (c == a + dx[k] && d == b + dy[k]) { init = 4 * init + k; }
    }
  }
  ...
  const ll INF = 1LL << 60;
  vvv(ll, dist, H, W, OK.size(), INF);
  using T = tuple<ll, ll, ll>;
  auto [a, b] = pos[0];
  deque<T> que;
  dist[a][b][init] = 0;
  que.push_back(mt(a, b, init));
  ...
  while (!que.empty()) {
    ...
    FOR(d, 4) {
      ...
      if (G[nx][ny] == '#') continue;
      if (chmin(dist[nx][ny][nxt], dist[x][y][s] + 1))
        que.push_back(mt(nx, ny, nxt));
    }
  }

  ll ANS = INF;
  FOR(x, H) FOR(y, W) if (G[x][y] == '@') ANS = MIN(dist[x][y]);
  if (ANS == INF) ANS = -1;
  print(ANS);
}
