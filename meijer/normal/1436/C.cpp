...
void program() {
    ...
    int smaller=0, bigger=0, other=0;
    RE(i,n) if(b[i]==0) other++;
    RE(i,n) if(b[i]==-1) smaller++;
    RE(i,n) if(b[i]==1) bigger++;
    ...
    mi ans = fact[x-1]/fact[x-1-smaller];
    ans *= fact[n-x]/fact[n-x-bigger];
    ans *= fact[other];
    OUTL((ll)ans);
}