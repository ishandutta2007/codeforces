...
static char inbuf[BUFSIZE + 16];
static char* linp = inbuf + BUFSIZE;
static char* inp = linp;
int read_int() {
    if (inp == linp) {
        std::cin.read(inbuf, BUFSIZE);
    ...
    if (dif < 11) {
        ...
        inp = inbuf;
        std::cin.read(inbuf + dif, BUFSIZE - dif);
        //        fread(inbuf + dif, 1, BUFSIZE - dif, stdin);
    }
    while (*inp < '0') {
        ++inp;
    }
