...
long long cnt(long long a, long long b) {
    return min(r, min(min(a, n - a + 1), n - r + 1)) * min(r, min(min(b, m - b + 1), m - r + 1));
}
...
int main(){
    ...
    for (long long i = max(1LL, n / 2 - 3); i <= min(n, n / 2 + 3); i++) {
        for (long long j = max(1LL, m / 2 - 3); j <= min(m, m / 2 + 3); j++) {
            s.insert(make_pair(cnt(i, j), make_pair(i, j)));
            was[i][j] = true;
        }
    }
    long long ans = 0;
    for (int it = 0; it < k; it++) {
        pair<long long, pair<long long, long long> > st = *s.rbegin();
        s.erase(st);
        ans += st.first;
        long long x = st.second.first;
        long long y = st.second.second;
     //   cout << x << ' ' << y << endl;
        for (int k = 0; k < 4; k++) {
            long long x2 = x + dx[k];
            long long y2 = y + dy[k];
            ...
            was[x2][y2] = true;
            s.insert(make_pair(cnt(x2, y2), make_pair(x2, y2)));
        }
    }
    cout.precision(10);
    cout << fixed << double(ans) / ((n - r + 1) * (m - r + 1))<< endl;
    return 0;
}