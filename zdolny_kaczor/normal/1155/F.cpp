...
bool dfs(int curr, int to, int prev = -1) {
	if (curr == to) return true;
	for (int x : graf[curr]) if (x != prev) {
		if (dfs(x, to, curr)) {
			onion(x, curr);
			return true;
		}
...
void go() {
	...
	vpii curr;
	for (pii x : edges) {
		if (add(x.first, x.second)) {
			cou[x.first][x.second]++;
			curr.push_back(x);
		}
	}
	if (curr.size() < best.size()) swap(curr, best);
}
bool conn[nax][nax];
bool can[1 << nax][nax][nax];
vector <pii> find(int mask, int x, int y) {
	vector <pii> ans;
	while (mask) {
		bool found = false;
		for (int i = 1; i <= n; ++i) {
			if (conn[i][y] && can[mask ^ (1 << y)][x][i]) {
...
int main() {
	...
	vector <pair <vpii, vpii> > cycles;
	for (int mask = 2; mask <= all; mask += 2) {
		bool super = false;
		int first = __builtin_ctz(mask);
		forbits(i, all ^ mask)
			if (can[mask ^ (1 << i)][first][first]) {
				super = true;
				break;
			}
		if (!super && __builtin_popcount(mask) >= 3) {
			if (can[mask][first][first]) {
				vector <pii> cycle = find(mask, first, first);
				set <pii> cycle_s(cycle.begin(), cycle.end());
				vector <pii> rest;
				for (pii e : edges)
					if (!cycle_s.count(e) && !cycle_s.count({e.second, e.first}))
						rest.push_back(e);
				cycles.emplace_back(cycle, rest);
			}
		}
	}
	assert(!cycles.empty());
	while (clock() < 0.1 * CLOCKS_PER_SEC) {
		int si = rand() % cycles.size();
		edges = cycles[si].first;
		int s = edges.size();
		edges.insert(edges.end(), cycles[si].second.begin(), cycles[si].second.end());
		random_shuffle(edges.begin() + s, edges.end());
		go();
	}
	printf("%d\n", (int) best.size());
	for (pii x : best)
