...
void tarjan(int x, int pre) {
    vis[x] = 1; stk[++ tp] = x; for (int i = head[x];i;i = nxt[i]) {
        const int v = e[i]; if(v == pre) continue;
        if(!vis[v]) tarjan(v, x);
        else if(!b[v]){
            int cur = x, Mx = -1, Mn = n + 1;
            do {
                cur = stk[tp --]; b[cur] = 1;
                chmax(Mx, cur); chmin(Mn, cur);
            }while(cur ^ v);
            chmin(t[Mn], Mx - 1);
        }
    } tp -= !b[x];
...
signed main(){
    ...
    rep(i, 1, n) t[i] = n;
    rep(i, 1, n) (!vis[i]) && (tarjan(i, 0), 1);
    per(i, n - 1, 1) chmin(t[i], t[i + 1]);
    rep(i, 1, n) s[i] = s[i - 1] + t[i] - i + 1;
    int q = read(); while(q --) {
        int l, r; read(l, r);
        int L = l, R = r, ans = l - 1; while(L <= R) {
            int mid = (L + R) >> 1;
            if(t[mid] <= r) ans = mid, L = mid + 1;
            else R = mid - 1;
        }
        LL res = s[ans] - s[l - 1]; print(res + (1ll * (r - ans) * (r - ans + 1) >> 1));
    }
}
/*
