...
template <typename T> void ROTATE(vector<vector<T>> &v) {
    vector<vector<T>> res(v[0].size(), vector<T>(v.size()));
    rep(i, v.size()) rep(j, v[0].size()) res[v[0].size() - 1 - j][i] = v[i][j];
    v.swap(res);
}
...
int main() {
    ...
    im.update(1, n + 1, 1);
    RAT<ll> seg(n + 1);
    using node = IntervalManager<int>::node;
    while(q--) {
        STR(s);
        if(s == "Color") {
            INT(l, r, c);
            ...
            im.update(
                l, r, c,
                [&](int l, int r, int x) {
                    dump("add", l, r, x, -add[x]);
                    seg.update(l, r, -add[x]);
                },
                [&](int l, int r, int x) {
                    dump("del", l, r, x, add[x]);
                    seg.update(l, r, add[x]);
                });
        } else if(s == "Add") {
            INT(c, x);
