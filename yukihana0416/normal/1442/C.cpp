...
void work()
{
    ...
    while(Q.size())
    {
        ...
        if(x + 1 < 30 && dis[x + 1][u] > dis[x][u] + (1 << x))
        {
            dis[x + 1][u] = dis[x][u] + (1 << x);
            if(!inq[x + 1][u])
            {
                inq[x + 1][u] = 1;
                Q.emplace(mk(x + 1, u));
    ...
    LL tmp = LINF;
    for(int i = 0; i < 30; i++) get_min(tmp, dis[i][n]);

    if(tmp < LINF)
    {
        cout <<tmp % MOD_998 <<endl;
    }
    else
    {
        for(int i = 0; i < 2; i++)
            for(int j = 1; j <= n; j++)
                dis2[i][j] = INF, inq2[i][j] = 0, dis3[i][j] = INF;
        queue<pii> Q;
        Q.emplace(mk(0, 1));
        dis2[0][1] = 0;
        dis3[0][1] = 0;
        ...
        while(Q.size())
        {
            ...
            if(x == 0)
            {
                for(auto v : G[u])
                {
                    if(dis2[x][v] > dis2[x][u] || (dis2[x][v] == dis2[x][u] && dis3[x][v] > dis3[x][u] + 1))
                    {
                        dis2[x][v] = dis2[x][u];
                        dis3[x][v] = dis3[x][u] + 1;
                        pre[x][v] = mk(x, u);
        ...
        int mn = min(dis2[0][n], dis2[1][n]);

        if(dis2[0][n] == dis2[1][n])
        {
            int cnt = min(dis3[0][n], dis3[1][n]);
            mint ans = mint(2).pow(mn) - 1 + cnt;
            cout <<ans <<endl;
        }
        else
        {
            int cnt;
            int x, y;
            if(dis2[0][n] < dis2[1][n]) x = 0, y = n, cnt = dis3[0][n];
            else x = 1, y = n, cnt = dis3[1][n];
