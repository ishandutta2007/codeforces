...
// for len in 1..=n
//   for i in len..=n {
//     ans += binom(i - 1, len - 1) * (m - 1)^(i - len) * m^(n - i + len)
//
// for len in 1..=n
//   for i in 0..=(n - len) 
//     ans += binom(i + len - 1, len - 1) * (m - 1)^i * m^(n - i)
//
// for i in 0..n 
//   for len in 1..=(n - i) 
//     ans += binom(i + len - 1, len - 1) * (m - 1)^i * m^(n - i)
//
// for len in 1..=(n - i)
//   ans += binom(i + len - 1, len - 1)

fn run() {
    input!(n: usize, m: usize);
    let pc = precalc::Precalc::new(n + m);
    let mut pow = vec![(M::one(), M::one()); n + 1];
    for i in 1..=n {
        pow[i].0 = M::from(m) * pow[i - 1].0;
        pow[i].1 = M::from(m - 1) * pow[i - 1].1;
    }
    let mut ans = pow[n].0;
    for i in 0..n {
        ans += pc.comb(i + n - i, n - i - 1) * pow[i].1 * pow[n - i].0;
    }
    /*
    for i in 0..n {
        for len in 1..=(n - i) {
            ans += pc.comb(i + len - 1, len - 1) * pow[i].1 * pow[n - i].0;
        }
    }
    */
    /*
    for len in 1..=n {
        for i in len..=n {
            ans += pc.comb(i - 1, len - 1) * pow[i - len].1 * pow[n - i + len].0;
        }
    }
    */
    println!("{}", ans);
