...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> cnt(n+1);
	for(int x:a) ++cnt[x];
	vector<ll> sumc(n+2),sumca(n+2);
	rep(i,n+1){
		sumc[i+1]=sumc[i]+cnt[i];
		sumca[i+1]=sumca[i]+cnt[i]*i;
	}
	int t=accumulate(all(a),0ll)%2;
	ll tooS=-1,tooL=n+1;
	rep(k,n+1){
		if(k){ //small
			ll R=suma[n]-suma[n-k];
			ll L=k*(k-1ll);
			if(n-k==0 || a[n-k-1]<=k){
				L+=suma[n-k];
			}else{
			...
			ll rem=R-L;
			if(rem>0){
				if(rem>k) MX<ll>(tooS,a[n-k-1]);
				else MX<ll>(tooS,rem-1);
			}
		}
		{ //large
			ll R=suma[n]-suma[n-k];
			ll L=k*(k+1ll);
			if(n-k==0 || a[n-k-1]<=k+1){
				L+=suma[n-k];
			}else{
				L+=suma[sumc[k+2]];
				L+=(n-k-sumc[k+2])*(k+1);
			...
			ll rem=R-L;
			// out(k,L,R,1);
			if(rem+(k==n?0:a[n-k-1])>0){
				MN<ll>(tooL,(k==n?0:a[n-k-1]));
				// out((k==n?0:a[n-1-k]),1);
			}else{
				MN(tooL,-rem+1);
				// out(-rem+1,1);
