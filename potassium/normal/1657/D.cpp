...
int main() {
	...
	for (int i = 0; i < n; i++) {
		int c,d,h; R(c,d,h);
		if (mp.count(c) == 0 || mp[c] < 1LL * d * h) mp[c] = 1LL * d * h;
	}

	vector<pll> x(mp.begin(), mp.end());
	const int m = (int)x.size();

	vector<ll> maxv(C + 1, 0);
	for (int i = 0; i < m; i++)
		for (int j = 1; j * x[i].first <= C; j++)
			chkmax(maxv[j * x[i].first], 1LL * j * x[i].second - 1);

	for (int t = 1; t <= C; t++)
		chkmax(maxv[t], maxv[t - 1]);

	int q; R(q);
	for (int i = 0; i < q; i++) {
		ll d,h; R(d,h);
		int ans = lower_bound(maxv.begin(), maxv.end(), d * h) - maxv.begin();
		cout << (ans == C + 1 ? -1 : ans) << " \n"[i + 1 == q];
	}
