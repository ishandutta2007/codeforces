...
#endif
#ifndef CHTHOLLYTREE_H_
#define CHTHOLLYTREE_H_
...
namespace data_structure
{
	...
	public:
		struct Node
		{
			mutable RangeType left, right; // [left, right)
			mutable ValueType val;
			Node (RangeType left, RangeType right, ValueType val) : left(left), right(right), val(val) { }
			bool operator < (const Node& rhs) const
			{
				return left < rhs.left;
			}
			friend bool operator < (const Node& lhs, RangeType rhs)
			{
				return lhs.left < rhs;
			}
		};
		RangeType left_bound, right_bound;
		std::set<Node, std::less<>> data;
		ChthollyTree(std::vector<Node> data) : data(data.begin(), data.end())
		{
			assert(!data.empty());
			std::sort(data.begin(), data.end());
			for (int i = 1; i < (int)data.size(); ++i)
				assert(data[i - 1].right == data[i].left);
			left_bound = data.front().left;
			right_bound = data.back().right;
		}
		// Split [L, R) into [L, pos) and [pos, R)
		void Split(RangeType pos)
		{
			assert(left_bound <= pos && pos <= right_bound);
			if (pos == right_bound) return;
			auto it = data.lower_bound(pos);
			if (it != data.end() && it->left == pos) return;
			it--;
			Node l(it->left, pos, it->val);
			Node r(pos, it->right, it->val);
			data.erase(it);
			if (l.left < l.right) data.insert(l);
			if (r.left < r.right) data.insert(r);
		}
		template<typename Operation>
		void Operate(RangeType left, RangeType right, const Operation& operation)
		{
			assert(left_bound <= left && left <= right && right <= right_bound);
			Split(left);
			Split(right);
			for (auto it = data.lower_bound(left); it != data.end() && it->right <= right; ++it)
				operation(it->left, it->right, it->val);
			for (auto it = data.lower_bound(left); it != data.end() && it->left <= right; ++it)
				if (it != data.begin() && std::prev(it)->val == it->val)
				{
					it->left = std::prev(it)->left;
...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
		a[i] = rnd() % vmax + 1;
	using Tree = ChthollyTree<int, ll>;
	using Node = Tree::Node;
	vector<Node> v;
	for (int i = 1; i <= n; ++i)
		v.push_back(Node(i, i + 1, a[i]));
	Tree tree(v);
	for (int i = 1; i <= m; ++i)
	{
		int op = rnd() % 4 + 1;
		...
		{
			int x = rnd() % vmax + 1;
			tree.Operate(l, r + 1, [&](int left, int right, ll& val)
				{
					val += x;
				});
		}
		...
		else
		{
			int x = rnd() % vmax + 1;
			int y = rnd() % vmax + 1;
			modular::SetModular(y);
			modint res = 0;
			tree.Operate(l, r + 1, [&](int left, int right, ll& val)
				{
					res += modint(val).Pow(x) * (right - left);
				});
