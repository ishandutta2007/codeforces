#line 1 "codeforces-workspace/contest/1483/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1483/c/c.cpp"
...
void main() {
  ...
  for (auto i : range(n)) {
    ...
    s.emplace(i, now, max(op, now + b));

    // if (s.size()) {
    //   s.emplace(-1, now + b, max(now + b, get<2>(s.top())));
    // } else {
    //   s.emplace(-1, now + b, now + b);
    // }
  }

  cout << get<2>(s.top()) << endl;
}
