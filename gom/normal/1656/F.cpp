...
const int N=5e5+5,M=1e5+5,Q=1e6+5;
...
ll Do(ll x){
	ll r=0;
	if(a[1]+x>=0){
		for(int i=2;i<=n;i++) r+=a[1]*a[i]+x*(a[1]+a[i]);
		return r;
	}
	if(a[n]+x<0){
		for(int i=1;i<n;i++) r+=a[n]*a[i]+x*(a[n]+a[i]);
		return r;
	}
	for(int i=1;i<=n;i++){
		if(a[i]+x<0&&a[i+1]+x>=0){
			for(int j=i+1;j<n;j++) r+=a[1]*a[j]+x*(a[1]+a[j]);
			for(int j=2;j<=i;j++) r+=a[n]*a[j]+x*(a[n]+a[j]);
			r+=a[1]*a[n]+x*(a[1]+a[n]);
			return r;
		}
	}
