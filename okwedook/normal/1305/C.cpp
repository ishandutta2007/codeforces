...
signed main() {
    ...
    cin >> a;
    sort(a);
    vector<vector<zet>> poww(mod, vector<zet>(pmd));
    for (int i = 0; i < mod; ++i) poww[i][0] = 1;
    for (int i = 0; i < mod; ++i)
        for (int j = 1; j < pmd; ++j)
    ...
    for (auto i : a) {
        int now = i % mod;
        for (int was = 0; was <= now; ++was)
            ans *= poww[now - was][cnt[was]];
        for (int was = now + 1; was < mod; ++was)
            ans *= poww[now - was + mod][cnt[was]];
        ++cnt[now];
        if (cnt[now] == pmd) cnt[now] = 0;
    }
    cout << ans;
    return 0;
}