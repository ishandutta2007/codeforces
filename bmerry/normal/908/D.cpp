...
int main(int argc, const char **argv)
{
    ...
    int K, pas, pbs;
    cin >> K >> pas >> pbs;
    mr pa = mr(pas) / mr(pas + pbs);
    mr pb = mr(pbs) / mr(pas + pbs);
    vector<vector<mr>> state(K + 1, vector<mr>(K + 1));
    mr scale1 = pa / pb;
    state[1][0] = 1;
    mr ans;
    for (int a = 1; a <= K; a++)
        for (int s = 0; s <= K; s++)
        {
            ...
            {
                mr t = s + a;
                ans += (scale1 + t) * state[a][s];
            }
            else
            {
                state[a + 1][s] += pa * state[a][s];
                state[a][s + a] += pb * state[a][s];
            }
        }
    cout << ans << '\n';
