...
int sa[N],sb[N];
void get_colu(int f[],int n,int k){
	static int a[N],b[N];
	int top=1; for(;top<=n;top<<=1);
	memset(a,0,sizeof(int)*top);
	for(int i=1;i<=n;++i) a[i-1]=inv[i];
	POLY::ln(a,a,top);
	for(int i=0;i<top;++i) a[i]=1ll*a[i]*k%MOD;
	POLY::exp(a,b,top);
	for(int i=0;i<=n;++i){
		if(i<k) f[i]=0;
		else f[i]=1ll*ifac[k]*b[i-k]%MOD*fac[i]%MOD;
	}
}
int main(){
	...
	int ans=0;
	for(int i=1;i<=n;++i)
		ans=(ans+1ll*sa[i-1]*sb[n-i]%MOD*comb(n-1,i-1))%MOD;
	printf("%d\n",ans);
	return 0;
}