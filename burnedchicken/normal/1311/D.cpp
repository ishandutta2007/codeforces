...
//i_am_noob
int t,a,b,c,ans[maxn],ans1[maxn],ans2[maxn],k,minn,minid;
...
int main(){
	...
	while(t--){
		...
		rep2(i,1,maxn){
			k=c%i;
			ans[i]+=abs(b-i)+min(k,i-k);
			ans2[i]=(k<i-k?c/i*i:(c/i+1)*i);
		}
		minn=1e9;
		rep2(i,1,maxn){
			if(ans[i]<minn){
		...
		print1(ans[minid]);
		print3(ans1[minid],minid,ans2[minid]);
	}
	return 0;
}