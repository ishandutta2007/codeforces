...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	int n,t;
	cin>>n>>t;
	vector<mint> sgn(t+1,1);
	for(ll p:primes){
		for(ll i=0;i<=t;i+=p) sgn[i]=MOD-sgn[i];
		for(ll i=0;i<=t;i+=p*p) sgn[i]=0;
	}
	out(sgn,1);
	vector<mint> re(t+1),dif(t+2);
	reps(i,1,t+1) re[i]=modpow(i,n);
		out(re,1);
	reps(i,1,t+1){
		dif[i]+=dif[i-1];
		//re[i]+=dif[i];
	...
	mint ret=0;
	reps(i,1,t+1) out((re[i]+dif[i]).v,i,1);
	reps(i,1,t+1) ret+=((re[i]+dif[i]).v^i)%MOD;
	cout<<ret<<endl;
  return 0;
}