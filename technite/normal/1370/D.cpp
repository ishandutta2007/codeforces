...
//
bool solve(int mid,vector<int> v,int k)
{
    int count=0;
    vector<int> a;
    for(int i=0;i<v.size();i++)
    {
        if(a.size()%2) a.pb(v[i]);
        else if(v[i]<=mid) a.pb(v[i]);
    }
    if(a.size()>=k) return true;
    a.clear();
    for(int i=0;i<v.size();i++)
    {
        if(a.size()%2==0) a.pb(v[i]);
        else if(v[i]<=mid) a.pb(v[i]);
    }
    return (a.size()>=k);
}
...
int main()
{
    ...
    vector<int> v(n);
    int mn=1e9,mx=0;
    for(int i=0;i<n;i++) {cin>>v[i];mn=min(mn,v[i]);mx=max(mx,v[i]);}
        //cout<<mn<<" "<<mx<<'\n';   
    while(mn<mx)
    {
        int mid=mn +(mx-mn)/2;
        if(solve(mid,v,k)) mx=mid;
        else mn=mid+1;
        //cout<<mn<<" "<<mx<<'\n';
    }
    cout<<mn<<'\n';
