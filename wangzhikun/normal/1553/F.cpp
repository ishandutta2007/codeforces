...
int main() {
	...
	while(n--){
		...
		re(p);
		ans += sum.query(1,p-1);
		for(int i=p;i<=300000;i+=p){
			int r = min(i+p-1,300000);
			ans+=sum.query(i,r)-cnt.query(i,r)*i;
		}
		ans+=p*cnt.query(p,300000);
		int lwb = p,clb = p,crb;
		for(int i=2;i<40;i++){
			crb = clb-1;
			clb = p/i+1;
			if(clb<=crb){
				ans+=p*cnt.query(clb,crb)-sum.query(clb,crb)*(i-1);
				lwb = min(lwb,clb);
			}
		}
		FOR(i,1,lwb){
