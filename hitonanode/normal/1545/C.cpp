...
void solve() {
    int N;
    cin >> N;
    vector A(N * 2, vector<int>(N));

    cin >> A;
    for (auto &v : A) for (auto &x : v) x--;
    vector vpcnt(N, vector<int>(N));
    vector<int> is_line_fixed(N * 2);
    vector<int> is_line_banned(N * 2);
    vector<int> st;

    vector<vector<int>> to(N * 2);
    REP(i, N * 2) REP(j, i) {
        bool match = false;
        REP(t, N) if (A[i][t] == A[j][t]) match = true;
        if (match) {
            to[i].push_back(j);
            to[j].push_back(i);
        }
    ...
    vector vy(N, vector<int>(N));
    REP(i, N * 2) {
        bool fix = false;
        REP(j, N) {
            int a = A[i][j];
            if (vpcnt[a][j] == 1) fix = true;
        }
        if (fix) {
            is_line_fixed[i] = 1;
            st.push_back(i);
            REP(j, N) vy[A[i][j]][j] = 1;
        }
    }
    ...
    while (st.size()) {
        int x = st.back();
        st.pop_back();
        for (auto i : to[x]) if (!is_line_banned[i]) {
            is_line_banned[i] = 1;
            REP(y, N) {
                int v = A[i][y];
                vpcnt[v][y]--;
                if (vpcnt[v][y] == 1 and !vy[v][y]) {
                    REP(x, N * 2) if (!is_line_banned[x] and A[x][y] == v) {
                        is_line_fixed[x] = 1;
                        st.push_back(x);
                    }
                }
    ...
    vector<int> can_use;
    vector<int> is_can_use(N * 2);
    REP(i, N * 2) if (!is_line_fixed[i]) {
        bool match = false;
        for (auto j : to[i]) if (is_line_fixed[j]) match = true;
        if (!match) {
            can_use.push_back(i);
            is_can_use[i] = 1;
        }
    ...
    UnionFind uf(N * 2 * 2);
    for (auto i : can_use) {
        for (auto j : to[i]) if (is_can_use[j]) {
            uf.unite(i, j + N * 2);
            uf.unite(j, i + N * 2);
        }
    }
    ...
    while (mp.size()) {
        auto [r1, c1] = *mp.begin();
        int r2 = uf.find((r1 + N * 2) % (N * 4));
        int c2 = mp[r2];
        if (c1 > c2) fuse.insert(r1);
        else fuse.insert(r2);
        if (c1 == c2) deg++;
        mp.erase(r1);
        mp.erase(r2);
    ...
    mint retn = mint(2).pow(deg);
    vector<int> ret;
    REP(i, N * 2) if (is_line_fixed[i]) ret.push_back(i);
    for (auto i : can_use) if (fuse.count(uf.find(i))) ret.push_back(i);
    sort(ALL(ret));
    dbg(ret);
    assert(ret.size() == N);
    cout << retn << '\n';
