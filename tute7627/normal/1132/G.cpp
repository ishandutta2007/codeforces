...
int main(){
  ...
  vector<ll>in(n,-1),out(n,-1),dep(n,-1);
  ll time=0;
  rrep(i,0,n){
    if(in[i]!=-1)continue;
    auto dfs=[&](auto &&f,ll k,ll d)->void{
      in[k]=time++;
      dep[k]=d;
      for(auto to:g[k])f(f,to,d+1);
      out[k]=time;
    };
    dfs(dfs,i,1);
  }
  rep(i,0,n-k+1)seg.set(in[i],0);
  rep(i,n-k+1,n)seg.set(in[i],dep[i]);
  seg.build();
  vector<ll>ret(n-k+1);
  rrep(i,0,n-k+1){
    seg.update(in[i],in[i]+1,dep[i]);
    //seg.print();
    ret[i]=seg.query(0,n);
    seg.update(in[i+k-1],out[i+k-1],-1);
  }
  debug(ret,n-k+1);
  return 0;
}