...
template<typename E> struct lowest_common_ancestor {
    ...
    struct euler_tour {
        ...
        euler_tour(int V = 0) : first_visit(V), last_visit(V) {}
        int operator()(int i, int j) const {
            return tour_depth[i] < tour_depth[j] ? i : j;
        }
    } et;
    sparse_table<int, euler_tour> table;
    ...
    lowest_common_ancestor(const tree<E>& _t, int _root = 0) : t(_t), root(_root),
        ...
        table = sparse_table<int, euler_tour>(index, et);
    }
...
int main() {
    ...
    tr = tree<E>(N); re(tr);
    lowest_common_ancestor<E> hld(tr);
    ...
    assert(sz(tr.nbrs[0]) == sz(hld.children(0)));
    FOR (u, 1, N) assert(sz(tr.nbrs[u]) == sz(hld.children(u)) + 1);
    ...
    F0R (q, Q) {
        if (sz(todo) >= LIM) {
            ...
            vi nodes;
            F0R (u, N) if (adj_sum[u].v) nodes.pb(u);
            ...
            trav (node, nodes) {
                ...
                trav (nbr, tr.nbrs[node]) if (nbr != hld.par(node)) {
            ...
            for (int u : hld.preorder) {
                val[u] += adjust[u];
                trav (v, tr.nbrs[u]) if (v != hld.par(u)) adjust[v] += adjust[u];
            }
            todo.clear();
        }
