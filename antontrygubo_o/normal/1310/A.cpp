...
int main() {
    ...
    while (current<n)
    {
        level = a[current].first;
        while (current<n && a[current].first == level)
        {
            in.insert(a[current].second);
            cursum+=a[current].second;
            current++;
        }

        while (in.size()&&(current==n || (level<a[current].first)))
        {
            int x = *prev(in.end());
            cursum-=x;
            total+=cursum;
