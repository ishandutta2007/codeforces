...
int main(void)
{
	...
	rep(i, 1, n){
		...
		rep(j, 1, n){
			if(a[j] == -1) b[j] = -1;
			else if(a[j] < a[i] || (a[j] == a[i] && j < i)) b[j] = 1;
			else b[j] = 0;
		}
		b[i] = 0;
		...
		rep(j, 0, n) rep(k, 0, n) dp[j][k] = mint(0);
		dp[0][0] = mint(1);
		
		rep(j, 0, n-1) rep(k, 0, n){
			if(j+1 != i) dp[j+1][k] += dp[j][k];
			if(j+1 < i){
				if(k+b[j+1] <= n) dp[j+1][max(0LL, k+b[j+1])] += dp[j][k];
			}
			else{
				if(k+b[j+1] >= 0 && k+b[j+1] <= n) dp[j+1][k+b[j+1]] += dp[j][k];
			}
		}
		
		mint sum = mint(0);
		rep(k, 0, n) sum += dp[n][k];
		ans += sum * mint(a[i]);
	}
	outl(ans);
