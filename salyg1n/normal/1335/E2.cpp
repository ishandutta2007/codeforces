...
const int MOD = 998244353, MAXN = 1e5 + 10, MAX = 201;
...
void solve() {
	...
	for (int i = 1; i < MAX; ++i) {
		...
		for (int j = 1; j < MAX; ++j)
			c[j] = b[j].size() - 2;
		maxravno(ans, max(c, 1));
		for (int j = 1; j < b[i].size() / 2; ++j) {
			for (int k = b[i][j - 1] + 1; k <= b[i][j]; ++k)
				c[a[k]]--;
			for (int k = b[i][b[i].size() - j] - 1; k >= b[i][b[i].size() - j - 1]; --k)
				c[a[k]]--;
			maxravno(ans, j * 2 + max(c, 1));
		}
	}
	cout << ans << '\n';
}
