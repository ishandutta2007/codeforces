...
int main() {
    ...
    loop(t) {
        ...
        rep(i, n) {
            sat.set(id(i, k), true);
            rep(j, 1, k) {
                sat.implies(id(i, j), true, id(i, j + 1), true);
            }
        }
        rep(i, n - 1) rep(j, 1, k + 1) {
            sat.implies(id(i, j), false, id(i + 1, j), false);
        }
        ...
        loop(m) {
            ...
            if (qt == 1) {
                ...
                --i;
                if (x != 1) {
                    sat.add_clause(id(i, x - 1), true, id(i, x), false);
                } else {
                    sat.set(id(i, x), false);
                }
            } else if (qt == 2) {
                input(int, i, j, x);
                --i, --j;
                loop(2) {
                    for (int p = 1; p <= k; ++p) {
                        int q = x - p - 1;
                        if (1 <= q and q <= k) {
                            sat.implies(id(i, p), false, id(j, q), true);
                        } else if (q < 1) {
                            sat.set(id(i, p), true);
                        }
                    }
                ...
                loop(2) {
                    for (int p = 1; p <= k; ++p) {
                        int q = x - p - 1;
                        if (1 <= q and q <= k) {
                            sat.implies(id(i, p), true, id(j, q), false);
                        } else if (q > k) {
                            sat.set(id(i, p), false);
                        }
                    }
                    swap(i, j);
                }
        ...
        if (sat.satisfiable()) {
            ...
            vector<int> res(n, 1);
            rep(i, n) rep(j, 1, k) {
                if (not ans[id(i, j)] and ans[id(i, j + 1)]) res[i] = j + 1;
            }
            print(res);
        } else {
            print(-1);
