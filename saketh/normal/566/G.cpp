...
int main() {
    ...
    int N, M; re(N, M); pt st; re(st);
    vector<pt> hip(N), lop(M); re(hip, lop);

    ll mX = 0, mY = 0;
    trav (p, lop) { ckmax(mX, p.x); ckmax(mY, p.y); }
    ...
    lop.erase(lop.begin());
    reverse(all(lop));
    assert(is_sorted(all(lop), [](pt a, pt b){ return a.x < b.x; }));

    auto MAX = []{ ps("Max"); exit(0); };
    trav (p, hip) {
        if (p.x >= lop.back().x) MAX();
        auto it = lb(all(lop), p.x, [](pt p, ll x){ return p.x < x; });
        if (it->x == p.x) { if (it->y <= p.y) MAX(); }
        else if (cross(*prev(it), p, *it) <= 0) MAX();
    }
    ps("Min");
