...
void solve_case(int tc = 0) {
	...
	MaxSparseTable<int> st_mx;
	MinSparseTable<int> st_mn;
	st_mx.init(a);
	st_mn.init(a);
	int ans = 0;
	int cur = 0;
	while (cur != n - 1) {
		...
		if (a[cur] < a[cur + 1]) {
			...
			int loc;
			if (st_mn.query(cur, hi) == a[cur]) {
				loc = hi;
			} else {
				loc = lo;
			}
			cur = st_mx.index(cur, loc);
 		} else {
 			while (hi - lo > 1) {
				int mid = (lo + hi) >> 1;
				if (st_mx.query(cur, mid) == a[cur]) {
					lo = mid;
				} else {
					hi = mid - 1;
				}
			...
			int loc;
			if (st_mx.query(cur, hi) == a[cur]) {
				loc = hi;
			} else {
				loc = lo;
			}
			cur = st_mn.index(cur, loc);
 		}
	}
	ps(ans);
