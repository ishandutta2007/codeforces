...
for(int x=1;x<=n;x++)
cin>>d[x],sum[x]=sum[x-1]+d[x];
int sumlen=sum[n];
if(sumlen%2==1)ex(qp(m,n));
unordered_map<int,int,custom_hash>idsum;
vector<pair<int,int>>yz;
for(int x=1;x<=n;x++)
if(idsum[sum[x]-sumlen/2]==0)idsum[sum[x]]=x;
else yz.push_back({idsum[sum[x]-sumlen/2],x});
int zs=yz.size(),ans=0,jc=1;
for(int i=0;i<=zs&&i<=m;i++)
(ans+=qp(m-1-i,zs-i)*jc%p*qp(m-i,n-zs-i)%p*C(zs,i)%p)%=p,jc=jc*(m-i)%p;
cout<<ans<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
