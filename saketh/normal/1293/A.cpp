...
void solve() {
    ...
    set<int> closed;
    F0R (i, K) { int v; re(v); closed.insert(v); }

    int up = S; while (up <= N && closed.count(up)) up++;
    int dw = S; while (dw >= 1 && closed.count(dw)) --dw;

    ps(min(up > N ? INT_MAX : up - S, dw < 1 ? INT_MAX : S - dw));
}
