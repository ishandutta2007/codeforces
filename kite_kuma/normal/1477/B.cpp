...
bool s() {
	...
	function<monoid(monoid, monoid)> plus = [&](monoid a, monoid b) { return monoid(a.first + b.first, a.second + b.second); };
	function<monoid(monoid, ll)> mapping = [&](monoid a, ll b) {
		if(b == unit) return a;
		return monoid(ll(a.second) * b, a.second);
	};
	function<ll(ll, ll)> replace = [&](ll a, ll b) {
		if(b == unit) return a;
		return b;
	};
	vector<monoid> raw_data;
	rep(i, n) { raw_data.emplace_back(ll(f[i] - '0'), 1); }
	monoid u(0LL, 0);
	LazyST<monoid, ll> lst(n, plus, mapping, replace, u, unit, raw_data);

	lst.print();
	drep(i, q) {
		int l, r;
		tie(l, r) = v[i];
		monoid res = lst.get(l, r);
		ll d = res.first * 2;
	...
	string t;
	rep(i, n) { t += char('0' + lst.get(i, i + 1).first); }
	return t == s;
}
