...
i64 n;
i64 ans;
vector<int> zeros[1<<18],ones[1<<18];
i64 zsiz[1<<18],osiz[1<<18];
stack<int> track;
...
signed main() {
    cin>>n;
    init(zsiz,-1),init(osiz,-1);
    for(int ii=0;ii<n-1;++ii) {
        int x,y,t; cin>>x>>y>>t;
        x--,y--;
        if(t) {
            ones[x].emplace_back(y);
            ones[y].emplace_back(x);
        } else {
            zeros[x].emplace_back(y);
            zeros[y].emplace_back(x);
        }
    }
    for(int i=0;i<n;++i) {
        i64 siz=zdfs(i);
        while(!track.empty()) {
            int t=track.top(); track.pop();
            zsiz[t]=siz-1;
        }
        ans+=siz*(siz-1);
        siz=odfs(i);
