...
void solve()
{
    ...
    int n, k;
    cin >> n >> k;
    vector<int> cnt_cards(N), cnt_players(N), h(k + 1);
    for (int i = 0; i < n * k; i++)
    {
        int a;
        cin >> a;
    ...
    ll ans = 0;
    vector<vector<ll> > dp(n * k + 1, vector<ll>(n + 1, -INFLL));
    for (int j = 0; j <= n * k; j++) dp[j][0] = 0;
    for (int p = 0; p < n; p++)
    {
        for (int c = 0; c <= n * k; c++)
        {
            for (int e = 0; e <= k && c + e <= n * k; e++)
            {
                dp[c + e][p + 1] = max(dp[c + e][p + 1], dp[c][p] + h[e]);
            }
        }
    }
    for (int c = 0; c < N; c++)
    {
        //cout << cnt_cards[c] << " " << cnt_players[c] << endl;
        //cout << dp[cnt_cards[c]][cnt_players[c]] << endl;
        ans += dp[cnt_cards[c]][cnt_players[c]];
    }
    cout << ans << "\n";
}
