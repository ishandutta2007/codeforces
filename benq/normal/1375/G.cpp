...
T go(int x, int y) {
	T cur = T();
	trav(t,adj[x]) if (t != y) cur = cur+go(t,x);
	return tran(cur);
}
...
T getEdge(int x, int y) {
	if (par[x] == y) return down[x];
	assert(par[y] == x);
	return up[y];
}
...
int main() {
	...
	dfs(1); //exit(0);
	DFS(1);
	FOR(i,1,n+1) dbg(i,down[i],up[i]);
	int ans = MOD;
	FOR(i,1,n+1) {
		T p = T();
		trav(t,adj[i]) p = p+getEdge(t,i); // getEdge(t,i);
		ckmin(ans,p[0]);
	}
	ps(ans);
