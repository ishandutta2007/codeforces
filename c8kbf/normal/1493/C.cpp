...
inline void AC() {
    ...
    bool kk = true;
    for(_ i = 0; i < 26; ++i) if(b[i] % k) kk = false;
    if(kk) {
        for(_ i = 1; i <= n; ++i) putchar(a[i]+'a');
        clr();
        return;
    }
    for(_ i = n; i >= 1; --i) {
        for(_ j = a[i]+1; j < 26; ++j) {
            --b[a[i]];
            ++b[j];
            _ df = 0;
            for(_ l = 0; l < 26; ++l) {
            ...
            if(df <= n-i && !((n-i-df)%k)) {
                for(_ l = 1; l < i; ++l) putchar(a[l]+'a');
                putchar((char)j+'a');
                for(_ l = df+1; l <= n-i; ++l) putchar('a');
                for(_ l = 0; l < 26; ++l) {
                    _ ad = k-(b[l]%k);
                    if(ad == k) ad = 0;
                    for(_ jj = 1; jj <= ad; ++jj) putchar((char)l+'a');
                }
                clr();
                return;
            }
