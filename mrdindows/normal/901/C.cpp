...
vector<int> vp[310101];
vector<pii> vq[310101];
int where[303030];
ll tr[1203003];
...
ll getSum(int cur, int l, int r, int x, int y, int maxVal) {
    if (where[l] >= maxVal) {
        return maxVal * (y - x + 1LL);
    }
    if (where[r] <= maxVal && x == l && y == r) {
        return tr[cur];
    }
    int m = (l + r) / 2;
    int dcur = cur + cur;
    if (y <= m) {
        return getSum(dcur, l, m, x, y, maxVal);
    } else if (x > m) {
        return getSum(dcur + 1, m + 1, r, x, y, maxVal);
    } else {
        return getSum(dcur, l, m, x, m, maxVal) + getSum(dcur + 1, m + 1, r, m + 1, y, maxVal);
    }
}
int solve() {
    int n, m;
    ...
    for (auto p : v) {
        s.insert(p.second);
        vp[p.first + 1].push_back(p.second);
    }
    s.insert(n);
    for (int i = 0; i < n; ++i) {
        for (auto q : vp[i]) {
...
//        DBN(i, where[i]);
    ...
    while (q--) {
        ...
        cin >> x >> y;
        --x; --y;
        cout << getSum(1, 0, n - 1, x, y, y) - (y + x - 2) * (y - x + 1LL) / 2 << '\n';
    }
    return 0;
}