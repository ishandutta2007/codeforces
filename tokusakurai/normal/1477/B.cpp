...
int main(){
    ...
    while(T--){
        ...
        auto g = [](pii a, int b) {return b == -1? a : pii(b*a.second, a.second);};
        auto h = [](int a, int b) {return b == -1? a : b;};

        vector<pii> v(N);
        rep(i, N) v[i] = pii(T[i]-'0', 1);

        Lazy_Segment_Tree<pii, int> seg(v, f, g, h, pii(0, 0), -1);
        ...
        rep3(i, Q-1, 0){
            int x = seg.query(L[i], R[i]).first;
            int n = R[i]-L[i];
            if(x*2 == n) {flag = false; break;}
        ...
        rep(i, N){
            if(seg[i].first != S[i]-'0') flag = false;
        }
