...
int main(){
  ...
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  vector<map<ll,ll>>mp(sz);
  vector<map<ll,ll>>mi(sz);
  modint ret=1;
  auto add=[&](ll i,ll x){
    auto fact=osak.factoring(x);
    for(auto z:fact){
      ll now=0;
      ll pre=0;
      if(mp[z.fi].size()==n&&!mi[z.fi].empty())pre=mi[z.fi].begin()->fi;
      if(mp[z.fi].count(i)){
        now=mp[z.fi][i];
        mi[z.fi][now]--;
        if(mi[z.fi][now]==0)mi[z.fi].erase(now);
      }
      mi[z.fi][now+z.se]++;
      mp[z.fi][i]=now+z.se;
      ll aft=0;
      if(mp[z.fi].size()==n&&!mi[z.fi].empty())aft=mi[z.fi].begin()->fi;
      ret*=pow(z.fi,aft-pre);
    }
  };
  rep(i,0,n)add(i,a[i]);
  while(q--){
