...
class BSet (val things:IntArray){
    fun ceiling(a:Int):Int?{
        ...
        return if(left == things.size) null else things[left]
    }
    fun floor(a:Int):Int?{
        var left = 0
        var right = things.size
        ...
        return if(left == 0) null else things[left - 1]
    }
}
...
fun main(){
    solve.cases{
        ...
        for( en in arr.entries){
            ...
            var last = nv!!
            while(true){
                val atleast = (last - v) * 2 + v
                val nnv = S.ceiling(atleast)
                if(nnv != null){
                    here ++
                    last = nnv
                }else{
