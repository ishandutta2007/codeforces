...
struct segtree {
	struct node {
		int sum, mn, lz;
		node(int sum = 0, int mn = inf, int lz = 0) : sum(sum), mn(mn), lz(lz) {}
		#define ls(p) (p << 1)
		#define rs(p) (p << 1 | 1)
	...
	void sprdwn(int p, int l, int r) {
		int mid = l + r >> 1;
		if(lz(p)) {
			sum(ls(p)) = mid - l + 1, lz(ls(p)) = true;
			sum(rs(p)) = r - mid, lz(rs(p)) = true;
			lz(p) = false;
		}
	}
	void sprup(int p) {
	...
	void add(int l, int r, int p = 1, int tl = 1, int tr = n) {
		if(l <= tl && r >= tr) return sum(p) = tr - tl + 1, lz(p) = true, void();
		sprdwn(p, tl, tr);
		int mid = tl + tr >> 1;
		if(l <= mid) add(l, r, ls(p), tl, mid);
		if(r > mid) add(l, r, rs(p), mid + 1, tr);
		sprup(p);
	}
	int _sum(int l, int r, int p = 1, int tl = 1, int tr = n) {
		if(l > r) return 0;
		if(l <= tl && r >= tr) return sum(p);
		sprdwn(p, tl, tr);
		int mid = tl + tr >> 1, res = 0;
		if(l <= mid) res += _sum(l, r, ls(p), tl, mid);
		if(r > mid) res += _sum(l, r, rs(p), mid + 1, tr);
		return res;
	}
	void chk(int x, int v, int p = 1, int tl = 1, int tr = n) {
		if(tl == tr) return chkmn(mn(p), v), void();
		sprdwn(p, tl, tr);
		int mid = tl + tr >> 1;
		if(x <= mid) chk(x, v, ls(p), tl, mid);
		else chk(x, v, rs(p), mid + 1, tr);
		sprup(p);
	}
	int _mn(int l, int r, int p = 1, int tl = 1, int tr = n) {
		if(l > r) return inf;
		if(l <= tl && r >= tr) return mn(p);
		sprdwn(p, tl, tr);
		int mid = tl + tr >> 1, res = inf;
		if(l <= mid) chkmn(res, _mn(l, r, ls(p), tl, mid));
		if(r > mid) chkmn(res, _mn(l, r, rs(p), mid + 1, tr));
		return res;
	}
} sgt;
...
void mian() {
	...
	while(qu--) {
		...
		if(tp == 0) {
			...
			int x = read();
			if(sgt._sum(x, x)) puts("NO");
			else {
				int fd = x;
				PER(i, 20, 0) if(fd > 1 << i && (sgt._sum(fd - (1 << i), x) == x - (fd - (1 << i))))
					fd -= 1 << i;
				int mn = sgt._mn(fd, x);
				if(mn == inf) puts("N/A");
				else {
					if(sgt._sum(x, mn) == mn - x) puts("YES");
					else puts("N/A");
				}
			}
		}
