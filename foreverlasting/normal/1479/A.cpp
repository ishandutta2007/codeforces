...
namespace MAIN{
    int n;
    int a[N];
    inline int get(const res &i){
        printf("? %d\n",i),fflush(stdout);
        return read();
    }
    inline void MAIN(){
        n=read(),a[0]=n+1,a[n+1]=n+1;
        res l=1,r=n;
        while(l<=r){
            res mid=(l+r)>>1;
            for(res i=mid-1;i<=mid+1;i++)if(!a[i])a[i]=get(i);
            if(a[mid-1]>a[mid]&&a[mid]<a[mid+1]){printf("! %d\n",mid),fflush(stdout);return;}
        ...
        if(r<=n&&r>=1){
            res mid=r;
            for(res i=mid-1;i<=mid+1;i++)a[i]=get(i);
            if(a[mid-1]>a[mid]&&a[mid]<a[mid+1]){printf("! %d\n",mid),fflush(stdout);return;}
        }
    }
}
