...
int n;
ll ans[200002],suf[200002];
int main() {
	init_inv_with_fac(1050000);
	NTT_set(1050000);
	cin>>n;A.resize(n),B.resize(n);
	F(i,(n+1)>>1,n)A[i]=fac[n-i-1];
	F(i,0,n)B[i]=ifac[i];
	A=A*B;
	F(i,1,n)ans[i]=1ll*A[n-i+1]*fac[n-i]%mod*(i-1)%mod;
	ll sum=fac[n-1];
	UF(i,n,2){
		suf[i]=1ll*ans[i]*inv[i-1]+suf[i+1];
		suf[i]%=mod;
	}
	UF(i,n,2){
		
		ans[i-1]=ans[i-1]+mod-suf[i];
		sum-=ans[i];
	}
	sum=(sum%mod+mod)%mod;write(sum,' ');
	F(i,2,n)write(ans[i]%mod,' ');
	return 0;
}