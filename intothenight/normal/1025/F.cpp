...
int main(){
	...
	long long res = 0;
	for(auto i = 0; i < n; ++ i){
		vector<point<long long>> b(a);
		b.erase(find(b.begin(), b.end(), a[i]));
		sort_by_angle(b.begin(), b.end(), a[i]);
		auto pivot = partition_point(b.begin(), b.end(), [&](auto p){ return p > a[i]; });
		deque<point<long long>> left(b.begin(), pivot), right(pivot, b.end());
		auto calc = [&](int x){
			return x * (x - 1) / 2;
		};
		auto next = [&](int i){
		...
		for(auto s = 0, t = 0; s < n - 1; ++ s){
			while(next(t) != s && ori(a[i], b[s], b[next(t)]) > 0){
				t = next(t);
			}
			int sz = s <= t ? t - s + 1 : t + n - 1 - s + 1;
			res += calc(sz - 1) * 1LL * calc(n - 1 - sz);
			if(s == t){
				t = next(t);
			}
		}
