...
lld modpow(lld a, lld n){
	if (n == 0) return 1;
	lld tmp = modpow(a, n/2);
	tmp = tmp * tmp % P;
	if (n & 1) tmp = tmp * a % P;
	return tmp;
}
...
int main(){
	...
	while(mp.begin()->second % x == 0){
		it = mp.begin();
		lld pa = it->first, pb = it->second;
		if (mp.find(pa+1) == mp.end()) mp[pa+1] = 0;
		mp[pa+1] = mp[pa+1] + pb / x;
		mp.erase(mp.begin());
	}
	ans = ans * modpow(x, min(mx, mp.begin()->first)) % P;
	cout << ans << endl;
	return 0;
}