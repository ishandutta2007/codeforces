...
inline bool check(multiset<int> &a,int len,int cc)
{
    return (*a.begin()==1&&*a.rbegin()==len&&!cc);
}
int main()
{
    ...
    while(T--)
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(suf.find(a[i])!=suf.end()) sc++;
            suf.insert(a[i]);
        }
        vc<pi> ans;
        for(int i=0;i<n-1;i++)
        {
            if(pre.find(a[i])!=pre.end()) break;
            pre.insert(a[i]);
            suf.erase(suf.find(a[i]));
            if(suf.find(a[i])!=suf.end()) sc--;
            int pcnt=i+1,scnt=n-pcnt;
            if(check(pre,pcnt,pc)&&check(suf,scnt,sc))
                ans.emplace_back(pcnt,scnt);
        }
        print(ans.size());
