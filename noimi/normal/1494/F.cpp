...
template <typename T> vector<edge<T>> eulerian_path(Edges<T> es, int s, bool directed) {
    if(empty(es)) return vector<edge<T>>();
    int V = 0;
    ...
    rep(i, n) {
        ...
        if(F) {
            ...
            if(uf.size(i) != target) {
                ...
                bool flag = false;
                rep(j, n) {
                    if(used[j] and !uf.same(i, j) and uf.size(i) + uf.size(j) == target) {
                        flag = true;
                        used[j] = false;
                        nxt.eb(j);
                        break;
            ...
            rep(j, n) {
                if(i == j) {
                    for(auto e : g[i]) E.eb(i, e, 0);
                } else
                    for(auto e : g[j])
                        if(j < e and e != i) E.eb(j, e, 0);
            }

            int st = i;
            rep(j, n) {
                if(!used[j] and si(g[j]) & 1 and i != j) st = j;
            }

            auto w = eulerian_path(E, st, false);
            vi v{st};
            for(auto e : w) v.eb(e.to);
            v.eb(-1);
            rep(j, n) if(used[j]) v.eb(j), v.eb(i);
            cout << si(v) << endl;
            for(auto e : v) {
                if(e == -1)
                    cout << e << " ";
