...
fun List<Int>.issorted():Boolean{
    return (0..size-2).all { this[it+1] >= this[it] }
}
fun List<Int>.removal():Int{
    var ret = 1
    for((i,v) in this.groupBy { it }.entries){
        ret = ret modM (FACT.store[v.size])
    }
    return ret
}
...
fun main(){
    ...
    solve.cases{
        ...
        val st = all.sortWith(compareBy<Pair<Int, Int>> { it.first }.thenBy { it.second })
        val stt2 = all.map{it.second}

        val base = FACT.store[n]
        val base1 = all.map{it.first}.removal()
        val base2 = all.map{it.second}.removal()
        var removed = 0
        if(stt2.issorted()){
            removed =1
            for((a,v) in all.groupBy { it.first }.entries){
                removed = removed modM v.map{it.second}.removal()
            }
        }
        put(removed modPlus base modMinus (base1 modPlus base2))
    }
    done()
}
