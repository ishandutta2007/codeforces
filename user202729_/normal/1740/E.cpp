// time-limit: 1000
// problem-url: https://codeforces.com/contest/1740/problem/E
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
int main(){
	...
	for(int const i: range(1, numNode)){
		int const par=read<int>()-1;
		add[par].push_back(i);
	}
	auto const work=makeYComb([&](auto work, int node)->std::pair<int, int>{
		// return (max depth below (leaf is 1), max value (including the case of using 1 chain with max depth))
		int maxDepth=1;
		int maxValue=0;
		for(auto child: add[node]){
			auto const [maxDepth1, maxValue1]=work(child);
			maxValue+=maxValue1;
			maxDepth=std::max(maxDepth, maxDepth1+1);
		}
		maxValue=std::max(maxValue, maxDepth);
		return {maxDepth, maxValue};
	});
	std::cout<<work(0).second<<'\n';


}

// samples are a bit weak actually