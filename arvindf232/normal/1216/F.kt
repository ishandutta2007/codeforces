...
class minstack {
    ...
    fun add(a:Long){
        val newMin = if(stack.isEmpty()) a else minOf(stack.first().second,a)
        stack.addFirst(Pair(a,newMin))
    }
    val size:Int
        get(){
            return stack.size
...
fun main(){
    solve.cases{
        val n = getint()
        val k = getint()
        val L = getstr().map{it == '1'}

        val large = 1000000000000000000L

        val DL = LongArray(n){Long.MAX_VALUE}
        val DA = LongArray(n){Long.MAX_VALUE}
...
//        val Aqueue = minqueue()
        ...
        for(i in 0 until n){
            if(L[i])
                 DL[i] = i + 1 + minOf((Lqueue.minimum ?: large),DA.getOrNull(i - k - 1) ?: 0)

            DA[i] = minOf(DL[i],(Lqueue.minimum ?: large), i + 1 + (DA.getOrNull(i-1) ?: 0))
            Lqueue.addLast(DL[i])
//            Aqueue.addLast(DA[i])
            if(Lqueue.size > k) Lqueue.removeFirst()
//            if(Aqueue.size > k) Aqueue.removeFirst()
        }
        put(DA.last())
