...
void solve()
{
	...
	for (int i = 1; i <= q; ++i)
	{
		if (e[i].t == 0 && e[i].z == 1)
		{
			int cnt = live[e[i].y] - live[e[i].x - 1];
			if (cnt == 1)
			{
				int tmp = tree.Query(e[i].x - 1, e[i].y).first;
				freshmax(tmp, e[i].id);
				freshmin(mark[*H.lower_bound(e[i].x)], tmp);
			}
		}
	}
