...
template<typename Node, typename Tag> struct Segment_Tree_Lazy
{
	...
	function<Node(Node,Node)> combine;
	function<Node(Node,Tag,int,int)> upd;
	function<Tag(Tag,Tag)> combine_tags;
	void init(int _n, Node _neutral_node, Tag _neutral_tag, function<Node(Node,Node)> f, function<Node(Node,Tag,int,int)> g, function<Tag(Tag,Tag)> h)
	{
	...
	void Set(int c, int ss, int se, int qi, Node f)
	{
		...
		val[c]=combine(val[c<<1],val[c<<1|1]);
	}
	...
	Node Get(int c, int ss, int se, int qs, int qe)
	{
		...
		return combine(Get(c<<1,ss,mid,qs,qe),Get(c<<1|1,mid+1,se,qs,qe));
	}
...
int main()
{
	...
	for(int i=0;i<A;i++) ST[i].init(n,0,-1,[](int x, int y){ return x+y;},[](int x, int y, int ss, int se){ return y==-1?x:(se-ss+1)*y;},[](int x, int y){ return y==-1?x:y;});
