...
template <class T, class F> struct UnionFind2 {
    vector<T> data;
    vector<int> sz;
    int num;
    const F f;
    ...
    UnionFind2(int n, const F &f = plus<T>(), const T &unit = T()) : num(n), f(f), unit(unit), sz(n, -1), data(n, unit) {}
...
template <typename T> struct Parallel_binary_search {
    // 
    int Q;
    vector<T> ok, ng, mid;
    vector<int> idx;
    Parallel_binary_search(int Q, T OK, T NG) : Q(Q), ok(Q, OK), ng(Q, NG), mid(Q), idx(Q) { iota(begin(idx), end(idx), 0); }
...
// https://atcoder.jp/contests/joi2012ho/submissions/27295749
