...
int query(int p, int l, int r, int L, int R) {
	...
	int mid = (l + r) >> 1, res = 999999; spread(p);
	if (L <= mid) Mn(res, query(ls, l, mid, L, R));
	if (R > mid) Mn(res, query(rs, mid + 1, r, L, R));
	return res;
}
...
int main() {
	...
	for (int i = 1;i <= n; ++i) {
		int t = 9999999;
		Mn(t, query(1, 0, n, 0, a[i] - 1));
		if (a[i] != n) Mn(t, query(1, 0, n, a[i] + 1, n));
		++t, Mn(t, query(1, 0, n, a[i], a[i]));
