...
int main(){
	...
	forn(i,n-1){
		...
		int l = s.length();
		string send = s;
		send += curr.substr(max((int)0,(int)(curr.length()-l)),l);
		// cout<<i<<" "<<send<<endl;
		v32 z;
		z_func(send,z);
		int ma = 0;
		// forn(i,send.length()) cout<<i<<" "<<z[i]<<ln;
		forsn(i,s.length(),send.length()){
			if(z[i]+i>= send.length() && z[i]>0) ma = max(ma,z[i]);
		}
		curr += s.substr(ma,s.length()-ma);
	}
	cout<<curr<<ln;
