...
int main()
{
    ...
    if (posa.size()<=k+2 || posb.size()<=k) {cout<<n<<endl; return 0;}

    int ans = 0;
    for (int i = 0; i+k+1<posa.size(); i++) ans = max(ans, posa[i+k+1] - posa[i] - 1);
    for (int i = 0; i+k+1<posb.size(); i++) ans = max(ans, posb[i+k+1] - posb[i] - 1);
