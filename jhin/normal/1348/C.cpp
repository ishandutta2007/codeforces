        ...
        while(t--)
        {
            cin>>n>>k>>s;map<char,int>tmp;string nus[k],nus2[k];string smx="",smx2="";
            sort(s.begin(),s.end());
            for(int i=0;i<n;i++)tmp[s[i]]++;
            for(int i=0;i<k;i++)nus[i]+=s[i],nus2[i]+=s[i];
              for(int i=k;i<n;i++)nus[0]+=s[i];
              ...
              for(int i=0;i<k;i++)
              {
                  ...
                  if(smx2<nus2[i])smx2=nus2[i];
              }
              for(int i=0;i<k;i++)
              {

                  if(smx<nus[i])smx=nus[i];
              }
              cout<<min(smx,smx2)<<endl;
        }
