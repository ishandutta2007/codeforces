...
fun main(){
    solve.cases{
        ...
        for(i in 0 until n){
            for(j in 0 until m){
                if(i < 2 || j == 0 || j == m-1) continue
                val here = board[i][j]
                if(here != board[i-1][j] || here != board[i-2][j] || here != board[i-1][j-1] || here != board[i-1][j+1]) continue
                DP[i][j] = minOf(DP[i-1][j-1] , DP[i-1][j+1], DP[i-2][j]) + 1
            }
        }
        var ret = 0L
        for(i in 0 until n){
