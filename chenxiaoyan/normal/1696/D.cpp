...
constexpr int N = 2.5e5 + 10, LOG_N = 20;
...
int n;
int a[N], b[N], pos[N];
int mnp(int l, int r) { return pos[-stm._mx(l, r)]; }
int mxp(int l, int r) { return pos[st._mx(l, r)]; }

int solve(int l, int r) {
  int mn = mnp(l, r), mx = mxp(l, r);
  if(mn != l && mn != r) return solve(l, mn) + solve(mn, r);
  else if(mx != l && mx != r) return solve(l, mx) + solve(mx, r);
  else return l != r;
}

void mian() {
  n = read();
  REP(i, 1, n) a[i] = read(), b[i] = -a[i], pos[a[i]] = i;
  st.init(n, a), stm.init(n, b);
  int ans = solve(1, n);
  prt(ans), pc('\n');
}
