...
int solve(vi x,int bit)
{
    ...
    //cout<<bit<<" "<<on.size()<<" "<<off.size()<<endl;
    if(on.size()>=2&&off.size()>=2)
    {
        temp+=min(on.size()-1+solve(off,bit-1),off.size()-1+solve(on,bit-1));
    }
    else if(on.size()<=1&&off.size()<=1)
    {
        return 0;
    }
    else if(on.size()>1)
