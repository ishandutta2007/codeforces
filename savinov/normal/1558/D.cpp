...
void solve(int test_number) {
    ...
    forn(i, m) {
        int x = nxt() - 1;
        if (x - prev_x - 1 > 0) {
            r += crope(x - prev_x - 1, '0');
        }
        prev_x = x;
        size_t y = nxt() - 1;
        if (r[y] == '1') {
            r = r.substr(0, y) + "0" + r.substr(y, r.length() - y);
        } else {
            r = r.substr(0, y + 1) + "1" + r.substr(y + 1, r.length() - y - 1);
            ++cc;
        }
        assert(r.length() == x + 1);
    }
    ...
    long long res = f[v] * inv(f[v - k], mod) % mod * inv(f[k], mod) % mod;
