// Training Session (https://codeforces.com/contest/1598/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        // Three equal a's
        for (auto& [_, bs] : by_a)
            ans += sz(bs) * ll(sz(bs) - 1) * (sz(bs) - 2) / 6;

        // Two equal a's and one different
        map<int, ll> pairs_by_a, pairs_by_b;
        ll all_pairs = 0;
        for (auto& [a, bs] : by_a) {
            all_pairs += (pairs_by_a[a] = sz(bs) * ll(sz(bs) - 1) / 2);
            trav(b, bs)
                pairs_by_b[b] += sz(bs) - 1;
        }
        for (auto& [a, bs] : by_a)
            trav(b, bs)
                ans += all_pairs - pairs_by_a[a] - pairs_by_b[b] + (sz(bs) - 1);
