...
int n,m;
vc<int> e[MAXN], re[MAXN];
vc<pii> es;

void FindCycle(){
	static int vis[MAXN], inst[MAXN];
	function<void(int,int)> Dfs = [&]( int pos , int col ){
		vis[pos] = col;
		inst[pos] = true;
		for( int v : e[pos] ){
			if( inst[v] and vis[v] == col ) END(-1);
			if(vis[v]) continue;
			Dfs(v,col);
		}
		inst[pos] = false;
...
void Solve(){
	static int dp[MAXN];
	function<int(int,vc<int>[])> Dfs = [&]( int pos , vc<int> g[] ){
...
int main(){
	...
	int ans = 0;
	For(i,n) if(!isE[i]) ++ans;
	printf("%d\n",ans);
	For(i,n) putchar(isE[i]?'E':'A');
	putchar(10);
