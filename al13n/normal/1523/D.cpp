...
int main(int argc, char **argv){
  ios_base::sync_with_stdio(false);cin.tie(0);cout.precision(20);srand(time(0));mtrng.seed(time(0));
  ...
  pair<int,string> ans(0,string(m,'0'));
  forn(qq,60){
    int i0=mtrng()%sz(A);
    vector<int> P;
    forn(i,m) if(A[i0][i]=='1') P.pb(i);
    vector<int> B(1<<sz(P));
    forn(i,n){
      int t=0;
      forv(j,P) if(A[i][P[j]]=='1') t|=1<<j;
      ++B[t];
    }
    forv(q,P) forv(i,B) if(!(i&(1<<q))) B[i]+=B[i|(1<<q)];
    forv(t,B){
      int r=__builtin_popcount(t);
      if(r<=ans.X || B[t]<(n+1)/2) continue;
      string s(m,'0');
      forv(i,P) if(t&(1<<i)) s[P[i]]='1';
      ans=mp(r,s);
    }
  }
  cout<<ans.Y;
