...
void work()
{
    ...
    auto ck = [](vector<string> &ss, int xx, int yy)
    {
        for(int i = -1; i <= 1; i++)
            for(int j = -1; j <= 1; j++)
            {
                int x = xx + i;
                int y = yy + j;
                if(0 <= x && x < n && 0 <= y && y < m && ss[x][y] == '1') return 0;
            }
        return 1;
    };
    ...
    int cnt1 = 0;
    for(int i = 1; i < n; i++)
        if(ck(s1, i, 0) == 1) s1[i][0] = '1', cnt1++;
    for(int j = 0; j < m; j++)
        if(ck(s1, n - 1, j) == 1) s1[n - 1][j] = '1', cnt1++;
    for(int i = n - 1; i >= 0; i--)
        if(ck(s1, i, m - 1) == 1) s1[i][m - 1] = '1', cnt1++;
    for(int i = m - 1; i >= 0; i--)
        if(ck(s1, 0, i) == 1) s1[0][i] = '1', cnt1++;
    ...
    int cnt2 = 0;
    for(int i = 0; i < n; i++)
        if(ck(s2, i, 0) == 1) s2[i][0] = '1', cnt2++;
    for(int j = 0; j < m; j++)
        if(ck(s2, n - 1, j) == 1) s2[n - 1][j] = '1', cnt2++;
    for(int i = n - 1; i >= 0; i--)
        if(ck(s2, i, m - 1) == 1) s2[i][m - 1] = '1', cnt2++;
    for(int i = m - 1; i >= 0; i--)
        if(ck(s2, 0, i) == 1) s2[0][i] = '1', cnt2++;
