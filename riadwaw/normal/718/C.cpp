...
template <>
struct IdentityHelper<MX> {
	static MX identity() {
		MX res;
		for (std::size_t i = 0; i < 2; ++i) {
			res.r[i][i] = Z::rawValueOf(1);
		}
		return res;
	}
};
...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		struct Add {
			Fib operator() (const Fib& l, const Fib& r) const {
				return {l.cur + r.cur, l.next + r.next};
			}
		};

		struct ApplyUpdate {
			Fib operator() (const Fib& old, const MX& mx, size_t l = 0, size_t r = 0) const {
				Fib res;
				res.cur = mx.r[0][0] * old.cur + mx.r[0][1] * old.next;
				res.next = mx.r[1][0] * old.cur + mx.r[1][1] * old.next;
				return res;
			}
		};
		...
		for (int i: range(n)) {
			fibs[i].cur = Z();
			fibs[i].next = Z::rawValueOf(1);
			int x;
			in >> x;
			for (int pw: range(30)) {
				if (x & (1 << pw)) {
					fibs[i] = ApplyUpdate()(fibs[i], powers[pw]);
				}
			}
		}


		TopDownSegmentTree<Fib, Add, MX, ApplyUpdate, std::multiplies<MX>> tree(fibs, Fib{}, Add(), identity<MX>());
