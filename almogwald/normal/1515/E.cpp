...
int main() {
	...
	cin.tie(0);
	defn defm
	vector<vevecl> a(n+1, vevecl(n+1,veci(2)));
	a[1][1][1] = 1;
	lol ans = 0;
	fort(i, 2, n + 1) {
		forn(j, i + 1) {
			...
			forn(k, j + 1) {
				if (k == 1) {
					if (j < i) {
						forn(kk, j) {
							a[i][j][k] = (a[i][j][k] + a[i - 2][j - 1][kk]) % m;
						}
						a[i][j][k] = ((lol)a[i][j][k] * j) % m;
					}
				}
				else {
					a[i][j][k] = ((lol)a[i - 1][j - 1][k - 1] * 2*j) % m;
					a[i][j][k] = ((lol)a[i][j][k] * modInverse(k,m)) % m;
				}
				if (i == n) {
					ans = (ans + a[i][j][k]) % m;
				}
