...
fun main(){
    solve.cases{
        ...
        var best = plarge
        for(l in 0 until n){
            val r = BinarySearchLastTrue(l,n-1){
                S.sumQuery(l,it) <= s
            }
            if(r == null || S.sumQuery(l,r).toInt() != s) continue
            best = minOf(best, n - (r - l + 1))
        }
        if(best == plarge){
            put(-1)
        }else{
...
//                val r = i + s - 1
//                if(r !in 0 until index.size) continue
//                best = minOf(best,n- (r - i + 1))
//            }
//            put(best)
//        }
