...
bool naive(int n){
	for (int i = 0; i < n; ++i){
		for (int j = 0; j < n; ++j){
			for (int k = 0; k < n; ++k){
				if (a[j][k] > a[i][j] + a[i][k]) return false;
			}
		}
	}
	return true;
...
int main(){
	...
	FI(n){
		FJ(n){
			cin >> a[i][j];
			if (i != j && !a[i][j]) NO();
			if (i > j && a[i][j] != a[j][i]) NO();
			if (i > j) ribs[rs++] = { a[i][j], i, j };
		}
		if (a[i][i]) NO();
	}
	FI(n + 1) p[i] = i;
	sort(ribs, ribs + rs, [](const tri &t1, const tri &t2){return t1.a < t2.a; });
	
	for (int i = 0; i < rs; ++i){
		if (unite(ribs[i].b, ribs[i].c)){
			v[ribs[i].b].push_back({ ribs[i].c, ribs[i].a });
			v[ribs[i].c].push_back({ ribs[i].b, ribs[i].a });
		}
	}
	for (beg = 0; beg < n; ++beg){
		memset(used, 0, n);
