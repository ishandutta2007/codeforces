...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 0; i < n; i++) {
                if (i != 0) {
                    space -= segs[i][0] - segs[i - 1][1];
                    plus -= segs[i - 1][1] - segs[i - 1][0];
                }

                while (h > space && scan < n) {
                    plus += segs[scan][1] - segs[scan][0];
                    scan++;
                    if (scan == n) {
                        space = (int) 1e9;
                    } else {
                        space += segs[scan][0] - segs[scan - 1][1];
                    }
                }
