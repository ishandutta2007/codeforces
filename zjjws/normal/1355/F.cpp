...
inline void init()
{
    for(int i=2;i*i<=1e9;i++)
    {
        ...
        for(auto j:prime){int now=i*j;if(1LL*now*now>1e9)break;pri[now]=true;if(i%j==0)break;}
    }
    return;
    LL lst=1;
    int cost=0;
    for(auto i:prime)
    {
        int cutt=0,j;for(j=1;1LL*j*i<=1e9;j*=i)
        {
            cutt++;
            if(cost<22)
            {
                if(lst<=1e18/i)lst*=i;else {cost++;lst=i;}
            }
            // if(cost==12){printf("i:%d\n",i);return;}
        }cuts[cutt]++;
        if(cutt!=2)printf("%d %d %d\n",i,j,cutt);
    }
    // for(int i=2;i<=29;i++)printf("cutt=%d have %d\n",i,cuts[i]);
...
inline int qst(LL x){qstcutt++;printf("? %lld\n",x);fflush(stdout);int ans;rin>>ans;return ans;/* return Gcd(x,4194304); */}
inline void work()
{
    LL ans=1;
    LL X=1;
    qstcutt=0;lst=1;
    for(vector<int>::iterator i=prime.begin(),j,k;i!=prime.end();i=j)
    {
        if(qstcutt==21&&lst>1)
        {
            LL pro=1;for(;pro*lst<=1e9;pro*=lst);
            for(j=i;j!=prime.end()&&pro<=1e18/(*j);j++)pro*=(*j);if(pro>1e18)pro/=(*(--j));
            pro=qst(pro);int cutt=1;for(;pro%lst==0;pro/=lst,X*=lst)cutt++;ans*=cutt;
            for(k=i;k<j;k++)if(pro%(*k)==0)ans<<=1,X*=(*k);break;
        }
        LL pro=1;for(j=i;j!=prime.end()&&pro<=1e18/(*j);j++)pro*=(*j);if(pro>1e18)pro/=(*(--j));
        pro=qst(pro);if(qstcutt==22){for(k=i;k!=j;k++)if(pro%(*k)==0)ans<<=1,X*=(*k);break;}
        for(k=i;k!=j;k++)if(pro%(*k)==0)
        {
            if(lst>1)
            {
                int lsts=1,nows=1,cutt;
                for(;1LL*lsts*lst<=1e9;lsts*=lst);
                for(;1LL*nows*(*k)<=1e9;nows*=(*k));
                LL ass=qst(1LL*lsts*nows);
                for(cutt=1;ass%lst==0;ass/=lst,X*=lst)cutt++;ans*=cutt;
                for(cutt=1;ass%(*k)==0;ass/=(*k),X*=(*k))cutt++;ans*=cutt;
                lst=1;if(qstcutt==22)break;
            }
            else lst=(*k);
        }
        if(qstcutt==22)break;
...
inline void Work(){int T;for(rin>>T;T;T--)work();return;}
int main()
{
