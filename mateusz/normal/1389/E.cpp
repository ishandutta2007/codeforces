...
//         }
        long long days = m * d;
        if (d % w == 1) {
            long long mul = min(m, d);
            printf("%lld\n", mul * (mul - 1) / 2);
        } else {
            long long c = w / __gcd(w, d - 1);
            long long mul = min(m, d) / c;
            long long ans = c * mul * (mul - 1) / 2 + (min(m, d) % c) * mul;
            
//             for (int i = 1; i <= min(m, d); i++) {
//                 ans += (min(m, d) - i) / w + ((min(m, d) - i) % w >= (w / __gcd(d - 1, w)));
//             }
            printf("%lld\n", ans);
        }
    }
