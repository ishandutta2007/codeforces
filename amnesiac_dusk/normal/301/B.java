...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		...
		for(int i=1; i<n-1; i++)
		{
			dist[co[i][0]][co[i][1]]=a[i-1];
		}
		long[][]matrix=new long[301][301];
		for(int x=0; x<=300; x++)
			for(int y=0; y<=300; y++)
				matrix[x][y]=inf;
		matrix[co[0][0]][co[0][1]]=0;
		for(int k=0; k<200; k++)
		for(int x=1; x<300; x++)
			for(int y=1; y<300; y++)
			{
				matrix[x+1][y]=Math.min(matrix[x][y]+d-dist[x+1][y], matrix[x+1][y]);
				matrix[x][y+1]=Math.min(matrix[x][y]+d-dist[x][y+1], matrix[x][y+1]);			
				matrix[x-1][y]=Math.min(matrix[x][y]+d-dist[x-1][y], matrix[x-1][y]);
				matrix[x][y-1]=Math.min(matrix[x][y]+d-dist[x][y-1], matrix[x][y-1]);			
			}
		pr(matrix[co[n-1][0]][co[n-1][1]]);
