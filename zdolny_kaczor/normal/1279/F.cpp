...
int solve(int n, int k, int l) {
	...
	while (low <= high) {
		...
		dp[0] = {0, 0};
		REP(i, n) {
			dp[i + 1] = dp[i] + pii(seq[i], 0);
			if (i + 1 >= l)
				mini(dp[i + 1], dp[i + 1 - l] + pii(med, 1));
		}
		pii val = dp[n];
		maxi(ans, val.first - k * 1ll * med);
		if (val.second > k) {
			low = med + 1;
		}
		else {
	...
	debug(range(seq, 0, n), imie(k), imie(l), imie(ans));
	assert(ans >= 0);
	rans;
}
int main() {
	int n, k, l;
	scanf("%d%d%d%s", &n, &k, &l, str);
	if (k * 1ll * l >= n) return 0 * printf("0\n");
	REP(i, n) seq[i] = (str[i] >= 'a' && str[i] <= 'z');
	int ans = solve(n, k, l);
	REP(i, n) seq[i] = (str[i] >= 'A' && str[i] <= 'Z');
	mini(ans, solve(n, k, l));
	printf("%d\n", ans);
}