...
vi e[MN];
vector<pii>qr[MN],qv[MN];
int fat[MN],dfn[MN],id[MN],tot;
void dfs0(int u,int fa) {
	fat[u]=fa; id[dfn[u]=++tot]=u;
...
void dfs1(int u,int fa) {
	f[u]=0;
	for(auto v:e[u]) { if(v==fa) continue;
		dfs1(v,u); f[u]+=max(1,f[v]-k);
	}
}
void dfs2(int u,int fa) {
	for(auto v:e[u]) { if(v==fa) continue;
		dfs2(v,u);
		foR(x,S,0) foR(y,S-x,0) {
...
signed main() {
	...
	For(i,1,q) {
		int x=read(),k=read();
		if(k<=B) qr[k].pb(mkp(x,i));
		else qv[x].pb(mkp(k,i));
	}
	for(k=0;k<=B;k++) {
//		cerr<<k<<endl;
		...
		for(auto it:qr[k]) {
//			cerr<<it.second<<' '<<f[it.first]<<endl;
