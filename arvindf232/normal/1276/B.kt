...
fun main(){
    solve.cases{
        ...
        val hasa = BooleanArray(n)
        val hasb = BooleanArray(n)
        for(e in all){
            if(e.a == a || e.b == a){
                val other = if(e.a == a) e.b else e.a
                if(other == b) continue
                hasa[dsu.find(other)] = true
            }
            if(e.b == b || e.a == b){
                val other = if(e.b == b) e.a else e.b
                if(other == a) continue
                hasb[dsu.find(other)] = true
            }
        }
        var aa = 0
        var bb = 0
        dsu.eachComponent {
            if(hasa[it] xor hasb[it]){
                if(hasa[it]){
                    aa += dsu.getsize(it)
                }else{
                    bb += dsu.getsize(it)
