...
bool substr(const string& s, const string& t)
{
	...
	{
		bool bad = false;
		for (int j = 0; (bad == false) && (j < sz(s)); ++j)
		{
			if (i + j < sz(t))
				bad = (s[j] != t[i + j]);
			else
...
int main()
{
	...
	string s, t;
	cin >> n >> s >> t;
	if (substr(s, "abca") == false && substr(t, "abca") == false)
	{
		for (int i = 0; i < n; ++i)
			cout << "abc";
	}
	else if (substr(s, "acba") == false && substr(t, "acba") == false)
	{
		for (int i = 0; i < n; ++i)
			cout << "acb";
	}
	...
	for (int i = 1; i < 200001; ++i)
	{
		...
		int x = i - 1, y = i;
		int op = 0;
		while ((mda < k) && (y < 400001))
		{
			++op;
			x = 2 * x + 1;
			y = 2 * y + 1;
			int add = min(k - mda, c[y] - c[x]);
			mda += add;
			ansi += add * op;
		}
		if (mda == k)
