...
void solve()
{
    ...
    if (x >= 0 && y >= 0 && y >= x)
    {
        ans = min(ans, c[0] * abs(x) + c[1] * (abs(y) - abs(x)));
    }
    if (x <= 0 && y >= 0)
    {
        ans = min(ans, c[1] * abs(y) + c[2] * abs(x));
    }
    if (x <= 0 && y <= 0 && y >= x)
    {
        ans = min(ans, c[2] * (abs(x) - abs(y)) + c[3] * abs(y));
    }
    if (x <= 0 && y <= 0 && y <= x)
    {
        ans = min(ans, c[3] * abs(x) + c[4] * (abs(y) - abs(x)));
    }
    if (x >= 0 && y <= 0)
    {
        ans = min(ans, c[4] * abs(y) + c[5] * abs(x));
    }
    if (x >= 0 && y >= 0 && y <= x)
    {
        ans = min(ans, c[5] * (abs(x) - abs(y)) + c[0] * (abs(y)));
    }
    cout << ans << "\n";
}
