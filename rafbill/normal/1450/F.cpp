...
void solve(){
  ...
  int last = -1; int lasti = 0;
  vii ranges;
  for(li i = 0; i < (li)(n); ++i) {
    if(A[i] == last) {
      ranges.emplace_back(lasti, i-1);
      lasti = i;
    }
    last = A[i];
  }
  ranges.emplace_back(lasti, n-1);
  ...
  int i = 0;
  for(li j = 0; j < (li)(n); ++j) if(cnt[j]>cnt[i]) i = j;


  int slack = 0;
  for(li x = 0; x < (li)(n-1); ++x) {
    if(A[x] != A[x+1]) {
      if(A[x] != i && A[x+1] != i) {
        slack++;
      }
  ...
  if(cnt[i] <= ((int)ranges.size()+slack)+1) {
    int r = ranges.size();
    while(cnt[i] > r+1) r++;
    cout << r-1 << endl;
  }else{
