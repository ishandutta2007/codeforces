...
signed main() {
  ...
  // vector<unordered_map<ll, ll, custom_hash>> memo(12);
  vector<map<ll, ll>> memo(12);
  ll ans = 0;
  auto self = 0;
  vi tens({1});
  FOR(i, 12) { tens.eb(tens.back() * 10 % K); }

  // print(tens);

  for (auto&& x : A) {
    auto s = to_string(x).size();
    memo[s][x % K]++;
    if ((x * tens[s] + x) % K == 0) ++self;
  }
  ...
  for (auto&& x : A) {
    FOR3(s, 1, 12) {
      auto now = tens[s] * x % K;
      auto rest = (K - now) % K;
      if (memo[s].count(rest)) ans += memo[s][rest];
    }
  }
  print(ans - self);
  return 0;
