...
constexpr ll op(ll x, ll y) { return x < y ? x : y; }
constexpr ll e() { return numeric_limits<ll>::max() / 2; }
...
int main() {
    ...
    seg.set(0, 0);
    vec<ll> dp(n + 1, numeric_limits<ll>::max() / 2);
    dp[0] = 0;
    rep(i, n) {
        ...
        if (i != n - 1) {
            int p = i - k;
            if (p >= 0 and s[p] == '1') {
                chmin(dp[i + 1], seg.prod(max(0, p - k), i + 1) + p + 1);
            }
        } else {
            for (int j = max(n - 1 - k, 0); j < n; ++j) {
                if (s[j] != '1') continue;
                chmin(dp[n], seg.prod(max(0, j - k), n) + j + 1);
            }
        }
        seg.set(i + 1, dp[i + 1]);
    }
