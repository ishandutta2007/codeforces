...
void solve() {
    ...
    for (int i = 1, k, d; i <= m; i++) {
        cin >> k >> d;
        a[k] -= d;
        auto it = train.upper_bound({k, 2'000'000'000});
        if (it == train.end()) it = train.find(*train.rbegin());
        else it = prev(it);
        auto [prev_pos, prev_speed] = *it;
        if (prev_speed > a[k]) {
            if (prev_pos == k) train.erase(it);
            while (true) {
                auto it = train.upper_bound({k, 2'000'000'000});
                if ((it == train.end()) || (it->second < a[k])) break;
                train.erase(it);
            }
            train.insert({k, a[k]});
        }
