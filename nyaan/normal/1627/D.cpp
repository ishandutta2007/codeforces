...
using mint1 = LazyMontgomeryModInt<998244353>;
using mint2 = LazyMontgomeryModInt<1000000007>;

using namespace Nyaan;

template <typename mint>
void calc(vl& ex, vl B) {
  vector<mint> C(sz(B));
  rep(i, sz(B)) C[i] = B[i];
  multiple_transform::zeta_transform(C);
  rep(i, sz(C)) C[i] = C[i].pow(21);
  multiple_transform::mobius_transform(C);
  rep(i, sz(B)) ex[i] |= C[i] != 0;
}
...
void q() {
  ...
  each(x, A) B[x]++;
  vl ex(TEN(6) + 10);
  calc<LazyMontgomeryModInt<924844033>>(ex, B);
  calc<LazyMontgomeryModInt<998244353>>(ex, B);
  calc<LazyMontgomeryModInt<1000000007>>(ex, B);
  calc<LazyMontgomeryModInt<1000000009>>(ex, B);
  calc<LazyMontgomeryModInt<1000000021>>(ex, B);
  out(Sum(ex) - N);
}
