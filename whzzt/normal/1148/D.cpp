...
const int N=6e5+5;
int n,m,a[N],b[N],dp[N],from[N],col[N];
struct bit{
	pii cv[N];
	bit(){ memset(cv,0,sizeof(cv)); }
	void upd(int x,pii y){ while(x<=m) chkmax(cv[x],y),x+=lowbit(x); }
	pii ask(int x){ pii r(0,0); while(x) chkmax(r,cv[x]),x-=lowbit(x); return r; }
};
void solve(vector<int> id,int tg){
	sort(id.begin(),id.end(),[&](int x,int y){ return b[x]<b[y]; });
	//for(auto t:id) cerr<<t<<' '; cerr<<endl;
	bit cur;
	for(int x:id){
		pii g=cur.ask(a[x]);
		dp[x]=g.fi+1,from[x]=g.se,col[x]=tg;
		cur.upd(b[x],pii(dp[x],x));
	}
}
int main(){
	scan(n); m=n<<1;
	...
	for(int i=1;i<=n;++i){
		scan(a[i],b[i]);
		if(a[i]>b[i]) aa.pb(i);
		else swap(a[i],b[i]),bb.pb(i);
	}
	solve(aa,0); solve(bb,1);
	int o=0;
	for(int i=1;i<=n;++i) if(dp[i]>dp[o]) o=i;
	vector<int> res; bool vv=col[o];
	while(o) res.pb(o),o=from[o];
	if(!vv) reverse(ALL(res));
	cout<<res.size(); bool f=true;
	for(int g:res){
		if(f) cout<<endl; else cout<<" ";
		cout<<g; f=false;
	}cout<<endl;
}