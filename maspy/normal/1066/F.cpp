...
signed main() {
  ...
  auto to_xy = [](ll lv, ll x) {
    if (x < 0) return mp(lv, lv + x);
    return mp(lv - x, lv);
  };
  ...
  const ll INF = 1LL << 60;
  for (auto&& k : X) {
    auto [xl, yl] = to_xy(k, L[k]);
    auto [xr, yr] = to_xy(k, R[k]);
    auto d = abs(xl - xr) + abs(yl - yr);
    auto dpl = INF, dpr = INF;
    {
      auto [x, y] = to_xy(dp.k, dp.left);
      chmin(dpr, dp.dpl + abs(x - xl) + abs(y - yl) + d);
      chmin(dpl, dp.dpl + abs(x - xr) + abs(y - yr) + d);
    }
    {
      auto [x, y] = to_xy(dp.k, dp.right);
      chmin(dpr, dp.dpr + abs(x - xl) + abs(y - yl) + d);
      chmin(dpl, dp.dpr + abs(x - xr) + abs(y - yr) + d);
    }
    dp = DP({k, L[k], dpl, R[k], dpr});
  }
  print(min(dp.dpl, dp.dpr));
