...
int main(void) {
    ...
    while (q--) {
        ...
        prob[t] = 1;
        for (int d = 0; d < 15; ++d) if (m >> d & 1) {
            for (int i = 0; i < n; ++i) save[i] = prob[i], prob[i] = 0;
            for (int i = 0; i < n; ++i) for (int j = 0; j < n; ++j) prob[i] += save[j] * f[i][j][d];
        }
        double ans = 0;
        for (auto line: lines) {
            double opt = 0;
            for (auto u: line) opt += prob[u];
