...
pll dfs1(int v,int pr){
  ...
  if(!c) return R[v]=mp(C[v],0);
  return R[v]=mp(min(s,s+mn+C[v]),s+mn);
}

void dfs2(int v,int pr,int m){
  LL s=0,mn=0,mn2=0;
  for(int p:G[v]){
    if(p==pr) continue;
    s+=R[p].X;
  ...
  if((m&1)&&R[v].X==s+mn+C[v]) ans.pb(v);
  for(int p:G[v]){
    if(p==pr) continue;
    LL t=R[p].Y-R[p].X;
    int mm=0;
    if((m&1)&&R[v].X==s) mm|=1;
    if((m&1)&&R[v].X==s+t+C[v]) mm|=2;
    if((m&1)&&R[v].X==s+mn+C[v]&&(t!=mn||mn2==mn)) mm|=1;
    if((m&2)&&R[v].Y==s+t) mm|=2;
    if((m&2)&&R[v].Y==s+mn&&(t!=mn||mn2==mn)) mm|=1;
    if(mm) dfs2(p,v,mm);
  }
}
