...
void f1(char c) {
  for (int i = 0; i < 13; ++i) if (!ans[0][i]) { ans[0][i] = c; return ;}
  for (int i = 12; i >= 0; --i) if (!ans[1][i]) { ans[1][i] = c; return ;}
}
void f2(char c) {
  for (int i = 0; i < 13; ++i) if (!ans[1][i]) { ans[1][i] = c; return ;}
  for (int i = 12; i >= 0; --i) if (!ans[0][i]) { ans[0][i] = c; return ;}
}
