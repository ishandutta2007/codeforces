...
struct dsu
{
	...
	ll root(ll u) { return par[u] = ((u == par[u]) ? u : root(par[u])); }

	bool connected(ll x, ll y) { return root(x) == root(y); }
...
int main() {
	...
	d.init(n);
	for(ll i = 0; i < n-1; i++) {
		while(j < q.size() && q[j].first < v[i].first)
			r[q[j++].second] = ans;
		d.unite(v[i].second.first, v[i].second.second);
