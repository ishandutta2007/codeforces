...
int main() {
	...
	REP(_, t) {
		...
		ll oth_sum = 0;
		ll min_los = numeric_limits<ll>::max();
		REP(i, n) {
			ll first = a[i], oth = max(0ll, a[i] - b[(i + n - 1) % n]);
			debug(imie(i), imie(first), imie(oth));
			oth_sum += oth;
			mini(min_los, first - oth);
		}
		debug(imie(oth_sum), imie(min_los));
		printf("%lld\n", oth_sum + min_los);
	}
	exit(0);
}
