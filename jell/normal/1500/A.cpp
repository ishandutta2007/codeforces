#line 1 "codeforces-workspace/contest/1500/a/a.cpp"
...
#line 11 "codeforces-workspace/contest/1500/a/a.cpp"
...
void main() {
  ...
  vector<vector<int>> cnt(2500010);
  for (auto &&[i, x] : enumerate(a)) {
    cnt[x].emplace_back(i);
  }

  vector<int> *two = nullptr;
  for (auto &&v : cnt) {
    if (v.size() > 1) {
      if (two) {
        cout << "YES\n";
        cout << v[0] + 1 << " " << (*two)[0] + 1 << " " << v[1] + 1 << " "
             << (*two)[1] + 1 << "\n";
        return;
      }
      if (v.size() >= 4) {
  ...
  vector<pair<int, int>> pairs(5000010, {-1, -1});
