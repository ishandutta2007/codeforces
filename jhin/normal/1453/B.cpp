#include <bits/stdc++.h>
    ...
    if(x>1)p.push_back(x),cnt++,vv.push_back(1);
        ...
        int m,n,x,y,z,k,mn=3e9,mx,a[500003],b[500003],c[500003];bool yes1,yes2;vector<long long>v;char ch;string st[102];
            long long ans;
        ...
        while(t--)
        {
          ...
          for(int i=2;i<n;i++)
          {
              int dif=abs(a[i]-a[i-2]);
              int dif2=abs(a[i]-a[i-1])+abs(a[i-1]-a[i-2]);
              mx=max(mx,dif2-dif);
          }

          mx=max(mx,b[n-1]);mx=max(mx,b[1]);
