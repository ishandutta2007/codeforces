...
signed main(){fastio
    ...
    rep(i,0,n-1){
        ...
        for(auto p:P){
            rep(j,0,t){
                if(p+j>k)break;
                chmax(dp[i+1][p+j],dp[i][p]+a[j]);
            }
        }
        rep(j,0,k){
            chmax(dp[i+1][j],dp[i][j]);
            if(j-t>=0)chmax(dp[i+1][j],dp[i][j-t]+a[t]);
        }
        rep(j,1,k)chmax(dp[i+1][j],dp[i+1][j-1]);
    }
    //for(auto x:dp){for(auto y:x)cout<<y<<" ";cout<<endl;}cout<<endl;
    cout<<dp[n][k]<<endl;
