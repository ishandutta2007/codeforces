...
void solve(){
	ll n, r1, r2, r3, d; read(n,r1,r2,r3,d);
	vector<ll>v(n);read(v);
	vector<vl>dp(n, vl(3));
	dp[0][0] = min(r1*v[0]+r3,r3*(v[0]+1));
	dp[0][1] = min(r1*(v[0]+2)+2*d, r2+min({r1,r2,r3})+2*d);
	dp[0][2] = dp[0][1];
	for(int i = 1; i < n; ++i){
		dp[i][0] = min({dp[i-1][0],dp[i-1][1],dp[i-1][2]})+min(r1*v[i]+r3,r3*(v[i]+1));
		dp[i][1] = min({dp[i-1][0],dp[i-1][1],dp[i-1][2]})+min(r1*(v[i]+2)+2*d, r2+min({r1,r2,r3})+2*d);
		dp[i][2] = min({dp[i-1][0],dp[i-1][1]-2*d,dp[i-1][2]})+min(r1*(v[i]+2)+2*d, r2+min({r1,r2,r3})+2*d);
	}
	dp[n-1][0] = min(dp[n-1][0], (dp[n-2][1]-d)+min(r1*v[n-1]+r3,r3*(v[n-1]+1)));
	cout << min({dp[n-1][0],dp[n-1][1],dp[n-1][2]})+(n-1)*d << "\n";
