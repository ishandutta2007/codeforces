...
void solve() {
	...
	struct ST {
		...
		void upd(int i, int l, int r, int qi, int qx) {
			if (l + 1 == r) {
				mn[i] = qx;
				return;
			}
	...
	for (int i = 1; i < isz(ls); i++) {
		ls[i] = a[i - 1] + ls[i - 1];
		l.upd(0, 0, n + 1, i, ls[i]);
		rs[n - i] = a[n - i] + rs[n - i + 1];
		r.upd(0, 0, n + 1, n - i, rs[n - i]);
	}

	vector<int> cl(n, -1), cr(n, n);
	stack<int> s;
	for (int i = 0; i < n; i++) {
		while (isz(s) && a[s.top()] <= a[i]) {
			s.pop();
		}
		if (isz(s)) {
			cl[i] = s.top();
	...
	int ans = 0;
	for (int i = 0; i < n; i++) {
		int z1 = l.get(0, 0, n + 1, cl[i] + 1, i + 1);
		int z2 = r.get(0, 0, n + 1, i + 1, cr[i] + 1);

		int z = ls[i + 1] - z1 + rs[i] - z2 - 2 * a[i];
		ans = max(ans, z);
	}
