...
void solve() {
	scanf("%d",&n);
	int i,mx=0,m=0,k;
	for(i=1;i<=n;i++) scanf("%d%d",&a[i].l,&a[i].r),mx=max(mx,a[i].r);
	for(i=1;i<=n;i++) {
		b[++m]=(B){a[i].l,1,i};
		b[++m]=(B){a[i].r,-1,i};
	}
	sort(b+1,b+m+1);
	int now=0,OK=0,p=0,j;
	for(i=1;i<=m;i=j) {
		for(j=i;j<=m&&b[i].p==b[j].p;j++) ;
		for(k=i;k<j;k++) now+=b[k].v;
		if(j>m) break;
		if(!now) {
			OK=1;
			p=j-1; break;
	...
	if(!OK) {puts("-1"); return ;}
	for(i=1;i<=p;i++) ans[b[i].id]=1;
	for(i=p+1;i<=m;i++) ans[b[i].id]=2;
