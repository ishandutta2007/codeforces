...
void solve() {
    ...
    REP(i, x) {
        ...
        REP(q, n) {
            while (next.empty() == false and next.front().second <= q - k) {
                next.pop_front();
            }
            while (next.empty() == false and next.back().first <= dp[i][q]) {
                next.pop_back();
            }
            next.push_back(mp(dp[i][q], q));
            dp[i + 1][q + 1] = max(dp[i + 1][q + 1], next.front().first + inputs[q]);
        }
    }
    int ans = -1;
    cout << max(-1LL,dp[x][n]) << endl;
}
#undef int
int main() {
    init();
