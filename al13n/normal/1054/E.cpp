...
void emit(int a,int b,int c,int d,int e,int f,bool q){
  if(q){
    if(mp(c,d)!=mp(e,f)) ans.pb(mp(mp(e,f),mp(c,d)));
    if(mp(c,d)!=mp(a,b)) ans.pb(mp(mp(c,d),mp(a,b)));
  }else{
    if(mp(c,d)!=mp(a,b)) ans.pb(mp(mp(a,b),mp(c,d)));
    if(mp(c,d)!=mp(e,f)) ans.pb(mp(mp(c,d),mp(e,f)));
  }
}

void doit(int a,int b,string&s,bool q){
  reverse(all(s));
  for(char c:s){
    pii to=c=='0'?mp(0,0):mp(n-1,m-1);
    pii via;
    if(to==mp(a,b)) via=mp(0,m-1);
    else if(!to.X) via=mp(0,b);
    else via=mp(a,m-1);
    emit(a,b,via.X,via.Y,to.X,to.Y,q);
  }
  reverse(all(s));
}
...
void verify(){
  auto C=A;
  for(auto p:ans){
    string&from=C[p.X.X][p.X.Y];
    assert(sz(from));
    assert(p.X!=p.Y);
    string&to=C[p.Y.X][p.Y.Y];
    to.insert(to.begin(),from.end()-1,from.end());
...
int main(int argc, char **argv){
  ...
  P.pb(mp(0,0));
  P.pb(mp(n-1,m-1));
  forn(j,m-2) P.pb(mp(0,j+1));
  forn(i,n-2) P.pb(mp(i+1,m-1));
  forn(i,n-1) forn(j,m-1) P.pb(mp(i+1,j));
  assert(sz(P)==n*m);
  for(pii p:P) doit(p.X,p.Y,A[p.X][p.Y],false);
  assert(sz(ans)<=s*2);
  reverse(all(P));
  for(pii p:P) doit(p.X,p.Y,B[p.X][p.Y],true);
  assert(sz(ans)<=s*4);
  ...
  cout<<sz(ans)<<'\n';
  for(auto p:ans) cout<<p.X.X+1<<' '<<p.X.Y+1<<' '<<p.Y.X+1<<' '<<p.Y.Y+1<<'\n';
