...
void solve() {
  ll N = in();
  auto A = vin(N);
  const ll MAX = 300'010;
  /*
  a_i  CNT
  f(x) = sum (x mod a) = F1[x]x + F0[x]  F1, F0
  3  fenwick tree 
  */
  FenwickTree<ll> CNT(MAX + 10);
  FenwickTree<ll> F0(MAX + 10);
  FenwickTree<ll> F1(MAX + 10);
  ll sum_A = 0;
  ll ans = 0;
  vi ANS;
  FORIN(x, A) {
    ...
    ll res_1 = sum_A;
    FOR(q, max_q + 1) {
      ll l = q * x, r = min(q * x + x, MAX + 1);
      ll cnt = CNT.sum(r) - CNT.sum(l);
      res_1 -= cnt * q * x;
    }
