...
int solve(int k, int pos) {
    ...
    long long & cos = cost[k][pos];
    if (res != -1) {
        return res;
    }
    if (k == 0) {
        int pp = upper_bound(p + pos, p + n, a[pos]) - p;
        int x = maxPos(pos, pp - 1);
        if (x != pos) {
            res = solve(k, x);
            cos = 0;
        } else {
...
int main() {
    ...
    while (m--) {
        ...
        for (int i = 19; i > 0; --i) {
            if (solve(i, x) < y) {
                ans += cost[i][x];
                ans += p[nex[i][x] + 1] - a[maxPos(x, nex[i][x])];
                x = nex[i][x] + 1;
            }
        }
        if (solve(0, x) < y) {
