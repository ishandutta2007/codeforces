...
int n, m, q, a[maxn], tmp[maxn], A[maxn], ans[maxn], vis[maxn];
struct BIT {
	int c[maxn];
...
void test() {
	for (int i = 1; i <= m; ++i) vis[i] = rnd() & 1;
	tr.clr();
	for (int i = 1; i <= n; ++i) {
		if (vis[a[i]]) tr.add(i, 1);
		tmp[i] = a[i];
	}
	for (int i = 1; i <= q; ++i) {
		if (b[i].op == 1) {
			if (vis[tmp[b[i].l]]) tr.add(b[i].l, -1);
			if (vis[b[i].x]) tr.add(b[i].l, 1);
			tmp[b[i].l] = b[i].x;
		}
