...
int t[MAXN], tot, dp[MAXN];

signed main(){
    read(n); int ans = 0; read(a + 1, a + n + 1); std :: sort(a + 1, a + n + 1);
    rep(i, 1, n) {
        int it = 0; t[++ it] = a[i];
        for (int j = 2;j * j <= a[i];++ j)
            if(a[i] % j == 0) {
                t[++ it] = j;
                if(j * j != a[i]) t[++ it] = a[i] / j;
            }
        int x = 0; rep(j, 1, it) chmax(x, dp[t[j]]);
        chmax(ans, ++ x);
        rep(j, 1, it) chmax(dp[t[j]], x);
        // print(dp + 1, dp + tot + 1);
    } print(ans);
}