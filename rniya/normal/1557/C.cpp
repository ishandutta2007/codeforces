...
void solve() {
    ...
    vector<vector<mint>> calc(2, vector<mint>(2, 0));
    calc[1][n & 1] = 1;
    for (int i = 0; i < n; i++) calc[0][i & 1] += COM.C(n, i);
    ...
    for (int i = 0; i < k; i++) {
        dp[i + 1][1] += dp[i][1] * (calc[0][0] + calc[1][1]);
        dp[i + 1][0] += dp[i][1] * calc[1][0];
        dp[i + 1][0] += dp[i][0] * power;
    }

    mint ans = dp[k][0] + dp[k][1];
    cout << ans << '\n';
}
