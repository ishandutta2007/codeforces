...
int main() {
...

    {
        ...
        sort(tmp.begin(), tmp.end(), [&](int l, int r) { return a[l] < a[r]; });
    ...
    int to = a[0];
    for (int& x : a) to += (b[0] == 1 and x > a[0]);
    if (to < a[n - 1]) {
        cout << 0 << '\n';
        return 0;
    }

    vector<vector<int>> cnt(2 * n, vector<int>(2, 0));
    for (int i = 0; i < n; i++) cnt[a[i]][b[i]]++;
    mint ans = 1;

    for (int i = 0, sum = 0; i < min(2 * n, to); i++) {
        for (int j = 0; j < 2; j++) {
    ...
    if (to < 2 * n) {
        ans *= BINOM.fac(cnt[to][0]);
        ans *= BINOM.fac(cnt[to][1]);
        ans *= BINOM.C(n, cnt[to][1]);
    }
