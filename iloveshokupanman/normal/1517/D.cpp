...
int main() {
    ...
    vvll hor(N, vll(M - 1, 0));
    rep(i, N)rep(j, M - 1)cin >> hor[i][j];
    vvll ver(N - 1, vll(M, 0));
    rep(i, N - 1)rep(j, M)cin >> ver[i][j];

    vvvll dp(N, vvll(M, vll(K/2 + 1, 0)));

    repn(k, K/2) {
        rep(i, N)rep(j, M) {
            dp[i][j][k] = INF;
            //cout << i << j << endl;
            if (i < N - 1)dp[i][j][k] = min(dp[i][j][k], dp[i + 1][j][k-1] + ver[i][j]);
            if (i > 0)dp[i][j][k] = min(dp[i][j][k], dp[i - 1][j][k-1] + ver[i - 1][j]);

            if (j < M - 1)dp[i][j][k] = min(dp[i][j][k], dp[i][j + 1][k-1] + hor[i][j]);
            if (j > 0)dp[i][j][k] = min(dp[i][j][k], dp[i][j - 1][k-1] + hor[i][j - 1]);
