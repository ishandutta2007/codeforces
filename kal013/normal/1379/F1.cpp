...
bool possible(int n,int m,set<pair<int,int> > Q){
	assert( Q.size() > 0 );
	vector<pair<int,int> > od,ev;
	for(auto e : Q){
		assert( (e.first & 1) == (e.second & 1));
		if (e.first & 1){
			if (Q.find({e.first + 1, e.second + 1}) != Q.end()){
				return false;
			}

			if (od.size() == 0 || (od.back().second > e.second) )
				od.push_back(e);
		}
		else{

			while(ev.size() > 0 && ev.back().second < e.second){
				ev.pop_back();
			}
			ev.push_back(e);
		}
	...
	for(auto e: od){
		while( j < ev.size() && ev[j].first < e.first){
			++j;
		}
		if (j < ev.size() && ev[j].second > e.second){
			return false;
		}
	}
