...
vector<mint> solve(const vector<ll> &xs, const vector<ll> &bs, const vector<pii> &qs){
	fill(bit,bit+nn+1,PR());
	cauto n=(int)xs.size();
	vector<ll> sums(n);
	rep(i,n) sums[i]=bs[i];
	reps(i,1,n) sums[i]+=sums[i-1];
	vector<mint> summ(n);
	rep(i,n) summ[i]=(sums[i]%MOD+MOD)%MOD;
	vv<int> qrs(n);
	cauto q=(int)qs.size();
	rep(i,q) qrs[qs[i].Y].eb(i);
	vector<mint> re(q);
	for(cauto &i:Inds(n)-sortedWith([&](cauto &i){return pair(-sums[i],i);})){
		if(i+1<n){
			cauto len=xs[i+1]-xs[i];
			add(i,PR(summ[i]*len,len));
		}
		for(cauto &qi:qrs[i]){
			cauto &[l,r]=qs[qi];
			cauto s=sum(r)-sum(l);
			out(qi,l,r,s,summ[r],1);
			re[qi]=s.X-summ[r]*s.Y;
		}
	}
	out(sums,xs,qs,re,1);
	return re;
...
int main(){
	...
	cauto vs=readV(n);
	auto bs=readV(n);
	cauto qs=readV(q,pii(1,1));
	cauto re0=solve(vs,bs,qs);
	for(auto &b:bs) b=-b;
	cauto re1=solve(vs,bs,qs);
	rep(i,q) cout<<re0[i]+re1[i] NL;
	return 0;
}