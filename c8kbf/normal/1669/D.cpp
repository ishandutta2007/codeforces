...
inline void AC() {
    ...
    n = read();
    for(_ i = 1; i <= n; ++i) a[i] = getUp();
    if(count(a+1, a+1+n, 'W') == n) {
        yes(7);
        return;
    }
    if(count(a+1, a+1+n, 'R') == n || count(a+1, a+1+n, 'B') == n) {
        no(7);
        return;
    }
    str t;
    bool ok = true;
    for(_ i = 1; i <= n; ++i) if(a[i] == 'W') {
        ok &= stok(t);
        t.clear();
    } else t += a[i];
    ok &= stok(t);
...
bool stok(str x) {
    ...
    _ sz = x.size();
    if(count(x.begin(), x.end(), 'R') == sz || count(x.begin(), x.end(), 'B') == sz) return false;
    return true;
}