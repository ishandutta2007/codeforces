...
#line 14 "Library/src/data_structure/segment_tree/basic.hpp"
...
struct mono {
  ...
  mono operator+(const mono &rhs) const {
    if (ide) return rhs;
    if (rhs.ide) return *this;
    mint c = 0;
    if (r == 1) c = 9 - rhs.l;
    return {l,
            rhs.r,
            w * rhs.w + rw * rhs.lw * c,
            lw * rhs.w + lrw * rhs.lw * c,
            w * rhs.rw + rw * rhs.lrw * c,
            lw * rhs.rw + lrw * rhs.lrw * c,
            false};
  }
};
...
void main() {
  ...
  for (auto &&x : seg) {
    ...
    cin >> d;
    d -= '0';
    x = {d, d, d + 1, 1, 1, 0, false};
  }
  while (m--) {
    int x, d;
    cin >> x >> d;
    --x;
    seg[x] = {d, d, d + 1, 1, 1, 0, false};
    cout << seg.fold().w << "\n";
  }
}
