...
struct myclock_t
{
    ...
    void set(int crt_ln, const std::string &crt_func)
    {
        ...
        else
        {
            debug_stream << "[ " << crt_ln << " : " << crt_func << " ] "
                         << "myclock_t::set failed (yet to be built clock!)\n";
        }
    }
    void get(int crt_ln, const std::string &crt_func)
    {
        ...
        {
            debug_stream << "[ " << crt_ln << " : " << crt_func << " ] "
                         << "myclock_t::get failed (yet to be built clock!)\n";
        }
    }
};
#ifdef LOCAL
...
{
    for(P i = __first; i != __second; ++i) std::cin >> *i;
}
...
void solve()
{
    vector<int> idx(n);
    iota(all(idx), 1);
    sort(all(idx), [](int a, int b) { return d[a] > d[b]; });
    for(int i = 1; i < n; i++)
    {
        std::cout << idx[i] * 2 << " " << idx[i - 1] * 2 << "\n";
    };
    vector<int> now(n * 2);
    for(int i = 0; i < n; i++)
    {
    ...
    for(int i : idx)
    {
        if(d[i] == len)
        {
            std::cout << now[ii] << " " << i * 2 - 1 << "\n";
            now[++ii] = i * 2 - 1;
        }
        else
        {
            std::cout << now[ii - (len - d[i])] << " " << i * 2 - 1 << "\n";
            len--;
        }
    }
}