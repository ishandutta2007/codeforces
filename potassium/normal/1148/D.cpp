...
int main() {
	int n; R(n);
	vector<pair<int,pair<int,int> > > incre, decre;
	for (int i = 0; i < n; i++) {
		int x,y; R(x,y);
		if (x > y)
			decre.pb({i+1,{x,y}});
	...
	if (incre.size() > decre.size()) {
		W(incre.size()); WEL;
		sort(incre.begin(),incre.end(),[](pair<int,pair<int,int> > a, pair<int,pair<int,int> > b) {return a.second.second > b.second.second;});
		for (auto i : incre)
			W(i.first);
	} else {
		W(decre.size()); WEL;
		sort(decre.begin(),decre.end(),[](pair<int,pair<int,int> > a, pair<int,pair<int,int> > b) {return a.second.second < b.second.second;});
		for (auto i : decre)
			W(i.first);
	}
