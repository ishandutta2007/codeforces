...
int bfs() {
    ...
    while (qc < qs) {
        ...
        for (int e : g[cur]) {
            int to = edges[e].to;
            if (edges[e].cap == edges[e].val) continue;
            if (d[to] != -1) continue;
            d[to] = d[cur] + 1;
            q[qs++] = to;
        }
...
int dfs(int cur, int flow) {
    ...
    for (; ptr[cur] < g[cur].size(); ++ptr[cur]) {
        ...
        int to = edges[e].to;
        if (d[to] == d[cur] + 1) {
            int res = dfs(to, min(flow, edges[e].cap - edges[e].val));
            if (res) {
                edges[e].val += res;
                edges[e ^ 1].val -= res;
                return res;
...
int main()
{
    ...
    FI(m){
        ...
        for (tri p1 : v[x]){
            for (tri p2 : v[y]){
                if (p1.a == p2.a){
                    addEdge(p1.c, p2.c, min(p1.b, p2.b));
                }
            }
        }
    }
