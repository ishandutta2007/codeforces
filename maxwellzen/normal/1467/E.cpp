...
void solve() {
    ...
    for (auto p : loc) {
        vi pos = p.S;
        int num = pos.size();
        for (int i : pos) seg.upd(st[i], 1);
        for (int i : pos) {
            if (num>1) cnt[st[i]]++, cnt[st[i]+1]--;
            for (int nb : adj[i]) if (nb!=par[i]) {
                if (num - seg.query(st[nb], ed[nb]) > 1) cnt[st[nb]]++, cnt[ed[nb]+1]--;
            }
            if (seg.query(st[i], ed[i]) > 1) cnt[0]++, cnt[st[i]]--, cnt[ed[i]+1]++;
        }
        for (int i : pos) seg.upd(st[i], -1);
    }
