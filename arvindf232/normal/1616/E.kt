...
fun main(){
    solve.cases{
        ...
        val occurence = simpleSegTree(n)
        for(i in 0 until n){
            occurence.addValue(i,1)
        }
        for(v in pos){
            v.sortDescending()
        }
        var finalret = Long.MAX_VALUE
        var res = 0L
        for(j in 0 until n){
            val cc = target[j].code
            for(cd in cc - 1 downTo 0){
                if(pos[cd].isNotEmpty()){
                    val willgetFrom = pos[cd].last()
                    finalret = minOf(finalret, res + occurence.query(0,willgetFrom) - 1)
                }
            }
            if(pos[cc].isNotEmpty()){
                val willgetFrom = pos[cc].last()
                pos[cc].removeLast()
                res += occurence.query(0,willgetFrom) - 1
                occurence.addValue(willgetFrom,-1)
            }else{
                break
            }
        }
        put(if(finalret == Long.MAX_VALUE) -1 else finalret)
    }
    done()
}
