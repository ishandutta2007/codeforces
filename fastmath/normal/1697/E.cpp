...
void gen() {
    ...
    for (int x = -10; x <= 10; ++x) {
        //bool can = 1;
        //each (e, cur) can &= x != e.x;
        //if (can) {

            for (int y = -10; y <= 10; ++y) {
                //bool can = 1;
                //each (e, cur) can &= y != e.y;
                //if (can) {

                    bool can = 1;
                    each (e, cur) can &= mp(x, y) != e;
                    if (cur.size() > 1) {
                        int d = dist(cur[0], cur[1]);
                        each (e, cur) can &= dist(mp(x, y), e) == d;
                    }
...
signed main() {
    ...
    FOR (i, n) {
        FOR (j, n) {
            if (i != j) {
                ckmin(mind[i], dist(a[i], a[j]));
            }
        }
        FOR (j, n) {
            if (i != j) {
                if (dist(a[i], a[j]) == mind[i]) {
                    cnt[i]++;
                    clo[i][j] = 1;
                }
            }
    ...
    FOR (i, n) {
        FOR (j, i) {
            if (clo[i][j] && clo[j][i] && cnt[i] == 1 && cnt[j] == 1) {
                h[2]++;
            }
        }
    }
    ...
    FOR (i, n) {
        FOR (j, i) {
            FOR (k, j) {
                vi w = {i, j, k};
                bool ok = 1;
                each (a, w) each (b, w) if (a != b) ok &= clo[a][b];
                each (a, w) ok &= cnt[a] == 2;
                if (ok) {
                    h[3]++;
                }
    ...
    FOR (i, h[2] + 1) {
        FOR (j, h[3] + 1) {
            FOR (k, h[4] + 1) {
                ...
                ans += comb(h[2], b) * comb(h[3], c) * comb(h[4], d) * (fac[n] / fac[n - a - b - c - d]);                
            }
