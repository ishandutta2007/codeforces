...
void main_() {
	//[0,0]..[x,y]
	auto sub = [&](ll x, ll y) {
		return divup(x * y, 2ll);
	};
	auto rec_w = [&](ll sx, ll sy, ll gx, ll gy) {
		return sub(gx, gy) - sub(sx - 1, gy) - sub(gx, sy - 1) + sub(sx - 1, sy - 1);
	};
	auto rec_b = [&](ll sx, ll sy, ll gx, ll gy) {
		return (gx - sx + 1) * (gy - sy + 1) - rec_w(sx, sy, gx, gy);
	};
	LL(m, n);
	ll w = rec_w(1, 1, n, m);
	ll b = rec_b(1, 1, n, m);
	...
	if(max(x1, x3) <= min(x2, x4) and max(y1, y3) <= min(y2, y4)) {
		w -= rec_b(max(x1, x3), max(y1, y3), min(x2, x4), min(y2, y4));
		b += rec_b(max(x1, x3), max(y1, y3), min(x2, x4), min(y2, y4));
	}
	debug(w, b);
	print(w, b);
}