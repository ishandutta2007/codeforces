...
int main() {
	...
	scanf("%d%d", &n, &k);
	fac[0] = 1;
	for (int i = 1; i < nax; ++i) fac[i] = fac[i - 1] * 1ll * i % mod;
	fin[nax - 1] = powe(fac[nax - 1], mod - 2);
	for (int i = nax - 2; i >= 0; --i) fin[i] = fin[i + 1] * (i + 1ll) % mod;
	ll ans = 0;
	for (int i = 0; i <= n; ++i) for (int j = 0; j <= n; ++j) {
		int can_one = (n - i) * (n - j);
		ll curr = choose(n, i) * choose(n, j) % mod * powe(k, can_one) % mod * powe(k - 1, n * n - can_one) % mod;
		if ((i + j) % 2) ans -= curr;
		else ans += curr;
	}
	ans %= mod;
