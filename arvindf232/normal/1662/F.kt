...
class simpleMaxTree (nSuggest  :Int){
    ...
    inline fun destroyall(l:Int, r:Int, atleast:Int,justkilled:(Int)->Unit){
        segDivision(l,r){
            i,l->
            while(this.arr[i] >= atleast){
                val d = maxPos(i)
                setValue(d,Int.MIN_VALUE)
                justkilled(d)
            }
        }
    }
    fun printall(){
        just dei (0 until n).map { queryFunctional(it,it) }.conca()
    }
}
fun debug(){}
...
fun main(){
    solve.cases{
        ...
        val dist = IntArray(n)
        val powers = getline(n)

        val left = simpleMaxTree(IntArray(n){-(it - powers[it])}) // negated
        val right = simpleMaxTree(IntArray(n){it + powers[it]})



        val Q = fastEmptyList(n)
        Q.addLast(a)
        left.setValue(a,Int.MIN_VALUE)
        right.setValue(a,Int.MIN_VALUE)

        while(Q.isNotEmpty()){
            val v = Q.removeFirst()
            val p = powers[v]
            val d = dist[v]
            left.destroyall(v,minOf(v + p,n-1),-v){
                w ->
                dist[w] = d + 1
                Q.addLast(w)
                right.setValue(w,Int.MIN_VALUE)
            }

            right.destroyall(maxOf(0,v-p),v,v){
                w ->
                dist[w] = d + 1
                Q.addLast(w)
                left.setValue(w,Int.MIN_VALUE)
            }
