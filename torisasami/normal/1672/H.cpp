...
int main() {
    ...
    while (q--) {
        ...
        l--, r--;
        int val = 0;
        auto itrl = lower_bound(all(v0), l);
        auto itrr = lower_bound(all(v0), r);
        chmax(val, (int)(itrr - itrl));
        itrl = lower_bound(all(v1), l), itrr = lower_bound(all(v1), r), chmax(val, (int)(itrr - itrl));
        cout << val + 1 << '\n';
    }
}