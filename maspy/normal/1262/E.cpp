...
void solve() {
  ...
  W += 2;
  vc<string> G(H);
  FOR(x, H) G[x].assign(W, '.');
  FOR3(x, 1, H - 1) {
    STR(S);
    FOR(y, W - 2) G[x][y + 1] = S[y];
  }
  ...
  auto check = [&](ll t) -> bool {
    ...
    FOR(x, H) FOR(y, W) if ((dist[x][y] <= t) != (G[x][y] == 'X')) return false;
    return true;
  };
  ll t = binary_search(check, 0, min(H, W));
  print(t);
  FOR3(x, 1, H - 1) {
    string S;
    S.assign(W - 2, '.');
    FOR3(y, 1, W - 1) if (D[x][y] > t) S[y - 1] = 'X';
    print(S);
  }
}
