...
public class CFCR549 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                permutation[i] = io.readInt();
            }
            int[] next = new int[n + 1];
            for (int i = 1; i <= n; i++) {
                next[permutation[i]] = permutation[i % n + 1];
            }
            ...
            int[] nextIndex = new int[200000 + 1];
            for (int i = m; i >= 1; i--) {
                int nextI = nextIndex[next[seq[i]]];
                if (nextI > 0) {
                    nodes[nextI].children.add(nodes[i]);
                }
                nextIndex[seq[i]] = i;
            }
            ...
            Segment root = Segment.build(1, m, nodes);
            for (int i = 0; i < q; i++) {
                int l = io.readInt();
                int r = io.readInt();
                int min = Segment.query(l, r, 1, m, root);
                if (min <= r) {
                    io.cache.append(1);
                } else {
                    io.cache.append(0);
        ...
        public void visit(Node root, Deque<Node> deque) {
            if (root.visited) {
                return;
            }
            root.visited = true;
    ...
    public static class Segment implements Cloneable {
        ...
        public static Segment build(int l, int r, Node[] nodes) {
            Segment segment = new Segment();
            if (l != r) {
                int m = (l + r) >> 1;
                segment.left = build(l, m, nodes);
                segment.right = build(m + 1, r, nodes);
                segment.pushUp();
            } else {
                segment.min = nodes[l].end;
            }
            return segment;
        }
