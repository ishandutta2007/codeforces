...
void main(int case_id) {
  int n, d;
  scan(n, d);
  vla<int> a(n + 2);
  a[0] = 0;
  scan(a.drop(1).take(n));
  a[n + 1] = d + 1;
  int mn = inf, mn_i;
  for (int i : rep1(n)) {
    if (chmin(mn, a[i] - a[i - 1] - 1)) { mn_i = i; }
  }
  DUMP(a);
  DUMP(mn_i);
  int ans = 0;
  auto go = [&](int i) {
    if (case_id == 5) { DUMP(i); }
    auto v = rep(n + 2).filter(_1 != i).to();
    basic_string<int> s;
    for (int z : rep(sz(v) - 2)) { s += a[v[z + 1]] - a[v[z]] - 1; }
    all(s).sort();
    chmax(ans, min(s[0], max((s.back() - 1) / 2, a[v.back()] - a[reversed(v)[1]] - 2)));
  };
  if (1 < mn_i) { go(mn_i - 1); }
  go(mn_i);
  println(ans);
