...
{
    int n; cin>>n;
    vector<pair<pair<int, int>, ll>> seg;
    for (int i = 0; i<n; i++)
    {
        int l, r, c; cin>>l>>r>>c; seg.push_back(mp(mp(l, r), c));
    }

    int minn = seg[0].first.first;
    int maxx = seg[0].first.second;
    ll min_price = seg[0].second;
    ll max_price = seg[0].second;
    ll all_price = seg[0].second;

    cout<<seg[0].second<<' ';
    for (int i = 1; i<n; i++)
    {
        if (seg[i].first.first<minn)
        {
            minn = seg[i].first.first;
            min_price = seg[i].second;
            all_price = INF;
        }
        if (seg[i].first.second>maxx)
        {
            maxx = seg[i].first.second;
            max_price = seg[i].second;
            all_price = INF;
        }
        if (seg[i].first.first == minn) min_price = min(min_price, seg[i].second);
        if (seg[i].first.second == maxx) max_price = min(max_price, seg[i].second);
        if (seg[i].first.first == minn && seg[i].first.second == maxx) all_price = min(all_price, seg[i].second);

        cout<<min(min_price + max_price, all_price)<<' ';
    }
    cout<<endl;
}
