...
struct EulerTour {
  ...
  EulerTour(T& G, int root = 0) : G(G), root(root) { build(); };
...
#line 2 "/home/maspy/library/graph/treedoubling.hpp"
...
void solve() {
  ...
  TreeDoubling<Graph<int>, ll> TD(ET, [](ll x, ll y) { return x + y; }, 0, true);
  FOR(v, N) TD.set(v, edge_len[v]);
  TD.build();
  vi ANS(N);
  auto& par = ET.parent;
  FOR(v, N) {
    auto check = [&](ll x) -> bool { return x <= A[v]; };
    ll to = TD.max_ancestor(v, check);
    ANS[v] += 1;
    if (par[to] != -1) ANS[par[to]] -= 1;
  }
  FORIN(v, ET.VR) {
    auto p = par[v];
    if (p != -1) ANS[p] += ANS[v];
