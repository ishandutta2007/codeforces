...
int main() {
    const int N = 70;
    vector<string> E(1 << 8), T(1 << 8), F(1 << 8);
    {
        F[15 << 4] = "x";
        F[12 * 17] = "y";
        F[2 + 8 + 32 + 128] = "z";
    ...
    int rem = 3 * (1 << 8);
    rem -= 3;
    auto ins = [&](vector<string> &v, int b, const string &str) {
        if(empty(v[b]) or si(str) < si(v[b]))
            rem--, v[b] = str;
        else {
            if(si(v[b]) < si(str)) return;
            chmin(v[b], str);
        }
    };

    rep(_, 100) {
        rep(i, 1 << 8) rep(j, 1 << 8) if(!empty(E[i]) and !empty(T[j])) { ins(E, i | j, E[i] + "|" + T[j]); }
        rep(i, 1 << 8) if(!empty(T[i])) ins(E, i, T[i]);
        rep(i, 1 << 8) rep(j, 1 << 8) if(!empty(T[i]) and !empty(F[j])) ins(T, i & j, T[i] + "&" + F[j]);
        rep(i, 1 << 8) if(!empty(F[i])) ins(T, i, F[i]);
        rep(i, 1 << 8) if(!empty(F[i])) ins(F, ~i & allbit(8), "!" + F[i]);
        rep(i, 1 << 8) if(!empty(E[i])) ins(F, i, "(" + E[i] + ")");
    }
    ...
    TEST {
        ...
        int x = b.to_ullong();
        string res = E[x];
        if(cmp(F[x], res)) res = E[x];
        if(cmp(T[x], res)) res = T[x];
        OUT(res);
    }
}