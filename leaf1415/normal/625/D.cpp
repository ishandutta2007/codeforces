...
void nonzero(string &s){
	while(sz(s) >= 2 && s.back() == '0') s.pop_back();
}
string add(string s, string t)
{
	...
	reps(i, t){
		ll x = t[i]-'0';
		if(x >= 10 && i+1 == sz(t)) t += "0";
		t[i] = x%10+'0', t[i+1] += x/10;
	}
	nonzero(t);
	return t;
}

string calc(ll d, ll u)
{
	if(d > u) return "0";
	
	vector<ll> vec, vec2;
	reps(i, s) vec.push_back(s[i]-'0');
	vec.push_back(0);
	...
	while(d <= u){
		...
		if(vec[u+1] > 0) b += 10;
		if(b == 19) b = 9;
		vec[d] -= b%10, vec[d+1] -= b/10;
		
		ll p = d+1;
		while(vec[p] < 0){
			if(p+1 == sz(vec)) return "0";
			while(vec[p] < 0) vec[p+1]--, vec[p] += 10;
			p++;
		}
		
		if(d < u) vec[u] -= b%10, vec[u+1] -= b/10;
		p = u;
		while(vec[p] < 0){
			if(p+1 == sz(vec)) return "0";
			while(vec[p] < 0) vec[p+1]--, vec[p] += 10;
	...
	reps(i, t){
		...
		t[r] = min(9LL, vec2[i]) + '0';
		t[l] = max(0LL, vec2[i]-9) + '0';
	}
	nonzero(t);
	//outl(t);
	
	string rt = t;
	reverse(all(rt)), nonzero(rt);
	
	//outl(add(t, rt));
	
	if(s == add(t, rt)) return t;
	else return "0";
}
...
int main(void)
{
	...
	string ans = calc(0, n-1);
	if(ans == "0") ans = calc(0, n-2);
