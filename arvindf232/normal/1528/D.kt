...
class cannon(val from:Int, val to:Int,  val cost:Int){
...
fun main(){
    solve.cases{
        ...
        val m = getint
        val s = n
        val d = Array(s){LongArray(s){phuge} }
        //easiest to populate the distance array directly
        val all = Array(n){ mutableListOf<cannon>() }
        repeat(m){
            val a = getint
            val b = getint
            val time = getint
            val c = cannon(a,b,time)
            all[a].add(c)
        }
        //pos, time
        val doit = Array(n+1){fastEmptyList(n)}
        for(root in 0 until n){
            var base = plarge
            for(c in all[root]){
                base = minOf(base,c.cost)
            }
            val bn = base % n
            ...
            for(c in all[root]){
                for(i in 0 until n){
                    var newd =  c.to + i
                    if(newd >= n ) newd -= n
                    dist[newd] = minOf(dist[newd], i + c.cost)
                }
            }
            for(i in 0 until n){
                doit[dist[i] - base].addLast(i)
            }

            outer@while(pt <= n){
                while(doit[pt].isEmpty()){
                    pt ++
                    if(pt > n){
                        break@outer
                    }
                }
                val v = doit[pt].removeLast()
                if(dist[v] != pt + base){
                    continue
                }
                for(c in all[v]) {
                    ...
                    if(dist[newv] > dist[v] + c.cost) {
                        dist[newv] = dist[v] + c.cost
                        doit[dist[newv] - base].addLast(newv)
                    }
                }
                val next = v.next(n)
                if(dist[next] > dist[v] + 1 ){
                    dist[next] = dist[v] + 1
                    doit[dist[next] - base].addLast(next)
                }
            }
            dist[root] = 0
            put(dist.conca)
...
/*
huge inaccuracy
did not read the problem requirments
    - all pairs shortest path , not just shortest path

somehow stuck   (probably because bad ideas carried forward)
or assumed no fundamentals

single variable swaps casuseso much pain
