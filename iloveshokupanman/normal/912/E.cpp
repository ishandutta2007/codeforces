...
int main() {
    ...
    auto dfs = [&](auto&& dfs, ll e, ll i, ll x) ->void {
        lis[e].push_back(x);
        for (ll j = i; j < N; j += 2) {
            if (x > L / p[j]) { continue; }
            dfs(dfs, e, j, x * p[j]);
    ...
    dfs(dfs, 0, 0, 1);
    dfs(dfs, 1, 1, 1);
    rep(i, 2)sort(lis[i].begin(), lis[i].end());

    //for (ll x : lis[1])cout << x << " ";
    //cout << endl;
    ...
    auto count = [&](ll x) {
        ll ret = 0;
        for (ll y : lis[0]) {
            auto itr = upper_bound(lis[1].begin(), lis[1].end(), x / y);
            ret += distance(lis[1].begin(), itr);
        }
        //cout << x << " " << ret << endl;
        return ret;
    };
