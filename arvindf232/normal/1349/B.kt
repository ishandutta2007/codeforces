...
fun main(){
    solve.cases{
        ...
        val types = IntArray(n){
            if(L[it] == k) 1 else if(L[it] >= k ) 2 else 0
        }
        if(types.all { it != 1 }){
            put("NO")
            return@cases
        ...
        for(i in 1..n){
            rsum[i] = rsum[i-1] + if(types[i-1] >= 1) 1 else -1
        }
        val prefixMin = rsum.copyOf()
        for(i in 1 until n){
            prefixMin[i] = minOf(prefixMin[i-1],prefixMin[i])
        }
        for(i in 2..n){
            if(rsum[i] - prefixMin[i-2] >= 1){
                put("YES")
                return@cases
            }
        }
...
condition can be complicdaetd
 - reembre DP/ DS method of calculations
