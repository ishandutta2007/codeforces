...
void mian() {
	...
	multiset<int> lft;
	int sum = 0;
	REP(i, 1, n) a[i] = read(), sum += a[i], lft.insert(a[i]);
	st.insert(sum);
	while(SZ(st)) {
		if(SZ(st) > n) return puts("NO"), void();
		int x = *st.rbegin();
		if(lft.find(x) != lft.end()) st.erase(st.find(x)), lft.erase(lft.find(x));
		else {
			st.erase(st.find(x));
			st.insert(x >> 1), st.insert(x + 1 >> 1);
		}
	}
	puts("YES");
}
