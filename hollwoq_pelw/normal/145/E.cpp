/*
 /+===================================================+\
//+---------------------------------------------------+\
|.|\...>>>>>>> Hollwo_Pelw's 2nd template<<<<<<<...//|.|
\+---------------------------------------------------+//
 \+===================================================+/
*/
#include <bits/stdc++.h>
using namespace std;
// type
typedef long long ll;
typedef long double ld;
// pair
#define F                   first
#define S                   second
#define pii                 pair<int, int>
#define pll                 pair<ll, ll>
#define pdd                 pair<ld, ld>
// vector & !!?(string)
#define eb                  emplace_back
#define pb                  push_back
#define all(a)              a.begin(), a.end()
#define rall(a)             a.rbegin(), a.rend()
#define sz(a)               (int) a.size()
// I/O
#define setpre(n)           fixed << setprecision(n)
#define Ptest(x)            return cout << x << endl, (void) 0;
bool endline = false;
template<class T>
istream& operator >> (istream& inp, vector<T>& v){
    for (auto& it : v) inp >> it;
    return inp;
}
template<class T>
ostream& operator << (ostream& out, vector<T>& v){
    for (auto& it : v) out << it << (endline ? "\n" : " ");
    return out;
}
template<class T, class U>
istream& operator >> (istream& inp, pair<T, U>& v){
    inp >> v.F >> v.S;
    return inp;
}
template<class T, class U>
ostream& operator << (ostream& out, pair<T, U>& v){
    out << v.F << ' ' << v.S;
    return out;
}
void debug(){
    cout << endl;
}
template <typename H, typename... T>
void debug(H a, T... b){
    cout << a << ' ';
    debug(b...);
}
// geometry calculate
#define pi                  acos(-1.0)
#define g_sin(a)            sin(a*pi/180)
#define g_cos(a)            cos(a*pi/180)
#define g_tan(a)            tan(a*pi/180)
// set val
#define ms0(a)              memset(a,        0, sizeof(a));
#define ms1(a)              memset(a,        1, sizeof(a));
#define msn1(a)             memset(a,       -1, sizeof(a));
#define msinf(a)            memset(a, 0x3f3f3f, sizeof(a));

void FAST_IO(string filein = "", string fileout = ""){
    if (fopen(filein.c_str(), "r")){
        freopen(filein.c_str(), "r", stdin);
        freopen(fileout.c_str(), "w", stdout);
    }
    cin.tie(0), cout.tie(0) -> sync_with_stdio(0);
}

void Hollwo_Pelw();

signed main(){
    #ifdef hollwo_pelw_local
        FAST_IO("input.inp", "output.out");
        auto start = chrono::steady_clock::now();
    #else
        FAST_IO(".inp", ".out");
    #endif
    int testcases = 1;
    // cin >> testcases;
    for (int test = 1; test <= testcases; test++){
        // cout << "Case #" << test << ": ";
        Hollwo_Pelw();
    }
    #ifdef hollwo_pelw_local
        auto end = chrono::steady_clock::now();
        cout << endl;
        cout << "Excution time : " << chrono::duration_cast<chrono::milliseconds> (end - start).count() << "[ms]" << endl;
    #endif
    return 0;
}

// constant
const int allmod[3] = {(int) 1e9 + 7, 998244353, (int) 1e9 + 9};
const int mod = allmod[0];
const int MAXN = 1 << 19;
const int inf = 2e9;
const ll linf = 1e18;
// #define int long long

int n, m, arr[1 << 20];

struct node {
    int cnt[4];
    // 0 -> 01
    // 1 -> 10
    // 2 -> 0
    // 3 -> 1
} st[1 << 22];

node merge(node l, node r) {
    node m;
    m.cnt[0] = max(l.cnt[0] + r.cnt[3], l.cnt[2] + r.cnt[0]);
    m.cnt[1] = max(l.cnt[1] + r.cnt[2], l.cnt[3] + r.cnt[1]);
    m.cnt[2] = l.cnt[2] + r.cnt[2];
    m.cnt[3] = l.cnt[3] + r.cnt[3];
    return m;
}

void rev(node & x) {
    swap(x.cnt[0], x.cnt[1]);
    swap(x.cnt[2], x.cnt[3]);
}

node tonode(int x) {
    node r;
    r.cnt[0] = r.cnt[1] = 1;
    r.cnt[2 ^ x] = 1;
    r.cnt[3 ^ x] = 0;
    return r;
}

int lz[1 << 22];

#define left id << 1, tl, tm
#define right id << 1 | 1, tm + 1, tr

void build(int id = 1, int tl = 1, int tr = n) {
    if (tl == tr) {
        st[id] = tonode(arr[tl]);
    }else{
        int tm = tl + tr >> 1;
        build(left), build(right);
        st[id] = merge(st[id << 1], st[id << 1 | 1]);
    }
    // cout << tl << ' ' << tr << endl;
    // for (int i = 0; i < 4; i++)
    //     cout << st[id].cnt[i] << ' ';
    // cout << endl;
}

void apply(int id) {
    lz[id] ^= 1;
    rev(st[id]);
}

void push(int id){
    if (lz[id]) {
        apply(id << 1);
        apply(id << 1 | 1);
    }
    lz[id] = 0;
}

void upd(int l, int r, int id = 1, int tl = 1, int tr = n){
    if (l > r || l > tr || tl > r) return ;
    if (l <= tl && tr <= r) return apply(id);
    int tm = tl + tr >> 1; push(id);
    upd(l, r, left), upd(l, r, right);
    st[id] = merge(st[id << 1], st[id << 1 | 1]);
}

void Hollwo_Pelw() {
    cin >> n >> m;
    for (int i = 1; i <= n; i++){
        char c; cin >> c;
        arr[i] = c == '7';
    }
    build();
    for (int i = 1, l, r; i <= m; i++){
        string t; cin >> t;
        if (t[0] == 'c') 
            cout << st[1].cnt[0] << "\n";
        else
            cin >> l >> r, upd(l, r);
    }
}



/*

./-=====>>><<<-------- DEBUG -------->>><<<=====-\.
/.................................................\
+====================== INP ======================+



+====================== OUT ======================+



+======================*****======================+
\................................................./
.\-=====>>><<<--------= END =-------->>><<<=====-/.

*/