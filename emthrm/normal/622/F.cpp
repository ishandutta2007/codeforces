...
template <typename T>
T lagrange_interpolation(const vector<T> &y, T t) {
  int n = y.size();
  assert(t < 0 || t >= n);
  vector<T> fact(n, 1);
...
ModInt faulhaber(ll n, int k) {
  if (n < 1) return 0;
  vector<ModInt> y(k + 2, 0);
  FOR(i, 1, k + 2) y[i] = y[i - 1] + ModInt(i).pow(k);
  if (n - 1 < k + 2) return y[n - 1];
  return lagrange_interpolation(y, ModInt(n - 1));
}

int main() {
  ll n; int k; cin >> n >> k;
  cout << faulhaber(n + 1, k) << '\n';
  return 0;
}