...
void solve() {
    ...
    vector<int> balans(n);
    for (int i = 0; i < n - 1; ++i) {
        balans[i + 1] = balans[i] + (s[i] == '0' ? 1 : -1);
    }
    int b = balans.back() + (s.back() == '0' ? 1 : -1);
    int minb = *min_element(balans.begin(), balans.end());
    int maxb = *max_element(balans.begin(), balans.end());
    if (b == 0) {
        if (maxb >= x) answer(-1); else answer(0);
    }
    int ans = 0;
    for (int cur_b : balans){
        if ((x - cur_b) % b == 0 && (x - cur_b) / b >= 0) ++ans;
    }
    answer(ans);
}