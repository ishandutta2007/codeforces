...
void main() {
    R(n);
    REP1(i,1,n) R(a[i]);
    REP1(i,1,n) st[0][i]=a[i];
    int lgn=lg2(n);
    REP(i,lgn) {
        int l=1<<i;
        for ( int j=1; j+l+l-1<=n; j++ ) st[i+1][j]=st[i][j]|st[i][j+l];
    }
    map<int,int> pre;
    LL cnt=0;
    REP1(i,1,n) {
        int l=i,r=i;
        for ( int j=lgn; j>=0; j-- ) {
            if ( l-(1<<j)>=1 && (st[j][l-(1<<j)]|a[i])==a[i] ) l-=1<<j;
            if ( r+(1<<j)<=n && (st[j][r+1]|a[i])==a[i] ) r+=1<<j;
        }
        if ( pre.count(a[i]) ) chkmax(l,pre[a[i]]+1);
        cnt+=(i-l+1LL)*(r-i+1LL)-1;
        pre[a[i]]=i;
    }
    LL ans=n*(n-1LL)/2-cnt;
    W(ans);
}
