...
void solve(){
    ...
    a.resize(m);
    Rev(a);
    vec(vector<ll>,group,m);
    vector<ld>ave;
    each(g,group){
        LL(k);
        g.resize(k);
        in(g);
        ave.push_back(sum(g)/(ld)g.size()-EPS);
    }
    auto idx=iota(m);
    sort(all(idx),[&](ll x,ll y){return ave[x]<ave[y];});
    vec(ll,rev,m);
    rep(m)rev[idx[i]]=i;
    auto ave_sort=ave;
    Sort(ave_sort);
    ...
    rep(m){
        const ll at=idx[i];
        int x=0;
        if(i)x|=ave_sort[i]<=a[i-1];
        x<<=1;
        x|=ave_sort[i]<=a[i];
        x<<=1;
        if(i+1<m)x|=ave_sort[i]<=a[i+1];
        seg.set(i,x);
    }
    rep(at,m){
        const ll i=rev[at];
        ...
        const ll s=sum(g);
        each(x,g){
            const ld ave=(s-x)/(ld)(g.size()-1)-EPS;
            ll j=lower_bound(all(ave_sort),ave)-ave_sort.begin();
            if(j>i)j--;
            bool ans;
            if(i<=j){
                ans=seg.get(0,i)&2&&seg.get(i+1,j+1)&4&&seg.get(j+1,m)&2&&ave<=a[j];
            }
            else{
                ans=seg.get(0,j)&2&&seg.get(j,i)&1&&seg.get(i+1,m)&2&&ave<=a[j];
            }
            print(ans);
        }
    }
