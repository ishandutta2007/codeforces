...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) rep(j, 1, n) scanf("%lf", &p[i][j]), p[i][j] /= 100;//cin >> p[i][j], p[i][j] /= 100;
	used[n] = true;
	rep(i, 1, n){
		if(used[i]) continue;
		esum[i] = 1;
		pmul[i] = 1-p[i][n];
		if(1-pmul[i] > 0) dp[i] = esum[i] / (1-pmul[i]);
		else dp[i] = inf;
	}
	
	rep(i, 1, n-1){
		double min_val = inf; ll min_j = 0;
		rep(j, 1, n){
			if(used[j]) continue;
			if(min_val > dp[j]){
				min_val = dp[j];
		...
		rep(j, 1, n){
			if(used[j]) continue;
			esum[j] += pmul[j] * p[j][min_j] * dp[min_j];
			pmul[j] *= 1-p[j][min_j];
			if(1-pmul[j] > 0) dp[j] = esum[j] / (1-pmul[j]);
			else dp[j] = inf;
		}
	}
	outf(dp[1]);
