...
void bfs(vector<ll> G[], int n, int S, ll dist[])
{
	queue<int> Q;
	Q.push(S);
...
ll n, m, f, k;
ll h[10005], p[10];
vector<ll> G[10005], g[10005];
ll dist[10005];
bool reach[10][10005];
bool dp[10005][1<<6];
bool trans[10005][1<<6];
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n){
			g[i].clear();
			for(auto u : G[i]){
				if(dist[i] + 1 == dist[u]) g[i].pb(u);
			}
		}
		rep(i, 0, k-1){
			rep(j, 1, n) reach[i][j] = false;
			dfs(h[p[i]], reach[i]);
		}
		...
		rep(i, 1, f){
			rep(j, 0, K-1){
				vector<P> vec;
				rep(l, 0, k-1){
					if(j & (1<<l)) vec.pb(P(dist[h[p[l]]], l));
				}
				sort(all(vec));
				
				trans[i][j] = true;
				rep(l, 1, (int)sz(vec)-1){
					if(!reach[vec[l-1].se][h[p[vec[l].se]]]) trans[i][j] = false;
				}
				if(sz(vec) && !reach[vec.back().se][h[i]]) trans[i][j] = false;
			}
		}
		rep(i, 0, k-1) rep(j, 1, K-1) trans[p[i]][j] = false;
		
		//rep(i, 1, f) rep(j, 0, K-1) if(trans[i][j]) outl(i, j);
		
		rep(i, 0, f) rep(j, 0, K-1) dp[i][j] = false;
		dp[0][0] = true;
		rep(i, 0, f-1){
			rep(j, 0, K-1){
				if(!dp[i][j]) continue;
				rep(l, 0, K-1){
					if(trans[i+1][l]) dp[i+1][j|l] = true;
				}
			}
		}
		ll ans = inf;
		rep(j, 0, K-1) if(dp[f][j]) chmin(ans, k-popcount(j));
		outl(ans);
	}
