...
void solve() {
    cin >> n >> m >> s;
    rep(i, 1, n * m) {
        pre1[i] = s[i - 1] > 48 ? i : i > 1 ? pre1[i - 1] : 0;
        pre2[i] = s[i - 1] > 48 ? i : i > m ? pre2[i - m] : 0;
        ans1[i] = ans2[i] = 0;
    }
    rep(i, 1, n * m) if(pre1[i] > max(0, i - m)) ans1[i]++;
    rep(i, 1, n * m) if(pre2[i]) ans2[i]++, ans2[min(i + m, n * m + 1)]--;
    rep(i, 1, n * m) {
        if(i > m) ans1[i] += ans1[i - m];
        ans2[i] += ans2[i - 1];
        cout << ans1[i] + ans2[i] << ' ';
    }
    cout << '\n';
