...
// freopen("output.txt","w",stdout);
	...
	rep(i,1,n){
		cin>>x[i];
		y = 1, cnt = 0;
		while(!(x[i]&1))y<<=1, x[i]>>=1, ++cnt;
		if(!hs.count(x[i]))hs[x[i]] = ++id;
		v[hs[x[i]]].pb(i);
		x[i] = cnt;
	}
	for(auto i:hs){
		rep(k,0,50)pos[k].clear();
		for(int j:v[i.se])pos[x[j]].pb(j);
		rep(k,0,50)if(pos[k].size()){
			sort(all(pos[k]));
			for(int j = 1; j<pos[k].size(); j+= 2)pos[k+1].pb(pos[k][j]);
			if(pos[k].size()&1)ans.pb({pos[k].back(),i.fi<<k});
		}
	}
	sort(all(ans));
	cout<<ans.size()<<endl;
	for(auto i:ans)cout<<i.se<<' '; cout<<endl;
	return 0;
}