...
public class D {
	...
	public void solve() throws IOException {
		...
		for (int qwerty = 1; qwerty <= test; qwerty++) {
			...
			long[] prefPC = new long[n];
			for (int i = 1; i < n; i++) {
				if (i % 2 == 1) {
					prefPC[i] = prefPC[i - 1] + a[i - 1];
				} else {
					prefPC[i] = prefPC[i - 1] - a[i - 1];
				}
			}
			long[] difCPC = new long[n];
			long[] difPPC = new long[n];
			long[] difCCP = new long[n];
			long[] difPCP = new long[n];
			difCPC[1] = -2 * a[0];
			difPCP[1] = 2 * a[0];
			for (int i = 2; i < n; i++) {
				difCPC[i] = difCPC[i - 1];
			...
			Fenvik fCP = new Fenvik(sz);
			Fenvik fPC = new Fenvik(sz);
			long ans = 0;
			long sum = 0;
			for (int i = 0; i < n; i++) {
				sum += a[i];
			...
			for (int i = 1; i < n; i++) {
				if (i % 2 == 1) {
					fCP.add(map.get(difCCP[i]), 1);
					if (i > 1) {
						fCP.add(map.get(difPCP[i]), 1);
					}
//					System.out.println(i + " " + difCCP[i] + " " + difPCP[i]);
					long floor = -sufP[i] + prefPC[i];
//					System.out.println(i + " " + -floor);
					Long higher = set.higher(floor);
					if (higher != null) {
						int min = map.get(higher);
						ans += fCP.sum(min, sz - 1);
					}
					if (i == n - 1) {
						break;
					}
					floor = -sufC[i] + prefPC[i];
//					System.out.println(i + " " + -floor);
					higher = set.higher(floor);
					if (higher != null) {
						int min = map.get(higher);
						ans += fCP.sum(min, sz - 1);
					}
				} else {
					fPC.add(map.get(difCPC[i]), 1);
					fPC.add(map.get(difPPC[i]), 1);
//					System.out.println(i + " " + difCPC[i] + " " + difPPC[i]);
					long floor = -sufP[i] - prefPC[i];
//					System.out.println(i + " " + -floor);
					Long higher = set.higher(floor);
					if (higher != null) {
						int min = map.get(higher);
						ans += fPC.sum(min, sz - 1);
					}
					if (i == n - 1) {
						break;
					}
