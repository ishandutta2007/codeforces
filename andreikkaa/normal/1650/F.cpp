...
inline void _solve() {
    ...
    for(int i = 0; i < n; ++i) {
        for(int j = 0; j <= sz(e[i]); ++j) {
            fill(dp[j], dp[j] + P, make_pair(INF, -1));
        }
        ...
        for(int j = 0; j < sz(e[i]); ++j) {
            for(int k = 0; k < P; ++k) {
                ...
                if(k + e[i][j].y.x < P && dp[j][k].x + e[i][j].x < dp[j + 1][k + e[i][j].y.x].x) {
                    dp[j + 1][k + e[i][j].y.x] = {dp[j][k].x + e[i][j].x, 0};
                }
            }
        }
        ...
        int opt = 100;
        for(int j = 100; j < P; ++j) {
            if(dp[sz(e[i])][j].x < dp[sz(e[i])][opt].x) {
                opt = j;
            }
        }

        if(cur + dp[sz(e[i])][opt].x > a[i]) {
            cout << -1 << endl;
            return;
        }
        cur += dp[sz(e[i])][opt].x;

        for(int j = sz(e[i]); j > 0; --j) {
            if(dp[j][opt].y == 0) {
                ans.push_back(e[i][j - 1].y.y);
                opt -= e[i][j - 1].y.x;
            } else {
