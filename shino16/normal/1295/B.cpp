...
int main() {
  ...
  rep(t) {
    ...
    map<int, int> cnt;
    int balance = 0; cnt[0]++;
    rep(i, n) cnt[balance += (s[i] == '0') - (s[i] == '1')]++;
    cnt[balance]--;
    debug(balance);
    int mi = min_element(all(cnt))->first, ma = max_element(all(cnt))->first;
    debug(mi );debug(ma);
    if (x > ma and balance <= 0 or x < mi and balance >= 0) {
      OUT(0); continue;
    }
    if (balance == 0) {
      OUT(-1); continue;
    ...
    ll res = 0;
    ll distance = min(abs(x-mi), abs(x-ma));
    if (x > ma or x < mi) x -= balance * (abs(distance / balance)-1);
    debug(distance);
    debug(x);
    while (not cnt.count(x)) x -= balance;
    while (cnt.count(x)) {
      debug(x);
      res += cnt[x];
      x -= balance;
