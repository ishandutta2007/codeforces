...
class Solver {
    ...
    public void solve() {
        ...
        Func<int, int, int, int, int> cnt2 = (i1, j1, i2, j2) => {
            int h1 = Math.Max(i1 - 1, 0);
            int h2 = Math.Min(i2, h);
            int w1 = Math.Max(j1 - 1, 0);
            int w2 = Math.Min(j2, w);
            if (h1 > h2 || w1 > w2) return 0;
            return imos[h2][w2] - imos[h2][w1] - imos[h1][w2] + imos[h1][w1];
        };
        Func<int, int, int> cnt = (i, j) => {
            return cnt2(i - r, j - r, i + r, j + r);
        };
        ...
        MyMath.setfacts(10010);
        long ans = 0;
        for (int i = 0; i < n; i++)
        {
            ans = (ans + (MyMath.comb(n, m) - MyMath.comb(n - c[i], m) + M) % M * p[i][2] % M * p[i][2]) % M;

            for (int j = i + 1; j < n; j++)
            {
                int cc = cnt2(Math.Max(p[i][0], p[j][0]) - r, Math.Max(p[i][1], p[j][1]) - r, Math.Min(p[i][0], p[j][0]) + r, Math.Min(p[i][1], p[j][1]) + r);
                ans = (ans + ((MyMath.comb(n, m) - MyMath.comb(n - c[i], m) - MyMath.comb(n - c[j], m) + MyMath.comb(n - c[i] - c[j] + cc, m)) % M + M) % M * p[i][2] % M * p[j][2] % M * 2) % M;
            }
        }
        Prt(ans);
