...
int solve() {
    ...
    for (int mask = 1; mask < (1 << n); ++mask) {
        for (int i = 0; i < n; ++i) {
            if (checkbit(mask, i)) {
                cost[mask] = cost[mask ^ (1 << i)] + w[n - 1 - i];
            }
        }
        masks[mask] = {cost[mask], mask};
    }
    int N = 1 << n;
    sort(masks, masks + N);
    FI(m) {
    ...
    for (int x = 0; x < N; ++x) {
        for (int i = 0; i < N; ++i) {
            xorSums[x][i + 1] = xorSums[x][i] + cnt[x ^ masks[i].second];
        }
    }
    while (q--) {
        string t;
        ...
        int ans = xorSums[mask][l];
        cout << ans << '\n';
//        int res = 0;
//        for (int i = 0; i < (1 << 12); ++i) {
//            if (cost[i ^ mask] <= k) {
//                if (cnt[i]) {
//                    DBN(i, cnt[i], cost[i ^ mask]);
//                }
//                res += cnt[i];
//            }
//        }
...
pii p[103030];
template <class T>
vector<T> min_cyclic_shift(vector<T> s) {
    int n = (int)s.size();
    for (int i = 0 ; i < n; ++i) {
        auto c = s[i];
        s.push_back(c);
    ...
    return {s.begin() + ans, s.begin() + ans + n / 2};
}
...
auto encode(int n) {
    ...
    convex_hull(v);
    vector<array<ll, 3>> difs;
    v.push_back(v[0]);
    v.push_back(v[1]);
    for (int i = 0; i + 2 < v.size(); ++i) {
        array<ll, 3> code;
        code[0] = dist(v[i], v[i + 1]);
        code[1] = dist(v[i + 1], v[i + 2]);
        code[2] = cw(v[i], v[i + 1], v[i + 2]);
        difs.push_back(code);
    }
    return min_cyclic_shift(difs);
}
int solveE() {
    int n, m;
    cin >> n >> m;
