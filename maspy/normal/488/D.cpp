...
void solve() {
  ...
  VEC(ll, A, N);
  using T = pi;
  SegTree<T> seg([](T x, T y) { return mp(min(x.fi, y.fi), max(x.se, y.se)); },
                 mp(INF, -INF));
  seg.init(N);
  FOR(i, N) seg.set(i, mp(A[i], A[i]));

  SegTree<ll> dp([](ll x, ll y) { return min(x, y); }, INF);
  dp.init(N + 1);
  dp.set(0, 0);

  FOR3(R, 1, N + 1) {
    auto check = [&](T e) -> bool { return e.se - e.fi <= S; };
    ll L = seg.min_left(check, R);
    ll M = R - D;
    if (L <= M) {
      ll x = dp.fold(L, M + 1) + 1;
      if (x < INF) dp.set(R, x);
    }
  }
  ll ANS = dp.fold(N, N + 1);
  if (ANS == INF) ANS = -1;
  print(ANS);
}
