...
int main(){
    ...
    rep(zz,T){
        ...
        rep(i,n){
            rep(k,s[i].size()){
                a[i].push_back(mp[s[i][k]]);
                p[i][mp[s[i][k]]].push_back(k);
            }
        }
        vv(int,dp,52,1<<n,-inf);
        pair<int,int> ppp = make_pair(-1,-1);
        vector<vector<pair<int,int>> > pre(52,vector<pair<int,int> > (1<<n,ppp));
        rep(cc,52){
            bool f = 1;
            rep(i,n){
                if(p[i][cc].empty()){
        ...
        vector<tuple<vector<int>,int,int> > cand;
        rep(bits,1<<n){
            rep(cc,52){
                bool f = 1;
                vector<int> ss;
                rep(i,n){
                    if((bits>>i)&1){
                        ...
                        ss.push_back(p[i][cc][1]);
                    }else{
                        if(p[i][cc].size()<1){
                            f = 0;
                            break;
                        }
                        ss.push_back(p[i][cc][0]);
                    }
                }
                if(f){
                    cand.push_back(MT(ss,cc,bits));
                }
            }
        }
        sort(all(cand));
        for(auto& [v,cc,bits]:cand){
            if(dp[cc][bits]<0)continue;
            dbg(cc,bits,dp[cc][bits]);
            rep(nc,52){
                bool ng = 0;
                int nbits = 0;
                rep(i,n){
                ...
                if(ng)continue;
                dbg(nc,nbits);
                if(chmax(dp[nc][nbits],dp[cc][bits]+1)){
                    pre[nc][nbits] = MP(cc,bits);
                }
            }
        }
        int mx = 0;
