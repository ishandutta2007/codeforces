...
int main() {
    ...
    F0R(i,m+2) F0R(j,3) F0R(k,3) dp[i][j][k] = -MOD;
    dp[1][0][0] = 0;
    FOR(i,1,m+1) F0R(j,3) F0R(k,3) if (dp[i][j][k] != -MOD) {
        F0R(z,3) if (j+k+z <= co[i] && k+z <= co[i+1] && z <= co[i+2]) {
            ckmax(dp[i+1][k][z],dp[i][j][k]+z+(co[i]-j-k-z)/3);
        }
    }
    pr(dp[m+1][0][0]);
    // you should actually read the stuff at the bottom
