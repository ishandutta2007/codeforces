...
public class Main
{
	public static void main(String[]args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
			Arrays.fill(matrix[i],1000_000_000);
		matrix[0][0]=a[0].charAt(0)=='a'?0:1;
		for(int i=1; i<2*n-1; i++)
		{
			if(i<n) {
			matrix[i][0]=matrix[i-1][0]+((a[i].charAt(0)=='a')?0:1);
			matrix[0][i]=matrix[0][i-1]+((a[0].charAt(i)=='a')?0:1);
			}
			for(int j=max(1,i-n+1); j<i&&j<n; j++)
				matrix[i-j][j]=min(matrix[i-j-1][j],matrix[i-j][j-1])+((a[i-j].charAt(j)=='a')?0:1);
		}
		if(matrix[n-1][n-1]<=k)
		{	
		...
		for(int i=len; i<2*n-2; i++)
		{
			...
			for(int j=max(0,i-n+1); j<=i&&j<n; j++)
				...
				{
					if(i-j!=n-1)
						minc=min(minc,a[i-j+1].charAt(j));
					if(j!=n-1)
						minc=min(minc,a[i-j].charAt(j+1));
				}
			p(minc);
			for(int j=max(0,i-n+1); j<=i&&j<n; j++)
				if(matrix[i-j][j]==k)
				{
					if(i-j!=n-1)
					{
						if(a[i-j+1].charAt(j)==minc)
							matrix[i-j+1][j]=k;
					}
					if(j!=n-1)
					{
						if(a[i-j].charAt(j+1)==minc)
							matrix[i-j][j+1]=k;
					}
				}
		}
