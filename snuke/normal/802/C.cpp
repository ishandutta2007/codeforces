...
struct Mincost {
  ...
  TC solve(int sv, int tv, TL flow) {
    ...
    while (flow>0) {
      dijk(sv);
      if (dist[tv] == LINF) return -1; // (double) LINF !!
...
int main() {
  ...
  rep(i,n) {
    if (pi[i] != INF) {
      ll co = ll(pi[i]-i)*INF*2-INF - c[a[i]-1];
      g.add(n+i,pi[i],co,1);
    }
  }
  ll dec = ll(INF)*(n*2)*m - (ll)INF*n - g.solve(sv,tv,m);
  ll ans = 0;
  rep(i,n) ans += c[a[i]-1];
  ans -= dec;
  cout<<ans<<endl;
