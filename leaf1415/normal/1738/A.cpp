...
{
	ll ans = 0, t = 0;
	if(sz(avec)) ans += avec[0], avec.erase(avec.begin()), t = 1;
	
	while(1){
		if(t == 0) {
			if(sz(avec) == 0) break;
			ans += 2*avec.back(), avec.pop_back(), t = 1;
		}
		if(t == 1) {
			if(sz(bvec) == 0) break;
			ans += 2*bvec.back(), bvec.pop_back(), t = 0;
		}
	}
	for(auto x : avec) ans += x;
	for(auto x : bvec) ans += x;
	return ans;
}
...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> avec, bvec;
		rep(i, 1, n){
			if(a[i] == 0) avec.pb(b[i]);
			else bvec.pb(b[i]);
		}
		sort(all(avec)), sort(all(bvec));
		cout << max(calc(avec, bvec), calc(bvec, avec)) << "\n";
	}
