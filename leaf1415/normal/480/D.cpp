...
ll n, S;
ll l[505], r[505], w[505], s[505], v[505];
ll L[505], R[505];
vector<ll> rvec[505];
ll dp[505][1005], dp2[505];
...
int main(void)
{
	...
	vector<P> vec;
	rep(i, 1, n) vec.pb(P(r[i]-l[i]+1, i));
	sort(all(vec));
	
	vector<ll> comp;
	rep(i, 1, n) comp.pb(r[i]);
	uniq(comp);
	rep(i, 1, n){
	...
	for(auto p : vec){
		...
		rep(i, 0, S){
			...
			rep(j, 1, n){
				for(auto jd : rvec[j]){
					if(id == jd) continue;
					if(r[jd] > r[id] || l[jd] < l[id]) continue;
					if(i-w[jd] >= 0) chmax(dp2[j], dp2[L[jd]-1] + dp[jd][min(i-w[jd], s[jd])] + v[jd]);
				}
				chmax(dp2[j], dp2[j-1]);
			}
			dp[id][i] = dp2[n];
		}
		rep(i, 1, S) chmax(dp[id][i], dp[id][i-1]);
	}
	
	//rep(i, 1, n) outa(dp[i], 0, S);
