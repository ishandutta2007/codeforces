...
constexpr int MAXA = 256;

int n;
vc<int> poses[MAXA];

int Solve( vc<bool> &a , int tot1 ){
	int ava0 = 0, ava1 = tot1;
	int ans = 0;
	int pl = 0, pr = a.size()-1;
	while(a[pl]){ --ava1; ++pl; }
	while(a[pr]){ --ava1; --pr; }
	ava0 = 2;
	while( pl < pr ){
		Mymax(ans,ava0+ava1);
		++pl; while( pl <  a.size() and a[pl] ){ --ava1; ++pl; }
		--pr; while( pr >= 0 and a[pr] ){ --ava1; --pr; }
		if( pl >= pr ) break;
		ava0 += 2;
	}
	return ans;
...
int main(){
	...
	For(TwT,T){
		...
		int ans = 0;
		For(x,200) Mymax(ans,(int)poses[x].size());
		For(a0,200)
			For(a1,200){
				if( a0 == a1 ) continue;
				if( poses[a0].empty() or poses[a1].empty() ) continue;
				if( poses[a0].size() + poses[a1].size() <= ans ) continue;
				static vc<bool> t;
				t.clear();
				auto a0p = poses[a0].begin();
				auto a1p = poses[a1].begin();
				while( a0p != poses[a0].end() or a1p != poses[a1].end() ){
					if( a1p == poses[a1].end() or (a0p!=poses[a0].end()&&(*a0p)<(*a1p)) ){
						t.Eb(0);
						++a0p;
					}else{
						t.Eb(1);
				...
				int tans = Solve(t,poses[a1].size());
				MyMax(ans,tans);
			}
		printf("%d\n",ans);
		For(i,200) poses[i].clear();
	}
