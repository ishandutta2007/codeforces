...
void solve()
{
    ...
    for (int i = 0; i<n; i++)
    {
        if (dsu.find_set(a[i]) != dsu.find_set(b[i]))
        {
    ...
    vector<int> szs;
    for (auto it: roots) szs.push_back(dsu.sz[it]);

    //print(szs);

    int adder = 0;
    for (auto it: szs) adder+=(it/2);

    ll ans = 0;
    for (int i = 0; i<adder; i++) ans-=2*i;
    for (int i = n-1; i>=n-adder; i--) ans+=2*i;
    cout<<ans<<endl;
