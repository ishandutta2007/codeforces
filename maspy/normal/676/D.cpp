...
void solve() {
  ...
  FOR(x, H) {
    STR(row);
    FOR(y, W) { A[x][y] = S.find(row[y]); }
  }

  auto can_go = [&](ll x, ll y, ll s, ll d) -> bool {
    //  s d 
    d = (d + s) % 4;
    return A[x][y] >> d & 1;
  };
  ...
  que.eb(0, a, b);
  while (len(que)) {
    auto [s, x, y] = que.front();
    que.pop_front();
    auto TO = [&](ll ns, ll nx, ll ny) -> void {
      if (ns == 4) ns = 0;
      if (chmin(dist[ns][nx][ny], dist[s][x][y] + 1)) que.eb(ns, nx, ny);
    };
    TO(s + 1, x, y);
    FOR(d, 4) {
      ll nx = x + dx[d], ny = y + dy[d];
      if (!isin(nx, ny)) continue;
      if (!can_go(x, y, s, d)) continue;
      if (!can_go(nx, ny, s, (d + 2) % 4)) continue;
      TO(s, nx, ny);
    }
  }
  ll ANS = INF;
  FOR(s, 4) chmin(ANS, dist[s][c][d]);
  if (ANS == INF) ANS = -1;
  print(ANS);
}
