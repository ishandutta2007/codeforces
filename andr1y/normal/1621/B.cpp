...
signed main(){
    ...
    while(q--){
        ll n;
        cin >> n;
        pll minv = {inf, inf}, maxv = {0, inf};
        map<pll, ll> vs;
        for(ll i = 0;i<n;i++){
            ll l, r, c;
            cin >> l >> r >> c;
            if(vs.count({l, r})) vs[{l, r}] = min(vs[{l, r}], c);
            else vs[{l, r}] = c;
            if(l < minv.x || (l == minv.x && c < minv.y)) minv = {l, c};
            if(r > maxv.x || (r == maxv.x && c < maxv.y)) maxv = {r, c};
            ll res = minv.y + maxv.y;
            if(vs.count({minv.x, maxv.x})) res=min(res, vs[{minv.x, maxv.x}]);
            cout<<res<<'\n';
        }
    }
}