...
int main() {
	...
	vector<int> phase(m, -1);
	phase[0] = 0;
	
	vector<vector<bool>> isPoss(m, vector<bool>(g + 1, false));
	for (int t = 0; ; t++) {
		vector<pii> q;
		int ql = 0;
		...
		while (ql < q.size()) {
			...
			if (node + 1 < m) {
				int newTime = time + d[node + 1] - d[node];
				if (newTime <= g && !isPoss[node + 1][newTime]) {
					isPoss[node + 1][newTime] = true;
					q.pb({node + 1, newTime});
				}
			}
			
			if (node - 1 >= 0) {
				int newTime = time + d[node] - d[node - 1];
				if (newTime <= g && !isPoss[node - 1][newTime]) {
					isPoss[node - 1][newTime] = true;
					q.pb({node - 1, newTime});
				}
