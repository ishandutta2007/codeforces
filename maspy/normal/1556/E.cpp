...
void solve() {
  ...
  auto Ac = cumsum(A);
  const ll INF = 1LL << 60;
  SegTree<ll> seg_max([](ll x, ll y) { return max(x, y); }, -INF);
  SegTree<ll> seg_min([](ll x, ll y) { return min(x, y); }, INF);
  seg_max.init(N + 1);
  seg_min.init(N + 1);
  FOR(i, N + 1) seg_max.set_val(i, Ac[i]);
  FOR(i, N + 1) seg_min.set_val(i, Ac[i]);
  ...
  FOR(_, Q) {
    ...
    ll ANS = [&]() -> ll {
      if (Ac[l] != Ac[r]) return -1;
      ll x = seg_max.fold(l, r);
      if (x > Ac[l]) return -1;
      ll y = seg_min.fold(l, r);
      return Ac[l] - y;
    }();
    print(ANS);
  }
