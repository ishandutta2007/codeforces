...
use text_scanner::{fscan, scan, scan_iter, scanln, scanln_iter};
use utils::adj4_iter;
...
fn run() {
    ...
    let t: usize = fscan(&mut stdin).unwrap();
    for _ in 0..t {
        let h: usize = fscan(&mut stdin).unwrap();
        let w: usize = fscan(&mut stdin).unwrap();
        let mut grid = vec![vec![0usize; w]; h];
        for y in 0..h {
            for x in 0..w {
                grid[y][x] = fscan(&mut stdin).unwrap();
            }
        }
        let mut max_grid = vec![vec![0usize; w]; h];
        for y in 0..h {
            for x in 0..w {
                if x > 0 {
                    max_grid[y][x] += 1;
