...
void solve(){
	...
	// auto recur = [&] (const auto& self, int l, int r) -> vector<Mint>{
	// 	if (l > r){
	// 		return vector<Mint>{1};
	// 	}
	// 	if (l == r){
	// 		return vector<Mint>{prod[l], 1};
	// 	}
	// 	int m = ((l + r) >> 1);
	...
	// vector<Mint> f = recur(recur, 0, (int)(prod.size()) - 1);
	Mint ans = 0;
	Mint x_prod = 1;
	for(int z = 0; z < cnt; ++z){
		x_prod *= (k + 1);
