...
fun main(){
    solve.cases{
        ...
        val m = getint
        val str = getstr
        val arr = IntArray(n * m ){if(str[it] == '1') 1 else 0 }
        val S = arr.torsq()

        val ret1 = IntArray(n * m + 1)
        val ret = tempIntervalSum(n * m + 1)
        for(rem in 1..m){
            val got = BooleanArray(n){ S.sumQuery((it-1) * m + rem ,it * m + rem -1 ) > 0}
            var have = 0
            for(t in 0 until n) {
                have += got[t].chi
                ret1[t * m + rem] += have
        ...
        for(c in 0 until m){
            val got = BooleanArray(n){ arr[it * m + c] == 1}
            var now = 0
            for(t in 0 until n){
                if(got[t]){
                    now = 1
                }
                ret.add(c+1 + t * m  , c + 1 +m -1 + t * m , now)
            }
        }
        val ret2 = ret.compile()
        for(i in 1..n *m){
            put(ret1[i] + ret2[i])
        }
