...
struct Segnode {
    ...
    static constexpr int64_t NINF = INT64_MIN / 4;
    int64_t value = NINF;

    Segnode(int64_t value = NINF) : value(value) {}
...
int main() {
    ...
    while (T--) {
        ...
        auto bound = [&](int xval) {
            return lower_bound(begin(pos), end(pos), xval) - begin(pos);
        };
        ...
        vector<int64_t> rain(X);
        int64_t slope = 0;
        for (int i = 1; i < X; i++) {
            slope += add[i - 1];
            rain[i] = rain[i - 1] + slope * (pos[i] - pos[i - 1]);
        }
        debug(pos);
        debug(rain);

        vector<int64_t> minus(X), plus(X);
        for (int i = 0; i < X; i++) {
            minus[i] = rain[i] - pos[i];
            plus[i] = rain[i] + pos[i];
        }

        segtree<Segnode> plain_seg(X, rain);
        segtree<Segnode> minus_seg(X, minus);
        segtree<Segnode> plus_seg(X, plus);
        ...
        for (int i = 0; i < N; i++) {
            ...
            overflown |= plain_seg.query_range(0, a + 1).value > M;
            overflown |= plain_seg.query_range(c, X).value > M;
            overflown |= minus_seg.query_range(a, b + 1).value > M - (x[i] - p[i]);
            overflown |= plus_seg.query_range(b, c + 1).value > M + (x[i] + p[i]);
            ans[i] = "10"[overflown];
        }
        cout << ans << '\n';
    }
