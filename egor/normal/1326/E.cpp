...
public:
    void solve() {
        ...
        auto q = in.readIntArray(n);
        decreaseByOne(p, q);

        SegmentTree<ll, ll> tree(n, [](int a, int b) -> int {
            return min(a, b);
        }, [](int a, int b) -> int {
            return a + b;
        ...
        auto pos = inverse(p);
        int at = n - 1;
        tree.update(0, pos[n - 1] + 1, -1);
        arri ans(n);
        for (int i : range(n)) {
            ans[i] = at + 1;
            tree.update(0, q[i] + 1, 1);
            while (tree.query(0, n) >= 0 && at > 0) {
                at--;
                tree.update(0, pos[at] + 1, -1);
            }
        }
        out.printLine(ans);
    }
