...
/*link https://codeforces.com/group/nIDHK2OChY/contest/252957/problem/E */
/*input
2
2 1 5
*/
...
int dfs(int d, int p) {
	...
	return sum + dydukai[d];
}

void findViso(int d, int p) {
	apl[d] = true;
...
int main () {
	...
	for (auto && [_, y] : b) {
		...
		for (auto && i : y) {
			gra[addd(un[a[i].a])].emplace_back(addd(un[a[i].b]), a[i].i);
			gra[addd(un[a[i].b])].emplace_back(addd(un[a[i].a]), a[i].i);
		}

		debug(pern);

		for (auto && [x, y] : pern) {
			dydukai[y] = un.getSize(x);
			debug(x, un.getSize(x));
		}

		apl = vector<bool>(timer, false);
		...
		for (auto && i : y) {
			un.merge(a[i].a, a[i].b);
		}
	}

	cout << kie << " " << kur.size() << "\n";
	sort(kur.begin(), kur.end());
