...
int main() {
	...
	vpi a(n), b(m); re(a,b);
	F0R(i,MX) cost[i] = MOD;
	trav(t,b) ckmin(cost[t.f],t.s);
	F0R(i,MX) if (cost[i] != MOD) L.upd(i,i,INF-cost[i]);
	vector<array<int,3>> v(p); re(v);
	sort(all(a)); sort(all(v));
	int ind = 0; ll ans = -INF;
	trav(t,a) {
		while (ind < sz(v) && t.f > v[ind][0]) {
			L.upd(v[ind][1]+1,MX-1,v[ind][2]);
			ind ++;
		}
		ckmax(ans,L.mx[1]-t.s);
	}
	ps(ans);
