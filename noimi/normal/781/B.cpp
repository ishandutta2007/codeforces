...
int main() {
    ...
    rep(i, n) {
        STR(a, b);
        s[i][0] = a.substr(0, 3), s[i][1] = a.substr(0, 2) + b.substr(0, 1);
    }
    TwoSat two(n);
    rep(i, n) {
        rep2(j, 0, n - 1) {
            ...
            if(s[i][0] == s[j][0]) {
                two.IF(i, two.rev(j));
                two.IF(two.rev(i), two.rev(j));
            }
            if(s[i][0] == s[j][1]) { two.IF(i, j); }
            if(s[i][1] == s[j][0]) { two.IF(i + n, j + n); }
            if(s[i][1] == s[j][1]) two.IF(i + n, j);
        }
    }
    auto res = two.solve();
    if(res.empty()) {
