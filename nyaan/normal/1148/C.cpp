...
int main(){
  ...
  rep(i,N){
    ...
    if(visited[i] == 0){
      ...
      while(cur != dst){
        if(dst - cur >= N / 2){
          ans[ans_idx++] = mp(cur + 1, dst + 1);
        }
        else if(dst <= N/2 - 1 && cur <= N/2 - 1){
          ans[ans_idx++] = mp(cur + 1, N);
          ans[ans_idx++] = mp(N, dst + 1);
          ans[ans_idx++] = mp(cur + 1, N);
        }
        else if(dst >= N/2 && cur >= N/2){
          ans[ans_idx++] = mp(cur + 1 ,1);
          ans[ans_idx++] = mp(1, dst + 1);
          ans[ans_idx++] = mp(cur + 1, 1);
        }
        else{
          int a = cur + 1, b = dst + 1;
