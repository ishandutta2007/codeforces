...
void dfs(int v, int p)
{
    for (auto u : graph[v])
    {
        ...
        {
            dfs(u.fi, v);
            ll len = dp1[u.fi].fi + u.se;
            if (len > dp1[v].first)
            {
                dp2[v] = dp1[v];
                dp1[v] = {len, u.fi};
            }
            else if (len > dp2[v].first)
            {
                dp2[v] = {len, u.fi};
            }
        }
...
void dfs2(int v, int p, ll len)
{
    if (p != v)
    {
        dpup[v] = dpup[p] + len;
        if (dp1[p].second != v) dpup[v] = max(dpup[v], dp1[p].first + len);
        if (dp2[p].second != v) dpup[v] = max(dpup[v], dp2[p].first + len);
    }
    for (auto u : graph[v])
    {
        if (u.fi != p)
...
signed main()
{
    ...
    sort(rall(v));
    dfs3(v.back().second, v.back().second);
    int q;
    cin >> q;
    while (q--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            //cout << v[i].first << " " << v[i].second + 1 << " " << L[v[i].second] << " " << R[v[i].second] << "\n";
            add(L[v[i].second], 1);
            //cout << j << "\n";
            while (j < i && v[j].first > v[i].first + l)
                add(L[v[j].second], -1), j++;
            //cout << j << "\n";
            ans = max(ans, get(R[v[i].second]) - get(L[v[i].second] - 1));
        }
        cout << ans << "\n";
    }
}
