...
//        }
    ...
    val order = IntArray(basic_setup)
...
fun main(){
    solve.cases{
        ...
        G.setup()
        G.calQuickPA()
        val colours = Array(k+1){ mutableListOf<Int>() }
        for(i in 0 until n){
            if(C[i] != 0){
                colours[C[i]].add(i)
            }
        ...
        fun pathset(v:Int,c:Int,ans:Int){
            var v = v
            repeat(G.depth[v] - G.depth[ans]){
                v = G.parent[v]
                setcolour(v,c)
            }
        }
        out@for(col in 1..k){
            val here = colours[col]
            if(here.size <= 1){
                continue
            }
            here.sortBy { G.order[it] }
            for(i in 0 until here.size - 1 ){
                val l = here[i]
                val r = here[i+1]
                val lca = G.LCA(l,r)
                pathset(l,col,lca)
...
//                v dei here
            ...
            G.NS(v){ w->
                if(w == p) return@NS
                val next = dfs(w,v)
                here = intArrayOf((here[0] modM next[0]) modPlus (next[1] modM here[0]),(here[1] modM next[0]) modPlus (here[0] modM next[1])modPlus (next[1] modM here[1]))
            }
//           v dei here
            return here
        }
...
size n (base array )
and size here ( current array)
didn't output for 3 times
still depth direction

debug recursive by printing,
didn't think my transition through

don't outputs 0 untili exit
6 3
1 0 3 0 0 2
1 2
2 3
