...
constexpr int MAXA = 80000010;

int n;
int a[MAXN];

int distriSum[MAXA+10];
...
int main(){
	...
	int ans = 0;
	For0(bit,25){
		const int maxA = (1<<bit+2)+10;
		#define F(x) ((x)&((1<<bit+1)-1))
		For(i,n) ++distriSum[F(a[i])];
		For(i,maxA+10) distriSum[i] += distriSum[i-1];

		int tans = [&](){
			auto Sol = [&]( int sumL , int sumR ){
				ll cnt = 0;
				For(i,n){
					const int uf = F(a[i]);
					int tL = max( 0 , sumL - uf );
					int tR = max( 0 , sumR - uf );
					int tcnt = distriSum[tR] - (tL?distriSum[tL-1]:0);
					if( uf*2 >= sumL and uf*2 <= sumR ) --tcnt;
					cnt += tcnt;
				}
				assert(cnt&1^1);
				cnt /= 2;
			...
			// const int sumL = 1<<bit, sumR = (1<<bit+1)-1;
			ll cnt = 0;
			cnt += Sol(1<<bit,(1<<bit+1)-1);
			const int b = 1<<bit+1;
			cnt += Sol(b+(1<<bit),b+(1<<bit+1)-1);

			return (cnt&1) ? (1<<bit) : 0;
		}();
		ans += tans;

		Forx(i,0,maxA+10) distriSum[i] = 0;
	}
