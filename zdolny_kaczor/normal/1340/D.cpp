...
bool is_edge(int x, int y) {
	for (int v : graf[x]) if (v == y) return true;
	return false;
}
void dfs(int x, int t, int y = -1) {
	assert((t == 0) == (x == 1));
	debug(imie(x), imie(t));
	...
	for (int v : graf[x]) if (v != y) {
		if (curr == bou) {
			curr -= graf[x].size();
			ans.emplace_back(x, curr);
			debug(imie(x), imie(curr));
		}
		dfs(v, curr + 1, x);
		debug(imie(x), imie(curr + 1));
		ans.emplace_back(x, curr + 1);
		curr++;
	}
	if (x != 1 && t - 1 < curr) {
		ans.emplace_back(x, t - 1);
		debug(imie(x), imie(t - 1));
	}
}
int main() {
	int n;
	...
	bou = 0;
	REP(i, n + 1) maxi(bou, (int) graf[i].size());
	debug(imie(bou));
	dfs(1, 0);
	debug(imie(ans));
#if defined (LOCAL) || defined(LOCAL2)
	assert(ans.size() == set <pii>(ALL(ans)).size());
	for (auto [_, v] : ans) assert(v <= bou);
	assert(ans[0] == pii(1, 0));
	assert(ans.back().first == 1);
	REP(i, ans.size() - 1) {
		auto [v0, t0] = ans[i];
		auto [v1, t1] = ans[i + 1];
		bool good = ((v0 == v1 && t0 > t1) || (t0 + 1 == t1 && is_edge(v0, v1)));
		if (!good) {
			debug(arr(ans, i), arr(ans, i + 1));
			abort();
		}
	}
#endif
