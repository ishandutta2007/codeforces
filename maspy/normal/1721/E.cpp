...
#line 3 "/home/maspy/compro/library/string/rollinghash.hpp"
...
void solve() {
  ...
  FOR(i, 1, N) {
    if (RH.query(SH, i, N) != RH.query(SH, 0, N - i)) continue;
    ll base = N - i;
    FOR(s, 1, 11) {
      if (N - i + s > N) break;
      ll x = RH.query(SH, N - i, N - i + s);
      chmax(MP[x], base + s);
    }
  }
  ...
  FOR(Q) {
    ...
    vi ANS(n);
    FOR(i, n) {
      ll x = RH.query(TH, 0, i + 1);
      if (MP.count(x)) chmax(ANS[i], MP[x]);
    }

    FOR(k, 1, n + 1) {
      // t 
      FOR(i, 1, k + 1) {
        ll x = RH.query(TH, k - i, k);
        if (i <= N && x == RH.query(SH, 0, i)) chmax(ANS[k - 1], i);
      }
      // t 
      FOR(i, k) {
        if (k - i >= N) continue;
        ll a = RH.combine(RH.query(SH, 0, N), RH.query(TH, 0, i), i);
        ll b = RH.combine(RH.query(SH, k - i, N), RH.query(TH, 0, k), k);
        if (a == b) chmax(ANS[k - 1], N + i);
      }
    }
    print(ANS);
  }
