...
signed main(){
    ...
    rep(j,n+2){
        ...
        if(used[i-1] and used[i+1]){
            auto it = --s.lower_bound(pii(i,0));
            auto it2 = s.lower_bound(pii(i,0));
            int l = it->first;
            int r = it2->second;
            it = s.erase(it);
            s.erase(it);
            d.erase(d.lower_bound(i-l));
            d.erase(d.lower_bound(r-i));
            d.emplace(r-l+1);
            s.emplace(l,r);
        }
        else if(used[i-1]){
        ...
        used[i] = true;
        if(a[id[j+1]] > a[id[j]]  and *d.begin() == *(--d.end())){
            if(cnt < d.size()){
                cnt = d.size();
                ans = a[i]+1;
            }
