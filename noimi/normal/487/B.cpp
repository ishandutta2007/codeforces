...
pll e() { return pll(-1e15, 1e15); }
pll f(pll x, pll y) { return pll(max(x.fi, y.fi), min(x.se, y.se)); }
...
int main() {
    LL(n, s, l);
    atcoder::segtree<pll, f, e> seg(n );
    rep(i, n) {
        LL(x);
        seg.set(i, pll(x, x));
    }
    DualSegmentTree<int> dp(
        n + 1, [](int x, int y) { return min(x, y); }, inf<int>);
    dp.update(0, 1, 0);
    rep(i, n) {
        if(i + l > n) continue;
        int r = seg.max_right(i, [&](pll p) { return p.fi - p.se <= s; });
        dp.update(i + l, min<int>(r, n) + 1, dp[i] + 1);
    }
    int ans = dp[n];
