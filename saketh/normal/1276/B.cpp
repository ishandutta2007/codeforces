...
void solve() {
    int N, M, A, B; re(N, M, A, B); --A, --B;

    vpii edges(M); re(edges);
    trav (e, edges) --e.f, --e.s;

    union_find uA(N), uB(N);

    trav (e, edges) {
        if (e.f != A && e.s != A) uA.unio(e.f, e.s);
        if (e.f != B && e.s != B) uB.unio(e.f, e.s);
    }

    ll ans = 0;

    // first find total pairs.
    F0R (u, N) if (u != A && u != B) {
        ans += u - int(u > A) - int(u > B);
    }

    // subtract pairs that match in B
    vi ctB(N);
    F0R (u, N) if (u != A && u != B) {
        ans -= ctB[uB.rep(u)]++;
    }

    // subtract pairs that match in A
    vi ctA(N);
    F0R (u, N) if (u != A && u != B) {
        ans -= ctA[uA.rep(u)]++;
    }

    // add back pairs that match in both
    map<pii, int> ct;
    F0R (u, N) if (u != A && u != B) {
        pii key { uA.rep(u), uB.rep(u) };
        ans += ct[key]++;
    }
