...
//    n = 1e4;
    vector<int> a(n);
     cin >> a;
//    mt19937 rng(228);
//    iota(all(a),100000-100);
//    sort(rall(a));
    Mint ans = 0;
    reverse(all(a));
    vector <pair<int,Mint>> now;
//     now[a[0]]++;
    now.pb({a[0],1});
    for (int _ = 1; _ < n; _++) {
        int x = a[_];
        vector <pair<int,Mint>> new_now;
        Mint total = n - _;
        Mint totalll = _;
        for (auto to: now) {
            if (to.fi >= x) {
...
//                break;
            ...
            totalll -= to.se;
            int k = (x + to.fi - 1) / to.fi;
            ans += total * to.se * (k - 1);
            int new_y = x / k;
            new_now.pb({new_y,to.se});
        }
        new_now.pb({x, 1 + totalll});
        sort(all(new_now));
        now.clear();
        int i=0;
        while (i<new_now.size())
        {
            int j=i;
            Mint sum=0;
            while (j<new_now.size()&&new_now[i].fi==new_now[j].fi)
            {
                sum+=new_now[j++].se;
            }
            now.pb({new_now[i].fi,sum});
            i=j;
        }
    }
    //679243814
