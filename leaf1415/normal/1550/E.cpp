...
bool check(ll x)
{
	...
	rep(j, 0, k-1) succ[j][L] = n+1;
	per(i, L-1, 0){
		rep(j, 0, k-1) succ[j][i] = succ[j][i+1];
		if(a[i+1] == -1) rep(j, 0, k-1) succ[j][i] = i+1;
		else if(a[i+1] != -2) succ[a[i+1]][i] = i+1;
	}
	
	ll K = 1 << k;
	rep(i, 0, K-1) dp[i] = n+1;
	dp[0] = 0;
	...
	rep(i, 0, K-1){
		...
		rep(j, 0, k-1){
			if(i & (1<<j)) continue;
			chmin(dp[i|(1<<j)], succ[j][dp[i]]+x-1);
		}
	}
	return dp[K-1] <= n;
}
