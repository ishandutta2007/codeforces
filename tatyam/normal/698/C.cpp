...
int main(){
    ...
    chmin(k,cnt);
    vec(ld,q,1<<n);
    rep(n)rep(bit,1<<n)if(~bit>>i&1)q[bit]+=p[i];
    vec(ld,dp,1<<n);
    dp[0]=1;
    rep(bit,1<<n)rep(n)if(~bit>>i&1&&q[bit])dp[bit|1<<i]+=dp[bit]*p[i]/q[bit];
    vec(ld,ans,n);
    rep(bit,1<<n)if(popcnt(bit)==k)rep(n)if(bit>>i&1)ans[i]+=dp[bit];
    out(ans);
}