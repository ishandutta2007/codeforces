...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	cauto n=read();
	cauto m=read();
	vector<tuple<int,ll,int>> tps(n);
	for(auto &[a,b,c]:tps) cin>>a>>b>>c;
	vector<int> re(m+1,MOD); re[0]=0;
	rep(i,n){
		cauto [t,x,ub]=tps[i];
		if(t==1){
			cauto ad=(x+99999)/100000;
			vector<ll> prv(ad,-1e16);
			rep(j,re.size()){
				if(re[j]<=i) prv[j%ad]=j;
				else if((j-prv[j%ad])/ad<=ub) re[j]=i+1;
			}
		}else{
			vector<ll> dp(m+1);
			rep(j,dp.size()){
				...
				if(dp[j]){
					MN(re[j],i+1);
					if(cauto t=(j*x+99999)/100000;t<=dp.size()) MX(dp[t],dp[j]-1);
				}
			}
		}
	}
	reps(i,1,m+1) cout<<(re[i]==MOD?-1:re[i])<<" ";
	cout NL;
  return 0;
}