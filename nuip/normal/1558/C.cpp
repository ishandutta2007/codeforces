...
int main(){
	...
	TT{
		...
		// reps(i,2,n) swap(vs[rand()%(i/2+1)*2+i%2],vs[i]);
		
		{
			int ok=1;
			rep(i,n)if((i+vs[i])%2) ok=0;
			if(!ok){
				cout<<-1 NL;
				continue;
			}
		...
		rrep(v,n)if(v%2){
			...
			cauto b=v+1;
			out(a,b,1);
			auto ai=find(all(vs),a)-vs.begin();
			auto bi=find(all(vs),b)-vs.begin();
			if(abs(ai-bi)>1){
				rev(bi+1);
				ai=find(all(vs),a)-vs.begin();
				rev(ai);
				bi=ai-1;
			}
			if(bi+1==ai){
