...
void solve() {
  ...
  // [i, infty) 
  for (auto&& p: P) B[N + p]++;
  FOR_R(i, len(B) - 1) B[i] += B[i + 1];
  // print("A", A);
  // print("B", B);
  vc<mint> f(LIM), g(LIM);
  vc<mint> f0(LIM), g0(LIM);
  vc<mint> POW2 = powertable_1<mint>(2, N + 10);
  FOR(i, LIM) f0[i] = 1;
  FOR(i, LIM) g0[i] = 1;
  FOR(i, LIM) f[i] = POW2[A[i]];
  FOR(i, LIM) g[i] = POW2[B[i]];
  auto h = convolution_leq(f, g, 1);
  auto h0 = convolution_leq(f0, g0, 1);
  mint ANS = 0;
  FOR(i, N) {
    mint coef = h[N + N + i + i + 1] - h0[N + N + i + i + 1];
    ANS += coef * C[i];
  }
  mint den = mint(2).pow(M) - mint(1);
  print(ANS / den);
}
