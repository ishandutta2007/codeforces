...
#line 1 "/home/maspy/compro/library/graph/restore_path.hpp"
...
#line 3 "/home/maspy/compro/library/graph/find_nontransitive_triple.hpp"
...
void solve() {
  ...
  [&]() {
    ...
    auto P = restore_path(par, N - 1);
    if (len(ANS) == 0 || len(ANS) > len(P)) ANS = P;
  }();
  vc<int> nbd(N);
  for (auto&& e: G[0]) nbd[e.to] = 1;
  // 0 -> a -> b -> 0 -> goal
  [&]() {
    if (len(ANS) >= 1 && len(ANS) < 5) return;
    FOR(a, N) if (nbd[a]) {
      for (auto&& e: G[a]) {
        if (e.to == 0 || nbd[e.to]) continue;
        ANS = {0, a, e.to, 0, N - 1};
        return;
      }
    }
  ...
  // 0 -> a -> b -> c -> a -> goal
  [&]() {
    if (len(ANS) >= 1 && len(ANS) < 6) return;
    Graph H(N);
    for (auto&& e: G.edges) {
      if (nbd[e.frm] && nbd[e.to]) H.add(e.frm, e.to);
    }
    H.build();
    auto [a, b, c] = find_nontransitive_triple(H);
    if (a == -1) return;
    ANS = {0, a, b, c, a, N - 1};
  }();

  if (len(ANS) == 0) return print(-1);
  for (auto&& v: ANS) ++v;
  print(len(ANS) - 1);
  print(ANS);
}
