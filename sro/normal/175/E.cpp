...
int main(){
	...
	loop(i,Mask(n)){
		...
		loop(j,n){
			if(!Bit(i,j)){
				int posf=j/2;
				double damf=rf*df*2,dame=re*de*2;
				loop(k,n){
					if(Bit(i,k)){
						int poss=k/2;
						damf+=max(.0,min(poss+rs,posf+rf)-max(poss-rs,posf-rf))*df;
						dame+=max(.0,min(poss+rs,posf+re)-max(poss-rs,posf-re))*de;
					}
				}
				res+=damf;
				cha.pub(damf-dame);
