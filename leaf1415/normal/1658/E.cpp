...
int main(void)
{
	...
	rep(y, 1, n) rep(x, 1, n){
		cin >> a[x][y];
		vec.pb(P(a[x][y], y*10000+x));
	}
	sort(rall(vec));
	
	ll lu = -inf, ru = inf, lv = -inf, rv = inf;
	for(auto p : vec){
		ll x = p.se % 10000, y = p.se / 10000;
		ll u = x + y, v = x - y;
		if(u < lu || u > ru || v < lv || v > rv) continue;
		chmax(lu, u - k), chmin(ru, u + k);
		chmax(lv, v - k), chmin(rv, v + k);
	}
	
	rep(y, 1, n){
		rep(x, 1, n){
			ll u = x + y, v = x - y;
			if(lu <= u && u <= ru && lv <= v && v <= rv) cout << "M";
			else cout << "G";
		}
		cout << endl;
	}
