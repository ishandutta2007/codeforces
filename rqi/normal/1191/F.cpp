...
vector<pi> polls;
Tree<pi> t; // x val, number of things with that x val
ll ans;
int main() {
    setIO();
    ll n;
    ...
    for(ll i = 0; i < n; i++){
    	...
    	cin >> x >> y;
    	polls.pb(mp(y, x));
    	if(t.lower_bound(mp(x, 0)) == t.end()){
    		t.insert(mp(x, 1));
    	}
    	else if(t.lower_bound(mp(x, 0))->f == x){
    		ll count = t.lower_bound(mp(x, 0))->s;
    		t.erase(*t.lower_bound(mp(x, 0)));
    		t.insert(mp(x, count+1));
    	}
    	else t.insert(mp(x, 1));
    }
    sort(polls.begin(), polls.end());
    ll leftbound = 0; // first element to the left that you cannot include
    for(ll i = 0; i < n; i++){
    	if(i > 0){
    		if(polls[i-1].f != polls[i].f){
    			leftbound = 0;
    		}
    	}
    	ll y = polls[i].f;
    	ll x = polls[i].s;
    	ans+=((ll)t.order_of_key(mp(x, 0))+1-(ll)t.order_of_key(mp(leftbound+1, 0)))*(1+(ll)t.size()-(ll)t.order_of_key(mp(x+1, 0)));
    	if(t.lower_bound(mp(x, 0))->s == 1){
    		t.erase(t.lower_bound(mp(x, 0)));
    	}
    	else{
    		ll count = t.lower_bound(mp(x, 0))->s;
