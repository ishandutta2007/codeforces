...
namespace Manacher{
	...
	void Manacher(){
		...
		For(i,n){
			...
			while( i-p[i] >= 0 and i+p[i] <= n and a[i-p[i]] == a[i+p[i]] )
				++p[i];
			if( i+p[i]-1 > mxright ){
				mxpos = i;
				mxright = i+p[i]-1;
...
string Solve( const string &s ){
	...
	// O(fuck);
	Manacher::Process(fuck);
	tuple<int,int,int> ans = {-1,-1,-1};
	Forx(i,0,Manacher::n){
		if( Manacher::p[i] + i != Manacher::n + 1 ) continue;
		int tans = Manacher::p[i];
		int tl = i - tans + 1;
		int tr = i + tans - 1;
		ans = max( ans , {tans,tl,tr} );
	}

	string shit = "";
	Forx(i,0,a-1) shit += s[i];
	Forx(i,get<1>(ans),get<2>(ans))
		if( Manacher::a[i] != '#' )
			shit += Manacher::a[i];
	Forx(i,n-a,n-1) shit += s[i];
...
int main(){
	...
	For(TwT,T){
		...
		if( ans1.length() > ans2.length() ){
			for( auto c : ans1 ) putchar(c);
		}else{
			for( auto c : ans2 ) putchar(c);
		}
		putchar(10);
	}
