...
void solve() {
    ...
    F0R (i, 3) {
        ...
        sort(all(bags[i]));
        total += accumulate(all(bags[i]), int64_t(0), plus<int64_t>());
    }

    int64_t ans = LLONG_MIN;

    F0R (i, 3) {
        ckmax(ans, total - 2 * accumulate(all(bags[i]), int64_t(0), plus<int64_t>()));

        FOR (j, i + 1, 3) {
            ckmax(ans, total - 2 * int64_t(bags[i][0] + bags[j][0]));
        }
    }
