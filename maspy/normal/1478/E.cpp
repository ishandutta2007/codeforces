...
void solve() {
  ...
  STR(S, T);
  vc<pi> seg_raw(N);
  FOR(i, N) seg_raw[i] = mp(1, T[i] - '0');
  LazySegTree<Lazy_CntSum_Set<ll, -1>> seg(seg_raw);
  ...
  for (auto&& [L, R]: query) {
    ...
    ll one = sum, zero = R - L - sum;
    if (one == zero) return NO();
    if (one < zero) seg.apply(L, R, 0);
    if (one > zero) seg.apply(L, R, 1);
  }
  auto A = seg.get_all();
  FOR(i, N) {
    ll s = S[i] - '0';
