...
int main() {
    ...
    auto possible = [&](int distance) -> bool {
        ll total = 1;
        vll diffs(distance + 1);
        diffs[0] = +1, diffs[1] = -1;

        auto add_range = [&](int l, int r, ll delta) {
            if (l > distance)
                return;
            diffs[l] += delta;
            if (r <= distance)
                diffs[r] -= delta;
            total += delta * (min(r, distance + 1) - l);
        };
        ...
        for (int v : l) {
            while (i < distance && count == 0)
                adv();

            count--;
            add_range(i, i + 1, -1);
            for (int u : {v / 2, (v - 1) / 2}) {
                add_range(i + 2, i + 2 + u, +1);
            }
    ...
    int ans = binarysearch<int>(possible, 1, MAX);
