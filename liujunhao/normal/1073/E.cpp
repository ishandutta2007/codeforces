...
int pow10[22];
int a[22];
pii dfs(int i,int s,int flag,int flag2){
	if(!flag&&!flag2&&~f[i][s].first)
		return f[i][s];
	if(!i){
		if(!s)
			return {0,1};
	...
	if(i==1)
		bg=0;
	for(int j=bg;j<=(flag?a[i]:9);j++){
		if(s&(1<<j)){
			pii now=dfs(i-1,s,j==a[i]&&flag,0),now2=dfs(i-1,s^(1<<j),j==a[i]&&flag,0);
			now.first=(now.first+now2.first)%MOD;
			now.second=(now.second+now2.second)%MOD;
			ret.second=(ret.second+now.second)%MOD;
			ret.first=(ret.first+now.first+1ll*pow10[i-1]*j%MOD*now.second)%MOD;
		}
	}
	if(bg==1)
		ret.first=(ret.first+dfs(i-1,s,0,flag2).first)%MOD;
	if(!flag&&!flag2)
		f[i][s]=ret;
	return ret;
}
int solve(LL n,int k){
	...
	for(int s=1;s<1024;s++)
		if(__builtin_popcount(s)<=k)
			ret=(ret+dfs(len,s,1,1).first)%MOD;
	return ret;
}
LL l,r,k;
LL ans;
int main()
{
	...
	int ans=(solve(r,k)-solve(l-1,k)+MOD)%MOD;
	printf("%d\n",ans);
}