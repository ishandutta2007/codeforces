...
int main() {
    ...
    vv(ll, an, 3, 3);
    rep(i, 3) rep2(j, i + 1, 2) an[i][j] = an[j][i] = AND(i, j);
    vv(ll, o, 3, 3);
    rep(i, 3) rep2(j, i + 1, 2) o[i][j] = o[j][i] = OR(i, j);
    ll A = an[0][1] & an[1][2] & an[2][0];
    ll O = o[0][1] | o[1][2] | o[2][0];
    vll res(n);
    rep(i, 3) res[i] = A;
    rep(i, 31) {
        if(~A & 1 << i and O & 1 << i) {
            ...
            rep(j, 3) {
                if(~o[j][(j + 1) % 3] & 1 << i) { res[(j + 2) % 3] |= 1 << i; }
            }
        }
    }
    rep2(i, 3, n - 1) {
