...
#define int unsigned int
	...
	const int THRES = 20000;
	const int BLOCK = 100000;
	vector<int> p;
	{
		vector<bool> isp(THRES + 1, true);
		for (int i = 2; i <= THRES; i++) {
			if (isp[i]) {
				ans += calc(i) * cnt(i);
				if (i <= THRES)
					p.emplace_back(i);
				for (int j = i; j <= THRES; j += i)
					isp[j] = false;
			}
		}
	}
	
	vector<bool> isp(BLOCK, true);
	for (int l = THRES + 1, r = THRES + BLOCK; l <= n; l += BLOCK, r += BLOCK) {
