...
int main(){
  ...
  vi sz(n);
  for(li i = 0; i < (li)(n); ++i) sz[i] = G[i].size();

  vector<array<pii, 2>> at0(n, {make_tuple(1,-1), make_tuple(1,-1)});
  vector<array<pii, 2>> at1(n, {make_tuple(0,-1), make_tuple(0,-1)});
  function<tuple<li,li>(li,li)> dfs = [&](li i, li p) -> tuple<li,li> {
    vi NG;
    for(li j : G[i]) if(j != p) {
        li a0, a1; tie(a0,a1) = dfs(j,i);
        li b0 = a1 + 1;
        li b1 = max(a0,a1) + (sz[i]-2);
        if(b0 > get<0>(at0[i][0])) { at0[i][1] = at0[i][0]; at0[i][0] = make_tuple(b0,j);}
        else if(b0 > get<0>(at0[i][1])) { at0[i][1] = make_tuple(b0,j); }
        if(b1 > get<0>(at1[i][0])) { at1[i][1] = at1[i][0]; at1[i][0] = make_tuple(b1,j);}
        else if(b1 > get<0>(at1[i][1])) { at1[i][1] = make_tuple(b1,j); }
      } else {
        NG.push_back(j);
      }
    G[i] = NG;
    li x0 = get<0>(at0[i][p == get<1>(at0[i][0])]);
    li x1 = get<0>(at1[i][p == get<1>(at1[i][0])]);
    return make_tuple(x0,x1);
  };

  li ans = 0;
  for(li i = 0; i < (li)(n); ++i) ans = max(ans, get<0>(dfs(i,-2)));
  cout << ans << endl;
