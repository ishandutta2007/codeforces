...
int main() {
	...
	vector<vector<ll>> dp(n + 1, vector<ll>(4, 1e15));
	vector<vector<bool>> fix(n + 1, vector<bool>(4, false));
	minheap<pair<ll, pii>> pq;
	dp[1][0] = 0;
	pq.push({0, {1, 0}});

	const static vector<int> mul = { 1, 0, 2, 1 };
	while (!pq.empty()) {
		int u = pq.top().se.fi;
		int mask = pq.top().se.se;
		pq.pop();

		if (fix[u][mask]) continue;
		fix[u][mask] = true;
		...
		for (auto& e : g[u]) {
			...
			int w = e.se;
			for (int b = 0; b < 4; b++)
				if ((mask & b) == 0 && chkmin(dp[v][mask | b], dp[u][mask] + w * mul[b]))
					pq.push({dp[v][mask | b], {v, mask | b}});
		}
	}
