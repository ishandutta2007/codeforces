...
// NOTE: Read numbers with reversed bits
...
int main(){
	...
	binary_trie<int, mx> trie;
	using ptr = shared_ptr<binary_trie<int, mx>::node>;
	auto root = trie.build(a);
	for(auto qi = 0; qi < qn; ++ qi){
		int type, l, r;
		cin >> type >> l >> r, ++ r;
		ptr a, b, c;
		tie(a, b) = trie.split(root, l);
		tie(b, c) = trie.split(b, r);
		if(type == 1){
			int x;
			cin >> x;
			b = trie.update_bitwise_and(b, x);
		}
		else if(type == 2){
			int x;
			cin >> x;
			b = trie.update_bitwise_or(b, x);
		}
		else if(type == 3){
			int x;
			cin >> x;
			b = trie.update_bitwise_xor(b, x);
		}
		else{
			cout << b->count << "\n";
		}
		root = trie.merge(trie.merge(a, b), c);
	}
	return 0;
}
