...
int main(void)
{
	...
	rep(i, 1, n){
		if(a[i] == 1) cnt1++;
		else if(a[i]%2) ovec.push_back(a[i]);
		else evec.push_back(a[i]);
	}
	
	vector<ll> ans, ans2, ans3;
	rep(i, 1, cnt1) ans.push_back(1);
	for(auto x : evec){
		if(prime[x+1] == x+1){
			ans.push_back(x);
			break;
	...
	for(auto x : evec) for(auto y : ovec){
		if(prime[x+y] == x+y){
			ans2.push_back(x), ans2.push_back(y);
			goto end;
		}
	}
	end:;
	...
	if(sz(ans) < sz(ans3)) swap(ans, ans3);
	outl(sz(ans));
	outl(ans);
