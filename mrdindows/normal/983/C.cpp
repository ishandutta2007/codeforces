...
int a[5005];
int b[5050];
int dp[2002][10][10][10][10];
int n;
int go(int cur, int x, int y, int z, int w) {
    if (x > y) swap(x, y);
    if (y > z) swap(y, z);
    if (x > y) swap(x, y);
    if (cur == n && z == 0) return 0;
    auto& res = dp[cur][x][y][z][w];
    if (res) return res - 1;
    res = INF;
    int ans = INF;
    if (x) ans = min(ans, go(cur, 0, y, z, x) + abs(x - w));
    if (y) ans = min(ans, go(cur, 0, x, z, y) + abs(y - w));
    if (z) ans = min(ans, go(cur, 0, x, y, z) + abs(z - w));
    if (cur != n && a[cur] != w) {
        ans = min(ans, go(cur, x, y, z, a[cur]) + abs(a[cur] - w));
    }
    if (cur != n && a[cur] == w && !x) {
        ans = min(ans, go(cur + 1, b[cur], y, z, w));
    }
    if (cur != n && a[cur] == w && x) {
        ans = min(ans, go(cur + 1, x, y, z, b[cur]) + abs(b[cur] - w));
        ans = min(ans, go(cur + 1, b[cur], y, z, x) + abs(x - w));
        ans = min(ans, go(cur + 1, b[cur], x, z, y) + abs(y - w));
        ans = min(ans, go(cur + 1, b[cur], x, y, z) + abs(z - w));
    }
    res = ans + 1;
    return ans;
}
