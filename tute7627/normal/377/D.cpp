...
namespace add_max{
  ...
  LazySegmentTree<P,M,decltype(f),decltype(g),decltype(h)>make(int n){
    return {n,f,g,h,MP(-INF,-INF),0};
  }
}
...
int main(){
  ...
  bool judge = true;
  ll n;cin>>n;
  vector<ll>l(n),v(n),r(n);
  ll sz=300005;
  vector<vector<ll>>gl(sz),gv(sz);
  rep(i,0,n){
    cin>>l[i]>>v[i]>>r[i];
    gl[l[i]].PB(i);
    gv[v[i]].PB(i);
  ...
  rep(i,0,sz)seg.set(i,MP(0,i));
  seg.build();
  ll nl=0,nr=0;
  rep(i,0,sz){
    for(auto z:gl[i]){
      seg.update(v[z],r[z]+1,1);
    }
    auto tmp=seg.query(i,sz);
    if(chmax(res,tmp.fi)){
      nl=i,nr=tmp.se;
      //cout<<nl spa nr spa tmp.fi<<endl;
    }
    for(auto z:gv[i]){
      seg.update(v[z],r[z]+1,-1);
    }
  ...
  vector<ll>ret;
  rep(i,0,n){
    if(nl>=l[i]&&nr<=r[i]&&nl<=v[i]&&nr>=v[i])ret.PB(i+1);
  }
  cout<<ret.size()<<endl;
  debug(ret,ret.size());
  return 0;
