...
void up(){
	int const numValue=read<int>();
	int const multiple=read<int>();

	std::map<int, std::map<int, int>> exponents; // base value -> (exponent value -> count)
	for(int _=numValue; _--;){
		int value=read<int>();
		int exponent=0;
		...
		exponents[value][exponent]+=1;
	}

	int result {};
	for(auto& [_, item]: exponents){
		while(not item.empty()){
			auto [exponent, count]=*item.begin(); item.erase(item.begin());
			if(not item.empty() and item.begin()->first==exponent+1){
				auto& [_, other_count]=*item.begin();
				auto d=std::min(count, other_count);
				count-=d; other_count-=d;
			}
			result+=count; // cannot be paired
		}
	}
	std::cout<<result<<'\n';
