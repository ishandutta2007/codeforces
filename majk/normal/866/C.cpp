...
public:
    void solve(istream& cin, ostream& cout) {
        ...
        vector2<double> EXP(N+1, R+1);
        double ans = bsh(0, 1e9, [&](double x) {
            fill(EXP[N].begin(),EXP[N].end(),0);

            for (int j = N-1; j >= 0; --j) {
                for (int i = 0; i <= R; ++i) {
                    double good = i+F[j] <= R ? EXP[j+1][i+F[j]] : x;
                    double bad = i+S[j] <= R ? EXP[j+1][i+S[j]] : x;
                    double play = S[j] + bad + (F[j] - S[j] + good - bad) * P[j];
                    EXP[j][i] = min(play, x);
                }
            }
