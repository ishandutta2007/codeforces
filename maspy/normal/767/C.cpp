...
void solve() {
  ...
  ll v1 = [&]() -> ll {
    ...
    ll sum_A = SUM(A);
    FOR_R(i, N) if (DP[V[i]] * 3 == sum_A) return V[i];
    return -1;
  }();
  if (v1 == -1) return print(-1);
  ll v2 = [&]() -> ll {
    vi DP(N);
    ll sum_A = SUM(A);
    ll L = hld.LID[v1], R = hld.RID[v1];
    FOR_R(i, N) {
      if (L <= i && i < R) continue;
      ll v = V[i];
      DP[v] += A[v];
      ll p = par[v];
      if (p != -1) DP[p] += DP[v];
      if (DP[v] * 3 == sum_A) return v;
    }
    return -1;
  }();
  if (v2 == -1) return print(-1);
  if (v1 == root || v2 == root) return print(-1);
  print(v1 + 1, v2 + 1);
}
