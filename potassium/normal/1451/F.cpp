...
int main() {
	CASE(t) {
		int n,m; R(n,m);
		vector<vector<int>> a(n, vector<int>(m)); R(a);
		
		vector<vector<int>> diags(n + m - 1);
		for (int diag = 0; diag <= n + m - 2; diag++) {
			for (int r = max(0, diag - m + 1); r <= min(n - 1, diag); r++) {
				int c = diag - r;
				diags[diag].pb(a[r][c]);
			}
		}
		
		vector<int> xsum(n + m - 1, 0);
		for (int diag = 0; diag <= n + m - 2; diag++) {
			for (auto& i : diags[diag])
				xsum[diag] ^= i;
		}
		...
		const string FIRST = "Ashish", SECOND = "Jeel";
		W(lose ? SECOND : FIRST);
	}
