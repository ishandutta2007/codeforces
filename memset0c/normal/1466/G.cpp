...
namespace hash{
  ...
  template<const int mod=998244353,const int bas=131131>
  struct singleHash{
    static const singleHash base={bas};
    int a;
    inline singleHash(){}
...
const int N=1e5+10,M=1e6+10,K=110,mod=1e9+7;
using z=modint::Z<mod>;

const int mod0=998244353,mod1=1000000007,bas0=131131,bas1=131131;
using hash_t=hash::doubleHash<mod0,mod1>;
const hash_t base={131131,131131};

int n,q,l,m,k,u[N],e,sta[N];
char s[K],t[N],a[M<<1],b[M],all[M];
z cnt[N][26],ans[N],pw2[M<<1],ipw2[M<<1];
hash_t ha[M<<1],hb[M],pw[M<<1];

inline hash_t queryA(int l,int r){return ha[r]-ha[l-1]*pw[r-l+1];}
inline hash_t queryB(int l,int r){return hb[r]-hb[l-1]*pw[r-l+1];}
...
int main(){
  ...
  pw[0]=hash_t(1),pw2[0]=ipw2[0]=1;
  for(int i=1;i<(M<<1);i++){
    pw[i]=pw[i-1]*base;
    pw2[i]=pw2[i-1]*2;
    ipw2[i]=ipw2[i-1]*((mod+1)>>1);
  }
  read(n,q),read(s+1),read(t+1);
  for(int i=1;i<=n;i++){
    for(int j=0;j<26;j++)cnt[i][j]=cnt[i-1][j];
    cnt[i][t[i]-'a']+=pw2[n-i];
//    log("t[%d]=%c >> %d\n",i,t[i],pw2[n-i].x);
  }
//  for(int i=1;i<=n;i++)for(int j=0;j<26;j++)cerr<<cnt[i][j].x<<" \n"[j==25];
  for(int i=1;i<=q;i++){
    read(u[i]),read(all+sta[i]);
    sta[i+1]=sta[i]+strlen(all+sta[i]);
  }
  m=l=strlen(s+1);
  for(k=0;m<=1e6;k++){
    if(!k){
      ...
      a[m+1]=t[k];
      for(int i=1;i<=m;i++)a[m+1+i]=a[i];
      m=m<<1|1;
    }
    for(int i=1;i<=m;i++)ha[i]=ha[i-1]*base+hash_t(a[i]);
    for(int p=1;p<=q;p++)
      if(ans[p].x==-1&&u[p]>=k&&m>=sta[p+1]-sta[p]){
//        log("p=%d k=%d m=%d\n",p,k,m);
        e=sta[p+1]-sta[p];
        for(int i=1;i<=e;i++){
          b[i]=all[sta[p]+i-1];
          hb[i]=hb[i-1]*base+hash_t(b[i]); 
        }
        ans[p]=0;
        z apr=fpow(z(2),u[p]-k);
//        for(int j=1;j<=e;j++)log("%c",b[j]);log("\n");
        hash_t hb_all=queryB(1,e);
        for(int i=1;i+e-1<=m;i++)
          if(hb_all==queryA(i,i+e-1)){
            ans[p]+=apr;
//            log("%d << combo at %d\n",ans[p].x,i);
          }
        for(int i=1;i<=e;i++)
          if((i==1||queryB(1,i-1)==queryA(m-i+2,m))&&(i==e||queryB(i+1,e)==queryA(1,e-i))){
            ans[p]+=(cnt[u[p]][b[i]-'a']-cnt[k][b[i]-'a'])*ipw2[n-u[p]];
//            log("%d << concat at %d [%c]\n",ans[p].x,i,b[i]);
          }
      }
  }
//  for(int i=1;i<=q;i++)cerr<<ans[i].x<<" \n"[i==q];
  for(int i=1;i<=q;i++)if(ans[i].x==-1)ans[i]=0;
