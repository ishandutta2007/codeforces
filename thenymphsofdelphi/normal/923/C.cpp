...
void add(int x){
    ...
    FordE(i, K, 0){
        bool kek = x & (1ll << i);
        if (trie[idx].nxt[kek] == -1){
            trie.pb(Node(idx));
            trie[idx].nxt[kek] = trie.size() - 1;
        }
        idx = trie[idx].nxt[kek];
    }
    trie[idx].cnt++;
}
...
void dfs(int u){
    if (trie[u].nxt[0] != -1){
        dfs(trie[u].nxt[0]);
        trie[u].cnt += trie[trie[u].nxt[0]].cnt;
    }
    if (trie[u].nxt[1] != -1){
        dfs(trie[u].nxt[1]);
        trie[u].cnt += trie[trie[u].nxt[1]].cnt;
    }
}
...
signed main(){
    ...
    ForE(i, 1, n){
        ...
        FordE(j, K, 0){
            if (a[i] & (1ll << j)){
                if (trie[u].nxt[1] != -1 && trie[trie[u].nxt[1]].cnt){
                    u = trie[u].nxt[1];
                }
                else{
                    u = trie[u].nxt[0];
                ...
                if (trie[u].nxt[0] != -1 && trie[trie[u].nxt[0]].cnt){
                    u = trie[u].nxt[0];
                }
                else{
                    u = trie[u].nxt[1];
