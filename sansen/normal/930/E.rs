...
fn run() {
    ...
    let mut a = a;
    a.sort_by_key(|p| (p.1, !p.0));
    a.dedup_by(|a, b| a.0 <= b.0 && b.1 <= a.1);
    let mut b = b;
    b.sort_by_key(|p| (p.1, !p.0));
    b.dedup_by(|a, b| a.0 <= b.0 && b.1 <= a.1);
    let mut z = vec![0, w];
    z.extend(a.iter().map(|a| a.0));
    z.extend(a.iter().map(|a| a.1));
    z.extend(b.iter().map(|a| a.0));
    z.extend(b.iter().map(|a| a.1));
    z.sort();
    z.dedup();
    let mut cond = vec![vec![]; z.len() - 1];
    for &(a, b) in a.iter() {
        let l = z.binary_search(&a).unwrap();
        let r = z.binary_search(&b).unwrap();
        cond[l].push((r, 0));
    }
    for &(a, b) in b.iter() {
        let l = z.binary_search(&a).unwrap();
    ...
    let mut seg = vec![];
    for _ in 0..2 {
        seg.push(segment_tree::PURQ::new(z.len() + 1, M::zero(), |a, b| *a + *b));
    }
    let mut dp = vec![M::zero(); z.len() + 1];
    dp[0] = M::one();
    for (l, cond) in cond.iter().enumerate() {
        let sub = seg[0].find(l, l + 1) + seg[1].find(l, l + 1);
        let way = dp[l] - sub;
        for &(r, k) in cond.iter() {
            let sub = seg[k].find(l + 1, r);
            seg[k].update(r, way - sub);
        }
        dp[l + 1] = way * M::new(2).pow((z[l + 1] - z[l]) as u64);
    }
    dp[z.len() - 1] -= seg[0].find(z.len() - 1, z.len()) + seg[1].find(z.len() - 1, z.len());
    println!("{}", dp[z.len() - 1]);
}
