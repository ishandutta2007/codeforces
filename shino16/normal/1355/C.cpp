...
int main() {
	...
	rep(z, c, d+1) {
		...
		dump(m1, m2, rect);
		if (xy <= a+b) res += rect;
		else if (xy <= min(m1, m2)) res += rect - (xy-b-a) * (xy-b-a+1) / 2;
		else if (xy <= m2) res += rect - (xy-b-b + xy-a-b) * (b-a+1) / 2;
		else if (xy <= m1) res += rect - (xy-a-b + xy-a-c) * (c-b+1) / 2;
		else if (xy <= b+c) res += (b+c-xy+1) * (b+c-xy+2) / 2;
		dump(z, res);
	}
