...
fn run() {
    ...
    for i in 0usize..(1 << m) {
        ...
        for (j, &(a, b)) in e.iter().enumerate() {
            if i >> j & 1 == 1 {
                c += set[a] as usize + set[b] as usize;
                set[a] = false;
                set[b] = false;
                for &(a, b) in [p[a], p[b]].iter() {
                    l = l.max(a);
                    r = r.min(b);
                }
            }
        ...
        if l <= r {
            let sign = if i.count_ones() % 2 == 0 {
                M::one()
            } else {
                -M::one()
            };
            event.push((l, c, sign));
            event.push((r + 1, c, -sign));
        }
        for (j, &(a, b)) in e.iter().enumerate() {
            if i >> j & 1 == 1 {
                set[a] = true;
    ...
    let mut dp = vec![M::zero(); 2 * m + 1];
    for i in 1..=n {
        while x < event.len() && event[x].0 == i {
            let (_, c, v) = event[x];
            x += 1;
            dp[c] += v;
        }
        if cnt[i] >= i {
            for (j, dp) in dp.iter().enumerate().take(i + 1) {
                ans += *dp * pc.comb(cnt[i] - j, i - j);
            }
        }
    }
    println!("{}", ans);
