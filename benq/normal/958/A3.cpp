...
pi nor(P x) {
	return {round(x.f),round(x.s)};
}

void test(pair<P,P> x, pair<P,P> y) {
	P rot = (y.s-y.f)/(x.s-x.f); rot /= abs(rot);
	vector<pair<P,int>> ma, mb;
	F0R(i,sz(a)) ma.pb({a[i]-x.f,i});
	F0R(i,sz(b)) mb.pb({(b[i]-y.f)/rot,i});
	sort(all(ma)), sort(all(mb));
	vector<pair<double,pi>> match;
	trav(t,ma) {
		pair<double,int> bes = {5,MOD};
		auto des = t; des.f.f -= bes.f;
		int ind = lb(all(mb),des)-mb.begin();
		while (ind < sz(mb) && mb[ind].f.f <= t.f.f+bes.f) {
			ckmin(bes,{abs(mb[ind].f-t.f),mb[ind].s});
			ind ++;
		}
		if (bes.s != MOD) match.pb({bes.f,{t.s,bes.s}});
	}
	if (sz(match) < N) return;
	sort(all(match)); match.rsz(N);
	trav(t,match) ps(t.s.f+1,t.s.s+1);
	exit(0);
}

double rd() { return ((double)rand())/(1LL<<31); }
P rp() { return P(20000*rd()-10000,20000*rd()-10000); }
...
int main() {
    ...
    re(b);
    /*P t0 = rp(), t1 = rp();
    ld d0 = 2*PI*rd(), d1 = 2*PI*rd();
    cout << fixed << setprecision(8);
    F0R(i,N) {
    	P z = rp();
    	a[i] = (z+t0)*dir(d0); 
    	a[i].f = round(100*a[i].f)/100;
    	a[i].s = round(100*a[i].s)/100;
    	b[i] = (z+t1)*dir(d1); 
    	b[i].f = round(100*b[i].f)/100;
    	b[i].s = round(100*b[i].s)/100;
    	// ps(a[i],b[i]); exit(0);
    }*/
    auto A = convex_hull(a), B = convex_hull(b);
    // ps(sz(A),sz(B));
    vector<pair<P,P>> AA, BB;
    F0R(i,sz(A)) AA.pb({A[i],A[(i+1)%sz(A)]});
    F0R(i,sz(B)) BB.pb({B[i],B[(i+1)%sz(B)]});
    trav(x,AA) trav(y,BB) if (abs(abs(x.f-x.s)-abs(y.f-y.s)) <= EPS) {
    	test(x,y);
    	test({x.s,x.f},y);
    }
    exit(5);
