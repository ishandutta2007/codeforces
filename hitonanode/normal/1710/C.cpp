...
int main() {
    string S;
    cin >> S;
    vector<mint> dp(1 << 6); // dp[S_tiebreak][count_type]
    constexpr int DD = 1 << 3;
    auto f = [&](int i, int j) {
        // assert(i >= 0 and i < DD);
        // assert(j >= 0 and j < DD);
        return i * DD + j;
    ...
    // vector<int> s2mask(DD);
    // REP(s, DD) {
    //     REP(i, 3) {
    //         int j = (i + 1) % 3;
    //         int si = (s >> i) & 1;
    //         int sj = (s >> j) & 1;
    //         if (si != sj) s2mask[s] |= 1 << i;
    //     }
    // }
    // dbg(s2mask);
    vector<int> s2mask = {0, 1, 2, 4, 4, 2, 1, 0};

    const int s_all = DD - 1;

    for (const char c : S) {
        vector<mint> dpnxt(dp.size());

        REP(S_tiebreak, DD) {
            int S_can = (c == '1') ? s_all : S_tiebreak;
            REP(S_add, DD) {
                if (~S_can & S_add) continue;
                int S_tiebreak_nxt = S_tiebreak;
                if (c == '1') S_tiebreak_nxt |= s_all & (~S_add);
                REP(S_count, DD) {
                    dpnxt[f(S_tiebreak_nxt, S_count | s2mask[S_add])] += dp[f(S_tiebreak, S_count)];
                }
            }
        }
