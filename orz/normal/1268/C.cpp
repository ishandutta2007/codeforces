...
int another_main()
{
	...
	for (int i = 0; i < n; ++i)
		s[i + 1] = s[i] + b[i];
	set<int> positions;
	ll sum_left = 0, sum_right = 0, num_left = 0, num_right = 0;
	positions.insert(b[0]);
	add(b[0]);
	int current_position = b[0];
	cout << 0;
	ll perms = 0;
	for (int i = 1; i < n; ++i)
	{
		...
		if (b[i] > current_position)
		{
			...
			while (num_right > num_left)
			{
				...
				++it;
				ll new_current_position = *it;
				//sum_left += num_left * (new_current_position - current_position);
				//sum_right -= num_right * (new_current_position - current_position);
				++num_left;
				--num_right;
				sum_left += current_position;
				sum_right -= new_current_position;
		...
		else
		{
			...
			while (num_left > num_right)
			{
				...
				--it;
				ll new_current_position = *it;
				//sum_left -= num_left * (current_position - new_current_position);
				//sum_right += num_right * (current_position - new_current_position);
				--num_left;
				++num_right;
				sum_left -= new_current_position;
				sum_right += current_position;
				current_position = new_current_position;
			}
		}
		ll left_add = ((current_position * num_left - (num_left) * (num_left + 1) / 2) - sum_left);
		ll right_add = (sum_right - (current_position * num_right + (num_right) * (num_right + 1) / 2));
		cout << ' ' << perms + left_add + right_add;
	}
	cout << '\n';
	return 0;
}
