...
void solve() {
  ...
  if (H == 1) {
    ...
    vi A(W, q);
    FOR(i, r) A[i]++;
    print(MAX(A), MIN(A), A[y]);
    return;
  }
  ll n = (H + H - 2) * W;
  auto [q, r] = divmod(K, n);
  vv(ll, A, H, W);
  FOR(x, H) FOR(y, W) { A[x][y] += (x == 0 || x == H - 1 ? q : q + q); }
  FOR(x, H - 1) FOR(y, W) if (r) {
    A[x][y] += 1;
    --r;
  }
  FOR3_R(x, 1, H) FOR(y, W) if (r) {
    A[x][y] += 1;
    --r;
  }
  const ll INF = 1LL << 60;
  ll a = 0, b = INF;
  FOR(x, H) FOR(y, W) chmax(a, A[x][y]);
  FOR(x, H) FOR(y, W) chmin(b, A[x][y]);
  print(a, b, A[x][y]);
}
