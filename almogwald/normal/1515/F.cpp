...
#define YES co "YES" el
#define NO co "NO" el
typedef vector<veci> veveci;
typedef vector<veci> vevecl;
int main() {
	...
	fori(i, n) {
		color[i] = i;
		allcolor[i].push_back(i);
		color2[i] = i;
		allcolor2[i].push_back(i);
		into(scolor[i])
			sum += scolor[i];
	}
	if (sum < x * (n - 1)) {
	...
	fori(i, m) {
		defa defb
			a--; b--;
		cons[a].push_back({ b,i+1 });
		cons[b].push_back({ a,i+1 });
		cons2.push_back({ a,b });
	}
	YES
		veci loop(n);
	...
	fori(i, n) {
		...
		int cur = i;
		while (scolor[cur] >= x && allcolor[cur].size()>0) {
			int pos = allcolor[cur].back();
			if (loop[pos] == cons[pos].size()) {
				allcolor[cur].pop_back();
				con
			}
			int oth = cons[pos][loop[pos]].ff;
			if (color[oth] == cur || loop[oth] == cons[oth].size()) {
				loop[pos]++;
				con
			}
			co cons[pos][loop[pos]++].ss el
				num++;
				lol sum = scolor[cur]+ scolor[color[oth]] -x;
			lol sumi = cur + color[oth];
			merge(color, allcolor, cur, color[oth]);
			merge(color2, allcolor2, color2[pos], color2[oth]);
			cur = color[oth];
			scolor[cur] = sum;
			scolor[sumi-cur] = 0;
		}
	...
	fori(i, m) {
		int l = cons2[i].ff;
		int r = cons2[i].ss;
		if (color2[l] == color2[r]) {
			con
		}
		co i+1 el
			merge(color2, allcolor2, color2[l], color2[r]);
		num++;
	}
		ex
}