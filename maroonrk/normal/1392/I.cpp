...
vi waf(vi a){
	...
	vi c(n+1);
	rep(i,n+1){
		if(i-1>=0)chmax(c[i],a[i-1]);
		if(i<n)chmax(c[i],a[i]);
	}
	return c;
}

vi sub(vi a,vi b,vi xs){
	vi c=waf(a),d=waf(b);
	int q=si(xs);
	vi res(q);
	auto add=[&](vi ysp,vi sgm,int w){
		vi kh(V+1),tk(V+1);
		for(auto v:ysp)kh[v]++;
		for(auto v:sgm)tk[v]++;
		vi z=integer_convolution::multiply(kh,tk);
		per(i,si(z)-1)z[i]+=z[i+1];
		rep(i,q){
			if(xs[i]<si(z))res[i]+=z[xs[i]]*w;
		}
	};
	add(a,b,1);
	add(c,d,1);
...
signed main(){
	...
	int n,m,q;cin>>n>>m>>q;
	vi a=readvi(n),b=readvi(m);
	vi xs=readvi(q);
	
	vi ans1=sub(a,b,xs);
	for(auto&v:a)v=V-v;
	for(auto&v:b)v=V-v;
	for(auto&v:xs)v=2*V-(v-1);
	vi ans2=sub(a,b,xs);
	vi ans(q);
	rep(i,q)ans[i]=ans1[i]-ans2[i];
