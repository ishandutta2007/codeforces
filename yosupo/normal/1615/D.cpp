...
void solve() {
    ...
    auto dfs = [&](auto self, int p, int b) -> void {        
        for (auto e : tr[p]) {
            int d = e.to;
            if (d == b) continue;
            dp[d] = dp[p] ^ (e.dist == -1 ? 0 : e.dist);
            if (e.dist != -1) {
                // ts[p] == ts[d]
                ts.add_clause(p, false, d, true);
                ts.add_clause(p, true, d, false);
            }
            self(self, d, p);
        }
    };
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (__builtin_popcountll(dp[u] ^ dp[v]) % 2 == p) {
            // ts[u] == ts[v]
            ts.add_clause(u, false, v, true);
    ...
    for (int i = 0; i < n - 1; i++) {
        if (wv[i] == -1) {
            wv[i] = (ans[uv[i]] == ans[vv[i]] ? 0 : 1);
        }
        pr.writeln(uv[i] + 1, ' ', vv[i] + 1, ' ', wv[i]);
    }
}
