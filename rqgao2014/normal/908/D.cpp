...
#endif
	int PA,PB;
	read(k,PA,PB);p=(ll)PA*exp(PB+PA,mod-2)%mod;
	f[0][1]=1;
	for(int i=0;i<=k;i++)
		for(int j=0;j<=k;j++){
			int tmp=f[i][j];if(!tmp) continue;
			if(i+j>=k){
//				dprintf("%d %d %d\n",i,j,tmp);
				ch(ans,(ll)(i+j)*tmp);
				ch(ans,(ll)tmp*p%mod*exp(mod+1-p,mod-2));
				continue;
			}
			ch(f[i+j][j],(ll)tmp*(mod+1-p)),ch(f[i][j+1],(ll)tmp*p);
		}
	printf("%d\n",ans);
	return 0;
}