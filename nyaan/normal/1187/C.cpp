...
int main(){
  ...
  out("YES"); 
  int cur = (nd[0] == 1 ? 1 : 10000);
  cout << cur;
  rep(i,N - 1){
    cout << " ";
    if(nd[i] == 1 && nd[i+1] == 1) cout << ++cur;
    else 
    if(nd[i] == 1 && nd[i+1] == 0) cout << (cur=10000);
    else 
    if(nd[i] == 0 && nd[i+1] == 1) cout << (cur = 1);
    else
    if(nd[i] == 0 && nd[i+1] == 0) cout << --cur;
  }
  out();
}