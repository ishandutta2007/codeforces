...
int sl[N];
int sr[N];
int C(int a,int b){return a<b?0:1LL*sl[a]*sr[b]%M*sr[a-b]%M;}
void init()
{
    sl[0]=1;for(int i=1;i<N;i++)sl[i]=1LL*sl[i-1]*i%M;sr[N-1]=ksm(sl[N-1],M-2);
    sr[0]=1;for(int i=N-2;i>0;i--)sr[i]=1LL*sr[i+1]*(i+1)%M;
    return;
}
...
vector<int>dp[N][2];
vector<int>nxt[N];
void getlar(int now,int fa)
{
    lar[now]=1;for(auto to:nxt[now])if(to!=fa)getlar(to,now),lar[now]+=lar[to];
    dp[now][0].resize(lar[now]+1);dp[now][1].resize(lar[now]+1);
    return;
}
void dfs(int now,int fa)
{
    lar[now]=1;dp[now][0][1]=dp[now][1][1]=1;
    for(auto to:nxt[now])if(to!=fa)
    {
        dfs(to,now);
        for(int i=1;i<=lar[now]+lar[to];i++)tp[0][i]=tp[1][i]=0;
        for(int i=1;i<=lar[now];i++)for(int j=1;j<=lar[to];j++)
        {
            tp[0][i+j]+=1LL*dp[now][0][i]*dp[to][1][j]%M;rmod(tp[0][i+j]);
            tp[1][i+j]+=1LL*dp[now][1][i]*dp[to][1][j]%M;rmod(tp[1][i+j]);
            tp[0][i+j-1]+=1LL*dp[now][0][i]*dp[to][0][j]%M;rmod(tp[0][i+j-1]);
            tp[1][i+j-1]+=1LL*dp[now][1][i]*dp[to][0][j]%M;rmod(tp[1][i+j-1]);
            tp[1][i+j-1]+=1LL*dp[now][0][i]*dp[to][1][j]%M;rmod(tp[1][i+j-1]);
        }
        lar[now]+=lar[to];
        for(int i=1;i<=lar[now];i++)dp[now][0][i]=tp[0][i],dp[now][1][i]=tp[1][i];
    }
    return;
}
int main()
{
    ...
    getlar(1,0);dfs(1,0);
    for(int j=1;j<=lar[1];j++){g[n-j]+=dp[1][1][j];rmod(g[n-j]);}
    for(int i=0;i<n;i++)g[i]=1LL*g[i]*ksm(n,(n-i-2+M-1)%(M-1))%M;
    for(int i=0;i<n;i++)for(int j=i;j<n;j++)
    {
        if((j-i)&1){f[i]-=1LL*g[j]*C(j,i)%M;lmod(f[i]);}
        else {f[i]+=1LL*g[j]*C(j,i)%M;rmod(f[i]);}
    }
