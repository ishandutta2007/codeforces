...
template <typename G> struct SCC {
    const G &g;
    Graph gg, rg;
    vector<int> comp, order, used, sz;
    int num;

    SCC(G &g) : g(g), gg(g.size()), rg(g.size()), comp(g.size(), -1), used(g.size()), sz(g.size()) {
    ...
    size_t size(int idx) { return sz[comp[idx]]; }
};
...
main() {
    ...
    scc.build(t);
    int ans = n;
    vi cnt(t.size(), INT_MAX);
    vi vis(n);
    queue<int> q;
    rep(i, n) {
        if(scc.size(i) < 3 or !t[scc[i]].empty()) continue;
        rep(j, n) vis[j] = n;
        vis[i] = 0;
        q.emplace(i);
        while(!q.empty()) {
            ...
            for(auto e : g[now]) {
                if(e == i)
                    vis[i] = vis[now] + 1;
                else if(chmin(vis[e], vis[now] + 1))
                    q.emplace(e);
            }
            if(vis[i]) {
                chmin(cnt[scc[i]], vis[i]);
                break;
            }
        }
    }
    rep(i, scc.num) {
        if(scc.sz[i] > 1 and t[i].empty()) ans += 998 * cnt[i] + 1;
    }
    cout << ans << endl;
}