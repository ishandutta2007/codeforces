...
public:
    void solve(istream& cin, ostream& cout) {
        ...
        FF invTot = FF{1}/tot;
        for (int i = 1; i <= tot; ++i) {
            X[i] = 1 + X[i-1] * invTot * i;
            X[i] /= 1 - invTot * i - frac*FF{tot-i}*invTot;
        }

        vector<FF> Y = X;
        for (int i = tot; i > 0; --i) Y[i-1] += Y[i];

        FF ans = 0;
        for (int i = 0; i < N; ++i) ans += Y[A[i]];
        cout << (ans - (N-1)*Y[0])/N << endl;


        // f(0) = 0
        // f(m) = 1 + m/A * [f(m-1) + f(m)] + (A-m)/A*(n-2)/(n-1)f(m)
        // f(m)[1 - m/A - (A-m)/A*(n-2)/(n-1)] = 1 + m/A * f(m-1)
    }
};
