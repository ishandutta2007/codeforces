...
template <int Modulus> class modint {
  ...
  public:
    ...
    constexpr modint(const int x = 0) noexcept : a(((x % Modulus) + Modulus) % Modulus) {}
...
template <typename T> vector<T> or_conv(vector<T> f, vector<T> g) {
    int n = 1;
...
const int N = 1 << 21;
int dp[N], tdp[N], use[N];
int main() {
    INT(n);
    VV(int, a, n, n);
    // vv(int, a, n, n);
    // rep(i, n) rep(j, n) a[i][j] = rnd(MOD);

    vv(mint, b, n, n);
    rep(i, n) rep(j, n) b[i][j] = a[i][j] * inv(10000);
    ...
    rep(S, 4) {
        ...
        rep(l, n) {
            ...
            vi need(n);
            rep(i, n) {
                if(S & 1 and i == l) need[i] = true;
                if(S & 2 and i == n - 1 - l) need[i] = true;
            }
            REC([&](auto &&ff, int i, int B, mint m = 1) -> void {
                if(i == n) {
                    tdp[B] = m.a;
                } else {
                    if(need[i])
                        ff(i + 1, B | 1 << i, m * b[l][i]);
                    else
                        ff(i + 1, B | 1 << i, m);
                    ff(i + 1, B, m * b[l][i]);
                }
            ...
            (0, 0);
            rep3(i, allbit(n), 0) tdp[i] -= tdp[0];

            rep(i, 1 << n) dp[i] = dp[i] * tdp[i] % MOD;
        }
