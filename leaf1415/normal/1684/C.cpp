...
bool check(ll p)
{
	rep(q, 0, w-1){
		...
		rep(y, 0, h-1){
			ll dif = 0;
			if(p+1 < w && a[y*w+p] > a[y*w+p+1]) dif--;
			if(p-1 >= 0 && a[y*w+p-1] > a[y*w+p]) dif--;
			if(q+1 != p && q+1 < w && a[y*w+q] > a[y*w+q+1]) dif--;
			if(p+1 != q && q-1 >= 0 && a[y*w+q-1] > a[y*w+q]) dif--;
			swap(a[y*w+p], a[y*w+q]);
...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> vec;
		rep(y, 0, h-1) rep(x, 0, w-2){
			if(a[y*w+x] > a[y*w+x+1]) vec.pb(x), cnt[y]++;
		}
		...
		reps(i, vec){
			if(check(vec[i])) goto end;
			if(check(vec[i]+1)) goto end;
		}
		outl(-1);
		end:;
	}
