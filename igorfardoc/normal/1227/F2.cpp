...
int main()
{
    ...
    for(int sum = 1; sum <= am; sum++) {
    	int here = (ll)c(am, sum) * bin_pow(k - 2, am - sum) % mod;
    	if(sum % 2 == 1) {
    		here = (ll)here * bin_pow(2, sum - 1) % mod;
    	} else {
    		here = (ll)here * (bin_pow(2, sum - 1) - (ll)c(sum, sum / 2) * obr(2) % mod + mod) % mod;
    	}
    	ans = (ans + here) % mod;
    }
