...
class kruskalEdge(val from:Int, val to:Int, val price:Long):Comparable<kruskalEdge>{
...
//    val adjList = List(n){ mutableListOf<kruskalEdge>()}
    fun addEdgeWithPrice(a:Int,b:Int,price:Long){
...
//        if(!directed){ adjList[b].add(e)}
    ...
    fun KruskalMST():List<kruskalEdge>?{
...
fun main(){
    solve.cases{
        ...
        val G = weightedGraph(n+1,false)
        val root = n
        for(i in 0 until n){
            G.addEdgeWithPrice(i,root,C[i].toLong())
        }
        for(i in 0 until n){
            for(j in i+1 until n){
                val price = (X[i] - X[j]).absoluteValue + (Y[i] - Y[j]).absoluteValue
                val pp = price.toLong() * (K[i] + K[j])
                G.addEdgeWithPrice(i,j,pp)
            }
        }
        val ret = G.KruskalMST()!!
        val result = ret.sumOf { it.price }
        val stations = ret.filter{it.to == root || it.from == root}
        val actstaitons = stations.map{if(it.to == root) it.from else it.to}
        val other = ret.filter { it.to != root && it.from != root }

        put(result)
        put(actstaitons.size)
        put(actstaitons.map{it+1}.joinToString(" "))
        put(other.size)
        for(o in other){
            put("${o.from+1} ${o.to+1}")
        }
