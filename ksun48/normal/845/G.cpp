...
int main(){
	...
	for(int j = 0; j < MAXB; j++){
		for(int i = 0; i < n; i++){
			...
			jumpmax[j][i] = jumpmax[j-1][i] ^ jumpmax[j-1][jump[j-1][i]];
		}
	}
	vector<int> cyc;
	for(int i = 0; i < m; i++){
		...
		int x = u[a];
		int y = v[a];
		cyc.push_back(c[a] ^ pathq(x,y));
	}
	int ans = pathq(0,n-1);
	for(int l = 30; l >= 0; l--){
		int cur = 0;
		for(int i = 0; i < cyc.size(); i++){
			if( (cyc[i] ^ cur) < cyc[i]) cyc[i] = (cyc[i] ^ cur);
			if( (cyc[i] & (1<<l)) != 0){
				cur = cyc[i];
			}
			if( (ans ^ cur) < ans) ans = (ans ^ cur);
		}
	}
	cout << ans << endl;
}