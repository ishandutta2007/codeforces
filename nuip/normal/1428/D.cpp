...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rrep(i,n){
		...
		if(v==1){
			...
			if(s3.empty()){
				if(s2.empty()){
					if(s1.empty()) no();
					tie(r,c)=s1.back(); s1.pop_back();
				}else{
					tie(r,c)=s2.back(); s2.pop_back();
				}
			}else{
				tie(r,c)=s3.back(); s3.pop_back();
			}
			re.eb(usd,c);
			re.eb(usd,i);
			s3.eb(re.back());
	...
	for(cauto [r,c]:re) ++rs[r], ++cs[c];
	assert(2>=*max_element(all(rs)));
	assert(2>=*max_element(all(cs)));
	cout<<re.size() NL;
	for(cauto [r,c]:re) cout<<r+1<<" "<<c+1 NL;
  return 0;
}