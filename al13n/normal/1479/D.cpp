...
array<node,10000000> nodes;
int nodecnt;

node*alloc(){
  assert(nodecnt<sz(nodes));
  return &nodes[nodecnt++];
}

node* mktree(int a,int b){
  node*n=alloc();
  if(b==a+1) return n;
  int c=(a+b)/2;
  n->l=mktree(a,c);
...
node*add(int p,LL v,int a,int b,const node*n){
  node*m=alloc();
  *m=*n;
  m->v^=v;
  if(b==a+1) return m;
  int c=(a+b)/2;
  if(p<c) m->l=add(p,v,a,c,n->l);
  else m->r=add(p,v,c,b,n->r);
  return m;
}

int find(int l,int a,int b,const node*n,const node*m,int p,LL v){
  if(!(n->v^m->v^((p>=a&&p<b)?v:0ll)) || l>=b) return -1;
  if(b==a+1) return a;
  int c=(a+b)/2;
  int t=find(l,a,c,n->l,m->l,p,v);
  if(t!=-1) return t;
  return find(l,c,b,n->r,m->r,p,v);
}
...
void dfs(int v,node*n,int d,int pr){
  n=add(A[v],H[A[v]],0,sz(A),n);
  V[v]=n;
  D[v]=d;
  J[v][0]=pr;
...
int jump(int a,int d){
  for(int i=sz(J[a])-1;i>=0;--i) if(D[J[a][i]]>=d) a=J[a][i];
  return a;
}
...
int lca(int a,int b){
  ...
  b=jump(b,d);
  if(a==b) return a;
  for(int i=sz(J[0])-1;i>=0;--i) if(J[a][i]!=J[b][i]){
      a=J[a][i];
      b=J[b][i];
    }
  return J[a][0];
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    --u;--v;--a;
    int w=lca(u,v);
    int t=find(a,0,sz(A),V[u],V[v],A[w],H[A[w]]);
    if(t==-1||t>=b) cout<<"-1\n";
    else cout<<t+1<<'\n';
  }
