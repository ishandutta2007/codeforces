...
void solve() {
  ...
  auto Ac = cumsum(A);
  vi I;
  FOR(i, N) if (A[i] > 1) I.eb(i);
  ll ANS = 0;
  // 1 
  FOR(j, len(I) - 1) {
    ll l = I[j] + 1, r = I[j + 1];
    ll n = r - l;
    if (K == 1 && n >= K) ANS += n - K + 1;
  }
  FOR(k, 1, 50) {
    FOR(j, len(I) - k + 1) {
      i128 prod = 1;
      ...
      ll sm = prod / K;
      if (sm * K != prod) continue;
      ll now = Ac[I[j + k - 1] + 1] - Ac[I[j]];
      ll lx = I[j] - I[j - 1] - 1;
      ll rx = I[j + k] - I[j + k - 1] - 1;
      ll add = sm - now;
      if (add < 0 || add > lx + rx) continue;
      ANS += min(add, lx) - max(0, add - rx) + 1;
    }
  }
  print(ANS);
}
