...
void solve() {
  ld PI = acos(0.0) * 2;
  DBL(a, b, theta);
  if (theta == 0 || theta == 180) return print(a * b);
  a *= 0.5, b *= 0.5;
  if (a < b) swap(a, b);
  ...
  if (theta < phi + phi) {
    ld ANS = 4 * a * b;
    ld x, y;
    y = (a - a * cos(theta)) / sin(theta);
    x = (a - b * sin(theta)) / cos(theta);
    ANS -= (a - x) * (b - y);

    x = (b - b * cos(theta)) / (-sin(theta));
    y = (b - a * sin(theta)) / cos(theta);
    ANS -= (x + a) * (b - y);
    print(ANS);
  } else {
    ld xl = (b - b * cos(theta)) / (-sin(theta));
    ld xr = (-b - b * cos(theta)) / (-sin(theta));
    ld ANS = (xr - xl) * b * 2;
    print(ANS);
  }
}
