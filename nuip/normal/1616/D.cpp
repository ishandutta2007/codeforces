...
int main(){
	...
	TT{
		...
		cauto x=read();
		vector<ll> ss(n+1);
		rep(i,n) ss[i+1]=ss[i]+vs[i]-x;
		fill(dp[0][0][0],dp[n+2][0][0],-MOD);
		dp[1][0][0][1]=0;
		reps(i,1,n+1)rep(a,2)rep(b,2)rep(c,2){
			MX(dp[i+1][b][c][0], dp[i][a][b][c]);
			if((!b || !c || i-3<0 || ss[i-3]<=ss[i]) &&
				 (!c || i-2<0 || ss[i-2]<=ss[i])) MX(dp[i+1][b][c][1],dp[i][a][b][c]+1);
		}
		// out(ss,1);
		// rep(i,n+2)rep(a,2)rep(b,2)rep(c,2)if(dp[i][a][b][c]>0) out(i,a,b,c,dp[i][a][b][c],1);
		cout<<*max_element(dp[n+1][0][0],dp[n+2][0][0]) NL;
	}
	return 0;
}