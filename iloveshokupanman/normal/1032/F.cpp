...
int main() {
    ...
    auto dfs = [&](auto&& dfs, ll u, ll v)->vmint {
        vvmint lis;

        for (ll w : g[v]) {
            if (u == w) { continue; }
            lis.push_back(dfs(dfs, v, w));
        }
        ...
        mint thd = 0;
        for (auto v : lis) {
            mint now = (fst+snd) / (v[0] + v[2] * 2) * (v[0]+v[1]);
            thd += now;
        }

        //cout << v << " " << fst.val() << " " << snd.val() << " " << thd.val() << endl;
        return { fst,snd,thd };
    };

    vmint ret = dfs(dfs, -1, 0);

    cout << (ret[0] + ret[2]).val() << endl;
}