...
int main(){
    ...
    rep(i,N){
        rep(j,4){
            auto[dx,dy] = step[j];
            bool o = false;
            if(pts.count(make_pair(x[i]+dx,y[i]+dy))){
                if(pts[make_pair(x[i]+dx,y[i]+dy)]>i){
                    G.unite(i,pts[make_pair(x[i]+dx,y[i]+dy)]);
                }
            }
            else{
                o = true; ans[i] = make_pair(x[i]+dx,y[i]+dy);
            }
            if(o){
                st.emplace_back(i);
            }
