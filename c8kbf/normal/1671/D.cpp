...
inline void AC() {
    ...
    m = read();
    rd(a, n);
    _ mn = *min_element(a+1, a+1+n), mx = *max_element(a+1, a+1+n), c = 0;
    for(_ i = 2; i <= n; ++i) c += abs(a[i]-a[i-1]);
    if(mn <= 1 && mx >= m) {
        writeln(c);
        return;
    } ef(mn <= 1) {
        _ cr = min(abs(m-a[1]), abs(m-a[n]));
        for(_ i = 1; i <= n; ++i) cr = min(cr, 2*abs(m-a[i]));
        writeln(c+cr);
    } ef(mx >= m) {
        _ cr = min(abs(1-a[1]), abs(1-a[n]));
        for(_ i = 1; i <= n; ++i) cr = min(cr, 2*abs(1-a[i]));
        writeln(c+cr);
    } else {
        _ cr = 1E17;
        for(_ i = 1; i <= 3; ++i) for(_ j = 1; j <= 3; ++j) if(i != j || (i == 3 && j == 3)) {
            _ cn = 1E17, cx = 1E17;
            if(i == 1) cn = abs(1-a[1]);
            ef(i == 2) cn = abs(1-a[n]);
            else for(_ k = 1; k <= n; ++k) cn = min(cn, 2*abs(1-a[k]));
            if(j == 1) cx = abs(m-a[1]);
            ef(j == 2) cx = abs(m-a[n]);
            else for(_ k = 1; k <= n; ++k) cx = min(cx, 2*abs(m-a[k]));
            cr = min(cr, cx+cn);
        }
        writeln(c+cr);
    }
