...
vll sums(const vi &a) {
	vll ans(a.size() + 1);
	REP(i, a.size()) ans[i + 1] = ans[i] + a[i];
	rans;
}
void solve() {
	int d, n, m;
	...
	debug(imie(small), imie(big));
	sort(RALL(small));
	sort(RALL(big));
	vll ssu = sums(small), bsu = sums(big);
	ll ans = ssu.back();
	for (int take_big = 1; take_big <= (int) big.size(); ++take_big) {
		ll waste = (take_big - 1) * 1ll * d;
		ll big_waste = big.size() - take_big;
		ll waste_small = max(0ll, waste - big_waste);
		if (waste_small <= (ll)small.size()) {
			ll val = ssu[small.size() - waste_small] + bsu[take_big];
			debug(imie(take_big), imie(val));
			maxi(ans, val);
		}
	}
	printf("%lld\n", ans);
