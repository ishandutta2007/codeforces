...
template<class T> T& cmin(T& a, T b) { if(b < a) a = b; return a; }

template<class T> T& cmax(T& a, T b) { if(b > a) a = b; return a; }
...
int n, m, x[N], y[N], as[N], c[N], ls[N];

vector<tuple<int, int, int>> v[500];
...
void solve() {
    ...
    rep(i, 1, m) {
        ...
        else v[x[id] + y[id] <= k ? x[id] + y[id] : 0].eb(id, ls[id], i), ls[id] = 0;

    }

    rep(i, 1, n) if(ls[i]) v[x[i] + y[i] <= k ? x[i] + y[i] : 0].eb(i, ls[i], m + 1);

    for(auto [i, l, r] : v[0]) for(l += x[i]; l < r; l += x[i] + y[i])

        as[l]++, as[min(l + y[i], r)]--;

    rep(i, 1, m) as[i] += as[i - 1];

    rep(d, 1, k) {

        mem(c, 0);

        for(auto [i, l, r] : v[d]) For(j, l + x[i], min(r, l + x[i] + y[i]))
