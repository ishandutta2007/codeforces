// time-limit: 2000
// problem-url: https://codeforces.com/contest/1753/problem/B
...
int main(){
	...
	while(a.back()<x){
		auto last=a.back();
		auto iter=std::find_if(a.rbegin(), a.rend(), [&](auto const& it){
			return it!=last;
		}).base();
		auto countLast=int(a.end()-iter);

		// have countLast  last!
		if(countLast%(last+1)!=0){
			std::cout<<"No\n";
			return 0;
		}
		a.erase(iter, a.end());
		a.insert(a.end(), std::size_t(countLast/(last+1)), last+1);
	}
	std::cout<<"Yes\n";
