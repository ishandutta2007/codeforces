...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> vs;
	for(auto [a,b]:cnt) vs.pb(b);
	out(vs,1);
	vector<mint> dp(2*n+1); dp[0]=1;
	for(auto v:vs){
		vector<mint> nxt(2*n+1);
		rep(i,n+1){
			nxt[i+v]+=dp[i];
			reps(x,1,v){
				nxt[i+v-x]+=dp[i]*comb(v,x)*fact[v-1]*finv[v-x-1];
			}
		}
		dp=nxt;
	}
