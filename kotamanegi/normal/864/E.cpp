...
pair<long long, pair<long long, long long>> dp[2002][4000] = {};
long long to_vary[2002] = {};
void solve() {
    int n;
    cin >> n;
    vector<long long> wow;
    vector<tuple<long long, long long, long long, long long>> input;
    REP(i, n) {
        long long t, d, p;
        cin >> t >> d >> p;
        input.push_back(make_tuple(d, t, p, i + 1));
        wow.push_back(t);
    }
    sort(input.begin(), input.end());
    REP(i, n) {
        to_vary[get<3>(input[i])] = i;
        long long t = get<1>(input[i]);
        long long d = get<0>(input[i]);
        long long p = get<2>(input[i]);
        long long itring = get<3>(input[i]);
        if (d - t > 0) {
            for (int q = 0; q < d - t; ++q) {
                if (dp[i + 1][q + t].first <= dp[i][q].first + p) {
                    dp[i + 1][q + t].first = dp[i][q].first + p;
                    dp[i + 1][q + t].second.second = dp[i][q].second.first;
                    dp[i + 1][q + t].second.first = itring;
                }
            }
        }
        for (int q = 0; q <= 2000; ++q) {
            if (dp[i + 1][q].first <= dp[i][q].first) {
                dp[i + 1][q].first = dp[i][q].first;
                dp[i + 1][q].second = dp[i][q].second;
            }
        }
    }
    long long ans = 0;
    ...
    int pro = 0;
    int geting = n;
    for (int i = 0; i <= get<0>(input[n - 1]); ++i) {
        if (dp[n][i].first > ans) {
            ans = dp[n][i].first;
            now = i;
        }
    ...
    while (true) {
        if (dp[geting][now].second.first == 0) break;
        visited.push_back(dp[geting][now].second.first);
        int pre_now = now;
        pre_now -= wow[dp[geting][now].second.first - 1];
        geting = to_vary[dp[geting][now].second.first];
        now = pre_now;
    }
