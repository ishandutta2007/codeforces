...
int main() {
  auto [h, w, q] = input<int, int, int>();
  auto a = input<vector<int>>(h * w) - 1;
  DUMP(a);
  constexpr array di{0, 1, 0};
  constexpr array dj{1, 0, -1};
  while (q--) {
    for (int i = 0, j = input() - 1;;) {
      if (i == h) {
        print.go(j + 1);
        print.go(" \n"[q == 0]);
        break;
