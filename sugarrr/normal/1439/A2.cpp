...
bool anspro(){
    ...
    rep(i,0,n-1)rep(j,0,m-1)a[i+1][j+1]=S[i][j]-'0';
    ...
    rep(i,1,n-2){
        for(ll j=1;j<=m;j+=2){
            ...
            ll cnt=0;
            if(a[i][j]==1){cnt++;push(i,j,ret);}
            if(a[i][j+1]==1){cnt++;push(i,j+1,ret);}
            push(i+1,j,ret);cnt++;
            if(cnt==2){
                push(i+1,j+1,ret);cnt++;
            }
            ans.pb(ret);
        }
    }
    rep(j,1,m-2){
        vl ret;
        ll cnt=0;
        if(a[n-1][j]==1){cnt++;push(n-1,j,ret);}
        if(a[n][j]==1){cnt++;push(n,j,ret);}
        if(cnt==0)continue;
        push(n-1,j+1,ret);cnt++;
        if(cnt==2){
            push(n,j+1,ret);cnt++;
        }
        ans.pb(ret);
