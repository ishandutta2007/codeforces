...
int main() {
    ...
    Combinatorics<MI> c(n + 1);
    vector dp(n, vector(m, MI(0)));
    rep(i, n) {
        rep(j, m) {
            int l = x[j], r = x[j + 1], w = r - l;
            repr(k, i) {
                if (lr[k][0] >= r || lr[k][1] <= l)
                    break;
                int cnt = i - k + 1;
                MI choose_nums = 1;
                rep(uniq, 1, cnt + 1) {
                    choose_nums *= w - uniq + 1;
                    choose_nums /= uniq;
                    auto choose_same = c.binom(cnt - 1, cnt - uniq);
                    dp[i][j] += choose_nums * choose_same * (k ? (j ? dp[k - 1][j - 1] : MI(0)) : MI(1));
                }
            }
        }
