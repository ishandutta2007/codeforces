...
void solve() {
  ...
  const ll INF = 1LL << 60;
  ll B_num = (N + B_sz - 1) / B_sz;
  while (A.size() < B_num * B_sz) A.eb(-INF);
  N = A.size();
  ...
  */
  vi ADD(B_num);
  vv(ll, seq, B_num, B_sz);

  auto build = [&](ll b) -> void {
    ll L = b * B_sz;
    ll R = L + B_sz;
    FOR3(i, L, R) A[i] += ADD[b];
    ADD[b] = 0;
  ...
  auto calc = [&](ll y) -> ll {
    ...
    FOR(b, B_num) {
      ll x = y - ADD[b];
      if (UB(seq[b], x) == LB(seq[b], x)) continue;
      ll L = b * B_sz, R = (b + 1) * B_sz;
      FOR3(i, L, R) if (A[i] == x) {
        l = i;
        break;
      }
