...
int main() {
	...
	ll ssz = s.size(), lsz = l.size();
	vector<ll> ps(ssz + 1), pl(lsz + 1);
	for (int i = 1; i <= ssz; i++)
		ps[i] = ps[i - 1] + s[i - 1];
	for (int i = 1; i <= lsz; i++)
		pl[i] = pl[i - 1] + l[i - 1];
	...
	ll ans = 0;
	for (ll take = 0; take <= lsz; take++) {
		ll mute = max(d * (take - 1), 0);
		ll ltake = min(mute, lsz - take);
		mute -= ltake;
		if (mute > ssz)
			continue;
...
//		W(take, ps[snum], pl[take]);
		chkmax(ans, ps[snum] + pl[take]);
	}
