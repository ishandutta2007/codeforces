...
std::pair<int, mint> GaussElimination(vector<vector<mint>>& a, int pivot_end = -1, bool diagonalize = false) {
...
void solve() {
    ...
    for (int i = 0; i < m; i++) idx[a[i]][b[i]] = idx[b[i]][a[i]] = i;
    vector<vector<int>> A;
    vector<mint> B;
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            for (int k = j + 1; k < n; k++) {
                if (idx[i][j] >= 0 and idx[j][k] >= 0 and idx[k][i] >= 0) {
                    vector<int> cand(m, 0);
                    int sum = 0;
                    for (int x : {idx[i][j], idx[j][k], idx[k][i]}) {
                        if (c[x] == -1)
                            cand[x] = 1;
                        else
                            sum = (sum + 3 - (c[x] - 1)) % 3;
    ...
    if (A.empty()) {
        for (int i = 0; i < m; i++) cout << (c[i] == -1 ? 1 : c[i]) << (i + 1 == m ? '\n' : ' ');
        return;
    }

    vector<vector<mint>> M(A.size(), vector<mint>(A[0].size()));
    for (int i = 0; i < A.size(); i++) {
        for (int j = 0; j < A[i].size(); j++) {
            M[i][j] = A[i][j];
        }
    }
    auto res = LinearEquation(M, B);
    if (res.empty()) {
        cout << -1 << '\n';
        return;
    }

    for (int i = 0; i < m; i++) cout << (c[i] == -1 ? res[0][i].val() + 1 : c[i]) << (i + 1 == m ? '\n' : ' ');
}
