...
array<int,3> dfs1(int v,int p)
{
    array<int,3> ans=(g[v].size()>=3?array<int,3>{-1,0,v}:array<int,3>{-INF,0,0});
    multiset<int> got;
    for(int t:g[v])
        if(t!=p)
        {
    ...
    while(got.size()<2) got.insert(0);
    ans[0]++;
    if(ans[2]==v||ans[2]==0)
        ans[1]=(*got.begin())+(*got.rbegin());
    if(ans[2]==0)
        ans[1]++;
    return ans;
}

bool dfs2(int v,int p,int to)
{
    if(v==to)
        return d[v]=true;
    for(int t:g[v])
...
{
    pii ans=(!add[v]?pii{-1,v}:pii{-INF,0});
    for(int t:g[v])
        if(t!=p&&!d[t])
            ans=max(ans,dfs3(t,v));
    ans.x++;
    return ans;
}

bool dfs4(int v,int p,int to)
{
    if(v==to)
        return add[v]=true;
    for(int t:g[v])
