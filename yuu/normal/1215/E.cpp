...
vector<int> pos[20];
int64_t f[1 << 20];
int64_t after_cost[20][20];  // after_cost[i][j] = cost to move i to after j
int cnt[20];
void solve() {
    cin >> n;
    a | ARRAY | 0 | n - 1 | ARRAY_INPUT;
    for (int i = 0; i < n; i++) {
        a[i]--;
        cnt[a[i]]++;
        for (int j = 0; j < 20; j++) after_cost[j][a[i]] += cnt[j];
    }
    fill(f, f + (1 << 20), 1e18);
    f[0] = 0;
    for (int i = 0; i < (1 << 20); i++) {
        ...
        p[0].clear();
        p[1].clear();
        for (int j = 0; j < 20; j++) p[(i >> j) & 1].emplace_back(j);
        for (auto&& j : p[0]) {
            int64_t cost = f[i];
            for (auto&& k : p[1]) cost += after_cost[j][k];
