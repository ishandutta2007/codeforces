...
int low(const vi &a, const vi &b, int bit) {
    assert(sz(a) >= sz(b));
    ...
    vi az, bz, ao, bo;
    trav (e, a) ((e >> bit)&1 ? ao : az).pb(e);
    trav (e, b) ((e >> bit)&1 ? bo : bz).pb(e);

    int ans = INT_MAX;
    if (!ao.empty() && !bo.empty()) {
        if (sz(ao) < sz(bo))
            swap(ao, bo);
        ckmin(ans, low(ao, bo, bit - 1));
    }
    if (!az.empty() && !bz.empty()) {
        if (sz(az) < sz(bz))
            swap(az, bz);
        ckmin(ans, low(az, bz, bit - 1));
    }
    if (ans == INT_MAX) {
        ans = low(a, b, bit - 1);
    }
