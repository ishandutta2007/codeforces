...
int main(){
    ...
    STR(t);
    each(c,t){
        if(s.back().size()>=1100000)break;
        s.push_back(s.back()+c+s.back());
    }
    vv(Modint,cnt,n+1,26);
    rep(i,1,n+1){
        cnt[i]=cnt[i-1];
        cnt[i][t[i-1]-'a']+=2_M .pow(n-i);
    }
    rep(_,q){
        LL(m);
        STR(w);
        const ll at=partition_point(all(s),[&](const string&s){return s.size()<w.size();})-s.begin();
        if(at>m){
            out(0);
            continue;
        }
        auto&S=s[at];
        auto z=Z_algorithm(w+S);
        Modint ans1=0;
        rep(i,w.size(),z.size())if(z[i]>=w.size())ans1++;
        ans1*=2_M .pow(m-at);
        auto front=Z_algorithm(string(rall(S))+string(rall(w))),back=Z_algorithm(S+w);
        Modint ans2=0;
        rep(w.size())if(i==0||front.rbegin()[i-1]==i)if(i+1==w.size()||back[S.size()+i+1]==w.size()-i-1)ans2+=cnt[m][w[i]-'a']-cnt[at][w[i]-'a'];
        ans2/=2_M .pow(n-m);
        out(ans1+ans2);
    }
}