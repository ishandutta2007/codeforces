...
int n,a[maxn];
string str;
modint pre[maxn],suf[maxn],presum[maxn],ans;
...
signed main(){
    ...
    rep(n) a[i]=str[i]-'0';
    pre[0]=suf[0]=0;
    rep(n) pre[i+1]=modint(10)*pre[i]+modint(a[i]);
    rep(n) suf[i+1]=suf[i]+(modint(10).Pow(i))*modint(a[n-1-i]);
    presum[0]=0;
    rep2(i,1,n+1) presum[i]=presum[i-1]+pre[i];
    rep(n) ans+=suf[i]*modint(n-i)+presum[n-1-i]*modint(10).Pow(i);
    print1(ans);
    return 0;
}