...
int find_pred(const int v, vi& p)
{
	if (p[v] == v)
		return v;
	else
		return p[v] = find_pred(p[v], p);
}
...
struct dsu
{
	...
	int find_ans(const int v)
	{
		if (v == p[v])
			return v;
		else
			return p[v] = find_ans(p[v]);
	}
...
vi find_ans(const vi& a, int& b)
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		if (pans[a[i]] != -1)
		{
			if (d.find_ans(i) != d.find_ans(a[i]))
			{
				int q = find_pred(i, p);
				ans[pr] = q;
				pans[q] = pr;
