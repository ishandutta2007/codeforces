...
pii a[100007];
int minn[27][100007],gcdd[27][100007];
int n,t;
...
int main()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		scanf("%d",&aa);
		a[i]=mp(aa,i);
		minn[0][i]=gcdd[0][i]=aa;
	}
	sort(a+1,a+n+1);
	for(int i=1;i<=20;i++)
		for(int j=1;j+MASK(i)-1<=n;j++)
		{
			gcdd[i][j]=gcd(gcdd[i-1][j],gcdd[i-1][j+MASK(i-1)]);
			minn[i][j]=min(minn[i-1][j],minn[i-1][j+MASK(i-1)]);
		}
	scanf("%d",&t);
	while(t--)
	{
		...
		scanf("%d%d",&l,&r);
		int dg=0;
		while(MASK(dg+1)<=r-l+1)
			dg++;
		int Gcd=gcd(gcdd[dg][l],gcdd[dg][r-MASK(dg)+1]);
		int Min=min(minn[dg][l],minn[dg][r-MASK(dg)+1]);
