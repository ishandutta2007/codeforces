    ...
    bool dij() {
        ...
        while (!pq.empty()) {
            ...
            for (auto& e : adj[v]) {
                ...
                if (l[v] != inf_T && e.f+eps < e.w && c+d+eps < l[e.v])
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            if (y[i] > y[j])
                f.edge(2*i, 2*j+1, n, hypot(x[i]-x[j], y[i]-y[j]));
    auto [r, c] = f.solve(N-3, N-1);
    if (r+0.5 < n-1) cout << "-1\n";
    else cout << setprecision(20) << fixed << c << "\n";
}