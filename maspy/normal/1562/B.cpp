...
void solve() {
  ...
  ll ANS = [&]() -> ll {
    FOR(i, keta) if (!is_prime[A[i]]) return A[i];
    FOR(b, keta) FOR(a, b) {
      ll x = A[a] * 10 + A[b];
      if (!is_prime[x]) return x;
    }
    FOR(c, keta) FOR(b, c) FOR(a, b) {
      ll x = A[a] * 100 + A[b] * 10 + A[c];
      if (!is_prime[x]) return x;
    }
    FOR(d, keta) FOR(c, d) FOR(b, c) FOR(a, b) {
      ll x = A[a] * 1000 + A[b] * 100 + A[c] * 10 + A[d];
      if (!is_prime[x]) return x;
    }
    return 0;
  }();
