...
signed main(){
    ...
    ll cnt=0;
    each(i,a){
        if(i)x.push_back(cnt);
        else cnt++;
    }
    vv(ll,dp,cnt+1,n*(n-1)/2+1,LINF); //  0,  -> 
    dp[0][0]=0;
    x.push_back(cnt);
    each(i,x){
        vv(ll,dp2,cnt+1,n*(n-1)/2+1,LINF);
        rep(d,0,cnt+1){
            const ll cost=d*(d-1)/2;
            rep(x,0,cnt+1-d)rep(y,0,n*(n-1)/2+1)if(dp[x][y]!=LINF)chmin(dp2[x+d][y+abs(x+d-i)],dp[x][y]+cost);
        }
        dp=move(dp2);
    }
    rep(n*(n-1)/2)chmin(dp[cnt][i+1],dp[cnt][i]);
    vector<ll>ans;
    rep(n*(n-1)/2+1)ans.push_back(cnt*(cnt-1)/2-dp[cnt][i]);
    out(ans);
}