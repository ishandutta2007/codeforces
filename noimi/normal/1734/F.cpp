...
int main() {
    ...
    map<tuple<ll, ll, bool>, ll> mp;
    TEST {
        LL(n, m);
        auto res = REC([&](auto &&f, ll st, ll n, bool flip = false) -> ll {
            const auto tt = tuple(st, n, flip);
            if(mp.count(tt)) return mp[tt];
            auto &res = mp[tt];
            // dump(st, n);
            if(!n) return res = 0;
            if(st == 0) { return res = (flip ? n : 0); }
            ll r = st + n;
            ll t = 1, c = 0;
            while(t < r) t <<= 1, c++;
            ll mid = t / 2;
            if(st < mid) {
                ll L = mid - st;
                return res = (f(st, L, flip) + f(L, n - L, flip ^ 1));
            } else {
                return res = f(st - mid, n, flip ^ 1);
            }
        })(n, m);
        OUT(res);
    }
