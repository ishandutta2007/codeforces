...
signed main() {
  ...
  FOR(_, T) {
    ...
    Matrix<ll> cum(26, N + 1);
    FOR(c, 26) {
      FOR(i, N) { cum[c][i + 1] = cum[c][i] + (A[i] == c); }
    }

    auto calc = [&](auto self, ll L, ll R) -> vi {
      vi DP(26, INF);
      if (R == L + 1) {
        FOR(c, 26) { DP[c] = (A[L] == c ? 0 : 1); }
        return DP;
      }
      ll M = (L + R) / 2;
      vi DPL = self(self, L, M);
      vi DPR = self(self, M, R);
      FOR(c, 25) {
        ll cnt_L = cum[c][M] - cum[c][L];
        ll cnt_R = cum[c][R] - cum[c][M];
        chmin(DP[c], DPL[c + 1] + (R - M) - cnt_R);
        chmin(DP[c], DPR[c + 1] + (M - L) - cnt_L);
      }
      return DP;
    };

    print(calc(calc, 0, N)[0]);
  }
