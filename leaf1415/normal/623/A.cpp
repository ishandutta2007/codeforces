...
int main(void)
{
	...
	rep(i, 1, n){
		for(auto u : G[i]){
			if(!uf.same(i, u)) g[uf.root(i)].push_back(uf.root(u));
		}
	}
	rep(i, 1, n) uniq(g[i]);
	...
	rep(i, 1, n) rep(j, 1, n){
		if(abs(ans[i-1] - ans[j-1]) <= 1) G2[i].push_back(j);
	}
	rep(i, 1, n) sort(all(G2[i]));
