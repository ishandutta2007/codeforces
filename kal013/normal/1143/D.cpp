...
inline pii mul(int x,int y,int k,int n){
    int del=y-x;
    if (del<0) del+=k;
    assert(del>=0);
    int mm=1;
...
void solve(){
    ...
    cin>>n>>k;
    cin>>a>>b;
    int r1=1+a,r2=(k+1-a)%(n*k);
    if (a==0) r2=r1;
    int t1=1+b,t2=(k+1-b)%(n*k);
    if (b==0) t2=t1;
    pii tr1=mul(r1,t1,k,n),tr2=mul(r1,t2,k,n),tr3=mul(r2,t1,k,n),tr4=mul(r2,t2,k,n);
    int mi=min(tr1.F,tr2.F),ma=max(tr1.S,tr2.S);
    mi=min(mi,min(tr3.F,tr4.F));
    ma=max(ma,max(tr3.S,tr4.S));
    d2(mi,ma);
}
signed main(){
