...
void solve() {
	...
	for (int i = 0; i < m; ++i) {
		while (geko.empty() == false and geko.back().second < B[i].second) {
			geko.pop_back();
		}
		geko.push_back(B[i]);
	}
	REP(i, n) {
		for (int q = geko.size()-1; q >= 0; --q) {
			int itr = geko[q].first - A[i].first;
			if (itr < 0) break;
			calcs[itr] = max(calcs[itr], geko[q].second + 1 - A[i].second);
		}
	}
	int ans = 1e18;
	for (int i = MAX - 2; i >= 0; --i) {
		calcs[i] = max(calcs[i + 1], calcs[i]);
		ans = min(ans, i + calcs[i]);
	}
	cout << ans << endl;
}
#undef int
