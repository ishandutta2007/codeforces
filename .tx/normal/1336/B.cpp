...
void solve() {
	int nr, ng, nb;
	cin >> nr >> ng >> nb;
	vector<vector<int>> mt(3);
	mt[0].resize(nr);
	mt[1].resize(ng);
	mt[2].resize(nb);
	cin >> mt[0] >> mt[1] >> mt[2];
	for (int i = 0; i < 3; i++) {
		sort(begin(mt[i]), end(mt[i]));
	}
	
	auto f = [](llong x, llong y, llong z) {
		return (x - y) * (x - y) + (y - z) * (y - z) + (x - z) * (x - z);
	};


	auto fi = [&](int i, int j, int k) {
		return f(mt[0][i], mt[1][j], mt[2][k]);
	};

	llong ans = fi(0, 0, 0);

	for (int it = 0; it < 3; it++) {
		for (int i = 0; i < isz(mt[0]); i++) {
			int j = lower_bound(begin(mt[1]), end(mt[1]), mt[0][i]) - begin(mt[1]);
			int k = lower_bound(begin(mt[2]), end(mt[2]), mt[0][i]) - begin(mt[2]);
			if (j < isz(mt[1]) && k < isz(mt[2])) {
				ans = min(ans, fi(i, j, k));
			}
			if (j && k < isz(mt[2])) {
				ans = min(ans, fi(i, j - 1, k));
			}
			if (k && j < isz(mt[1])) {
				ans = min(ans, fi(i, j, k - 1));
			}
			if (j && k) {
				ans = min(ans, fi(i, j - 1, k - 1));
			}
		}
		mt[0].swap(mt[1]);
		mt[1].swap(mt[2]);
