...
int main() {
  ...
  rep(t) {
    ...
    rep(time, 2) {
      int u = 0, v = s.size()-1;
      while (u < s.size() and s[u] == r[u]) u++;
      while (v > u-1 and s[v] == r[v]) v--;
      int w = 0;
      debug(u);
      debug(v);
      rep(ss, 1, v-u+1) {
        bool ok = true;
        rep(i, ss) {
          if (s[u+i] != r[v-ss+1+i]) { ok = false; break; }
        }
        if (ok) chmax(w, ss);
        debug(ok);
        debug(ss);
      }
      rep(i, u+w) res[time].push_back(s[i]);
      rep(i, v+1, s.size()) res[time].push_back(s[i]);
      swap(s, r);
    }
    OUT(res[0].size() > res[1].size() ? res[0] : res[1]);
    debug((res[0].size() > res[1].size() ? res[0] : res[1]).size());
  }
}