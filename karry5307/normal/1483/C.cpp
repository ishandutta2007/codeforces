...
// Fast IO
ll n,tp;
ll x[MAXN],y[MAXN],f[MAXN],st[MAXN],dd[MAXN],ST[MAXN][22];
inline ll read()
{
...
// Functions
inline void change(ll pos)
{
	ST[pos][0]=f[pos];
	for(register int i=1;i<=22&&pos+(1<<i)<=n+22;i++)
	{
...
int main()
{
	...
	f[n+1]=0,change(n+1),st[0]=n+1,dd[0]=-2e18;
	Rep(i,n,0)
	{
		while(tp&&x[i]<x[st[tp]])
		{
			tp--;
		}
		st[++tp]=i,f[i]=dd[tp]=max(query(i+1,st[tp-1])+y[i],dd[tp-1]),change(i);
	}
	printf("%lld\n",f[1]);
}