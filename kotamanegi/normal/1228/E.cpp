...
#define int ll
modint<MAX_MOD> dp[251][251];
modint<MAX_MOD> mae[1000];
modint<MAX_MOD> gyaku[1000];
modint<MAX_MOD> comb(int a, int b) {
    modint<MAX_MOD> ans = mae[a];
    ans *= gyaku[b];
    ans *= gyaku[a - b];
    return ans;
}
void solve(){
    ...
    for (int i = 1; i < 1000; ++i) {
        mae[i] = mae[i - 1] * i;
        gyaku[i] = modint<MAX_MOD>(1) / mae[i];
    }
    int n, k;
    cin >> n >> k;
    dp[0][0] = 1;
    REP(i, n) {
        REP(q, n + 1) {
            modint<MAX_MOD> neko = 1;
            if (q != 0) neko = pows<modint<MAX_MOD>>(k, q);
            for (int j = q; j <= n; ++j) {
                modint<MAX_MOD> hoge = dp[i][q] * neko;
                if (j == q) {
                    modint<MAX_MOD> tmp = 1;
                    if (q != 0) tmp = pows<modint<MAX_MOD>>(k - 1, q);
                    hoge -= dp[i][q] * tmp;
                }
                int rem = n - q;
                int use = j - q;
                if (rem != use) {
                    hoge *= pows<modint<MAX_MOD>>(k - 1, rem - use);
                }
                hoge *= comb(rem, use);
                dp[i + 1][j] += hoge;
            }
