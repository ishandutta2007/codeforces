...
fn dfs(u: usize, e: &[Vec<usize>], sz: &mut [usize]) {
	sz[u] = 1;
	for &v in e[u].iter() {
		if sz[v] == 0 {
			dfs(v, e, sz);
...
fn main() {
	...
	let mut sz = v!([n] = 0);
	dfs(0, &e, &mut sz);
	let lca = lca::TreeOnline::from_adj_list(e, Some(0));
	rep!{[rin.u()]
		l!(a,b = rin.u()-1);
		if a == b {
			writeln!(rout, "{}", n).ok();
		...
		if lca.dep[a] == lca.dep[b] {
			let x = lca.go_up(a, d/2-1);
			let y = lca.go_up(b, d/2-1);
			let res = n - sz[x] - sz[y];
			writeln!(rout, "{}", res).ok();
		} else {
			let z = if lca.dep[a] > lca.dep[b] {a} else {b};
			let x = lca.go_up(z, d/2-1);
			let y = lca.go_up(x, 1);
			let res = sz[y] - sz[x];
			writeln!(rout, "{}", res).ok();
		}
