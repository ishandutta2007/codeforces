...
void solve() {
  ...
  vc<set<ll>> nbd(N);
  FOR(v, N) FORIN(e, G[v]) nbd[v].insert(e.to);

  if (N > 10) {
    vi TO(N);
    FOR(v, N) TO[v] = v;
    while (1) {
      random_shuffle(all(TO));
      bool ok = [&]() {
        FORIN(e, G.edges) {
          ll a = TO[e.frm], b = TO[e.to];
          if (nbd[a].count(b)) return false;
        }
        return true;
      }();
      if (!ok) continue;
    ...
    vc<pi> H;
    FOR(b, N) FOR(a, b) {
      if (nbd[a].count(b)) continue;
      H.eb(mp(a, b));
    }
    while (1) {
      ll ms = clock() * 1000 / CLOCKS_PER_SEC;
      if (ms > 1000) return print(-1);
      ...
      bool ok = [&]() {
        ...
        FOR(v, N) if (deg[v] > 2) return false;
        return true;
      }();
      if (!ok) continue;
      FOR(i, M) {
