...
int gl[100010];
void dfs(int v, int curg = 0, int p = 0) {
	tin[v] = ++timer;
	up[v][0] = p;
	gl[v] = curg ;
	for (int i = 1; i <= l; ++i)
...
int main() {
	...
	while (m--){
		...
		--a; --b;
		int res = lca(a, b); 
		int dist = gl[a] - gl[res] + gl[b] - gl[res];
		if (dist & 1){
			cout << "0\n";
			continue;
		}
		...
		if (a == b){
			...
			int mid = find(a, hd);
			int pra = find(a, hd - 1);
			ans = count(mid) - count(pra);
		}
		else if (gl[a] == gl[b]) {
			int pra = find(a, gl[a] - gl[res] - 1);
			int prb = find(b, gl[b] - gl[res] - 1);
			ans = n - count(pra) - count(prb);
		}
		else {
			int mid = find(a, dist / 2);
			int pra = find(a, dist / 2 - 1);
			ans = count(mid) - count(pra);
		}
		cout << ans << '\n';
	}
