...
void solve() {
  ...
  vc<int> vs(K);
  iota(all(vs), 0);
  auto [dist, par, root] = dijkstra(G, vs);
  VEC(pi, query, Q);
  for (auto&& [a, b]: query) --a, --b;

  // merge events
  vc<tuple<ll, ll, ll>> event;
  for (auto&& e: G.edges) {
    if (root[e.frm] != root[e.to]) {
      ll cost = dist[e.frm] + dist[e.to] + e.cost;
      event.eb(cost, root[e.frm], root[e.to]);
    }
  }
  sort(all(event));
  ...
  auto OK = parallel_binary_search(Q, len(event), 0, init, upd, check);
  vi ANS(Q);
  FOR(q, Q) ANS[q] = get<0>(event[OK[q] - 1]);
