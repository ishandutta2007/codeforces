...
ll calc(int n, int k) {
	int st = pwm(k, n, FFT_MOD);
	for (int a = 0; 2 * a <= n; ++a) {
		st = (st - (ll)cnk(n, a) * cnk(n - a, a) % FFT_MOD * pwm(k - 2, n - 2 * a, FFT_MOD) % FFT_MOD + FFT_MOD) % FFT_MOD;
	}
	st = (ll)st * inv(2, FFT_MOD) % FFT_MOD;
	return st;
}
...
void solve() {
	fct[0] = ifct[0] = 1;
	for (int i = 1; i < N; ++i) {
		fct[i] = fct[i - 1] * ll(i) % FFT_MOD;
		ifct[i] = inv(fct[i], FFT_MOD);
	}
	int n, k;
	cin >> n >> k;
	vector<int> v(n);
	...
	for (int i = 0; i < n; ++i) {
		if (vv[i] != v[i]) {
			...
			mul = (ll)mul * k % FFT_MOD;
		}
	}
	cout << calc(ne, k) * mul % FFT_MOD << endl;
}
