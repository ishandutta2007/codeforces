...
void solve(){
  ...
  while(Q--){
    ...
    if(op[0]==':'){
      ...
      if(s.size()>3){
        L[now].resize(3);
        R[now]=s.substr(s.size()-3);
      }
      ans[now]=0;
      for(size_t i=0;i+3<s.size();++i)ans[now]+=s[i]=='h'&&s[i+1]=='a'&&s[i+2]=='h'&&s[i+3]=='a';
    }else{
      cin>>s>>op>>t;
      ll a=ind[s],b=ind[t];
      debug(a,b,L[a],R[a],L[b],R[b]);
      ll nowans=ans[a]+ans[b];
      string nowL=L[a]+L[b];
      string nowR=R[a]+R[b];
      debug(nowL,nowR);
      if(nowL.size()>3)nowL.resize(3);
      if(nowR.size()>3)nowR=nowR.substr(nowR.size()-3);
      debug(nowans,nowL,nowR);
      s=R[a]+L[b];
      for(size_t i=0;i+3<s.size();++i)nowans+=s[i]=='h'&&s[i+1]=='a'&&s[i+2]=='h'&&s[i+3]=='a';
      swap(L[now],nowL);
      swap(R[now],nowR);
      swap(ans[now],nowans);
    }
    debug(now,ans[now],L[now],R[now]);
  }
  print(ans[now]);
}
int main(){
