...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
		for (int j = 0; j <= z; ++j) f[i][j] = -INFll;
	f[1][0] = a[1];
	ll res = a[1];
	for (int i = 1; i <= n; ++i)
		for (int j = 0; j <= z; ++j)
		{
			if (i - 1 + j * 2 <= k) freshmax(res, f[i][j]);
			if (i - 1 + j * 2 + 1 <= k && i >= 2 && j < z) freshmax(res, f[i][j] + a[i - 1]);
			if (i >= 2) freshmax(f[i][j + 1], f[i][j] + a[i] + a[i - 1]);
			freshmax(f[i + 1][j], f[i][j] + a[i + 1]);
		}
	writeln(res);
}
