...
int main(){
    ...
    FORR(i,in)
        cin >> i;
    vector<double> ans;
    /* set<pair<double,double>> st; //{-height,x} */
    vector<pair<double,double>> st; //{x,height}
    const double rt2 = 2*r;
    const double sqr2 = rt2*rt2;
    FORR(i,in){
        /* bool w = 0; */
        double besth = r;
        FORR(x,st){
            if(abs(i-x.e1) <= rt2){
                /* w = 1; */
                double newh = x.e2 + sqrt(sqr2-pow(abs(i-x.e1),2));
                if(newh > besth)
                    besth = newh;
            }
            /* if(abs(i-x.e2) <= rt2){ */
            /*     w = 1; */
            /*     double height = (-x.e1)+sqrt(sqr2-pow(abs(i-x.e2),2)); */
            /*     ans.pb(height); */
            /*     st.insert({-height,i}); */
            /*     break; */
            /* } */
