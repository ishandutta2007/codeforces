...
fun main(){
    solve.cases {
        ...
        fun solve(arr:IntArray):Boolean {
            val have = BooleanArray(31)
            val dsu = DisjointUnionSets(arr.size + 31)
            for((j, a) in arr.withIndex()) {
                for(i in 0 until 31) {
                    if(a.has(i)) {
                        have[i] = true
                        dsu.union(j + 31, i)
            ...
            if(dsu.components == have.count { !it } + 1) {
                return true
            }
            return false
        }
        ...
        val bins = Array(33){mint}
        for(i in 0 until n){
            val here = L[i]
            bins[here.countTrailingZeroBits()].add(i)
        }
        val v = bins.indexOfLast { it.isNotEmpty() }
        if(bins[v].size == 1){
            val v1 = bins[v][0]
            L[v1] --
            put(ret+1)
