...
public class E {
	...
	void solve() throws Exception
	{
		...
		for(int i = -1;i <= p;i++){
			for(int j = i+1;j <= p;j++){
				int L = i == -1 ? -1 : i == p ? n : ast[i];
				int R = j == -1 ? -1 : j == p ? n : ast[j];
				L++;
				String y = x.substring(0, L) + "(" + x.substring(L, R) + ")" + x.substring(R);
				base = Math.max(base, p3.eval(y.toCharArray()));
			}
		}
		out.println(base);
	}
...
//		char[] u = new char[n*2+1];
//		Arrays.fill(u, '+');
//		for (int i = 0; i < n*2+1; i+=2) {
//			u[i] = (char)('0'+gen.nextInt(10));
//		}
//		for(int i = 0;i < 15;i++){
//			int w = gen.nextInt(n);
//			u[w*2+1] = '*';
//		}
//		sb.append(new String(u));
//		INPUT = sb.toString();
