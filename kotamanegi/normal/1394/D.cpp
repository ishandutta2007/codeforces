...
int state = 0;
std::pair<ll, ll> baby[100010];
ll modlog(ll x, ll y, const ll p) {
    ll H = sqrt(p) + 1;
...
int out[3000000];
int visited[3000000];
vector<int> vertexs[3000000];
vector<int> cost, height;
int memo;
pair<int, int> dp(int now,int back) {
    visited[now] = 1;
    vector<int> go;
    int base = 0;
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        pair<int, int> hoge = dp(x, now);
        base += hoge.first;
        go.push_back(hoge.second - hoge.first);
    }
    pair<int, int> ans = mp(1e18, 1e18);
    sort(ALL(go));
    if (back == -1) {
        int tmp = base;
        int now_in = in[now] + (int)go.size();
        int now_out = out[now];
        memo = tmp + cost[now] * max(now_in, now_out);
        for (int q = 0; q < go.size(); ++q) {
            now_in--;
            now_out++;
            tmp += go[q];
            memo = min(memo, tmp + cost[now] * max(now_in, now_out));
        }
    }
    else {
        {
            int tmp = base;
            int now_in = in[now] + (int)go.size();
            int now_out = out[now] + 1LL;
            ans.first = tmp + cost[now] * max(now_in, now_out);
            for (int q = 0; q < go.size(); ++q) {
                now_in--;
                now_out++;
                tmp += go[q];
                ans.first = min(ans.first, tmp + cost[now] * max(now_in, now_out));
            }
        }
        {
            int tmp = base;
            int now_in = in[now] + (int)go.size() + 1LL;
            int now_out = out[now];
            ans.second = tmp + cost[now] * max(now_in, now_out);
            for (int q = 0; q < go.size(); ++q) {
                now_in--;
                now_out++;
                tmp += go[q];
                ans.second = min(ans.second, tmp + cost[now] * max(now_in, now_out));
            }
        }
    }
    return ans;
...
void solve() {
    ...
    int ans = 0;
    REP(i, n) {
        if (visited[i] or vertexs[i].size() > 1) continue;
        dp(i,-1);
        ans += memo;
    }
