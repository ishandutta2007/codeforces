// Div. 7 (https://codeforces.com/contest/1633/problem/A)
    ...
    while (t--) {
        ...
        if (n < 100) {
            for (int i = 14; i < 100; i += 7)
                ans = min(ans, pair((n % 10 != i % 10) + (n / 10 != i / 10), i));
        } else {
            for (int i = 105; i < 1000; i += 7) {
                int d = (n % 10 != i % 10) + (n / 10 % 10 != i / 10 % 10) + (n / 100 != i / 100);
                ans = min(ans, pair(d, i));
            }
        }
