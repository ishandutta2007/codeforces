...
int main() {
	...
	function<bool(int, int)> isFib = [&](int u, int size) {
		if (fib[size] <= 3)
			return true;
		...
		function<void(int, int)> dfs = [&](int u, int p) {
			...
			if (sz[u] == fib[size - 2])
				cand.push_back({{u, p}, size - 2});
			if (sz[u] == fib[size - 1])
				cand.push_back({{u, p}, size - 1});
		};
		dfs(cen, -1);
		assert(sz[cen] == fib[size]);

		if (cand.empty())
			return false;
 
		int now = cand[0].fi.fi;
		int p = cand[0].fi.se;
		int z = cand[0].se;
		assert(p != -1);
		...
		g[now].erase(find(all(g[now]), p));
		g[p].erase(find(all(g[p]), now));
		
		return isFib(now, z) && isFib(p, size - 1 + size - 2 - z);
	};
