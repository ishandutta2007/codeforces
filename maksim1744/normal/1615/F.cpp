...
void test_case(int test) {
    ...
    vector<vector<Mint>> sm(n + 1, vector<Mint>(n + 1));
    ...
    for (int i = 0; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            ...
            if (i == 0) {
                sm[i][j] = 0;
                cnt[i][j] = (t[j - 1] == '1' ? 0 : cnt[i][j - 1]);
                continue;
            }
            if (j == 0) {
                sm[i][j] = 0;
                cnt[i][j] = (s[i - 1] == '1' ? 0 : cnt[i - 1][j]);
                continue;
            }

            if (s[i - 1] != '0' && t[j - 1] != '0') {
                cnt[i][j] += cnt[i - 1][j - 1];
                sm[i][j] += sm[i - 1][j - 1] + cnt[i - 1][j - 1] * abs(i - j);
            }
            if (s[i - 1] != '1') {
                cnt[i][j] += cnt[i - 1][j];
                sm[i][j] += sm[i - 1][j];
