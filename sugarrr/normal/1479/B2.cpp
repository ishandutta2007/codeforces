...
int main(){fastio
    ...
    rep(i,1,n){
        ...
        ll mn=1+min(seg.prod(0,a[i]),seg.prod(a[i]+1,n+1));
        chmin(mn,seg.get(a[i]));
        seg.apply(0,n+1,1);
        if(i>=2&&seg.get(a[i-1])>mn){
            seg.set(a[i-1],mn);
        }
        /*ll mx=max(seg.prod(0,a[i]),seg.prod(a[i]+1,n+1));
        if(a[i]!=a[i-1]){
            seg.apply(0,n+1,1);
        }
        if(i>=2&&seg.get(a[i-1])<mx+1){
            seg.set(a[i-1],mx+1);
        }*/
     /*   rep(j,0,n){
            ll t=seg.get(j);
