...
struct edge {
    int idx, p, c, l;
    edge(int idx, int p, int c, int l): idx(idx), p(p), c(c), l(l) {}
};
...
bool chk(int x) {
    ...
    int sum=0;
    int mx=-1, sx=-1;
    rep(i, edges.size()) if(edges[i].c==1&&edges[i].l<=x) if(edges[i].p>sx) sx=edges[i].p, mx=i;
    rep(i, edges.size()) if(edges[i].l<=x&&(edges[i].c!=1||i==mx)) {
        sum+=edges[i].p;
        if(edges[i].c%2==1) f.add_edge(0, edges[i].idx, edges[i].p);
        else f.add_edge(edges[i].idx+n, 2*n+1, edges[i].p);
        if(edges[i].c%2==1) rep(j, edges.size()) if(edges[j].c%2==0&&edges[j].l<=x&&(edges[j].c!=1||j==mx)&&isprime[edges[i].c+edges[j].c]) f.add_edge(edges[i].idx, edges[j].idx+n, INF);
    }
    int fl=f.flow(0, 2*n+1);
    return sum-fl>=k;
}
