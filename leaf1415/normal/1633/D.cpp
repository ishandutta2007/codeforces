...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, k) dp[0][i] = -inf;
		dp[0][k] = 0;
		
		rep(i, 0, n-1){
			rep(j, 0, k) dp[(i+1)&1][j] = -inf;
			rep(j, 0, k){
				chmax(dp[(i+1)&1][j], dp[i&1][j]);
				if(j-dist[b[i+1]] >= 0) chmax(dp[(i+1)&1][j-dist[b[i+1]]], dp[i&1][j] + c[i+1]);
			}
		}
		ll ans = -inf;
		rep(i, 0, k) chmax(ans, dp[n&1][i]);
		outl(ans);
	}
