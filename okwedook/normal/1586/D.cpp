...
bool onpc = false;
vector<int> true_ans = {3, 1, 2, 5, 4};

int ask(const vector<int> &val) {
    print("? ");
    for (auto i : val) {
        print(i, ' ');
    }
    ...
    flush();
    if (onpc) {
        vector<int> s(sz(true_ans));
        for (int i = 0; i < sz(true_ans); ++i) {
            s[i] = true_ans[i] + val[i];
        }
        map<int, int> cnt;
        for (auto i : s) cnt[i]++;
