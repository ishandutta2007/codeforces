...
void chemthan() {
  ...
  auto solve1 = [&] () -> long long {
    if (sz(v) == 2 && v[0] + 1 == v[1]) {
      return min(x, y + y);
    }
    return (long long) sz(v) / 2 * y;
  };
  auto solve2 = [&] () -> long long {
    vector<vector<long long>> dp(n, vector<long long>(2, LINF));
    dp[0][a[0] != b[0]] = 0;
    FOR(i, 0, n - 1) {
      vector<vector<long long>> ndp(n, vector<long long>(2, LINF));
      int t = a[i + 1] != b[i + 1];
      FOR(j, 0, n) {
        chkmin(ndp[j][t], dp[j][0]);
        if (j + 1 < n) {
          chkmin(ndp[j + 1][t], dp[j][1]);
        }
        chkmin(ndp[j][t ^ 1], dp[j][1] + x);
        if (j) {
          chkmin(ndp[j - 1][t ^ 1], dp[j][0] + y);
        }
      }
      dp = ndp;
    }
