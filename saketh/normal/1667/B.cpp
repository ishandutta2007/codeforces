...
void solve() {
    ...
    segment_tree<int, decltype(maxx)> st(sz(univ), INT_MIN, maxx);

    auto index = [&](int64_t prefix_sum) {
        return lb(all(univ), prefix_sum) - univ.begin();
    };

    vi sm(sz(univ), INT_MIN);

    vi dp(N + 1);

    auto record = [&](int i) {
        int inx = index(pref[i]);

        {
            int cur = st.accumulate(inx, inx + 1);
            st.assign(inx, max(cur, dp[i] - i));
        }
        {
            ckmax(sm[inx], dp[i]);
        }
    ...
    FOR (i, 1, N + 1) {
        ...
        // a[i] is alone
        dp[i] = dp[i - 1] + sign(a[i - 1]);

        // tail has positive sum
        ckmax(dp[i], i + st.accumulate(0, inx));
