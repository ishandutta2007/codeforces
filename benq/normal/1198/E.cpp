...
int main() {
    ...
    trav(t,Y) t.s = co++;
    trav(t,v) {
        int x1 = X[t.f.f], x2 = X[t.f.s+1]-1;
        int y1 = Y[t.s.f], y2 = Y[t.s.s+1]-1;
        FOR(i,x1,x2+1) FOR(j,y1,y2+1) col[i][j] = 1;
    }
    vpi XX(all(X)), YY(all(Y));
    F0R(i,sz(XX)-1) D.addEdge(0,2+i,XX[i+1].f-XX[i].f);
    F0R(j,sz(YY)-1) D.addEdge(102+j,1,YY[j+1].f-YY[j].f);
    F0R(i,sz(XX)-1) F0R(j,sz(YY)-1) if (col[i][j]) D.addEdge(2+i,102+j,INF);
    ps(D.maxFlow(0,1));
}
