...
void solve()
{
    ...
    for (int i = 0; i<26; i++) if (cnt[i]%2) {cout<<"No"<<endl; return;}
    cout<<"Yes"<<endl;
    //cout<<">>>>>>>"<<endl;
    vector<pair<int, int>> operations;
    for (int i = 0; i<n-1; i++) if (S[i]!=T[i])
    {
        for (int j = i+1; j<n; j++) if (S[j]==S[i])
        {
            operations.push_back(mp(j, i));
            swap(S[j], T[i]);
            break;
        }
        if (S[i]==T[i]) continue;
        ...
        {
            operations.push_back(mp(n-1, j));
            swap(S[n-1], T[j]);
            operations.push_back(mp(n-1, i));
            swap(S[n-1], T[i]);
            break;
        }
    }
    cout<<operations.size()<<endl;
    for (auto it: operations) cout<<it.first+1<<' '<<it.second+1<<endl;
}
