...
int check(int s) {
    ...
    memset(f, 0, sizeof f);
    for (int i = 1; i <= n; i++) {
        if (s && x + a[i] == s) continue;
        f[x + N] = 1;
        x = x + a[i];
    }
    return f[x + N] == 0;
}
void dich1(int l, int r) {
    if (l > r) return;
    int mid = (l + r) >> 1;
    if (check(mid)) {
        ans = mid;
...
int main() {
    ...
    std::cin >> s + 1;
    n = strlen(s + 1);
    for (int i = 1; i <= n; i++) a[i] = s[i] == 'R' ? 1 : -1;
    if (check(0)) return std::cout << 1 << std::endl, 0;
    a[n] == 1 ? dich1(-n, -1) : dich2(1, n);
    std::cout << (ans > 0 ? ans : -ans) << std::endl;
    return 0;
}