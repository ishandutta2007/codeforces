...
int main() {
    ...
    auto res = y_combinator([&](auto solve, int from, int to) -> vector<mint> {
        if (from == to) return {1, 0};
        if (to - from == 1) return {from, 1};
        int mid = (from + to) >> 1;
        auto left = solve(from, mid);
        auto right = solve(mid, to);
        return FFT::multiply<MOD>(all(left), all(right));
    })(0, n - 1);

    if (a + b >= n) cout << "0\n";
    else cout << res[a + b] * choose(a + b, a) << '\n';
}