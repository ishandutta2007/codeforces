...
public class Main {
    ...
    static class TaskB {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            IntList recolored = null;
            int[] queue = new int[n];
            for (char x = 'B'; x <= 'R'; x += 'R' - 'B') {
                ListIndependentSetSystem setSystem = new ListIndependentSetSystem(n);
                for (int i = 0; i < m; i++) {
                    if (c[i] == x) {
                ...
                for (int i = 0; i < m; i++) {
                    if (c[i] != x) {
                        if (setSystem.get(u[i]) == setSystem.get(v[i])) {
                            good = false;
                            break;
                        }
                        graph.addSimpleEdge(setSystem.get(u[i]), setSystem.get(v[i]));
                    }
                }
                if (!good) {
                    continue;
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    pos.addAll(setSystem.getChildren(i));
                    pos.add(i);
                    for (int j = 0; j < size; j++) {
                        int cur = queue[j];
                        for (int k = graph.firstOutbound(cur); k != -1; k = graph.nextOutbound(k)) {
                            ...
                            if (color[next] == 0) {
                                ...
                                if (color[next] == 1) {
                                    pos.addAll(setSystem.getChildren(next));
                                    pos.add(next);
                                } else {
                                    neg.addAll(setSystem.getChildren(next));
                                    neg.add(next);
                                }
                                queue[size++] = next;
                            }
                ...
                if (recolored == null || recolored.size() > current.size()) {
                    recolored = current;
                }
            }
            if (recolored == null) {
                ...
                recolored.sort();
                out.printLine(recolored.size());
                for (int i = 0; i < recolored.size(); i++) {
                    recolored.set(i, recolored.get(i) + 1);
                }
                out.printLine(recolored);
            }
        }
