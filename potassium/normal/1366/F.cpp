...
struct MonoCHT {
	...
	bool isUseless(Line& l1, Line& l2, Line& l3) {
		return (l3.c - l1.c) * (l1.m - l2.m) < (l2.c - l1.c) * (l1.m - l3.m);
	}

	void push_back(Line l) {
		while (Lines.size() >= 2 && isUseless(Lines[ Lines.size() - 2 ], Lines[ Lines.size() - 1 ], l))
			Lines.pop_back();
...
using Line = MonoCHT<long long>::Line;

int main() {
	int n,m,q; R(n,m,q);
	vector<vector<pii>> g(n + 1);
	vector<int> beste(n + 1, 0);
	...
	const int MAXDEP = min(n, q + 1);
	vector<vector<ll>> dp(n + 1, vector<ll>(MAXDEP, LLONG_MIN));
	dp[1][0] = 0;
	for (int d = 0; d + 1 < MAXDEP; d++) {
		for (int u = 1; u <= n; u++) {
			for (auto& tmp : g[u]) {
				int v = tmp.F;
				int w = tmp.S;

				chkmax(dp[v][d + 1], dp[u][d] + w);
			}
		}
	}
	...
	for (int i = 0; i < l.size(); i++) {
		if (i + 1 < l.size() && l[i].F == l[i + 1].F) continue;
		hull.pb({l[i].F, l[i].S});
	}
	...
	ll calc = MAXDEP;
	for (int i = 0; i < hull.Lines.size(); i++) {
		ll isect = q;
		Line& l1 = hull.Lines[i];
		if (i + 1 < hull.Lines.size()) {
			Line& l2 = hull.Lines[i + 1];
			chkmin(isect, (l1.c - l2.c) / (l2.m - l1.m));
		}

		if (isect < calc)
			continue;

		Mint len = isect - calc + 1;
		ans += len * l1.c;
		Mint tri = (isect + calc) * (isect - calc + 1);
		ans += tri * l1.m / 2;
		calc = isect + 1;
		if (calc == q + 1) break;
	}
