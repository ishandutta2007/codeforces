...
void solve(){
	...
	while(nxt <= n){
		...
		hash_table<int,null_type>zan;
		for(int i=0;i<v;i++) zan.insert(i);
		for(int x=nxt;x<=n;x++){
			if(zan.find(a[x]) != zan.end()) zan.erase(a[x]);
			//cout<<zan.size()<<" "<<x<<endl;
			//for(auto a:zan)cout<<a<<" ";cout<<endl;
			cnt[a[x]] --;
			if(zan.empty()){
				nxt = x+1;
				goto v;
