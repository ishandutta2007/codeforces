...
int main() {
	...
	BIT<ll, add> bit(m), bit2(m);
	for (int i = 1; i < m; i++) {
		bit.upd(i, ddb[i]);
		bit2.upd(i, (m - i + 1) * ddb[i]);
	}
	...
	while (q--) {		
		...
		for (int i = 2; i < n; i++) {
			ll now = a[i] - a[i - 1];
			ll sum = 0;
			for (int j = 65536; j >= 1; j >>= 1) {
				if (best[i] + j < m && sum + bit.bit[best[i] + j] <= now) {
					sum += bit.bit[best[i] + j];
					best[i] += j;
				}
			}
			chkmax(best[i], 1);
		...
		for (int i = 1; i < n; i++) {
			ans1 += (best[i + 1] - best[i] + 1) * a[i];
			if (i > 1) ans1 += bit2.qry(best[i]) - (m - best[i]) * bit.qry(best[i]);
		}
