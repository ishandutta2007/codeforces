...
int trie[1 << 21][2], trie_size;

void insert(int x) {
	int p = 0;
	for (int d = 29; ~d; --d) {
		int i = x >> d & 1;
		if (!trie[p][i]) trie[p][i] = ++trie_size;
		p = trie[p][i];
	}
}
...
int dfs(int i, int d) {
	if (!~d) return 0;
	if (trie[i][0] && trie[i][1]) return 1 << d ^ min(dfs(trie[i][0], d - 1), dfs(trie[i][1], d - 1));
	return dfs(trie[i][0] ^ trie[i][1], d - 1);
}
