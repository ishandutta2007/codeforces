...
int main() {
    ...
    ll ans = 6;
    sort(p, p + 3);
    if (p[0].first == p[1].first && p[1].first == p[2].first || p[0].second == p[1].second && p[2].second == p[1].second) {
        cout << 1 << endl;
        return 0;
    }
    auto check = [](pii p1, pii p2, pii p3) {
        return (p1.first == p2.first && (p3.second <= min(p1.second, p2.second) || p3.second >= max(p1.second, p2.second)));
    };
    if (check(p[0], p[1], p[2]) || check(p[1], p[2], p[0]) || check(p[2], p[0], p[1])) {
        cout << 2 << endl;
        return 0;
    }
    FI(3) {
