...
int main() {
    ...
    if(n == 2) drop(2);
    int r;
    rep(i, n) if(si(g[i]) > 1) r = i;
    REC([&](auto &&f, int x, int p) -> pll {
        if(si(g[x]) == 1) return pll(1, 0);
        vpll v;
        for(auto e : g[x]) {
            if(e != p) v.eb(f(e, x));
        }
        ...
        vpll w;
        for(auto [s, x] : v) { w.eb(x + s * s + s - 2 * n * s, s); }
        sort(all(w));
        // dump(w);
        for(auto [A, s] : w) {
            if(!empty(cht.H)) chmin(mi, cht.query(s) + A - n + 1LL * n * n);
            cht.add(2 * s, A);
        }
        ll res = inf<ll>;
        ll sum = 1;
        for(auto [s, _] : v) sum += s;
        for(auto [s, x] : v) { chmin(res, (sum - s) * (sum - s - 1) + x); }
        return pll(sum, res);
    })
    (r, -1);
    OUT(1LL * n * (n - 1) - mi / 2);
