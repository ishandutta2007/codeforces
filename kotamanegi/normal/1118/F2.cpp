...
modint<MOD> dp[400000][2];

vector<int> NV[400000];

void dp_dfs(int now, int back) {
    if (color[now] != 0) {
        dp[now][1] = 1;
        for (auto x : NV[now]) {
            if (x == back) continue;
            dp_dfs(x, now);
            dp[now][1] *= dp[x][0] + dp[x][1];
        }
    }
    else {
        dp[now][0] = 1;
        vector<modint<MOD>> nexter;
        for (auto x : NV[now]) {
            if (x == back) continue;
            dp_dfs(x, now);
            nexter.push_back(dp[x][0] + dp[x][1]);
            dp[now][0] *= dp[x][0] + dp[x][1];
        }
        vector<modint<MOD>> jun;
        vector<modint<MOD>> gyaku;
        jun.push_back(1);
        gyaku.push_back(1);
        for (int q = 0; q < (int)nexter.size() - 1; ++q) {
            jun.push_back(jun.back() * nexter[q]);
            gyaku.push_back(gyaku.back() * nexter[nexter.size() - 1 - q]);
        }
        reverse(ALL(gyaku));
        int itr = 0;
        for (auto x : NV[now]) {
            if (x == back)continue;
            dp[now][1] += dp[x][1] * jun[itr] * gyaku[itr];
            itr++;
        }
    }
    return;
...
void solve() {
    ...
    REP(q, 25) {
        REP(i, n) {
            backs[q + 1][i] = backs[q][backs[q][i]];
        }
    }
    for (int i = 1; i <= k; ++i) {
        int b = colorings[i][0];
        for (int q = 1; q < colorings[i].size(); ++q) {
            b = lca(b, colorings[i][q]);
        }
        if (color[b] != i&&color[b] != 0) {
            cout << 0 << endl;
            return;
        }
        color[b] = i;
        REP(q, colorings[i].size()) {
            if (colorings[i][q] == b) continue;
            colorings[i][q] = backs[0][colorings[i][q]];
            while (color[colorings[i][q]] == 0 && b != colorings[i][q]) {
                color[colorings[i][q]] = i;
                colorings[i][q] = backs[0][colorings[i][q]];
            }
            if (color[colorings[i][q]] != i) {
                cout << 0 << endl;
                return;
            }
        }
    ...
    for (int i = 1; i <= k; ++i) {
        for (int q = 1; q < colorings[i].size(); ++q) {
            um(colorings[i][0], colorings[i][q]);
        }
    }
    ...
    REP(i, edges.size()) {
        ...
        REP(q, 2) {
            NV[edges[i].first].push_back(edges[i].second);
