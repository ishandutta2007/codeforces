...
int main() {
    TEST {
        ...
        //     dump(x, y, now);
        //     rep(i, n) {
        //         if(d[x][i] == 1 and chmin(dp[i][y], now + 1)) q.emplace(i, y);
        //         if(d[y][i] == 1 and chmin(dp[i][x], now + 1)) q.emplace(i, x);
        //     }
        // }

        rep(i, n) {
            fore(e, g[i]) {
                chmin(ans, (d[0][i] + d[e][n - 1] + 1) * e.cost);
                rep(k, n) { chmin(ans, (2 + d[0][k] + d[k][n - 1] + d[i][k]) * e.cost); }
            }
        }
