...
int main()
{
	...
	gn(n);
	readedge();
	rep(i,1,n+1)gn(col[i]);

	int v1=1,v2=2;
	int bo=0;
	for(int u=1;u<=n;u++){
		if(bo==1)break;
		for (int i=g[u];~i;i=e[i].next)if(col[e[i].v]!=col[u]){
			v1=e[i].v;
			v2=u;
			bo=1;
			break;
	...
	if(test(v1))printf("YES\n%d\n",v1);
	else if(test(v2))printf("YES\n%d\n",v2);
	else printf("NO\n");
#ifdef JCVB
	debug("time: %d\n",int(clock()-_time_jc));
#endif
