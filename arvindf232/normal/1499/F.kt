    ...
    solve.cases{
        ...
        G.leafFirst { v->
            ...
            G.subs(v){
                w->
                val other = DP[w]
                val new = IntArray(maxOf(now.size, other.size + 1))
                var empty = 0
                for(right in DP[w].indices){
                    if(right in 0..k){
                        empty = empty modPlus DP[w][right]
                    }
                }
                for(left in now.indices){
                    for(right in DP[w].indices){
                        if(left + (right+1) <= k){
                            new[maxOf(left,right+1)] = new[maxOf(left,right+1)] modPlus (now[left] modM DP[w][right])
                        }
                    }
                    if(left in 0..k) {
                        new[left] = new[left] modPlus (now[left] modM empty)
                    }
                }
                now= new
            }
