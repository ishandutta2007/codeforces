...
void solve()
{
    ...
    // want 25 or 75
    // if begins with 20 or 70 (0 close 2 or 7) -> can just take 0 and make 50
    // if begins with 50

    // 50117 -> 6 moves to make 11750
    // 50117 -> 51017 -> 15017 -> 10517 -> 10157 -> 10175 (5 moves)
    // 500111117 -> need two additional moves to deal with zeroes

    auto find_digits = [&](int a, int b)
    {
        int cnt = 0;
        int res = 0;
    ...
    ans = min(ans, find_digits(5, 2));
    ans = min(ans, find_digits(2, 5) + 1);
    ans = min(ans, find_digits(5, 7));
    ans = min(ans, find_digits(7, 5) + 1);
