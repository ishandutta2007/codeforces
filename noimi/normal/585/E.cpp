...
template <typename T> void f(vector<T> &a) {
    int n = a.size();
    for(int k = 1; k < n; ++k) {
        for(int i = k * 2; i < n; i += k) a[k] += a[i];
    }
}
template <typename T> void invf(vector<T> &a) {
    int n = a.size();
    for(int k = n - 1; k >= 1; --k) {
        for(int i = k * 2; i < n; i += k) a[k] -= a[i];
    }
}
template <typename T> vector<T> mul(vector<T> a, vector<T> b) {
    f(a), f(b);
    rep(i, si(b)) a[i] *= b[i];
    invf(a);
    return a;
}
template <typename T> void mul2(vector<T> &a, vector<T> &b) {
    f(a), f(b);
    rep(i, si(b)) a[i] *= b[i];
    invf(a);
}
...
int main() {
    ...
    vmint P(500001);
    P[0] = 1;
    rep(i, 500000) P[i + 1] = P[i] * 2;
    rep2(i, 1, N - 1) cnt[i] = P[cnt[i].a] - 1;
    inverse_multiple_transform(cnt);
    cnt[1] = 0;
    gcd_convolution::mul2(num, cnt);
    cout << num[1] << endl;
}