...
int main() {
    ...
    while (t--) {
        ...
        M ret = 0;
        if (n % 2 == 0) {
            M ways = 1, tot = power(M(2), n);
            ret = power(cnt - 1, k);    // all bits are 0
            for (int i=k-1; i>=0; i--) {
                ret += ways * power(tot, i);    // say the ith bit is all 1s
                ways *= cnt - 1;
            }
        } else {
            ret = power(cnt + 1, k);    // either both are 0, or both are all on
        }
        cout << ret << "\n";
    }
