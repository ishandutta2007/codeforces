...
template <typename T>
void chmax(T &x, const T &y) { if (y > x) x = y; }
...
int main() {
    ...
    vec<vec<vec<int>>> dp(1 << n, vec<vec<int>>(n, vec<int>(n, 0)));
    rep(i, n) {
        dp[1 << i][i][i] = IINF;
    }
    rep(s, 1 << n) {
        ...
        for (int i : bits[s]) {
            for (int j : bits[s]) {
                ...
                int t = s ^ (1 << j);
                for (int k : bits[t]) {
                    chmax(dp[s][i][j], min(cost[k][j], dp[t][i][k]));
                }
            }
        }
    }
    ...
    rep(i, n) rep(j, n) {
        int c = IINF;
        rep(k, m - 1) {
            c = min(c, abs(a[j][k] - a[i][k + 1]));
        }
        chmax(ans, min(dp[full][i][j], c));
    }
    print(ans);
    return 0;
}