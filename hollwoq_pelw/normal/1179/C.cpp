...
const int MAXN = 1e6 + 0 , MAXM = 3e5 + 5;
...
//void build(int node = 1, int tl = 1, int tr = MAXN){
//    if (tl == tr){
//        st[node] = 0;
//        lz[node] = 0;
//        return ;
...
void upd(int l, int r, int val, int node = 1, int tl = 1, int tr = MAXN){
    if (l > r || tl > r || l > tr)
        return ;
    if (l <= tl && tr <= r){
        add(val, node, tl, tr);
...
int query(int node = 1, int tl = 1, int tr = MAXN){
//    if (tl > r || l > tr || l > r) return {-inf, -inf};
...
//    cerr << "Segtree: Range from " << tl << " to " << tr << " : " << st[node] << endl;
//    if (tl == tr)
//        return ;
