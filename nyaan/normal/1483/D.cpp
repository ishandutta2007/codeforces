...
void Nyaan::solve() {
  ...
  rep(_, Q) {
    inl(u, v, l);
    --u, --v;
    qs[u].emplace_back(v, l);
    qs[v].emplace_back(u, l);
  }

  vvl d(N,vl(N,TEN(10)));
  rep(i,N)each(e,g[i])d[i][e.to]=e.cost;
  warshall_floyd(d);

  vvl mx(N, vl(N, -TEN(10)));
  rep(u, N) {
    // qs[u]v,l
    each2(v, l, qs[u]) { rep(t, N) amax(mx[u][t], l - d[v][t]); }
  }
  trc(mx);

  Edges<ll> es;
  rep(i, N) each(e, g[i]) if (i < e.to) es.emplace_back(e);

  vi ok(M);

  rep(i, M) {
    if (ok[i]) continue;
    int s = es[i].src, t = es[i].to;
    ll cost = es[i].cost;
    rep(u, N) if (d[u][s] + cost <= mx[u][t]) ok[i] = 1;
  }

  out(accumulate(all(ok), 0LL));
}