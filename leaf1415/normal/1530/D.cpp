...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n){
			if(b[i] == 0) b[i] = *S.begin(), S.erase(S.begin());
		}
		
		vector<ll> vec;
		rep(i, 1, n) if(b[i] == i) vec.push_back(i);
		
		if(sz(vec) >= 2){
			reps(i, vec) b[vec[i]] = vec[(i+1)%sz(vec)];
		}
		if(sz(vec) == 1){
			ll p = a[vec[0]], q = 0;
			rep(i, 1, n) if(b[i] == p) q = i;
			b[vec[0]] = p, b[q] = vec[0];
		}
