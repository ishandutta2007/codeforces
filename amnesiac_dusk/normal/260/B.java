...
public class Main
{
	...
	{
		if(!(a.charAt(0)!='-'&&a.charAt(1)!='-'&&a.charAt(3)!='-'&&a.charAt(4)!='-'&&a.charAt(9)!='-'&&a.charAt(2)=='-'&&a.charAt(5)=='-'&&a.charAt(6)=='2'&&a.charAt(7)=='0'&&a.charAt(8)=='1'))
			return 0;
		int d=Integer.parseInt(a.substring(0, 2));
		int m=Integer.parseInt(a.substring(3,5));
		int y=Integer.parseInt(a.substring(6,10));
		if(y<2013||y>2015)
			return 0;
		if(m<=0||m>12)
			return 0;
		...
		if(d==0)
			return 0;
		return d*1000000+m*10000+2010+a.charAt(9)-'0';
	}
	...
	public static void main(String[] args) throws Exception
	{
		...
		{
			int te=check(s.substring(i, i+10));
			if(te!=0)
			{
				ans.put(te, ans.getOrDefault(te, 0)+1);
			}
		}
		int max=0;
		for(int te:ans.values())
		{
			max=Math.max(max, te);
		}
		for(Map.Entry<Integer, Integer> te:ans.entrySet())
		{
			if(max==te.getValue())
			{
				int answ=te.getKey();
