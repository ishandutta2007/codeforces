...
ll ask(vector<int> vs){
	cout<<"?";
	for(cauto &x:vs) cout<<" "<<x+1;
	cout<<endl; cout.flush();
	return read();
}

vv<int> fun(const int n,const int t){
	vv<int> re;
	cauto solve=
		[&](const int ub)->void{
			out(ub,1);
			for(int i=0;i+t<=ub;i+=t){
				re.eb();
				rep(j,t) re.back().eb(i+j);
			}
			if(ub<n){
				vector<int> A(t),B(t);
				iota(all(A),0);
				iota(all(B),0);
				reps(i,ub,n) (i%2?A:B)[(i-ub)/2]=i;
				re.eb(A);
				re.eb(B);
			}
		};
	if(n%t==0){
		re.resize(n/t,vector(t,0));
		rep(i,n) re[i/t][i%t]=i;
		return re;
	}
	if(n/t>1 && (t+n%t)%2==0){ //best
		solve((n/t-1)*t);
		return re;
	}
	// t:odd, n%t:even
	...
	if(n%2==0 && n/t==1){
		cauto er=fun(n,n-t);
		if(re.empty() || er.size()<re.size()){
			re.clear();
			for(cauto &vs:er){
				vector<int> usd(n);
				for(cauto &v:vs) usd[v]=1;
				re.eb();
				rep(i,n)if(!usd[i]) re.back().eb(i);
			}
		}
	}
	assert(re.size());
...
vv<int> fun2(const int n,const int t){
	vv<int> re;
	cauto solve=
		[&](const int ub)->void{
			out(ub,1);
...
int main(){
	...
	if(0){
		reps(n,1,20)reps(t,1,n+1){
			if(t%2==0 && n%2) continue;
			cauto a=fun(n,t);
			cauto b=fun2(n,t);
			if(a.size()>b.size()){
				out(n,t,a,b,1);
