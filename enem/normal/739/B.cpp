...
ll dfs(ll x,V(ll) &path,ll p[],V(pll) ad[],ll sum,ll a[],ll ans[],priority_queue<ll> v[])
{
    path.PB(sum);
    ll l=0,r=path.size()-1;
    while(l<r)
    {
        if((sum-path[(l+r)/2])<=a[x])
        {
            r=(l+r)/2;
        }
        else
    ...
    for(auto it:ad[x])
    {
        vt.PB(dfs(it.F,path,p,ad,sum+it.S,a,ans,v));
        if(v[vt[vt.size()-1]].size()>v[mxi].size())
        {
            mxi=vt[vt.size()-1];
        }
    }
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll ans[n];
        V(ll) path;
        priority_queue<ll> v[n];
        dfs(0,path,p,ad,0,a,ans,v);
        REP(i,0,n)
        {
            cout<<ans[i]<<' ';
        }
