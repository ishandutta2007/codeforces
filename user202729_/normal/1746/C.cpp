...
// actually will be bad if later fail system tests
// will prove later maybe
// ????




// okay this one is proven
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	...
	struct Item{int diff, index;};
	std::vector<Item> items(len-1);
	for(int const i: range(0, len-1)){
		items[i]={a[i+1]-a[i], i};
	}
	std::sort(begin(items), end(items), [&](auto const& first, auto const& sec){
		return first.diff>sec.diff;
	});

	for(auto [diff, index]: items) std::cout<<index+2<<' ';
	std::cout<<"1\n";
}