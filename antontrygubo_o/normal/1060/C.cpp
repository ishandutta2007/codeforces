...
vector<ll> produce(vector<ll> a)
{
    ...
    {
        ll cur = 0;
        for (int i = 0; i<len; i++) cur+=a[i];
        maxx[len] = min(maxx[len], cur);
        for (int i = len; i<n; i++)
        {
            cur+=a[i]; cur-=a[i-len];
            maxx[len] = min(maxx[len], cur);
...
int main()
{
    ...
    auto A = produce(a); auto B = produce(b);

    ll x; cin>>x;

    ll ans = 0;
    for (int i = 0; i<=n; i++)
        for (int j = 0; j<=m; j++) if (B[j] == 0 || A[i]<=x/B[j]) ans = max(ans, 1ll*i*j);
