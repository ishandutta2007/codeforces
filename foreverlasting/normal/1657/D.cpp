...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read(),C=read();
        for(res i=1;i<=n;i++){
            res c=read(),d=read(),h=read();
            A[c].pb((LL)d*h);
        }
        for(res i=1;i<=C;i++){
            if(A[i].empty())continue;
            LL mx=*max_element(A[i].begin(),A[i].end());
            for(res j=i;j<=C;j+=i)f[j]=max(f[j],mx*j/i);
        }
        for(res i=2;i<=C;i++)f[i]=max(f[i],f[i-1]);
        res q=read();
        while(q--){
            res d=read();
            LL h=Read();
