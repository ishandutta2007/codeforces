...
public:
    void solve() {
        ...
        arr2d<int> qty(n + 1, m + 1, 0);
        for (int i : range(n)) {
            for (int j : range(m)) {
                qty(i + 1, j + 1) = qty(i + 1, j) + qty(i, j + 1) - qty(i, j) + (c(i, j) == 'a' ? 1 : 0);
            }
        }
        ll answer = 0;
        for (int i : range(m)) {
            for (int j : range(i + 1, m)) {
                map<int, queue<int>> q;
                for (int l : range(n)) {
                    if (c(l, i) == c(l, j)) {
                        while (!q[c(l, i)].empty()) {
                            int top = q[c(l, i)].front();
                            if (qty(l + 1, j + 1) + qty(top, i) - qty(l + 1, i) - qty(top, j + 1) <= k) {
                                break;
                            }
                            q[c(l, i)].pop();
                        }
