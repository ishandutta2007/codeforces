...
int main(){
  ...
  while(t--){
    ...
    while(1){
      ...
      auto [px,py,pd]=pre;
      /*if(abs(px)<=10){
        cout<<px spa py spa pd<<endl;
        auto [ppx,ppy,ppd]=ch(pre);
        cout<<"p" spa ppx spa ppy spa ppd<<endl;
      }
      else break;*/
      PM nst;
      if(px==gx||py==gy){
        nst.fi=(1-prob)*st.fi;
        nst.se=prob+(1-prob)*(st.se+1);
      }
      else{
        nst=MP(st.fi,st.se+1);
      }
      //if(h==2)cout<<px spa py spa pd spa nst spa prob<<endl;
      if(mp.count(pre)){
        auto xst=mp[pre];
        cout<<(xst.se-nst.se)/(nst.fi-xst.fi)-1<<endl;
        break;
      }
      else{
        mp[pre]=nst;
