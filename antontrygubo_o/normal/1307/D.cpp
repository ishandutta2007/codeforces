...
int main() {
    ...
    for (auto it: special) a.push_back(mp(d1[it], dn[it]));

    sort(a.begin(), a.end());
    for (int i = 0; i<k-1; i++) if (a[i].second<=a[i+1].second) {cout<<d1[n]; return 0;}

    //for (auto it: a) cout<<it.first<<' '<<it.second<<endl;

    int maxx = 0;
    for (int i = 0; i<k-1; i++) maxx = max(maxx, a[i].first + a[i+1].second + 1);
    maxx = min(maxx, d1[n]);
    cout<<maxx;
