...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        res l=1,r=1000000000,ret=0;
        c[n+1]=0;
        while(l<=r){
            res mid=(l+r)>>1;
            res fl=0;
            if(a[n]-mid<0){r=mid-1;continue;}
            LL mx=a[n]-mid;
            mx-=mx%3,c[n]=mx;
            for(res i=n-1;i>=3;i--){
                if(c[i+2]*2+c[i+1]-3*(mid-a[i])<0){fl=1;break;}
                LL o=c[i+2]*2+c[i+1]-3*(mid-a[i]);
                o/=3;
                c[i]=min(o-(o%3),a[i]-(a[i]%3));
            }
            if(c[3]+2*c[4]<(mid-a[2])*3)fl=1;
            if(2*c[3]<(mid-a[1])*3)fl=1;
            if(fl)r=mid-1;
