...
int main() {
	...
	vector<Mint> fac(MAXN + 1, Mint(1, MOD107)), inv = fac;
	for (int i = 2; i <= MAXN; i++) fac[i] = fac[i - 1] * i;
	inv[MAXN] = fac[MAXN].inv();
	for (int i = MAXN; i >= 3; i--) inv[i - 1] = inv[i] * i;

	Mint ans(0, MOD107);
	for (int dist = 1; dist <= min(n - 1, m); dist++) {
		int cc = dist + 1;
		Mint cayley = (n == cc ? Mint(1, MOD107) : cc * Mint(n, MOD107).pow(n - cc - 1));
		Mint nonpath = Mint(m, MOD107).pow(n - 1 - dist);
		Mint path = fac[m - 1] * inv[dist - 1] * inv[m - dist];
		Mint pathnodes = fac[n - 2] * inv[n - cc];
		ans += cayley * nonpath * path * pathnodes;
	}
