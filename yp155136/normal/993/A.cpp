...
int main () {
    ...
    REP0(i,n)
    {
        REP0(j,n)
        {
            pii pa = v1[i],pb = (v1[(i+1)%4]);
            pii qa = v2[i],qb = (v2[ (i+1)%4 ]);
            if ( sgn( (pa-qa)^(pa-qb) ) * sgn( (pb-qa)^(pb-qb) ) == -1 )
            {
                swap(pa,qa);
                swap(pb,qb);
                if ( sgn( (pa-qa)^(pa-qb) ) * sgn( (pb-qa)^(pb-qb) ) == -1 )
    ...
    for (int i=-100;100>=i;i++)
    {
        ...
        {
            pii p = MP(i,j)-cen;
            int dis = abs(p.F) + abs(p.S);
            if (dis <= r && mp[MP(i,j)]) ans = "YES";
        }
    }
    cout << ans << endl;
}