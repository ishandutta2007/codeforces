...
fn run() {
    ...
    let mut val = vec![M::one(); k + 2];
    for i in 1..=(k + 1) {
        val[i] = val[i - 1] + M::from(i + 1).pow(k as u64);
    }
    if n <= k + 2 {
        println!("{}", val[n - 1]);
        return;
    }
    let pc = precalc::Precalc::new(k + 1000000);
    let mut nu = M::one();
    for (i, _) in val.iter_mut().enumerate() {
        nu *= M::from(n) - M::from(i + 1);
    }
    for (i, val) in val.iter_mut().enumerate() {
        *val *= nu * (M::from(n) - M::from(i + 1)).inv();
    }
    let mut de = M::one();
    for i in 1..=(k + 1) {
        de *= M::from(i);
    }
    for (i, val) in val.iter_mut().enumerate().rev() {
        *val *= de.inv();
        if i > 0 {
            de *= pc.inv(i);
        }
    ...
    let ans = val.into_iter().fold(M::zero(), |s, a| s + a);
    println!("{}", ans);
}
