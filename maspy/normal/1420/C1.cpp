...
struct Mono {
  ...
  using X = value_type;
  static X from_element(ll x) { return {0, x, -x, 0}; }
  static X op(X x, X y) {
    auto [xpp, xpn, xnp, xnn] = x;
    auto [ypp, ypn, ynp, ynn] = y;
    return {max(xpp + ypp, xpn + ynp), max(xpp + ypn, xpn + ynn),
            max(xnp + ypp, xnn + ynp), max(xnn + ynn, xnp + ypn)};
  }
  static constexpr X unit() { return {0, -INF, -INF, 0}; }
...
void solve() {
  LL(N, Q);
  VEC(ll, A, N);
  vc<Mono::value_type> seg_raw(N);
  FOR(i, N) seg_raw[i] = Mono::from_element(A[i]);
  SegTree<Mono> seg(seg_raw);

  auto calc = [&]() -> void {
    auto [a, b, c, d] = seg.prod_all();
    print(max(a, b));
  };
  ...
  FOR(Q) {
    ...
    --a, --b;
    swap(A[a], A[b]);
    seg.set(a, Mono::from_element(A[a]));
    seg.set(b, Mono::from_element(A[b]));
    auto e = seg.prod_all();
    calc();
  }
}
