...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  deque<ll> v;
  {
    ...
    rep(i,N){
      cin>>j;
      v.emplace_back(j);
      if(!se.insert(j).second)se.erase(j);
    }
    if(se.size()){
      print(-1);
      continue;
  ...
  rep(i,N/2){
    ...
    rep1(j,v.size()){
      if(v[j]==v[0])break;
      ins.emplace_back(v[j]);
    }
    sz+=2+ins.size();
    rep(j,2+ins.size())v.pop_front();
    for(auto x:ins){
      query.emplace_back(sz++,x);
      v.emplace_front(x);
    }
    szlis.emplace_back((ins.size()+1)*2);
  }
  print(query.size());
  for(auto[x,y]:query)print(x<<" "<<y);
  print(szlis.size());
  output(szlis);
}
}