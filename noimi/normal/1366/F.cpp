...
signed main() {
    ...
    d[0] = 0;
    vll ma(n);
    rep(i, n) for(auto e : g[i]) chmax(ma[i], e.cost);
    vector<pll> v;
    mint ans;
    rep(i, m) {
        vll nxt(n, -INF);
        rep(j, n) {
            for(auto e : g[j]) chmax(nxt[e], d[j] + e.cost);
        }
        d = nxt;
        rep(j, n) if(d[j] > 0) v.eb(ma[j], d[j] - (i + 1) * ma[j]);
        ans += *max_element(all(d));
    }
    UNIQUE(v);
    ConvexHullTrickAddMonotone<ll, false> cht;
    for(auto e : v) cht.add(e.fi, e.se);
    ll pre = m;
    ll A = 0, B = 0;
    vector<pll> P;
    for(auto e : cht.H) P.eb(e);
    for(auto& e : P) e.fi *= -1, e.se *= -1;
    for(auto [a, b] : P) {
        ll nxt = min(q, (B - b) / (a - A));
        if(nxt > pre) {
            ans += B * (nxt - pre);
            ans +=
                A * (nxt + pre + 1) % MOD * (nxt - pre) % MOD * ((MOD + 1) / 2);
            pre = nxt;
        }
        A = a, B = b;
    }
    if(pre < q) {
        ll nxt = q;
        ans += B * (nxt - pre);
        ans += A * (nxt + pre + 1) % MOD * (nxt - pre) % MOD * ((MOD + 1) / 2);
    }
    cout << ans << endl;
}