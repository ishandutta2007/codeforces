...
int solve() {
    ...
    for (auto& p : points) {
        ...
        while (!used[cur]) {
            used[cur] = true;
            cur = allPoints[cur].val;
            dsu.unite(cur, p.id);
        }
    }
    vector<pii> ans;
    int zero = points[0].id;
    for (int i = 1; i + 1 < points.size(); ++i) {
        int a = points[i].id;
        int b = points[i + 1].id;
        if (dsu.unite(a, b)) {
            ans.push_back({a, b});
            swap(allPoints[a].val, allPoints[b].val);
        }
    }
    for (int i = 1; i < points.size(); ++i) {
        int cur = allPoints[zero].val;
        ans.push_back({zero, cur});
        swap(allPoints[zero].val, allPoints[cur].val);
    }
    cout << ans.size() << endl;
    for (auto p : ans) {
