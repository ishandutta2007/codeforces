...
inline double calc(int t){
	return t+min((x-1.0*a*t)/b,(y-1.0*b*t)/a);	
}
void Solve(){
	x=read(),y=read(),a=read(),b=read();
	int l=0,r=min(x/a,y/b)+1,ans=0;
	while(r-l>10){
		int mid1=(1LL*l+l+r)/3;
		int mid2=(1LL*l+r+r)/3;
		double d1=calc(mid1);
		double d2=calc(mid2);
		ans=max(ans,(int)floor(max(d1,d2)));
		if(d1<=d2){
			l=mid1;
		}
		else{
