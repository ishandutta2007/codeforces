...
ff(x,0,k-1)
cin>>a[x].first,a[x].second=x+1,mx=max(mx,a[x].first);
int n=1;
while(n*n-(n/2)*(n/2)<m||(n+1)/2*n<mx)n++;
int ans[n+1][n+1];
me(ans,0);
sort(a.begin(),a.end(),[&](auto a,auto b){return a>b;});
while(a[a.size()-1].first==0)a.pop_back();
int qwq=0;
a.push_back({n*n,0});
for(int x=2;x<=n;x+=2)
for(int y=1;y<=n;y+=2)
ans[x][y]=a[qwq].second,a[qwq].first--,qwq+=(a[qwq].first==0);
for(int x=1;x<=n;x+=2)
for(int y=1;y<=n;y+=2)
ans[x][y]=a[qwq].second,a[qwq].first--,qwq+=(a[qwq].first==0);
for(int x=1;x<=n;x+=2)
