...
void solve(){
    INT(n);
    VEC(int, a, n);
    VV<> dp(n + 1, V<>(n + 1, 0)), dp1(n + 1, V<>(n + 1, 0)), dp2(n + 1, V<>(n + 1, 0));
    dp[0][0] = 1;
    REP(i, n){
        REPS(j, n) dp2[j][a[i]] += dp1[j][a[i]];
        REPS(j, n) dp1[a[i]][j] += dp[a[i]][j];
        REPS(j, n) dp[j][a[i]] += dp[j][0];
        dp[a[i]][0] += dp[0][0];
    }
    ll res = 0;
    REPS(i, n) REPS(j, n) res += dp2[i][j];
    print(res);
}
