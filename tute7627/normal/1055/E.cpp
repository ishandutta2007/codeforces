...
int main(){
  ...
  vector<ll>idx(n);
  rep(i,0,n)idx[ord[i]]=i;
  vector<ll>g(n,-1);
  rep(i,0,s){
    ll l,r;cin>>l>>r;l--;
    chmax(g[l],r);
  }
  rep(i,0,n-1)chmax(g[i+1],g[i]);
  ll ok=n,ng=-1;
  while(ok-ng>=2){
    ll mid=(ok+ng)/2;
    vector<ll>b(n);
    rep(i,0,n)b[i]=idx[i]<=mid;
    vector<ll>c(n+1);
    rep(i,0,n)c[i+1]=c[i]+b[i];
    auto dp=vec(n+1,m+1,0);
    rep(i,0,n)rep(j,0,m+1){
      chmax(dp[i+1][j],dp[i][j]);
      if(j==m||g[i]<=i)continue;
      chmax(dp[g[i]][j+1],dp[i][j]+c[g[i]]-c[i]);
    }
