...
fun main(){
    ...
    solve.cases{
        ...
        var ret = 0
        for(lights in 0..n){
            val fullzero = n + (k-1) - k.toLong() * lights
            if(fullzero < 0 ) continue
            val fz = fullzero.toInt()
            val counts = FACT.choose(fz + lights,lights)
            val odds = FACT.invchoose(n,lights)
            ret = ret modPlus (odds modM counts)
        }
        put(ret)
