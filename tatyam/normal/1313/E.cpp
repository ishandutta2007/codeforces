...
pll operator+=(pll& a, const pll& b){ a.first += b.first; a.second += b.second; return a; }
template<class T>
struct SegmentTree{
...
signed main(){
    LL(n,m);
    STR(a,b,s);
    auto A=Z_algorithm(s+a);
    b+=s;
    Rev(b);
    auto B=Z_algorithm(b);
    A=vector<ll>(A.end()-n,A.end());
    vector<ll>c(n);
    rep(n)c[i]=B[n+m-1-i];
    swap(B,c);
    each(i,A)chmin(i,m-1);
    each(i,B)chmin(i,m-1);
    ll end=0;
    RSQ<pll> seg(m+1);
    ll ans=0;
    rep(n){
        while(end<n&&end<i+m-1){
            seg.add(B[end],{1,B[end]});
            end++;
        }
        pll cnt=seg.get(m-A[i],seg.size);
        ans+=cnt.second-cnt.first*(m-A[i]-1);
        seg.add(B[i],{-1,-B[i]});
    }
    out(ans);
}