// Even Number Addicts (https://codeforces.com/contest/1738/problem/C)
...
int main() {
    array<array<array<array<bool, 2>, 2>, 101>, 101> dp{};
    dp[0][0][0][0] = true;
    dp[0][0][1][1] = true;
    rep(i, 101) rep(j, 101 - i) rep(k, 2) rep(x, 2) {
        if (i)
            dp[i][j][k][x] |= !dp[i - 1][j][k][(i + j) % 2 == k ? 1 - x : x];
        if (j)
            dp[i][j][k][x] |= !dp[i][j - 1][k][x];
    }
    ...
    while (t--) {
        ...
        println(dp[cnt[1]][cnt[0]][n % 2][0] ? "Alice" : "Bob");
    }
}