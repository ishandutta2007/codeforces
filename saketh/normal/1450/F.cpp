...
void solve() {
    ...
    int freq = max_element(all(ct)) - ct.begin();
    int ends = accumulate(all(ct), 0);

    int adjust = max(0, ct[freq] - (2 + ends - ct[freq]));

    int can_adjust = 0;
    F0R (i, N - 1) {
        if (a[i] != a[i + 1] && a[i] != freq && a[i + 1] != freq) {
            can_adjust++;
        }
    }

    if (adjust <= 2 * can_adjust) {
        ps(ops + adjust / 2);
    } else {
        ps(-1);
    }
