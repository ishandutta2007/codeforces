#line 1 "codeforces-workspace\\contest\\1609\\f\\f.cpp"
...
#line 13 "codeforces-workspace\\contest\\1609\\f\\f.cpp"
...
void main() {
  ...
  constexpr auto max_pop = 61;
  constexpr auto inf = (i64(1) << max_pop) - 1;

  auto ans = n;

  auto solve = [&](i32 left, i32 mid, i32 right) {
    {  // max, min : Left
      auto ma = -inf, mi = inf;
      for (auto i = mid, j = mid; i-- > left;) {
        chgr(ma, a[i]);
        chle(mi, a[i]);
        while (j < right and a[j] <= ma and a[j] >= mi) ++j;
        if (__popcount(mi) == __popcount(ma)) ans += j - mid;
      }
    }

    {  // max, min : Right
      auto ma = -inf, mi = inf;
      for (auto i = mid, j = mid; i < right; ++i) {
        chgr(ma, a[i]);
        chle(mi, a[i]);
        while (j > left and a[j - 1] < ma and a[j - 1] > mi) --j;
        if (__popcount(mi) == __popcount(ma)) ans += mid - j;
      }
    }

    {
      ...
      auto ma = -inf, mi = inf;
      auto tmp = inf;
      array<int, max_pop> cnt{};
      queue<i64> q;

      for (auto i = mid, j = mid; i-- > left;) {
        chgr(ma, a[i]);
        chle(mi, a[i]);
        while (j < right and a[j] <= ma) {
          chle(tmp, a[j]);
          q.push(tmp);
          cnt[__popcount(tmp)]++;
          ++j;
        }
        while (!q.empty() and q.front() >= mi) {
          cnt[__popcount(q.front())]--;
          q.pop();
        }
        ans += cnt[__popcount(ma)];
      }
    }

    {
      ...
      for (auto i = mid, j = mid; i-- > left;) {
        chgr(ma, a[i]);
        chle(mi, a[i]);
        while (j < right and a[j] >= mi) {
          chgr(tmp, a[j]);
          q.push(tmp);
          cnt[__popcount(tmp)]++;
          ++j;
        }
        while (!q.empty() and q.front() <= ma) {
          cnt[__popcount(q.front())]--;
          q.pop();
        }
        ans += cnt[__popcount(mi)];
      }
    }
  };

  queue<pair<int, int>> dc;
  for (auto i : range(n)) {
    dc.emplace(i, i + 1);
  }

  while (dc.size() > 1) {
    const auto [l, m] = dc.front();
    dc.pop();
    if (m == n) {
      dc.emplace(l, m);
      continue;
    }
    const auto r = dc.front().second;
    dc.pop();
    dc.emplace(l, r);
    solve(l, m, r);
  }
