...
void bfs(ll id)
{
	rep(y, 1, h) rep(x, 1, w) dist[id][x][y] = inf;
	
	queue<P> Q;
	rep(y, 1, h) rep(x, 1, w) if(c[x][y] == '1'+id){
		dist[id][x][y] = 0, Q.push(P(x, y));
	}
	...
	while(sz(Q)){
		...
		rep(i, 0, 3){
			...
			if(c[nx][ny] == '#') continue;
			if(dist[id][nx][ny] < inf/2) continue;
...
int main(void)
{
	...
	rep(y, 1, h) rep(x, 1, w){
		if(c[x][y] != '.') continue;
		chmin(ans, dist[0][x][y]+dist[1][x][y]+dist[2][x][y]-2);
	}
	rep(i, 0, 2){
		ll d[3] = {(ll)inf, (ll)inf, (ll)inf};
		rep(y, 1, h) rep(x, 1, w) if(c[x][y] == '1'+i){
			rep(j, 0, 2) chmin(d[j], dist[j][x][y]);
		}
		chmin(ans, d[0]+d[1]+d[2]-2);
	}
	if(ans > inf/2) ans = -1;
	outl(ans);
