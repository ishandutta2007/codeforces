...
void solve() {
  ...
  vector<pair<long long, long long>> towers(m);
  rep(i, 0, m) {
    cin >> towers[i].first >> towers[i].second;
  }
  sort(towers.begin(), towers.end());
  for (int i = 1; i < m; ++i) {
    towers[i].second += towers[i - 1].second;
  }

  for (auto min_power : strength) {
    auto it = upper_bound(towers.begin(), towers.end(), make_pair(min_power, ll_inf));
    if (it == towers.begin())
      cout << 0 << ' ';
    else
      cout << (--it)->second << ' ';
