...
int main() {
  int n, q; cin >> n >> q;
  vector<set<int>> r_rook(n), c_rook(n);
  IntervalManagedBySet<int> row, col;
  while (q--) {
    int t; cin >> t;
    if (t == 3) {
      int y1, x1, y2, x2; cin >> y1 >> x1 >> y2 >> x2; --y1; --x1; --y2; --x2;
      cout << (row.contains(y1, y2) || col.contains(x1, x2) ? "Yes\n" : "No\n");
      // REP(i, n) cout << (row.contains(i) ? 'o' : 'x');
      // cout << ' ';
      // REP(i, n) cout << (col.contains(i) ? 'o' : 'x');
      // cout << '\n';
    } else {
      int y, x; cin >> y >> x; --y; --x;
      if (t == 1) {
        if (r_rook[y].empty()) row.insert(y);
        r_rook[y].emplace(x);
        if (c_rook[x].empty()) col.insert(x);
        c_rook[x].emplace(y);
      } else if (t == 2) {
        r_rook[y].erase(x);
        if (r_rook[y].empty()) row.erase(y);
        c_rook[x].erase(y);
        if (c_rook[x].empty()) col.erase(x);
      }
    }
  }
  return 0;
