...
class edge(val a:Int, val b:Int, var ret:Int?)
...
fun main(){
    solve.cases{
        ...
        repeat(n-1){
            ...
            val b = getint() - 1
            val c = getint()
            val cc = if(c == -1) null else c
            alledges.add(edge(a,b,cc))
        }

        for(e in alledges){
            if(e.ret != null){
                val dif = (e.ret!!.countOneBits() %2 == 1)
                dsu.union(e.a,e.b,dif)
            }
        }
        ...
        for(i in 0 until n){
            val result = dsu.find(i)
            ret[i] = result.second
        }

        for(e in alledges){
            if(e.ret == null){
                val dif = ret[e.a] != ret[e.b]
                if(dif){
                    e.ret = 1
                }else{
                    e.ret = 0
