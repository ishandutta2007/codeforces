...
void solve() {
  ...
  vi A0(N), A1(N);
  FOR(i, N) if (P[i] < 100) A0[i] = 1;
  FOR(i, N) if (P[i] > 0) A1[i] = 1;
  A0 = cumsum(A0);
  A1 = cumsum(A1);

  const ll INF = 1LL << 60;
  vv(ll, dist, N + 1, N + 1, INF);
  dist[0][1] = 0;
  queue<pi> que;
  que.push(mp(0, 1));
  ...
  while (!que.empty()) {
    ...
    auto add = [&](ll a, ll b) -> void {
      if (chmin(dist[a][b], dist[i][j] + 1)) que.push(mp(a, b));
    };

    // i, j 
    if (P[i] > 0 && A1[N] - A1[j] > 0) {
      if (j + 1 == N)
        add(N, N);
      else
        add(j + 1, j + 2);
    ...
    // i 
    if (A1[N] - A1[j] > 0 && P[i] < 100) { add(j, j + 1); }
    // j 
    if (P[i] > 0 && A0[N] - A0[j] == N - j) { add(i, j + 1); }
  }
  ll ANS = 0;
  FOR(i, N + 1) FOR(j, N + 1) if (dist[i][j] <= K)++ ANS;
  print(ANS);
}
