...
int main(){
	...
	REP(i, 1, 200) REP(j, 1, 200) if ((i + 1) * (j + 1) == res) li.push_back({i, j});
	rep(t, 3){
		ll mi = 1e9, p;
		REP(np, 2, 201){
			map<ll, ll> cnt;
			for(auto e: li){
				ll ni = e.first, nj = e.second;
				ll v1 = (ni + 1) / np * (nj + 1), v2 = (ni + np) / np * (nj + 1);
				cnt[v1]++;
				if(v1 != v2)
					cnt[v2]++;
			}
		...
		cout << endl;
		cin >> res;
		vector<pair<ll, ll>> nli;
		for(auto e: li){
			ll ni = e.first, nj = e.second;
			ll v1 = (ni + 1) / p * (nj + 1), v2 = (ni + p) / p * (nj + 1);
			if(v1 == res || v2 == res)
				nli.pb(e);
		}
		li = nli;
	}
	cout << "! " << li[0].first * 2 + li[0].second * 2 << endl;
}