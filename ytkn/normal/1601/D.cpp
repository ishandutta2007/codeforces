...
int main(){
    ...
    int m = cp.size();
    vector<vector<int>> alpinists(m);
    d = cp[d];
    for(int i = 0; i < n; i++){
        a[i] = cp[a[i]];
        s[i] = cp[s[i]];
        if(s[i] < d) continue;
        alpinists[a[i]].push_back(s[i]);
    }
    for(int i = 0; i < m; i++) sort(alpinists[i].begin(), alpinists[i].end(), greater<int>());
    vector<int> dp(m);
    for(int i = 0; i < n; i++){
        if(a[i] <= d && s[i] >= d){
            dp[d]++;
    ...
    int ans = dp[d];
    // debug_value(ans);
    vector<vector<int>> erases(m);
    for(int i = d+1; i < m; i++){
        int cnt_ok = 0;
        for(int s: alpinists[i]){
            int r = min(i-1, s)+1;
            if(r > d){
                int tmp = seg.prod(d, r)+cnt_ok+1;
                chmax(dp[i], tmp);
                chmax(ans, tmp);
            }
            if(s >= i) cnt_ok++;
        ...
        for(int s: alpinists[i]){
            if(s >= i){
                seg.apply(0, i, 1);
                erases[s].push_back(i);
            }
        }
        seg.set(i, dp[i]+seg.get(i));
        for(int j: erases[i]){
            seg.apply(0, j, -1);
        }
    }
