...
LL a[N];
LL b[N];
bool myru(LL x,LL y){return x>y;}

bool check(int x)
{
    int les=n+x-(n+x)/4;
    LL s_a=(les>=x)?(1LL*x*100+a[les-x]):(1LL*les*100);
    LL s_b=b[min(les,n)];
    return s_a>=s_b;
}
void work()
{
    rin>>n;
    for(int i=1;i<=n;i++)rin>>a[i];sort(a+1,a+n+1,myru);
    for(int i=1;i<=n;i++)rin>>b[i];sort(b+1,b+n+1,myru);
