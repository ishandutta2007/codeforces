...
#line 2 "C:/Users/W/c/lib/math/factorize.cpp"

#line 4 "C:/Users/W/c/lib/math/factorize.cpp"
...
int main() {
  int n = IN; auto a = IN.vi(n);
  vector<map<ll, int>> ps(n);
  rep(i, n) ps[i] = factorize(a[i]);
  rep(i, n) dump(a[i], ps[i]);
  map<ll, vector<int>> pk;
  for (auto&& pnk : ps) {
    for (auto [p, k] : pnk) {
      pk[p].push_back(k);
    }
  ...
  for (auto&& [p, ar] : pk) {
    if (ar.size() == n) {
      nth_element(ar.begin(), ar.begin()+1, ar.end());
      res *= power(p, ar[1]);
    } else if (ar.size() == n-1)
      res *= power(p, *min_element(all(ar)));
  }
  OUT(res);
}