...
fun main(){
    solve.run{
        ...
        for(i in 0..26){
            val maxs = B.indices.filter{B[it] == 0}
            ret.add(maxs)
            for(i in maxs){
                for(j in 0 until m){
                    B[j] -= (i-j).absoluteValue
                }
            }
            for(i in maxs){
                B[i] = -1
...
//            just dei B
        ...
        val retstr = IntArray(m)
        var pointer = 0
        for(i in 25 downTo 0){
            if(pointer >= ret.size){
                break
            }
            if(countArray[i] >= ret[pointer].size){
                for(a in ret[pointer]){
                    retstr[a] = i
                }
                pointer ++
            }
        }
        put(retstr.map{('a'.toInt() + it).toChar()}.joinToString(""))
...
general rule : be more careful and delet things ,
