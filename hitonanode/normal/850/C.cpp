...
// using ump = gp_hash_table<int, vector<int>, custom_hash>; // fast unordered_set / unordered_map

constexpr int D = 20;
array<int, 1 << D> grundy;
...
{
    if (S == 1) return false;
    if (S < grundy.size()) return grundy[S];
    vector<int> V;
    FOR(d, 1, 30) if (S >= 1 << d)
    {
        V.emplace_back(rec_solve(nxt(S, d)));
    }
    return mex(V);
}
...
int main()
{
    ...
    REP(S, 1 << D) if (S % 2)
    {
        ...
        else
        {
            ...
            FOR(d, 1, D) if (S >= 1 << d)
            {
                vec.emplace_back(grundy[nxt(S, d)]);
            }
            grundy[S] = mex(vec);
        }
    }
    ...
    int XOR = 0;
    for (auto p : mp) XOR ^= solve(p.second);
    dbg(XOR);
    cout << (XOR ? "Mojtaba" : "Arpa") << '\n';
}