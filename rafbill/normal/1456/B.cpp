...
int main(){
  ...
  for(li i = 0; i < (li)(100); ++i) if(cnt[i]>=3) {
    cout << 1 << endl;
    return 0;
  }

  vector<tuple<li,li,li>> V;

  for(li l = 0; l < (li)(n); ++l) for(li r = (l); r <= (li)(n-1); ++r) {
    li x = 0;
    for(li i = (l); i <= (li)(r); ++i) x ^= A[i];
    V.emplace_back(x,l,r);
  }
  sort(begin(V), end(V));
  ...
  li ans = -1;
  for(auto p : V) {
    li x,l,r; tie(x,l,r) = p;
    li sz = (r-l+1);
    for(li i = (r+1); i <= (li)(n-1); ++i) if(B[i] != -1) {
      li lans = sz + B[i]-2;
      if(ans==-1 || lans<ans)ans=lans;
    }
    if(B[l]==-1 || sz < B[l]) B[l] = sz;
  }
  cout << ans << endl;
