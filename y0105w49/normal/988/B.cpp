...
bool ss(const string &s,const string &t) {
	if(sz(s)>sz(t)) return 0;
	for(int i=0;i+sz(s)<=sz(t);i++)
		if(t.substr(i,sz(s))==s) return 1;
	return 0;
}
...
int32_t main() { // CURSOR START
	...
	sort(s,s+n,[](const string &a,const string &b){return sz(a)<sz(b);});
	for(int i=1;i<n;i++) if(!ss(s[i-1],s[i])) return cout<<"NO"<<endl,0;
	cout<<"YES"<<endl;
