...
int main() {
	CASE(t) {
		...
		vector<int> len;
		for (auto& i : sieve.factorize(n))
			len.emplace_back(n / i.first);

		const int m = (int)len.size();
		vector<vector<ll>> sum(m);
		for (int i = 0; i < m; i++) {
			sum[i].resize(len[i], 0);
			for (int j = 0; j < n; j++)
				sum[i][j % len[i]] += a[j];
		}
		...
		for (int i = 0; i < m; i++)
			for (int j = 0; j < len[i]; j++)
				ans.push(sum[i][j % len[i]] * len[i]);
		
		auto query = [&]() {
			while (!lazy.empty() && ans.top() == lazy.top()) {
				ans.pop();
				lazy.pop();
			}
			return ans.top();
		...
		auto upd = [&](int idx, ll v) {
			ll d = v - a[idx];
			a[idx] = v;

			for (int i = 0; i < m; i++) {
				lazy.push(sum[i][idx % len[i]] * len[i]);
				sum[i][idx % len[i]] += d;
				ans.push(sum[i][idx % len[i]] * len[i]);
			}
		};
