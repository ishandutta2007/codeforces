...
void solve() {
  ...
  VEC(ll, X, Q);
  VEC(ll, Y, Q);
  FOR(i, N) { tie(A[i], C[i]) = mp(A[i] + C[i], A[i] - C[i]); }
  ...
  FOR(q, Q) {
    ll x = X[q], y = Y[q];
    FOR(i, N) {
      if (A[i] <= x + y && C[i] <= x - y && x <= B[i]) ++ANS[q];
    }
  }
  print(ANS);
  */
  Fenwick2D<Group_Add<int>, false> bit(A, C);
  auto I = argsort(B);
  reverse(all(I));
  auto J = argsort(X);
  reverse(all(J));
  ...
  FOR(q, Q) {
    ...
    const ll INF = 1LL << 60;
    // print("calc", x + y + 1, x - y + 1);
    ANS[qid] = bit.sum(-INF, -INF, x + y + 1, x - y + 1);
  }
  print(ANS);
}
