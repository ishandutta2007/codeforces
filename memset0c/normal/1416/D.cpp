...
std::priority_queue<int,std::vector<int>,cmp> f[N],g[N];
...
pair<vector<int>,int> merge(int u,int v){
  u=find(u),v=find(v);
  if(u==v)return {{},-1};
  if(ele[u].size()>ele[v].size())std::swap(u,v);
  ele[v].insert(ele[v].end(),ele[u].begin(),ele[u].end());
  anc[u]=v;
  return {ele[u],u};
}
...
int main(){
  ...
  for(int i=1;i<=q;i++){
    read(y[i].o),read(y[i].x);
    if(y[i].o==2)mrk[y[i].x]=true;
  }
  for(int i=1;i<=n;i++){
    anc[i]=i;
    ele[i].push_back(i);
  ...
  for(int i=q;i>=1;i--){
    if(y[i].o==2){
      dat[i]=merge(e[y[i].x].u,e[y[i].x].v);
    }
    // printf("%d : ",i);
  ...
  for(int i=1;i<=q;i++){
    ...
    // for(int i=1;i<=n;i++)printf("%d%c",find(i)," \n"[i==n]);
    if(y[i].o==1){
      int ans=pop(find(y[i].x));
      vis[ans]=true;
      print(p[ans],'\n');
    }else{
      int src=find(e[y[i].x].u);
      for(int x:dat[i].first){
        anc[x]=dat[i].second;
        if(!vis[x]){
          f[dat[i].second].push(x);
          g[src].push(x);
        }
      }
    }
