...
int main() {
    ...
    auto go = [&](auto& self, int loc, int par) -> array<mn, 3> {
        mn dont = 1;

        vmn ch, chabv;
        trav (nbr, adj[loc]) if (nbr != par) {
            auto [u, d, a] = self(self, nbr, loc);
            dont *= u + d;
            ch.pb(2 * u + d);
            chabv.pb(a);
        }

        vmn pref = prefixes(ch, mn(1), multiplies<mn>());
        vmn suff = suffixes(ch, mn(1), multiplies<mn>());

        mn use = 0;
        F0R (i, sz(ch)) use += pref[i] * suff[i+1] * chabv[i];

        return {use, dont, pref.back()};
    };

    auto [u, d, a] = go(go, 0, 0);
    ps(u + d);
