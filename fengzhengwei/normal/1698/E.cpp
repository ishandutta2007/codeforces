...
struct pr{int x,y;};
int n,m,k,q,a[xx],b[xx],A[xx],B[xx];
int s;
int ty[xx],suf[xx];
int main(){
	int T=read();
	while(T--)
	{
		...
		for(int i=1;i<=n;i++)if(B[i]!=-1)b[B[i]]=i;
//		for(int i=1;i<=n;i++)a[i]=read();
//		for(int i=1;i<=n;i++)b[i]=read();
		for(int i=1;i<=n;i++)ty[i]=0; 
		for(int i=1;i<=n;i++)if(b[i]!=-1)ty[b[i]]=i;
		int cr=1;
		for(int i=1;i<=n;i++)
		{
			if(ty[a[i]])
...
//		cerr<<cr<<"$$\n";
		...
		vector<int>v;
		for(int i=1;i<=n;i++)
			if(!ty[a[i]])v.push_back(max(1,i-s));
		ll ans=1;
//		for(auto it:v)cout<<it<<"@\n";
		suf[n+1]=0;
		for(int i=n;i>=1;i--)suf[i]=suf[i+1]+(b[i]==-1);
