...
double query(vector<Q> vec, bool b)
{
	if(b) for(auto &q : vec) swap(q.fi, q.se);
	outl("?", sz(vec));
	for(auto q : vec) printf("%.11f %.11f\n", q.fi, q.se);
...
double calc(bool b)
{
	...
	vec.pb(Q(105, -2));
	vec.pb(Q(1, -2));
	ll p = floor(query(vec, b)*1000+eps);
	...
	vec.pb(Q(101, 101));
	vec.pb(Q((1-B)*p, 101));
	return query(vec, b) - B + (1-B)*(p-1);
}

int main(void)
{
	cin >> w >> h;
	double xans = calc(false);
	double yans = calc(true);
