...
//        DBN(p, s, k / val);
    ...
    for (int i = 0; i < res.size(); ++i) {
        ...
        for (int dir = 0; dir < 4; ++dir) {
            ...
            int toy = y + dy[dir];
            if (!check(tox, toy)) continue;
            if (used2[tox][toy]) continue;
            if (!used[tox][toy]) continue;
            res.push_back({tox, toy});
            used2[tox][toy] = true;
        }
    }
    FI(n) FJ(m) {
    ...
    if (res.size() > k / ansval) {
        res.resize(k / ansval);
    }
    for (auto p : res) {
        a[p.first][p.second] = ansval;
    }
    FI(n) {
        FJ(m) {
            printf("%d ", a[i][j]);
