...
int main() {
	...
	int n, mod;
	cin >> n >> mod;
	Graph g(n, -1, true, false, 0);

	constexpr ll ten = 10;
	const ll ten_inv = modinv(ten, mod);
	const int n3 = n * 5;
	const int mid = n * 2;
	V<ll> power_ten_memo(n3);
	power_ten_memo[mid] = 1;
	rep(i, mid, n3 - 1) { power_ten_memo[i + 1] = power_ten_memo[i] * ten % mod; }
	drep(i, mid) { power_ten_memo[i] = power_ten_memo[i + 1] * ten_inv % mod; }
	auto power_ten = [&](int upper_right) { return power_ten_memo[upper_right + mid]; };
	vector<ll> dn(n), up(n);
	map<ll, int> up_tmp, dn_tmp;
	vector<int> remain(n, 1), subtree_size(n);

	auto dfs = [&](auto self, int now, int par) -> void {
		subtree_size[now] = 1;
		foa(e, g[now]) {
			if(e.to != par && remain[e.to]) {
				self(self, e.to, now);
	...
	auto calc = [&](auto self, int now, int height, int par, ll preedgecost) -> void {
		dn[now] = (dn[par] * ten + preedgecost) % mod;
		up[now] = (up[par] + power_ten(height - 1) * preedgecost) % mod;
		// dns[id][(mod - dn[now]) * power_ten(-height) % mod]++;
		// ups[id][up[now]]++;
		up_tmp[up[now]]++;
		dn_tmp[(mod - dn[now]) * power_ten(-height) % mod]++;

		foa(e, g[now]) {
			if(par == e.to) continue;
			if(!remain[e.to]) continue;
			self(self, e.to, height + 1, now, e.cost);
		}
		return;
	};

	debug(power_ten_memo);

	ll ans = 0;
	auto centroid_decomposition = [&](auto self, const int start_point) -> void {
		debug(remain);
		dfs(dfs, start_point, -1);
		int center = start_point;
		{
			const int whole_size = subtree_size[start_point];
			debug(whole_size);
			int par = -1;
			bool flag = true;
			while(flag) {
				flag = false;
				foa(e, g[center]) {
					if(remain[e.to] && e.to != par && subtree_size[e.to] * 2 > whole_size) {
						par = center;
						center = e.to;
						flag = true;
						break;
		...
		dn[center] = up[center] = 0LL;
		debug(center, subtree_size);

		map<ll, int> upsum, dnsum;
		foa(e, g[center]) {
			if(remain[e.to]) {
				up_tmp.clear();
				dn_tmp.clear();
				calc(calc, e.to, 1, center, e.cost);

				foa(t, up_tmp) {
					upsum[t.first] += t.second;
					if(!t.first) ans += t.second;
				}
				foa(t, dn_tmp) {
					dnsum[t.first] += t.second;
					if(!t.first) ans += t.second;
					if(up_tmp.count(t.first)) ans -= ll(t.second) * up_tmp[t.first];
				}
			}
		}

		foa(t, dnsum) {
			if(upsum.count(t.first)) ans += ll(t.second) * upsum[t.first];
		}

		remain[center] = 0;
		foa(e, g[center]) {
			if(remain[e.to]) self(self, e.to);
		}
