...
void solve() {
  ...
  vector<pll> inserts(c);
  vector<ll> lens = {n};
  for (auto& [l, r] : inserts) {
    cin >> l >> r;
    --l;
    lens.pb(lens.back() + (r - l));
  }

  auto answer = YPredicate([n, inserts, lens, s](auto& self, ll pos) -> char {
    if (pos < n) {
      return s[pos];
    }
    auto it = upper_bound(all(lens), pos) - 1;
    int idx = int(it - lens.begin());
    return self(pos - lens[idx] + inserts[idx].first);
  });
