...
int main() {
    ...
    int evens = 0, odds = 0;
    vector< vi > even_parts(p);
    vector< vi > odd_parts(k-p);
    vi rem;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (x % 2 == 1) {
            ...
            if (p > 0) {
                even_parts[evens % p].push_back(x);
            }
            else odd_parts[0].push_back(x);
            evens++;
        }
    }
    ...
    while (!rem.empty()) {
        ...
        if (p > 0) {
            even_parts[evens % p].push_back(rem.back());
            rem.pop_back();
            even_parts[evens % p].push_back(rem.back());
            rem.pop_back();
            evens++;
            ...
            odd_parts[0].push_back(rem.back());
            rem.pop_back();
            odd_parts[0].push_back(rem.back());
            rem.pop_back();
        }
    ...
    for (int i = 0; i < k-p; i++) if (odd_parts[i].empty()) {
        cout << "NO" << endl;
        return 0;
    }
    for (int i = 0; i < p; i++) if (even_parts[i].empty()) {
        cout << "NO" << endl;
        return 0;
    }
    ...
    cout << "YES" << endl;
    for (int i = 0; i < p; i++) {
        cout << sz(even_parts[i]);
        for (int x : even_parts[i]) cout << " " << x;
        cout << '\n';
    }
    for (int i = 0; i < k-p; i++) {
        cout << sz(odd_parts[i]);
        for (int x : odd_parts[i]) cout << " " << x;
        cout << '\n';
    }
