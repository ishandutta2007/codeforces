...
void solve(int tc) {
	...
	vi A(N); re(A);
	F0R(i,N) id[A[i]] = i;
	Dinic<int> D; D.init(4*N+2);
	F0R(i,2*N) {
		D.ae(4*N,i,1);
		D.ae(i+2*N,4*N+1,1);
	}
	auto bip = [&](int x, int y) {
		D.ae(x,y+2*N,1);
	};
	F0R(i,N) bip(i,i+N);
	F0R(i,N) {
		for (int _j: divi[A[i]]) {
			int j = id[_j]; if (j == -1) continue;
			// dbg("OH",j,i);
			bip(j,i);
			bip(j,i+N);
			bip(j+N,i+N);
	...
	ps(D.maxFlow(4*N,4*N+1)-N);
	F0R(i,N) id[A[i]] = -1;
}

int main() {
	setIO();
	FOR(i,1,50001) id[i] = -1;
	FOR(i,1,50001) for (int j = 2*i; j <= 50000; j += i) divi[j].pb(i);
