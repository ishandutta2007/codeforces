...
int main() {
    ...
    for (int start = 0; start < 2; start++) {
        array<vector<pii>, 2> candies{};
        for (auto [t, h, m] : input)
            candies[t == start].push_back({h, m});
        ...
        array<priority_queue<int>, 2> queues{};
        while (true) {
            auto &q = queues[score % 2];
            auto &c = candies[score % 2];

            while (!c.empty() && c.back().first <= height) {
                q.push(c.back().second);
                c.pop_back();
            }
