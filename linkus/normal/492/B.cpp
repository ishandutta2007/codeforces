...
inline bool ok(long double cur){
    FOR(i,1,n){
        if(arr[i-1] + cur < arr[i] - cur)
            return false;
    }
    return true;
}
...
int main(){
    ...
    long double low = ans = max(arr[0],l-arr[n-1]);
    long double high = l;
    bool chg = 0;
    /* for(double x = n; x >= 0.000000001; x /= 2){ */
    /*     while(!ok(ans + x)) {ans += x; chg = 1;} */
    /* } */
    long double mid;
