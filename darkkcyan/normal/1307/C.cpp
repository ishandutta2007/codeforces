...
string s;
llong cur_alpha_cnt[26] = {0};
llong cnt_pair_alpha[26][26] = {0};
int main(void) {
    ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0);
    cin >> s;
    for (auto c: s) {
        int i = c - 'a';
        rep(before, 26) cnt_pair_alpha[before][i] += cur_alpha_cnt[before];
        cur_alpha_cnt[i]++;
    }

    llong ans = 0;
    rep(u, 26) {
        ans = max(ans, cur_alpha_cnt[u]);
        rep(v, 26) ans = max(ans, cnt_pair_alpha[u][v]);
    }
    cout << ans;
