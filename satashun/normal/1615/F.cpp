...
void slv() {
    ...
    V<int> vs(n, -1), vt(n, -1);
    V<int> ps(n + 1), pt(n + 1), ws(n + 1), wt(n + 1);
    ...
    rep(i, n) {
        rep(j, n) {
            if (vs[i] == 0 || vt[j] == 0) continue;
            Mint vl, vr;
            {
                int p = ps[i], q = pt[j], a = ws[i], b = wt[j];
                /*
                rep(c1, a + 1) {
                    rep(c2, b + 1) {
                        if (c1 + p == c2 + q) {
                            vl += comb(a, c1) * comb(b, c2);
                        }
                    }
                }*/
                vl = comb(a + b, b + q - p);
            ...
            {
                int p = ps[n] - ps[i + 1], q = pt[n] - pt[j + 1],
                    a = ws[n] - ws[i + 1], b = wt[n] - wt[j + 1];
                /*
            rep(c1, a + 1) {
                rep(c2, b + 1) {
                    if (c1 + p == c2 + q) {
                        vr += comb(a, c1) * comb(b, c2);
                    }
                }
            }*/
                vr = comb(a + b, b + q - p);
