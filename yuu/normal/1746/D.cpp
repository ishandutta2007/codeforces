...
int64_t f[200001][2];
int64_t s[200001];
void solve(int u, int low, int high) {
    assert(high - low <= 1);
    l[u] = low;
    for (auto&& v : g[u]) solve(v, low / g[u].size(), (high / g[u].size()) + ((high % g[u].size()) != 0));
    for (int i = low; i <= high; i++) {
        auto&& ans = f[u][i - low];
        ans = s[u] * i;  // score for this vertex
        if (g[u].empty()) continue;
        int md = i % g[u].size();
        int val_low = i / g[u].size();
        int val_high = (i / g[u].size()) + (md != 0);
        static vector<int64_t> best;
        best.clear();
        for (auto&& v : g[u]) {
            ans += f[v][val_high - l[v]];
            best.push_back(f[v][val_low - l[v]] - f[v][val_high - l[v]]);
        }
        md = g[u].size() - md;  // number of low
