...
int n,m;
int c[510];
modint<mod> dp[510][510],dp2[510][510];
bool vis[510][510];
...
void solve(int x,int y) {
    ...
    int p=min_element(c+x,c+y)-c;
    dp[x][y]=dp2[x][p]*dp2[p+1][y];
    for(int k=x; k<=y; ++k) {
        dp2[x][y]+=dp[x][k]*dp[k][y];
    }
}
...
signed main() {
    ...
    rep(i,n+1) dp[i][i]=dp2[i][i]=1;
    solve(0,n);
    // rep(i,n+1) {
    //     rep(j,i+1) {
    //         dump(j,i,dp[j][i],dp2[j][i],vis[j][i]);
    //     }
    // }
    cout<<dp[0][n]<<"\n";
}