...
void main_() {
	...
	if(SZ(t) and t.back() > a + b + c) {
		print(-1);
		return;
	}

	//a + b + c
	while(SZ(t) and t.back() > b + c) {
		res++;
		t.pop_back();
	}
	int a_add = 0, b_add = 0, c_add = 0;
	//b + c
	while(SZ(t) and t.back() > a + c) {
		a_add++;
		res++;
		t.pop_back();
	}
	//a + c
	while(SZ(t) and t.back() > max(a + b, c)) {
		b_add++;
		res++;
		t.pop_back();
	}
	...
	if(a + b > c) {
		...
		int cur = min(b_need, b_add);
		b_need -= cur, b_add -= cur;
		cur = min(a_need, a_add);
		a_need -= cur, a_add -= cur;
		if(b_need == 0 and b_add) {
			cur = min(b_add, a_need);
			a_need -= cur, b_add -= cur;
		}
		int ab_add = 0;
		while(SZ(t) and t.back() > b) {
			if(c_add) {
				c_add--;
			} else if(ab_add) {
				ab_add--;
