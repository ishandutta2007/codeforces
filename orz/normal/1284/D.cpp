...
struct event
{
	int a, n;
	bool start;
	event(const int a, const int n, const bool start)
		: a(a), n(n), start(start) {}
...
int another_main()
{
	...
	{
		int cc = ev[x].a;
		while ((x < sz(ev)) && (cc == ev[x].a))
		{
			if (ev[x].start)
			{
				mxs.insert(v[ev[x].n].eb);
				mns.insert(v[ev[x].n].sb);
			}
			else
			{
				auto mx = mxs.find(v[ev[x].n].eb);
				mxs.erase(mx);
				auto mn = mns.find(v[ev[x].n].sb);
				mns.erase(mn);
			}
			++x;
		}
	...
	while (x < sz(ev))
	{
		...
		while ((x < sz(ev)) && (cc == ev[x].a))
		{
			if (ev[x].start)
			{
				mxs.insert(v[ev[x].n].ea);
				mns.insert(v[ev[x].n].sa);
			}
			else
			{
				auto mx = mxs.find(v[ev[x].n].ea);
				mxs.erase(mx);
				auto mn = mns.find(v[ev[x].n].sa);
				mns.erase(mn);
			}
			++x;
		}
