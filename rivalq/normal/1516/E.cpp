...
int solve(){
 		...
 		function<vector<mod_int>(int)> eval = [&](int n){
 			if(n == 1){
 				return vector<mod_int>{1,1};
 			}
 			else{
 				vector<mod_int>f = {1};
 				if(n & 1){
 				...
 				vector<mod_int> c = eval(n/2);
 				vector<mod_int>c2(k+1);
 				mod_int m = n/2;
 				for(int i = 0; i < c.size(); i++){
 					mod_int val = 1;
 					mod_int val2 = 1;
 					for(int j = i; j <= k; j++){
 						c2[j] += c[i]*val*val2*inv[j - i];
 						val2 *= m;
 						val *= (m - j);
 					}
 				}
