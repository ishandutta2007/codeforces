...
int main() {
    ...
    vll t(n + 1), s(n + 1);
    // vi v(n);
    auto dfs = [&](auto &&f, int l, int r, int i) -> vi {
        if(!i) { return {a[l]}; }
        vi a = f(f, l, l + r >> 1, i - 1), b = f(f, l + r >> 1, r, i - 1);
        for(auto e : a) s[i] += lb(b, e);
        for(auto e : a) t[i] += si(b) - ub(b, e);
        for(auto e : b) a.eb(e);
        inplace_merge(a.begin(), a.begin() + (1 << i - 1), a.end());
        return move(a);
    };
    dfs(dfs, 0, N, n);
    INT(Q);
