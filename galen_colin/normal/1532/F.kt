...
*/
    ...
    fun solve(tc: Int) {
        val n = `in`!!.nint()
        val cnt = IntArray(n)
        val inp = arrayOfNulls<String>(2 * n - 2)
        val a = arrayOfNulls<String>(n)
        val b = arrayOfNulls<String>(n)
        for (i in 0 until 2 * n - 2) {
            val s = `in`!!.nstr()
            inp[i] = s
            val l = s.length
            if (cnt[l] == 0) a[l] = s else b[l] = s
            ++cnt[l]
        }
        for (it in 0..1) {
            if (n == 2 || a[n - 1]!!.substring(1, n - 1) == b[n - 1]!!.substring(0, n - 2)) {
                val check = a[n - 1]!!.substring(0, 1) + b[n - 1]
                var pos = true
                for (l in 1 until n) {
                    if (a[l] != check.substring(0, l) || b[l] != check.substring(n - l, n)) {
                        val t = a[l]
                        a[l] = b[l]
                        b[l] = t
                    }
