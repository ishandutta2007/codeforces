// A Perfectly Balanced String? (https://codeforces.com/contest/1673/problem/B)
...
int main() {
    ...
    while (t--) {
        ...
        int n = sz(s);
        vector cnts(n + 1, array<int, 26>{});
        rep(i, n)
            cnts[i + 1] = cnts[i], cnts[i + 1][s[i] - 'a']++;
        ...
        rep(i, n) {
            ...
            if (prev[s[i] - 'a'] != -1)
                rep(j, 26)
                    if (has[j] && cnts[i + 1][j] - cnts[p][j] == 0)
                        bal = false;
            prev[s[i] - 'a'] = i;
        }
