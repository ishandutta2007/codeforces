...
void solve()
{
	...
	auto go = [&]() -> ll
	{
		...
		for (int i = 0; i <= n; ++i)
			...
			{
				ll L = s[i] + c[i] * (-k);
				ll R1 = -(s[n] - s[j]) + (c[n] - c[j]) * (-k);
				ll R2 = -(s[n] - s[j]) + (c[n] - c[j]) * k;
				ll m1 = (s[j] - s[i]) + (c[j] - c[i]) * (-k);
				ll m2 = (s[j] - s[i]) + (c[j] - c[i]) * k;
				m1 += L, m2 += L;
				if (R1 > m1) swap(R1, m1), swap(R2, m2);
				if (m1 <= R2)
					freshmax(res, min(m2, R2) - L + 1);
				//if ((c[j] - c[i]) * (-k) <= R - L - m && R - L - m <= (c[j] - c[i]) * (k))
					//freshmax(res, min(max(-L, 0LL) + max(R, 0LL), m) + 1);
			}
		return res;
	};
