#line 1 "codeforces-workspace/contest/1499/e/e.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/1499/e/e.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1499/e/e.cpp"
...
#line 27 "codeforces-workspace/contest/1499/e/e.cpp"
...
void main() {
  ...
  string x, y;
  cin >> x >> y;
  auto dpx = make_vector<mint>({x.size() + 1, y.size() + 1, (size_t)2});
  auto dpy = dpx;
  mint ans;
  for (auto i : range(x.size() + 1)) {
    for (auto j : range(y.size() + 1)) {
      for (auto k : range(2)) {
        if (i) {
          if (i != x.size() && x[i] != x[i - 1])
            dpx[i + 1][j][k] += dpx[i][j][k];
          if (j != y.size() && x[i - 1] != y[j])
            dpy[i][j + 1][1] += dpx[i][j][k];
        }
        if (j) {
          if (j != y.size() && y[j] != y[j - 1])
            dpy[i][j + 1][k] += dpy[i][j][k];
          if (i != x.size() && x[i] != y[j - 1])
            dpx[i + 1][j][1] += dpy[i][j][k];
        }
      }

      if (i < x.size()) dpx[i + 1][j][0] += 1;
      if (j < y.size()) dpy[i][j + 1][0] += 1;
      ans += dpx[i][j][1];
      ans += dpy[i][j][1];
    }
  }
