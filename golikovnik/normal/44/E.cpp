...
int main() {
  ...
  cin >> s;
  int n = (int) s.size();
  vector<vector<int>> can(n + 1, vector<int>(k + 1));
  can[0][0] = 1;
  for (int len = 0; len < n; ++len) {
    for (int have = 0; have < k; ++have) {
      for (int take = a; take <= b && take + len <= n; ++take) {
        can[take + len][have + 1] |= can[len][have];
      }
    }
  }
  if (!can[n][k]) {
  ...
  vector<int> lens;
  while (n) {
    for (int pr = n - 1; pr >= 0; --pr) {
      if (can[pr][k - 1] && a <= n - pr && n - pr <= b) {
        lens.push_back(n - pr);
        n = pr;
        --k;
        break;
