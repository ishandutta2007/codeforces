...
mat g;
datas dfs(ll now,ll par){
  pvec v;
  for(auto x:g[now]){
    if(x==par)continue;
    v.emplace_back(dfs(x,now));
  }
  if(v.empty())return datas(0,1);
  sort(all(v),[](datas l,datas r){
    return l.second<r.second;
  });
  datas res=v[0];
  ll i;
  rep1(i,v.size()){
    chmax(res.first,max(v[i].first,v[i].second+1));
  }
  ++res.second;
  return res;
}
int main(){
  ...
  pvec v;
  for(auto x:g[0])v.emplace_back(dfs(x,0));
  sort(all(v),[](datas l,datas r){
    return l.second>r.second;
  });
  ll ans=max(v[0].first,v[0].second);
  rep1(i,v.size())chmax(ans,max(v[i].first,v[i].second+1));
  print(ans);
}
}