// hopefully I won't be distracted
...
int main(){
	...
	auto finalResult=INT64_MAX;
	for(int const zeroIndex: range(0, number)){
		int64_t last=0, result {};
		for(int const i: range(zeroIndex+1, number)){
			auto q=last/a[i]+1;
			assert(a[i]*q>last); assert(a[i]*(q-1)<=last);
			last=a[i]*q; result+=q;
		}
		last=0;
		for(int i=zeroIndex; i--;){
			auto q=last/a[i]+1;
			assert(a[i]*q>last); assert(a[i]*(q-1)<=last);
			last=a[i]*q; result+=q;
		}
		finalResult=std::min(finalResult, result);
	}
