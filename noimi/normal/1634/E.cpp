...
int main() {
    ...
    bool flag = false;
    fore(x, y, mp) if(y) flag = true;
    if(flag)
        NO();
    else {
        map<pii, pii> to[2];
        vector<string> ans(n);
        rep(i, n) ans[i] = string(si(v[i]), 'L');
        map<int, pii> mem;
        rep(i, n) {
            mp.clear();
            rep(j, si(v[i])) {
            ...
            vpi w;
            fore(x, y, mp) { w.eb(y, x); }
            SORT(w);
            rep(j, 0, si(w), 2) {
                to[0][pii(i, w[j].fi)] = pii(i, w[j + 1].fi);
                to[0][pii(i, w[j + 1].fi)] = pii(i, w[j].fi);
            }
            rep(j, si(w)) {
                if(mem.count(w[j].se)) {
                    auto k = mem[w[j].se];
        ...
        while(si(to[0])) {
            int t = 0;
            auto p = begin(to[0])->fi;
            while(1) {
                if(!to[t].count(p)) break;
                auto nxt = to[t][p];
                ans[nxt.fi][nxt.se] = 'R';
                to[t].erase(p);
                to[t].erase(nxt);
