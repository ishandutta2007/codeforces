...
void solve()
{
    ...
    set<ll> vals;
    for (auto it: pref) vals.insert(it);
    map<ll, int> mapka;
    ...
    for (int i = 0; i<=n; i++) pref[i] = mapka[pref[i]];

    vector<int> dp(n+1);
    dp[0] = 0;
    SegmentTree tree(cnt);
    ...
    {
        dp[i] = dp[i-1];
        if (pref[i-1]<pref[i]) dp[i]++;
        else if (pref[i-1]>pref[i]) dp[i]--;

        int val = -INF;
        if (pref[i]>0)
        {
            val = tree.maxx(0, pref[i]-1);
        }

        dp[i] = max(dp[i], val + i);

        tree.upd(pref[i], dp[i] - i);
    }
    cout<<dp[n]<<endl;
}
