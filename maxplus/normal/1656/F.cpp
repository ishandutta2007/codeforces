...
int main() {
  ...
  for (int testcase = $; testcase; --testcase) {
    ...
    cin >> range{a, n};
    sort(a, a + n);
    int64_t s = accumulate(a, a + n, 0ll), mi = a[0], ma = a[n - 1];
    if (s + mi * (n - 2ll) > 0 || s + ma * (n - 2ll) < 0) {
      print("INF");
      continue;
    }
    int64_t ans = -1e18;
    for (int64_t i = 0, csl = 0, csr = s; i < n - 1; ++i) {
      int64_t t = -a[i];
      csl += a[i], csr -= a[i];
      maxi(ans, (mi * (n - i - 2) + s + ma * i) * t + mi * csr + ma * csl - mi * ma);
    }
    print(ans);
  }
}