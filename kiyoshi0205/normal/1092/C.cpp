...
int main(){
  ...
  sort(all(v),[](auto l,auto r){return l.first.size()>r.first.size();});
  string ans(N*2-2,'S');
  s=v[0].first+v[1].first.back();
  // s=v[1].first+v[0].first.back();
  debug(s);
  debug(v);
  rep1(i,N*2-2){
    if(v[i].first!=v[i-1].first){
      ans[v[i].second]=v[i].first==s.substr(0,v[i].first.size())?'P':'S';
      ans[v[i-1].second]=ans[v[i].second]^'S'^'P';
    }else{
      ans[v[i].second]='P';
      ans[v[i-1].second]='S';
    }
  ...
  rep(i,N*2-2){
    if(ans[i]=='P'&&s.substr(0,a[i].size())!=a[i])break;
    if(ans[i]=='S'&&s.substr(N-a[i].size(),a[i].size())!=a[i])break;
  }
  if(i<N*2-2){
    s=v[1].first+v[0].first.back();
    rep1(i,N*2-2){
