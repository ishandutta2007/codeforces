...
void _main() { /* CURSOR START */
	...
	vector<ll> ps(n+1),ps2(n+1);
	for(int i=0;i<n;i++) ps[i+1]=ps[i]+a[i];
	for(int i=0;i<n;i++) ps2[i+1]=ps2[i]+a[n-1-i];

	set<ll> ss;
	for(ll x:ps) if(x+x<=ps.back()) ss.insert(x);
	reverse(all(ps2));
	for(ll x:ps2) if(ss.count(x)) return cout<<x<<endl,nop;
	assert(0);
}