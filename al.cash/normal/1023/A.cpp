...
public:
    ...
    ASolver(text_input_file& in, text_output_file& out) : in(in), out(out), err(stdout) {}
...
//        read(in, n, m, s, "", t, "");
        ...
        read(in, t);
        int u = -1;
        for (int i : irange(n)) if (s[i] == '*') u = i;
        if (u == -1) return s == t;
        int e = n - u - 1;
        return n <= m + 1 && s.substr(0, u) == t.substr(0, u) &&
            s.substr(u + 1, e) == t.substr(m - e, e);
    }

    void solve() {
        writeln(out, ok() ? "YES" : "NO");
    }
};

void solve(text_input_file& in, text_output_file& out) {
    ASolver solver(in, out);
    solver.solve();
}
