...
int main(){
    ...
    vec(ll,fret,n);
    vec(ll,st,n,5);
    rep(n)fret[i]=b[i]-a[st[i]];
    ll mx=max(fret),ans=mx-1;
    priority_queue q([&](ll a,ll b){return fret[a]>fret[b];},iota(n));
    for(ll low=2;;low++){
        chmax(low,fret[q.top()]);
        while(fret[q.top()]<low){
            ll at=q.top();
            q.pop();
            if(!st[at])return out(ans);
            st[at]--;
            fret[at]=b[at]-a[st[at]];
            chmax(mx,fret[at]);
            q.push(at);
        }
        chmax(low,fret[q.top()]);
