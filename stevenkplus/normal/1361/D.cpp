...
int main() {
  ...
  for (auto &v: lines) {
    ...
    int neg = min(sz(v), (k + 1) / 2);
    for (int i = 0; i < neg; ++i) {
      vals.push_back(v[i] * (k - 1 - 2 * i));
    }
    double sm = 0;
    for (int i = sz(v) - 1; i >= neg; --i) {
      vals.push_back(v[i] * (k - 1 - 2 * neg) - 2 * sm);
      sm += v[i];
    }
  }
  pp(vals);
