...
int win[32][32][2][2];
int rec(int z, int o, int f, int s){
	if(z == 0 and o == 0){
		if(f) return 0;
		return 1;
	}
	int &ret = win[z][o][f][s];
	if(ret >= 0) return ret;
	if(z and rec(z-1, o, s, f) == 1) return ret = 0;
	if(o and rec(z, o-1, s, f^1) == 1) return ret = 0;
	return ret = 1;
}
void solve(){
	int n; cin >> n;
	...
	if(cnt0 > 16) cnt0 = cnt0%16+16;
	if(cnt1 > 16) cnt1 = cnt1%16+16;
	if(rec(cnt0, cnt1, 0, 0) == 0) o("WIN");
	else o("LOSE");
}
signed main(){
