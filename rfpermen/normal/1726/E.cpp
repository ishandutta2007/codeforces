...
//	freopen("output.txt","w",stdout);
	...
	combin(MAX-5);
	z[1] = 1;
	rep(i,2,300000)z[i] = (z[i-1] * (i*2-1))%MOD;
	x[0] = x[1] = 1;
	rep(i,2,300000)x[i] = (x[i-1] + (i-1)*x[i-2])%MOD;
	cin>>tc;
	rep(ntc,1,tc){
		cin>>n;
		ans = x[n];
		rep(i,1,n/4)ans+= cb(n-i*2,i*2)*z[i] % MOD * pw(2,i) % MOD * x[n-i*4] % MOD;
		ans%= MOD;
		cout<<ans<<endl;
	}
	return 0;
