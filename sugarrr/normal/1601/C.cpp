...
using S=ll;
S op(S a,S b){return S{min(a,b)};}
S e(){return S{inf};}//infseg0
using F=ll;
...
void solve(){
    ...
    rep(i,0,m-1)cin>>b[i];SORT(b);
    ll TEN =0;
    {
        vl c(n);rep(i,0,n-1)c[i] = a[i].fi;
        vl t=c;ERASE(t);
        for(auto &x:c)x = POSL(t,x);
    ...
    vl INIT(n+1);rep(i,0,n)INIT[i] = i;
    lazy_segtree<ll,op,e,F,mapping,composition,id>seg(INIT);
    SORT(a);
    ll pa1 = 0;
    ll pa2 = 0;
    ll sum = 0;
    for(auto x:b){
        while(pa1<=n-1 && x>=a[pa1].fi){
            seg.apply(a[pa1].se+1,n+1,-1);
            pa1++;
        }
        while(pa2<=n-1 && x> a[pa2].fi){
            seg.apply(0,a[pa2].se+1,1);
            pa2++;
        }
       // cerr<<x<<":";rep(i,0,n)cerr<<seg.get(i)<<" ";cerr<<endl;
        sum += seg.all_prod();
    }
    DEB(TEN);
    DEB(sum);
