...
//{}
std::vector<std::pair<long long, int>> PF(long long n) {
	std::vector<std::pair<long long, int>> divisor;
	long long upplim = sqrt(n);
	for(int i = 2; i <= upplim; i++) {
		if(n % i) continue;
		int cnt = 0;
		while(n % i == 0) {
			cnt++;
		...
		divisor.emplace_back(i, cnt);
	}
	if(n != 1) divisor.emplace_back(n, 1);
	return divisor;
}
...
void solve() {
	...
	int ma = 0, tmp;
	for(auto p : vec)
		if(chmax(ma, p.second)) tmp = p.first;
