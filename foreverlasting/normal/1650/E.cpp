...
namespace MAIN{
    int n,d;
    int a[N],b[N],pmn[N],smn[N];
    inline void MAIN(const res &Case){
        n=read(),d=read();
        for(res i=1;i<=n;i++)a[i]=read(),b[i]=a[i]-a[i-1]-1;
        a[n+1]=d;
        pmn[0]=smn[n+1]=d+1;
        res mx=0,smx=0;
        for(res i=1;i<=n;i++){
            pmn[i]=min(pmn[i-1],b[i]);
            if(b[mx]<b[i])smx=mx,mx=i;
            else if(b[smx]<b[i])smx=i;
        }
        smn[n+2]=d+1;
        for(res i=n;i;i--)smn[i]=min(smn[i+1],b[i]);
        res ans=0;
//        printf("%d %d\n",smn[3],pmn[0]);
        for(res i=1;i<=n;i++){
            ans=max(ans,min({i==1?a[2]-1:inf,pmn[i-1],smn[i+2],max((a[i+1]-a[i-1]-2)/2,min(a[i+1]-a[i-1]-1,max({mx==i?(b[smx]-1)/2:(b[mx]-1)/2,i==n?d-a[n-1]-1:d-a[n]-1,i==1?(a[2]-2)/2:(a[1]-2)/2})))}));
        }
        printf("%d\n",ans);
    }
}
