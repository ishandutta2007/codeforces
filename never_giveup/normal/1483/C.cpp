...
const int maxn = 4e5 + 100, maxw = 7 * maxn, inf = 2e9 + 100;
...
void solve() {
    ...
    cin >> n;
    auto get = [&]() -> ll {
        return ans.empty() ? -llinf : *ans.rbegin();
    };
    auto rem = [&](ll w) {
        ans.erase(ans.find(w));
    };
    auto add = [&](ll w) {
        ans.insert(w);
    };
    ...
    for (int i = 0; i < n; i++) {
        ...
        while (!q.empty() && A[q.back().fr] > h) {
            setmax(w, q.back().sc);
            rem(q.back().sc + B[q.back().fr]);
            q.pop_back();
        }
        q.push_back({i, w});
        add(x + w);
