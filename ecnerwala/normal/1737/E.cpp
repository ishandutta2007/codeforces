...
int main() {
	...
	while (T--) {
		...
		std::vector<num> suff(N+1);
		for (int i = N-1; i >= 0; i--) {
			// your group size needs to be at least
			int group_size = (i + 1 + 1) / 2;
			num prob_survive = pow(inv(num(2)), group_size - (i == N-1));
			ans[i] = prob_survive * (1 - suff[min(N, 2 * i + 1)]);
