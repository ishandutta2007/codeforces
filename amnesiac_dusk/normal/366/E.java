...
public class Main
{
	public static void main(String[]args) throws Exception
	{
		...
		for(int i=1; i<=n; i++)
			for(int j=1; j<=m; j++)
			{
				if(ho[a[i][j]][0][0]+ho[a[i][j]][0][1]<i+j)
				{
					ho[a[i][j]][0][0]=i;
					ho[a[i][j]][0][1]=j;
				}
				if(ho[a[i][j]][1][0]-ho[a[i][j]][1][1]<i-j)
				{
					ho[a[i][j]][1][0]=i;
					ho[a[i][j]][1][1]=j;
				}
				if(ho[a[i][j]][2][0]+ho[a[i][j]][2][1]>i+j)
				{
					ho[a[i][j]][2][0]=i;
					ho[a[i][j]][2][1]=j;
				}
				if(ho[a[i][j]][3][0]-ho[a[i][j]][3][1]>i-j)
				{
					ho[a[i][j]][3][0]=i;
					ho[a[i][j]][3][1]=j;
				}	
			}
		int[][]mdist=new int[k+1][k+1];
		for(int i=1; i<=k; i++)
			for(int j=1; j<=k; j++)
				for(int l=0; l<4; l++)
					for(int o=0; o<4; o++)
						mdist[i][j]=max(mdist[i][j],pdist(ho[i][l],ho[j][o]));
		int te[]=nia(s);
		int ans=0;
		for(int i=1; i<s; i++)
			ans=max(ans,mdist[te[i]][te[i-1]]);
		pr(ans);
		System.out.print(output);
	}
	static int pdist(int a[],int b[])
	{
		if(a[1]==0||b[1]==0)
			return 0;
