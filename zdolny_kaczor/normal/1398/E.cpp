...
void solve() {
	int n;
	scanf("%d", &n);
	seg_set<pii, Sum, less<pii>, true> dou, oth, tot;
	REP(i, n) {
		int t, v;
		scanf("%d%d", &t, &v);
		if (v > 0) {
			if (t) dou.insert(pii(v, i));
			else oth.insert(pii(v, i));
			tot.insert(pii(v, i));
		}
		else {
			v *= -1;
			if (t) dou.erase(dou.lower_bound(pii(v, 0)));
			else oth.erase(oth.lower_bound(pii(v, 0)));
			tot.erase(tot.lower_bound(pii(v, 0)));
		}
		debug(imie(dou), imie(oth), imie(tot));
		ll all_sum = tot.read(ALL(tot));
		ll dous = tot.read(tot.find_by_order(tot.size() - dou.size()), tot.end());
		if (dous == dou.read(ALL(dou)) && !dou.empty()) {
			debug("fix");
			dous = tot.read(tot.find_by_order(tot.size() - dou.size() + 1), tot.end()) + (oth.empty() ? 0 : oth.rbegin()->first);
		}
		printf("%lld\n", all_sum + dous);
	}
}
