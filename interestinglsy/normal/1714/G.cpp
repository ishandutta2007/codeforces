...
void dfs(int pos, ll a_sum) {
	prefix_sums.push_back((prefix_sums.empty() ? 0 : prefix_sums.back()) + b[pos]);
	// Print(prefix_sums);
	anss[pos] = upper_bound(prefix_sums.begin(), prefix_sums.end(), a_sum) - prefix_sums.begin() - 1;
	for (int v : sons[pos]) {
		dfs(v, a_sum + a[v]);
	}
...
int main() {
	...
	For(qi, q) {
		...
		dfs(1, 0);
		Forx(i, 2, n) cout << anss[i] << " ";
		cout << endl;
		For(i, n) sons[i].clear();
	}
}