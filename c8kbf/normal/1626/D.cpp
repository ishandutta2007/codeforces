...
inline void AC() {
    ...
    n = read();
    for(_ i = 1; i <= n; ++i) a[i] = 0;
    for(_ i = n; i--; ) ++a[read()];
    _ p = 1, sm = 0;
    for(_ i = 1; i <= n; ++i) {
        if(sm + a[i] > p) ga.push_back(mp(sm, p-sm));
        sm += a[i];
        for(; sm > p; ) p <<= 1;
    }
    ga.push_back(mp(sm, p-sm));
    p = 1;
    sm = 0;
    for(_ i = n; i >= 1; --i) {
        if(sm + a[i] > p) gb.push_back(mp(sm, p-sm));
        sm += a[i];
        for(; sm > p; ) p <<= 1;
    }
    gb.push_back(mp(sm, p-sm));
    _ s = 1E11;
    for(_ i = 1; i <= n; i <<= 1) {
        for(_p j : ga) for(_p k : gb) {
            _ lf = n-j.first-k.first;
            if(lf > i) continue;
            s = min(s, i-lf+j.second+k.second);
        }
    }
    writeln(s);
