...
int main() {
    ...
    rep(i,n) {
        rep(j,2) {
            if (a[1-j][i] == '0' && a[j][i+1] == '0') chmax(dp[j][i+1],dp[j][i]);
            else if (a[1-j][i] == '0' && a[j][i+1] == '1') chmax(dp[j][i+1],dp[j][i]+1);
            else if (a[1-j][i] == '1' && a[j][i+1] == '0') {
                chmax(dp[j][i+1],dp[j][i]);
                chmax(dp[1-j][i+1],dp[j][i] + 1 + (a[1-j][i+1] - '0'));
            }
            else {
                chmax(dp[j][i+1],dp[j][i]+1);
                chmax(dp[1-j][i+2],dp[j][i] + 1 + (a[1-j][i+1] - '0') + (a[1-j][i+2] - '0'));
            }
        }
    }
