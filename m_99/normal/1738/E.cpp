...
int main(){
	...
	rep(_,_t){
		...
		while(l.size()!=0 || r.size()!=0){
			mint tt = 1;
			if((l.size()!=0&&l.back()==0) || (r.size()!=0&&r.back()==0)){
				int cl = 0,cr = 0;
				...
				if(!F){
					for(int j=0;j<=min(cl,cr);j++){
						if(l.size()==0&&r.size()==0)tt += C.junretsu(cl-j,j) * C.junretsu(cr-j,j);
						else tt += C.junretsu(cl-j,j+1) * C.junretsu(cr-j,j+1);
						//cout<<tt.val()<<endl;
					}
				}
				else{
					tt = mint(2).pow(cl+cr+1);
					//tt--;
				}
			}
			else tt = 2;
