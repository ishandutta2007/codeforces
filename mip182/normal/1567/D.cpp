...
void solve() {
    ...
    for (auto to: s)
        sum += to - '0';
    vector<int> now(s.size());
    for (int i = 0; i < s.size(); i++)
        now[i] = s[i] - '0';
    {
        int ind = (int) (s.size()) - 2;
        while (ind >= 0) {
            if (sum < n && now[ind]) {
                now[ind + 1] += 10;
                now[ind]--;
                sum += 9;
                ind = min(ind + 1, (int) (s.size()) - 2);
            } else
                ind--;
        }
    }
    for (int i = 0; i < s.size(); i++) {
        while (n > 1 && sum > 1 && now[i] > 0) {
            n--;
            sum--;
            cout << 1;
            for (int f = 0; f < (int) (s.size()) - i - 1; f++)
                cout << 0;
            cout << " ";
            now[i]--;
        }
    ...
    while (now[i] == 0)
        i++;
    for (; i < s.size(); i++)
        cout << now[i], assert(now[i] >= 0 && now[i] < 10);
    cout << '\n';
}
