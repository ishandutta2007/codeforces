...
void solve() {
	...
	while (true) {
		...
		if (!go(nxtR, nxtC) && !go(nxtR, curC) && !go(curR, nxtC))
			assert(false);

		if (dr == 1) { if (curR != maxR) ++maxR; ++minR; }
		if (dr == -1) { if (curR != minR) --minR; --maxR; }
		if (dc == 1) { if (curC != maxC) ++maxC; ++minC; }
		if (dc == -1) { if (curC != minC) --minC; --maxC; }
