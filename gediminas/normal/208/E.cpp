...
vi vaik[maxn + 1];
int ats[maxn] = {};
vector<pii> uzkl[maxn + 1];

map<int, int> *dfs(int d, int g) {
	vector<map<int, int>*>vis;

	map<int, int> *ret = nullptr;

	for (auto && i : vaik[d]) {
		auto t = dfs(i, g + 1);

		if (ret == NULL or t->size() > ret->size())
			ret = t;
...
int main () {
	...
	auto getPar = [&](int v, int g) -> int {
		for (int j = logn - 1; j >= 0; j--) {
			if (p[v][j] != 0 and (1 << j) <= g) {
				g -= (1 << j);
				v = p[v][j];
			}
		}
