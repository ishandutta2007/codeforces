...
void solve()
{
    vector<int> phi(1+n),ophi(n-2,inf<int>);
    phi[1]=1;
    for(int i=2,j; i<=n; i++)
    {
        bool dec=false;
        ...
        if(i%2)
        {
            ophi.emplace_back(phi[i]);
        }
    }
    sort(begin(phi)+3, end(phi));
    sort(all(ophi));
    cout << min(accumulate(begin(phi)+3, begin(phi)+k+3, 0LL) + 2, accumulate(begin(ophi), begin(ophi)+k, 0LL) + 1) << "\n";
}