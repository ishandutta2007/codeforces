...
using H = polynomial_hash<modnum<int(1e9 + 7)>, 3, 256>;

int main() {
    setIO();

    map<int, set<H>> hashes;

    auto add = [&](const string &s) { hashes[sz(s)].insert(H::get_prefixes(all(s)).back()); };
    auto remove = [&](const string &s) { hashes[sz(s)].erase(H::get_prefixes(all(s)).back()); };
    auto count = [&](const string &s) {
        auto prefixes = H::get_prefixes(all(s));
        const int n = sz(s);
        ll ans = 0;
        for (auto it = hashes.begin(); it != hashes.end();) {
            ...
            const int len = it->first;
            for (int i = 0; i + len <= n; i++) {
                H hash = H::get_substring(prefixes, i, i + len);
                if (it->second.count(hash))
                    ans++;
            }
            it++;
        }
