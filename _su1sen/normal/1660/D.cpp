...
tuple<int, int, int> solve(vector<int> a) {
    ...
    if (not neg) {
        int res = 0;
        for (int e : a) res += abs(e) == 2;
        return { res, 0, 0 };
    }
    int x = find_if(all(a), [](int e) { return e < 0; }) - a.begin();
    int y = find_if(a.rbegin(), a.rend(), [](int e) { return e < 0; }) - a.rbegin();
...
int main() {
    ...
    loop(t) {
        ...
        int al = n, ar = 0;
        rep(i, z - 1) {
            int l = zeros[i] + 1, r = zeros[i + 1];
            auto [v, x, y] = solve(vector<int>(a.begin() + l, a.begin() + r));
            if (chmax(m, v)) {
                al = l + x;
                ar = n - r + y;
            }
