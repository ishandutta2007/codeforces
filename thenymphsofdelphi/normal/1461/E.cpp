...
bool cal1(int k, int l, int r, int t, int x, int y){
    if (k < l || k > r){
        return 0;
    }
    if (x > y){
    ...
    int day = (k - l) / x; t -= day; k -= x * day;
    // special day is when we MUST pour y litres of water.
    // there are only a maximum of x specials day, and x <= 1e6
    // control by set or bool
    set <int> stt;
    while (stt.find(k) == stt.end()){
        stt.insert(k);
        if (k + y > r){
            return 0;
        }
