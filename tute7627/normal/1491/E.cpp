...
int main(){
  ...
  /*n=fb.back();
  Graph<int>g(n);
  rep(i,0,n-1)g[i].PB(i+1),g[i+1].PB(i);*/

  vector<bool>ng(n);
  auto rec=[&](auto &&f,ll r,ll k)->bool{
    if(k<=2)return true;
    bool ret=false;
    vector<P>cand;//k-1,k-2
    auto dfs=[&](auto &&df,ll v,ll par)->ll{
      ll sz=1;
      for(auto to:g[v]){
        if(to==par||ng[to])continue;
        ll add=df(df,to,v);
        if(add==fb[k-1])cand.EB(to,v);
        if(add==fb[k-2])cand.EB(v,to);
        sz+=add;
      }
      return sz;
    };
    ...
    for(auto z:cand){
      bool ok=true;
      ng[z.se]=true;
      if(!f(f,z.fi,k-1))ok=false;
      ng[z.se]=false;
      ng[z.fi]=true;
      if(!f(f,z.se,k-2))ok=false;
      ng[z.fi]=false;
      return ok;
    }
    return false;
  };
  ans2(fb.back()==n&&rec(rec,0,fb.size()-1));
  return 0;
}