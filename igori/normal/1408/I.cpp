...
signed main()
{
    ...
    vector<int> x;
    int X = 0;
    vector<vector<int> > sets(c - L + 1);
    vector<vector<int> > cnk(k + 1, vector<int>(k + 1));
    for (int i = 0; i <= k; i++)
    {
        cnk[i][0] = cnk[i][i] = 1;
        for (int j = 1; j < i; j++)
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        sets[cnt].push_back(x);
    }
    vector<vector<int> > dp_big((1 << c), vi(k + 1));
    dp_big[0][0] = 1;
    for (int cnt = 0; cnt <= c - L; cnt++)
    {
        ln;
        vector<vector<int> > dp(1 << (L + 1), vi(k + 1));
        dp[0][0] = 1;
        ln;
        for (auto e : sets[cnt])
        {
            for (int tb = k; tb >= 0; tb--)
            {
                for (int i = 0; i < la; i++)
                {
                    for (int t = 1; t + tb <= k; t++)
                    {
                        int z = e ^ (e - t);
                        z &= la - 1;
                        dbg(tb + t);
                        dbg(t);
                        dp[i ^ z][t + tb] = (dp[i ^ z][t + tb] + 1ll * cnk[tb + t][t] * dp[i][tb]) % MOD;
                    }
                }
            }
        }
        int sett = 0;
        for (int j = L; j < L + cnt; j++) sett += (1 << j);
        dbg(cnt);
        dbg(sett);
        ln;
        for (int tb = k; tb >= 0; tb--)
        {
            for (int mask = 0; mask < (1 << (4 + cnt)); mask++)
            {
                for (int hm = 0; hm < la; hm++)
                {
                    for (int t = 1; t + tb <= k; t++)
                    {
                        int y = hm & ((1 << L) - 1);
                        ...
                        dp_big[mask ^ y][t + tb] = (dp_big[mask ^ y][t + tb] + 1ll * dp[hm][t] * dp_big[mask][tb] % MOD * cnk[t + tb][t]) % MOD;
                    }
                }
            }
        }
    ...
    forn(mask, 1 << origc)
    {
        cout << 1ll * dp_big[X ^ mask][k] * W % MOD << " ";
    }
}
