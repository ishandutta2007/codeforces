...
signed main()
{
	...
	while(T--)
	{
		...
		upn rd(x[i]),rd(y[i]);
		int l=0,r=10000;
		int ans=-1,as=-1;//cout<<check(1);edl;return 0;
		while(l<=r)
		{
			int mid=(l+r)>>1;
			int w=check(mid);
			if(w)
			{
				ans=mid;as=w;r=mid-1;
			}
			else l=mid+1;
		}
		
		cout<<ans;edl;cout<<qpow(2,as);edl;
	}
}
/*
