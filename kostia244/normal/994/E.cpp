...
int main() {
	doin();//if cnt == 1: know ans=lst for sure, if cons == 1: they know answer for sure
	int n, m;
	cin >> n >> m;
	vi a, b;
	getv(a, n);
	...
	for(auto& i : a) i*=2;
	for(auto& i : b) i*=2;
	map<int, pair<ll, ll>> k;
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < m; j++) {
			k[a[i] + (b[j]-a[i])/2].first|=1ll<<i;
			k[a[i] + (b[j]-a[i])/2].second|=1ll<<j;
		}
	}
	vpi q;
	for(auto i : k)
	...
	for(int i = 0; i < q.size(); i++) {
		for(int j = i; j < q.size(); j++) {
			...
			t.first|=q[j].first;
			t.second|=q[j].second;
			maxq(ans, __builtin_popcountll(t.first)+__builtin_popcountll(t.second));
		}
	}
	cout << ans;
	return 0;
