    ...
    for (int x : lca.rev_tour_list) {
        for (int k = 0; k < 2 * K; k++) {
            ...
            for (int child : lca.adj[x])
                dp[x][k] ^= dp[child][mod_2k(k - 1)];
        }
    }

    vector<vector<int>> up_dp(N, vector<int>(2 * K, 0));

    for (int node : lca.tour_list) {
        int parent = lca.parent[node];

        for (int k = 0; k < 2 * K; k++) {
            up_dp[node][k] = dp[node][k];

            if (parent >= 0) {
                up_dp[node][k] ^= dp[node][mod_2k(k - 2)];
                up_dp[node][k] ^= up_dp[parent][mod_2k(k - 1)];
            }
        }
    }

    for (int node = 0; node < N; node++)
        cout << bool(up_dp[node][K]) << (node < N - 1 ? ' ' : '\n');
}