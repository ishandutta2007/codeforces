...
int main(){
	...
	For(TwT,T){
		...
		Forx(l,1,n*m)
			Forx(r,l,n*m){
				if( l*k <= cntr and r*k >= cntr and r-l < mindelta ){
					mindelta = r-l;
					lo = l; hi = r;
				}
			}

		static int have[MAXN];
		int left = cntr - lo*k;
		For(i,k) have[i] = lo;
		For(i,k){
			int use = min( hi - have[i] , left );
			have[i] += use;
			left -= use;
		}
		assert(!left);

		static char ansl[MAXN][MAXN];
		int x = 1, y = 1, xd = 1;
		int p = 1, nowhave = 0;
		For(i,n*m){
			ansl[x][y] = s[p-1];
