...
template <class T>
V<T> dijkstra(const Graph<T>& g, V<int> vs) {
    int n = g.size();
    V<T> ds(n, numeric_limits<T>::max() / 2);
    using P = pair<T, int>;
...
void slv() {
    ...
    auto get_id = [&](int r, int c) { return r * n * 2 + c; };
        ...
        rep(j, n * 2) {
            ...
            rep(dir, 4) {
                ...
                if (nj < 0)
                    ...
                    nj = 0;
                int q = get_id(ni, nj);
                if (!(ni < n && nj < n)) g.add_directed_edge(p, q, c[ni][nj]);
            }
        }
    }
    ...
    auto ds = dijkstra(g, vs);
    ll ans = TEN(18);
    for (int i = n; i < n * 2; ++i) {
        for (int j = n; j < n * 2; ++j) {
            chmin(ans, ds[get_id(i, j)]);
        }
    }
    print(ans + need);
}
