...
#endif
	scanf("%s%s",s1,s2);
	for (int i=0;i<26;i++) for (int j=0;j<26;j++) D[i][j]=100000000;
	int n;
	for (scanf("%d",&n);n>0;n--)
	{
		char a[10],b[10];
		int w;
		scanf("%s%s%d",&a,&b,&w);
		checkmin(D[a[0]-'a'][b[0]-'a'],w);
	}
	for (int i=0;i<26;i++) checkmin(D[i][i],0);
	for (int k=0;k<26;k++) for (int i=0;i<26;i++) for (int j=0;j<26;j++) checkmin(D[i][j],D[i][k]+D[k][j]);
	...
	else
	{
		...
		for (int i=0;s1[i];i++)
		{
			...
			for (int key=0;key<26;key++) 
				if (D[a][key]+D[b][key]<w)
					w=D[a][key]+D[b][key],p=key;
			if (w>=100000000)
			{
				printf("-1\n");
				return 0;
