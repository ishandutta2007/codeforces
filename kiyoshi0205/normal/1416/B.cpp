...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep(i,N){
    ...
    if(i){
      if(v[i]%(i+1)){
        ansi.eb(1);ansj.eb(i+1);ansx.eb(i+1-v[i]%(i+1));
        v[0]-=i+1-v[i]%(i+1);
        v[i]+=i+1-v[i]%(i+1);
      }
      assert(v[i]%(i+1)==0);
      ansi.eb(i+1);ansj.eb(1);ansx.eb(v[i]/(i+1));
      v[0]+=v[i];
      v[i]=0;
    }
  }
  ...
  j=v[0]/N;
  rep1(i,N){
    ansi.eb(1);ansj.eb(i+1);ansx.eb(j);
  }
  print(ansi.size());
  rep(i,ansi.size())print(ansi[i]<<" "<<ansj[i]<<" "<<ansx[i]);
}
}