...
int main(){
	...
	point<double> a, b, bin;
	int n;
	cin >> a >> b >> bin >> n;
	vector<point<double>> p(n);
	vector<double> costa(n), costb(n);
	vector<double> prefmin(n + 1, numeric_limits<double>::max() / 3);
	vector<double> suffmin(n + 1, numeric_limits<double>::max() / 3);
	double sum = 0;
	for(auto i = 0; i < n; ++ i){
		cin >> p[i];
		sum += 2 * distance(p[i], bin);
		costa[i] = distance(p[i], a) - distance(p[i], bin);
		costb[i] = distance(p[i], b) - distance(p[i], bin);
		prefmin[i + 1] = min(prefmin[i], costb[i]);
	}
	for(auto i = n - 1; i >= 0; -- i){
		suffmin[i] = min(suffmin[i + 1], costb[i]);
	}
	double res = sum + min(*min_element(costa.begin(), costa.end()), *min_element(costb.begin(), costb.end()));
	debug(res, sum);
	for(auto i = 0; i < n; ++ i){
		ctmin(res, sum + costa[i] + min(prefmin[i], suffmin[i + 1]));
		debug(i, res, costa[i], prefmin[i], suffmin[i + 1]);
	}
