...
void build()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        r1[i + 1] = (r1[i] * b1 + s[n - i - 1]) % mod1;
        r2[i + 1] = (r2[i] * b2 + s[n - i - 1]) % mod2;
    }
}
...
int get_rhash1(int l, int r)
{
    int ans = r1[r] - r1[l] * p1[r - l];
    ans %= mod1;
    if (ans < 0)
    {
        ans += mod1;
...
int get_rhash2(int l, int r)
{
    int ans = r2[r] - r2[l] * p2[r - l];
    ans %= mod2;
    if (ans < 0)
    {
        ans += mod2;
...
{
    return get_hash1(l, r) == get_rhash1(n - r, n - l) && get_hash2(l, r) == get_rhash2(n - r, n - l);
}
