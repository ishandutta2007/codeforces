...
int main()
{
    ...
    int n_min = max(0, n-25);
    int n_max = n-1;

    int k_min = max(0, k-2);
    int k_max = k;

    vector<vector<int>> tot(n_max - n_min + 1, vector<int>(k_max - k_min + 1));

    for (int n1 = n_min; n1<=n_max; n1++)
        for (int k1 = k_min; k1<=k_max; k1++)
        {
            int n_tmp = n1 - n_min;
            int k_tmp = k1 - k_min;

            for (int take = k1; take<=n1; take++) tot[n_tmp][k_tmp]^=C1(n1, take);
        }
    ...
    for (int l = 0; l<n; l++)
    {
        ...
        for (int r = l; r<n; r++)
        {
            ...
            needxor = max(needxor, 0);

            //cout<<l<<' '<<r<<' '<<deg<<' '<<rem<<' '<<needxor<<endl;

            if (tot[rem - n_min][needxor - k_min]) ans[deg]^=1;
        }
    }

    int last = 0;
    for (int i = 0; i<MAX; i++) if (ans[i]) last = i;
