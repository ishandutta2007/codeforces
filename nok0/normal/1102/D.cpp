...
int main(){
    ...
    if(vec[0] < n / 3){
        REP(i, n){
            int d = s[i] - '0';
            if(vec[d] > n / 3) s[i] = '0', vec[0]++, vec[d]--;
            if(vec[0] == n / 3) break;
        }
    }
    if(vec[2] < n / 3){
        RREP(i, n){
            int d = s[i] - '0';
            if(vec[d] > n / 3) s[i] = '2', vec[2]++, vec[d]--;
            if(vec[2] == n / 3) break;
        }
    }
    if(vec[0] > n / 3){
        RREP(i, n){
            int d = s[i] - '0';
            if(d == 0 and vec[d] > n / 3) s[i] = '1', vec[1]++, vec[d]--;
            if(vec[0] == n / 3) break;
        }
    }
    if(vec[2] > n / 3){
        REP(i, n){
            int d = s[i] - '0';
            if(d == 2 and vec[d] > n / 3) s[i] = '1', vec[1]++, vec[d]--;
            if(vec[2] == n / 3) break;
        }
    }
    assert(vec[0] == n / 3 and vec[1] == n / 3 and vec[2] == n /3);
    print(s);
}