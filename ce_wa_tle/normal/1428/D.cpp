...
inline ll abs(ll u){return max(u,-u);}
ll d[333333];
ll xm[333333],ls2,ls3,ls3xls0,top=0;
ll cx[333333],cy[333333];
inline void add(ll x,ll y){++top,cx[top]=x,cy[top]=y;}
int main(){
//	IO(D);
	ll rill(n);
	rep(i,1,n)rill(d[i]);
	...
	rep(i,1,n){
		if(!d[i])continue;
		while(ls2<=now&&xm[ls2]!=2)ls2++;
		while(ls3<=now&&xm[ls3]!=3)ls3++;
		while(ls3xls0<=now&&(ls3xls0<=ls2||xm[ls3xls0]))ls3xls0++;
		if(d[i]==1){
			if(ls3<=now&&ls3xls0<=now+1){
				add(i,ls3);
				add(i,ls3xls0);
				xm[ls3]=xm[ls3xls0]=1;
				if(ls3xls0>now)now++;
