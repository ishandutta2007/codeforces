...
ll n,flg,p,t,kk,ress,cur;
ll x[MAXN],y[MAXN],res[MAXN];
char s[MAXN]; 
// Fast IO
inline ll read()
{
...
int main()
{
	...
	while(1)
	{
		...
		for(;cur<n;cur++)
		{
			...
			if(x[cur]==1&&x[cur+1]==-1)
			{
				x[cur]=-1,x[cur+1]=1,kk--;
				printf("1 %d\n",cur),cur++;
			}
		}
		ress=0;
		for(;cur<n;cur++)
		{
			if(x[cur]==1&&x[cur+1]==-1)
			{
				x[cur]=-1,x[cur+1]=1,res[++ress]=cur++;
			}
		}
		if(ress)
		{
			kk--,printf("%d ",ress);
			for(register int i=1;i<=ress;i++)
			{
				printf("%d ",res[i]);
			}
			puts("");
