...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        set<string> st;
        string a[n];
        REP(i,0,2) REP(j,0,2) t[i][j]=0;
        REP(i,0,n)
        {
            string s;
            cin>>s;
            a[i]=s;
            t[s[0]-'0'][s[s.size()-1]-'0']++;
            st.insert(s);
        }
        if(t[0][0]!=0&&t[1][1]!=0&&t[0][1]==0&&t[1][0]==0)
        {
            cout<<-1<<'\n';
            continue;
        }
        if(abs(t[0][1]-t[1][0])<=1)
        {
            cout<<0<<'\n';
            continue;
        }
