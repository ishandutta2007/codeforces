...
bool solve(){
    ...
    pll a,b,c,d;
    in(a,b,c,d);
    if(set{a,b,c}.contains(d))return 1;
    set<pll>corner;
    corner.emplace(1,1);
    corner.emplace(1,2);
    corner.emplace(1,n-1);
    corner.emplace(1,n);
    corner.emplace(2,1);
    corner.emplace(2,n);
    corner.emplace(n-1,1);
    corner.emplace(n-1,n);
    corner.emplace(n,1);
    corner.emplace(n,2);
    corner.emplace(n,n-1);
    corner.emplace(n,n);
    if(corner.contains(a)+corner.contains(b)+corner.contains(c)==3){
        return (a.first==d.first)+(b.first==d.first)+(c.first==d.first)==2 ||
        (a.second==d.second)+(b.second==d.second)+(c.second==d.second)==2;
    }
    auto mod2=[](pll x){ return pair{x.first%2,x.second%2}; };
    a=mod2(a);
    b=mod2(b);
    c=mod2(c);
    d=mod2(d);
    return set{a,b,c}.contains(d);
}
int main(){
    LL(t);
    rep(t)YES(solve());
