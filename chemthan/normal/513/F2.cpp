...
void solve() {
	...
	FOR(i, 0, d) {
		...
		int y = cor[1][i].se;
		FOR(j, 0, n * m) {
			if (dis[u][j] < INF) vl.pb((ll) x * dis[u][j]);
			if (dis[j][v] < INF) vl.pb((ll) y * dis[j][v]);
		}
	}
	sort(all(vl)); uni(vl);
	int lo = 0, hi = sz(vl) - 1;
	while (lo < hi) {
		...
		FOR(i, 0, d) {
			...
			int y = cor[1][i].se;
			FOR(j, 0, n * m) {
				if (dis[u][j] < INF && (ll) x * dis[u][j] <= vl[mid]) {
					dinic.add(i, j + 2 * d, 1, 0);
				}
				if (dis[j][v] < INF && (ll) y * dis[j][v] <= vl[mid]) {
	...
	if (lo == sz(vl) - 1) printf("-1");
	else printf("%I64d", vl[lo]);
}
