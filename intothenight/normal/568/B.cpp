...
int main(){
	...
	int n;
	cin >> n;
	auto power = precalc_power(Zp(2), n);
	combinatorics<Zp> C(n);
	vector<Zp> tot(n + 1), part(n + 1);
	tot[0] = part[0] = 1;
	for(auto i = 1; i <= n; ++ i){
		tot[i] = 2 * tot[i - 1];
		for(auto sz = 2; sz <= i; ++ sz){
			tot[i] += C.C(i - 1, sz - 1) * tot[i - sz];
		}
		for(auto sz = 1; sz <= i; ++ sz){
			part[i] += C.C(i - 1, sz - 1) * part[i - sz];
		}
	}
	cout << tot[n] - part[n] << "\n";
	return 0;
