...
void solve() {
  ...
  vc<int> A(N), B(N);
  vc<int> IA, IB;
  FOR(i, N) if (S[i] == '0') A[i] = 1, IA.eb(i);
  FOR(i, N) if (S[i] == '1') B[i] = 1, IB.eb(i);
  auto Ac = cumsum(A);
  auto Bc = cumsum(B);

  auto out = [&](int x) -> void {
    if (x == 1) print("tokitsukaze");
    if (x == -1) print("quailty");
    if (x == 0) print("once again");
  };

  if (SUM(A) == 0 || SUM(B) == 0) return out(1);
  ...
  FOR(L, N - K + 1) {
    FOR(2) {
      ...
      ll l = Bc[L] - Bc[0];
      ll r = Bc[N] - Bc[L + K];
      if (l == 0 && r == 0) return out(1);
      ll a = IB[0], b = IB.back();
      if (l && r) lose = 0;
      if (l == 0) {
        if (b - K + 1 >= L + K && Bc[b - K + 1] - Bc[L + K]) lose = 0;
      }
      if (r == 0) {
        if (a + K <= L && Bc[L] - Bc[a + K]) lose = 0;
      }
    }
  }
  if (lose) return out(-1);
  out(0);
}
