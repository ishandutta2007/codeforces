...
bool check()
{
    ...
    {
        vector<int> pos;
        for (int i = 0; i<n; i++) if (a[i]&(1<<bit)) pos.push_back(i);

        int k = pos.size();
        for (int i = 0; i<k-1; i++) dsu.union_sets(pos[i], pos[i+1]);
    }

    return (dsu.sz[dsu.find_set(0)] == n);
}

int largest(int n)
{
    int ans = 0;
    while (n%(1<<(ans+1)) == 0) ans++;
    return ans;
}
...
void solve()
{
    ...
    int pos = 0;
    for (int i = 0; i<n; i++) if (largest(a[i])> largest(a[pos])) pos = i;
    a[pos]--;
