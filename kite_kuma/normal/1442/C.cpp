...
int main() {
	...
	const int inf = 1e9;
	V<p> d(n * 2, make_pair(inf, inf));	 // rev,move
	d[0] = {0, 0};

	using t = tuple<int, int, int>;	 // rev,move,now
	...
	// while(!s.empty()) {
	// 	int now, dis;
	// 	std::tie(now, dis) = s.front();
	// 	if(s[now] < dis) continue;
	...
	// int hant, move;
	// tie(hant, move) = d[goal];

	// ll an = modpow(2, hant, mod) - 1 + mod + move;
	// drop(an % mod);
	...
	// 	tie(hanten, move, now) = r.top();
	// 	r.pop();
	// 	if(hanten > d[now].first + 1 || hanten >= d[now].first && move >= d[now].second) {
	// 		continue;
	// 	}
	...
	// 		debug(hanten, move);
	// 		dump(d);
	// 		ll res = modpow(2, hanten, mod) - 1 + mod;
	// 		res += move;
	// 		drop(res % mod);
	// 	}
	...
	// 	if(hanten % 2 == 0) {
	// 		foa(e, gra.edges[now]) {
	// 			if(chmin(d[e.to], make_pair(hanten, move + 1)) || d[e.to] > make_pair(hanten - 1, move + 1)) {
	// 				r.push(make_tuple(hanten, move + 1, e.to));
	// 			}
	// 		}

	// 		foa(e, rev.edges[now]) {
	// 			if(chmin(d[e.to], make_pair(hanten + 1, move + 1)) || d[e.to] > make_pair(hanten, move + 1)) {
	// 				r.push(make_tuple(hanten + 1, move + 1, e.to));
	// 			}
	// 		}
	// 	} else {
	// 		foa(e, rev.edges[now]) {
