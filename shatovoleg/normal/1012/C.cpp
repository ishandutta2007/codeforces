...
void run() {
    ...
    const int dsz = (n + 1) >> 1;
    constexpr int umask = 3;
    vector<vector<array<int, umask>>> dp(n, vector<array<int, umask>>(dsz, {{INF, INF, INF}}));
    dp[0][0][0] = 0;
    for (int i = 1; i + 1 < n; ++i) {
        dp[i][0][0] = 0;
        for (int j = 1; j < dsz; ++j) {
            for (int pmsk = 0; pmsk < umask; ++pmsk) {
                int b1 = pmsk & 1, b2 = (pmsk >> 1) & 1;
                for (int now = 0; now <= 1 - b2; ++now) {
                    int cost = 0;
                    if (now) {
                        int prv = a[i - 1];
                        if (b1 && i >= 2) {
                        ...
                        cost = max(prv - a[i] + 1, 0) + max(a[i + 1] - a[i] + 1, 0);
                    }
                    cmin(dp[i][j][b2 | (now << 1)], dp[i - 1][j - now][pmsk] + cost);
                }
            }
        }
    }
    for (int i = 1; i < dsz; ++i) {
        cout << *min_element(all(dp[n - 2][i])) << " ";
    }
    cout << endl;
}