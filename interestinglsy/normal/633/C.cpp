...
#define MAXM 100010
#define NODE 1000010
#define S(a,b) (tree[a].son[b])
#define ed(a) (tree[a].ed)
struct Node{
	int son[26];
	int ed;
	Node(){ Ms(son);ed = 0; }
...
void Insert( int &pos , const string &s , int nxt , int len , int id ){
	if(!pos) pos = nownode++;
	if( nxt == len ){
		ed(pos) = id;
		return;
	}
	Insert( S(pos,tolower(s[nxt])-'a') , s , nxt+1 , len , id );
}

bool dp[MAXN];
int tran[MAXN];
void Dfs( int tpos , int spos , int stpos ){
	if(!tpos) return;//O(tpos);
	if( ed(tpos) && (spos>=0?dp[spos]:1) ){
		dp[stpos] = 1;
		tran[stpos] = ed(tpos);
		return;
	}
	if( spos == -1 ) return;
	Dfs( S(tpos,s[spos]-'a') , spos-1 , stpos );
}

void Print( int pos , string s ){
	if(ed(pos)) cout << s << " " << ed(pos) << endl;
	For0(i,26)
		if(S(pos,i))
			Print(S(pos,i),s+char(i+'a'));
}
...
int main(){
	...
	Read(m);
	For(i,m){
		In.Readstring(word[i]);
		int len = word[i].length();
		//Insert(root,word[i],0,len,i);
		int pos = root;
		int nxt = 0;
		while( nxt != len ){
			char c = tolower(word[i][nxt])-'a';
			if(!S(pos,c)) S(pos,c) = nownode++;
			pos = S(pos,c);
			nxt++;
		}
		ed(pos) = i;
