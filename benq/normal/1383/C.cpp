...
void sol() {
	...
	F0R(i,1<<20) {
		maxTopo[i] = 0;
		F0R(j,20) if (i&(1<<j)) {
			ckmax(maxTopo[i],maxTopo[i^(1<<j)]);
			ckmax(maxTopo[i],1+maxTopo[i&suc[j]]);
		}
	}
	vi comp[20];
	F0R(i,20) comp[D.get(i)].pb(i);
	int ans = 0;
	F0R(i,20) if (sz(comp[i])) {
		int mask = 0; trav(t,comp[i]) mask |= 1<<t;
		ans += 2*sz(comp[i])-maxTopo[mask]-1;
		//dbg(i,comp[i]);
		// if (T.topo(comp[i])) {
		// 	//dbg("OK");
		// 	ans += sz(comp[i])-1;
