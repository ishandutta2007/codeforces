...
void solve(__attribute__((unused)) bool read) {
  ...
  dp[par][0] = 1;
  for (int i = 0; i < n; ++i) {
    dp[par ^ 1].assign(s + 1, 0);
    for (int cur = 0; cur < dp[par].size(); ++cur) {
      if (dp[par][cur]) {
        dp[par ^ 1][cur + a[i]] = dp[par ^ 1][cur + b[i]] = 1;
      }
    }
    par ^= 1;
  }
  int cur_res = (int)1e18 + 100;
  for (int cur = 0; cur < dp[par].size(); ++cur) {
    if (dp[par][cur]) {
      relax_min(cur_res, cur * cur + (s - cur) * (s - cur));
    }
  }
  //cout << cur_res << endl;
  cur_res += (n - 2) * s2;
