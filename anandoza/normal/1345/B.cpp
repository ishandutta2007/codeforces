...
// Never evaluates l or r. Returns first i in [l, r] s.t. pred(i).
template <typename Int, typename Pred> Int binarysearch(const Pred &pred, Int l, Int r) {
    l--;
    while (r - l > 1) {
        auto mid = l + (r - l) / 2;
        if (pred(mid))
...
void solve() {
    ...
    int ans = 0;
    while (n >= 2) {
        auto size = [&](ll h) { return 2 * h + 3 * h * (h - 1) / 2; };
        ll h = -1 + binarysearch(L1(u, size(u) > n), 1LL, n);
        ans++;
        n -= size(h);
    }
