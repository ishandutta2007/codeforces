...
int main(){
	...
	*/
	...
	matrix<mint,decltype(f0),decltype(f1)> AA(A,f0,f1,0LL,1);
	AA = AA.beki(k);
	int x = 0;
	rep(i,cnt0){
		x += a[i];
	...
	vector<vector<mint>> B(1,vector<mint>(n+1,0));
	B[0][x] = 1;
	matrix<mint,decltype(f0),decltype(f1)> BB(B,f0,f1,0LL,1);
	
	BB *= AA;
	cout<<BB.value[0][0].val()<<endl;
