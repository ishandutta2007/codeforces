...
public class Main
{
	public static void main(String[]args) throws Exception
	{
		...
		if(odd==0||odd==2)
		{
			...
			for(int i=0; i<n; i++)
			{
				int u=values[i][0],val=values[i][1],v=values[i][2];
				type[max(u,v)][min(u,v)].add(val);
			}
	...
	static void dfs(int pare,int at)
	{
		if(par[at]!=0)
			return;
		par[at]=pare;
