...
class CPekoraAndTrampoline {
 public:
  void solveOne() {
    ...
    vector<int64> s(n);
    for (auto& x : s) cin >> x;
    vector<int64> arrivalsDelta(n);
    int64 res = 0;
    int64 arrivals = 0;
    for (int i = 0; i < n; ++i) {
      arrivals += arrivalsDelta[i];
      int64 extra = max((int64) 0, s[i] - 1 - arrivals);
      res += extra;
      int64 total = extra + arrivals;
      if (i + s[i] + 1 < n) --arrivalsDelta[i + s[i] + 1];
      if (i + 2 < n) ++arrivalsDelta[i + 2];
      if (i + 1 < n) arrivalsDelta[i + 1] += total - (s[i] - 1);
      if (i + 2 < n) arrivalsDelta[i + 2] -= total - (s[i] - 1);
    }
    cout << res << "\n";
  }
