...
fun main(){
    solve.cases{
        val n = getint
        val m = getint
        val need = ceil(sqrt(n.toDouble()) - 0.0000001).toInt()
        ...
        val onstack = IntArray(n){-1}
        val visited = BooleanArray(n)

        fun Graph.dfs(v:Int,rank:Int):Int{
            path.add(v)
            onstack[v] = rank
            visited[v] = true
            var died = -1
            NS(v){w ->
                if(died >= 1) return@NS
                if(onstack[w] != -1){
                    val len = onstack[v] - onstack[w] +1
                    if(len >= need){
                        put(2)
                        put(len)
                        for(i in onstack[w]..onstack[v]){
                            put(path[i]+1)
                        }
                        done()
                        System.exit(0)
...
//                    if(comp[w] == -1){
//                        extension = w
//                    }else if(comp[w] == comp[last]){
//                        if(rank[last] - rank[w] + 1 >= need){
//                            put(2)
//                            put(rank[last] - rank[w] + 1 )
//                            for(i in rank[w]..rank[last]){
//                                put(path[i]+1)
//                            }
//                            return@cases
//                        }
...
//                    rank[extension] = path.size - 1
//                    continue
//                }
//            }
//            just dei path
//            for(i in 0 until path.size step (need - 1)){
//                dead.add(path[i])
//            }
//
//        }
        val D = dead.take(need)
        put(1)
        for(i in D){
            put(i + 1 )
        }
...
inaccuracy : the end of line being fine does not mean the whole thing is fine
costs at least 15 minutes with critical incorrect assumptions 
 */