...
using B = unsigned __int128;
mint solve(int n, V<B> mat);

mint solve_grps(int n, const V<B>& mat, const V<B>& sets) {
    mint ans = 1;
    dbg(sets);
    V<int> idx(n, -1);
    for (auto cur : sets) {
...
mint solve(int n, V<B> mat) {
    ...
    mat.erase(remove_if(mat.begin(), mat.end(), [&](B x) {
        return (x == 0 || x == mask);
    }), mat.end());
    ...
    }
    {
        ...
        if (l >= 2) {
            ...
            auto ans = fact<mint>(l) * solve_grps(n, mat, sets);
            return ans;
        }
    }
    int k = int(mat.size());
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 0; j < k; j++) {
            ...
            if (mat[j] & ~mat[i]) v.push_back(mat[i] & mat[j]);
        }
        if (!ok) continue;
        sort(v.begin(), v.end());
        int l = int(v.size());
    ...
    while (true) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (mi == ma) continue; // no division
            for (int j = mi + 1; j < ma; j++) {
                if ((sets[j] & mat[i]) != sets[j]) {
                    // error
                    return 0;
                }
            }
            if ((sets[ma] & mat[i]) != sets[ma]) {
                update = true;
                B one = sets[ma] & mat[i];
                sets.insert(sets.begin() + ma + 1, sets[ma] ^ one);
                sets[ma] = one;
            }
            if ((sets[mi] & mat[i]) != sets[mi]) {
                update = true;
                B one = sets[mi] & mat[i];
                sets.insert(sets.begin() + mi + 1, one);
                sets[mi] = sets[mi] ^ one;
            }
        }
        if (!update) break;
    }
    return 2 * solve_grps(n, mat, sets);
}
