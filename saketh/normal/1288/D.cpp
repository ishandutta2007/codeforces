...
int main() {
    ...
    while (lo < hi) {
        ...
        F0R (i, N) {
            int m = 0;
            F0R (j, M) if (a[i * M + j] >= mi) m |= 1 << j;
            who[m] = i;
        }

        F0R (j, M) F0R (i, sz(who)) if ((i & (1 << j)) && ~who[i]) {
            who[i ^ (1 << j)] = who[i];
        }

        bool ok = 0;
        F0R (i, sz(who)) if (~who[i] && ~who[i ^ (sz(who) - 1)]) {
            ok = 1;
            ans = { who[i], who[i ^ (sz(who) - 1)] };
            break;
        }
