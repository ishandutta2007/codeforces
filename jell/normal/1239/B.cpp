...
void __solve()
{
    ...
    for(i32 amin=M-2; amin<=M+2; ++amin)
    {
        const i32 inc[4]={0,2,-2,0};
        i32 dp[4]; // parity of index, how swap used
        init(dp,-inf<i32>);
        if(amin<=0) dp[0]=0;
        pii flip[4];
        for(i32 i=1; i<=n; ++i)
        {
            ...
            if(s[i-1]==')')
            {
                for(i32 t : {0,2})
                {
                    if(sbmax(dp[t|1],dp[t]+(amin==acc[i]+inc[t|1])))
                    {
                        (flip[t|1]=flip[t]).second=i-1;
                    }
                }
            }
            else
            {
                for(i32 t : {0,1})
                {
                    if(sbmax(dp[t|2],dp[t]+(amin==acc[i]+inc[t|2])))
                    {
                        (flip[t|2]=flip[t]).first=i-1;
                    }
                }
            }
            for(i32 t=0; t<4; ++t)
            {
                if(acc[i]+inc[t]<amin) dp[t]=-inf<i32>;
                else sbmax(dp[t],prv[t]+(amin==acc[i]+inc[t]));
            }
        }
        if(sbmax(ans,dp[0]))
        {
