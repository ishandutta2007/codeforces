...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (s1[i] != s2[i])
        {
            if (matrix[s1[i] - 'a'][s2[i] - 'a'] == 0)
            {
                matrix[s1[i] - 'a'][s2[i] - 'a'] = 1;
                graph[s1[i] - 'a'].pb(s2[i] - 'a');
                graph[s2[i] - 'a'].pb(s1[i] - 'a');
            }
        }
    }
    for (int i = 0; i < 26; i++)
    ...
    {
        //cout << color_count[i] << " ";
        ans += max(0, color_count[i] - 1);
    }
    cout << ans << endl;
    for (int i = 0; i < ansl.sz; i++)
    {
        cout << ansl[i].fi << " " << ansl[i].se << endl;
    }
}