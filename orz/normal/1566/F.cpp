...
{
	const int INF = 1e9 + 1;
	const ll SINF = 1e12 + 2;
	const ll SSINF = 1e18 + 2;
	const int MINF = -INF;
	int t;
	cin >> t;
	for (int I = 0; I < t; ++I)
	{
		...
		{
			p.Y = -p.Y;
			while (sz(w) && (w.back().Y >= p.X && w.back().Y >= p.Y))
				w.ob();
			w.pb(p);
		}
		int q = 0;
		...
		for (int i = 0; i < q; ++i)
		{
			if (into[i] == 0)
			{
				if (i)
				{
					rel(dp[0][i + 1], ((ll)z[i].X - z[i - 1].X) + dp[0][i]);
					rel(dp[1][i + 1], 2ll * ((ll)z[i].X - z[i - 1].X) + dp[1][i]);
					rel(dp[2][i + 1], dp[2][i]);
					rel(dp[3][i + 1], dp[3][i]);
				}
			}
