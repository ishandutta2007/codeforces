...
int main(){
  ...
  rep(i,H)rep(j,W){
    if(i)chmin(dist[i][j],dist[i-1][j]+1);
    if(j)chmin(dist[i][j],dist[i][j-1]+1);
  }
  brep(i,H)brep(j,W){
    if(i<H-1)chmin(dist[i][j],dist[i+1][j]+1);
    if(j<W-1)chmin(dist[i][j],dist[i][j+1]+1);
  }
  rep(i,H)rep(j,W){
    if(i)chmin(dist[i][j],dist[i-1][j]+1);
    if(j)chmin(dist[i][j],dist[i][j-1]+1);
