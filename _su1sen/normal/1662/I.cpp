...
void solve() {
    ...
    otera::Compressor<ll> ices(is), ps;
    vc<ll> len(n);
    rep(i, n) {
        int l = ices.max_leq(200LL * i, -INF);
        int r = ices.min_geq(200LL * i, INF);
        len[i] = min(abs(l - 200LL * i), abs(r - 200LL * i));
        debug(l, r, len[i]);
        ll pos = 200LL * i;
        ps.add(pos - len[i] + 1);
        ps.add(pos + len[i] - 1);
    ...
    rep(i, n) {
        if(len[i] == 0) continue;
        ll pos = 200LL * i;
        int l = ps.get(pos - len[i] + 1);
        int r = ps.get(pos + len[i] - 1);
        bit.add(l, r + 1, p[i]);
    }
    ll ans = 0;
    rep(i, sz) {
        chmax(ans, bit.sum(i, i + 1));
    }
    out(ans);
}
