...
void solve() {
    ...
    ll x;
    read(x);
    vector<int> dp(1 << LST, mod);
    dp[0] = 0;
    for (int i = 0; i < n; ++i) {
        vector<int> newdp(1 << LST, mod);
        for (int get = 0; get < 2; ++get) {
            for (int was = 0; was < (1 << LST); ++was) {
                int now = ((was << 1) | get) & ((1 << LST) - 1);
                bool flag = true;
                for (int l = 0; l < LST; ++l) {
                    ll sum = 0;
                    for (int r = l; r < LST; ++r) {
