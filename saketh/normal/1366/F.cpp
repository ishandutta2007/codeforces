...
int main() {
    ...
    F0R (d, M) {
		F0R (i, N) dp[d+1][i] = INT_MIN;

        F0R (i, N) {
            if (dp[d][i] == INT_MIN) continue;
            trav (e, g[i])
                ckmax(dp[d+1][e.f], dp[d][i] + e.s);
        }
    }
	...
	F0R (d, M + 1) {
		...
		F0R (i, N) {
			if (dp[d][i] != INT_MIN) {
				ckmax(high, dp[d][i]);
				lines.eb(max_adj[i], dp[d][i] - ll(d) * max_adj[i]);
			}
		}
		//pd(d, high);
		ans += high;
	...
	line_container_monotonic<ll> lc;
	trav (e, lines) lc.insert_line(e.f, e.s);

	ll LO = M + 1, HI = Q + 1;

	const mn HALF = mn(2).inv();

	F0R (i, sz(lc)) {
		ll lo = i>0 ? lc[i-1].l+1 : LO;
		ll hi = lc[i].l;
		if (hi < LLONG_MAX) hi++;
		...
		if (l < r) {
			/*for (ll d = l; d < r; d++) {
				ll get = lc[i].a * d + lc[i].b;
				ans += get;
			}*/

			ans += mn(lc[i].b) * (r - l);
			ans += mn(lc[i].a) * mn(r - 1 + l) * mn(r - l) * HALF;
		}
	}
