...
int main() {
    ...
    for (int jj = 0; jj < t; jj++) {
        ...
        for (int i = 0; i < n-6; i++) {
            ll fl = 0;
            for (int j = 0; j < 7; j++) {
                if (!(s[i + j] == h[j] || s[i + j] == '?'))fl = 1;
            }
            if (fl == 1)continue;
            if (!(i - 6 < 0)&&s.substr(i - 6, 6) == "abacab")continue;
            if (!(i - 4 < 0) && s.substr(i - 4, 4) == "abac")continue;
            if (!(i + 10 >= n) && s.substr(i+7 , 4) == "caba")continue;
            if (!(i + 13 >= n) && s.substr(i+7 , 6) == "bacaba")continue;
            ans = "Yes";
            for (int j = 0; j < 7; j++) {
                s[i + j] = h[j];
            }
