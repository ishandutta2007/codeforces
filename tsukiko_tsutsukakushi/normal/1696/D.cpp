...
void solve() {
    ...
    atcoder::segtree<P, op1, e1> rmax(v);
    atcoder::segtree<P, op2, e2> rmin(v);
    int cur = 0;
    int ans = 0;
    while(cur < n - 1) {
        auto check = [&](int x) -> bool {
            if(a[cur] < a[cur + 1]) {
                if((rmin.prod(cur, x + 1)).fr < a[cur]) return false;
            } else {
                if((rmax.prod(cur, x + 1)).fr > a[cur]) return false;
            }
            return true;
        };
        int ok = cur + 1, ng = n;
        ...
        cur = (a[cur] < a[cur + 1] ? (rmax.prod(cur, ok + 1)).sc : (rmin.prod(cur, ok + 1)).sc);
        ++ ans;
    }
    out(ans);
}
