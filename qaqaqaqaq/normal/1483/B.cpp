...
signed main(){
    ...
    while (t--){
        ...
        rep(i,n) q.push({i,(i+1)%n});
        while (sz(q)){
            auto [u,v]=q.front();
            q.pop();
            if (rem.find(u)==rem.end()||rem.find(v)==rem.end()) continue;
            if (__gcd(a[u],a[v])>1) continue;
            ans.pb(v);
            rem.erase(v);
            auto nxt=rem.lower_bound(v);
            if (nxt==rem.end()) nxt=rem.begin();
            q.push({u,*nxt});
        }
        cout<<sz(ans)<<" ";
