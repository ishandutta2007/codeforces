...
int main() {
   ...
   vector<vector<vector<int>>> dp(n + 1, vector<vector<int>>(n + 2, vector<int>(k + 2, -inf))); // dp[i][a][x] -> a a's, on char i, x moves used
   dp[0][0][0] = 0;
   int ans = 0;
   for (int i = 0; i <= n; ++i) {
      for (int a = 0; a <= n; ++a) {
         for (int x = 0; x <= k; ++x) {
            if (i < n) {
               int isA = s[i] == t[0];
               int isB = s[i] == t[1];
               mx(dp[i + 1][a + 1][x + !isA], dp[i][a][x]);
               mx(dp[i + 1][a][x + !isB], dp[i][a][x] + a);
               mx(dp[i + 1][a][x], dp[i][a][x]);
            } else {
               if (x <= k) {
                  mx(ans, dp[i][a][x]);
               }
