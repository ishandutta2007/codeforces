...
int main() {
    ...
    for (int i = 0; i<n; i++)
    {
        ...
        int idx = pos[i];
        X.insert(idx);
        inversions+=(i - X.order_of_key(idx));
        //idx -= i;
        if (i!=0 && idx>*prev(left.end())) {right.insert(idx); sum_right+=idx;}
        else {left.insert(idx); sum_left+=idx;}
        ...
        /*cout<<median<<' '<<inversions<<endl;
        cout<<sum_left<<' '<<sum_right<<endl;*/

        ll L = left.size();
        ll R = right.size();

        cout<< (median*L - sum_left - ((L-1)*L)/2) + (sum_right - median*R - ((R+1)*R)/2) + inversions<<' ';
    }
