...
inline fun BFS(dist:IntArray, NS:(Int)->IntArray): IntArray {
	val Q = fastDeque(0,dist.size)
	val n = dist.size
	for(i in 0 until n){
		if(dist[i] == 0){
			Q.addLast(i)
...
fun main(){
    solve.cases{
		...
		val k = getint
		val str = getstr
		val L = BooleanArray(n){str[it] == 'B'}

		val dist = IntArray(n){-1}
		for(i in 0 until n){
			val right = (i+1) % n
			val left = (i-1 + n) % n
			if(L[i] == L[left] || L[i] == L[right]){
				dist[i] = 0
			}
		}
		BFS(dist){
		...
		val ret = CharArray(n)
		for(i in 0 until n){
			val iters = minOf(dist[i],k)
			var final = L[i]
			if(iters %2 ==1 ){
				final = !final
			}
			ret[i] = if(final) 'B' else 'W'
		}
		put(ret.conca)
    }
    done()
