...
void solve() {
    ...
    vi parent(n, -1);
    set<int> cycle_vertices;

    auto find_cycle = [&](auto self, int cur) {
        trav (next, adj[cur]) {
            if (next == parent[cur])
                continue;
            if (parent[next] != -1) {
                cycle_vertices.insert(next);
                for (int i = cur; i != next; i = parent[i])
                    cycle_vertices.insert(i);
                return true;
            }
    ...
    parent[0] = 0;
    find_cycle(find_cycle, 0);

    union_find uf(n);
    for (int i = 0; i < n; i++) {
        trav (j, adj[i]) {
            if (cycle_vertices.count(i) && cycle_vertices.count(j))
                continue;
            uf.unio(i, j);
        }
    }
