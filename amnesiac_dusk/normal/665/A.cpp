...
void solve() {
	...
	for(int i=0; i<5; i++)
		stime[i]-='0';
	int night=24*60;
	int timin=stime[0]*600+stime[1]*60+stime[3]*10+stime[4];
	int timout=stime[0]*600+stime[1]*60+stime[3]*10+stime[4]+ta;
	int ans=0;
	for(int i=5*60; i<night; i++) {
		if((i-300)%b==0&&i<timout&&i+tb>timin) {
			ans++;
		}
	}
	cout<<ans<<endl;
