...
bool solve() {
	...
	for (int x : a) cou[x]++;
	for (int x : b) cou[x]++;
	debug(imie(a), imie(b), imie(cou));
	for (int i = 1; i <= n; ++i) if (cou[i] != 2) {
		return false;
	}
	vector <vpii> graf(n + 1);
	...
	for (int i = 1; i <= n; ++i) {
		if (!good[i]) {
			...
			do {
				good[wh] = true;
				for (auto [x, num] : graf[wh]) if (num != p) {
					if (a[num] == x) left.push_back(num);
					else right.push_back(num);
					debug(imie(wh), imie(p), imie(x));
					p = num;
					wh = x;
					break;
				}
			...
			for (int c : left) swaps.push_back(c);
		}
	}
	printf("%d\n", (int) swaps.size());
	for (int x : swaps) printf("%d ", x + 1);
	printf("\n");
	return true;
}
int main() {
