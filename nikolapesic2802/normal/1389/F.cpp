...
// URL : http://codeforces.com/contest/1389/problem/F
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int main()
{
	...
	map<int,int> ind;
	for(int i=0;i<m;i++)
		events.pb({points[i],{INT_MAX,i+1}}),ind[points[i]]=i+1;
	sort(all(events));
	for(auto p:events){
		if(p.s.f!=INT_MAX){
			int pos=ind[p.s.f]-1;
			dp[p.s.s^1].rangeAdd(0,pos,1);
		}
		else{
			dp[0].singleSet(p.s.s,dp[1].getMax(0,p.s.s-1));
			dp[1].singleSet(p.s.s,dp[0].getMax(0,p.s.s-1));
		}
	}
	printf("%i\n",max(dp[0].getMax(m,m),dp[1].getMax(m,m)));
	return 0;
}