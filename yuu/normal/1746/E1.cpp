...
#endif
int n;
bool ask(const vector<int>& input) {
    cout << "? ";
    cout << input.size();
...
void solve() {
    ...
    while (candidate.size() >= 4) {
        static vector<int> guess_1, guess_2, guess_A, guess_B;
        guess_1.clear();
        guess_2.clear();
        guess_A.clear();
        guess_B.clear();
        for (int i = 0; i < candidate.size(); i++)
            if (i & 1) guess_1.push_back(candidate[i]);
            else guess_2.push_back(candidate[i]);
        for (int i = 0; i < candidate.size(); i++)
            if (i & 2) guess_A.push_back(candidate[i]);
            else guess_B.push_back(candidate[i]);

        bool ans_1 = ask(guess_1);
        bool ans_A = ask(guess_A);
        candidate.clear();
        fill(good + 1, good + n + 1, 0);
        if (ans_1)
            for (auto&& x : guess_1) good[x] = 1;
        else
            for (auto&& x : guess_2) good[x] = 1;
        if (ans_A)
            for (auto&& x : guess_A) good[x] = 1;
        else
            for (auto&& x : guess_B) good[x] = 1;
        for (int i = 1; i <= n; i++)
            if (good[i]) candidate.push_back(i);
    }
    if (candidate.size() < 3)
        for (auto&& x : candidate) answer(x);
    ...
    if (is_a) {
        answer(a);
        // if a is not the actual answer, then they lied, and we have one guaranteed correct guess
        bool is_b = ask({b});
        if (is_b) answer(b);
        else answer(c);
    }
    ...
    // is_a == N must be true, otherwise we are lied to twice in a row
    answer(b);
    answer(c);
}
