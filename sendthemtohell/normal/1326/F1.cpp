...
inline void solve() {
	...
	for (int msk = 0; msk < (1 << n); ++msk) {
		...
		memset(cnt1, 0, sizeof cnt1); // 64 * 15 
		do {
			int ms = 0;
			for (int i = 0; i + 1 < pl; ++i) {
				ms |= (1 << i) * know[lp[i]][lp[i + 1]];
			}
			++cnt1[lp[pl - 1]][ms];
		} while (next_permutation(lp, lp + pl));


		memset(cnt2, 0, sizeof cnt2); // 64 * 15 
		do
		{
			int ms = 0;
			for (int i = 0; i + 1 < pr; ++i) {
				ms |= (1 << i) * know[rp[i]][rp[i + 1]];
			}
			++cnt2[rp[0]][ms];
		} while (next_permutation(rp, rp + pr));
		...
		for (int i = 0; i < pl; ++i) {
			...
			for (int j = 0; j < pr; ++j) {
				int fstr = rp[j];
				int o = 0;
				if (know[lstl][fstr]) o = (1 << (k - 1));

				for (int ml = 0; ml < (1 << (k - 1)); ++ml) {
					if (cnt1[lstl][ml] == 0) continue;
					for (int mr = 0; mr < (1 << (n - k - 1)); ++mr) {
						ans[ml | o | mr << k] += cnt1[lstl][ml] * cnt2[fstr][mr];
					}
				}
			}
		}
