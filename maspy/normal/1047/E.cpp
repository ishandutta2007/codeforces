...
void solve() {
  ...
  FOR_R(i, N) {
    ll v = hld.V[i];
    for (auto&& e: G[v]) A[v] += A[e.to];
  }
  sort(all(A));
  // k 
  auto check = [&](ll k) -> bool {
    if (A.back() % k != 0) return false;
    ll s = A.back() / k;
    if (UB(A, s) == LB(A, s)) return false;
    ll cnt = 0;
    if (k < 500) {
      FOR3(i, 1, k + 1) { cnt += UB(A, s * i) - LB(A, s * i); }
    } else {
      for (auto&& a: A) cnt += (a % s == 0);
    }
    return cnt == k;
  };
  vi can(N + 1);
  FOR3(k, 1, N + 1) can[k] = check(k);
  // print(can);
  vc<mint> DP(N + 1);
  FOR3_R(n, 1, N + 1) {
    if (!can[n]) continue;
    DP[n] = 1;
    FOR3(i, 2, N / n + 1) DP[n] += DP[n * i];
  }
  print(DP[1]);
}
