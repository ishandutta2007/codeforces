...
void up(){
	...
	int pos=0, kLeft=k;
	while(true){
		if(not std::binary_search(begin(a), end(a), pos)){
			if(kLeft==0) break;
			--kLeft;
		}
		++pos;
	...
	assert(not std::binary_search(begin(a), end(a), pos));
	std::map<int, int> count;
	for(int const index: range(
				int(std::lower_bound(begin(a), end(a), pos)-begin(a))
				, number)){
		++count[a[index]];
	}

	std::vector<std::pair<int, int>> items(begin(count), end(count));
	std::sort(begin(items), end(items), [&](auto const& first, auto const& sec){
		return first.second>sec.second;
	});

	kLeft=k;
	while(not items.empty() and items.back().second<=kLeft){
		kLeft-=items.back().second;
		items.pop_back();
	}
