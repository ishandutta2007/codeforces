...
fun main(){
    FACT.preCal(10005)
    fun pack(a:IntArray, k:Int):Int{
        val DP = IntArray(k+1)
        DP[0] = 1
        for(v in a){
            val new = DP.copyOf()
            for(i in 0 until k){
                new[i+1] = new[i+1] modPlus (v modM DP[i])
            }
            new.copyInto(DP)
        }
        return DP.last()
...
//    just dei pack(intArrayOf(3,4,5),2)
    ...
    solve.cases{
        ...
        for(i in 0 until n){
            ...
            G.root = i
            G.treeOrderDFS()
            val type = G.markcomponent()

            val have = Array(n){IntArray(n)}
            for(i in 0 until n){
                have[dist[i]][type[i]] ++
            }
            for((i,a) in have.withIndex()){
                if(i * k > n) break
                total = total modPlus pack(a,k)
            }
        }
        put(total)
