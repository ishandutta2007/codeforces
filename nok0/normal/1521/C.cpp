...
struct solver {
	...
	int ask(int t, int i, int j, int x) {
		x = min(x, n - 2);
		cnt++;
		cout << '?' << " " << t << " " << i + 1 << " " << j + 1 << " " << x + 1 << endl;
		if(test) {
			if(t == 1) {
				return max(min(x, res[i] - 1), min(x + 1, res[j] - 1));
			} else
				return min(max(x, res[i] - 1), max(x + 1, res[j] - 1));
		}
		INT(a);
		assert(a != -1);
		return a - 1;
	...
	void search(int i) {
		debug(i);
		if(perm[cir] < n / 2) {
			int v = ask(1, cir, i, n - 1);
			debug(v);
			if(v >= perm[cir] + 1) {
				perm[i] = v;
				return;
			}
			int nx = ask(2, i, cir, 0);
			perm[i] = nx;
			return;
		} else {
			int v = ask(2, i, cir, 0);
