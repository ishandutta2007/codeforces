#line 1 "codeforces-workspace/contest/1456/c/c.cpp"
/*
 * @file template.cpp
 * @brief Template
 */
...
#line 12 "codeforces-workspace/contest/1456/c/c.cpp"
...
void main() {
  ...
  vector<i64> cs(size(c) + 1);
  for (auto i = 0; i < size(c); ++i) {
    cs[i + 1] = cs[i] + (i / (k + 1) * c[i]);
  }
  vector<i64> sf(size(c) + 1), sf2(sf);
  for (auto i = size(c); i--;) {
    sf[i] = sf[i + 1] + c[i];
    sf2[i] = sf2[i + 1] + c[i] * i;
  }
  reverse(begin(sf), end(sf));
  // reverse(begin(sf2), end(sf2));

  i64 ans = -1e18;
  for (auto j = 0; j <= size(c); ++j) {
    if (j * (k + 1) < size(c)) continue;
    i64 cd = ceil_div(size(c) - j, k);
    i64 tp = j - cd;
    i64 now = ini * j;
    now -= sf2[size(c) - tp] + sf[tp] * (cd - (size(c) - tp));
    now -= cs[size(c) - tp];
    chge(ans, now);
  }
