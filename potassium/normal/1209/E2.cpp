...
int main() {
	...
	while (t--) {
		...
		vector<pii> toSort(m);
		for (int i = 0; i < m; i++) {
			toSort[i].F = *max_element(all(vv[i]));
			toSort[i].S = i;
		}
		sort(rall(toSort));
		if (m > n) m = n;
        ...
        vector<vector<int>> cost(m,vector<int>(4096+7,0));
        for (int i = 0; i < m; i++) {
            for (int mask = 0; mask < (1<<n); mask++) {
                for (int rot = 0; rot < n; rot++) {
                    ...
                    for (int j = 0; j < n; j++)
                        if ((mask >> j) & 1 == 1)
                            sum += v[i][(j+rot)%n];
                    cost[i][mask] = max(cost[i][mask],sum);
                }
            }
        }

		vector<vector<int>> dp(m+7,vector<int>(4096+7,0));
		dp[0][0] = 0;
		for (int i = 1; i <= m; i++) {
            for (int fmask = 0; fmask < (1<<n); fmask++) {
                for (int mask = fmask; ; mask = (mask-1) & fmask) {
                    int diff = fmask ^ mask;
                    dp[i][fmask] = max(dp[i][fmask], dp[i-1][mask] + cost[i-1][diff]);
                    if (mask == 0) break;
                }
            }
		}
