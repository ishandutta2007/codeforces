...
void solve() {
  ...
  std::vector<mint> dp1(n + 1), dp2(n + 1); // all, 2 way
  dp1[0] = 1;
  dp2[0] = 0;
  const mint inv2 = mint(1) / mint(2);
  for (const usize i : rep(1, n + 1)) {
    dp2[i] = dp1[i - 1] * (dp1[i - 1] + 1) * inv2;
    dp1[i] = 1 + dp1[i - 1] + dp2[i];
  }
  mint ans = 0;
  {
    const auto w3 = [&](const usize d) -> mint {
      mint ret = dp1[d];
      if (d > 0) {
        const mint t = dp1[d - 1];
        ret += t * (t - 1) * (t - 2) / 6;
        ret += t * (t - 1);
        ret += t;
      }
  ...
  {
    mint sum = 0;
    for (const usize b : revrep(1, n - 1)) {
      sum += dp2[n - b - 1];
      ans += dp2[b] * sum;
    }
  }
