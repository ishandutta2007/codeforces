...
void solve() {
    ...
    if (max({d12, d23, d31}) * 2 > A) {
        pd("triangle inequality");
        ps(NO);
        return;
    }
    ...
    for (int x = 1; x <= 3; x++) {
        if (x == root_node)
            continue;
        auto last_node = root_node;
        for (int i = 0; i < a[x] - 1; i++) {
            edges.emplace_back(next_node, last_node);
            last_node = next_node;
            next_node++;
        }
        edges.emplace_back(x, last_node);
    }
    while (next_node <= n) {
        edges.emplace_back(root_node, next_node);
        next_node++;
    }
    for (auto [u, v] : edges)
        ps(u, v);
