...
fun main(){
    solve.cases{
        ...
        val extra = IntArray(10)

        var now = str
        for(i in 0..9){
            val lastpos = now.indexOfLast { it - '0' == i }
            if(lastpos == -1) continue

            val first = now.slice(0..lastpos)
            val second = now.slice(lastpos + 1.. now.lastIndex)
