...
template<class T> struct point{
	...
	template<class U> explicit operator point<U>() const{ return point<U>(static_cast<U>(x), static_cast<U>(y), ind); }
...
int main(){
	...
	for(auto p: C){
		ctmin(res, max(1.0 * obj.x / p.x, 1.0 * obj.y / p.y));
	}
	long long temp = min(2e9 / obj.x, 2e9 / obj.y);
	line<long long> L(point<long long>(), point<long long>{obj.x * temp, obj.y * temp});
	for(auto i = 0; i < (int)C.size(); ++ i){
		int j = (i + 1) % (int)C.size();
		if(i == j) continue;
		auto p = C[i], q = C[j];
		auto [t, r] = intersect_closed_segments_no_parallel_overlap(L, line{p, q});
		if(t){
			ctmin(res, obj.norm() / r.norm());
		}
	}
