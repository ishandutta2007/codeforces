...
std::vector<mint> pow2(std::vector<mint> a) {
    int n = int(a.size());
    if(!n) return {};
    if(n <= 60) {
        std::vector<mint> ans(n + n - 1);
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < n; j++) { ans[i + j] += a[i] * a[j]; }
        }
        return ans;
    }
    int z = 1 << internal::ceil_pow2(n + n - 1);
    a.resize(z);
    internal::butterfly(a);
...
int main() {
    ...
    rep(i, k + 1) f[i] = 1;
    rep(t, n) {
        f = convolution::pow2(f);
        // f.resize(k + 1);
        // dump(f);
        if(t == n - 1) break;
        auto g = f;
        // Poly nf(k + 1);
        // rep(i, k * 2 + 1) rep(j, k + 1) { nf[min(i, j)] += f[i]; }
        per(i, k * 2) f[i] += f[i + 1];
        rep(i, k + 1) f[i] += g[i] * (k - i);
        f.resize(k + 1);
        // swap(f, nf);
    }
    if(!(m < f.size())) drop(0);
