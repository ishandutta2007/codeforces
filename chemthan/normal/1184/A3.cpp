...
void chemthan() {
    ...
    for (auto p : pr) {
        ...
        int z = PRT::query(p);
        vi r = FFT::chirpz_transform(a, z, p, p);
        FOR(i, 0, sz(r)) if (!r[i]) {
            int x = fpow(z, i + i, p);
            if (1 < x && x < p - 2) {
                cout << p << " " << x << "\n";
        ...
        FOR(i, 0, sz(a)) {
            a[i] = mult(a[i], fpow(z, i, p), p);
        }
        r = FFT::chirpz_transform(a, z, p, p);
        FOR(i, 0, sz(r)) if (!r[i]) {
            int x = fpow(z, i + i + 1, p);
            if (1 < x && x < p - 2) {
                cout << p << " " << x << "\n";
                return;
            }
