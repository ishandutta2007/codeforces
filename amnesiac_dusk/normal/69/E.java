...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		int n=ni(),k=ni();
		int[]a=nia(n);
		Map<Integer, TreeSet<Integer>> ho=new HashMap<Integer, TreeSet<Integer>>();
		Map<Integer,Integer> hola=new HashMap<Integer, Integer>();
		for(int i=0; i<k-1; i++)
		{
			int te=hola.getOrDefault(a[i],0);
			if(te!=0)
				ho.get(te).remove(a[i]);
			hola.put(a[i], te+1);
			TreeSet<Integer> lol=ho.getOrDefault(te+1, new TreeSet<Integer>());
			lol.add(a[i]);
			ho.put(te+1, lol);
		}
		for(int i=k-1; i<n;)
		{
			...
			lol.add(a[i]);
			ho.put(te+1, lol);
			if(ho.containsKey(1)&&ho.get(1).floor(1000000000)!=null)
				pr(ho.get(1).floor(1000000000));
			else
				pr("Nothing");
			i++;
			te=hola.get(a[i-k]);
			ho.get(te).remove(a[i-k]);
			hola.put(a[i-k], te-1);
			TreeSet<Integer> hol=ho.getOrDefault(te-1, new TreeSet<Integer>());
			hol.add(a[i-k]);
			ho.put(te-1, hol);
		}
