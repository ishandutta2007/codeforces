...
void test() {
	...
	if (xlim%2) {
		for (int y=0;y<ylim-1;y+=2) {
			...
			for (int ny=y;ny<=y+1;ny++) {
				if (a[xlim-1][ny]) v.PB(MP(xlim-1,ny));
			}
			vpii o; for (int ny=y;ny<=y+1;ny++) o.PB(MP(xlim-2,ny));
			if (v.size()>0) {
				for (auto &w:v) go(w);
				assert(v.size()<=2);
				for (int i=0;i<3-int(v.size());i++) go(o[i]);
			}
		}
		assert(final.size()%3==0);
	}
	if (ylim%2) {
		for (int x=0;x<xlim-1;x+=2) {
			vpii v; for (int nx=x;nx<=x+1;nx++) if (a[nx][ylim-1]) v.PB(MP(nx,ylim-1));
			vpii o; for (int nx=x;nx<=x+1;nx++) o.PB(MP(nx,ylim-2));
			if (v.size()>0) {
				for (auto &w:v) go(w);
				for (int i=0;i<3-int(v.size());i++) go(o[i]);
			}
