...
int val(int x) { return dp[max(0, x - 2)] - x + 1; }

void mian() {
	n = read();
	REP(i, 1, n) a[i] = read();
	X = read();
	lft1[1] = 1; REP(i, 2, n) lft1[i] = a[i] + a[i - 1] >= 2 * X ? lft1[i - 1] : i;
	lft2[1] = lft2[2] = 1; REP(i, 3, n) lft2[i] = a[i] + a[i - 1] + a[i - 2] >= 3 * X ? lft2[i - 1] : i - 1;
	static int q[N], head, tail; head = tail = 0;
	REP(i, 1, n) {
		int lft = max(lft1[i], lft2[i]);
			while(head < tail && val(q[tail - 1]) <= val(i)) --tail;
		q[tail++] = i;
		while(head < tail && q[head] < lft) ++head;
		dp[i] = dp[i - 1];
		chkmx(dp[i], val(q[head]) + i);
	}
	prt(dp[n]), pc('\n');
}
