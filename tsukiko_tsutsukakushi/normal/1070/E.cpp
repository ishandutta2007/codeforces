...
void solve() {
    ...
    for (auto [val, ls] : pos) {
        ...
        while (true) {
            ...
            int x = seg.max_right(0, [&](int e) { return e <= q * m; });
            long long sum = ft.sum(0, x) + ft.sum(0, n);
            if (sum > t) {
                int last_pos = seg.max_right(0, [&](int e) { return e < cnt; });
                seg.set(last_pos, 0);
                ft.add(last_pos, -p[last_pos]);
            } else break;
        }
        if (chmax(ans_c, seg.all_prod())) {
            ans_d = val;
        }
    }
    print(ans_c, min(t, (long long) ans_d));
}
