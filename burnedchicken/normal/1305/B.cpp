...
int main(){
	...
	cin >> str;
	n=sz(str);
	rep(n) if(str[i]==')') dq2.push_back(i+1),cnt2++;
	rep(n){
		if(cnt1==cnt2) break;
		if(str[i]=='(') dq1.push_back(i+1),cnt1++;
		else dq2.pop_front(),cnt2--;
	}
	if(!cnt1) print1(0);
	else{
		print1(1);
		print1(cnt1<<1);
		rep(cnt1) print0(dq1.front()),dq1.pop_front();
		rep(cnt1) print0(dq2.front()),dq2.pop_front();
	}
	return 0;
}