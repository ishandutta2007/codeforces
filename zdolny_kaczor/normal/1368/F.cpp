...
int main() {
	...
	debug(imie(R), imie(k));
	REP(i, n) if (i % k == 0) blocked[i] = true;
	debug(range(blocked, 0, n));
	while (true) {
		debug(range(curr, 0, n));
		int cou = 0;
		REP(i, n) cou += curr[i];
		if (cou >= R) done("0\n");
		vector <int> move;
		REP(i, n) if ((int)move.size() < k && !curr[i] && !blocked[i]) move.push_back(i);
		assert((int) move.size() == k);
		printf("%d", k);
		for (int x : move) printf(" %d", x + 1);
		for (int x : move) curr[x] = 1;
		printf("\n");
		fflush(stdout);
		int c;
		scanf("%d", &c);
