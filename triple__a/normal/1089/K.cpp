...
signed main(){
    ...
    vi hi(maxn,0);
    rep(i,maxn) hi[i]=-i;
    lazy_segtree<S,op,e,F,mapping,composition,id> seg(hi);
    BIT C(maxn);
    rep1(i,q){
        char c;
        cin>>c;
        ...
        if (c=='?'){
            int t;
            cin>>t;
//            for (int i=1;i<=t;++i) cout<<seg.get(i)<<" ";
//            cout<<endl;
            int mn=seg.prod(0,t+1);
//            cout<<mn<<endl;
            int ft=seg.max_right(0,[&](S l){return l>mn;});
            int sum=C.query(t)-C.query(ft-1);
//            cout<<sum<<" "<<ft<<" "<<t<<endl;
            cout<<sum+ft-t<<"\n";
        }
    }
