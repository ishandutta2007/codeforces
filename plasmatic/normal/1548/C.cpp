...
const int mod = 1e9 + 7, MV = 3000020;
...
int main() {
    ...
    // r.a = {1, 3, 3, 1};
    vector<ll> cur(3*(N+1)+1);
    cur[0] = 1;
    for (auto i = 1; i <= 3*(N+1); i++) {
        cur[i] = (cur[i-1] * (3*(N+1)-i+1) % mod) * inv[i] % mod;
    }
    cur[0]--; fix(cur[0]);
    cur[1] -= 3; fix(cur[1]);
    cur[2] -= 3; fix(cur[2]);
    ...
    for (auto i = 3*(N+1); i >= 3; i--) {
        // cout<<"i="<<(i)<<", "; cout<<"cur[i]="<<(cur[i])<<", "; cout<<"cur[i-1]="<<(cur[i-1])<<", "; cout<<"cur[i-2]="<<(cur[i-2])<<", "; cout << endl; // db i,cur[i],cur[i-1],cur[i-2]
        fix(cur[i]);
        cur[i-1] -= 3*cur[i];
        cur[i-2] -= 3*cur[i];
    }

    // cout << "cur=["; for (auto __x:cur)cout<<__x<<", "; cout<<"], "; cout << endl; // db I:cur
