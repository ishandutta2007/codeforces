...
void mian() {
    ...
    REP(n, 0, nn) {
        if(n == 0) {
            ...
            REP(m, 1, nn + 1) {
                f[n][m] = h[n - 1][nn + 1][n][m - 1];
            }
        }
        REP(n1, 0, n) REP(m1, 1, nn + 1) REP(m2, 1, nn + 1)
            addto(g[n][min(m1, m2)], (ll)f[n1][m1] * f[n - n1][m2] % P);
        REP(m, 1, nn + 1) {
            auto hh = h[m == 1 ? n - 1 : n][m == 1 ? nn + 1 : m - 1];
            REP(a, 0, nn) REP(b, 0, nn + 1) {
                int lim = min(a / (n + 1), b / m);
                int pw = 1;
                REP(i, 0, lim) {
                    int cmb = (ll)pw * ifc[i] % P;
                    addto(h[n][m][a][b], (ll)hh[a - i * (n + 1)][b - i * m] * cmb % P);
                    pw = (ll)pw * (g[n][m] + i) % P;
                }
            }
        }
