...
void solve() {
	...
	for (int i = n; i >= 1; --i) {
		if (gogo[i].empty() == true) continue;
		int diffs = gogo[i][0] + 1;
		for (int q = 1; q < gogo[i].size(); ++q) {
			diffs = max(diffs, gogo[i][q] - gogo[i][q - 1]);
		}
		diffs = max(n - gogo[i].back(), diffs);
		ans[diffs] = min(ans[diffs], i);
	}
	for (int i = 1; i <= n; ++i) {
