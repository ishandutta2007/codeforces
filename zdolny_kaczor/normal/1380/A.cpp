...
void solve() {
	...
	vi a(n);
	REP(i, n) scanf("%d", &a[i]);
	vpii su(n, pii(inf, -1)), pr(n, pii(inf, -1));
	REP(i, n - 1) pr[i + 1] = min(pr[i], pii{a[i], i});
	for (int i = n - 1; i; --i) su[i - 1] = min(su[i], pii(a[i], i));
	REP(i, n) {
		if (a[i] > pr[i].first && a[i] > su[i].first) {
			printf("YES\n%d %d %d\n", pr[i].second + 1, i + 1, su[i].second + 1);
			return;
		}
	}
	printf("NO\n");
