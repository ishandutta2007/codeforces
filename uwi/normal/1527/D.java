...
public class D {
	...
	void go()
	{
		...
		for(int i = 1;i < n;i++){
			if(i == 1){
				int an = lf.anc(i, dep[i]-1);
				fr -= des[an];
			}
			if(amd == 0){
				amd = i;
			}else{
				if(dep[amd] < dep[i] && lf.anc(i, dep[i] - dep[amd]) == amd){
					amd = i;
				}else if(dep[amd] >= dep[i] && lf.anc(amd, dep[amd] - dep[i]) == i) {
				}else if(dep[amd2] < dep[i] && lf.anc(i, dep[i] - dep[amd2]) == amd2 && lf.lca(i, amd) == 0) {
					amd2 = i;
				}else if(dep[amd2] >= dep[i] && lf.anc(amd2, dep[amd2] - dep[i]) == i) {
				}else{
					break;
				}
			}
			if(amd2 == 0) {
				nums[i + 1] = (long) fr * des[amd];
			}else{
				nums[i + 1] = (long) des[amd2] * des[amd];
			}
		}
		for(int i = 0;i <= n;i++){
			out.print(nums[i] - nums[i+1]).print(" ");
		}
		out.println();
	}
