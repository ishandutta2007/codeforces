...
int main() {
	...
	for (int i = 0; i < n; i++) {
		v1.emplace_back('a' <= s[i] && s[i] <= 'z');
		v2.emplace_back('A' <= s[i] && s[i] <= 'Z');
	}
	...
	int bestv, bestc;
	auto go = [&](int C, vector<int>& v) {
		for (int i = 1; i <= n; i++) {
			dp[i] = dp[i - 1];
			used[i] = used[i - 1];
			
			int L = max(0, i - l);
			if (chkmax(dp[i], dp[L] + v[i] - v[L] - C))
				used[i] = used[L] + 1;
		}
		
		bestv = bestc = -1;
		for (int i = 1; i <= n; i++) {
			if (chkmax(bestv, dp[i]))
				bestc = used[i];
			else if (bestv == dp[i])
				chkmin(bestc, used[i]);
		}
