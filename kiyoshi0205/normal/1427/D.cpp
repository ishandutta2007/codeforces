...
int main(){
  ...
  rep(K,N-1){
    rep(j,N)if(v[j]==K+1)break;
    //j is K+1 position
    vec ch;
    if(!f){
      //   opos,opos+1,,opos+K
      //...012...
      if(j==opos+K+1)continue;
      if(j<opos){
        if(j)ch.eb(j);
        ch.eb(1);
        ch.eb(opos+K-j);
        if(opos+K<N-1)ch.eb(N-1-opos-K);
      }else{
        f^=1;
        if(opos)ch.eb(opos);
        rep(i,K+1)ch.eb(1);
        ch.eb(j-opos-K);
        if(j<N-1)ch.eb(N-1-j);
      }
    }else{
      //...210...
      if(j==opos-K-1)continue;
      if(j<opos){
        ...
        ch.eb(opos-K-j);
        rep(i,K+1)ch.eb(1);
        if(opos<N-1)ch.eb(N-1-opos);
      }else{
        if(opos-K)ch.eb(opos-K);
        ch.eb(K+1);
        ch.eb(j-opos);
        if(j<N-1)ch.eb(N-1-j);
      }
