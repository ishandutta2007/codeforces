...
int main() {
    ...
    rep(i, n) {
        if(s.count(i)) {
            ...
            while(!empty(q)) {
                int now = q.front();
                q.pop();
                for(auto it = s.begin(); it != end(s);) {
                    if(edges.count(pii(now, *it)))
                        it++;
                    else {
                        uf.unite(i, *it);
                        g[i].eb(*it, 0);
    ...
    int use = n - uf.num;
    int muda = 0;
    if(use < (ll)(n) * (n - 1) / 2 - m) muda = 1;
    if(muda) {
        sort(all(E));
        ll ans = 0;
        for(auto e : E) {
            if(uf.unite(e.from, e.to)) ans += e.cost;
        }
        cout << ans << endl;
    } else {
        ll ans = X;
        ...
        for(auto e : E) {
            if(uf.unite(e.from, e.to)) {
                ans += e.cost;
                g[e.from].eb(e.to, 1), g[e.to].eb(e.from, 1);
            } else
                w.eb(e.from, e.to, e.cost);
        }
