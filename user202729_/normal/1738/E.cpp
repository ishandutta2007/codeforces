...
auto comb=makeComb<modular>(endN);
...
void up(){
	...
	auto const sum=p.back();
	p.pop_back();

	auto const countp=[&](int64_t x){
		auto [a, b]=std::equal_range(begin(p), end(p), x);
		assert(a<=b);
		return int(b-a);
	};
	...
	modular result=
		modular(2).pow(
				sum%2==0 ? countp(sum>>1) : 0
				);

	int last=0;
	for(int const index: range(1, int(p.size())+1)){
		if(index==int(p.size()) or p[index]!=p[index-1]){
			int curCount=index-last;
			auto value=p[index-1];
			if(value*2>=sum) break;
			last=index;

			assert(curCount==countp(value));
			int flipCount=countp(sum-value);
			if(flipCount>0){
				result*=comb(curCount+flipCount, curCount);
			}
		}
	}
	...
	if(number<15){
		int result1 {};
		for(int const mask: range(0, 1<<(number-1))){
			std::vector<int64_t> pieces{a[0]};
			...
			if(std::equal(begin(pieces), end(pieces), pieces.rbegin())){
				result1++;
			}
		}

		assert(result1==int(result));
	}
#endif
}
