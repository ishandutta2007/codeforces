...
int main() {
    ...
    mint sum = 1;
    for (int i = 1; i <= n; i++) {
        dp[i] = sum * BINOM.inv(i);
        sum += dp[i];
        if (i - (n - 1) / 2 >= 0) sum -= dp[i - (n - 1) / 2];
    }

    vector<mint> L(n, 0), R(n, 0);
    for (int i = (n - 1) / 2; i < n - 1; i++) L[n - 1 - i] = BINOM.fac(n - 2 - i) * dp[i];
    for (int i = (n - 1) / 2; i < n; i++) R[i] = BINOM.finv(n - 1 - i);
    auto v = atcoder::convolution(L, R);

    auto calc = [&](int i) -> mint {
        if (n - 1 - i < (n - 1) / 2) return 0;
        mint res = BINOM.fac(i) * BINOM.fac(n - 1 - i) * dp[n - 1 - i];
        res += v[n - 1 + i] * i * BINOM.fac(n - 1 - i);
        if (i - 1 >= 0) res -= BINOM.fac(i - 1) * dp[n - 1 - i] * i * BINOM.fac(n - 1 - i);
        return res;
    };
    for (int i = 0; i < n; i++) cout << calc(i) << (i + 1 == n ? '\n' : ' ');
    return 0;
}