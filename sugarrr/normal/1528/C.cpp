...
using S=ll;
S op(S a,S b){return S{max(a,b)};}
S e(){return 0;}//-infseg0
using F=ll;
S mapping(F f,S x){return S{f+x};}
...
void solve(){
   ...
   // rep(i,1,n)cerr<<t[i].fi<<" "<<t[i].se<<endl;cerr<<endl;
    auto print=[&](){
        rep(i,0,sz-1)cerr<<seg.get(i)<<" ";cerr<<endl;
    };
    ll end=0;
    ll ans=0;
    auto dfs=[&](ll pos,auto dfs)->void{
        ...
        ll val=seg.get(A.fi);
        if(val==0){
            assert(seg.prod(A.fi,A.se+1) == 0);
            seg.apply(A.fi,A.se+1,pos);
            ans++;
            chmax(end,ans);
        }else{
            l_l B=t[val];
            assert(B.fi<A.fi && A.se<B.se && val<pos);
            seg.apply(B.fi,B.se+1,-val);
            seg.apply(A.fi,A.se+1,pos);
        }
        ...
        assert(seg.prod(A.fi,A.se+1)==pos);
        seg.apply(A.fi,A.se+1,-pos);
        ans--;
