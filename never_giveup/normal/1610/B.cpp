...
const int maxn = 2e5 + 5000, maxw = 1e6 + 10, inf = 1e9 + 100, LG = 17, SQ = 500, md = 1e9 + 993;
...
void solve() {
    ...
    reverse(all(a));
    ha y(a);
    auto get = [&](ha const &h, vector<int> &v) {
        v.push_back(n);
        int l = -1;
        int x = 0;
        for (int i : v) {
            int len = i - l - 1;
            if (len > 0) {
                x = (x * (ll)h.g[len] + h.get(l + 1, i - 1)) % md;
            }
            l = i;
        }
        v.pop_back();
