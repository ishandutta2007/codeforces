...
struct ranges{
  ...
  bool inters(pii p)const{
    auto it=S.upper_bound(mp(p.Y,+INF*2));
  ...
  void add(pii p){
    auto it2=S.upper_bound(mp(p.Y+1,+INF*2));
    while(it2!=S.begin()){
      --it2;
      assert(it2->X<=p.Y+1);
      if(it2->Y<p.X-1) break;
      p.X=it2->X;
      it2=S.erase(it2);
    }
    assert(!inters(p));
...
bool doit(){
  ...
  for(const auto&p:R){
    E.pb(mp(mp(p.Y.X,-1),p.X));
    E.pb(mp(mp(p.Y.Y,+1),p.X));
  }
  sort(all(E));
  ranges r;
  for(auto&e:E){
    if(e.X.Y==-1){
      if(r.inters(e.Y)) return false;
    }else{
      r.add(e.Y);
    }
  }
...
int main(int argc, char **argv){
  ...
  cin>>n;
  R.resize(n);
  forn(i,n) cin>>R[i].X.X>>R[i].X.Y>>R[i].Y.X>>R[i].Y.Y;
  forn(q,2){
    dbg(R);
    if(!doit()){
      cout<<"NO";
