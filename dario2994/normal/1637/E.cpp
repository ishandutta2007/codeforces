...
void solve() {
    ...
    vector<int> counters;
    for (const auto& pp: cnt) counters.push_back(pp.second);
    sort(all(counters));
    counters.erase(unique(all(counters)), counters.end());
    int k = counters.size();
    vector<vector<int>> buckets(k);
    for (const auto& pp: cnt) {
        int c = pp.second;
        int i = lower_bound(all(counters), c) - counters.begin();
        buckets[i].push_back(pp.first);
    }
    // dbg(counters, buckets);

    auto make_hash = [&](ULL x, ULL y) {
        return (x < y) ? (x ^ (y << 30)) : (y ^ (x << 30));
    };

    unordered_set<ULL> forbidden;
    for (const auto& pp: cnt) {
        int x = pp.first;
        forbidden.insert(make_hash(x, x));
    }
    for (int i = 0; i < m; i++) {
        int x, y;
        cin >> x >> y;
        forbidden.insert(make_hash(x, y));
    }
    ...
    for (int i = 0; i < k; i++) for (int j = i; j < k; j++) {
        auto& a = buckets[i];
        auto& b = buckets[j];
        LL coef = counters[i] + counters[j];
        int best = 0;
        int best_b = -1;
        for (int ita = SZ(a)-1; ita >= 0; ita--) {
            for (int itb = SZ(b)-1; itb > best_b; itb--) {
                if (!forbidden.count(make_hash(a[ita], b[itb]))) {
                    best = max(best, a[ita] + b[itb]);
                    best_b = itb;
                }
            }
            if (best_b == SZ(b)-1) break;
        }
        res = max(res, coef * best);
    }
    cout << res << "\n";
}
