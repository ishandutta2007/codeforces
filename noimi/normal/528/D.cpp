...
int main() {
    ...
    rep(i, n) {
        rep(j, 4) {
            if(s[i] == S[j]) can[j][max(0LL, i - k)]++, can[j][min((ll)n, i + k + 1)]--;
        }
    }
    rep(i, 4) rep(j, n) can[i][j + 1] += can[i][j];
    vi ans(n, 1);
    rep(i, 4) {
        vmint f(m + 1);
        rep(j, m) if(t[j] == S[i]) f[m - j] = 1;
        vmint g(n);
        rep(j, n) if(!can[i][j]) g[j] = 1;
        f = f * g;
        rep2(j, m, m + n - 1) if(f[j].a) ans[j - m] = 0;
    }
    cout << accumulate(ans.begin(), ans.begin() + n - m + 1, 0) << endl;
}