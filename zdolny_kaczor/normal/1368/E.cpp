...
void solve() {
	...
	auto ban = [&](int x) {
		if (rem[x]) return;
		rem[x] = true;
		for (int v : rgraf[x]) graf[v].erase(x);
	};
	for (int i = 1; i <= n; ++i) {
		bool has = false;
		for (int x : rgraf[i]) if (!rem[x]) for (int y : rgraf[x]) if (!rem[y]) has = true;
		if (has) rem[i] = true;
	}
	vi ans;
	for (int i = 1; i <= n; ++i) if (rem[i]) ans.push_back(i);
	printf("%d\n", (int) ans.size());
	for (int x : ans) printf("%d ", x);
	printf("\n");
	assert((int) ans.size() <= 4 * n / 7);
	for (int i = 1; i <= n; ++i) for (int x : graf[i]) for (int y : graf[x]) assert(rem[i] || rem[x] || rem[y]);
}
int main() {
