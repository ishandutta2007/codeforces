...
int main() {
	CASE(t) {
		...
		while (p > 1) {
			factors.emplace_back(sieve.mpf[p]);
			p /= sieve.mpf[p];
		}
		...
		const int FULL = 1 << n;
		vector<int> maskVal(FULL, 1);
		vector<int> maskMul(FULL, 1);
		for (int mask = 1; mask < FULL; mask++) {
			maskMul[mask] = (__builtin_popcount(mask) % 2 == 0 ? 1 : -1);
			int low = mask & -mask;
			int last = mask ^ low;
			maskVal[mask] = maskVal[last] * factors[__lg(low)];
		}

		auto count = [&](ll x) {
			ll ans = 0;
			for (int mask = 0; mask < FULL; mask++)
				ans += maskMul[mask] * x / maskVal[mask];
			return ans;
		};
