...
int main(){
	...
	queue<int> q;
	vector<int> topo;
	for(int i=1;i<=m;++i) scan(u,v),adj[u].pb(v),in[v]++;
	for(int i=1;i<=n;++i) if(!in[i]) q.push(i);
	while(q.size()){
		u=q.front(); topo.pb(u); q.pop();
	...
	for(u=n;u>=0;--u) if(layer[u]) {
		puts("WIN");
		for(v=1;v<=n;++v) if(sg[v]==u&&(h[v]^layer[u])<h[v]) break;
		h[v]^=layer[u]; layer[u]=0;
