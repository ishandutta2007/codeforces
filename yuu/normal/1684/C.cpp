...
void solve() {
    cin >> n >> m;
    for (int i = 0; i < n; i++) a[i] = &buffer[i * m];
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) cin >> a[i][j];
    bool sorted = true;
    int r = 0;
    for (r = 0; r < n; r++) {
        sorted = is_sorted(a[r], a[r] + m);
        if (!sorted) break;
    }
    if (sorted) {
        cout << "1 1\n";
    ...
    f[m - 1] = m - 1;
    for (int i = m - 2; i >= 0; i--)
        if (a[r][i] <= a[r][i + 1]) f[i] = f[i + 1];
        else f[i] = i;
    auto good = [&](int i, int j) -> bool {
        if (i == j) return false;
        if (i > j) swap(i, j);
        if (f[0] < i - 1) return false;
        if (i && (a[r][j] < a[r][i - 1])) return false;
        if (i + 1 == j) {
            if (a[r][i] < a[r][j]) return false;
        } else {
            if (a[r][j] > a[r][i + 1]) return false;
            if (a[r][i] < a[r][j - 1]) return false;
            if (f[i + 1] < j - 1) return false;
        }
        if ((j + 1 < m) && (a[r][i] > a[r][j + 1])) return false;
        if ((j + 1 < m) && (f[j + 1] < m - 1)) return false;
        return true;
    };
    ...
    for (auto&& x : bad) {
        for (int i = 0; i <= 1; i++) {
            ...
            for (int j = 0; j < m; j++) {
                if (good(x - i, j)) {
                    guesses.emplace_back(x - i, j);
                }
            }
        }
    }
    for (auto&& [x, y] : guesses) {
        if (test(x, y)) {
            cout << x + 1 << ' ' << y + 1 << '\n';
            return;
        }
