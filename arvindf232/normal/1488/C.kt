...
fun cuts(l:Int, r:Int, here:Int):Int {
    val rr = r - here
    val ll = here - l
    return minOf(2 * rr + ll, rr + 2 * ll )
}
const val singleCase = false
fun main(){
    solve.cases{
        var (n,x,y) = getline(3)
        x = minOf(x,y).also { y = maxOf(x,y) }
        var best = Int.MAX_VALUE
        for(cut in 1..n){
            if(x !in 1..cut || y !in (cut+1)..n){
                continue
            }
            best = minOf(best, maxOf(cuts(1,cut,x), cuts(cut+1,n,y) ))
