...
void up(){
	...
	while(not a.empty()){
		...
		// found i=index of second-last equal to a.back()
		for(int const j: range(0, int(a.size())-2-i)){
			ops.push_back({
				i+j, a.rbegin()[j+1]
			});
		}

		lensRev.push_back((int(a.size())-1-i)*2);
		a.erase(a.begin()+i);
		a.pop_back();
	...
	std::cout<<ops.size()<<'\n';
	for(auto [a, b]: ops) std::cout<<a<<' '<<b<<'\n';

	std::cout<<lensRev.size()<<'\n';
	for(int index=int(lensRev.size()); index--;)
		std::cout<<lensRev[index]<<' ';
	std::cout<<'\n';
