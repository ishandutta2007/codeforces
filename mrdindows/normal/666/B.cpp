...
vector<int> v[5005];
int s[3003][3003];
vector<pii> trimax[3003];
vector<pii> revmax[3003];
bool used[3003];
int n, m;
int q[3003];
int qs = 0;
...
void dej(int x) {
    ...
    for (int iter = 0; iter < 3; ++iter) {
        ...
        trimax[x].push_back({r, d[r]});
        d[r] = -INF;
    }
    reverse(all(trimax[x]));
    for (auto p : trimax[x]) {
        d[p.first] = p.second;
    }
}
int main() {
    ...
    for (int x = 0; x < n; ++x) {
        for (int iter = 0; iter < 3; ++iter) {
            ...
            revmax[x].push_back({r, s[r][x]});
            s[r][x] = -INF;
        }
        reverse(all(revmax[x]));
        for (auto p : revmax[x]) {
    ...
    for (int a = 0; a < n; ++a) {
        for (int b = 0; b < n; ++b) {
            if (a == b) continue;
            int res = s[a][b];
            for (auto p1 : revmax[a]) {
                for (auto p2 : trimax[b]) {
                    if (p1.first == a || p2.first == b || p1.first == b || p1.first == p2.first || p2.first == a) continue;
                    int cur = res + p1.second + p2.second;
                    if (cur > ans) {
                        ans = cur;
                        vans = {p1.first, a, b, p2.first};
                    }
                }
            }
        }
