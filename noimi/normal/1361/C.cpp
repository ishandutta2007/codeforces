...
signed main(){
    ...
    while(abs(ok-ng) > 1){
        ...
        int s = 0;
        rep(i,N) if(uf.find(i) == i and cnt[i]) s ++;
        if(s > 1) flag = false;
        if(flag) ok = mid;
        else ng = mid;
    ...
    rep(i,n){
        ...
        a%=N,b%=N;
        if(a > b){
            mp[{b,a}].eb(i*2+2,i*2+1);
        }
        else mp[{a,b}].eb(i*2+1,i*2+2);
        ed.eb(a,b,0);
        st = a;
    }
    auto P = eulerian_path(ed,st,false);
    for(auto e:P){
        int a = e.src, b = e.to;
        if(a > b){
            auto [x,y] = mp[{b,a}].back();
            mp[{b,a}].pop_back();
            cout << y << " " << x << " ";
        }
        else{
            auto [x,y] = mp[{a,b}].back();
            mp[{a,b}].pop_back();
            cout << x <<" " << y <<" ";
        }
    }
