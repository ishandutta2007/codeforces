...
ll N,M,K,H,W,dp[501][501],powlis[501][501];
ll ans;
int main(){
  startupcpp();
  ll i,j,k;
  cin>>N>>K;
  rep(i,501)rep(j,501)powlis[i][j]=modpow(i,j);
  rep(i,K)dp[1][i+1]=i+1;
  For(i,2,N+1){
    For(j,i,K+1){
      rep(k,i)(dp[i][j]+=dp[i-k][j-(i-1)]*modncr(i,k)%mod*powlis[i-1][k])%=mod;
    }
  }
  print((modpow(K,N)-dp[N][K]+mod)%mod);
}