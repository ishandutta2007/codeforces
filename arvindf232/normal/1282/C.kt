...
class problem( val type:Int, val time:Int){
    ...
    solve.cases{
        ...
        val time = getline(n)
         ...
         val all = MutableList(n){
             problem(types[it], time[it] )
         }
        all.sortBy { it.time }
        var typesleft = IntArray(2)
        for(a in all){
            typesleft[a.type] ++
        }
        val costs = intArrayOf(a,b)
        var timenow = 0
        var pointsNow = 0
        var max = 0
        val Q  = ArrayDeque<problem>()
        Q.addAll(all)
        var pass = false
        fun check(){
            var timeleft = minOf(t,(if(Q.isEmpty()) Int.MAX_VALUE else Q.first().time - 1)) - timenow
            var pnow = pointsNow
            for(i in 0 until 2){
                val solves = minOf(typesleft[i], timeleft / costs[i])
                pnow += solves
                timeleft -= solves * costs[i]
            }
            max = maxOf(pnow,max)
        }
        check()
        outer@while(Q.isNotEmpty()){
            pass = true
            while(Q.isNotEmpty() && (Q.first().time <= timenow || pass)){
                pass = false
                val here = Q.removeFirst()
                typesleft[here.type] --
                pointsNow ++
                timenow += costs[here.type]
                if(timenow > t) break@outer
            }
            check()
        }
        put(max)
