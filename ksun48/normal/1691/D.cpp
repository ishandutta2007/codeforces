...
void solve(){
	...
	auto amax = RangeMaxQuery<pair<ll, int> >(pairs);
	vector<ll> psums(n+1, 0);
	for(int i = 0; i < n; i++){
		psums[i+1] = psums[i] + a[i];
	}
	auto pmax = RangeMaxQuery<ll>(psums);
	auto pmin = RangeMinQuery<ll>(psums);
	bool works = true;
	y_combinator([&](auto self, int l, int r) -> void {
		if(l > r) return;
		int m = amax.query(l, r).second;
		self(l, m-1);
		self(m+1, r);
		ll right_max = pmax.query(m+1, r+1) - psums[m+1];
		ll left_max = psums[m] - pmin.query(l, m);
