...
struct dynamic_li_chao
{
	...
	void add_line(ll a, ll b)
	{
		...
		update(-2e12,2e12+1,&start,func.size()-1);
	}
	...
	ll get(ll x)
	{
		return getK(-2e12,2e12+1,&start,x);
	}
...
ll n,m,i,j,k,aa[200001],bb[200001],cc[200001],res=-9e18-7;
ll dp[100001];
int main()
{
	fio;
	cin>>n;
	for (i=1;i<=n;i++) {cin>>aa[i]; bb[i]=bb[i-1]+aa[i]; cc[i]=cc[i-1]+aa[i]*i; }
	//if (n==200000) cout<<bb[200000]*200000<<endl;
//	if (n==10) cout<<cc[10]<<endl;
	bruh.add_line(0,0);
	for (i=1;i<=n;i++)
	{
		ll l=i;
	//	cout<<"add "<<l<<' '<<' '<<bb[l]<<' '<<l<<' '<<cc[l]<<' '<<bb[l]*l-cc[l]<<endl;
		bruh.add_line(l,bb[l]*l-cc[l]);
		ll r=i;
//		cout<<"get "<<-bb[r]<<' '<<cc[r]<<endl;
		ll cur=bruh.get(-bb[r])+cc[r];
	//	cout<<cur<<endl;
		res=max(res,cur);
	}
	cout<<res;
...
/*
f(l,r)=(cc[r]-cc[l])-(bb[r]-bb[l])*(l)=cc[r]+(bb[l]*l-cc[l]-bb[r]*l);
*/