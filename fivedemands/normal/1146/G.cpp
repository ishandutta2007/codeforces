#include<bits/stdc++.h>//flow from tourist
using namespace std;
template <typename T>
class flow_graph {
  public:
...
int main(){
	...
	cin >> n >> h >> m;
	flow_graph<long long>g(n*h+m+2,0,n*h+m+1);
	for(int i=1; i<=n ;i++){
		for(int j=1; j<=h ;j++){
			g.add(0,(i-1)*h+j,2*j-1,0);
		}
	}
	for(int i=1; i<=m ;i++){
		...
		for(int j=l; j<=r ;j++){
			for(int k=x+1; k<=h ;k++){
				g.add((j-1)*h+k,n*h+i,1e9,0);
			}
		}
		g.add(n*h+i,n*h+m+1,c,0);
	}
	cout << h*h*n-g.max_flow() << endl;
}