...
#define tm idgheriobe

ld a,v;

ld sp(ld w,ld l)
{
    return sqrt(2*a*l+w*w);
}
...
signed main()
{
    ...
    loop(100)
    {
        ld mx=(x*2+y)/3,my=(y*2+x)/3;
        ld chx=abs(sp(w,d-mx)-sp(0,mx));
        ld chy=abs(sp(w,d-my)-sp(0,my));
//        cout<<x<<" "<<y<<" -> ("<<mx<<" -> "<<chx<<") ("<<my<<" -> "<<chy<<")\n";
        if(chx<chy)
            y=my;
        else
            x=mx;
