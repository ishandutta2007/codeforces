...
//template
#define MAXN 3000
int n,len[MAXN+10],fi;
char s[MAXN+10][2][MAXN+10],t[MAXN+10],t2[MAXN+10];
bool same[MAXN+10];
vector<pair<char,char> >a,b;
pii c[MAXN+10];
int nxt[MAXN+10];
void Get_nxt(char *T,int *nxt){
	int len=strlen(T),i=1,j=0;
    nxt[0]=-1;
    while(i<len){
        if(j==-1||T[i]==T[j])
            nxt[++i]=++j;
...
int main()
{
	...
	while(1){
		for(i=1;i<=n;i++)
			if(!same[i]){
				if(c[i].second<len[i]-1&&s[i][0][c[i].second+1]==s[fi][0][c[fi].second+1]);
				else
					break;
			}
		if(i<=n)
	...
	while(1){
		for(i=1;i<=n;i++){
			if(!same[i]){
				if(c[i].first&&s[i][0][c[i].first-1]==s[fi][0][c[fi].first-1]);
				else
					break;
			}
		}
	...
	for(i=c[fi].first;i<=c[fi].second;i++){
		t[i-c[fi].first]=s[fi][0][i];
		t2[i-c[fi].first]=s[fi][1][i];
	}
	Get_nxt(t,nxt);
	for(i=1;i<=n;i++){
		int now=kmp(s[i][0],t,nxt);
		if(same[i]){
			if(now!=-1){
				puts("NO");
				return 0;
