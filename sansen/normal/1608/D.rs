...
fn run() {
    ...
    let cnt = |s: usize, t: usize| -> usize {
        assert!(s.max(t) < 3);
        cnt[s * 3 + t]
    };
    let left = |s: usize| -> usize {
        cnt(s, 0) + cnt(s, 1) + cnt(s, 2)
    };
    let right = |s: usize| -> usize {
        cnt(0, s) + cnt(1, s) + cnt(2, s)
    };
    ...
    for lw in 0..=n {
        let mut cond = true;
        cond &= left(0) <= lw;
        cond &= left(0) + left(2) >= lw;
        cond &= right(1) <= lw;
        cond &= right(1) + right(2) >= lw;
        if !cond {
            continue;
        }
        ans += pc.comb(left(2), lw - left(0)) * pc.comb(right(2), lw - right(1));
        if cnt(1, 1) == 0 && cnt(0, 0) == 0 && (0 < lw && lw < n) && lw - left(0) >= cnt(2, 1) {
            ans -= pc.comb(cnt(2, 2), lw - left(0) - cnt(2, 1));
        }
    }
    println!("{}", ans);
}
