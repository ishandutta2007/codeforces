...
string s;int a,b,c;vector<ll>vc;multiset<ll> an;
int main()
{
ios_base::sync_with_stdio(false);
    ...
    cin>>n>>x>>y;
    x+=y;
    for(int i=0;i<n;i++) {cin>>y;vc.push_back(y);}
    for(int i=0;i<n;i++) {cin>>y;an.insert(abs(vc[i]-y));}
    while(x){auto it=an.end();it--;int tmp=abs(*it-1);an.erase(it);an.insert(tmp);x--;}
