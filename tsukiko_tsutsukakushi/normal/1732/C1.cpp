...
void solve() {
    ...
    vc<int> right(n, -1);
    rep(j, C) {
        if(csum[j] % 2 == 1 and csum[j] > 1) {
            int sz = csum[j];
            chmax(need, g[j][sz - 2] + 1);
            chmax(right[g[j][0]], g[j][sz - 1] + 1);
            chmax(right[g[j][1]], inf);
        } else if(csum[j] % 2 == 0 and csum[j] > 0) {
            chmax(need, g[j].back() + 1);
            chmax(right[g[j].front()], inf);
        }
    }
    dump(right);
    int l = 0, r = n;
