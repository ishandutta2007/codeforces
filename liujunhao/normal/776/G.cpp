...
LL dfs(int mx,int j,int k){
	...
	for(int i=0;i<16;i++)
		ret+=dfs(max(i,mx),(j*16+i)&65535,k-1);
	return ret;
}
LL cal(LL n){
	if(n<0)
		return 0;
	int b[70],len=0,i,j,mx,now;
	LL ans(0);
	while(n){
		b[len++]=n&15;
		n>>=4;
	...
	for(i=len-1;i>=0;i--){
		for(j=0;j<b[i];j++)
			ans+=dfs(max(mx,j),(now*16+j)&65535,i);
		now=(now*16+b[i])&65535;
		mx=max(mx,b[i]);
	}
//	cout<<ans<<endl; 
	return ans+dfs(mx,now,0);
}
int main()
{
