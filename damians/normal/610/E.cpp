...
struct elem {//element of a monoid M
	int cnt[10][10];
	int lc,rc;
	elem(int c=-1) {lc=rc=c;memset(cnt,0,sizeof(cnt));}
...
elem combine_elems(const elem& a,const elem& b) {
	...
	c.lc=a.lc;
	c.rc=b.rc;
	FOR(i,10)FOR(j,10) c.cnt[i][j]=a.cnt[i][j]+b.cnt[i][j];
	if (a.rc!=-1 && b.lc!=-1) c.cnt[a.rc][b.lc]++;
	return c;
}
...
int main()
{
	...
	while(q--) {
		...
		if (tp==1) {
			int a,b;cin>>a>>b;a--;b--;
			string s;cin>>s;
			modify(a,b,oper(s[0]-'a'));
		}
		if (tp==2) {
			string s;cin>>s;
			int wyn=1;
			FOR(i,k)FOR(j,i+1) wyn+=d[1].cnt[s[i]-'a'][s[j]-'a'];
			cout<<wyn<<"\n";
		}
	}
