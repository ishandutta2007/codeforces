...
void dec() {
    array<int, 4> b;
    b[0] = query(a[1], a[2], a[3]);
    b[1] = query(a[0], a[2], a[3]);
    b[2] = query(a[0], a[1], a[3]);
    b[3] = query(a[0], a[1], a[2]);
    int mx = *max_element(b.begin(), b.end());
    vector<int> v;
    repeat(i, 0, 4) if (b[i] != mx) v << i;
    for (auto &i: v) i = a[i];
    repeat(i, 0, 4) a.pop_front();
...
void Solve() {
    ...
    if (a.size() == 3) {
        repeat(i, 0, n) if (a[0] != i && a[1] != i && a[2] != i) {
            a.push_back(i);
            break;
        }
        dec();
