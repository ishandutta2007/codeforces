...
int main() {
	...
	for(int i=0; i<n; ++i) {
		cin >> ai >> bi;
//		cout << (--mp.upper_bound(bi))->second << endl;
		mp.insert({bi, (--mp.upper_bound(bi))->second});
		mp.insert({ai, (--mp.upper_bound(ai))->second});
//		mp[bi]=(--mp.upper_bound(bi))->second;
//		mp[ai]=(--mp.upper_bound(ai))->second;
//		cout << "mp " << i << " " << mp[ai] << " " << mp[bi] << endl;
		auto it=mp.find(ai);
		while(it->first<bi) {
