...
#line 1 "/home/maspy/compro/library/other/integer_kth_root.hpp"
...
void solve() {
  LL(N);
  unordered_map<ll, pi> MEMO;
  MEMO.reserve(1 << 22);

  auto f = [&](auto self, ll N) -> pi {
    if (N == 0) return {0, 0};
    if (MEMO.count(N)) return MEMO[N];
    pi& ANS = MEMO[N];
    ll K = integer_kth_root(3, N);
    FOR(i, 4) {
      ll x = K - 2 + i;
      if (x <= 0) continue;
      ll p = x * x * x;
      ll q = (x + 1) * (x + 1) * (x + 1);
      if (p > N) break;
      auto [val, X] = self(self, min(q - 1, N) - p);
      val++;
      X += p;
      if (ANS.fi <= val) ANS = {val, X};
    }
    return ANS;
  };
  pi ANS = f(f, N);
