...
int main() {
    ...
    for_inc_range(i, 2, 2 * n) {
        if (getType(a[i - 1].second) != getType(a[i].second)) {
            ...
            best.insert(make_pair(lcp, make_pair (i - 1, i)));
        }
    }
    ...
    int ret = 0;
    while (!best.empty()) {
        pair<int, pair <int, int>> curBest = *best.rbegin();
        best.erase(curBest);
        int i = curBest.second.second;
        int prevI = prev[i];
        if (prevI != curBest.second.first)
            continue;
        ...
        if (theNext != -1 && thePrev != -1 && getType(a[thePrev].second) != getType(a[theNext].second)) {
            ...
            best.insert(make_pair(lcp, make_pair (thePrev, theNext)));
        }
    }
