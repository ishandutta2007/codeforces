...
ull pr[1000005], frac[1000005];
int go[1000005];
void solve(){
	rep(i, 1000005) pr[i] = mt();
	int n; cin >> n;
	if(n <= 3){
		cout << 1 << endl << 1 << endl;
		return;
	...
	ull all = 0;
	repn(i, n) all ^= frac[i];
	hash_table<ull, int>ht;
	repn(i, n){
		ht[frac[i]] = i;
		if(all == frac[i]){
			cout << n-1 << endl;
			repn(j, n) if(i != j) cout << j << " ";
			cout << endl;
			return;
		}
	}
	...
	assert(n%4 == 3);
	cout<<n-3<<endl;
	repn(i, n-1) if(i != 2 and i != (n-1)/2) cout << i << " ";
	cout << endl;
}
signed main(){
	cin.tie(0);
