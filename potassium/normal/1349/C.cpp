...
int main() {
	...
	queue<pii> q;
	vector<vector<int>> fix(n, vector<int>(m, -1));
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			for (int dir = 0; dir < 4; dir++) {
				int newR = i + dr[dir];
				int newC = j + dc[dir];
				if (0 <= newR && newR < n && 0 <= newC && newC < m && s[newR][newC] == s[i][j]) {
					q.push({i, j});
					fix[i][j] = 0;
					break;
				}
	...
	while (!q.empty()) {
		...
		for (int dir = 0; dir < 4; dir++) {
			...
			if (0 <= newR && newR < n && 0 <= newC && newC < m && fix[newR][newC] == -1) {
				q.push({newR, newC});
				fix[newR][newC] = fix[nowR][nowC] + 1;
			}
		}
	}
	
	while (t--) {
		int r,c; ll p; R(r,c,p);
		r--; c--;
		...
		if (fix[r][c] == -1) {
			...
			if (p > fix[r][c]) {
				W((p - fix[r][c]) % 2 == 0 ? s[r][c] : (s[r][c] == '1' ? '0' : '1'));
			} else {
				W(s[r][c]);
			}
		}
