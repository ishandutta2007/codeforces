...
const vector<ll>grundy={0, 0, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 4, 0, 5, 2, 2, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 2, 7, 4, 0, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 4, 4, 5, 5, 2, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7, 4, 8, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 4, 4, 5, 5, 9, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7};
bool solve(){
    LL(n);
    STR(s);
    const ll R=count(s,'R');
    const ll B=count(s,'B');
    if(R>B)return 1;
    if(R<B)return 0;
    rep(n)if(i&1)s[i]^='B'^'R';
    ll G=0;
    each(key,val,run_press(s)){
        if(val<68)G^=grundy[val];
        else G^=grundy[val%34+68];
    }
    return G;
}
int main(){
    LL(t);
    rep(t)out(solve()?"Alice":"Bob");
}