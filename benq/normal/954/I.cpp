...
int main() {
    ...
    setIO(); re(S,T);
    F0R(i,6) F0R(j,6) if (i != j) {
        vi a, b;
        F0R(k,sz(S)) a.pb(S[k] == char('a'+i));
        F0Rd(k,sz(T)) b.pb(T[k] == char('a'+j));
        vi c = conv(a,b); // cout << c << "\n";
        F0R(k,sz(S)-sz(T)+1) if (c[k+sz(T)-1]) ed[k].pb({i,j});
    }
    F0R(i,sz(S)-sz(T)+1) {
        DSU<6> D = DSU<6>();
        int ans = 0;
        for (auto a: ed[i]) if (D.unite(a.f,a.s)) ans ++;
        cout << ans << " ";
    }
    // you should actually read the stuff at the bottom
}
