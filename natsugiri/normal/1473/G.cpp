...
//using ntt_2 = NTT<2013265921, 31>;
//using ntt_3 = NTT<2113929217, 5>;
...
template<class T> struct Polynomial {
	...
	REP (i, x.size()) REP (j, y.size()) z.a[i+j] += x.at(i) * y.at(j);
	z.normalize();
	return z;
    }
...
//    }
    ...
    int len = 0;
    REP (i, N) {
	int nxt_len = len + A[i] - B[i];
	// int R = len + A[i];
	// 0 <= x <= len;
	// 0 <= y <= nxt_len;
	// (x, len-x) -> (R-y, R-nxt_len+y);
	// (R-x-y, R-len-nxt_len+x+y);
	// (len+a-x-y, -len+b+x+y);
	Polynomial f, g;
	for (int x=0; x<=len; x++) {
	    f.add(len-x, p.at(x));
	}
	for (int z=0; z<=len+nxt_len; z++) {
	    if (z <= len+A[i] && len <= B[i]+z) {
		g.add(z, fact_inv[len+A[i]-z] * fact_inv[-len + B[i] + z]);
	    }
	}
	Polynomial h = f * g;
	Polynomial q;
	for (int y=0; y<=nxt_len; y++) {
	    q.add(y, fact[A[i] + B[i]] * h.at(len + y));
	}
	swap(p, q);
	// s(y) = sum f(x) * g(x+y)
	// z = x + y;
	// g(z) = fact(a+b) / fact(len+a-z) / fact(-len+b+z);

	len = nxt_len;
    }

    Mint ans = 0;
    REP (x, len+1) ans += p.at(x);
