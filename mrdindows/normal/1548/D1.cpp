...
//    auto extract_time = [](const FILETIME& ft) {
...
auto __c__ = []() { if (__ir == __er) {std::fill(__bur, __bur + __BS, 0); cin.read(__bur, __BS); __ir = __bur; };};
template<class T = int> T readInt() {
    __c__(); while (*__ir && (*__ir < '0' || *__ir > '9') && *__ir != '-') ++__ir; __c__();
    bool m = false; if (*__ir == '-') {++__ir; __c__(); m = true;}
    T r = 0; while (*__ir >= '0' && *__ir <= '9') {r = r * 10 + *__ir - '0', ++__ir; __c__();}
    ++__ir; return m ? -r : r;
}
string readString() {
    string r; __c__(); while (*__ir && isspace(*__ir)) {++__ir; __c__();}
    while (!isspace(*__ir)) {r.push_back(*__ir); ++__ir; __c__();}
    ++__ir; return r;
}
char readChar() {
    __c__(); while (*__ir && isspace(*__ir)) {++__ir; __c__();} return *__ir++;
}
...
template<class T1, class T2, class T3 = int> inline void addmod(T1 &x, T2 y, T3 m = USUAL_MOD) { x = (x + y) % m; }
...
//    FREOUT("Data/output.txt");
#endif
    return solve();
}
pii a[6060];
ll area(pii b, pii c) {
    return abs(b.first * ll(c.second) - c.first * ll(b.second));
}
ll picka(pii b, pii c) {
    const int F1 = 4;
    const int F2 = 4;
//    b.first += F1;
//    b.second += F2;
...
//    }
    int bound = 0;
    bound += gcd(abs(b.first), abs(b.second));
    bound += gcd(abs(c.first), abs(c.second));
    bound += gcd(abs(b.first - c.first), abs(b.second - c.second));
    return s % 4 == bound % 4;
}
ll pickachu(pii b, pii c) {
    ll sb = 0;
    ll sc = 0;
    sb += b.first * ll(b.second) * 2;
    sc -= c.first * ll(c.second) * 2;
    
    sb -= gcd(abs(b.first), abs(b.second));
    sc -= gcd(abs(c.first), abs(c.second));
    
    sb += gcd(abs(b.first - c.first), abs(b.second - c.second));
    return (sb + sc) % 4 == 0;
}
int cnt[4][4];
int solve() {
    if (0) {
        FREOUT("Data/input.txt");
        int n = 30;
        cout << n << endl;
        FI(n) {
            int x = 2 * rand() % 5000000;
            int y = 2 * rand() % 5000000;
            cout << x << ' ' <<  y << endl;
        }
        return 0;
    }
...
//                ll cur = cnt[x1][y1];
//                cur *= cnt[x2][y2] - (p1 == p2);
//                cur *= cnt[x3][y3] - (p1 == p3) - (p2 == p3);
//                DBN(p1, p2, p3, cur);
//                tot += cur;
//                ans2 += cur * picka({x2 - x1, y2 - y1}, {x3 - x1, y3 - y1}) % 2;
//            }
//        }
//    }
//    DBN(tot);
//    DBN(ans2 / 6);
    ...
    FI(n) {
        ...
        int cnt[4][4] = {};
        FJ(i) {
            auto foo = [](int x) { return (x % 4 + 4) % 4; };
            cnt[foo(a[j].first)][foo(a[j].second)]++;
        }
        for (int p1 = 0; p1 < 4; ++ p1) {
            for (int p2 = 0; p2 < 4; ++p2) {
                int x1 = p1 % 2 * 2;
