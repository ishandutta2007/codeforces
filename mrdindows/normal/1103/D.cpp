...
ll dp[MAX_PRIMES + 1][1 << MAX_PRIMES];
map<vector<int>, pair<multiset<int>, int>> whatAndCosts;
int solve() {
    int n = readInt();
    ll k = readInt<ll>();
    ll g = 0;
    FI(n) {
        a[i].first = readInt<ll>();
        g = gcd(g, a[i].first);
    }
    if (g == 1) {
        cout << 0 << endl;
    ...
    vector<pll> factors = factorize(g);
    FI(n) {
        a[i].second = readInt();
    }
    FI(n) {
    ...
    for (int i = 0; i < n; ) {
        int f = 0;
        for (auto p : factors) {
            if (a[i].first % p.first == 0) ++f;
        }
        auto what = a[i].first;
        while (i < n) {
            if (a[i].first != what) break;
            if (f > 0) {
                --f;
                a[realN++] = a[i];
            }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (cnt == 0) {
            ...
            for (int mask : what) {
                int zmask = z ^ mask;
                for (int i = zmask; ; i = (i - 1) & zmask) {
                    minMoves[i | mask] = min(minMoves[i | mask], minMoves[i] + 1);
                    if (i == 0) {
                        break;
                    }
                }
        ...
        res.first.insert(cost);
        if (res.first.size() > cnt) {
            res.first.erase(--res.first.end());
        }
    }
    ...
    dp[0][0] = 0;
    for (auto& [what, p] : whatAndCosts) {
        for (auto cost : p.first) {
            for (int cnt = K - 1; cnt >= 0; --cnt) {
                for (int sm : what) {
                    int withoutSm = full ^ sm;
                    for (int mask = withoutSm;; mask = (mask - 1) & withoutSm) {
                        dp[cnt + 1][mask | sm] = min(dp[cnt + 1][mask | sm], dp[cnt][mask] + cost);
                        if (mask == 0) break;
                    }
                }
            }
