...
public class C {
	...
	Vector v0 = new Vector(random.nextDouble(), random.nextDouble());
	...
	public void solve() {
		...
		for (int i = 0; i < n; i++) {
			if (s.charAt(i) == '0') {
				xlast[cur] = i - cur;
				long pr = pref[cur + 1] + Math.max(0, i - cur - xlast[cur + 1]) * 1L * cur;
				for (; cur > 0; cur--) {
					xlast[cur] = i - cur;
					pref[cur] = pr;
					pr += cur - 1;
				...
				cur++;
				ans += pref[cur + 1] + Math.max(0, i - cur - xlast[cur + 1]) * 1L * cur + cur * (cur + 1L) / 2;
			}
		}
		out.println(ans);
