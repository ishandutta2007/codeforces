...
point ed,a[MAXN+10],c,d;
int n,f[MAXN+10],fac[MAXN+10],inv[MAXN+10];
int ans;
void read(){
	Read(ed.x),Read(ed.y),Read(n);
	int i,x,y;
//	Read(c.x),Read(c.y),Read(d.x),Read(d.y);
	for(i=1;i<=n;i++){
		Read(x),Read(y);
	//	ty=cross(c,point(x,y))/cross(c,d);
	//	tx=cross(d,point(x,y))/cross(d,c);
		//if(tx*c.x+ty*d.x==x&&tx*c.y+ty*d.y==y&&tx>=0&&ty>=0)
			a[i]=point(x+y-2,y-1);
	//	else
	//		i--,n--;
	}
	ed=point(ed.x+ed.y-2,ed.y-1);
	int t=MAXN;
...
void solve(){
	...
	ans=C(ed.x,ed.y);
	for(i=1;i<=n;i++){
		if(ed.x-a[i].x>=0&&ed.y-a[i].y>=0)
			ans=((ans-C(ed.x-a[i].x,ed.y-a[i].y)*1ll*f[i])%MOD+MOD)%MOD;
		for(j=i+1;j<=n;j++)
			if(a[j].x-a[i].x>=0&&a[j].y-a[i].y>=0)
				f[j]=((f[j]-C(a[j].x-a[i].x,a[j].y-a[i].y)*1ll*f[i])%MOD+MOD)%MOD;
	}
	cout<<ans<<endl;
}
int main()
