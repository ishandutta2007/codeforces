...
int main() {
    TEST {
        ...
        per(i, n) {
            ...
            rep(j, i + 2, n + 1, 2) {
                // 
                int l1 = (dp[i + 2][j] ? dp[i + 2][j] : (s[i] < s[i + 1] ? 1 : s[i] == s[i + 1] ? 0 : -1));
                int l2 = (dp[i + 1][j - 1] ? dp[i + 1][j - 1] : (s[i] < s[j - 1] ? 1 : s[i] == s[j - 1] ? 0 : -1));
                int r1 = (dp[i][j - 2] ? dp[i][j - 2] : (s[j - 1] < s[j - 2] ? 1 : s[j - 1] == s[j - 2] ? 0 : -1));
                int r2 = (dp[i + 1][j - 1] ? dp[i + 1][j - 1] : (s[j - 1] < s[i] ? 1 : s[i] == s[j - 1] ? 0 : -1));

                dp[i][j] = max(min(l1, l2), min(r1, r2));
            }
        }
        int res = dp[0][n];
        OUT(res == -1 ? "Bob" : res == 0 ? "Draw" : "Alice");
    }
}