...
void solve()
{
    ...
    ll need = 0;
    for (int i = 1; i<n-1; i++) need+=(a[i]+1)/2;
    ...
    if (cnt2==0) {cout<<-1<<endl; return;}
    if (cnt1>=2) {cout<<need<<endl; return;}

    int who = 0;
    for (int i = 1; i<n-1; i++) who = max(who, a[i]);

    if (who%2==0) {cout<<need<<endl; return;}
    if (n==3||who==3) {cout<<-1<<endl; return;}
    cout<<need<<endl;
}
