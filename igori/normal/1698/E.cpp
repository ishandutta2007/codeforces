...
void solve()
{
    ...
    vi c(n);
    forn(i, n) c[i] = b[pos[i]];
    forn(i, n) if (c[i] == -2) c[i] = -1;

    forn(i, n) pos[i] = -1;
    forn(i, n) if (c[i] != -1) pos[c[i]] = i;
    forn(i, n) if (pos[i] > i + s)
    {
        cout << 0 << "\n";
        return;
    }
    ...
    for (int i = 0; i < n; i++)
    {
        if (i + s < n) cnt += c[i + s] == -1;
        if (pos[i] == -1) ans = 1ll * ans * cnt % MOD, cnt--;
        //cout << "placed till i = " << i << ", ans = " << ans << "\n";
    }
    cout << ans << "\n";
}
