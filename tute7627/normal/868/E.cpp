  ...
  rep(i,1,m+1){
    rep(j,0,n){
      ...
      while(ng-ok>=2){
        ...
        ll cnt=0;
        rep(o,0,n){
          if(g[o].size()!=1)continue;
          ll idx=lower_bound(ALL(dp[o]),mid-d[j][o])-dp[o].begin();
          cnt+=max(0LL,i-idx);
        }
        if(cnt>=i)ok=mid;
        else ng=mid;
  ...
  while(ng-ok>=2){
    ...
    for(auto sub:g[s]){
      ...
      auto dfs=[&](auto &&f,ll k,ll par)->void{
        cnt-=x[k];
        if(g[k].size()==1){
          ll idx=lower_bound(ALL(dp[k]),mid-d[s][k])-dp[k].begin();
          cnt+=max(0LL,m-idx);
        }
        for(auto to:g[k]){
          if(to!=par)f(f,to,k);
