...
int read(int l, int r, int wh = 1, int lb = 0, int rb = nax - 1) {
	if (l > rb || r < lb) return inf;
	if (l <= lb && r >= rb) return tree[wh];
	int sb = (lb + rb) / 2;
	return min(read(l, r, wh * 2, lb, sb), read(l, r, wh * 2 + 1, sb + 1, rb));
}
...
int main() {
	...
	for (int i = 3 * n - 1; i >= 0; --i) {
		debug(imie(stos));
		auto it = lower_bound(stos.begin(), stos.end(), pii{(a[i] + 1) / 2, -inf});
		if (it == stos.end()) debug("end");
		else debug(imie(*it));
		assert(it != stos.begin());
		it--;
		next[i] = it->second;
		pii curr = {a[i], i};
		while (!stos.empty() && stos.back().first >= curr.first)
			stos.pop_back();
		stos.push_back(curr);
		debug(arr(next, i), range(a, i, 3 * n - 1));
		put(i, next[i]);
	}
	for (int i = 0, head = 0; i < n; ++i) {
		while (head < 3 * n - 1 && valid(i, head + 1))
			head++;
		debug(range(a, i, head));
		ans[i] = head;
	}
	for (int i = 0; i < n; ++i) {
