...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int[] ans = new int[5];
            for (int i = 0; i < 4; i++) {
                io.cache.append("? ").append(i + 1).append(' ').append(i + 2).append('\n');
                io.flush();
                ans[i] = io.readInt();
            }

            int[] perm = new int[]{4, 8, 15, 16, 23, 42};
            List<Integer> res = new ArrayList<>();
            dfs(perm, new boolean[6], res, ans);
            io.cache.append("! ");
            for (int i : res) {
        ...
        public boolean dfs(int[] perm, boolean[] occupied, List<Integer> deque, int[] ans) {
            if (perm.length == deque.size()) {
                for (int i = 0; i < 4; i++) {
                    if (ans[i] != deque.get(i) * deque.get(i + 1)) {
                        return false;
                    }
                }
                return true;
            ...
            for (int i = 0; i < 6; i++) {
                ...
                occupied[i] = true;
                deque.add(perm[i]);
                if (dfs(perm, occupied, deque, ans)) {
                    return true;
                }
                occupied[i] = false;
