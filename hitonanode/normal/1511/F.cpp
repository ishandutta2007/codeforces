...
int main() {
    ...
    vector<string> words{""};
    for (auto s : S) {
        REP(i, s.size()) words.emplace_back(s.substr(i));
    }
    words = sort_unique(words);
    const int D = words.size();

    const int K = 5;
    matrix<mint> mat(D * K, D * K);
    REP(i, D * (K - 1)) mat[i][i + D] += 1;

    for (auto w : words) {
        int x = lower_bound(words.begin(), words.end(), w) - words.begin();
        if (w.empty()) {
            for (auto w1 : S) for (auto w2 : S) {
                int l = min(w1.size(), w2.size());
                if (w1.substr(0, l) == w2.substr(0, l)) {
                    auto wnxt = w1.substr(l) + w2.substr(l);
                    int j = lower_bound(ALL(words), wnxt) - words.begin();
                    int e = l - 1;
                    if (e >= 0 and e < K) mat[e * D + j][x] += 1;
                }
            }
        } else {
            for (auto w1 : S) {
                int l = min(w.size(), w1.size());
                if (w.substr(0, l) == w1.substr(0, l)) {
                    auto wnxt = w.substr(l) + w1.substr(l);
                    int j = lower_bound(ALL(words), wnxt) - words.begin();
                    int e = l - 1;
                    if (e >= 0 and e < K) mat[e * D + j][x] += 1;
                }
