...
void dfs(int u, int p, int c)
{
    ...
    {
        if (used[g[u][i].Y]==1) continue;
        if (g[u][i].X==p) continue;
        if (was[g[u][i].X]==1)
        {
            lul.push_back({g[u][i].X,g[u][i].Y});
            continue;
        }
        dfs(g[u][i].X,u,g[u][i].Y);
        if (used[g[u][i].Y]==0)
        {
            lul.push_back({g[u][i].X,g[u][i].Y});
        }
    }
    if (p!=-1) lul.push_back({p,c});
    for (int i=0;i+1<lul.size();i+=2)
    {
        ans.push_back({lul[i].X,u,lul[i+1].X});
        used[lul[i].Y] = 1;
        used[lul[i+1].Y] = 1;
    }
}
