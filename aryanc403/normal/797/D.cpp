...
const lli mod = 1000000007L;
    ...
    lli m;
    string s;
    vector<pair<lli,ii>> a;
    set<lli> ss,tt;
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

void dfs(lli u,lli l,lli r,bool fl)
{
    if(u==-1)
        return;

    if(fl&&l<=a[u].X&&a[u].X<=r)
    {
        tt.insert(a[u].X);
        dfs(a[u].Y.X,l,a[u].X-1,fl);
        dfs(a[u].Y.Y,a[u].X+1,r,fl);
        return;
    }
    ...
    if(r<a[u].X)
    {
        dfs(a[u].Y.Y,l,a[u].X-1,false);
        dfs(a[u].Y.X,l,r,fl);
        return;
    }

    dfs(a[u].Y.Y,l,a[u].X-1,false);
    dfs(a[u].Y.X,l,r,false);
}
...
// cin>>T;while(T--)
{
    ...
    dfs(*ss.begin(),0,INF,true);

    dbg(tt);
    repA(i,1,n)
        cnt+=tt.find(a[i].X)==tt.end();
