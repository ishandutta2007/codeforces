...
signed main() {
  ...
  auto make_graph = [&](auto lim) {
    ...
    FOR(i, H) {
      FOR(j, H) {
        FOR(w, W) {
          G1[i][j] = G1[i][j] && (abs(A[i][w] - A[j][w]) >= lim);
          if (w) {
            G2[i][j] = G2[i][j] && (abs(A[i][w] - A[j][w - 1]) >= lim);
          }
        }
      }
    }
  ...
  auto check = [&](ll lim) -> bool {
    auto [G1, G2] = make_graph(lim);

    // subset, first -> set of last
    Grid<ll> dp(1 << H, H);
    FOR(i, H) { dp[1 << i][i] = 1 << i; }
    FOR(s, 1 << H) {
      FOR(i, H) {
        FOR(j, H) {
          if (!(dp[s][i] & 1 << j)) continue;
          FOR(k, H) {
            if (!G1[j][k]) continue;
            auto t = s | 1 << k;
            if (s == t) continue;
    ...
    FOR(i, H) {
      FOR(j, H) {
        if ((DP[i] & 1 << j) && G2[i][j]) return true;
      }
    }
    return false;
  };
