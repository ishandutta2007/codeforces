...
signed main() {
    ...
    int n, m;
    read(n, m);
    vector<pair<int, pii>> segm(n);
    for (auto &i : segm) {
    	read(i.s.f, i.s.s, i.f);
    	--i.s.f, --i.s.s;
    }
    sort(segm);
    int ans = mod;
    ...
    t.build();
    for (int i = 0; i < n; ++i) {
    	t.upd(segm[i].s.f, segm[i].s.s, 1);
    	bool flag = false;
    	while (ptr <= i && t.get(0, m - 1).val > 0) {
    		t.upd(segm[ptr].s.f, segm[ptr].s.s, -1);
    		++ptr;
    		flag = true;
    	}
    	dbg(i, ptr);
    	if (flag)
    		chmin(ans, segm[i].f - segm[ptr - 1].f);
    }
    println(ans);
    #ifdef DEBUG
