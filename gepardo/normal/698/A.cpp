/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 363 (Div. 1)         |
|       Task: A                                     |
|       Date: 19. 07. 2016                          |
\--------------------------------------------------*/
// Includes
...
inline bool canSport(int n)
{
    return bool(n & 2);
}

inline bool canCode(int n)
{
    return bool(n & 1);
}
...
main()
{
    in.getv(v, n);
    dp[0][0] = 1;
    dp[0][1] = canCode (v[0]) ? 0 : inf;
    dp[0][2] = canSport(v[0]) ? 0 : inf;
    for (int i = 1; i < n; i++)
    {
        dp[i][0] = min3(dp[i-1][0], dp[i-1][1], dp[i-1][2]) + 1;
        dp[i][1] = canCode (v[i]) ? min(dp[i-1][0], dp[i-1][2]) : inf;
        dp[i][2] = canSport(v[i]) ? min(dp[i-1][0], dp[i-1][1]) : inf;
    }
    out = min3(dp[n-1][0], dp[n-1][1], dp[n-1][2]);
    return 0;
}