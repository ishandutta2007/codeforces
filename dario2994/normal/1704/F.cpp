...
int main() {
    ...
    for (int n = 2; n < N; n++) {
        set<int> vals;
        for (int i = 0; i <= n-2; i++) vals.insert(nim[i] ^ nim[n-2-i]);
        while (vals.count(nim[n])) nim[n]++;
    }

    // for (int n = 0; n <= 20; n++) cout << nim[34*n+2] << ", ";
