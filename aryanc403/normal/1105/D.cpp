/*
  Warn - Don't change next line else you will get WA verdict. Online Judge is configured to give WA if next line is not present.
  Author - Aryan Choudhary (@aryanc403)
 
  const short DEBUG { 0 };
  #define debug(x) if (DEBUG) cout << #x << " = " << x << '\n'
*/

#pragma warning(disable:4996)
#pragma comment(linker, "/stack:200000000")
#pragma GCC optimize ("Ofast")
#pragma GCC target ("sse,sse2,sse3,ssse3,sse4,popcnt,abm,mmx,avx,tune=native")
#pragma GCC optimize ("-ffloat-store")

#include<iostream>
#include<bits/stdc++.h>
#include<stdio.h>
using namespace std;
#define fo(i,n)   for(i=0;i<(n);++i)
#define repA(i,j,n)   for(i=(j);i<=(n);++i)
#define repD(i,j,n)   for(i=(j);i>=(n);--i)
#define pb push_back
#define mp make_pair
#define X first
#define Y second
#define endl "\n"
#define PI 3.1415926535897932384626433832795
typedef long long int lli;
typedef long double mytype;
typedef pair<lli,lli> ii;
typedef vector<ii> vii;
typedef vector<lli> vi;

//const lli [3] ={ 999119999L,1000000007L,1000992299L};
//const lli [3] ={ 97L,101L,103L};
//const lli  = chrono::high_resolution_clock::now().time_since_epoch().count();
clock_t time_p=clock();
void aryanc403()
{
    time_p=clock()-time_p;
    cerr<<"Time Taken : "<<(float)(time_p)/CLOCKS_PER_SEC<<"\n";
}

class CMP
{
public:
    bool operator()(pair<lli,ii> a , pair<lli,ii> b) //For min priority_queue .
    {
        return ! ( a.X <= b.X );
    }
};

void add( map<lli,lli> &m, lli x,lli cnt=1)
{
    map<lli,lli> ::iterator jt;
    jt=m.find(x);
    if(jt==m.end())         m.insert(mp(x,cnt));
    else                    jt->Y+=cnt;
}

void del( map<lli,lli> &m, lli x,lli cnt=1)
{
    map<lli,lli> ::iterator jt;
    jt=m.find(x);
    if(jt->Y<=cnt)            m.erase(jt);
    else                      jt->Y-=cnt;
}

bool cmp(const pair<lli,ii> &a,const pair<lli,ii> &b)
{
    return a.X<b.X;
}
const lli INF = 0xFFFFFFFFFFFFFFFL;
const lli mod = 1000000007L;

    lli T,n,i,j,k,in,cnt[15],l,r,p;
    lli m;
    string s[1005];
    set<ii> st[15],tt[15];
    vi a;
    bool fl;
    vi :: iterator it;
    priority_queue < pair<lli,ii> , vector < pair<lli,ii> > , CMP > pq;// min priority_queue .

void dfs(lli x,lli y,lli c,lli p)
{
    if(x<0||x>=n||y<0||y>=m||c>a[p]||s[x][y]!='.')
        return;
    s[x][y]='0'+p;
    cnt[p]++;
    tt[p].insert({x,y});
    // dfs(x-1,y,c+1,p);
    // dfs(x+1,y,c+1,p);
    // dfs(x,y-1,c+1,p);
    // dfs(x,y+1,c+1,p);
    pq.push({c+1,{x-1,y}});
    pq.push({c+1,{x+1,y}});
    pq.push({c+1,{x,y-1}});
    pq.push({c+1,{x,y+1}});

}

void add(lli p)
{
    lli ct=cnt[p],c=0;
    
    for(auto it=st[p].begin();it!=st[p].end();)
    {
        lli x=it->X,y=it->Y;
        if(s[x][y]=='0'+p)
        {
            // dfs(x-1,y,c+1,p);
            // dfs(x+1,y,c+1,p);
            // dfs(x,y-1,c+1,p);
            // dfs(x,y+1,c+1,p);
            pq.push({1,{x-1,y}});
            pq.push({1,{x+1,y}});
            pq.push({1,{x,y-1}});
            pq.push({1,{x,y+1}});
        }
        st[p].erase(it++);
    }
    
    // for(auto it=st[p].begin();it!=st[p].end();)
    // {
    //     lli x=it->X,y=it->Y;
    //     if(s[x][y]=='0'+p)
    //         pq.insert({0,{x,y}});
    //     st[p].erase(it++);
    // }
    
    while(!pq.empty())
    {
        auto u=pq.top();pq.pop();
        if(u.X>a[p])
            continue;
        dfs(u.Y.X,u.Y.Y,u.X,p);
    }
    
    tt[p].swap(st[p]);
    
    if(ct!=cnt[p])
        fl=true;
}

int main(void) {
    ios_base::sync_with_stdio(false);cin.tie(NULL);cout.tie(NULL);
    // freopen("txt.in", "r", stdin);
    // freopen("txt.out", "w", stdout);
// cout<<std::fixed<<std::setprecision(35);
// cin>>T;while(T--)
{

    cin>>n>>m>>p;
    a.clear();a.reserve(n+1);
    a.pb(0);
    fo(i,p)
    {
        cin>>in;
        a.pb(in);
    }
    fo(i,n)
    {
        cin>>s[i];
        fo(j,m)
        {
            if('0'<=s[i][j]&&s[i][j]<='9')
            {
                st[s[i][j]-'0'].insert({i,j});
                cnt[s[i][j]-'0']++;
            }
        }
    }
    fl=true;
    while(fl)
    {
        fl=false;
        lli i;
        repA(i,1,p)
            add(i);
    }
    //fo(i,n)     cerr<<s[i]<<endl;
    repA(i,1,p)
        cout<<cnt[i]<<" ";
}   aryanc403();
    return 0;
}