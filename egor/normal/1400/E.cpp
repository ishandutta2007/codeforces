...
class EOchistiteMultimnozhestvo {
public:
    void solve() {
        int n = in.readInt();
        auto a = in.readIntArray(n);

        arr2d<int> minPos(n, n);
        for (int i : range(n)) {
            int cur = i;
            for (int j : range(i, n)) {
                if (a[j] < a[cur]) {
        ...
        RecursiveFunction rec = [&](const auto& self, int from, int to, int cur) -> int {
            if (from > to) {
                return 0;
            }
            int& res = answer(from, to);
            if (res != -1) {
                return res;
            }
            int val = a[minPos(from, to)];
            return res = min(to - from + 1, val - cur + self(from, minPos(from, to) - 1, val) + self(minPos(from, to) + 1, to, val));
        };
        out.printLine(rec(0, n - 1, 0));
    }
};
...
int main() {
    ...
    auto time = clock();
#endif
    EOchistiteMultimnozhestvo solver;
