...
class seg( val x:Int, val y:Int, val id:Int):Comparable<seg>{
    override fun compareTo(other:seg):Int {
        return if(this.y != other.y) (this.y).compareTo(other.y) else (this.id).compareTo(other.id)
    }
}
...
fun main(){
    solve.cases{
        ...
        val all = MutableList(n){
            seg(getint(), getint(), it)
        }
        all.sort()
        val max = 200002
        val clusters = Array(max){ mutableListOf<seg>() }
        for(a in all){
            clusters[a.x].add(a)
        }
        val now = TreeSet<seg>()
        val ret = mutableListOf<seg>()
        for(l in 0 until max){
            for(c in clusters[l]){
                now.add(c)
            }
            while(now.isNotEmpty() && now.first().y < l){
                now.remove(now.first())
            }
            while(now.size > k){
                val out = now.last()
                now.remove(out)
