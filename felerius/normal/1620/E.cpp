// Replace the Numbers (https://codeforces.com/contest/1620/problem/E)
...
int main() {
    ...
    while (q--) {
        ...
        if (ty == 1)
            ...
            int y; read(y);
            typ2.push_back({sz(a), x, y});
        }
    }

    vector num(500'005, 0);
    iota(all(num), 0);
    repr(i, sz(a) - 1, 0) {
        while (sz(typ2) && typ2.back()[0] == i + 1) {
            auto [_, x, y] = typ2.back();
            typ2.pop_back();
            num[x] = num[y];
        }
