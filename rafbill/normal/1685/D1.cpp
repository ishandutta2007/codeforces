...
vi solve(li n, vi p, li& ans){


  vi r(n);
  for(li i = 0; i < (li)(n); ++i) r[p[i]] = i;
  union_find uf(n);
  for(li i = 0; i < (li)(n); ++i) uf.unite(i,r[i]);
  for(li i = 0; i < (li)(n-1); ++i) if(uf.find(i) != uf.find(i+1)) {
    swap(r[i],r[i+1]);
    uf.unite(i,i+1);
    ans += 2;
  }
...
int main(){
  ...
  li t; cin>>t; for(li t_ = 0; t_ < (li)(t); ++t_) {
    ...
    vi q = solve(n,p,ans);
    for(li i = 0; i < (li)(n); ++i) cout << 1+q[i] << ' ';
    cout << endl;
  }
