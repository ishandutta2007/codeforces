...
bool isOk(int x, int y) {
    return x >= 0 && y >= 0 && x < n && y < m && s[x][y] >= '1' && s[x][y] <= '3';
}
bool isCor(int x, int y) {
    return x >= 0 && y >= 0 && x < n && y < m && s[x][y] != '#';
}
void fill(int i, int j) {
    int c = lastc++;
    static pii q[1001001];
    int qs = 0;
    int qc = 0;
    q[qs++] = {i, j};
    while (qc < qs) {
...
int main() {
    ...
    for (int st = 0; st < 3; ++st) {
        static pii q[1001000];
        FI(n) {
            FJ(m) dist[st][i][j] = INF;
        }
        int qs = 0;
        int qc = 0;
        FI(n) {
        ...
        while (qc < qs) {
            ...
            int x = curp.first;
            int y = curp.second;
            int curd = dist[st][x][y];
            for (int dir = 0; dir < 4; ++dir) {
                int tox = x + dx[dir];
                int toy = y + dy[dir];
                if (!isCor(tox, toy)) continue;
                if (!used[tox][toy]) {
                    used[tox][toy] = true;
                    dist[st][tox][toy] = curd + 1;
                    q[qs++] = {tox, toy};
                }
            }
        }
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            for (int x = 0; x < n; ++x) {
                for (int y = 0;y < m; ++y) {
                    if (s[x][y] == '1' + j && dist[i][x][y] - 1 < mind[i][j]) {
                        mind[i][j] = dist[i][x][y] - 1;
                    }
                }
            }
        }
