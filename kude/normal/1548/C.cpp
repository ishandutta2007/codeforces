...
constexpr int FACT_SIZE = 3000100;
...
int main() {
    ...
    cin >> n >> q;
    // f = (1+x) ^ (3n + 3)
    vector<mint> f(3 * n + 3 + 1);
    rep(i, 3 * n + 3 + 1) f[i] = comb(3 * n + 3, i);
    // g = f / 3x
    const mint inv3 = mint(1) / 3;
    rep(i, 3 * n + 3) f[i] = f[i + 1] * inv3;
    f.pop_back();
    // divide by 1 + x + 1/3x^2, i.e., take acc with -x + -1/3x^2
    rep(i, 3 * n) f[i + 1] -= f[i], f[i + 2] -= f[i] * inv3;
    rep(_, q) {
        int x;
        cin >> x;
        cout << f[x].val() << '\n';
