...
fun main(){
    solve.cases{
        ...
        val blacksize = IntArray(n)
        for(i in 0 until n){
            if(L[i] == 1) {
                blacksize[i]++
            }
        }
        G.calculateSubtreeSum(blacksize)

        val root = G.root
        val total = blacksize[root]
        fun contribution(v:Int):Int{
            return (blacksize[v] >= 2 && key[v]).chi
        }
        ...
        fun root_change(from:Int, to:Int){
            bad -= contribution(from)
            bad -= contribution(to)
            blacksize[from] = total - blacksize[to]
            blacksize[to] = total
            bad += contribution(from)
            bad += contribution(to)
        }
