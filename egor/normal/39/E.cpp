...
public:
    void solve() {
        ...
        map<pii, int> res;
        int q = 0;
        RecursiveFunction rec = [&](const auto& self, int a, int b) -> int {
            if (power(infint(a), b) >= n) {
                return -1;
            }
            pii key = {a, b};
            if (res.count(key)) {
            ...
            val = -1;
            if (power(infint(a), b + 1) < n) {
                val = max(self(a, b + 1), self(a + 1, b));
                return -val;
            }
            if (b != 1) {
                return -(val = self(a + 1, b));
            }
            if (n % 2 == a % 2) {
                val = 1;
            } else {
        ...
        int ans = rec(a, b);
        out.printLine(ans == 1 ? "Stas" : ans == 0 ? "Missing" : "Masha");
        err.printLine(q);
    }
};
