...
#line 1 "/home/maspy/library/geometry/convexhull.hpp"
...
void solve() {
  LL(N);
  VEC(pi, XY, N);
  auto I1 = ConvexHull(XY, "lower");
  auto I2 = ConvexHull(XY, "upper");
  vi X1, Y1, X2, Y2;
  FORIN(i, I1) X1.eb(XY[i].fi);
  FORIN(i, I1) Y1.eb(XY[i].se);
  FORIN(i, I2) X2.eb(XY[i].fi);
  FORIN(i, I2) Y2.eb(XY[i].se);

  ll xmin = MIN(X1), xmax = MAX(X1);

  LL(M);
  FOR(_, M) {
    LL(x, y);
    bool ok = [&]() {
      if (x <= xmin || xmax <= x) return false;
      {
        ll r = LB(X1, x);
        ll l = r - 1;
        ll x1 = X1[l];
