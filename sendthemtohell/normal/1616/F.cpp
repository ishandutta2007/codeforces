...
inline void solve() {
	...
	while (t--) {
		...
		for (int i = 0; i < m; ++i) {
			...
			tb[j][k] = tb[k][j] = { i, w };
			if (w != -1) {
				memset(mat[ptr], 0, sizeof mat[ptr]);
				mat[ptr][i] = 1;
				mat[ptr][m] = w % 3;
				++ptr;
				while (ptr >= 8000);
		...
		for (int i = 1; i <= n; ++i) {
			for (int j = i + 1; j <= n; ++j) {
				for (int k = j + 1; k <= n; ++k) {
					if (tb[i][j].y != 0 && tb[j][k].y != 0 && tb[k][i].y != 0) {
						memset(mat[ptr], 0, sizeof mat[ptr]);
						mat[ptr][tb[i][j].x] = 1;
						mat[ptr][tb[j][k].x] = 1;
						mat[ptr][tb[k][i].x] = 1;
		...
		int cur = 0;
		for (int j = 0; j < m; ++j) {
			for (int at = cur; at < ptr; ++at) {
				if (mat[at][j] != 0) {
					for (int i = j; i <= m; ++i) {
						swap(mat[at][i], mat[cur][i]);
					}
					break;
				}
			}
			if (mat[cur][j] == 0) continue;
			int kek = inv(mat[cur][j], 3);
		...
		{
			vector<int> ans(m, 3);
			for (int rw = cur - 1; rw >= 0; --rw) {
				int st;
				for (st = 0; st < m; ++st) if (mat[rw][st] != 0) break;
				while(mat[rw][st] != 1);
				ans[st] = (mat[rw][m] == 0 ? 3 : mat[rw][m]);
				for (int rww = rw - 1; rww >= 0; --rww) {
					mat[rww][m] -= mat[rww][st] * mat[rw][m];
					mat[rww][m] += 27;
					mat[rww][m] %= 3;
				}
			}
