...
int main() {
	...
	CASE(t) {
		int n,k; R(n,k);
		vector<vector<vector<int>>> g(k + 1, vector<vector<int>>(k + 1, vector<int>(4, 0)));

		auto add = [&](int r, int c, int dir) {
			g[r][c][dir]++;
			g[r + dr[dir]][c + dc[dir]][dir ^ 1]++;
		};
		...
		for (int i = 0; i < n; i++) {
			...
			if (x.first == y.first) {
				add(x.first, x.second, UP);
				add(k - x.first, k - x.second, DOWN);
			}
			else {
				add(x.first, x.second, LEFT);
				add(k - x.first, k - x.second, RIGHT);
			}
		}

		vector<vector<int>> dist(k + 1, vector<int>(k + 1, INT_MAX));
		minheap<pair<int, pii>> pq;
		dist[k / 2][k / 2] = 0;
		pq.push({ 0, { k / 2, k / 2 } });
		while (!pq.empty()) {
			...
			for (int dir = 0; dir < 4; dir++) {
				int rr = r + dr[dir];
				int cc = c + dc[dir];
				if (0 <= rr && rr <= k && 0 <= cc && cc <= k && chkmin(dist[rr][cc], dist[r][c] + g[r][c][dir]))
		...
		int minv = INT_MAX;
		for (int i = 0; i <= k; i++) {
			chkmin(minv, dist[0][i]);
			chkmin(minv, dist[i][0]);
			chkmin(minv, dist[k][i]);
			chkmin(minv, dist[i][k]);
		}
