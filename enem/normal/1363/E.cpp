...
tuple<ll,ll,ll> dfs(ll x,ll y,V(ll) ad[],ll a[],ll b[],ll c[])
{
    ll ans=0;
    ll l=0;
    ll r=0;
    ...
    for(auto it:ad[x])
    {
        ...
        {
            a[it]=min(a[it],a[x]);
            tuple<ll,ll,ll> t=dfs(it,x,ad,a,b,c);
            ans+=G(0,t);
            l+=G(1,t);
            r+=G(2,t);
        }
    }
    return MT(ans+min(l,r)*a[x],l-min(l,r),r-min(l,r));
}
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        tuple<ll,ll,ll> t=dfs(0,-1,ad,a,b,c);
        if(G(1,t)!=0||G(2,t)!=0)
            cout<<-1;
        else
            cout<<2*G(0,t);
