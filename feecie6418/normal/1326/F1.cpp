...
int g[20][20],n,q;
ll f[300005][20],a[20][300005],b[300005],ans[300005],fin[305],id;
...
int main(){
	...
	for(int i=1;i<(1<<n);i++){
		for(int j=0;j<n;j++)if(i&(1<<j))for(int k=0;k<n;k++)if(g[j][k])f[i+(1<<k)][k]+=f[i][j];
	...
	for(int i=1;i<(1<<n-1);i<<=1)for(int j=0;j<(1<<n);++j)if(i&j)ans[i^j]-=ans[j];
