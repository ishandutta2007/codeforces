...
#define int ll
int it[2000000];
vector<pair<int, int>> hints; // (first_day, loop_period)
int judge(int go) {
	int ans = go;
	for (int i = 0; i < hints.size(); ++i) {
		if (go < hints[i].first) continue;
		ans--;
		int now = go - hints[i].first;
		ans -= now / hints[i].second;
	}
	return ans;
}
void solve() {
	...
	REP(i, m) {
		...
		if (it[c] == -1) continue;
		int loops = lcm(n, m);
		pair<ll, ll> hoge = crt({ it[c],i }, { n,m });
		if (hoge.second == 0) continue;
		hints.push_back(mp(hoge.first + 1, hoge.second));
	}
	int bot = 1000000000000000000LL;
	int top = 0;
	while (bot - top > 1) {
		int mid = (bot + top) / 2LL;
		if (judge(mid) >= k) {
