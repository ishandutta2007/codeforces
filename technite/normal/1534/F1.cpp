...
vvi adj,adj_rev,adj_scc;
vector<bool> vis,used;
vi order,component,roots,root_nodes;
...
int main()
{
    ...
    vi values(m);
    vector<string> s(n);
    vvi cnt(n,vi(m,-1)),ahead(n,vi(m,-1));
    for(int i = 0 ; i < n ; i++)
    {
        cin>>s[i];
    }
    ...
    for (int i = n-1; i >= 0; i--) 
    {
        for (int j = 0; j < m; j++) 
        {
            if (i < n-1) ahead[i][j] = ahead[i+1][j];
            if (s[i][j] == '#') ahead[i][j] = cnt[i][j];
        }
    }
    adj=adj_rev=adj_scc=vvi(total);
    vis=used=vector<bool>(total,0);
    roots=vi(total,0);
    for(int i = 0 ; i < n ; i++)
    {
        for(int j  = 0 ; j < m ; j++)
        {
            if(cnt[i][j]==-1)
                continue;
            //cout<<i<<" "<<j<<endl;
            if(i<n-1 and ahead[i+1][j]!=-1)
            {
                add(cnt[i][j],ahead[i+1][j]);
            }
            //cout<<i<<" "<<j<<endl;
            if(i>0 and cnt[i-1][j]!=-1)
            {
                add(cnt[i][j],cnt[i-1][j]);
            }
            if(j<m-1 and ahead[i][j+1]!=-1)
            {
                add(cnt[i][j],ahead[i][j+1]);
            }
            if(j>0 and ahead[i][j-1]!=-1)
            {
                add(cnt[i][j],ahead[i][j-1]);
            }
        }
