...
class Solver {
    ...
    public void solve() {
        ...
        var dmin = new long[n + 1];
        dmin[0] = LM;
        for (int i = 0; i < n; i++)
        {
            asum[i + 1] = asum[i] + a[i];
            dmin[i + 1] = Math.Min(dmin[i], d[i]);
        }
        var rmax = new long[n + 1];
        rmax[n] = 0;
        for (int i = n - 1; i >= 0 ; i--)
        {
            DBG(i, n);
            rmax[i] = Math.Max(rmax[i + 1], asum[n] - asum[i] - d[i]);
        }
        ...
        if (k == 0) {
            ...
            for (int i = 1; i < n; i++)
            {
                ans = Math.Max(ans, Math.Max(asum[i] - dmin[i], 0) + rmax[i]);
            }
            long benef = -LM;
            for (int i = 0; i < n - 2; i++)
            {
                benef = Math.Max(benef + a[i], a[i] - d[i]);
                ans = Math.Max(ans, benef + asum[n] - asum[i + 2]);
            }
            Prt(ans);
        }
        else {
            Prt(Math.Max(Math.Max(asum[n] - dmin[n - 1], a[n - 1] - d[n - 1]), 0));
        }
