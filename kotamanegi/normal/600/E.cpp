...
int tmp_ans[200000];
int cnter[200000];
map<int, int> bobo[200000];
vector<int> vertexs[200000];
void dfs(int now, int back) {
    bobo[now][color[now]]++;
    cnter[now] = 1;
    tmp_ans[now] = color[now];
    for (auto& x : vertexs[now]) {
        if (back == x) continue;
        dfs(x, now);
        if (bobo[x].size() > bobo[now].size()) {
            swap(bobo[x], bobo[now]);
            swap(cnter[x], cnter[now]);
            swap(tmp_ans[x], tmp_ans[now]);
        }
        for (auto& y : bobo[x]) {
            bobo[now][y.first] += y.second;
            if (cnter[now] < bobo[now][y.first]) {
                cnter[now] = bobo[now][y.first];
                tmp_ans[now] = 0;
            }
            if (cnter[now] == bobo[now][y.first]) {
                tmp_ans[now] += y.first;
