...
void q() {
  ...
  SegmentTree seg(
      vi(N + 1, -inf), [](int a, int b) { return max(a, b); }, -inf);
  SegmentTree segmid(
      vi(N + 1, -inf), [](int a, int b) { return max(a, b); }, -inf);
  // 
  seg.update(0, 0);
  ...
  sort(all(kukan), [](pl s, pl t) {
    return pl{s.fi, -s.se} < pl{t.fi, -t.se};
  });

  each2(l, r, kukan) {
    // [l, r) 
    ll best = seg.query(0, l);
    // overlap 
    ll be2 = segmid.query(l, r);
    be2 += l;
    be2 -= 1;
    amax(best, be2);
