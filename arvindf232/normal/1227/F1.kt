...
fun main(){
    ...
    solve.cases{
        ...
        for(changes in 1..notfree){
            ...
            kfact = kfact modM ( changes)
            if(changes % 2 == 0){
                halfkfact = halfkfact modM ((changes / 2 ).inverse())
                val inv = halfkfact
                val comb = kfact modM (inv modM inv)
...
//            changes dei base
            val rest = intPow(k-2,notfree - changes, pI)
            ret += (base) modM (rest) modM (FACT.choose(notfree, changes).toInt())
            ret = ret.adjust()
        }
        ret = ret modM frees
