...
#define Bit(mask,pos) ((mask)<<1>>(pos)&1)
#define Add(mask,pos) ((mask)|1<<(pos)>>1)
///////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
#define MAXN 18
#define MAXM 10010
...
void Init(){
	For(i,n)
		For(j,n){
			if( i == j ) continue;
			int tans = INF;
			For(k,m) Mymin(tans,abs(a[i][k]-a[j][k]));
			cost[i][j] = tans;
		}
}

int dp[MASK][MAXN];
#define NOW dp[mask][last]
#define KNOW ~NOW
int Dfs( int mask , int last , int bottom ){
	if( mask == (1<<n)-1 ) return cost[last][bottom];
	if(KNOW) return NOW;
	int ans = 0;
	For(u,n){
		if(Bit(mask,u)||u==bottom) continue;
		int tans = cost[last][u];
		if( tans <= ans ) continue;
		Mymin(tans, Dfs(Add(mask,u),u,bottom) );
		Mymax(ans,tans);
	}
	return NOW = ans;
}
...
int main(){
	...
	if( n == 1 ){
		int ans = INF;
		For(i,m-1) Mymin(ans,abs(a[1][i]-a[1][i+1]));
		END(ans);
	}
	Init();
	int ans = 0;
	For(top,n)
		For(bottom,n){
			...
			int tans = INF;
			For(i,m-1) Mymin(tans,abs(a[top][i+1]-a[bottom][i]));
			if( ans >= tans ) continue;
			// O(tans);
			Msn(dp,-1);
			int mask = 0;
			mask = Add(mask,top);
			mask = Add(mask,bottom);
			Mymin(tans , Dfs(mask,top,bottom) );
			Mymax(ans,tans);
			// cout << top << " " << bottom << endl;
			// O(tans);
			// O(Dfs(mask,top,bottom));
		}
	cout << ans << endl;
