...
fun solve(max:Int, odd:Int, even:Int, sumnow:Int):Int {
	val x = listOf(max,odd, even , sumnow)
	if(x in cache){
		return cache[x] !!
	}
	if(odd == 0 && even == 0){
	...
	var now = if(max==1) -1 else 1
	if(odd >= 1){
		val newsum = if(max == 1) 1 - sumnow else sumnow
		val next = solve(-max, odd - 1 , even, newsum)
		if(max == 1) now = maxOf(now,next) else now = minOf(now,next)
	}
	if(even >= 1){
		val next = solve(-max, odd, even-1, sumnow)
		if(max == 1) now = maxOf(now,next) else now = minOf(now,next)
	}
	cache[x] = now
	return now
...
fun main(){
	...
	val DP = Array(2){Array(103){IntArray(103)} }
    ...
    solve.cases{
		...
		val ans = solve(1,counts[1], counts[0],0)
		if(ans == 1){
			put("Alice")
		}else{
			put("Bob")
