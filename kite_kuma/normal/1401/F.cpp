...
int main() {
	...
	vector<ll> data(len * 2);

	rep(i, len) data[len + i] = a[i];
	// debug(data);
	rrep(i, len) data[i] = data[i << 1 | 1] + data[i << 1];
	// debug(data);
	vector<ll> trans(n + 1);  // ixor
	rep(i, n + 1) trans[i] = 0;
	...
	rep(i, m) {
		...
		if(t == 1) {
			...
			while(i > 0) {
				i--;
				x >>= 1;
				data[x ^ trans[i]] = data[trans[i + 1] ^ (x << 1)] + data[(x << 1 | 1) ^ trans[i + 1]];
			}
		} else if(t == 2) {
			ll k;
			cin >> k;
