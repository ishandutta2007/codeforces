...
inline void AC() {
    ...
    if(n <= 4) {
        writeln(n-1);
        for(_ i = 1; i <= n-1; ++i) writeln(mp(1, i));
    } else {
        _ t = (2*n+1)/3;
        writeln(t);
        set<_> st;
        for(_ i = 1; i <= t; ++i) st.insert(i);
        _ p = 1, mx = 0;
        for(_ i = n-t-1; i >= 0; i -= 2) {
            writeln(mp(p, i+p));
            if(i) writeln(mp(i+p, p));
            st.erase(p);
            st.erase(i+p);
            mx = max(mx, i+p);
            ++p;
        ...
        for(; !st.empty(); st.erase(st.begin())) writeln(mp(*st.begin(), *st.begin()));
    }
