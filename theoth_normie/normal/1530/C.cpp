...
priority_queue<int,vector<int>,less<int>> trash1,trash2;
int sum1,sum2;
int main()
{
	fio;
    ...
    for (t1=0;t1<t;t1++)
    {
        ...
        while(trash1.size()) trash1.pop();
        while(trash2.size()) trash2.pop();
        for (i=1;i<=n;i++)
        {
            cin>>u;
            sum1+=u;
        ...
        while(sum1<sum2)
        {
            ...
            sum1+=100;
            trash2.push(0);
            if (pq1.size()>(n+a)-(n+a)/4)
            {
                sum1-=pq1.top();
                pq1.pop();
            }
            if (pq2.size()<(n+a)-(n+a)/4)
            {
                sum2+=trash2.top();
                pq2.push(trash2.top());
                trash2.pop();
