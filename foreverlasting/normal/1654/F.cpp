...
namespace MAIN{
    int n;
    char str[N];
    int buc[N],sa[N],fr[N],sc[N],m;
    inline void MAIN(const res &Case){
        n=read(),scanf("%s",str);
        res sz=1<<n;
        m=26;
        for(res i=0;i<sz;i++)buc[fr[i]=str[i]-'a']++;
        for(res i=1;i<=m;i++)buc[i]+=buc[i-1];
        for(res i=sz-1;~i;i--)sa[--buc[fr[i]]]=i;
        for(res k=1;k<=sz;k<<=1){
            for(res i=1;i<=m;i++)buc[i]=0;
            for(res i=0;i<sz;i++)buc[fr[sa[i]^k]]++;
            for(res i=1;i<=m;i++)buc[i]+=buc[i-1];
            for(res i=sz-1;~i;i--)sc[--buc[fr[sa[i]^k]]]=sa[i];
            for(res i=1;i<=m;i++)buc[i]=0;
            for(res i=0;i<sz;i++)buc[fr[sc[i]]]++;
            for(res i=1;i<=m;i++)buc[i]+=buc[i-1];
            for(res i=sz-1;~i;i--)sa[--buc[fr[sc[i]]]]=sc[i];
            for(res i=0;i<sz;i++)sc[i]=fr[i];
            fr[sa[0]]=m=1;
            for(res i=1;i<sz;i++)fr[sa[i]]=(sc[sa[i]]==sc[sa[i-1]]&&sc[sa[i]^k]==sc[sa[i-1]^k])?m:++m;
            if(m==sz)break;
        }
        for(res i=0;i<sz;i++)printf("%c",str[i^sa[0]]);
    }
}
int main(){
//    srand(time(0));
