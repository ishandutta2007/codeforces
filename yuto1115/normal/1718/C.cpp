...
public:
    constexpr dual_segtree() : dual_segtree(0) {}
    
    constexpr dual_segtree(int n) : dual_segtree(vector<S>(n, M::e)) {}
    
    constexpr dual_segtree(const vector<S> &init) : _n(int(init.size())) {
        sz = 1;
        while (sz < _n) sz *= 2;
        d.assign(sz * 2, M::e);
...
public:
    ...
    vi enum_divisors(int now) {
        assert(now >= 1);
        vp v;
        for (auto p: factor_list(now)) v.pb(p);
        vi res;
        auto f = [&](auto &f, int i, int x) -> void {
            if (i == SZ(v)) {
                res.pb(x);
                return;
            }
...
void solve() {
    ...
    auto add = [&](int l, int r, ll val) {
        ...
        int k = l2[r - l + 1];
        chmax(tb[l][k], val);
        chmax(tb[r + 1 - (1 << k)][k], val);
    };
    rep(i, sz) {
        vl sum(dv[i]);
        rep(j, n) sum[j % dv[i]] += a[j];
        vi pre(dv[i]);
        vi a2 = a;
        rep(t, q) {
            auto[p, x] = qs[t];
            int j = p % dv[i];
            add(pre[j], t, sum[j] * dv[i]);
            sum[j] += x - a[p];
            a[p] = x;
            pre[j] = t + 1;
        }
        rep(j, dv[i]) add(pre[j], q, sum[j] * dv[i]);
        a = a2;
    }
//    if (n >= 1000) {
//        vecout(tb[0]);
//        return;
//    }
    dual_segtree <M> st(q + 1);
    rep(i, q + 1) rep(j, 20) {
            if (i + (1 << j) > q + 1) break;
            st.apply(i, i + (1 << j), tb[i][j]);
        }
    rep(i, q + 1) print(st.get(i));
}
