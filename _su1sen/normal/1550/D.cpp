...
int main() {
    factorial<mint> fac(200010);
    input(int, t);
    loop(t) {
        input(int, n, l, r);
        vector<int> al(n), ar(n);
        int min_max_abs = numeric_limits<int>::max();
        int min_min_abs = numeric_limits<int>::max();
        rep(i, n) {
            al[i] = l - (i + 1);
            ar[i] = r - (i + 1);
            int max_abs = max(abs(al[i]), abs(ar[i]));
            int min_abs = min(abs(al[i]), abs(ar[i]));
            chmin(min_max_abs, max_abs);
            chmin(min_min_abs, min_abs);
        }
        mint ans = fac.binom(n, n / 2) * min_min_abs;
        if (n & 1) {
            ans += ans;
        }
        int i = 0, j = n - 1;
        for (int k = min_min_abs + 1; k <= min_max_abs;) {
            bool bl = false, br = false;
            while (i < n and abs(al[i]) < k) {
                ++i;
                bl = true;
            }
            while (j >= 0 and abs(ar[j]) < k) {
                --j;
                br = true;
            }
            int next_k = k + 1;
            if (not (bl or br)) {
                next_k = numeric_limits<int>::max();
                if (i < n) {
                    chmin(next_k, abs(al[i]));
                }
                if (j >= 0) {
                    chmin(next_k, abs(ar[j]));
                }
                ++next_k;
            }
            int cl = n - i;
            int cr = j + 1;
            int cm = max(0, (cl + cr) - n);
            cl -= cm, cr -= cm;
            ans += fac.binom(cm, (n / 2) - cl) * (next_k - k);
            if (n & 1) {
                ans += fac.binom(cm, (n - (n / 2)) - cl) * (next_k - k);
            }
            k = next_k;
        }
        print(ans);
