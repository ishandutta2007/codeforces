...
void solve() {
	...
	vector <pll> in;
	REP(i, n) REP(j, 6) in.emplace_back(b[i] - a[j], i);
	sort(in.begin(), in.end());
	vll seen(n);
	int complete = 0;
	auto touch = [&](int x, int v) {
		if (seen[x]) complete--;
		seen[x] += v;
		if (seen[x]) complete++;
	};
	...
	int l = 0, r = 0;
	while (true) {
		debug(range(in, l, r), imie(seen), imie(complete));
		if (complete == n) {
			ll ran = in[r - 1].first - in[l].first;
			mini(ans, ran);
			touch(in[l].second, -1);
			l++;
		}
		else {
			if (r == (int)in.size()) break;
			touch(in[r].second, 1);
			r++;
		}
	}
