...
void __solve()
{
    ...
    for(int i=0,c=n/2+1-k; i<n; i++)
    {
        if(k>1)
        {
            ...
            assert(~ii);
            ans.emplace_back(i+1,ii+1);
            string tmp=revstr(string(s.begin()+i, s.begin()+ii+1));
            copy(all(tmp),s.begin()+i);
            ...
            ans.emplace_back(i+1,ii+1);
            tmp=revstr(string(s.begin()+i, s.begin()+ii+1));
            copy(all(tmp),s.begin()+i);
        }
        else
        {
