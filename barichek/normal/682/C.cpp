...
void dfs(int now,int pred,int rebr)
{
    predok[now]=pred;
    bool ok=false;
    visit[now]=true;
    dist[now]=max((ll)rebr,(ll)dist[pred]+(ll)rebr);
    //cout<<now<<" "<<pred<<" "<<dist[pred]<<" "<<rebr<<"\n";
    for (int i=0;i<reb[now].size();i++)
    {
        //cout<<reb[now][i].fir<<'\n';
        if (!visit[reb[now][i].fir]) dfs(reb[now][i].fir,now,reb[now][i].sec), kilreb[now]++, ok=true;
    }
    if (dist[now]>a[now]) wantdel[now]=true;
}
...
void dfs1(int now)
{
    ...
    {
        //cout<<reb[now][i].fir<<'\n';
        if (!visit1[reb[now][i].fir]&&predok[now]!=reb[now][i].fir) dfs1(reb[now][i].fir);
    }
}
...
int main()
{
    ...
    dfs(1,0,0);
    //for (int i=1;i<=n;i++)
      //  cout<<i<<" "<<dist[i]<<" "<<kilreb[i]<<"\n";
    int ans=0;
    for (int i=1;i<=n;i++)
        if (!visit1[i]&&wantdel[i]) dfs1(i);
        for (int i=1;i<=n;i++)
            if (visit1[i]) ans++;
    /*
    while (!listki.empty())
    {
        int dop=*listki.begin();
        writeln(dop);
        listki.erase(listki.begin());
        kilreb[predok[dop]]--;
        if (kilreb[predok[dop]]==0&&predok[dop]!=1&&dist[predok[dop]]>a[predok[dop]]) listki.pb(predok[dop]);
        ans++;
    }
    */
    writeln(ans);
