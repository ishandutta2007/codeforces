...
void solve() {
    ...
    for (int i = 1, a; i <= n; i++) {
        ...
        else zero.emplace_back(a);
    }
    if (max(neg.size(), pos.size()) + (min(1, int(zero.size()))) >= 3) {
        cout << "NO\n";
        return;
    } else {
        for (auto&& x : neg)
            for (auto&& y : zero)
                for (auto&& z : pos) {
                    if (val.count(x + y + z) == 0) {
                        cout << "NO\n";
                        return;
                    }
                }
        vector<int64_t> x;
        for (auto&& a : neg) x.emplace_back(a);
        for (auto&& a : pos) x.emplace_back(a);
        if (zero.size()) x.emplace_back(0);
        for (int i = 0; i < x.size(); i++) {
            for (int j = i + 1; j < x.size(); j++) {
                for (int k = j + 1; k < x.size(); k++) {
                    if (val.count(x[i] + x[j] + x[k]) == 0) {
                        cout << "NO\n";
                        return;
                    }
                }
