...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  reverse(all(s));
  N=accumulate(all(ch1),0LL);
  map<char,int> mp;
  rep(i,N)mp[s[i]]++;
  rep(i,t.size())if(mp[t[i]]!=ch1[i])break;
  if(i!=int(t.size())){
    print(-1);
    continue;
  }
  reverse(all(t));
  string chs=s.substr(0,N),cht;
  for(auto x:t){
    cht+=chs;
    if(cht.size()>s.size())break;
    string nxts;
    for(auto y:chs)if(x!=y)nxts+=y;
    chs=move(nxts);
  }
  if(cht==s){
    print(s.substr(0,N)<<" "<<t);
  }else print(-1);
}}