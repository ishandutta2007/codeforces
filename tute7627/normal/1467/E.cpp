...
int main(){
  ...
  vector<ll>imos(n+1);
  for(auto z:mp){
    for(auto v:z.se)bit.add(in[v],1);
    for(auto v:z.se){
      if(bit.sum(in[v],out[v]-1)!=bit.sum(n-1)){
        imos[in[v]]++;
        imos[out[v]]--;
      }
      for(auto to:g[v]){
        if(in[v]>in[to])continue;
        if(bit.sum(in[to],out[to]-1)==0)continue;
        imos[0]++;
        imos[in[to]]--;
        imos[out[to]]++;
      }
  ...
  rep(i,0,n)if(imos[i]==0)res++;
  cout<<res<<endl;
  /*rep(i,0,n){
    cout<<i spa imos[in[i]]<<endl;
  }*/
  return 0;
}