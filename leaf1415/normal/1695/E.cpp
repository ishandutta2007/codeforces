...
ll n;
multiset<ll> G[600005][2];
vector<ll> ans[2];
string ansS[2], ansT[2];
stack<P> stk;

void pathdfs(int v, int t, int b, bool flag){
	stk.push(P(v, b));
	if(flag && v == t) return;
	int u = *G[v][b].begin();
	G[v][b].erase(G[v][b].begin()), G[u][b].erase(G[u][b].lower_bound(v));
	pathdfs(u, t, 1-b, true);
}
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> u >> v;
		G[u][0].insert(v), G[u][1].insert(v);
		G[v][0].insert(u), G[v][1].insert(u);
	}

	rep(i, 1, 2*n){
		if(sz(G[i][0]) == 0) continue;
		...
		vector<ll> tmp;
		while(stk.size()){
			int v = stk.top().first, b = stk.top().second; stk.pop();
			if(G[v][b].size() == 0) tmp.push_back(v);
			else pathdfs(v, v, b, false);
		}
		tmp.pop_back();
		...
		rep(i, 0, m-1){
			ans[0].pb(tmp[i]);
			ans[1].pb(tmp[2*m-1-i]);
		}
		if(m % 2 == 0){
			rep(i, 1, m/2) ansS[0] += "LR", ansS[1] += "LR";
			ansT[0] += "U", ansT[1] += "D";
			rep(i, 1, m/2-1) ansT[0] += "LR", ansT[1] += "LR";
			ansT[0] += "U", ansT[1] += "D";
		}
		else{
			rep(i, 1, (m-1)/2) ansS[0] += "LR", ansS[1] += "LR";
			ansS[0] += "U", ansS[1] += "D";
			ansT[0] += "U", ansT[1] += "D";
			rep(i, 1, (m-1)/2) ansT[0] += "LR", ansT[1] += "LR";
		}
	}
