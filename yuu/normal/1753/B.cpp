...
public:
    ...
    Multi& operator+=(const Multi& other) {
        for (size_t i = 0; i < COUNT; i++) data[i] += other[i];
        return *this;
    }
...
using Hash_t = Multi<int64_t, COUNT>;
Hash_t base;
const vector<int> fixed_base = {999'727'999, 999'999'937, 1'000'000'007, 1'000'000'009};
...
void solve() {
    ...
    for (int i = 0; i < COUNT; i++)
        if (i < fixed_base.size()) {
            base[i] = fixed_base[i];
        } else {
            base[i] = uniform_int_distribution<int64_t>(1e9, 2e9)(rng);
        }
    for (int i = 0; i < COUNT; i++) h[i] = 1;
    for (int i = 0; i < COUNT; i++) g[i] = 0;
    int p = 1;
