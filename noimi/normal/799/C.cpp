...
int main() {
    ...
    auto f = [&](vpi v, int x) {
        vpi w;
        sort(all(v), [](auto x, auto y) { return x.se < y.se; });
        for(auto [b, p] : v) {
            int k = lb(w, pii(x - p, inf<int>)) - 1;
            if(k != -1) { chmax(res, b + w[k].se); }
            if(empty(w) or w.back().se < b) w.eb(p, b);
        }
    };
    f(C, c), f(D, d);
    chmax(res, 0);
