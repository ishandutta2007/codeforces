...
{
    dep[x]=dep[fa]+1;dth[x]=x;
    for(auto to:nxt[x])if(to!=fa){findson(to,x);if(dep[dth[to]]>dep[dth[son[x]]])son[x]=to;}
    if(son[x])dth[x]=dth[son[x]];
    return;
}
...
{
    if(son[x])work(son[x],x),ans[x]=ans[son[x]]+1;
    for(auto to:nxt[x])if(to!=fa&&to!=son[x])
    {
        work(to,x);
        for(int i=l[dth[x]],j=l[dth[to]];j<=r[dth[to]];i++,j++)
        {
            f[i]+=f[j];
            if(f[i]>f[ans[x]+l[dth[x]]-1]||(f[i]==f[ans[x]+l[dth[x]]-1]&&i<ans[x]+l[dth[x]]-1))ans[x]=i-l[dth[x]]+1;
        }
    }
    f[--l[dth[x]]]=1;if(f[l[dth[x]]+ans[x]]==1)ans[x]=0;
    return;
}
int main()
{
    ...
    findson(1,0);
    for(int i=1;i<=n;i++)minl[i]=INF;
    for(int i=1;i<=n;i++)minl[dth[i]]=min(minl[dth[i]],dep[i]);
    int lst=0;
    for(int i=1;i<=n;i++)if(dth[i]==i)l[i]=r[lst]+1,r[i]=l[i]+dep[i]-minl[i],l[i]=r[i]+1,lst=i;
    // for(int i=1;i<=n;i++)printf("%d ",dth[i]);puts("");
    // for(int i=1;i<=n;i++)printf("%d ",r[dth[i]]);puts("");
    work(1,0);
