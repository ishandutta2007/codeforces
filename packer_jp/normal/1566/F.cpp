...
int main() {
    ...
    while (t--) {
        ...
        a[n] = -1e10, a[n + 1] = 1e10;
        vll l(m), r(m);
        for (ll i : rep(m)) cin >> l[i] >> r[i];
        for (ll i : rep(n + 2)) evs.emplace_back(a[i], LLONG_MAX);
        for (ll i : rep(m)) evs.emplace_back(l[i], r[i]);
        sort(all(evs));
        ll last, i = -1;
        vector<pll> tar;
        vector dp(n + 2, vll(2, LLONG_MAX));
        dp[0][0] = dp[0][1] = 0;
        for (auto [l, r] : evs) {
            if (r == LLONG_MAX) {
                // point
                ...
                while (!tar.empty() && tar.back().second >= l) tar.pop_back();
                ll p = last;
                for (auto [sl, sr] : tar) {
                    chmin(dp[i + 1][0], dp[i][0] + (p - last) * 2 + (l - sr) * 1);
                    chmin(dp[i + 1][0], dp[i][1] + (p - last) * 1 + (l - sr) * 1);
                    chmin(dp[i + 1][1], dp[i][0] + (p - last) * 2 + (l - sr) * 2);
                    chmin(dp[i + 1][1], dp[i][1] + (p - last) * 1 + (l - sr) * 2);
                    p = sl;
                }
                chmin(dp[i + 1][0], dp[i][0] + (p - last) * 2);
                chmin(dp[i + 1][0], dp[i][1] + (p - last) * 1);
                chmin(dp[i + 1][1], dp[i][0] + (p - last) * 2);
                chmin(dp[i + 1][1], dp[i][1] + (p - last) * 1);
                last = l;
                ++i;
                tar.clear();
            } else {
                // segment
                if (!tar.empty() && tar.back().first == l) continue;
                while (!tar.empty() && tar.back().second >= r) tar.pop_back();
                tar.emplace_back(l, r);
            }
        }
        cout << dp[n + 1][0] << endl;
    }
