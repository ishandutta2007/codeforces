...
inline void solve()
{
	n=read(),tots=tott=0,res=0;
	for(register int i=1;i<=2*n;i++)
	{
		x[i]=abs(read()),y[i]=abs(read());
		!x[i]?s[++tots]={x[i],y[i]}:t[++tott]={x[i],y[i]};
	}
	sort(s+1,s+tots+1,greater<Item>()),sort(t+1,t+tott+1);
	for(register int i=1;i<=n;i++)
	{
		res+=hypot(s[i].y,t[i].x);
	}
	printf("%.12lf\n",res);
}
int main()
