...
constexpr ll e() { return LINF; }
constexpr ll mapping(ll f, ll x) { return f + x; }
constexpr ll composition(ll f, ll g) { return f + g; }
constexpr ll id() { return 0LL; }
...
int main() {
    ...
    pd[0] = 0;
    rep(x, k) {
        atcoder::lazy_segtree<ll, op, e, ll, mapping, composition, id> seg(pd);
        vec<int> last(n, -1);
        rep(i, n + 1) {
            pd[i] = seg.prod(0, i);
            if (i < n) {
                int v = a[i];
                seg.apply(0, last[v] + 1, i - last[v]);
                last[v] = i;
            }
        }
    }
