...
void solve() {
    ...
    auto dfs = [&](auto&&dfs, int v, int p) -> void {
        val[v] = (p == -1 ? 1LL : val[p]) * cnt[v];
        if(val[v] > k) val[v] = k + 1;
        for(int nv: g[v]) {
            if(nv == p) continue;
            dfs(dfs, nv, v);
        }
    ...
    auto dfs2 = [&](auto&&dfs2, int v, int p) -> void {
        ...
        for(int nv: g[v]) {
            ...
            dfs2(dfs2, nv, v);
            sum0 += dp[nv][0];
            cand.eb(dp[nv][1] - dp[nv][0]);
        }
        int c0 = (cnt[v] == 0 ? 0 : k / val[v]);
        Sort(cand); Rev(cand);
        int now = sum0;
        int use0 = k / (p == -1 ? 1 : val[p]);
        int use1 = use0 + 1;
        int res0 = k / (p == -1 ? 1 : val[p]) - c0 * cnt[v];
        int res1 = res0 + 1;
        int sz = si(cand);
        dp[v][0] = dp[v][1] = now;
        dp[v][0] += s[v] * use0;
        dp[v][1] += s[v] * use1;
        debug(v, now, res0, res1);
        debug(use0, use1);
        dump(cand);
        rep(i, sz) {
            if(i < res0) dp[v][0] += cand[i];
            if(i < res1) dp[v][1] += cand[i];
        }
        debug(dp[v][0], dp[v][1]);
        return;
    };
