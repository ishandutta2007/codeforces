...
map<int, int> last;
int n, m, pref[MAX_N], a[MAX_N], ans[MAX_N], tree[MAX_N];
vector<vector<pair<int, int> > > z(MAX_N);

void update(int i, int p, int h)
{
    for (; i < n; i |= i + 1)
        tree[i] ^= p ^ h;
}
...
int xor_sub(int l, int r)
{
    return xor_pref(r) ^ xor_pref(l - 1) ^ pref[r + 1] ^ pref[l];
}
...
{   //*
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        scanf("%d%d", &l, &r);
        l--, r--;
        z[r].push_back(make_pair(l, i));
    }
    for (int i = 0; i < n; i++)
    {
        if (!(last.find(a[i]) == last.end()))
            update(last[a[i]], a[i], 0);
        last[a[i]] = i;
        update(i, 0, a[i]);
        for (pair<int, int> t : z[i])
            ans[t.second] = xor_sub(t.first, i);
    }
    for (int i = 0; i < m; i++)
        cout << ans[i] << "\n";
    return 0;
