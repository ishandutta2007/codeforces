...
int main() {
    ...
    vmint p(m + 1), ip(m + 1);
    p[0] = ip[0] = 1;
    rep(i, m) p[i + 1] = p[i] * (n - i), ip[i + 1] = ip[i] / (n - i);

    rep2(i, 1, m - 1) {
        // i + 1 
        rep3(j, m, 0) {
            rep(s, k + 1) {
                if(dp[j][s].a == 0) continue;
                mint f = dp[j][s];
                int ret = n - j;
                for(int t = 1; t * (i + 1) + j <= m and s + i * t <= k; t++) {
                    // n - j  (i + 1) * t 
                    f *= p[(i + 1) * t + j] * ip[j + (i + 1) * (t - 1)] / (i + 1);

                    dp[j + (i + 1) * t][s + i * t] += f / fact(t);
                }
            }
        }
    }
    ...
    mint ans[2];
    rep(i, m + 1) rep2(j, 0, k) ans[j & 1] += dp[i][j];
    vmint out(k + 1);
    rep3(i, k, 1) {
        out[i] = ans[i & 1];
        rep(j, m + 1) ans[i & 1] -= dp[j][i];
    }
    rep2(i, 1, k) cout << out[i] << " ";
    cout << endl;
}