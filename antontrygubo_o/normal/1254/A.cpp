...
void solve()
{
    ...
    int cnt = 0;
    for (int i = 0; i<r; i++)
        for (int j = 0; j<c; j++) if (a[i][j]=='R') cnt++;
        vector<pair<int, int>> guys;
        for (int i = 0; i<r; i++)
        {
            if (i%2==0) for (int j = 0; j<c; j++) guys.push_back(mp(i, j));
            else for (int j = c-1; j>=0; j--) guys.push_back(mp(i, j));
        }
        vector<int> pref_sum(r*c+1);



        for (int i = 0; i<r*c; i++)
        {
            pref_sum[i+1] = pref_sum[i] + ((a[guys[i].first][guys[i].second]=='R') ? 1 : 0);
        }
    /*for (auto it: pref_sum) cout<<it<<' ';
    cout<<endl;*/

        vector<int> last_met(cnt+1);
        for (int i = 0; i<=r*c; i++)
        {
            last_met[pref_sum[i]] = i-1;
        }
        last_met[0] = -1;
    vector<int> need(k, cnt/k);
        for (int i = 0; i<cnt%k; i++) need[i]++;

    /*for (auto it: need) cout<<it<<' ';
    cout<<endl;*/
    vector<int> pref(k+1);

    for (int i = 1; i<=k; i++) pref[i] = pref[i-1] + need[i-1];
    ...
    for (auto it: last_met) cout<<it<<' ';
    cout<<endl;*/

    vector<vector<char>> ans(r, vector<char>(c));
    for (int i = 0; i<k; i++)
    {
        for (int t = last_met[pref[i]]+1; t<=last_met[pref[i+1]]; t++) ans[guys[t].first][guys[t].second] = s[i];
    }
    for (int i = 0; i<r; i++)
    {
