...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        REP(i, n) {
            REP(j, m) {
                int x = lig[j].first - rob[i].first + 1, y = lig[j].second - rob[i].second + 1;
                if (x > 0 && y > 0) {
                    real.eb(x, y);
                }
            }
        ...
        int ans = real[0].first;

        vector<int> pre;
        int largest = INT32_MIN;
        for (int i = 0; i < real.size(); i++) {
            chkmax(largest, real[i].second);
            if (i < real.size()-1) {
                chkmin(ans, largest + real[i+1].first);
            }
            else {
                chkmin(ans, largest);
            }
