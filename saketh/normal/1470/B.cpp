...
int non_square_part(int v) {
    int res = 1;
    for (auto pr : sv.factor(v))
        if (pr.s&1)
            res *= pr.f;
    return res;
}
...
void solve() {
    ...
    map<int, int> ct;
    trav (e, a) {
        ckmax(ans[0], ++ct[non_square_part(e)]);
    }

    // apply one iteration... all the groups with even size go to non-square part 1
    // OR can be odd size if non-square part is 1
    // all the groups with odd size keep the same non-square part
    int join = 0;
    trav (e, ct) {
        if (e.s % 2 == 0 || e.f == 1)
            join += e.s;
    ...
    ans[1] = max(join, ans[0]);

    int Q; re(Q); F0R (q, Q) {
        int64_t query; re(query);

        ps(ans[min(query, int64_t(1))]);
    }
}
