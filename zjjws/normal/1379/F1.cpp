...
{
    int x,y,z;rin>>x>>y;
    if(qy.find(make_pair(x,y))==qy.end()){if(x&1)zjj[x+1>>1].insert(y+1>>1);else gyq[x>>1].insert(y>>1);}
    else {if(x&1)zjj[x+1>>1].erase(y+1>>1);else gyq[x>>1].erase(y>>1);}
    qy[make_pair(x,y)]^=1;updata(1,n,1,x+1>>1);puts(nod[1].ans?"NO":"YES");return;
}
int main()
{
