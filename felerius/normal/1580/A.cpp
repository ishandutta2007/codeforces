// Portal (https://codeforces.com/contest/1580/problem/A)
...
int main() {
    ...
    while (T--) {
        ...
        auto sum = [&](int yl, int yr, int xl, int xr) {
            int ans = pref[xr - 1][yr - 1];
            if (xl)
                ans -= pref[xl - 1][yr - 1];
            if (yl)
                ans -= pref[xr - 1][yl - 1];
        ...
        int mn = 16;
        rep(l, m) rep(r, l + 4, m + 1) {
            deque<int> one_rows;
            rep(b, 1, n + 1) {
                auto check_row = [&](int t) {
                    int w = r - l, h = b - t;
                    if (h >= 5) {
                        int ans = sum(l + 1, r - 1, t + 1, b - 1);
                        ans += 2 * (h - 2) - sum(l, l + 1, t + 1, b - 1) - sum(r - 1, r, t + 1, b - 1);
                        ans += 2 * (w - 2) - sum(l + 1, r - 1, t, t + 1) - sum(l + 1, r - 1, b - 1, b);
                        mn = min(mn, ans);
                    }
                };
                ...
                if (sum(l + 1, r - 1, b - 1, b))
                    one_rows.push_back(b - 1);
                if (sz(one_rows) > mn)
                    one_rows.pop_front();
            }
        }
