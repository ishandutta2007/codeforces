...
int main() {
	...
	int s,t; R(s,t);
	if (find(all(a), t) == a.end()) a.emplace_back(t);

	dsu dsu(n + m + 7);
	vector<int> dist(n + m + 7, INT_MAX);
	vector<int> root(n + m + 7);
	for (int i = 0; i < n + m + 7; i++) root[i] = i;
