...
signed main()
{
	...
	{
		rdn;int a0,x,y,M;
		rd(a0);rd(x);rd(y);rdk;rd(M);
		int pre,nw;pre=a0;
		int vl=0;
		int gd=1;
		
		up(i,1,k-1)gd=gd/__gcd(gd,i)*i;
//		cout<<gd;edl;
		vl+=a0/gd;
		dp[0][a0%gd]++;
		up(i,1,n-1)
...
//		cout<<gd;edl;
		fc[0]=1;
		up(i,1,k)fc[i]=fc[i-1]*n%mod;
		int ans=0;
		up(i,1,k)
		{
			ans+=vl*gd%mod*fc[k-1]%mod;ans%=mod;
		}
//		cout<<vl;edl;
		...
		up(i,1,k)
		{
			up(j,0,gd-1)
			{
				ans+=j*dp[(i-1)&1][j]%mod*fc[k-i]%mod;ans%=mod;
				if(i!=k)
				{
					dp[i&1][j]+=dp[(i-1)&1][j]*(n-1)%mod;dp[i&1][j]%=mod;
					(dp[i&1][j-j%i]+=dp[(i-1)&1][j])%=mod;
				}
			}
