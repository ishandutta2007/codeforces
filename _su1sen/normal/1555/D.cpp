...
using S = array<array<int, 6>, 6>;

S op(S x, S y) {
    S res;
    rep(i, 6) rep(j, 6) res[i][j] = inf;
    rep(i, 6) rep(j, 6) rep(k, 6) {
        chmin(res[i][k], x[j][k] + y[i][j]);
    }
    return res;
}
S e() {
...
int main() {
    ...
    rep(t, 3) rep(i, 6) rep(j, 6) A[t][i][j] = inf;
    ...
    rep(t, 3) {
        rep(s0, 3) rep(s1, 3) {
            rep(s2, 3) {
                if (s0 == s1 or s1 == s2) continue;
                int c = t != s2;
                if (s0 == s2) c = inf;
                A[t][state[s1][s2]][state[s0][s1]] = c;
            }
        }
    }
    ...
    loop(q) {
        ...
        --l;
        int c = s[l] - 'a';
        auto T = seg.prod(l + 1, r);

        int ans = inf;
        rep(x, 3) rep(y, 3) {
            if (x == y) continue;
            rep(i, 6) {
                chmin(ans, T[i][state[x][y]] + (c != y));
            }
        }
        print(ans);
    }
