...
int main() {
	qread(n, k);
	rep(i, 1, n) rep(j, 1, n) qread(a[i][j]), id[(i - 1) * n + j] = mp(i, j);
	sort(id + 1, id + (n * n) + 1, [&](pii x, pii y) { return a[x.fi][x.se] > a[y.fi][y.se]; });
	dp[id[1].fi][id[1].se] = 1;
	int mx1 = id[1].fi + id[1].se, mn1 = id[1].fi + id[1].se, mx2 = id[1].fi - id[1].se, mn2 = id[1].fi - id[1].se;
	rep(i, 2, n * n) {
		int x = id[i].fi, y = id[i].se;
		if(max(max(abs(x + y - mx1), abs(x + y - mn1)), max(abs(x - y - mx2), abs(x - y - mn2))) <= k) {
			dp[x][y] = 1;
			To_max(mx1, x + y);
			To_min(mn1, x + y);
			To_max(mx2, x - y);
