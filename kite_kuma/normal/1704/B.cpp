...
int solve() {
	...
	segment_tree<pair<ll, ll>> stminmax(
		n, [](pair<ll, ll> a, pair<ll, ll> b) { return make_pair(min(a.first, b.first), max(a.second, b.second)); }, make_pair(INF, -INF));
	
	rep(i,n){
		stminmax.update(i, make_pair(a[i], a[i]));
	}
	...
	while(nxt < n) {
		...
		while(ng - ok > 1) {
			int mid = (ok + ng) / 2;
			auto pr = stminmax.get(nxt, mid);
			if(pr.second - pr.first > x * 2) {
				ng = mid;
			} else
				ok = mid;
		}
