...
data operator+(const data&a,const data&b){
	...
	c.rignum=b.rignum;
	c.len=a.len+b.len;
	c.leflen=a.leflen==a.len && a.rignum>b.lefnum ?a.len+b.leflen:a.leflen;
	c.riglen=b.riglen==b.len && a.rignum<b.lefnum ?a.riglen+b.len:b.riglen;
	c.leflen1=a.leflen1==a.len && a.rignum>b.lefnum?a.len+b.leflen:a.leflen1;
	c.riglen1=b.riglen1==b.len && a.rignum<b.lefnum?b.len+a.riglen:b.riglen1;
	if(a.riglen==a.len && a.rignum<b.lefnum)upmax(c.leflen1,a.len+b.leflen1);
	if(b.leflen==b.len && a.rignum>b.lefnum)upmax(c.riglen1,b.len+a.riglen1);

	c.midlen=max(a.midlen,b.midlen);
	if(a.rignum>b.lefnum){
		upmax(c.midlen,a.riglen1+b.leflen);
	}else if(a.rignum<b.lefnum){
		upmax(c.midlen,a.riglen+b.leflen1);
	}
	return c;
}
void operator+=(data&a,ll b){
	a.lefnum+=b;
	a.rignum+=b;
}
int n;
...
int main()
{
	...
	int _time_jc=clock();
#endif
	one.leflen=one.riglen=one.leflen1=one.riglen1=one.midlen=one.len=1;
	gn(n);
	rep(i,1,n+1)gn(a[i]);
	seginit(n);
	int m;gn(m);
