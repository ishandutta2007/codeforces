...
vi solve(ll N, ll M, vector<vi>& data) {
  vector<vi> BEGIN(N);
  vector<vi> END(N);
  vi ANS(N, -1);
  FOR(i, M) {
  ...
  FOR(i, N) {
    if (END[i].size() > 1) return vi({-1});
  }
  // -, 
  vi PROG(N, 0);
  priority_queue<pi> que;
  FOR(day, N) {
    for (auto& task : BEGIN[day]) {
      que.push(mp(-data[task][1], task));
    }
    if (!END[day].empty()) {
      ll task = END[day][0];
      if (PROG[task] < data[task][2]) return vi({-1});
      ANS[day] = M;
      continue;
    }
    if (que.empty()) continue;
    auto [pri, task] = que.top();
    ++PROG[task];
    ANS[day] = task;
    if (PROG[task] == data[task][2]) {
      que.pop();
    }
  }
  for_each(all(ANS), [](ll& x) { return ++x; });
  return ANS;
}
