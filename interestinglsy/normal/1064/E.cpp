...
int l,r,y;
void Binarysearch( int stx , int edx , int deltay , int cnt ){
	l = stx+10, r = edx-10;
	int mid;
	y = 500000000+deltay;
	while(cnt){
		mid = l+r >> 1;
		if(Ask(mid,y)) l = mid+10;
		else r = mid-10;
		cnt--;
		//y += deltay;
		//deltay = (ld)deltay / 4;
...
int main(){
	...
	For(i,n){
		int ux = (i-1)*30000000;
		int ok = Ask(ux,500000000);
		if(ok) stx = ux;
		else{ edx = ux; n -= i; break; }
	}

	if( edx == -1 ){
		cout << "233 233 666 233" << endl;
		END();
	}

	Binarysearch(stx,edx,300000000,n>>1); n -= (n>>1);
	int ansx = (l+r>>1), ansy = y;
	Binarysearch(stx,edx,-300000000,n);
	cout << ansx << " " << ansy << " ";
	cout << (l+r>>1) << " " << y << endl;
