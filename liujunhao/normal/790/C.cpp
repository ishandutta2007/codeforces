...
//template
#define MAXN 75
int f[MAXN+5][MAXN+5][MAXN+5][2],n,cv,ck,co;
char s[MAXN+10];
void read(){
	Read(n);
	scanf("%s",s+1);
...
void solve(){
	...
	****f=0;
	int i,j,k,ccv,cck,cco,cr;
	for(i=0;i<=cv;i++)
		for(j=0;j<=ck;j++)
			for(k=0;k<=co;k++){
				ccv=cck=cco=cr=0;
				int nxtv=0,nxtk=0,nxto=0;
				for(int x=1;x<=n;x++)
					if(s[x]=='V'){
						if(ccv<i)
							ccv++;
				...
				if(i<cv){
					f[i+1][j][k][1]=min(f[i+1][j][k][1],min(f[i][j][k][0],f[i][j][k][1])+nxtv-1);
				}
				if(j<ck){
					f[i][j+1][k][0]=min(f[i][j+1][k][0],f[i][j][k][0]+nxtk-1);
				}
				if(k<co)
					f[i][j][k+1][0]=min(f[i][j][k+1][0],min(f[i][j][k][0],f[i][j][k][1])+nxto-1);
			}
	cout<<min(f[cv][ck][co][0],f[cv][ck][co][1])<<endl;
}
int main()
{
	read();
