...
inline ll mul(ll x,ll y,ll mod){
	ll t=x*y-(ll)((ld)x*y/mod)*mod;
	return t<0?t+mod:t;
}
inline ll exp(ll x,ll y,ll mod){
	ll ans=1;
	while(y){
...
inline int getlen(ll a){
	int ans=0;
	while(a) a/=10,ans++;
	return ans;
}
inline ll calc(int g,int p,int a,ll x){
//	dprintf("%d %d %d %lld\n",g,p,a,x);
	ll mod=p,phi=p-1,d=0;
...
int Main(int argv){
	...
	for(int m=0;;m++){
		...
		if(!m&&a%5==0) goto power;//continue;
		if(getlen(b)<=m){
