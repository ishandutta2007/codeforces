...
struct xorset{
    ...
    int get(int x)
    {
        return items.find(x ^ val) != items.end();
    }
...
int merge(xorset &a, xorset &b, int through)
{
...
void dfs(int v, int p, vvi &g, vi &a)
{
    ...
    for (auto u : g[v]) if (u != p)
    {
        int res = merge(sets[u], sets[v], a[v]);
        if (res == 1)
        {
            fl = 1;
            break;
