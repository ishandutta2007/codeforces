...
template <class T> struct PSP {
    atcoder::mf_graph<T> flow;
    int s, t, num;
    T sum;
    vector<tuple<int, int, T>> E;
    PSP(int n) : s(n), t(n + 1), sum(0), num(n + 2) {}

    // x  val  p 
    void set(int x, int val, T p) {
        if(val == 0) {
            if(p < 0)
                E.eb(x, t, -p);
            else if(p > 0) {
    ...
    void add(int x, int xval, int y, int yval, T z) {
        if(xval == yval) {
            assert(z >= 0);
            if(xval == 0)
                both_zero(x, y, z);
    ...
    T solve() {
        flow = atcoder::mf_graph<T>(num);
        for(auto &&[x, y, z] : E) { flow.add_edge(x, y, z); }
        return sum - flow.flow(s, t);
    }

  private:
    void zero_one(int x, int y, T z) {
        assert(z < 0);
        E.eb(x, y, -z);
    }
    void both_zero(int x, int y, T z) {
        sum += z;
        E.eb(s, num, z);
        E.eb(num, x, numeric_limits<T>::max());
        E.eb(num, y, numeric_limits<T>::max());
        num++;
    }

    void both_one(int x, int y, T z) {
        sum += z;
        E.eb(num, t, z);
        E.eb(x, num, numeric_limits<T>::max());
        E.eb(y, num, numeric_limits<T>::max());
        num++;
    }
};

template <class T> struct PSP_bipartite : PSP<T> {
    int L, R;
    PSP_bipartite(int n, int m) : L(L), R(R), PSP<T>(L + R) {}
    void set_left(int x, int val, T p) { set(x, val, p); }
    void set_right(int x, int val, T p) { set(x + L, val ^ 1, p); }
};
...
int main() {
    ...
    rep(m) {
        INT(x, y, z);
        x--, y--;
        psp.add(x, true, y, true, z);
    }

    rep(i, n) psp.set(i, 1, -a[i]);
