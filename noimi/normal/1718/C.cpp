...
int main() {
    TEST {
        ...
        vi st(si(d));
        rep(i, si(d)) sum[i].resize(d[i]);
        int p = 0;
        { rep(i, si(d)) st[i] = p, p += d[i]; }
        RMT<ll> seg;
        auto f = [&](int i, ll x, bool flag = true) {
            ll y = x - a[i];
            a[i] = x;
            // dump(i, x, y);
            rep(k, si(d)) {
                int j = i % d[k];
                sum[k][j] += y * d[k];
                if(flag) seg.set(st[k] + j, sum[k][j]);
            }
        };
        rep(i, n) { f(i, in(), false); }
        vl setter(p);
        p = 0;
        rep(i, si(d)) rep(j, d[i]) setter[p++] = sum[i][j];
        seg = RMT<ll>(setter);
