...
struct fenwick_tree{
	...
	inline int revf(int c, int b) {
		return c - b;
	}
	inline void set(int id, int val) {int d = val - query(id,id); if (d != 0) insert(id,d);}
	inline void insert(int id, int val) {while (id < size) {BIT[id] = func(BIT[id],val); id += (id & -id);}}
	inline long long query_from1(int id) {long long ans = 0; while (id > 0) {ans = func(ans,BIT[id]); id -= (id & -id);} return ans;}
    inline long long query(int l, int r) {if (l > r) return 0; return revf(query_from1(r),query_from1(l-1));}
	inline fenwick_tree(int N) {BIT.resize(N+1,0); size = N+1;}
	inline fenwick_tree(vector<int>& v) {BIT.resize(v.size()+1,0); size = v.size()+1; for (int i = 1; i <= v.size(); i++) BIT[i] = v[i-1]; for (int i = 1; i <= v.size(); i++) {int idx = i + (i & -i); if (idx <= v.size()) BIT[idx] = func(BIT[idx],BIT[i]);}}
};

int main() {
	int n,m; R(n,m);
	
	vector<int> upd(m); R(upd);
	vector<pii> ans(n + 1);
	for (int i = 1; i <= n; i++)
		ans[i] = {i, i};
	...
	for (int i = 0; i < m; i++) {
		...
		s.insert(now);
		if (last[now] != -1) {
			bit.insert(last[now] + 1, -1);
			
			int idx = 1 + bit.query(last[now] + 1, i);
			//W("idx", now, idx);
			chkmax(ans[now].S, idx);
		} else {
			int rank = (int)s.size() - (int)s.order_of_key(now) - 1 + now;
			//W(now, rank);
			chkmax(ans[now].S, rank);
		}
	...
	for (int i = 1; i <= n; i++)
		if (last[i] == -1) {
			int rank = (int)s.size() - (int)s.order_of_key(i) + i;
			//W(i, rank);
			chkmax(ans[i].S, rank);
		} else {
			int idx = bit.query(last[i] + 1, m);
			//W("idx", i, idx);
			chkmax(ans[i].S, idx);
		}
