...
int main() {
    const int N = 1000000;
    // int N = 10;
    auto euler = euler_phi_table(N);
    vmint G(N + 1);
    rep2(i, 1, N) {
        for(int j = 0; j <= N; j += i) { G[j] += mint(i) * euler[j / i]; }
    }
    rep2(i, 1, N) { G[i] = mint(i) * (i + 1) * (2 * i + 1) / 6 + mint(i - 1) * i * (2 * i - 1) / 6 - mint(i) * 2 * i + 2 * G[i]; }
    vmint a(N + 1), b(N + 1), c(N + 1);
    rep2(i, 1, N) {
        a[i] = a[i - 1] + G[i];
        b[i] = b[i - 1] + G[i] * i;
        c[i] = c[i - 1] + G[i] * i * i;
