...
void solve(){
    int n;
    cin >> n;
    vector<array<int, 2>> cur(n);
    cin >> cur;

    int z = 0;
    vector<array<int, 2>> values;
    {
    	 gp_hash_table<int, int, custom_hash> cntr;
	    for(const auto &e: cur){
	    	z = max(z, e[1]);
	    	cntr[e[0]] += e[1];
	    }
	    for(auto &e: cntr){
	    	values.push_back({e.second, e.first});
	    }
    ...
    fraction values_sum = 0;
    for(int t = 1; t <= values.size(); ++t){
		values_sum += values[t - 1][0];	
    	if (t <= z){
    		vector<int> tot;
    	 	gp_hash_table<int, int, custom_hash> cntr;
    		for(auto &e: cur){
    ...
    gp_hash_table<int, int, custom_hash> cntr;
    for(auto &e: cur){
    	cntr[e[0]] += min(e[1], best_t);
    }
    vector<array<int, 2>> tot;
    for(auto &e: cntr){
    	tot.push_back({e.second, e.first});
    }
    sort(all(tot)); reverse(all(tot));
    cout << best_t << endl;
    for(int z = 0; z < best_t; ++z){
    	cout << tot[z][1] << " ";
    }
    cout << endl;
