...
bool dfs(int i) {
    if (cost[i]>k) return false;
    if (l[i]>-1) {
        cost[l[i]] = cost[i]+1;
        if (dfs(l[i])) choose[i]=true, cost[i]=0;
    }
    if (cost[i]<=k && good[i]) choose[i]=true, k -= cost[i], cost[i] = 0;
    if (choose[i] && r[i]>-1) {
        cost[r[i]] = 1;
        dfs(r[i]);
    }
...
void solve() {
    ...
    good.rsz(n);
    R0F(i, n) {
        if (ans[i]<last) good[ord[i]]=true;
        else good[ord[i]]=false;
        if (i>0 && ans[i-1] != ans[i]) last = ans[i];
    }
    cost.assign(n, 0); cost[0]=1;
    dfs(0);
    ord.clear(); ans = "";
    make(0);
    cout << ans << nl;
