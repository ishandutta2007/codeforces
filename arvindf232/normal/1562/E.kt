...
//        for(i in 0 until n){
//            for(j in 0 until i){
//                "$i $j" dei suf.calculateLCPquick(i,j)
//            }
//        }
        ...
        for(i in 0 until n){
            var atleast = ( n - i)
            for(j in 0 until i){
                if(suf.pos[j] > suf.pos[i]) continue
                val costed = suf.calculateLCPquick(j,i)
                atleast = maxOf(atleast,DP[j] + (n-i) - costed)
            }
            DP[i] = atleast
        }
        put(DP.maxOrNull()!!)
