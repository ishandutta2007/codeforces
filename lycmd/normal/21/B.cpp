...
int main(){
	...
	cin>>a>>b>>c;
	flg1|=equ(a,0.0)&&equ(b,0.0)&&equ(c,0.0);
	flg2|=equ(a,0.0)&&equ(b,0.0)&&!equ(c,0.0);
	if(equ(a,0.0))n=makeseg(point(0,-c/b),point(1,-c/b));
	else if(equ(b,0.0))n=makeseg(point(-c/a,0),point(-c/a,1));
	else n=makeseg(point(0,-c/b),point(1,-(a+c)/b));
	cin>>a>>b>>c;
	flg1|=equ(a,0.0)&&equ(b,0.0)&&equ(c,0.0);
	flg2|=equ(a,0.0)&&equ(b,0.0)&&!equ(c,0.0);
	if(equ(a,0.0))m=makeseg(point(0,-c/b),point(1,-c/b));
	else if(equ(b,0.0))m=makeseg(point(-c/a,0),point(-c/a,1));
	else m=makeseg(point(0,-c/b),point(1,-(a+c)/b));
	if(flg2)cout<<"0\n";
	else if(flg1)cout<<"-1\n";
	else if(!islinepara(n,m))cout<<"1\n";
	else if(equ(cross(n.v,m.p-n.p),0))cout<<"-1\n";
	else cout<<"0\n"; 
}