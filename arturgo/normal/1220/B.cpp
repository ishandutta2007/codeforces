...
int main() {
	...
	auto nombres = read<vector<vector<long long>>>(taille, taille);

	for(int iNombre = 0;iNombre < taille;iNombre++) {
		writesp((int)sqrt(nombres[iNombre][(iNombre + 1) % taille] * nombres[iNombre][(iNombre + 2) % taille] / nombres[(iNombre + 1) % taille][(iNombre + 2) % taille]));
	}
	writeln();
	return 0;
}