...
void solve() {
  ...
  set<li> B(begin(A), end(A));
  vi C(begin(B), end(B));
  map<li,li> D; for(li i = 0; i < (li)(C.size()); ++i) D[C[i]] = i;
  int m = D.size();
  for(li i = 0; i < (li)(n); ++i) A[i] = D[A[i]];
  vi fi(m) ,la(m);
  for(li i = 0; i < (li)(m); ++i) { fi[i] = n; la[i] = 0; }
  for(li i = 0; i < (li)(n); ++i) la[A[i]] = i;
  for(li i = (n-1); i >= (li)(0); --i) fi[A[i]] = i;
  int ans = 0;
  int cnt = 0;
