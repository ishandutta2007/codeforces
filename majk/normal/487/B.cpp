...
public:
    void solve(istream& cin, ostream& cout) {
		...
		int Q = logceil(N);
		vector2<int> MIN(Q+1, N), MAX(Q+1, N);
		for (int i = 0; i < N; ++i) {
			MIN[0][i] = MAX[0][i] = A[i];
		}
		for (int j = 0; j < Q; ++j) {
			for (int i = 0; i < N; ++i) {
				if (i + (1<<j) < N) {
					MIN[j+1][i] = min(MIN[j][i], MIN[j][i + (1<<j)]);
					MAX[j+1][i] = max(MAX[j][i], MAX[j][i + (1<<j)]);
				} else {
					MIN[j+1][i] = MIN[j][i];
					MAX[j+1][i] = MAX[j][i];
				}
		...
		auto minr = [&](int f, int t) { if (t==f+1) return A[f]; else { int q = logceil(t-f)-1; return min(MIN[q][f], MIN[q][t-(1<<q)]); } };
		auto maxr = [&](int f, int t) { if (t==f+1) return A[f]; else { int q = logceil(t-f)-1; return max(MAX[q][f], MAX[q][t-(1<<q)]); } };

		minheap<pii> H; H.push({0, 0});
		minheap<pair<int, pii>> G;
		for (int i = 0; i < N; ++i) {
			if (!H.empty()) {
				int maxEnd = bsh(i+L, N, [&](int w) { return maxr(i, w) - minr(i, w) <= S;});
				if (maxEnd != -1) {
					G.push({i + L, {H.top().x+1, maxEnd}});
				}
			}
			while (!H.empty() && H.top().y == i) { H.pop(); }
			while (!G.empty() && G.top().x == i+1) {
				H.push(G.top().y);
				G.pop();
			}
		}
