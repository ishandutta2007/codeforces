...
void print_is(vi &ans) {
    ps(1);
    pc(ans);
    exit(0);
}

void print_cycle(vi &ans) {
    ps(2);
    ps(sz(ans));
    pc(ans);
    exit(0);
...
int main() {
    ...
    auto dfs = [&](auto &&self, int cur) -> void {
        for (int next : adj[cur]) {
            if (depth[next] == -1 || next == parent[cur])
                continue;
            vi ans;
            for (int v = cur; v != next; v = parent[v]) {
                ans.push_back(v + 1);
            }
        ...
        if (depth[cur] + 1 == k) {
            vi ans;
            for (int i = 0, v = cur; v >= 0; v = parent[v], i++) {
                if (i % 2 == 0)
                    ans.push_back(v + 1);
            }
            print_is(ans);
    ...
    array<vi, 2> sets;
    for (int i = 0; i < n; i++) {
        sets[depth[i] % 2].push_back(i + 1);
    }
    vi ans = sets[sz(sets[1]) > sz(sets[0])];
    ans.resize((k + 1) / 2);
