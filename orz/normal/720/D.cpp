...
struct Tree1 {
	...
	void add(int v, int tl, int tr, int pos, int val) {
		if (tl == tr) {
			...
			push(v);
			if (pos <= tm) {
				add(v + v, tl, tm, pos, val);
			}
			else {
				add(v + v + 1, tm + 1, tr, pos, val);
			}
			pop(v);
		}
	}
	...
	void make_zero(int v, int tl, int tr, int l, int r) {
		...
		if (tl >= l && tr <= r) {
			...
			push(v);
			int tm = (tl + tr) >> 1;
			make_zero(v + v, tl, tm, l, r);
			make_zero(v + v + 1, tm + 1, tr, l, r);
			pop(v);
		}
	}
	...
	int sum(int v, int tl, int tr, int l, int r) {
		...
		push(v);
		int tm = (tl + tr) >> 1;
		int ans = (sum(v + v, tl, tm, l, r) + sum(v + v + 1, tm + 1, tr, l, r)) % MOD;
		pop(v);
		return ans;
	}
...
struct Tree2 {
	...
	int nearest_one(int x) {
		auto it = segs.lower_bound(make_pair(x, -1));
		if (it != segs.begin()) {
			auto it2 = it;
			it2--;
			if (it2->first <= x && it2->second >= x) {
				return x;
			}
		}
		if (it != segs.end()) {
...
int main()
{
	...
	int n, m, k;
	cin >> n >> m >> k;
	vector<pair<pii, pii>> events; // ((-y, type), (-x, x0)))
	for (int i = 0; i < k; ++i)
	{
		int x1, x2, y1, y2;
		cin >> x1 >> y1 >> x2 >> y2;
		if ((x1 >= 2) && (y2 < m))
			events.eb(pii(-(y2 + 1), 0), pii(-(x1 - 1), -100));
		events.eb(pii(-y2, -1), pii(-x1, -x2));
		events.eb(pii(-y1, 1), pii(-x1, -x2));
	}
	...
	for (int i = 0; i < sz(events); ++i)
	{
		...
		if (type == -1)
		{
			t2.fill(-ev.Y.X, -ev.Y.Y, 1);
			t1.make_zero(-ev.Y.X, -ev.Y.Y);
		}
		else if (type == 0)
		{
