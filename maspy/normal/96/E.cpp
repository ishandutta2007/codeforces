...
void solve() {
  ...
  */
  ...
  auto calc = [&](string S, bool inclusive) -> mint {
    auto A = s_to_vi(S, '0');
    ll N = A.size();
    mint ANS = 0;
    // 
    ...
    FOR(n, N) {
      FOR(x, A[n]) {
        bool luck = (x == 4 || x == 7);
        if (k < K && luck) continue;
        ll kk = (luck ? 0 : k + 1) + 1;
        FOR(d, K + 1) if (d + kk > K) { ANS += DP[N - n - 1][d]; }
      }
      bool luck = A[n] == 4 || A[n] == 7;
      if (k < K && luck) {
        k = -1;
        break;
      }
    ...
    if (inclusive && k != -1) ANS += 1;

    mint total = 0;
    FORIN(x, A) total = total * 10 + x;
    total += 1;
