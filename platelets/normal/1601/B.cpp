...
int n, a[N], b[N], fa[N], pre[N], d[N], q[N];
...
int main() {
    ...
    while(l <= r) {
        int u = q[l++], nu = u + b[u];
        for(int v = nu - a[nu]; (v = find(v)) <= nu; v = fa[v] = v + 1)
        if(v < n && !d[v]) d[v] = d[u] + 1, pre[v] = u, q[++r] = v;
    }
    if(!d[0]) puts("-1"), exit(0);
    cout << d[0] << endl;
    vector<int> as;
    for(int u = 0; u < n; u = pre[u]) as.pb(u);
    reverse(all(as));
    for(int u : as) printf("%d ", u);
}