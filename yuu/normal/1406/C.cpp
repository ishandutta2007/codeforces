...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        centroid[i] = n - f[i];
        for (auto&& v : g[i]) centroid[i] = max(centroid[i], f[v]);
    }
    vector<int> min_centroids;
    for (int i = 1; i <= n; i++) {
        if ((!min_centroids.empty()) && (centroid[i] < centroid[min_centroids.back()])) min_centroids.clear();
        if (min_centroids.empty() || (centroid[i] == centroid[min_centroids.back()])) min_centroids.push_back(i);
    }

    if (min_centroids.size() == 1) {
        cout << 1 << ' ' << g[1][0] << '\n';
        cout << 1 << ' ' << g[1][0] << '\n';
        return;
    }
    // 2 centroids
    assert(min_centroids.size() == 2);
    int u = min_centroids[0];
    int v = min_centroids[1];
