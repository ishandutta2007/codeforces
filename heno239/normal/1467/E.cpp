...
Data dfs(int id, int fr) {
	...
	//
	//initialize
	vector<int>& v = mp[a[id]];
	int num = lower_bound(all(v), ris[trans[id]]) - upper_bound(all(v), trans[id]);
	if (num > 0)res = false;
	//
	for (edge e : G[id]) {
		if (e.to == fr)continue;
...
void invdfs(int id, int fr, Data pre) {
	...
	//calcurate id's ans
	bool b = true;
	vector<int>& vv = mp[a[id]];
	if (vv.size() > 1)b = false;
	//cout << "?? " << ris[trans[id]] << " " << trans[id] << "\n";
	//cout << "? " << id << " " << num << "\n";
	for (Data d : v)b = b & d;
	if (b) {
		ans++; //cout << id + 1 <<" "<<num<< "\n";
	...
	rep(i, ids[id].size()) {
		...
		//update for return
		//cout << "wow " << to << "\n";
		int num = lower_bound(all(vv), ris[trans[to]]) - lower_bound(all(vv), trans[to]);
		num = vv.size() - num;
		if (num != 1)d = false;
		//
		invdfs(to, id, d);
...
void solve() {
	...
	int tmp = 0;
	function<void(int, int)> dfs3 = [&](int id, int fr) {
		trans[id] = tmp; tmp++;
