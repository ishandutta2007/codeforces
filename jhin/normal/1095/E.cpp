...
cin>>n>>s;
for(int j=0;j<n;j++) {if(m<-2)break;if(s[j]=='(')m++;else m--;}
for(int j=n-1;j>=0;j--) {if(z>2)break;if(s[j]=='(')z++;else z--;}
//cout<<m<<" "<<z<<endl;
if(m==-2)
{
   m=0; for(int j=0;j<n;j++) {if(s[j]=='(')m++;else x++,m--;if(m==-1)break;}
}
else if(z==2)
{
    m=0;for(int j=n-1;j>=0;j--) {if(s[j]=='(')x++,m++;else m--;if(m==1)break;}
}
