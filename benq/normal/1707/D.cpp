...
vmi dfs(int x, int p) {
	...
	vmi csum(sz(all_consider)), ret(N+1);
	V<vmi> greater_than(sz(all_consider), vmi(N+1));
	F0R(del,N+1) {
		mi prod = 1;
		F0R(i,sz(all_consider)) {
			csum[i] += all_consider[i][del];
			prod *= csum[i];
		}
		vmi prefix_prod(sz(all_consider)), suffix_prod(sz(all_consider));
		{
			mi cprod = 1;
			F0R(i,sz(all_consider)) {
				prefix_prod[i] = cprod;
		...
		ret[del] += prod;
		F0R(i,sz(all_consider)) {
			// dbg("HUH", del, i, prefix_prod[i], suffix_prod[i]);
			greater_than[i][del] += prefix_prod[i] * suffix_prod[i];
		}
	}
	// dbg("PREV", x, ret, greater_than);
	if (x != 1) F0R(i,sz(all_consider)) {
		mi csum = 0;
		F0R(j,N+1) {
			ret[j] += csum * all_consider[i][j];
			csum += greater_than[i][j];
		}
	}
	// dbg("HA", x, ret);
	return ret;
...
int main() {
	...
	genComb(N+1);
	F0R(i,N-1) {
		// only j+1 real things
		// only i+1 real things
		// i-j
		F0R(j,i) v[i] -= scmb.at(i+1).at(j+1) * v[j];
		pr(v[i],' ');
	}
