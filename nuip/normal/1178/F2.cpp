...
unordered_map<int,mint> memo[1123456];
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cin>>n>>m;
	vector<int> cs(m);
	rep(i,m) cin>>cs[i], --cs[i];
	vv<int> inds(n);
	rep(i,m) inds[cs[i]].pb(i);
	vector<int> lbs,rbs;
	rep(i,n){
		int x=inds[i][0];
		int ok=1;
		reps(j,i+1,n)if(inds[j][0]<=x && x<=inds[j].back()){ ok=0; break;}
		if(ok) lbs.pb(x);
	}
	rep(i,n){
		int x=inds[i].back();
	...
	function<mint(int,int)> dfs=
		[&](int l,int r){
			...
			if(memo[l].count(r)) return memo[l][r];
			int mni;
			rep(i,n){
				auto it=lower_bound(all(inds[i]),l);
				if(it==inds[i].end() || *it>=r) continue;
				mni=i; break;
			}
			out(l,r,mni,1);
			auto &v=inds[mni];
			int L=inds[mni][0],R=inds[mni].back();
			if(L<l || r<=R) return mint(0);
			mint ls=0,rs=0,mp=1;
			reps(i,1,v.size())if(v[i-1]+1<v[i]) mp*=dfs(v[i-1]+1,v[i]);
			for(auto x:lbs)if(l<=x && x<=L) ls+=dfs(l,x)*dfs(x,L);
			for(auto x:rbs)if(R<=x && x<r) rs+=dfs(R+1,x+1)*dfs(x+1,r);
			out(l,r,L,R,ls,rs,mp,1);
			return memo[l][r]=ls*mp*rs;
		};
	cout<<dfs(0,m)<<endl;
	out(memo,1);
  return 0;
