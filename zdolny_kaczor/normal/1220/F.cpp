...
vpii left, right;
int add(vpii &t, int v) {
	int ret = 0;
	while (!t.empty() && t.back().first > v) {
		maxi(ret, t.back().second);
		ret++;
		t.pop_back();
	}
	t.emplace_back(v, ret);
	return ret + t.size();
}
...
int main() {
	...
	REP(i, n) scanf("%d", a + i);
	int p1 = -1;
	REP(i, n) if (a[i] == 1) p1 = i;
	int si1 = 0;
	REP(i, n) {
		int p = (p1 + i) % n;
		maxi(si1, add(left, a[p]));
		debug(imie(left), imie(si1), arr(a, p));
		lsi[i] = si1;
	}
	int si2 = 0;
	REP(i, n) {
		int p = (p1 + n - i) % n;
		maxi(si2, add(right, a[p]));
		debug(imie(right), imie(si2), arr(a, p));
		rsi[i] = si2;
	}
	pii ans = {nax, nax};
	REP(i, n) {
		int dep = max(lsi[n - i - 1], rsi[i]);
		int shi = (p1 + n - i) % n;
		debug(arr(rsi, imie(i)), arr(lsi, imie(n) - imie(i) - 1), imie(shi));
		mini(ans, pii(dep, shi));
	}
	printf("%d %d\n", FS(ans));
}