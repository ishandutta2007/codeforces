...
fun main(){
    solve.cases{
        val n = getint
        val L = getline(n)
        val things = mutableMapOf<Int,Int>()
        for(a in L){
            things[a] = things.getOrDefault(a,0) + 1
        }
        if(things.values.any{it %2 != 0}){
            put(-1)
            return@cases
        }
        ...
        while(LL.isNotEmpty()){
            val here = LL[0]
            val next = (1..LL.lastIndex).first { LL[it] == here }
            for(i in 1 until next){
                ret.add(Pair(LL[i],shifts + i + next))
            }
            val new2 = LL.slice(1 until next).reversed()
            val new3 = LL.slice(next +1 until LL.size)
            LL = new2 + new3
            shifts += 2 * next
            total.add(2 * next)
