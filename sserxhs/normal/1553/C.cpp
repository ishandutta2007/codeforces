...
const int N=12,M=1e6+2,inf=1e9;
char s[N];
int a[N],b[N];
...
int main()
{
	...
	{
		cin>>s+1;n=10;
		for (i=1;i<=n;i++) if (s[i]=='1') a[i]=1; else if (s[i]=='?') a[i]=-1; else a[i]=0;
		memcpy(b,a,sizeof a);
		for (i=1;i<=n;i+=2) if (a[i]==-1) a[i]=1;
		for (i=2;i<=n;i+=2) if (a[i]==-1) a[i]=0;
		for (i=1;i<=n;i+=2) if (b[i]==-1) b[i]=0;
		for (i=2;i<=n;i+=2) if (b[i]==-1) b[i]=1;
		x=0;ans=10;
		for (i=1,j=5;i<=n;i++)
		{
			if (i&1) x+=a[i]; else x-=a[i];
			if (i%2==0) --j;
			if (x>j) {ans=min(ans,i);break;}
		}
		x=0;
		for (i=1,j=5;i<=n;i++)
		{
			if (1^i&1) x+=b[i]; else x-=b[i];
			if (i&1) --j;
			if (x>j) {ans=min(ans,i);break;}
		}
		enter(ans);
