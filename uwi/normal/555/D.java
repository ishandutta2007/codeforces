	...
	void solve()
	{
		...
		for(;Q >= 1;Q--){
			...
			if(n == 1){
				...
				int prevunderind = -5;
				long prevunderlen = -5;
				int id = Arrays.binarySearch(xs, x[wh]);
				while(true){
					long wlen = len;
					// under
					assert len >= 0;
					{
						int ind = Arrays.binarySearch(xs, xs[id]+len);
						if(ind < 0){
							ind = -ind-2;
						}
						if(id == prevunderid && ind == prevunderind && len >= xs[ind]-xs[id]){
							long sh = prevunderlen - len;
							len -= ((long)len-(xs[ind]-xs[id]))/sh*sh;
						}
						prevunderid = id;
						prevunderind = ind;
						prevunderlen = len;
