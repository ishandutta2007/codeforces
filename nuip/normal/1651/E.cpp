...
int main(){
	...
	vv<int> vss(n+n);
	{
		...
		for(cauto &[a,b]:es){
			...
			if(vs.size()) continue;
			vs.eb(a); vs.eb(b);
			while(vs.back()!=a) vs.eb(xs[vs.back()]^vs[vs.size()-2]);
			cauto n=vs.size();
			reps(i,1,n) vs.eb(vs[i]);
		}
	}
	out(vss,1);
	ll re=0;
	for(cauto &vs:vss){
		...
		cauto m=uf.size(vs[0]);
		{
			int l=n,r=0,L=n+n,R=n;
			for(cauto &v:vs){
				MN((v<n?l:L),v);
				MX((v<n?r:R),v);
			}
			re+=m/2*(l+1ll)*(n-r)*(L-n+1)*(n+n-R);
			out(vs,l,r,L,R,m/2,(l+1ll)*(n-r)*(L-n+1)*(n+n-R),1);
		}
		reps(o,1,m+1){
			int l=n,r=0,L=n+n,R=n;
			rep(i,m-1){
				...
				MN((v<n?l:L),v);
				MX((v<n?r:R),v);
				int lw=-1,rw=n,Lw=n-1,Rw=n+n;
				for(cauto &v:{vs[o-1],vs[o+i+1]}){
					if(v<n){
						if(l<=v) MN(rw,v);
						if(v<=r) MX(lw,v);
					}else{
				...
				cauto c=L-Lw;
				cauto d=Rw-R;
				out(o,i,l,r,L,R,a,b,c,d,1);
				if(a>0 && b>0 && c>0 && d>0){
					re+=(i+1ll)/2*a*b*c*d;
					out((i+1ll)/2,a*b*c*d,o,o+i,1);
				}
			}
		}
	}
