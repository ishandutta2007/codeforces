...
void dfs(int cur, int par = -1) {
    ...
    for (int to : v[cur]) {
        if (to != par) {
            ...
            remove(codes[cur]);
            remove(codes[to]);
            trees[to].erase(codes[cur]);
            trees[cur].insert(toc);
            codes[cur] = curc;
            codes[to] = toc;
            add(curc);
            add(toc);
