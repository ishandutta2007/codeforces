...
ll find_ans(const vvi& g, const vvi& h, const int r = 0)
{
	const int C = INT_MAX;
	int n = sz(g);
	vvi s(n, vi(n, C));
...
vi comp(const vi& a, const int n, const int u = -1)
{
	vb v(n, true);
	for (int b : a)
		v[b] = false;
...
int another_main()
{
	...
	else
	{
		pii d(1e9, 0);
		for (int i = 0; i < n; ++i)
			d = min(d, pii(sz(g[i]), i));
		dsu ds(n);
		int min_vertex = d.Y;
		for (int u : comp(g[min_vertex], n, min_vertex))
			ds.unite(min_vertex, u);
		for (int i = 0; i < n; ++i)
			if (ds.find_set(min_vertex) != ds.find_set(i))
				for (int u : comp(g[i], n, i))
					ds.unite(u, i);
		unordered_map<int, int> fs;
		for (int i = 0; i < n; ++i)
			if (!fs.count(ds.find_set(i)))
				fs[ds.find_set(i)] = sz(fs);
		vvi gg(sz(fs)), hh(sz(fs));
		for (int i = 0; i < n; ++i)
		{
			int ii = fs[ds.find_set(i)];
			for (int j = 0; j < sz(g[i]); ++j)
			{
				int k = g[i][j];
				int kk = fs[ds.find_set(k)];
				if (ii < kk)
				{
					gg[ii].push_back(kk);
					gg[kk].push_back(ii);
					hh[ii].push_back(h[i][j]);
					hh[kk].push_back(h[i][j]);
				}
			}
		}
		cout << find_ans(gg, hh);
