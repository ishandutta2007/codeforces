...
const ll mx = 20;
vector<ll> facs[200005];
ll sp2[200005];
map<vector<ll>, ll> last;
ll dp[200005][23];
set<ll> dup;
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		...
		for (pair<ll, ll> x: cnt) {
			if (x.s % 2 == 1) {
				facs[i].push_back(x.f);
			}
		}
				
		if (last.find(facs[i]) != last.end()) {
			dup.insert(last[facs[i]]);
		}
		...
		for (ll j = 0; j <= k; j++) {
			...
			for (ll l = j; l <= k; l++) {
				ll v = 0;
				if (pos > 0) v = dp[pos - 1][l - j];
				dp[i][l] = min(dp[i][l], v + 1);
			}
		}
