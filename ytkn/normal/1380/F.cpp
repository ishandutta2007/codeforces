...
V operator*(M m, V v){
    return V{
        m.a00*v.a0+m.a01*v.a1, 
        m.a10*v.a0+m.a11*v.a1, 
    };
}
...
M make_mat(int a, int b){
    return M{mint(f1(a)), mint(f2(10*a+b)), mint(1), mint(0)};
}
...
int main(){
    ...
    for(int i = 0; i < m; i++){
        ...
        a[x] = d;
        if(x != n-1){
            seg.set(x, make_mat(a[x+1], a[x]));
        }
        if(x-1 >= 0){
            seg.set(x-1, make_mat(a[x], a[x-1]));
        }
        M mat = seg.all_prod();
        mint a0 = 1;
        mint a1 = f1(a[0]);
