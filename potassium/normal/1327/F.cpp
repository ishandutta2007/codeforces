...
int main() {
	int n,k,m; R(n,k,m);
	vector<pair<pii, int>> seg(m); R(seg);

	Mint ans(1, MOD998);
	const int K = 1 << k;
	for (int b = 1; b < K; b <<= 1) {
		vector<int> seg1(n + 2, 0);
		vector<int> lb(n + 2, 0);

		for (auto& i : seg) {
			if (i.second & b) {
				seg1[i.first.first]++;
				seg1[i.first.second + 1]--;
			}
			else {
				chkmax(lb[i.first.second + 1], i.first.first);
			}
		}
		...
		for (int i = 1; i <= n + 1; i++) {
			dp[i] = (seg1[i] ? 0 : get(lb[i], i - 1)) % MOD998;
			ps[i] = (dp[i] + ps[i - 1]) % MOD998;
		}
