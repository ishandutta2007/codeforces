...
vc<ll>& primetable(int LIM) {
  ...
  if(done >= LIM){
    while(len(primes) && primes.back() >= LIM) primes.pop_back();
    done = LIM;
    return primes;
  }
...
void solve() {
  ...
  for (auto&& p: primes) FOR(i, N / p + 1) pf[p * i] = p;

  auto get_pf = [&](ll n) -> vc<pi> {
    vc<pi> res;
    while (n > 1) {
      ll p = pf[n];
      ll k = 0;
  ...
  FOR(i, N - 1) {
    ...
    c /= g, d /= g;
    // b = a * x 
    mint x = mint(d) / mint(c);
    G.add(a, b, x);
    for (auto&& [p, e]: get_pf(c)) { query[p].eb(a, b, e); }
    for (auto&& [p, e]: get_pf(d)) { query[p].eb(b, a, e); }
  }
  G.build();
  ...
  vi dat(N);
  const ll INF = 1LL << 60;
  LazyTreeMonoid<decltype(hld), Lazy_Min_Add<ll, INF>, 0> TM(hld, dat);
  FOR(p, N + 1) if (len(query[p])) {
    for (auto&& [a, b, c]: query[p]) {
      if (par[a] == b) {
        TM.apply_subtree(a, c);
      } else {
        TM.apply_subtree(b, -c);
      }
    }
    ll k = TM.prod_path(0, 0) - TM.prod_subtree(0);
    for (auto&& [a, b, c]: query[p]) {
      if (par[a] == b) {
        TM.apply_subtree(a, -c);
      } else {
