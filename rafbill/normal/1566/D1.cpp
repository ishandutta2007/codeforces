...
bool cmp(pii const& a, pii const& b) {
  li ia,ja; tie(ia,ja) = a;
  li ib,jb; tie(ib,jb) = b;
  if(ia==ib) return ja > jb;
  else return ia<ib;
}
...
li solve(){
  li n,m; cin>>n>>m;
  vi A(n*m);
  for(li i = 0; i < (li)(n*m); ++i) cin>>A[i];

  vii S(n*m);

  map<li,vi> at;
  for(li i = 0; i < (li)(n*m); ++i) at[A[i]].emplace_back(i);

  li curi = 0, curj = 0;
  for(auto const& pv : at) {
    vi v = pv.second;
    vii s;
    for(li k = 0; k < (li)(v.size()); ++k) {
      s.emplace_back(curi,curj);
      curj++;
      if(curj == m) {
        curj = 0;
        curi++;
    ...
    for(li i = 0; i < (li)(v.size()); ++i) S[v[i]] = s[i];
  }

  li ans = 0;
  vector<BIT<li>> BITs(n, BIT(m+1));
