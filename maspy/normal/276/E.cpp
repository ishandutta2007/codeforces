...
void solve() {
  ...
  ll root_val = 0;
  FenwickTree<ll> bit1(N + 10);
  FenwickTree<ll> bit2(N + 10);

  auto add = [&](ll v, ll x, ll d) -> void {
    if (d >= dep[v]) root_val += x;
    if (v == 0) {
      bit2.add(0, x);
      bit2.add(d + 1, -x);
      return;
    }
    ll ROOT = ET.LA(v, dep[v] - 1);
    // 
    ll lid = LID[ROOT], rid = RID[ROOT];
    ll left = max(lid, LID[v] - d);
    ll right = min(rid, LID[v] + d + 1);
    bit1.add(left, x);
    bit1.add(right, -x);
  ...
  auto calc = [&](ll v) -> ll {
    if (v == 0) return root_val;
    ll x = bit1.sum(LID[v] + 1);
    ll y = bit2.sum(dep[v] + 1);
    return x + y;
  };
