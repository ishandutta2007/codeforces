...
int main() {
    ...
    for (int i : reversed(tr.preorder)) {
        ...
        int max_child = 0;
        tr.for_each_child(i, [&](auto e) {
            auto child = e.get_nbr(i);
            ckmax(max_child, h[child]);
        });
    ...
    vector<int> children{};
    tr.for_each_child(root, [&](auto e) {
        auto child = e.get_nbr(root);
        children.push_back(h[child]);
    });
    sort(rall(children));
    while (sz(children) < 2)
        children.push_back(0);

    cost += h[root] - children[0];
    cost += h[root] - children[1];
