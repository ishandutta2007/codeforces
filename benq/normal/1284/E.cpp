...
ll comb(ll a) { return a*(a-1)*(a-2)/6; }
 
int main() {
    ios_base::sync_with_stdio(0); cin.tie(0);
    re(n); vP v(n); re(v);
    ll res = (ll)n*(n-1)*(n-2)*(n-3)*(n-4)/24;
    F0R(i,n) {
        vd lef;
        F0R(j,n) if (i != j) {
            lef.pb(angle(v[j]-v[i]));
            if (lef.bk < 0) lef.bk += 2*PI;
        }
        lef.rsz(2*sz(lef));
        F0R(i,sz(lef)/2) lef[i+sz(lef)/2] = lef[i]+2*PI;
        sort(all(lef));
        F0R(i,sz(lef)-1) assert(lef[i] <= lef[i+1]);
        int r = -1;
        F0R(l,sz(lef)/2) {
            while (r-l+1 < sz(lef)/2 && lef[r+1]-lef[l] <= PI) r ++;
            int R = lb(all(lef),lef[l]+PI)-begin(lef)-1;
            // assert(r == R);
            // ps("??",l,r);
            res -= comb(r-l);
        }
