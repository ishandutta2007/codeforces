...
void solve() {
  ...
  FOR_(K) {
    ...
    p = R - L;
    SWAG<Mono> swag0, swag1;
    vi newDP0(N + N + 1, INF), newDP1(N + N + 1, INF);
    FOR(n, N + N + 1) {
      swag0.push(DP0[n]);
      swag1.push(DP1[n]);
      chmin(newDP0[n], swag1.prod() + 1);
      chmin(newDP1[n], swag0.prod() + 1);
      if (len(swag0) > p) {
        swag0.pop();
        swag1.pop();
      }
    }
    FOR_R(n, N + N + 1 - p) { chmin(newDP0[n + p], DP0[n]); }
    FOR(n, N + N + 1) chmin(newDP1[n], DP1[n]);
    swap(DP0, newDP0);
    swap(DP1, newDP1);
  }
  ll p = N + N - prev_R;
