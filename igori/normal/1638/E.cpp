...
void solve()
{
    int n, q;
    cin >> n >> q;
    vector<ll> applied(n + 1);
    vector<ll> fenw(n + 1);
    set<array<ll, 3> > S; // left, color, init
    S.insert({0, 1, 0});
    S.insert({1, 1, 0});
    S.insert({n + 1, 1, 0});
    for (int i = 1; i <= q; i++)
    {
        ...
        if (opName[0] == 'C')
        {
            int l, r, c;
            cin >> l >> r >> c;
            auto it = S.lower_bound({l, INF, 0});
            it--;
            ll sv1 = (*it)[1], sv2 = (*it)[2];
            while (1)
            {
                auto it2 = it;
                it2++;
                if ((*it)[0] > r) break;
                int L = max((ll)l, (*it)[0]);
                int R = min((ll)r, (*it2)[0] - 1);
                add(L, applied[(*it)[1]] - (*it)[2], fenw);
                add(R + 1, (*it)[2] - applied[(*it)[1]], fenw);
                sv1 = (*it)[1], sv2 = (*it)[2];
                if ((*it)[0] >= l) S.erase(it);
                it = it2;
            }
            if (r + 1 != (*it)[0]) S.insert({r + 1, sv1, sv2});
            S.insert({l, c, applied[c]});
        }
        if (opName[0] == 'A')
        {
            int c, x;
        ...
        if (opName[0] == 'Q')
        {
            ...
            cin >> pos;
            ll res = get(pos, fenw);
            auto it = S.lower_bound({pos, INF, 0});
            it--;
            auto it2 = *it;
            res += applied[it2[1]] - it2[2];
            cout << res << "\n";
        }
        #ifdef LOCAL
