...
fn rec(n: usize, m: usize, k: usize, memo: &mut [Vec<Vec<Option<usize>>>]) -> usize {
	assert!(n > 0 && m > 0 && n*m >= k);
	if let Some(res) = memo[n][m][k] {
		return res;
	}
	...
	if n*m == k || k == 0 {
		...
		for i in 1..n {
			for j in 0..=k {
				if j <= i*m && k-j <= (n-i)*m {
					min!(res, rec(i,m,j,memo) + rec(n-i,m,k-j,memo) + m*m);
				}
			}
		}
		for i in 1..m {
			for j in 0..=k {
				if j <= i*n && k-j <= (m-i)*n {
					min!(res, rec(n,i,j,memo) + rec(n,m-i,k-j,memo) + n*n);
				}
			}
		}
	}
	memo[n][m][k].replace(res);
	res
}
...
fn main() {
	...
	let mut dp = v!([31][31][51] = None);
	rep!{[rin.u()]
		l!(n,m,k = rin.u());
		let res = rec(n,m,k,&mut dp);
		writeln!(rout, "{}", res).ok();
	}
}