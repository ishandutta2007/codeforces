...
int main(){
    ...
    rep(ii,T){
        ...
        rep(i,N){
            v.emplace_back(x[i],d[i],i);
        }
        sort(all(v));
        ll id[N];
        rep(i,N){
            auto[xx,dd,ii] = v[i];
            x[i] = xx; d[i] = dd; id[i] = ii;
        }
        stack<Pr> ev,od;
        vector<ll> ans(N,-1);
        rep(i,N){
            if(x[i]%2==0){
                if(d[i]=='L'){
                    if(ev.empty()){
                        ev.emplace((-1)*x[i],id[i]);
                    }
                    else{
                        auto[xx,ii] = ev.top(); ev.pop();
                        ans[id[i]] = (x[i]-xx)/2;
                        ans[ii] = (x[i]-xx)/2;
                    }
                }
                ...
                if(d[i]=='L'){
                    if(od.empty()){
                        od.emplace((-1)*x[i],id[i]);
                    }
                    else{
                        auto[xx,ii] = od.top(); od.pop();
                        ans[id[i]] = (x[i]-xx)/2;
                        ans[ii] = (x[i]-xx)/2;
                    }
                }
        ...
        while(ev.size()>1){
            auto[xx,ii] = ev.top(); ev.pop();
            auto[yy,jj] = ev.top(); ev.pop();
            ans[ii] = (2*M-xx-yy)/2; ans[jj] = (2*M-xx-yy)/2;
        }
        while(od.size()>1){
            auto[xx,ii] = od.top(); od.pop();
            auto[yy,jj] = od.top(); od.pop();
            ans[ii] = (2*M-xx-yy)/2; ans[jj] = (2*M-xx-yy)/2;
        }
        out(ans);
    }
