...
struct Eratos {
    ...
    void init(int MAX) {
        ...
        mebius.assign(MAX+1, 1);
        min_factor.assign(MAX+1, -1);
        isprime[0] = isprime[1] = false;
        min_factor[0] = 0, min_factor[1] = 1;
        for (int i = 2; i <= MAX; ++i) {
            if (!isprime[i]) continue;
...
void solve() {
    ...
    map<int, int> up, down;
    map<int, int> ma;
    auto times = [&](int x, int y) -> void {
        for(auto [p, co]: er.prime_factors(y)) {
            if(down.count(p)) {
                int can = min(co, down[p]);
                co -= can, down[p] -= can;
            }
            up[p] += co;
        }
        for(auto [p, co]: er.prime_factors(x)) {
            if(up.count(p)) {
                int can = min(co, up[p]);
                co -= can, up[p] -= can;
            }
    ...
    auto dfs = [&](auto&&dfs, int v, int p, mint val) -> void {
        ans += val;
        for(auto [nv, x, y]: g[v]) {
            if(nv == p) continue;
            mint nval = val * mint(y) / mint(x);
            times(x, y);
            dfs(dfs, nv, v, nval);
            times(y, x);
        }
