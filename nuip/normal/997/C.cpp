...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	out(modpow(mint(3),1),mint(3),1);
  ll n;
	cin>>n;
	mint re=0;
	out(modpow(modpow(mint(3),n)-3,n),1);
	out(modpow(mint(3),n)-3,n,1);
	re+=modpow(modpow(mint(3),n)-3,n);
	re*=MOD-1;
	reps(i,1,n){
		re+=modpow(modpow(mint(3),n-i),n)*comb(n,i)*(modpow(3,i)-3);
		re+=modpow(modpow(mint(3),n-i)-1,n)*comb(n,i)*3;
		re*=MOD-1;
	}
	re+=(modpow(3,n)-3);
	//if(n%2) re*=MOD-1;
	if(n%2==0) re*=(MOD-1);
	cout<<modpow(mint(3),n*n%(MOD-1))+re<<endl;
	//cout<<modpow(mint(3),n*n%(MOD-1))+re*(MOD-1)<<endl;
  return 0;
}

/*
	mint re=modpow(3,n*n%(MOD-1));
	mint sgn=MOD-1;
	reps(k,1,2*n+1){
		if(k<n){
			re+=modpow(3,((n-k)*n+k)%(MOD-1))*comb(n,k)*2*sgn;
		}else{
			re+=3*sgn;
		}
		reps(i,1,k)if(i<n && n-k+i>0 && n-k+i<n) re+=modpow(3,((n-i)*(n-k+i)+1)%(MOD-1))*comb(n,i)*comb(n,k-i)*sgn;
		sgn*=MOD-1;
	}
	cout<<re<<endl;
