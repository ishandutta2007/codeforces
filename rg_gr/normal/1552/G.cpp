// Problem: G. A Serious Referee
// Contest: Codeforces - Codeforces Global Round 15
// URL: https://codeforces.com/contest/1552/problem/G
// Memory Limit: 512 MB
// Time Limit: 1000 ms
// 
...
int p[114][514],l[114],g[114],s[114][514];
int check1(vi x)
{
...
int check(vi x)
{
	if(check1(x))return 1;
	for(auto &p:x)if(!p)p=2;
	return !std::is_sorted(x.begin(),x.end());
}
void solve(vi x,int k)
{
	// printf("%d : ",k);for(auto p:x)odb(p);puts("");
	if(l[k]==0)
	{
		int p=0;
		for(auto j:x)if(!j)p++;
		if(p>1)puts("REJECTED"),exit(0);
		if(p&&check(x))puts("REJECTED"),exit(0);
		for(int j=0;j+1<x.size();j++)if(x[j]>x[j+1])puts("REJECTED"),exit(0);
		return;
	}
	int zmn=0,zmx=l[k];
	for(int i=1;i<=l[k];i++)
		if(x[p[k][i]]==1)zmn++;
		else if(x[p[k][i]]==2)zmx--;
	for(int i=1;i<=zmn;i++)x[p[k][i]]=1;
	for(int i=zmn+1;i<=l[k];i++)x[p[k][i]]=2;
	solve(x,k+1);
	for(int i=zmn+1;i<=zmx;i++)
	{
		x[p[k][i]]=1;
		solve(x,k+1);
	}
...
signed main()
{
	...
	vi q(n);
	rg(k)
	g[i]=read();rg_(j,g[i])s[i][j]=read()-1;gr
	gr
	for(int i=1;i<=k;i++)d[i]=i;
	// std::sort(d+1,d+1+k,[&](int x,int y){return g[x]>g[y];});
	for(int i=1;i<=k;i++)
	{
		l[i]=g[d[i]];
		for(int j=1;j<=l[i];j++)p[i][j]=s[d[i]][j];
	}
	solve(q,1);
	puts("ACCEPTED");
	return 0;
