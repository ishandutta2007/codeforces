...
const ll INF = 4e18, MOD = 998244353, MOD2 = 998244353, LOG = 30;
...
void mul(vector<ll> &a, vector<ll> &b, vector<ll> &rs){
    rs = fft::multiply_mod(a, b, MOD);
    return;
    int n = a.size();
    int m = b.size();
    rs.assign(n + m - 1, 0);
...
void solve1() {
    ...
    for (int cnt = 0; cnt <= n; ++cnt) {
        if (cnt & 1) ans = ((ans - f[n - cnt] * val[cnt]) % MOD + MOD) % MOD;
        else ans = ((ans + f[n - cnt] * val[cnt]) % MOD + MOD) % MOD;
    }
    cout << ans;
}
