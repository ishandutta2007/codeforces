...
int main() {
  ...
  d[0][1] = d[0][2] = -INF_LL;
  REP(i, n) {
    REP(j, 3) d[i+1][j] = -INF_LL;
    REP(x, 3) REP(y, 3) {
      ll sum = 0;
      FOR(j, min(x,y), max(x,y)+1) sum += a[j][i];
      smax(d[i+1][y], d[i][x] + sum);
    }
    if (i > 0) {
      ll sum = 0;
      REP(j, 3) sum += a[j][i-1] + a[j][i];
      smax(d[i+1][0], d[i-1][2] + sum);
      smax(d[i+1][2], d[i-1][0] + sum);
    }
  }
  writeln(d[n][2]);
