...
int main(){
    ...
    while(TT--) {
        ...
        rep(i,m) {
            ...
            int bb = 1e9;
            rep(j,n) {
                chmin(aa,dist[0][j]+dist[A[i]][j]+dist[n-1][j]+2);
                chmin(bb,dist[0][j]+dist[B[i]][j]+dist[n-1][j]+2);
            }
            chmin(ans,1LL*min(aa,bb)*C[i]);
            chmin(ans,1LL*(dist[0][A[i]]+dist[n-1][B[i]]+1)*C[i]);
            chmin(ans,1LL*(dist[0][B[i]]+dist[n-1][A[i]]+1)*C[i]);
        } 
        cout << ans << '\n';
    }
}