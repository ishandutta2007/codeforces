...
Hash getHash(Hash *a, int l, int r) {
    Hash res = a[r];
    FI(2) {
        res.vals[i] = (res.vals[i] - prp[i][r - l] * (ll) a[l].vals[i]) % p[i];
        if (res.vals[i] < 0) {
            res.vals[i] += p[i];
        }
    }
...
int main() {
    ...
    srand(time(0));
    int ps = 0;
    for (int i = 1e9 + 34; ps < 2; ++i) {
        if (rand() % 10 == 0 && isPrime(i)) {
            p[ps++] = i;
        }
    }
    for (int j = 0; j < 2; ++j) {
...
//    auto p[2] = {getHash(hs, 1, 2), getHash(<#Hash *a#>, <#int l#>, <#int r#>), <#int l#>, <#int r#>)
...
//        cout << endl;
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            while (l < r) {
                int m = (l + r + 1) / 2;
                int len = m - curmax;
                if (getHash(hs, i - 1, i - 1 + len) == getHash(ht, curmax, curmax + len)) {
                    l = m;
                } else {
                    r = m - 1;
                }
            }
            dp[f][i + l - curmax] = max(dp[f][i + l - curmax], l);
//            DBN(f, i, i + l - curmax, curmax, l);
            if (l == m) {
                ok = true;
            }
//            int k = dp[i - 1][r][0];
