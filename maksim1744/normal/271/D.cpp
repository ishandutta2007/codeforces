...
int main() {
    ...
    vector<Mint> h(s.size());
    vector<Mint2> h2(s.size());
    h[0] = s[0] - 'a';
    h2[0] = s[0] - 'a';
    for (int i = 1; i < s.size(); ++i)
        h[i] = h[i - 1] * 79 + s[i] - 'a';
    for (int i = 1; i < s.size(); ++i)
        h2[i] = h2[i - 1] * 131 + s[i] - 'a';
    vector<Mint> degp(s.size() + 5, 1);
    for (int i = 1; i < degp.size(); ++i)
        degp[i] = degp[i - 1] * 79;
    vector<Mint2> degp2(s.size() + 5, 1);
    for (int i = 1; i < degp2.size(); ++i)
        degp2[i] = degp2[i - 1] * 131;

    auto geth = [&](int l, int r) {
        Mint res = h[r];
        if (l != 0)
            res = res - h[l - 1] * degp[r - l + 1];
        Mint2 res2 = h2[r];
        if (l != 0)
            res2 = res2 - h2[l - 1] * degp2[r - l + 1];
        return make_pair(res.value, res2.value);
    };
