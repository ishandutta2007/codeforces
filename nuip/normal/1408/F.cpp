...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(_,20){
		...
		rep(i,m)if(i%2==0){
			if(i+1<m){
				if(cur[i].size()!=cur[i+1].size()){
					vs.eb(cur[i]);
					vs.eb(cur[i+1]);
					continue;
				}
				rep(j,cur[i].size()) re.eb(cur[i][j],cur[i+1][j]);
				vs.eb(cur[i]);
				auto &v=vs.back();
				v.insert(v.end(),all(cur[i+1]));
			}else{
				vs.eb(cur[i]);
			}
		}
	...
	out(vs,1);
	while(vs.size()>2){
		if(vs.back().size()==vs[vs.size()-2].size()){
			cauto v=vs.back(); vs.pop_back();
			rep(i,v.size()){
				re.eb(v[i],vs.back()[i]);
				vs.back().eb(v[i]);
			}
		}else{
			const int n=vs.back().size();
			rep(i,n){
				cauto w=vs[0].back(); vs[0].pop_back();
				re.eb(vs.back()[i],w);
				vs.back().eb(w);
			}
		}
