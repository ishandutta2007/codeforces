...
vector<long long> gogo_a,gogo_b;
int main() {
#define int long long
	...
	sort(gogo_a.begin(), gogo_a.end());
	sort(gogo_b.begin(), gogo_b.end());
	long long ans = 0LL;
	for (long long i = 0; i < gogo_a.size(); ++i) {
		long long arrival_mid = ta + gogo_a[i];
		long long bot = gogo_b.size();
		long long top = -1;
		while (abs(top - bot) > 1) {
			long long mid = (top + bot) / 2LL;
			if (gogo_b[mid] >= arrival_mid) {
				bot = mid;
			}
			else {
				top = mid;
		...
		long long now_go = bot + k;
		if (now_go >= gogo_b.size()) {
			cout << -1 << endl;
			return 0;
		}
		ans = max(ans, gogo_b[now_go] + tb);
		k--;
		if (k < 0) break;
	}
	cout << ans << endl;
