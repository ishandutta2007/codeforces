...
//		cin>>a[i].x>>a[i].y;
	...
	int id=n;
	P tmp[3],l,r,mid;
	double ans=0,now,K,ke,be;
	for(int i=n-1;i>=1;--i){
		tmp[0]=a[id]-O;
		tmp[1]=a[i+1]-O;
		tmp[2]=a[i]-O;
		if((tmp[1]/tmp[0])>=0&&(tmp[2]/tmp[0])>=0){
			ans+=sqrt(dis(a[i]-a[i+1]));
		}
		else if((tmp[2]/tmp[0])>=0){
			K=tmp[0].y/tmp[0].x;
			ke=(tmp[1].y-tmp[2].y)/(tmp[1].x-tmp[2].x);
			be=tmp[1].y-ke*tmp[1].x;
			r.x=be/(K-ke);
			r.y=K*r.x;
//			l=a[i+1];r=a[i];
//			for(int j=0;j<15;++j){
...
//				mid.x/=2;
//				mid.y/=2;
//				if(((mid-O)/tmp[0])>=0) r=mid;
//				else l=mid;
//			}
			r=r+O;
			now=dis(a[i]-r);
			if(now>0) ans+=sqrt(now);
//			ans+=sqrt(dis(a[i]-a[i+1]));
		}
		if((tmp[2]/tmp[0])>=0){
			id=i;
