...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		for(int i=1;i<=n+1;i++)f[i]=g[i]=-inf;
		f[1]=0;
		for(int i=1;i<=n;i++){
			int c=a[i]-'0',d=b[i]-'0',e=min(i+2,n+1);
			f[i+1]=max(f[i+1],f[i]+(a[i+1]-'0'));
			if(d)g[e]=max(g[e],f[i]+d+(b[i+1]-'0')+(b[i+2]-'0'));
			g[i+1]=max(g[i+1],g[i]+(b[i+1]-'0'));
			if(c)f[e]=max(f[e],g[i]+c+(a[i+1]-'0')+(a[i+2]-'0'));
//			printf("%d %d\n",f[i],g[i]);
		}
		printf("%d\n",max(f[n+1],g[n+1]));
	}
