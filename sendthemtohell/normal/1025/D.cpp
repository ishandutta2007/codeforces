...
bool go(int l, int r, int side) {
	if (used[side][l][r]) return dp[side][l][r];
	used[side][l][r] = true;
	if (l == r) return dp[side][l][r] = true;
	dp[side][l][r] = false;
	for (int i = l; i < r; ++i) {
		if (gc[i][side ? l - 1 : r] != 1) {
			dp[side][l][r] = dp[side][l][r] || (go(l, i, 0) && go(i + 1, r, 1));
		}
	}
	return dp[side][l][r];
}
