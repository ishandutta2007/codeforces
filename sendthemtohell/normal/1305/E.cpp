...
inline void solve() {
	...
	while (sz(ans) < n && ptr < N && m > cnt) {
		while (ptr < N && sp[ptr] > m - cnt) {
			++ptr;
		}
		ans.push_back(ptr);
		cnt += sp[ptr];
		for (int i = 0; i < sz(ans) - 1; ++i) {
			if (ans[i] + ptr < N) ++sp[ans[i] + ptr];
		}
		++ptr;
	}
	if (sz(ans) < n) {
		if (sz(ans)) {
			...
			int i = 1;
			while (sz(ans) < n) {
				ans.push_back(mx * i * 2 + 1);
				++i;
			}
		} else {
			for (int i = 0; i < n; ++i) {
				ans.push_back(100000 + i);
			}
		}
	}
	assert(ptr < N);
	...
	for (int i = 0; i < n; ++i) {
		for (int j = i + 1; j < n; ++j) {
			if (binary_search(all(ans), ans[i] + ans[j])) {
				++c2;
				//cerr << "? " << i << " " << j <<  endl;
			}
		}
