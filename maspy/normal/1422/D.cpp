...
void solve() {
  ...
  ++M;
  vi X, Y;
  for (auto&& [a, b]: dat) { X.eb(a), Y.eb(b); }
  UNIQUE(X);
  UNIQUE(Y);
  for (auto&& [a, b]: dat) a = LB(X, a), b = LB(Y, b);

  ll NX = len(X), NY = len(Y);
  auto xid = [&](ll i) -> ll { return M + i; };
  auto yid = [&](ll i) -> ll { return M + NX + i; };

  Graph<ll, 0> G(M + NX + NY);
  FOR(i, NX - 1) {
    ll d = X[i + 1] - X[i];
    G.add(xid(i), xid(i + 1), d);
  }
  FOR(i, NY - 1) {
    ll d = Y[i + 1] - Y[i];
    G.add(yid(i), yid(i + 1), d);
  }
  FOR(i, M) {
    auto [a, b] = dat[i];
    G.add(i, xid(a), 0);
  ...
  const ll INF = 1LL << 60;
  auto [dist, par] = dijkstra(G, 0, INF);
  ll ANS = INF;
  FOR(i, M) {
    auto [a, b] = dat[i];
    ll x = dist[i];
