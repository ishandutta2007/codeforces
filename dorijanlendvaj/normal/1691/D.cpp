...
int t,n,h[N];
pii seg1[M*2+10];
array<ll,3> seg2[M*2+10]; //sum, max pref sum, max suf sum
...
pii ge1(int l,int r,int lo=0,int hi=M,int i=1)
{
	...
	if (lo>=r || hi<=l) return {-MOD,-1};
	int mid=(lo+hi)/2;
	return max(ge1(l,r,lo,mid,i*2),ge1(l,r,mid,hi,i*2+1));
}

array<ll,3> mer(array<ll,3> a,array<ll,3> b)
{
	array<ll,3> c;
	c[0]=a[0]+b[0];
	c[1]=max(a[1],a[0]+b[1]);
...
{
	i+=M;
	seg2[i]={x,max(0,x),max(0,x)};
	for (i/=2;i;i/=2) seg2[i]=mer(seg2[i*2],seg2[i*2+1]);
}

array<ll,3> ge2(int l,int r,int lo=0,int hi=M,int i=1)
{
	if (lo>=l && hi<=r) return seg2[i];
	if (lo>=r || hi<=l) return {0,0,0};
	int mid=(lo+hi)/2;
	return mer(ge2(l,r,lo,mid,i*2),ge2(l,r,mid,hi,i*2+1));
}

bool ok;
int kol=0;

void wor(int l,int r)
{
	if (l>=r) return;
	int m=ge1(l,r).y;
	//if (kol++<20) cout<<l<<' '<<r<<' '<<m<<endl;
	if (ge2(l,m)[2]>0 || ge2(m+1,r)[1]>0) ok=0;
	wor(l,m);
	wor(m+1,r);
}
...
int main()
{
	...
	for (int i=M-1;i;--i) seg1[i]=max(seg1[i*2],seg1[i*2-1]);
	cin>>t;
	while (t--)
	{
		kol=0;
