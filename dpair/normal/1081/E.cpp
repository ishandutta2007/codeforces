...
int n; LL ans[100005];
signed main(){ int lst = 0; read(n);
    for (int i = 1;i <= n;i += 2) {
        const int x = read(); int u = -1;
        for (int j = 1;j * j <= x;++ j) if(x % j == 0 && !((j ^ (x / j)) & 1) && (((x / j) - j) >> 1) > lst) u = j;
        if(~u) {
            const int a = (u + (x / u)) >> 1, b = ((x / u) - u) >> 1;
            ans[i] = 1ll * b * b - 1ll * lst * lst;
            ans[i + 1] = 1ll * a * a - 1ll * b * b;
            lst = a;
