...
vpii gen(int n) {
	...
	for (int i = 0; i + 2 < 2 * n; i += 2)
		if (i != n - 2) ans.emplace_back(i, i + 1);
	ans.emplace_back(n - 2, 2 * n - 2);
	ans.emplace_back(n - 1, 2 * n - 1);
	return ans;
}
int n;
...
void put_edge(int a, int b) {
	static int num = 0;
	debug(imie(a), imie(b));
	graf[a].emplace_back(b, num);
	graf[b].emplace_back(a, num);
	num++;
}
bool did[nax];
int sum(const vi &x, int mod) {
	int ans = 0;
	for (int c : x) (ans += c) %= mod;
	rans % mod;
}
void output(const vi &x) {
	for (int a : x) printf("%d ", a == 0 ? 2 * n : a);
	printf("\n");
}
int main() {
	scanf("%d", &n);
	if (n % 2) {
		...
		for (int i = 0; i < n; ++i) put_edge(i, i + n);
		vi s1, s2;
		REP(i, 2 * n) if (!did[i]) {
			int last = -1;
			vi cycle;
			...
			int len = cycle.size();
			assert(len % 2 == 0);
			REP(i, len) (i % 2 ? s1 : s2).push_back(cycle[i]);
		}
		int su1 = sum(s1, 2 * n), su2 = sum(s2, 2 * n);
		assert(su1 == 0 || su2 == 0);
		if (su1 == 0) output(s1);
		else output(s2);
	}
		...
		vpii g = gen(n);
		REP(i, n) {
			ord[g[i].first] = ord[g[i].second] = i;
		}
		for (int i = 1; i <= 2 * n; ++i) printf("%d ", 1 + ord[i % (2 * n)]);
		printf("\n");
	}
}
//#STAY AT HOME