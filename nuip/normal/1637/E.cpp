...
int main(){
	...
	TT{
		...
		cauto m=read();
		cauto vs=readV(n,0);
		cauto ps=readV<pii>(m);
		map<int,int> freq;
		for(cauto &v:vs) ++freq[v];
		vector<int> fs;
		for(cauto &p:freq) fs.eb(p.Y);
		sort(all(fs)); UNIQUE(fs);
		const int N=fs.size();
		vv<int> invs(N);
		for(cauto &p:freq) invs[lower_bound(all(fs),p.Y)-fs.begin()].eb(p.X);
		for(auto &vs:invs) sort(rall(vs));
		out(freq,fs,invs,1);
		cauto zip=vs-distinct;
		auto g=table(zip.size(),N,vector<int>());
		for(cauto &[a,b]:ps){
			g[lower_bound(all(zip),a)-zip.begin()][lower_bound(all(fs),freq[b])-fs.begin()].eb(b);
			g[lower_bound(all(zip),b)-zip.begin()][lower_bound(all(fs),freq[a])-fs.begin()].eb(a);
		}
		rep(i,zip.size()) g[i][lower_bound(all(fs),freq[zip[i]])-fs.begin()].eb(zip[i]);
		ll re=0;
		rep(vi,zip.size()){
			cauto &v=zip[vi];
			cauto fv=freq[v];
			rep(fi,fs.size()){
				...
				sort(rall(ban));
				cauto &vs=invs[fi];
				if(vs.size()==ban.size()) continue;
				int i=0;
				while(i<ban.size() && ban[i]==vs[i]) ++i;
				out(i,f*ll(v+vs[i]),fs[fi],fv,v,vs[i],1);
				MX(re,f*ll(v+vs[i]));
			}
		}
		cout<<re NL;
