...
const int da[]={-1,-1,0,1,1,1,0,-1};
const int db[]={0,1,1,1,0,-1,-1,-1};
vector<int> A;
map<pii,set<pii>> M;

const set<pii>& doit(int q,int d){
  if(M.count(mp(q,d)))
    return M[mp(q,d)];
  auto& r=M[mp(q,d)];
  if(q==sz(A))
    return r;
  forn(i,A[q])
    r.insert(mp((i+1)*da[d],(i+1)*db[d]));
  for(int dd:{(d+1)%8,(d+7)%8}){
    auto& t=doit(q+1,dd);
    for(pii p:t)
      r.insert(mp(p.X+A[q]*da[d],p.Y+A[q]*db[d]));
  }
  return r;
}
