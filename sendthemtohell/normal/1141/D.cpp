...
inline void solve() {
	...
	for (int i = 0; i < sz(s); ++i) {
		lb[ind(s[i])].push_back(i + 1);
	}

	for (int i = 0; i < sz(t); ++i) {
		rb[ind(t[i])].push_back(i + 1);
	}

	vector<pair<int, int>> ans;
	vector<int> restl, restr;

	for (int i = 0; i < 26; ++i) {
		while (sz(lb[i]) && sz(rb[i])) {
			ans.push_back({ lb[i].back(), rb[i].back() });
			lb[i].pop_back();
			rb[i].pop_back();
		}
		while (sz(lb[i])) {
			restl.push_back(lb[i].back());
			lb[i].pop_back();
		}
		while (sz(rb[i])) {
			restr.push_back(rb[i].back());
			rb[i].pop_back();
		}
	}
	while (sz(lb[26]) && sz(restr)) {
		ans.push_back({ lb[26].back(), restr.back() });
		lb[26].pop_back();
		restr.pop_back();
	}
	while (sz(rb[26]) && sz(restl)) {
		ans.push_back({ restl.back(), rb[26].back() });
		rb[26].pop_back();
		restl.pop_back();
	}
	int i = 26;
