...
void solve() {
  map<ll, tuple<mint, mint, mint>> MP;
  auto f = [&](auto& f, ll N) -> tuple<mint, mint, mint> {
    if (MP.count(N)) return MP[N];
    mint win = 0, lose = 0, draw = 0;
    if (N == 0) { return {mint(0), mint(0), mint(1)}; }
    if (N == 1) { return {mint(1), mint(0), mint(1)}; }

    // 
    win += C<mint>(N + N - 1, N - 1);
    //  2 
    lose += C<mint>(N + N - 2, N - 2);
    // 
    auto [a, b, c] = f(f, N - 1);
    win += b;
    lose += a;
    draw += c;
    return (MP[N] = {win, lose, draw});
  };
