...
int main() {
    ...
    for (int bit = 0; bit < K; bit++) {
        ...
        //sort_by(not_all, a.f != b.f ? a.f < b.f : a.s > b.s);
        ...
        int tail = 0;
        FOR (i, 1, P) {
            // constraints that need to already have been satisfied
            while (tail < sz(keep) && keep[tail].s <= i) {
                tail++;
            }
            int prev_zero = tail ? keep[tail-1].f : 0; // previous zero occurs >= this
