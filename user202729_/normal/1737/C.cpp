...
void up(){
	int const size=read<int>();
	std::array<std::pair<int, int>, 4> points;
	for(auto& [x, y]: points) {
		std::cin>>x>>y;
		--x;
		--y; // insist on using 0-indexed coordinates
	}

	while(true){
		auto const correctOrientation=[&]{
			// 2 of 3 has the same min X-coordinate
			std::sort(begin(points), end(points)-1, [&](auto const& first, auto const& sec){
				return first.first<sec.first;
			});
			if(points[0].first!=points[1].first) return false;

			// 2 of 3 has the same min Y-coordinate
			std::sort(begin(points), end(points)-1, [&](auto const& first, auto const& sec){
				return first.second<sec.second;
			});
			if(points[0].second!=points[1].second) return false;

			return true;
		};
		if(correctOrientation()) break;
		for(auto& p: points) p={p.second, size-1-p.first}; // also rotate the target cell
	}

	std::sort(begin(points), end(points)-1);
	// now points has the form something like (x, y) (x, y+1) (x+1, y)
	
	if(points[0]==std::make_pair(0, 0)){
		std::cout<<
			((points[3].first==0 or points[3].second==0)
				? "YES\n": "NO\n"
			);
	}else{
		std::cout<<
			((((points[3].first^points[0].first)&1)==0 or ((points[3].second^points[0].second)&1)==0)
				? "YES\n": "NO\n"
			);
	}
