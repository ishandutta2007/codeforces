	...
	while(fabs(high-low)>EPS){
	    ...
	    while(l<n){
    		...
    		For(i,tmp.size()){
    			...
    			int t1=prv_tim+t[x],t2=prv_tim+tmp_time;
    			mxsc[x]=p[x]*(1.0-((ld)c*(t1))/((ld)T));
    			mnsc[x]=p[x]*(1.0-((ld)c*(t2))/((ld)T));
    // 			trace(x,p[x],mnsc[x],c,t1,t2,mxsc[x]);
    // 			int p1=p[x],p2=p[y];
    // 			ld currrr=(p2-p1)*T;
    // 			currrr/=((ld) p2*t2-p1*t1);
    // 			c=min(c,currrr);
    		}
    		prv_tim+=tmp_time;
    		l=r+1;
