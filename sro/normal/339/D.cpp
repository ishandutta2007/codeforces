...
struct Segtree{
	int dt[1600005];
	void change(int now,int nl,int nr,int to,int x,int ceng=n){
		if(!ceng){
			dt[now]=x;
			return;
		}
		int m=(nl+nr)>>1;
		if(m>=to){
			change(now<<1,nl,m,to,x,ceng-1);
		}else{
			change(now<<1|1,m+1,nr,to,x,ceng-1);
		}
		if(ceng%2){
...
int main(){
	...
	for(int i=0;i<(1<<n);i++){
		int x;
		fio>>x;
		seg.change(1,0,MASK(n)-1,i,x);
	}
	for(int i=0;i<m;i++){
		int x,y;
		fio>>x>>y;
		seg.change(1,0,MASK(n)-1,x-1,y);
		fio<<seg.dt[1]<<endl;
	}
	return 0;
}