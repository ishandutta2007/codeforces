...
{
    int b = a + 1;
    if (v[a].ff == v[b].ff && v[a].ss > v[b].ss) return 0; //Down
    if (v[a].ff == v[b].ff && v[a].ss < v[b].ss) return 1; //Up
    if (v[a].ss == v[b].ss && v[a].ff < v[b].ff) return 2; //Right
    if (v[a].ss == v[b].ss && v[a].ff > v[b].ff) return 3; //Left
}

int getDir(int a)
{
    //cout << "getDir " << a << " = " << getd(a) << endl;
    return getd(a);
}

bool fall(int a, int b)
{
    int d1 = getDir(a), d2 = getDir(b);
    if (d1 == 0 && d2 == 2) return true;
    if (d1 == 1 && d2 == 3) return true;
    if (d1 == 2 && d2 == 1) return true;
    if (d1 == 3 && d2 == 0) return true;
    return false;
}
