...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int x = first; x <= last; x++)
            mult.push_back(choose(n, x));

        vector<mod_int> product = ntt.mod_multiply(ways, mult);
        int start = (int(product.size()) - g) / 2;
        int end = (int(product.size()) + g) / 2;
        ways = vector<mod_int>(product.begin() + start, product.begin() + end);
    }

    cout << accumulate(ways.begin(), ways.end(), mod_int(0)) << '\n';
}