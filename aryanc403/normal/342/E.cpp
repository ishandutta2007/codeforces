...
struct Centroid{
    const static lli maxN = 100000L;

    lli sz[maxN+5],n;
    bool blocked[maxN+5];
    vi e[maxN+5];

    map<lli,lli> cldDis[maxN+5];
    lli p[maxN+5],dp[maxN+5];

    Centroid(lli n=0){    init(n);}
    ...
    void run(lli start,lli par=-1)
    {
        lli sz=PreDfs(start);
        lli u=search(start,sz);
        dfsCldDis(u,u);
        p[u]=par;
        blocked[u]=true;
        for(auto x:e[u])
    ...
    void dfsCldDis(lli u,lli root,lli p=-1,lli h=0)
    {
        cldDis[root][u]=h;
        for(auto x:e[u])
        {
