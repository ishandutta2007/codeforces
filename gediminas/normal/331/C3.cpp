...
int main () {
	...
	for (int j = 1; j < 19; j++) {
		for (int i = 0; i < 10; i++) {
			for (int k = 0; k < 10; k++) {
				...
				for (int l = 9; l >= 0; l--) {
					// debug(max(i, l), j - 1, pa);
					din[i][j][k] += din[max(i, l)][j - 1][pa];
					pa = pask[max(i, l)][j - 1][pa];
				}
	...
	while (x > 0) {
		skait.push_back(x % 10);
		x /= 10;
	}

	reverse(skait.begin(), skait.end());
	...
	while (!skait.empty()) {
		...
		if (!skait.empty())
			ma = *max_element(skait.begin(), skait.end());

		while (d >= 0) {
			ats += din[max(ma, d)][dev][pa];
			pa = pask[max(ma, d)][dev][pa];
			debug(d, ats, pa);
			d--;
		}
