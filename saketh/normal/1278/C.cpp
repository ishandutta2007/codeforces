...
void solve() {
    ...
    map<int, int> rbal; // rbal[b] = highest amount we can leave on the right if seeking balance b
    int bal = 0;
    F0R (right, N + 1) {
        rbal[bal] = right;
        if (right < N) bal += (*(jars.rbegin() + right) == 1) ? 1 : -1;
    }
    ...
    bal = 0;
    F0R (left, N + 1) {
        auto it = rbal.find(-bal);
        if (it != rbal.end()) ckmax(ans, left + it->s);
        if (left < N) bal += (*(jars.begin() + left) == 1) ? 1 : -1;
    }
