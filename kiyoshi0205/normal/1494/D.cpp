...
int main(){
  ...
  rep(i,N){
    rep(j,N){
      ll x;
      cin>>x;
      if(i!=j)g[x].emplace_back(i,j);
      else ansv[i]=x;
    }
  }
  rep(i,5001){
    auto lis=g[i];
    if(lis.empty())continue;
    debug(i,lis);
    unionfind dsu(N);
    for(auto x:lis)dsu.unite(x);
    auto sg=dsu.groups();
    debug(sg);
    for(auto v:sg){
      ansv.emplace_back(i);
      //K to root of v 
      for(auto& x:v)x=solve.root(x);
      uniq(v);
      debug(v);
      for(auto x:v){
        debug(x,K);
        ansg.emplace_back(x,K);
        solve.unite(x,K);
      }
      K++;
    }
  ...
  print(K);
  rep(i,K-1){
    print(ansg[i].first+1<<" "<<ansg[i].second+1);
  }
}