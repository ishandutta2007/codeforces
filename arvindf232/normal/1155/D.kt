...
fun main(){
    solve.cases{
        val n = getint()
         val x = getint()
        val L = getlineL(n)
        val DP = Array( n+1){LongArray(9){nhuge} }
        DP[0][0] = 0
        for(p in 0 until n){
            for(left in 0 until 9){
                val l1 = left /3
                val l2 = left % 3
                for(right1 in l1 until 3){
                    for(right2 in l2 until 3){
                        val right = right1 * 3 + right2
                        val here = DP[p][left]
                        val extra = if(right1 != 1) 0L else if(right2 == 1) x * L[p] else L[p]
                        DP[p+1][right] = maxOf(DP[p+1][right],here + extra)
                    }
                }
            }
        }
