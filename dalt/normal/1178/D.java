...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            EulerSieve sieve = new EulerSieve(2000);
            Set<Integer> primes = new HashSet<>();
            for (int i = 0; i < sieve.primeLength; i++) {
                primes.add(sieve.primes[i]);
            }
            ...
            int nextRound = (n + 1) / 2;
            int now = 1;
            while (!primes.contains(ans.size())) {
                ans.add(new int[]{now, now + nextRound});
                now++;
            }

            io.cache.append(ans.size()).append('\n');
            for (int[] edge : ans) {
                io.cache.append(edge[0]).append(' ').append(edge[1]).append('\n');
            }
        }
    }
