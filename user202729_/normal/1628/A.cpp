...
void up(){
	...
	std::vector<std::vector<int>> posReverse(number+1);
	for(int index=number; index--;){
		posReverse[a[index]].push_back(index);
	}
	...
	while(left<number){
		int value=0;
		int cutPoint=left;  // if there's no 0 left in the sequence, the rest of the array b are all zero, make as many zeroes as possible
		while(not posReverse[value].empty()){
			assert(posReverse[value].back()>=left);
			cutPoint=std::max(cutPoint, posReverse[value].back());
			value++;
		}
		b.push_back(value);
		assert(cutPoint>=left);
		while(left<cutPoint+1){
			assert(posReverse[a[left]].back()==left);
			posReverse[a[left]].pop_back();
			left++;
		}
	}
	std::cout<<b.size()<<'\n';
...
// still make a few mistakes >< fortunately samples are reasonably strong