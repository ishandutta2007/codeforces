...
namespace linearadd_sum{
  struct M{
    ll sum,left,len;
  };
  struct OM{
    ll a,b;
    bool operator==(OM x) const{
      return a == x.a && b == x.b;
    }
    bool operator!=(OM x) const{
      return !(*this == x);
  ...
  auto f=[](M x,M y)->M{
    return {x.sum+y.sum,min(x.left,y.left),x.len+y.len};
  };
  auto g=[](M x,OM y)->M{
    return {x.sum+(y.a*x.left+y.b)*x.len+y.a*x.len*(x.len-1)/2,x.left,x.len};
  };
  auto h=[](OM x,OM y)->OM{
    return {x.a+y.a,x.b+y.b};
  };
  LazySegmentTree<M,OM,decltype(f),decltype(g),decltype(h)>make(int n){
    return {n,f,g,h,{0,INF,0},{0,0}};
  }
}
...
int main(){
  ...
  seg.build();
  rrep(i,k,n){
    ll num=max(0LL,CEIL(seg[i].sum,k));
    //cout<<i spa num spa seg[i].sum<<endl;
    seg.update(i-k+1,i+1,{-num,+num*(i-k)});
    //cout<<i spa num spa seg[i].sum<<endl;
    res+=num;
    //cout<<seg[i].sum<<endl;
  }
  ...
  ll mx=0;
  rrep(i,0,k){
    //cout<<seg[i].sum spa i spa CEIL(seg[i].sum,i+1)<<endl;
    chmax(mx,CEIL(seg[i].sum,i+1));
  }
  res+=mx;
  cout<<res<<endl;
  return 0;
