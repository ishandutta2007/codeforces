...
constexpr int MAX_B = 62;
int dp[MAX_N][MAX_B];
ll ans;
int n, k;
int lowest_bit_considered;
ll arr[MAX_N], jum;
...
int f(int pos, int kleft) {
	...
	FORD (j, pos, 1) {
		jum += arr[j];
		ll tmp = jum >> lowest_bit_considered;
		tmp &= ans >> lowest_bit_considered;
		if (tmp == (ans >> lowest_bit_considered)) {
			res |= f(j - 1, kleft - 1);
		}
	}
	return res;
