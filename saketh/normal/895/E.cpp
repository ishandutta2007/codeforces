...
int main() {
    ...
    struct N { double v; int w; };
    auto nn = [](N a, N b) { return N{a.v+b.v, a.w+b.w}; };

    struct U { double m, b; };
    auto uu = [](U a, U b) { return U{ a.m*b.m, a.b + a.m * b.b}; };

    auto un = [](U u, N n) { return N{u.m * n.v + u.b * n.w, n.w}; };

    int M, Q; re(M, Q);
    segment_tree_lazy st(M, N{0.,0}, U{1.,0.}, nn, uu, un);
    st.set_leaves([](int _) { int v; re(v); return N{double(v), 1}; });

    F0R (q, Q) {
        int t; re(t);
        if (t == 1) {
            int a, b, c, d; re(a, b, c, d); --a, --c;

            double e1 = st(a, b).v / (b - a);
            double e2 = st(c, d).v / (d - c);

            st(a, b, U{ (b-a-1)/double(b-a), e2/(b-a) });
            st(c, d, U{ (d-c-1)/double(d-c), e1/(d-c) });
        } else {
            int a, b; re(a, b); --a;
            ps(st(a, b).v);
        }
