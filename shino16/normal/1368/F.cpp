...
int main() {
  ...
  int R = 0; int k = -1;
  rep(kx, 1, n-1) if (chmax(R, n - divceil(n, kx) - kx + 1)) k = kx;
  ...
  VB table(n, false);
  priority_queue<int, vector<int>, greater<>> todo;
  VI v_todo;

  int mj = 0;
  rep(i, n/k + 1) rep(j, k-1) if (i*k+1+j < n) v_todo.push_back(i*k+1+j), todo.push(i*k+1+j), chmax(mj, i*k+1+j);
  ...
  while (r < R) {
    ...
    prt(move(go));
    int x = read();
    rep(j, x, x+k) if (table[j % n]) table[j % n] = false, r--, todo.push(j % n);
  }
