...
typedef pair<ll, vector<ll> > R;

ll n, m;
vector<ll> vec[15];
set<vector<ll> > S, usedS;
...
int main(void)
{
	...
	rep(i, 1, m){
		vector<ll> tmp;
		rep(j, 1, n) cin >> x, tmp.push_back(x);
		S.insert(tmp);
	}
	
	vector<vector<ll> > avec;
	priority_queue<R> Q;
	
	vector<ll> tmp; ll sum = 0;
	rep(i, 1, n) tmp.push_back(sz(vec[i])), sum += vec[i].back();
	Q.push(R(sum, tmp)), usedS.insert(tmp);
	
	rep(i, 1, m+1){
		sum = Q.top().first, tmp = Q.top().second;
		//outl(Q.top());
		Q.pop();
		avec.push_back(tmp);
		...
		rep(j, 1, n){
			if(tmp[j-1] >= 2){
				sum -= vec[j][tmp[j-1]-1], tmp[j-1]--, sum += vec[j][tmp[j-1]-1];
				if(usedS.count(tmp) == 0) Q.push(R(sum, tmp)), usedS.insert(tmp);
				sum -= vec[j][tmp[j-1]-1], tmp[j-1]++, sum += vec[j][tmp[j-1]-1];
			}
		}
	}
