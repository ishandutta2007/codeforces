#line 1 "codeforces-workspace\\contest\\1240\\c\\c2.cpp"
#line 1 "codeforces-workspace/contest/1240/c/c.cpp"
#include <bits/extc++.h>
...
template <class T> class coordinate_compression {
 ...
 public:
  ...
  size_t order_of(const T &val) const {
    return std::lower_bound(uniquely.begin(), uniquely.end(), val) -
           uniquely.begin();
  }
...
long double trinary(long double first, long double last, const long double eps,
                    comp_type comp) {
  assert(first < last);
...
#line 7 "codeforces-workspace/contest/1240/c/c.cpp"
namespace workspace {
void main();
}
...
void main() {
  ...
  cin >> n >> k;
  const i64 inf = 1e18;
  vector<i64> dp1(n, -inf), dp0(n, -inf);
  struct edge {
    int to, cost;
  };
  ...
  fixed_point([&](auto dfs, int now, int pre) -> void {
    ...
    sort(begin(ngh), end(ngh),
         [&](int x, int y) { return dp0[x] - dp1[x] > dp0[y] - dp1[y]; });
    for (int j = 0;; j++) {
      if (j == k) {
        dp1[now] = sum;
        break;
      ...
      chge(dp0[now], sum);
      if (j == size(ngh)) break;
      sum += dp0[ngh[j]] - dp1[ngh[j]];
    }
  })(0, -1);
  cout << max(dp0[0], dp1[0]) << eol;
}
}  // namespace workspace