...
modBase<> geometricProgression(modBase<> v, int n) {
	return (modBase<>(1) - fexp(v, n)) / (modBase<>(1) - v);
}
...
int main() {
	...
	while(t--) {
		int n;
		std::cin >> n;
		std::cout << (geometricProgression(8, n / 3) * fexp(modBase<>(2), n % 3 + 1) - geometricProgression(-1, n / 3) * fexp(modBase<>(-1), n % 3 + 1)) * 4 / 3 << '\n';
	}
}