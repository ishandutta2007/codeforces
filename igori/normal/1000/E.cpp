...
//___________________
int n, m;
vector<pii> graph[301000];
int in[301000];
int out[301000];
int go[301000];
int c[301000];
int t = 1;
vector<int> other[301000];
...
void dfs2(int v, int fl)
{
    ...
    for (int i = 0; i < graph[v].size(); i++)
    {
        ...
        if (r == 0 && c[u] != 0)
        {
            other[c[u]].push_back(fl);
            other[fl].push_back(c[u]);
        }
    }
}
