...
int main(int argc, const char **argv)
{
    ...
    for (int i = 0; i < N - 1; i++)
    {
        ...
        int b = min(r[i], r[i + 1]);
        int c = max(0, b - a);
        p[i] = mr(1) - mr(c) / (mr(r[i] - l[i]) * mr(r[i + 1] - l[i + 1]));
        psum += p[i];
    }
    mr ans = psum * psum;
    for (int i = 0; i < N - 1; i++)
    {
        ...
        if (i < N - 2)
        {
            int a = max({l[i], l[i + 1], l[i + 2]});
            int b = min({r[i], r[i + 1], r[i + 2]});
            int c = max(0, b - a);
            mr qij = mr(c) / (mr(r[i] - l[i]) * mr(r[i + 1] - l[i + 1]) * mr(r[i + 2] - l[i + 2]));
            mr pij = p[i] + p[i + 1] + qij - 1;
            ans += (pij - p[i] * p[i + 1]) * 2;
        }
    }
    cout << ans << '\n';
