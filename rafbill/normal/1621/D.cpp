...
const li dx[4] = {-1,1,0,0};
const li dy[4] = {0,0,-1,1};

void solve(){
  li n; cin>>n;
  vvi C(2*n,vi(2*n));
  for(li i = 0; i < (li)(2*n); ++i) for(li j = 0; j < (li)(2*n); ++j) cin>>C[i][j];
  li ans = 0;
  for(li i = (n); i <= (li)(2*n-1); ++i) for(li j = (n); j <= (li)(2*n-1); ++j) {
    ans += C[i][j];
    C[i][j] = 0;
  }
  cout << ans+min(
    {C[0][n],C[n-1][n],C[0][2*n-1],C[n-1][2*n-1],
