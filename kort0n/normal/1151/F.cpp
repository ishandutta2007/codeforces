...
int main() {
    ...
    Matrix<ll> A(one + 1, one + 1);
    cerr << one << endl;
    ll factor = beki(N * (N - 1) / 2, mod - 2);
    for(int i = 0; i <= one; i++) {
        ll Rone = i;
        ll Rzero = one - Rone;
        ll Lone = one - Rone;
        ll Lzero = N - one - Rzero;
        ll sum = N * (N - 1) / 2;
        if(i > 0) {
            ll tmp = Rone * Lzero;
            sum -= tmp;
            A[i-1][i] = tmp * factor % mod;
        }
        if(i < one) {
            ll tmp = Rzero * Lone % mod;
            sum -= tmp;
            A[i+1][i] = tmp * factor % mod;
        }
        A[i][i] = sum * factor % mod;
    }
    //cerr << A << endl;
    A ^= k;
    ll Rone = 0;
