...
inline void clear(){tot = Trie[0][0] = Trie[0][1] = 0; }
inline int NewNode() {
    const int rt = ++ tot;
...
inline void ins(const int x) {
    int rt = 0; per(i, 7, 0) {
        const bool ck = x >> i & 1;
        if(!Trie[rt][ck]) Trie[rt][ck] = NewNode();
        rt = Trie[rt][ck];
    } 
}
int g[256];
inline int query(const int x) {
    int rt = 0, ret = 0; per(i, 7, 0) {
        const int ck = x >> i & 1;
        if(Trie[rt][ck ^ 1]) ret |= ((ck ^ 1) << i), rt = Trie[rt][ck ^ 1];
        else ret |= (ck << i), rt = Trie[rt][ck];
    } return ((ret ^ x) << 8) | g[ret];
}

int fa[MAXN], f[MAXN][256], p[MAXN], dep[MAXN];
int t[256];
const int lim = 256;
void dfs(int x, int pre) {
    dep[x] = dep[fa[x] = pre] + 1;
    if(dep[x] >= lim) {
        memset(g, 0, sizeof(g)); clear();
        int u = x; while(dep[x] - dep[u] < lim) {
            const int v = a[u] >> 8; chmax(g[v], (a[u] ^ (dep[x] - dep[u])) & 255);
            if(t[v] != x) ins(v), t[v] = x;
            u = fa[u];
        } REP(i, 0, lim) f[x][i] = query(i);
        p[x] = u;
    }
...
signed main(){
    ...
    dfs(1, 0); while(q --) {
        int u, v; read(u, v); int d = 0, ans = 0;
        while(dep[v] - dep[u] >= lim) {
            chmax(ans, f[v][d]);
            v = p[v]; ++ d;
        } d <<= 8;
        while(v ^ fa[u]){
