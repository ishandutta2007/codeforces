...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    string s0=s;
    bool f=false;
    if(s[0]==s.back()) f=true;
    else{
      forv(i,s){
        if((!i||s[i-1]!=s[i])&&(i+1==sz(s)||s[i+1]!=s[i])){
          f=true;
          dbg(i);
    ...
    if(!f){
      assert(sz(s)>1&&s[0]==s[1]);
      int c=0;
      while(s[c]==s[0]) ++c;
      if((c-2)*2<=sz(s)-2){
        string t=s.substr(c,c-2);
        forv(i,t){
          s[2+i*2]=t[i];
          s[3+i*2]=s[0];
        }
        ...
        ++c;
        int p=c;
        while(p<sz(s)&&s[p]==s[1]) ++p;
        if(p==sz(s)){
          dbg("B",c);
          rotate(s.begin()+2,s.begin()+c,s.end());
        }else{
          dbg("C",c,p);
          swap(s[c],s[p]);
        }
