  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		while(r-l>1){
			...
			out(t,ub,total,1);
			(ub>=mx && total>=sum ? r : l) = m;
		}
		out(r,1);
		auto re=table(r,r,0);
		vector<pii> ps;
		rep(i,r)rep(j,r)if(i%2==0 && j%2==1) ps.eb(i,j);
		rep(i,r)rep(j,r)if(i%2==0 && j%2==0) ps.eb(i,j);
		rep(i,r)rep(j,r)if(i%2==1 && j%2==0) ps.eb(i,j);
		vector<int> inds(n); iota(all(inds),0);
		sort(rall(inds),[&](cauto &i,cauto &j){ return vs[i]<vs[j];});
		for(cauto v:inds)rep(_,vs[v]){
				cauto [a,b]=ps.back(); ps.pop_back();
				re[a][b]=v+1;
			}
		cout<<r NL;
		rep(i,r){
