...
{
    tree[2 * V + 1] += push[V] * (M - L);
    push[2 * V + 1] += push[V];
    tree[2 * V + 2] += push[V] * (R - M);
    push[2 * V + 2] += push[V];
    push[V] = 0;
}
...
signed main()
{
    ...
    string s;
    cin >> n >> s;
    ll ans111 = 0, ans1100 = 0, ans0011 = 0, ans010 = 0;

    //solve(n, s);

    vector<pair<int, ll> > bl;
    int cnt = 0;
    ...
    L[bl.size()] = n - cnt;
    R[bl.size()] = n - 1;
    bl.push_back({s.back() - '0', cnt});
    for (auto e : bl)
    {
        if (e.first == 1)
        {
            for (ll len = 1; len < e.second; len++)
            {
                ans111 += (e.second - len + 1) * len;
            }
        }
    }
    //cout << ans111 << endl;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            int R = i - ends - 1;
            int L = Get(ends, ends + 1);
            //cout << i << " " << L << " " << R << endl;
            if (L <= R)
            {
                ans0011 += 1ll * (R - L + 1) * (ends + 1);
            }
        }
        if (s[i] == '1') ends++;
        else ends = 0;
        Add(0, ends - 1, 1);
        Set(ends - 1, i - ends + 2);
        //for (int j = 0; j <= n; j++) cout << Get(j, j + 1) << " ";
        //cout << endl;
    }
    //cout << ans0011 << endl;
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        ...
        Add(0, ends - 1, -1);
        Set(ends - 1, i + ends - 2);
        if (i - 1 >= 0 && s[i] == '1' && s[i - 1] == '1')
        {
            int L = i + ends;
            int R = Get(ends, ends + 1);
            if (L <= R)
            {
                ans1100 += 1ll * (R - L + 1) * (ends);
            }
        }
    }
    //cout << ans1100 << endl;

    vector<pair<int, int> > blocks_of_ones;
    vector<int> z;
    int m = bl.size();
    for (int i = 0; i < bl.size(); i++)
    {
        ...
        if (bl[i].first == 1)
        {
            blocks_of_ones.push_back({bl[i].second, -i});
        }
        else
        {
            z.push_back(i);
    ...
    sort(blocks_of_ones.begin(), blocks_of_ones.end());
    vector<int> le(m), ri(m);
    for (int i = 0; i < m; i++) le[i] = i - 1, ri[i] = i + 1;
    for (auto e : z)
    {
        int LL = le[e];
        int RR = ri[e];
    ...
    for (auto e : blocks_of_ones)
    {
        ...
        //cout << lele << " " << leri << " " << rile << " " << riri << endl;
        if (lele <= leri && rile <= riri)
        {
            ans010 += (leri - lele + 1) * (riri - rile + 1) * len;
        }
        if (LL != -1) ri[LL] = RR;
        if (RR != m) le[RR] = LL;
    }
    ...
    cout << ans111 + ans1100 + ans0011 + ans010 << endl;
}
