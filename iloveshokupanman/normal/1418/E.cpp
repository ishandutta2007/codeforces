...
int main() {
    ...
    vll r(N + 1);
    repn(i, N)r[i] = r[i - 1] + d[i - 1];

    rep(i, M) {
        ll a, b;
        cin >> a >> b;

        auto itr1 = lower_bound(d.begin(), d.end(), b);
        ll x = distance(d.begin(), itr1);
        ...
        if (count < a) { cout << 0 << endl; }
            ...
            mint ans = (mint)sum * (mint)(count - a) / (mint)count;
            ans += (mint)(r[x]) * (mint)(count+1 - a) / (mint)(count + 1);
            cout << ans.val() << endl;
        }
    }
