...
int boo[]={0,1,10,100,1000,10000,100000};
int sta[]={0,2,11,101,1001,10001,100001};
void gao(){
	cl(g2);
	if(tot>1){
		int deg[8]={0};
		rep(i,1,tot+1)deg[i]=1;
		rep(i,1,tot-2 + 1)deg[pru[i]]++;
		rep(i,1,tot-2 + 1){
			int le;
			rep(j,1,tot+1)if(deg[j]==1){
				le=j;
				break;
			}
			g2[min(le,pru[i])][max(le,pru[i])]++;
			deg[pru[i]]--;
			deg[le]--;
		}
		int u,v;
		rep(i,1,tot+1)if(deg[i]==1){
			u=i;break;
		}
		rep(i,1,tot+1)if(deg[i]==1){
			v=i;
	...
	rep(i,1,tot+1)rep(j,i+1,tot+1)if(g2[i][j]>gg[i][j])return;
	...
	int id[8][8];
	int eid[8][8][2];
	rep(i,1,tot+1)rep(j,i,tot+1){
		id[i][j]=++ndtot;
		ae(s,id[i][j],gg[i][j]-g2[i][j]);
		eid[i][j][0]=ae(id[i][j],i,inf);
		eid[i][j][1]=ae(id[i][j],j,inf);
	}
	int su=0;
	rep(i,1,tot+1)ae(i,t,num[i]-1),su+=num[i]-1;
	
	if(su==dinic()){
		rep(i,1,tot+1)rep(j,i+1,tot+1)if(g2[i][j])printf("%d %d\n",boo[i],boo[j]);

		rep(i,1,tot+1)rep(j,i,tot+1){
			for (int c=1;c<=flowof(eid[i][j][0]);c++)printf("%d %d\n",sta[i]++,boo[j]);
			for (int c=1;c<=flowof(eid[i][j][1]);c++)printf("%d %d\n",sta[j]++,boo[i]);
		}
		exit(0);
	}
}
