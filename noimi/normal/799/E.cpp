...
int main() {
    ...
    vi rev(n);
    rep(i, n) rev[id[i]] = i;
    rep(i, n) setter.eb(a[id[i]], 1);
    atcoder::segtree<pll, f, e> seg(setter);
    ...
    vv(int, s, 4);
    rep(i, n) { s[v[i]].eb(i); }
    rep(i, 4) sort(all(s[i]), [&](int i, int j) { return a[i] < a[j]; });

    int p = si(s[1]), q = si(s[2]);
    ll sum = 0;
    rep2(i, 1, 2) for(auto e : s[i]) {
        sum += a[e];
        seg.set(rev[e], pll(0, 0));
    }
    ll mi = inf<ll>;
    rep(i, si(s[3]) + 1) {
        while(p and i + p > k) {
        ...
        if(i + p >= k and i + q >= k and rem >= 0 and i + p + q <= m) {
            int to = seg.max_right(0, [&](pll x) { return x.se <= rem; });
            chmin(mi, sum + seg.prod(0, to).fi);
        }
        if(i < si(s[3])) {
            sum += a[s[3][i]];
            seg.set(rev[s[3][i]], pll(0, 0));
        }
    }
    if(mi == inf<ll>) mi = -1;
    OUT(mi);
