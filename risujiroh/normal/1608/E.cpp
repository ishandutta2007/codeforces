...
template <class T> const auto inf = std::numeric_limits<T>::max() / 2;
...
Z solve_sub(Z h, Z w, const std::vector<Z>& x, const std::vector<Z>& y, const std::vector<Z>& c) {
  auto n = sz(x);
  auto list = std::vector<std::vector<Z>>(h);
  for (auto i : rep(n)) {
    list[x[i]].push_back(i);
  }
  auto lc = std::array<Z, 3>();
  auto rc = std::array<Z, 3>();
  rc.fill(n / 3);
  auto fx = std::vector(7, Fenwick<Z>(h));
  auto fy = std::vector(7, Fenwick<Z>(w));
  auto ret = Z(0);
  for (auto cur_x : rep(h)) {
    for (auto i : list[cur_x]) {
      for (auto mask : rep(7) | IF(_ >> c[i] & 1)) {
        fx[mask].add(x[i], 1);
        fy[mask].add(y[i], 1);
      }
      ++lc[c[i]];
    ...
    for (auto col_r : rep(3)) {
      auto max = Z(0);
      for (auto p : rep(3) | IF(_ != col_r)) {
        auto q = col_r ^ p ^ 3;
        if (lc[p] == 0) {
          continue;
        }
        {
          auto ptx = fx[1 << p | 1 << q].kth(lc[p] - 1);
          chmax(max, std::min(fx[1 << q].sum(ptx), lc[p] - fx[1 << p].sum(ptx)));
          ++ptx;
          chmax(max, std::min(fx[1 << q].sum(ptx), lc[p] - fx[1 << p].sum(ptx)));
        }
        {
          auto pty = fy[1 << p | 1 << q].kth(lc[p] - 1);
          chmax(max, std::min(fy[1 << q].sum(pty), lc[p] - fy[1 << p].sum(pty)));
          ++pty;
          chmax(max, std::min(fy[1 << q].sum(pty), lc[p] - fy[1 << p].sum(pty)));
        }
      }
      chmax(ret, std::min(rc[col_r], max));
    }
  }
  return ret;
}
...
void solve(Z) {
  auto n = input<Z>();
  assert(n % 3 == 0);
  auto x = std::vector<Z>(n);
  auto y = std::vector<Z>(n);
  auto c = std::vector<Z>(n);
  for (auto i : rep(n)) {
    scan(x[i], y[i], c[i]);
    --c[i];
  }
  auto ans = Z(0);
  for ([[maybe_unused]] auto _ : rep(4)) {
    for (auto i : rep(n)) {
      std::tie(x[i], y[i]) = std::pair(y[i], -x[i]);
    }
    auto vx = x;
    ALL(std::sort, vx);
    vx.resize(ALL(std::unique, vx) - vx.begin());
    auto nx = x;
    for (auto& e : nx) e = ALL(std::lower_bound, vx, e) - vx.begin();
    auto vy = y;
    ALL(std::sort, vy);
    vy.resize(ALL(std::unique, vy) - vy.begin());
    auto ny = y;
    for (auto& e : ny) e = ALL(std::lower_bound, vy, e) - vy.begin();
    chmax(ans, solve_sub(sz(vx), sz(vy), nx, ny, c));
  }
  ans *= 3;
  println(ans);
}