#line 1 "codeforces-workspace\\contest\\1558\\c\\c.cpp"
...
#line 16 "codeforces-workspace\\contest\\1558\\c\\c.cpp"
//*/
...
void main() {
  ...
  for (; n > 1; n -= 2) {
    op(next(find(begin(p), end(p), n)));
    auto it = find(begin(p), end(p), n - 1);
    op(it);
    if (p.front() != n) {
      op(next(next(it)));
      op(p.begin() + 3);
