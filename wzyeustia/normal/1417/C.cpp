...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        vector<int> arr(n), last(n+5, -1), lens(n+5, 0);
        REP(i, n) {
            cin >> arr[i];
            chkmax(lens[arr[i]], i-last[arr[i]]);
            last[arr[i]] = i;
        }
        FOR(i, 1, n) {
            chkmax(lens[i], n-last[i]);
        }
        ...
        vector<int> ans;
        int idx = 0, currentMin = 2e9;
        FOR(i, 1, n) {
            while (idx < check.size() && check[idx].first <= i) {
                chkmin(currentMin, check[idx].second);
                idx++;
            }
            if (currentMin == 2e9) ans.eb(-1);
            else  ans.eb(currentMin); 
        }
