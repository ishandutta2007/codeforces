...
int n,m;
struct Player{ int ag; ll cost; };
vc<Player> players;
ll profit[MAXN*2];

ll Solve(){
	static ll dp[MAXN*2][MAXN];
	Msn(dp,-INF);
	/*
	 i  player ag <= j k  ag
	*/
	...
	for( auto [uAg,cost] : players ){
		const ll single = profit[uAg] - cost;
		fORx(have,0,n-1)
			Mymax( dp[uAg][have+1] , dp[uAg][have] + single );

		int lim = n;
		Forx(ag,uAg,n+m+1){
			Forx(have,0,lim+1)
				Mymax( dp[ag+1][have/2] , dp[ag][have] + (have/2)*profit[ag+1] );
			lim /= 2;
			// if(!lim) break;
		}
	}
...
int main(){
	...
	Read(n,m);
	players.resize(n);
	For(i,n) Read(players[i-1].ag);
	For(i,n) Read(players[i-1].cost);
	Read(profit,1,n+m);
	reverse(All(players));
	// sort(All(players),[]( const Player &a , const Player &b ){
	// 	return a.ag < b.ag;
	// });
