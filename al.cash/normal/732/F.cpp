...
int n, m;
int head[N];
int edges[3 * N];  // each triple contains two next indices and vertex xor
    ...
    void dfs(int u) {
        ...
        while (true) {
            if (i < 0) {
                ...
                int subtree = size;
                --stack_size;
                u = stack[stack_size].u;
                i = stack[stack_size].i;
                size = stack[stack_size].size;
                int o = i - i % 3;
                i = edges[i];
                if (head[v] < head[u]) {
                    head[u] = head[v];
                ...
                if (head[v] <= F) {
                    ...
                    i = edges[i];
                    if (stack_size > 0 && v != stack[stack_size - 1].u && head[v] < head[u]) {
                        head[u] = head[v];
                        size &= F;
                    }
                    if ((head[v] >> 29) != 0) {
                        // we can corrupt edge pointers after using it both ways
                        edges[o] = u;
                        edges[o + 1] = v;
                    }
                }
