...
void fill(int am, vt<vt<array<int,2>>> &tr,vi &out, int st, int x){
	int cn = 0;
	for(int i = 0; i < sz(tr[x]); ++i){
		if(out[tr[x][i][1]])continue;
		out[tr[x][i][1]]=((st+cn+1)%am)+1;
		fill(am,tr,out,(st+cn+1)%am,tr[x][i][0]);
		cn++;
	}
}
