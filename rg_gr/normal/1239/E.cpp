...
#endif
#include <bitset>
std::bitset<50005*25>b[51][25];
int a[55];
int L[55],R[55],l=0,r=0;
void dfs(int u,int k,int t)
{
	...
	if(b[u-1][t].test(k))dfs(u-1,k,t),R[++r]=a[u];
	else dfs(u-1,k-a[u],t-1),L[++l]=a[u];
}
signed main()
{
	initprog();
	...
	int mid=s+1>>1;
	// for(int j=0;j<=200;j++)odp(j,(b[4][1]).test(j));
	// odl(mid);
	for(int j=mid;;j++)if(b[n*2][n-1].test(j))
	{
		// odl(j+a[1]+a[2]);
		// return 0;
		dfs(2*n,j,n-1);
