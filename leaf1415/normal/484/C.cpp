...
int main(void)
{
	...
	rep(q, 1, Q){
		...
		ll logn = digitnum(L+5, 2);
		vector<ll> vec, vec2;
		rep(i, 0, k-1) vec.pb(L+i);
		rep(i, 0, d-1) for(int j = i; j < k; j+=d) vec2.pb(vec[j]);
		rep(i, 0, L+n-1) succ[0][i] = i-1;
		rep(i, 0, k-1) succ[0][vec2[i]] = L+i-1;
		succ[0][0] = succ[0][L+n] = L+n;
		rep(i, 1, logn-1) rep(j, 0, L+n) succ[i][j] = succ[i-1][succ[i-1][j]];
