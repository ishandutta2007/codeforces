...
LL n,k,m,tot,a[N],ct[N];
LL len1[N],len2[N],head,tail,L[N];
long long ans;
...
int main()
{
	...
	while(ans)
	{
		...
		{
			ans-=(len1[head]+len2[tail]-(len1[head]+len2[tail])%k)*(m-1);
			break;
		}
		else
		{
			ans-=(len1[head]+len2[tail])*(m-1);
			head=head+len1[head];
			tail=tail-len2[tail];
		}
	}
