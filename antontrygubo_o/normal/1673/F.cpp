...
int main()
{
    ...
    {
        int n = (1<<(i-1));
        for (int j = n-1; j>=0; j--) row.push_back(row[j] + (1<<(2*i-2)));
    }

    for (auto it: row) col.push_back(it*2);

    vector<vector<int>> a(32, vector<int>(32));
    for (int i = 0; i<32; i++)
        for (int j = 0; j<32; j++) a[i][j] = row[i]^col[j];

    int n, k; cin>>n>>k;
    for (int i = 0; i<n; i++)
    {
        for (int j = 0; j<n-1; j++) cout<<(a[i][j]^a[i][j+1])<<' ';
        cout<<endl;
    }

    for (int i = 0; i<n-1; i++)
    {
        for (int j = 0; j<n; j++) cout<<(a[i][j]^a[i+1][j])<<' ';
        cout<<endl;
    }
    ...
    while (k--)
    {
        ...
        for (int i = 0; i<32; i++)
            for (int j = 0; j<32; j++)
            {
                if ((a[curx][cury]^a[i][j])==x)
                {
                    who = mp(i, j);
                }
            }
        cout<<who.first+1<<' '<<who.second+1<<endl;
        curx = who.first;
        cury = who.second;
    }
}