#line 1 "codeforces-workspace\\contest\\1610\\d\\d.cpp"
...
#line 13 "codeforces-workspace\\contest\\1610\\d\\d.cpp"
...
#line 29 "codeforces-workspace\\contest\\1610\\d\\d.cpp"
namespace workspace {
...
void main() {
  ...
  for (auto p = 1; p < 1 << 30; p <<= 1) {
    auto pt = partition(begin(a), end(a), [&](auto x) { return x % (p * 2); });
    auto k = pt - begin(a);
    if (!k) continue;
    if (p > 1) --k;
    ans += (mint(2).pow(k) - 1) * mint(2).pow(end(a) - pt);
    a.erase(begin(a), pt);
  }
