...
int main() {
    ...
    for (auto &e : edges)
        if (!UF.unite(e[0], e[1])) {
            ...
            delta[lca.tour_start[b]]++;
            delta[lca.tour_end[b]]--;
            goal++;

            if (lca.is_ancestor(a, b)) {
                delta[0]++;
                delta[N]--;
                int child = lca.child_ancestor(a, b);
                delta[lca.tour_start[child]]--;
                delta[lca.tour_end[child]]++;
            } else {
                delta[lca.tour_start[a]]++;
                delta[lca.tour_end[a]]--;
            }
        }
