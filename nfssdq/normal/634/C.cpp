/*********************************************************************\
   |--\        ---       /\        |-----------| -----   /-------|    |
   |   \        |       /  \       |               |    /             |
   |    \       |      /    \      |               |   |              |
   |     \      |     /      \     |               |   |----|         |
   |      \     |    / ------ \    |-------|       |        |-----|   |
   |       \    |   /          \   |               |              |   |
   |        \   |  /            \  |               |              /   |
  ---        -------            ------           ----- |---------/    |
                                                                      |
    codeforces = nfssdq  ||  topcoder = nafis007                      |
    mail = nafis_sadique@yahoo.com || nfssdq@gmail.com                |
    IIT,Jahangirnagar University(41)                                  |
                                                                      |
**********************************************************************/

#include <bits/stdc++.h>

using namespace std;

#define xx         first
#define yy         second
#define pb         push_back
#define mp         make_pair
#define LL         long long
#define inf        INT_MAX/3
#define mod        1000000007ll
#define PI         acos(-1.0)
#define linf       (1ll<<60)-1
#define FOR(I,A,B) for(int I = (A); I < (B); ++I)
#define REP(I,N)   FOR(I,0,N)
#define ALL(A)     ((A).begin(), (A).end())
#define set0(ar)   memset(ar,0,sizeof ar)
#define vsort(v)   sort(v.begin(),v.end())
#define setinf(ar) memset(ar,126,sizeof ar)

//cout << fixed << setprecision(20) << p << endl;

template <class T> inline T bigmod(T p,T e,T M){
    LL ret = 1;
    for(; e > 0; e >>= 1){
        if(e & 1) ret = (ret * p) % M;
        p = (p * p) % M;
    } return (T)ret;
}
template <class T> inline T gcd(T a,T b){if(b==0)return a;return gcd(b,a%b);}
template <class T> inline T modinverse(T a,T M){return bigmod(a,M-2,M);}

LL seg[2][800001], a, b;
void insert(int idx, int s, int e, int p, LL v){
    if(s == e){
        seg[0][idx] = min(b, seg[0][idx]+v);
        seg[1][idx] = min(a, seg[1][idx]+v);
        return;
    }
    int mid = (s + e) / 2;
    if(p <= mid) insert(idx * 2 + 1, s, mid, p, v);
    else insert(idx * 2 + 2, mid + 1, e, p, v);
    REP(i, 2) seg[i][idx] = seg[i][idx * 2 + 1] + seg[i][idx * 2 + 2];
}
int query(int f, int idx, int s, int e, int st, int ed){
    if(st > ed) return 0;
    if(s == st && e == ed) return seg[f][idx];
    int mid = (s + e) / 2;
    if(ed <= mid) return query(f, idx * 2 + 1, s, mid, st, ed);
    else if(st > mid) return query(f, idx * 2 + 2, mid + 1, e, st, ed);
    else return query(f, idx * 2 + 1, s, mid, st, mid) +
                query(f, idx * 2 + 2, mid + 1, e, mid + 1, ed);
}

int main() {
    ios_base::sync_with_stdio(0); cin.tie(0);
    int n, k, q; cin >> n >> k >> a >> b >> q;
    while(q--){
        int t; cin >> t;
        if(t == 1){
            int di, ai; cin >> di >> ai;
            insert(0, 1, n, di, ai);
        } else {
            int p; cin >> p;
            cout << query(0, 0, 1, n, 1, p-1) + query(1, 0, 1, n, p+k, n) << "\n";
        }
    }
}