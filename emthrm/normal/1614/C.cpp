...
constexpr int N = 200000, B = 30;
ModInt p2[N + 1]{};

void solve() {
  int n, m; cin >> n >> m;
  vector<int> l(m), r(m), x(m); REP(i, m) cin >> l[i] >> r[i] >> x[i], --l[i], --r[i];
  ModInt ans = 0;
  REP(bit, B) {
    vector<int> is0(n, 0);
    REP(i, m) {
      if (x[i] >> bit & 1) continue;
      ++is0[l[i]];
      if (r[i] + 1 < n) --is0[r[i] + 1];
    }
    FOR(i, 1, n) is0[i] += is0[i - 1];
    const int stand = count(ALL(is0), 0);
    ModInt pat = 0;
    for (int i = 1; i <= stand; i += 2) {
      pat += ModInt::nCk(stand, i);
    }
    ans += pat * p2[n - stand] * (1 << bit);
  }
  cout << ans << '\n';
}
