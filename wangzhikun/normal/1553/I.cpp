...
template<class T> V<T> add(V<T> A, V<T> B) { // only special moduli for NTT
...
vector<vmi> solve(int l,int r){
	vector<vmi> ret;
	if(l == r){
		if(c[l]>1) ret= {{1},{0,2},{1},{0,2}};
		else ret =  {{1},{0,2},{1},{0,1}};
	}else{
		int m = (l+r)/2;
		auto a = solve(l,m),b = solve(m+1,r);
		ret.pb(add(mul(a[0],b[0]),mul(a[1],b[2])));
		ret.pb(add(mul(a[0],b[1]),mul(a[1],b[3])));
		ret.pb(add(mul(a[2],b[0]),mul(a[3],b[2])));
		ret.pb(add(mul(a[2],b[1]),mul(a[3],b[3])));
	}
	//ps(l,r,ret);
	return ret;
}
int main() {
	...
	//ps(c);
	vector<vmi> C = solve(0,sz(c)-1);
	mi ans = 0;
	F0R(i,sz(C[3])){
		ans+=fac(i)*C[3][i]*((sz(c)-i)%2?(-1):1);
	}
	ps(ans.v);
	return 0;
}