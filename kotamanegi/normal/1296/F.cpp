...
void solve() {
    ...
    auto maxs = [](int a, int b) {
        return max(a, b);
    };
    auto maxse = [](int a, int b, int c) {
        return max(a, b);
    };
    auto maxsw = [](int a, int b) {
        return max(a, b);
    };
    LazySegmentTree<int> seg(n + 2LL, maxs, maxse, maxsw, 0LL, 0LL);
    int m;
    cin >> m;
    REP(i, m) {
        int a, b, c;
        cin >> a >> b >> c;
        a--; b--;
        queries.push_back(make_tuple(a, b, c));
        vector<pair<int, int>> geko = Graphs.query(a, b);
        geko.back().first++;
        if (geko.back().first == geko.back().second) geko.pop_back();
        for (auto x : geko) {
            seg.update(x.first, x.second, c);
        }
    }
    auto mins = [](int a, int b) {return min(a, b); };
    SegmentTree<int> checks(2 * n + 2, mins, 1e6);
    REP(i, n) {
        int hoge = seg.query(i, i + 1);
        if (hoge == 0) hoge = 1e6;
        checks.update(i, hoge);
    }
    REP(i, m) {
        int a = get<0>(queries[i]);
        int b = get<1>(queries[i]);
        int c = get<2>(queries[i]);
        vector<pair<int, int>> geko = Graphs.query(a, b);
        geko.back().first++;
        if (geko.back().first == geko.back().second) geko.pop_back();
        int now = 1e9;
        for (auto x : geko) {
            now = min(now, checks.query(x.first, x.second));
        }
        if (now != c) {
            cout << -1 << endl;
            return;
    ...
    REP(i, n - 1) {
        if (Graphs.depth[inputs[i].first] > Graphs.depth[inputs[i].second]) {
            swap(inputs[i].second,inputs[i].first);
        }
        cout << checks[Graphs.zip[inputs[i].first]] << " ";
    }
    cout << endl;
}
