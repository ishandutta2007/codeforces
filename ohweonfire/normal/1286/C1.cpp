...
int main()
{
	...
	construct(cnt1,chk1,v1,n);
	vector<string> v2=query(1,n-1);
	construct(cnt2,chk2,v2,n-1);
...
//	puts("");
	if(n%2==0)
	{
		for(int i=n/2;i>=1;i--)
		{
			...
			memcpy(ans[n-i+1],chk1[i],sizeof(ans[i]));
			for(int j=0;j<26;j++)chk2[i-1][j]-=ans[n-i+1][j];
		}
	}
	else
	{
		memcpy(ans[n/2+1],chk1[n/2+1],sizeof(ans[0]));
		for(int j=0;j<26;j++)chk2[n/2][j]-=ans[n/2+1][j];
		for(int i=n/2;i>=1;i--)
		{
			memcpy(ans[i],chk2[i],sizeof(ans[i]));
			for(int j=0;j<26;j++)chk1[i][j]-=ans[i][j];
