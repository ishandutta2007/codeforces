...
const int N=350;
int  n,m,s,e;
int ans[N],dp[MaxN][N],a[MaxN],b[MaxN];
vi val[MaxN];
void solve(){
    
    cin>>n>>m>>s>>e;   
    For(i,N-1) ans[i+1]=INF;
    for(int i=1;i<=n;++i) cin>>a[i];
    for(int i=1;i<=m;++i) {cin>>b[i];val[b[i]].pb(i);}
    int mxcoins=s/e;
    For(i,n+1) dp[i][0]=0;
    For(i,mxcoins){
        dp[0][i+1]=INF;
    }
    for(int j=1;j<=mxcoins;++j){
        for(int i=1;i<=n;++i){
            if (i<j) dp[i][j]=INF;
            else{
                dp[i][j]=dp[i-1][j];
                int x=dp[i-1][j-1];
                // trace(x,val[a[i]]);
                auto it=upper_bound(all(val[a[i]]),x);
                if (it!=val[a[i]].end()){
                    dp[i][j]=min(dp[i][j],*it);
                }
                ans[j]=min(ans[j],dp[i][j]+i+j*e);
            }
            // trace(i,j,dp[i][j]);
        }
    }
