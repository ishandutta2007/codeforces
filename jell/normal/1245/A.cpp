...
namespace math
{
    ...
    {
        std::vector<int_t> res(__builtin_ctzll(x), 2);
        x >>= __builtin_ctzll(x);
        res.reserve(64);
        for(int_t p = 3; p * p <= x; p += 2)
        {
            while(!(x % p))
                x /= p, res.emplace_back(p);
        }
    ...
    template <class int_t>
    constexpr std::pair<int_t, int_t> modulo_composition(int_t k, int_t m,
                                                         int_t l, int_t n)
    {
        assert(m > 0 and n > 0);
        int_t g, x, y;
...
void __solve()
{
    const string infi="Infinite";
    const string fi="Finite";
    i32 a,b; cin>>a>>b;
    if(gcd(a,b)>1)
    {
        cout << infi << "\n";
