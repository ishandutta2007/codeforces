...
void main_() {
  ...
  auto p = md_vec(n, n, mint()); // p[i][j]: i defeats j
  for (const usize i : rep(0, n)) {
    for (const usize j : rep(0, n)) {
      p[i][j] = mint(a[i]) / (a[i] + a[j]);
    }
  }

  auto dp = md_vec(1 << n, mint());

  const auto out = [&](const usize s, const usize t) -> mint {
    std::vector<usize> ts, ss;
    for (const usize i : rep(0, n)) {
      if (t >> i & 1) {
        ts.push_back(i);
      } else if (s >> i & 1) {
    ...
    mint ret = 1;
    for (const usize i : ts) {
      for (const usize j : ss) {
        ret *= p[i][j];
      }
    }
    return ret;
  ...
  for (const usize s : rep(1, 1 << n)) {
    dp[s] = 1;
    for (const usize t : subsets(s)) {
      if (t == 0 || t == s) {
        continue;
      }
      dp[s] -= dp[t] * out(s, t);
  ...
  mint ans = 0;
  for (const usize s : rep(1, 1 << n)) {
    ans += popcount32(s) * dp[s] * out((1 << n) - 1, s);
  }
