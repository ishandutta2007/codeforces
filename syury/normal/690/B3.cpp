...
struct pt{
    ...
    ll dot(const pt & p, const pt & q)const{
...
void solve(){
    ...
    for(int i = 0; i < m; i++){
        ...
        s[i] = {x, y};
        nonz.insert(s[i]);
        rnonz.emplace(s[i].Y, s[i].X);
    }
    auto isZero = [&n, &nonz, &rnonz](int i, int j){
        if(i < 0 || j < 0 || i >= n || j >= n)
            return true;
        else{
            if(nonz.count(make_pair(i, j)))
                return false;
            auto it = nonz.lower_bound(make_pair(i, j));
            if(it == nonz.end() || it->X != i)return true;
            if(it == nonz.begin() || (--it)->X != i)return true;
            swap(i, j);
            it = rnonz.lower_bound(make_pair(i, j));
            if(it == rnonz.end() || it->X != i)return true;
            if(it == rnonz.begin() || (--it)->X != i)return true;
            return false;
        }
    };
    vector<int> ds = {-1, 1};
    for(auto& cell: s){
        ...
        for(int dx: ds){
            for(int dy: ds){
                ...
                /*else if(isZero(i - dx, j - dy) && isZero(i - dx, j) && isZero(i, j - dy) && !isZero(i + dx, j) && !isZero(i, j + dy)){
                    cand.emplace_back(i + max(0, dx), j + max(0, dy));
                }*/
            }
        }
