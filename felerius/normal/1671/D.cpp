...
int main() {
    ...
    while (t--) {
        ...
        auto [mn_it, mx_it] = minmax_element(all(a));
        int mn = *mn_it, mx = *mx_it;
        ll ans = INFL;

        array mn_cost = {a[0] - 1, a.back() - 1, 2 * (mn - 1)};
        array mx_cost = {abs(a[0] - x), abs(a.back() - x), 2 * max(0, x - mx)};
        rep(i, 3) rep(j, 3)
            if (i != j || i == 2)
                ans = min(ans, base + mn_cost[i] + mx_cost[j]);
