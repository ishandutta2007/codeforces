...
namespace Graph{
	...
	struct Edge{
		int to; int cost;
		Edge(){}
		Edge( int ti , int ci ){ to = ti; cost = ci; }
	};
}
#define USEPBDS
#ifdef USEPBDS
...
int main(){
	...
	For(i,n-1) lnk[Mp(a[1][i],a[1][i+1])] = 1;

	For(i,m)
		For(j,n-1)
			edge[i][j] = lnk.count(Mp(a[i][j],a[i][j+1]));
	For(i,m)
		fOR(j,n)
			dis[i][j] = edge[i][j] ? dis[i][j+1]+1 : 1;
	For(i,m)
		For(j,n)
			posi[i][a[i][j]] = j;

 // For(i,m) Print(dis[i],1,n);
