...
template <typename T> WeightedGraph<T> make2(int n, int m, int offset = 1) {
...
main() {
    ...
    auto g = make(n, m);
    auto id = iota(n);
    rep(i, n) g[i].eb(i), sort(all(g[i]));
    sort(all(id), [&](int x, int y) { return g[x] < g[y]; });
    UnionFind uf(n);
    rep(i, n - 1) {
        if(g[id[i]] == g[id[i + 1]]) uf.unite(id[i], id[i + 1]);
    }
    int N = 0;
    rep(i, n) if(uf.find(i) == i) N++;
    cout << endl;
    vector<set<int>> G(n);
    rep(i, n) {
        for(auto e : g[i]) {
            if(!uf.same(i, e) and !G[uf.find(i)].count(uf.find(e))) {
                G[uf.find(i)].emplace(uf.find(e));
                G[uf.find(e)].emplace(uf.find(i));
            }
        }
    }
    rep(i, n) {
    ...
    vi ans(n);
    auto dfs = [&](auto &&f, int x, int p, int k) -> void {
        ans[x] = k;
        for(auto e : G[x]) {
            if(e != p) f(f, e, x, k + 1);
        }
    ...
    if(N == 1) {
        ...
        rep(i, n) {
            if(G[uf.find(i)].size() == 1) {
                dfs(dfs, uf.find(i), -1, 1);
                break;
            }
        }
    }
