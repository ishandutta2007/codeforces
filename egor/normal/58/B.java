...
public class Task58B {
	...
	private static List<Test> createTests() {
		...
		tests.add(new Test("10", "10 5 1"));
		tests.add(new Test("4", "4 2 1"));
		tests.add(new Test("3", "3 1"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
...
//		return tokenCheck(result, output);
		...
		if (resultTokens.length != outputTokens.length)
			return resultTokens.length + " tokens received but " + outputTokens.length + "expected";
		if (!resultTokens[0].equals(input))
			return "first coin must be " + input + " but " + resultTokens[0] + " received";
		try {
			for (int i = 1; i < resultTokens.length; i++) {
				int previousCoin = Integer.parseInt(resultTokens[i - 1]);
				int currentCoin = Integer.parseInt(resultTokens[i]);
				if (previousCoin % currentCoin != 0)
					return "coin  " + previousCoin + " not divisible by " + currentCoin;
			}
		} catch (NumberFormatException e) {
			return "not an integer in result " + result;
		}
		return null;
	}
	...
	private static void test() {
		...
		for (Test test : tests) {
			...
			try {
				new Task58B(in, new PrintWriter(out)).run();
	...
	private Task58B(InputReader in, PrintWriter out) {
		this.in = in;
		this.out = out;
		testMode = true;
	}
