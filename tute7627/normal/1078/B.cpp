...
int main(){
  ...
  rep(i,0,n)cin>>a[i];
  ll sz=10005;
  auto dp=make_v(sz,n+1,modint(0));
  vector<ll>cnt(105);
  dp[0][0]=1;
  rep(i,0,n){
    cnt[a[i]]++;
    rrep(o,0,n){
      rrep(j,0,sz-a[i])dp[j+a[i]][o+1]+=dp[j][o];
    }
  }
  Comb comb(200);
  //debug(cnt,30);
  ...
  rep(i,0,105){
    rep(j,1,cnt[i]+1){
      //cout<<comb.C(cnt[i],j) spa dp[i*j][j] spa i spa j<<endl;
      if(comb.C(cnt[i],j)==dp[i*j][j]){
        chmax(res,j);
        if(j==cnt[i]){
          rep(o,0,105)if(o!=j&&cnt[o]+j==n)chmax(res,n);
        }
      }
    }
  }
