...
int32_t main() {
	...
	h=n/2;
	for(int mk=0;mk<(1<<h);mk++) {
		int z=0;
		for(int i=0;i<h;i++) if(mk & (1<<i)) add(z,a[i]);
		L[Ln++]=z;
	}
	for(int mk=0;mk<(1<<(n-h));mk++) {
		int z=0;
		for(int i=0;i<n-h;i++) if(mk & (1<<i)) add(z,a[h+i]);
		R[Rn++]=z;
	}

	int Z=0;
	sort(L,L+Ln); sort(R,R+Rn);
	assert(!L[0] && !R[0]);
	// dbg(vector<int>(L,L+Ln));
	// dbg(vector<int>(R,R+Rn));
	for(int i=0;i<Ln;i++) {
		int x=L[i];
		int j=int(upper_bound(R,R+Rn,m-1-x)-R)-1;
		assert(j>=0);
		Z=max(Z,R[j]+x);
		add(x,R[Rn-1]);
		Z=max(Z,x);
