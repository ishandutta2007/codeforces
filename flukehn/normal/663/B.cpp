...
const int N=1e3+11,M=1e3;
int fff[10];
char s[N],t2[N],t1[N],*f,*g;
int n,y,q;
void make_b(char *a,char *b,int n){
	int p=M-n+1;
	if(a[p]>b[p]){
		return;	
	}
	for(++a[--p];a[p]==10;)a[p]=0,++a[--p];
	upmn(y,p);
}
void Solve(){
//debug("ok")
