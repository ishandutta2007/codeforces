...
void build(ll v,ll tl,ll tr,ll st[],ll lz[],bool f[],string &fi)
{
    if(tl==tr)
    {
        st[v]=fi[tl]-'0';
    ...
    build((v<<1),tl,((tl+tr)>>1),st,lz,f,fi);
    build((v<<1)|1,((tl+tr)>>1)+1,tr,st,lz,f,fi);
    //operation
...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        build(1,0,n-1,st,lz,f,fi);

        bool bad=false;
        REPI(i,0,q)
        {
            ll t=query(1,0,n-1,b[i].F-1,b[i].S-1,st,lz,f);

            if(2*t==(b[i].S-b[i].F+1))
            {
                bad=true;
            }
            else if(2*t<(b[i].S-b[i].F+1))
            {
                update(1,0,n-1,b[i].F-1,b[i].S-1,0,true,st,lz,f);
            }
            else
            {
                update(1,0,n-1,b[i].F-1,b[i].S-1,1,true,st,lz,f);
            }
        }
        ...
        if(!bad)
        {
            REP(i,0,n)
            {
                if(query(1,0,n-1,i,i,st,lz,f)!=s[i]-'0') bad=true;
            }
        }
        if(bad) cout<<"NO";
        else cout<<"YES";
