#line 1 "codeforces-workspace/contest/1519/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1519/c/c.cpp"
...
void main() {
  ...
  for (auto &&x : s) {
    cin >> x.second;
    u[x.first].emplace_back(x.second);
  }
  ...
  for (auto k : range(1, n + 1)) {
    while (!u.empty() && u.back().size() <= k) u.pop_back();
    i64 ans = 0;
    for (auto &&v : u) {
      ans += v[(v.size() - 1) / k * k];
    }
