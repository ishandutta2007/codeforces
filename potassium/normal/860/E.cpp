...
int main() {
	...
	const int MAXJ = 20;
	vector<vector<int>> jump(MAXJ + 1, vector<int>(n + 1));
	...
	vector<ll> ans(n + 1, 0);
	for (int d = 0; d <= n; d++) {
		if (dep[d].empty()) break;

		for (auto& u : dep[d])
			ans[u] += d + 1;

		vector<int> l((int)dep[d].size() - 1);
		for (int i = 0; i + 1 < (int)dep[d].size(); i++)
			l[i] = lca(dep[d][i], dep[d][i + 1]);

		for (int round = 0; round < 2; round++) {
			ll sum = 0;
			vector<pair<int, int>> stk;
			stk.reserve(dep[d].size());
			for (int i = 0, u = dep[d][0]; i < (int)dep[d].size(); i++, u = dep[d][i]) {
				if (i != 0) {
					int cnt = 0;
					while (!stk.empty() && depth[stk.back().first] > depth[l[i - 1]]) {
						sum -= 1LL * (depth[stk.back().first] + 1) * stk.back().second;
						cnt += stk.back().second;
						stk.pop_back();
					}

					if (!stk.empty() && stk.back().first == l[i - 1])
						stk.back().second += cnt;
					else
						stk.push_back({ l[i - 1], cnt });
					sum += 1LL * (depth[l[i - 1]] + 1) * cnt;
			...
			reverse(dep[d].begin(), dep[d].end());
			reverse(l.begin(), l.end());
		}
	}

	function<void(int, int)> dfs_ans = [&](int u, int p) {
		if (p != -1)
			ans[u] += ans[p];
		for (auto& v : g[u])
			dfs_ans(v, u);
