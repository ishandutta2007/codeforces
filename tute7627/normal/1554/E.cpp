...
int gcd(int x,int y){int r;while(y!=0&&(r=x%y)!=0){x=y;y=r;}return y==0?x:y;}
...
int main(){
  ...
  while(t--){
    ...
    auto div=divisor_enum(n-1);
    vector<modint>ret(n);
    ret[0]=modint(2).pow(n-1);
    for(auto d:div){
      if(d==1)continue;
      int gc=0;
      bool sw=true;
      auto dfs=[&](auto &&f,int k,int par)->int{
        int sum=0;
        for(auto to:g[k]){
          if(to==par)continue;
          int tmp=f(f,to,k);
        ...
        //cout<<"out" spa d spa k spa sum<<endl;
        return sum;
      };
