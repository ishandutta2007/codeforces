...
inline void AC() {
    ...
    rd(a, n);
    if(n == 3) {
        if(count(a+1, a+1+n, a[1]+a[2]+a[3])) yes(7);
        else no(7);
    } else {
        bool ok = false;
        if(count(a+1, a+1+n, 0) >= n-1) ok = true;
        if(count(a+1, a+1+n, 0) == n-2) {
            sort(a+1, a+1+n);
            if(a[1] == -a[n]) ok = true;
        }
        if(n <= 50) {
            ok = true;
            for(_ i = 1; i <= n; ++i) for(_ j = i+1; j <= n; ++j) for(_ k = j+1; k <= n; ++k) {
                if(!count(a+1, a+1+n, a[i]+a[j]+a[k])) ok = false;
            }
        }
        if(ok) yes(7);
        else no(7);
