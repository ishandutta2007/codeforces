...
ll get(ll p)
{
	if(res[p] != -1) return res[p];
	
	vector<P> vec;
	rep(x, 1, 200) rep(y, 1, 200){
		if(x % (1<<p) == 0) vec.push_back(P(x, y));
	}
	
	outl("?", sz(vec));
	reps(i, vec) cout << vec[i].first << " " << vec[i].second << " ";
	cout << endl;
...
int main(void)
{
	...
	while(ub-lb>1){
		mid = (ub+lb)/2;
		if(get(0) == (get(mid) << mid)) lb = mid;
		else ub = mid;
	}
	
	ll h = abs(2*get(lb+1) - (get(0)>>lb)) - 1;
	ll w = get(0) / (h+1) - 1;
