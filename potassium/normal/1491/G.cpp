...
int main() {
	...
	for (int u = 1; u <= n; u++) {
		...
		do {
			trav[h] = true;
			cycles.back().emplace_back(h);
			h = a[h];
		} while (h != u);
	}

	vector<pii> ans;
	vector<bool> side(n + 1, true);
	...
	auto kill_with_isolated = [&](vector<int>& c, int u) {
		const int len = (int)c.size();
		assert(len >= 2);
 
		add(c[0], u);
		c.insert(c.begin() + 1, u);
		kill_01down(c);
	};

	auto join = [&](vector<int>& c1, vector<int>& c2) {
		const int len1 = (int)c1.size();
		const int len2 = (int)c2.size();
	...
	if ((int)cycles.size() == 1) {
		...
		vector<int> tmpC;
		while (!cycles.empty()) {
			if ((int)cycles.back().size() == 1) {
				isolated = cycles.back()[0];
			}
			else if (!has) {
				has = true;
				tmpC = cycles.back();
		...
		if (has) {
			assert(isolated != -1);
			kill_with_isolated(tmpC, isolated);
		}
	}
