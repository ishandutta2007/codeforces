...
//val n= 100000
//val x = 79847
//val all = BooleanArray(n+1){it > 0 }
//
//fun fakeask(type:Char, k:Int):Int{
//    if(type == 'A'){
//
//    }
//}
fun main(){
    ...
    solve.cases{
        ...
        var qs = 0
        fun ask(type:Char, k:Int):Int {
            qs ++
            if(qs > 10000) {
                println("QUERY EXCEEDED")
            }
            assert( qs <= 10000)
            println("$type $k")
            System.out.flush()
        ...
        val ourcopy = BooleanArray(n+1){true}
        ourcopy[0] = false
        var surviving = n
        fun crossout(v:Int){
            for(i in v..n step v) {
        ...
        var step = 0
        val backtrack = mint
        var largefactor:Int? = null

        fun confirm(){
            if(largefactor != null ) return
            val all = ask('A',1 )
            if(all == surviving){
                backtrack.clear()
                return
                ...
                for(p in backtrack) {
                    ...
                    if(test == 1) {
                        if(largefactor != null )
                            println("MORE THAN ONE LARGE FACTOR")
                        largefactor = p
                    }
                }
                backtrack.clear()
        ...
        for(pi in 0 until sieve.primeCounter - 1) {
            val p = sieve.primes[pi]
            if(p <= cut) continue
            if(p > n) break
            ask('B',p)
        ...
        for(pi in 0 until sieve.primeCounter - 1) {
            ...
            val again = ask('B',p)
            if(again != 0) {
                smallprimefactors.add(p)
            }
        }
        var ret = largefactor ?: 1
        for(s in smallprimefactors){
            var max = 1
            var now = s
            while(now <= n) {
                val test = ask('A',now)
                if(test !in 0..1){
                    println("BAD $now $test FINAL QUERY")
                    crash()
                }
                if(test == 1){
                    max = now
...
have sol but no clean pivot

32 minutes if working

again forgetting to confirm again (literally said it to myself , and the reason why I hvae to code it as seperate function)

 sometimes crashing give WA not RE
 */