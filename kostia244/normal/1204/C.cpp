//resubmission to check it any counter tests were added
...
int main() {
	...
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++)
			g[i][j] = (tg[i][j] == '0' ? (i != j)*2000000ll : 1);
...
//		for (int j = 1; j <= n; j++)cout << g[i][j]<<" ";cout << "\n";}
	...
	for(int i = 2; i < p.size(); i++) {
		int pi = 2;
		if(ans.size() > 1 && g[ans[ans.size()-2]][ans.back()]+g[ans.back()][p[i]] <= g[ans[ans.size()-2]][p[i]]) {
			ans.pop_back();
			pi++;
		}
		ans.pb(p[i]);
