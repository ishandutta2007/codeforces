...
int a[nax];
bool Try(const vpii &v) {
	debug(imie(v));
	ll sum = 0;
	vector <pair <ll, int> > sums = {{0, 0}};
	int n = v.size();
	REP(i, n) {
		sum += v[i].first;
	...
	REP(i, n) {
		if (sums[i].first == sums[i + 1].first) {
			int l = sums[i].second, r = sums[i + 1].second;
			printf("%d\n", r - l);
			for (int j = l; j < r; ++j) {
				out += a[v[j].second];
				printf("%d ", v[j].second);
...
const int bsiz = 501;
bool has(const bitset<bsiz> &b, int v) {
	v += bsiz / 2;
	if (v >= 0 && v < bsiz) return b[v];
	return 0;
}
bitset<bsiz> dp[nax];
bool BitsetTry(const vpii &v) {
	const int zero = bsiz / 2;
	dp[0][zero] = 1;
	int n = v.size();
	REP(i, n) {
		if (has(dp[i], -v[i].first)) {
			vector <int> ans = {v[i].second};
			int where = zero - v[i].first;
			FORD(j, i - 1, 0) {
				if (!dp[j][where]) {
			...
			printf("\n");
			debug("bitset worked", imie(ans));
			assert(out == 0);
			return true;
		}
		if (v[i].first > 0)
			dp[i + 1] = dp[i] | (dp[i] << v[i].first);
		else
			dp[i + 1] = dp[i] | (dp[i] >> -v[i].first);
	}
	debug(imie(v), "not found");
	return false;
}
int main() {
	int t;
	...
	REP(_, t) {
		...
		sort(ALL(in));
		if (Try(in)) continue;
		sort(ALL(in), [](pii a, pii b){return abs(a.first) < abs(b.first);});
		if (BitsetTry(in)) continue;
		if (Try(in)) continue;
		vpii pos, neg;
		for (pii x : in)
			if (x.first > 0)
				...
				neg.push_back(x);
		{
			set <pii> negs(ALL(neg)), poses(ALL(pos));
			ll sum = 0;
			vector <pii> ord;
			while (!negs.empty() && !poses.empty()) {
				if (sum >= 0) {
					auto it = negs.lower_bound(pii(max<ll>(-sum, -inf), -inf));
					if (it == negs.end()) it--;
					sum += it->first;
					ord.push_back(*it);
					negs.erase(it);
					...
					auto it = poses.lower_bound(pii(min<ll>(-sum, inf), inf));
					if (it != poses.begin()) it--;
					sum += it->first;
					ord.push_back(*it);
					poses.erase(it);
				}
			}
			ord.insert(ord.end(), ALL(negs));
			ord.insert(ord.end(), ALL(poses));
			debug("try greedy", imie(ord));
			if (Try(ord)) continue;
		}
		{
			set <pii> negs(ALL(neg)), poses(ALL(pos));
		...
		while (true) {
			...
			while (ip < (int) pos.size()) {
				ord[ip + in] = pos[ip];
				ip++;
			}
			while (in < (int) neg.size()) {
				ord[ip + in] = neg[in];
				in++;
			}
			debug(imie(ord));
