...
public class D {
	...
	public class Edge {
		...
		public Edge(int to, int x, int y) {
			this.to = to;
			this.x = x;
			this.y = y;
		}
	...
	public int dfs(int v, int prev, int value) {
		int res = value;
		for (Edge e : to[v]) {
			if (e.to == prev) {
				continue;
			...
			while (y > 1) {
				prime[lp[y]]--;
				if (prime[lp[y]] < min[lp[y]]) {
					min[lp[y]] = prime[lp[y]];
				}
				y /= lp[y];
			}
			res = sum(res, dfs(e.to, v, value1));
			x = e.x;
			y = e.y;
			while (x > 1) {
				prime[lp[x]]--;
	...
	public void solve() {
		...
		for (int qwerty = in.nextInt(); qwerty > 0; --qwerty) {
			...
			for (int i = 0; i < n - 1; i++) {
				...
				int x = in.nextInt();
				int y = in.nextInt();
				to[u].add(new Edge(v, y, x));
				to[v].add(new Edge(u, x, y));
			}
			int res = dfs(0, -1, 1);
			for (int i = 0; i < min.length; i++) {
				if (min[i] < 0) {
					res = mult(res, pow(i, -min[i]));
				}
			}
			out.println(res);
		}
