...
int main() {
    ...
    for (ll x = 19; x >= 0; x--) {
        ...
        rep(j, N) {
            if (S[j] < (1ull << x)) { lis.push_back(S[j]); }
        }
        ...
        for (ll v : lis) {
            ...
            if (v > 0) { basis.push_back({ now,v }); }
        }

       // for (auto p : basis) { cout << p.first << " " << p.second << endl; }
        ...
        rep(i, x) {
            vll tmp = op;
            op.push_back(basis[i].first);
            op.insert(op.end(), tmp.begin(), tmp.end());
        }
