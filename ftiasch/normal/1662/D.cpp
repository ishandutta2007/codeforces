...
#line 2 "/tmp/tmp-4448YIAt3r6ERVNY.cpp"

#line 4 "/tmp/tmp-4448YIAt3r6ERVNY.cpp"

std::vector<char> canonicalize(const std::string &s) {
  int b = 0;
  std::vector<char> result;
  for (char c : s) {
    if (c == 'B') {
      b ^= 1;
    } else if (!result.empty() && result.back() == c) {
      result.pop_back();
    } else {
      result.push_back(c);
    }
...
int main() {
  ...
  int T = read();
  while (T--) {
    auto a = canonicalize(read<std::string>());
    auto b = canonicalize(read<std::string>());
    std::cout << (a == b ? "YES" : "NO") << "\n";
  }
}