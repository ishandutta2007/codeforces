...
main()
{
    ...
    for (int i = 0; i < n; i++)
        ...
        {
            if (i == j) continue;
            if (v[i] - v[j] >= 0) vab[v[i] - v[j]]++;
        }
    ccd = cab * cab;
    for (int i = 0; i < 6000; i++)
        for (int j = 0; j < 6000; j++)
        {
            int delta = vab[i] * vab[j];
            vcd[i + j] += delta;
        }
    cef = cab * cab * cab;
    for (int i = 0; i < 6000; i++)
...
/*
    cout << cab << endl;
    for (int i = 0; i <= 20; i++) cout << vab[i] << " "; cout << endl;

    cout << ccd << endl;
    for (int i = 0; i <= 20; i++) cout << vcd[i] << " "; cout << endl;

    cout << vef << " " << cef;
*/
    cout << (long double) vef / cef;
