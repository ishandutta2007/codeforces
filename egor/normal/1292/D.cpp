...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        auto p = primes(5000);
        auto d = divisorTable(5001);
        arr<vi> divisors(5001);
        divisors[0] = vi();
        divisors[1] = vi();
        for (int i : range(2, 5001)) {
            ...
            if (d[i] == i) {
                ...
                for (int q = i; q > 1; q /= d[q]) {
                    divisors[i][lower_bound(all(p), d[q]) - p.begin()]++;
                }
            }
        }
        ll answer = 0;
        ...
        while (true) {
            ...
            for (int i : range(5001)) {
                if (still[i] && !divisors[i].empty()) {
                    cur[divisors[i].size() - 1] += q[i];
                }
            }
            ...
            for (int i : range(5001)) {
                if (still[i] && divisors[i].size() - 1 != pos) {
                    answer += q[i] * accumulate(all(divisors[i]), 0ll);
                    lost += q[i];
                    still[i] = false;
                } else if (still[i]) {
                    minim(step, divisors[i].back());
                }
            }
            if (pos == -2) {
                break;
            ...
            for (int i : range(5001)) {
                if (still[i]) {
                    divisors[i].back() -= step;
                    if (divisors[i].back() == 0) {
                        divisors[i].pop_back();
                    }
                }
            }
