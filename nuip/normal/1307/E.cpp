...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n)rep(j,n+10) sums[i][j+1]+=sums[i][j];
	int mx=0;
	mint re=0;
	{//no L
		re=1;
		rep(i,n)if(sums[i][cnt[i]]){
			++mx;
			re*=sums[i][cnt[i]];
		}
		out(mx,re,1);
	...
	vector<int> fcnt(n);
	rep(i,n){
		if(ex[a[i]][fcnt[a[i]]+1]){
		int num=1;
		mint way=1;
		{
			int r=cnt[a[i]]-1-fcnt[a[i]];
			int x=sums[a[i]][r];
			if(r>=fcnt[a[i]]+1) --x;
			out(i,r,x,1);
			if(x){
		...
		rep(j,n)if(j!=a[i]){
			...
			int b=cnt[j]-a;
			if(a>b) swap(a,b);
			ll tmp=sums[j][a]*(sums[j][b]-1ll);
			if(tmp>0){
				way*=tmp%MOD;
				num+=2;
			}else if(sums[j][b]){
				way*=sums[j][b]+sums[j][a];
				num+=1;
			}
		}
		if(mx<num){
