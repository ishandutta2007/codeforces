...
void solve(int node, int par, int inT) {
//	W(node, par, inT);
	ans.pb({node, inT});
...
//	W("node", node, g[node]);
	...
	int canContainChild = max(inT - 1, 0);
	int childCount = g[node].size();
	int diff = childCount - canContainChild;

//	W(canContainChild, childCount, diff);

	int curT = inT;
	if (diff >= 0) {
		for (int i = 0; i < diff; i++) {
//			W(node, "called", g[node][i]);
			solve(g[node][i], node, ++curT);
			ans.pb({node, curT});
		}
		ans.pb({node, 0});
		curT = 0;
		for (int i = diff; i < g[node].size(); i++) {
//			W(node, "called", g[node][i]);
			solve(g[node][i], node, ++curT);
			ans.pb({node, curT});
		}
