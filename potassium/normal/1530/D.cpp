...
int main() {
	CASE(t) {
		...
		vector<bool> taken_fr(n, false), taken_to(n, false);
		vector<int> ans(n, -1);
		for (int u = 0; u < n; u++) {
			if (!taken_to[a[u]]) {
				taken_fr[u] = true;
		...
		vector<int> dump_fr, dump_to;
		for (int u = 0; u < n; u++) {
			if (!taken_fr[u]) dump_fr.emplace_back(u);
			if (!taken_to[u]) dump_to.emplace_back(u);
		}

		assert(dump_fr.size() == dump_to.size());
		const int sz = dump_fr.size();

		if (sz == 1 && dump_fr[0] == dump_to[0]) {
			for (int u = 0; u < n; u++)
				if (a[u] == a[dump_fr[0]] && taken_fr[u]) {
					taken_fr[u] = false;
					taken_fr[dump_fr[0]] = true;
					ans[dump_fr[0]] = a[dump_fr[0]];
					dump_fr[0] = u;
					break;
				}			
		}
		...
		sort(all(dump_fr));
		while (true) {
			shuffle(all(dump_to), rng);
			bool can = true;
			for (int i = 0; i < sz; i++)
				if (dump_fr[i] == dump_to[i])
					can = false;

			if (!can) continue;

			for (int i = 0; i < sz; i++)
				ans[dump_fr[i]] = dump_to[i];
			break;
		}
