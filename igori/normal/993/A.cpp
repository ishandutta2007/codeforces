...
void solve()
{
    ...
    forn(i, 4) cin >> a[i].first >> a[i].second, a[i].first += 100, a[i].second += 100;
    forn(i, 4) cin >> b[i].first >> b[i].second, b[i].first += 100, b[i].second += 100;
    sort(all(a));
    sort(all(b));
    const int N = 201;
    vector<vector<int> > f(N, vector<int>(N));

    int dn = b[0].second, up = b[0].second;
    for (int l = b[0].first; l <= b[1].first; l++)
    {
        for (int i = dn; i <= up; i++)
            f[l][i] = 1;
        dn--;
    ...
    dn = up = b[0].second;
    for (int l = b[3].first; l >= b[1].first; l--)
    {
        for (int i = dn; i <= up; i++)
            f[l][i] = 1;
        dn--;
    ...
    {
        for (int j = a[0].second; j <= a[1].second; j++)
        {
            if (f[i][j])
            {
                cout << "YES\n";
