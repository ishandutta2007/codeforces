#line 1 "codeforces-workspace\\contest\\1672\\d\\d.cpp"
...
#line 13 "codeforces-workspace\\contest\\1672\\d\\d.cpp"
...
#line 2 "Library\\src\\string\\rle.hpp"
...
#line 9 "Library\\src\\string\\rle.hpp"
...
#line 29 "codeforces-workspace\\contest\\1672\\d\\d.cpp"
...
void main() {
  ...
  input n;
  input<vector<i32>> a(n), b(n);
  for (auto &&e : a) --e;
  for (auto &&e : b) --e;

  auto ra = run_length_encoding(begin(a), end(a));
  auto rb = run_length_encoding(begin(b), end(b));

  vector<i32> use(n);
  for (auto [c, k] : reversed(ra)) {
    if (not rb.empty() and rb.back().first == c) {
      auto l = rb.back().second;
      rb.pop_back();
      use[c] += l - 1;
      k -= 1;
    }
