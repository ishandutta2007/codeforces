...
int n;
int f[100007],t[100007],len[100007];//Road
int m;
...
int preDFS(int now,int lst)
{
	fa[now]=lst;
	hvson[now]=1;
	for(pii i:nei[now])
		if(i.first!=lst)
			hvson[now]+=preDFS(i.first,now);
	return hvson[now];
}
...
int main()
{
	...
	{
		fio>>f[i]>>t[i]>>len[i];
		nei[f[i]].pb(mp(t[i],len[i]));
		nei[t[i]].pb(mp(f[i],len[i]));
	}
	preDFS(1,-1);
	for(int i=0;i<n-1;i++)
	{
		if(fa[t[i]]!=f[i])swap(f[i],t[i]);
		jl[i]=3.00*hvson[t[i]]*(n-hvson[t[i]])/n/(n-1)*2;
		ans+=jl[i]*len[i];
	}
	fio>>m;
	while(m--)
