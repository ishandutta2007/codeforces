...
P dfs(int v)
{
	P ret = P(0, trie.vec[v].num), res0, res1;
	if(trie.vec[v].next[0] != -1) res0 = dfs(trie.vec[v].next[0]);
	if(trie.vec[v].next[1] != -1) res1 = dfs(trie.vec[v].next[1]) + P(1, 0);
	chmax(ret, res0), chmax(ret, res1);
	
	if(trie.vec[v].next[0] != -1 && trie.vec[v].next[1] != -1){
		chmax(ans, make_pair(res0.first+res1.first, P(res0.second, res1.second)));
	}
	return ret;
}
...
int main(void)
{
	...
	dfs(0);
	if(ans.first == 0) ans = make_pair(0, P(1, 2));
	outl(ans.second.first, ans.second.second, ans.first);
