pub mod solution {
//{"name":"F. ","group":"Codeforces - Educational Codeforces Round 131 (  Div. 2)","url":"https://codeforces.com/contest/1701/problem/F","interactive":false,"timeLimit":6500,"tests":[{"input":"7 5\n8 5 3 2 1 5 6\n","output":"0\n0\n1\n2\n5\n1\n5\n"}],"testType":"single","input":{"type":"stdin","fileName":null,"pattern":null},"output":{"type":"stdout","fileName":null,"pattern":null},"languages":{"java":{"taskClass":"FTochki"}}}

use std::ops::Not;

use crate::algo_lib::collections::sorted::SortedTrait;
...
fn solve_case(d: i32, queries: Vec<i32>) -> Vec<i64> {
    ...
    let queries: Vec<usize> = queries
        ...
        .collect();
    let mut alive = vec![0i32; coords.len()];
    let mut cnt = vec![0; coords.len()];
    ...
    for query in queries.into_iter() {
        ...
        if alive[query] != 0 {
            res += cc * (cc - 1);
            res += unsafe { calc_res(&alive[from[query]..query], &cnt[from[query]..query]) } * 2;
            unsafe {
                add_const(&mut cnt[from[query]..query], 1);
            }
        } else {
            res -= cc * (cc - 1);
            unsafe {
