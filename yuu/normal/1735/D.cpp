...
// A pair of card uniquely define the third card if they are in a set.
// Which mean a set cannot share more than 1 card with any other set, otherwise they would be identical
// A metaset only has 5 cards, so 1 card is shared, the others form 2 pairs of pairwise distinct card.
// Which mean we can count the number of meta set based on the shared card
// -> For a card, find the number of set it is a part of.

int n, k;
int c[1001][21];
int check[21];
int64_t cnt[1001];
int64_t encode(int* c) {
    int64_t res = 0;
    for (int i = 1; i <= k; i++) (res *= 3) += c[i];
    return res;
}

void get_third_card(const int* a, const int* b, int* c) {
    for (int i = 1; i <= k; i++) {
        if (a[i] == b[i]) c[i] = a[i];
        else c[i] = 3 ^ a[i] ^ b[i];
    }
...
void solve() {
    cin >> n >> k;
    for (int i = 1; i <= n; i++) c[i] | ARRAY | 1 | k | ARRAY_INPUT;
    for (int i = 1; i <= n; i++) {
        for (int j = i + 1; j <= n; j++) {
            get_third_card(c[i], c[j], check);
            auto it = id.find(encode(check));
            if (it != id.end()) {
                cnt[i]++;
                cnt[j]++;
                cnt[it->second]++;
