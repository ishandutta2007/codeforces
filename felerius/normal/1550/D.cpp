// Excellent Arrays (https://codeforces.com/contest/1550/problem/D)
...
Combinatorics<MI7, 200'000> comb;
...
int main() {
    ...
    while (t--) {
        int n, l, r; cin >> n >> l >> r;
        int arbit = min(r - n, 1 - l);
        MI7 ans = arbit * (n % 2 ? 2 : 1) * comb.binom(n, n / 2);

        for (int i = arbit + 1; ; ++i) {
            int below_only = max(0, n - (r - i));
            int above_only = max(0, l + i - 1);
            int both = n - below_only - above_only;

            if (n % 2) {
                // more below
                int both_below_1 = (n + 1) / 2 - below_only;
                ans += comb.binom(both, both_below_1);

                // more above
                int both_below_2 = n / 2 - below_only;
                ans += comb.binom(both, both_below_2);
            } else {
                int both_below = n / 2 - below_only;
                ans += comb.binom(both, both_below);
            }
