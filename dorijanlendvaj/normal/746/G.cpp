...
int main()
{
	...
	int ma=1;
	for (int i=1;i<=m;++i) ma+=max(0,h[i]-h[i-1]);
	if (k<ma)
	{
		cout<<-1;
		exit(0);
	...
	for (int i=1;i<=m;++i)
	{
		int x=h[i]-h[i-1];
		int y=h[i];
		for (int j=0;y>0 && j<x;--y,++j)
		{
			tre.push_back({no[i-1][0],id});
			no[i].push_back(id);
			++id;
		}
		if (no[i-1].size()>1)
		...
		for (int j=0;y>0 && j<no[i-1].size();++j,--y)
		{
			tre.push_back({no[i-1][j],id});
			no[i].push_back(id);
			++id;
		}
		//cout<<i<<' '<<ha<<endl;
	...
	cout<<n<<en;
	for (auto a: tre) cout<<a.first<<' '<<a.second<<en;
}