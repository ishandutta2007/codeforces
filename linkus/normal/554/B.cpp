...
void rec(int cur, string arr[], bool gut[], int gutcnt){
    if(cur == n)
        mx = max(mx,gutcnt);
    if(gutcnt <= mx)
        return;
    bool oldgut[n];
    memcpy(oldgut,gut,n);
    int oldgutcnt = gutcnt;
    FOR(i,0,n){
        if(arr[i][cur] == '0' && gut[i])
            gut[i] = 0, --gutcnt;
    }
    rec(cur+1, arr, gut, gutcnt);
    FOR(i,0,n){
        if((arr[i][cur]^1) == '0' && oldgut[i])
            oldgut[i] = 0, --oldgutcnt;
    }
    rec(cur+1, arr, oldgut, oldgutcnt);
}
