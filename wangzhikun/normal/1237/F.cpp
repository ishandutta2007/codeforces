...
int ocuh[4040] = {0},ocuw[4040] = {0};

mi dp1[4040][2040],dp2[4040][2040];
...
int main() {
	...
	F0R(i,h){
		...
		F0R(j,(h+2)/2){
			dp1[i+1][j]+=dp1[i][j];
			if(!ocuh[i] && !ocuh[i+1]) dp1[i+2][j+1]+=dp1[i][j];
		}
	}
	F0R(i,w){
		if(!ocuw[i])ew+=1;
		F0R(j,(w+2)/2){
			dp2[i+1][j]+=dp2[i][j];
			if(!ocuw[i] && !ocuw[i+1]) dp2[i+2][j+1]+=dp2[i][j];
		}
	}
	mi ans = 0;
	F0R(i,h/2+1){
		F0R(j,w/2+1){
			mi c1 = dp1[h][i]*C(eh-i*2,j);
			mi c2 = dp2[w][j]*C(ew-j*2,i);
			ans+=c1*c2*fac(i)*fac(j);
		}
	}
	ps(ans.v);
	// END OF CODE
