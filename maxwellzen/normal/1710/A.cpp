...
void solve() {
    ...
    read(a, k);
    ll sum1=0, sum2=0;
    ll extra1 = 0, extra2 = 0;
    F0R(i, k) {
        if (a[i]/n >= 2) sum1 += a[i]/n, extra1 += 2;
        if (a[i]/m >= 2) sum2 += a[i]/m, extra2 += 2;
    }
    if (sum1>=m && (m%2==0 || extra1<sum1)) cout << "Yes\n";
    else if (sum2>=n && (n%2==0 || extra2<sum2)) cout << "Yes\n";
    else cout << "No\n";
}
