...
int cal(){
	...
	for (int i=-1; i<n; ++i){
		...
		for (int j=i+1; j<n; ++j){
			if (pre[j] != i) continue;
			if (t % a[j]) return intmax;
			t /= a[j];
		}
	}
	for (int i=0; i<n; ++i){
		if (pre[i] == -1) ans += cnt[i];
		if (cnt[i] == 1) ans --;
	}
	int root = 0;
	for (int i=0; i<n; ++i) if (pre[i] == -1) root ++;
	if (root >= 2) ans ++;
	//printf("%d \n", ans);
	return ans + n;
}
...
int dfs(int s){
	...
	int ans = intmax;
	for (int i=-1; i<s; ++i){
		if (i != -1 && a[i] % a[s] != 0) continue;
		pre[s] = i;
		checkmin(ans, dfs(s+1));
	}
	return ans;
}
...
int main(){
	...
	for (int i=0; i<n; ++i) cnt[i] = fac(a[i]);
	int ans = 0;
	ans = dfs(0);
	cout << ans << endl;
	return 0;
