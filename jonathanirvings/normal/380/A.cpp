...
int solve(LL x)
{
	if (x <= n && risan[x] > -1) return risan[x];
	int L = 1;
	int R = m;
	int res = -1;
	while (L <= R)
	...
	int ret = 0;
	if (query[res] == 1) ret = query1[res];
	else
	{
		int tmp = (x - ada[res-1]) % query2[res].fi;
		if (tmp == 0) tmp = query2[res].fi;
		ret = solve(tmp);
	}
	if (x <= n) risan[x] = ret;
	return ret;
...
int main()
{
	...
	REPN(i,m)
	{
		...
		{
			scanf("%d %d",&query2[i].fi,&query2[i].se);
			ada[i] = ada[i-1] + (LL)query2[i].se * query2[i].fi;
		}
	}
	MIN(n,ada[m]);
	scanf("%d",&t);
