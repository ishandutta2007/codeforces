...
int main(){
  ...
  while(t--){
    ...
    SegmentTree<ll,decltype(ff)>val(n,ff,-LLONG_MAX);
    vector<vector<P>>g(n+1);
    seg.set(0,0);
    g[1].EB(0,1);
    seg.build();
    rep(i,0,n)val.set(i,a[i]);
    val.build();
    rep(i,0,n){
      for(auto z:g[i]){
        if(seg[z.fi]<z.se)seg.update(z.fi,z.se);
      }
      ll mx=seg.query(0,a[i]+1);
      ll nxt=-1;
      if(i<n-1)nxt=val.find_first(i+1,[&](ll x){return x>=a[i];});
      if(nxt!=-1)g[nxt+1].EB(a[i],mx+2);
      seg.update(a[i],mx+1);
      //seg.print();
    }
    cout<<seg.seg[1]<<endl;
