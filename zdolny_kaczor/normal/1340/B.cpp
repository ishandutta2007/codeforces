...
#endif
//#STAY AT HOME
string strs[10] = {"1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011"};
int nums[10];
const int nax = 2100;
bool dp[nax][nax];
int convert(string x) {
	int ans = 0;
	for (char c : x) ans = ans << 1 ^ (c - '0');
	rans;
}
int in[nax];
int main() {
	REP(i, 10) nums[i] = convert(strs[i]);
	int n, k;
	scanf("%d%d", &n, &k);
	REP(i, n) {
		char row[10];
	...
	dp[n][0] = 1;
	for (int i = n - 1; i >= 0; --i) {
		REP(d, 10) if ((nums[d] & in[i]) == in[i]) {
			int v = popc(in[i] ^ nums[d]);
			for (int c = 0; c <= k; ++c) if (dp[i + 1][c]) dp[i][c + v] = 1;
		}
	}
	if (!dp[0][k]) done("-1\n");
	int left = k;
	...
	for (int i = 0; i < n; ++i) {
		bool found = false;
		for (int d = 9; d >= 0; --d) if ((nums[d] & in[i]) == in[i]) {
			int v = popc(in[i] ^ nums[d]);
			if (v <= left && dp[i + 1][left - v]) {
				ans << d;
				left -= v;
				found = true;
				break;
