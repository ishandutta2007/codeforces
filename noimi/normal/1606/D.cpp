...
int main() {
    TEST {
        ...
        vi A;
        rep(i, n) A.eb(a[i][0]);
        int b = min_element(all(A)) - begin(A);
        ...
        sat.IF(b, r + n);
        {
            ...
            rep(i, n) { s.emplace(*min_element(begin(a[i]), begin(a[i]) + t), *max_element(begin(a[i]), begin(a[i]) + t), i); }
            int R = -inf<int>;
            while(si(s) > 1) {
                auto [l, r, i] = *begin(s);
                chmax(R, r);
                s.erase(begin(s));
                auto [l2, r2, i2] = *begin(s);
                if(R >= l2) {
                    sat.IF(i, i2);
                    sat.IF(i2, i);
                }
        ...
        {
            set<tuple<int, int, int>> s;
            rep(i, n) { s.emplace(*min_element(begin(a[i]) + t, begin(a[i]) + m), *max_element(begin(a[i]) + t, begin(a[i]) + m), i); }
            int R = -inf<int>;
            while(si(s) > 1) {
                auto [l, r, i] = *begin(s);
                chmax(R, r);
        ...
        if(empty(col))
...

            {
                ...
                if(rmin <= bmax) flag = false;
            }

            {
                int rmax = -inf<int>, bmin = inf<int>;
                rep(i, n) rep2(j, t, m - 1) {
                    if(col[i])
                        chmin(bmin, a[i][j]);
                    else
                        chmax(rmax, a[i][j]);
                }
                if(bmin <= rmax) flag = false;
            }

            if(flag) {
                assert(inc(t, 1, m) and SUM(col) and SUM(col) < n);
                YES();
                rep(i, n) cout << "RB"[col[i]];
                cout << " " << t << endl;
            } else {
                NO();
            }
