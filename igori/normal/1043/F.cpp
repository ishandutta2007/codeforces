...
ll n;
ll a;
ll cnt[400000], cnt2[400000];
ll dp[10][400000];
ll fact[400000], antifact[400000];

ll C(ll n, ll k)
{
    if (k > n || n < 0 || k < 0)
        re 0;
    re fact[n] * antifact[n - k] % MOD * antifact[k] % MOD;
}
...
signed main()
{
    ...
    antifact[mx] = inq(fact[mx], MOD - 2, MOD);
    for (int i = mx - 1; i >= 0; i--)
    {
        antifact[i] = antifact[i + 1] * (i + 1) % MOD;
    }
    for (int i = 1; i <= 7; i++)
    {
        for (int j = mx; j >= 1; j--)
        {
            dp[i][j] = C(cnt[j], i) % MOD;
