...
void solve() {
  ...
  */
  vi CNT_edge(1 << N);
  vv(ll, OK_LCA, N, 1 << N);
  FOR(s, 1 << N) FOR(a, N) FOR(b, a) {
    if ((s & 1 << a) && (s & 1 << b)) CNT_edge[s] += G[a][b];
  }

  FOR(s, 1 << N) FOR(r, N) if (s & 1 << r) {
    auto& ok = OK_LCA[r][s];
    ok = 1;
    for (auto&& [a, b, c]: LCA) {
      int cnt = (s >> a & 1) + (s >> b & 1);
  ...
  auto can = [&](ll s1, ll r1, ll s2, ll r2) -> bool {
    // , 
    // 
    ...
    // 
    ll between = CNT_edge[s1 | s2] - CNT_edge[s1] - CNT_edge[s2];
    if (between != G[r1][r2]) return false;
    // LCA 
    return OK_LCA[r2][s2];
  };
  ...
  FOR(s, 1 << N) {
    ...
    FOR_subset(t, s) {
      ll s1 = t, s2 = s - t;
      FOR(r1, N) if (DP[r1][s1]) FOR(r2, N) if (DP[r2][s2]) {
        if ((s1 - (1 << r1)) > s2) continue;
        if (can(s1, r1, s2, r2)) DP[r1][s] += DP[r1][s1] * DP[r2][s2];
      }
    }
  }
