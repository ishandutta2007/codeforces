...
bool solve() {
	...
	if (n == 1) return true;
	if (n == 2) return false;
	if (popc(n) == 1) return true;
	if (n % 2 == 0 && is_prime(n / 2)) return true;
	return false;
}
void bru() {
	const int N = 200;
	vi won(N);
	for (int i = 1; i < N; ++i) {
		for (int j = 3; j < N; j += 2) if (i % j == 0 && !won[i / j])
			won[i] = true;
		if (i > 1 && !won[i - 1]) won[i] = true;
		if (!won[i]) debug(imie(i), "loses");
	}
}
int main() {
	...
	int t = 1;
	scanf("%d", &t);
	REP(_, t) puts(solve() ? "FastestFinger" : "Ashishgup");
	exit(0);
}
//#STAY AT HOME