...
int main(){
	scanf("%d%d",&n,&k);int cur=k,e=1;
	while(cur<n) cur*=k,e++;
	printf("%d\n",e);
	for(int i=1,c=1;i<cur;i*=k,c++){
		for(int j=1;j<n;j+=i*k){
			vector<vector<int> > gr;
			for(int l=j,t=1;t<=k&&l<=n;t++,l+=i){
				vector<int> tmp;
				for(int o=l;o<=min(l+i-1,n);o++) tmp.pb(o);
				gr.pb(tmp);
			}
			for(int x=0;x<gr.size();x++) for(int y=x+1;y<gr.size();y++){
				for(int p1:gr[x]) for(int p2:gr[y]) a[p1][p2]=c;
			}
		}
	}
	for(int i=1;i<=n;i++) for(int j=i+1;j<=n;j++) printf("%d ",a[i][j]);
	return 0;
}