...
int main() {
    ...
    srand(time(0));
    re(n); 
    int lo = 0, hi = 1000000000, query = 0;
    // lo = hi = 24;
    while (lo < hi) {
        int mid = (lo+hi)/2;
        cout << "> " << mid << endl;
    ...
    vi ind; FOR(i,1,n+1) ind.pb(i);
    F0R(i,sz(ind)) swap(ind[i],ind[ran()%(i+1)]);
    int z = 0;
    vi v = {hi};
    while (query < 60 && z < sz(ind)) {
        query ++;
        cout << "? " << ind[z] << endl;
        // int t = res[ind[z]-1]; 
        int t; re(t); 
    ...
    int d = 0;
    sort(all(v));
    FOR(i,1,sz(v)) d = __gcd(d,v[i]-v[i-1]);
    // if (d == 88) cout << "! " << lo << " " << hi << endl;
