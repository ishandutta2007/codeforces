...
int main() {
    ...
    F0R (i, sz(extrema) - 1) {
        ...
        legs.pb(abs(extrema[i + 1] - extrema[i]));
    }
    legs.pb(LLONG_MAX / 2);

    int M = max(0, sz(extrema) / 2 - K);

    while (M) {
        vector<pair<ll, int>> v(sz(legs));
        F0R (i, sz(v)) v[i] = mp(legs[i], i);
        ...
        bool big = false;
        F0R (i, sz(legs)) {
            if (mp(legs[i], i) < v[M] && LEGS.back() > legs[i] && legs[i] <= legs[i + 1]) { // should merge this
                m++;
                profit -= legs[i];
                LEGS.back() += -legs[i] + legs[i + 1];
                i++;
                big = false;
            } else if (3 * M < sz(v) && mp(legs[i], i) >= v[3 * M]) { // will never merge this
                if (big) {
                    LEGS.back() += legs[i];
                } else {
                    LEGS.pb(legs[i]);
