...
struct CHT{
	...
	{
		ll a1 = p1.first, b1 = p1.second;
		ll a2 = p2.first, b2 = p2.second;
		ll a3 = p3.first, b3 = p3.second;
...
vector<ll> vec[10005], vec2[100005];
ll ans[100005];
CHT cht[355], cht2[355];
...
int main(void)
{
	...
	rep(i, 0, 10000){
		for(auto qid : vec[i]){
			ll l = y[qid]-x[qid]+1, r = y[qid], z = a[y[qid]];
			
			ll L = l/B+1, R = r/B, mn = inf;
			if(L > R) rep(j, l, r) chmin(mn, p[j]*z + q[j]);
			else{
				rep(j, l, L*B-1) chmin(mn, p[j]*z + q[j]);
				rep(j, R*B, r) chmin(mn, p[j]*z + q[j]);
				rep(j, L, R-1) chmin(mn, cht[j].query(z));
			}
			ans[qid] = mn + s[y[qid]-1] + (x[qid]-y[qid])*z;
		}
	}
	
	vector<P> lvec;
	rep(i, 1, n){
		p[i] = a[i], q[i] = (i-1)*a[i]-s[i-1];
		lvec.pb(P(a[i], i));
	}
	sort(all(lvec)), reverse(all(lvec));
	reps(i, lvec){
		ll id = lvec[i].se;
		cht2[id/B].add(p[id], q[id]);
	}
	
	per(i, n, 0){
		for(auto qid : vec2[i]){
			ll l = y[qid]-x[qid]+1, r = y[qid], z = x[qid]-y[qid];
			...
			if(L > R) rep(j, l, r) chmin(mn, p[j]*z + q[j]);
				...
				rep(j, l, L*B-1) chmin(mn, p[j]*z + q[j]);
				rep(j, R*B, r) chmin(mn, p[j]*z + q[j]);
				rep(j, L, R-1) chmin(mn, cht2[j].query(z));
			}
			chmin(ans[qid], mn + s[y[qid]]);
		}
	}
