#line 1 "codeforces-workspace\\contest\\1161\\a\\a.cpp"
...
#line 11 "codeforces-workspace\\contest\\1161\\a\\a.cpp"
...
void main() {
  ...
  vector<i32> rpos(n, -1);
  for (auto &&[i, x] : enumerate(pos)) {
    rpos[x] = i;
  }

  vector<i32> lpos(n, k);
  for (auto &&[i, x] : reversed(enumerate(pos))) {
    lpos[x] = i;
  }
  ...
  partial_sum(begin(lat), end(lat), lat.begin());
