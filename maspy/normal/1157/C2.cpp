...
signed main() {
  ...
  auto solve = [&](auto self, ll L, ll R, ll nxt) -> string {
    string S;
    if (L == R) return S;
    auto vl = A[L], vr = A[R - 1];
    if (vl >= nxt) {
      if (vl <= vr || vr < nxt) {
        S = self(self, L + 1, R, vl + 1) + 'L';
      }
    }
    if (vr >= nxt) {
      if (vr <= vl || vl < nxt) {
        auto T = self(self, L, R - 1, vr + 1) + 'R';
        if (S.size() < T.size()) swap(S, T);  // S = T;
      }
    }
    // print(L, R, nxt, S);
    return S;
