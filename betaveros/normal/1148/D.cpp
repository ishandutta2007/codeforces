...
typedef struct UpPair {
	int i;
	int a;
	bool operator<(const UpPair & other) const {
		return a > other.a;
	}
} UpPair;

typedef struct DownPair {
	int i;
	int b;
	bool operator<(const DownPair & other) const {
		return b < other.b;
	}
} DownPair;

vector<UpPair> ups;
vector<DownPair> downs;
...
int main() {
	...
	fori (i, 0, n) {
		...
		scan_dd(a, b);
		if (a < b) {
			ups.push_back((UpPair) { .i = i + 1, .a = a });
		} else {
			downs.push_back((DownPair) { .i = i + 1, .b = b });
		}
	}
