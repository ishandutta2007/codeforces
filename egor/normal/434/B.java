...
class TaskB {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		int[][] left = new int[columnCount][rowCount];
		int[][] right = new int[columnCount][rowCount];
		int[][] up = new int[rowCount][columnCount];
		int[][] down = new int[rowCount][columnCount];
		for (int i = 0; i < rowCount; i++)
			updateRow(i, table, left, right, columnCount);
		for (int i = 0; i < columnCount; i++)
			updateColumn(i, table, up, down, rowCount);
		int[] rowOrder = ArrayUtils.createOrder(rowCount);
		int[] columnOrder = ArrayUtils.createOrder(columnCount);
		for (int i = 0; i < queryCount; i++) {
			...
			if (type == 1) {
				table[row][column] ^= 1;
				updateRow(row, table, left, right, columnCount);
				updateColumn(column, table, up, down, rowCount);
			} else {
				int result = 0;
				result = Math.max(result, calculate(left[column], row, rowOrder, rowCount));
				result = Math.max(result, calculate(right[column], row, rowOrder, rowCount));
				result = Math.max(result, calculate(up[row], column, columnOrder, columnCount));
				result = Math.max(result, calculate(down[row], column, columnOrder, columnCount));
				out.printLine(result);
			}
		}
    }

	private int calculate(int[] value, int at, int[] order, int count) {
		int left = 0;
		int right = count - 1;
		...
		for (int i : order) {
			if (i < left || i > right)
				continue;
			answer = Math.max(answer, value[i] * (right - left + 1));
			if (at == i)
				return answer;
			if (at < i)
				right = i - 1;
	...
	private void updateColumn(int column, int[][] table, int[][] up, int[][] down, int rowCount) {
		up[0][column] = table[0][column];
		for (int i = 1; i < rowCount; i++)
			up[i][column] = (1 + up[i - 1][column]) * table[i][column];
		down[rowCount - 1][column] = table[rowCount - 1][column];
		for (int i = rowCount - 2; i >= 0; i--)
			down[i][column] = (1 + down[i + 1][column]) * table[i][column];
	}

	private void updateRow(int row, int[][] table, int[][] left, int[][] right, int columnCount) {
		left[0][row] = table[row][0];
		for (int i = 1; i < columnCount; i++)
			left[i][row] = (1 + left[i - 1][row]) * table[row][i];
		right[columnCount - 1][row] = table[row][columnCount - 1];
		for (int i = columnCount - 2; i >= 0; i--)
			right[i][row] = (1 + right[i + 1][row]) * table[row][i];
	}
}
