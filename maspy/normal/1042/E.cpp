...
void solve() {
  ...
  vi X;
  X.reserve(H * W);
  FOR(i, H) FOR(j, W) X.eb(A[i][j]);
  UNIQUE(X);
  FOR(i, H) FOR(j, W) A[i][j] = LB(X, A[i][j]);
  if (A[x0][y0] == 0) return print(0);
  vvc<pi> pos(len(X));
  FOR(i, H) FOR(j, W) pos[A[i][j]].eb(i, j);

  ll N = len(X);
  mint sm = 0;
  mint dp0 = 0, dpx = 0, dpy = 0, dpxx = 0, dpyy = 0;
  ...
  FOR(k, len(X)) {
    ...
    if (k > 0) {
      mint cf = dp0.inverse();
      for (auto&& [x, y]: pos[k]) {
        mint v = mint(x * x + y * y) * dp0;
        v -= mint(2 * x) * dpx + mint(2 * y) * dpy;
        v += dpxx + dpyy;
        v += sm;
        v *= cf;
        if (x == x0 && y == y0) { return print(v); }
        add += v;
      }
    }
    sm += add;
