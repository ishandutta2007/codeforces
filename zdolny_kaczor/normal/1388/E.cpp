...
void solve() {
	...
	REP(_, n) {
		int x1, x2, y;
		scanf("%d%d%d", &x1, &x2, &y);
		ord.emplace_back(x1, y);
		ord.emplace_back(x2, y);
		seg.emplace_back(x1, x2, y);
	}
	set_pre(10);
	...
	REP(i, n) REP(j, i) {
		...
		auto [l2, r2, y2] = seg[j];
		if (y1 != y2) {
			ld v1 = (r1 - l2) / (ld) (y2 - y1);
			ld v2 = (r2 - l1) / (ld) (y1 - y2);
			if (v1 > v2) swap(v1, v2);
	...
	vpii top, bot;
	for (pii x : ord) {
		while (top.size() >= 2u && vec(top.back(), x, *(top.rbegin() + 1)) >= 0) top.pop_back();
		while (bot.size() >= 2u && vec(bot.back(), x, *(bot.rbegin() + 1)) <= 0) bot.pop_back();
		top.push_back(x);
		bot.push_back(x);
	}
	...
	debug(imie(top));
	int im = -1, iM = -1;
	const int l = top.size();
	auto proj = [&](ld x, pii a) {
	...
	auto q = [&](ld x) {
		if (im == -1) {
			assert(iM == -1);
			im = iM = 0;
			pair <ld, int> M = {-numeric_limits<ld>::infinity(), -1}, m = {numeric_limits<ld>::infinity(), -1};
			REP(i, l) {
				ld v = proj(x, top[i]);
				mini(m, make_pair(v, i));
				maxi(M, make_pair(v, i));
			}
			im = m.second;
			iM = M.second;
			ld ret = M.first - m.first;
			...
			assert(iM != -1);
			for (int dir : {-1, 1}) {
				auto go = [&](int &i, ld mul) {
					while (true) {
						int to = (i + dir + l) % l;
						ld here = proj(x, top[i]) * mul, there = proj(x, top[to]) * mul;
						if (there > here) i = to;
						else break;
					}
				};
			...
			ld ret = proj(x, top[iM]) - proj(x, top[im]);
			debug(imie(x), imie(ret));
			return ret;
		}
	};
	...
	if (c.empty()) {
		...
		mini(ans, q(c[0].first));
		for (auto [v, x] : c) {
			int b = in;
			in += x;
			if (in == 0 || b == 0) mini(ans, q(v));
		}
	}
	printf("%.10Lf\n", ans);
}
int main() {
	debug(fun(numeric_limits<ld>::epsilon));
