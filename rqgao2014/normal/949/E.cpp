...
inline vi dfs(int x,vi a){
	...
	vi L(0),R(0);
	for(int i:a)
		if(i&1){if(i^1) L.pb((i-1)>>1);} else L.pb(i>>1);
	L.erase(unique(all(L)),L.end());
	if(!SZ(L)) return (vi){1<<x};
	
	for(int i:a)
		if(i&1){if(i^(-1)) R.pb((i+1)>>1);} else R.pb(i>>1);
	R.erase(unique(all(R)),R.end());
	if(!SZ(R)) return (vi){-(1<<x)};
	
	L=dfs(x+1,L);L.pb(1<<x);
	R=dfs(x+1,R);R.pb(-(1<<x));
	
	if(SZ(L)<SZ(R)) return L; else return R;
}
