...
void solve() {
    ...
    for (int i = n-1; i >= 0; i--) {
        // dbg(i, cnt);
        int z = a[i];
        vector<pair<int,int>> new_cnt = {{z, 1}};
        for (auto pp: cnt) {
            int q = pp.first;
            LL how_many = pp.second;
            LL k = (z+q-1)/q;
            res += how_many*(k-1)*(i+1);

            int new_z = z/k;
            if (new_cnt.back().first != new_z) new_cnt.push_back({new_z, 0});
            new_cnt.back().second += how_many;
        }
        cnt = new_cnt;
    }
    cout << res << "\n";
