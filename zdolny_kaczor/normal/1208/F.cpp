...
#define log __l0g
const int nax = 1e6 + 44, vax = (1 << 21), log = 21;
pii suf[vax];
int pref[vax];
void ins(pii &a, int v) {
	if (v == a.first || v == a.second) return;
	if (v > a.first) {
		a.second = a.first;
		a.first = v;
	}
	else if (v > a.second) a.second = v;
}
int a[nax];
int main() {
	debug(imie(vax));
	...
	REP(i, vax) pref[i] = nax;
	REP(i, n)
		mini(pref[a[i]], i);
	REP(i, n)
		ins(suf[a[i]], i);
	REP(m, vax) if (pref[m] != nax) debug(arr(pref, m));
	REP(m, vax) if (suf[m] != pii(-1, -1)) debug(arr(suf, m));
	REP(b, log) FORD(m, vax - 1, 0) if (m >> b & 1)
		mini(pref[m ^ (1 << b)], pref[m]);
	REP(b, log) FORD(m, vax - 1, 0) if (m >> b & 1) {
		ins(suf[m ^ (1 << b)], suf[m].first);
		ins(suf[m ^ (1 << b)], suf[m].second);
	}
	REP(m, vax) if (pref[m] != nax) debug(arr(pref, m));
	REP(m, vax) if (suf[m] != pii(-1, -1)) debug(arr(suf, m));
	int ans = 0;
	FORD(b, log - 1, 0) {
		int tried = ans ^ (1 << b);
		bool can = false;
		for (int left = tried; ; left = (left - 1) & tried) {
			int right = left ^ tried;
			if (pref[left] < suf[right].second) {
				can = true;
				break;
			}
			if (!left) break;
