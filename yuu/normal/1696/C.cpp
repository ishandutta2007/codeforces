...
void expand(int a) {
    ...
    if ((x.empty()) || (x.back().first != a)) x.emplace_back(a, weight);
    else x.back().second += weight;
}
...
bool contract(int a) {
    ...
    if (x[0].first != a) return false;
    x[0].second -= weight;
    if (x[0].second < 0) return false;
    if (x[0].second == 0) x.pop_front();
    return true;
}
void solve() {
    cin >> n >> m;
    ...
    for (int i = 1, b; i <= k; i++) {
        cin >> b;
        if (good) good = contract(b);
    }
    if (!x.empty()) good = false;
    if (good) cout << "Yes\n";
    else cout << "No\n";
}
