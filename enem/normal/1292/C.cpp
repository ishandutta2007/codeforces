...
static ll n;
static V(ll) ad[3005];
static ll dp2[3005][3005],dp[3005],dpp[3005],pr[3005];
static pll dp3[3005][3005];

ll tt(ll x,ll y)
{
    if(y==pr[x]) return dp[x];
    else return dpp[y];
}
...
void dfs(ll x,ll y,ll px,ll x0)
{
    dp3[x0][x]=MP(px,y);
    for(auto it:ad[x])
    {
...
ll xd(ll x,ll y)
{
    ...
    ll px=dp3[x][y].F,py=dp3[x][y].S;
    return dp2[x][y]=tt(x,px)*tt(y,py)+max(xd(px,y),xd(x,py));
}
