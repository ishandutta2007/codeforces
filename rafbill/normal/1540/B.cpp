...
int main(){
  ...
  for(li i = (1); i <= (li)(n); ++i) pre[0][i] = 1;
  for(li i = (1); i <= (li)(n); ++i) pre[i][0] = 0;
  for(li i = (1); i <= (li)(n); ++i) for(li j = (1); j <= (li)(n); ++j) {
    pre[i][j] = (pre[i-1][j] + pre[i][j-1])/ZN(2);
  }
  ...
  for(li i = 0; i < (li)(n); ++i) {
    vi sz(n);
    auto dfs0 = letrec([&](auto dfs0, int j, int p) -> void {
      sz[j]=1;
    ...
    vi C;
    C.reserve(n);
    auto dfs = letrec([&](auto dfs, int j, int p) -> void {
      if(j < i) {
        ZN lans=0;
        int d = C.size();
        for(li k = 0; k < (li)(d); ++k) {
