...
Binomial<mint> BINOM;
const int MAX_K = 12;
mint calc[1 << (MAX_K + 1)][MAX_K + 1][MAX_K + 1]
         [MAX_K + 1],  //  mask  i  j  k 
    cnt[2 * MAX_K + 1][MAX_K + 1][MAX_K + 1],  //  i  j  k 
    dp[MAX_K + 1][2 * MAX_K + 1][MAX_K + 1]
      [MAX_K + 1];  // i  j  k  l 

void precalc() {
    for (int len = 2; len <= MAX_K + 1; len++) {
        ...
        for (int i = 1; i < len; i++) calc[1 << i][i][0][0]++;
        for (int mask = 0; mask < (1 << len); mask++) {
            int f = popcount(mask);
            if (mask == MSK(f)) continue;
            for (int i = 0; i <= MAX_K; i++) {
                for (int j = 0; j <= MAX_K; j++) {
                    for (int k = 0; k <= MAX_K; k++) {
                        mint val = calc[mask][i][j][k];
                        if (val == 0) continue;
                        for (int l = 0; l < len; l++) {
                            if (mask >> l & 1) continue;
                            int nmask = mask | 1 << l, nj = j, nk = k + (i > l);
                            for (int m = l + 1; m < len; m++) nj += (mask >> m & 1);
                            if (nj > MAX_K or nk > MAX_K) continue;
                            calc[nmask][l][nj][nk] += val;
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < len; i++) {
            for (int j = 0; j <= MAX_K; j++) {
                for (int k = 0; k <= MAX_K; k++) {
                    cnt[len][j][k] += calc[MSK(len)][i][j][k];
                }
            }
        }
    }
        ...
        for (int j = 0; j <= 2 * MAX_K; j++) {
            for (int k = 0; k <= MAX_K; k++) {
                for (int l = 0; l <= MAX_K; l++) {
                    mint val = dp[i][j][k][l];
                    if (val == 0) continue;
                    for (int p = 1; j + p <= 2 * MAX_K; p++) {
                        for (int q = 1; k + q <= MAX_K; q++) {
                            for (int r = 1; l + r <= MAX_K; r++) {
                                if (cnt[p][q][r] == 0) continue;
                                dp[i + 1][j + p][k + q][l + r] += val * cnt[p][q][r];
                            }
                        }
                    }
                }
