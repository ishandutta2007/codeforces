...
class H1SoedinenieNityamiProstayaVersiya {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        ...
        mod = MODF;
        Combinations c(any + 1);
        modint answer = 0;
        modint ways = 0;
        for (int i : range(any + 1)) {
            int cOpen = open + i;
            int cClose = close + any - i;
            int delta = abs(cOpen - cClose);
            if (delta % 4 != 0) {
                continue;
            }
            answer += delta / 4 * c(any, i);
            ways += c(any, i);
        }
...
int main() {
    ...
    auto time = clock();
#endif
    H1SoedinenieNityamiProstayaVersiya solver;
