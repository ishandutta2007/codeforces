...
void solve() {
  ...
  // first, last -> max length
  // last 
  using T = vc<vc<ll>>;
  auto mul = [&](T A, T B) -> T {
    vv(ll, C, K, K);
    FOR(a, K) FOR3(b, a, K) FOR3(c, b, K) chmax(C[a][c], A[a][b] + B[b][c]);
    return C;
  };

  auto single = [&](ll a) -> T {
    vv(ll, A, K, K);
    FOR(b, K) if (a <= b) A[a][b] = 1;
    return A;
  };

  vv(ll, dp, K, K);
  FORIN(a, A) dp = mul(dp, single(a));

  vv(ll, ANS, K, K);
  while (p) {
    if (p & 1) ANS = mul(ANS, dp);
    dp = mul(dp, dp);
    p >>= 1;
  }
  ll x = 0;
  FOR(k, K) chmax(x, MAX(ANS[k]));
  print(x);
}
