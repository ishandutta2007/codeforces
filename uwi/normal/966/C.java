...
public class C2 {
	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			a[i] = nl();
			int x = 63-Long.numberOfLeadingZeros(a[i]);
			ap[x]++;
		}
		long[][] g = new long[61][];
		for(int i = 0;i < 61;i++){
		...
		Node root = null;
		for(int i = 60;i >= 0;i--){
			Node[] lnodes = nodes(root);
			int p = 0;
			long x = 0;
			for(long e : g[i]){
				Node cur = new Node(e);
				while(p < lnodes.length && x == 1){
					x ^= lnodes[p].v>>>i&1;
					p++;
				}
				if(x == 1 || p > lnodes.length){
					out.println("No");
					return;
				}
				int ind = p == lnodes.length ? count(root) : index(lnodes[p]);
				root = insert(root, ind, cur);
				x ^= 1;
			}
		}
