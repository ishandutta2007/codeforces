...
Data merge (Data a, Data b) {
	...
	return Data (a.mx, min(a.ls, b.ls), max(a.rs, b.rs));
}
struct edge {
	int a, b, c, d;
} we[N];
int dfn[N], mp[N], idt, up1[20][N], w2[N], W[N], up2[20][N], ch[N][2], dep[N];
void dfs1 (int x) {
	dfn[x] = ++idt, mp[idt] = x;
...
int main () {
	...
	while (q--) {
		...
		cin >> v >> x;
		int z = jump (x, v);
		cout << em[z].mx << ' ' << w2[lca(lca(mp[em[z].ls], mp[em[z].rs]), x)] << '\n';
	}
	return 0;
}