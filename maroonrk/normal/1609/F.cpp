...
struct N{
	int mn,lz,val;
	N(pi rw=pi(inf,0)):mn(rw.a),lz(0),val(rw.b){}
	void add(int v){
		mn+=v;
		lz+=v;
	}
...
void slv(){
	...
	{
		int cnt[L]{};
		rep(i,n)cnt[popcount(a[i])]++;
		rep(i,L)qs[i].resize(cnt[i]*4);
	}
	rep(_,2){
		using A=array<int,2>;
		vc<A> t(n,A{-1,-1});
		...
		auto dfs=[&](auto self,int v)->int{
			if(v==-1)return 0;
			int x=self(self,t[v][0]);
			int y=self(self,t[v][1]);
			int z=popcount(abs(a[v]));
			qs[z][head[z]++]=T(v-x,v,v+y+1,-1);
			qs[z][head[z]++]=T(v+1,v,v+y+1,1);
			return 1+x+y;
		};
		dfs(dfs,root);
	...
	seglazy<N> seg(vc<pi>(n,pi(2,1)));
	ll ans=0;
	rep(lv,L)if(si(qs[lv])){
		sort(all(qs[lv]));
		int pre=0;
		for(auto [x,l,r,v]:qs[lv]){
			ans+=seg.getall().getcnt()*ll(x-pre);
			seg.ch(l,r,&N::add,v);
			pre=x;
		}
	}
