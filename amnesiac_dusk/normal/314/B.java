...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		...
		int d=ni();
		String a=ns(),c=ns();
		boolean[][]hola=new boolean[a.length()][c.length()];
		int i,j;
		int n=a.length(),m=c.length();
		for(i=0,j=0; ; i++)
		{
			if(hola[i%n][j%m]&&j%m==0)
				break;
			hola[i%n][j%m]=true;
			if(a.charAt(i%n)==c.charAt(j%m))
				j++;
		}
		i%=a.length();
		int te=i;
		int po=0;
		hola=new boolean[a.length()][c.length()];
		for(j=0; ; i++)
		{
			if(hola[i%n][j%m])
				break;
		...
		i=0;
		int ans=0;
		while(i+p<b*a.length())
		{
			i+=p;
			ans+=po;
		}
		for(j=0;i<b*a.length() ; i++)
		{
			if(a.charAt(i%n)==c.charAt(j%m))
				j++;
			if(j==m)
