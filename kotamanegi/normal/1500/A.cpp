...
#define int ll
vector<pair<int, int>> gogo[6000000];
void solve() {
	int n;
	cin >> n;
	vector<int> inputs;
	...
	REP(i, n) {
		for (int q = i + 1; q < n; ++q) {
			int target = inputs[i] + inputs[q];
			gogo[target].push_back(mp(i, q));
			if (gogo[target].size() >= 2 and gogo[target][0].first != gogo[target].back().first) {
				for (int j = 0; j < gogo[target].size(); ++j) {
					for (int p = j + 1; p < gogo[target].size(); ++p) {
						if (gogo[target][j].first != gogo[target][p].first and gogo[target][j].first != gogo[target][p].second and gogo[target][j].second != gogo[target][p].first and gogo[target][j].second != gogo[target][p].second) {
							cout << "YES" << endl;
							cout << gogo[target][j].first + 1<< " " << gogo[target][j].second + 1 << " " << gogo[target][p].first + 1 << " " << gogo[target][p].second + 1 << endl;
							return;
						}
					}
				}
