...
int main () {
	...
	int n, m, k;
	cin >> n >> m >> k;
	vector <vector <int>> animal(n, vector <int>(m));
	fop (i,0,n) fop (j,0,m) cin >> animal[i][j];
	vector <int> dp(m - k + 1, 0), pre(m - k + 1, 0);
	int now = 0;
	fop (i,0,m) {
	    now += animal[0][i];
	    if (i >= k) now -= animal[0][i - k];
	    if (i >= k - 1) {
	        dp[i - k + 1] = now;
	    }
            if (n > 1) {
                now += animal[1][i];
	        if (i >= k) now -= animal[1][i - k];
            }
	    if (i >= k - 1) {
	        dp[i - k + 1] = now;
	    }
	...
	fop (i,1,n) {
	    ...
	    fop (j,0,k) root.add(0, j + 1, -animal[i][j]);
            ...
            fop (j,0,k) {
                now += animal[i][j];
                if (i + 1 < n) now += animal[i + 1][j];
            }
            fop (j,0,m - k + 1) {
	        pre[j] = root.query(0, m) + now;
                now -= animal[i][j];
                now += animal[i][j + k];
                if (i + 1 < n) {
                    now -= animal[i + 1][j];
