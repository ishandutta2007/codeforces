...
void solve() {
    otera::factorial<mint> bc(300'300);
    INT(n, m);
    VEC(int, d, n);
    vc<int> sum(2 * n + 1, 0);
    rep(i, 2 * n) {
    ...
    mint ans = 0;
    for(int x = 0; x <= cnt; ++ x) {
        // two :: x: same, cnt - x: different
        // one :: n - 2 * cnt
        if(m - x < 0) continue;
        if(cnt - x > 0 and m - x == 0) continue;
        mint res = bc.com(cnt, x) * bc.fact(m) * bc.finv(m - x) * mint(m - x).pow(n - 2 * cnt) * mint(m - x).pow(cnt - x) * mint(m - x - 1).pow(cnt - x);
        ans += res;
        debug(x, res);
    }
    out(ans);
