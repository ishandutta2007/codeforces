...
vector<ll> Dijkstra(const vector<vector<ll>>&g,ll at){
    ...
    while(q.size()){
        ...
        if(used[at])continue;
        used[at]=1;
        each(i,g[at])if(chmin(cost[i],cost[at]+1))q.emplace(i);
    }
    return cost;
}
signed main(){
    ...
    auto a=Dijkstra(g,0),b=Dijkstra(g,n-1);
    vector<pll>p(k);
    rep(k)p[i]={a[f[i]],b[f[i]]};
    SORT(p);
    ll ans=0,cnt=-LINF;
    each(i,p){
        chmax(ans,cnt+i.second+1);
        chmax(cnt,i.first);
    }
    chmin(ans,a[n-1]);
    out(ans);
