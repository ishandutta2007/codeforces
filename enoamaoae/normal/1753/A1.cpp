...
void calc(){
    ...
    rep(i,1,n){
        cll x=readll();
        if(x==0)s[i]=1;
        else if(lst==0)lst=x,lstw=i;
        else{
            if(lst==x)s[lstw]=1,s[i]=-1;
            else s[lstw]=s[i]=1;
            lst=0;
        }
    }
    if(lst)myputs("-1");
        ...
        ll t=0;
        rep(i,1,n)if(i==1||s[i]==s[i-1])t++;
        writell(t,10);
        ll lst=1;
        rep(i,2,n)if(i==1||s[i]==s[i-1])writell(lst),writell(i-1,10),lst=i;
        writell(lst),writell(n,10);
    }
}
int main(){
    init();
