...
int main(){
	...
	TT{
		...
		cauto vs=readV(n,0);
		vector<int> inds;
		rep(i,n)if(vs[i]) inds.eb(i);
		if(inds.size()%2){
			cout<<-1 NL;
			continue;
		}
		vector<pii> ints;
		rep(i,inds.size())if(i%2){
			cauto l=inds[i-1];
			cauto r=inds[i];
			if((vs[l]==vs[r]) != (l%2==r%2)){
				ints.eb(l,r+1);
			}else{
				ints.eb(l,l+1);
				ints.eb(l+1,r+1);
		...
		vector<int> usd(n);
		for(cauto &[l,r]:ints)reps(i,l,r) usd[i]=1;
		rep(i,n)if(!usd[i]) ints.eb(i,i+1);
		sort(all(ints));
		cout<<ints.size() NL;
		for(cauto &[l,r]:ints) cout<<l+1<<" "<<r NL;
	}
	return 0;
}