...
void solve() {
  ...
  vi COEF;
  for (auto&& [val, cnt]: CNT) { COEF.eb(cnt); }
  UNIQUE(COEF);
  vc<vi> vals(len(COEF));

  FOR(k, len(vals)) sort(all(vals[k]));

  auto find_max = [&](vi& V, vi& NG) -> ll {
    FOR_R(i, len(V)) {
      ll v = V[i];
      if (binary_search(all(NG), v)) continue;
      return v;
    }
    return -1;
  };
  ...
  for (auto&& [y, cnt]: CNT) {
    ...
    FOR(k, len(COEF)) {
      ...
      ll x = find_max(V, NG);
      if (x == -1) continue;
      ll f_val = (coef + cnt) * (x + y);
      // print(x, y, coef, cnt, f_val);
      chmax(ANS, f_val);
    }
    ll k = LB(COEF, cnt);
    vals[k].eb(y);
