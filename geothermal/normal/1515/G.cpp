...
void upd(int p, ll X) {
    ...
    val[p] = __gcd(abs(val[p]), abs(X));
}
void upd2(int p, ll X) {
    if (X == 0) return;
    if (val2[p] == 0) {
        val2[p] = X;
    }
    val2[p] = __gcd(X, val2[p]);
}

void dfs(int v, int p) {
    if (p != -1) dep[v] = dep[p] + 1;
    trav(a, graph[v]) {
        if (comp[a.f] != comp[v]) continue;
        if (dep[a.f] != -1) {
            dbg(v, a.f, num[v], num[a.f]);
            //if (num[v] - num[a.f] + a.s > 0) {
                upd(comp[v], num[v]-num[a.f]+a.s);
            //}else upd2(comp[v], abs(num[v] - num[a.f] + a.s));
        } else {
            num[a.f] = num[v] + a.s;
            dfs(a.f, v);
        }
...
void solve() {
    ...
    while(Q--) {
        ...
        //if ((G != 0 && (T-S) % __gcd(G, (ll) T) == 0) || (G == 0 && S == 0)) {
        //if ((T-S)%__gcd(G, G2)==0) {
        if ((T-S)%G==0) {
            prn("YES");
        } else prn("NO");
