...
void test_case(int test) {
    ...
    for (int i = 1; i < p2.size(); ++i)
        p2[i] = p2[i - 1] * 2;
    Mint i2 = Mint(1) / 2;
    vector<Mint> pi2(n + 5, 1);
    for (int i = 1; i < p2.size(); ++i)
        pi2[i] = pi2[i - 1] * i2;


    vector<Mint> probr(n);
    vector<Mint> probr2(n);
    vector<Mint> probr2suf(n + 10, 0);
    for (int i = n - 1; i >= 0; --i) {
        if ((i + 1) > (n - 1 - i)) {
            probr[i] = 1;
            probr2[i] = probr[i] * pi2[i];
        } else {
            show(n, i, i * 2 + 1);
            probr[i] = probr2suf[i + 1] - probr2suf[i * 2 + 1];
            probr2[i] = probr[i];
            probr[i] *= p2[i];
        }
        probr2suf[i] = probr2suf[i + 1];
        probr2suf[i] += probr2[i];
    }
    show(probr);
    for (int i = 0; i < n; ++i) {
        Mint prob = (i == n - 1 ? Mint(1) : i2);
        prob *= pi2[i / 2];
        prob *= probr[i];
        cout << prob << '\n';
    }
