            ...
            long long m,x,n,y,z,k,one,gd,two,mn=1e9,mid,l,r,pos;vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
    ...
    long long xa,yb;
        ...
        long long a[500001];
            ...
            int main()
            {
                ...
                {
                    cin>>n>>r;set<int> mys;
                    for(int i=0;i<n;i++){cin>>x;mys.insert(x);}cnt=0;y=0;
                    while(!mys.empty())
                    {
                        y+=r;//cout<<*mys.begin()<<" ";
                        mys.erase(--mys.end());

                        for(auto it=mys.begin();it!=mys.end()&&(*it)<=y;){auto it2=it;it++;mys.erase(it2);}
                        cnt++;
