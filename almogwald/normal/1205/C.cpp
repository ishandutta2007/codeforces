...
int main() {
	...
	a[0][0] = 1;
	fort(i,2, n) {
		out('?') out(i - 1) out(1) out(i + 1) co 1 el
			defb
			a[0][i] = a[0][i - 2] ^ b ^ 1;
	}
	out('?') out(n - 1) out(1) out(n) co 2 el
	defb
	a[1][n-1] = a[0][n - 2] ^ b ^ 1;
	out('?') out(n - 2) out(1) out(n-1) co 2 el
		into(b)
		a[1][n - 2] = a[0][n - 3] ^ b ^ 1;
	fortb(i, 2, n) {
		out('?') out(i - 1) out(2) out(i + 1) co 2 el
			defb
			a[1][i-2] = a[1][i] ^ b ^ 1;
	}
	fori(i, n) {
		fort(j, 2, n) {
			out('?') out(i +1) out(j-1) out(i + 1) co j + 1 el
				defb
				a[j][i] = a[j-2][i] ^ b ^ 1;
		}
	}
	vector<veci> c(n, veci(n));
	fori(i, n) {
		fori(j, n) {
			c[i][j] = a[i][j] ^ ((i + j) % 2);
		}
		...
		fori(j, n) {
			fori(k, 4) {
				...
				if (a[i][j] == a[i + k][j - k + 3]) {
					...
					if (x & y) {
						out('?') out(j + 1) out(i + 1) out(j + 1 + 3 - k) co i + 1 + k el
							into(found)
							found ^= 1;
							br
					}
