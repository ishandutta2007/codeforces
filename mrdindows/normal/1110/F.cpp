...
//            continue;
//        }
//        if (cnt[i] == 1 && cnt[i + 3] == 2 && used[i + 1] && used[i + 2]) {
//            cnt[i] = 0;
//            cnt[i + 3] = 0;
//            used[i + 1]--;
//            cnt[i + 1];
...
vector<tri> qs[N];
ll ans[N];
void go(int cur, ll d = 0) {
    tout[cur] = cur;
    h[cur] = d;
    for (auto [to, w] : v[cur]) {
        go(to, d + w);
        tout[cur] = max(cur, tout[to]);
    }
}
ll tr[1 << 20];
ll pu[1 << 20];
...
ll get(int cur, int l, int r, int x, int y) {
    if (x > r || y < l) return LLINF;
    ...
    return min(get(dcur, l, m, x, y), get(dcur + 1, m + 1, r, x, y));
}
int n, q;
void dfs(int cur) {
//    DBN(cur, tout[cur]);
    for (auto [x, y, id] : qs[cur]) {
        ans[id] = get(1, 0, n - 1, x, y);
    }
    for (auto [to, w] : v[cur]) {
        add(1, 0, n - 1, 0, n - 1, w);
        add(1, 0, n - 1, to, tout[to], - 2 * w);
        dfs(to);
        add(1, 0, n - 1, 0, n - 1, -w);
        add(1, 0, n - 1, to, tout[to], 2 * w);
    }
