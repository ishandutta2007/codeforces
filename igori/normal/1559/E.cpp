...
void solve()
{
    ...
    for (int i = 1; i <= m; i++)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            //cout << "L = " << L << ", R = " << R << endl;
            vector<ll> dp2(m / i + 1);
            for (int io = 1; io < m / i + 1; io++)
                dp[io] = dp[io] + dp[io - 1] >= MOD ? dp[io] + dp[io - 1] - MOD : dp[io] + dp[io - 1];
            for (int io = L; io < m / i + 1; io++)
                dp2[io] = (dp[io - L] - (io - R - 1 >= 0 ? dp[io - R - 1] : 0) + MOD) % MOD;
            dp = dp2;
        }
        //cout << i << " " << m / i << "\n";
