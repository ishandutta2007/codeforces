...
inline constexpr ll extgcd(ll a, ll b, ll &x, ll &y) { //ax + by = gcd(x,y)
    ll g = a; x = 1; y = 0;
    if (b != 0){
        g = extgcd(b, a % b, y, x);
        y -= (a / b) * x;
...
inline constexpr ll inv(ll a, ll m=MODD) { // 1/a (mod m)
    ll x=0, y=0;
...
int main(){
    ...
    vec(ll,_invb,m+1);
    vec(ll,_invc,m+1);
    rep(2*m+1)_inva[i]=inv(plus+minus-m+i);
    rep(m+1)_invb[i]=inv(plus+i);
    rep(m+1)_invc[i]=inv(minus-m+i);
    auto inva=_inva.begin()-plus-minus+m;
    auto invb=_invb.begin()-plus;
    auto invc=_invc.begin()-minus+m;
    vec(ll,p,m+1);
    p[0]=1;
    rep(m){
        vec(ll,p2,m+1);
        rep(j,m){
            p2[j]+=p[j]*(minus-i+j)%MODD*inva[plus+j+minus-i+j]%MODD;
            p2[j+1]+=p[j]*(plus+j)%MODD*inva[plus+j+minus-i+j]%MODD;
        }
        p.swap(p2);
    }
    ll ans1=0,w1=1,ans2=0,w2=1;
    rep(m+1){
        ans1+=w1*p[i]%MODD;
        w1+=w1*invb[plus+i];
        w1%=MODD;
    ...
    ans2%=MODD;
    rep(n){
        if(a[i])out(ans1*w[i]%MODD);
        else out(ans2*w[i]%MODD);
    }
}