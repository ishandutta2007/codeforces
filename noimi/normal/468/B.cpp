...
int main() {
    ...
    rep(i, n) mp[v[i]] = i;
    if(a == b) {
        rep(i, n) if(!mp.count(a - v[i])) drop(NO);
        YES();
        cout << vi(n, 1) << endl;
        exit(0);
    }
    Graph g(n);
#define add_edge(i, j) g[i].eb(j)
    rep(i, n) {
        if(used[i]) continue;
        if(mp.count(a - v[i])) add_edge(i, mp[a - v[i]]);
        if(mp.count(b - v[i])) add_edge(i, mp[b - v[i]]);
    }
    vi deg(n);
    ...
    while(!empty(q)) {
        ...
        for(auto e : g[x]) {
            if(used[e]) continue;
            used[x] = used[e] = 1;
            if(v[x] + v[e] == b) ans[x] = ans[e] = 1;
            for(auto ee : g[e]) {
                deg[ee]--;
                if(deg[ee] == 1) q.emplace(ee);
            }
        }
    }
    if(used != vi(n, 1)) drop(NO);
    YES();
    cout << ans << endl;
}