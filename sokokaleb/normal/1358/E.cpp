...
void solve(int tc_num = -1) {
	...
	if (accumulate(arr, arr + real_n, 0LL) + 1LL * x * (n >> 1) > 0) {
		cout << n << "\n";
		return;
	}
	...
	// sum <= 0
	ll current_sum = 1LL * x * (n >> 1);
	if (current_sum > 0LL && -_get(pq.top(), 0) + current_sum > 0LL) {
		cout << (n >> 1) << "\n";
		return;
	}
	LOG("ASU", real_n);
	...
	FORD (l, real_n - 1, 0) {
		current_sum += arr[l];
		diff += arr[l] - x;
		LOG(l, current_sum, diff);
		if (current_sum <= 0LL) continue;
		while (_get(pq.top(), 1) >= l) pq.pop();
		auto [a, b] = pq.top();
		LOG(-a, b);
		if (-_get(pq.top(), 0) + current_sum - diff > 0LL) {
			cout << (n - l) << "\n";
			return;
		}
	}
