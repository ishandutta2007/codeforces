...
const int maxn = 1e5 + 100, maxw = 1e6 + 100, inf = 1e9 + 100, mod = 1e9 + 7;

int n, k, q[maxn], q1[maxn];
...
void solve() {
    ...
    for (int i = 0; i + k <= n; i++) {
        bool is = 0;
        for (int j = 0; j < 2; j++)
            if (g[j].empty() || *g[j].rbegin() - *g[j].begin() + 1 <= k)
                is = 1;
        if (is)
            ans[i]++;
        if (i + k < n) {
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        g[q[i]].insert(i);
    for (int i = 0; i < 2; i++)
        if (g[i].empty() || *g[i].rbegin() - *g[i].begin() + 1 <= k)
            FIRST();
    for (int i = 0; i < 2; i++) {
        solve();
        for (int j = 0; j < n; j++)
