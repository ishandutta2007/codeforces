...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		cin >> H[i] >> M[i];
		val[i].first = (M[i] >= (m/2)) ? (M[i] - (m/2)) : M[i];
		val[i].second = i;
	}	
	...
	for(int i = 0; i < n; ++i){
		val.push_back({val[i].first + m/2,val[i].second});
	}
	int j = 0;
	...
	for(int i = 0; i < n; ++i){
		...
		while(j + 1 < val.size() && val[j+1].first < val[i].first + k){
			++j;
		}

		ans = min(ans, {j - i,i});
	}
	int cur_time = val[ans.second].first + k;
	if (cur_time >= m/2)
		cur_time -= m/2;
	assert(0 <= cur_time && cur_time <= (m/2));
	cout << ans.first << " " << cur_time << endl;

	for(int x = ans.second + 1; x <= ans.second + ans.first; ++x){
		cout << val[x].second + 1 << " ";
	}
	cout << endl;
}
