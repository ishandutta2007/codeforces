...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read(),k=read();
        for(res i=1;i<=n;i++)for(res j=1;j<=n;j++)p[read()]=mp(i,j);
        a[p[n*n].fi][p[n*n].se]=1;
        res A=p[n*n].fi+p[n*n].se,B=p[n*n].fi-p[n*n].se,C=p[n*n].fi-p[n*n].se,D=p[n*n].fi+p[n*n].se;
        for(res i=n*n-1;i>=1;i--){
            auto [x,y]=p[i];
            if(A<=x+y+k&&x+y-k<=D&&B<=x-y+k&&x-y-k<=C){
                a[x][y]=1;
                A=max(A,x+y),D=min(D,x+y),B=max(B,x-y),C=min(C,x-y);
            }
        }
        for(res i=1;i<=n;i++){
            for(res j=1;j<=n;j++)putchar(!a[i][j]?'G':'M');
            puts("");
        }
    }
}
