...
int main(int argn, char **argv) {
    ...
    REP(I, t) {
        ...
        ll total = 0;
        REP(i, n) {
            if (i == 0) chkmax(best, abs(arr[0] - arr[1]));
            else {
                if (i == n-1) chkmax(best, abs(arr[i] - arr[i-1]));
                else {
                    chkmax(best, abs(arr[i] - arr[i-1]) + abs(arr[i] - arr[i+1]) - abs(arr[i-1] - arr[i+1]));
                }
            }
