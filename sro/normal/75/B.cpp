...
void input()
{
	string name1,name2,eater;
	int scor;
	cin>>name1>>eater;
	if(eater=="posted")
	{
		scor=15;
		cin>>eater>>name2>>eater;
		name2.erase(SZ(name2)-2,2);
	}
	else if(eater=="commented")
	{
		scor=10;
		cin>>eater>>name2>>eater;
		name2.erase(SZ(name2)-2,2);
	...
	if(!score.count(name1)&&name1!=me)
	{
		names.push_back(name1);
		score[name1]=0;
	}
	if(!score.count(name2)&&name2!=me)
	{
		names.push_back(name2);
		score[name2]=0;
	}
	if(name1==me)score[name2]+=scor;
	if(name2==me)score[name1]+=scor;
}

bool sort_comp(string a,string b)
{
	if(score[a]>score[b])return 1;
	if(score[b]>score[a])return 0;
	if(a<b)return 1;
	if(a>b)return 0;
	return 0;
}
...
int main()
{
	...
	for(int i=0;i<n;i++)
		input();
	sort(names.begin(),names.end(),sort_comp);
