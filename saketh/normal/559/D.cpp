...
template<typename T> struct point {
    ...
    // number of lattice points on segment ab, excluding a and b
    friend T lattice(const point& a, const point& b) {
        if (a == b) return 0;
        return gcd(abs(a.x - b.x), abs(a.y - b.y)) - 1;
    }

    // number of lattice points in p's interior
    friend T lattice(const polygon& p) {
        T r = abs(area(p)) + 2;
        for (int i = 0; i < sz(p); i++) {
            r -= 1 + lattice(p[i], p[i+1 < sz(p) ? i+1 : 0]);
        }
        assert(r % 2 == 0);
        return r / 2;
    }
...
int main() {
    ...
    using pt = point<ll>;
    int N; re(N); vector<pt> p(N);
    F0R (i, N) { int x, y; re(x, y); p[i] = {x,y}; }

    double ans = lattice(p);
    F0R (i, N) p.pb(p[i]);

    const int B = 100;
    vd p2(B);
    p2[0] = 1;
    FOR (i, 1, B) p2[i] = p2[i-1] / 2;

    double de = 1;
    if (N < B) de -= (1 + N + double(N) * (N - 1) / 2) * p2[N];

    vll cr(sz(p));
    F0R (i, sz(p)-1) cr[i] = cross(p[i], p[i+1]);

    vi lat(sz(p));
    F0R (i, sz(p)-1) lat[i] = lattice(p[i], p[i+1]);

    F0R (i, N) {
        ll a = 0, b = 2 * (1 + lattice(p[i], p[i+1]));
        ll _a = cross(p[i+1], p[i]), _b = lattice(p[i+1], p[i]) + 1;
        FOR (k, 2, min(N, 60)) {
            a -= _a;
            a += cr[i+k-1];
            a += (_a = cross(p[i+k], p[i]));

            b -= _b;
            b += lat[i+k-1] + 1;
            b += (_b = lattice(p[i+k], p[i]) + 1);

            double nu = p2[k+1];
            if (N < B) nu -= p2[N];

            ans -= nu / de * (_b - 1 + (abs(a) + 2 - b)/2);
        }
    }
