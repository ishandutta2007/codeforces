...
int ask(int x,int x2,int y,int y2){
    cout<<"? "<<x<<" "<<y<<" "<<x2<<" "<<y2<<endl;
    cout.flush();
    return in();
}
...
signed main(){
    ...
    vec x,y;
    rep2(i,1,n){
        if(ask(i,i,1,n)&1) x.pb(i);
        if(ask(1,n,i,i)&1) y.pb(i);
    }
    if(x.size() and y.size()){
        if(ask(x[0],x[0],y[0],y[0])&1){
        ...
        if(x.size()){
            ...
            while(l<r-1){
                int mid=(l+r)/2;
                if(ask(x[0],x[0],1,mid)&1)r=mid;
                else l=mid;
            }
            cout<<"! "<<x[0]<<" "<<r<<" "<<x[1]<<" "<<r<<endl;
        }
        else{
            int l=0,r=n;
            while(l<r-1){
                int mid=(l+r)/2;
                if(ask(1,mid,y[0],y[0])&1)r=mid;
                else l=mid;
            }
            cout<<"! "<<r<<" "<<y[0]<<" "<<r<<" "<<y[1]<<endl;
        }
    }
}