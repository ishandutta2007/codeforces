...
main() {
    ...
    sort(all(b));
    sort(all(ab));
    while(a.size() <= k) a.eb(1e13);
    while(b.size() <= k) b.eb(1e13);
    while(ab.size() <= k) ab.eb(1e13);
    multiset<ll> mul;
    rep(i, n) mul.emplace(v[i]);
    ll ans = LLONG_MAX;
    ll tmp = 0;
    pqg<ll> pq;
    auto del = [&](ll y) {
        auto it = mul.lower_bound(y);
        if(it == mul.end()) return false;
        if(*it != y) return false;
        mul.erase(it);
        return true;
    };
    int mem = 0;
    rep(i, k) del(ab[i + 1]), tmp += ab[i + 1];
    while(mul.size() > m - k) {
        pq.emplace(*(--mul.end()));
        mul.erase(--mul.end());
    }
    for(auto e : mul) tmp += e;
    if(mul.size() + k == m)
        if(chmin(ans, tmp)) mem = k;
    rep3(i, k - 1, 0) {
        if(i + (k - i) * 2 > m) break;
        if(!del(a[k - i])) tmp += a[k - i];
        if(!del(b[k - i])) tmp += b[k - i];
        mul.emplace(ab[i + 1]);
        if(!pq.empty()) {
            tmp += pq.top();
            mul.emplace(pq.top());
        ...
        while(mul.size() > m - i - (k - i) * 2) {
            tmp -= *(--mul.end());
            pq.emplace(*(--mul.end()));
            mul.erase(--mul.end());
        }
        if(mul.size() + i + (k - i) * 2 == m)
            if(chmin(ans, tmp)) mem = i;
    }
    if(ans > (ll)3e9) {
        cout << -1 << endl;
    } else {
        ...
        rep(i, n) {
            if(w[i] == 1) a.emplace_back(i);
            if(w[i] == 2) b.emplace_back(i);
            if(w[i] == 3) ab.emplace_back(i);
        }
        vi used(n);
        sort(all(a), [&](int x, int y) { return v[x] < v[y]; });
        sort(all(ab), [&](int x, int y) { return v[x] < v[y]; });
        sort(all(b), [&](int x, int y) { return v[x] < v[y]; });
        rep(i, mem) used[ab[i]] = true;
        rep(i, k - mem) used[a[i]] = true, used[b[i]] = true;
        rep(i, mem) cout << ++ab[i] << " ";
        rep(i, k - mem) cout << ++a[i] << " " << ++b[i] << " ";
        rep(i, n) if(!used[i]) id.eb(i);
        sort(all(id), [&](int x, int y) { return v[x] < v[y]; });
        rep(i, m - mem - (k - mem) * 2) cout << ++id[i] << " ";
    }
}