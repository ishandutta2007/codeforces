#line 1 "codeforces-workspace/contest/1375/f/f.cpp"
...
#line 11 "codeforces-workspace/contest/1375/f/f.cpp"
...
void fin(i64 a, i64 b, int k = 0) {
  assert(a > b);
  if (!k) {
    int ret = send(a - b);
    if (ret == p[2].second) send(a);  // finish
  }
  send(b);
  send(b * 3);
  send(b);
...
void main() {
  ...
  auto x = p[1].first - p[0].first, y = p[2].first - p[1].first;
  while (x > y) {
    int ret = send(y);
    assert(ret != p[1].second);
    if (ret == p[2].second)
      y += y;
    else
      x -= y;
