...
inline double fu(double ) { return (double)1; }
inline void sanfen(double ll,double rr)
{
	double l=ll,r=rr,lmid,rmid;
    while(r-l>1e-6)
    {
        lmid = l + (r-l)/3; rmid = r - (r-l)/3; if(fu(lmid)>fu(rmid)) l = lmid; else r = rmid;  // 
		double k=(r-l)/3.0; lmid=l+k, rmid=r-k; if(fu(lmid)>fu(rmid)) r = rmid; else l = lmid;  // 
    }
    printf("%.10lf\n",l);
}
