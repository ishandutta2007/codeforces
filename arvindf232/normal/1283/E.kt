...
fun main(){
    solve.rand {
        share(4)
        share((List(2){116486} + List(2){49798}).shuffled())
    }
//    solve.userand()
    solve.cases{
        val n = getint()
        ...
        L.sort()
//        L.radixsort()
        val DP_max = Array(n+1){IntArray(8){nlarge} }
        val DP_min = Array(n+1){IntArray(8){plarge} }
        DP_max[0][0] = 0
        DP_min[0][0] = 0
        for(p in 0 until n){
            val pre = if(p == 0) -5 else L[p-1]
            val here = L[p]
            for(left in 0 until 8){
                val new = (left) shr minOf(3,(here - pre))
                for(i in 0 until 3){
                    val extras = if(new and (1 shl i) != 0) 0 else 1
                    val newnew = new or (1 shl i)
                    DP_max[p+1][newnew] = maxOf(DP_max[p+1][newnew], extras + DP_max[p][left])
                    DP_min[p+1][newnew] = minOf(DP_min[p+1][newnew], extras + DP_min[p][left])
                }
            }
        }
        put(DP_min.last().minOrNull()!!)
        put(DP_max.last().maxOrNull()!!)
    }
    done()
}
