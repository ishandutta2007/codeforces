...
public:
  ...
  vector<int> pangkat(vector<int> a, int b){
    int len = b * sz(a);
    int pw = 1;
    while(pw < len) pw <<= 1;
    len = pw;
    a.resize(len, 0);
    fft(a, false);
    for(int i = 0; i < len; i++) a[i] = powmod(a[i], b);
    fft(a, true);
    return a;
  }
...
int main(){
    ...
    vector<int> ini(10, 0);
    for(int i = 0; i < k; i++) ini[d[i]] = 1;
    auto res = ntt.pangkat(ini, n / 2);
    long long jaw = 0;
    for(int i = 0; i < sz(res); i++){
      jaw = jaw + 1LL * res[i] * res[i] % mod;
      jaw %= mod;
    }
    cout << jaw << endl;
}