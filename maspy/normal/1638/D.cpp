...
void solve() {
  ...
  auto check = [&](ll x, ll y) -> ll {
    if (x < 0 || x >= H - 1) return -1;
    if (y < 0 || y >= W - 1) return -1;
    if (done[x][y]) return -1;
    ll col = 0;
    FOR(a, 2) FOR(b, 2) chmax(col, A[x + a][y + b]);
    if (col == 0) return -1;
    FOR(a, 2) FOR(b, 2) {
      ll c = A[x + a][y + b];
  ...
  auto ope = [&](ll x, ll y, ll c) -> void {
    ANS.eb(x + 1, y + 1, c);
    A[x + 0][y + 0] = 0;
    A[x + 0][y + 1] = 0;
    A[x + 1][y + 0] = 0;
    A[x + 1][y + 1] = 0;
    FOR3(dx, -2, 3) FOR3(dy, -2, 3) {
      ll c = check(x + dx, y + dy);
      if (c != -1) que.eb(x + dx, y + dy, c);
    }
  };
  ...
  FOR(x, H) FOR(y, W) if (A[x][y] != 0) return print(-1);
  reverse(all(ANS));
  print(len(ANS));
  for (auto&& [x, y, c]: ANS) print(x, y, c);
}
