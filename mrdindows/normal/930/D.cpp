...
//            }
    ...
    sort(all(v), COMPARE_BY(v[0], second));
    {
        int cur = 0;
        int ptr = 0;
        FI(N + 1) {
            while (ptr < v.size() && v[ptr].second == i) {
                cur = max(cur, v[ptr].first);
                ++ptr;
            }
            up[i] = cur;
        }
    ...
    vector<tri> evs;
    {
        ...
        for (int i = N; i >= 0; --i) {
            while (ptr < v.size() && v[ptr].second == i) {
                cur = min(cur, v[ptr].first);
                ++ptr;
            }
            dw[i] = cur;
            if (dw[i] <= up[i]) {
//                DBN(i, dw[i], up[i]);
                evs.push_back({dw[i], i, 1});
                evs.push_back({up[i] + 1, i, -1});
            }
        }
    }
    int ptr = 0;
    ...
    long long ans = 0;
    for (int i = 0; i <= N; ++i) {
        while (ptr < evs.size() && evs[ptr].a == i) {
            if (evs[ptr].c == 1) {
                add<1>(evs[ptr].b);
            } else {
                add<-1>(evs[ptr].b);
...
int solve() {
    ...
    FI(n) {
        ...
        cin >> x >> y;
        const int T = 200002;
        tasks[(x + y + 4000000) % 2].push_back({(x + y + T) / 2, (x - y + T) / 2});
    }
    ll ans = 0;
    FI(2) {
        ans += solve(tasks[i]);
