...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n; i++)
			a[i] = lower_bound(all(used), a[i]) - used.begin() + 1;

		vector<vector<int>> pos(used.size() + 7);
		for (int i = 0; i < n; i++)
			pos[a[i]].pb(i);

		int ans = 0;
		vector<int> dp(pos.size());
		for (int sz = 0, l = 1, r = 1; !pos[r].empty(); r++) {
			if (r > 1 && pos[r].front() < pos[r - 1].back()) {
				l = r;
				sz = 0;
			}
			sz += pos[r].size();

			int lcnt = upper_bound(all(pos[l - 1]), pos[l].front()) - pos[l - 1].begin();
			int rcnt = pos[r + 1].end() - upper_bound(all(pos[r + 1]), pos[r].back());

			chkmax(ans, sz + lcnt + rcnt);
		}
		
		for (int r = 2; !pos[r].empty(); r++) {
			for (int i = 0; i < pos[r].size(); i++) {
				int lcnt = upper_bound(all(pos[r - 1]), pos[r][i]) - pos[r - 1].begin();
				int rcnt = pos[r].size() - i;
				
				chkmax(ans, lcnt + rcnt);
			}
		}
