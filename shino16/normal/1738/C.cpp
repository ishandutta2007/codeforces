...
void gen() {
  win[0][0][0] = true;
  rep(a, 100) rep(c, 2) win[a+1][0][c] = !win[a][0][(a+c)%2];
  rep(b, 100) rep(c, 2) win[0][b+1][c] = c == 0;
  rep(a, 100) rep(b, 100) rep(c, 2) {
    win[a+1][b+1][c] |= !win[a][b+1][(a+c)%2];
    win[a+1][b+1][c] |= !win[a+1][b][(a+c)%2];
  }
}

void solve() {
  int n = in, *a = in.arr<int>(n);
  int cnt[2] = {};
  rep(i, n) cnt[a[i]&1]++;
  out(win[cnt[1]][cnt[0]][0] ? "Alice" : "Bob");
}
