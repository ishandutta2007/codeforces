...
void dfs(ll x,ll p,ll w)
{
    ...
    for(ll i=1;i<=19;i++)
    {
        mx[i][x]=max(mx[i-1][x],mx[i-1][up[i-1][x]]);
    }
    for(auto z:adj[x])
    {
        if(z.ff!=p)
...
int main () 
{
    ...
    vector<pair<pair<ll,ll>,pair<ll,ll>>>v,ns;
    rep(i,m)
    {
        ll x,y,z;
        cin>>x>>y>>z;
    ...
    for(auto x:v)
    {
        ...
        {
            merge(a,b);
            adj[a].pb(mp(b,x.ff.ff));
            adj[b].pb(mp(a,x.ff.ff));
        }
        else
        {
            swap(x.ff.ff,x.ff.ss);
            ns.pb(x);
        }
    }
    sort(all(ns));
    ...
    for(auto x:ns)
    {
        cout<<find(x.ss.ff,x.ss.ss)<<endl;
    }
