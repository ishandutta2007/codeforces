...
public:
    void solve(istream& cin, ostream& cout) {
		int N; ll P; cin >> N >> P;
		vector<pair<double, pii>> S(N);

		ll totA = 0;
		for (int i = 0; i < N; ++i) {
			int a, b; cin >> a >> b;
			totA += a;
			S[i] = {(double)b / (double)a, {b,a}};
		}

		if (totA <= P) { cout << -1 << endl; return; }
		sort(S.begin(), S.end());
		double lastT = 0.0;
		double excess = 0.0;
		ll usage = 0;
		for (int i = 0; i < N; ++i) {
			double nextT = S[i].x;
			if (usage > P && (usage-P)*(nextT-lastT) > excess) {
				break;
			}
			excess += (P-usage)*(nextT-lastT);
			lastT = nextT;
			usage += S[i].y.y;
		}

		cout << fixed << setprecision(10) << lastT + excess/(usage-P) << endl;
    }
};
