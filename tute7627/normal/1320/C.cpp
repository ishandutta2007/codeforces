...
int main(){
  ...
  ll n,m,p;cin>>n>>m>>p;
  LazySegmentTree<ll,ll,decltype(ff),decltype(gg),decltype(hh)>seg(m,ff,gg,hh,-INF,0);
  vector<ll>a(n),ca(n),b(m),cb(m),x(p),y(p),z(p);
  rep(i,0,n)cin>>a[i]>>ca[i];
  rep(i,0,m)cin>>b[i]>>cb[i];
  rep(i,0,p)cin>>x[i]>>y[i]>>z[i];
  vector<ll>oa(n),ob(m),oc(p);
  iota(ALL(oa),0);
  iota(ALL(ob),0);
  iota(ALL(oc),0);
  sort(ALL(oa),[&](ll i,ll j){return a[i]<a[j];});
  sort(ALL(ob),[&](ll i,ll j){return b[i]<b[j];});
  sort(ALL(oc),[&](ll i,ll j){return x[i]<x[j];});
  auto vb=b;
  sort(ALL(vb));
  rep(i,0,m){
    seg.set(i,-cb[ob[i]]);
  ...
  for(auto i:oa){
    //seg.print();
    while(r<p&&x[oc[r]]<a[i]){
      ll idx=upper_bound(ALL(vb),y[oc[r]])-vb.begin();
      seg.update(idx,m,z[oc[r]]);
      r++;
    }
    //seg.print();
    chmax(res,seg.query(0,m)-ca[i]);
  }
  cout<<res<<endl;
  return 0;
}