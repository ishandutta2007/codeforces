...
bool bad(int i, int j) {
    if (i==j) return true;
    if (i>j) swap(i, j);
    if (badpairs.find(mp(i, j)) != badpairs.end()) return true;
    return false;
}
...
void solve() {
    ...
    F0R(i, m) {
        cin >> x >> y;
        badpairs.insert(mp(x, y));
    }
    freq.clear();
    for (int i : a) freq[i]++;
    freqs.clear(); byfreq.clear(); byfreq.rsz(n+1);
    for (pi p : freq) {
        if (byfreq[p.S].empty()) freqs.pb(p.S);
        byfreq[p.S].pb(p.F);
    }
    F0R(i, n+1) {
        sort(all(byfreq[i]));
        reverse(all(byfreq[i]));
    }
    debug(freqs, byfreq);
    ll ans = 0;
    for (int f1 : freqs) for (int f2 : freqs) if (f1 > f2) {
        x=f1, y=f2;
        if (byfreq[f2].size() < byfreq[f1].size()) swap(x, y);
        for (int p : byfreq[x]) {
            for (int q : byfreq[y]) if (!bad(p, q)) {
                ans = max(ans, ll(p+q)*ll(f1+f2));
                break;
            }
        }
    }
    for (int f : freqs) {
        int k = byfreq[f].size();
        for (int i : byfreq[f]) {
            for (int j : byfreq[f]) if (!bad(i, j)) {
                ans = max(ans, ll(i+j)*ll(f+f));
                break;
            }
        }
    }
