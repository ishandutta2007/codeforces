...
unordered_set <pii> touched;
int col[2];
void dfs(pii x) {
	if (touched.count(x)) return;
	int c = (x.first + x.second) & 1;
	if (col[c] == 0) return;
	touched.insert(x);
	col[c]--;
	for (pii d : vpii{{-1, 0}, {0, -1}, {1, 0}, {0, 1}})
		dfs(x + d);
}
int main() {
	int q;
	...
	REP(quq, q) {
		touched.clear();
		scanf("%d%d", col + 1, col);
		if (col[0] > col[1]) dfs({0, 0});
		else dfs({0, 1});
		if (col[0] || col[1]) printf("NO\n");
		else {
			printf("YES\n");
			int shift = 0;
			for (pii x : touched)
