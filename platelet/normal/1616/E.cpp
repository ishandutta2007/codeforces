...
void solve() {
    ...
    ll ans = inf, sum = 0;
    vector<int> p[26];
    per(i, n, 1) p[s[i] - 97].pb(i);
    rep(i, 1, n) {
        int pos = n + 1;
        For(j, 0, t[i] - 97) if(SZ(p[j])) cmin(pos, p[j].back());
        auto calc = [&](int i) {
            int res = 0;
            rep(j, 0, 25) res += lower_bound(p[j].rbegin(), p[j].rend(), i) - p[j].rbegin();
            return res;
        };
        if(pos <= n) cmin(ans, sum + calc(pos));
        if(p[t[i] - 97].empty()) break;
        sum += calc(p[t[i] - 97].back());
        p[t[i] - 97].pop_back();
    }
    printf("%lld\n", ans < inf ? ans : -1);
}
