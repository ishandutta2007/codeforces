...
void solve(){
    ...
    bool f=false;
    For(i,m){
        curr[pt[i].F].insert(i);
        curr[pt[i].S].insert(i);
    }
    for(int i=x+1;i<=n;i++){
        if (curr[i].size()+curr[x].size()<m) continue;
        int cnt=curr[x].size();
        for(auto y:curr[i]){
            if (curr[x].find(y)==curr[x].end()) ++cnt;
        }
        if (cnt==m){
            f=true;
            d1("YES");
