...
/*Main-head---------------------------------------------------------------------------------------*/
    ...
    cin>>n;
    do{
        if((int)o.size()>n)break;
        for(int i=0;i<(int)o.size();++i)if(o[i]*2<=2*n*n)o.push_back(o[i]*2);
        if((int)o.size()>n)break;
        for(int i=0;i<(int)o.size();++i)if(o[i]*3<=2*n*n)o.push_back(o[i]*3);
        if((int)o.size()>n)break;
        for(int i=0;i<(int)o.size();++i)if(o[i]*5<=2*n*n)o.push_back(o[i]*5);
        if((int)o.size()>n)break;
        for(int i=0;i<(int)o.size();++i)if(o[i]*7<=2*n*n)o.push_back(o[i]*7);
        if((int)o.size()>n)break;
        for(int i=0;i<(int)o.size();++i)if(o[i]*11<=2*n*n)o.push_back(o[i]*11);
    }
    while(0);
    for(int x=0;x<n;x++)
    cout<<*(o.rbegin()+x)<<' ';
