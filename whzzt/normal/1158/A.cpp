...
int n,m,b[N],g[N],ff[N];
i64 sum;
int main(){
	scan(n,m);
	read(b+1,b+n+1); sort(b+1,b+n+1);
	read(g+1,g+m+1); sort(g+1,g+m+1);
	for(int x=1;x<=n;++x) sum+=b[x]; sum*=m;
	if(b[n]>g[1]) puts("-1"),exit(0);
	for(int x=1;x<=m;++x){
		int p=upper_bound(b+1,b+n+1,g[x])-b-1;
		if(!p) puts("-1"),exit(0);
		sum+=g[x]-b[p];
		if(g[x]!=b[p]) ff[p]++;
	}
	if(ff[1]==m) puts("-1"),exit(0);
	for(int x=2;x<=n;++x) if(ff[x]==m) sum+=b[x]-b[x-1];
	cout<<sum<<endl;
}