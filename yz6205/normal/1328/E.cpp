...
void dfs(int u, int f) {
	fa[u][0] = f;
	repa (i,29) fa[u][i] = fa[ fa[u][i-1] ][ i-1 ];
	dep[u] = dep[f]+1;
	for (int v: e[u]){
		if (v == f) continue;
		dfs(v, u);
...
void solve() {
	...
	rep (_,q) {
		int k; scanf(I,&k);
		vector < int > q; rep (i,k) q.push_back( sc.nextInt() );
		sort(q.begin(), q.end(), [](int u, int v) {
			return dep[u] > dep[v];
		});
		int pos = q[0];
		for (int u: q) {
			//while (dep[pos] >= dep[u]) pos = fa[pos];
			pos = up(pos, dep[pos] - dep[u] + 1);
			if (alert(pos != fa[u][0], "NO")) goto nxt;
		}
		puts("YES");
		nxt:;
	}
