...
int main(){
	...
	while(T--)
	{
		...
		for(int i=0;i<=n+1;i++)sum[i]=t[i]=0;
		int nr=0;
		for(int i=1;i<=n+1;i++)die[i]=0,rg[i]=-1;
		int l=b[0],r=b[0];
		t[a[l]]=1;
		int ct=0;
		for(int i=0;i<n;i++)
		{
			...
			int ty=b[i];
			die[ty]=1;
			while(l>ty)l--,t[a[l]]=1;
			while(r<ty)r++,t[a[r]]=1;
			++ct;
			while(t[nr])rg[nr]=(r-l+1)-ct,nr++;
		}
		for(int i=1;i<=n;i++)if(die[i])rg[a[i]]=-1;
		vector<int>v;
		for(int i=0;i<=n;i++)
			if(rg[i]!=-1)v.push_back(rg[i]);
		sort(v.begin(),v.end());
		ll ans=1;
		int re=0;
		for(auto it:v)
