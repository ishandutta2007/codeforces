// time-limit: 1000
// problem-url: https://codeforces.com/contest/1704/problem/F

// eh... what happened with E?
...
void up(){
	...
	//{0, 1, 1};
{0, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 4, 0, 5, 2, 2, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 2, 7, 4, 0, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 
4, 4, 5, 5, 2, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7, 4, 8, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 
4, 4, 5, 5, 9, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7, 4, 8, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 
4, 4, 5, 5, 9, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7, 4, 8, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 
4, 4, 5, 5, 9, 3, 3, 0, 1, 1, 3, 0, 2, 1, 1, 0, 4, 5, 3, 7, 4, 8, 1, 1, 2, 0, 3, 1, 1, 0, 3, 3, 2, 2, 
}; // this number 34 seems familiar...
	while(int(g.size())<=std::max(n, 500)){ // too lazy. Is linear anyway
		auto index=int(g.size());

		/*
		std::vector<bool> seen(index+1);
		seen[g[index-2]]=true;
		for(int const i: range(0, index-3+1)){
			seen[g[i] xor g[index-3-i]]=true;
		}
		int i=0;
		while(seen[i]) i++;
		*/
	...
	auto const numB=int(std::count(begin(s), end(s), 'B'));
	auto const numR=n-numB;
...
Bob:
	...
	while(i<n-1){
		if(s[i]==s[i+1]) i++;
			...
			int oldI=i;
			do i++; while(i<n-1 and s[i]!=s[i+1]);
			result=result xor g[i-oldI];
		}
	}
	if(result==0) goto Bob; else goto Alice;
}