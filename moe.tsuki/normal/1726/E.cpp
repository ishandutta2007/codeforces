...
{
	f[0] = f[1] = 1;
	F (i, maxn - 3) f[i + 2] = ((((f[i] + f[i + 1]) % mod) * fact(i + 1, 2)) % mod * fact(i + 2, 3)) % mod;
	F (i, maxn - 1) f[i] = f[i] * fact(i, 2) % mod;
	//F (i, 10) dbg(f[i]);
	while (t --> 0)
	{
		int n; cin >> n;
		...
		F (i, n) if (i * 4 <= n)
		{
			ans = (ans + ((C(n - i - i, i + i) * fact(i + i, 2) % mod) * fact(i, 3) % mod) * f[n - i * 4] % mod) % mod;
		}
		cout << ans << '\n';
	}
}
