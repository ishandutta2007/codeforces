...
int main(){
  ...
  ld ret=0;
  rep(i,0,n)ret+=2*distance(p[i],t);
  pair<ld,ll>mxa1(INF,-1);
  pair<ld,ll>mxa2(INF,-1);
  pair<ld,ll>mxb1(INF,-1);
  pair<ld,ll>mxb2(INF,-1);
  rep(i,0,n){
    ld da=distance(p[i],a)-distance(p[i],t);
    auto pre=mxa1;
    if(!chmin(mxa1,MP(da,i)))chmin(mxa2,MP(da,i));
    else mxa2=pre;
    ld db=distance(p[i],b)-distance(p[i],t);
    pre=mxb1;
    if(!chmin(mxb1,MP(db,i)))chmin(mxb2,MP(db,i));
    else mxb2=pre;
  }
  ld add=min(mxa1.fi,mxb1.fi);
  if(mxa1.se!=mxb1.se)chmin(add,mxa1.fi+mxb1.fi);
  else{
    chmin(add,mxa1.fi+mxb2.fi);
    chmin(add,mxa2.fi+mxb1.fi);
  }
  //cout<<mxa1<<endl;cout<<mxa2<<endl;cout<<mxb1<<endl;cout<<mxb2<<endl;
  //cout<<ret spa add<<endl;
  lfs<<ret+add<<endl;
  return 0;
}