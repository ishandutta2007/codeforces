...
int main()
{
	...
	int K;cin>>K;
	vector<vector<pair<int,int>>>vs(K);
	for (int i = 0; i < K; ++i) {
		int n;cin>>n;
		map<int,int>mp;
		int sum=0;
		...
		for (auto m : mp) {
			vs[i].push_back(make_pair(sum-m.first,m.second));
		}
	...
	for (int i = 0; i < vs.size(); ++i) {
		auto v(vs[i]);
		for (auto n : v) {
			if (mp.find(n.first) != mp.end()) {
				pair<int,int>ans1=make_pair(i,n.second);
				pair<int,int>ans2=mp[n.first];
				cout<<"YES"<<endl;

				cout<<ans1.first+1<<" "<<ans1.second<<endl;
				cout<<ans2.first+1<<" "<<ans2.second<<endl;
				return 0;
			}
			else {
				mp[n.first]=make_pair(i,n.second);
			}
		}
	}
	cout<<"NO"<<endl;
