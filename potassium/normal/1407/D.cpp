...
struct MonoStack {
	...
	vector<int> push(int x) {
		v.clear();
		bool dup = false;
		while (!val.empty() && !cmp(val.back(), x)) {
			if (!cmp(x, val.back())) dup = true;
			v.push_back(val.back());
			val.pop_back();
		}
	...
	MonoStack(function<bool(T, T)> cmp) : cmp(cmp) { }
};
...
int main() {
	...
	vector<int> dp(n, 0);
	auto minDp = [&](int x, int y) {
		return dp[x] < dp[y];
	};
	
	for (int i = 0; i < n; i++) {
		vector<int> vx = inc.push(i);
		vector<int> vy = dec.push(i);

		int x = (vx.empty() ? 1e9 : dp[*min_element(all(vx), minDp)]);
		int y = (vy.empty() ? 1e9 : dp[*min_element(all(vy), minDp)]);
