...
inline void solve() {
	...
	for (int i = 0; i < n; ++i) {
		sort(all(rw[i]));
		rw[i].erase(unique(all(rw[i])), rw[i].end());
	}
	for (int j = 0; j < m; ++j) {
		sort(all(col[j]));
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < m; ++j) {
			int posr = lower_bound(all(rw[i]), a[i][j]) - rw[i].begin();
			int posc = lower_bound(all(col[j]), a[i][j]) - col[j].begin();

			int cnt = max(posr, posc);
			int cnt2 = max(sz(rw[i]) - posr, sz(col[j]) - posc);
