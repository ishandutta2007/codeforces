...
void spfa(int st)
{
    for(int i = 0; i < n; i++) dp[i] = LINF, vis[i] = 0;
    ...
    for(int i = 0; i < n; i++)
    {
        int best = -1;
        for(int j = 0; j < n; j++)
            if(vis[j] == 0 && (best == -1 || dp[best] > dp[j]))
                best = j;

        vis[best] = 1;
        get_min(dp[(best + 1) % n], dp[best] + 1);
        for(auto pr : G[best])
        {
            int v = pr.fi;
            int c = pr.se;
            get_min(dp[(v + dp[best]) % n], dp[best] + c);
        }
    }
    dp[st] = 0;
}
...
void work()
{
    ...
    {
        spfa(i);
        for(int j = 0; j < n; j++) printf("%lld%c", dp[j], " \n"[j == n - 1]);
    }
}
