// Equidistant Vertices (https://codeforces.com/contest/1551/problem/F)
...
int main() {
    ...
    while (t--) {
        ...
        MI7 ans = 0;
        rep(v0, n) {
            vector dist_groups(n / 2, vector<int>());
            ...
            trav(v2, adj[v0]) {
                ...
                rep(i, 1, n / 2)
                    if (dist_cnt[i])
                        dist_groups[i].push_back(dist_cnt[i]);
            }

            rep(d, 1, n / 2) {
                vector<MI7> pos(k + 1, 0);
                pos[0] = 1;
                trav(g, dist_groups[d])
                    repr(i, k, 1)
                        pos[i] += pos[i - 1] * g;
                ans += pos[k];
            }
