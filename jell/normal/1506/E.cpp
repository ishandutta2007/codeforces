#line 1 "codeforces-workspace/contest/1506/e/e.cpp"
...
#line 11 "codeforces-workspace/contest/1506/e/e.cpp"
...
void main() {
  ...
  int n;
  cin >> n;
  vector<int> q(n), p(n), app(n + 1);
  cin >> q;

  for (auto &&[x, y] : zip(q, p)) {
    if (app[x]) {
      continue;
    }
    app[x] = 1;
  ...
  vector<int> mi(p), ma(p);
  set<int> is, as;
  int pre = 0;

  for (auto &&[x, y] : zip(mi, ma)) {
    if (x) {
      for (auto z : range(pre + 1, x)) {
        is.emplace(z);
        as.emplace(z);
      }
      pre = x;
