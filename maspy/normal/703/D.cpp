...
#line 2 "/home/maspy/compro/library/other/uniqueproductquery.hpp"
...
#line 1 "/home/maspy/compro/library/alg/monoid_xor.hpp"
template <typename T>
struct Monoid_Xor {
  using value_type = T;
  using X = value_type;
...
void solve() {
  ...
  FOR(q, Q) {
    LL(L, R);
    X.add(--L, R);
    range_xors[q] = seg.prod(L, R);
  }
  auto unique_xors = X.run();
  vc<int> ANS(Q);
  FOR(q, Q) ANS[q] = range_xors[q] ^ unique_xors[q];
