...
const int LEFT = 1, RIGHT = 2, SAME = 0;
...
void solve() {
    ...
    while (sz(players) >= 4) {
        ...
        for (int i = 0; i < sz(players); i += 4) {
            int a = players[i], b = players[i + 1], c = players[i + 2], d = players[i + 3];
    ...
    if (sz(players) == 1) {
        finish(players.front());
    } else if (sz(players) == 2) {
        auto result = query(players.front(), players.back());
        if (result == LEFT)
            finish(players.front());
