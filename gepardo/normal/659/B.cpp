...
main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        string s = in; int a = (int)in - 1, b = in;
        v[a].pb({b, s});
    }
    for (int i = 0; i < m; i++) sort(v[i].begin(), v[i].end());
    for (int i = 0; i < m; i++) reverse(v[i].begin(), v[i].end());
    for (int i = 0; i < m; i++)
    {
        if (v[i].size() == 2) out.putLn(v[i][0].ss, ' ', v[i][1].ss);
            else if (v[i][1].ff != v[i][2].ff) out.putLn(v[i][0].ss, ' ', v[i][1].ss);
                else out.putLn('?');
    }
    return 0;
}