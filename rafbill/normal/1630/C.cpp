...
struct segt {
  ...
  void set_(int i, int a, int b, int x, li v) {
    if(a == b) {
      A[i].v = v;
      return;
    }
    ...
    if(x <= c) {
      ...
      set_(2 * i + 1, c + 1, b, x,v);
    }
    update(i);
  }

  void set(int l, int r) {
    return set_(1, 0, n - 1, l, r);
  }

  li query_(int i, int a, int b, int l, int r) {
    if(l <= a && b <= r) {
      return A[i].v;
    }
    int c = (a + b) / 2;
    if(r <= c) {
      ...
      return query_(2 * i + 1, c + 1, b, l, r);
    } else {
      return max(query_(2 * i, a, c, l, r),
...
int main(){
  ...
  li ans = 0;
  for(li i = 0; i < (li)(n); ++i) {
    smax(ans, plop[A[i]]+i-1);
    if(seen[A[i]]) {
      smax(ans, st.query(first[A[i]],i)+i-1);
    }
    smax(plop[A[i]], ans-i);
    seen[A[i]]=1;
    st.set(i,ans-i);
