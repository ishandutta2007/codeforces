...
struct Sum2D{
  ...
  T query(ll x1,ll x2,ll y1, ll y2){
    //if(x1>x2)swap(x1,x2);if(y1>y2)swap(y1,y2);
    if(x1<0||x2>h||y1<0||y2>w||x1>=x2||y1>=y2)return 0;
    else return data[x2][y2]-data[x2][y1]-data[x1][y2]+data[x1][y1];
  }
};
int main(){
  cin.tie(nullptr);
  ...
  vector<string>s(h);
  rep(i,0,h)cin>>s[i];
  auto tate=vec(h,w,0LL);
  auto yoko=vec(h,w,0LL);
  rep(i,0,h)rep(j,0,w){
    if(i<h-1&&s[i][j]=='.'&&s[i+1][j]=='.')tate[i][j]=1;
    if(j<w-1&&s[i][j]=='.'&&s[i][j+1]=='.')yoko[i][j]=1;
  }
  Sum2D ts(tate);
  Sum2D ys(yoko);
  ll q;cin>>q;
  while(q--){
    ...
    r1--;c1--;
    ll ret=0;
    ret+=ts.query(r1,r2-1,c1,c2);
    ret+=ys.query(r1,r2,c1,c2-1);
    cout<<ret<<endl;
  }
  return 0;
}