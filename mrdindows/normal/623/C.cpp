...
void build(int n) {
    ...
    trs[n + 1] = {INF, -INF};
    for (int i = 1; i <= n; ++i) {
        trp[i].first = min(trp[i - 1].first, p[i - 1].second);
        trp[i].second = max(trp[i - 1].second, p[i - 1].second);
        
        trs[n - i + 1].first = min(trs[n - i + 2].first, p[n - i].second);
        trs[n - i + 1].second = max(trs[n - i + 2].second, p[n - i].second);
    }
}
...
void solve() {
    ...
    res = min(res, (ymax - ymin) * (ymax - ymin));
    res = min(res, (xmax - xmin) * (xmax - xmin));
    sort(p, p + n);
    build(n);
    int lp = 0;
    int rp = 0;
    while (lp < n) {
        auto upd = [&](int lp, int rp) {
            if (lp > rp) return false;
            pii leftr = get(0, lp - 1);
            pii rightr = get(rp + 1, n - 1);
            ll ymin = min(leftr.first, rightr.first);
            ll ymax = max(leftr.second, rightr.second);
            ll xmin = p[lp].first;
            ll xmax = p[rp].first;
            ll yabs = max(abs(ymin), abs(ymax));
            ll xabs = max(abs(xmin), abs(xmax));
            ll cur = max(xabs * xabs + yabs * yabs, max(sqr(ymax - ymin), sqr(xmax - xmin)));
            res = min(res, cur);
            return cur > sqr(xmax - xmin);
        };
        while (upd(lp, rp) && rp < n - 1) {
            ++rp;
        }
        ++lp;
    }
