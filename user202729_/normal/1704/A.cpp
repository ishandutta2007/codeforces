// time-limit: 1000
// problem-url: https://codeforces.com/contest/1704/problem/A

// ah right this is div 1+2 contest...
...
void up(){
	int const lenA=read<int>();
	int const lenB=read<int>();
	std::string const a=read<std::string>();
	std::string const b=read<std::string>();
	...
	std::cout<<((
			std::any_of(a.begin(), a.end()-(lenB-1), [&](auto const& it){
		return it==b[0];
	}) and std::equal(a.end()-(lenB-1), a.end(), b.begin()+1))
			? "YES\n": "NO\n");
