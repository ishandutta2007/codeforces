...
// cin>>T;while(T--)
{
    ...
    for(lli msk=1;msk<lim;++msk){
        const int i=msk&(-msk),oth=msk^i;
        aux[msk]=aux[i]+aux[oth];
        auto &cur=values[msk];
        for(int j=0;j<N;++j)
            cur[j]=min(values[i][j],values[oth][j]);
        auto &cnt=dp[msk];
        cnt=1;
        for(int j=0;j<N;++j)
            cnt*=cur[j]+1;
    ...
    //     auto &cnt=answers[msk];
    //     for(lli smsk=msk;;smsk=(smsk-1)&msk){
    //         cnt+=dp[smsk];
    //         if(smsk==0)
    //             break;
    //     }
    ...
    auto print=[&]()->lli{
        ...
        lli ans=0;
        for(lli msk=1;msk<lim;++msk){
            ans^=(aux[msk]&63)*(aux[msk]>>6)*(dp[msk].val());
        }
        return ans;
    };
