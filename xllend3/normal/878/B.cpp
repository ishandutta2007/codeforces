...
int main(){
	...
	int nn=n,sum=0;n=0;
	rep(i,1,nn){
		if(n&&a[i]==a[n])++sum;else sum=1;
		a[++n]=a[i];
		if(sum==t){
			n-=t;sum=0;
			dep(j,n,1)if(a[j]==a[n])++sum;else break;
		}
	}
	int l=1,r=n;
	while(1){
		int ll=l,rr=r;
		while(a[ll]==a[l]&&ll<=rr)++ll;--ll;
		while(a[rr]==a[l]&&ll<rr)--rr;++rr;
		//printf("%d %d %d %d\n",l,ll,rr,r);
		while(ll-l+1+r-rr+1>=t){
			int p=min(ll-l+1,t);
			l+=p;r-=t-p;
		}
		if(a[l]!=a[ll]&&a[r]!=a[ll]);else break;
	}
	//printf("%d %d\n",l,r);
	rep(i,l,r)S.insert(a[i]);
	if(SZ(S)==1){
		//puts("A");
		if(1ll*(r-l+1)*m%t!=0)printf("%lld\n",1ll*(r-l+1)*m%t+l-1+(n-r));
		else{
			int sum=0,top=0;
			rep(i,1,l-1){
				if(top&&a[i]==b[top])++sum;else sum=1;
				b[++top]=a[i];
				if(sum==t){
					sum=0;top-=t;
					dep(j,top,1)if(b[top]==b[j])++sum;else break;
				}
			}
			rep(i,r+1,n){
				if(top&&a[i]==b[top])++sum;else sum=1;
