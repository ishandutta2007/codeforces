...
void solve() {
	...
	scanf("%d", &n);
	vpii in(n);
	for (auto &[a, b] : in) scanf("%d%d", &a, &b);
	bool pos = false, neg = false;
	for (auto [x, y] : in) if (y < 0) neg = true; else if (y > 0) pos = true;
	if (pos && neg) {
		puts("-1");
		return;
	}
	...
	low /= 2;
	while (high - low > 1e-9 && high / low > 1 + 1e-9) {
		vector <pair <ld, int> > line;
		ld r = (low + high) / 2;
		for (auto [x, y] : in) {
			ld off = sqrt(K(r) - K(r - y));
			line.emplace_back(x - off, 1);
			line.emplace_back(x + off, -1);
		}
		sort(ALL(line));
		int cou = 0;
