...
namespace Poly
{
	const int mod=998244353,G=3,Gi=332748118,Mx=2100000;
	int rev[Mx+2],w[Mx+2],wi[Mx+2],inv[Mx+2];
	...
	inline void NTT(vector<int> &A,bool o)
	{
		...
		for(int i=1;i<n;i<<=1)for(int j=0;j<n;j+=(i<<1))for(int k=0;k<i;++k)tmp=((LL)A[i+j+k]*(o? w[i+k]:wi[i+k]))%mod,add(A[i+j+k]=A[j+k],mod-tmp),add(A[j+k],tmp);
	}
	inline vector<int> Mul(vector<int> A,vector<int> B)
	{
		int n=A.size()-1,m=B.size()-1,t,len,mid,W,Wi,inv;
		for(t=-1,len=1;len<=n+m;len<<=1,++t);
		mid=len>>1,W=Pow(G,(mod-1)/len),Wi=Pow(Gi,(mod-1)/len),w[mid]=wi[mid]=1,inv=Pow(len,mod-2);
		for(int i=1;i<len;++i)rev[i]=(rev[i>>1]>>1)|((i&1)<<t);
		for(int i=mid+1;i<len;++i)w[i]=((LL)w[i-1]*W)%mod,wi[i]=((LL)wi[i-1]*Wi)%mod;
	...
	inline vector<int> Int(vector<int> A)
	{
		...
		for(int i=A.size()-1;i;--i)A[i]=((LL)A[i-1]*inv[i])%mod;
		return A[0]=0,A;
	}
	...
	inline void Solve()
	{
		int n,m;
		vector<int> A,B;
		scanf("%d%d",&n,&m),A.resize(100001);
		for(int i=1,x;i<=n;++i)scanf("%d",&x),A[x]=1;
		B=Mul(A,-4),add(B[0],1),B=Sqrt(B,m+1),add(B[0],1),B=Mul(Inv(B,m+1),2);
