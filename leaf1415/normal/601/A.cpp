...
void bfs()
{
	...
	rep(i, 1, n) rep(j, 1, n) rep(k, 0, 1) dist[i][j][k] = inf;
	dist[1][1][0] = 0;
	
	while(Q.size()){
		T t = Q.front(); Q.pop();
		ll u = t.first.first, v = t.first.second, z = t.second;
		rep(i, 1, n){
			ll nu = u, nv = v, nz = 1-z;
			if(z == 0){
				if(G[u][i] == 1) continue;
			...
			if(z == 1 && nu == nv && nu != n) continue;
			
			if(dist[nu][nv][nz] < inf/2) continue;
			Q.push(T(P(nu, nv), nz));
			dist[nu][nv][nz] = dist[u][v][z] + 1;
		}
	}
}
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, n) rep(j, 1, n) if(i != j) G[i][j] = -1;
	ll u, v;
	rep(i, 1, m){
		cin >> u >> v;
		G[u][v] = G[v][u] = 1;
