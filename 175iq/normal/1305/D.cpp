...
void solve()
{
    ...
    function<void(int, int)> ask_query = [&](int u, int v)
    {
        cout<<"? "<<u<<" "<<v<<endl;
        return;
    };

    function<int()> read_query_output = [&]()
    {
        cini(val);
        return val;
    };
    ...
    while(leaves.sz >= 2)
    {
        ...
        {
            int neig_v1 = *gr[v1].begin();
            int neig_v2 = *gr[v2].begin();
            ...
            if(deg[neig_v1] == 1)
                leaves.insert(neig_v1);

            if(deg[neig_v2] == 1)
                leaves.insert(neig_v2);
        }
    }
