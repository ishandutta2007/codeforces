...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int q = io.readInt();
            for (int i = 0; i < q; i++) {
                solveSingle(io.readLong(), io.readLong(), io.readLong());
            }
        }

        public void solveSingle(long a, long b, long m) {
            long min = a;
            long max = a;
            if (a == b) {
                io.cache.append(1).append(' ').append(a).append('\n');
                return;
            }
            if (b <= a + m) {
                io.cache.append(2).append(' ').append(a).append(' ').append(b).append('\n');
                return;
            }
            long[] rs = new long[50];
            int n = 1;
            ...
            while (true) {
                ...
                long bit = 1;
                for (int i = n; i >= 2 && r > 0; i--, bit = bit << 1) {
                    if ((r & bit) > 0) {
                        rs[i]++;
                        r -= bit;
                    }
                    long c = Math.min(r / (bit << 1), (m - rs[i]) / 2);
                    rs[i] += 2 * c;
                    r -= c * (bit << 1);
                }

                long x = a;
                io.cache.append(n + 1).append(' ');
                io.cache.append(x).append(' ');
                for (int i = 2; i <= n; i++) {
                    long y = x;
                    x = x * 2 + rs[i];
                    io.cache.append(x - y).append(' ');
                }
