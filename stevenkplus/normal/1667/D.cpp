...
int main() {
   ...
   while (T--) {
      ...
      yc([&](auto rec, int cur) -> void {
         ...
         if (ch % 2 == 0) {
            DP[cur][0] = cnt0 == ch / 2 && cnt1 == ch / 2;
            DP[cur][1] = cnt0 == ch / 2 + 1 && cnt1 == ch / 2 - 1;
         } else {
            DP[cur][0] = cnt0 == ch / 2 && cnt1 == ch / 2 + 1;
            DP[cur][1] = cnt0 == ch / 2 + 1 && cnt1 == ch / 2;
         }
      })(root);
      int x = ed[root][0];
      bool ans = DP[x][0];
      ...
      yc([&](auto rec, int cur) -> void {
         ...
         for (int nxt: ed[cur]) {
            ...
            par[nxt] = cur;
            ++ch;
            if (DP[nxt][0]) v0.push_back(nxt);
            if (DP[nxt][1]) v1.push_back(nxt);
         }
         if (ch % 2 == 0) {
            if (DP[cur][0]) {
               pp("A");
