...
private:
	...
	struct initializer{
		...
		initializer(){
			for(int p=1;p<=L;++p){
				int buf1=qpow_constexpr(G,(mod-1)/(1<<p),mod);
				int buf0=qpow_constexpr(invG,(mod-1)/(1<<p),mod);
				GG[0][p].resize(1<<p);
				GG[1][p].resize(1<<p);
				GG[0][p][0]=GG[1][p][0]=1;
				for(int i=1;i<(1<<p);++i){
...
int sp[N],s[N],n,m,a[N],pw2[N],g[N];
...
void CDQ(int L,int R){
	...
	poly A(mid-L+1),B(R-mid);
	for(int i=L;i<=mid;++i){
		A[i-L]=pw2[s[max(0,i)]];
	}
	for(int i=mid+1;i<=R;++i){
		B[i-(mid+1)]=i<0?0:pw2[m-s[min(n,i)]];
	}
	A=Mul(A,B);
	for(int i=L+mid+1;i<=mid+R;++i){
		if(i>=0&&i<=2*n){
			g[i]=(g[i]+A[i-(L+mid+1)])%mod;
		}
	}
}
int main(){
	...
	for(int i=1;i<=n;++i){
		ll tot=0;
		for(int j=0;j<n;++j){
			tot+=pw2[m-(s[min(i+j,n)]-s[max(0,i-j-1)])]-1;
		}
		ans=(ans+tot%mod*a[i])%mod;
		cerr<<" !! "<<i<<" "<<tot<<endl;
	} 
	 */    
	CDQ(-n,2*n);
	for(int i=1;i<=n;++i){
		ans=(ans+1LL*(g[2*i-1]-n-min(i,n-i+1))*a[i])%mod;
	}                              
	ans=(ans+mod)%mod;                                                                                                                         
	ans=1LL*ans*qpow(pw2[m]-1,mod-2)%mod;
	cout<<ans<<'\n';
	return 0;
}