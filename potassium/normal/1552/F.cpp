...
int main() {
	int n; R(n);
	vector<pair<pii, int>> portal(n); R(portal);

	const int SHIFT = 100;
	vector<pii> pos(2 * n);
	for (int i = 0; i < n; i++) {
		pos[i * 2] = { portal[i].first.first, -(i + SHIFT) };
		pos[i * 2 + 1] = { portal[i].first.second, i + SHIFT };
	}
	sort(all(pos));
	...
	for (int i = 0; i < 2 * n; i++) {
		if (pos[i].second > 0) {
			l[abs(pos[i].second) - SHIFT] = i;
		}
		else {
			r[abs(pos[i].second) - SHIFT] = i;
		}
	}

	vector<Mint> ps(2 * n, Mint(0, MOD998));
	vector<Mint> plen(n, Mint(0, MOD998));
	for (int i = 0; i < 2 * n; i++) {
		if (i > 0) ps[i] = ps[i - 1];
		if (pos[i].second < 0) {
			int id = abs(pos[i].second) - SHIFT;
			Mint width = Mint(portal[id].first.first - portal[id].first.second, MOD998);
			Mint rem = ps[i - 1] - ps[l[id]];
			plen[id] = width + rem;
			ps[i] += plen[id];
		}
	}

	Mint ans(portal[n - 1].first.first + 1, MOD998);
	for (int i = 0; i < n; i++)
		if (portal[i].second)
			ans += plen[i];
