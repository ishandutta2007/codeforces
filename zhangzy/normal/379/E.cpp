...
double doit(int t,point le,point ri,int type)
{
	//printf("(%.2lf,%.2lf)-(%.2lf,%.2lf) (%.2lf,%.2lf)-(%.2lf,%.2lf)\n",room[t].st.x,room[t].st.y,room[t].en.x,room[t].en.y,le.x,le.y,ri.x,ri.y);
	...
	if ((le.y>=room[t].st.y-eps&&ri.y>=room[t].en.y-eps)||type)
	{
...
int main()
{
	...
	for (int ttt=1;ttt<=n;ttt++)
	{
		...
		for (int i=1;i<=cnt;i++)
			ans+=doit(i,a[room[i].bel-1],a[room[i].bel],0);
		cout<<ans<<endl;
	}
