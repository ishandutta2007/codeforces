...
// }}}
using mn = modnum<int(1'000'000'007)>;
using vmn = vector<mn>;
...
int main() {
    ...
    auto left = [&](int i) -> mn {
        mn tens = mn(10).pow(n - 1 - i);
        mn ways = mn::ncr(i + 1, 2);
        return tens * ways;
    };
    vmn rv;
    rv.pb(0);
    F0R (i, n) { rv.pb(rv.back() + mn(i) * mn(10).pow(i - 1)); }
    auto right = [&](int i) -> mn { return rv[n - i]; };

    mn ans;
    F0R (i, n) { ans += a[i] * (left(i) + right(i)); }
