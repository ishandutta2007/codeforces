...
struct Graph{
    ...
    void remove(int root, int d){
        ...
        remove(2*root, d+1), remove(2*root+1, d+1);
    }

    int search(int root, int d){
        int ptr = root;
        while(true){
            if(ptr >= n/2 || (a[2*ptr] == 0 && a[2*ptr+1] == 0)) break;
            if(a[2*ptr] > a[2*ptr+1]) ptr = 2*ptr, d++;
            else ptr = 2*ptr+1, d++; 
        }
        return d;
    }
    ...
    void dfs(int ptr){
        if(ptr >= n/2 || (a[2*ptr] == 0 && a[2*ptr+1] == 0)) a[ptr] = 0;
        else if(a[2*ptr] > a[2*ptr+1]) a[ptr] = a[2*ptr], dfs(2*ptr);
        else a[ptr] = a[2*ptr+1], dfs(2*ptr+1);
    }
