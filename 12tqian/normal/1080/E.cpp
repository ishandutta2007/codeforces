...
ll countPal(vi& s) {
    ...
    ll ret = 0;
	trav(x, ans) ret += (x+1)/2;
    f0r(i, sz(ans)){
        if(i%2 == 0){
            if(s[i/2] <0) ret -= (ans[i] + 1)/2;
        }
    }
	return ret;
}
...
int main(){
    ...
    f0r(row, n){
        f0r(i, m){
            ...
            f1r(j,i,m){
                cnt[grid[row][j]]++;
                cur = ad(cur, po[grid[row][j]]);
                int val = cnt[grid[row][j]];
                if(val%2 == 0) numOdd--;
                else numOdd++;
                if(numOdd<=1) hsh[row][i][j] = cur;
            }
        }
    }
    ll ans = 0;
    f0r(i, m){
        f1r(j, i, m){
            ...
            int num = 0;
            f0r(row, n){
                if(hsh[row][i][j] != 0) use.eb(hsh[row][i][j]);
                else{
                    use.eb(bad);
                    bad--;
                    num++;
