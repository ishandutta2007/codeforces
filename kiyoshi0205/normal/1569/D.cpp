...
ll solve(pvec x,pvec vx){
  if(vx.empty())return 0;
  pvec ev(x);
  for(auto t:vx)ev.emplace_back(t);
  vsort(ev);
  ll cnt=0,ans=0;
  map<ll,ll> mp;
  for(auto [a,b]:ev){
...
int main(){
  ...
  rep(i,K){
    cin>>A>>B;
    auto xitr=lower_bound(all(x),datas(A,-1));
    if(xitr==x.end()||*xitr!=datas(A,-1)){
      vx.emplace_back(A,B);
      continue;
    }
    auto yitr=lower_bound(all(y),datas(B,-1));
    if(yitr==y.end()||*yitr!=datas(B,-1)){
      vy.emplace_back(B,A);
      continue;
    }
  }
  print(solve(x,vx)+solve(y,vy));
}
}