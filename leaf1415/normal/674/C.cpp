...
struct CHT{
	...
	{
		double a1 = p1.first, b1 = p1.second;
		double a2 = p2.first, b2 = p2.second;
		double a3 = p3.first, b3 = p3.second;
		return (a2-a1)*(b3-b2) >= (b2-b1)*(a3-a2);
	}
	double calc(P p, double x){
...
int main(void)
{
	...
	rep(i, 1, n) c[i] = c[i-1] + (double)s[i]/a[i];
	rep(i, 1, n) d[i] = d[i-1] + 1.0/a[i];
	
	cht[0].add(0, 0);
	rep(i, 1, k) cht[i].add(0, inf);
	
	rep(i, 1, n) per(j, k, 1){
		double dp = c[i] + cht[j-1].query(d[i]);
		if(i == n && j == k){
			printf("%.11f\n", dp);
			return 0;
		}
		cht[j].add(-s[i], dp-c[i]+d[i]*s[i]);
	}
