...
ZN solve2(li i, li x, vi A, vi B) {
  if(A.empty() || B.empty()) return 0;
  if(i < 0) {
    return (p2[A.size()]-1)*(p2[B.size()]-1);
  }
  vi A0,A1;
  for(li a : A) {
    if(a&(1ll<<i)) A1.emplace_back(a); else A0.emplace_back(a);
  }
  vi B0,B1;
  for(li a : B) {
    if(a&(1ll<<i)) B1.emplace_back(a); else B0.emplace_back(a);
  }
  if(x&(1ll<<i)) {
    ZN r0 = solve2(i-1,x,A0,B1);
    ZN r1 = solve2(i-1,x,A1,B0);
    return r0*r1 + r0 * (p2[A1.size()]+p2[B0.size()]-1) + r1 * (p2[A0.size()]+p2[B1.size()]-1) +
      (p2[A0.size()]-1)*(p2[B0.size()]-1) + (p2[A1.size()]-1)*(p2[B1.size()]-1);
  }else{
    return solve2(i-1,x,A0,B0) + solve2(i-1,x,A1,B1);
  }
}

ZN solve1(int i, li x, vi A) {
  if(A.empty()) return 0;
  if(i < 0) return p2[A.size()]-1;
  vi A0,A1;
  for(li a : A) {
    if(a&(1ll<<i)) A1.emplace_back(a); else A0.emplace_back(a);
  }
  if(x&(1ll<<i)) {
    return (p2[A0.size()]-1) + (p2[A1.size()]-1) + solve2(i,x,A0,A1);
  }else{
    return solve1(i-1,x,A0) + solve1(i-1,x,A1);
  }
}
...
int main(){
  ...
  p2[0]= 1;
  for(li i = 0; i < (li)(222'200); ++i) p2[i+1] = p2[i]*ZN(2);

  li n,x; cin>>n>>x;
  vi A(n); for(li i = 0; i < (li)(n); ++i) cin>>A[i];

  auto ans = solve1(30,x,A);
  cout << ans << endl;
