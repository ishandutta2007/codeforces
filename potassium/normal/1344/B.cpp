...
int main() {
	...
	vector<vector<bool>> trav = vector<vector<bool>>(n, vector<bool>(m, 0));
	int ans = 0;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if (s[i][j] == '#' && !trav[i][j]) {
				...
				while (!q.empty()) {
					...
					for (int dir = 0; dir < 4; dir++) {
						int newr = now.F + dr[dir];
						int newc = now.S + dc[dir];

						if (0 <= newr && newr < n && 0 <= newc && newc < m && s[newr][newc] == '#' && !trav[newr][newc]) {
							trav[newr][newc] = 1;
							q.push({newr, newc});
						}
					}
