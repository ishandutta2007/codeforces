...
struct primeSieve {
	...
	vector<pair<T,int>> factorize(T x) {
		...
		if (x <= MAXN) {
			while (x != 1) {
				if (factors.empty() || factors.back().first != mpf[x])
					factors.emplace_back(make_pair(mpf[x], 0));
			...
			for (auto i : primes) {
				...
				factors.emplace_back(make_pair(i, 0));
				while (x % i == 0) {
					factors.back().second++;
					x /= i;
				}
			}
			if (x != 1) factors.emplace_back(make_pair(x, 1));
		}
...
int main() {
	...
	vector<int> prime_idx(301, -1);
	for (int i = 0; i < 62; i++)
		prime_idx[sieve.primes[i]] = i;

	vector<ll> prime_mask(301, 0);
	for (int i = 1; i <= 300; i++)
		for (auto& j : sieve.factorize(i))
			prime_mask[i] |= 1LL << prime_idx[j.fi];

	vector<Mint> prime_phi(62);
	for (int i = 0; i < 62; i++)
		prime_phi[i] = Mint(sieve.primes[i] - 1, MOD107) / Mint(sieve.primes[i], MOD107);
	
	vector<S> st_base(n);
	for (int i = 0; i < n; i++) {
		st_base[i].prod = a[i];
		st_base[i].mask = prime_mask[a[i]];
		st_base[i].len = 1;
	}
	...
	while (q--) {
		...
		if (cmd[0] == 'T') {
			...
			int l,r,x; R(l,r,x); l--; r--;
			F lazy;

			lazy.prod = x;
			lazy.mask = prime_mask[x];
