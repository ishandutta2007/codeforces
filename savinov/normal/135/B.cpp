...
int scal(PII & a, PII & b)
{
    return a.x * b.x + a.y * b.y;
}


bool is_rect(vector <PII> & a)
{
    sort(all(a));
    do
    {
        PII t = mp(a[1].x - a[0].x, a[1].y - a[0].y);
        if (!t.x && !t.y)
            continue;
        bool ok = true;
        For(i, 1, 3)
        {
            PII tt = mp(a[(i + 1) & 3].x - a[i].x, a[(i + 1) & 3].y - a[i].y);
            if (scal(t, tt))
            {
                ok = false;
                break;
...
bool is_square(vector <PII> & a)
{
    ...
    do
    {
        ...
        PII t1 = mp(a[1].x - a[0].x, a[1].y - a[0].y);
        PII t2 = mp(a[2].x - a[1].x, a[2].y - a[1].y);
        if (ok && scal(t1, t1) == scal(t2, t2)) return true;
    }while(next_permutation(all(a)));
    return false;
}
