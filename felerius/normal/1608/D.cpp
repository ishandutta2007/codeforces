// Dominoes (https://codeforces.com/contest/1608/problem/D)
...
int main() {
    ...
    int in_w = 0, out_w = 0;
    int double_q = 0;
    bool can_be_disconnected = true, can_be_bw = true, can_be_wb = true;
    rep(_, n) {
        read(s);
        can_be_disconnected &= (s != "WW" && s != "BB");
        can_be_bw &= (s[0] != 'W' && s[1] != 'B');
        can_be_wb &= (s[0] != 'B' && s[1] != 'W');
    ...
    MI num_disconnected = (can_be_disconnected ? MI(2).pow(double_q) : MI(0));

    Combinatorics<MI> comb(n);
    MI num_valid = 0;
    rep(k, max(in_w, out_w), n + 1)
        num_valid += comb.binom(right_q, k - in_w) * comb.binom(left_q, k - out_w);

    println(num_valid - num_disconnected + can_be_bw + can_be_wb);
}