...
void dfs(int x)
{
	...
	for (auto v:lj[x]) if (v!=f[x])
	{
		g[x][0]+=max(g[v][0]+abs(l[x]-l[v]),g[v][1]+abs(l[x]-r[v]));
		g[x][1]+=max(g[v][0]+abs(r[x]-l[v]),g[v][1]+abs(r[x]-r[v]));
	}
}
int main()
{
	...
	{
		read(n);
		for (i=1;i<=n;i++) read(l[i],r[i]),lj[i].clear(),f[i]=0;
		for (i=1;i<n;i++) read(x,y),lj[x].push_back(y),lj[y].push_back(x);
		dfs(1);
		enter(max(g[1][0],g[1][1]));
	}
}