#line 1 "codeforces-workspace\\contest\\1598\\d\\d.cpp"
...
#line 10 "codeforces-workspace\\contest\\1598\\d\\d.cpp"
// #include "lib/cxx20"
...
#line 13 "codeforces-workspace\\contest\\1598\\d\\d.cpp"
...
#line 30 "codeforces-workspace\\contest\\1598\\d\\d.cpp"

namespace workspace {

using mat = matrix<i64, 3>;
using vec = matrix<i64, 1, 3>;

struct endo : mat {
  endo() : mat(mat::eye()) {}
  endo(const mat& x) : mat(x) {}
  endo operator*(const endo& x) const { return {(mat) * this * (mat)x}; }
};

struct mono : vec {
  mono operator+(mono const& x) const { return {(vec) * this + (vec)x}; }
  mono operator*(endo const& x) const { return {(vec) * this * (mat&)(x)}; }
};
...
void main() {
  ...
  lazy_segment_tree<mono, endo> sg(N, {0, 0, 1});
  endo self = mat{{{1, 0, 0}, {0, 1, 0}, {0, 1, 1}}};
  endo other = mat{{{1, 0, 0}, {1, 1, 0}, {0, 0, 1}}};
  endo eye = mat{{{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}};

  for (auto&& p : peop) {
    auto all = sg.fold();
    assert(!(all[0][0] & 1));
    auto pairs = all[0][0] / 2;
    auto cnts = all[0][1];
    ...
    for (auto x : p) {
      ...
      all = sg.fold();
      auto tmp = eye;
      tmp[2][0] = cnts - sg[x][0][1];
      sg.update(x, x + 1, tmp);
      all = sg.fold();
      sg.update(x + 1, sg.size(), other);
      all = sg.fold();
    }

    for (auto x : p) {
      sg.update(x, x + 1, self);
    }
  }
