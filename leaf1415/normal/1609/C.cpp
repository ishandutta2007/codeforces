...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) if(a[i] != 1) vec[i%e].push_back(i);
		rep(i, n+1, n+e) vec[i%e].push_back(i);
		
		ll ans = 0;
		rep(i, 1, n){
			vector<ll> &cvec = vec[i%e];
			ll p = lower_bound(all(cvec), i) - cvec.begin();
			if(cvec[p] > n) continue;
			
			
			if(cvec[p] <= n && prime[a[cvec[p]]] != a[cvec[p]]) continue;
			//if(cvec[p+1] <= n && prime[a[cvec[p+1]]] != a[cvec[p+1]]) continue;
			
			ans += (cvec[p+1]-cvec[p]) / e;
			if(i == cvec[p]) ans--;
			//outl(i, ans);
		}
		outl(ans);
