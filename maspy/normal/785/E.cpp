...
void solve() {
  ...
  vi ANS(Q + Q);
  FenwickTree<ll> bit_L(N);
  FenwickTree<ll> bit_R(N);

  auto calc = [&](auto dfs, ll L, ll R, vi QID) -> void {
    if (R == L + 1) return;
    ll M = (L + R) / 2;

    FOR3(i, L, M) bit_L.add(i, 1);
    FOR3(i, M, R) bit_R.add(i, 1);
    FOR3(i, L, R) A[i] = i;
    vi QID_L, QID_R;
    ...
    FORIN(q, QID) {
      ...
      if (i < M) {
        ...
        QID_R.eb(q);
        diff -= bit_L.sum_all() - bit_L.sum(A[i] + 1);
        bit_R.add(A[i], -1);
        A[i] = x;
        diff += bit_L.sum_all() - bit_L.sum(A[i] + 1);
        bit_R.add(A[i], +1);
    ...
    FOR3(i, L, M) bit_L.add(A[i], -1);
    FOR3(i, M, R) bit_R.add(A[i], -1);
