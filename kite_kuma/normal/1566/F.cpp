...
ll solve() {
	...
	foa(t, a) pts.insert(t);

	V<pair<ll, ll>> segs;
	rep(m) {
		LL(l, r);
		auto it = pts.lower_bound(l);
		if(it != pts.end() && *it <= r) continue;
	...
	auto a_includes_b = [](const pll &a, const pll &b) -> bool { return a.first <= b.first && a.second >= b.second; };
	...
	foa(seg, segs) {
		...
		while(!st.empty()) {
			pll tp = st.top();
			if(a_includes_b(seg, tp)) {
				use = false;
				break;
			} else if(a_includes_b(tp, seg)) {
				st.pop();
			} else {
				break;
			}
	...
	// V<ll> from_lef, from_rig(m + 1, INF);
	// from_lef[0] = 0;
	// from_rig[0] = 0;
	...
	int segs_lef_idx, segs_rig_idx;
	segs_lef_idx = 0;

	vll dp(m + 1, INF);
	dp[0] = 0;
	vll dp_turn_in_lef(n, INF);
	vll dp_turn_in_rig(n, INF);

	dp_turn_in_lef[0] = dp_turn_in_rig[0] = 0;

	rep(i, n - 1) {
		ll pt_left = a[i];
		ll pt_right = a[1 + i];
		for(segs_rig_idx = segs_lef_idx; segs_rig_idx < m; segs_rig_idx++) {
			if(segs[segs_rig_idx].second > pt_right) break;
		}

		debug(segs_lef_idx, segs_rig_idx);
		if(segs_lef_idx == segs_rig_idx) {
			dp_turn_in_lef[i + 1] = dp_turn_in_lef[i];
			dp_turn_in_rig[i + 1] = dp_turn_in_rig[i];
		}

		dp[segs_lef_idx] = min(dp_turn_in_lef[i], dp_turn_in_rig[i]);
		rep(j, segs_lef_idx, segs_rig_idx) {
			ll lef, rig;
			tie(lef, rig) = segs[j];
			ll dist = lef - pt_left;
			auto &now = dp[j + 1];
			now = min(dp_turn_in_lef[i] + dist, dp_turn_in_rig[i] + dist * 2);
		}

		// chmin(dp_turn_in_lef[i + 1], dist * 2 + now);
		// chmin(dp_turn_in_rig[i + 1], dist + now);
		rep(j, segs_lef_idx, segs_rig_idx) {
			ll lef, rig;
			tie(lef, rig) = segs[j];
			auto &now = dp[j];
			ll dist = pt_right - rig;
			chmin(dp_turn_in_lef[i + 1], dist * 2 + now);
			chmin(dp_turn_in_rig[i + 1], dist + now);
		}
		chmin(dp_turn_in_lef[i + 1], dp[segs_rig_idx]);
		chmin(dp_turn_in_rig[i + 1], dp[segs_rig_idx]);

		debug(pt_left, pt_right);
		debug(dp);
		debug(dp_turn_in_lef);
		debug(dp_turn_in_rig);

		segs_lef_idx = segs_rig_idx;
	}

	return min(dp_turn_in_lef.back(), dp_turn_in_rig.back());

	// rep(i, m) {
	// 	ll lef, rig;
	// 	tie(lef, rig) = segs[i];

	// 	while(!pts.empty() && *(pts.begin()) < lef) {
	// 		auto it = pts.begin();
	// 		leftpoint = *it;
	// 		pts.erase(it);
	// 	}
