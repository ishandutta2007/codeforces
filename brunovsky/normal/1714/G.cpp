...
int main() {
    ...
    while (T--) {
        ...
        vector<int> r(N);
        y_combinator([&](auto self, int u, int d, int64_t A, int64_t B) -> void {
            seg.update_point(d, B);
            auto searcher = [A](auto node) { return node.value > A; };
            r[u] = seg.suffix_binary_search(searcher).first - 1;
            for (auto [a, b, v] : tree[u]) {
                self(v, d + 1, A + a, B + b);
            }
            seg.update_point(d, Segnode::inf);
        })(0, 0, 0, 0);
        for (int i = 1; i < N; i++) {
            cout << r[i] << " \n"[i + 1 == N];
        }
