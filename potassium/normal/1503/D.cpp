...
int main() {
	...
	vector<int> link(2 * n + 1, 0);
	vector<int> side(2 * n + 1, -1);
	...
	int lastL = 0, lastR = 2 * n + 1;
	int ans = 0;
	bool can = 1;
	while (lastL < lastR) {
		assert(lastL + lastR == 2 * n + 1);
		int boundL = lastL, boundR = lastR;
		int total = 0;
		int cost = 0;
		...
		boundL++;
		while (lastL < boundL || lastR > boundR) {
			if (lastL < boundL) {
				lastL++;
				if (taken[lastL]) continue;

				int other = link[lastL];
				if (other <= boundL) can = 0;
				chkmin(boundR, other);

				taken[lastL] = taken[other] = 1;
				total++;
				pii now = { lastL, other };
				if (side[lastL] == 2) { cost++; }
				cur.emplace_back(now);
			}
			else if (lastR > boundR) {
				lastR--;
				if (taken[lastR]) continue;

				int other = link[lastR];
				if (other >= boundR) can = 0;
				chkmax(boundL, other);

				taken[lastR] = taken[other] = 1;
				total++;
				pii now = { lastR, other };
				if (side[lastR] == 2) { cost++; }
				cur.emplace_back(now);
			}
			else assert(false);
		}
		...
		for (int i = 0; i < total - 1; i++) {
			if (cur[i].fi > cur[i + 1].fi) can = 0;
			if (cur[i].se < cur[i + 1].se) can = 0;
		}

		cost = min(cost, total - cost);
		ans += cost;
	}
