...
class E1ChtenieKnigProstayaVersiya {
public:
    void solve() {
        int n = in.readInt();
        int k = in.readInt();
        arri t, a, b;
        in.readArrays(n, t, a, b);

        arr2d<vi> books(2, 2, vi());
        for (int i : range(n)) {
            books(a[i], b[i]).push_back(t[i]);
        }
        for (int i : range(2)) {
            for (int j : range(2)) {
                sort(all(books(i, j)));
        ...
        if (books(1, 1).size() + min(books(0, 1).size(), books(1, 0).size()) < k) {
            out.printLine(-1);
            return;
        }
        int cur = min(k, int(min(books(1, 0).size(), books(0, 1).size())));
        ll value = accumulate(books(1, 1).begin(), books(1, 1).begin() + (k - cur), 0ll)
                   + accumulate(books(0, 1).begin(), books(0, 1).begin() + cur, 0ll)
                   + accumulate(books(1, 0).begin(), books(1, 0).begin() + cur, 0ll);
        ll answer = value;
        while (cur > 0 && k - cur < books(1, 1).size()) {
            value += books(1, 1)[k - cur];
            value -= books(0, 1)[cur - 1];
            value -= books(1, 0)[cur - 1];
            minim(answer, value);
            cur--;
        }
...
int main() {
    ...
    auto time = clock();
#endif
    E1ChtenieKnigProstayaVersiya solver;
