...
main() {
    ...
    vv(int, ans, n, m);
    map<int, vpi> mp;
    rep(i, n) rep(j, m) { mp[a[i][j]].eb(i, j); }
    vv(int, bb, n);
    vv(int, cc, m);
    for(auto &[_, v] : mp) {
        UnionFind uf(v.size());
        int idx = 0;
        for(auto [i, j] : v) {
            ans[i][j] = max(b[i], c[j]) + 1;
            bb[i].eb(idx), cc[j].eb(idx);
            uf.unite(idx, bb[i].front()), uf.unite(idx, cc[j].front());
            idx++;
        }
        for(auto [i, j] : v) bb[i].clear(), cc[j].clear();
        vi now(v.size());
        rep(i, v.size()) chmax(now[uf.find(i)], ans[v[i].fi][v[i].se]);
        rep(k, v.size()) {
            auto [i, j] = v[k];
            ans[i][j] = now[uf.find(k)];
            b[i] = c[j] = ans[i][j];
        }
    }
