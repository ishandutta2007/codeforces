...
bool is_lucky(int n)
{
	return (n==0)?true:((n%10==4 || n%10==7) && is_lucky(n/10));
}
int main()
{
#ifdef _MSC_VER
	...
	for (int i=0;i<n-1;i++) if (!is_lucky(c[i])) merge(a[i],b[i]);
	int64 R=(int64)n*(n-1)*(n-2);
	for (int i=0;i<n;i++) if (father[i]<0) 
	{
		R-=(int64)cnt[i]*(cnt[i]-1)*(cnt[i]-2);
		R-=(int64)cnt[i]*(cnt[i]-1)*(n-cnt[i])*2;
	}
	cout<<R<<endl;
	return 0;
}