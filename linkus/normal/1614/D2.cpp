...
int main(){
    ...
    FOR(i,2,N){
        ...
        for(int j = 0; j <= wh_pr_ind[i] && i * primes[j] < N; ++j){
            ...
            /* if(j + 1 == wh_pr_ind[i]){ */
            /*     /1* func[i * primes[j]] = func[i] / cnt[i] * (cnt[i] + 1); *1/ */
            /*     func[i * primes[j]] = 0; */
    ...
    static ll dp[N];
    static int cnt[N];
    /* static pair<ll,int> dpcnt[N]; // -> slower this way xd. */
    ...
    for(int i = N; --i;){
        const int crcnt = cnt[i];
        ll crdp = dp[i];
        /* const int crcnt = dpcnt[i].e2; */
        /* ll crdp = dpcnt[i].e1; */
        ...
        FORR(j, primes){
            ...
            if(x >= N)
                break;
            /* dp[i] = max(dp[i], dp[x] + (ll)i * (crcnt - cnt[x])); */
            crdp = max(crdp, dp[x] + (ll)i * (crcnt - cnt[x]));
            /* crdp = max(crdp, dpcnt[x].e1 + (ll)i * (crcnt - dpcnt[x].e2)); */
        }
        dp[i] = crdp;
        /* dpcnt[i].e1 = crdp; */
        /* if(i < 10) */
