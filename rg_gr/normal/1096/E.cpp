...
#ifdef COMB
	fac[0]=inv[0]=inv[1]=1;
	rg(20000)fac[i]=fac[i-1]*i%mod;gr
	rg(20000)if(i>1)inv[i]=inv[mod%i]*(mod-mod/i)%mod;gr
	rg(20000)rnv[i]=inv[i];gr
	rg(20000)inv[i]=inv[i]*inv[i-1]%mod;gr
#endif
}
#ifdef COMB
int C(int n,int m)
...
#ifdef cmlSAM
struct SAM{
	int ch[2000005][26],lnk[2000005],len[2000005],lst=1,cc=1;
	int sz[2000005];
	void insert(int c)
	{
...
signed main()
{
	...
	for(int i=0;i<=n;i++)
	{
		ways[i]=ball0(m-i*k,n);
		// printf("%d %d\n",m-i*k,n);
		// odl(ways[i]);
	}
	for(int i=0;i<=n;i++)mae(ans,me((((i+1)&1)?1:mod-1),me(C(n,i),ways[i])));
	ans=de(ball0(m,n),ans);
	// odl(ans);
	// odl(ans);
	mve(ans,n);
	mve(ans,ball0(m-k,n));
