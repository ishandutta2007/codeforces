...
class multiset(val st: TreeMap<Int, Int> = TreeMap()):MutableMap<Int,Int> by st {
    ...
    fun addone(v:Int){
        st.compute(v){k,v -> (v ?: 0) +1 }
//        st[v] = st.getOrDefault(v,0) + 1
    }
    fun removeone(v:Int){
        st.compute(v){k,v ->
            if(v!! -1 == 0) null else v- 1
        }
...
//        fun mask(a:Int,keys:IntArray):Int {
//            var ret = 0
//            for((i,v) in keys.withIndex()){
//                if(a.has(v)){
//                    ret += 1 shl i
//                    }
//            }
