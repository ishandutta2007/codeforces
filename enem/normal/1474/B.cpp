...
int main()
{
    ...
    {
        if(d[i]==i) prime.PB(i);
        for(ll j=0; j<((ll)prime.size()) && prime[j]<=d[i] && i*prime[j]<N; j++)
        {
            d[i*prime[j]]=prime[j];
        }
    }
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll n;
        cin>>n;
        ll x=*lower_bound(prime.begin(),prime.end(),n+1);
        ll y=*lower_bound(prime.begin(),prime.end(),n+x);
        cout<<x*y;
