...
int n,m,w[N],x[N],y[N],vis[N],pd[N];
vector <pii> e[N];
vector <int> ans;
queue <int> q;
...
int main(){
	...
	for(int i=1;i<=m;i++){
		cin>>x[i]>>y[i];
		w[x[i]]--,w[y[i]]--;
		e[x[i]].pb((pii){y[i],i}),e[y[i]].pb((pii){x[i],i});
	} for(int i=1;i<=n;i++)if(w[i]>=0)q.push(i),vis[i]=1;
	while(!q.empty()){
		int t=q.front(); q.pop();
		for(pii it:e[t]){
			if(!pd[it.se]){
				w[it.fi]++,ans.pb(it.se),pd[it.se]=1;
				if(w[it.fi]>=0&&!vis[it.fi])q.push(it.fi),vis[it.fi]=1;
			}
		}
