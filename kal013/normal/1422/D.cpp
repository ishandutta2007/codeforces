...
void solve(){
	...
	for(auto e: mid){
		adj[0].push_back({e.id,S.distance(e)});
		adj[e.id].push_back({F.id, e.distance2(F)});
	}


	for(int i = 1; i < m; ++i){
		int u = mid[i].id, v = mid[i - 1].id;

		adj[u].push_back({v, mid[i].distance(mid[i - 1])});
		adj[v].push_back({u, mid[i].distance(mid[i - 1])});
	}


	for(int i = 1; i < m; ++i){
		int u = mid2[i].id, v = mid2[i - 1].id;

		adj[u].push_back({v, mid2[i].distance(mid2[i - 1])});
		adj[v].push_back({u, mid2[i].distance(mid2[i - 1])});
	}



	adj[0].push_back({F.id, S.distance2(F)});

	
	min_heap<pair<long long,long long>> queue;

	queue.push({0, 0});
	const long long INF = 1e15;
	vector<long long> dist(m + 4, INF);
	dist[0] = 0;
	while(!queue.empty()){
		auto X = queue.top();
		queue.pop();
