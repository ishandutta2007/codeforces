    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                int where = pos[i];
                if (Math.abs(where - tmpPos) * 2 >= n) {
                    publish(i, tmpPos);
                    publish(where, tmpPos);
                    publish(i, tmpPos);
                } else if (tmpPos == where) {
                    publish(i, tmpPos);
                } else {
                    int anotherSide = tmpPos == 1 ? n : 1;
                    publish(i, tmpPos);
                    publish(tmpPos, anotherSide);
                    publish(where, anotherSide);
                    publish(tmpPos, anotherSide);
                    publish(i, tmpPos);
                }
            ...
            io.cache.append(ops.size()).append('\n');
            for (OP op : ops) {
                io.cache.append(op.i).append(' ').append(op.j).append('\n');
            }
            if (debug.allowDebug) {
                io.cache.append(Arrays.toString(perm));
            }
        }
    }
