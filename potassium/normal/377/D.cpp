...
S op(S l, S r) {
	S s;
	tie(s.maxv, s.maxp) = max<pii>({ {l.maxv, l.maxp}, {r.maxv, r.maxp} });
	return s;
}
...
int main() {
	int n; R(n);
	vector<vector<int>> a(n, vector<int>(3)); R(a);
	
	vector<pair<int, int>> Event;
	const int SHIFT = 1000;
	for (int i = 0; i < n; i++) {
		Event.pb({a[i][0], +(i + SHIFT)});
		Event.pb({a[i][1] + 1, -(i + SHIFT)});
	}
	sort(all(Event));
	
	const int MAXV = 300000 + 7;
	vector<S> st_base(MAXV);
	...
	int optL = -1, optR = -1;
	for (auto& e : Event) {
		int idx = abs(e.S) - SHIFT;
		int isAdd = e.S > 0;
		
		st.apply(a[idx][1], a[idx][2] + 1, isAdd ? add1 : minus1);
		
		if (chkmax(ans, st.all_prod().maxv)) {
			optL = e.F;
			optR = st.all_prod().maxp;
		}
	}
	
	vector<int> take;
	for (int i = 0; i < n; i++)
		if (a[i][0] <= optL && optL <= a[i][1] && a[i][1] <= optR && optR <= a[i][2])
			take.pb(i + 1);
