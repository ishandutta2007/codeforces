...
int main(){
    ...
    RSQ<ll>zero(n),one(n);
    rep(n)(s[i]=='0'?zero:one).add(i,1);
    ll cost=0;
    rep(n){
        if(s[i]!='0')cost+=y*zero.get(i+1,zero.size);
        else cost+=x*one.get(i+1,zero.size);
    }
    ll ans=cost;
    rep(n)if(s[i]=='?'){
        cost-=x*zero.get(0,i)+y*zero.get(i+1,zero.size);
        zero.add(i,1);
        one.add(i,-1);
        cost+=y*one.get(0,i)+x*one.get(i+1,zero.size);
        chmin(ans,cost);
    }
    out(ans);
}