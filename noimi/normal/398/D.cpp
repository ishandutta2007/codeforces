...
int main() {
    ...
    vi big(n);
    constexpr int S = 100;
    rep(i, n) if(cnt[i] >= S) big[i] = true;

    vi ans(n);
    vector<unordered_set<int>> bigs(n);
    rep(i, n) for(auto e : v[i]) {
        if(big[e])
            bigs[i].emplace(e);
        else
            ans[i] += online[e];
    ...
    for(auto [c, a, b] : query) {
        ...
        if(c == 'C') {
            int cur = ans[a];
            for(auto e : bigs[a]) { cur += online[e]; }
            cout << cur << endl;
        }
    }
}