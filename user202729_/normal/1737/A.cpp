// time-limit: 1000
// problem-url: https://codeforces.com/contest/1737/problem/A

// I know, but this is Codeforces contest...
...
void up(){
	...
	assert(int(s.size())==n);
	std::array<char, 'z'+1> count {};
	for(auto ch: s) count[ch]++;

	int const countEach=n/k;
	for(int _=k; _--;){
		int pos='a';
		int numIncluded=0;
		while(count[pos]!=0){
			...
			++pos;
			numIncluded++;
			if(numIncluded==countEach) break;
		}
		std::cout<<char(pos);
	}
	std::cout<<'\n';
