...
fun solve() {
    var (n, m) = readInts()
    var a = Array<MutableList<Int>>(n) { MutableList<Int>(2) { 123 } }
    for (i in 0 until n) {
        a[i] = readInts().toMutableList()
    }
    var ans = MutableList<Pair<Int, Int>>(0) { Pair(1, 1) }
    var b = Array<MutableList<Int>>(n) { MutableList<Int>(m) { 0 } }
