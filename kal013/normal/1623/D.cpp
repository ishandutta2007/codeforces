...
void solve(){
    ...
    auto next_ = [&] (const array<int, 4>& X){
    	array<int, 4> Y = X;
    	...
    	for(int j = 0; j < 2; ++j){
			...
			Y[j] = min(max(Y[j] + Y[j + 2], 0), N[j] - 1);
		}	
		return Y;
    };

    array<int, 4> cur = {b[0], b[1], 1, 1};

    vector<vector<int>> dist[2][2];

    for(int i = 0; i < 2; ++i){
    	for(int j = 0; j < 2; ++j){
    		dist[i][j].resize(N[0], vector<int>(N[1], -1));
    	}
    }

    auto get_dist = [&] (const array<int, 4>& X){
		int i = max(X[2], 0), j = max(X[3], 0);
		return dist[i][j][X[0]][X[1]];
    };
    auto upd_dist = [&] (const array<int, 4>& X, int d){
    	int i = max(X[2], 0), j = max(X[3], 0);
    	if (dist[i][j][X[0]][X[1]] == -1){
    		dist[i][j][X[0]][X[1]] = d;
    		return true;
    	}
    	else{
    		assert (dist[i][j][X[0]][X[1]] < d);
    		return false;
    	}
    };
  	vector<array<int, 4>> bfs = {cur};  
  	upd_dist(cur, 0);
...

  	{
  		...
  		vector<array<int, 4>> stk;
  		for(int j = idx; j < bfs.size(); ++j){
  			auto z = bfs[j];
  			if (z[0] == d[0] || z[1] == d[1])
  				stk.push_back(z);
  		}
  		for(int k = 1; k < stk.size(); ++k){
  			dis.push_back(get_dist(stk[k]) - get_dist(stk[k - 1]));
  		}
  		dis.push_back(get_dist(stk[0]) - get_dist(stk.back()) + bfs.size() - idx);
  	...
  	vector<array<mint, 2>> dp(dis.size() + 1);
  	dp[0] = {1, 0};

  	mint denom = mint(1) / (1 - p);
  	for(int i = 0; i < dis.size(); ++i){
  		// 
  		dp[i + 1] = {denom * dp[i][0], denom * (dp[i][1] - p) - dis[i]};
  	}

  	mint exp = (dp.back()[1] - dp[0][1]) / (dp[0][0] - dp.back()[0]);
