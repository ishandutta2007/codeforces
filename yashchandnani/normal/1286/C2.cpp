...
int main() {
	...
	if(lx){
		...
		repA(i,2,2){
			...
			rep(j,26) rep(k,cnt[i][j][1]) p[i][j].pb((2*lx+1)/2);
		}
		repA(i,st,n){
			p[3][ans[i-1]-'a'].pb(i-st+1);		
		}
		rep(i,26){
			sort(all(p[2][i]));
			sort(all(p[3][i]));
			while(sz(p[2][i])){
				if(sz(p[3][i])&&p[3][i].back()==p[2][i].back()) p[2][i].pop_back(),p[3][i].pop_back();
				else{
					ans[nn-p[2][i].back()]='a'+i;
					p[2][i].pop_back();
				}
			}
		}
