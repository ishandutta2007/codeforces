#include <bits/stdc++.h>
        ...
        while(t--)
        {
            cin>>n;vector<int>odd,eve;
            for(int i=0;i<2*n;i++)
            {
                cin>>a[i];
                if(a[i]%2)odd.push_back(i+1);
                else eve.push_back(i+1);
            }
            int si=odd.size();
           // cout<<odd.size()<<" "<<eve.size()<<endl;
if(si%2)
{
    for(int i=1;i<odd.size();i+=2)cout<<odd[i]<<" "<<odd[i+1]<<endl;
    for(int i=1;i<eve.size();i+=2)cout<<eve[i]<<" "<<eve[i+1]<<endl;
}
else
{
    if(odd.size()>1)
    {
        for(int i=2;i<odd.size();i+=2)cout<<odd[i]<<" "<<odd[i+1]<<endl;
        for(int i=0;i<eve.size();i+=2)cout<<eve[i]<<" "<<eve[i+1]<<endl;

    }
    else
    {
        for(int i=2;i<eve.size();i+=2)cout<<eve[i]<<" "<<eve[i+1]<<endl;
}
}
