...
int main() {
    ...
    auto dfs = [&](auto dfs, int root, int siz) -> pair<mint, string> {
        if (siz == 1) return { 1, string(1, s[root - 1]) };
        int len = siz >> 1;

        auto [lx, ls] = dfs(dfs, 2 * root, len);
        auto [rx, rs] = dfs(dfs, 2 * root + 1, len);

        string res;
        res += s[root - 1];
        auto [s1, s2] = minmax(ls, rs);
        res += s1;
        res += s2;
