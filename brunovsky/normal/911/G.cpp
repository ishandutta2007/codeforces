...
int main() {
    ...
    map<int, Splay*> trees;
    for (auto& [v, is] : pos) {
        trees[v] = Splay::construct_array(is.size(), is);
    }
    ...
    while (Q--) {
        int l, r, x, y;
        cin >> l >> r >> x >> y, l--;
        if (auto range = splice_key_range(trees[x], l, r)) {
            trees[y] = meld(trees[y], range);
        }
    }

    vector<int> b(N);
    for (auto& [v, tree] : trees) {
        for (auto node : unstitch(tree)) {
            b[node->key] = v;
        }
    }
    for (int i = 0; i < N; i++) {
