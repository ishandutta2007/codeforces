...
struct node{
	int v,wt;
	node *next;
}*adj[MAXN*9+10],edge[MAXN*50+10],*ecnt=edge;
...
void build(int i,int l,int r){
	...
	addedge(i,i<<1,0);
	addedge(i,(i<<1)|1,0);
	addedge(4*n+((i<<1)|1),i+4*n,0);
	addedge(4*n+(i<<1),4*n+i,0);
	int mid((l+r)>>1);
	build(i<<1,l,mid);
	build((i<<1)|1,mid+1,r);
}
void insert_in(int i,int l,int r,int ll,int rr,int now,int wt){
	if(ll<=l&&r<=rr)
		return addedge(now,i,wt);
	if(ll>r||rr<l)
		return;
	int mid((l+r)>>1);
	insert_in(i<<1,l,mid,ll,rr,now,wt);
	insert_in((i<<1)|1,mid+1,r,ll,rr,now,wt);
}
void insert_out(int i,int l,int r,int ll,int rr,int now,int wt){
	if(ll<=l&&r<=rr)
		return addedge(i+4*n,now,wt);
	if(ll>r||rr<l)
		return;
	int mid((l+r)>>1);
	insert_out(i<<1,l,mid,ll,rr,now,wt);
	insert_out((i<<1)|1,mid+1,r,ll,rr,now,wt);
}
void read(){
	...
	for(i=1;i<=Q;i++){
		...
		if(p==1){
			...
			addedge(u+8*n,v+8*n,wt);
		}
		else if(p==2){
			Read(u),Read(l),Read(r),Read(wt);
			insert_in(1,1,n,l,r,u+8*n,wt);
		}
		else{
			Read(u),Read(l),Read(r),Read(wt);
			insert_out(1,1,n,l,r,u+8*n,wt);
		}
	}
}
