...
int main() {
    ...
    vi loc(N); F0R (i, N) loc[--p[i]] = i;

    vll sums(N + 1);
    F0R (i, N) sums[i+1] = sums[i] + i;

    vll smus(N + 1);
    F0Rd (i, N) smus[i] = i + smus[i+1];

    ll within = 0;
    binary_indexed_tree<int> active(N);

    binary_indexed_tree<int> inactive(N);
    binary_indexed_tree<ll>  insum(N);
    F0R (i, N) {
        inactive.add(loc[i], 1);
        insum.add(i, i);
    }

    auto eval = [&](int s, int m) {
        int cutoff = inactive.lower_bound([&s](int v){ return v >= s; });
        return insum.sum(cutoff) - sums[s]
             + smus[N - (N - s - m)] - insum.sum(cutoff, N);
    };

    F0R (i, N) {
        within += active.sum(loc[i], N);

          active.add(loc[i],  1);
        inactive.add(loc[i], -1);
           insum.add(loc[i], -loc[i]);

       ll best = LLONG_MAX;
       {
           int cutoff = active.lower_bound([&i](int v){ return v > (i+1)/2; });
           int s = inactive.sum(cutoff);
           best = insum.sum(cutoff) - sums[s]
               + smus[N - (N - s - (i+1))] - insum.sum(cutoff, N);
       }
        ...
        while (lo <= hi) {
            ...
            ll c1 = eval(v1, i+1), c2 = eval(v2, i+1);
            if (c1 < c2) {
                ckmin(best, c1);
                hi = v2 - 1;
            } else {
