...
struct point
{
	...
	void kamp()
	{
		kampas = atan2((long double)y, (long double)x);
	}
...
ll distan(point a, point b)
{
	return (a.x - b.x) * (a.x - b.x) +  (a.y - b.y) * (a.y - b.y);
}
ld distans(point a, point b)
{
	return sqrtl(distan(a, b));
}
...
vector<point> findConvex(int n) {
	...
	vector<point>Convex;
	Convex.push_back(X[0]);

	for (int i = 1; i < n; i++)
	{
		while (Convex.size() >= 2)
		{
			if (cross(X[i] - Convex[Convex.size() - 1], Convex[Convex.size() - 1] - Convex[Convex.size() - 2]) > 0)
			{
				Convex.pop_back();
			}
			else break;
		...
		Convex.push_back(X[i]);
	}

	return Convex;
}

vi computeTemporaryArray(vpll pattern) {
	vi lps(pattern.size());
	int index = 0;
...
bool KMP(vpll text, vpll pattern) {
	vi lps = computeTemporaryArray(pattern);
	int i = 0;
	int j = 0;
...
int main()
{
	...
	cin >> n >> m;
	debug(n);
	vector<point> a = findConvex(n);
	debug(m);
	vector<point> b = findConvex(m);
	...
	debug(a, b);
	{
		...
		for (int i = 0; i < n; i++) {
			if (cross(a[(i + 2) % n] - a[(i + 1) % n], a[(i + 1) % n] - a[(i) % n]) == 0 and abs(distans(a[(i + 2) % n], a[(i + 1) % n]) + distans(a[(i) % n], a[(i + 1) % n]) - distans(a[(i + 2) % n], a[(i) % n])) <= 1e-5) {
				blog[(i + 1) % n] = true;
			}
		}
	...
	}
	{
		...
		for (int i = 0; i < m; i++) {
			if (cross(b[(i + 2) % m] - b[(i + 1) % m], b[(i + 1) % m] - b[(i) % m]) == 0 and abs(distans(b[(i + 2) % m], b[(i + 1) % m]) + distans(b[(i) % m], b[(i + 1) % m]) - distans(b[(i + 2) % m], b[(i) % m])) <= 1e-5)
				blog[(i + 1) % m] = true;
		}
	...
	for (int i = 0; i < n; i++) {
		al.emplace_back(distan(a[i], a[(i + 1) % n]), cross(a[(i + 2) % n] - a[(i + 1) % n], a[(i + 1) % n] - a[(i) % n]));
	}

	vpll bl;

	for (int i = 0; i < m; i++) {
		bl.emplace_back(distan(b[i], b[(i + 1) % m]), cross(b[(i + 2) % n] - b[(i + 1) % n], b[(i + 1) % n] - b[(i) % n]));
	}
