...
string read_line() { string value; getline(cin, value); return value; }
...
vector<int> voisins[200 * 1000];

bool estPasse[200 * 1000];
long long score = 0;

long long maxi = 0;

long long  calculeScore(int noeud, int parent = -1) {
	if(estPasse[noeud])
		return INFINI;
	estPasse[noeud] = true;
	...
	for(int voisin : voisins[noeud]) {
		if(voisin != parent) {
			maxScore = max(maxScore, calculeScore(voisin, noeud));
		}
	}
	...
	maxi = max(maxi, maxScore + scores[noeud]);
	return maxScore + scores[noeud];
}

int main() {
	fast_io();

	int nbVilles = read<int>(), nbRoutes = read<int>();

	scores = read<vector<long long>>(nbVilles);

	for(int iRoute : range(0, nbRoutes)) {
		auto arc = read<pair<int, int>>();
		voisins[arc.first - 1].push_back(arc.second - 1);
		voisins[arc.second - 1].push_back(arc.first - 1);
	}

	int depart = read<int>();

	calculeScore(depart - 1);
	writeln(score + maxi);
