...
bool eq(const string &s1, const string &s2, int prf) {
    auto p1 = s1.end() - prf;
    auto p2 = s2.begin();
    for (; p1 != s1.end(); ++p1, ++p2) {
        if (*p1 != *p2) {
...
string mrg(const string &s1, const string &s2) {
    for (int prf = min(sz(s1), sz(s2)); prf > 0; --prf) {
        if (eq(s1, s2, prf)) {
            string ans = s1;
            ans.append(s2.begin() + prf, s2.end());
            return ans;
        }
        if (eq(s2, s1, prf)) {
            string ans = s2;
            ans.append(s1.begin() + prf, s1.end());
            return ans;
        }
    }
