...
void solve(){
  ...
  vi P, N, Z;
  for(li i = 0; i < (li)(n); ++i) if(A[i]>0) P.push_back(A[i]);
  else if(A[i]<0) N.push_back(A[i]);
  else Z.push_back(A[i]);
  if(accumulate(begin(P), end(P),0ll) < -accumulate(begin(N), end(N),0ll)) {
    cout << "YES" << endl;
    for(auto x : N) cout << x << ' ';
    for(auto x : P) cout << x << ' ';
    for(auto x : Z) cout << x << ' ';
    cout << endl;
  }else if(accumulate(begin(P), end(P),0ll) > -accumulate(begin(N), end(N),0ll)){
    cout << "YES" << endl;
    for(auto x : P) cout << x << ' ';
     for(auto x : N) cout << x << ' ';
    for(auto x : Z) cout << x << ' ';
