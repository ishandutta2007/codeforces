...
void solve() {
  ...
  LST<int, int, decltype(f), decltype(f), decltype(f)> seg(2 * N, f, f, f, inf,
                                                           inf);

  map<int, int> ans;
  rep1(n, N) {
    // rep(i, 2 * N) out(seg.query(i, i + 1));
    int i = inv[n];
    int segi = seg.query(i, i + 1);
    if (segi < n) die("NO");
    // trc(n,i);
    // okiba aru?
    auto lb = s.lower_bound(i);
    if (lb == s.begin()) die("NO");
    lb--;
    int j = *lb;
    s.erase(lb);
    ans[j] = n;
  ...
  vi bns;
  each(p, ans) bns.push_back(p.second);
  out("YES");
  out(bns);
}