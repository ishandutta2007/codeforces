...
fun solve(inp: InputReader, out: OutputWriter, testNumber: Int) {
    var s = inp.readString().substring(2)
    var preLen = s.indexOf('(')
    var perLen = s.length - preLen - 2
    var prePeriod: Long
    if (preLen == 0) {
        prePeriod = 0.toLong()
    } else {
        prePeriod = s.substring(0, preLen).toLong()
    }
    var period = s.substring(preLen + 1, s.length - 1).toLong()
    var prePeriodDen = IntegerUtils.power(10.toLong(), preLen.toLong())
    var periodDen = (IntegerUtils.power(10.toLong(), perLen.toLong()) - 1)
    var num = prePeriod * periodDen + period
    var den = periodDen * prePeriodDen
    var g = IntegerUtils.gcd(num, den)
    num /= g
    den /= g
    out.printLine(num.toString() + "/" + den.toString())
}
