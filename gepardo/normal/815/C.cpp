...
signed main() {
	int n = get<int>(), b = get<int>();
	vi c(n), d(n);
	vector<vi> g(n);
	for (int i = 0; i < n; i++) {
		c[i] = get<int>(); d[i] = get<int>();
		if (i != 0) {
			int p = get<int>() - 1;
			g[p].push_back(i);
		}
	}
	vector< vector<i64> > dp0(n, vector<i64>(n+1, inf64));
	vector< vector<i64> > dp1(n, vector<i64>(n+1, inf64));
	vi st(n);
	...
	function<void(int)> dfs = [&](int v) {
		...
		int stCur = 1;
		for (int to : g[v]) {
			vector<i64> ndp0(n+1, inf64);
			vector<i64> ndp1(n+1, inf64);
			for (int i = 0; i <= stCur; i++) {
				for (int j = 0; j <= st[to]; j++) {
					umin(ndp0[i+j], dp0[v][i] + dp0[to][j]);
					umin(ndp1[i+j], dp1[v][i] + min(dp0[to][j], dp1[to][j])); 
				}
			}
			dp0[v] = ndp0;
			dp1[v] = ndp1;
	...
	int ans = 0;
	while (ans <= n && min(dp0[0][ans], dp1[0][ans]) <= b) {
		ans++;
	}
	putLn(ans-1);
