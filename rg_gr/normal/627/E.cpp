...
int l[5005],r[5005],b[5005],*f=b+100;
#include <vector>
std::vector<int>v[5005];
signed main()
{
	initprog();
	int rp=read(),c=read(),n=read(),k=read();rg(n)a[i].read();gr
	std::sort(a+1,a+1+n,[&](qwq x,qwq y){return x.y<y.y;});
	a[0].y=0;
	a[n+1].y=c+1;
	rg(n)v[a[i].x].push_back(i);gr
	int ans=0;
	rg(rp)
	memset(l,0,sizeof(l));
	memset(r,0,sizeof(r));
	...
	r[0]=n+1,l[n+1]=0;
	rg_(j,n)
	if(a[j].x>=i)r[cur]=j,l[j]=cur,cur=j,r[j]=n+1,l[n+1]=j;
	gr
	r[n+1]=n+1;
	int d=r[0],e=0;rg(k)e=r[e];gr
	
	int x=0;
	while(d!=n+1)
	{
		// odp(d,e);
		x+=(a[d].y-a[l[d]].y)*(c-a[e].y+1);
		d=r[d];
		e=r[e];
	}
	// puts("");
	...
	for(int j=rp;j>=i;j--)
	{
		...
		{
			int pos=g;
			for(int d=1;d<=k+1;d++)pos=l[pos],f[-d]=pos;
			f[0]=pos=g;
			for(int d=1;d<=k+2;d++)pos=r[pos],f[d]=pos;
			for(int q=-k-1;q<=1;q++)
			{
				int d=f[q],e=f[q+k-1];
				if(d==0||e==n+1)continue;
				// printf("remove %d %d : %d\n",d,e,(a[d].y-a[l[d]].y)*(c-a[e].y+1));
				x-=(a[d].y-a[l[d]].y)*(c-a[e].y+1);
			}
			for(int d=1;d<=k+2;d++)f[d-1]=f[d];
