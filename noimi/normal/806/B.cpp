...
main() {
    ...
    vi score(5);
    rep(i, 5) score[i] = f(cnt[i], n);
    vv(int, cost, 5, 7);
    rep(i, 5) {
        int now = 0;
        if(a[0][i] == -1 and a[1][i] == -1) continue;
        rep2(j, score[i] + 1, 6) {
            while(1) {
                if(f(cnt[i], n + now) == j) {
                    cost[i][j] = now;
    ...
    const int inf = 2e9;
    int mi = inf;
    auto solve = [&](auto &&f, vi &v) -> void {
        if(v.size() == 5) {
            int l = 0, r = inf;
            int V[2] = {};
            rep(i, 5) { rep(j, 2) if(a[j][i] != -1) V[j] += 500 * v[i] / 250 * (250 - a[j][i]); }
            if(V[0] <= V[1]) return;
            rep(i, 5) {
                int tmp = score[i];
                if(v[i] < tmp) {
                    ...
                    chmax(l, cost[i][v[i]]);
                    if(a[0][i] == -1 and v[i] < 6) chmin(r, cost[i][v[i] + 1] - 1);
                } else {
                    if(a[0][i] == -1 and v[i] < 6) chmin(r, cost[i][v[i] + 1] - 1);
                }
            }
