...
void solve() {
    ...
    // (p[i], ival)
    vc<int> right(n + 1, 0);
    vc<int> dval(n, n + 1), ival(n, -1);
    auto upd = [&](auto&& upd, int r) -> void {
        if(r + 1 == n) {
            right[n] = n;
            return;
        }
        int nival = n + 1, ndval = -1;
        if(p[r] < p[r + 1]) {
            chmax(ndval, dval[r]);
        } 
        if(p[r] > p[r + 1]) {
        ...
        if(dval[r + 1] == ndval && ival[r + 1] == nival) return;
        dval[r + 1] = ndval, ival[r + 1] = nival;
        debug(r + 1, dval[r + 1], ival[r + 1]);
        right[r + 1] = r + 1;
        if(nival != n + 1 || ndval != -1) {
            upd(upd, r + 1);
            if(r + 2 <= n) right[r + 1] = right[r + 2];
        }
        return;
    };
    for(int l = n - 1; l >= 0; -- l) {
