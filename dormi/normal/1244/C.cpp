...
int main(){
	...
	BigInt div=p/gcd;
	BigInt bbestw=bestw,bbestd=bestd;
	bbestw*=div;
	bbestd*=div;
	BigInt best=bbestd.abs()/(BigInt)w+(bbestd.abs()%(BigInt)w>0?1:0);
	bbestw-=best*(BigInt)d;
	bbestd+=best*(BigInt)w;
	if(bbestw<0){
		prl("-1");
		return 0;
	}
	if(bbestw+bbestd>(BigInt)n){
		prl("-1");
		return 0;
	}
	prl(bbestw,bbestd,(BigInt)n-bbestw-bbestd);
	return 0;
}