...
public class C2 {
	...
	void solve()
	{
		...
		while(true){
			...
			long x = 0, y = 0;
			for(int i = 0;i < n;i++){
				long mx = x - co[i][0], my = y - co[i][1];
				long px = x + co[i][0], py = y + co[i][1];
				if(mx*mx+my*my <= px*px+py*py){
					x = mx; y = my;
					ans[co[i][2]] = -1;
				}else{
					x = px; y = py;
	...
	// e[i, j] = (maximize or minimize)_k (e[i^1<<k, j^1<<k]+e[i^1<<k, j])/2
