...
/*int get(int d) {
    if (d < 0) return 0;
    return C.comb(d+p-2,p-2);
}

int getMax(int r) {
}*/

int get(int a, int b, int c) { // sum, # lef, max 
    if (b == 0) return a == 0;
    if (c < 0 || a < 0) return 0;
    int ret = 0;
    F0R(i,b+1) {
        int x = C.comb(b,i);
        int t = a-(c+1)*i; if (t < 0) break;
        MUL(x,C.comb(t+b-1,b-1));
        if (i&1) SUB(ret,x);
        else AD(ret,x);
    }
    // pr(a,b,c,ret);
...
int main() {
    ...
    setIO(); re(p,s,r); C.init(MOD);
    if (p == 1) {
        pr(1);
        exit(0);
    }
    while (r <= s) {
        F0R(i,p) AD(a,mul(mul(C.comb(p-1,i),get(s-r*(i+1),p-1-i,r-1)),inv(i+1)));
        AD(b,get(s-r,p-1,10000));
        // pr(s-r,p-1,b);
        r ++;
    }
    // pr(a,b);
