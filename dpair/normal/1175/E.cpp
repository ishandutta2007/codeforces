...
int n, m; const int N = 5e5;

int f[21][MAXN];
signed main(){
    read(n, m); rep(i, 1, n) {int l, r; read(l, r); chmax(f[0][l], r); }
    rep(i, 0, N) chmax(f[0][i], f[0][i - 1]);
    rep(j, 1, 20) rep(i, 0, N) f[j][i] = mx(f[j - 1][i], f[j - 1][f[j - 1][i]], f[j][i - 1]);

    while(m --) {
        int l, r, ans = 0; read(l, r);
        per(i, 20, 0) {if(f[i][l] < r) ans |= (1 << i), l = f[i][l]; }
        if(r > f[0][l]) print("-1");
        else print(ans + 1);
    }
}