...
template <class Flow, class Cost, int MAXV, int MAXE> struct MinCostFlow {
	struct Edge {
		int to;
		Flow cap, flow;
		Cost cost;
	...
	Edge edges[2 * MAXE], *head[MAXV], *pred[MAXV];
	Flow limit[MAXV];
	Cost dist[MAXV];
	...
	void init() {
		fill(head, head + MAXV, nullptr);
		cnt = 0;
	}

	void add_edge(int st, int ed, Flow cap, Cost cost) {
		edges[cnt++] = {ed, cap, 0, cost, nullptr, head[st]};
		edges[cnt++] = {st, 0, 0, -cost, nullptr, head[ed]};
		head[st] = &edges[cnt - 2];
		head[ed] = &edges[cnt - 1];
		head[st]->rev = head[ed];
		head[ed]->rev = head[st];
	...
	pair<Flow, Cost> go(int src, int sink) {
		Flow max_flow = 0;
		Cost min_cost = 0;
		while (true) {
			fill(dist, dist + MAXV, numeric_limits<Cost>::max());
			limit[src] = numeric_limits<Flow>::max();
			pred[src] = pred[sink] = nullptr;
			dist[src] = 0;
			while (!que.empty()) que.pop();
			que.push( {0, src});
			while (!que.empty()) {
				int at = que.top().second;
				Cost cdist = -que.top().first;
				que.pop();
				if (cdist > dist[at]) continue;
				for (auto e = head[at]; e; e = e->next) {
					if (cdist + e->cost < dist[e->to] && e->flow < e->cap) {
						dist[e->to] = cdist + e->cost;
						limit[e->to] = min(limit[at], e->cap - e->flow);
						pred[e->to] = e;
			...
			Flow by = limit[sink];
			max_flow += by;
			min_cost += dist[sink] * by;
			for(auto e = pred[sink]; e; e = pred[e->rev->to]) {
				e->flow += by;
				e->rev->flow -= by;
			}
		}
...
void _main() { /* CURSOR START */
	int n; cin>>n;
	const int N=5<<10;
	static MinCostFlow<int,int,N+2,N+N+N+N> mf;
	int src=0,snk=n+1;
	for(int i=1;i<=n;i++) {
		int a; cin>>a;
		if(a) mf.add_edge(src,i,1,0);
		else mf.add_edge(i,snk,1,0);
	}
	for(int i=2;i<=n;i++) {
		mf.add_edge(i,i-1,n,1);
		mf.add_edge(i-1,i,n,1);
	}
	cout<<mf.go(src,snk).se<<endl;
}