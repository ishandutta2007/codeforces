...
mint ank(int n, int k) { return k < 0 || k > n ? mint(0) : fact[n] * ifact[n - k]; }
...
void solve() {
  ...
  n = sz(a);
  vector<mint> d(s0 / 2 + 1, 0);
  vector<mint> nd(s0 / 2 + 1, 0);
  int s = 0;
  d[0] = 1;
  s = a[0];
  for (int i = 1; i < n; ++i) {
    fill(all(nd), 0);
    for (int cnt = 0; cnt < sz(d); ++cnt)
      for (int take = 0; take <= a[i] && take <= 1 && take + cnt < sz(d); ++take) {
        int also = take;
        if (take == 0)
          also = 0;
        else if (cnt == 0 && take == sz(d) - 1)
          also = take;
        else if (cnt == 0)
          ++also;
        else if (cnt + take == sz(d) - 1)
          --also;
        int prv = cnt + 1;
        if (cnt == 0) prv = 0;
        if (cnt == sz(d) - 1)
          prv = cnt;
        nd[cnt + take] += d[cnt] * ank(a[i], take) * ank(s - prv - cnt, also);
      }
