...
void read_proj(int id)
{
	int num = enc(read_token());
	pid[id] = num;
	scanf("%d", &ver[id]);
	int gsz;
	scanf("%d", &gsz);
...
void solve()
{
	...
	{
		int num = q[i];
		int now = vnums[{num, prod_ver[num]}];
		for (auto p : go[now])
		{
			int pnum = p.first;
			int curv = p.second;
			if (!used[pnum])
			{
				q[qsz++] = pnum;
				used[pnum] = true;
				dist[pnum] = dist[num] + 1;
				prod_ver[pnum] = curv;
				continue;
			}
			else if (dist[pnum] == dist[num] + 1)
				prod_ver[pnum] = max(prod_ver[pnum], curv);
		}
	}
	printf("%d\n", qsz - 1);
	vector<pair<string, int>> deps;
	for (int i = 1; i < qsz; ++i)
		deps.emplace_back(projs[q[i]], prod_ver[q[i]]);
