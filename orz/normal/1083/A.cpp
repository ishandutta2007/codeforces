...
pll dfs(const int& v, const vvi& g, const vvi& gm, vb& nv, const vi& w)
{
	nv[v] = false;
	ll ans1 = 0, ans2 = 0;
	ll best1 = 0, best2 = 0;
	for (int i = 0; i < sz(g[v]); ++i)
	{
		...
		if (nv[to])
		{
			...
			else if (new_path > best2)
				best2 = new_path;
		}
	}
	ans2 = best1 + best2 + w[v];
	ans1 = max(ans1, ans2);
	return pll(ans1, best1 + w[v]);
}
