...
int main() {
    ...
    int N; re(N); vi p(N);
    F0R (i, N) { int x, y; re(x, y); p[x-1] = y-1; }

    struct node { int mv = INT_MAX/2, mc = 0; };
    auto nn = [](node a, node b)->node{
        int nm = min(a.mv, b.mv);
        return {nm, (a.mv > nm ? 0 : a.mc) + (b.mv > nm ? 0 : b.mc)};
    };
    auto un = [](int u, node n)->node{ return {n.mv + u, n.mc}; };

    segment_tree_lazy st(N, node{}, 0, nn, plus<int>(), un);
    st.set_leaves([](int _)->node{ return {0,1}; });

    ll ans = 0;

    vpii mn{{INT_MIN, -1}}, mx{{INT_MAX, -1}};
    F0R (i, N) {
        while (p[i] < mn.back().f) {
            pii rem = mn.back(); mn.pop_back();
            st(mn.back().s + 1, rem.s + 1, rem.f);
        }
        st(mn.back().s + 1, i+1, -p[i]);
        mn.pb({ p[i], i });

        while (p[i] > mx.back().f) {
            pii rem = mx.back(); mx.pop_back();
            st(mx.back().s + 1, rem.s + 1, -rem.f);
        }
        st(mx.back().s + 1, i+1, p[i]);
        mx.pb({ p[i], i });
