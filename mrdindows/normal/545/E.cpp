...
struct St{
	...
	bool operator<(const St &st) const {
		return this->dist < st.dist || 
			this->dist == st.dist && edges[this->edge].c < edges[st.edge].c || 
			this->dist == st.dist && edges[this->edge].c == edges[st.edge].c && this->edge < st.edge;
	}
};
int main()
{
	...
	cin >> u;
	--u;
	FI(n) d[i] = { INF * 1ll * INF, INF };
	d[u] = { 0, 0 };
	set<St> q;
	q.insert({ u, 0, 0 });
// 	for (int edge : v[u]) {
// 		int to = edges[edge].a == u ? edges[edge].b : edges[edge].a;
// 		int s = edges[edge].c;
// 		if (d[to].first > s  || d[to].first == s && d[to].second > s) {
// 			d[to].first = s;
// 			d[to].second = s;
// 
// 			St st = { s, edge};
...
// 	}
	...
	while (!q.empty()) {
		...
		q.erase(p);
		for (int e : v[cur]) {
			int to = edges[e].a == cur ? edges[e].b : edges[e].a;
			int s = edges[e].c;
			if (d[to].first > s + w || d[to].first == s + w && d[to].second > s) {
				q.erase({ to, d[to].first, de[to] });
				d[to].first = s + w;
				d[to].second = s;
				de[to] = e;
				q.insert({ to, d[to].first, e });
			}
		}
	}
	cout << ans << endl;
