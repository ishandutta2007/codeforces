...
void solve() {
    ...
    SegmentTree<mat> seg(temp, mul, g, e);

    // while (true) {
    //     int l, r;
    //     cin >> l >> r;
    //     const auto anss = seg.query(l, r, 0, 0, seg.n);
    //     DEBUG_MAT(anss);
    // }
    ...
    while (q--) {
        ...
        seg.update(p, defo[c - 'a'], 0, 0, seg.n);

        const auto &temp = seg.query(0, n, 0, 0, seg.n);

        ll ans = INF;
        rep(i, N) chmin(ans, temp[0][i]);
        cout << ans << endl;
    }
}
