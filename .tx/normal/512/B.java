...
public class Main extends PrintWriter {
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0, curSz = gcds.size(); j < curSz; j++) {
                int gcd = gcds.get(j);
                int ngcd = gcd(gcd, l[i]);
                Integer cost = d.get(gcd);
                Integer ncost = d.get(ngcd);
                if (ncost == null) {
                    gcds.add(ngcd);
                    d.put(ngcd, cost + c[i]);
                } else {
                    d.put(ngcd, min(ncost, cost + c[i]));
                }
            }
            Integer lc = d.get(l[i]);
            if (lc == null) {
                gcds.add(l[i]);
                d.put(l[i], c[i]);
            } else {
                d.put(l[i], min(lc, c[i]));
            }
        }
        Integer ans = d.get(1);
        println(ans == null ? -1 : ans);
    }
