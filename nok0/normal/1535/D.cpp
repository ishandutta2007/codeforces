...
void main_() {
	...
	auto cut = [&](int p, int x) {
		while(p >= 0) {
			data[p] -= data[x];
			if(p == 0 or !has_p[p]) break;
			p = (p - 1) >> 1;
		}
	};
	auto link = [&](int p, int x) {
		while(p >= 0) {
			data[p] += data[x];
			if(p == 0 or !has_p[p]) break;
			p = (p - 1) >> 1;
	...
	while(q--) {
		...
		int p = perm[--i];
		int l = p * 2 + 1, r = p * 2 + 2;
		if(s[i] != '0' and c == '0') cut(p, r), has_p[r] = 0;
		if(s[i] != '1' and c == '1') cut(p, l), has_p[l] = 0;
		if(s[i] == '1' and c != '1') link(p, l), has_p[l] = 1;
		if(s[i] == '0' and c != '0') link(p, r), has_p[r] = 1;
		s[i] = c;
		print(data.front());
	}
}
