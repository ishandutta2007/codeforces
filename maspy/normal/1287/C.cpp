...
void solve() {
  ...
  //  -> 
  const ll INF = 1LL << 60;
  vvv(ll, DP, N, N + 1, 2, INF);
  using T = tuple<int, int, int>;
  vvv(T, PAR, N, N + 1, 2, {-1, -1, -1});
  FOR(i, N) {
    FOR(x, 2) {
      if (A[i] != -1 && (x + A[i]) % 2 != 0) continue;
      if (i == 0) {
        DP[i][x][x] = 0;
        continue;
      }
      FOR(j, N + 1) FOR(k, 2) {
        ...
        ll j1 = j + (x == 1 ? 1 : 0);
        if (chmin(DP[i][j1][x], DP[i - 1][j][k] + (x != k ? 1 : 0))) {
          PAR[i][j1][x] = {i - 1, j, k};
        }
      }
    }
  ...
  print(MIN(DP[i][j]));
  /*
  ll k = min_element(all(DP[i][j])) - DP[i][j].begin();
  vi X;
  while (i >= 0) {
    X.eb(k);
    if (i == 0) break;
  ...
  FOR(i, N) {
    if (A[i] != -1) continue;
    if (X[i] == 0) A[i] = pick(ev);
    if (X[i] == 1) A[i] = pick(od);
  }
  for (auto&& x: A) ++x;
  print(A);
  */
