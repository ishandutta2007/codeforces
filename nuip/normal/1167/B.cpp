...
int main(){
	...
	// 			int dif=0;
	// 			rep(i,4)if(A[i]*A[i+1]!=B[i]*B[i+1]) dif=1;
	// 			if(!dif) out(A,B,1);
	// 		}
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,4){
		...
		do{
			int dif=0;
			rep(i,4)if(re[i]*re[i+1]!=a[i]) dif=1;
			if(dif) continue;
			cout<<"!";
			rep(i,6) cout<<" "<<re[i];cout<<endl;
			return 0;
		}while(next_permutation(all(re)));
  return 0;
}