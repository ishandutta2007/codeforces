...
int main() {
    ...
    rep(i, m) sum += x[i].se;
    if(sum < n) drop(-1);
    RmQ<ll> seg(vll(n + 1, inf<ll>));
    seg.update(0, 0);
    rep(i, m) {
        vll v(n + 1);
        ll s = 0, t = 0;
        rep(j, n + 1) {
            v[j] = seg.query(max(0LL, j - x[i].se), j + 1) + s - t;
            // dump(max(0LL, j - x[i].se), j + 1, seg.query(max(0LL, j - x[i].se), j + 1), s, t, v[j]);
            if(j == n) break;
            s += abs(a[j] - x[i].fi);
            if(i < m - 1) t += abs(a[j] - x[i + 1].fi);
        }
        rep(i, n + 1) seg.set(i, v[i]);
        // dump(v);
        seg.build();
    }
    cout << seg[n] << endl;
