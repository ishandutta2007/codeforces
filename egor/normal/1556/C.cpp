...
class CSzhatayaSkobochnayaPosledovatelnost {
public:
    void solve() {
        int n = in.readInt();
        ...
        for (int i = 0; i < n; i += 2) {
            ...
            for (int j = i + 1; j < n; j++) {
                if (j % 2 == 0) {
                    ...
                    if (c[j] > openTo) {
                        answer += openTo - openFrom + 1;
                        break;
                    } else if (c[j] >= openFrom) {
                        answer += c[j] - openFrom + 1;
                        openFrom = 0;
                        openTo -= c[j];
                    } else {
                        openFrom -= c[j];
...
int main() {
    ...
    auto time = clock();
#endif
    CSzhatayaSkobochnayaPosledovatelnost solver;
