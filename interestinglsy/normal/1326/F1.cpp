...
constexpr int MAXMASK = 262144;

int n,k;
bool know[MAXN][MAXN];
int knowMask[MAXN][MAXN];
	...
	For(mask,k-1){
		...
		For0(last,n){
			...
			For0(v,n){
				...
				if(know[last][v])
					dp[mask|1<<v][v] += dp[mask][last];
			}
			g[count][mask] += dp[mask][last];
			// P(bitset<4>(mask),last,dp[mask][last]);
		}
	}
	For(len,n){
		auto g = ::g[len];
...
void Enumerate(){
	static vc<int> lens;
	function<void(int,int,function<void()>)>
	Dfs = [&]( int totLen , int lastLen , const auto &Callback ){
		if( totLen == n ){
			Callback();
			return;
		}
		Forx(nowLen,1,min(lastLen,n-totLen)){
			lens.Eb(nowLen);
			Dfs(totLen+nowLen,nowLen,Callback);
			lens.pop_back();
		}
	};

	auto GetLensHash = [&]( const vc<int> lens ){
		int hsh = 1;
		for( auto p : lens )
			hsh = hsh<<p | 1;
		return hsh;
	};
	static vc<int> masks[524288];
	...
	For0(mask,1<<n-1){
		...
		if(cnt1) lens.Eb(cnt1+1);
		sort(All(lens),[]( int a , int b ){ return a > b; });
		// O(accumulate(All(lens),0));
		assert( accumulate(All(lens),0) == n );
		// Print(lens);
		masks[GetLensHash(lens)].Eb(mask);
	}
	...
	Dfs(0,n,[&](){
		...
		// Print(lens);
		// Print(d,0,k-1);
		int curHsh = GetLensHash(lens);
		for( auto mask : masks[curHsh] )
			dp[mask] = full;
	});
	// For0(i,1<<n-1){
	// 	P(bitset<3>(i),dp[i]);
