...
void update1(int n,int s,int e,int l,int v){
    rmv(n,s,e);
    if (s==e){
        seg[n]=v;
        return;
    }
    if (l<=M) update1(2*n,s,M,l,v);
    else update1(2*n+1,M+1,e,l,v);
}
void update2(int n,int s,int e,int l,int r,int v){
    rmv(n,s,e);
    if (r<s || l>e) return;
    if (l<=s && e<=r){
        lazy[n]=max(lazy[n],v);
    ...
    update2(2*n,s,M,l,r,v);
    update2(2*n+1,M+1,e,l,r,v);
}

int query(int n,int s,int e,int l){
    rmv(n,s,e);
    if (s==e) return seg[n];
    if (l<=M) return query(2*n,s,M,l);
    return query(2*n+1,M+1,e,l);
}
void solve(){
    int n,q,tp,p,x;
    cin>>n;
