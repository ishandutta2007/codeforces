...
int main(){
  ...
  for(int cas=1;cas<=T;cas++){
    read(n);
    for(int i=1;i<=n;i++)lst[i]=max[i]=0,ans[i]=1919810;
    for(int i=1;i<=n;i++)read(a[i]);
    for(int i=1;i<=n;i++){
      max[a[i]]=std::max(max[a[i]],i-lst[a[i]]);
      lst[a[i]]=i;
    }
    for(int i=1;i<=n;i++){
      max[i]=std::max(max[i],n+1-lst[i]);
    }
    for(int i=1;i<=n;i++){
      ans[max[i]]=std::min(ans[max[i]],i);
    }
    for(int i=2;i<=n;i++){
