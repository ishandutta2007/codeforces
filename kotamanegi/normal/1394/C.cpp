...
int judge(int a, int b) {
    int ans = 0;
    REP(i, inputs.size()) {
        int value = max({ 0LL,inputs[i].first - a,inputs[i].second - b }) + max({ 0LL,a - inputs[i].first,b - inputs[i].second });
        ans = max(ans, value);
    }
    return ans;
}
pair<int, int> eval(int now) {
    int bot = 0;
    int top = 6e5;
    if (now == 0) bot++;
    while (top - bot > 3) {
        int mid1 = (bot + bot + top) / 3;
        int mid2 = (bot + top + top) / 3;
        if (judge(now, mid1) < judge(now, mid2)) {
            top = mid2;
        }
        else {
            bot = mid1;
...
void solve() {
    ...
    int top = 6e5;
    while (top - bot > 3) {
        int mid1 = (bot + bot + top) / 3LL;
        int mid2 = (bot + top + top) / 3LL;
