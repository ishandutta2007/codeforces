...
void solve() {
  ...
  sort(all(A));
  vi CNT;
  for (auto&& k: key) CNT.eb(UB(A, k) - LB(A, k));
  auto I = argsort(CNT);
  vi rank(len(key));
  FOR(i, len(I)) rank[I[i]] = i;

  SegTree<Monoid_CntSum> seg(len(I));
  FOR(i, len(I)) { seg.set(i, {1, CNT[I[i]]}); }
  ...
  FOR(x, N + 2) {
    ...
    if (loss > K) break;
    // 
    auto check = [&](auto e) -> bool { return e.se <= K; };
    auto n = seg.max_right(check, 0);
    auto e = seg.prod(0, n);
    ll mex = x;
    ll total = seg.prod_all().fi;
    ll diff = total + x - e.fi;
    // print(mex, diff);
    chmin(ANS, diff - mex);

    // x 
    ll idx = LB(key, x);
    if (idx < len(key) && key[idx] == x) {
      ll i = rank[idx];
      seg.set(i, {0, 0});
    }
  }
