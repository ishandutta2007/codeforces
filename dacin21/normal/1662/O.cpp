...
void solve(){
    constexpr int R = 21;
    constexpr int A = 360;
    vector<bitset<A> > in(R), along(R);
    auto ind = [&](int r, int a){
        return A*r + a;
    };
    int n;
    cin >> n;
    ...
    for(int r=1; r<R; ++r){
        for(int a=0; a<A; ++a){
            if(!in[r][a]) uni.u(ind(r-1, a), ind(r, a));
        }
    }
    for(int r=0; r<R; ++r){
        for(int a=0; a<A; ++a){
            if(!along[r][a]) uni.u(ind(r, a), ind(r, (a+A-1)%A));
        }
    }
    if(uni.f(ind(0, 0)) == uni.f(ind(R-1, 0))){
        cout << "YES\n";
    } else {
        cout << "NO\n";
    }
