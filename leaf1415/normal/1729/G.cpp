...
void bfs01(vector<edge> G[], int n, int S, ll dist[])
{
	deque<int> deq;
	deq.push_back(S);
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, n) if(i+sz(t) <= sz(s) && s.substr(i, sz(t)) == t) b[i] = true;
			
		rep(i, 0, n+1) rep(j, 0, n+1) G[get(i, j)].clear();
		rep(i, 0, n){
			rep(j, 0, n){
				ll pj = j;
				if(j == 0) pj = -inf;
				if(b[i+1] && i+1 >= pj+sz(t)) G[get(i+1, i+1)].pb(edge(get(i, j), 1));
				if(i+1 < sz(t) || !b[i+1-sz(t)+1] || (pj >= i+1-2*sz(t)+2)) G[get(i+1, j)].pb(edge(get(i, j), 0));
			}
		}
		ll T = get(n+1, n+1);
		rep(i, 0, n) G[T].pb(edge(get(n+1, i), 0));
		...
		per(i, T, 1){
			for(auto e : G[i]){
				if(dist[i] + e.cost == dist[e.to]) dp[e.to] += dp[i];
			}
		}
		outl(dist[0], dp[0]);
	}
