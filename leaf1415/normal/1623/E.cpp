...
vector<ll> avec;
void dfs2(int v, int last, int ng)
{
	//outl(v, last, ng);
	avec.pb(v);
	
	if(l[v]) dfs2(l[v], last, ng);

	if(ng == 0 && !b[v] && a[v]){
		ll d = sz(avec)-1-last;
		if(d <= k){
			k -= d;
			rep(i, 1, d) b[avec.end()[-i]] = true;
		}
	}
	
	if(b[v]) last = sz(avec)-1;
	else ng = 1;
	if(r[v]) dfs2(r[v], last, ng);
...
int main(void)
{
	...
	ll pos = n;
	per(i, n-2, 0){
		if(c[vec[i]] != c[vec[i+1]]) pos = i+1;
		if(pos < n && c[vec[pos]] > c[vec[i]]) a[vec[i]] = true;
	}
