...
//    FREOUT("out.txt");
    ...
    FI(d) {
        int z[4] = {
            cntL[max(divans[i][0], divans[i][2]) - 1],
            cntR[min(divans[i][0], divans[i][2]) + 1],
            cntU[max(divans[i][1], divans[i][3]) - 1],
            cntD[min(divans[i][1], divans[i][3]) + 1],
        };
        if (divans[i][0] != divans[i][2]) {
            z[0]--;
            z[1]--;
        } else {
            z[2]--;
