...
int main() {
    ...
    vector<long double> rmx(N + 1), rmn(N + 1);
    rep(i, N)rmx[i + 1] = rmx[i] + mx[i];
    for(ll i=N-1;i>=0;i--)rmn[i] = rmn[i + 1] + mn[i];

    vector<long double> ra(N + 1), rb(N + 1);

    rep(i, N + 1) {
        long double prd = rmx[i];
        long double sum = rmx[i] - rmn[i] + 1;

        //cout << prd << " " << sum << endl;

        ra[i] = (sum + sqrt(sum * sum - 4.0 * prd + 1e-16)) / 2.0;
        rb[i] = (sum - sqrt(sum * sum - 4.0 * prd + 1e-16)) / 2.0;
    }

    rep(i, N)cout << ra[i + 1] - ra[i] << " ";
    cout << endl;
    rep(i, N)cout << rb[i + 1] - rb[i] << " ";
    cout << endl;
}