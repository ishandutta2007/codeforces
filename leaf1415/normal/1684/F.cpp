...
int main(void)
{
	...
	while(T--){
		cin >> n >> m;
		rep(i, 1, n) cin >> a[i];
		rep(i, 1, n) lvec[i].clear(), rvec[i].clear();
		rep(i, 1, m){
			cin >> l[i] >> r[i];
			lvec[l[i]].pb(i), rvec[r[i]].pb(i);
		}
		...
		S.insert(inf);
		rep(i, 1, n){
			for(auto x : lvec[i]) S.insert(i);
			L[i] = *S.begin();
			for(auto x : rvec[i]) S.erase(S.lower_bound(l[x]));
		}
		S.clear(), S.insert(-inf);
		per(i, n, 1){
			for(auto x : rvec[i]) S.insert(i);
			R[i] = *S.rbegin();
			for(auto x : lvec[i]) S.erase(S.lower_bound(r[x]));
		}
		
		ll l = inf, r = -inf;
		rep(i, 1, n) seg.update(i, -inf);
		for(auto p : mp){
			vector<ll> &vec = p.se;
			reps(i, vec){
				if(i-1 >= 0 && L[vec[i]] <= vec[i-1]) chmin(l, vec[i]);
				if(i+1 < sz(vec) && vec[i+1] <= R[vec[i]]) chmax(r, vec[i]);
				
				if(R[vec[i]] < 0) continue;
				ll p = upper_bound(all(vec), R[vec[i]]) - vec.begin() - 1;
				if(i < p) seg.add(vec[i]+1, n, vec[p]);
			}
		}
		if(l > inf/2){
			outl(0);
