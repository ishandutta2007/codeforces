...
int main() {
    ...
    vector<long long> a(c + 1, -inf);
    rep(i, n) {
        input(long long, cost, damage, health);
        chmax(a[cost], damage * health);
    }

    vector<long long> b(c + 1, -inf);
    rep(i, 1, c + 1) {
        if (a[i] == -inf) continue;
        long long cost = 0;
        repinf(j, 1) {
    ...
    rep(i, 1, c + 1) chmax(b[i], b[i - 1]);

    
    input(int, m);
    vector<int> ans(m);
    rep(i, m) {
        input(long long, damage, health);
        int x = lower_bound(all(b), damage * health + 1) - b.begin();
        ans[i] = x > c ? -1 : x;
    }
