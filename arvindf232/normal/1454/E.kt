...
fun main(){
    solve.cases{
        ...
        fun dfsSize(a:Int, p:Int):Int{
            var here = 1
            G.NS(a){v ->
                if(visited[v]) return@NS
                if(v == p) return@NS
                here += dfsSize(v,a)
            }
            return here
        ...
        fun choose2(n:Int):Long{
            return 1L*  (n) * (n-1) / 2
        }
        var total = choose2(n) * 2
        for(a in C){
            total -= choose2(sizes[a])
        }
...
fun Graph.getAnyCycle():List<Int>{
	val visited = BooleanArray(n)
