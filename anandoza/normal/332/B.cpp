...
int main() {
    ...
    auto make_dp = [&]() {
        vll prefix{0};
        partial_sum(all(x), back_inserter(prefix));

        vll dp(n + 1); // ans with first i elements
        dp[0] = 0;
        for (int i = 1; i <= n; i++) {
            dp[i] = dp[i - 1];
            int r = i, l = i - k;
            if (l >= 0)
                ckmax(dp[i], prefix[r] - prefix[l]);
        }
        return dp;
    };
    ...
    for (int left = 0; left <= n; left++) {
        ...
        ll score = dp[left] + rev[right];
        if (score > best.first) {
            best = {score, left};
        }
    }

    int left = best.second;

    vll prefix{0};
    partial_sum(all(x), back_inserter(prefix));

    auto get = [&](int l) -> ll { return prefix[l + k] - prefix[l]; };
