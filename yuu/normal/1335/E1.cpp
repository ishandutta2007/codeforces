...
vector<int> pos[201];
int f[201][200001];
int most_popular(int u, int v) {
    int ans = 0;
    for (int i = 1; i <= 200; i++) ans = max(ans, f[i][v] - f[i][u - 1]);
    return ans;
}
void solve() {
    cin >> n;
    ...
    for (int i = 1; i <= 200; i++)
        for (int j = 1; j <= n; j++) f[i][j] = f[i][j - 1] + (a[j] == i);
    for (int i = 1; i <= n; i++) pos[a[i]].emplace_back(i);
    int ans = most_popular(1, n);
    for (int i = 1; i <= 200; i++) {
        for (int x = 1; x * 2 <= pos[i].size(); x++) {
            ans = max(ans, x * 2 + most_popular(pos[i][x - 1] + 1, pos[i][pos[i].size() - x] - 1));
        }
    }
    cout << ans << '\n';
}
