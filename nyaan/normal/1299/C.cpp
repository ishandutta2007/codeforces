...
int ccw(P &a, P &b, P &c)
{
	ll t = (b.first - a.first)*(c.second - a.second) - (b.second - a.second)*(c.first - a.first);
  trc(a , b , c , t);
	if (t < 0)return -1;
	if (t == 0)return 0;
	return 1;
...
void solve(){
  ...
  vector<P> conv(N + 10); 
  conv.emplace_back(0LL, 0LL); int k = 1;
  // 
  for(int i = 0; i < N; i++ ){
    trc(i , k);
    trc(conv);
    ...
    while(k >= 2 && ccw(conv[k - 2] , conv[k - 1] , cur) <= 0 ) k--;
    conv[k] = cur;
    k++;
  }
  conv.resize(k);
  ...
  vd ans(N);
  rep(i , sz(conv) - 1){
    P left = conv[i] , right = conv[i + 1];
    ll nya = s[right.first] - s[left.first];
    for(int i = left.first ; i < right.first ; i++){
      ans[i] = 1.0 * nya / (right.first - left.first);
    }
  }
  out(ans);
