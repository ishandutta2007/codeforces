...
bool get(ll x, ll y)
{
	...
	ll p = lower_bound(all(vec[y]), x) - vec[y].begin();
	if(p < sz(vec[y]) && vec[y][p] == x) return true;
	else return false;
}
...
int main(void)
{
	...
	rep(i, 0, 100000){
		sort(all(vec[i]));
		if(sz(vec[i]) >= B) bvec.pb(i);
	}
	
	ll ans = 0;
	rep(y, 0, 100000){
		if(sz(vec[y]) >= B) continue;
		reps(i, vec[y]){
			reps(j, vec[y]){
				if(i <= j) break;
				ll x1 = vec[y][i], x2 = vec[y][j], d = abs(x1-x2);
				if(y+d <= 100000 && sz(vec[y+d]) < B && get(x1, y+d) && get(x2, y+d)) ans++;
			}
		}
	}
	rep(y, 0, 100000){
		for(auto x : vec[y]){
			for(auto y2 : bvec){
				ll d = y2 - y;
				if(d == 0) continue;
				if(sz(vec[y]) >= B && d < 0) continue;
				if(get(x, y2) && get(x+d, y) && get(x+d, y2)) ans++;
			}
		}
	}
