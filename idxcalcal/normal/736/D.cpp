...
int main() {
  ...
  n = read(), m = read();
  for (ri i = 1; i <= m; ++i) {
    b[i].fi = read() - 1, b[i].se = read() - 1;
    a[b[i].fi][b[i].se] = 1;
  }
  for (ri i = 0; i < n; ++i) a[i][i + n] = 1;
  for (ri i = 0, ps; i < n; ++i) {
    for (ps = i; ps < n; ++ps) if (a[ps][i]) break;
    if (ps ^ i) swap(a[ps], a[i]);
    for (ri j = 0; j < n; ++j) if (j != i && a[j][i]) a[j] ^= a[i];
  }
  for (ri i = 1; i <= m; ++i) {
    if (!a[b[i].se][b[i].fi + n]) puts("YES");
    else puts("NO");
  }
  return 0;
}