...
int main() {
	...
	const int IMP = -1e9;
	vector<vector<int>> dp(n + 1, vector<int>(100 * n + 107, IMP));
	dp[0][0] = 0;
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 100 * (n - 1); j >= 0; j--) {
			for (int take = 0; take < n; take++) {
				if (dp[take][j] != IMP) {
					chkmax(dp[take + 1][j + a[i]], dp[take][j] + b[i]);
				}
			}
		}
	}
	...
	for (int i = 0; i < n; i++) {
		for (int maxA = 0; maxA <= 100 * n; maxA++) {
			double maxB = (bSum + dp[i + 1][maxA]) / 2.0;
			chkmax(ans[i], min<double>(maxA, maxB));
		}
	}
