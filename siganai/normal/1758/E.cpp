...
int main() {
    ...
    while(TT--) {
        ...
        auto ret = uf.groups();
        vi val(n+m,-1);
        auto dfs = REC([&](auto f,int now) -> bool {
            if(val[now] == -1) val[now] = 0;
            for(auto &nex:g[now]) {
                if(val[nex] == -1) {
                    if(nex >= n && a[now][nex - n] != -1) {
                        val[nex] = (2 * h - a[now][nex - n] - val[now]) % h;
                    }
                    if(nex < n && a[nex][now - n] != -1) {
                        val[nex] = (2 * h - a[nex][now - n] - val[now]) % h;
                    }
                    if(!f(nex)) return false;
                }
                else {
                    if(nex >= n && a[now][nex - n] != -1) {
                        if(val[nex] != (2 * h - a[now][nex - n] - val[now]) % h) return false;
                    }
                    if(nex < n && a[nex][now - n] != -1) {
                        if(val[nex] != (2 * h - a[nex][now - n] - val[now]) % h) return false;
                    }
                }
            }
            return true;
        ...
        set<int> s;
        rep(i,n) rep(j,m) {
            if(a[i][j] == -1 && (!uf.same(i,j+n))) {
                s.insert(uf.find(i));
                s.insert(uf.find(j+n));
            } 
        }
        cout << mint(h).pow((int)s.size() - 1) << '\n';
    }
}