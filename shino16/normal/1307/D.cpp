...
template <typename T> T divfloor(T m, T d) {
  if (m >= 0 and d >= 0 or m <= 0 and d <= 0) return m / d;
...
int main() {
  ...
  VI dist0 = dijkstra(graph, 0);
  VI dist1 = dijkstra(graph, n-1);

  vector<PII> ps;
  rep(i, k) ps.emplace_back(dist0[a[i]], dist1[a[i]]);
  sort(all(ps), [](auto s, auto t){return s._1-s._2 < t._1-t._2;});

  int val = 0, maxI = 0;
  rep(j, 1, k) {
    chmax(maxI, ps[j-1]._1);
    chmax(val, maxI + ps[j]._2);
  }
  OUT(min(dist0[n-1], val + 1));
}