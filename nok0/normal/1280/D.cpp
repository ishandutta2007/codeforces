...
void main_() {
	...
	graph tmp(n, -1);
	array<pll, 2> base;
	base.fill(pll(-INF, -INF));
	//dp[i][j][k]:ijk:,,{good ,}
	vector dp(n, vector(2, base));
	auto g = tmp.root_to_leaf();
	auto dfs = [&](auto dfs, int now) -> void {
		dp[now][0][0] = {0, b[now]};
		dp[now][1][1] = {b[now] > 0, 0};
		for(auto [to, cost] : g[now]) {
			dfs(dfs, to);
			int x = dp[now].size();
			int y = dp[to].size();
			vector ndp(x + y - 1, base);
			REP(i, x) {
				REP(j, y) {
					REP(ik, 2) {
						REP(jk, 2) {
							if(ik) {
								if(jk) {
									chmax(ndp[i + j][1], pll{dp[now][i][ik].first + dp[to][j][jk].first, 0});
								}
							} else {
								if(jk) {
									chmax(ndp[i + j][0], pll{dp[now][i][ik].first + dp[to][j][jk].first, dp[now][i][ik].second});
								} else {
									chmax(ndp[i + j][0], pll{dp[now][i][ik].first + dp[to][j][jk].first, dp[now][i][ik].second + dp[to][j][ik].second});
									if(i + j + 1 < x + y - 1) chmax(ndp[i + j + 1][1], pll{dp[now][i][ik].first + dp[to][j][jk].first + (dp[now][i][ik].second + dp[to][j][jk].second > 0), 0});
								}
							}
						}
					}
