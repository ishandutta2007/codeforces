...
int main() {
	...
	REP(_, t) {
		...
		vector <vi> occs(201);
		REP(i, n) occs[in[i]].push_back(i);
		int ans = 0;
		REP(i, 201) maxi(ans, (int)occs[i].size());
		REP(i, 201) REP(j, 201) if (i != j) {
			const vi &a = occs[i], &b = occs[j];
			if (a.empty() || b.empty()) continue;
			debug(imie(i), imie(j));
			vi eaten(a.size() + 2);
			int p = 0;
			REP(i, a.size()) {
				while (p < (int) b.size() && b[p] < a[i]) p++;
				eaten[i + 1] = p;
			}
			eaten.back() = b.size();
			debug(imie(a), imie(b), imie(eaten));
			REP(take, a.size() / 2 + 1) {
				int curr = 2 * take + (eaten[a.size() + 1 - take] - eaten[take]);
				debug(imie(a), imie(b), imie(take), imie(curr));
				maxi(ans, curr);
			}
		}
		printf("%d\n", ans);
