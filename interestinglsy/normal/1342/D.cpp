...
int main(){
	...
	static vc<int> cnt[MAXN];
	For(i,n) cnt[a[i]].Eb(i);

	static int putted[MAXN];
	static vc<int> ansl[MAXN];
	fOR(i,k){
		if(cnt[i].empty()) continue;
		int l = 1, r = MAXN-1, ans = r;
		while( l <= r ){
			int mid = l+r >> 1;
			if(lim[i]-putted[mid]){
				ans = mid;
		...
		Forx(j,ans,MAXN-1){
			int canPut = lim[i] - putted[j];
			int use = min(canPut,(int)cnt[i].size());
			putted[j] += use;
			For(_t,use){
				ansl[j].Eb(i);
				cnt[i].pop_back();
