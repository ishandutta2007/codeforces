...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int x;
        cin >> x;
        if (x == 1) a[i % e].push_back(1);
        else if (isp[x] == 0) a[i % e].push_back(2);
        else a[i % e].push_back(4);
    }
    ...
    for (int i = 0; i < e; i++)
    {
        for (int j = 0; j < a[i].size(); j++)
        {
            ...
            {
                int L = j, R = j;
                while (L - 1 >= 0 && a[i][L - 1] == 1) L--;
                while (R + 1 < a[i].size() && a[i][R + 1] == 1) R++;
                ans = ans + 1ll * (j - L + 1) * (R - j + 1) - 1;
            }
        }
    }
    cout << ans << "\n";
