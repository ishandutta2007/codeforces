...
public class TaskD {
	...
	private void solve() {
		...
		int[] inRays = in.readIntArray(rayCount);
		int[] outRays = in.readIntArray(rayCount);
		int[] order = new int[rayCount];
		int[] reverseOut = new int[rayCount];
		for (int i = 0; i < rayCount; i++)
			reverseOut[outRays[i] - 1] = i;
		for (int i = 0; i < rayCount; i++)
			order[i] = reverseOut[inRays[i] - 1];
		int[] d = new int[rayCount + 1];
		Arrays.fill(d, -Integer.MAX_VALUE);
		d[0] = 1;
		int size = 0;
		for (int i = 0; i < rayCount; i++) {
			int index = Arrays.binarySearch(d, 0, size, -order[i]);
			if (index < 0) {
				index = -index - 1;
				d[index] = -order[i];
