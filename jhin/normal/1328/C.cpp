    #include <bits/stdc++.h>
            ...
            string ss[5000];
                ...
                int main()
                {
                  ...
                  /*  cin>>t;
                    vector<long long>tmp;tmp.push_back(1);
                    for(long long i=1;i<100000;i++)tmp.push_back((i*(i+1))/2);
                    while(t--)
                    {
                       cin>>x>>y;


                      auto it= lower_bound(tmp.begin(),tmp.end(),y);
                      if(y<*it)it--;
...
{
    cin>>n>>s;
    string s1="",s2="";yes=true;
    for(int i=0;i<n;i++)
    {
        if(s[i]=='2'&&yes)s1+='1',s2+='1';
        else if(s[i]=='1'&&yes)s1+='1',s2+='0',yes=false;
        else if(s[i]=='0'&&yes)s1+='0',s2+='0';
        else if(!yes)
            s1+='0',s2+=s[i];
    }
