...
void solve() {
	...
	if (nesi == 0) {
		ll ans = 0;
		REP(i, n) ans += pos[i] * 1ll * i;
		printf("%lld\n", ans);
		return;
	}
	REP(i, nesi) sums[i + 1] = sums[i] + neg[i];
	vll largest(n + 1, -llinf);
	ll curr = 0;
	for (int i = 0; i < nesi; ++i) curr += neg[i] * 1ll * (i / (k + 1));
	int top = (nesi - 1) / (k + 1) + 1;
	largest[top] = curr;
	int inc = nesi % (k + 1) == 1 ? 2 : 1;
	debug(arr(largest, top), imie(inc));
	debug(imie(sums));
	debug(imie(nesi), imie(top));
	for (int i = top + 1; i <= nesi; ++i) {
		// debug(range(neg, nesi - inc, nesi), imie(inc));
		assert(inc <= nesi);
		curr += (sums.back() - sums[nesi - inc]);
		inc++;
		if ((nesi - i) % k == 0) inc++;
		largest[i] = curr;
		debug(imie(i), imie(curr), imie(neg));
	}
	ll sum = 0, psum = 0;
