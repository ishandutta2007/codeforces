...
#line 1 "/home/maspy/library/other/random.hpp"
...
vi primetable(int N) {
  ...
  FOR(p, len(is_prime)) if (is_prime[p] && p <= N) primes.eb(p);
  return primes;
}
...
void solve() {
  ...
  FOR(i, P) {
    ...
    ll q = p;
    while (q <= N) {
      FOR(j, N / q + 1) num[q * j] ^= HP[i];
      q *= p;
    }
  }
  vi fact(N + 1);
  FOR(i, N) fact[i + 1] = num[i + 1] ^ fact[i];
  ll full = 0;
  FOR(x, N + 1) full ^= fact[x];

  if (N <= 10) {
    // solve small
    vi ANS;
    FOR(s, 1 << N) {
      if (popcnt(s) <= len(ANS)) continue;
      ll x = 0;
      FOR(i, N) if (s & 1 << i) x ^= fact[i + 1];
      if (x == 0) {
        ANS = {};
        FOR(i, N) if (s & 1 << i) ANS.eb(i + 1);
      }
    }
    print(len(ANS));
    print(ANS);
  ...
  // N - 1
  FOR3(x, 1, N + 1) if (len(OMIT) > 1 && full == fact[x]) { OMIT = {x}; }
  assert(check());

  [&]() {
    // N - 2
    if (len(OMIT) <= 1) return;
