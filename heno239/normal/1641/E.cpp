...
poly ordered_multiply(poly p, poly q, bool eq) {
	poly res;
	/*rep(i, p.size())Rep(j, i + 1, q.size()) {
		addv(res, i + j, p[i] * q[j]);
	}*/
...
void solve() {
	...
	poly p1 = ordered_multiply(pl, pr, true);
	pl.clear();
	pr.clear();
	for (int i = -ad; i < n; i++) {
		int loc = 2 * i;
	...
	poly p2 = ordered_multiply(pr, pl, false);
	poly pp;
	rep(i, p1.size())addv(pp, i, p1[i]);
	rep(i, p2.size())addv(pp, i, p2[i]);
	modint ans = 0;
	rep(i, n) {
		modint val = 0;
		if (i + 2 * ad < pp.size())val = pp[i + 2 * ad];
		int cr = 2 * n - 1 - i;
		int cl = max(abs(i - p[0]), abs(i - p.back()));
		//all cleared
		int len = cr;
		//cout << i << " " << len << "\n";
		//cout << i << " " << val * (modint)mod_pow(2, m) << "\n";
		modint dec = mod_pow(2, -m);
		val -= dec * (modint)len;
		val *= a[i];
		ans += val;
		//cout << i << " " << val * (modint)mod_pow(2, m) << "\n";
