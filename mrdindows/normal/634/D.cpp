...
int root = 1;
int lastn = 2;
inline int getMin(int cur, int l, int r) {
    return cur == 0 ? l + n : trmin[cur];
}
inline int getMax(int cur, int l, int r) {
    return cur == 0 ? r : trmax[cur];
}
inline void create(int &cur, int l, int r) {
    if (cur) return;
    cur = lastn++;
    trmin[cur] = l + n;
    trmax[cur] = r;
}
inline void push(int cur, int m, int l, int r) {
    if (!pu[cur]) return;
    create(lp[cur], l, m);
    create(rp[cur], m + 1, r);
...
//    DBN("u", l, r, x, y);
    ...
    if (x == l && y == r) {
        ...
        int m = (l + r) / 2;
        push(cur, m, l, r);
        upd(lp[cur], l, m, x, y, val);
        upd(rp[cur], m + 1, r, x, y, val);
        trmin[cur] = min(getMin(lp[cur], l, m), getMin(rp[cur], m + 1, r));
        trmax[cur] = max(getMax(lp[cur], l, m), getMax(rp[cur], m + 1, r));
    }
}
int getMin(int cur, int l, int r, int x, int y) {
    if (cur == 0) return getMin(cur, x, y);
    if (l == x && r == y) {
        return trmin[cur];
    } else {
        int m = (l + r) / 2;
        push(cur, m, l, r);
        if (y <= m) {
            return getMin(lp[cur], l, m, x, y);
        } else if (x > m) {
            return getMin(rp[cur], m + 1, r, x, y);
        } else {
            return min(getMin(lp[cur], l, m, x, m), getMin(rp[cur], m + 1, r, m + 1, y));
        }
    }
}
int getMax(int cur, int l, int r, int x, int y) {
    if (cur == 0) return getMax(cur, x, y);
    if (l == x && r == y) {
        return trmax[cur];
    } else {
        int m = (l + r) / 2;
        push(cur, m, l, r);
        if (y <= m) {
            return getMax(lp[cur], l, m, x, y);
        } else if (x > m) {
            return getMax(rp[cur], m + 1, r, x, y);
        } else {
            return max(getMax(lp[cur], l, m, x, m), getMax(rp[cur], m + 1, r, m + 1, y));
        }
    }
}
pii a[201010];
int main() {
    ...
    for (int i = 0; i <= m; ++i) {
        int pos = a[i].first;
        int howManyNeed = getMax(root, 0, d, pos, pos);
//        DBN(i, s, pos, howManyNeed);
        while (howManyNeed > 0) {
            if (s.empty()) {
                cout << -1 << endl;
                return 0;
            ...
            int id = it->second;
            int pos = a[id].first;
            int howManyCanBuy = getMin(root, 0, d, pos, d);
            int howManyBuy = min(howManyCanBuy, howManyNeed);
            if (howManyBuy == 0) {
                s.erase(it);
                continue;
            }
            upd(root, 0, d, pos, d, howManyBuy);
            int cost = a[id].second;
            ans += howManyBuy * (ll)cost;
//            DBN(pos, cost);
//            DBN(howManyNeed, howManyCanBuy);
            howManyNeed -= howManyBuy;
        }
        if (i != m) {
