...
object solve{
    ...
    var tn:Long = 0
    fun cases(onecase:(Int,Int)->Unit){
...
fun Graph.hasDirectedCycle():Boolean{
...
fun Graph.toplogicalSort(): IntArray {
    val ans = IntArray(n)
    var pointer = -1
    this.DFS_Long_Exhaust_iter({
        pointer ++
        ans[pointer] = it
    })
    return ans.reversedArray()
}
const val singleCase = false
fun Graph.topologicalSort2(): List<Int> {
    //backwards, start from outdegree 0 vertex
    val ret = mutableListOf<Int>()
    DFS_Long_Exhaust_iter({ret.add(it)},{}){a,b,_ ->}
    return ret
}
fun main(){
    solve.cases{t , ti ->
//        if(t > 5 && ti != 78) return@cases

        val n = getint()
        val m = getint()
        val G = Graph(n,true)
        val all = mutableListOf<Pair<Int,Int>>()
        val directed = mutableListOf<Pair<Int,Int>>()
        repeat(m){
            val t = getint()
             val a = getint() - 1
            val b = getint() - 1
...
//        }
        ...
        if(G.hasDirectedCycle()){
            ...
            put("YES")
            val ts = G.topologicalSort2()
            val pos = IntArray(n)
            for((i,v) in ts.withIndex()){
                pos[v] = i
            }
