...
public class Task58C {
	...
	private void solve() {
		...
		int[] trees = in.readIntArray(treeCount);
		int[] countVariant = new int[100001];
		for (int i = 0; i < treeCount; i++) {
			int distance = Math.min(i, treeCount - i - 1);
			if (trees[i] > distance)
				countVariant[trees[i] - distance]++;
		}
		int max = 0;
		for (int variant : countVariant)
			max = Math.max(variant, max);
		out.println(treeCount - max);
	}
...
//		return strictCheck(result, output);
	...
	private static void test() {
		...
		for (Test test : tests) {
			...
			try {
				new Task58C(in, new PrintWriter(out)).run();
	...
	private Task58C(InputReader in, PrintWriter out) {
		this.in = in;
		this.out = out;
		testMode = true;
	}
