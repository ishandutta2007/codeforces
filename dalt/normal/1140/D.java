...
public class CR548 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int n = io.readInt();
            long[][] mem = new long[501][501];
            boolean[][] visited = new boolean[501][501];

            long result = dp(mem, visited, 1, n);
            io.cache.append(result);
        }

        public long dp(long[][] mem, boolean[][] visited, int i, int j) {
            if (j - i < 2) {
                return 0;
            }
            if (!visited[i][j]) {
                ...
                long r = inf;
                for (int k = i + 1; k < j; k++) {
                    r = Math.min(r, (i * j) * k + dp(mem, visited, i, k)
                            + dp(mem, visited, k, j));
                }
                mem[i][j] = r;
            }
