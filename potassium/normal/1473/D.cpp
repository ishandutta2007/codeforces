...
struct S {
	int minv, maxv;
	S(int val) { minv = maxv = val; }
	S(int x, int y) : minv(x), maxv(y) { }
};
...
S op(S l, S r) {
	return S{ min(l.minv, r.minv), max(l.maxv, r.maxv) };
}
...
int main() {
	CASE(t) {
		...
		while (m--) {
			...
			if (1 < l) {
				S qry = st.prod(1, l);
				chkmin(minv, qry.minv);
				chkmax(maxv, qry.maxv);
			}

			if (r < n) {
				S qry = st.prod(r + 1, n + 1);
				int dx = ps[l - 1] - ps[r];
				chkmin(minv, qry.minv + dx);
				chkmax(maxv, qry.maxv + dx);
			}
