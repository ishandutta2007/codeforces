...
vc<int> e[MAXN];
int vis[MAXN], dfn[MAXN], nowtime, cyc;
void Dfs( int u ){
	vis[u] = 1;
	for( auto v : e[u] ){
		if(!vis[v]) Dfs(v);
...
bool Check( int lim ){	// <=lim
	...
	For(i,m)
		if( edges[i].c <= lim )
			if( dfn[edges[i].f] < dfn[edges[i].t] ){
				rev[i] = 1;
				++cnt;
			}
	return 1;
...
// XXXXXXXOOOOOOO
int Binarysearch(){
	int l = 0, r = 1000000001, mid, ans = INF;
	while( l <= r ){
		mid = l+r >> 1;
		if(Check(mid)){
			Mymin(ans,mid);
...
int main(){
	...
	Read(n,m);
	For(i,m) Read(edges[i].f,edges[i].t,edges[i].c);
