...
#line 2 "/tmp/tmp-4448RvrXKQ2nhC4Y.cpp"

#line 4 "/tmp/tmp-4448RvrXKQ2nhC4Y.cpp"
...
int main() {
  ...
  while (T--) {
    ...
    int max_r = 0, max_w = 0;
    for (int i = 0; i < m; ++i) {
      max_r = std::max(max_r, read());
      max_w = std::max(max_w, read());
    }
    if (max_r + max_w <= n) {
      std::cout << std::string(max_r, 'R') + std::string(n - max_r, 'W')
                << "\n";
    } else {
      std::cout << "IMPOSSIBLE\n";
    }
