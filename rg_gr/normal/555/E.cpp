// Problem: E. Case of Computer Network
...
arr dfn,low,cut;
int cc=0;
void dfs(int u,int fa,int from=00)
{
	dfn[u]=low[u]=++cc;
	int qaq=0;
	for(int i=h[u];i;i=e[i].nxt)
	{
		...
		if(!dfn[v])
		{
			...
			if(low[v]>dfn[u]&&v!=fa)cut[i]=cut[i^1]=1;
		}
		if(i!=(from^1))low[u]=min(low[u],dfn[v]);
	}
}
arr bel;int dd=0;
arr up,down;
...
{
	bel[u]=dd;
	fe(u)if(!cut[i]&&!bel[v])dfs2(v,u);gr
}
int n=read(),m=read(),q=read();namespace qwq{
	newe(1000005);
int F[1000005][21],dep[1000005],bl[1000005],cc;
...
int LCA(int u,int v)
{
	...
	int s=dep[u]-dep[v];
	for(int i=0;i<20;i++)if((s>>i)&1)u=F[u][i];
	if(v==u)return u;
...
{
	fe(u)
	if(v!=fa)pre(v,u),up[u]+=up[v],down[u]+=down[v];
	gr
}
signed main()
{
	// dfs(1,0);
	rg(n)if(!bl[i])cc++,dfs(i,0);gr
	rg(q)
	int s=read(),t=read();
	...
	// rg(n)odp(up[i],down[i]);gr
	rg(n)if(F[i][0]==0)pre(i,0);gr
	// rg(n)odp(up[i],down[i]);gr
	rg(n)if(up[i]&&down[i])return puts("No"),0;gr
	puts("Yes");
	return 0;
}
};signed main()
{
	initprog();
	rg(m)int u=read(),v=read();addedge(u,v,1);addedge(v,u,1);gr
	dfs(1,0);
	// rg(m)odb(cut[i*2]);gr puts("");
	rg(n)if(!bel[i])++dd,dfs2(i,i);gr
	rg_(u,n)fe(u)if(bel[u]!=bel[v])qwq::addedge(bel[u],bel[v],i>>1);gr gr
	qwq::main();
	return 0;
}