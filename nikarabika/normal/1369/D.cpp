...
const int MODS = 5;
LL Mods[MODS] = {998244353, 987654323, 123456789, 1000000009, 1000000007};
LL f[maxn];
LL g[maxn];
bool mark[maxn];
...
int main() {
    ...
    for (int _ = 0; _ < MODS; _++) {
        LL Mod = Mods[_];
        for (int i = 3; i < maxn; i++) {
            g[i] = (2 * f[i - 2] + f[i - 1]) % Mod;
            f[i] = g[i];
            if (mark[i] and f[i - 2] == g[i - 2] and f[i - 1] == g[i - 1])
                f[i] = (1 + 2 * g[i - 2] + g[i - 1]) % Mod;
            else
                mark[i] = false;
        }
    }
