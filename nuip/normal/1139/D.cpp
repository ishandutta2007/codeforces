...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	iota(tot,tot+112345,0);
	for(int p:primes)for(int i=p;i<112345;i+=p) tot[i]/=p, tot[i]*=p-1;
	int n;
	cin>>n;
	vector<mint> dp(n+1);
	vv<int> div(n+1);
	reps(i,2,n+1)for(int j=i+i;j<=n;j+=i) div[j].pb(i);
	reps(i,2,n+1){
		int m=div[i].size();
		vector<int> cnt(m);
		rep(j,m) cnt[j]=n/div[i][j]-n/i;
		rrep(j,m)reps(k,j+1,m)if(div[i][k]%div[i][j]==0) cnt[j]-=cnt[k];
		rep(j,m) dp[i]+=cnt[j]*dp[div[i][j]];
		// for(auto x:div[i]){
		// 	dp[i]+=n/i*tot[i/x]*dp[x];
		// 	int t=n%i/x;
		// 	reps(j,1,t+1)if(__gcd(j,n/x)==1) dp[i]+=dp[x];
		// }
		// {
		// 	int s=0;
		// 	for(int j:div[i]) s+=way[j];
		// 	// out(s+n/i,i,div[i],1);
		// 	assert(s+n/i==n);
		// }
	...
	cout<<accumulate(all(dp),mint(0))*inv[n]+1<<endl;
  return 0;
}