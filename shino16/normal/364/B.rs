...
fn main() {
    scan!(n: usize, d: usize, c: [usize; n]);

    let mut ok = vec![false; 10005 * 50];
    ok[0] = true;
    for &c in &c {
        for i in (0..ok.len() - c).rev() {
            ok[i + c] |= ok[i];
        }
    }
    let mut cum = vec![0; ok.len() + 1];
    for i in 0..ok.len() {
        cum[i + 1] = cum[i] + (ok[i]) as usize;
    }
    let mut prev = 0;
    let mut cnt = 0;
    while prev + d + 1 < cum.len() && cum[prev + d + 1] != cum[prev + 1] {
        let j = lower_bound(&cum[prev + 1..prev + d + 1], &cum[prev + d + 1]) + prev;
        prev = j;
        cnt += 1;
    }
    prtln!(prev, cnt);
