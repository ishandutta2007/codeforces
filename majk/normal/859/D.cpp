...
public:
    void solve(istream& cin, ostream& cout) {
        ...
        vector<double> Q(T, 1.0), S(T, 0.0);
        for (int i = 0; i < N; ++i) {
            vector<double> R(T, 0.0), Z = S;
            ...
            for (int j = 0; j < T; j += (2<<i)) {
                ...
                for (int k = 0; k < (1<<i); ++k) {
                    for (int l = 0; l < (1<<i); ++l) {
                        int t1 = j + k, t2 = j + (1<<i) + l;
                        R[t1] += Q[t1] * Q[t2] * P[t1][t2] / 100.0;
                        R[t2] += Q[t1] * Q[t2] * P[t2][t1] / 100.0;
                        Z[t1] += (((1 << i)) * (Q[t1] * Q[t2] * P[t1][t2] / 100.0));
                        Z[t2] += (((1 << i)) * (Q[t1] * Q[t2] * P[t2][t1] / 100.0));
                    }
                }
            }
        ...
        cout << fixed << setprecision(20) << *max_element(S.begin(),S.end()) << endl;
    }
};
