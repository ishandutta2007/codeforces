...
void main_() {
	INT(n, q);
	STR(s);
	vector dp(251, vector(251, vector(251, inf)));
	dp[0][0][0] = 0;

	VV<> nxt(n + 1, V<>(26, inf));
	RREP(i, n) {
		nxt[i][s[i] - 'a'] = i + 1;
		REP(j, 26) { chmin(nxt[i][j], nxt[i + 1][j]); }
	}
	...
	REP(_, q) {
		...
		if(op == '+') {
			...
			if(idx == 0) {
				...
				REP(i, len[1] + 1) {
					REP(j, len[2] + 1) {
						chmin(dp[len[0]][i][j], next(dp[len[0] - 1][i][j], str[0].back()));
						if(i) chmin(dp[len[0]][i][j], next(dp[len[0]][i - 1][j], str[1][i - 1]));
						if(j) chmin(dp[len[0]][i][j], next(dp[len[0]][i][j - 1], str[2][j - 1]));
					}
				}
			}
			...
			if(idx == 1) {
				...
				REP(i, len[0] + 1) {
					REP(j, len[2] + 1) {
						chmin(dp[i][len[1]][j], next(dp[i][len[1] - 1][j], str[1].back()));
						if(i) chmin(dp[i][len[1]][j], next(dp[i - 1][len[1]][j], str[0][i - 1]));
						if(j) chmin(dp[i][len[1]][j], next(dp[i][len[1]][j - 1], str[2][j - 1]));
					}
				}
			}
			...
			if(idx == 2) {
				...
				REP(i, len[0] + 1) {
					REP(j, len[1] + 1) {
						chmin(dp[i][j][len[2]], next(dp[i][j][len[2] - 1], str[2].back()));
						if(i) chmin(dp[i][j][len[2]], next(dp[i - 1][j][len[2]], str[0][i - 1]));
						if(j) chmin(dp[i][j][len[2]], next(dp[i][j - 1][len[2]], str[1][j - 1]));
					}
				}
			}
		}
