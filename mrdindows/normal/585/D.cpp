...
int a[3][30];
vector<tri> v[30];
vector<pair<int, string>> pred[30];

void restore(tri f, tri s) {
...
//        DBN(s);
        for (int i = 0; i < v[cur].size(); ++i) {
//            DBN(v[cur][i], pred[cur][i]);
            if (v[cur][i] == s) {
                auto pr = pred[cur][i];
//                DBN(pr);
                s = v[cur + 1][pr.first];
                ans2.push_back(pr.second);
                break;
            }
        }
        ++cur;
...
int main() {
    ...
    for (int i = 0; i <= hn; ++i) {
        for (int j = 0; j < v[i].size(); ++j) {
            auto t = v[i][j];
            v[i + 1].push_back({t.a + a[0][i], t.b + a[1][i], t.c});
            pred[i + 1].push_back({j, "LM"});
            v[i + 1].push_back({t.a + a[0][i], t.b, t.c + a[2][i]});
            pred[i + 1].push_back({j, "LW"});
            v[i + 1].push_back({t.a, t.b + a[1][i], t.c + a[2][i]});
            pred[i + 1].push_back({j, "MW"});
        }
    }
    v[n + 1].push_back({0, 0, 0});
    ...
    for (int i = 0; i < v[hn + 2].size(); ++i) {
//        DBN(v[hn + 2][i], pred[hn + 2][i]);
    }
    ...
    int ans = -INF;
    for (auto p : t) {
        tri e(p.a - p.b, p.b - p.c, -INF);
        auto ptr = lower_bound(str.begin(), str.end(), e);
        if (ptr == str.end() || ptr->a != e.a || ptr->b != e.b) {
            continue;
        }
        ok = true;
        ans = max(p.a - ptr->c, ans);
    }
    if (!ok) {
    ...
    for (auto p : t) {
        ...
        if (ans == p.a - ptr->c) {
            restore({-ptr->c, -ptr->c + ptr->a, -ptr->c + ptr->a + ptr->b}, p);
            return 0;
        }
    }
    return 0;
