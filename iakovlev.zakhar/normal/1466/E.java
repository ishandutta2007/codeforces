...
public class D {
	...
	public class Vertex implements Comparable<Vertex> {
		...
		public Vertex(int weight, int nei) {
			this.weight = weight;
			this.nei = nei;
		}
	...
	public void solve() {
		int maxp = 60;
		long[] pow = new long[maxp];
		pow[0] = 1;
		for (int i = 1; i < pow.length; i++) {
			pow[i] = mult(pow[i - 1], 2);
		}
		f : for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			...
			for (int j = 0; j < x.length; j++) {
				...
				for (int l = 0; l < b.length; l++) {
					long bit = (x[j] >> l) & 1;
					left = sum(left, mult(pow[l], b[l] * bit));
					right = sum(right, mult(pow[l], sum(n * bit, b[l] * (1 - bit))));
				}
				ans = sum(ans, mult(left, right));
			}
			out.println(ans);
		}
	}
