...
li brute(vi const& A) {
  ...
  li sum = accumulate(begin(A), end(A),0ll);
  li cap = (sum-1)/2;

  li ans = 0;
  for(li m = 0; m < (li)(1<<n); ++m) {
    li x = 0, y = 0;
    for(li i = 0; i < (li)(n); ++i) if(m&(1<<i)) x += A[i]; else y += A[i];
    if(x >= y) continue;

    vi cs,ps;
    for(li i = 0; i < (li)(n); ++i)if(m&(1<<i)) cs.push_back(i); else ps.push_back(i);
    for(li i = (1); i <= (li)(cs.size()-2); ++i) if(cs[i]-cs[i-1]>cs[i+1]-cs[i]) goto lail;
    for(li i = (1); i <= (li)(ps.size()-2); ++i) if(ps[i]-ps[i-1]<ps[i+1]-ps[i]) goto lail;
    ans++;
      lail:;
  }
  return ans;
...
li solve(vi const& A){
  ...
  for(li bp = 0; bp < (li)(2); ++bp) for(li bc = 0; bc < (li)(2); ++bc) {
    li cur = (bc ? A[n-1] : 0);
    for(li i = ((bp?1:0)); i <= (li)(n-2); ++i) {
      cur += A[i];
      if(cur > cap) break;
      auto can = [&](li x) -> bool {
        return cur + dp[2+i+2*x] - dp[2+i] <= cap;
      };
      li lo = 0, hi = ((bc ? n-3 : n-2)-i)/2;
      while(lo < hi) {
        li mi = (lo+hi+1)/2;
        if(can(mi)) lo = mi;
        else hi = mi-1;
