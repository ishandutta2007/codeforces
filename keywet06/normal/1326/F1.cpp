...
std::vector<int> a;
std::vector<long long> ans, ch;
std::vector<std::vector<long long>> cht;
...
void dfs(int r, int x, std::vector<long long> g) {
    ...
    for (int i = x; i <= r; ++i) {
        ...
        a.push_back(i);
        auto g0 = g;
        for (int j = 0; j < (1 << n); ++j) g0[j] *= cht[i][j];
        dfs(r - i, i, g0);
        a.pop_back();
    }
}
...
int main() {
    ...
    cht.assign(1 << n, std::vector<long long>(n));
    ch.resize(1 << n);
    for (int s = 1; s < (1 << n); ++s) {
        if ((s & -s) == s) {
            cht[s][__builtin_ctz(s)] = 1;
        } else {
            for (int v = 0; v < n; ++v)
                if (s >> v & 1) {
                    for (int u = 0; u < n; ++u) {
        ...
        for (int i = 0; i < n; ++i) ch[s] += cht[s][i];
    }
    cht.assign(n + 1, std::vector<long long>(1 << n));
    for (int i = 1; i <= n; ++i) {
        for (int s = 0; s < (1 << n); ++s) {
            if (__builtin_popcount(s) == i) cht[i][s] = ch[s];
        }
        f(cht[i]);
    }
