...
void solve() {
  ...
  vi add_col(N + 1);
  // 
  FenwickTree<Group_Add<ll>> bit(N + 1);

  auto add_f = [&](ll L, ll R, ll col) -> void {
    bit.add(L, -add_col[col]);
    bit.add(R, add_col[col]);
  };

  auto rm_f = [&](ll L, ll R, ll col) -> void {
    bit.add(L, add_col[col]);
    bit.add(R, -add_col[col]);
  };

  Intervals<int> I;
  I.set(0, N, 1, add_f, rm_f);
  ...
  FOR_(Q) {
    ...
    if (S == "Query") {
      LL(i);
      --i;
      auto [l, r, col] = I.get(i);
      ll ANS = add_col[col] + bit.sum(i + 1);
      print(ANS);
    }
    elif (S == "Add") {
      LL(c, x);
