...
struct unionfind{
  private:
  int maxN;
  vector<int> par,treesize,val;
  public:unionfind(int N) :maxN(N),par(N),treesize(N,1),val(N,0){
...
int main(){
  ...
  rep(i,N){
    ...
    if(j==1){
      cin>>A;
      --A;
      if(tree.rval(A))continue;
      tree.addsize(A);
      ans.eb(i+1);
    }else{
      cin>>A>>B;
      --A;--B;
      if((tree.rval(A)&&tree.rval(B))||!tree.unite(A,B))continue;
      // tree.addsize(A);
      ans.eb(i+1);
    }
  }
  print(modpow(2,ans.size())<<" "<<ans.size());
  output(ans);
}