...
int q; cin >> q; while (q--){
    ...
    ForE(i, 1, n){
        ...
        For(j, 0, 60){
            if (a[i] & (1ll << j)){
                tans1 += cnt[j] * ((1ll << j) % mod) % mod; if (tans1 >= mod) tans1 -= mod;
            }
        }
        ll tans2 = 0;
        For(j, 0, 60){
            if (a[i] & (1ll << j)){
                tans2 += n * ((1ll << j) % mod) % mod; if (tans2 >= mod) tans2 -= mod;
            }
            else{
                tans2 += cnt[j] * ((1ll << j) % mod) % mod; if (tans2 >= mod) tans2 -= mod;
            }
        }
        ans += tans1 * tans2 % mod; if (ans >= mod) ans -= mod;
    }
    cout << ans << endl;
}
}
