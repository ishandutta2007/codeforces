...
private:
	...
	Matrix Zero() { return Matrix<N, T>(zero, one); }
	Matrix Id() {
		Matrix<N, T> ans = Zero();
		for (int i = 0; i < N; i++)
			ans[i][i] = one;
...
public:
	vector<vector<T>> entry = vector<vector<T>>(N, vector<T>(N));
	vector<T>& operator [] (int idx) { return entry[idx]; }

	Matrix operator + (Matrix& other) {
		Matrix ans = Zero();
		for (int i = 0; i < N; i++)
			for (int j = 0; j < N; j++)
				ans[i][j] = entry[i][j] + other[i][j];
	...
	Matrix(const T zero, const T one) : zero(zero), one(one) {
		for (int i = 0; i < N; i++)
			for (int j = 0; j < N; j++)
				entry[i][j] = zero;
	}
...
int main() {
	...
	Matrix<3, Mint> m(Mint(0, MOD107 - 1), Mint(1, MOD107 - 1));
	m[0][1] = m[1][2] = m[2][0] = m[2][1] = m[2][2] = Mint(1, MOD107 - 1);
	m = m.pow(n - 1);
		
	Mint ans = Mint(1, MOD107);
	for (int i = 0; i < 3; i++)
		ans *= f[i].pow(m[0][i]);
		
	ans *= Mint(c, MOD107).pow(n).inv();
	W(ans);
