...
bool check(vector<pii> vec){
    set<int> st;
    for(auto& [x,y]: vec) st.insert(x),st.insert(y);
    return vec.empty()||sz(st)>sz(vec);
}
...
void orzck(){
    ...
    while(1){
        ...
        vector<pii> vec;
        vector<int> bad;
        for(auto& arr: edges) if(!d.Union(arr[0],arr[1])) vec.pb({arr[0],arr[1]}),bad.pb(arr[2]);
        if(check(vec)){
            for(auto i: bad) res[i]=1;
            rep(m) cout << res[i];
            cout << "\n";
            return;
        }
