...
_ n, x, y, mx[lg], t[lg], cnt, r[maxn], lt[lg], rt[lg], tp, o[maxn], a[maxn];
queue<node> q;
vector<_> g[maxn];

void dfs(_ x, _ fa);
inline void merge(_ &x, _ &y, _ l, _ r);
inline void update(_ &x, _ c, _ l, _ r);
inline void pushUp(_ x);
inline void AC() {
...
inline void pushUp(_ x) {
    mx[x] = max(mx[lt[x]], mx[rt[x]]);
    if(mx[lt[x]] < mx[rt[x]]) t[x] = t[rt[x]];
    ef(mx[lt[x]] > mx[rt[x]]) t[x] = t[lt[x]];
    else t[x] = t[lt[x]]+t[rt[x]];
    return;
}
...
inline void update(_ &x, _ c, _ l, _ r) {
    ...
    _ md = (l+r)>>1;
    if(c <= md) update(lt[x], c, l, md);
    else update(rt[x], c, md+1, r);
    pushUp(x);
    return;
}
