...
void solve() {
  LL(N, D, H);
  if (N == 2) return print(1, 2);
  if (D == 1) return print(-1);
  if (2 * H < D) return print(-1);
  vi P(D + 1);
  ll nxt = 0;
  FOR_R(i, H + 1) P[i] = nxt++;
  FOR3(i, H + 1, len(P)) P[i] = nxt++;

  vc<pi> ANS;
  FOR(i, D) ANS.eb(P[i], P[i + 1]);
  while (nxt < N) ANS.eb(P[1], nxt++);
  for (auto&& [a, b]: ANS) print(a + 1, b + 1);
}
