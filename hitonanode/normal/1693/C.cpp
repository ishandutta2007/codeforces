...
int main() {
    int N, M;
    cin >> N >> M;
    // shortest_path<int> revgraph(N);
    vector<multiset<int>> child(N);
    ...
    while (pq.size()) {
        ...
        pq.pop();
        if (dnow > dist[now]) continue;
        if (visited[now]) continue;
        visited[now] = 1;

        for (auto par : rev[now]) {
            child[par].erase(child[par].lower_bound(now));
            chmax(farthest[par], dnow);
            int dnxt = farthest[par] + child[par].size() + 1;
            if (chmin(dist[par], dnxt)) pq.emplace(dnxt, par);
        }
    }
    dbg(dist);
