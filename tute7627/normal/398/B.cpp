...
int main(){
  ...
  vector<real>p(n+1);
  rep(i,0,n)p[i]=real(n-i)/n;
  vector<real>pi(n);
  rep(i,0,n)pi[i]=real(1)/p[i];
  auto dp=vec(n+1,n+1,real(0));
  dp[n][n]=0;
  rrep(i,0,n+1){
    rrep(j,0,n+1){
      if(i==n&&j==n)continue;
      if(i==n)dp[i][j]=dp[i][j+1]+pi[j];
      else if(j==n)dp[i][j]=dp[i+1][j]+pi[i];
      else{
        real np=1-(1-p[i])*(1-p[j]);
        dp[i][j]=1/np;
        dp[i][j]+=dp[i+1][j]*p[i]*(1-p[j])/np;
        dp[i][j]+=dp[i][j+1]*p[j]*(1-p[i])/np;
        dp[i][j]+=dp[i+1][j+1]*p[i]*p[j]/np;
      }
    }
  }
  //debug(dp,n+1,n+1);
  lfs<<dp[sx.size()][sy.size()]<<endl;
  return 0;
}