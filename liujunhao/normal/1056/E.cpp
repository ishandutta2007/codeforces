...
typedef pair<int,uint>puu;
puu hs[MAXN+10],hash_pow[MAXN+10];
int lens,lent,cnt[2],ans;
puu st[2];
inline puu Get_val(int l,int r){
	return {((hs[r].first-1ll*hs[l-1].first*hash_pow[r-l+1].first)%MOD+MOD)%MOD,hs[r].second-hs[l-1].second*hash_pow[r-l+1].second};
}
int main()
{
	scanf("%s%s",t+1,s+1);
	...
	*hash_pow={1,1};
	for(i=1;i<=lens;i++){
		hs[i]={(1ll*hs[i-1].first*p1+s[i]-'a'+1)%MOD,hs[i-1].second*p2+s[i]-'a'+1};
		hash_pow[i]={1ll*hash_pow[i-1].first*p1%MOD,hash_pow[i-1].second*p2};
	}
	for(i=1;i<=lent;i++)
		cnt[t[i]-'0']++;
	for(i=1;i*cnt[0]<lens;i++){
		int j=(lens-i*cnt[0])/cnt[1];
		if(i*cnt[0]+j*cnt[1]!=lens)
			continue;
		bool flag[2]={0,0};
		int pos=1;
		puu now;
		for(k=1;k<=lent;k++){
			now=Get_val(pos,pos+((t[k]-'0'==0)?i:j)-1);
			pos+=((t[k]-'0'==0)?i:j);
			if(!flag[t[k]-'0'])
				st[t[k]-'0']=now;
			else{
