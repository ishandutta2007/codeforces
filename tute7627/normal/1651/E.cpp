...
struct Namori{
  ...
  vector<vector<edge<T>>>cycle_edge;//
  vector<int>vn,en;//
  Graph<T>forest;
  vector<int>root;
...
int main(){
  ...
  vector<ll>to(2*n),from(2*n);
  for(auto cc:namori.cycle){
    rep(i,0,cc.size()){
      to[cc[i]]=cc[(i+1)%cc.size()];
      from[to[cc[i]]]=cc[i];
    }
  }
  rep(i,0,n){
    ...
    while(1){
      ...
      ll mul=(mi+1)*(n-mx);
      if(mx2==-100){
        res-=mul*(get(l-n)+get(2*n-r-1)+get(r-l-1));
      }
      else{
        if(r<mi2)res-=mul*(mi2-r)*(2*n-mx2);
        if(mx2<l)res-=mul*(l-mx2)*(mi2-n+1);
        if(l<mi2&&mx2<r)res-=mul*(mi2-l)*(r-mx2);
      }
      now=to[now];
      chmin(mi2,now);
      chmax(mx2,now);
