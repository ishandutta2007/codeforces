...
void solve() {
    ...
    auto ord = TS.build();
    vector<vector<tuple<ll, mint, bool>>> dat(n);  //  S 
    for (int i = 0; i < n; i++) {
        int v = ord[i];
        if (a[v] > 0) dat[v].emplace_back(0, a[v], (a[v] >= MAX));
        vector<tuple<ll, mint, bool>> cand;
        for (int& u : rG[v]) {
            for (auto& t : dat[u]) {
                ll aa;
                mint b;
                bool c;
                tie(aa, b, c) = t;
                cand.emplace_back(aa + 1, b + 1, (c or (b + 1).val() >= MAX));
            }
        }
        sort(ALL(cand), [](tuple<ll, mint, bool> p, tuple<ll, mint, bool> q) { return get<0>(p) < get<0>(q); });
        for (auto& t : cand) {
            ll aa, A;
            mint b, B;
            bool c, C;
            tie(aa, b, c) = t;
            if (!dat[v].empty()) tie(A, B, C) = dat[v].back();
            if (dat[v].empty() or (!C and B.val() < aa))
                dat[v].emplace_back(t);
            else {
                auto& q = dat[v].back();
                get<1>(q) = A + (B - A) + (b - aa);
                get<2>(q) = (C or c or get<1>(q).val() >= MAX);
            }
        }
    }

    mint ans = (dat[ord[n - 1]].empty() ? 0 : get<1>(dat[ord[n - 1]].back()));
    cout << ans << '\n';
}
