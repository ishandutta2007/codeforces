...
inline void solve(ld tt) {
    ...
    for(int i = 0; i < sz(nv); ++i) {
        if(nv[i].y >= 4) {
            cout << nv[i].x << " " << nv[i].x << " " << nv[i].x << " " << nv[i].x << endl;
            return;
        }
        if(i >= 1 && bst > (nv[i].x + nv[i - 1].x) * (nv[i].x + nv[i - 1].x) / (ld)nv[i].x / (ld)nv[i - 1].x) {
            ans = {nv[i].x, nv[i - 1].x};
            bst = (nv[i].x + nv[i - 1].x) * (nv[i].x + nv[i - 1].x) / (ld)nv[i].x / (ld)nv[i - 1].x;
        }
    }
    cout << ans << " " << ans << endl;
}
