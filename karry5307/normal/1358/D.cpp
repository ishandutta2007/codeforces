...
ll n,m,res,lst,inte,l,r;
ll x[MAXN],sum[MAXN],prefix[MAXN];
// Fast IO
inline ll read()
{
...
inline ll getP(ll p)
{
	...
	ll pos=upper_bound(prefix+1,prefix+n+1,p)-prefix-1,res=sum[pos];
	p-=prefix[pos];
	return res+(p+1)*p/2;
}
int main()
{
	...
	for(register int i=1;i<=n;i++)
	{
		...
		sum[i]=x[i]*(x[i]+1)/2,sum[i]+=sum[i-1];
	}
	for(register int i=1;i<=n;i++)
	{
		r=prefix[i],l=prefix[i]-m+1;
		if(l>=1)
		{
			res=max(res,getP(r)-getP(l-1));
		}
		else
		{
			l=l+prefix[n];
			res=max(res,sum[n]-getP(l-1)+getP(r));
		}
	}
	printf("%lld\n",res);
}