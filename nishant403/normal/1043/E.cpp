...
//source : https://codeforces.com/blog/entry/45835
...
void solve()
{
  ...
  while(m--)
  {
      ...
      res[u] -= min(a[u].F.F+a[v].F.S,a[u].F.S+a[v].F.F);
      res[v] -= min(a[u].F.F+a[v].F.S,a[u].F.S+a[v].F.F);
  }
  ...
  f(i,n)
  {
      ...
      res[a[i].S]+=tx+((i+1)*a[i].F.S);
      res[a[i].S]+=sumy-ty + ((n-i-1)*a[i].F.F);
      res[a[i].S]-=a[i].F.F+a[i].F.S;
  }
    
   write(res.begin(),res.end()); 
}
