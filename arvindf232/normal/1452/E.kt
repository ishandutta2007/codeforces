...
class item ( val l:Int, val r:Int):Comparable<item>{
    val sum = l + r
...
fun main(){
    solve.cases{
        ...
        val all = MutableList(m){
            item(getint - 1 , getint - 1 )
        }
        var ret = 0L
        all.sortBy { it.sum }
        val allsum = tempIntervalSum(n)
        for(a in all){
            allsum.add(a.l, a.r, 1)
        }
        val allq = allsum.compile().torsq()
        for(left in 0 until n){
            if(left + k - 1 < n){
                ret = maxOf(ret, allq.sumQuery(left, left + k- 1 ))
            }
        }
        val found = mutableSetOf<Pair<Int,Int>>()
        for(cutsum in 0 until 2 * n){
            val left = tempIntervalSum(n)
            val right = tempIntervalSum(n)
            for(a in all){
                if(a.sum <= cutsum){
                    left.add(a.l,a.r,1)
                }else{
            ...
            val LQ = left.compile().torsq()
            val RQ = right.compile().torsq()


            for(leftcut in 0 until n){
                val rightcut = cutsum - leftcut
                if(rightcut !in 0 until n || rightcut <= leftcut) continue

                val interior = rightcut - leftcut - 1
                if(interior < k) {
                    val l2 = rightcut - 1
                    val l1 = rightcut - k
                    val r1 = leftcut + 1
                    val r2 = leftcut + k
                    if(l1 !in 0 until n || l2 !in 0 until n || r1 !in 0 until n || r2 !in 0 until n) {
                    }else{
                        val t1 = LQ.sumQuery(l1, l2)
                        val t2=  RQ.sumQuery(r1,r2)
                        ret = maxOf(ret,t1 + t2 )
//                        found.add(Pair(l1,r1))
                    }
                }
...
//        for((a,b) in found.toList().sortedBy {it.first}){
//            println("$a $b")
//        }
        put(ret)
...
/*
 36 minutes
 thought no fundamentals, but pull through, there is fundamentals
