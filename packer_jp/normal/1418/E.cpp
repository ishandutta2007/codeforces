...
signed main() {
    ...
    rep(i, n) { ev.emplace_back(d[i], 1, i); }
    rep(i, m) { ev.emplace_back(b[i], 0, i); }
    sort(all(ev));
    vector<mint> ans(m);
    int big = n;
    mint X = 0, Y = 0;
    ...
    for (auto[val, type, idx]:ev) {
        if (type == 1) {
            ...
            //shield
            if (big == 0) { ans[idx] = 0; }
            else { ans[idx] = X * max(0LL, big - a[idx]) / big + Y * max(0LL, big + 1 - a[idx]) / (big + 1); }
        }
    }
    for (mint x:ans) { cout << x << endl; }
}