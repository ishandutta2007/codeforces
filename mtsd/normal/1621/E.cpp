...
int main(){
    ...
    rep(i,T){
        ...
        rep(i,m){
            ...
            C[i] = ceil(S,k);
            rep(j,k){
                c[i][j] = ceil(S-b[i][j],k-1);
            }
        }
        vector<ll> X;
        rep(i,n)X.push_back(a[i]);
        rep(i,m)X.push_back(C[i]);
        rep(i,m)rep(j,c[i].size())X.push_back(c[i][j]);
        UNIQUE(X);
        ...
        int M = X.size();
        vector<int> pppp(X.size());
        segtree<int>seg(pppp);
        rep(i,n){
            seg.range(0,mp[a[i]]+1,1);
        }
        rep(i,m){
            seg.range(0,mp[C[i]]+1,-1);
        }
        rep(i,m){
            seg.range(0,mp[C[i]]+1,+1);
            rep(j,c[i].size()){
                seg.range(0,mp[c[i][j]]+1,-1);
                if(seg.query(0,M)<0){
                    cout << 0;
                }else{
                    cout << 1;
