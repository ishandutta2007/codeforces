...
void up(){
	...
	std::vector<std::string> scenes(number);
	...
	for(auto& s: scenes){
		...
		if(not okay){
			before.insert(s);
			if(before.count(std::string(s.rbegin(), s.rend())) or
					(s.size()==3 and before.count(std::string(s.rbegin(), s.rend()-1)))
					){
				std::cout<<"YES\n";
				okay=true; // (be careful with folding loops.)
			}
		}
	}
	if(okay) return;
	...
	for(int i=number; i--;){
		auto const& s=scenes[i];
		if(s.size()==2) after.insert(s);
		if(s.size()==3 and after.count(std::string(s.rbegin()+1, s.rend()))){
			std::cout<<"YES\n";
			return;
		}
	}
