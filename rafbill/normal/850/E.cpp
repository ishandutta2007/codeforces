...
int main(){
  ...
  int n; cin>>n;
  string s; cin>>s;
  FOR(i,1<<n) data[i]=(s[i]=='1'?1:0);

  // lli ans2=0;
  // FOR(a,1<<n) FOR(b,1<<n) if(data[a]&&data[b]) data2[a^b] += 1; // ans += (1<<(n-__builtin_popcount(a^b)));
  // cout << 3*ans2 << endl;

  fwht(data,1<<n);
  FOR(i,1<<n) data[i]=(data[i]*data[i])%MOD;
  fwht(data,1<<n);
  lli im=invmod(1<<n);
  FOR(i,1<<n) (data[i] *= im) %= MOD;
  lli ans=0;
  FOR(i,1<<n) (ans += data[i] * ((1<<(n-__builtin_popcount(i)))%MOD)) %= MOD;
  cout << (3*ans)%MOD << endl;
  return 0;
