...
void add(Node *root, int k, int number) {
...
int find_max(Node *root, int k, int number) {
    if (root == nullptr) {
        return -INF;
    }
...
int find_min(Node *root, int k, int number) {
    if (root == nullptr) {
        return INF;
    }
...
vector <int> solve(Node *root, int k) {
    if (root == nullptr || k == 0) {
        return vector <int>(1 << k, INF);
    }

    auto s0 = solve(root->ch[0], k - 1);
    auto s1 = solve(root->ch[1], k - 1);

    vector <int> res(1 << k, INF);
    for (int i = 0; i < (1 << k); ++i) {
        res[i] = min(res[i], s0[i & ~(1 << (k - 1))]);
        res[i] = min(res[i], s1[i & ~(1 << (k - 1))]);

        if (i & (1 << (k - 1))) {
            res[i] = min(res[i], find_min(root->ch[0], k - 1, i) + (1 << (k - 1)) - find_max(root->ch[1], k - 1, i));
        } else {
            res[i] = min(res[i], find_min(root->ch[1], k - 1, i) + (1 << (k - 1)) - find_max(root->ch[0], k - 1, i));
        }
    }

//    cerr << "c " << root->cnt << endl;
//    for (auto p : res) {
//        cerr << p << " ";
//    }
//    cerr << endl;
...
void solve(int) {
    ...
    vector <int> answer = solve(root, k);
    for (int x : answer) {
        cout << x << " ";
    }
    cout << "\n";
