...
int main() {
  ...
  sort(ALL(d));
  vector cum(n + 1, ModInt(0));
  REP(i, n) cum[i + 1] = cum[i] + d[i];
  while (m--) {
    int a, b; cin >> a >> b;
    int l = lower_bound(ALL(d), b) - d.begin(), gte = n - l;
    if (gte < a) {
      cout << 0 << '\n';
      continue;
    }
    ModInt ans = 0;
    if (gte > 0) ans += (cum[n] - cum[l]) * a * com.fact[gte - 1] * com.fact[gte + n - gte] / com.fact[gte];
    ans += (cum[l] - cum[0]) * com.fact[gte] * a * com.fact[gte + 1 + n - gte - 1] / com.fact[gte + 1];
    ans /= com.fact[n];
    cout << cum[n] - ans << '\n';
  }
  return 0;
