...
signed main(){
    ...
    int ans=0;
    {
        auto dfs=[&](auto &f,int x,int p)->void{
            ...
            for(auto e:g[x]){
                f(f,e,x);
                chmax(mem[x][1],mem[e][0]);
                chmax(ma[1][1],mem[e][0]);
                if(ma[1][1]>ma[1][0])swap(ma[1][0],ma[1][1]);
                chmax(mem[x][0],max(mem[e][0]-1,mem[e][1]-1));
                chmax(ma[0][1],max(mem[e][0]-1,mem[e][1]-1));
                if(ma[0][1]>ma[0][0])swap(ma[0][1],ma[0][0]);
            }
            mem[x][1]++;
            mem[x][0]+=g[x].size();
            chmax(ans,ma[1][0]+ma[1][1]+1);
            chmax(ans,ma[0][0]+ma[0][1]+(int)g[x].size()+(x!=0));
            return;
        };
        dfs(dfs,0,-1);
    }
