...
template <typename T> WeightedTree<T> make2(int n, int offset = 1) {
...
template <typename T> struct LiChaoTree {
    struct Line {
        ...
        inline bool over(const Line &b, const T &x) const { return get(x) < b.get(x); }
    };
...
main() {
    ...
    vll A = a;
    UNIQUE(A);
    for(auto &e : a) e = lb(A, e);
    auto g = make(n);
    LiChaoTree<ll> cht(A, (ll)1e17);
    vi sub(n);
    {
        auto dfs = [&](auto &&f, int x, int p) -> void {
            ...
            for(auto e : g[x])
                if(e != p) f(f, e, x);
            if(!g[x].empty() and g[x][0] == p) swap(g[x][0], g[x][g[x].size() - 1]);
            for(auto &e : g[x])
                if(e != p) {
                    sub[x] += sub[e];
                    if(sub[e] > sub[g[x].front()]) swap(e, g[x][0]);
                }
            reverse(all(g[x]));
        };
        dfs(dfs, 0, -1);
    ...
    vll ans(n, 1e17);
    auto dfs = [&](auto &&f, int x, int p, bool flag = true) -> void {
        if(flag)
            for(auto e : g[x]) {
                if(e == p) continue;
                f(f, e, x);
                if(e != g[x].back()) rep(_, sub[e]) cht.roll_back();
            }
        for(auto e : g[x]) {
            if(e == p) continue;
            if(!flag or e != g[x].back()) f(f, e, x, false);
        }
        if(flag) {
            if(g[x].size() == 1 and x) {
                ans[x] = 0;
