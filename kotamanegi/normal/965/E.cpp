...
int cnt[300000];
int endings[300000];
vector<int> dfs(int now, int depth) {
    vector<int> ans;
    ans.push_back(0);
    REP(q, endings[now]) {
        ans.push_back(1e9);
    ...
    reverse(ALL(ans));
    vector<vector<int>> nexter;
    REP(i, 26) {
        if (trie[i][now] == -1) continue;
        nexter.push_back(dfs(trie[i][now], depth + 1));
    }
    REP(i, nexter.size()) {
        vector<int> B(min(depth + 1, (int)nexter[i].size() + (int)ans.size() - 1), 1e9);
        REP(q, nexter[i].size()) {
            REP(j, ans.size()) {
                if (q + j >= depth+1) break;
                B[q + j] = min(B[q + j], nexter[i][q] + ans[j]);
            }
        }
        ans = B;
    }
    for (int q = 1; q < ans.size(); ++q) {
        ans[q - 1] = min(ans[q - 1], ans[q] + depth);
    }
    while (ans.size() > min(depth,cnt[now]) + 1) {
        ans.pop_back();
    }
    return ans;
}
void solve(){
    ...
    int ans = 0;
    REP(i, 26) {
        if (trie[i][0] == -1) continue;
        ans += dfs(trie[i][0], 1)[0];
    }
    cout << ans << endl;
}
#undef int
