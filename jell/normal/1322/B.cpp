...
struct solver
{
    ...
    solver()
    {
        ...
        for(int i=0,mask=0;i<=h;++i)
        {
            ...
            {
                {
                    auto l=lower_bound(begin(a),itr,pw-*itr);
                    auto r=lower_bound(begin(a),itr,pw*2-*itr);
                    if((r-l)&1) prty=!prty;
                    // dump(l,r);
                    dump(l-begin(a),r-begin(a));

                }
                {
                    auto l=lower_bound(begin(a),itr,pw*3-*itr);
                    auto r=lower_bound(begin(a),itr,pw*4-*itr);
                    if((r-l)&1) prty=!prty;
                    // dump(l,r);
                    dump(l-begin(a),r-begin(a));
                }
