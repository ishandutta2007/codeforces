// Problem with Random Tests (https://codeforces.com/contest/1743/problem/D)
...
int main() {
    ...
    string t;
    for (int i = n - 1, j = idx[0]; i >= 0; --i, --j)
        t.push_back(s[i] == '1' || (j >= 0 && s[j] == '1') ? '1' : '0');
    reverse(all(t));
    println(t);
}