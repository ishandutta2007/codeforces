...
struct Add{
  ...
  Add(T ad={}):ad(ad){}
	...
	bool operator==(const Add &lfun){ return ad.mx==lfun.ad.mx;}

	template<class S>
	void eval(MinMax<S> &s,int l,int r){
		s=s+ad;
	}
	static Add e;
};
...
int main(){
	...
	LazySegTree<MinMax<pii>,Add<MinMax<pii>>> lt(vector<pii>(zip.size(),{-1,-1}));
	vector<pii> dp(n,{-1,-1});
	rep(i,n){
		for(auto &[l,r]:ints[i]){
			l=lower_bound(all(zip),l)-zip.begin();
			r=lower_bound(all(zip),r)-zip.begin();
		}
		for(cauto &[l,r]:ints[i]) MX(dp[i],lt(l,r).mx);
		dp[i].X++;
		for(cauto &[l,r]:ints[i]) lt(l,r,MinMax<pii>({dp[i].X,i}));
	}
	out(dp,1);
	cauto mxi=max_element(all(dp))-dp.begin();
	set<int> re; rep(i,n) re.emplace(i);
	for(int i=mxi;i>=0;i=dp[i].Y) re.erase(i);
	cout<<re.size() NL;
