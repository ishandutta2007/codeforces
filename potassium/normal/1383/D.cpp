...
int main() {
	int n,m; R(n,m);
	vector<vector<int>> a(n, vector<int>(m, -1)), ans = a; R(a);
	...
	for (int i = n * m; i >= 1; i--) {
		...
		int rp = (isR ? rs[i] : lastR);
		int cp = (isC ? cs[i] : lastC);
		...
		if (isR) re[lastR] = lastC - 1;
		if (isC) ce[lastC] = lastR - 1;
	}
