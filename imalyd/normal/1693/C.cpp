...
const int MN = 200005;
vector<int>E[MN];
int D[MN],dis[MN],V[MN];
struct inff{
	int dis,v;
	inline bool operator<(const inff &i)const{
		return i.dis<dis;
	}
};
priority_queue<inff>Q;
int main()
{
	int n,m;in(n);in(m);
	while(m--){
	...
	while(!Q.empty()){
		...
		for(vector<int>::iterator i=E[v].begin(),ii=E[v].end();i!=ii;++i){
			int u=*i, w=dis[v]+D[u];
			if(w<dis[u])
			Q.push((inff){dis[u]=w,u});
			--D[u];
		}
	}
	ouu(dis[1]);
