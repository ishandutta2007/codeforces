...
void upd(int &o, mint x, int p, int l, int r, int id)
{
    int oo = ++tcnt;
    T[oo] = T[o];
    o = oo;
    ...
    int mid = MID(l, r);
    if(p <= mid) upd(T[o].lch, x, p, lson);
    else upd(T[o].rch, x, p, rson);
    T[o].x = T[T[o].lch].x * T[T[o].rch].x;
}

mint que(int o, int L, int R, int l, int r, int id)
{
    if(L <= l && r <= R) return T[o].x;
    int mid = MID(l, r);
    if(R <= mid) return que(T[o].lch, L, R, lson);
    if(mid < L) return que(T[o].rch, L, R, rson);
    return que(T[o].lch, L, R, lson) * que(T[o].rch, L, R, rson);
}

int n;
int a[maxn];

int primes[maxm], low_prime[maxm];
mint po[20][maxm], inv[20][maxm];
...
void work()
{
...

    {
        ...
        for(int i = 2; i <= mx; i++)
        {
            if(low_prime[i] == 0)
            {
                ...
                po[0][i] = 1;
                for(int j = 1; j < 20; j++) po[j][i] = po[j - 1][i] * i;
                inv[19][i] = po[19][i].inv();
                for(int j = 18; ~j; j--) inv[j][i] = inv[j + 1][i] * i;
            }

            for(int j = 0; j < m; j++)
            {
                if(i * primes[j] > mx) break;
                low_prime[i * primes[j]] = primes[j];
    ...
    for(int i = 1; i <= n; i++)
    {
        ...
        while(x != 1)
        {
            ...
            while(!stk[p].empty() && stk[p].top().a <= cnt)
            {
                int c = stk[p].top().a, j = stk[p].top().b, r = stk[p].top().c;
                stk[p].pop();
//                upd(rt[i], mint(p).pow(r).inv(), j, 1, n, 1);
                upd(rt[i], inv[r][p], j, 1, n, 1);
            }
...
////                upd(rt[i], mint(p).pow(r).inv(), j, 1, n, 1);
//                upd(rt[i], inv[r][p], j, 1, n, 1);
////                upd(rt[i], mint(p).pow(c - cnt), j, 1, n, 1);
//                upd(rt[i], po[c - cnt][p], j, 1, n, 1);
                int t = r - (c - cnt);
                if(t > 0) upd(rt[i], inv[t][p], j, 1, n, 1);

                stk[p].emplace(item{c, j, c - cnt});
            }

//            upd(rt[i], mint(p).pow(cnt), i, 1, n, 1);
            upd(rt[i], po[cnt][p], i, 1, n, 1);
            stk[p].emplace(item{cnt, i, cnt});
        }
    }
