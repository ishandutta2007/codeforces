...
int main(){
  ...
  dfs_tree(0,g,-1,dfst);
  vector<ll>ord(n);
  rep(i,0,n)ord[dfst[i]]=i;
  vector<ll>dp(n);
  for(auto &z:idxs){
    sort(ALL(z),[&](ll x,ll y){return ord[x]<ord[y];});
    //debug(z,z.size());
    dp[z[0]]++;
    rep(i,1,z.size()){
      dp[z[i]]++;
      dp[lca.query(z[i-1],z[i])]--;
    }
  }
  {
