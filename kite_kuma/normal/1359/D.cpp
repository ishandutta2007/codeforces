...
struct BIT {
	...
	// ll LowerBound(long long start,T val){
	// 	val += sum0(start);
...
int main() {
	...
	vvll dp(n + 1, vll(61, -dekai));
	// auto dp1 = dp0;
	ll res = 0;
	rep(i, n) {
		ll k = a[i] + 30;
		if(chmax(dp[i + 1][k], 0LL)) {
			// cout << i << " " << k << endl;
		}
		rep(j, 61) {
			if(k > j) {
				if(chmax(dp[i + 1][k], dp[i][j] + j - 30)) {
					// cout << i << " " << j << " " << k << ednl;
				}
			} else {
				if(chmax(dp[i + 1][j], dp[i][j] + a[i])) {
					// cout << i << " " << j << " " << k << enld;
				}
			}
		}
