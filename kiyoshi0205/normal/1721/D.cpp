  ...
  brep(j,30){
    ...
    for(auto& x:g){
      ...
      for(auto& x:a){
        if(x>>j&1){
          x^=1<<j;
          nxta[1].emplace_back(x);
        }else nxta[0].emplace_back(x);
      }
      for(auto& x:b){
        if(x>>j&1){
          x^=1<<j;
          nxtb[1].emplace_back(x);
        }else nxtb[0].emplace_back(x);
      }
      f&=nxta[0].size()==nxtb[1].size();
      if(f){
        if(nxta[0].size())nxtg.emplace_back(move(nxta[0]),move(nxtb[1]));
        if(nxta[1].size())nxtg.emplace_back(move(nxta[1]),move(nxtb[0]));
      }
    }
    if(f){
      swap(g,nxtg);
