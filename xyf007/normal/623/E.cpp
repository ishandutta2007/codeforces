...
long long n, fac[30001], inv[30001];
int k;
std::vector<int> F0;
...
std::vector<int> Solve(int n) {
  if (n == 1) return F0;
  auto f0 = Solve(n >> 1), f1 = f0;
  long long t = Ksm(2LL, n >> 1), w = 1LL;
  for (int i = 0; i <= k; i++) f1[i] = f1[i] * w % kMod, w = w * t % kMod;
  auto f = atcoder::convolution_mtt(f0, f1, kMod);
  f.resize(k + 1);
  if (n & 1) {
    long long w = 1LL;
    for (int i = 0; i <= k; i++) f[i] = f[i] * w % kMod, w = w * 2 % kMod;
    f = atcoder::convolution_mtt(f, F0, kMod);
    f.resize(k + 1);
  }
  return f;
}
int main(int argc, char const *argv[]) {
  ...
  F0.resize(k + 1);
  std::copy(inv + 1, inv + k + 1, F0.begin() + 1);
  auto c = Solve(n);
  for (int i = 0; i <= k; i++) c[i] = c[i] * fac[i] % kMod;
  long long ans = 0LL;
  for (int i = n; i <= k; i++) ans += C(k, i) * c[i] % kMod;
  std::cout << ans % kMod;
  return 0;
}