...
string input;
vector<string> fnduan,fd1,fd2;

bool isnumber(string s)
{
	for(int i=0;i<SZ(s);i++)if(!isdigit(s[i]))return false;
...
int main()
{
	...
	fnduan.push_back(s);
	for(int i=0;i<SZ(fnduan);i++)
		if(isnumber(fnduan[i]))
			fd1.push_back(fnduan[i]);
		else fd2.push_back(fnduan[i]);
	if(SZ(fd1)==0)printf("-");
	else
	{
		printf("\"");
		for(int i=0;i<SZ(fd1);i++)
		{
			cout<<fd1[i];
			printf("%c",i==SZ(fd1)-1?'\"':',');
		}
