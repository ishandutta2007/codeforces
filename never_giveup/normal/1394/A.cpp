...
const int maxn = 2e5 + 100, maxw = 1e4 + 100, inf = 1e9 + 100, sq = 100, LG = 18, mod = 1e9 + 933;

int a[maxn], n, d, m, pos;

ll ans[maxn], pref[maxn], suf[maxn];

ll get(int id) {
    if (ans[id] == -1) {
        int rem = min(pos, n - (d + 1) * (id - 1) - 1);
        ans[id] = suf[id] + pref[rem];
    }
    return ans[id];
}
