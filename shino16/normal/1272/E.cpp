...
int main() {
  ...
  VVI g_rev(n);
  rep(i, n) {
    if (i-a[i]>=0) g_rev[i-a[i]].push_back(i);
    if (i+a[i]<n) g_rev[i+a[i]].push_back(i);
  }

  VVI d(2, VI(n, INF));
  rep(par, 2) {
    queue<int> que;
    rep(i, n) if (a[i]%2 == par) d[par][i] = 0, que.push(i);
    while (not que.empty()) {
      int i = que.front(); que.pop();
      for (int next : g_rev[i])
        if (chmin(d[par][next], d[par][i]+1))
          que.push(next);
    }
  }
  VI res(n);
  rep(i, n) res[i] = d[~a[i]&1][i] >= INF ? -1 : d[~a[i]&1][i];
  OUT.range(res);
}