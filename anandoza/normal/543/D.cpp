...
int main() {
    ...
    y_combinator([&](auto self, int i) -> void {
        ...
        vmn prefix{1};
        for (int c : C)
            prefix.push_back(prefix.back() * (1 + dp[c]));
        vmn suffix{1};
        for (int c : reversed(C))
            suffix.push_back(suffix.back() * (1 + dp[c]));

        for (int j = 0; j < sz(C); j++) {
            int c = children[i][j];

            auto old_i = dp[i], old_c = dp[c];

            dp[i] = prefix[j] * suffix[sz(C) - 1 - j];
            if (par[i] != -1)
                dp[i] *= (1 + dp[par[i]]);

            dp[c] *= (1 + dp[i]);

            // we traversed edge down from i -> c

            self(c);

            dp[i] = old_i, dp[c] = old_c; // traverse back up from c -> i (restore values)
        }
    })(0);
