...
void add(int id,int s,int t,int x,int y)
{
	checkmax(tree[id],y);
	if (s==t)
		S[x].insert(y);
	...
	{
		int m=(s+t)/2;
		if (x<=m) add(id*2+1,s,m,x,y);
		else add(id*2+2,m+1,t,x,y);
	}
}
void remove(int id,int s,int t,int x,int y)
{
	if (s==t)
	{
		S[x].erase(y);
		tree[id]=(S[x].empty())?-1:*(--S[x].end());
	}
	else
	{
		int m=(s+t)/2;
		if (x<=m) remove(id*2+1,s,m,x,y);
		else remove(id*2+2,m+1,t,x,y);
		tree[id]=max(tree[id*2+1],tree[id*2+2]);
	}
}
ipair find(int id,int s,int t,int x,int y)
{
	if (t<x || tree[id]<y) return MP(-1,-1);
	if (s==t)
		return MP(s,*S[s].lower_bound(y));
	int m=(s+t)/2;
	ipair tmp=find(id*2+1,s,m,x,y);
	if (tmp.first>=0) return tmp;
	return find(id*2+2,m+1,t,x,y);
}
int main()
{
#ifdef _MSC_VER
	...
	for (int i=0;i<cx;i++) M[VX[i]]=i;
	for (int i=0;i<n;i++) X[i]=M[X[i]];
	for (int i=0;i<maxsize;i++) S[i].clear();
	for (int i=0;i<n;i++)
		if (C[i]=='a')
			add(0,0,n-1,X[i],Y[i]);
		else if (C[i]=='r')
			remove(0,0,n-1,X[i],Y[i]);
		else
		{
			ipair r=find(0,0,n-1,X[i]+1,Y[i]+1);
			if (r.first<0)
				printf("-1\n");
			else
				printf("%d %d\n",VX[r.first],r.second);
