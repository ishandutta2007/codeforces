...
int main() {
  ...
  vector <vector <vector <Mint>>> dp(n + 1, vector <vector <Mint>> (n + 2, vector <Mint> (n + 2)));
  Mint ans = 0;
      ...
      for (int lst = mn; lst <= n + 1; ++lst) {
        for (int extra = 0; extra <= n + 1; ++extra) {
          ...
          for (int nw = lst + 1; nw <= n + 1; ++nw) {
            for (int go = 1; go + cnt <= n && mn * nw >= mn * cnt + extra + nw * go && extra + (n - cnt) * (nw - mn) <= n + 1; ++go) {
              dp[cnt + go][nw][extra + (nw - mn) * go] += dp[cnt][lst][extra] * rfac[go];
            }
          }
        }
      }
