...
int main() {
  ...
  vector<uint64_t> ds;
  for (int i = 0; i < 5; ++i) {
    ds.push_back(s[i + 1] - s[i]);
  }
  sort(ds.begin(), ds.end());
  auto tds = ds[2];
  int wrong = -1;
  for (int i = 1; i < k; ++i) if (s[i] - s[i - 1] != tds) {
    wrong = i;
    break;
  }
  auto tddss = wrong >= 3? ss[2] - 2 * ss[1] + ss[0]: ss[5] - 2 * ss[4] + ss[3];
  int64_t ess = (ss[wrong + 1] + ss[wrong - 1] - tddss) / 2 - ss[wrong];
  int64_t es = (s[wrong + 1] + s[wrong - 1]) / 2 - s[wrong];
  print(wrong, (es + ess / es) / 2);
}