...
int curs = 1;
int val[200001];
void go(int cur, bool e)
{
    used[cur] = true;
    int begp, begs, endp, ends;
    begp = curp;
    begs = curs;
    if (e)
    {
        typ[cur] = 'p';
        newvalp[curp++] = val[cur];
    }
    else
    {
        typ[cur] = 's';
        newvals[curs++] = val[cur];
    }
    for (int to : v[cur])
    {
        if (!used[to])
    ...
    endp = curp - 1;
    ends = curs - 1;
    ap[cur] = pii(begp, endp);
    as[cur] = pii(begs, ends);
    //DBN(cur, ap[cur], as[cur]);
}
struct STree
{
    char type;
    ...
    FI(m)
    {
        ...
        if (t == 1)
        {
            ...
            if (typ[x] == 'p')
            {
                pt.q = y;
                st.q = -y;
                pt.add(1, 1, N, ap[x].first, ap[x].second);
                st.add(1, 1, N, as[x].first, as[x].second);
            }
            else
            {
                pt.q = -y;
        ...
        else
        {
            ...
            if (typ[x] == 'p')
            {
                ans = pt.get(1, 1, N, ap[x].first);
            }
            else
            {
                ans = st.get(1, 1, N, as[x].first);
            }
            Prn(ans);
        }
    }
//  printarray(newvalp + 1, curp);
//  printarray(newvals + 1, curs);
    return 0;
}