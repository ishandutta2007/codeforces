...
struct SlideDeq{
  ...
  void push(T x,int idx){
    if(isMin)while(!deq.empty() && deq.back().first >= x)deq.pop_back();
    if(!isMin)while(!deq.empty() && deq.back().first <= x)deq.pop_back();
    deq.emplace_back(x, idx);
  }
  void pop(int idx, bool lower = true){
    while(lower && deq.front().second < idx)deq.pop_front();
    while(!lower && deq.front().second > idx)deq.pop_front();
  }
  T query(){
...
int main(){
  ...
  rep(i,0,n)cin>>x[i];
  vector<ll>p(m),c(m);
  rep(i,0,m)cin>>p[i]>>c[i];
  sort(ALL(x));
  {
    auto ord=ascend(p);
    rearrange(ord,p,c);
  }
  auto dp=vec(m+1,n+1,INF);
  dp[0][0]=0;
  rep(i,0,m){
    ll sum=0;
    SlideDeq<ll,true>deq;
    rep(j,0,n+1){
      deq.push(dp[i][j]-sum,j);
      deq.pop(j-c[i]);
      dp[i+1][j]=sum+deq.query();
      if(j<n)sum+=abs(p[i]-x[j]);
    }
  }
  ans(dp[m][n]==INF,-1,dp[m][n]);
  return 0;
}