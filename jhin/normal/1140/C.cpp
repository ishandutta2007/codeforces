...
long long m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000],c2[500000];string s2;vector<int>v,v2[100003];long long sum,ans,num;char ch;
...
for(int i=n-1;i>=0;i--)
{
    myst.insert(v1[i].second);
    sum+=v1[i].second;
    if(myst.size()==k){sum-=*myst.begin();myst.erase(myst.begin());}
    a[i]=sum;
}

for(int i=0;i<n;i++)
{
    //cout<<v1[i].first<<" "<<v1[n-1].second<<" "<<v1[i-1].second<<endl;
    ans=max(v1[i].first*(v1[i].second+a[i+1]),ans);
}
cout<<ans;
