...
struct bru {
	...
	bool edge(int a, int b) {
		return (matr[a][b] == '1') ^ (flipped >> a & 1) ^ (flipped >> b & 1);
	}
	int cou_v = 0;
	void dfs(int x, bool inv) {
		if (visited[x]) return;
		cou_v++;
		visited[x] = true;
		REP(i, n) if (x != i && edge(x, i) == inv) dfs(i, inv);
	}
	bool connected() {
		cou_v = 0;
		memset(visited, 0, sizeof(visited));
		dfs(0, true);
		if (cou_v != n) return false;
		cou_v = 0;
		memset(visited, 0, sizeof(visited));
		dfs(0, false);
		if (cou_v != n) return false;
	...
	void run_bru() {
		int best = nax, cou = 0;
		for (flipped = 0; flipped < (1 << n); ++flipped) {
			if (connected()) {
				int c = popc(flipped);
				if (c == best) cou++;
				else if (c < best) {
		...
		if (best == nax) printf("-1\n");
		else {
			assert(best <= 2);
			if (n >= 7) assert(best <= 1);
			FOR(i, 1, best) cou = cou * 1ll * i % mod;
			printf("%d %d\n", best, cou);
		}
		exit(0);
	}
...
vector <int> graf[nax];
vector <int> grafT[nax];
bool edge(int i, int j) {return matr[i][j] == '1';}
void add(vector <vector <int> > &coms, int v, bool rev) {
	int lmost = coms.size(), rmost = -1;
	REP(i, (int)coms.size()) {
		for (int x : coms[i])
			if (edge(v, x) == rev)
				mini(lmost, i);
	...
	if (lmost > rmost) {
		assert(rmost - lmost == -1);
		coms.insert(coms.begin() + lmost, vector<int>{v});
	}
	else {
		for (int i = lmost + 1; i <= rmost; ++i) {
			coms[lmost].insert(coms[lmost].end(), ALL(coms[i]));
			coms[i].clear();
		}
		coms[lmost].push_back(v);
		coms.erase(coms.begin() + lmost + 1, coms.begin() + rmost + 1);
	}
}
int conns = 0;
void go(int l, int r, vector <vector <int> > &coms) {
	debug(imie(l), imie(r), imie(coms));
	assert(l != r);
	if (r - l == 1) {
		add(coms, l, true);
		debug(imie(l), imie(coms));
		if (coms.size() == 1u) conns++;
		return;
	}
	int m = (l + r) / 2;
	vector <vector <int> > r_coms = coms;
	for (int i = l; i < m; ++i)
		add(r_coms, i, false);
	for (int i = m; i < r; ++i)
		add(coms, i, false);
...
int main() {
	scanf("%d", &n);
	REP(i, n) scanf("%s", matr[i]);
	if (n <= 6) bru().run_bru();
	REP(i, n) REP(j, n) if (matr[i][j] == '1') graf[i].push_back(j);
