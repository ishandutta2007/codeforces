...
void solve() {
    ...
    auto for_each_neighbor = [&](int i, int j, auto f) {
        for (auto [x, y] : array<pii, 4>{{{i + 1, j}, {i - 1, j}, {i, j + 1}, {i, j - 1}}}) {
            if (x < 0 || x >= n || y < 0 || y >= m)
                continue;
            f(x, y);
        }
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++) {
            ...
            for_each_neighbor(i, j, [&](int x, int y) -> void {
                if (grid[x][y] != X)
                    bad[i][j]++;
            });
        }
    ...
    y_combinator([&](auto self, int i, int j) -> void {
        ...
        for_each_neighbor(i, j, [&](int x, int y) {
