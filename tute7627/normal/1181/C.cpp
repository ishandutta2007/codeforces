...
int main(){
  ...
  rep(i,0,n){
    rep(j,0,m){
      if(i!=0&&s[i-1][j]==s[i][j])continue;
      ll mi=INF;
      ll len=x[i][j];
      ll top=i-len,bot=i+len;
      if(top<0||bot>=n)continue;
      if(x[top][j]<len||x[bot][j]<len)continue;
      rep(o,top,bot+len)chmin(mi,y[o][j]);
      res+=mi;
    }
  }
  cout<<res<<endl;
