...
int main(){
  ...
  while(t--){
    ...
    ll ret=0;
    vector<ll>x(3),y(3);
    ll z=d[gcd(gcd(a[0],a[1]),a[2])];
    rep(i,0,3)y[i]=d[gcd(a[i],a[(i+1)%3])]-z;
    rep(i,0,3)x[i]=d[a[i]]-z-y[i]-y[(i+2)%3];
    //cout<<z<<endl;
    //debug(y,3);
    //debug(x,3);
    ll sum=z;
    ...
    ret=c[sum+2][3];
    rep(i,0,3){
      ret-=c[x[i]+x[(i+1)%3]+y[i]+2][3];
      ret-=c[x[i]+1][2]*(y[(i+1)%3]+z);
      ret+=c[x[i]+2][3];
    }
    cout<<ret<<endl;
  }
