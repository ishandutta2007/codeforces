...
//	boolean[] b = new boolean[MAX];
	...
	public long stupid(int n, long x, long[] a) {
		long ans = 0;
		while (a[0] <= x) {
			long cur = x;
			for (int i = 0; i < a.length; i++) {
	...
	public void solve() {
		for (int qwerty = in.nextInt(); qwerty > 0; --qwerty) {
			...
			for (int i = 0; i < dp.length; i++) {
				...
				dp[i][0][1] = Integer.MAX_VALUE / 2;
				dp[i][1][0] = Integer.MAX_VALUE / 2;
				dp[i][1][1] = Integer.MAX_VALUE / 2;
			}
			dp[0][0][0] = 0;
			for (int i = 1; i < n; i++) {
				if (a[i] != a[i - 1]) {
					dp[i][0][0] = Math.min(dp[i][0][0], dp[i - 1][0][0]);
					dp[i][0][1] = Math.min(dp[i][0][1], Math.min(dp[i - 1][0][1], dp[i - 1][1][1]));
					dp[i][1][1] = Math.min(dp[i][1][1], Math.min(dp[i - 1][0][0] + 1, dp[i - 1][1][1] + 1));
				} else {
					dp[i][0][1] = Math.min(dp[i][0][1], Math.min(dp[i - 1][0][0], dp[i - 1][1][1]));
					dp[i][1][1] = Math.min(dp[i][1][1], Math.min(dp[i - 1][0][0] + 1, dp[i - 1][1][1] + 1));
				}
				if (i + 1 < n) {
					dp[i + 1][1][1] = Math.min(dp[i + 1][1][1], dp[i - 1][0][0] + 1);
				}
			}
			out.println(Math.min(dp[n - 1][0][0], Math.min(dp[n - 1][0][1], dp[n - 1][1][1])));
		}
	}
