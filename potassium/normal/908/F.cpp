...
int main() {
	...
	if (Gq.empty()) {
		ll ans = 0;
		ans += (Rq.empty() ? 0 : Rq.back().F - Rq.front().F);
		ans += (Bq.empty() ? 0 : Bq.back().F - Bq.front().F);
	...
	ll ans = 0;
	ans += Gq.front().F - Rq.front().F;
	ans += Gq.front().F - Bq.front().F;
	ans += Rq.back().F - Gq.back().F;
	ans += Bq.back().F - Gq.back().F;
	
	while (Rq.front().S != 'G') Rq.pop_front();
	while (Bq.front().S != 'G')	Bq.pop_front();
	while (Rq.back().S != 'G') Rq.pop_back();
	while (Bq.back().S != 'G') Bq.pop_back();
	
	assert(Rq.front() == Bq.front());
	int lastG = Rq.front().F;
	Rq.pop_front();
	Bq.pop_front();
	
	while (!Rq.empty()) {
		int maxR = 0, maxB = 0;
		int lastR = lastG, lastB = lastG;
		int nowG = -1;
		
		auto pop = [&](deque<pt>& q, int& last, int& maxv) {
			while (true) {
				char color = q.front().S;
				if (color == 'G')
					nowG = q.front().F;

				chkmax(maxv, q.front().F - last);
				chkmax(last, q.front().F);
		...
		ll x1 = (nowG - lastG) * 3LL - maxR - maxB;
		ll x2 = (nowG - lastG) * 2LL;
