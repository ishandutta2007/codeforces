...
int main(){
	int n; scanf("%d",&n);
	int ans=fpow(3,1ll*n*n%(MOD-1));
	for(int i=1;i<=n;++i){
		ans=(ans+2ll*(i&1?MOD-1:1)*fpow(3,i)%MOD*comb(n,i)%MOD*fpow(3,1ll*(n-i)*n%(MOD-1)))%MOD;
//		printf("[%d]",ans);
		ans=(ans+3ll*comb(n,i)*(i&1?MOD-1:1)%MOD*fpow(3,1ll*(n-i)*n%(MOD-1))%MOD*(fpow(1+MOD-fpow(3,MOD-1-n+i),n)+MOD-1))%MOD;
	}
	ans=mo2(fpow(3,1ll*n*n%(MOD-1))-ans);
	printf("%d\n",ans);
	return 0;
}