...
pii inte(pii a, pii b) {
	return {max(a.first, b.first), min(a.second, b.second)};
}
pii unio(pii a, pii b) {
...
void solve() {
	...
	scanf("%d%d", &n, &k);
	pii s1, s2;
	scanf("%d%d%d%d", FS(&s1), FS(&s2));
	debug(imie(s1), imie(s2));
	int il = len(inte(s1, s2)), ul = len(unio(s1, s2));
	...
	if (il) {
		...
		int reach = ul - l1 - l2;
		int ones = ul;
		debug(imie(reach), imie(ones));
		ll ans = numeric_limits<ll>::max();
		for (int t = 1; t <= n; ++t) {
			ll curr = reach * 1ll * t;
			ll all_ones = ones * 1ll * t;
			if (all_ones >= k)
				curr += k;
			else
				curr += all_ones + 2 * (k - all_ones);
			mini(ans, curr);
		}
		printf("%lld\n", ans);
	}
