    ...
    int main()
    {
        ...
        cin>>n;
        for(int i=0;i<n;i++){cin>>a[i];mymp[a[i]]++;}
        for(auto it=mymp.begin();it!=mymp.end();it++)
        {
            int tmp=it->second;
            int ind=it->first;
            while(tmp>1){muls.insert(ind);tmp--;}
        }
        for(int i=0;i<n;i++)
        {auto tmp=muls;int cnt=0;
            if(tmp.empty())return cout<<0,0;
            for(int j=i;j<n;j++)
            {
                auto it=tmp.lower_bound(a[j]);
                if(it!=tmp.end()){if(*it==a[j])
                tmp.erase(it);}cnt++;
                if(tmp.empty()){mn=min(mn,cnt);break;}
