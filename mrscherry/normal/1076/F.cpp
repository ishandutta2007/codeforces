...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ll nx = inf, ny = inf;
        if (dpx <= k) {
            if ((dpx + x - 1ll) / k <= y && y <= (ll)x * k) {
                nx = min(nx, (dpx + x - 1ll) / k < y ? 1ll : (dpx + x - 1ll) % k + 1ll);
            }
            if ((dpx + x - 1ll) / k < y && y <= (x + 1ll) * k) {
                ny = min(ny, y <= (ll)x * k + 1ll ? 1ll : y - (ll)x * k);
            }
        }
        if (dpy <= k) {
            if ((dpy + y - 1ll) / k <= x && x <= (ll)y * k) {
                ny = min(ny, (dpy + y - 1ll) / k < x ? 1ll : (dpy + y - 1ll) % k + 1ll);
            }
            if ((dpy + y - 1ll) / k < x && x <= (y + 1ll) * k) {
                nx = min(nx, x <= (ll)y * k + 1ll ? 1ll : x - (ll)y * k);
            }
        }
        dpx = nx;
        dpy = ny;
    }
    if (dpx <= k || dpy <= k) cout << "YES";
    else cout << "NO";
}