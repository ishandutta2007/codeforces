...
{
	if(n == 0) return P(0, 0);
	ll a = upper_bound(all(vec), n) - vec.begin() - 1;
	return max(calc(n-a*a*a)+P(1, a*a*a), calc(a*a*a-1-(a-1)*(a-1)*(a-1))+P(1, (a-1)*(a-1)*(a-1)));
}
...
int main(void)
{
	...
	cin >> n;
	rep(i, 0, 100000) vec.push_back(i*i*i);
	
	P res = calc(n);
	outl(res.first, res.second);
