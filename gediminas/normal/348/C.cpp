...
struct aibe {
	...
	ll getSum() {
		if (k < sqrtN) {
			...
			for (auto && x : didel) {
				ats += aibes[x].prid * aibes[x].sutap[i];
			}
...
int main () {
	...
	for (int i = 0; i < m; i++) {
		aibes[i].i = i;
		cin >> aibes[i].k;
		aibes[i].ind.resize(aibes[i].k);

		for (int j = 0; j < aibes[i].k; j++) {
			cin >> aibes[i].ind[j];
			aibes[i].ind[j]--;
			aibes[i].sum += a[aibes[i].ind[j]];
		}
	...
	for (auto && i : didel) {
		...
		for (auto && x : aibes[i].ind) {
			temp[x] = true;
		}

		aibes[i].sutap.resize(m);

		for (int j = 0; j < m; j++) {
			if (i == j)
				continue;

			for (auto && x : aibes[j].ind) {
				if (temp[x])
					aibes[i].sutap[j]++;
			}
		}
