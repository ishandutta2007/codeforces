...
vv<int> readG(const int &n,const int &m){ vv<int> g(n); rep(_,m){ cauto a=read<int>(1),b=read<int>(1); g[a].pb(b); } return move(g);}
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	// 	es[i]={a,b};
	// }
	vv<pair<int,ll>> g(21*n);
	for(cauto [a,b]:es)rep(i,21){
			if(i%2) g[n*i+b].eb(n*i+a,1);
			else g[n*i+a].eb(n*i+b,1);
		}
	rep(i,19)rep(v,n) g[n*i+v].eb(n*(i+1)+v,1<<i);
	rep(v,n){
		g[n*20+v].eb(n*19+v,1ll<<30);
		g[n*19+v].eb(n*20+v,1ll<<30);
	}
	vector<ll> d;
	dij(g,0,d);
	const ll INF=1e18;
	ll mn=INF;
	rep(i,19) MN(mn,d[n*i+n-1]);
	if(mn==INF){
		reps(i,19,21) MN(mn,d[n*i+n-1]);
		cauto x=mn>>30;
		mint re=mn%(1ll<<30);
		mint cur=1<<18;
		rep(_,x){
