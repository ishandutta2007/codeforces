#line 1 "codeforces-workspace/contest/1411/g/g.cpp"
...
#line 6 "codeforces-workspace/contest/1411/g/g.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1411/g/g.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1411/g/g.cpp"
...
template <auto Mod, unsigned Storage> struct modint_base {
  ...
  // Direct constructor
  template <class _Tp>
  constexpr modint_base(_Tp __n, direct_ctor_t) : value(__n) {}
 ...
 public:
  ...
  constexpr modint_base(_Tp __n) noexcept
      : value((__n %= mod) < 0 ? __n += mod : __n) {}

  constexpr modint_base(bool __n) noexcept : value(__n) {}
  ...
  constexpr modint_base operator+() const noexcept { return *this; }
  ...
  constexpr modint_base &operator+=(const modint_base &__x) noexcept {
  ...
  template <class _Tp>
  constexpr
      typename std::enable_if<is_integral_ext<_Tp>::value, modint_base>::type &
  ...
  constexpr
    ...
    return modint_base{*this} += __x;
  }

  constexpr modint_base operator+(modint_base __x) const noexcept {
    return __x += *this;
  }
  ...
  constexpr friend
      typename std::enable_if<is_integral_ext<_Tp>::value, modint_base>::type
      operator+(_Tp const &__x, modint_base __y) noexcept {
    return __y += __x;
  }
  ...
  constexpr modint_base operator-(const modint_base &__x) const noexcept {
    modint_base __t;
    if (((__t.value = value) -= __x.value) < 0) __t.value += mod;
    return __t;
  }
  ...
  constexpr friend
      typename std::enable_if<is_integral_ext<_Tp>::value, modint_base>::type
      operator-(_Tp __x, const modint_base &__y) noexcept {
 ...
 public:
  ...
  friend constexpr
      typename std::enable_if<is_integral_ext<_Tp>::value, modint_base>::type
      pow(modint_base b, _Tp e) noexcept {
    if (e < 0) {
      e = -e;
  ...
  constexpr
    ...
    modint_base __r, b;
    __r.value = 1;
    for (b.value = __e < 0 ? __e = -__e, _div(1, value) : value; __e;
         __e >>= 1, b *= b)
      if (__e & 1) __r *= b;
    return __r;
  }
  ...
  template <class _Os>
  friend _Os &operator<<(_Os &__os, const modint_base &__x) noexcept {
    return __os << __x.value;
  }

  friend std::istream &operator>>(std::istream &__is,
                                  modint_base &__x) noexcept {
    intmax_t value;
    __x = (__is >> value, value);
    return __is;
  }
};
...
template <auto Mod, unsigned Storage>
unsigned modint_base<Mod, Storage>::storage = Storage;
...
#line 2 "Library/src\\algebra\\convolution\\bitxor.hpp"
...
#line 10 "Library/src\\algebra\\convolution\\bitxor.hpp"

#line 12 "Library/src\\algebra\\convolution\\bitxor.hpp"
...
#line 28 "codeforces-workspace/contest/1411/g/g.cpp"
...
void main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<vector<int>> grh(n), rgrh(n);
  vector<int> out(n);
  for (auto i : range(m)) {
    int a, b;
    cin >> a >> b;
    --a, --b;
    out[a]++;
    grh[a].emplace_back(b);
    rgrh[b].emplace_back(a);
  }
  ...
  vector<int> g(n);
  {
    ...
    while (!q.empty()) {
      ...
      while (ban.count(g[now])) {
        g[now]++;
      }
    }
    assert(out == vector<int>(n));
  }

  m = *max_element(begin(g), end(g));
  m = __bit_ceil(m + 1);
  vector<mint> f(m);
  f[0] = 1;
  for (auto &&x : g) {
