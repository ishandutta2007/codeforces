fn run() {
    input!(n: usize, k: usize, f: usize);
    let mut dp = vec![M::one(); k + 1];
    for _ in 1..n {
        let mut sq = dp.square();
        let mut a = vec![M::one(); k + 1];
        for i in (0..(sq.len() - 1)).rev() {
            sq[i] = sq[i] + sq[i + 1];
        }
        for i in (0..k).rev() {
            a[i] = a[i] + a[i + 1];
    ...
    let ans = dp.square().get(f).map_or(M::zero(), |p| *p);
    println!("{}", ans);
}
    ...
    T: NTTFriendly,
{
    ...
    fn square(&self) -> Vec<Self::Item> {
        ...
        let size = (self.len() * 2 - 1).next_power_of_two();
        let mut f = vec![ModInt::zero(); size];
        f[..self.len()].copy_from_slice(self);
        f.ntt();
        f.iter_mut().for_each(|f| *f = *f * *f);
        f.intt();
        f.truncate(self.len() * 2 - 1);
        f
    }
}
