...
struct Rerooting {
    ...
    Rerooting(int n, const F1 &f1, const F2 &f2, const T &ident) : g(n), f1(f1), f2(f2), ident(ident), dp(n, ident), subdp(n, ident) {}
...
void solve() {
    INT(n);
    auto f1 = [&](LP a, LP b) {
        return LP{a.fr + b.fr, a.sc + b.sc};
    };
    auto f2 = [&](LP a, ll b) {
        return LP{a.fr + a.sc + b, a.sc + b};
    };
    Rerooting<ll, LP> rt(n, f1, f2, LP{0, 0});
    rep(i, n - 1) {
        INT(u, v);
        -- u, -- v;
        rt.add_edge(u, v, 1LL);
