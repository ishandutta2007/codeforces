...
vc<int> a;
int Consist( int l , int r ){
	int lcnt = lower_bound(All(a),l) - a.begin();
	int rcnt = upper_bound(All(a),r) - a.begin();
	return rcnt - lcnt;
}
ll Solve( int l , int r ){
	ll con = Consist(l,r);
	if(!con) return cA;
	if( l == r ) return cB*con;

	ll ans1 = cB * con * (r-l+1);
	ll ans2 = Solve(l,l+r>>1) + Solve((l+r>>1)+1,r);
