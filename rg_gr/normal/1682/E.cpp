// Problem: CF1682E Unordered Swaps
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/CF1682E
// Memory Limit: 250 MB
// Time Limit: 2000 ms
// 
...
{
	f[u]=fa,d[u]=d[fa]+1;
	fe(u)if(v!=fa)r[v]=e[i].w,dfs(v,u);gr
}
void run(int u,int v)
{
	std::vector<int>a,b;
	while(d[u]>d[v])a.push_back(r[u]),u=f[u];
	while(d[v]>d[u])b.push_back(r[v]),v=f[v];
	while(u!=v)
	{
		a.push_back(r[u]),u=f[u];
		b.push_back(r[v]),v=f[v];
	}
	std::reverse(b.begin(),b.end());
	for(auto x:b)a.push_back(x);
	for(int i=0;i+1<a.size();i++)
		g[a[i]].push_back(a[i+1]),oh[a[i+1]]++;
}
signed main()
{
	initprog();
	...
	while(!q.empty())
	{
		int u=q.front();q.pop();
		odb(u);
		for(auto v:g[u]){oh[v]--;if(oh[v]==0)q.push(v);gr
	}
	return 0;
}