// X(or)-mas Tree (https://codeforces.com/contest/1615/problem/D)
...
int main() {
    ...
    while (t--) {
        int n, m; read(n, m);
        vector e(n - 1, array{0, 0, 0});
        vector adj2(n, vector<pair<int, bool>>());
        for (auto& [x, y, v] : e) {
            read(x, y, v); --x, --y;
            if (v != -1) {
                int p = __builtin_parity(v);
                adj2[x].emplace_back(y, p);
                adj2[y].emplace_back(x, p);
            }
        }
        rep(_, m) {
            int a, b, p; read(a, b, p); --a, --b;
            adj2[a].emplace_back(b, p);
            adj2[b].emplace_back(a, p);
        }
        ...
        rep(i, n) {
            ...
            while (sz(q)) {
                int v = q.front(); q.pop();
                for (auto [v2, p] : adj2[v]) {
                    if (par[v2] == -1)
                        par[v2] = p ? 1 - par[v] : par[v], q.push(v2);
                    else
                        valid &= ((par[v2] != par[v]) == p);
                }
            }
        }
        ...
        println("YES");
        for (auto [x, y, v] : e)
            println(x + 1, y + 1, (v == -1 ? int(par[x] ^ par[y]) : v));
    }
}