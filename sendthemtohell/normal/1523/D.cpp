...
void upd(int& cur, string& curs, const vector<int>& pos, int ms) {
    if (popc(ms) > cur) {
        cur = popc(ms);
        for (int i = 0; i < sz(curs); ++i) curs[i] = '0';
    ...
    for (int tr = 0; tr < 100; ++tr) {
        ...
        for (int i = 0; i < sz(pos); ++i) {
            for (int mask = 0; mask < (1 << sz(pos)); ++mask) {
                if (mask & (1 << i)) {
                    msk[mask] += msk[mask ^ (1 << i)];
                }
            }
        }
        for (int ms = 0; ms < (1 << sz(pos)); ++ms) {
            if (msk[ms] * 2 >= n) {
                upd(ans, anss, pos, ms ^ ((1 << sz(pos)) - 1));
            }
        }
    }
    cout << anss << endl;
