...
int m,x,n,y,z,k,mx,sum,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000];string s,s2;vector<int>v,v2[100003];long long ans,num;char ch;
...
int md=siz%ro;
int cl=siz/ro;
if(ro==1)return cout<<1<<" "<<siz<<endl<<s,0;
if(md)cl++;md=ro-md;if(md==ro)md=0;
cout<<ro<<" "<<cl<<endl;
for(int i=0,j=0;i<siz;i++,j++)
{
   if(((j)%cl)==0){cout<<endl;used=false;}
   if(md&&!used){cout<<'*';used=true;md--;i--;}
   else cout<<s[i];
}
