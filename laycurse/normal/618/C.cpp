...
int N, X[110000], Y[110000], ind[110000];
ll getSignedArea(int x1, int y1, int x2, int y2, int x3, int y3){return (ll)(x2-x1)*(y3-y1)-(ll)(x3-x1)*(y2-y1);}
int getAreaSign(int x1, int y1, int x2, int y2, int x3, int y3){ll k=getSignedArea(x1,y1,x2,y2,x3,y3);if(k<0)return -1;if(k>0)return 1;return 0;}

int main(){
  int i, j, k;

  reader(&N);
  rep(i,N) reader(X+i,Y+i), ind[i] = i+1;
  sort(N, X, Y, ind, mem);

  REP(i,2,N) if(getAreaSign(X[0],Y[0],X[1],Y[1],X[i],Y[i])) break;
  writerLn(ind[0],ind[1],ind[i]);
