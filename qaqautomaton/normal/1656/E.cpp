...
namespace run{
	...
	void dfs(int x,int f){
		...
		s[x]=a[x];
		for(int i=to[x].size()-1;~i;--i)if(to[x][i]==f){swap(to[x][i],to[x].back());to[x].pop_back();}
	...
	bool main(){
		...
		dfs(1,0);
		ll ss=s[1];
		/*for(int i=1;i<=n;++i)write(s[i],i==n?'\n':' ');
		return 0;*/
		/*for(int x=1;x<=n;++x){
			if(to[x].size()){
				ll v=s[to[x][0]];
				for(auto i:to[x])assert(s[i]==v);
				if(x>1){
					assert(ss-s[x]==v);
				}
