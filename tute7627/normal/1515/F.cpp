...
template<typename T>void debug(PQ<T> st){while(!st.empty()){if(!st.empty()){cout<<st.top();st.pop();}while(!st.empty()){cout<<" "<<st.top();st.pop();}cout<<endl;}}
...
int main(){
  ...
  rep(_,0,n-1){
    ...
    if(av>=x){
      que.pop();
      //cout<<k spa g[k].size()<<endl;
      while(!g[k].empty()){
        e=poll(g[k]);
        //cout<<e spa used[e] spa "check"<<endl;
        if(!used[e])break;
      }
    }
    else{
    ...
    used[e]=true;
    cout<<e+1<<endl;
    ll ar=uf.root(u[e]),br=uf.root(v[e]);
    uf.unite(u[e],v[e]);
    ll nr=uf.root(u[e]);
    if(ar==nr)swap(ar,br);
    /*cout<<"que1"<<endl;
