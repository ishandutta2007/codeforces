...
void connect_row_single(int i) {
    // make a single connection from empty cell on row i to empty cells on row i - 1 or row i - 2
    assert(i - 1 > 0);
    int pos = s[i].find('X');
    if (pos == -1) {
        int prev_pos = s[i - 1].find('X');
        if (prev_pos == -1) s[i - 1][prev_pos = 0] = 'X';
        s[i][prev_pos] = 'X';
    } else {
        s[i - 2][pos] = 'X';
        s[i - 1][pos] = 'X';
...
void solve() {
    ...
    while (row_done < n) {
        if (row_done + 1 < n) {
            clear_row(row_done + 1);
            if (row_done > 0) connect_row_single(row_done);
            row_done = row_done + 3;
        } else {
            break;
        }
