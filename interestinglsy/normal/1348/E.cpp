...
ll Solve(){
	...
	fOR(pos,n){
		...
		auto udp = dp[cur];
		For0(aRemain,k){
			if( aRemain > aSum[pos-1] ) break;
			int bRemain = (sum[pos-1]-aRemain) % k;

			ll ans = 0;
			int lim = Min(a[pos],k-1);
			Forx(aLeft,0,lim){
				ll tProfit = (a[pos]+b[pos]-aLeft)/k;
				ll bLeft = (a[pos]+b[pos]-aLeft)%k;
				if( bLeft > b[pos] ) continue;
				ll tans = ldp[(aRemain+aLeft)%k]
							+ tProfit + (aRemain+aLeft)/k + (bRemain+bLeft)/k;
				MyMax(ans,tans);
			}
			udp[aRemain] = ans;
		}
