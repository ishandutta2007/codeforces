...
long long m,x,n,y,z,k,mx,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000];string s2;vector<int>v,v2[100003];long long sum,ans,num;char ch;
...
for(int i=0;i<s.size();i++)
{
    ...
    if(s[i]!=s[i+1])
    {
        sort(v.begin(),v.end());
        int tmp=v.size();
        for(int j=v.size()-1;j>=0&&j>=tmp-k;j--)
        {
          sum+=v[j];
        }
        v.clear();
