...
int main() {
    ...
    for (auto x: s) cnt.push_back(mp(map[x], x));
    sort(cnt.begin(), cnt.end());

    vector<int> cntcnt(n+1);
    for (auto it: cnt) cntcnt[it.first]++;

    vector<int> sufcntcnt(n+1);
    sufcntcnt[n] = cntcnt[n];
    for (int i = n-1; i>=1; i--) sufcntcnt[i] =sufcntcnt[i+1] + cntcnt[i];

    vector<int> pref(n+1);
    pref[0] = 0;

    for (int i = 1; i<=n; i++) pref[i] = pref[i-1] + sufcntcnt[i];
    ...
    for (ll i = 1; i<=n; i++)
    {
        if (pref[i]>=i*i && pref[i] - (pref[i]%i) > res)
        {
            res = pref[i] - pref[i]%i;
            best = i;
        }
    ...
    int h = best;
    int w = res/best;
    for (auto it: cnt) for (int i = 0; i<min(best, it.first); i++) easy.push_back(it.second);
