...
// 0    1         2         3       4       5
// ans, min(a+b), min(c-b), cnt(a), cnt(b), cnt(c)
template<class T> struct Seg { // comb(ID,b) = b
    const T ID = {0, 0, 0, 0, 0, 0}; 
    T comb(T a, T b) {
        T ans = {0,0,0,0,0,0};
        ans[1] = min(a[1]+b[4], b[1]+a[3]);
        ans[2] = min(a[2]+b[5]-b[4], b[2]);
        ans[3] = a[3]+b[3];
        ans[4] = a[4]+b[4];
        ans[5] = a[5]+b[5];
        ans[0] = 1e9;
...
str s;
Seg<ai<6>> st;
ai<6> aval = {0, 0, 0, 1, 0, 0};
ai<6> bval = {0, 0, -1, 0, 1, 0};
ai<6> cval = {0, 0, 0, 0, 0, 1};
ai<6> vals[3];
