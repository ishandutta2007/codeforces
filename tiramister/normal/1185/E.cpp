...
void solve() {
    ...
    vector<tuple<int, int, int, int>> ans(27, make_tuple(-1, -1, -1, -1));
    for (int k = 25; k >= 0; --k) {
        char c = 'a' + k;
        bool found = false;
        ...
        if ((lx != rx) && (ly != ry)) {
            ...
            ans[k] = make_tuple(lx, ly, rx, ry);
        }
    }

    cout << "YES\n";
    while (!ans.empty() && get<0>(ans.back()) < 0) ans.pop_back();

    cout << ans.size() << "\n";
    for (auto t : ans) {
        int lx, ly, rx, ry;
        tie(lx, ly, rx, ry) = t;
        cout << lx + 1 << " " << ly + 1 << " " << rx + 1 << " " << ry + 1 << "\n";
    }
}
