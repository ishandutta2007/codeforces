...
map<char, int> g = {{'A', 0}, {'C', 1}, {'G', 2}, {'T', 3}};
...
int main() {
    ...
    sparse_table s_or(all(s), [](int x, int y){ return x|y; });
    for (int i = 0; i < n; i++)
        s[i] = s_or.query(max(0, i-k), min(n-1, i+k));
    vector<ll> p(4*n), q(4*m);
    for (int i = 0; i < n; i++)
        for (int j = 0; j < 4; j++)
            p[4*i+j] = (~s[i]>>j)&1;
    for (int i = 0; i < m; i++)
