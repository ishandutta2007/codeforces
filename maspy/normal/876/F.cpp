...
#line 1 "/home/maspy/compro/library/alg/monoid_max_idx.hpp"
...
void solve() {
  ...
  VEC(ll, A, N);
  vc<pi> seg_raw(N);
  FOR(i, N) seg_raw[i] = mp(A[i], i);
  SegTree<Monoid_Max_Idx<ll>> seg1(seg_raw);
  SegTree<Monoid_Or<ll>> seg2(A);

  ll ANS = N * (N - 1) / 2;

  FOR(M, N) {
    auto check1 = [&](pi e) -> bool { return e <= mp(A[M], M); };
    ll L = seg1.min_left(check1, M);
    ll R = seg1.max_right(check1, M);
    auto check2 = [&](ll x) -> bool { return x <= A[M]; };
    chmax(L, seg2.min_left(check2, M + 1));
    chmin(R, seg2.max_right(check2, M));
    // print(L, M, R);
    ANS -= (M - L + 1) * (R - M) - 1;
  }
  print(ANS);
