...
int main() {
    ...
    for (int i = 1; i <= MAX; i++) {
        bad_pairs.emplace_back(bad[i], i);
    }
    sort(rall(bad_pairs));

    binary_indexed_tree bit(MAX + 1, 0, plus<int>(), minus<int>());

    vi ans{0};
    for (int n = 1, limit = 0; n <= MAX; n++) {
        while (limit * limit <= n)
            limit++;

        while (bad_pairs.back().first < limit) {
            bit.add(bad_pairs.back().second, 1);
            bad_pairs.pop_back();
        }

        ans.push_back(n - bit.accumulate(0, n + 1));
    }
