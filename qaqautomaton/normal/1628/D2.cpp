...
namespace run{
	...
	Z C(int n,int m){return n<0||m<0||n<m?0:fac[n]*invf[m]*invf[n-m];}
	bool main(){
		int n,m,k;
		read(n,m,k);
		...
		m=n-m;
		if(m==0)return write((ll)n*k%p,'\n');
		Z ans(0);
		for(int i=1;i<n;++i){
			ans+=i*C(n-i-1,m-1)*fpm((p+1)>>1,n-i);
			//write(ans.x,'?');
		}
		...
		for(int i=n-1;i;--i){
			f[i][0]=f[i+1][0]+k;
			for(int j=1;j<=m;++j){
				f[i][j]=(f[i+1][j]+f[i+1][j-1])*((p+1)>>1);
			}
		}
		return write(f[1][m].x,'\n');
	}
}
signed main(){
#ifdef QAQAutoMaton 
