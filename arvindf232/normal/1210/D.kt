...
fun main(){
    solve.cases{
		...
		var initial = 0L
		for(i in 0 until n){
			val v = willinvert[i].size
			initial += v.toLong() * (degree[i].toLong() - v)
		}
        put(initial)
		val q = getint
		repeat(q){
			val v = getint - 1
            val ss = willinvert[v].size
			for(c in willinvert[v]){

				val d = degree[c]
				val s = willinvert[c].size
				initial -= s.toLong() * (d - s)
				willinvert[c].add(v)
				initial += (s + 1).toLong() * (d - s - 1 )
			}
            initial -= ss.toLong() * (degree[v] - ss)
...
/*
forgot one method of change in the maintained math value
 */