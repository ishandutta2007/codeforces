...
void __solve()
{
    ...
    int f[55][26], g[55][26];
    {
        vector<string> ss(l + 1);
        for(int i = 0; i <= l; i++)
        {
            if(i < l) ss[i + 1] = ss[i] + s[i];
            for(int j = 0; j < 26; j++)
            {
                string tmp = ss[i] + char('a' + j);
                for(int h = 0; h <= min(i + 1, l); ++h)
                {
                    if(tmp.substr(i + 1 - h) == ss[h])
                    {
                        f[i][j] = h;
                    }
                }
    ...
    }
    {
        ...
        for(int i = 0; i <= m; i++)
        {
            if(i < m) ss[i + 1] = ss[i] + t[i];
            for(int j = 0; j < 26; j++)
            {
                string tmp = ss[i] + char('a' + j);
                for(int h = 0; h <= min(m, i + 1); ++h)
                {
                    if(tmp.substr(i + 1 - h) == ss[h])
                    {
                        g[i][j] = h;
            ...
            for(int k = 0; k <= m; k++)
            {
                if(c[i] == '*')
                {
                    for(int z = 0; z < 26; z++)
                    {
                        ...
                        chmax(dp[i + 1][jj][kk], dp[i][j][k] + inc);
                    }
                }
                else
                {
