...
static ll n,a[N],dp[N][3],x,y,z,xr,rr;

ll xd(ll t,ll tt)
{
    if(t<105) return dp[t][tt];
    else
    {
        ll t1=(t-100)%rr;
        return dp[100-rr+t1][tt];
    }
}
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,1,105)
        {
            dp[i][0]=mex(xd(max(i-x,0LL),0),xd(max(i-y,0LL),1),xd(max(i-z,0LL),2));
            dp[i][1]=mex(xd(max(i-x,0LL),0),1000,xd(max(i-z,0LL),2));
            dp[i][2]=mex(xd(max(i-x,0LL),0),xd(max(i-y,0LL),1),1000);
        }
        REP(j,1,51)
        {
            bool flag=1;
            REP(kkk,50,101)
            {
                REP(jjj,0,3)
                {
                    if(dp[kkk][jjj]!=dp[kkk-j][jjj]) flag=0;
                }
            }
            if(flag==1)
            {
        ...
        REP(i,0,n)
        {
            ll t1,t2;
            t1=xd(a[i],0);
            t2=xd(max(a[i]-x,0LL),0);
            if(((xr^t1)^t2)==0) ans++;
            t2=xd(max(a[i]-y,0LL),1);
            if(((xr^t1)^t2)==0) ans++;
            t2=xd(max(a[i]-z,0LL),2);
            if(((xr^t1)^t2)==0) ans++;
        }
        cout<<ans;
