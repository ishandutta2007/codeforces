...
fun main() {
	cases{
		...
		product(pset){
			...
			val new = v.map{it - 1}
			val value = (0 until k).map{A[it] * new[it]}.sum()
			if(! v.all { it == 1 }) {
				left.add(value)
				if(value == 0) {
					put("YES")
		...
		product(pSet2){
			...
			val new = v.map{it - 1}
			val value = (k until n).map{A[it] * new[it - k]}.sum()
			if(! v.all { it == 1 }) {
				if(value == 0){
					put("YES")
					return@cases
...
//		val items = mutableListOf<Int>(0)
//		val relevant = (A zip found!!).filter{it.second != 0 }.map{it.first * it.second}
//		for(i in 0 until relevant.lastIndex){
//			val new = items.last() + relevant[i]
//			items.add(new)
//		}
