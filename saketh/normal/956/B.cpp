...
int main() {
    ...
    F0Rd (i, N) {
        if (i+1 < N) {
            auto it = third.upper_bound(energy[i] + U);
            if (it != third.begin() && *prev(it) > energy[i + 1]) {
                can = 1;
                double total = *prev(it) - energy[i];
                double waste = energy[i + 1] - energy[i];
                ckmax(ans, 1. - waste / total);
            }
        }
        third.insert(energy[i]);
    }
