...
int main() {
    ...
    rep(n) {
        CHR(c);
        INT(x);
        if(c == '*' and x == 1) continue;
        v.eb(c == '+', x);
    }
    n = si(v);

    int t = 0;
    fore(e, v) if(e.fi == 0) t++;
    ...
    map<int, vi> mp;
    rep(i, t) { mp[mul[i]].eb(i); }
    vector<pair<int, vi>> idxs{all(mp)};
    vi nums;
    fore(e, idxs) nums.eb(si(e.se) + 1);
    ...
    fore(id, ndFOR(nums)) {
        ...
        vi used(t + 1);
        rep(i, si(id)) { rep(j, id[i]) used[idxs[i].se[j]] = true; }
        int ma = 0;
        dump(id, c, pcnt, used);
        ll s = bin_search(ll(8e18), 0LL, [&](ll x) {
            ll m = 1;
            int now = 0;
            rep(i, t + 1) {
                if(m > 1) {
                    ll ok = ceil(x, M - M / m);
                    int k = rend(a[i]) - lower_bound(rall(a[i]), ok);
                    assert(k >= 0);
                    now += k;
                }
                if(!used[i]) m *= mul[i];
