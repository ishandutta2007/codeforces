 ...
 */
template <typename T> vector<int> topological_sort(const T &g) {
...
int main() {
    TEST {
        INT(n, m);
        VEC(int, a, n);
        Graph g = getG(n, m, true);
        auto id = topological_sort(g);
        vvc<pair<int, pair<mint, ll>>> dp(n);
        rep(i, n) if(a[i]) dp[i].eb(0, pair<mint, ll>(a[i], a[i]));
        fore(i, id) {
            if(i == id.back()) break;
            fore(e, dp[i]) e.fi++;
            fore(e, g[i]) {
                dump(i, e);
                vector<pair<int, pair<mint, ll>>> nxt;
                auto &l = dp[i], &r = dp[e];
                int i = 0, j = 0;
                dump(l, r);
                while(i < si(l) or j < si(r)) {
                    if(j == si(r) or (i < si(l) and l[i].fi < r[j].fi)) {
                        if(empty(nxt) or nxt.back().fi + nxt.back().se.se < l[i].fi) {
                            nxt.eb(l[i]);
                        } else {
                            nxt.back().se += l[i].se;
                            chmin(nxt.back().se.se, inf<int>);
                        }
                        i++;
                    } else {
                        if(empty(nxt) or nxt.back().fi + nxt.back().se.se < r[j].fi) {
                            nxt.eb(r[j]);
                        } else {
                            nxt.back().se += r[j].se;
                            chmin(nxt.back().se.se, inf<int>);
        ...
        if(empty(dp[id.back()]))
            ...
            OUT(dp[id.back()].back().fi + dp[id.back()].back().se.fi);
    }
}