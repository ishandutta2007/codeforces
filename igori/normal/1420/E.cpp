...
// dp[i][j][k] -
// in heaps (1...i) there are j stones in total and (summ of sz * (sz - 1) / 2) == k
// how many operations did we used?
...
void solve()
{
    ...
    for (int l = 0; l < k; l++)
    {
        for (int r = l; r < k; r++)
        {
            for (int toh = 0; toh < m; toh++)
            {
                for (int z = l; z <= r; z++)
                    dist[l][r][toh] += abs(toh - fr[z]);
            }
        }
    }
    dp[0][0][0] = 0;
    ...
    {
        dp[0][h1][h1 * (h1 - 1) / 2] = dist[0][h1 - 1][0];
    }
    for (int i = 0; i + 1 < m; i++)
    {
        for (int st = 0; st <= k; st++)
        {
            for (int put = 0; put + st <= k; put++)
            {
                for (int res = 0; res <= k * (k - 1) / 2; res++)
                {
                    dp[i + 1][st + put][res + (put - 1) * put / 2] = min(dp[i + 1][st + put][res + (put - 1) * put / 2], dp[i][st][res] + dist[st][st + put - 1][i + 1]);
                }
            }
        }
    }
