...
int chkl(){
	int i,j;
	for(j=0;j<m;++j)F[0][j]=F[0][j&1],F[1][j]=F[1][j&1];
	for(j=0;j<m;++j){
		int cnt_1=0;
		int cnt_2=0;
		for(i=0;i<n;++i){
		...
		if(cnt_2>cnt_1)swap(F[0][j],F[1][j]);
	}
	int sum=0;
	for(i=0;i<n;++i)for(j=0;j<m;++j)F[i][j]=F[i&1][j],sum+=F[i][j]!=G[i][j];
	return sum;
}
int chkc(){
	int i,j;
	for(i=0;i<n;++i)F[i][0]=F[i&1][0],F[i][1]=F[i&1][1];
	for(i=0;i<n;++i){
		int cnt_1=0;
		int cnt_2=0;
		for(j=0;j<m;++j){
		...
		if(cnt_2>cnt_1)swap(F[i][0],F[i][1]);
	}
	int sum=0;
	for(i=0;i<n;++i)for(j=0;j<m;++j)F[i][j]=F[i][j&1],sum+=F[i][j]!=G[i][j];
	return sum;
}
int main(){
	int i,j;
	...
	ans=n*m+1;
	do{
		F[0][0]=str[0],F[0][1]=str[1];
		F[1][0]=str[2],F[1][1]=str[3];
		if(chkmin(ans,chkl()))W=F;
		F[0][0]=str[0],F[0][1]=str[1];
		F[1][0]=str[2],F[1][1]=str[3];
		if(chkmin(ans,chkc()))W=F;
