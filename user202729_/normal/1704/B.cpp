// time-limit: 1000
// problem-url: https://codeforces.com/contest/1704/problem/B
...
void up(){
	...
	/*
	 * uh misunderstood statement. This is bad
	 *
	 * ...
	 * reminder: always check sample test case before coding
	 * ...
	 *
	 * although in this case the samples are not very small.
	 * At least there's note/sample explanation......

	std::sort(begin(a), end(a));

	auto left=a.begin();
	int maxFixed=0;
	for(auto right=a.begin(); right!=a.end();){

		++right;

		while(right[-1]-*left > x*2){ // cannot overflow
			++left;
		}

		assert(left<right);
		debug(*left, right[-1]);
		maxFixed=std::max(maxFixed, int(right-left));
	}
	...
	int numChange=-1;
	while(left<n){
		int right=left+1, min=a[left], max=a[left];
		while(right<n and
				std::max(max, a[right])-std::min(min, a[right]) <= x*2){
