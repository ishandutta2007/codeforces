...
int main() {
    ...
    rep(i, M) {
        ...
        rep(j, k) {
            ...
            int d = v[i][nj] - v[i][j];
            while(nj < k) {
                if(v[i][nj] - v[i][nj - 1] != d) break;
                // l[i][nj] = v[i][j];
                nj++;
            }
            int ans = nj;
            nj--;
            rep2(t, j, nj - 1) l[i][t] = ans;
            j = nj - 1;
        }
    }

    vi idx(n);
    rep(i, M) rep(j, si(v[i])) idx[v[i][j]] = j;

    int okcnt = 0, varcnt = 0;
    vi ok(M);
    vpi p(M, pii(-1, -1));
    auto check = [&](int x) {
        okcnt -= ok[x];
        if(p[x].fi != -1) {
            ok[x] = (p[x].se < l[x][p[x].fi]);
        } else
            ok[x] = 0;
        okcnt += ok[x];
    };
    auto add_left = [&](int i) {
        int x = a[i];
        p[x].fi = idx[i];
        if(p[x].se == -1) p[x].se = idx[i], varcnt++;
        check(x);
    };
    auto add_right = [&](int i) {
        int x = a[i];
        p[x].se = idx[i];
        if(p[x].fi == -1) p[x].fi = idx[i], varcnt++;
        check(x);
    };
    auto del_left = [&](int i) {
        int x = a[i];
    ...
    INT(Q);
    vi ans(Q);
    auto out = [&](int i) { ans[i] = varcnt + (okcnt ? 0 : 1); };
