...
void work() {
	read(b, q, y, c, r, z);
	if (r % q != 0 || !inB(c) || !inB(c + (z - 1) * r)) return write(0, '\n');
	if (!inB(c - r) || !inB(c + z * r)) return write(-1, '\n');
	vector <ll> P;
	for (ll i = 1; i * i <= r; ++i) if (r % i == 0) {
		P.push_back(i);
		if (i * i != r) P.push_back(r / i);
	}
	mi ans = 0;
	for (ll p : P)
		if (lcm(p, q) == r) ans += (r / p) * (r / p);
	write(ans.x, '\n');
}
