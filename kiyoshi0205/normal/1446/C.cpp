...
//how many erase?
ll solve(ll N,map<ll,vec,greater<ll>>& mp){
  if(N<=2)return 0;
  ll cnt=0,sum=0,ans=N-2;
  assert(mp.size());
  for(auto& x:mp){
    sum+=x.second.size();
    if(x.second.size()<=2){
      chmin(ans,N-cnt-(ll)x.second.size()-(N>sum));
      cnt++;
      continue;
    }
    map<ll,vec,greater<ll>> next;
    for(auto& p:x.second){
      ll j=p;
      while((j&(-j))!=j){
        j-=j&(-j);
    ...
    debug(N,cnt,sum);
    chmin(ans,N-cnt-(sum<N)-(ll)x.second.size()+solve(x.second.size(),next));
    ++cnt;
    debug(x);
  }
  debug(ans);
