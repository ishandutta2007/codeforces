...
void solve() {
    ...
    for (int i = 0; i < n1; ++i)
        flow.add_edge(s, i, 1);
    for (int i = 0; i < n2; ++i)
        flow.add_edge(i + n1, t, 1);
    int res = flow.runFlow();
    flow.runBfs();
    vector <bool> cover(n1 + n2, false);
    vector <int> matching(n1 + n2, 0);
    for (int i = 0; i < n1; ++i) if (!flow.vis[i])
        cover[i] = true;
    for (int i = 0; i < n2; ++i) if (flow.vis[i + n1])
        cover[i + n1] = true;
    ll sum = 0;
    for (int i = 0; i < n1; ++i) for (auto e : flow.adj[i]) {
        if (!e.f) {
            int x = E[{i, e.v - n1}];
            matching[i] = x, matching[e.v] = x, sum += x;
        }
    }
    test(matching);
    test(cover);
    ...
    while (q--) {
        ...
        if (x == 1) {
            ...
            while (!cover[st])
                st++;
            cout << (st < n1 ? st + 1 : -(st - n1 + 1)) << endl;
            sum -= matching[st];
            cout << sum << endl;
            cover[st] = false;
        } else {
            cout << count(all(cover), true) << endl;
            for (int i = 0; i < n1 + n2; ++i) if (cover[i]) {
                cout << matching[i] << ' ';
            }
            cout << endl;
        }
