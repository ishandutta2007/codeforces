...
struct UnionFind {
    ...
    UnionFind(int n) : node(n), cnt(n), num(n) { data.assign(n, -1); }
...
int main() {
    ...
    for(auto [_, i, j] : v) {
        uf.unite(i, j);
        int root = uf.find(i);
        if(uf.size(root) * (uf.size(root) - 1) / 2 == uf.cnt[root]) {
            int now = si(g);
            g.eb(vi());
            sz.eb(uf.size(root) - 1);
            for(auto &e : uf.node[root]) g.back().eb(e);
            uf.node[root].clear();
            uf.node[root].eb(now);
        }
    }
