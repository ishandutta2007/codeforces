...
vector<int>sum(1);
for(int x=1;x<=n;x++)
sum.push_back(sum[x-1]+s[x]-'0');
int ans=0;
for(int x=1;x<B;x++)
{
    for(int y=0;y<=n;y++)
    a[sum[y]*x-y+n]++,st[++top]=sum[y]*x-y+n;
    while(top>=0){if(a[st[top]])ans+=1ll*a[st[top]]*(a[st[top]]-1)/2,a[st[top]]=0;top--;}
}
st[++top]=n+1;
for(int x=n;x;x--)
{
    if(s[x]-'0')st[++top]=x;
    auto top2=top;
    for(int y=1;y<B&&top2>=1;y++)
    ans-=max(0ll,(st[top2]-x)/y-B+1),top2--,ans+=max(0ll,(st[top2]-x)/y-B+1);
}
cout<<ans<<endl;
#endif
}
