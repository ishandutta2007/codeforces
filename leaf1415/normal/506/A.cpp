...
int main(void)
{
	...
	rep(i, 0, m) rep(j, -300, 300) dp[i][zero+j] = -inf;
	dp[d][zero] = c[d];
	
	rep(i, d, m){
		rep(j, -300, 300){
			rep(k, -1, 1){
				if(i+d+j+k <= m && d+j+k >= 1){
					chmax(dp[i+d+j+k][zero+j+k], dp[i][zero+j] + c[i+d+j+k]);
				}
			}
		}
	}
	...
	ll ans = 0;
	rep(i, d, m) rep(j, -300, 300) chmax(ans, dp[i][zero+j]);
	outl(ans);
