...
void tc() {
	...
	linc[0] = 1;
	fori (i, 1, n) {
		linc[i] = a[i] > a[i-1] ? linc[i-1] + 1 : 1;
	}
	rinc[n-1] = 1;
	for (int i = n-2; i >= 0; i--) {
		rinc[i] = a[i] < a[i+1] ? rinc[i+1] + 1 : 1;
	}
	int best = 1;
	if (n >= 2) {
		best = 2;
		maxify(best, linc[n-2] + 1);
		maxify(best, rinc[1] + 1);
		for (int i = 1; i < n - 1; i++) {
			if (a[i+1] - a[i-1] >= 2) maxify(best, linc[i-1] + rinc[i+1] + 1);
			else {
				maxify(best, linc[i-1] + 1);
				maxify(best, rinc[i+1] + 1);
			}
		}
	}
	printf("%d\n", best);
