...
struct Rin
{
    ...
    Rin&operator >>(char &x){for(x=gc();x>'z'||x<'a';x=gc());return *this;}
}rin;
...
{
    rin>>n;L=(n<<1)+1;
    for(int i=1;i<=n;i++)rin>>c[i];
    for(int i=1;i<=n;i++)d[i<<1]=c[i],d[(i<<1)+1]='|';d[1]='|';d[L+1]='#';
    
    l[1]=0;int r=1,lst=1;
    for(int i=2;i<=L;i++)
    {
        if(r>i)l[i]=min(r-i,l[(lst<<1)-i]);
        for(;d[i+l[i]+1]==d[i-l[i]-1];l[i]++);if(i+l[i]>r)lst=i,r=i+l[i];
        a[(i-l[i]+1)>>1]++;a[(i>>1)+1]--;
        b[(i+1)>>1]++;b[(i+l[i]+1)>>1]--;
        ans+=(l[i]+1>>1);if(ans>=M)ans-=M;
    }
    // for(int i=1;i<=L;i++)printf("%3c",d[i]);puts("");
    // for(int i=1;i<=L;i++)printf("%3d",l[i]);puts("");
    ans=prpr(prpr(ans,ans-1),25561994);
    for(int i=1;i<=n;i++){a[i]+=a[i-1];a[i]%=M;}//
    for(int i=1;i<=n;i++){b[i]+=b[i-1];b[i]%=M;}
    for(int i=n-1;i>=1;i--){a[i]+=a[i+1];a[i]%=M;}//
    for(int i=1;i<n;i++){ans-=1LL*b[i]*a[i+1]%M;if(ans<0)ans+=M;}
    printf("%d\n",ans);
    return 0;
}