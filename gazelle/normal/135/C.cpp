...
int main() {
    ...
    vector<string> ans;
    if(z + h >= o + 1) ans.pb("00");
    if(o + h >= z + 2) ans.pb("11");

    if((z >= o && h >= z - o) || (o >= z) && h >= o - z - 1) {
        bool ok_z = false, ok_o = false;

        if(z >= o) {
            if(h >= 0) ok_o = true;
            if(h - (z - o) >= 2) ok_z = true;
        }

        if(o > z) {
            if(h >= 0) ok_z = true;
            if(h - (o - z) >= 1) ok_o = true;
        }

        if(s[n - 1] == '0' || (s[n - 1] == '?' && ok_z)) ans.pb("10");
        if(s[n - 1] == '1' || (s[n - 1] == '?' && ok_o)) ans.pb("01");
    }
