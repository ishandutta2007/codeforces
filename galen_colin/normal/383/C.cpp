...
vi edges[200005];
int st[200005], en[200005], depth[200005];
int TIME = -1;
segtree<sum_t> sumt[2];
...
int main() {
  ...
  f0r(q, Q) {
    ...
    if (type == 1) {
      ...
      sumt[depth[x] % 2].update(st[x], en[x], y);
    } else {
      cin >> x;
      --x;

      cout << a[x] + sumt[depth[x] % 2].query(st[x], st[x]).val - sumt[(depth[x] + 1) % 2].query(st[x], st[x]).val << '\n'; 
    }
  }
}