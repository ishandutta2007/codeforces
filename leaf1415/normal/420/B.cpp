...
{
	set<ll> T = S;
	if(sz(T) && T.count(x) == 0) return false;
	rep(i, 1, m){
		if(c[i] == '+') T.insert(id[i]);
		else T.erase(id[i]);
		if(sz(T) && T.count(x) == 0) return false;
	}
	return true;
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, m) cin >> c[i] >> id[i];
	
	rep(i, 1, m){
		if(c[i] == '-' && !used[id[i]]) S.insert(id[i]);
		used[id[i]] = true;
	}
	...
	rep(i, 1, m){
		if(cnt == 0 && c[i] == '+'){
			if(check(id[i])) ans.pb(id[i]);
			break;
		}
		if(c[i] == '+') cnt++;
		else cnt--;
	}
	if(cnt == 0) if(check(id[m])) ans.pb(id[m]);
