...
fun main(){
    solve.cases{
        ...
        fun knowWhere(base:Int,delta:Int):Int?{
            if(delta == n) return null
        ...
        var currentdelta = 0
        val storedDelta = IntArray(n){-1}
        while(ans.any { it == -1 }){
            while(ans[basePointer] != -1){
                basePointer --
            }
            currentdelta ++
            val t = knowWhere(basePointer, currentdelta)
            if(t == null){
                ans[basePointer] = currentMin - currentdelta
                for((i,v) in storedDelta.withIndex()){
                    if(v != -1){
                        ans[i] = v + ans[basePointer]
                    }
                }
                currentMin = ans[basePointer]
                for(i in storedDelta.indices){
                    storedDelta[i] = -1
                }
                currentdelta = 0
                basePointer --
