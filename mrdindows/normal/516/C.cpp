...
void build(int cur, int l, int r){
	if (l == r){
		...
		build(dcur + 1, m + 1, r);
		tr[cur] = {
			max(tr[dcur].left + tr[dcur + 1].sum, tr[dcur + 1].left),
			max(tr[dcur].mid, tr[dcur + 1].mid, tr[dcur].left + tr[dcur + 1].right),
			max(tr[dcur + 1].right + tr[dcur].sum, tr[dcur].right),
			tr[dcur].sum + tr[dcur + 1].sum
		};
	}
}
Ans get_ans(int cur, int l, int r, int x, int y){
	//DBN(cur, l, r, x, y);
	if (l == x && r == y){
		return tr[cur];
	}
		...
		int dcur = cur + cur;
		if (y <= m){
			return get_ans(dcur, l, m, x, y);
		}
		else if (x > m){
			return get_ans(dcur + 1, m + 1, r, x, y);
		}
		else {
			Ans res1 = get_ans(dcur, l, m, x, m);
			Ans res2 = get_ans(dcur + 1, m + 1, r, m + 1, y);
			Ans res = {
				max(res1.left + res2.sum, res2.left),
				max(res1.mid, res2.mid, res1.left + res2.right),
				max(res2.right + res1.sum, res1.right),
				res1.sum + res2.sum
			};
			return res;
		}
...
int main()
{
	...
	cin >> n >> m;
	nn = n + n - 1;
	FI(n) cin >> d[i], d[i+n] = d[i];
	FI(n) cin >> h[i], h[i] *= 2, h[i+n] = h[i];
	h[n + n] = h[n];
	build(1, 0, nn);
	FI(m){
		int l, r;
