...
int main() {
    ...
    auto rank = [&](string a) {
        mn res = 0;
        vi ct(26); trav (c, a) ct[c - 'a']++;

        mn ways = mn::factorial(sz(a));
        trav (e, ct) ways *= mn::inverse_factorial(e);

        F0R (i, sz(a)) {
            ways *= mn::inverse_factorial(sz(a) - i) * mn::factorial(sz(a) - i - 1);
            F0R (j, a[i] - 'a') if (ct[j]) {
                res += ways * mn::factorial(ct[j]) * mn::inverse_factorial(ct[j] - 1);
            }
            ways *= mn::factorial(ct[a[i] - 'a']) * mn::inverse_factorial(ct[a[i]-'a']-1);
            --ct[a[i] - 'a'];
        }
    ...
    // count permutations of a larger than a and smaller than b
    string a, b; re(a, b);

    const int N = sz(a);

    // find the greatest permutation of a which is less than b
    string last(N, '?');
    vi ct(26); trav (c, a) ct[c - 'a']++;
    int i = 0; while (i < N && ct[b[i] - 'a']) last[i] = b[i], ct[b[i] - 'a']--, i++;

    auto have_smaller = [&](char v) {
        F0R (i, v - 'a') if (ct[i]) return true;
        return false;
    };
    while (i >= 0 && (i == N || !have_smaller(b[i]))) {
        --i;
        if (i >= 0) ct[b[i] - 'a']++;
    }
    if (i == -1) { ps(0); return 0; }

    F0Rd (j, b[i] - 'a') if (ct[j]) { last[i] = 'a' + j; --ct[j]; break; }
    i++;

    int j = 25;
    while (i < N) { while (!ct[j]) --j; last[i++] = 'a' + j; --ct[j]; }

    if (last < a) ps(0);
    else ps(rank(last) - rank(a));
