...
int main(){
	...
	vector<long long int> Sas;
	for (int i = 0; i < indices.size() - 1; ++i){
		Sas.push_back(abs(indices[i+1]-indices[i]));
	}
	...
	for (int i = 0; i < q; ++i){
		...
		long long int ans = Sas[a];
		long long int nowsum = Sas[a];
		aa.emplace_back(make_pair(a - 1, 99999999999));
		aa.emplace_back(make_pair(a, Sas[a]));
		for (int i = a + 1; i < b; ++i){
			while (Sas[i]>aa.back().second){
				pair<long long int, long long int> t(aa.back());
				aa.pop_back();
				nowsum -= (t.first - aa.back().first)*t.second;
			}
			nowsum += (i - aa.back().first)*Sas[i];
			aa.emplace_back(make_pair(i, Sas[i]));
			ans += nowsum;
		}
		cout << ans << endl; 
	}
