...
vl person,happ,tot,good,bad;
vvi adj;
bool answer=true;
void dfs0(int x,int parent=-1)
{
    tot[x]=person[x];
    for(auto node:adj[x])
    {
...
{
    if(good[x]<0||bad[x]<0||good[x]>tot[x]||bad[x]>tot[x]||good[x]+bad[x]!=tot[x])
        answer=false;
    if(!answer) return;
    ll good1=0;
    ll bad1=0;
    ...
    {
        if(node==parent) continue;
        if(abs(happ[node]%2)==abs(tot[node]%2))
        {   
            good[node]=(tot[node]+happ[node])/2;
            bad[node]=(tot[node]-happ[node])/2;
            good1+=good[node];
            bad1+=bad[node];
        }   
        else answer=false;
...
int main()
{
    ...
    TT
    {
        ...
        int n,m;
        cin>>n>>m;
        person=happ=tot=good=bad=vl(n,0);
        adj=vvi(n);
        ...
        dfs0(0);
        if(abs(tot[0]%2)!=abs(happ[0]%2)) answer=false;
        good[0]=(tot[0]+happ[0])/2;
        bad[0]=(tot[0]-happ[0])/2;
        dfs1(0);
        if(answer) cout<<"YES"<<'\n';
        else cout<<"NO"<<'\n';
