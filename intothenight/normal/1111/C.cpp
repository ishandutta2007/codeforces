...
int main(){
	...
	auto solve = [&](int l, int r, auto &solve)->ll{
		int pl = lower_bound(all(c), l) - c.begin();
		int pr = lower_bound(all(c), r) - c.begin();
		if(l + 1 == r){
			if(pl == pr) return 1LL * a;
			else return 1LL * b * (pr - pl);
		}
		int m = l + r >> 1;
		if(pl == pr) return 1LL * a;
		else return min(solve(l, m, solve) + solve(m, r, solve), 1LL * b * (pr - pl) * (r - l));
	};
	cout << solve(0, 1 << n, solve);
	return 0;
}
