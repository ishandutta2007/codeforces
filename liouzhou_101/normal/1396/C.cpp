...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		s[i] = s[i - 1] + min(a[i - 1] * r1 + r1, r2) + r1;
	}
	t[n] = min(a[n - 1] * r1 + r3, r2 + d + d + r1);
	for (int i = n - 1; i >= 1; --i)
	{
		t[i] = t[i + 1] + min(a[i - 1] * r1 + min(r1 + r1, r3), r2 + r1);
	}
	...
	for (int i = 1; i <= n; ++i)
	{
		f[i] = f[i - 1] + d + min(a[i - 1] * r1 + r3, r2 + d + d + r1);
		if (i - 1 - 1 >= 0)
		{
			int j = i - 1;
			freshmin(tmp, f[j - 1] - s[j - 1] - 3LL * j * d);
		}
		freshmin(f[i], tmp + d + 3LL * i * d + s[i]);

		freshmin(res, f[i - 1] + d + t[i] + 2LL * (n - i) * d);
	}
	freshmin(res, f[n]);
	writeln(res);
}
