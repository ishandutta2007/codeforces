...
vector<int> v[MAXN];
int u[MAXN], w[MAXN];
int L[MAXN], R[MAXN], F[MAXN], G[MAXN];
...
void solve()
{
	...
	for (int i = 1; i <= m; ++ i)
	{
		L[i] = v[i].front(), R[i] = v[i].back();
	}
	
	for (int i = 1; i <= m; ++ i)
	{
		int x = L[i], y = R[i];
		if (i > 1 && R[i-1] < L[i]) F[i] = F[i-1]+1; else F[i] = 1;
	}
	
	for (int i = m; i >= 1; -- i)
	{
		int x = L[i], y = R[i];
		if (i < m && R[i] < L[i+1]) G[i] = G[i+1]+1; else G[i] = 1;
	}
	
	int ans = m-1;
	for (int i = 1; i <= m; ++ i)
	{
		freshmin(ans, m-(F[i]+G[i]-1));
	}
	writeln(ans);
}
