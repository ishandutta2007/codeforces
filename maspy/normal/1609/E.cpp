...
void solve() {
  ...
  */
  ...
  FOR(i1, S) FOR(i2, S) {
    ...
    ll b3 = b1 | b2;
    ll c3 = c1 | c2;
    ll ab3 = ab1 | ab2 | (a1 & b2);
    ll bc3 = bc1 | bc2 | (b1 & c2);
    ll abc3 = abc1 | abc2 | (a1 & bc2) | (ab1 & c2);

    ll s3 =
      (a3 << 0) | (b3 << 1) | (c3 << 2) | (ab3 << 3) | (bc3 << 4) | (abc3 << 5);
    ll i3 = LB(state, s3);
    assert(state[i3] == s3);
    TO[i1][i2] = i3;
  }

  using E = array<int, 14>;

  auto char_state = [&](char x) -> E {
    E e;
    FOR(k, S) e[k] = INF;
    e[1] = e[2] = e[4] = 1;
    e[1 << (x - 'a')] = 0;
  ...
  SegTree<E> seg(
    [&](E x, E y) {
      ...
      FOR(i, S) {
        if (x[i] == INF) continue;
        FOR(j, S) chmin(z[TO[i][j]], x[i] + y[j]);
      }
      return z;
    },
  ...
  STR(A);
  vc<E> seg_raw(N);
  FOR(i, N) { seg_raw[i] = char_state(A[i]); }
  ...
  FOR(_, Q) {
    ...
    --p;
    CHR(c);
    seg.set(p, char_state(c));

    E e = seg.fold(0, N);
    ll ANS = INF;
    FOR(k, S - 1) chmin(ANS, e[k]);
    print(ANS);
  }
}
