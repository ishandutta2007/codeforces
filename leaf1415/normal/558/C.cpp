...
int main(void)
{
	...
	rep(i, 1, n){
		chmin(mn, sz(s[i]));
		reps(j, s[i]) if(s[i][j] == '0') zero[j] = true;
	}
	...
	rep(i, 1, n){
		ll p = -1;
		reps(j, s[i]){
			if(s[i][j] == '1' && (zero[j] || j >= mn)){
				p = j;
				break;
			}
		}
		if(p != -1) ans += sz(s[i]) - p, s[i].resize(p);
	}
	
	vector<ll> vec;
	rep(i, 1, n) vec.push_back(sz(s[i]));
	sort(all(vec));
	
	for(auto x : vec) ans += abs(vec[sz(vec)/2] - x);
	outl(ans);
