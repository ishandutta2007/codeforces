...
const int mod = 119 << 23 | 1, nax = 1010;
unt dp[nax][nax * 2][2]; //0 - takie same; 1 - rne
void add(unt &a, unt b) {
	a += b;
	if (a >= mod) a -= mod;
}
int main() {
	...
	scanf("%d%d", &n, &k);
	dp[1][1][0] = dp[1][2][1] = 1;
	FOR(i, 1, n - 1) FOR(j, 1, 2 * i) {
		debug << arr3(dp, i, j, 0) arr3(dp, i, j, 1);
	...
	debug << arr3(dp, n, k, 0) arr3(dp, n, k, 1);
	printf("%u\n", (dp[n][k][1] + dp[n][k][0]) * 2 % mod);
}