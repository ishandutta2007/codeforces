...
int main() {
    ...
    vll olis;
    rep(i, N) {
        if (a[i])olis.push_back(i);
    }
    olis.push_back(N);
    ll M = olis.size();
    ll L = N * (N - 1) / 2;
    vvvll dp(N + 2, vvll(M + 1, vll(L + 1, INF)));
    dp[0][0][0] = 0;
    repn(i, N + 1)repn(j, min(M,i))rep(k, min(L,N*j)+1) {
        ll mv = abs(i - 1 - olis[j - 1]);
        if (k >= mv) {
            rep(l, i) {
                ll now = dp[l][j - 1][k - mv] + (i - l - 1) * (i - l - 2) / 2;
