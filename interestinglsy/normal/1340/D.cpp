...
void Dfs( int pos , int fa , int arriveT , int backT ){
	ansl.Pb({pos,arriveT});
	if( e[pos].size() == 1 and fa ){
		assert( backT < arriveT );
		ansl.Pb({pos,backT});
		return;
	}
	int sons = e[pos].size()-1;
	if( backT >= sons ){
		ansl.Pb({pos,backT-sons});
		int nowT = backT-sons;
		for( int v : e[pos] ){
			if( v == fa ) continue;
			Dfs(v,pos,nowT+1,nowT);
			++nowT;
			ansl.Pb({pos,nowT});
		}
		assert( nowT == backT );
...
int main(){
	...
	For(i,n) Mymax(ans,(int)e[i].size());
