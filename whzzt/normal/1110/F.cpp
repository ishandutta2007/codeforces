...
namespace DS{
	i64 mv[N<<2],sv[N<<2];
	void modify(int u,int l,int r,int s,int t,i64 v){
		if(l==s&&r==t){mv[u]+=v,sv[u]+=v;return;}
		int mid=(l+r)>>1,ls=u<<1,rs=ls|1;
		if(t<=mid) modify(ls,l,mid,s,t,v);
		else if(s>mid) modify(rs,mid+1,r,s,t,v);
		else modify(ls,l,mid,s,mid,v),modify(rs,mid+1,r,mid+1,t,v);
		mv[u]=min(mv[ls],mv[rs])+sv[u];
	}
	i64 ask(int u,int l,int r,int s,int t){
		if(l==s&&r==t)return mv[u];
		int mid=(l+r)>>1,ls=u<<1,rs=ls|1;
		if(t<=mid) return ask(ls,l,mid,s,t)+sv[u];
		if(s>mid) return ask(rs,mid+1,r,s,t)+sv[u];
		return min(ask(ls,l,mid,s,mid),ask(rs,mid+1,r,mid+1,t))+sv[u];
	}
}
using namespace DS;
int n,q,fa[N],fd[N],ldfn[N],rdfn[N];i64 res[N];
i64 dis[N];
struct no{int u,l,r,t;};
vector<no>op[N];
vector<int>go[N];
void dfs(int u){
	for(auto o:op[u])res[o.t]=ask(1,1,n,o.l,o.r);
	for(int v:go[u]){
		modify(1,1,n,1,n,fd[v]);
		modify(1,1,n,ldfn[v],rdfn[v],-fd[v]*2);
		dfs(v);
		modify(1,1,n,ldfn[v],rdfn[v],fd[v]*2);
		modify(1,1,n,1,n,-fd[v]);
	}
...
int main(){
	...
	scan(n,q);
	ldfn[1]=1,rdfn[1]=n;
	for(i=2;i<=n;++i)scan(fa[i],fd[i]),ldfn[i]=rdfn[i]=i,dis[i]=dis[fa[i]]+fd[i];
	for(i=n;i>=2;--i)go[fa[i]].pb(i),chkmin(ldfn[fa[i]],ldfn[i]),chkmax(rdfn[fa[i]],rdfn[i]);
	for(i=1;i<=n;++i){
		if(ldfn[i]==rdfn[i])modify(1,1,n,i,i,dis[i]);
		else modify(1,1,n,i,i,1ll<<60);
	}
	for(i=1;i<=q;++i){
		no k;
		scan(k.u,k.l,k.r),k.t=i;
		op[k.u].pb(k);
	}
	dfs(1);
	for(i=1;i<=q;++i)print(res[i]),putc('\n');
}