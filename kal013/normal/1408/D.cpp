...
void solve(){
	int n, m;
	cin >> n >> m;
	vector<pair<int,int>> rob(n), sea(m);
	...
	for(auto e: sea){
		while(fin.size() > 0 && fin.back().second <= e.second){
			assert(fin.back().first <= e.first);
			fin.pop_back();
		}
		fin.push_back(e);
	}
