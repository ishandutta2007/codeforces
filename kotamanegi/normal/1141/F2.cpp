...
void solve() {
    ...
    vector<pair<int, int>> ans;
    map<int, vector<pair<int, int>>> targets;
    REP(i, n) {
        int target = 0;
        for (int q = i; q >= 0; --q) {
            target += inputs[q];
            targets[target].push_back(mp(i, q));
        }
    }
    for (auto x : targets) {
        int now = 0;
        ...
        for (int q = 0; q < x.second.size(); ++q) {
            if (now <= x.second[q].second) {
                nexts.push_back(mp(x.second[q].second, x.second[q].first));
                now = x.second[q].first + 1;
            }
        }
        if (ans.size() < nexts.size()) {
            ans = nexts;
        }
    }
    cout << ans.size() << endl;
