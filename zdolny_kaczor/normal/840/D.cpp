...
#endif
const int K = 4;
using res = array<pii, K>;
res mer(res a, const res &b) {
	debug(imie(a), imie(b));
	for (auto [vb, cb] : b) {
		if (!cb) continue;
		bool is = false;
		...
		if (!is) {
			...
			REP(i, K) mini(m, a[i].second);
			if (m >= cb) REP(i, K) a[i].second -= cb;
			else {
				bool did = false;
				REP(i, K) {
					a[i].second -= m;
...
const int nax = 1 << onl(19)loc(3);
vector <int> occs[nax];
int cou(int x, int l, int r) {
	return lower_bound(ALL(occs[x]), r) - lower_bound(ALL(occs[x]), l);
}
int a[nax];
res tree[nax * 2];
res query(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
	if (l >= rb || r <= lb) return {};
	if (l <= lb && r >= rb) {
		debug(range(a, lb, rb), arr(tree, wh));
		return tree[wh];
	}
	int sb = (lb + rb) / 2;
	return mer(query(l, r, wh * 2, lb, sb), query(l, r, wh * 2 + 1, sb, rb));
}
int main() {
	int n, q;
	scanf("%d%d", &n, &q);
	...
	REP(i, n) tree[i + nax][0] = {a[i], 1};
	FORD(i, nax - 1, 1) tree[i] = mer(tree[i * 2], tree[i * 2 + 1]);
	REP(_, q) {
		int l, r, k;
		scanf("%d%d%d", &l, &r, &k);
		l--;
		res cans = query(l, r);
		debug(range(a, l, r), imie(cans));
		int ans = -1;
		for (auto [x, _] : cans) {
			if ((ans == -1 || ans > x) && cou(x, l, r) > (r - l) / k)
				ans = x;
		}
		printf("%d\n", ans);
	}
