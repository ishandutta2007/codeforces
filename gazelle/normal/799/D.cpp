...
int solve(int h, int w, int n, vector<ll> & x) {
    if(h == 1 && w == 1) return 0;

    ll min_a = inf * inf;
    __int128 acm = 1;
    vector<bool> ok(h + 1, false);
    ...
    for(int i = n - 1; i >= 0; --i) {
        acm *= x[i];
        if(acm >= inf * inf) break;

        for(ll j = h; j >= 0; --j) {
            if(not ok[j]) continue;
            if(j * x[i] >= h) min_a = min(min_a, j * x[i]);
            else              ok[j * x[i]] = true;
        }
        
        if(min_a != inf * inf && acm / min_a >= w) return n - 1 - i + 1;
    }
...
int main() {
    ...
    int ans1 = solve((h - 1) / a + 1, (w - 1) / b + 1, n, x);
    int ans2 = solve((h - 1) / b + 1, (w - 1) / a + 1, n, x);
