...
void ins(set<int> &v, int x)
{
    for (auto it: v)
    {
        if (isless(it, x)) wins[x]++;
        else wins[it]++;
    }
    v.insert(x);
}

void ers(set<int> &v, int x)
{
    for (auto it: v) if (it!=x)
    {
        if (isless(it, x)) wins[x]--;
        else wins[it]--;
    }
...
void solve()
{
    ...
    b.clear();
    wins = vector<int>(2*n);
    for (int i = 0; i<n-1; i++) ins(a, i);
    ins(b, n-1);

    for (int i = n; i<2*n; i++)
    {
        if (a.size()<b.size()) ins(a, i);
        else ins(b, i);
        int bestA = *a.begin();
        int bestB = *b.begin();
        for (auto it: a) if (wins[it]<wins[bestA]) bestA = it;
        for (auto it: b) if (wins[it]<wins[bestB]) bestB = it;
        if (isless(bestA, bestB)) ers(a, bestA);
        else ers(b, bestB);
    }
