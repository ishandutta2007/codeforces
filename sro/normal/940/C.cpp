...
int main()
{
	cin>>n>>m;
	cin>>s;
	for(int i=0;i<n;i++)vv.pb(s[i]);
	sort(vv.begin(),vv.end());
	if(SZ(s)<m)
	{
		t=s;
		for(int i=SZ(s);i<m;i++)t+=vv[0];
		cout<<t<<endl;
		return 0;
	}
	else if(SZ(s)==m)
	{
		...
		while(1)
		{
			if(s[SZ(s)-k]==vv[SZ(vv)-1])
			{
				t[SZ(s)-k]=vv[0];
				k++;
				continue;
			}
			int bb=upper_bound(ALL(vv),t[SZ(s)-k])-vv.begin();
			t[SZ(s)-k]=vv[bb];
			break;
		}
		cout<<t<<endl;
	...
	else
	{
		...
		while(1)
		{
			...
			int bb=upper_bound(ALL(vv),t[m-k])-vv.begin();
			t[m-k]=vv[bb];
			break;
		}
		cout<<t<<endl;
