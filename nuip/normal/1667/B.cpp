...
int minf(vector<int> &bit, int i){ ++i;
  int s=MOD;
  while(i>0){
		MN(s,bit[i]);
    i-=i&-i;
...
void add(vector<int> &bit, int i,int x){ ++i; ++i;
  while(i<bit.size()){
		MN(bit[i],x);
    i+=i&-i;
  }
...
int main(){
	...
	TT{
		...
		cauto zip=sums-sorted-distinct;
		vector<int> bit(n+10,MOD);
		add(bit,lower_bound(all(zip),0)-zip.begin(),0);
		int prv=0;
		rep(i,n){
			cauto &sum=sums[i+1];
			cauto si=lower_bound(all(zip),sum)-zip.begin();
			int mn=MOD;
			MN(mn,prv+(vs[i]>0?0:vs[i]?2:1));
			out(i,prv,si,1);
			MN(mn, minf(bit,si));
			add(bit,si,mn);
			out(i,mn,bit,1);
