...
using F= int;
F op_min(F a,F b){return min(a,b);}
F e_min(){return INF;}
int target;
bool f_min(F x){
	return x>=target;
...
F op_max(F a,F b){return max(a,b);}
F e_max(){return -1;}
bool f_max(F x){
	return x<=target;
}
...
void solve(){
	...
	vector<int> p(N);
	vector<int> rev(N+1);
	rep(i,N) cin>>p[i],rev[p[i]]=i;
	segment_tree<F,op_min,e_min> seg_min(p);
	segment_tree<F,op_max,e_max> seg_max(p);
	vector<int> dp(N,INF);
	dp[0]=0;
	rep(i,N-1){
		chmin(dp[i+1],dp[i]+1);
		target=p[i];
		if(p[i]<p[i+1]){
			int b=seg_min.max_right<f_min>(i);
			int v=seg_max.query(i,b);
			chmin(dp[rev[v]],dp[i]+1);
		}else{
			int b=seg_max.max_right<f_max>(i);
			int v=seg_min.query(i,b);
			chmin(dp[rev[v]],dp[i]+1);
		}
	}
	cout<<dp[N-1]<<"\n";
