...
void main_() {
	...
	auto slv = [&](V<pii> vec, bool p) -> ll {
		if(vec.empty()) return 0ll;
		foa(v, vec) debug(v);
		ll res = 0;
		V<> lefd(len, inf), lefu(len, -inf), rigd(len, inf), rigu(len, -inf);
		for(auto [x, y] : vec) {
			chmax(lefu[x], y);
			chmin(lefd[x], y);
			chmax(rigu[x], y);
		...
		REP(i, len - 1) {
			chmax(lefu[i + 1], lefu[i]);
			chmin(lefd[i + 1], lefd[i]);
		}
		RREP(i, len - 1) {
			chmax(rigu[i], rigu[i + 1]);
			chmin(rigd[i], rigd[i + 1]);
		}
		REP(i, 1 + p, len - 1, 2) {
			int lower = max(lefd[i - 1], rigd[i + 1]);
			int upper = min(lefu[i - 1], rigu[i + 1]);
			if(lower > upper) continue;
			res += (upper - lower) / 2;
		}
		return res;
	};
	print(slv(odd, 1) + slv(even, 0));
}