...
#define stack __stack_
int hand[nax], stack[nax];
int reach[nax];
int main() {
	int n;
	scanf("%d", &n);
	REP(i, n) scanf("%d", hand + i);
	REP(i, n) scanf("%d", stack + i);
	reverse(stack, stack + n);
	reverse(hand, hand + n);
	REP(i, n) if (stack[i]) stack[i] = n - stack[i] + 1;
	REP(i, n) if (hand[i]) hand[i] = n - hand[i] + 1;
	REP(i, n) if (stack[i]) reach[stack[i]] = n - i;
	REP(i, n) reach[hand[i]] = 0;
	int beg = stack[0];
	FOR(i, 1, n) debug << arr(reach, i);
	if (beg) {
		bool matches = beg;
		REP(i, n - beg + 1)
			if (stack[i] != beg + i) {
				debug << "mismatch on" arr(stack, i);
				matches = false;
				break;
			}
		FOR(i, 1, beg - 1)
			if (reach[i] >= beg - i) {
				debug << "can't reach " arr(reach, i);
				matches = false;
				break;
			}
		if (matches) return 0 * printf("%d\n", beg - 1);
	}
	int ans = 0;
	FOR(i, 1, n) {
		debug << arr(reach, i) imie(reach[i] + i);
		maxi(ans, reach[i] + i);
	}
	printf("%d\n", ans);
}