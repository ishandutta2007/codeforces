...
{   //*
    ...
    if (p == c.p)
    {
        circle ans = circle(point(c.p.x + (r / 2), c.p.y), c.r / 2);
        cout << ans;
        return 0;
    }
    line l = line_point(p, c.p);
    pair<point, point> t = intersection_circle_line(c, l);
    if (length(t.first, p) < length(t.second, p))
        swap(t.first, t.second);
    point u = t.first;
    circle ans = circle(otnosh(u, p, 1, 1), length(p, u) / 2);
    cout << ans;
    return 0;
}