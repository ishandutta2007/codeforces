...
public:
	...
	// _update:
	SegmentTree(size_t _n, Monoid _def, F _operation, F _update)
...
void main_() {
	...
	S id;
	REP(i, m) { id[i] = i; }
	auto op = [&](const S a, const S b) {
		S ret;
		REP(i, m) { ret[i] = b[a[i]]; }
		return ret;
	};
	auto update = [&]([[maybe_unused]] const S x, const S y) { return y; };
	SegmentTree<S> seg(q, id, op, update);

	VV<int> event_start(n + 1), event_end(n + 1);
	V<pii> query(q);
	REP(i, q) {
		INT(l, r);
		cin >> query[i];
		event_start[--l].push_back(i);
		event_end[r].push_back(i);
	}

	REP(i, n) {
		for(auto &v : event_start[i]) {
			auto c = id;
			auto [x, y] = query[v];
			c[x - 1] = y - 1;
			seg.set(v, c);
		}
		for(auto &v : event_end[i]) seg.set(v, id);
		cout << seg.query(0, q)[a[i]] + 1 << " ";
	}
	cout << endl;
}
