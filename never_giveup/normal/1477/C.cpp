...
const int maxn = 2e5 + 100, maxw = 7 * maxn, inf = 2e9 + 100, sq = 450, LG = 20, mod1 = 1e9 + 993;
...
bool les(int a, int b, int c) {
    auto x = dif(pt[b], pt[a]), y = dif(pt[b], pt[c]);
    return dp(x.fr, x.sc, y.fr, y.sc) > 0;
}

bool check(vector<int> a) {
    for (int i = 1; i + 1 < a.size(); i++)
        if (!les(a[i - 1], a[i], a[i + 1]))
            return 0;
    return 1;
}
...
void go(int it) {
    ...
    for (int i = 0; i < 5; i++) {
        int pos = rnd() % (n - it);
        //swap(rem[pos], rem[n - it - 1]);
        if (it > 1 && !les(p[it - 2], p[it - 1], rem[pos])) {
        } else {
            p[it] = rem[pos];
            swap(rem[pos], rem[n - it - 1]);
            go(it + 1);
...
void solve() {
    ...
    go(0);
    //while ((clock() - st) / (double)CLOCKS_PER_SEC <= 1.5) {
    /*while (1) {
        vector<int> p(n);
