...
int main(){
	...
	FOR(_, q){
		...
		VI pref(27);
		int odd = -1;
		REP(i, 1, 27) pref[i] = tree[i-1].query(a, b);
		REP(i, 1, 27) if(pref[i]&1){
			if(odd != -1){
				odd = -2;
				break;
			}
		...
		if(odd == -2 || (odd != -1 && (b-a+1)%2 == 0)) continue;
		FOR(i, 26) tree[i].update(a, b, 0);
		if(odd != -1) pref[odd+1]--;
		REP(i, 1, 27) pref[i]>>=1;
		...
		if(odd != -1) tree[odd].update((a+b)/2, (a+b)/2, 1);
	}
	std::string ans;
