...
void solve(){
  ...
  for(ll i=1;i*i<=n-1;i++){
    if((n-1)%i==0){
      d.pb(i);
      if(i*i!=(n-1)) d.pb((n-1)/i);
    }
  }
  vector<ll> solve(n+1);
  solve[1]=modPow(2,n-1,mod);
  for(auto e:d){
    if(e==1) continue;
    ll k=dfs(0,-1,e);
    if(ans[e]==0){
  ...
  for(int i=1;i<=n;i++){
    for(int j=i*2;j<=n;j+=i) u[j].pb(i);
  }
  for(int i=n;i>=1;i--){
    for(auto e:u[i]){
     //cout<<i<<" "<<e<<" "<<solve[e]<<" "<<solve[i]<<endl;
      
      (solve[e]+=mod-solve[i])%=mod;
    }
  }
  for(int i=1;i<=n;i++){
    cout<<solve[i]<<" ";
