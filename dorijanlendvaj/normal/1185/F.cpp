...
int main()
{
	...
	{
		int z=read();
		for (int i=0;i<(1<<9);++i) if ((i&z)==z) ++nu[i];
	}
	for (int i=0;i<m;++i)
	{
		int c;
	...
	int w=1<<9;
	for (int i=0;i<w;++i) sort(pr[i].begin(),pr[i].end());
	int x=-1,y=-1,c=MOD*2,p=-1;
