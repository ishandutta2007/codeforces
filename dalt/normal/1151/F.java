...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Matrix t = new Matrix(numCounts[0] + 1, numCounts[0] + 1);
            for (int j = 0; j <= numCounts[0]; j++) {
                t.mat[j][j] += numCounts[0] * (numCounts[0] - 1) / 2;
                t.mat[j][j] += numCounts[1] * (numCounts[1] - 1) / 2;
                t.mat[j][j] += (numCounts[0] - j) * (numCounts[1] - numCounts[0] + j);
                t.mat[j][j] += j * (numCounts[0] - j);
                if (j + 1 <= numCounts[0]) {
                    t.mat[j][j + 1] = (j + 1) * (numCounts[1] - (numCounts[0] - j - 1));
                }
                if (j - 1 >= 0) {
                    t.mat[j][j - 1] = (numCounts[0] - j + 1) * (numCounts[0] - j + 1);
                }
            }
            Matrix state = new Matrix(numCounts[0] + 1, 1);
            int prev = 0;
            ...
            Matrix finalState = Matrix.mul(Matrix.pow(t, k), state);
            int validCount = finalState.mat[numCounts[0]][0];
            int total = Mathematics.pow(n * (n - 1) / 2, k, mod);
            int possibilities = mod((long)validCount * Mathematics.inverse(total, mod));

            io.cache.append(possibilities);
        }
