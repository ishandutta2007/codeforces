...
void solve(){
	...
	int mex = n;
	while(mex >= 0){
		// all elems bigger than or eq to mex to be removed kinda
		while(sz(z) && z.back().fr >= mex){
			int e = z.back().fr;
			int freq = z.back().sc;
			int r = rank[e];
		...
		// mex should be possible also
		int req = mex - vals.ook(mex); // how many are needed
		// cout << mex << " " << req << "\n";
		if(req <= k){
			// mex possible
			int tmp = t.descent_right(0,k);
			// till tmp can't, but till tmp-1 can
			int rem_classes = ranks.ook(tmp);
			
			int dist = sz(v) - rem_classes + req;
			
			// cout << mex << " " << tmp << " " << rem_classes << "\n";
			amin(ans, dist - mex);
		}
