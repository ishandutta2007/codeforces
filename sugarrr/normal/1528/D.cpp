...
signed main(){fastio
    ...
    rep(i,0,n-1){
        vl dis(n,inf);
        dis[i]=0;
        pqg<l_l>q;q.push({dis[i],i});
        while(!q.empty()){
            l_l t=q.top();q.pop();
            if(dis[t.se] != t.fi)continue;
            if(dis[t.se] >0){
                ll po=t.se;
                ll nx=po+1;if(nx>=n)nx-=n;
                ll nxval=dis[po]+1;
                while(1){
                    if(chmin(dis[nx],nxval)){
                        q.push({dis[nx],nx});
                    }else if(nx==i){
                    ...
                    po=nx;
                    nx=po+1;if(nx>=n)nx-=n;
                    nxval++;
                }
            }
            for(auto P:v[t.se]){
                ll pos=(P.fi+t.fi)%n;
                if(chmin(dis[pos],P.se+t.fi)){
                    q.push({dis[pos],pos});
                }
                if(t.se == pos || pos == i){
                    pos++;if(pos>=n)pos-=n;
                    if(chmin(dis[pos],P.se+t.fi+1)){
                        q.push({dis[pos],pos});
                    }
                }
            }
        }
        rep(j,0,n-1)cout<<dis[j]<<" ";cout<<endl;
    }
