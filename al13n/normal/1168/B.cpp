...
LL doit(){
  for(int k=1;k*2<sz(S);++k) if(S.back()==S[sz(S)-1-k] && S.back()==S[sz(S)-1-k-k]) return 0;
  S+='0';
  LL r=doit();
  S.back()='1';
  r+=doit();
...
int main(int argc, char **argv){
  ...
  //doit(); // output: 00100111

  string S;
  cin>>S;
  LL ans=0;
  forv(a,S){
    int b=min(sz(S)+1,a+3);
    while(b<=sz(S)){
      bool f=false;
      for(int k=1;k*2<b-a;++k){
        if(S[b-1]==S[b-1-k] && S[b-1]==S[b-1-k-k]){
          f=true;
          break;
        }
      }
