...
void MAIN(){
	...
	for(int i=1;i<=n;++i){
		for(int j=1;j<=3;++j) A[i][j]=B[i][j]=0;
		for(int j=1;j<=n;++j){
			A[i][4]=B[i][4]=j;
			for(int k=3;k>=1;--k){
				if(dis[i][A[i][k]]<dis[i][A[i][k+1]]){
					swap(A[i][k],A[i][k+1]);
				}
				if(dis[B[i][k]][i]<dis[B[i][k+1]][i]){
					swap(B[i][k],B[i][k+1]);
				}
			}
		}
	...
	for(int i=1;i<=n;++i){
		for(int j=1;j<=n;++j){
			if(i==j) continue;
			for(int x=1;x<=3;++x){
				if(B[i][x]==i||B[i][x]==j) continue;
				for(int y=1;y<=3;++y){
					if(A[j][y]==i|A[j][y]==j||A[j][y]==B[i][x]) continue;
					if(ans[0]<dis[B[i][x]][i]+dis[i][j]+dis[j][A[j][y]]){
						ans[0]=dis[B[i][x]][i]+dis[i][j]+dis[j][A[j][y]];
						ans[1]=B[i][x];
						ans[2]=i;
						ans[3]=j;
						ans[4]=A[j][y];
	...
	for(int i=1;i<=4;++i) cout<<ans[i]<<" ";cout<<endl;
	return;
}
/*
*/
