...
const int nax = 1 << 17;
ll rsum[nax], csum[nax];
sim> ll k_lar(c beg, c end, int k) {
	debug(range(beg, end), imie(k));
	vector <ll> v(beg, end);
...
int main() {
	...
	REP(i, n) REP(j, m) scanf("%d", &board[i][j]);
	if (n <= 4 || m <= 4) {
		ll sum = 0;
		REP(i, n) REP(j, m) sum += board[i][j];
		printf("%lld\n", sum);
		return 0;
	}
	for (int i = 0; i < n; ++i) {
	...
	vector <pair <ll, pii> > crosses;
	crosses.reserve(m * n);
	REP(i, n) REP(j, m) {
		rsum[i] += board[i][j];
		csum[j] += board[i][j];
	}
	REP(i, n) REP(j, m) crosses.emplace_back(rsum[i] + csum[j] - board[i][j], make_pair(i, j));
	sort(ALL(crosses), greater<urs(crosses[0])>());
	ll ans = max(k_lar(rsum, rsum + n, 4), k_lar(csum, csum + m, 4));
	REP(i, n) {
		vector <ll> vals(m);
		REP(j, m) vals[j] = csum[j] - board[i][j];
		maxi(ans, k_lar(ALL(vals), 3) + rsum[i]);
	}
	REP(j, m) {
		vector <ll> vals(n);
		REP(i, n) vals[i] = rsum[i] - board[i][j];
		maxi(ans, k_lar(ALL(vals), 3) + csum[j]);
	}
	for (int i = 0; i < n * m && clock() < 0.44 * CLOCKS_PER_SEC; ++i) {
		int go = n * m / max(1, i - 10);
		for (int j = i + 1; j < go; ++j) {
			if (crosses[i].second.first == crosses[j].second.first || crosses[i].second.second == crosses[j].second.second) continue;
			ll val = crosses[i].first + crosses[j].first - board[crosses[i].second.first][crosses[j].second.second] - board[crosses[j].second.first][crosses[i].second.second];
			maxi(ans, val);
			if (crosses[i].first + crosses[j].first <= ans) break;
		}
	}
	printf("%lld\n", ans);
}