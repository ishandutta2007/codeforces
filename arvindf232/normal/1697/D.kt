...
fun main(){
    solve.cases{
        ...
        var difcount = 1
        val lastoccu = IntArray(26){-1}
        lastoccu[0] = 0
        for(r in 1 until n){
            val types = (0 until difcount).map { Pair(it,lastoccu[it]) }.sortedByDescending { it.second }
            val t = BinarySearchLastTrue(0,types.lastIndex){ask(types[it].second, r) == 2 + it} ?: -1
            var new = t + 1
            if(new == difcount){
                difcount ++
            }else{
        ...
        val who = CharArray(26)
        for(i in 0 until difcount){
            println("? 1 ${lastoccu[i] + 1 }")
            System.out.flush()
            val ans = getstr[0].toChar()
            who[i] = ans
        }
        println("! ${ret.map { who[it] }.conca()}")
