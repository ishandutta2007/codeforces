...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  vec xlis,ylis;
  rep(i,H-1)xlis.emplace_back(i+1);
  brep1(i,H)xlis.emplace_back(i+1);
  rotate(xlis.begin(),xlis.begin()+A-1,xlis.end());
  rep(i,W-1)ylis.emplace_back(i+1);
  brep1(i,W)ylis.emplace_back(i+1);
  rotate(ylis.begin(),ylis.begin()+B-1,ylis.end());
  pvec v;
  i=j=0;
  ll nowp=1,t;
  rep(t,2*(H-1)*(W-1)){
    if(xlis[i]==C||ylis[j]==D){
      v.emplace_back(nowp*p%mod,t);
      (nowp*=1-p+mod)%=mod;
    }
    if(++i==2*(H-1))i=0;
    if(++j==2*(W-1))j=0;
  ...
  ans=0;
  p=modpow(1-p+mod,v.size());
  for(auto&[x,t]:v){
    ans+=moddevide(t*x%mod,1-p+mod);
    ans+=moddevide((H-1)*(W-1)*2%mod*x%mod*p%mod,(1-p+mod)*(1-p+mod)%mod);
  }
  print(ans%mod);
}
}