...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        for(res i=1;i<=n1;i++)A.pb(mp(read(),i));
        for(res i=1;i<=n2;i++)b[i]=read();
        for(res i=1;i<=n3;i++)c[i]=read();
        for(res i=1;i<=n4;i++)d[i]=read();
        sort(A.begin(),A.end());
        for(res i=1,m=read();i<=m;i++){
            res u=read(),v=read();
            M[mp(u,v)]=1;
        }
        for(res i=1;i<=n2;i++){
            for(auto [va,x]:A)if(!M.count(mp(x,i))){B.pb(mp(va+b[i],i));break;}
        }
        if(B.empty()){puts("-1");return;}
        A=B,vector<Pair>().swap(B),map<Pair,int>().swap(M);
        sort(A.begin(),A.end());
        for(res i=1,m=read();i<=m;i++){
            res u=read(),v=read();
            M[mp(u,v)]=1;
        ...
        for(res i=1;i<=n3;i++){
            for(auto [va,x]:A)if(!M.count(mp(x,i))){B.pb(mp(va+c[i],i));break;}
        }
        if(B.empty()){puts("-1");return;}
        A=B,vector<Pair>().swap(B),map<Pair,int>().swap(M);
        sort(A.begin(),A.end());
        ...
        res ans=inf;
        for(res i=1;i<=n4;i++){
            for(auto [va,x]:A)if(!M.count(mp(x,i))){ans=min(ans,va+d[i]);break;}
        }
        printf("%d\n",ans==inf?-1:ans);
    }
}
