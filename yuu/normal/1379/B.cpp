...
void solve() {
    ...
    for (int64_t a = l; a <= r; a++) {
        int64_t low_add = l - r, high_add = r - l;
        // m - high_add <= a * n <= m - low_add
        int64_t low_n = (m - high_add) / a;
        while (a * low_n > m - high_add) low_n--;
        while (a * low_n < m - high_add) low_n++;
        if (low_n < 1) low_n = 1;
        if (a * low_n > m - low_add) continue;
        int64_t add = m - a * low_n;
        int64_t b = add;
        int64_t c = 0;
        if (b > 0) {
            c += r - b;
