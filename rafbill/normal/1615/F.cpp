...
void solve(){
  li n; cin>>n;
  string s,t; cin>>s>>t;
  for(li i = 0; i < (li)(n); ++i) if(s[i] != '?' && (i&1)) s[i]^=1;
  for(li i = 0; i < (li)(n); ++i) if(t[i] != '?' && (i&1)) t[i]^=1;

  vector<vector<ZN>> cnt(n+1, vector<ZN>(2*n+1));
  vector<vector<ZN>> sum(n+1, vector<ZN>(2*n+1));
  cnt[0][n] = 1;
  sum[0][n] = 0;
  for(li i = 0; i < (li)(n); ++i) {
    for(li j = 0; j < (li)(2*n+1); ++j) if(cnt[i][j].value != 0) {
      if((s[i] != '1')) {
        if(t[i] != '1') {
          cnt[i+1][j] += cnt[i][j];
          sum[i+1][j] += sum[i][j] + cnt[i][j] * abs(j-n);
        }
        if(t[i] != '0'){
          cnt[i+1][j+1] += cnt[i][j];
          sum[i+1][j+1] += sum[i][j] + cnt[i][j] * abs(j-n);
        }
      }
      if(s[i] != '0') {
        if(t[i] != '1') {
