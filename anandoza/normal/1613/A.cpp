...
void solve() {
    ...
    pair<int, string> left{s1.size() + p1, s1};
    pair<int, string> right{s2.size() + p2, s2};

    if (left.first == right.first) {
        while (left.second.size() < right.second.size())
            left.second.push_back('0');
        while (right.second.size() < left.second.size())
            right.second.push_back('0');
    }
