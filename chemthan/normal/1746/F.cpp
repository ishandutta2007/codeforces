...
void chemthan() {
  ...
  int dim = sz(dc);
  int magic = 50;
  vector<vi> vals(dim, vi(magic));
  FOR(i, 0, dim) {
    FOR(j, 0, magic) vals[i][j] = mrand(2);
  }
  FOR(i, 0, n) a[i] = lower_bound(all(dc), a[i]) - dc.begin();
  vector<vi> fens(magic, vi(n + 1));
  ...
  FOR(i, 0, n) {
    FOR(j, 0, magic) {
      upd(fens[j], i, vals[a[i]][j]);
    }
  }
  auto answer = [&] (int l, int r, int k) {
    FOR(i, 0, magic) {
      int x = query(fens[i], r) - query(fens[i], l - 1);
      if (x % k) {
        return 0;
      }
    }
