...
int count1[55][2505];
int count23[55][55][2505];
int g[55],t[55];
int ordine[55][55][55][4];
long long fact[55];
int main()
{
    ...
    for(i=1;i<=n;++i)
    {
        ...
        if(g[i]==1)
        {
            for(int cat1=i;cat1>=0;--cat1)
                for(int tc=0;tc+t[i]<=T;++tc)
                    count1[cat1+1][tc+t[i]]=(count1[cat1+1][tc+t[i]]+count1[cat1][tc])%mod;
            ++nr1;
            continue;
        }
        else
            for(int cat2=i;cat2>=0;--cat2)
                for(int cat3=i;cat3>=0;--cat3)
                    for(int tc=0;tc+t[i]<=T;++tc)
                    {
                        if(g[i]==2)
    ...
    for(int cat1=0;cat1<=nr1;++cat1)
        for(int cat2=0;cat2<=nr2;++cat2)
            for(int cat3=0;cat3<=nr3;++cat3)
            {
                for(int lst=1;lst<=3;++lst)
                {
                    if(ordine[cat1][cat2][cat3][lst]==0)
                        continue;
                    for(int newlst=1;newlst<=3;++newlst)
                    {
                        if(lst==newlst)
                            continue;
                        if(newlst==1)
