...
int calc(int a, int b, int c, int d) {
    ...
    if (a >= c or b >= d) return dp[a][b][c][d] = 0;
    if (a == c - 1 and b == d - 1) {
        return dp[a][b][c][d] = req[a][b];
    }
    dp[a][b][c][d] = max(c - a, d - b);
    for (int i = a + 1; i < c; ++i) {
        dp[a][b][c][d] = min(dp[a][b][c][d], calc(a, b, i, d) + calc(i, b, c, d));
    }
    for (int q = b + 1; q < d; ++q) {
        dp[a][b][c][d] = min(dp[a][b][c][d], calc(a, b, c, q) + calc(a, q, c, d));
    }
    return dp[a][b][c][d];
}
void solve(){
