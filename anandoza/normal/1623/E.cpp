...
int main() {
    ...
    vector<bool> is_less(n, false);
    for (auto it = next(inorder.rbegin()); it != inorder.rend(); it++) {
        int cur = *it, right = *prev(it);
        is_less[cur] = color[cur] < color[right] || (color[cur] == color[right] && is_less[right]);
    }

    vector<bool> changed(n, false);
    int pending_cost = 0;
    y_combinator([&](auto self, int cur) -> bool {
        if (cur == -1)
            return false;

        pending_cost++;

        if (self(children[cur].f))
            changed[cur] = true;

        if (!changed[cur] && is_less[cur] && pending_cost <= k) {
            changed[cur] = true;
            k -= pending_cost;
        }
