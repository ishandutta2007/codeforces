...
int com(int n, int k) {
    rep(i, M) {
        unless(n & (1<<i)) if(k & (1<<i)) return 0;
    }
    return 1;
}
...
void solve() {
    ...
    VEC(int, b, n);
    vc<int> ans(N, 0);
    vvc<int> sum(M + 5, vc<int>(n + 1, 0));
    rep(i, M + 5) {
        rep(j, n) {
            sum[i][j + 1] = sum[i][j] ^ com(n - i, j);
        }
    }
    rep(i, n) {
        ll now = b[i];
        rep(j, M) {
            ...
            int cnt = i + (n - 1 - (i + j));
            int need = (i == 0 ? 0 : 1) + (i + j == n - 1 ? 0 : 1);
            if(cnt < need) continue;
            // for(int x = max(0LL, k - need); x <= cnt - need; ++ x) {
            //     ans[now] ^= com(cnt - need, x);
            // }
            ans[now] ^= sum[n - (cnt - need)][cnt - need + 1] ^ sum[n - (cnt - need)][max(0LL, k - need)];
        }
    }
    string ret = "";
    bool check = 0;
