...
void solve() {
  ...
  struct T {
    ll lx, rx;
    ll cnt;  // increasing subseq
    ll l_cnt, r_cnt, full;
  };

  auto seg_f = [&](T a, T b) -> T {
    if (a.cnt == -1) return b;
    if (b.cnt == -1) return a;
    ...
    c.cnt = a.cnt + b.cnt + (a.rx <= b.lx ? a.r_cnt * b.l_cnt : 0);
    c.l_cnt = a.l_cnt + (a.rx <= b.lx ? a.full * b.l_cnt : 0);
    c.r_cnt = b.r_cnt + (a.rx <= b.lx ? b.full * a.r_cnt : 0);

    c.full = (a.rx <= b.lx ? a.full * b.full : 0);
    return c;
  };

  T e({0, 0, -1, 0, 0, 0});

  SegTree<T> seg(seg_f, e);
  seg.init(N);
  FOR(i, N) {
    ll x = A[i];
    T t = T({x, x, 1, 1, 1, 1});
