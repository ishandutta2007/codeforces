...
vector<hashset<int>> used(20);
vector<int> a;

void addnum(int i) {
    for (int b = 0; b < 20; ++b)
        if (a[i] >> b & 1) used[b].insert(i);
}

void delnum(int i) {
    for (int b = 0; b < 20; ++b)
        if (a[i] >> b & 1) used[b].erase(i);
}
...
signed main() {
    ...
    a.resize(n);
    read(a);
    for (int i = 0; i < n; ++i) addnum(i);
    for (int bit = 0; bit < 20; ++bit) {
        while (sz(used[bit]) > 0) {
            auto f = *used[bit].begin();
            delnum(f);
            bool flag = false;
            for (int b = 0; b < 20; ++b)
                if (sz(used[b]) > 0 && (a[f] >> b & 1) == 0) {
                    int i = *used[b].begin();
                    delnum(i);
                    flag = true;
                    int x = a[f] | a[i];
