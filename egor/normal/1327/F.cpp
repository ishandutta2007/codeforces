...
public:
    void solve() {
        ...
        int m = in.readInt();
        arri l, r, x;
        in.readArrays(m, l, r, x);

        mod = MODF;
        SegmentTree<int, int> tree(n, [](int a, int b) -> int {
            return a | b;
        }, [](int a, int b) -> int {
            return a | b;
        ...
        for (int i : range(m)) {
            tree.update(l[i] - 1, r[i], x[i]);
        }
        arri val(n);
        for (int i : range(n)) {
            val[i] = tree.query(i, i + 1);
        }
        modint answer = 1;
        for (int bit : range(k)) {
            vec<pii> psec;
            ...
            sort(all(psec));
            modint total = 1;
            arr<modint> val(pp.size() + 1);
            int at = 0;
            int start = -1;
            int j = 0;
            val[0] = 1;
            for (int i : pp) {
                while (at < psec.size() && psec[at].first <= i) {
                    while (start < int(pp.size()) && (start == -1 || pp[start] < psec[at].second)) {
                        total -= val[start + 1];
                        start++;
                    }
                    at++;
