...
int main(){
    ...
    ll ok = 0, ng = m;
    {
        auto check = [&](ll x) -> bool {
            auto get_weight = [&](ll c) -> ll {
                if(edge[c][2]) return edge[c][2];
                if(c < x) return INF;
                return 1;
            };
            ...
            while(q.size()){
                ...
                if(used[at]) continue;
                used[at] = 1;
                each(i, g[at]) if(chmin(cost[i], cost[at] + get_weight(i.cost))) q.emplace(cost[i], i);
            }
            return cost[t] <= l;
        };
    ...
    if(!check(ok)) return NO();
    YES();
    rep(a)if(edge[i][2]==0)edge[i][2]=INF;
    if(a<m&&edge[a][2]==0)edge[a][2]=ok;
    rep(i,a+1,m)if(edge[i][2]==0)edge[i][2]=1;
    each(i,edge)out(i);
}