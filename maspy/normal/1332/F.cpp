...
void solve() {
  ...
  */
  ...
  using T = tuple<mint, mint, mint, mint>;
  T unit = {mint(1), mint(0), mint(0), mint(0)};
  auto f_ee = [&](T A, T B) -> T {
    auto [a00, a01, a10, a11] = A;
    auto [b00, b01, b10, b11] = B;
    return {a00 * b00, mint(0), a00 * b10 + a10 * (b00 + b10),
            a00 * b11 + a10 * b11 + a11 * (b00 + b10 + b11)};
  };
  auto f_ev = [&](T A, ll v) -> T {
    auto [a00, a01, a10, a11] = A;
    return {a00 + a01, a00 + a01, a10 + a11, a10};
  };
  auto f_ve = [&](T A, auto e) -> T {
    auto [a00, a01, a10, a11] = A;
    return {a00 + a10 + a11, mint(0), a00 + a10, a01 + a11};
  };
  auto dp = tree_dp(G, f_ee, f_ev, f_ve, unit);
  auto [a00, a01, a10, a11] = dp[0];
  mint ANS = a00 + a10 + a11 - mint(1);
  print(ANS);
}
