...
void solve(){
    ...
    for(auto x:rjun){
        ...
        for(auto c:chi[x]){
            sm += dp[c].fi;
            yob.pb(dp[c].se-dp[c].fi);
        }
        sort(all(yob),[](ll x,ll y){return x>y;});
        
        ll noko = bou[x].fi - bou[chi[x][0]].fi*chi[x].size();
        rep(i,0,noko-1){
            dp[x].fi += yob[i];
        }
        noko = bou[x].se - bou[chi[x][0]].fi*chi[x].size();
        rep(i,0,noko-1){
            dp[x].se += yob[i];
        }
