...
class stack(val  sum:Long, val length:Int)
fun main(){
    solve.cases{
        ...
        fun check():Boolean{
            ...
            val last2 = st[st.lastIndex -1 ]
            val last = st.last()
            if(last2.sum * last.length >= last2.length * last.sum){
                val new = stack(last2.sum + last.sum,last2.length + last.length)
                st.removeLast()
                st.removeLast()
                st.add(new)
                return true
        ...
        var pointer = -1
        for(c in st){
            val give = c.sum.toDouble() / c.length.toDouble()
            repeat(c.length){
                pointer++
                ret[pointer] = give
            }
...
/*
even though segg tree chees, just 
stacks and priority queue are also pretty useful 
