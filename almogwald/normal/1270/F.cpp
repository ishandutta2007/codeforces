...
int main() {
	...
	ones.push_back(n);
	forn(i, (n / num) + 3) {
		fori(j,((int)ones.size())-i-1) {
			if (ones[j + i + 1] - ones[j] - 1 < i * num) {
				con
			}
			ans += multri(ones[j + i + 1] - ones[j] - 1, i);
			ans -= multri(ones[j + i] - ones[j] - 1, i);
			ans -= multri(ones[j + i+1] - ones[j+1] - 1, i);
			ans += multri(ones[j + i] - ones[j+1] - 1, i);
		}
	}
	co ans el
		ex
