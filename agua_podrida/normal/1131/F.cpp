...
void b(){
    ...
    cin>>n;
    vvi ordenes(n+1);
    F1(i,n) ordenes[i].pb(i);
    vi set(n+1);
    F1(i,n) set[i] = i;
    int x,y;
    F0(i,n-1){
        cin>>x>>y;
        if(ordenes[set[x]].size() < ordenes[set[y]].size()) swap(x,y);
        int sy = set[y];
        F0(k,ordenes[sy].size()) {
            ordenes[set[x]].pb(ordenes[sy][k]);
            set[ordenes[sy][k]] = set[x];
        }
    }
    int s = set[1];
    F0(i,n) cout<<ordenes[s][i]<<' ';
    cout<<endl;
}
