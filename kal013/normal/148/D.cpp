...
bool vis[N][N][2];
long double state(int i,int j,int k){
	assert(i >= 0 && j >= 0 && k >= 0);
	...
	if (k == 0){
		dp[i][j][k] += p2 * (1 - state( i , j - 1, k ^ 1));
		return dp[i][j][k];
	}

	if (j == 1){
		dp[i][j][k] += p2 * (1 - state(i - 1, j - 1, k^1));
	}
	else{
		long double p3 = i;
		p3 /= (i + j - 1);
		long double p4 = 1 - p3;
		dp[i][j][k] += p2 * p3 *(1 - state(i - 1, j - 1, k^1));
		dp[i][j][k] += p2 * p4 *(1 - state(i , j - 2, k^1));
