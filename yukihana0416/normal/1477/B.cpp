...
void update(int f, int L, int R, int l, int r, int id)
{
    if(L <= l && r <= R)
    {
        flag[id] = f;
    ...
    lazy(l, r, id);
    int mid = MID(l, r);
    if(L <= mid) update(f, L, R, lson);
    if(mid < R) update(f, L, R, rson);
    mer(id);
}

void get_res(int l, int r, int id)
{
    if(l == r)
    {
        if(T[id][0]) ss[l] = '0';
...
void work()
{
    ...
    for(int i = q; i; i--)
    {
        ...
        for(int j = 0; j < 2; j++) qres[j] = 0;
        query(l, r, 1, n, 1);

//        cout <<l <<" " <<r <<" " <<qres[0] <<" " <<qres[1] <<endl;

        if(qres[0] * 2 < r - l + 1)
        {
            update(1, l, r, 1, n, 1);
        }
        else if(qres[1] * 2 < r - l + 1)
        {
            update(0, l, r, 1, n, 1);
        }
        else
