...
int main(){
  ...
  vi A(n+16);
  A[0] = 0;
  for(li i = 0; i < (li)(n-1); ++i) { cin>>A[1+i]; A[1+i] -= 1; }
  ...
  auto lca = [&](int u, int v) {
    while(u != v) {
      int u3 = max(0, A[u] - XC[u/B]);
      int v3 = max(0, A[v] - XC[v/B]);
      int u2 = (XC[u/B] < B ? C[u] : u3);
      int v2 = (XC[v/B] < B ? C[v] : v3);
      if((u/B) > (v/B)) u = u2;
      else if((u/B) < (v/B)) v = v2;
      else if(u2 != v2) { u = u2; v = v2; }
      else if(u > v) { u = u3; }
      else { v = v3; }
    }
    return u;
  ...
  for(li q_ = 0; q_ < (li)(q); ++q_) {
    ...
    if(ty == 1) {
      ...
      if(l/B == r/B) {
        for(li i = (l); i <= (li)(r); ++i) A[i] -= x;
        if(XC[l/B] < B) calc((l/B)*B, (l/B+1)*B-1);
      }else{
        for(li i = ((l/B)+1); i <= (li)((r/B)-1); ++i) {
          XC[i] += x;
          if(XC[i] < B) calc(i*B, (i+1)*B-1);
        }
        for(li i = (l); i <= (li)((l/B+1)*B-1); ++i) A[i] -= x;
        if(XC[l/B] < B) calc((l/B)*B, (l/B+1)*B-1);
        for(li i = ((r/B)*B); i <= (li)(r); ++i) A[i] -= x;
        if(XC[r/B] < B) calc((r/B)*B, (r/B+1)*B-1);
      }
