  ...
  auto query = [&]() -> void {
    ...
    for (auto& v: hld.V) {
      ...
      if (G.root[v] == v) {
        ll& dp = DP[v];
        dp = len(S);
        FOR(x, 10) if (!(can[v] >> x & 1)) chmin(dp, FIRST[x]);
      } else {
        DP[v] = DP[hld.parent[v]];
        if (DP[v] < len(S) && F[DP[v]] == A[v]) ++DP[v];
      }
      if (DP[v] == len(S)) return YES();
    }
    return NO();
  };
  FOR(_, Q) query();
