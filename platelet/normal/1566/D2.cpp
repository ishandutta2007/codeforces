...
void solve() {
    ...
    sort(b + 1, b + n * m + 1, [](int x, int y) { return a[x] ^ a[y] ? a[x] < a[y] : x < y; });
    int as = 0;
    For(i, 0, n) {
        sort(b + i * m + 1, b + i * m + m + 1, [](int x, int y) { return a[x] ^ a[y] ? a[x] < a[y] : x > y; });
        rep(j, 1, m) For(k, 1, j) as += b[i * m + k] < b[i * m + j];
    }
    cout << as << endl;
}
int main() {
