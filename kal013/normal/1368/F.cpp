...
inline pair<long long,long long> rn(int n){
	pair<long long,long long> rem = {0,-1};
    for(long long k =1;k<=n;++k){
    	long long cnt = n - (n + k)/(k + 1) - k ;
    	if (cnt > rem.first){
    		rem = {cnt,k+1};
    	}
...
void solve(){
    ...
    for(int i=0;i<n;++i) {arr[i] = 0;active[i]=false;}

    if (n<=3){
    	cout<<0<<endl;
    	exit(0);
    }
    pair<int,int> Rn = rn(n);
    assert(Rn.first > 0 && Rn.second > 0);
	...
	assert(num_al == Rn.first + k-1);


    while(num_queries < TOT && cnt < Rn.first){
    	vector<int> ans;
    	for(int i=0;i<n;++i){
    		if (active[i]  && arr[i] == 0){
    			ans.push_back(i);
  ...
  //   while(cnt < rn){
		// vector<int> ans;
		// for(int i = 0; i<n && ans.size() < 2 ;i+=2){
		// 	if (arr[i]) continue;
		// 	ans.push_back(i);
		// }    	
		// query(ans);
