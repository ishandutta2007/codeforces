...
inline void solve() {
    ...
    mv[sz(b) - 1][b.back() - '0'] = 0;
    for (int st = 1; st < m; ++st) {
        string t = b.substr(st);
        if (t == b.substr(0, sz(t))) {
            mv[sz(b) - 1][b.back() - '0'] = sz(t);
            break;
        }
    }
    ...
    for (int st = 0; st < n; ++st) {
        ...
        for (int cur_prf = 0; cur_prf < m; ++cur_prf) {
            for (int cnt = 0; cnt <= n; ++cnt) {
                dp[(st & 1) ^ 1][mv[cur_prf][b[cur_prf] - '0']][cnt + (cur_prf == m - 1)] = min(
                    dp[(st & 1) ^ 1][mv[cur_prf][b[cur_prf] - '0']][cnt + (cur_prf == m - 1)]
                    , dp[(st & 1)][cur_prf][cnt] + (a[st] != b[cur_prf])
                );

                dp[(st & 1) ^ 1][mv[cur_prf][(b[cur_prf] ^ '1' ^ '0') - '0']][cnt] = min(
