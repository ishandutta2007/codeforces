...
vector<pair<int, int>> ans;
void dfs(int now, int val, int target,int back) {
    ans.push_back(mp(now, val));
    int cnt = 0;
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        cnt++;
    ...
    if (target == 0) {
        for (auto& x : vertexs[now]) {
            if (x == back) continue;
            dfs(x, val + 1, val, now);
            ans.push_back(mp(now, val + 1));
            val++;
        }
        if (now != 1) {
            ans.push_back(mp(now, target));
        }
    }
    else {
        for (auto& x : vertexs[now]) {
...
void solve(){
    ...
    REP(i, n) {
        maxs = max(maxs, (int)vertexs[i + 1].size());
    }
    dfs(1,0,0,-1);
    cout << ans.size() << endl;
    REP(i, ans.size()) {
