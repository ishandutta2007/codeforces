...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		vector<int> need_print(n);
		vector<int> ans(n);
		for (int i: range(n)) {
			in >> queries[i].type >> queries[i].time >> queries[i].num;
			queries[i].id = i;
			need_print[i] = queries[i].type == 3;
		}

		sort(queries, [](const Query& l, const Query& r) {
			return tie(l.num, l.id) < tie(r.num, r.id);
		});

		for (int i = 0; i < n;) {
			int j = i;
			while (queries[i].num == queries[j].num) {
				++j;
			}
			CoordinateMinimizer<int> cm;
			for (int k = i; k < j; ++k) {
				cm.add(queries[k].time);
			}
			cm.finalize();
			BottomUpSumSegmentTree<int> tree(j - i);
			for (int k = i; k < j; ++k) {
				int time = cm.find(queries[k].time);
				if (queries[k].type == 3) {
					//cerr << "get " << time << endl;
				//	cerr << tree.getResult(0, time);
					ans[queries[k].id] = tree.getResult(0, time);
				} else {
					int add = 1;
					if (queries[k].type == 2) {
						add = -1;
					}
					//cerr << "add " << add << ' ' << time << endl;
					tree.set(time, add);
				}
			}
			i = j;
