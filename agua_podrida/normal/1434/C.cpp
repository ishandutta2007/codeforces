...
void a(){
    ...
    int res = INF;
    set<pair<int,pi>> s;
    F0(i,n) s.insert({b[i]-a[0],{i,0}});
    auto it = s.begin();
    while(it->second.second != 5){
        int i = it->second.first;
        int j = it->second.second;
        pair<int,pi> nuevo = {b[i] - a[j+1],{i,j+1}};
        s.erase(it);
        s.insert(nuevo);
        it = s.begin();
        mini(res,(--s.end())->first-it->first);
    }
    cout<<res<<'\n';
}
...
ll dano(ll a, ll b, ll c, ll d, ll t){
    ll res = a * (t/d) + a;
    if(t-c >= 0) res -= (t-c)/d * b*c + b*c;
    ll k1 = (t-c)/d + 1;
    if(t-c < 0) k1 = 0;
    ll k2 = (t-1)/d;
    ll p = (t-k1*d)*b;
...
void c(){
    ...
    ll k = (a-1) / (b*d);
    ll res = dano(a,b,c,d,k*d);
    if(k > 0) maxi(res,dano(a,b,c,d,k*d-d));
    maxi(res,dano(a,b,c,d,k*d+d));
    cout<<res<<'\n';
}
