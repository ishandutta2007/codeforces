...
inline void AC() {
    ...
    read(a);
    _ dx = -1;
    for(_ i = 0; i < a.length()-1; ++i) if(a[i]+a[i+1]-'0'-'0' >= 10) dx = i;
    if(~dx) {
        str o;
        if(dx != 0) o += a.substr(0, dx);
        o += to_string(a[dx]+a[dx+1]-'0'-'0');
        if(dx != a.length()-2) o += a.substr(dx+2);
        writeln(o);
        return;
    }
    for(_ i = 1; i < a.length(); ++i) if(a[i] != '0') {
        str o;
        if(i != 1) o += a.substr(0, i-1);
        o += to_string(a[i]+a[i-1]-'0'-'0');
        if(i != a.length()-1) o += a.substr(i+1);
        writeln(o);
        return;
    }
    writeln(a.substr(0, a.length()-1));
