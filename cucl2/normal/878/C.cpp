...
{
	int mn[15],mx[15],sz;
	bool operator<(const jiucai& ga)const
	{
		for(int i=0;i<k;i++)
			if(mx[i]>ga.mn[i])
				return 0;
...
int main()
{
	...
	while(n--)
	{
		...
		xfs.sz=1;
		auto be=gjc.lower_bound(xfs),en=gjc.upper_bound(xfs);//a=xfs
		for(auto it=be;it!=en;)//a=xfslevel
		{
			for(int i=0;i<k;i++)
			{
				xfs.mx[i]=max(xfs.mx[i],it->mx[i]);
				xfs.mn[i]=min(xfs.mn[i],it->mn[i]);
			}
			xfs.sz+=it->sz;
			gjc.erase(it++);
		}
