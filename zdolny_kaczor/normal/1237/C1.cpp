...
using line = map <int, int>;
using plane = pair <int, map <int, line> >;
using volume = pair <int, map <int, plane> >;
vpii ans;
volume all;
void assert_good(plane &p) {
	int tot = 0;
	for (auto &c : p.second) {
		tot += c.second.size();
		assert(!c.second.empty());
	}
	debug(imie(tot), imie(p));
	assert(tot == p.first);
}
void assert_good(volume &vol) {
	onl(return;)
	int tot = 0;
	for (auto &x : vol.second) {
		tot += x.second.first;
		assert(x.second.first);
		assert_good(x.second);
	}
	debug(imie(tot), imie(vol));
	assert(tot == vol.first);
}
void add(line &l, int z, int num) {
	l[z] = num;
}
void add(plane &p, int y, int z, int num) {
	p.first++;
	add(p.second[y], z, num);
}
void add(volume &v, int x, int y, int z, int num) {
	add(v.second[x], y, z, num);
	v.first++;
}
void erase_pair(line &l) {
	assert(l.size() >= 2u);
	auto it = l.begin();
	auto it2 = it;
	it2++;
...
int erase_close(line &l, int z) {
	auto it = l.lower_bound(z);
	assert(!l.empty());
	if (it == l.end()) it--;
	int r = it->second;
	debug(imie(z), imie(l), imie(*it), imie(r));
	l.erase(it);
	return r;
}
int erase_close(plane &p, int y, int z) {
	auto it = p.second.lower_bound(y);
	if (it == p.second.end()) it--;
	int r = erase_close(it->second, z);
	p.first--;
	debug(imie(y), imie(z), imie(p), imie(*it), imie(r));
	if (it->second.empty()) p.second.erase(it);
	return r;
}
void erase_pair(plane &p) {
	assert(!p.second.empty() && p.first >= 2);
	p.first -= 2;
	auto it = p.second.begin();
	if (it->second.size() >= 2u) {
		debug("go down to line");
		erase_pair(it->second);
		if (it->second.empty())
			p.second.erase(it);
		return;
	}
	...
	int z = it->second.begin()->first;
	int num = it->second.begin()->second;
	debug("extract", imie(z), imie(num));
	p.second.erase(p.second.begin());
	int num2 = erase_close(p.second.begin()->second, z);
	if (p.second.begin()->second.empty())
		p.second.erase(p.second.begin());
	debug(imie(num), imie(num2));
	ans.emplace_back(num, num2);
	debug("left with ", imie(p));
}
void erase_pair(volume &v) {
	assert(!v.second.empty() && v.first >= 2);
	v.first -= 2;
	auto it = v.second.begin();
	if (it->second.first >= 2) {
		debug("go down to plane");
		erase_pair(it->second);
	...
	debug(imie(p));
	assert(p.first == 1);
	int y = p.second.begin()->first;
	int z = p.second.begin()->second.begin()->first;
	int num = p.second.begin()->second.begin()->second;
	debug("extract", imie(y), imie(z), imie(num));
	it++;
	v.second.erase(v.second.begin());
	int num2 = erase_close(v.second.begin()->second, y, z);
	if (v.second.begin()->second.first == 0)
		v.second.erase(v.second.begin());
	debug(imie(num), imie(num2));
	ans.emplace_back(num, num2);
}
