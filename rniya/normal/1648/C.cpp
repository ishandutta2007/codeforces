...
mint mapping(mint l, mint r) { return l * r; }
...
int main() {
    ...
    for (int& x : s) cnt[x]++;
    mint rest = BINOM.fac(n);
    for (int i = 0; i < MAX; i++) rest *= BINOM.finv(cnt[i]);
    int cur = n - 1;
    vector<mint> init(MAX);
    for (int i = 0; i < MAX; i++) init[i] = rest * BINOM.inv(n) * cnt[i];
    atcoder::lazy_segtree<mint, op, e, mint, mapping, composition, id> seg(init);  // i 
    // auto del = [&](int x) {
    //     rest *= BINOM.inv(cur--);
    //     rest *= cnt[x]--;
    // };
    ...
    for (int i = 0; i < m; i++) {
        if (i >= n) break;
        int x = t[i];
        // for (int j = 0; j < 5; j++) cerr << seg.get(j) << (j + 1 == 5 ? '\n' : ' ');
        ans += seg.prod(0, x);
        if (cnt[x] == 0) break;
        seg.apply(0, MAX, BINOM.inv(cur--));
        // for (int j = 0; j < 5; j++) cerr << seg.get(j) << (j + 1 == 5 ? '\n' : ' ');
        seg.apply(0, x, cnt[x]);
        seg.apply(x + 1, MAX, cnt[x]);
        cnt[x]--;
        seg.apply(x, cnt[x]);
        if (i == n - 1 and n < m) ans++;
        // debug(ans);
    }
