...
void solve(){
  string s;
  cin>>s;
  vector<vector<ll>> n(26);
  for(int i=0;i<int(s.size());i++){
    n[s.at(i)-'a'].push_back(i);
  }
  ll ans=0;
  vector<ll> a(int(s.size()),-1);
  for(int i=0;i<26;i++){
    if(int(n[i].size())!=0){
    ll tmp=n[i][0];
    for(int j=1;j<int(n[i].size());j++){
      if(abs(n[i][j]-tmp)<=2&&a[n[i][j]]==-1&&a[tmp]==-1){
        ans++;
        a[n[i][j]]=1;
      }
      if(j!=int(n[i].size())-1){
        if(abs(n[i][j+1]-tmp)<=2&&a[n[i][j+1]]==-1&&a[tmp]==-1){
        ans++;
        a[n[i][j+1]]=1;
      }
      }
