...
ll count_gre(db mid) {
	// mid *= mid;
	V<pair<db,int>> intervals;
	int cnt = 0;
	trav(t,v) {
		// ll dist = t.f*t.f+t.s*t.s;
		...
		if (frac <= 1) {
			...
			intervals.pb({nor(ang-frac),cnt});
			intervals.pb({nor(ang+frac),cnt});
		}
		cnt ++;
	}
	sort(all(intervals));
	trav(t,intervals) dbg(t);
	vi st(N,-1), en(N,-1);
	F0R(i,sz(intervals)) {
		if (st[intervals[i].s] == -1) st[intervals[i].s] = i;
		else en[intervals[i].s] = i;
	}
	B = BIT<int,400005>();
	ll tot = 0;
	F0R(i,sz(intervals)) {
		if (i == st[intervals[i].s]) {
			// dbg("END",en[intervals[i].s]);
			B.upd(en[intervals[i].s]+1,1);
			tot += B.query(st[intervals[i].s]+2,en[intervals[i].s]);
		} else {
			// dbg("??",i,st[intervals[i].s],en[intervals[i].s],B.query(st[intervals[i].s]+2,en[intervals[i].s]));
		}
	}
	return tot;
}
...
int main() {
	...
	// }
	// dbg(count_gre(0.1));
	// count_gre(0.4); exit(0);
	// for (db d = 0.1; d <= 10; d += 0.1) {
	// 	dbg(d,count_gre(d));
	// }
	// ps(count_gre(0.9));
	// exit(0);
	...
	F0R(_,50) {
		dbg(_);
		db mid = (lo+hi)/2;
		ll num = count_gre(mid);
		num = (ll)N*(N-1)/2-num;
		// how many <= mid
		// smallest such that num >= k
		if (num >= K) hi = mid;
		else lo = mid;
	}
	// dbg(ti,ti2);
