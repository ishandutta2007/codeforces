...
string s;
mint mem[110][110][110];
bool visited[110][110][110];
mint f(int x,int l,int r){
    if(visited[x][l][r]){
        return mem[x][l][r];
    }
    if(x==0){
        if(l==r and (l==0 or r==s.size())) return 2;
        if(l==r) return 1;
        if(l==r-1 and s[l]=='0')return 1;
        else return 0;
    }
    if(x==1){
        if(l==r and (l==0 or r==s.size()))return 2;
        if(l==r)return 1;
        if(l==r-1 and s[l]=='1')return 1;
        else return 0;
    }
    mint res;
    rep2(i,l,r){
        res+=f(x-1,l,i)*f(x-2,i,r);
    }
    visited[x][l][r]=1;
    return mem[x][l][r]=res;
}
