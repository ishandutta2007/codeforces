...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, a-1) rvec.push_back(i);
		lvec.push_back(a);
		rep(i, b+1, n) lvec.push_back(i);
		rvec.push_back(b);
		rep(i, a+1, b-1){
			if(sz(lvec) < n/2) lvec.push_back(i);
			else rvec.push_back(i);
		}
		
		if(sz(lvec) != n/2) outl(-1);
		else{
			lvec.insert(lvec.end(), all(rvec));
			outl(lvec);
		}
	}
