...
int main() {
	...
	if (n <= THRES) adj.resize(n + 1, vector<ll>(n + 1, 0));
	...
	auto getNext = [&](int now) {
		auto it = toTrav.upper_bound(now);
		if (it == toTrav.end())
			return -1;
		else
			return *it;
	...
	auto isDisconnect = [&](int u, int v) -> bool {
		return g[u].count(v);
	};
 
	function<void(int)> dfs = [&](int u) {
		toTrav.erase(u);
		for (int now = getNext(0); now != -1; now = getNext(now)) {
			if (isDisconnect(u, now))
				continue;
 
			assert(dsu1.uni(u, now));
			dfs(now);
		}
	};
