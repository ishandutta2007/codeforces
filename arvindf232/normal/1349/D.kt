...
fun main(){
    solve.cases{
        ...
        val dp = IntArray(s+1)
        dp[0] = 0
        dp[1] = intPow(n,s,pI) modMinus 1
        for(i in 1 until s){
            val term1 = dp[i]
            val term2 = (1 modDivide (n-1)) modM (i modDivide s) modM dp[i-1]
            val term3 = ((n-2) modDivide (n-1)) modM (i modDivide s) modM dp[i]
            val product = term1 modMinus 1 modMinus term2 modMinus term3
            val new = product modDivide (s - i) modM s
            dp[i+1] = new
        }
        var cost = 0
        for(i in 0 until n){
            cost = cost modPlus (dp[s -L[i]])
        }
        cost = cost modMinus ((n-1) modM (dp.last()))
        cost = cost modDivide n
        put(cost)
...
/*
true generating functions maybe
