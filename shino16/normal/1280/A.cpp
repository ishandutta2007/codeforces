...
public:
  ...
  modint(const ll x = 0) noexcept : value(x) {
    value %= MOD;
...
modint pow(modint base, size_t exp) {
  modint res = 1;
  while (exp) {
    if (exp % 2) res *= base;
    base *= base;
...
int main() {
  ...
  rep(t) {
    ...
    // s.reserve(x*4);
    int l = 0;
    for (; s.size() < x; l++) if (s[l]-'0') {
      string copied = s.substr(l+1);
      rep(s[l]-'0'-1) s += copied;
    }
    debug(l);
    debug(s);
    modint right = s.size() - l;
    for (; l < x; l++) {
      right--;
      right *= s[l] - '0';
    }
