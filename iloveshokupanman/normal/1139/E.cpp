...
int main() {
    ...
    vector<P> ed(N);
    rep(i, N)cin >> ed[i].second;
    rep(i, N)cin >> ed[i].first;
    rep(i, N)ed[i].first--;
    ...
    vector<set<pair<ll,P>>> lis(M);
    rep(i, N) {
        if (ed[i].second >= M) { continue; }
        lis[ed[i].second].insert({ day[i],ed[i] });
    }
    ...
    repn(x, M) {
        //cout << x << endl;
        for (auto p : lis[x-1]) { q.push(p); }

        while (!q.empty()) {          
            auto p = q.top();
            //cout << p.first << " " << p.second.first << " " << p.second.second << endl;
            q.pop();
            g.add_edge(p.second.first, p.second.second + M, 1);
            K += g.flow(s, t);
            //cout <<"fl"<< K << endl;
            if (K >= x) { ivans[x] = min(ivans[x-1], p.first); break; }
            if (q.empty()) { ivans[x] = -1; }
        }
        
    }
    ivans[M + 1] = -1;

    //rep(i, M + 1)cout << ivans[i] << endl;
    //cout << endl;

    repn(i, D) {
        repn(j, M + 1)if (ivans[j] < i) { cout << j - 1 << endl;  break; }
    }
