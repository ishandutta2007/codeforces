...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		d.run(i);
		for (ll j = 0; j < n; j++) dist[i][j] = d.dists[j];
	}
	
	ll ans = 0;
	for (ll i = 0; i < k; i++) {
		ans += dist[rt[i].f][rt[i].s];
	}
	
	for (ll i = 0; i < m; i++) {
		ll cur = 0;
		for (ll j = 0; j < k; j++) {
			cur += min({dist[rt[j].f][rt[j].s], dist[rt[j].f][edges[i].f] + dist[edges[i].s][rt[j].s], dist[rt[j].f][edges[i].s] + dist[edges[i].f][rt[j].s]});
		}
		ans = min(ans, cur);
	}
