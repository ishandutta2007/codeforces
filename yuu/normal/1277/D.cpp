...
#define assert(x) (x)
#endif
vector<pair<int, string>> g[2][2];
int size[2][2];
void solve() {
    int n;
    for (int i = 0; i <= 1; i++)
        for (int j = 0; j <= 1; j++) g[i][j].clear();
    cin >> n;
    string s;
    for (int i = 1; i <= n; i++) {
        cin >> s;
        g[s[0] - '0'][s.back() - '0'].emplace_back(i, s);
    }
    if (g[0][1].empty() && g[1][0].empty()) {
        if (g[0][0].empty() || g[1][1].empty()) {
            cout << "0\n";
        } else {
            cout << "-1\n";
        }
        ...
        for (auto&& [id, s] : b) {
            ...
            if (!hs.count(h)) can_change.emplace_back(id);
        }
        if (a.size() + can_change.size() + 1 < b.size() - can_change.size()) {
            cout << "-1\n";
            return;
        }
        int sz_a = a.size();
        int sz_b = b.size();
        while (sz_b > sz_a + 1) {
            ans.emplace_back(can_change.back());
            can_change.pop_back();
            sz_b--;
            sz_a++;
        }
