...
signed main(){
    ...
    cout.precision(10);
    cin>>n;
    rep(i,n) cin>>a[i]>>b[i], sum+=b[i];
    rep(i,n+1) rep(j,maxm) dp[i][j] = -100000000;
    dp[0][0] = 0;
    rep(i,n){
        for (int j=i;j>-1;--j) for (int k=j*100;k>-1;--k) dp[j+1][k+a[i]]=max(dp[j+1][k+a[i]],dp[j][k]+b[i]);
    }
    rep1(i,n){
        int res=0;
        rep(j,maxm) res=max(res,min(2*j,sum+dp[i][j]));
        cout<<1.0*res/2<<" ";
    }
    return 0;
}