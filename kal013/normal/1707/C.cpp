...
void solve(){
...

	{
		...
		for(int i = 0; i < m; ++i){
			...
			--edges[i][0]; --edges[i][1];
			tree[i] = merge(edges[i][0], edges[i][1]);
			if (tree[i])
				q.add_edge(edges[i][0], edges[i][1]);
		}
	}
	q.build();
	...
	auto mark_range = [&] (int x, int y, int v) {
		// trace(x, y);
		assert (x <= y + 1);
		if (x < n){
			p[x] += v;
	...
	for(int i = 0; i < m; ++i){
		...
		if (l == u){
			int z = q.child_ancestor(u, v);
			mark_range(q.hld_tin[z], q.hld_tout[z], 1);
			mark_range(q.hld_tin[v], q.hld_tout[v], -1);
		}
		else{
			mark_range(0, n - 1, 1);
			mark_range(q.hld_tin[u], q.hld_tout[u], -1);
			mark_range(q.hld_tin[v], q.hld_tout[v], -1);
		}
	}
