	...
	int mx(int x, int y)
	{
		...
		if (y - x - 1 == 0)
			if (mx0)
				...
				return a[0][x];
		int kk = sps(y - x);
		int ans = max(a[kk - 1][x], a[kk - 1][y - (1 << (kk - 1))]);
		if (mx0)
			ans = max(ans, 0);
		return ans;
	}
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		if (w <= 2 * l + 5)
		{
			...
			for (int j = 0; j < w; ++j)
			{
				anses[j] = sp.mx(j + l - w, j + 1);
			}
			for (int j = 0; j < w - 1; ++j)
				raz[j] += anses[j + 1] - anses[j];
		}
		else
		{
			vi anses(l);
			...
			for (int j = w - l; j < w; ++j)
			{
				anses[j + l - w] = sp.mx(j + l - w, j + 1);
			}
			for (int j = 0; j < l - 1; ++j)
				raz[w - l + j] += anses[j + 1] - anses[j];
		}
	}
