...
int solve(const int l, const int r, vector<int> vs){
	if(vs.size()%2){
		int re=0;
		for(cauto &v:vs) re^=v;
		reps(i,l,r) re^=i;
	...
	vector<int> nxt;
	while(st.size()){
		cauto a=*st.begin(); st.erase(st.begin());
		if(st.count(a^1)){
			nxt.eb(a/2);
			st.erase(a^1);
		}else{
	...
	if(po.empty()){
		for(auto &v:vs) v/=2;
		return solve(l/2,r/2,nxt)*2;
	}
	for(cauto &v:po){
		cauto x=v^l;
		vector<int> us;
		for(cauto &v:vs) us.eb(v^x);
		sort(all(us));
		int ok=1;
		rep(i,r-l)if(us[i]!=l+i) ok=0;
		if(ok) return x;
	}
}
...
int main(){
	...
	TT{
		cauto l=read(0);
		cauto r=read(-1);
		cauto vs=readV(r-l,0);
		cout<<solve(l,r,vs) NL;
	}
	return 0;
}