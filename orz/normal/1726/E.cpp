...
int another_main()
{
	...
	vi fact(C, 1);
	for (int i = 2; i < C; ++i)
		fact[i] = prod(fact[i - 1], i, M);
	vi ifact(C, 1);
	vi dfact(C, 1);
	vi p2(C, 1);
	for (int i = 2; i < C; ++i)
		dfact[i] = prod(i, dfact[i - 2], M);
	for (int i = 1; i < C; ++i)
		p2[i] = sum(p2[i - 1], p2[i - 1], M);
	...
	for (int I = 0; I < t; ++I) {
		...
		vi F(n + 1, 1);
		for (int i = 2; i <= n; ++i)
			F[i] = sum(F[i - 1], prod(F[i - 2], i - 1, M), M);
		int ans = 0;
		for (int k = 0; k <= n / 4; ++k) {
			int l = 2 * k;
			int pairs = prod(fact[n - l], prod(ifact[l], ifact[n - 2 * l], M), M);
			int pairup = prod(pairs, dfact[max(l - 1, 0)], M);
			int ans_cur_prime = prod(p2[k], pairup, M);
			int ans_cur = prod(ans_cur_prime, F[n - 2 * l], M);
			ans = sum(ans, ans_cur, M);
		}
		cout << ans << '\n';
	}
	return 0;
