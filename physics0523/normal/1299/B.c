...
int main(void){
  ...
  if(n%2==1){printf("NO\n");return 0;}
  for(i=0;i<n;i++){
    scanf("%lld%lld",&vec[i].xz,&vec[i].yz);
  }
  k=n/2;
  for(i=0;i<n;i++){
    if(crossproduct(vector_minus(vec[(i+1)%n],vec[i]),vector_minus(vec[(i+k+1)%n],vec[(i+k)%n]))!=0){printf("NO\n");return 0;}
    if(basedist(vec[(i+1)%n],vec[i])!=basedist(vec[(i+k+1)%n],vec[(i+k)%n])){printf("NO\n");return 0;}
  }
  printf("YES\n");
  return 0;
}