...
signed main()
{
    ...
    while(v>=2&&!s[2].empty())
    {
        int can[3]={};
        can[2]=s[2].begin()->x;
        if(s[1].size()>=1) can[1]+=s[1].begin()->x;
        if(s[1].size()>=2) can[1]+=next(s[1].begin())->x;
        if(can[1]>=can[2])
            ans+=s[1].begin()->x,out.insert(s[1].begin()->y),v-=1,s[1].erase(s[1].begin());
        else
            ans+=s[2].begin()->x,out.insert(s[2].begin()->y),v-=2,s[2].erase(s[2].begin());
    }
    while(v>=1&&!s[1].empty())
        ans+=s[1].begin()->x,out.insert(s[1].begin()->y),v-=1,s[1].erase(s[1].begin());
    cout<<ans<<"\n"<<rng(out);
