...
inline void W(ll a, ll b, ll c, ll d) { io.write(a, ' '), io.write(b, ' '), io.write(c, ' '), io.write(d, '\n'); }
main() {
    ll n, m, l, r;
    read(n), read(m), read(l), read(r);
    vv(int, div, n + 1);
    rep2(i, 1, n) {
        for(int j = i; j <= n; j += i) div[j].eb(i);
    }
    ll sum = 0;
    rep2(i, 1, n) {
        ll x = i;
