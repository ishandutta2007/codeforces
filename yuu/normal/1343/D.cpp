...
void solve() {
    ...
    for (int i = 0; i <= k * 2; i++) cost[i] = 0;
    for (int i = 1; i * 2 <= n; i++) {
        int x = a[i];
        int y = a[n - i + 1];
        if (x > y) swap(x, y);
        // x < y
        cost[0] += 2;  // by default, we spend 2 move for everything
        // for all target, we only have to use 1 move if
        // 1 <= target - x <= k or 1 <= target - y <= k
        // or 1 + x <= target <= x + k
        // or 1 + y <= target <= y + k
        // x + k >= y + 1 because 1 <= x, y <= k
        // so 1 + x <= target <= y + k get discount of 1
        cost[x + 1] -= 1;
        cost[y + k + 1] += 1;
        // finally x + y get an extra discount because we don't need to change anything
        cost[x + y] -= 1;
        cost[x + y + 1] += 1;
    }
    for (int i = 1; i <= k * 2; i++) cost[i] += cost[i - 1];
    cout << *min_element(cost + 1, cost + k * 2 + 1) << '\n';
}
