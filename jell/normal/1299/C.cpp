...
struct solver
{
    ...
    solver()
    {
        ...
        vector<int> hull; // index
        for(int i=n; i>=0; --i)
        {
            while(hull.size()>1)
            {
                int m=hull.size();
                int i1=hull[m-1],i2=hull[m-2];
                assert(i1<i2);
                if((pref[i2]-pref[i1])*(i1-i) > (i2-i1)*(pref[i1]-pref[i]))
                {
                    break;
                }
                hull.pop_back();
        ...
        while(hull.size()>1)
        {
            ...
            assert(i1<i2);
            dump(i1,i2);
            long double avg=(long double)(pref[i2]-pref[i1])/(i2-i1);
            for(int i=i1; i<i2; ++i) ans[i]=avg;
        }
        cout << ans << "\n";
    }
}; // struct solver