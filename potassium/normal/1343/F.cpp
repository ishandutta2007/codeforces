...
void erase(vector<int>& occ, const vector<int>& toErase, vector<int>& lastSeen, int currentT) {
	for (auto& i : toErase) {
		occ[i]--;
		chkmin(lastSeen[i], (int)toErase.size() + currentT - 1);
	}
}

void test(int n, int v, const vector<vector<int>>& a, const vector<vector<int>>& app, vector<int> occ) {
	ans.clear();
	vector<bool> isErased(n - 1, false);
	vector<int> isUsed(n + 1, false);
	vector<int> lastSeen(n + 1, INT_MAX);
	int lastUsed;
	
	ans.pb(v);
	for (auto& cur : app[v]) {
		erase(occ, a[cur], lastSeen, 0);
		isErased[cur] = true;
		isUsed[v] = true;
		lastUsed = cur;
	}
	...
	for (int t = 1; t + 1 < n; t++) {
		int takenIdx = 0;
		for (auto& i : a[lastUsed]) {
			if (occ[i] == 1)
				if (lastSeen[i] < lastSeen[takenIdx])
					takenIdx = i;
		}
		if (takenIdx == 0 || lastSeen[takenIdx] == INT_MAX) {
			isFound = 0;
			break;
		} else {
			int eraseIdx = -1;
			for (auto& cur : app[takenIdx])
				if (!isErased[cur]) {
					eraseIdx = cur;
					break;
				}
			erase(occ, a[eraseIdx], lastSeen, t);
			isErased[eraseIdx] = true;

			ans.pb(takenIdx);
			isUsed[takenIdx] = true;
			lastUsed = eraseIdx;
		}
	}
