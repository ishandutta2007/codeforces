...
void solve() {
    cin >> s >> t;
    n = s.size(); m = t.size();
    sl.rsz(n,1); tl.rsz(m,1);
    FOR(i,1,n) if (s[i]!=s[i-1]) sl[i] = sl[i-1]+1;
    FOR(i,1,m) if (t[i]!=t[i-1]) tl[i] = tl[i-1]+1;
    a.rsz(n, vm(m));
    b.rsz(n, vm(m));
    mi ans = 0;
    F0R(i, n) F0R(j, m) {
        if (i>0) {
            ...
            if (s[i] != t[j]) a[i][j] += b[i-1][j];
        }
        if (s[i]!=t[j]) a[i][j] += tl[j];
        if (j>0) {
            if (t[j] != s[i]) b[i][j] += a[i][j-1];
            if (t[j] != t[j-1]) b[i][j] += b[i][j-1];
        }
        if (t[j]!=s[i]) b[i][j] += sl[i];
        ans += a[i][j] + b[i][j];
    }
    cout << ans << nl;
}
