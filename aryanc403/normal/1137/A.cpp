...
const lli maxN = 1000L;

    lli T,n,i,j,k,in,cnt,u,v,x,y;
    lli m;
    string s;
    lli a[maxN+5][maxN+5];
    lli row[maxN+5][maxN+5],col[maxN+5][maxN+5];
    lli szr[maxN+5],szc[maxN+5];
...
void preProcess()
{
    ...
    fo(i,n)
    {
        ...
        szr[i]=(lli)c.size();
        fo(j,m)
            row[i][j]=lower_bound(c.begin(),c.end(),a[i][j])-c.begin();
    }
    ...
    fo(j,m)
    {
        ...
        szc[j]=(lli)c.size();
        fo(i,n)
            col[i][j]=lower_bound(c.begin(),c.end(),a[i][j])-c.begin();
    }
}
...
// cin>>T;while(T--)
{
    ...
    fo(i,n)
    {
        fo(j,m)
        {
            lli x=row[i][j]+1,y=col[i][j]+1;
            // dbg(i,j,x,y);
            if(x>y)
                cout<<max(szr[i],szc[j]+x-y)<<" ";
            else
                cout<<max(szr[i]+y-x,szc[j])<<" ";
        }
        cout<<endl;
    }
