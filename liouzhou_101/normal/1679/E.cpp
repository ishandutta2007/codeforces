...
void solve()
{
	...
	vector<vector<modint>> a(18, vector<modint>(1 << 17));
	for (int i = 0; i < n; ++i)
	{
		{
			int L = i, R = i;
			...
			while (1)
			{
				...
				for (int k = 1; k <= 17; ++k)
				{
					a[k][cur] += modint(k).Pow(free + (c[n] - c[R + 1]) + (c[L]));
				}
				L -= 1; R += 1;
				if (L < 0 || R >= n) break;
			}
	...
	for (int k = 1; k <= 17; ++k)
		BitTransform<modint>::ZetaTransform(a[k]);
	int q;
	read(q);
	while (q--)
	{
		...
		for (auto c : s)
			cur |= 1 << (c - 'a');
		writeln(a[bit_popcount(cur)][cur].GetValue());
	}
}
