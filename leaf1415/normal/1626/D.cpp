...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> vec;
		for(auto p : mp) vec.pb(p.snd);
		ll m = sz(vec);
		
		vector<ll> svec; ll s = 0;
		svec.pb(0);
		rep(i, 0, m-1) s += vec[i], svec.pb(s);
		
		ll ans = inf;
		rep(i, 0, m){
			chmin(ans, c[svec[i]] + c[0] + c[svec[m]-svec[i]]);
			rep(j, 0, 19){
				ll p = upper_bound(all(svec), svec[i]+(1<<j)) - svec.begin() - 1;
				chmin(ans, c[svec[i]] + c[svec[p]-svec[i]] + c[svec[m]-svec[p]]);
				if(p == m) break;
			}
		}
		outl(ans);
