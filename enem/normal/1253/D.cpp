...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            st[fn(i,rn)].F=min(st[fn(i,rn)].F,i);
            st[fn(i,rn)].S=max(st[fn(i,rn)].S,i);
        }
        ll ans=0;
        REP(i,0,n)
        {
            ...
            REP(j,i,r+1)
            {
                if(fn(j,rn)!=fn(i,rn))
                {
                    st[fn(j,rn)].F=st[fn(i,rn)].F=min(st[fn(i,rn)].F,st[fn(j,rn)].F);
                    r=st[fn(j,rn)].S=st[fn(i,rn)].S=max(st[fn(i,rn)].S,st[fn(j,rn)].S);
                    un(i,j,rn,sz);
                    ans++;
                }
            }
