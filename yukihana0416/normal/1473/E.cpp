...
void work()
{
            ...
            for(auto pr : G[u])
            {
                ...
                if(~s & 1)
                {
                    if(dp[v][s | 1] > dp[u][s])
                    {
                        ...
                        {
                            inq[v][s | 1] = 1;
                            q.emplace_front(mk(v, s | 1));
                        }
                    }
                }
                if(~s & 2)
                {
                    if(dp[v][s | 2] > dp[u][s] + w * 2)
                    {
                        dp[v][s | 2] = dp[u][s] + w * 2;
                        if(!inq[v][s | 2])
                        {
                            inq[v][s | 2] = 1;
                            q.emplace_back(mk(v, s | 2));
                        }
                    }
                }
            }
    ...
    for(int i = 2; i <= n; i++) cout <<dp[i][3] <<" "; cout <<endl;
}
