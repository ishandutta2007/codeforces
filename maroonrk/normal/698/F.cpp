...
void slv(){
	...
	vi to(n+1,-1),ot(n+1,-1);
	vi tp(n+1,1);
	rng(i,2,n+1)if(pri[i]){
		tp[i]=n/i;
		if(tp[i]>=i)tp[i]=-1;
	}else{
		tp[i]=-1;
	...
	vi cnt(n+1);
	rng(i,1,n+1)cnt[g[i]]++;
	rng(i,1,n+1)if(a[i])cnt[g[a[i]]]--;
	//cerr<<cnt<<endl;
	mint ans=1;
	rng(i,1,n+1)ans*=fact[cnt[i]];
	vi u(n+1);
	rng(i,1,n+1)if(tp[i]!=-1&&to[i]==-1)
		u[tp[i]]++;
	rng(i,1,n+1)ans*=fact[u[i]];
	print(ans);
}
