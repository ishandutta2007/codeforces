...
public:
    void solve() {
        ...
        Graph<BiWeightedEdge<ll>> graph(n);
        for (int i : range(m)) {
            graph.addEdge(u[i], v[i], w[i]);
        }
        auto res = dijkstra(graph, 0);
        if (res[n - 1].second.first == -1) {
            out.printLine(-1);
            return;
        }
        vi answer(1, n);
        int cur = n - 1;
        while (cur) {
            if (cur != res[cur].second.second->to) {
                throw "Jopa";
            }
            cur = res[cur].second.first;
