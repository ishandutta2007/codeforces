...
vector<pii> get_rgt(const vector<pair<pii, int>> &segm) {
    fill(all(cnt), 0);
    for (auto i : segm) {
        ++cnt[i.f.s];
    }
    ...
    vector<pii> ans(sz(segm));
    for (auto i : segm) {
        ans[--cnt[i.f.s]] = {i.f.s, i.s};
    }
    return ans;
}

vector<pii> get_lft(const vector<pair<pii, int>> &segm) {
    fill(all(cnt), 0);
    for (auto i : segm) {
        ++cnt[i.f.f];
    }
...
signed main() {
    ...
    while (clock() < 2.9 * CLOCKS_PER_SEC) {
        ...
        // dbg(segm);
        // return 0;
        auto rgt = get_rgt(segm);
        auto lft = get_lft(segm);
        int ptr = 0;
        int mn = mod;
        for (auto s : lft) {
            while (ptr < sz(rgt) && rgt[ptr].f < s.f) {
                chmin(mn, rgt[ptr].s);
                ++ptr;
            }
            if (ptr > 0) {
