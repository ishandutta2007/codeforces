...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, m) cin >> b[i];
		ll k = 0;
		rep(i, 1, m) k = gcd(k, b[i]);
		
		rep(i, 0, k-1) vec[i].clear();
		rep(i, 1, n) vec[i%k].pb(a[i]);
		rep(i, 0, k-1) sort(all(vec[i]));
		
		rep(i, 0, k-1){
			mx[i][0] = mx[i][1] = -inf;
			ll sum = 0, psum = 0;
			reps(j, vec[i]) sum += vec[i][j];
			reps(j, vec[i]){
				chmax(mx[i][j%2], sum-2*psum);
				psum += vec[i][j];
			}
			chmax(mx[i][sz(vec[i])%2], sum-2*psum);
		}
		ll ans = -inf;
		rep(i, 0, 1){
			ll tmp = 0;
			rep(j, 0, k-1) tmp += mx[j][i];
			chmax(ans, tmp);
		}
		cout << ans << "\n";
	}
