...
struct splay_tree <%
  ...
  bool is_left_child(node_t* x) const <%
    return x != nil  and  x->is_left_child();
  %>
  ...
  node_t* next(node_t* u) const <%
    ...
    %>
    while (u != nil  and  !is_left_child(u)) <%
      u = u->parent;
    %>
    u = u != nil ? u->parent : u;
    return u;
  ...
  node_t* at(int i) <%
    if (i < 0  or  i >= size()) return nil; // do we want to throw?
    node_t* u = root;
    while (true) <%
      if constexpr (has_push) push(u);
...
struct splay_node_base_common <%
  ...
  bool is_left_child() const <% return parent == nil  or  this == parent->left; %>
