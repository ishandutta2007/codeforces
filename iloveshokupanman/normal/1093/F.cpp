...
int main() {
    ...
    vvll c(N + 1, vll(K + 1, 0));

    repn(i, N)repn(j, K) {
        if (a[i - 1] == -1 || a[i - 1] == j) { c[i][j] = c[i - 1][j]; }
        else { c[i][j] = i - 1; }

        c[i][j] = max(c[i][j], i - L+1);

        //cout << i << " " << j << " " << c[i][j] << endl;
    }

    vvmint dp(N + 1, vmint(K + 1, 0));
    vvmint rdp(N + 1, vmint(K + 1, 0));
    ...
    repn(i, N) {
        repn(j, K) {
            if (a[i] == -1 || a[i] == j) {
                dp[i][j] = rdp[i - 1][0];
                dp[i][j] -= rdp[i - 1][j];
                if (c[i][j] > 0)dp[i][j] -= rdp[c[i][j] - 1][0] - rdp[c[i][j] - 1][j];
                dp[i][0] += dp[i][j];
            }
