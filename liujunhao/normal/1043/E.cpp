...
LL ans[MAXN+10],sumx[MAXN+10],sumy[MAXN+10];
main()
{
	Read(n);
	Read(m);
	int i,u,v;
	for(i=1;i<=n;i++)
		Read(a[i].x),Read(a[i].y),a[i].pos=i;
	sort(a+1,a+n+1,[&](node x,node y){return x.x-x.y<y.x-y.y;});
	for(i=1;i<=m;i++){
		Read(u),Read(v);
		adj[u].pb(v);
		adj[v].pb(u);
	...
	for(i=1;i<=n;i++)
		ans[a[i].pos]=1ll*a[i].x*(n-i)+sumy[i+1]+1ll*a[i].y*(i-1)+sumx[i-1];
	sort(a+1,a+n+1,[&](node x,node y){return x.pos<y.pos;});
	for(i=1;i<=n;i++){
		for(auto j:adj[i])
			ans[i]-=min(a[i].x+a[j].y,a[i].y+a[j].x);
		printf("%lld ",ans[i]);
	}
}