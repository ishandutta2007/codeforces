...
void work()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        reader >>m;
        G[i] = vector<int> (m);
        for(int j = 0; j < m; j++) reader >>G[i][j];
    }

    vector<LL> s(n, 0);
    for(int i = 0; i < n; i++)
    {
        int sz = min(k, (int)G[i].size());
        for(int j = 0; j < sz; j++) s[i] += G[i][j];
    }

    LL ans = 0;

    function<void(int, int, int, vector<LL>)> dfs = [&](int l, int r, int id, vector<LL> dp)
    {
        if(l == r)
        {
            LL pre = 0;
            get_max(ans, dp[k]);
            int sz = min(k, (int)G[l].size());
            for(int i = 0; i < sz; i++)
            {
                pre += G[l][i];
                get_max(ans, pre + dp[k - 1 - i]);
            }
            return ;
        }
        ...
        for(int i = mid + 1; i <= r; i++)
        {
            int sz = min(k, (int)G[i].size());
            for(int j = k; j >= sz; j--)
                get_max(dpl[j], dpl[j - sz] + s[i]);
        }
        for(int i = l; i <= mid; i++)
        {
            int sz = min(k, (int)G[i].size());
            for(int j = k; j >= sz; j--)
                get_max(dpr[j], dpr[j - sz] + s[i]);
        }
