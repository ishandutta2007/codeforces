...
li solve(){
  li n; cin>>n;
  string a[2]; for(li i = 0; i < (li)(2); ++i) cin>>a[i];

  segment_tree<max_segment_op<li>> st(n+1);
  st.set_point(0, 0);
  st.build();

  auto get = [&](li x, li y) -> li {
    if(x <= y) return st.query_range(x,y);
    else return -(1ll<<50);
  };
  ...
  for(li i = 0; i < (li)(n); ++i) {
    ...
    li cur = max(get(0, min(last0,last1)) + 2,
                 max(get(last1+1, last0) + 1,
                     get(last0+1, last1)));

    st.set_point(i+1,cur);
  }
...
int main(){
  ...
  int t; cin >> t; for(li i = 0; i < (li)(t); ++i) {
    li x =solve();
    cout << x << endl;
  }
