...
int todep(int v,int d){
  for(int i=sz(J[v])-1;i>=0;--i) if(D[v]-d>=(1<<i)) v=J[v][i];
  return v;
}
int lca(int a,int b){
  if(todep(a,0)!=todep(b,0)) return -1;
  int d=min(D[a],D[b]);
  a=todep(a,d);
  b=todep(b,d);
  if(a==b) return a;
  for(int i=sz(J[a])-1;i>=0;--i) if(J[a][i]!=J[b][i]) a=J[a][i],b=J[b][i];
  assert(J[a][0]==J[b][0]);
  return J[a][0];
}
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    bool r;
    if(t==1){
      r=todep(b,D[a])==a && C[b][1]==C[a][1] && a!=b;
    }else{
      int c=lca(a,b);
      r=c!=-1 && C[b][0]==C[c][0] && C[a][1]==C[c][1] && b!=c;
    }
    cout<<(r?"YES\n":"NO\n");
  }
