...
pair<int, ll> x_and_min_inverse(vi &v, int keta) {
	int n = v.size();
	int bit_mask = 0;

	// vector<int> vertices;
	rep(i, keta, 30) { bit_mask |= (1 << i); }

	vertices.clear();
	rep(i, n) {
		int val = v[i];
		vertices[bit_mask & val].push_back(i);
	}

	ll low = 0;
	ll high = 0;
	foa(t, vertices) {
		if(t.first & (1 << keta)) continue;
		if(vertices.count(t.first ^ (1 << keta))) {
			auto &a = vertices[t.first];
			auto &b = vertices[t.first ^ (1 << keta)];
			int sz = b.size();
			foa(val, a) {
				auto it = lb(b, val);
				low += it;
	...
	return (low <= high ? make_pair(0, low) : make_pair(1, high));
}
...
void solve() {
	...
	rep(keta, 30) {
		int bit;
		ll inv;
		tie(bit, inv) = x_and_min_inverse(a, keta);
		x |= bit << keta;
		ans += inv;
	}
	print(ans, x);
