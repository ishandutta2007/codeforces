...
void solve(){
    ...
    for (auto& c : s1) c = c-'0'+'a';
    for (auto& c : s2) c = c-'0'+'a';
    SuffixArray cmp_l = SuffixArray::construct(s1);
    SuffixArray cmp_r = SuffixArray::construct(s2);


    auto _comp_l = [&](int i1, int i2, int len) { 
        bool ans = cmp_l.rank[i1] < cmp_l.rank[i2] && cmp_l.get_lcp(i1, i2) < len;
        return ans;
    };

    auto leq_l = [&](int i1, int i2, int len) {
        return !_comp_l(i2, i1, len);
    };

    auto _comp_r = [&](int i1, int i2, int len) { 
        bool ans = cmp_r.rank[i1] < cmp_r.rank[i2] && cmp_r.get_lcp(i1, i2) < len;
        return ans;
    };

    auto leq_r = [&](int i1, int i2, int len) {
        return !_comp_r(i2, i1, len);
    };

    auto good_l = [&](int i, int len) -> bool {
        if (len < sz(l)) return 0;
        if (len > sz(l)) return 1;
        return leq_l(sz(a), i, len);
    };
    auto good_r = [&](int i, int len) -> bool {
        if (len < sz(r)) return 1;
        if (len > sz(r)) return 0;
        return leq_r(i, sz(a), len);
    };

    int n = sz(a);
    vector<num> dp(n), pre(n);
    int l_bound = 0, r_bound = 0;
    for (int i = 0; i < n; i++) {
        l_bound++, r_bound--;
        if (r_bound < 0) r_bound++;
        //int l_bound = i+1, r_bound = 0;
        while (good_l(i-(l_bound-1)+1, l_bound-1)) l_bound--;
        while (r_bound <= i && good_r(i-(r_bound+1)+1, r_bound+1)) r_bound++;

        auto get = [&](int l, int r) -> num {
        ...
        if (good_l(0, i+1)) {
            dp[i] += get(i-r_bound+1, i-l_bound+1);
            if (sz(l) == 1 && l[0] == '0') {
                if (a[i] == '0') dp[i] += (i ? dp[i-1] : 1);
            }
        }
        ...
        if (a[i] != '0') {
            for (int j = i; j >= 0; j--) {
                ...
                if (good_l(j, len) && good_r(j, len)) {
                    cerr << i << ' ' << len << ' ' << l_bound << ' ' << r_bound << '\n';
                    assert(l_bound <= len && len <= r_bound);
                    //num pdp = (j ? dp[j-1] : 1);
                    //dp[i] += pdp;
                } else {
                    cerr << i << ' ' << len << ' ' << l_bound << ' ' << r_bound << '\n';
                    assert(len < l_bound || len > r_bound);
                }
            }
        }
        */
