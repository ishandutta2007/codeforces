...
int main()
{
	...
	vector<long long int>ans(1,v[0]);
	for (int i = 0; i < N; ++i) {
		long long int k=v[i];
		for (long long int j = 1; j <= 1e10; j *= 2) {
			auto it=lower_bound(v.begin(),v.end(),k+j);
			if(it==v.end())continue;
			else {
				if (*it == k + j) {
					if(ans.size()<=2) ans = vector<long long int>{ k,k + j };
					auto jt=lower_bound(v.begin(),v.end(),k+2*j);
					if(jt==v.end())continue;
					else {
						if (*jt == k + 2 * j) {
							ans = vector<long long int>{ k,k + j,k + 2 * j };
						}
					}
				}
			}
