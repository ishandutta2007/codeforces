...
ll n,q,x[MAX],a,b;
arll(3) tree[1<<18],ret;

inline arll(3) merge(arll(3) le,arll(3) ri){
	ret[0] = le[0]+ri[0];
	ret[1] = max(le[1],le[0]+ri[1]);
	ret[2] = min(le[2],le[0]+ri[2]);
	return ret;
}

void build(int id,int le,int ri){
	if(le==ri){
		tree[id] = {x[le],max(0ll,x[le]),min(0ll,x[le])};
		return;
	}
	int mid = le+ri>>1;
...
//	db(tree[1][2]);
	while(q--){
		cin>>a>>b;
		ret = que(1,1,n);
//		rep(i,0,2)cout<<ret[i]<<" "; cout<<endl;
		if(ret[0]!=0 || ret[2]<0)cout<<"-1\n";
