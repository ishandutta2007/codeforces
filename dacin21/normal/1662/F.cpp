...
struct Seg{
    ...
    Seg(int n_) : n(n_), v(4*n) {}


    void update(int u, int a, int b, int const&l, int const&r, pair<int, int> const&val){
        if(r<=a || l>=b) return;
        if(l<=a && b<=r){
            v[u].insert(val);
            return;
        ...
        const int m = a + (b-a)/2;
        update(2*u, a, m, l, r, val);
        update(2*u+1, m, b, l, r, val);
    }
    template<typename T>
    void q(int u, int a, int b, int x, T fun){
        if(x<a || x>=b) return;
        fun(v[u]);
        if(b-a == 1) return;
        const int m = a + (b-a)/2;
...
void solve(){
    ...
    for(int i=0; i<n; ++i){
        cin >> p[i];
        st.update(1, 0, st.n, i-p[i], i+p[i]+1, make_pair(i, p[i]));
    }
    queue<int> q;
    ...
    while(!q.empty()){
        ...
        q.pop();
        const int i = u;
        st.q(1, 0, st.n, i, [&](set<pair<int, int> > &s){
            auto it1 = s.lower_bound(make_pair(i-p[i], -inf));
            auto it2 = s.lower_bound(make_pair(i+p[i], inf));
            for(auto it = it1; it != it2; ++it){
                enqueue(it->first, dist[u]+1);
            }
            s.erase(it1, it2);
        });
    }
