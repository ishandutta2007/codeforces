...
// Problem: C1: Simple Polygon Embedding (https://codeforces.com/contest/1354/problem/C1)
...
int main() {
    ...
    while (t--) {
        int n; cin >> n;
        Point p;
        ld ang_base = PI / n, ang = 0;
        rep(i, n) {
            p += Point(sin(ang), cos(ang));
            ang += ang_base;
        }
        ld dist = (p + Point(sin(ang), cos(ang) / 2) - Point(0.0, 0.5)).len();
        cout << dist << '\n';
    }
