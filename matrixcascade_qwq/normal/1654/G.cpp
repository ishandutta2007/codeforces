...
signed main()
{
	...
	while(T--)
	{
		...
		pu(i,n,1)
		{
			...
			deque<int>q;
			fp g[i] pf q.push_back(to),dis[to]=i;
			while(q.size())
			{
				int x=q.front();q.pop_front();
				fp v[x] pf
				{
					if(dis[to]>dis[x]+1&&(d[x]==d[to]||(dis[x]==d[x]&&d[x]<d[to])))
					{
						dis[to]=dis[x]+1;q.push_back(to);
					}
					else if(dis[to]>dis[x]&&(d[to]>d[x])&&dis[x]>d[x])
					{
						dis[to]=dis[x];q.push_front(to);
					}
				}
			}
			up(j,1,n)if(dis[j]<=d[j])gg[j]=i;
		}
		upn
		{
			if(!gg[i])opls(d[i]-1);
