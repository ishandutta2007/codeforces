...
int a[N];
int goright(int x) {
	pair<int, int> dp[2] = {{a[x], 0}, {n + 1, a[x]}};
	for(int i = x + 1; i <= n; i++) {
		int dec = inf, inc = -inf;
		for(int d = 0; d < 2; d++) {
			if(a[i] < dp[d].fi) {
		...
		//cout << dec << ' ' << inc << endl;
		dp[0] = make_pair(a[i], dec);
		dp[1] = make_pair(inc, a[i]);
		if(dec == inf && inc == -inf) return i - 1;
	}
	return n;
}
int goleft(int x) {
...
int main() {
	...
	while(ri != n) {
		...
		ri = goright(le);
		//cout << le << ' ' << ri << endl;
		ans = ans + (LL)(ri - le + 1 + prvri - le + 2) * (ri - prvri) / 2;
		prvri = ri;
	}
	cout << ans << endl;
}