#line 1 "codeforces-workspace/contest/1451/e1/e1.cpp"
/*
 * @file template.cpp
 * @brief Template
 */
...
#line 12 "codeforces-workspace/contest/1451/e1/e1.cpp"
...
void main() {
  ...
  if (~fi) {
    cout << "OR " << fi + 1 << " " << se + 1 << endl;
    vector<int> ans(n);
    int ret;
    cin >> ret;
    for (auto i = 0; i < n; ++i) {
      ans[i] = res[i] ^ res[fi] ^ ret;
    }
    cout << "! " << ans << endl;
    return;
  }
  int ret1, ret2;
  cout << "AND 1 " << xo[1] + 1 << endl;
  cin >> ret1;
  cout << "AND 1 " << xo[2] + 1 << endl;
  cin >> ret2;
  int ret = ret1 | ret2;
  cout << "!";
  for (auto i = 0; i < n; ++i) {
