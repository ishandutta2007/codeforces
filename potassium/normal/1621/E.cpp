...
int main() {
	CASE(t) {
		...
		vector<S> st_base;
		for (int i = 0; i < n; i++) {
			if (st_val.empty() || a[i] > st_val.back()) {
				st_val.emplace_back(a[i]);
				st_base.emplace_back(S{ 1 });
			}
			else {
				st_base.back().val++;
			}
		}
		st_val.emplace_back(INT_MAX);
		st_base.emplace_back(S{ 0 });

		const int len = st_val.size();
		for (int i = len - 2; i >= 0; i--)
			st_base[i].val += st_base[i + 1].val;
		SegTree<S, op, e, F, mapping, composition, id> st = st_base;

		auto add = [&](ll val, int x) {
			int pos = lower_bound(st_val.begin(), st_val.end(), val) - st_val.begin();
			st.apply(0, pos + 1, F{ x });
		};
		...
		for (int i = 0; i < m; i++) {
			...
			for (int j = 0; j < k[i]; j++) {
				ll req = (sum[i] - b[i][j] + k[i] - 2) / (k[i] - 1);
				add(req, -1);
				ans += "01"[st.all_prod().val >= 0];
				add(req, 1);
			}
