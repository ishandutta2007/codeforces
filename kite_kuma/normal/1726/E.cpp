...
#line 2 "SPJ-Library/combinatorics/combination.hpp"
...
int main() {
	...
	cin >> t;
	combination<mint> c;
	constexpr int val_max = 312345;
	vector<mint> dbl(val_max, 1);
	rep(i, 2, val_max) { dbl[i] = dbl[i - 2] * i; }
	...
	rep(t) {
		...
		rep(four, n + 10) {
			debug(four);
			int one_plus_two = n - four * 4;
			if(one_plus_two < 0) break;
			const int adj = four * 2;
			mint res = mint(2).pow(four);
			debug(four);
			res *= c.comb(one_plus_two + adj, adj);
			debug(four, adj);
			if(adj) res *= dbl[adj - 1];
			debug(four);
			res *= dp[one_plus_two];
			debug(four);
			ans += res;
