...
int main(int argc, char const *argv[]) {
  ...
  mint sum;
  for (int i = S; i >= 1; i--) {
    f[i] = fac[n - i] * fac[i - 1] * C(n - S, i - 1) - sum * iv[i];
    sum += f[i];
  }
  for (int i = 1; i <= n; i++) std::cout << f[i].val() << ' ';
  return 0;
}