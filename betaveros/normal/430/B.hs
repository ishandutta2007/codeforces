...
destroy :: (Eq a) => [a] -> [a] -> Int
destroy (x:xs) (y:ys) | x == y = let
	(xs1, xs2) = span (== x) xs
	(ys1, ys2) = span (== y) ys
	c = length xs1 + length ys1 + 2
	in if c >= 3 then c + destroy xs2 ys2 else 0
destroy _ _ = 0

bestDestroy :: (Eq a) => a -> [a] -> Int
bestDestroy x = go []
	where
		go as bs@(b:br) = max (destroy as (x:bs)) (go (b:as) br)
		go _ [] = 0
...
main = do
	[_,_,x] <- inputInts
	xs <- inputInts
	print . max 0 $ bestDestroy x xs - 1