...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, n) rep(j, 0, 10000) dp[i][j] = inf;
		dp[0][0] = 0;
		rep(i, 0, n-1){
			rep(j, 0, 10000){
				if(j+a[i+1] <= 10000) chmin(dp[i+1][j+a[i+1]], dp[i][j] + 2*j*a[i+1] + 2*(sum[i]-j)*b[i+1]);
				if(j+b[i+1] <= 10000) chmin(dp[i+1][j+b[i+1]], dp[i][j] + 2*j*b[i+1] + 2*(sum[i]-j)*a[i+1]);
			}
		}
		
		ll ans = inf;
		rep(i, 0, 10000) chmin(ans, dp[n][i]);
