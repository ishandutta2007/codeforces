...
int main(){
  ...
  bool judge = true;
  auto ff=[&](P x,P y){
    return MP(max(x.fi,y.fi),max(x.se,y.se));
  };
  auto gg=[&](P x,ll y){
    if(y)swap(x.fi,x.se);return x;
  };
  auto hh=[&](ll x,ll y){
    return x^y;
  };
  ll n;cin>>n;
  using L=LazySegmentTree<P,ll,decltype(ff),decltype(gg),decltype(hh)>;
  vector<L>seg(2,L(n,ff,gg,hh,MP(0LL,0LL),0LL));
  vector<vector<ll>>g(n),gf(n);
  vector<ll>u(n-1),v(n-1),flip(n-1);
  rep(i,0,n-1){
    cin>>u[i]>>v[i]>>flip[i];u[i]--;v[i]--;
    g[u[i]].PB(v[i]);
    g[v[i]].PB(u[i]);
    gf[u[i]].PB(flip[i]);
    gf[v[i]].PB(flip[i]);
  ...
  Diameter dia(g);
  vector<ll>h(2);
  h[0]=dia.root;h[1]=dia.end;
  auto in=vec(2,n,0LL);
  auto out=vec(2,n,0LL);
  {
    auto dfs=[&](auto &&f,ll k,ll par,ll d,ll flip,ll &time,ll tt)->void{
      in[tt][k]=time++;
      P input(d,0);
      if(flip)swap(input.fi,input.se);
      seg[tt].set(in[tt][k],input);
      rep(i,0,g[k].size()){
        if(g[k][i]==par)continue;
        f(f,g[k][i],k,d+1,flip^gf[k][i],time,tt);
      }
      out[tt][k]=time;
    };
    rep(i,0,2){
      ll time=0;
      dfs(dfs,h[i],-1,0,0,time,i);
      seg[i].build();
    }
  }
  ll q;cin>>q;
  while(q--){
    ...
    rep(i,0,2){
      ll t=u[s];
      if(in[i][u[s]]<in[i][v[s]])t=v[s];
      seg[i].update(in[i][t],out[i][t],1);
      chmax(mx,seg[i].query(0,n).fi);
    }
    cout<<mx<<endl;
  }
  return 0;
