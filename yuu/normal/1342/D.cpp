...
void solve() {
    cin >> n >> k;
    m | ARRAY | 1 | n | ARRAY_INPUT | SORT | REVERSE;
    c | ARRAY | 1 | k | ARRAY_INPUT;

    int required = 1;
    for (int i = 1; i <= n; i++) ans[i].clear();
    for (int i = 1; i <= n; i++) {
        int low = 1, high = required, mid, res = required + 1;
        while (low <= high) {
            mid = (low + high) / 2;
            if (ans[mid].size() < c[m[i]]) {
                res = mid;
                high = mid - 1;
            } else {
                low = mid + 1;
        ...
        if (res == required + 1) required++;
        ans[res].emplace_back(m[i]);
    }
    cout << required << '\n';
    for (auto&& x : ans | ARRAY | 1 | required) {
        cout << x.size();
        for (auto&& y : x) cout << ' ' << y;
        cout << '\n';
    }
