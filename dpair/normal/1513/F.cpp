...
struct NODE{
    int a, b, c;
    inline bool operator< (const NODE &tmp) const{return b < tmp.b;}
}p[MAXN];
int dp[2];
signed main(){
    read(n); read(a + 1, a + n + 1); read(b + 1, b + n + 1);
    LL ans = 0, ret = 0; rep(i, 1, n) ans += abs(a[i] - b[i]);
    ret = ans; rep(i, 1, n) {
        // debug("i = %d, a = %d, b = %d\n", i, a[i], b[i]);
        if(a[i] > b[i]) p[i].a = b[i], p[i].b = a[i], p[i].c = 1;
        else p[i].a = a[i], p[i].b = b[i], p[i].c = 0;
    } std::sort(p + 1, p + n + 1); dp[0] = dp[1] = INF; per(i, n, 1) {
        // debug("a = %d, b = %d, c = %d, 0 = %d, 1 = %d\n", p[i].a, p[i].b, p[i].c, dp[0], dp[1]);
        chmin(ans, ret - ((p[i].b - mx(dp[p[i].c ^ 1], p[i].a)) << 1));
        chmin(dp[p[i].c], p[i].a);
    } print(ans);
}
