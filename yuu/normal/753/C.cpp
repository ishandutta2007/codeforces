...
pair<int, int> evaluate(const string& x, const string& y) {
    int cow = 0, bull = 0;
    for (int i = 0; i < 4; i++)
        if (x[i] == y[i]) {
            cow++;
...
void guess(const string& s) {
    ...
    for (auto&& y : temp) {
        auto [b, c] = evaluate(s, y);
        if ((bull == b) && (cow == c)) solution_space.emplace_back(y);
    }
}
void solve() {
    for (int i = 0; i < 10000; i++) {
        ...
        for (int j = 0; j < 4; j++)
            for (int l = j + 1; l < 4; l++)
                if (t[j] == t[l]) goto bad;
        solution_space.emplace_back(t);
    bad:;
    }
    while (solution_space.size() >= 1) {
        cerr << solution_space.size() << '\n';
        pair<int, string> best{1e9, ""};
        if (solution_space.size() == 5040) {
            best = {0, "0123"};
        } else {
            for (auto&& x : solution_space) {
                fill(cnt, cnt + 25, 0);
                for (auto&& y : solution_space) {
                    auto [cow, bull] = evaluate(x, y);
                    cnt[cow * 5 + bull]++;
                    if (cnt[cow * 5 + bull] >= best.first) break;
                }
                best = min(best, {*max_element(cnt, cnt + 25), x});
            }
        }
        guess(best.second);
    }
