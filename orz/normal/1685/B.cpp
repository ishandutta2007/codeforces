...
bool find_ans(int a, int b, int c, int d, const string& s) {
	int A = 0, B = 0;
	for (char x : s)
		if (x == 'A')
			++A;
	...
	vi aaa(sz(aa[0]) + 1), bbb(sz(aa[1]) + 1);
	for (int i = 0; i < sz(aa[0]); ++i)
		aaa[i + 1] = aaa[i] + aa[0][i];
	for (int i = 0; i < sz(aa[1]); ++i)
		bbb[i + 1] = bbb[i] + aa[1][i];
	for (int i = c + d - pr; i >= 0; --i) {
		int j = c + d - pr - i;
		if (i < sz(aaa) && j < sz(bbb) && aaa[i] <= c && bbb[j] <= d)
			return true;
	}
	return false;
}
