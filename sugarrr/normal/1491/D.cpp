...
bool check(ll a,ll b){
    ...
    if((1ll<<k)==a){
        ll p=sai(b);
        if((1ll<<p)==b)return true;
        return false;
    }
    if(a+(1ll<<k)==b)return true;
    return check(a-(1ll<<k),b-(1ll<<k));
}*/
...
int main(){fastio
    ...
    rep(i,1,n){
        ...
        //cout<<i<<":";
        //rep(j,1,n)if(f[j])cout<<bitset<6>(j)<<" ";cout<<endl;
        rep(j,1,n)if(f[j])cout<<(j)<<" ";cout<<endl;
        rep(j,1,n){if(check(i,j))cout<<j<<" ";}cout<<endl;
    }
    //*/
    return 0;
}