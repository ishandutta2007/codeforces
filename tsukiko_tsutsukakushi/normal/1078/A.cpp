...
void solve() {
    ...
    auto di = [&](ld p, ld q, ld r, ld s) -> ld {
        return sqrtl((p - r) * (p - r) + (q - s) * (q - s));
    };
    rep(i, 2) {
        rep(j, 2) {
            ld res = 0.0;
            ld x3 = (i == 0 ? x1 : -(ld)(b * y1 + c) / (ld)a);
            ld y3 = (i == 0 ? -(ld)(a * x1 + c) / (ld)b : y1);
            res += (i == 0 ? abs(y1 - y3) : abs(x1 - x3));

            ld x4 = (j == 0 ? x2 : -(ld)(b * y2 + c) / (ld)a);
            ld y4 = (j == 0 ? -(ld)(a * x2 + c) / (ld)b : y2);
            res += (j == 0 ? abs(y2 - y4) : abs(x2 - x4));
