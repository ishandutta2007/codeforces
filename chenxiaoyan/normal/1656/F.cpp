...
int solve(int t) {
	int ans = 0, pre = 1, suf = n + 1;
	auto val = [&](int x, int y) { return a[x] * a[y] + t * (a[x] + a[y]); };
	int las = 0;
	REP(i, 1, n) if(a[i] + t < 0) las = i;
	REP(_, 1, n - 1) {
		ti3 t((i128)1 << 120, 0, 0);
		int a = 1, b = suf == n + 1 ? pre : n;
		int ae = las <= pre || las >= suf ? las : pre;
		int be = las + 1 <= pre || las + 1 >= suf ? las + 1 : suf == n + 1 ? pre : suf;
		int c = pre + 1, d = suf - 1;
		chkmn(t, mt(val(a, d), a, d));
		if(1 <= ae && ae <= n) chkmn(t, mt(val(ae, d), ae, d));
		chkmn(t, mt(val(b, c), b, c));
		if(1 <= be && be <= n) chkmn(t, mt(val(be, c), be, c));
		if(Z(t) == pre + 1) ++pre; else --suf;
		ans += X(t);
	}
	return ans;
}
...
void mian() {
	...
	int sum = 0;
	REP(i, 1, n) sum += a[i];
	if(sum + (n - 2) * a[1] > 0 || sum + (n - 2) * a[n] < 0) return puts("INF"), void();
	int t = -1e12;
	PER(i, 50, 0) if(t + (1ll << i) <= 1e12 && solve(t + (1ll << i) + 1) - solve(t + (1ll << i)) >= 0) t += 1ll << i;
	prt(solve(t + 1)), pc('\n');
}
