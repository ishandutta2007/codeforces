...
#line 3 "/home/maspy/compro/library/graph/tree_diameter.hpp"
...
void solve() {
  ...
  G.read_tree();
  auto deg = degree(G);
  auto [n, P] = tree_diameter(G);
  // if (n % 2 == 1) return print(-1);
  ll root = P[len(P) / 2];
  vi cand = {root, P[0], P.back()};
  HLD<decltype(G)> hld(G, root);

  auto check = [&](int r) -> bool {
    auto [dist, par] = bfs01(G, r);
    vvc<int> dat(MAX(dist) + 1);
    FOR(v, N) { dat[dist[v]].eb(deg[v]); }
    for (auto&& X: dat) {
