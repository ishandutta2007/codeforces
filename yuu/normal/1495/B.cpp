...
bool check(int x) {
    if ((p[x] < p[x + 1]) || (p[x] < p[x - 1])) return 0;
    int a = l[x];
    int b = r[x];
    if (a < b) swap(a, b);
    if (a % 2 == 0) a--;
    if (a >= b) return 0;
    return max(best_l[x - l[x]], best_r[x + r[x]]) < max(l[x], r[x]);
}
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) best_l[i] = max({best_l[i - 1], r[i], l[i]});
    for (int i = n; i >= 1; i--) best_r[i] = max({best_r[i + 1], r[i], l[i]});
    int ans = 0;
