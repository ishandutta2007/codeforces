...
int main() {
    ...
    if(n == 1) {
        ...
        while(s.size() >= 3) {
            ...
            int cnt = 0;
            for(auto &p:s) {
                if(cnt < n) a.emplace_back(p);
                else if(cnt < 2 * n) b.emplace_back(p);
                cnt++;
            }
            qes(a);
            STR(s1);
            if(s1 == "YES") {
                ...
                if(s2 == "YES") {
                    set<int> ns;
                    for(auto &aa:a) ns.insert(aa);
                    for(auto &bb:b) ns.insert(bb);
                    swap(s,ns);
                }
                else {
                    for(auto &bb:b) s.erase(bb);
                }
            }
            else {
                qes(a);
