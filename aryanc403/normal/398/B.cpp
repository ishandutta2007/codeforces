...
const lli maxN = 2000L;
    ...
    lli T,n,i,j,k,in,cnt,l,r,c,u,v,x,y;
    lli m;
    string s;
    set<lli> rows,cols;
    mytype dp[maxN+5][maxN+5];
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

mytype sol(lli r,lli c)
{
    vector<vector<mytype>> dp(r+1,vector<mytype>(c+1,0));
    repA(i,0,r)
    repA(j,0,c)
    {
        if(i==0&&j==0)
        ...
        if(i)
            dp[i][j]+=(n-j)*i*dp[i-1][j];

        if(i&&j)
            dp[i][j]+=j*i*dp[i-1][j-1];
        dp[i][j]/=n*n-(n-i)*(n-j);
        // dbg(i,j,dp[i][j]);
    }
    dbg(r,c);
