...
signed main() {
    ...
    set<int> points;
    multiset<int> segs;
    points.emplace(-1e12), points.emplace(1e12), segs.emplace(2e12);
    auto add_pt = [&](int x) {
        auto p = points.upper_bound(x);
        int l = *prev(p), r = *p;
        segs.erase(segs.lower_bound(r - l));
        segs.emplace(r - x), segs.emplace(x - l);
        points.emplace(x);
    };
    auto remove_pt = [&](int x) {
        points.erase(x);
        auto p = points.upper_bound(x);
        int l = *prev(p), r = *p;
        segs.erase(segs.lower_bound(r - x)), segs.erase(segs.lower_bound(x - l));
        segs.emplace(r - l);
    };
    auto solve = [&]() -> int {
        if (points.size() <= 4) { return 0; }
        return *prev(prev(points.end())) - *next(points.begin()) - *prev(prev(prev(segs.end())));
    };
    rep(i, n) {
        int a;
        cin >> a;
    ...
    while (q--) {
        int type, x;
        cin >> type >> x;
        if (type == 0) { remove_pt(x), cout << solve() << endl; }
        else { add_pt(x), cout << solve() << endl; }
    }
}