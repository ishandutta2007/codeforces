...
fun main(){
    solve.cases{
        ...
        val DPfree = LongArray(n+1){nhuge}
        val DP = LongArray(n+1){nhuge}
        val L = getline(n)
        ...
        for(i in n-1 downTo 0){
            val has = str[i] == '1'
            if(has){
                DPfree[i] = maxOf(DPfree[i], DPfree[i+1] + L[i])
                DP[i] = maxOf(DP[i], DP[i+1]+ L[i])
                DPfree[i] = maxOf(DPfree[i], DP[i+1])
            }else{
                DP[i] = maxOf(DP[i], DPfree[i+1] + L[i])
                DP[i] = maxOf(DP[i], DP[i+1])
            }
        }
        put(maxOf(DP[0], DPfree[0]))
