...
void solve()
{
	...
	for (int k = 0; k < n; ++k)
		for (int i = 0; i < n; ++i)
			for (int j = 0; j < n; ++j)
			{
				//freshmin(d[i][j], d[i][k] + d[k][j]);
				freshmin(c[i][j], c[i][k] + c[k][j]);
			}

	ll res = d[0][n - 1];

	vector<ll> minv(n, INF);
	for (int i = 0; i < n; ++i)
	{
		for (int j = 0; j < n; ++j)
			freshmin(minv[i], d[i][j]);
	}

	for (int i = 0; i < n; ++i)
		for (int j = 0; j < n; ++j)
		{
			freshmin(res, (c[i][0] + c[j][n - 1] + 1) * d[i][j]);
			freshmin(res, (c[i][0] + c[i][j] + c[i][n - 1] + 2) * minv[j]);
		}
	writeln(res);
	...
	while (!q.empty())
	{
		...
		for (int y = 0; y < n; ++y)
		{
			if (freshmin(dis[y], dis[x] + d[x][y]))
				u[y] = 1;
			for (int z = 0; z < n; ++z)
			{
				if (freshmin(dis[y], dis[x] + tmp_d[x][z] * (c[z][y] + 1)))
					u[y] = 1;
			}
		}
		for (int y = 0; y < n; ++y)
