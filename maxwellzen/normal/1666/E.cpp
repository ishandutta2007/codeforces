...
bool check(int mn, int mx) {
    ...
    F0R(i, n-1) {
        x += mn; y += mx;
        if (x>a[i+1] || y<a[i]) return false;
        ckmin(y, a[i+1]); ckmax(x, a[i]);
    }
    x += mn; y += mx;
    return x<=l && y>=l;
}
...
void solve() {
    ...
    for (int step = 1<<30; step > 0; step /= 2) {
        if (mx>step && check(mx-step)) mx -= step;
    }
    int mn = 1;
    for (int step = 1<<30; step > 0; step /= 2) {
        if (mn+step <= mx && check(mn+step, mx)) mn += step;
    }
    c.rsz(n, 0);
    FOR(i, 1, n) {
        c[i] = min(a[i], c[i-1]+mx);
    }
    pts.rsz(n+1); pts[0] = 0; pts[n] = l;
    ROF(i, 1, n) {
        if (pts[i+1]-mn <= c[i]) pts[i] = pts[i+1]-mn;
        else pts[i] = c[i];
    }
    F0R(i, n) cout << pts[i] << ' ' << pts[i+1] << nl;
}
