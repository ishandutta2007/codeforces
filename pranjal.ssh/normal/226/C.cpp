...
const int N = 1e5;
inline void fib_mat_mul(ll F[2][2], ll M[2][2],ll MOD)
{
...
void mat_pow(ll F[2][2], ll n, ll MOD)
{
  if( n == 0 || n == 1)
      return;
  ll M[2][2] = {{1,1},{1,0}};
...
{
  boost;
  ll m,l,r,k;cin>>m>>l>>r>>k;
  ll ans=0;
  --l;
  int nr = sqrtl(r), nl = sqrt(l);
  FOR(i,1,nr)
    if(r/i - l/i >= k)
      ans=max(ans,i+0LL);
  FOR(i,1,nr)
