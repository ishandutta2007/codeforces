...
fn solve(input: &mut Input, output: &mut Output, _test_case: usize) {
    let side: String = input.read();
    let handle: u8 = input.read();
    let answer = if (side == "front".to_string()) ^ (handle == 1) {
        'R'
    } else {
        'L'
    };
    output.line(format!("{}", answer).as_str());
}

fn run(mut input: Input, mut output: Output) {
