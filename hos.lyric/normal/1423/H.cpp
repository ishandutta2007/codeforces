...
struct chronogical_segtree{
	int n,s;
	vvc<Query> qs;
	chronogical_segtree(int nn){
		n=nn;
		s=1;
		while(s<n)s*=2;
		qs.resize(s*2);
...
signed main(){
	...
	vi zs;
	auto unko=[&](){
		ft.pb({es[head].a,es[head].b,es[head].d,si(zs)});
		head++;
	};
	rep(i,q){
		int t;sc.read(t);
		if(t==1){
			...
			cur++;
			while(head<si(es)&&es[head].c<=cur-k){
				unko();
			}
		}else{
			assert(0);
	...
	while(head<si(es))unko();
	
	chronogical_segtree<pi> seg(si(zs));
	unionfind uf(n);
	auto unite=[&](pi e){
		uf.unite(e.a,e.b);
	};
	...
	for(auto e:ft){
		seg.add(e.c,e.d,pi(e.a,e.b));
	}
	seg.solve(unite,snapshot,rollback,getans);
	for(auto v:ans)pr.writeln(v);
}