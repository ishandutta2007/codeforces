...
//File I/O.
#define FILE_IN "twopaths.inp"
#define FILE_OUT "twopaths.out"
...
#define MAXN 3003 // maximum number of vertices you will be using
#define int_t int
...
int main()
{
	...
	{
		if (bb[i]<0) {
		gr.add_edge(0,i,-bb[i]); }
		if (bb[i]>0) {
		gr.add_edge(i,n+1,bb[i]); u+=bb[i];}
	}
	for (i=1;i<=n;i++) reach[i][i]=1;
	for (i=1;i<=n;i++) for (j=i-1;j>=1;j--)
	{
		if ((aa[i]%aa[j]==0)and(!reach[i][j])) {
		reach[i]|=reach[j]; 
		gr.add_edge(j,i,1e9);
		}
	}
	cout<<u-gr.calc_flow(0,n+1);
}