...
signed main(){
  ...
  vector<Segment> ss;
  ss.emplace_back(Point(min(A.x,B.x),min(A.y,B.y)),Point(max(A.x,B.x),min(A.y,B.y)));
  ss.emplace_back(Point(min(A.x,B.x),min(A.y,B.y)),Point(min(A.x,B.x),max(A.y,B.y)));
  ss.emplace_back(Point(min(A.x,B.x),max(A.y,B.y)),Point(max(A.x,B.x),max(A.y,B.y)));
  ss.emplace_back(Point(max(A.x,B.x),min(A.y,B.y)),Point(max(A.x,B.x),max(A.y,B.y)));

  Point s=Point(0,-(double)(c)/(double)(b));
  Point d=Vector((double)(b),-(double)(a));

  Line l(s-d,s+d);
  for(int i=0;i<4;i++){
    for(int j=0;j<4;j++){
      if(i==j) continue;
      Point p1=getCrossPointLL(ss[i],l);
      Point p2=getCrossPointLL(ss[j],l);
      ss.emplace_back(p1,p2);
    }
  }
  ...
  if(0){
    ...
    for(int i=0;i<n;i++)
      for(int j:G[i])
        cout<<i<<"->"<<j<<":"<<abs(ps[i]-ps[j])<<endl;
  }

  Dijkstra<double> ds(n);
  for(int i=0;i<n;i++)
    for(int j:G[i])
      ds.add_edge(i,j,abs(ps[i]-ps[j]));
