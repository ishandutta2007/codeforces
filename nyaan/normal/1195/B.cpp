...
////////////
ll N,K;
ll func(ll x){return (N - x) * (N - x + 1) - 2 * (K + x);}

void nibutan(ll l,ll r){
  if(func(l) == 0){out(l); exit(0);}
  if(func(r) == 0){out(r); exit(0);}
  if(func(l) > 0) swap(l , r);
  whlie( abs(r - l) > 1){
    ll med = (l + r) / 2;
    if(func(med) == 0){out(med); exit(0);}
    else if(func(med) > 0) r = med;
    else l = med;
  }
  if(func(l) == 0){out(l); exit(0);}
  if(func(r) == 0){out(r); exit(0);}
