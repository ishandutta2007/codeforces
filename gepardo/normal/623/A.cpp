...
main()
{
    ...
    {
        //cout << i << endl;
        if (cnt[i] == 0) { it[i] = 'b'; continue; }
        if (it[i] == 0)
        {
            bool useda = true, usedc = false;
            for (int j = 0; j < n; j++)
            {
                if (!q[i][j]) continue;
                if (it[j] == 'a') useda = true;
                if (it[j] == 'c') usedc = true;
            }
            if (!useda) it[i] = 'c'; else it[i] = 'a';
        }
        //cout << it[i] << endl;
        if (it[i] != 0)
        {
            for (int j = 0; j < n; j++)
            {
                if (it[j] == 0)
                {
                    if (q[i][j])
                    {
                        if (it[i] == 'a') it[j] = 'c'; else it[j] = 'a';
                    }
                    continue;
                }
                if (it[j] == 'b')
                {
                    if (q[i][j]) { cout << "No"; return 0; }
                    else continue;
                }
                if (it[j] == it[i] && q[i][j]) { cout << "No"; return 0; }
                if (it[j] != it[i] && (!q[i][j])) { cout << "No"; return 0; }
            }
            continue;
        }
