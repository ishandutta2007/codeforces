...
void solve() {
  ...
  FOR(_, N) {
    STR(S);
    ll n = S.size();
    FOR(R, n + 1) FOR(L, R) { ss.insert(S.substr(L, R - L)); }
  }

  auto to_s = [&](vi A) -> string {
    string S;
    FORIN(x, A) S += 'a' + x;
    return S;
  };
  ...
  FOR(a, 26) {
    string S = to_s(vi({a}));
    if (!ss.count(S)) return print(S);
  }
  FOR(a, 26) FOR(b, 26) {
    string S = to_s(vi({a, b}));
    if (!ss.count(S)) return print(S);
  }
  FOR(a, 26) FOR(b, 26) FOR(c, 26) {
    string S = to_s(vi({a, b, c}));
    if (!ss.count(S)) return print(S);
  }
}
