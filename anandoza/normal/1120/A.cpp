...
int main() {
    ...
    auto incr = [&](int v) {
        auto count = ++have[v];
        auto it = need.find(v);
        if (it != need.end() && it->second == count)
            good++;
    };
    ...
    for (int l = 0, r = 0; l < m; l++) {
        int left_cost = l % k;
        int remaining_steps = (m - n * k) - left_cost;

        while (r < m && r < l + k + remaining_steps) {
            incr(a[r]);
            r++;
        }
        while (remaining_steps >= 0 && r > l + k + remaining_steps) {
            r--;
            decr(a[r]);
        }

        if (good == sz(need) && remaining_steps >= 0) {
            ps(m - n * k);

            vi ans;
            for (int i = l - left_cost; i < l; i++)
                ans.push_back(i + 1);
            for (int i = l; i < r && sz(ans) < m - n * k; i++) {
