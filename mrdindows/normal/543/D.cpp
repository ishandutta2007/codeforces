...
void go1(int cur)
{
    ...
    ll sumb = 1;
    for (int to : g[cur]) {
        sumb = (sumb * (b[to] + 1)) % mod;
    }
    b[cur] = sumb;
}

void go(int cur, ll p = 1) {
    ans[cur] = p * b[cur] % mod;
    int k = (int) g[cur].size();
    vector<ll> prefs(k+1);
    vector<ll> sufs(k+1);
    prefs[0] = 1;
    sufs[0] = 1;
    for (int i = 0; i < k; ++i) {
        int to = g[cur][i];
        prefs[i + 1] = (prefs[i] * (b[to] + 1)) % mod;
        sufs[i + 1] = (sufs[i] * (b[g[cur][k - i - 1]] + 1)) % mod;
    }
    for (int i = 0; i < k; ++i) {
        int to = g[cur][i];
        go(to, (prefs[i] * p % mod * sufs[k - i - 1] + 1) % mod);
    }
}
