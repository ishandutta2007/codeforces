...
void solve() {
    ...
    for (auto i : segm) {
        taken[i.f] = taken[i.s] = true;
    }
    auto inter = [](pii p1, pii p2) {
        if (p1.f > p2.f) swap(p1, p2);
        return p1.s > p2.f && p2.s > p1.s;
    };
    vector<int> unused;
    for (int i = 0; i < 2 * n; ++i)
        if (!taken[i])
            unused.pb(i);
    dbg(unused);
    for (int i = 0, j = sz(unused) / 2; j < sz(unused); ++i, ++j)
        segm.pb({unused[i], unused[j]});
    // for (int i = 0; i < 2 * n; ++i)
    //     if (!taken[i]) {
    //         pii mx = {-1, -1};
    ...
    //         }
    //         segm.pb({i, mx.s});
    //         taken[i] = taken[mx.s] = true;
    //     }
    int ans = 0;
    for (int i = 0; i < sz(segm); ++i)
        for (int j = i + 1; j < sz(segm); ++j) {
            ans += inter(segm[i], segm[j]);
        }
    println(ans);
}
