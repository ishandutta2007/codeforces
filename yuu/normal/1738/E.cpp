...
void prepare() {
    f[0] = 1;
    for (int i = 1; i <= 100000; i++) (f[i] = f[i - 1] * i) %= base;
    r[100000] = power(f[100000], base - 2);
    for (int i = 100000; i >= 1; i--) (r[i - 1] = r[i] * i) %= base;
}
...
int64_t a[100002];
int64_t s[100002];
vector<pair<int64_t, int>> prefix, suffix;
int64_t get_in_fix(int k) {
    if (k == 0) return 1;
    int64_t res = 0;
    for (int i = 0; i <= k; i += 2) (res += C(k, i)) %= base;
    return res;
}
int64_t get_prefix_suffix(int a, int b) {
    int64_t res = 0;
    for (int i = 0; i <= min(a, b); i++) (res += C(a, i) * C(b, i)) %= base;
    return res;
}
...
void solve() {
    ...
    suffix.clear();
    for (int i = 1; i <= n; i++)
        if (prefix.empty() || (a[i] != prefix.back().first)) prefix.emplace_back(a[i], 1);
        else prefix.back().second++;
    for (int i = n; i >= 1; i--)
        if (suffix.empty() || (s[i] != suffix.back().first)) suffix.emplace_back(s[i], 1);
        else suffix.back().second++;
    while ((!prefix.empty()) && (prefix.back().first * 2 > a[n])) prefix.pop_back();
    while ((!suffix.empty()) && (suffix.back().first * 2 > a[n])) suffix.pop_back();
    int64_t ans = 1;
    if ((a[n] % 2) == 0) {
        int64_t in_fix = 0;
        if ((!prefix.empty()) && (prefix.back().first == a[n] / 2)) {
            assert(suffix.back().first == a[n] / 2);
            in_fix = prefix.back().second;
            prefix.back().second = 1;
            suffix.back().second = 1;
        }
        (ans *= get_in_fix(in_fix)) %= base;
        if (in_fix == n) {  // special case, we don't get to choose to cut the left and right side
            cout << ans << '\n';
            return;
        }
    };

    for (int i = 0, j = 0; i < prefix.size(); i++) {
        while ((j < suffix.size()) && (suffix[j].first < prefix[i].first)) j++;
        if (j >= suffix.size()) break;
        // cerr << suffix[j].second << ' ' << prefix[i].second << '\n';
        // cerr << prefix[i].first << ' ' << suffix[j].first << '\n';
        if (suffix[j].first == prefix[i].first) (ans *= get_prefix_suffix(prefix[i].second, suffix[j].second)) %= base;
    }
    cout << ans << '\n';
}
