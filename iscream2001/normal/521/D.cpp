...
void MAIN(){
	...
	for(int i=1;i<=K;++i){
		...
		if(b[i].x>a[i]){
			V[i].pb((node){b[i].x-a[i],0LL,b[i].id});
		}
	}
	for(int i=1;i<=K;++i){
		if(V[i].size()==0) continue;
		sort(V[i].begin(),V[i].end(),[&](node &x,node &y){
			return x.x>y.x;
		});
		for(auto z:V[i]){
			ans.pb((node){a[i]+z.x,a[i],z.id});
			a[i]+=z.x;
		}
	}
	if(ans.size()==0){
	...
	sort(ans.begin(),ans.end(),[&](node &x,node &y){
		return (__int128_t)x.x*(__int128_t)y.y>(__int128_t)y.x*(__int128_t)x.y;
	});
	cnt=0;
	...
	cout<<cnt<<endl;
	sort(b+1,b+1+cnt,[&](node &x,node &y){
		return op[x.id]<op[y.id];
	});
	for(int i=1;i<=cnt;++i) cout<<b[i].id<<" ";cout<<endl;
	return;
}
