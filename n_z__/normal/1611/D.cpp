...
namespace nz{
/*namespace-head----------------------------------------------------------------------------------*/
int f[200001],p[200001],idp[200001],ans[200001];
/*var-and-func------------------------------------------------------------------------------------*/
void Main(){
/*Main-head---------------------------------------------------------------------------------------*/
    int n;
    ...
    for(int x=1;x<=n;x++)
    cin>>p[x],idp[x]=-1;
    if(f[p[1]]!=p[1]){cout<<-1<<endl;Fwrite::flush();return;}
    idp[p[1]]=0;
    ans[p[1]]=0;
    for(int x=2;x<=n;x++)
    if(idp[f[p[x]]]<0){puts("-1");Fwrite::fuck();return;}
    else ans[p[x]]=x-idp[f[p[x]]],idp[p[x]]=x;
    for(int x=1;x<=n;x++)
    cout<<ans[x]<<' ';
    cout<<endl;
    Fwrite::flush();
