...
int main()
{
	...
	{
		re ans=0;
		n=read();for(re i=1;i<=n;i++) a[i]=b[i]=read(),l[i]=r[i]=0;
		sort(b+1,b+n+1);re m=unique(b+1,b+n+1)-b-1;
		for(re i=1;i<=n;i++)
		{
			a[i]=lower_bound(b+1,b+m+1,a[i])-b;
			if(!l[a[i]])l[a[i]]=i;r[a[i]]=i;
		}
		for(re i=1,j=1;i<m;i=j)
		{
			if(r[i]>=l[i+1]) {j++;continue;}
			while(j<m&&r[j]<l[j+1]) j++;
			ans=max(ans,j-i);
		}
		write(m-ans-1,'\n');
	}
