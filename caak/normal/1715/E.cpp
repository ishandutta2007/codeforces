...
_ n, m, k, d[maxn], x, y, c, a[maxn], b[maxn];
struct edge {
    _ to, cost;
};
v(edge) g[maxn];
...
inline void AC() {
    ...
    for(; k--; ) {
        ...
        reverse(d+1, d+1+n);
        reverse(a+1, a+1+n);
        for(_ i = 1; i <= n; ++i) d[i] = min({d[i], a[i], b[i]});
        dijkstra();
    }
    wr(d, n);
...
void dijkstra() {
    for(_ i = 1; i <= n; ++i) pq.push((node){i, d[i]});
    for(; !pq.empty(); ) {
        node t = pq.top();
        pq.pop();
        if(d[t.p] < t.d) continue;
        for(edge i : g[t.p]) if(d[i.to] > d[t.p]+i.cost) {
            d[i.to] = d[t.p]+i.cost;
            pq.push((node){i.to, d[i.to]});
        }
    }
    return;
}
...
_ vl(_ pt, _ sl) {
    return -2*pt*sl+sl*sl+pt*pt+d[pt];
}

bool cmp(_ x, _ y, _ z) {
    _ xx = -x, xy = d[x]+x*x, yx = -y, yy = d[y]+y*y, zx = -z, zy = d[z]+z*z;
