...
public:
    void solve() {
        ...
        auto cmp = [](const pii& x, const pii& y) {
            int xLen = x.second - x.first;
            int yLen = y.second - y.first;
            if (xLen != yLen) {
                return xLen > yLen;
            }
        ...
        for (int i : range(n)) {
            pii cur = *s.begin();
            s.erase(cur);
            int mid = (cur.first + cur.second - 1) / 2;
            a[mid] = i + 1;
            if (cur.first != mid) {
                s.emplace(cur.first, mid);
            }
            if (mid + 1 != cur.second) {
                s.emplace(mid + 1, cur.second);
            }
        }
        out.printLine(a);
    }
