...
int find_next(int now) {
	if (nexts[now] == now) return now;
	return nexts[now] = find_next(nexts[now]);
}
int find_back(int now) {
	if (back[now] == now) return now;
	return back[now] = find_back(back[now]);
}
void solve(){
	cin >> n;
	vector<int> val;
	...
	priority_queue<int, vector<int>, greater<int>> gogo;
	vector<int> ans;
	set<int> dont;
	for (int i = 1; i <= n; ++i) {
		gogo.push(i);
	...
	while (gogo.empty() == false) {
		...
		int now_back = find_back((itr + n - 1) % n);
		if (gcd(val[now_back], val[itr]) != 1) continue;
		if (dont.find(now) != dont.end()) {
			//go next time
			gogo.push(now + n);
			continue;
		}
		...
		nexts[itr] = (itr + 1)%n;
		back[itr] = (itr + n - 1) % n;
		int next_go = find_next(itr);
		int Index = now / n * n + next_go;
		if (next_go <= itr) {
			Index += n;
		}
		gogo.push(Index);
