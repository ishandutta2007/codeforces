...
struct Segtree{
	...
	long long qmax(int l,int r,int now=1,int nl=1,int nr=n){
		if(wc[now]){
			dt[now<<1]=wc[now];
			dt[now<<1|1]=wc[now];
			wc[now<<1]=wc[now];
		...
		int m=(nl+nr)>>1;
		return max(qmax(l,r,now<<1,nl,m),qmax(l,r,now<<1|1,m+1,nr));
	}
}seg;
