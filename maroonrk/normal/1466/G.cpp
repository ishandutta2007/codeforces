...
void slv(){
	...
	string rw=s;
	for(auto c:t){
		if(si(rw)>=ten(6))break;
		rw=rw+c+rw;
	}
	rhash::init(vmax);
	rhash::gen g(rw);
	vvc<pair<string,int>> qs(n+1);
	rep(_,q){
		int tar;cin>>tar;
		string w;cin>>w;
		qs[tar].eb(w,_);
	...
	rep(tar,n+1){
		for(const auto&kv:qs[tar]){
			const string&w=kv.a;
			rhash::gen h(w);
			mint val=0;
			rep(i,si(w)){
				int x=i,y=si(w)-1-i;
				if(g.get(si(rw)-x,si(rw))==h.get(0,x)
				&& g.get(0,y)==h.get(si(w)-y,si(w))){
					val+=cnt[w[i]-'a'];
					int z=max(topbit(x/(si(s)+1)),topbit(y/(si(s)+1)));
					rep(j,min(z+1,tar)){
						if(t[j]==w[i]){
							val-=p2[tar-j-1];
						}
			...
			if(si(w)<=si(s)){
				mint tmp=0;
				rep(i,si(s)-si(w)+1)if(g.get(i,i+si(w))==h.get(0,si(w)))tmp+=1;
				tmp*=p2[tar];
				val+=tmp;
			}
			ans[kv.b]=val;
