...
struct Node2 {
  ...
  void setX(ll x1, ll x2) {
    max1 = x1;
    max2 = x2;
    max = x1 + x2;
  }

  void update(Node2 const &l, Node2 const &r, int cl, int cr) {
    max1 = std::max(l.max1, r.max1);
    max2 = std::max(l.max2, r.max2);
    max = std::max(std::max(l.max, r.max), l.max1 + r.max2);
  }

  void push(Node2 &l, Node2 &r, int cl, int cr) {
  }
};
...
int main() {
  ...
  cin >> a;
  vector<vector<ll>> as(3, vector<ll>(n + 1));
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < n; ++j)
      as[i][j + 1] = as[i][j] + a[i][j];

  vector<ll> c1(n), c3(n);
  for (int i = 0; i < n; ++i) {
    c1[i] = as[0][i + 1] - as[1][i];
    c3[i] = (as[2][n] - as[2][i]) + as[1][i + 1];
  }
  ...
  SegTree<Node> d1(n, [&](Node&v, int i) { v.setX(c1[i]); });

  sort(all(sgs), [](Seg const& a, Seg const& b) { return a.r < b.r; });
  for (Seg const& s : sgs) {
    if (s.r == n) continue;
    ll x = d1.getMax(s.l, s.r) - s.w;
    d1.withLeaf(s.r, [&](Node& v) { v.setX(max(x, v.max)); });
  }

  SegTree<Node2> d2(n, [&](Node2& v, int i) { v.setX(d1.getLeaf(i).max, c3[i]); });

  ll ans = -LINF;
  for (Seg const& s : sgs) {
    ll cans = -LINF;
    ll cmax1 = -LINF;
    d2.withSeg(s.l, s.r, [&](Node2& v, int, int) {
      cans = max(cans, v.max);
      cans = max(cans, cmax1 + v.max2);
      cmax1 = max(cmax1, v.max1);
    });
    ans = max(ans, cans - s.w);
  }
  cout << ans << "\n";
