...
void responderQuery(vvi& aDonde, int lgn, vi& resultados){
    int a,b;
    cin>>a>>b;
    int res = 0;
    while(a < b){
...
void e(){
    ...
    vi res;
    F0(i,m){
        responderQuery(aDondeLlegoDesde,lgn,res);
    }
    F0(i,m) cout<<res[i]<<'\n';
}
...
int permutacionDerecha(vi& a, int i){
    set<int> conj;
    int maximo = 1;
    int res = 0;
    i++;
    while(i <a.size() && a[i] != 1){
        ...
        conj.insert(a[i]);
        if(maximo == conj.size()) res++;
        i++;
    }
    return res;
}

int permutacionIzquierda(vi& a, int i){
    set<int> conj;
    conj.insert(1);
    int maximo = 1;
    int res = 0;
...
int permutacionesDesde(vi& a, int i){
    int res = 1;
    res += permutacionDerecha(a,i);
    res += permutacionIzquierda(a,i);
    //res += permutacionMedio(a,i);
    return res;
}
