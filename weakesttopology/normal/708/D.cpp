...
int main() {
    ...
    std::cin >> N >> M;
    int src = N, snk = N + 1;
    mcf_graph<int64_t, int64_t> mcf(N + 2);
    ...
    for (int j = 0; j < M; ++j) {
        ...
        if (c - f < 0) {
            add += f - c;
            mcf.add_edge(u, v, inf, 2);
            mcf.add_edge(v, u, f - c, 0);
            mcf.add_edge(v, u, c, 1);
        } else {
            mcf.add_edge(u, v, c - f, 1);
            mcf.add_edge(u, v, inf, 2);
            mcf.add_edge(v, u, f, 1);
        }
        d[u] += f;
        d[v] -= f;
    }
    mcf.add_edge(N - 1, 0, inf, 0);
    int64_t sum = 0;
    for (int u = 0; u < N; ++u) {
        if (d[u] > 0) {
            mcf.add_edge(u, snk, d[u], 0);
            sum += d[u];
        } else {
            mcf.add_edge(src, u, -d[u], 0);
        }
    }
    auto [flow, cost, slopes] = mcf.mincostflow(src, snk);
    assert(flow == sum);
    cost += add;
    std::cout << cost << '\n';
    exit(0);
