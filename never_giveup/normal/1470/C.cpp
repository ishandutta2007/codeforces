...
const int maxn = 2e5 + 100, maxw = 5e6 + 100, inf = 2e9 + 100, sq = 450, LG = 20, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
void solve() {
    cin >> n >> k;
    auto prv = [&](int i, int j = 1) {
        i = (i - j) % n;
        if (i < 0)
            i += n;
        return i;
    };
    auto nxt = [&](int i, int j = 1) {
        i = (i + j) % n;
        return i;
    };
    if (n < 100) {
