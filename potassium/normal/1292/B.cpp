...
int main() {
	ll x0, y0, ax, ay, bx, by; R(x0,y0,ax,ay,bx,by);
	ll xs, ys, t; R(xs, ys, t);
	...
	for (int mid = 0; mid < nodes.size(); mid++) {
		ll rem = t - dist(nodes[mid], s);
		if (rem < 0) continue;
		
		//route 0
		int re0 = -1;
		for (int i = mid; i >= 0; i--)
			if (dist(nodes[i], nodes[mid]) > rem) {
				re0 = i;
				break;
			}
			
		chkmax(ans, mid - re0);
		ll rem01 = rem - dist(nodes[0], nodes[mid]);
		if (rem01 > 0) {
			int re01 = -1;
			for (int i = 0; i < nodes.size(); i++)
				if (dist(nodes[i], nodes[0]) > rem01) {
					re01 = i;
					break;
				}
		...
		ll rem11 = rem - dist(nodes[n-1], nodes[mid]);
		if (rem11 > 0) {
			int re11 = -1;
			for (int i = n-1; i >= 0; i--)
				if (dist(nodes[i], nodes[n-1]) > rem11) {
					re11 = i;
					break;
				}
