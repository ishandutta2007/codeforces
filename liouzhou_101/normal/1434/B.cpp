...
void solve()
{
	...
	for (int i = 1; i <= 2 * n; ++i)
	{
		...
		else
		{
			int x;
			read(x);
			events.push_back({ 1, x });
			pos[x] = cnt;
			H.insert(pos[x]);
			--has;
			if (has < 0) return writeln("NO");
		}
	}
	...
	for (int i = 1; i <= n; ++i)
	{
		H.erase(H.lower_bound(pos[i]));
		auto it = H.lower_bound(pos[i]);
		it--;
		int tmp = father(*it + 1);
		...
		res[tmp] = i;
		f[father(tmp)] = father(tmp + 1);
	}

	{
		int cnt = 0;
		priority_queue<int, vector<int>, greater<int>> q;
		for (auto [op, id] : events)
		{
			if (op == 0)
			{
				cnt += 1;
