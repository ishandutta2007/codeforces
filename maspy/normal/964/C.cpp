...
void solve() {
  ...
  auto mul = [&](T t1, T t2) -> T {
    auto [x1, a1, b1] = t1;
    auto [x2, a2, b2] = t2;
    return {x1 * a2 + x2 * b1, a1 * a2, b1 * b2};
  };
  vc<T> dat(k);
  FOR(i, k) {
    char s = S[i];
    if (s == '+') dat[i] = {mint(1), mint(a), mint(b)};
    if (s == '-') dat[i] = {mint(-1), mint(a), mint(b)};
  }
  n = (n + 1) / k;
  T t = {mint(0), mint(1), mint(1)};
  for (auto&& v: dat) t = mul(t, v);

  T ans = {mint(0), mint(1), mint(1)};
  T p = t;
  while (n) {
