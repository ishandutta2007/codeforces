...
//			int[] one = new int[n + 1];
//			int[] max = new int[n + 1];
			Pair one1 = new Pair(Long.MAX_VALUE, -1);
			Pair one2 = new Pair(Long.MAX_VALUE, -1);
			Pair x1 = new Pair(Long.MAX_VALUE, -1);
			Pair x2 = new Pair(Long.MAX_VALUE, -1);
			long ans = Long.MAX_VALUE;
			for (int i = 0; i <= n; i++) {
				if (i == 0) {
					Pair one = new Pair(a[i] - 1, 0);
					Pair max = new Pair(Math.abs(a[i] - x), 0);
					if (one.compareTo(one1) < 0) {
						one2 = one1;
						one1 = one;
					} else if (one.compareTo(one2) < 0) {
						one2 = one;
					}
					if (max.compareTo(x1) < 0) {
						x2 = x1;
						x1 = max;
					} else if (max.compareTo(x2) < 0) {
						x2 = max;
					}
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[0] - 1));
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[0] - x));
				} else if (i == n) {
					Pair one = new Pair(Math.abs(a[i - 1] - 1), n);
					Pair max = new Pair(Math.abs(a[i - 1] - x), n);
					if (one.compareTo(one1) < 0) {
						one2 = one1;
						one1 = one;
					} else if (one.compareTo(one2) < 0) {
					...
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[n - 1] - 1));
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[n - 1] - x));
				} else {
					Pair one = new Pair(Math.abs(a[i] - 1) + Math.abs(a[i - 1] - 1) - Math.abs(a[i] - a[i - 1]), i);
					Pair max = new Pair(Math.abs(a[i] - x) + Math.abs(a[i - 1] - x) - Math.abs(a[i] - a[i - 1]), i);
					if (one.compareTo(one1) < 0) {
						one2 = one1;
						one1 = one;
					} else if (one.compareTo(one2) < 0) {
					...
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[i] - 1) + Math.abs(a[i - 1] - x) - Math.abs(a[i] - a[i - 1]));
					ans = Math.min(ans, sum + x - 1 + Math.abs(a[i - 1] - 1) + Math.abs(a[i] - x) - Math.abs(a[i] - a[i - 1]));
				}
			}
			if (x1.y != one1.y) {
				ans = Math.min(sum + x1.x + one1.x, ans);
			} else {
				ans = Math.min(sum + x2.x + one1.x, ans);
				ans = Math.min(sum + x1.x + one2.x, ans);
			}
			out.println(ans);
		}
	}
