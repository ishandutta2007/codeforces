...
fun allEqual(a: IntArray): Boolean {
    for (i in a) if (i != a[0]) return false
    return true
}

fun solve() {
    val (n,u,v) = io.ints(3)
    val a = io.ints(n).toIntArray()
    for (i in 0..n-2) if (abs(a[i]-a[i+1]) > 1) {
        println(0)
        return
    }
    if (allEqual(a)) {
        println(v+minOf(u,v))
    } else println(minOf(u,v))
//    for (i in 0..n-1) if (abs())
}
