...
void mian() {
	n = read(), m = read();
	reads(a + 1);
	REP(i, 1, n) Sum[i] = Sum[i - 1] + (a[i] ^ 48);
	int c1 = round((db)Sum[n] / n * m)  + .5;
	if((ll)c1 * n != (ll)Sum[n] * m) return puts("-1"), void();
	REP(i, 1, n - m + 1) if(sum(i, i + m - 1) == c1) return printf("%d\n%d %d\n", 1, i, i + m - 1), void();
	REP(i, 1, m - 1) if(sum(1, i) + sum(n - (m - i) + 1, n) == c1) return printf("%d\n%d %d\n%d %d\n", 2, 1, i, n - (m - i) + 1, n), void();
}
