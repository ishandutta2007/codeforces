...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		int n, l, r;
		cin >> n >> l >> r;
		combinatorics<Zp> C(n);
		int max_val = numeric_limits<int>::max();
		map<int, array<int, 2>> events;
		for(auto i = 1; i <= n; ++ i){
			++ events[i - l][0];
			++ events[r - i][1];
			ctmin(max_val, max(i - l, r - i));
		}
		array<int, 2> bad = events[0];
		events.erase(0);
		int prev = 0;
		Zp res = 0;
		for(auto [val, e]: events){
			if(val > max_val){
				break;
			}
			if(n & 1){
				res += (val - prev) * (C.C(n - bad[0] - bad[1], n / 2 - bad[0]) + C.C(n - bad[0] - bad[1], n / 2 - bad[1]));
			}
			else{
				res += (val - prev) * C.C(n - bad[0] - bad[1], n / 2 - bad[0]);
			}
			bad[0] += e[0], bad[1] += e[1];
			prev = val;
		}
