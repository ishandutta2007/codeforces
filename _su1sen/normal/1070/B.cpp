...
struct Node {
    ...
    void insert_white(long long prefix, int dep) {
        Node* cur = this;
        rep(d, dep) {
            cur->white = true;
            int idx = kth_bit(prefix, 31 - d);
            if (not cur->ch[idx]) cur->ch[idx] = new Node();
            cur = cur->ch[idx];
        }
        cur->white = true;
        cur->subtree = true;
    ...
    optional<pair<long long, int>> insert_black(long long prefix, int dep) {
        Node* cur = this;
        rep(d, dep) {
            if (not cur->white) {
                long long nxt_prefix = prefix >> (32 - d) << (32 - d);
                return pair<long long, int> { nxt_prefix, d };
            }
            if (cur->subtree) return nullopt;
            int idx = kth_bit(prefix, 31 - d);
            if (not cur->ch[idx]) cur->ch[idx] = new Node();
            cur = cur->ch[idx];
        }
        if (cur->white) return nullopt;
        return pair<long long, int> { prefix, dep };
    }
};

pair<long long, int> parse(string s) {
    if (s.find('/') == s.npos) s += "/32";
    long long prefix = 0;
    int i = 0;
    rrep(k, 4) {
        auto j = s.find(k ? '.' : '/', i);
        assert(j != s.npos);
        assert(s[j] == (k ? '.' : '/'));
        long long val = stoi(s.substr(i, j - i));
        prefix |= val << (8 * k);
        i = j + 1;
    }
...
string to_ip(long long prefix) {
    constexpr int mask = (1 << 8) - 1;
    int a = (prefix >> 24) & mask;
    int b = (prefix >> 16) & mask;
    int c = (prefix >>  8) & mask;
    int d = (prefix >>  0) & mask;
...
int main() {
    ...
    vector<pair<long long, int>> bs;
    rep(i, n) {
        input(string, s);
        bool white = s[0] == '+';
        auto [prefix, dep] = parse(s.substr(1));

        if (white) {
            root->insert_white(prefix, dep);
        } else {
            bs.emplace_back(prefix, dep);
        }
    }

    vector<pair<long long, int>> ranges;
    for (auto [prefix, dep] : bs) {
        auto opt_range = root->insert_black(prefix, dep);
        if (not opt_range.has_value()) {
            delete root;
            print(-1);
            return 0;
        }
        ranges.emplace_back(*opt_range);
    }

    auto as_range = [&](auto p) {
        return make_pair(p.first, p.first + (1LL << (32 - p.second)));
    };

    sort(all(ranges), [&](auto p, auto q) {
        auto [pl, pr] = as_range(p);
        auto [ql, qr] = as_range(q);
        if (pr != qr) return pr < qr;
        return pl > ql;
    });
    vector<pair<long long, int>> st;
    for (auto ran : ranges) {
        auto [l, r] = as_range(ran);
        while (st.size()) {
            auto [l2, r2] = as_range(st.back());
            if (r2 <= l) break;
            assert(l <= l2 and r2 <= r);
            st.pop_back();
        }
        st.push_back(ran);
    }
