...
#define assert(x) (x)
#endif
// T(n) is a root that has 3 child, T(n - 2), T(n - 1), T(n - 2)
// If we pick the root, then we can continue to do the same in T(n - 2), 4 copies of T(n - 3), and 4 copy of T(n - 4)
// If we do not pick the root, then we can do in 1 copy of T(n - 1) and 2 copies of T(n - 2)
// F(n) = max(4 + F(n - 2) + 4F(n - 3) + 4F(n - 4), F(n - 1) + 2F(n - 2))
// 4 + F(n - 2) + 4F(n - 3) <=> F(n - 1) + 2F(n - 2)
// 4 + 4F(n - 3) <=> F(n - 1) + F(n - 2)
//
constexpr int base = 1e9 + 7;
int64_t f[2000001];
void precal() {
    for (int i = 3; i <= 20; i++) {
        f[i] = max(f[i - 2] + f[i - 3] * 4 + f[i - 4] * 4 + 4, f[i - 1] + f[i - 2] * 2);
        cerr << i << ' ' << f[i] << ' ' << f[i] - (f[i - 2] + f[i - 3] * 4 + f[i - 4] * 4 + 4) << '\n';
    }
    for (int i = 21; i <= 2000000; i++) {
        if (i % 3 == 2) {
            (f[i] = f[i - 1] + f[i - 2] * 2) %= base;
        } else {
            (f[i] = f[i - 2] + f[i - 3] * 4 + f[i - 4] * 4 + 4) %= base;
        }
    }
    // for (int i = 10; i <= 2000000; i++) (f[i] = f[i - 1] + f[i - 2] * 2) %= base;
    cerr << f[100] << '\n' << f[2000000] << '\n';
}
void solve() {
    int n;
