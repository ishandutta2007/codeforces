...
void dfs(int x,int fa = -1){
    ...
    int mk = 0;
    if(lson != -1)swap(val[x],val[lson]);
    each(ct,G[x]){
        if(ct == fa || ct == lson)continue;
        each(ccv,val[ct]){
            if(val[x].count(ccv^a[x]))mk = 1;
        }
        each(ccv,val[ct]) val[x].insert(ccv);
    }
    if(val[x].count(xv[x]^a[x]))mk = 1;
    else val[x].insert(xv[x]);
    if(mk){
        val[x].clear();
        ans+=1;
    }
