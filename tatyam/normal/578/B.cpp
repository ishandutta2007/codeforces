...
template<class T>
struct ROQ : SegmentTree<T>{
    using Base = SegmentTree<T>;
    T f(const T& a, const T& b) const {return a | b;}
    ROQ(ll n, const T& def_value = T()) : SegmentTree<T>(n, def_value){}
    ROQ(const vector<T>& v, const T& def_value = T()) : SegmentTree<T>(v, def_value){
