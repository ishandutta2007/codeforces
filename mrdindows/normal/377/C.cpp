...
int go(int action, int mask)
{
    ...
    {
        if (d[action][mask] != INF)
        {
            return d[action][mask];
        }
        if (q[action] == 'p')
        {
            int res = 0;
        ...
        else
        {
            ...
            for (int j = 0; j < m; ++j)
            {
                if (!checkbit(mask, j))
                {
                     int p = go(action + 1, mask | (1 << j));
                     if (p > maxres) maxres = p;
                     if (p < minres) minres = p;
                }
            }
            int ans;
            if (t[action] == 1)
