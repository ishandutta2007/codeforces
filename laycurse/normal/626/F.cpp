...
void mint_init(unsigned md=MD, mint val[]=mval, int vals=10000, mint inv[]=minv, int invs=10000){int i;val[0].setmod(md);val[0].val=0;REP(i,1,vals){val[i].val=val[i-1].val+mint::R;if(val[i].val>=md)val[i].val-=md;}inv[1].val=1;REP(i,2,invs){inv[i].val=md-((ll)(md/i)*inv[md%i].val%md);}REP(i,1,invs)inv[i].val=(ull)inv[i].val*mint::R%md;}
void reader(mint *x){int k;reader(&k);(*x)=k;}
void writer(mint a, char c){writer((int)a,c);}


int N, K, A[300];
mint dp[101][2000], nx[101][2000];
...
int main(){
  ...
  dp[0][0] = 1;
  rep(k,N){
    rep(i,101) rep(j,K+1) nx[i][j] = mval[0];
    rep(i,101) rep(j,K+1) if(dp[i][j].val){
      nx[i][j] += dp[i][j] * mval[i+1];
      if(i)       nx[i-1][j] += dp[i][j] * mval[i];
      if(i+1<101) nx[i+1][j] += dp[i][j];
    }
    rep(i,101) rep(j,K+1) dp[i][j] = mval[0];
    if(k<N-1){
      rep(i,101) rep(j,K+1) if(j+i*(A[k+1]-A[k]) <= K) dp[i][j+i*(A[k+1]-A[k])] += nx[i][j];
    } else {
      rep(i,101) rep(j,K+1) dp[i][j] = nx[i][j];
    }
  }

  res = 0;
  rep(i,K+1) res += dp[0][i];
  writerLn(res);
