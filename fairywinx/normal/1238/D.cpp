...
signed main() {
	...
	for(int i = 0; i < n; ++i) {
		...
		if (s[i] == 'A')
			...
			type = 2;
		if (type == 1) 
			ans += (long long) (i - last[i].first - 1) * (next[i].first - i - 1);
		else
			ans += (long long) (i - last[i].second - 1) * (next[i].second - i - 1);
		if (type == 1) {
			if (last[i].second != -1)
				ans += min(last[last[i].second].second, last[i].first) + 1;
		}
		else {
			if (last[i].first != -1)
				ans += min(last[i].second, last[last[i].first].first) + 1;
		}
	}
	for(int i = 0; i < n; ++i) {
		if (s[i] == 'A')
			ans += (long long) (next[i].second - i) * (next[i].second - i - 1) / 2, i = next[i].second - 1;
		else
			ans += (long long) (next[i].first - i) * (next[i].first - i - 1) / 2, i = next[i].first - 1;
	}
	cout << ans;
