...
struct cm {
	...
	cm(ld xx = 0, ld yy = 0) : x(xx), y(yy) {}

	cm operator*(const cm& oth) const {
		return cm(x * oth.x - y * oth.y, x * oth.y + y * oth.x);
	}

	cm operator+(const cm& oth) const {
		return cm(x + oth.x, y + oth.y);
	}
...
void fft(cm *a, bool inv, int pww) {
	for (int i = 0; i < (1 << pww); ++i) {
		rev[i] = (rev[i >> 1] >> 1 | ((i & 1) << (pww - 1)));
		if (rev[i] < i) {
			swap(a[i], a[rev[i]]);
		}
	}
	...
	cm *w = inv ? wwi : ww;
	for (int lg = 0; lg < pww; ++lg) {
		int bl = (1 << lg);
		for (int ptr = 0; ptr < (1 << pww); ptr += 2 * bl) {
			for (int pos = 0; pos < bl; ++pos) {
				cm tmp = a[ptr + pos] + a[ptr + pos + bl] * w[pos];
				a[ptr + bl + pos] = a[ptr + pos] - a[ptr + pos + bl] * w[pos];
				a[ptr + pos] = tmp;
			}
		}
		w += bl;
