// CodeForces 1658.D2 388535 (Hard Version)
...
namespace data_structure
{
	...
	public:
		...
		Node* Get(const std::vector<int>& data)
		{
			Node* it = root;
			for (auto x : data)
			{
				assert(0 <= x && x < alpha);
				if (it->son[x] == nullptr) return nullptr;
				it = it->son[x];
			}
			return it;
		}
...
void solve()
{
	...
	auto get_min = [&](const vector<int>& u)
	{
		auto it = g.GetRoot();
		int res = 0;
		for (int i = 0; i < K; ++i)
	...
	auto get_max = [&](const vector<int>& u)
	{
		auto it = g.GetRoot();
		int res = 0;
		for (int i = 0; i < K; ++i)
