...
int n,warna[100005],a,b,m;
map<int,bool> adj[100005];
pii risan = mp(0,0);
...
int main()
{
	...
	REP(i,n) adj[warna[i]][warna[i]] = 1;
	TC(m)
	{
		scanf("%d %d",&a,&b);
		--a; --b;
		adj[warna[a]][warna[b]] = 1;
		adj[warna[b]][warna[a]] = 1;
		//adj[warna[a]].insert(warna[b]);
		//adj[warna[b]].insert(warna[a]);
	}
	REP(i,n) 
	{
		int cek = SIZE(adj[warna[i]]);
		MAX(risan,mp(cek,-warna[i]));
	}
	printf("%d\n",-risan.se);
	return 0;
}