...
void solve(){
    pre();
    //let there be x rows that are **for sure** bad, inclusion exclusion on x
    //
    //each column contributes (k^(n-x) * (k-1)^x) - ((k-1)^(n-x) * (k-1)^x)
    ...
    for (int x = 0; x <= n; x++) {
        num cur = 1;
        for (int i = 0; i < n; i++) cur *= (po(k, n-x) * po(k-1, x)) - (po(k-1, n-x) * po(k-1, x));
