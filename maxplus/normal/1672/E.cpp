...
int main() {
  ...
  int n = $;
  PQ<Range> pq;
  int64_t ans = 1e18, inf = n * 2001;
  pq.push({0, inf, inf, 1});
  int nq = 30 + n;
  while (pq.size()) {
    auto [wl, wr, hl, hr] = pq.top(); pq.pop();
    if (ans <= (wl + 1) * hr || ans <= wl * (hr - 1)) {
      continue;
    }
    if (!nq--) {
      break;
    ...
    int64_t wm;
    {
      auto d = uniform_int_distribution(wl + 1, wr - 1);
      wm = (d(rng) + d(rng) * 3 + d(rng) * 9 + 6) / 13;
    }
    print('?', wm);
    int64_t hm = $;
    if (!hm) {
