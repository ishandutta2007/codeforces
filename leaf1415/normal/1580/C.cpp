...
int main(void)
{
	...
	rep(i, 1, m){
		...
		if(p <= B){
			if(op == 1){
				add[k] = i;
				for(ll j = x[k]; j < p; j++) sum[p][(i+j)%p]++;
			}
			if(op == 2) for(ll j = x[k]; j < p; j++) sum[p][(add[k]+j)%p]--;
			//outa(sum[2], 0, 1);
		}
		rep(j, 2, B) ans[i] += sum[j][i%j];
	}

	rep(i, 1, n){
		ll p = x[i] + y[i];
		if(p <= B) continue;
		if(sz(vec[i]) % 2) vec[i].push_back(m+1);

		for(int j = 0; j < sz(vec[i]); j += 2){
			ll s = vec[i][j], t = vec[i][j+1];
			for(ll k = 1; ; k++){
				ll l = s+(k-1)*p+x[i], r = s+k*p;
				if(l >= t) break;
				chmin(l, t), chmin(r, t);
				dif[l]++, dif[r]--;
			}
