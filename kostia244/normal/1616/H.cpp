...
int n, x;
vector<mint> p2(C);
mint solve(vector<int> a, vector<int> b, int h) {
	if(a.empty() || b.empty() || h == -1)
		return p2[a.size()+b.size()];
	vector<int> A[2], B[2];
	for(auto i : a)
		A[((i>>h)&1)].push_back(i);
	for(auto i : b)
		B[((i>>h)&1)].push_back(i);
	mint ans = 0;
	if(((x>>h)&1)==0) {//not allowed to have
		ans = solve(A[0], B[0], h-1) + solve(A[1], B[1], h-1) - 1;
		ans += (p2[a.size()]-1) - (p2[A[0].size()]-1) - (p2[A[1].size()]-1);
		ans += (p2[b.size()]-1) - (p2[B[0].size()]-1) - (p2[B[1].size()]-1);
	} else {
		ans = solve(A[0], B[1], h-1)*solve(A[1], B[0], h-1);
	}
	// cout << h << ":" << endl;
	// cout << A[0].size() << " " << A[1].size() << " " << B[0].size() << " " << B[1].size() << endl;
...
int main() {
	...
	for(auto &i : a) block[i>>(b+1)].push_back(i);
	mint ans = 0;
	for(auto [_, V] : block) {
		vector<int> u, v;
		for(auto i : V)
			((i>>b)%2 ? u : v).push_back(i);
		ans += solve(u, v, b-1)-1;
	}
	cout << ans << '\n';
}