...
set<pair<int, int>> on_edge;
list<pair<int, int*>> g[200001];
int weight[200001];
constexpr int BUCKET_SIZE = 9000;
int r[200001];
int root(int u) {
    if (r[u] < 0) return u;
    return r[u] = root(r[u]);
...
int done[200001];
int turn;
bool dfs(int u, const int& target) {
    if (u == target) return true;
    done[u] = turn;
    for (auto&& [v, w_ptr] : g[u])
        if (*w_ptr) {
            if (done[v] != turn) {
...
void solve() {
    ...
    bool last = 0;
    for (int start = 1; start <= m; start += BUCKET_SIZE) {
        int end = min(start + BUCKET_SIZE - 1, m);
        vector<pair<int, int>> possible_change;
        vector<pair<int, int>> alive;
        for (int i = start; i <= end; i++)
            if (e[i].t == 1) {
                possible_change.emplace_back(e[i].x[0], e[i].y[0]);
                possible_change.emplace_back(e[i].x[1], e[i].y[1]);
            }
        fill(r + 1, r + n + 1, -1);
        possible_change | UNIQUE;
        for (auto&& edge : on_edge)
            if (!binary_search(possible_change.begin(), possible_change.end(), edge)) unite(edge.first, edge.second);
            else alive.push_back(edge);

        for (auto&& [x, y] : possible_change) {
            x = root(x);
            y = root(y);
            if (x > y) swap(x, y);
        }
        ...
        fill(weight, weight + possible_change.size(), 0);
        for (int i = 0; i < possible_change.size(); i++) {
            auto&& [x, y] = possible_change[i];
            g[x].emplace_back(y, weight + i);
            g[y].emplace_back(x, weight + i);
        }

        auto modify_weight = [&](int x, int y, int d) {
            x = root(x);
            y = root(y);
            if (x > y) swap(x, y);
            int pos =
                    lower_bound(possible_change.begin(), possible_change.end(), pair<int, int>{x, y}) - possible_change.begin();
            weight[pos] += d;
        };

        for (auto&& [x, y] : alive) modify_weight(x, y, 1);

        for (int i = start, x, y; i <= end; i++) {
            x = e[i].x[last];
            y = e[i].y[last];
            // cerr << "translated: " << i << ' ' << e[i].t << ' ' << x << ' ' << y << '\n';
            if (e[i].t == 1) {
                // add or remove
                if (on_edge.count({x, y})) {
                    on_edge.erase({x, y});
                    modify_weight(x, y, -1);
                } else {
                    on_edge.emplace(x, y);
