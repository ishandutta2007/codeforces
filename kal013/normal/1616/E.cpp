...
void solve(){
    ...
    cin >> s >> t;
 	...
 	for(int len = 0; len < n; ++len){
 		...
 		for(int k = 0; k < j; ++k){
 			if (cnt_s[k] == pos_s[k].size())
 				continue;

 			auto zz = st.get(pos_s[k][cnt_s[k]]).mn;
 			assert (zz >= len);

 			ans = min(ans, zz - len + cost);
 			// t[0..len - 1] + j ... 
 		}

 		if (cnt_s[j] >= pos_s[j].size())
 			break;
 		auto zz = st.get(pos_s[j][cnt_s[j]]).mn;
 		assert (zz >= len);
 		cost += zz - len;
 		st.modify(0, pos_s[j][cnt_s[j]], 1);
 		++cnt_s[j];
