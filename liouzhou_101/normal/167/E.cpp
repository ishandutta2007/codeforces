...
namespace algorithm
{
	...
	{
	public:
		static ValueType Determinant(std::vector<std::vector<ValueType>> a)
		{
			int n = a.size();
			assert(n >= 1);
			for (int i = 0; i < n; ++i)
			...
			for (int i = 0; i < n; ++i)
			{
				int chosen = i;
				for (int j = i + 1; j < n; ++j)
					if (a[j][i] > a[chosen][i]) chosen = j;
				if (a[chosen][i] == 0) return 0;
				if (chosen != i)
				{
				...
				det *= a[i][i];
				ValueType inv = ValueType(1) / a[i][i];
				for (int k = i; k < n; ++k)
					a[i][k] *= inv;
				for (int j = i + 1; j < n; ++j)
					for (int k = n - 1; k >= i; --k)
...
using modular = SafeUserModularInt32;
...
void solve()
{
	...
	vector<int> id(n);
	for (int i = 0; i < n; ++i)
		if (in_degree[i] == 0) id[i] = cnt++;
	vector<vector<modint>> dp(cnt, vector<modint>(n));
	...
	modint res = GaussianElimination<modint>::Determinant(a);
