...
void solve(){
  ...
  for(li i = 0; i < (li)(n); ++i) A[i] -= 1;
  vi B(n); for(li i = 0; i < (li)(n); ++i) B[A[i]] = i;
  vi count(n, 1);
  vi to(n); for(li i = 0; i < (li)(n); ++i) to[i] = i;
  function<int(int)> find = [&](int i) -> int {
    if(i >= n) return i;
    return to[i] = (to[i]==i?i:find(to[i]));
  };
  multiset<int> vs;
  for(li i = 0; i < (li)(n); ++i) vs.insert(1);
  for(li i = 0; i < (li)(n); ++i) {
    int x = B[i];
    if(count[x] != *vs.rbegin()) {
      cout << "No" << endl;
      return;
    }
    int cnt = count[x];
    count[x] = 0;
    vs.erase(vs.find(cnt));
    to[x] = x+1;
    int y = find(x);
    if(y < n) {
      vs.erase(vs.find(count[y]));
      count[y] += cnt;
      vs.insert(count[y]);
    }
  }
