...
int main() {
    ...
    // dp2[0] = 1;
    // rep(i, n - 1) {
    //     dp[i + 1] = dp[i] * 8 + dp2[i] * 2;
    //     dp2[i + 1] = dp2[i] * 6;
    // }

    // mint top, bot = 1, k = 1, ans, y, z;
    // per(i, si(s)) bot += k * (s[i] - '0'), k *= 2;

    // vmint DP(n), DP2(n);

    int i = si(s);

    vvvv(mint, tmp, 2, 2, 2, 2);
    tmp[1][1][1][0] = true;
    fore(e, s) {
        i--;
        // k *= inv(2);
        vvvv(mint, ntmp, 2, 2, 2, 2);
        rep(i, 2) rep(j, 2) rep(k, 2) rep(p, 2) {
            rep(x, 2) rep(y, 2) rep(z, 2) {
                if(i and x > (e - '0')) continue;
                if(j and x < y) continue;
                if(k and y < z) continue;
                int np = p;
                if(!j and !k and !p) {
                    if((x ^ y) + (y ^ z) > (x ^ z)) np = 1;
                }
                ntmp[i & (x == (e - '0'))][j & (x == y)][k & (y == z)][np] += tmp[i][j][k][p];
            }
        }
        swap(tmp, ntmp);
        // dump(tmp[1][0][1][0]);
