...
void solve()
{
	int n;
	read(n);
	set<pair<pair<int, int>, pair<int, int>>> H;
	auto add = [&](int r1, int a1, int r2, int a2)
	{
		pair<int, int> x1 = { r1, a1 };
		pair<int, int> x2 = { r2, a2 };
		if (x1 > x2) swap(x1, x2);
		H.insert({ x1, x2 });
	};
	for (int i = 0; i < n; ++i)
	{
		...
		else
		{
			...
			read(r1, r2, a);
			for (int r = r1; r < r2; ++r)
				add(r, a, r, (a - 1 + 360) % 360);
		}
	}
	auto place = [](int r, int a)
	{
		return r * 360 + a;
	};
	auto check = [&](int r1, int a1, int r2, int a2)
	{
		pair<int, int> x1 = { r1, a1 };
		pair<int, int> x2 = { r2, a2 };
		if (x1 > x2) swap(x1, x2);
		return !H.count({ x1, x2 });
	};
	data_structure::DisjointSet g(21 * 360);
	for (int a = 0; a < 360; ++a)
		g.Union(place(0, a), place(0, (a + 1) % 360));
	for (int r = 1; r <= 20; ++r)
		for (int a = 0; a < 360; ++a)
		{
			if (check(r, a, r - 1, a))
				g.Union(place(r, a), place(r - 1, a));
			if (check(r, a, r, (a - 1 + 360) % 360))
				g.Union(place(r, a), place(r, (a - 1 + 360) % 360));
		}
	writeln(g.Find(place(0, 0)) == g.Find(place(20, 0)) ? "YES" : "NO");
}
