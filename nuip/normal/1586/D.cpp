...
int main(){
	...
	cauto ask=
		[&](const int ad){
			if(ad==0 || ad>=n || ad<=-n) return -1;
			int a=1;
			int b=abs(ad)+1;
			if(ad<0) a=n+1-a, b=n+1-b;
			assert(a+ad==b);
			cout<<"?";
			rep(_,n-1) cout<<" "<<a;
			cout<<" "<<b<<endl;
			cout.flush();
#ifdef NUIP
			rep(i,n)if(a+perm[i]==b+perm[n-1]) return i;
			return -1;
#else
			return read(1);
#endif
	...
	vector<int> ans(n);
	ans[n-1]=n-memo.size();
	reps(i,1,memo.size()) ans[memo[i]]=n-memo.size()+i;
	out(memo,ans,1);
	rep(i,ans.back()) ans[ask(i-ans.back())]=i;
	out(ans,1);
	// if(ans!=perm){
	// 	out(ans,perm,1);
	// 	exit(0);
