...
int main() {
	...
	auto getCost = [&](ll v, ll s) {
		ll sm = v / s;
		
		ll lcnt = v % s;
		ll scnt = s - lcnt;
		
		return sm * sm * scnt + (sm + 1) * (sm + 1) * lcnt;
	};
	
	auto getMarg = [&](int pos) {
		ll curCost = getCost(a[pos], seg[pos]);
		ll nxtCost = getCost(a[pos], seg[pos] + 1);
		
		return curCost - nxtCost;
	};
