...
int main(){
	...
	TT{
		...
		cauto m=read();
		cauto vs=readV(n*m);
		map<int,vector<int>> invs,invp;
		{
			auto srt=vs; sort(all(srt));
			rep(i,n*m) invs[srt[i]].eb(i);
			rrep(i,n*m) invp[vs[i]].eb(i);
		}
		vector<int> inds(n*m);
		for(cauto &p:invs){
			cauto &ss=p.Y;
			...
			reps(row,l+1,r){
				rrep(j,m){
					inds[row*m+j]=ps.back();
					ps.pop_back();
				}
			}
			rreps(i,r*m,ss.back()+1){
				inds[i]=ps.back();
				ps.pop_back();
			}
			assert(ps.empty());
		}
		// rep(i,n){rep(j,m) ouT(inds[i*m+j]+1); cout NL;}
		vector<int> inv(n*m);
		rep(i,n*m) inv[inds[i]]=i;
		ll re=0;
		for(cauto &i:inv){
			re+=sum(i)-sum(i/m*m);
			add(i,1);
