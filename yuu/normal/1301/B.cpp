...
int get(int max_allowed) {
    int low = 0, high = 1e9;
    for (int i = 2; i <= n; i++) {
        if (a[i] < 0) {
            if (a[i - 1] < 0) continue;
            low = max(low, a[i - 1] - max_allowed);
            high = min(high, a[i - 1] + max_allowed);
            if (low > high) return -1;
        } else if (a[i - 1] < 0) {
            low = max(low, a[i] - max_allowed);
            high = min(high, a[i] + max_allowed);
            if (low > high) return -1;
        } else {
            if (abs(a[i] - a[i - 1]) > max_allowed) return -1;
        }
    }
    if (low > high) return -1;
    else return low;
