...
int a,b,c;
vector<bool> ans;
vector<pair<pii,int>> risan;

void dfs(int u,int par)
{
  if (par >= 0)
  {
    if (asli.count(mp(u,par)))
    {
      risan.pb(mp(mp(u,par),asli[mp(u,par)]));
    }
    else risan.pb(mp(mp(u,par),ans[u] ^ ans[par]));
  }
...
int main()
{
  ...
  TC(Tcases)
  {
    ...
    else
    {
      ...
      puts("YES");
      REP(i,SIZE(risan))
        printf("%d %d %d\n",risan[i].fi.fi+1,risan[i].fi.se+1,risan[i].se);
    }
  }
  return 0;
}