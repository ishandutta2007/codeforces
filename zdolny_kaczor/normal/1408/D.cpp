...
void solve() {
	...
	scanf("%d%d", &n, &m);
	vpii ab(n), cd(m);
	for (auto &x : ab) scanf("%d%d", FS(&x));
	for (auto &x : cd) scanf("%d%d", FS(&x));
	vpii diffs;
	for (auto [a, b] : ab) for (auto [c, d] : cd) {
		int x = c - a, y = d - b;
		if (x >= 0 && y >= 0) {
			diffs.emplace_back(x + 1, y + 1);
		}
	}
	diffs.emplace_back(0, inf);
	diffs.emplace_back(inf, 0);
	sort(diffs.begin(), diffs.end());
	debug(imie(diffs));
	vpii boundary;
	for (auto curr : diffs) {
		while (!boundary.empty() && boundary.back().second <= curr.second)
			boundary.pop_back();
	...
	for (int i = 0; i < l - 1; ++i) {
		int curr = boundary[i].first + boundary[i + 1].second;
		debug(imie(curr));
		mini(ans, curr);
	}
	printf("%d\n", ans);
