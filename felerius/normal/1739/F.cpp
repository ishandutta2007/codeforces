// Keyboard Design (https://codeforces.com/contest/1739/problem/F)
...
int main() {
    ...
    vector<int> profit(1);
    rep(_, n) {
        int ci; string s; read(ci, s);

        array<set<int>, 12> adj{};
        rep(i, sz(s) - 1) {
        ...
        if (!all_of(all(adj), [&](auto& row) { return sz(row) <= 2; }))
            continue;

        int v0 = 0;
        while (v0 < 12 && sz(adj[v0]) != 1)
            ++v0;
        if (v0 == 12)
            continue;

        vector<int> path = {v0, *begin(adj[v0])};
        while (1) {
            int v = path.back(), v2 = path[sz(path) - 2];
            if (sz(adj[v]) == 1)
                break;
            if (*begin(adj[v]) == v2)
                path.push_back(*rbegin(adj[v]));
            else
                path.push_back(*begin(adj[v]));
        }

        int v1 = ac.add(all(path));
        int v2 = ac.add(rbegin(path), rend(path));
        profit.resize(sz(ac));
        profit[v1] += ci;
        profit[v2] += ci;
    }

    ac.build([&](int v, int suf_link, int, int) {
        profit[v] += profit[suf_link];
    });

    int ans = 0;
    array<int, 12> p{}, p_opt{};
    iota(all(p), 0);
    iota(all(p_opt), 0);
    auto enumerate = [&](int v, int prof, auto depth, auto&& self) -> void {
        if constexpr (depth() == 12) {
            if (prof > ans)
                ans = prof, p_opt = p;
        } else {
            rep(i, depth(), 12) {
                int v2 = ac.next(v, p[i]);
                int prof2 = prof + profit[v2];
                swap(p[i], p[depth()]);
                self(v2, prof2, integral_constant<int, depth() + 1>{}, self);
                swap(p[i], p[depth()]);
            }
        }
    };
    enumerate(0, 0, integral_constant<int, 0>{}, enumerate);

    string s;
    rep(i, 12)
        s.push_back(char('a' + p_opt[i]));
    println(s);
}