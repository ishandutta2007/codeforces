#line 1 "codeforces-workspace\\contest\\1572\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1572\\b\\b.cpp"
...
void main() {
  ...
  partial_sum(begin(a), end(a), begin(a), bit_xor<>());
  a.emplace(a.begin(), 0);
  ...
  auto pos = 1;
  for (; pos < a.size() and a[pos]; pos += 2) continue;

  if (pos >= a.size()) eject("NO");

  for (auto i = pos; i + 3 < a.size(); i += 2) {
    ope(i + 1);
  }

  for (auto i = pos; i - 2 > 0; i -= 2) {
    ope(i - 2);
  }
  ...
  assert(count(begin(a), end(a), 0) == a.size());
