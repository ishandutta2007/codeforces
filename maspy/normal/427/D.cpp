  ...
  */
  auto calc = [&](string S) -> vi {
    ...
    FOR(i, N) {
      ll j = sa.SA[i];
      ll n = 1;
      if (i) chmax(n, LCP[i - 1] + 1);
      if (i < len(LCP)) chmax(n, LCP[i] + 1);
      min_len[j] = n;
    }
    return min_len;
  };
  ...
  2 index  LCS
  */
  ll N1 = len(S1), N2 = len(S2);
  vv(ll, LCS, N1, N2);
  FOR_R(i, N1) FOR_R(j, N2) {
    if (S1[i] != S2[j]) continue;
    ll x = (i + 1 < N1 && j + 1 < N2 ? LCS[i + 1][j + 1] : 0);
    LCS[i][j] = x + 1;
  }

  ll ANS = INF;
  FOR(i, N1) FOR(j, N2) {
    ll r = LCS[i][j]; // max length
    ll l = max(LEN1[i], LEN2[j]);
    if (l <= r) chmin(ANS, l);
  }
  if (ANS == INF) ANS = -1;
  print(ANS);
