...
struct TreeMax {
    ...
    int getMax(int cur, int l, int r, int x, int y) {
        ...
        int m = (l + r) / 2;
        int dcur = cur + cur;
        return max(getMax(dcur, l, m, x, y), getMax(dcur + 1, m + 1, r, x, y));
    }
};
TreeMax tr[15];
int a[205050];
...
int solve() {
    ...
    for (int i = 2; i <= N; ++i) {
        ...
        int par = i / pr[i];
        if (pr[i] == pr[par]) {
            code[i] = code[par / pr[i]];
            vecs[i] = vecs[par / pr[i]];
        } else {
            code[i] = pr[i] * code[par];
            vecs[i] = vecs[par];
            vecs[i].add(pr[i]);
        }
        if (code[i] == i) {
...
//    DBN(code[4], code[8], code[16], code[4 * 9 * 3]);
//    DBN(cnt);
//    exit(0);
//    CURTIME();
    int n, q;
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 0; j < (1 << v.sz); ++j) {
            ...
            for (int k = 0; k < 8; ++k) {
                best[cb + k] = max(best[cb + k], where[what][k]);
            }
            where[what][cb] = i;
        }
//        DBN(i, vecs[a[i]].a);
        for (int j = 0; j < 15; ++j) {
            if (best[j] != 0) {
//                DBN(i, j, best[j]);
                tr[j].update(1, 1, n, i, best[j]);
            }
        }
        while (cur < q && qq[cur].a <= i) {
            int ans = 0;
            while (ans < 14) {
                int t = tr[ans].getMax(1, 1, n, qq[cur].b, qq[cur].a);
//                DBN(ans, qq[cur].b, qq[cur].a, t);
                if (t >= qq[cur].b) {
                    break;
                }
                ++ans;
