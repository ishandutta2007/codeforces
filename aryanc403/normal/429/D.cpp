...
lli d(const ii &a,const ii &b)
{
    return (a.X-b.X)*(a.X-b.X)+(a.Y-b.Y)*(a.Y-b.Y);
}

lli solve(vii &a,lli l,lli r)
{
    if(l==r)
        return INF;
    lli m=(l+r)/2,ans=INF,i,j;
    // sort(a.begin()+l,a.begin()+r+1,cmpy);
    ans=min(ans,solve(a,l,m));
    ans=min(ans,solve(a,m+1,r));
    vii b;
    // sort(a.begin()+l,a.begin()+m+1,cmpy);
    repA(i,l,r)
        if(d(a[i],a[m])<=ans)
            b.pb(a[i]);
    dbg(l,r,a[m],b);
    m=(lli)b.size();
    fo(i,m) repA(j,i+1,m-1)
        ans=min(ans,d(b[i],b[j]));
