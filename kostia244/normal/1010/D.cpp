...
int calc(int i) {
	...
	if(g[i].t == "NOT")
		return res[i] = !calc(g[i].x);
	int a = calc(g[i].x), b= calc(g[i].y);
	return res[i] = g[i].eval(a, b);
}
...
void dfs(int i) {
	...
	bool canx = !((g[i].t=="AND"&&res[g[i].y]==0)||(g[i].t=="OR"&&res[g[i].y]==1));
	bool cany = !((g[i].t=="AND"&&res[g[i].x]==0)||(g[i].t=="OR"&&res[g[i].x]==1));
	if(canx)
		dfs(g[i].x);
	if(cany)
		dfs(g[i].y);
