...
void solve() {
    ...
    vector<string> s(n);
    read(s);
    vector<vector<bool>> can(m + 1, vector<bool>(1 << n));
    vector<vector<char>> p(m + 1, vector<char>(1 << n));
    vector<vector<int>> ws(m + 1, vector<int>(1 << n));
    can[0][0] = true;
    for (int i = 1; i <= m; ++i) {
        map<char, int> used;
        for (int j = 0; j < n; ++j)
