...
int dp[10000] = {};
vector<pair<int, int>> nexts[10000];
void solve(){
    int n;
    cin >> n;
    Compressor<int> zipper;
    REP(i, n) {
        int a, b;
        cin >> a >> b;
        zipper.add(a);
    ...
    zipper.add(1e9);
    zipper.exec();
    vector<tuple<int, int, int>> sortings;
    vector<pair<int, int>> inputs;
    REP(i, n) {
        int a = zipper.fetch();
        int b = zipper.fetch();
        sortings.push_back(make_tuple(b - a, a, b));
    }
    sort(ALL(sortings));
    REP(i, sortings.size()) {
        inputs.push_back(mp(get<1>(sortings[i]), get<2>(sortings[i])));
    }
    vector<int> cnter(n, 0);
    REP(i, n) {
        REP(q, i) {
            if (inputs[i].first <= inputs[q].first and inputs[q].second <= inputs[i].second) {
                nexts[inputs[q].first].push_back(mp(inputs[q].second + 1, cnter[q]));
            }
        }
        for (int q = inputs[i].first; q <= inputs[i].second; ++q) {
            while (nexts[q].empty() == false) {
                dp[nexts[q].back().first] = max(dp[nexts[q].back().first], dp[q] + nexts[q].back().second);
                nexts[q].pop_back();
            }
            dp[q + 1] = max(dp[q + 1], dp[q]);
            dp[q] = 0;
        }
        cnter[i] = dp[inputs[i].second + 1] + 1;
        dp[inputs[i].second + 1] = 0;
    }
    cout << cnter.back() - 1 << endl;
}
#undef int
int main() {
    init();
