...
int tout[202020];
int gl[202020];
void goInits(int cur, int p, int curg = 0) {
    tin[cur] = ++T;
    pr[cur][0] = p;
    gl[cur] = curg;
    for (int i = 1; i < K; ++i) {
...
vector<tri> vq[202020];
int dp[202020];
int dp2[202020];
int getNext(int from, int to) {
    if (to == from) {
        return 200020;
    }
    int dif = gl[to] - gl[from] - 1;
    for (int i = 0; i < K; ++i) {
        if (dif & 1) {
            to = pr[to][i];
        }
...
vector<pii> needCalc[201012];
int sumDDifOnnTheWay(int from, int to) {
    if (from == to) return 0;
    int dif = gl[to] - gl[from];
    int res = 0;
    for (int i = 0; i < K; ++i) {
        if (dif & 1) {
            res += prsum[to][i];
...
void go(int cur) {
    int p = pr[cur][0];
    for (int i = 1; (1 << i) - 1 <= gl[cur]; ++i) {
        needCalc[p].push_back({cur, i});
        p = pr[p][i];
    }
    dp[cur] = 0;
    for (int to : v[cur]) {
    ...
    for (auto q : vq[cur]) {
        ...
        int cost = q.c;
//        DBN(cur, x, y);
        dp[cur] = max(dp[cur], cost + dp2[cur] + sumDDifOnnTheWay(cur, x) + sumDDifOnnTheWay(cur, y));
    }
//    DBN(cur, dp2[cur], dp[cur]);
    prsum[cur][0] = dp2[cur] - dp[cur];
    for (auto p : needCalc[cur]) {
//        DBN(cur, p);
        int w = p.first;
        int lvl = p.second;
        prsum[w][lvl] = prsum[w][lvl - 1] + prsum[pr[w][lvl - 1]][lvl - 1];
    }
}
int32_t solve() {
    int n, m;
