...
int main() {
   ...
   //read(FILENAME);
    ...
    long long sum = 0;
    for (int i = 0; i < sz(g[0]) - 1; i++) {
        int l = g[0][i];
        int r = g[0][i + 1];
        auto it = lower_bound(all(g[1]), l);
        auto it1 = lower_bound(all(g[2]), l);
        bool no = false;
        if (it != g[1].end() && it1 != g[2].end()) {
            if (*it <= r && *it1 <= r) {
                no = true;
            }
        }
        if (no) {
            ...
            if (it != g[1].end() && *it <= r) {
                vector<int> pos;
                auto it2 = it;
                while (it2 != g[1].end() && *it2 <= r) {
                    pos.push_back(*it2);
                    it2++;
                }
                long long f = min(pos.back() - l, r - pos[0]);
                for (int j = 0; j <sz(pos) - 1; j++) {
                    chkmin(f, pos[j] - l + r - pos[j + 1]);
                }
                sum1 += f;
            }
            if (it1 != g[2].end() && *it1 <= r) {
                auto it2 = it1;
                 vector<int> pos;
                while (it2 != g[2].end() && *it2 <= r) {
                    pos.push_back(*it2);
                    it2++;
                }
                long long f = min(pos.back() - l, r - pos[0]);
    ...
    if (sz(g[0]) == 0) {
        ...
        if (sz(g[0]) == 1) {
            for (int it = 1; it <= 2; it++) {
                if (!g[it].empty()) {
                    if (g[it][0] <= g[0][0] && g[it].back() >= g[0][0]) {
                        sum += g[it].back() - g[it][0];
                    } else {
                        if (g[it].back() <= g[0][0]) {
                            sum += g[0][0] - g[it][0];
                        } else {
