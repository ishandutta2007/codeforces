...
int n, m;
int a[101][101];
int64_t get(int x, int y, int u, int v) {
    static set<tuple<int, int, int>> s;
    s.clear();
    s.insert({a[x][y], x, y});
    s.insert({a[x][v], x, v});
    s.insert({a[u][y], u, y});
    ...
    values.clear();
    for (auto&& [a, b, c] : s) values.emplace_back(a);
    int med = values[values.size() / 2];
    int64_t res = 0;
    for (auto&& x : values) res += abs(med - x);
    return res;
}
void solve() {
    cin >> n >> m;
