...
void solve(){
    ...
    for(int c = 0; c < n; c++){
        ...
        for(int u: g[c]){
            ...
            vector<int> cnt(mx+1);
            for(int i: visited) cnt[dist[i]]++;
            for(int d = 1; d <= mx; d++) vv[d].push_back(cnt[d]);
        }
        for(int d = 1; d <= n-1; d++){
            vector<mint> cur = {mint(1)};
            for(int x: vv[d]){
                cur = naive_convolution(cur, {mint(1), mint(x)});
            }
            if(cur.size() >= k+1) ans += cur[k];
        }
    }
    cout <<  ans << endl;
}
