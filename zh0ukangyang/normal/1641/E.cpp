...
void solve(int L, int R) {
	...
	A *= B;
	L(i, 0, sz(A) - 1) 
		(rns[i + L + mid + 1] += A[i]) %= mod;
//	L(i, L, mid) 
//		L(j, mid + 1, R) 
//			(rns[i + j] += (ll) wb[i] * wa[j] % mod) %= mod;
	solve(L, mid);
	solve(mid + 1, R);
}
...
//		L(j, 0, min(n * 3 - i, i - 1)) {
//			int x = i - j, y = j + i;
//			(ns += (ll) zp * wa[y] % mod * wb[x] % mod) %= mod, 
//				cout << zp << " : " << (ll) qpow(2, m - (pre[y] - pre[x - 1])) << '\n';
//		}
//	}
	
	ns = (ll) ns * qpow(2, m) % mod;
	L(i, n + 1, n * 2) 
		(ns += mod - (ll) min(i, n * 3 - i + 1) * a[i - n] % mod) %= mod;
	ns = (ll) ns * qpow(qpow(2, m) - 1) % mod;
	cout << ns << '\n';
	return 0;
}