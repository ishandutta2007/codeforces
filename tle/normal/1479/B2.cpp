...
void main()
{
	...
	f[0]=0; mt.insert(pii(0,0));
	int dt=0;
	for(int i=1;i<=n;++i) {
		int u1=mt.begin()->fi+dt+1;
		if(f.count(a[i])) u1=min(u1,f[a[i]]+dt);
		dt+=(a[i]!=a[i-1]); u1-=dt;
		if(!f.count(a[i-1]))
			f[a[i-1]]=u1;
		else {
			mt.erase(pii(f[a[i-1]],a[i-1]));
			f[a[i-1]]=min(f[a[i-1]],u1);
		}
		mt.insert(pii(f[a[i-1]],a[i-1]));
	}
	cout<<mt.begin()->fi+dt<<"\n";
}
