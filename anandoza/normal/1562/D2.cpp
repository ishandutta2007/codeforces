...
void solve() {
    ...
    vi prefix{0};
    for (int i = 0; i < n; i++)
        prefix.push_back(prefix.back() + (i % 2 == 0 ? 1 : -1) * (s[i] == '+' ? 1 : -1));

    map<int, vector<int>> poss;
    for (int i = 0; i + 1 <= n; i++) {
        poss[prefix[i] + prefix[i + 1]].push_back(i);
    }
    ...
    while (q--) {
        ...
        int target = prefix[l] + prefix[r];
        auto &row = poss[target];
        auto it = lower_bound(all(row), l);
        ps(*it + 1);
    }
}
