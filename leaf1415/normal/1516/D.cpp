...
int main(void)
{
	...
	rep(i, 2, 100000){
		if(prime[i]) continue;
		for(int j = 2*i; j <= 100000; j+=i) prime[j] = i;
	}
	...
	rep(i, 1, n){
		while(r < n){
			...
			if(!flag) break;
			for(auto p : mp[r+1]) fcnt[p.first] += p.second;
			r++;
		}
		succ[0][i] = r+1;
		for(auto p : mp[i]) fcnt[p.first] -= p.second;
	}
	succ[0][n+1] = n+1;
	rep(i, 1, 16) rep(j, 1, n+1) succ[i][j] = succ[i-1][succ[i-1][j]];
	
	ll l;
	rep(q, 1, Q){
		cin >> l >> r;
		ll ans = 0, p = l;
		per(i, 16, 0) if(succ[i][p] <= r) ans += 1<<i, p = succ[i][p];
		ans++;
		cout << ans << "\n";
	}
