// time-limit: 1000
// problem-url: https://codeforces.com/contest/1698/problem/F


// okay I did prove this is correct, but I don't really trust my ability to prove things *especially* for CP stuff
// ...
// anyway, in real contest it would be more efficient to not prove...
// ??? unless it FST...?
...
impossible:
	...
	auto const op=[&](int left, int right){
		assert(a[left]==a[right-1]);
		assert(left<right);
		std::reverse(a.begin()+left, a.begin()+right);
		operations.push_back({left, right});
	};

	if(a.size()>2)
	for(int const index: range(1, int(a.size())-1)){
		if(a[index]!=b[index]){
			for(int const j: range(index+1, number)){
				if(a[j]==a[index-1] and a[j-1]==b[index]){
					op(index-1, j+1);
					goto next_index;
				}
			}
			...
			for(int const j: range(index, number-1)){
				if(a[j]==a[index-1] and a[j+1]==b[index]){

					std::map<int, int> posOf; // on left side
					for(int const k: range(index, j+1)){
						posOf[a[k]]=k;
					}
					for(int const k: range(j+1, number)){
						auto const iter=posOf.find(a[k]);
						if(iter!=posOf.end()){
							op(iter->second, k+1);
							op(index-1, iter->second+k-j+1);
							goto next_index;
						}
					}
