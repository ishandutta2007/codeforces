...
////        x=fft(N,-2*PI/N,x);
////        vll ret;
////        for(int i=0;i<x.size();i++) ret.push_back((ll) (x[i].real()/N+0.5));
////        return ret;
////    }
//    vi getFFT(const vi & A)
//    {
//        vector <C> x;
...
//        int N=1;
//        while(N<x.size()) N<<=1;
//        while(x.size()<N) x.push_back(C(0,0));
//        x=fft(N,2*PI/N,x);
...
bool isless(const vi & a, const vi & b) {
    if(a.size() != b.size()) return a.size() < b.size();
    int pt = a.size() - 1;
    while(pt>=0 && a[pt] == b[pt]) pt--;
    if(pt>=0) return a[pt] < b[pt];
    else return false;
}
...
int main()
{
    ...
    scanf("%s", c);
    string s(c);
    for(auto sm : s) n.pb(int(sm - '0'));
    reverse(all(n));
    ld u = log(ld(10))/log(ld(3)) * (s.size() - 1) + log(ld(s[0]-'0'))/log(ld(3));
    int md = u;
    md = md - min(4,md);
    auto fr = kpow(3,md);
    int best = 1e9;
