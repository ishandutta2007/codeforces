...
void solve() {
  ...
  vc<mint> L(N + 1);
  FOR(x, 2, N + 1) { L[x] = mint(2).pow(x / 2 - 1); }

  // 
  vc<mint> dp(N + 1);
  vc<mint> dpc(N + 1); // sum of [i, N]
  dp[N] = dpc[N] = 1;
  FOR_R(i, 1, N) {
    dp[i] = dpc[i + 1];
    if (2 * i <= N) dp[i] -= dpc[2 * i];
    dpc[i] = dpc[i + 1] + dp[i];
  }
  FOR(x, 1, N + 1) dp[x] *= L[x];

  mint den = SUM<mint>(dp);
  assert(den == mint(2).pow(N - 2));
  mint cf = mint(1) / den;
  for (auto&& x: dp) x *= cf;
  FOR(i, 1, N + 1) print(dp[i]);
}
