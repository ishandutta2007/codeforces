...
int main()
{
	...
	if (num == 2)
	{
		int a = awalsatu[SIZE(awalsatu) - 1];
		int b = awalnol[SIZE(awalnol) - 1];
		REP(i,n) {
			if (i > 0) printf("->");
			if (a - 1 <= i && i < b) printf("(");
			printf("%d",data[i]);
	...
	FOR(i,1,n)
	{
		if (i == n - 3) printf("->(%d",data[i]);
		else if (i == n - 2) printf("->%d)",data[i]);
