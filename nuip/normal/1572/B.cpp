...
vector<int> solve(vector<int> vs){
	const int n=vs.size();
	if(accumulate(all(vs),0)%2) return {};
	if(n%2==0 && vs[0] && vs.back()){
		int ng=1;
		reps(i,2,n)if(i%2==0 && vs[i]!=vs[i-1]) ng=0;
		if(ng) return {};
	}
	vector<int> re;
	cauto op=
		[&](const int i){
			cauto t=vs[i]^vs[i+1]^vs[i+2];
			vs[i]=vs[i+1]=vs[i+2]=t;
			re.eb(i);
		};
	if(vs[0] && vs.back()){
		int i=1;
		while(vs[i]==vs[i+1]) i+=2;
		while(i>0){
			op(i-1);
			i-=2;
		}
	...
	assert(*max_element(all(vs))==0);
	if(flip)for(auto &i:re) i=n-1-(i+2);
	return re;
}
...
int main(){
	...
	TT{
		...
		if(re.empty()){
			...
			cout<<"YES" NL;
			if(re.size()>n) rep(_,MOD) cout<<"2"<<endl;
			cout<<re.size() NL;
