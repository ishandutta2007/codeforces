...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read(),m=read();
        for(res i=1;i<=n;i++)s[i]=0;
        for(res i=1;i<=m;i++){
            vector<int>().swap(f[i]),id[i]=i;
            res k=read();
            while(k--)f[i].pb(read());
        }
        sort(id+1,id+m+1,[&](const res &A,const res &B){return f[A].size()<f[B].size();});
        for(res i=1;i<=m;i++){
            auto A=f[id[i]];
            if(A.size()==1){
                res x=A[0];
                s[x]++,ans[id[i]]=x;
                if(s[x]>(m+1)/2){puts("NO");return;}
            }
            else {
                for(auto x:A)if(s[x]+1<=(m+1)/2){s[x]++,ans[id[i]]=x;break;}
            }
        }
        puts("YES");
