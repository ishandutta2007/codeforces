...
int main() {
    ...
    order_statistics_tree<ll> flips;

    auto get_ans = [&]() -> ll {
        if (flips.find(1) == flips.end())
            return 1;
        return *next(flips.begin());
    };

    auto flip = [&](ll i) {
        auto res = flips.insert(i);
        if (!res.second)
            flips.erase(res.first);
    };

    auto erase_range = [&](ll l, ll r) {
        vector<ll> to_delete{flips.lower_bound(l), flips.lower_bound(r)};
        for (auto i : to_delete)
            flip(i);
    };

    auto on = [&](ll i) -> bool { return flips.order_of_key(i + 1) % 2 == 1; };
