...
void solve() {
    ...
    reverse(all(ord));
    dp.assign(n, vll(n, 0));
    big.assign(n, vb(n, false));
    for (int i : ord) {
        dp[i][0] = a[i];
        if (a[i]>=n) big[i][0]=true;
        for (int nb : rev[i]) F0R(j, n-1) {
            dp[i][j+1] = (dp[i][j+1]+dp[nb][j])%MOD;
            if (big[nb][j] || dp[i][j+1]>=n) big[i][j+1]=true;
        }
    }
    int x = ord.back();
    bool ok = false;
    ll ans = 0;
    F0R(i, n) {
        if (ok) ans = (ans+dp[x][i])%MOD;
        else {
            if (big[x][i] || dp[x][i]>0) ans = (max(ans, (ll)i) + dp[x][i])%MOD;
            if (big[x][i]) ok=true;
            if (ans>=n) ok=true;
        }
    }
