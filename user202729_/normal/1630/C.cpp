...
int main(){
	...
	std::vector<int> last(number); // numbers that never happen does not matter
	...
	std::vector<int> lastEqual(number);
	for(int const index: range(0, number)){
		lastEqual[index]=last[a[index]];
	}

	// compute prefix max (inclusive) of lastEqual.
	for(int const index: range(1, number)){
		lastEqual[index]=std::max(lastEqual[index], lastEqual[index-1]);
	}

	int right=1;
	int result=1;
	while(right<number){
		right=std::max(right+1, lastEqual[right-1]+1);
		++result;
	}
