...
void dfs(int v, vector<int> &in, vector<int> &out, vector<int> &up, int &T, int &q)
{
    in[v] = up[v] = T++;
    for (;;)
    {
        if (q == 3 * (int)in.size())
        {
            while ((double)clock() / CLOCKS_PER_SEC < 1.0) ;
            cout << "! No" << endl;
            exit(0);
        }
        int x;
...
void solve()
{
    ...
    //for (auto e : p) cout << "! " << e.first << " " << e.second << endl; cout << endl;
    //for (auto e : q) cout << "! " << e.first << " " << e.second << endl; cout << endl;

    sort(all(p));
    sort(all(q));
    ll ans = 0;
    for (int mask = 0; mask < (1 << (int)q.size()); mask++)
    {
        ll a = 1, b = 1, t = 1;
        ...
        {
            while (j < p.size() && p[j].first < q[i].first) j++;
            ll X = 1, Y = 1;
            for (int j = 0; j < q[i].second; j++) Y *= q[i].first;
            if (j < p.size() && p[j].first == q[i].first)
                for (int k = 0; k < p[j].second; k++) X *= p[j].first;
            if (mask & (1 << i)) if (X == Y) t = 0;
            if (mask & (1 << i)) b *= Y, a *= X;
            else a *= Y, b *= X;
            //cout << i << " " << q.size() << " " << j << " " << p.size() << endl;
            //cout << X << " " << Y << endl;
        }
        //cout << a << " " << b << " " << t << endl;
        ans += (t == 1 && l <= a && a <= r && l <= b && b <= r);
    }
    cout << ans << "\n";
}
