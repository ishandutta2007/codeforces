...
int main() {
	...
	for(int i = 0; i < m; i++) {
		in >> a >> b;
		mat1[a - 1][b - 1] = mat1[b - 1][a - 1] = 1;
		mat2[a - 1][b - 1] = mat2[b - 1][a - 1] = INT_MAX;
	}
	for(int k = 0; k < n; k++)
		for(int i = 0; i < n; i++)
			for(int j = 0; j < n; j++) {
				if(mat1[k][j] != INT_MAX && mat1[i][k] != INT_MAX && mat1[i][k] + mat1[k][j] < mat1[i][j])
					mat1[i][j] = mat1[i][k] + mat1[k][j];
				if(mat2[k][j] != INT_MAX && mat2[i][k] != INT_MAX && mat2[i][k] + mat2[k][j] < mat2[i][j])
					mat2[i][j] = mat2[i][k] + mat2[k][j];
			}
	int ans = Max(mat1[0][n - 1], mat2[0][n - 1]);
