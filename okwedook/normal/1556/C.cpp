...
signed main() {
    ...
    for (int i = 1; i < n; ++i) {
        if (i & 1) {
            p.pb({curr - c[i], curr - 1});
            curr -= c[i];
        } else {
            p.pb({curr + 1, curr + c[i]});
            curr += c[i];
        }
    }
    dbg(p);
    auto intersect = [](pll a, pll b) {
        return pll(max(a.f, b.f), min(a.s, b.s));
    };
    auto getlen = [](pll a) {
        return max(0ll, a.s - a.f + 1);
    };
    auto inside = [](ll x, pll a) {
        return a.f <= x && x <= a.s;
    };
    ll ans = 0;
    for (int i = 0; i < sz(p); ++i) {
        ...
        for (int j = i + 1; j < sz(p); ++j) {
            if (i % 2 == 0 && j % 2 == 1) {
                // dbg(p[i], p[j]);
                // dbg(p[i].f, p[i].s + mncurr);
                ll was = ans;
                ans += getlen(intersect({p[i].f, p[i].s + mncurr}, p[j]));
                // dbg(ans - was);
            }
            if (i % 2 == 1 && j % 2 == 1) {
                // dbg(mncurr == 0 && inside(p[i].s, p[j]));
                ans += mncurr == 0 && inside(p[i].f, p[j]);
            }
            if (j & 1) curr -= c[j];
            else curr += c[j];
            chmin(mncurr, curr);
