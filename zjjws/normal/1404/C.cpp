...
namespace FHQ
{
    ...
    int rs[N];
    int siz[N];
    inline void up(int i){siz[i]=siz[ls[i]]+siz[rs[i]]+1;return;}
    inline int joojoo(int x,int y){if(!x||!y)return x+y;if(w[x]>w[y])return rs[x]=joojoo(rs[x],y),up(x),x;return ls[y]=joojoo(x,ls[y]),up(y),y;}
    inline void slim(int &x,int &y,int &key,int now)
    {
        if(!now){x=y=0;return;}
        if(v[now]<=key)x=now,slim(rs[now],y,key,rs[now]);else y=now,slim(x,ls[now],key,ls[now]);up(now);return;
    }
    inline int find(int now,int k)
    {
        if(siz[rs[now]]>=k)return find(rs[now],k);
        if(siz[rs[now]]+1<k)return find(ls[now],k-(siz[rs[now]]+1));
        return now;
    }
    inline void insert(int &num)
    {
        v[num]=b[num];w[num]=rand();ls[num]=rs[num]=0;siz[num]=1;
        int x,y;slim(x,y,v[num],root);root=joojoo(joojoo(x,num),y);return;
    }
}
...
namespace President_Tree
{
    ...
    int gyq[N<<6];
    int gyqcutt;
    inline void insert(int l,int r,int &lst,int &now,int &v)
    {
        gyq[now]=gyq[lst]+1;if(l==r)return;
        int mid=l+r>>1;if(v<=mid)insert(l,mid,ls[lst],ls[now]=++gyqcutt,v),rs[now]=rs[lst];
        else insert(mid+1,r,rs[lst],rs[now]=++gyqcutt,v),ls[now]=ls[lst];return;
    }
    inline int check(int l,int r,int &lst,int &now,int &v)
    {
        if(l>=v||gyq[now]==gyq[lst])return gyq[now]-gyq[lst];
        int mid=l+r>>1,ans=0;if(v>mid)return check(mid+1,r,rs[lst],rs[now],v);return gyq[rs[now]]-gyq[rs[lst]]+check(l,mid,ls[lst],ls[now],v);
    }
}
...
inline void work()
{
    int l,r;rin>>l>>r;l++;r=n-r;if(l>r){puts("0");return;}
    printf("%d\n",check(1,n,st[l-1],st[r],l));return;
}
int main()
{
    srand(time(0));rin>>n>>q;st[0]=(gyqcutt=1);
    for(int i=1;i<=n;i++)rin>>a[i];
    for(int i=1;i<=n;i++)
    {
        int les=i-a[i];
        if(les==0)b[i]=i,cutt++;
        if(les>0&&les<=cutt){using namespace FHQ;b[i]=b[find(root,les)];cutt++;}
        if(b[i]){using namespace FHQ;insert(i);}
        if(b[i])insert(1,n,st[i-1],st[i]=++gyqcutt,b[i]);else st[i]=st[i-1];
    }
