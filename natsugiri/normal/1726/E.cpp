...
Mint dp[300011];
Mint odd_fact[300011];
Mint pow2[300011];
void init() {
    dp[0] = 1;
    dp[1] = 1;
    for (int i=2; i<=300000; i++) dp[i] = dp[i-1] + (i - 1) * dp[i-2];

    odd_fact[1] = 1;
    for (int i=3; i<=300000; i+=2) odd_fact[i] = odd_fact[i-2] * i;

    pow2[0] = 1;
    for (int i=1; i<=300000; i++) pow2[i] = pow2[i-1] + pow2[i-1];
...
void MAIN() {
    ...
    for (int a=0; a*4<=N; a++) {
	int s = N - a*4;
	Mint tmp = nCk(s + a*2, s) * dp[s] * (a? odd_fact[a*2-1]: 1) * pow2[a];
	ans += tmp;
    }
