...
#line 2 "C:/Users/W/c/lib/util/compress.cpp"

#line 4 "C:/Users/W/c/lib/util/compress.cpp"
...
int main() {
  ll n = IN, bi = IN; bi *= 8; bi /= n;
  if (bi >= 62) EXIT(0);
  bi = 1LL << bi;
  auto a = IN.vi(n);

  Compress<int> comp(all(a));
  int m = comp.size();
  if (m <= bi) EXIT(0);

  VI c(m), l(m+1), r(m+1);
  rep(i, n) c[comp[a[i]]]++;
  rep(i, m) l[i+1] = l[i] + c[i];

  int x = INF;
  rep(i, m+1-bi) {
    chmin(x, n - (l[i+bi] - l[i]));
  }
