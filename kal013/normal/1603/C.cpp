...
void solve(){
	...
	int prv = 0;
	dp[a[n - 1]][prv] = {0, 1};
	vector<int> awake = {a[n - 1]};

	for(int i = n - 2; i >= 0; --i){

		int cur = prv ^ 1;
		vector<int> new_awake;

		for (const auto& ids: awake){
			int moves = (a[i] - 1) / ids;
			int new_v = a[i] / (moves + 1);
			assert (new_v <= a[i] && new_v >= 1);
			if (!vis[new_v]){
				new_awake.push_back(new_v);
				vis[new_v] = true;
			}
			array<mint, 2> tmp = {dp[ids][prv][0] + dp[ids][prv][1] * moves, dp[ids][prv][1]};
			dp[ids][prv] = {0, 0};
			array<mint, 2>& t_dp = dp[new_v][cur];
			t_dp[0] += tmp[0]; t_dp[1] += tmp[1];
		}
		dp[a[i]][cur][1] += 1;
		if (!vis[a[i]]){
			new_awake.push_back(a[i]);
			vis[a[i]] = true;
		}
		for(const auto &e: new_awake){
			vis[e] = false;
			ans += dp[e][cur][0];
		}

		swap(awake, new_awake);
		swap(cur, prv);

	}
	for(const auto &e: awake){
		dp[e][0] = {0, 0};
		dp[e][1] = {0, 0};
		vis[e] = false;
	}
