...
int main(){
	...
	cauto t=read();
	vv<int> indss(t);
	for(auto &inds:indss) inds=readV(read(),1);
	out(indss,1);
	cauto no=[](){ cout<<"REJECTED" NL; exit(0);};
	vector<pll> cands{{0,0}};
	for(cauto &inds:indss){
		cauto cur=cands;
		cands.clear();
		for(cauto &[fix,val]:cur){
			int unk=0;
			int one=0,zero=0;
			for(cauto &i:inds){
				if(fix>>i&1){
			...
			rep(ad,unk+1){
				...
				for(cauto &i:inds){
					...
					if((nval>>i&1)!=exp) nval^=1ll<<i;
				}
				cands.eb(nfix,nval);
			}
		}
		sort(all(cands)); UNIQUE(cands);
	}
	cauto check=
		[&](ll st){
			st^=(1ll<<n)-1;
			return __builtin_popcountll(st+1)==1;
		};
	for(auto [fix,val]:cands){
		out(fix,val,1);
		fix^=(1ll<<n)-1;
		if(__builtin_popcountll(fix)>1) no();
		if(!check(val)) no();
		if(fix){
			cauto d=__builtin_ctzll(val);
			if(!check(val|(1ll<<d))) no();
		}
	}
	cout<<"ACCEPTED" NL;
	return 0;
