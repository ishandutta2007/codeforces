// Required Length (https://codeforces.com/contest/1681/problem/D)
...
void solve(int done, uint64_t x) {
    uint64_t rem = target / x;
    int lower = 0;
    while (rem > 1)
        rem /= 9, ++lower;
    if (done + lower >= upper)
    ...
    repr(i, 9, 2) {
        if (!has[i])
            continue;
        auto nx = __int128_t(x) * i;
        if (nx >= target)
            upper = min(upper, done + 1);
        else
            solve(done + 1, uint64_t(nx));
    }
}
