...
int main(){
    ...
    VEC(ll,a,n);
    map<pll,ll>mem;
    function<ll(ll,ll)> grundy=[&](ll x,ll ban){
        ban&=(2LL<<x)-1;
        if(mem.count({x,ban}))return mem[{x,ban}];
        ll& ans=mem[{x,ban}];
        vector<ll>m;
        rep(i,1,x+1)if(!(ban&1LL<<i))m.push_back(grundy(x-i,ban|1LL<<i));
        Sort(m);
        each(x,m)if(x==ans)ans++;
        return ans;
    };
    ll ans=0;
    each(x,a)ans^=grundy(x,0);
    YES(!ans);
}