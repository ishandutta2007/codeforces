...
void solve()
{
    ...
    for (int i = 0; i<=n; i++) b.push_back(mp(pref[i], i));
    sort(b.begin(), b.end());
    ...
    //for (int i = 0; i<=n; i++) cout<<i<<": "<<b[i].first<<' '<<b[i].second<<endl;

    for (int i = 0; i<=n; i++)
    {
        while (pos<=n && b[pos].first<b[i].first - s)
        {
            //cout<<i<<' '<<pos<<' '<<b[i].first - s<<' '<<b[pos].first<<endl;
            vals.insert(b[pos].second);
            pos++;
        }
        ...
        auto it = vals.lower_bound(pos1);

        int who = n+1;
        if (it!=vals.end()) who = *it;


        if (who == pos1+1) continue;

        //cout<<i<<": "<<pos1<<' '<<who<<endl;
