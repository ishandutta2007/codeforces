...
void test_case(int test) {
    int n;
    cin >> n;
    vector<Mint> f2(n + 10, 1);
    for (int i = 2; i < f2.size(); ++i)
        f2[i] = f2[i - 2] * i;
    vector<Mint> p2(n + 10, 1);
    for (int i = 1; i < p2.size(); ++i)
        p2[i] = p2[i - 1] * 2;
    Mint ans = 0;
    vector<Mint> perm2(n + 1);
    perm2[0] = 1;
    for (int n = 1; n < perm2.size(); ++n) {
        if (n >= 1) perm2[n] += perm2[n - 1];
        if (n >= 2) perm2[n] += perm2[n - 2] * (n - 1);
    }
    for (int c4 = 0; c4 * 4 <= n; ++c4) {
        Mint prod = 1;
        if (c4) prod = C(n - c4 * 2, c4 * 2) * f2[c4 * 2 - 1] * p2[c4];

        ans += prod * perm2[n - c4 * 4];
    }
    cout << ans << '\n';
}
