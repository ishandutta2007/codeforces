...
for(int i=1;i<n+1;i++)
{
    for(int j=1;j<n+1;j++)cin>>seq[i][j];
}
for(int i=2;i<n;i++)
    for(int j=2;j<n;j++)
{
    if(seq[i][j]=='.'&&seq[i-1][j]=='.'&&seq[i+1][j]=='.'&&seq[i][j-1]=='.'&&seq[i][j+1]=='.')
    {
        seq[i][j]='#';seq[i-1][j]='#';seq[i+1][j]='#';seq[i][j-1]='#';seq[i][j+1]='#';
    }
}
for(int i=1;i<n+1;i++)
    for(int j=1;j<n+1;j++)
{
    if(seq[i][j]!='#')return cout<<"NO",0;
}
cout<<"YES";
return 0;
}