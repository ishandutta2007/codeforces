...
bool extgcd(T a,T b,T &x,T &y,T d){
    ...
    if(!b) {x = d/a, y = 0; return true;}
    bool res = extgcd(b,a%b,y,x,d);
    y -= a/b * x;
    return res;
}
...
signed main(){
    ...
    if(!dx){
        if(x == 0 or x == n){
            cout << x << " " << (dy == 1 ? m:0) << endl;
        }
        else cout << - 1 << endl;
    }
    else if(!dy){
        if(y == 0 or y == m) cout << (dx == 1 ? n:0) <<" " << y << endl;
        else cout << -1 << endl;
    }
    else{
        bool rx = false, ry = false;
        if(dx < 0) rx = true, x = n - x;
        if(dy < 0) ry = true, y = m - y;
        y += (n - x);
        __int128_t cnty = y / m;
        y %= m;
        __int128_t p,q;
        if(!extgcd<__int128_t>(-n,m,p,q,y)) cout << -1 << endl;
        else{
            ll g = gcd(n,m);
            n/=g, m/= g;
            if(p >= 0){
                q -= p/(__int128_t)m * (__int128_t)n;
                p = p%(__int128_t)m;
            }
            else{
                __int128_t k = (m-1-p) / m;
                p += k * (__int128_t)m;
                q += k * (__int128_t)n;
            }
            // assert(p<m and p >= 0 and q + cnty >= 1);
            // assert(y + p * n == q * m); 
            cout << (rx ^ ((p+1) & 1) ? n * g : 0 ) << " ";
            cout << (ry ^ ((cnty + q) & 1) ? m * g : 0 ) << endl;
        }
    }
}