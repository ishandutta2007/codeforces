...
struct popu
{
	...
	popu(const int n, const int p) : v(n), p(p), z(0)
	{
		for (int i = 0; i < n; ++i)
			if (i != p)
				add(i);
	...
	int change(const int t, const int val)
	{
		if (t == p)
			z += val;
		else
	...
	else
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (v[i] != p)
			{
				...
				if (qq[v[i]] >= qq[p])
					new_last = i + 1;
				else if (qq[p] - qq[v[i]] < sz(leftmost[v[i]]))
					if (leftmost[v[i]][qq[p] - qq[v[i]]] == n + 1)
						leftmost[v[i]][qq[p] - qq[v[i]]] = i + 1;
					else
						new_last = i + 1 - leftmost[v[i]][qq[p] - qq[v[i]]];
				last = max(last, new_last);
			}
			else
			{
				while (!uu.empty() && uu.back() )
			}
			ans = max(ans, last);
		}
		*/
