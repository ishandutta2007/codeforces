...
#ifdef _LOCAL
    p = iota(n);
    rep3(i, n - 1, 1) { swap(p[i], p[rnd(i + 1)]); }
#endif
    int mi = 32 - __builtin_clz(n - 1);
    int a;
    rep(i, 30) {
        int x = rnd(n);
        int y;
        do { y = rnd(n); } while(x == y);
        if(chmin(mi, popcount(ask(x, y)))) a = x;
    }
    vi res = iota(n);
    res.erase(remove(all(res), a), res.end());
    while(!res.empty()) {
        vi mem(res.size());
        int s = (1 << 12) - 1;
        rep(i, res.size()) {
        ...
        vi nxt;
        rep(i, res.size()) if(mem[i] == s) nxt.eb(res[i]);
        res = nxt;
        if(!nxt.empty()) {
            a = nxt.back();
            res.pop_back();
