...
pll dfs(const int& u, const vvi& g)
{
	int s = sz(g[u]);
	if (s == 0)
		return pll(1, 1);
	...
	vl allans(s);
	for (int i = 0; i < s; ++i)
		allans[i] = sum(answers[i].X, answers[i].Y, MOD);
	ll all_cnt = 1;
	ll all_blocked = 1;
	for (int i = 0; i < s; ++i)
	{
		all_blocked = prod(all_blocked, answers[i].X, MOD);
		all_cnt = prod(all_cnt, allans[i], MOD);
	}
	pll ans;
	ans.Y = raz(all_cnt, all_blocked, MOD);

	vl left_prod_all(s), right_prod_all(s);
	if (s)
	{
		left_prod_all[0] = 1;
		for (int i = 0; i < s - 1; ++i)
			left_prod_all[i + 1] = prod(left_prod_all[i], answers[i].X, MOD);
		right_prod_all[s - 1] = 1;
		for (int i = s - 2; i >= 0; --i)
			right_prod_all[i] = prod(right_prod_all[i + 1], answers[i + 1].X, MOD);
	}

	ans.X = all_cnt;
	for (int i = 0; i < s; ++i)
		ans.X = raz(ans.X, prod(prod(left_prod_all[i], right_prod_all[i], MOD), answers[i].Y, MOD), MOD);
