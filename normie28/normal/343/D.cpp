...
int main()
{
    ...
    while(n--){
        int a,b;cin>>a>>b;
        if(a==1){
           auto fir=vals.lower_bound(st[b]);
           auto sec=vals.upper_bound(en[b]);
           if(fir!=vals.end()&&*fir<=en[b]&&par[b]){
            vals.insert(st[par[b]]);
           }
           vals.erase(fir,sec);
        }else if(a==2){
            vals.insert(st[b]);
           ...
           auto el=vals.lower_bound(st[b]);
           cout<<!(el!=vals.end()&&*el<=en[b])<<endl;
        }
