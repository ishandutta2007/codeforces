...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	cauto n=read();
	cauto ab=readV<pii>(n);
	// cauto ab=vector<pii>(n,pii(100000,99995));
	vector<mint2> dp{1};
	for(cauto [a,b]:ab){
		cauto X=(int)dp.size();
		cauto Z=X+a-b;
		vector<mint2> binoms(X+Z);
		rep(i,X+Z) binoms[i]=comb(a+b,b-X+i).v;
		cauto tmp=convolution(dp,binoms);
		dp.resize(Z);
		rep(i,Z) dp[i]=tmp[i+X];
		// vector<mint2> nxt(dp.size()+a-b);
		// rep(x,dp.size())rep(z,nxt.size()) nxt[z]+=dp[x]*comb(a+b,b-x+z).v;
		// for(cauto &v:nxt) cout<<v.val()<<" ";cout NL;
		// for(cauto &v:tmp) cout<<v.val()<<" ";cout NL;
		// out(X,Z,1);
		// dp=nxt;
	}
	cout<<accumulate(all(dp),mint2(0)).val() NL;
  return 0;
}