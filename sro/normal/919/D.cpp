...
vector<int> nei[300007],fnei[300007];
char s[300007];
bool lgl[300007],dgl[300007];
int dp[300007][27];
int n,m;
...
inline int dpfs(int now,char zm)
{
	if(dp[now][zm-'a']!=-1)return dp[now][zm-'a'];
	int sum=0;
	for(int i:fnei[now])
		sum=max(sum,dpfs(i,zm));
	if(zm==s[now-1])sum++;
	return dp[now][zm-'a']=sum;
}
...
int main()
{
	...
	for(int i=1;i<=n;i++)if(!dgl[i])dfs(i);
	int ans=0;
	FILL1(dp);
	for(int i=1;i<=n;i++)
		for(int j=0;j<26;j++)
			ans=max(ans,dpfs(i,j+'a'));
	printf("%d\n",ans);
	return 0;
}