...
void judge(ll N,ll K){
  ...
  rep(i,1<<N){
    ...
    {
      map<string,ll> ch;
      rep(j,N-K+1)ch[s.substr(j,K)]++;
      for(auto x:ch)if(x.second==1)f=true;
      if(!f)continue;
    }
    {
      map<string,ll> ch;
      rep(j,N-K+2)ch[s.substr(j,K-1)]++;
      for(auto x:ch)if(x.second==1)f=false;
      if(!f)continue;
    }
    if(chmin(ans,countdigits(i,2)))t=s;
  }
  print(t);
}
...
int main(){
  ...
  if(N>A){
    if(K==1)ans[0]='1';
    else ans[K-2]=ans[K*2-3]='1';
  }else{
    K=(N-K)/2+1;
    ll i;
    rep(i,N)if((i+1)%K==0)ans[i]='1';
  }
  print(ans);
}