...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        V<mint> ndp1(n + 1 + n);
        if (i % 2) {
            if (s[i] != '?') s[i] = char('0' + (1 - (s[i] - '0')));
            if (t[i] != '?') t[i] = char('0' + (1 - (t[i] - '0')));
        }

        dbg(s[i], t[i]);
        for (int f = 0; f < 2; f++) {
            if (s[i] != '?' && s[i] != char('0' + f)) continue;
            for (int g = 0; g < 2; g++) {
                if (t[i] != '?' && t[i] != char('0' + g)) continue;
                int diff = 0;
                if (f && !g) diff = 1;
                if (!f && g) diff = -1;
                for (int j = -i; j <= i; j++) {
                    ndp0[j + diff + offset] += dp0[j + offset];
                    ndp1[j + diff + offset] += dp1[j + offset] + dp0[j + offset] * mint(abs(j + diff));
                }
            }
        }
        dp0 = ndp0;
