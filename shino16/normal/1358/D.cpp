...
int main() {
	...
	VLL d2 = d;
	d.insert(d.end(), all(d2));
	n *= 2;

	int I;
	ll s = 0;
	rep(i, n) if ((s += d[i]) >= x) { I = i; break; }
	dump(I);

	VLL total(n);
	rep(i, n) total[i] = d[i] * (d[i] + 1) / 2;

	VLL acc(total.size()+1); rep(i, total.size()) acc[i+1] = acc[i] + total[i];
	...
	auto sum = [&]() {
		return (d[from] + day + 1) * (d[from] - day) / 2;
	};

	ll res = sum() + acc[til+1] - acc[from+1];
		dump(til, from, day, res);
	...
	while (til+1 < n) {
		...
		chmax(res, sum() + acc[til+1] - acc[from+1]);
		dump(til, from, day, sum() + acc[til+1] - acc[from+1]);
	}
