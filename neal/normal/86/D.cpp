...
struct mo_state {
    ...
    void add_left(const mo_value &x) {
        sum += (long long) (2 * freq[x] + 1) * values[x];
        freq[x]++;
        mo_ops++;
    }
...
int main() {
    ...
    IO::read_int(N);
    IO::read_int(Q);
    vector<mo_value> compressed(N);
    gp_hash_table<int, int> lookup;
    ...
    for (int i = 0; i < N; i++) {
        ...
        compressed[i] = lookup[a];
    }

    mo solver(compressed);
