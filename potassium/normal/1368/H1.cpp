...
int main() {
	int n,m,q; R(n,m,q);
	vector<string> a(2), b(2); R(a, b);
	
	auto solve = [&]() {
		vector<int> dp0(n, INT_MAX), dp1(n, INT_MAX);
		dp0[0] = dp1[0] = 0;
		for (auto& c : b[0])
			if (c == 'B')
				dp0[0]++;
		...
		dp0[0] += (a[0][0] == 'B') + (a[1][0] == 'B');
		dp1[0] += (a[0][0] == 'R') + (a[1][0] == 'R');
		
		for (int i = 1; i < n; i++) {
			int Bcnt = (a[0][i] == 'B') + (a[1][i] == 'B');
			dp0[i] = min(dp0[i - 1], dp1[i - 1] + m) + Bcnt;
			dp1[i] = min(dp1[i - 1], dp0[i - 1] + m) + 2 - Bcnt;
		}
