...
int main() {
	...
	int l = 0, r = 500007;
	int mid, ymx, ymn, xmx, xmn, yxmx, yxmn;
	
	function<void()> run = [&]() {
		ymx = xmx = yxmx = INT_MAX;
		ymn = xmn = yxmn = INT_MIN;
		...
		for (auto& i : p) {
			...
			chkmin(xmx, i.F + mid);
			chkmax(xmn, i.F - mid);
			chkmin(yxmx, i.S - i.F + mid);
			chkmax(yxmn, i.S - i.F - mid);
		}
		...
		if (xmx < xmn) is = 0;
		if (yxmx < yxmn) is = 0;
		if (ymn - xmx > yxmx) is = 0;
		if (ymx - xmn < yxmn) is = 0;
...
//	W(ymx, ymn, xmx, xmn, yxmx, yxmn);
	int x,y;
	function<void()> check = [&]() {
		if (x < 0 || y < 0 || x + y <= 0) return;
//		W("checking", x, y, xmn <= x, x <= xmx, ymn <= y, y <= ymx, yxmn <= y - x, y - x <= yxmx);
		if (xmn <= x && x <= xmx && ymn <= y && y <= ymx && yxmn <= y - x && y - x <= yxmx) {
			W(string(x, 'B') + string(y, 'N'));
			exit(0);
		}
	};
	...
	x = xmn, y = ymx; check();
	x = xmx, y = ymx; check();
	for (int yx = yxmn; yx <= yxmx; yx = (yx == yxmx ? yxmx + 1 : yxmx)) {
		for (x = xmn; x <= xmx; x = (x == xmx ? xmx + 1 : xmx)) {
			y = x + yx; check();
		}
		for (y = ymn; y <= ymx; y = (y == ymx ? ymx + 1 : ymx)) {
			x = y - yx; check();
		}
	}
