...
bool used[1<<17];
u_map<int,vector<int>> grp2[5];
u_map<int,vector<int>> grp1;
...
signed main() {
    ...
    rep(i,n) {
        ...
        grp1[cnt].emplace_back(i);
        grp2[t][cnt].emplace_back(i);
    }
    ...
    cout<<m<<"\n";
    vector<pair<pii,pii>> lylics(m);

    int idx=0;
    for(int i=0; i<5; ++i) {
        for(auto &x:grp2[i]) {
            for(int i=1; i<x.sec.size(); i+=2) {
                if(idx<m) {
                    lylics[idx++].sec=mkp(x.sec[i-1],x.sec[i]);
                    used[x.sec[i-1]]=used[x.sec[i]]=1;
                } else break;
            }
        }
    }
    ...
    for(auto &x:grp1) {
        ...
        vector<int> nv;
        for(int i:v) {
            if(!used[i]) nv.emplace_back(i);
        }
        for(int i=1; i<nv.size(); i+=2) {
            if(idx<m) {
                lylics[idx++].fir=mkp(nv[i],nv[i-1]);
            } else break;
        }
    }

    for(auto &p:lylics) {
        cout<<s[p.fir.fir]<<" "<<s[p.sec.fir]<<'\n';
        cout<<s[p.fir.sec]<<" "<<s[p.sec.sec]<<'\n';
    }
}