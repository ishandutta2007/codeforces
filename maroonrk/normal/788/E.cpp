...
struct N{
	...
	static N merge(const N&x,const N&y){
		N res;
		rep(i,6)res.buf[i]=x.buf[i]+y.buf[i];
		rep(i,3)rng(j,i+1,3)rng(k,j+1,4)
			res.buf[id[i][k]]+=x.buf[id[i][j]]*y.buf[id[j][k]];
		return res;
	}
};
...
signed main(){
	...
	vvc<T> rw;
	vvc<int> idx(h);
	rep(i,n)idx[lwb(vs,a[i])].pb(i);
	BIT<int> bit(n);
	rep(lv,h){
		int s=si(idx[lv]);
		vc<T> tmp(s);
		rep(i,s){
			tmp[i]=T(bit.sum(0,idx[lv][i])+i,1,bit.sum(idx[lv][i],n)+s-1-i);
		}
		rep(i,s){
			bit.add(idx[lv][i],1);
		}
	...
	mint ans=0;
	rep(lv,h)ans+=ss[lv].getall().buf[2];
	...
	rep(_,m){
		...
		int lv=lwb(vs,a[x]);
		int i=lwb(idx[lv],x);
		ans-=ss[lv].getall().buf[2];
		if(t==1){
			ss[lv].set(i,N());
		}else{
			ss[lv].set(i,N(rw[lv][i]));
		}
		ans+=ss[lv].getall().buf[2];
		print(ans);
	}
