...
void mian() {
  ...
  REP(i, 1, n) REP(j, 1, n) dis[i][j] = abs(a[i] - a[j]) + abs(b[i] - b[j]);
  REP(i, 1, n) {
    int mn = inf;
    REP(j, 1, n) if(i != j) chkmn(mn, dis[i][j]);
    REP(j, 1, n) if(dis[i][j] == mn) nei[i].pb(j);
  }
  REP(x, 1, n) {
    bool flg = true;
    for(int y : nei[x]) {
      vi shd = nei[x];
      shd.erase(find(ALL(shd), y));
      shd.insert(lower_bound(ALL(shd), x), x);
      flg &= nei[y] == shd;
    }
    if(flg) {
      for(int y : nei[x]) ufs.mrg(x, y);
    }
  }
  vi v;
  REP(i, 1, n) ++cnt[ufs.root(i)];
  REP(i, 1, n) if(!ufs.fa[i]) v.pb(cnt[i]);
  ...
  for(int x : v) {
    ...
    REP(i, 0, n) if(f[i]) {
      addto(g[i + 1], f[i]);
      if(x > 1) addto(g[i + x], f[i]);
    }
    REP(i, 0, n) f[i] = g[i];
  }
  int ans = 0;
  REP(i, 1, n) addto(ans, (ll)f[i] * fc[n] % P * ifc[n - i] % P);
  prt(ans), pc('\n');
}
