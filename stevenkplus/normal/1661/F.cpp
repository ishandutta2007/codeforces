...
int main() {
   ...
   ll lo = 0;
   ll hi = 1E18;
   auto get3 = [&](ll dist, ll parts) -> ll {
      ll a = dist / parts, b = a + 1;
      return a * a * (parts - dist % parts) + b * b * (dist % parts);
   };
   auto get2 = [&](ll dist, ll y) -> pll {
      ll lo = 1;
      ll hi = dist;
      while (lo < hi) {
         ll mid = (lo + hi) / 2;
         ll delta = get3(dist, mid) - get3(dist, mid + 1);
         if (delta <= y) {
            hi = mid;
         } else {
            lo = mid + 1;
      ...
      return pll(get3(dist, lo), lo);
   };
   auto get = [&](ll y) -> ll {
      ll res = 0;
      // split everything so that the marginal return of an additional split is at most y
      // then what is cost?
      for (int i: v) {
         pll p = get2(i, y);
         res += p.fi;
