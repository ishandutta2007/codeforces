...
int coso(int i, int n){
    int res = 0;
    F0(j,n) if(i & (1<<j)) res ^= j;
    return res;
}

int color[(1<<16)],res[(1<<16)];
int valor[(1<<16)];
vi nuevo_frente;
...
void E(){
    ...
    F0(i,(1<<n)) res[valor[i]] = i;
    F0(i,(1<<n)) color[res[i]] = coso(i,n);
    F0(i,(1<<n)) cout<<res[i]<<' ';
    cout<<'\n';
    if((1<<n) % n) cout<<"-1\n";
    else {
        F0(i,(1<<n)) cout<<color[i]<<' ';
        cout<<'\n';
    }
