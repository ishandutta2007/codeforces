...
int bestPath = 0;
int p3[303];
void go(vector<int> v, int res = 0, int path = 0, int step = 0) {
    sort(all(v));
    reunique(v);
...
//    DBN(v, res, path);
    if (v.size() == 2) {
        FI(2) {
            if (v[i] == 0) { v.erase(v.begin() + i); break; }
        }
    }
    if (v.size() == 1 && abs(v[0]) == 1) {
        if (v[0] == 1) go({}, res + 1, path + 1 * p3[step], step + 1);
        else go({}, res + 1, path + 2 * p3[step], step + 1);
        return;
    }
    bool ok = false;
    for (int x : v) {
    ...
    if (!ok) {
        ...
        go(t, res + 1, path + 1 * p3[step], step + 1);
        t = v;
        for (int& x : t) {
            if (x % 2) x += 1;
            x /= 2;
        }
        go(t, res + 1, path + 2 * p3[step], step + 1);
    }
}
int solve() {
    p3[0] = 1;
