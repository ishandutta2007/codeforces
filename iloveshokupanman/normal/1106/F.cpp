...
vvll pw(vvll a, ll K,ll MOD) {
    ...
    vvll ret = pw(a, K / 2,MOD);
    ret = mul(ret, ret,MOD);
    if (K % 2 == 1)ret = mul(ret, a,MOD);
    return ret;
}

ll baby(ll x, ll y, ll MOD) {//x^ret=y mod MOD
    ll t = sqrt(MOD)+1;
    ...
    ll R = inv_mod(now, MOD);

 
    rep(i, t) {
        if (mp.count(y)) { return t*i+mp[y]; }
        y = y * R % MOD;
    }
...
int main() {
    ...
    //cout << (((mint)3).pow(U)).val() << endl;
   
    ll G = gcd(P - 1, T);
    if (U % G != 0) { cout << -1 << endl; return 0; }
