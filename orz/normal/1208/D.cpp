...
struct st
{
	...
	void add(int v, int x, int y, int pos, ll val)
	{
		if (x + 1 == y)
		{
			a[v] += val;
		...
		int m = (x + y) / 2;
		if (pos < m)
			add(2 * v + 1, x, m, pos, val);
		else
			add(2 * v + 2, m, y, pos, val);
		a[v] = a[2 * v + 1] + a[2 * v + 2];
	}
	int minim(ll val)
	{
		return minim(0, 0, n, val);
	}
	int minim(int v, int x, int y, ll val)
	{
		//if (val <= 0)
		//	return x - 1;
		if (x + 1 == y)
		...
		int m = (x + y) / 2;
		if (val <= a[2 * v + 1])
			return minim(2 * v + 1, x, m, val);
		else
			return minim(2 * v + 2, m, y, val - a[2 * v + 1]);
	}
};
