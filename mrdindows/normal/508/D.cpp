...
int main()
{
    ...
    for (int i = 0; i < vn; ++i){
        ...
        static bool k = false;
        if (!k && (v[i].size() || g[i].size())){
            go(i);
            k = true;
            for (int j = 0; j < vn; ++j){
                if ((v[j].size() || g[j].size()) && !used[j]){
                    cout << "NO" << endl;
                    return 0;
                }
            }
        ...
        if (g[i].size() == v[i].size()){
            if (g[i].size() > 0)
                p = i;
            continue;
        }
        else if (g[i].size() == v[i].size() + 1){
            gv++;
            ig = i;
        }
        else if (v[i].size() == g[i].size() + 1){
            vg++;
            iv = i;
        }
        else {
    ...
    if (vg != 0){
        ...
        for (int i = 0; i < ans.size(); ++i){
            int x = ans[i];
            if (i == ans.size()-1 || x == iv && ans[i+1] == ig){
                for (int j = i + 1; j != i; ++j){
                    if (j == ans.size()) j = 0;
                    if (j == i) break;
                    st.push_back(ans[j]);
