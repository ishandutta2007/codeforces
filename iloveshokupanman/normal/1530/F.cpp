...
int main() {
    ...
    modint::set_mod(31607);
    ll N;
    cin >> N;
    vvmint a(N, vmint(N, 0));
    rep(i, N)rep(j, N) {
        ll x;
        cin >> x;
        a[i][j] = (mint)(x) / 10000;
    ...
    vvmint prod(N,vmint(1ull << N,1));
    rep(j, N)rep(x, 1ull << N) {
        rep(i, N) {
            if ((x >> i) & 1) {
                ll y = x ^ (1ull << i);
                prod[j][x] = prod[j][y] * a[i][j];
                break;
            }
        }
        //cout << j << " " << x << " " << prod[j][x].val() << endl;
    }
    ...
    rep(x, 1ull << L) {
        ...
        vll c(N, x & ((1ull << N) - 1));
        if ((x >> N) & 1) {
            rep(j, N)c[j] = c[j] | (1ull << j);
        }
        if ((x >> (N + 1)) & 1) {
            rep(j, N)c[j] = c[j] | (1ull << (N - 1 - j));
        }
       ...
       // cout << x << " " << dp[x].val() << endl;
       // rep(j, N)cout << c[j] << endl;
    }
