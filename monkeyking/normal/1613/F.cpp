...
//		for(int i=0;i<=s;i++) F[m][i]=1LL*combinatorics::C(s,i)*pw[s-i]%mod;
//		m++;
//	}
//	for(int i=0;i<m-1;i++)
//	{
...
//		if((n-l-i)&1) chadd(res,mod-v);else chadd(res,v);
//	}
//	cout<<res<<endl;
//	return 0;
//}
...
struct NTT
{
	...
	void inverse(vector<int> &v)
	{
		...
		inverse(f0);
		vector<int> rv=f0;multiply(rv,2);
		sqr(f0);multiply(v,f0,f0);
		v=sub(rv,f0);
		v.resize(n);
	}
