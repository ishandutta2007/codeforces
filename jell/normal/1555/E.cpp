#line 1 "codeforces-workspace\\contest\\1555\\e\\e.cpp"
...
#line 13 "codeforces-workspace\\contest\\1555\\e\\e.cpp"
// #include "lib/cxx20"
...
#line 16 "codeforces-workspace\\contest\\1555\\e\\e.cpp"
//*/
...
#line 33 "codeforces-workspace\\contest\\1555\\e\\e.cpp"
...
struct endo {
  int v{-inf};
  endo operator*(endo x) const { return {max(v, x.v)}; }
};

struct mono {
  int v{inf};
  mono operator+(mono x) const { return {min(v, x.v)}; }
  mono operator*(endo x) const { return {max(v, x.v)}; }
};
...
void main() {
  ...
  input n, m;
  input<vector<array<int, 3>>> s(n);
  sort(begin(s), end(s), [](auto x, auto y) { return x.back() < y.back(); });
  lazy_segment_tree<mono, endo> sgt(m - 1);
  for (auto &&x : sgt) {
    x = {-inf};
  }
  auto ans = inf;
