...
int main() {
    ...
    vector<pll> S;
    for (auto i : L) {
        if (sz(S) > 0 && S.back().fs == i.fs) S.pop_back();
        while (sz(S) > 1 && (S[sz(S) - 2].se - S[sz(S) - 1].se) * (i.fs - S[sz(S) - 1].fs) > (S[sz(S) - 1].se - i.se) * (S[sz(S) - 1].fs - S[sz(S) - 2].fs))
            S.pop_back();
        S.push_back(i);
    }
    ll ans = 0;
    for (int i = 0; i < sz(S) - 1; ) {
        int j = i + 1;
        while (j < sz(S) - 1 && (S[i].se - S[j].se) * (S[j + 1].fs - S[j].fs) == (S[j].se - S[j + 1].se) * (S[j].fs - S[i].fs)) j++;
        ans++;
        i = j;
    }
    cout << ans;
