...
int main() {
    ...
    for (int i = 0; i < m; i++) cin >> l[i] >> r[i] >> w[i], --l[i];
    ...
    for (int i = 0; i < N; i++) mp[H[i]] = i;
    for (int i = 0; i < n; i++) {
        h[i] = mp[h[i]];
        pos[h[i]].emplace_back(i);
    }

    vector<int> lb(m), ub(m);
    for (int i = 0; i < m; i++) lb[i] = 0, ub[i] = N;
    auto f = [](int a, int b) { return max(a, b); };
    auto g = [](int a, int b) { return a + b; };
    LazySegmentTree<int, int> nseg(
        n, [](int a, int b) { return min(a, b); }, g, g, INF, 0);
    nseg.build(h);
    ...
    auto solve = [&]() {
        ...
        LazySegmentTree<int, int> seg(n, f, g, g, 0, 0);
        for (int i = N - 1; i >= 0; i--) {
            for (int& x : pos[i]) {
                seg.set_val(x, (x ? seg[x - 1] : 0) + 1);
                int nxt = nseg.find_first(x + 1, [&](int val) { return val <= i; });
                if (nxt < 0) nxt = n;
                seg.update(x + 1, nxt, seg[x]);
            }
            for (int& x : mid[i]) {
                int res = seg.query(l[x] + w[x] - 1, r[x]);
                (res >= w[x] ? lb[x] : ub[x]) = i;
            }
        }
    };
    for (int i = 0; i < MAX_LOG; i++) solve();

    for (int i = 0; i < m; i++) cout << H[lb[i]] << '\n';
    return 0;
}