...
int main() {
    ...
    F0R(it, K) {
        ...
        poly.pb(NTT::mod_multiply_all(polys));
        if (it > 0) {
            vl res = NTT::mod_multiply(poly[it], poly[it-1]);
            poly[it] = res;
        }
/*        trav(a, poly[it]) {
            cout << a << " ";
    ...
    while(Q--) {
        ...
        F0R(i, K) {
            int cur = (Q - B[i]*2 - 2) / 2;
            if (cur < 0 || cur >= sz(poly[i])) continue;
            ans += poly[i][cur]; ans %= MOD;
        }
        cout << ans << nl;
    }
