...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		cauto n=read();
		cauto m=read();
		vector<string> ab=readV<string>(n);
		if(m%2){
			cout<<"YES" NL;
			rep(i,m+1) cout<<i%2+1<<" ";
			cout NL;
			continue;
		}
		if(n==2){
			if(ab[0][1]==ab[1][0] || m%2){
				cout<<"YES" NL;
				rep(i,m+1) cout<<i%2+1<<" ";
				cout NL;
			}else{
			...
			cout<<"YES" NL;
			vector<int> p;
			rep(i,3)rep(j,i)if(ab[i][j]==ab[j][i]){
				p={i,j};
				break;
			}
			if(p.empty() && ab[0][1]==ab[1][2] && ab[1][2]==ab[2][0]) p={0,1,2};
			if(p.size()){
				rep(i,m+1) cout<<p[i%p.size()]+1<<" ";
				cout NL;
				continue;
			}
			if(m%2){
			...
			if(ab[0][1]==ab[1][2]) p={0,1,2};
			if(ab[1][2]==ab[2][0]) p={1,2,0};
			if(ab[2][0]==ab[0][1]) p={2,0,1};
			deque<int> re;
			rep(i,3) re.eb(p[i]);
			while(re.size()<m+1){
				if(re[0]==p[1]){
