...
mint perm(ll n,ll r){ if(n<r||r<0)return 0; return fact[n]*finv[n-r];}
class Doralion{
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	int len=0;
	rep(x,n)if(cnt[x]){
		rep(i,n) re+=dp[i]*perm(cnt[x],2)*fact[n-i-2];
		// rep(i,n) if((dp[i]*perm(cnt[x],2)*fact[n-i-2]).v) out(x,i,(dp[i]*perm(cnt[x],2)*fact[n-i-2]),1);
		rrep(i,n) dp[i+1]+=dp[i]*cnt[x];
		len+=cnt[x];
	}
	cout<<re*finv[n]<<endl;
  return 0;
