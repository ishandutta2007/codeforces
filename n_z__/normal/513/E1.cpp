...
for(int x=1;x<=n;x++)
cin>>a[x];
array<int,4>dp[n+2][k+2];
memset(dp,0x80,sizeof(dp));
for(int x=0;x<=n;x++)
dp[x][0]={0,0,0,0};
for(int x=1;x<=n;x++)
for(int y=1;y<=k;y++)
dp[x][y][0]=max(dp[x-1][y][0],dp[x-1][y-1][3])-(1+(y!=1&&y!=k))*a[x],dp[x][y][1]=max({dp[x-1][y][1],dp[x][y][0],y!=1&&y!=k?dp[x-1][y-1][1]:dp[n+1][k+1][0]}),dp[x][y][2]=max(dp[x-1][y][2],dp[x-1][y-1][1])+(1+(y!=1&&y!=k))*a[x],dp[x][y][3]=max({dp[x-1][y][3],dp[x][y][2],y!=1&&y!=k?dp[x-1][y-1][3]:dp[n+1][k+1][0]});
cout<<max(dp[n][k][1],dp[n][k][3])<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
/*Main-end----------------------------------------------------------------------------------------*/
