...
ll solve1(ll cnt00, ll cnt11, int last)
{
    ll ops = min(cnt00, cnt11);
    cnt00-=ops; cnt11-=ops;
    ll ans = ops*(a+b);
    if (last == 0 && cnt00>0) ans+=a;
    if (last == 1 && cnt11>0) ans+=b;
    return ans;
}

ll solve2(vector<int> blocks, int cnt00, int cnt11)
{
    ll ans = 0;
    ...
    ll curans = 0;
    ans = max(ans, solve1(cnt00, cnt11, 0));
    for (int i = 0; i<K; i++)
    {
        int need = max(0, blocks[i]-1);
        if (cnt11<need) return ans;
        cnt11-=need;
        cnt00-=need;
        blocks[i]-=need;
        curans+=(a+b)*need;

        ans = max(ans, curans + solve1(cnt00, cnt11, 0));

        if (blocks[i])
        {
            curans-=c;
            if (i<K-2) cnt11++;
            if (cnt11==0) return ans;
            cnt11--;
            curans+=b;
            ans = max(ans, curans + solve1(cnt00, cnt11, 0));
        }
...
void solve()
{
    ...
    for (int i = 0; i<n; i++)
    {
        ...
        {
            if (i==0 || cnt!=0) blocks.push_back(cnt);
            cnt = 0;
        }
    }
    ...
    int cnt11 = 0;
    for (int i = 0; i<n-1; i++) if (s[i]=='1' && s[i+1]=='1') cnt11++;
    int cnt00 = 0;
    for (int i = 0; i<n-1; i++) if (s[i]=='0' && s[i+1]=='0') cnt00++;


    vector<int> vals;
    for (int i = 1; i<=K-2; i++) vals.push_back(blocks[i]);
    sort(vals.begin(), vals.end());
    vals.push_back(blocks[0]);
    vals.push_back(blocks[K-1]);

    ll ans = solve2(vals, cnt00, cnt11);
    if (cnt11) ans = max(ans, b + solve2(vals, cnt00, cnt11-1));
    cout<<ans<<endl;
}
