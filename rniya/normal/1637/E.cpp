...
void solve() {
    ...
    for (auto& p : ng) sort(ALL(p.second));
    map<int, int> cnt;
    for (int& x : a) cnt[x]++;
    vector<int> cand;
    ...
    for (auto p : cnt) {
        ...
        num[p.second].emplace_back(p.first);
    }
    mkuni(cand);
    for (auto& v : num) sort(v.rbegin(), v.rend());
        ...
        for (int& y : cand) {
            ...
            for (int& x : num[y]) {
                if (a == x) continue;
                {
                    ...
                    if (itr != v.end() and *itr == x) continue;
                }
                chmax(ans, 1LL * (b + y) * (a + x));
                break;
            }
        }
    }
