...
int first[100002][30];
int last[100002][30];
int64_t prefix_sum[100001];
int prefix_xor[100001];
tuple<int64_t, int, int> get(int l, int r) {
    if (l > r) return {-1e18, 0, 0};
    return {prefix_sum[r] - prefix_sum[l - 1] - (prefix_xor[r] ^ prefix_xor[l - 1]), l - r, l};
}
void solve() {
    cin >> n >> q;
    a | ARRAY | 1 | n | ARRAY_INPUT;
    fill(first[n + 1], first[n + 1] + 30, n + 1);
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < 30; j++)
            if ((a[i] >> j) & 1) last[i][j] = i;
            else last[i][j] = last[i - 1][j];
    }
    for (int i = n; i >= 1; i--) {
        for (int j = 0; j < 30; j++)
            if ((a[i] >> j) & 1) first[i][j] = i;
            else first[i][j] = first[i + 1][j];
    }
    ...
    for (int i = 1, l, r; i <= q; i++) {
        ...
        for (int j = 0; j < 30; j++) {
            for (int k = 0; k < 30; k++) {
                auto res = get(first[l][j], last[r][k]);
                if (ans < res) ans = res;

                if (last[r][k]) {
                    auto res = get(first[l][j], last[last[r][k] - 1][k]);
                    if (ans < res) ans = res;
                }
                if (first[l][j] < n + 1) {
                    auto res = get(first[first[l][j] + 1][j], last[r][k]);
                    if (ans < res) ans = res;
                }
                if (last[r][k] && (first[l][j] < n + 1)) {
                    auto res = get(first[first[l][j] + 1][j], last[last[r][k] - 1][k]);
                    if (ans < res) ans = res;
                }
            }
        }
        auto&& [value, ans_l_r, ans_l] = ans;
        cout << ans_l << ' ' << (ans_l - ans_l_r) << '\n';
    }
}
