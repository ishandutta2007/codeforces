...
public class TaskD {
	...
	private void solve() {
		...
		Map<Integer, Integer> exact = new HashMap<Integer, Integer>();
		for (int i = 0; i < queryCount; i++) {
			String command = in.readString();
			if ("decay".equals(command))
				out.printf("%.8f\n", get(1, count, exact, 0));
			else {
				int vertex = in.readInt();
				int toAdd = in.readInt();
				put(vertex, toAdd, count);
				exact.put(vertex, get(exact, vertex) + toAdd);
			}
		}
	}

	private double get(int vertex, Map<Integer, Integer> count, Map<Integer, Integer> exact, int threshold) {
		int current = get(exact, vertex);
		int left = get(count, 2 * vertex);
		int right = get(count, 2 * vertex + 1);
		if (current + Math.max(left, right) <= threshold)
			return threshold;
		if (left == right)
			return current + left;
		if (left > right)
			return 0.5 * (current + left + get(2 * vertex, count, exact, Math.max(current + right, threshold)));
		return 0.5 * (current + right + get(2 * vertex + 1, count, exact, Math.max(current + left, threshold)));
	}

	private int get(Map<Integer, Integer> map, int vertex) {
		if (map.containsKey(vertex))
			return map.get(vertex);
		return 0;
	}

	private void put(int vertex, int toAdd, Map<Integer, Integer> count) {
		if (vertex == 0)
			return;
		count.put(vertex, get(count, vertex) + toAdd);
		put(vertex / 2, toAdd, count);
	}

	private static List<Test> createTests() {
		List<Test> tests = new ArrayList<Test>();
		tests.add(new Test("1 4\n" +
				"add 1 3\n" +
				"add 2 10\n" +
				"add 3 11\n" +
				"decay", "13,50000000"));
		tests.add(new Test("2 8\n" +
				"add 1 1\n" +
				"add 2 2\n" +
				"add 3 6\n" +
				"add 4 4\n" +
