...
void run_case() {
    ...
    for (int i = 0; i < N; i++)
        if (lca.depth[i] > lca.depth[deepest])
            deepest = i;

    int deep = lca.depth[deepest];
    vector<int> suffix_min(N + 1, deep);

    for (int i = 0; i < N; i++) {
        int i_depth = lca.depth[i];
        int anc = lca.get_lca(deepest, i);
        int down = (deep - i_depth) / 2;
        suffix_min[i_depth] = min(suffix_min[i_depth], lca.depth[anc] + down);
    }

    for (int i = N - 1; i >= 0; i--)
        suffix_min[i] = min(suffix_min[i], suffix_min[i + 1]);

    auto possible = [&](int x, int limit) -> bool {
        int place_depth = x + deep - limit;
        return place_depth <= suffix_min[limit + 1];
    };
