...
int main() {
    ...
    while (t--) {
        ...
        std::random_shuffle(p.begin(), p.end(), [&](int n) {return rnd() % n;});
        const int m = std::min(n, 30);
        int x = p[0];
        for (int i = 1; i < m; ++i)
            if (query({x}, {p[i]}) == "SECOND")
                x = p[i];
        ...
        while (true) {
            ...
            if (query(stones, r) == "FIRST")
                break;
            stones.insert(stones.end(), r.begin(), r.end());
        }
        int c = stones.size();
        for (int i = c / 2; i > 0; i /= 2) {
            std::vector<int> s(stones.begin(), stones.begin() + i), r(p.begin() + c, p.begin() + c + i);
            if (c + i < n && query(s, r) == "EQUAL")
                c += i;
        }
