...
namespace mem{ //v2.1.1 => size: 14.16KiB
  #define MEM_IO
  #define MEM_MATH
  #define MEM_UTILS
  #define MEM_MODINT
...
struct SegmentTree{
  ...
  inline void maintain(int u){
    ...
    p[u].sum=p[u<<1].sum+p[u<<1|1].sum+(long long)p[u].cnt*p[u].tag;
  }
  ...
  void modify(int u,int l,int r,int x){
    // if(u==1)log<RED>("modify %d %d => %d\n",l,r,x);
    if(p[u].l==l&&p[u].r==r){
      p[u].tag+=x;
      p[u].sum+=(long long)x*p[u].cnt;
      return;
    }
  ...
  inline long long query(int k,int x){return p[1].sum;}
}V;

struct JirySegmentTree{
  struct segment{
    int l,r,mid,max,smx,cnt;
  }p[N<<2];
  std::function<int(int)> init;
  std::function<void(int,int)> upd;
  inline void pushdown(int u){
    p[u<<1].max=std::min(p[u].max,p[u<<1].max);
    p[u<<1|1].max=std::min(p[u].max,p[u<<1|1].max);
  }
  inline void maintain(int u){
    if(p[u<<1].max>p[u<<1|1].max){
      p[u].max=p[u<<1].max;
      p[u].cnt=p[u<<1].cnt;
      p[u].smx=std::max(p[u<<1].smx,p[u<<1|1].max);
    }else if(p[u<<1|1].max>p[u<<1].max){
      p[u].max=p[u<<1|1].max;
      p[u].cnt=p[u<<1|1].cnt;
      p[u].smx=std::max(p[u<<1|1].smx,p[u<<1].max);
    }else{
      p[u].max=p[u<<1].max;
      p[u].cnt=p[u<<1].cnt+p[u<<1|1].cnt;
      p[u].smx=std::max(p[u<<1].smx,p[u<<1|1].smx);
    }
  }
  inline void set(int u,int x){
    upd(p[u].max,-1);
    p[u].max=x;
    upd(p[u].max,1);
  }
  void build(int u,int l,int r){
    p[u].l=l,p[u].r=r,p[u].mid=(l+r)>>1;
    if(l==r){
      p[u].cnt=1,p[u].max=init(l),p[u].smx=-1;
      upd(p[u].max,1);
      return;
    }
    build(u<<1,l,p[u].mid);
  ...
  void modify(int u,int k,int x){
    // if(u==1)log<BLUE>("modify %d => %d\n",k,x);
    if(p[u].l==p[u].r)return set(u,x);
    pushdown(u);
    modify(k<=p[u].mid?u<<1:u<<1|1,k,x);
    maintain(u);
  }
  void update(int u,int l,int r,int x){
    // if(u==1)log<BLUE>("update %d %d => %d\n",l,r,x);
    if(p[u].max<=x)return;
    if(p[u].l==p[u].r){
      set(u,x);
      return;
    }
    if(p[u].l==l&&p[u].r==r&&p[u].smx<x){
      upd(p[u].max,-p[u].cnt);
      p[u].max=x;
      upd(p[u].max,p[u].cnt);
      return;
  ...
  int query(int u,int k){
    ...
    pushdown(u);
    int res=query(k<=p[u].mid?u<<1:u<<1|1,k);
    maintain(u);
    return res;
  }
}L,R;
...
int main(){
  ...
  read(n);
  V.build(1,1,n);
  L.upd=[&](int k,int x){if(k)V.modify(1,1,-k,-x);};
  L.init=[&](int k){return -n;};
  L.build(1,1,n);
  R.upd=[&](int k,int x){if(k-1)V.modify(1,1,k-1,x);};
  R.init=[&](int k){return 1;};
  R.build(1,1,n);
  for(int i=1;i<=n;i++){
    read(a[i]);
    p[a[i]]=i;
  }
  for(int i=1;i<=n;i++){ //s[r-1]-s[l]-1
    V.update(1,p[i]);
    if(1<p[i])R.update(1,1,p[i]-1,p[i]);
    if(p[i]<n)L.update(1,p[i]+1,n,-p[i]);
    L.modify(1,p[i],0);
    R.modify(1,p[i],n+1);
    // for(int i=1;i<=n;i++)log<GREEN>("%d%c",-L.query(1,i)," \n"[i==n]);
    // for(int i=1;i<=n;i++)log<GREEN>("%d%c",R.query(1,i)," \n"[i==n]);
    print(V.p[1].sum+(long long)V.p[1].cnt*(n-i),'\n');
  }
}