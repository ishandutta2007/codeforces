...
void solve(){
    int n, m;
    cin >> n >> m;
    vector<vector<int64_t>> c(n, vector<int64_t>(m));
    cin >> c;
    ...
    for(int i = 0; i < n; ++i){
    	sums[i] = 0;
    	for(int j = 0; j < m; ++j){
    		p[i][j] = c[i][j] + (j > 0 ? p[i][j - 1] : 0);
    		sums[i] += p[i][j];
    	}
    }
    int64_t target = *max_element(sums.begin(), sums.end());
    	

    int idx = min_element(sums.begin(), sums.end()) - sums.begin();

    cout << idx + 1 << ' ' << target - sums[idx] << '\n';
