...
int main(void)
{
	...
	rep(i, 0, 502) rep(j, 0, 502) beki[i][j] = modpow(i, j);
	...
	rep(i, 0, x){
		rep(j, 2, n){
			rep(k, 1, j){
				if(i+j <= x) dp[i+j-1][k] += dp[i][j] * comb(j, k) * beki[j-1][j-k]; 
			}
			if(i+1 <= x) dp[i+1][0] += dp[i][j] * beki[min(j-1, x-i)][j]; 
		}
	}
	
	//rep(i, 0, x) outa(dp[i], 0, n);
	
	mint ans = 0;
	rep(i, 1, x) ans += dp[i][0];
	outl(ans);
