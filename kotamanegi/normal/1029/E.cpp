...
tuple<int, int, int> dfs(int now, int back) {
    ...
    for (auto x : vertexs[now]) {
        if (x == back) continue;
        next.push_back(dfs(x, now));
    }
    tuple<int, int, int> ans = make_tuple(1, 0, 0);
    int sol = 1e9;
    int ok = 0;
    for (auto x: next) {
        get<0>(ans) += min({ get<0>(x),get<1>(x),get<2>(x) });
        if (get<1>(x) < get<0>(x)) {
            get<1>(ans) += get<1>(x);
            if (get<0>(x) - get<1>(x) < sol) {
                sol = get<0>(x) - get<1>(x);
            }
        }
        else {
...
void solve() {
    ...
    int ans = 0;
    for (auto x : vertexs[0]) {
        for (auto y : vertexs[x]) {
            if (y == 0) continue;
            vector<tuple<int, int, int>> nexts;
            for (auto z : vertexs[y]) {
                if (z == x) continue;
                nexts.push_back(dfs(z, y));
            }
            int adds = 0;
            for (int i = 0; i < nexts.size(); ++i) {
                adds += min(get<0>(nexts[i]), get<1>(nexts[i]));
            }
            int tmps = 0;
            for (int i = 0; i < nexts.size(); ++i) {
                tmps += min({get<0>(nexts[i]), get<1>(nexts[i]), get<2>(nexts[i])});
            }
            tmps++;
            ans += min(tmps, adds);
        }
