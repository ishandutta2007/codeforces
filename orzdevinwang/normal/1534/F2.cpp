...
int main() {
	...
	G.work(n * m);
	L(j, 1, m) {
		L(i, 1, n) mp[G.kind[P(i, j)]] = j + 1;
		int now = 0;
		L(i, 1, n) if(s[i][j] == '#') {
			++now;
			if(now >= a[j]) mp[G.kind[P(i, j)]] = (j == 1 ? 1 : lef[i][j - 1]);
		}
		L(i, 1, n) lef[i][j] = mp[G.kind[P(i, j)]];
	...
	R(j, m, 1) {
		...
		L(i, 1, n) if(s[i][j] == '#') {
			++now;
			if(now >= a[j]) mp[G.kind[P(i, j)]] = (j == m ? m : rig[i][j + 1]);
		}
		L(i, 1, n) rig[i][j] = mp[G.kind[P(i, j)]];
