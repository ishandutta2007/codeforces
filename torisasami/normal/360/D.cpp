...
int main(){
	...
	rep(i, m) cb[i] = gcd(p - 1, b[i]);
	set<ll> sa, sb;
	rep(i, n) sa.insert(ca[i]);
	rep(i, m) sb.insert(cb[i]);
	set<ll> s;
	for(auto &ea: sa)
		for(auto &eb: sb)
			s.insert(gcd(p - 1, ea * eb));
	ll sz = div.size();
	ll dp[s.size() + 1][sz][2] = {};
	dp[0][0][0] = 1;
	map<ll, ll> ma_p;
	...
	for(auto &e: s){
		rep(j,sz){
			rep(k,2){
				dp[i + 1][j][k] += dp[i][j][k];
				dp[i + 1][ma_p[rm[j] * e / gcd(rm[j], e)]][k ^ 1] += dp[i][j][k];
			}
		}
		i++;
	}
	ll ans = p - 1;
	rep(j, sz) ans += (p - 1) / rm[j] * (dp[s.size()][j][1] - dp[s.size()][j][0]);
	cout << ans << endl;
	// for(auto &e: s)
	// 	cout << e << " ";
	// cout << endl;
