...
fun Long.factors():MutableList<Long> {
    val ret = mutableListOf(1L)
    val fat = factorSingleNumber(this)
...
fun main(){
    ...
    solve.cases{
        ...
        val total = x + y + x + y - 4
        val ret = mong
        for(v in total.factors()){
            val need = listOf(listOf(x-2,y),listOf(x-1,y-1), listOf(x,y-2), listOf(x-1,y,y-2), listOf(x,x-2,y-1))
            if(need.any { it.all { s -> s % v == 0L } }){
                ret.add(v)
                continue
            }
        }
