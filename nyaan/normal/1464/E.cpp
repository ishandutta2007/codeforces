...
void Nyaan::solve() {
  ini(N, M);
  auto g = graph(N, M, true);
  auto grundy = GrundyNumber(g);

  trc(grundy);
  ll mx = *max_element(all(grundy));
  ...
  vm Y(A);
  rep(i,A)Y[i] = ID[i] - X[i];
  auto Z = ID;

  walsh_hadamard_transform(Y);
  walsh_hadamard_transform(Z);
  trc(Y);
  trc(Z);
  rep(i,A) Z[i] /= Y[i];
  walsh_hadamard_transform(Z, true);
