...
void solve() {
  ...
  UNIQUE(X);
  ll K = len(X);
  using E = tuple<ll, ll, ll>; // cnt, sum, max
  SegTree<E> seg(
    [&](E x, E y) {
      auto [xc, xs, xm] = x;
      auto [yc, ys, ym] = y;
      return E({xc + yc, xs + ys, max(xm, ym)});
    },
    E({0, 0, 0}));
  seg.init(K);
  vc<E> seg_raw(K);
  FOR(k, K) get<2>(seg_raw[k]) = X[k];
  FOR(i, N) {
    auto k = LB(X, A[i]);
    get<0>(seg_raw[k]) += 1;
    get<1>(seg_raw[k]) += A[i];
  ...
  FOR(q, Q) {
    ...
    if (t == 1) {
      ll k_rm = LB(X, A[a]);
      get<0>(seg_raw[k_rm]) -= 1;
      get<1>(seg_raw[k_rm]) -= A[a];
      A[a] = b;
      ll k_ad = LB(X, A[a]);
      get<0>(seg_raw[k_ad]) += 1;
      get<1>(seg_raw[k_ad]) += A[a];
      seg.set(k_rm, seg_raw[k_rm]);
      seg.set(k_ad, seg_raw[k_ad]);
    } else {
      auto check = [&](E e) -> bool {
        auto [cnt, sum, max] = e;
        return max * cnt - sum <= a;
      };
      ll R = seg.max_right(check, 0);
      auto [cnt, sum, max] = seg.fold(0, R);
      ld ANS = ld(sum + a) / cnt;
      print(ANS);
    }
    // print("total", seg.fold(0, K));
  }
}
