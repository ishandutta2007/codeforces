...
void solve(){
    ...
    REP(i, n) {
        ...
        if (inputs.size() == 1) {
            pair<int, int> now = inputs.top();
            inputs.pop();
            if (now.second == 0 and now.first == a) {
                inputs.push(mp(a, 1));
                continue;
            }
            inputs.push(now);
        ...
        pair<int,int> now = inputs.top();
        inputs.pop();
        pair<int, int> backs = inputs.top();
        inputs.pop();
        if (now.second == 0) {
            if (now.first == a) {
                if (backs.second == 1) {
                    int hoge = max(backs.first, a);
                    while (inputs.empty() == false && inputs.top().second == 1) {
                        hoge = max(hoge, inputs.top().first);
                        inputs.pop();
                    }
                    inputs.push(mp(hoge, 1));
                    continue;
                }
                else {
                    inputs.push(backs);
            ...
            assert(backs.second == 0);
            if (backs.first == a&&now.first <= a) {
                int hoge = a;
                while (inputs.empty() == false && inputs.top().second == 1) {
                    hoge = max(hoge, inputs.top().first);
                    inputs.pop();
    ...
    if (inputs.size() == 3) {
        pair<int, int> a = inputs.top();
        inputs.pop();
        pair<int, int> b = inputs.top();
        inputs.pop();
        pair<int, int> c = inputs.top();
        inputs.pop();
        if (b.second == 1||a.second == 0||c.second == 0) {
            cout << "NO" << endl;
            return;
        }
        if (a.first <= b.first && c.first <= b.first) {
            cout << "YES" << endl;
            return;
        }
        cout << "NO" << endl;
