...
pair<vector<ll>, vector<ll>> dfs(int v, int p)
{
	vector<pair<vector<ll>, vector<ll>>> a;
	vector<ll> cnt(k), sum(k);
	...
	for (int i = 0; i < sz(a); ++i)
	{
		for (int j = 0; j < k; ++j)
		{
			...
			{
				int pl = (j + l + k - 1) / k;
				anspl += pl * a[i].x[j] * (sumcnt[l] - a[i].x[l]);
				ans += a[i].y[j] * (sumcnt[l] - a[i].x[l]);
			}
		}
	}

	++cnt[0];
	rotate(cnt.begin(), cnt.begin() + k - 1, cnt.end());
	rotate(sum.begin(), sum.begin() + k - 1, sum.end());
	sum[0] += cnt[0];
	return { cnt, sum };
}
