...
Mint calc(ll x) {
	ll sumFreq = 0, sumEle = 0;
	for (int i = x; i < MAXN; i += x) {
		sumFreq += freq[i];
		sumEle += (freq[i] * i) % MOD998;
		sumEle %= MOD998;
	}

	Mint ans = Mint(0, MOD998);
	for (ll i = x; i < MAXN; i += x) {
		Mint pw = TWO.pow(sumFreq - 3 + MOD998 - 1);
		Mint p = Mint(i, MOD998) * i * (sumFreq - 1) * pw * 2;
		Mint q = Mint(i, MOD998) * (sumEle - i) * sumFreq * pw;
