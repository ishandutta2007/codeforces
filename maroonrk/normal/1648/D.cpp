...
struct N{
	int a,b,c;
	N(pi p=pi(inf,inf)):a(p.a),b(p.b),c(min(a+b,inf)){}
	static N merge(const N&a,const N&b){
		N res(pi(min(a.a,b.a),min(a.b,b.b)));
		res.c=min({a.c,b.c,a.a+b.b});
		return res;
	}
};
...
void slv(){
	...
	vi c=readvi(n);
	a[0]=-a[0];
	rng(i,1,n)a[i]=a[i-1]+(-a[i]+b[i-1]);
	c[n-1]=-c[n-1];
	per(i,n-1)c[i]=c[i+1]+(-c[i]+b[i+1]);
	...
	segtree<MinNode> seg(a);
	rep(i,n){
		for(auto [j,k]:g[i]){
			seg.point_merge(i,seg.composite(j,i).v+k);
		}
	}
	vc<pi> rw(n);
	rep(i,n)rw[i]=pi(seg.x[i+seg.s].v,c[i]);
	segtree<N> work(rw);
	int ans=inf;
	for(auto [l,r,k]:qs){
		auto w=work.composite(l,r);
		chmin(ans,w.c+k);
	}
	int base=accumulate(all(b),int(0));
	print(base-ans);
}
