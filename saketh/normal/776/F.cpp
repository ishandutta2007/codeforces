...
int main() {
    ...
    F0R (m, M) {
        ...
        diags.eb(min(a, b), max(a, b));
    }
    F0R (i, N) sort(all(adj[i]));

    vvb used(N);
    F0R (i, N) used[i].resz(sz(adj[i]));

    vvi regs;
    F0R (i, N) F0R (j, sz(adj[i])) if (!used[i][j]) {
        vi reg;
        for (int v = i, e = j; !used[v][e]; ) {
            used[v][e] = true;
            reg.pb(v);

            int vv = adj[v][e];
            int ee = (index(adj[vv], v) + 1) % sz(adj[vv]);
            tie(v, e) = {vv, ee};
        }
        if (i || j) {
    ...
    F0R (i, R) F0R (j, sz(regs[i])) {
        int a = regs[i][j],
            b = regs[i][(j+1)%sz(regs[i])];
        if (a > b) swap(a, b);
        auto it = lb(all(diags), mp(a, b));
        if (it != diags.end() && *it == mp(a, b)) {
            int inx = it - diags.begin();
            if (match[inx] != -1) {
                radj.pb({ match[inx], i });
            } else match[inx] = i;
        }
    ...
    vi ans(R, -1);
    auto go = [&](auto& self, int loc, int val) -> void {
        loc = tr.centroid(loc);
        ans[loc] = val;
        tr.erase(loc);
        trav (e, tr.adj[loc]) if (int nbr = e[loc]; !tr.erased[nbr]) {
            self(self, nbr, val + 1);
        }
    };
    go(go, 0, 1);
