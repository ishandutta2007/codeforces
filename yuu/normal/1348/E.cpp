...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j = 0; j < k; j++) {  // leave j of a from this group
            int instant_a = a[i] - j;
            if (instant_a < 0) break;
            int64_t add = instant_a / k;
            instant_a %= k;
            int rem_b = b[i];
            if (instant_a) {
                if (rem_b + instant_a < k) continue;
                add++;
                rem_b -= (k - instant_a);
            }
            add += rem_b / k;
            rem_b %= k;
            for (int current_a = 0; current_a < k; current_a++) {
                if (g[current_a] < 0) continue;
                int current_b = (sum - current_a) % k;
                assert(current_b >= 0);
                f[(current_a + j) % k] =
                        max(f[(current_a + j) % k], g[current_a] + add + (current_a + j >= k) + (current_b + rem_b >= k));
            }
        }
        sum += a[i] + b[i];
    }
    cout << *max_element(f, f + k) << '\n';
}
