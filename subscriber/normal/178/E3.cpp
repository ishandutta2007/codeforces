...
//	freopen("1.out","w",stdout);	
	...
	if (a[i][j]+a[i-1][j]+a[i+1][j]+a[i][j-1]+a[i][j+1]>2)a[i][j]=1;else a[i][j]=0;		
	for (int it=0;it<50;it++)
	for (int i=2;i<=n;i++)for (int j=2;j<=n;j++)
	if (a[i][j]+a[i-1][j]+a[i+1][j]+a[i][j-1]+a[i][j+1]+a[i-1][j-1]+a[i-1][j+1]+a[i+1][j-1]+a[i+1][j+1]+a[i+2][j]+a[i-2][j]+a[i][j-2]+a[i][j+2]>6)a[i][j]=1;else a[i][j]=0;		
	N=n/O;
	for (int i=1;i<=N;i++)for (int j=1;j<=N;j++){
		A=B=0;
		for (int x=0;x<O;x++)for (int y=0;y<O;y++)if (a[i*O+x][j*O+y])A++;else B++;
