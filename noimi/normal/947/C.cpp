...
signed main(){
    ...
    multiset<int> s;rep(i,n)s.insert(in());
    vec b;
    rep(i,n){
        int l = 0,r = 1 << 30;
        auto prel = s.lower_bound(l),prer = s.upper_bound(r);
        rep(j,30){
            int mid = (l+r)>>1;
            auto nxt = s.lower_bound(mid);
            if(a[i]&(mid-l)){
                if(nxt != prer) l = mid,prel=nxt;
                else r=mid,prer = nxt;
            }
            else if(prel!=nxt)r=mid,prer=nxt;
            else l = mid,prel = nxt;
        }
        s.erase(s.find(l));
        b.pb(l^a[i]);
