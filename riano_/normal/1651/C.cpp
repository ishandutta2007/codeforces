...
int main(){
    ...
    rep(ii,T){
        ...
        rep(i,N) cin >> a[i];
        rep(i,N) cin >> b[i];
        ll AC = abs(a[0]-b[0]); ll AD = abs(a[0]-b[N-1]);
        ll BC = abs(a[N-1]-b[0]); ll BD = abs(a[N-1]-b[N-1]);
        ll A = 2e9,B = 2e9,C = 2e9,D = 2e9;
        rep(i,N){
            chmin(A,abs(a[0]-b[i])); chmin(B,abs(a[N-1]-b[i]));
            chmin(C,abs(a[i]-b[0])); chmin(D,abs(a[i]-b[N-1]));
        }
        cout << min({AC+B+D,AD+B+C,BC+A+D,BD+A+C,A+B+C+D,AC+BD,AD+BC}) << endl;
    }
