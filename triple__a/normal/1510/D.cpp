...
signed main(){
    ...
    cin>>n>>q;
    rep(i,n) cin>>a[i], g[a[i]%10].pb(a[i]);
    rep(i,10) sort(range(g[i]));
    rep(i,10){
        hi[i].pb(1);
        rep1(j,sz(g[i])){
            hi[i].pb(hi[i][j-1]*i%10);
//            cerr<<hi[i][j]<<" ";
        }
//        cerr<<endl;
    }
    if (q==0){
        int evn=0;
        for (int i=0;i<10;i+=2) evn+=sz(g[i]);
        if (sz(g[0])||(sz(g[5])&&evn)){
            cout<<n<<endl;
            rep(i,n) cout<<a[i]<<" ";
            return 0;
        }
    ...
    if (q==5){
        if (sz(g[5])){
            cout<<sz(g[1])+sz(g[3])+sz(g[5])+sz(g[7])+sz(g[9])<<endl;
            rep(i,n) if (a[i]&1) cout<<a[i]<<" ";
            return 0;
        }
        cout<<-1;
        return 0;
    ...
    if (q&1){
        for (int i=0;i<10;i+=2) g[i].clear();
    }
    rep(i,1<<16){
        int c[8];
        double ans=0;
        ...
        bool ok=1;
        rep(j,8) c[j]=w&3, w>>=2;
        rep(j,8) if (c[j]>sz(g[d[j]])) ok=false;
        if (!ok) continue;
        rep(j,8) rem=rem*hi[d[j]][sz(g[d[j]])-c[j]]%10;
        if (rem!=q) continue;
        rep(j,8) rep(k,c[j]) ans+=log(g[d[j]][k]), ansp*=g[d[j]][k];
        if (ans<res-1e-5||(ans<res+1e-5&&hii>ansp)) {res=ans, hii=ansp; rep(j,8) val[j]=c[j];}
    }
    if (res>999){
        cout<<-1;
        return 0;
    ...
    cout<<sum<<endl;
    rep(i,8){
        for (int j=val[i];j<sz(g[d[i]]);++j) cout<<g[d[i]][j]<<" ";
    }
    return 0;
}