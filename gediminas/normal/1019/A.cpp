...
int main () {
	...
	while (p <= (int)b.size()) {
		set<pair<int, pair<ll, int>> > pas;

		int pp[m] = {};

		for (int i = 1; i < m; i++) {
			if (a[i].size() > 0)
				pas.emplace(-a[i].size(), makep(a[i][0], i));
		}

		int tur = p + a[0].size();
		ll kain = 0;

		while (!pas.empty() and tur <= -pas.begin()->x) {
			pair<int, pair<ll, int>> d = *pas.begin();
			pas.erase(pas.begin());
			pp[d.y.y]++;
			kain += d.y.x;
			tur++;

			if (pp[d.y.y] < (int)a[d.y.y].size()) {
				pas.emplace(-(a[d.y.y].size() - pp[d.y.y]), makep(a[d.y.y][pp[d.y.y]], d.y.y));
			}
		}
