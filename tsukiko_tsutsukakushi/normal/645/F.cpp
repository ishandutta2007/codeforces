...
void solve() {
    ...
    // for(int i = N - 1; i >= 1; -- i) {
    //     if(cnt[i] == 0) continue;
    //     // res[i] = bc.com(cnt[i], k);
    //     val[i] = bc.com(cnt[i], k);
    ...
    for(int i = 1; i < N; ++ i) {
        for(int j = i; j < N; j += i) {
            ans += mint(i) * bc.com(cnt[j], k) * er.mebius[j / i];
            pre[j] += mint(i) * er.mebius[j / i];
        }
    }
    debug(ans);
    rep(_, q) {
        INT(x);
        for(int d: er.divisors(x)) {
            ++ cnt[d];
            ans += (bc.com(cnt[d], k) - bc.com(cnt[d] - 1, k)) * pre[d];
        }
        out(ans);
    }
}
