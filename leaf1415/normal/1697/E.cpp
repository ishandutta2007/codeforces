...
ll n;
ll x[105], y[105];
ll g[105][105], G[105][105], deg[105];
bool used[105];
...
int main(void)
{
	...
	rep(i, 1, n){
		...
		rep(j, 1, n){
			if(i == j) continue;
			chmin(d, abs(x[i]-x[j]) + abs(y[i]-y[j]));
		}
		rep(j, 1, n){
			if(i == j) continue;
			if(d == abs(x[i]-x[j]) + abs(y[i]-y[j])) g[i][j] = 1, deg[i]++;
		}
	}
	rep(i, 1, n) rep(j, 1, n){
	...
	rep(i, 1, n) rep(j, 1, n){
		...
		if(deg[i] == 1 && deg[j] == 1 && G[i][j]){
			cnt2++;
			used[i] = used[j] = true;
		}
	}
	rep(i, 1, n) rep(j, 1, n) rep(k, 1, n){
		if(used[i] || used[j] || used[k] || i == j || j == k || k == i) continue;
		if(deg[i] == 2 && deg[j] == 2 && deg[k] == 2 && G[i][j] && G[j][k] && G[k][i]){
			cnt3++;
	...
	rep(i, 1, n) rep(j, 1, n) rep(k, 1, n) rep(l, 1, n){
		...
		if(deg[i] == 3 && deg[j] == 3 && deg[k] == 3 && deg[l] == 3 && 
			G[i][j] && G[j][k] && G[k][l] && G[l][i] && G[i][k] && G[j][l]){
			cnt4++;
	...
	rep(i, 0, cnt2){
		rep(j, 0, cnt3){
			rep(k, 0, cnt4){
				mint tmp = comb(cnt2, i) * comb(cnt3, j) * comb(cnt4, k);
				tmp *= perm(n, n-i-2*j-3*k);
				ans += tmp;
			}
		}
	}
