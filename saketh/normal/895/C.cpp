...
int main() {
    ...
    vi index(MAXV + 1);
    F0R (i, sz(sv.primes)) index[sv.primes[i]] = i;

    int N; re(N); vi a(N); re(a);
    map<int, int> ct; trav (e, a) ct[e]++;

    const int MAXB = sz(sv.primes);
    vmn dp(1 << MAXB);
    dp[0] = 1;
    ...
    trav (e, ct) {
        mn even = 0, odd = 0;
        F0R (pick, e.s + 1) {
            (pick&1 ? odd : even) += mn::ncr(e.s, pick);
        }

        int mask = 0;
        trav (pr, sv.factor(e.f)) if (pr.s&1) mask |= 1 << index[pr.f];
