...
struct Rerooting{
	...
	void dfs(int v, int p)
	{
		...
		T sum = Ident();
		for(auto e : G[v]) if(e.to != p) sum = ope2(sum, ope(get(v, e.to)));
		dp[v] = ope3(v, sum);
	}
	void dfs2(int v, int p)
	{
		...
		sum = lsum[0] = Ident();
		rep(i, 0, m-1){
			sum = ope2(sum, ope(get(v, G[v][i].to)));
			lsum[i+1] = sum;
		}
		sum = rsum[m+1] = Ident();
		per(i, m-1, 0){
	...
	void calc(){
		...
		rep(i, 1, n){
			T sum = Ident();
			for(auto e : G[i]) sum = ope2(sum, ope(get(i, e.to)));
			ans[i] = ope3(i, sum);
		}
	}
	void print(){
...
ll dfs(int v, int p)
{
	ll sum = 0, deg = 0;
	for(auto u : G[v]){
		if(u != p) sum += rt.get(v, u).se, deg++;
	}
	if(deg == 0) return hmax;
	
	vector<ll> vec;
	for(auto u : G[v]){
		if(u == p) continue;
		vec.pb(dfs(u, v)-rt.get(v, u).se);
	}
	if(sz(vec) >= 2){
		sort(all(vec));
		ll tmp = 0;
		tmp += sum + vec[0] + vec[1];
		if(p != -1) tmp += rt.get(v, p).se;
		chmin(ans, tmp);
	}
	return sum + vec[0];
}
