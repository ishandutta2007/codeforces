// Problem: C. Pollywog
// Contest: Codeforces - Codeforces Round #459 (Div. 1)
...
std::set<int>st;
int a[1000005];
int state[1<<9|1],istate[1<<9|1];
int cc=0;
struct LJC00175DengG{
	int a[72][72];
	LJC00175DengG()
	{
	...
	LJC00175DengG operator*(const LJC00175DengG& b)
	{
		LJC00175DengG c;
		rg(cc)rg_(j,cc)rg_(k,cc)mne(c.a[i][j],a[i][k]+b.a[k][j]);gr gr gr
		return c;
	}
	LJC00175DengG operator^(int x)
	{
		LJC00175DengG res,t=(*this);
		rg(cc)mne(res(i,i),0);gr
		while(x)
		{
			if(x&1)res=res*t;
			t=t*t;
...
signed main()
{
	...
	rg(m)a[i]=read();gr
	rrg(1<<m)
	if(__builtin_popcount(i)==n)++cc,istate[i]=cc,state[cc]=i;
	gr
	rg(q)int pos=read();v[pos]=read();for(int i=max(pos-10,0);i<=min(pos+10,k-1);i++)st.insert(i);gr
	rg(cc)
	int s=state[i];
	if(s&1)
	{
		...
		{
			if((t>>j)&1);
			else mne(trans(i,istate[t|(1<<j)]),a[j+1]);
		}
	}
	else
	{
	...
	for(int i=1;i+n-1<k;i++)
	{
		if(st.count(i))
		{
			...
			for(int j=1;j<=cc;j++)
			{
				...
				if(s&1)
				{
					...
					{
						if((t>>k)&1);else{
							mne(a2(1,istate[t|(1<<k)]),ans(1,j)+v[i+k+1]+a[k+1]);
						}
					}
				}
				else
				{
					mne(a2(1,istate[s>>1]),ans(1,j));
				}
			}
			ans=a2;
		}
		...
		{
			int g;if(st.size()==0||(*--st.end())<i)g=k-n;else g=(*st.lower_bound(i))-1;
			// odp(i,g);
			ans=ans*(trans^(g-i+1));
			i=g;
		}
