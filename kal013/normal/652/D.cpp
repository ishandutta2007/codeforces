...
/*/-----------------------------Code begins----------------------------------/*/
int seg[maxn],laz[maxn];
#define M ((s+e)/2)
void rmv(int node,int s,int e){
    if (laz[node]==0) return;
    seg[node]+=(e-s+1)*laz[node];
    if (s!=e){
        laz[2*node]+=laz[node];laz[2*node+1]+=laz[node];
    }
    laz[node]=0;
}
...
inline int query(int node,int s,int e,int l){
    assert(s<=l && l<=e);
    rmv(node,s,e);
    if (s==e) return seg[node];
    if (l<=M) return query(2*node,s,M,l);
    return query(2*node+1,M+1,e,l);
}
...
void solve(){
    ...
    For(i,2*n){
        ...
        if (l[j]==a[i]){
            ...
            ans[j]=query(1,0,n-1,bck[j]);
            update(1,0,n-1,0,bck[j],1);
        }
    }
    For(i,n){
        d1(ans[i]);
