...
void calc(int n, int bound) {
    dp2[1][1][0] = dp2[1][1][1] = 1;
    for(int i = 2; i <= n; ++i) {
        dp2[i][1][0] = dp2[i - 1][1][1];
        dp2[i][1][1] = dp2[i - 1][1][0];
        for(int cur = 2; cur <= i; ++cur) {
            dp2[i][1][0] += dp2[i - 1][cur][1];
            dp2[i][1][1] += dp2[i - 1][cur][0];
            if(cur <= bound) {
                dp2[i][cur][0] = dp2[i - 1][cur - 1][0];
                dp2[i][cur][1] = dp2[i - 1][cur - 1][1];
            }
            else {
                dp2[i][cur][0] = 0;
                dp2[i][cur][1] = 0;
    ...
    for(int cur = 1; cur <= bound; ++cur) {
        dp[bound] += dp2[n][cur][0] + dp2[n][cur][1];
        dp[bound] %= MM;
    }
}
...
inline void solve(ld tt) {
    ...
    for(int i = 1; i <= min(k,n); ++i) {
        ans += sub(dp[i], dp[i - 1]) * dp[min((k - 1) / i, n)];
        ans %= MM;
        //cout << ans << " ";
    }
    cout << ans * pwm(2, MM - 2, MM) % MM << endl;
}
