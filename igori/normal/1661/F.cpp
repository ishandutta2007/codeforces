...
void solve()
{
    ...
    auto split_cost = [&](int len, int k)
    {
        int avg = len / k;
        int rem = len % k;
        return 1ll * avg * avg * (k - rem) + 1ll * (avg + 1) * (avg + 1) * rem;
    };
    ...
    while (L + 1 < R)
    {
        long long Mx = (L + R) / 2;
        // cout << Mx << " :\n";
        // can we reach <= m energy after splits with effectiveness at least Mx
        long long energy = 0, portals = 0;
        for (int i = 0; i < n; i++)
        {
            ...
            {
                long long M = (L + R) / 2;
                //cout << M << " " << split_cost(a[i], M - 1) << " " << split_cost(a[i], M) << "\n";
                if (split_cost(a[i], M - 1) - split_cost(a[i], M) >= Mx)
                    L = M;
                else
                    R = M;
            }
            // cout << " -> " << Mx << " " << L << " " << split_cost(a[i], L) << "\n";
            energy += split_cost(a[i], L);
            portals += L - 1;
        }

        long long energy_strict = 0, portals_strict = 0;
        for (int i = 0; i < n; i++)
        {
            long long L = 1, R = a[i] + 1;
            while (L + 1 < R)
            ...
            //cout << " -> " << Mx << " " << L << " " << split_cost(a[i], L) << "\n";
            energy_strict += split_cost(a[i], L);
            portals_strict += L - 1;
        }
        //cout << "result = " << energy << " " << portals << "\n";
        //cout << "result = " << energy_strict << " " << portals_strict << "\n";
        //cout << "\n";
        if (energy <= m)
        {
            L = Mx;
            //cout << "upd " << portals_strict + (max(energy_strict - m, 0LL) + Mx - 1) / Mx << "\n";
            ans = min(ans, portals_strict + (max(energy_strict - m, 0LL) + Mx - 1) / Mx);
        }
        else
            R = Mx;
    }
