...
pair<ll, int> sum(int p) {
    ...
    for (int i = p; i >= 0; i = (i & (i + 1)) - 1) {
        res.first += ft[i].first;
        res.second += ft[i].second;
    }
    return res;
}
pair<ll, int> sum(int l, int r) {
    auto resR = sum(r);
    auto resL = sum(l - 1);
    return {resR.first - resL.first, resR.second - resL.second};
}
...
int solve() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (x <= K) {
            ...
            for (int from = x; from <= N; from += x) {
                int to = from + x - 1;
                auto f = sum(from, to);
                ans += f.first - f.second * 1LL * from;
            }
            ans += sum(1, x - 1).first;
        }
        ...
        int to = x;
        for (int d = 1; d <= (N / K) + 2; ++d) {
            int from = x / (d + 1) + 1;
            from = max(from, K + 1);
            if (from <= to) {
                auto f = sum(from, to);
                ans += f.second * 1LL * x - f.first * 1LL * d;
                to = from - 1;
            }
        }
//        DBN(ans);
        ans += sum(x + 1, N).second * 1LL * x;
//        DBN(ans);
        cout << ans << ' ';
