  ...
  auto calc_diam = [&](ll root) -> ll {
    ll a = -1;
    {
      ...
      while (!que.empty()) {
        ...
        a = v;
        FORIN(e, G[v])
        if (chmin(dist1[e.to], dist1[e.frm] + 1)) que.push_back(e.to);
      }
    }
    ll ANS = 0;
    {
      ...
      while (!que.empty()) {
        ...
        ANS = dist2[v];
        FORIN(e, G[v])
        if (chmin(dist2[e.to], dist2[e.frm] + 1)) que.push_back(e.to);
      }
    }
    return ANS;
  };

  FOR(v, N) if (uf[v] == v) diam[v] = calc_diam(v);
