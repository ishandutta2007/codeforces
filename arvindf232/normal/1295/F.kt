...
fun main(){
    ...
    solve.cases{
        ...
        val cc = coordinateCompression(co)
        val new = mutableListOf<Pair<Int,Int>>()
        for((a,b) in all){
            val aa = cc.reverse[a-1]!! + 1
            val bb = cc.reverse[b]!!
            new.add(Pair(aa,bb))
        }
        var base = 1
        for((a,b) in all){
            base = base modDivide (b-a+1)
        }
        val lower = IntArray(cc.forward.size+1)
        val upper = IntArray(cc.forward.size+1)
        for(i in 0 until cc.forward.size){
            lower[i+1] = if(i == 0) 0 else cc.forward[i-1] + 1
            upper[i+1] = cc.forward[i]
        }
        lower[0] = -1
        upper[0] = -1
        val ins = lower.size

        val len = IntArray(ins){upper[it] - lower[it] + 1}

        val ok = Array(n){BooleanArray(ins)}
        for(i in 0 until n){
            for(p in 0 until ins){
                ok[i][p] = lower[p] >= all[i].first && upper[p] <= all[i].second
            }
        }

        val DP = Array(n+1){IntArray(ins)}
        DP[0][0] = 1
        for(p in 0 until n){
            val f = DP[p].torsq()
//            for(from in 0 until ins){
                for(to in 1 until ins){
                    for(pass in 1..n){
                        if(p + pass !in 0 .. n) continue
                        if((1..pass).any { !ok[p + it -1][to] }) continue
                        DP[p + pass][to] = DP[p + pass][to] modPlus (FACT.bigChoose((len[to] + pass - 1).adjust(),pass) modM f.sumQuery(0,to-1))
                    }
                }
//            }
        }
