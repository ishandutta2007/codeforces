...
int main() {
    ...
    F0R (i, N) {
        edges.pb({ N, i, c[i] });
        FOR (j, i + 1, N) edges.pb({ i, j,
                ll(k[i] + k[j]) * (abs(x[i] - x[j]) + abs(y[i] - y[j]))});
    }
    ...
    ll ans = 0;
    sort_by(edges, a.c < b.c);
    union_find uf(N + 1);
    trav (e, edges) if (uf.unio(e.u, e.v)) {
        ans += e.c;
        if (e.u == N) power.pb(e.v + 1);
        else conn.eb(e.u + 1, e.v + 1);
    }

    ps(ans);
    ps(sz(power)); pc(power);
    ps(sz(conn)); trav (p, conn) ps(p.f, p.s);
