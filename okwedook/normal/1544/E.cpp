...
void solve() {
    ...
    string ans = "";
    if (*max_element(all(pi)) == 1) {
        ans = s;
    }
    sort(s);
    int ptr = 0;
    while (ptr < sz(s) && s[ptr] == s[0]) ++ptr;
    int ptr2 = ptr;
    while (ptr < sz(s) && s[ptr] == s[ptr2]) ++ptr;
    if (ptr != sz(s)) {
        int ptr3 = ptr;
        while (ptr < sz(s) && s[ptr] == s[ptr3]) ++ptr;
        reverse(s.begin() + 1, s.begin() + ptr2 + 1);
        reverse(s.begin() + ptr2 + 1, s.begin() + ptr3 + 1);
    } else {
        reverse(s.begin() + 1, s.begin() + ptr);
    }
    if (ans == "" || s < ans) ans = s;
    println(ans);
}
