...
struct RmQ : SegmentTree<T>{
    ...
    RmQ(const vector<T>& v, const T& def_value) : SegmentTree<T>(v, def_value){
        for(ll i = SegmentTree<T>::size; --i;) SegmentTree<T>::data[i] = f(SegmentTree<T>::data[i * 2], SegmentTree<T>::data[i * 2 + 1]);
    }
};
template<class T>
struct RMQ : SegmentTree<T>{
...
signed main(){
    LL(n);
    VEC(ll,a,n);
    if(min(a)*2>=max(a))return out(vector<ll>(n,-1));
    ...
    rep(n){
        ...
        while(abs(ok-ng)>1){
            cen=(ok+ng)/2;
            if(mn.get(i,i+cen)*2>=a[i])ok=cen;
            else ng=cen;
        }
        if(ng==n+1)b[i]=LINF;
        else b[i]=ok;
    }
    rrep(n-1)chmin(b[i],b[i+1]+1);
    chmin(b.back(),b.front()+1);
    rrep(n-1)chmin(b[i],b[i+1]+1);
    out(b);
}