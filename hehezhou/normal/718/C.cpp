...
struct Matrix
{
    ...
    inline friend Matrix operator*(Matrix x, Matrix y)
    {
        ...
        for (ll i = 0; i <= cnt; ++i)
            for (ll j = 0; j <= cnt; ++j)
                for (ll k = 0; k <= cnt; ++k)
                    ADD(ans[i][j], 1ll * x[i][k] * y[k][j] % mod);
        return ans;
    }
    inline friend Matrix operator+(Matrix x, Matrix y)
    {
        ...
        for (ll i = 0; i <= cnt; ++i)
            for (ll j = 0; j <= cnt; ++j)
                ADD(ans[i][j], x[i][j]), ADD(ans[i][j], y[i][j]);
        return ans;
    }
...
inline void pushdown(ll now)
{
    if (!judge(q[now].tag))
    {
        ...
        q[now * 2].num = q[now * 2].num * cur;
        q[now * 2 + 1].num = q[now * 2 + 1].num * cur;
        q[now * 2].tag = q[now * 2].tag * cur;
        q[now * 2 + 1].tag = q[now * 2 + 1].tag * cur;
        init(q[now].tag);
    }
}
...
void change(ll now, ll l, ll r, ll cl, ll cr, Matrix k)
{
    ...
    pushdown(now);
    ll mid = (l + r) / 2;
    change(now * 2, l, mid, cl, cr, k);
    change(now * 2 + 1, mid + 1, r, cl, cr, k);
    pushup(now);
}
