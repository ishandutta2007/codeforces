...
class FPoiskVGlubinu {
public:

	vector<vector<int>> g;

	vector<int> tin, tout, size;
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		struct Merge {
			Value operator()(Value l, Value r) const {
				Value res;
		...
		struct ApplyUpdate {
			Value operator()(Value v, int x, size_t l, size_t r) {
				return {
					v.mx + x,
					v.cnt
				};
		...
		TopDownSegmentTree<Value, Merge, int, ApplyUpdate, std::plus<>> tree(vector<Value>(n, {0, 1}));
		...
		for (int i: range(q)) {
			...
			if (tout[a] < tout[b]) {
				tree.update(tin[a], tout[a], mult);
				tree.update(tin[b], tout[b], mult);
				need += mult;
			} else {
				int c = *partition_point(g[a].begin(), g[a].end(), [&](int v) {
					return tout[v] < tout[b];
				});
				tree.update(tin[c], tout[c], -mult);
				tree.update(tin[b], tout[b], mult);
			}

			auto found = tree.getResult(0, n);
			if (found.mx == need) {
				out << found.cnt << "\n";
			} else {
				out << 0 << "\n";
