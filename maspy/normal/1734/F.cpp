...
void solve() {
  ...
  auto dfs = [&](auto& dfs, ll N, ll M, bool same) -> ll {
    T key = {N, M, same};
    if (MP.count(key)) return MP[key];
    auto& res = MP[key];
    if (N + M < len(S)) {
      FOR(i, M) {
        if (same && S[i] == S[N + i]) ++res;
        if (!same && S[i] != S[N + i]) ++res;
      }
      return res;
    }
    if (N % 2 == 0) {
      res += dfs(dfs, N / 2, ceil(M, 2), same);
      res += dfs(dfs, N / 2, floor(M, 2), same);
    }
    if (N % 2 == 1) {
      res += dfs(dfs, N / 2, ceil(M, 2), !same);
      res += dfs(dfs, (N + 1) / 2, floor(M, 2), !same);
    }
    return res;
  };

  ll ANS = dfs(dfs, N, M, 0);
  print(ANS);
}
