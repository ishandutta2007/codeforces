...
int main(){
    ...
    sort(all(av));
    sort(all(bv));
    REP(i, SZ(stv) - 1) stv[i + 1] += stv[i];
    REP(i, SZ(av) - 1) av[i + 1] += av[i];
    REP(i, SZ(bv) - 1) bv[i + 1] += bv[i];
    ll res = INF;
    REP(i, min(strong, k) + 1){
        if(SZ(av) <= k - i or SZ(bv) <= k - i) continue;
        chmin(res, stv[i] + av[k - i] + bv[k - i]);
    }
    print(res);
}