...
int main() {
    ...
    const int T = ceil(sqrt(N));

    vi indep;
    vb used(N);

    queue<int> low;
    F0R (i, N) if (deg[i] < T - 1) low.push(i);
    while (!low.empty()) {
        int u = low.front();
        low.pop();

        bool take = 1;
        trav (v, adj[u]) take &= !used[v];
        if (take) {
            used[u] = 1;
            indep.pb(u + 1);
        }
    ...
    int start = 0;
    while (deg[start] < T-1) start++;

    vi path = {start};
    vi index_in_path(N, -1);
    index_in_path[start] = 0;

    while (1) {
        bool ext = 0;
        int earliest = sz(path);
        trav (v, adj[path.back()]) if (deg[v] >= T-1) {
            if (index_in_path[v] == -1) {
                index_in_path[v] = sz(path);
                path.pb(v);
                ext = 1;
                break;
            } else {
                ckmin(earliest, index_in_path[v]);
            }
        }
        if (!ext) {
            int cyc_len = sz(path) - earliest;
            assert(cyc_len >= T);

            ps(2);
            ps(cyc_len);
            FOR (inx, earliest, sz(path)) {
                if (inx > earliest) pr(" ");
                pr(path[inx] + 1);
            }
            ps();
