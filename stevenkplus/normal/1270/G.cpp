...
int main() {
  ...
  for (int t = 1; t <= T; ++t) {
    ...
    vector<pii> ch(n);
    for (int &i: v) cin >> i;
    multiset<pii> neg, pos;
    for (int i = 0; i < n; ++i) {
      if (v[i] <= 0) neg.insert(pii(v[i], i));
      else pos.insert(pii(v[i], i));
    }
    int m = n;
    while (sz(pos) > 0) {
      pii l = *neg.begin();
      ...
      if (l.fi >= x - m) {
        ...
        int val = l.fi + h.fi;
        v.push_back(val);
        ch.push_back(pii(l.se, h.se));
        if (val <= 0) neg.insert(pii(val, id));
        else pos.insert(pii(val, id));
      }
    }
    pp(v);
    pp(ch);
    ...
    vector<int> ans;
    auto go = y_combinator([&](auto go, int cur) -> void {
