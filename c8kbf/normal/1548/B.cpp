...
_ n, a[maxn], s, l, r, md, f[maxn][31], lg2[maxn];
...
inline void AC() {
    ...
    for(_ i = 1; i <= n-1; ++i) f[i][0] = abs(a[i+1]-a[i]);
    for(_ j = 1; j <= lg2[n-1]; ++j) for(_ i = 1; i+(1<<j)-1 <= n-1; ++i) f[i][j] = gcd(f[i][j-1], f[i+(1<<(j-1))][j-1]);
    s = 0;
    l = 1;
    r = n;
    for(; l <= r; ) {
...
_ qr(_ x, _ y) {
    return gcd(f[x][lg2[y-x+1]], f[y-(1<<lg2[y-x+1])+1][lg2[y-x+1]]);
}

bool check(_ x) {
    for(_ i = 1; i+x-1 <= n-1; ++i) if(qr(i, i+x-1) != 1) return true;
    return false;
}