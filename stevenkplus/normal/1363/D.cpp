...
int main() {
  ...
  for (int t = 1; t <= T; ++t) {
    ...
    for (int i = 0; i < 10; ++i) {
      if (sz(v) == 1) break;
      vector<int> w(v.begin(), v.begin() + sz(v) / 2);
      if (q(w) == globalMax) {
        v = w;
      } else {
        v = vector<int>(v.begin() + sz(v) / 2, v.end());
      }
    }
    if (sz(v) > 1) { while (true);}

    vector<int> ans(k, globalMax);
    for (int i = 0; i < k; ++i) {
      bool ismaxgroup = false;
      for (int j: inp[i]) {
        if (j == v[0]) {
          ismaxgroup = true;
          break;
