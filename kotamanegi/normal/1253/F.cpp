...
void solve(){
    ...
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> nexts;
    REP(i, k) {
        dist[i] = mp(0, i);
        nexts.push(mp(0, i));
    }
    while (!nexts.empty()) {
        ...
        nexts.pop();
        if (now.first != dist[now.second].first) continue;
        for (auto& x : vertexs[now.second]) {
            if (dist[x.first].second != -1) {
                edges.push_back(make_tuple(now.first + x.second + dist[x.first].first, dist[x.first].second, dist[now.second].second));
            }
            if (dist[x.first].first > now.first + x.second) {
                dist[x.first] = mp(now.first + x.second, dist[now.second].second);
                nexts.push(mp(now.first + x.second, x.first));
            }
        }
    }
    sort(ALL(edges));
    HLDecomposition HL(n);
    SegmentTree<int> seg(2 * n, [](int a, int b) {return max(a, b); }, 0);
    REP(i, n) {
        ut[i] = i;
    }
    vector<tuple<int, int, int>> used;
    REP(i, edges.size()) {
        if (um(get<1>(edges[i]), get<2>(edges[i]))) {
            used.push_back(edges[i]);
            HL.add_edge(get<1>(edges[i]), get<2>(edges[i]));
        }
    }
    HL.build();
    REP(i, used.size()) {
#undef int
        vector<pair<int,int>> lcas = HL.query(get<1>(used[i]), get<2>(used[i]));
#define int ll
        if (lcas.size() == 1) lcas.back().first++;
        seg.update(lcas[0].first, get<0>(used[i]));
    }
    REP(i, query) {
        int a, b;
        cin >> a >> b;
        a--; b--;
#undef int
        vector<pair<int, int>> lcas = HL.query(a, b);
#define int ll
        int ans = 0;
        lcas.back().first++;
        REP(q, lcas.size()) {
            ans = max(ans, seg.query(lcas[q].first, lcas[q].second));
        }
        cout << ans << endl;
    }
}
