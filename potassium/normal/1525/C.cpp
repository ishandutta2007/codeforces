...
int main() {
	CASE(t) {
		int n,m; R(n,m);
		vector<pair<pair<int, char>, int>> pt(n);
		for (int i = 0; i < n; i++) R(pt[i].fi.fi);
		for (int i = 0; i < n; i++) R(pt[i].fi.se);
		for (int i = 0; i < n; i++) pt[i].se = i;
		...
		for (int par = 0; par <= 1; par++) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (dir[i] == 'L' && !dq.empty() && dir[dq.back()] == 'R') {
					ans[idx[dq.back()]] = ans[idx[i]] = (a[i] - a[dq.back()]) / 2;
					dq.pop_back();
				}
				else {
					dq.emplace_back(i);
			...
			while ((int)dq.size() >= 2 && dir[dq[1]] == 'L') {
				int x = dq.front(); dq.pop_front();
				int y = dq.front(); dq.pop_front();

				ans[idx[x]] = ans[idx[y]] = (a[x] + a[y]) / 2;
			}
			while ((int)dq.size() >= 2 && dir[dq[(int)dq.size() - 2]] == 'R') {
				int x = dq.back(); dq.pop_back();
				int y = dq.back(); dq.pop_back();

				ans[idx[x]] = ans[idx[y]] = m - (a[x] + a[y]) / 2;
			}
			if ((int)dq.size() >= 2) {
				assert((int)dq.size() == 2);
				assert(dir[dq.front()] == 'L' && dir[dq.back()] == 'R');

				int x = dq.front();
				int y = dq.back();

				ans[idx[x]] = ans[idx[y]] = (2 * m + a[x] - a[y]) / 2;
			}
		}
