...
int main() {
    ...
    auto eval = [&](vi& pp) {
        mn ways = 1;
        for (int i = 0, elts = 0, tp = 1; i <= sz(pp); tp *= pp[i++]) {
            int mp = N/tp - (i<sz(pp) ? N/(pp[i]*tp) : 0);
            ways *= mn::ncr(elts + mp - 1, mp - 1) * mn::fact(mp);
            elts += mp;
            if (i == sz(pp)) break;
        }
        ans += ways;
    ...
    vi pp(31 - __builtin_clz(N), 2);
    eval(pp);

    if (3 << (31 - __builtin_clz(N)) / 2 <= N) {
        pp.back() = 3;
        do {
            eval(pp);
