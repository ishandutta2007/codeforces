...
int main() {
    ...
    BFS bfs1(adj, {special[0]});
    int root = max_element(all(bfs1.distance)) - bfs1.distance.begin();
    BFS bfs2(adj, {root});
    int other = max_element(all(bfs2.distance)) - bfs2.distance.begin();
    int diam = bfs2.distance[other];
    vi path = bfs2.get_path(other);
    vi center = sz(path) % 2 == 1 ? vi{path[sz(path) / 2]} : vi{path[sz(path) / 2 - 1], path[sz(path) / 2]};
    BFS bfs3(adj, center);
    int max_dist = *max_element(all(bfs3.distance));
    int start = -1;
    for (int i = n - 1; i >= 0; i--)
        if (bfs3.distance[i] == max_dist)
            start = i;

    int ans = 2 * (accumulate(all(keep), 0) - 1) - diam;
    ps(start + 1);
    ps(ans);
