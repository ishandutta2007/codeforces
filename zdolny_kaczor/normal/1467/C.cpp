...
void solve() {
	...
	for (int &x : a1) scanf("%d", &x);
	for (int &x : a2) scanf("%d", &x);
	for (int &x : a3) scanf("%d", &x);
	ll s1 = sum(a1), s2 = sum(a2), s3 = sum(a3);
	ll m1 = *min_element(ALL(a1)), m2 = *min_element(ALL(a2)), m3 = *min_element(ALL(a3));
	ll loss = numeric_limits<ll>::max();
	mini(loss, s1);
	mini(loss, s2);
	mini(loss, s3);
	mini(loss, m1 + m2);
