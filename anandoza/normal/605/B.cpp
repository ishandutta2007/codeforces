...
int main() {
    ...
    int n, m;
    re(n, m);
    vector<pair<int, bool>> edges(m);
    re(edges);
    for (auto &[w, i] : edges)
        i = !i;

    // bool signifies "not included"

    vi order(m);
    iota(all(order), 0);
    sort_by(order, edges[l] < edges[r]);
    ...
    for (int i : order) {
        auto [w, not_included] = edges[i];
        if (!not_included) {
            ans[i] = {vertices - 1, vertices};
            vertices++;
            if (vertices > n) {
                ps(-1);
                return 0;
