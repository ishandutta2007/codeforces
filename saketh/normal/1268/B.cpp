...
int main() {
    ...
    F0Rd (i, N) {
        if (singletons) {
            int last = parity + 2 * (singletons - 1);
            int room = ht[i] - (last + 1);
            ans += room / 2 + singletons;

            if (parity) {
                if (room&1) singletons++;
            } else {
                if (!(room&1)) singletons--;
            }
            if (singletons) parity = !parity;
            else parity = 0;
        } else {
            ans += ht[i] / 2;
            if (ht[i]&1) { parity = 0, singletons = 1; }
        }
    }
