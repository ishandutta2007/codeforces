...
char lowest(int i, int j) {
	char c[4] = {get(i - 1, j), get(i, j - 1), get(i, j + 1), get(i + 1, j)};
	for (char d = 'A'; ; ++d) {
		bool seen = false;
		for (char x : c) if (x == d) seen = true;
		if (!seen) return d;
	}
}
int main() {
	...
	REP(i, n) {
		REP(j, m) {
			if (ans[i][j]) continue;
			char here = lowest(i, j);
			int siz = 1;
			while (true) {
				if (i + siz == n || j + siz == m) break;
				bool blocked = false;
				REP(k, siz + 1) if (ans[i + siz][j + k] || ans[i + k][j + siz]) blocked = true;
				if (blocked) break;
				if (lowest(i, j + siz) < here) break;
				if (get(i - 1, j + siz) == here) break;
				siz++;
			}
			REP(k1, siz) REP(k2, siz) ans[i + k1][j + k2] = here;
			debug(dump(ans, 0, n, 0, m));
		}
	}
	REP(i, n) printf("%s\n", ans[i]);
	exit(0);
