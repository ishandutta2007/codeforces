...
static void solve()
{
    ...
    for (int i = 0; i < N; i++)
    {
        int x, y;
        cin >> x >> y;
        pnts.emplace_back(pnt(x, y), i);
    }
    CompareAngle cmp(pnt(0, 0));
    sort(RA(pnts), [=](auto a, auto b) { return cmp(a.first, b.first); });
    pnts.push_back(pnts[0]);
    pnt best = 0;
    int p = -1, q = -1;
    for (int i = 0; i < N; i++)
    {
        pnt angle = conj(pnts[i].first) * pnts[i + 1].first;
        if (angle.imag() < 0)
            angle = conj(angle);
        if (p == -1 || cross(angle, best) > 0)
        {
            best = angle;
            p = pnts[i].second;
            q = pnts[i + 1].second;
