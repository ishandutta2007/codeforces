...
public:
    void solve() {
        int n = in.readInt();
        int q = in.readInt();
        auto a = in.readIntArray(1 << n);

        unsigned state = 0;
        arr2d<ll> sums(n + 1, 1 << n);
        for (int i : range(1 << n)) {
            sums(0, i) = a[i];
        }
        for (int i : range(1, n + 1)) {
            for (int j = 0; j < (1 << n); j += (1 << i)) {
                sums(i, j) = sums(i - 1, j) + sums(i - 1, j + (1 << (i - 1)));
            }
        }

        RecursiveFunction sum = [&](const auto& self, int pos, int l, int r, int depth) -> ll {
            int end = pos + (1 << depth);
            if (r <= pos || l >= end) {
                return 0;
            }
            ...
            if (isSet(state, depth - 1)) {
                return self(pos + (1 << (depth - 1)), l + (1 << (depth - 1)), r + (1 << (depth - 1)), depth - 1) +
                       self(pos, l - (1 << (depth - 1)), r - (1 << (depth - 1)), depth - 1);
            } else {
                return self(pos, l, r, depth - 1) + self(pos + (1 << (depth - 1)), l, r, depth - 1);
            }
        };
        for (int z : range(q)) {
            int type = in.readInt();
            if (type == 1) {
                unsigned x = in.readInt() - 1;
                int k = in.readInt();
                x ^= state;
                int delta = k - sums(0, x);
                for (int i : range(n + 1)) {
                    sums(i, x) += delta;
                    x = unsetBit(x, i);
                }
