...
signed main(){
    LL(n,k);
    using bit=bitset<7>;
    const string num_s[10] = {"1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011"};
    bit num[10];
    rep(10)num[i]=bit(num_s[i]);
    vec(bit,a,n);
    each(i,a){
        STR(s);
        i=bit(s);
    ...
    dp[n][k]=1;
    rrep(n){
        rrep(j,10)if((~num[j]&a[i]).none()){
            ll c=(num[j]^a[i]).count();
            rep(l,c,k+1)if(dp[i+1][l]&&!dp[i][l-c]){
                dp[i][l-c]=1;
                back[i][l-c]=j;
            }
        }
    }
    if(!dp[0][0])return out(-1);
    ll cost=0;
    rep(n){
        ll j=back[i][cost];
        ll c=(num[j]^a[i]).count();
