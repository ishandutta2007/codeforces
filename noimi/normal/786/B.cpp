...
template <typename T = ll> struct SectionGraph {
    int sz;
    WeightedGraph<T> g;
    SectionGraph(int n) {
        sz = 1;
    ...
    // [l,r)  [L,R) 
    void add_edge(int l, int r, int L, int R, T w) {
        if(r - l == 1 and R - L == 1) {
            g[sz + l].eb(sz + L, w);
            return;
        }
        ...
        if(R - L > 1)
            for(int d = R - L; d >= 1; d = R - L) {
                int Lt = L | ((1U << 31) >> __builtin_clz(d));
                int k = __builtin_ctz(Lt);
                g[id].eb(((sz | L) >> k), w);
                L += Lt & (-Lt);
            }
        if(r - l > 1)
            for(int d = r - l; d >= 1; d = r - l) {
                int L = l | ((1U << 31) >> __builtin_clz(d));
                int k = __builtin_ctz(L);
                int node = (sz | l) >> k;
                if(node < sz) node += sz * 2;
                g[node].eb(id, w);
                l += L & (-L);
            }
    }
...
main() {
    ...
    auto d = dijkstra(g.g, s - 1 + g.sz);
    rep(i, n) cout << (d[g.sz + i] == LLONG_MAX ? -1 : d[g.sz + i]) << " \n"[i == n - 1];
}