...
#define update(cell) do { \
    cell = n; \
    if (i) \
        smin(cell, nxt[min(n, dp[i - 1][j][k] + 1)][t[0][i - 1] - 'a']); \
    if (j) \
        smin(cell, nxt[min(n, dp[i][j - 1][k] + 1)][t[1][j - 1] - 'a']); \
    if (k) \
        smin(cell, nxt[min(n, dp[i][j][k - 1] + 1)][t[2][k - 1] - 'a']); \
} while (false)

const int maxn = 100 * 1000 + 10,
      maxt = 250 + 10,
      sigma = 30;
int dp[maxt][maxt][maxt];
int nxt[maxn][sigma];
int n, q;
string s, t[3];
