...
class point(val x:Int, val v:Int):Comparable<point>{
    var cx = -1
...
fun main(){
    solve.cases{
        ...
        val XI = getline(n)
        val SI = getline(n)
        val all = MutableList(n){point(XI[it], SI[it])}
        all.sort()
        val allx = all.map{it.x}
        val COM = coordinateCompression(allx)
        for(a in all){
            a.cx = COM.reverse[a.x]!!
        }

        val counts = simpleSegTree(n)
        val sums = longSumSimple(n)
        var total = 0L

        for(a in all){

            val num = counts.queryFunctional(0,a.cx)
            val subtract = sums.queryFunctional(0,a.cx)


//            just dei Pair(num, subtract)

            val new = 1L * num * a.x - subtract
            total += new
            counts.addValue(a.cx,1)
            sums.addValue(a.cx,a.x.toLong())
        }
...
/*
just do a full sort
if equal vaues is sus, it is sus
 */