...
// PRZETESTOWANE, ALE TROCH POWOLNE
// mona sprbowa przyspieszy; zadanie do testowania poprawnoci:
// https://themis.lo14.wroc.pl/MIA33/PODWA

// w drzewie nie mog pojawia si duplikaty; w takim wypadku insert dziaa jak update
...
int main () {
   ...
   int uniq=0;
   REPD(i,n-1,0) {
      tr.insert(mp(-a[i],uniq++),i);
      int wyn = tr.sum_of_augs_over_all_xs_geq_than_x(mp(-a[i]+1, -INF));
//      DBG(wyn)
      w[i] = wyn - i - 1;
      if (w[i] < 0) w[i] = -1;
   }
   FOR(i,n) printf("%d ",w[i]);
