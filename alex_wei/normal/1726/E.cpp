...
void solve() {
  ...
  for(int i = 1; i <= n; i++) {
    f[i] = 0;
    add(f[i], f[i - 1]);
    if(i > 1) add(f[i], 1ll * f[i - 2] * (i - 1) % mod);
  }
  op[0] = 1;
  for(int j = 1; j <= n; j++) op[j] = 1ll * op[j - 1] * (2 * j - 1) % mod;
  int ans = 0;
  for(int i = n, c = 0; i >= 0; i -= 4, c++) {
    int coef = bin(n - 2 * c, 2 * c);
    add(ans, 1ll * ksm(2, c) * f[i] % mod * coef % mod * op[c] % mod);
  }
  cout << ans << "\n";
}
bool Med;
