...
fun main(){
    solve.cases{
        ...
        val k = getint()
        val max = 502
        val t = TrieImplicit(max)
        val W = IntArray(max)
        repeat(n){
            val s = getstr()
            val w = getint()
            val arr = s.map{it.toInt() - '0'.toInt()}.toIntArray()
            val node = t.addString(arr)
            W[node] += w
        }
//        val store = Array(max){Array(k+1){IntArray(0)}}
        fun TrieImplicit.dfs(v:Int):Array<IntArray>{
            var ret = Array(k+1){IntArray(max){huge} }
            for(i in 0 until max){
                ret[0][i] = i * W[v]
            }
            for(c in 0 until 10){
                if(next[v][c] != -1){
                    val other = dfs(next[v][c])
                    val new = Array(k+1){IntArray(max){huge} }
                    for(m in 0 until max-1){
                        for(kleft in 0..k){
                            for(kright in 0..k){
                                if(kleft + kright > k) continue
                                new[kleft+kright][m] = minOf(new[kleft+kright][m],ret[kleft][m] + other[kright][m+1])
                            }
                        }
                        for(s in 0..k){
                            if(new[s][m] >= huge){
            ...
            for(s in 0 until k){
                for(m in 0 until max-1){
                    ret[s+1][m] = minOf(ret[s][0],ret[s+1][m])
                }
            }
//            store[v] = ret
            return ret
            //  n^2
            // convolution (yes, montone up, mostly)
            // only if convex
            // invert function representations , nice trick
        }
        put(t.dfs(0).minOf { it[0] })
...
/*
fatal flaw;
tree convolution confused with +1 rank (
One of the convolution never need to upone
 */