static_assert(R"("
  ...
  for (int i : rep(n)) {
    ...
    if (!d.same(0, 2 * i) && !d.same(0, 2 * i + 1)) { d.merge(0, 2 * i); }
    dir[i] = d.same(0, 2 * i);
  }
  DUMP(dir);
  vla<basic_string<int>> g(n);
  vla deg(n, 0);
  for (auto [t, i, j] : info) {
    assert(dir[i] != dir[j]);
    if (t == 1) {
  ...
  queue<int> q;
  for (int i : rep(n).filter(LAMBDA(deg[_1] == 0))) { q.push(i); }
  vla<pair<char, int>> ans(n);
  int cur = 0;
  while (!empty(q)) {
    int i = q.front();
    q.pop();
    ans[i].first = "LR"[dir[i]];
    ans[i].second = cur++;
    for (int j : g[i]) {
      --deg[j];
      if (deg[j] == 0) { q.push(j); }
  ...
  if (cur < n) { return println("NO"); }
  println("YES");
  for (auto [c, x] : ans) { println(c, x); }
")");
