...
ll n,k,num,a[N],b[N],p[N];
bool pd[N];
set <int> s;

void solve(){
	cin>>n>>k; pd[0]=pd[n+1]=1; ll ans=1;
	for(int i=1;i<=n;i++)cin>>a[i],p[a[i]]=i,pd[i]=0,s.insert(i);
	for(int i=1;i<=k;i++)cin>>b[i];
	for(int i=k;i;i--){
		int ps=p[b[i]];
		auto it=s.lower_bound(ps),it2=s.upper_bound(ps);
		int pre=it==s.begin()?0:*--it;
		int suf=it2==s.end()?n+1:*it2;
		if(pd[pre]&&pd[suf]){
			puts("0");
			return;
		} else if(!pd[pre]&&!pd[suf])ans=ans*2%mod;
		pd[ps]=1;
	} cout<<ans<<endl;
}
