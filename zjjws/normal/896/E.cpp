...
{
    int typ,l,r,x,ans;
    inline void Read(){rin>>typ>>l>>r>>x;return;}
}b[M];
int X[M];
int Y[M];
int tail;
...
inline void Gura(int x,int y){f[x]=y;X[++tail]=x;Y[tail]=y;c[y]+=c[x];c[x]=0;return;}
inline void Work()
{
    int len=sqrt(n);
    for(int L=1,R,s,sl,sr;L<=n;L=R+1)
    {
        R=min(n,L+len-1);sl=1;sr=s=0;
        for(int i=0;i<S;i++)f[i]=i;
        for(int i=L;i<=R;i++)c[a[i]]++,sr=max(sr,a[i]);
        for(int i=1;i<=m;i++)
        {
            int l=max(L,b[i].l),r=min(R,b[i].r),x=b[i].x+s;if(l>r||x>sr||x<sl)continue;
            if(b[i].typ==1)
            {
                if(x==sr)continue;
                if(l!=L||r!=R){for(int j=l;j<=r;j++)if(find(a[j])>x)c[find(a[j])]--,c[a[j]=(find(a[j])-x+s)]++;}
                else
                {
                    if(x-sl+1>=sr-x){for(int j=x+1;j<=sr;j++)Gura(j,j-x+s);sr=x;}
                    else {for(int j=sl;j<=x;j++)Gura(j,j+x-s);sl=x+1;s=x;}
                }
            }
            else
            {
                if(x==s){b[i].ans+=c[0];continue;}
                if(l!=L||r!=R){for(int j=l;j<=r;j++)if(find(a[j])==x)b[i].ans++;}
                else b[i].ans+=c[x];
            }
        }
        for(;tail;tail--)f[X[tail]]=false,c[Y[tail]]=0;
        for(int i=L;i<=R;i++)if(c[a[i]])c[a[i]]--;
    }
    for(int i=1;i<=m;i++)if(b[i].typ==2)printf("%d\n",b[i].ans);
    return;
}
int main()
{
    ...
    double t=clock();
    rin>>n>>m;for(int i=1;i<=n;i++)rin>>a[i];for(int i=1;i<=m;i++)b[i].Read();Work();
    // printf("cost time %.2lf\n",((clock()-t)*1.0)/CLOCKS_PER_SEC);
    return 0;
}