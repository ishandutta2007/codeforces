...
ll id() { return 0; }
using seg = atcoder::lazy_segtree<ll, op, e, ll, add, add, id>;
...
int main() {
	...
	vector<ll> dist(n+1, 1ll<<60), vis(n+1), bad(n+1);
	...
	auto add = [&](int v, int val) {
		if(dist[v] <= val) return;
		dist[v] = val;
		pq.push({-val, v});
	};
	auto recalc = [&](int v, int cval) {
		if(vis[v]) return;
		bad[v]--;
		ll val = 1 + bad[v] + cval;
		add(v, val);
