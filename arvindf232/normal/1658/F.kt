...
fun main(){
    solve.cases{
        ...
        val m = getint
        val str = getstr
        val Q = IntArray(n){if(str[it] == '1') 1 else 0}.torsq()
        val counts = Q.sumQuery(0,n-1)
        val need = 1L * m * counts
        if(need % n != 0L){
            put(-1)
            return@cases
        }
        ...
        for(left in 0 until n){
            val right = left + m - 1
            var has = Q.sumQuery(left, minOf(n-1,right))
            if(right >= n){
                has += Q.sumQuery(0,right - n)
            }
            if(has == want){
                val taken = if(right >= n) 2 else 1
                put(taken)
                if(right >= n){
                    put("${1} ${right - n + 1 }")
                }
                put("${left+1} ${minOf(n-1,right) + 1 }")
