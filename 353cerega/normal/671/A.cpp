...
int main()
{
    ...
    vector<vector<ld>> d(3,vector<ld>(n));
    ld ans = 0;
    int pos = -1, pos2 = -1;
    ld mx = 0;
    ld mx2 = 0;
    for (int i=0;i<n;i++)
    {
        ...
        for (int j=0;j<3;j++)
        {
            d[j][i] = sqrt((p[i].X-st[j].X)*(p[i].X-st[j].X)+(p[i].Y-st[j].Y)*(p[i].Y-st[j].Y));
        }
        ans += d[2][i]*2;
        if (d[2][i]-d[0][i]>mx)
        {
    ...
    {
        if (i==pos) continue;
        M = max(M,mx+d[2][i]-d[1][i]);
        M = max(M,mx2+d[2][i]-d[0][i]);
    }
    cout << ans-M << endl;
}