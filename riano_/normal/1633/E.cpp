...
int main(){
    ...
    vector<Tp> ed;
    rep(i,M){
        ll u,v,c; cin >> u >> v >> c; ed.emplace_back(c,u,v);
    }

    set<ll> th; th.insert(0);
    rep(i,M){
        th.insert(get<0>(ed[i]));
        rep(j,M){
            if(j<=i) continue;
            th.insert((get<0>(ed[i])+get<0>(ed[j])+1)/2);
        }
    }
    ...
    for(ll x:th){
        vector<tuple<ll,ll,ll,ll>> ed2;
        rep(i,M){
            auto[c,u,v] = ed[i];
            if(c<=x) ed2.emplace_back(x-c,1,u,v);
            else ed2.emplace_back(c-x,-1,u,v);
        }
        sort(all(ed2));
        //main 
        unionfind uf(N+1);
    ...
    rep(i,p){
        cin >> q;
        Tp tmp = make_tuple(q,2e18,2e18);
        ll s = lower_bound(all(w),tmp) - w.begin() -1;
        auto[x,cs,pm] = w[s];
        ans ^= (cs+pm*(q-x));
    }
    rep(i,k-p){
