...
signed main(){
    ...
    while(q--){
        ...
        dp[1][0] = 1;
        dp[1][1] = 1;
        dp[1][2] = (a[0]+a[1] >= x+x ? 2 : 0);
        for(ll i = 2;i<n;i++){
            dp[i][0] = max({dp[i-1][0], dp[i-1][1], dp[i-1][2], dp[i-1][3]});
            dp[i][1] = dp[i-1][0]+1;
            dp[i][2] = (a[i]+a[i-1] >= x+x ? dp[i-1][1]+1 : 0);
            dp[i][3] = (a[i]+a[i-1] >= x+x && a[i] + a[i-1] + a[i-2] >= x+x+x ? max(dp[i-1][2], dp[i-1][3])+1 : 0);
        }
        ll res=0;
        for(ll i = 0;i<n;i++) res=max({res, dp[i][0], dp[i][1], dp[i][2], dp[i][3]});
        cout<<res<<'\n';
    }
}