...
void go(vvi& adj, vi& pre, vi& enter, vi& leave, vi& depth, int loc, int par) {
    enter[loc] = sz(pre);
    pre.pb(loc);
    for (int nbr : adj[loc]) if (nbr != par) {
        depth[nbr] = depth[loc] + 1;
        go(adj, pre, enter, leave, depth, nbr, loc);
    }
    leave[loc] = sz(pre);
}
...
int main() {
    ...
    int N, M; re(N, M); vi val(N); re(val);
    ...
    vi pre, enter(N), leave(N), depth(N);
    go(adj, pre, enter, leave, depth, 0, 0);

    binary_indexed_tree<ll> odd(N + 1), even(N + 1);
    F0R (m, M) {
        int type, x, v;
        re(type);
        if (type == 1) {
            re(x, v); --x;
            (depth[x]&1 ? odd : even).add(enter[x], v);
            (depth[x]&1 ? odd : even).add(leave[x], -v);
            (depth[x]&1 ? even : odd).add(enter[x], -v);
            (depth[x]&1 ? even : odd).add(leave[x], v);
        } else {
            re(x); --x;
            ps(val[x] +(depth[x]&1 ? odd : even).sum(enter[x] + 1));
        }
    }
