...
void solve() {
    ...
    REP(e, N - 1) {
        ...
        cin >> a >> b;
        --a, --b;
        graph.add_edge(a, b, 1), graph.add_edge(b, a, 1);
    }
    graph.solve(0);
    const auto D0 = graph.dist;

    constexpr int INF = 1 << 22;
    vector<int> ret(N + 1, INF);
    REP(r, N) {
        vector<int> max0(N + 1, -INF), max1(N + 1, -INF);
        graph.solve(r);
        REP(i, N) {
            int d0 = D0[i], d1 = graph.dist[i];
            if (d0 <= d1) {
                chmax(max0[0], d0);
            } else {
                chmax(max0[d0 - d1], d0);
        ...
        REP(i, N) chmax(max0[i + 1], max0[i]);
        IREP(i, N) chmax(max1[i], max1[i + 1] - 1);
        FOR(x, 1, N + 1) chmin(ret[x], max(max0[x], max1[x]));
    }
    FOR(x, 1, N + 1) cout << ret[x] << ' ';
    cout << '\n';
}
