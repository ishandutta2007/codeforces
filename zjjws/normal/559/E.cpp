...
int f[N][N][2];
pair<int,int>a[N];
#define L(i) (a[i].first-a[i].second)
#define R(i) (a[i].first+a[i].second)
void Read(){rin>>n;for(int i=1;i<=n;i++)rin>>a[i].first>>a[i].second;return;}
void Work()
{
    sort(a+1,a+n+1);a[0]=make_pair(-0x3f3f3f3f,0);
    for(int i=1;i<=n;i++)f[i][i][0]=f[i][i][1]=a[i].second;
    for(int i=0;i<n;i++)for(int j=0;j<=i;j++)for(int t_1=0;t_1<2;t_1++)
    {
        int l,r=-0x3f3f3f3f,x,y;
        for(int k=i+1;k<=n;k++)
        {
            // if(k==i+1)
            // {
                // f[k][k][0]=max(f[k][k][0],f[k-1][j][0]+min(a[k].second,a[k].first-a[j].first));
                // f[k][k][1]=max(f[k][k][1],f[k-1][j][0]+a[k].second);
                // int r=R(j);
                // if(a[k].first>r)f[k][k][0]=max(f[k][k][0],f[k-1][j][1]+min(a[k].second,a[k].first-r));
                // if(R(k)>r)f[k][k][1]=max(f[k][k][1],f[k-1][j][1]+min(a[k].second,R(k)-r));
            // }
            // if(R(k)>r)r=R(k),lst=k;
            // else
            // {
            //     l=min(L(k),a[lst].first);
            //     // if(k==4)printf("i:%d j:%d lst:%d \n",i,j,lst);
            //     // if(k==4&&lst==2)printf("i:%d j:%d [%d,%d] L(j):%d R(j):%d\n",i,j,l,r,L(j),R(j));
            //     if(r>a[j].first)f[k][lst][1]=max(f[k][lst][1],f[i][j][0]+r-max(l,a[j].first));
            //     if(r>R(j))f[k][lst][1]=max(f[k][lst][1],f[i][j][1]+r-max(l,R(j)));
            // }
            for(int t_2=0;t_2<2;t_2++)
            {
                int r_1=t_1?R(j):a[j].first,r_2=t_2?R(k):a[k].first;
                if(r_2>r)r=r_2,x=k,y=t_2;
                f[k][x][y]=max(f[k][x][y],f[i][j][t_1]+min(a[k].second,r_2-r_1)+r-r_2);
            }
        }
    ...
    int ans=0;
    for(int i=1;i<=n;i++)ans=max(ans,max(f[n][i][0],f[n][i][1]));printf("%d\n",ans);
    // for(int i=1;i<=n;i++)for(int j=1;j<=i;j++)for(int _=0;_<2;_++)printf("f[%d][%d][%d]:%d\n",i,j,_,f[i][j][_]);
    return;
}
int main()
{
