...
int main()
{
    ...
    forn(i, n)
    {
        ...
        forn(j, can[i].size())
        {
            can[i + 1].pb(mp(can[i][j].x, 0));
            can[i + 1].pb(mp(can[i][j].x + a[i], 1));
        }
        sort(all(can[i + 1]), cmp);
        int s = min(k + 1, unique(can[i + 1].begin(), can[i + 1].end(), cmp2) - can[i + 1].begin());
        can[i + 1].resize(s);
    }
    assert(can[n].size() > k);
    For(i, 1, k)
    {
        ...
        ford(j, n)
        {
            PII t =  *lower_bound(all(can[j + 1]), mp(cur, 0), cmp);
            if (t.y)
            {
                ans.pb(a[j]);
                cur -= a[j];
