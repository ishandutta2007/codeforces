...
int main() {
    ...
    auto para = [&](int i, int j) { return a[i] * b[j] == a[j] * b[i]; };
    auto cross = [&](int i, int j) {
        ll A = a[i] * b[j] - a[j] * b[i];
        frac x = frac(c[i] * b[j] - c[j] * b[i]) / A;
        frac y = frac(c[i] * a[j] - c[j] * a[i]) / A;
        return make_pair(x, y);
    };
    using A = array<array<int, 2>, 5>;
    auto dfs = [&](auto &&f, A &v, int i) -> void {
        if(i == n) {
            int num = 0;
            rep(j, k) if(v[j][0] != -2) num++;
            cout << "YES\n" << num << endl;
            rep(j, num) cout << ++v[j][0] << " " << ++v[j][1] << endl;
            exit(0);
        }
        rep(j, k) {
            if(v[j][0] == -2) {
            ...
            if(v[j][1] == -2) {
                if(para(v[j][0], i)) continue;
                v[j][1] = i;
                f(f, v, i + 1);
                v[j][1] = -2;
            } else {
                if(!para(v[j][0], i) and cross(v[j][0], v[j][1]) == cross(v[j][0], i)) {
                    f(f, v, i + 1);
                    break;
                }
            }
