...
map <bigint, int> mp[N<<1];
int p[N<<1][N][N], gas[N<<1], g[N<<1][N], k;
vector <bigint> v;

int find(bigint A) {
	for (int i = 0; i < N - 1 << 1; i ++) {
		if (a[i][1] > A) return -1;
		for (int j = 1; j <= gas[i]; j ++) {
			if (A == a[i][j]) return i;
		}
	}
	return -1;
...
inline bool cmp(int i, int j) {return a[k][i] < a[k][j];}

int main() {
//	freopen("d.in", "r", stdin);
	a[0][1] = bigint(2), a[1][1] = bigint(13);
	gas[0] = 1, gas[1] = 1, p[1][1][1] = 1;
	g[0][1] = 1, g[1][1] = 1;
	for (int i = 2; i < 2 * N - 2; i ++) {
		int cnt = 0;
		for (int j = 1; j <= gas[i-2]; j ++) {
			for (int k = 1; k <= gas[i-1]; k ++) if (p[i-1][j][k]) {
				B = a[i-2][j] * 12;
				int l = 0;
				for (int r = 1; r <= cnt; r ++) {
					if (a[i][r] == B) {l = r; break;}
				}
				if (!l) a[i][l = ++cnt] = B;
				p[i][k][l] += p[i-1][j][k];
				g[i][l] += p[i-1][j][k];
				B = a[i-1][k] * 13 - a[i-2][j] * 12;
				l = 0;
				for (int r = 1; r <= cnt; r ++) {
					if (a[i][r] == B) {l = r; break;}
				}
	...
	cout << "YES" << endl << 1 << endl << k + 1 << endl;

	for (int i = 1; i <= gas[k]; i ++) {
		if (a[k][i] != A) v.push_back(a[k][i]);
	}
