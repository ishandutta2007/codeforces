...
void main_() {
	INT(sx, sy, gx, gy, tx, ty);
	INT(n);
	V<> x(n), y(n);
	auto dist = [&](int a, int b, int c, int d) {
		return sqrt(ld(a - c) * (a - c) + ld(b - d) * (b - d));
	};
	V<ld> d(n), ds(n), dg(n);
	rep(i, n) {
		cin >> x[i] >> y[i];
		d[i] = dist(x[i], y[i], tx, ty);
		ds[i] = dist(x[i], y[i], sx, sy);
		dg[i] = dist(x[i], y[i], gx, gy);
	}
	ld res = 0;
	rep(i, n) {
		res += d[i] * 2;
	}
	V<pair<ld, int>> as(n), ag(n);
	rep(i, n) {
		as[i] = {ds[i] - d[i], i};
		ag[i] = {dg[i] - d[i], i};
	}
	SORT(as);
	SORT(ag);
	ld ans = res + as[0].first;
	chmin(ans, res + ag[0].first);
	rep(i, min(2, n)) {
		rep(j, min(2, n)) {
			if(as[i].second == ag[j].second) continue;
			chmin(ans, res + as[i].first + ag[j].first);
		}
	}
	print(ans);
}