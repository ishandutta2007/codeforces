...
void sub(pii &a,const pii &b) {
	sub(a.fi,b.fi,P);
	sub(a.se,b.se,P+2);
}

void INS(int x) {
	for(int c=N-x;--c>=0;) add(dp[c+x],dp[c]);
}
void DEL(int x) {
	for(int c=0;c+x<N;c++) sub(dp[c+x],dp[c]);
}
...
int32_t main() { // CURSOR START
	...
	for(int i=1;i<=n;i++) {
		...
		for(int j=1;j<=n;j++) if(dp[j].fi && dp[j].se) Z[j]=1;
		for(int x:del[i]) DEL(x);
	}
	vector<int> zz;
	for(int i=1;i<=n;i++) if(Z[i]) zz.pb(i);
	printf("%d\n",sz(zz));
	for(int i=0;i<sz(zz);i++) printf("%d%c",zz[i]," \n"[i==sz(zz)-1]);
}