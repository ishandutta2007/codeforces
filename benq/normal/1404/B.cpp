...
template<int SZ> struct TreeDiameter {
	int N, par[SZ], dist[SZ], diaLen, ans[SZ] ;
...
void solve() {
	re(n,a,b,DA,DB); a--,b--;
	// if DA >= DB
	// or there exists vert where every dist away from it is <= DA
	F0R(i,n) TT.adj[i].clear();
	F0R(i,n-1) {
		int a,b; re(a,b); a--,b--;
		TT.ae(a,b);
	}
	TT.init(n);
	int big = -MOD;
	F0R(i,n) ckmax(big,TT.ans[i]);
	ckmin(DB,big);
	// dbg("OOPS",big);
	TT.genDist(a);
	int initialDist = TT.dist[b];
	dbg("HA",a,b,initialDist);
	// F0R(i,n) dbg(i,TT.ans[i]);
	if (DB > 2*DA && initialDist > DA) ps("Bob");
	else ps("Alice");
}
