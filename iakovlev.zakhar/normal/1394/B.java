...
public class B {
	...
	public class Conditions implements Comparable<Conditions> {
		...
		public void zip() {
			...
			for (int i = 0; i < number.length; i++) {
				Pair p = new Pair(number[i], index[i]);
				if (set.contains(p)) {
					can[number[i]][index[i]][number[i]][index[i]] = false;
				}
				set.add(p);
			}
			...
			for (int i = 0; i < sz; i++) {
				for (int j = i + 1; j < sz; j++) {
					can[number[i]][index[i]][number[j]][index[j]] = false;
					can[number[j]][index[j]][number[i]][index[i]] = false;
				}
			}
		}

		@Override
		public int compareTo(Conditions o) {
			return sz - o.sz;
		}
	}

	public void rec(int sz) {
		if (sz == k) {
			ans++;
			return;
		}
	...
	public boolean check() {
		f: for (Conditions cond : c) {
			boolean flag = false;
			for (int i = 0; i < cond.sz; i++) {
				if (a[cond.number[i]] == cond.index[i]) {
					if (flag) {
						return false;
					}
					flag = true;
	...
	public void solve() {
		...
		c = new Conditions[n];
		int[] sz = new int[n];
		ArrayList<Edge>[] to = new ArrayList[n];
		...
		for (int i = 0; i < m; i++) {
			int u = in.nextInt() - 1;
			int v = in.nextInt() - 1;
			to[u].add(new Edge(v, in.nextInt()));
			sz[v]++;
		}
		for (int i = 0; i < to.length; i++) {
			c[i] = new Conditions(sz[i]);
			if (sz[i] == 0) {
				out.print(0);
				return;
			}
			Collections.sort(to[i]);
		}
		for (int i = 0; i < to.length; i++) {
			for (int j = 0; j < to[i].size(); j++) {
				c[to[i].get(j).to].add(to[i].size() - 1, j);
			}
		}
