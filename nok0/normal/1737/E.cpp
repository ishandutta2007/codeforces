...
V<mint> g(int n) {
	...
	rep(bit, 1 << n) {
		...
		int sv = n;
		bool db = dir == V<>{-1, -1, 1, -1, -1};
		while(count(all(pl), -1) != n - 1) {
			rep(i, n) {
				if(pl[i] == -1) continue;
				pl[i] += dir[i];
				if(pl[i] == 0) dir[i] = 1;
				if(pl[i] == (n + 2) * 2) dir[i] = -1;
			}
			rep(i, n) {
				if(pl[i] == -1) continue;
				rep(j, i + 1, n) {
...
void main_() {
	// INT(n);
	// mint al = mint(2).pow(n);
	// vector<mint> v(n);
	// rep(i, n) {
	// }
	// reps(i, 20) if(i % 2 == 0) f(i);{
	...
	vector<mint> res(n + 1);
	reps(i, n) {
		res[i] = dp[n - i] * mint(2).pow(i / 2) * 2;
	}
	mint inv = mint(2).pow(n);
	inv = 1 / inv;
	foa(v, res) v *= inv;
	res.erase(res.begin());
	printel(res);
