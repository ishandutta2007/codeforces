...
int main() {
    ...
    FOR(i, 2, N + 1) {
        vector<mint> v(i, 1);
        ten_dist.push_back(nttconv(v, ten_dist.back()));
    }
    // dbg(ten_dist);
    ...
    REP(same_length, N - 1) {
        ...
        REP(a, K) FOR(b, a + 1, K) {
            mint cum = 0;
            IREP(s, ten_dist[K - 1].size()) {
                cum += ten_dist[K - 1][s];
                if (s >= b - a + 1) ans += cum * ten_dist[K - 1][s - (b - a + 1)];
                // REP(t, ten_dist[K - 1].size()) {
                //     if (a + s > b + t) ans += ten_dist[K - 1][s] * ten_dist[K - 1][t];
                // }
            }
            // IREP(h, ten_dist[K - 1].size()) {
            //     if (h + b - a < ten_dist[K - 1].size()) {
            //         ans += cum * ten_dist[K - 1][h + (b - a)];
            //     }
            //     cum += ten_dist[K - 1][h];
            // }
        }
