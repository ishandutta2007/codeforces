...
class HZSShufflesCards {
public:
    void solve(istream& cin, ostream& cout) {
        int N, M; cin >> N >> M;
        auto F = FF::fact(N+M+2);
        auto I = FF::invfact(N+M+2);
        FF pref = 0, xx = 0, yy = 0, ans = 0;
        for (int i = 1; i <= N; ++i) {
            xx += F[M+i-2] * I[i-1];
            FF right = (xx * (i+1) - yy + F[M+i-1] * I[i]) * (M+N+1) * I[M+i+1] * F[M+i] + pref;
            ans = M * right * F[i-1] * I[i];
            pref += ans;
            yy += F[M+i-1] * I[i-1];
        }
        cout << ans * F[N] * I[M + N - 1];
    }
};
