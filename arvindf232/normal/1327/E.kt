...
fun main(){
    ...
    solve.cases{
        val n = getint()
        val power10 = IntArray(n+3){1}
        for(i in 1 until power10.size){
            power10[i] = power10[i-1] modM 10
        }
        for(i in 1..n){
            val total = n-i+ 1
            if(total == 1){
                ...
                val ext = minOf(total, 2)
                val int = total - ext
                val a1 = ext modM 90 modM power10[n-i-1]
                val a2 = if(int > 0) int modM 810 modM power10[n-i-2] else 0
                put(a1 modPlus a2 )
            }
        }
    }
