...
int main() {
    ...
    vector<int> B(N);
    cin >> B;
    vector dp(N + 3, vector<mint>(N + 3));
    vector cache(N + 3, vector<mint>(N + 3));
    dp[0][0] = 1;
    int lo = 0, hi = 0;
    ...
    REP(i, N - 1) chmax(los[i + 1], los[i]);
    IREP(i, N - 1) chmin(his[i], his[i + 1]);
    ...
    REP(i, N) {
        ...
        FOR(x, lo, nxthi + 1) {
            IREP(y, N + 1) {
                const mint cur = dp[x][y];
                dp[x][y] = 0;
                if (x + 1 <= nxthi) {
                    cache[x + 1][y] += cur;
                    if (y) cache[x + 1][y - 1] += cache[x][y] * y;
                }

                if (x >= nxtlo and x <= nxthi) {
                    if (cur) {
                        dp[x][y + 1] += cur;
                        dp[x][y] += cur * (x + y);
                    }
    ...
    mint ret = 0;
    REP(mex, dp.size()) {
        if (mex < lo or mex > hi) continue;
        REP(i, dp[mex].size()) {
            if (N >= mex + i) ret += dp[mex][i] * mint(N - mex).fac() * mint(N - mex - i).facinv();
        }
    }
    cout << ret << '\n';
}