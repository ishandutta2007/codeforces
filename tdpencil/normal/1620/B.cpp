...
void solve(int test_case = 0) {
    ...
    int x; cin >> x;
    vi a(x); generate(all(a), nxt);
    int y; cin >> y;
    vi b(y); generate(all(b), nxt);

    cin >> x;
    vi c(x); generate(all(c), nxt);
    cin >> y;
    vi d(y); generate(all(d), nxt);
    ans = max(ans, (a[sz(a)-1] - a[0])*1LL*h);
    ans = max(ans, (b[sz(b)-1] - b[0])*1LL*h);
    ans = max(ans, (c[sz(c)-1] - c[0])*1LL*w);
    ans = max(ans, (d[sz(d)-1] - d[0])*1LL*w);
    cout << ans << "\n";
}
