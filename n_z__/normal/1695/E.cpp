...
bitset<601001>v;
vector<int>w(2*n+100);
int ans[n*2+100][10];
char ans1[n*2+100][10],ans2[n*2+100][10];;
me(ans,0);
me(ans1,0);
me(ans2,0);
int qq=0;
...
{
    int qwq=0;
    function<void(int)>dfs=[&](int now){qwq++;w[qwq]=now;if(v[now])return;v[now]=1;for(auto q:e[now])if(!be[q.second]){be[q.second]=1;dfs(q.first);qwq++;w[qwq]=now;}};
    dfs(x);
    qwq--;
    qwq/=2;
    if(qwq==1)ex(-1);
    for(int y=1;y<=qwq;y++)
    ans[qq+y][1]=w[y],ans[qq+qwq-y+1][2]=w[y+qwq];
    for(int y=1;y<=qwq;y++)
    {
        if(qwq%2==1&&y==qwq)ans1[qq+y][1]='U',ans1[qq+y][2]='D';
        else if(y%2==1)ans1[qq+y][1]=ans1[qq+y][2]='L';
        else ans1[qq+y][1]=ans1[qq+y][2]='R';
        if(y==1||(y==qwq&&qwq%2==0))ans2[qq+y][1]='U',ans2[qq+y][2]='D';
        else if(y%2==1)ans2[qq+y][1]=ans2[qq+y][2]='R';
        else ans2[qq+y][1]=ans2[qq+y][2]='L';
    }
    qq+=qwq;
}
cout<<2<<' '<<n<<endl<<endl;
for(int x=1;x<=n;x++)
cout<<ans[x][1]<<' ';cout<<endl;
for(int x=1;x<=n;x++)
cout<<ans[x][2]<<' ';cout<<endl;
cout<<endl;
for(int x=1;x<=n;x++)
cout<<ans1[x][1];cout<<endl;
for(int x=1;x<=n;x++)
cout<<ans1[x][2];cout<<endl;
cout<<endl;
for(int x=1;x<=n;x++)
cout<<ans2[x][1];cout<<endl;
for(int x=1;x<=n;x++)
cout<<ans2[x][2];cout<<endl;
cout<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
