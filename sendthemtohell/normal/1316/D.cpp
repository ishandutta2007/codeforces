...
void dfs(int x, int y) {
	used[x][y] = true;
	for (int d = 0; d < 4; ++d) {
		if (ok(x + dx[d], y + dy[d]) && !used[x + dx[d]][y + dy[d]] && tab[x + dx[d]][y + dy[d]] == tab[x][y]) {
			ans[x + dx[d]][y + dy[d]] = mv[(d + 2) % 4];
			dfs(x + dx[d], y + dy[d]);
		}
	}
}
...
inline void solve() {
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			cin >> tab[i][j];
			if (tab[i][j].x != -1) --tab[i][j].x;
			if (tab[i][j].y != -1) --tab[i][j].y;
		}
	}
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			if (tab[i][j] == make_pair(i, j)) {
				ans[i][j] = 'X';
				dfs(i, j);
			}
		}
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			if (!used[i][j]) {
				if (tab[i][j] != make_pair(-1, -1)) {
					cout << "INVALID\n";
					return;
				}
				bool kk = false;
				for (int d = 0; d < 4; ++d) {
					if (ok(i + dx[d], j + dy[d]) && tab[i + dx[d]][j + dy[d]] == make_pair(-1, -1)) {
						kk = true;
						ans[i][j] = mv[d];
						break;
					}
