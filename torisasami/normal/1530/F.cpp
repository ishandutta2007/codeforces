...
int main(){
	...
	mint b[n][n];
	rep(i, n) rep(j, n) b[i][j] = (mint)a[i][j] / 10000;
	mint ib[n][n];
	rep(i, n) rep(j, n) ib[i][j] = b[i][j].inverse();
	mint s[n][1 << n];
	rep(i,n) rep(j,1<<n){
		if(j == 0){
			s[i][j] = 1;
	...
	rep(t,1<<n){
		...
		rep(d, 4){
			...
			rep(i,n){
				...
				if(d & 2 && !(t >> (n - 1 - i) & 1) && (i != n - 1 - i || !(d & 1))){
					val *= ib[i][n - 1 - i];
					res *= b[i][n - 1 - i];
				}
				res *= (mint)1 - val;
			}
			if((__builtin_popcount(t) + __builtin_popcount(d)) % 2)
				ans += res;
			else
				ans -= res;
		}
