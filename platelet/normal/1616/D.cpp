...
void solve() {
    ...
    deque<int> q1, q2;
    rep(i, 1, n) {
        while(SZ(q1) && a[q1.front() - 1] > a[i])
            if(j++ == q1.front()) q1.pop_front();
        while(SZ(q1) && a[q1.back() - 1] < a[i - 1]) q1.pop_back();
        q1.pb(i);
        while(SZ(q2) && g[q2.back()] < g[i - 1]) q2.pop_back();
        q2.pb(i - 1);
        while(q2.front() < j - 1) q2.pop_front();
        // printf("[%d, %d]\n", j - 1, i);
        g[i] = f[i - 1] - 1, f[i] = max(g[i], g[q2.front()]);
        // printf("g[%d] = %d, f[%d] = %d\n", i, g[i], i, f[i]);
    }
    cout << f[n] + n << '\n';
}
