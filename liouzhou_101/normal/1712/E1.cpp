...
void init()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		while (!d[i].empty() && d[i].back() >= i) d[i].pop_back();
		while (!e[i].empty() && e[i].back() >= i) e[i].pop_back();
	}
}
...
void solve()
{
	...
	{
		// lcm(i,j,k)=k
		int st = lower_bound(d[k].begin(), d[k].end(), l) - d[k].begin();
		int ed = upper_bound(d[k].begin(), d[k].end(), r) - d[k].begin();
		res += ll(ed - st) * (ed - st - 1) / 2;

		// lcm(i,j,k)=2k and lcm(j,k)=k
		for (int j = 0, st = (int)e[k].size() - 1, ed = 0; j < d[k].size(); ++j)
		{
			if (d[k][j] >= l)
			{
				while (st >= 0 && e[k][st] >= l && d[k][j] + e[k][st] > k) st -= 1;
				while (ed < e[k].size() && e[k][ed] < d[k][j]) ed += 1;
				res += max(ed - st - 1, 0);
			}
		}

		// lcm(i,j,k)=2k and lcm(j,k)=2k
		for (int j = 0, st = (int)d[2 * k].size() - 1, ed = 0; j < e[k].size(); ++j)
		{
			if (e[k][j] >= l)
			{
				while (st >= 0 && d[2 * k][st] >= l && e[k][j] + d[2 * k][st] > k) st -= 1;
				while (ed < d[2 * k].size() && d[2 * k][ed] < e[k][j]) ed += 1;
				res += max(ed - st - 1, 0);
			}
		}
	}
	res = ll(r - l + 1) * (r - l) * (r - l - 1) / 6 - res;
	writeln(res);
}
