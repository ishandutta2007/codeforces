...
//	cout<<pv1[2]<<endl;
	...
	for (int i=0;i<q;i++){
		...
		L--;
		R--;
		int d1=cnt1[r][2]+cnt1[r][1];
		if (l) d1-=cnt1[l-1][2]+cnt1[l-1][1];
		int d2=cnt2[R][2]+cnt2[R][1];
		if (L){
			d2-=cnt2[L-1][2]+cnt2[L-1][1];
		}
		if ((abs(d1-d2))%2!=0 || d1>d2){
			ans[i]='0';
		}
		else{
			int f1=pv1[r],f2=pv2[R];
			f1=max(f1,l-1);
			f2=max(f2,L-1);
			//cout<<f1<<f2<<endl;
			int e1=r-f1,e2=R-f2;
