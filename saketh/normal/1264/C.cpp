...
int main() {
    ...
    vmn prod = suffixes(by, mn(1), [](int v, mn p) { return p * 100 * mn::small_inv(v); });
    vmn prod_inv = suffixes(by, mn(1), [](int v, mn p) { return p * v * mn::small_inv(100); });
    vmn prod_sum = prefixes(prod, mn(0), plus<mn>());

    auto beat = [&](int i, int j) {
        return (prod_sum[j] - prod_sum[i]) * prod_inv[j];
    };

    auto pp = [](pii a, pii b) { return pii{min(a.f, b.f), max(a.s, b.s)}; };
    segment_tree ckpt(N, pii{N, -1}, pp);
    ...
    F0R (q, Q) {
        int v; re(v); --v;
        int bef = ckpt(0, v).s;
        int aft = ckpt(v+1, N).f;
        if (is[v]) {
            ans -= beat(bef, v);
            ans -= beat(v, aft);
            ans += beat(bef, aft);
            ckpt.replace(v, {INT_MAX, INT_MIN});
            is[v] = 0;
        } else {
            ans -= beat(bef, aft);
            ans += beat(bef, v);
