...
public class Main
{
	...
	{
		int n=ni(),a=ni(),b=ni(),k=ni();
		long[]buil=new long[n+2];
		buil[a]=1;
		if(Math.abs(a-b)<=1)
		{
			pr(0);
		...
		long[]predp=new long[n+2];
		while(k-->0)
		{
			for(int i=1; i<=n; i++)
			{
				int dif=Math.abs(b-i);
				predp[Math.max(1,i-dif+1)]+=buil[i];
				predp[i]-=buil[i];
				predp[i+1]+=buil[i];
				predp[Math.min(n+1,i+dif)]-=buil[i];
			}
			for(int i=1; i<=n; i++)
				predp[i]=(predp[i]+predp[i-1])%mod;
			for(int i=1; i<=n; i++)
				buil[i]=predp[i];
			Arrays.fill(predp, 0);
		}
