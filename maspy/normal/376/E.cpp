...
void solve() {
  ...
  pi S = {-1, -1};
  FOR(x, H) FOR(y, W) if (G[x][y] == 'S') S = {x, y};
  ...
  ll dx[] = {1, 0, -1, 0};
  ll dy[] = {0, 1, 0, -1};
  vvv(ll, dist, H, W, 1 << N, INF);
  using T = tuple<ll, ll, ll>;
  deque<T> que;
  dist[S.fi][S.se][0] = 0;
  que.eb(S.fi, S.se, 0);
  while (!que.empty()) {
    auto [x, y, b] = que.front();
    que.pop_front();
    ...
    FOR(d, 4) {
      ...
      FOR(i, N) {
        ...
        if (dx[d] != 0) continue;
        if (nx >= px) continue;
        if (dy[d] == 1 && ny == py) nb ^= 1 << i;
        if (dy[d] == -1 && y == py) nb ^= 1 << i;
      }
      if (chmin(dist[nx][ny][nb], dist[x][y][b] + 1)) que.eb(nx, ny, nb);
    }
  }

  ll ANS = 0;
  FOR(b, 1 << N) {
    ll x = -dist[S.fi][S.se][b];
    FOR(i, N) if (b & 1 << i) {
      x += A[i];
      chmax(x, -INF);
    }
