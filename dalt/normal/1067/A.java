...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            arrayIndex = new ArrayIndex(200, n);
            f = new int[200 * n];
            g = new int[200 * n];
            Arrays.fill(f, -1);
            Arrays.fill(g, -1);
            ...
            for (int i = 0; i < 200; i++) {
                int index = arrayIndex.indexOf(i, 0);
                g[index] = 0;
                f[index] = i == data[0] || data[0] == -1 ? 1 : 0;
                if (i > 0) {
                    f[index] = modular.plus(f[index], f[arrayIndex.indexOf(i - 1, 0)]);
                }
            }
        ...
        public int g(int i, int j) {
            ...
            int index = arrayIndex.indexOf(j, i);
            if (g[index] == -1) {
                if (data[i] != -1 && data[i] != j) {
                    g[index] = g(i, j - 1);
                    return g[index];
                }
                g[index] = modular.plus(f(i - 1, j), -f(i - 1, j - 1));
                g[index] = modular.plus(g[index], g(i - 1, 199));
                g[index] = modular.plus(g[index], -g(i - 1, j - 1));
                g[index] = modular.plus(g[index], g(i, j - 1));
            }
            return g[index];
        }
        ...
        public int f(int i, int j) {
            ...
            if (f[index] == -1) {
                ...
                f[index] = modular.plus(f(i - 1, j - 1), g(i - 1, j - 1));
                f[index] = modular.plus(f[index], f(i, j - 1));
            }
            return f[index];
        }
    }
