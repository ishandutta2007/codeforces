...
int main() {
    ...
    for (string &w : words) {
        H w_hash;
        int overlap = 0;
        for (int o = 1; o <= min(sz(w), sz(ans)); o++) {
            w_hash = concatenate(w_hash, H(w[o - 1]));
            if (w_hash == H::get_substring(prefixes, sz(ans) - o, sz(ans)))
                overlap = o;
        }
        for (auto c = w.begin() + overlap; c != w.end(); c++) {
            ans.push_back(*c);
            prefixes.push_back(concatenate(prefixes.back(), H(*c)));
        }
    }
