...
class Solver {
    ...
    char ask(IList<int> a, IList<int> b) {
    ...
    int solve(IList<int> cand, IList<int> heavy) {
        Assert(cand.Count > 0);
        ...
        if (ask(fir, heavy.Take(fir.Length).ToList()) == '<') {
            return solve(fir, heavy);
        }
        else {
            return solve(sec, heavy);
        }
    }
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            {
                var c = ask(new int[]{ maxid }, new int[]{ seq[i] });
                if (c == '<') {
                    maxid = seq[i];
                }
            }
            ...
            while (true) {
                ...
                while (idx < n && lis.Count < heavy.Count) {
                    if (idx != maxid) {
                        lis.Add(idx);
                    }
                    ++idx;
                }
                var c = ask(lis, heavy.Take(lis.Count).ToList());
                Assert(c != '>');
                if (c == '=') {
