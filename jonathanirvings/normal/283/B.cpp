...
LL jahja(int now)
{
	...
	if (now + data[now] > n) 
	{
		return dp[now] = data[now];
	}
	if (now + data[now] - data[now + data[now]] <= 0)
	{
		LL ret = data[now] + data[now + data[now]];
		return dp[now] = ret;
	} else
	{
		LL ret = data[now] + data[now + data[now]];
		ret += jahja(now + data[now] - data[now + data[now]]);
		return dp[now] = ret;
	}
	//return ret;
}
...
int main()
{
	...
	sudah[1] = 1;
	scanf("%d",&n);
	FORN(i,2,n) scanf("%d",&data[i]);
	FORN(i,1,n-1)
	{
		LL res = i + data[1 + i] + jahja(i + 1 - data[1 + i]);
		//VALUE(i + 1 - data[1 + i]);
		if (res < 0) puts("-1"); else
		printf("%I64d\n",res);
	}
	return 0;
}