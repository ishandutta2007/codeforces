#line 1 "codeforces-workspace\\contest\\1610\\e\\e.cpp"
...
#line 13 "codeforces-workspace\\contest\\1610\\e\\e.cpp"
...
void main() {
  ...
  for (auto [i, e] : enumerate(a)) {
    auto len = 0;
    auto pos = begin(a) + i + 1;
    if (pos != end(a) and *pos == e) continue;
    while (pos != end(a)) {
      ++len;
      pos = lower_bound(pos, end(a), *pos * 2 - e);
    }
    for (auto j : rrange(1 + i)) {
      if (a[j] != e) break;
      ++len;
    }
    chgr(ans, len);
