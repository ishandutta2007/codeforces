...
int solve() {
    ...
    for (int i = 1; i < n; ++i) {
        for (int c = 1; c <= i * 2; ++c) {
            addmodfast(dp[i + 1][c + 2][1], dp[i][c][1], fftmod);
            addmodfast(dp[i + 1][c][1], dp[i][c][1], fftmod);
            addmodfast(dp[i + 1][c][0], dp[i][c][1], fftmod);
            addmodfast(dp[i + 1][c][0], dp[i][c][1], fftmod);
            addmodfast(dp[i + 1][c + 1][1], dp[i][c][0], fftmod);
            addmodfast(dp[i + 1][c + 1][1], dp[i][c][0], fftmod);
            addmodfast(dp[i + 1][c + 1][0], dp[i][c][0], fftmod);
            addmodfast(dp[i + 1][c][0], dp[i][c][0], fftmod);
        }
    }
    cout << (dp[n][k][0] + dp[n][k][1]) % fftmod << endl;
    return 0;
}