...
int n,m;
ll a[MAXN], b[MAXN];
ll sum[MAXN], suma[MAXN];
ll minsum[MAXN];
ll x;

// OOOOOXXXXX
ll Binsearch( ll asum ){
	ll l = 0, r = m, ans = 0, mid;
	while( l <= r ){
		mid = l+r >> 1;
		if( minsum[mid]*asum <= x ){
			Mymax(ans,mid);
			l = mid+1;
		}else{
			r = mid-1;
...
int main(){
	...
	For(i,m) sum[i] = sum[i-1] + b[i];
	For(len,m)
		Forx(st,1,m-len+1){
			int ed = st+len-1;
			Mymin(minsum[len],sum[ed]-sum[st-1]);
		}
	//Print(minsum,1,m);
	Read(x);
	...
	Forx(ast,1,n)
		Forx(alen,1,n){
			int aed = ast+alen-1;
			if( aed > n ) break;
			ll blen = Binsearch(suma[aed]-suma[ast-1]);
			ll tans = alen*blen;
			//cout << ast << " " << alen << " " << blen << "  " << tans << endl;
			Mymax(ans,tans);
		}
