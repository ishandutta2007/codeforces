...
void solve() {
  ...
  const ll INF = 1LL << 60;
  vv(T, DP0, N, N, T({INF, -1, -1}));
  vv(T, DP1, N, N, T({INF, -1, -1}));
  ...
  while (!que.empty()) {
    ...
    if (t == 0) {
      FORIN(e, G[x]) {
        ll xx = e.to;
        if (chmin(DP1[xx][y].dist, DP0[x][y].dist + 1)) {
          DP1[xx][y].px = x;
          DP1[xx][y].py = y;
          que.push(mt(1, xx, y));
        }
      ...
      FORIN(e, G[y]) {
        ll yy = e.to;
        if (x == yy) continue;
        if (chmin(DP0[x][yy].dist, DP1[x][y].dist + 1)) {
          DP0[x][yy].px = x;
          DP0[x][yy].py = y;
          que.push(mt(0, x, yy));
        }
  ...
  if (DP0[N - 1][0].dist == INF) {
    print(-1);
    return;
  }
  vi ANS0 = {N - 1}, ANS1 = {0};
  while (1) {
    ll x = ANS0.back(), y = ANS1.back();
    ll px, py;
    if (x == 0 && y == N - 1) break;
    px = DP0[x][y].px, py = DP0[x][y].py;
    x = px, y = py;
    px = DP1[x][y].px, py = DP1[x][y].py;
    x = px, y = py;
    ANS0.eb(x);
    ANS1.eb(y);
  }
