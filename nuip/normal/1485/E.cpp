...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		out(par,dep,1);
		vv<int> deps(n);
		rep(v,n) deps[dep[v]].eb(v);
		rep(d,n) sort(all(deps[d]),[&](cauto &v,cauto &w){ return as[v]<as[w];});
		vector<ll> dp(n);
		vector<ll> base(n);
		const ll INF=1e16;
		rrep(d,n){
			auto &vs=deps[d];
			for(cauto v:vs){
				MX(dp[v],base[v]+as[vs.back()]-as[v]);
				MX(dp[v],base[v]+as[v]-as[vs[0]]);
			}
			{
				ll mx=-INF;
				for(cauto v:vs){
			...
			for(cauto v:vs) MX(base[par[v]],dp[v]);
			for(cauto v:vs) out(d,v+1,base[v],dp[v],1);
		}
		cout<<dp[0] NL;
	}
  return 0;
