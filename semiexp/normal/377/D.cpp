...
struct segtree
{
	...
	void adjust(int p)
	{
		if(p >= N) mxv[p] = ofs[p];
			...
			if(mxv[p*2] < mxv[p*2+1]) rg[p] = rg[p*2+1];
			else rg[p] = rg[p*2];
...
int main()
{
	...
	for(int i=0;i<act.size();i++){
		...
		if(act[i].second.first == 0){
			...
			ret = max(ret, make_pair(seg.query(), make_pair(L[p], seg.right())));
		}else{
			seg.add(V[p], 300001, -1);
			seg.add(R[p]+1, 300001, 1);
		}
	...
	vector<int> ans;
	for(int i=0;i<N;i++) if(L[i] <= lf && rg <= R[i] && lf <= V[i] && V[i] <= rg) ans.push_back(i+1);
