...
basic_string<int> g[50005];
int jc[50005]={1},tot,n,m,col[50005],tp[50005],rt,ans=1;//0p 1q
void End(){
	puts("0"),exit(0);
}
void dfs(int x){
...
void dfs3(int x){
	if(x<=n)return ;
	if(!tp[x])ans=1ll*ans*jc[g[x].size()]%mod;
	else ans=2*ans%mod;
	for(int y:g[x])dfs3(y);
}
int main(){
	cin>>n>>m,tot=rt=n+1,tp[tot]=0;
	for(int i=1;i<=n;i++)g[n+1]+=i,jc[i]=1ll*jc[i-1]*i%mod;
	for(int i=1,k,x;i<=m;i++){
		for(int j=1;j<=n;j++)col[j]=1;
		cin>>k;
		while(k--)cin>>x,col[x]=2;
		dfs(rt),dfs2(rt,0);
	}
	dfs3(rt),cout<<ans<<'\n';
}