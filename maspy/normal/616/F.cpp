...
void solve() {
  ...
  vi RID(N);
  FOR(i, N) {
    if (i) LID[i] = RID[i - 1] + 1;
    RID[i] = LID[i] + S[i].size();
  }
  ...
  // 
  FOR(i, T.size()) {
    if (i > 0 && LCP[i - 1] == LEN[i]) continue;
    if (i < LCP.size() && LCP[i] == LEN[i]) continue;
    chmax(ANS, LEN[i] * COST[i]);
  }

  UnionFind uf(T.size());

  vvc<ll> sz_to_i(MAX(LCP) + 1);
  FOR(i, LCP.size()) sz_to_i[LCP[i]].eb(i);
  FOR_R(sz, MAX(LCP) + 1) {
    if (sz == 0) break;
    vi I;
    FORIN(i, sz_to_i[sz]) {
      auto a = uf[i], b = uf[i + 1];
      uf.merge(a, b);
      if (uf[a] == a) {
        COST[a] += COST[b];
        COST[b] = 0;
