...
struct segmentTree {
    ...
    segmentTree(int n) { t.resize(4 * n + 3); }
...
int main() {
    ...
    vector<Event> ev;
    for (int i = 0; i < n; ++i) {
        ev.emplace_back(segs[i].l, -2, i);
        ev.emplace_back(segs[i].r, 2, i);
    }
    for (int i = 0; i < m; ++i) {
        ev.emplace_back(tvs[i].l, -1, i);
        ev.emplace_back(tvs[i].r, 1, i);
    }
    ...
    for (int i = 0; i < ev.size(); ++i) {
        ...
        if (ev[i].type == -2) {
            ...
            if (best.first != 0) {
                int64 curRes =
                    1LL * (xx[segs[id].r] - xx[segs[id].l]) * best.first;
                if (curRes > res) {
                    res = curRes;
                    resx = id + 1;
                    resy = best.second + 1;
            ...
            openR.emplace(segs[id].r, id);
            openL.emplace(segs[id].l, id);
        } else if (ev[i].type == 2) {
            ...
            if (!openR.empty()) {
                ...
                int64 curRes = 1LL * (cr - cl) * tvs[id].cost;
                if (curRes > res) {
                    res = curRes;
                    resx = segId + 1;
                    resy = id + 1;
