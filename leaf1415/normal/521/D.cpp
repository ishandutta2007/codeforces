...
struct Frac{
	ll a, b, z;
	Frac(){Frac(0, 1);}
	Frac(ll a, ll b){this->a = a, this->b = b;}
	bool operator<(const Frac &ope)const{
		if(exceed(b, ope.a, 1e18)) return true;
		if(exceed(a, ope.b, 1e18)) return false;
		return a*ope.b < b*ope.a;
	}
	bool operator>(const Frac &ope)const{return ope < *this;}
};
ostream& operator <<(ostream &os, Frac &ope){return os << P(ope.a, ope.b);}
ostream& operator <<(ostream &os, const Frac &ope){return os << P(ope.a, ope.b);}

ll k, n, m;
ll a[100005], t[100005];
P mx[100005];
vector<P> vec[100005], mvec;

typedef pair<Frac, ll> Q;
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> t[i] >> p >> b;
		if(t[i] == 1) chmax(mx[p], P(b, i));
		if(t[i] == 2) vec[p].pb(P(b, i));
		if(t[i] == 3) mvec.pb(P(b, i));
	}
	rep(i, 1, k) if(mx[i].fst > a[i]) vec[i].pb(P(mx[i].fst - a[i], mx[i].snd));
	rep(i, 1, k) sort(all(vec[i])), reverse(all(vec[i]));
	
	vector<Q> qvec;
	rep(i, 1, k){
		ll pre = a[i];
		for(auto p : vec[i]){
			qvec.pb(Q(Frac(p.fst, pre), p.snd));
			pre += p.fst;
		}
	}
	for(auto p : mvec) qvec.pb(Q(Frac(p.fst-1, 1), p.snd));
	sort(all(qvec)), reverse(all(qvec));
	...
	ans[0].insert(ans[0].end(), all(ans[1]));
	ans[0].insert(ans[0].end(), all(ans[2]));
