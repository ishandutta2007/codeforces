...
int main(){
	...
	cauto g=readD(n,m);
	vv<int> gi(n);
	rep(v,n)for(cauto &u:g[v]) gi[u].eb(v);
	vv<int> vs(m+1);
	vector<int> ds(n,MOD);
	vector<int> sgn(n,-1);
	vector<int> okc(n);
	...
	rep(d,m){
		vector<int> nes;
		for(cauto &u:vs[d])if(ds[u]==d)for(cauto &v:gi[u]){
					++okc[v];
					if(MX(sgn[v],d)) nes.eb(v);
				}
		for(cauto &v:nes)if(cauto nd=d+1+(int)g[v].size()-okc[v]; MN(ds[v],nd)) vs[nd].eb(v);
		out(nes,1);
	}
	cout<<ds[0] NL;
	return 0;
