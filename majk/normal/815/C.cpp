...
public:
	...
	ui size(int u) {
		ui r = 1;
		for (int v:E[u]) r += size(v);
		return S[u] = r;
	}

	void shrink(vector<int> &V) {
		ui ns = V.size();
		while (ns > 1 && V[ns-1] > B) --ns;
		if (ns != V.size()) V.resize(ns);
	}

	void combine(const vector<int> &A, const vector<int> &B, vector<int> &C) {
	...
	void dfs(int u, vector<int> &CC, vector<int> &DD) {
		CC.resize(2); DD.resize(2);
		CC[0] = 0; DD[0] = B+1;
		CC[1] = C[u];
		DD[1] = D[u];
		for (int v: E[u]) {
			vector<int> xC, xD;
			dfs(v, xC, xD);
			vector<int> rC(CC.size() + max(xC.size(), xD.size())-1, B+1), rD(DD.size() + max(xD.size(), xC.size())-1, B+1);
			combine(CC, xC, rC);
			combine(DD, xC, rD);
			combine(DD, xD, rD);
//			shrink(rC);
//			shrink(rD);
    ...
    void solve(istream& cin, ostream& cout) {
		cin >> N >> B;
		D.resize(N); C.resize(N); E.resize(N);
		S = vector<ui>(N, 0);
		for (int i = 0; i < N; ++i) {
			cin >> C[i] >> D[i];
			D[i] = C[i] - D[i];
...
/*
		size(0);
		for (int i = 0; i < N; ++i) {
			sort(E[i].begin(), E[i].end(), [&](int a, int b) -> bool { return S[a] < S[b]; });
		}
*/
		vector<int> C1, C2;
		dfs(0, C1, C2);
		...
		int ans = 0;
		for (int i = 0; i < C1.size(); ++i) if (C1[i] <= B) ans = max(ans, i);
		for (int i = 0; i < C2.size(); ++i) if (C2[i] <= B) ans = max(ans, i);
		cout << ans << endl;
