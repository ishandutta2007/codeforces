...
int main()
{
	...
	while(t--)
	{
		n=read();m=read();k=read();for(int i=1;i<=n;i++) a[i]=read();
		int ans=0;
		if (m <= k)
		{
		...
		{
			for(int i=0,tmp=m-k-1,nw;nw=1e9,i<=k;ans=max(ans,nw),i++)
			for(int j=0;j<=tmp;j++) nw=min(nw,max(a[i+j+1],a[n-k-tmp+i+j]));
			write(ans,'\n');
		}
	}
	return ot(),0;
