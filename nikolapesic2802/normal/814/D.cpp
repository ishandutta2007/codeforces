...
int main()
{
	int n;
	scanf("%i",&n);
	vector<pair<int,pair<int,int> > > cir(n);
	for(int i=0;i<n;i++)
		scanf("%i %i %i",&cir[i].s.f,&cir[i].s.s,&cir[i].f);
	sort(cir.rbegin(),cir.rend());
	vector<int> dep(n);
	ll ans=0;
	for(int i=0;i<n;i++){
		for(int j=i+1;j<n;j++)
			if((ll)(cir[j].s.f-cir[i].s.f)*(cir[j].s.f-cir[i].s.f)+(ll)(cir[j].s.s-cir[i].s.s)*(cir[j].s.s-cir[i].s.s)<(ll)cir[i].f*cir[i].f)
				dep[j]=max(dep[j],dep[i]+1);
		//printf("%i %i %i: %i\n",cir[i].s.f,cir[i].s.s,cir[i].f,dep[i]);
		if(dep[i]<=1){
			ans+=(ll)cir[i].f*cir[i].f;
		}
		else{
			if(dep[i]%2==1)
				ans+=(ll)cir[i].f*cir[i].f;
	...
	cout << fixed << setprecision(20) << acos(-1)*ans;
	return 0;
}