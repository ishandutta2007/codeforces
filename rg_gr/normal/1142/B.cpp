...
#endif
int a[1000005],b[1000005],lst[1000005],dp[1000005],pre[1000005],L[1000005];
int f[21][1000005];
signed main()
{
	initprog();
	int n=read(),m=read(),k=read();
	rg(n)a[i]=read();pre[a[i]]=a[i-1];gr
	pre[a[1]]=a[n];
	rg(m)b[i]=read();gr
	rg(m)
	lst[b[i]]=i;
	f[0][i]=lst[pre[b[i]]];
	// odl(f[0][i]);
	rg_(j,20)f[j][i]=f[j-1][f[j-1][i]];gr
	L[i]=L[i-1];
	int p=i;
	for(int j=0;j<=20;j++)if((n-1)>>j&1)p=f[j][p];
	L[i]=max(L[i],p);
	gr
	rg(k)
	int l=read(),r=read();
