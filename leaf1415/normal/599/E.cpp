...
ll n, m, Q;
vector<ll> G[13], anc[13];
vector<P> qvec[13];
ll dp[1<<13][13], dp2[1<<13][13];
bool valid[13][13][1<<13];
...
int main(void)
{
	...
	rep(i, 1, Q){
		cin >> a >> b >> c;
		a--, b--, c--;
		qvec[c].push_back(P(a, b));
		if(c != a) anc[a].push_back(c);
		if(c != b) anc[b].push_back(c);
	}
	rep(i, 0, n-1){
		for(auto p : qvec[i]){
		...
		rep(j, 0, N-1){
			...
			rep(k, 0, n-1){
				...
				for(auto p : qvec[i]){
					...
					if(u != i && v != i && (j & (1<<u)) && (j & (1<<v))) valid[i][k][j] = false;
				}
				for(auto v : G[i]) if(v != k && (j & (1<<v))) valid[i][k][j] = false;
				rep(l, 0, n-1){
					if((j & (1<<l)) == 0) continue;
					for(auto v : G[l]) if(!(l == k && v == i) && (j & (1<<v)) == 0) valid[i][k][j] = false;
				}
			}
		}
	}
	...
	rep(i, 0, N-1){
		rep(j, 0, n-1){
			...
			for(int k = i; k; k = (k-1)&i){
				...
				rep(l, 0, n-1){
					if((k & (1<<l)) == 0) continue;
					if(!valid[j][l][k]) continue;
					//outl(i, j, k, l);
					dp2[i][j] += dp[k-(1<<l)][l] * dp2[i-k][j];
				}
			}
		}
		rep(j, 0, n-1){
			...
			for(auto p : qvec[j]){
				ll u = p.first, v = p.second;
				if(u != j && (i & (1<<u)) == 0) flag = false;
				if(v != j && (i & (1<<v)) == 0) flag = false;
			}
			for(auto v : anc[j]) if(i & (1<<v)) flag = false;
			if(flag) dp[i][j] = dp2[i][j];
		}
	}
	//rep(i, 0, N-1) outa(dp[i], 0, n-1);
	outl(dp[N-2][0]);
