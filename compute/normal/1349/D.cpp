...
int main()
{
    ...
    for(int i=2;i<=m;i++)
    {
        ll p1=(m-i+1)*inv(m)%mod;
        ll p2=(n-2+mod)%mod*inv(n-1)%mod;
        ll var=(1-p1*p2%mod+p1-1+mod)%mod;
        e[i]=(1+(1-p1+mod)%mod*e[i-1]%mod)*inv(var)%mod;
        //dbg(i,e[i]);
    }
    for(int i=m;i>=1;i--) suf[i]=(e[i]+suf[i+1])%mod;
    ll C=suf[1];
    ll ans=mod-(n-1)*C%mod;
    for(int i=0;i<n;i++) ans=(ans+suf[a[i]+1])%mod;
    ans=ans*inv(n)%mod;
    print(ans);
}