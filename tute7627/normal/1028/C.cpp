...
int main(){
  ...
  ll n;cin>>n;
  auto a=vec(4,n,0LL);
  rep(i,0,n)cin>>a[0][i]>>a[2][i]>>a[1][i]>>a[3][i];
  auto b=vec(8,n+1,0LL);
  rep(i,0,8){
    b[i].assign(n+1,i%4<2?-INF:INF);
    if(i%2==0){
      rep(j,0,n){
        b[i][j+1]=(i%4<2?max(b[i][j],a[i/2][j]):min(b[i][j],a[i/2][j]));
      }
    }
    if(i%2==1){
      rrep(j,0,n){
        b[i][j]=(i%4<2?max(b[i][j+1],a[i/2][j]):min(b[i][j+1],a[i/2][j]));
      }
    }
  }
  rep(i,0,n){
    ...
    lx=max(b[0][i],b[1][i+1]);
    rx=min(b[2][i],b[3][i+1]);
    ly=max(b[4][i],b[5][i+1]);
    ry=min(b[6][i],b[7][i+1]);
    if(lx<=rx&&ly<=ry){
      cout<<lx spa ly<<endl;
      return 0;
    }
