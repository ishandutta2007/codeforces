...
void solve() {
	...
	while (x != 1) {
		int now_val = x;
		int top = upper_count(x);
		REP(q, top) {
			ans.push_back(make_tuple(x, x, 0));
			x *= 2;
		}
		int hoge = x ^ now_val;
		ans.push_back(make_tuple(x, now_val, 1));
		ans.push_back(make_tuple(x, hoge, 0));
		x = hoge + x;
		ans.push_back(make_tuple(hoge, hoge, 0));
		ans.push_back(make_tuple(x, hoge * 2, 1));
		x ^= (hoge << 1);
	}
	cout << ans.size() << endl;
	for (auto x : ans) {
		string s = "+^";
		cout << get<0>(x) << " " << s[get<2>(x)] <<  " " << get<1>(x) << endl;
	}
}
#undef int
int main() {
