		...
		up(i,0,n+n)
		{
			up(j,0,i)
			{
				up(t,0,1)
				{
					...
					if(j)Add(dp[i+1][j-1][t],1ll*dp[i][j][t]*j%mod);
					if(!t&&j>=k)Add(dp[i+1][j][1],dp[i][j][t]);
					
				}
			}
		}
		fac[0]=1;
		up(i,1,n+n+10)fac[i]=1ll*fac[i-1]*i%mod;
		int ans;
		ans=Frac(1ll*fac[n]*qpow(2,n)%mod*l%mod*dp[n+n+1][0][1]%mod,fac[n+n+1]);
		opd(ans);
	}
}
/*
