...
ll solve(vector<ll> as, vector<ll> bs){
	ll re=0;
	reverse(all(as));
	ll ok=0;
	for(cauto a:as)if(cauto it=lower_bound(all(bs),a); it!=bs.end() && *it==a) ++ok;
	ll cnt=0;
	rep(i,bs.size()){
		cauto b=bs[i];
		while(as.size() && b+cnt>=as.back()){
			++cnt;
			if(cauto it=lower_bound(all(bs),as.back()); it!=bs.end() && *it==as.back()) --ok;
			as.pop_back();
		}
		cauto t=lower_bound(all(bs),b+cnt)-bs.begin()-i;
		MX(re,t+ok);
	}
	return re;
}
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		for(cauto a:as) if(a<0) al.eb(-a); else ar.eb(a);
		for(cauto b:bs) if(b<0) bl.eb(-b); else br.eb(b);
		reverse(all(al)); reverse(all(bl));
		cout<<(solve(al,bl)+solve(ar,br)) NL;
	}
  return 0;
}