...
fn run(sc: &mut scanner::Scanner, out: &mut std::io::BufWriter<std::io::StdoutLock>) {
    ...
    for _ in 0..t {
        ...
        let b: Vec<usize> = sc.next_vec(k);
        let mut save = vec![false; n + 1];
        for b in b.iter() {
            save[*b] = true;
        }
        let mut ia = vec![0; n + 1];
        ...
        for b in b {
            let mut next = M::zero();
            let mut del = (0, 0);
            if let Some(p) = set.range((ia[b] + 1, 0)..).next() {
                if !save[p.1] {
                    next += ans;
                    del = *p;
                }
            }
            if let Some(p) = set.range(..(ia[b], 0)).rev().next() {
                if !save[p.1] {
                    next += ans;
                    del = *p;
                }
