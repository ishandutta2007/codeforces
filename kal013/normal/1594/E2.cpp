...
void solve(){
	map<string, int> mpr = {{"white", 0}, {"yellow", 3}, {"green", 1}, {"blue", 4}, {"red", 2}, {"orange", 5}};
	long long k, n;
	cin >> k >> n;
	gp_hash_table<long long, int, custom_hash> colors;
	...
	gp_hash_table<long long, mint, custom_hash> dp[6];

	auto get_cnt = [&] (long long x, long long c){
		mint ret = 1;

		for(long long nc: {x << 1, (x << 1) | 1}){
			if (nc > tot)
				break;
			if (values.find(nc) == values.end())
				continue;
			mint cur = 0;
			for(int j = 0; j < 6; ++j){
				assert (dp[j].find(nc) != dp[j].end());
				if (abs(j - c) % 3){
					cur += dp[j][nc];
				}
			}
	...
	mint ans = (mint(4)) ^ ((long long) (tot - values.size()));
	mint oth = 0;
	for(int i = 0; i < 6; ++i){
		oth += dp[i][1];
	}
