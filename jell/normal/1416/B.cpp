#line 1 "codeforces-workspace\\contest\\1416\\b\\b.cpp"
#include <bits/extc++.h>
...
#line 9 "codeforces-workspace\\contest\\1416\\b\\b.cpp"
...
void main() {
  ...
  auto sum = accumulate(begin(a), end(a), 0l);
  if (sum % n) {
    cout << -1 << eol;
    return;
  }
  ...
  for (int i = 1; i < n; i++) {
    if (a[i] % (i + 1)) {
      ops.emplace_back(1, i + 1, i + 1 - a[i] % (i + 1));
      a[i] += i + 1 - a[i] % (i + 1);
    }
  ...
  for (auto i = 1; i != n; ++i) {
    ops.emplace_back(1, i + 1, g);
    // ops.emplace_back(1, i + 1, g - a[i]);
  }
  cout << size(ops) << eol;
  for (auto &&e : ops) {
    cout << e << eol;
