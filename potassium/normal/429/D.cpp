...
int main() {
	...
	auto cmp_yx = [](pt x, pt y) {
		if (x.second != y.second) return x.second < y.second;
		return x.first < y.first;
	};
	set<pt, decltype(cmp_yx)> s(cmp_yx);
	...
	for (int r = 0, l = 0; r < n; r++) {
		while (l < r && sqr(p[r].first - p[l].first) >= d) {
			s.erase(p[l]);
			l++;
		}

		auto it = s.lower_bound({ -1, p[r].second - sqrt(d) - 1 });
		while (it != s.end() && sqr(it->second - p[r].second) <= d) {
			chkmin(d, calc(*it, p[r]));
			it++;
		}
