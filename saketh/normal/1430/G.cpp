...
int main() {
    ...
    const ll INF = LLONG_MAX / N / N;
    const ll BIG_NUM = INF / N / N;

    dinic<ll> g(1 + N * (N - 1) + 1);

    vvi indicators(N);

    F0R (i, N) {
        g.arc(0, 1 + i * (N - 1), BIG_NUM);
        F0R (j, N - 2) {
            g.arc(1 + i * (N - 1) + j, 1 + i * (N - 1) + j + 1, BIG_NUM + (j + 1) * wt[i]);
        }
        g.arc(1 + i * (N - 1) + (N - 2), g.V - 1, BIG_NUM + (N - 1) * wt[i]);
    }

    F0R (i, N) {
        F0R (j, N) if (adj[i] & (1 << j)) {
            g.arc(0, 1 + i * (N - 1), INF);
            F0R (k, N - 2) {
                g.arc(1 + j * (N - 1) + k, 1 + i * (N - 1) + k + 1, INF);
            }
            g.arc(1 + j * (N - 1) + N - 2, g.V - 1, INF);
        }
    }

    auto res = g.solve(0, g.V - 1);
