...
map<pi, deque<pi>> dp; //0 position, dp value
int cur_time;
...
int main() {
    ...
    for(auto e: events){ //update curses, dp, cur_time
        // dbg(e);
        // dbg(dp);
        if(e.f.f > cur_time){
            cur_time = e.f.f;
        ...
        if(e.f.s){ //adding curse
            ...
            // dbg(in_dp_l, in_dp_r, dp_rang_l, dp_rang_r);
            if(in_dp_l && in_dp_r && dp_rang_l == dp_rang_r){
                // dbg("merging");
                dp[dp_rang_l] = mergeDP(orig_dp_rang_l, orig_dp_rang_r);
