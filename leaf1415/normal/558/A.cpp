...
ll calc(vector<P> vec, vector<P> vec2)
{
	reverse(all(vec)), reverse(all(vec2));
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> x >> a;
		if(x > 0) vec.push_back(P(x, a));
		else nvec.push_back(P(-x, a));
	}
	sort(all(vec)), sort(all(nvec));
	outl(max(calc(vec, nvec), calc(nvec, vec)));
