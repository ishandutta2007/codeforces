...
void solve() {
  ...
  vi pf(LIM + 1);
  for (auto&& p: primes) { FOR(i, LIM / p + 1) pf[p * i] = p; }
  vi TO(LIM + 1);
  TO[1] = 1;
  FOR3(x, 2, LIM + 1) {
    ll p = pf[x];
    ll to = TO[x / p];
    TO[x] = (to % p == 0 ? to / p : to * p);
  }

  for (auto&& a: A) a = TO[a];
  sort(all(A));
  if (A[0] == 1) return print(1);
  FOR(i, N - 1) if (A[i] == A[i + 1]) return print(2);
  ...
  auto solve_frm = [&](ll s) -> void {
    dist[1] = -1;
    for (auto&& p: primes) dist[p] = -1;
    dist[s] = 0;
    ll l = 0, r = 0;
    que[r++] = s;
    while (l < r) {
      auto v = que[l++];
      if (2 * dist[v] > ANS) break;
      for (auto&& e: G[v]) {
        ll w = e.to;
        if (w < s) continue;
        if (dist[w] == dist[v]) {
          chmin(ANS, 2 * dist[v] + 1);
          break;
        }
        if (dist[w] == dist[v] + 1) {
          chmin(ANS, 2 * dist[w]);
