...
void splitbykey(Node* rt, Node*& x, Node*& y, int key) {
    insert(rt, key);
    rt->l->p = rt->r->p = nil;
    x = rt->l, y = rt->r;
}
...
void dfs(int u, Node* rt) {
    ...
    while (rt != nil && findkey(rt, a[u]) != nil) {
        rt->l->p = rt->r->p = nil;
        rt = join(rt->l, rt->r);
    }
    Node *x, *y;
    splitbykey(rt, x, y, a[u]);
    dfs(l[u], x), dfs(r[u], y);
}
...
void solve() {
    ...
    FOR(i, 0, n) {
        cin >> a[i] >> l[i] >> r[i];
        if (l[i] > 0) l[i]--, f[l[i]] = 1;
        if (r[i] > 0) r[i]--, f[r[i]] = 1;
    }
    Node* rt = nil;
    FOR(i, 0, n) {
        insert(rt, a[i]);
