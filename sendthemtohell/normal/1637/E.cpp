...
ll do_bfs(int i, int j, const set<pair<int, int>>& ban) {
	ll mult = i + j;

	set<pair<int, pair<int, int>>, greater<>> st;
	st.insert({ kek[i][0] + kek[j][0], {0, 0} });
	while (sz(st)) {
		auto p = *st.begin();
		st.erase(st.begin());
		if (!ban.count({ kek[i][p.y.x], kek[j][p.y.y] }) && (i != j || p.y.x != p.y.y)) {
			return mult * (kek[i][p.y.x] + kek[j][p.y.y]);
		}
		if (p.y.x + 1 < sz(kek[i])) st.insert({ kek[i][p.y.x + 1] + kek[j][p.y.y], {p.y.x + 1, p.y.y} });
		if (p.y.y + 1 < sz(kek[j])) st.insert({ kek[i][p.y.x] + kek[j][p.y.y + 1], {p.y.x, p.y.y + 1} });
	}
	return -1;
}
...
inline void _solve() {
	...
	while (t--) {
		...
		mrg.push_back({ v[0],0 });
		for (int i = 0; i < n; ++i) {
			if (mrg.back().x == v[i]) ++mrg.back().y;
			else mrg.push_back({ v[i], 1 });
		}
		...
		sort(all(cnts));
		cnts.erase(unique(all(cnts)), cnts.end());
		assert(sz(cnts) * sz(cnts) <= 2 * n);

		for (auto u : cnts) {
			kek[u].clear();
		}
		for (auto p : mrg) {
			kek[p.y].push_back(p.x);
		}
		...
		for (auto i : cnts) {
			for (auto j : cnts) {
				if (i == j) {
					mx = max(mx, do_bfs(i, i, ban));
				}
				else {
					mx = max(mx, do_bfs(i, j, ban));
				}
			}
		}
		cout << mx << endl;
