// Game Master (https://codeforces.com/contest/1608/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        string ans(n, '0');
        int mn_winning_map2_score = *max_element(all(b));
        set<int> rem_map2_scores(all(b));
        trav(i, idx) {
            if (mn_winning_map2_score <= *rbegin(rem_map2_scores)) {
                ans[i] = '1';
                mn_winning_map2_score = min(mn_winning_map2_score, b[i]);
            }

            rem_map2_scores.erase(b[i]);
        }
