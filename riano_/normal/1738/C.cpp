    ...
    rep(ii,T){
        ...
        rep(i,e+1){
            rep(j,o+1){
                ...
                rep(k,2){
                    if((i+j+N)%2==0){
                        if((i>0&&dp[i-1][j][k])||(j>0&&dp[i][j-1][1-k])){
                            dp[i][j][k] = 1;
                        }
                    }
                    else{
                        dp[i][j][k] = 1;
                        if((i>0&&dp[i-1][j][k]==0)||(j>0&&dp[i][j-1][k]==0)){
                            dp[i][j][k] = 0;
                        }
                    }
                }
        ...
        // if(a[N-K+2]*(N-K+1)<s[N-K+1]) ok = false;
        // if(ok){
        //     cout << "YES" << "\n";
        // }
        // else{
