...
int main()
{
    ...
    for (int i = 0; i < kx; i++)
    {
        ...
        if (need <= lenx[i])
        {
            ...
            for (int j = 0; j < (int)sobx[i].size(); j++)
            {
                ...
                if (curlen >= need)
                {
                    printf("%d %d %d %d\n", 0, ry[i], sobx[i][j].x - (curlen - need), ry[i]);
                    return 0;
                }
                curk += sobx[i][j].t;
                lastx = sobx[i][j].x;
    ...
    for (int i = 0; i < ky; i++)
    {
        ...
        if (need <= leny[i])
        {
            ...
            for (int j = 0; j < (int)soby[i].size(); j++)
            {
                ...
                if (curlen >= need)
                {
                    printf("%d %d %d %d\n", rx[i], 0, rx[i], soby[i][j].x - (curlen - need));
                    return 0;
                }
                curk += soby[i][j].t;
                lastx = soby[i][j].x;
    ...
    for (int i = 0; i < ky; i++) if (rx[i] + 1 != rx[i + 1] && rx[i] + 1 != n) emptyx = rx[i] + 1;
    int need = (answer ^ m);
    if (need <= m && emptyx != -1)
    {
    ...
    for (int i = 0; i < kx; i++) if (ry[i] + 1 != ry[i + 1] && ry[i] + 1 != m) emptyy = ry[i] + 1;
    need = (answer ^ n);
    if (need <= n && emptyy != -1)
    {
