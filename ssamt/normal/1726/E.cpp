...
int main() {
	...
	for(int cl=0; cl<t; cl++) {
		...
		vector<Mod> inv_fac;
		for(i=0; i<=n; i++) {
			inv_fac.push_back(Mod(1)/fac[i]);
		}
		vector<Mod> count{Mod(1), Mod(1)};
		for(i=2; i<=n; i++) {
			count.push_back(count[i-2]*(i-1)+count[i-1]);
		}
		vector<Mod> mul{Mod(1)};
		for(i=1; i<=n; i++) {
			mul.push_back(mul[i-1]*(2*i-1));
		}
		Mod answer;
		for(i=0; 2*i<=n; i+=2) {
			Mod add = fac[n-i]*inv_fac[i]*inv_fac[n-2*i]*pow_mod(2, i/2)*mul[i/2];
			add *= count[n-2*i];
			answer += add;
		}
		printf("%lld\n", answer.val);
