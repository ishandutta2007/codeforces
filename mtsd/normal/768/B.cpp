...
int main(){
    LL(n,L,R);
    map<ll,ll> mp;
    auto calc = [&](auto &&f,ll m)->ll{
        if(m<=1){
            mp[m] = 1;
            return 1;
        }
        return mp[m] = f(f,m/2)*2 + 1;
    };
    calc(calc,n);
    L--;
    auto dfs = [&](auto &&f,ll l,ll r,ll m)->ll{
        if(r-l<=0)return 0;
        if(m==0||m==1)return m;
        ll len = mp[m/2];
        // len (0/1) len
        if(l>len){
            return f(f,l-len-1,r-len-1,m/2);
        }
        if(r<=len){
            return f(f,l,r,m/2);
        }
        return f(f,l,len,m/2) + m%2 + f(f,0,r-len-1,m/2);
    };
    cout << dfs(dfs,L,R,n) << endl;
    return 0;
}