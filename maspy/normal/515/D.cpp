...
void solve() {
  ...
  ll dx[] = {1, 0, -1, 0};
  ll dy[] = {0, 1, 0, -1};
  auto deg = [&](ll x, ll y) -> ll {
    ll r = 0;
    FOR(d, 4) {
      ll nx = x + dx[d], ny = y + dy[d];
      if (isin(nx, ny) && G[nx][ny] == '.') r++;
    }
    return r;
  };
  ...
  vc<pi> que;
  FOR(x, H) FOR(y, W) if (deg(x, y) == 1) que.eb(x, y);
  ...
  while (!que.empty()) {
    ...
    FOR(d, 4) {
      ...
      if (isin(nx, ny) && G[nx][ny] == '.') {
        ...
        G[nx][ny] = TO[d];
        FOR(e, 4) {
          ll mx = nx + dx[e], my = ny + dy[e];
          if (isin(mx, my) && G[mx][my] == '.') que.eb(mx, my);
        }
      }
    }
  }
  ll dot = 0;
  FOR(x, H) dot += count(all(G[x]), '.');
  if (dot > 0) return print("Not unique");
  FOR(x, H) print(G[x]);
}
