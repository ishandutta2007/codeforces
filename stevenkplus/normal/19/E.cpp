...
int main() {
  ...
  vector<int> depths(n);
  vector<int> pars(n);
  // cross edges that make bad cycles (odd cycles with dfs tree)
  vector<int> cross;
  // number of times i->par[i] is part of a bad cycle
  vector<int> counts(n);
  auto dfs = y_combinator([&](auto dfs, int cur, int prv = -1, int par = -1, int depth = 1) -> void {
    depths[cur] = depth;
    pars[cur] = par;
    for (pii p: ed[cur]) if (p.fi != prv) {
      int nxt = p.fi;
      if (depths[nxt]) {
        if (depths[nxt] > depths[cur]) continue;
        if (depths[nxt] % 2 == depths[cur] % 2) {
          cross.push_back(p.se);
          ++counts[cur];
          --counts[nxt];
        } else {
          --counts[cur];
        ...
        dfs(nxt, cur, p.se, depth + 1);
        counts[cur] += counts[nxt];
      }
    }
  });
  for (int i = 0; i < n; ++i) {
  ...
  if (sz(cross) == 0) {
    ...
    if (sz(cross) == 1) {
      ans.push_back(cross[0]);
    }
    for (int i = 0; i < n; ++i) {
      if (counts[i] == sz(cross)) {
        ans.push_back(pars[i]);
      }
    }
  }
