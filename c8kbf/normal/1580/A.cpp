...
_ n, m, a[maxn][maxn], b[maxn][maxn], p[maxn][maxn], p2[maxn][maxn], c[maxn], dp[maxn], s;
...
inline void AC() {
    ...
    m = read();
    s = 1E17;
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) a[i][j] = getDg() == '1';
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) b[i][j] = !a[i][j];
    ...
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) {
        if(j == 1) p2[i][j] = 0;
        p2[i][j] = p2[i][j-1]+b[i][j];
    }
    for(_ i = 1; i <= m; ++i) for(_ j = i+3; j <= m; ++j) {
        for(_ k = 1; k <= n-1; ++k) c[k] = -(p2[k][j-1]-p2[k][i])+p[k][j-1]-p[k][i]+b[k][i]+b[k][j]+p2[k+1][j-1]-p2[k+1][i];
        c[n] = 1E17;
        for(_ k = 1; k <= n; ++k) dp[k] = 1E17;
        for(_ k = n; k >= 1; --k) dp[k] = min(0LL, c[k]+dp[k+1]);
        for(_ k = 1; k <= n-4; ++k) {
            _ c = p2[k][j-1]-p2[k][i]+p2[k+4][j-1]-p2[k+4][i];
            for(_ l = k+1; l <= k+3; ++l) c += b[l][i]+b[l][j]+p[l][j-1]-p[l][i];
            s = min(s, c+dp[k+4]);
        }
    }
    writeln(s);
