...
int main() {
    ...
    while (Q--) {
        ...
        if (t == 1) {
            ...
            if (2 * x <= r - l) {
                if (!rev) {
                    rep(i, x) bit.add(l + 2 * x - 1 - i, bit[l + i]);
                    l += x;
                } else {
                    rep(i, x) bit.add(r - 2 * x + i, bit[r - 1 - i]);
                    r -= x;
                }
            } else {
                if (!rev) {
                    rep(i, r - l - x) bit.add(r - 2 * (r - l - x) + i, bit[r - 1 - i]);
                    r = l + x;
                    rev = true;
                } else {
                    rep(i, r - l - x) bit.add(l + 2 * (r - l - x) - 1 - i, bit[l + i]);
                    l = r - x;
                    rev = false;
                }
            }
            ...
            if (!rev) {
                cout << bit.query(l + s, l + t) << '\n';
            } else {
                cout << bit.query(r - t, r - s) << '\n';
            }
        }
    }
}