...
void MAIN() {
	...
	REP (j, M) {
		...
		if (F[i][j] == '#') {
		    t++;
		    if (t == A[j]) {
			que.push_back(idx[i][j]);
			vis[idx[i][j]] = true;

			must[SCC::scc[idx[i][j]]] = true;
			break;
		    }
		}
	    }
	...
	for (int k=0; k<(int)que.size(); k++) {
	    int v = que[k];
	    EACH (e, G[v]) {
		if (SCC::scc[v] != SCC::scc[*e]) {
		    covered[SCC::scc[*e]] = true;
		}
		if (!vis[*e]) {
		    que.push_back(*e);
		    vis[*e] = true;
...
// #ifdef LOCAL
// 	REP (i, N) REP (j, M) if (F[i][j] == '#') {
// 	    int v = SCC::scc[idx[i][j]];
// 	    eprintf("must %d, covered %d\n", must[v], covered[v]);
// 	}
// #endif

	REP (i, cnt) EACH (e, G[i]) {
	    R[*e].push_back(i);
	}

	REP (j, M) {
	    for (int i=N; i--;) if (F[i][j] == '#') {

		int v = idx[i][j];
		int new_v = SCC::scc[v];
		if (must[new_v] && !covered[new_v]) {
		    ans++;
		    ...
		    int best = v;
		    EACH (e, que) {
			if (P[best].second < P[*e].second
				|| (P[best].second == P[*e].second && P[best].first > P[*e].first)) {
			    best = *e;
			}
		    }

		    // eprintf("push %d %d\n", P[best].first, P[best].second);
		    ...
		    EACH (e, que) covered[SCC::scc[*e]] = true;
		}
	    }
	}
    }
