...
pair<int,int> dfs(int now, int back) {
    int ans = 0;
    int up = 0;
    int down = 0;
    if (get<1>(inputs[now]) != get<2>(inputs[now])) {
        if (get<1>(inputs[now]) == 0)up++;
        else down++;
    }
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        get<0>(inputs[x]) = min(get<0>(inputs[x]), get<0>(inputs[now]));
        pair<int, int> next = dfs(x, now);
        ans += next.first;
        if (next.second >= 0) {
            up += next.second;
        }
        else {
            down += abs(next.second);
        }
    }
    int hoge = min(up, down);
    up -= hoge;
    down -= hoge;
    ans += hoge * get<0>(inputs[now]) * 2LL;
    return mp(ans, up - down);
}
void solve(){
    int n;
    cin >> n;
