...
template <class T, class S> constexpr pair<T, S> operator-(const pair<T, S> &p) { return pair<T, S>(-p.fi, -p.se); }
...
#define S second
    ...
    inline bool check(const P &a, const P &b, const P &c) {
        if(b.S == a.S || c.S == b.S) return sgn(b.F - a.F) * sgn(c.S - b.S) >= sgn(c.F - b.F) * sgn(b.S - a.S);
    ...
    pair<T, T> query2(T x) {
        ...
        assert(!empty());
        int l = 0, r = H.size();
        auto get_day = [&](pll p) { return ceil(x + p.se, -p.fi); };
        while(l < r - 1) {
            int tl = (r * 2 + l) / 3, tr = (r + l * 2) / 3;
            if(l == tl) tl++;
            if(r == tr) tr--;
            if(get_day(H[tl]) < get_day(H[tr]))
                r = tr;
            else
                l = tl;
        }
        ll day = -1, rem;
        rep2(i, -5, 5) {
            if(l + i < 0 or l + i >= si(H)) { continue; }
            ll d = get_day(H[l + i]);
            ll r = get_y(-H[l + i], d) - x;
            if(day == -1)
                day = d, rem = r;
            else if(day > d or (day == d and rem < r))
                day = d, rem = r;
        }
        return {day, rem};
    }
...
int main() {
    ...
    ll st = 0;
    VEC(pll, a, n);
    rep(i, n) if(a[i].se == 0) chmax(st, a[i].fi);
    vpll w;
    for(auto e : a)
        if(e.fi > st) w.eb(e);
    sort(all(w));
    ...
    for(auto [v, c] : w) {
        auto p = cht.H.back();
        ll ng = 0, ok = ceil(c + p.se, -p.fi);
        while(ng < ok - 1) {
            ll mid = ok + ng >> 1;
            (cht.query(mid) >= c ? ok : ng) = mid;
        }
        ll get = cht.query(ok);
        ll d = ok, ret = get - c;
        cht.add(v, ret - d * v);
        dump(v, ret - d * v, get);
        ll need = max(d, ceil(s + d * v - ret, v));
        chmin(ans, need);
    }
    cout << ans << endl;
}