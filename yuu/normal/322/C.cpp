...
void solve() {
    ...
    for (auto&& c : s) {
        ...
        int64_t req_x = a - dx;
        int64_t req_y = b - dy;
        if (tdx && (req_x % tdx)) continue;
        if (tdy && (req_y % tdy)) continue;
        if ((tdx == 0) && req_x) continue;
        if ((tdy == 0) && req_y) continue;
        if ((tdx == 0) && (tdy == 0)) {
            good = 1;
            break;
        } else if (tdx == 0) {
            if (req_y / tdy < 0) continue;
            good = 1;
            break;
        } else if (tdy == 0) {
            if (req_x / tdx < 0) continue;
            good = 1;
            break;
        } else {
            if (req_x / tdx < 0) continue;
            if (req_x / tdx == req_y / tdy) {
                good = 1;
                break;
            }
        }
