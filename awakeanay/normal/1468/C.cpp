...
int main()
{
    ...
    while(test--)
    {
        ...
        loop(i,0,q)
        {
            ...
            if(a==1){cin>>m; s.insert(cur); cur++; moneywise.insert({-m,cur-1}); money[cur-1]=m;}

            if(a==2){cout<<*s.begin()<<" "; moneywise.erase({-money[(*s.begin())],*s.begin()}); s.erase(s.begin());}
            ...
            if(a==3)
            {
                ...
                cout<<g.sd<<" ";
                s.erase(g.sd);
                moneywise.erase(moneywise.begin());
            }
        }
