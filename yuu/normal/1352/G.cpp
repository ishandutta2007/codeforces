...
#endif
int ans[1001];
constexpr int ans_4[] = {2, 4, 1, 3};
constexpr int ans_5[] = {2, 4, 1, 3, 5};
constexpr int ans_6[] = {2, 4, 1, 5, 3, 6};
constexpr int ans_7[] = {2, 4, 1, 5, 7, 3, 6};
void solve() {
    int n;
    cin >> n;
    if (n < 4) {
    ...
    int pos = 1;
    while (n - pos + 1 >= 8) {
        for (int i = 0; i < 4; i++) ans[pos + i] = ans_4[i] + pos - 1;
        pos += 4;
    }
    assert(n - pos + 1 >= 4);
    if (n - pos + 1 == 4) {
        ...
        for (int i = 0; i < 5; i++) ans[pos + i] = ans_5[i] + pos - 1;

    } else if (n - pos + 1 == 6) {
        for (int i = 0; i < 6; i++) ans[pos + i] = ans_6[i] + pos - 1;

    } else if (n - pos + 1 == 7) {
        for (int i = 0; i < 7; i++) ans[pos + i] = ans_7[i] + pos - 1;
    }
