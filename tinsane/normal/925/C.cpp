...
bool add(int v, int& l, int r, ll& x, ll hb)
{
	if (l == r)
		return true;
	if (nodes[v].l == -1)
	...
	else if(add(nodes[v].l, l, r, x, hb))
		return true;
	x ^= nodes[v].val;
	if (nodes[v].r == -1)
	{
	...
	else if (add(nodes[v].r, l, r, x, hb))
		return true;
	return false;
}

void print(int v, ll& x)
{
	if (nodes[v].l != -1)
		print(nodes[v].l, x);
	auto px = x;
