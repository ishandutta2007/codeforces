...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		...
		for(auto l = 0; l < n; ++ l){
			for(auto r = l + 5; r <= n; ++ r){
				int suffmin = numeric_limits<int>::max() / 2;
				for(auto j = 4; j <= m; ++ j){
					int cur = 0;
					cur += r - l - 2 - (pref[r - 1][j - 4] - pref[l + 1][j - 4]);
					debug(cur);
					for(auto k = j - 3; k < j - 1; ++ k){
						cur += pref[r - 1][k] - pref[l + 1][k] + (~a[l][k] & 1) + (~a[r - 1][k] & 1);
						debug(cur);
					}
					suffmin = min(suffmin + pref[r - 1][j - 2] - pref[l + 1][j - 2] + (~a[l][j - 2] & 1) + (~a[r - 1][j - 2] & 1), cur);
					ctmin(res, suffmin + r - l - 2 - (pref[r - 1][j - 1] - pref[l + 1][j - 1]));
					debug(l, r, j, suffmin, suffmin + r - l - 2 - (pref[r - 1][j - 1] - pref[l + 1][j - 1]));
				}
			}
		}
		cout << res << "\n";
