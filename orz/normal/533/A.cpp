...
void dfs_fill(const int& v, const vvi& c, vi& undertree, vi& n_fair, vi& n_cheat, const vi& h)
{
	undertree.pb(v);
	for (int i = 0; i < sz(c[v]); ++i)
	{
		n_fair[c[v][i]] = min(n_fair[v], h[c[v][i]]);
		n_cheat[c[v][i]] = min(n_cheat[v], h[c[v][i]]);
		dfs_fill(c[v][i], c, undertree, n_fair, n_cheat, h);
	}
}
...
int main()
{
	...
	*/
	...
	vvi r_cheat(t), r_fair(t);

	vvi undertrees(t);
	vi n_fair(n), n_cheat(n);
	for (int I = 0; I < t; ++I)
	{
		n_fair[trees[I]] = h[trees[I]];
		n_cheat[trees[I]] = 2e9;
		dfs_fill(trees[I], c, undertrees[I], n_fair, n_cheat, h);
	}
	...
	for (int I = 0; I < t; ++I)
	{
		for (int i = 0; i < sz(undertrees[I]); ++i)
		{
			...
			auto it = humans.lower_bound(pii(-n_fair[to], 0));
			if (it != humans.end())
			{
				r_fair[I].pb(it->Y);
				humans.erase(it);
		...
		for (int i = 0; i < sz(r_fair[I]); ++i)
			humans.emplace(-s[r_fair[I][i]], r_fair[I][i]);
		...
		for (int i = 0; i < sz(undertrees[I]); ++i)
		{
			...
			if (to != trees[I])
			{
				auto it = humans.lower_bound(pii(-n_cheat[to], 0));
				if (it != humans.end())
				{
					r_cheat[I].pb(it->Y);
					humans.erase(it);
		...
		for (int i = 0; i < sz(r_cheat[I]); ++i)
			humans.emplace(-s[r_cheat[I][i]], r_cheat[I][i]);
	}

	SegmentTree st(k - x); // -1 -2 ... -(k - x)


	for (int I = 0; I < t; ++I)
	{
		for (int i = 0; i < sz(r_fair[I]); ++i)
			st.add(r_fair[I][i] - x + 1, k - x, 1);
	}
	...
	for (int I = 0; I < t; ++I)
	{
		if (I)
		{
			for (int i = 0; i < sz(r_fair[I - 1]); ++i)
				st.add(r_fair[I - 1][i] - x + 1, k - x, 1);
			for (int i = 0; i < sz(r_cheat[I - 1]); ++i)
				st.add(r_cheat[I - 1][i] - x + 1, k - x, -1);
		}
		for (int i = 0; i < sz(r_fair[I]); ++i)
			st.add(r_fair[I][i] - x + 1, k - x, -1);
		for (int i = 0; i < sz(r_cheat[I]); ++i)
			st.add(r_cheat[I][i] - x + 1, k - x, 1);
		if (st.check())
