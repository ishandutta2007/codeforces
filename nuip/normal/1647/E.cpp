...
int main(){
	...
	cauto ps=readV(n,1);
	cauto vs=readV(n,1);
	cauto dec=n-(ps-distinct).size();
	cauto t=(*max_element(all(vs))+1-n)/dec;
	out(t,dec,1);
	rep(i,n) dbl[i][0]=ps[i];
	rep(d,D-1)rep(i,n) dbl[i][d+1]=dbl[dbl[i][d]][d];
	vector<int> dsts(n);
	vector<int> mns(n,MOD);
	rep(v,n){
		int i=v;
		rep(d,D)if(t>>d&1) i=dbl[i][d];
		dsts[v]=i;
		MN(mns[i],v);
	}
	set<int> unu;
	rep(i,n) unu.emplace(i);
	vector<int> re(n,-1);
	rep(i,n)if(vs[i]<n){
		assert(mns[i]<MOD);
		re[mns[i]]=vs[i];
	...
	out(re,1);
	rep(i,n)if(re[i]<0){
		cauto it=unu.lower_bound(vs[dsts[i]]);
		assert(it!=unu.end());
		re[i]=*it;
		unu.erase(it);
	}
	assert(unu.empty());
	assert(*min_element(all(re))==0);
	assert(*max_element(all(re))==n-1);
	assert(set<int>(all(re)).size()==n);
	assert(re.size()==n);
	out(re,1);
	for(cauto &v:re) cout<<v+1<<" ";
	cout NL;
	if(0){
		auto tmp=re; sort(all(tmp));
		rep(i,n)if(tmp[i]!=i) rep(_,MOD) cout<<string(MOD,'1');
	}
	if(1){
		vector<int> vals(n,MOD);
		rep(v,n){
			int i=v;
			rep(d,D)if(t>>d&1) i=dbl[i][d];
			MN(vals[i],re[v]);
		}
		rep(i,n)if(vs[i]<n && vs[i]!=vals[i]) assert(0);
	}
	return 0;
}