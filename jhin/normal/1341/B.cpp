        ...
        while(t--)
        {
           cin>>n>>k;vector<int> tmp(n);
           for(int i=0;i<n;i++)cin>>tmp[i];
           cnt=0;
            for(int i=n-2;i>n-k;i--)
            {
                if(tmp[i]>tmp[i-1]&&tmp[i]>tmp[i+1])cnt++;

            }
            mx=cnt,x=n-k+1;//1 2 4 3 1 5 3
            for(int i=n-2,j=n-k-1;j>=0;i--,j--)
            {
                if(tmp[i]>tmp[i+1]&&tmp[i]>tmp[i-1])cnt--;
                if(tmp[j+1]>tmp[j]&&tmp[j+1]>tmp[j+2])cnt++;
                if(cnt>=mx)mx=cnt,x=j+1;
            }
