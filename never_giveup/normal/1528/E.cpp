...
Mint dp[maxn], sdp[maxn];

Mint d2[maxn], sd2[maxn];

void solve() {
    Mint i2 = Mint(1) / 2;
    dp[0] = sdp[0] = 1;
    for (int n = 1; n < maxn; n++) {
        dp[n] = dp[n - 1] + (dp[n - 1] * (dp[n - 1] + 1) * i2);
        if (n > 1)
            dp[n] += dp[n - 1] * sdp[n - 2];
        d2[n] = dp[n] - dp[n - 1];
        sd2[n] = sd2[n - 1] + d2[n];
    ...
    Mint ans = dp[n];
    if (n - 2 >= 0) {
        ans += sdp[n - 2] * (sdp[n - 2] + 1) * i2 * dp[n - 1];
    }
    for (int i = 0; i <= n - 2; i++) {
        //ans += dp[n - 1] * (dp[i] * (i == 0 ? 0 : sdp[i - 1]));
        //ans += dp[n - 1] * (dp[i] * (dp[i] + 1) * i2);

        ans += dp[n - 1] * (dp[n - 1] + 1) * i2 * dp[i];
    }
    ans += dp[n - 1] * (dp[n - 1] + 1) * (dp[n - 1] + 2) / 6;
