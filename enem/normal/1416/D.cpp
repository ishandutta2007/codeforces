...
int main()
{
    ...
    REPI(i,0,q)
    {
        if(b[i].F==1)
        {
            ll x=rn[p[ad[b[i].S].F]];
            ll y=rn[p[ad[b[i].S].S]];
            if(x==y)
            {
                spl[i]=MP(-1,-1);
            }
    ...
    REP(i,0,q)
    {
        // cout<<i<<' '<<b[i].F<<' '<<b[i].S<<' '<<spl[i].F<<' '<<spl[i].S<<'\n';
        // REP(j,0,n)
        // {
        //     cout<<j<<' ';
        //     for(auto it:sz[j]) cout<<pi[it]<<' ';
        //     cout<<'\n';
        // }
        // cout<<endl;
        if(b[i].F==0)
        ...
        {
            if(spl[i]==MP((ll)-1,(ll)-1)) continue;
            V(ll) del;
            for(auto it:sz[spl[i].S])
            {
                rn[it]=spl[i].S;
                if(sz[spl[i].F].count(it)) sz[spl[i].F].erase(sz[spl[i].F].find(it));
                else del.PB(it);
            }
            for(auto it:del)
            {
                sz[spl[i].S].erase(sz[spl[i].S].find(it));
            }
        }
    }
