...
void solve() {
    ...
    l[0] = 0;
    r[n - 1] = 0;
    for (int i = 1; i < n; ++i) l[i] = l[i - 1] + (a[i] - a[i - 1]) * 1.0 / i;
    for (int i = n - 2; i >= 0; --i) r[i] = r[i + 1] + (a[i + 1] - a[i]) * 1.0 / (n - 1 - i);
    int p = 0;
    while (p < n - 2 && l[p + 1] < r[p + 1]) ++p;
    ld pl = a[p], pr = a[p + 1];
    ld vl = (p + 1);
    ld vr = (n - p - 1);
    if (l[p] > r[p + 1]) pr -= (l[p] - r[p + 1]) * vr;
    else pl += (r[p + 1] - l[p]) * vl;
    println(max(l[p], r[p + 1]) + (pr - pl) / (vl + vr));
}
