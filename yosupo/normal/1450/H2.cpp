...
mint naive(int x, int y) {
    assert((x + y) % 2 == 0);

    mint ans = 0;
    for (int h = x - y; h <= x + y; h++) {
        if (h % 4) continue;
        // x + z - (y - z) == h
        // 2 * z == h + y - x
        int z = (h + y - x) / 2;
        mint comb = C.C(y, z);
        ans += comb * mint(abs(h) / 4);
    }
...
void init() {
    for (int by = 0; by < BN; by++) {
        ...
        // for (int s = y - MN + 20; s <= -y + MN - 20; s += 2) {
        //     table[by][s + MN] = naive(s, y);
        // }
        int ss = y - MN + 20;
        table[by][ss + MN] = naive(ss, y);
        table[by][ss + 2 + MN] = naive(ss + 2, y);
        for (int s = ss + 4; s <= -y + MN - 20; s += 2) {
            mint f = table[by][s - 4 + MN];

            // s + z - (y - z) == 0
            // 2 * z = y - s
            int zero = (y - s) / 2;

            mint all = (zero % 2 == 0) ? all0 : all1;

            // ~ zero : -1
            // (zero+1) : 1

            if (zero < 0) f += all;
            else if (y <= zero) f -= all;
            else {
                f -= comb[zero];
                f += all - comb[zero];
            }
...
//            dbg(s, y, f.val(), naive(s, y).val(), zero);
//            assert(f == naive(s, y));
        }
    }
}
...
mint solve(int x, int y) {
    ...
    for (int h = x - dify; h <= x + dify; h += 2) {
        assert(h % 2 == 0);
        assert((h + dify - x) % 2 == 0);
        int z = (h + dify - x) / 2;
        mint comb = C.C(dify, z);
        ans += comb * table[by][h + MN];
    }
    return ans;
}
...
void test() {
    for (int x = -10; x <= 10; x++) {
        for (int y = 0; y <= 30; y++) {
            if ((x + y) % 2) continue;
            assert(naive(x, y) == solve(x, y));
        }
    }
}

mint solve2(int x, int y) {
    mint ans = solve(x, y);
    return ans / mint(2).pow(y - 1);
}
