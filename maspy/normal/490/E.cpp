...
void solve() {
  LL(N);
  VEC(string, A, N);
  auto calc = [&](string S, string T) -> string {
    // S T 
    assert(count(all(S), '?') == 0);
    if (len(S) > len(T)) return "";
    if (len(S) < len(T)) {
      FOR(i, len(T)) if (T[i] == '?') { T[i] = (i == 0 ? '1' : '0'); }
      return T;
    }
    ll N = len(S);
    vc<pair<string, bool>> st;
    st.eb("", false);
    while (!st.empty()) {
      auto [X, is_large] = st.back();
      st.pop_back();
      ll n = len(X);
      if (n == N) {
        if (is_large) return X;
        continue;
      }
      if (T[n] != '?') {
        if (is_large) {
      ...
      // 
      ll x = S[n] - '0';
      if (x < 9) st.eb(X + char('0' + x + 1), true);
      st.eb(X + char('0' + x), false);
    }
    return "";
  };
