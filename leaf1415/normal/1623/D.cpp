...
int main(void)
{
	...
	cin >> T;
	while(T--){
		cin >> h >> w >> sy >> sx >> ty >> tx >> pro;
		mint p = mint(pro)/100;
		
		ll L = 4*(h-1)*(w-1);
		rep(i, 0, L) used[i] = false;
		
		ll x = sx, y = sy, dx = 1, dy = 1;
		if(x == tx || y == ty) used[0] = true;
		rep(i, 1, L){
			if(x == 1) dx = 1;
			if(x == w) dx = -1;
			if(y == 1) dy = 1;
