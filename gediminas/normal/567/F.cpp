...
bool tikr(int a, int b, int c, int d) {
	for (auto && i : pries[c]) {
		if (a <= i and i <= b)
			return false;
	}

	for (auto && i : prieskart[c]) {
		if (a <= i and i <= b and i != d and i != c)
			return false;
	}
	...
	for (auto && i : prieskart[d]) {
		if (a <= i and i <= b and i != c and i != d)
			return false;
	}
...
int main () {
	...
	for (int i = 0; i < k; i++) {
		...
		if (z == "=") {
			...
			pokart[a].push_back(b);
			prieskart[b].push_back(a);
		}
		else if (z == ">=") {
			prieskart[a].push_back(b);
			pokart[b].push_back(a);
		}
	}

	for (int i = 0; i < 2 * n; i++) {
		debug(i, pries[i], prieskart[i], po[i], pokart[i], kartu[i]);
	}
