...
int deg[MAXN + 5], fa[MAXN + 5], dfn[MAXN + 5], tim, edt[MAXN + 5], rid[MAXN + 5];
void dfs0(int x, int f) {
	fa[x] = f; rid[dfn[x] = ++tim] = x;
...
struct node {int l, r, lz; pii mx;} s[MAXN * 4 + 5];
void pushup(int k) {s[k].mx = max(s[k << 1].mx, s[k << 1 | 1].mx);}
void build(int k, int l, int r) {
	s[k].l = l; s[k].r = r; if (l == r) return s[k].mx = mp(-INF, l), void();
	int mid = l + r >> 1; build(k << 1, l, mid); build(k << 1 | 1, mid + 1, r);
	pushup(k);
}
void tag(int k, int v) {s[k].lz += v; s[k].mx.fi += v;}
...
void assign(int k, int p, int v) {
	if (s[k].l == s[k].r) return s[k].mx.fi = v, void();
	pushdown(k); int mid = s[k].l + s[k].r >> 1;
...
int main() {
	...
	build(1, 1, n);
	for (int i = 2; i <= n; i++) if (deg[i] == 1) assign(1, dfn[i], 0);
	for (int i = 1; i <= n; i++) modify(1, dfn[i], edt[i], 1);
	int W = 0, lim = min(sum, k);
	for (int i = 1; i <= lim; i++) {
		pii pp = s[1].mx; int pt = rid[pp.se]; assign(1, pp.se, -INF);
//		printf("%d %d\n", pt, pp.fi);
		while (pt && !vis[pt]) vis[pt] = 1, modify(1, dfn[pt], edt[pt], -1), pt = fa[pt], W++;
	}
	ll res = INFll;
