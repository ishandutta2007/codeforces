...
int main() {
    ...
    const int OG = N/2 + 1;
    vvmn dp(OG, vmn(K + 1)), DP = dp;
    dp[0][0] = 1;

    F0R (i, N) {
        trav (e, DP) fill(all(e), 0);

        int adj = i ? a[i] - a[i-1] : 0;
        F0R (og, OG) F0R (k, K+1) {
            int inc = k + og * adj;
            if (inc > K) break;

            // this student is alone
            DP[og][inc] += dp[og][k];

            // create new unfinished group
            if (og+1<OG) DP[og+1][inc] += dp[og][k];

            // add to existing unfinished group
            if (og) {
                DP[og-1][inc] += og * dp[og][k];
                DP[og][inc] += og * dp[og][k];
            }
        }
