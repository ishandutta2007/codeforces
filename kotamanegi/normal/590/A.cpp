...
void solve(){
	...
	for (int i = 1; i < bads.size(); ++i) {
		int sizing = bads[i] - (bads[i - 1] + 2);
		ans = max(ans, (sizing + 1LL) / 2LL);
		if (sizing % 2 == 1) {
			for (int q = bads[i - 1] + 1; q < bads[i]; ++q) {
				Ans[q] = inputs[bads[i - 1] + 1];
			}
		}
		else {
			for (int q = 0; q < sizing / 2; ++q) {
				Ans[bads[i - 1] + 2 + q] = Ans[bads[i - 1] + 1];
				Ans[bads[i] - 1 - q] = Ans[bads[i]];
			}
		}
	}
	cout << ans << endl;
