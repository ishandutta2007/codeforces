...
void solve() {
  ...
  FOR(_, N) {
    FOR_R(d, N + N + 1) {
      F[d] = 0;
      if (d >= 2) F[d] -= F[d - 2] * mint(2);
      if (d >= 1) F[d] += F[d - 1] * mint(2);
    }
  }

  auto integral = [&](vc<mint>& F) -> mint {
    mint I = 0;
    FOR(d, F.size()) { I += F[d] * MC.inv(d + 1); }
    return I;
  };
  ...
  FOR3(k, 1, N + 1) {
    // (2x-2x^2) (1-2x+2x^2) 
    FOR(d, F.size()) F[d] *= MC.inv(2);
    FOR(d, F.size()) F[d] = (d == F.size() - 1 ? mint(0) : F[d + 1]);
    FOR(d, F.size() - 1) F[d + 1] += F[d];
    ...
    P[N - k] = integral(F) * MC.C(N, k);
  }

  mint ANS = 0;
  FOR3(k, K, N + 1) ANS += P[k] * mint(L);
  print(ANS);
}
