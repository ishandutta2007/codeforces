...
ll solve(ll t)
{
	...
	ll rem1 = 0, inc = 0, ext = 0, tae = 0;
	
	if(l > 1) R.push(l-1);
	if(l == 1) tae++;
	
	if(r < n-2) R.push((n-1)-r-1);
	if(r == n-2) tae++;
	...
	while(1){
		if(t == 1){
			...
			if(rem1 == 0 && inc > 0){
				...
				else if(inc > 0) inc--, rem1++, sum -= c;
				else if(tae > 0) tae--, sum -= c;
				else break;
			}
		}
		chmax(ret, sum);
