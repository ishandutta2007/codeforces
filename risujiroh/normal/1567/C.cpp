...
template <class C, class T> void iota(C& c, T val) { iota(begin(c), end(c), val); }
template <class C, class T, class Op = plus<>> T accumulate(const C& c, T init, Op op = Op{}) {
  return accumulate(begin(c), end(c), init, op);
}
...
T inner_product(const C1& c1, const C2& c2, T init, Op1 op1 = Op1{}, Op2 op2 = Op2{}) {
  assert(len(c1) == len(c2));
  return inner_product(begin(c1), end(c1), begin(c2), init, op1, op2);
}
...
struct Print {
  ...
  template <class T> void go(const vector<T>& v) {
    for (int i : Rep(len(v))) go(" \0"[i == 0]), go(v[i]);
  }
...
int solve(int n, int carry) {
  ...
  for (int x : Rep(19)) {
    if ((x + carry) % 10 == n % 10) {
      if (x + carry % 10 < 10) {
        res += (10 - abs(x - 9)) * solve(n / 10, carry / 10);
      } else {
        res += (10 - abs(x - 9)) * solve(n / 10, carry / 10 + 10);
      }
    }
  }
  return memo[{n, carry}] = res;
}
