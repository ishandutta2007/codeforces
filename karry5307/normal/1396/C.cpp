...
ll n,d,r1,r2,r3,res;
ll x[MAXN],f[MAXN],tp[MAXN],pr[MAXN];
// Fast IO
inline ll read()
{
...
int main()
{
	...
	{
		x[i]=read();
		tp[i]=min(r1+r2,(x[i]+2)*r1),pr[i]=pr[i-1]+tp[i];
	}
	for(register int i=1;i<=n;i++)
	{
		f[i+1]=min(f[i+1],f[i]+r1*x[i]+r3+d);
		f[n+1]=min(f[n+1],f[i]+d*(n-i)*2+pr[n-1]-pr[i-1]+min(r1*x[n]+r3,tp[n]+2*d));
		if(i<n)
		{
			f[i+2]=min(f[i+2],f[i]+d*(i+2>n?3:4)+tp[i]+tp[i+1]);
		}
		if(i+1<n)
		{
			f[i+3]=min(f[i+3],f[i]+d*(i+3>n?6:7)+tp[i]+tp[i+1]+tp[i+2]);
		}
	}
	printf("%lld\n",f[n+1]);
}