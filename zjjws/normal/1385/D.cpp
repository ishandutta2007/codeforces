...
struct Rin
{
    ...
    inline Rin&operator >>(char &x){for(x=gc();x>'z'||x<'a';x=gc());return *this;}
}rin;
...
const int N=1.5e5+3;
char a[N];
inline int check(char *a,int n,char c){int sum=0;for(int i=1;i<=n;i++)sum+=(a[i]!=c);return sum;}
inline int count(char *a,int n,char c)
{
    if(n==1)return a[1]!=c;
    return min(check(a,n>>1,c)+count(a+(n>>1),n>>1,c+1),check(a+(n>>1),n>>1,c)+count(a,n>>1,c+1));
}
inline void work()
{
