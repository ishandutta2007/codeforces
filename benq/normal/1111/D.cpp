...
Combo<MX> C;
int numWays, ans[52][52], oc[52], dp[50001];
string s; 
...
void ADD(int x) {
    if (!x) return;
    F0Rd(i,sz(s)/2+1-x) AD(dp[i+x],dp[i]);
}

void REM(int x) {
    if (!x) return;
    FOR(i,x,sz(s)/2+1) SUB(dp[i],dp[i-x]);
}

void solve(int ind) {
    if (!oc[ind]) return;
    F0R(i,sz(s)/2+1) dp[i] = 0;
    dp[0] = 1;
    F0R(i,52) if (i != ind) ADD(oc[i]);
    F0R(i,52) {
        if (i == ind) ans[ind][i] = dp[sz(s)/2];
        else {
            REM(oc[i]);
            ans[ind][i] = dp[sz(s)/2];
            ADD(oc[i]);
    ...
    // everything besides ind to make sz(s)/2 
}
...
int main() {
    ...
    numWays = mul(C.fac[sz(s)/2],C.fac[sz(s)/2]);
    F0R(i,52) MUL(numWays,C.ifac[oc[i]]);
    F0R(i,52) solve(i);
    int q; re(q);
    F0R(i,q) {
        int a,b; re(a,b);
        a = cat(s[a-1]), b = cat(s[b-1]);
        pr(mul(mul(2,numWays),ans[a][b]));
    }
    // you should actually read the stuff at the bottom
}
