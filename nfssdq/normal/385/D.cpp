...
pair < double, pair < double, double > > pp[25];
double dis[1<<21];
int vis[1<<21];
int n;
double l, r;
double go(int bp){
    ...
    REP(i, n){
        if(bp & (1<<i)){
            ...
            double pre = go(nb);
            double deg = 0.0;
            point p1 = point(pp[i].xx, pp[i].yy.xx);
            point p2 = point(pre, 0.0);
            line ln = line(p1, p2);
            deg = ln.angle();
            deg = deg * 180.0/ pi;
            deg += pp[i].yy.yy;
            ...
            deg = deg * pi / 180.0;
            line ln1 = line(p1, deg);
            line ln2 = line(point(l, 0.0), point(r, 0.0));
            point p3 = ln1.crosspoint(ln2);
            if(p3.x > r)p3.x = r;
            dis[bp] = max(dis[bp], p3.x);
        }
    }
    return dis[bp];
}
main()
{
    ...
    LL a = 0,b = 0,c,d,e = 0,f = 0,g,h = 1,x,y,z;
    cin >> a >> b >> c;
    REP(i, a)cin >> pp[i].xx >> pp[i].yy.xx >> pp[i].yy.yy;
    double cur = (double)b;
    n = a; l = b; r = c;
    REP(i, 1<<a){
        cur = max(cur, go(i));
    }
    cur = min(cur, (double)c);
    cout << fixed << setprecision(10) << (cur - l) << endl;
}