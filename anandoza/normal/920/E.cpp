...
int main() {
    ...
    int n, m;
    re(n, m);
    set<pii> missing_edges;
    for (int i = 0; i < m; i++) {
        int u, v;
        re(u, v);
        u--, v--;
        missing_edges.insert({u, v});
        missing_edges.insert({v, u});
    }
    ...
    auto bfs = [&](int start) -> void {
        ...
        while (!q.empty()) {
            ...
            for (int next : unvisited) {
                if (missing_edges.count({cur, next}))
                    continue;

                to_enqueue.push_back(next);
            }

            for (int next : to_enqueue) {
                q.push(next);
    ...
    for (int i = 0; i < n; i++) {
        if (unvisited.count(i)) {
            auto before = sz(unvisited);
            bfs(i);
            auto after = sz(unvisited);
