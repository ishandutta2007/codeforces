...
//
//	int[] a = shuffle(1145, gen);
//
	int get(int l, int r)
	{
			out.println("? " + (l+1) + " " + (r+1-1));
			out.flush();
//		assert l < r;
//		int[] t = Arrays.copyOfRange(a, l, r);
//		Arrays.sort(t);
//		for(int i = l;i < r;i++){
//			if(a[i] == t[t.length-2]){
//				return i;
...
//		return -1;
	...
	void solve()
	{
		...
		while(r-l > 3) {
			int h = l + r >> 1;
			if (sec < h) {
				int nsec = get(Math.min(sec, l), Math.max(sec+1, h));
				if (sec == nsec) {
					r = h;
				} else {
					l = h;
				...
				int nsec = get(Math.min(sec, h), Math.max(sec+1, r));
				if (sec == nsec) {
					l = h;
				} else {
					r = h;
