...
int main() {
  using M = monoid::RangeSumAndUpdateQuery<ll>;
  int n, q; cin >> n >> q;
  vector<M::Node> a;
  set<pair<int, int>> st;
  REP(i, n) {
    int ai; cin >> ai;
    a.emplace_back(M::Node{ai, 1});
    st.emplace(-ai, i);
  }
  ...
  while (q--) {
    ...
    if (t == 1) {
      ...
      // while (ub - lb > 1) {
      //   int mid = (lb + ub) >> 1;
      //   (seg[mid].sum < y ? ub : lb) = mid;
      // }
      // seg.apply(ub, x + 1, y);

      auto it = st.lower_bound({-y, 0});
      ...
      for (const auto &p : era) st.erase(p);
      st.emplace(-y, x);
      if (s != -1) st.emplace(f, s);
      seg.apply(l, x + 1, y);
    } else if (t == 2) {
      int ans = 0;
      while (x < n && y > 0) {
        auto it = st.lower_bound({-y, 0});
        chmax(x, it == st.begin() ? 0 : prev(it)->second + 1);
        int r = seg.find_right(x, [&](const M::Node &a) { return a.sum <= y; });
        ans += r - x;
        y -= seg.get(x, r).sum;
        x = r;
      }
