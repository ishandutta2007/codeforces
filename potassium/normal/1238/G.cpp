...
int main() {
	CASE(q) {
        int n,m,c,c0; R(n,m,c,c0);

        map<int,int> mm;
        vector<pair<int,pii>> friends(n+2);
        friends[0] = {0,{0,c0}};
        for (int i = 1; i <= n; i++) {
            int t,a,b; R(t,a,b);
            friends[i].F = t;
            friends[i].S.F = b; //cost
            friends[i].S.S = a; //amount
        }
        friends[n+1] = {m,{0,1}};
        sort(all(friends));
        ...
        int maptotal = c0;
        ll cost = 0;
        mm.insert(friends[0].S);
        for (int i = 1; i <= n+1; i++) {
            int dist = friends[i].F - friends[i-1].F;
            maptotal += friends[i].S.S - dist;

            while (!mm.empty() && dist > 0) {
                pii cur = *mm.begin();
                ll take = min(dist,cur.S);
                cost += take*cur.F;
                dist -= take;

                //W("take", take, cur, cost, dist);

                if (take == cur.S) mm.erase(mm.begin()); else mm.begin()->S -= take;
            }
            ...
            mm[friends[i].S.F] += friends[i].S.S;

            while (maptotal > c) {
                pii cur = *--mm.end();
                ll take = min(maptotal-c,cur.S);
                maptotal -= take;
                if (take == cur.S) mm.erase(--mm.end()); else (--mm.end())->S -= take;
            }
        }
