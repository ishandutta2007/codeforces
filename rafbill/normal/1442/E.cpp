...
void solve(){
  ...
  vector<vector<multiset<li>>> H(2, vector<multiset<li>>(n));

  auto dfs1 = letrec([&](auto dfs1, int i, int p) -> void {
    for(int j : G[i]) if(j != p) dfs1(j, i);
    for(li k = 0; k < (li)(2); ++k) if((A[i] & (1<<k)) == 0) {

      H[k][i].insert(0);
      for(int j : G[i]) if(j != p) {
        H[k][i].insert(min(*H[k][j].rbegin(), *H[k^1][j].rbegin()+1));
      }
    } else {
  ...
  auto dfs2 = letrec([&](auto dfs2, int i, int p) -> void {
    for(int j : G[i]) if(j != p) {
        array<li, 2> h;
        for(li k = 0; k < (li)(2); ++k) if((A[i] & (1<<k)) == 0) {
          h[k] = min(*H[k][j].rbegin(), *H[k^1][j].rbegin()+1);
          H[k][i].erase(H[k][i].find(h[k]));
        }
        for(li k = 0; k < (li)(2); ++k) if((A[j] & (1<<k)) == 0) {
          H[k][j].insert(min(*H[k][i].rbegin(), *H[k^1][i].rbegin()+1));
        }
        dfs2(j, i);
        for(li k = 0; k < (li)(2); ++k) if((A[i] & (1<<k)) == 0) {
          H[k][i].insert(h[k]);
  ...
  li ans = 1ll<<50;
  for(li i = 0; i < (li)(n); ++i) for(li k = 0; k < (li)(2); ++k) ans = min(ans, *H[k][i].rbegin());
  cout << ans+1 << endl;
}
