...
fn run() {
    ...
    let mut d = a.iter().zip(b).map(|p| *p.0 - p.1).collect::<Vec<_>>();
    let save = d.clone();
    d.push(0);
    for i in (0..n).rev() {
        d[i] += d[i + 1];
    ...
    let mut rmq = segment_tree::PURQ::new(d.len(), (-inf, inf), |a, b| {
        (a.0.max(b.0), a.1.min(b.1))
    });
    for (i, d) in d.iter().enumerate() {
        rmq.update_tmp(i, (*d, *d));
    }
    rmq.update_all();
    // sum, max
    let mut seg = segment_tree::PURQ::new(save.len(), (0i64, -inf), |a, b| {
        (a.0 + b.0, a.1.max(a.0 + b.1))
    });
    for i in 0..save.len() {
        seg.update_tmp(i, (save[i], save[i]));
    }
    seg.update_all();
    let out = std::io::stdout();
    let mut out = std::io::BufWriter::new(out.lock());
    for (l, r) in ask {
        let ans = if d[l] != d[r] || seg.find(l, r).1 > 0 {
            -1
        } else {
            let p = d[l];
            let (a, b) = rmq.find(l, r + 1);
            (a - p).max(p - b)
        };
        writeln!(out, "{}", ans).ok();
    }
