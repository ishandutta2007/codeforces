...
int main() {
    ...
    vvll lis(N + 2);
    repn(i, N+1)lis[i].push_back(0);
    repn(i, N) {
        ll a;
        cin >> a;
        lis[a].push_back(i);
    }
    repn(i, N+1)lis[i].push_back(N + 1);
    lazy_segtree<ll, max, e, ll, max, max, e> dp(N+2);

    repn(x, N+1) {
        bool b = 1;
        rep(i, lis[x].size() - 1) {
            if (lis[x][i + 1] == lis[x][i] + 1) { continue; }
            ll t = dp.get(lis[x][i] + 1);
            if (t < lis[x][i + 1]) { b = 0; }
        }
        if (b) { cout << x << endl; return 0; }
        

        rep(i, lis[x].size() - 1) {
            dp.apply(lis[x][i] + 1, lis[x][i + 1] + 1, lis[x][i + 1]);
        }
    }
    cout << N + 2 << endl;
