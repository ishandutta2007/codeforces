...
const int N = 250'228;
const int L = 19;
pair<int, int> tmn[L][N], tmx[L][N];

int get_mn(int l, int r) {
    int g = __lg(r - l);
    return min(tmn[g][l], tmn[g][r - (1 << g)]).y;
}

int get_mx(int l, int r) {
    int g = __lg(r - l);
    return max(tmx[g][l], tmx[g][r - (1 << g)]).y;
}
...
void _solve() {
    ...
    for (int j = 1; (1 << j) <= n; ++j) {
        for (int i = 0; i + (1 << j) <= n; ++i) {
            tmn[j][i] = min(tmn[j - 1][i], tmn[j - 1][i + (1 << (j - 1))]);
            tmx[j][i] = max(tmx[j - 1][i], tmx[j - 1][i + (1 << (j - 1))]);
        }
    }
