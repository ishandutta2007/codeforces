...
ll mh[SZ],fh[SZ];
ll ans;
void ud(int x,int fa=0) {
	{
	ll M=ur[x],F=uf[x];
	for esb(x,e,b) if(b!=fa)
		M=max(M,mx[b]),F+=f[b];
	if(M<h[x]) {
		ll dt=h[x]-M;M+=dt;F+=dt;
	}
//	cout<<M<<","<<F<<":"<<x<<'\n';
	ans=min(ans,F);
	}
	...
	mh[fn+1]=fh[fn+1]=0;
	for(int i=fn;i>=1;--i) {
		mh[i]=max(mh[i+1],mx[fs[i]]);
		fh[i]=fh[i+1]+f[fs[i]];
	}
	ll mxx=0,sf=0;
	for(int i=1;i<=fn;++i) {
		int b=fs[i];
		ur[b]=max(max(mxx,mh[i+1]),ur[x]);
		uf[b]=sf+fh[i+1]+uf[x];
		if(ur[b]<h[x]) {
			ll dt=h[x]-ur[b];
			ur[b]+=dt; uf[b]+=dt;
