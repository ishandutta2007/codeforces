...
public:
    void solve() {
        ...
        auto construct = [&](ll suffix, int nDigsSuffix, int sumDigs, bool canBe9) -> void {
            int sumSuffix = 0;
            ll ten = 1;
            for (int i : range(nDigsSuffix)) {
                sumSuffix += suffix / ten % 10;
                ten *= 10;
            }
            if (sumSuffix > sumDigs) {
                return;
            }
            ll res = suffix;
            sumDigs -= sumSuffix;
            while (sumDigs > 0) {
                int cur = min(canBe9 ? 9 : 8, sumDigs);
                sumDigs -= cur;
                res += ten * cur;
                canBe9 = true;
                ten *= 10;
            ...
            if (sum > n) {
                ...
                for (int j = 1; j <= iN9; j++) {
                    if (iN9 % j == 0) {
                        ...
                        if (i <= k && n9 * 9 <= n) {
                            ll suf = 0;
                            for (int l : range(n9 - 1)) {
                                suf += 9;
                                suf *= 10;
                            }
                            suf += 9 - (k - i);
                            construct(suf, n9, s, false);
                        }
                    }
                }
            } else {
                for (int i : range(10 - k)) {
                    construct(i, 1, s, true);
                }
            }
        }
