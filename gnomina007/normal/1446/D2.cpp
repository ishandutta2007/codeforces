...
int small_check(vector<int> &v, vector<vector<int>> &cols, int mx, int cand)
{
	...
	{
	//	cerr << "CAND i " << cand << ' ' << i << endl;
		int pos = cols[cand][i];
		int posi = lower_bound(cols[mx].begin(), cols[mx].end(), pos) - cols[mx].begin();
	//	cerr << pos << ' ' << posi << endl;
		vector<int> addon(sz + 2, -1);
		for (int i = 0; i <= sz + 1; i++)
			if (posi > i)
				addon[i] = cols[mx][posi - i - 1];
		int bal = 0, where = pos;
		int candp = i, mxp = posi;
		while (true)
		{
		//	cerr <<"BW "<< bal << ' ' << where << endl;
			if (bal >= sz - i + 1)
				break;
