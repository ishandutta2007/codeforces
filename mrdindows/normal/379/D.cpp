...
bool checkf(int x, int y, int fa, int sa, int fc, int sc)
{
    int leni = x * 2;
    if (leni + fc + fa > n) return false;
    int lenj = y * 2;
    if (lenj + sc + sa > m) return false;   
    ax[1] = fa;
    ax[2] = sa;
    cx[1] = fc;
    cx[2] = sc;
...
int main()
{
    ...
    for (int i = 0; i <= n/2; ++i)
    {
        for (int j = 0; j <= m / 2; ++j)
        {
            ...
            for (int t = 1; t < (1 << 4); ++t)
            {
                checkf(i, j, checkbit(t, 0), checkbit(t, 1), checkbit(t, 2), checkbit(t, 3));
            }
