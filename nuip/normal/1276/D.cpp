...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<mint> tops(n),btms(n),mus(n),mos(n);
	...
	function<void(int,int)> dfs=
		[&](int v,int p){
			...
			{
				mint mo=1;
				for(auto u:g[v])if(u!=p) mo*=btms[u]+mus[u];
				mos[v]+=mo;
				tops[v]+=mo;
				out(v,mo,1);
			}
			...
			for(auto u:g[v]){
				if(u==p){
					...
					rs.pop_back();
					if(isL){
						mos[v]+=l*tops[u]*rs.back();
						tops[v]+=l*tops[u]*rs.back();
						// out(v,l*tops[u]*r,l,tops[u],r,1);
					}else{
						mus[v]+=l*tops[u]*rs.back();
					}
					l*=btms[u]+mus[u];
				}
			}
	...
	rep(v,n)if(g[v].size()==1){
		...
		out(u,v,1);
		dfs(u,v);
		cout<<tops[u]+btms[u]+mus[u] NL;
		break;
	}
	out(tops,btms,mus,mos,1);
  return 0;
}