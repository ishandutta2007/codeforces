...
void dfs(int n,int p,int extr[]){
	int id = depth[n]&1;
	ans = min(ans, subt[n][id] + extr[id] - 1 );
	// ans = min(subt[n] + extr - 1, ans);
	...
	// for(auto e: adj[n]){
	// 	if (e != p){
	// 		int arr[2] = {extr[0],extr[1]};
	// 		arr[0] += subt[n][0];
	// 		arr[1] += subt[n][1];
	// 		dfs(e,n,tot_sub - subt[e] - 1 + extr);
...
void solve(){
	...
	ans = n;
	dfs(1,0);
	cout << min(subt[1][0],subt[1][1]) - 1 << endl;
	// int arr[2] = {0,0};
	// dfs(1,0,arr);
	// for(int i =  1; i <=n; ++i){
	// 	dfs(i,0);
	// 	ans = min(ans, subt[i] - 1);
	// }
	// cout << ans << endl;
}
int main(){
