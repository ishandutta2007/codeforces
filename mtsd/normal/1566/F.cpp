...
int main(){
    ...
    rep(zz,T){
        ...
        rep(i,p.size()){
            X.push_back(MP(p[i].first,+(i+1)));
            X.push_back(MP(p[i].second,-(i+1)));
        }
        sort(all(X));
        vector<int>s(p.size()+1),t(p.size()+1);
        rep(i,(int)X.size()-1){
            if(abs(X[i].second)==abs(X[i+1].second)&&X[i].first==X[i+1].first)swap(X[i],X[i+1]);
        }
        rep(i,X.size()){
            auto [x,id] = X[i];
            if(id>0){
        ...
        rep(i,n){
            ...
            if(i==0){
                ...
                if(id==0){
                    ...
                    chmin(dp[i][0],dp[i-1][3] + d);
                    chmin(dp[i][1],dp[i-1][4] + d*2);
                    chmin(dp[i][1],dp[i-1][0]);
                    chmin(dp[i][2],dp[i-1][3] + d);
                }else if(id>0){
                    int SS = s[id];
                    int TT = t[id];
                    if(SS>TT) TT = SS;
                    dbg(x,id,TT);
                    chmin(dp[i][0],dp[i-1][1]+d);
                    chmin(dp[i][0],dp[i-1][2]+d*2);
                    chmin(dp[i][1],dp[i-1][1]+d);
                    chmin(dp[i][2],dp[i-1][2]+d*2);
                    chmin(dp[i][3],dp[i-1][3]+d);
                    chmin(dp[i][4],dp[i-1][4]+d*2);
                    
                    chmin(dp[TT][3],dp[i-1][0]);
                    chmin(dp[TT][4],dp[i-1][0]);
                }else{
                    chmin(dp[i][0],dp[i-1][0]);
                    chmin(dp[i][1],dp[i-1][1]+d);
                    chmin(dp[i][2],dp[i-1][2]+d*2);
            ...
            dbg(i,x,dp[i][0],dp[i][1],dp[i][2],dp[i][3],dp[i][4]);
        }
        cout << dp[n-1][0] << "\n";
    }
