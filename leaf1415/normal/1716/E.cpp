...
{
	if((b & (1<<pos[i])) == 0){
		seg[i].fi = max(seg[i*2].fi, seg[i*2+1].fi);
		chmax(seg[i].fi, seg[i*2].se.se + seg[i*2+1].se.fi);
		seg[i].se.fi = max(seg[i*2].se.fi, sum[i*2]+seg[i*2+1].se.fi);
		seg[i].se.se = max(seg[i*2+1].se.se, sum[i*2+1]+seg[i*2].se.se);
	}
	else{
		seg[i].fi = max(seg[i*2].fi, seg[i*2+1].fi);
		chmax(seg[i].fi, seg[i*2].se.fi + seg[i*2+1].se.se);
		seg[i].se.fi = max(seg[i*2+1].se.fi, sum[i*2+1]+seg[i*2].se.fi);
		seg[i].se.se = max(seg[i*2].se.se, sum[i*2]+seg[i*2+1].se.se);
	}
}
...
int main(void)
{
	...
	rep(i, 1, (1<<19)-1) pos[i] = digitnum(i, 2) - 1;
	rep(i, 0, N-1) seg[N+i] = T(max(0LL, a[i]), P(max(0LL, a[i]), max(0LL, a[i]))), sum[N+i] = a[i];
	per(i, N-1, 1) merge(i), sum[i] = sum[i*2] + sum[i*2+1];
	
	rep(i, 0, N-1){
		ans[b] = seg[1].fi;
		ll nb = (i+1)^((i+1)>>1), d = nb^b;
		b = nb;
		per(j, d*2-1, 1) merge(j);
	}
