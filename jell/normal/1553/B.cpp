#line 1 "codeforces-workspace\\contest\\1553\\b\\b.cpp"
...
#line 16 "codeforces-workspace\\contest\\1553\\b\\b.cpp"
//*/
...
void main() {
  ...
  for (auto j : range(t.size())) {
    if (j >= s.size()) break;
    for (auto i : range(j, s.size())) {
      if (s.substr(i - j, j) == t.substr(0, j)) {
        // if(t.substr(j)==rev)
        auto u = s.substr(0, i + 1);
        reverse(begin(u), end(u));
        auto a = t.substr(j);
        ans += a == u.substr(0, a.size());
      }
    }
  }
