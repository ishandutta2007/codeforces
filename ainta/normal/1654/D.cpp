...
struct TTT{
	...
	void Add(int nd, int b, int e, int s, int l, mint x){
		if(s>l)return;
		if(s<=b&&e<=l){
			Add2(nd,x);
			return;
	...
	void Add3(int nd, int b, int e, int s, int l, int x, mint y){
		//printf("%d %d %d %d\n",s,l,x,y.v);
		if(x<0){
			mint iy = 1;
			iy /= y;
...
int main(){
    ...
    while(T--){
		...
		rng(i,1,n){
			...
			for(auto [b,e,c] : U[i]){
				IT2.Add3(1,1,n,b,e,c,i);
			}
			IT2.Add3(1,1,n,1,n,-IT.Mn[1],i);
			for(auto [b,e,c] : U[i]){
				IT.Add(1,1,n,b,e,-c);
			}
		}
