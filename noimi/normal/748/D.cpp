...
signed main() {
    ...
    for(auto [s, v] : mp2) {
        string ss = s;
        reverse(all(ss));
        if(se.count(s) or se.count(ss)) continue;
        se.emplace(s);
        if(ss == s) {
            ...
            if(f) chmax(mi, v.back()), ans -= v.back();
        } else {
            auto w = mp2[ss];
            sort(all(v), greater<ll>());
            sort(all(w), greater<ll>());
            rep(i, (int)v.size() - 1) v[i + 1] += v[i];
            rep(i, (int)w.size() - 1) w[i + 1] += w[i];
            ll ma = 0;
            rep(i, min(v.size(), w.size())) chmax(ma, v[i] + w[i]);
            ans += ma;
        }
    }
    cout << ans + mi << endl;
