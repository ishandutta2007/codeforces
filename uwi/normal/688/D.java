...
public class B2 {
	...
	void solve()
	{
		int n = ni(), K = ni();
		int[] c = na(n);
		int[] primes = sieveEratosthenes((int)Math.sqrt(K));
		int[] f = factorRoughly(K, primes);
		
		for(int fp : f){
			if(Arrays.stream(c).anyMatch(v -> v % fp == 0))continue;
			out.println("No");
			return;
		}
		out.println("Yes");
	...
	public static int[] factorRoughly(int n, int[] primes)
	{
		int[] ret = new int[9];
		int rp = 0;
		for(int p : primes){
