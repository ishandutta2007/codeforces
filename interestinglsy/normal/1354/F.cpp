...
int n,k;
struct Info{ ll a,b; int id; }p[MAXN], inpP[MAXN];

bool vis[MAXN][MAXN]; ll mem[MAXN][MAXN]; bool route[MAXN][MAXN];
#define NOW mem[pos][c]
#define VIS vis[pos][c]
#define ROUTE route[pos][c]
ll Solve( int pos , int c ){
	if( pos == n ) return c == k-1 ? 0 : -LINF;
	if(VIS) return NOW;
	VIS = true;
	ROUTE = 0;
	ll ans = Solve(pos+1,c) + (k-1)*p[pos].b;
	if( c != k-1 ){
		ll tans = Solve(pos+1,c+1) + p[pos].a + c*p[pos].b;
		if( tans > ans ){
			ROUTE = 1;
			ans = tans;
		}
...
void Solve(){
	...
	For(omit,n){
		Ms(vis);
		For(i,n-1) p[i] = i < omit ? inpP[i] : inpP[i+1];
		pli tans = {Solve(1,0)+inpP[omit].a+(k-1)*inpP[omit].b,omit};
		Mymax(omitAns,tans);
	}
	Ms(vis);
	int omit = omitAns.S;
	For(i,n-1) p[i] = i < omit ? inpP[i] : inpP[i+1];
	Solve(1,0);
	static vc<int> ansl; ansl.clear();
	static bool chosen[MAXN]; Ms(chosen);
	int c = 0;
	For(pos,n-1){
		int realPos = pos < omit ? pos : pos+1;
		if(route[pos][c]){
			chosen[realPos] = true;
			++c;
		}else{
			chosen[realPos] = false;
		}
	}
	For(i,n) if( i != omit and chosen[i] ){
		ansl.Pb(inpP[i].id);
	}
	For(i,n) if( i != omit and !chosen[i]){
		ansl.Pb(inpP[i].id);
...
int main(){
	...
	For(TwT,T){
		Read(n,k);
		For(i,n){
			Read(inpP[i].a,inpP[i].b);
			inpP[i].id = i;
		}
		sort(inpP+1,inpP+1+n,[]( Info x , Info y ){ return x.b < y.b; });
		Solve();
	}
