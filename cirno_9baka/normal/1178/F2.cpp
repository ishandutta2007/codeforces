...
unsigned long long mult(unsigned a,unsigned b)
{
...
long long dfs(int sl,int sr)
{
	...
	for(i=l[y]+1;i<=r[y];++i)
		...
		{
			if(la<i-1)
				ans=mult(ans,dfs(la+1,i-1));
			la=i;
		}
	for(i=l[y]-1;i>=sl;--i)
		if(!s[i-sl])
			s1=Mod(s1+mult(dfs(i+1,l[y]-1),dfs(sl,i)));
	s1=(s1+dfs(sl,l[y]-1))%M;
	for(i=r[y];i<=sr;++i)
		if(!s[i-sl])
			s2=Mod(s2+mult(dfs(r[y]+1,i),dfs(i+1,sr)));
	return f[sl][sr]=mult(mult(s1,s2),ans);
}
int main()
{
	gi(n),gi(m);
