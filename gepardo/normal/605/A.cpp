/*-------------------( alex256 )--------------------+
|    Contest: Codeforces Round 335 (Div. 1)         |
|       Task: A                                     |
|       Date: 09. 12. 2015                          |
+--------------------------------------------------*/
#include <algorithm>
...
int main()
{
    ...
    vector <int> g, ps;
    g.push_back(1), ps.push_back(1);
    for (int i = 1; i < n; i++)
        if (p[i] > p[i - 1]) g[g.size() - 1]++, ps[ps.size() - 1]++;
        else g.push_back(1), ps.push_back(ps[ps.size() - 1] + 1);
    int ans = INF;
    for (int i = 0; i < g.size(); i++) ans = min(ans, n - g[i]);
    cout << ans;
    //ovector(g, g.size(), " "); cout << endl;
    //ovector(ps, ps.size(), " "); cout << endl;
