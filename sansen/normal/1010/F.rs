fn run() {
    ...
    for &v in topo.iter().rev() {
        ...
        size[v] += g[v].iter().map(|u| size[*u]).sum::<usize>();
    }
    let solve_path = recurse(|rec, mut f: Vec<Vec<M>>| -> (Vec<M>, Vec<M>) {
        if f.len() == 1 {
            let f = f[0].clone();
            return (f.clone(), f);
        }
        let all = f.iter().map(|f| f.len()).sum::<usize>();
        let mut m = 1;
        let mut size = f[0].len();
        while size + f[m].len() < all - size - f[m].len() {
            size += f[m].len();
            m += 1;
        }
        let g = f.split_off(m);
        let (a, b) = rec(f);
        let (c, d) = rec(g);
        (a.add(&b.multiply(&c)), b.multiply(&d))
    });
    let product = |f: Vec<Vec<M>>| -> Vec<M> {
        let mut h = BinaryHeap::new(|a: &Vec<M>, b: &Vec<M>| a.len() < b.len());
        for f in f {
            h.push(f);
        }
        while h.len() > 1 {
    ...
    let fast = recurse(|rec, mut v: usize| -> Vec<M> {
        let mut f = vec![];
        while !g[v].is_empty() {
            let mut light = vec![vec![M::zero(), M::one()]];
            for &u in g[v][1..].iter() {
                light.push(rec(u));
            }
            f.push(product(light));
            v = g[v][0];
        }
        f.push(vec![M::zero(), M::one()]);
        let mut ans = solve_path(f).0;
        ans[0] += M::one();
        ans
    });
    let dp = fast(root);
    let mut ans = M::zero();
    let mut nu = M::one();
    let pc = precalc::Precalc::new(n + 1);
    for i in 1..dp.len() {
        ans += nu * pc.ifact(i - 1) * dp[i];
        nu *= M::from(x + i);
    }
    println!("{}", ans);
}
