...
li dp[K][N];
li C(li l, li r) {
  if(l==1) return r*(r+1)/2;

  li v = 0;
  for(li fr = l, to; fr <= r; fr = to + 1) {
    li d = r/fr;
    to = r/d;
    v += (to-fr+1) * sumtot[d];
  }
...
void solve(li k, li l, li r, li optl, li optr) {
  if(l > r) return;
  li m = (l+r)/2;

  li optto = min(m,optr);
  for(li i = (optl); i <= (li)(optto); ++i) vs[i] = dp[k-1][i-1];

  for(li fr = optl, to; fr <= m; fr = to + 1) {
    li d = m/fr;
    to = m/d;
    for(li i = (optl); i <= (li)(min(optto,to)); ++i) {
      vs[i] += (to-max(i,fr)+1) * sumtot[d];
    }
  }

  if(optl == 1) vs[0] += m*(m+1)/2;

  li best = 1e18;
  int bi = -1;
  for(li i = (optl); i <= (li)(optto); ++i) if(vs[i]<best) { best=vs[i]; bi=i; }
...
void pre(){
  sumtot[0] = 0;
  for(li i = 0; i < (li)(N); ++i) sumtot[i+1] = sumtot[i]+primes.totient(i+1);
  for(li n = (1); n <= (li)(N-1); ++n) dp[1][n] = C(1,n);
  for(li k = (2); k <= (li)(K-1); ++k) {
    solve(k,1,N-1,1,N-1);
  }
}
...
int main(){
  ...
  li t; cin>>t;
  for(li t_ = 0; t_ < (li)(t); ++t_) { li r = solve();
    cout << r << '\n';
  }
  cout << flush;
  return 0;
