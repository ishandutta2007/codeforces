...
begin
 ...
 begin
  if aa > bb then aa := aa mod bb else bb := bb mod aa;
 end;
 a := a div (aa + bb);
 b := b div (aa + bb);
 d := min(n div a, m div b);
 a := a * d;
 b := b * d;
 if ((2 * x >= a) and (2 * x <= n + n - a)) then aa := (2 * x - a) div 2 else
 if 2 * x < a then aa := 0 else
 aa := n - a;
 if ((2 * y >= b) and (2 * y <= m + m - b)) then bb := (2 * y - b) div 2 else
 if 2 * y < b then bb := 0 else
 bb := m - b;
 writeln(aa, ' ', bb, ' ', aa + a, ' ', bb + b);
end.