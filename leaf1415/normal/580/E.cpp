...
const ll L = 2; ll hmod[] = {950009857, 943718401, 935329793, 924844033, 469762049, 167772161};
struct Hash{
	ll h[L];
	Hash(){ for(int i = 0; i < L; i++) h[i] = 0;}
	Hash(ll x){ for(int i = 0; i < L; i++) h[i] = x;}
...
struct LazySegTree{
	...
	void eval(int l, int r, int k){
		if(delay[k] != NoDelay()){
			seg[k] = Hash(delay[k]) * Y[r-l+1] * X[l];
			if(l < r){
	...
	SEG query(int a, int b){
		if(a > b) return Ident();
		return query(a, b, 1, 0, (1<<size)-1) * X[a].inverse();
	}
};
...
int main(void)
{
	...
	X[0] = Hash(1);
	rep(i, 1, 200000) X[i] = X[i-1] * Hash(100000);
	rep(i, 0, 16) Y[1<<i] = (X[1<<i]-Hash(1)) * (X[1]-Hash(1)).inverse();
	
	seg.init();
	reps(i, s) seg.seg[(1<<17)+i] = Hash(s[i])*X[i];
	per(i, (1<<17)-1, 1) seg.seg[i] = seg.ope(seg.seg[i*2], seg.seg[i*2+1]);
	...
	rep(i, 1, Q){
		cin >> t >> l >> r >> d;
		l--, r--;
		if(t == 1) seg.add(l, r, d+'0');
		else{
			bool ans = (seg.query(l, r-d) == seg.query(l+d, r));
			if(ans) cout << "YES" << "\n";
			else cout << "NO" << "\n";
		}
	}
