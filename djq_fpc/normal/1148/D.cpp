...
int n,m,i,j,f[600005],pref[600005],g[600005],preg[600005],a[600005],b[600005];
...
int main()
{
	...
	fz1(i,n){
		if(a[i]<b[i]){
			sa.push_back(make_pair(-b[i],i));
		}
	}
	fz1(i,n){
		if(a[i]>b[i]){
			sb.push_back(make_pair(a[i],i));
		}
	}
	...
	if (sa.size()>sb.size()){
		printf("%d\n",sa.size());
		ff(sa,it) printf("%d ",it->second);
	}
	else{
		printf("%d\n",sb.size()); 
		ff(sb,it) printf("%d ",it->second);
	}
	return 0;
}