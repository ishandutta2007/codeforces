...
class Graph(val n:Int, val m:Int, val directed:Boolean, val L:IntArray) {
    ...
    fun sizeRecur(root:Int){
        go(root,-1)
    }
    tailrec fun findCentroid(newRoot:Int,totalSize:Int,banned:Int):Int{
        var heaviestChild = -1
        var heaviestSize = -1
        var ok:Boolean = true
        NS(newRoot){
    ...
    val localdepth = IntArray(n)
    val pathsum = LongArray(n)
    val rootheavypath = LongArray(n)
    val leafheavypath = LongArray(n)

    fun local_leaf_order(v:Int, from:Int, res:MutableList<Int>){
        NS(v) { w ->
            if (w == from || centroidLevel[w] != 0) return@NS
            local_leaf_order(w,v,res)
        }
        res.add(v)
    }
    fun getLocalInfo(v:Int):MutableList<MutableList<Int>>{
        val ret = mutableListOf<MutableList<Int>>()
        NS(v) { w ->
            if (centroidLevel[w] != 0) return@NS
    ...
    fun maintain(v:Int, from:Int){
        localdepth[v] = localdepth[from] + 1
        pathsum[v] = pathsum[from] + L[v]
        rootheavypath[v] = rootheavypath[from] + pathsum[v]
        leafheavypath[v] = leafheavypath[from] + (localdepth[v]).toLong() * L[v]
        NS(v) { w ->
            if (w == from || centroidLevel[w] != 0) return@NS
            maintain(w,v)
        }
    }
    fun local_dfs(v:Int, from:Int){
        NS(v) { w ->
            if (w == from || centroidLevel[w] != 0) return@NS
            local_dfs(w,v)
        }
    ...
    var ret = 0L
    fun centroidSolve(current:Int,size:Int){
        localdepth[current] = 0
        pathsum[current] = L[current].toLong()
        rootheavypath[current] = pathsum[current]
        leafheavypath[current] = 0
        NS(current) { w ->
            if (centroidLevel[w] != 0) return@NS
            maintain(w,current)
        }
        val comps = getLocalInfo(current)
        comps.add(mutableListOf(current))
        var ds = CHT_full()
        for(c in comps){
            for(v in c){
                val q1 = ds.query(1 + localdepth[v].toLong()) ?: continue
                ret = maxOf(ret, q1 + rootheavypath[v])
            }
            for(v in c){
                ds.add(pathsum[v] - L[current], leafheavypath[v])
            }
        }
        ds = CHT_full()
    ...
    fun centroidStep(current:Int,parent:Int){
        sizeRecur(current) // does not start from centroid
        val compSize = sizesCentroid[current]
        val newCentroid = findCentroid(current,sizesCentroid[current],-1)
        centroidSolve(newCentroid,compSize)
...
//        }
//        val L = getline(n)
//        val d = G.calculateDepth()
//        val topheavy = LongArray(n)
//        G.rootFirst {
//            if(it != G.root) topheavy[it] += topheavy[G.parent[it]]
//            topheavy[it] += d[it].toLong() * L[it]
//        }
//        val real = Array(n){ mutableListOf<Pair<Long,Int>>() }
//        //topheavy value and depth
//        var ans = 0L
