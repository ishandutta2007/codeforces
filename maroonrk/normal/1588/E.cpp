...
void slv(){
	...
	rep(i,n){
		rep(j,n){
			pi dif=sub(ps[i],ps[j]);
			if(norm(dif)>sq(R))ls[i][j]=true;
		}
	}
	vc<pi> qs(n);
	rep(i,n){
		rep(j,n)qs[j]=sub(ps[j],ps[i]);
		rng(j,i+1,n){
			int atmost=floor(sqrtl(norm(qs[j]))*R);
			B chk=ls[i]&ls[j];
			bool ok=true;
			int w=norm(qs[j]);
			for(int k=chk._Find_first();k<n;k=chk._Find_next(k)){
				int a=abs(crs(qs[j],qs[k]));
				if(a>atmost){
					ok=false;
					break;
				}
