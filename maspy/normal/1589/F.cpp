...
void solve() {
  ...
  ll V = (K << N) + 1;
  ll root = 0;
  auto idx = [&](ll k, ll s) -> ll { return ((k << N) | s) + 1; };
  Graph<int, 1> G(V);

  FOR(k, K) {
    bool ok = [&]() -> bool {
      FOR(i, N) if (pos[i][k][0] == -1) return 0;
      return 1;
    }();
    if (!ok) continue;
    G.add(root, idx(k, 0));
  ...
  FOR(k, K) FOR(s, 1 << N) {
    vi I(N);
    FOR(i, N) I[i] = ((s & 1 << i) ? pos[i][k][1] : pos[i][k][0]);
    if (MIN(I) == -1) continue;
    FOR(to, K) {
      ll t = [&]() -> ll {
        ll t = 0;
        FOR(i, N) {
          if (pos[i][to][0] == -1) return -1;
          if (pos[i][to][0] > I[i]) continue;
          if (pos[i][to][1] == -1) return -1;
          if (pos[i][to][1] <= I[i]) return -1;
          t |= 1 << i;
        }
        return t;
      }();
      if (t == -1) continue;
      G.add(idx(k, s), idx(to, t));
    }
  }
  ...
  vi DP(V, -INF), PAR(V, -1);
  DP[0] = 0;
  for (auto&& v: toposort(G)) {
    if (DP[v] == -INF) continue;
    for (auto&& e: G[v]) {
      if (chmax(DP[e.to], DP[v] + 1)) PAR[e.to] = v;
    }
  }
  ll v = max_element(all(DP)) - DP.begin();
  vi P = {v};
  while (P.back()) P.eb(PAR[P.back()]);
  P.pop_back();
  reverse(all(P));
  string ANS;
  for (auto&& x: P) {
