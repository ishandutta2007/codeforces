...
public:
    void solve() {
        ...
        mod = MODF;
        arr2d<modint> answer(n, n);
        arr<bool> onPath(n, false);
        ...
        for (int i : range(n)) {
            ...
            for (int j : range(i, n)) {
                ...
                for (int k : order) {
                    ...
                    cur = 0;
                    for (auto& e : graph[k]) {
                        if (dst(i, k) - 1 == dst(i, e.to) && dst(j, k) - 1 == dst(j, e.to)) {
                            cur++;
                        }
                    }
                    res *= cur;
