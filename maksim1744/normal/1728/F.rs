...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let mut b: Vec<(i32, i32)> = Vec::with_capacity(n * n);
    ...
    let mut b0: Vec<i32> = Vec::with_capacity(b.len());
    for &(x, _) in b.iter() {
        b0.push(x);
    }
    b0.dedup();

    let mut pos: Vec<i32> = vec![0; b0.len()];
    let mut ind = 0;
    for (i, (x, _ai)) in b.iter_mut().enumerate() {
        if b0[ind] != *x {
            ind += 1;
            pos[ind] = i as i32;
        }
    ...
    fn matching_dfs(v: usize, edges: &Vec<(i32, i32)>, pos: &Vec<i32>, from: &mut Vec<i32>, u: &mut Vec<bool>) -> bool {
        u[v] = true;
        for &(_v, k) in edges[pos[v] as usize..].iter().take_while(|&&(v1, _k)| v1 == v as i32) {
            let k = k as usize;
            if from[k] == -1 || (!u[from[k] as usize] && matching_dfs(from[k] as usize, edges, pos, from, u)) {
                from[k] = v as i32;
                return true;
            }
        }
    ...
    let mut u = vec![false; b.len()];
    let mut from: Vec<i32> = vec![-1; b.len()];
    for v in 0..b.len() {
        if matching_dfs(v, &edges, &pos, &mut from, &mut u) {
            u.fill(false);
        }
    }

    let mut ans: i64 = 0;
    for x in from.into_iter().filter(|&x| x >= 0) {
        ans += b[x as usize] as i64;
    }
    outln!(ans);
}
