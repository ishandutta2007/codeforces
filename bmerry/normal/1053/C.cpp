...
public:
    ...
    {
        size_type p = 1;
        while (p * 2 < values.size())
            p <<= 1;
        size_type ans = 0;
        T cur = identity;
        for (; p > 0; p >>= 1)
        {
            if (ans + p < values.size() && op(cur, values[ans + p]) <= value)
            {
                ans += p;
                cur = op(cur, values[ans]);
            }
        }
        return ans;
    }
...
int main(int argc, const char **argv)
{
    ...
    for (int i = 0; i < Q; i++)
    {
        ...
        {
            x--;
            ll w1 = bw.sum_exclusive(y);
            ll w0 = bw.sum_exclusive(x);
            ll wh = w0 + (w1 - w0) / 2;
            int idx = bw.upper_bound(wh);
            ll p = a[idx];
            wh = bw.sum_exclusive(idx);
            mr ansL = mr(wh - w0) * (p - idx) - (bws.sum_exclusive(idx) - bws.sum_exclusive(x));
            mr ansR = (bws.sum_exclusive(y) - bws.sum_exclusive(idx)) - mr(w1 - wh) * (p - idx);
            mr ans = ansL + ansR;
            cout << ans << '\n';
        }
    }
