...
vector<mint> lagrange_interpolation(vector<mint> x, vector<mint> y) {
    ...
    rep(i,0,n) if(x[i]!=0)xinv[i] = x[i].inv();
    vector<mint> ret(n+1);  // f(x)
    rep(i,0,n){
        if(y[i]==0) continue;
        // 0
        if(x[i] == 0) {
            rep(j,0,n) ret[j] += dp[cur][j+1] * y[i];
        } else {
...
signed main(){fastio
    ll n;cin>>n;
    vl_l e(n-1);
    rep(i,0,n-2){cin>>e[i].fi>>e[i].se;e[i].fi--;e[i].se--;}
    vvi a(n-1,vi(n-1,-1));
    rep(i,0,n-2)a[i][i] = n-1;
    vi y(n);
    vi x(n);rep(i,0,n-1)x[i] = i+1;
    rep(i,0,n-1){
        y[i] = det(a);
        for(auto x:e){
            if(x.fi!=n-1){
            ...
            if(x.fi!=n-1 && x.se!=n-1){
                a[x.fi][x.se]--;
                a[x.se][x.fi]--;
            }
        }
    }
    auto C=lagrange_interpolation(x,y);
    for(auto x:C)cout<<x.val()<<" ";
