...
int main() {
	...
	k=2000;
	poly F(k+1);
	for (int i=1;i<=k;i++) F[i]=ivjc[i];
	dp[0]={1};
	for (int i=1;i<=k;i++) {
		dp[i]=dp[i-1]*F;
	//	printf("%d\n",i);
		while ((int)dp[i].size()>k+1) dp[i].pop_back();
	}
	read(T);
	while (T--) {
		read(n),read(m),read(k);
		int ans=0,ivm=ksm(m,mod-2),CC=ksm(m,n);
		for (int i=1;i<=k&&i<=n;i++) {
			CC=(ll)CC*(n-i+1)%mod*ivm%mod*((m+1)/2)%mod;
			int tmp=(ll)CC*ivjc[i]%mod;
			tmp=(ll)tmp*jc[k]%mod*dp[i][k]%mod;
			update(ans,tmp);
		}
		printf("%d\n",ans);
	}
