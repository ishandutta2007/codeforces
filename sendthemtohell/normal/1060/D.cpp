...
inline void solve() {
	set<pair<pair<int, int>, int>> fs;
	set<pair<pair<int, int>, int>> sc;
	int n;
	cin >> n;
	ll ans = 0;
	...
	cin >> v;
	for (int i = 0; i < n; ++i) {
		fs.insert({{-v[i].x, -v[i].y}, i});
		sc.insert({{-v[i].y, -v[i].x}, i});
	}
	while (!fs.empty()) {
		auto kek1 = *fs.begin();
		auto kek2 = *sc.begin();
		...
		sc.erase(sc.begin());
		if (kek1.y == kek2.y) {
			ans += max(-kek1.x.x, -kek1.x.y) + 1;
		}
		else {
			fs.erase({ {kek2.x.y, kek2.x.x}, kek2.y });
			sc.erase({ { kek1.x.y, kek1.x.x }, kek1.y });
			ans += max(-kek1.x.x, -kek2.x.x) + 1;
			fs.insert({ {kek2.x.y, kek1.x.y}, n });
			sc.insert({ { kek1.x.y, kek2.x.y }, n });
			++n;
		}
	}
	cout << ans << endl;
