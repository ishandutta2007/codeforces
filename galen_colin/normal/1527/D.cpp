...
ll dfs(ll v, ll p) {
	st[v] = ++tt;
	sz[v] = 1;
	for (ll x: edges[v]) if (x != p) sz[v] += dfs(x, v);
	en[v] = tt;
	return sz[v];
}

bool par(ll a, ll b) { // b parent of a
	return st[b] <= st[a] && en[a] <= en[b];
}

bool between(ll a, ll b, ll c) { // b between a and c
	ll v = lca.get_lca(a, c);
	return par(b, v) && (par(a, b) || par(c, b));
}
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i <= n + 5; i++) c[i] = 0;
	
	c[1] = n * (n - 1) / 2;
	
	dfs(a, -1);
	for (ll x: edges[a]) c[1] -= sz[x] * (sz[x] - 1) / 2;
			
	for (ll i = 1; i < n; i++) {
		if (loc.find(i) == loc.end()) break;
		
		ll x = loc[i];
		// cout << x << endl;
		if (between(x, a, b)) a = x;
		else if (between(a, b, x)) b = x;
		else if (!between(a, x, b)) break;
		
		// cout << a << " " << b << " " << relsz(a, b) << endl;
		
		c[i + 1] = relsz(a, b);
	}
	
	c[0] = n * (n - 1) / 2;
	for (ll i = 0; i < n; i++) c[i] -= c[i + 1];
