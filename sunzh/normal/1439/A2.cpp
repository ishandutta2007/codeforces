...
void solve(int x,int y){
	if(Map[x][y]){
		if(Map[x+1][y]&&Map[x+1][y+1]){
			++ans;Map[x][y]^=1;Map[x+1][y+1]^=1;Map[x+1][y]^=1;
			vector<PII>v;v.ep(x,y);v.ep(x+1,y+1);v.ep(x+1,y);
			opt.ep(v);
		}
		else{
			++ans;Map[x][y]^=1;Map[x+1][y+1]^=1;Map[x][y+1]^=1;
			vector<PII>v;v.ep(x,y);v.ep(x+1,y+1);v.ep(x,y+1);
			opt.ep(v);
		} 
	}
