...
_ n, m, x, y, p, fa[maxn], dp[maxn], dl[maxn], op, cnt;
char a[maxn];
v_ g[maxn], h[maxn], b[maxn];
v(_p) lg;
b(maxn) v;
...
inline void AC() {
    ...
    for(; m--; ) {
        ...
        if(op == 1) {
            ...
            _ sz = g[p].size()-dl[p];
            ++dl[p];
            lg.push_back(mp(x, sz));
        } else {
            p = fa[x];
            _ pa = lower_bound(g[p].begin(), g[p].end(), x)-g[p].begin(), pb = upper_bound(g[p].begin(), g[p].end(), y)-g[p].begin()-1, c = 0;
            _ sz = pb-pa+1;
            for(_p i : lg) if(x <= i.first && i.first <= y) {
                if(fa[i.first] == p) {
                    --c;
                    --sz;
                }
        ...
        if(lg.size() > big) {
            ...
            for(_ i = 0; i <= n; ++i) {
                dl[i] = 0;
                h[i].clear();
                for(_ j : g[i]) if(!v[j]) h[i].push_back(j);
                swap(h[i], g[i]);
            }
            dfs(0);
        }
...
void dfs(_ x) {
    ...
    for(_ i = 0; i < g[x].size(); ++i) {
        dfs(g[x][i]);
        b[x].push_back(dp[g[x][i]]);
    }
    for(_ i : b[x]) s += i;
    for(_ i = 1; i < b[x].size(); ++i) b[x][i] += b[x][i-1];
    dp[x] = s+sz*(sz-1)/2+1;
    return;
}