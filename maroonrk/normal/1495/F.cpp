...
void slv(){
	...
	rep(i,q){
		...
		if(*itr==v){
			int x=*prev(itr),y=*next(itr);
			buf.pb({x,v,i*2+1});
			buf.pb({v,y,i*2+1});
			buf.pb({x,y,i*2});
			s.erase(itr);
		}else{
			int x=*prev(itr),y=*itr;
			buf.pb({x,v,i*2});
			buf.pb({v,y,i*2});
			buf.pb({x,y,i*2+1});
			s.insert(v);
	...
	vi sum{0};
	for(auto v:a)sum.pb(sum.back()+v);
	sort(all(buf));
	seglazy<N> seg(vi(n+1,0));
	vi idx{n};
	per(x,n){
		while(p[x]>p[idx.back()])idx.pop_back();
		int y=idx.back();
		dmp2(x,y);
		idx.pb(x);
		int val=sum[y]-sum[x]-b[x];
		dmp(val);
		int w=seg.point_get(y).mx;
		if(val>w){
			seg.ch(y,n+1,&N::add,val-w);
		}
		while(si(buf)&&buf.back().x==x){
			auto cur=buf.back();buf.pop_back();
			int res=sum[cur.y]-sum[cur.x]-seg.point_get(cur.y).mx;
			dmp2(cur.x,cur.y,res,seg.point_get(cur.y).mx);
			if(cur.i%2==0)ans[cur.i/2]+=res;
			else ans[cur.i/2]-=res;
		}
	}
