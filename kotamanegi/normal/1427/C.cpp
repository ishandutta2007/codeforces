...
void solve() {
	...
	inputs.push_back(make_tuple(0, 1, 1));
	REP(i, n) {
		int a, b, c;
		cin >> a >> b >> c;
	...
	for (int i = 1; i <= n; ++i) {
		dp[i] = -1e9;
		for (int q = max(0LL, i - 2500LL); q < i; ++q) {
			int dist = abs(get<1>(inputs[i]) - get<1>(inputs[q])) + abs(get<2>(inputs[i]) - get<2>(inputs[q]));
			if (get<0>(inputs[i]) - get<0>(inputs[q]) >= dist) {
				dp[i] = max(dp[i], dp[q] + 1LL);
			}
		}
	}
	int ans = 0;
