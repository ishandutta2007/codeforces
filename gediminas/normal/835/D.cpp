...
int main () {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = i + 1; j < n; j++) {
			hashiukas[i][j] = (hashiukas[i][j - 1] * 26 + a[j] - 'a') % 43841547845541059;
		}
	}
	...
	for (int i = 2; i < n; i++) {
		for (int j = 0; j + i < n; j++) {
			if (a[j] == a[j + i] and poli[0][j + 1][j + i - 1]) {
				poli[0][j][j + i] = true;
				kie[0]++;
			}
		}
	...
	for (int k = 1; k < 13; k++) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = i + (1 << k) - 1; j < n; j++) {
				...
				if (hashiukas[i][i + ilg] == hashiukas[j - ilg][j] and poli[(k - 1) % 2][i][i + ilg] and poli[(k - 1) % 2][j - ilg][j]) {
					poli[k % 2][i][j] = true;
					kie[k]++;
				}
			}
