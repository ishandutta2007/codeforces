...
ll ask(int l, int r) {
    if (onpc) {
        vector<int> v(realans.begin() + l, realans.begin() + r + 1);
        ll ans = 0;
...
void solve() {
    ...
    int k = r;
    int j = k - (ask(0, k) - ask(0, k - 1));
    int i = j - 1 - (ask(0, j - 1) - ask(0, j - 2));
    println("! ", i + 1, ' ', j + 1, ' ', k + 1);
    cout << flush;
}
...
signed main() {
    ...
    if (onpc) {
        int n = 10;
        realans = vector<int>(n);
        iota(all(realans), 1);
        int i = 3, j = 9, k = 10;
        reverse(realans.begin() + i - 1, realans.begin() + j - 1);
        reverse(realans.begin() + j - 1, realans.begin() + k); 
        dbg(realans);
    }
    int t;
    read(t);
