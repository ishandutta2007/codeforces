...
ipair getdir(int x1,int y1,int x2,int y2)
{
	int dx=x1-x2;
	int dy=y1-y2;
	...
	dx/=d;
	dy/=d;
	return min(MP(dx,dy),MP(-dx,-dy));
}
bool ck2(int x[],int y[],bool sq)
{
	if (getdir(x[0],y[0],x[1],y[1])!=getdir(x[2],y[2],x[3],y[3]))
		return false;
	if (getdir(x[1],y[1],x[2],y[2])!=getdir(x[3],y[3],x[0],y[0]))
		return false;
	if (dist2(x[0],y[0],x[1],y[1])!=dist2(x[2],y[2],x[3],y[3]))
		return false;
	if (dist2(x[1],y[1],x[2],y[2])!=dist2(x[3],y[3],x[0],y[0]))
		return false;
	if (dist2(x[0],y[0],x[1],y[1])+dist2(x[1],y[1],x[2],y[2])!=dist2(x[0],y[0],x[2],y[2]))
		return false;
	if (sq && dist2(x[0],y[0],x[1],y[1])!=dist2(x[0],y[0],x[3],y[3]))
		return false;
	return true;
}
bool ck(int sx[],int sy[],bool sq)
{
	int p[4];
	for (int i=0;i<4;i++) p[i]=i;
	do{
		int x[4],y[4];
		for (int i=0;i<4;i++) x[i]=sx[p[i]],y[i]=sy[p[i]];
		if (ck2(x,y,sq)) return true;
...
#endif
	int sx[8],sy[8];
	for (int i=0;i<8;i++) scanf("%d%d",&sx[i],&sy[i]);
	for (int set=0;set<two(8);set++) if (countbit(set)==4)
	{
		int x1[4],y1[4],x2[4],y2[4];
		int c1=0,c2=0;
		for (int i=0;i<8;i++)
			if (contain(set,i)) x1[c1]=sx[i],y1[c1++]=sy[i];
			else x2[c2]=sx[i],y2[c2++]=sy[i];
		if (ck(x1,y1,true) && ck(x2,y2,false))
		{
			printf("YES\n");
			for (int i=0;i<8;i++) if (contain(set,i)) printf("%d ",i+1);
			printf("\n");
			for (int i=0;i<8;i++) if (!contain(set,i)) printf("%d ",i+1);
			printf("\n");
			return 0;
