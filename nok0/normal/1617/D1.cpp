...
void main_() {
	...
	V<> ans(n, -1);
	REP(i, n) {
		if(i == cur or i == cur + 1) continue;
		cout << "? " << cur + 1 << " " << cur + 2 << " " << i + 1 << endl;
		INT(res);
		ans[i] = res;
	}
	V<> one, zero;
	REP(i, n) {
		if(ans[i] == -1) continue;
		if(one.empty() and ans[i] == 1) one.pb(i);
		if(zero.empty() and ans[i] == 0) zero.pb(i);
	}
	cout << "? " << one[0] + 1 << " " << zero[0] + 1 << " " << cur + 1 << endl;
	cin >> ans[cur];
	ans[cur + 1] = !ans[cur];
	V<> res_vec;
	REP(i, n) {
		if(ans[i] == 0) res_vec.pb(i);
	}
	ADD(res_vec);
	cout << "! " << SZ(res_vec) << " " << res_vec << endl;
	return;
}