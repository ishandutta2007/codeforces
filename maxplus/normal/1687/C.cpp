...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    set<pair<int, int>> bad, good, done;
    for (int i = 0; i < m; ++i) {
      int l = $ - 1, r = $;
      B[i][0] = l, B[i][1] = r;
      b[i] = 0;
      if (a[l])   bad.emplace(l, i), ++b[i];
      if (a[r])   bad.emplace(r, i), ++b[i];
      if (!b[i]) good.emplace(l, r);
    }
    while (good.size()) {
      auto [l, r] = *good.begin(); good.erase(good.begin());
      done.emplace(l, r);
      for (auto it = bad.lower_bound(pair{l, -1}); it != bad.end() && it->first <= r; ) {
        if (!--b[it->second]) {
          good.emplace(B[it->second][0], B[it->second][1]);
        }
        it = bad.erase(it);
      }
    }
