...
int _hiL[N*4],_loL[N*4],_hiR[N*4],_loR[N*4];
int *hiL=_hiL+N+N;
int *loL=_loL+N+N;
int *hiR=_hiR+N+N;
int *loR=_loR+N+N;

// inline int count(int L,int R,int X,int par) {
// 	dbg("COUNT",L,R,X,par);
// 	return (R-L+2)/2;
// 	// if(((X+R)&1)!=par) --R;
// 	// if(((X+L)&1)!=par) --L;
...
int32_t main() {
	...
	for(int par=0;par<2;par++) {
		memset(_hiL,0x80,sizeof _hiL);
		memset(_hiR,0x80,sizeof _hiR);
		memset(_loL,0x7f,sizeof _loL);
		memset(_loR,0x7f,sizeof _loR);
		for(int i=1;i<=n;i++) if(((x[i]+y[i])&1)==par) {
			int X=x[i]+y[i];
			int Y=x[i]-y[i];
			hiL[X]=max(hiL[X],Y);
			hiR[X]=max(hiR[X],Y);
		...
		for(int i=-N-N;++i<N+N;) hiL[i]=max(hiL[i],hiL[i-1]);
		for(int i=-N-N;++i<N+N;) loL[i]=min(loL[i],loL[i-1]);
		for(int i=N+N-1;--i>=-N-N;) hiR[i]=max(hiR[i],hiR[i+1]);
		for(int i=N+N-1;--i>=-N-N;) loR[i]=min(loR[i],loR[i+1]);
		for(int i=-N-N+1;i<N+N-1;i++) {
			if((i&1)==par) continue;
			int hi=min(hiL[i-1],hiR[i+1])-1;
			int lo=max(loL[i-1],loR[i+1])+1;
			if(lo>hi) continue;
			Z+=(hi-lo+2)>>1;
			// Z+=count(lo,hi,i,par);
		}
