...
template <typename T> struct DiagonalPoints {
    set<pair<T, T>> s;
    multiset<T> ans;
    DiagonalPoints() {
    ...
    void add(T x, T y) {
        auto rp = make_pair(x, y);
        auto it = s.lower_bound(rp);
        if(y <= it->se) return;
        bool flag = true;
        while(1) {
            it = prev(it);
            if(it->se <= y) {
                ...
                T x = p->fi + it->se, y = it->fi + q->se;
                if(flag) ans.erase(ans.lower_bound(y));
        ...
        auto [i, _] = s.emplace(x, y);
        auto p = prev(i), q = next(i);
        ans.emplace(p->fi + y);
        ans.emplace(x + q->se);
        if(flag) ans.erase(ans.lower_bound(p->fi + q->se));
    }
    T min() { return *begin(ans); }
};
int main() {
    INT(n);
    VV(int, a, 3, n);
    vi v;
    rep(i, 3) rep(j, n) v.eb(a[i][j]);
    UNIQUE(v);
    int m = si(v);
    vv(ll, d, m, 3, inf<int>);
    rep(i, 3) rep(j, n) { chmin(d[lb(v, a[i][j])][i], j + 1); }
    ...
    rep(i, m) {
        if(d[i][0] != inf<int>)
            t[d[i][0] - 1].eb(d[i][1], d[i][2]);
        else
            s.add(d[i][1], d[i][2]);
    }
    ll ans = n + s.min();
    rep3(i, n - 1, 0) {
        for(auto [x, y] : t[i]) s.add(x, y);
        chmin(ans, i + s.min());
    }
