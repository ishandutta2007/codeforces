...
void solve() {
  ...
  // 
  bool ok1 = [&]() -> bool {
    if (intersect_segment(A, B, W1, W2)) return false;
    if (intersect_segment(A, B, M1, M2)) {
      // 
      // 
      if ((B - A).det(M2 - M1) == 0) return true;
      if ((B - M1).det(B - A) == 0) return false;
      if ((B - M2).det(B - A) == 0) return false;
      return false;
    }
    return true;
  }();
  ...
  bool ok2 = [&]() -> bool {
    ...
    xy<ll> C = xy(A.x, -A.y);
    xy<ll> D = xy(B.x, -B.y);
    if (intersect_segment(B, C, W1, W2)) return false;
    if (intersect_segment(A, D, W1, W2)) return false;
    if (!intersect_segment(B, C, M1, M2)) return false;
    // 
    if ((C - B).det(M2 - M1) == 0) return false;
    //  ok
    if ((B - M1).det(B - C) == 0) return true;
    if ((B - M2).det(B - C) == 0) return true;
    return true;
  }();
  // print(ok1, ok2);
  YES(ok1 || ok2);
