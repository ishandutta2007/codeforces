...
void solve(){
    ...
    REP(i, m) {
        int k, pos;
        cin >> k >> pos;
        queries.push_back(make_tuple(k, pos, i));
    }
    sort(ALL(queries));
    reverse(ALL(queries));
    SegmentTree<int> seg(n, [](int a, int b) {return a + b; }, 0LL);
    REP(i, n) {
        seg.update(inputs[i].second, 1LL);
        while (queries.empty() == false && get<0>(queries.back()) == i + 1) {
            int target = get<1>(queries.back());
            int bot = -1;
            int top = n;
            while (abs(bot - top) > 1) {
                int mid = (bot + top) / 2LL;
                if (seg.query(0, mid) < target) {
                    bot = mid;
                }
                else {
                    top = mid;
            ...
            ans[get<2>(queries.back())] = gogos[bot];
            queries.pop_back();
        }
    }
    REP(i, m) {
