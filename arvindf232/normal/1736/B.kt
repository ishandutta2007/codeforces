...
fun main(){
    solve.cases{
		...
		val ret = mong
		for(i in 0..n){
			val left = if(i == 0) 0 else L[i-1]
			val right = if(i == n) 0 else L[i]
			if(left == 0){
				ret.add(right.toLong())
			}else if(right == 0){
				ret.add(left.toLong())
			}else{
				ret.add(lcm(left.toLong(),right.toLong()))
			}

		}
		val got = mong
		for(i in 0 until ret.lastIndex){
			got.add(gcd(ret[i], ret[i+1]))
		}
		val z = L.map { it.toLong() }
		if(z == got){
			put("YES")
		}else{
			put("NO")
