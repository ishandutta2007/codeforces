...
// size of array.
template <class T, size_t N> size_t size(T (&array)[N]) { return N; }
...
struct solver
{
    ...
    solver()
    {
        ...
        {
            vector<p32> vert(n); cin>>vert;
            p32 cent;
            {
                cent={(vert[0].first+vert[n/2].first),(vert[0].second+vert[n/2].second)};
            }
            for(int i=0,j=n/2; i*2<n; ++i,++j)
            {
                p32 tmp={(vert[i].first+vert[j].first),(vert[i].second+vert[j].second)};
                if(tmp!=cent)
                {
                    cout << "NO" << "\n";
                    return;
