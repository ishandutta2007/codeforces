...
int n,u,v,a[maxn],dep[maxn],fa[maxn][18],val[maxn][18],cur,curval;
vector<int> adj[maxn];
modint ans;

void dfs(int u, int pa){
    fa[u][0]=pa,val[u][0]=a[u];
    dep[u]=pa==-1?0:dep[pa]+1;
    rep2(i,1,18){
        if(fa[u][i-1]==-1) fa[u][i]=-1,val[u][i]=val[u][i-1];
        else fa[u][i]=fa[fa[u][i-1]][i-1],val[u][i]=__gcd(val[u][i-1],val[fa[u][i-1]][i-1]);
    }
...
signed main(){
    ...
    rep(n){
        ...
        while(1){
            if(cur==val[i][17]){
                ans+=modint(val[i][17]%MOD)*modint(dep[u]+1);
                break;
            }
            v=u;
            rep3(i,17,0){
                if(fa[u][i]==-1) continue;
                if(__gcd(cur,val[u][i])==cur){
                    u=fa[u][i];
                }
            }
            ans+=modint(cur%MOD)*modint(dep[v]-dep[u]);
            cur=__gcd(cur,a[u]);
        }
    }
    print1(ans);
