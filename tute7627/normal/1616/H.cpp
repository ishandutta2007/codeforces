...
void solve(){
  ...
  rep(i,0,n)two[i+1]=two[i]*2;
  auto rec=[&](auto &&f,int d,vector<vector<int>>v)->modint{
    if(d==-1){
      ll ret=0;
      rep(i,0,v.size())ret+=v[i].size();
      return two[ret];
    }
    if(v.size()==1&&v[0].empty())return 1;
    if(v.size()==2&&(v[0].empty()||v[1].empty())){
      return two[v[0].size()+v[1].size()];
    }
    if(k>>d&1){
      if(v.size()==1){
        vector<vector<int>>nv(2);
        for(auto z:v[0]){
          nv[z>>d&1].PB(z);
        }
        return f(f,d-1,nv);
      }
      vector<vector<vector<int>>>nv(2,vector<vector<int>>(2));
      rep(i,0,2){
        for(auto z:v[i]){
          nv[((z>>d)&1)^i][i].PB(z);
        }
      ...
      if(v.size()==1)ret-=1;
      //cout<<d spa v spa ret<<endl;
      return ret;
    }
  };
  cout<<rec(rec,29,vector<vector<int>>(1,a))-1<<endl;
}
int main(){
  cin.tie(nullptr);
