...
vector<Q> queries[101010];
vector<int> v[101010];
vector<int> vtr[301010];
int mem[101010 * 20 * 8];
int* getMem(int n) {
    static auto memPtr = mem;
    memPtr += n;
    return memPtr - n;
}
struct Fenw {
...
void init(int cur, int l, int r) {
    if (l == r) {
        ...
        init(dcur, l, m);
        init(dcur + 1, m + 1, r);
        vtr[cur].resize(vtr[dcur].size() + vtr[dcur + 1].size());
        std::merge(all(vtr[dcur]), all(vtr[dcur + 1]), vtr[cur].begin());
    }
    reunique(vtr[cur]);
    fw[cur].init((int)vtr[cur].size() + 2);
//    DBN(l, r, vtr[cur]);
}
void addPoint(int cur, int l, int r, int x, int y) {
//    DBN("add", l, r, x, y);
    fw[cur].add(int(lower_bound(all(vtr[cur]), y) - begin(vtr[cur])));
    if (l != r) {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        if (x <= m) {
            addPoint(dcur, l, m, x, y);
        } else {
            addPoint(dcur + 1, m + 1, r, x, y);
        }
    }
}
int getSum(int cur, int l, int r, int x1, int x2, int y1, int y2) {
    if (x1 > r || x2 < l) return 0;
    if (x1 <= l && r <= x2) {
//        DBN("getSum", l, r);
        auto to = int(upper_bound(all(vtr[cur]), y2) - begin(vtr[cur])) - 1;
        auto from = int(lower_bound(all(vtr[cur]), y1)  - begin(vtr[cur]));
        return fw[cur].getSum(from, to);
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        return getSum(dcur, l, m, x1, x2, y1, y2) + getSum(dcur + 1, m + 1, r, x1, x2, y1, y2);
    }
}
int ans[101010];
int solve() {
    ...
    FI(n) {
        ...
        FJ(3) {
            cin >> a[j];
            intervals[j].first = min(intervals[j].first, a[j]);
            intervals[j].second = max(intervals[j].second, a[j]);
        }
    }
    FI(m) {
        cin >> points[i];
        v[points[i].b].push_back(points[i].c);
    }
    const int N = 100000;
    init(1, 0, N);
    ...
    FI(k + 1) {
        ...
        if (i != k) {
            FJ(3) {
                cin >> a[j];
                f[j].first = min(f[j].first, a[j]);
                f[j].second = max(f[j].second, a[j]);
            }
            if (f == intervals) {
                ans[i] = -INF;
                continue;
        ...
        queries[f[0].first - 1].push_back({f[1].first, f[1].second, f[2].first, f[2].second, i, -1});
        queries[f[0].second].push_back({f[1].first, f[1].second, f[2].first, f[2].second, i, 1});
    }
    for (int i = 0; i <= N; ++i) {
        while (curp < m && points[curp].a == i) {
            addPoint(1, 0, N, points[curp].b, points[curp].c);
            ++curp;
        }
        for (auto& q : queries[i]) {
//            cout << endl;
//            DBN(i, q.x1, q.x2, q.y1, q.y2);
            ans[q.id] += q.f * getSum(1, 0, N, q.x1, q.x2, q.y1, q.y2);
//            DBN(q.id, ans[q.id], q.x1, q.x2, q.y1, q.y2);
        }
    }
    if (ans[k] > 0) {
        cout << "INCORRECT" << endl;
