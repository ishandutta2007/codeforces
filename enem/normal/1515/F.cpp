...
bool un(ll x,ll y,ll rn[],ll sz[],ll a[], V(pll) ad[])
{
    x=fn(x,rn);
    y=fn(y,rn);
    if(x==y)
        return false;
    if(ad[x].size()<ad[y].size())
        swap(x,y);
    sz[x]+=sz[y];
    a[x]+=a[y];
    while(!ad[y].empty())
...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            pll t = (*pr.rbegin());
            if(ad[fn(t.S, rn)].empty()) break;
            pll it = ad[fn(t.S, rn)].back();
            ad[fn(t.S, rn)].DB();
            if(fn(t.S, rn) != fn(it.F, rn) && a[fn(t.S, rn)] + a[fn(it.F, rn)] >= x)
            {
                ans.PB(it.S);
                pr.erase(pr.find(MP(a[fn(it.F, rn)], fn(it.F, rn))));
                pr.erase(pr.find(MP(a[fn(t.S, rn)], fn(t.S, rn))));
                un(t.S, it.F, rn, sz, a, ad);
                a[fn(t.S, rn)]-=x;
                pr.insert(MP(a[fn(t.S, rn)], fn(t.S, rn)));
            }
        }
        if(sz[fn(0, rn)]==n)
        {
