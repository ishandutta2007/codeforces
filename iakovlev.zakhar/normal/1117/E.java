...
public class E {
	...
	public void solve() {
		...
		for (int i = 0; i < n; i++) {
			s1.append(l[i / (l.length * l.length)]);
		}
		System.out.println("? " + s1.toString());
		String a1 = in.next();
		StringBuilder s2 = new StringBuilder();
		for (int i = 0; i < n; i++) {
			s2.append(l[(i / l.length) % l.length]);
		}
		System.out.println("? " + s2.toString());
		String a2 = in.next();
		StringBuilder s3 = new StringBuilder();
		for (int i = 0; i < n; i++) {
			s3.append(l[i % l.length]);
		}
		System.out.println("? " + s3.toString());
		String a3 = in.next();
		for (int i = 0; i < ans.length; i++) {
			ans[l.length * l.length * (a1.charAt(i) - 'a') + l.length * (a2.charAt(i) - 'a') + (a3.charAt(i) - 'a')] = s.charAt(i);
		}
		out.print("! ");
		for (int i = 0; i < ans.length; i++) {
			out.print(ans[i]);
