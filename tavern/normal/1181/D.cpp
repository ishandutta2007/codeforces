/*----------------by syr----------------*/
/*

         iii                  ii
     rBQBBBBBBE       BBR iBBBBQBBL     XBBBBBBQBBBBB
   iBBQJ    7BBB      BBsSBBr   BBQ      i      cBBB
  rBBU       iBBw     BBBQi     HBBi           KBBi
  BBH         BB5    iBBB       isL          wBB5
 GBB         iBBi    6BB                   iBBB
 BBQ         BQB     BBD                  QBBi
 BBB        BQB     iQBi                1BBv
 sBBg     wBBB      QBB               iBBB
  7BBBBBBBBBi       BBR              wBBBBBBBBBBBBB
     irvi           ii               ii    i i iii
                i5U
               BBB
               BB7
              1BB
      iPBBBBBKBBR    JBR1       rQBO   BR  UBQP  iBBQi
    7BBBGs7sXBBBi     QBBr     gBBE   rBB BB2BB7HBZQBB
   QBBi      sBQ       BBB   iBBB     SQBBR  BBBB  cBQ
  gBQ        BBg        BBB KBBi      MBBH   BBB   BBs
 iBBv       iBBi         QBBBL        BBR   pBB   iBB
 pBB        BBB         iBBBB        iBB    BBL   KBB
 MBB       BBBR        BBB JBBi      DBR   iBQ    BBL
 GBB     7BBBB2      PBBH   BBBi     BQr   DBB   iBB
  BQBXwgBBP BB7    1BBB      BBQ7   1BB    BBc   BBB
   2BBBBw   BB    EBBS        QBBi  HBa   iBB    BB7
*/
#include<bits/stdc++.h>

using namespace std;

#define PH push
#define MP make_pair
#define PB push_back
#define fst first
#define snd second
#define FOR(i, x, y) for(int i = (x); i < (y); ++i)
#define REP(i, x, y) for(int i = (x); i <= (y); ++i)
#define x0 x0123456789
#define y0 y0123456789
#define x1 x1234567890
#define y1 y1234567890
#define x2 x2345678901
#define y2 y2345678901
typedef double db;
typedef long long ll;
typedef long double ldb;
typedef unsigned long long ull;
typedef pair<int, int> pii;
typedef pair<ll, ll> pll;

const int INF = 1e9 + 7;
const int maxn = 5e5 + 5;

ll n, m, q;
int a[maxn], ans[maxn], tot[maxn];
ll sum[maxn];
vector<int> vec[maxn], year;
vector<pii> num;
vector<pair<pii, int> > qry;

class FenwickTree{
private:
	int dat[maxn];
public:
	void add(int x){
		for(++x; x <= m; x += (x & (-x)))
			dat[x] ++;
		return;
	}
	int query(int x){
		int ret = 0;
		for(++x; x; x -= (x & (-x)))
			ret += dat[x];
		return ret;
	}
}fen;

int main(){
	scanf("%d%d%d", &n, &m, &q);
	FOR(i, 0, n){
		int x;
		scanf("%d", &x);
		--x;
		++a[x];
	}
	FOR(i, 0, m){
		vec[a[i]].PB(i);
	}
	int last = 0;
	REP(i, 0, n){
		if(vec[i].size() && year.size()){
			sum[i] = sum[last] + (1ll * tot[last] * (i - last));
			tot[i] = tot[last] + vec[i].size();
			year.PB(i);
			last = i;
		}
		else if(vec[i].size()){
			sum[i] = 0;
			tot[i] = vec[i].size();
			year.PB(i);
			last = i;
		}
	}
	FOR(i, 0, q){
		ll x;
		scanf("%lld", &x);
		x -= n;
		if(x > sum[year.back()]){
			x -= sum[year.back()] + 1;
			ans[i] = x % m;
			continue;
		}
		int lb = -1, rb = year.size();
		for(; lb + 1 < rb; ){
			int md = lb + rb >> 1;
			if(sum[year[md]] < x)
				lb = md;
			else
				rb = md;
		}
		x -= sum[year[lb]] + 1;
		x %= tot[year[lb]];
		//printf("%d %d %d %d\n", year[lb], sum[year[lb]], x, i);
		qry.PB(MP(MP(year[lb], x), i));
	}
	sort(qry.begin(), qry.end());
	FOR(i, 0, m) num.PB(MP(a[i], i));
	sort(num.begin(), num.end());
	int p1 = 0, p2 = 0;
	REP(i, 0, n){
		for(; p1 < m && num[p1].fst == i; ++p1){
			fen.add(num[p1].snd);
		}
		for(; p2 < qry.size() && qry[p2].fst.fst == i; ++p2){
			int lb = -1, rb = m;
			for(; lb + 1 < rb; ){
				int md = lb + rb >> 1;
				if(fen.query(md) > qry[p2].fst.snd)
					rb = md;
				else
					lb = md;
			}
			ans[qry[p2].snd] = rb;
		}
	}
	FOR(i, 0, q)
		printf("%d\n", ans[i] + 1);
	return 0;
}