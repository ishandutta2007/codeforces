...
void test_case(int test) {
    ...
    ll res = tree.ask(0, n).mn;
    auto it = where.begin();
    for (int i = 1; i < m; ++i) {
        if (b[i] != b[i - 1]) {
            if (where.count(b[i - 1]))
                for (int ind : where[b[i - 1]])
                    tree.modify(0, ind, 1);
            while (it != where.end() && it->first <= b[i - 1])
                ++it;
            while (it != where.end() && it->first < b[i]) {
                for (int ind : it->second) {
                    tree.modify(ind + 1, n, -1);
                    tree.modify(0, ind, 1);
                }
                ++it;
            }
            if (where.count(b[i]))
                for (int ind : where[b[i]])
                    tree.modify(ind + 1, n, -1);
        }
