...
void solve() {
    ...
    const int Z = *max_element(all(a));
    trav (i, a)
        i--;

    vector<vi> prefix(n + 1, vi(Z));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < Z; j++) {
    ...
    for (int l = 1; l <= n; l++) {
        ...
        auto good = [&](int r) { return prefix[n][v] - prefix[n - r][v] >= x; };
        int r = binarysearch(good, 0, n);
        if ((n - r) - l < 0)
            continue;

        int y = 0; // the count of the most frequent element in a[l, n-r)
        for (int u = 0; u < Z; u++) {
            ckmax(y, prefix[n - r][u] - prefix[l][u]);
        }

        ckmax(ans, x + y + x);
    }
    for (int u = 0; u < Z; u++) {
        ckmax(ans, prefix[n][u]);
    }
