#line 1 "codeforces-workspace/contest/1483/b/b.cpp"
...
#line 11 "codeforces-workspace/contest/1483/b/b.cpp"
...
void main() {
  ...
  while (!nx.empty()) {
    auto f = nx.lower_bound(now);
    if (f == end(nx)) f = begin(nx);
    now = f->first;
    if (f->second == now) {
    ...
    ans.emplace_back(nx[now] + 1);
    rem.erase(nx[now]);
    auto tmp = nx[now];
    nx.erase(f);
    if (nx.count(tmp)) nx.erase(tmp);

    if (auto ub = rem.upper_bound(now); ub == end(rem)) {
      ub = begin(rem);
      if (gcd(a[*ub], a[now]) == 1) {
        nx[now] = *ub;
      }
