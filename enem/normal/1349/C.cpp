...
int main()
{
    ...
    static const ll N=(1LL<<10);
    static ll n,m,q,x,y,p,a[N][N],st[N][N];
    static queue<pair<ll,pll>> pr;
    static pair<ll,pll> t;
    static char c;
    cin>>n>>m>>q;
    REP(i,0,n)
    {
    ...
    while(pr.size()>0)
    {
        t=pr.front();
        pr.pop();
        if(st[t.S.F][t.S.S]==(1LL<<60))
        {
            st[t.S.F][t.S.S]=t.F;
            if(t.S.F!=0)
            {
                pr.push(MP(t.F+1,MP(t.S.F-1,t.S.S)));
            }
            if(t.S.F!=n-1)
            {
                pr.push(MP(t.F+1,MP(t.S.F+1,t.S.S)));
            ...
            if(t.S.S!=0)
            {
                pr.push(MP(t.F+1,MP(t.S.F,t.S.S-1)));
            }
            if(t.S.S!=m-1)
            {
                pr.push(MP(t.F+1,MP(t.S.F,t.S.S+1)));
