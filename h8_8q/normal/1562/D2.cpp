...
signed main()
{
	...
	while(t--)
	{
		...
		for(int i=1;i<=n;++i)
		{
			s1.update(s1.rt[i],s1.rt[i-1],-2*n,2*n,sum1[i]+sum1[i-1],i);
			s2.update(s2.rt[i],s2.rt[i-1],-2*n,2*n,sum2[i]+sum2[i-1],i);
		}
		while(q--)
		{
			int l=read(),r=read();
			if(l%2==1)
			{
				...
				{
					int tmp=s1.query(s1.rt[r],-2*n,2*n,sum1[r]+sum1[l-1]);
					if(tmp>=l) printf("1\n%lld\n",tmp);
					else
					{
						int tmp=s1.query(s1.rt[r],-2*n,2*n,sum1[r]+sum1[l]);
						printf("2\n%lld %lld\n",l,tmp);
					}
				}
			}
			else
			{
				...
				{
					int tmp=s2.query(s2.rt[r],-2*n,2*n,sum2[r]+sum2[l-1]);
					if(tmp>=l) printf("1\n%lld\n",tmp);
					else
					{
						int tmp=s2.query(s2.rt[r],-2*n,2*n,sum2[r]+sum2[l]);
						printf("2\n%lld %lld\n",l,tmp);
					}
				}
			}
