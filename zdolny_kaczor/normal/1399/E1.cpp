...
void solve() {
	...
	for (auto [v, cou] : edges) curr += v * 1ll * cou;
	int siz = edges.size();
	REP(i, siz) gains.push({edges[i].second * 1ll * ceil_div(edges[i].first, 2), i});
	while (curr > s) {
		auto [v, i] = gains.top();
		gains.pop();
		curr -= v;
		edges[i].first /= 2;
		gains.push({edges[i].second * 1ll * ceil_div(edges[i].first, 2), i});
		ops++;
		debug(imie(edges));
	}
	printf("%d\n", ops);
