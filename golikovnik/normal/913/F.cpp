...
Binomial<mint> C(3005);

vector<mint> powers(mint base, int mx) {
...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j <= i; ++j) {
      dp[i + 1][j] += dp[i][j];
      dp[i + 1][j + 1] += dp[i][j] * pr[i - j];
    }
  }

  auto pq = powers(q, n * n + 10);
  auto getF = [&](int x, int y) {
    return dp[x][y] * pq[(x-y) * y];
  };

  vector<mint> ev(n + 1);
  vector<mint> pscc(n + 1);
  pscc[1] = 1;
  for (int i = 2; i <= n; ++i) {
    //  ev[i] = pscc[i] * (i * (i - 1) / 2 + ev[i]) + ...
    //  ev[i] = 1/(1-pscc[i]) * (pscc[i] * i * (i - 1) / 2 + ...)
    pscc[i] = 1;
    for (int k = 1; k < i; ++k) {
      pscc[i] -= getF(i, k) * pscc[k];
    }
    ev[i] = pscc[i] * (i * (i - 1) / 2);
  
    for (int k = 1; k < i; ++k) {
      ev[i] += getF(i, k) * pscc[k] * (mint(k * (k - 1) / 2) + ev[k] + k * (i - k) + ev[i - k]);
    }
