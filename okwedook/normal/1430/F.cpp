...
signed main() {
    ...
    int n, k;
    read(n, k);
    vector<ll> dp(n + 1, inf), dp2(n + 1, inf);
    vector<ll> a(n + 1), l(n + 1, -1), r(n + 1, -1);
    for (int i = 1; i <= n; ++i)
        read(l[i], r[i], a[i]);
    ll sum = 0;
    for (auto i : a) sum += i;
    ...
    for (int i = 1; i <= n; ++i) {
        ll cnt = k;
        for (int j = i; j <= n; ++j) {
            ll need = max(0ll, (a[j] - cnt + k - 1) / k);
            if (need > r[j] - l[j]) break;
            cnt = cnt + need * k - a[j];
            if (j == n) chmin(dp[j], dp[i - 1]);
            else {
                if (need < r[j] - l[j] || j < n && l[j + 1] > r[j])
                    chmin(dp[j], dp[i - 1] + cnt);
            }
        }
    }
    dbg(dp);
    println(dp.back() >= inf ? -1 : dp.back() + sum);
    #ifdef DEBUG
