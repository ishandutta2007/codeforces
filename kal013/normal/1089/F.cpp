...
bool pos(int reach,int curr,int n){
    if (reach==0) return true;
    if (curr==fact.size()) return false;
    if (nxt.find({curr,reach})!=nxt.end()){
        if (nxt[{curr,reach}]!=-1) return true;
        return false;
    }
    if (reach%fact[curr]==0){
        nxt[{curr,reach}]=curr;
        sol.pb({reach/fact[curr],n/fact[curr]});
        return true;
    }
    if (curr==fact.size()-1) return false;
    int i=0;
    while (i*fact[curr]<reach){
        if (pos(reach-i*fact[curr],curr+1,n)) {
            if (i!=0) sol.pb({i,n/fact[curr]});
            return true;}
        ++i;
    }
    nxt[{curr,reach}]=-1;
