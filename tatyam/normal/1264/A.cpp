...
void solve(){
    ...
    VEC(ll,a,n);
    vector<ll>b={0};
    rep(n-1)if(a[i]!=a[i+1])b.push_back(i+1);
    b.push_back(n);
    while(b.back()*2>n)b.pop_back();
    if(b.size()<4)return(void)out(0,0,0);
    const ll g=b[1];
    auto p=lower_bound(all(b),g*2+1);
    if(p==b.end())return(void)out(0,0,0);
    const ll s=*p-g;
    p=lower_bound(all(b),g*2+s+1);
    if(p==b.end())return(void)out(0,0,0);
    out(g,s,b.back()-g-s);
}
int main(){
