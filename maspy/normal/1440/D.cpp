...
void solve() {
  ...
  vi que;
  FOR(v, N) if (deg[v] < K) que.eb(v);
  vc<unordered_set<int>> nbd(N);
  FOR(v, N) for (auto&& e: G[v]) nbd[v].insert(e.to);

  auto check = [&](vi& V) -> bool {
    if (len(V) != K) return false;
    if (K * (K - 1) / 2 > M) return false;
    FOR(j, len(V)) FOR(i, j) {
      if (!nbd[V[i]].count(V[j])) return 0;
    }
    return 1;
  };
  ...
  while (len(que)) {
    ...
    for (auto&& e: G[v]) {
      if (deg[e.to] == K) que.eb(e.to);
      if (deg[e.to]) deg[e.to]--;
    }
    deg[v] = 0;
  }
  vi V;
  FOR(v, N) if (deg[v] >= K) V.eb(v);
  if (len(V) > 0) {
    print(1, len(V));
    for (auto&& v: V) ++v;
    print(V);
