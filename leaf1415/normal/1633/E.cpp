...
{
	vector<P> vec;
	rep(i, 1, m) vec.pb(P(abs(x-w[i]), i));
	sort(all(vec));
	
	P ret; UnionFind uf(n);
	for(auto p : vec){
		ll id = p.snd;
		if(uf.same(u[id], v[id])) continue;
		uf.unite(u[id], v[id]);
		if(w[id] >= x) ret.fst--, ret.snd += w[id];
		else ret.fst++, ret.snd -= w[id];
	}
	return ret;
}
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, m) cin >> u[i] >> v[i] >> w[i], w[i] *= 2;
	
	vector<ll> vec;
	vec.pb(0), vec.pb(200000001);
	rep(i, 1, m) rep(j, 1, m){
		if(w[i] >= w[j]) continue;
		vec.pb((w[i]+w[j])/2);
	}
	rep(i, 1, m) vec.pb(w[i]);
	uniq(vec);
	
	reps(i, vec){
		if(i == sz(vec)-1) break;
		mp[vec[i]] = calc(vec[i]);
		if(vec[i+1] != vec[i]+1) mp[vec[i]+1] = calc(vec[i]+1);
	}
	
	cin >> p >> Q >> a >> b >> c;
	rep(i, 1, p) cin >> q[i];
	rep(i, p+1, Q) q[i] = (q[i-1]*a+b)%c;
	...
	rep(i, 1, Q){
		q[i] *= 2;
		auto it = mp.upper_bound(q[i]); it--;
		P res = it->snd;
		ll tmp = res.fst*q[i] + res.snd;
		ans ^= tmp/2;
		//outl(q[i], tmp);
	}
	outl(ans);
