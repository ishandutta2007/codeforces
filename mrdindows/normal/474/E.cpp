...
int ind = 0;
int dd[100010];
void upd(int &cur, ll l, ll r, ll pos, int val){
	if (!cur) {
		cur = lastn++; 
		tr[cur] = { 0, -INF };
	}
	if (l == r){
		tr[cur] = max(tr[cur], pii( val, ind ));
	}
	else
	{
		ll m = (l + r) >> 1;
		if (pos <= m)
			upd(pl[cur], l, m, pos, val);
		else
			upd(pr[cur], m + 1, r, pos, val);
		tr[cur] = max(tr[pl[cur]], tr[pr[cur]]);
	}
}
pii get_max(int &cur, ll l, ll r, ll x, ll y){
	if (!cur) return{ 0, 0 };
	...
	{
		ll m = (l + r) >> 1;
		return max(get_max(pl[cur], l, m, x, y), get_max(pr[cur], m + 1, r, x, y));
	}
}
pii get_max(ll l, ll r){
	if (l > r) return pii( 0, 0 );
	pii res = get_max(root, 1, 1e15 + 1, l, r);
	return res;
}
int main(){
	int n;
	...
	FI(n){
		ind = i + 1;
		cin >> x;
		pii ans = max(get_max(1, x - d), get_max(x + d, 1e15+1));
		pred[ind] = ans.second;
		dd[ind] = ans.first + 1;
		upd(root, 1, 1e15 + 1, x, ans.first + 1);
	}
	int cur = max_element(dd + 1, dd + n + 1) - dd;
	vector<int> ans;
	while (cur > 0){
		ans.push_back(cur);
		cur = pred[cur];
