...
int sum(int I,int J){ ++I; ++J;
  int s=0;
	for(int i=I;i>0;i-=i&-i)
		for(int j=J;j>0;j-=j&-j) s+=bit[i][j];
  return s;
}

void add(int I,int J,ll x){ I+=2; J+=2;
	for(int i=I;i<nn;i+=i&-i)
		for(int j=J;j<nn;j+=j&-j) bit[i][j]+=x;
}
...
int main(){
	...
	cauto n=read(0);
	cauto t=read(0);
	cauto vs=readT(n,n,1);
	vector<pii> ps;
	rep(i,n)rep(j,n) ps.eb(i,j);
	sort(all(ps),[&](cauto &p, cauto &q){ return vs[p.X][p.Y]>vs[q.X][q.Y];});
	vector<string> re(n,string(n,'G'));
	int cnt=0;
	for(cauto &[r,c]:ps){
		cauto a=r+n-1-c;
		cauto b=r+c;
		cauto x=max(0,a-t);
		cauto X=min(n+n,a+t+1);
		cauto y=max(0,b-t);
		cauto Y=min(n+n,b+t+1);
		cauto s=sum(X,Y)-sum(X,y)-sum(x,Y)+sum(x,y);
		out(r,c,a,b,x,X,y,Y,cnt,s,1);
		if(s==cnt){
			add(a,b,1);
			++cnt;
			re[r][c]='M';
