...
void dij()
{
	...
	while(!pq.empty())
	{
		...
		int tpa=tp.first,tpb=tp.second;
		if(dis[tpb]!=tpa)continue;
		while(tme[tpb].count(tpa))tpa++;
		for(int i=0;i<SZ(nei[tpb]);i++)
		{
			int ddd=nei[tpb][i].first,whr=nei[tpb][i].second;
			if(tpa+ddd<dis[whr])
			{
				dis[whr]=tpa+ddd;
				pq.push(mp(dis[whr],whr));
			}
		}
	}
}
...
int main()
{
	...
	for(int i=0;i<m;i++)
	{
		int a,b,c;
		scanf("%d%d%d",&a,&b,&c);
		nei[a].push_back(mp(c,b));
		nei[b].push_back(mp(c,a));
	}
	for(int i=1;i<=n;i++)
	{
		int k;
