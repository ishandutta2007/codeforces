...
void main(int) {
  ...
  int k = (a.fold1(LIFT(max)) - n) / (n - sz(p.to<set>()));
  DUMP(k);
  if (k == 0) { return println(a); }
  vla<int> pk = power1(p, k, [n](auto&& x, auto&& y) { return vla<int>(rep(n).map(y[x[_1]])); });
  DUMP(pk);
  vla2<int> g(n, rep(n).map(LAMBDA(pair(pk[_1], _1))));
  DUMP(g);

  vla ans(n, -1);
  {
    vla<int> order = rep(n);
    order.sort(a[_1] < a[_2]);
    vla pos(n + 1, -1);
    for (int i : order.take_while(a[_1] <= n)) {
      ans[g[i][0]] = a[i];
      pos[a[i]] = g[i][0];
    }
    DUMP(ans);
    priority_queue q(_2 < _1, vector<int>());
    for (int id : rep1(n)) {
      if (pos[id] == -1) {
        while (~ans[q.top()]) { q.pop(); }
        ans[q.top()] = id;
        q.pop();
      } else {
        for (int i : g[pk[pos[id]]]) { q.push(i); }
      }
    }
  }
  println(ans);
...
TC(D) class view_base {
  template <int N>
  static inline auto get_lambda = []<class T>(T&& x) -> decltype(auto) {
...
#define WRAP(SIG, CLS, ...) \
  ...
  WRAP(template <int N> auto elements(), mapped, get_lambda<N>)
  WRAP(auto keys(), mapped, get_lambda<0>)
  WRAP(auto values(), mapped, get_lambda<1>)
...
class vla : public view_base<vla<T>> {
 ...
 public:
  ...
  vla(vla const& x) : n(x.n), o(x.o), a(new T[n]) { copy_n(x.a, n, a); }
  vla(vla& x) : vla(as_const(x)) {}
