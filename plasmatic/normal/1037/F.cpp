...
int main(){
    ...
    ll tot = 0;
    repi(0, K - 1) {
        int st = ((N - i - 1) / (K - 1) - 1) * (K - 1) + i;

        ll ctot = 0;
        map<int, int> cnts;

        for (int j = st; j >= 0; j -= K - 1) {
            int curRangeRmq = rmq.q(j + 1, j + K);

            int tadd = 1;
            while (!cnts.empty() && cnts.begin()->first < curRangeRmq) {
                auto p = cnts.begin();
                tadd += p->second;
                ctot = msub(ctot, mmul(p->second, p->first));
                cnts.erase(p);
            }
            ctot = madd(ctot, mmul(tadd, curRangeRmq));
            cnts[curRangeRmq] += tadd;

            // db(i); db(j); db(curRangeRmq); dbln;

            // values greater than curRangeRMQ
            tot = madd(tot, ctot);
        }
    }
