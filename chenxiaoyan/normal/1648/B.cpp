...
void mian() {
	n = read(), m = read();
	REP(i, 1, m) hav[i] = Sum[i] = 0;
	REP(i, 1, n) a[i] = read(), hav[a[i]] = true;
	REP(i, 1, m) Sum[i] = Sum[i - 1] + hav[i];
	REP(i, 1, m) if(!hav[i]) REP(y, 1, m / i) if(hav[y]) {
		int l = y * i, r = min(m, y * (i + 1) - 1);
		if(l > r) continue;
		if(Sum[r] - Sum[l - 1]) return puts("No"), void();
	}
	puts("Yes");
}
