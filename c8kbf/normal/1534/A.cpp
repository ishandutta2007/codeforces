...
inline void AC() {
    ...
    for(_ i = 1, p = 1; i <= n; ++i, p ^= 1) for(_ j = 1, k = p; j <= m; ++j, k ^= 1) b[i][j] = k ? 'R' : 'W';
    bool ok = true;
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) if(a[i][j] != '.' && a[i][j] != b[i][j]) ok = false;
    if(ok) {
        writeln("YES");
        for(_ i = 1; i <= n; ++i) {
            b[i][m+1] = 0;
    ...
    ok = true;
    for(_ i = 1, p = 0; i <= n; ++i, p ^= 1) for(_ j = 1, k = p; j <= m; ++j, k ^= 1) b[i][j] = k ? 'R' : 'W';
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) if(a[i][j] != '.' && a[i][j] != b[i][j]) ok = false;
    if(ok) {
        writeln("YES");
        for(_ i = 1; i <= n; ++i) {
