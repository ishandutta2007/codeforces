    ...
    Mint calc(int n, int k) {
      if (!mem[n][k]) {
        ...
        if (k == 0) {
          ...
          foreach (a; 0 .. k + 1) {
            const b = k - a;
            ret += calc(n - 1, a) * calc(n - 1, b);
          }
          // use root
          foreach (s; 0 .. 2 + 1) foreach (t; 0 .. 2 + 1) {
            if (s + t <= 2) {
              // k = a + b - (s + t) + 1
              foreach (a; 0 .. k + 2) {
                const b = k - 1 + (s + t) - a;
                if (a >= s && b >= t) {
                  Mint tmp = calc(n - 1, a) * calc(n - 1, b);
                  if (s == 1) tmp *= a;
                  if (s == 2) tmp *= a * (a - 1);
                  if (t == 1) tmp *= b;
                  if (t == 2) tmp *= b * (b - 1);
