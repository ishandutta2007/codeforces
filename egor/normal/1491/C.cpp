...
public:
    void solve() {
        ...
        SegmentTree<ll, ll> tree(n, sum, sum, [](ll a, ll b, int l, int r) -> ll {
            return a + b * (r - l);
        });
        for (int i : range(n)) {
            ll arrive = tree.query(i, i + 1);
            int leave = s[i] - 1;
            tree.update(i + 2, i + leave + 2, 1);
            if (leave > arrive) {
                answer += leave - arrive;
            } else {
                tree.update(i + 1, i + 2, arrive - leave);
            }
        }
