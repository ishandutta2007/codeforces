...
zet getsegm(int n, int t, int k) {
    if (t * k > n) return 0;
    return C(n - (k - 1) * t, t);
}

zet getgood(int n, int t, int k) {
    return getsegm(n + k - 1, t, k) * fact[t];
}
...
signed main() {
    ...
    while (tt--) {
        ...
        zet stay = 1;
        for (int t = 1; t <= n; ++t) {
            zet cntall = getgood(n, t - 1, k) * (n - (t - 1));
            zet cntgood = getgood(n, t, k);
            zet cntbad = cntall - cntgood;
            // dbg(cntall, cntgood, cntbad);
            ans += (cntbad / cntall) * stay * t;
            stay *= cntgood / cntall;
        }
        println(ans);
    }
