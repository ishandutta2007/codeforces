...
int lens,ks;
inline int bel(int x){return (x-1)/lens+1;}
inline int lowbit(int x){return x&(-x);}
inline void add(int x,int y,int *f){for(;x<=ks;x+=lowbit(x))f[x]+=y;return;}
inline int check(int x,int *f){int ans=0;for(;x;x-=lowbit(x))ans+=f[x];return ans;}
inline void change(int x,int y){for(;y<=n;y+=lowbit(y))add(x,1,f[y]);return;}
inline void insert(int now){for(auto i:pos[now])change(bel(i),v[i]);return;}
inline void Init(){lens=sqrt(n);ks=n/lens+(n%lens>0);insert(1);return;}
inline int find(int x,int y,int s)
{
    if(!x||!y)return 0;int ans=0,now=bel(x);
    for(int i=(now-1)*lens+1;i<=x;i++)ans+=(v[i]<=y&&a[i]<=s);for(;y;y-=lowbit(y))ans+=check(now-1,f[y]);
    return ans;
}
inline int work(int l,int r,int s){if(l>r)return 0;/* printf("work [%d,%d] s:%d ans:%d\n",l,r,s,find(r,l,s)-find(l-1,l,s)); */return find(r,l,s)-find(l-1,l,s)==s;}
inline bool Work(int now)
{
    int l=1;if(pos[now-1].empty())return false;if(pos[now].empty())return work(1,n,now-1);
    for(auto i:pos[now]){if(work(l,i-1,now-1))return true;l=i+1;}if(work(l,n,now-1))return true;
    return false;
}
int main()
{
    rin>>n;for(int i=1;i<=n;i++)rin>>a[i],v[i]=(pos[a[i]].empty()?1:pos[a[i]].back()+1),pos[a[i]].push_back(i),m+=(a[i]==1);
    // for(int i=1;i<=n;i++)printf("%d ",v[i]);puts("");
    if(m==n){puts("1");return 0;}Init();for(int i=2;i<=n+1;i++){if(!Work(i)){printf("%d\n",i);return 0;}insert(i);}printf("%d\n",n+2);return 0;
}