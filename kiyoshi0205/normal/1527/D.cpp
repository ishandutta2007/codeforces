...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  dfs(0);
  mat g(N);
  vec vis(N,0),cnt(N+2,0);
  cnt[0]=cnt[1]=N*(N-1)/2;
  for(auto x:tree[0])cnt[1]-=sz[x]*(sz[x]-1)/2;
  vis[0]=1;
  ll L=0,R=0;
  rep1(i,N){
    cnt[i+1]=cnt[i];
    ...
    if(pos==L){
      L=i;
      cnt[i+1]=R?sz[R]*sz[L]:(N-sz[g[1][0]])*sz[L];
    }else{
      R=i;
      cnt[i+1]=L?sz[R]*sz[L]:sz[R]*(N-sz[g[1][0]]);
    }
  }
  vec ans(N+1);
  rep(i,N+1)ans[i]=cnt[i]-cnt[i+1];
  output(ans);
}
}