...
{
	G[S].clear();
	rep(i, 1, n) G[S].pb(edge(i, dist[i]));
	dijkstra(G, S, S, dist);
}

void calc2()
{
	rep(i, 1, n) ndist[i] = dist[i];
	
	CHT cht;
	rep(i, 1, n){
		cht.add(-2*i, i*i + dist[i]);
		chmin(ndist[i], cht.query(i) + i*i);
	}
	...
	rep(i, 1, n){
		cht.add(-2*i, i*i + dist[i]);
		chmin(ndist[n+1-i], cht.query(i) + i*i);
	}
	
	rep(i, 1, n) dist[i] = ndist[i];
}
...
int main(void)
{
	...
	rep(i, 2, n) ans[i] = dist[i] = inf;
	rep(i, 0, k){
		if(i >= 1) calc2();
		calc();
		rep(j, 1, n) chmin(ans[j], dist[j]);
	}
	outa(ans, 1, n);
