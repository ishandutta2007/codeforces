...
int main(){
  ...
  ll n,k;cin>>n>>k;
  Comb comb(10000);
  auto ct=vec(2*k+1,k+2,modint(0));
  rep(i,0,2*k+1){
    modint now=1;
    ct[i][0]=1;
    rep(j,1,k+2){
  ...
  auto dp=vec(k+1,2*k+1,modint(0));//,
  dp[0][0]=1;
  rep(i,2,k+2){
    rrep(j,0,k+1){
      rep(o,0,2*k+1){
        if(dp[j][o]==0)continue;
        modint cf=1;
        ll cur=o;
        for(ll t=1;j+(i-1)*t<=k&&cur<=2*k;t++){
          cf*=ct[cur][i]*comb.inv(t)*comb.fact(i-1);
          cur+=i;
          dp[j+(i-1)*t][o+i*t]+=cf*dp[j][o];
        }
      }
    }
  }
  //debug(ct,2*k+1,k+2);
  //debug(dp,k+1,2*k+1);
  vector<modint>ret(k+1);
  rep(i,0,k+1)rep(j,0,2*k+1){
    ret[i]+=dp[i][j];
  }
  rep(i,0,k-1)ret[i+2]+=ret[i];
  rep(i,1,k+1){
    cout<<ret[i];
    if(i!=k)cout<<" ";
    else cout<<endl;
