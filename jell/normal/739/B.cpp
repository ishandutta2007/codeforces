#line 1 "codeforces-workspace\\contest\\739\\b\\b.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\739\\b\\b.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\739\\b\\b.cpp"
...
#line 36 "codeforces-workspace\\contest\\739\\b\\b.cpp"
...
void main() {
  ...
  fixed_point get_depth = [&](auto dfs, int now) -> void {
    for (auto &&[to, cost] : tree[now]) {
      depth[to] = depth[now] + cost;
      dfs(to);
    }
  };
  get_depth(0);

  compression com(begin(depth), end(depth));

  segment_tree<int> cnt(com.size());

  fixed_point dfs = [&](auto dfs, int now) -> void {
    auto here = com.upper_bound(depth[now]);
    ans[now] -= cnt.fold(0, here);
    for (auto &&[to, cost] : tree[now]) {
      dfs(to);
    }
    ans[now] += cnt.fold(0, here);
    cnt[com.lower_bound(depth[now] - r[now])]++;
  };
