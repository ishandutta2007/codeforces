...
int main(){
  ...
  ll res=0,buf=0;
  ll n,m,k;cin>>n>>m>>k;
  auto a=vec(n+1,m,0LL);
  rep(i,0,n)rep(j,0,m)cin>>a[i][j];
  vector<LazySegmentTree<ll,ll>>seg;
  rep(i,0,n)seg.emplace_back(m-k+1);
  rep(i,0,n){
    ll nowsum=0;
    rep(j,0,k)nowsum+=a[i][j]+a[i+1][j];
    if(i==0){
      rep(j,0,m-k+1){
        seg[i].set(j,nowsum);
        if(j!=m-k){
          nowsum-=a[i][j]+a[i+1][j];
          nowsum+=a[i][j+k]+a[i+1][j+k];
        }
      }
      seg[i].build();
    }
      ...
      rep(j,0,k){
        seg[i-1].update(0,min(m-k+1,j+1),-a[i][j]);
      }
      rep(j,0,m-k+1){
        seg[i].set(j,seg[i-1].query(0,m-k+1)+nowsum);
        if(j!=m-k){
          nowsum-=a[i][j]+a[i+1][j];
          nowsum+=a[i][j+k]+a[i+1][j+k];
          seg[i-1].update(max(0,j-k+1),j+1,a[i][j]);
          seg[i-1].update(j+1,min(m-k+1,j+k+1),-a[i][j+k]);
        }
      }
      seg[i].build();
    }
  ...
  cout<<seg[n-1].query(0,m-k+1)<<endl;
  return 0;
}