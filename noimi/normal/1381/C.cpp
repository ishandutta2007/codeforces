...
main() {
    ...
    while(T--) {
        ...
        rep(i, n) cnt[a[i]]++;
        int inf;
        rep(i, n + 2) if(!cnt[i]) inf = i;
        vi v(n + 2);
        pq<pii> pq;
        rep2(i, 1, n + 1) pq.emplace(cnt[i], i);
        rep(_, x) {
            auto [p, q] = pq.top();
            pq.pop();
            v[q]++;
        ...
        vi ans(n);
        rep(i, n) if(v[a[i]]) v[a[i]]--, ans[i] = a[i];
        y -= x;
        vector<vi> index(n + 2);
        rep(i, n) {
            if(!ans[i]) index[a[i]].eb(i);
        }
        priority_queue<pii> P;
        rep2(i, 1, n + 1) if(index[i].size()) P.emplace(index[i].size(), i);
        vi rem;
        while(y) {
            if(P.size() < 2) break;
            int X = P.top().se;
            ...
            if(y == 1) {
                ...
                rem.eb(index[X].back());
                y -= 2;
                index[X].pop_back(), index[Y].pop_back();
                if(index[X].size()) P.emplace(index[X].size(), X);
                if(index[Y].size()) P.emplace(index[Y].size(), Y);
            }
        }
        if(y and P.size()) {
            int k = P.top().se;
            ...
            vi s;
            rep(i, n) {
                if(a[i] != ans[i] and a[i] != k and ans[i] != k) s.eb(i);
            }
            if(s.size() >= t) {
                y = 0;
                while(index[k].size()) {
                    ans[index[k].back()] = ans[s.back()];
                    ans[s.back()] = k;
                    index[k].pop_back(), s.pop_back();
                }
            }
        }
        if(y) {
