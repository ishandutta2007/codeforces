...
int main() {
	...
	const int SIZE = 3 * n + 7;
	
	vector<Mint> fact(SIZE, 1), rev(SIZE, 1);
	for (int i = 2; i < SIZE; i++)
		fact[i] = fact[i-1] * i;
	rev[SIZE - 1] = fact[SIZE - 1].inv();
	...
	int f = n - k;
	Mint zero = fact[n] * rev[f] * rev[k];
	Mint fill = 0;
	for (int i = f; i >= 1; i--) {
		bool add = ((f - i) % 2 == 0);
		Mint base = i;
		Mint way = fact[f] * rev[i] * rev[f - i];
		Mint acc = base.pow(n) * way;
		fill += acc * (add ? 1 : -1);
	}
	W(zero * fill * 2);
