...
int main(){
	...
	number_theory nt(30000);
	int sz = (int)nt.prime.size();
	auto __solve_tc = [&](int __tc_num){
		int d;
		cin >> d;
		int res = numeric_limits<int>::max();
		for(auto i = 0, j = 0; i < sz - 1; ++ i){
			ctmax(j, i + 1);
			while(j < sz - 1 && nt.prime[j] - nt.prime[i] < d){
				++ j;
			}
			if(nt.prime[i] >= d + 1 && nt.prime[j] - nt.prime[i] >= d){
				ctmin(res, nt.prime[i] * nt.prime[j]);
			}
		}
		cout << res << "\n";
		return 0;
