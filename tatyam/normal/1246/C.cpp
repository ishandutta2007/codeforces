...
signed main(){
    ...
    vv(Modint,tate,n+1,m+1);
    vv(Modint,yoko,n+1,m+1);
    VEC(string,s,n);
    vv(ll,rock,n,m);
    rep(n)rep(j,m)rock[i][j]=s[i][j]=='R';
    PartialSum2D<ll>rocks(rock);
    tate[0][0]=1;
    tate[1][0]=-1;
    yoko[0][0]=1;
    yoko[0][1]=-1;
    rep(n)rep(j,m){
        tate[i+1][j]+=tate[i][j];
        yoko[i][j+1]+=yoko[i][j];
        ll rtate=rocks.get(i+1,n,j,j+1),ryoko=rocks.get(i,i+1,j+1,m);
        tate[i+1][j]+=yoko[i][j];
        tate[n-rtate][j]-=yoko[i][j];
        yoko[i][j+1]+=tate[i][j];
        yoko[i][m-ryoko]-=tate[i][j];
    }
    out(tate[n-1][m-1]+yoko[n-1][m-1]);
}