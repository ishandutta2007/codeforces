...
char go(int n, ll k) {
    ...
    if (k < x[0].size()) return x[0][k];
    k -= x[0].size();
    if (k < len[n - 1]) return go(n - 1, k);
    k -= len[n - 1];
    if (k < x[1].size()) return x[1][k];
    k -= x[1].size();
    if (k < len[n - 1]) return go(n - 1, k);
    k -= len[n - 1];
    return x[2][k];
...
int solve() {
    ...
    int maxN = 1;
    for (int i = 1; i <= 70; ++i) {
        len[i] = len[i - 1] * 2 + strlen("What are you doing while sending \"\"? Are you busy? Will you send \"\"?");
        if (len[i] >= 1.5e18) break;
        maxN = i;
    }
    for (int i = maxN + 1; i <= 100000; ++i) {
        len[i] = LLINF;
    }
    int q;
    cin >> q;
