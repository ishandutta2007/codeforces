...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // show(i, ms);
        // show(s, r);
        while (r < n - 1 && (*--ms.end()) <= *ms.begin() + s) {
            ms.insert(a[r + 1]);
            ++r;
        }
        // show(i, r);
        // shows;
        if (r == n - 1 && (*--ms.end()) <= *ms.begin() + s) ++r;
        tor[i] = r;
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        int mn = tree.ask(i + dist, tor[i]);
        show(i, i+dist, tor[i], mn);
        if (mn != -iinf)
            tree.set(i, mn - 1);
    }
    int ans = -tree.ask(0, 0);
    if (ans == iinf) ans = -1;
    cout << ans << endl;
