...
int main() {
    ...
    vl dp(H + 1, inf<ll>), dp2(H + 1, inf<ll>);
    dp[0] = dp2[0] = 0;

    // if(t[0] > t[1]) swap(t[0], t[1]), swap(p[0], p[1]);

    ll g = gcd(t[0], t[1]);
    ll tt = min(i128(t[0]) * t[1] / g, (i128)1e18);
    rep(2) {
        rep(i, 1, H + 1) {
            ll T = t[0] * i;
            ll k = T / t[1];
            ll d = (p[0] - s) * i + (p[1] - s) * k + (T / tt) * s;
            chmin(dp2[min(d, H)], T);
            // dump(i, T, k, d);
        }
        swap(t[0], t[1]), swap(p[0], p[1]);
    }
    ...
    rep(2) {
        rep(i, 1, H + 1) {
            ...
            if(!k) continue;
            k--;
            ll d = (p[0] - s) * (i - 1) + (p[1] - s) * k + (k * t[1] / T) * s + (p[0] + p[1] - s);
            chmin(dp[min(d, H)], T);
        }
        swap(t[0], t[1]), swap(p[0], p[1]);
    }

    per(i, H) chmin(dp[i], dp[i + 1]), chmin(dp2[i], dp2[i + 1]);
    rep(i, 1, H + 1) { rep(j, i) chmin(dp[i], dp[j] + dp[i - j]); }

    dump(dp, dp2);

    ll mi = inf<ll>;

    rep(i, H + 1) chmin(mi, dp[i] + dp2[H - i]);
