...
void solve() {
  ...
  SegTree<Monoid_Min_Idx<ll, INF>> seg(N + N);
  auto set = [&](ll i) -> void {
    seg.set(i, mp(A[i], i));
    seg.set(i + N, mp(A[i], i + N));
  };
  FOR(i, N) set(i);
  FenwickTree<Group_Add<ll>> bit(N + N);
  FOR(i, N + N) bit.add(i, 1);

  ll i = 0;
  ll ANS = 0;
  FOR_(N) {
    auto [x, j] = seg.prod(i, N + N);
    ANS += bit.sum(i, j + 1);
    j %= N;
    bit.add(j, -1), bit.add(j + N, -1);
    A[j] = INF;
    set(j);
    i = j;
  }
