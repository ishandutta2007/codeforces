...
#define logn 20
#define num 150
lol multri(int dist,int mul) {
	int lowest = (dist % mul) + 1;
	int higest = dist - (num) * mul + 1;
	if (higest < lowest) {
		return 0;
	}
	return ((lowest + higest) * (lol)(1+((higest - lowest) / mul))) >> 1;
}
bool func1(poin a, poin b) {
	return a.first < b.first;
}
bool func2(poin a, poin b) {
	return a.second < b.second;
}
int main() {
	//ifstream cin("svejksis.txt");
	//ofstream cout("svejkval.txt");
	...
	bool ans = true;
	fori(i, n) {
		into(a[i].first.first)
			into(a[i].first.second)
			into(a[i].second.first)
			into(a[i].second.second)
	}
	sort(all(a), func1);
	vector<veci> f1(logn, veci(n));
	vector<veci> f2(logn, veci(n));
	forib(i, n) {
		f1[0][i] = a[i].second.first;
		f2[0][i] = a[i].second.second;
		forn(j, logn) {
			int x = max(min(i + (1 << (j-1)),(int) n - (1 << (j - 1))),i);
			f1[j][i] = max(f1[j - 1][i], f1[j - 1][x]);
			f2[j][i] = min(f2[j - 1][i], f2[j - 1][x]);
		}
	}
	fori(i, n-1) {
		int k = logn-1;
		int pos = i + 1;
		while (k >= 0) {
			if (pos + (1 << k) > n || a[pos + (1 << k) - 1].first.first > a[i].first.second) {
				k--;
				con
			}
			if (f1[k][pos] > a[i].second.second || f2[k][pos] < a[i].second.first) {
				ans = false;
			}
			pos += 1 << k;
		}
	...
	sort(all(a), func2);
	forib(i, n) {
		f1[0][i] = a[i].first.first;
		f2[0][i] = a[i].first.second;
		forn(j, logn) {
			int x = max(min(i + (1 << (j - 1)), (int)n - (1 << (j - 1))), i);
			f1[j][i] = max(f1[j - 1][i], f1[j - 1][x]);
			f2[j][i] = min(f2[j - 1][i], f2[j - 1][x]);
	...
	fori(i, n - 1) {
		...
		int pos = i + 1;
		while (k >= 0) {
			if (pos + (1 << k) > n || a[pos + (1 << k) - 1].second.first > a[i].second.second) {
				k--;
				con
			}
			if (f1[k][pos] > a[i].first.second || f2[k][pos] < a[i].first.first) {
				ans = false;
			}
			pos += 1 << k;
		}
