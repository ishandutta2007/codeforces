...
int n,m,k[100010],l1[100010],r1[100010],l2[100010],r2[100010];
set<int> A,B;
vi ans;
pi solve(int num){
	if(num == n+1){
		if(A.size())return mp(k[num-1],*A.begin());
		if(B.size())return mp(*B.begin(),k[num-1]);
		ps("No");
		exit(0);
	}
	int lk = k[num-1],oa = A.size(),ob = B.size();
	int acnt = oa?*A.begin():-1,bcnt = ob?*B.begin():-1;
	set<int> insA,insB,remA,remB;
	if(oa && !B.count(k[num-1])){
		insB.insert(k[num-1]);
		B.insert(k[num-1]);
	}
	if(ob && !A.count(k[num-1])){
		insA.insert(k[num-1]);
		A.insert(k[num-1]);
	}
	if(k[num]>=l1[num] && k[num]<=r1[num]){
		while(A.size() && *A.rbegin()>r2[num]){
			remA.insert(*A.rbegin());
			A.erase(*A.rbegin());
		}
		while(A.size() && *A.begin()<l2[num]){
			remA.insert(*A.begin());
			A.erase(*A.begin());
		}
	}else{
	...
	if(k[num]>=l2[num] && k[num]<=r2[num]){
		while(B.size() && *B.rbegin()>r1[num]){
			remB.insert(*B.rbegin());
			B.erase(*B.rbegin());
		}
		while(B.size() && *B.begin()<l1[num]){
			remB.insert(*B.begin());
			B.erase(*B.begin());
		}
	}else{
	...
	for(auto ct:remA)A.insert(ct);
	for(auto ct:remB)B.insert(ct);
	for(auto ct:insA)A.erase(ct);
	for(auto ct:insB)B.erase(ct);
	return ret;
}
