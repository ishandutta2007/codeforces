...
signed main(){fastio
    vl A={ 1, 4, 9, 20, 41, 84, 169, 340, 681, 1364, 2729, 5460, 10921, 21844, 43689, 87380, 174761, 349524, 699049, 1398100, 2796201, 5592404};
    {
        ll n;cin>>n;
        for(auto x:A){
            if(x==n || x+1 ==n){
    ...
    rep(n,3,N-1){
        rep(t,1,n){
            ...
            auto ok=[&](ll a,ll b){
                ...
                if(p1 +1 == p2){
                    if(P(a+1)==p2)return true;
                }
                return false;
            };
            if(!ok(sz1,sz2))continue;
            ll a1=0,a2=0;
            rep(i,1,sz1){
                if(abs(t-i)%2==0)continue;
                a1 += dp[sz1][i];
            }
            rep(i,t+1,n){
                if(abs(i-t)%2==1)continue;
                a2 += dp[sz2][i-t];
            }
            dp[n][t] += a1*a2;
        }
