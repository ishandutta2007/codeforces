...
int n, a[100000], m, q[100000], b;

int index(int left, int right, int val) {
	if(right - left <= 1)
		return val <= a[left] ? left : right;
	int mid = (left + right) / 2;
	if(val < a[mid])
		return index(left, mid, val);
	else if(val > a[mid])
		return index(mid + 1, right, val);
	else
		return mid;
}
