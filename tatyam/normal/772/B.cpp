...
signed main(){
    ...
    sumy/=n;
    each(i,a){
        i={real(i)-sumx,imag(i)-sumy};
    }
    ld ans=DINF;
    a.push_back(a[0]);
    rep(n)chmin(ans,abs(a[i]-a[i+1])/2);
    a.push_back(a[1]);
    rep(n){
        ld x1=real(a[i]),x2=real(a[i+2]),y1=imag(a[i]),y2=imag(a[i+2]);
        ld line_x=(y2-y1),line_y=(x1-x2),line_c=y1*x2-x1*y2;
        ld x=real(a[i+1]),y=imag(a[i+1]);
        ld d=abs(line_x*x+line_y*y+line_c)/hypotl(line_x,line_y);
        chmin(ans,d/2);
    }
    out(ans);
}