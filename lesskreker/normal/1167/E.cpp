...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (isz(s) && *rbegin(s) > a) {
            int mn = *s.upper_bound(a);
            int mx = *rbegin(s);

            if (a != 1) {
                ss.emplace_back(1, a - 1);
            }
            if (a + 1 < mx) {
                ss.emplace_back(a + 1, mx - 1);
            }
            if (mn < x) {
                ss.emplace_back(mn + 1, x);
            }
        }
    ...
    for (int i = 0; i < isz(ss); i++) {
        // cout << "ss: " << ss[i].first << ' ' << ss[i].second << '\n';
        if (i + 1 == isz(ss) || ss[i + 1].first != ss[i].first) {
            if (ss[i].second > l) {
                if (ss[i].first > l) {
                    int len = ss[i].second - ss[i].first + 1;
                    ans -= 1LL * len * (len + 1) / 2;
                } else {
                    int p = max(0, l - ss[i].first + 1);
                    int q = ss[i].second - l;

                    ans -= 1LL * p * q + 1LL * q * (q + 1) / 2;
                }
                l = ss[i].second;
            }
        }
