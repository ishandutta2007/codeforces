...
int main() {
    ...
    const int B = accumulate(all(a), 0);
    ...
    FOR (b, 1, B) {
        // f(b) = b/B * (1 + f(b - 1) + f(b)) + (1 - b/B) * ((n-2)/(n-1) * (1 + f(b)) + 1/(n-1))
        // (1 - b/B - (1 - b/B)*(n-2)/(n-1)) f(b) = b/B * (1 + f(b - 1)) + (1 - b/B) * 1
        mn bB = mn(b) * mn::small_inv(B);

        f[b] = (bB * (1 + f[b-1]) + (1 - bB)) / (1 - bB - (1 - bB) * (N - 2) * mn::small_inv(N - 1));
    }
