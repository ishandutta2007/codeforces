...
int main() {
    ...
    std::sort(v, v + n);
    for (int i = 0; i < n; ++i)
        t[i] = std::lower_bound(v, v + n, t[i]) - v;
    for (int i = 0; i < n; ++i)
        p[a[i] | b[i] << 1].push_back(i);
    for (int i = 0; i < 4; ++i)
        std::sort(p[i].begin(), p[i].end(), [&](int i, int j) {return t[i] < t[j];});
    long long ans = -1;
    for (auto i : p[0])
        add(i, 1);
    for (auto i : p[3])
        add(i, 1);
    for (int i = k; i < int(p[1].size()); ++i)
        add(p[1][i], 1);
    for (int i = k; i < int(p[2].size()); ++i)
        add(p[2][i], 1);
    long long sum = 0, c = -1;
    ...
    for (int i = 0; i <= k && i <= int(p[3].size()); ++i) {
        ...
        if (k - i <= int(std::min(p[1].size(), p[2].size())) && 2 * k - i <= m) {
