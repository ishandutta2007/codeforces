...
int main(){
	...
	fORx(i,0,m){
		f[i][m-i] = g[i][m-i] = 1;
		fORx(j,0,min(m-i-1,sum[0])){
			f[i][j] = ( (sum[1]+i+1)*f[i+1][j] + (sum[0]-j)*f[i][j+1] )%MOD * Inv(sum[2]+i-j) % MOD;
			g[i][j] = ( (sum[1]+i)*g[i+1][j] + (sum[0]-j-1)*g[i][j+1] )%MOD * Inv(sum[2]+i-j) % MOD;
		}
	}
	For(i,n)
		printf("%d\n",int( w[i]*(addonce[i]?f[0][0]:g[0][0])%MOD ));
