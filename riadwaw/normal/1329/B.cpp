...
class BDreamoonLyubitPosledovatelnosti {
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		vector<Z> cnt;
		for (int i = 1; i <= 30; ++i) {
			cnt.push_back(Z::valueOf(min((1<<i) - 1, d) - min((1 << (i - 1)) - 1, d)));
		}

		vector<Z> dp(cnt.size() + 1);
		dp[0] = 1;

		for (auto c: cnt) {
			for (int x: downrange(cnt.size())) {
				dp[x + 1] += dp[x] * c;
			}
		}

		out << accumulate(dp.begin(), dp.end(), Z()) - 1 << "\n";
	}
};
...
int main() {
	...
	BDreamoonLyubitPosledovatelnosti solver;
