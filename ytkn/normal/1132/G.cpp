...
int main(){
    ...
    auto make_ord = [&](){
        auto g = make_graph();
        auto et = EulerTour(g, n);
        return make_tuple(et.ord, et.in, et.out);
    };
    auto [ord, in, out] = make_ord();
    Seg seg(vector<int>(ord.size()));
    for(int i = 0; i < k; i++){
        seg.apply(in[i], out[i]+1, 1);
    }
    for(int i = 0; i+k <= n; i++){
        cout << seg.all_prod() << ' ';
        if(i+k < n){
            seg.apply(in[i+k], out[i+k]+1, 1);
            seg.set(in[i], -inf);
            seg.set(out[i], -inf);
        }
    }
