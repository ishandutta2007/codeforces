...
struct node{
	vec2 p;
	double a;
	node(vec2 p=vec2(0, 0), double a=0):p(p), a(a){}
	bool operator<(const node &B) const{
		return p < B.p || (p == B.p && dblcmp(a, B.a) < 0);
	}
};
const int N = 1100;
...
int main(){
	...
	for (int i=0; i<n; ++i){
		double a, b, c, d;
		cin >> a >> b >> c >> d;
		p[i] = vec2(a/b, c/d).inverse();
	}
	sort(p, p+n);
	for (int i=0; i<n; ++i){
		for (int j=i+1; j<n; ++j){
			mp[node((p[i]+p[j])/2, (p[j]-p[i]).angle())] ++;
		}
	}
	lld ans = 0;

	for (map<node, int>::iterator it=mp.begin(); it != mp.end();){
		lld mul = 1;
		lld sum = 1;
		node t = it->first;
		for (; it != mp.end(); ++it){
			if (!(t.p == it->first.p)) break;
			//printf("%d ", it->second);
			mul = mul * (it->second+1) % P;
			sum += it->second;
		}
		//puts("");
		ans += mul - sum;
	}
	cout << checkmod(ans, P) << endl;
	return 0;
}