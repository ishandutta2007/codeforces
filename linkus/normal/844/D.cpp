...
int main(){
    ...
    int t = min(n-1,999);
    int i;
    //I don't care it's the same code
    while(t--){
        while(i = RAND(n)+1,~table[i].e1);
        /* if(~table[i].e1){ */
        /*     ++t; */
        /*     continue; */
        /* } */
        /* while(i = distribution(generator), ~table[i].e1); */
        cout << "? " << i << endl;
        cin >> cur >> tmp;
        table[i] = {cur,tmp};
        if(cur <= x && cur >= best.e1)
            best = {cur,tmp};
    }
    curid = best.e2;
    /* int lastn = curn; */
    ...
    while(curn < x){
        ...
        if(table[curid].e1 == -1){
            ...
            curn = table[curid].e1, curid = table[curid].e2;
        }
        /* cout << ++curid; */
        /* cin >> curn; */
    }
