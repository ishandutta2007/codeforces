...
// freopen("output.txt","w",stdout);
	...
	rep(i,1,n)if(x[i]!=MOD)y.pb({x[i],i});
	sort(all(y));
	...
	while(!pq.empty()){
		...
		if(vis[nw]<c)continue;
		st = 0;
		while(id!=y.size() && y[id].fi<c){
			if(vis[y[id].se]<=y[id].fi)++ans;
			else {
				vis[y[id].se] = y[id].fi;
				pq.push(y[id]);
				st = 1;
