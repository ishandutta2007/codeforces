...
public class F {
	...
	void go()
	{
		...
		SegmentTreeRMQ stmax = new SegmentTreeRMQ(ra);
		int max = 0;
		for(int i = 1;i <= n-2;i++){
			max = Math.max(max, a[i-1]);
			int ind = stmin.firstle(i, max-1);
			if(ind == -1){
				ind = n;
			}
			if(i < ind && ind < n && stmin.min(i, ind) == max && stmax.min(ind, n) == -max){
				out.println("YES");
				out.println(i + " " + (ind-i) + " " + (n-ind));
				return;
			}
			if(i < ind-1 && stmin.min(i, ind-1) == max && stmax.min(ind-1, n) == -max){
				out.println("YES");
				out.println(i + " " + (ind-1-i) + " " + (n-(ind-1)));
				return;
			}
		}
		out.println("NO");
