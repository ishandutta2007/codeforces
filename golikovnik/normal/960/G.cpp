...
int main() {
  ...
  auto solve = [&](auto&& self, int l, int r) -> vector<mint> {
    if (l + 1 == r) {
      return vector<mint>{1, l};
    }
    int m = (l + r) / 2;
    return multiply(self(self, l, m), self(self, m, r));
  };
  auto prod = n == 1 ? vector<mint>{1} : solve(solve, 0, n - 1);
  cout << prod[n - 1 - a - b] * C(a + b, a) << '\n';
