...
int main() {
    ...
    rep(2) {
        ...
        RMT<int> seg(a);
        RmT<int> seg2(a);
        vi r(n); // r[i] = min(x | a[x] > a[i])
        rep(i, n) r[i] = seg.max_right(i, [&](int x) { return x <= a[i]; });
        vi l(n);
        rep(i, n) l[i] = seg2.min_left(i, [&](int x) { return x >= a[i]; }) - 1;

        RMT<int> seg3(n);
        auto id = iota(a);
        fore(i, id) {
            if(r[i] < n) {
                int k = seg3.max_right(r[i] + 1, [&](int x) { return x <= r[i]; });
                chmin(maxr[i], k);
            }
            seg3.set(i, l[i]);
        }
    ...
    auto swag = get_SWAG<int>([](int x, int y) { return min(x, y); });

    ll ans = 0;
    int j = 0;
    rep(i, n) {
        while(j < (swag.empty() ? inf<int> : swag.fold_all())) swag.push(maxr[j]), j++;
        ans += j - i;
        swag.pop();
    }
    OUT(ans);
