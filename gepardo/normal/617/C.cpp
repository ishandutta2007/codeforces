...
main()
{
    int n = in, x1 = in, y1 = in, x2 = in, y2 = in;
    vector < pair<int,int> > v, q(n + 1);
    in.getv(v, n);
    for (int i = 0; i < n; i++)
        q[i].ff = (x1 - v[i].ff) * (x1 - v[i].ff) + (y1 - v[i].ss) * (y1 - v[i].ss),
        q[i].ss = (x2 - v[i].ff) * (x2 - v[i].ff) + (y2 - v[i].ss) * (y2 - v[i].ss);
    int minres = LLINF;
    q[n] = {0, 0};
    for (int i = 0; i <= n; i++)
    {
