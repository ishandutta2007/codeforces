...
template<typename T> inline ostream& operator<<(ostream& os, const vector<T>& v) { for (int i = 0; i < sz(v); i++) { if (i) { os << "\n"; } os << v[i]; } return os; }
...
vector<string> a = getVector<string>(n), b = getVector<string>(n);
vpii frontOp, backOp;

signed main() {
	auto apply1 = [&](vector<string>& a, vpii& ops) -> bool {
		bool good = false;
		for (;;) {
			bool ok = false;
			for (int i = 1; i < n; i++) {
				for (int j = 1; j < m; j++) {
					if (a[i-1][j-1] == 'L' && a[i-1][j] == 'R' &&
					    a[i][j-1] == 'L' && a[i][j] == 'R') {
						ok = true;
						a[i-1][j-1] = a[i-1][j] = 'U';
						a[i][j-1] = a[i][j] = 'D';
	...
	auto apply2 = [&](vector<string>& a, vpii& ops) -> bool {
		bool good = false;
		for (;;) {
			bool ok = false;
			for (int i = 1; i < n; i++) {
				for (int j = 1; j < m; j++) {
					if (a[i-1][j-1] == 'U' && a[i-1][j] == 'U' &&
					    a[i][j-1] == 'D' && a[i][j] == 'D') {
						ok = true;
						a[i-1][j-1] = a[i][j-1] = 'L';
						a[i-1][j] = a[i][j] = 'R';
	...
	frontOp.insert(frontOp.end(), backOp.rbegin(), backOp.rend());
	putLn(frontOp.size());
	putLn(frontOp);
	return 0;
}