...
bool check(vi cur) {
    F0R(a, n) {
        int cnt = 0;
        F0R(b, m) if (cur[b] != s[a][b]) cnt++;
        if (cnt>2) return false;
    }
    return true;
}
int check2(vi cur, int k) {
    F0R(a, n) {
        int cnt = 0;
        F0R(b, m) if (cur[b] != s[a][b]) cnt++;
        if (cnt>2 && cur[k]==-1) {
...
void solve() {
    ...
    FOR(i,1,n) {
        ...
        if (cnt>2) {
            vi v;
            F0R(j, m) if (s[0][j] != s[i][j]) v.pb(j);
            int k = v.size();
            F0R(sub, 1<<k) {
                vi cur = s[0];
                F0R(bit, k) if (sub & 1<<bit) cur[v[bit]] = s[i][v[bit]];
                if (check(cur)) {
                    cout << "Yes\n";
                    print(cur, m);
                    return;
            ...
            if (k==3) {
                F0R(a, 3) F0R(b, 3) if (a!=b) {
                    vi cur = s[0];
                    cur[v[a]] = s[i][v[a]];
                    cur[v[b]] = -1;
                    int res = check2(cur, v[b]);
                    if (res != -1) {
                        cur[v[b]] = res;
                        cout << "Yes\n";
                        print(cur, m);
