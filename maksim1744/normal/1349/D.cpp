...
int main() {
    ...
    /*
        sum f(ai) -- potential, with every move E(sum(f(ai))) should decrease by 1

        sum f(ai) = 1 + sum_{i=1}^n (ai/s * f(ai - 1) + (s-ai)/s*1/(n-1) * f(ai + 1) + (s-ai)/s * (n-2)/(n-1) * f(ai))

        f(ai) = 1/n + ai/s * f(ai - 1) + (s-ai)/s*1/(n-1) * f(ai + 1) + (s-ai)/s * (n-2)/(n-1) * f(ai)
    */
    ...
    vector<map<int, Mint>> mt(s + 1);
    for (int i = 0; i <= s; ++i) {
        mt[i][i] += 1;
        mt[i][i] -= Mint(s - i) / s * (n - 2) / (n - 1);
        if (i > 0) {
            mt[i][i - 1] -= Mint(i) / s;
        }
        if (i + 1 <= s) {
            mt[i][i + 1] -= Mint(s - i) / s * 1 / (n - 1);
        }
        mt[i][s + 1] = Mint(1) / n;
    }
    mt[s].clear();
    ...
    for (int i = 0; i < mt.size(); ++i) {
        ...
        int j = i + 1;
        if (j < mt.size()) {
            if (mt[j].count(i) && mt[j][i] != 0) {
                Mint k = mt[j][i] / mt[i][i];
                for (auto [u, mi] : mt[i])
                    mt[j][u] -= k * mi;
                mt[j].erase(i);
            }
    ...
    vector<Mint> f;
    for (int i = 0; i < mt.size(); ++i) {
        f.pb(mt[i][s + 1] / mt[i][i]);
    }
    show(f);

    Mint need = f[0] * (n - 1) + f[s];
    Mint cur = 0;
    for (int k : a)
        cur += f[k];
