...
int main()
{
    ...
    {
        int x = c[i].X, y = c[i].Y;
        if (num.count({x,y-1})!=0)
        {
            g[i].push_back(num[{x,y-1}]);
            dw[i]++;
            g1[num[{x,y-1}]].push_back(i);
        }
        if (num.count({x+1,y-1})!=0)
        {
            g[i].push_back(num[{x+1,y-1}]);
            dw[i]++;
            g1[num[{x+1,y-1}]].push_back(i);
        }
        if (num.count({x-1,y-1})!=0)
        {
            g[i].push_back(num[{x-1,y-1}]);
