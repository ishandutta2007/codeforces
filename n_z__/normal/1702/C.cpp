...
//when using um,us,gp or cc,add a hash function named custom_hash,use it like unordered_map<int,int,custom_hash>map;
int n,k;
cin>>n>>k;
vector<int>u(n+1);
for(int x=1;x<=n;x++)
...
for(int x=1;x<=k;x++)
cin>>a[x]>>b[x];
gp_hash_table<int,int,custom_hash>q,h;
for(int x=1;x<=n;x++)
{
    if(q[u[x]]==0)q[u[x]]=x;
    h[u[x]]=x;
...
for(int x=1;x<=k;x++)
{
    if(q[a[x]]==0||q[b[x]]==0||q[a[x]]>h[b[x]])cout<<"NO"<<endl;
    else cout<<"YES"<<endl;
}
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
