...
string s,t;
ll ans;
vec dlis[400001],dlis2[400001];
int main(){
  startupcpp();
  for(ll i=1;i<=400000;++i){
    for(ll j=i;j<=400000;j+=i){
      dlis[j].emplace_back(i);
    }
    for(ll j=i;j<=400000;j+=i*2){
      dlis2[j].emplace_back(i);
    }
  }
// int codeforces;cin>>codeforces;while(codeforces--){
  ...
  for(ll i=0;i<Q;++i){
    ...
    ans[i]=(R-L+1)*(R-L)/2*(R-L-1)/3;
  }
  vrsort(query);
  BinaryIndexedTree bit(400001);
  size_t qj=0;
  for(ll i=200000-2;i>=1;--i){
    for(ll k=i*2;k<=200000;k+=i){
      bit.act(k,prev(dlis[k].end())-upper_bound(all(dlis[k]),i));
    }
    if(~i&1){
      for(ll k2=i*3;k2<=400000;k2+=i*2){
        const ll k=k2/2;
        bit.act(k,lower_bound(all(dlis[k2]),k)-upper_bound(all(dlis[k2]),max(i,k-i)));
      }
    }
    for(ll k2=i*4;k2<=400000;k2+=i*2){
      const ll k=k2/2; 
      bit.act(k,lower_bound(all(dlis2[k2]),k)-upper_bound(all(dlis2[k2]),max(i,k-i)));
    }
    while(qj<query.size()&&get<0>(query[qj])==i){
      ans[get<2>(query[qj])]-=bit.query(get<1>(query[qj]));
      ++qj;
    }
  }
