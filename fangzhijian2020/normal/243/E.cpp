...
void solve(int x,int lim){
	...
	if(col[x]==3){
		...
		if(t[3].size()+(bool)lim>2){puts("NO");exit(0);}
...
int main(){
	n=tot=read();root=++tot;opt[tot]=1;
	for(int i=1;i<=n;++i)scanf("%s",c[i]+1);
	for(int i=1;i<=n;++i)v[root]+=i;
	fac[0]=1;for(int i=1;i<=n;++i)fac[i]=1ll*fac[i-1]*i%mod;
	for(int i=1;i<=n;++i){
		for(int j=1;j<=n;++j)col[j]=1+(c[i][j]=='0');
		dfs(root);solve(root,0);
	}
	DFS(root);puts("YES");
	for(int i=1;i<=n;++i){
		for(int j=1;j<=n;++j)putchar(c[i][t[j]]);
		puts("");
	}
	return 0;
}