...
int solve(int n,int a0,int Q,function<pii(int)> ask){
  vector<pii> A;
  A.pb(ask(a0));
  vector<int> I(n);
  iota(all(I),1);
  ...
  for(int a:I)
    A.pb(ask(a));
  sort(all(A));
  int i=lower_bound(all(A),mp(Q,-10))-A.begin();
  if(i==0)
    return A[0].X;
  --i;
  int a=A[i].Y;
    ...
    for(int lim=n;lim<=100;++lim){
      forn(qq,1000){
        ...
        forn(i,n)
          iI[I[i]]=i;
        int Q=rand()%(lim+2);
        int r=solve(n,I[0]+1,Q,[&](int a){
            int i=iI[a-1];
            if(i==n-1)
              return mp(A[i],-1);
            return mp(A[i],I[i+1]+1);
          });
...
void test2(){
  const int n=50000;
  int r=solve(n,25201,1000000000,[&](int a){
      static int c=0;
      ++c;
      assert(c<2000);
      assert(a>=1 && a<=n);
      return mp(a+1000,a==n?-1:a+1);
    });
  assert(r==-1);
}
