...
int main(){
	...
	vector<vector<vi>> dp(m + 1, vector<vi>(3, vi(3, -1)));
	dp[0][0][0] = 0;
	int res = 0;
	for(int i = 0; i < m; ++ i){
		for(int j = 0; j < 3; ++ j){
			for(int k = 0; k < 3; ++ k){
				if(dp[i][j][k] != -1){
					for(int l = 0; l < 3 && j + k + l <= count[i] && k + l <= count[i + 1] && l <= count[i + 2]; ++ l){
						ctmax(dp[i + 1][k][l], dp[i][j][k] + l + (count[i] - j - k - l) / 3);
						ctmax(res, dp[i + 1][k][l]);
					}
				}
			}
		}
