...
int main(void)
{
	...
	rep(i, 1, n){
		...
		lvec.push_back(l[i]), rvec.push_back(r[i]);
	}
	...
	rep(i, 1, sz(comp)-1){
		ll x = n;
		x -= lvec.end() - lower_bound(all(lvec), i);
		x -= upper_bound(all(rvec), i-1) - rvec.begin();
		ans += mint(comp[i]-comp[i-1]) * comb(x, k);
	}
	outl(ans);
