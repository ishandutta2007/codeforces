...
int main() {
    ...
    vpii range(N, {INT_MAX, INT_MIN});
    F0R (i, N) {
        ckmin(range[uf.rep(i)].f, i);
        ckmax(range[uf.rep(i)].s, i);
    }

    sort(all(range));
    while (range.back().f == INT_MAX) range.pop_back();

    int ans = 0;
    for (int i = 0, j = 0; i < sz(range); i = j) {
        int end = range[i].s;
        while (j < sz(range) && range[j].f <= end) {
            ckmax(end, range[j++].s);
        }
        ans += j - i - 1;
    }
