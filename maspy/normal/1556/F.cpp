...
void solve() {
  ...
  FOR(i, N) FOR(j, N) {
    if (i == j) continue;
    P[i][j] = mint(A[i]) / mint(A[i] + A[j]);
  }

  vv(mint, ALL_lose, 1 << N, N);
  FOR(s, 1 << N) FOR(v, N) {
    if (s & 1 << v) continue;
    mint lose = 1;
    FOR(i, N) {
  ...
  FOR(s, 1 << N) {
    ...
    while (1) {
      ...
      mint p = 1;
      ll u = s - t;
      FOR(v, N) if (u & 1 << v) p *= ALL_lose[t][v];
      MEMO[s].eb(mp(t, p));
      if (t == 0) break;
      t = (t - 1) & s;
    }
