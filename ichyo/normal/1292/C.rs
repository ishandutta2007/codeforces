...
fn dfs(
    ...
    count_m: &mut [i64],
    dfs_m: &mut Vec<Vec<i64>>,
) -> i64 {
    if dfs_m[ei0][ei1] != i64::max_value() {
        return dfs_m[ei0][ei1];
    }
    let base = count(ei0, edges, g, count_m) * count(ei1, edges, g, count_m);
    let u0 = edges[ei0].1;
    let u1 = edges[ei1].1;
    let mut add_max = 0;
    for &(_, next_ei0) in &g[u0] {
        if next_ei0 / 2 == ei0 / 2 {
            continue;
        }
        add_max.set_max(dfs(next_ei0, ei1, edges, g, count_m, dfs_m));
    }
    for &(_, next_ei1) in &g[u1] {
        if next_ei1 / 2 == ei1 / 2 {
            continue;
        }
        add_max.set_max(dfs(ei0, next_ei1, edges, g, count_m, dfs_m));
    }
    dfs_m[ei0][ei1] = base + add_max;
    dfs_m[ei0][ei1]
}
...
fn count(
    ...
    if count_m[ei] != i64::max_value() {
        return count_m[ei];
    }
    let u = edges[ei].1;
    let mut res = 1;
    for &(_, next_ni) in &g[u] {
        if next_ni / 2 == ei / 2 {
            continue;
        }
        res += count(next_ni, edges, g, count_m);
    }
    count_m[ei] = res;
    res
}

fn solve() -> Result<(), Box<Error>> {
    // WARNING: usize is 32bit!
    let stdin = std::io::stdin();
    let mut stdin = stdin.lock();
    let n: usize = fscan(&mut stdin)?;
    let mut g = vec![Vec::new(); n];
    let mut edges = Vec::new();
    for i in 0..n - 1 {
        let (u, v): (usize, usize) = fscan(&mut stdin)?;
        g[u - 1].push((v - 1, 2 * i));
        g[v - 1].push((u - 1, 2 * i + 1));
        edges.push((u - 1, v - 1));
        edges.push((v - 1, u - 1));
    }
    let mut count_m = vec![i64::max_value(); edges.len() + 10];
    let mut dfs_m = vec![vec![i64::max_value(); edges.len() + 10]; edges.len() + 10];
    let mut ans = 0;
    for i in 0..n - 1 {
        ans.set_max(dfs(2 * i, 2 * i + 1, &edges, &g, &mut count_m, &mut dfs_m));
    }
    println!("{}", ans);
    Ok(())
}
