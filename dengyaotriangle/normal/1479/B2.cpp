...
int qry(node* rt,int cl,int cr,int l,int r){
    ...
    else return min(qry(rt->c[0],cl,cm,l,cm),qry(rt->c[1],cm+1,cr,cm+1,r));
}
int main(){
    int n;
    cin>>n;
    ...
    chg(rt,0,n,0,1);
    for(int i=2;i<=n;i++){
        int w=min(qry(rt,0,n,0,a[i]-1),(a[i]==n?int(1e9):qry(rt,0,n,a[i]+1,n)))+1;
        w=min(w,qry(rt,0,n,a[i],a[i]));
