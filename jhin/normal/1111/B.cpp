...
map<string,int> mp1;
set<int>myset;
string s[10000],s1;int tmp;bool yes,lft,rt;
multiset<string>lfts[100001],rts[100001];
bool isv(char c)
{
    if(c=='i'||c=='e'||c=='o'||c=='a'||c=='u')return true;
    return false;
}

int t,sum2,y3,z3,mxx,mxy;bool used;//))(())))
//char ch;
int main()
{
...
cin>>n>>m>>k;
for(int i=0;i<n;i++){cin>>x;v.push_back(x);mx=max(mx,x);}
sort(v.begin(),v.end());
...
while(k>0&&v.size()>1)
{long long news=v.size();
long long canadd=news*m;
long long mxadd=k;
canadd=min(canadd,mxadd);
long long canadd2=(news-1)*m;
long long mxadd2=k-1;
canadd2=min(canadd2,mxadd2);
    if(double(sum-v[0]+canadd2)/(news-1)>double(sum+canadd)/news)
    {sum-=v[z];
    v.erase(v.begin());
    k--;}
    else break;
...
canadd=min(canadd,mxadd);
int si=v.size();
cout<<setprecision(10)<<double(sum+canadd)/si;
return 0;
}