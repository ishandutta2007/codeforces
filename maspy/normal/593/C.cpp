...
void solve() {
  ...
  auto op = [&](string x, string a, string b) -> string { return "(" + a + x + b + ")"; };
  auto abs = [&](string x) -> string { return "abs(" + x + ")"; };
  auto ch = [&](ll a, ll b) -> string {
    string x = op("-", "t", to_string(a));
    string absx = abs(x);
    string y = op("-", absx, "1");
    string z = op("-", abs(y), y);
    return op("*", to_string(b / 2), z);
  };
  string f, g;
  FOR(i, N) {
    string a = ch(i, X[i]);
