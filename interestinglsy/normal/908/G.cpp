...
ll pow10Sum[MAXN];
void Init(){
	pow10[0] = pow10Sum[0] = 1;
	For(i,MAXN-1) pow10[i] = pow10[i-1]*10 % MOD;
	For(i,MAXN-1) pow10Sum[i] = Mymo( pow10Sum[i-1] + pow10[i] );
}
...
int mem[MAXN][MAXN][2];
#define NOW mem[pos][geqCnt][eqR]
ll Dfs( int pos , bool eqR , int geqCnt , int target ){
	if( pos == n ){
		if(!geqCnt) return 0;
		return pow10Sum[geqCnt-1] % MOD;
	}
	if(~NOW) return NOW;
	const int lim = s[pos] - '0';
	ll ans = 0;
	Forx(x,0,9){
		if( eqR and x > lim ) continue;
		ll tans = Dfs(pos+1,eqR&(x==lim),geqCnt+(x>=target),target);
		Addi(ans,tans);
	}
	return NOW = ans;
}
