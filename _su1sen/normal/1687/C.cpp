...
long long mapping(long long f, long long g) {
    return f == numeric_limits<long long>::max() ? g : f;
}
...
int main() {
    ...
    loop(t) {
        ...
        rep(i, n + 1) c[i] = b[i] - a[i];

        set<int> nonzero;
        rep(i, n + 1) {
            if (c[i]) nonzero.insert(i);
        }
        vector<pair<int, int>> available;

        vector<vector<int>> rs(n + 1), ls(n + 1);
        rep(i, m) {
            input(int, l, r);
            --l;
            if (nonzero.count(l) or nonzero.count(r)) {
                rs[l].push_back(r);
                ls[r].push_back(l);
            } else {
                available.emplace_back(l, r);
            }
        }
        ...
        while (available.size()) {
            auto [l, r] = available.back();
            available.pop_back();
            for (auto it = nonzero.lower_bound(l); it != nonzero.end();) {
                int x = *it;
                if (x > r) break;
                for (int y : rs[x]) {
                    if (nonzero.count(y) == 0) {
                        available.emplace_back(x, y);
                    }
                }
                for (int y : ls[x]) {
                    if (nonzero.count(y) == 0) {
                        available.emplace_back(y, x);
                    }
                }
                it = nonzero.erase(it);
            }
