...
// Tested https://codeforces.com/contest/762/problem/E (implicit = false)
// https://www.spoj.com/problems/ADAAPHID/ (implicit = false)
...
struct TreapNode{
	...
	template<typename Q>
	static pair<TreapNode<T, implicit>*, TreapNode<T, implicit>*> erase(TreapNode<T, implicit>* root, Q key){
		if (!root)
			return {root, nullptr};
		root -> propagate();
		...
		TreapNode<T, implicit>* node;
		if ((implicit && own_value == key + 1) || (!implicit && root -> value == key)){
			node = merge(root -> children[0], root -> children[1]);
			root -> children = {nullptr, nullptr};
			root -> pull();
			return {node, root}; 
		}
		else if (go_right){
			tie(root -> children[1], node) = erase(root -> children[1], (implicit ? key - own_value: key));
		}
		else{
			tie(root -> children[0], node) = erase(root -> children[0], key);
		}
		root -> pull();
		return {root, node};
	}
	...
	template<typename Q>
	static pair<TreapNode<T, false>*, TreapNode<T, false>*> split(TreapNode<T, false>* x, Q y){
		if (!x) return {nullptr, nullptr};
		x -> propagate();
		TreapNode<T, false> *l, *r;
		if (x -> value < y){
			l = x;
			tie(l -> children[1], r) = split(x -> children[1], y);
		}
		else{
			r = x;
	...
	template<typename Q>
	static pair<TreapNode<T, true>*, TreapNode<T, true>*> split(TreapNode<T, true>* x, Q y){
		if (!x) return {nullptr, nullptr};
		x -> propagate();
		TreapNode<T, true> *l, *r;
		int own_value = sub_size(x -> children[0]) + 1;
		if (own_value <= y){
			l = x;
	...
	static TreapNode<T, implicit>* find_by_order(TreapNode<T, implicit>* root, int k){
		if (!root)
			return nullptr;
		root -> propagate();
	...
	static TreapNode<T, true>* reverse(TreapNode<T, true>* root, int l, int r){
		if (l > r)
			return root;
...
struct item{
	...
	inline void pull(const item* l, const item* r){
		
	}
	bool operator < (long long z) const {
		return x < z;
	}
	...
	friend bool operator <= (long long z, const item& x) {
		return z <= x.x;
	}
...
void solve(){
	...
	TreapNode<item, true>* root = nullptr;
	for(auto &e: a){
		TreapNode<item, true>* curr = new TreapNode<item, true>();
		curr -> value.x = e;
		root = TreapNode<item, true>::merge(root, curr);
	}
	...
	for(int z = 0; z < q; ++z){
		...
		--l; --r;
		if (t == 1){
			TreapNode<item, true> *ret;
			tie(root, ret) = TreapNode<item, true>::erase(root, r);

			root = TreapNode<item, true>::insert(root, ret, l);
		}
		else{
			root = TreapNode<item, true>::reverse(root, l, r);
		}
	}
	for(int i = 0; i < m; ++i){
		int x;
		// cin >> x;
		read_int(x);
		cout << TreapNode<item, true>::find_by_order(root, x - 1) -> value.x << " ";
	}
