...
int main()
{
    ...
    P p[300005];
    for(int i = 0; i < n; i++) p[i] = P(w[i], i);
    sort(p, p + n);
    auto F = [](int a, int b){return min(a, b);};
    auto G = [](int f, int a){return a + f;};
    auto H = [](int f, int g){return f + g;};
    auto seg = get_lazy_segment_tree(m - 1, F, G, H, INF, 0);
    seg.apply(0, m - 1, -INF);
    int j = 0;
    int v = p[j].second;
    int ans = INF;
