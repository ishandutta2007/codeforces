...
void solve() {
  const ll N = 30, M = 30, K = 50;
  const ll INF = 1LL << 60;
  FOR(n, N + 1) FOR(m, M + 1) FOR(k, K + 1) DP[n][m][k] = INF;

  FOR3(n, 1, N + 1) {
    FOR3(m, 1, M + 1) {
      chmin(DP[n][m][0], 0);
      if (n * m <= K) chmin(DP[n][m][n * m], 0);
      ...
      FOR3(n1, 1, n) {
        ...
        FOR(k1, K + 1) {
          FOR(k2, K + 1) {
            if (k1 + k2 > K) break;
            chmin(DP[n][m][k1 + k2], DP[n1][m][k1] + DP[n2][m][k2] + m * m);
          }
        }
      }
      FOR3(m1, 1, m) {
        ...
        FOR(k1, K + 1) {
          FOR(k2, K + 1) {
            if (k1 + k2 > K) break;
            chmin(DP[n][m][k1 + k2], DP[n][m1][k1] + DP[n][m2][k2] + n * n);
          }
        }
      }
    }
