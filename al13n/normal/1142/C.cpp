...
int main(int argc, char **argv){
  ...
  sort(all(A));
  vector<pll> R;
  auto ok=[](pll a,pll b,pll c){
            if(c.X==b.X) return false;
            if(a.X==b.X) return true;
            ld bb=(a.Y-b.Y)*1./(a.X-b.X)-(a.X+b.X);
            ld cc=a.Y-a.X*a.X-bb*a.X;
            return c.Y<-EPS+c.X*c.X+c.X*bb+cc;
          };
  for(pll p:A){
    while(sz(R)>1 && !ok(R[sz(R)-2],R.back(),p)) R.pop_back();
    R.pb(p);
  }
  dbg(R);
  if(sz(R)>1 && R[0].X==R[1].X){
    assert(sz(R)==2||R[2].X>R[1].X);
    R.erase(R.begin());
  }
  cout<<sz(R)-1;
