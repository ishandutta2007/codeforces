...
mint solve(int n1, int n2) {
    if (n1 == 1) return mint(n1).fac() * mint(n2).fac() * (n2 - 1);
    else return 0;
}
...
int main() {
    ...
    mint ret = solve(ns, ng) + solve(ng, ns) + mint(ns).fac() * mint(ng).fac() * 2;
    ret *= mint(N).nCr(n0) * mint(n0).fac();
    dbg(ret);
    dbg(cnt);
    mint den = 1;
    for (auto [a, s] : cnt) if (s > 1) den *= mint(s).fac();
    cout << ret / den << '\n';
}