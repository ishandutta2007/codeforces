...
signed main(){fastio
    ...
    rep(zzz,0,M){
        if(zzz>=1){
            ...
            p--;
            ll y;
            rep(j,0,2)if(mpmp[j]==x)y=j;
            rep(i,0,2)a[i].erase(p);
            a[y].insert(p);
            PP b=pp[y];
            seg.set(p,b);
        }
        auto cal=[&](){
            {
                ll emp=0;
                rep(i,0,2)if((int)a[i].size()==0)emp++;
                if(emp==1){
                    rep(i,0,2){
                        if((int)a[i].size()==0){
                            if(i==0)return (ll)a[2].size();
                            else return (ll)a[i-1].size();
                        }
                    }
                    assert(0);
                    return (ll)0;
            ...
            auto win=[&](ll l,ll r){
                PP b=seg.prod(l,r+1);
                {
                    ...
                    assert(emp>=1);
                    if(emp==2){
                        return max(b.fi,max(b.se.fi,b.se.se));
                    }
                }
                ll x,y=-1;
                if(b.fi==0){
                ...
                if(l==0){
                    ll valb;
                    if(y==0)valb=b.fi;
                    else if(y==1)valb=b.se.fi;
                    else valb=b.se.se;
                    return valb+*a[y].begin();
                }else{
                    ll valb;
                    if(y==0)valb=b.fi;
                    else if(y==1)valb=b.se.fi;
                    else valb=b.se.se;
                    return valb+(n-1-(*--a[y].end()));
                }
            };
            ll x,y,z=0;
            rep(i,1,2){
                if(*a[z].begin() < *a[i].begin())z=i;
            }
            ...
            if(FR){
                ...
                assert(L<=mn);
                ll ans=win(0,L-1)+win(mn+1,n-1)+(mn-L+1);
                return ans;
            }else{
                ll ans=win(L,n-1);
                ll POS=*--a[x].end();
