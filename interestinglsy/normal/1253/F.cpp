...
void Kruskal(){
	int dsu[MAXN];
	For(i,n) dsu[i] = i;
	function<int(int)> Findfa = [&]( int pos ){
		return dsu[pos] == pos ? pos : dsu[pos] = Findfa(dsu[pos]);
	};
	function<bool(int,int)> Unite = [&]( int a , int b ){
	...
	for( auto &e : edges ) e.c = d[e.f] + d[e.t] + e.c;
	sort(All(edges),[]( E a , E b ){ return a.c < b.c; });
...
ll upmx[MAXN][LOG];
int dep[MAXN];
void PreDfs( int pos , int fa , ll faw ){
	dep[pos] = dep[fa] + 1;
	up[pos][0] = fa;
	upmx[pos][0] = faw;
	For(i,LOG-1){
		up[pos][i] = up[up[pos][i-1]][i-1];
		upmx[pos][i] = max( upmx[pos][i-1] , upmx[up[pos][i-1]][i-1] );
	}
	// O(pos);
	// Print(upmx[pos],0,3);
