...
int main() {
    ...
    const long long p = cld(d + 1, a);

    long long ans = 1;
    while (i < m or j <= n) {
        while (i < m and t[i] <= last_open + d) ++i;
        j = std::min(n + 1, cld(last_open + d + 1, a));
        if (i == m and j > n) break;

        long long nt = i == m ? numeric_limits<long long>::max() : t[i];
        long long na = j > n ? numeric_limits<long long>::max() : j * a;
        ...
        long long x = min(cld(n + 1 - j, p), cld(nt - na, p * a));
        assert(x > 0);
        ans += x;
        j += x * p;
        last_open = (j - p) * a;
