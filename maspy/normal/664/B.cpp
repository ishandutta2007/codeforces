...
void solve() {
  string S;
  getline(cin, S);
  auto no = [&]() -> void { print("Impossible"); };
  ll N = count(all(S), '?');
  ll p = S.find('=');
  ll lim = stoi(S.substr(p + 2));
  vi sgn(N, 1);
  FOR3(i, 1, N) { sgn[i] = (S[4 * i - 2] == '+' ? 1 : -1); }
  vi A(N);
  FOR(i, N) A[i] = (sgn[i] == 1 ? 1 : lim);
  ll val = 0;
  FOR(i, N) val += A[i] * sgn[i];
  if (val > lim) return no();

  FOR(i, N) {
    ll add = min(lim - 1, lim - val);
    A[i] += (sgn[i] == 1 ? add : -add);
    val += add;
  }
  if (val != lim) return no();
