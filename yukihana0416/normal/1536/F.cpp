...
mint C(int n, int m) {return po[n] * inv[m] * inv[n - m];}
...
void work()
{
    ...
    mint ans = 0;
    for(int i = max(2, n / 4 * 2); i <= n; i += 2)
    {
        if(i >= n - i) ans = ans + C(i, n - i) * po[i - 1] * 2;
    }
    cout <<ans * n <<endl;
}
...
//freopen("out.txt", "w", stdout);
    ...
    po[0] = 1;
    for(int i = 1; i < maxn; i++) po[i] = po[i - 1] * i;
    inv[maxn - 1] = po[maxn - 1].inv();
