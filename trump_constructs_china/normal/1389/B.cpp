...
int sum(int n) {
    int res=0; rep (i,n) res+=a[i]; return res; }

int mx(int n, int l) {
    int ans=0; rep (i,n-1) checkMax(ans, a[i]+a[i+1]); return ans*l; }
...
void solve() { 
    ...
    rep (back,z+1) {
        int len = k+1 - back*2;
        if (back>0 && len>0) { 
            ses(sum(len+1)); ses(mx(len+1,back-1));
            checkMax(ans, sum(len+1)+mx(len+1,back-1)+a[len-1]); }
        checkMax(ans, sum(len) + mx(len,back)); }
    printf(IN,ans);
}