...
int main(){
  ...
  ll sz=52;
  auto d=vec(n,sz,INF);
  using T=tuple<ll,ll,ll>;//dist,prew,pos
  priority_queue<T,vector<T>,greater<T>>que;
  que.emplace(0,0,0);
  d[0][0]=0;
  while(!que.empty()){
    auto [dist,pre,pos]=que.top();
    //cout<<pos spa pre spa dist<<endl;
    que.pop();
    if(dist>d[pos][pre])continue;
    for(auto e:g[pos]){
      ll nd=dist;
      if(pre!=0)nd=dist+(pre+e.cost)*(pre+e.cost);
      ll np=(pre==0?e.cost:0);
      if(d[e.to][np]>nd){
        d[e.to][np]=nd;
        que.emplace(nd,np,e.to);
      }
    }
  }
  vector<ll>ret(n);
  rep(i,0,n)ret[i]=(d[i][0]==INF?-1:d[i][0]);
  debug(ret,n);
  return 0;
}