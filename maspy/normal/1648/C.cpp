...
void solve() {
  ...
  VEC(ll, A, N);
  VEC(ll, B, M);
  ll LIM = max(MAX(A), MAX(B));

  auto CNT = bincount(A, LIM + 1);
  SegTree<Mono> seg(len(CNT));
  auto set = [&](ll i) -> void { seg.set(i, mint(CNT[i])); };
  FOR(i, LIM + 1) set(i);

  ll n = SUM(CNT);
  mint den = 1;
  for (auto&& c: CNT) den *= fact_inv<mint>(c);
  mint ANS = 0;
  FOR(i, len(B)) {
    if (n == 0) {
      ANS += mint(1);
    ...
    // i  A 
    ll b = B[i];
    mint x = fact<mint>(n - 1) * den * seg.prod(0, b);
    // print("den", den);
    ANS += x;
    --n;
    if (CNT[b] == 0) break;
