...
void solve() {
    int m, n;
    re(m, n); // n friends, m shops

    vector<vector<int>> p(m, vi(n));
    re(p);
    ...
    auto impossible = [&](int threshold) -> bool {
        for (auto &shop : p) {
            int count = 0;
            for (auto x : shop)
                count += x >= threshold;
    ...
    auto ans = -1 + binarysearch(impossible, 0, int(2e9));
    ckmin(ans, *min_element(all(best)));
