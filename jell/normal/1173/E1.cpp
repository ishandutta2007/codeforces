...
modint solve(int t) {
    ...
    for(int i=1; i<=m; ++i) {
        ...
        init(dp[nxt],0);
        for(int x=0; x<i; ++x) {
            for(int y=0; x+y<i && tdis>=y; ++y) {
                if(!dp[now][x][y]) continue;
                int d=i-1-x-y;
                if(!a[t]) d=-d;
                if(d+w[t]<0) break;
                dp[nxt][x+1][y]+=dp[now][x][y]*(x+tliks)*inv[al+x-y+d];
                dp[nxt][x][y]+=dp[now][x][y]*(d+w[t])*inv[al+x-y+d];
                dp[nxt][x][y+1]+=dp[now][x][y]*(tdis-y)*inv[al+x-y+d];
            }
        }
    }
...
signed main() {
    for(int i=1; i<3010; ++i) inv[i]=modint::inverse(i);
    cin>>n>>m;
    rep(i,n) cin>>a[i];
    rep(i,n) cin>>w[i];
    rep(i,n) {
