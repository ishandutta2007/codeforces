...
void solve() {
    cin >> n >> m;
    // ans = number of way to choose a[i] such that there exists i and j that gcd(a[i], j) = 1 for i >= j > 1
    ...
    int64_t mmod = m % base;
    int64_t ans = 0, good = mmod;
    int64_t multiplier = 1;
    int64_t sum = 0;
    for (int i = 2; i <= n; i++) {
        if (i == p[i]) {
            multiplier *= i;
            if (multiplier > m) multiplier = m + 1;
        }

        int64_t count = (m / multiplier) % base;  // number of things that will never be bad

        (ans = (ans * mmod + good * (mmod - count))) %= base;
        (good *= count) %= base;
        (sum += ans) %= base;
    }
    if (sum < 0) (sum += base) %= base;
    cout << sum << '\n';
}
