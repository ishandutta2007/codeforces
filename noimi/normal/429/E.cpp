...
template <typename T> vector<edge<T>> eulerian_path(Edges<T> es, int s, bool directed, vector<vector<pair<edge<T>, int>>> &g) {
#define src from
    for(auto &e : es) {
...
int main() {
    ...
    vll xs;
    rep(i, n) xs.eb(l[i]), xs.eb(r[i]);
    UNIQUE(xs);
    rep(i, n) l[i] = lb(xs, l[i]), r[i] = lb(xs, r[i]);
    int m = si(xs);
    vi cnt(m), ans(n, -1);
    UnionFind uf(m);
    rep(i, n) cnt[l[i]]++, cnt[r[i]]++;
    rep(i, n) uf.unite(l[i], r[i]);

    vpi v;
    rep(i, n) v.eb(l[i], r[i]);
    rep(i, m - 1) {
        if(cnt[i] & 1) v.eb(i, i + 1), cnt[i + 1] ^= 1, uf.unite(i, i + 1);
    }

    vector<Edges<int>> es(m);
    vector<vector<pair<edge<int>, int>>> g(m);
    rep(i, si(v)) es[uf[v[i].fi]].eb(v[i].fi, v[i].se, i);
    rep(i, m) {
        if(uf[i] == i) {
            auto E = eulerian_path(es[i], i, false, g);
            for(auto &e : E) {
                if(e.cost < n and ans[e.cost] == -1) ans[e.cost] = e.from < e.to;
            }
        }
    }
