...
lint ar[MAXN];
vector<int> by_bit[60];

lint ans[MAXN];
lint ansl;

lint doit(int bit, lint cur)
{
	if (bit != 0)
		cur = doit(bit - 1, cur);
	while (!by_bit[bit].empty() && (cur ^ ar[by_bit[bit].back()]) > cur)
	{
		cur ^= ar[by_bit[bit].back()];
		ans[ansl++] = by_bit[bit].back();
		by_bit[bit].pop_back();
		if (bit != 0)
			cur = doit(bit - 1, cur);
	}
...
void solve()
{
	...
	For(i, n)
		For(j, 60)
			if (ar[i] >= (1ll << j) && ar[i] < (1ll << (j + 1)))
			{
				by_bit[j].push_back(i);
				highest_bit = max(highest_bit, j);
				break;
			}
