...
int main(){
  vector<strat*> strats;
  FOR(a,3) strats.eb(new strat_period(vi{a}));
  FOR(a,3) FOR(b,3) if(a!=b) strats.eb(new strat_period(vi{a,b}));
  FOR(a,3) FOR(b,3) FOR(c,3) if(a!=b||b!=c||a!=c)strats.eb(new strat_period(vi{a,b,c}));
  FOR(a,3) FOR(b,3) strats.eb(new strat_last(a,b));
  FOR(i,10) {
    cout << RPS[init[i]] << endl << flush;
    vector<strat*> nstrats;
    bool ok; string t; cin>>t;
    ok = t == "player";
    for(auto s : strats) {
      bool sok = s->update(init[i]);
      if(sok == ok) nstrats.eb(s);
    }
    strats = nstrats;
  }
  // cerr << strats.size() << endl;
