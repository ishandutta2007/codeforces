...
{
    for (int i=1;i<=n;i++)
        if (i!=start) q.insert(mp(maxll,i)), dist[i]=maxll;
    q.insert(mp(0,start));
    dist[start]=0;

    while (!q.empty()&&q.begin()->fir!=maxll)
    {
        pair<ll,ll> now=*q.begin();
        q.erase(q.begin());
        for (int i=0;i<reb[now.sec].size();i++)
            if (dist[reb[now.sec][i].fir]>now.fir+reb[now.sec][i].sec)
            {
                q.erase(mp(dist[reb[now.sec][i].fir],reb[now.sec][i].fir));
                dist[reb[now.sec][i].fir]=now.fir+reb[now.sec][i].sec;
                q.insert(mp(dist[reb[now.sec][i].fir],reb[now.sec][i].fir));
                from[reb[now.sec][i].fir]=now.sec;
            }
    }
}
...
int main()
{
    ...
    while (m--)
    {
        int zn1,zn2,zn3;
        read(zn1,zn2,zn3);
        reb[zn1].pb(mp(zn2,zn3));
        reb[zn2].pb(mp(zn1,zn3));
    }
    dijkstra(1);
    if (dist[n]==maxll) return writeln(-1), 0;
