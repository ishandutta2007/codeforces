...
int q; cin >> q; while (q--){
    ...
    bool ans = 1;
    FordE(i, m, 1){
        int sum = it.get(1, 1, n, query[i].fi, query[i].se);
        if (sum * 2 == query[i].se - query[i].fi + 1){
            ans = 0;
            break;
        }
        if (sum * 2 < query[i].se - query[i].fi + 1){
            it.update(1, 1, n, query[i].fi, query[i].se, 0);
        }
        else{
            it.update(1, 1, n, query[i].fi, query[i].se, 1);
        }
    }
    if (!ans){
        cout << "NO" << endl;
