...
public:
    int N, M;
    vector<vector<pii>> E;
    vector<int> A, AA, U, F, B;

    inline int fastmod(ll m) {
        while (m >= 2*M) m -= 2*M;
        while (m >= M) m -= M;
        return int(m);
    }

    void run(int u, int p, int w, int curForw, int curBack, int depth) {
        F.push_back((ll(M-curForw) * AA[depth]) % M);
        B.push_back(curBack);

        for (pii v:E[u]) {
            if (v.x != p && U[v.x] >= w) {
                run(v.x, u, w, fastmod(curForw * 10ll + v.y), fastmod(curBack + v.y * ll(A[depth])), depth+1);
            }
        }
    }
    ...
    ll cnt(int f, int b) {
        ...
        while (f<F.size()) { C[F[f]]++; f++; }
        ll ans = 0;
        while (b<B.size()) {
            auto it = C.find(B[b]);
            if (it != C.end()) ans += it->y;
            ++b;
        }
        return ans;
    }
    ...
    ll perf(int ctr) {
        F.clear(); B.clear();
        F.push_back(0); B.push_back(0);

        ll ans = 0;
        for (pii v:E[ctr]) {
            if (U[v.x] >= U[ctr]) {
                int f = F.size(), b = B.size();
                run(v.x, ctr, U[ctr], v.y%M, v.y%M, 1);
                ans -= cnt(f, b);
            }
        }
        return ans + cnt(0, 0) - 1;
    }
    ...
    void solve(istream& cin, ostream& cout) {
        ...
        U = CentroidDecomposition<pii>{E}.findCentroids();
