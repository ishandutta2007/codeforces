...
int main() {
    int N;
    cin >> N;
    vector strs(N, vector<int>(26));
    REP(i, N) {
        string s;
        cin >> s;
        for (auto c : s) strs[i][c - 'a']++;
    }
    vector<mint> dp(1 << N, 1);
    dp[0] = 0;
    REP(d, 26) {
        vector<int> hi(dp.size(), 1 << 30);
        REP(i, N) hi[1 << i] = strs[i][d];
        FOR(S, 1, dp.size()) {
    ...
    REP(S, dp.size()) {
        lint isum = 0;
        REP(i, N) if ((S >> i) & 1) isum += i + 1;
        lint k = __builtin_popcount(S);
        ret ^= (dp[S] * (k & 1 ? 1 : -1)).val * isum * k;
    }
    cout << ret << '\n';
}