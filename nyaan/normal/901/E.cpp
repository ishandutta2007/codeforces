...
int find_mod(int n) {
  // p = nk + 1p
  int st = (TEN(9) + 8) / n * n + 1;
  do {
    if (is_prime(st)) return st;
  } while (st += n);
  assert(false);
}
...
void Nyaan::solve() {
  ...
  in(_b, _c);
  auto verify = [&](vl& a) {
    each(x, a) if (abs(x) > TEN(4)) return false;
    rep(_, 50) {
      int i = randint(0, n);
      ll ci = 0;
      rep(k, n) {
        ll buf = _b[k - i + (k - i < 0) * n] - a[k];
        ci += buf * buf;
        if (ci > _c[i]) break;
      }
      if (ci != _c[i]) return false;
    }
    return true;
  };

  vm b(n), c(n);
  rep(i, n) b[i] = _b[i], c[i] = _c[i];

  int p = find_mod(n);
  mint::set_mod(p);
  mint pr = mint(PrimitiveRoot(p)).pow((p - 1) / n);
  mint ipr = pr.inverse(), inv2 = mint(2).inverse();
  auto ntt = [&](const vm& f) { return ChirpZ(f, pr); };
  auto intt = [&](const vm& f) {
    static mint in = mint(n).inverse();
    vm F = ChirpZ(f, ipr);
    each(x, F) x *= in;
    return F;
  };
  ...
  // F
  // 2A = B' ((s_b + s)I - C)
  // 2A = s B'^(-1)I + B'^(-1)(s_b I - C)
  // B'^(-1)IB
  // B'^(-1)(s_b 1 - C)D
  auto B = ntt(b);
  each(x, B) x = x.inverse();
  auto C = ntt(c);
  auto I = ntt(vm(n, 1));
  vm D(n);
  rep(i, n) D[i] = B[i] * (sb * I[i] - C[i]);
  rep(i, n) B[i] = B[i] * I[i];
  // 2A = sB + D
  // s
  trc(B[0] * 3, B[1]);
  ...
  //
  s[1] -= 4;
  vm ss = QuadraticEquation(s[2], s[1], s[0]);
  trc(ss);

  vvl anss;
  each(S, ss) {
    vm A(n);
    rep(i, n) A[i] = (S * B[i] + D[i]) * inv2;
    vl a(n);
    rep(i, n) a[i] = A[i].get();
    each(x, a) if (abs(x) > abs(x - p)) x -= p;
    trc(a);
    if (verify(a)) anss.push_back(a);
  }
  sort(all(anss));
  out(anss.size());
  each(ans, anss) out(ans);
}