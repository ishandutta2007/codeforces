...
void chemthan() {
  ...
  while (test--) {
    ...
    auto calc = [&] (int mi) {
      ...
      FOR(i, 0, n) FOR(j, 0, m) if (a[i][j] == 'B') {
        chkmax(minsum, i + j - mi);
        chkmin(maxsum, mi + i + j);
        chkmax(minsub, i - j - mi);
        chkmin(maxsub, mi + i - j);
      }
      FOR(sum, minsum, maxsum + 1) {
        FOR(sub, minsub, maxsub + 1) {
          if ((sum + sub) % 2 == 0) {
            int a = (sum + sub) / 2;
            int b = (sum - sub) / 2;
            if (0 <= a && a < n && 0 <= b && b < m) {
