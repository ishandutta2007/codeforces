...
void _solve() {
    ...
    fill((int*)dp, (int*)dp + N * N, INT_MAX);
    dp[0][0] = 0;

    int ans = INT_MAX;
    for(int cur = 0; cur < n; ++cur) {
        for(int skip = 0; skip <= k; ++skip) {
            if(dp[cur][skip] == INT_MAX) {
                continue;
            }

            int pls = n - cur - 1;
            if(skip + pls <= k) {
                ans = min(ans, dp[cur][skip] + a[cur] * (l - d[cur]));
            }

            for(int nxt = cur + 1; nxt < n && skip + nxt - cur - 1 <= k; ++nxt) {
                dp[nxt][skip + nxt - cur - 1] = min(dp[nxt][skip + nxt - cur - 1],
                        dp[cur][skip] + a[cur] * (d[nxt] - d[cur]));
            }
        }
    }
