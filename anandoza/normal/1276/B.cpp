...
ll count(union_find& X, int a, int b) {
    int n = X.uf.size();
    ll r = 0;
    F0R(i,n) {
        if (i != X.rep(i)) continue;
        ll s = X[i].size;
        if (X.rep(a) == X.rep(i)) s--;
        if (X.rep(b) == X.rep(i)) s--;
        r += s * (s-1) / 2;
    }
    return r;
}
...
void solve() {
    ...
    F0R(i,m) {
        ...
        re(u, v);
        u--; v--;
        if (u != a && v != a) A.unio(u, v);
        if (u != b && v != b) B.unio(u, v);
    }
    union_find C(n);
    map<pii,int> c;
    F0R(i,n) {
        pii p = {A.rep(i), B.rep(i)};
        if (c.count(p)) {
            C.unio(c[p], i);
        }
        c[p] = i;
