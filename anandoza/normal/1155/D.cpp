...
int main() {
    ...
    vll forward(n + 1);
    for (int i = 0; i < n; i++)
        ckmax(forward[i + 1], forward[i] + a[i]);

    vll diff_forward(n + 1);
    for (int i = 0; i < n; i++)
        ckmax(diff_forward[i + 1], max(forward[i], diff_forward[i]) + x * a[i]);

    vll backward(n + 1);
    for (int i = 0; i < n; i++)
        ckmax(backward[i + 1], backward[i] + a[n - 1 - i]);

    ll ans = 0;
    for (int l = 0; l <= n; l++)
        ckmax(ans, diff_forward[l] + backward[n - l]);
