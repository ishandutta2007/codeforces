...
int main() {
  ...
  int B = __builtin_sqrt(n) + 1;
  vector<vector<array<int, 3>>> mo(B);
  for (int i = 0; i < q; ++i) {
    int l = $ - 1, r = $ - 1;
    int b = l / B;
    mo[b].push_back({l, r, i});
  }
  bool inc = 1;
  for (auto& b: mo) if (b.size()) {
    inc ^= 1;
    if (inc) {
      sort(b.begin(), b.end(), Ordering<3, 2, 1>{});
    } else {
      sort(b.begin(), b.end(), Ordering<3, -2, -1>{});
    }
    for (auto [l, r, i]: b) {
      state.movto(l, r);
      ans[i] = max(state.b[0], state.b[1]) + 1;
    }
  }
  print(range{ans, q});
}