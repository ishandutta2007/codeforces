...
int main() {
    ...
    TEST {
        ...
        REC([&](auto &&f, int x, int p) -> void {
            fore(e, g[x]) {
                if(e == p) continue;
                fore(a, b, d[e.cost.se]) { mp[a] -= b; }
                fore(a, b, d[e.cost.fi]) {
                    mp[a] += b;
                    chmax(ma[a], mp[a]);
                }
                f(e, x);
                fore(a, b, d[e.cost.se]) mp[a] += b;
                fore(a, b, d[e.cost.fi]) mp[a] -= b;
            }
        })
        (0, -1);
        mint ans = 1;
        rep(i, 1, n + 1) ans *= mint(i).pow(ma[i]);
        mint s;
        REC([&](auto &&f, int x, int p, mint t) -> void {
            s += t;
            fore(e, g[x]) {
                if(e == p) continue;
                f(e, x, t * inv(e.cost.fi) * e.cost.se);
            }
        })
        (0, -1, ans);
        OUT(s);
