...
char s[MAX];
int n,range[MAX],sum[MAX];
vector<int> haveSum[2*MAX];
SuffixArray sa;
void init(void) {
    scanf("%d",&n);
    scanf("%s",s);
    REP(i,n) sum[i]=(i==0?0:sum[i-1])+(s[i]=='('?1:-1);
    REP(i,n) haveSum[sum[i]+MAX].push_back(i);
    vector<pair<int,int> > v1,v2;
    REP(i,n) {
        v1.push_back(make_pair(i==0?0:sum[i-1],i));
        v2.push_back(make_pair(sum[i],i));
    }
    sort(ALL(v1));
    sort(ALL(v2));
    set<int> pos;
    int j=0;
    REP(i,n) {
        while (j<v2.size() && v2[j].fi<v1[i].fi) {
            pos.insert(v2[j].se);
            j++;
        }
        __typeof(pos.begin()) it=pos.lower_bound(v1[i].se);
        range[v1[i].se]=it==pos.end()?n-1:*it-1;
    }
    sa=SuffixArray(string(s));
}
void process(void) {
    long long res=0;
    FOR(i,1,n) {
        int cur=sa.sa[i];
        int maxRange=range[cur];
        int minRange=i==1?cur:cur+sa.lcp[i-1];
        if (maxRange<minRange) continue;
        int tmp=cur==0?0:sum[cur-1];
        int L=lower_bound(ALL(haveSum[tmp+MAX]),minRange)-haveSum[tmp+MAX].begin();
        int R=upper_bound(ALL(haveSum[tmp+MAX]),maxRange)-haveSum[tmp+MAX].begin();
        if (R>=L) res+=R-L;
    }
    cout<<res<<endl;
}
