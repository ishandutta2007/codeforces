...
int main() {
	...
	while (l + 1 != r) {
		...
		fill(range(id), -1);
		rep(i, 0, n) {
			rep(j, 0, m) mask[i] ^= (a[i][j] >= x) << j;
			id[mask[i]] = i;
		}
		rep(i, 0, m) rep(s, 1, 1 << m) if (s >> i & 1) maxx(id[s ^ 1 << i], id[s]);
		bool ok = false;
		rep(i, 0, n) if (~id[(1 << m) - 1 ^ mask[i]]) ok = true;
		(ok ? l : r) = x;
	}
	fill(range(mask), 0);
	fill(range(id), -1);
	rep(i, 0, n) {
		rep(j, 0, m) mask[i] ^= (a[i][j] >= l) << j;
		id[mask[i]] = i;
	}
	rep(i, 0, m) rep(s, 1, 1 << m) if (s >> i & 1) maxx(id[s ^ 1 << i], id[s]);
	rep(i, 0, n) if (~id[(1 << m) - 1 ^ mask[i]]) {
		cout << i + 1 << " " << id[(1 << m) - 1 ^ mask[i]] + 1 << endl;
		break;
	}
}