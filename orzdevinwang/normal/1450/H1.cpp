...
struct WORKER {
	...
	void fix() {
		ans = 0;
		L(i, 0, t) (ans += C(cnt, i)) %= mod;
	}
	void cntup() { ans = (ans << 1) % mod, (ans += mod - C(cnt, t)) %= mod, ++cnt; }
	void cntdown() { --cnt, (ans += C(cnt, t)) %= mod, ans = 1ll * ans * inv2 % mod; }
...
void cntdown() { c.tdown(), d.tdown(), a.cntdown(), b.cntdown(), c.cntdown(), d.cntdown(), cnt--; }
int main() {
	scanf("%d%d", &n, &m);
	scanf("%s", s + 1);
	jc[0] = njc[0] = 1;
