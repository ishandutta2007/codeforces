#line 1 "codeforces-workspace/contest/1452/e/e.cpp"
/*
 * @file template.cpp
 * @brief Template
 */
...
#line 12 "codeforces-workspace/contest/1452/e/e.cpp"
...
#line 22 "codeforces-workspace/contest/1452/e/e.cpp"
...
void main() {
  ...
  sort(begin(ps), end(ps), [](auto p1, auto p2) {
    return p1.first + p1.second < p2.first + p2.second;
  });
  auto com = [&](auto p1, auto p2) -> int {
    return max(0, min(p1.second, p2.second) - max(p1.first, p2.first));
  };

  vector<i64> lo{0}, ro{0};
  vector<int> s(n);
  for (auto i = 0; i < m; ++i) {
    for (auto j = 0; j < n; ++j) {
      s[j] += com(ps[i], pair(j, j + k));
    }
    lo.emplace_back(*max_element(begin(s), end(s)));
  }
  s.assign(n, 0);
  for (auto i = m; i-- != 0;) {
    for (auto j = 0; j < n; ++j) {
    ...
    ro.emplace_back(*max_element(begin(s), end(s)));
  }
  reverse(begin(ro), end(ro));
