...
template <typename G> struct CentroidDecomposition {
    ...
    inline int build(Graph &t, int idx) {
...
int main() {
    TEST {
        ...
        VV(char, s, n, m);
        int sx, sy;
        rep(i, n) rep(j, m) if(s[i][j] == 'L') sx = i, sy = j;
        queue<pii> q{{{sx, sy}}};
        while(!empty(q)) {
            auto [x, y] = q.front();
            q.pop();
            ...
            fore(dx, dy, dx4) {
                int nx = x + dx, ny = y + dy;
                if(inc(nx, 0, n) and inc(ny, 0, m) and s[nx][ny] == '.') v.eb(nx, ny);
            }
            if(s[x][y] == 'L') {
                fore(e, v) q.emplace(e);
                continue;
            }
            if(si(v) > 1) continue;
            s[x][y] = '+';
