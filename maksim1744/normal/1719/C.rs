...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let mut pref = vec![Vec::new(); n];
    let mut cur = a[0];
    for (i, x) in a.iter().enumerate().skip(1) {
        let mut x = *x;
        if x > cur {
            std::mem::swap(&mut x, &mut cur);
        }
        pref[cur].push(i);
    }

    for _ in 0..q {
        let (i, k): (usize, usize) = cin.read();
        let i = a[i - 1];
        outln!(pref[i].partition_point(|&y| y <= k) + if i == n - 1 && k > n - 1 { k - (n - 1) } else { 0 });
    }
}
