...
int ans[2];
int n;
void solve(int l, int r, int ones, int blabla = 0) {
    if (l == r) {
        ans[ones] = l;
        return;
    }
    ...
    int k = ask(s);
    int left = m - l + 1;
    // k = (left - onesLeft) + (ones - onesLeft) + blabla
    // k = left + ones + blabla - 2 * onesLeft
    int onesLeft = (left + ones + blabla - k) / 2;
    int onesRight = ones - onesLeft;
    if (onesLeft == 0) {
        ans[0] = l;
        solve(m + 1, r, onesRight, blabla + onesLeft);
    } else if (onesLeft == left) {
        ans[1] = l;
        solve(m + 1, r, onesRight, blabla + onesLeft);
    } else {
        solve(l, m, onesLeft, blabla + onesRight);
    }
}
int32_t solve() {
    cin >> n;
