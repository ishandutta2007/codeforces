...
int main(){
    ...
    rep(ii,T){
        ...
        vector<int> vis(N+1,2e9);
        set<Tp> ss;
        vector<ll> tnx = nx;
        //out(tnx);
        vector<vector<ll>> pss; ll pv = -1;
        rep(i,N){
            if(vis[i+1]<2e9) continue;
            ll s = i+1; 
            ll nw = i+1;
            while(vis[nw]!=s){
                //cout << nw << endl;
            ...
            vis[nw] = s;
            if(i+1!=1){
                if(pv!=i) pss.push_back({i,i+1});
                else pss[pss.size()-1].push_back(i+1);
                pv = i+1;
            }
        }
