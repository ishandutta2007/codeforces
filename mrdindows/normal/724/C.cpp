...
int main() {
    ...
    FI(k) {
        cin >> dat[i];
        pii from = dat[i];
        int t = min(from.first, from.second);
        from.first -= t;
        from.second -= t;
        v[0][from].push_back(i);
        from = dat[i];
        t = min(from.first, m - from.second);
        from.first -= t;
        from.second += t;
        v[1][from].push_back(i);
        from = dat[i];
        t = min(n - from.first, from.second);
        from.first += t;
        from.second -= t;
        v[2][from].push_back(i);
        from = dat[i];
        t = min(n - from.first, m - from.second);
        from.first += t;
        from.second += t;
        v[3][from].push_back(i);
    }
    int x = 0, y = 0;
    int dx = 1, dy = 1;
    ll curTime = 0;
    while (true) {
//        DBN(x, y);
        int timeX = dx > 0 ? n - x : x;
        int timeY = dy > 0 ? m - y : y;
        int t = min(timeX, timeY);
        auto &mv = v[code(dx, dy)];
        if (mv.count({x, y})) {
            for (int q : v[code(dx, dy)][{x, y}]) {
                if (used[q]) continue;
                int pos = (dat[q].first - x) / dx;
                ans[q] = pos + curTime;
                used[q] = true;
            }
            v[code(dx, dy)][{x, y}].clear();
        }
        curTime += t;
        x += t * dx;
        y += t * dy;
