...
int DP(int d,int c,int f){
	...
	if (f){
		{
			if (!c){
				Add(res,DP(d-1,c,~k>>d&1));
			}
			else {
				if (k>>d&1){
					Add(res,(LL)(DP(d-1,c,0)+DP(d-1,c,1))*(1<<(c-1))%mod);
				}
				else {
					Add(res,(LL)DP(d-1,c,1)*(1<<(c-1))%mod);
				}
			}
		}
		if (k>>d&1){
			Add(res,(LL)DP(d-1,c+1,1)%mod);
		}
	}
	else {
		Add(res,(LL)DP(d-1,c,0)*(1<<c)%mod);
		Add(res,(LL)DP(d-1,c+1,0)%mod);
	}
//	printf("DP(%d,%d,%d) = %d\n",d,c,f,res);
	return res;
}
