...
int main(){
	riano_; string ans = "NO";
    ll N; cin >> N;
    //main 
    unionfind uf(N);
    vector<Pr> ev(2*N+1); ll l[N],r[N];
    set<Tp> cross,right;
    rep(i,N){
        cin >> l[i] >> r[i];
        ev[l[i]] = make_pair(i,0); ev[r[i]] = make_pair(i,1);
        right.emplace(l[i],r[i],i);
    }
    int cc = 0;
    for(ll cen=1;cen<=2*N;cen++){
        auto[id,lb] = ev[cen];
        if(lb==0){
            right.erase(make_tuple(l[id],r[id],id));
            cross.insert(make_tuple(l[id],r[id],id));
        }
        else{
            cross.erase(make_tuple(l[id],r[id],id));
            Tp tg = make_tuple(l[id],2e9,0);
            auto itr = cross.lower_bound(tg);
            while(itr!=cross.end()){
                if(cc==N) break;
                auto[l2,r2,i2] = *itr;
                uf.unite(id,i2); cc++; itr++;
                //cout << id << " " << i2 << endl;
            }
        }
    }
    if(cc==N-1&&uf.size(0)==N) ans = "YES";
