...
int n,k; Edg
//find k nodes s.t. ancestor union is maximal possible
int fa[SZ],bg[SZ],ed[SZ],D=0,dep[SZ],mk[SZ];
void dfs(int x,int f=0) {
	fa[x]=f;
	bg[x]=++D;
	for esb(x,e,b) if(b!=f) {
		dep[b]=dep[x]+1; dfs(b,x);
	}
	ed[x]=D;
}
#define M M_
...
void edt(int x,int l,int r,int t) {
	if(l>r||r<ls[x]||rs[x]<l) return;
	pd(x);
	if(l<=ls[x]&&rs[x]<=r) {
		tg[x]+=t;
		return;
	}
	edt(x+x,l,r,t),edt(x+x+1,l,r,t);
	upd(x);
}
void main()
{
	...
	dfs(1);
	for(int i=1;i<=n;++i) seg[bg[i]+M]=pii(dep[i]+5,i);
	init();
	int ans=0;
	auto f=[&](int r,int lw) {
		ll ans=8e18;
		...
		{
			ll w=L;
			ans=min(ans,w*(r*2-n)+w*w);
		}
		{
			ll w=R;
			ans=min(ans,w*(r*2-n)+w*w);
		}
		for(int o=opt-2;o<=opt+2;++o)
		if(L<=o&&o<=R) {
			ll w=o;
			ans=min(ans,w*(r*2-n)+w*w);
		}
	...
	for(int i=1;i<=k;++i) {
		...
		{
		ll w=ans-i,r=i,b=n-w-r;
		anss=max(anss,f(i,ans-i));
		}
	}
	cout<<anss<<"\n";
}
