...
main() {
    ...
    auto solve = [](vi &x) {
        ...
        while(!v.empty()) {
            ...
            v.pop_back();
            vmint tmp(s / 2 + 1);
            rep2(j, 0, s / 2) tmp[j] = cmb(s - j, j);
            vmint dp(s / 2 + now.size());
            rep(i, s / 2 + 1) {
                rep(j, now.size()) { dp[i + j] += tmp[i] * now[j]; }
            }
            now = dp;
        }
        vv(mint, res, 1801, 3601);
        int m = n - accumulate(all(x), 0);
        rep(i, now.size()) {
            rep(j, 1801) { res[i][j] = now[i] * cmb(m - i * 2, j) * prd(j); }
        }
        return res;
    };
    auto A = solve(a), B = solve(b);
    mint ans;
    rep(i, 1801) rep(j, 1801) { ans += A[i][j] * B[j][i]; }
    cout << ans << endl;
}