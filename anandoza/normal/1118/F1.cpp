...
int main() {
    ...
    vector<array<int, 3>> count(n);
    for (int i : reversed(tree.preorder)) {
        count[i][a[i]]++;
        if (tree.parent[i] != -1) {
            for (int j = 0; j < 3; j++) {
                count[tree.parent[i]][j] += count[i][j];
            }
        }
    }
    pd(count);
    ...
    int ans = 0;
    for (int i = 1; i < n; i++) {
        if (count[i][1] == count[0][1] && count[i][2] == 0)
            ans++;
        if (count[i][2] == count[0][2] && count[i][1] == 0)
            ans++;
    }
