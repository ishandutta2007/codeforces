...
{
	int T=read();
	rg(T)int n=read();rg(n)a[i]=read();gr
	int ans=n+n-1;
	rg(n-2)if((a[i]<a[i+1]&&a[i+1]>a[i+2])||(a[i]>a[i+1]&&a[i+1]<a[i+2]))ans++;gr
	rg(n-3)
	int b=a[i],c=a[i+1],d=a[i+2],e=a[i+3];
	if((b<=c&&c<=d)||(b<=c&&c<=e)||(c<=d&&d<=e)||(b<=d&&d<=e))continue;
	if((b>=c&&c>=d)||(b>=c&&c>=e)||(c>=d&&d>=e)||(b>=d&&d>=e))continue;
	ans++;gr
	oldl(ans);gr
	return 0;
