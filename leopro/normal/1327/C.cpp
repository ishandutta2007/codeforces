...
void solve() {
    autoint n, m;
    vector<char> actions;
    for (int i = 0; i < n - 1; ++i) actions.push_back('D');
    for (int i = 0; i < m - 1; ++i) actions.push_back('L');
    for (int i = 0; i < n; ++i){
        for (int j = 0; j < m - 1; ++j) actions.push_back(i & 1 ? 'L' : 'R');
        if (i != n - 1) actions.push_back('U');
    }
