...
struct io_setup {
    io_setup(bool speed_up = true, int precision = 20) {
...
int main() {
    ...
    rep(r, n + 1) rep(l, r) {
        mp[s[r] - s[l]].emplace_back(l, r);
    }
    for (auto &[k, v] : mp) {
        vector<pair<int, int>> w;
        int sz = v.size();
        rep(i, sz) {
            if (i == sz - 1 or v[i].second != v[i + 1].second) {
                w.push_back(v[i]);
            }
        }
        int sz2 = w.size();
        rep(i, 1, sz2) {
            chmax(w[i].first, w[i - 1].first);
        }
        v.swap(w);
    }
    auto find_next = [&](int l, int r) {
        int sum = s[r] - s[l];
        const auto &v = mp[sum];
        int i = lower_bound(all(v), make_pair(r, r)) - v.begin();
        if (i < int(v.size())) {
            return v[i];
        } else {
    ...
    while (ans_l < n) {
        pairs.emplace_back(ans_l + 1, ans_r);
        auto [nl, nr] = find_next(ans_l, ans_r);
        ans_l = nl, ans_r = nr;
    }
    print(pairs.size());
    print_all(pairs, "\n");
