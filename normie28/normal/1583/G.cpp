...
int ar[200001],br[200001],cr[200001];
int sus[200001];
int main()
{
	fio;
	...
	for (i=0;i<n;i++) ar[vec[i]]=i+1;
	
	sort(vec.begin(), vec.end(), [](int a, int b){
		return (br[a]<br[b]);
	});
	for (i=0;i<n;i++) br[vec[i]]=i+1;
	...
	a=n;
	for (i=1;i<=n;i++) {
		b=st.get(1,0,n,cr[i],n);
		st.upd(1,0,n,cr[i]-1,b);
		if (sus[i]==1) st.upd(1,0,n,cr[i]-1,1);
	}
	cout<<st.get(1,0,n,0,n);
}
// a;