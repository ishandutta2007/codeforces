...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		{
			if(good[i].length()==1||good[i].length()>s.length())
				continue;
			int[]lps=new int[good[i].length()];
			lps[0]=0;
			for(int j=1,k=0; j<good[i].length(); j++)
			{
				if(good[i].charAt(j)==good[i].charAt(k))
				{
					k++;
					lps[j]=k;
				}
			...
			int[]dp=new int[te];
			for(int j=0,k=0; j<te;)
			{
				if(s.charAt(j)==good[i].charAt(k))
				{
					k++;
					dp[j]=k;
					j++;
			...
			String temp=new StringBuilder(good[i]).reverse().toString();
			Arrays.fill(lps, 0);
			for(int j=1,k=0; j<temp.length();)
			{
			...
			int[]dp2=new int[te];
			for(int j=0,k=0; j<te; j++)
			{
				if(rs.charAt(j)==temp.charAt(k))
				{
					k++;
//					if(k==temp.length())
//					{
...
//					}
			...
			for(int k=0; k<s.length()-1; k++)
				if(dp[k]+dp2[s.length()-2-k]>=temp.length())
				{
					ans++;
					continue out;
				}
