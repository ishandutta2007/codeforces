...
main() {
    ...
    sort(all(a));
    vll rui{0};
    rep(i, n) rui.eb(rui.back() + a[i]);
    ll ans = 0;
    ll mem1 = 0, mem2 = 0;
    rep3(i, n - 1, -1) {
        ll now = t * (n - 1 - i);
        ll cost = (n - 1 - i) * A - (rui[n] - rui[i + 1]);
        if(cost > m) break;
        ll rem = m - cost;
        ll l = 0, r = A + 1;
        while(l < r - 1) {
            ...
            int k = lb(a, mid);
            k = min<int>(k, i + 1);
            if(k * (mid)-rui[k] - rui[0] > rem)
                r = mid;
            else
                l = mid;
        }
        if(chmax(ans, now + l * s)) { mem1 = (n - 1 - i), mem2 = l; }
    }
    cout << ans << endl;
    vll res(n);
    rep3(i, n - 1, n - mem1) res[id[i]] = A;
    rep(i, n) if(!res[id[i]]) res[id[i]] = max(mem2, a[i]);
    print(res);
}