...
fps calc(const V<fps>& a) {
  int n = sz(a);
  if (!n) return fps{1};

  auto dd = [&](auto rc, int l, int r) -> Affine<fps> {
    if (l + 1 == r) {
      return Affine{a[l], fps{1}};
    }
    int m = (l + r) / 2;
    auto L = rc(rc, l, m);
    auto R = rc(rc, m, r);
...
void Nyaan::solve() {
  ...
  auto dfs = [&](auto rc, int c) -> V<fps> {
    if (g[c].empty()) return V<fps>{fps{0, 1}};
    auto v = rc(rc, g[c][0]);

    if (sz(g[c]) == 1) {
      v.push_back(fps{0, 1});
    } else {
      v.push_back(calc(rc(rc, g[c][1])) << 1);
    }
    return move(v);
  };

  auto f = calc(dfs(dfs, 0));
  trc(f);
