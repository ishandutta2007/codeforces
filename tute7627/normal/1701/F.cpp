...
namespace add_sum{
  ...
  auto f=[](M x,M y)->M{
    return {x.sum+y.sum,x.sum2+y.sum2,x.len+y.len};
  };
  auto g=[](M x,L y)->M{
    return {x.sum+x.len*y,x.sum2+y*y*x.len+2*x.sum*y,x.len};
  };
  auto h=[](L x,L y){
    return x+y;
  };
  LazySegmentTree<M,L,decltype(f),decltype(g),decltype(h)>make(int n){
    return {n,f,g,h,{0,0,0},0};
  }
}
int main(){
  cin.tie(nullptr);
  ...
  ll q,d;cin>>q>>d;
  ll sz=200005;
  auto seg=add_sum::make(sz);
  //rep(i,0,sz)seg.set(i,{0,0,0});
  //seg.build();
  vector<ll>x(sz);
  while(q--){
    ll a;cin>>a;
    x[a]^=1;
    if(x[a]==1){
      auto s=seg.query(max(0LL,a-d),a);
      seg.update(a,{s.len,s.len*s.len,1});
      seg.update(a+1,min(a+d+1,sz),1);
    }
    else{
      seg.update(a,{0,0,0});
      seg.update(a+1,min(a+d+1,sz),-1);
    }
    auto s=seg.query(0,sz);
    //rep(i,0,10)cout<<seg[i].len <<",";cout<<endl;
    cout<<(s.sum2-s.sum)/2<<endl;
  }
  return 0;
}