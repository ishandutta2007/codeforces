...
void works() {
	...
	s[++top]=mkp(-INF,3*n+1); 
	foR(i,3*n,1) {
		int tx=lower_bound(s+1,s+1+top,mkp((a[i]+1)/2,-INF))-s-1; 
		pos[i]=s[tx].second; 
		while(top&&s[top].first>=a[i]) {
			top--; 
		}
		s[++top]=mkp(a[i],i); 
	}
	int mn=3*n+1; 
	For(i,1,n) ckmin(mn,pos[i]) ;
	if(mn==3*n+1) {
	...
	int nowr=3*n; 
	foR(i,3*n,1) {
		st.insert(mkp(pos[i],i)); 
		while((st.begin()->first)<=nowr) {
			st.erase(mkp(nowr,i)); 
			nowr--; 
		}
		if(i<=n) ans[i]=nowr; 
	}
	For(i,1,n) printf("%d ",ans[i]-i+1); 
	printf("\n"); 
}
signed main() {
	#ifdef wasa855
