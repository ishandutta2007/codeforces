...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        ta = 1, tb = -(a[i] + b[i]), tc = a[i];
        s[i] = (-tb - sqrt(std::abs(tb * tb - 4 * ta * tc))) / (2 * ta);
        t[i] = (-tb + sqrt(std::abs(tb * tb - 4 * ta * tc))) / (2 * ta);
    }
    for (int i = 1; i <= n; ++i) std::cout << s[i] - s[i - 1] << ' ';
    std::cout << std::endl;
    for (int i = 1; i <= n; ++i) std::cout << t[i] - t[i - 1] << ' ';
    std::cout << std::endl;
    return 0;
}