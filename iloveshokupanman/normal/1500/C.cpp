...
int main() {
    ...
    if (mpa != mpb) { cout << -1 << endl; return 0; }
    ...
    vector<set<ll>> fr(N - 1), bk(N - 1);
    vll c(M);

    rep(i, N - 1) {
        rep(j, M) {
            if (B[i][j] < B[i + 1][j]) { fr[i].insert(j); }
            if (B[i][j] > B[i + 1][j]) { bk[i].insert(j); c[j]++; }
        }
    }
    ...
    rep(i, M) {
        if (c[i] == 0) { ans.push_back(i); q.push(i); }
    }
    ...
    while (!q.empty()) {
        ...
        rep(i, N - 1) {
            if (fr[i].count(x)) {
                ...
                for (ll y : bk[i]) {
                    c[y]--;
                    if (c[y] == 0) { ans.push_back(y); q.push(y); }
                }
                bk[i].clear();
            }
        }
