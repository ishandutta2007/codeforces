...
char s[N];int n,top,res;pi a[N];
int main(){
	scanf("%s",s+1),n=strlen(s+1);
	fp(i,1,n){
		if(s[i]-'A'+1!=a[top].fi)a[++top].fi=s[i]-'A'+1;
		++a[top].se;
	}
	if(top&1^1)return puts("0"),0;
	if(a[(top+1)>>1].se<2)return puts("0"),0;
	fp(i,1,top>>1)if(a[i].fi!=a[top-i+1].fi||a[i].se+a[top-i+1].se<3)return puts("0"),0;
	printf("%d\n",a[(top+1)>>1].se+1);
	return 0;
}