...
using mn1 = modnum<int(1e9 + 21)>;
using mn2 = modnum<int(1e9 + 33)>;
using mn3 = modnum<int(1e9 + 87)>;
...
struct num_hash {
    ...
    num_hash() : a(0), b(0), c(0) {}
    num_hash(int64_t v) : a(v), b(v), c(v) {}

    num_hash& operator += (const num_hash& o) { a += o.a; b += o.b; c += o.c; return *this; }
    num_hash& operator *= (const num_hash& o) { a *= o.a; b *= o.b; c *= o.c; return *this; }

    friend num_hash operator + (const num_hash& a, const num_hash& b) { return num_hash(a) += b; }
    friend num_hash operator * (const num_hash& a, const num_hash& b) { return num_hash(a) *= b; }

    friend bool operator != (const num_hash& a, const num_hash& b) { return a.a != b.a || a.b != b.b || a.c != b.c; }
};
...
void solve() {
    ...
    vi order(N);
    iota(all(order), 0);
    sort_by(order, sz(pos[a]) > sz(pos[b]));

    int root = order[0];

    int root_ct = sz(pos[root]);
    //int root_loops = max(0, root_ct - (N - root_ct));

    vvi g(N);
    vvi r(N);

    F0R (i, N) {
        if (a[i] == b[i]) { // self-loop
            if (a[i] != root) { // must involve the root
                ps("WA");
                return;
            }

            /*if (--root_loops < 0) { // only this many allowed
                ps("WA");
                return;
            }*/
        }

        // build graph excluding a[i]'s incoming edges
        if (a[i] != root) {
            g[b[i]].pb(a[i]);
            r[a[i]].pb(b[i]);
        }
    ...
    if (scc.components != N) { // no loops NOT involving the root allowed
        ps("WA");
        return;
    }
    ...
    sort_by(proc, scc.label[a] > scc.label[b]);

    vector<num_hash> to(N); // count paths from the root to the node
    to[root] = 1;

    for (int x : proc) {
        for (int y : r[x]) {
            assert(scc.label[y] > scc.label[x]);
            to[x] += to[y];
        }
    }
    ...
    reverse(all(proc));
    vector<num_hash> fr(N); // count paths from the node to the root
    for (int x : proc) {
        for (int y : g[x]) {
            if (y == root) {
                fr[x] += 1;
    ...
    F0R (i, N) {
        if (i != root && sz(pos[i]) == sz(pos[root])) {
            if (to[i] * fr[i] != cyc) {
                ps("WA");
                return;
            }
