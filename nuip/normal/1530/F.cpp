...
template <int mod=MOD> struct ModInt{
  ...
  ModInt operator*(const ModInt &n)const{return int(v)*n.v%mod;}
  ModInt operator/(const ModInt &n)const{return int(v)*modpow_(n.v%mod,-1,mod)%mod;}
  ModInt operator+(const ll &n)const{return v+n<mod ? v+n : v+n-mod;}
  ModInt operator-(const ll &n)const{return v-n<0 ? v-n+mod : v-n;}
  ModInt operator*(const ll &n)const{return int(v)*(n%mod)%mod;}
  ModInt operator/(const ll &n)const{return int(v)*modpow_(n%mod,-1,mod)%mod;}
  ...
  ModInt& operator*=(const ModInt &n){v=int(v)*n.v%mod; return *this;}
  ModInt& operator/=(const ModInt &n){v=int(v)*modpow_(n.v,-1,mod)%mod; return *this;}
  ModInt& operator+=(const ll &n){v+=n; if(v>=mod) v-=mod; return *this;}
  ModInt& operator-=(const ll &n){v-=n; if(v<0) v+=mod; return *this;}
  ModInt& operator*=(const ll &n){v=int(v)*n%mod; return *this;}
  ModInt& operator/=(const ll &n){v=int(v)*modpow_(n,-1,mod)%mod; return *this;}
...
int main(){
	...
	// cauto output=
	// 	[&](vector<mint> dp){
	// 		rep(i,n+2)rep(st,1<<(n+2))if(st>>i&1) dp[st^(1<<i)]-=dp[st];
	// 		out(dp,1);
	// 	};
	vector<mint> dp(1<<(n+2),1);
	rep(i,n){
		cauto a=i;
		cauto b=n-1-i;
		mint P=1; rep(j,n) P*=ps[i][j];
		rep(st,(1<<(n))){
			mint p=1;
			rep(j,n)if(st>>j&1) p*=ps[i][j];
			if((st>>n&1) && (~st>>a&1)) p*=ps[i][a];
			if((st>>n&2) && (~st>>b&1) && !(a==b && (st>>n&3)==3)) p*=ps[i][b];
			if(st>>a&1){
				if(st>>b&1){
					dp[st]*=p-P;
					dp[st|(1<<n)]*=p-P;
					...
					dp[st]*=p-P;
					dp[st|(1<<n)]*=p-P;
					dp[st|(2<<n)]*=p*ps[i][b]-P;
					dp[st|(3<<n)]*=p*ps[i][b]-P;
				}
			}else{
				if(st>>b&1){
					dp[st]*=p-P;
					dp[st|(1<<n)]*=p*ps[i][a]-P;
					dp[st|(2<<n)]*=p-P;
					dp[st|(3<<n)]*=p*ps[i][a]-P;
				}else{
					dp[st]*=p-P;
					dp[st|(1<<n)]*=p*ps[i][a]-P;
					dp[st|(2<<n)]*=p*ps[i][b]-P;
					dp[st|(3<<n)]*=p*(a==b?ps[i][a]:ps[i][a]*ps[i][b])-P;
				}
			}
		}
		// out(dp,1);
