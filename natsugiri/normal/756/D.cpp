...
int N;
char S[10011];
Mint dp[26][5011]; // [last][len]
Mint nxt[5011], sum[5011];

int main() {
    init();
    scanf("%d%s", &N, S);

    REP (i, N) if (i == 0 || S[i] != S[i-1]) {
	memset(nxt, 0, sizeof nxt);
	const int c = S[i] - 'a';
	nxt[1] += 1;

	REP (j, i+2) nxt[j+1] += sum[j] - dp[c][j];

	REP (j, i+2) {
	    sum[j] = sum[j] - dp[c][j] + nxt[j];
	    dp[c][j] = nxt[j];
	}
    }

    Mint ans = 0;
    REP (t, 26) for (int i=1; i<=N; i++) if (dp[t][i].x) {
	ans += nCk(N - 1, i-1) * dp[t][i];
    }
    printf("%d\n", ans.getInt());
