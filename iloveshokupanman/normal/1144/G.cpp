...
int main() {
    ...
    for (ll i = 1; i < N; i++) {
        ...
        if (a[i - 1] < a[i] && dpi[i - 1] > dpi[i]) {
            dpi[i] = dpi[i - 1];
            c[0][i] = 0;
        }

        if (dpd[i - 1] < a[i] && a[i - 1] > dpi[i]) {
            dpi[i] = a[i - 1];
            c[0][i] = 1;
        }

        if (a[i - 1] > a[i] && dpd[i - 1] < dpd[i]) {
            dpd[i] = dpd[i - 1];
            c[1][i] = 1;
        }

        if (dpi[i - 1] > a[i] && a[i - 1] < dpd[i]) {
            dpd[i] = a[i - 1];
            c[1][i] = 0;
        }
       

        //cout << dpi[i] << " " << dpd[i] << " " << c[0][i] << " " << c[1][i] << endl;
    }

    if (dpi[N - 1] == -INF && dpd[N - 1] == INF) { cout << "NO" << endl; return 0; }
    cout << "YES" << endl;

    vll ans;
    if (dpi[N - 1] > -INF) { ans.push_back(0); }
    else { ans.push_back(1); }

    for (ll i = N - 1; i >= 1; i--) {
        //cout << i << " " << ans.back() << endl;
        ans.push_back(c[ans.back()][i]);
    }
