...
const int MODE=51123987;

int n,A[160];
int f[60][60][60][8],g[60][60][60][8];
int C[300][300];
...
int main()
{
...
#endif
	memset(C,0,sizeof(C));
	for (int i=0;i<300;i++) for (int j=0;j<=i;j++)
		C[i][j]=(j==0)?1:((C[i-1][j-1]+C[i-1][j])%MODE);
	char s[160];
	scanf("%d",&n);
	scanf("%s",s);
	...
	{
		int key=A[step];
		if (step-1>=0 && key==A[step-1]) continue;
		memset(g,0,sizeof(g));
		if (key==0)
		{
			for (int set=0;set<8;set++) if (contain(set,key))
				for (int c0=0;c0<59;c0++) for (int c1=0;c1<60 && c0+c1<=step;c1++) for (int c2=0;c2<60 && c0+c1+c2<=step;c2++) if (f[c0][c1][c2][set])
					addIt(g[c0+1][c1][c2][7-two(key)],f[c0][c1][c2][set]);
		}
		if (key==1)
		{
			for (int set=0;set<8;set++) if (contain(set,key))
				for (int c0=0;c0<59;c0++) for (int c1=0;c1<60 && c0+c1<=step;c1++) for (int c2=0;c2<60 && c0+c1+c2<=step;c2++) if (f[c0][c1][c2][set])
					addIt(g[c0][c1+1][c2][7-two(key)],f[c0][c1][c2][set]);
		}
		if (key==2)
		{
			for (int set=0;set<8;set++) if (contain(set,key))
				for (int c0=0;c0<59;c0++) for (int c1=0;c1<60 && c0+c1<=step;c1++) for (int c2=0;c2<60 && c0+c1+c2<=step;c2++) if (f[c0][c1][c2][set])
					addIt(g[c0][c1][c2+1][7-two(key)],f[c0][c1][c2][set]);
		}
		for (int c0=0;c0<60;c0++) for (int c1=0;c1<60 && c0+c1<=step;c1++) for (int c2=0;c2<60 && c0+c1+c2<=step;c2++) for (int set=0;set<8;set++) if (f[c0][c1][c2][set])
			if (contain(set,key))
				addIt(g[c0][c1][c2][set-two(key)],f[c0][c1][c2][set]);
			else
				addIt(g[c0][c1][c2][set],f[c0][c1][c2][set]);
		memcpy(f,g,sizeof(f));
	}
	int R=0;
	vector<int> pList;
	for (int c0=0;c0<60;c0++) for (int c1=0;c1<60 && c0+c1<=n;c1++) for (int c2=0;c2<60 && c0+c1+c2<=n;c2++)
		if (c0+c1+c2==n && abs(c0-c1)<=1 && abs(c0-c2)<=1 && abs(c1-c2)<=1)
		{
			pList.push_back(c0);
			pList.push_back(c1);
			pList.push_back(c2);
		}
	for (int c0=0;c0<60;c0++) for (int c1=0;c1<60 && c0+c1<=n;c1++) for (int c2=0;c2<60 && c0+c1+c2<=n;c2++) for (int set=0;set<8;set++) if (f[c0][c1][c2][set])
		for (int i=0;i<SIZE(pList);i+=3)
		{
			int d0=pList[i];
			int d1=pList[i+1];
			int d2=pList[i+2];
			if (c0>d0 || c1>d1 || c2>d2) continue;
			if (c0==0 && d0>0 || c1==0 && d1>0 || c2==0 && d2>0) continue;
			int64 t=f[c0][c1][c2][set];
			if (c0) t=t*C[d0-1][c0-1]%MODE;
			if (c1) t=t*C[d1-1][c1-1]%MODE;
			if (c2) t=t*C[d2-1][c2-1]%MODE;
			R=(R+(int)t)%MODE;
		}
	printf("%d\n",R%MODE);
	return 0;
