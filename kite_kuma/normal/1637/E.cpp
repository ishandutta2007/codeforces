...
ll solve(V<pair<ll, ll>> badpairs, vll a) {
	set<pair<ll, ll>> badpair_set(badpairs.begin(), badpairs.end());
	auto is_good = [&](ll x, ll y) { return x != y && !badpair_set.count(pair(x, y)) && !badpair_set.count(pair(y, x)); };

	map<ll, int> cnt;
	foa(t, a) cnt[t]++;
	vector<int> countings;
	foa(t, cnt) countings.push_back(t.second);
	unq_sort(countings);
	const int cs = countings.size();
	auto cnt_to_id = [&](int cnt_value) { return lb(countings, cnt_value); };
	VV<ll> countingvalues(cs);
	foa(t, cnt) { countingvalues[cnt_to_id(t.second)].push_back(t.first); }
	foa(v, countingvalues) sort(rall(v));
	ll ans = -1;
	debug(countings);
	debug(countingvalues);
	rep(i, cs) rep(j, i, cs) {
		const int cntsum = countings[i] + countings[j];
		foa(x, countingvalues[i]) {
			foa(y, countingvalues[j]) {
				if(is_good(x, y)) {
					if(chmax(ans, (x + y) * ll(cntsum))) debug(ans, x, y);
					break;
				}
			}
		}
...
int main() {
	...
	rep(t) {
		...
		cin >> n >> m;
		VEC(ll, a, n);
		V<pair<ll, ll>> badpairs;
		rep(m) {
			ll x, y;
			cin >> x >> y;
			badpairs.emplace_back(x, y);
		}
		print(solve(badpairs, a));
	}
