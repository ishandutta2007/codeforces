...
int main() {
    int N, X;
    cin >> N >> X;
    vector<vector<mod_int>> dp(N + 1, vector<mod_int>(X + 1, 0));
    ...
    for (int n = 2; n <= N; n++)
        for (int x = n - 1; x <= X; x++)
            for (int alive = 1; alive <= n; alive++) {
                mod_int choose_alive = choose(n - 1, alive - 1);
                mod_int dead_ways = mod_int(n - 1).pow(n - alive);
                dp[n][x] += choose_alive * dead_ways * dp[alive][x - (n - 1)];
            }

    mod_int ans = mod_int(X).pow(N) - N * dp[N][X];
    cout << ans << '\n';
}