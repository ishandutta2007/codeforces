...
int main() {
    ...
    while (tt--) {
        ...
        f0r(i, n+1) {
            precnt[i] = (i?precnt[i-1]:0) + cnt[i];
        }
        auto sum = [&](int l, int r) -> ll { 
            if (l>r) return 0;
            return pre[r] - (l?pre[l-1]:0);
        };
        auto sumcnt = [&](int l, int r) -> ll { 
            if (l>r) return 0;
            return precnt[r] - (l?precnt[l-1]:0);
        };
        ans = 1e18;
        f0r(i, n+1) {
            ll res = sum(0, i-1);
                res += sum(i+1, n) - 1LL * sumcnt(i+1, n) * i;
                ckmin(ans, res);
        }
        ps(ans);
    }
