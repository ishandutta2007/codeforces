...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/ds/segtree2d.hpp"
...
void solve() {
  ...
  FOR(N) {
    LL(a, b, c);
    S.eb(a), F.eb(b), T.eb(c);
  }
  FOR(Q) {
    LL(a, b, c);
    L.eb(a), R.eb(b), B.eb(c);
  }
  using Mono = Monoid_Min<int>;
  SegTree2D<Mono, int, 0> seg(S, F);
  auto I = argsort(T);
  auto J = argsort(B);
  reverse(all(J));
  vi ANS(Q);
  ...
  map<int, int> to_idx;
  FOR(i, N) to_idx[T[i]] = i;

  for (auto&& qid: J) {
    ll b = B[qid];
    while (len(I) && T[I.back()] >= b) {
      int i = I.back();
      I.pop_back();
      seg.multiply(S[i], F[i], T[i]);
    }
    ll t = seg.prod(0, R[qid], L[qid] + 1, 1 << 30);
    ANS[qid] = (t == Mono::unit() ? -1 : to_idx[t] + 1);
  }
  print(ANS);
}
