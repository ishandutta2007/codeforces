...
int main(){
    ...
    VEC(pll,a,n);
    vector<ll>Y;
    each(x,y,a)Y.push_back(y);
    Uniq(Y);
    unordered_map<ll,ll>py;
    ll at=0;
    py[Y[0]]=at;
    rep(i,1,Y.size()){
        at+=min(k,Y[i]-Y[i-1]);
    ...
    each(x,y,a){
        y=py[y];
        q.push_back({x,1,y,y+k});
        q.push_back({x+k,-1,y,y+k});
    }
    Sort(q);
    Rev(q);
    vec(int,s,30000001);
    vec(ll,s_cnt,n+1);
    ll cnt_max=0;
    for(ll i=q.back()[0];;cnt_max?i++:i=q.back()[0]){
        while(q.back()[0]==i){
            auto[x,c,l,r]=q.back();
            q.pop_back();
            cnt_max+=c;
        ...
        if(q.empty())break;
        rep(i,1,cnt_max+1)ans[i]+=s_cnt[i];
    }
