...
void solve(){
  ...
  vector<vector<tuple<li,li,li>>> G(n);

  for(li ei = 0; ei < (li)(n-1); ++ei){
    li i,j,x,y;
    cin>>i>>j>>x>>y;
    --i; --j;
    G[i].emplace_back(j,y,x);
    G[j].emplace_back(i,x,y);
  }
  ...
  auto dfs = letrec([&](auto dfs, int i, int p) -> void {
    for(auto e : G[i]) {
      ...
      for(auto p : M) {
        ...
        base[p.first] = max(base[p.first], -delta[p.first]);
      }
  ...
  ZN cur = 1;
  for(li p = (1); p <= (li)(n); ++p) for(li j = 0; j < (li)(base[p]); ++j) cur *= p;
  ZN ans = 0;
