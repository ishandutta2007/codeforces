...
void solve() {
    ...
    str cp = s; sort(all(cp));
    if (cp >= t) {
        cout << -1 << nl;
        return;
    }
    loc.clear(); loc.rsz(26);
    R0F(i, n) loc[s[i]-'a'].pb(i);
    st.init(n);
    F0R(i, n) st.upd(i, 1);
    ll ans = 1e18, cur = 0;
    vis.assign(n, false);
    ...
    F0R(i, n) {
        ...
        F0R(ch, t[i]-'a') if (! loc[ch].empty()) ans = min(ans, cur + st.query(0, loc[ch].back())-1);
        int ch = t[i]-'a';
        if (s[x] != t[i]) {
            if (loc[ch].empty()) break;
            int y = loc[ch].back();
            st.upd(y, 0);
            cur += st.query(0, y);
            vis[y]=true;
