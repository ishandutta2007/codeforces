...
int main(int argc, char** argv) {
    ...
    for (int ii = 0; ii < 2; ii++) {
        ...
        bool ok = true;
        for (int i = 1; i < n; i++) {
            if (isz(up) && a[i] <= a[up.back()] && isz(down) && a[i] >= a[down.back()]) {
                ok = false;
                break;
            }
            if (cup) {
                if (!isz(up) || a[i] > a[up.back()]) {
                    up.push_back(i);
                } else {
                    if (a[i] < a[up.back()] && (down.empty() || a[up.back()] < a[down.back()])) {
                ...
                if (!isz(down) || a[i] < a[down.back()]) {
                    down.push_back(i);
                } else {
                    if (a[i] > a[down.back()] && (up.empty() || a[up.back()] < a[down.back()])) {
