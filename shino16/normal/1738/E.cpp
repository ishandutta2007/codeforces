...
void solve() {
  ...
  if (count(all(a), 0) == n) return out(mint(2).pow(n-1));
  int cl0 = 0; while (a[cl0] == 0) cl0++;
  a.erase(a.begin(), a.begin() + cl0);
  int cr0 = 0;
  while (a.back() == 0) cr0++, a.pop_back();
  n = a.size();
  ...
  while (l < r) {
    ...
    if (sl == sr) {
      int l0 = 0, r0 = 0;
      while (l < r && a[l] == 0) l0++, l++;
      while (l < r && a[r-1] == 0) r0++, r--;

      if (l != r) {
        mint now = 0;
        rep(k, min(l0, r0)+2) now += (mint)binom(l0+1, k) * binom(r0+1, k);
        ans *= now;
      } else {
        ans *= mint(2).pow(l0 + 1);
      }
  ...
  mint now = 0;
  rep(k, min(cl0, cr0)+1) now += (mint)binom(cl0, k) * binom(cr0, k);
  ans *= now;
