...
inline void AC() {
    ...
    for(_ i = 0; i <= 25; ++i) g[i].clear();
    for(_ i = 0; i < n; ++i) g[a[i]-'a'].push_back(i+1);
    build(1, n, 1);
    _ ad = 0;
    for(_ i = 1; i <= n; ++i) {
        for(_ j = 0; j < b[i-1]-'a'; ++j) if(!g[j].empty()) s = min(s, query(1, n, g[j].front(), 1)+ad);
        if(g[b[i-1]-'a'].empty()) break;
        ad += query(1, n, g[b[i-1]-'a'].front(), 1);
        update(g[b[i-1]-'a'].front(), n, 1, n, 1);
        g[b[i-1]-'a'].pop_front();
    }
    if(s == 1E18) exit(1);
    writeln(s);
...
_ query(_ l, _ r, _ p, _ pos) {
    ...
    _ md = (l+r)>>1;
    pushDown(pos);
    if(p <= md) return query(l, md, p, pos<<1);
    else return query(md+1, r, p, pos<<1|1);
}