...
void solve() {
    ...
    map<pii, int> sides;
    auto put = [&](int i, int a, int b) {
        if (a > b) swap(a, b);
        auto it = sides.find({a, b});
        if (it != sides.end()) {
            int j = it->s;
            adj[i].pb(j);
            adj[j].pb(i);
    ...
    vi order = {1};
    int cur = -1;
    trav (e, sides) if (e.f.f == 0) cur = e.f.s;
    for (int last = 0; cur; tie(cur, last) = mp(nbr[cur] ^ last, cur)) order.pb(cur + 1);

    pc(order);

    vi cuts;
    auto dfs = [&](auto& self, int loc, int par) -> void {
        trav (nbr, adj[loc]) if (nbr != par) self(self, nbr, loc);
        cuts.pb(loc + 1);
    };
