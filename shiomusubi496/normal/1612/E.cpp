...
int main() {
    ...
    reps (i, 20) {
        vector<PLL> D(200001);
        rep (j, 200001) D[j].second = j;
        rep (j, N) D[A[j]].first += min(i, B[j]);
        D.erase(remove_if(D.begin(), D.end(), [](const PLL& p) -> bool { return p.first == 0; }), D.end());
        if (D.size() < i) continue;
        sort(D.rbegin(), D.rend());
        ll sum = 0;
        rep (j, i) sum += D[j].first;
        if (p.second * i < p.first * sum) {
            p = {i, sum};
            C.resize(i);
            rep (j, i) C[j] = D[j].second;
        }
    }
    cout << C.size() << endl;
