...
void solve()
{
	...
	for (int i = 1; i <= n; ++ i)
	{
		...
		for (int j = 0; j <= len; ++ j)
		{
			...
			{
				int jj = min(max(a[i].X-a[ii].X-1-j, 0), L[ii]);
				freshmin(f[i][j], f[ii][jj]+max(j-s[i], 0));
			}
			freshmin(ans, f[i][j]+max(m-(a[i].X+j), 0));
		}
		
		for (int j = len-1; j >= 0; -- j)
			freshmin(f[i][j], f[i][j+1]);
