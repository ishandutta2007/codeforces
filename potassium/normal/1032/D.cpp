...
int main() {
	ll a,b,c; R(a,b,c);
	vector<pair<long double, long double>> p(2); R(p);
	
	auto fixX = [&](long double x) {
		if (b == 0) return;
		p.pb({x, -(a * x + c) / b});
	};
	
	auto fixY = [&](long double y) {
		if (a == 0) return;
		p.pb({-(b * y + c) / a, y});
	};
	
	auto d = [&](int x, int y) {
		long double dx = abs(p[x].F - p[y].F);
		long double dy = abs(p[x].S - p[y].S);
	...
	vector<vector<long double>> dist(n, vector<long double>(n, 1e12));
	for (int i = 0; i < n; i++)
		for (int j = 0; j < n; j++)
			dist[i][j] = d(i, j);
