...
LL ask(int l,int r,int now){
	...
	LL re=ask(l+1,r,now+1),ned;
	LL g=__gcd(a[l],m[now]);
//	if(g==1){
		if(flag==0) ned=re;
		else ned=re+m[now+1];
//	}
...
//	cout<<l<<" "<<ned<<" "<<flag<<endl;
	...
	flag=1;
	v=v%m[now]*qpow(a[l],ned,m[now])%m[now];
	return v;
}
void MAIN(){
	int u,v,ans;
	...
	while(m[cnt]>1) {
		++cnt;
		m[cnt]=getphi(m[cnt-1]);
	}
//	for(int i=0;i<=cnt;++i) cout<<m[i]<<" ";cout<<endl;
	for(int i=1;i<=n;++i){
		cin>>a[i];
	}
	cin>>Q;
