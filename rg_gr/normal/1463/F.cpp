...
signed main()
{
	...
	// odp(L,R);
	// return 0;
	for(int i=0;i<(1<<max(x,y));i++)
	{
		if(i&(i>>x))goto a;
		if(i&(i>>y))goto a;
		wl[i]=s*__builtin_popcount(i)+__builtin_popcount(((1<<L)-1)&i);
		wr[i]=s*__builtin_popcount(i)+__builtin_popcount(((1<<R)-1)&i);
		a:;
		for(int j=0;j<max(x,y);j++)if(i>>j&1)mxe(wr[i],wr[i^(1<<j)]);
	}
	int ans=0;
	for(int i=0;i<(1<<max(x,x));i++)
	{
		int ban1=i<<x;
		int ban2=i<<y;
		// int ban3=
		ban1|=ban2;
		int ok=(1ll<<x+y)-1;ok^=ban1;ok>>=x;
		// odp(i,ok);
		// printf(": ");odp(wl[i],wr[ok]);
		// if(wl[i]+wr[ok]>672)printf("!!!\n");
		ans=max(ans,wl[i]+wr[ok]);
	}
	oldl(ans);
	return 0;
}