...
int main() {
	...
	vector<ll> ps(n);
	partial_sum(p.begin(), p.end(), ps.begin());

	ll ans = 0;
	if (x[0] > 0) chkmax(ans, ps[min((x[0] - 1) / 100, n - 1)]);
	if (x[m - 1] <= (n - 1) * 100) chkmax(ans, ps[n - 1] - ps[x[m - 1] / 100]);

	for (int i = 1; i < m; i++) {
		int l = x[i - 1] / 100 + 1;
		int r = (x[i] - 1) / 100;
		int len = (x[i] - x[i - 1] - 1) / 200;
		chkmin(r, n - 1);

		for (int j = l; j <= r; j++)
			chkmax(ans, ps[j] - ps[max(j - len - 1, l - 1)]);
	}
