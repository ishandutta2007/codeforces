...
void solve(){
	...
	FOR(L,2,n){
		FOR(l,1,n-L+1){
			...
			FOR(i,1,len[a[l]]){
				if(p[a[l]][i]>l&&p[a[l]][i]<=r){
					gmin(dp[l][r],dp[l][p[a[l]][i]-1]+dp[p[a[l]][i]][r]);
				}
			}
			FOR(i,1,len[a[r]]){
				if(p[a[r]][i]<r&&p[a[r]][i]>=l){
					gmin(dp[l][r],dp[l][p[a[r]][i]]+dp[p[a[r]][i]+1][r]);
				}
			}
		}
	}
