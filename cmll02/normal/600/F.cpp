// Problem: F. Edge coloring of bipartite graph
...
#define f u
#define g v
int in[9873],out[3625],f[697875],g[253487];
signed main()
{
	initprog();
	...
	if(A!=B)
	{
		for(int t=B,pos=v[i];pos;pos=q[pos][t],t^=A^B)
			std::swap(q[pos][A],q[pos][B]);
	}
	gr
	odl(ans);
	rg(k)rg_(j,ans)if(q[u[i]][j]==v[i])odb(j);gr gr
	return 0;
}