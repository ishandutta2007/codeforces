...
struct dsu {
	...
	void mark(int x) { bad[find(x)] = 1;}
	bool B(int x) { return bad[find(x)]; }
};
int main() {
	...
	while(n--) {
		...
		for(auto &i : a) cin >> i;
		set<int> x(all(a));
		if(x.size() != a.size()) D.mark(a[0]);
		for(int i = 1; i < a.size(); i++) {
			D.join(a[i-1], a[i]);
			if(!prv[a[i]]) prv[a[i]] = a[i-1];
			if(!nxt[a[i-1]]) nxt[a[i-1]] = a[i];
			if(prv[a[i]] != a[i-1] || nxt[a[i-1]] != a[i])
				D.mark(a[0]);
		}
	}
	poly p(k+1);
	p[0]+=1;
	int tlen = 0;
	for(int i = 1; i <= k; i++) if(!D.B(i) && !prv[i]) {
		int len = 1, t = i;
		while(nxt[t]) len++, t = nxt[t];
		tlen += len;
		p[len]-=1;
	}
	p = p.inv(m+1);
