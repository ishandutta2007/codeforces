...
void solve() {
    ...
    vi mx(n + 1);
    auto dfs_pre = [&](auto &dfs_pre, int u, int p, vi &num, vi &dem) -> void {
        debug(u);
        for (auto[v, r]: G[u]) {
            if (v == p) continue;
            auto[x, y] = r;
            auto xi = fac[x].begin();
            auto yi = fac[y].begin();
            vector<pair<int, P>> hist;
            while (xi != fac[x].end() or yi != fac[y].end()) {
                int nx = (xi == fac[x].end() ? inf : xi->first);
                int ny = (yi == fac[y].end() ? inf : yi->first);
                debug(nx, ny);
                if (nx == ny) {
                    hist.eb(nx, P(num[nx], dem[nx]));
                    num[nx] += xi->second;
                    dem[nx] += yi->second;
                    int m = min(num[nx], dem[nx]);
                    num[nx] -= m;
                    dem[nx] -= m;
                    chmax(mx[nx], dem[nx]);
                    ++xi, ++yi;
                    ...
                    hist.eb(ny, P(num[ny], dem[ny]));
                    dem[ny] += yi->second;
                    int m = min(num[ny], dem[ny]);
                    num[ny] -= m;
                    dem[ny] -= m;
                    chmax(mx[ny], dem[ny]);
                    ++yi;
            ...
            debug(hist);
            dfs_pre(dfs_pre, v, u, num, dem);
            for (auto[val, ori]: hist) {
                num[val] = ori.first;
                dem[val] = ori.second;
            }
        }
    ...
    vi dem(n + 1), num(n + 1);
    dfs_pre(dfs_pre, 0, -1, num, dem);
    mint now = 1;
    rep(i, 2, n + 1) now *= mint(i).pow(mx[i]);
    debug(now);
    mint ans = 0;
    auto dfs = [&](auto &dfs, int u, int p, vi &ls) -> void {
        ans += now;
        for (auto[v, r]: G[u]) {
            if (v == p) continue;
            auto[x, y] = r;
            mint mul = 1, div = 1;
            for (auto[val, cnt]: fac[x]) {
                ls[val] += cnt;
                mul *= mint(val).pow(cnt);
            }
            for (auto[val, cnt]: fac[y]) {
                ls[val] -= cnt;
                div *= mint(val).pow(cnt);
            }
            now *= mul / div;
            dfs(dfs, v, u, ls);
            for (auto[val, cnt]: fac[x]) {
...
int main() {
    fac.resize(200010);
    rep(i, 2, 200010) fac[i] = sv.factor_list(i);
    int t;
    cin >> t;
    rep(i, t) solve();
}