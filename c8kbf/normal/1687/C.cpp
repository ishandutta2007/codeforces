...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) a[i] -= b[i];
    for(_ i = 1; i <= m; ++i) d[i] = 0;
    for(_ i = 1; i <= n; ++i) g[i].clear();
    st.clear();
    for(_ i = 1; i <= m; ++i) {
        c[i].first = read()-1;
        c[i].second = read();
        if(!a[c[i].first] && !a[c[i].second]) q.push(i);
        else {
            if(a[c[i].first]) {
                ++d[i];
                g[c[i].first].push_back(i);
            }
            if(a[c[i].second]) {
                ++d[i];
                g[c[i].second].push_back(i);
            }
        }
    }
    for(_ i = 1; i <= n; ++i) if(a[i]) st.insert(i);
    for(; !q.empty(); q.pop()) {
        _ t = q.front();
        _ l = c[t].first, r = c[t].second;
        for(set<_>::iterator it = st.lower_bound(l); it != st.end() && *it <= r; st.erase(it), it = st.lower_bound(l)) {
            a[*it] = 0;
            for(_ j : g[*it]) if(!--d[j]) q.push(j);
        }
    }
    bool ok = true;
    for(_ i = 1; i <= n; ++i) if(a[i]) ok = false;
    if(ok) yes(7);
    else no(7);
