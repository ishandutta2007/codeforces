...
int main(){
  ...
  vector<ll>val;
  rep(i,0,n){
    cin>>a[i].fi.fi>>a[i].fi.se>>b[i].fi>>b[i].se;
    a[i].se=i;
    val.PB(a[i].fi.fi);
    val.PB(a[i].fi.se);
    val.PB(b[i].fi);
  ...
  LazySegmentTree<ll>seg(mp.size(),f,g,h,0,0);
  rep(i,0,n){
    seg.update(mp[b[i].fi],mp[b[i].se]+1,1);
  }
  priority_queue<P,vector<P>,greater<P>>que;
  ll idx=0;
  rep(i,0,n){
    while(idx<n&&a[idx].fi.fi<=a[i].fi.se){
      //cout<<i spa idx spa "ins"<<endl;
      auto t=a[idx].se;
      seg.update(mp[b[t].fi],mp[b[t].se]+1,-1);
      que.emplace(a[idx].fi.se,a[idx].se);
      idx++;
    }
    while(!que.empty()&&que.top().fi<a[i].fi.fi){
      //cout<<i spa que.top().se spa "del"<<endl;
      auto t=que.top().se;
      seg.update(mp[b[t].fi],mp[b[t].se]+1,1);
      que.pop();
    }
    auto t=a[i].se;
    if(seg.query(mp[b[t].fi],mp[b[t].se]+1)>=1)judge=false;
    //for(auto z:mp)cout<<z.fi spa seg.query(z.se,z.se+1)<<endl;
  }
  {
    vector<pair<P,ll>>a2(n);
    vector<P>b2(n);
    rep(i,0,n){
      a2[i]=MP(b[i],i);
      b2[a[i].se]=a[i].fi;
    ...
    auto b=b2;
    sort(ALL(a));
    //rep(i,0,n)cout<<a[i].fi spa a[i].se<<endl;
  LazySegmentTree<ll>seg(mp.size(),f,g,h,0,0);
  rep(i,0,n){
    seg.update(mp[b[i].fi],mp[b[i].se]+1,1);
    //cout<<mp[b[i].fi] spa mp[b[i].se]<<endl;
  }
  priority_queue<P,vector<P>,greater<P>>que;
  ll idx=0;
  //cout<<judge<<endl;
  ...
  rep(i,0,n){
    ...
    auto t=a[i].se;
    if(seg.query(mp[b[t].fi],mp[b[t].se]+1)>=1)judge=false;
    //if(i==0)for(auto z:mp)cout<<z.fi spa seg.query(z.se,z.se+1)<<endl;
  }
  }
  ans2(judge);
  return 0;
