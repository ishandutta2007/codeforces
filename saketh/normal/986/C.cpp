...
int main() {
    ...
    int N, M; re(N, M); vi a(M); re(a);

    vb exists(1 << N);
    trav (e, a) exists[e] = 1;

    vb sub_exists = exists;
    F0R (m, 1 << N) {
        F0R (i, N) if (m & (1 << i)) {
            sub_exists[m] = sub_exists[m] | sub_exists[m ^ (1 << i)];
        }
    }

    vi singletons, _a;
    trav (e, a) {
        if (!sub_exists[(1 << N) - 1 - e]) {
            exists[e] = false;
            singletons.pb(e);
        } else {
            _a.pb(e);
    ...
    trav (e, a) exists[(1 << N) - 1 - e] = 1;

    vb sup_exists = exists;
    F0Rd (m, 1 << N) {
        F0R (i, N) if (!(m & (1 << i))) {
            sup_exists[m] = sup_exists[m] | sup_exists[m ^ (1 << i)];
        }
    }
    ...
    F0R (m, 1 << N) {
        F0R (i, N) if (m & (1 << i)) {
            ...
            if (sup_exists[m] && sub_exists[m ^ (1 << i)])
                uf.unio(m, m ^ (1 << i));
        }
    }
    ...
    vi ccs;
    F0R (m, 1 << N) if (exists[m]) ccs.pb(uf.rep(m));
    dedup(ccs);

    ps(sz(ccs) + sz(singletons));
