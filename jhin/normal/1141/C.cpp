...
for(int i=1;i<n;i++)
{
    b[i]=b[i-1]+a[i-1];mx=max(mx,b[i]);
}
int pls=n-mx;
for(int i=0;i<n;i++)if(b[i]+pls<1||b[i]+pls>n)return cout<<-1,0;else myset.insert(b[i]+pls);
if(myset.size()!=n)cout<<-1;
else
{
   for(int i=0;i<n;i++)cout<<b[i]+pls<<" ";
}
