...
void solve(){
    ...
    int ans = 1e18;
    vector<int> kouho_start;
    REP(i, n) {
        REP(q, m) {
            kouho_start.push_back(grid[i][q] - (i + q));
        }
    }
    REP(i, kouho_start.size()) {
        if (kouho_start[i] > 0) continue;
        REP(q, n) {
            REP(t, m) {
                dp[q][t] = 1e18;
            }
        }
        dp[0][0] = -kouho_start[i];
        REP(q, n) {
            REP(t, m) {
                if (q + 1 < n) {
                    if (grid[q + 1][t] >= (q + 1 + t) + kouho_start[i]) {
                        dp[q + 1][t] = min(dp[q + 1][t], dp[q][t] + grid[q + 1][t] - ((q + 1 + t) + kouho_start[i]));
                    }
                }
                if (t + 1 < m) {
                    if (grid[q][t + 1] >= (q + t + 1) + kouho_start[i]) {
                        dp[q][t + 1] = min(dp[q][t + 1], dp[q][t] + grid[q][t + 1] - ((q + 1 + t) + kouho_start[i]));
                    }
                }
            }
        }
