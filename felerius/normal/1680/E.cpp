// Moving Chips (https://codeforces.com/contest/1680/problem/E)
...
int main() {
    ...
    while (t--) {
        ...
        array<string, 2> s;
        read(s);
        while (n && s[0].back() == '.' && s[1].back() == '.')
            s[0].pop_back(), s[1].pop_back(), --n;
        auto mask = [&](int i) {
            return (int(s[1][i] == '*') << 1) | int(s[0][i] == '*');
        };

        vector dp(n, array{INF, INF, INF, INF});
        dp[0][mask(0)] = 0;
        dp[0][1] = min({dp[0][1], dp[0][2] + 1, dp[0][3] + 1});
        dp[0][2] = min({dp[0][2], dp[0][1] + 1, dp[0][3] + 1});

        rep(i, 1, n) {
            int m = mask(i);
            rep(j, 4)
                dp[i][j | m] = min(dp[i][j | m], dp[i - 1][j] + __builtin_popcount(j));

            dp[i][1] = min({dp[i][1], dp[i][2] + 1, dp[i][3] + 1});
            dp[i][2] = min({dp[i][2], dp[i][1] + 1, dp[i][3] + 1});
        }

        println(min(dp[n - 1][1], dp[n - 1][2]));
    }
}