...
void doit(int l,int r) {
	ans.pb(mkp(l,r)); 
	For(i,0,r-l) if(l+i<r-i) swap(a[l+i],a[r-i]);  
}
void works() {
	n=read(); For(i,1,n) a[i]=read(); For(i,1,n) b[i]=read(); 
	if(a[1]!=b[1]||a[n]!=b[n]) Fail;  
	ans.clear(); 
	For(i,2,n) if(a[i]!=b[i]) {
		...
		For(j,i+1,n) {
			if(a[j-1]==b[i]&&a[j]==b[i-1]) {
				ok=1; doit(i-1,j); 
				break; 
			} else if(a[j-1]==b[i-1]&&a[j]==b[i]) {
				pos=j; 
			}
		}
		if(ok) continue; 
		if(!pos) Fail; 
		For(j,i-1,pos-1) For(k,pos,n) if(a[j]==a[k]&&!ok) {
			ok=1; doit(j,k); 
			break;  
		} 
		if(!ok) Fail;
		For(j,i+1,n) if(a[j-1]==b[i]&&a[j]==b[i-1]){doit(i-1,j);break;}
	}
	printf("YES\n"); 
	printf("%d\n",SZ(ans)); 
	for(auto it:ans) {
