...
int main() {
	...
	while (true) {
		...
		for (int i = 0; i < n; i++) {
			int mask = (p[i].first & 1) * 2 + (p[i].second & 1);
			pts[mask].emplace_back(i);
		}
		...
		if (done) {
			vector<int> ans;
			bool hasBoth = (int)(pts[0].size() + pts[3].size()) > 0 && (int)(pts[1].size() + pts[2].size()) > 0;

			if (hasBoth) {
				ans.insert(ans.end(), pts[0].begin(), pts[0].end());
				ans.insert(ans.end(), pts[3].begin(), pts[3].end());
			}
			else {
				ans.insert(ans.end(), pts[0].begin(), pts[0].end());
				ans.insert(ans.end(), pts[1].begin(), pts[1].end());
			}
