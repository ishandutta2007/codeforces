...
void solve() {
    ...
    auto dfs = [&](auto self, int p, ll x) -> pair<ll, ll> {
        ll sum = s[p] * x;
        ll csz = ll(tr[p].size());

        if (csz == 0) return {sum, s[p]};
        ll y = x / csz;
        V<ll> inc;
        for (int d : tr[p]) {
            auto res = self(self, d, y);
            sum += res.first;
            inc.push_back(res.second);
        }

        sort(inc.begin(), inc.end(), greater<ll>());
        for (int i = 0; i < int(x % csz); i++) {
            sum += inc[i];
        }
        return {sum, s[p] + inc[x % csz]};
    };

    auto ans = dfs(dfs, 0, k);

    dbg(ans);
    pr.writeln(ans.first);
}
