...
//
// https://tio.run/##Pc/BaoNAEAbg@zzF4CEkuCbZVU/JimBpTy2F5LYswTTSKEYh2ZSA@Ox2xmBO88HO/w97yd25uOSu/MmHodFyDfAw5UFgdbCrDX61zsitLre6wdkMiRXRalqDt9YAfF/LxqX7ducIv@nHtTylc9znx7own@3JUBl1WYHKio7Y9KKreFjE1RLn1LNr67/CwHudO1c06TNLuUBy0if5L9EM5CRfWq3piXvlq3qUBYF0/l7fb0GiLMACgc51mZE2SDLayYxiKVbIClkRK2LFrLh/xmSQeA9P4Jrm0utxAbAZf27sNAFEx72qZ6gJ4YRoQjzCDsM/
...
void up(){
	...
	std::vector<std::vector<int>> a(number);
	for(auto& row: a){
		row.resize(number);
	...
	int width=number, pos=0, result=0;
	while(width>0){

		int row=pos, col=pos, stepCount=0;
		auto const step=[&]{
			if((stepCount&2)==0){
				result^=a[row][col];
			}
