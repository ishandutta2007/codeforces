...
int main(){
  ...
  bool judge = true;
  ll k,n,m;cin>>k>>n>>m;
  vector<ll>l1(n),r1(n),l0(m),r0(m);
  rep(i,0,n)cin>>l1[i]>>r1[i],l1[i]--;
  rep(i,0,m)cin>>l0[i]>>r0[i],l0[i]--;
  Compress<ll>mp;
  mp.add(l1,r1,l0,r0);
  mp.add(0LL);
  mp.add(1LL);
  ...
  mp.build();
  ll sz=mp.size();
  vector<vector<ll>>g1(sz),g0(sz);
  rep(i,0,n)g1[mp[r1[i]]].PB(mp[l1[i]]);
  rep(i,0,m)g0[mp[r0[i]]].PB(mp[l0[i]]);
  auto seg0=affine_sum::make(sz);
  auto seg1=affine_sum::make(sz);
  seg0.update(0,1);
  seg1.update(0,1);
  rep(i,1,sz){
    //seg0.print();
    //seg1.print();
    for(auto l:g0[i])seg1.update(0,l+1,0);
    for(auto l:g1[i])seg0.update(0,l+1,0);
    modint pre0=seg0.query(0,i+1);
    modint pre1=seg1.query(0,i+1);
    if(i!=sz-1){
      ll len=mp.val(i+1)-mp.val(i);
      modint cfall=pow(2LL,len-1)-1;
      modint sum=cfall*(pre0+pre1);
      //cout<<cfall spa pre0 spa pre1 spa sum<<endl;
      seg0.update(i+1,sum);
      seg1.update(i+1,sum);
      seg0.update(i,pre1+seg0[i]);
      seg1.update(i,pre0+seg1[i]);
    }
  }
  cout<<seg0.query(0,sz)+seg1.query(0,sz)<<endl;
  return 0;
}