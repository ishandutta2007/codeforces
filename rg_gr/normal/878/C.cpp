...
int n=read(),k=read();
struct qwq{
	int maxn[15]={},minn[15]={},sz;
	bool operator<(const qwq& b)const{
		int f=0;
		rg(k)if(maxn[i]>=b.minn[i])return 0;gr
		return 1;
	}
	void merge(qwq b)
	{
		rg(k)maxn[i]=max(maxn[i],b.maxn[i]);gr
		rg(k)minn[i]=min(minn[i],b.minn[i]);gr
		sz+=b.sz;
	}
};
std::set<qwq> s;
...
{
	rg(n)
	qwq x;rg(k)x.maxn[i]=x.minn[i]=read();gr
	x.sz=1;
	auto t=s.lower_bound(x);
	while(t!=s.end())
	{
		if(x<*t==0&&*t<x==0);else break;
		auto T=t;++T;
		x.merge(*t);
		s.erase(t);
		t=T;
