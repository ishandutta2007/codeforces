...
int main() {
    ...
    while (t --> 0) {
        ...
        rep(i, m) {
            ...
            read(x);
            if (sum > 0) {
                ll loop_num = x <= max_sum ? 0 : (x - max_sum + sum - 1) / sum;
                int lb = lower_bound(all(s), x - sum * loop_num) - s.begin();
                ans[i] = loop_num * n + seg.prod(lb, n);
            } else {
                int lb = lower_bound(all(s), x) - s.begin();
                if (lb == n) {
                    ans[i] = -1;
                } else {
                    ans[i] = seg.prod(lb, n);
