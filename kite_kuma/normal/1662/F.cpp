...
int solve() {
	...
	const auto er = make_pair(inf, -1);
	segment_tree<pair<int, int>> goright(
		n, [](pair<int, int> a, pair<int, int> b) { return min(a, b); }, er);
	const auto el = make_pair(-inf, -1);
	segment_tree<pair<int, int>> goleft(
		n, [](pair<int, int> a, pair<int, int> b) { return max(a, b); }, el);

	VEC(int, p, n);
	rep(i, n) {
		if(i != a) {
			goleft.update(i, make_pair(p[i] + i, i));
			goright.update(i, make_pair(-p[i] + i, i));
		}
	}
	queue<int> q;
	q.push(a);
	...
	auto remove = [&](int idx) {
		goright.update(idx, er);
		goleft.update(idx, el);
		return;
	};

	auto update = [&](int from, int to) -> void {
	...
	while(q.size()) {
		...
		const int reach_r = min(n, now + p[now] + 1);
		const int reach_l = max(0, now - p[now]);
		while(1) {
			pair<int, int> nxt = goright.get(now, min(n, reach_r));
			debug(now, reach_r);
			debug(nxt);
			if(nxt.first > now) break;
			update(now, nxt.second);
		}

		while(1) {
			pair<int, int> nxt = goleft.get(reach_l, now + 1);
			debug(nxt);
			if(nxt.first < now) break;
			update(now, nxt.second);
		}
