...
int main(){
	...
	while(T--){
		...
		int mi[n][m];
		ll ans = 0;
		rep(i, n) rep(j, m) mi[i][j] = min(l[i][j], r[i][j]) - i;
		rep(j,m){
			SegmentTree<ll> seg(n + 1, [](ll a, ll b) { return min(a, b); }, 1e9);
			rep(i, n) seg.set(i, mi[i][j]);
			seg.set(n, -1e9);
			seg.build();
			rep(i, n) ans += seg.find_first(i, [&](ll x) { return x + i <= 0; }) - i;
		}
		cout << ans << endl;
	}
}