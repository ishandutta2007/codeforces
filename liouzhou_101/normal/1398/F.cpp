...
void solve()
{
	...
	for (int i = n - 1; i >= 1; --i)
	{
		if (s[i] == '0')
		{
			...
			else if (s[i + 1] == '1') f[i] = 1;
			else
			{
				f[i] = 1 + to[i + 1];
				if (i + to[i + 1] + 1 <= n && s[i + to[i + 1] + 1] == '0') f[i] = 1 + f[i + 1];
			}
		}
		else if (s[i] == '1')
		{
			if (s[i + 1] == '1') f[i] = f[i + 1] + 1;
			else if (s[i + 1] == '0') f[i] = 1;
			else
			{
				f[i] = 1 + to[i + 1];
				if (i + to[i + 1] + 1 <= n && s[i + to[i + 1] + 1] == '1') f[i] = 1 + f[i + 1];
			}
		}
		else
		{
	...
	for (int x = 1; x <= n; ++x)
	{
		...
		for (auto y : v[x]) F[father(y)] = father(y + 1);
		write(res, x == n ? '\n' : ' ');
	}
}
