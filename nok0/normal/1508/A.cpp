...
void main_() {
	INT(n);
	VEC(string, s, 3);
	sort(all(s), [](string x, string y) { return count(all(x), '0') > count(all(y), '0'); });
	if(count(all(s[1]), '0') >= n) {
		V<> a(2 * n + 1), b(2 * n + 1);
		{
			int cnt = 0;
			REP(i, 2 * n) {
		...
		REP(i, 2 * n + 1) { chmax(a[i], b[i]); }
		string res = "";
		REP(i, 2 * n + 1) {
			REP(j, a[i]) { res.push_back('1'); }
			res.push_back('0');
		}
		...
		string res = "";
		REP(i, 2 * n + 1) {
			REP(j, a[i]) { res.push_back('0'); }
			res.push_back('1');
		}
		while(res.size() < 3 * n) res.push_back('1');
		res.resize(3 * n);
		print(res);
	}
}
