    ...
    while(t--){
        ...
        rep(i,T){
            ...
            while(1){
                int ma=0;
                rep(j,T){
                    chmax(ma,cnt[r][j]-cnt[l+1][j]);
                }
                if(l==r) chmax(ans,x*2-1);
                else chmax(ans,ma+x*2);
                if(x>=(int)pl[i].size()-1)break;
                l=pl[i][x]; r=pl[i][pl[i].size()-1-x];
                if(l>r)break;
                x++;
            }
        }
