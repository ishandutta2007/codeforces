...
void solve() {
    ...
    trav (i, a)
        count[i]++;
    if (*max_element(all(count)) > (n + 1) / 2) {
        ps(-1);
        return;
    }
    ...
    for (int i = 0, start = 0; i <= n; i++) {
        if (i == n || (i > start && a[i - 1] == a[i])) {
            parts.push_back(vi(0));
            for (int j = start; j < i; j++) {
                parts.back().push_back(a[j]);
            }
            start = i;
        }
    }
    ...
    int maxDeficiency = 0;
    for (int i = 0; i < n; i++) {
        int diff = sz(parts) - half[i] - bad[i];
        int same = bad[i];
        if (same <= diff + 1) {
            continue;
        }
        int deficiency = same - (diff + 1);
        ckmax(maxDeficiency, deficiency);

        // int availSplitDiff = 0;
        // trav (p, parts) {
        //     if (p.front() != i && p.back() != i) {
        ...
        //     }
        // }
        // pd(availSplitDiff, availSplitSame);
    }
    pd(sz(parts));

    int ans = sz(parts) - 1 + maxDeficiency;
    ps(ans);
}
