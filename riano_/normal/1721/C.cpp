...
int main(){
    ...
    rep(ii,T){
        ...
        vector<Pr> p; ll aa[N];
        rep(i,N){
            ll a; cin >> a; p.emplace_back(a,(-1)*i);
            aa[i] = a;
        }
        vector<ll> bb(N);
        rep(i,N){
            ll b; cin >> b; p.emplace_back(b,1); bb[i] = b;
        }
        ...
        rep(i,2*N){
            ...
            if(t==1) d.emplace_back(s);
                ...
                if(m.count(s)){
                    ...
                    mn[j] = d.back()-aa[j]; mx[j] = d.front()-aa[j];
                    m[s] = make_pair(mx[j],mn[j]);
                }
                d.pop_back();
            }
        }
        sort(all(bb));
        rep(i,N){
            ll k = lower_bound(all(bb),aa[i])-bb.begin(); mn[i] = bb[k]-aa[i];
        }
        out(mn);
        out(mx);
    }
