...
fun main(){
    solve.cases{
		val n = getint
		val arr = Array(n){IntArray(n){getint} }

		val DP = Array(n){LongArray(n){phuge} }
		val best = Array(n){IntArray(n)}

		val outsidesum = Array(n){LongArray(n)}
		val sums = Array(n){arr[it].torsq()}
		for(l in 0 until n){
			for(r in l until n){
				for(v in 0 until n){
					if(v !in l..r){
						outsidesum[l][r] += sums[v].sumQuery(l,r)
					}
				}
			}
		}
		...
		for(len in 1..n){
			for(l in 0 until n){
                ...
                var bestid = -1
				for(mid in l..r){
					val leftcost = if(l > mid-1) 0 else (DP[l][mid-1] + outsidesum[l][mid-1])
					val rightcost = if(mid + 1 > r) 0 else (DP[mid+1][r] + outsidesum[mid+1][r])
                    val total = leftcost + rightcost
                    if(total < mincost){
                        mincost = total
                        bestid = mid
                    }
//					DP[l][r] = minOf(DP[l][r], leftcost + rightcost)
				}
                DP[l][r] = mincost
                best[l][r] = bestid
			}
		}
        val ret = IntArray(n){nlarge}
        fun giveme(l:Int, r:Int,p:Int){
            if(l > r){ return
            }else if(l == r){
                ret[l] = p +1
                return
