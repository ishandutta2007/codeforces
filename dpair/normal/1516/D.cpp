...
int cnt[MAXN];
int nxt[MAXN][21], lst[MAXN];


inline void add(int x, int pos){
    for (int i = 2;i * i <= x;++ i) {
        if(x % i == 0){
            chmin(nxt[pos][0], lst[i]);
            lst[i] = pos;
        }
        while(x % i == 0) x /= i;
    }
    if(x > 1) {
        chmin(nxt[pos][0], lst[x]);
        lst[x] = pos;
    }
}
...
signed main(){
    ...
    rep(j, 1, n + 1) rep(i, 0, 20) nxt[j][i] = n + 1;
    rep(i, 1, 100000) lst[i] = n + 1;
    per(i, n, 1) add(a[i], i), chmin(nxt[i][0], nxt[i + 1][0]);
    rep(j, 1, 20) rep(i, 1, n) nxt[i][j] = nxt[nxt[i][j - 1]][j - 1];
    while(m --){
        int l, r, ans = 0;read(l, r);
        per(i, 20, 0) if(nxt[l][i] <= r) ans |= (1 << i), l = nxt[l][i];
        print(ans + 1);
    }
}