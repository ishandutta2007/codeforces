...
int main(){
  ...
  rep(i,0,4)rep(j,0,4){
    if(i==j)continue;
    //cout<<v[i] spa v[j] spa distance(v[i],v[j])<<endl;
    //cout<<mandist(p1,v[i]) spa mandist(v[j],p2)<<endl;
    chmin(ret,mandist(p1,v[i])+distance(v[i],v[j])+mandist(v[j],p2));
  }
  lfs<<ret<<endl;
  return 0;
}