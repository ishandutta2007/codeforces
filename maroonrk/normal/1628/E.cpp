...
struct N{
	int lz;
	pi mx,rwmx;
	N(pi v=none):lz(0),mx(none),rwmx(v){}
	void activate(){
		lz=1;
		mx=rwmx;
	}
	...
	static N merge(const N&x,const N&y){
		N res(max(x.rwmx,y.rwmx));
		res.mx=max(x.mx,y.mx);
		return res;
	}
};

//dep,par  not verified
//yukicoder no.1024
template<class E>
struct treedfs{
	const vvc<E>&g;
	...
	treedfs(const vvc<E>&gg,int r):g(gg),n(si(g)),cnt(0),par(n),dep(n),in(n),out(n){
		dfs(r,-1,0);
	}
};
...
void slv(){
	...
	int root=uf.find(0);
	treedfs<int> z(t,root);
	vc<pi> rwmx(n),rwmn(n);
	rep(i,n){
		rwmx[i]=pi(z.in[i],i);
		rwmn[i]=pi(-z.in[i],i);
	}
	seglazy<N> maxseg(rwmx);
	seglazy<N> minseg(rwmn);
	vi vis(n,-2);
	rep(_,q){
		int kind;cin>>kind;
		if(kind==1){
			int l,r;cin>>l>>r;
			l--;
			maxseg.ch(l,r,&N::activate);
			minseg.ch(l,r,&N::activate);
		}else if(kind==2){
			int l,r;cin>>l>>r;
			l--;
			maxseg.ch(l,r,&N::inactivate);
			minseg.ch(l,r,&N::inactivate);
		}else if(kind==3){
			int x;cin>>x;x--;
			for(int pre=-1,y=x;y!=-1;y=par[y]){
				vis[y]=pre;
				pre=wei[y];
			}
			int ans=-1;
			auto upd=[&](int v){
				...
				chmax(ans,max(pre,vis[y]));
			};
			{
				int v=maxseg.getall().mx.b;
				if(v!=-1)upd(v);
			}
			{
				int v=minseg.getall().mx.b;
				if(v!=-1)upd(v);
			}
			print(ans);
