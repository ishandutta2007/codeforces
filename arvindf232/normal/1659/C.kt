...
fun main(){
    solve.cases{
        ...
        for(con in 0 until n){
            val root = if(con == 0) 0 else L[con-1]
            val concost = root.toLong() * (a+b)
            val others = LQ.sumQuery(con,n-1) - (n-1-con+1).toLong() * root
            min = minOf(min, concost + others * b)
        }
