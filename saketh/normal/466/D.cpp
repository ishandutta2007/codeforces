...
int main() {
    ...
    F0R (i, N) {
        ...
        if (a[i] > H) { ps(0); return 0; }
        int need = H - a[i];

        // do nothing here
        DP[need] = dp[need];

        // open an interval here
        if (need) DP[need] += dp[need-1];

        // close an interval here
        if (need) DP[need-1] += need * dp[need];

        // close and open here
        if (need) DP[need-1] += (need - 1) * dp[need-1];

        // add a singleton here
        if (need) DP[need-1] += dp[need-1];
