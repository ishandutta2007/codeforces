...
void foldL(ll p)
{
	rep(i, 1, p){
		ll j = 2*p+1-i;
		ll x = seg.query(L+i, L+i), y = seg.query(L+j, L+j);
		seg.update(L+j, x+y);
	}
	L += p;
}
void foldR(ll p)
{
	rep(i, 1, p){
		ll j = 2*p+1-i;
		ll x = seg.query(R+1-i, R+1-i), y = seg.query(R+1-j, R+1-j);
		seg.update(R+1-j, x+y);
	}
	R -= p;
}
...
int main(void)
{
	...
	rep(i, 1, Q){
		...
		if(t == 1){
			...
			if(!rev){
				if(L+2*p <= R) foldL(p);
				else foldR(R-L-p), rev = !rev;
			}
			else{
				if(L+2*p <= R) foldR(p);
				else foldL(R-L-p), rev = !rev;
			}
		}
		else{
			cin >> l >> r;
			if(!rev) cout << seg.query(L+l+1, L+r) << "\n";
			else cout << seg.query(R-r+1, R-l) << "\n";
		}
