...
inline void solve(ll len){
	if(len<block){
		rep(lp,1,len){
			tmp = {	(x.back()[0]+x.back()[1])%MOD,
					(x.back()[0]+x.back()[1]+x.back()[2])%MOD,
					(x.back()[1]+x.back()[2])%MOD};
			rep(j,0,2)if(mask&(1<<j))tmp[j] = 0;
			x.pb(tmp);
		}
		x = {x.back()};
	}
		...
		tmp = {0,0,0};
		rep(j,0,2)if((mask&(1<<j))==0){
			z.clear();
			rep(lp,1,block)z.pb(x[lp][j]);
			tmp[j] = work(z, len-1);
		}
		x = {tmp};
	}
