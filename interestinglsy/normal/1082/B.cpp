...
int main(){
	...
	int duan = 0;
	int mxduan = 0, totlen = 0;
	int r = 1;
	while( !a[r] && r <= n ) r++;
	if( r > n ){ printf("0\n"); END(); }
	for( int l = r ; l <= n ; l = r ){
		while( r <= n && a[r] ) r++;
		Forx(i,l,r-1) len[i] = r-l;
		duan++;
		Mymax(mxduan,r-l);
		totlen += r-l;
		duans.Pb({l,r-1});
	...
	if( duan == 1 ){ printf("%d\n",mxduan); END(); }
	if( duan == 2 ){
		if( duans[0].r+1 == duans[1].l-1 ) printf("%d\n",totlen);
		else printf("%d\n",mxduan+1);
		END();
	}
	int ans = mxduan+1;
	For0(i,duans.size()-1){
		if( duans[i].r+1 == duans[i+1].l-1 ) Mymax(ans,duans[i].r-duans[i].l+1+duans[i+1].r-duans[i+1].l+1+1);
	}
	printf("%d\n",ans);
