...
fn run() {
    ...
    let mut prod = a.iter().map(|a| M::from(*a)).collect::<Vec<_>>();
    prod.push(M::one());
    for i in (0..n).rev() {
        prod[i] = prod[i + 1] * prod[i];
    }
    let mut query = vec![vec![]; n];
    for (i, &(l, r)) in ask.iter().enumerate() {
        query[l].push((r, i));
    }
    let pc = precalc::Precalc::new(m);
    ...
    for (l, a) in a.iter().enumerate().rev() {
        ...
        while x > 1 {
            ...
            if last[p] < n {
                let y = last[p];
                let v = seg.find(y, y + 1);
                seg.update(y, v * M::from(p) * pc.inv(p - 1));
            }
            last[p] = l;
            val *= M::from(p - 1) * pc.inv(p);
        }
        seg.update(l, val);
        for &(r, k) in query[l].iter() {
            ans[k] = prod[l] * prod[r].inv() * seg.find(l, r);
        }
    }
