...
bool can_solve(int cnt, bool output = false)
{
	if (output)
		printf("%d\n%d\n", cnt, cnt);
	ll best = 0;
	...
	for (int i = 0; i < n && left > 0; ++i)
		if (probs[i].first >= cnt)
		{
			...
			--left;
			if (output)
				printf("%d ", probs[i].id + 1);
		}
	return best <= tot && left == 0;
}
...
void solve()
{
	...
	for (int i = 0; i < n; ++i)
	{
		scanf("%d%d", &probs[i].first, &probs[i].second);
		probs[i].id = i;
	}
	sort(probs, probs + n, [](Prob a, Prob b) {return a.second < b.second; });
	int l = 0;
	int r = n+1;
	while (r - l > 1)
	{
