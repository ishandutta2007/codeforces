...
void solve(){
	...
	vi R(r), G(g), B(b); read(R, G, B);
	vt<vvi> dp(225, vvi(225, vi(225, 0)));
	sort(all(R));
	sort(all(G));
	sort(all(B));
	dp[r][g][b] = 1;
	...
	for(int i = n-2; i>= 0; i-=2){
		for(int j = max(0, i-400); j<=min(i, 200); ++j){
			for(int k = max(0, i-200-j); k <= min(200,i-j); ++k){
				int ans = 0;
				if(dp[j+1][k+1][i-j-k])ans = max(ans, dp[j+1][k+1][i-j-k]+R[j]*G[k]);
				if(dp[j+1][k][i-j-k+1])ans = max(ans, dp[j+1][k][i-j-k+1]+R[j]*B[i-j-k]);
				if(dp[j][k+1][i-j-k+1])ans = max(ans, dp[j][k+1][i-j-k+1]+G[k]*B[i-j-k]);
				dp[j][k][i-j-k] = ans;
				answer = max(answer, dp[j][k][i-j-k]);
			}
		}
	}
	print(answer-1);
