...
public class E {
	...
	void solve()
	{
		...
		while(high - low > 2){
			...
			int r = low + (high-low)/3*2;
			long costl = f(l, a, A, R, M);
			long costr = f(r, a, A, R, M);
			if(costl < costr){
				high = r;
			}else{
				low = l;
		...
		for(int i = low;i <= high;i++){
			ans = Math.min(ans, f(i, a, A, R, M));
		}
		out.println(ans);
	}
	
	long f(int h, int[] a, long A, long R, long M)
	{
		long na = 0, nr = 0;
		for(int v : a){
			if(v < h){
