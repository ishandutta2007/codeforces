...
void solve()
{
	...
	for (int x = 1; x <= n; ++x)
	{
		for (auto y : v[x])
		{
			if (u[x] && u[y] && (x != fa[y] && y != fa[x]))
			{
				...
				for (auto it = vy.rbegin(); it != vy.rend(); ++it)
					if (*it != x) vx.push_back(*it);
				writeln(2);
				writeln<int>(vx.size());
				print(vx);
				return;
	...
	vector<int> res;
	for (int i = 1; i <= n; ++i)
		if (u[i] && c[i]) res.push_back(i);
	writeln(1);
	print(res.begin(), res.begin() + (k + 1) / 2);
}
