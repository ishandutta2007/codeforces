...
int main() {
    ...
    int N; re(N); vpii v(N); re(v);

    mn s1 = 1;
    sort(all(v));
    for (int i = 0; i < sz(v); ) {
        int j = i; while (j < sz(v) && v[j].f == v[i].f) j++;
        s1 *= mn::fact(j - i);
        i = j;
    }
    ...
    sort_by(v, a.s < b.s);
    for (int i = 0; i < sz(v); ) {
        int j = i; while (j < sz(v) && v[j].s == v[i].s) j++;
        s2 *= mn::fact(j - i);
        i = j;
    }
    ...
    for (int i = 0; i < sz(v); ) {
        int j = i; while (j < sz(v) && v[j] == v[i]) j++;
        s12 *= mn::fact(j - i);
        i = j;
    }

    bool both = true;
    FOR (i, 1, sz(v)) both &= v[i].f >= v[i-1].f && v[i].s >= v[i-1].s;
    if (!both) s12 = 0;

    ps(mn::fact(N) - s1 - s2 + s12);
