...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	cauto n=read();
	auto mp=table(n,n,0ll);
	rep(i,n)rep(j,n)if(i+j && i+j<2*n-2){
		if(i%2) mp[i][j]=1ll<<(i+j-1);
	}
	rep(i,n)rep(j,n) cout<<mp[i][j]<<" \n"[j+1==n];
	rep(i,n)rep(j,n) assert(mp[i][j]<=10000000000000000);
	cout.flush();
	auto q=read();
	while(q--){
		cauto d=read();
		...
		reps(i,1,re.size()){
			auto [cx,cy]=re[i-1];
			if(cx+1==n || (mp[cx+1][cy]^d)>>(cx+cy)&1){
				re[i]={cx,cy+1};
			}else{
				re[i]={cx+1,cy};
			}
		...
		out(re,1);
		for(auto [a,b]:re) cout<<a+1<<" "<<b+1<<endl;
		cout.flush();
	}
  return 0;
}