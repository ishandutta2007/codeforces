...
int main() {
	...
	for(int i = 0; i < t; i++) {
		...
		ll both_cost = INF;
		// bool same = true;
		auto totalcost = [&]() { return min(both_cost, larger_cost + smaller_cost); };
		rep(n) {
			LL(l, r, c);
			r++;
			// if(chmin(lef, l)) {
			...
			// 	lef = l;
			// 	smaller_cost = c;
			// } else if(l == lef && c < smaller_cost) {
			// 	smaller_cost = c;
			// }
			// ans.push_back(same ? smaller_cost : (smaller_cost + larger_cost));

			if(l == lef) {
				chmin(smaller_cost, c);
			} else if(l < lef) {
				smaller_cost = c;
				lef = l;
				both_cost = INF;
			...
			if(l == lef && rig == r) chmin(both_cost, c);
