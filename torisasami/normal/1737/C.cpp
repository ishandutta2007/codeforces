...
int main() {
    ...
    while (T--) {
        ...
        int x, y;
        cin >> x >> y;
        int cr = ((r[0] == r[1]) || (r[0] == r[2]) ? r[0] : r[1]);
        int cc = ((c[0] == c[1]) || (c[0] == c[2]) ? c[0] : c[1]);
        if (min(abs(cr - 1), abs(cr - n)) == 0 && min(abs(cc - 1), abs(cc - n)) == 0)
            cout << (x == cr || y == cc ? "YES" : "NO") << '\n';
        else
            cout << (((cr + cc) % 2 == (x + y) % 2) && (cr % 2 != x % 2) ? "NO" : "YES") << '\n';
    }
}