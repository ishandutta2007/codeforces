...
signed main(){
	...
	vvc<pi> table(n,vc<pi>(n,pi(-1,-1)));
	rep(i,n)rep(j,i){
		int k=i*(i-1)/2+j+1;
		mcf.ae(0,k,1,0);
		table[i][j]=mcf.ae(k,1+es+i,dec[i][j]==-1||dec[i][j]==0,0);
		table[j][i]=mcf.ae(k,1+es+j,dec[i][j]==-1||dec[i][j]==1,0);
	}
	
	rep(i,n){
		rep(j,n){
			mcf.ae(1+es+i,s-1,1,j*2);
		}
	}
	
	int f=mcf.calc(0,s-1,es);
	rep(i,n)rep(j,n)if(i==j)
		dec[i][j]=0;
	else if(dec[i][j]==-1){
		dec[i][j]=mcf.g[table[i][j].a][table[i][j].b].cap;
	}
	rep(i,n){
		string str;
		rep(j,n)str+='0'+dec[i][j];
		cout<<str<<endl;
	}
}