...
int main(){
    ...
    mint cnt_all = comb(s-r+p-1, p-1);
    mint ans = 0;
    vector<vector<mint>> cnt(p, vector<mint>(s+1));
    cnt[0][0] = 1;
    for(int i = 0; i < p-1; i++){
        auto cumsum = Cumsum<mint>(cnt[i]);
        for(int j = 0; j <= s; j++){
            cnt[i+1][j] = cumsum.sum(j-(r-1), j+1);
        }
    }
    for(int i = 1; i*r <= s && i <= p; i++){
        ...
        mint cb = comb(p-1, j);
        mint tmp_cnt = 0;
        for(int sum = 0; sum <= s_rem; sum++){
            int x = s_rem-sum;
            // xi
            tmp_cnt += cb*cnt[j][sum]*comb(x+i-1, i-1);
        }
        ans += tmp_cnt*cnt_all.inv()*inv[i];
    }
    cout << ans << endl;
}