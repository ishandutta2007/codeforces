...
vector<bool> isprime(const int N){
...
ll N,memo[401][201][201],bunshi[500],invlis[500];
bool vis[401][201][201];
ll calc(ll used,ll loopsize,ll K){
  auto& ans=memo[used][loopsize][K];
  if(vis[used][loopsize][K])return ans;
  vis[used][loopsize][K]=true;
  if(loopsize==0)return ans=K==0;
  ll x=1;
  for(ll T=0;K>=loopsize*T&&N-used>=(loopsize+1)*T;++T){
    ans+=calc(used+(loopsize+1)*T,loopsize-1,K-loopsize*T)*bunshi[used+T*(loopsize+1)]%mod*x%mod*modncrlistm[T]%mod;
    debug(used,loopsize,K,T,ans);
    (x*=invlis[loopsize+1])%=mod;
  }
  ans%=mod;
  (ans*=modinv(bunshi[used]))%=mod;
  return ans;
}
int main(){
  startupcpp();
  ...
  bunshi[0]=1;
  rep(i,450)bunshi[i+1]=bunshi[i]*(N-i)%mod;
  rep1(i,450)invlis[i]=modinv(i);
  ll cnt[2]={1,0};
  vec ans;
  rep1(i,K+1){
    (cnt[i&1]+=calc(0,i,i))%=mod;
