...
int main(){
    ...
    vector<ll> max_p(M+1, -inf);
    for(int i = 0; i < n; i++) {
        cin >> c[i] >> d[i] >> h[i];
        chmax(max_p[c[i]], h[i]*d[i]);
    }
    for(int x = 1; x <= M; x++){
        if(max_p[x] == -inf) continue;
        for(int j = 2; x*j <= M; j++){
            chmax(max_p[x*j], max_p[x]*j);
        }
    }
    int m; cin >> m;
    vector<ll> D(m), H(m);
    ...
    for(int i = 1; i <= M; i++){
        if(max_p[i] == -inf) continue;
        int idx = cp[max_p[i]];
        if(seg.get(idx) > i) seg.set(idx, i);
    }
    for(int i = 0; i < m; i++){
        int idx = cp[H[i]*D[i]];
        ll ans = seg.prod(idx+1, N);
        if(ans > C) cout << -1 << ' ';
        else cout << ans << ' ';
    }
    cout << endl;
