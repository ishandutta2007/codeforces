...
void solve() {
	...
	int cnter = r - l;
	edges.push_back(make_tuple(1, 32, 0));
	int itr = 2;
	while ((1 << (itr - 2)) <= cnter) {
		for (int q = 1; q < itr; ++q) {
			edges.push_back(make_tuple(q, itr, (1 << (itr - 2))));
		}
		cnter -= (1 << (itr - 2));
		edges.push_back(make_tuple(itr, 32, 0));
		itr++;
	}
	int now_target = (1 << (itr-2));
	edges.push_back(make_tuple(itr, 32, 0));
	for (int q = 0; q < 30; ++q) {
		if ((1 << q) & cnter) {
			cnter -= (1 << q);
			edges.push_back(make_tuple(q + 2, itr, now_target - (1 << q)));
			now_target += (1 << q);
		}
	}
	cout << "32 ";
	...
	REP(i, edges.size()) {
		if (get<1>(edges[i]) == 32) get<2>(edges[i]) += l;
