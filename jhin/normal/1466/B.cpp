#include <bits/stdc++.h>
    ...
    if(x>1)p.push_back(x),cnt++,vv.push_back(1);
       ...
       while(t--)
       {
           cin>>n;map<int,int>mp;set<int>numer;
            for(int i=0;i<n;i++)
            {
                cin>>a[i];mp[a[i]]++;
            }
            ...
            {
                int tm=it->first,tm2=it->second;
                auto it2=numer.lower_bound(tm);
                if(tm2==1)
                {
                if(it2==numer.end())numer.insert(tm);
                else numer.insert(tm+1);
                }
                else
                {
                    numer.insert(tm);numer.insert(tm+1);
                }

            }
            cout<<numer.size()<<endl;
