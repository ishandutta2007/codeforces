...
int main() {
  ...
  for (ri tt = read(), x, y; tt; --tt) {
    cl();
    double pre = Inf, cur, res;
    int chs = 0;
    int h = read(), c = read(), t = read();
    res = pre = Abs(h - t), chs = 1;
    cur = Abs((c + h) / 2.0 - t);
    if (cur < pre) res = cur, chs = 2;
    if (t * 2 > c + h) {
      int x = (h - t) / (t * 2 - c - h);
      for (ri i = max(1, x - 5); i <= max(1, x + 5); ++i) {
        int a = i + 1, b = i;
        cur = Abs((double) ((ll) a * h + (ll) b * c) / (b + a) - t);
        if (cur > pre) break;
        pre = cur;
        if (cur < res) res = cur, chs = a + b;
      }
    }
    cout << chs << '\n';
  }
