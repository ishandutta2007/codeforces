...
int main() {
	...
	vector<int> v(n); for (auto& i : v) R(i);
	...
	sort(all(query), [=](pair<pii,int> a, pair<pii,int> b) {
		pii x = a.F;
		pii y = b.F;
		
		int xb = x.F / blocksize;
		int yb = y.F / blocksize;
		
		if (xb != yb) return xb < yb;
		if (xb % 2) return x.S > y.S; else return x.S < y.S;
	});
