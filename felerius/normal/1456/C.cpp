// New Game Plus! (https://codeforces.com/contest/1456/problem/C)
...
int main() {
    ...
    ll bonus = 0, bonus_sum = 0;
    while (sz(c) && bonus >= 0)
        bonus_sum += bonus, bonus += c.back(), c.pop_back();

    int x = min(sz(c), k + 1);
    if (x) {
        int smallest = x == k + 1 ? sz(c) / x : 0;
        bonus_sum += bonus * smallest;
        rep(i, sz(c))
            bonus_sum += (i / x) * c[i];
    }
