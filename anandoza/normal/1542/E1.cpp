...
int main() {
    ...
    vvmn OLD;
    {
        vvmn dp(n + 1, vmn(2 * MAX + 1));
        auto get = [&](int i, int j) -> mn & { return dp[i][j + MAX]; };
        get(0, 0) += 1;

        vvmn pascal(n + 1, vmn(n + 1));
        for (int i = 0; i <= n; i++) {
            ...
            for (int a = 0; a < k; a++) {
                for (int b = 0; b < k; b++) {
                    ...
                    for (int inv = -MAX; inv <= MAX; inv++) {
                        if (inv + delta < -MAX || inv + delta > MAX)
                            continue;
                        get(k, inv + delta) += get(k - 1, inv);
                    }
                }
            }
        }
    ...
    vvmn dp(n + 1, vmn(2 * MAX + 1));
    auto get = [&](int i, int j) -> mn & { return dp[i][j + MAX]; };
    auto get_OLD = [&](int i, int j) -> mn & { return OLD[i][j + MAX]; };
    get(0, 0) += 1;
            ...
            for (int b = a; b < k; b++) {
                ...
                if (delta == 0) {
                    ...
                    for (int inv = -MAX; inv <= MAX; inv++) {
                        if (inv + delta < -MAX || inv + delta > MAX)
                            continue;
                        get(k, inv + delta) += get_OLD(k - 1, inv);
                    }
                }
            }
        }
