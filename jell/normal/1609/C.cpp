#line 1 "codeforces-workspace\\contest\\1609\\c\\c.cpp"
...
#line 13 "codeforces-workspace\\contest\\1609\\c\\c.cpp"
...
#line 29 "codeforces-workspace\\contest\\1609\\c\\c.cpp"

namespace workspace {

least_factor<1 << 20> lpf;

void main() {
  // start here!

  input n, e;
  vector<vector<i32>> a(e);
  for (auto i : range(n)) {
    input x;
  ...
  i64 ans = 0;
  for (auto &&v : a) {
    for (auto i = 0, j = 0, last = -1; i < v.size(); ++i) {
      if (i > j) j = i;
      while (j < v.size()) {
        if (v[j] == 1)
          ++j;
      ...
      if (last >= i) ans += max(0, j - max(i + 1, last));
    }
  }
  print(ans);
}
