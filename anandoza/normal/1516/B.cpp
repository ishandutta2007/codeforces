...
void solve() {
    ...
    vi prefix{0};
    partial_sum(all(a), back_inserter(prefix), std::bit_xor<int>());

    auto get = [&](int l, int r) { return prefix[r] ^ prefix[l]; };

    for (int x = 1; x < n; x++) {
        for (int y = 1; x + y <= n; y++) {
            int first = get(0, x);
            int second = get(x, x + y);
            int third = get(x + y, n);

            if (first == second && (second == third || x + y == n)) {
                ps(YES);
                return;
            }
        }
