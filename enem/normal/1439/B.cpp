        ...
        while(((ll)st.size())>k)
        {
            if((*mp1.begin()).F<k-1)
            {
                ...
                for(auto it:ad[t])
                {
                    if(st.count(it))
                    {
                        mp1.erase(mp1.find(MP(mp2[it],it)));
                        mp2[it]--;
                        mp1.insert(MP(mp2[it],it));
                    }
                }
            }
            else if((*mp1.begin()).F==k-1)
            {
                V(ll) tmp;
                ll t=(*mp1.begin()).S;
                tmp.PB(t);
            ...
            else
            {
                if(k*(k-1)<=2*m)
                {
                    ...
                    REP(ot,0,nv)
                    {
                        ...
                        REP(i,0,k)
                        {
                            swap(spec[ot][i],spec[ot][i+rng()%(k-i)]);
                        }
                        REP(i,0,k)
                        {
                            REP(j,i+1,k)
                            {
                                if(!adst[spec[ot][i]].count(spec[ot][j])) bad=true;
                            }
                            if(bad) break;
                        }
                        if(!bad)
