...
fn run() {
    ...
    z.extend(p.iter().map(|p| p.1 + 1));
    z.sort();
    z.dedup();
    let mut add = vec![vec![]; z.len() - 1];
    let mut sub = vec![vec![]; z.len() - 1];
    for (i, &(l, r)) in p.iter().enumerate() {
        let l = z.binary_search(&l).unwrap();
        let r = z.binary_search(&(r + 1)).unwrap();
        add[r - 1].push(i);
        sub[l].push(i);
    }
    ...
    for (i, z) in z.windows(2).enumerate().rev() {
        let d = M::new(z[1] - z[0]);
        for &v in add[i].iter() {
            elem[v] = true;
        }
        let mut next = dp.clone();
        for (i, dp) in dp.iter().enumerate().filter(|p| !p.1.is_zero()) {
            let mut way = *dp;
            for j in (i..n).take_while(|e| elem[*e]) {
                way *= d + M::from(j - i);
                next[j + 1] += way * pc.ifact(j - i + 1);
            }
        }
        dp = next;
        for &v in sub[i].iter() {
            elem[v] = false;
            dp[..=v].iter_mut().for_each(|p| *p = M::zero());
        }
    }
    let ans = p.iter().fold(dp[n], |s, p| s * M::new(p.1 - p.0 + 1).inv());
    println!("{}", ans);
}
