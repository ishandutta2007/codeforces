...
//	rg(n)a[i]=read();gr
#endif
	int l=1,r=n,L=0x3f3f3f3f,R=0x3f3f3f3f;
	while(r-l>3)
	{
		int m=l+r>>1;
		int lm=l+m>>1;
		int rm=r+m>>1;
		int M=query(m),LM=query(lm),RM=query(rm);
		if(LM<M)r=m;
		else if(RM<M)l=m;
		else l=lm,r=rm;
	}
	for(int i=l-1;i<=r+1;i++)query(i);
	for(int i=l;i<=r;i++)if(a[i]<a[i-1]&&a[i]<a[i+1])return printf("! %d\n",i),0;
	return 0;
}