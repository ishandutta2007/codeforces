...
void BFS(int ind) {
	rep(i, 0, adjlist[ind].size()) {
		if (!visited.count(adjlist[ind][i])) {
			cout << ind + 1 << " " << adjlist[ind][i] + 1 << endl;
...
int main(){
	...
	rep(i, 0, adjlist.size()) {
		if (adjlist[i].size() > mostDecree.first) {
			mostDecree.first = adjlist[i].size();
			mostDecree.second = i;
		}
	}
	pq.push(mostDecree.second);
	visited.insert(mostDecree.second);
	while (!pq.empty()) {
		int tmp = pq.top();
		pq.pop();
		BFS(tmp);
