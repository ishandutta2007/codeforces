...
int find(int val, int lo, int hi, int cn){
	if(lo+1 == hi)return cn+1;
	if((int)T.order_of_key(val) == hi){
		return find(val-(val&(-val))+((val&(-val))>>1),lo,hi,cn);
	}
	if((int)T.order_of_key(val) == lo){
		return find(val+((val&(-val))>>1),lo,hi,cn);
	}
	return max(
	find(val-(val&(-val))+((val&(-val))>>1),lo,T.order_of_key(val),cn+1),
	find(val+((val&(-val))>>1),T.order_of_key(val),hi,cn+1)
	);
