...
int main() {
	...
	vector<Mint> ps(n);
	for (int i = 0; i < n; i++)
		ps[i] = Mint(d[i], MOD998);
	for (int i = 1; i < n; i++)
		ps[i] += ps[i - 1];
	auto get = [&](int l, int r) {
		if (l > r) return Mint(0, MOD998);
		return ps[r] - (l > 0 ? ps[l - 1] : Mint(0, MOD998));
	};
	
	while (m--) {
		int a,b; R(a,b);
		
		int idx = lower_bound(all(d), b) - d.begin();
		
		Mint smallSum = get(0, idx - 1);
		Mint largeSum = get(idx, n - 1);
		int largeCount = n - idx;
		
		if (largeCount < a) {
			W(0);
		} else {
			W(smallSum * (largeCount + 1 - a) / (largeCount + 1) + largeSum * (largeCount - a) / largeCount);
		}
	}
