...
void solve(){
  ...
  for(li i = 0; i < (li)(n); ++i) if(A[i][0] == A[i][A[i].size()-1]) {
    cout << "YES" << endl;
    return;
  }

  unordered_set<string> S2, S3;
  unordered_set<string> T3;
  for(li i = 0; i < (li)(n); ++i) {
    { string t;
      t += A[i][A[i].size()-1];
      t += A[i][A[i].size()-2];
      if((A[i].size() == 3 && S2.count(t)) || (A[i].size() == 2 && (S2.count(t) || S3.count(t)))) {
        cout << "YES" << endl;
        return;
      }
      if(A[i].size() == 3) {
    ...
    { string t;
      t += A[i][0];
      t += A[i][1];
      if(A[i].size() == 2) S2.insert(t);
      else S3.insert(t);
      if(A[i].size() == 3) T3.insert(A[i]);
    }
  }
  cout << "NO" << endl;
}
