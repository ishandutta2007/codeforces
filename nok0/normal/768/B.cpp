...
void main_() {
	...
	ll tmp = 0;
	map<ll, ll> memo, oc;
	auto dfs = [&](auto dfs, ll now) {
		if(memo.count(now)) return memo[now];
		if(now <= 1) return memo[now] = 1;
		return memo[now] = dfs(dfs, now / 2) * 2 + 1;
	};
	auto dfso = [&](auto dfso, ll now) {
		if(oc.count(now)) return oc[now];
		if(now <= 1) return oc[now] = now;
		return oc[now] = dfso(dfso, now / 2) * 2 + (now & 1);
	};
	dfs(dfs, n);
	dfso(dfso, n);
	auto dfs2 = [&](auto dfs2, ll now) -> void {
		auto val = memo[now];
		if(tmp + val - 1 < l) {
			tmp += val;
			return;
		}
		if(tmp >= l and tmp + val - 1 < r) {
			res += oc[now];
			tmp += val;
			return;
		}
