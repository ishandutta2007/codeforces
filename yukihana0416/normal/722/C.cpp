...
LL T[maxn<<2];
LL Left[maxn<<2],Right[maxn<<2];
int cut[maxn<<2];
inline void mer(int id)
{
    cut[id]=cut[ls]||cut[rs];
    T[id]=max(T[ls],T[rs]);
    Max(T[id],Right[ls]+Left[rs]);
    Left[id]=Left[ls];
    Right[id]=Right[rs];
    if(!cut[ls]) Left[id]=T[ls]+Left[rs];
    if(!cut[rs]) Right[id]=T[rs]+Right[ls];
}
...
void Cut(int pos,int l,int r,int id)
{
    if(l==r)
    {
        cut[id]=1;
        T[id]=Left[id]=Right[id]=0;
        return ;
    }
    int mid=MID(l,r);
    if(pos<=mid) Cut(pos,lson);
    else Cut(pos,rson);
    mer(id);
}
