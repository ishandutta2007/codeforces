...
const int maxn = 1e6 + 100, maxm = 1e6 + 100, maxw = 1e7 + 10, inf = 1e9 + 100, LG = 18, SQ = 300;

void precalc() {}

int bfs(vector<vector<int>> const &e) {
    int n = e.size();
//    for (int i = 0; i < n; i++) {
//        for (int j : e[i])
//            cerr << i << ' ' << j << '\n';
...
int fast(vector<int> a) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        while (!x.empty() && a[x.back()] < a[i])
            x.pop_back();
        while (!y.empty() && a[y.back()] > a[i])
            y.pop_back();
        // x > a[i]
        // y < a[i]
        if (x.empty()) {
            if (!y.empty())
                e[i].emplace_back(y[0]);
        } else if (y.empty()) {
            if (!x.empty())
                e[i].emplace_back(x[0]);
        } else {
            int ps = max(x.back(), y.back());
            if (x.back() != ps) {
                int id = lower_bound(all(x), ps, greater<int>()) - x.begin();
                assert(id < x.size());
                assert(x[id] < ps);
                e[i].emplace_back(x[id]);
            } else {
                int id = lower_bound(all(y), ps, greater<int>()) - y.begin();
                assert(id < y.size());
                assert(y[id] < ps);
                e[i].emplace_back(y[id]);
            }
        }
        x.emplace_back(i);
        y.emplace_back(i);
...
int slow(vector<int> a) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            int x = max_element(a.begin() + i, a.begin() + j + 1) - a.begin();
            int y = min_element(a.begin() + i, a.begin() + j + 1) - a.begin();
            if (min(x, y) == i && max(x, y) == j) {
                e[i].emplace_back(j);
                e[j].emplace_back(i);
            }
        }
