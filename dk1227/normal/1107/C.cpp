...
int main() {
	...
	rep(i, 1, n) {
		v[++tot].pb(a[i]);
		bel[i] = tot;
		while(s[i] == s[i + 1] && i <= n) ++i, v[tot].pb(a[i]), bel[i] = tot;
		sort(all(v[tot]));
	}
	rep(i, 1, tot) {
//		rep(j, 0, v[i].size() - 1) cout << v[i][j] << ' ';
//		puts("");
		if(v[i].size() > k) per(j, v[i].size() - 1, v[i].size() - k) ans += v[i][j];
		else rep(j, 0, v[i].size() - 1) ans += v[i][j];
	}
	printf("%lld\n", ans);
 	return 0;
}