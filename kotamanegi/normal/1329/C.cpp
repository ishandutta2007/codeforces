...
void solve(){
    ...
    for (int i = (1 << a); i <= (1 << (a + 2)); ++i) {
        inputs[i] = 0;
    }
    vector<int> moves;
    priority_queue<pair<int, int>> next;
    next.push(mp(inputs[1], 1));
    while (!next.empty()) {
        pair<int, int> now = next.top();
        next.pop();
        if (dry(now.second) == 1) continue;
        moves.push_back(now.second);
        execs(now.second);
        if (inputs[now.second] != 0) {
            next_do.push(now.second);
        }
        while (!next_do.empty()) {
            int now = next_do.top();
            next_do.pop();
            ...
            if (inputs[now] != 0) {
                if (now < (1 << b)) {
                    if (inputs[now * 2] == 0 && inputs[now * 2 + 1] == 0) continue;
                }
                next.push(mp(inputs[now], now));
            }
        }
    }
    int ans = 0;
