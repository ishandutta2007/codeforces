...
public class A {
	...
	public static void main(String args[]) throws Exception {
		...
		for(int ttt=1;ttt<=tt;ttt++) {
			...
			// n * n + 1 / 2combos
			// ideally just split it into number of thing + 1 groups of 0s
			// call res = n -m (num of zeros)
			// res / m + 1 is the the number that aren't in the group
			// add extra zer0es
			// extra zeroes = 
			// 10101 -> m = 3
			// 11110 m = 4
			// one left -> extra 0
			// n * n + 1 / 2 - (res / m + 1) * (res / m + 1 + 1) / 2 - res / m + 1 * extra (res / m + 1 %  m + 1);
			// extra 
			long res = (n - m);
			long x = (res / (m + 1));
			assert(x>=0);
			out.println((n * (n + 1)) / 2L - (x * (x+1)) / 2L * (m + 1) - (x + 1) * (res % (m+1))); 
			// then have extra 
