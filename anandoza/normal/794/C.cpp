...
int main() {
    ...
    multiset<char> s;
    multiset<char, greater<char>> t; // best at beginning
    {
        string s_string;
        re(s_string);
        string t_string;
        ...
        const int n = sz(s_string);
        sort(rall(s_string)); // best (smallest) at end
        sort(all(t_string));  // best (biggest) at end
        while (sz(s) < (n + 1) / 2) {
            s.insert(s_string.back());
            s_string.pop_back();
        }
        while (sz(t) < n / 2) {
            t.insert(t_string.back());
            t_string.pop_back();
        }
    }
    const int n = sz(s) + sz(t);
    ...
    while (true) {
        if (s.empty() || t.empty() || *s.begin() < *t.begin()) {
            ans[l++] = *s.begin();
            s.erase(s.begin());
        } else {
            ans[r--] = *s.rbegin();
