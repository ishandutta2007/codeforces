...
{
	map<ll, ll> mp;
	rep(x, 1, w) mp[uf.root(x)] += b;
	rep(y, 1, h) mp[uf.root(w+y)] += b;
	rep(y, 1, h) rep(x, 1, w){
		if(c[x][y] == '?') continue;
		mp[uf.root(x)] += c[x][y]-'0';
		mp[uf.root(w+y)] += c[x][y]-'0';
	}
	for(auto p : mp) if(p.se % 2) return false;
	return true;
}
...
int main(void)
{
	...
	if(h % 2 == 0){
		rep(y, 1, max(h, w)) rep(x, 1, max(h, w)){
	...
	if(w % 2 == 0){
		...
		rep(x, 1, w){
			ll cnt = 0;
			rep(y, 1, h) if(c[x][y] == '?') cnt++;
			ans *= modpow(2, max(0LL, cnt-1));
		}
		if(s == -1) ans *= 2;
		outl(ans);
		return 0;
	...
	ll cnt = 0;
	rep(x, 1, w) rep(y, 1, h){
		if(c[x][y] == '?') uf.unite(x, w+y), cnt++;
	}
	
	set<ll> S;
	rep(i, 1, w+h) S.insert(uf.root(i));
	cnt = cnt - (w+h) + sz(S);
	
	mint ans = 0;
	if(check(0)) ans += modpow(2, cnt);
	if(check(1)) ans += modpow(2, cnt);
	outl(ans);
