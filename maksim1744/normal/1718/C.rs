...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let mut ar: Vec<i64> = vec![0; last];
    for i in 0..n {
        for &md in mods.iter() {
            ar[ans[md][i % md]] += a[i] * md as i64;
        }
    }

    type Max = rlib_segtree::segtree_items::Max<i64>;
    let mut tree = Segtree::<Max>::from_slice(&ar.iter().map(|&i| Max::new(i)).collect::<Vec<_>>());

    for it in 0..=q {
        if it != 0 {
            // change
            let (i, x): (usize, i64) = cin.read();
            let i = i - 1;
            let dif = x - a[i];
            a[i] += dif;
            for &md in mods.iter() {
                let pos = ans[md][i % md];
                ar[pos] += dif * md as i64;
                tree.set(pos, Max::new(ar[pos]));
            }
        }
        // eprintln!("a = {:?}", a);
        // eprintln!("ans = {:?}", ans);
        // eprintln!("ar = {:?}", ar);
        outln!(tree.ask(0, last - 1).v);
    }
}
...
mod __cargo_equip {
    pub(crate) mod crates {
        ...
        pub mod __rlib_rand_0_1_0 {pub mod lcg{use std::time::SystemTime;use crate::__cargo_equip::crates::__rlib_rand_0_1_0::randomable::*;pub struct LinearCongruentialGenerator64<const A:u64,const C:u64>{state:u64,}impl<const A:u64,const C:u64>LinearCongruentialGenerator64<A,C>{pub fn from_seed(seed:u64)->Self{Self{state:seed}}pub fn from_time()->Self{Self{state:SystemTime::now().duration_since(SystemTime::UNIX_EPOCH).unwrap().as_nanos()as u64,}}pub fn next_raw(&mut self)->u64{self.state=self.state.wrapping_mul(A).wrapping_add(C);self.state}pub fn next<T,R>(&mut self,range:R)->T where R:Randomable<T>,{range.gen_from_u64(self.next_raw())}}}pub mod randomable{pub trait Randomable<T:Sized>{fn gen_from_u64(self,rng:u64)->T;}macro_rules!make_randomable{($it:ty,$ut:ty)=>{impl Randomable<$it>for std::ops::Range<$it>{fn gen_from_u64(self,rng:u64)->$it{assert!(!self.is_empty());let len=(self.end as$ut).wrapping_sub(self.start as$ut);((rng%len as u64)as$ut).wrapping_add(self.start as$ut)as$it}}impl Randomable<$ut>for std::ops::Range<$ut>{fn gen_from_u64(self,rng:u64)->$ut{assert!(!self.is_empty());let len=self.end-self.start;(rng%len as u64)as$ut+self.start}}};}make_randomable!(i8,u8);make_randomable!(i16,u16);make_randomable!(i32,u32);make_randomable!(i64,u64);make_randomable!(isize,usize);impl Randomable<f64>for std::ops::Range<f64>{fn gen_from_u64(self,rng:u64)->f64{assert!(!self.is_empty());let len=self.end-self.start;(rng as f64/u64::MAX as f64)*len+self.start}}}pub type Rng=lcg::LinearCongruentialGenerator64<6364136223846793005,1442695040888963407>;}
