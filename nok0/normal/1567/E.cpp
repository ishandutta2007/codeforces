...
void main_() {
	...
	while(q--) {
		...
		if(t == 1) {
			...
			if(x - 1 >= 0) {
				if(a[x - 1] <= a[x])
					lm.update_upd(x, x + 1, lm[x - 1]), ch.erase(x);
				else
					lm.update_chmax(x, n, x), ch.insert(x);
			}
			if(x + 1 < n) {
				if(a[x] <= a[x + 1])
					ch.erase(x + 1), lm.update_chmin(x, *ch.upper_bound(x + 1), lm[x]);
				else
					lm.update_chmax(x + 1, n, x + 1), ch.insert(x + 1);
			}
		} else {
			INT(l, r);
			--l;
			...
			//[l, ok]  l [ok+1,r) l
			int ok = l, ng = r, mid;
			while(mid = (ok + ng) >> 1, abs(ok - ng) > 1) (lm[mid] <= l ? ok : ng) = mid;
			ll res = r - l;
			res += (ll)(l + ok) * (ok - l + 1) / 2 - (ll)l * (ok - l + 1);
			res += (ll)(ok + 1 + r - 1) * (r - ok - 1) / 2 - lm.query_sum(ok + 1, r);
			print(res);
		}
	}
}