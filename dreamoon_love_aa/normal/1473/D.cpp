...
void solve() {
    ...
    FOR(i,1,n){
        ...
        dp_l[i][1]=now;
        dp_l[i][0]=min(dp_l[i-1][0],now);
        dp_l[i][2]=max(dp_l[i-1][2],now);
    }
    dp_r[n+1][0]=dp_r[n+1][1]=dp_r[n+1][2]=0;
    now=0;
    for(int i=n;i>0;i--){
        if(s[i]=='+')now--;
        else now++;
        dp_r[i][1]=now;
        dp_r[i][0]=min(dp_r[i+1][0],now);
        dp_r[i][2]=max(dp_r[i+1][2],now);
    }
    while(m--){
        int l,r;
        R(l,r);
        int ma=dp_l[l-1][2];
        int mi=dp_l[l-1][0];
        ma=max(ma,dp_l[l-1][1]+dp_r[r+1][2]-dp_r[r+1][1]);
        mi=min(mi,dp_l[l-1][1]+dp_r[r+1][0]-dp_r[r+1][1]);
        W(ma-mi+1);
    }
}
void input() {
