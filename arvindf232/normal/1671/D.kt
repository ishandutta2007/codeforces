...
fun main(){
    solve.cases{
        ...
        fun IntArray.cost():Long {
            var ret = 0L
            val n= this.size
            for(i in 0 until n-1){
                ret += (this[i] - this[i+1]).absoluteValue
            }
            return ret
        }
        ...
        important[n-1] = true
        if(n >= 3){
            var max = (1 until n-1).maxOf { L[it] }
            val min = (1 until n-1).minOf { L[it] }
            val i1 = (1 until n-1).first { L[it] == max }
            val i2 = (1 until n-1).first { L[it] == min }
            important[i1] = true
            important[i2] = true
        }
        ...
        val c = compressed.toIntArray()

        val compressedBase = c.cost()
        ...
        for(i in 0..c.size){
            for(j in 0..c.size + 1){
                ...
                a.add(j,x)
//                just dei a
                mincost = minOf(mincost, a.toIntArray().cost())
            }
        }
        val minextra = mincost - compressedBase
        put(base + minextra)
