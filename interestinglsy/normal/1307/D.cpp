...
void GetMinPath( int st , int dis[] ){
	For(i,n) dis[i] = INF;
	static queue<int> q;
	q.push(st);
	dis[st] = 0;
...
int from1[MAXN], fromN[MAXN];

bool Check( int lim ){
	static multiset<int> aYs;
	static bool inAys[MAXN];
	aYs.clear();
	Ms(inAys);
	auto p = asY.begin();
	for( auto x : as ){
		while( p != asY.end() and fromN[*p] >= lim-from1[x] ){
			aYs.insert(from1[*p]);
			inAys[*p] = 1;
			++p;
		}
		if(inAys[x]) aYs.erase(aYs.find(from1[x]));
		if( !aYs.empty() and *aYs.rbegin() >= lim-fromN[x] ){
			return true;
		}
		if(inAys[x]) aYs.insert(from1[x]);
	}
	return false;
}
...
int main(){
	...
	GetMinPath(n,fromN);
	int minPath = from1[n];
	sort(All(as),[]( int a , int b ){ return from1[a] < from1[b]; });
	asY = as;
	sort(All(asY),[]( int a , int b ){ return fromN[a] > fromN[b]; });

	int l = 0, r = minPath+10, ans = 0;
	while( l <= r ){
		int mid = l+r >> 1;
		if(Check(mid)){
			MyMax(ans,mid);
