...
int main() {
    ...
    loop(t) {
        ...
        int cl = numeric_limits<int>::max();
        int cr = numeric_limits<int>::min();
        long long cc = numeric_limits<int>::max();

        map<int, long long> lmp;
        map<int, long long> rmp;

        rep(i, n) {
            input(int, l, r);
            input(long long, c);
            if (auto it = lmp.find(l); it == lmp.end()) {
                lmp[l] = c;
            } else {
                chmin(it->second, c);
            }
            if (auto it = rmp.find(r); it == rmp.end()) {
                rmp[r] = c;
            } else {
                chmin(it->second, c);
            }

            if (l < cl or r > cr) cc = numeric_limits<int>::max();
            chmin(cl, l);
            chmax(cr, r);
            if (l == cl and r == cr) chmin(cc, c);

            auto c1 = lmp.begin()->second;
            auto c2 = rmp.rbegin()->second;
