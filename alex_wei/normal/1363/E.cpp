...
int n,a[N],b[N],c[N],x[N],y[N];
ll ans;
vector <int> e[N];

void dfs(int id,int f){
	if(f)a[id]=min(a[id],a[f]);
	if(b[id]!=c[id])b[id]?x[id]++:y[id]++;
	for(int to:e[id])if(to!=f)dfs(to,id),x[id]+=x[to],y[id]+=y[to];
	ll z=min(x[id],y[id]);
	ans+=z*2*a[id],x[id]-=z,y[id]-=z;
}
...
int main(){
	...
	for(int i=1;i<n;i++){
		int u,v; cin>>u>>v;
		e[u].pb(v),e[v].pb(u);
	} dfs(1,0),cout<<(x[1]||y[1]?-1:ans)<<endl;
	return 0;
}