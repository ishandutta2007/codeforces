...
auto nn = [](const node &a, const node &b) {
    ...
    F0R (i, 4) {
        F0R (j, 4) {
            mn comb = a.ways[i] * b.ways[j];

            if ((i&1) && (j&2)) {
                if (a.l != 1) continue;

                int inner = 10 * a.l + b.f;
                comb *= calc(inner);
            }
            else if ((i&1) || (j&2)) continue;

            res.ways[(i&2) + (j&1)] += comb;
        }
    }
...
int main() {
    ...
    st.set_leaves([&](int i) { return node(c[i] - '0'); });

    F0R (m, M) {
        int x, d; re(x, d);
        st.replace(x - 1, node(d));
