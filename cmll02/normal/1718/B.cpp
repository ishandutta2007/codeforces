...
#endif
int f[10005],g[10005],a[1000005],lm[1000005];
std::map<int,int>F,G;
signed main()
{
	initprog();
	f[0]=f[1]=1;int k=0;
	rg(100)if(i>1)f[i]=f[i-1]+f[i-2];if(f[i]>1000000000000){k=i;break;}gr
	g[0]=1;
	rrg(k+1)g[i]=g[i-1]+f[i];gr
	rrg(k+1)F[f[i]]=i,G[g[i]]=i;gr
	int T=read();rg(T)
	int n=read();
	int u=0;
	rg(n)a[i]=read(),u+=a[i];gr
	rg(n)lm[i]=-1;gr
	if(G.count(u))
	{
		int s=G[u];
		...
		for(int i=s;i>=0;i--)
		{
			...
			for(int j=1;j<=n;j++)
			{
				if(lm[j]!=i+1&&(v==-1||a[j]>a[v]))v=j;
			}
			if(v==-1)goto no;
			if(a[v]>=f[i])a[v]-=f[i],lm[v]=i;
			else goto no;
		}
		puts("YES");
	}
	...
	// {
		...
		// {
			...
			// {
				...
				// {
					// if(mp==-1)mp=k;
					// else if(a[k]>a[mp])mp=k;
				// }
			// }
			// if(mp==-1){puts("NO");goto gg;}
			// a[k]-=j;
		// }
		// puts("YES");
		// gg:;
