...
int cou[nax][2];
int &get(ll v, ll i) {
	if (i < nax) return cou[i][0];
	else return cou[v / i][1];
}
int main() {
	int n;
	scanf("%d", &n);
	...
	for (int i = 0; i < n && clock() < 3.8 * CLOCKS_PER_SEC; ++i) {
		ll v = a[i];
		vector <ll> divs, primes;
		for (int i = 1; i * 1ll * i <= v; ++i) {
			if (v % i == 0) {
				if (i > ans) divs.push_back(i);
				if (v / i != i && v / i > ans) divs.push_back(v / i);
			}
		}
		for (int p = 2; p * 1ll * p < v; ++p) {
			if (v % p == 0) {
				primes.push_back(p);
				while (v % p == 0) v /= p;
			}
		...
		v = a[i];
		REP(j, n) get(v, gcd(v, a[j]))++;
		debug(imie(v), imie(primes), imie(divs));
		for (ll p : primes)
			for (ll x : divs)
				if (x % p == 0)
					get(v, x / p) += get(v, x);
		for (ll x : divs) if (get(v, x) >= (n + 1) / 2) {
			ans = x;
			break;
		}
