...
int main() {
	...
	for(int i=0;i<N;++i){
		...
		Mod m(a);
		m=Mod(100)/Mod(a);
		seg.update(i,make_pair(m,Mod(m)));
	}
	set<int>aset;
	aset.emplace(0);
	aset.emplace(N);

	Mod answer=seg.query(0,N).second;
	while(Q--){
		int x;cin>>x;x--;
		const int m=x;
		if(aset.find(x)==aset.end()){
			auto it=aset.emplace(x).first;
			int l=*prev(it);
			int r=*next(it);
			auto ap=seg.query(l,r);
			auto bp=seg.query(l,m);
			auto cp=seg.query(m,r);
			answer-=ap.second;
			answer+=bp.second;
			answer+=cp.second;
