...
//template
#define MAXN 300000
int n,a[MAXN+10],p[MAXN+10],pcnt,cnt[MAXN+10];
bool f[MAXN+10],vis[MAXN+10];
void prepare(){
	int i;
	for(i=2;i<=600;i++){
		if(!f[i])
			p[++pcnt]=i;
		for(int j=2;i*j<=600;j++)
			f[i*j]=1;
	}
}
map<int,int>s;
	...
	for(i=1;i<=n;i++){
		...
		for(int j=1;j<=n;j++){
			if(j==i)
				continue;
			for(auto x:s2){
				if(!s.count(__gcd(x,a[j]))){
					s[__gcd(x,a[j])]=s[x]+1;
				//	s2.insert(__gcd(x,a[j]));
				}
				else
					s[__gcd(x,a[j])]=min(s[__gcd(x,a[j])],s[x]+1); 
			}
			for(auto x:s)	
				s2.insert(x.first);
		}
