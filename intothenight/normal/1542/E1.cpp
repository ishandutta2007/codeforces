...
int main(){
	...
	for(auto i = n - 1; i >= 0; -- i){
		// first differing at i
		debug(p);
		vector<Zp> pref((int)p.size() + 1);
		for(auto i = 0; i < (int)p.size(); ++ i){
			pref[i + 1] = pref[i] + p[i];
		}
		vector<Zp> dp(n - i);
		// dp[x]: sum of p[xinv] * p[yinv]
		// where xinv > x + yinv
		for(auto xinv = 0; xinv < (int)p.size(); ++ xinv){
			for(auto yinv = 0; yinv < (int)p.size(); ++ yinv){
				for(auto x = 1; x < min(xinv - yinv, n - i); ++ x){
					dp[x] += p[xinv] * p[yinv];
				}
			}
		}
		for(auto x = 0; x < n - i; ++ x){
			for(auto y = x + 1; y < n - i; ++ y){
				res += dp[y - x] * t[n][i];
			}
		}
		vector<Zp> q(n - i, 1);
		vector<Zp> p_next((int)p.size() + (int)q.size() - 1);
		for(auto i = 0; i < (int)p.size(); ++ i){
			for(auto j = 0; j < (int)q.size(); ++ j){
				p_next[i + j] += p[i] * q[j];
			}
		}
		swap(p, p_next);
