...
int solve(int n, const vector<ll>& a) {
	auto solve = [&](int l, int r, ll c) -> ll {
		// 2 sum p > sum + c
		const int len = r - l + 1;
		assert(len >= 0);
		if (len == 0) {
		...
		vector<ll> ps(len, 0);
		for (int i = l; i <= r; i++)
			ps[i - l] = (i - 2 >= l ? ps[i - 2 - l] : 0) + a[i];

		vector<ll> ss(len, 0);
		for (int i = r; i >= l; i--)
			ss[i - l] = (i + 1 <= r ? ss[i + 1 - l] : 0) + a[i];
		...
		ll ans = 0;
		{
			...
			for (int i = len; i >= 0; i -= 2) {
				ll sum = (i < len ? ss[i] : 0) + (i - 2 >= 0 ? ps[i - 2] : 0);
				chkmin(lp, i);
				ll sub = (lp - 2 >= 0 ? ps[lp - 2] : 0);
				while (lp <= i && (sum - sub) * 2 > total + c) {
					sub = (lp < len ? ps[lp] : 1e15);
					lp += 2;
				}
				ans += lp / 2;
			}
	...
	if (n >= 3) {
		ll pre = -a[0]+a[1]+a[2];
		ll suf = -a[n-3]-a[n-2]+a[n-1];
		ans += solve(3, n - 1, pre);
		ans += solve(0, n - 4, suf);

		if (n >= 6)
			ans += solve(3, n - 4, pre + suf);
	}

	const ll total = accumulate(all(a), 0LL);
	ll sum = 0;
	for (int i = 0; i < n - 2; i++) {
		sum += a[i];
		if (i >= 1 && sum * 2 > total) ans++;
	}
