...
int main() {
	...
	cin.tie(0);
	def(n) def(q)
		segtreminplus* t = new segtreminplus(n, 20);
	veci arr(n);
	intov(arr, n)
		veci first(n, -1);
	fori(i, n) {
	...
	forib(i, n) {
		if(arr[i]<=i+1)
		t->change(i, i+1-arr[i]);
		point cur = t->val(0, n - 1); 
		while (cur.ff <= 0) {
			first[cur.ss] = i;
			t->add(cur.ss, n - 1, -1);
			t->change(cur.ss, maxi);
			cur = t->val(0, n - 1);
		}
	}
	segtresum* tt = new segtresum(n, 20);
	vector<point> events;
	fori(i, n) {
		events.push_back({ ff[i],i });
	}
	veci x(q);
	veci y(q);
	fori(i, q) {
	...
	forinb(i, events) {
		if (events[i].ss >= 0) {
			tt->add(events[i].ss, 1);
		}
		else {
			int cur = -1 - events[i].ss;
			int l = x[cur];
			int r = n - 1 - y[cur];
			ans[cur] = tt->val(l, r);
		}
