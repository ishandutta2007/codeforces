...
const int nax = 1 << 17;
vpii at[nax * 2];
void add(int l, int r, pii e, int wh = 1, int lb = 0, int rb = nax) {
	if (wh == 1) debug(imie(l), imie(r), imie(e));
	if (l >= rb || r <= lb) return;
	if (l <= lb && r >= rb)
		at[wh].push_back(e);
	else {
		int sb = (lb + rb) >> 1;
		add(l, r, e, wh << 1, lb, sb);
		add(l, r, e, wh << 1 | 1, sb, rb);
	}
}
map <pii, int> since;
int ans[nax];
int u[nax], col[nax], siz[nax];
int get_col(int x) {
	int ans = 0;
	while (x != u[x]) {
		ans ^= col[x];
...
void onion(int a, int b, bool flip) {
	debug(imie(a), imie(b), imie(flip));
	a = find(a);
	b = find(b);
	assert(a != b);
	if (siz[a] > siz[b]) {
...
void rollback(int to) {
	assert(to <= (int) ops.size());
	while ((int)ops.size() > to) {
		auto [a, b] = ops.back();
		debug(imie(a), imie(b));
		ops.pop_back();
		u[a] = a;
...
void go(int wh, int lb = 0, int rb = nax) {
	int to = ops.size();
	bool err = false;
	int sb = (lb + rb) >> 1;
	if (!at[wh].empty()) debug(arr(at, wh), imie(lb), imie(rb));
	for (auto [a, b] : at[wh]) {
		int fa = find(a), fb = find(b), ca = get_col(a), cb = get_col(b);
		debug(imie(a), imie(b), imie(ca), imie(cb), imie(fa), imie(fb));
		if (fa == fb && ca == cb) {
			err = true;
			debug("err at", imie(a), imie(b), imie(ca), imie(cb));
			break;
		}
		if (fa != fb)
			onion(a, b, ca == cb);
...
int main() {
	...
	REP(i, q) {
		pii e;
		scanf("%d%d", FS(&e));
		auto it = since.find(e);
		if (it == since.end()) since[e] = i;
		else {
			add(it->second, i, e);
			since.erase(it);
		}
