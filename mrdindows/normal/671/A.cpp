...
double d[101010];
double e[101010];
double maxpref[101010];
double maxsuf[101010];
...
//        DBN(i, d[i], e[i]);
    ...
    maxpref[0] = 0;
    for (int i = 1; i <= n; ++i) {
        maxpref[i] = max(maxpref[i - 1], e[i - 1] - d[i - 1]);
    }
    maxsuf[n + 1] = 0;
    for (int i = n; i >= 1; --i) {
        maxsuf[i] = max(maxsuf[i + 1], e[i - 1] - d[i - 1]);
    }
    for (int i = 1; i <= n; ++i) {
        double res = dist(a, m[i - 1]) + dist(m[i - 1], t) + sum - e[i - 1] - max(maxpref[i - 1], maxsuf[i + 1]);
        ans = min(ans, res);
    }
    return ans;
}
