...
void solve(){
	...
	vector<ll> nc(n), nms(n);

	for(int i = 0; i < n; ++i){
		nc[i] = cost[req[i].second];
		nms[i] = mask[req[i].second];
	}
	...
	ll ans = INF;
	for(int i = 0; i < n; ++i){
		vector<ll> prv(tot, INF);
		ll cms = mask[i];

		ll ad = req[i].first - pr;


		for(int j = 0; j < tot; ++j){
			prv[j | cms] = min(prv[j | cms], dp[j] + req[i].first * b + cost[i]); 
		}

		ans = min(ans, prv[tot - 1]);

		for(int j = 0; j < tot; ++j){
			dp[j] = min(dp[j], prv[j] - req[i].first * b);
		}
