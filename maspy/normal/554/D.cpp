...
void solve() {
  ...
  //  -> 
  map<ll, ll> MEMO;
  auto dfs = [&](auto self, ll se) -> ll {
    if (MEMO.count(se)) return MEMO[se];
    auto& ANS = MEMO[se];
    ll n = popcnt(se);
    if (n == N) {
      ANS = 1;
    ...
    // n - 1  n 
    bool must = (n >= 1 && !(se & (1LL << (n - 1))));
    FOR(k, 3) {
      ll i = n - 1 + k;
      if (must && k) break;
      if (i < 0 || i >= N || se & 1LL << i) continue;
      cout.flush();
      ANS += self(self, se | 1LL << i);
    }
    chmin(ANS, 1LL << 60);
    return ANS;
  };

  auto f = [&](ll se) -> ll { return dfs(dfs, se); };

  if (K > f(0)) return print(-1);
  vi ANS(N);
  ll se = 0;
  FOR(n, N) {
    bool must = (n >= 1 && !(se & (1LL << (n - 1))));
