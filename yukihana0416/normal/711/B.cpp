...
//        cout <<n <<endl;
        ...
        LL sum=0;
        if(posx==0) for(int i=0;i<n;i++) sum+=a[1][i];
        else for(int i=0;i<n;i++) sum+=a[0][i];

        LL temp=0;
        for(int i=0;i<n;i++) temp+=a[posx][i];
        a[posx][posy]=sum-temp;
        ...
        LL ck=0;
        for(int i=0;i<n;i++) ck+=a[i][i];
        if(ck!=sum) check=0;
        ck=0;
        for(int i=0,j=n-1;i<n;i++,j--) ck+=a[i][j];
        if(ck!=sum) check=0;

        if(check)
        {
            cout <<a[posx][posy] <<endl;
        }
        else
        {
            puts("-1");
