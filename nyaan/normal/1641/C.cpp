...
void q() {
  ...
  LazySegmentTree neg(vi(N, inf), f, f, f, inf, inf);
  ...
  rep(i, Q) {
    ...
    if (T[i]) {
      ...
      L[i]--;
      if (X[i] == 0) {
        neg.update(L[i], R[i], i);
      }
    }
  }
  vl neg2(N);
  rep(i, N) neg2[i] = neg.get_val(i);
  trc(neg2);
  ...
  SegmentTree seg(
      ...
      [](pp a, pp b) {
        pl c = max(a.fi, b.fi);
        pl d = max({a.fi + b.fi - c, a.se, b.se});
        return pp{c, d};
      },
      pp{pl{-1, -1}, pl{-1, -1}});
