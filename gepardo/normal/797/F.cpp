...
class MinStack {
	private:
		vector< pair<int64_t, int64_t> > stk;
	public:
		inline size_t size() {
			return stk.size();
		}
		...
		inline void push(int64_t val) {
			stk.emplace_back(val, min(getMin(), val));
		}
...
class MinQueue {
	private:
		MinStack sHead, sTail;
	public:
		inline size_t size() {
			return sHead.size() + sTail.size();
		}
	
		inline bool empty() {
			return sHead.empty() && sTail.empty();
		}
		...
		inline int64_t getMin() {
			return min(sHead.getMin(), sTail.getMin());
		}
...
signed main() {
	...
	vi mice = getVector<int>(n);
	vpii holes = getVector<pii>(m);
	sort(mice.begin(), mice.end());
	sort(holes.begin(), holes.end());
	vector<int64_t> dp(n+1, llInf), pdp(n+1, llInf);
	dp[0] = 0;
	for (int i = 0; i < m; i++) {
		pdp = dp;
		int64_t addToAll = 0;
		...
		qu.push(pdp[0]);
		for (int j = 0; j < n; j++) {
			addToAll += abs(mice[j] - holes[i].F);
			qu.push(pdp[j+1] - addToAll);
			while (sz(qu) > holes[i].S + 1) {
				qu.pop();
			}
			dp[j+1] = qu.getMin() + addToAll;
		}
	}
	putLn((dp[n] >= llInf) ? -1 : dp[n]);
	return 0;
}