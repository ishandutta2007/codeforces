...
// for input 10000 or 100000, double must be changed to long double
using T=double;
using C=std::complex<T>;

uint32_t constexpr reverseBit(uint32_t it, int numBit){
	it=(it&0x55555555)<<1|((it>>1)&0x55555555);
	it=(it&0x33333333)<<2|((it>>2)&0x33333333);
	it=(it&0x0f0f0f0f)<<4|((it>>4)&0x0f0f0f0f);
	it=(it&0x00ff00ff)<<8|((it>>8)&0x00ff00ff);
	return (it>>16|it<<16)>>(32-numBit);
}
static_assert(reverseBit(0b110101, 6)==0b101011, "");

uint32_t constexpr ceilingPow2(unsigned it){
	return 1<<(31^__builtin_clz(it*2-1));
}
static_assert(ceilingPow2(4)==4, "");
static_assert(ceilingPow2(5)==8, "");
static_assert(ceilingPow2(8)==8, "");



void fft(C* data, int len){
	int const numBit=__builtin_ctz((unsigned)len);
	assert(len==(1<<numBit));
	for(unsigned i=1; i<(unsigned)len; ++i)
		if(auto j=reverseBit(i, numBit); j>i)
			std::swap(data[i], data[j]);

	C w=T{-1};
	for(int block=1; block<len; block<<=1, w=std::sqrt(w)){
		for(int base=0; base<len; base+=block*2){
			C x=T{1};
			for(int index=0; index<block; ++index, x*=w){
				auto& even=data[base+index]; auto& odd=data[base+block+index];
				auto t=odd*x; odd=even-t; even+=t;
			}
		}
	}
}
void ifft(C* data, int len){
	T const factor=T{1.}/len;
	fft(data, len);
	std::reverse(data+1, data+len);
	std::for_each(data, data+len,[&](auto&it){ it*=factor; });
}


int main(){
	unsigned number, k; io::read(number, k);

	modular fact[number+1], inverseFact[number+1]; // factorial~
	fact[0]=inverseFact[0]=1;
	for(int index=1; index<=number; ++index)
		inverseFact[index]=1/(fact[index]=fact[index-1]*index);

	auto const binom=[&](int n, int k){
		return fact[n]*inverseFact[k]*inverseFact[n-k];
	};

	auto const dataLen=ceilingPow2((number/2+1)*6);
	C data[dataLen];
	{
		auto dataEnd=data;
		for(int i=0;; ++i){
			if(i*2>(int)number) break;
			modular const value=binom(number-i, i);
			*dataEnd++=T{(int)value&((1<<15)-1)};
			*dataEnd++=T{(int)value>>15};
			*dataEnd++=T{0};
		}
		std::fill(dataEnd, data+dataLen, C{});
	}
	fft(data, dataLen);
	for(auto & it: data) it=it*it;
	ifft(data, dataLen);

	modular result{};
	for(int delta=0; delta+k<=number; ++delta){
		auto const i=delta+k;
		modular count1_i=
				(((int64_t)std::round(data[i*3+2].real())%MOD)<<30)+
				(((int64_t)std::round(data[i*3+1].real())%MOD)<<15)+
				(int64_t)std::round(data[i*3].real());
		result+=fact[number-i]*count1_i*fact[i]*inverseFact[delta]*((delta&1) ? -1: 1);
	}
	io::write((int)(result*inverseFact[k]), '\n');
}