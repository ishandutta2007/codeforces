...
int GetAns( int a1 , int b1 , int diff ){
	if( a1 == b1 ) return diff;
	else return INF;
}
...
int main(){
	...
	For(TwT,T){
		...
		In.ReadString(b);
		int same = 0, diff = 0;
		For0(i,n) a[i] == b[i] ? ++same : ++diff;
		
		int t00 = 0, t01 = 0, t10 = 0, t11 = 0;
		For0(i,n){
			if( a[i] == '0' ) b[i] == '0' ? ++t00 : ++t01;
			else b[i] == '0' ? ++t10 : ++t11;
		}
		...
		int ans = GetAns(a1,b1,t01+t10);
		For0(i,n){
			if( a[i] != '1' ) continue;
			if( b[i] == '0' ) Mymin( ans , GetAns(a0+1,b1,t00+t11+1)+1 );
			else Mymin( ans , GetAns(a0+1,b1,t00+t11-1)+1 );
		}
