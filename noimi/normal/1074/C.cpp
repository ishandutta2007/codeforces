...
signed main(){
    ...
    int xx[2],yy[2];
    xx[0] = *max_element(all(x));
    xx[1] = *min_element(all(x));
    yy[0] = *max_element(all(y));
    yy[1] = *min_element(all(y));
    int ans = 0;
    rep(i,n){
        rep(j,2){
            rep(k,2){
                chmax(ans,abs(x[i]-xx[j])+abs(y[i]-yy[k]));
            }
        }
    }
    cout <<ans*2<<" ";
    rep2(i,4,n)cout<<(-xx[1]-yy[1]+xx[0]+yy[0])*2<<" ";cout<<endl;
