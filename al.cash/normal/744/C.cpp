...
int main() {
  ...
  REP(i, n) {
    read("", c[i], r[i], b[i]);
    if (r[i] > n) sr += r[i] - n, r[i] = n;
    if (b[i] > n) sb += b[i] - n, b[i] = n;
  }
  REP(mask, 1 << n) REP(w, 2) REP(m, M+1) dp[mask][w][m] = INF;
  dp[0][sr > sb][min(abs(sr - sb), M)] = 0;
  REP(mask, 1 << n) REP(w, 2) REP(m, M+1) if (dp[mask][w][m] != INF) {
    int hr = w == 0 ? m : 0;
    int hb = w == 1 ? m : 0;
    ...
    REP(i, n) if (~mask >> i & 1) {
      int xr = max(r[i] - kr, 0);
      int xb = max(b[i] - kb, 0);
      int v = max(xr - hr, xb - hb);
      int xw = hb + v == xb ? 0 : 1;
      smin(dp[mask | 1 << i][xw][min(M, hr + hb + 2 * v - xr - xb)],
           dp[mask][w][m] + v);
    }
  }

  int ans = INF;
  REP(w, 2) REP(m, M+1) smin(ans, dp[(1 << n) - 1][w][m]);
  writeln(n + max(sr, sb) + ans);
