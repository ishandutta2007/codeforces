...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		...
		List<pair> ho=new ArrayList<pair>();
		for(int i=1; i<=n; i++)
			for(int j=1; j<=m; j++)
				if('A'<=a[i][j]&&a[i][j]<='Z')
					ho.add(new pair(i,j));
		//0-up, 1-down, 2-left, 3-right
		int[][][]avl=new int[n+2][m+2][4];
		for(int i=1; i<=m; i++)
			for(int j=1; j<=n; j++)
				if(a[j][i]!='#')
					avl[j][i][2]=avl[j][i-1][2]+1;
		for(int i=1; i<=n; i++)
			for(int j=1; j<=m; j++)
				if(a[i][j]!='#')
					avl[i][j][0]=avl[i-1][j][0]+1;
		for(int i=n; i>0; i--)
			for(int j=m; j>0; j--)
				if(a[i][j]!='#')
					avl[i][j][1]=avl[i+1][j][1]+1;
		for(int i=m; i>0; i--)
			for(int j=n; j>0; j--)
				if(a[j][i]!='#')
					avl[j][i][3]=avl[j][i+1][3]+1;
		...
		for(pair te:ho)
		{
			...
			for(int i=0; i<k; i++)
			{
				if(avl[x][y][dir[i][0]]>dir[i][1])
				{
					switch(dir[i][0])
					{
					case 0:x-=dir[i][1];break;
					case 1:x+=dir[i][1];break;
					case 2:y-=dir[i][1];break;
					case 3:y+=dir[i][1];break;
					}
				}
				else
				{
