...
int main() {
    ...
    starts[n] = 0;
    starts[n+1] = std::numeric_limits<int>::max();
    sort(starts.begin(), starts.end());
    auto it = unique(starts.begin(), starts.end());
    starts.resize(it - starts.begin());
    
    vector<Edge> edges;
    for (auto edge : v) {
        int s = lower_bound(starts.begin(), starts.end(), edge.s) - starts.begin();
        int e = lower_bound(starts.begin(), starts.end(), edge.e) - starts.begin();
        edges.emplace_back(s, e, -edge.c, 1);
    }
    for (int i = 0; i + 1 < (int)starts.size(); i++) {
        edges.emplace_back(i, i+1, 0, k);
    }

    MinCostMaxFlow mcmf;
    mcmf.solve(edges, 0, starts.size() - 1, starts.size());
    for (int i = 0; i < n; i++) {
        cout << (1 - mcmf.directed_edges[2*i].capacity) << " ";
    }
    cout << endl;
}