...
int main() {
	CASE(t) {
		int n,m; R(n,m);
		vector<string> s(n, string(m, '_')); R(s);
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++) {
				int p = (i + j) % 2;
				if (s[i][j] != '.') {
					if (put[p] != '_' && put[p] != s[i][j]) can = false;
					put[p] = s[i][j];
				}
			}
		
		if (put[0] != '_' && put[1] != '_' && put[0] == put[1]) can = false;
		...
		if (put[0] != '_') put[1] = put[0] ^ 'R' ^ 'W';
		if (put[1] != '_') put[0] = put[1] ^ 'R' ^ 'W';
		if (put[0] == '_') {
			put[0] = 'R';
			put[1] = 'W';
		}
