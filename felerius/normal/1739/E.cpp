// Cleaning Robot (https://codeforces.com/contest/1739/problem/E)
...
int main() {
    ...
    vector<array<array<int, 2>, 2>> dp(n, array{array{-INF, -INF}, array{-INF, -INF}});
    dp[0][0] = {0, 0};
    dp[0][1] = {-INF, g[0][1] ? 1 : -INF};

    rep(i, n - 1) {
        rep(j, 2) {
            chmax(dp[i + 1][j][0], dp[i][j][0] + g[i + 1][j]);
            if (g[i + 1][1 - j])
                chmax(dp[i + 1][1 - j][1], dp[i][j][0] + g[i + 1][j] + g[i + 1][1 - j]);
            chmax(dp[i + 1][j][0], dp[i][j][1] + g[i + 1][j]);
        }
    }

    println(max({dp[n - 1][0][0], dp[n - 1][0][1], dp[n - 1][1][0], dp[n - 1][1][1]}));
}