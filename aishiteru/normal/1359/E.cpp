...
const int N=1000005;
#define P 998244353
int n,k,fac[N],ifac[N],inv[N],ans;
il int C(ct n,ct m){return n<m||m<0?0:(0ll+fac[n])*ifac[m]%P*ifac[n-m]%P;}
int main(){
	scanf("%d%d",&n,&k);it i;
	fac[0]=fac[1]=ifac[0]=ifac[1]=inv[0]=inv[1]=1;
	for(i=2;i<=n;++i) fac[i]=(0ll+fac[i-1])*i%P,inv[i]=(0ll+inv[P%i])*(P-P/i)%P,ifac[i]=(0ll+ifac[i-1])*inv[i]%P;
	for(i=1;i<=n;++i) ans=(ans+C(n/i-1,k-1))%P;
	printf("%d",ans);
	return 0;
}