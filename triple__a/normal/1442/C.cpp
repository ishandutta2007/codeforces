...
void bfs(int u){
    ...
    while (sz(q)){
        ...
        int vt,wt,wtr;
        tie(vt,wt,wtr)=ret;
        if (wtr>=mn[vt]+20) continue;
        for (auto c:g[vt]){
            if (c.S>0){
                if (dist[c.F].find(wtr)==dist[c.F].end()){
                    dist[c.F][wtr]=wt+1;
                    q.push({c.F,wt+1,wtr});
                    mn[c.F]=min(mn[c.F],wtr);
                }
            }
            else{
                if (dist[c.F].find(wtr+1)==dist[c.F].end()){
                    dist[c.F][wtr+1]=wt;
                    q.push({c.F,wt,wtr+1});
                    mn[c.F]=min(mn[c.F],wtr+1);
                }
            }
        }
    }
...
signed main(){
    ...
    rep1(i,2*n) mn[i]=1e9;
    rep1(u,n) g[u].pb({u+n,-1}), g[u+n].pb({u,-1});
    rep(i,m){
        int u,v;
        cin>>u>>v;
        g[u].pb({v,1});
    ...
    for (auto c:dist[n]){
 //       cerr<<c.F<<" "<<c.S<<endl;
        if (c.F<=20) mnc=min(basics::power(2,c.F,1e18)-1+c.S,mnc);
        else{
            hhm=c.F, hh=c.S;
            break;
        }
...
//    cerr<<hhm<<" "<<hh<<endl;
    if (mnc<1e17){
        ...
        cout<<(basics::power(2,hhm,mod)-1+hh+mod)%mod;
    }
    return 0;
}