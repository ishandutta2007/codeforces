...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		vector<int> mxL(n),mnL(n),curL(n);
		rep(i,n-1){
			curL[i+1]=curL[i]+(s[i]=='+'?1:-1);
			mxL[i+1]=max(mxL[i],curL[i+1]);
			mnL[i+1]=min(mnL[i],curL[i+1]);
		}
		vector<int> mxR(n),mnR(n),curR(n);
		rrep(i,n-1){
			curR[i]=curR[i+1]+(s[i+1]=='-'?1:-1);
			mxR[i]=max(mxR[i+1],curR[i]);
			mnR[i]=min(mnR[i+1],curR[i]);
		}
		out(mxL,mnL,curL,1);
		out(mxR,mnR,curR,1);
		for(cauto [l,r]:qs){
			cauto d=-curR[r]+curL[l];
			cauto mx=max(mxL[l],mxR[r]+d);
			cauto mn=min(mnL[l],mnR[r]+d);
			out(l,r,d,mx,mn,1);
			cout<<mx-mn+1 NL;
		}
	}
