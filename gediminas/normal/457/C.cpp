...
/*link https://codeforces.com/problemset/problem/457/C */
/*input
5
1 2
1 2
...
int main () {
	...
	set<pii, greater<pii>> kit;

	for (auto && i : kiti) {
		kit.emplace(i.second->size(), i.first);
	}
	...
	auto add = [&](int a, int b) -> void {
		kit.erase(pii(kiti[a]->size(), a));
		kiti[a]->insert(b);
		kit.insert(pii(kiti[a]->size(), a));
	};

	auto remove = [&](int a, int b) -> void {
		kit.erase(pii(kiti[a]->size(), a));
		kiti[a]->erase(kiti[a]->find(b));
		kit.insert(pii(kiti[a]->size(), a));
	};
	...
	multiset<pii>::iterator it = visi.begin();

	while (turiu <= kit.begin()->x) {
		pii dab = *it;
		remove(dab.y, dab.x);
		sum += dab.x;
		it++;
	...
	while (it != visi.begin()) {
		...
		while (turiu <= kit.begin()->x) {
			int dab = *kiti[kit.begin()->y]->begin();
			remove(kit.begin()->y, dab);
			sum += dab;
			turiu++;
		}
		mini(ats, sum);
