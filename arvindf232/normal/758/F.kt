...
fun main(){
    solve.cases{
        ...
        if(n ==2){
            put(1L * count * (count - 1))
            return@cases
        }
        sieve.calculateSieveFast()
        val level = IntArray(max)
        val DPhash = IntArray(max)
        val po = sieve.primeOf

        DPhash[1] = 1
        for(i in 2 until max){
            val p = po[i]
            val last = i / p
            DPhash[i] = DPhash[last] * p
            if(po[last] == po[i]){
...
//            DPhash[i] = DPhash[sieve.afterPrimePowerDivison[i]]
//        }
    }
    done()
}
