...
void solve(){
	...
	ll pc = 0, pd = 0, prevc = 0, prevd = 0;
	for(int i = 0; i < vc.size(); i++){
		if(prevc) for(int j = prevc+1; j < vc[i].f; j++) preC[j] = pc;
		if(C <= 2*(vc[i].f)){
			if(preC[C-vc[i].f])
				ans = max(ans,vc[i].s+preC[C-vc[i].f]);
		}
	...
	for(int i = 0; i < vd.size(); i++){
		if(prevd) for(int j = prevd+1; j < vd[i].f; j++) preD[j] = pd;
		if(D <= 2*(vd[i].f)){
			if(preD[D-vd[i].f]) ans = max(ans,vd[i].s+preD[D-vd[i].f]);
		}
