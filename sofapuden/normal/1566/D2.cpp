...
vector<tree<int,null_type,less<int>, rb_tree_tag, tree_order_statistics_node_update>> pla;
...
void solve(){
	...
	for(int i = n-1; i >= 0; --i){
		for(int j = 0; j < m; ++j){
			int ind = M[v[i*m+j].first].back();
			M[v[i*m+j].first].pop_back();
			ans+=pla[i].order_of_key(ind);
			pla[i].insert(ind);
		}
	}	
	cout << ans << '\n';
