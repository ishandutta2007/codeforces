#include <vector>
#include <list>
#include <map>
#include <set>
#include <algorithm>
#include <utility>
#include <sstream>
#include <iostream>
#include <cmath>
#include <cstring>
#include <cstdlib>
#include <cctype>
#include <cstdio>
#include <numeric>
#include <climits>
#include <complex>
#define REP(i,n) for(int i=0;i<n;i++)
#define PB push_back
#define ALL(x) x.begin(),x.end()
#define FOREACH(e,x) for(typeof(x.begin()) e=x.begin();e!=x.end();e++)
#define FOR(i,a,b) for(int i=a;i<b;i++)//for i in [a,b)
#define MP make_pair
#define SIZE(x) int(x.size())
#define SORT(x) sort(ALL(x))
#define UNIQUE(x) x.resize(unique(ALL(x))-x.begin())
#define CLR(x,t) memset(x,t,sizeof x)
#define ACM accumulate
#define MC(a,b) memcpy(a,b,sizeof a) //memcpy
#define two(a) (1<<(a))
#define contain(s,a) (((s)&TWO(a))!=0)
using namespace std;
typedef long long int64;
typedef pair<int,int> ipair;
typedef vector<int> vi;
typedef stringstream SS;
template<class T> inline void checkmin(T&x,T c){if(c<x)x=c;}
template<class T> inline void checkmax(T&x,T c){if(c>x)x=c;}
//Finished

template<class It>
string toString(It l,It r){
	SS ss;
	ss<<"[";
	for(It i=l;i!=r;i++)
	{
		ss<<*i;
		if(i+1!=r)
			ss<<",";
	}
	ss<<"]";
	return ss.str();
}

const int MAX_POINT=100000;
typedef complex<double> point;
point ps[MAX_POINT];
int nPs;
const double PI=acos(0)*2;

int64 count(point c){
	static double a[MAX_POINT];
	for(int i=0;i<nPs;i++){
		point tmp=ps[i]-c;
		a[i]=atan2(tmp.imag(),tmp.real());
	}

	double total=0;
	for(int i=0;i<nPs;i++){
		double diff=a[i]-a[(i+1)%nPs];
		if(diff<0)diff=-diff;
		if(diff>PI)
			diff=2*PI-diff;
		total+=diff;
	}

	if(fabs(total-2*PI)>1e-6){
		return 0;
	}
	int64 cur=0;
	int j=0;
	for(int i=0;i<nPs;i++){
		while(j<i+nPs){
			double delta=a[j%nPs]-a[i];
			if(delta<0)
				delta+=2*PI;
			if(delta>PI)
				break;
			++j;
		}
		cur+=int64(j-i-1)*(j-i-2)/2;
	}
	cur=int64(nPs)*(nPs-1)*(nPs-2)/6-cur;
	return cur;
}

int main(){
	scanf("%d",&nPs);
	for(int i=0;i<nPs;i++){
		scanf("%lf%lf",&ps[i].real(),&ps[i].imag());
	}
	reverse(ps,ps+nPs);

	int nT;cin>>nT;
	REP(t,nT){
		point c;
		cin>>c.real()>>c.imag();
		cout<<count(c)<<endl;
	}
}