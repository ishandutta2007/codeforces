...
LL calc(const pii &a,const pii &b,const pii &c)
{
    LL res=((LL)(b.fi-a.fi)*(c.se-a.se)-(LL)(b.se-a.se)*(c.fi-a.fi));
    return absx(res);
}
...
void work()
{
    while(~scanf("%d%d",&n,&S))
    {
        for(int i=0;i<n;i++) read(p[i].fi), read(p[i].se);
        int x=0,y=1,z=2;
        while(1)
        {
            bool flag=1;
        ...
        printf("%d %d\n",p[x].fi+p[y].fi-p[z].fi, p[x].se+p[y].se-p[z].se);
        printf("%d %d\n",p[y].fi+p[z].fi-p[x].fi, p[y].se+p[z].se-p[x].se);
        printf("%d %d\n",p[z].fi+p[x].fi-p[y].fi, p[z].se+p[x].se-p[y].se);
    }
}
