...
struct segtree{
	...
	int getans(int v = 1){
		if(tl[v] == tr[v]) return t[v].v + tl[v];
		pushdown(v);
		return min(getans(2*v),getans(2*v+1));
	}
...
void solve(){
	...
	rep(i,0,n){
		if(b[i] <= y[0])
			t.rupd(0,y[0]-b[i],x[0]+1-a[i]);
		rep(j,0,m-1){
			if(y[j]+1 > y[j+1]) continue;
			t.rupd(y[j]+1-b[i],y[j+1]-b[i],x[j+1]+1-a[i]);
		}
	}
