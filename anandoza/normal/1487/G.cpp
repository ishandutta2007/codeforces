...
constexpr int Z = 26;
constexpr int A = 0, B = 1, X = 2;
using S = array<array<mn, 3>, 3>;
...
int main() {
    ...
    vector<vector<S>> dp(n + 2, vector(n + 2, S{}));
    // [count of A][count of B][first of last 2][second of last 2]

    auto get_type = [&](int i) { return min(i, 2); };
    ...
    for (int i = 0; i < Z; i++) {
        for (int j = 0; j < Z; j++) {
            ...
            dp[a][b][get_type(i)][get_type(j)] += 1;
        }
    }

    vector<vector<S>> next = dp;
    ...
    vvmn counts(n + 1, vmn(n + 1));
    for (int a = 0; a <= n; a++) {
        for (int b = 0; b <= n; b++) {
            for (int f = 0; f < 3; f++)
                for (int s = 0; s < 3; s++)
                    counts[a][b] += dp[a][b][f][s];
        }
    }
    ...
    for (int a = 0; a <= n; a++) {
        for (int b = 0; b <= n; b++) {
            if (a + 1 <= n)
                counts[a + 1][b] += counts[a][b];
            if (b + 1 <= n)
                counts[a][b + 1] += counts[a][b];
            if (a + 1 <= n && b + 1 <= n)
                counts[a + 1][b + 1] -= counts[a][b];
        }
    }
    ...
    // - sum_{i=a to z} where c[i] is exceeded
    for (int i = 0; i < Z; i++) {
        ans -= counts[n][n] - counts[c[i]][n];
    }

    // + sum_{i=a to z} sum_{j=a to z} where c[i] is exceeded and c[j] is exceeded
    for (int i = 0; i < Z; i++) {
        for (int j = i + 1; j < Z; j++) {
            ans += counts[n][n] - counts[c[i]][n] - counts[n][c[j]] + counts[c[i]][c[j]];
        }
    }
