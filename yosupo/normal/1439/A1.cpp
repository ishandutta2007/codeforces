...
void solve() {
    ...
    auto op = [&](const V<P>& v) {
        for (auto p : v) {
            g[p.first][p.second] = rev(g[p.first][p.second]);
        }
        arr.push_back(v);
    };
    for (int i = 0; i < n - 2; i++) {
        for (int j = 0; j < m - 1; j++) {
            ...
            if (g[i][j + 1] == '1') v.push_back({i, j + 1});
    ...
    for (int j = 0; j < m - 2; j++) {
        if (g[n - 2][j] == '0' && g[n - 1][j] == '0') continue;
        V<P> v;
        if (g[n - 2][j] == '1') v.push_back({n - 2, j});
        if (g[n - 1][j] == '1') v.push_back({n - 1, j});
