...
int main(){
  ...
  rep(i,N){
    cin>>a[v[i]-1];
    if(i==N-1||i==0)chmin(ans,a[v[i]-1]);
  }
  rep(i,N)sum[i+1]=sum[i]+a[i];
  LazySegmentTree tree(sum,[](ll a,ll b){return min(a,b);},[](ll a,ll b){return a+b;},inf);
  rep(i,N-1){
    tree.update(0,v[i],a[v[i]-1]);
    tree.update(v[i],N+1,-a[v[i]-1]);
    chmin(ans,tree.get(0,N+1));
  }
  print(ans);
  return 0;
}