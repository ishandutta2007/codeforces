...
void main_() {
	...
	V<pii> es;
	REP(i, n + m - 1) {
		es.eb(ab[i + 1].first - ab[i].first, i);
	}
	SORT(es, >);
	...
	int now = 0;
	ll res = 0;
	V<multiset<int>> use(n + m), non(n + m);
	V<ll> score(n + m);
	V<ll> cnt(n + m);
	REP(i, n + m) {
		if(ab[i].second == 0) {
			use[i].insert(ab[i].first);
			score[i] = ab[i].first;
			res += score[i];
			cnt[i] = 1;
		} else
			non[i].insert(ab[i].first);
	}

	atcoder::dsu uf(n + m);
	auto merge = [&](int i) {
		auto x = uf.leader(i);
		auto y = uf.leader(i + 1);
		res -= score[x];
		res -= score[y];
		uf.merge(i, i + 1);
		auto leader = uf.leader(i);
		if(leader == y) swap(x, y);
		cnt[x] += cnt[y];
		while(cnt[x] > SZ(use[x]) and SZ(non[x])) {
			auto itr = --non[x].end();
			use[x].insert(*itr);
			score[x] += *itr;
			non[x].erase(itr);
		...
		while(use[y].size()) {
			use[x].insert(*use[y].begin());
			score[x] += *use[y].begin();
			use[y].erase(use[y].begin());
			if(SZ(use[x]) > cnt[x]) {
				non[x].insert(*use[x].begin());
				score[x] -= *use[x].begin();
				use[x].erase(use[x].begin());
			}
		}
		while(non[y].size()) {
			use[x].insert(*non[y].begin());
			score[x] += *non[y].begin();
			non[y].erase(non[y].begin());
			if(SZ(use[x]) > cnt[x]) {
				non[x].insert(*use[x].begin());
				score[x] -= *use[x].begin();
				use[x].erase(use[x].begin());
	...
	for(auto [k, i] : querys) {
		while(SZ(es) and es.back().first <= k) {
			auto [ig, j] = es.back();
			merge(j);
			es.pop_back();
		}
