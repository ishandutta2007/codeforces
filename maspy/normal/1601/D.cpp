...
void solve() {
  ...
  sort(all(AB), [](auto& x, auto& y) { return x.fi + x.se < y.fi + y.se; });
  vi key = {D};
  FORIN(ab, AB) {
    key.eb(ab.fi);
    key.eb(ab.se);
  ...
  using E = int;
  const E INF = 1LL << 30;
  SegmentTree<E, E> seg([](E x, E y) { return max(x, y); },
                        [](E x, E y) { return x + y; },
                        [](E x, E y) { return x + y; }, -INF, 0);
  ...
  FORIN(ab, AB) {
    ...
    assert(a != b);
    if (a < b) {
      ll x = seg.fold(0, a + 1) + 1;
      if (seg.fold(b, b + 1) < x) seg.set(b, x);
    }
    elif (b < a) {
      seg.operate_range(b, a + 1, 1);
      ll x = seg.fold(0, b) + 1;
      if (seg.fold(b, b + 1) < x) seg.set(b, x);
    }
  }
  ll ANS = seg.fold(0, key.size());
  print(ANS);
}
