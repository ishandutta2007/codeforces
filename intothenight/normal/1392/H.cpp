...
int main(){
	...
	combinatorics<2000000> C;
	int n, m;
	cin >> n >> m;
	Zp p = 0;
	for(auto i = 0; i <= n; ++ i){
		p += (i + 1) * (m * C.inv[n + m - i] * C.C(n, i) / C.C(n + m, i));
	}
	vector<Zp> dp(n + 1), pref(n + 1);
	for(auto x = 1; x <= n; ++ x){
		Zp k = m * C.fact[x] * C.invfact[m + x];
		dp[x] = (1 + k * pref[x - 1]) / (1 - k * C.fact[m + x - 1] * C.invfact[x]);
		pref[x] = pref[x - 1] + dp[x] * C.fact[m + x - 1] * C.invfact[x];
	}
	cout << p * dp[n] << "\n";
	return 0;
}
