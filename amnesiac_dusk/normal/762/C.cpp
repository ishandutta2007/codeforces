...
int pf[100005],pof[100005];
void solve() {
	string a,b;
	cin>>a>>b;
	int n=sz(a),m=sz(b);
	for(int i=0,j=0; i<sz(b); i++) {
		while(j<n&&b[i]!=a[j])
			j++;
		trace(i,j);
		pf[i]=j;
	...
	for(int i=sz(b)-1,j=sz(a)-1; i>=0; i--) {
		while(j>=0&&b[i]!=a[j])
			j--;
		pof[i]=j;
		j--;
	...
	trace(tempo.fi,tempo.se);
	if(ans>m)
		cout<<b<<endl;
	else if(ans)
		cout<<b.substr(0, tempo.fi)<<b.substr(tempo.se)<<endl;
	else
		cout<<"-"<<endl;
}
signed main() {
