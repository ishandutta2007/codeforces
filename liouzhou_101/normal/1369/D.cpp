...
//mod_int res0[MAXN], res1[MAXN];
mod_int f[MAXN][2];
int choice[MAXN];

void solve()
{
	//mod_int a0 = 0, b0 = 0, c0 = 1, a1 = 0, b1 = 0, c1 = 0;
	f[1][0] = 0, f[1][1] = 0;
	f[2][0] = 0, f[2][1] = 0;
	for (int i = 3; i <= 2000000; ++i)
	{
		f[i][0] = f[i - 1][choice[i - 1]] + f[i - 2][choice[i - 2]] * 2;
		f[i][1] = f[i - 1][0] + f[i - 2][0] * 2 + 1;
		mod_int tmp = f[i][0] - f[i][1];
		if (int(tmp) <= 10) choice[i] = 0; else choice[i] = 1;
		/*
		if (int(a0) < int(a1)) write('0'); else if (int(a0) == int(a1)) write('0'); else write('1');
		res0[i] = a0; res1[i] = a1;
		mod_int ta0 = a0 + b0, tb0 = c0, tc0 = 2 * b1 + c1, ta1 = a1 + b1, tb1 = c1, tc1 = 2 * b0 + c0;
		a0 = ta0, b0 = tb0, c0 = tc0, a1 = ta1, b1 = tb1, c1 = tc1;
		*/
	}
	int T;
	read(T);
