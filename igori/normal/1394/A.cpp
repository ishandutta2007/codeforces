...
void solve()
{
    ...
    ll ans = 0;
    ll bbj = 0;
    for (int big_jokes = 0; big_jokes <= b.size(); big_jokes++)
    {
        ll waste = (big_jokes - 1) * d;
        if ((big_jokes - 1) * (d + 1) + 1 <= n)
        {
            waste -= ((int)b.size() - big_jokes);
            ll small_jokes = a.size();
            small_jokes -= max(waste, 0LL);
            ans = max(ans, bbj + (small_jokes >= 0 ? p[small_jokes] : 0));
        }
        if (big_jokes < b.size())
        {
            bbj += b[big_jokes];
