...
mint solve() {
    ...
    int dr = 1, dc = 1;
    vector<int> match_times;
    const int T = (N - 1) * (M - 1) * 2;
    REP(t, T) {
        if (rb == rd or cb == cd) match_times.push_back(t);
        rb += dr;
        if (rb > N) rb = N - 1, dr *= -1;
        if (rb <= 0) rb = 2, dr *= -1;
        cb += dc;
    ...
    mint pconti = (1 - p1).pow(match_times.size());
    mint E0 = 0;
    REP(i, match_times.size()) {
        E0 += match_times[i] * (1 - p1).pow(i) * p1;
    }
    return (E0 + pconti * T) / (1 - pconti);
}
