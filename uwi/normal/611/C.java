...
public class C {
	...
	void solve()
	{
		...
		for(int Q = ni();Q > 0;Q--){
			...
			long ret = 0;
			ret += right[r2+1][c2-1+1];
			ret -= right[r1-1+1][c2-1+1];
			ret -= right[r2+1][c1-1+1];
			ret += right[r1-1+1][c1-1+1];
			ret += down[r2-1+1][c2+1];
			ret -= down[r1-1+1][c2+1];
			ret -= down[r2-1+1][c1-1+1];
			ret += down[r1-1+1][c1-1+1];
			out.println(ret);
		}
	}
