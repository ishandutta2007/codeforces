// GCD of an Array (https://codeforces.com/contest/1493/problem/D)
...
int main() {
    ...
    rep(i, n) {
        int ai; cin >> ai;
        a[i].emplace_back(ai, 0);
    }
    rep(t, q) {
        int x, i; cin >> i >> x; --i;
        a[i].emplace_back(x, t + 1);
    }

    map<int, vector<pair<int, int>>> evt;
    map<int, pair<int, int>> atend;
    rep(i, n) {
        map<int, int> ma;
        for (auto [ai, t] : a[i]) {
            while (ai > 1) {
                int f = sieve[ai];
                ai /= f;
                evt[f].emplace_back(t, ma[f]++);
            }
        }

        for (auto& [f, e] : ma) {
            auto it = atend.emplace(f, pair(0, INF)).first;
            it->second.first++;
            it->second.second = min(it->second.second, e);
        }
    }

    vector<MI7> gcds(q + 1, 1);
    for (auto& [f, evts] : evt) {
        auto [cnt, mn] = atend[f];
        if (cnt < n)
            continue;
        sort(all(evts), greater<>{});
        int exp = mn;
        for (auto& [t, val] : evts) {
            if (val < exp)
                gcds[t] *= MI7(f).pow(exp - val), exp = val;
        }

        gcds[0] *= MI7(f).pow(exp);
    }

    rep(i, 1, q + 1)
        gcds[i] *= gcds[i - 1];
    gcds.erase(begin(gcds));
