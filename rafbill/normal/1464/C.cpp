...
bool brute(int n, vi A, li t) {
  for(li m = 0; m < (li)(1<<n); ++m) {
    li q = 0;
    for(li i = 0; i < (li)(n); ++i) q += ((m&(1<<i))?1:-1) * (1<<A[i]);
    if(q == t) return 1;
  }
  return 0;
}

bool solve(int n, vi A, li t){
  t = abs(t);
  vi cnt(62);
  for(li i = 0; i < (li)(n); ++i) cnt[A[i]]++;
  for(li i = (61); i >= (li)(0); --i) while(cnt[i]) {
    if(abs(t-(1ll<<i)) < abs(t+(1ll<<i))) {
      t -= (1ll<<i);
    }else{
      t += (1ll<<i);
    }
...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) {
    ...
    li a = (c-'a');
    if(i == n-1) t -= (1ll<<a);
    else if(i == n-2) t += (1ll<<a);
    else A.push_back(a);
  }

  cout << (solve(A.size(),A,t) ? "Yes" : "No") << endl;
