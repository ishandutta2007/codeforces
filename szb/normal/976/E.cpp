...
const ll N=600100;
ll x[N],y[N],n,A,B,ans[N],sum[N],answ,cogito;
bool cmp(ll a,ll b){return a>b;}
int main(){
	n=read(),A=read(),B=read();
	For(i,1,n){x[i]=read(),y[i]=read();if (x[i]-y[i]>0)ans[++ans[0]]=x[i]-y[i];cogito+=y[i];}
	if (!B)return writeln(cogito),0;
	sort(ans+1,ans+ans[0]+1,cmp);
	For(i,1,max(B,ans[0]))sum[i]=sum[i-1]+ans[i];
	For(i,1,n){
		ll a=x[i]<<A;
		if (ans[B]>=(x[i]-y[i]))		Max(answ,sum[B]-ans[B]+		max(a-y[i],0ll));
		else							Max(answ,sum[B]-max(x[i]-y[i],0ll)+	max(a-y[i],0ll));
	}
	writeln(cogito+answ);
}