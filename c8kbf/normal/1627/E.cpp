...
_ n, m, k, a[maxn], x, y, h;
_p d;
struct mb {
    bool tp;
    _ p, vl;
    _p ds;
    inline bool operator < (const mb &b) const {
...
inline void AC() {
    ...
    for(; k--; ) {
        ...
        d.second = read();
        h = read();
        g[x].insert((mb){true, y, h, d});
    }
    g[1].insert((mb){false, 1, 0, mp(-1, -1)});
    for(_ i = 1; i <= n; ++i) {
        dp = ss(g[i].size());
        for(_ j = 0; j <= g[i].size()+1; ++j) dp[j] = 1E17;
        _ c = 0, ls = -1, vl = -1;
        for(mb j : g[i]) {
            ++c;
            if(!~ls) {
                ...
                if(j.tp) dp[c] = vl+(j.p-ls)*a[i];
                else {
                    if(j.vl <= vl+(j.p-ls)*a[i]) {
                        ls = j.p;
                        vl = j.vl;
                        dp[c] = vl;
                    } else dp[c] = vl+(j.p-ls)*a[i];
                }
            }
        }
        i(s(mb)) it = g[i].end();
        ...
        for(; it != g[i].begin(); ) {
            ...
            if(!~ls) {
                ...
                if(j.tp) dp[c] = min(dp[c], vl+(ls-j.p)*a[i]);
                else {
                    if(j.vl <= vl+(ls-j.p)*a[i]) {
                        ls = j.p;
                        vl = j.vl;
                        dp[c] = min(dp[c], vl);
                    } else dp[c] = min(dp[c], vl+(ls-j.p)*a[i]);
                }
            }
        }
        c = 0;
        for(mb j : g[i]) {
            ++c;
            if(!j.tp) continue;
            g[j.ds.first].insert((mb){false, j.ds.second, dp[c]-j.vl, mp(-1, -1)});
        }
    }
    _ s = 1E17;
    for(mb i : g[n]) if(!i.tp) s = min(s, i.vl+(m-i.p)*a[n]);
    if(s > 1E16) writeln("NO ESCAPE");
    else writeln(s);
