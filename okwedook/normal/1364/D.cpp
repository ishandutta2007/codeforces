...
void orient(int v, int par = 0, int he = 0) {
    curr.pb(v);
    h[v] = he;
    p[v] = par;
    if (h[v] > h[maxh]) maxh = v;
    for (auto i : G[v])
        if (h[i] >= 0 && h[i] < h[v] - 1 && h[v] - h[i] + 1 <= k) {
            int len = h[v] - h[i] + 1;
            println(2);
            println(len);
            while (len--) print(curr.back() + 1, ' '), curr.popb();
            exit(0);
        }
    for (auto i : G[v])
        if (h[i] == -1) orient(i, v, he + 1);
    order.pb(v);
    curr.popb();
}

void findvert(vector<int> &ans) {
    ans.clear();
