...
void solve() {
    ...
    F0R(i, n) {
        mx[i] = mp(-1, -1);
        F0R(j, m) {
            if (i+s[j].size() <= n && t.substr(i, s[j].size())==s[j]) {
                mx[i] = max(mx[i], mp(int(i+s[j].size()), j));
            }
        }
    ...
    int x = 0;
    while (x<n) {
        if (mx[x].F==-1 || mx[x].F <= x) {
            cout << -1 << nl;
            return;
        }
        ans.pb(mp(mx[x].S+1, mx[x].F - s[mx[x].S].size() + 1));
        x = mx[x].F;
    }
    cout << ans.size() << nl;
    for (pi p : ans) cout << p.F << ' ' << p.S << nl;
}
