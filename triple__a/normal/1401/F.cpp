...
int calc(int l,int dep,int r){
    if (l<=0) return 0;
    if (dep<0) return 0;
    if (l&(1<<dep)){
        if (mov&(1<<dep)){
            return s[dep][r+1]+calc(l^(1<<dep),dep-1,r<<1);
        }
        else{
            return s[dep][r]+calc(l^(1<<dep),dep-1,(r+1)<<1);
        }
    }
    else{
        if (mov&(1<<dep)){
            return calc(l,dep-1,(r+1)<<1);
        }
        else{
            return calc(l,dep-1,r<<1);
        }
    }
}
signed main(){
