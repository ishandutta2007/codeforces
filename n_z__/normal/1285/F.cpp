...
pair<int,int>a[n+1];
for(int x=1;x<=n;x++)
cin>>a[x].second,a[x].first=0;
mt19937 mt(time(0));
shuffle(a+1,a+1+n,mt);
auto lcm=[&](int x,int y)->int{return x*y/gcd(x,y);};
int B=5000,b1=50,b2=2000,b3=50;
if(n<=B)
{
    int ans=0;
    for(int x=1;x<=n;x++)
    for(int y=x+1;y<=n;y++)
    ans=max(ans,lcm(a[x].second,a[y].second));
    cout<<ans<<endl;
    return;
}
int ans=0;
for(int x=1;x<=n;x++)
for(int y=1;y<=b1;y++)
a[x].first=max(a[x].first,lcm(a[x].second,a[y].second)),ans=max(ans,a[x].first);
sort(a+1,a+1+n,[&](pair<int,int>a,pair<int,int>b)->bool{return a.first>b.first;});
for(int x=1;x<=b2;x++)
for(int y=x+1;y<=b2;y++)
ans=max(ans,lcm(a[x].second,a[y].second));
for(int x=1;x<=b3;x++)
for(int y=1;y<=n;y++)
ans=max(ans,lcm(a[x].second,a[y].second));
