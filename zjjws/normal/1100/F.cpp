...
{
    int s[20];
    inline void clear(){memset(s,0,sizeof(s));return;}
    inline void insert(int x){for(int i=19;i>=0;i--){if(!((x>>i)&1))continue;if(s[i])x^=s[i];else {s[i]=x;break;}}}
}g[N];
inline gyq operator +(gyq &x,gyq &y){gyq z=x;for(int i=19;i>=0;i--)if(y.s[i])z.insert(y.s[i]);return z;}
inline int check(gyq now){int ans=0;for(int i=19;i>=0;i--)if(!((ans>>i)&1)&&now.s[i])ans^=now.s[i];return ans;}
...
int ans[N];
inline void work(int l,int r,vector<int>now)
{
    if(l==r){for(auto i:now)ans[i]=a[l];return;}if(now.empty())return;
    int mid=l+r>>1;g[mid].clear();g[mid].insert(a[mid]);
    for(int i=mid+1;i<=r;i++)g[i]=g[i-1],g[i].insert(a[i]);
    for(int i=mid-1;i>=l;i--)g[i]=g[i+1],g[i].insert(a[i]);
    vector<int>ls,rs;for(auto i:now){if(z[i].r<mid)ls.push_back(i);if(z[i].l>mid)rs.push_back(i);ans[i]=check(g[z[i].l]+g[z[i].r]);}
    work(l,mid,ls);work(mid+1,r,rs);return;
}
int main()
{
    rin>>n;for(int i=1;i<=n;i++)rin>>a[i];rin>>q;for(int i=1;i<=q;i++)z[i].Read();
    vector<int>st;for(int i=1;i<=q;i++)st.push_back(i);work(1,n,st);for(int i=1;i<=q;i++)printf("%d\n",ans[i]);
    return 0;
}