...
int main() {
    ...
    for (int x : lca.rev_tour_list) {
        black_count[x] = black[x];

        for (int child : lca.adj[x])
            black_count[x] += black_count[child];
    }

    int B = black_count[0];
    vector<vector<int>> black_adj(N);

    for (int x = 1; x < N; x++) {
        int par = lca.parent[x];

        if (black_count[x] >= 2)
            black_adj[x].push_back(par);

        if (B - black_count[x] >= 2)
            black_adj[par].push_back(x);
    }

    vector<int> black_neighbors;

    for (int x = 0; x < N; x++)
        if (black[x]) {
            black_neighbors.push_back(x);

            if (lca.parent[x] >= 0)
                black_neighbors.push_back(lca.parent[x]);

            for (int child : lca.adj[x])
                black_neighbors.push_back(child);
        }
    ...
    auto dfs = y_combinator([&](auto self, int node) -> void {
        ...
        for (int neigh : black_adj[node])
            self(neigh);
    });

    for (int x : black_neighbors)
        dfs(x);

    output_vector(visited);
}