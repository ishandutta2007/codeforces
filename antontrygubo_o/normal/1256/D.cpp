...
void solve()
{
    ...
    vector<char> ans;
    int cur = 0;
    while (cur<zeros.size()&&zeros[cur]<=cur + k)
    {
        ans.push_back('0');
        k-=(zeros[cur]-cur);
        cur++;
    }
    if (cur>0) for (int i = cur; i<=zeros[cur-1]; i++) ans.push_back('1');
    if (cur==0) for (int i = 0; i<n; i++) ans.push_back(s[i]);
    else for (int i = zeros[cur-1] + 1; i<n; i++) ans.push_back(s[i]);

    for (int i = 1; i<n; i++) if (ans[i]=='0' && ans[i-1]!='0')
    {
        for (int j = i; j>i-k; j--) swap(ans[j], ans[j-1]);
        break;
    }
