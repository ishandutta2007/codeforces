...
while(1)
{
    shuffle(aaa,aaa+m,mt);
    vector<int>bfa(n+1);
  	vector<pair<int,int>>f;
    for(int x=1;x<=n;x++)
    bfa[x]=x;
    function<int(int)>ffa=[&](int now){return bfa[now]=(bfa[now]!=now?ffa(bfa[now]):now);};
    for(int x=0;x<m;x++)
    {
        auto [u,v]=e[aaa[x]];
        if(ffa(u)!=ffa(v))bfa[ffa(u)]=ffa(v);
        else f.push_back({u,v});
    }
    unordered_set<int,custom_hash>s;
    for(auto [u,v]:f)
    s.insert(u),s.insert(v);
    if(s.size()!=3||f.size()!=3)
    {
        for(auto q:e){
            for(auto w:f)
            if(q==w){cout<<0;goto nxt;}
            cout<<1;
            nxt:;
        }
        ex("");
