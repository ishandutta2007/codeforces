...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) {
    li l; cin>>l;
    vi A(l); for(li i = 0; i < (li)(l); ++i) cin>>A[i];
    li cur = 1e9;
    bool ok = 1;
    for(int x : A) {
      if(x > cur) ok = 0;
    ...
    if(ok) {
      evs.emplace_back(1, A[0]);
      evs.emplace_back(0, A[l-1]);
    } else {
      ans0++;
    }
  }
  ...
  sort(begin(evs), end(evs)); reverse(begin(evs), end(evs));
  ans += ans0*n + ans0*n - ans0*ans0;
  for(auto ev : evs) {
    int ty; li x; tie(ty,x) = ev;
    if(ty == 1) {
      bit.add(2e6-x, 1);
    } else {
      ans += bit.get(2e6-x-1);
