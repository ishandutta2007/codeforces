...
int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003];string s2;vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
...
{
    if(seq[i][j]=='X'&&seq[i-1][j-1]=='X'&&seq[i-1][j+1]=='X'&&seq[i+1][j-1]=='X'&&seq[i+1][j+1]=='X')return true;
    return false;
}
        int main()
    {
      ...
      for(int i=1;i<=n;i++)
      {
         for(int j=1;j<=n;j++)cin>>seq[i][j];
      }

      for(int i=2;i<=n;i++)
      {
          for(int j=2;j<=n;j++)if(fun(i,j))x++;
