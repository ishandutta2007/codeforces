...
vv<int> solve(const vector<string> &mp){
	cauto n=mp.size();
	cauto m=mp[0].size();
	auto re=table(n,m,-(int)MOD);
	rep(i,n)rep(j,m){
		if(i) MX(re[i][j],re[i-1][j]+1);
		if(j) MX(re[i][j],re[i][j-1]+1);
		if(mp[i][j]=='B') MX(re[i][j],0);
	}
	out(mp,re,1);
	return re;
}
...
int main(){
	...
	TT{
		cauto n=read(0);
		cauto m=read(0);
		auto mp=readV<string>(n);
		auto dst=table(n,m,0);
		rep(_,2){
			rep(_,2){
				cauto cur=solve(mp);
				rep(i,n)rep(j,m) MX(dst[i][j],cur[i][j]);
				for(auto &v:mp) reverse(all(v));
				for(auto &v:dst) reverse(all(v));
			}
			reverse(all(mp));
			reverse(all(dst));
		}
		...
		int mn=MOD;
		pii re;
		rep(i,n)rep(j,m)if(MN(mn,dst[i][j])) re={i,j};
		cout<<re.X+1<<" "<<re.Y+1 NL;
	}
	return 0;
}