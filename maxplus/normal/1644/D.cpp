...
int main() {
  ...
  for (int t = $; t; --t) {
    int n = $, m = $, k = $, q = $, dr = 0, dc = 0;
    vector<pair<int, int>> qs;
    copy_n(istream_iterator<pair<int, int>>(cin), q, back_insert_iterator(qs));
    uint64_t ans = 1;
    for (auto [r, c]: range{qs.rbegin(), qs.rend()}) if (bool cr, cc; dr != n && dc != m && ((cr = !!change(dedr[r], t)) | (cc = !!change(dedc[c], t)))) {
      dr += cr, dc += cc;
      (ans *= k) %= 998244353;
    }
    print(ans);
