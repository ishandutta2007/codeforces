...
public class TaskD {
	...
	private void solve() {
		...
		int d = in.readInt();
		int[] dx = new int[moveCount];
		int[] dy = new int[moveCount];
		in.readIntArrays(dx, dy);
		int[][][][] result = new int[2 * d + 1][2 * d + 1][2][2];
		for (int[][][] array0 : result) {
			for (int[][] array1 : array0) {
		...
		x += d;
		y += d;
		if (go(x, y, 0, 0, result, dx, dy, d))
			out.println("Anton");
		else
			out.println("Dasha");
	}

	private boolean go(int x, int y, int firstUsed, int secondUsed, int[][][][] result, int[] dx, int[] dy, int d) {
		if (x > 2 * d || y > 2 * d)
			return true;
		if (result[x][y][firstUsed][secondUsed] != -1)
			return result[x][y][firstUsed][secondUsed] == 1;
		if ((x - d) * (x - d) + (y - d) * (y - d) > d * d) {
			result[x][y][firstUsed][secondUsed] = 1;
			return true;
		}
		//noinspection SuspiciousNameCombination
		if (firstUsed == 0 && !go(y, x, secondUsed, 1, result, dx, dy, d)) {
			result[x][y][firstUsed][secondUsed] = 1;
			return true;
		}
		for (int i = 0; i < dx.length; i++) {
			if (!go(x + dx[i], y + dy[i], secondUsed, firstUsed, result, dx, dy, d)) {
				result[x][y][firstUsed][secondUsed] = 1;
				return true;
			}
		}
		result[x][y][firstUsed][secondUsed] = 0;
		return false;
	}

	private static List<Test> createTests() {
		List<Test> tests = new ArrayList<Test>();
		tests.add(new Test("0 0 2 3\n" +
				"1 1\n" +
				"1 2", "Anton"));
		tests.add(new Test("0 0 2 4\n" +
				"1 1\n" +
				"1 2", "Dasha"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
