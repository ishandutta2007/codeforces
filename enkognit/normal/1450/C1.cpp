...
{
    if (x<1 || y<1 || x>n || y>n || tt[x][y] || s[x][y]!='X') return;
    tt[x][y]=1;
    v[(x+y)%3].pb(mp(x,y));
    q.push(mp(x,y));
}
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= n; j++)
            if (!tt[i][j] && s[i][j]=='X')
            {
                ...
                if (v[0].size()>=v[1].size() && v[1].size()<=v[2].size())
                {
                    z=1;
                }
                if (v[0].size()>=v[2].size() && v[1].size()>=v[2].size())
                {
                    z=2;
                }
                //cout << v[0].size() << " " << v[1].size() << " " << v[2].size() << "\n";
                for (int u = 0; u < v[z].size(); u++)
                {
                    s[v[z][u].fi][v[z][u].se]='O';
                }
            }
    for (int i = 1; i <= n; i++)
    {
