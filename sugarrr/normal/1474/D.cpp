...
void internal_calb(vl &a,vl & ret){
    ll n=a.size();
    rep(i,0,n-1){
        if(i==0)ret[i]=a[i];
        else ret[i]=a[i]-ret[i-1];
    }
}
vl dis(vl a){
    ll n=a.size();
...
bool anspro(){
    ...
    ll f1=0,f2=0;
    while(pos>=0){
        if(abs((a[pos+1]-a[pos])*2)==abs(b[n-1])){
            if(pos%2==0&&f2<=1){
                vl nx=a;
                swap(nx[pos],nx[pos+1]);
                if(hantei(nx))return true;
                f2++;
            }
            if(pos%2==1&&f1<=1){
                vl nx=a;
