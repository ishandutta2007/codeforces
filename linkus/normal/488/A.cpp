...
inline ll fstoi(const string &str){auto it=str.begin();bool neg=0;ll num=0;if(*it=='-')neg=1;else num=*it-'0';++it;while(it<str.end()) num=num*10+(*it++-'0');if(neg)num*=-1;return num;}
...
int main(){
    ...
    int t = 1;
    while(1){
        if(find(all(s),'8')!=s.end())
            REE(t);
        ++t;
        s = to_string(fstoi(s)+1);
    }
}