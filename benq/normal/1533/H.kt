...
fun solve() {
    val (n,m) = io.ints(2)
    val s = Array(n){io.read().map{(it-'A').toInt()}.toIntArray()}
    val ans = LongArray(6){0L}
    fun measure(a: Array<BooleanArray>): Long {
        val hei = IntArray(n+2){0}
        var ans = 0L
        val stk = IntArray(n+2){0}
        for (j in m-1 downTo 0) {
            ...
            var lst = 0
            for (i in 1..n+1) {
                while (hei[stk[lst]] > hei[i]) {
                    val x = stk[lst]
                    --lst
                    ans += hei[x]*(x-stk[lst])*(i-x)
                }
                stk[++lst] = i
            }
        }
    ...
    for (i in 0 until 32) {
        val a = s.map{row -> row.map{(i and (1 shl it)) != 0}.toBooleanArray()}.toTypedArray()
        ans[i.countOneBits()] += measure(a)
    }
//    println(ans.joinToString(" "))
    ...
    println((1..5).map{ans[it]}.joinToString(" "))
}
