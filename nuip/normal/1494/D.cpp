...
vv<int> mp;
vector<int> val,par;
int solve(const vector<int> &inds){
	...
	int mx=0;
	for(cauto i:inds)for(cauto j:inds) MX(mx,mp[i][j]);
	UF uf(n);
	rep(i,n)rep(j,n)if(mx!=mp[inds[i]][inds[j]]) uf(i,j);
	vv<int> mem(n);
	rep(i,n) mem[uf[i]].eb(i);
	// out(mem,1); exit(0);
	vector<int> roots;
	rep(i,n)if(uf[i]==i){
		cauto &vs=mem[i];
		cauto m=vs.size();
		vector<int> nxt(m);
		rep(i,m) nxt[i]=inds[vs[i]];
		cauto r=solve(nxt);
		roots.eb(r);
	}
	cauto r=(int)val.size();
	val.eb(mx);
	par.eb(-1);
	for(cauto v:roots) par[v]=r;
	return r;
}
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	solve(inds);
	cout<<val.size() NL;
	for(cauto x:val) cout<<x<<" ";
	cout NL;
	cout<<find(all(par),-1)-par.begin()+1 NL;
	rep(i,par.size())if(par[i]!=-1) cout<<i+1<<" "<<par[i]+1 NL;
  return 0;
}