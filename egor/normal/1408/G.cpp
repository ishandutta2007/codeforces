...
public:
    void solve() {
        ...
        sort(all(order), [&](const pii& x, const pii& y) -> bool {
        ...
        for (const auto& p : order) {
            ...
            if (dsu.join(x, y)) {
                ...
                addAll(roots[x], all(roots[y]));
                edges[x] += edges[y];
            }
            edges[x]++;
            int sz = dsu.size(x);
            if (sz * (sz - 1) / 2 == edges[x]) {
                int cur = graph.vertexCount;
                graph.addVertices(1);
                for (int i : roots[x]) {
                    graph.addEdge(cur, i);
                }
        ...
        mod = MODF;
        RecursiveFunction rec = [&](const auto& self, int cur) -> arr<modint> {
            if (graph[cur].empty()) {
                return {0, 1};
            }
            arr<modint> res = {1};
            for (auto& e : graph[cur]) {
                auto call = self(e.to);
                arr<modint> nres(res.size() + call.size() - 1, 0);
                for (int i : range(res.size())) {
                    for (int j : range(call.size())) {
                        nres[i + j] += res[i] * call[j];
                    }
                }
                res = nres;
            }
        ...
        auto res = rec(graph.vertexCount - 1);
        res = arr<modint>(res.begin() + 1, res.end());
        out.printLine(res);
    }
};
...
int main() {
    ...
    auto time = clock();
#endif
    GPodschetKlasterizatsii solver;
