...
int main(void)
{
	...
	rep(i, 1, 160000) sort(all(vec[i]));
	
	rep(y, 1, h) rep(x, 1, w) pre[x][y] = P(-inf, -inf), suc[x][y] = P(inf, inf);
	...
	per(s, h, 1){
		rep(i, 1, 160000){
			...
			for(auto p : vec[i]){
				ll x = p.fi, y = p.se;
				if(y < s) continue;
				if(pre[x][y].fi < pos) pre[x][y] = P(pos, s);
				if(y == s) pos = x;
			}
			pos = inf;
			per(j, sz(vec[i])-1, 0){
				ll x = vec[i][j].fi, y = vec[i][j].se;
				if(y < s) continue;
				if(suc[x][y].fi > pos) suc[x][y] = P(pos, s);
				if(y == s) pos = x;
			}
		}
		...
		rep(y, s, h){
			rep(x, 1, w){
				chmin(ub[x], suc[x][y].fi-1);
				if(pre[x][y].fi >= 0) chmin(ub[pre[x][y].fi], x-1);
			}
			per(x, w-1, 1) chmin(ub[x], ub[x+1]);
			rep(x, 1, w) chmax(ans, (ub[x]-x+1) * (y-s+1));
		}
	}
	outl(ans);
