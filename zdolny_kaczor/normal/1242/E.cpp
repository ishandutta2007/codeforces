...
#define arr(a,i)ngy(emz1(a,yyo(i))?a[yyo(i)]:urs(a[yyo(i)])(),#a "["+bte(imie(i))+"]")
#define arr2(a,i,j)ngy(emz1(a,yyo(i))&&emz1(a[yyo(i)],yyo(j))?a[yyo(i)][yyo(j)]:urs(a[yyo(i)][yyo(j)])(),#a "["+bte(imie(i))+"]["+bte(imie(j))+"]")
...
pii merge(pii a, pii b) {
	int M = a.second + b.second - 2;
	int m = -1;
	if (a.first >= b.second)
		m = fix(a.first - b.second + 2);
	else if (b.first >= a.second)
		m = fix(b.first - a.second + 2);
	else
		m = fix((a.second + b.second) % 2);
	return {m, M};
}
pii merge(int a, int b) {
	return merge(pii(a, a), pii(b, b));
}
// set <int> gen(pii m) {
	// assert(m.first <= m.second && (m.first - m.second) % 2 == 0 && m.first >= 3);
	// set <int> ans;
	// for (int i = m.first; i <= m.second; i += 2)
		// ans.insert(i);
	// rans;
// }
// set <int> merge(int a, int b) {
	// set <int> ans;
	// for (int v = 1; v < min(a, b); ++v) {
		// int c = a + b - 2 * v;
		// if (c >= 3)
			// ans.insert(c);
	// }
	// rans;
// }
// void test(pii a, pii b) {
	// set <int> ans1 = gen(merge(a, b));
	// set <int> ans2;
	// for (int aa : gen(a))
		// for (int bb : gen(b))
			// for (int cc : merge(aa, bb))
				// ans2.insert(cc);
	// debug(imie(a), imie(b), imie(ans1), imie(ans2));
	// assert(ans1 == ans2);
// }
pii all_merge(vector <int> in) {
	sort(ALL(in));
	assert(!in.empty());
	pii a = {in[0], in[0]};
...
// void bru_merge(vector <int> in) {
	...
	// for (int m = 1; m < (1 << l); ++m) {
		...
		// else {
			...
			// }
		// }
		// assert(wh[m] == gen({*wh[m].begin(), *wh[m].rbegin()}));
		// vector <int> sub;
		// forbits(i, m) sub.push_back(in[i]);
		// pii all_m = all_merge(sub);
		// debug(imask(m), imie(sub), arr(wh, m), imie(all_m));
		// assert(wh[m] == gen(all_m));
	// }
// }
// int test_main() {
	// REP(i, 1000) {
		// int n = 10;
		// vector <int> in(n);
		// for (int i = 0; i < n; ++i) in[i] = rand() % 10 + 3;
		// bru_merge(in);
		// cerr << "i = " << i << endl;
	// }
	// return 0;
...
vector <int> ans[nax];
int calc(vector <pii> &in) {
	pii ans = {in[0].first, in[0].first};
	for (int i = 1; i < (int) in.size(); ++i)
		ans = merge(ans, {in[i].first, in[i].first});
	rans.first;
}
void order_best(vector <pii> &in) {
	pair <int, vector <pii> > best;
	sort(ALL(in));
	best.first = calc(in);
	best.second = in;
	reverse(ALL(in));
	mini(best, make_pair(calc(in), in));
...
int main() {
	...
	order_best(in);
	pref.resize(n);
	pref[0] = {in[0].first, in[0].first};
	for (int i = 1; i < n; ++i)
		pref[i] = merge(pref[i - 1], {in[i].first, in[i].first});
	debug(imie(in));
	debug(imie(pref));
	int per = pref.back().first;
	debug("optimal out = ", imie(per));
	deque <int> curr;
	REP(_, per) curr.push_back(get_next());
	debug(imie(curr));
	for (int i = n - 1; i > 0; --i) {
		curr.push_back(curr.front());
		curr.pop_front();
		pii can = merge(in[i].first, per);
		int per2 = max(can.first, pref[i - 1].first);
		debug(imie(can), imie(per2));
		assert(per2 <= can.second && per2 <= pref[i - 1].second);
		int join = per2 + in[i].first - per;
		assert(join % 2 == 0);
		join /= 2;
		assert(join > 0);
		debug(imie(join));
		vector <int> next;
		int rem = in[i].first - join - 1;
		...
		debug(imie(next), imie(curr));
		swap(ans[in[i].second], next);
		per = per2;
	}
	reverse(ALL(curr));
	ans[in[0].second] = vector <int>(ALL(curr));
	assert(per == in[0].first);
	printf("%d\n", ile);
	REP(i, n) {
