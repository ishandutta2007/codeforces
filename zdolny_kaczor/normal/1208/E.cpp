...
int main() {
	...
	REP(_, n) {
		...
		int ran = w - l + 1;
		if (ran <= l) {
			deque <pair <ll, int> > kol = {{0, -1}};
			curr.push_back(0);
			int pos = 0;
			REP(i, w) {
				debug(imie(i) - imie(ran), imie(kol));
				while (!kol.empty() && kol.front().second <= i - ran) kol.pop_front();
				while (pos < min<int>(curr.size(), i + 1)) {
					pair <ll, int> add = {curr[pos], pos};
					debug(imie(add));
					pos++;
					while (!kol.empty() && kol.back() < add) kol.pop_back();
					kol.push_back(add);
				}
				add(i, i, kol.front().first);
			}
		}
		else {
			ll M = 0;
