// Checker for Array Shuffling (https://codeforces.com/contest/1672/problem/F2)
...
vector<int> find_cycle(G&& g) {
    auto [adj, n, to] = graph(forward<G>(g));
    vector<int> p;
    vector state(n, uint8_t(0));
    auto dfs = [&, &adj = adj, &to = to](int v, auto&& self) -> bool {
        state[v] = 1;
        p.push_back(v);
        trav(e, adj[v]) {
            if (state[to(e)] == 2) continue;
            if (state[to(e)] == 1) {
                if (!Directed && sz(p) > 1 && p[sz(p) - 2] == to(e)) continue;
                p.erase(begin(p), find(all(p), to(e)));
                return true;
            }
            if (self(to(e), self)) return true;
        }
        state[v] = 2;
        p.pop_back();
        return false;
...
int main() {
    ...
    while (t--) {
        ...
        map<int, int> node;
        trav(ai, a)
            node.emplace(ai, sz(node));

        int m = sz(node);
        vector adj(m, vector<int>());
        rep(i, n)
            adj[node[a[i]]].push_back(node[b[i]]);

        int v0 = node[mx.second];
        adj[v0].clear();
        trav(row, adj)
            row.erase(remove(all(row), v0), end(row));

        auto cyc = find_cycle<true>(adj);
        println(sz(cyc) ? "WA" : "AC");
    }
}