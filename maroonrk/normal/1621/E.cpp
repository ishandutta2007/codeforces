...
void slv(){
	...
	rep(i,m){
		int k;cin>>k;
		b[i]=readvi(k);
		sum[i]=accumulate(all(b[i]),int(0));
	}
	vc<pi> vs(n);
	rep(i,n)vs[i]=pi(a[i],1);
	mkuni(vs);
	seglazy<N> seg(vi(si(vs),0));
	auto add=[&](pi x,int w){
		int i=lower_bound(all(vs),x,cmp)-vs.bg;
		seg.ch(0,i,&N::add,w);
	};
	rep(i,n)add(pi(a[i],1),1);
	rep(i,m){
		add(pi(sum[i],si(b[i])),-1);
	}
	rep(i,m){
		add(pi(sum[i],si(b[i])),1);
		for(auto v:b[i]){
			add(pi(sum[i]-v,si(b[i])-1),-1);
			if(seg.getall().mn>=0){
				cout<<1;
			}else{
				cout<<0;
