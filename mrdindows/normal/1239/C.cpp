...
TT1T2 ostream& operator << (ostream& os, priority_queue<T1, T2> v) {
    bool f = 1; os << "["; while (!v.empty()) { if (!f) os << ", "; f = 0; os << v.top(); v.pop(); } return os << "]";
}
template <class Ch, class Tr, class Container>
basic_ostream <Ch, Tr> & operator << (basic_ostream <Ch, Tr> & os, const Container& x) {
...
int solve() {
    ...
    set<int> already;
    set<pli> q;
    set<int> whoIsInQ = {INF};
    ll curT = 0;
    ll last = 0;
    while (!q.empty() || !want.empty() || !already.empty()) {
        
        ll nextT = LLINF;
        if (!q.empty()) {
            nextT = min(nextT, max(last, q.begin()->first) + p);
        }
        if (!want.empty()) {
            nextT = min(nextT, want.begin()->first);
        }
//        DBN(curT, q, want, already, nextT);
        curT = nextT;
        
        if (!q.empty() && max(last, q.begin()->first) + p <= curT) {
            int who = q.begin()->second;
            ans[who] = max(last, q.begin()->first) + p;
            last = ans[who];
            q.erase(q.begin());
            whoIsInQ.erase(who);
        }
        while (!want.empty()) {
            int who = want.begin()->second;
            ll when = want.begin()->first;
            if (when <= curT) {
                already.insert(who);
        ...
        while (!already.empty()) {
            int minInQ = *whoIsInQ.begin();
            if (*already.begin() > minInQ) {
                break;
            }
            q.insert({curT, *already.begin()});
            whoIsInQ.insert(*already.begin());
