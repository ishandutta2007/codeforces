...
void solve(){
	int n,d; cin>>n>>d;
	for(int i=1;i<=n;i++)cin>>a[i]; a[++n]=d;
	long double l=0,r=5e8;
	while(r-l>1e-9){
		long double m=(l+r)/2;
		long double x=0,y=0,now=0;
		for(int i=1;i<=n;i++){
			long double dis=a[i]-a[i-1],t=dis/i;
			if(now+t<=m)now+=t,x=a[i];
			else{
				x+=i*(m-now);
				break;
			}
		...
		for(int i=n-1;~i;i--){
			long double dis=a[i+1]-a[i],t=dis/(n-i);
			if(now+t<=m)now+=t,y=d-a[i];
			else{
				y+=(n-i)*(m-now);
				break;
			}
