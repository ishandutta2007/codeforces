...
int dfs(int bg,int en,int cap){
	...
	lgl[bg]=1;
	for(int i=0;i<SZ(egs[bg]);i++){
		if(egs[bg][i].cap&&!lgl[egs[bg][i].to]){
			int dis=dfs(egs[bg][i].to,en,min(cap,egs[bg][i].cap));
			if(dis){
				egs[bg][i].cap-=dis;
				egs[egs[bg][i].to][egs[bg][i].rev].cap+=dis;
				return dis;
			}
		}
	}
...
bool notsushu[20005];
vector<int> nei[1005];
vector<int> what[1005];

void Dfs(int now,int whr){
	lgl[now]=1;
	what[whr].pb(now);
...
int main(){
	...
	for(int i=1;i<=n;i++){
		for(int j=1;j<=n;j++){
			if((a[i]%2)&&!(a[j]%2)&&!notsushu[a[i]+a[j]]){
				NewEdge(i,j,1);
			}
		}
	}
