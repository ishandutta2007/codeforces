...
ll rect(int x1, int y1, int x2, int y2) {
    ll ans = 0;
    FOR(i, x1, x2+1) FOR(j, y1, y2+1) ans += c[i][j];
    return ans;
}
...
void solve() {
    ...
    read(c, 2*n, 2*n);
    ans = 0;
    FOR(i, n, 2*n) FOR(j, n, 2*n) ans += c[i][j], c[i][j]=0;
    add = 1e18;
    add = min(add, c[n][n-1]);
    add = min(add, c[2*n-1][n-1]);
    add = min(add, c[n][0]);
    add = min(add, c[2*n-1][0]);
    add = min(add, c[n-1][n]);
    add = min(add, c[n-1][2*n-1]);
    add = min(add, c[0][n]);
    add = min(add, c[0][2*n-1]);
    cout << ans+add << nl;
}
