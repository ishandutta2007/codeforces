...
signed main(){
    ...
    cin>>n>>T;
    cin>>s;
    T-=basics::power(2,s[n-1]-'a',INF), T+=basics::power(2,s[n-2]-'a',INF);
    for (int i=0;i<n-2;++i){
        T+=basics::power(2,s[i]-'a',INF);
        cnt[s[i]-'a']++;
    } 
    if (T<0||T&1){
        cout<<"No\n";
    ...
    for (int i=25;i>-1;--i){
        int tms=T/basics::power(2,i+1,INF);
        if (tms<=cnt[i]) T-=tms*basics::power(2,i+1,INF);
        else T-=cnt[i]*basics::power(2,i+1,INF);
    }
    if (T>0) cout<<"No\n";
    else cout<<"Yes\n";
}