...
int main(){
  ...
  rep(i,M)cin>>b[i];
  b.eb(inf);
  LazySegmentTree tree(v,[](ll x,ll y){return x+y;},[](ll x,ll y){return x+y;},0);
  rep(i,N){
    if(a[i]==b[j]){
      tree.update(j,j+1,tree.get(j-1,j)+p[i]);
      ++j;
      continue;
    }else if(a[i]>b[j])continue;
    ll itr=lower_bound(all(b),a[i])-b.begin();
    if(p[i]<0)tree.update(itr,j,p[i]);
    if(a[i]==b[itr]){
      ll bf=tree.get(itr,itr+1),now=tree.get(itr-1,itr)+p[i];
      if(bf>now){
        tree.update(itr,itr+1,now-bf);
      }
    }
  }
  if(j<M){
