...
void solve() {
    runs.clear(); insertions.clear();
    cin >> n;
    read(a, n);
    freq.clear(); for (int i : a) freq[i]++;
    for (pi p : freq) if (p.S%2 != 0) {
        cout << -1 << nl;
        return;
    }
    int x = 0;
    while (x < a.size()) {
        ...
        vi toadd;
        FOR(i, x+1, nxt) {
            insertions.pb(mp(i+nxt-x, a[i]));
            toadd.pb(a[i]);
        }
        runs.pb(2*(nxt-x));
        vi b;
        b.insert(b.end(), a.begin(), a.begin()+nxt+1);
        b.insert(b.end(), all(toadd));
        reverse(all(toadd));
        b.insert(b.end(), all(toadd));
        b.insert(b.end(), a.begin() + nxt + 1, a.end());
        debug(b);
        a = b;
        x = 2*nxt - x;
        debug(a);
    }
    cout << insertions.size() << nl;
    for (pi p : insertions) cout << p.F << ' ' << p.S << nl;
    cout << runs.size() << nl;
    for (int i : runs) cout << i << ' ';
    cout << nl;
}
