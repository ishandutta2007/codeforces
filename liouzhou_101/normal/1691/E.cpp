...
void solve()
{
	...
	vector<DisjointSet> s = { DisjointSet(v[0].size()), DisjointSet(v[1].size()) };
	DisjointSet g(n);
	...
	for (int c = 0; c < 2; ++c)
	{
		auto& a = v[c];
		auto& b = v[1 - c];
		int da = c == 0 ? 0 : v[0].size();
		int db = c == 0 ? v[0].size() : 0;
		for (int i = 0; i < a.size(); ++i)
		{
			auto& [x, y] = a[i];
			auto l = lower_bound(b.begin(), b.end(), pair<int, int>(x, -1)) - b.begin();
			auto r = lower_bound(b.begin(), b.end(), pair<int, int>(y + 1, -1)) - b.begin() - 1;
			// [l, r]
			if (l > r) continue;
			g.Union(i + da, l + db);
			g.Union(i + da, r + db);
