...
constexpr int apply(int f, int x) { return f + x; }
constexpr int comp(int f, int g) { return f + g; }
...
int main() {
    int q;
    read(q);
    atcoder::lazy_segtree<int, op, e, int, apply, comp, id> seg(200000);
    int cnt = 0;
    while (q --> 0) {
        string s;
        int id;
        ...
        --id;
        if (s == "L") {
            seg.apply(0, 200000, 1);
            seg.apply(id, -seg.prod(id, id + 1));
            ++cnt;
        } else if (s == "R") {
            seg.apply(id, cnt - seg.prod(id, id + 1));
            ++cnt;
        } else if (s == "?") {
            int l = seg.prod(id, id + 1);
            int r = cnt - l - 1;
            print(min(l, r));
        } else { print("!!!"); }
    }
