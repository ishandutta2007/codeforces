...
void mian() {
	...
	vi out;
	REP(i, 1, n) {
		static int dp[N][M], pa[N][M];
		REP(j, 0, SZ(op[i])) memset(dp[j], 0x3f, sizeof(dp[j]));
		dp[0][0] = 0;
		REP(k, 1, SZ(op[i])) {
			int t, p, id; tie(t, p, id) = op[i][k - 1];
			REP(j, 0, 100) dp[k][j] = dp[k - 1][j], pa[k][j] = j;
			REP(j, 0, 100) if(dp[k - 1][j] + t < dp[k][min(100ll, j + p)])
				dp[k][min(100ll, j + p)] = dp[k - 1][j] + t, pa[k][min(100ll, j + p)] = j;
		}
		int shd = dp[SZ(op[i])][100];
		now += shd;
		if(now > a[i]) return puts("-1"), void();
		int j = 100;
		PER(k, SZ(op[i]), 1) {
			if(pa[k][j] < j) out.pb(Z(op[i][k - 1]));
			j = pa[k][j];
		}
	}
	prt(SZ(out)), pc('\n');
	for(int x : out) prt(x), pc(" \n"[x == out.back()]);
}
