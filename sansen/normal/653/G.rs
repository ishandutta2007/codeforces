...
fn run() {
    ...
    let w = 20;
    let mut ans = M::zero();
    for g in g.into_iter().filter(|g| !g.is_empty()) {
        let mut cnt = vec![0; w];
        let mut l = n;
        for g in g {
            cnt[g] += 1;
        ...
        for c in cnt[1..].iter() {
            if l > 0 && r > 0 {
                let mut way = M::new(2).pow(l as u64);
                let mut x = 0;
                // binom(l, i) (i >= y)
                let mut left = |y: usize| -> M {
                    if y > l {
                        return M::zero();
                    }
                    while x < y {
                ...
                let mut way = M::new(2).pow(r as u64);
                let mut x = 0;
                // binom(r, i) (i >= y)
                let mut right = |y: usize| -> M {
                    if y > r {
                        return M::zero();
                    }
                    while x < y {
                ...
                for i in 1..=r {
                    ans += M::from(i) * pc.comb(r, i) * left(i);
                }
                for i in 2..=r {
                    ans += M::from(i - 1) * pc.comb(l, i - 1) * right(i);
                }
            }
            l += *c;
            r -= *c;
