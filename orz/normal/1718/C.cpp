...
int another_main()
{
	...
	for (int I = 0; I < t; ++I) {
		...
		vi a(n);
		cin >> a;
		vpui fc = fac.factor(n);
		vector<multiset<ll>> numbers(sz(fc));
		vector<vector<ll>> sums(sz(fc));
		for (int i = 0; i < sz(fc); ++i) {
			int p = fc[i].X;
			int pp = n / p;
			sums[i].resize(pp);
			for (int j = 0; j < n; ++j)
		...
		for (int i = -1; i < q; ++i) {
			if (i >= 0) {
				...
				ll addend = x - a[P];
				a[P] = x;
				for (int ii = 0; ii < sz(fc); ++ii) {
					int p = fc[ii].X;
					int pp = n / p;
					numbers[ii].erase(numbers[ii].find(sums[ii][P % pp]));
					sums[ii][P % pp] += addend;
					numbers[ii].insert(sums[ii][P % pp]);
				}
			}
			ll mx = 0;
			for (int ii = 0; ii < sz(fc); ++ii) {
				int p = fc[ii].X;
				int pp = n / p;
				mx = max(mx, *numbers[ii].rbegin() * pp);
			}
			cout << mx << '\n';
		}
	}
