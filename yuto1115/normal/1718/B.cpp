...
vi zeckendorf(ll l) {
    vi v;
    rrep(i, 60, 1) {
        if (fibo[i] > l) continue;
        if (not v.empty()) assert(v.back() > i + 1);
        v.pb(i);
        l -= fibo[i];
    }
    REV(v);
...
void solve() {
    ...
    rep(i, k) {
        ze[i] = zeckendorf(c[i]);
        ze2[i] = zeckendorf(c[i] - 1);
        for (int j: ze[i]) ++cnt[j];
    }
    rep(i, k) {
        for (int j: ze[i]) --cnt[j];
        for (int j: ze2[i]) ++cnt[j];
        bool ok = true;
        ok &= ze2[i].empty() or ze2[i][0] > 1;
        rep(j, 1, n) ok &= cnt[j] == 1;
        rep(j, n, 60) ok &= cnt[j] == 0;
        if (ok) {
            print("YES");
            return;
        }
