...
{
    int n; cin>>n;
    vector<ll> a(n); for (int i = 0; i<n; i++) {cin>>a[i]; a[i]--;}
    vector<vector<int>> pos(n);

    for (int i = 0; i<n; i++) pos[a[i]].push_back(i);

    vector<int> odd;
    for (int i = 0; i<n; i++) if (pos[i].size()%2) odd.push_back(i);

    vector<int> ans(n);

    if (odd.size()>1) {cout<<"NO"<<endl; return;}
    if (odd.size()==1)
    {
        int x = odd[0];
        if (pos[x].size() == 1 && pos[x][0]==n/2) {cout<<"NO"<<endl; return;}
        vector<int> tmp;
    ...
    for (auto vec: pos) if (!vec.empty())
    {
        int k = vec.size();
        for (int i = 0; i<k; i+=2)
        {
            prs.push_back(mp(vec[i], vec[i+1]));
        }
    }

    for (int i = 0; i<n/2; i++)
    {
        ans[i] = prs[i].first; ans[n-1-i] = prs[i].second;
    }

    DSU dsu(n);
    for (int i = 0; i<n; i++) dsu.union_sets(i, ans[i]);
    for (int i = 0; i<n/2; i++) dsu.union_sets(i, n-1-i);

    for (int i = 1; i<n/2; i++)
    {
        if (dsu.find_set(i)!=dsu.find_set(0))
        {
            swap(ans[0], ans[i]);
            swap(ans[n-1], ans[n-1-i]);
            dsu.union_sets(0, i);
        }
    }

    DSU dsu1(n);
    for (int i = 0; i<n; i++) dsu1.union_sets(i, ans[i]);

    for (int i = 0; i<n/2; i++)
    {
        if (dsu1.find_set(i)!=dsu1.find_set(n-1-i))
        {
            swap(ans[i], ans[n-1-i]);
            dsu1.union_sets(i, n-1-i);
        }
    }
