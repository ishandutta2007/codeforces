...
int main(){
  ...
  //0
  set<string>fil;
  for(auto s:r)fil.insert(s);
  for(auto s:e)fil.insert(s);
  vector<string>ve,vr;
  rep(i,0,n){
    string t=to_string(i+1);
    if(fil.count(t))continue;
    if(i<ex)ve.PB(t);
    else vr.PB(t);
  }
  auto tr=[](string k){
    if(k[0]=='0')return -1LL;
    for(auto c:k)if(!isdigit(c))return -1LL;
    return stoll(k);
  };
  auto check=[&](string s){
    ll k=tr(s);
  ...
  auto comp=[&](string x,string y){
    ...
    else if(!check(x)&&check(y))return true;
    else return x<y;
  };
  sort(ALL(r),comp);
  sort(ALL(e),comp);
  ...
  auto mv=[&](string x,string y){
    ss<<"move" spa x spa y<<endl;
    vac(x);
    cnt++;
  };
  //cout<<"r:";debug(r,r.size());
  while(!r.empty()||!e.empty()){
    while(!r.empty()){
      ll t=tr(r.back());
      if(t>ex&&t<=n){
        //cout<<t spa r.back()<<endl;
        r.pop_back();
      }
      else break;
    }
    while(!e.empty()){
      ll t=tr(e.back());
      if(t>=1&&t<=ex)e.pop_back();
      else break;
    }
    if(r.empty()&&e.empty())break;
    if(!vr.empty()){
      //assert(!r.empty());
      string s1=r.back(),s2=vr.back();
      r.pop_back();
      vr.pop_back();
      mv(s1,s2);
      //cout<<r.back() spa vr.back()<<endl;
      
    }
    else if(!ve.empty()){
      string s1=e.back(),s2=ve.back();
      e.pop_back();
      ve.pop_back();
      mv(s1,s2);
    }
      ...
      if(!r.empty()){
        ...
        mv(e.back(),aki);
        e.back()=aki;
        rotate(e.begin(),e.begin()+e.size()-1,e.end());
      }
    }

    /*cout<<"e:";debug(e,e.size());
    cout<<"ve:";debug(ve,ve.size());
    cout<<"r:";debug(r,r.size());
    cout<<"vr:";debug(vr,vr.size());cout<<endl;*/
    //cout<<"rs" spa r.size()<<endl;
    //if(cnt>=3)break;
  }
  cout<<cnt<<endl;
  cout<<ss.str();
