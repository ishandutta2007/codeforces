...
string s[N];
bitset<55> visc, visr;
vector<set<int>> Rs, Cs;

void dfsC(int, set<int>*, set<int>*);

void dfsR(int r, set<int>* R, set<int>* C) {
    if (visr.test(r)) return;
    visr.set(r, 1);
    R->insert(r);
    FOR (j, 0, m) {
        if (s[r][j] == '#') {
...
void dfsC(int c, set<int>* R, set<int>* C) {
    if (visc.test(c)) return;
    visc.set(c, 1);
    C->insert(c);
    FOR (i, 0, n) {
        if (s[i][c] == '#') {
