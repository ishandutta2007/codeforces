...
int main(void)
{
	...
	while(Q--){
		...
		rep(i, 0, 1) rep(j, 0, T) rep(k, 0, 2) dp[i][j][k] = inf;
		dp[0][0][0] = 1, dp[0][0][1] = dp[0][0][2] = 0;
		rep(i, 0, S-1){
			rep(j, 0, T) rep(k, 0, 2) dp[(i+1)&1][j][k] = inf;
			rep(j, 0, T){
				rep(k, 0, 2){
					rep(l, 0, 2){
						if(k > l) continue;
						if(j+1 <= T && s[i+1] == t[j+1]){
							if(k == 1) chmin(dp[(i+1)&1][j+1][l], dp[i&1][j][k] + 0);
							else chmin(dp[(i+1)&1][j+1][l], dp[i&1][j][k] + 1);
						}
						if(k == 0) chmin(dp[(i+1)&1][j][l], dp[i&1][j][k] + 2);
						if(k == 2) chmin(dp[(i+1)&1][j][l], dp[i&1][j][k] + 1);
					}
				}
			}
		}
		if(dp[S&1][T][2] > inf/2) outl(-1);
		else outl(dp[S&1][T][2]);
	}
