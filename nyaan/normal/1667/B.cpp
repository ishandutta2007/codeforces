...
void q() {
  ...
  SegmentTree seg(
      vl(sz(xs), -infLL), [](ll s, ll t) { return max(s, t); }, -infLL);

  vl dp(N + 1, infLL);
  dp[0] = 0;
  seg.update(lb(xs, r[0]), 0 - 0);

  rep1(i, N) {
    // 
    int id = lb(xs, r[i]);
    ll mx = seg.query(0, id);
    // 
    amax(mx, dp[i - 1] + (a[i - 1] > 0 ? 1 : a[i - 1] == 0 ? 0 : -1) - i);
