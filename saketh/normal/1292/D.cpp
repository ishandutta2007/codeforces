...
int main() {
    ...
    {
        assert(sz(sv.primes) == PC);

        vi pi(MAXV);
        F0R (i, PC) pi[sv.primes[i]] = i;

        FOR (fact, 1, MAXV) {
            memcpy(rep[fact], rep[fact - 1], sizeof(rep[fact]));
            trav (e, sv.factor(fact)) rep[fact][pi[e.f]] += e.s;
        }
    }

    vi ct(MAXV);
    int N; { re(N); F0R (i, N) { int v; re(v); ct[v]++; } }
    ct[1] += ct[0], ct[0] = 0;
    ...
    FOR (fact, 1, MAXV) {
        ...
        F0Rd (p, PC) {
            int m = rep[fact][p];
            ans += ll(m) * ct[fact];
            auto it = trie[loc].find(m);
            if (it == trie[loc].end()) {
                loc = trie[loc][m] = sz(trie);
                trie.eb(), wt.pb(0);
            } else {
                loc = it->s;
            }
    ...
    auto search = [&](auto& self, int loc, ll cur) -> void {
        ckmin(ans, cur);
        int away = N - wt[loc], last = 0;
        trav (e, trie[loc]) {
            cur += ll(e.f - last) * (2 * away - N);
            self(self, e.s, cur);
            last = e.f;
            away += wt[e.s];
        }
