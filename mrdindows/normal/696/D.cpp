...
#define LLINF       4000111000111000111LL
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            int pos = -1;
            while (true) {
                pos = (int)s[i].find(s[j], pos + 1);
                if (pos == -1) break;
                bonus[i][pos + s[j].size() - 1] += a[j];
            }
        }
        bonus[i][s[i].size() - 1] += a[i];
    }
    int p = 0;
    for (int i = 0; i < n; ++i) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 1; j < s[i].size(); ++j) {
            addEdge(idx[i][j - 1], idx[i][j]);
        }
        for (int j = 0; j < n; ++j) {
            for (int fi = 0; fi < s[i].size(); ++fi) {
                for (int k = 0; k < s[j].size() && fi + k < s[i].size(); ++k) {
                    addEdge(idx[i][fi + k], idx[j][k]);
                    if (s[i][fi + 1 + k] != s[j][k]) {
                        break;
                    }
                }
