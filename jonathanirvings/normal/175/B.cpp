...
int main()
{
    ...
    SORTA(nilai,m);
    REVERSEA(nilai,m);
    //REPN(i,m) printf("%s %d\n",nama[nilai[i].se].c_str(),nilai[i].fi);
    printf("%d\n",m);
    REPN(i,m)
    {
    		int more = 0;
		...
		REPN(k,m)
		{
			if (nilai[i].fi >= nilai[k].fi) ++more;
			if (nilai[i].fi < nilai[k].fi) ++less;
		}
		//printf("%d %d\n",more,less);
		printf("%s ",nama[nilai[i].se].c_str());
		if (less > m / 2) puts("noob"); else
		if (less > m / 5 && more >= (int)((double)m / 2.0)) puts("random"); else
		if (less > m / 10 && more >= (int)((double)(m * 4) / 5.0)) puts("average"); else
		if (less > m / 100 && more >= (int)((double)(m * 9) / 10.0)) puts("hardcore"); else
		puts("pro");
    }
	return 0;
}