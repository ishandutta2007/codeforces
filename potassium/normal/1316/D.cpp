...
int main() {
	int n; R(n);
	vector<vector<pii>> dest(n, vector<pii>(n)); R(dest);
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			if (dest[i][j].fi != -1) {
				dest[i][j].fi--;	
	...
	auto getCell = [&](int r, int c, int dir) -> pii {
		int nr = r + dr[dir];
		int nc = c + dc[dir];
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			...
			for (int dir = 0; dir < 4; dir++) {
				pii dd = getCell(i, j, dir);
				if (dd.fi != -1 && dest[dd.fi][dd.se].fi == -1) {
					has = 1;
					board[i][j] = dirs[dir];
					board[dd.fi][dd.se] = dirs[dir ^ 1];
				}
			}
	...
	while (!q.empty()) {
		...
		for (int dir = 0; dir < 4; dir++) {
			pii d = getCell(now.fi, now.se, dir);
			if (d.fi == -1)
				continue;
			if (board[d.fi][d.se] != '_')
				continue;

			if (dest[d.fi][d.se] == dest[now.fi][now.se]) {
				board[d.fi][d.se] = dirs[dir ^ 1];
				q.push(d);
			}
		}
	}
