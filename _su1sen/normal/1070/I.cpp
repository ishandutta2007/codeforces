...
int main() {
    ...
    loop(t) {
        ...
        rep(i, n) {
            g.add_edge(m + i, dst, a[i]);
        }
        if (g.flow(src, dst) != accumulate(all(a), 0)) {
            print(vector<int>(m));
            continue;
        }
        vector<int> ans(m);
        ...
        for (auto e : g.edges()) {
            if (e.from == src or e.to == dst or e.flow == 0) continue;
            int eid = e.from;
            int vid = e.to - m;
            ins[vid].push_back(eid);
        }
        int next_col = 1;
        rep(i, n) {
            rep(j, a[i] / 2) {
                ans[ins[i][2 * j]] = ans[ins[i][2 * j + 1]] = next_col++;
            }
        }
        print(ans);
    }
