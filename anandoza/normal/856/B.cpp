...
struct trie {
    ...
    int adv(int pos, char c, bool create = false) {
        auto res = data[pos].children[c - FIRST];
        if (res == 0 && create) {
            res = data[pos].children[c - FIRST] = sz(data);
            data.push_back({{}, pos});
        }
        return res;
    }

    int parent(int pos) { return data[pos].parent; }
};
...
void solve() {
    ...
    for (auto &w : s) {
        int pos = 0;
        for (char c : w) {
            pos = t.adv(pos, c, true);
        }
    }

    const int k = sz(t.data) - 1;
    vvi adj(k);
    ...
    for (auto &w : s) {
        int pos = 0;
        int par = t.adv(0, w.front());
        for (char c : range(w.begin() + 1, w.end())) {
            pos = t.adv(pos, c);
            par = t.adv(par, c);
