...
int main() {
    ...
    std::map<int, std::map<int, int>> mp_uv, mp_vu;
    ...
    std::vector<int> d = suisen::manhattan_minimum_distances(points);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (auto it = mp_vu[cv + d[i]].upper_bound(cu - d[i]); s[i].size() < 4 and it != mp_vu[cv + d[i]].end() and it->first <= cu + d[i]; ++it) {
            s[i].push_back(it->second);
        }
        for (auto it = mp_uv[cu - d[i]].upper_bound(cv - d[i]); s[i].size() < 4 and it != mp_uv[cu - d[i]].end() and it->first <= cv + d[i]; ++it) {
            s[i].push_back(it->second);
        }
        for (auto it = mp_vu[cv - d[i]].lower_bound(cu - d[i]); s[i].size() < 4 and it != mp_vu[cv - d[i]].end() and it->first < cu + d[i]; ++it) {
            s[i].push_back(it->second);
        }
