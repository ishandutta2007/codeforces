...
void go(int i, int j) {
    while (1) {
        ...
        if (s == 0)
            return;
        int id = -1;
        for (int it = 0; it < kto.size(); it++)
            if (kto[it] == s) {
                id = it;
                break;
            }
        vector<pair<int, int>> g;
        g = {{i, j}, {i + 1, j}, {i, j + 1}, {i + 1, j + 1}};
        for (int t = 0; t < 4; t++) {
            vector<int> o;
...
int main() {
    ...
    while (t--) {
        ...
        if (m % 2 == 1) {
            for (int i = 0; i < n - 2; i++)
                if (a[i][m - 1] == 1)
                    add({i, m - 1, i + 1, m - 1, i, m - 2});
            if (n % 2 == 0) {
                if (a[n - 2][m - 1] == 1)
                    add({n - 2, m - 1, n - 1, m - 1, n - 2, m - 2});
                if (a[n - 1][m - 1] == 1)
