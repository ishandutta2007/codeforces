...
static void solve()
{
    ...
    reverse(RA(s));
    reverse(RA(forbid));
    auto sa = make_suffix_array(vector<string>{s});
    ...
    for (const auto &e : sa)
    {
        srun = min(srun, e.share);
        if (forbid[e.ofs] == '0')
        {
            share.push_back(srun);
            ans = max(ans, ll(N - e.ofs));
            srun = N;
        }
    }
    ...
    for (int i = 0; i < K; i++)
    {
        while (st.top().first >= share[i])
            st.pop();
        prev[i] = st.top().second;
        st.emplace(share[i], i);
    }
    ...
    for (int i = K - 1; i >= 0; i--)
    {
        while (st.top().first >= share[i])
            st.pop();
        ll d = st.top().second - prev[i];
        ans = max(ans, d * share[i]);
        st.emplace(share[i], i);
    }
    cout << ans << '\n';
}
