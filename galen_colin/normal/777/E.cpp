...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		p[i] = make_pair(make_pair(a[i], b[i]), h[i]);
	}
	...
	for (ll i = 0; i < n; i++) {
		ll v = p[i].s;
		if (p[i].f.s > 0) v += st.query(0, p[i].f.s - 1).val;
		dp[i] = v;
		ans = max(ans, dp[i]);
		
		ll at = st.query(p[i].f.f, p[i].f.f).val;
		if (dp[i] > at) st.update(p[i].f.f, p[i].f.f, dp[i] - at); // perform a[p[i].f.f] += dp[i] - at
	}
