...
ll query(int a, int b) {
    if (mem.find(mp(a, b)) != mem.end()) return mem[mp(a, b)];
    cout << "? " << a+1 << ' ' << b+1 << endl;
    ll ans; cin >> ans;
    if (ans==-1) exit(0);
    mem[mp(a, b)]=ans;
...
void solve() {
    ...
    while (l<r) {
        m = (l+r)/2;
        if (query(0, m)==query(0, n-1)) r=m-1;
        else if (query(0, m)==0) l=m+1;
        else break;
    }
    ll diff = query(0, m) - query(0, m-1);
    int i = m - diff;
    diff = query(m, n-1) - query(m+1, n-1);
    int j = m + diff;
    debug(diff, j);
    ll total = query(0, n-1);
    total -= ll(i-j)*(i-j-1)/2;
    ll ans = (round(sqrt(1 + 8*total)) + 1) / 2;
    debug(ans);
    if (query(0, m) > ll(m-i)*(m-i+1)/2) {
        cout << "! " << i-ans+1 << ' ' << i+1 << ' ' << j+1 << nl;
    } else {
        cout << "! " << i+1 << ' ' << j+2 << ' ' << j+ans+1 << nl;
    }
}
// 2 1 5 4 3    ans: 1, 3, 5
// 1 3 2 5 4    ans: 2, 4, 5
