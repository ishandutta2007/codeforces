...
int main() {
    TEST {
        ...
        auto ask = [&](int i, int j) {
            OUT("SCAN", i + 1, j + 1);
            LOCAL { return abs(i - x) + abs(i - xx) + abs(j - y) + abs(j - yy); }
            cout.flush();
            return in();
        };
        auto ans = [&](int i, int j) -> int {
            OUT("DIG", i + 1, j + 1);
            LOCAL { return (i == x and j == y) or (i == xx and j == yy); }
            cout.flush();
            return in();
        };

        int x1 = ask(0, 0), x2 = ask(n - 1, 0);
        int js = (x1 + (x2 - (n - 1) * 2)) / 2;
        int is = (x1 - (x2 - (n - 1) * 2)) / 2;
        int y1 = ask(is / 2, 0), y2 = ask(0, js / 2);
        int i1 = (x1 - y1) / 2;
        int i2 = is - i1;
        int j1 = (x1 - y2) / 2;
        int j2 = js - j1;
        ...
        if(ans(i1, j1)) {
            ...
            assert(ans(i1, j2) and ans(i2, j1));
        }
    }
}