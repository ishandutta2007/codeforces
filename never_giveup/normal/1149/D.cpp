...
const int maxn = 72, maxw = 1e6 + 100, inf = 1e9 + 100, mod = 1e9 + 7;
...
int col[maxn], pol[maxn];

int sz;

bool vis[maxn];

void kekfs(int v, vector<int> &g) {
    vis[v] = 1;
    g.push_back(v);
    for (int i : e[v][0])
        if (!vis[i])
...
int main() {
    ...
    assert(sz <= 17);
    priority_queue<pair<int, pair<int, char> > > g;
    for (int i = 0; i < (1 << sz); i++)
        for (int j = 0; j < n; j++)
            q[i][j] = inf;
    int sosat = (col[0] == -1 ? 0 : (1 << col[0]));
    q[sosat][0] = 0;
    g.push({0, {sosat, 0}});
    while (!g.empty()) {
        int m, v;
        ...
        for (int i : e[v][1]) {
            int &o = col[i], w = sosat + B;
            if (pol[i] != -1 && pol[i] == pol[v])
                continue;
            int m1 = m;
            if (o != -1) {
                if ((1 << o) & m)
