...
ll calc(ll a, ll p){
	return (a / p) * (a / p) * (p - a % p) + (a / p + 1) * (a / p + 1) * (a % p);
}
...
int main()
{
	...
	rep(i, n){
		...
		s += p;
		if (p > 1)
			que.push(mp(calc(a[i], p) - calc(a[i], p - 1), mp(a[i], p)));
	}
	while(s>k){
		auto q = que.top();
		que.pop();
		s--;
		ans -= q.first;
		ll na = q.second.first, np = q.second.second;
		np--;
		if(np>1)
			que.push(mp(calc(na, np) - calc(na, np - 1), mp(na, np)));
	}
	cout << ans << endl;
}