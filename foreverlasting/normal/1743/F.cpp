...
namespace MAIN {
	int n;
	int tr[N<<2];
	void modify(int rt,int l,int r,const int &L,const int &R,const int &v){
		if(L<=l&&r<=R){tr[rt]=v;return;}
		int mid=(l+r)>>1;
	...
	void solve(int rt,int l,int r){
		if(l==r){a[l]=tr[rt];return;}
		tr[rt<<1]=max(tr[rt<<1],tr[rt]);
		tr[rt<<1|1]=max(tr[rt<<1|1],tr[rt]);
		int mid=(l+r)>>1;
	...
	inline void MAIN(const int &Case) {
		...
		for(int i=0;i<=N-10;i++){
			int p=a[i];
			if(p==0)continue;
			if(p==1)ans+=Z{2}.qpow(n-p);
			else ans+=Z{3}.qpow(p-2)*Z{2}.qpow(n-p+1);
		}
		printf("%d\n",ans);
	}
}
