...
ll dist[400005], rec[400005];
ll vx[] = {1, -1, -1, 1}, vy[] = {-1, -1, 1, 1};

void bfs01(vector<edge> G[], int n, int S, ll dist[], ll rec[])
{
	deque<int> deq;
	deq.push_back(S);
	...
	while(T--){
		...
		rep(y, 0, h-1){
			if(!b[y*w+0]){
				ll cost = 1;
				if(c[y*w+0] == '#') cost = 0;
				G[S].pb(edge(y*w+0, cost));
			}
			if(!b[y*w+w-1]){
				ll cost = 1;
				if(c[y*w+w-1] == '#') cost = 0;
				G[y*w+w-1].pb(edge(T, cost));
			}
		}
		
		bfs01(G, T, S, dist, rec);
