...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		...
		if (d >= 0)
		{
			int pos = H.find_first_of([&](node& it, int L, int R)
				{
					return it.val < d;
				});
			freshmin(pos, i + 1);
			if (1 <= pos - 1)
			{
				H.modify(1, pos - 1, [&](node& it, int, int)
					{
						it.val += 1;
						it.tag += 1;
					});
		...
		for (auto [x, id] : ques[i])
		{
			int tmp = H.query(x, x).val;
			res[id - 1] = tmp;
		}
	}
	print(res, "\n");
