...
mod modint {
    ...
    pub trait Modulo {
        ...
        fn ini_2() -> u64 {
            Self::reduce(Self::ini() * Self::ini()) as u64
        }
    }
    ...
    impl<T: Modulo> ModInt<T> {
        ...
        pub fn inv(&self) -> Self {
            assert!(!self.is_zero());
            Self::build(T::reduce(self.pow(T::modulo() as u64 - 2).0 as u64 * T::ini_2()))
        }
    }
    #[allow(dead_code)]
...
fn run() {
    ...
    a.iter().for_each(|a| dp[*a] += 1);
    let mut mo = vec![M::one(); m + 1];
    ...
    let mut ans = M::zero();
    for (dp, mo) in dp.iter().zip(mo.iter()).skip(1) {
        if !mo.is_zero() {
            ans += (M::new(2).pow(*dp as u64) - M::one()) * *mo;
        }
    }
    println!("{}", ans);
}
