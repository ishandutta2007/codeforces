...
int main(){
  ...
  pow2[0] = 1;
  for(li i = 0; i < (li)(N-1); ++i) pow2[i+1] = pow2[i]*2;
  for(li i = 0; i < (li)(N); ++i) ipow2[i] = Zn<>(1)/pow2[i];
  ...
  for(li a = 0; a < (li)(26); ++a) {
    pre1[a][0] = 0;
    for(li i = 0; i < (li)(n); ++i) pre1[a][i+1] = 2*pre1[a][i]+((t[i] == 'a'+a)?1:0);
  }
  ...
  for(li qi = 0; qi < (li)(q); ++qi) {
    ...
    string w; cin>>w;
    Zn<> ans = 0;
    for(li i = 0; i < (li)(ns+1); ++i) {

      string v;
      bool ok = 1;
      int last = 0;
      for(int k = i; k < (int)w.size(); k += ns+1){

        if(k != 0) {
          int sz = k - last;

          if(memcmp(s.data()+ns-sz, w.data()+last, sz) != 0) {
            ok = 0;
            break;
          }
        }
      ...
      { int sz = (int)w.size()-last;

        if(memcmp(last == 0 ? (s.data()+ns-i) : s.data(), w.data()+last, sz) != 0){
          ok = 0;
        }
      }
      if(!ok) continue;

      auto solve = letrec([&](auto solve, int i, string v) -> void {
        if(v.empty()) {
          ans += pow2[sk-i];
        }else if(v.size() == 1) {

          ans += pre1[v[0]-'a'][sk] - pre1[v[0]-'a'][i] * pow2[sk-i];

        }else{
          if(i == sk) return;
          string vs[2];
          for(li j = 0; j < (li)(v.size()); ++j) vs[j&1].push_back(v[j]);
          for(li d = 0; d < (li)(2); ++d) {
            bool ok = 1;
            for(char c : vs[d]) if(c != t[i]) { ok = 0; break; }
            if(!ok) continue;
            solve(i+1, vs[d^1]);
          }
        }
