...
void solve() {
  ...
  const ll INF = 1LL << 60;
  vi nbd(N, INF);
  ll min_e = INF, min_v = -1;
  vv(ll, E, N, N, INF);
  FOR(i, N) FOR3(j, i + 1, N) {
    LL(x);
    E[i][j] = E[j][i] = x;
    if (chmin(min_e, x)) min_v = i;
  }
  FOR(i, N) FOR(j, N) E[i][j] -= min_e;
  FOR(v, N) {
    ll x = MIN(E[v]);
    FOR(to, N) chmin(E[v][to], 2 * x);
  }
  // FOR(v, N) print(E[v]);

  Graph<ll, 1> G(N);
  FOR(i, N) FOR(j, N) if (i != j) G.add(j, i, E[i][j]);
  G.prepare();
  auto [dist, par] = dijkstra(G, min_v);
  FOR(root, N) {
    ll ANS = dist[root] + min_e * (N - 1);
    print(ANS);
  }
}
