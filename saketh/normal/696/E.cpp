...
template<typename T, typename U, bool PROP = true> struct segment_tree_lazy {
    ...
    segment_tree_lazy(int _SZ, TT _tt, UU _uu, UT _ut) : SZ(_SZ), tt(_tt), uu(_uu), ut(_ut) {
        table.resize(2 * SZ), has.resize(SZ), ops.resize(SZ);
    }
...
int main() {
    ...
    struct girl { ll wt = INF; int loc = -1; };
    auto gg = [](girl a, girl b){
        return a.wt < b.wt ? a : b.wt < a.wt ? b : a.loc < b.loc ? a : b;
    };
    auto ug = [](ll u, girl g) { return girl{g.wt+u, g.loc}; };
    segment_tree_lazy<girl, ll> st(N, gg, plus<ll>(), ug);

    vector<priority_queue<int, vi, greater<int>>> at(N);
    F0R (m, M) {
        int cloc; re(cloc); --cloc;
        int ci = hld.index(cloc);
        if (st[ci].loc == -1) {
            st.replace(hld.index(cloc), {m+1, cloc});
        }
        at[cloc].push(m+1);
    }
    ...
    F0R (q, Q) {
        ...
        if (t == 1) {
            ...
            vi res;
            for (; k; k--) {
                girl g = hld.accumulate_commutative(v, u, true, girl{}, gg,
                    [&](int i, int j){ return st(i, j); });
                if (g.loc == -1) break;

                res.pb(at[g.loc].top());
                ll added = g.wt - at[g.loc].top();
                at[g.loc].pop();
                st.replace(hld.index(g.loc), at[g.loc].empty() ? girl{} : girl{at[g.loc].top()+added, g.loc});
            }
