  ...
  cout<<fixed<<setprecision(0);
	...
	for(auto x:a){
		...
		rep(i,m)if(x>>i&1){
			if(!bs[i]){
				bs[i]=x;
				rep(j,m)if(i!=j && (bs[j]>>i&1)) bs[j]^=x;
				f=1;
				break;
			}
		}
	...
	out(inds,bits,1);
	vector<mint> re(m+1);
	const int t=bits.size();
	if(t<M){
		dp[0][0]=1;
		for(auto x:bs)if(x){
				int v=0;
				rep(i,t)if(x>>bits[i]&1) v|=1<<i;
				out(v,1);
				rrep(i,m)rep(j,1<<t) dp[i+1][j^v]+=dp[i][j];
			}
		rep(i,m+1)rep(j,1<<t) re[i+__builtin_popcount(j)]+=dp[i][j];
	}else{
		vector<ll> vs;
		for(auto x:bs)if(x) vs.pb(x);
		const int n=vs.size();
		rep(i,1<<n){
			ll s=0;
			rep(j,n)if(i>>j&1) s^=vs[j];
			++re[__builtin_popcountll(s)];
			out(i,s,1);
		}
	}
	for(auto &x:re) x*=pw;
	out(re,1);
	for(auto &x:re) cout<<x<<" "; cout NL;
  return 0;
}