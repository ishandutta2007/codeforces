...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read();
        res L=2e9,R=-2e9,u=0,v=0,s=2e9;
        for(res i=1;i<=n;i++){
            res l=read(),r=read(),c=read();
            if(L>l)u=c,L=l,s=2e9;
            else if(L==l)u=min(u,c);
            if(r>R)v=c,R=r,s=2e9;
            else if(r==R)v=min(v,c);
            if(L==l&&r==R)s=min(s,c);
            printf("%d\n",min(u+v,s));
        }
    }
}
