...
const int perc = inverse(100, P);
int fd(const int x, const int y, const vi& S, const vi& T, const vi& pp)
{
	int g = prod(raz(raz(T[y], T[x], P), prod(x, raz(S[y], S[x], P), P), P), inverse(pp[x], P), P);
	return prod(g, prod(pp[x], inverse(pp[y], P), P), P);
}
...
int another_main()
{
	...
	vi pp(n + 1, 1);
	for (int i = 0; i < n; ++i)
		pp[i + 1] = prod(pp[i], p[i], P);
	vi S(n + 1), T(n + 1);
	for (int i = 0; i < n; ++i)
	{
		S[i + 1] = sum(S[i], prod(pp[i], Q[i], P), P);
		T[i + 1] = sum(T[i], prod(prod(pp[i], Q[i], P), (i + 1), P), P);
	}
	vi u(q);
	cin >> u;
	--u;
	...
	for (int I = 0; I < q; ++I)
	{
		...
		if (it == s.end())
		{
			...
			++it; ++it;
			int x3 = *it;
			ans = sum(ans, fd(x1, x2, S, T, pp), P);
			ans = sum(ans, fd(x2, x3, S, T, pp), P);
			ans = raz(ans, fd(x1, x3, S, T, pp), P);
		}
		else
		{
			int x2 = *it;
			...
			--it;
			s.erase(it);
			ans = raz(ans, fd(x1, x2, S, T, pp), P);
			ans = raz(ans, fd(x2, x3, S, T, pp), P);
			ans = sum(ans, fd(x1, x3, S, T, pp), P);
		}
		cout << ans << '\n';
	}
	return 0;
