...
void solve() {
	...
	for (int i = 0; i <= n - i - 1; ++i)
		for (int j = 0; j <= m - j - 1; ++j) {
			vpii wh = {{i, j}, {n - i - 1, j}, {i, m - j - 1}, {n - i - 1, m - j - 1}};
			sort(ALL(wh));
			unq(wh);
			vi ord;
			for (auto [x, y] : wh) ord.push_back(a[x][y]);
			sort(ALL(ord));
			int med = ord[ord.size() / 2];
			debug(imie(wh), imie(ord), imie(med));
			for (int c : ord) ans += abs(c - med);
		}
	printf("%lld\n", ans);
}
int main() {
