...
int main() {
	...
	for (int cas = 1; cas <= T; ++cas) {
		...
		std::sort(all(c[0]));
		std::sort(all(c[1]));
		int u = c[0].size(), v = c[1].size();
		if (u && v) {
			int x = std::min(u - 1, v), y = std::min(u, v);
			i64 t0 = std::accumulate(c[0].end() - x, c[0].end(), 0ll) + std::accumulate(c[1].end() - y, c[1].end(), 0ll);
			x = std::min(u, v);
			y = std::min(u, v - 1);
			i64 t1 = std::accumulate(c[0].end() - x, c[0].end(), 0ll) + std::accumulate(c[1].end() - y, c[1].end(), 0ll);
			ans += std::max(t0, t1);
		}
		std::cout << ans << '\n';
	}
