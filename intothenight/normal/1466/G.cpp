...
int main(){
	...
	auto pow2 = precalc_power(Zp(2), 2000000);
	vector<int> width(31);
	...
	string s, t;
	cin >> n >> qn >> s >> t;
	array<vector<Zp>, 26> pref;
	for(auto c = 0; c < 26; ++ c){
		pref[c].resize(n + 1);
		for(auto i = 0; i < n; ++ i){
			pref[c][i + 1] = pref[c][i] * 2 + (c == t[i] - 'a');
		}
	}
	int slen = (int)s.size();
	for(auto qi = 0; qi < qn; ++ qi){
		...
		debug(k, w);
		-- k;
		vector<int> ok(slen + 1, true);
		for(auto r = 0; r <= slen; ++ r){
		...
		debug(ok);
		Zp res = 0;
		for(auto i = 0; i <= slen; ++ i) if(ok[i]){
			if(i < (int)w.size()){
				string c;
				for(auto j = i; j < (int)w.size(); j += slen + 1){
					c.push_back(w[j]);
				}
				debug("non-empty", i, c);
				int m = (int)c.size();
				function<void(int, int)> solve = [&](int i, int h){
					// startpos, height
					if(m <= i + width[h]){
						if(h <= k){
							debug(i, c[i], h, k);
							debug(pref[c[i] - 'a'][k + 1] - pref[c[i] - 'a'][h] * pow2[k - h + 1]);
							res += pref[c[i] - 'a'][k + 1] - pref[c[i] - 'a'][h] * pow2[k - h + 1];
						}
						return;
					}
					for(auto r = 0; r < 2; ++ r){
						bool ok = true;
						for(auto j = i + r * width[h]; j < m; j += width[h] << 1){
							if(t[h] != c[j]){
								ok = false;
								break;
							}
