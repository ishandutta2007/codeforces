...
void solve() {
  ...
  FOR_(N) {
    ...
    vc<mint> newDP(K + 1);
    FOR(a, K) FOR3(b, a + 1, K + 1) {
      newDP[b] += DP[a] * fact_inv<mint>(b - a);
    }
    swap(DP, newDP);
    print(DP);
  }
  */
  ...
  FOR3(n, N + 1, K + 1) {
    ...
    c1 *= mint(2).pow(n);
    mint lhs = 0, rhs = 0;
    FOR3(k, N, n) lhs += f[k] * q[n + 1 - k];
    FOR3(k, N, n) rhs += g[k] * p[n + 1 - k];
    f[n] = (rhs - lhs) / (c0 - c1);
    g[n] = f[n] * mint(2).pow(n);
  }

  FOR(k, K + 1) f[k] *= fact_inv<mint>(K - k);

  print(SUM(f) * fact<mint>(K));
}
