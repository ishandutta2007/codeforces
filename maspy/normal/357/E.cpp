...
void solve() {
  LL(N);
  VEC(ll, A, N);
  vi CNT = bin_count(A, 5);
  vc<pi> X;
  FOR(i, 5) X.eb(i, CNT[i]);

  auto calc = [&](vc<pi> X, vc<pi> Y) -> ll {
    ll ANS = 0;
    while (1) {
      // 0 
      while (!X.empty() && X.back().se == 0) X.pop_back();
      while (!Y.empty() && Y.back().se == 0) Y.pop_back();
      if (X.empty()) break;
      assert(!Y.empty());
      auto [x, nx] = X.back();
      auto [y, ny] = Y.back();
  ...
  FOR(p, N + 1) {
    ll q = (total - 3 * p) / 4;
    if (q < 0 || p + q > N || 3 * p + 4 * q != total) continue;
    vc<pi> Y = {{0, N - p - q}, {1, 0}, {2, 0}, {3, p}, {4, q}};
    chmin(ANS, calc(X, Y));
  }
  if (ANS == INF) ANS = -1;
  print(ANS);
