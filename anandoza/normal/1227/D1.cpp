...
int main() {
    ...
    vvi queries_by_k(n + 1);
    for (int i = 0; i < m; i++)
        queries_by_k[queries[i].first].push_back(i);

    binary_indexed_tree bit(n, 0, plus<int>());

    vi ans(m);
    for (int k = 1; k <= n; k++) {
        bit.add(indices[k - 1], 1);

        for (int i : queries_by_k[k]) {
            int index = -1 + bit.binary_search(L1(u, u > queries[i].second));
            ans[i] = a[index];
        }
    }
