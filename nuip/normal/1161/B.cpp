...
void kmp(const vector<int> &str,vector<int> &re){
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	out(ps,1);
	int g=n;
	rep(i,n)if(ps[i].size()){
		auto &v=ps[i];
		if(v.size()==1){ g=1; break;}
		sort(all(v));
		int m=v.size();
		vector<int> dif{v[0]+n-v.back()};
		reps(i,1,m) dif.pb(v[i]-v[i-1]);
		vector<int> km;
		kmp(dif,km);
		out(dif,km,1);
		if(m%(m-km[m])){ g=1; break;}
		out(m/(m-km[m]),1);
		g=__gcd(m/(m-km[m]),g);
	}
