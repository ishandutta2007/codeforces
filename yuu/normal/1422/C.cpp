...
public:
    ...
    result_t(int64_t sum = 0, int64_t cnt = 0) : sum(sum), cnt(cnt) {}
    ...
    result_t add_digit(int d) { return result_t((sum * 10 + cnt * d) % base, cnt); }
};
result_t f[100001][3];
void solve() {
    cin >> s;
    ...
    for (int i = 0; i < n; i++) {
        f[i + 1][0] += f[i][0].add_digit(s[i]);
        f[i + 1][1] += f[i][0];
        f[i + 1][1] += f[i][1];
        f[i + 1][2] += f[i][1].add_digit(s[i]);
        f[i + 1][2] += f[i][2].add_digit(s[i]);
    }
    cout << (f[n][2].sum + f[n][1].sum) % base << '\n';
}
