...
int main(void){
  ...
  long long de[524288]={0};
  long long p[524288];
  long long mi=0,ma=0;
  rs g[524288],rg[524288];
  mkj x[524288],rx[524288];
  scanf("%lld%lld",&n,&m);
  getperm(p,m);
  for(i=0;i<m;i++){
    scanf("%lld%lld",&g[p[i]].st,&g[p[i]].fi);
    g[p[i]].kr=1;
    rg[p[i]]=g[p[i]];
    llswap(&rg[p[i]].st,&rg[p[i]].fi);
  }
  ...
  dijkstra(p[k-1],n,dist,rg,rx);
  //for(i=1;i<=n;i++){printf("%lld : %lld\n",i,dist[i]);}
  for(i=0;i<m;i++){
    if(dist[g[i].st]-1==dist[g[i].fi]){
      de[g[i].st]++;
    }
  }
  for(i=0;i<k-1;i++){
    if(dist[p[i]]-1!=dist[p[i+1]]){mi++;ma++;}
    else if(de[p[i]]!=1){ma++;}
  }
  printf("%lld %lld\n",mi,ma);
  return 0;
