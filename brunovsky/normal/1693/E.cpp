...
int main() {
    ...
    int64_t ans = 0;
    Splay* Ls = nullptr; // we know the left
    Splay* Rs = nullptr; // we know the right
    Splay* Es = nullptr; // we don't know left nor right
    for (int v = N; v >= 1; v--) {
        if (pos[v].empty()) {
            continue;
        }
...
// based on ecnerwala's sub
// editorial says you just greedy
// reduce the numbers without figuring out what they become, do that later