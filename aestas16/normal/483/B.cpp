...
int n1, n2, p1, p2, l, r, ans;

int Chk(int x) {
    int tmp = x / (p1 * p2), tmp1 = cmin(n1, x / p2 - tmp), tmp2 = cmin(n2, x / p1 - tmp);
    int _n1 = n1 - tmp1, _n2 = n2 - tmp2;
    return x - x / p1 - x / p2 + tmp >= _n1 + _n2;
}
...
signed main() {
    ...
    cin >> n1 >> n2 >> p1 >> p2, l = n1+n2, r = 1e18;
    while (l <= r) {
        int mid = (l + r) >> 1;
        if (Chk(mid))
            ans = mid, r = mid - 1;
