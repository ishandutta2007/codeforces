...
void Solve1(){
	Forx(x,xa,xc) cl[x][ya] = 1;
	Forx(y,min(ya,yb),max(ya,yb)) cl[xc][y] = 1;
	Forx(x,xc,xb) cl[x][yb] = 1;
}
void Solve2(){
	if( xa <= xc && xc <= xb ){
		Forx(y,max(ya,yb),yc) cl[xc][y] = 1;
		Forx(y,yc,min(ya,yb)) cl[xc][y] = 1;
		yc = yc<min(ya,yb) ? min(ya,yb) : max(ya,yb);
	}else{
		if( min(ya,yb) <= yc && yc <= max(ya,yb) ){
			Forx(x,xc,xa) cl[x][yc] = 1;
			Forx(x,xb,xc) cl[x][yc] = 1;
			xc = xc<xa ? xa : xb;
		}else{
			Forx(x,xc,xa) cl[x][yc] = 1;
			Forx(x,xb,xc) cl[x][yc] = 1;
...
int main(){
	...
	if( xa <= xc && xc <= xb && min(ya,yb) <= yc && yc <= max(ya,yb) ){
		Solve1();
	}else{
		Solve2();
	}
