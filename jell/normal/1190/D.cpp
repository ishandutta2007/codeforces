...
signed main() {
    ...
    rep(i,n) {
        cin>>p[i].fir>>p[i].sec;
        xv.emplace_back(p[i].fir);
        yv.emplace_back(p[i].sec);
    }
    xv=cmprs(xv);
    yv=cmprs(yv);
    rep(i,n) {
    ...
    Segtree<int,int> sg(n,0,plus<int>(),[](int &x,int y){x=y;});
    
    for(int i=0,j=0; i<n; i=j) {
        while(j<n && p[i].sec==p[j].sec) {
            ++j;
        }
        i64 tot=sg.query(0,p[i].fir)+1;
        for(int k=i; k<j; ++k) {
            int r=n;
            if(k+1<j) {
                r=p[k+1].fir;
            }
            i64 crt=sg.query(p[k].fir+1,r);
            ans+=tot*(crt+1);
            tot+=crt+1;
        }
        for(int k=i; k<j; ++k) {
