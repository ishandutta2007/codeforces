...
void q() {
  ...
  vi a(N);
  in(a);
  V<pair<int, int>> p = RunLengthEncoding(a);
  rep(_, N - 1) {
    V<pair<ll, int>> q;
    vi v;
    rep(i, sz(p) - 1) v.emplace_back(p[i + 1].fi - p[i].fi);
    sort(all(v));
    auto r = RunLengthEncoding(v);
    ll z = 0;
    rep(i, sz(p)) z += p[i].se - 1;
    if (z) r.insert(begin(r), mkp(0, z));
    p = r;
    trc(r);
  }
  out(p[0].fi);
