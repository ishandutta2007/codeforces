...
void unite(int u,int v){
    ...
    if (pu==pv) return;
    if (sz(g[pv])<sz(g[pu])) swap(pu,pv);
    p[pu]=pv, a[pv]+=a[pu], a[pv]-=x, s.insert({-a[pv],pv});
    g[pv].insert(range(g[pu]));
}
...
signed main(){
    ...
    cin>>n>>m>>x;
    int sum=0;
    rep1(i,n) cin>>a[i], sum+=a[i], s.insert({-a[i],i});
    rep1(i,n) p[i]=i;
    if (sum<(n-1)*x){
        cout<<"NO\n";
        return 0;
    ...
    rep1(i,n-1){
        ...
        int pu=par(idx);
        while (1){
            auto [nxt,iid]=*(g[pu].begin());
            int pv=par(nxt);
            if (pv==pu) {g[pu].erase({nxt,iid}); continue;}
            s.erase({-a[pv],pv});
            cout<<iid<<" ";
            unite(idx,nxt);
            break;
