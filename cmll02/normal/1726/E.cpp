...
// URL: https://codeforces.ml/contest/1726/problem/E
// Memory Limit: 256 MB
// Time Limit: 3000 ms
// 
...
signed main()
{
	initprog();
	a[0]=1;
	rg(1000000)a[i]=ae(a[i-1],me(a[i-2],i-1));
	gr
	int T=read();rg(T)
	int n=read(),ans=0,d=1;
	rrg(n/4+1)
	mae(ans,me(C(n-2*i,2*i),me(me(d,qp(2,i)),a[n-4*i])));
	// printf("(%d)",me(C(n-2*i,2*i),me(d,a[n-4*i])));
	mle(d,i*2+1);
	gr
	odl(ans);
	gr
