...
//      val = (uint64_t)val * other.val % mod;
  ...
  constexpr static ModNum primitive_root() {
    ...
    for (int root = 2; ; ++root) {
      bool bad = 0;
      for (auto p: range{primes.begin(), cprimes}) if (bad |= fpow(ModNum{root}, mod / p).val == 1) {
        break;
      }
      if (!bad) {
        return root;
...
struct NTT {
  void ntt(Poly& a, bool invert) {
    constexpr auto primitive = ModNum::primitive_root();
    constexpr int root_pw = mod - 1 & 1 - mod;
    constexpr auto root = fpow(primitive, (mod - 1) / root_pw), iroot = fpow(root, root_pw - 1);
    int n = a.size();
  ...
  auto stirling(int n, int k = -1) {
    if (k == -1) {
      k = n;
    }
    Poly a(k + 1), b(k + 1, 1);
    for (int i = 1; i <= k; ++i) {
      a[i] = fpow(ModNum{i}, n) * fact(i);
      b[i] = i & 1? -fact(i): fact(i);
    }
    return prod(a, b, k + 1);
  }
} ntt;
...
int main() {
  int n = $, k = $;
  for (int i = 1; i; i = i == n? 0: (n - 1) / ((n - 1) / i) + 1) {
    auto v = ntt.stirling(i, min(i, k));
    a[i] = accumulate(v.begin() + 2, v.end(), 0m);
  }
  for (int i = n; i; --i) {
    b[i] = a[(n - 1) / i + 1];
