...
int Solve(){
	...
	a = " " + a;
	b = " " + b;
	static int aSS[MAXN][26], bSS[MAXN][26];
	Ms(aSS); Ms(bSS);
	fOR(i,n){
		memcpy(aSS[i],aSS[i+1],sizeof aSS[i]);
		memcpy(bSS[i],bSS[i+1],sizeof bSS[i]);
		++aSS[i][a[i]-'a'];
		++bSS[i][b[i]-'a'];
	}

	static int mem[MAXN][MAXN];
	Forx(i,0,n) Forx(j,0,n) mem[i][j] = -1;
	#define NOW mem[sPos][tPos]
	function<int(int,int)> Dfs = [&]( int sPos , int tPos ){
		// P(sPos,tPos);
		assert( sPos <= tPos );
		assert( 0 <= sPos and tPos <= n );
		if( sPos == 0 ) return 0;
		if(~NOW) return NOW;
		const int x = b[tPos] - 'a';
		int ans = Dfs(sPos-1,tPos) + 1;
		if( a[sPos] == b[tPos] )
			Mymin( ans , Dfs(sPos-1,tPos-1) );
		if( aSS[sPos+1][x] > bSS[tPos+1][x] )
			Mymin( ans , Dfs(sPos,tPos-1) );
		return NOW = ans;
	};
	int ans = Dfs(n,n);
	return ans;
