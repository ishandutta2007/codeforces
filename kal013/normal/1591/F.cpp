...
void solve(){
	...
	vector<array<mint, 2>> pref(n + 1);
	dp[0] = {1, 0};
	pref[0] = {1, 0};


	auto get_sum = [&] (int l,int r){
		array<mint, 2> tmp = pref[r];
		if (l > 0){
			tmp[0] -= pref[l - 1][0];
			tmp[1] -= pref[l - 1][1];
		}
	...
	vector<int> stk;
	vector<array<mint, 2>> cum_dp;

	array<mint, 2> cum_sum = {0, 0};
	for(int i = 1; i <= n; ++i){
...

		{
			...
			stk.push_back(i - 1);
			array<mint, 2> tmp = get_sum(lo, i - 1);
			tmp[0] *= a[i - 1]; tmp[1] *= a[i - 1];
			cum_sum[0] += tmp[0]; cum_sum[1] += tmp[1];
			cum_dp.push_back(tmp);
		}

		// trace(stk, cum_sum, cum_dp, get_sum(1, 1));
		


		// int v = INT_MAX;
		// for(int j = i - 1, l = 0; j >= 0; --j, l ^= 1){
		// 	v = min(v, a[j]);
		// 	for(int k = 0; k < 2; ++k){
		// 		dp[i][k] += dp[j][k ^ l] * v;
		// 	}
		...
		for(int k = 0; k < 2; ++k){
			dp[i][k ^ 1 ^ (i & 1)] = cum_sum[k];
		}
		for(int k = 0; k < 2; ++k){
			pref[i][k] = pref[i - 1][k] + dp[i][k ^ (i & 1)];
		}
	}
	// trace(dp, pref);
