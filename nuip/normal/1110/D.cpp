...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	int n,m;
	// n=1000000; m=1000000;
	// vector<int> cnt(m,1); cnt.resize(m+3);
	cin>>n>>m;
	vector<int> cnt(m+3);
	rep(i,n){
		int x;
	...
	for(int x:cnt){
		fill(nxt[0],nxt[M],-1);
		rep(i,M)rep(j,M)if(cur[i][j]!=-1){
			int t=min({i,j,x});
			rep(k,t+1) MX(nxt[j-k][x-k],cur[i][j]+k+(i-k)/3);
		}
		swap(cur,nxt);
		out(x,1);
		// rep(i,M){rep(j,M) cout<<cur[i][j]<<",";cout<<endl;}cout<<endl;
	}
	out(base,1);
	cout<<*max_element(cur[0],cur[M])+base<<endl;
  return 0;
}