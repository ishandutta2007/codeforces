...
void solve() {
  ...
  // idx, before, after
  using CHANGE = tuple<int, int, int>;
  Mo_3d<CHANGE> mo;
  vi B = A;
  FOR(q, Q) {
    auto [a, b, c] = query[q];
    if (a == 1) {
      ...
      mo.add_change(CHANGE({b, B[b], c}));
      B[b] = c;
    }
  }

  vi ANS(len(mo));
  vc<int> CNT_VAL(len(X));
  vc<int> CNT_MEX(N + 10);
  CNT_MEX[0] = N + 10;
  ...
  auto ADD_CHANGE = [&](CHANGE x, int l, int r) -> void {
    auto [i, b, c] = x;
    assert(A[i] == b);
    A[i] = c;
    if (l <= i && i < r) {
  ...
  auto RM_CHANGE = [&](CHANGE x, int l, int r) -> void {
    auto [i, c, b] = x;
    assert(A[i] == b);
    A[i] = c;
    if (l <= i && i < r) {
  ...
  auto CALC = [&](int qid) -> void {
    int mex = 0;
    while (CNT_MEX[mex]) ++mex;
    ANS[qid] = mex;
  };

  mo.calc(ADD, ADD, RM, RM, ADD_CHANGE, RM_CHANGE, CALC, 5000);
  for (auto&& ans: ANS) print(ans);
}
