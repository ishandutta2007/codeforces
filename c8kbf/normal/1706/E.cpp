...
_ n, m, k, x, y, fd[maxn], rp[maxn], t[maxn<<2], h[maxn], f[maxn][21], lg2[maxn];;
v_ g[maxn];
queue<node> q;
...
inline void AC() {
    ...
    read({n, m, k});
    for(_ i = 1; i <= n+m+2; ++i) g[i].clear();
    for(_ i = 1; i <= n; ++i) fd[i] = rp[i] = i;
    for(_ i = n+1; i <= n+m; ++i) {
        x = find(read());
        y = find(read());
        if(x == y) continue;
    ...
    for(; k--; ) {
        ...
        y = read();
        if(x == y) writesc(0);
        else writesc(query(min(x, y), max(x, y), 1, n, 1)-n);
    }
    clr();
...
_ query(_ L, _ R, _ l, _ r, _ pos) {
    ...
    _ md = (l+r)>>1;
    if(L <= md && R > md) return lca(query(L, R, l, md, pos<<1), query(L, R, md+1, r, pos<<1|1));
