...
ll count_inversions(const vector<int> &p) {
    const int n = sz(p);
    binary_indexed_tree bit(n, 0LL, plus<ll>(), minus<ll>());

    ll ans = 0;
    for (int i : p) {
        ans += bit.accumulate(i, n);
        bit.add(i, 1);
    }
...
vector<int> get_permutation(const vi &a, const vi &b) {
    const int n = sz(a);
...
vi transform(const string &s) {
    vi a;
    for (char c : s)
        a.push_back(transform(c));
    return a;
}
...
void solve() {
    ...
    auto slow = [&]() {
        ...
        do {
            auto p = get_permutation(a, b);
            ckmax(ans, {count_inversions(p), transform(b)});
