...
signed main() {
    ...
    int n, m;
    read(n, m);
    vector<vector<ll>> sh(n, vector<ll>(n, inf));
    ...
    for (int i = 0; i < n; ++i) chmin(sh[i][i], 0);

    for (int k = 0; k < n; ++k)
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j)
                chmin(sh[i][j], sh[i][k] + sh[k][j]);
    vector<vector<pii>> triplet(n);
    int q;
    read(q);
    for (int i = 0; i < q; ++i) {
        int u, v, l;
    ...
    for (int a = 0; a < n; ++a)
        for (int b = 0; b < n; ++b) {
            for (auto t : triplet[a]) {
                int d = t.s;
                chmax(mx[b][d], t.f - sh[a][b]);
            }
        }
