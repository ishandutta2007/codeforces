...
struct custom_hashes {
    ...
    size_t operator() (const pair<T1, T2>& x) const {
        ...
        static const uint64_t FIXED_RANDOM = chrono::steady_clock::now().time_since_epoch().count();
        return splitmix64(x.fi^splitmix64(x.se+FIXED_RANDOM));
    }
};
...
namespace NT{
    ...
    constexpr int MAX_SIEVE_VALUE=5e6;
    bitset<MAX_SIEVE_VALUE+1> isprime;
    bool _init=0;
    inline void init() { //sieve for small primes
        _init=1;
        #define n MAX_SIEVE_VALUE
        isprime.set();
    ...
    template<typename T>
    typename enable_if<is_integral<T>::value, bool>::type is_prime(const T& n) {
        if(n<2) return 0;
        if(!_init) init();
...
void gen(const vll& factors, const vi& exponents, vll& ans, int l, int r){ //helper function for generating factors set from factorization
    if(l==r) {
        ll tmp=1;
        rep(i, exponents[l]+1) {
            ans.pb(tmp);
...
vll get_factors(ll n) { //get a vector of all factors
    ...
    vll f; vi d;
    for(auto& v: m) {
        if(f.empty()||f.back()!=v) f.pb(v), d.pb(1);
        else d.back()++;
    }
...
ll root(ll n, int k){
    double t=powl(n, 1./k);
    if(abs(t-llround(t))>1e-6) return -1;
    else return llround(t);
}

safe_map<ll, int> pw;
void init_pw() {
    rep1(i, 10000) if(is_prime(i)) {
        ll p=1; while(p<=1e12/i) {
            p*=i; pw[p+1]=i;
        }
    }
}

optional<ll> check(ll n) {
    if(is_prime(n-1)) return n-1;
    if(pw.find(n)!=pw.end()) return pw[n];
    ll x=root(n-1, 2);
    if(~x&&is_prime(x)) return x;
    return {};
}

safe_map<pll, ll> f;
ll get(ll n, ll mx=LLONG_MAX) {
    if(mx>n) mx=n+1;
    if(n==1) return 1;
    if(mx==1) return 0;
    if(f.find({n, mx})!=f.end()) return f[{n, mx}];
    vll v=get_factors(n);
    ll ans=0;
    for(auto& x: v) {
        auto p=check(x);
        if(p.has_value()&&*p<=mx) ans+=get(n/x, *p-1);
    }
    return f[{n, mx}]=ans;
}
