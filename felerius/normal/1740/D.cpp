// Knowledge Cards (https://codeforces.com/contest/1740/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        bool pos = true;
        repr(i, k, 1) {
            while (scratch.find(i) == end(scratch))
                scratch.emplace(a.back()), a.pop_back();
            pos &= sz(scratch) <= n * m - 3;
            scratch.erase(i);
        }

        println(pos ? "YA" : "TIDAK");
    }
}