...
void solve(istream& cin, ostream& cout) {
    ...
    for (int i = 0; i < N; ++i) {
        for (int v:E[i]) {
            F[Enter[i]].push_back(Enter[v]);
        }
    }

    cout << N << '\n';
    int Z = min(N, (int)sqrt(N * 20));
    for (int i = 2; i <= Z; ++i) cout << solve(i) << '\n';

    for (int i = Z+1; i <= N; ) {
        if (Ans[i] == N+1) Ans[i] = solve(i);
        int r = i;
        while (r <= N && Ans[r] >= Ans[i]) ++r;
        --r;
        int l = r;
        while (Ans[l] != Ans[i]) --l;

        int h = Ans[r] == Ans[i] ? r : bsh(l, r, [&](int q) {
            if (Ans[q] == N+1) Ans[q] = solve(q);
            return Ans[q] == Ans[i];
        });

        for (int j = i; j <= h; ++j) cout << Ans[i] << '\n';
        i = h + 1;
    }
}
};
