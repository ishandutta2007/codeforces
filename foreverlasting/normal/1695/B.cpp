...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        if(n&1)puts("Mike");
            ...
            res mn=inf,Mn=inf;
            for(res i=1;i<=n;i+=2)mn=min(mn,a[i]);
            for(res i=2;i<=n;i+=2)Mn=min(Mn,a[i]);
            if(mn<Mn)puts("Joe");
            else if(mn>Mn)puts("Mike");
            else {
                res id=0,Id=0;
                for(res i=1;i<=n;i+=2)if(mn==a[i]){id=i;break;}
                for(res i=2;i<=n;i+=2)if(Mn==a[i]){Id=i;break;}
                if(id<Id)puts("Joe");
                else puts("Mike");
            }
        }
