#line 1 "codeforces-workspace\\contest\\1562\\e\\e.cpp"
...
#line 16 "codeforces-workspace\\contest\\1562\\e\\e.cpp"
//*/
...
#line 1 "Library\\src\\data_structure\\disjoint_sparse_table.hpp"
/**
 * @file disjoint_sparse_table.hpp
 * @brief Disjoint Sparse Table
 */

#line 8 "Library\\src\\data_structure\\disjoint_sparse_table.hpp"
...
class segment_tree {
 ...
 public:
  ...
  template <class... _Args> reference emplace_back(_Args &&...__args) noexcept {
    if (size() != capacity())
      return operator[](__size++) = value_type{std::forward<_Args>(__args)...};
  ...
  value_type fold(size_type first, size_type last) {
    ...
    // first += capacity(), last += capacity() - 1;
    // _Monoid left{}, right{};
   ...
   * @return Minimum left (>= 0) s.t. [left, right) satisfies the condition.
   */
   ...
   * @param __pred Predicate in the form of 'bool(_Monoid)'
   * @return Maximum right (<= size) s.t. [left, right) satisfies the condition.
   */
  template <class _Pred>
...
#line 2 "Library\\src\\string\\suffix_array.hpp"
...
#line 11 "Library\\src\\string\\suffix_array.hpp"

#line 13 "Library\\src\\string\\suffix_array.hpp"
...
template <class str_type> class suffix_array {
 ...
 public:
  ...
  size_t find(const str_type &key) const {
    ...
    size_t lower{npos}, upper{sa.size()};
    while (upper - lower > 1) {
...
#line 35 "codeforces-workspace\\contest\\1562\\e\\e.cpp"
...
void main() {
  ...
  struct mono {
    size_t v = 1 << 30;
    mono operator+(mono x) const { return {std::min(v, x.v)}; }
  };

  vector<mono> lcp;
  for (auto &&x : sa.lcp_array()) {
    lcp.push_back({x});
  }
  // segment_tree<mono> sgt(begin(lcp), end(lcp));
  disjoint_sparse_table<mono> dst(lcp);

  vector<i32> rank(s.size());
  for (auto i : range(s.size())) {
    rank[sa[i]] = i;
  }

  vector<i32> dp(s.size());

  for (auto i : range(s.size())) {
    vector<i32> acc(s.size() - i + 1);
    for (auto j : range(i)) {
      if (rank[j] < rank[i]) {
        // auto len = sgt.fold(rank[j] + 1, rank[i] + 1).v;
        auto len = dst.fold(rank[j] + 1, rank[i] + 1).v;
        if (len + 1 < acc.size()) chgr(acc[len + 1], dp[j] + 1);
      }
    }
    for (auto j : range(acc.size() - 1)) {
      chgr(acc[j + 1], acc[j] + 1);
    }
    dp[i] = acc.back();
  }
