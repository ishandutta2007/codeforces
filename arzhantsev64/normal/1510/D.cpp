...
vector<vector<int>> bydig(10);

pair<ld, vector<int>> ans = {-1, {}};
vector<int> deleted;
int n, d;

int mpow(int d, int n) {
    int ans = 1;
    while (n > 0) {
        if (n & 1) ans = (ans * d) % 10;
        d = (d * d) % 10;
...
void rec(int i, ld sum, int currd) {
    currd %= 10;
    if (i == sz(bydig)) {
        if (currd == d && sz(deleted) < n) {
            dbg(deleted, currd);
            dbg(sum);
            chmax(ans, make_pair(sum, deleted));
        }
        return;
    }
    rec(i + 1, sum, currd * mpow(i, sz(bydig[i])));
    for (int cnt = 1; cnt < 4 && cnt <= sz(bydig[i]); ++cnt) {
        deleted.pb(bydig[i][cnt - 1]);
        sum -= log2(deleted.back());
        rec(i + 1, sum, currd * mpow(i, sz(bydig[i]) - cnt));
    }
    for (int cnt = 1; cnt < 4 && cnt <= sz(bydig[i]); ++cnt) {
        deleted.popb();
    }
...
signed main() {
    ...
    if (d % 5 != 0) {
        for (auto i : bydig[0]) st.erase(st.find(i));
        bydig[0].clear();
        for (auto i : bydig[5]) st.erase(st.find(i));
        bydig[5].clear();
    }
    if (d % 2 != 0) {
        for (auto i : bydig[0]) st.erase(st.find(i));
        bydig[0].clear();
        for (auto i : bydig[2]) st.erase(st.find(i));
        bydig[2].clear();
        for (auto i : bydig[4]) st.erase(st.find(i));
        bydig[4].clear();
        for (auto i : bydig[6]) st.erase(st.find(i));
        bydig[6].clear();
        for (auto i : bydig[8]) st.erase(st.find(i));
        bydig[8].clear();
    }
    n = 0;
    for (auto i : bydig) n += sz(i);
    for (auto &i : bydig) sort(i);
    ld sum = 0;
    for (auto i : bydig)
        for (auto j : i)
            sum += log2(j);
    ...
    for (auto i : ans.s) st.erase(st.find(i));
    if (sz(st) == 0) {
        println(-1);
        return 0;
    }
