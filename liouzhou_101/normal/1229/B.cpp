...
void dfs(int x)
{
	g[x][0] = __gcd(a[x], a[pre[x][0]]);
	for (int k = 1; k < MAXK; ++ k)
	{
		pre[x][k] = pre[pre[x][k-1]][k-1];
		g[x][k] = __gcd(g[x][k-1], g[pre[x][k-1]][k-1]);
	}
	...
	while (cur)
	{
		...
		{
			if (d == 0 && g[cur][k] == 0 || d != 0 && g[cur][k]%d == 0) cur = pre[cur][k];
		}
		ans = (ans+u64(dis[tmp]-dis[cur]+1)*d)%MOD;
		//printf("%d %lld\n", x, u64(dis[tmp]-dis[cur]+1)*d);
		cur = pre[cur][0];
		d = __gcd(d, a[cur]);
	}
