...
cin>>T;while(T--)
{
    ...
    auto solve=[&](lli up,lli down,bool invert)->vi{
        if(up-1>down)
            return {-1};
        vi a={1};
        for(int i=2;i<=2*up+1;i++)
            a.pb(i^1);
        if(sz(a)>n)
            return {-1};
        if(up==down){
    ...
    vi a;
    if(up>=down)
        a=solve(up,down,false);
    else
        a=solve(down,up,true);

    // assert(a[0]==-1||mp(up,down)==chk(a));
    for(auto x:a)
        cout<<x<<" ";
    cout<<endl;
}   aryanc403();
