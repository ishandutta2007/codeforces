                ...
                for(auto [dx1,dy1]:moves){
                    ...
                    if(to_x1>=0&&to_y1>=0){
                        for(auto [dx2,dy2]:moves){
                            int to_x2=x2+dx2;
                            int to_y2=y2+dy2;
                            if(to_x2<n&&to_y2<n&&b[to_x1][to_y1]==b[to_x2][to_y2]&&!dp[cur][to_x1][to_y1][to_x2][to_y2]){
            ...
            for(int y1=0;y1<n;++y1){
                for(int x2=x1;x2<n;++x2){
                    for(int y2=y1;y2<n;++y2){
                        if(x1+y1+2<=x2+y2&&dp[0][x1][y1][x2][y2]!=dp[1][x1][y1][x2][y2]){
                            int ans=(ask(x1,y1,x2,y2)==dp[1][x1][y1][x2][y2]);
                            cout<<"!"<<endl;
                            for(int i=0;i<n;++i){
                                for(int j=0;j<n;++j){
