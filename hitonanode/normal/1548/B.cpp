...
lint op(lint x, lint y) { return gcd(x, y); }
lint id(lint x) { return x; }
int solve() {
    int N;
    cin >> N;
    vector<lint> A(N);
    cin >> A;
    SlidingWindowAggregation<lint, lint, id, op> swag(0);
    int ret = 1;
    dbg(A);
    REP(i, N - 1) {
        swag.push(abs(A[i] - A[i + 1]));
        while (swag.size() and swag.fold_all() == 1) swag.pop();
        chmax(ret, swag.size() + 1);
    }
    return ret;
}
