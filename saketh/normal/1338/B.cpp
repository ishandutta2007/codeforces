...
int main() {
    ...
    F0R (i, N) if (tr.children[i].empty()) {
        leaf_count++;

        if (tr.depth[i]&1) odd = 1;
        else even = 1;

        leaf_parents.insert(tr.par[i]);
    }

    int low = (odd && even) ? 3 : 1;
    int high = N - 1 - leaf_count + sz(leaf_parents);
