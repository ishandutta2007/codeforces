...
void bfs(int v, int t, vi &dist, vector<vi> &graph)
{
    fill(all(dist), -1);
    dist[v] = 0;
    vi q = {v};
    ...
    vector<int> ways(graph.size());
    vector<int> waysP(graph.size());
    ways[v] = 1;
    int j = 1;
    for (int i = 1; i < q.size(); i++)
    {
        while (dist[q[j]] < dist[q[i]])
        {
            for (auto f : graph[q[j]])
            {
                if (dist[f] == dist[q[j]])
                    waysP[q[j]] += ways[f], waysP[q[j]] %= MOD;
            }
            j++;
        }
        for (auto f : graph[q[i]])
        {
            if (dist[f] + 1 == dist[q[i]])
                ways[q[i]] += ways[f], ways[q[i]] %= MOD,
                waysP[q[i]] += waysP[f], waysP[q[i]] %= MOD;
        }
    }
    while (j < q.size())
    {
