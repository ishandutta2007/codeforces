...
int main() {
    ...
    while(TT--) {
        ...
        //debug(ans,time);
        rep(i,1,n) {
            if(time < xt[i].second + abs(xt[i].first - ans)) {
                if(time + abs(xt[i].first - ans) <= xt[i].second) {
                    ans = xt[i].first;
                    time = xt[i].second;
                }
                else {
                    ans += (xt[i].second + abs(xt[i].first - ans) - time) / 2;
                    time = xt[i].second + abs(xt[i].first - ans);
                }
                //debug( xt[i].second , abs(xt[i].first - ans));
            }
            //debug(ans,time);
        }
        cout << ans << '\n';
