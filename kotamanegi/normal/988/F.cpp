...
void solve(){
    ...
    edges.insert(edges.begin(), mp(0, 0));
    REP(i, m) {
        int a, b;
        cin >> a >> b;
    ...
    dp[0][0] = 0;
    for (int i = 0; i < a; ++i) {
        while (itr < edges.size() && edges[itr].first == i) {
            dp[i][itr] = dp[i][0];
            itr++;
        }
        REP(q, itr) {
            if (q != 0 || bad[i] == 0) {
                dp[i + 1][q] = min(dp[i + 1][q], dp[i][q] + edges[q].second);
                dp[i + 1][0] = min(dp[i + 1][0], dp[i][q] + edges[q].second);
            }
        }
    }
    if (dp[a][0] == 1e18) {
