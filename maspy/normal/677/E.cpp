...
#line 1 "/home/maspy/compro/library/other/cumsum2d.hpp"
template <typename T>
struct Cumsum2D {
  int H, W;
  vc<vc<T>> dat;
...
void solve() {
  ...
  tuple<ll, ll, double> best = {-1, -1, -1.0};

  auto calc = [&](auto A) -> void {
    vc<Cumsum2D<int>> C(4);
    FOR(v, 4) {
      vv(int, B, N, N);
      FOR(x, N) FOR(y, N) B[x][y] = A[x][y] == v;
      C[v].build(B);
    }
    ...
    FOR(x, N) FOR(y, N) {
      ...
      auto check = [&](ll d) -> bool {
        vi CNT(4);
        FOR(v, 4) {
          CNT[v] += C[v].get(x, x + 1, y - d, y);
          CNT[v] += C[v].get(x, x + 1, y + 1, y + d + 1);
          CNT[v] += C[v].get(x - d, x + d + 1, y, y + 1);
        }
        assert(SUM(CNT) == 4 * d + 1);
        if (CNT[0] > 0) return false;
        ll n2 = CNT[2], n3 = CNT[3];
        double v = n2 * log(2) + n3 * log(3);
        if (get<2>(best) <= v) best = {n2, n3, v};
        return true;
      };
      ll lim = min(x, y);
      chmin(lim, N - 1 - x);
      chmin(lim, N - 1 - y);
      binary_search(check, 0, lim + 1);
    }
  };
  ...
  auto [n2, n3, v] = best;
  if (n2 == -1) return print(0);

  mint ANS = mint(2).pow(n2) * mint(3).pow(n3);
  print(ANS);
}
