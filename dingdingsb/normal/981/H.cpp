...
mint fac[N],ifac[N],s[N],sum[N],dsum[N];
int fa[N],sz[N];
#define mid (l+r>>1)
poly work1(int l,int r,vector<int>&a){
	if(l==r){
		poly f;f.resize(2);
		f[0]=1;f[1]=a[l];return f;
	}
	return work1(l,mid,a)*work1(mid+1,r,a);
}
pair<poly,poly>work2(int l,int r,vector<pair<int,mint>>&a){
	if(l==r){
		poly F,G;F.resize(2);G.resize(1);
		F[0]=1;F[1]=a[l].first;G[0]=a[l].second;
		return {F,G};
	}
	auto L=work2(l,mid,a),R=work2(mid+1,r,a);
	return {L.first*R.first,L.first*R.second+L.second*R.first};
}
void dfs1(int u){
	vector<int>tmp;sz[u]=1;
	for(auto v:e[u])if(v!=fa[u])
		fa[v]=u,dfs1(v),sz[u]+=sz[v],tmp.push_back(sz[v]);
	poly f;
	if(sz[u]==1)f.resize(1),f[0]=1;
	else f=work1(0,tmp.size()-1,tmp);
	for(int i=0;i<=k&&i<f.size();i++)
		s[u]+=fac[k]*ifac[k-i]*f[i];
	for(auto v:e[u])
		if(v!=fa[u])
			sum[u]+=sum[v];
	sum[u]+=s[u];
...
signed main(){
	...
	in::read(n,k);
	if(k==1){out::write((mint(n)*(n-1)/2).x);out::flush();return 0;}
	for(int i=1,u,v;i<n;i++)
		in::read(u,v),e[u].push_back(v),e[v].push_back(u);
	dfs1(1);dfs2(1);
	mint ans=0;
	for(int i=1;i<=n;i++)
		ans+=s[i]*(sum[1]-sum[i]+s[i]-dsum[i]);
	ans/=2;
	for(int u=1;u<=n;u++){
		if(sz[u]==1)continue;
		poly F;F.resize(2);F[0]=1;F[1]=n-sz[u];
		vector<pair<int,mint>>tmp;
		for(auto v:e[u])if(v!=fa[u])
			tmp.push_back({sz[v],sum[v]});
		F=F*work2(0,tmp.size()-1,tmp).second;
		for(int i=0;i<=k&&i<F.size();i++)
			ans+=fac[k]*ifac[k-i]*F[i];
	}
	out::write(ans.x);
	out::flush();
	return 0;
