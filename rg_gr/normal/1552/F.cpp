...
signed main()
{
	...
	for(int i=1;i<=n;i++)
	{
		int pos=std::upper_bound(x+1,x+1+n,y[i])-x;
		cost[i]=-y[i]+x[i]+(f[i-1]-f[pos-1]);
		cost[i]=(cost[i]%mod+mod)%mod;
		f[i]=(f[i-1]+cost[i])%mod;
		if(p[i])ans=(ans+cost[i])%mod;
	}
	oldl(ans);
	return 0;
}