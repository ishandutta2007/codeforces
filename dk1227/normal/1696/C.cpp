...
void Main() {
	qread(n, m);
	rep(i, 1, n) qread(a[i]);
	qread(k); rep(i, 1, k) qread(b[i]);
	t1 = t2 = 0;
	rep(i, 1, n) {
		pii nw;
		if(a[i] % m) nw = mp(a[i], 1);
		else nw = get(a[i]);
		
		if(t1 && x[t1].fi == nw.fi) x[t1].se += nw.se;
		else x[++t1] = nw;
	}
	rep(i, 1, k) {
		pii nw;
		if(b[i] % m) nw = mp(b[i], 1);
		else nw = get(b[i]);
		
		if(t2 && y[t2].fi == nw.fi) y[t2].se += nw.se;
		else y[++t2] = nw;
	}
	if(t1 != t2) puts("No");
	else {
		rep(i, 1, t1) if(x[i] != y[i]) {
			puts("No");
			return ;
		}
		puts("Yes");
