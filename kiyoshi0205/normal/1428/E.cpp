...
ll calc(datas k){
  ll x=k.first,c=k.second;
  ll q=x/c,r=x%c;
  return (q+1)*(q+1)*r+q*q*(c-r);
}
...
int main(){
  ...
  ll i,j;
  cin>>N>>K;
  priority_queue<tdata> que;
  datas f=datas(0,1),s=f;
  s.second++;
  rep(i,N){
    cin>>f.first;
    s.first=f.first;
    if(s.first>1)que.push(tdata(calc(f)-calc(s),f));
    else ans++;
  }
  f.first=5;
  f.second=4;
