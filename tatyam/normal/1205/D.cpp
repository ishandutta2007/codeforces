...
int main(){
    ...
    ll mn=LINF,cen=-1;
    {
        auto dfs = [&](ll from, ll at, auto dfs) -> void {
            ...
            if(siz[at]*2>n&&chmin(mn,siz[at]))cen=at;
        };
        dfs(-1, 0, dfs);
    }
    {
    ...
    ll d=0;
    vec(ll,cost,n);
    sort(all(g[cen]),[&](ll x,ll y){return siz[x]>siz[y];});
    ll s=0;
    each(i,g[cen]){
        auto dfs = [&](ll from, ll at, ll diff, auto dfs) -> void {
            cost[at]=d+=diff;
            out(from+1,at+1,min(cost[at]-cost[from],1000000LL));
            each(i, g[at]) if(i != from){
                dfs(at, i, diff, dfs);
            }
        };
