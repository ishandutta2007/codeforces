...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto g=read();
	cauto b=read();
	auto rs=readV(r); sort(rall(rs));
	auto gs=readV(g); sort(rall(gs));
	auto bs=readV(b); sort(rall(bs));
	rep(ri,r+1)rep(gi,g+1)rep(bi,b+1){
		if(ri<r&&gi<g) MX(dp[ri+1][gi+1][bi], dp[ri][gi][bi]+rs[ri]*gs[gi]);
		if(ri<r&&bi<b) MX(dp[ri+1][gi][bi+1], dp[ri][gi][bi]+rs[ri]*bs[bi]);
		if(gi<g&&bi<b) MX(dp[ri][gi+1][bi+1], dp[ri][gi][bi]+gs[gi]*bs[bi]);
	}
	// rep(ri,r+1){
	// 	rep(gi,g+1)rep(bi,b+1) cout<<dp[ri][gi][bi]<<" \n"[bi==b];
	// 	cout NL;
	// }
	cout<< *max_element(dp[0][0],dp[212][0]) NL;
  return 0;
}