...
struct hsh{
  ...
  hsh():a(0),b(0){}
  hsh(LL x):a(x),b(x){}
  hsh(LL a,LL b):a(a%M1),b(b%M2){}

  hsh operator*(const hsh&h)const{
    return hsh(a*h.a,b*h.b);
  }
  hsh operator+(const hsh&h)const{
    return hsh(a+h.a,b+h.b);
  }
  hsh operator-(const hsh&h)const{
    return hsh(a+M1-h.a,b+M2-h.b);
  }
  bool operator==(const hsh&h)const{
    return a==h.a&&b==h.b;
  }
  bool operator!=(const hsh&h)const{
    return !(*this==h);
...
int main(int argc, char **argv){
  ...
  P[0]=hsh(1);
  P[1]=hsh(P1,P2);
  for(int i=2;i<sz(P);++i) P[i]=P[i-1]*P[1];
  vector<hsh> H(sz(S)+1);
  forv(i,S) H[i+1]=H[i]+hsh(S[i])*P[i];

  auto get=[&](int a,int l){
             return (H.at(a+l)-H.at(a))*P.at(sz(S)-a);
           };
  ...
  int res=0;
  for(int l0=1;l0*c0+c1<=sz(S);++l0){
    int l1=sz(S)-l0*c0;
    if(l1%c1) continue;
    l1/=c1;
    hsh h0=get(f0*l1,l0);
    hsh h1=get(f1*l0,l1);
    if(l0==l1 && h0==h1) continue;
    int p=0;
    bool f=false;
    for(char c:T){
      if((c=='0' && get(p,l0)!=h0) || (c=='1' && get(p,l1)!=h1)){
        f=true;
        break;
      }
      if(c=='0') p+=l0;
