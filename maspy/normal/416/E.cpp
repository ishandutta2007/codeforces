...
void solve() {
  ...
  vv(ll, D, N, N, INF);
  FOR(a, N) FOR(b, N) D[a][b] = G[a][b];
  FOR(k, N) FOR(i, N) FOR(j, N) chmin(D[i][j], D[i][k] + D[k][j]);
  ...
  FOR(m, M) {
    ...
    ll to = E[m][1];
    ll c = E[m][2];
    FOR(b, N) if (c + D[to][b] == D[a][b]) CNT[a][b]++;
    swap(a, to);
    FOR(b, N) if (c + D[to][b] == D[a][b]) CNT[a][b]++;
  }

  vi ANS;
  FOR(a, N) FOR3(b, a + 1, N) {
    ll ans = 0;
    FOR(c, N) {
      // c 
      if (D[a][c] + D[c][b] == D[a][b]) ans += CNT[c][b];
    }
    ANS.eb(ans);
  }
  print(ANS);
