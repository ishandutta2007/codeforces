...
ll solve(){
    ...
    auto BFS = [&](ll s){
        ...
        rep(q.size()){
            ll at=q[i];
            each(i,g[at])if(chmin(cost[i],cost[at]+1))q.push_back(i);
        }
        return cost;
    };
    auto cost1=BFS(0),cost2=BFS(n-1),cost3=cost1;
    rep(n)cost3[i]+=cost2[i]+1;
    {
        auto& cost=cost3;
        priority_queue q(greater<>{},vector<pll>{});
        rep(n)q.emplace(cost[i],i);
        while(q.size()){
            auto [c,i]=q.top();
            q.pop();
            each(j,g[i])if(chmin(cost[j],cost[i]+1))q.emplace(cost[j],j);
        }
    }
    ll ans=LINF;
    each(a,b,c,edge)chmin(ans,c*(1+cost1[a]+cost2[b]));
    each(a,b,c,edge)chmin(ans,c*(1+cost1[b]+cost2[a]));
    each(a,b,c,edge)chmin(ans,c*(1+cost3[a]));
    each(a,b,c,edge)chmin(ans,c*(1+cost3[b]));
    return ans;
}
int main(){
    LL(t);
