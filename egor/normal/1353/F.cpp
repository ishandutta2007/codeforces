...
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        auto a = in.readTable<ll>(n, m);

        arr2d<ll> cur(n, m);
        arr2d<ll> cost(n, m);
        ll infty = numeric_limits<ll>::max() / 2;
        ll answer = infty;
        for (int i : range(n)) {
            for (int j : range(m)) {
                cur(i, j) = a(i, j);
                cost(i, j) = 0;
                for (int k : RevRange(i + 1)) {
                    for (int l : RevRange(j + 1)) {
                        if (k == i && l == j) {
                            continue;
                        }
                        cost(k, l) = infty;
                        if (k + 1 <= i) {
                            cur(k, l) = cur(k + 1, l) - 1;
                            if (a(k, l) >= cur(k, l)) {
                                minim(cost(k, l), cost(k + 1, l) + a(k, l) - cur(k, l));
                            }
                        }
                        if (l + 1 <= j) {
                            cur(k, l) = cur(k, l + 1) - 1;
                            if (a(k, l) >= cur(k, l)) {
                                minim(cost(k, l), cost(k, l + 1) + a(k, l) - cur(k, l));
                            }
                        }
                    }
                }
                ...
                minim(answer, cost(0, 0) + cost(n - 1, m - 1));
            }
        }
