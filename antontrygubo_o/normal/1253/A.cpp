...
void solve()
{
    ...
    for (int i = 0; i<n; i++) if (b[i]<0) {cout<<"NO"<<endl; return;}
    bool eq = true;
    for (int i = 0; i<n; i++) if (b[i]!=0) eq = false;
    if (eq) {cout<<"YES"<<endl; return;}
    int l = 0;
    while (b[l]==0) l++;
    int r = n-1;
    while (b[r]==0) r--;
    for (int i = l; i<=r; i++) if (b[i]!=b[l]) {cout<<"NO"<<endl; return;}
    cout<<"YES"<<endl;
}
