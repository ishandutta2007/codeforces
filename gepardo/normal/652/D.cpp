...
main()
{
    ...
    vector <int> q;
    for (int i = 0; i < n; i++) q.pb(v[i].ff), q.pb(v[i].ss);
    sort(q.begin(), q.end());

    map<int,int> dMap;
    for (int i = 0; i < q.size(); i++) dMap[q[i]] = i;
    ...
    for (int i = 0; i < n; i++) w.pb({v[i], i});
    sort(w.begin(), w.end());

    fenwick <int> f(2 * n);
    vector <int> ans(n);

    for (int i = 0; i < n; i++) f.inc(v[i].ss, 1);

    for (int i = 0; i < n; i++)
    {
        ans[w[i].ss] = f.sum(w[i].ff.ss) - 1;
        f.inc(w[i].ff.ss, -1);
    }
