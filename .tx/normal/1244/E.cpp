...
void solve() {
    ...
    while (isz(st) > 1 && k) {
        ...
        if (x.second < y.second) {
            ...
            llong t = min(k, 1LL * (z.first - x.first) * x.second);
            k -= t;
            int m = t / x.second;
            if (x.first + m == z.first) {
                st.erase(x);
                st.erase(z);
                st.insert({z.first, x.second + z.second});
            } else {
                st.erase(x);
                st.insert({x.first + m, x.second});
            }
        } else {
            auto it = rbegin(st);
            ++it;
            auto z = *it;
            llong t = min(k, 1LL * (y.first - z.first) * y.second);
            k -= t;
            int m = t / y.second;
            if (y.first - m == z.first) {
                st.erase(y);
                st.erase(z);
                st.insert({z.first, y.second + z.second});
            } else {
                st.erase(y);
                st.insert({y.first - m, y.second});
            }
        }
    }

    cout << rbegin(st)->first - begin(st)->first << "\n";
}//jtufxz