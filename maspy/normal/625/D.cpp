...
void solve() {
  ...
  auto check = [&](vi X) -> bool {
    while (len(X) && X[0] == 0) X.erase(X.begin());
    vi Y = X;
    reverse(all(Y));
    FOR(i, len(Y)) Y[i] += X[i];
    Y.eb(0);
    FOR(i, len(Y)) if (Y[i] >= 10) {
      Y[i] -= 10;
      Y[i + 1] += 1;
    }
    if (Y.back() == 0) Y.pop_back();
    reverse(all(Y));
    auto A = s_to_vi(S, '0');
    if (Y == A) {
      string ANS;
      for (auto&& x: X) ANS += '0' + x;
      print(ANS);
      return true;
    }
    return false;
