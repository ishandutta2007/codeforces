...
// \sum_{i=0}^{n-1} r^i i^d = r^n g(n) - g(0)
// g: poly, deg(g) = d
// r f(n + 1) - g(n) = n^d
// g(n) = (r e^(d/dn) - 1)^-1 n^d
Poly sumPowerPoly(Mint r, int d) {
  assert(d < LIM_INV);
  assert(r.x != 1);
  Poly gs(d + 1);
  for (int i = 0; i <= d; ++i) gs[i] = r * invFac[i];
  gs[0] -= 1;
  gs = gs.inv(d + 1);
  for (int i = 0; i <= d; ++i) gs[i] *= fac[d] * invFac[d - i];
  reverse(gs.begin(), gs.end());
  return gs;
}
vector<Mint> sumPowerPolys(Mint r, int d, const vector<Int> &ns) {
  const Poly gs = sumPowerPoly(r, d);
  const int nsLen = ns.size();
  vector<Mint> xs(nsLen);
  for (int i = 0; i < nsLen; ++i) xs[i] = ns[i];
  vector<Mint> ys = SubproductTree(xs).multiEval(gs);
  for (int i = 0; i < nsLen; ++i) ys[i] = r.pow(ns[i]) * ys[i] - gs[0];
  return ys;
}
...
int main() {
  for (; ~scanf("%d%d%d", &N, &K, &P); ) {
    ...
    const auto fs = sumPowerPolys(Mint(P), K, ns);
// cerr<<"fs = ";pv(fs.begin(),fs.end());
    ...
    for (int i = 0; i < N; ++i) {
      ...
      for (int s = 0; s < 2; ++s) for (int t = 0; t < 2; ++t) {
        nxt[s][t] += crt[s][t] * (A[i] + 1);
        if (!s) nxt[1][t] += crt[s][t] * fs[i];
        if (!t) nxt[s][1] += crt[s][t] * A[i];
        if (!s && !t) nxt[1][1] += crt[s][t] * fs[i];
      }
