...
main() {
    ...
    rep(i, n) {
        rep(j, n) {
            ...
            I = min(I, J) + 1;
            rep(k, I) {
                if(s[i] == s[j] and k == J) continue;
                if(k and s[i].substr(s[i].size() - k, k) != s[j].substr(0, k)) continue;
                ll res = 0;
                string S = s[i];
                rep2(ii, k, s[j].size() - 1) S.pb(s[j][ii]);
                rep(t, S.size()) {
                    rep(ii, n) {
                        if(t + s[ii].size() <= s[i].size()) continue;
                        if(S.substr(t, s[ii].size()) == s[ii]) res += a[ii];
                    }
                }
                g[i].eb(s[j].size() - k, res, j);
            }
        }
    }
    ll ans = 0;
    vv(ll, dp, 41001, n, -LLONG_MAX);
    rep(i, n) {
        ll st = a[i];
        rep(j, n) {
            if(s[j].size() > s[i].size()) continue;
            if(i == j) continue;
            rep(k, s[i].size() + 1 - s[j].size()) if(s[i].substr(k, s[j].size()) == s[j]) st += a[j];
        }
        dp[s[i].size()][i] = st;
    }
    rep(i, 40501) {
        rep(j, n) {
            for(auto [d, c, J] : g[j]) { chmax(dp[i + d][J], dp[i][j] + c); }
        }
    }
    if(L <= 40500) {
        rep(i, L + 1) rep(j, n) chmax(ans, dp[i][j]);
        cout << ans << endl;
        return 0;
    }
    rep(i, n) {
        rep2(d, 1, 210) {
            ll T = dp[40500][i] - dp[40500 - d][i];
            bool flag = true;
            rep2(j, 40000, 40500) { flag &= dp[j][i] - dp[j - d][i] == T; }
            if(!flag)
                continue;
            else {
                rep2(j, 40000, 40500) { chmax(ans, dp[j][i] + T * ((L - j) / d)); }
            }
        }
    }
    cout << ans << endl;
