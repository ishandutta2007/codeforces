...
template <int char_size, int margin> struct Trie {
    ...
    int add(const string &str, int str_index, int node_index, int id) {
        if(str_index == str.size()) {
            ...
            int res = add(str, str_index + 1, nodes[node_index].nxt[c], id);
            update_child(node_index, nodes[node_index].nxt[c], id);
            return res;
        }
    }

    int add(const string &str, int id) { return add(str, 0, 0, id); }

    int add(const string &str) { return add(str, nodes[0].exist); }
...
template <int char_size, int margin> struct AhoCorasick : Trie<char_size + 1, margin> {
    ...
    void build(bool heavy = true) {
        ...
        for(int i = 0; i < this->size(); i++) { correct[i] = (int)this->nodes[i].accept.size(); }
        queue<int> que;
    ...
    pii move(const char &c, int now, const vi &cost) {
        int sum = 0;
        while(this->nodes[now].nxt[c - margin] == -1) now = this->nodes[now].nxt[FAIL];
        now = this->nodes[now].nxt[c - margin];
        for(auto &v : this->nodes[now].accept) sum += cost[v];
        return {now, sum};
    }
};
...
int main() {
    ...
    vector<string> s;
    vi cost(n);
    rep(i, n) s.eb(read()), cin >> cost[i];
    rep(i, n) aho.add(s[i]);
    // rep(i, n) cnt[aho.add(s[i])] += cost[i];
    aho.build();
    auto solve = [&](string p) {
        if(p.size() == 1 and p[0] == 0) return mint(0);
        int N = aho.nodes.size();
        vv(mint, dp, N, k + 1);
        int same = 0, scost = 0;
        rep(i, si(p)) {
            vv(mint, ndp, N, k + 1);
            rep(j, N) {
                rep(t, m) {
                    auto [nxt, add] = aho.move(t, j, cost);
                    rep2(s, 0, k - add) { ndp[nxt][s + add] += dp[j][s]; }
                }
            }
            rep2(t, 0, (int)p[i]) {
                if(!i and !t) continue;
                auto [nxt, add] = aho.move(t, same, cost);
                if(t == p[i]) {
                    same = nxt, scost += add;
                } else if(scost + add <= k)
                    ndp[nxt][scost + add] += 1;
            ...
            if(i) {
                rep2(t, 1, m - 1) {
                    auto [nxt, add] = aho.move(t, 0, cost);
                    if(add <= k) ndp[nxt][add] += 1;
                }
            }
            swap(dp, ndp);
        }
        mint ans;
        rep(i, N) rep(j, k + 1) ans += dp[i][j];
        if(scost <= k) ans += 1;
        return ans;
    };
    l.back()--;
