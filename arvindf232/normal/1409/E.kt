...
fun main(){
    solve.cases{
        ...
        val DP = Array(3){IntArray(n+1){nlarge} }
        ...
        for(i in 0 until n){
            for(left in 0 until 3){
                ...
                if(left < 2){
                    val now = X[i]
                    val upto = S.floorIndex(now + k)
                    DP[left+1][upto+1] = maxOf(DP[left+1][upto+1],here + (upto - i + 1))
                }
                DP[left][i+1] = maxOf(DP[left][i+1],here)
            }
        }
        put((0 until 3).maxOf { DP[it][n] })
    }
    done()
}
