...
int main() {
	CASE(t) {
		...
		vector<ll> val(n);
		for (int i = 1; i + 1 < n; i++)
			val[i] = max(0, max(a[i - 1], a[i + 1]) + 1 - a[i]);

		vector<ll> ps = val, ss = val;
		for (int i = 3; i + 1 < n; i++) ps[i] += ps[i - 2];
		for (int i = n - 4; i >= 1; i--) ss[i] += ss[i + 2];

		if (n % 2 == 1) {
			W(ps[n - 2]);
		}
		else {
			ll ans = min(ps[n - 3], ss[2]);
			for (int i = 1; i + 3 < n; i += 2)
				chkmin(ans, ps[i] + ss[i + 3]);
