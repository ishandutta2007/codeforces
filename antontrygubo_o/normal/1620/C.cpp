...
void solve()
{
    ...
    for (auto c: s)
    {
        ...
        {
            if (cur>0) cnt.push_back(cur*k+1);
            cur = 0;
        }
    }
    if (cur>0) cnt.push_back(cur*k+1);
    ...
    int t = cnt.size();
    vector<ll> suf_prod(t+1);
    suf_prod[t] = 1;
    for (int i = t-1; i>=0; i--)
    {
        if (suf_prod[i+1]<=INF/cnt[i]) suf_prod[i] = suf_prod[i+1]*cnt[i];
        else suf_prod[i] = INF;
    }
