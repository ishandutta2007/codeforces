...
#[derive(Debug,Clone,Copy)]
	pub enum Var { Pos(usize), Neg(usize) }
	use Var::*;
	impl Var {
		fn neg(self) -> Self { Self::from_id(self.to_id() ^ 1) }
		fn inner(self) -> usize { match self { Pos(x) => x, Neg(x) => x } }
		fn to_id(self) -> usize { match self { Pos(x) => 2*x, Neg(x) => 2*x+1 } }
		fn from_id(id: usize) -> Self { if id % 2 == 0 {Pos(id/2)} else {Neg(id/2)} }
	}
	pub struct TwoSAT { n_vars: usize, g: Digraph1 }
	impl TwoSAT {
		pub fn new(n_vars: usize) -> Self { Self {n_vars, g: Digraph1::new(n_vars*2)} }
		pub fn add_true(&mut self, a: Var) {
			assert!(a.inner() < self.n_vars);
			self.g.add_edge(a.neg().to_id(), a.to_id());
		}
		pub fn add_false(&mut self, a: Var) { self.add_true(a.neg()); }
		pub fn add_or(&mut self, a: Var, b: Var) {
			assert!(a.inner().max(b.inner()) < self.n_vars);
			self.g.add_edge(a.neg().to_id(), b.to_id());
			self.g.add_edge(b.neg().to_id(), a.to_id());
		}
		pub fn solve(&self) -> Option<Box<[bool]>> {
			let scc = Scc::new(&self.g, SccEdge::Unique);
			for v in 0..self.n_vars {
				if scc.id[Pos(v).to_id()] == scc.id[Neg(v).to_id()] { return None; }
			}
			let mut comp_a = vec![None; scc.n];
			let mut var_a = vec![None; self.n_vars];
			let mut q = Vec::new();
			for i in (0..scc.n).rev() {
				if comp_a[i].is_some() { continue; }
				q.push(i);
				while let Some(sid) = q.pop() {
					if let Some(false) = comp_a[sid] { panic!("yo"); }
					if comp_a[sid].is_some() { continue; }
					comp_a[sid].replace(true);
					for &u in scc.nodes[sid].iter() {
						match Var::from_id(u) {
							Pos(x) => var_a[x].replace(true),
							Neg(x) => var_a[x].replace(false)
						};
					}
					let neg_sid = scc.id[Var::from_id(scc.nodes[sid][0]).neg().to_id()];
					if let Some(true) = comp_a[neg_sid] { panic!("yah"); }
					comp_a[neg_sid].replace(false);
					for &sv in scc.edges.as_ref().unwrap()[sid].iter() {
						q.push(sv);
					}
				}
			}
			let res = var_a.into_iter().map(|x| x.unwrap()).collect();
			Some(res)
		}
	}//}}}
}
...
fn main() {
	...
	l!(n,m = rin.u());
	let mut ts = two_sat::TwoSAT::new(m);
	let a = rin.vi(n);
	let mut s = v!([n][2]);
	for i in 0..m {
		rep!{[rin.u()]
	...
	for i in 0..n {
		...
		let y = s[i][1];
		if a[i] == 0 {
			ts.add_or(Pos(x), Pos(y));
			ts.add_or(Neg(x), Neg(y));
		} else {
			ts.add_or(Pos(x), Neg(y));
			ts.add_or(Neg(x), Pos(y));
		}
	}
	if let Some(_) = ts.solve() {
		writeln!(rout, "YES").ok();
	} else {
		writeln!(rout, "NO").ok();
	}
