...
main() {
    ...
    rep(i, n) if(a[i] <= ma - n) a[i] = ma - n + 1;
    rep(i, n) a[i] -= ma - n + 1;
    vi cnt(n * 4 + 1);
    rep(i, n) cnt[a[i]]++;
    rep(i, n * 4) cnt[i + 1] += cnt[i];
    rep(i, n * 4 + 1) cnt[i] %= p;
    vi ans(n * 4 + 1);
    rep(i, n * 4 + 1) {
        int st = i - cnt[i];
        int en = i - cnt[i] - (n - 1 - cnt[i]) / p * p;
        if(en < 0) en += (-en - 1) / p * p + p;
        // cout << en << " " << st + p << endl;
        // chmax(en, i - (n - 1 - cnt[i]) / p * p);
        if(st < 0 or en < 0) continue;
        ans[en]++;
        if(st + p < n * 2) ans[st + p]--;
    }
    rep(i, n * 4 - p) { ans[i + p] += ans[i]; }
    // print(ans);
    int c = 0;
    rep(i, n * 2) c += !ans[i];
    cout << c << endl;
    rep(i, n * 2) {
        if(!ans[i]) cout << i + ma - n + 1 << " ";
    }
    cout << endl;
    // rep2(x, 1, 2000) {
    //     int now = 1;
