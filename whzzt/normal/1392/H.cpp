...
int main(){
	...
	cin>>n>>m;
	int a=0;
	for(int i=0;i<=n;++i) a=(a+binom(n-i+m,m))%mod;
	a=(i64)a*fpow(binom(n+m,m),mod-2)%mod;
	int ret=0;
	for(int i=1;i<=n;++i){
		int w=(i64)a*inv[i]%mod*(m+i)%mod*binom(n,i)%mod;
		if(i&1) ret=(ret+w)%mod;
		else ret=(ret-w+mod)%mod;
	}
	cout<<ret<<'\n';
}