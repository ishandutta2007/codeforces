...
vector<vector<ModInt>> f(const int n, const string& s, const string& t) {
  vector dp(n + 1, vector(n * 2 + 1, ModInt(0)));
  dp[0][n] = 1;
  REP(i, n) REP(j, n * 2 + 1) {
    REP(x, 2) REP(y, 2) {
      ...
      const int nxt = j + x - y;
      if (0 <= nxt && nxt <= n * 2) dp[i + 1][nxt] += dp[i][j];
    }
  }
  return dp;
}
...
void solve() {
  ...
  const auto l = f(n, s, t);
  reverse(ALL(s));
  reverse(ALL(t));
  const auto r = f(n, s, t);
  ModInt ans = 0;
  REP(i, n) for (int j = -n; j <= n; ++j) {
    ans += l[i + 1][j + n] * r[n - 1 - i][-j + n] * abs(j);
  }
  cout << ans << '\n';
}
