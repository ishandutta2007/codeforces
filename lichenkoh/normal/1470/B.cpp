...
void go() {
  ...
  for (int i=0;i<n;i++) {
    int ai=rint();
    int fsz=factorize(ai);
    int key=1;
    for (int fi=0;fi<fsz;fi++) {
      if (factorize_dest[fi].snd&1) {
        key *= factorize_dest[fi].fst;
      }
    }
    ++h[key];
  }
  int hi0=0;
  for (auto &w:h) chkmax(hi0, w.snd);
  int sumeven=0;
  int hiodd=0;
  for (auto &w:h) {
    if (w.fst!=1) {
      if (w.snd&1) chkmax(hiodd,w.snd);
      else sumeven+=w.snd;
    }
  }
  int hi1=max(sumeven+h[1],hiodd);
  int qn=rint();
