...
signed main(){fastio
    ll n,m;cin>>n>>m;
    vector<map<ll,ll>> dp(n+1,{(l_l{-1,0})});
    while(m--){
        ll a,b,c;cin>>a>>b>>c;
        
        auto itr=dp[a].lower_bound(c);itr--;
        ll val=(*itr).se+1;
        l_l ret={c,val};
        bool F=true;
        {
            auto itr=dp[b].upper_bound(c);itr--;
            if((*itr).se>=ret.se){
                F=false;
            }else{
                if((*itr).fi==ret.fi){
        ...
        {
            itr=dp[b].find(ret.fi);
            while(itr!=dp[b].begin()){
                itr--;
                if(NG(ret,*itr)){
                    itr=dp[b].erase(itr);
                }else{
        ...
        rep(i,1,n){
            cout<<i<<":"<<endl;
            for(auto x:dp[i])cout<<x.fi<<" "<<x.se<<endl;
        }cout<<endl;
         //*/
    }
