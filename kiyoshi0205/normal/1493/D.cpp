...
int main(){
  ...
  rep(i,N){
    ...
    while(x!=1){
      if(v[x]==1){
        auto& t=cnt[x][i];
        if(t)S[x].erase(S[x].lower_bound(t));
        S[x].emplace(++t);
        break;
      }
      auto& t=cnt[v[x]][i];
      if(t)S[v[x]].erase(S[v[x]].lower_bound(t));
      S[v[x]].emplace(++t);
      assert(v[x]>1);
      x/=v[x];
    }
  ...
  rep(i,200001){
    if((int)cnt[i].size()<N)continue;
    (ans*=modpow(i,*S[i].begin()))%=mod;
  }
  while(K--){
    int x;
    cin>>i>>x;
    ...
    while(x!=1){
      if(v[x]==1){
        auto& t=cnt[x][i];
        nowmin=(int)S[x].size()<N?0:*S[x].begin();
        if(t)S[x].erase(S[x].lower_bound(t));
        S[x].emplace(++t);
        nextmin=(int)S[x].size()<N?0:*S[x].begin();
        if(nowmin<nextmin)(ans*=x)%=mod;
        break;
      }
      auto& t=cnt[v[x]][i];
      nowmin=(int)S[v[x]].size()<N?0:*S[v[x]].begin();
      if(t)S[v[x]].erase(S[v[x]].lower_bound(t));
      S[v[x]].emplace(++t);
      nextmin=(int)S[v[x]].size()<N?0:*S[v[x]].begin();
      if(nowmin<nextmin)(ans*=v[x])%=mod;
      x/=v[x];
    }
    print(ans);
  }
