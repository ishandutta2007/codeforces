...
int main() {
    ...
    rep2(i, 1, n - 1) rep(a, 3) rep(b, 3) rep(c, 3) {
        if(dp[i][a][b][c] == 0) continue;
        if(chmax(b, a - 1) or chmax(c, b - 1)) assert(0);
        rep(j, m) {
            int na = 0, nb = 0, nc = 0;
            chmax(na, b);
            chmax(na, a + (j == s[i - 1] - 'a') - 1);

            chmax(nb, c);
            chmax(nb, na - 1);
            chmax(nb, b + (j == s[i] - 'a') - 1);

            chmax(nc, nb - 1);
            chmax(nc, c + (i < n - 1 ? j == s[i + 1] - 'a' : 0) - 1);
    ...
    ll ans = 0;
    // dump(dp);
    OUT(dp[n][2][1][0] + dp[n][1][1][0]);
    // rep(i, n) {
    //     rep(j, 2) rep(k, 2) {
    //         rep(c, m) {
    //             if(s[i] - 'a' != c) {
    //                 if(i < n - 1 and s[i + 1] - 'a' == c and !j) { dp[i + 2][j + 1][k] += dp[i][j][k]; }
    //                 if(!k) dp[i][j][1] += dp[i][j][k];
    //                 if(!j and !k and !(i < n - 1 and s[i + 1] - 'a' == c)) dp[i + 1][j + 1][k + 1] += dp[i][j][k];
    //             } else {
    //                 dp[i + 1][j][k] += dp[i][j][k];
    //             }
    //         }
