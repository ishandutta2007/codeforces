...
void rj::main(int) {
  const auto n = input<i64>();
  const auto a = input<i64>();
  const auto b = input<i64>();
  const auto c = input<i64>();
  const auto s = input<string>();
  auto c0 = 0ll;
  auto c1 = 0ll;
  auto c00 = 0ll;
  auto c11 = 0ll;
  ...
  if (c00 < c11) {
    auto ans = c00 * a + (c00 + 1) * b;
    c0 -= c00;
    c1 -= c00 + 1;
    if (c < b) {
      ans += max(min(c0, c1 - 1), 0ll) * (b - c);
    }
    println(ans);
  } else if (c00 == c11) {
    auto ans = c00 * a + c00 * b;
    c0 -= c00;
    c1 -= c00;
    if (c < b) {
      ans += max(min(c0, c1 - 1), 0ll) * (b - c);
