...
int lab[maxn];
bool check(int u, int p, int val) {
    lab[u] = 0;
    for (auto v: gr[u]) {
        if (v == p) continue;
        if (!check(v, u, val)) return false;
        if (lab[v] % val) {
            ++lab[v];
            if (lab[v] % val) return false;
        } else ++lab[u];
    }
    return true;
}
...
int main() {
    ...
    while (ntest--) {
        ...
        acc[1] = 1;
        rep(i, 0, n - 1) acc[1] += acc[1];
        rep(i, 2, n) {
            if ((n - 1) % i) continue;
            if (!check(1, 1, i)) continue;
            if (lab[1] % i) continue;
            acc[i] = 1;
        }
        rep(i, 1, n + 1) clog << db(i) << db(acc[i]) << endl;
        for (int i = n; i-- > 1; ) {
            for (int f = i * 2; f < n; f += i)
                acc[i] -= acc[f];
        }
        rep(i, 1, n + 1) cout << acc[i] << ' ';
        cout << '\n';
    }
