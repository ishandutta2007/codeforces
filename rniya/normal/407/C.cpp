...
const int MAX_K = 105, MAX_N = 100010;

int main() {
    cin.tie(0);
    ios::sync_with_stdio(false);
    Combination<mint> COM(MAX_N + MAX_K);
    int n, m;
    cin >> n >> m;
    vector<mint> a(n);
    for (auto& x : a) cin >> x;
    vector<vector<int>> add(n);
    vector<vector<pair<int, int>>> del(n + 1);
    for (int i = 0; i < m; i++) {
        int l, r, k;
        cin >> l >> r >> k;
        l--;
        add[l].emplace_back(k);
        del[r].emplace_back(k, r - l);
    }

    vector<mint> dp(MAX_K, 0);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j + 1 < MAX_K; j++) dp[j] += dp[j + 1];
        for (int& k : add[i]) {
            for (int j = 0; j <= k; j++) {
                dp[j] += COM.C(k, j);
            }
        ...
        for (auto& p : del[i]) {
            int k = p.first, len = p.second;
            for (int j = 0; j <= k; j++) {
                dp[j] -= COM.C(k + len, k - j);
            }
        }
        a[i] += dp[0];
    }
