...
int main() {
	...
	enum { IN = 5, NOT = 0, AND = 1, OR = 2, XOR = 3 };
	...
	auto calc = [&](auto calc, int now) -> int {
		if(tp[now] == NOT) {
			return res[now] = !calc(calc, chi[now].first);
		} else if(tp[now] == IN)
			return res[now] = in[now];
		int x, y;
		x = calc(calc, chi[now].first);
		y = calc(calc, chi[now].second);
		if(tp[now] == AND) {
			return res[now] = x & y;
		} else if(tp[now] == OR)
			return res[now] = x | y;
	...
	auto dfs = [&](auto dfs, int now) -> void {
		...
		int x, y;
		x = res[chi[now].first], y = res[chi[now].second];

		if(tp[now] == AND) {
			if(x) dfs(dfs, chi[now].second);
			if(y) dfs(dfs, chi[now].first);
			return;
		} else if(tp[now] == OR) {
			if(!y) dfs(dfs, chi[now].first);
			if(!x) dfs(dfs, chi[now].second);
