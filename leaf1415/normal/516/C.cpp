...
struct SegTree{
	...
	SEG id(){  //identity element
		return Q(P(-inf, -inf), P(-inf, -inf));
	}
	SEG ope(SEG a, SEG b){ //operator
		Q ret;
		if(a.fst < b.fst) swap(a, b);
		ret.fst = a.fst;
		ret.snd = max(a.snd, b.fst);
		return ret;
	}
	...
	int left_find(int r, SEG x){ //return largest l s.t. comp(query(l, r), x) = true
		...
		while(1){
			...
			if(k % 2) sum = ope(sum, seg[k-1]);
			k /= 2;
		}
		k--;
		while(k < (1<<size)){
			if(comp(ope(sum, seg[k*2+1]), x)) k = k*2+1;
			else sum = ope(sum, seg[k*2+1]), k = k*2;
		}
		return k - (1<<size);
	}
...
ll n, m;
ll d[100005], h[100005];
ll a[200005], b[200005];
SegTree aseg(18), bseg(18);
...
int main(void)
{
	...
	aseg.init(), bseg.init();
	rep(i, 0, 2*n){
		aseg.update(i, Q(P(a[i], i), P(-inf, -inf)));
		bseg.update(i, Q(P(b[i], i), P(-inf, -inf)));
	}
	...
	rep(i, 1, m){
		...
		Q ares = aseg.query(L, R), bres = bseg.query(L, R);
		if(ares.fst.snd != bres.fst.snd) cout << ares.fst.fst + bres.fst.fst << "\n";
		else cout << max(ares.fst.fst + bres.snd.fst, ares.snd.fst + bres.fst.fst) << "\n";
	}
