...
void solve() {
	...
	struct ST {
		...
		int getk(int i, int l, int r, int k) {
			if (l + 1 == r) {
				return l;
			}
			...
			int m = (l + r) >> 1;
			if (a[i * 2 + 1] > k) {
				return getk(i * 2 + 1, l, m, k);
			}
			return getk(i * 2 + 2, m, r, k - a[i * 2 + 1]);
		}

		void upd(int i, int l, int r, int qi, int qd) {
			if (l + 1 == r) {
				a[i] += qd;
				return;
			}
	...
	for (int iq = 0; iq < q; iq++) {
		...
		if (x > 0) {
			...
			int z = st.getk(0, 0, n, -x - 1);
			st.upd(0, 0, n, z, -1);
		}
	}

	if (st.a[0]) {
		cout << st.getk(0, 0, n, 0) + 1 << "\n";
	} else {
		cout << "0\n";
	}
}