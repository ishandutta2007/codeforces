...
signed main() {
  ...
  // 
  auto X = A;
  auto n = unique(all(X)) - X.begin();

  X.resize(n);
  N = X.size();
  vi CNT(N);
  FOR(i, N) { CNT[i] = upper_bound(all(A), X[i]) - lower_bound(all(A), X[i]); }

  vi dp(N + 1);
  FOR(_, K) {
    vi newdp(N + 1);
    FOR3(i, 1, N + 1) {
      newdp[i] = max(dp[i], newdp[i - 1]);
      auto right = X[i - 1];
      auto sz = 0;
      FOR3_R(j, i - 6, i) {
        if (j < 0) continue;
        if (right - X[j] > 5) break;
        sz += CNT[j];
        chmax(newdp[i], dp[j] + sz);
      }
    }
    swap(dp, newdp);
  }
