...
int jc[300005],Inv[300005],jcinv[300005];
...
signed main() {
	int n;
	cin >> n;
	jc[0]=jc[1]=Inv[1]=jcinv[1]=jcinv[0]=1;
	for(int i=2;i<=n;i++)
	{
		jc[i]=jc[i-1]*i%mod;
	...
	for(int i=n;i>=1;i--)
	{
		if(!qwq[i]) continue;
		vector <int> qaq;
		for(int j=0;j<=qwq[i];j++)
			qaq.push_back(ksm(i,j)*C(qwq[i],j)%mod);
		v=multiply(v,qaq);
	}
	v.resize(n+1);
	int ans=0;
