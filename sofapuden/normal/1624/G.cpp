...
int ufa(int x, vector<int> &uf){
	return uf[x] = (x^uf[x] ? ufa(uf[x],uf) : uf[x]);
}

void solve(){
	int n, m; cin >> n >> m;
	vector<array<int,3>> v(m);
	for(auto &x : v)cin >> x[1] >> x[2] >> x[0], x[1]--, x[2]--;
	sort(all(v));
	int ans = 0;
	for(int i = 0; i < 30; ++i){
		vector<int> uf(n);
		...
		int sz = n;
		for(int j = 0; j < m; ++j){
			if(ufa(v[j][1],uf) != ufa(v[j][2],uf)){
				sz--;
				uf[ufa(v[j][1],uf)] = uf[ufa(v[j][2],uf)];
			}
			if(sz == 1){
				for(int z = 30; z >= 0; --z){
					if((1<<z)&v[j][0]){
