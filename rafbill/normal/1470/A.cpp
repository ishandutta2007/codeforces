...
void solve(){
  li n,m;cin>>n>>m;
  vi K(n); for(li i = 0; i < (li)(n); ++i) { cin>>K[i]; K[i]--; }
  vi C(m); for(li i = 0; i < (li)(m); ++i) cin>>C[i];
  for(li i = 0; i < (li)(n); ++i) K[i] = C[K[i]];
  sort(begin(C), end(C));
  sort(begin(K), end(K), greater<li>());
  li ans = 0;
  for(li i = 0; i < (li)(n); ++i) {
    if(i < m) ans += min(K[i], C[i]);
    else ans += K[i];
  }
  cout << ans << '\n';
}
...
int main(){
  ...
  int t;cin>>t;for(li i = 0; i < (li)(t); ++i)solve();cout << flush;
