...
int ltn(int n, vector<int> a, vector<int> b) {
	set<int> s;
	REP(i, n) {
	...
	bool ok = true;
	FOR(i, nx, nx + n) {
		if(b[i] != i - nx + 1) ok = false;
	}
	if(ok) return nx;
	else return inf;
}

int gteqn(int n, vector<int> a, vector<int> b) {
	int ret = 0;
	REP(i, n) {
		if(b[i] != 0) {
			ret = max(ret, i + 2 - b[i]);
...
int main() {
	...
	REP(i, n) cin >> a[i];
	REP(i, n) cin >> b[i];
	cout << min(ltn(n, a, b), gteqn(n, a, b)) << endl;
	return 0;
}
