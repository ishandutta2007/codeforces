...
int main(){
  ...
  int i,j,k,x;
  cin>>H>>W>>K;
  vector<int> ans(K,-5000000);
  ans[0]=0;
  while(H--){
    vector<vector<int>> fdp(W/2+1,vector<int>(K,-5000000));
    fdp[0][0]=0;
    rep(i,W){
      cin>>x;
      brep(j,W/2)rep(k,K){
        chmax(fdp[j+1][(k+x)%K],fdp[j][k]+x);
      }
    }
    // matoutput(fdp);
    vector<int> next(K,-5000000);
    next[0]=0;
    for(auto v:fdp){
      rep(i,K){
        if(v[i]<0)continue;
        rep(j,K)chmax(next[(i+j)%K],ans[j]+v[i]);
      }
    }
    swap(ans,next);
    // output(ans);
