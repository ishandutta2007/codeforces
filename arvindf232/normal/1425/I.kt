...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    val depth:IntArray by lazy { this.calculateDepth() }
...
//    solve.userand()
    solve.cases{
            ...
            fun eat(x:Int,t:Int,q:query){
                ...
                if(tmax[x] <= t){
                    ...
                    G.subs(x){
                            w ->
                        eat(w,t,q)
                        min = minOf(min ,tmin[w])
                        max = maxOf(max, tmax[w])
                    }
                    tmin[x] = min
                    tmax[x] = max
                }
            }
            first@for((t,q) in qs.withIndex()){
                var now = q.x
                while(now != 0){
                    now = G.parent[now]
                    if(tmin[now] > t) continue@first
                }
                eat(q.x,t,q)
                now = q.x
                while(now != 0){
                    ...
                    var max = time[now]
                    G.subs(now){
                        min = minOf(min,tmin[it])
                        max = maxOf(max, tmax[it])
                    }
                    tmin[now] = min
                    tmax[now] = max
                }
        ...
        for((i,v) in intime.withIndex()){
            reverse[v]= i
        }

        val bins = Array(q){ mint }
        ...
        fun eatbig(x:Int,t:Int,q:query){
            val s1 = intime[x]
            val s2 = outtime[x]
            while(true){
                val next = st.ceiling(s1)
                if(next != null && next <= s2){
                    st.remove(next)
                    val x = reverse[next]
                    q.anscount2 ++
                    q.anssum2 += depth[x]
                    val sp = Speed[x]
                    if(sp + t in bins.indices){
                        bins[sp + t].add(next)
                    }
                }else{
                    break
        ...
        for(q in qs){
//            assert(q.anscount == q.anscount2)
//            assert(q.anssum2 == q.anssum)
            q.anscount += q.anscount2
            q.anssum += q.anssum2
        }
        for(q in qs){
            put("${q.anssum - q.anscount * depth[q.x]} ${q.anscount}")
        }
...
cannot confuse intime and normal time
custom sort, intime mistake

memory oredr careful , offline sweep then offline sweep

69 45
1 4 9 10 8 2 9 9 10 5 8 9 5 6 8 7 2 8 7 6 2 6 10 8 6 2 5 5 3 2 8 8 5 3 6 2 1 4 7 2 7 3 7 4 10 10 7 5 4 7 5 10 7 1 1 10 7 7 7 2 3 4 2 8 4 7 4 4 6
1 1 1 4 4 4 7 7 9 9 11 11 13 13 13 16 16 18 18 18 3 3 23 23 23 17 12 10 22 22 2 2 2 24 24 6 6 35 35 35 14 14 39 39 39 15 15 30 36 36 5 28 28 41 41 43 43 43 48 52 52 52 33 62 62 62 61 61
44
13
9
1
...
segment tree beats: within subtree : can do somethign on 2 paths, and also some subtree
 */