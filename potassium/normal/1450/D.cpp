...
int main() {
	CASE(t) {
		...
		for (int i = 1; i < mex; i++) {
			...
			chkmax(minc[i], v.back() - v.front());
			if (st.query(v.front(), v.back()) < i)
				minc[i] = INT_MAX;
			else {
				int l = v.front();
				int L = l;
				for (int jump = MAXN; jump >= 1; jump >>= 1) {
					if (L - jump >= 0 && st.query(L - jump, l) >= i)
						L -= jump;
				}
				...
				int R = r;
				for (int jump = MAXN; jump >= 1; jump >>= 1) {
					if (R + jump < n && st.query(r, R + jump) >= i)
						R += jump;
				}
