...
public class Main
{
	...
	{	
		int n=ni();
		Map<pair, Long> hola=new HashMap<pair, Long>();
		for(int i=0; i<n; i++)
		{
			int k=ni(),b=ni();
			if(k!=0)
			{
				int k1=k/Math.abs(gcd(k,b)),b1=b/Math.abs(gcd(k,b));
				if(k1*b1<0)
					hola.put(new pair(-Math.abs(k1),Math.abs(b1)), hola.getOrDefault(new pair(-Math.abs(k1),Math.abs(b1)), 0L)+k);
				else
					hola.put(new pair(Math.abs(k1),Math.abs(b1)), hola.getOrDefault(new pair(Math.abs(k1),Math.abs(b1)), 0L)+k);
			}
		}
		int ans=0;
		for(long i:hola.values())
//			if(i!=0)
				ans++;
		pr(ans);
