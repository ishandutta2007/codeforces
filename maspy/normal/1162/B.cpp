...
void solve() {
  ...
  VV(ll, A, H, W);
  VV(ll, B, H, W);
  FOR(x, H) FOR(y, W) if (A[x][y] > B[x][y]) swap(A[x][y], B[x][y]);
  auto NG = [&]() -> void { print("Impossible"); };
  FOR(x, H) FOR(y, W) {
    if (x && A[x - 1][y] >= A[x][y]) return NG();
    if (x && B[x - 1][y] >= B[x][y]) return NG();
    if (y && A[x][y - 1] >= A[x][y]) return NG();
    if (y && B[x][y - 1] >= B[x][y]) return NG();
  }
  print("Possible");
}
