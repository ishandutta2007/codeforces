...
/* --------------- fast io --------------- */ // end
int F[10010][2010];
int fre[1001010],a[1001010],b[1001010],cnt,n,i,Testing,X,K,Q;
bool check()
{
	for (i=1;i<=n;i++)
...
//	freopen("testdata.in","r",stdin);
	...
	for (; Testing; Testing--) {
		   cin>>n;
		   for (i=1;i<=n;i++) b[i]=0;
		   for (i=1;i<=n;i++) cin>>a[i],F[0][i]=a[i];cnt=0;
		   while (check())
		       {
		           cnt++; 	
		       	   for (i=1;i<=n;i++) b[i]=a[i];
				   for (i=1;i<=n;i++) fre[a[i]]++;
				   for (i=1;i<=n;i++) a[i]=fre[a[i]],F[cnt][i]=a[i]; 
				   for (i=1;i<=n;i++) fre[b[i]]=0;
			   }
		   cin>>Q;
		   for (;Q;Q--)
		      { 
