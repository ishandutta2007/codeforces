...
bool dfs(int i, int c) {
    ...
    val[i]=c;
    for (pi nb : adj[i]) {
        if (vis[nb.F] && val[nb.F] != (c^nb.S)) return true;
        else if (!vis[nb.F] && dfs(nb.F, (c^nb.S))) return true;
    }
    return false;
}
...
void solve() {
    cin >> n >> m;
    u.rsz(n-1); v.rsz(n-1); w.rsz(n-1); adj.clear(); adj.rsz(n);
    F0R(i, n-1) {
        cin >> u[i] >> v[i] >> w[i]; u[i]--; v[i]--;
        if (w[i]>-1) {
            int tmp = __builtin_popcount(w[i])%2;
            adj[u[i]].pb(mp(v[i], tmp));
            adj[v[i]].pb(mp(u[i], tmp));
        }
    }
    F0R(i, m) {
        cin >> a >> b >> p; a--; b--;
    ...
    vis.assign(n, false); val.rsz(n);
    F0R(i, n) if (!vis[i] && dfs(i, 0)) {
        NO; return;
    }
    YES;
    F0R(i, n-1) {
        if (w[i]>-1) cout << u[i]+1 << ' ' << v[i]+1 << ' ' << w[i] << nl;
        else cout << u[i]+1 << ' ' << v[i]+1 << ' ' << (val[u[i]]^val[v[i]]) << nl;
    }
}
