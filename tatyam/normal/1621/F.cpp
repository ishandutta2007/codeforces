...
void solve(){
    ...
    if(b<=c){
        ll zero=0,one=0;
        rep(n-1)if(s[i]=='0'&&s[i+1]=='0')zero++;
        rep(n-1)if(s[i]=='1'&&s[i+1]=='1')one++;
        chmin(zero,one+1);
        chmin(one,zero+1);
        out(zero*a+one*b);
        return;
    ...
    rep(n-1)if(s[i]=='1'&&s[i+1]=='1')one++;
    ll edge=0;
    auto run=run_press(s);
    if(run.back().first=='0'){
        edge++;
        zero+=run.back().second-1;
    }
    run.pop_back();
    if(run.size()){
        if(run.front().first=='0'){
            edge++;
            zero+=run.front().second-1;
        }
        run.erase(run.begin());
    }
    vector<ll>cen;
    each(a,b,run)if(a=='0')cen.push_back(b);
    Sort(cen);
    Rev(cen);
    out(max([=]()mutable{
        ll ans=0,max=0;
