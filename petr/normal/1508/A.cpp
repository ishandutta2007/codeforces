...
class ABinaryLiterature {
 public:
  void solveOne() {
    ...
    while (true) {
      int done = 0;
      for (int i = 0; i < 3; ++i) if (p[i] == s[i].size()) ++done;
      if (done >= 2) break;
      if (done == 1) {
        int mp = -1;
        int mpi = -1;
        for (int i = 0; i < 3; ++i) if (p[i] < s[i].size()) {
          if (p[i] > mp) {
            mp = p[i];
            mpi = i;
          }
        }
        cout << s[mpi].substr(mp);
        break;
      }
      int n0 = 0;
      int n1 = 0;
      for (int i = 0; i < 3; ++i) if (p[i] < s[i].size()) {
        if (s[i][p[i]] == '0') ++n0; else ++n1;
      }
      char c = n0 >= n1 ? '0' : '1';
      cout << c;
      for (int i = 0; i < 3; ++i) if (p[i] < s[i].size()) {
