...
void add(mint bit[],int i,mint x){ ++i; ++i;
  while(i<nn){
    bit[i]+=x;
    i+=i&-i;
  }
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	while(q--){
		...
		if(x<0){
			int i=-x-1;
			add(bitaw,i,(MOD-a[i]*w[i]%MOD)%MOD);
			add(bitw,i,-w[i]);
			w[i]=y;
			add(bitaw,i,a[i]*w[i]%MOD);
			add(bitw,i,w[i]);
			...
			mint re=0;
			re+=a[c]*((sum(bitw,c)-sum(bitw,l))%MOD)%MOD;
			re-=sum(bitaw,c)-sum(bitaw,l);
			re+=sum(bitaw,r)-sum(bitaw,c);
			re-=a[c]*((sum(bitw,r)-sum(bitw,c))%MOD)%MOD;
			out(c,1);
			cout<<re<<endl;
		}
	}
