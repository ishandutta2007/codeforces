...
inline void AC() {
    ...
    n = read();
    for(_ i = 1; i <= n; ++i) a[i] = n;
    for(_ i = n, j = 1; i >= 1; --i, ++j) {
        a[n] = j;
        _ x = qr(true);
        if(x) {b[n] = i;
            break;}
    ...
    if(!b[n]) b[n] = 1;
    for(_ i = 1, j = b[n]; i < b[n]; ++i, --j) {
        a[n] = 1;
        for(_ k = 1; k <= n-1; ++k) a[k] = j;
        b[qr(true)] = i;
    }
    for(_ i = b[n]+1, j = 2; i <= n; ++i, ++j) {
        for(_ k = 1; k <= n-1; ++k) a[k] = 1;
        a[n] = j;
        b[qr(true)] = i;
    }
    for(_ i : rg(n)) a[i] = b[i];
    qr(false);
