...
#pragma region datastructure BIT
#include <vector>
...
bool is_prime(const u64 n) {
	if(n <= 1) return 0;
	if(~n & 1) return n == 2;
	if(n < (1ll << 30)) return is_prime_constexpr(n);
	u64 d = n - 1;
	while(~d & 1) d >>= 1;
...
void main_() {
	...
	V<mint> res(q, 1);
	V<> u(q), v(q), x(q);
	unordered_map<int, V<pii>> event;
	REP(i, q) {
		cin >> u[i] >> v[i] >> x[i];
		--u[i], --v[i];
		auto tmp = pair_factorize(x[i]);
		for(auto [val, cnt] : tmp) {
			event[val].eb(cnt, n + i);
		}
	}
	REP(i, n) {
		auto tmp = pair_factorize(a[i]);
		for(auto [val, cnt] : tmp) {
			if(!event.count(val)) continue;
			REP(j, cnt) {
				event[val].eb(j + 1, i);
			}
		}
	}
	for(auto &[key, vec] : event) {
		SORT(vec);
		BIT<ll> BT(n);
		for(auto [ig, idx] : vec) {
			if(idx < n)
				...
				idx -= n;
				ll tmp = hld.query(
				    u[idx], v[idx], 0ll, [&](int l, int r) { return BT.sum(l, r); }, [](ll x, ll y) { return x + y; });
				res[idx] *= POW(mint(key), tmp);
			}
		}
	}
	printel(res);
