...
//    v.push_back((int)s.size() + 1);
    ...
    for (int j = 0; j < v.size(); ++j) {
        FI(2) {
//            DBN(j, i, sumLL[j][i], sumLR[j][i], sumRL[j][i], sumRR[j][i]);
        }
//        DBN(j, sumLL[)
    }
    for (int j = 0; j < v.size(); ++j) {
        ll cur = 0;
        cur += x * (j + 1) * (v.size() - 1 - j);
        cur += sumLL[j][1] * y + sumLR[j][1] * x;
        if (j + 1 != v.size()) {
            cur += sumRL[j + 1][0] * x + sumRR[j + 1][0] * y;
        }
        ans = min(ans, cur);
//        DBN(j, cur, sumLR[j][1] * x);
    }
    for (int j = 0; j < v.size(); ++j) {
        ll cur = 0;
        cur += y * (j + 1) * (v.size() - 1 - j);
        cur += sumLL[j][0] * x + sumLR[j][0] * y;
        if (j + 1 != v.size()) {
            cur += sumRL[j + 1][1] * y + sumRR[j + 1][1] * x;
        }
        ans = min(ans, cur);
//        DBN(j, cur, sumLL[j][0] * x);
    }
    if (ans == LLINF) {
        ans = 0;
    }
