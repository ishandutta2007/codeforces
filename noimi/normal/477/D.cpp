...
int main() {
    ...
    rep(i, n) ord[sa[i]] = i;
    {
        ...
        rep2(i, 1, n - 1) {
            ...
            rep(j, i) {
                if(i + (i - j) > n) continue;
                if(ord[i] > ord[j] or rh.get(j, i) == rh.get(i, i * 2 - j)) {
                    v[i * 2 - j] += dp[i][j];
                } else if(i * 2 - j + 1 <= n)
                    v[i * 2 - j + 1] += dp[i][j];
            }
            rep(j, n) v[j + 1] += v[j];
            rep(j, n + 1) dp[j][i] += v[j];
        }
        cout << accumulate(all(dp[n]), (mint)0) << endl;
    }
    {
        ...
        rep2(i, 1, n - 1) {
            ...
            rep(j, i) {
                if(i + (i - j) > n) continue;
                if(ord[i] > ord[j] or rh.get(j, i) == rh.get(i, i * 2 - j)) {
                    chmin(v[i * 2 - j], dp[i][j] + 1);
                } else if(i * 2 - j + 1 <= n)
                    chmin(v[i * 2 - j + 1], dp[i][j] + 1);
            }
            rep(j, n) chmin(v[j + 1], v[j]);
            rep(j, n + 1) chmin(dp[j][i], v[j]);
        }
        ll ans = inf<ll>;
        ll t = 0;
        rep(i, min(40, n)) {
            t += (1LL << i) * (s[n - 1 - i] - '0');
            if(dp[n][n - i - 1] == inf<int>) continue;
            chmin(ans, t + dp[n][n - i - 1]);
        }
        if(ans != inf<ll>) { cout << ans % MOD << endl, exit(0); }
        rep2(i, 40, n - 1) {
            if(dp[n][n - i - 1] == inf<int>) continue;
            mint ans;
            rep2(j, n - 1 - i, n - 1) ans = ans * 2 + (s[j] - '0');
            ans += dp[n][n - i - 1];
            cout << ans << endl;
            exit(0);
        }
