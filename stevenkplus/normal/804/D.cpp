...
double query(int a, int b) {
  ...
  pp("query", a, b);
  double p = 1.0 / sizes[a] / sizes[b];
  vector<int> &c = counts[a];
  vector<int> &s = sums[b];
  vector<ll> &ps = prodsums[b];
  pp(c);
  pp(s);
  pp(ps);
  pp(p);
  ...
  pp(diam[b]);
  double ans = 0;
  for (int i = 0; i <= diam[a]; ++i) {
    int need = max(0, diam[b] - i - 1);
    if (c[i] == 0 || s[need] == 0) continue;
    double val = 0;
    val += p * c[i] * s[need] * ((i + 1) + 1.0 * ps[need] / s[need]);
    val += p * c[i] * (sizes[b] - s[need]) * diam[b];
    pp(i, val);
    ans += val;
  }
  pp(ans);
...
int main() {
  ...
  for(int i = 1; i <= n; ++i) {
    ...
    go(i);
    counts[i] = vector<int>(diam[i] + 1);
  }

  for (int i = 1; i <= n; ++i) {
    ++counts[find(i)][dist[i]];
    ++sizes[find(i)];
  }
  for (int i = 1; i <= n; ++i) {
    if (find(i) != i) continue;
    int d = diam[i];
    sums[i] = vector<int>(d + 2);
    prodsums[i] = vector<ll>(d + 2);
    for (int j = d; j >= 0; --j) {
      sums[i][j] = sums[i][j + 1] + counts[i][j];
      prodsums[i][j] = prodsums[i][j + 1] + ll(j) * counts[i][j];
    }
  }
