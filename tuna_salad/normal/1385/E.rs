...
fn dfs(u: usize, e: &[Vec<usize>], vis: &mut [i32], pos: &mut [usize], t: &mut usize) -> bool {
	vis[u] = 1;
	for &v in e[u].iter() {
		if vis[v] == 1 || (vis[v] == 0 && !dfs(v, e, vis, pos, t)) {
			return false;
		}
	}
	pos[u] = *t;
...
fn main() {
	...
	rep!{[rin.u()]
		...
		l!(mut vis,pos = vec![0; n]);
		let mut ok = true;
		let mut t = 0;
		for i in 0..n {
			if vis[i] == 0 && !dfs(i, &e, &mut vis, &mut pos, &mut t) {
				ok = false;
				break;
			}
		}
