...
int main() {
	...
	auto dfs = [&](auto dfs, int now, int par) -> tuple<ll, ll, int> {
		// int back_min = height[now] + 1;
		// int back_min_from_here = height[now];
		...
		foa(e, g[now]) {
			...
			if(height[e.to] == -1) {
				...
				ll a, b;
				int c;
				tie(a, b, c) = dfs(dfs, e.to, now);
				chmax(f, a);
				gl += b;
				// chmin(back_mi/n, c);
				can_back |= c;
			} else {
				// chmin(back_min_from_here, height[e.to]);
				can_back = 1;
			}
		}

		if(can_back) {
			return make_tuple(f, gl + w[now], 1);
		}
		return make_tuple(w[now] + f, 0LL, 0);
	};

	ll a, b;
	tie(a, b, ignore) = dfs(dfs, s, -1);
