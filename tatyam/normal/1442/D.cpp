...
int main(){
    ...
    vec(vector<ll>,bag,n);
    auto dfs=[&](vector<ll>dp,ll l,ll r,auto dfs)->void{
        if(l+1==r){
            bag[l]=dp;
            return;
        }
        ...
        const ll cen=(l+r)/2;
        rep(i,l,cen){
            const ll k=a[i].size()-1;
            rrep(j,k,dp.size())chmax(dp[j],dp[j-k]+a[i][k]);
        }
        swap(dp,dp2);
        dfs(dp2,cen,r,dfs);
        rep(i,cen,r){
    ...
    dfs(vector<ll>(k+1),0,n,dfs);
    ll ans=0;
    rep(n)rep(j,a[i].size())chmax(ans,a[i][j]+bag[i][k-j]);
    out(ans);
}