...
int sn = 10000, sk = 6, rans = 3321;
vector<int> curr;

void process(vector<int> &was, int imp) {
    vector<int> ans(sz(was));
    for (int i = 0; i < sz(was); ++i) {
        int l = i - 1;
        int r = i + 1;
        if (l < 0) l += sz(was);
        if (r >= sz(was)) r -= sz(was);
        if (i == imp) {
            ans[r] += was[i];
        } else {
            ans[l] += was[i] / 2;
...
bool checkimp(int imp) {
    vector<int> start(n, k);
    for (int i = 0; i < sz(questions); ++i) {
        if (start[questions[i]] != answers[i])
            return false;
        process(start, imp);
    }
    return true;
