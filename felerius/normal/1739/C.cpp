// Card Game (https://codeforces.com/contest/1739/problem/C)
...
namespace cp_lib_hash { template <bool> class Hash; }
...
namespace cp_lib_modint {
    ...
    struct Base : ModIntTag {
        ...
        template <bool> friend class cp_lib_hash::Hash;
...
struct Combinatorics {
    ...
    template <class It>
    MI multinom(int n, It it, It it_end) const {
        MI res = f[n];
...
int main() {
    ...
    Combinatorics<MI> binom(60);

    array<pair<MI, MI>, 31> dp{};
    dp[1] = {1, 0};
    rep(i, 2, 31) {
        dp[i].first = binom.binom(2 * i - 1, i - 1) + dp[i - 1].second;
        dp[i].second = binom.binom(2 * i - 2, i - 2) + dp[i - 1].first;
    }
