...
template <typename I> int f(I begin, I end) {
    vi a;
    ...
    for (int i = 0, j = 0; i < n; i = j) {
        while (j < n && msb(a[i]) == msb(a[j]))
            j++;

        int before = max(count - 1, 0);
        int gain = f(a.begin() + i, a.begin() + j);
        ckmax(ans, gain - before);
        count++;
    }
