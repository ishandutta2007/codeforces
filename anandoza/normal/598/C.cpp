...
int main() {
    ...
    auto less_than = [&](int i, int j) {
        T dot_i = dot(p[i], p[i + 1]);

        T dot_j = dot(p[j], p[j + 1]);

        T norm_i = p[i].norm() * p[i + 1].norm();
        T norm_j = p[j].norm() * p[j + 1].norm();

        if (dot_i < 0 && dot_j < 0)
            return dot_i * dot_i * norm_j < dot_j * dot_j * norm_i;
    ...
    int best = 0;
    for (int i = 0; i < n; i++) {
        best = min(best, i, less_than);
    }

    ps(indices[p[best]], indices[p[best + 1]]);
