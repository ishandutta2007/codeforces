// time-limit: 2000
// problem-url: https://codeforces.com/contest/1630/problem/D
...
void up(){
	...
	for(auto& it: b) {
		std::cin>>it;
		gcdb=std::gcd(gcdb, it);
	}

	auto const computeResult=[&]{
		int64_t finalResult{};
		for(int const i: range(0, gcdb)){
			int64_t sumAbs {}; bool parityNegative {};
			int minAbs=INT_MAX;
			for(int j=i; j<int(a.size()); j+=gcdb){
				//anyZero|=a[j]==0;
				parityNegative^=a[j]<0;
				sumAbs+=std::abs(a[j]);
				minAbs=std::min(minAbs, std::abs(a[j]));
			}
			// don't need to track anyZero, in that case minAbs=0 so doesn't matter
			if(parityNegative==0){
				// can make all positive
				finalResult+=sumAbs;
			}else{
				// cannot...
				finalResult+=sumAbs-2*minAbs;
			}
		}
		return finalResult;
	};

	auto const computeResultAlt=[&]{
		for(int const i: range(0, gcdb)){
			a[i]=-a[i];
		}
	...
	for(auto& it: b) {
		...
		if(
			...
			result=std::max(result, computeResultAlt());
			break;
		}
	}
