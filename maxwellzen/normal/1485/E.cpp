...
void dfs(int i, int p, int d) {
    dep[i] = d;
    if (bydep.size()<=d) bydep.rsz(d+1);
    bydep[d].pb(i);
    for (int nb : adj[i]) if (nb!=p) dfs(nb,i,d+1);
}
...
void solve() {
    ...
    a.rsz(n,0);
    FOR(i,1,n) cin >> a[i];
    dep.rsz(n); bydep.clear();
    dfs(0,0,0);
    d = bydep.size();
    mn.assign(d,1e9+1);
    mx.assign(d,0);
    F0R(i,d) for (int j : bydep[i]) ckmin(mn[i], a[j]), ckmax(mx[i], a[j]);
    debug(dep); debug(mn); debug(mx);
    dp.assign(n,0);
    R0F(j, d) {
        ll x=-1e18, y=-1e18;
        for (int i : bydep[j]) {
            for (int nb : adj[i]) if (dep[nb]==dep[i]+1) ckmax(dp[i], dp[nb]);
            ckmax(x, dp[i]-a[i]);
            ckmax(y, dp[i]+a[i]);
        }
        for (int i : bydep[j]) {
            dp[i] = max({max(mx[j]-a[i],a[i]-mn[j])+dp[i], x+a[i], y-a[i]});
        }
    }
    cout << dp[0] << nl;
}
