...
#define int long long
  ...
  RE (i, n) {
    make2(int, a, b);
    pol.Add({(LD)a, (LD)b});
    LD here = max(1. * p / a, 1. * q / b);
    mini(res, here);
  }
  pol.MakeConvexHull();
  pol.pts.PB(pol.pts[0]);
  Line to_goal({0, 0}, goal);
  for (int i = 0; i < SZ(pol.pts) - 1; i++) {
    Line l = Line(pol.pts[i], pol.pts[i + 1]);
    Point P = Utils::InterLineLine(l, to_goal);
    if (P.IsZero()) {
    } else if (Utils::PtBelongToSeg(l, P)) {
      mini(res, goal.Norm() / P.Norm());
    }
  }
  cout<<res<<endl;
