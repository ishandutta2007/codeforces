...
inline void solve()
{
	...
	sort(a+1,a+n+1);if(a[n]>a[n-1]+1) return puts("0"),void();
	if(a[n]==a[n-1]) return printf("%d\n",fc[n]),void();
	int cn=0;for(int i=1;i<=n;i++) cn+=a[i]==a[n-1];
	int rs=fc[n];for(int i=1;i<=n;i++) rs=(rs-1ll*C(i-1,cn)*fc[cn]%P*fc[n-1-cn]%P+P)%P;
	printf("%d\n",rs);
}
