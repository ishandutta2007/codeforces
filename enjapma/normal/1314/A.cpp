...
void solve(){
    ...
    for(ll i=0;i<n;i++){
        if(table[i].first == table[i+1].first){
            bag.push(P(table[i].second, table[i].first));
            size++;
        }else if(table[i].first < table[i+1].first){
            bag.push(P(table[i].second, table[i].first));
            size++;
            for(ll j=table[i].first;j<table[i+1].first;j++){
                if(size == 0)break;
                P heavy = bag.top(); bag.pop();
                ans += heavy.first * (j - heavy.second);
                size--;
            }
        }
    }
