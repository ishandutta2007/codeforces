...
int n, m, k;
int a[nax][nax];
vi parse(const vi &in) {
	int allowed = m / 2;
	vector <vi> best(allowed + 1, vi(k, -inf));
	best[0][0] = 0;
	for (int c : in)
		for (int taken = allowed - 1; taken >= 0; --taken)
			REP(r, k)
				maxi(best[taken + 1][(r + c) % k], best[taken][r] + c);
	vi ans(k, -inf);
	for (int i = 0; i <= allowed; ++i) REP(r, k) maxi(ans[r], best[i][r]);
	return ans;
}
vi merge(const vi &a, const vi &b) {
	vi ans(k, -inf);
	for (int i = 0; i < k; ++i) for (int j = 0; j < k; ++j)
		maxi(ans[(i + j) % k], a[i] + b[j]);
	rans;
}
void solve() {
	scanf("%d%d%d", &n, &m, &k);
	...
	vi tot(k, -inf);
	tot[0] = 0;
	REP(i, n) tot = merge(tot, parse(vi(a[i], a[i] + m)));
	printf("%d\n", tot[0]);
}
int main() {
