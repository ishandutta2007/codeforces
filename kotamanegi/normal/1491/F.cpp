...
int Fib[60];
int find_fib(int now) {
	for (int q = 1; q <= 40; ++q) {
		if (now == Fib[q]) return q;
	}
	return -1;
}
int dists[300000];
int calc_dists(int now, int back) {
	dists[now] = 1;
	for (auto x : vertexs[now]) {
		if (x.second == 1) continue;
		if (x.first == back) continue;
		calc_dists(x.first, now);
		dists[now] += dists[x.first];
	}
	return dists[now];
}

bool apply_kouho(int now, int back, int itr, int S) {
	if (itr <= 1) return true;
	for (auto& x : vertexs[now]) {
		if (x.second == 1) continue;
		if (x.first == back) continue;
		if (apply_kouho(x.first, now, itr, S) == true) {
			return true;
		}
		if (dists[x.first] == Fib[itr - 1]) {
			x.second = 1;
			calc_dists(x.first, now);
			calc_dists(now, x.first);
			if (apply_kouho(x.first, now, itr - 1, x.first) == true and
				apply_kouho(now, x.first, itr - 2, now) == true) {
				x.second = 0;
				return true;
			}
			x.second = 0;
		...
		else if (dists[x.first] == Fib[itr - 2]) {
			x.second = 1;
			calc_dists(x.first, now);
			calc_dists(now, x.first);
			if (apply_kouho(x.first, now, itr - 2, x.first) == true and
				apply_kouho(now, x.first, itr - 1, now) == true) {
				x.second = 0;
				return true;
			}
			x.second = 0;
...
void solve() {
	...
	while(kouho.size() != 1){
		...
		for (int j = 0; j < kouho.size(); ++j) {
			gogo[j % 2].push_back(kouho[j]);
		}
		cout << "? " << gogo[0].size() << " 1" << endl;
		output(gogo[0]);
		cout << target << endl;
		int an;
		cin >> an;
