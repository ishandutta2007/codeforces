...
void solve()
{
    ...
    {
        if (a[l] <= a[r]) p.push_front(a[l]), l++;
        else p.push_back(a[r]), r--;
    }
    //for (auto e : p) cout << e + 1 << " "; cout << "\n";
    deque<int> q1, q2;
    for (int l = 0, r = n - 1; l <= r; )
    {
        if (p[l] < p[r]) q1.push_front(p[l]), q2.push_front(p[l]), l++;
        else if (p[l] > p[r]) q1.push_back(p[r]), q2.push_back(p[r]), r--;
        else q1.push_front(p[l]), q2.push_back(p[l]), l++;
    }
