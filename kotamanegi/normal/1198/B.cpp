...
void solve(){
    ...
    REP(i, n) {
        int a;
        cin >> a;
        vertexs[i].push_back(mp(-1, a));
    }
    int query;
    cin >> query;
    SegmentTree<int> seg(query, [](int a, int b) {return max(a,b); }, -1);
    REP(i, query) {
        int a, b;
        cin >> a >> b;
        if (a == 1) {
    ...
    REP(i, n) {
        int now_money = vertexs[i][0].second;
        for (int q = 1; q < vertexs[i].size(); ++q) {
            now_money = max(now_money, seg.query(vertexs[i][q - 1].first + 1, vertexs[i][q].first));
            now_money = vertexs[i][q].second;
        }
        now_money = max(now_money, seg.query(vertexs[i].back().first + 1, query));
        cout << now_money << " ";
    }
    cout << endl;
}
