...
int main() {
  using T = monoid::RangeSumAndUpdateQuery<int>;
  constexpr int M = 200000 + 18;
  int n, q; cin >> n >> q;
  vector<int> a(n); REP(i, n) cin >> a[i];
  LazySegmentTree<T> seg(M);
  auto add = [&](int x) -> void {
    int lb = x - 1, ub = M;
    while (ub - lb > 1) {
      const int mid = (lb + ub) / 2;
      (seg.get(x, mid + 1).sum == mid - x + 1 ? lb : ub) = mid;
    }
    seg.apply(x, lb + 1, 0);
    seg.set(lb + 1, T::Monoid{1, 1});
  };
  REP(i, n) add(a[i]);
  while (q--) {
    int k, l; cin >> k >> l; --k;
    const int r = seg.find_right(a[k], [&](const T::Monoid& x) -> bool { return x.sum == 0; });
    seg.set(r, T::Monoid{0, 1});
    seg.apply(a[k], r, 1);
    a[k] = l;
    add(a[k]);
    cout << seg.find_left(M, [&](const T::Monoid& x) -> bool { return x.sum == 0; }) << '\n';
  }
  return 0;
}