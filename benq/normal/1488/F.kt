...
fun solve() {
    val n = io.int()
    val c = ArrayList(io.ints(n))
    c.add(0,MOD)
    val q = io.int()
    data class Pair(val f: Int, val s: Int) {

    }
    val queries = Array<ArrayList<Pair>>(n+1){ArrayList<Pair>()}
    for (i in 0 until q) {
        val (l,r) = io.ints(2)
    ...
    stk.add(0)
    val cum = ArrayList<Long>()
    cum.add(0)
    val result = LongArray(q){0}
    for (i in 1..n) {
        while (stk.size > 0 && c[stk[stk.size-1]] < c[i]) {
            stk.removeAt(stk.size - 1)
            cum.removeAt(cum.size-1)
        }
        cum.add(cum[cum.size-1]+(i-stk[stk.size-1]).toLong()*c[i])
        stk.add(i)
//        println(stk)
//        println(cum)
        for (p in queries[i]) {
            ...
            while (lo < hi) {
                val mid = (lo+hi)/2
                if (stk[mid] >= p.f) hi = mid
                else lo = mid+1
            }
            // lowest ind such that stk[ind] <= p.f
            result[p.s] = cum[cum.size-1]-cum[lo]+(stk[lo]+1-p.f).toLong()*c[stk[lo]]
        }
    }
