...
int dp[2005][2005],f[2005][2005],n,m,mod=1e9+7;
...
//	cin.tie(0);
	...
	FOR(i,1,n){
		int sum0=0,sum1=0;
		for(int j=m-1;j>=1;j--){
			add(sum0,dp[i-1][j]);add(sum0,f[i-1][j]*(m-j)%mod);
			add(sum1,sum0);
			dp[i][j]=sum1;add(dp[i][j],mod-f[i-1][j]*(m-j)%mod);
			add(ans,dp[i][j]);
		}
		sum0=sum1=0;
		rep(j,1,m){
			add(sum0,f[i-1][j]);add(sum1,sum0);
			f[i][j]=sum1;
			add(f[i][j],1);
			add(ans,f[i][j]*(m-j)%mod);
		}
	}
	cout<<ans;
	io.out();
