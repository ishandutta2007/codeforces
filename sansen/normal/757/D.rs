...
fn run() {
    ...
    let mut ans = M::zero();
    for i in 0..n {
        let mut from = std::mem::take(&mut dp[i]);
        from.push((0usize, M::one()));
        from.sort_by_key(|p| p.0);
        from.dedup_by(|a, b| {
            a.0 == b.0 && {
                b.1 += a.1;
                true
        ...
        for (bit, way) in from {
            if bit > 0 && (bit + 1).count_ones() == 1 {
                ans += way;
            }
            let mut val = 0;
            for j in i..n {
                val = 2 * val + (s[j] - b'0') as usize;
                if val == 0 {
                    continue;
                }
                if val >= up {
                ...
                dp[j + 1].push((bit | (1 << (val - 1)), way));
            }
        }
    }
    for &(bit, way) in dp[n].iter() {
        if bit > 0 && (bit + 1).count_ones() == 1 {
            ans += way;
        }
    }
