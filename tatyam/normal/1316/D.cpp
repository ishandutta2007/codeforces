...
signed main(){
    LL(n);
    VV(pll,a,n,n);
    each(i,a)each(j,i)if(j.first>0){
        j.first--; j.second--;
    }
    vec(string,s,n);
    each(i,s)i.resize(n,' ');
    queue<pll>q;
    rep(n)rep(j,n)if(a[i][j]==pll{i,j}){
        s[i][j]='X';
        q.emplace(i,j);
        while(q.size()){
            auto [x,y]=q.front();
            q.pop();
            rep(k,4){
                ll x2=x+dx[k],y2=y+dy[k];
                if(x2<0||n<=x2||y2<0||n<=y2)continue;
                if(a[x2][y2]==pll{i,j}&&s[x2][y2]==' '){
                    s[x2][y2]=D[k^2];
                    q.emplace(x2,y2);
                }
            }
    ...
    rep(n)rep(j,n)if(a[i][j]==pll{-1,-1}){
        rep(k,4){
            ll x2=i+dx[k],y2=j+dy[k];
            if(x2<0||n<=x2||y2<0||n<=y2)continue;
            if(a[x2][y2]==pll{-1,-1})s[i][j]=D[k];
        }
    }
    each(i,s)each(j,i)if(j==' ')return out("INVALID");
    out("VALID");
    each(i,s)out(i);
}