// Keshi in Search of AmShZ (https://codeforces.com/contest/1693/problem/C)
...
int main() {
    ...
    while (sz(q)) {
        ...
        for (auto v2 : adj[v])
            if (--rem_in_deg[v2] + 1 + d < dist[v2])
                dist[v2] = rem_in_deg[v2] + 1 + d, q.emplace(-dist[v2], v2);
    }
