...
vector<ll> lvec[600005], rvec[600005];
ll dp[300005], pre[300005];
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, m) cin >> a[i] >> l[i] >> r[i];
	
	vector<ll> comp;
	rep(i, 1, m) comp.push_back(l[i]);
	rep(i, 1, m) comp.push_back(r[i]);
	uniq(comp);
	rep(i, 1, m){
	...
	ll C = sz(comp);
	rep(i, 1, m) lvec[l[i]].push_back(a[i]), rvec[r[i]].push_back(a[i]);
	
	SCC scc(n); map<ll, ll> mp;
	rep(i, 0, C-1){
		for(auto x : lvec[i]){
			if(mp.count(x) == 0){
				auto it = mp.lower_bound(x);
				if(it != mp.end()) scc.add_edge(x, it->first);
				if(it != mp.begin()){
					it--;
					scc.add_edge(it->first, x);
				}
			}
			mp[x]++;
		}
		...
		for(auto x : rvec[i]){
			...
			if(mp[x] == 0){
				...
				if(it != mp.begin()){
					auto pit = it; pit--;
					if(it != mp.end()) scc.add_edge(pit->first, it->first);
				}
				mp.erase(x);
			}
		}
	...
	ll ans = 0, ans_i;
	rep(i, 1, n) if(ans < dp[i]) ans = dp[i], ans_i = i;
