...
void solve() {
  ...
  auto check_three = [&](ll x, ll y, ll dx, ll dy) -> bool {
    FOR(k, 3) {
      ll nx = x + k * dx, ny = y + k * dy;
  ...
  auto check = [&]() -> bool {
    FOR(x, N) FOR(y, N) {
      if (check_three(x, y, 1, 0)) return true;
      if (check_three(x, y, 0, 1)) return true;
      if (check_three(x, y, 1, -1)) return true;
      if (check_three(x, y, 1, 1)) return true;
    }
    return false;
  };
