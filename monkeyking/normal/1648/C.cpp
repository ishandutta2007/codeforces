...
//map<int,int> cnt;
//int s[200005];
//__gnu_pbds::tree<int,__gnu_pbds::null_type,less<int>,__gnu_pbds::rb_tree_tag,__gnu_pbds::tree_order_statistics_node_update> squ[200005];
//const int B=2e5;
//int a[200005];
//
//void solve()
...
//		for(auto [x,_]:cnt)
//		{
//			int k=squ[x].order_of_key(a[i]);
//			int v=1LL*now*fac[x]%mod*ifac[x-1]%mod*fac[n-i-1]%mod;
//			// debug(i,x,k,v);
//			res=(1LL*v*k+res)%mod;
//		}
//		squ[s[a[i]]].erase(a[i]);
//		cnt[s[a[i]]]--;
//		now=1LL*now*fac[s[a[i]]]%mod;
//		if(cnt[s[a[i]]]==0) cnt.erase(s[a[i]]);
//		if(s[a[i]]==0)
//		{
//			flag=0;
//			break;
...
//		if(s[a[i]])
//		{
//			now=1LL*now*ifac[s[a[i]]]%mod;
//			squ[s[a[i]]].insert(a[i]);
//			cnt[s[a[i]]]++;
//		}
//	}
//	if(flag && n<m) chadd(res,1LL*now*fac[n-min(n,m)]%mod);
//	cout<<res<<endl;
//}
//
//int main()
