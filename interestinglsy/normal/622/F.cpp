...
// 'f43bccc55241582959fc20a40cea2cd4'
mt19937 Rand(0xf43bccc55241582959fc20a40cea2cd4);
#define il inline
#define elif else if
...
il int Mymo( const int &x ){ return x >= MOD ? x-MOD : x; }
il void Mymod( int &x ){ x = x >= MOD ? x-MOD : x; }
...
void Init(){
	fact[0] = 1;
	For(i,k+2) fact[i] = fact[i-1]*i % MOD;
	invfact[k+2] = Inv(fact[k+2]);
	fOR0(i,k+2) invfact[i] = invfact[i+1]*(i+1) % MOD;
	// O(invfact[2]);
}
...
int main(){
	...
	static ll sum[MAXK];
	For(i,k+2) sum[i] = Mymo(sum[i-1]+Pow(i,k));
	if( n <= k+2 ) END(sum[n]);

	ll ans = 0;
	ll premul = 1;
	static ll sufmul[MAXK];
	sufmul[k+2] = 1;
	fOR(i,k+1)
		sufmul[i] = sufmul[i+1] * (n-i-1) % MOD;
	Forx(i,1,k+2){
		if( i != 1 ) ( premul *= (n-i+1) ) %= MOD;
		ll p1 = invfact[i-1] * premul % MOD;
		ll p2 = invfact[k+2-i] * (((k+2-i)&1)?MOD-1:1) % MOD * sufmul[i] % MOD;
		// cout << i << " " << premul << " " << sufmul[i] << endl;
		( ans += (p1*p2)%MOD*sum[i] ) %= MOD;
	}
	cout << ans << endl;
