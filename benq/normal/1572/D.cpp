...
struct MCMF { 
	...
	C calc(int s, int t, int K) { assert(s != t);
		...
		while (path(s,t)) { // p -> potentials for Dijkstra
			...
			ckmax(ans,2000000*totFlow-totCost);
			assert(totFlow <= K);
			if (totFlow == K) break;
...
int main() {
	...
	F0R(i,N) {
		...
		vi costs; costs.reserve(1<<(N-1));
		F0R(j,1<<N) if (j&(1<<i)) costs.pb(j);
		// <= 2*K
		const int index = 2*K-2;
		if (index < sz(costs)) {
			// 2*K people in total
			// first 2*(K-1) might be occupied
			nth_element(begin(costs),begin(costs)+index,end(costs),cmp);
			F0R(j,index+1) add_ans(costs[j]);
		} else {
			for (int j: costs) add_ans(j);
		}
	}
	// dbg_time();
	// ps(sz(edges));
	...
	for (pi p: edges) compress[p.f], compress[p.s];
	int cnt = 0;
	each(t,compress) t.s = cnt++;
	int src = sz(compress);
	int snk = sz(compress)+1;
	MCMF M; M.init(sz(compress)+2);
	each(t,compress) {
		if (pct(t.f)&1) M.ae(src,t.s,1,1000000-A[t.f]);
		else M.ae(t.s,snk,1,1000000-A[t.f]);
	}
	each(t,edges) M.ae(compress.at(t.f),compress.at(t.s),1,0);
	ps(M.calc(src,snk,K));

	dbg_time();
	// dbg(edges);

	// take the 400 smallest pairs of each type?
