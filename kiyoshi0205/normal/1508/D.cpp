...
int main(){
  ...
  rep(i,v.size()-1)if(v.back().x>v[i].x)swap(v[i],v.back());

  auto cent=v.back();
  v.pop_back();
  for(auto& t:v)t.x-=cent.x,t.y-=cent.y;
  sort(all(v),[](S& l,S& r){
    return atan2l(l.x,l.y)<atan2l(r.x,r.y);
  });
  rep1(j,v.size()){
    if(dsu.unite(v[j-1].id,v[j].id)){
      ans.push_back({v[j-1].id+1,v[j].id+1});
      swap(p[v[j-1].id],p[v[j].id]);
    }
  }
  debug(p);
  rep(j,v.size()){
