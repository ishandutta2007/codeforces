...
#endif
// int t[1000005],rnk[1000005];
int dp[100005][11];
char a[100005];
int trans[]={0,10,0,2,5,9,3,9,5,2,0};
signed main()
{
	initprog();
	...
	for(int j=0;j<=10;j++)
		for(int k=0;k<j;k++)
			if(k==a[i])dp[i][ae(trans[j],k)]+=dp[i-1][j];
	for(int j=0;j<=10;j++)
		ans+=dp[i][j],debug(dp[i][j],' ');
	debug('\n');
	gr
	oldl(ans);
	// int cc=0;rg(9)t[++cc]=i;gr
	// for(int i=1;i<=1000;i++)
	// {
		// for(int j=0;j<i%11;j++)t[++cc]=t[i]*10+j;
		// rnk[t[i]]=i;
	// }
	// rg(cc)rnk[t[i]]=i;gr
	// rg(cc)
	// int tt=rnk[t[i]*10];
	// if(tt)odp(i%11,(i+110000-tt)%11);
	// gr
	
	// gr
	// rg(cc)odb(t[i]);if(i%11==10)puts("");gr
	return 0;
}