...
public:
    void solve() {
        ...
        int maxDif = numeric_limits<int>::min();
        ...
        for (int i : range(n)) {
            ...
            maxim(maxB, b[i]);
            minim(minB, b[i]);
            maxim(maxDif, a[i] - b[i]);
            minim(minDif, a[i] - b[i]);
        }
        int answer = numeric_limits<int>::max();
        int atA = -1;
        int atB = -1;
        for (int i : range(minA, maxA + 1)) {
            int mn = min(i - maxDif, minB);
            int mx = max(i - minDif, maxB);
            int cDelta = max({i - minA, maxA - i, (mx - mn + 1) / 2});
            minim(answer, cDelta, atA, i, atB, (mn + mx) / 2);
        }
        if (atA == 0 && atB == 0) {
            atA = 1;
            atB = 1;
