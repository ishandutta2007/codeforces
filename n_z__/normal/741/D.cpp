...
const int N=5e5+9,S=1<<22,INF=-2e9;
int OP,s,t,mxl,he[N],ne[N],sz[N],pr[N],d[N],b[N],ans[N],f[S];
inline void cmx(int&x,int y){
	if(x<y)x=y;
}
void pre(int x){
	sz[x]=1;
	for(int y=he[x];y;y=ne[y]){
		d[y]=d[x]+1;b[y]^=b[x];
		pre(y);sz[x]+=sz[y];
		if(sz[pr[x]]<sz[y])pr[x]=y;
	}
}
void upd(int x){
	s=b[x];
	if(OP==0)f[s]=INF;
	if(OP==1){t=S;do cmx(mxl,f[s^(t>>=1)]+d[x]);while(t);}
	if(OP==2)cmx(f[s],d[x]);
	for(int y=he[x];y;y=ne[y])upd(y);
}
void dfs(int x){
	if(!pr[x])return cmx(f[b[x]],d[x]);
	for(int y=he[x];y;y=ne[y])
		if(pr[x]!=y)dfs(y),OP=0,upd(y),mxl=INF;
	dfs(pr[x]);ans[x]=ans[pr[x]];
	mxl=2*d[x];cmx(f[s=b[x]],d[x]);t=S;
	do cmx(mxl,f[s^(t>>=1)]+d[x]);while(t);
	for(int y=he[x];y;y=ne[y])
		if(pr[x]!=y)OP=1,upd(y),OP=2,upd(y),cmx(ans[x],ans[y]);
	cmx(ans[x],mxl-2*d[x]);
}
//222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222//
/*default-var-------------------------------------------------------------------------------------*/
