...
void solve() {
    ...
    REP(i,m){
        ...
        cin >> a;
        if(sum > 0){
            int remains = (a - maxs + sum - 1) / sum;
            remains = max(remains,0LL);
            ans[i] = remains * n;
            queries.push_back(mp(a - sum * remains,i));
        }else{
            queries.push_back(mp(a,i));
        }
    }
    sort(ALL(queries));
    reverse(ALL(queries));
    ...
    REP(i,n){
        now += inputs[i];
        while(queries.empty() == false and queries.back().first <= now){
            ans[queries.back().second] += i+1;
            queries.pop_back();
        }
    }
    REP(i,ans.size()){
