...
mint solve( const VI &es ) {
    ...
    djs.init(SZ(v));
    for ( int i:es ) {
        int a=lower_bound(ALL(v),ea[i])-v.begin();
        int b=lower_bound(ALL(v),eb[i])-v.begin();
        djs.u(a,b);
    }
    int cnt=0;
    REP(i,SZ(v)) if ( djs.is_root(i) ) cnt++;
    return mint(2).pow(n-SZ(v)+cnt);
}
...
void main() {
    ...
    for ( auto &i:es ) sort_uniq(i.second);
    mint ans=0;
    ans+=mint(2).pow(n)*(mint(2).pow(k)-SZ(es));
    for ( auto &i:es ) ans+=solve(i.second);
    W(ans);
}
