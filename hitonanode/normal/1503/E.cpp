...
int main() {
    ...
    ncr.assign(H + W + 10, vector<mint>(H + W + 10));
    REP(i, ncr.size()) ncr[i][0] = 1;
    FOR(i, 1, ncr.size()) FOR(j, 1, ncr[i].size()) ncr[i][j] = ncr[i - 1][j] + ncr[i - 1][j - 1];

    auto p = [&](int i, int j) { return ncr[i + j][min(i, j)]; };

    auto solve = [&](int H, int W) -> mint {
        mint ret = 0;
        FOR(h, 1, H) {
            mint tmp = 0;
            REP(w, W) {
                ret += tmp * p(H - 1 - h, w) * p(H - h, W - 1 - w);
                if (w) tmp += p(h, w - 1) * p(h - 1, W - w);
            }
        }
        return ret;
    };

    mint ret = solve(H, W) + solve(W, H);

    REP(i, H - 1) REP(j, W - 1) {
        ret += p(i, j + 1) * p(i + 1, W - 1 - (j + 1)) * p(H - 1 - i, j) * p(H - 2 - i, W - j - 1);
    }
