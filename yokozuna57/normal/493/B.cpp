...
int main(){
	...
	if(a == b){
		for(int i = 0 ; i < min(vec_a.size(),vec_b.size()) ; i++){
			if(vec_a[i] > vec_b[i]){ puts("first"); return 0; }
			if(vec_a[i] < vec_b[i]){ puts("second"); return 0; }
		}
		if(x > 0){ puts("first"); return 0; }
		else { puts("second"); return 0; }
	}
	if(a > b){ puts("first"); return 0; }
	else { puts("second"); return 0; }
}