...
void build(ll v,ll tl,ll tr,ll st[][2][2],ll a[])
{
    if(tl==tr)
    {
        st[v][0][0]=st[v][0][1]=0;
        st[v][1][0]=st[v][1][1]=a[tl];
        return;
    }
    build((v<<1),tl,((tl+tr)>>1),st,a);
    build((v<<1)|1,((tl+tr)>>1)+1,tr,st,a);
    st[v][0][0]=min(st[(v<<1)][0][0]+st[(v<<1)|1][0][0],st[(v<<1)][1][0]-st[(v<<1)|1][1][1]);
    st[v][0][1]=max(st[(v<<1)][0][1]+st[(v<<1)|1][0][1],st[(v<<1)][1][1]-st[(v<<1)|1][1][0]);
    st[v][1][0]=min(st[(v<<1)][0][0]+st[(v<<1)|1][1][0],st[(v<<1)][1][0]-st[(v<<1)|1][0][1]);
    st[v][1][1]=max(st[(v<<1)][0][1]+st[(v<<1)|1][1][1],st[(v<<1)][1][1]-st[(v<<1)|1][0][0]);
    return;
}


void update(ll v,ll tl,ll tr,ll pos,ll val,ll st[][2][2])
{
    if(pos<tl||pos>tr)
    {
        return;
...
int main()
{
    ...
    */
    ...
    static const ll N=300005;
    static ll ntc,n,q,t,x,l,r,a[N],st[N<<2][2][2];
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,q)
        {
            ...
            update(1,0,n-1,l,a[r],st);
            update(1,0,n-1,r,a[l],st);
