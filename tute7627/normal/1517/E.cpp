...
void solve(){
  ...
  rep(i,0,n)limit+=a[i];
  limit=(limit+1)/2;//
  auto cum=vec(2,n+1,0LL);
  rep(i,0,n){
    cum[i&1][i+1]+=a[i];
    rep(j,0,2)cum[j][i+1]+=cum[j][i];
  }
  modint ret=0;
  //cout<<limit<<endl;
  {
    ...
    rrep(i,0,n+1){
      if(i<n)csum+=a[i];
      //cout<<i spa n-i spa csum<<endl;
      if(csum<limit&&(i>=2||(n-i)>=2))ret+=1;
    }
  }
  //cout<<ret<<endl;
  rep(st,0,2){
    ...
    rep(i,st+1,n+1){
      rep(last,0,2){
        ...
        if(i==n&&i-st>=2)continue;
        ll nc=csum+a[n-1]*last;
        if(nc>=limit)continue;
        ll over=limit-nc+cum[~i&1][i];
        ll r=lower_bound(ALL(cum[~i&1]),over)-cum[~i&1].begin();
        r--;
        //cout<<st spa i spa csum spa nc spa over spa r<<endl;
        if(last==1)chmin(r,n-3);
        //cout<<r spa (r-i+2)/2<<endl;
        ret+=max(0LL,(r-i+2)/2);
      }
      if(i!=n)csum+=a[i];
    }
