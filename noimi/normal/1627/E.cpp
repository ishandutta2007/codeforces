...
int main() {
    TEST {
        INT(n, m, k);
        VEC(ll, x, n);
        vvc<tuple<int, int, int, ll>> v(n);
        vvc<pair<int, ll>> w(n + 1);
        rep(i, k) {
            INT(a, b, c, d, h);
            a--, b--, c--, d--;
            v[a].eb(b, c, d, h);
        ...
        rep(i, n) {
            ...
            rep(2) {
                int l = 0, r = 0;
                ll mi = inf<ll>;
                while(l < si(w[i]) or r < si(v[i])) {
                    if(l == si(w[i]) or (r < si(v[i]) and get<0>(v[i][r]) < w[i][l].fi)) {
                        ll c = mi + x[i] * get<0>(v[i][r]);
                        if(c < ten(16)) {
                            auto [B, C, D, h] = v[i][r];
                            w[C].eb(D, c - h);
                        }
                        ...
                        chmin(mi, w[i][l].se - w[i][l].fi * x[i]);
                        l++;
                        // dump(l, r, mi);
                    }
                }
                fore(x, y, w[i]) x = m - 1 - x;
                fore(b, c, d, h, v[i]) b = m - 1 - b;
                REV(w[i]), REV(v[i]);
            }
        }
        if(si(w[n])) {
