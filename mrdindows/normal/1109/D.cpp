...
ll foo(int n, int k) {
    if (n == 0) return 1;
    ll res = k * 1LL * powmod(n + k, n - 1, mod) % mod;
//    if (res != dp[n][k]) {
//        DBN(n, k, res, dp[n][k]);
//    }
    return res;
...
//                dp[n][k] += dp[n - last][k - 1] * cnk(n, last) * kely(last + 1);
//            }
//            cout << dp[n][k] << ' ';
//            cout << k * power(n + k, n - 1) << endl;
////            DBN(n, k, dp[n][k]);
////            res[n + k][k] = dp[n][k];
//        }
//    } cout << endl;
...
//    } cout << endl;
    ...
    for (int i = 0; i <= min(n - 2, m - 1); ++i) {
        ll t = cnk(n - 2, i) * fact[i] % mod * cnk(m - 1, i) % mod;
        t = t * powmod(m, n - 1 - (i + 1), mod) % mod;
        int free = n - 2 - i;
//        DBN(i, t, free, i + 2, dp[free][i + 2]);
//        ans += t * dp[free][i + 2] % mod;
        ans += t * foo(free, i + 2) % mod;
    }
    cout << ans % mod << endl;
    return 0;
}