...
void solve() {
  ...
  string ANS(N, '?');
  FOR(i, N) {
    if (r && S[i] == 'S') ANS[i] = 'R', --r;
    if (p && S[i] == 'R') ANS[i] = 'P', --p;
    if (s && S[i] == 'P') ANS[i] = 'S', --s;
  }
  ll win = (N - r - p - s);
  if (win < ceil(N, 2)) return NO();
  YES();
  FOR(i, N) if (r && ANS[i] == '?') ANS[i] = 'R', --r;
  FOR(i, N) if (p && ANS[i] == '?') ANS[i] = 'P', --p;
  FOR(i, N) if (s && ANS[i] == '?') ANS[i] = 'S', --s;
  print(ANS);
}
