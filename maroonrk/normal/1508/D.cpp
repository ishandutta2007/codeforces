...
void slv(){
	...
	if(root!=-1){
		pi z=xy[root];
		rep(i,n)xy[i]=sub(xy[i],z);
		vi idx;
		rep(i,n)if(i!=root)idx.pb(i);
		sort(all(idx),[&](int i,int j){return argless(xy[i],xy[j]);});
		unionfind uf(n);
		rep(i,n)uf.unite(i,a[i]);
		rep(ii,si(idx)){
			int u=idx[ii],v=idx[(ii+1)%si(idx)];
			if(ccw(xy[u],xy[v])>0){
				if(uf.unite(u,v)){
					add(u,v);
				}
