...
int main() {
    ...
    VEC(int, a, n);
    s--;
    rep(i, n) if(i != s) ret.emplace(a[i], i);
    vector<tuple<int, int, int>> query;
    rep(i, q) {
        INT(j, k);
        query.emplace_back(k, j, i);
    }
    vi ans(q);
    sort(all(query));
    ...
    auto add = REC([&](auto &&f, int i) -> void {
        for(auto D : {d, -d}) {
            while(1) {
                auto it = ret.lower_bound(pii(a[i] + D, 0));
                if(it == end(ret)) break;
                if(it->fi > a[i] + D + k) {
                    pq.emplace(it->fi - a[i] - D, it->se, i);
                    break;
                } else {
                    int mem = it->se;
                    ret.erase(it);
            ...
            while(1) {
                auto it = ret.lower_bound(pii(a[i] + D, 0));
                if(it == begin(ret)) break;
                it = prev(it);
                if(it->fi < a[i] + D - k) {
                    pq.emplace(a[i] + D - it->fi, it->se, i);
                    break;
                } else {
                    int mem = it->se;
                    ret.erase(it);
    ...
    for(auto [_k, to, idx] : query) {
        k = _k;
        while(!pq.empty()) {
            auto [kk, u, v] = pq.top();
            if(kk > k) break;
            auto it = ret.find(pii(a[u], u));
            pq.pop();
            if(it == end(ret)) continue;
            ret.erase(it);
            add(u), add(v);
        }
        ans[idx] = (!ret.count(pii(a[to - 1], to - 1)));
    }
