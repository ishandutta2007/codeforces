...
void transpose(vector<vector<bool>> &w) {
    vector<vector<bool>> ans(sz(w[0]), vector<bool>(sz(w)));
...
zet solve(const vector<vector<bool>> &w) {
    zet ans = 0;
    int cntall = 0;
    for (int i = 0; i < sz(w); ++i)
        for (int j = 0; j < sz(w[i]); ++j) {
    ...
    for (int i = 0; i < sz(w); ++i) {
        for (int j = 0; j < sz(w[i]); ) {
            ...
            int ptr = j;
            while (ptr < sz(w[i]) && w[i][ptr] == 1) {
                ++ptr;
            }
            int len = ptr - j;
            for (int l = 2; l <= len; ++l) {
                ans += mypow(zet(2), cntall - l) * (len - l + 1) * (l & 1 ? -1 : 1);
            }
            j = ptr;
        }
    }
