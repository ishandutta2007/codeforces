...
for(int i=n;i>=1;i--){
	sum2+=a[i];
	mp2[sum2]++;
	tmp=tmp/(modint)C(mp1[sum2]+mp2[sum2]-1,mp1[sum2])*(modint)C(mp1[sum2]+mp2[sum2],mp1[sum2]);
	mp1[sum]--;
    tmp=tmp/(modint)C(mp1[sum]+1+mp2[sum],mp1[sum]+1)*(modint)C(mp1[sum]+mp2[sum],mp1[sum]);
	sum-=a[i];
	if(!mp1[sum2]) continue;
	tmp=tmp/(modint)C(mp1[sum2]+mp2[sum2],mp1[sum2])*(modint)C(mp1[sum2]+mp2[sum2]-1,mp2[sum2]);
	ans+=tmp;
	tmp=tmp/(modint)C(mp1[sum2]+mp2[sum2]-1,mp2[sum2])*(modint)C(mp1[sum2]+mp2[sum2],mp1[sum2]);
}
cout<<ans<<endl;
#endif
}
