...
bool solve() {
    ...
    auto adj = [&](int i, int j) {
        vector<pii> f;
        for (auto [x, y] : vpii{{i + 1, j}, {i - 1, j}, {i, j - 1}, {i, j + 1}}) {
            if (x < 0 || x >= n || y < 0 || y >= m)
                continue;
            f.push_back({x, y});
        }
    ...
    // replace everything adjacent to B with X
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (g[i][j] != B)
                continue;
    ...
    auto bfs = [&](set<int> walls) {
        vvb good(n, vb(m, false));
        queue<pii> q;

        auto process = [&](int i, int j) {
            if (walls.count(g[i][j]))
                return;
            q.push({i, j});
            good[i][j] = true;
        };
    ...
    // we need paths to G without crossing X or B
    auto good = bfs({X, B});

    // check that all G are still reachable
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (g[i][j] != G)
                continue;
