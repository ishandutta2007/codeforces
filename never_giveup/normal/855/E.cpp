...
ll calc(int B, ll value){
    if (value <= B)
        return 0;
    vector<int> g;
    while (value)
        g.push_back(value % B), value /= B;
    reverse(g.begin(), g.end());
    int n = g.size();
    ll answer = 0;
    for (int i = 2; i < n; i++)
        answer += q[B - 1][i][0] - q[B - 1][i - 1][1];
    int mask = 0;
    for (int same = 0; same < n; same++){
        for (int now = (same ? 0 : 1); now < g[same]; now++)
            answer += q[B - 1][n - same - 1][mask ^ (1 << now)];
        mask ^= (1 << g[same]);
    }
    if (mask == 0)
        answer++;
