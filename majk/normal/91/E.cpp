...
namespace LinearEnvelope {
	...
	struct Upper : public vector<Line<T>> {
		...
		void insert_line(T m, T b, int i = 0) {
			while (M >= 1 && ((at(M-1).b < b) || (at(M-1).b == b && at(M-1).m < m))) {
				parent::pop_back();
				M -= 1;
			}

			while (M >= 2 && fractionGreaterOrEqual(at(M-2).b - at(M-1).b, at(M-1).m - at(M-2).m, at(M-1).b - b, m - at(M-1).m)) {
				parent::pop_back();
				M -= 1;
			}
		...
		pair<T,int> advance(T x) {
			t += x;
			while (i+1 < M && at(i).m * t + at(i).b < at(i+1).m * t + at(i+1).b) ++i;
