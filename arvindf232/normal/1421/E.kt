...
fun main(){
    solve.cases{
        ...
        var DP = Array(2){LongArray(3){nhuge}}
        DP[0][0] = 0L
        for(i in 0 until n) {
            val newDP = Array(2){LongArray(3){nhuge} }
            val v = L[i]
            for(a in 0 until 2){
                for(b in 0 until 3) {
                    val here = DP[a][b]
                    for(other in 0 until 3){
                        if(other == b) continue
                        val dif = (other - b + 3)  % 3
                        val new = if(dif == 1){here + v} else here - v
                        val newa = if(b == 2) 1 else a
                        newDP[newa][other] = maxOf(newDP[newa][other], new)
                    }
                }
            }
            DP = newDP
