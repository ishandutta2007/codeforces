...
ll n;
vector<Q> G[200005];
map<ll, ll> mp, tmp, tmp2;
mint ans;

void dfs(int v, int p, mint m)
{
	ans += m;
	
	for(auto q : G[v]){
		ll u = q.fi, a = q.se.fi, b = q.se.se;
		if(u == p) continue;
		
		vector<ll> avec; ll t = a;
		while(prime[t]){
			avec.pb(prime[t]);
			t /= prime[t];
		}
		...
		for(auto x : bvec) tmp2[x]++;
		for(auto x : avec) chmax(mp[x], max(0LL, (++tmp[x]) - tmp2[x]));
		dfs(u, v, m/a*b);
		for(auto x : avec) tmp[x]--;
		for(auto x : bvec) tmp2[x]--;
	}
}
...
int main(void)
{
	...
	while(T--){
		...
		mp.clear(), tmp.clear(), ans = 0;
		dfs(1, -1, 1);
		
		//outl(mp);
		
		for(auto p : mp) ans *= modpow(p.fi, p.se);
		outl(ans);
	}
