...
int main() {
	...
	for (int mid1 = l/2, mid2 = (l+1)/2; ; mid1--, mid2++) {
		bool is1Valid = (s[mid1] != '0');
		bool is2Valid = (s[mid2] != '0');
		
		if (is1Valid || is2Valid) {
			if (is1Valid && is2Valid) {
				string ans1 = sum(s.substr(0, mid1), s.substr(mid1, l-mid1));
				string ans2 = sum(s.substr(0, mid2), s.substr(mid2, l-mid2));
				...
				string ans;
				if (is1Valid)
					ans = sum(s.substr(0, mid1), s.substr(mid1, l-mid1));
				else
					ans = sum(s.substr(0, mid2), s.substr(mid2, l-mid2));
				W(ans);
				return 0;
			}
		}
