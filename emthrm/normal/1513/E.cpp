...
int main() {
  ...
  ModInt::init(n);
  if (sm <= 1 || la <= 1) {
    ModInt ans = ModInt::fact(n);
    for (auto [_, v] : mp) ans *= ModInt::fact_inv(v);
    cout << ans << '\n';
    return 0;
  }
  ModInt sm_arr = ModInt::fact(sm), la_arr = ModInt::fact(la);
  for (auto [k, v] : mp) {
    if (k < ave) {
      sm_arr *= ModInt::fact_inv(v);
    } else if (k > ave) {
      la_arr *= ModInt::fact_inv(v);
    }
  }
  ModInt ans = 0;
  for (int i = sm; n - i >= la; ++i) {
    ans += ModInt::nCk(i - 1, sm - 1) * sm_arr * ModInt::nCk(n - i, la) * la_arr;
  }
  for (int i = la; n - i >= sm; ++i) {
    ans += ModInt::nCk(i - 1, la - 1) * la_arr * ModInt::nCk(n - i, sm) * sm_arr;
  }
  cout << ans << '\n';
  return 0;
}