...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> re(n); iota(all(re),0);
	sort(all(re),[&](cauto &i,cauto &j){ return ps[i]<ps[j];});
	srand(time(0));
	while(1){
		rep(i,n-2){
			cauto [ax,ay]=ps[re[i]]-ps[re[i+1]];
			cauto [bx,by]=ps[re[i+2]]-ps[re[i+1]];
			if(ax*bx+ay*by<=0) swap(re[i+1],re[i+2]);
			rrep(j,i){
				cauto [ax,ay]=ps[re[j]]-ps[re[j+1]];
				cauto [bx,by]=ps[re[j+2]]-ps[re[j+1]];
				if(ax*bx+ay*by>0) break;
				swap(re[j+1],re[j+2]);
			}
		}
		int ok=1;
		rep(i,n-2){
			cauto [ax,ay]=ps[re[i]]-ps[re[i+1]];
			cauto [bx,by]=ps[re[i+2]]-ps[re[i+1]];
			if(ax*bx+ay*by==0){ ok=0; break;}
		}
		if(ok) break;
		random_shuffle(all(re));
	}
	rep(i,n) cout<<re[i]+1<<" "; cout NL;
  return 0;
}