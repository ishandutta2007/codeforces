...
public class TaskA {
	...
	private void solve() {
		...
		char[] relation = in.readString().toCharArray();
		int[] candies = new int[studentCount];
		...
		while (true) {
			...
			for (int i = 1; i < studentCount; i++) {
				...
				} else if (relation[i - 1] == 'L') {
					if (candies[i - 1] <= candies[i]) {
						candies[i - 1] = candies[i] + 1;
						updated = true;
					}
				} else if (relation[i - 1] == 'R') {
					if (candies[i - 1] >= candies[i]) {
	...
	private static List<Test> createTests() {
		...
		tests.add(new Test("5\n" +
			"=RRR", "1 1 2 3 4 "));
		tests.add(new Test(generateTest(), generateResult()));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
