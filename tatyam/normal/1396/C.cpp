...
signed main(){
    ...
    vec(ll,kill,n);
    vec(ll,kill2,n-1);
    back.back()=min({p*a.back()+w,p*(a.back()+2)+t*2,l+p+t*2});
    rrep(n-1)back[i]=back[i+1]+t*2+min({p*a[i]+w,p*(a[i]+2),l+p});
    rep(n)kill[i]=min({p*a[i]+w,p*(a[i]+2)+t*2,l+p+t*2})+t;
    rep(n-1)kill2[i]=min({p*a[i]+w,p*(a[i]+2),l+p})+min({p*a[i+1]+w,p*(a[i+1]+2),l+p})+t*4;
    kill.back()-=t;
    kill2.back()-=t;
    kill.insert(kill.begin(),0);
    rep(n){
        kill[i+1]+=kill[i];
        if(i)chmin(kill[i+1],kill[i-1]+kill2[i-1]);
    }
    back.push_back(0);
    ll ans=LINF;
    rep(n+1)chmin(ans,kill[i]+back[i]);
    out(ans);
}