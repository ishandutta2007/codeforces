...
public:
    void solve() {
        int n = in.readInt();
        auto commands = in.readStringArray(n);

        arr<pair<string, bool>> unlocked(4 * n, make_pair("", false));
        arr<pair<string, bool>> locked(4 * n, make_pair("", false));
        auto apply = [&](pair<string, bool>& state, int root) -> void {
            if (state.second) {
                if (locked[root].first != "none") {
                    state.first = locked[root].first;
                }
                state.second = locked[root].second;
            } else {
                if (unlocked[root].first != "none") {
                    state.first = unlocked[root].first;
                }
                state.second = unlocked[root].second;
            }
        };
        auto initPoint = [&](int root, int left) -> void {
            locked[root].first = "none";
            locked[root].second = true;
            unlocked[root].first = commands[left];
            unlocked[root].second = false;
            if (commands[left] == "lock") {
                unlocked[root].first = "none";
                unlocked[root].second = true;
            } else if (commands[left] == "unlock") {
                unlocked[root].first = "none";
                locked[root].second = false;
            }
        };
        auto doUpdate = [&](int root) -> void {
            locked[root] = locked[2 * root + 1];
            apply(locked[root], 2 * root + 2);
            unlocked[root] = unlocked[2 * root + 1];
            apply(unlocked[root], 2 * root + 2);
        };
        ...
        init(0, 0, n);
        RecursiveFunction update = [&](const auto& self, int root, int left, int right, int at) -> void {
            int mid = (left + right) / 2;
            if (left + 1 == right) {
                initPoint(root, left);
                return;
        ...
        auto printAnswer = [&]() -> void {
            pair<string, bool> base("blue", false);
            apply(base, 0);
            out.printLine(base.first);
        };
        printAnswer();
