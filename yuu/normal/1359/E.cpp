...
// (x % a) % b == (x % b) % a for a < b
// then (x % a) % b == (x % b) % a = (x % a)
// (x % b) % a == (x % a)
// x % b = a * k + (x % a)
// x = a * h + (x % a)
// x = b * l + (x % b)
// x = b * l + a * k + (x % a) = a * h + (x % a)
// b * l = a * (h - k)
// l can be any number since x can be any number, b * l / a must always be integer, or b must be divisible by a
// generalisation: every number must be divisible by the smallest one
...
void solve() {
    ...
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) (ans += C(n / i - 1, k - 1)) %= base;
    cout << ans << '\n';
}
