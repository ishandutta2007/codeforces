...
int main() {
	...
	set <int> active;
	REP(_, n) {
		debug(imie(timer), imie(active));
		if (active.empty() && in_queue.empty() && pos < n)
			maxi(timer, ord[pos].first);
		while (pos < n && ord[pos].first < timer) {
			debug(arr(ord, pos));
			if (in_queue.empty() || ord[pos].second < *in_queue.begin()) {
				debug("to queue");
				in_queue.insert(ord[pos].second);
				kol.push(ord[pos].second);
			}
			else {
				debug("to active");
				active.insert(ord[pos].second);
			}
			pos++;
		}
		bool wake_up_one = true;
		if (active.empty()) wake_up_one = false;
		else if (pos == n || ord[pos].first > timer) wake_up_one = true;
		else wake_up_one = *active.begin() < ord[pos].second;
		if (!active.empty() && !in_queue.empty() && *in_queue.begin() <= *active.begin())
			wake_up_one = false;
		if (wake_up_one) {
			int ins = *active.begin();
			debug("wake up ", imie(ins), range(ord.begin() + pos, ord.end()), imie(in_queue));
			active.erase(ins);
			kol.push(ins);
			in_queue.insert(ins);
		}
		...
		in_queue.erase(serve);
	}
	assert(in_queue.empty() && kol.empty() && active.empty());
