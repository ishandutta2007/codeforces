...
int main() {
	...
	scanf("%s", in);
	vector <int> edges;
	for (int i = 0; i < n; ++i) if (in[i] == in[(i + n - 1) % n]) edges.push_back(i);
	if (edges.empty()) {
		if (k % 2) for (int i = 0; i < n; ++i) in[i] ^= 'W' ^ 'B';
		printf("%s\n", in);
		return 0;
	}
	for (int j = 0; j < (int)edges.size(); ++j) {
		int b = edges[j], e = edges[(j + 1) % edges.size()];
		int le = e - b;
		if (le <= 0) le += n;
		int pre = min(k + 1, (le + 1) / 2);
		for (int i = 0; i < pre; ++i)
			in[(b + i) % n] = in[b];
		for (int i = le - pre; i < le; ++i)
			in[(b + i) % n] = in[(b + le - 1) % n];
		for (int i = pre; i < le - pre; ++i)
			in[(b + i) % n] = in[(b + i - 1) % n] ^ 'B' ^ 'W';
	}
	printf("%s\n", in);
}