...
inline bool lt(long double a, long double b){
	return (a + EPS < b);
}
...
inline bool gt(long double a, long double b){
	return lt(b, a);
}
...
void solve(){
	...
	cin >> a;
	long double prlo = lo, prhi = lo;
	while(lt(lo, hi)){
		if (eq(prlo,lo) && eq(prhi,hi)){
			break;
		}
