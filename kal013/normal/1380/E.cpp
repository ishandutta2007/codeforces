...
const int N = 2e5 + 10;
set<pair<int,int>> dsu[N];

int pos[N];

void insert_range(set<pair<int,int>>& A,pair<int,int> U){
	auto it = A.upper_bound(U);
	if (it != A.end()){
		auto X = *it;
		if (X.first == U.second + 1){
			U.second = X.second;
			A.erase(it);
		}
	}
...
void solve(){
    ...
    for(int i = 1; i < m; ++i){
    	...
    	if (dsu[pa].size() < dsu[pb].size()){
    		swap(pa,pb);
    	}
    	num -= dsu[pa].size() + dsu[pb].size();
    	for(auto e: dsu[pb]){
    		insert_range(dsu[pa],e);	
    	}
    	dsu[pb].clear();
