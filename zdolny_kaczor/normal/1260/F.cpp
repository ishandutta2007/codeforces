...
// void get_rec_r(vector <ll> &ans, node *n, int l, int r, int lb, int rb) {
	// if (l >= rb || r <= lb) return;
	// if (n->l) {
		// int sb = (lb + rb) / 2;
		// get_rec_r(ans, n->l, l, r, lb, sb);
		// get_rec_r(ans, n->r, l, r, sb, rb);
	// }
	// else {
		// for (int i = max(l, lb); i < min(r, rb); ++i)
			// ans.push_back(n->val);
	// }
// }
vector <ll> get_r(node *n, int l, int r) {
	vector <ll> ans;
	// get_rec_r(ans, n, l, r, 0, nax);
	rans;
}
...
#define node_range(n, i, j) #n "[" + bte(imie(i)) + ".." + bte(imie(j)) +  ") = " + bte(get_r(n, yyo(i), yyo(j)))
ll ans = 0;
int dep[nax];
node *tree[nax];
ll inv[nax];
