    ...
    for (int sz = 1; sz <= 4; ++sz) {
        for (int i = 0; i + sz <= 4; ++i) {
            for (int j = 0; j + sz <= 4; ++j) {
                ...
                if (i == 0) {
                    ++tot;
                    mmoves[sz].push_back(~move);
                }
                moves[sz].push_back(~move);
            }
        }
    }
//    DBN(tot);
    ...
    dp[0][a[0] + (a[1] << 4) + (a[2] << 8) + (a[3] << 12)] = 0;
    for (int i = 0; i < n - 3; ++i) {
        for (int mask = (1 << 16) - 1; mask >= 0; --mask) {
            int dpCur = dp[i][mask];
            if (dpCur == INF) continue;
            if (good(mask)) {
                int &tmp = dp[i + 1][(mask >> 4) + (a[i + 4] << 12)];
                tmp = min(tmp, dpCur);
            }
            if (i < n - 8) {
                for (int k = 1; k <= 4; ++k) {
                    for (int move : mmoves[k]) {
                        int to = mask & move;
    //                    DBN(mask, to, dpCur + c[k]);
                        dp[i][to] = min(dp[i][to], dpCur + c[k]);
                    }
                }
            } else {
                for (int k = 1; k <= 4; ++k) {
                    for (int move : moves[k]) {
                        int to = mask & move;
                        //                    DBN(mask, to, dpCur + c[k]);
                        dp[i][to] = min(dp[i][to], dpCur + c[k]);
                    }
