...
ll new_val[MAX_N], total_anak[MAX_N];
int par[MAX_N], child_count[MAX_N], locc[MAX_N];
bool vis[MAX_N];
...
void dfs(int u, int par, int len) {
	...
	for (const auto& v : adj[u]) {
		if (v != par) {
			...
			total_anak[u] += new_val[v];
		}
	}
	new_val[u] = 1LL * (1LL + child_count[u]) * len;
}
...
int main(int argc, char** argv) {
	...
	ll ans = 0LL;
	FORN (k, 1, K) {
		const pll val = take(PQ);
		ans += val.first;
		const int& par = ::par[val.second];
		if (par != -1) {
			--locc[par];
			if (locc[par] == 0LL) {
				PQ.push(mp(new_val[par] - total_anak[par], par));
			}
		}
	}
	cout << ans << "\n";
