...
fun main(){
    solve.cases{
		...
		repeat(n){
			val str = getstr
			var aa = str.count{it == 'B'}
			val bb = str.count{it == 'N'}
			A[it] = aa
			B[it] = bb
		}

		fun solve(t:Int):Pair<Int,Int>?{
			var maxx = plarge
			var maxy = plarge
			var maxd = plarge
			var minx = 0
			...
			for(i in 0 until n){
				...
				maxx = minOf(maxx, a + t)
				maxy = minOf(maxy, b + t)
				maxd = minOf(maxd, b-a + t)
				minx = maxOf(minx, a -t)
				miny = maxOf(miny , b- t)
				mind = maxOf(mind, b-a-t)
			}
			if(maxx < minx || maxy < miny || maxd < mind) {
				return null
			}
			for(x in minx..maxx) {
				var aminy = if(x == 0) 1 else 0
				var amaxy = plarge
				aminy = maxOf(aminy, miny )
				amaxy = minOf(amaxy, maxy )
				aminy = maxOf(aminy, mind + x )
				amaxy = minOf(amaxy, maxd + x)
				if(aminy <= amaxy){
					return Pair(x,aminy)
				}
			}
		...
		val x = solve(1)
		val d = BinarySearchFirstTrue(0,500000){
			solve(it)!= null
		}!!
