...
int main() {
    ...
    ll pt = 1;
    vector<ll> v, va = {1}, vr = {1};
    for (auto p : mapp) {
        pt = (pt * (p.second + 1)) % (mod - 1);
        v.push_back(p.second + 1);
    }
    for (int i = 0; i < v.size(); ++i) {
        va.push_back(va.back() * v[i] % (mod - 1));
        vr.push_back(vr.back() * v[v.size() - i - 1] % (mod - 1));
    }
    int k = 0;
    for (auto p : mapp) {
        ++k;
        ll t = p.second * (p.second + 1ll) / 2 % (mod - 1);
        ans = ans * powmod(p.first, t * va[k - 1] % (mod - 1) * vr[v.size() - k] % (mod - 1), mod) % mod;
    }
    cout << ans << endl;
    return 0;
}