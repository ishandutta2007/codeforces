...
int main() {
	...
	vector<pair<pii,int>> castle(5001);
	for (int i = 1; i <= n; i++) {
		int a,b,c; R(a,b,c);
		castle[i] = {{a,b},c};
	}

	vector<int> portal(5001);
	for (int i = 1; i <= n; i++)
		portal[i] = i;
	...
	vector<vector<int>> to(5001);
	for (int i = 1; i <= n; i++)
		to[portal[i]].pb(i);

	for (int i = 1; i <= 5000; i++)
		sort(all(to[i]), [&](int a, int b) {return castle[a].S > castle[b].S;});
	...
	for (int i = 1; i <= n; i++) {
		...
		k += b;
		for (int j = 0; j < to[i].size() && k >= 1; j++) {
			ans += castle[to[i][j]].S;
			defend.insert(castle[to[i][j]].S);
			k--;
		}
	}
