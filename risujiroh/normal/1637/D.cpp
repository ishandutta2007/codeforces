...
#define let auto const
  ...
  let b = input<vla<int>>(n);
  auto f = bitset<10001>();
  f[0] = 1;
  for (let i : rep(n)) { f = f << a[i] | f << b[i]; }
  let s = i64(all(a).fold(0) + all(b).fold(0));
  auto ans = inf_v<i64>;
  for (let i : rep<i64>(10001).filter(LAMBDA(f[_1]))) { chmin(ans, i * i + (s - i) * (s - i)); }
  ans += (n - 2) * (all(a).map(_1 * _1).fold(i64(0)) + all(b).map(_1 * _1).fold(i64(0)));
  println(ans);
}