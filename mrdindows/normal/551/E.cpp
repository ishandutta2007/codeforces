...
struct Block{
    ...
    void plus(int lp, int rp, ll what)
    {
        if (lp < l) {
            lp = l;
        }
        ...
        for (int i = 0; i < size; ++i) {
            if (arr[i].second >= lp && arr[i].second <= rp) {
                arr[i].first += what;
            }
        }
        sort();
    ...
    pii find(ll y) {
        ...
        y -= bonus;
        int l = 0; int r = size;
        if (arr[0].first > y || arr[size - 1].first < y) return {-1, -1};
        while (l < r) {
            int m = (l + r) / 2;
            if (arr[m].first >= y) {
                r = m;
...
int main()
{
    ...
    for (int i = 0; i < n; i += B) {
        int &l = blocks[bk].l = i;
        int &r = blocks[bk].r = min(n, i + B) - 1;
        blocks[bk].size = r - l + 1;
        
        for (int j = l; j <= r; ++j) {
            blocks[bk].arr[j - l] = {a[j], j};
        }
        blocks[bk].sort();
        ++bk;
    }
    while(q --){
        ...
        if (t == 2) {
            ...
            int maxp = -1;
            for (int i = 0; i < bk; ++i) {
                auto res = blocks[i].find(x);
                if (res.second == -1) continue;
                minp = min(minp, res.first);
                maxp = max(maxp, res.second);
            }
            if (maxp == -1) {
                cout << "-1\n";
            } else {
