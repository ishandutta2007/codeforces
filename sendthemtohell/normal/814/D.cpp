...
typedef pair<pair<int, int>, pair<int, int>> ppii;

ld S(ppii crc) {
	return (ld)crc.y.x * (ld)crc.y.x * PI;
}

inline bool operator^(ppii a, ppii b) {
	ll dst = ((ll)a.x.x - b.x.x) * ((ll)a.x.x - b.x.x) + ((ll)a.x.y - b.x.y) * ((ll)a.x.y - b.x.y);
	return dst < (ll)b.y.x * b.y.x;
}

inline bool cmp(ppii a, ppii b) {
	return a.y.x > b.y.x;
}
