...
int main(){
	...
	for(int i=0;i<SZ(nums)-1;i++){
		gc=gcd(nums[i+1]-nums[i],gc);
	}
	int mxi=0;
	for(int i=1;i*i<=gc;i++){
		if(gc%i==0){
			mxi=i;
			if(r-i*(n-1)<=nums[0]&&r-i*(n-1)>=0){
				fio<<"! "<<r-i*(n-1)<<' '<<i<<endl;shit;
				return 0;
			}
		}
	}
	for(int i=mxi;i>=1;i--){
		if(gc%i==0){
			...
			if(r-(gc/i)*(n-1)<=nums[0]&&r-(gc/i)*(n-1)>=0){
				fio<<"! "<<r-(gc/i)*(n-1)<<' '<<(gc/i)<<endl;shit;
				return 0;
			}
		}
