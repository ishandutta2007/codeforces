...
int solve(){
 		...
 		rep(i,1,n+1){
 			rep(j,1,m+1){
 				if(i%2==j%2){
 					ms.add_edge(s,get(i,j),1,0);
 					for(int k=0;k<4;k++){
 						int xd=i+dir[k].x;
 						int yd=j+dir[k].y;
 						if(xd>=1 && xd<=n && yd<=m && yd>=1){
 							ms.add_edge(get(i,j),get(xd,yd),1,a[i][j]!=a[xd][yd]);
 						}
 					}
 				}
 				else{
 					ms.add_edge(get(i,j),t,1,0);
 			}	}
 		}
 		auto ans = ms.minCostFlow();
 		cout<<ans.y<<endl;
 return 0;
}
signed main(){
