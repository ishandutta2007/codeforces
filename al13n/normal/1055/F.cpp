...
void gen(){
  ...
  cout<<n<<" 500000000000\n";
  forn(i,n-1){
    LL w=std::abs(rand()+(RAND_MAX+1ll)*rand());
    cout<<rand()%(i+1)+1<<' '<<w<<'\n';
  }
}
...
int main(int argc, char **argv){
  ...
  LL K;
  scanf("%d%lld",&n,&K);
  //n=1000000;K=500000000000ll;
  vector<LL> A(n);
  forn(i,n-1){
    int p;
    LL w;
    scanf("%d%lld",&p,&w);
    //p=rand()%(i+1)+1; w=std::abs(rand()+(RAND_MAX+1ll)*rand());
    --p;
    A[i+1]=w^A[p];
  }
  sort(all(A));
  ...
  for(int q=61;q>=0;--q){
    dbg(q);
    LL cnt=0;
    for(int i1=0,i2=0;i1<sz(A)&&i2<sz(T);){
      int ii1=i1+1;
      while(ii1<sz(A) && (A[ii1]>>q)==(A[i1]>>q)) ++ii1;
      while(i2<sz(T) && (T[i2]>>q)<(A[i1]>>q)) ++i2;
      int ii2=i2;
      while(ii2<sz(T) && (T[ii2]>>q)==(A[i1]>>q))++ii2;
      cnt+=1ll*(ii1-i1)*(ii2-i2);
      i1=ii1;
      i2=ii2;
    }
    //dbg(q,R,cnt,T);
