...
void cs(){
	rep(r,1,n)rep(c,1,m){
		...
		if(!inside(r+1,c+1))continue;
		val = 1;
		rep(k,0,7)if(!x[r+dr[k]][c+dc[k]]){
			val = 0; break;
		}
		if(val){
			y[r][c] = 1;
			rep(k,0,7)y[r+dr[k]][c+dc[k]] = 1;
		}
	}
}
void cas(){
	rep(r,1,n)rep(c,1,m){
		...
		if(!inside(r+2,c+2))continue;
		val = 1;
		rep(i,r-2,r+2)rep(j,c-2,c+2)if(!x[i][j]){
			val = 0; break;
		}
		if(val)rep(i,r-2,r+2)rep(j,c-2,c+2)z[i][j] = 1;
	}
