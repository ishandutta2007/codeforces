...
mi c3(mi x) { return x*(x+1)*(x+2)/6; }

int main() {
	setIO();
	re(N);
	dp.rsz(N+1), root.rsz(N+1);
	vmi less(N+1), two(N+1);
	dp[0] = root[0] = 1;
	FOR(i,1,N+1) {
		less[i] = less[i-1]+dp[i-1];

		dp[i] = dp[i-1];
		dp[i] += dp[i-1]*less[i-1]+c2(dp[i-1]);
		root[i] += less[i]-less[i-1];
		root[i] += c2(less[i])-c2(less[i-1]);
		// root[i] = dp[i-1]+dp[i-1]*less[i-1]+c2(dp[i-1]); // two
		root[i] += c3(less[i])-c3(less[i-1]);
		two[i] = c2(less[i])-c2(less[i-1]);
		// root[i] += c3(dp[i-1]);
		// root[i] += c2(dp[i-1])*less[i-1];
		// root[i] += dp[i-1]*c2(less[i-1]);
	}
	// dbg("HA",two[1]); exit(0);
	mi ans = 2*root[N]-1;
	mi cum = 0;
	F0R(len,N-1) cum += two[len];
	FOR(a,1,N) {
		ans += two[a]*cum;
		cum -= two[N-1-a];
	}
	...
	// FOR(len,1,N+1) {
	// 	F0R(j,N-len+1) {
	// 		ans += two[j]*two[N-len-j];
	// 	}
	// }
	ps(ans);
	// dbg(dp);
	// dbg(root);
	// number rooted <= binary trees with height h
