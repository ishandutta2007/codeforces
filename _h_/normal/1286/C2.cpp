...
int main(){
	...
	if(n == 1){
		...
		vector<string> l1(c2(n+1)), l2(c2(n/2+1)), l3(c2(n/2));
		cout << "? " << 1 << " " << n << endl;
		trav(s, l1) cin >> s;
		cout << "? " << 1 << " " << n/2 << endl;
		trav(s, l2) cin >> s;
		cout << "? " << 1 << " " << n/2-1 << endl;
		trav(s, l3) cin >> s;
		string s1 = info(l1, n), s2 = info(l2, n/2), s3 = info(l3, n/2-1);
		string ans(n, '?');
		vi diff(26);
		trav(c, s2) ++diff[c-'a'];
		trav(c, s3) --diff[c-'a'];
		function<void(int,char)> setchar = [&](int i, char c){
			ans[i] = c;
			if(n/2-1-i < i){
				char d = s2[i] ^ s2[n/2-1-i] ^ c;
				setchar(n/2-1-i, d);
			} else if(n/2-2-i > i){
				char d = s3[i] ^ s3[n/2-2-i] ^ c;
				setchar(n/2-2-i, d);
			}
		};
		rep(c,0,26) if(diff[c]) setchar(n/2-1, c+'a');
		rep(i,n/2,n){
			if(i == n-1-i){
				ans[i] = s1[i];
			} else {
				ans[i] = s1[i] ^ s1[n-1-i] ^ ans[n-1-i];
			}
		}
		cout << "! " << ans << endl;
	}
