...
int main() {
    ...
    auto ord = top.sorted(), rev_ord = ord;
    reverse(all(rev_ord));

    vector<int> l(n, 1), r(n, k);
    for (int u : rev_ord) {
        if (a[u] != 0) l[u] = r[u] = a[u];
        for (int v : g[u]) {
            chmax(l[u], l[v] + 1);
        }
    }
    ...
    vector<vector<pair<int, int>>> ranges(k + 1);
    rep(i, n) {
        ranges[r[i]].emplace_back(l[i], i);
    }

    vector<int> ans(n, 1);
    priority_queue<pair<int, int>> pq;
    int nxt = k;
    for (auto e : ranges[nxt]) pq.push(e);
    while (nxt != 0) {
        if (pq.empty()) {
            print(-1);
            return 0;
