...
void main_() {
	...
	vector nx(n + 1, vector(26, n));
	RREP(i, n) {
		REP(j, 26) { nx[i][j] = nx[i + 1][j]; }
		nx[i][s[i] - 'a'] = i;
	}
	...
	REP(i, n) {
		REP(j, 26) {
			if((i and s[i - 1] - 'a' == j) or nx[i][j] == n) continue;
			REP(k, i + 1) {
				dp[nx[i][j] + 1][k + 1] += dp[i][k];
			}
		}
	}
	vector pat(n + 1, mint(0));
	REP(i, n) {
		REP(j, 1, n + 1) { pat[j] += dp[i + 1][j]; }
	}
	//
	debug(pat);
	mint res = 0;
	...
	REP(j, 1, n + 1) {
		//nj1
		res += fact.binom(n - j + j - 1, j - 1) * pat[j];
	}
	print(res);
}