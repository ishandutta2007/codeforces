...
main() {
    ...
    while(T--) {
        ...
        rep(i, m) {
            ...
            if(pre == i) {
                if(i == m - 1) break;
                int s = lb(cnt[i], cnt[i + 1].front());
                if(s == 0)
                    continue;
                else
                    now = cnt[i][s - 1], sum = s, i++;
            }
            pre = i;
            while(i < m) {
                if(cnt[i].front() < now) {
                    sum += cnt[i].size() - lb(cnt[i], now);
                    break;
                } else {
                    sum += cnt[i].size();
                    now = cnt[i].back();
        ...
        rep(i, m - 1) {
            rep(j, cnt[i].size()) { chmax<int>(ma, j + 1 + cnt[i + 1].size() - lb(cnt[i + 1], cnt[i][j])); }
        }
        cout << n - ma << endl;
    }
}