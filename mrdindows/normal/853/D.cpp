...
bool check(int s, int mask) {
    ...
    int needBonuses = total - s;
    if (needBonuses < 0) needBonuses = 0;
    int canGetBonuses = s / 10;
    if (canGetBonuses < needBonuses) return 0;
    
    int needFull = (needBonuses + 99) / 100;
    int canFull = s / 1000;
    int haveBonuses = 0;
...
//        DBN(i, haveBonuses, s);
        if (a[i] == 2000) {
            if (s >= 2000 || (needFull > 0 && s >= 2000)) {
                ...
                if (haveBonuses + s < 2000) {
                    return 0;
                }
                int t = min(haveBonuses, 2000);
                haveBonuses -= t;
                s -= 2000 - t;
            }
        } else {
            ...
            if (s >= 1000 && !sk) {
                ...
                if (haveBonuses + s < 1000) {
                    return 0;
                }
                int t = min(haveBonuses, 1000);
                haveBonuses -= t;
                s -= 1000 - t;
            }
        }
