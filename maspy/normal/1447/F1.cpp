...
#line 3 "/home/maspy/compro/library/alg/lazy_minmax_add.hpp"
...
void solve() {
  ...
  // most freq value?
  ll x1 = 0;
  FOR(x, N + 1) if (len(IDS[x]) > len(IDS[x1])) { x1 = x; }
  // 
  FOR(x, N + 1) if (x != x1 && len(IDS[x1]) == len(IDS[x])) { return print(N); }

  const ll INF = 1LL << 60;
  using Lazy = Lazy_MinMax_Add<ll, INF>;
  vc<pi> seg_raw(N + 1);
  LazySegTree<Lazy> seg(seg_raw);
  FOR(i, N) if (A[i] == x1) seg.apply(i + 1, N + 1, 1);
  ...
  auto calc = [&](ll x) -> void {
    ...
    ll a = seg.max_right(check, 0);
    ll b = seg.min_left(check, N + 1) - 1;
    chmax(ANS, b - a);
  };
  ...
  FOR(x, N + 1) if (x != x1) {
    ...
    ll n = len(I);
    vi cand;
    for (auto&& i: I) seg.apply(i + 1, N + 1, -1);
    for (auto&& i: I) {
      ll x = seg.prod(i, i + 1).fi;
      FOR3(d, -1, 1) cand.eb(x + d);
    }
    UNIQUE(cand);
    for (auto&& x: cand) calc(x);
    for (auto&& i: I) seg.apply(i + 1, N + 1, +1);
  }
  print(ANS);
}
