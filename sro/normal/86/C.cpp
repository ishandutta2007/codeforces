...
namespace ac{
	...
	void add(int m,int* s,int t){
		int now=0;
		for(int i=0;i<m;i++){
			if(nei[now][s[i]]==-1){
				//cerr<<nei[now][s[i]]<<' '<<s[i]<<' '<<now<<endl;
				tag[cnt]=0;
				FILL1(nei[cnt]);
				nei[now][s[i]]=cnt++;
			}
		...
		for(int j=0;j<ac::cnt;j++){
			for(int k=0;k<15;k++){
				...
				for(int l=0;l<4;l++){
					int go=ac::nei[j][l];
					if(ac::tag[go]>k){
						dp[i+1][go][0]=(dp[i+1][go][0]+dp[i][j][k])%Mod;
					}else if(k<14){
						dp[i+1][go][k+1]=(dp[i+1][go][k+1]+dp[i][j][k])%Mod;
					}
				}
			}
		}
