...
int main() {
	...
	vector<Mint> val(k + 1, Mint(0, MOD107));
	val[1] = 1;
	for (int i = 2; i <= k; i++)
		if (mpf[i] == i)
			val[i] = Mint(i, MOD107).pow(n);
		else
			val[i] = val[mpf[i]] * val[i / mpf[i]];
	...
	vector<Mint> ans(k + 1, Mint(0, MOD107));
	for (int i = 1; i <= k; i++) {
		ans[i] += ans[i - 1];
		for (int j = i; j <= k; j += i) {
			ans[j] -= mu[i] * val[(j - 1) / i];
			ans[j] += mu[i] * val[j / i];
		}
	}

	Mint res = Mint(0, MOD107);
	for (int i = 1; i <= k; i++)
		res += (ll)ans[i] ^ i;
