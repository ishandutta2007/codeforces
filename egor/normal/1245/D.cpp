...
public:
	void solve(std::istream& inp, std::ostream& outp) {
        ...
        int n = in.readInt();
        auto[x, y] = in.readArrays<int, int>(n);
        auto c = in.readIntArray(n);
        auto k = in.readIntArray(n);
        DSU dsu(n + 1);
        vector<tuple<longint, int, int> > edges;
        edges.reserve(n * (n + 1) / 2);
        for (int i = 0; i < n; ++i) {
            edges.emplace_back(c[i], 0, i + 1);
            for (int j = 0; j < i; j++) {
                edges.emplace_back(longint(abs(x[i] - x[j]) + abs(y[i] - y[j])) * (k[i] + k[j]), i + 1, j + 1);
            }
        }
        sort(edges);
        longint cost = 0;
        ...
        out.printLine(stations);
        out.printLine(connections.size());
        for (const auto& connection : connections) {
            out.printLine(connection.first, connection.second);
        }
	}
};
