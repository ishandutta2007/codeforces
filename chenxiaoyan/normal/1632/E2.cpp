...
int fa[N], dep[N], dfn[N], mxdfn[N], nowdfn, mng[N];
void dfs(int x = 1) {
...
void dfs0(int x = 1) {
	...
	for(int y : nei[x]) if(y != fa[x]) {
		dfs0(y);
		ti3 t(len[x], a[x], b[x]);
		chkmx(t, mt(len[y], a[y], b[y]));
		for(int A : {a[x], b[x]}) for(int B : {a[y], b[y]}) {
			chkmx(t, mt(dep[A] + dep[B] - 2 * dep[x], A, B));
		}
		tie(len[x], a[x], b[x]) = t;
	}
}
...
void mian() {
	...
	nowdfn = 0, dfs();
	Mx[0] = mX[n + 1] = 0;
	REP(i, 1, n) Mx[i] = max(Mx[i - 1], dep[mng[i]]);
	PER(i, n, 1) mX[i] = max(mX[i + 1], dep[mng[i]]);
	REP(i, 2, n) {
		int x = mng[i];
		up[x] = max(Mx[i - 1], mX[mxdfn[x] + 1]);
	}
	dfs0();
	REP(i, 0, n + 1) tag1[i] = tag2[i] = inf;
	tag1[n] = mX[1];
	REP(x, 2, n) {
		int a = up[x], b = len[x] + 1 >> 1;
		chkmn(tag1[max(0, a - b)], a);
		chkmn(tag2[max(1, a - b)], b);
	}
	int mn = inf;
	REP(i, 1, n) chkmn(mn, tag2[i]), ans[i] = mn + i;
	mn = inf;
	PER(i, n, 1) chkmn(mn, tag1[i]), chkmn(ans[i], mn);
