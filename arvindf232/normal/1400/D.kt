...
inline fun merge(left:List<Int>, right:List<Int>, cb:(Int)->Unit){
    var lp = 0
    var rp = 0
    while(lp <= left.lastIndex && rp <= right.lastIndex){
        if(left[lp] < right[rp]){
            cb(0)
            lp ++
        }else{
    ...
    while(lp <= left.lastIndex){
        cb(0)
        lp ++
    }
    while(rp <= right.lastIndex){
        cb(1)
        rp ++
    }
}
...
//        share(10)
//        repeat(10){
//            share(if(Random.nextBoolean()) 1 else 0 )
//        }
//    }
//    solve.userand()
    solve.tc {
...
//    solve.usetc()
    solve.cases{
        ...
        val all = Array(n+1){ mutableListOf<Int>() }
        ...
        fun c4(a:Int):Long {
            if(a <= 3) return 0L
            else return 1L * (a) * (a-1) * (a-2) * (a-3) / 24
        }
        var total = 0L
        for(i in 0..n){
            for(j in i..n){
