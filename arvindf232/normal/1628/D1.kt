...
fun main(){
    ...
    solve.cases{
        ...
        val DP = Array(n+1){IntArray(mop+1)}

        for(phrase in n downTo 0){
            for(mus in mop downTo 0){
                if(phrase == n){
                    DP[phrase][mus] = 0
                }else{
                    if(mus == 0){
                        DP[phrase][mus] = DP[phrase +1][mus] modPlus k
                    }else{
                        DP[phrase][mus] = (DP[phrase+1][mus] + DP[phrase+1][mus-1]) modM inv2
                    }
                }
            }
        }
