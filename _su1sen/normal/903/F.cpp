...
int main() {
    ...
    auto to_key = [&](array<int, 4> cur) {
        return cur[0] | (cur[1] << 4) | (cur[2] << 8) | (cur[3] << 12);
    };

    vector<map<int, int>> memo(n);
    auto dfs = [&](auto dfs, int i, array<int, 4> cur) {
        if (i == n) return 0;

        int key = to_key(cur);
        if (auto it = memo[i].find(key); it != memo[i].end()) return it->second;

        int res = numeric_limits<int>::max();

        auto inner_dfs = [&](auto inner_dfs, int j, array<int, 4> cur, int c) {
            if (j == 4) {
                if (cur[0]) return;
                cur[0] = cur[1];
                cur[1] = cur[2];
                ...
                chmin(res, c + dfs(dfs, i + 1, cur));
                return;
            }

            rrep(k, 5 - j) {
                // [0, k) * [j, j + k)
                const int mask = ((1 << (j + k)) - 1) ^ ((1 << j) - 1);
                ...
                if (k > 0 and not b) continue;
                inner_dfs(inner_dfs, j + 1, ncur, c + a[k]);
            }
        };
        inner_dfs(inner_dfs, 0, cur, 0);

        return memo[i][key] = res;
    };
    print(dfs(dfs, 0, { g[0], g[1], g[2], g[3] }));
