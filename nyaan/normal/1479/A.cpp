...
void Nyaan::solve() {
  ...
  auto ok = [&](int i) {
    ...
    if (memo[i - 1] == -1) return false;
    if (memo[i - 0] == -1) return false;
    if (memo[i + 1] == -1) return false;
    if (memo[i] < min(memo[i - 1], memo[i + 1])) return true;
    return false;
  };
