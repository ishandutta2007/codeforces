...
node*nu(int v,int m,node*l,node*r){
  node*n=nodebuf+bufp++;
  n->v=v;
  n->m=m;
  n->l=l;
...
node*mk0(int a,int b){
  if(b==a+1)
    return nu(0,0,0,0);
  int c=(a+b)/2;
  return nu(0,0,mk0(a,c),mk0(c,b));
}

node*add(int l,int r,int v,int a,int b,node*n){
  if(l>=b || r<=a)
    return n;
  if(l<=a && r>=b)
    return nu(n->v+v,n->m+v,n->l,n->r);
  int c=(a+b)/2;
  node*n1=add(l,r,v,a,c,n->l);
  node*n2=add(l,r,v,c,b,n->r);
  return nu(n->v,n->v+max(n1->m,n2->m),n1,n2);
}

int fnd(int v,int a,int b,node*n){
  if(n->m<v)
    return -1;
  if(b==a+1)
    return a;
