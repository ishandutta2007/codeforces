...
int64_t v_b, v_s;
int64_t x_u, y_u;
pair<double, pair<int64_t, int>> dist = {1e18, {1e18, -1}};
void solve() {
    cin >> n;
    cin >> v_b >> v_s;
    ...
    cin >> x_u >> y_u;
    for (int i = 2; i <= n; i++) {
        pair<double, pair<int64_t, int>> ans = {
                (sqrtl(y_u * y_u + (x_u - x[i]) * (x_u - x[i])) / v_s + static_cast<double>(x[i]) / v_b),
                {y_u * y_u + (x_u - x[i]) * (x_u - x[i]), i}};
        dist = min(dist, ans);
    }
    cout << dist.second.second << '\n';
}
