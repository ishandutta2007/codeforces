...
int lastn = 2;
int root = 1;
void draw(int &cur, int l, int r, int x, int y){
	if (cur == 0){
		cur = lastn++;
	}
	if (x < l) x = l;
	...
	if (x == l && y == r){
		...
		if (y <= m)
			...
			draw(rp[cur], m + 1, r, x, y);
		else {
			draw(lp[cur], l, m, x, m);
			draw(rp[cur], m + 1, r, m + 1, y);
		}
		tr[cur] = tr[lp[cur]] + tr[rp[cur]];
	}

}
int sum(int cur, int l, int r, int x, int y){ if (y > r) y = r;
	if (cur == 0) return 0;
	if (full[cur])
		return y - x + 1;
	if (x == l && y == r){
		...
		int m = (l + r) >> 1;
		if (y <= m) return sum(lp[cur], l, m, x, y);
		else if (x > m) return sum(rp[cur], m + 1, r, x, y);
		else
			return sum(lp[cur], l, m, x, m) + sum(rp[cur], m + 1, r, m + 1, y);
	}
}
int main()
{
	...
	int last = INF;
	sort(q, q + m, [](const tri &p1, const tri &p2){
		return p1.a < p2.a;
	});
	int lastq = m - 1;
	for (int i = n - 1; i >= 0; --i){
		draw(root, 1, INF, p[i], p[i] + l[i] - 1);
		while (lastq >= 0 && q[lastq].a == i){
			int x = q[lastq].a;
			int y = q[lastq].b;
			int ind = q[lastq].c;
			...
			int beg = p[x];
			int end = p[y];
			ans[ind] = end - beg - sum(root, 1, INF, beg, end - 1);
			--lastq;
		}
	}
	FI(m){
