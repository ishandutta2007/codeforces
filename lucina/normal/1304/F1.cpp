...
struct Segtree {
    ...
    inline int query (int v, int x, int y, int l, int r) {
        ...
        return max(query(v * 2, x, mid, l, min(r, mid)), query(v * 2 + 1, mid + 1, y, max(l, mid + 1), r ));
    }
...
int main () {
    ...
    for (int i = 2 ; i <= n ; ++ i) {
        ...
        for (int j = 1 ; j <= m ; ++ j) {
            int toAdd = max(dp[i - 1][j], seg.query(1, j));
            int to = min(j + k - 1, m);
            dp[i][j] = max(dp[i][j], toAdd + a[i][to] - a[i][j - 1]);
       ...
       for (int j = m - k + 1 ; j >= 1 ; -- j) {
            ...
            int toAdd = max(dp[i - 1][j], seg.query(j, m));
            int to = min(m, j + k - 1);
            dp[i][j] = max(dp[i][j], toAdd +  a[i][to] - a[i][j - 1]);
