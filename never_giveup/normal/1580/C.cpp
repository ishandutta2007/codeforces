...
const int maxn = 310 + 100, maxw = 5e6 + 10, inf = 1e9 + 100, LG = 20, SQ = 500;
...
void solve() {
    ...
    for (int i = 0; i < n; i++)
        cin >> mas[i].fr >> mas[i].sc;
    vector<int> added(n, -1);
    vector<int> prs(m);
    int sprs = 0;
    int CUR;
    auto add = [&](int t, int x, int y, int val) {
        if (x + y < SQ) {
            t = t % (x + y);
            for (int i = t; i < min(x + y, x + t); i++)
                q[x + y][i] += val;
            for (int i = 0; i < t - y; i++)
                q[x + y][i] += val;
        } else {
    ...
    for (CUR = 0; CUR < m; CUR++) {
        ...
        //add
        {
            ...
            if (t == 1) {
                tot++;
                added[k] = CUR;
                add(added[k], mas[k].fr, mas[k].sc, 1);
            } else {
                tot--;
                add(added[k], mas[k].fr, mas[k].sc, -1);
            }
