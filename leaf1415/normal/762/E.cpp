...
int main(void)
{
	...
	cin >> n >> k;
	rep(i, 1, n) cin >> x[i] >> r[i] >> f[i];

	rep(i, 1, 10000) bt[i] = BinaryTrie(30);
	rep(i, 1, n) vec.push_back(P(x[i], -i)), vec.push_back(P(x[i]+r[i], i));
	sort(all(vec));
	...
	for(auto p : vec){
		...
		if(id < 0){
			id *= -1;
			rep(d, -k, k){
				if(f[id] + d <= 0 || f[id] + d > 10000) continue;
				ans += bt[f[id]+d].size() - bt[f[id]+d].lower_bound(max(0LL, x[id]-r[id]));
			}
			bt[f[id]].insert(x[id]);
		}
		else bt[f[id]].erase(x[id], 1);
		//outl(p, ans);
	}
	outl(ans);
