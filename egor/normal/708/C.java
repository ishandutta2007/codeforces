...
public class Main {
    ...
    static class TaskC {
        ...
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = graph.firstOutbound(i); j != -1; j = graph.nextOutbound(j)) {
                    if (size[j] > limit && max[j] < size[j] - limit) {
                        answer[i] = 0;
                        break;
                    }
                }
        ...
        private void dfs3(int vert, int last, int top) {
            int best = top;
            int at = -1;
            int second = 0;
            for (int i = graph.firstOutbound(vert); i != -1; i = graph.nextOutbound(i)) {
            ...
            for (int i = graph.firstOutbound(vert); i != -1; i = graph.nextOutbound(i)) {
                ...
                if (size[i ^ 1] <= limit) {
                    max[i ^ 1] = Math.max(max[i ^ 1], size[i ^ 1]);
                }
                dfs3(next, vert, max[i ^ 1]);
            }
        }

        private int dfs2(int vert, int last) {
            int res = 0;
            for (int i = graph.firstOutbound(vert); i != -1; i = graph.nextOutbound(i)) {
                int next = graph.destination(i);
                if (next == last) {
                ...
                int call = dfs2(next, vert);
                if (size[i] <= limit) {
                    call = Math.max(call, size[i]);
                }
                max[i] = call;
                res = Math.max(res, call);
            }
            return res;
        }

        private int dfs(int vert, int last) {
            int res = 1;
            for (int i = graph.firstOutbound(vert); i != -1; i = graph.nextOutbound(i)) {
                int next = graph.destination(i);
                if (next == last) {
