...
fn run() {
    ...
    // up a, b, c
    // a^b, b^c, c^a 
    let mut dp = [[M::zero(); 8]; 8];
    dp[7][0] = M::one();
    for s in s.iter() {
        let d = (*s - b'0') as usize;
        let mut next = [[M::zero(); 8]; 8];
        for (up, dp) in dp.iter().enumerate() {
            for (cond, dp) in dp.iter().enumerate() {
                for bit in 0usize..8 {
                    if d == 0 && up & bit != 0 {
                        continue;
                    }
    ...
    let ans = dp.iter().fold(M::zero(), |s, a| s + a[7]);
    println!("{}", ans);
}
