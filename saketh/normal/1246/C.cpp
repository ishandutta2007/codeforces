...
int main() {
    ...
    FOR (i, 1, N + 1) {
        string row; re(row);
        FOR (j, 1, M + 1) grid[i][j] = row[j-1];
    }

    vvi right(N + 2, vi(M + 2)), down = right;
    FORd (i, 1, N + 1) FORd (j, 1, M + 1) {
        right[i][j] = int(grid[i][j] == 'R') + right[i][j+1];
         down[i][j] = int(grid[i][j] == 'R') +  down[i+1][j];
    }

    vvmn horiz(N+2, vmn(M+2)), vert = horiz;
    vert[1][0] = 1;

    vi v0(M + 2); vmn vtot(M + 2);
    FOR (i, 1, N + 1) {
        int h0 = 0;
        mn htot = 0;
        FOR (j, 1, M + 1) {
            htot += vert[i][j-1];
            while (h0 < j && right[i][h0+1] > M - j) htot -= vert[i][h0++];
            horiz[i][j] = htot;

            vtot[j] += horiz[i-1][j];
            while (v0[j] < i && down[v0[j]+1][j] > N - i) vtot[j] -= horiz[v0[j]++][j];
            vert[i][j] = vtot[j];
        }
    }

    ps(horiz[N][M] + vert[N][M]);
