...
{
	int leftset=0;
	for (int i=1;i<=3;i++) if (set&(1<<(i-1)))
	{
		char s0[60];
		strcpy(s0,s);
		if (i&1) R1(s0);
		...
		int c=compare(s,s0);
		if (c>0) return 0;
		if (c==0) leftset|=(1<<(i-1));
	}
	if (leftset==0)
	{
		int64 R=1;
		for (int i=0;i<n;i++) if (s[i]=='*') R*=2;
		return R;
	}
	string buffer=s;
	if (leftset==2)
	...
	if (leftset==4)
	{
		for (int i=0;i<n-1-i;i++)
			if (buffer[i]!='*' && buffer[n-1-i]!='*')
				buffer[i]='0',buffer[n-1-i]='1';
			else
				break;
	}
	if (M.find(buffer)!=M.end()) return M[buffer];
	int pos=-1;
