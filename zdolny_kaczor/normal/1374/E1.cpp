...
void solve() {
	...
	REP(_, n) {
		int t, a, b;
		scanf("%d%d%d", &t, &a, &b);
		if (a && !b) alice.push_back(t);
		if (a && b) common.push_back(t);
		if (!a && b) bob.push_back(t);
	}
	auto fix = [&](vi &v) {
		sort(ALL(v));
		v.resize(k, numeric_limits<int>::max());
	};
	fix(alice);
	fix(bob);
	fix(common);
	ll ans = numeric_limits<ll>::max(), curr = 0;
	for (int x : common) curr += x;
	mini(ans, curr);
	REP(i, k) {
		curr += alice[i];
		curr += bob[i];
