...
/////////////////////////////////////////////////////////////////////
#define MAXN 100010
#define Pq priority_queue< int , vc<int> , greater<int> >
#define TREE 400040
...
ll Query( int gl , int gr , int l , int r , int pos ){
    if( gr < l  ||  r < gl ) return LINF;
    if( gl <= l  &&  r <= gr ) return tree[pos];
    return min(Query(gl,gr,LSON),Query(gl,gr,RSON));
}
...
int main(){
    ...
    Forx(i,1,n){
        dp[i] = dp[i-1];
        if( i-c+1 >= 1 )
            Mymax(dp[i],dp[i-c] + Query(i-c+1,i,ROOT) );
    }
    cout << tot - dp[n] << endl;
