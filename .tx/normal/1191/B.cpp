...
int main(int argc, char** argv) {
    ...
    auto has = [](const vector<pair<int, char>>& a) {
        ...
        for (auto& i : z) {
            ...
            i.second.resize(unique(begin(i.second), end(i.second)) - begin(i.second));

            for (int j = 2; j < i.second.size(); j++) {
                if (i.second[j] - 1 == i.second[j - 1] && i.second[j] - 2 == i.second[j - 2]) {
                    return true;
                }
            }
        }
