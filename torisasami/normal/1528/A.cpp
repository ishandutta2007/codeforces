...
int main(){
	...
	while(T--){
		...
		function<pair<ll, ll>(ll, ll)> dfs = [&](ll now, ll par){
			pair<ll, ll> res = {0, 0};
			for(auto &e: li[now]){
				if(e != par){
					auto val = dfs(e, now);
					res.first += max(abs(l[now] - l[e]) + val.first, abs(l[now] - r[e]) + val.second);
					res.second += max(abs(r[now] - l[e]) + val.first, abs(r[now] - r[e]) + val.second);
				}
			}
			return res;
		};
