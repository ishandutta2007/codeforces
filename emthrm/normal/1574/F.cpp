...
int main() {
  ...
  while (n--) {
    int c; cin >> c;
    vector<int> a(c); REP(i, c) cin >> a[i], --a[i];
    FOR(i, 1, c) graph[a[i - 1]].emplace_back(a[i]);
  }
  vector<int> deg(k, 0);
  REP(i, k) {
    sort(ALL(graph[i]));
  ...
  map<int, int> mp;
  vector<int> is_visited(k, false);
  REP(i, k) {
    if (deg[i] > 0 || is_visited[i]) continue;
    bool flag = true;
    int component = 1;
    for (int ver = i; ; ver = graph[ver].front(), ++component) {
      if (is_visited[ver] || graph[ver].size() > 1 || deg[ver] > 1) {
        flag = false;
        break;
      }
      is_visited[ver] = true;
      if (graph[ver].empty()) break;
    }
    if (flag) ++mp[component];
  }
  vector<ModInt> dp(m + 1, 0);
  dp[0] = 1;
  REP(i, m) for (auto [len, num] : mp) {
    if (i + len <= m) dp[i + len] += dp[i] * num;
  }
  cout << dp[m] << '\n';
  return 0;
}