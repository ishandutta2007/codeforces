...
template <typename G> struct TwoEdgeConnectedComponents : LowLink<G> {
    ...
    TwoEdgeConnectedComponents(const G &g) : LLo(g) {}
...
vmint merge(vmint &f, vmint &g) {
    rep(i, si(f)) f[i] *= ifact(i);
    rep(i, si(g)) g[i] *= ifact(i);
    auto res = f * g;
    rep(i, si(res)) res[i] *= fact(i);
    return res;
}
int main() {
    INT(n, m);
    ...
    UnionFind uf(n);
    {
        TwoEdgeConnectedComponents<Graph> two(g);
        two.build();
        vi cnt(n);
        rep(i, n) cnt[two[i]]++;
        rep(i, n) if(cnt[two[i]] > 1) can[i] = false;
        Graph G(n);
        rep(i, n) for(auto e : g[i]) if(can[i] and can[e]) G[i].eb(e), uf.unite(i, e);
        rep(i, n) for(auto e : g[i]) if(can[i] and !can[e]) con[i] = true;
        swap(g, G);
    }
    rep(i, n) if(can[i]) bel[uf.find(i)].eb(i);
    vmint ans{1};
    vv(mint, dp, n);
    rep(i, n) {
        if(empty(bel[i])) continue;
        auto &v = bel[i];
        vmint sum(si(v) + 1);
        for(auto r : bel[i]) {
