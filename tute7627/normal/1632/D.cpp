...
int main(){
  ...
  //while(t--){
    ...
    vector<ll>g(n,-1);
    rep(i,0,n){
      while(r<n&&(swag.empty()||swag.size()<swag.query())){
        //cout<<i spa r<<endl;
        swag.push(a[r]);
        r++;
      }
      //cout<<r spa swag.size() spa swag.query()<<endl;
      if(swag.size()==swag.query()){
        g[i]=r;
      }
      swag.pop();
    }
