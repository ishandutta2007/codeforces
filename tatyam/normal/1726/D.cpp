...
void solve(){
    ...
    vec(ll,parent,n,-1);
    vec(ll,depth,n);
    auto dfs = [&](ll at, auto dfs) -> void {
        visit[at]=1;
        each(i, g[at]) if(!visit[i]){
            depth[i]=depth[at]+1;
            parent[i]=at;
            ans[i.cost]='1';
            dfs(i, dfs);
    ...
    if(count(cnt,2)==3){
        vector<ll>circ;
        rep(n)if(cnt[i]==2)circ.push_back(i);
        sort(circ.begin(), circ.end(),[&](ll i,ll j){return depth[i]<depth[j];});
        const ll x=circ[2],y=circ[0],z=parent[x];
        rep(m){
            if(edge[i]==pll{x,y}||edge[i]==pll{y,x})ans[i]='1';
            if(edge[i]==pll{x,z}||edge[i]==pll{z,x})ans[i]='0';
        }
    }
    out(ans);
}
