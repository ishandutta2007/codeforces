...
ll n, T;
ll a[40005];
ll ldp[40005][105], rdp[40005][105];
ll cnt[105];
...
int main(void)
{
	...
	uniq(comp);
	rep(i, 1, n) a[i] = lower_bound(all(comp), a[i]) - comp.begin() + 1;
	
	rep(i, 1, n) cnt[a[i]]++;
	
	ll N = n * 400;
	rep(i, n+1, N) a[i] = a[(i-1)%n+1];
	
	rep(i, 0, N) rep(j, 1, n) ldp[i][j] = -inf;
	ldp[0][1] = 0;
	rep(i, 0, N-1){
		rep(j, 1, n){
			chmax(ldp[i+1][j], ldp[i][j]);
			if(a[i+1] >= j) chmax(ldp[i+1][a[i+1]], ldp[i][j]+1);
		}
	}
	
	rep(i, 0, N+1) rep(j, 1, n) rdp[i][j] = -inf;
	rdp[N+1][n] = 0;
	per(i, N+1, 2){
		rep(j, 1, n){
			chmax(rdp[i-1][j], rdp[i][j]);
			if(a[i-1] <= j) chmax(rdp[i-1][a[i-1]], rdp[i][j]+1);
		}
	}
	
	if(T <= 400){
		ll ans = 0;
		rep(i, 1, n) chmax(ans, ldp[n*T][i]);
		outl(ans);
		return 0;
	}
	...
	rep(i, 1, n) rep(j, 1, n){
		rep(k, i, j){
			chmax(ans, ldp[200*n][i] + rdp[N+1-200*n][j] + cnt[k] * (T-400));
		}
	}
	outl(ans);
