...
using S = array<array<int, 3>, 3>;
S e() {
    S s;
    rep(i, 3) rep(j, 3) s[i][j] = (i <= j ? 0 : inf<int>);
    return s;
}
S f(S x, S y) {
    S res = e();
    rep(i, 3) rep(j, 3) res[i][j] = inf<int>;
    rep(i, 3) rep(j, i, 3) rep(k, j, 3) { chmin(res[i][k], x[i][j] + y[j][k]); }
    return res;
}
S make(char c) {
    S res = e();
