...
signed main()
{
    ...
    loop(read<int>())
    {
        ...
        for(int i=0;i<n;i++)
            for(int j=0;j<m;j++)
                if(s[i][j]!='.')
                    xl[s[i][j]-'a']=min(xl[s[i][j]-'a'],i),
                    xr[s[i][j]-'a']=max(xr[s[i][j]-'a'],i),
                    yl[s[i][j]-'a']=min(yl[s[i][j]-'a'],j),
                    yr[s[i][j]-'a']=max(yr[s[i][j]-'a'],j);
        for(int i=24;i>=0;i--)
            if(xl[i]==N)
                xl[i]=xl[i+1],
                xr[i]=xr[i+1],
        ...
        else
        {
            ...
            cout<<k<<"\n";
            for(int i=0;i<k;i++)
                cout<<xl[i]+1<<" "<<yl[i]+1<<" "<<xr[i]+1<<" "<<yr[i]+1<<"\n";
        }
    }
