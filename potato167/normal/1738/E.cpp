...
void solve(){
	...
	ll ans=1;
	for(auto x:m){
		if(x.first*2ll>sum) continue;
		if(x.first*2ll==sum){
			ans=(ans*jyo(2,x.second,mod))%mod;
		}
		else if(m.count(sum-x.first)){
			ll tmp=0;
			ll a=x.second;
			ll b=m[sum-x.first];
			rep(i,min(a,b)+1){
				tmp=(tmp+table.Comb(a,i)*table.Comb(b,i))%mod;
			}
			ans=(ans*tmp)%mod;
		}
	}
