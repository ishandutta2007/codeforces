...
void Mina(){
	...
	For(i,n)
		For(j,n){
			Mymin( mina[i][j] , min(mina[i-1][j],mina[i][j-1]) + (a[i][j]!='a') );
			if( mina[i][j] <= k ){
				Mymax(farth,i+j);
			}
		}
	For(i,farth-1) putchar('a');
	For(i,n)
		For(j,n)
			if( i+j == farth && mina[i][j] <= k )
				posi[++top] = Mp(i,j);
	if(!top) posi[++top] = Mp(0,1);
}
int dx[] = {1,0};
int dy[] = {0,1};
pii nxtposi[MAXD]; int nxttop = 0;
bool vis[MAXN][MAXN];
void Solve(){
	while(top){
		nxttop = 0;
		int umin = 233;
		For(i,top){
			int x = posi[i].F, y = posi[i].S;//cout << x << " " << y << endl;
			For0(k,2){
				int vx = x+dx[k];
				int vy = y+dy[k];
				if( vx > n || vy > n || vx <= 0 || vy <= 0 ) continue;
				if( a[vx][vy] < umin ){
					nxttop = 0;
					umin = a[vx][vy];
				}
				if( a[vx][vy] == umin ){
					...
					vis[vx][vy] = 1;
					nxtposi[++nxttop] = Mp(vx,vy);
				}
			}
		}
		if( umin != 233 ) putchar(umin);
		top = nxttop;
		For(i,top)
			posi[i] = nxtposi[i];
	}
...
int main(){
	...
	Read(n,k);
	For(i,n) For(j,n) In.Readalpha(a[i][j]);
