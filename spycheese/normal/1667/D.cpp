...
void solve() {
  ...
  function<void(int, int)> dfs1 = [&](int v, int pv) {
    ...
    int k = sz(e[v]) + 1;
    if (k % 2 == 1) {
      d[1][v] = (cnt0 >= k / 2) && (cnt1 + 1 >= k / 2 + 1);
      d[0][v] = (cnt0 + 1 >= k / 2) && (cnt1 >= k / 2 + 1);
    } else {
      d[0][v] = (cnt0 + 1 >= k / 2) && (cnt1 >= k / 2);
      d[1][v] = (cnt0 >= k / 2) && (cnt1 + 1 >= k / 2);
    }
  };
  ...
  function<void(int, int)> dfs2 = [&](int v, int f) {
    vector<int> vs[3];
    for (int nv : e[v]) {
      if (d[0][nv] && d[1][nv])
        vs[2].push_back(nv);
    ...
    while (ck > 0) {
      if (vs[ck % 2].empty()) {
        ...
        ord[v].push_back(vs[2].back());
        vs[2].pop_back();
      } else {
        ord[v].push_back(vs[ck % 2].back());
        vs[ck % 2].pop_back();
      }
      --ck;
    }
  ...
  function<void(int)> go = [&](int v) {
    if (!ord[v].empty() && ord[v].back() == p[v] && !ord[p[v]].empty() && ord[p[v]].back() == v) {
      cout << v + 1 << " " << p[v] + 1 << "\n";
      ord[v].pop_back();
      ord[p[v]].pop_back();
      go(p[v]);
      if (!ord[v].empty()) go(ord[v].back());
      if (!ord[p[v]].empty()) go(ord[p[v]].back());
    }
  };
  for (int i = 0; i < n; ++i)
    go(i);
