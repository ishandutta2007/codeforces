...
int getprf(string s) {
	...
	vector<pair<ll, ll>> hs(sz(s)), hrs(sz(s));
	for (int i = 1; i < sz(s); ++i) {
		hs[i].x = (hs[i - 1].x * P1K + (s[i - 1] - '0' + 1)) % FFT_MOD;
		hs[i].y = (hs[i - 1].y * P1M + (s[i - 1] - '0' + 1)) % 5757767;
		hrs[i].x = (hrs[i - 1].x + (s[sz(s) - i] - '0' + 1) * pwm(P1K, i - 1, FFT_MOD)) % FFT_MOD;
		hrs[i].y = (hrs[i - 1].y + (s[sz(s) - i] - '0' + 1) * pwm(P1M, i - 1, 5757767)) % 5757767;
	}
...
inline void solve() {
	...
	int zs = count(all(s), '0'), os = count(all(s), '1');
	int zt = count(all(t), '0'), ot = count(all(t), '1');
	if (zs < zt || os < ot) {
		cout << s << endl;
		return;
	}
	...
	int prf = getprf(t);
	zt = count(t.begin() + prf, t.end(), '0');
	ot = count(t.begin() + prf, t.end(), '1');

	while (zs >= zt && os >= ot) {
		ans += string(t.begin() + prf, t.end());
		zs -= zt;
		os -= ot;
	}
	ans += string(zs, '0');
