...
#line 1 "/home/maspy/library/graph/decompose_complete.hpp"
vc<vi> to_paths(ll N) {
  // 
  assert(N >= 0 && N % 2 == 0);
  ll K = N / 2;
...
void solve() {
  ...
  auto get_cyc = [&](ll N) -> vc<vi> {
    if (N % 2 == 1) return to_cycles(N);
    auto [cycles, match] = to_cycles_and_matching(N);
    return cycles;
  };
  auto cycles = get_cyc(N);
  if (K > len(cycles)) return print(-1);
  cycles.resize(K);
  vc<pi> ANS;
  FORIN(P, cycles) {
    FOR(i, len(P)) {
      ll j = (i == len(P) - 1 ? 0 : i + 1);
      ANS.eb(P[i], P[j]);
    }
  }
  print(len(ANS));
