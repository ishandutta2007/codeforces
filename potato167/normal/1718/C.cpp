...
void solve(){
	...
	vector<ll> D=table[n];
	ll S=0;
	rep(i,(int)(D.size())) S+=D[i];
	segment_tree<F,op,e> seg(S);
	rep(i,n){
		int tmp=0;
		rep(j,(int)(D.size())){
			int ind=tmp+i%D[j];
			seg.set(ind,seg.get(ind)+D[j]*p[i]);
			tmp+=D[j];
		}
	}
	rep(i,q+1){
		...
		rep(j,(int)(D.size())){
			int ind=tmp+x%D[j];
			seg.set(ind,seg.get(ind)+D[j]*(y-p[x]));
			tmp+=D[j];
		}
		p[x]=y;
	}
