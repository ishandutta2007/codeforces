...
int main() {
	...
	int a,b; R(a,b); a--; b--;
	flow.addedge(SRC, a * 2 + 0, LLONG_MAX);
	flow.addedge(2 * a + 1, SINK, LLONG_MAX);
	flow.addedge(SRC, b * 2 + 1, LLONG_MAX);
	flow.addedge(2 * b + 0, SINK, LLONG_MAX);

	for (int i = 0; i < m; i++) {
		int u,v,w; R(u,v,w); u--; v--;
		flow.addedge_undir(u * 2 + 0, v * 2 + 1, w);
		flow.addedge_undir(u * 2 + 1, v * 2 + 0, w);
	}

	W(flow.maxflow(SRC, SINK));
	vector<int> mask(n, 0);
	for (int u = 0; u < 2 * n; u++)
		if (flow.isTrav[u])
			mask[u / 2] |= 1 << (u % 2);
