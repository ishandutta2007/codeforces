...
cin>>T;while(T--)
{
    ...
    auto runFwd=[&](lli pos){
        const lli sm=a[pos-1];
        lli cur=0;
        auto it=lower_bound(all(a),2*a[pos]-sm);
        while(it!=a.end()){
            cur++;
            pos=it-a.begin();
            it=lower_bound(all(a),2*a[pos]-sm);
        }
        return cur;
    };
    ...
    for(int i=0;i+1<n;++i){
        if(a[i]==a[i+1])
            continue;
        const int f=runFwd(i+1);
        ans=max(ans,f+1+mm[a[i]]);//a[i] is smallest and it repeats.
        // const int b=runBck(i,a[i+1]-a[i]);
    }
