...
fn pareto(s: &mut [(i32,i32)]) -> Vec<(i32,i32)> {
	let mut st = v!([]);
	s.sort();
	for &(x,y) in s.iter() {
		while let Some( (xx,yy) ) = st.pop() {
			if yy > y {
				st.push( (xx,yy) );
				break;
			}
...
fn main() {
	...
	let light = pareto(&mut light);
	let m = light.len();
	let mut bad = v!([]);
	for (x,y) in rob {
		let mut i = 0;
		while i < m && light[i].0 < x {
			i += 1;
		}
		if i == m || light[i].1 < y {
			continue;
		...
		let mut j = i;
		while j+1 < m && light[j+1].1 >= y {
			j += 1;
		}
		for k in i..=j {
			bad.push( (light[k].0-x, light[k].1-y) );
		}
	}
	let bad = pareto(&mut bad);
	let mut sol = 100000000;
	if bad.is_empty() {
		sol = 0;
	} else {
		let k = bad.len();
