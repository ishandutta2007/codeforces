...
ll Q;
vector<ll> dvec[400005];
vector<P> avec[200005], qvec[200005];
ll ans[200005];
BIT bit(200005);
...
int main(void)
{
	...
	rep(i, 1, 400000) for(int j = i; j <= 400000; j+=i) dvec[j].pb(i);
	
	rep(i, 1, 200000){
		vector<ll> &vec = dvec[2*i]; ll n = sz(vec);
		rep(j, 0, n-1){
			if(vec[j] >= i) break;
			ll sum = 0;
			rep(k, j+1, n-1){
				if(vec[k] >= i) break;
				bool flag = false;
				if(i % vec[j] || i % vec[k]) flag = true;
				if(!flag || vec[j]+vec[k]>i) sum++;
			}
			avec[i].pb(P(vec[j], sum));
		}
	}
	...
	rep(i, 1, 200000){
		for(auto p : avec[i]) bit.add(p.fi, p.se);
		for(auto p : qvec[i]) ans[p.se] -= bit.query(p.fi, i);
	}
