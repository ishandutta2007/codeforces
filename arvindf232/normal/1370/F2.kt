...
fun main(){
    solve.cases{
        ...
        var qs = 0
        fun ask(v:List<Int>):Pair<Int, Int> {
            qs ++
            if(qs > 11){
                println("QUERIES")
                crash()
            ...
            assert(v.size != 0 )
            println("? ${v.size}")
            println(v.map { it + 1 }.conca())
            System.out.flush()
            val node = getint - 1
            val distance = getint
            return Pair(node, distance)
        }
        val (root,dist) = ask((0 until n).toList())
        G.root = root
        G.treeOrderDFS(false)
        val depth = G.calculateDepth()
        val maxdepth = depth.maxOrNull()!!
        val atleast = dist divceil 2
        val atmost = minOf(maxdepth,dist)
        var highestfound = -1
        var highestnode = -1
        val actualdist = BinarySearchLastTrue(atleast,atmost){de ->
            val asking = ((0 until n).filter { depth[it] >= de })
            val (node, hered) = ask(asking)
            if(hered == dist){
                if(de > highestfound){
                    highestfound = de
                    highestnode = node
            ...
            return@BinarySearchLastTrue hered == dist
        }!!
        val final = highestnode
        val finaldist = dist
        val d = G.BFS(final)
        val finalcandidates = (0 until n).filter { d[it] == finaldist}
        val (final_node2, _) = ask(finalcandidates)
        println("! ${final+1} ${final_node2+1}")
        System.out.flush()
        val str = getstr
        if(str == "Correct"){
            return@cases
