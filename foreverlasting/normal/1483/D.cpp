...
#define tup tuple<int,int,int>
    ...
    inline void MAIN(const res &Case){
        n=read(),m=read();
        for(res i=1;i<=n;i++)for(res j=i+1;j<=n;j++)dis[i][j]=dis[j][i]=INF;
        for(res i=1;i<=m;i++){
            res u=read(),v=read(),w=read();
            dis[u][v]=dis[v][u]=min(dis[u][v],(LL)w);
            edge[i]=tup(u,v,w);
        }
        for(res k=1;k<=n;k++)for(res i=1;i<=n;i++)for(res j=1;j<=n;j++)dis[i][j]=min(dis[i][k]+dis[k][j],dis[i][j]);
        res q=read();
        while(q--){
            res u=read(),v=read();
            l[u][v]=l[v][u]=max(l[u][v],(LL)read());
        }
        for(res i=1;i<=n;i++){
            for(res v=1;v<=n;v++){
                for(res j=1;j<=n;j++)F[i][v]=max(F[i][v],l[j][i]-dis[j][v]);
            }
        }
        res ans=0;
        for(res t=1;t<=m;t++){
