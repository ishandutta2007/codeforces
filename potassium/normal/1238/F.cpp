...
inline void dfs_sz(int node, int parent) {
	sz[node] = 1;
	for (auto& i : g[node]) {
...
inline void dfs_solve(int node, int parent) {
	for (auto& i : g[node]) {
		if (i == parent) continue;
		dfs_solve(i,node);
		if (dp[i] > dp[best[node][0]]) {
			best[node][1] = best[node][0];
			best[node][0] = i;
		} else if (dp[i] > dp[best[node][1]]) {
			best[node][1] = i;
		}
	}
	
	if (parent == -1) {
		dp[node] = 1 + dp[best[node][0]] + dp[best[node][1]] + g[node].size() - (best[node][0] != 0) - (best[node][1] != 0);
		ans = max(ans,dp[node]);
	} else {
		dp[node] = 1 + dp[best[node][0]] + g[node].size() - 1 - (best[node][0] != 0);
	}
}

inline void dfs_reroot(int node, int parent) {
	for (auto& i : g[node]) {
		if (i == parent) continue;
		int originaldpnode = dp[node];
		int originaldpi = dp[i];
		
		int newdp;
		if (best[node][0] == i)
			newdp = 1 + dp[best[node][1]] + g[node].size() - 1 - (best[node][1] != 0) - (parent != -1);
		else
			newdp = 1 + dp[best[node][0]] + g[node].size() - 1 - (best[node][0] != 0) - (parent != -1);
			
		dp[node] = newdp;
			
		bool isTaken;
		if (newdp > dp[best[i][1]]) isTaken = 1; else isTaken = 0;
		
		auto originalbest = best[i];

		if (isTaken) {
			if (newdp > dp[best[i][0]]) {
				best[i][1] = best[i][0];
				best[i][0] = node;
			} else if (newdp > dp[best[i][1]]) {
				best[i][1] = node;
			}
		}
		
		dp[i] = 1 + dp[best[i][0]] + dp[best[i][1]] + g[i].size() - (best[i][0] != 0) - (best[i][1] != 0);
		//W(i,dp[i],best[i][0],dp[best[i][0]],best[i][1],dp[best[i][1]],g[i].size());
		ans = max(ans,dp[i]);
...
int main() {
	CASE(q) {
		int n; R(n);
		g.clear(); sz.clear(); best.clear(); dp.clear();
		g.resize(n+7); sz.resize(n+7,0); best.resize(n+7,vector<int>(2,0)); dp.resize(n+7);
		cur = 0;
		ans = 0;
		for (int i = 0; i < n-1; i++) {
			int u,v; R(u,v);
