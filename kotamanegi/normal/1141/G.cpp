...
#define int ll
int ans[400000];
vector<pair<int, int>> vertexs[400000];
int cnt[400000];
void dfs(int now, int back, int k,int B_color) {
    int now_color = -1;
    for (auto& x : vertexs[now]) {
        if (x.first == back) continue;
        now_color++;
        if (B_color == now_color) {
            now_color++;
        }
        now_color = min(now_color, k - 1);
        ans[x.second] = now_color;
        dfs(x.first, now, k, now_color);
    }
    return;
}
void solve() {
