...
#line 4 "/home/maspy/compro/library/seq/stirling_number_2.hpp"

// n  (labeled)  k  (no label) 
// label 
template <typename mint>
...
void init() { A = stirling_number_2_2d<mint>(LIM, LIM); }

void solve() {
  LL(N, M, K);

  mint a = floor(M, 2), b = ceil(M, 2);
  mint ANS = 0;

  mint inv = mint(M).inverse();
  mint cf = mint(M).pow(N);
  mint p = b * inv;
