...
void solve(vector<ll>v){
    if(v.size()==1)return;
    ll min=LINF,mindex=-1,max=-LINF,maxdex=-1;
    each(i,v){
        if(chmin(min,a[i]))mindex=i;
        if(chmax(max,a[i]))maxdex=i;
    }
    if(min+1==max){
        each(i,v)if(i!=mindex)add_edge(i,mindex);
        return;
    }
    auto b=query(maxdex);
    const ll d=a[maxdex]-a[mindex];
    sort(v.begin(),v.end(),[&](ll x,ll y){return a[x]+b[x]<a[y]+b[y];});
    vector<vector<ll>>vs(d+1);
    each(i,v)vs[(a[i]-a[mindex]-b[i]+d)/2].push_back(i);
    rep(d)add_edge(vs[i][0],vs[i+1][0]);
    each(i,vs)solve(i);
}
int main(){
    in(n);
