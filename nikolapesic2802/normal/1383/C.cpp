...
// URL : http://codeforces.com/contest/1383/problem/A
// Memory Limit : 256 MB
// Time Limit : 1000 ms
...
vector<vector<int> > graf(20),one(20);
vector<int> done(20);
vector<int> comp;
void dfs(int tr){
	done[tr]=1;
...
int solve(){
	...
	for(int i=0;i<n;i++)
		ok[1<<i]=1;
	int mx=0;
	for(int mm=1;mm<(1<<n);mm++){
		if(!ok[mm])
			continue;
		mx=max(mx,__builtin_popcount(mm));
		for(int i=0;i<n;i++)
			if((mm&(1<<i))==0){
				if((msk[i]&mm)==0){
					ok[mm^(1<<i)]=1;
...
void test(){
	...
	rd(n,a,b);
	for(int i=0;i<20;i++)
		graf[i].clear(),one[i].clear(),done[i]=0;
	for(int i=0;i<n;i++)
		if(a[i]!=b[i])
			graf[a[i]-'a'].pb(b[i]-'a'),graf[b[i]-'a'].pb(a[i]-'a'),one[a[i]-'a'].pb(b[i]-'a');
	int sol=0;
	for(int i=0;i<20;i++){
		if(done[i])
			continue;
