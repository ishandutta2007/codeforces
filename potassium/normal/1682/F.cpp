...
S op(S l, S r) {
	return S{ (l.m + r.m) % MOD107, (l.c + r.c) % MOD107 };
}
...
int main() {
	...
	for (int i = 0; i < q; i++) {
		int l,r; R(l,r);
		query[i].first = make_pair(l, r);
		query[i].second = i;
	}
	sort(query.begin(), query.end(), [&](auto x, auto y) {
		return ps[x.first.first - 1] < ps[y.first.first - 1];
	});
	...
	vector<S> st_base(n, e());
	for (int i = 0; i < n; i++) {
		st_base[i].m = -w[i];
		st_base[i].c = +w[i] * (v[i] % MOD107) % MOD107;
	}
	SegTree<S, op, e, F, mapping, composition, id> st = st_base;

	vector<ll> ans(q);
	for (int i = 0; i < q; i++) {
		ll curL = ps[query[i].first.first - 1];
		while (!pq.empty() && pq.top().first <= curL) {
			int idx = pq.top().second;
			pq.pop();

			st.set(idx, S{ +w[idx], -w[idx] * (v[idx] % MOD107) % MOD107 });
		}

		S res = st.prod(query[i].first.first, query[i].first.second);
		ans[query[i].second] = res.m * (curL % MOD107) + res.c;
	}

	for (auto& i : ans)
		i = (i % MOD107 + MOD107) % MOD107;
