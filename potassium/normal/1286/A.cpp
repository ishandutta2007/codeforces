...
int main() {
	...
	int ocnt = (n+1)/2, ecnt = n/2;
	for (int i = 0; i < n; i++)
		if (v[i] != 0)
			if (v[i] & 1) ocnt--; else ecnt--;
	...
	for (int i = 0; i < n; i++) {
		if (v[i] != 0) {
			if (i > 0) {
				...
				if (last == -1) {
					ans++;
					if (v[i] & 1) o.pb({len, -1}); else e.pb({len, -1});
				} else {
					if ((v[i] + v[last]) & 1 ^ 1) {
						ans += 2;
						if (v[i] & 1) o.pb({len, -2}); else e.pb({len, -2});
					} else {
						ans++;
					}
				}
	...
	if (len > 0) {
		ans++;
		if (v[last] & 1) o.pb({len, -1}); else e.pb({len, -1});
	}
	...
	for (auto& i : o) {
		if (i.S == -1) o1.pb(i.F); else o2.pb(i.F);
	}
	for (auto& i : e) {
		if (i.S == -1) e1.pb(i.F); else e2.pb(i.F);
	}
	
	int best = 0;
	int now = 0;
	for (int choose2 = -1; choose2 < (int)o2.size(); choose2++) {
		if (choose2 >= 0) {
			ocnt -= o2[choose2];
			if (ocnt < 0) break;
			now += 2;
