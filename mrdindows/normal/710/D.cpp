...
ll cnt(ll val, ll from, ll step) {
    if (from > val) return 0;
    return (val - from) / step + 1;
}

ll solve_naive(ll a1, ll b1, ll a2, ll b2, ll x, ll y) {
    ll ans = 0;
    for (ll i = max({x, b1, b2}); i <= y; ++i) {
...
ll solve(ll a1, ll b1, ll a2, ll b2, ll x, ll y)  {
    if (b2 < b1) {
        swap(a1, a2);
        swap(b1, b2);
    }
...
//    DBN(b1, b2);
    ...
    ll from = b2 + a2 * r;
//    DBN(from, step, x, y);
    return cnt(y, from, step) - cnt(x - 1, from, step);
}
int main() {
    ll a1, b1, a2, b2, x, y;
    cin >> a1 >> b1 >> a2 >> b2 >> x >> y;
    auto res = solve(a1, b1, a2, b2, x, y);
    cout << res << endl;
//
//    DBN(solve_naive(a1, b1, a2, b2, x, y));
        ...
        for (b1 = 1; b1 < 20; ++b1) {
            for (a2 = 1; a2 < 20; ++a2) {
                for (b2 = 1; b2 < 20; ++b2) {
                    for (x = 0; x < 100; ++x) {
                        for (int y = x; y < 100; ++y) {
                            if (solve(a1, b1, a2, b2, x, y) != solve_naive(a1, b1, a2, b2, x, y)) {
                                DBN(a1, b1, a2, b2);
                                DBN(x, y);
                                DBN(solve(a1, b1, a2, b2, x, y), solve_naive(a1, b1, a2, b2, x, y));
                            }
                        }
                    }
                }
