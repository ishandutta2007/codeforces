...
var dp = Array<IntArray>(101,{IntArray(101,{0})})
val cur = IntArray(100)

fun PrintWriter.ok(s: String) : Boolean {
    for (i in 0..s.length)
    ...
    for (i in 0..s.length)
        for (j in 0..s.length-i) {
            ...
            if (s[i+j]-'a' < cur[i] || s[i+j]-'a' < cur[j]) return true
            if (s[i+j]-'a' == cur[i]) dp[i+1][j] = 0
            if (s[i+j]-'a' == cur[j]) dp[i][j+1] = 0
        }
    return false
}

fun PrintWriter.undo(s: String, _i: Int, _j: Int) {
    var i = _i
    var j = _j
    val res = CharArray(i+j,{'1'})
    while (i > 0 || j > 0) {
        if (i > 0 && dp[i-1][j] == 0 && s[i+j-1]-'a' == cur[i-1]) {
            i --
            res[i+j] = 'R'
        } else {
            j --
...
fun PrintWriter.gen(s: String) {
    ...
    for (i in 0..s.length)
        for (j in 0..s.length-i) {
            ...
            if (s[i+j]-'a' < cur[i]) {
                undo(s,i,j)
                for (k in i+j..s.length-1) print('R')
                return
            }
            if (s[i+j]-'a' < cur[j]) {
                undo(s,i,j)
                for (k in i+j..s.length-1) print('B')
                return
            }
        }
...
fun PrintWriter.solve() {
    ...
    for (i in 1..t) {
        val s = read()
        for (j in 0..s.length-1) cur[j] = 25
