...
class H1MultithreadingEasyVersion {
public:
    void solve(istream& cin, ostream& cout) {
        int N, M; cin >> N >> M;
        string S; cin >> S;
        vector<int> E(3, 0), O(3, 0);
        for (int i = 0; i < N; ++i) {
            auto &F = i%2?O:E;
            if (S[i] == 'b') F[0]++;
            else if (S[i] == 'w') F[1]++;
        ...
        auto F = FF::fact(2*N);
        auto I = FF::invfact(2*N);

        FF ans = 0;
        for (int q = N%2; q <= N; q += 2) {
            FF cur = 0;
            if (O[2]-E[0]+O[0]+q >= 0 && O[0]-E[0]+q <= E[2]) cur += F[E[2]+O[2]] * I[(O[2]-E[0]+O[0]+q)] * I[E[2]+E[0]-O[0]-q];
            if (O[2]-E[0]+O[0]-q >= 0 && O[0]-E[0]-q <= E[2]) cur += F[E[2]+O[2]] * I[(O[2]-E[0]+O[0]-q)] * I[E[2]+E[0]-O[0]+q];
            ans += cur * q;
        }

//        ans *= F[E[2]] * F[O[2]];
        ans /= FF{2}.pow(count(S.begin(),S.end(),'?'));
        cout << ans << '\n';
    }
};
...
int main() {
	...
	H1MultithreadingEasyVersion solver;
