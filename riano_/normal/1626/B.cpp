...
int main() {
    ...
    rep(ii,T){
        ...
        rrep(i,N-1){
            ...
            if((int)(S[i]-'0')+(int)(S[i+1]-'0')>=10){
                int a = (int)(S[i]-'0')+(int)(S[i+1]-'0');
                TT[i] = '1'; TT[i+1] = (char)(a-10+'0'); ok = true;
                break;
            }
        }
        if(ok){
        ...
        rep(i,N-1){
            int a = (int)(S[i]-'0')+(int)(S[i+1]-'0');
            if((int)(S[i]-'0')<a||i==N-2){
                int a = (int)(S[i]-'0')+(int)(S[i+1]-'0');
                TT = S.substr(0,i) + (char)(a+'0') + S.substr(i+2,N-i-1); ok = true;
                break;
            }
        }
        cout << TT << "\n";
    ...
    // invf.assign(N+1,mint(1));
    // rep(i,N) fact[i+1] = fact[i]*mint(i+1);
    // rep(i,N+1) invf[i] = modinv(fact[i].val);
