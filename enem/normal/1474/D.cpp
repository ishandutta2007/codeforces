...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        if(dp[n-1]!=0) bad=true;
        if(!bad) cout<<"YES";
        else
        {
            if(abs(dp[n-1])%2!=0) cout<<"NO";
            else
            {
                ll x=dp[n-1]/2;
                set<ll> pos1;
                ...
                {
                    if(dp[i]<0) f1=true;
                    if((n-i)%2==1 && a[i]-a[i+1]==x || (n-i)%2==0 && a[i+1]-a[i]==x)
                    {
                        if(f1 && dp[i]+a[i+1]-a[i]>=0) pos1.insert(i);
                        else if(!f1) pos1.insert(i);
                    }
                    if(f1) break;
                }
                set<ll> pos2;
                ...
                REPI(i,0,n-1)
                {
                    ...
                    if(dp[i]>=0 && ((n-i)%2==1 && a[i]-a[i+1]==x || (n-i)%2==0 && a[i+1]-a[i]==x)) pos2.insert(i);

                    if((n-i)%2==1) dp[i]-=x;
                    else dp[i]+=x;

                    if(dp[i]<0&&bud) bad=true;
                    else if(dp[i]<0) f2=true;
                    if(f2) break;
                }
                if(bad) cout<<"NO";
                else
