...
pair<short, short> pidor[N * N];
int ptr;

void dfs() {
start:;
    assert(ptr <= n * m + 1);
    int i = pidor[ptr - 1].x, j = pidor[ptr - 1].y;
    used[i][j] = 1;
    short ni = i + dx[tb[i][j]];
    short nj = j + dy[tb[i][j]];
    if (!ch(ni, nj)) {
        dp[i][j] = 1;
        used[i][j] = 2;
        --ptr;
    ...
    goto start;
after:;
    i = pidor[ptr - 1].x, j = pidor[ptr - 1].y;
    ni = i + dx[tb[i][j]];
    nj = j + dy[tb[i][j]];
    //dfs(ni, nj, ht + 1);
    if (!dp[i][j]) {
...
//vector<char> gen = { 'L', 'R', 'U', 'D' };


char gen(int i, int j) {
    if (i == 0 && j != m - 1) return 'R';
    if (j == 0) return 'U';
    if (i == n - 1) return 'L';
    return 'D';
}
