...
int main(){
	...
	vector<modular> mat(n);
	for(auto mask = 0; mask < n; ++ mask){
		mat[mask] = b[__builtin_popcount(mask)];
	}
	fast_walsh_hadamard_transform(e);
	fast_walsh_hadamard_transform(mat);
	for(auto i = 0; i < n; ++ i){
		e[i] *= mat[i] ^ t;
	}
	fast_walsh_hadamard_transform(e, true);
	for(auto x: e){
		cout << x << "\n";
	}
	return 0;
