...
int n, m, a[N], b[N], g[N * 2], s[N * 4], c[N * 4];
vector<int> v[N * 2];

void get(int& x) { x = lb(g + 1, g + n + m + 1, x) - g; }
void add(int i) {
    for(; i <= n + m; i += i & -i) c[i]++;
}
...
void upd(int i, int l, int r, int o) {
    if(l == r) return void(c[o] = max(++s[o], 0));
    i <= mid ? upd(i, lch) : upd(i, rch);
    s[o] = s[lc] + s[rc], c[o] = max(c[lc], s[lc] + c[rc]);
}
void solve() {
    scanf("%d%d", &n, &m);
    rep(i, 1, n) scanf("%d", &a[i]), g[i] = a[i];
    rep(i, 1, m) scanf("%d", &b[i]), g[i + n] = b[i];
    sort(b + 1, b + m + 1);
    sort(g + 1, g + n + m + 1);
    rep(i, 1, n + m) c[i] = 0, v[i].clear();
    ll as = 0;
    per(i, n, 1) get(a[i]), v[a[i]].pb(i), add(a[i]), as += qry(a[i] - 1);
    bld(1, n, 1);
    int low = 0;
    rep(i, 1, m) {
        get(b[i]);
        For(j, b[i - 1], b[i]) {
            for(int k : v[j]) low++, upd(k, 1, n, 1);
            for(int k : v[j + 1]) upd(k, 1, n, 1);
        }
        as += low - c[1];
    }
    printf("%lld\n", as);
