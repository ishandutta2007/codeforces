...
int a[N];
int cutt;
inline void quest(int x){if(!a[x]){cout<<"? "<<x<<endl;a[x]=rin();cutt++;}return;}
inline void work()
{
    int n;
    n=rin();
    ...
    for(;l<=r;)
    {
        if(100-cutt>=(r-l-1))
        {
            for(int i=l+1;i<r;i++)quest(i);
            for(int i=l;i<=r;i++)if(a[i]<min(a[i-1],a[i+1])){cout<<"! "<<i<<endl;return;}
        }
        int mid=l+r>>1;
        quest(mid);quest(mid-1);quest(mid+1);
        if(a[mid-1]>a[mid]&&a[mid]<a[mid+1]){cout<<"! "<<mid<<endl;return;}
