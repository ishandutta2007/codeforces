...
int n, a, b;
int ans = 6;
void backtrack(int count, int left, int a_count, int b_count) {
    if (count >= ans) return;
    ...
    if (a_count < 4) {
        if (left >= a) backtrack(count, left - a, a_count + 1, b_count);
        else backtrack(count + 1, n, a_count, b_count);
    }
    if (b_count < 2) {
        if (left >= b) backtrack(count, left - b, a_count, b_count + 1);
        else backtrack(count + 1, n, a_count, b_count);
    }
}
void solve() {
