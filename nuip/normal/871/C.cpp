...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	int L,R;
	{
		...
		sort(all(as)); UNIQUE(as);
		sort(all(bs)); UNIQUE(bs);
		for(auto &[a,b]:ps){
			a=lower_bound(all(as),a)-as.begin();
			b=lower_bound(all(bs),b)-bs.begin();
		}
		L=as.size();
		R=bs.size();
	}
	UF uf(L+R);
	for(auto [l,r]:ps) uf.unite(l,L+r);
	vector<int> cnt(L+R);
	for(auto [l,r]:ps) ++cnt[uf.root(l)];
	mint re=modpow(2,L+R);
	rep(i,L+R)if(uf.root(i)==i && uf.size(i)==cnt[i]+1){
		int sz=uf.size(i);
		mint pw=modpow(2,sz);
		re/=pw;
		re*=pw-1;
