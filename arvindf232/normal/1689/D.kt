...
fun main(){
    solve.cases{
        ...
        fun solve(x:Int,report:Boolean):Boolean {
            var maxsum = plarge
            var minsum = nlarge
            var maxdia = plarge
            var mindia = nlarge
            for(i in 0 until n) {
                for(j in 0 until m){
                    ...
                    maxsum = minOf(maxsum, i+j+x)
                    minsum = maxOf(minsum, i+j-x)
                    maxdia = minOf(maxdia, i-j+x)
                    mindia = maxOf(mindia, i-j-x)
                }
            }
            for(i in 0 until n) {
                for(j in 0 until m) {
                    if((i + j) in minsum..maxsum && (i - j) in mindia..maxdia) {
                        if(report){
                            put("${i + 1} ${j + 1}")
                        }
                        return true
        ...
        val best = BinarySearchFirstTrue(0, n + m +3){
            solve(it, false)
        }
        solve(best!!,true)
