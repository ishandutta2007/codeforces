...
static map<int,int> ad[N];
static queue<int> pr;
static vector<int> v;
static vector<int> dis;
static int n,m,t1,t2,t3;
...
int main()
{
    ...
    while(!pr.empty())
    {
        for(auto it:ad[pr.front()])
        {
            if(dis[it.F]==-1)
            {
                if(it.S==3)
                {
                    dis[it.F]=dis[pr.front()]+1;
                    pr.push(it.F);
                }
                else if(v[it.F]==-1&&it.S==1)
                {
                    v[it.F]=1;
                }
                else if(v[it.F]==-1&&it.S==2)
                {
                    v[it.F]=0;
                }
                else if(v[it.F]==0&&it.S==1)
                {
                    dis[it.F]=dis[pr.front()]+1;
                    pr.push(it.F);
                }
