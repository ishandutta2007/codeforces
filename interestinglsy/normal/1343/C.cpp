...
ll Solve(){
	int posiMxLen = 0, negaMxLen = 0;
	static ll posiLenSum[MAXN], negaLenSum[MAXN];
	Forx(i,0,n+10) posiLenSum[i] = negaLenSum[i] = -LINF;
	posiLenSum[0] = negaLenSum[0] = 0;
	ll ans = -LINF; int ansLen = 0;
	For(i,n){
		int uDp = a[i] > 0 ? negaMxLen+1 : posiMxLen+1;
		ll uMxSum = (a[i] > 0 ? negaLenSum[uDp-1] : posiLenSum[uDp-1])+a[i];
		a[i] > 0 ? Mymax(posiMxLen,uDp) : Mymax(negaMxLen,uDp);
		a[i] > 0 ? Mymax(posiLenSum[uDp],uMxSum) : Mymax(negaLenSum[uDp],uMxSum);
		if( uDp > ansLen ){
			ans = uMxSum;
			ansLen = uDp;
		}elif( uDp == ansLen )
