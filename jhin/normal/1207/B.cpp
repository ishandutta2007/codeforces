    ...
    int main()
    {
        ...
        cin>>n>>m;yes=true;
        for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)cin>>bar[i][j];
        for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)
        {
            if(bar[i][j]&&bar[i][j+1]&&bar[i+1][j]&&bar[i+1][j+1])
            {bar2[i][j]=1;bar2[i][j+1]=1;bar2[i+1][j]=1;bar2[i+1][j+1]=1;tmpre.push_back(i);tmpre.push_back(j);}
        }
        for(int i=1;i<=n;i++)for(int j=1;j<=m;j++)if(bar[i][j]!=bar2[i][j])yes=false;
      if(yes)
      {cout<<tmpre.size()/2<<endl;
          for(int i=0;i<tmpre.size();i+=2)cout<<tmpre[i]<<" "<<tmpre[i+1]<<endl;
      }
     else cout<<-1;
        return 0;
    }