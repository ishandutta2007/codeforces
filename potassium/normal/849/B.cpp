...
int n;
vector<ll> v;
inline bool trystart(int a, int b) {
	bool isEven = (a == 0 && b == 2 && (v[b]-v[a]) % (b-a) != 0);
	ll d = ((v[b] - v[a]) / (b - a));
	ll first = v[a] - d*a;
	
	vector<int> remain;
	for (int i = 0; i < n; i++) {
		ll cur = first + (v[b]-v[a])*i/(b-a);
		if ((isEven && (i-a) % 2 == 1) || cur != v[i]) remain.pb(i);
	}
	
	if (remain.size() == 0) return false;
	
	ll second = v[remain[0]];
	for (int i = 0; i < remain.size(); i++) {
		int diff = remain[i] - remain[0];
		if (isEven && diff % 2 == 1) return false;
		ll cur = second + (v[b]-v[a])*diff/(b-a);
		if (cur != v[remain[i]]) return false;
	}
...
int main() {
	...
	if (trystart(0,1) || trystart(0,2) || trystart(1,2)) W("Yes"); else W("No");
