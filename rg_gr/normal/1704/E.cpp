...
signed main()
{
	...
	int T=read();rg(T)
	int n=read(),m=read();
	rg(n)t[i]=deg[i]=h[i]=0;gr cnt=1;
	rg(max(n,m)+1)rg_(j,max(n,m)+1)f[i-1][j-1]=g[i-1][j-1]=0;gr gr
	rg(n)a[i]=read();f[0][i]=a[i]%mod,g[0][i]=a[i]>=mod;gr
	rg(m)
	int u=read(),v=read();addedge(u,v,1);
	deg[v]++;
	gr
	int lst;
	rg(n)
	...
	while(!q.empty())
	{
		...
		#define i ii
		f[i][v]+=f[i-1][u];
		if(f[i][v]>=mod)f[i][v]-=mod,g[i][v]=1;
		else g[i][v]|=g[i-1][u];
		t[v]--;
		if(t[v]==0)q.push(v);
		#undef i
	...
	int ti=0,G=0,ns=0;
	// rg_(lst,n)
	// rrg(n+1)odb(g[i][lst]);gr puts("");
	// gr
	rrg(n+1)
	if(ns+f[i][lst]>=mod||g[i][lst])G=1;
	mae(ns,f[i][lst]);
	if(G||ns)mde(ns,1),ti=i+1;
	// odp(ns,G);
	gr
	// odp(ti,ns);
	odl(ae(ti,ns));
