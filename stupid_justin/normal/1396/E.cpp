...
void DFS(int u,int fa){Fa[u]=fa;sz[u]=1;dep[u]=dep[fa]+1;for (int v:T[u]) if (v!=fa) DFS(v,u),sz[u]+=sz[v];}
int now;
struct subtree{int id;bool operator<(const subtree &o)const{return sz[id]<sz[o.id];}};
priority_queue<subtree> Q;
struct lca{int x;bool operator<(const lca &o)const{return dep[x]==dep[o.x]?x<o.x:dep[x]<dep[o.x];}};
set<lca> S[N];//
