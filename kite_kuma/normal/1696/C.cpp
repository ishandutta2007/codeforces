...
V<pair<ll, ll>> shrink(vll v, ll m) {
	// int n  =(int) v.size();
	V<pair<ll, ll>> ret;							// kind, copies
	auto ad = [&](auto self, ll kind, ll copies) {	// cp < m
		if(ret.empty() || ret.back().first != kind) {
			ret.emplace_back(kind, copies);
			return;
		}
		ret.back().second += copies;
		// if(ret.back().second % m == 0) {
		// 	ll newvalue = ret.back().first * m;
		// 	ll newcp = ret.back().second / m;
		// 	ret.pop_back();
		// 	self(self, newvalue, newcp);
		// 	// ret.emplace_back(newvalue, newcp);
		// }
		return;
	};
	foa(t, v) {
