...
typedef array<int, 3> atpl;
typedef array<int, 2> apr;
typedef array<int, 4> qpr;
...
namespace number_theory {
	template<class T> vector<T> sieve(int n) {
		...
		for(ll i = 2; i <= n; i++)if(prime[i]) for(ll j=i*i; j <= n; j+=i) prime[j]=0;
...
#define scan(a) cin>>a;
#define scanpair(a) cin>>a.first>>a.second
#define scanar(a, n) {for(int _ = 0; _ < n; _++) cin >> a[_];}
void solve()
{
    string s;
    cin >> s;
