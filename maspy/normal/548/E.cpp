...
void solve() {
  ...
  FORIN(p, primes) FOR3(i, 1, max / p + 1) {
    mu[p * i] *= -1;
    if (i % p == 0) mu[p * i] = 0;
    ll n = p * i;
    ll sz = div[n].size();
    FOR(j, sz) div[n].eb(div[n][j] * p);
  }

  //  square free number d d 
  vi CNT(max + 1);
  vc<bool> isin(N);
  ll ANS = 0;

  auto add = [&](ll x, ll k) -> void { FORIN(d, div[x]) CNT[d] += k; };

  auto calc = [&](ll x) -> ll {
    ll cnt = 0;
    FORIN(d, div[x]) cnt += mu[d] * CNT[d];
    return cnt;
  };
