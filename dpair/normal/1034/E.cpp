...
int n, lim; unsigned long long a[MAXN], b[MAXN];

template < bool opt >
inline void FWT(unsigned long long *a) {
    for (int mid = 1;mid < lim;mid <<= 1) {
        for (int j = 0;j < lim;j += (mid << 1)) {
            REP(k, 0, mid) {
                if constexpr (opt) a[j | mid | k] += a[j | k];
...
signed main(){
    read(n); lim = 1 << n; char ch = getcha(); 
    while(ch <= 32) ch = getcha(); REP(i, 0, lim) a[i] = (1ull << (__builtin_popcount(i) << 1)) * (ch - 48), ch = getcha();
    while(ch <= 32) ch = getcha(); REP(i, 0, lim) b[i] = (1ull << (__builtin_popcount(i) << 1)) * (ch - 48), ch = getcha();
    FWT < 1 > (a); FWT < 1 > (b); REP(i, 0, lim) a[i] = a[i] * b[i];
    FWT < 0 > (a); REP(i, 0, lim) putcha(((a[i] >> (__builtin_popcount(i) << 1)) & 3) + 48);
}