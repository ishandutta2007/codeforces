...
int main() {
	...
	vector<vector<int>> discard(1);
	
	for (int i = 0; i < n; i++) {
		occ[a[i]]++;
		
		if (discard.size() <= occ[a[i]])
			discard.pb({});
			
		discard[occ[a[i]]].pb(a[i]);
	}
	
	for (auto& i : discard)
		sort(all(i));
	
	vector<ll> sz(discard.size(), 0);
	for (int i = 1; i < discard.size(); i++)
		sz[i] = m - discard[i].size();
	for (int i = 2; i < discard.size(); i++)
		sz[i] += sz[i-1];
	...
	while (q--) {
		...
		auto it = lower_bound(sz.begin() + 1, sz.end(), k);
		if (it == sz.end()) {
			k -= sz.back();
			W((k-1) % m + 1);
			continue;
		...
		while (R >= L) {
			int mid = (L + R) / 2;
			int rank = mid - (lower_bound(all(discard[loc]), mid) - discard[loc].begin());
