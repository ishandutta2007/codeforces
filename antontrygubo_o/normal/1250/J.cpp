...
int main() {
    ...
    vector<queue<int>> cnt1(26);
    vector<queue<int>> cnt2(26);

    queue<int> free1;
    queue<int> free2;

    for (int i = 0; i<n; i++) if (a[i]!='?') cnt1[a[i]-'a'].push(i); else free1.push(i);
    for (int i = 0; i<n; i++) if (b[i]!='?') cnt2[b[i]-'a'].push(i); else free2.push(i);

    for (int i = 0; i<26; i++)
    {
        while (cnt1[i].size() && cnt2[i].size())
        {
            answer.push_back(mp(cnt1[i].front(), cnt2[i].front()));
            cnt1[i].pop();
            cnt2[i].pop();
        }
    }
    ...
    for (int i = 0; i<26; i++)
    {
        while (free1.size() && cnt2[i].size())
        {
            answer.push_back(mp(free1.front(), cnt2[i].front()));
            free1.pop();
            cnt2[i].pop();
        }
    }
    ...
    for (int i = 0; i<26; i++)
    {
        while (free2.size() && cnt1[i].size())
        {
            answer.push_back(mp(cnt1[i].front(), free2.front()));
            free2.pop();
            cnt1[i].pop();
        }
    }

    while (free1.size() && free2.size())
    {
        answer.push_back(mp(free1.front(), free2.front()));
        free1.pop();
        free2.pop();
    }

    cout<<answer.size()<<endl;
    for (auto it: answer) cout<<it.first+1<<' '<<it.second+1<<endl;
}