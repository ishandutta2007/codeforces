...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        cs.emplace_back(mx, i);
    }
    sort(rbegin(cs), rend(cs));
    vector<int> ids;
    for (int i = 0; i < n && i < m; i++) {
        ids.push_back(cs[i].second);
    }
    int k = isz(ids);
    ...
    vector<vector<int>> d1(k, vector<int>(1 << n));
    for (int i = 0; i < k; i++) {
        for (int ms = 0; ms < (1 << n); ms++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                mx = max(mx, c);
                rotate(begin(b[i]), begin(b[i]) + 1, end(b[i]));
            }
            d1[i][ms] = mx;
        }
    }

    vector<vector<int>> d(k + 1, vector<int>(1 << n));
    for (int i = 0; i < k; i++) {
        for (int ms = 0; ms < (1 << n); ms++) {
            for (int sms = ms; sms > 0; sms = (sms - 1) & ms) {
                d[i + 1][ms] = max(d[i + 1][ms], d[i][ms ^ sms] + d1[i][sms]);
            }
            d[i + 1][ms] = max(d[i + 1][ms], d[i][ms]);
        }
    }
    cout << d[k][(1 << n) - 1] << "\n";
}