...
void orzck(){
    ...
    rep(n) (a[i]?vec1:vec2).pb(b[i]);
    sort(all(vec1),greater<int>());
    sort(all(vec2),greater<int>());
    if(sz(vec1)==sz(vec2)) print(accumulate(b,b+n,0ll)*2-*min_element(b,b+n));
    else{
        int res=0,m=min(sz(vec1),sz(vec2));
        rep(m) res+=vec1[i]+vec2[i];
        print(res+accumulate(b,b+n,0ll));
    }
}
