...
inline pii get (int root, int lo, int hi, int L, int R){
	...
	if (R<=mid){
		...
		return pii((t0.first + (LL)all[root] * sum[min(R,hi)-max(lo,L)])%mod, (t0.second + (LL)all[root] * (sum[min(R,hi)-max(lo,L)+1]-1))%mod);
	}
	if (L>mid){
		pii t0 = get(root*2+1,mid+1,hi,L,R);
		return pii((t0.first + (LL)all[root] * sum[min(R,hi)-max(lo,L)])%mod, (t0.second + (LL)all[root] * (sum[min(R,hi)-max(lo,L)+1]-1))%mod);
	}
	pii t0 = get(root*2, lo, mid, L, R);
