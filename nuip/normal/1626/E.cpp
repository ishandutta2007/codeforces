...
vector<int> solve(const vector<ll> &vs, const vv<int> &g){
	const int n=vs.size();
	cauto m=accumulate(all(vs),0);
	if(m==0) return vector(n,0);
	if(m==1){
		vector<int> re(n);
		cauto v=max_element(all(vs))-vs.begin();
		re[v]=1;
		for(cauto &u:g[v]) re[u]=1;
		return re;
	}
	cauto add=[](pii &p, const int x){
							if(p.X==x || p.Y==x) return false;
							if(p.X<0){
								p.X=x;
								return true;
							}else if(p.Y<0){
		...
		cauto dfs=
			[&](auto &&dfs,int v,int p)->void{
				...
				for(cauto &u:g[v])if(u!=p){
						dfs(dfs,u,v);
						if(bs[u].X>=0) add(bs[v],bs[u].X);
						if(bs[u].Y>=0) add(bs[v],bs[u].Y);
						if(vs[u]) add(bs[v],u);
					}
			};
		dfs(dfs,0,-1);
	...
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p,pii pr)->void{
				as[v]=pr;
				set<int> st;
				if(pr.X>=0) st.emplace(pr.X);
				if(pr.Y>=0) st.emplace(pr.Y);
				for(cauto &u:g[v])if(u!=p){
						cauto &pr=bs[u];
						if(pr.X>=0) st.emplace(pr.X);
						if(pr.Y>=0) st.emplace(pr.Y);
					}
				if(vs[v]) st.emplace(v);
				// out(v,pr,st,1);
				for(cauto &u:g[v])if(u!=p){
						cauto &pr=bs[u];
						if(pr.X>=0) st.erase(pr.X);
						if(pr.Y>=0) st.erase(pr.Y);
						pii p{-1,-1};
						auto it=st.begin();
						if(it!=st.end()){
							p.X=*it;
	...
	// out(as,bs,ps,1);
	vector<pii> oks(n,{-1,-1});
	queue<int> que;
	rep(v,n)if(vs[v] && add(oks[v],n)) que.emplace(v);
	while(que.size()){
		cauto v=que.front(); que.pop();
		cauto &[a,b]=oks[v];
		// out(v,a,b,1);
		if(ps[v]>=0 && oks[ps[v]].Y<0){
			cauto &[A,B]=bs[v];
			int f=0;
			if(a>=0 && A>=0 && a!=A && add(oks[ps[v]],A)) f=1;
			if(a>=0 && B>=0 && a!=B && add(oks[ps[v]],B)) f=1;
			if(b>=0 && A>=0 && b!=A && add(oks[ps[v]],A)) f=1;
			if(b>=0 && B>=0 && b!=B && add(oks[ps[v]],B)) f=1;
			if(f) que.emplace(ps[v]);
		}
		for(cauto &u:g[v]){
			if(u==ps[v]) continue;
			if(oks[u].Y>=0) continue;
			cauto &[A,B]=as[u];
			int f=0;
			if(a>=0 && A>=0 && a!=A && add(oks[u],A)) f=1;
			if(a>=0 && B>=0 && a!=B && add(oks[u],B)) f=1;
			if(b>=0 && A>=0 && b!=A && add(oks[u],A)) f=1;
			if(b>=0 && B>=0 && b!=B && add(oks[u],B)) f=1;
			if(f) que.emplace(u);
		}
	}
	// out(oks,1);
	vector<int> re(n);
	rep(i,n) re[i]=(oks[i].X>=0);
	return re;
}

#ifdef NUIP

#include "lib/random_graph.cpp"
...
void test(){
	rep(_,10000){
		...
		rep(v,n)for(cauto &u:g[v]) h[perm[v]].eb(perm[u]);
		vector<ll> vs(n);
		vector<ll> us(n);
		rep(st,1<<n){
			rep(i,n) vs[i]=(st>>i&1);
			rep(i,n) us[perm[i]]=vs[i];
			cauto exp=solve(vs,g);
			cauto act=solve(us,h);
			rep(v,n)if(exp[v]!=act[perm[v]]){
				out(v,exp[v],perm[v],act[perm[v]],vs,g,1);
				exit(0);
			}
		}
	}
