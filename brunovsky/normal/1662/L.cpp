...
int main() {
    ...
    auto left = iota_vector(N, [&](int i, int j) { return lt[i] < lt[j]; });
    auto right = iota_vector(N, [&](int i, int j) { return rt[i] < rt[j]; });
    vector<int> rank(N);
    for (int i = 0; i < N; i++) {
        rank[left[i]] = i;
    }

    // You don't go through them in order of time
    // You go through them in order of l or r and then you don't get the problem

    segtree<Segnode> seg(N, 0);
    for (int i : right) {
        if (v * t[i] >= abs(a[i])) {
