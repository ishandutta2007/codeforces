...
const int N = 1505, P = 6543;
int bad[N], v[N], vrow[N], vcol[N];
int a[N][N], b[N][N], pi[N];
mod ha[N], hb[N];
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            cin >> a[i][j], ha[i] = P*ha[i]+a[i][j];
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m; j++)
            cin >> b[i][j], hb[i] = P*hb[i]+b[i][j];
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            if (!v[j] && ha[i] == hb[j]) {
                v[j] = 1, pi[j] = i;
