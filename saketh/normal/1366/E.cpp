...
int main() {
    ...
    vi uniq = a; dedup(uniq);

    const int U = sz(uniq);
    vi locc(U);
    F0R (i, N) locc[index(uniq, a[i])] = i;

    vi pref_max(U + 1, -1);
    F0R (i, U) pref_max[i + 1] = max(pref_max[i], locc[i]);

    mn ans = 1;

    F0R (i, M) {
        auto it = lb(all(uniq), b[i]);
        if (it == uniq.end() || *it != b[i]) {
            ans = 0;
            break;
        }
        int inx = it - uniq.begin();

        int lo = pref_max[inx]; // must go after this one
        int hi = locc[inx]; // can't go after this one
