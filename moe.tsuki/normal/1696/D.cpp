...
const int maxn = 267890, mod = 1000000007;
	...
	while (t --> 0)
	{
		...
		for (int i = 1; i <= n; ++i)
		{
			...
			if (a[i] > a[i - 1])
			{
				...
				ans[i] = 1 + ans[*MN.lower_bound(mx.top())];
			} else
			{
				while (mn.top() != 0 && a[mn.top()] > a[i])
				{
					MN.erase(mn.top());
					mn.pop();
				}
				ans[i] = 1 + ans[*MX.lower_bound(mn.top())];
			}
			mx.push(i);
			mn.push(i);
			MX.insert(i);
