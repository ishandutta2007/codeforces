...
int main(int argc, char** argv) {
	...
	cin >> n >> x;
	FOR (i, 0, n) {
		cin >> data[i].l >> data[i].r >> data[i].cost;
		data[i].len = data[i].r - data[i].l + 1;
		posL[i] = i;
		posR[i] = i;
	}

	sort(posL, posL + n, [](const auto& A, const auto& B) {
		const auto& AA = data[A];
		const auto& BB = data[B];
		return AA.r < BB.r;
	});

	sort(posR, posR + n, [](const auto& A, const auto& B) {
		const auto& AA = data[A];
		const auto& BB = data[B];
		return AA.l < BB.l;
	});
	...
	FOR (i, 0, n) {
		const Data& d = data[i];
		minCost[d.len].insert(d.cost);
	}

	int ans = -1;
	int ptrL = 0, ptrR = 0;
	FORN (pos, 1, 200000) {
		while (ptrR < n && data[posR[ptrR]].l == pos) {
			const Data& d = data[posR[ptrR]];
			minCost[d.len].erase(minCost[d.len].find(d.cost));
			++ptrR;
		}

		while (ptrL < n && data[posL[ptrL]].r == pos) {
			const Data& d = data[posL[ptrL]];
			int sisa = x - d.len;
			if (sisa > 0 && !minCost[sisa].empty()) {
				int res = d.cost + *minCost[sisa].begin();
