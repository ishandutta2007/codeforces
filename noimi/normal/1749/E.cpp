...
template <typename T> pair<vector<int>, T> dijkstra(Wgraph<T> &g, int s, int t, const T INF = numeric_limits<T>::max()) {
...
int main() {
    TEST {
        ...
        int s = n * m, t = n * m + 1;
        rep(i, n) {
            if(ok[i][0]) g[s].eb(i * m + 0, ok[i][0] - 1);
            if(ok[i][m - 1]) g[i * m + (m - 1)].eb(t, ok[i][m - 1] - 1);
        }
        ...
        rep(i, n) {
            rep(j, m) {
                for(int ni : {i - 1, i + 1})
                    for(int nj : {j - 1, j + 1}) {
                        if(inc(ni, 0, n) and inc(nj, 0, m) and ok[i][j] and ok[ni][nj]) { g[i * m + j].eb(ni * m + nj, ok[ni][nj] - 1); }
                    }
            }
        }

        auto d = dijkstra(g, s, t, inf<int>).fi;
