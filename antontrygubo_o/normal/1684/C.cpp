...
{
    cin>>n>>m;
    a.clear(); a = vector<vector<int>>(n, vector<int>(m));
    for (int i = 0; i<n; i++)
        for (int j = 0; j<m; j++) cin>>a[i][j];

    vector<pair<ll, int>> sum(m);
    for (int i = 0; i<m; i++)
    {
        sum[i].second = i;
        for (int j = 0; j<n; j++) sum[i].first+=a[j][i];
    }
    ...
    for (int i = 0; i<m; i++)
    {
        for (int j = 0; j<n; j++) b[j][i] = a[j][sum[i].second];
    }

    for (int i = 0; i<n; i++)
        for (int j = 0; j<m-1; j++)
        {
            if (b[i][j]>b[i][j+1]) {cout<<-1<<endl; return;}
        }
    ...
    {
        bool check = true;
        for (int j = 0; j<n; j++) if (a[j][i]!=b[j][i]) check = false;
        if (!check) diff.push_back(i);
    }

    if (diff.size()==0) {cout<<1<<' '<<1<<endl;}
    else if (diff.size() == 2) {cout<<diff[0] + 1 <<' '<<diff[1] + 1<<endl;}
    else cout<<-1<<endl;
}
