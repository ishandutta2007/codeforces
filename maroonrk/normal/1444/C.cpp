...
struct unionfind{
	vi p,s,z;
	int c;
	unionfind(int n):p(n,-1),s(n,1),z(n),c(n){}
	pi find(int a){
		if(p[a]==-1)return pi(a,p[a]);
		int root,lz;tie(root,lz)=find(p[a]);
		return pi(root,z[a]^lz);
	}
	vc<pair<int*,int>> buf;
	void save(int&a){
		buf.eb(&a,a);
...
void slv(){
	...
	rep(_,m){
		...
		a--;b--;
		if(c[a]==c[b]){
			int res=uf.unite(a,b);
			if(res==-1)ok[c[a]]=0;
		}else{
			es[minmax(c[a],c[b])].eb(a,b);
		}
	}
	int home=uf.snapshot();
	int cnt=0;
	rep(i,k)if(ok[i])cnt++;
	ll ans=ll(cnt)*(cnt-1)/2;
	for(const auto&ysp:es){
		int a,b;tie(a,b)=ysp.a;
		if(ok[a]&&ok[b]){
			ans--;
			bool good=true;
			for(const auto&e:ysp.b){
				if(uf.unite(e.a,e.b)==-1)good=false;
			}
			if(good)ans++;
			uf.rollback(home);
		}
