...
int dist(point a, point b) {
	return max((lol)0, max(a.ff - b.ff, a.ss - b.ss)) + max((lol)0, max(b.ff - a.ff, b.ss - a.ss));
}
map<lol, vecl> mapit;
vecl fix(int start, lol has, vecl &cons) {
	if (mapit.find(has) != mapit.end()) {
		return mapit[has];
	}
	lol hass = has;
	while ((((lol)1 << start)& has)==0) {
		start++;
	}
	has -= ((lol)1 << start);
	vecl cur = fix(start, has, cons);
	vecl oth = fix(start, has& cons[start], cons);
	while (cur.size() <= oth.size()) {
		cur.push_back(0);
	}
	forin(i, oth) {
		cur[i + 1] += oth[i];
...
int main() {
	...
	defn defm
		vector<veci> cons(n);
	veci start(n); veci end(n);
	vecl pow(n + 1,1);
	fori(i, n) {
		into(start[i]) into(end[i])
			pow[i + 1] = ((i + 1) * pow[i])mod;
	}
	veci nonzero;
	veci ord(n);
	fori(i, m) {
	...
	forin(i, nonzero) {
		...
		ord[nonzero[i]] = num++;
		forin(j, cons[nonzero[i]]) {
			stk.push_back(cons[nonzero[i]][j]);
		}
		wne(stk) {
			int cur = stk.back();
			stk.pop_back();
			...
			ord[cur] = num++;
			forin(j, cons[cur]) {
				stk.push_back(cons[cur][j]);
			}
		}
	}
	vecl consimplfied(nonzero.size(),((lol)1 << nonzero.size())-1);
	forin(i,consimplfied) {
		forin(j, cons[nonzero[i]]) {
			consimplfied[ord[nonzero[i]]-1] -= ((lol)1 << ord[cons[nonzero[i]][j]]) >> 1;
		}
	}
	vector<point> adds;
	vector<point> subs;
	fori(i, n) {
		adds.push_back({ start[i], i });
		subs.push_back({ end[i], i });
	}
	sort(all(adds));
	sort(all(subs));
	int numregular = 0;
	...
	int bb = 0;
	forn(i, n + 1) {
		while(aa<n && adds[aa].ff==i){
			if (ord[adds[aa].ss]==0) {
				numregular++;
			}
			else {
				cur += ((lol)1 << ord[adds[aa].ss]) >> 1;
				lastans = fix(0, cur, consimplfied);
			}
			aa++;
		}
		forin(j, lastans) {
			int left = i - j;
			lastans[j] = lastans[j] mod;
			if (numregular >= left && left>= 0) {
				lol x = (((pow[numregular] * modInverse(pow[left]))mod) * modInverse(pow[numregular-left]))mod;
				ans = (ans + x * lastans[j])mod;
			}
		}
		while (bb < n && subs[bb].ff == i) {
			if (ord[subs[bb].ss] == 0) {
				numregular--;
			}
			else {
				cur -= ((lol)1 << ord[subs[bb].ss]) >> 1;
				lastans = fix(0, cur,consimplfied);
			}
			bb++;
		}
