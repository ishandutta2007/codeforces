...
ll solve(vector<pair<ll, P>> bl) {
    ll N = bl.size();
    vll val(4), deg(4);
    dsu d(4);
    rep(i, N) {
        ll u = bl[i].second.first;
        ll v = bl[i].second.second;
        val[u] += bl[i].first;
        val[v] += bl[i].first;
        deg[u]++; deg[v]++;
        d.merge(u, v);
    ...
    bool b = 1;
    rep(i, 4)b &= deg[i] % 2;
    if (lis.size() == 1 && b == 1) { return 0; }

    ll ans = 0;
    for (auto v : lis) {
        ll now = 0;
        for (ll x : v)now += val[x];
        ans = max(now, ans);
    }
    return ans/2;
...
int main() {
    ll N;
    cin >> N;
    vector<pair<ll, P>> bl(N);
    rep(i, N)cin >> bl[i].second.first >> bl[i].first >> bl[i].second.second;
    rep(i, N) {
        bl[i].second.first--;
        bl[i].second.second--;
    }
    ...
    ll ans = 0;
    rep(x, 1ull << L) {
        vector<pair<ll, P>> nbl;
        rep(i, N) {
            if (i >= L || ((x >> i) & 1))nbl.push_back(bl[i]);
        }
        ll now = solve(nbl);
        ans = max(now, ans);
    }
