...
void solve(){
  ...
  vector<ll>a(n);
  rep(i,1,n)cin>>a[i];
  vector<vector<ll>>dv(n);
  ll dm=0;
  {
    auto dfs=[&](auto &&f,ll k,ll d)->void{
      dv[d].PB(k);
      chmax(dm,d);
      for(auto to:g[k]){
        f(f,to,d+1);
  ...
  rep(i,0,dm){
    vector<ll>val;
    for(auto z:dv[i+1])val.PB(a[z]);
    auto mp=compress(val);
    ll sz=mp.size();
    vector<ll>mx(sz,-INF);
    for(auto z:dv[i]){
      for(auto to:g[z]){
        chmax(mx[mp[a[to]]],dp[z]);
        chmax(dp[to],dp[z]+a[to]-val[0]);
        chmax(dp[to],dp[z]+val[sz-1]-a[to]);
      }
    }
    auto segl=range_max::make(sz);
    auto segr=range_max::make(sz);
    rep(i,0,sz){
      segl.set(i,mx[i]-val[i]);
      segr.set(i,mx[i]+val[i]);
    }
    segl.build();
    segr.build();
    for(auto z:dv[i+1]){
      ll idx=mp[a[z]];
      chmax(dp[z],a[z]+segl.query(0,idx));
      chmax(dp[z],segr.query(idx,sz)-a[z]);
      /*if(n==14&&z==5){
        segl.print();
        segr.print();cout<<idx spa sz<<endl;
        cout<<a[z] spa val[idx] spa idx spa dp[z] spa sz<<endl;
        cout<<segl.query(0,idx) spa segr.query(0,sz)<<endl;
      }*/
    }
  }
  //if(n==14)debug(dp,n);
