...
bool check(ll l, ll r)
{
	...
	rep(i, r+1, n-(r+1)+1) if(a[i] != a[n-i+1]) return false;
	
	if(r < n-r+1){
		rep(i, n-r+1, n-l+1) cnt[a[i]]--;
	}
	else{
		rep(i, r+1, n-l+1) cnt[a[i]]--;
	}
	
	rep(i, 1, n) if(cnt[i] < 0) return false;
	
	ll ocnt = 0;
	rep(i, 1, n) ocnt += cnt[i]%2;
	return ocnt <= 1;
}

ll solve(ll l)
{
	ll ub = n-l+1, lb = l-1, mid;
	while(ub-lb>1){
		mid = (ub+lb)/2;
		if(check(l, mid)) ub = mid;
		else lb = mid;
	}
	return l*(n-ub+1);
}
