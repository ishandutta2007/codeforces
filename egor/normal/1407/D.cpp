...
class DDiskretnieTsentrobezhniePrizhki {
public:
    void solve() {
        int n = in.readInt();
        ...
        vi small(1);
        vi big(1);
        arri answer(n, numeric_limits<int>::max());
        answer[0] = 0;
        for (int i : range(1, n)) {
            int last = -1;
            while (!small.empty() && h[small.back()] > h[i]) {
                if (h[small.back()] != last) {
                    minim(answer[i], answer[small.back()] + 1);
                }
                last = h[small.back()];
                small.pop_back();
            }
            ...
            while (!big.empty() && h[big.back()] < h[i]) {
                if (h[big.back()] != last) {
                    minim(answer[i], answer[big.back()] + 1);
                }
                last = h[big.back()];
                big.pop_back();
            }
...
int main() {
    ...
    auto time = clock();
#endif
    DDiskretnieTsentrobezhniePrizhki solver;
