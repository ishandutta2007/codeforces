...
void setup() {
	fact[0] = 1;
	for(ll i = 1; i < 20200; i++)
		fact[i] = (fact[i-1]*i)%mod;
	for(ll i = 0; i < 20200; i++)
		ifact[i] = bp(fact[i], mod-2);
}
ll nck(ll n, ll k) {
	ll res = (fact[n]*ifact[n-k])%mod;
...
int main() {
	doin();
	setup();
	ll a, b, c, ansa, ansb, ansc;
	ansa=ansb=ansc=0;
	cin >> a >> b >> c;
	for(ll i = 0; i <= min(a, b); i++) {
		ll x = MM(MM(nck(a, i)*nck(b, i))*fact[i]);
		ansa = MM(ansa+x);
	}
	for(ll i = 0; i <= min(b, c); i++) {
		ll x = MM(MM(nck(c, i)*nck(b, i))*fact[i]);
		ansb = MM(ansb+x);
	}
	for(ll i = 0; i <= min(a, c); i++) {
		ll x = MM(MM(nck(c, i)*nck(a, i))*fact[i]);
		ansc = MM(ansc+x);
	}
	cout << MM(MM(ansa*ansb)*ansc);
}