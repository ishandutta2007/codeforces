...
struct SumKeeper{
	...
	il bool IsFull( int x1 , int y1 , int x2 , int y2 ){
		return Sum(x1,y1,x2,y2) == (x2-x1+1)*(y2-y1+1);
	}
}sums[4];

int expand[MAXN][MAXN];
void GetExpand(){
	For(i,n-1) For(j,m-1){
		auto Check = [&]( int t )->bool{
			if( i-t+1 < 1 or j-t+1 < 1 or i+t > n or j+t > m ) return false;
			return	sums[0].IsFull(i-t+1,j-t+1,i,j) and
					sums[1].IsFull(i-t+1,j+1,i,j+t) and
					sums[2].IsFull(i+1,j+1,i+t,j+t) and
					sums[3].IsFull(i+1,j-t+1,i+t,j);
		};
		int ans = 0;
		fOR0(p,9){
			int vAns = ans + (1<<p);
			if(Check(vAns)) ans = vAns;
		}
		expand[i][j] = ans;
	}

	// For(i,n) Print(expand[i],1,m);
}
...
struct MaxKeeper{
	...
	int st[10][10][MAXN][MAXN];
	void Build(){
		GetLo();
		For(i,n) For(j,m) st[0][0][i][j] = expand[i][j];
		For(i,n)
			Forx(lg,1,8)
				For(j,m-(1<<lg-1))
					st[0][lg][i][j] = max(st[0][lg-1][i][j],st[0][lg-1][i][j+(1<<lg-1)]);
		For(j,m)
			Forx(lg,1,8)
				For(i,n-(1<<lg-1))
					st[lg][0][i][j] = max(st[lg-1][0][i][j],st[lg-1][0][i+(1<<lg-1)][j]);
		Forx(lgI,1,8)
			Forx(lgJ,1,8)
				For(i,n-(1<<lgI-1))
					For(j,m-(1<<lgJ-1))
						st[lgI][lgJ][i][j] = max({
							st[lgI-1][lgJ-1][i][j],
							st[lgI-1][lgJ-1][i+(1<<lgI-1)][j],
							st[lgI-1][lgJ-1][i][j+(1<<lgJ-1)],
							st[lgI-1][lgJ-1][i+(1<<lgI-1)][j+(1<<lgJ-1)]
						});
	}

	il int QueryMax( int x1 , int y1 , int x2 , int y2 ){
		int loX = lo[x2-x1+1], loY = lo[y2-y1+1];
		int ans = max({
			st[loX][loY][x1][y1],
			st[loX][loY][x2-(1<<loX)+1][y1],
			st[loX][loY][x1][y2-(1<<loY)+1],
			st[loX][loY][x2-(1<<loX)+1][y2-(1<<loY)+1]
		});
		return ans;
	}
}maxKeeper;
...
int Solve( int x1 , int y1 , int x2 , int y2 ){
	auto Check = [&]( int len )->bool{
		if( len*2 > x2-x1+1 or len*2 > y2-y1+1 ) return false;
		return maxKeeper.QueryMax(x1+len-1,y1+len-1,x2-len,y2-len) >= len;
	};
	int ans = 0;
	fOR0(p,9){
		int vAns = ans + (1<<p);
