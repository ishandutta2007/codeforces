...
int main() {
    ...
    unordered_map<ss, mn> dp;
    ss es; es[0] = 1; dp[es] = 1;

    vvi ks(N + 1);
    F0R (i, 1 << N) ks[__builtin_popcount(i)].pb(i);

    F0R (i, N) {
        decltype(dp) DP;
        F0R (es, 1 << N) {
            mn ch = 1;
            F0R (j, N) ch *= es & (1 << j) ? p[i][j] : 1 - p[i][j];

            trav (e, dp) {
                ss r;
                trav (m, ks[i+1]) {
                    for (int u = m & es; u; u -= u&-u) {
                        if (e.f[m ^ (u&-u)]) r[m] = 1;
                    }
                }
                DP[r] += e.s * ch;
    ...
    mn ans = 0;
    trav (e, dp) if (e.f.count()) ans += e.s;
    ps(ans);
