...
void solve(LL x)
{
    LL gate=(4*x/3) + 123456;
    for (LL i=1;gate>=i*i;i++)
    {
        LL need=i*i-x;
        if (need <= 0) continue;
        //find a m such that (n/m) == need
        if (st.find(need) == st.end()) continue;
        LL L=1,R=i+1; //L is the answer
        while (R-L != 1)
        {
            LL mid=(L+R)>>1;
            LL val=i/mid;
            if (val*val >= need) L=mid;
            else R=mid;
        }
        //cout << "i = "<<i<<" , L = "<<L<<endl;
        if ((i/L)*(i/L) == need)
