...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (a[i] == 0) ways = ways * invm % MOD;
        if (b[i] == 0) ways = ways * invm % MOD;

        if (a[i] == 0 && b[i] == 0) ans = (ans + ways * m % MOD * (m - 1) % MOD * (MOD + 1) / 2) % MOD;
        else if (a[i] == 0) ans = (ans + ways * (m - b[i])) % MOD;
        else if (b[i] == 0) ans = (ans + ways * (a[i] - 1)) % MOD;
        else ans = (ans + ways * (a[i] > b[i])) % MOD;

        if (a[i] == 0 && b[i] == 0) ways = ways * m % MOD;
        if (a[i] != b[i] && a[i] != 0 && b[i] != 0) break;
    }

    //cout << ans << "\n";
    cout << ans * binpow(q, MOD - 2) % MOD << "\n";
}
