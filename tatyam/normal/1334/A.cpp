...
int solve(){
    LL(n);
    VEC(pll,a,n);
    a.insert(a.begin(),pll{});
    rep(n)if(a[i+1].first-a[i].first<a[i+1].second-a[i].second)return NO();
    rep(n)if(a[i+1].first<a[i].first)return NO();
    rep(n)if(a[i+1].second<a[i].second)return NO();
    return YES();
}
signed main(){
    LL(t);
