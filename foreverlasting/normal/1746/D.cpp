...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		n=read(),k=read();
		for(int i=1;i<=n;i++)vector<int>().swap(G[i]),map<int,LL>().swap(f[i]);
		for(int i=2;i<=n;i++)G[read()].pb(i);
		for(int i=1;i<=n;i++)s[i]=read();
		function<void(int,int)> dfs=[&](int x,int lim){
			if(f[x].count(lim))return;
			f[x][lim]=(LL)lim*s[x];
			if(G[x].empty())return;
			int sz=(int)G[x].size();
			int L=lim/sz;
			...
			for(auto tox:G[x]){
				...
				LL g=f[tox][L];
				res+=g;
				if(lim%sz==0) continue;
				dfs(tox,L+1);
				LL h=f[tox][L+1];
				A.pb(h-g);
			}
			sort(A.begin(),A.end(),greater());
			for(int i=0,s=lim%sz;i<s;i++)res+=A[i];
			f[x][lim]+=res;
		};
		dfs(1,k),printf("%lld\n",f[1][k]);
	}
