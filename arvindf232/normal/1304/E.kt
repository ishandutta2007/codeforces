...
fun main(){
    solve.cases{
        ...
        repeat(q){
            ...
            val (a,b,x,y,k) = L
            val a1 = G.distance(x,y)
            val a2 = G.distance(x,b) + 1 + G.distance(a,y)
            val a3 = G.distance(x,a) + 1 + G.distance(b,y)
            var best = IntArray(2){Int.MAX_VALUE}
            for(i in listOf(a1,a2,a3)){
                best[i %2 ] = minOf(best[i%2], i)
            }
            val can = best[k%2]
            if(k >= can) {
                put("YES")
