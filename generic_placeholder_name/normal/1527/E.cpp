...
public:
    SegTree(int n): n(n), mn(4*n, 0), lz(4*n, 0){}
...
int32_t main() {
    ...
    vi lst(n, -1);
    vector<SegTree> dp(k + 1, n);
    lst[a[0]] = 0;
    dp[0].upd(1, n - 1, INF);
    rep1(i, k - 1) dp[i].upd(0, 0, INF);
    rep1(i, n - 1) {
        auto& l = lst[a[i]];
        rep1(j, k - 1) dp[j].upd(i, i, dp[j - 1].get(0, i - 1));
        if(~l) rep(j, k) dp[j].upd(0, l, i - l);
        l = i;
    }
    cout << dp[k - 1].get(0, n - 1) << endl;
}