...
int main(){
  ...
  auto b=vec(3,n+1,0LL);
  rep(i,0,3)rep(j,0,n)b[i][j+1]=b[i][j]+a[i][j];
  vector<ll>l(q),r(q),k(q);
  rep(i,0,q)cin>>l[i]>>r[i]>>k[i],l[i]--,r[i]--;
  ...
  vector<vector<pair<pi,ll>>>add(n+1);
  vector<ll>init(n+1);
  init[0]+=inf;
  init[n]-=inf;
  rrep(i,0,n){
    for(auto z:g[i]){
      if(mp.count(l[z])&&mp[l[z]]<=k[z])continue;
      auto it=mp.lower_bound(l[z]);
      auto itl=prev(it);
      if(itl->se<=k[z])continue;
      add[i].EB(MP(l[z],it->fi),-itl->se);
      init[l[z]]-=itl->se;
      init[it->fi]+=itl->se;
      while(1){
        if(it->se>=k[z]){
          auto nit=next(it);
          add[i].EB(MP(it->fi,nit->fi),-it->se);
          init[it->fi]-=it->se;
          init[nit->fi]+=it->se;
          it=mp.erase(it);
        }
  ...
  //debug(add);
  auto seg=add_min::make(n+1);
  rep(i,0,n)init[i+1]+=init[i];
  rep(i,0,n+1){
    seg.set(i,init[i]);
  }
  seg.build();
  ...
  rep(i,0,n+1){
    ...
    //seg.print();
    if(i>0){
      chmin(mi,seg.query(0,i));
      chmin(res,mi-(b[2][n]-b[2][i-1])-b[1][i]);
    }
    if(i==n)break;
    chmin(mi,-b[0][i+1]+b[1][i]);
    //cout<<mi spa -b[0][i+1]+b[1][i]<<endl;
    seg.update(i,i+1,mi);
    //seg.print();
    if(i>0){
      for(auto z:add[i-1]){
        seg.update(z.fi.fi,z.fi.se,-z.se);
      }
    }
  }
  cout<<-res<<endl;
