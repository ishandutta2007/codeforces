...
fun main(){
    solve.cases{
        ...
        val free = IntArray(str.length){if(str[it] == '?') 1 else 0}
        val rsq = rsqArr(free)
        val n = str.length
        for(d in 0 until str.length){
            for(left in str.indices){
                val right = d + left
                if(! str.indices.contains(right)){
                    continue
                }
                val leftmove = if(left < str.lastIndex) DP[left+1][right] else 0
                val rightmove = if(right > 0) DP[left][right-1] else 0
                if(str[left] == ')'){
                    DP[left][right] = leftmove
                }else if(str[right] == '('){
                    DP[left][right] = rightmove
                }else if(left == right) {
                    DP[left][right] = 0
                }else{
                    var ret = if(left <= right -2) DP[left+1][right-1] else 0
                    val counts = if(left <= right -2) rsq.sumQuery(left+1,right-1).toInt() else 0
                    ret = (ret + intPow(2,counts,pI)).adjust()
                    val freeA = str[left] == '?'
                    val freeB = str[right] == '?'
                    if(freeA) ret = (ret + leftmove).adjust()
                    if(freeB) ret = (ret + rightmove).adjust()
                    if(freeA && freeB) ret = (ret - if(left <= right -2) DP[left+1][right-1] else 0).adjust()
                    DP[left][right] = ret
                }
            }
        }
