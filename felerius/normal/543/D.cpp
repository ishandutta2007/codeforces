...
// Problem: (70) Road Improvement (Difficulty: 6) (http://codeforces.com/problemset/problem/543/D)
...
void dfs1(int v) {
    dp[v] = 1;
    trav(v2, adj[v])
        dfs1(v2), dp[v] *= dp[v2] + 1;
}

void dfs2(int v, MI up) {
    int d = sz(adj[v]);
    vMI pref(d);
    rep(i, d)
        pref[i] = dp[adj[v][i]] + 1;
    auto suff = pref;
    reps(i, 1, d) pref[i] *= pref[i-1];
    for (int i = d - 2; i >= 0; --i) suff[i] *= suff[i+1];

    res[v] = dp[v] * (up + 1);
    rep(i, d)
        dfs2(adj[v][i], (up + 1) * (i ? pref[i-1] : 1) * (i + 1 == d ? 1 : suff[i+1]));
}
