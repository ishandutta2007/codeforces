#line 1 "codeforces-workspace\\contest\\956\\b\\b.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\956\\b\\b.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\956\\b\\b.cpp"
...
void main() {
  ...
  input<int> n, u;
  input<vector<i32>> e(n);
  auto pos = e.begin();
  double ans{};
  for (auto i : iterate(e)) {
    while (pos != e.end() && *pos <= *i + u) {
      ++pos;
    }
    if (prev(pos) <= i + 1) {
      continue;
    }
    chgr(ans, double(*prev(pos) - *next(i)) / (*prev(pos) - *i));
  }
  if (ans == 0) {
    eject(-1);
  }
