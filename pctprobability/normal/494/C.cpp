...
map<ll,ld> merge(map<ll,ld> a1,map<ll,ld> a2){
  map<ll,ld> m;
  for(auto e:a1){
    if(e.se<eps) continue;
    for(auto e2:a2){
      if(e2.se<eps) continue;
      m[max(e.fi,e2.fi)]+=e.se*e2.se;
    }
  }
  return m;
}
map<ll,ld> add(map<ll,ld> a1,ld p){
  map<ll,ld> m;
  for(auto e:a1){
    m[e.fi+1]+=e.se*p;
    m[e.fi]+=e.se*(1.0-p);
...
ll l[10000],r[10000],a[1000000],seen[10000];
ll n,q;
ld p[10000];
map<ll,ld> ans[10000];
vector<ll> g[10000];
