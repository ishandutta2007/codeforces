...
public class E {
	...
	void solve()
	{
		...
		int[] argsmax = new int[n+1];
		Arrays.fill(args, -1);
		Arrays.fill(argsmax, -1);
		dpmax[0] = 0;
		argsmax[0] = 0;
		
		long def = (ai[n-1]>>>32) - (ai[0]>>>32);
		for(int i = 0;i < n;i++){
			if(i-2 >= 0){
				dp[i+1] = dpmax[i-2];
				args[i+1] = argsmax[i-2];
			}
			if(i < n-1){
				long v = dp[i+1] + (ai[i+1]>>>32) - (ai[i]>>>32);
				dpmax[i+1] = Math.max(dpmax[i], v);
				if(v > dpmax[i]){
					argsmax[i+1] = i+1;
				}else{
					argsmax[i+1] = argsmax[i];
				}
			}
		}
		int clus = 1;
