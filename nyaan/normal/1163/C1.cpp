...
inline pii nya(pii &i,pii &j){
    int x = i.fi-j.fi, y = i.se-j.se;
    int k = gcd( abs(x),abs(y) );
    if(x > 0) return mp( x / k, y / k);
    else if(x == 0) return mp(x / k, abs(y) / k);
    else return mp( x / (-1) / (k), y / (-1) / k);
}
...
int main(){
    ...
    FOR(i,1,N){
        ...
        rep(i,sz(cur)) line.pb(cur[i]);
    }

    sort(all(line) );
    trc(line);
    ll ans = (ll) sz(line) * ( (ll)sz(line) - (ll)1) / (ll)2;
    trc(ans);
    pii cur; int curcnt = 0;
    rep(i,sz(line) + 1){
        if(i == 0) {cur = line[i];curcnt = 1;}
        else if(i == sz(line)){
            ans -= curcnt * (curcnt - (ll)1) / (ll)2;
            break;
        }
        else {
            if(cur.fi == line[i].fi && cur.se == line[i].se){
                curcnt++;
            }
            else{
                ans -= curcnt * (curcnt - (ll)1) / (ll)2;
