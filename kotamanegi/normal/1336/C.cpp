...
#define int ll
modint<MOD> dp[3002][3002];
void solve() {
    string s, t;
    cin >> s >> t;
    for (int q = t.length(); q <= s.length(); ++q) {
        dp[0][q] = 1;
    }
    for (int lefts = 0; lefts <= s.length();++lefts) {
        for (int rights = s.length(); rights > lefts; --rights) {
            if (dp[lefts][rights].value == 0) continue;
            char now_look;
            int tmp = (rights - lefts) - 1;
            now_look = s[tmp];
            if (lefts >= t.length() || t[lefts] == now_look) {
                dp[lefts + 1][rights] += dp[lefts][rights];
            }
            if (rights - 1 >= 0) {
                if (rights - 1 >= t.length() || t[rights - 1] == now_look) {
                    dp[lefts][rights - 1] += dp[lefts][rights];
                }
            }
        }
    }
