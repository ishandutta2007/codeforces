...
signed main(){
    ...
    vec(pll,p,0);
    p.emplace_back(x,y);
    while(double(x)*ax+bx<LINF&&double(y)*ay+by<LINF){
        x=x*ax+bx;
        y=y*ay+by;
        p.emplace_back(x,y);
    }
    ll ans=0;
    rep(p.size())rep(j,i,p.size())if(abs(xs-p[i].first)+abs(ys-p[i].second)+abs(p[i].first-p[j].first)+abs(p[i].second-p[j].second)<=t||abs(xs-p[j].first)+abs(ys-p[j].second)+abs(p[i].first-p[j].first)+abs(p[i].second-p[j].second)<=t)chmax(ans,j-i+1);
    out(ans);
}