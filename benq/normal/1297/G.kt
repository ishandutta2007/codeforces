...
var dp = Array<ArrayList<Int>>(100005,{ArrayList<Int>()})
val nex = Array<IntArray>(100005,{IntArray(10,{-1})})
...
fun PrintWriter.solve() {
    ...
    for (i in 1..m) {
        ...
        if (m%i == 0) {
            todo.add(i)
            if (i*i != m) todo.add(m/i)
        }
    }
    todo.sort()
    for (i in 1..9) {
        var ind = 0
        for (j in 0..todo.size-1) {
            while (i.toLong()*todo[ind] < todo[j]) ind ++
            if (i.toLong()*todo[ind] == todo[j].toLong()) nex[j][i] = ind
        }
    }
    dp[0] = ArrayList<Int>()
    for (j in 0..todo.size-1) dp[0].add(0)
    dp[0][0] = 1
    for (i in 1..100000) {
        for (j in 0..todo.size-1) {
            dp[i].add(0)
            for (q in 1..9) if (nex[j][q] != -1) {
                dp[i][j] += dp[i-1][nex[j][q]]
                dp[i][j] = minOf(dp[i][j],MOD)
            }
        }
        if (dp[i][todo.size-1] >= k) {
            var I = i
            var cur = todo.size-1
            while (I > 0) {
                for (q in 1..9) if (nex[cur][q] != -1) {
                    if (dp[I-1][nex[cur][q]] >= k) {
                        I -= 1
                        cur = nex[cur][q]
                        print(q)
                        break
