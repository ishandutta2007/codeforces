//copied from https://judge.yosupo.jp/submission/7968
#include <bits/stdc++.h>
using namespace std;
...
signed main(){
    ...
    for (int i = 1; i <= n; i += 1) if (mu[i]) {
        int nn = (n + i - 1) / i;
        if(not mp.contains(nn)) {
            mp[nn] = stirling_second(nn);
            for (int i = 1; i <= nn; i += 1) mp[nn][i] += mp[nn][i - 1];
        }
        if (mu[i] == 1) ans += mp[nn][min(nn, k)] - mint(1);
        else ans -= mp[nn][min(nn, k)] - mint(1);
    }
    cout << ans << "\n";
}