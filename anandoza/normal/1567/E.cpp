...
struct S {
    ...
    friend S operator+(const S &l, const S &r) {
        ...
        S s{l.prefix, r.suffix, l.first, r.last, l.interior + r.interior, l.len + r.len};

        if (l.last <= r.first) {
            if (l.prefix == l.len)
                s.prefix = l.len + r.prefix;
            if (r.suffix == r.len)
                s.suffix = r.len + l.suffix;
            s.interior += l.suffix * r.prefix;
        }
...
S create(int x) { return S{1, 1, x, x, 1, 1}; }

S identity{-1, -1, -1, -1, -1, -1};
...
int main() {
    ...
    segment_tree st(n, identity, plus<S>());
    st.assign(L1(u, create(a[u])));
    ...
    while (q--) {
        ...
        if (t == 1) {
            ...
            re(i, v);
            i--;
            st.assign(i, create(v));
        } else {
            int l, r;
            re(l, r);
            l--;
            auto res = st.accumulate(l, r);
            ps(res.interior);
        }
    }
