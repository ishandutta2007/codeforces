#line 1 "codeforces-workspace/contest/1461/e/e.cpp"
/**
 * @file template.cpp
 * @brief Template
 */
...
template <class Tp, typename = std::nullptr_t> struct helper {
  helper(std::istream &is, Tp &x) {
    if constexpr (has_begin<Tp>::value)
      for (auto &&e : x) helper<typename std::decay<decltype(e)>::type>(is, e);
    else
...
template <> struct helper<__int128_t, std::nullptr_t> {
  helper(std::istream &is, __int128_t &x) {
    std::string s;
    is >> s;
...
template <> struct helper<__uint128_t, std::nullptr_t> {
  helper(std::istream &is, __uint128_t &x) {
    std::string s;
    is >> s;
    x = 0;
    bool negative = s.front() == '-' ? s.erase(s.begin()), true : false;
...
namespace internal {
static auto *const cin_ptr = (istream *)&std::cin;
}
...
#line 15 "codeforces-workspace/contest/1461/e/e.cpp"
...
void main() {
  ...
  i128 k, l, r, t, x, y;
  cin >> k >> l >> r >> t >> x >> y;
  tie(l, r) = pair(k * 2 - r, k * 2 - l);
  if (l < 0) {
    k -= l, r -= l, l = 0;
  }
  if (x > y) {
    if (k - y >= l) k -= y;
    cout << (k + t * x - y * (t - 1) > r ? "No" : "Yes") << eol;
    return;
  }
  vector<bool> chkd(x);
  while (not chkd[k % x]) {
