...
int pointVsConvexPolygon(const Point<F1>& point, const Polygon<F2>& poly, int top) {
  ...
  auto orientation = ccw(Point<ll>(point), poly[top], poly[0]);
  if (orientation == 0) {
  ...
  if (orientation < 0) {
    ...
    return sgn(ccw(itr[0], Point<ll>(point), itr[-1]));
  } else {
    auto itl = upper_bound(poly.rbegin(), poly.rend() - top-1, point);
    return sgn(ccw(itl == poly.rbegin() ? poly[0] : itl[-1], Point<ll>(point), itl[0]));
  }
}
...
int main() {
  ...
  Polygon<int> A;
  read(A);
  int s = static_cast<int>(min_element(A.rbegin(), A.rend()) - A.rbegin());
  Polygon<int> T(A.rbegin() + s, A.rend());
  T.insert(end(T), A.rbegin(), A.rbegin() + s);
  A.swap(T);
  s = static_cast<int>(max_element(all(A)) - begin(A));
  bool inside = true;
  int m;
  read(m);
  REP(i, m) {
    Point<int> p;
    read(p);
    if (pointVsConvexPolygon(p, A, s) >= 0) {
      inside = false;
      break;
    }
  }
  writeln(inside ? "YES" : "NO");
