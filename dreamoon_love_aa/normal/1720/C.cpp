...
void solve() {
    ...
    REP(i,n)REP(j,m){
        one+=s[i][j]=='1';
        if(i+1<n&&s[i][j]=='0'&&s[i+1][j]=='0')good=1;
        if(j+1<m&&s[i][j]=='0'&&s[i][j+1]=='0')good=1;
        if(i+1<n&&j+1<m&&s[i][j]=='0'&&s[i+1][j+1]=='0')good=1;
        if(i+1<n&&j+1<m&&s[i][j+1]=='0'&&s[i+1][j]=='0')good=1;
    }
    if(good)W(one);
    else if(one==n*m)W(one-2);
    else W(one-1);
}
void input() {
    R(n,m);
