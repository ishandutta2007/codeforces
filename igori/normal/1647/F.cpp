...
int solve(vector<int> a)
{
    ...
    for (int i = pos - 1; i >= 0; i--)
    {
        if (a[i] < ATL) continue;
        int cur = 0;
        for (int j = increase[i] + 1; j < pos && j <= increase[i + 1] + 1; j++)
        {
            if (j != i + 1) cur = a[j - 1];
            if (a[i] > a[j] && cur < dp[j])
            {
        ...
        if (increase[i + 1] == pos) dp[i] = max(dp[i], a[i + 1]);

        if (i <= left && dp[i] != 0 && (i == 0 || (max(a[i], dp[i]) >= suf[i - 1][1] && min(a[i], dp[i]) >= suf[i - 1][2])))
            res++;
    }
