...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) cin >> a[i], mp[a[i]].pb(i);
	
	S.insert(0), S.insert(n+1), T.insert(n);
	...
	for(auto p : mp){
		...
		vector<ll> &vec = p.snd;
		for(auto x : vec){
			auto r = S.lower_bound(x), l = S.lower_bound(x); l--;
			T.erase(T.lower_bound(*r-*l-1));
			S.insert(x);
			T.insert(*r-x-1), T.insert(x-*l-1);
		}
		len = *T.rbegin();
	}
	per(i, n, 1) chmax(ans[i], ans[i+1]);
