// Busy Robot (https://codeforces.com/contest/1463/problem/C)
...
int main() {
    ...
    while (T--) {
        ...
        int target = x[0];
        rep(i, 1, n) {
            int passed = t[i] - t[i - 1];
            if (abs(pos.back() - target) <= passed) {
                pos.push_back(target);
                target = x[i];
            } else {
                int dir = target < pos.back() ? -1 : 1;
                pos.push_back(pos.back() + dir * passed);
            }
        }

        int cnt = (min(pos.back(), target) <= x[n-1] && x[n-1] <= max(pos.back(), target));
        rep(i, n - 1) {
            int mn = min(pos[i], pos[i + 1]), mx = max(pos[i], pos[i + 1]);
            cnt += mn <= x[i] && x[i] <= mx;
        }
