...
using i32 = int_least32_t; using i64 = int_least64_t; using ui32 = uint_least32_t; using ui64 = uint_least64_t;
...
void __solve()
{
    ...
    // unordered_map<pll,bool> chec;
    bool cyc[1<<15]={};
    vector<queue<pll>> ques(1<<15);
    ...
    if(dp[all])
    {
        cout << "Yes" << "\n";
        vector<pll> ans(k);
        for(int now=all; now; now^=dp2[now])
        {
            auto que=ques[dp2[now]];
            int prv=-1;
            int fi=-1;
            while(!que.empty())
            {
                i64 x,i; tie(x,i)=que.front();
                que.pop();
                if(fi<0) fi=i;
                ans[i].first=x;
                ans[i].second=prv;
