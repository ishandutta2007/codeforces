#pragma GCC optimize("Ofast")
#pragma GCC target("sse,sse2,sse3,ssse3,sse4,sse4.1,sse4.2,popcnt,mmx,avx")
#include "bits/stdc++.h"
#include <assert.h>
using namespace std;
typedef long long ll;
typedef long double ld;
#define PB push_back
#define MP make_pair
const int MOD=1000000007;
#define endl "\n"
#define fst first
#define snd second
const int UNDEF = -1;
const int INF=(1<<30)-1;
template<typename T> inline bool chkmax(T &aa, T bb) { return aa < bb ? aa = bb, true : false; }
template<typename T> inline bool chkmin(T &aa, T bb) { return aa > bb ? aa = bb, true : false; }
typedef pair<ll,ll> pll;typedef vector<ll> vll;typedef pair<int,int> pii;typedef vector<int> vi;typedef vector<vi> vvi;typedef vector<pii> vpii;
#ifdef ONLINE_JUDGE
#define assert(...) /* nothing */
#endif
#define DEBUG_CAT
#ifdef DEBUG_CAT
#define dbg(...)   printf( __VA_ARGS__ )
#else 
#define dbg(...)   /****nothing****/
#endif
int rint();char rch();long long rlong();
// mt19937 rng; rng.seed(std::chrono::high_resolution_clock::now().time_since_epoch().count());
// template<typename T> int bins(vector<T> &v, T key) {int imin=0,imax=v.size();while(imin<imax) {int imid=(imin+imax)>>1;if (v[imid]<key) imin=imid+1;else imax=imid;}return imin;}
const int mn=80;
const int mx=5;
const int mk=5;
int a[mn][mn];
vpii b[mn][mn];
int dist[mx][mx];
int g[mx];
int n,k;
int f[2][mx];
bitset<mn> valid;
int final=INF;
inline void rst(int d) {
  for (int x=0;x<mx;x++) f[d][x]=INF;
}
void go(int xlim) {
  valid.set();
  for (int x=0;x<xlim;x++) valid[g[x]]=false;
  for (int x=0;x<xlim;x++) {
    int vx=g[x];
    for (int y=0;y<xlim;y++) {
      int got=INF;
      int vy=g[y];
      for (auto &w:b[vx][vy]) {
        if (valid[w.snd]) {
          got=w.fst; break;
        }
      }
      dist[x][y]=got;
    }
  }
  rst(0);
  f[0][0]=0;
  for (int d=0;d<k;d++) {
    int cur=d&1; int nxt=cur^1;
    rst(nxt);
    for (int x=0;x<xlim;x++) {
      for (int y=0;y<xlim;y++) {
        chkmin(f[nxt][y],f[cur][x]+dist[x][y]);
      }
    }
  }
  int now=k&1;
  int ans=f[now][0];
  chkmin(final,ans);
}

void guessEven(int idx, int lim) {
  go(idx);
  if (idx<k) {
    for (int x=1;x<=lim;x++) {
      g[idx]=x;
      guessEven(idx+1,x-1);
    }
  }
}
int main() 
{
  ios_base::sync_with_stdio(false); cin.tie(0);
  n=rint(); k=rint()/2;
  for (int x=0;x<n;x++) for (int y=0;y<n;y++) a[x][y]=rint();
  for (int x=0;x<n;x++) {
    for (int y=0;y<n;y++) {
      for (int z=0;z<n;z++) {
          b[x][y].PB(MP(a[x][z]+a[z][y], z));
      }
      sort(b[x][y].begin(),b[x][y].end());
      b[x][y].resize(min((int)b[x][y].size(), 6));
    }
  }
  guessEven(1,n-1);
  printf("%d\n",final);
}




static char stdinBuffer[1024];
static char* stdinDataEnd = stdinBuffer + sizeof (stdinBuffer);
static const char* stdinPos = stdinDataEnd;

void readAhead(size_t amount)
{
    size_t remaining = stdinDataEnd - stdinPos;
    if (remaining < amount) {
       memmove(stdinBuffer, stdinPos, remaining);
       size_t sz = fread(stdinBuffer + remaining, 1, sizeof (stdinBuffer) - remaining, stdin);
       stdinPos = stdinBuffer;
       stdinDataEnd = stdinBuffer + remaining + sz;
       if (stdinDataEnd != stdinBuffer + sizeof (stdinBuffer))
         *stdinDataEnd = 0;
    }
}

int rint()
{
    readAhead(16);

    int x = 0;
    bool neg = false;
    while(*stdinPos==' '||*stdinPos=='\n') ++stdinPos;
    if (*stdinPos == '-') {
       ++stdinPos;
       neg = true;
    }

    while (*stdinPos >= '0' && *stdinPos <= '9') {
       x *= 10;
       x += *stdinPos - '0';
       ++stdinPos;
    }

    return neg ? -x : x;
}
char rch()
{
    readAhead(16);
    while(*stdinPos==' '||*stdinPos=='\n') ++stdinPos;
    char ans=*stdinPos;
    ++stdinPos;
    return ans;
}
long long rlong()
{
    readAhead(32);

    long long x = 0;
    bool neg = false;
    while(*stdinPos==' '||*stdinPos=='\n') ++stdinPos;
    if (*stdinPos == '-') {
       ++stdinPos;
       neg = true;
    }

    while (*stdinPos >= '0' && *stdinPos <= '9') {
       x *= 10;
       x += *stdinPos - '0';
       ++stdinPos;
    }

    return neg ? -x : x;
}