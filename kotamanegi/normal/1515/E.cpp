...
void solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int q = 1; q <= i; ++q) {
            int rem = i - q;
            modint hoge = comb(rem + (q-1), rem);
            settings[i] += comb(rem + (q-1), rem);
        }
    }
    modint ans = init();
    ans = settings[n];
    for (int i = 1; i < n; ++i) {
        for (int q = 1; i + q < n; ++q) {
            Go[n - i - q - 1][i + q] += settings[i] * settings[q] * gyaku[i] * gyaku[q];
        }
    }
    for (int i = n - 2; i >= 2; --i) {
        for (int q = 1; q <= n; ++q) {
            for (int j = 2; j <= i; ++j) {
                Go[i - j][q + (j - 1)] += Go[i][q] * settings[(j - 1)] * gyaku[(j - 1)];
            }
        }
    }
    for (int i = 1; i <= n; ++i) {
