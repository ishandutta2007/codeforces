...
fun main(){
    solve.cases{
        val n = getint()
        val foundnow = mutableListOf<String>()
        val fset = mutableSetOf<String>()
        val types = IntArray(4)
        val reversible = IntArray(4)
        val revstore = Array(4){ mutableListOf<Int>() }
        repeat(n){
            foundnow.add(getstr())
            fset.add(foundnow.last())
        }
        for((i,str) in foundnow.withIndex()){
            val type = ((str[0] == '1').chi ) * 2 + (str.last() == '1').chi
            types[type] ++
            if(!fset.contains(str.reversed())){
                if(type in 1..2){
                    reversible[type] ++
                    revstore[type].add(i)
                }
        ...
        fun ok(a:IntArray):Boolean {
            if(a[1] == a[2] && a[1] == 0){
                ...
                return (a[1] - a[2]).absoluteValue <= 1
            }
        }
        var done = false
        fun doit(a:Int){
            ...
            for(i in 0..reversible[a]){
                val new = types.copyOf()
                if(a == 1){
                    new[1] -= i
                    new[2] += i
                }else{
                ...
                if(ok(new)){
                    put(i)
                    put(revstore[a].take(i).map{it+1}.joinToString(" "))
                    done = true
                    return
                }
            }
