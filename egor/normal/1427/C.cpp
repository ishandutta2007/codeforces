...
class CTyazhelayaRabotaPaparatstsi {
public:
    void solve() {
        int r = in.readInt();
        int n = in.readInt();
        arri t, x, y;
        in.readArrays(n, t, x, y);
        decreaseByOne(x, y);

        arr2d<int> q(r, r, numeric_limits<int>::min() / 2);
        int from = 0;
        int to = 0;
        for (int i : range(n)) {
            if (t[i] < r) {
        ...
        vi res(n, numeric_limits<int>::min() / 2);
        int answer = 0;
        for (int i : range(t[n - 1] + 1)) {
            if (i < r) {
                q(i, 0) = 0;
            }
            if (i - t[from] >= r) {
        ...
        maxim(answer, *max_element(all(res)));
...
int main() {
    ...
    auto time = clock();
#endif
    CTyazhelayaRabotaPaparatstsi solver;
