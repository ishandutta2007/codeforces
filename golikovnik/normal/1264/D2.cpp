...
int main() {
  ...
  auto decR = [&] {
    curf -= C(lq + rq, rq + r - l - 1);
    --r;
  };
  auto decRQ = [&] {
    curf -= C(lq + rq, rq + r - l - 1);
    int R = min(rq + lq, rq + r - l - 2);
    if (R >= 0) {
      curf = (curf + C(rq + lq - 1, R)) * C.inv(2);
    }
    --rq;
  };
  auto incL = [&] {
  ...
  auto incLQ = [&] {
    int R = min(rq + lq, rq + r - l - 1);
    if (R >= 0) {
      curf = curf * 2 - C(rq + lq, R);
    }
    if (R == rq + lq && rq + r - l - 1 > rq + lq) {
      ++curf;
    }
    ++lq;
  };
