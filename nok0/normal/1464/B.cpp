...
#pragma region datastructure Range Add Range Sum BIT
#include <vector>
...
public:
	RangeAddRangeSumBIT(size_t n_) : n(n_ + 1), vec1(n_ + 2, 0), vec2(n_ + 2, 0) {}
...
void main_() {
	...
	REP(_, 2) {
		int zc = 0, oc = 0;
		ll tmp = 0;
		RangeAddRangeSumBIT<int> zcnt(n), ocnt(n), rigo(n), rigz(n);
		REP(i, n) {
			if(s[i] != '1') {
				zcnt.add(i + 1, n, 1);
				tmp += y * ocnt.sum(i, i + 1);
			} else {
				ocnt.add(i + 1, n, 1);
				tmp += x * zcnt.sum(i, i + 1);
			}
		}
		chmin(res, tmp);
		REP(i, n) {
			if(s[i] == '1') rigo.add(0, i, 1);
			if(s[i] != '1') rigz.add(0, i, 1);
		}
		debug(tmp);
		REP(i, n) {
			if(s[i] == '?') {
				//[0]1
				tmp -= x * rigo.sum(i, i + 1);
				//1[0]
				tmp -= y * ocnt.sum(i, i + 1);
				//0[1]
				tmp += x * zcnt.sum(i, i + 1);
				//[1]0
				tmp += y * rigz.sum(i, i + 1);
				zcnt.add(i + 1, n, -1);
				ocnt.add(i + 1, n, 1);
				chmin(res, tmp);
			}
