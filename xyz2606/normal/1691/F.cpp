...
int k;
int ans[N], tmp[N], siz[N], vst[N], fa[N], fac[N], invfac[N];
vector<int> e[N];
int c(int n) {
	if(n < k) return 0;
...
void dfs(int v) {
	for(int y : e[v]) {
		if(fa[y] == v) {
			...
			ans[y] = ans[v];
			int bv = tmp[v], by = tmp[y];
			ans[y] = (ans[y] - tmp[v] * (LL)siz[v] % mod + mod) % mod;
			tmp[v] = (tmp[v] - c(siz[v]) + mod) % mod;
			tmp[v] = (tmp[v] + c(siz[v] - siz[y])) % mod;
			tmp[v] = (tmp[v] + c(siz[y])) % mod;
			ans[y] = (ans[y] + tmp[v] * (LL)(siz[v] - siz[y]) % mod + mod) % mod;

			ans[y] = (ans[y] - tmp[y] * (LL)siz[y] % mod + mod) % mod;
			tmp[y] = (tmp[y] - c(siz[y]) + mod) % mod;
			tmp[y] = (tmp[y] + c(siz[v])) % mod;
			tmp[y] = (tmp[y] - c(siz[v] - siz[y])) % mod;
			ans[y] = (ans[y] + tmp[y] * (LL)siz[v] % mod + mod) % mod;
...
int main() {
	...
	invfac[n] = fastpo(fac[n], mod - 2, mod);
	for(int i = n - 1; i >= 0; i--) {
	...
	for(int i = n - 1; i >= 0; i--) {
		...
		for(int y : e[v]) {
			if(y != fa[v]) {
				tmp[v] = (tmp[v] - c(siz[y]) + mod) % mod;
			}
		}
		ans[1] = (ans[1] + (LL)tmp[v] * siz[v]) % mod;
	}
	dfs(1);
	int tot =0 ;
	for(int i = 1; i <= n; i++) {
