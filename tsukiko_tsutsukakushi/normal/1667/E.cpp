...
void solve() {
    ...
    // i == 0
    {
        mint tmp = bc.fact(n - 1);
        for(int j = (n + 1) / 2; j <= n - 1; ++ j) {
            tmp -= bc.com(n - 1, j) * bc.fact(j - 1) * bc.fact(n - 1 - j);
        }
        ans.eb(tmp);
    }
    vc<mint> d(n, 1);
    for(int j = (n + 1) / 2 + 1; j <= n - 1; ++ j) {
        d[j] = d[j - 1] - mint(1) / mint(j - 1);
    }
    // rep(i, 1, n) {
    //     mint res = 0;
    //     for(int x = (n + 1) / 2; x <= n - i; ++ x) {
    //         res += bc.fact(n - x - 1) * bc.finv(n - i - x) * d[x];
    //     }
    //     res *= mint(i) * bc.fact(n - i - 1);
    //     ans.eb(res);
    // }
    vc<mint> a(n, 0);
    for(int i = (n + 1) / 2; i <= n - 1; ++ i) {
        a[i] = bc.fact(n - i - 1) * d[i];
    }
    Rev(a);
    vc<mint> b(n + 1, 0);
    ...
    for(int i = 1; i <= n - 1; ++ i) {
        mint res = mint(i) * bc.fact(n - i - 1) * c[n - 1 + i];
        ans.eb(res);
    }
    out(ans);
}
