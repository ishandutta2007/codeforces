...
mint val(ll l,ll r,vi &s,vi &t){
    
    mint a=s[r-1]/s[l-1];
    mint b;
    if(l-2>=0)b=(t[r-2]-t[l-2])/s[l-1];
    else b=t[r-2];
    return b/a;
}
l_l find(set<ll>&se,ll p){
    assert(se.find(p)==se.end());
    l_l ret;
...
int main(){fastio
    ...
    vi s(n+1);
    s[0]=p[0];
    rep(i,1,n)s[i]=p[i]*s[i-1];
    vi t(n+1);
    t[0]=s[0];
    rep(i,1,n)t[i]=t[i-1]+s[i];
    mint ans=val(1,n+1,s,t);
   // cout<<ans.val()<<endl;
    /*
    rep(i,1,n){
        rep(j,i+1,n+1){
            cout<<i<<" "<<j<<":"<<val(i,j,s,t).val()<<endl;
        }
    }*/
    set<ll>se;se.insert(1);se.insert(n+1);
    while(q--){
        ll a;cin>>a;
        l_l F;
        if(se.find(a)==se.end()){
            F=find(se,a);
            se.insert(a);
            mint ad=val(F.fi,a,s,t)+val(a,F.se,s,t)-val(F.fi,F.se,s,t);
            ans+=ad;
        }else{
            se.erase(a);
            F=find(se,a);
