...
int main() {
    ...
    cin.tie();
        vector<vector<ll>>vs(1,vector<ll>{});
    init();
    {
        ld sum=1;
        for(auto pr:primes){
            if(sum*pr<1e18){
                sum*=pr;
                vs.back().push_back(pr);
            }else{
                vs.push_back(vector<ll>{pr});
                sum=pr;
            }
        }
    }
    ...
    while(Q--){
        ...
        for(int i=0;i<divs.size();i+=2){
            ...
            {
                ll total1=1;
                while(total1*div1<=(1e9)){
                    total1*=div1;
                }
                ll total2=1;
                while(total2*div2<=(1e9)){
                    total2*=div2;
                }
            total=total1*total2;
            }
