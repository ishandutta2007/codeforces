...
int get_t(int v, int l, int r, int at) {
	...
	return get_t(v << 1, l, (l + r) / 2, at) + get_t(v << 1 | 1, (l + r) / 2, r, at);
}

void set_t(int v, int l, int r, int at) {
	if (l > at || r <= at) {
		return;
	}
	if (r - l == 1) {
	...
	set_t(v << 1, l, (l + r) / 2, at);
	set_t(v << 1 | 1, (l + r) / 2, r, at);
	t[v] = (t[v << 1] + t[v << 1 | 1]);
}

vector<int> pos_char[26];
...
inline void solve() {
	...
	while (q--) {
		...
		string s, t;
		cin >> s >> t;
		for (int i = 0; i < 26; ++i) pos_char[i].clear();
		for (int i = sz(s) - 1; i >= 0; --i) {
			pos_char[s[i] - 'a'].push_back(i);
		}
		...
		for (int i = 0; i < n; ++i) {
			for (char ch = 'a'; ch < t[i]; ++ch) {
				if (sz(pos_char[ch - 'a'])) {
					ans = min(ans, cur + pos_char[ch - 'a'].back() - i + get_t(1, 0, n, pos_char[ch - 'a'].back()));
				}
			}
			if (!sz(pos_char[t[i] - 'a'])) {
				break;
			}
			cur += pos_char[t[i] - 'a'].back() - i + get_t(1, 0, n, pos_char[t[i] - 'a'].back());
			set_t(1, 0, n, pos_char[t[i] - 'a'].back());
			pos_char[t[i] - 'a'].pop_back();
		}
		cout << ans << endl;
	}
}
