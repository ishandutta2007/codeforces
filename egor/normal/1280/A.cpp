...
public:
    void solve(istream &inp, ostream &outp) {
        ...
        arr<char> c(x);
        int end = min(x, int(s.size()));
        copy(s.begin(), s.begin() + end, c.begin());
        modint answer = s.size();
        for (int i : Range(x)) {
            int times = c[i] - '0' - 1;
            answer += (answer - i - 1) * times;
            if (end != x) {
                int len = end - i - 1;
                for (int j : Range(times)) {
                    minim(len, x - end);
                    if (len == 0) {
                        break;
                    }
                    copy(c.begin() + i + 1, c.begin() + len + i + 1, c.begin() + end);
                    end += len;
                }
            }
        }
