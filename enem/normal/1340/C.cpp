...
static int n,m,g,r,d[10005],dis[10005][1005];
static deque<pair<int,pair<int,int>>> pr;
static pair<int,pair<int,int>> t;
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        pr.push_front(MP(0,MP(0,0)));
        while(pr.size()>0)
        {
            t=pr.front();
            pr.pop_front();
            if(dis[t.S.F][t.S.S]==-1)
            {
                dis[t.S.F][t.S.S]=t.F;
                if(t.S.F<m-1)
                {
                    if(t.S.S+(d[t.S.F+1]-d[t.S.F])<g)
                    {
                        pr.push_front(MP(t.F,MP(t.S.F+1,t.S.S+(d[t.S.F+1]-d[t.S.F]))));
                    }
                    else if(t.S.S+(d[t.S.F+1]-d[t.S.F])==g)
                    {
                        pr.push_back(MP(t.F+1,MP(t.S.F+1,0)));
                    }
                }
                if(t.S.F>0)
                {
                    if(t.S.S+(d[t.S.F]-d[t.S.F-1])<g)
                    {
                        pr.push_front(MP(t.F,MP(t.S.F-1,t.S.S+(d[t.S.F]-d[t.S.F-1]))));
                    }
                    else if(t.S.S+(d[t.S.F]-d[t.S.F-1])==g)
                    {
                        pr.push_back(MP(t.F+1,MP(t.S.F-1,0)));
                    }
                }
