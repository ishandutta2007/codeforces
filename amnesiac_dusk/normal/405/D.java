...
public class Main
{
	public static void main(String[] args)
	{
		...
		for(int i:a)
			nums[i]=true;
		vecti hola=new vecti();
		int simple=1;
		for(int i=0; i<n; i++)
		{
			if(!nums[1000001-a[i]]&&nums[a[i]])
				hola.add(1000001-a[i]);
			if(nums[1000001-a[i]]&&nums[a[i]])
			{
				while(nums[simple]||nums[1000001-simple])
					simple++;
				hola.add(simple);
				hola.add(1000001-simple);
				simple++;
				nums[1000001-a[i]]=false;
			}
		}
		pr(n);
		for(int i=0; i<n; i++)
