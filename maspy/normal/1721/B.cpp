...
void solve() {
  LL(H, W, sx, sy, D);
  --sx, --sy;
  auto ok = [&](int x, int y) -> bool { return abs(x - sx) + abs(y - sy) > D; };

  bool a = 1, b = 1;
  FOR(x, H) if (!ok(x, 0)) a = 0;
  FOR(y, W) if (!ok(H - 1, y)) a = 0;
  FOR(y, W) if (!ok(0, y)) b = 0;
  FOR(x, H) if (!ok(x, W - 1)) b = 0;
  if (a || b) return print(H + W - 2);
  print(-1);
}
