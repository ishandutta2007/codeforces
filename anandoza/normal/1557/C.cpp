...
vector<mn> powers_of_two{1};
mn twos(int n) {
    while (sz(powers_of_two) <= n)
        powers_of_two.push_back(powers_of_two.back() * 2);
...
void solve() {
    ...
    // let's count xor > and, where we do casework on the length of the same_prefix [0, k)
    for (int same_prefix = 0; same_prefix < k; same_prefix++) {
        mn count = 1;

        // xor == and if we have an even number of 1s (but not all) OR all 1s (odd number)
        count *= (twos(n - 1) - bool(n % 2 == 0) + bool(n % 2 == 1)).pow(same_prefix);

        // xor > and if we have an odd number of 1s (but not all)
        count *= twos(n - 1) - bool(n % 2 == 1);

        // anything is fine
        count *= twos(n).pow(k - 1 - same_prefix);

        ans += count;
    }

    ans = twos(n).pow(k) - ans;
