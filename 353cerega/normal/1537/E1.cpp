...
int main()
{
    ...
    {
        int sz = cur.size();
        vector<vector<pair<ll,ll>>> nxt(sz);
        for (int i=0;i<sz;i++)
        {
            nxt[i].resize(cur[i].size());
        }
        vector<ll> cnt(sz);
        for (ll i=0;i<sz;i++)
        {
        ...
        for (int i=0;i<sz;i++)
        {
            ...
            {
                if (j==0 or nxt[i][j].X!=nxt[i][j-1].X)
                {
                    cur.push_back({});
                    C++;
                }
                kek[nxt[i][j].Y] = C-1;
                cur.back().emplace_back(nxt[i][j].Y);
            }
        }
    }
    string ans;
