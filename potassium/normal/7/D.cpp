...
int main() {
	...
	vector<int> plen(m, 0);
	{
		...
		for (int i = 0; i < m; i++) {
			if (cen * 2 - i >= 0)
				plen[i] = max(0, min(plen[cen * 2 - i], cen + plen[cen] - 1 - i));
			while (0 <= i - plen[i] && i + plen[i] < m && t[i - plen[i]] == t[i + plen[i]])
				plen[i]++;
			if (i + plen[i] >= cen + plen[cen])
				cen = i;
		}
	}
	...
	W(accumulate(deg.begin(), deg.end(), 0));
