...
int main(void)
{
	...
	while(T--){
		...
		mint bunsi = 0, bunbo = 0;
		for(auto p : mp){
			ll d = p.fst, k = n/d;
			if(g % k) continue;
			
			mint tmp = fact[d];
			for(auto x : vec) tmp *= fact_inv[x/k];
			bunbo += tmp * p.snd;
	
			if(d < 2) continue;
			mint mul = fact[d-2], sum = 0;
			for(auto x : vec) mul *= fact_inv[x/k];
			for(auto x : vec){
				if(x/k < 2) continue;
				sum += mul * fact[x/k] * fact_inv[x/k-2];
			}
			sum = tmp - sum;
			bunsi += sum * p.snd;
		}
