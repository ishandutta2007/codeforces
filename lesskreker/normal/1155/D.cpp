...
int main(int argc, char** argv) {
    ...
    vector<vector<llong>> d(n, vector<llong>(5));
    d[0][1] = a[0];
    d[0][2] = 1LL * a[0] * x;
    d[0][3] = d[0][2];
    d[0][4] = d[0][3];
    ...
    for (int i = 1; i < n; i++) {
        d[i][1] = max(d[i - 1][1], 0LL) + a[i];
        d[i][2] = max(d[i - 1][2] + 1LL * a[i] * x, max(d[i - 1][1] + 1LL * a[i] * x, 1LL * a[i] * x));
        d[i][3] = max(d[i - 1][3] + a[i], d[i][2]);
        d[i][4] = max(d[i - 1][4], d[i][3]);
    }
