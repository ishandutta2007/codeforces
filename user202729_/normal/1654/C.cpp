...
/*
// guess
// if correct, TODO prove correctness
*/

/*
 * okay the guess was wrong, this one seems more correct
 */
...
void up(){
	...
	*/
	...
	std::priority_queue<int> require;
	int64_t sum {};
	for(int _=number; _--;){
	...
	while(not have.empty()){
		auto item=have.top(); have.pop();
		if(item>require.top()){
			have.push(item>>1);
			have.push((item+1)>>1);
		}else if(item<require.top()){
			std::cout<<"NO\n";
			return;
		}else{
			require.pop();
	...
	/*
	int64_t sum=std::accumulate(begin(a), end(a), int64_t(0));
	int64_t least=sum, most=sum;
	while(true){
		values.erase(least);
		if(most!=least) values.erase(most);
		if(least==0) break;
		least>>=1; most++; most>>=1;
	}
	std::cout<<(values.empty() ? "YES\n": "NO\n");
	*/
