...
int main() {
    ...
    loop(t) {
        ...
        atcoder::segtree<pair<int, int>, op_min, e_min> seg_min(init);
        atcoder::segtree<pair<int, int>, op_max, e_max> seg_max(init);

        vector<int> d(n, e_min().first);
        d[0] = 0;

        rep(i, n - 1) {
            if (p[i] < p[i + 1]) {
                int r = seg_min.max_right(i, [&](auto v) { return v.first >= p[i]; });
                int j = seg_max.prod(i, r).second;
                chmin(d[j], d[i] + 1);
            } else {
                int r = seg_max.max_right(i, [&](auto v) { return v.first <= p[i]; });
                int j = seg_min.prod(i, r).second;
                chmin(d[j], d[i] + 1);
            }
        }
        print(d[n - 1]);
