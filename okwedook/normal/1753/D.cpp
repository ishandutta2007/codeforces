...
signed main() {
    ...
    auto cmp = [&dist](pii a, pii b) {
        auto eval = [&dist](pii p) {
            return make_tuple(dist[p.f][p.s], p);
        };
    ...
    while (sz(st)) {
        ...
        dbg(f, dist[f.f][f.s]);
        for (auto [tp, d] : dir) {
            pii p = {f.f + d.f, f.s + d.s};
            // dbg(p);
            if (!check_in(p)) continue;
            if (!dir_type.count(s[p.f][p.s])) continue;
            pii add = dir[s[p.f][p.s]];
            pii next = {p.f + add.f, p.s + add.s};
            ll check = dist[f.f][f.s] + (dir_type[tp] == dir_type[s[p.f][p.s]] ? Q : P);
            if (dist[next.f][next.s] > check) {
                st.erase(next);
                dist[next.f][next.s] = check;
                st.insert(next);
            }
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            if (i + 1 < n) {
                chmin(ans, dist[i][j] + dist[i + 1][j]);
            }
            if (j + 1 < m) {
                chmin(ans, dist[i][j] + dist[i][j + 1]);
            }
        }
    }
