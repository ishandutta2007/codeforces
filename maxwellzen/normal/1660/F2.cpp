...
void solve() {
    cin >> n >> s;
    cnt.assign(n+1, 0);
    F0R(i, n) cnt[i+1] = (s[i]=='+' ? 1 : -1);
    F0R(i, n) cnt[i+1] += cnt[i];
    int mn = 1e9, mx = -1e9;
    F0R(i, n+1) mn = min(mn, cnt[i]), mx = max(mx, cnt[i]);
    F0R(i, n+1) cnt[i] -= mn;
    mx -= mn;
    Seg<ll> st[3];
    F0R(i, 3) st[i].init(mx+1);
    ll ans = 0;
    F0R(i, n+1) {
        int cur = cnt[i]%3;
        ans += st[cur].query(cnt[i], mx);
        st[cur].upd(cnt[i], st[cur].query(cnt[i], cnt[i])+1);
    }
    cout << ans << nl;
}
