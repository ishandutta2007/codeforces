...
int n, a[MAXN], s[MAXN], h[MAXN], dp[MAXN];

signed main(){
    read(n); read(a + 1, a + n + 1); memset(h, 31, sizeof(h)); memset(dp, 31, sizeof(dp));
    rep(i, 1, n) s[i] = s[i - 1] + a[i];
    dp[0] = 0; h[0] = 0; rep(i, 1, n) REP(j, 0, i) {
        if(s[i] - s[j] >= h[j] && dp[i] >= dp[j] + i - j - 1) {
            dp[i] = dp[j] + i - j - 1;
            h[i] = s[i] - s[j];
        }
    } print(dp[n]);
