...
void solve() {
    ...
    REP(i, m) {
        int a, b;
        cin >> a >> b;
        if (inputs[a - 1] - inputs[b - 1] < 0) {
            swap(a, b);
        }
        pairs.push_back(make_tuple(inputs[a - 1] - inputs[b - 1], a, b));
    }
    sort(ALL(pairs));
    int bad[4000] = {};
    int t;
    ...
    REP(i, n) {
        ...
        int Need = -1;
        REP(i, pairs.size()) {
            if (get<1>(pairs[i]) == Back) {
                if (bad[get<2>(pairs[i])] == 0) {
                    Need = get<2>(pairs[i]);
                    break;
                }
            }else if(get<2>(pairs[i]) == Back) {
                if (bad[get<1>(pairs[i])] == 0) {
                    Need = get<1>(pairs[i]);
                    break;
                }
            }
