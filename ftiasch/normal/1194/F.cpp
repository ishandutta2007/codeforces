...
int main() {
  ...
  std::vector<ModT> power_of_inv_2(n + 1);
  power_of_inv_2[0] = ModT{1};
  for (int i = 1; i <= n; ++i) {
    power_of_inv_2[i] = power_of_inv_2[i - 1] * ModT{2}.inverse();
  }
  long long sum_a = 0;
  ModT result{0}, prob{1};
  for (int i = 1, k = 0, a; i <= n; ++i) {
    scanf("%d", &a);
    sum_a += a;
    k++;
    if (k < i) {
      prob += binom(i - 1, k) * power_of_inv_2[i];
    }
    if (sum_a <= m) {
      int maxk = std::min(m - sum_a, static_cast<long long>(i));
      while (k > maxk) {
        prob -= binom(i, k) * power_of_inv_2[i];
        k--;
      }
      result += prob;
      //   for (int k = 0; k <= maxk; ++k) {
      //     result += binom(i, k) * power_of_inv_2[i];
      //   }
    }
  }
  printf("%d\n", result.get());
