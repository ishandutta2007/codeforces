...
ll ins(ll x, ll y) {
    ...
    y++;
    if (y < x) return 0;
    ll i = min(n, s.lower_bound(y))-1;
    debug(i);
    if (i >= x) {
        debug(x, i, s.sum(x, i), y);
        for (ll j = 0; j < n; j++)
            debug(s.get(j));
        ll ans = (i-x+1)*y - s.sum(x, i);
        s.set(x, i, y);
        return ans;
    }
    return 0;
...
int main() {
    ...
    for (ll i = N-1; i > 0; i--) {
        ...
        for (ll j = 0; j < n; j++)
            debug(s.get(j));
        ans += i*ins(0, p[i][p[i].size()-2]-1);
        ans += i*ins(p[i][1]+1, n-1);
        ans += i*ins(p[i][0]+1, p[i][p[i].size()-1]-1);
        debug(ans);
    }
    cout << ans << "\n";
}