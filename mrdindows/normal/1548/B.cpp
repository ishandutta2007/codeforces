...
T fast_gcd(T u, T v) {
    ...
    int shl = 0;
    {
        int u_bsf = __builtin_ctzll(u);
        u >>= u_bsf;
        int v_bsf = __builtin_ctzll(v);
        v >>= v_bsf;
        shl = std::min(u_bsf, v_bsf);
    }
    while (u && v && u != v) {
        T d = u & v & 1 ? (u + v) >> 1 : 0;
...
unsigned long long yosupo_gcd(unsigned long long a, unsigned long long b) {
    if (a == 0) return b;
    if (b == 0) return a;
    int shift;
    {
        int a_bsf = __builtin_ctzll(a);
        a >>= a_bsf;
        int b_bsf = __builtin_ctzll(b);
        b >>= b_bsf;
...
void build(int cur, int l, int r) {
    if (l == r) {
        ...
        build(dcur, l, m);
        build(dcur + 1, m + 1, r);
        tr[cur] = fast_gcd(tr[dcur], tr[dcur + 1]);
    }
}
pair<ll, int> getMostRight(int cur, int l, int r, int x, ll g) {
    if (r < x) return {g, r + 1};
    if (l == r) {
        ll newg = fast_gcd(g, tr[cur]);
        if (newg == 1) {
            return {g, l};
        } else {
            return {newg, l + 1};
        ...
        int m = (l + r) / 2;
        int dcur = cur + cur;
        auto res = getMostRight(dcur, l, m, x, g);
        if (res.second == m + 1) {
            res = getMostRight(dcur + 1, m + 1, r, x, res.first);
        }
        return res;
    }
}
int solve() {
    ...
    while (tc--) {
        ...
        for (int i = 0; i < n; ++i) {
            if (a[i] != 1) {
                ans = max(ans, getMostRight(1, 0, n - 1, i, 0).second - i);
            }
        }
        writeInt(ans + 1);
    }
