...
fun main(){
    ...
    solve.cases{
        val n = getint()
         val L = getline(n)
        val all = Array(201){IntArray(n)}
        val occu = List(max){ mutableListOf<Int>() }
        ...
        var ret = 0
        for(i in 0 until max){
            ret = maxOf(ret,all[i].last())
        }

        for(i in 0 until max){
            val v = occu[i]
            for(i in v.indices){
                val other = v.lastIndex - i
                if(i >= other){
                    break
                }
                val a1 = v[i]
                ...
                for(j in 0 until max){
                    bestpossible = maxOf(bestpossible,all[j][a2-1] - all[j][a1])
                }
                ret = maxOf(ret, bestpossible + bonus )
            }
        }
        put(ret)
    }
