...
#define rc (x<<1|1)
const int N=1e5+100;
int n,a[N],p[N];int c[N];
signed main(){
	read(n);
	for(int i=1;i<=n;i++)read(a[i]),p[i]=i;
	while(1.*clock()/CLOCKS_PER_SEC<.4){
		int g1=0,g2=0;
		shuffle(p+1,p+1+n,rnd);
		for(int i=1;i<=n;i++)
			if(!g1||a[p[i]]%g1)g1=__gcd(g1,a[p[i]]),c[p[i]]=0;
			else g2=__gcd(g2,a[p[i]]),c[p[i]]=1;
		if(g1==1&&g2==1){
			puts("YES");
			for(int i=1;i<=n;i++)pc("12"[c[i]]),pc(' ');
			return 0;
		}
	}
	puts("NO");
