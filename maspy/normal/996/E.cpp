...
void solve() {
  ...
  */
  FastSet ss(N);
  FOR(i, N) ss.insert(i);
  ll LIM1 = 1'000'000'000'000;
  ll LIM2 = 2'000'000'000'000;
  ...
  auto dfs = [&](auto& dfs, ll n) -> void {
    if (n <= 3) {
      ...
      int s = [&]() -> int {
        FOR(s, 1 << n) {
          ...
          if (vx * vx + vy * vy <= LIM2) return s;
        }
        return -1;
      }();
      assert(s != -1);
      FOR(i, n) { ANS[I[i]] = (s & 1 << i ? 1 : -1); }
      return;
    }
    assert(n >= 4);
    auto a = ss.next(0);
    ...
    FOR(p, 4) FOR(q, p) {
      int i = I[p], j = I[q];
      // i + j 
      ll vx = XY[i].fi + XY[j].fi;
      ll vy = XY[i].se + XY[j].se;
      if (vx * vx + vy * vy > LIM1) continue;
      XY[i].fi += XY[j].fi;
      XY[i].se += XY[j].se;
      ss.erase(j);
      dfs(dfs, n - 1);
