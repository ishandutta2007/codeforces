...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (i > 0 && which_tower[i] == which_tower[i - 1])
            answer--;
    }
    ...
    for (int e = 0; e < M - 1; e++) {
        ...
        cin >> a >> b;
        a--; b--;
        lca.add_edge(vertex, actual[a]);
        lca.add_edge(vertex, actual[b]);
        actual[a] = actual[b] = vertex;
        vertex++;
    }
    ...
    for (int i = 0; i < N - 1; i++)
        if (which_tower[i] != which_tower[i + 1])
            decrement[lca.get_lca(which_tower[i], which_tower[i + 1]) - M]++;
