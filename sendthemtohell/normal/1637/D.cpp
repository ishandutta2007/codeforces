...
inline void _solve() {
	...
	while (t--) {
		...
		dp[0].set(OFF);//= true;
		for (int i = 0; i < n; ++i) {
			dp[(i + 1) & 1] |= dp[i & 1] >> v[i].x << v[i].y;
			dp[(i + 1) & 1] |= dp[i & 1] >> v[i].y << v[i].x;
			dp[i & 1].reset();
		}

		ll sm = accumulate(all(a), 0LL);
		ll mx = LLINF;
		for (int sum = -100 * n; sum <= 100 * n; ++sum) {
			if (dp[n & 1].test(OFF + sum)) { // a + b = sm, a - b = sum
				ll lft = (sm + sum) / 2;
				ll rgt = sm - lft;
				//cerr << "! " << lft << " " << rgt << endl;
				mx = min(mx, lft * lft + rgt * rgt);
			}
		}
		cout << mx + ans << endl;
	}
