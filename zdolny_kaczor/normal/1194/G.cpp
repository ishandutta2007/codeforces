...
int val(int x, int y, int xallowed, int yallowed) {
	assert(x <= y);
	// debug(imie(x), imie(y), imask(xallowed), imask(yallowed));
	memset(dp, 0, sizeof(dp));
	dp[len][0][0][0] = dp[len][0][0][1] = 1;
	for (int i = len - 1; i >= 0; --i)
		for (int sx = 0; sx < 10; ++sx)
			for (int sy = 0; sy < 10; ++sy) {
				if (dp[i + 1][sx][sy][0] == 0 && dp[i + 1][sx][sy][1] == 0) continue;
				// if (dp[i + 1][sx][sy][0])
					// debug(imie(dp)[imie(i + 1)][imie(sx)][imie(sy)][0]);
				// if (dp[i + 1][sx][sy][1])
					// debug(imie(dp)[imie(i + 1)][imie(sx)][imie(sy)][1]);
				for (int d = 0; d < 10; ++d) {
					int hx = sx + d * x, hy = sy + d * y;
					assert(hx < 100 && hy < 100);
					if ((xallowed >> (hx % 10) & 1) && (yallowed >> (hy % 10) & 1)) {
						// if (dp[i + 1][sx][sy][0])
							// debug(imie(d), imie(dp)[imie(i)][imie(hx) / 10][imie(hy) / 10][0], "+= ", imie(dp)[imie(i + 1)][imie(sx)][imie(sy)][0]);
						add(dp[i][hx / 10][hy / 10][0], dp[i + 1][sx][sy][0]);
						if (hy % 10 < n[i] - '0') {
							// if (dp[i + 1][sx][sy][0])
								// debug(imie(d), imie(dp)[imie(i)][imie(hx) / 10][imie(hy) / 10][1], "+= ", imie(dp)[imie(i) + 1][imie(sx)][imie(sy)][0]);
							add(dp[i][hx / 10][hy / 10][1], dp[i + 1][sx][sy][0]);
						}
						else if (hy % 10 == n[i] - '0') {
							// if (dp[i + 1][sx][sy][1])
								// debug(imie(d), imie(dp)[imie(i)][imie(hx) / 10][imie(hy) / 10][1], imie(dp)[imie(i) + 1][imie(sx)][imie(sy)][1]);
							add(dp[i][hx / 10][hy / 10][1], dp[i + 1][sx][sy][1]);
						}
					}
				}
			}
	// for (int sx = 0; sx < 10; ++sx) for (int sy = 0; sy < 10; ++sy) {
		// if (dp[0][sx][sy][0])
			// debug(imie(dp)[imie(0)][imie(sx)][imie(sy)][0]);
		// if (dp[0][sx][sy][1])
			// debug(imie(dp)[imie(0)][imie(sx)][imie(sy)][1]);
	// }
	int ans = dp[0][0][0][1] - 1;
	loc(
		if (len <= 8) {
			int nval = atoi(n);
			int brans = 0;
			for (int a = 1; a * y <= nval; ++a)
				if (match(a * x, xallowed) && match(a * y, yallowed))
					brans++;
			if (ans != brans) {
				debug(imie(x), imie(y), imask(xallowed), imask(yallowed), imie(ans), imie(brans));
				assert(false);
			}
		}
	)
...
int main() {
	...
	len = strlen(n);
	int mn = 0;
	for (int i = 0; i < len; ++i) mn = (mn * 10ll + n[i] - '0') % mod;
	ll ans = mn;
	for (int x = 1; x <= 9; ++x) for (int y = x + 1; y <= 9; ++y) {
		if (gcd(x, y) > 1) continue;
		int dig = 9 / y;
		ll curr = 0;
		unordered_map <pii, ll> coef;
		int all = (1 << 10) - 1;
		for (int mask = 1; mask < (1 << dig); ++mask) {
			...
			for (int subx = mustx;; subx = (subx - 1) & mustx) {
				for (int suby = musty;; suby = (suby - 1) & musty) {
					int allowedx = all ^ subx, allowedy = all ^ suby;
					int mul = (popc(mask) + popc(subx) + popc(suby)) % 2 ? 1 : -1;
					coef[pii(allowedx, allowedy)] += mul;
					if (!suby) break;
				}
				if (!subx) break;
			}
		...
		for (auto &c : coef) {
			if (c.second) {
				debug(imask(c.first.first ^ all), imask(c.first.second ^ all), imie(c.second));
				curr += c.second * val(x, y, c.first.first, c.first.second);
				runs++;
			}
		}
		debug(imie(x), imie(y), imie(curr), imie(runs));
		ans += 2 * curr;
	}
	ans %= mod;
	if (ans < 0) ans += mod;
