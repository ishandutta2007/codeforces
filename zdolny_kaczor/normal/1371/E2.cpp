...
void solve() {
	...
	REP(i, n) {
		...
		int top = 1 + i;
		maxi(low, -off + 1);
		if (top % p == 0) mini(high, top - off - 1);
		int rem = fix(-off, p);
		debug(imie(-off), imie(top - off), imie(rem));
		blocks.emplace_back(-off, 1, rem);
		blocks.emplace_back(top - off, -1, rem);
	}
	debug(imie(low), imie(high));
	sort(ALL(blocks));
	debug(imie(blocks));
	vi blocked(p);
	set <int> valid;
	REP(i, p) valid.insert(i);
	int pos = 0;
	auto push = [&]() {
		assert(pos < (int) blocks.size());
		auto [_, val, rem] = blocks[pos];
		pos++;
		blocked[rem] += val;
		assert(blocked[rem] >= 0);
		if (blocked[rem]) valid.erase(rem);
		else valid.insert(rem);
	};
	int siz = blocks.size();
	REP(i, siz + 1) {
		int l = (i == 0 ? low : get<0>(blocks[i - 1]));
		int r = i == siz ? high - 1 : get<0>(blocks[i]);
		maxi(l, low);
		mini(r, high + 1);
		debug(imie(l), imie(r), imie(valid));
		if (!valid.empty()) {
			while (l < r) {
				int re = l % p;
				if (blocked[re] == 0) good.push_back(l);
				auto it = valid.upper_bound(re);
				if (it == valid.end()) it = valid.begin();
				int target = *it;
				int inc = target - re;
				debug(imie(l));
				if (inc <= 0) inc += p;
				debug(imie(re), imie(target), imie(inc));
				l += inc;
				debug("jump to", imie(l));
			}
		}
		if (i != siz) push();
	}
	printf("%d\n", (int) good.size());
	for (int c : good) printf("%d ", c);
	printf("\n");
}
int main() {
