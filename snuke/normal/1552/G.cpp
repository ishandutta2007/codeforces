...
#define dame { puts("REJECTED"); return;}
...
struct Solver {
  ...
  bool dfs(vi a, int i=0) {
    if (i == k) {
      int one = 0;
      rep(j,n) {
        one |= a[j];
        if (one && !a[j]) return false;
      }
      return true;
    }
    vi& p = ps[i];
    ...
    int mx = cnt+cx;
    rep(j,sz(p)) a[p[j]] = j < cnt ? 0 : 1;
    for (int c = cnt;; ++c) {
      if (!dfs(a,i+1)) return false;
      if (c == mx) break;
      a[p[c]] = 0;
    }
    return true;
  ...
  void solve() {
    ...
    //     o += ns%cnt;
    //     ns /= cnt;
    //     rep(j,sz(p)) a[p[j]] = j < o ? 0 : 1;
    //   }
    //   if (!s) {rep(i,n) if (a[i] == -1) dame;}
    //   int one = 0;
    //   rep(i,n) {
    //     one |= a[i];
    //     if (one && !a[i]) dame;
