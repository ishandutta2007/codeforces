...
bool edgesWork(vpi eds){
	...
	{ //add to edges, update has_to, has_from
		for(auto u: eds){
			addEdge(u.f, u.s);
	...
	vector<pair<int, pair<bool, bool>>> comps; //(single element, (can merge left, can merge right))
	for(int i = 1; i <= n; i++){
		if(dsu.get(i) == i){
			//component i
			int L = -1, R = -1;
			...
			bool can_merge_left = true;
			bool can_merge_right = true;
			...
			comps.pb(mp(L, mp(can_merge_left, can_merge_right)));
	...
	dbg(comps);
	int merges_allowed = sz(comps)-(n+1-k);

	if(merges_allowed < 0) return false;
...
void solve(){
	...
	vi cur_perm(n+1, -1);
	vb val_assigned(n+1, false);
	for(int i = 1; i <= n; i++){
		bool worked = false;
		dbg(cur_perm);
		for(int j = 1; j <= n; j++){
			if(val_assigned[j]) continue;
			cur_perm[i] = j;
			vpi edges;
			for(int z = 1; z+1 <= i; z++){
