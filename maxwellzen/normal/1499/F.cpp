...
void dfs(int i, int p) {
    ...
    for (int nb : adj[i]) if (nb!=p) {
        dfs(nb, i);
        mi sum = 0;
        F0R(j, sz[nb]) sum += dp[nb][j];
        vm nxt(max(sz[i], sz[nb]+1), 0);
        F0R(j, sz[i]) nxt[j] = dp[i][j]*sum;
        F0R(a, sz[nb]) F0R(b, sz[i]) if (a+1+b<=k) nxt[max(b,a+1)] += dp[nb][a]*dp[i][b];
        dp[i] = nxt;
        ckmax(sz[i], sz[nb]+1);
        if (sz[i]>k+1) {
            sz[i] = k+1;
