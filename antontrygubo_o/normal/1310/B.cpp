...
{
    vector<vector<ll>> ans(2, vector<ll>(2, -1e9));
    if (l+1==r)
    {
        for (int i = 0; i<2; i++)
            for (int j = 0; j<2; j++)
            {
                if (cnt(l, r)<i+j) ans[i][j] = -1e9;
                else ans[i][j] = min(1, cnt(l, r));
            }
        return ans;
    }
    int mid = (l+r)/2;
        ...
        for (int ld = 0; ld<2; ld++)
            for (int ru = 0; ru<2; ru++)
                for (int rd = 0; rd<2; rd++)
                {
                    if (lu == 0 && ru == 0)
                    {
                        ans[0][max(ld, rd)] = max(ans[0][max(ld, rd)], left[lu][ld] + right[ru][rd] + 2*max(ld, rd));
                    }
                    else if (lu == 1 && ru == 1)
                    {
                        ans[1][1] = max(ans[1][1], left[lu][ld] + right[ru][rd] + max(ld, rd) + 2);
                    }
                    else
                    {
                        ans[0][1] = max(ans[0][1], left[lu][ld] + right[ru][rd] + max(ld, rd) + 2);
                        ans[1][max(ld, rd)] = max(ans[1][max(ld, rd)], left[lu][ld] + right[ru][rd] + 1 + 2*max(ld, rd));
                    }
                }
    return ans;
...
int main() {
    ...
    ll maxx = 0;
    for (int i = 0; i<2; i++)
        for (int j = 0; j<2; j++) maxx = max(maxx, it[i][j] + (i|j));
