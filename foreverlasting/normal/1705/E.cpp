...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		n=read(),q=read();
		set<int> s0,s1,s_1;
		s0.insert(-1),s1.insert(-1),s_1.insert(-1);
		for(int i=0;i<N;i++)s0.insert(i);
		function<void(int,int)> add=[&](int x,int t){
			s0.erase(x),s1.erase(x),s_1.erase(x);
			c[x]+=t;
			if(c[x]==-2)c[x]=0,add(x+1,-1);
			else if(c[x]==2)c[x]=0,add(x+1,1);
			if(!c[x])s0.insert(x);
			else if(c[x]==1)s1.insert(x);
			else s_1.insert(x);
		};
		for(int i=1;i<=n;i++)a[i]=read(),add(a[i],1);
		while(q--){
			int k=read(),t=read();
			add(a[k],-1),add(a[k]=t,1);
			int p=*s1.rbegin();
			int x=*next(s1.rbegin());
			int y=*prev(s0.lower_bound(p));
			if(p==x+1)printf("%d\n",p);
			else if(p==y+1){
				int z=*prev(s_1.lower_bound(y));
				if(z<=x)printf("%d\n",p);
				else printf("%d\n",y);
			}
			else {
