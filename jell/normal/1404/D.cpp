#line 1 "codeforces-workspace\\contest\\1404\\d\\d.cpp"
#include <bits/extc++.h>
...
#line 13 "codeforces-workspace\\contest\\1404\\d\\d.cpp"
...
struct workspace::solver {
  solver() {
    ...
    if (n & 1) {
      cout << "Second" << endl;
      vector<vector<int>> pir(n);
      vector<int> a(n * 2), b(n * 2);
      cin >> a;
      for (int i = 0; i < n * 2; i++) {
        pir[--a[i]].emplace_back(i);
      }
      for (auto v : pir) {
        assert(size(v) == 2);
        int x = v[0], y = v[1];
      ...
      vector<bool> take(n * 2);
      for (int i = 0; i < n; i++) {
        if (take[i] || take[i + n]) continue;
        int now = i;
        do {
          take[now] = 1;
      ...
      if (accumulate(begin(a), end(a), 0ll) % (n * 2) == 0) {
        for (int i = 0; i < n; i++) {
          a[i] ^= pir[i][1] ^ pir[i][0];
        }
      }
      ...
      cout << "First" << endl;
      vector<int> ans(n * 2);
      for (int i = 0; i < n * 2; i++) ans[i] = i % n + 1;
      cout << ans << endl;
    }
  }
};