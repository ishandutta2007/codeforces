...
int main(void)
{
	...
	rep(i, 1, 20){
		rep(j, 1, n) p[i][j] = P(0, j);
		rep(j, 1, n) p[i][m[j]].first += min(k[j], i);
		sort(p[i]+1, p[i]+n+1), reverse(p[i]+1, p[i]+n+1);
	}
	
	P ans = P(0, 1); ll sum = 0;
	rep(i, 1, min(n, 20LL)){
		//outa(p[i], 1, n);
		sum = 0;
		rep(j, 1, i) sum += p[i][j].first;
		//outl(i, sum);
		if(ans.first*i < ans.second*sum) ans = P(sum, i);
	}
	...
	vector<ll> avec; ll ans_i = ans.second;
	rep(i, 1, ans_i) avec.push_back(p[min(20LL, ans_i)][i].second);
