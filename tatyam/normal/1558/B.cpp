...
Modint Mcomb(ll n, ll r){ return comb(n + r - 1, n - 1); } // rn
Modint catalan(ll n){ reserve(n * 2 + 1); return fac[n * 2] * inv[n] * inv[n + 1]; }
int main(){
    LL(n,m);
    mod=m;
    vec(Modint,dp_sum,n+1);
    auto get=[&](ll l, ll r){
        if(r>n)return dp_sum[l];
        return dp_sum[l]-dp_sum[r];
    };
    dp_sum[n]=1;
    Modint s=1;
    rrep(i,1,n){
        Modint dp=s;
        rep(k,2,n/i+1)dp+=get(i*k,(i+1)*k);
        if(i==1)return out(dp);
        s+=dp;
