...
const int N=1e6+5,C=5;
int n,m,cnt[N],f[N][C][C],a[N],ans;
int main(){
	int i,j,k;
	scan(n,m);
	read(a+1,a+n+1);
	...
	memset(f,-1,sizeof(f));
	f[1][0][0]=0;
	for(i=1;i<=m;++i)for(j=0;j<C;++j)for(k=0;k<C;++k)if(f[i][j][k]!=-1){
		if(cnt[i]<j)continue;
		int c=cnt[i]-j;
		for(int r=0;r<=2&&r<=c;++r){
			chkmax(f[i+1][k+r][r],f[i][j][k]+(c-r)/3+r);
		}
	}
	cout<<f[m+1][0][0]<<endl;
}