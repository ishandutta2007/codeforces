...
int main(){
    ...
    vmint pow2{1};
    rep(i,T)pow2.pb(pow2.back()*2);
    vmint G(T);
    rep(i,T){
        if(f0[i].a==0)continue;
        else if(f0[i].a==1)G[i]=f2[i];
        else
        G[i] = f2[i]*pow2[f0[i].a-1] + (f1[i]*f1[i]-f2[i])*pow2[f0[i].a-2];
    }
    rep(j,6){
        rep(i,T){
            if(i/d[j]%10 != 9){
