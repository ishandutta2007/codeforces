...
void solve() {
  ...
  SegTree<Monoid_Min_Idx<int>> seg_mi(N);
  SegTree<Monoid_Max_Idx<int>> seg_ma(N);
  FOR(i, N) seg_mi.set(i, {hld.LID[i], i});
  FOR(i, N) seg_ma.set(i, {hld.LID[i], i});

  FOR(Q) {
    LL(l, r);
    --l;
    int v1 = seg_mi.prod(l, r).se;
    int v2 = seg_ma.prod(l, r).se;
    int x1 = Mono::op(seg.prod(l, v1), seg.prod(v1 + 1, r));
    int x2 = Mono::op(seg.prod(l, v2), seg.prod(v2 + 1, r));
    int d1 = hld.depth[x1];
    int d2 = hld.depth[x2];
    if (d1 >= d2) {
      print(v1 + 1, d1);
