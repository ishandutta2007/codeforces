...
#define clz(x) __builtin_clz(x)
    ...
    SegmentTree(const vector<T> &a, const F f, const T &unit) : f(f), sz((int)a.size() > 1 ? 1 << (32 - clz(a.size() - 1)) : 1), unit(unit) {
        int n0 = a.size();
...
template <typename T> struct RAQ : SegmentTree<T> {
    ...
    RAQ(const vector<T> &a) : SegmentTree<T>(a, plus<T>(), 0) {}
};

#include <ext/pb_ds/assoc_container.hpp>
#include <ext/pb_ds/tree_policy.hpp>
using namespace __gnu_pbds;
using T = tree<int, null_type, less<int>, rb_tree_tag, tree_order_statistics_node_update>;
int main() {
    INT(n);
    vi cnt(n + 1);
    rep(_, 2) {
