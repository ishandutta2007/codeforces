...
int main() {
    TEST {
        ...
        cm.build();
        int mi = 16;
        rep2(l, 1, m - 1) rep2(r, l + 2, m - 1) {
            rep(i, n) {
                int s = (r - l) - cm.query(i, l, i + 1, r);
                rep2(j, i + 1, n - 2) {
                    s += cm.query(j, l, j + 1, r);
                    s += 2 - cm.query(j, l - 1, j + 1, l) - cm.query(j, r, j + 1, r + 1);
                    if(j - i >= 3) { chmin(mi, s + r - l - cm.query(j + 1, l, j + 2, r)); }
                    if(s >= mi) break;
                }
            }
        }
