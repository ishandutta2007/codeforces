...
fun main(){
    solve.cases{
        ...
        val FACT = LongArray(15)
        FACT[0] =1L
        for(i in 1 until FACT.size) {
            FACT[i] = i * FACT[i-1]
        }
        val rightcut = minOf(14,n)
        val leftcut = n - rightcut
        fun kpermutation(a:Long):MutableList<Int> {
            val all = (0 until rightcut).toMutableList()
            val ret = mint
            var indexnow = a
            for(l in rightcut - 1  downTo 0){
...
//        for(i in 0..100){
//            i dei kpermutation(i.toLong())
//        }
        val base = IntArray(leftcut){it}
        val Q = base.torsq()
        repeat(q){
            val t = getint
            if(t == 1){
                ...
                val p = kpermutation(index)
//                index dei p
                var leftside = Q.sumQuery(l,minOf(r,base.lastIndex))
                val shift = base.size
                for(v in maxOf(l,shift)..r){
                    leftside += p[v - shift] + shift
                }
                leftside += r - l + 1
                put(leftside)
            }else if( t== 2){
