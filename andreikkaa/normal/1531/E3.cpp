...
bool rec(int l, int r, int &ptr) {
    if(r - l == 1) {
        ...
        if(i < m) {
            memcpy(buf + idx, arr + i, (m - i) * sizeof(int));
            idx += m - i;
        } else {
            memcpy(buf + idx, arr + j, (r - j) * sizeof(int));
            idx += r - j;
        }
        memcpy(arr + l, buf, idx * sizeof(int));
        return true;
    }
}
