...
int main(int argc, char **argv){
  ...
  forn(i,n){
    ...
    cin>>a;
    LL t=a*r1+r3;
    A[i]=mp(t, min(t,min(a*r1+2*r1, r2+r1)));
  }
  dbg(A);
  array<LL,3> D={LINF,LINF,LINF};
  D[0]=0;
  forn(i,n-1){
    auto v=A[i];
    array<LL,3> DD={LINF,LINF,LINF};
    smin(DD[0],D[0]+v.X+d);
    smin(DD[1],D[0]+v.Y+d*2);
    smin(DD[2],D[0]+v.Y+d*3);
    smin(DD[1],D[1]+v.Y+d*2);
    smin(DD[0],D[2]+v.Y+d);
    smin(DD[1],D[2]+v.Y+d*2);
    smin(DD[2],D[2]+v.Y+d*3);
    D=DD;
    dbg(v,D);
  }
  pll v=A[n-1];
  LL r=min(min(D[0]+v.X,D[1]+v.X),D[2]+v.Y);
  cout<<r;
