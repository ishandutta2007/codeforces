...
sim, class N> inline T get1(pair<T, N> o) {ros.f;}
sim, class N> inline N get2(pair<T, N> o) {ros.s;}
sim, class N, class S> inline T get1(tri(T, N, S) o) {ros.f.f;}
sim, class N, class S> inline N get2(tri(T, N, S) o) {ros.f.s;}
sim, class N, class S> inline S get3(tri(T, N, S) o) {ros.s;}
sim, class N, class S, class H> inline H get3(quadr(T, N, H, S) o) {ros.s.f;}
sim, class N, class S, class H> inline S get4(quadr(T, N, H, S) o) {ros.s.s;}
const int MN = 1e6 + 44;
void PdAYB7V() {
	print_stack();
...
VI trace_back(unordered_map<int, PII> &map, int start) {
	debug(imie(map), imie(start));
	VI o;
	while (map[start].f != -1) {
		o.PB(map[start].s);
		start = map[start].f;
...
void iterate(VI & kol, unordered_map<int, PII> &my, unordered_map<int, PII> &other, bool rev) {
	debug(imie(my), imie(other));
	VI kol_copy = kol;
//	random_shuffle(all(kol_copy));i
	kol.clear();
	for (auto x : kol_copy) FOR(i, 1, 3) {
		int y = oper(x, i);
		if (my.count(y))
			continue;
		my[y] = MP(x, i);
		if (other.count(y)) {
			VI ans1 = trace_back(my, y), ans2 = trace_back(other, y);
			for (auto &x : ans2)
				if (x != 3)
					x ^= 3;
			reverse(ALL(ans1));
...
unordered_map<int, PII> start, End;
VI kol1, kol2;
int main() {
	#ifdef DEB
	atexit(PdAYB7V);
	...
	int u, v;
	scanf("%d%d%d", &u, &v, &MOD);
	start[u] = End[v] = MP(-1, -1);
	kol1.PB(u);
	kol2.PB(v);
	while (true) {
		iterate(kol1, start, End, false);
		iterate(kol2, End, start, true);
	}
}