...
int layerof(int u, int v) { return max(u, v); }
int manh(array<int, 2> a, array<int, 2> b) { return abs(a[0] - b[0]) + abs(a[1] - b[1]); }
array<int, 2> aboveof(array<int, 2> pos) { return {pos[0] + 1, pos[1]}; }
array<int, 2> rightof(array<int, 2> pos) { return {pos[0], pos[1] + 1}; }
using Opt = array<array<int64_t, 2>, 2>;
constexpr int64_t inf = INT64_MAX / 3;

struct Monoid {
    Opt opt = {{{0, inf}, {inf, 0}}};

    void merge(const Monoid& L, const Monoid& R) {
        for (int x = 0; x < 2; x++)
            for (int y = 0; y < 2; y++)
                opt[x][y] = min(L.opt[x][0] + R.opt[0][y], L.opt[x][1] + R.opt[1][y]);
    }
};
...
int main() {
    ...
    int N;
    cin >> N;
    vector<array<int, 2>> top(N + 1), bot(N + 1);
    for (int i = 1; i < N; i++) {
        cin >> top[i][0] >> top[i][1];
        cin >> bot[i][0] >> bot[i][1];
        assert(top[i][0] == i && bot[i][1] == i);
    }

    vector<Monoid> init(N - 1);
    for (int i = 1; i < N - 1; i++) {
        init[i].opt[0][0] = 1 + manh(aboveof(top[i]), top[i + 1]);
        init[i].opt[0][1] = 1 + manh(aboveof(top[i]), bot[i + 1]);
        init[i].opt[1][0] = 1 + manh(rightof(bot[i]), top[i + 1]);
        init[i].opt[1][1] = 1 + manh(rightof(bot[i]), bot[i + 1]);
    }
    ...
    while (Q--) {
        ...
        l2--;
        Opt x = seg.query_range(l1, l2).x.opt;
        int64_t ans = INT64_MAX;
        // clang-format off
        setmin(ans, manh({x1, y1}, top[l1]) + x[0][0] + 1 + manh(aboveof(top[l2]), {x2, y2}));
        setmin(ans, manh({x1, y1}, top[l1]) + x[0][1] + 1 + manh(rightof(bot[l2]), {x2, y2}));
        setmin(ans, manh({x1, y1}, bot[l1]) + x[1][0] + 1 + manh(aboveof(top[l2]), {x2, y2}));
        setmin(ans, manh({x1, y1}, bot[l1]) + x[1][1] + 1 + manh(rightof(bot[l2]), {x2, y2}));
        // clang-format on
        cout << ans << '\n';
    }
