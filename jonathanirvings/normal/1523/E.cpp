...
struct ModularInt {
  ...
  ModularInt operator^(int x) const {
    if (x == 0) {
      return 1;
    }
    ModularInt tmp = (*this) ^ (x >> 1);
    ModularInt tmp2 = tmp * tmp;
    if (x & 1) {
      return tmp2 * num;
    }
    return tmp2;
...
{
  fact[0] = 1;
  FORN(i,1,200000) fact[i] = fact[i-1] * i;
  ...
  TC(T)
  {
    ...
    FORN(i,1,n)
    {
      int sum = (n - i) - (i - 1) * (k - 1);
      if (sum < 0) break;
      res += fact[sum + i] / fact[sum] * fact[n - i];
    }
    printf("%d\n",(res / fact[n]).num);
  }
}