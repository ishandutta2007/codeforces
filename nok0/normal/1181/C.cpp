...
int main() {
	...
	RREP(i, h) {
		REP(j, w) {
			if(i == h - 1 or s[i][j] != s[i + 1][j])
				lowcnt[i][j] = 1;
			else
				lowcnt[i][j] = lowcnt[i + 1][j] + 1;
		}
	}
	ll res = 0, len = 0;
	...
	REP(i, h) {
		REP(j, w) {
			int cnt = lowcnt[i][j];
			if(i + cnt * 2 < h and lowcnt[i + cnt][j] == cnt and lowcnt[i + cnt * 2][j] >= cnt) {
				if(j - 1 > -1 and s[i][j - 1] == s[i][j] and lowcnt[i][j - 1] == cnt and s[i + cnt][j - 1] == s[i + cnt][j] and lowcnt[i + cnt][j - 1] == cnt and s[i + cnt * 2][j - 1] == s[i + cnt * 2][j] and lowcnt[i + cnt * 2][j - 1] >= cnt) {
					len++;
				} else
					len = 1;
			} else
