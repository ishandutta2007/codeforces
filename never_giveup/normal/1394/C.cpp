...
const int maxn = 3e5 + 100, maxw = 1e4 + 100, inf = 1e9 + 100, sq = 450, LG = 18, mod = 1e9 + 933;
...
bool check(int ds) {
    int a = 0, b = 0, c = inf, d = inf, e = -inf, f = inf;
    for (int i = 0; i < n; i++) {
        int x = q[i].fr, y = q[i].sc;
        setmax(a, x - ds);
        setmax(b, y - ds);
        setmin(c, x + ds);
        setmin(d, y + ds);
