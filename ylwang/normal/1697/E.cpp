...
vi g[MN]; int tot,bel[MN],ok[MN],vis[MN],f[MN];
...
void works() {
	...
	For(i,1,n) {
		g[i].pb(i); int mn=INF;  
		For(j,1,n) { if(j==i) continue; ckmin(mn,dist(i,j)); }
		For(j,1,n) {
			if(dist(i,j)==mn) g[i].pb(j); 
		}
...
//		fprint(g[i]);
	...
	For(i,1,n) {
		if(vis[i]) continue; 	
		int ok=1; 
		for(auto j:g[i]) if(g[j]!=g[i]) ok=0; 
		if(ok) for(auto j:g[i]) vis[j]=1; 
//		cerr<<i<<' '<<SZ(g[i])<<endl;
		foR(j,n-1,0) {
			if(ok&&j+SZ(g[i])<=n) addmod(f[j+SZ(g[i])],f[j]); 
			addmod(f[j+1],f[j]); f[j]=0; 
		}
	}
	int ans=0; 
	For(i,1,n) {
		addmod(ans,mul(f[i],mul(fac[i],C(n,i))));; 
	}
	cout<<ans<<endl;
}
