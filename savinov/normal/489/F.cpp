...
long long solve(int one, int two) {
    //cerr << one << " " << two << endl;
    if (one == 0 && two == 0) {
        return 1;
    }
    long long & res = dp[one][two];
    if (res != -1) {
        return res;
    }
    res = 0;
    if (one > 1) res += (one * (one - 1)) / 2 * solve(one - 2, two) % mod;
    if (two > 1) res += (two * (two - 1)) / 2 * solve(one + 2, two - 2) % mod;
    if (one > 0 && two > 0) res += one * two * solve(one, two - 1) % mod;
    res %= mod;

    //cerr << one << " " << two << " " << res << endl;
