...
int main(){
    ...
    dp[0][0] = 0;
    REP(i, n){
        ll allkillcost = r1 * a[i] + r3;
        ll somekillcost = min(r1 + r2, r1 * (a[i] + 2));
        ll x = min(dp[i][0] + allkillcost, dp[i][1] + somekillcost);
        if(i == n - 1) chmin(x, dp[i][1] - d + allkillcost);
        ll y = dp[i][0] + somekillcost + 2 * d;
        dp[i + 1][0] = x;
        dp[i + 1][1] = y;
    }
    cout << d * (n - 1) + min(dp[n][0], dp[n][1]) << endl;
}