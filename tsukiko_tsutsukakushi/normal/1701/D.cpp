...
void solve() {
    INT(n);
    VEC(int, b, n);
    vc<int> mi(n, inf), ma(n, -inf);
    vvc<P> g(n + 1);
    pqg<P> pque;
    rep(i, n) {
        int id = i + 1;
        ma[i] = (b[i] == 0 ? inf : id / b[i]);
        mi[i] = (id % (b[i] + 1) == 0 ? id / (b[i] + 1) + 1 : (id + b[i]) / (b[i] + 1));
        assert(mi[i] <= ma[i]);
        if(mi[i] < 1) pque.push(P{ma[i], i});
        else g[mi[i]].eb(ma[i], i);
    }
    dump(mi);
    dump(ma);
    vc<int> a(n, -1);
