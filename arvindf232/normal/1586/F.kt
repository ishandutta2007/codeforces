...
fun beatenIn(here:IntArray,beat:IntArray):Int{
    val n = here.size
    return (0 until n).first { beat[it] < here[it] }
}
...
fun main(){
    solve.cases{
        ...
        val everything = mutableListOf<IntArray>()
        product(List(dim){k}){
            everything.add(it.toIntArray())
        }
        everything.sortedBy { it.sum() }

        val ret = List(n){IntArray(n)}

        for(i in n-1 downTo 0){
            for(j in i+1 until n){
                val ii = n-1-i
                val jj = n-1-j
                val use = beatenIn(everything[ii],everything[jj])
                ret[ii][jj] = use
                ret[jj][ii] = use
            }
        }
