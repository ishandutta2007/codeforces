...
struct CHT {
    ...
    ll get(ll x) {
        int lf = 1, rg = s - 1;
        if(s == 1 || x <= isec(l[0], l[1])) lf = 0;
        else while(lf < rg) {
            int md = (lf + rg + 1) >> 1;
            if(isec(l[md - 1], l[md]) <= x) lf = md;
            else rg = md - 1;
        }
        return l[lf].eval(x);
    }
...
struct rect {
    ll x, y, a;
    bool operator < (const rect& he) const { return x < he.x; }
};
...
#endif // HOME
    cin >> n;
    for(int i = 0; i < n; i++) cin >> a[i].x >> a[i].y >> a[i].a;
    sort(a, a + n);

    cht.clr();
    cht.add({0, 0, 0});
    for(int i = 0; i < n; i++) {
        dp[i] = cht.get(-a[i].y) + a[i].x * a[i].y - a[i].a;
        cht.add({a[i].x, dp[i]});
        ans = max(ans, dp[i]);
    }
    cout << ans;
