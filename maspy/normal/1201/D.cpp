...
void solve() {
  ...
  auto f = [&](ll a, ll b) -> ll {
    ...
    ll d = ss.next(a);
    ll cost = INF;
    if (0 <= c && c < M) chmin(cost, abs(a - c) + abs(b - c));
    if (0 <= d && d < M) chmin(cost, abs(a - d) + abs(b - d));
    return cost;
  };

  FOR(x, 1, N) {
    if (len(Y[x]) == 0) continue;
    ll dx = x - px;
    ll L = MIN(Y[x]);
    ll R = MAX(Y[x]);
    tie(c1, c2) = mp(min(f(y1, R) + c1, f(y2, R) + c2) + dx + abs(R - L),
                     min(f(y1, L) + c1, f(y2, L) + c2) + dx + abs(R - L));
    y1 = L;
    y2 = R;
    px = x;
  }
