...
int main() {
    ...
    vec<int> dep(n);
    vec<vec<ll>> dp(n, vec<ll>(n + 1, numeric_limits<int>::min()));
    auto dfs = [&](auto self, int u, int p, int d) -> void {
        ...
        for (int v : g[u]) {
            if (v == p) continue;
            self(self, v, u, d + 1);
            vec<ll> next_dp(n + 1, numeric_limits<int>::min());
            rep(i, d, n + 1) {
                if (dp[u][i] == numeric_limits<int>::min()) continue;
                rep(j, d + 1, n + 1) {
                    if (dp[v][j] == numeric_limits<int>::min()) continue;
                    int dmin = i < n and j < n ? i + j - 2 * d : k + 1;
                    if (dmin > k) {
                        chmax(next_dp[min(i, j)], dp[u][i] + dp[v][j]);
                    }
                }
            }
            swap(dp[u], next_dp);
