...
void work()
{
    reader >>n >>m >>k;
    for(int i = 1; i <= m; i++)
        reader >>a[i].fi >>a[i].se;

    if(k == n)
    {
        int ans = 0;
        for(int i = 1; i <= m; i++) ans += a[i].se - a[i].fi + 1;
        cout <<ans <<endl;
        return ;
    }

    sort(a + 1, a + 1 + m, [](const pii &lhs, const pii &rhs) {return lhs.fi + lhs.se < rhs.fi + rhs.se;});

    for(int i = 1; i + k - 1 <= n; i++)
    {
        for(int j = 1; j <= m; j++)
        {
            auto f = [](int l, int r, int L, int R)
            {
            ...
            v[i][j] = f(i, i + k - 1, a[j].fi, a[j].se);
        }
        v[i][m + 1] = 0;
        for(int j = 1; j <= m + 1; j++) v[i][j] += v[i][j - 1];
    }
    ...
    for(int i = 1; i + k - 1 <= n; i++)
    {
        ...
        {
            while(p <= m && get_abs((i + i + k - 1) - (a[p].fi + a[p].se)) <= get_abs((j + j + k - 1) - (a[p].fi + a[p].se))) p++;
            get_max(ans, v[i][p - 1] + v[j][m + 1] - v[j][p - 1]);
        }
    }
