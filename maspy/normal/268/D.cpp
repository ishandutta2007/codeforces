...
void solve() {
  LL(N, H);
  vvvv(mint, DP, 2, H + 1, H + 1, H + 1);
  DP[1][1][1][1] = 4;
  FOR(_, N - 1) {
    vvvv(mint, newDP, 2, H + 1, H + 1, H + 1);
    FOR(ok, 2) FOR(a, H + 1) FOR(b, H + 1) FOR(c, H + 1) {
      auto& dp = DP[ok][a][b][c];
      ll a1 = min(H, a + 1);
      ll b1 = min(H, b + 1);
      ll c1 = min(H, c + 1);
      newDP[ok][a1][b1][c1] += dp;
      ll h = (ok ? 1 : H);
      if (a < H)
        newDP[1][h][b1][c1] += dp;
      else
  ...
  FOR(a, H + 1)
  FOR(b, H + 1)
  FOR(c, H + 1) if (ok || a < H || b < H || c < H) ANS += DP[ok][a][b][c];
  print(ANS);
}
