...
vector<ll> f(vector<int> a) {
    vector<ll> ans;
    for (int i = 1; i < sz(a); ++i) {
        ans.push_back(max(0, a[i - 1] - a[i]));
        if (i > 1) {
            ans[i - 1] += ans[i - 2];
        }
    }
...
void _solve() {
    ...
    auto ps = [](const vector<ll>& a, int l, int r) {
        return a[r - 1] - (l == 0 ? ll(0) : a[l - 1]);
    };
    ...
    for (int i = 0; i < m; ++i) {
        ...
        if (l < r) {
            cout << ps(lr, l - 1, r - 1) << endl;
        }
        else {
            cout << ps(rl, n - l, n - r) << endl;
        }
    }
}
