...
int main() {
    ...
    ll ans = 0;
    vvc<pll> dp(n);
    rep(i, n) dp[i].eb(-inf<int>, 0);

    vi R(n);
    rep(i, n) {
        int r = seg.max_right(i, [&](int x) { return x >= -i; });
        R[i] = r;
        if(r < n) {
            int nr = seg.max_right(r + 1, [&](int x) { return x >= -i; });
            dp[r].eb(-i, nr - r);
            // dp[r].back().se += dp[r].end()[-2].se;
        }
        ans += r - i;
    }
    ...
    rep(i, n) {
        SORT(dp[i]);
        rep(j, 1, si(dp[i])) dp[i][j].se += dp[i][j - 1].se;
    }
    ...
    rep(i, n) query.eb(R[i], inf<int>, i);
    SORT(query);
    REV(query);
    ...
    fore(p, x, i, query) {
        if(x == inf<int>) {
            ...
            int r = max(0, min(p + 1, -x));
            int l = max(0, -a[p]);
            dump(l, r);
            res[i] = ans - rst.prod(l, r) + cnt.prod(l, r) * p;
        }
    }
