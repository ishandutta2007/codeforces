...
int main() {
    ...
    auto get = [&](vi& v) -> int {
        ...
        int res = 0;
        while (i1 != sz(v)) {
            while (i2 != sz(v)-1 && v[i2] == v[i2+1]) i2++;
            res++;
            i1 = ++i2;
        }
        return res;
    ...
    while (i1 != sz) {
        ...
        pi best = {n*2, 0};
        f0r(i, sz(tmp)-1) {
            ckmin(best, {(tmp[i] == aft) + (tmp[i+1] == bef), i});
        }
        f0r(j, best.s+1) {  
            v1.pb(tmp[j]);
        }
        f1r(j, best.s+1, sz(tmp)) {
            v2.pb(tmp[j]);
        }
        i1 = ++i2;
    }
