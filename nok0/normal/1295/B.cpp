...
void solve() {
	...
	V<> barance(n + 1);
	REP(i, n) {
		barance[i + 1] = barance[i] + (s[i] == '0' ? 1 : -1);
	}
	debug(barance);
	int roop = barance.back();
	int res = !x;
	REPS(i, n) {
		if(barance[i] == x and !roop) {
			print(-1);
			return;
		}
		if(barance[i] <= x and roop > 0 and (x - barance[i]) % roop == 0) res++;
		if(barance[i] >= x and roop < 0 and (barance[i] - x) % roop == 0) res++;
	}
	print(res);
