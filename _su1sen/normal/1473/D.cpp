...
int main() {
    ...
    loop(t) {
        ...
        vector<int> min_l(n + 1), max_l(n + 1), min_r(n + 1), max_r(n + 1);
        min_l[0] = max_l[0] = lev[0];
        rep(i, n) {
            min_l[i + 1] = min(min_l[i], lev[i + 1]);
            max_l[i + 1] = max(max_l[i], lev[i + 1]);
        }
        min_r[n] = max_r[n] = lev[n];
        rrep(i, n) {
            min_r[i] = min(min_r[i + 1], lev[i]);
            max_r[i] = max(max_r[i + 1], lev[i]);
        }
        ...
        loop(m) {
            ...
            int a = min_l[l], b = max_l[l];
            int c = min_r[r], d = max_r[r];

            int min_val = min(a, lev[l] + c - lev[r]);
            int max_val = max(b, lev[l] + d - lev[r]);
