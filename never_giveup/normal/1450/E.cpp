...
const int md = 1e9 + 7;
using MD = std::integral_constant<decay<decltype(md)>::type, md>;
...
const int maxn = 2e5 + 100, maxw = 1e6 + 100, inf = 1e9 + 100, sq = 450, LG = 18, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        cin >> e[i].fr.fr >> e[i].fr.sc >> e[i].sc;
        e[i].fr.fr--;
        e[i].fr.sc--;
        if (e[i].sc == 1) {
            d[e[i].fr.fr][e[i].fr.sc] = 1;
            d[e[i].fr.sc][e[i].fr.fr] = -1;
        } else {
            d[e[i].fr.fr][e[i].fr.sc] = 1;
            d[e[i].fr.sc][e[i].fr.fr] = 1;
        }
    ...
    for (int st = 0; st < n; st++) {
        ...
        for (int i = 0; i < m; i++) {
            if (e[i].sc == 1) {
                if (q[e[i].fr.fr] + 1 != q[e[i].fr.sc]) {
                    can = 0;
                }
            } else {
                if (abs(q[e[i].fr.fr] - q[e[i].fr.sc]) != 1) {
                    can = 0;
                }
            }
        }
