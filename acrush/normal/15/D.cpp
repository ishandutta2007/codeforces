...
int64 SA[maxsize][maxsize];
int sizeQ;
pair<int64,int> Q[maxsize*maxsize];
int r_cnt,r_pos[maxsize*maxsize];
int64 r_cost[maxsize*maxsize];
int C[maxsize][maxsize];

void solve(int n,int A[],int R[],int L)
{
	int next[maxsize],stack[maxsize],size;
	stack[size=0]=n+1;
	for (int i=n;i>=1;i--)
	{
		for (;size>0 && A[stack[size]]>=A[i];size--);
		next[i]=stack[size];
		stack[++size]=i;
	}
	int key=1;
	...
	{
		if (key<i) key=i;
		for (;next[key]<=i+L-1;key=next[key]);
		R[i]=A[key];
	}
}
void addC(int x0,int y0)
{
	for (int x=x0;x<=n;x=(x|(x-1))+1)
		for (int y=y0;y<=m;y=(y|(y-1))+1)
			C[x][y]++;
}
int getC(int x0,int y0)
{
	int R=0;
	for (int x=x0;x>0;x&=(x-1))
		for (int y=y0;y>0;y&=(y-1))
			R+=C[x][y];
	return R;
}
int main()
{
	...
	for (int i=1;i<=n;i++) solve(m,A[i],B[i],b);
	for (int j=1;j<=m-b+1;j++)
	{
		int C[maxsize],D[maxsize];
		for (int i=1;i<=n;i++) C[i]=B[i][j];
		solve(n,C,D,a);
		for (int i=1;i<=n-a+1;i++) B[i][j]=D[i];
	}
	memset(SA,0,sizeof(SA));
	for (int i=1;i<=n;i++) for (int j=1;j<=m;j++) SA[i][j]=SA[i-1][j]+SA[i][j-1]-SA[i-1][j-1]+A[i][j];
	sizeQ=0;
	for (int i=a;i<=n;i++) for (int j=b;j<=m;j++)
		Q[sizeQ++]=MP(SA[i][j]+SA[i-a][j-b]-SA[i-a][j]-SA[i][j-b]-(int64)a*(int64)b*(int64)B[i-a+1][j-b+1],i*10000+j);
	sort(Q,Q+sizeQ);
	r_cnt=0;
	memset(C,0,sizeof(C));
	for (int step=0;step<sizeQ;step++)
	{
		int x2=Q[step].second/10000;
		int y2=Q[step].second%10000;
		int x1=x2-a+1;
		int y1=y2-b+1;
		if (getC(x2,y2)+getC(x1-1,y1-1)-getC(x2,y1-1)-getC(x1-1,y2)==0)
		{
			r_pos[r_cnt]=x1*10000+y1;
			r_cost[r_cnt++]=Q[step].first;
			for (int x=x1;x<=x2;x++) for (int y=y1;y<=y2;y++) addC(x,y);
		}
	}
	printf("%d\n",r_cnt);
	for (int i=0;i<r_cnt;i++)
		printf("%d %d %I64d\n",r_pos[i]/10000,r_pos[i]%10000,r_cost[i]);
	return 0;
}