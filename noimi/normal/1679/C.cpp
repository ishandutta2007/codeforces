...
int main() {
    ...
    rep(q) {
        ...
        if(t == 1) {
            ...
            x--, y--;
            s1[x]++, s2[y]++;
            if(s1[x] == 1) seg1.set(x, 1);
            if(s2[y] == 1) seg2.set(y, 1);
        } else if(t == 2) {
            INT(x, y);
            x--, y--;
            s1[x]--, s2[y]--;
            if(s1[x] == 0) seg1.set(x, 0);
            if(s2[y] == 0) seg2.set(y, 0);
        } else {
            INT(xl, yl, xr, yr);
            xl--, yl--;
            Yes(seg1.prod(xl, xr) == (xr - xl) or seg2.prod(yl, yr) == (yr - yl));
        }
    }
}