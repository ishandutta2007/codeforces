...
ll Solve( ll st ){
	...
	For(i,n)
		For(j,m){
			...
			dp[i][j] = LINF;
			ll expVal = st + (i-1) + (j-1);
			if( a[i][j] < expVal ) continue;
			ll cost = a[i][j] - expVal;

			if( i != 1 ) Mymin( dp[i][j] , dp[i-1][j] + cost );
			if( j != 1 ) Mymin( dp[i][j] , dp[i][j-1] + cost );
		}
	return dp[n][m];
}
