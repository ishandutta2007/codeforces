...
int main() {
	...
	rep(t) {
		int n; cin >> n;
		VI a(n);
		priority_queue<PII> hp;	// (len, -i)
		hp.emplace(n, 0);
		int t = 0;
		while (not hp.empty()) {
			auto [len, i] = hp.top(); hp.pop();
			i *= -1;
			a[i+(len-1)/2] = ++t;
			if ((len-1)/2) hp.emplace((len-1)/2, -i);
			if (len-1-(len-1)/2) hp.emplace(len-1-(len-1)/2, -(i+(len-1)/2+1));
		}
		copy(all(a), make_ostream_joiner(cout, " "));
		cout << newl;
	}
}