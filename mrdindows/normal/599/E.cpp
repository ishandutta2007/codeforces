...
vector<tri> lcas;
vector<pii> pars[120];
bool areConnected[101][101];
int n, m, q;
ll makeTree(int mask, int root);
ll compose(int mask, int lastr, int root) {
    if (mask == 0) return 1;
    ll &res = dp[mask][lastr][root];
    if (res != -1) {
        return res;
    }
    ll ans = 0;
    for (int sm = mask; sm > 0; sm = mask & (sm - 1)) {
        int outm = mask ^ sm;
        bool bad = false;
        for (auto p : pars[root]) {
            if (checkbit(sm, p.first) && checkbit(sm, p.second)) {
                bad = true;
                break;
            }
        }
        ...
        for (int oneMoreRoot = lastr + 1; oneMoreRoot < n; ++oneMoreRoot) {
            if (checkbit(sm, oneMoreRoot)) {
                bool bad = false;
                for (auto x : w[oneMoreRoot]) {
                    if (x != root && !checkbit(sm, x)) {
                        bad = true;
                        break;
                    }
                }
                if (!bad) {
                    ans += makeTree(sm, oneMoreRoot) * compose(outm, oneMoreRoot, root);
                }
            }
        }
    }
//    DBN(mask, lastr, root, ans);
    res = ans;
    return ans;
}
ll dpm[1 << 13][13];
ll makeTree(int mask, int root) {
    ll &res = dpm[mask][root];
    if (res != -1) return res;
    bool bad = false;
    for (auto p : pars[root]) {
        if (!checkbit(mask, p.first) || !checkbit(mask, p.second)) {
            bad = true;
            break;
        }
    }
    if (bad) res = 0;
    else res = compose(mask ^ (1 << root), -1, root);
    return res;
}
int main() {
    cin >> n >> m >> q;
    FI(m) {
        ...
        w[y].push_back(x);
        edges.push_back({x, y});
        areConnected[x][y] = areConnected[y][x] = true;
    }
    FI(q) {
        int x, y, z;
        cin >> x >> y >> z;
        --x; --y; --z;
        lcas.push_back({x, y, z});
        pars[z].push_back({x, y});
    }
    for (int mask = 0; mask < (1 << n); ++mask) {
        FI(13) FJ(13) dp[mask][i][j] = -1;
        FI(13) dpm[mask][i] = -1;
    }
    ll ans = makeTree((1<<n) - 1, 0);
    cout << ans << endl;
