...
int solve() {
    ...
    while (tt--) {
        ...
        for (int z = 0; z < 4; ++z) {
            ...
            dp[0] = 0;
            dp[2] = 1 - s[0] + s[1];
            dp[3] = min(1 - s[0] + s[1] + s[2], 1 - s[0] + 1 - s[1] + s[2]);
            for (int i = 4; i <= n; ++i) {
                dp[i] = min({   dp[i - 2] + 1 - s[i - 2] + s[i - 1],
                                dp[i - 3] + 1 - s[i - 3] + 1 - s[i - 2] + s[i - 1],
                                dp[i - 3] + 1 - s[i - 3] + s[i - 2] + s[i - 1],
                                dp[i - 4] + 1 - s[i - 4] + 1 - s[i - 3] + s[i - 2] + s[i - 1]
                });
            }
            s = s.substr(1) + s[0];
            ans = min(ans, dp[n]);
