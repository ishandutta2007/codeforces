...
int main(){
  ...
  rep(j,30){
    rep(i,N)vis[i]=0;
    rep(i,N)g[i].clear();
    for(auto&[u,v,x]:query)if(~x>>j&1)vis[u]=vis[v]=1;
    for(auto&[u,v,x]:query)if(x>>j&1){
      if(vis[u]|vis[v]){
        assert(!(vis[u]&vis[v]));
        if(vis[u])que.emplace(v);
        else que.emplace(u);
      }else if(u==v)que.emplace(u);
    }
    while(!que.empty()){
    ...
    rep(i,N){
      if(vis[i])continue;
      vis[i]=1;
      for(auto x:g[i])que.emplace(x.first);
      while(!que.empty()){
        auto x=que.front();que.pop();
        if(vis[x])continue;
        vis[x]=1;
        ans[x]|=1<<j;
        for(auto[y,yid]:g[x]){
          if(yid+1!=(ll)g[y].size()){
            auto [ny,nyid]=g[y].back();
            swap(g[y][yid],g[y].back());
            g[ny][nyid].second=yid;
          }
          g[y].pop_back();
        }
        g[x].clear();
