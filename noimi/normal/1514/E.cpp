...
vector<int> string_to_vec(const string &s, char margin = 'a') {
    vector<int> res(si(s));
    rep(i, si(s)) res[i] = s[i] - margin;
    return res;
}
...
int main() {
    TEST {
        ...
        {
            auto ask = [&](int i, vi v) {
                cout << 2 << " " << i << " " << si(v) << " " << v << endl;
                cout.flush();
                return in();
            };

            rep(i, n - 1) g[a[i]].eb(a[i + 1]);
            vi w;
            rep(i, n - 1) w.eb(a[i]);
            rep3(i, si(a) - 1, 0) {
                if(!empty(w) and w.back() == a[i]) w.pop_back();
                while(!empty(w) and ask(a[i], w)) {
                    g[a[i]].eb(w.back());
                    w.pop_back();
                }
            }
