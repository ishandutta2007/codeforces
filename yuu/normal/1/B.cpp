...
int to_number(const string& value) {
    int ans = 0;
    for (auto&& c : value) (ans *= 26) += (c - 'A');
    ans++;
    int power = 1;
...
void solve() {
    cin >> s;
    bool is_r_c = true;
    if (s[0] != 'R') is_r_c = false;
    if (is_r_c && (!isdigit(s[1]))) is_r_c = false;
    if (is_r_c) {
        int c_pos = s.find('C');
        if (c_pos == -1) is_r_c = false;
    }
    if (is_r_c) {
        int r = 0, c = 0;
        bool is_c = 0;
