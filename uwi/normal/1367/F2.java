...
public class F2 {
	...
	void go()
	{
		...
		for(int i = 0;i < n-1;i++){
			...
			fnex[i] = b[i+1].length-lowerBound(b[i+1], b[i][b[i].length-1]);
		}
		for(int i = 1;i < n;i++){
			if(b[i].length == 0)continue;
			fpre[i] = lowerBound(b[i-1], b[i][0]);
		}
		
		int dp = 0;
		int cur = 0;
		for(int i = n-1;i >= 0;i--){
			if(b[i].length > 0 && b[i+1].length > 0 && b[i][b[i].length-1] < b[i+1][0]){
				cur += b[i].length;
			}else{
				cur = fnex[i] + b[i].length;
			}
			dp = Math.max(dp, cur + fpre[i]);
		}
		ans = Math.max(ans, dp);
		
		for(int i = 0;i < n-1;i++){
			if(b[i].length > 0 && b[i+1].length > 0){
				int k = 0;
				for(int j = 0;j < b[i].length;j++){
					while(k < b[i+1].length && b[i+1][k] <= b[i][j])k++;
					ans = Math.max(ans, j+1 + (b[i+1].length - k));
				}
			}
		}
