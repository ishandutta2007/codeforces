...
int64_t a[2];
int64_t w[2];
bool check(int64_t cnt) {
    if (cnt <= a[0]) return p / w[0] + f / w[0] >= cnt;
    for (int i = 0; i <= a[0]; i++) {
        if (w[0] * i > p) break;
        if (w[0] * (a[0] - i) > f) continue;
        if ((p - w[0] * i) / w[1] + (f - w[0] * (a[0] - i)) / w[1] + a[0] >= cnt) return true;
    }
    return false;
}
void solve() {
    ...
    int low = 1, high = a[0] + a[1], mid, res = 0;
    while (low <= high) {
        mid = (low + high) / 2;
        if (check(mid)) {
            res = mid;
