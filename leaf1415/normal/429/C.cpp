...
bool dfs(int p)
{
	...
	if(p == sz(vec)) return true;
	
	ll q = vec[p];
	for(auto &v : dvec[q-1]){
		for(auto x : v) a[x]--;
		bool flag = true;
		for(auto x : v) if(a[x] < 0){
			flag = false;
			break;
		}
		a[q]++;
...
vector<ll> tmp;
void part(int n, int p, int d)
{
	if(n == 0){
		if(sz(tmp) >= 2) dvec[d].pb(tmp);
		return;
	}
	if(n < p) return;
	part(n, p+1, d);
...
int main(void)
{
	...
	if(sz(vec) == 0 || vec.back() != n){
		no();
		return 0;
	}
	
	rep(i, 1, n) part(i, 1, i);
