...
void solve() {
  ...
  vv(bool, done, N + 1, 24);
  // (a^b) >= (b^c)+(a^c) 
  auto dfs = [&](auto& dfs, int n, int xa, int xb, int xc, int k) -> mint {
    assert(0 <= k && k <= 2);
    int s = (12 * xa + 6 * xb + 3 * xc + k);
    if (done[n][s]) return dp[n][s];
    mint& res = dp[n][s];
    done[n][s] = 1;

    if (n == 0) {
      if (xa == 0 && xb == 0 && xc == 0 && k == 0) { res += mint(1); }
      return res;
    }
    int last = S[n - 1] - '0';
    FOR(a, 2) FOR(b, 2) FOR(c, 2) {
      int ya = ceil(a + xa - last, 2);
      int yb = ceil(b + xb - last, 2);
      int yc = ceil(c + xc - last, 2);
      int l = a ^ b;
      int r = (b ^ c) + (a ^ c);
      int kk = ceil(r - l + k, 2);
      res += dfs(dfs, n - 1, ya, yb, yc, kk);
    }
    return res;
  };

  mint base = dfs(dfs, N, 0, 0, 0, 0);
  mint full = 0;
  for (auto&& s: S) {
    int x = s - '0';
    full = mint(2) * full + mint(x);
  }
  full += mint(1);
  mint x3 = full;
  mint x2 = (full - 1) * full;
  mint x1 = base - x2 - x2 - x3;
  mint ANS = full * full * full - mint(3) * (x1 + x2) - x3;
  print(ANS);
}
