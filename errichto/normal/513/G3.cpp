...
int main(int argc, char *argv[])
{
	...
	RI(i, n) FOR(j, i + 1, n) RI(low, i) FOR(high, j, n) wzorek[i][j][low+high]++;
	
	RI(t, times) {
		RI(i, n) FOR(j, i + 1, n) if(i <= n + 1 -j) {
			if(t < (times > 140 ? 80 : 141) || (t < 80 && min(i, n+1-j) < 30) || (t < 120 && min(i,n+1-j)<20) || (t < 200 && min(i,n+1-j)<12) || (t < 300 && min(i, n + 1 - j) < 8) || (t < 700 && min(i, n + 1 - j) < 5) || (t < 1200 && min(i, n + 1 - j) < 4)) {
					...
					RI(s, 2 * n) if(wzorek[i][j][s])
						p[i][j][t] += (1.L - p[s - i][s - j][t-1]) * wzorek[i][j][s];
					
					p[i][j][t] *= 2.L / n / (n + 1);
					p[j][i][t] = p[n+1-i][n+1-j][t] = p[n+1-j][n+1-i][t] = p[i][j][t];
			}
			else p[i][j][t] = p[n+1-i][n+1-j][t] = p[n+1-j][n+1-i][t] = p[j][i][t] = 0.5;
		}
	}
