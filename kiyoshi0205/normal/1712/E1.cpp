...
int codeforces;cin>>codeforces;while(codeforces--){
  ll L,R;
  cin>>L>>R;
  ll ans=(R-L+1)*(R-L)/2*(R-L-1)/3;
  for(ll i=L;i<=R-2;++i){
    for(ll k=i*2;k<=R;k+=i){
      ans-=prev(dlis[k].end())-upper_bound(all(dlis[k]),i);
    }
    if(~i&1){
      for(ll k2=i*3;k2<=R*2;k2+=i*2){
        const ll k=k2/2;
        ans-=lower_bound(all(dlis[k2]),k)-upper_bound(all(dlis[k2]),max(i,k-i));
      }
    }
    for(ll k2=i*4;k2<=R*2;k2+=i*2){
      const ll k=k2/2; 
      ans-=lower_bound(all(dlis2[k2]),k)-upper_bound(all(dlis2[k2]),max(i,k-i));
    }
  }
  print(ans);
}
