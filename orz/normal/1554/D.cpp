...
string find_string(vs& v, vi& w, const int n);

void make(vs& v, vi& w, const int n)
{
	if (n % 3 == 2)
	{
		int a = n / 3;
		string p = find_string(v, w, a);
		v[n] = p + char('a' + w[a]) + p + char('a' + w[a] + 1) + p;
		w[n] = w[a] + 2;
	}
	else if (n % 3 == 1)
	{
		...
		else
		{
			int a = n / 3 - 2;
			string p = find_string(v, w, a);
			v[n] = p + char('a' + w[a]) + char('a' + w[a]) + char('a' + w[a]) + p + char('a' + w[a] + 1) + char('a' + w[a] + 1) + char('a' + w[a] + 1) + p + char('a' + w[a] + 2);
			w[n] = w[a] + 3;
		}
	}
	else
