...
int main() {
	...
	rep(t) {
		...
		rep(i, n) dsu.unite(a[i], b[i]);
		vector<int> components;
		foa(g, dsu.groups()) { components.push_back(int(g.size())); }

		cout << solve(components, n) << '\n';
	}
	return 0;
}