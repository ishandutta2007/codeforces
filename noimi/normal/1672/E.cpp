...
int main() {
    int cases = oj_local(1, 1000);
    INT(n);
    while(cases--) {

        vi a(n);
        LOCAL {
            rep(i, n) a[i] = rnd(1, 2000);
            // a = {5, 2, 7, 3, 5, 6};
        }
        ll mi = inf<ll>;
        int iter = 0;
        ...
        auto ask = [&](int x) {
            ...
            if(x <= k) return 0LL;
            if(iter == n + 30) { rep(i, inf<int>) OUT(1); }
            iter++;
            LOCAL {
                if(MAX(a) > x) return (ll)0;
                int tmp = x + 1;
                int res = 0;
                rep(i, n) {
                    if(tmp >= a[i] + 1) {
        ...
        int sum = bin_search(n * 2000 + n, 0, [&](int x) { return ask(x) == 1; }) - (n - 1);
        int r = ceil(sum, n);
        if(n == 1) {
            ans(sum);
            exit(0);
