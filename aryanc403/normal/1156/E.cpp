...
const lli logN = 20;
    ...
    lli m;
    vi a;
    lli idx[maxN+5],rmq[maxN+5][logN+5];
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

lli maxi(lli l,lli r)
{
    lli i=1,j=0,ans=0;
    while(i<=r-l+1)
    {
        ans=max({ans,rmq[l][j],rmq[r-i+1][j]});
        i*=2;j++;
        dbg(i,j);
    }
    dbg(l,r,ans);
...
// cin>>T;while(T--)
{
    ...
    repA(i,1,n)
    {
        rmq[i][j]=rmq[i][j-1];
        if(i+(1LL<<(j-1)) <= n)
        rmq[i][j]=max(rmq[i][j],rmq[i+(1LL<<(j-1))][j-1]);
    }
