...
int main(){
    ...
    vector<vector<int>> id1(MAX), id2(MAX);
    rep(i, N){
        id1[l[i]].eb(i), id2[a[i]].eb(i);
    }
    ...
    Lazy_Segment_Tree<int, int> seg(vector<int>(MAX+1, 0), f, g, h, -inf, 0);
    int K = 0, L = 0, R = 0;

    rep(i, MAX){
        each(e, id1[i]){
            seg.apply(a[e], r[e]+1, 1);
        }

        int x = seg.query(0, MAX+1);
        if(chmax(K, x)){
            L = i, R = seg.find_first(0, c, x);
        }
    ...
    cout << K << '\n';
    rep(i, N){
        if(l[i] <= L && L <= a[i] && a[i] <= R && R <= r[i]) cout << i+1 << ' ';
    }
    cout << '\n';
}