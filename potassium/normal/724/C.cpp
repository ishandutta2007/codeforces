...
int main() {
	...
	ll g;
	function<pii(int, int)> exgcd = [&](int x, int y) {
		if (x == 0) {
			g = y;
			return pii(0, -1);
		}
		pii sol = exgcd(y % x, x);
		return pii(-sol.se - y / x * sol.fi, -sol.fi);
	};
	ll p,q;
	tie(p, q) = exgcd(n, m);
	...
	while (k--) {
		...
		ll ans = min({ solve(x, y), solve(2 * n - x, y), solve(x, 2 * m - y), solve(2 * n - x, 2 * m - y) });
