...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        int n = in.readInt();
        auto end = in.readTable<int>(n, 2 * n);

        arr2d<char> answer(n, n, 0);
        string dir = "ULDR";
        for (int i : range(n)) {
            for (int j : range(n)) {
                if (end(i, 2 * j) == i + 1 && end(i, 2 * j + 1) == j + 1) {
                    answer(i, j) = 'X';
                    que<pii> q;
                    q.emplace(i, j);
                    while (!q.empty()) {
                        ...
                        for (int k : range(4)) {
                            int nr = r + DX4[k];
                            int nc = c + DY4[k];
                            if (isValidCell(nr, nc, n, n) && end(nr, 2 * nc) == i + 1 && end(nr, 2 * nc + 1) == j + 1 &&
                                answer(nr, nc) == 0) {
                                answer(nr, nc) = dir[k];
                                q.emplace(nr, nc);
                            }
                ...
                if (end(i, 2 * j) == -1) {
                    for (int k : range(4)) {
                        int nr = i + DX4[k];
                        int nc = j + DY4[k];
                        if (isValidCell(nr, nc, n, n) && end(nr, 2 * nc) == -1) {
                            answer(i, j) = dir[k ^ 2];
                            break;
                        }
                    }
        ...
        out.printLine("VALID");
        for (int i : range(n)) {
            out.printLine(string(all(answer[i])));
        }
    }
};
