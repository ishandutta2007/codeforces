...
struct bitree {
	...
	void add(int x, int v) {
		while(x <= 2e5) addto(sum[x], v), x += lowbit(x);
	}
	int Sum(int x) {
		int res = 0;
		while(x) addto(res, sum[x]), x -= lowbit(x);
		return res;
	}
} bit;
...
void mian() {
	...
	REP(i, 1, m) b[i] = read();
	int ans = 0, val = 1;
	REP(i, 1, 2e5) val = (ll)val * ifc[a[i]] % P;
	REP(i, 1, 2e5) bit.add(i, a[i]);
	REP(i, 1, min(n, m)) {
		int ad = (ll)fc[n - i] * val % P;
		ad = (ll)ad * bit.Sum(b[i] - 1) % P;
		addto(ans, ad);
		if(a[b[i]]) {
			val = (ll)val * a[b[i]] % P;
			--a[b[i]];
