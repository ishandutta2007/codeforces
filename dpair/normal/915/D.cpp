...
int head[505], e[MAXN], nxt[MAXN], cnt;
...
inline bool check() {
    hd = 0; ed = 0; rep(i, 1, n) {if(!p[i]) q[ed ++] = i;}
    while(hd < ed) {
        const int x = q[hd ++];
        for (int i = head[x];i;i = nxt[i]) {if(!(-- p[e[i]])) q[ed ++] = e[i];}
    }
    return ed == n;
}
...
signed main(){
    read(n, m); while(m --) {
        int u, v; read(u, v);
        add(u, v); ++ d[v];
    } memcpy(p, d, sizeof(p)); if(check()) return print("YES"), 0;
    rep(i, 1, n) {
        memcpy(p, d, sizeof(p)); if(!p[i]) continue;
        -- p[i]; if(check()) return print("YES"), 0;
    } print("NO");
}