...
int main() {
    ...
    int K = 400;
    vector<vector<pair<pair<int, int>, int>>> block(K);
    ...
    for (int i = 0; i<m; i++)
    {
        ...
        cin>>l>>r;
        l--;
        block[l/K].push_back(mp(mp(r, l), i));
    }

    vector<ll> answer(m);

    for (int i = 0; i<K; i++) if (block[i].size())
    {
        ll current = 0;
        sort(block[i].begin(), block[i].end());
        int t = block[i].size();
        vector<int> cnt(N);
        int l = block[i][0].first.second;
        ...
        answer[block[i][0].second] = current;
        for (int j = 1; j<t; j++)
        {
            while (r<block[i][j].first.first)
            {
                current+=cnt[b[r+1]^k];
                cnt[b[r+1]]++;
                r++;
            }
            while (l>block[i][j].first.second)
            {
                current+=cnt[b[l-1]^k];
                cnt[b[l-1]]++;
                l--;
            ...
            answer[block[i][j].second] = current;
        }
    }
