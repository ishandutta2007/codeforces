...
void solve(int x) {
    if(x == 1) return ;
    int p1 = 0, p2 = 0;
    for (int i = 1;i <= x;++ i) { if(a[i] == x) p1 = i; }
    sprint(p1); std::reverse(a + 1, a + p1 + 1);
    for (int i = 1;i <= x;++ i) { if(a[i] == x - 1) p2 = i; }
    sprint(p2 - 1); std::reverse(a + 1, a + p2);
    sprint(p2 + 1); std::reverse(a + 1, a + p2 + 2);
    sprint(3); std::swap(a[1], a[3]);
    sprint(x); std::reverse(a + 1, a + x + 1);
    solve(x - 2);
}

inline void work() {
    read(n); read(a + 1, a + n + 1);
    rep(i, 1, n) if((a[i] & 1) != (i & 1)) return print("-1");
    print((n >> 1) * 5);
    solve(n);
    print();
}
