...
void solve(){
    ...
    long long moves = 0;
    long long curr = max(  (min(r1,r2) - l2)* n , 0ll);
    if (curr >= k){
    	cout << 0 << endl;
    	return;
    }
    ...
    for(long long cnt = 1; cnt <= n; ++cnt){
    	...
    	assert(l2 >= l1);
    	long long cmoves =(max(l2 - r1, 0ll) * cnt);


    	r1 = max(r1, l2);
    	long long extra = (max(r2,r1) - l1) - (min(r1,r2) - l2);
