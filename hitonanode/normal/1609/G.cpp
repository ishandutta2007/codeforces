...
S op(S l, S r) {
    return {l.sz + r.sz, l.sum + r.sum, l.hi > r.hi ? l.hi : r.hi};
    // return l + r;
}
S e() { return {0, 0, -(1LL << 60)}; }
S mp(lint f, S x) {
    if (f) return {x.sz, x.sum + x.sz * f, x.hi + f};
    return x;
}
...
int main() {
    ...
    vector<S> db(M);
    db[0] = {1, B[0], 0};
    FOR(i, 1, M) db[i] = {1, B[i] - B[i - 1], B[i] - B[i - 1]};
    for (auto d : db) dbg(d.hi);
    atcoder::lazy_segtree<S, op, e, lint, mp, composition, id> tree(db);
    ...
    while (Q--) {
        ...
        idx.assign(N - 1, -1);
        REP(i, N - 1) {
            idx[i] = tree.max_right(0, [&](S x) { return x.hi < A[i + 1] - A[i]; }) - 1;
            if (idx[i] < 0) idx[i]++;
        }
        ...
        REP(i, N) {
            int l = (i ? idx[i - 1] : 0);
            int r = (i + 1 == N ? M - 1 : idx[i]) + 1;
            ret += A[i] * (r - l);
            // if (i + 1 < N) ret += tree.prod(0, r).sum;
        }
        dbg(ret);
        for (auto i : idx) ret += tree.prod(0, i + 1).sum;
        cout << ret << '\n';
    }
}