...
void solve() {
    ...
    int pre_ans = 0;
    int ans = 0;
    map<pair<int, int>, int> gogo;
    REP(i, n) {
        if (inputs[i].second == 0) {
            if (inputs[i].first == 0) ans++;
            continue;
        }
        int hoge = gcd(llabs(inputs[i].first), llabs(inputs[i].second));
        inputs[i].first /= hoge;
        inputs[i].second /= hoge;
        gogo[inputs[i]]++;
    }
    for (auto x : gogo) {
        pre_ans = max(pre_ans, x.second);
    }
