...
bool dfs(mint h, vector<int>& counts, vector<int>& perm, int n, const vector<mint>& values){
	int k = values.size();
	++vis;
	int s = 0;
	for(int i = 0; i < k; ++i){
	...
	for(int i = 0; i < k; ++i){
		if (counts[i] > 0){
			--counts[i];
			perm[n - 1] = i;
			if (dfs(h - n * values[i], counts, perm, n - 1, values)){
				return true;
			}
			perm[n - 1] = -1;
			++counts[i];
	...
	return dfs(h, counts, perm, n - 1, values);
}
...
void solve(){
    ...
    mint A, h;
    cin >> k >> A >> h;
    vector<int> powers = {1, 2, 3, 5, 9, 17};
    vector<int> count = {1, 1, 2, 4, 8, 16};
    
    mint sum = 0;

    vector<mint> values(k + 1);
    for(int i = 0; i <= k; ++i){
    	values[i] = (A^ powers[i]);
    }
    for(int i = 1; i <= (1 << k); ++i){
    ...
    for(int i = 0; i < k; ++i){    	
    	for(int j = 0; j < count[i]; ++j){
    		flatten.push_back(values[i]);
    		invert.push_back(i);
    	}
    }
	int M = flatten.size();
    gp_hash_table<int, int, custom_hash> reach;
    for (int i = 0; i < (1 << M); ++i){
    	mint tmp = h;
    	for(int j = 0; j < M; ++j){
    		if (i & (1 << j)){
    ...
    auto gen = [] (int mask, int matches){
    	vector<int> pos(matches + 1, -1);
    	vector<int> alive(matches + 1);
    	iota(all(alive), 0);
    	int idx = 0;
    	int iter = 0;
    	while (alive.size() > 1){
    		...
    		for(int k = 0; k < z; ++k){
    			assert (idx < matches);
    			if (mask & (1 << idx)){
    				tmp.push_back(alive[(k << 1)| 1]);
    				pos[alive[(k << 1)]] = iter;
    			}
    			else{
    				tmp.push_back(alive[(k << 1)]);
    				pos[alive[(k << 1) | 1]] = iter;
    			}
    			++idx;
    		}
    		++iter;
    ...
    int matches = (1 << (k - 1)) - 1;
    for(int i = 0; i < (1 << matches); ++i){
    	vector<int> perm = gen(i, matches);
    	mint cur = 0;
    	for(int j = 0; j < matches + 1; ++j){
    		cur += (2 * j + 1) * values[perm[j]];
    	}

    	if (reach.find(cur.x) != reach.end()){
    		vector<int> ans((1 << k), k);
    		int msk = reach[cur.x];
    		vector<int> swps(k);
    ...
    // if (dfs(h, count, perm, (1 << k), values)){
    // 	for(auto &e: perm){
    // 		if (e == -1){
    // 			e = k;
    // 		}
