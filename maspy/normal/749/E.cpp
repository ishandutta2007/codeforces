...
void solve() {
  LL(N);
  VEC(ll, A, N);
  FenwickTree<ll> bit_1(N + 1);
  FenwickTree<ll> bit_2(N + 1);
  ld ANS = 0;
  FOR(j, N) {
    ll lo_c = bit_1.sum(A[j]);
    ll lo_s = bit_2.sum(A[j]);
    ll hi_c = bit_1.sum_all() - lo_c;
    ll hi_s = bit_2.sum_all() - lo_s;
    ANS += (lo_c + lo_s) * (N - j) / ld(N * (N + 1));

    ANS += hi_c;
    ANS -= (hi_c + hi_s) * (N - j) / ld(N * (N + 1));
    bit_1.add(A[j], 1);
    bit_2.add(A[j], j);
  }
  print(ANS);
