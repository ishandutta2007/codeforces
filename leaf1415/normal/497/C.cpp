...
int main(void)
{
	...
	cin >> m;
	rep(i, 1, m) cin >> c[i] >> d[i] >> k[i];
	
	vector<P> evec;
	rep(i, 1, n) evec.push_back(P(a[i], i));
	rep(i, 1, m) evec.push_back(P(c[i], -i));
	sort(all(evec));
	
	for(auto p : evec){
		ll id = abs(p.snd);
		if(p.snd < 0) mp[d[id]].pb(P(k[id], id));
		else{
			auto it = mp.lower_bound(b[id]);
			if(it == mp.end()){
				no();
				return 0;
			}
			ans[id] = it->second.back().second;
			it->second.back().first--;
			if(it->second.back().first == 0) it->second.pop_back();
			if(sz(it->second) == 0) mp.erase(it);
		}
		//outl(mp);
	}
	yes();
