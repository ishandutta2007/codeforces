...
#line 1 "/home/maspy/library/graph/centroiddecomp.hpp"
template <typename T>
...
void solve() {
  ...
  FOR(q, Q) {
    query[q].se--;
    v_to_I[query[q].se].eb(q);
  }

  const ll INF = 1LL << 60;
  vi ANS(Q, INF);

  auto calc = [&](vi& V, vi& par, vi& dep) -> void {
    // collect query
    vi QUERY;
    FORIN(v, V) FORIN(q, v_to_I[v]) QUERY.eb(q);
    sort(all(QUERY));

    // root 
    ll x = INF;
    FORIN(v, V) if (v == 0) x = dep[0];
    FORIN(q, QUERY) {
      auto [t, v] = query[q];
      if (t == 1) {
        chmin(x, dep[v]);
  ...
  centroid_decomposition(G, calc);
  FORIN(x, ANS) if (x != INF) print(x);
}
