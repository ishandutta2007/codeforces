...
if(double(int(a[i]))==a[i]){c[i]=a[i];b[i]=a[i];}
    if(double(int(a[i]))>a[i]){c[i]=a[i];c[i]--;b[i]=a[i];}
    else if(double(int(a[i]))<a[i]){c[i]=a[i];c[i]++;b[i]=a[i];}
...
//for(int i=0;i<n;i++)cout<<b[i]<<" ";
 if(sum>0)
{
    for(int i=0;i<n;i++)if(c[i]<b[i]){swap(c[i],b[i]);sum--;if(sum==0)break;}
}
else if(sum<0)
{
    for(int i=0;i<n;i++)if(c[i]>b[i]){swap(c[i],b[i]);sum++;if(sum==0)break;}
