...
void perform_operation(const int n, vi& c, const vi& op, vvi& ans, int& pos)
{
	if (sz(op) >= 2)
	{
		ans.pb(op);
...
int another_main()
{
	...
	for (int i = 2; i <= n; ++i)
	{
		...
		perform_operation(n, c, op, ans, pos);
	}
	if (rev)
	{
		vi op;
