...
struct Rerooting {
    ...
    Rerooting(int n, const F1 &f1, const F2 &f2, const T &ident) : g(n), f1(f1), f2(f2), ident(ident), dp(n), subdp(n, ident) {}
...
// {even, odd}
typedef pair<mint, mint> P2;

void solve() {
    INT(n);
    VEC(int, a, n);
    auto f1 = [&](P2 a, P2 b) -> P2 {
        return P2{a.fr + b.fr, a.sc + b.sc};
    };
    auto f2 = [&](P2 a, int x) -> P2 {
        return P2{a.sc, a.fr + x};
    };
    Rerooting<int, P2> rt(n, f1, f2, P2{0, 0});
    rep(i, n - 1) {
        INT(u, v); -- u, -- v;
        rt.add_edge(u, v, 1);
    }
    ...
    rep(v, n) {
        ...
        mint sum = 0;
        rep(i, sz) {
            sum += dp[v][i].fr + dp[v][i].sc;
        }
        rep(i, sz) {
            mint cur = dp[v][i].fr + dp[v][i].sc;
            debug(v, i, dp[v][i].fr.val(), dp[v][i].sc.val());
            ans += mint(a[v]) * (dp[v][i].fr - dp[v][i].sc) * mint(sum - cur + 1);
        }
        ans += mint(a[v]) * (sum + 1);
    }
    out(ans.val());
