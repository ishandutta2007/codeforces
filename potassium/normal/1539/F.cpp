...
int main() {
	...
	auto go = [&](int add) {
		vector<S> ps_base(n), ss_base(n);
		for (int i = 0; i < n; i++) ps_base[i] = { i + 1 };
		for (int i = n - 1; i >= 0; i--) ss_base[i] = { n - i };
		SegTree<S, op, e, F, mapping, composition, id> ps = ps_base, ss = ss_base;

		int last = 0;
		for (int i = 0; i < n; i++) {
			if (i > 0 && order[i - 1].first != order[i].first) {
				for (int j = last; j < i; j++) {
					const int pos = order[j].second;
					ps.apply(pos, n, { -2 });
					ss.apply(0, pos + 1, { -2 });
				}
				last = i;
			...
			const int pos = order[i].second;
			int lbest = ss.prod(0, pos + 1).val - ss.get(pos).val;
			int rbest = ps.prod(pos, n).val - ps.get(pos).val;
			int total = lbest + rbest;

			chkmax(ans[pos], (total + add) / 2);
		}
	};
	go(1);
	reverse(all(order));
