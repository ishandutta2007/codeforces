...
signed main(){
    ...
    q.emplace(0,sx,sy);
    if(s[sx][sy]=='#' or s[gx][gy]=='#'){
        cout<<-1<<endl;return 0;
    }
    bool used2[n+2][m+2][4]={};
    rep(i,4)
    used2[sx][sy][i]=1;
    int d[5]={0,1,0,-1,0};
    int mem[N][N];
    rep(i,n+2)rep(j,m+2)mem[i][j]=1e7;
    mem[sx][sy]=0;
    while(!q.empty()){
        auto [c,x,y]=q.top(); q.pop();
        if(-c>mem[x][y])continue;
        rep(t,4){
            used2[x][y][t]=1;
            rep2(i,1,n+m){
                int X = x+d[t]*i,Y=y+d[t+1]*i;
                if(s[x+d[t]*i][y+d[t+1]*i]=='.'){
                    if(chmin(mem[X][Y],-c+((int)i+k-1)/k))q.emplace(c-(i+k-1)/k,X,Y);
                    if(used2[X][Y][t])break;
                    used2[X][Y][t]=1;
                }
                else break;
            }
    ...
    cout<<(mem[gx][gy]==1e7? -1:mem[gx][gy])<<endl;
}