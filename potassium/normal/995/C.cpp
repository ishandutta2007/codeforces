...
int main() {
	int n; R(n);
	vector<pair<pll, vector<int>>> v(n);
	for (int i = 0; i < n; i++) {
		R(v[i].F);
		v[i].S = {i};
	}
	...
	const ll len2 = len * len;
	
	vector<int> ans(n, 1);
	while (v.size() >= 3) {
		auto test = [&](pll v1, pll v2) {
			for (int sgn = -1; sgn <= 1; sgn += 2) {
				ll newx = v1.F + v2.F * sgn;
				ll newy = v1.S + v2.S * sgn;
				
				if (newx * newx + newy * newy <= len2)
					return sgn;
			}
			return 0;
		};
		...
		for (int i = SZ - 1; i >= SZ - 3; i--)
			for (int j = i - 1; j >= SZ - 3; j--) {
				int res = test(v[i].F, v[j].F);
				
				if (res) {
					int small = (v[i].S.size() < v[j].S.size() ? i : j);
					int large = i + j - small;
					if (res == -1) {
						for (auto& idx : v[small].S)
							ans[idx] *= -1;
					}
					for (auto& idx : v[small].S)
						v[large].S.pb(idx);
						
					v[large].F.F += v[small].F.F * res;
					v[large].F.S += v[small].F.S * res;
					
					v.erase(v.begin() + small);
					goto nxt;
				}
			}
	...
	auto test2 = [&](pll v1, pll v2) {
		ll negx = v1.F - v2.F;
		ll negy = v1.S - v2.S;
		ll negv = negx * negx + negy * negy;
		
		ll posx = v1.F + v2.F;
		ll posy = v1.S + v2.S;
		ll posv = posx * posx + posy * posy;
	...
	if (test2(v.front().F, v.back().F) == -1)
		for (auto& idx : v.back().S)
			ans[idx] *= -1;
