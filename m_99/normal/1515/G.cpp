...
long long get(vector<vector<pair<int,long long>>> E){
	...
	while(Q.size()>0){
		...
		rep(i,E[u].size()){
			int v = E[u][i].first;
			if(dis[v]!=Inf){
				ret = gcd(ret,dis[u] - dis[v] + E[u][i].second);
				continue;
			}
...
int main(){
	...
	rep(i,t.size()){
		rep(j,t[i].size()){
			pos[t[i][j]] = make_pair(i,j);
		}
	}
	
	vector<vector<vector<pair<int,long long>>>> E(t.size());
	rep(i,E.size()){
		E[i].resize(t[i].size());
	}
	
	rep(i,m){
		if(pos[a[i]].first!=pos[b[i]].first)continue;
		E[pos[a[i]].first][pos[a[i]].second].emplace_back(pos[b[i]].second,l[i]);
	}
	
	vector<long long> G(t.size());
	rep(i,t.size()){
		G[i] = get(E[i]);
	}
	...
	rep(_,Q){
		...
		long long g = gcd(G[v],t);
		
		if((t-s)%g!=0)printf("NO\n");
		else printf("YES\n");
