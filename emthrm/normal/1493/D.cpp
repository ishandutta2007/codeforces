...
int main() {
  ...
  ModInt ans = 1;
  FOR(i, 1, M) {
    if (osa.smallest_prime_factor[i] == i && st[i].size() == n) ans *= ModInt(i).pow(*st[i].begin());
  }
  while (q--) {
    int i, x; cin >> i >> x; --i;
    for (auto [k, v] : osa.query(x)) {
      int prev = 0;
      if (st[k].size() == n) prev = *st[k].begin();
      if (a[i][k] > 0) st[k].erase(st[k].lower_bound(a[i][k]));
      a[i][k] += v;
      st[k].emplace(a[i][k]);
      if (st[k].size() == n) ans *= ModInt(k).pow(*st[k].begin() - prev);
    }
    cout << ans << '\n';
  }
  return 0;
