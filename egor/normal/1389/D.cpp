...
public:
    void solve() {
        ...
        int base = max(0, min(r1, r2) - max(l1, l2));
        ll start = base;
        start *= n;
        if (start >= k) {
            out.printLine(0);
        ...
        int dist = max(0, max(l1 - r2, l2 - r1));
        int len = max(r1, r2) - min(l1, l2);
        ll answer = 0;
        for (int i : range(n)) {
            if (start + len - base <= k) {
                answer += 2 * len - (r1 - l1) - (r2 - l2);
                start += len - base;
                continue;
            }
            ll cand1 = i == 0 ? numeric_limits<ll>::max() : answer + 2 * (k - start);
            ll cand2 = answer + dist + (k - start);
            out.printLine(min(cand1, cand2));
            return;
        }
        out.printLine(answer + 2 * (k - start));
    }
};
