...
int main() {
	...
	const int MAXA = 8192;
	vector<vector<bool>> done(MAXA, vector<bool>(MAXA));
	vector<vector<int>> waiting(MAXA);

	auto brute = [&](int from, int val) {
		for (int i = from; i < MAXA && !done[i][val]; i++) {
			done[i][val] = true;
			waiting[i].emplace_back(val);
		}
	};
	brute(0, 0);
