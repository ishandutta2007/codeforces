...
{   //*
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (x == 'C')
        {
            if (p[i] <= c)
                ac = max(ac, b[i]);
            cc.push_back(make_pair(p[i], b[i]));
        }
        else
        {
            if (p[i] <= d)
                ad = max(ad, b[i]);
            dd.push_back(make_pair(p[i], b[i]));
        }
    }
    int ans = ac + ad;
    if (ac == -1 || ad == -1)
    ...
    for (int i = 1; i < cc.size(); i++)
        mx[i] = max(mx[i - 1], cc[i].second);
    for (int i = 1; i < cc.size(); i++)
    {
        int f = cc[i].first;
        int s = c - f;
        int t = upper_bound(cc.begin(), cc.end(), make_pair(s, BIG)) - cc.begin() - 1;
        if (t < 0)
            continue;
        if (t >= i)
            t = i - 1;
        ans = max(ans, cc[i].second + mx[t]);
    }
    mx.resize(dd.size());
    if (!dd.empty())
        mx[0] = dd[0].second;
    for (int i = 1; i < dd.size(); i++)
        mx[i] = max(mx[i - 1], dd[i].second);
    for (int i = 1; i < dd.size(); i++)
    {
        int f = dd[i].first;
        int s = d - f;
        int t = upper_bound(dd.begin(), dd.end(), make_pair(s, BIG)) - dd.begin() - 1;
        if (t < 0)
            continue;
        if (t >= i)
            t = i - 1;
        ans = max(ans, dd[i].second + mx[t]);
    }
    cout << ans;
    return 0;
}