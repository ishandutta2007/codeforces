...
const int MN=5e3+5; 
int n,a[MN],f[MN],s[MN][MN]; 
bool g[MN][MN]; 
void works() {
	n=read(); For(i,1,n) For(j,1,n) s[i][j]=0;  For(i,1,n) a[i]=read(),s[a[i]][i]++; 
	For(i,1,n+1) f[i]=-INF;  f[0]=0;  
	For(i,1,n) For(j,1,n) s[i][j]+=s[i][j-1]; 
	g[1][0]=1; 
	foR(i,n,1) {
		int lst=a[i]; g[i+1][i]=1; 
		For(j,i,n) {
			int x=s[lst][j]-s[lst][i-1],y=s[a[j]][j]-s[a[j]][i-1]; 
			lst=x>y?lst:a[j]; 
			g[i][j]=(max(x,y)*2<=(j-i+1))&&((j-i+1)%2==0); 
		}
	}
	For(i,1,n+1) {
		For(j,0,i-1) {
			if((i==n+1||!j||a[i]==a[j])&&g[j+1][i-1]) ckmax(f[i],f[j]+(i!=n+1)); 
		}
	}
	cout<<f[n+1]<<endl; 
}
