...
inline void AC() {
    ...
    memset(l, -4, sizeof l);
    for(_ i = 1; i <= n*k; ++i) {
        if(l[a[i]] > 0) g.push_back((rg){l[a[i]], i, a[i]});
        l[a[i]] = i;
    }
    for(rg i : g) if(!d[i.id]) {
        bool ok = true;
        for(_ j = i.x; j <= i.y; ++j) if(b[j] >= (n-1)/(k-1)+1) {
            ok = false;
            break;
        }
        if(ok) {
            o[i.id] = mp(i.x, i.y);
            d.set(i.id);
            for(_ j = i.x; j <= i.y; ++j) ++b[j];
        }
    }
    for(_ i = 1; i <= n; ++i) {
        writeln(o[i]);
        if(!o[i].first || !o[i].second) exit(1);
    }
