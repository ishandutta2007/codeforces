...
//Do someting
vector<int>v[1000001],qwq[1000001];
#endif
...
int n;
cin>>n;
vector<int>cnt(1000001),a(n+1),l1(n+1),r1(n+1),l2(n+1),r2(n+1);
ff(x,1,1000000)
v[x].clear();
ff(x,1,n)
cin>>a[x],v[a[x]].push_back(x);
stack<int>st;
ff(x,1,n)
{
	while(!st.empty()&&a[st.top()]<a[x])st.pop();
	if(!st.empty())l1[x]=st.top();
	st.push(x);
}
while(!st.empty())
st.pop();
...
for(int x=1;x<=n;x++)
{
	for(auto y:qwq[a[x]])
	{
		if(cnt[y]){
			int z=v[y][cnt[y]-1];
			if(z>l1[x]&&r2[z]>x)
			ans+=(z-max(l1[x],l2[z]))*(min(r1[x],r2[z])-x);
			//cerr<<x<<','<<y<<','<<1<<','<<ans<<endl;
		}
		if(cnt[y]<v[y].size())
		{
			int z=v[y][cnt[y]];
			if(z<r1[x]&&l2[z]<x)
			ans+=(x-max({l1[x],l2[z],cnt[y]?v[y][cnt[y]-1]:0}))*(min(r1[x],r2[z])-z);
			//cerr<<x<<','<<y<<','<<2<<','<<ans<<endl;
		}
	}
	cnt[a[x]]++;
