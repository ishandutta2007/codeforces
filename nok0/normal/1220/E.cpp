...
void main_() {
	...
	two_edge_connected_components cc(G, true);
	int root = cc[s];
	auto H = cc.graph_compressed.root_to_leaf(root);
	...
	REP(i, nn) {
		if(i == root) continue;
		if(SZ(cc.groups[i]) <= ecnt[i]) {
			loop[i] = 1;
			tmp++;
		}
	}

	auto vec = H.subtree_size(root);
	auto dfs = [&](auto dfs, int now) -> ll {
		ll ret = 0;
		for(auto [to, cost] : H[now])
			chmax(ret, dfs(dfs, to));
		ret += nw[now];
		return ret;
	};
	ll base = 0;
	...
	REP(i, nn) {
		if(!vec[i]) continue;
		for(auto [to, cost] : H[i])
			if(!vec[to]) chmax(res, dfs(dfs, to));
	}
	print(res + base);
	return;
}
