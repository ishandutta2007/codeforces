...
bool solve() {
  ...
  int cx[2]{};
  for (auto& c: s) ++cx[c -= 'A'];
  if (x[0] + xx[0] + xx[1] != cx[0] || x[1] + xx[0] + xx[1] != cx[1]) {
    return 0;
  }
  int cany = 0;
  vector<int> yy[2];
  for (int i = 0, last = 0; i <= s.size(); ++i) {
    if (i == s.size() || s[i] == s[i - 1]) {
      if (int hl = i - last >> 1; i + last & 1) {
        cany += hl;
      } else {
        yy[s[last]].push_back(hl);
        xx[s[last]] -= hl;
      }
      last = i;
    }
  ...
  for (auto c: {0, 1}) {
    sort(yy[c].begin(), yy[c].end());
  }
  while (unz(xx[0]) + unz(xx[1]) > cany) {
    bool which = xx[0] > 0;
    if (yy[which].empty()) {
      return 0;
    }
    xx[which] += yy[which].back();
    cany += yy[which].back() - 1;
    yy[which].pop_back();
  }
  return 1;
}
