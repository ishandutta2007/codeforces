...
void solve() {
    ...
    b.rsz(n+1); c.rsz(n+1);
    b[0]=0;
    FOR(i, 1, n+1) if (b[i-1]==-1 || a[i-1]<b[i-1]) b[i]=-1; else b[i]=a[i-1]-b[i-1];
    c[n]=0;
    R0F(i, n) if (c[i+1]==-1 || a[i]<c[i+1]) c[i]=-1; else c[i]=a[i]-c[i+1];
    if (c[0]==0) {YES; return;}
    F0R(i, n-1) {
        if (b[i] != -1 && c[i+2] != -1 && works(b[i], a[i+1], a[i], c[i+2])) {YES; return;}
    }
    NO;
}
