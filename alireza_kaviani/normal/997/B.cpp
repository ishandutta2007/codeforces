...
const ll INF = 8e18;
const ll MOD = 50006393431LL; // 998244353; // 1e9 + 9;
...
mll<> c(mll<> n, mll<> r) {
    if (r.val > n.val) return 0;
    mll<> ans = 1;
    for (ll i = 0; i < r.val; i++) ans *= (n - i);
    for (ll i = 0; i < r.val; i++) ans /= (i + 1);
    return ans;
}
...
mll<> get(ll n, ll m = 4) {
    ...
    if (n < 0) return 0;
    mll<> ans;
    for (ll i = 0; i < m; i++) ans += c(n - 1, i) * c(m, i + 1);
    return ans;
}
...
int main() {
    ...
    cin >> n;
    mll<> ans = get(n);
    ans -= get(n - 6) + get(n - 9) + get(n - 9);
    ans += get(n - 14) + get(n - 15) + get(n - 10);
    ans -= get(n - 15);
    cout << ans << endl;
