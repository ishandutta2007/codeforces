...
void solve(int n) {
	...
	auto cut_child = [&](auto self, int par, int chd) -> void {
		cut_edge(par, chd);
		if(reserved[chd] == -1) return;
		self(self, chd, reserved[chd]);
		reserved[chd] = -1;
		return;
	};
	...
	auto dfs = [&](auto self, int now, int par) -> int {
		...
		foa(e, g[now]) {
			...
			int under = self(self, e.to, now);
			if(under == ng) return ng;
			child[under].push_back(e.to);
		}
		while(child[deg[now]].size()) {
			debug(now, child, child[deg[now]]);
			const int predeg = deg[now];
			cut_child(cut_child, now, child[deg[now]].back());
			child[predeg].pop_back();
		}
		debug(now, par, child);
		const int zs = (int)child[0].size();
		const int os = (int)child[1].size();
		debug(zs, os);
		if(zs + os > 1) return ng;
		debug(deg, now);
		if(zs + os == 0) return deg[now];
		reserved[now] = child[zs ? 0 : 1].front();
		return deg[now];
	};

	int res = dfs(dfs, 0, -1);
	debug(res);
	rep(i, n) if(reserved[i] != -1) res = 1;
	if(res) {
		YES(false);
		return;
	}
