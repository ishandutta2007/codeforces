...
signed main(){
    ...
    rep2(i,1,n+1) cnt[i]=cnt[i-1]+Modint<MOD>((i-1)/(k-1));
    rep(n) ans+=Modint<MOD>(a[i])*(cnt[c[i]-b[i]-1]-cnt[c[i]-i-1]-cnt[i-b[i]-1]);
    print1(ans);
    return 0;
}