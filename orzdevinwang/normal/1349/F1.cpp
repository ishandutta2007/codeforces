...
int main() {
	...
	cin >> n, init(n * 2 + 3);
	poly Q(n + 3), ans(n + 1), t(n + 1);
//	G[0] = 1, G[1] = 1, G = G.ln();
	L(i, 2, n + 4) (Q[i - 2] += mod - ifac[i]) %= mod;
	L(i, 1, n + 3) (Q[i - 1] += ifac[i]) %= mod;
	Q = Q.inv();
//	L(i, 0, n * 2 + 2) Z[i] = fac[i];
//	L(i, 0, n + 2) eQ[i] = (ll) Q[n + 2 - i] * ifac[i] % mod;
//	eQ = eQ * Z;
//	L(i, 0, n) ans[i] = (ll) eQ[n + 2 + i] * ifac[i] % mod;
	L(i, 0, n * 2 + 2) L(j, 0, min(i, n + 2)) if(i - j <= n) (ans[i - j] += (ll) Q.v(n + 2 - j) * C(i, j) % mod) %= mod;
	L(i, 0, n) {
		int now = 1;
		L(j, 0, n + 2) (ans[i] += mod - (ll) Q[n + 2 - j] * now % mod) %= mod, now = (ll) now * (i + 1) % mod * inv[j + 1] % mod;
	}
//	cout << ans[0] << " " << ans[1] << "\n";
//	cout << (ll) ans[0] * fac[n] % mod << "\n";
	L(i, 0, n) {
