...
void solve(int l, int r)
{
    vector<char> capcha;
    for (int i = l; i <= r; i++)
        capcha.push_back(s[i]);
    vector<char> maxx;
    for (int mask = 0; mask < (1 << ((int)capcha.size())); mask++)
    {
        vector<char> pali;
        for (int i = 0; i < capcha.size(); i++)
        {
            if ((1 << i) & mask)
                pali.push_back(capcha[i]);
        }
        if (pali.size() > maxx.size() && is_pali(pali))
            maxx = pali;
    }
