...
int main(){
	...
	rep(i,1,m){
		*upper_bound(f,f+m+1,a[i])=a[i];
		g[i]=lower_bound(f,f+m+1,0x3f3f3f3f)-f;
		debug("%d%c",g[i]," \n"[i==n]);
	}
	memset(f,0x3f,sizeof f);
	per(i,1,m){
		*upper_bound(f,f+m+1,a[i])=a[i];
		gg[i]=lower_bound(f,f+m+1,0x3f3f3f3f)-f;
	}
	int res=0;
	rep(i,1,m+1)upmx(res,g[i-1]+gg[i]);
	out,res,'\n';
}