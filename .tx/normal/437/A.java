...
public class Main extends PrintWriter {
    ...
    boolean f(int l1, int l2, int l3, int l4) {
        if (l1 >= 2 * max(l2, max(l3, l4))) {
            return true;
        }
        return 2 * l1 <= min(l2, min(l3, l4));
    }

    void solve() throws IOException {
        int l1 = next().length() - 2;
        int l2 = next().length() - 2;
        int l3 = next().length() - 2;
        int l4 = next().length() - 2;
