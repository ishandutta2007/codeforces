...
void solve() {
  ...
  mint base = RNG(0, 1LL << 61);

  auto get = [&](vi A) -> vc<mint> {
    vi st;
    vc<mint> res = {0};
    vc<mint> cum = {0};
    for (auto&& a: A) {
  ...
  while (len(que)) {
    ...
    // [L,R)  M 
    vi left, right;
    FOR3_R(i, L, M) left.eb(A[i]);
    FOR3(i, M, R) right.eb(A[i]);
    auto XL = get(left), XR = get(right);
    XR.erase(XR.begin());
    sort(all(XL));
    for (auto&& v: XR) ANS += UB(XL, v) - LB(XL, v);
  }
  print(ANS);
}
