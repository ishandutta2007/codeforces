...
int main() {
    ...
    vector<vector<int>> divs(2 * M + 1);
    rep(d, 1, 2 * M + 1) rep(m, d, 2 * M + 1, d) {
        divs[m].push_back(d);
    }
    ...
    vector<int> cnt(M + 1);
    atcoder::fenwick_tree<long long> ft(M + 1);

    auto add = [&](int i, int v) {
        long long pc = cnt[i];
        long long cc = cnt[i] += v;
        long long df = (cc * (cc - 1) - pc * (pc - 1)) >> 1;
        ft.add(i, df);
    };
    rep(d, 1, M + 1) rep(m, 2 * d, M + 1, d) {
        add(m, 1);
    }
