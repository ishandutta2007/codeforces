...
int main(){
	...
	auto __solve_tc = [&](int __tc_num){
		...
		y_combinator([&](auto self, int u)->void{
			int recover = -1;
			if(auto it = s.lower_bound({df.pos[u], -1, -1}); it != s.begin() && df.end[u] <= (*prev(it))[1]){
				recover = (*prev(it))[2];
				s.erase(prev(it));
			}
			s.insert({df.pos[u], df.end[u], u});
			ctmax(res, (int)s.size());
			for(auto id: g.adj[u]){
				auto &e = g.edges[id];
				int v = u ^ e.from ^ e.to;
				self(v);
			}
			s.erase({df.pos[u], df.end[u], u});
			if(~recover){
				s.insert({df.pos[recover], df.end[recover], recover});
			}
		})(0);
		cout << res << "\n";
		return 0;
