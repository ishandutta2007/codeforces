...
//Z-algorithm
// index 
vector<ll> z_algo(string &U){
    ll N = U.size();
    vector<ll> A(N,0);
    A[0] = U.size();
    int i = 1, j = 0;
    while (i < U.size()) {
    while (i+j < U.size() && U[j] == U[i+j]) ++j;
        A[i] = j;
        if (j == 0) { ++i; continue;}
        int k = 1;
        while (i+k < U.size() && k+A[k] < j) A[i+k] = A[k], ++k;
        i += k; j -= k;
    }
    return A;
}
...
int main(){
    ...
    rep(ii,Q){
        ...
        rep(i,T.size()){
            ...
            if(idx.count(U)){
                aa.emplace_back(N+i+1-idx[U]);
            }
            else{
                rep(j,U.size()+1){
                    bool ok = true;
                    for(ll k=j;k<U.size();k++){
                        if(S[k-j]!=U[k]) ok = false;
                    }
                    if(ok){
                        aa.emplace_back(U.size()-j);
                        break;
                    }
                }
            }
        }
        rep(i,T.size()) S.pop_back();
        out(aa);
    }
    //cout << ans << endl;
}