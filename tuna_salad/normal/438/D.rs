...
mod simple_full_segtree {
	...
	impl Tree {
		...
		pub fn update_one(&mut self, p: usize, u: U) {
			assert!(p < self.n);
			assert!(u.0 == 3);
		...
		pub fn update(&mut self, ql: usize, qr: usize, u: U) {
			assert!(ql <= qr && qr < self.n);
			assert!(u.0 == 2);
			self.update_sub(ql, qr, 0, self.base-1, 1, u);
		}
		pub fn update_sub(&mut self, ql: usize, qr: usize, l: usize, r: usize, id: usize, u: U) {
			if r < ql || qr < l { return; }
			if self.values[id].max >= u.1 as i64 {
				if l == r {
					self.values[id].update(u);
				} else {
					let m = (l+r) >> 1;
					self.update_sub(ql, qr, l, m, id+id, u);
					self.update_sub(ql, qr, m+1, r, id+id+1, u);
					self.values[id] = Val::combine(&self.values[id+id], &self.values[id+id+1]);
				}
			}
		}
...
fn main() {
	...
	l!(n,m = rin.u());
	let a = rin.vi(n);
	let mut st = simple_full_segtree::Tree::new(&a);
	rep!{[m]
		let t = rin.u() as u8;
		if t == 3 {
			let k = rin.u()-1;
