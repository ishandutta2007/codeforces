...
int main() {
    ...
    degs[0] = 1;
    for (int i = 1; i<N; i++) degs[i] = mul(degs[i-1], k-2);

    vector<int> a(n);
    for (int i = 0; i<n; i++) cin>>a[i];
    int eq = 0;
    for (int i = 0; i<n; i++) if (a[i]==a[(i+1)%n]) eq++;
    int dif = n-eq;

    //cout<<eq<<' '<<dif<<endl;

    int total = po(k, dif);
    for (int i = 0; 2*i<=dif; i++)
    {
        int bad = mul(C(dif, 2*i), C(2*i, i));
        bad = mul(bad, degs[dif - 2*i]);
        //cout<<bad<<endl;
        total = sub(total, bad);
    }
    //cout<<total<<endl;
    total = mul(total, inv(2));
    //cout<<total<<endl;
    cout<<mul(total, po(k, eq));
