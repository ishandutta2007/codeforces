...
long long cnt_inversions(const vector<int>& perm){
	int n = perm.size();
	static bit<long long> T;
	T.init(n);
...
void solve(){
	...
	vector<int> prv(n, -1);
	for(int k = 0; k < 26; ++k){
		assert(pos[k].size() == pos2[k].size());
		if (pos[k].size() == 0){
			continue;
		}
		reverse(all(pos2[k]));
		for(int z = 0; z < pos[k].size(); ++z){
			int a = pos[k][z], b = pos2[k][z];
			prv[b] = a;
			
		}
	}
	cout << cnt_inversions(prv) << endl;
	// cout << (ans)/2 << endl;
