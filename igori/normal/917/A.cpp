...
struct min_queue{
    struct min_stack{
        ...
        void push(int x)
        {
            if (st.size() == 0) st.push_back({x, x});
            else st.push_back({x, min(x, st.back().second)});
        }
        void pop()
        {
            st.pop_back();
    ...
    {
        int ans = INF;
        if (le.st.size()) ans = min(ans, le.get());
        if (ri.st.size()) ans = min(ans, ri.get());
        return ans;
    }
};
...
void solve()
{
    string s;
    cin >> s;
    vector<bitset<5000> > bad_pref(s.size());
    vector<bitset<5000> > bad_suff(s.size());
    int n = s.size();
    for (int i = 0; i < n; i++)
    {
        int bal = 0, mn = 0;
