// Chopping Carrots (Hard Version) (https://codeforces.com/contest/1706/problem/D2)
...
struct Generator {
    ...
    explicit Generator(int a_) : a(a_), s(int(sqrt(a_))) {}
    ...
    bool done() const { return phase && !i; }

    void step() {
        if (phase)
            --i;
        else if (++i > s)
            phase = true, i = (s * s == a ? s - 1 : s);
    }
};
...
int main() {
    ...
    while (t--) {
        ...
        int mn = INF, mx = 0;
        rep(i, n) {
            cur[i] = gen[i].get().first;
            mn = min(mn, cur[i]);
            mx = max(mx, cur[i]);
            cnt[cur[i]]++;
            gen[i].step();
            if (!gen[i].done()) {
                int val = gen[i].get().first;
                nxt[i] = q[val];
                q[val] = i;
            }
        }
