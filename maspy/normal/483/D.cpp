...
#line 1 "/home/maspy/compro/library/alg/monoid_or.hpp"
template <typename E>
struct Monoid_Or {
  using value_type = E;
  using X = value_type;
...
#line 1 "/home/maspy/compro/library/alg/monoid_and.hpp"
template <typename E>
struct Monoid_And {
  using value_type = E;
  using X = value_type;
...
void solve() {
  ...
  VEC(QT, query, Q);
  for (auto&& [l, r, x]: query) --l;
  DualSegTree<Monoid_Or<int>> dseg(N);
  for (auto&& [l, r, x]: query) { dseg.apply(l, r, x); }
  auto A = dseg.get_all();

  SegTree<Monoid_And<int>> seg(A);
  for (auto&& [l, r, x]: query) {
    if (seg.prod(l, r) != x) return NO();
  }
  YES();
  print(A);
}
