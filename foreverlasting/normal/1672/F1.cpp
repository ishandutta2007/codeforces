...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        for(res i=1;i<=n;i++)A[a[i]=read()].pb(i);
        sort(A+1,A+n+1,[&](const auto &x,const auto &y){return x.size()>y.size();});
        res tot=n;
        while(tot){
            res pre=0,st=0;
            for(res i=1;i<=n;i++){
                if(A[i].empty())break;
                if(pre)b[A[i].back()]=a[pre];
                else st=A[i].back();
                pre=A[i].back(),A[i].pop_back(),tot--;
            }
            b[st]=a[pre];
        }
        for(res i=1;i<=n;i++)printf("%d ",b[i]);puts("");
    }
}
int main(){
//    srand(time(0));
