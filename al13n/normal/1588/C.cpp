...
struct segtree{
  vector<array<pll,2>> T;
  
  segtree(int n):T(n*4){}

  void init(int a,int b,int k,vector<LL>&A){
    if(b==a+1){
      T[k]=array<pll,2>{mp(LINF,-LINF),mp(LINF,-LINF)};
      T[k][a%2]=mp(A[a],A[a]);
      return;
    }
    int c=(a+b)/2;
    init(a,c,k*2+1,A);
    init(c,b,k*2+2,A);
    T[k]={mp(min(T[k*2+1][0].X,T[k*2+2][0].X),max(T[k*2+1][0].Y,T[k*2+2][0].Y)),
          mp(min(T[k*2+1][1].X,T[k*2+2][1].X),max(T[k*2+1][1].Y,T[k*2+2][1].Y))};
  }
  array<pll,2> get(int l,int r,int a,int b,int k){
    if(l>=b||r<=a) return {mp(LINF,-LINF),mp(LINF,-LINF)};
    if(l<=a&&r>=b) return T[k];
    int c=(a+b)/2;
    auto t1=get(l,r,a,c,k*2+1),t2=get(l,r,c,b,k*2+2);
    return {mp(min(t1[0].X,t2[0].X),max(t1[0].Y,t2[0].Y)),
            mp(min(t1[1].X,t2[1].X),max(t1[1].Y,t2[1].Y))};
  }
  int find(int l,LL a0,int a,int b,int k){
    if(l>=b) return b;
    if(T[k][0].Y-a0<=0 && T[k][1].X-a0>=0) return b;
    if(b==a+1) return a;
    int c=(a+b)/2;
    int r=find(l,a0,a,c,k*2+1);
    if(r!=c) return r;
    return find(l,a0,c,b,k*2+2);
  }
};
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    LL ans=0;
    for(int i=0;i<n;++i){
      int ii=T.find(i,A[i],0,n+1,0);
      assert(ii>i);
      auto& pp=P[A[i]];
      ans+=lower_bound(all(pp),ii)-upper_bound(all(pp),i);
    }
    cout<<ans<<'\n';
  }
