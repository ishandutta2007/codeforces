...
fun main(){
    solve.cases{
        ...
        val inv2 = (2).inverse()
        val i2p = IntArray(n+1)
        i2p[0] = 1
        for(i in 1..n){
            i2p[i] = i2p[i-1] mm inv2
        }
        val p2 = IntArray(n+1)
        p2[0] = 1
        for(i in 1..n){
        ...
        val DP = IntArray(n+1)
        val suffixSum = IntArray(n+1) // sum over DP[i]/ 2^i
        DP[n] = 1
        suffixSum[n] = i2p[n]
        for(i in n-1 downTo 1){
            val f = p2[i]
            val start = i + 1
            val end = minOf(2 * i -1,n)
            var have = suffixSum[start]
            if(end != n){
                have = have ms suffixSum[end+1]
            }
            DP[i] = DP[i] mp (have mm f)
//            for(j in i+1..){
//                val dif = j
//                var odds = intPow(inv2,dif,pI)
//                DP[i] = DP[i] mp (odds mm DP[j] mm f)
//            }
            if(n in i+1..minOf(2 * i-1,n)){
                val dif = n - i
                DP[i] = DP[i] mp( i2p[dif] mm DP[n])
            }
            suffixSum[i] = suffixSum[i+1] mp (DP[i] mm i2p[i])
        }
        val start = IntArray(n+1)
        for(i in 1..n){
            val needsize = (i+1)/2
            start[i] = intPow(inv2, needsize -1 ,pI)
        }
        ...
        for(i in 1..n){
            put(start[i] mm DP[i] mm penalty[i])
        }
