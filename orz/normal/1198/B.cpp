...
struct st
{
	...
	vl v;
	vl m;
	st(const vl& u) : n(sz(u)), m(n * 4, C), v(n * 4, C)
	{

	}
	void ssst(const int& ver, const int& vl, const int& vr, const vector<ll>& u)
	{
		if (vr - vl <= 1)
			v[ver] = u[vl];
		else
		{
			int mid = (vl + vr) / 2;
			ssst(2 * ver + 1, vl, mid, u);
			ssst(2 * ver + 2, mid, vr, u);
		}
	}
	void sst(const vl& u)
	{
	...
	void modify(const int& p, const ll& x)
	{
		modi(0, 0, n, p, x);
	}
	void push(const int& ver, const int& vl, const int& vr)
	{
		if (vr - vl <= 1)
			v[ver] = max(v[ver], m[ver]);
		else
		{
			m[2 * ver + 1] = max(m[2 * ver + 1], m[ver]);
			m[2 * ver + 2] = max(m[2 * ver + 2], m[ver]);
		}
		m[ver] = C;
	}
	void modi(const int& ver, const int& vl, const int& vr, const int& p, const ll& x)
	{
		push(ver, vl, vr);
		if (vr - vl <= 1)
			v[ver] = x;
		else
		{
			int mid = (vl + vr) / 2;
			if (p < mid)
				modi(2 * ver + 1, vl, mid, p, x);
			else
				modi(2 * ver + 2, mid, vr, p, x);
		}
	}
	void outt(const int& ver, const int& vl, const int& vr, vector<ll>& ans)
	{
		push(ver, vl, vr);
		if (vr - vl <= 1)
			ans.pb(v[ver]);
		...
		{
			int mid = (vl + vr) / 2;
			outt(2 * ver + 1, vl, mid, ans);
			outt(2 * ver + 2, mid, vr, ans);
		}
	}
	void out(vl& ans)
	{
