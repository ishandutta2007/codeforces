...
int main() {
    ...
    const int ML = 3601, MP = 1801;
    vvmn ways(ML, vmn(MP));
    ways[0][0] = 1;
    FOR (l, 1, ML) F0R (p, MP) {
        ways[l][p] = ways[l-1][p];
        if (l >= 2 && p >= 1) ways[l][p] += ways[l-2][p-1];
    }
    auto p2 = [&](vb used) {
        int MAX = sz(used) / 2 + 1;
        vmn dp(MAX); dp[0] = 1;
        for (int i = 0, j = 0; i < sz(used); i = j) {
            while (j < sz(used) && used[j] == used[i]) j++;
            if (!used[i]) {
                int len = j - i;
                F0Rd (tp, sz(dp)) FOR (hp, 1, len/2 + 1)
                    if (tp+hp < sz(dp))
                        dp[tp+hp] += dp[tp] * ways[len][hp];
            }
        }
        return dp;
    };

    int ER = H - accumulate(all(row), 0); vmn rdp = p2(row);
    int EC = W - accumulate(all(col), 0); vmn cdp = p2(col);

    F0R (i, sz(rdp)) rdp[i] *= mn::fact(i);
    F0R (i, sz(cdp)) cdp[i] *= mn::fact(i);
    ...
    F0R (ru2, sz(rdp)) {
        ...
        F0R (cu2, sz(cdp)) {
            int ec = EC - 2 * cu2;
            if (ec < ru2 || er < cu2) continue;
            ans += rdp[ru2] * cdp[cu2] * mn::ncr(er, cu2) * mn::ncr(ec, ru2);
        }
    }
    ps(ans);
