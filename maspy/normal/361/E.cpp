...
void solve() {
  ...
  FOR_R(i, N) {
    ...
    FOR(k, MAX) {
      DPS[i][k] += DPS_sum[k];
      ll nk = k + (N - i);
      if (nk < MAX) DPL[i][nk] += DPS_sum[k];
    }

    FOR3(j, i + 1, N) {
      if ((j - i - 1) * (N - j) >= MAX) continue;
      FOR(k, MAX) {
        ll nk = k + (j - i - 1) * (N - j);
        if (nk < MAX) DPS[i][nk] += DPL[j][k];
        nk = k + (N - i) + (j - i - 1) * (N - j);
        if (nk < MAX) DPL[i][nk] += DPL[j][k];
      }
    }

    FOR(k, MAX) DPS[i][k] *= A[i];
    FOR(k, MAX) DPL[i][k] *= 25 - A[i];
    FOR(k, MAX) DPS_sum[k] += DPS[i][k];
  }
