...
const int nax = 1010;
bool can[nax][nax * 2];
vpii poss = {{3, 3}, {4, 5}, {4, 6}, {5, 5}, {6, 7}, {7, 7}};
bool pri[nax * 2];
int main() {
	memset(pri, 1, sizeof(pri));
	pri[0] = pri[1] = false;
	FOR(p, 2, nax - 1) {
		if (pri[p]) for (int x = 2 * p; x < nax; x += p) pri[x] = false;
	}
	int n;
	scanf("%d", &n);
	can[0][0] = 1;
	REP(i, nax) REP(j, 2 * nax) if (can[i][j]) {
		for (pii x : poss) if (i + x.first < nax && j + x.second < 2 * nax)
			can[i + x.first][j + x.second] = 1;
	}
	FOR(i, 3, 1000) {
		bool sol = false;
		FOR(j, 1, nax * 2 - 1) if (pri[j] && can[i][j]) sol = true;
		if (!sol) {
			debug("err for", imie(i));
		}
	}
	vpii ans;
	int e = -1;
	FOR(i, 1, nax * 2 - 1) if (pri[i] && can[n][i]) {
		e = i;
		break;
	}
	assert (e != -1);
	...
	while (nn) {
		bool did = false;
		for (pii c : poss) {
			if (nn - c.first >= 0 && e - c.second >= 0 && can[nn - c.first][e - c.second]) {
				ans.push_back(c);
				nn -= c.first;
				e -= c.second;
				did = true;
...
//vpii poss = {{3, 3}, {4, 5}, {4, 6}, {5, 5}, {6, 7}, {7, 7}};
		if (c == pii{3, 3}) {
			...
			edges.emplace_back(z, x);
		}
		else if (c == pii{4, 5}) {
			vector <int> v;
			REP(_, 4) v.push_back(num++);
			REP(i, 4) REP(j, i) if (i != 1 || j != 0) edges.emplace_back(v[i], v[j]);
		}
		else if (c == pii{4, 6}) {
			vector <int> v;
			REP(_, 4) v.push_back(num++);
			REP(i, 4) REP(j, i) edges.emplace_back(v[i], v[j]);
		}
		else if (c == pii{5, 5}) {
			vector <int> v;
			REP(_, 5) v.push_back(num++);
			REP(i, 5) edges.emplace_back(v[i], v[(i + 1) % 5]);
		}
		else if (c == pii{6, 7}) {
			vector <int> v;
			REP(_, 6) v.push_back(num++);
			REP(i, 6) edges.emplace_back(v[i], v[(i + 1) % 6]);
			edges.emplace_back(v[0], v[3]);
		}
		else if (c == pii{7, 7}) {
			vector <int> v;
			REP(_, 7) v.push_back(num++);
			REP(i, 7) edges.emplace_back(v[i], v[(i + 1) % 7]);
	...
	FOR(i, 1, n) assert(pri[deg[i]]);
	printf("%d\n", m);
	for (pii x : edges) printf("%d %d\n", FS(x));
	return 0;
}