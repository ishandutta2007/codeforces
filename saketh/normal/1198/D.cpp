...
int calc(int i1, int i2, int j1, int j2) {
    int& v = dp[i1][i2][j1][j2];
    if (v != 0x7f7f7f7f) return v;

    if (i1 >= i2 || j1 >= j2) return v = 0;

    if (j2 - j1 >= i2 - i1) {
        v = j2 - j1;
        FOR (je, j1, j2) if (get_sum(i1, i2, je, je+1) == 0) {
            ckmin(v, calc(i1, i2, j1, je) + calc(i1, i2, je+1, j2));
        }
    } else {
        v = i2 - i1;
        FOR (ie, i1, i2) if (get_sum(ie, ie+1, j1, j2) == 0) {
            ckmin(v, calc(i1, ie, j1, j2) + calc(ie+1, i2, j1, j2));
        }
    }
    return v;
}
...
int main() {
    ...
    int N; re(N); vs grid(N); re(grid);

    F0R (i, N) F0R (j, N) {
        pref[i+1][j+1] = pref[i+1][j] + pref[i][j+1] - pref[i][j] + int(grid[i][j] == '#');
    }
