...
//vector<pii> rb[2];
//tri a[16];
//ll encode(int mask, int dif) {
//    return (ll(dif + INF) << 16) + mask;
//}
//int n;
//int fmask;
//int states = 0;
//int go(int mask, int dif) { // dif = r - b
//    if (mask == fmask) return 0;
//    auto& p = m[encode(mask, dif)];
//    if (p != 0) {
//        return p - 1;
//    }
//    ++states;
...
//    for (int i = 0; i < n; ++i) {
//        if (!checkbit(mask, i)) {
//            int x = max(a[i].b - d[0], 0);
//            int y = max(a[i].c - d[1], 0);
//            int days = max(x - r, y - b);
//            if (days < 0) {
//                days = 0;
//            }
//            int toR = r + days;
//            int toB = b + days;
//            int cur = days + go(mask | (1 << i), toR - x - toB + y);
////            DBN(cur, toR, toB);
//            res = min(res, cur);
//        }
//    }
