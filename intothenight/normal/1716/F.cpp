...
int main(){
	...
	const int mxk = 2000;
	vector<vector<modular>> dp(mxk + 1, vector<modular>(mxk + 1));
	dp[0][0] = 1;
	for(auto x = 0; x < mxk; ++ x){
		for(auto y = 0; y <= x; ++ y){
			dp[x + 1][y] += dp[x][y] * y;
			dp[x + 1][y + 1] += dp[x][y];
		}
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		...
		for(auto i = 0; i <= k; ++ i){
			res += dp[k][i] * pref * (modular(m) ^ n - i);
			pref *= modular(n - i) * (m + 1 >> 1);
		}
		cout << res << "\n";
		return 0;
	};
...
sum(i=0~n) i^k * C(n,i) * a^i * b^(n-i)

(b+aX)^n = sum(i=0~n) C(n,i) * a^i * X^i * b^(n-i)
*/
