...
int main() {
    ...
    V<int> cnt(D);
    V<ll> tbl0(B);
    atcoder::fenwick_tree<ll> c0(D), c1(D);
    ll sum = 0;
    for (int ph = 0; ph < n; ph++) {
        ...
        
        {
            ...
            if (a < B) {
                ...
                sum += c1.sum(0, a);
                for (int f = 1; f * a < D; f++) {
                    int lw = f * a, up = min(D, (f + 1) * a);
                    sum += c1.sum(lw, up) - 1LL * f * a * c0.sum(lw, up);                
                }
            }
        }
        {
            sum += 1LL * a * c0.sum(a + 1, D);
            // a % x
            for (int r = 1; r < B; r++) {
                // a / x == r
                // x * r <= a && a < x * (r + 1)
                int lw = a / (r + 1) + 1, up = a / r + 1;
                sum += 1LL * a * c0.sum(lw, up) - 1LL * r * c1.sum(lw, up);
            }
            for (int i = 1; i <= a / B; i++) {
                sum += 1LL * (a % i) * cnt[i];
            }
