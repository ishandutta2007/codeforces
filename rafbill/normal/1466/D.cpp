...
void solve(){
  int n; cin>>n;
  vi W(n); for(li i = 0; i < (li)(n); ++i) cin>>W[i];
  vvi G(n);
  ...
  vi I;
  for(li i = 0; i < (li)(n); ++i) for(li k = 0; k < (li)(G[i].size()-1); ++k) I.push_back(i);
  sort(begin(I), end(I), [&](int i, int j){return W[i]>W[j];});
  li cur = accumulate(begin(W), end(W),0ll);
  cout << cur << ' ';
  for(int i : I) {
    cur += W[i];
    cout << cur << ' ';
