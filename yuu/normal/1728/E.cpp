...
int n;
int64_t base_a;
int64_t change_a[300001];
constexpr int SMALL_LIMIT = 500;
int64_t precal[SMALL_LIMIT + 1][SMALL_LIMIT + 1];
vector<int> divs[300001];
void do_precal(const int& x) {
    fill(precal[x], precal[x] + SMALL_LIMIT + 1, -1);
    for (int use_x = 0; use_x <= n; use_x += x) {
        for (int possible_y : divs[n - use_x]) {
            precal[x][possible_y] = max(precal[x][possible_y], base_a + change_a[use_x]);
        }
    }
}

void bruteforce_x(int x, int y) {
    int64_t ans = -1;
    for (int use_x = 0; use_x <= n; use_x += x) {
        if ((n - use_x) % y) continue;
        ans = max(ans, base_a + change_a[use_x]);
    }
    cout << ans << '\n';
}

void bruteforce_y(int x, int y) {
    int64_t ans = -1;
    for (int use_y = 0; use_y <= n; use_y += y) {
        if ((n - use_y) % x) continue;
        ans = max(ans, base_a + change_a[n - use_y]);
    }
    cout << ans << '\n';
}
...
void solve() {
    ...
    sort(change_a + 1, change_a + n + 1);
    reverse(change_a + 1, change_a + n + 1);
    for (int i = 1; i <= n; i++) change_a[i] += change_a[i - 1];

    auto start = clock();
    for (int i = 1; i <= SMALL_LIMIT; i++) {
        for (int j = 0; j <= n; j += i) divs[j].emplace_back(i);
    }

    for (int i = 1; i <= SMALL_LIMIT; i++) do_precal(i);
    cerr << "Precal in: " << static_cast<double>(clock() - start) / CLOCKS_PER_SEC << '\n';
    int m;
    cin >> m;
    for (int i = 1, x, y; i <= m; i++) {
        cin >> x >> y;
        if (x > SMALL_LIMIT) bruteforce_x(x, y);
        else if (y > SMALL_LIMIT) bruteforce_y(x, y);
        else cout << precal[x][y] << '\n';
    }
}
