...
int main() {
    ...
    loop(t) {
        ...
        vector<int> cc(n + 1), cc2(n + 1);
        for (auto e : c) {
            cc[e] += e;
            cc2[e] += 1;
        }

        atcoder::segtree<int, op, e> seg(cc), seg2(cc2);
        ...
        rep(v, 1, n + 1) {
            cnt_r -= c[v - 1];
            lack += c[v - 1] == 0;
            seg.set(c[v - 1], seg.get(c[v - 1]) - c[v - 1]);
            seg2.set(c[v - 1], seg2.get(c[v - 1]) - 1);
            ...
            int r = seg.max_right(0, [&](int s) { return s <= k; });
            ...
            int x = (c2 / r) - d + seg2.prod(r + 1, n + 1);
