...
int main() {
	...
	for(int i = 0; i < xs.size(); i++) {
		...
		for(int j = i + 1; j < xs.size(); j++) {
			...
			if((dx == 0 && dy < 3) || (dx < 3 && dy == 0) || (dx == dy && dx < 3)) {
				if(dx < 2 && dy < 2) {
					if(empty(2 * p1.first - p2.first, 2 * p1.second - p2.second) || empty(2 * p2.first - p1.first, 2 * p2.second - p1.second)) {
						out << "YES";
						return 0;
					}
				}else {
					if(empty((p1.first + p2.first) / 2, (p1.second + p2.second) / 2)) {
						out << "YES";
						return 0;
					}
				}
