...
int T,ch[M][2],sub[M],col[M],ec=1;
io::StringReader ipv4([&](char c){return (c<='9'&&c>='0')||c=='.'||c=='/'||c=='-'||c=='+';});
char ob[100];
vector<pii>ans;
void dfs(int u,u32 g,int d){
	if(~sub[u]&1)return;
	if(sub[u]==1){
		ans.pb(pii(g,d));
		return;
	...
	dfs(ch[u][0],g,d-1);
	dfs(ch[u][1],g+(1u<<(d-1)),d-1);
}
int main(){
	int x,l,a,b,c,d,e;u32 g;
	for(scan(T);T;--T){
		ipv4.gs(ob),l=strlen(ob);
		if(ob[l-3]=='/'||ob[l-2]=='/')sscanf(ob+1,"%d.%d.%d.%d/%d",&a,&b,&c,&d,&e);
		else sscanf(ob+1,"%d.%d.%d.%d",&a,&b,&c,&d),e=32;
		g=(u32)a<<24|b<<16|c<<8|d;
		a=1+(*ob=='+');
		for(x=1,b=31;b>=32-e;b--){
			if(col[x]>0&&col[x]!=a)puts("-1"),exit(0);
			if(!ch[x][c=g>>b&1])ch[x][c]=++ec;
			sub[x]|=a,x=ch[x][c];
		}
		if(sub[x]>0&&sub[x]!=a)puts("-1"),exit(0);
		sub[x]|=a,col[x]=a;
	}
	dfs(1,0,32);
	print(ans.size()),putc('\n');
	for(auto it:ans){
		print((it.fi&0xFF000000u)>>24),putc('.');
		print((it.fi&0x00FF0000u)>>16),putc('.');
		print((it.fi&0x0000FF00u)>>8),putc('.');
		print((it.fi&0x000000FFu)),putc('/');
		print(32-it.se),putc('\n');
	}
}