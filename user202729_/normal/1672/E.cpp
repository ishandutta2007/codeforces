// worried (that algorithm may be wrong)
// <almost wrongly submitted to D somehow>
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
int main(){
	...
	auto const query=[&](int width){ // return height.
		std::cout<<"? "<<width<<std::endl;
		return read<int>();
	};

	int const maxL=2000;
	int width=number*maxL+number-1;
	//assert(query(width)==1);
		
	// first find minimum width such that height is still 1
	for(int step=1<<25; step; step>>=1){
		if(width-step>=number*1+number-1 and
				query(width-step)==1){
			width-=step;
		}
	}
	...
	while(true){
		//assert(query(width)==queryWidth);
		width=(target-1)/queryWidth;
		if(width<=0){
			break;
		}
		...
		target=std::min(target, width*queryWidth);
	}
...
//
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1672/problem/E