...
int main() {
	...
	static int a[101]; for(int i=0;i<n;i++) scanf("%d",a+i);
	static int ans[301][301];
	static int ct[301];
	for(int i=0;i<n;i++) ++ct[a[i]];
	for(int L=1;L<=300;L++) for(int R=L;R<=300;R++) {
		static int dp[101];
		dp[0]=0;
		for(int i=0;i<n;i++) {
			dp[i+1]=0;
			if(L<=a[i] && a[i]<=R) dp[i+1]=1;
			else continue;
			for(int j=0;j<i;j++) if(a[j]<=a[i])
				dp[i+1]=max(dp[i+1],dp[j+1]+1);
		}
		ans[L][R]=0;
		for(int i=1;i<=n;i++) ans[L][R]=max(ans[L][R],dp[i]);
	}

	static int ost[301],nst[301];
	for(int it=0;it<350&&T>1;it++) {
		for(int i=1;i<=300;i++) ost[i]=nst[i], nst[i]=0;
		for(int j=1;j<=300;j++) for(int i=j;i<=300;i++)
			nst[i]=max(nst[i],ost[j]+ans[j][i]);
		--T;
	}
	int fa=max(T-350,0);
	for(int i=1;i<=300;i++) nst[i]+=ct[i]*fa;
	T-=fa;
	for(;T>1;) {
		for(int i=1;i<=300;i++) ost[i]=nst[i], nst[i]=0;
		for(int j=1;j<=300;j++) for(int i=j;i<=300;i++)
	...
	int be=0;
	assert(T==1);
	for(int i=1;i<=300;i++) be=max(be,nst[i]+ans[i][300]);
	printf("%d\n",be);
}