...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) h[i] = h[i-1] + a[i];
		
		p[0] = P(0, 0);
		rep(i, 1, n) p[i] = max(p[i-1], P(h[i], i));
		
		q[n] = P(0, n);
		per(i, n-1, 0) q[i] = max(q[i+1], P(h[i], i));
		...
		while(1){
			ll ng = -1;
			rep(i, l, n){
				h[i] = h[i-1] + a[i], p[i] = max(p[i-1], P(h[i], i));
				if(h[i] < 0){
					ng = i;
					break;
				}
			...
			ll L = p[ng].se, R = q[ng].se;
			ll ub = n+1, lb = 0, mid;
			while(ub-lb>1){
				mid = (ub+lb)/2;
				if(q[mid].fi >= h[R] - h[L]) lb = mid;
				else ub = mid;
			}
			R = lb;
			ans.pb(P(L+1, R));
			reverse(a+L+1, a+R+1);
			rep(i, L+1, R) h[i] = h[i-1] + a[i], p[i] = max(p[i-1], P(h[i], i));
			l = R+1;
		}
		cout << sz(ans) << "\n";
