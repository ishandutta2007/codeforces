...
int main(){
    ...
    while(1){
        ...
        ll end=begin;
        while(s[end]=='1')end++;
        one.emplace_back(begin,end);
        begin=end;
    }
    each(l,r,one){
        const ll d=r-l;
        rep(d)dp[d-i]+=(l+1+i)*(n-r+1+i)-i*(i+1)/2;
    }
    set<pll>ones;
    sort(one.begin(),one.end(),[](pll a,pll b){return a.second-a.first<b.second-b.first;});
    rrep(i,1,n+1){
        while(one.size()&&one.back().second-one.back().first==i){
            ones.insert(one.back());
            one.pop_back();
        }
        if(ones.size()<=1)continue;
        auto p=ones.begin(),q=next(p);
        while(q!=ones.end()){
            const ll l=p->second-i,r=q->first+i;
            dp[i]-=(l+1)*(n-r+1);
            p=q++;
        }
    }
