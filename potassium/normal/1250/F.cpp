...
int r,c,k;
inline pii nxt(pii cur) {
	if (cur.F & 1) {
		if (cur.S == 0) return {cur.F+1, 0};
		else return {cur.F, cur.S-1};
	} else {
		if (cur.S == c-1) return {cur.F+1,c-1};
		else return {cur.F,cur.S+1};
	}
}
...
int main() {
	char X[70];
	for (int i = 0; i <= 9; i++) X[i] = '0' + i;
	for (int i = 0; i < 26; i++) X[i+10] = 'a' + i;
	for (int i = 0; i < 26; i++) X[i+36] = 'A' + i;
	CASE(t) {
		R(r,c,k);
		vector<string> board(r), ans(r);
		for (int i = 0; i < r; i++) R(board[i]);
		...
		int small = Rcnt / k;
		int lcnt = Rcnt - small * k;
		
		for (int i = 0; i < r; i++) ans[i] = board[i];
