...
signed main(){
	...
	linear_sieve();
	initphi();
	rng(i,1,vmax)for(int j=i;j<vmax;j+=i)
		qs[j].eb(i,phi[j/i]);
	rng(i,1,vmax)dp[0][i]=inf;
	rng(k,1,kmax){
		seglazy<N> seg(vi(dp[k-1],dp[k-1]+vmax));
		int pre=0;
		rng(i,1,vmax){
			for(auto [j,w]:qs[i]){
				if(pre<j){
			...
			dp[k][i]=seg.composite(pre,i).mn;
			pre=seg.max_right(pre,&N::ok,dp[k][i]).a;
		}
	}
