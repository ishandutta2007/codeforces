...
class SegTreeAl{
	...
	void add(int o,int l,int r,int L,int R,int v)
	{
		...
		if(minn[o]==minn[o<<1])cc[o]+=cc[o<<1];
		if(minn[o]==minn[o<<1|1])cc[o]+=cc[o<<1|1];
#endif
#ifdef cmlSEGSUM
...
#endif
int a[1000005],smx[1000005],smn[1000005],cc,dd;
signed main()
{
	initprog();
	int n=read();
	rg(n)int x=read(),y=read();a[x]=y;gr
	t.b(1,1,n);
	rg(n)t.add(1,1,n,i,i,inf);gr
	long long ans=0;
	rg(n)
	...
	while(cc&&a[i]>a[smx[cc]])t.add(1,1,n,smx[cc-1]+1,smx[cc],-a[smx[cc]]),cc--;
	while(dd&&a[i]<a[smn[dd]])t.add(1,1,n,smn[dd-1]+1,smn[dd],a[smn[dd]]),dd--;
	t.add(1,1,n,smx[cc]+1,i,a[i]);
	t.add(1,1,n,smn[dd]+1,i,-a[i]);
	smx[++cc]=i;
	smn[++dd]=i;
	ans+=t.cc[1];
	// rg(n)odb(t.qmin(1,1,n,i,i));gr puts("");
	// rg(cc)odb(smx[i]);gr puts("X/");
	// rg(dd)odb(smn[i]);gr puts("N");
	// oldl(ans);
	gr
	oldl(ans);
	return 0;
