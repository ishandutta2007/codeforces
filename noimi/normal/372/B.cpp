...
int main() {
    INT(n, m, q);
    vv(int, a, n, m);
    CumulativeSum2D<int> c(n, m);
    rep(i, n) rep(j, m) {
        CHR(cc);
        a[i][j] = cc == '0';
        if(a[i][j]) c.add(i, j, 1);
    }
    c.build();

    vvvv(int, ans, n + 1, n + 1, m + 1, m + 1);
    rep2(id, 1, n) rep(i, n + 1 - id) rep2(jd, 1, m) rep(j, m + 1 - jd) {
        int ii = i + id, jj = j + jd;
        auto &res = ans[i][ii][j][jj];
        if(i + 1 == ii) {
            rep2(y, j, jj - 1) {
                if(a[i][y]) {
                    int c = 0;
                    while(y < jj and a[i][y]) c++, y++;
                    res += c * (c + 1) / 2;
                }
            }
            // dump(i, ii, j, jj, res);
        } else {
            res = ans[i][ii - 1][j][jj] + ans[i + 1][ii][j][jj] - ans[i + 1][ii - 1][j][jj];
            rep2(y, j, jj - 1) {
                int cc = 0;
                while(y < jj and c.query(i, y, ii, y + 1) == ii - i) y++, cc++;
                res += cc * (cc + 1) / 2;
            }
        }
    }
