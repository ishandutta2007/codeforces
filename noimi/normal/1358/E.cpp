...
signed main(){
    ...
    LL(x);
    if(x >= 0){
        ll sum = accumulate(all(a),0LL) + (ll)(n-((n+1)/2)) * x;
        cout << (sum > 0 ? n:-1)<<endl;
    }
    else{
        int l = (n+1)/2;
        ...
        SegmentTree<ll> seg(n,[](ll x,ll y){return min(x,y);},INT_MAX);
        ll now = 0;
        seg.set(0,0);
        rep(i,l)seg.set(i+1,now+=-a[i]+x);
        seg.build();
        rep2(i,0,r){
            if(sum <= 0) continue;
            ll t = seg.query(0,r-i+1);
            if(sum + t > 0){
                cout << l + i << endl;exit(0);
            }
            sum+=x;
        }
        cout << -1 << endl;
