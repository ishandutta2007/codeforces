...
inline lli madd(lli a, lli b) {
  return (((a%MOD)+(b%MOD))%MOD+MOD)%MOD;
}

inline lli mmul(lli a, lli b) {
...
lli calc(vi A){
  ...
  V[0][0] = 1;
  FORU(i,1,b) {
    cur = mmul(mmul(cur,b+1-i),invmod(i));
    V[0][2*i] = cur;
  }
  lli c=2*b;
  if((sn&1)==0) {
    FORU(i,1,9) {
      FORU(j,0,c+i) {
        V[i][j+1] = madd(V[i][j+1], V[i-1][j]);
        V[i][j] = madd(V[i][j], (((i%4==1||i%4==0)?(j%2==0):(j%2==1))?-1:1) * V[i-1][j]);
      }
    }
  }else{
    FORU(i,1,9) {
      FORU(j,0,c+i) {
        V[i][j+1] = madd(V[i][j+1], V[i-1][j]);
        V[i][j] = madd(V[i][j], (((i%4==2||i%4==3)?(j%2==0):(j%2==1))?-1:1) * V[i-1][j]);
      }
    }
  }
  // FOR(k,5) {FOR(i,c+1+k) cout << clean(V[k][i]) << " "; cout << endl;}
  assert(c+nm == n-1);
  lli ans=0;
  FOR(i,n) ans = madd(ans,mmul(V[nm][i],A[n-1-i]));
  return ans;
}
