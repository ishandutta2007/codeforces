...
int main() {
    ...
    loop(t) {
        ...
        while (true) {
            ...
            if (cnt3 == 0 and cnt2 == 1) break;

            int arg_max = max_element(all(a)) - a.begin();
            int max_val = a[arg_max];

            int k = -1;
            int min_val = numeric_limits<int>::max();
            rep(i, 1, n + 1) {
                if (i == arg_max) continue;
                if ((long long) a[i] * a[i] >= max_val and chmin(min_val, a[i])) {
                    k = i;
                }
            }

            rep(i, 1, n + 1) {
                if (i == k or i == arg_max) continue;
                if ((long long) a[i] * a[i] >= max_val) {
                    op(i, arg_max);
                }
            }
            op(arg_max, k);
