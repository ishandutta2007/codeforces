...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		...
		for(int i=0; i<n; i++)
			gr[i]=ns().toCharArray();
		fastq hola=new fastq();
		for(int i=0; i<n; i++)
			for(int j=0; j<m; j++)
				if(gr[i][j]=='S')
					hola.add(new pair(i,j));
		int[][][]lol=new int[n][m][2];
		for(int i=0; i<n; i++)
			for(int j=0; j<m; j++)
			{
				lol[i][j][0]=-1000000000;
				lol[i][j][1]=-1000000000;
			}
		while(!hola.isEmpty())
		{
			pair te=hola.poll();
			int tea=te.a,teb=te.b;
			int teta=((tea%n)+n)%n;
			int tetb=((teb%m)+m)%m;
			if((lol[teta][tetb][0]!=te.a&&lol[teta][tetb][0]!=-1000000000)||(lol[teta][tetb][1]!=te.b&&lol[teta][tetb][1]!=-1000000000))
			{
				sop("Yes");
				exit();
			}
			else if(lol[teta][tetb][0]==-1000000000&&lol[teta][tetb][1]==-1000000000)
			{
			lol[teta][tetb][0]=tea;
			lol[teta][tetb][1]=teb;
			tea++;
			teta=((tea%n)+n)%n;
			if(gr[teta][tetb]!='#'&&((lol[teta][tetb][0]!=tea)||(lol[teta][tetb][1]!=teb)))
				hola.add(tea,teb);
			tea-=2;
			teta=((tea%n)+n)%n;
			if(gr[teta][tetb]!='#'&&((lol[teta][tetb][0]!=tea)||(lol[teta][tetb][1]!=teb)))
