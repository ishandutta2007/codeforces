#line 1 "codeforces-workspace/contest/1495/a/a.cpp"
...
#line 11 "codeforces-workspace/contest/1495/a/a.cpp"
...
void main() {
  ...
  for (auto i : range(n * 2)) {
    ...
    cin >> s >> t;
    if (!s) {
      y.emplace_back(abs(t));
    } else {
      x.emplace_back(abs(s));
    }
  }
  sort(begin(x), end(x));
  sort(begin(y), end(y));
  double ans = 0;
  for (auto &&[s, t] : zip(x, y)) {
    ans += hypot(s, t);
  }
  cout << ans << "\n";
}
