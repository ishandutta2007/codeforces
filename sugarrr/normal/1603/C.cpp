...
void solve(){
    ...
    auto calc=[](int x){
        ...
        for(int len=stalen-1;len>=1;len--){
            int a=x/len;
            int b=a;
            if(x%len!=0)b++;
            v.pb({a,b,len-1});
    ...
    for(auto x:a){
        ...
        vl sum(vs);
        for(auto y:dp){
            P ppp={1000000000,y.a,1000000000};
            ll pos=POSL(v,ppp)-1;
            num[pos] += y.num;
            sum[pos] += y.num*v[pos].kai + y.sum;
            ans +=y.num*v[pos].kai + y.sum;
        }
        dp.clear();
        rep(i,0,vs-1){
            if(num[i]>0){
                dp.pb({v[i].a,num[i],sum[i]});
            }
        }
    }
    cout<<ans.val()<<endl;
