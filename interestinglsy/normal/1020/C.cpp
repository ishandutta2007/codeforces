...
int main(){
	...
	For(i,n){
		...
		Read(c[i]);
		sel[p[i]].Pb(c[i]);
		//if( p[i] != 1 ) allcost.Pb(c[i]);
	}

	For(i,m){
		if(!sel[i].size()) continue;
		sort(All(sel[i]));
		sum[i][0] = sel[i][0];
		For(j,MAXN-2)
			sum[i][j] = sum[i][j-1] + (j<sel[i].size()?sel[i][j]:0);
	}
	/*
	sort(All(allcost));
	allsum[0] = allcost[0];
	For(j,MAXN-2)
		allsum[j] = allsum[j-1] + (j<allcost.size()?allcost[j]:0);
	*/
//B
	int sel1 = sel[1].size();
	ll ans = LINF;
	...
	//int i = 0;
		...
		int gain = sel1;
		Forx(j,2,m){
			if( sel[j].size() < i ) continue;
			tans += sum[j][sel[j].size()+1-i-1];
			gain += sel[j].size()+1-i;
		}//O(gain);
		if( gain < i ){
			allcost.clear();
			Forx(j,2,m){
				int st = 0;
				if( sel[j].size() >= i ) st = sel[j].size()-i+1;//O(st);
				//O(sel[j].size());
				if( sel[j].size() <= st ) continue;
				Forx(k,st,sel[j].size()-1){
					//O(sel[j].size()-1);
					allcost.Pb(sel[j][k]);
				}
			}
