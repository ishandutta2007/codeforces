...
bool trysolve(int m1, int m2, char c1, char c2, vector<string> s, int cntall) {
    int cnt = 0;
...
void solve() {
    ...
    for (auto i : s)
        for (auto j : i)
            cntall += j != '.';
    if (trysolve(0, 1, 'X', 'O', s, cntall)) return;
    if (trysolve(0, 2, 'X', 'O', s, cntall)) return;
    if (trysolve(1, 2, 'X', 'O', s, cntall)) return;
    if (trysolve(0, 1, 'O', 'X', s, cntall)) return;
    if (trysolve(0, 2, 'O', 'X', s, cntall)) return;
    if (trysolve(1, 2, 'O', 'X', s, cntall)) return;
    assert(false);
}
