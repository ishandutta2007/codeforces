...
int main() {
    ...
    cin >> S;
    mint i2 = mint(2).inv();
    vector dph(H + 1, vector<mint>(W + 1));
    vector dpv(H + 1, vector<mint>(W + 1));
    mint ret = 0;
    REP(i, H) REP(j, W) {
        if (S[i][j] == 'o') {
            ret += (dph[i][j] + dpv[i][j]) * i2;
            dpv[i + 1][j] += (1 - dpv[i][j]) * i2;
            dph[i][j + 1] += (1 - dph[i][j]) * i2;
        }
    }

    int no = 0;
    for (auto s : S) no += count(s.begin(), s.end(), 'o');
    dbg(no);
    cout << ret * mint(2).pow(no) << '\n';
}