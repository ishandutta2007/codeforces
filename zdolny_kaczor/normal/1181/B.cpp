...
bool cmp(const vector <int> &a, const vector <int> &b) {
	FORD(i, max<int>(a.size(), b.size()) - 1, 0) {
...
char in[nax];
vector <int> make_num(int i, int j) {
	vector <int> ans;
	for (int x = i; x < j; ++x) ans.push_back(in[x] - '0');
	reverse(ALL(ans));
	rans;
}
vector <int> ans = {};
int main() {
	...
	scanf("%d%s", &l, in);
	int h = l / 2;
	for (int i = min(h, l - 1), t = 0; i > 0 && t < 5; --i) {
		if (in[i] != '0') {
			vector <int> a = make_num(0, i), b = make_num(i, l);
			vector <int> sum = add(a, b);
			debug(imie(a), imie(b), imie(sum));
			if (ans.empty() || cmp(sum, ans))
				ans = sum;
			t++;
		}
	}
	for (int i = max(h, 1), t = 0; i < l && t < 5; ++i) {
		if (in[i] != '0') {
			vector <int> a = make_num(0, i), b = make_num(i, l);
			vector <int> sum = add(a, b);
			debug(imie(a), imie(b), imie(sum));
