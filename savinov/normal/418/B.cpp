...
struct Friend {
    long long cost;
    long long needMonitors;
    int problems;

    bool operator<(const Friend & r) const {
        return needMonitors < r.needMonitors;
    }
};
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        friends[i].cost = nxt();
        friends[i].needMonitors = nxt();
        friends[i].problems = 0;
        int k = nxt();
        for (int j = 0; j < k; ++j) {
            int z = nxt() - 1;
            friends[i].problems |= 1 << z;
        }
    }
    ...
    for (int i = 0; i < n; ++i) {
        monitors = max(monitors, friends[i].needMonitors);

        for (int j = (1 << m) - 1; j >= 0; --j) {
            dp[j | friends[i].problems] = min(dp[j | friends[i].problems], dp[j] + friends[i].cost);
        }
        ans = min(ans, monitors * b + dp[(1 << m) - 1]);
    }
    if (ans == inf) {
        ans = -1;
    }
