...
signed main(){
    ...
    while(1){
        ll next=min(endx);
        if(q.size())chmin(next,q.back().first);
        if(next==LINF)return out(dp[0]);
        rep(256){
            bitset<8>bit(i);
            dp[i]+=(next-at) * ((is_active&bit).count() & 1);
        }
        at=next;
        ll bit=0;
        rep(8)if(endx[i]==at){
        ...
        while(q.size()&&q.back().first==at){
            ll a,b;
            tie(a,b)=q.back();
            q.pop_back();
            b++;
            rep(8)if(!is_active[i]){
                is_active[i]=1;
                endx[i]=b;
                rep(j,256)if(j&1<<i)dp[j]=dp[j^1<<i];
                break;
            }
        }
    }
