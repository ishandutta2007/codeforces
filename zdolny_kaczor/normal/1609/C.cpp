...
ll calc(const vi &a) {
	...
	for (int i = 0; i <= n; ++i) {
		...
		if (v == 1) {
			...
			primes++;
			debug(imie(v), imie(curr), imie(prev));
			ans += (curr + 1) * 1ll * (prev + 1);
			prev = curr;
			curr = 0;
		}
		else {
...
void solve() {
	...
	vector <vi> b(e);
	for (int i = 0; i < n; ++i) b[i % e].push_back(a[i]);
	ll ans = 0;
	for (auto &x : b) ans += calc(x);
	printf("%lld\n", ans);
}
void sieve() {
	memset(is_prime, 1, sizeof(is_prime));
	is_prime[0] = is_prime[1] = false;
	for (int p = 2; p < nax; ++p) if (is_prime[p]) {
		for (int x = 2 * p; x < nax; x += p) is_prime[x] = false;
	}
}
int main() {
	sieve();
