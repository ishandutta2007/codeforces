...
int v[3][3];
double p[3];
double dp[1000005][3][3];
int a[1000005];
double ans[3][3];
// #define memset qwq
// void memset(double (*p)[3],int a,int b)
// {
	// for(int i=0;i<3;i++)
		// for(int j=0;j<3;j++)
			// p[i][j]=a;
// }
void update(int o,int l,int r,int p1,int v1)
{
	if(l==r)
	{
		a[l]=v1;	
		rg(3)rg_(j,3)dp[o][i-1][j-1]=-1e18;gr gr
		// memset(dp[o],0,sizeof(o));
		for(int i=0;i<3;i++)dp[o][i][i]=p[i]*a[l];
		return;
	}
	int m=l+r>>1;
	if(p1<=m)update(o<<1,l,m,p1,v1);
	else update(o<<1|1,m+1,r,p1,v1);
	// memset(dp[o],0,sizeof(o));
	rg(3)rg_(j,3)dp[o][i-1][j-1]=-1e18;gr gr
	for(int j=0;j<3;j++)
	for(int k=0;k<3;k++)
	for(int l=0;l<3;l++)
	for(int m=0;m<3;m++)
	if(v[k][l])dp[o][j][m]=std::max(dp[o][j][m],dp[o<<1][j][k]+dp[o<<1|1][l][m]);
}
...
void query(int o,int l,int r,int L,int R)
{
	...
	{
		double ans2[3][3]={};	
		rg(3)rg_(j,3)ans2[i-1][j-1]=-1e18;gr gr
		for(int j=0;j<3;j++)
		for(int k=0;k<3;k++)
		for(int l=0;l<3;l++)
		for(int m=0;m<3;m++)
		if(v[k][l])ans2[j][m]=std::max(ans2[j][m],ans[j][k]+dp[o][l][m]);
		for(int j=0;j<3;j++)for(int k=0;k<3;k++)ans[j][k]=ans2[j][k];
		return;
	}
	int m=l+r>>1;
...
signed main()
{
	initprog();
	if(x>y)std::swap(x,y);
	p[0]=0,p[1]=1.0/y,p[2]=1.0/(x+y);
	for(int i=0;i<3;i++)for(int j=0;j<3;j++)if(x*p[i]+y*p[j]<=1+(1e-12))v[i][j]=1;
	rg(n)update(1,1,n,i,read());gr
	// return 0;
	rg(m)
	int op=read(),x=read(),v=read();
	if(op==1)update(1,1,n,x,v);
	else 
	{
		rg(3)rg_(j,3)ans[i-1][j-1]=i==j?0:-1e18;gr gr
		query(1,1,n,x,v);
		double anss=0;
		rg(3)rg_(j,3)anss=std::max(anss,ans[i-1][j-1]);gr gr
		printf("%.18lf\n",anss);
	}
	gr
	return 0;
