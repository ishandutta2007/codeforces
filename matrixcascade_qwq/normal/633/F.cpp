...
void dfs(int x,int fa)
{
	...
	fp v[x] pf
	{
		if(to==fa)continue;
		dfs(to,x);
		chkmax(dp[x][0],dp[to][0]);
		//chkmax(dp[x][0],dp[to][3]+dp[x][4]);
		chkmax(dp[x][0],dp[to][1]+dp[x][1]);
		chkmax(dp[x][0],max(dp[to][2]+dp[x][4],dp[x][2]+dp[to][4]));
		chkmax(dp[x][1],dp[to][1]);
		chkmax(dp[x][1],dp[x][4]+dp[to][4]);
		chkmax(dp[x][2],dp[to][2]+a[x]);
		chkmax(dp[x][2],dp[x][4]+dp[to][1]);
		chkmax(dp[x][2],dp[to][4]+dp[x][3]+a[x]);
		chkmax(dp[x][3],dp[to][1]);
		chkmax(dp[x][4],dp[to][4]+a[x]);
	}
}
signed main()
{
