...
void solve(){
    ...
    vv(P<Modint>,dp1,n+1,n+1);
    vv(P<Modint>,dp2,n+1,n+1);
    dp1[0][0]={0,1};
    rep(n){
        rep(j,n){
            if(t[j]!='1')dp1[i][j+1]+=dp1[i][j];
            if(t[j]!='0')dp2[i][j]+=dp1[i][j];
        }
        dp2[i][n]+=dp1[i][n];
        if(s[i]!='1')dp2[i+1]=dp2[i];
        if(s[i]!='0')rep(j,n){
            dp2[i][j].first+=dp2[i][j].second*abs(i-j);
            dp1[i+1][j+1]+=dp2[i][j];
        }
    }
    {
