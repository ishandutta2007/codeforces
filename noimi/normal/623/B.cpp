...
main() {
    ...
    rep(_, 2) {
        ll st = s.front();
        for(auto e : {st - 1, st, st + 1}) {
            for(auto [p, __] : factor(e)) {
                ll now = (n - 1) * a;
                if(st != e) now += b;
                ll tmp = 0;
                vll l(n), r(n + 1);
                rep2(i, 1, n - 1) {
                    if(s[i] % p) {
                        tmp -= b - a;
                        if(s[i] % p != 1 and s[i] % p != p - 1) break;
                    } else {
                        tmp += a;
                    }
                    l[i] = tmp;
                ...
                rep2(i, 1, n - 1) chmax(l[i], l[i - 1]);
                rep3(i, n - 1, 1) chmax(r[i], r[i + 1]);
                rep2(i, 0, n - 1) chmin(ans, now - l[i] - r[i + 1]);
            }
        }
        reverse(all(s));
    }
