...
main() {
    ...
    sort(all(v), [](pii x, pii y) { return x.fi + x.se < y.fi + y.se; });
    auto f = [&](int ii, int jj, int i, int j) {
        if(i < ii or j < jj) return (mint)0;
        return C(i + j - ii - jj, i - ii);
    };
    int add = 0;
    if(v.front() != pii(1, 1)) {
        add++;
        v.insert(v.begin(), pii(1, 1));
        k++;
    }
    vv(mint, dp, k, 25);
    mint ans;
    rep3(i, k - 1, 0) {
        dp[i][1] = f(v[i].fi, v[i].se, n, m);
        rep2(j, i + 1, k - 1) dp[i][1] -= dp[j][1] * f(v[i].fi, v[i].se, v[j].fi, v[j].se);
    }
    rep2(t, 2, 22) {
        rep3(i, k - 1, 0) {
            dp[i][t] = f(v[i].fi, v[i].se, n, m);
            if(t != 22 or i) rep2(j, i + 1, k - 1) dp[i][t] -= dp[j][t] * f(v[i].fi, v[i].se, v[j].fi, v[j].se);
            rep(j, t) dp[i][t] -= dp[i][j];
        }
    }
    if(!add) s = (s + 1) >> 1;
    rep2(t, 1, 22) {
