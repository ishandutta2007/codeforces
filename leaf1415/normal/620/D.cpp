...
int main(void)
{
	...
	ll dif = suma-sumb, ans = abs(dif);
	vector<P> avec;
	
	rep(i, 1, n) rep(j, 1, m){
		ll tmp = abs(dif + 2 * (b[j] - a[i]));
		if(ans > tmp){
			ans = tmp;
			avec.clear(), avec.push_back(P(i, j));
		}
	}
	
	vector<R> vec, vec2;
	rep(i, 1, n) rep(j, 1, n) if(i < j) vec.push_back(R(a[i]+a[j], P(i, j)));
	rep(i, 1, m) rep(j, 1, m) if(i < j) vec2.push_back(R(b[i]+b[j], P(i, j)));
	sort(all(vec)), sort(all(vec2));
	
	for(auto p : vec){
		ll pos = lower_bound(all(vec2), R(p.first-dif/2, P(-inf, -inf))) - vec2.begin();
		rep(i, -2, 2){
			if(pos+i < 0 || pos+i >= sz(vec2)) continue;
			ll tmp = abs(dif + 2 * (vec2[pos+i].first - p.first));
			if(ans > tmp){
				ans = tmp;
				avec.clear();
				avec.push_back(P(p.second.first, vec2[pos+i].second.first));
				avec.push_back(P(p.second.second, vec2[pos+i].second.second));
			}
		}
	}
	...
	outl(ans);
	outl(sz(avec));
	for(auto p : avec) outl(p.first, p.second);
