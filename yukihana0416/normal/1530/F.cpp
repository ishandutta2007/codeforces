...
void work()
{
    ...
    for(int p = 0; p < 2; p++)
        for(int q = 0; q < 2; q++)
        {
            ...
            function<void(int, vector<mint>, mint)> dfs = [&](int dep, vector<mint> t, mint coef)
            {
                if(dep == n)
                {
                    for(int i = 0; i < n; i++) coef *= (mint(1) - t[i]);
                    ans += coef;
                    return ;
                }
                mint coef2 = -coef;
                for(int i = 0; i < n; i++) coef2 *= b[dep][i];
                dfs(dep + 1, t, coef2);
                for(int i = 0; i < n; i++) t[i] *= b[dep][i];
                dfs(dep + 1, t, coef);
            };
            dfs(0, vector<mint> (n, 1), coef);
        }
    cout <<ans <<endl;
}
