...
int main() {
	int n,k; R(n,k);
	vector<string> num(n); R(num);

	int totalSticks = 0;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < 7; j++)
			totalSticks += (num[i][j] == '1');
	}

	if (totalSticks + k > 7 * n) {
		W(-1);
		return 0;
	}

	vector<vector<int>> req(n, vector<int>(10, INT_MAX));
	vector<int> minReq(n, INT_MAX);
	for (int i = 0; i < n; i++) {
		int minRequire = INT_MAX;
		for (int j = 0; j <= 9; j++) {
			bool can = 1;
			int require = 0;
			for (int k = 0; k < 7; k++) {
				if (numbers[j][k] == '0' && num[i][k] == '1') {
					can = 0;
					break;
				}
				if (numbers[j][k] == '1' && num[i][k] == '0') {
					require++;
				}
			}
			if (can) {
				req[i][j] = require;
				chkmin(minReq[i], require);
			}
		}
	}
	
	vector<vector<vector<bool>>> dp(n + 1, vector<vector<bool>>(k + 1, vector<bool>(10, false)));
	dp[n][0][0] = 1;
	
	for (int idx = n - 1; idx >= 0; idx--) {
		for (int curDig = 0; curDig <= 9; curDig++) {
			int cost = req[idx][curDig];
			if (cost == INT_MAX) continue;
			for (int lastDig = 0; lastDig <= 9; lastDig++)
				for (int curK = req[idx][curDig]; curK <= k; curK++)
					if (dp[idx + 1][curK - cost][lastDig])
						dp[idx][curK][curDig] = 1;
		}
	}
	
	for (int firstDigit = 9; firstDigit >= 0; firstDigit--) {
		if (!dp[0][k][firstDigit]) continue;
		
		string ans;
		ans += '0' + firstDigit;
		int curIdx = 0, curK = k, curDig = firstDigit;
		for (; curIdx + 1 < n; curIdx++) {
			int cost = req[curIdx][curDig];
			int lastDig = -1;
			for (int lastDigit = 9; lastDigit >= 0; lastDigit--)
				if (dp[curIdx + 1][curK - cost][lastDigit]) {
					lastDig = lastDigit;
					break;
				}
			curK -= cost;
