...
char conv(int i, int r) {
  if (i == r) i--;
  if (i < 26) return 'A' + i;
  if ((i -= 26) < 26) return 'a' + i;
  return '0' + (i-26);
}
...
int main() {
  rep(IN) {
    int h = IN, w = IN, k = IN;
    auto field = IN.v<string>(h);
    int rice = 0;
    rep(r, h) rep(c, w) rice += field[r][c] == 'R';
    int r1 = rice / k, r2 = r1 + 1;
    int c2 = rice - r1 * k, c1 = k - c2;
    int i = 0, cnt = 0;
    ...
    rep(r, h) {
      if (r % 2) rep(c, w) go(r, c);
      else repr(c, w) go(r, c);
    }
