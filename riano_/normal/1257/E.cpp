...
F id(){ return 0; }
atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(200002); //size

    
int main(){
    riano_; ll ans = 2e9;
    ll N,M,K[3],H,W,Q,L,R,T,C,A,B,S; N = 0;
    rep(i,3){
        cin >> K[i]; N += K[i];
    }
    auto sum = v2ll(3,N+1,0);
    rep(i,3){
        rep(j,K[i]){
            ll a; cin >> a;
            sum[i][a]++;
    ...
    rep(i,200002) seg.set(i,0);
    rep(i,N+1){
        seg.set(i,sum[2][i]+sum[1][N]-sum[1][i]);
        //cout << sum[2][i]+sum[1][N]-sum[1][i] << endl;
    }
    rep(i,N+1){
        if(i!=0&&sum[2][i-1]!=sum[2][i]){
            seg.apply(i,N+1,-1);
        }
        ll cost = sum[0][N]+(ll)(i)-2*sum[0][i];
        // cout << i << endl;
        // cout << cost << endl;
        cost += seg.prod(i,N+1);
        //cout << cost << endl;
