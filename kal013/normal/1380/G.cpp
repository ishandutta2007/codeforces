...
vector<long long> P[SQ];


void solve(){
	int n;
	cin >> n;
	vector<long long> A(n),prf(n+1);
	for(int i = 0 ;i < n; ++i){
		cin >> A[i];
	}
	sort(all(A));
	...
	for(int i = 1; i < n ; ++i){
		...
		if (rem >= place){
			...
			// else{
				int v1 = place % rem;
				int st = 1 + v1;
				int kol = (place + rem - 1) / rem;

				ans[n-i] = prf[v1] * kol;

				for(int u = st, mul = place/rem;  u <= place; u += rem, --mul){
					ans[n-i] += (prf[u + rem - 1] - prf[u - 1])*mul;
				}
