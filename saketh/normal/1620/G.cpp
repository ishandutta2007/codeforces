...
int main() {
    ...
    vector<array<int, 26>> mins(1 << N, INF);
    ...
    mn total = accumulate(all(f), mn(0));

    // covered precisely by some subset of m
    F0R (i, N) {
        FOR (m, 1, 1 << N) {
            if (m & (1 << i)) {
                f[m] += f[m ^ (1 << i)];
    ...
    FOR (m, 1, 1 << N) {
        ...
        mn ct = total - f[ONES ^ m];

        ans ^= int64_t(ct.v) * sum * __builtin_popcount(m);
    }
