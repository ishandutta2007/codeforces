...
data class Member (val strength: Int, val index: Int)

fun solve() {
    val n = io.int()
    val a = ArrayList<Int>(io.ints(n))
    val b = ArrayList<Int>(io.ints(n+1))
    val m = io.int()
    val c = ArrayList<Int>(io.ints(m))
    val members = ArrayList<Member>()
    for (i in 0 until n) {
        members.add(Member(a[i],-1))
    }
    for (i in 0 until m) {
        members.add(Member(c[i],i))
    }
    members.sortBy{it.strength}
    b.sort()
    var prefAns = Int.MIN_VALUE
    var cnt = 0
    val curAns = IntArray(m){kotlin.Int.MIN_VALUE}
    for (mem in members) {
        if (mem.index == -1) {
            prefAns = maxOf(prefAns,b[cnt]-mem.strength)
            cnt += 1
        } else {
            curAns[mem.index] = maxOf(curAns[mem.index],maxOf(prefAns,b[cnt]-mem.strength))
        }
    }
    b.reverse()
    cnt = 0
    prefAns = Int.MIN_VALUE
    for (mem in members.asReversed()) {
        if (mem.index == -1) {
            prefAns = maxOf(prefAns,b[cnt]-mem.strength)
            cnt += 1
        } else {
