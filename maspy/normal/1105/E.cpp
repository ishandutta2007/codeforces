...
#line 2 "/home/maspy/compro/library/graph/maximum_independent_set.hpp"
...
void solve() {
  ...
  Graph G(M);
  for (auto&& s: bits) {
    FOR(i, M) FOR(j, i) if (s & 1LL << i) if (s & 1LL << j) { G.add(i, j); }
  }
  G.build();

  auto S = maximum_independent_set(G);
  print(len(S));
}
