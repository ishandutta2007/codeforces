...
int main() {
    input(int, t);
    loop(t) {
        input(long long, b, q, y);
        input(long long, c, r, z);
        ...
        long long j = (c + r * (z - 1) - b) / q;
        if (j < 0 or j >= y) {
            print(0);
            continue;
        }
        
        long long pre_i = (c - r - b) / q;
        long long pst_j = (c + r * z - b) / q;
        if (pre_i < 0 or pst_j >= y) {
            print(-1);
            continue;
        }
        ...
        auto f = [&](long long p) {
            if (long long g = gcd(p, q); p / g * q != r) return;

            long long lv = c;
            long long rv = c + r * (z - 1);

            long long llv = lv - r;
            long long rrv = rv + r;

            long long l_min = cld(c - lv, p);
            long long l_max = fld(c - llv - 1, p);

            long long r_min = cld(rv - c, p);
            long long r_max = fld(rrv - c - 1, p);

            ans += mint(l_max - l_min + 1) * mint(r_max - r_min + 1);
        };
