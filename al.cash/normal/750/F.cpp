...
bool find1(int v, int forb, bool first = false) {
  int k = query(v);
  if (k == 1) return false;
  if (k == 2) return true;
  if (!first && (h >= 6 && sz(p) == 2 * h - 5)) {
    p.pb(-1);
    p.pb(-1);
    return false;
  }
  for (int u : e[v]) if (u != forb)
    return find1(u, v, first);
  return false;
}
...
void solve() {
  e.assign(1 << h, vi());
  p.clear();
  if (find1(1, -1, true)) return;
  reverse(all(p));
  int v = p.back();
  if (sz(e[v]) != 1) {
    for (int u : e[v]) if (u != p[sz(p)-2]) {
      if (find1(u, v)) return;
      break;
    }
  }
  while (true) {
    ...
    v = p[m];
    vi forb;
    if (m != 0) forb = {p[m-1], p[m+1]};
    p.erase(p.begin() + m + 1, p.end());
    if (sz(p) == h - 3) {
      for (int u : e[v]) if (find(all(forb), u) == forb.end()) {
        int k = query(u);
        if (k == 2) return;
        vi c;
        for (int x : e[u]) if (x != v) {
          k = query(x);
          if (k == 2) return;
          for (int y : e[x]) if (y != u) c.pb(y);
        }
        REP(i, sz(c)-1) if (query(c[i]) == 2) return;
        writeln("! ", c.back());
        flush();
        return;
      }
