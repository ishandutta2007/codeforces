...
public:
    void solve(istream &inp, ostream &outp) {
        ...
        arri q(n);
        function<int(int, int)> calcQ = [&](int vert, int last) -> int {
            int &res = q[vert];
            res = 1;
...
//        arr<arr<pr>> answer(n);
        arr<pr> temp(n + 2);
        function<void(int, int)> go = [&](int vert, int last) {
            int id = -1;
            for (edge *e : graph[vert]) {
                int next = e->to;
                if (next == last) {
            ...
            arr<pr> cur = answer[vert];
            ll delta = w[vert] - b[vert];
            if (id == -1) {
//                cur = arr<pr>(2);
                cur[0] = {0, delta};
                cur[1] = {delta > 0 ? 1 : 0, 0ll};
                return;
            }
//            cur = arr<pr>(min(q[vert], m) + 1);
//            cur = arr<pr>(q[vert] + 1);
            arr<pr> aid = answer[id];
            for (int i : range(q[id] + 1)) {
                cur[i] = aid[i];
                cur[i].second += delta;
            }
//            int sz = min(q[id], m);
            int sz = q[id];
            for (int i : range(sz + 1, q[vert] + 1)) {
                cur[i] = {-1, 0ll};
            }
            for (edge *e : graph[vert]) {
                int next = e->to;
                ...
                arr<pr> anext = answer[next];
                for (int i : range(sz + 1)) {
                    temp[i] = cur[i];
                    cur[i] = {-1, 0};
                }
//                int nsz = min(sz + q[next], m);
                int nsz = sz + q[next];
                for (int i : range(sz + 1)) {
//                    for (int j : Range(1, min(q[next], nsz - i) + 1)) {
                    for (int j : range(q[next] + 1)) {
                        maxim(cur[i + j], {temp[i].first + anext[j].first, temp[i].second + anext[j].second});
                    }
                }
                sz = nsz;
            }
...
//                for (int i : RevRange(m)) {
                for (int i : RevRange(n)) {
                    cur[i + 1] = {cur[i].first + (cur[i].second > 0 ? 1 : 0), 0};
                }
                return;
            }
//            for (int i : RevRange(min(sz + 1, m))) {
            for (int i : RevRange(sz + 1)) {
                maxim(cur[i + 1], {cur[i].first + (cur[i].second > 0 ? 1 : 0), 0});
            }
        };
        go(0, -1);
