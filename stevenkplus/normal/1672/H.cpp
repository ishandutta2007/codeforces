...
// ============= Solution ============= //
// every non-empty string is start * 0 + count * 1 + end * 0, where count > 0
// if type = 1, flip 0 and 1
// note that representation is not unique
//
// using rules:
// 101 -> 1
// 0011 -> 01 (val += 1)
...
int main() {
   ...
   vector<node> seg(2 * n);
   for (int i = 0; i < n; ++i) {
      seg[n + i] = {0, s[i] == '1', 1, 0, 0};// {val, type, count, start, end};
   }
   for (int i = n - 1; i; --i) {
   ...
   while (q--) {
      ...
      for (l += n, r += n; l < r; l /= 2, r /= 2) {
         if (l & 1) vl.push_back(seg[l++]);
         if (r & 1) vr.push_back(seg[--r]);
      }
      reverse(vr.begin(), vr.end());
      vl.insert(vl.end(), vr.begin(), vr.end());
      node cur = vl[0];
      for (int i = 1; i < sz(vl); ++i) {
         cur = cur + vl[i];
      }
