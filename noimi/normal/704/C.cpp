...
main() {
    ...
    rep2(i, 1, m) {
        if(a[i].empty())
            ...
            int c = 0;
            if(a[i][0] < 0) a[i][0] = -a[i][0], c++;
            if(a[i][1] < 0) a[i][1] = -a[i][1], c++;
            if(c == 1) {
                g.add(a[i][0], a[i][1], 1);
            } else
                g.add(a[i][0], a[i][1], 0);
    ...
    rep2(i, 1, n) {
        if(g[i].size() == 1 and !used[i]) {
            ...
            auto dfs = [&](auto &&f, int x, int p) -> void {
                ...
                vv(mint, nxt, 2, 2);
                rep(i, 2) rep(j, 2) {
                    rep(k, 2) { nxt[i ^ (j or k)][c ^ k] += now[i][j]; }
                }
                now = nxt;
                for(auto e : g[x])
                    if(e != p) f(f, e, x);
            ...
            dfs(dfs, i, 0);
            vmint nxt{0, 0};
            rep(j, 2) rep(k, 2) rep(t, 2 - em[tail]) nxt[j ^ (k or t)] += now[j][k];
            vmint tmp(2);
            rep(j, 2) rep(k, 2) tmp[j ^ k] += dp[j] * nxt[k];
            dp = tmp;
        }
    }
    rep2(i, 1, n) {
        if(!used[i]) {
            if(g[i].empty()) {
                if(K[i] == 2) {
                    vmint DP(2);
                    DP[0] = dp[1] * 3 + dp[0], DP[1] = dp[0] * 3 + dp[1];
                    dp = DP;
                } else {
                    dp[0] = dp[0] + dp[1];
                    dp[1] = dp[0];
            ...
            if(g[i][0] == g[i][1]) {
                vi V;
                V.eb(g[i][0].cost), V.eb(g[g[i][0]][0].cost);
                sort(all(V));
                if(V == vi{0, 0}) {
                    dp[0] *= 4, dp[1] *= 4;
                } else {
            ...
            if(c) {
                now[0][1] = 1;
                dfs(dfs, i, g[i][1]);
                rep(j, 2) rep(k, 2) nxt[j ^ k] += now[j][k];
                rep(j, 2) rep(k, 2) now[j][k] = 0;
                now[0][0] = 1;
                dfs(dfs, i, g[i][1]);
                rep(j, 2) rep(k, 2) nxt[j ^ 1] += now[j][k];
            } else {
                now[0][1] = 1;
                dfs(dfs, i, g[i][1]);
                rep(j, 2) rep(k, 2) nxt[j ^ 1] += now[j][k];
            ...
            vmint DP(2);
            rep(j, 2) rep(k, 2) DP[j ^ k] += dp[j] * nxt[k];
            dp = DP;
        }
    }
    cout << dp[1] << endl;
