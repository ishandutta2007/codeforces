...
int grid[2000][2000];
int ans[1000][1000];
map<int, int> yoko[1000], tate[1000];
void solve() {
    int n, m;
    cin >> n >> m;
    REP(i, n) {
    ...
    REP(i, n) {
        ...
        int itr = 0;
        for (auto x = tate[i].begin(); x != tate[i].end();++x) {
            x -> second = itr;
            itr++;
        }
    }
    ...
    REP(q, m) {
        int itr = 0;
        for (auto x = yoko[q].begin(); x != yoko[q].end(); ++x) {
            x->second = itr;
            itr++;
        }
    }
    REP(i, n) {
        REP(q, m) {
            int now = tate[i][grid[i][q]] + 1;
            int maxs = max((int)tate[i].size(), now + (int)yoko[q].size() - yoko[q][grid[i][q]] - 1LL);
            int mins = min(1LL, now - yoko[q][grid[i][q]]);
            ans[i][q] = maxs - mins + 1LL;
        }
    }
    REP(i, n) {
        REP(q, m) {
