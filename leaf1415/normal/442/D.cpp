...
void dfs(int v)
{
	...
	rep(i, 0, 20){
		ll mn = inf; vector<ll> vec;
		for(auto u : G[v]){
			chmin(mn, get(u, i));
			vec.pb(get(u, i-1));
		}
		sort(all(vec));
		if(sz(vec) >= 2) chmin(mn, vec[1]);
		dp[v][i] = mn;
	}
}
...
int main(void)
{
	...
	/*rep(i, 0, n){
		rep(j, -1, 3) if(get(i, j) > inf/2) cout << "x" << " ";
		else cout << get(i, j)+1 << " ";
		cout << endl;
	}*/
	...
	rep(i, 0, n) ans[i] = 0;
	for(auto u : G[0]){
		rep(i, 0, 20) chmax(ans[min(dp[u][i], n)+1], i+1);
	}
	rep(i, 1, n) chmax(ans[i], ans[i-1]);
	rep(i, 1, n) ans[i]++;
