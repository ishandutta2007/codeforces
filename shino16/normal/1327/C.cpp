...
int main() {
  ...
  string op = "LRDU";
  VB done(k, false);
  rep(i, k) if (s[i] == f[i]) done[i] = true;
  auto dist = [&](int i)->int{
    return abs(s[i]._1-f[i]._1) + abs(s[i]._2-f[i]._2);
  };
  string ops;
  int maxD = 0, who;
  rep(time, 2*h*w) {
    if (time == 0 or done[who]) rep(i, k) if (not done[i] and chmax(maxD, dist(i))) who = i;
    debug(maxD);
    debug(dist(0));
    if (maxD == 0) {
      OUT(time);
    ...
    int opix =
      ...
      f[who]._1 > s[who]._1 ? 2 :
      3;
    ops.push_back(op[opix]);
    rep(i, k) {
      s[i] = PII{max(0, min(h-1, s[i]._1+dr[opix])), max(0, min(w-1, s[i]._2+dc[opix]))};
      if (s[i] == f[i]) done[i] = true;
    }
    maxD--;
    debug(opix);
    debugv(s);
