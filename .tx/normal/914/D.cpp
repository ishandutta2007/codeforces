...
struct ST {
    ...
    int find(int i, int l, int r, int ql, int qr, int qx) {
        if (r <= ql || qr <= l || a[i] % qx == 0) {
            return -1;
        }
        if (l + 1 == r) {
            return l;
        ...
        int m = (l + r) >> 1;
        int t = find(i * 2 + 1, l, m, ql, qr, qx);
        if (t == -1) {
            t =  find(i * 2 + 2, m, r, ql, qr, qx);
        }
        return t;
    }
...
int main(int argc, char** argv) {
    ...
    for (int iq = 0; iq < q; iq++) {
        ...
        if (t == 1) {
            ...
            cin >> l >> r >> x;
            --l; --r;
            int z = st.find(0, 0, n, l, r + 1, x);
            if (z == -1) {
                cout << "YES\n";
                continue;
            }
            int g = st.get(0, 0, n, z + 1, r + 1);
            if (g % x == 0) {
                cout << "YES\n";
            } else {
                cout << "NO\n";
