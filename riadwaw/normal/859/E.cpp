...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		map<int, int> cnt_by_root;
		//map<int, int> cnt_by_cycle_len;
		//cerr << ans << endl;
		for (int i: range(2 * n)) {
			if (root[i] != -1) {
				++cnt_by_root[root[i]];
			...
			//if (cycleLen[i] != -1) {
			//	//cerr << cycleLen[i] << endl;
			//	++cnt_by_cycle_len[cycleLen[i]];
			//}
		}


		for (auto p: cnt_by_root) {
			//cerr << p.second << endl;
			ans *= p.second; //+ 1;
		}

		//for (auto p: cnt_by_cycle_len) {
		//	int t = p.second / p.first;
		//	for (int i: range(t)) {
		//		ans *= p.first;
		//	}
		//}
