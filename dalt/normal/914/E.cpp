#if (defined CF914E) or (defined ONLINE_JUDGE)
...
const int g_charNum = 't' - 'a' + 1;
int g_mask = (1 << g_charNum) - 1;
int g_cnt[1 << g_charNum];
int g_version[1 << g_charNum];

int g_cntLocal[1 << g_charNum];
int g_versionLocal[1 << g_charNum];

#define ENSURE(ver, i) if(g_version[i] != ver){g_version[i]=ver;g_cnt[i]=0;}
#define ENSURE_LOCAL(ver, i) if(g_versionLocal[i] != ver){g_versionLocal[i]=ver;g_cntLocal[i]=0;}

int g_versionNum;
int g_localVersionNum;
inline int getCnt(int i) {
	ENSURE(g_versionNum, i);
	return g_cnt[i];
}
...
inline int getCntLocal(int i) {
	ENSURE_LOCAL(g_localVersionNum, i);
	return g_cntLocal[i];
}
...
void appendTag0(int rootTag, int from, int to)
{
...
void appendTag(int rootTag, int from, int to)
{
	...
	for (int i = from; i < to; i++)
	{
		...
		node->localAddition += getCnt(tag) - getCntLocal(tag);
		
		for (int bit = 1, until = 1 << g_charNum; bit < until; bit <<= 1)
		{
			int fixedTag = tag ^ bit;
			node->localAddition += getCnt(fixedTag) - getCntLocal(fixedTag);
		}
	}
}
...
void dac(Node root) {
	...
	nodeTraceSize = 0;
	traceBeginSize = 0;
	traceBegin[traceBeginSize] = 0;

	int childNum = root->edge.size();
	for (int i = 0; i < childNum; i++)
	{
		Node child = root->edge[i];
		//deleteElement(child->edge, root);
		...
		traceEnd[traceBeginSize] = nodeTraceSize;
		++traceBeginSize;
		traceBegin[traceBeginSize] = nodeTraceSize;
	}

	g_versionNum++;
	increment(0, 1);
	for (int i = 0; i < traceBeginSize; i++)
	{
		appendTag0(root->num, traceBegin[i], traceEnd[i]);
	}
	for (int i = 0; i < traceBeginSize; i++)
	{
...
void solve() {
	int n = io::readUnsignedInt();

	for (int i = 1; i < n; i++)
	{
		Node a = nodes + io::readUnsignedInt();
		Node b = nodes + io::readUnsignedInt();
		a->edge.push_back(b);
		b->edge.push_back(a);
	}
	...
	for(int i = 1; i <= n; i++)
	{
		io::writeUnsignedLong(nodes[i].cnt + 1);
		io::write(' ');
	}
}
