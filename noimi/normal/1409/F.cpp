...
main() {
    INT(n, k);
    STR(S, t);
    vvv(int, dp, n + 1, k + 1, n + 1, -1000000);
    dp[0][0][0] = 0;
    rep(i, n) {
        rep(j, k + 1) {
            rep(s, n + 1) {
                if(dp[i][j][s] < 0) continue;
                if(j < k and s < n) chmax(dp[i + 1][j + 1][s + 1], dp[i][j][s] + (t[0] == t[1] ? s : 0));
                if(j < k) chmax(dp[i + 1][j + 1][s], dp[i][j][s] + s);
                if(S[i] == t[0] and s < n) chmax(dp[i + 1][j][s + 1], dp[i][j][s] + (t[0] == t[1] ? s : 0));
                if(S[i] == t[1]) chmax(dp[i + 1][j][s], dp[i][j][s] + s);
                chmax(dp[i + 1][j][s], dp[i][j][s]);
            }
        }
    }

    int ans = 0;
    rep(j, k + 1) rep(s, n + 1) chmax(ans, dp[n][j][s]);
    cout << ans << endl;
}