...
vl primes;
int look[200010];
amogus_map<short, int, 6, true> arena[200010];

int a[200010], pcnt[20000], dep[200010];
vector<int> edges[200010];

amogus_map<int, vector<int>> fake[20000];
int dfsi(int u, int o, int e) {
    int n = a[u];
    while (n!=1) {
        int p = prime[n];
        while (n%p==0) n/=p;
    ...
    int c = u;
    for (int v:edges[u]) if (v-o) {
        int d = dfsi(v, u, e+1);

        if (arena[c].size()<arena[d].size()) swap(c, d);
        for (auto pa:arena[d]) {
            auto __p = pa; auto p = __p.first; auto x = __p.second;
            if (arena[c].count(p)) {
                int z = arena[c][p];
                if (x!=u) fake[p][u].push_back(x), fake[p][x].push_back(u);
                if (z!=u) fake[p][u].push_back(z), fake[p][z].push_back(u);
                arena[c][p] = u;
            } else {
                arena[c][p] = x;
            }
...
int ans = 0;
int dfs(int p, int u, int o) {
    int sz = a[u]%primes[p]==0;
    for (int v:fake[p][u]) if (v-o) {
        int down = dfs(p, v, u);
        ( ans += (C3(pcnt[p]) - C3(pcnt[p] - down) - C3(down) +M + M)%M * abs(dep[v] - dep[u])%M )%=M;
        sz += down;
    }
    return sz;
}
...
int main() {
    ...
    ({primes = prime_seive_list(prime, 200010);});
    for (int i=0; i<primes.size(); ++i) look[primes[i]] = i;
    ...
    for (int p=0; p<20000; ++p) {
        if (fake[p].size()==0) continue;
        int rt = (*begin(fake[p])).first;
        dfs(p, rt, -1);
    }
