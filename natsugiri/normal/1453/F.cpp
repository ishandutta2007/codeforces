...
template<class T> struct RMQ {
	...
	build_with_initializer([init](int) { return init; });
    }
...
void MAIN() {
	...
	for (int x=i+1; x<N; x++) {
	    if (i + A[i] < x) {
		aux[x] = INF;
		// aux[x] = H[x].min_v(x, N+1)+1 + add[x];
	    } else {
		int r = min(N, i+A[i]+1);
		aux[x] = H[x].min_v(r, N+1) + add[x];
	    }
	}

	for (int x=i+1; x<=i+A[i] && x<N; x++) {
	    add[x]++;
	}
	...
	for (int x=i+1; x<N; x++) {
	    assert(add[i] == 0);
	    if (aux[x] != INF) {
		H[i].modify(x, aux[x] - add[i]);
	    }
	}
    }

    int ans = H[0].min_v(0, N+1) + add[0];
