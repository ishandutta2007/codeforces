...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        auto c = in.readIntArray(n + m);
        decreaseByOne(l);

        arr2d<ll> answer(n + m + 1, n + 1, numeric_limits<ll>::min() / 2);
        ...
        for (int i : RevRange(n)) {
            for (int j : RevRange(n)) {
                int cur = j + 1;
                int at = l[i];
                maxim(answer(at, cur), answer(at, j) + c[at] - s[i]);
                while (cur > 0) {
                    maxim(answer(at + 1, cur / 2), answer(at, cur) + c[at + 1] * (cur / 2));
                    at++;
                    cur /= 2;
                }
            }
            for (int j : range(l[i], n + m)) {
                maxim(answer(j + 1, 0), answer(j, 0));
            }
        }
        out.printLine(answer(n + m, 0));
    }
};
