...
void solve()
{
    ...
    for (int mask = 0; mask < (1 << f.size()); mask++)
    {
        ...
        {
            if (withBit(mask, i)) res += f[i];
        }
        if (withBit(mask, 0) && withBit((n - res), 0)) continue;
        if (withBit(mask, 1) && withBit((n - res), 1)) continue;
        if (res <= n)
            ans = min(ans, popcountll(mask) + popcountll(n - res));
    }
    cout << ans << "\n";
}
