...
ll n, Q;
ll a[150005];
UnionFind uf(2200005);
map<P, vector<ll> > mp;
ll ans[300005];
ll M = 1100005;

void get(ll t, vector<ll> &vec)
{
	while(prime[t]){
		vec.push_back(prime[t]);
		t /= prime[t];
	}
}
...
int main(void)
{
	...
	rep(i, 1, n){
		vector<ll> res;
		get(a[i], res);
		for(auto p : res) uf.unite(a[i], M+p);
	}
	...
	rep(q, 1, Q){
		...
		if(!uf.same(a[s], a[t])){
			ans[q] = 2;
			ll u = uf.root(a[s]), v = uf.root(a[t]);
			if(u > v) swap(u, v);
			mp[P(u, v)].push_back(q);
		}
	}
	...
	rep(i, 1, n){
		...
		get(a[i]+1, res);
		for(auto p : res){
			ll u = r, v = uf.root(M+p);
			if(u > v) swap(u, v);
			
			if(mp.count(P(u, v))){
				vector<ll> &vec = mp[P(u, v)];
				for(auto x : vec) ans[x] = 1;
				mp.erase(P(u, v));
			}
		}
		uniq(res);
		...
		rep(i, 0, sz(res)-1){
			rep(j, i+1, sz(res)-1){
				ll u = uf.root(res[i]+M), v = uf.root(res[j]+M);
