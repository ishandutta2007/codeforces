...
int main(){
    ...
    FOR(i,1,N+1){
        ...
        A[j]++;
        if(A[j] == 1) {cnt++;}
        if(i == 1) {maxidx = j;maxcnt = 1;m = 1;}
        else{
            if(A[j] == m){maxcnt++;}
            if(A[j] > m) {m=A[j]; maxidx = j; maxcnt = 1;}
        }
        if(cnt == 1) ans = i;
        if(A[maxidx] == 1){ans = i;}
        if(cnt * A[minidx] + 1 == i){ans = i;}
        if(cnt * (A[maxidx] - 1) + 1 == i && maxcnt == 1){ans = i;}
        if( (cnt - 1) * A[maxidx] + 1 == i && maxcnt == cnt - 1){
            ans = i;
        }
        
        trc(A[maxidx],maxcnt,cnt);
    }
    out(ans);
