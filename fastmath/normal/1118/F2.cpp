...
void dfs(int u, int p) {
    ...
    if (w[u]) {
        ...
        int sz = down.size();
        vmi l(sz + 1, 1), r(sz + 1, 1);
        FOR (i, sz) {
            int v = down[i];
            l[i + 1] = l[i] * (dp[v][0]+dp[v][1]);
        }
        ROF (i, sz) {
            int v = down[i];
            r[i] = r[i + 1] * (dp[v][0]+dp[v][1]);
        }

        FOR (i, sz) {
            int v= down[i];
            dp[u][1] += dp[v][1] * l[i] * r[i + 1];
        }
    }

    debug3(u, dp[u][0],dp[u][1]);
    debug(w[u]);
}
...
signed main() {
    ...
    FORN (x, k) {
        ...
        each (e, who[x]) {
            int cur = e;
            while (cur != l && last[cur] == 0) {
                last[cur] = x;
                cur = par[cur];
            }
            if (last[cur] != x && last[cur] != 0) {
                cout << 0 << endl;
                exit(0);
            }
            last[cur] = x;
