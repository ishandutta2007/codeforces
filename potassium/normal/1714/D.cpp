...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			string& now = s[i];
			for (int j = 0; j + (int)now.length() - 1 < m; j++) {
				if (equal(now.begin(), now.end(), t.begin() + j)) {
					seg.push_back({ { j, j + now.length() - 1 }, i });
				}
			}
		}
		...
		int maxr = -1;
		vector<pii> ans;
		pair<pii, int> cur = { { -1, -1 }, -1 };
		while (!seg.empty()) {
			while (!seg.empty() && seg.back().first.first <= r) {
				if (chkmax(maxr, seg.back().first.second))
					cur = seg.back();
				seg.pop_back();
			}
			...
			r = maxr + 1;
			ans.emplace_back(cur.second + 1, cur.first.first + 1);
