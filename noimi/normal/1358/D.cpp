...
signed main(){
    ...
    int ra=0,rb=0;
    ll sum = 0;
    auto calc = [](ll t,ll s){return (t+s)*(s-t+1)/2;};
    while(x){
        if(x >= a[ra]) x-=a[ra++], sum += calc(1,a[ra-1]);
        else rb = x,x=0, sum += calc(a[ra]-rb+1,a[ra]);
    }
    ll ans = sum;
    rep2(i,1,n*2){
        ll d = a[i-1];
        sum -= calc(1,a[i-1]);
        while(ra < n*2 and d){
            if(d >= a[ra] - rb) d -= a[ra++] -rb, sum += calc(1,a[ra-1]-rb),rb=0;
            else sum+=calc(a[ra]-rb-d+1,a[ra]-rb), rb += d, d = 0;
        }
        chmax(ans,sum);
    }
    cout << ans << endl;
