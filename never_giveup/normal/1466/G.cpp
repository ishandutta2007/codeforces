...
const int maxn = 5e5 + 100, maxw = 5e6 + 100, inf = 1e9 + 100, sq = 450, LG = 20, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
int calc_ans(string const &a, string const &b) {
    string c = b + "#" + a;
    auto q = z_function(c);
    int ans = 0;
    for (int i : q)
...
void solve() {
    int n, zaps;
    cin >> n >> zaps;
    vector<Mint> opw(n + 2), pw(n + 2);
    {
        Mint ob = Mint(1) / 2;
        opw[0] = 1;
        pw[0] = 1;
        for (int i = 1; i < opw.size(); i++) {
            opw[i] = opw[i - 1] * ob;
            pw[i] = pw[i - 1] * 2;
        }
    }
    ...
    cin >> s0 >> t;
    {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < 26; j++)
                pref[j][i + 1] = pref[j][i];
            pref[t[i] - 'a'][i + 1] += opw[i + 1];
        }
    }
    auto get = [&](int i, int l, int r) -> Mint {
        if (l > r)
            return 0;
    ...
    while (zaps--) {
        ...
        Mint ans = 0;
        ans += pw[id - pos] * calc_ans(q[pos], s);
        vector<Mint> g(26);
        {
            string o;
            for (int i = s.size() - 1; i > 0; i--)
                o.push_back(q[pos][q[pos].size() - i]);
            int ps = o.size();
            o.push_back(0);
