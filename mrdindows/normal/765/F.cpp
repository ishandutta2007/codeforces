...
void build(Node*& cur, int l, int r) {
    ...
    if (l == r) {
        ...
        build(cur->rp, m + 1, r);
        int siz = r - l + 1;
        std::merge(cur->lp->a, cur->lp->a + m - l + 1, cur->rp->a, cur->rp->a + r - m, cur->a);
        int ans = INF;
        for (int i = 1; i < siz; ++i) {
            ans = min(ans, cur->a[i] - cur->a[i - 1]);
        }
        cur->ans = ans;
    }
}
int getAns(Node* cur, int l, int r, int x, int y);
void update(Node*& cur, int l, int r, int to, int x, int& d) {
    auto old = cur;
    cur = lastn++;
    *cur = *old;
    if (l == r) {
        int newAns = abs(x - cur->a[0]);
        if (newAns < cur->ans) {
            cur->ans = newAns;
            ++cnt;
        }
        ...
        int m = (l + r) / 2;
        int siz = r - l + 1;
        auto it = std::lower_bound(cur->a, cur->a + siz, x);
        if ((it == cur->a + siz || *it >= x + d) && (it == cur->a || *(it - 1) <= x - d)) {
            d = min(d, getAns(cur, l, r, l, to));
            return;
        }
        if (to > m) {
            update(cur->rp, m + 1, r, to, x, d);
            update(cur->lp, l, m, m, x, d);
        } else {
            update(cur->lp, l, m, to, x, d);
        }
        cur->ans = min(cur->ans, min(cur->lp->ans, cur->rp->ans));
    }
}
int getAns(Node* cur, int l, int r, int x, int y) {
    if (l == x && r == y) {
        ...
        int m = (l + r) / 2;
        if (y <= m) {
            return getAns(cur->lp, l, m, x, y);
        } else if (x > m) {
            return getAns(cur->rp, m + 1, r, x, y);
        } else {
            return min(getAns(cur->lp, l, m, x, m), getAns(cur->rp, m + 1, r, m + 1, y));
        }
    }
}
Node* roots[501001];
int solve() {
    ...
    for (int i = 1; i < n; ++i) {
        roots[i] = roots[i - 1];
        int d = INF;
        update(roots[i], 0, n - 1, i - 1, a[i], d);
    }
    int m;
    scanf("%d", &m);
    FI(m) {
