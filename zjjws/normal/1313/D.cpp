...
{
    int l,r,num;
    inline void Read(){rin>>l>>r;return;}
}a[N];
pair<int,int>b[N<<1];
inline void Read(){rin>>n>>m>>k;for(int i=1;i<=n;i++)a[i].Read(),a[i].num=i,b[i]=make_pair(a[i].l,i),b[i+n]=make_pair(a[i].r,i+n);return;}
int cutt;
int c[N<<1];
vector<int>d[N<<1];
int ns[1<<8];
inline void renum(int x,int y){if(x<=n)a[x].l=y;else a[x-n].r=y;return;}
inline void Init()
{
    sort(b+1,b+(n<<1)+1);for(int i=1,j;i<=(n<<1);i=j){c[++cutt]=b[i].first;for(j=i;j<=(n<<1)&&b[j].first==c[cutt];j++)renum(b[j].second,cutt);}
    for(int i=1,l,r;l=a[i].l,r=a[i].r,i<=n;i++)for(int j=l;j<=r;j++)d[j].push_back(i);
    for(int i=0;i<(1<<8);i++)for(int j=0;j<8;j++)if(i&(1<<j))ns[i]++;return;
}
...
int f[1<<8];
int g[1<<8];
inline bool myru_d(int x,int y){return a[x].r==a[y].r?a[x].num<a[y].num:a[x].r<a[y].r;}
inline bool myru_num(int x,int y){return val[x]==val[y]?ath[x]<ath[y]:val[x]<val[y];}
inline void Work()
{
    for(int i=0,ed=(1<<d[1].size());i<ed;i++)for(int j=d[1].size()-1;j>=0;j--)if(!(i&(1<<j))){f[i|(1<<j)]=f[i]^1;}
    for(int i=2;i<=cutt;i++)
    {
        memset(g,0,sizeof(g));int tail=-1,mid=d[i-1].size()-1;
        for(int j=0;j<=mid;j++)num[j]=j,val[j]=a[d[i-1][j]].r,ath[j]=a[d[i-1][j]].num;
        sort(num,num+mid+1,myru_num);sort(d[i].begin(),d[i].end(),myru_d);
        for(int j=0;j<d[i-1].size();j++)if(a[d[i-1][num[j]]].r>=i){mid=j-1;break;}
        for(int j=mid+1,k=0;j<d[i-1].size();j++){for(;k<d[i].size()&&d[i][k]!=d[i-1][num[j]];k++);jh(d[i][++tail],d[i][k]);}
        if(tail==-1)
        {
            int maxl=0;for(int j=0,ed=(1<<mid+1);j<ed;j++)maxl=max(maxl,f[j]);
            for(int j=0,ed=(1<<d[i].size());j<ed;j++)f[j]=(ns[j]&1)+maxl;continue;
        }
        for(int j=0,ed=(1<<tail+1);j<ed;j++)
        {
            for(int k=0,ed=(1<<mid+1);k<ed;k++)
            {
                int ks=k+(j<<mid+1),lst=0;for(int _=0;_<d[i-1].size();_++)if(ks&(1<<_))lst|=(1<<num[_]);
                g[j]=max(g[j],f[lst]+((ns[j]&1)*(c[i]-c[i-1]-1)));
            }
        }
        for(int j=0,ed=(1<<tail+1);j<ed;j++)f[j]=g[j];
        for(int j=(1<<tail+1),ed=(1<<d[i].size()),md=j-1;j<ed;j++)f[j]=f[j&md];
        for(int j=0,ed=(1<<d[i].size());j<ed;j++)f[j]+=(ns[j]&1);
    }
    int ans=0;for(int i=0,ed=(1<<d[cutt].size());i<ed;i++)ans=max(ans,f[i]);printf("%d\n",ans);
    return;
}
int main()
{
