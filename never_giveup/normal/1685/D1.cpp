...
int slow(vector<int> p) {
    ...
    do {
        int cur = 0;
        for (int i = 0; i < n; i++)
            cur += abs(q[i] - p[q[(i + 1) % n]]);
        pair<int, vector<int>> w = {cur, q};
        if (ans > w)
            ans = w;
...
pair<int, pair<int, int>> e[300][300];

int fast(vector<int> p) {
    int n = p.size();
    vector<int> o(n);
    for (int i = 0; i < n; i++)
        o[p[i]] = i;
    ...
    vector<pair<int, pair<int, int>>> du;
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++)
            if (e[i][j].fr != inf)
                du.emplace_back(e[i][j]);
    sort(all(du));
    vector<int> nxt(n);
    iota(all(nxt), 0);
    for (auto [_, w] : du) {
