...
int main() {
  ...
  ll q;
  cin>>q;
  vector<ll> tmp(modPow(2,a,mod)-1);
  vector<ll> t(modPow(2,a,mod)-1);
  ll g=modPow(2,a,mod)-1;
  for(int i=g-1;i>=0;i--){
    if(s[int(s.size())-1-i]=='?'){
      if(i>=modPow(2,a-1,mod)-1) t[i]=2;
      else t[i]=t[i*2+1]+t[i*2+2];
    }
    if(s[int(s.size())-1-i]=='1'){
      if(i>=modPow(2,a-1,mod)-1) t[i]=1;
      else t[i]=t[i*2+1];
    }
    if(s[int(s.size())-1-i]=='0'){
      if(i>=modPow(2,a-1,mod)-1) t[i]=1;
      else t[i]=t[i*2+2];
    }
  }
  while(q--){
    ...
    u--;
    s[u]=z;
    u=modPow(2,a,mod)-2-u;
    while(1){
   //   cout<<"U"<<" "<<u<<endl;
      if(s[int(s.size())-1-u]=='?'){
      if(u>=modPow(2,a-1,mod)-1) t[u]=2;
      else t[u]=t[u*2+1]+t[u*2+2];
    }
    if(s[int(s.size())-1-u]=='1'){
      if(u>=modPow(2,a-1,mod)-1) t[u]=1;
      else t[u]=t[u*2+1];
    }
    if(s[int(s.size())-1-u]=='0'){
      if(u>=modPow(2,a-1,mod)-1) t[u]=1;
      else t[u]=t[u*2+2];
    }
      if(u==0) break;
