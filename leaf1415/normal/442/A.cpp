...
int main(void)
{
	...
	rep(i, 0, (1<<5)-1){
		rep(j, 0, (1<<5)-1){
			rep(k, 0, 5) rep(l, 0, 5) vec[k][l].clear();
			rep(k, 1, n){
				ll x = 5, y = 5;
				if(i & (1<<a[k])) x = a[k];
				if(j & (1<<b[k])) y = b[k];
				vec[x][y].pb(P(a[k], b[k]));
			}
			
			bool flag = true;
			rep(k, 0, 5) rep(l, 0, 5){
				if(sz(vec[k][l]) == 0) continue;
				sort(all(vec[k][l]));
				if(vec[k][l].front() != vec[k][l].back()) flag = false;
			}
			if(flag) chmin(ans, (ll)(popcount(i) + popcount(j)));
		}
	}
	outl(ans);
