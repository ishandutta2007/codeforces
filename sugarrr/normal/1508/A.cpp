...
string che(string a,string b){
   ...
   // cout<<a<<" "<<b<<" "<<dp[n][n]<<endl;
    
    if(dp[n][n]<n/2)return "";
    string ans;
    ll px=n,py=n;
    while(px>0||py>0){
        if(py-1>=0&&dp[px][py]==dp[px][py-1]){
            ans+=b[py-1];py--;
        }else if(px-1>=0&&dp[px][py]==dp[px-1][py]){
            ans+=a[px-1];px--;
        }else{
            assert(a[px-1]==b[py-1]);
            ans+=a[px-1];px--;py--;
        }
    }
    reverse(all(ans));
