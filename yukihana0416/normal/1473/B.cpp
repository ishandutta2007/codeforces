...
void work()
{
    ...
    for(int i = n; i; i--)
    {
        if(s.size() % i == 0 && t.size() % i == 0)
        {
            ...
            if(f(string(s.begin(), s.begin() + i), s.size() / i) == s &&
               f(string(t.begin(), t.begin() + i), t.size() / i) == t &&
               string(s.begin(), s.begin() + i) == string(t.begin(), t.begin() + i))
            {
                cout <<f(string(s.begin(), s.begin() + i), s.size() * t.size() / i / i) <<endl;
                return ;
            }
        }
    }
