...
void solve() {
  ...
  vi pos(N);
  FOR(i, N) pos[P[i]] = i;
  for (auto&& x: A) x = pos[x];

  vvc<int> IDS(N);
  FOR(i, M) IDS[A[i]].eb(i);
  vi nxt(M, -1);
  FOR(i, M) {
    ll x = A[i];
    ll y = (x + 1) % N;
    ll j = LB(IDS[y], i);
    if (j == len(IDS[y])) continue;
    nxt[i] = IDS[y][j];
  }

  Doubling<Group_Add<int>, 20> X(M);
  FOR(i, M) X.set(i, nxt[i], 1);
  X.build();
  FOR(i, M) nxt[i] = X.calc(i, N - 1).fi;

  SegTree<Monoid_Min<int>> seg(M);
  FOR(i, M) {
    if (nxt[i] != -1) seg.set(i, nxt[i]);
  }
