...
int main() {
    ...
    vv(ll, pre, n + 1, 3);
    rep(i, n) {
        suf[i + 1][0] = suf[i][0] + (s[i] != '1');
        suf[i + 1][1] = suf[i][1] + (s[i] == '1');
        suf[i + 1][2] = suf[i][2] + (s[i] == '1' ? x * suf[i][0] : y * suf[i][1]);
    }
    rep3(i, n - 1, 0) {
        pre[i][0] = pre[i + 1][0] + (s[i] == '0');
        pre[i][1] = pre[i + 1][1] + (s[i] != '0');
        pre[i][2] = pre[i + 1][2] + (s[i] == '0' ? x * pre[i][1] : y * pre[i][0]);
    }
    ans = min(pre[0][2], suf[n][2]);
    rep(i, n) { chmin(ans, suf[i][2] + pre[i][2] + x * suf[i][0] * pre[i][1] + y * suf[i][1] * pre[i][0]); }
    cout << ans << endl;
}