...
void _solve() {
    ...
    auto c1 = [](pair<int, int> a, pair<int, int> b) {
        return a.x < b.x;
    };

    auto c2 = [](pair<int, int> a, pair<int, int> b) {
        return a.y < b.y;
    };

    cout << 2 * (max(0, -min_element(all(a), c1)->x) + max(0, -min_element(all(a), c2)->y) +
        max(0, max_element(all(a), c1)->x) + max(0, max_element(all(a), c2)->y)) << endl;
}
