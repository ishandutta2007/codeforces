...
mint solve() {
    ...
    int nhi = count(ALL(A), hi);
    if (nhi != 1) return mint(N).fac();
    for (auto &x : A) if (x == hi) x = -1;
    int nxthi = *max_element(ALL(A));
    int nnxthi = count(ALL(A), nxthi);
    if (hi > nxthi + 1) return 0;
    return mint(N).fac() / (nnxthi + 1) * nnxthi;
}
