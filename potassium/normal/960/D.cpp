...
int main() {
	const int LEVEL = 60;
	vector<ll> shift(LEVEL, 0);
	vector<ll> count(LEVEL, 1);
	...
	CASE(t) {
		...
		if (cmd == 1) {
			...
			shift[level] += k;
			if (shift[level] < 0) shift[level] += count[level];
		}
		else if (cmd == 2) {
			ll x,k; R(x,k);
			int level = __lg(x);
			...
			vector<ll> ans = { x };
			while (level > 0) {
				ll im = x + shift[level] - shift[level - 1] * 2;
				im %= count[level];
				if (im < 0) im += count[level];
				im += count[level];

				ans.emplace_back(im / 2);
				x = im / 2;
				level--;
			}
