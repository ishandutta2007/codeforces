...
void solve() {
    ...
    reverse(ALL(stocks));
    int tmp = ans;
    int base = stocks.size();
    REP(i, inputs.size()) {
        int penalty = (i * d + 1);
        int required_fullfill = (int)inputs.size() - penalty;
        required_fullfill *= -1;
        while (required_fullfill > base - (int)stocks.size() and stocks.empty() == false) {
            tmp -= stocks.back();
            stocks.pop_back();
        }
        if (required_fullfill > base - (int)stocks.size()) break;
        tmp += inputs[i];
        ans = max(ans, tmp);
    }
    cout << ans << endl;
