...
int main() {
    ...
    REP(i, s.size()) {
        if(s[i] == 'U') diff.second += 1;
        if(s[i] == 'D') diff.second -= 1;
        if(s[i] == 'L') diff.first  -= 1;
        if(s[i] == 'R') diff.first  += 1;
    }
    ...
    REP(i, s.size()) {
        ll dx = a - pos.first, dy = b - pos.second;
        
        bool good = true;

        if(diff.first == 0 && diff.second == 0) {
            if(dx != 0 || dy != 0) good = false;
        } else if(diff.first == 0) {
            if(dx != 0 || dy % abs(diff.second) != 0 || dy / diff.second < 0) good = false;
        } else if(diff.second == 0) {
            if(dy != 0 || dx % abs(diff.first)  != 0 || dx / diff.first < 0) good = false;
        } else {
            if(dx % abs(diff.first) != 0 || dy % abs(diff.second) != 0 || dx / diff.first != dy / diff.second || dx / diff.first < 0) good = false;
        }
