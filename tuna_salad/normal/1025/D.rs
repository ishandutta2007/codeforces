...
fn rec(l: usize, r: usize, p: usize, dp: &mut [Vec<Vec<Option<bool>>>], g: &[Vec<bool>]) -> bool {
	assert!(l <= r);
	if let Some(ans) = dp[l][r][p] {
		return ans;
	}
	let mut res = false;
	let j = if p == 0 {l-1} else {r+1};
	for i in l..=r {
		if g[j][i] && (i == l || rec(l,i-1,1,dp,g)) && (i == r || rec(i+1,r,0,dp,g)) {
			res = true;
			break;
		}
	}
	dp[l][r][p].replace(res);
	res
}
...
fn main() {
	...
	let n = rin.u();
	let a = rin.vi(n);
	let mut g = v!([n][n] = false);
	for i in 0..n {
		for j in 0..n {
			g[i][j] = gcd(a[i],a[j]) > 1;
		}
	}
	let mut dp = v!([n][n][2] = None);
	let mut sol = false;
	for i in 0..n {
		if (i == 0 || rec(0,i-1,1,&mut dp,&g)) && (i == n-1 || rec(i+1,n-1,0,&mut dp,&g)) {
			sol = true;
			break;
		}
	}
