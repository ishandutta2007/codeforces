...
public class D {
	...
	void go()
	{
		...
		SegmentTreeRMQ sti = new SegmentTreeRMQ(ih);
		for(int z = 0;z < Q;z++){
			int l = ni()-1, r = ni()-1;
			int minl = st.min(0, l+1);
			int suml = cum[l];
			int minr = st.min(r+1, n+1) - cum[r+1];
			int maxl = -sti.min(0, l+1);
			int maxr = -sti.min(r+1, n+1) - cum[r+1];
			int min = Math.min(minl, suml + minr);
			int max = Math.max(maxl, suml + maxr);
