...
bool in_super(int x, int from, vector<vector<int>> &edges, vector<int> &cmp, int c) {
    if (cmp[x] == 0) return true;
    if (cmp[x] == c) return false;
    cmp[x] = c;
    bool result = false;
    for (int i = 1; i < edges[x].size(); ++i) {
        int L = edges[x][i - 1], R = edges[x][i];
        for (int j = L + 1; j < R; ++j) {
            result |= (j != from && in_super(j, x, edges, cmp, c));
        }
    }
    return result;
}
...
void solve() {
    ...
    for (int i = 0; i < n; ++i) groups[cmp[i]].push_back(i);
    for (int i = 0; i < c; ++i) cout << groups[i].size() << " " << groups[i];
}