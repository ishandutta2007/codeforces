/*-------------------( alex256 )--------------------\
|    Contest: 8VC Venture Cup 2016 - Final Round    |
|       Task: C                                     |
|       Date: 28. 02. 2016                          |
\--------------------------------------------------*/
// Includes
...
template <typename T> class fenwick
{
    ...
    T sum(int l, int r) { if (l > r) return 0; else return sum(r) - sum(l - 1); }
...
main()
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if (id == 1)
        {
            int d = in, p = in; d--;
            order[d] += p;
            before.put(d, min(b, order[d]));
            after.put(d, min(a, order[d]));
        }
        else
        {
            int p = in; p--;
            out = before.sum(0, p - 1) + after.sum(p + k, n - 1);
            out.putLn();
        }
    }
    return 0;
