...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		...
		for(auto y: cur){
			if (trie[node][y] == -1){
				trie[node][y] = trie.size();
				trie.push_back({-1, -1});
				leaf.push_back(false);
				len.push_back(len[node] + 1);
			}
			node = trie[node][y];
		}
		leaf[node] = true;
	...
	auto switch_off = [&] (const auto& self, int node, bool apply) -> void{
		leaf[node] = leaf[node] & apply;
		if (trie[node][1] != -1){
			self(self, trie[node][1], false);
		}
		if (trie[node][0] != -1){
			node = trie[node][0];
			if (trie[node][0] != -1){
				self(self, trie[node][0], false);
			}
		}
	};
	...
	auto dfs = [&] (const auto& self, int node) -> void {
		if (leaf[node]){
			switch_off(switch_off, node, true);
		}

		if (trie[node][0] != -1){
			self(self, trie[node][0]);
		}
		if (trie[node][1] != -1){
			self(self, trie[node][1]);
		}
	};
