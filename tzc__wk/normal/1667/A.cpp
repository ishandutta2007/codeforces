...
#endif
const int MAXN = 5000;
int n, a[MAXN + 5]; ll mn = 1e18;
ll getnxt(ll pv, ll v) {return ((pv / v + 1) * v);}
int main(){
	scanf("%d", &n);
	for (int i = 1; i <= n; i++) scanf("%d", &a[i]);
	for (int i = 1; i <= n; i++) {
		ll pre = 0, sum = 0;
		for (int j = i - 1; j; j--) {pre = getnxt(pre, a[j]); sum += pre / a[j];}
		pre = 0;
		for (int j = i + 1; j <= n; j++) {pre = getnxt(pre, a[j]); sum += pre / a[j];}
		chkmin(mn, sum);
	}
	printf("%lld\n", mn);
	return 0;
