...
int solve() {
    ...
    while (q--) {
        ...
        ll ans = dist(x, y);
        for (auto t : notUsed) {
            ans = min(ans, distTo(x, t.b) + distTo(y, t.c) + t.a);
            ans = min(ans, distTo(x, t.c) + distTo(y, t.b) + t.a);
        }
        cout << ans << '\n';
    }
