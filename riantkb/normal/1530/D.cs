...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            {
                if (lis[i].Count > 1) {
                    lis[i].Sort((x, y) => cnt[y].CompareTo(cnt[x]));
                    for (int j = 0; j < lis[i].Count - 1; j++)
                    {
                        a[lis[i][j]] = true;
                    }
                }
            ...
            var alis = new List<int>();
            var blis = new List<int>();
            ...
            var ans = p.copy();
            for (int i = 0; i < alis.Count; i++)
            {
                ans[alis[i]] = blis[(i + 1) % blis.Count];
            }
            Prt(n - alis.Count);
