// Adapted from TemplateLong.cpp by Benjamin Qi.
// Everything else is written by Zhikun Wang.
...
inline namespace Output {
	template<class T> void pr_sep(ostream& os, str, const T& t) { os << ts(t); } template<class T, class... U> void pr_sep(ostream& os, str sep, const T& t, const U&... u) {pr_sep(os,sep,t); os << sep; pr_sep(os,sep,u...); } template<class ...T> void pr(const T&... t) { pr_sep(cout,"",t...); } void ps() { cout << "\n"; } template<class ...T> void ps(const T&... t) { pr_sep(cout," ",t...); ps(); } template<class ...T> void dbg_out(const T&... t) { pr_sep(cerr," | ",t...); cerr << endl; }void loc_info(int line, str names) { cerr << "Line(" << line << ") -> [" << names << "]: "; }
	template<int lev, class T> void dbgl_out(const T& t) { cerr << "\n\n" << ts_sep(ts_lev<lev>(t),"\n") << "\n" << endl; }
	#ifdef LOCAL
...
int p[N],cp[N],lu[N],ld[N],ru[N],rd[N];
int ans = 0;
void lku(int l,int r){ru[l] = r;ld[r] = l; ans++;}
void lkd(int l,int r){rd[l] = r;lu[r] = l; ans++;}
void cu(int l,int r){ans-=(ru[l]== r && ld[r] == l);}
void cd(int l,int r){ans-=(rd[l]== r && lu[r] == l);}
void chk(int l,int r){
	//dbg(l,r);
	if(p[l]<p[r]){
		if(p[l]>p[ld[r]]){cu(l,ru[l]);cu(ld[r],r);lku(l,r);}
	}else{
		if(p[r]>p[rd[l]]){cd(l,rd[l]);cd(lu[r],r);lkd(l,r);}
	}
}
int main() {
	cin.tie(0)->sync_with_stdio(0);
		...
		re(n,q);
	a.rsz(n); re(a);
	l.rsz(q); r.rsz(q); F0R(i,q) re(l[i],r[i]);
