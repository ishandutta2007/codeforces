...
int main()
{
	...
	for(int i=0;i<=18;i++)
		for(int j=1;j<=9;j++)
			fuck.pb(j*mii[i]);
	for(int i=0;i<=18;i++)
		for(int ii=i+1;ii<=18;ii++)
			for(int j=1;j<=9;j++)
				for(int jj=1;jj<=9;jj++)
					fuck.pb(j*mii[i]+jj*mii[ii]);
	for(int i=0;i<=18;i++)
		for(int ii=i+1;ii<=18;ii++)
			for(int iii=ii+1;iii<=18;iii++)
				for(int j=1;j<=9;j++)
					for(int jj=1;jj<=9;jj++)
						for(int jjj=1;jjj<=9;jjj++)
							fuck.pb(j*mii[i]+jj*mii[ii]+jjj*mii[iii]);
	sort(ALL(fuck));
	for(int i=0;i<n;i++)
	{
		INT a,b;
		fio>>a>>b<<(upper_bound(ALL(fuck),b)-lower_bound(ALL(fuck),a))<<endl;
	}
	return 0;
}