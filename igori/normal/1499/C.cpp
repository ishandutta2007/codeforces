...
void solve()
{
    ...
    ll mn0 = 1e9 + 2, mn1 = 1e9 + 2;
    ll si0 = 0, si1 = 0, cnt0 = 0, cnt1 = 0;

    for (int i = 0; i < n; i++)
    {
        if (i % 2 == 0) mn0 = min(mn0, a[i]), si0 += a[i], cnt0++;
        else mn1 = min(mn1, a[i]), si1 += a[i], cnt1++;

        ans = min(ans, si0 + si1 + (n - cnt0) * mn0 + (n - cnt1) * mn1);
    }
