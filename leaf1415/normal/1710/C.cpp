...
int main(void)
{
	...
	dp[0][1][1][1][0][0][0][0] = 1;
	rep(i, 0, n-1){
		ll d = s[i]-'0';
		rep(A, 0, 1) rep(B, 0, 1) rep(C, 0, 1){
			rep(X, 0, 1) rep(Y, 0, 1) rep(Z, 0, 1) rep(W, 0, 1){
				if(dp[i][A][B][C][X][Y][Z][W] == 0) continue;
				rep(a, 0, 1) rep(b, 0, 1) rep(c, 0, 1){
					ll x = a^b, y = b^c, z = c^a;
					ll p = (x+y+W)/2, q = (x+y+W)%2;
					ll nA = A, nB = B, nC = C, nX = X, nY = Y, nZ = Z;
					if(a < d) nA = 1; if(a > d) nA = 0;
					if(b < d) nB = 1; if(b > d) nB = 0;
					if(c < d) nC = 1; if(c > d) nC = 0;
					if(x < y) nX = 1; if(x > y) nX = 0;
					if(y < z) nY = 1; if(y > z) nY = 0;
					if(q > z) nZ = 1; if(q < z) nZ = 0;
					dp[i+1][nA][nB][nC][nX][nY][nZ][p] += dp[i][A][B][C][X][Y][Z][W];
				}
			}
		}
	}
	outl(dp[n][1][1][1][1][1][1][0] * mint(6));
