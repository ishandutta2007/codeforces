#line 1 "codeforces-workspace/contest/1491/e/e.cpp"
...
template <class _F> class _non_recursive {
 ...
 public:
  _non_recursive(_F __fn) noexcept : __fn(__fn), __cptr(new cache) {}
...
#line 11 "codeforces-workspace/contest/1491/e/e.cpp"
...
void main() {
  ...
  while (fib.back() < n) {
    auto m = fib.size();
    fib.emplace_back(fib[m - 1] + fib[m - 2]);
  }
  if (fib.back() != n) {
    eject("NO");
  }
  ...
  for (auto i : range(1, n)) {
    ...
    cin >> a >> b;
    --a, --b;
    tree[a].push_back({a, b, i});
    tree[b].push_back({b, a, i});
  }

  auto ok = vector<int>(n, 1);

  auto check = fixed_point([&](auto check, int k, int root) -> bool {
    if (k < 4) return true;

    vector<edge> cnd;

    auto dfs = fixed_point([&](auto self, int now, int pre) -> int {
      int sz = 1;
      ...
      for (auto&& e : tree[now]) {
        ...
        if (ok[id]) {
          ...
          if (cz == fib[k - 2]) {
            ...
            cnd.emplace_back(edge{to, now, id});
          }
    ...
    for (auto&& e : cnd) {
      ok[e.id] = false;
      return check(k - 1, e.src) && check(k - 2, e.to);
    }

    return false;
  });

  if (check(fib.size() - 1, 0))
    cout << "YES\n";
  else
    cout << "NO\n";
}
