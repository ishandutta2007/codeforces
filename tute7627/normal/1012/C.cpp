...
int main(){
  ...
  rep(i,0,n)cin>>a[i+1];
  ll h=(n+1)/2;
  rep(i,0,n+3)rep(j,0,h+1)rep(o,0,3)dp[i][j][o]=inf;
  dp[1][0][2]=0;
  dp[1][0][0]=0;
  //1:i-1 2:i-1
  rep(i,1,n+2){
    ll lc=max(0,a[i]-(a[i-1]-1));
    ll rc=max(0,a[i]-(a[i+1]-1));
    //cout<<a[i] spa lc spa rc<<endl;
    rep(j,0,h+1){
      //
      if(j<h){
        chmin(dp[i+1][j+1][0],dp[i][j][1]+lc);
        chmin(dp[i+1][j+1][2],dp[i][j][1]+max(lc,rc));
      }
      chmin(dp[i+1][j][2],dp[i][j][0]+rc);
      //
  ...
  //debug(dp,n+3,h+1);
  vector<ll>ret(h);
  rep(i,1,h+1)ret[i-1]=dp[n+2][i][0];
  debug(ret,h);
  return 0;
}