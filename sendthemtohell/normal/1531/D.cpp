...
void mrg(zal& a, const zal& b, const zal& c) {
    if (c.do_lock == 0) {
        a.do_lock = b.do_lock;
    }
    else {
    ...
    string lck = b.if_lock, unlck = b.if_unlock;
...
void build(int v, int l, int r, vector<string> &a) {
    if (r - l == 1) {
        t[v].if_lock = "";
        t[v].if_unlock = "";
        t[v].do_lock = 0;
    ...
    build(v << 1, l, (l + r) >> 1, a);
    build(v << 1 | 1, (l + r) >> 1, r, a);
    mrg(t[v], t[v << 1], t[v << 1 | 1]);
}

zal get_t(int v, int l, int r, int L, int R) {
    if (R <= l || r <= L) {
        return { "", "", 0 };
    }
    if (L <= l && r <= R) {
    ...
    zal ret;
    mrg(ret, get_t(v << 1, l, (l + r) >> 1, L, R), get_t(v << 1 | 1, (l + r) >> 1, r, L, R));
    return ret;
}

void set_t(int v, int l, int r, int at, string &a) {
    if (at < l || at >= r) {
        return;
    }
    if (r - l == 1) {
    ...
    set_t(v << 1, l, (l + r) >> 1, at, a);
    set_t(v << 1 | 1, (l + r) >> 1, r, at, a);
    mrg(t[v], t[v << 1], t[v << 1 | 1]);
}
...
inline void solve() {
    ...
    cin >> t;
    build(1, 0, n, a);
    auto z = get_t(1, 0, n, 0, n);
    if (sz(z.if_unlock)) {
        cout << z.if_unlock << endl;
    }
    else {
        cout << "blue\n";
    }
