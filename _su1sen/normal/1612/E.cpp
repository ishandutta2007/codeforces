...
int main() {
    ...
    rep(t, 1, K + 1) {
        ...
        rep(j, n) s[m[j]] += min(t, k[j]);
        sort(all(s), greater<long long>());
        long long sum = accumulate(s.begin(), s.begin() + t, 0LL);
        if (sum * y > t * x) {
            x = sum;
            y = t;
            argmax = t;
    ...
    rep(j, n) s[m[j]].first += min(t, k[j]);
    sort(all(s), greater<pair<long long, int>>());
    vector<int> ans;
    rep(i, t) {
        ans.push_back(s[i].second + 1);
    }
    print(t);
    print(ans);
    // print(x, y);
