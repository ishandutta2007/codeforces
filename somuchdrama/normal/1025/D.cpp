...
const int N=707;
int dl[N][N],dr[N][N];//l..r
int g[N][N];
int a[N];
bool getr(int l, int r);
bool getl(int l, int r){
    if(dl[l][r]==-1){
        bool f=0;
        for(int i=l;!f&&i<r;++i){
            if(g[i][r]!=1){
                f|=(getl(l,i)&getr(i,r-1));
            }
        }
        dl[l][r]=f;
    }
...
bool getr(int l, int r){
    if(dr[l][r]==-1){
        bool f=0;
        for(int i=l+1;!f&&i<=r;++i){
            if(g[l][i]!=1){
                f|=(getl(l+1,i)&getr(i,r));
            }
        }
        dr[l][r]=f;
    }
