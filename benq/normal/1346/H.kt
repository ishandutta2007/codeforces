...
val todo = Array<ArrayList<Int>>(2000005,{ArrayList<Int>()})

fun atMost(a: ArrayList<Int>, len: Int): Int {
    var (lo,hi) = arrayOf(0,a.size) // first greater
    while (lo < hi) {
        val mid = (lo+hi)/2
        if (a[mid] > len) hi = mid
        else lo = mid+1
...
fun solve(a: Pair<Int,Int>) {
    var ans = atMost(todo[a.first+a.second],a.second-a.first)
    val l = atMost(todo[a.first+a.second-2],a.second-a.first-2)
    val r = atMost(todo[a.first+a.second+2],a.second-a.first-2)
    ans = maxOf(ans,minOf(l,r))
    print(if (ans == -MOD) -1 else (a.second-a.first-ans)/2)
    print(' ')
}

fun main() {
    val (n,m) = rInts(2)
    val query = ArrayList<Pair<Int,Int>>()
    for (i in 1..n) {
        val (l,r) = rInts(2)
        query.add(Pair<Int,Int>(l,r))
    }
    for (i in 1..m) {
        val (L,R) = rInts(2)
        todo[L+R].add(R-L)
    }
    for(i in 0..2000000) todo[i].sort()
    for (a in query) solve(a)
}