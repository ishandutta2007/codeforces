...
pair<pll,pll> dfs(ll x,ll y,V(ll) ad[])
{
    pair<pll,pll> ans=MP(MP(1LL,1LL),MP(1LL,1LL));
    ll a=1,b=1;
    for(auto it:ad[x])
    {
        if(it!=y)
        {
            pair<pll,pll> t=dfs(it,x,ad);
            ans.F.F*=(t.F.S+t.S.S+M2-t.F.F)%M2;
            ans.F.F%=M2;
            ans.F.S*=(2*t.F.S+2*t.S.S+M2-t.F.F)%M2;
            ans.F.S%=M2;
            ans.S.F*=(t.F.S+t.S.S+M2-t.F.F)%M2;
            ans.S.F%=M2;
            ans.S.S*=(2*t.F.S+t.S.S+M2-t.F.F)%M2;
            ans.S.S%=M2;
        }
    }
    // cout<<'\n'<<x+1<<' '<<y+1<<'\n'<<'\n';
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        pair<pll,pll> t=dfs(0,-1,ad);
        cout<<(t.F.S+t.S.S+M2-t.F.F+M2-1)%M2;
