	...
	for (int i = 1; i <= q; ++i) {
		...
		if (pos[t[i]] != 0) {
			...
			if (L.first == -1 && R.first == -1) {
				...
				segs.erase(make_pair( - (R.second - R.first + 1), make_pair(-R.first, R.second)));
				segs.insert(make_pair( - (R.second - m + 1), make_pair(-m, R.second)));
				remove(R.first, &byLeft);
				remove(R.second, &byRight);
				byLeft.insert(make_pair(m, R.second));
				byRight.insert(make_pair(R.second, m));
			} else if (L.first != -1 && R.first == -1) {
				segs.erase(make_pair( - (L.second - L.first + 1), make_pair(-L.first, L.second)));
				segs.insert(make_pair( - (m - L.first + 1), make_pair(-L.first, m)));
				...
				remove(L.first, &byLeft);
				remove(L.second, &byRight);
				segs.insert(make_pair( - (R.second - L.first + 1), make_pair(-L.first, R.second)));
