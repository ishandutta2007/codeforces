...
int main() {
    ...
    vi cost_set(sets), cost_color(colors);
    for (int i = 0; i < sets; i++) cin >> cost_set[i];
    for (int j = 0; j < colors; j++) cin >> cost_color[j];

    ll tot_cst = 0;
    par = vi(sets+colors,-1);
    ...
    for (int s = 0; s < sets; s++) {
        ...
        for (int _ = 0; _ < tam; _++) {
            int c; cin >> c; c--;
            ll cst = cost_set[s] + cost_color[c];
            tot_cst += cst;
            edges.push_back({-cst, {s, sets+c}});
        }
    }

    sort(all(edges));
    for (auto [cst, e] : edges) {
