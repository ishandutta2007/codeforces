...
bool can[1 << kax];
int spl[1 << kax];
vector <pll> ans[1 << kax];
pii finans[kax];
void go(int x) {
	if (!ans[x].empty()) {
		int l = ans[x].size();
		for (int i = 0; i < l; ++i) {
			finans[ans[x][i].second] = {ans[x][i].first, ans[x][(i + l - 1) % l].second};
		}
	}
	else {
		go(spl[x]);
...
int main() {
	...
	for (pll x : where) {
		...
		bool fail = false;
		do {
			ll need = sum - (bsum[y.second] - y.first);
			debug(arr(bsum, y.second), imie(y), imie(need));
		...
		while (x != y && (int)curr.size() < k);
		if (!fail && x == y) {
			int mask = 0;
			for (pll p : curr) {
				mask |= (1 << p.second);
			}
			if (popc(mask) == (int) curr.size()) {
				debug(imie(curr));
				can[mask] = 1;
				debug(imask(mask));
				ans[mask] = curr;
	...
	if (can[all]) {
		...
		printf("Yes\n");
		for (int i = 0; i < k; ++i) {
			printf("%d %d\n", finans[i].first, finans[i].second + 1);
		}
	}
	else err();
}