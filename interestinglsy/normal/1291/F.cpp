...
void Solve(){
	const int bsz = k == 1 ? 1 : k/2;
	const int bcnt = n/bsz;
	auto GetLR = [&]( int id )->pii{
		return { id*bsz-bsz+1 , id*bsz };
	};
	For(st,bcnt){
		Reset();
		int delta = 0;
		For(_t,bcnt){
			int cur = (st+delta+bcnt+bcnt-1)%bcnt + 1;
			auto [bL,bR] = GetLR(cur);
			Forx(i,bL,bR) if(!isDead[i]) isDead[i] |= Ask(i);
			if( delta >= 0 ) ++delta;
			delta = -delta;
		}
	}
