...
int h[MN],n; char s[MN];
bool b[P+5]; 
int f[MN],g[MN];
int main () {
	n=read(); FI(s+1);
	For(i,1,n) h[i]=s[i]-'a'+1;
	int sq=sqrt(n<<1)+1,sum=1;
	For(i,0,MN-1) g[i]=1; 
	For(ans,2,sq) {
		sum+=ans; int fl=0; 
		foR(i,n-sum+1,1) {
			int p=i+ans;
			if(g[p]) b[h[p]]=1;
			if(b[h[i]]||b[h[i+1]]) f[i]=1,fl=1;
		}
		if(!fl) return printf("%d\n",ans-1),0;
		foR(i,n-sum+1,1) b[h[i+ans]]=0;
		memcpy(g,f,sizeof(f)); mst(f,0); 
		For(i,1,n-sum+1) h[i]=del((ll)h[i]*B%P+s[i+ans-1]-'a'+1);
	}
	printf("%d\n",sq);
