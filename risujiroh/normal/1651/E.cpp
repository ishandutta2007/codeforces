...
void main(int) {
  ...
  i64 ans = 0;
  vla vis(2 * n, false);
  for (int i : rep(n).filter(!vis[_1])) {
    vis[i] = true;
    basic_string<int> s{i};
    {
      int x = i;
      int y = g[x][0];
      while (true) {
    ...
    for (int l : rep(m)) {
      set<int> se;
      for (int r : rep(l, l + m - 1)) {
        se.insert(s[r]);
        if (l == r) { continue; }
        int x = g[s[l]][0] ^ g[s[l]][1] ^ s[l + 1];
        int y = g[s[r]][0] ^ g[s[r]][1] ^ s[r - 1];
        if (*begin(se) < x && x < *prev(se.lower_bound(n))) { continue; }
        if (*begin(se) < y && y < *prev(se.lower_bound(n))) { continue; }
        if (*se.lower_bound(n) < x && x < *prev(end(se))) { continue; }
        if (*se.lower_bound(n) < y && y < *prev(end(se))) { continue; }
        i64 cur = 1;
        {
          int t = *begin(se);
          int mx = -1;
        ...
        {
          int t = *prev(se.lower_bound(n));
          int mn = n;
          if (t < x && x < n) { chmin(mn, x); }
          if (t < y && y < n) { chmin(mn, y); }
          cur *= mn - t;
        }
        {
          int t = *se.lower_bound(n);
          int mx = n - 1;
          if (n <= x && x < t) { chmax(mx, x); }
          if (n <= y && y < t) { chmax(mx, y); }
          cur *= t - mx;
        }
        {
          int t = *prev(end(se));
      ...
      if (l == 0) {
        ...
        i64 cur = 1;
        cur *= *begin(se) + 1;
        cur *= n - *prev(se.lower_bound(n));
        cur *= *se.lower_bound(n) - n + 1;
        cur *= 2 * n - *prev(end(se));
        ans += m / 2 * cur;
      }
    }
