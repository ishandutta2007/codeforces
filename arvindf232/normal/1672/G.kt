...
fun want(s:Int, have:Int, want:Int):Int {
    val actual = (want - have + 3000) and 1
    if(s == 0){
        if(actual == 0){
            return 1
        }else{
...
fun main(){
    solve.cases{
        ...
        if(c %2 == 0){
            val transposed = Array(c){IntArray(r)}
            for(i in 0 until r){
                for(j in 0 until c){
        ...
        if(r % 2 == 0){
            ...
            for(type in 0 until 2){
                ...
                for(i in 0 until r) {
                    ...
                    options = options modM want(q,base,type)
                }
                total = total modPlus options
            }
            put(total)
            return@cases
        }else{
            var ret = 0
            nexttype@for(type in 0 until 2){
                var qmarks = qus
                val new = Array(r){board[it].copyOf()}
                val Q = fastEmptyList(4001)

                val ractive = BooleanArray(r)
                val cactive = BooleanArray(c)

                val rcount = IntArray(r)
                ...
                nextdelete@while(Q.isNotEmpty()){
                    ...
                    if(v < r){
                        ...
                        qmarks--
                        val needfix = (here + type) % 2
                        new[v][skipped] = needfix
                        ccount[skipped] --
                        if(ccount[skipped] == 1){
                            Q.addLast(r + skipped)
                        }
                ...
                val allcomps = mutableSetOf<Int>()
                val rwant = mutableMapOf<Int,Int>()
                val cwant = mutableMapOf<Int,Int>()
                var active = 0
                for(i in 0 until r){
                    if(ractive[i]) {
                        var here = 0
                        ...
                        val need = (here + type) % 2
                        val comp = s.find(i)
                        rwant[comp] = rwant.getOrDefault(comp,0) + need
                        active ++
                        allcomps.add(comp)
                    }else{
                        var here = 0
                ...
                for(j in 0 until c){
                    if(cactive[j]) {
                        ...
                        val need = (here + type) %2
                        val comp = s.find(j + r)
                        cwant[comp] = cwant.getOrDefault(comp,0) + need
                        active ++
                        allcomps.add(comp)
                    }else{
                        var here = 0
                ...
                for(c in allcomps){
                    if(rwant.getOrDefault(c,0) %2 != cwant.getOrDefault(c,0) %2 ){
                        continue@nexttype
                    }
                }
                active -= allcomps.size
                ret = ret modPlus intPow(2,qmarks - active, pI)
            }
            put(ret)
            return@cases
        }
