...
void solve(){
    ...
    rep(i,0,n-1)a[i] = color(a[i]);
    ll ans=0;
    rep(i,0,e-1){
        vl v;
        auto calc=[](vl v,ll lim){

            ll n=v.size();
            ll ans=0;
            vl s(n+1);rep(i,1,n)s[i]=s[i-1]+v[i-1];
            ll r=0;
            rep(l,0,n-1){
                //[l,r)lim
                while(r<=n-1 && s[r+1]-s[l]<=lim){
                    r++;
                }
