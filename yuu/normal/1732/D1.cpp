...
vector<tuple<int64_t, int, int>> appearance;
map<int64_t, int> current;
vector<pair<int, int64_t>> query;

class segment_tree_t {
public:
    segment_tree_t *left, *right;
    int l, r, m;
    vector<int64_t> contains;
    ...
    void add_segment(const int64_t& x, int u, int v) {
        if (l > v || r < u) return;
        if ((u <= l) && (v >= r)) {
            contains.emplace_back(x);
        } else {
            left->add_segment(x, u, v);
            right->add_segment(x, u, v);
        }
    }
    vector<pair<int64_t, int64_t>> check;
    inline static map<int64_t, segment_tree_t*> current;
    inline static map<int64_t, int64_t> ans;
    void solve() {
        for (auto&& x : contains) current.emplace(x, this);
        if (l == r) {
            auto&& k = query[l].second;
            int64_t f = ans[k];
            if (f == 0) f = k;
            while (true) {
                auto it = current.find(f);
                if (it != current.end()) {
                    it->second->check.emplace_back(k, f);
                    f += k;
                } else {
                    break;
                }
        ...
        for (auto&& x : contains) current.erase(x);
        for (auto&& [x, y] : check) {
            if (ans[x] > y) ans[x] = y;
        }
    }
...
void solve() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        cin >> c >> x;
        if (c == '?') {
            query.emplace_back(i, x);
        } else if (c == '-') {
            appearance.emplace_back(x, current[x], i);
            current.erase(x);
        } else {
            current[x] = i;
        }
    }
    for (auto&& [x, pos] : current) appearance.emplace_back(x, pos, q);
    auto tree = new segment_tree_t(0, query.size() - 1);
    for (auto&& [x, l, r] : appearance) {
        if (r < query[0].first) continue;
        if (l > query.back().first) continue;
        int first = lower_bound(query.begin(), query.end(), pair<int, int64_t>(l, -1)) - query.begin();
        int last = upper_bound(query.begin(), query.end(), pair<int, int64_t>(r, 1e18 + 1)) - query.begin() - 1;
        if (first > last) continue;
        tree->add_segment(x, first, last);
    }
    segment_tree_t::current.emplace(0, tree);
    tree->solve();
}
