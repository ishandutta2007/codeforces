...
vl solve(vector<complex<dd>>&v){
    ...
    rep(T,0,n/2-1){
        ll i=T*2;
        ll j=i+1;
        vector<complex<dd>>t(4);
        rep(k,0,3){
            t[k]=P;
            if(k&1)t[k]+=v[i];else t[k]-=v[i];
            if(k/2)t[k]+=v[j];else t[k]-=v[j];
        }
        ll mn=0;
        rep(k,1,3){
            if(abs(t[k])<abs(t[mn]))mn=k;
        }
        if(mn&1)ans[i]=1;else ans[i]=-1;
        if(mn/2)ans[j]=1;else ans[j]=-1;
        if(ans[i]==1)P+=v[i];else P-=v[i];
        if(ans[j]==1)P+=v[j];else P-=v[j];
    }
    if(abs(P)>=1500000)return {};
    return ans;
...
signed main(){fastio
    ll n;cin>>n;
    vector<complex<dd>>v(n);
    rep(i,0,n-1){
        dd a,b;cin>>a>>b;
        v[i]={a,b};
    }
    vl jun(n);rep(i,0,n-1)jun[i]=i;
    rep(z,1,10){
        random_shuffle(all(jun));
        vector<complex<dd>>a(n);
        rep(i,0,n-1){
            a[i]=v[jun[i]];
        }
        vl ret=solve(a);
