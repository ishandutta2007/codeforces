...
i32 r[2020][2020];
i32 c[2020][2020];
mint dp[2001][2001][2]; // 0:down, 1:right
mint dp1[2002][2001],dp0[2001][2001]; // sum of dp, 0:row and down, 1:column and right
...
void __solve()
{
    ...
    for(i32 i=n-1; i>=0; --i)
    {
        for(i32 j=m-1; j>=0; --j)
        {
            c[i][j]=c[i+1][j]+(g[i][j]=='R');
        }
    }

    auto is=[&](i32 i,i32 j)->bool
    {
        return g[i][j]=='R';
    };
    ...
    for(i32 i=n-1; i>=0; --i)
    {
        for(i32 j=m-1; j>=0; --j)
        {
            ...
            // update dp0 and dp1
            dp0[i][j]=dp0[i+1][j]+dp[i][j][0];
            dp1[i][j]=dp1[i][j+1]+dp[i][j][1];

            dump(i,j,dp[i][j][0],dp[i][j][1]);
        }
    }
