...
int main() {
	...
	vector<int> mobius(MAXN, 0);
	mobius[1] = 1;
	for (int i = 2; i < MAXN; i++) {
		int mpf = divisors[i][1];
		int last = i / mpf;
		mobius[i] = (last % mpf == 0 ? 0 : mobius[last] * -1);
	}
	...
	auto pop = [&](int v) {
		chkmax(ans, lcm(stk.back(), v));
		for (auto& d : divisors[stk.back()])
			cnt[d]--;
		stk.pop_back();
	};

	auto has_coprime = [&](int v) {
		int count = 0;
		for (auto& d : divisors[v])
			count += mobius[d] * cnt[d];
		return (count > 0);
	};
