...
int main() {
    ...
    GS(s)
    ll n = s.size();
    F(i, 1, n + 1) cts[i][s[i - 1] - 'a']++;
    F(i, 1, n + 1) F(j, 0, 26) cts[i][j] += cts[i - 1][j];
    //F(i, 1, n + 1) PN(cts[i], 26);
    G(q) while(q--) {
        G(l) G(r)
        ll x = 0;
        F(i, 0, 26) if(cts[r][i] - cts[l - 1][i]) x++;
        cout << ((x == 1 || (x == 2 && (s[r - 1] == s[l - 1]))) && r - l ? "No" : "Yes") << endl;
    }
}