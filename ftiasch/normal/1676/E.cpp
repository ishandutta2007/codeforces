...
#line 2 "/tmp/tmp-141040SESYnjKHgBtI.cpp"

#line 4 "/tmp/tmp-141040SESYnjKHgBtI.cpp"
...
int main() {
  ...
  while (T--) {
    ...
    while (q--) {
      int x = read();
      auto iterator = std::ranges::lower_bound(sum, x);
      std::cout << (iterator == sum.end() ? -1 : iterator - sum.begin())
                << "\n";
    }
  }
}