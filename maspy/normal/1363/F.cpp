...
void solve() {
  ...
  FOR(s, 26) CNT_S[s] = cumsum(CNT_S[s], 0);
  FOR(s, 26) CNT_T[s] = cumsum(CNT_T[s], 0);

  auto ok = [&](int i, int j) -> bool {
    if (S[i] != T[j]) return false;
    FOR(s, 26) if (CNT_S[s][i] > CNT_T[s][j]) return false;
    return true;
  };
  FOR(i, N) {
    FOR_R(j, N) if (ok(i, j)) { chmax(dp[j + 1], dp[j] + 1); }
    FOR(j, N) chmax(dp[j + 1], dp[j]);
  }
  ll ANS = N - dp[N];
  sort(all(S));
  sort(all(T));
