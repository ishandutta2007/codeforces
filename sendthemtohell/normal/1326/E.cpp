...
void add_t(int v, int l, int r, int ql, int qr, int vl) {
	push(v, l, r);
	if (qr <= l || ql >= r) {
		return;
	}
	...
	add_t(v << 1, l, (l + r) >> 1, ql, qr, vl);
	add_t(v << 1 | 1, (l + r) >> 1, r, ql, qr, vl);
...
inline void solve() {
	...
	int ans = n;
	vector<int> pr = { ans };
	add_t(1, 0, sz(q), 0, p.back().y + 1, 1);
	p.pop_back();
	for (int i = 0; i < n - 1; ++i) {
		add_t(1, 0, sz(q), 0, q[i], -1);
		while (get_t() == 0) {
			--ans;
			add_t(1, 0, sz(q), 0, p.back().y + 1, 1);
			p.pop_back();
