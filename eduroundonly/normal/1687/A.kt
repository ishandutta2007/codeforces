...
fun main(){
    solve.cases{
        ...
        val realk = minOf(k,n)
        val bonus = k - realk
        val fullbonus = n.toLong() * bonus + (realk-1).toLong() * (realk).toLong() / 2
        ...
        for(i in 0 until n){
            val right = i + realk - 1
            if(right !in 0 until n) continue
            ret = maxOf(ret, ss.sumQuery(i,right) + fullbonus )
        }
        put(ret)
