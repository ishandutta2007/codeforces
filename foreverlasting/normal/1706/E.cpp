...
namespace MAIN {
	int n,m,q;
	vector<int> G[N];
	int fa[N],a[N],dfn[N],idx,F[N][21],dep[N],mn[N][21],mx[N][21],Lg[N],pos[N];
	inline void MAIN(const int &Case) {
		...
		for(int i=1;i<=m;i++){
			...
			if(fu==fv)continue;
			cnt++,G[cnt].pb(fu),G[cnt].pb(fv),fa[fu]=fa[fv]=fa[cnt]=cnt,a[cnt]=i;
		}
		idx=0;
		function<void(int,int,int)> dfs=[&](int x,int fax,int depx){
			pos[dfn[x]=++idx]=x,F[x][0]=fax,dep[x]=depx;
			for(int i=1;i<=20;i++)F[x][i]=F[F[x][i-1]][i-1];
			for(auto tox:G[x])dfs(tox,x,depx+1);
		};
		dfs(cnt,0,0);
		...
		for(int i=1;i<=n;i++)mn[i][0]=mx[i][0]=dfn[i];
		...
		auto gmn=[&](int l,int r){
			int k=Lg[r-l+1];
			return min(mn[l][k],mn[r-(1<<k)+1][k]);
		};
		auto gmx=[&](int l,int r){
			int k=Lg[r-l+1];
...
//			printf("%d %d\n",Mn,Mx);
			int lca=glca(pos[Mn],pos[Mx]);
			printf("%d ",a[lca]);
		}
		puts("");
		for(int i=1;i<=cnt;i++)vector<int>().swap(G[i]);
	}
}
