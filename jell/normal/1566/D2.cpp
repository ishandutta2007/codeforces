#line 1 "codeforces-workspace\\contest\\1566\\d2\\d2.cpp"
...
#line 13 "codeforces-workspace\\contest\\1566\\d2\\d2.cpp"
...
void main() {
  ...
  map<int, int> pos;
  for (auto &&[i, e] : reversed(enumerate(all))) {
    pos[e] = i;
  }

  using namespace __gnu_pbds;
  vector<tree<i64, null_type, less<i64>, rb_tree_tag,
  ...
  for (auto &&x : a) {
    auto v = x * n * m - pos[x];
    auto &t = trees[pos[x]++ / m];
    ans += t.order_of_key(v);
    t.insert(v);
  }
  print(ans);
}
