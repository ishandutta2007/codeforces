...
int main() {
    TEST {
        ...
        fore(x, y, mp) v.eb(y, x, 0);
        SORT(v);
        rep(i, si(v)) get<2>(v[i]) = i, swap(get<0>(v[i]), get<1>(v[i]));
        SORT(v);
        // REV(v);
        int t = si(v);
        rep(i, si(v)) {
            int ret = get<0>(v[i]) - i;
            if(ret > k or get<0>(v[i]) >= n) {
                t = i;
                break;
            }
        }
        vi w;
        rep(i, t, si(v)) { w.eb(get<1>(v[i])); }
        SORT(w);
        int res = si(v) - t;
        fore(e, w) {
            if(k >= e) {
        ...
        // RST<int> seg(si(v)), seg2(si(v));

        // int mi = inf<int>;
        // rep(i, si(v) - 1) {
        //     int t = seg.max_right(0, [&](int x) { return x <= k; });
        //     int s = seg2.prod(0, t);
        //     dump(t, s);
        //     if(i < si(v) - 1) {
        //         int ret = get<0>(v[i + 1]) - (si(v) - i - 2);
        //         if(ret <= k) { chmin(mi, (i - s)); }
        //     } else {
        //         chmin(mi, i - s);
        //     }
        //     seg.set(get<2>(v[i]), get<1>(v[i]));
        //     seg2.set(get<2>(v[i]), 1);
        // }
        // OUT(mi);
    }
}