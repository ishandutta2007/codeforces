...
int fa[maxn][7],ans[maxn];
int find(int *f,int x)
{
	return f[x]==x?x:f[x]=find(f,f[x]);
}
void charm1_AK_ZJOI(int x1,int x2)
{
	up(i,0,n-1)f[i]=((s1[i]-'a')==x1);
	up(j,0,m-1)g[j]=((s2[j]-'a')==x2);
	reverse(g,g+m);
	int lim=1;
	for(;lim<=n*2;lim<<=1);
	times(f,g,lim,lim);
...
//	ntt(g,lim,1);
	...
	{
		if(!f[i])continue;
		if(find(fa[i],x1)!=find(fa[i],x2))fa[i][fa[i][x1]]=fa[i][x2],++ans[i];
	}
}
signed main()
{
