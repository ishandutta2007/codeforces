...
int curr_turn = 1;
bool on_stack[MN];
VI * curr_graf, *other_graf;
int dfs3(int x) {
	assert(!on_stack[x]);
	if (last_seen[x] == curr_turn)
		return -1;
	last_seen[x] = curr_turn;
	on_stack[x] = true;
	for (auto v : curr_graf[x])
		if (on_stack[v]) {
			cycles_cou++;
			cycles[x]++;
			on_stack[x] = false;
...
int main() {
	...
	FOR(i, 1, n) {
		REP(j, siz(graf[i]))
			if (nr[graf[i][j]] != com) {
				swap(graf[i][j], graf[i].back());
				graf[i].pop_back();
				j--;
			}
		REP(j, siz(grafT[i]))
			if (nr[grafT[i][j]] != com) {
				swap(grafT[i][j], grafT[i].back());
				grafT[i].pop_back();
				j--;
			}
		if (nr[i] == com)
	...
	other_graf = grafT;
	int iter = 0;
	while (clock() < CLOCKS_PER_SEC * 0.91) {
		iter++;
		if (iter > 1) {
			for (int x : left)
				random_shuffle(ALL(curr_graf[x]));//, [iter](int x, int y){return cycles[x] == cycles[y] ? (x * iter % 1234567 > y * iter % 1234567) : cycles[x] < cycles[y];});
			random_shuffle(ALL(left));
		}
		for (auto v : left)
			dfs3(v);
		curr_turn++;
		swap(curr_graf, other_graf);
	}
	FOR(i, 1, n) {
		if (cycles[i] == cycles_cou) {
			printf("%d\n", i);
			return 0;
		}
	}
