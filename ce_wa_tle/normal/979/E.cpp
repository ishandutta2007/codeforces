...
#define RE do{ll*__s=new ll[1];__s[-114514]=0;}while(0)
ll f[114514][8];
const ll mod=1e9+7;
...
ll __2xp[114514],*_2xp=__2xp+1;
int main(){
	//IO(CF979E Kuro and Topological Parity);
	ll rill(n,m);
	_2xp[0]=1;
	rep(i,1,n)_2xp[i]=sum(_2xp[i-1],_2xp[i-1]);
	f[0][0]=1;
	rep(i,1,n){
		ll rill(x);
		rep(j,0,7){
			const ll&fij=f[i-1][j];
			//debugln(sys(f,i-1,j));
			//writeln("ss",!x,fij,i,j,_2xp[i-1]%mod);
			if(x){//w
				if(j&2)
				add(f[i][j],fij*_2xp[i-2]%mod),
				add(f[i][(j^4)|1],fij*_2xp[i-2]%mod);//,writeln("ss",fij,i,j,_2xp[i-1]%mod);
				else
				add(f[i][(j^4)|1],fij*_2xp[i-1]%mod);//,writeln("ss",fij,i,j,_2xp[i-1]%mod);
			}
			if(~-x){//b
				if(j&1)
				add(f[i][j],fij*_2xp[i-2]%mod),
				add(f[i][(j^4)|2],fij*_2xp[i-2]%mod);//,writeln("ss",fij,i,j,_2xp[i-1]%mod);
				else
				add(f[i][(j^4)|2],fij*_2xp[i-1]%mod);//,writeln("ss",fij,i,j,_2xp[i-1]%mod);
			}
		}
	}
	writeln((f[n][m<<2]+f[n][m<<2|1]+f[n][m<<2|2]+f[n][m<<2|3])%mod);
	return 0;
}