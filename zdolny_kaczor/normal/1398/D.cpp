...
void solve() {
	...
	for (int i = 0; i <= R; ++i)
		for (int j = 0; j <= G; ++j)
			for (int k = 0; k <= B; ++k) {
				// debug(range(r, 0, i), range(g, 0, j), range(b, 0, k), arr(dp, i, j, k));
				maxi(dp[i][j + 1][k + 1], dp[i][j][k] + g[j] * b[k]);
				maxi(dp[i + 1][j][k + 1], dp[i][j][k] + r[i] * b[k]);
				maxi(dp[i + 1][j + 1][k], dp[i][j][k] + r[i] * g[j]);
				maxi(dp[i + 1][j][k], dp[i][j][k]);
				maxi(dp[i][j + 1][k], dp[i][j][k]);
				maxi(dp[i][j][k + 1], dp[i][j][k]);
			}
	printf("%d\n", dp[R][G][B]);
}
int main() {
