...
int main() {
    ...
    //     rep2(j, 1, n - 1) {
    //         d[i][j][0] = d[i][j - 1][n - 1];
    //         rep2(k, 1, n - 1) { d[i][j][k] = d[i][j - 1][k - 1]; }
    //     }
    // }
    ...
    rep(st, n) {
        ...
        rep(_, n) {
            int now;
            int mi = inf<int>;
            rep(i, n) if(!used[i] and chmin(mi, dist[i])) now = i;
            used[now] = true;
            int j = dist[now] % n;
            rep(k, n) chmin(dist[k], dist[now] + d[now][(k >= j ? k - j : (k + n - j))]);
        }
        rep(i, n) write(dist[i]);
        io.write_str("", '\n');
    }
}