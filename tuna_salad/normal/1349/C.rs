...
fn main() {
	...
	l!(r,c,q = rin.u());
	let b = rin.vvb(r, Some(c));
	let dist = {
		let mut dist = v!([r][c] = None);
		let mut q = VecDeque::new();
		let adj = |i:usize, j:usize| -> Vec<(usize,usize)> {
			let mut res = v!([4]);
			if i   > 0 { res.push( (i-1,j) ); }
			if j   > 0 { res.push( (i,j-1) ); }
			if i+1 < r { res.push( (i+1,j) ); }
			if j+1 < c { res.push( (i,j+1) ); }
			res
		};
		for i in 0..r {
			for j in 0..c {
				if dist[i][j].is_some() { continue; }
				l!(mut s,z = v!([]));
				dist[i][j].replace(0_i64);
				s.push( (i,j) );
				while let Some( (i,j) ) = s.pop() {
					z.push( (i,j) );
					for (ii,jj) in adj(i,j) {
						if b[i][j] == b[ii][jj] && dist[ii][jj].is_none() {
							dist[ii][jj].replace(0);
							s.push( (ii,jj) );
						}
					}
				}
		...
		while let Some( (i,j,d) ) = q.pop_front() {
			for (ii,jj) in adj(i,j) {
				if dist[ii][jj].is_none() {
					q.push_back( (ii,jj,d+1) );
					dist[ii][jj].replace(d+1);
				}
			}
		}
		dist
	...
	rep!{[q]
		l!(i,j = rin.u()-1);
		let p = rin.l();
		let t = dist[i][j].unwrap_or(p+1);
		let res = if t > p {
			b[i][j] - b'0'
		} else {
			(b[i][j] - b'0') ^ ((p ^ t) & 1) as u8
		};
		writeln!(rout, "{}", res).ok();
	}
}