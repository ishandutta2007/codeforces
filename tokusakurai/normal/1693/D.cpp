...
template <typename T>
vector<int> right_first(const vector<T> &a, bool greater = true) {
    int n = a.size();
    vector<int> dp(n);
    for (int i = n - 1; i >= 0; i--) {
        dp[i] = i + 1;
...
template <typename T>
vector<int> left_last(const vector<T> &a, bool greater = true) {
    int n = a.size();
    vector<int> dp(n);
    for (int i = 0; i < n; i++) {
        dp[i] = i - 1;
...
int main() {
    ...
    set<int> s1, s2;
    vector<int> l1 = left_last(P), r1 = right_first(P), l2 = left_last(P, false), r2 = right_first(P, false);

    // print(l1), print(r1), print(l2), print(r2);

    auto f = [](int a, int b) { return max(a, b); };
    auto c = [](int a, int b) { return a > b; };
    Segment_Tree<int> seg1(N, -inf, f, -inf), seg2(N, -inf, f, -inf);
    vector<int> R(N, N);

    rep(i, N) {
        int e = Q[i];
        chmin(R[e], seg1.find_first(0, c, r1[e]));
        seg1.change(e, l2[e]);
    }

    rep3(i, N - 1, 0) {
        int e = Q[i];
        chmin(R[e], seg2.find_first(0, c, r2[e]));
        seg2.change(e, l1[e]);
    }
