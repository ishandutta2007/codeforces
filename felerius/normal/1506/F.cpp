// Triangular Paths (https://codeforces.com/contest/1506/problem/F)
...
int main() {
    ...
    while (t--) {
        int n; cin >> n;
        vector p(n, pair(0, 0));
        rep(i, n) cin >> p[i].first, --p[i].first;
        rep(i, n) cin >> p[i].second, --p[i].second;
        sort(all(p));
        ...
        for (auto& [r2, c2] : p) {
            int rd = r2 - r, cd = c2 - c;
            int left = rd - cd;
            bool first_left = ((c + r) % 2 == 0);
            if (left == 0 && first_left)
                cost += rd;
            else if (first_left)
                cost += left / 2;
            else
