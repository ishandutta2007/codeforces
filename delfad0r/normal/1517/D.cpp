...
if(K % 2) {
	for(int n = 0; n < N; ++n) {
		for(int m = 0; m < M; ++m) {
			[&]() { auto& out = cout; put_stream(out, "-1 ");  }();
		}
		[&]() { auto& out = cout; put_stream(out, "\n");  }();
	}
	return 0;
}
...
vector<vector<vector<int>>> D(K + 1, vector<vector<int>>(N, vector<int>(M)));
for(int k = 1; k <= K; ++k) {
	for(int n = 0; n < N; ++n) {
		for(int m = 0; m < M; ++m) {
			D[k][n][m] = 1 << 30;
			if(n > 0) {
				D[k][n][m] = min(D[k][n][m], Y[n - 1][m] + D[k - 1][n - 1][m]);
			}
			if(n < N - 1) {
				D[k][n][m] = min(D[k][n][m], Y[n][m] + D[k - 1][n + 1][m]);
			}
			if(m > 0) {
				D[k][n][m] = min(D[k][n][m], X[n][m - 1] + D[k - 1][n][m - 1]);
			}
			if(m < M - 1) {
				D[k][n][m] = min(D[k][n][m], X[n][m] + D[k - 1][n][m + 1]);
			}
		}
	}
}
...
for(int n = 0; n < N; ++n) {
	for(int m = 0; m < M; ++m) {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(2 * D[K][n][m]), " ");  }();
	}
	[&]() { auto& out = cout; put_stream(out, "\n");  }();
}
