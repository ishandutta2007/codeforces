...
{
    int f, n;
    f_n_pair(int ff, int nn)
    {
        f = ff, n = nn;
    }
    bool operator<(const f_n_pair& other) const 
    { 
        if(f == other.f)
        {
            return n > other.n;
...
void solve()
{
    ...
    map<int, int> n_f;
    multiset<f_n_pair> f_n;

    function<void(int)> add = [&](int val)
    {   
        if(n_f.count(val))
        {
            int f_before = n_f[val];
            f_n.erase(f_n.find(f_n_pair(f_before, val)));

            f_n.insert(f_n_pair(f_before+1, val));

            n_f[val]++;
        }
        else
        {
            f_n.insert(f_n_pair(1, val));
            n_f[val] = 1;
        }
