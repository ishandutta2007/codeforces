//
// Created by Danny Mittal on 5/19/22.
//
...
int main() {
    ...
    for (int j = 0; j < n; j++) {
        if (3LL * xs[j] > m) {
            amtNeeded++;
            for (int k = 0; k < n; k++) {
                if (xs[j] % xs[k] == 0LL && (2LL * xs[j]) + xs[k] <= m) {
                    edges.push_back(make_pair(j, k));
                }
            }
        }
    }
    vpi matching = maxMatch(n, n, edges);
    if (matching.size() == amtNeeded) {
        cout << n - amtNeeded << '\n';
        for (auto match : matching) {
            ...
            matched[j] = true;
            matched[k] = true;
            cout << (2LL * xs[j]) + xs[k] << ' ' << xs[j] + xs[k] << '\n';
        }
        for (int j = 0; j < n; j++) {
            if (!matched[j]) {
                cout << 3LL * xs[j] << ' ' << 2LL * xs[j] << '\n';
            }
        }
    } else {
        cout << -1 << '\n';
