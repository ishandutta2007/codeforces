...
int main(){
	...
	range(i,1,n,i){
		auto compare=[&](int a,int b){
			if(rnk[a]!=rnk[b]){
				...
				int ra=(a+i<=n?rnk[a+i]:-1);
				int rb=(b+i<=n?rnk[b+i]:-1);
				return ra<rb;
			}
		};
		...
		tmp[sa[0]]=0;
		cont(j,n){
			tmp[sa[j]]=tmp[sa[j-1]]+compare(sa[j-1],sa[j]);
		}
		loop(i,n+1){
			rnk[i]=tmp[i];
		}
	...
	cont(i,n){
		int f=sa[i]+1;
		int val=o+qzh[f-1];
		ans+=max(0,(int)(lower_bound(all(hv[val]),pre[f-1])-lower_bound(all(hv[val]),f+lcp[i])));
	}
	fio<<ans<<ln;
	return 0;
}