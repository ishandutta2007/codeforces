...
//    tout[cur] = curT;
//}
//void add(ll *a, ll *b, int n, int x) {
//    for (int i = 0; i < n; ++i) {
//        a[i] += b[i] * x;
//    }
//}
//
//ll getMax1(int *a, int *b, int n) {
//    ll r0 = 0;
//    ll r1 = 0;
//    ll r2 = 0;
//    ll r3 = 0;
...
//        b += 4;
//    }
//    return r0 + r1 + r2 + r3 + z0 + z1 + z2 + z3;
//    // return max({r0, r1, r2, r3, -z0, -z1, -z2, -z3});
//}
...
//        const int Q = 50000;
//        FI(Q) {
//            q.push_back({rand() % 1000, n - rand() % 1000});
//        }
//        ll tot = 0;
//        cout << "GO" << endl;
//        double begin = clock();
//        for (auto p : q) {
//            tot += getMax2(a + p.first, b + p.first, p.second - p.first);
//        }
//        double end = clock();
//        cout << (end - begin) * 1.0 / CLOCKS_PER_SEC << endl;
//        cout << tot << endl;
//        return 0;
//    }
//    int n, q;
...
//        v[x - 1].push_back(i);
//    }
//    FI(n) cd[i].first = readInt();
//    FI(n) cd[i].second = readInt();
//    go(0);
//    FI(n) {
//        a[tin[i]] = cd[i].first;
//        b[tin[i]] = cd[i].second;
...
//            int from = tin[v];
//            int to = tout[v];
////            add(a + from, to - from, x);
//        } else {
//            int v = readInt() - 1;
//            int from = tin[v];
//            int to = tout[v];
////            ll ans = getMax(a + from, b + from, to - from);
////            writeInt(ans);
//        }
//    }
//    return 0;
...
//    shuffle(a + 1, a + n + 1, gen);
    ...
    for (int len = 2; len <= n; ++len) {
        for (int l = 1; l + len - 1 <= n; ++l) {
            int r = l + len - 1;
            int p = int(min_element(a + l, a + l + len) - a);
            ll s = 0;
            for (int i = l; i <= p; ++i) {
                s = (s + dp[l][i - 1] * dp[i][p - 1]) % fftmod;
            }
            ll t = 0;
            for (int j = p; j <= r; ++j) {
                t = (t + dp[p + 1][j] * dp[j + 1][r]) % fftmod;
            }
            dp[l][r] = s * t % fftmod;
        }
    }
