...
namespace IO {
    const int szeE = (1 << 20) + 1;
    char ibuf[szeE], *iS, *iT, obuf[szeE], *oS = obuf, *oT = obuf + szeE - 1;
    char _st[55];
    int _qr = 0;
    inline char gc() {
        return (iS == iT ? iT = (iS = ibuf) + fread(ibuf, 1, szeE, stdin), (iS == iT ? EOF : *iS++) : *iS++);
    }
    inline void qread() {}
...
void dfs(int x, int Fa) {
    ...
    for (auto v : e[x]) {
        ...
        if (ww[x] && !ww[v]) ww[v] = 1, ss[v] = pii(v, deg[v]);
        ...
        if (!(ww[x] && !ww[v]) && !ww[v]) {
            ...
            if (deg[x] == x)
                ...
                ww[x] = 1, ss[x] = pii(deg[x], deg[v]);
        }
    }

    sze[x] -= cn;

    if (x == 1 && cn < 2) ww[1] = 1, ss[1] = pii(1, deg[1]);
}

void Solve() {
	qread(n);
        rep(i, 1, n) {
        	sze[i] = deg[i] = ww[i] = fa[i] = 0;
            ss[i] = pii(0, 0);
            e[i].clear();
    	}
        ...
        rep(i, 1, n) {
            if (ww[i]) {
                if (now) printf("%d %d %d %d\n", i, fa[i], now, ss[i].fi), now = ss[i].se;
                else now = ss[i].se;
            }
        }	
}
