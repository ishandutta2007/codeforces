...
void read(){
	...
	for(i=1;i<=n;i++){
		...
		if(i>1)
			len+=Get_dist(a[i],a[i-1]);
	}
	len+=Get_dist(a[1],a[n]);
	pl=len/m;
}
...
bool check(double mid){
	...
	for(i=1;i<=m;i++){
		...
		tp=b[i%m+1];
		while(1){
//			double t=min(pl-nl,min(Get_sq(a[ni]-b[i]),;
			if(pl-nl<EPS)
				break;
			if(Get_sqdist(a[ni],np)<EPS){
				np=a[ni];
				ni=ni%n+1;
				continue;
			}
			if(Get_sqdist(a[ti],tp)<EPS){
				tp=a[ti];
				ti=ti%n+1;
				continue;
			}
			d=min(min(Get_dist(a[ni],np),Get_dist(a[ti],tp)),pl-nl);
			double dx1=a[ni].x-np.x,dx2=a[ti].x-tp.x,dy1=a[ni].y-np.y,dy2=a[ti].y-tp.y;
			double A=sqr(Get_sign(dx1)-Get_sign(dx2))+sqr(Get_sign(dy1)-Get_sign(dy2)),
				B=2*(np.x-tp.x)*(Get_sign(dx1)-Get_sign(dx2))+2*(np.y-tp.y)*(Get_sign(dy1)-Get_sign(dy2)),
				C=sqr(np.x-tp.x)+sqr(np.y-tp.y);
			...
			if(A!=0){
				...
				if(delta<0)
					...
					L=max((-B-sqrt(delta))/(2*A),0.)+nl;
					R=min((-B+sqrt(delta))/(2*A),d)+nl;
				}
			}
			else{
				if(C<=sqr(mid))
			...
			nl+=d;
			np+=point(Get_sign(dx1),Get_sign(dy1))*d;
			tp+=point(Get_sign(dx2),Get_sign(dy2))*d; 
		}
	}
	sort(all(c));
	int cc=0;
...
void solve(){
	...
	point np=a[1];
	while(now<m){
		nl=nl+Get_dist(a[ni],np);
		if(nl>=pl){
			nxt[++now]=ni;
			np=a[ni]-(a[ni]-np)*((nl-pl)/Get_dist(a[ni],np));
			b[now]=np;
			nl=0;
		}
		else
