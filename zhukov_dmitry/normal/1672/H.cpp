...
int fsum(int *f, int l, int r)
{
	return fsum(f, r) - fsum(f, l - 1);
}
...
int main()
{
	...
	forn(i, k)
	{
		...
		l--;
		r--;
		int k0 = fsum(f[0], l, r - 1);
		int k1 = fsum(f[1], l, r - 1);
		int ans = 1 + k0 + k1 - min(k0, k1);
		printf("%d\n", ans);
	}
