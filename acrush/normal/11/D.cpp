...
int n;
bool G[19][19];
int s,workId,vId[two(19)][19];
int64 f[two(19)][19];

int64 solve(int set,int last)
{
	if (vId[set][last]==workId) return f[set][last];
	vId[set][last]=workId;
	int64 &R=f[set][last];
	R=(int)(G[last][s]);
	for (int i=0;i<s;i++) if (!contain(set,i) && G[last][i]) R+=solve(set|two(i),i);
	return R;
}
int main()
{
	...
	{
		workId++;
		for (int i=0;i<s;i++) if (G[s][i])
			for (int j=0;j<s;j++) if (i!=j && G[i][j])
				R+=solve(two(i)|two(j),j);
	}
	cout<<R/2<<endl;
	return 0;
}