...
const int maxn = 3e3 + 100, maxw = 5e6 + 10, inf = 1e9 + 100, LG = 20, SQ = 450;
...
void solve() {
    ...
    for (int l = n - 1; l >= 0; l--) {
        for (int r = l + 1; r < n; r++) {
            if (a[l] != a[r]) {
                dp[l][r][0] = min(dp[l + 1][r][0], dp[l + 1][r][1]) + 1;
                dp[l][r][1] = min(dp[l][r - 1][0], dp[l][r - 1][1]) + 1;
            } else {
                dp[l][r][0] = dp[l][r][1] = min(dp[l + 1][r - 1][0], dp[l + 1][r - 1][1]) + 1;
            }
            {
                int id = pos[l], x = a[l];
                for (int it = id + 1; it < ext[x].size() && ext[x][it] <= r; it++)
                    setmin(dp[l][r][0], (short)(dp[l][ext[x][it] - 1][0] + dp[ext[x][it]][r][0]));
            }
            {
                int id = pos[r], x = a[r];
                for (int it = id - 1; it >= 0 && ext[x][it] >= l; it--)
                    setmin(dp[l][r][1], (short)(dp[l][ext[x][it]][1] + dp[ext[x][it] + 1][r][1]));
            }
            if (a[l] == a[r]) {
                for (int it = l; it < r; it++) {
                    short w = dp[l][it][0] + dp[it + 1][r][1];
                    setmin(dp[l][r][0], w);
                    setmin(dp[l][r][1], w);
                }
            }
