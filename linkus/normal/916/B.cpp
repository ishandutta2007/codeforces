...
#endif
template<class T> ostream& operator<<(ostream &os, map<T,T> V) { os<<"[";for(auto const &vv:V)os<<vv<<","; os<<"]"; return os; }
...
int main(){
    ...
    ll n,k;
    sc(n,k);
    /* whatis(__builtin_popcountll(n)) */
    if(k < __builtin_popcountll(n)) REE("No")
    cout << "Yes\n";
    int left = k;
    map<ll,ll> mp;
    for(ll i = 0; i < 63; ++i){
    ...
    while(mp.rbegin()->second <= left){
        int val = mp.rbegin()->first, cnt = mp.rbegin()->second;
        left -= cnt;
        mp.erase(prev(mp.end()));
        mp[val-1] += cnt << 1;
        /* mp.insert({val-1,cnt << 1}); */
    }
    /* exit(0); */
    while(left){
        int val = mp.begin()->e1;
        ...
        --mp.begin()->e2;
        mp[val-1] = 2;
        /* mp.insert(mp.begin(),{val-1,2}); */
        /* int val = mp.begin()->e1, cnt = mp.begin()->e2; */
        /* int chg = min(left,cnt); */
        /* mp.begin()->e2 -= chg; */
        /* mp[val-1] += chg << 1; */
        /* /1* mp.insert({val-1,chg << 1}); *1/ */
        /* left -= chg; */
    }
