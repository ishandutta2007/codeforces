...
vi d[N];
int in[N];
int rel[N]; // does there exist an a[j] relatively prime to i
int cnt[N];
Sieve<N> S;
int main() {
    setIO("");
    ...
    n = sz(a);  
    f0r(i, n) dp[a[i]] =  in[a[i]] = 1;

    rel[1] = 1;
    /// does there exist an a[i] relatively prime to i
    for (int x : a) {
        auto f = S.factor(x);
        int res = 1;
        vi plist;
    ...
    f1r(i, 2, N) {
        ...
        f0r(mask, (1<<sz)) {
            ...
            sum += (__builtin_popcount(mask)%2 ? 1 : -1) * cnt[prod];
        }
        if (sum != n) rel[i] = 1;
    }
    for (int i = N-1; i >= 1; i--) {
        ...
        for (int j = 1; j*i < N; j++) {
            ...
            // is there something relatively prime to j
            if (rel[j]) 
                ckmin(dp[i], dp[j*i]+1);
        }
    }
