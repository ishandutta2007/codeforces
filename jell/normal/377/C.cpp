#line 1 "codeforces-workspace\\contest\\377\\c\\c.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\377\\c\\c.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\377\\c\\c.cpp"
...
void main() {
  ...
  cin >> s;
  if (n > 20) {
    nth_element(begin(s), begin(s) + 20, end(s), greater{});
    s.erase(begin(s) + 20, end(s));
    n = 20;
  }
  ...
  for (auto &&[c, t] : acts) {
    for (auto S : range(dp.size())) {
      for (auto i : range(n)) {
        ...
        int gain = (c == 'p' ? s[i] : 0);
        if (t)
          chle(dp[S], dp[S | 1 << i] - gain);
        else
          chgr(dp[S], dp[S | 1 << i] + gain);
      }
    }
  }
  cout << dp[0] << "\n";
