...
int main() {
    ...
    int root = -1;
    {
        ...
        int j = 0;
        for (int i = 0; i < n; i++)
            if (bfs1.distance[i] > bfs1.distance[j])
                j = i;

        BFS bfs2(adj, {j});
        int j2 = 0;
        for (int i = 0; i < n; i++)
            if (bfs2.distance[i] > bfs2.distance[j2])
                j2 = i;

        auto p = bfs2.get_path(j2);
    ...
    bool ans = true;
    for (int i = 0; i < n; i++) {
        if (tr.neighbors[i].size() == 1)
            ans &= tr.depth[i] == k;
        else if (i == root)
            ans &= tr.neighbors[i].size() >= 3;
        else
            ans &= tr.neighbors[i].size() >= 4;
    }
