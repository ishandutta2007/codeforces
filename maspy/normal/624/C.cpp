...
void solve() {
  ...
  vi deg(N);
  FOR(i, N) deg[i] = SUM(G[i]);
  vi ANS(N, -1);
  ll a = min_element(all(deg)) - deg.begin();
  FOR(v, N) {
    if (G[v] == G[a]) ANS[v] = 0;
  }

  const ll INF = 1LL << 60;
  FOR(v, N) if (ANS[v] == 0) deg[v] = INF;
  ll c = min_element(all(deg)) - deg.begin();
  ll k = G[a][c] ? 1 : 2;
  FOR(v, N) {
    if (G[v] == G[c]) ANS[v] = k;
  }
  ll x = 3 - 0 - k;
  FOR(v, N) if (ANS[v] == -1) ANS[v] = x;

  FOR(a, N) FOR(b, N) {
    if (abs(ANS[a] - ANS[b]) <= 1 != (G[a][b])) return print("No");
  }
  Yes();
  string S;
  FOR(i, N) S += 'a' + ANS[i];
