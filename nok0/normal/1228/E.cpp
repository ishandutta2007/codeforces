...
int main() {
	INT(n, k);
	V<mint> powk(n + 1, 1), powmk(n + 1, 1);
	REP(i, n) {powk[i + 1] = powk[i] * k, powmk[i + 1] = powmk[i] * (k - 1);}
	
	VV<mint> dp(n + 1, V<mint>(n + 1, 0));
	dp[0][0] = 1;

	REP(i, n) {
		REP(j, n + 1) {
			if(j) {
				dp[i + 1][j] += dp[i][j] * (powk[j] - powmk[j]) * powmk[n - j];
			}
			FOR(l, 1, n - j + 1) {
				dp[i + 1][j + l] += dp[i][j] * COM(n - j, l) * powk[j] * powmk[n - j - l];
			}
		}
	}
