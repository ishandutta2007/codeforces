...
//    solve.usetc()
    solve.cases{
        val n= getint
        val q = getint
        val reversed = BooleanArray(n)
        val L = getline(1 shl n)
        fun nodenow(v:Int, level:Int = 0 ):Int {
            var now = 1
            assert(v and ( 1 shl (n-level)) != 0)
            for(i in n - 1 downTo level) {
                val has = (v and ( 1 shl (i-level) ))!= 0
                val actual = has xor reversed[i]
                if(actual){
                    now = (now shl 1) + 1
                }else{
                    now = (now shl 1 )
        ...
        fun indexNow(v:Int):Int{
            return nodenow(v + (1 shl n )) - (1 shl n)
        }
        ...
        fun query(l:Int, r:Int):Long {
            ...
            st.segDivision(l,r){
                    i, level ->
                ret += st.arr[nodenow(i,level)]
            }
            return ret
        }
        fun outputall(){
            just dei ""
            println(LongArray(1 shl n){query(it,it)}.conca)
            println(IntArray(1 shl n){indexNow(it)}.conca)
        }
...
1
4 1 1
feel like not really in control of the segments indicing
