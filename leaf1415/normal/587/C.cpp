...
vector<ll> mvec[100005][17];
LCA lca;

vector<ll> merge(vector<ll> &vec, vector<ll> &vec2){
	vector<ll> ret = vec;
	ret.insert(ret.end(), all(vec2));
	sort(all(ret));
	if(sz(ret) > 10) ret.resize(10);
	return ret;
}
...
int main(void)
{
	...
	rep(i, 1, n){
		sort(all(vec[i]));
		if(sz(vec[i]) > 10) vec[i].resize(10);
		mvec[i][0] = vec[i];
	}
	...
	if(n == 1){
		rep(q, 1, Q){
			cin >> u >> v >> c;
			vector<ll> ans = mvec[1][0];
			if(sz(ans) > c) ans.resize(c);
			cout << sz(ans) << " " << ans << "\n";
		}
		return 0;
	}
	...
	lca.init(G, n);
	ll logn = lca.logn;
	rep(i, 1, logn-1) rep(j, 1, n){
		mvec[j][i] = merge(mvec[j][i-1], mvec[lca.prev[j][i-1]][i-1]);
	}
	...
	rep(q, 1, Q){
		...
		vector<ll> ans = mvec[l][0];
		ll udep = lca.depth[u]-lca.depth[l], vdep = lca.depth[v]-lca.depth[l];
		rep(i, 0, logn-1){
			if(udep & (1<<i)){
				ans = merge(ans, mvec[u][i]);
				u = lca.prev[u][i];
			}
		}
		rep(i, 0, logn-1){
			if(vdep & (1<<i)){
				ans = merge(ans, mvec[v][i]);
				v = lca.prev[v][i];
			}
		}
		if(sz(ans) > c) ans.resize(c);
