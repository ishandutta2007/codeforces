...
void FWT(vector<Modint>& a){
    const int n=a.size();
    for(int d=1;d<n;d<<=1)
        for(int m=d<<1,i=0;i<n;i+=m)
            for(int j=0;j<d;j++){
                const Modint x=a[i+j],y=a[i+j+d];
                a[i+j]=x+y;
                a[i+j+d]=x-y;
            }
}
void IFWT(vector<Modint>& a){
    constexpr Modint inv=2_M .inv();
    const int n=a.size();
    for(int d=1;d<n;d<<=1)
        for(int m=d<<1,i=0;i<n;i+=m)
            for(int j=0;j<d;j++){
...
int main(){
    ...
    vector<ll>q;
    rep(n)if(!deg[i])q.push_back(i);
    vec(ll,grundy,n);
    vec(vector<ll>,prev,n);
    while(q.size()){
        const ll at=q.back();
        q.pop_back();
        Sort(prev[at]);
        each(i,prev[at])if(i==grundy[at])grundy[at]++;
        each(i,g[at]){
            prev[i].push_back(grundy[at]);
