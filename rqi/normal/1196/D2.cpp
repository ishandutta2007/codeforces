...
int main() {
    ...
    for(ll c = 0; c < q; c++){
        ...
        ans = min(ans, RGB[0][k-1]);
        
        ans = min(ans, RGB[1][k-1]);
        
        ans = min(ans, RGB[2][k-1]);
        for(ll i = k; i < n; i++){
            
        ans = min(ans, RGB[0][i]-RGB[0][i-k]);
        
        ans = min(ans, RGB[1][i]-RGB[1][i-k]);
        
        ans = min(ans, RGB[2][i]-RGB[2][i-k]);
        }
        cout<< ans << "\n";
    }
