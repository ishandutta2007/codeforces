...
void solve() {
    ...
    ll ans = 1;
    FOR(i, 2, n) if (sum - pref[i] <= mx) ans++;
    debug(ans);
    ll cur = 0;
    vll tmp; 
    F0R(rep, 2) {
        ...
        for (int i = rep; i < n; i += 2) {
            if (i>0) {
                cur -= a[i-1];
                tmp.pb(tmp.back() + a[i-1]);
            }
            // cur + tmp[i] + pref[i+1] <= mx
            int j = ub(all(tmp), mx - pref[i+1] - cur)-tmp.begin();
            debug(i, j);
            ans += j;
            if (i<n-3) {
                int j = ub(all(tmp), mx - pref[i+1] - cur - a[n-1])-tmp.begin();
                debug(i, j);
                ans += j;
            }
            if (i>1) {
                debug(i, tmp, mx-pref[i+1]-cur+a[0]);
                int j = ub(all(tmp), mx - pref[i+1] - cur + a[0])-tmp.begin();
                debug(i, j);
                if (rep==1) ans += max(0, j-2);
                else ans += max(0, j-1);
            }
            if (i<n-3 && i>1) {
                int j = ub(all(tmp), mx - pref[i+1] - cur - a[n-1] + a[0])-tmp.begin();
                debug(i, j);
                if (rep==1) ans += max(0, j-2);
                else ans += max(0, j-1);
            }
