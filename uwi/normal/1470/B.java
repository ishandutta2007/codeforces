...
public class B {
	...
	void go()
	{
		...
		int[] a = na(n);
		for(int i = 0;i < n;i++){
			a[i] = normalize(a[i], lpf);
		}
		a = radixSort2(a);
		int[][] uc = uniqcount(a);
		int ans0 = 0;
		for(int[] u : uc){
			ans0 = Math.max(ans0, u[1]);
		}
		int n1 = 0;
		int[][] nuc = new int[uc.length][];
		int p = 0;
		for(int[] u : uc){
			if(u[1] % 2 == 0 || u[0] == 1){
				u[0] = 1;
		...
		nuc = Arrays.copyOf(nuc, p);
		int ans = 0;
		for(int[] u : nuc){
			ans = Math.max(ans, u[1]);
		}
		for(int Q = ni();Q > 0;Q--){
			long w = nl();
			out.println(w >= 1 ? ans : ans0);
		}
	}
