...
void solve() {
    ...
    while (!nexts.empty()) {
        int now = nexts.front();
        nexts.pop();
        if (val[now].second == now or val[now].first != 1) continue;
        inputs.push_back(mp(now, val[now].second));
        val[val[now].second].first--;
        val[val[now].second].second ^= now;
        val[now].first--;
        if (val[val[now].second].first == 1)nexts.push(val[now].second);
    }
