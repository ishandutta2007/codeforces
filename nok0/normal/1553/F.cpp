...
void main_() {
	...
	REP(i, n) {
		...
		if(a[i] > sq) {
			REP(j, 1, sq + 1) {
				int l = max(a[i] / (j + 1) + 1, sq + 1);
				int r = a[i] / j + 1;
				if(l >= r) continue;
				ll tmp_sum = sum.sum(l, r);
				tmp_sum *= j;
				res += cnt.sum(l, r) * a[i] - tmp_sum;
			}
		}
		...
		//
		{
			...
			if(a[i] <= sq) {
				...
				for(int l = a[i]; l <= M; l += a[i]) {
					int r = min(l + a[i], M + 1);
					res += sum.sum(l, r) - cnt.sum(l, r) * l;
				}
			}
		}
		//
		{
			cnt.add(a[i], 1);
			sum.add(a[i], a[i]);
			if(a[i] <= sq) mins.pb(a[i]);
			REP(m, 1, sq + 1) {
				if(m < a[i]) mods[m][a[i] % m] += a[i] % m;
			}
		}
		ans.pb(res);
	}
