...
template <class T> int get_pos(vector<T>& v, const T& x) {
...
void solve_case(int tc = 0) {
	...
	for (auto [c, l, r] : v) {
		auto& use = active[c ^ 1];
		vi mash;
		while (!use.empty() && use.rbegin()->f >= l) { 
			mash.pb(use.rbegin()->s);
			use.erase(prev(use.end()));
		}
		vi tmp(2);
		f0r(i, 2) {
			tmp[i] = far[i][idx];
		...
		each(id, mash) {
			f0r(i, 2) {
				active[i].erase(mp(far[i][id], id));
				ckmax(tmp[i], far[i][id]);
			}
			dsu.unite(id, idx);
		}
		int z = dsu.get(idx);
