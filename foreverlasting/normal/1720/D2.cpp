...
namespace MAIN {
	int n;
	int a[N];
	int tr[N*31][2],mx[N*31][2],cnt;
	inline void MAIN(const int &Case) {
		n=read();
		for(int i=0;i<n;i++)a[i]=read();
		auto query=[&](const int &x,const int &y){
			int now=0,Mx=0;
			for(int i=30;~i;i--){
				int c=x>>i&1,d=y>>i&1,e=c^d;
//				printf("%d %d %d\n",i,e,c);
				Mx=max(Mx,mx[tr[now][e^1]][c]);
				now=tr[now][e];
				if(!now)break;
			}
			return Mx;
		};
		auto insert=[&](const int &x,const int &y,const int &z){
			int now=0;
			for(int i=30;~i;i--){
				int c=x>>i&1,d=y>>i&1,e=c^d;
				if(!tr[now][e])tr[now][e]=++cnt;
				now=tr[now][e],mx[now][d]=max(mx[now][d],z);
			}
		};
		int ans=0;
		for(int i=0;i<n;i++){
			int f=query(i,a[i])+1;
			ans=max(ans,f),insert(i,a[i],f);
//			printf("%d %d\n",i,f);
		}
		printf("%d\n",ans);
		for(int i=0;i<=cnt;i++)for(int j=0;j<=1;j++)tr[i][j]=mx[i][j]=0;
		cnt=0;
	}
}
