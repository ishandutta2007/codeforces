...
//        FREOUT("out.txt");
    ...
    string good;
    cin >> good;
    gs = set<char>(good.begin(), good.end());
    string t;
    cin >> t;
    int pos = (int) t.find('*');
    int pref = pos >= 0 ? pos : 0;
    int suf = pos >= 0 ? (int)t.size() - pos - 1 : t.size();
    auto pt = t.substr(0, pref);
    auto st = t.substr(t.size() - suf, suf);
    ...
    while (n--) {
        ...
        cin >> s;
        bool ok = true;
        if (s.size() < pref + suf || pos == -1 && pref + suf < s.size()) {
            ok = false;
        } else {
            if (!match(pt, s.substr(0, pref)) || !match(st, s.substr(s.size() - suf, suf))) {
                ok = false;
            }
            if (pos != -1) {
                for (int i = pref; i + suf < s.size(); ++i) {
                    if (gs.count(s[i])) {
                        ok = false;
                    }
                }
