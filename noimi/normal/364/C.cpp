...
int main() {
    auto pr = prime_sieve(100);
    ll now = 1;
    LL(k);
    vll v;
    for(auto e : pr) {
        if(now * e <= k * k * 2) v.eb(e), now *= e;
    }

    vpll cand;
    REC([&](auto &&f, int i, int b = 0, ll now = 1) -> void {
        if(i == si(v))
            cand.eb(b, now);
        else {
            while(now <= k * k * 2) {
    ...
    sort(all(cand), [](auto i, auto j) { return popcount(i.fi) > popcount(j.fi); });

    vll ans;
    vi cnt(si(v));
    for(auto [b, i] : cand) {
        if(si(ans) == k) break;
        rep(j, si(v)) if(b & 1 << j) cnt[j]++;
        ans.eb(i);
    }
    cout << ans << endl;
    assert(MIN(cnt) * 2 >= k);
