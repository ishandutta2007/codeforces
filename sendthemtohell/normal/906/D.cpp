...
ll phi[31];
//vector<pair<int,int>> pr_f[26];
ll w[N];

inline ll clc(ll x, int ptr) {
    ll res = 1;
    for (ll i = 2; i*i <= x; ++i) {
        if (!(x%i)) {
            //pr_f[ptr].pb({i,1});
...
void prec(ll m) {
    phi[0] = m;
    for (int i = 1; phi[i - 1] > 1; ++i) {
        phi[i] = clc(phi[i - 1], i - 1);
    }
}

ll tow(int l, int r,int kek = 0) {
    if (l + 1 == r || w[l] == 1 || kek > 10) {
        return w[l];
    }
    ll get = tow(l + 1, r, kek + 1);
    if (w[l] == 1) return 1;
    if (get > 30 || get < 0 || get*log10(w[l]) > 10) return -1;  // a ** get > 1e9 log(a) * get > 9
    return pw(w[l], get);
}

ll calc(int l, int r, int rec = 0) {

    if (l + 1 == r || phi[rec] == 1) {
        return w[l] % phi[rec];
    }
    else {
        return pwm(w[l], calc(l + 1, r, rec + 1) + phi[rec + 1]*(tow(l+1,r) < 0 || tow(l+1,r) >= phi[rec + 1]), phi[rec]);
    }
}
