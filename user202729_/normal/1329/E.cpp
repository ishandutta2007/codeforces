// :/  Given the conditions, the other solution is obviously O(m log m), not O(m log n). #iamsodumb
// Anyway, this is some more optimization. Should be faster.
...
void up(){
	...
	int64_t l, r;
	// find max l, min r  such that sum(cell(n/(r+1))) <= numAdd <= sum(floor(n/(l+1))  (for n in segments)
	// value: number of blocks between two adjacent black blocks (exclusive)
	assert(std::accumulate(begin(segments),end(segments),(int64_t)0)==len);
	l=len/(numAdd+numBlock+1); r=l+(len%(numAdd+numBlock+1)!=0);
	--l; --r;

	// now the desired value of l and r are <= current l and >= current r respectively
	...
	// ^ I forgot what they are already. I guess something like inclusive bound on the value of k.
	...
	// for each pair (l, r) in pairs, either resulting l <= l, or resulting r >= r
	// (note that it's impossible for l to be <= -1 or r to be >= INT64_MAX)
