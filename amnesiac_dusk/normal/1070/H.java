...
public class Main
{
	static Map<String,Integer> hola=new HashMap<String,Integer>();
	static Map<String,String> ans=new HashMap<String,String>();
	public static void main(String[]args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
		{
			...
			for(int j=0; j<a[i].length(); j++)
				for(int k=j+1; k<=a[i].length(); k++)
					te.add(a[i].substring(j,k));
			for(String tem:te)
			{
				hola.put(tem,hola.getOrDefault(tem,0)+1);
				ans.put(tem,a[i]);
			}
		}
		int q=ni();
		...
		{
			String te=ns();
			if(hola.containsKey(te))
				pr(hola.get(te)+" "+ans.get(te));
			else
				pr(0+" -");
		}
