...
int main(void)
{
	...
	rep(w, 1, 7){
		ll W = 1<<w;
		Matrix amat(W, W), bmat(W, W);
		
		rep(i, 0, W-1){
			rep(j, 0, W*2-1){
				ll ni = 0;
				if((j & 1) == 0 || (j & W) == 0) continue;
				rep(k, 0, w-1){
					if((i&(1<<k)) && (j&(1<<k)) && (j&(1<<(k+1)))) ni |= 0;
					else ni |= 1<<k;
				}
				amat.at(ni+1, i+1) += 1;
			}
	...
	rep(i, 1, 7){
		ll W = 1<<i;
		Matrix nvec(W, 1);
		rep(j, 0, W/2-1) nvec.at(j+W/2+1, 1) = vec.at(j+1, 1);
		vec = nvec;
		
		//rep(j, 1, W) cout << vec.at(j, 1) << " "; cout << endl;
		
		Matrix pmat = mat[i].pow(w[i]);
		vec = pmat * vec;
