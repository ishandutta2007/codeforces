...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		...
		vector<int> maxRem(m + 1, 0);
		for (int j = 0; j <= m; j++)
			if (ans[j] != INT_MAX) maxRem[j] = y;
			
		for (int j = 0; j <= m; j++) {
			if (maxRem[j] == 0) continue;

			ll nxt = INT_MAX;
			if (t == 1) nxt = (j + (x + DEN - 1) / DEN);
			if (t == 2) nxt = (1LL * j * x + DEN - 1) / DEN;
			if (nxt > m) break;

			chkmax(maxRem[nxt], maxRem[j] - 1);
			chkmin(ans[nxt], i);
		}
	}
