...
public:
	...
	bool win(int hp, int ap, int dp) {
		if (ATY + ap <= DEFM) return false;
		if (DEFY + dp >= ATM) return true;
		int kill = 1 + (HM-1) / (ATY + ap - DEFM);
		int die = 1 + (HY+hp-1) / (ATM - DEFY - dp);
		return kill < die;
	}

	void solve(istream& cin, ostream& cout) {
		cin >> HY >> ATY >> DEFY >> HM >> ATM >> DEFM >> h >> a >> d;
		int ans = 1000*1000;
		for (int ap = 0; ap < 200; ++ap) {
			for (int dp = 0; dp < 100; ++dp) {
				int hp = bsl(0, 100000, [&](int x) { return win(x, ap, dp); });
				if (hp != -1)
					ans = min(ans, hp*h + ap*a + dp*d);
			}
		}
		cout << ans << endl;
	}
