...
struct segtree {
	struct node { 
		int mx; 
		node(int mx = -inf) : mx(mx) {}
		#define ls(p) (p << 1)
		#define rs(p) (p << 1 | 1)
		#define mx(p) nd[p].mx
	} nd[N << 2];
	void sprup(int p) { mx(p) = max(mx(ls(p)), mx(rs(p))); }
	void chg(int x, int v, int p = 1, int tl = 1, int tr = n) {
		if(tl == tr) return mx(p) = v, void();
		int mid = tl + tr >> 1;
		if(x <= mid) chg(x, v, ls(p), tl, mid);
		else chg(x, v, rs(p), mid + 1, tr);
		sprup(p);
	}
	int _mx(int l, int r, int p = 1, int tl = 1, int tr = n) {
		if(l > r) return -inf;
		if(l <= tl && r >= tr) return mx(p);
		int mid = tl + tr >> 1, res = -inf;
		if(l <= mid) chkmx(res, _mx(l, r, ls(p), tl, mid));
		if(r > mid) chkmx(res, _mx(l, r, rs(p), mid + 1, tr));
		return res;
	}
} sgt;
...
void mian() {
	n = read();
	REP(i, 1, n) a[i] = read();
	REP(i, 1, n) lft[i] = las[a[i]] ? lft[las[a[i]]] : i, las[a[i]] = i;
//	debug(lft, 1, n);
	REP(i, 1, n) {
		val[i] = -inf;
		dp[i] = dp[i - 1];
		if(lft[i] < i) {
			val[i] = dp[lft[i] - 1] - lft[i] - 1;
			chkmx(dp[i], dp[lft[i] - 1] + i - lft[i] - 1);
			int fd = sgt._mx(lft[i] + 1, i - 1);
			chkmx(dp[i], fd + i - 1), chkmx(val[i], fd - 1);
//			REP(j, lft[i] + 1, i - 1) chkmx(dp[i], val[j] + i - 1), chkmx(val[i], val[j] - 1);
		}
		sgt.chg(i, val[i]);
	}
//	debug(dp, 1, n);
