...
main() {
    ...
    while(T--) {
        ...
        rep(i, n) S[i] = s[i] == 'L';
        rep(p, 2) rep(q, 2) {
            vvv(int, dp, n + 1, 2, 2, n);
            dp[1][p][q] = (S[0] == p) + (S[1] == q);
            rep2(i, 1, n - 2) {
                chmin(dp[i + 1][1][0], dp[i][1][1] + (!S[i + 1]));
                chmin(dp[i + 1][0][0], dp[i][1][0] + (!S[i + 1]));
                chmin(dp[i + 1][0][1], dp[i][1][0] + S[i + 1]);
                chmin(dp[i + 1][1][0], dp[i][0][1] + (!S[i + 1]));
                chmin(dp[i + 1][1][1], dp[i][0][1] + S[i + 1]);
                chmin(dp[i + 1][0][1], dp[i][0][0] + S[i + 1]);
            }
            ll A = min(dp[n - 1][1][0], dp[n - 1][0][0]), B = min(dp[n - 1][0][1], dp[n - 1][1][1]);
            auto &d = dp[n - 1];
            if(p == 1 and q == 1) chmin(ans, A);
            if(p == 1 and q == 0) chmin(ans, min({d[0][1], d[1][0], d[0][0]}));
            if(p == 0 and q == 1) chmin(ans, min({d[0][1], d[1][0], d[1][1]}));
            if(p == 0 and q == 0) chmin(ans, B);
        }
        cout << ans << endl;
    }
}