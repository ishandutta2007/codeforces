...
const int MAX = 2e5 + 5;
const ll INF = 1e9;
template<int SZ> struct shortPath {
...
int eval(int i, int j){
    return st.dist[v[i]] + ed.dist[v[j]];
}
int getx(int i){
    return st.dist[v[i]];
}
...
int main(){
    ...
    st.dijkstra(0, n-1);
    ed.dijkstra(n-1, 0);
    int best = st.dist[n-1];
    set<pi> use;
    f0r(i, k){
        use.insert(mp(st.dist[a[i]] - ed.dist[a[i]], a[i]));
    }
    int ans = 0;
    for(auto x: use) v.eb(x.s);
    set<int> tmp;
    tmp.insert(getx(0));
    f1r(i, 1, sz(v)){
        int val = *(tmp.rbegin()) + gety(i);
        ckmax(ans, min(best, val+1));
        tmp.insert(getx(i));
    }
    ps(ans);
}