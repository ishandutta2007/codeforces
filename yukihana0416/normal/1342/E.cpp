...
void work()
{
    ...
    po[0] = 1;
    for(int i = 1; i <= n; i++) po[i] = po[i - 1] * Mint(i);

    inv[n] = Mint(1) / po[n];
    for(int i = n - 1; ~i; i--) inv[i] = inv[i + 1] * Mint(i + 1);
    ...
    else
    {
        k = n - k;
        Mint ans;
        auto C = [&](int n, int m) {return po[n] * inv[m] * inv[n - m];};
        for(int i = 0; i <= k; i++)
            ans += Mint(i % 2 == 0 ? 1 : -1) * C(k, i) * Mint(fp(k - i, n));
        cout <<C(n, k) * ans * Mint(2) <<endl;
    }
}
