...
int main() {
    TEST {
        ...
        VV(char, s, 2, n);
        auto a = toint(s, ".*");
        while(a[0][n - 1] == 0 and a[1][n - 1] == 0) {
            rep(i, 2) a[i].pop_back();
            n--;
        }
        vv(int, dp, n, 5, inf<int>);
        bool f = false;
        rep(i, n) {
            int u = a[0][i], d = a[1][i];
            int need = u + d * 2;
            if(f) {
                ...
                chmin(dp[i][1], dp[i - 1][3] + 1);
                chmin(dp[i][2], dp[i - 1][2] + 1);
                chmin(dp[i][2], dp[i - 1][3] + 1);
                chmin(dp[i][3], dp[i - 1][3] + 2);
                chmin(dp[i][3], dp[i][1] + 1);
                chmin(dp[i][3], dp[i][2] + 1);
                chmin(dp[i][3], dp[i - 1][4] + 3);
                chmin(dp[i][4], dp[i - 1][4] + 2);
                chmin(dp[i][4], dp[i - 1][1] + 1);
                chmin(dp[i][4], dp[i - 1][2] + 1);
                chmin(dp[i][4], dp[i - 1][3] + 1);
                rep(t, 1, 4) {
                    if((need & t) != need) dp[i][t] = inf<int>;
                }
            } else {
                if(u or d) {
                    if(u and d)
            ...
            if(a[0][i] or a[1][i]) f = true;
        }
        int u = a[0].back(), d = a[1].back();
        if(u and d)
            OUT(dp.back()[3]);
        else if(u)
            OUT(dp.back()[1]);
