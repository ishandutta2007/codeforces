...
void init()
{
    pref_sum = vector<vector<int>>(n+1, vector<int>(m+1));
    for (int i = 1; i<=n; i++)
        for (int j = 1; j<=m; j++)
        {
            pref_sum[i][j] = pref_sum[i-1][j] + pref_sum[i][j-1] - pref_sum[i-1][j-1];
            if (a[i-1][j-1]=='X') pref_sum[i][j]++;
        }
}
...
int get(int l, int r, int d, int u)
{
    ...
    d = max(0, d);
    u = min(m-1, u);
    return pref_sum[r+1][u+1] + pref_sum[l][d] - pref_sum[r+1][d] - pref_sum[l][u+1];
}
...
int main() {
    ...
    /*for (int i = 0; i<=n; i++)
    {
        //cout<<pref_sum[i].size();
        for (int j = 0; j<=m; j++) cout<<pref_sum[i][j]<<' ';
        cout<<endl;
    }*/
    ...
    for (int i = 0; i<n; i++)
    {
        for (int j = 0; j<m; j++)
        {
            if (get(i-t, i+t, j-t, j+t)==(2*t+1)*(2*t+1)) cout<<"X";
            else cout<<".";
        }
        cout<<'\n';
    }
