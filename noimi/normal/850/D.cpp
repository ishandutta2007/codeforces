...
main() {
    INT(n);
    VEC(int, a, n);
    vvv(bool, dp, n + 1, 100, 3000);
    vvv(pii, pre, n + 1, 100, 3000);
    dp[0][0][0] = 1;
    vi tr(100);
    rep2(i, 1, 98) tr[i + 1] = tr[i] + i;
    sort(all(a));
    rep(i, n) {
        rep(j, a[i] * 2 + 2) {
            rep(k, 3000) {
    ...
    rep2(i, 1, 64) {
        if(dp[n][i][tr[i]]) {
            ...
            int t = n;
            while(t > 0) {
                auto [jj, kk] = pre[t][j][k];
                rep(_, j - jj) v.eb(a[t - 1]);
                t--, j = jj, k = kk;
            }
            break;
        }
    ...
    auto id = iota(m);
    rep3(i, m - 1, 0) {
        rep(j, v[id[i]]) g[id[i]][id[j]] = 1, g[id[j]][id[i]] = 0;
        rep2(j, v[id[i]], i - 1) g[id[i]][id[j]] = 0, g[id[j]][id[i]] = 1, v[id[j]]--;
        sort(id.begin(), id.begin() + i, [&](int a, int b) { return v[a] < v[b]; });
    }
    rep(i, m) g[i][i] = 0;
    rep(i, m) {
        rep(j, m) { cout << g[i][j]; }
