...
int main() {
    ...
    while (t --> 0) {
        ...
        rep(i, n) {
            int v = a[i];
            inv[a[i] = cmp[v]].push_back(i);
        }
        int ans = 0;
        for (auto &l : inv) chmax(ans, size_as_int(l));
        rep(k, m - 1) {
            auto &p = inv[k];
            auto &q = inv[k + 1];
            rep(i, size_as_int(p)) {
                int j = q.end() - lower_bound(all(q), p[i]);
                chmax(ans, i + 1 + j);
            }
        }
        int l = 0;
        while (l < m) {
            int sum = 0;
            int r = l;
            while (r + 1 < m and *inv[r].rbegin() < *inv[r + 1].begin()) {
                sum += size_as_int(inv[r]);
                ++r;
            }
            if (l < r) {
                if (l > 0) {
                    sum += lower_bound(all(inv[l - 1]), *inv[l].begin()) - inv[l - 1].begin();
                }
                sum += inv[r].end() - lower_bound(all(inv[r]), *inv[r - 1].rbegin());
                chmax(ans, sum);
            }
            l = r + 1;
        }
