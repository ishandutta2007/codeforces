...
vi tree[nax];
bool did[nax];
int plus[nax], minus[nax];
int t[nax], h[nax];
ll glans = 0;
pll dfs(int x, int y = -1) {
	did[x] = true;
	ll tot = 0;
	vll diff;
	...
	sort(RALL(diff));
	vll disu = {0};
	for (ll x : diff) disu.push_back(x + disu.back());
	auto get = [&](int plus_cou) {
		assert(plus_cou >= 0 && plus_cou <= (int) diff.size());
		return tot + disu[plus_cou];
	};
	pll ans = {0, 0};
	for (int pluses = 0; pluses <= (int) diff.size(); ++pluses) {
		int minuses = diff.size() - pluses;
		ll gain = get(pluses);
		int tot_pluses = pluses + plus[x];
		int tot_minuses = minuses + minus[x];
		int pairs_in = min(tot_pluses, tot_minuses);
		maxi(ans.first, gain + pairs_in * 1ll * t[x]);
		maxi(ans.second, gain + pairs_in * 1ll * t[x]);
		if (tot_minuses > pairs_in && y != -1) {
			maxi(ans.first, gain + (pairs_in + 1ll) * t[x]);
		}
		if (tot_pluses > pairs_in && y != -1) {
			maxi(ans.second, gain + (pairs_in + 1ll) * t[x]);
		}
	}
	if (y == -1) {
		debug(imie(x), imie(ans));
		glans -= max(ans.first, ans.second);
	}
	rans;
}
void solve() {
	...
	for (int i = 1; i <= n; ++i) if (!did[i]) dfs(i);
	printf("%lld\n", glans);
}
int main() {
