...
ll compute(int i, int j) {
    return abs(a[i] - b[j]) + abs(b[j] - a[i]) - abs(a[i] - b[i]) - abs(a[j] - b[j]);
}
...
int eval(Node* ptr) {
    return ptr ? ptr->val : INF * 2;
}
Node nodes[202002 * 32 * 4];
auto lastn = nodes;
struct DTreeMin {
    Node* root = nullptr;
    void update(Node*& cur, int l, int r, int pos, int val) {
        ...
        if (l != r) {
            ...
            cur->val = min(eval(cur->lp), eval(cur->rp));
        }
    }
    ...
    void update(int pos, int val) {
        update(root, 0, T9 + 1, pos, val);
    }
    int getMin(int x, int y) {
        return getMin(root, 0, T9 + 1, x, y);
    }
};
...
//    DBN(events);
    for (auto e : events) {
        ...
        ai = ::a[idx];
        bi = ::b[idx];
//        DBN(idx, ai, bi, e.second < INF);
        if (e.second < INF) {
            tless.update(ai, -ai - bi - abs(ai - bi)); // -1 -4
            tbigger.update(ai, ai - bi - abs(ai - bi));
        } else {
            ll res = (ll)tless.getMin(0, bi) - abs(ai - bi) + ai + bi;
//            DBN(res, tless.getMin(0, bi));
            ans = min(ans, original + res);
            res = (ll)tbigger.getMin(bi, INF) - abs(ai - bi) + ai - bi;
//            DBN(res, tbigger.getMin(bi, INF));
            ans = min(ans, original + res);
            // bj <= ai
