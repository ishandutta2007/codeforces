...
long long dfsmax(int left, int now, int u) {
    // debug() << "dfsmax left = " << left << " now = " << now << " u = " << u;
    u = need[now];
    if (dig[now]) {
    ...
    // long long lmax1 = dfsmax(left - 1, lto[now]), lmax0 = dfsmax(left, lto[now]);
    // long long rmax1 = dfsmax(left - 1, rto[now]), rmax0 = dfsmax(left, rto[now]);
    // long long lmin1 = dfsmin(left - 1, lto[now]), lmin0 = dfsmin(left, lto[now]);
    // long long rmin1 = dfsmin(left - 1, rto[now]), rmin0 = dfsmin(left, rto[now]);
    dpmax[left][now] = -inf;
    if (ptag) {
    ...
    // debug() << "dpmax[" << left << "][" << now << "] = " << dpmax[left][now];
    vmax[left][now] = true;
    return dpmax[left][now];
}

long long dfsmin(int left, int now, int u) {
    // debug() << "dfsmin left = " << left << " now = " << now << " u = " << u;
    u = need[now];
    if (dig[now]) {
        if (left > 0 || u - left > 0) return inf;
        return val[now];
    ...
    vmin[left][now] = true;
    // debug() << "dpmin[" << left << "][" << now << "] = " << dpmin[left][now];
...
int main() {
    ...
    if (p < m) ptag = true; 
    dfs(1);
    // for (int i = 1; i <= cnt; ++i) debug() << imie(i) << imie(need[i]);
    cout << dfsmax(min(p, m), 1, 0) << endl;
    // for (int i = 1; i <= cnt; ++i) debug() << imie(i) << imie(lto[i]) << imie(rto[i]) << imie(dig[i]);
}