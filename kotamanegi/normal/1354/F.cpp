...
void solve() {
    ...
    REP(i, n) {
        int a, b;
        cin >> a >> b;
        inputs.push_back(make_tuple(b, a, i + 1));
    }
    sort(ALL(inputs));
    int ans = 0;
    vector<int> Ans;
    REP(i, inputs.size()) {
        ...
        REP(q, n-1) {
            REP(j, k) {
                if (dp[q][j].first + (k-1) * get<0>(Go[q]) > dp[q+1][j].first) {
                    dp[q + 1][j].first = dp[q][j].first + (k - 1) * get<0>(Go[q]);
                    dp[q + 1][j].second = 0;
                }
                if (dp[q][j].first + j * get<0>(Go[q]) + get<1>(Go[q]) > dp[q + 1][j + 1].first) {
                    dp[q + 1][j + 1].first = dp[q][j].first + j * get<0>(Go[q]) + get<1>(Go[q]);
                    dp[q + 1][j + 1].second = 1;
                }
            }
        }
        int eval = dp[n - 1][k - 1].first + get<1>(inputs[i]) + (k - 1) * get<0>(inputs[i]);
        if (eval > ans) {
            vector<int> Die;
            vector<int> PreAns;
            int now = k - 1;
            for (int q = n - 1; q > 0; --q) {
                if (dp[q][now].second == 0) {
                    Die.push_back(get<2>(Go[q-1]));
                }
                else {
                    PreAns.push_back(get<2>(Go[q - 1]));
                    now--;
                }
            }
            reverse(ALL(PreAns));
            REP(q, Die.size()) {
                PreAns.push_back(Die[q]);
                PreAns.push_back(-Die[q]);
            }
            PreAns.push_back(get<2>(inputs[i]));
            Ans = PreAns;
            ans = eval;
        }
    }
