...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=0; i<m; i++)
		{
			edges[i]=new pair(ni()-1,ni()-1);
			adjed[edges[i].a].add(i);
			adjed[edges[i].b].add(i);
		}
		sort(a);
		par=new int[n];
		size=new int[n];
		...
		for(int k=0; k<n; k++)
		{
			int i=a[k][1];
			added[i]=true;
			for(int j=0; j<adjed[i].size; j++)
			{
				if(added[edges[adjed[i].a[j]].a]&&added[edges[adjed[i].a[j]].b])
				{
					int te=findpar(edges[adjed[i].a[j]].a);
					int te1=findpar(edges[adjed[i].a[j]].b);
					if(te!=te1)
					{
						ans+=(hola[i]+0L)*size[te]*size[te1];
						if(size[te]>size[te1])
						{
							par[te1]=te;
							size[te]+=size[te1];
