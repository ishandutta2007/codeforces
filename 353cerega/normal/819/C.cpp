...
int main()
{
    ...
    while (t--)
    {
        ...
        {
            if (cur[0]%j==0 or cur[1]%j==0 or cur[2]%j==0)
            {
                ll cnt = 0;
                for (int k=0;k<3;k++)
                {
        ...
        for (int k=0;k<3;k++)
        {
            if (cur[k]!=1)
            {
                if (a.size()>0 and a.back().X==cur[k]) a.back().Y++;
                else a.push_back({cur[k],1});
            }
        }
        cur = s;
        for (int j=2;j<=1000;j++)
        ...
        for (int k=0;k<3;k++)
        {
            if (cur[k]!=1)
            {
                if (b.size()>0 and b.back().X==cur[k]) b.back().Y++;
                else b.push_back({cur[k],1});
            }
        }
        ll S = s[0]*s[1]*s[2];
        ll N = n[0]*n[1]*n[2];
