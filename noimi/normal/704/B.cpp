...
main() {
    ...
    rep(i, n) {
        ...
        if(i == s) {
            rep(j, n + 3) {
                if(j < k) continue;
                chmin(dp[i + 1][j + 1], dp[i][j] + d[i] - x[i]);
                if(j) chmin(dp[i + 1][j], dp[i][j] + c[i] + x[i]);
            }
        } else if(i == t) {
            rep(j, n + 3) {
                if(j < k) continue;
                chmin(dp[i + 1][j + 1], dp[i][j] + b[i] - x[i]);
                if(j) chmin(dp[i + 1][j], dp[i][j] + a[i] + x[i]);
            }
        } else {
            rep(j, n + 3) {
                if(j < k) continue;
                chmin(dp[i + 1][j + 1], dp[i][j] + b[i] + d[i] - x[i] * 2);
                if(j - (i > s)) chmin(dp[i + 1][j], dp[i][j] + b[i] + c[i]);
                if(j - (i > t)) chmin(dp[i + 1][j], dp[i][j] + d[i] + a[i]);
                if(j and j - (i > t) and j - (i > s)) chmin(dp[i + 1][j - 1], dp[i][j] + a[i] + c[i] + x[i] * 2);
            }
        }
    }
    cout << dp[n][1] << endl;
