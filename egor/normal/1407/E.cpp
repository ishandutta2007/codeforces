...
class EYegorVRespublikeDagestan {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        arri u, v, t;
        in.readArrays(m, u, v, t);
        decreaseByOne(u, v);

        Graph<WeightedEdge<int>> graph(n);
        for (int i : range(m)) {
            graph.addEdge(v[i], u[i], t[i]);
        }
        arri color(n, -1);
        color[n - 1] = 0;
        ...
        while (!q.empty()) {
            ...
            for (auto& e : graph[cur]) {
                if (dist[e.to] == infty && color[e.to] == e.weight) {
                    dist[e.to] = dist[cur] + 1;
                    q.push(e.to);
                } else if (color[e.to] != e.weight) {
                    color[e.to] = 1 - e.weight;
                }
            }
        }
        for (int& i : color) {
        ...
        out.printLine(dist[0] == infty ? -1 : dist[0]);
        for (int i : color) {
            out.print(i);
        }
        out.printLine();
...
int main() {
    ...
    auto time = clock();
#endif
    EYegorVRespublikeDagestan solver;
