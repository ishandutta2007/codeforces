...
struct item {
    ...
    void update(const item &first, const item &second, int l, int r) {
        s = first.s + second.s;
        ps = max(first.ps, first.s + second.ps);
        rs = max(second.rs, second.s + first.rs);
        w = max({first.w, second.w, first.rs + second.ps});
    }
...
void solve() {
    ...
    g.push_back({p[n], n});
    for (int i = n - 1; i >= 0; i--) {
        while (!g.empty() && g.back().fr <= p[i])
            g.pop_back();
        for (auto [r, id] : que[i]) {
            if (p[r + 1] != p[i] || (!g.empty() && g.back().sc <= r + 1)) {
                ans[id] = -1;
            } else {
                ans[id] = max(t[0].ask(i, r).w, t[1].ask(i, r).w);
            }
        }
    }
    for (ll i : ans)
