...
fn run() {
    ...
    let m = 200_000;
    let mut prime = vec![false; m + 1];
    enumerate_prime(m, |p| prime[p] = true);
    let valid = |m: u32| -> bool {
        ...
        let e = two;
        let mut g = maxflow::Graph::new(e.len() + 2);
        let src = e.len();
        let dst = src + 1;
        let inf = k + 1;
        let mut sum = 0;
        for (i, &(a, b)) in e.iter().enumerate() {
            ...
            for (j, &(_, d)) in e.iter().enumerate().take(i) {
                if prime[b + d] {
                    if b & 1 == 0 {
                        g.add_edge(i, j, inf);
                    } else {
