...
int main(void)
{
	...
	while(T--){
		...
		if(w % 2){
			...
			rep(i, 1, h/2) ans[w][2*i-1] = 2;
			w--;
		}
		rep(x, 1, w) rep(y, 1, h/2) ans[x][2*y-1] = 2;
		rep(y, 1, h) rep(x, 1, w) c[x][y] = 0;
		
		rep(y, 1, h/2) rep(x, 1, w/2){
			if(k <= 0) break;
			ans[2*x-1][2*y-1] = 1;
			ans[2*x-1][2*y] = 1;
			ans[2*x][2*y-1] = 0;
		...
		rep(y, 1, H) rep(x, 1, W){
			...
			if(ans[x][y] == 1) c[x][y] = c[x+1][y] = g+'a';
			if(ans[x][y] == 2) c[x][y] = c[x][y+1] = g+'a';
		}
