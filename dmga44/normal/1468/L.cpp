...
{
    map<ll,ll> res;
    vector<ll> f=Factor::factorize(x);
    for(auto y : f)
        res[y]++;
    return res;
}
...
int32_t main()
{
    ...
    int n,k;
    cin >> n >> k;
    map<ll,vector<ll>> onlys;
    map<ll,vector<ll>> ds;
    for(int i=0;i<n;i++)
    {
        ll x;
        cin >> x;
        map<ll,ll> dec=decomp(x);
        if(dec.size()==1)
        {
            for(auto y : dec)
                onlys[y.f].push_back(x);
        }
        else
        {
            vector<ll> dss;
    ...
    if(sum>=k)
    {
        if(extra || !(k&1))
        {
            for(int i=0;i<ks.size();i++)
            {
                ...
                if(res.size()+2<=k)
                {
                    res.push_back(onlys[y][0]);
                    res.push_back(onlys[y][1]);
                }
            }
            if(res.size()<k)
                for(auto y : avs)
                {
                    int p=2;
                    while(p<onlys[y].size() && res.size()<k)
                    {
                        res.push_back(onlys[y][p]);
                        p++;
                    }
                }
        }
        else
        {
            ...
            for(auto y : avs)
                if(res.size()+2<=k && kkkk.count(y)==0)
                {
                    res.push_back(onlys[y][0]);
                    res.push_back(onlys[y][1]);
                }
