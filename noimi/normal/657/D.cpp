...
int main() {
    ...
    map<frac, vi> mp;
    rep(i, n) { mp[frac(t[i], p[i])].eb(i); }

    vl mi(n), ma(n);
    ll sum = 0;
    fore(_, v, mp) {
        fore(i, v) mi[i] = sum + t[i];
        fore(i, v) sum += t[i];
        fore(i, v) ma[i] = sum;
    }

    map<ll, vi> mp2;
    rep(i, n) mp2[p[i]].eb(i);

    vvc<int> idx;
    fore(_, v, mp2) idx.eb(v);

    OUT(bin_search_double(0.0, 1.0, [&](double x) {
        double pre = 0;
        fore(v, idx) {
            fore(i, v) {
                if(p[i] * (T - ma[i] * x) < pre) return false;
            }
            fore(i, v) { chmax(pre, p[i] * (T - mi[i] * x)); }
        }
        return true;
    }));
}