...
class dynamic_array {
    public:
        dynamic_array(){}
        explicit dynamic_array(const T default_value) : _default_value(default_value) {}
        explicit dynamic_array(int n, const T default_value) : data(n), _default_value(default_value) {}
    ...
    private:
        ...
        void ensure_size(int n) {
            if (data.size() < n + 1) {
                data.resize(n + 1, _default_value);
            }
        }
};

using state = tuple<int, int, int, int, string>;
const state default_state = {IINF, 0, 0, 0, ""};

constexpr int cost(const state &s) { return get<0>(s); } 
...
int main() {
    ...
    vec2<dynamic_array<state>> dp(n + 1, vec<dynamic_array<state>>(m + 1, dynamic_array<state>(default_state)));
    dp[0][0][0] = {0, -1, -1, -1, ""};
    rep(i, n + 1) rep(j, m + 1) {
        int l = dp[i][j].size();
        if (i == n and j == m) {
            ...
            int argmin = 0;
            rep(k, l) {
                if (chmin(min_length, cost(dp[i][j][k]) + k)) {
                    argmin = k;
                }
            }
            string ans = string(argmin, ')');
            int ci = n, cj = m, ck = argmin;
            while (ci > 0 or cj > 0) {
                auto [_, ni, nj, nk, add] = dp[ci][cj][ck];
                ans += add;
                ci = ni, cj = nj, ck = nk;
            }
            reverse(all(ans));
        ...
        rep(k, l) {
            if (i < n and j < m) {
                if (s[i] == '(' and t[j] == '(') {
                    if (cost(dp[i][j][k]) + 1 < cost(dp[i + 1][j + 1][k + 1])) {
                        dp[i + 1][j + 1][k + 1] = { cost(dp[i][j][k]) + 1, i, j, k, "(" };
                    }
                } else if (s[i] == '(' and t[j] == ')') {
                    if (cost(dp[i][j][k]) + 1 < cost(dp[i + 1][j][k + 1])) {
                        dp[i + 1][j][k + 1] = { cost(dp[i][j][k]) + 1, i, j, k, "(" };
                    }
                    if (k > 0) {
                        if (cost(dp[i][j][k]) + 1 < cost(dp[i][j + 1][k - 1])) {
                            dp[i][j + 1][k - 1] = { cost(dp[i][j][k]) + 1, i, j, k, ")" };
                        }
                    }
                } else if (s[i] == ')' and t[j] == '(') {
                    if (cost(dp[i][j][k]) + 1 < cost(dp[i][j + 1][k + 1])) {
                    ...
                    if (k > 0) {
                        ...
                        if (cost(dp[i][j][k]) + 2 < cost(dp[i + 1][j + 1][k])) {
                            dp[i + 1][j + 1][k] = { cost(dp[i][j][k]) + 2, i, j, k, ")(" };
                        }
                    }
                }
            } else if (i < n) {
                if (s[i] == '(') {
                    ...
                    if (k > 0) {
                        ...
                        if (cost(dp[i][j][k]) + 2 < cost(dp[i + 1][j][k])) {
                            dp[i + 1][j][k] = { cost(dp[i][j][k]) + 2, i, j, k, ")(" };
                        }
                    }
                }
            } else if (j < m) {
                if (t[j] == '(') {
                    ...
                    if (k > 0) {
                        ...
                        if (cost(dp[i][j][k]) + 2 < cost(dp[i][j + 1][k])) {
                            dp[i][j + 1][k] = { cost(dp[i][j][k]) + 2, i, j, k, ")(" };
                        }
                    }
                }
            }
