...
void dfs(int v, int p, vector<vector<int>> &dp, int k) {
	vector<pair<int, int>> a;
	for(auto i: G[v]) {
		if (i.first == p)
			continue;
		dfs(i.first, v, dp, k);
		a.push_back({i.second - dp[i.first][0] + dp[i.first][1], i.first});
	}
	...
	for(int i = 0; i < min((long long) a.size(), k - 1); ++i) {
		if (a[i].first < 0)
			break;
		res += a[i].first;
	}
	dp[v][1] = res;
	if (k <= a.size() && a[k - 1].first >= 0) 
		res += a[k - 1].first;
	dp[v][0] = res;
	// cout << '!' << dp[v][0] << ' ' << dp[v][1] << '\n';
	// cout << '\n';
