...
int main(){
    ...
    rep(ii,T){
        ...
        graph G(N+1);
        rep(i,N-1){
            ll p; cin >> p; G.unite(p,i+2,1,true);
        }
        //prepare
        //main
        if(logN==0){
        ...
        while(l<r){
            ...
            rep(i,K+1){
                ...
                auto itr = vs.end(); itr--; auto[d,id] = *itr;
                if(d<=c){
                    ok = true; break;
                }
                if(i==K) break;
                ...
                while(!go.empty()){
                    int s = go.front(); go.pop();
                    vs.erase(make_pair(v[s],(ll)s));
                    for(auto[x,cc,id]:G.G[s]){
                        if(dist[x]!=2e9) continue;
                        dist[x] = 0;
                        go.push(x);
                    }
                }
