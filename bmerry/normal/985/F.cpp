...
static constexpr mr hx = 37;
static constexpr mr ihx = hx.inverse();

static array<int, 26> extract(const array<vector<mr>, 26> &roll, int x, int L)
{
    array<int, 26> out;
    mr inv = power(ihx, x);
    for (int i = 0; i < 26; i++)
        out[i] = ((roll[i][x + L] - roll[i][x]) * inv).get();
    sort(RA(out));
    return out;
}
...
static void solve()
{
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        x--;
        y--;
        auto sig1 = extract(roll, x, L);
        auto sig2 = extract(roll, y, L);
        if (sig1 == sig2)
            cout << "YES\n";
        else
            cout << "NO\n";
