...
void main_() {
	...
	Graph H = G.root_to_leaf(s);
	auto d1 = G.bfs(s), d2 = G.bfs(t);

	VV<> sdist(dia + 1), tdist(dia + 1);

	auto dfs2 = [&](auto self, int now, int d, bool is_s) -> void {
		debug(d);
		if(is_s)
			sdist[d].push_back(now);
		else
			tdist[d].push_back(now);
		for([[maybe_unused]] auto &[to, ig] : H[now]) {
			self(self, to, d + 1, is_s);
		}
	};
	auto dfs = [&](auto self, int now) -> void {
		path.push_back(now);
		for([[maybe_unused]] auto &[to, ig] : H[now]) {
			if(d1[to] + d2[to] == dia)
				...
				if(d1[to] <= d2[to])
					dfs2(dfs2, to, d2[to], 0);
				else
					dfs2(dfs2, to, d1[to], 1);
			}
		}
	};
	dfs(dfs, s);
	...
	RREP(i, dia + 1) {
		for(auto &v : sdist[i]) {
			res.push_back(T(v + 1, s + 1, v + 1));
			score += i;
		}
		for(auto &v : tdist[i]) {
			res.push_back(T(v + 1, t + 1, v + 1));
			score += i;
		}
	}
	...
	REP(i, SZ(path) - 1) {
		score += dia - i;
		res.push_back(T(path[i] + 1, t + 1, path[i] + 1));
	}
