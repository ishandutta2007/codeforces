...
int Sdfs(int now, int back) {
    dist[now] = 0;
    for (auto x : vertexs[now]) {
        if (x == back) continue;
        dist[now] = max(dist[now], Sdfs(x, now) + 1);
    }
    return dist[now];
}
void solve(){
    ...
    REP(i, n) {
        ...
        int itr = 0;
        REP(q, n) {
            if (dist[q] == 1e9) continue;
            if (dist[itr] == 1e9 or dist[q] > dist[itr]) {
                itr = q;
            }
        }
        Sdfs(itr, -1);
        int len = dist[itr] / 2;
        go.push_back(mp(dist[itr] - len,-1));
    ...
    reverse(ALL(go));
    if (go.size() > 1) {
        ans = max(ans, go[0].first + go[1].first + 1);
        if (go.size() > 2) {
            ans = max(ans, go[1].first + go[2].first + 2);
        }
    }
    cout << ans << endl;
    for (int q = 1; q < go.size(); ++q) {
        cout << go[0].second + 1 << " " << go[q].second + 1 << endl;
    }
}
#undef int
int main() {
