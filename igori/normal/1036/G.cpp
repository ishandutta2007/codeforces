...
vector<int> psevdograph[2000000];
vector<int> istoki;
vector<int> stoki;
vector<vi> reachable;
ll color[4000000];
...
signed main()
{
    ...
    for (int i = 1; i + 1 < (1 << k); i++)
    {
        ...
        for (int j = 0; j < k; j++)
        {
            if ((i & (1 << j)))
            {
                for (int q = 0; q < reachable[j].sz; q++)
                {
                    r[is_stok[reachable[j][q]] - 1] = 1;
                    //cout << istoki[j] << " " << reachable[istoki[j]][q] << endl;
                }
            }
        }
        ll cnt = 0;
