...
int main() {
    ...
    auto f = [](S a, S b) {
        int k = min(a.a, b.b);
        return S{a.a + b.a - k, a.b + b.b - k, a.c + b.c + k};
    };
    SegmentTree<S> seg(si(s), f, S{0, 0, 0});
    rep(i, si(s)) {
        if(s[i] == '(') {
            seg.set(i, S{1, 0, 0});
        } else
    ...
    while(m--) {
        INT(l, r);
        l--;
        cout << seg.query(l, r).c * 2 << endl;
    }
    // vll l{0}, r{0};
    // for(auto e : s) { l.eb(l.back() + (e == '(')); }
    // reverse(all(s));
    // for(auto e : s) { r.eb(r.back() + (e == ')')); }
    // INT(m);
    // dump(l, r);
    // while(m--) {
    //     INT(x, y);
    ...
    //     while(ok < ng - 1) {
    //         int mid = ok + ng >> 1;
    //         if(l[x] + mid > l.back() or r[y] + mid > r.back()) {
    //             ng = mid;
    //             continue;
    //         }
    //         int X = lb(l, l[x] + mid), Y = lb(r, r[y] + mid);
    //         dump(ok, ng, X, Y);
    //         (X + Y <= si(s) ? ok : ng) = mid;
    //     }
    //     cout << ok * 2 << endl;
