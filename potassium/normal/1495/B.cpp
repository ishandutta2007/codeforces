...
int main() {
	...
	peak.insert(peak.begin(), 0);
	peak.emplace_back(n - 1);

	const int m = peak.size();
	vector<int> len(m);
	for (int i = 0; i < m; i++)
		len[i] = max(l[peak[i]], r[peak[i]]);
	...
	for (int i = 1; i < m - 1; i++) {
		int cur = peak[i];
		int x = min(l[cur], r[cur]);
		int y = max(l[cur], r[cur]);
		assert(x > 0);
