...
int main() {
    ...
    auto solve = [&](auto self, int l, int r) -> V<ll> {
        V<ll> ans(r - l + 1, -TEN(18));
        ans[0] = 0;
        if (l == r) return ans;
        int k = -1;
        for (int i = l; i < r; i++) {
        ...
        auto ldp = self(self, l, k);
        auto rdp = self(self, k + 1, r);
        int ls = int(ldp.size()), rs = int(rdp.size());
        for (int i = 0; i < ls; i++) {
            for (int j = 0; j < rs; j++) {
                ans[i + j] = max(ans[i + j], ldp[i] + rdp[j] - 2LL * i * j * a[k]);
                ans[i + j + 1] =
                    max(ans[i + j + 1], m * a[k] + ldp[i] + rdp[j] - (2LL * (i + 1) * (j + 1) - 1) * a[k]);
            }
        }
