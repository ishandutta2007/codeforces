...
void solve()
{
    ...
    ll Y = mid/b - XY;
    if (x<y)
    {if (pref[XY]*(x+y) + (pref[XY+Y]-pref[XY])* y + (pref[XY+X+Y]-pref[XY+Y])*x < k) {cout<<-1<<endl; return;}}

    else
    {if (pref[XY] * (x + y) + (pref[XY + X] - pref[XY]) * x + (pref[XY + X + Y] - pref[XY + X]) * y < k) {cout<<-1<<endl; return;}}
    ...
    while (r>l+1)
    {
        ...
        if (x<y)
        {
            if (pref[XY]*(x+y) + (pref[XY+Y]-pref[XY])* y + (pref[XY+X+Y]-pref[XY+Y])*x >=k) r = mid;
            else l = mid;
        }
        else {
            if (pref[XY] * (x + y) + (pref[XY + X] - pref[XY]) * x + (pref[XY + X + Y] - pref[XY + X]) * y >= k)
                r = mid;
            else l = mid;
        }
    }
