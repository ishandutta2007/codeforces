    ...
    TEST {
        ...
        rep(i, n) {
            if(ed <= t[i]) {
                ...
                ll a = (to - fr) / (to - fr) * (t[i] - st);
                ll b = (to - fr) / (to - fr) * (t[i + 1] - st);
                if(to > fr) {
                    if(fr + t[i] - st <= x[i] and x[i] <= min(to, fr + t[i + 1] - st)) ans++;
                } else {
                    if(fr - (t[i] - st) >= x[i] and x[i] >= max(to, fr - (t[i + 1] - st))) ans++;
                }
            }
        }
        cout << ans << endl;
