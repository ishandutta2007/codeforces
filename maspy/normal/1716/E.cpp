...
#line 1 "/home/maspy/compro/library/ds/xor_sparsetable.hpp"
// 
...
struct Mono {
  ...
  static X op(X x, X y) {
    ...
    if (sx == INF) return y;
    if (sy == INF) return x;
    return {sx + sy, max(lx, sx + ly), max(rx + sy, ry),
            max({rx + ly, tx, ty})};
  }
  static constexpr X unit() { return {INF, INF, INF, INF}; }
...
void solve() {
  ...
  using T = Mono::X;
  vc<T> seg_raw(1 << N);
  FOR(i, 1 << N) seg_raw[i] = {A[i], A[i], A[i], max(0LL, A[i])};

  Xor_SparseTable<Mono> seg(seg_raw);
  LL(Q);
  ll v = 0;
  FOR(Q) {
    LL(k);
    v ^= 1 << k;
    auto t = seg.prod(0, 1 << N, v);
    print(get<3>(t));
  }
}
