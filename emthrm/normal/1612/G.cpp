...
int main() {
  ...
  vector<int> w_((C - 1) * 2 + 1, 0);
  const auto w = w_.begin() + (C - 1);
  REP(i, m) {
    ++w[-c[i] + 1];
    if (c[i] + 1 <= C - 1) --w[c[i] + 1];
  }
  for (int i = -(C - 1) + 2; i <= C - 1; ++i) w[i] += w[i - 2];
  ll p = 0;
  ModInt f = 0, num = 1;
  for (int i = -(C - 1); i <= C - 1; ++i) {
    f += ModInt(w[i]) * (p * 2 + w[i] - 1) / 2 * i;
    p += w[i];
    num *= ModInt::fact(w[i]);
  }
  cout << f << ' ' << num << '\n';
  ...
  vector<int> a;
  REP(i, m) REP(_, c[i]) a.emplace_back(i);
  do {
    vector<vector<int>> p(m);
    REP(i, a.size()) p[a[i]].emplace_back(i);
    int g = 0;
    REP(i, m) {
      REP(x, c[i]) FOR(y, x + 1, c[i]) g += p[i][y] - p[i][x];
    }
    if (chmax(max_f, g)) {
      ans_a.clear();
      ans_a.emplace_back(a);
    } else if (g == max_f) {
      ans_a.emplace_back(a);
    }
  } while (next_permutation(ALL(a)));
  cout << max_f << '\n';
  for (const vector<int>& ans_a_i : ans_a) {
    REP(i, ans_a_i.size()) cout << ans_a_i[i] << " \n"[i + 1 == ans_a_i.size()];
  }
}