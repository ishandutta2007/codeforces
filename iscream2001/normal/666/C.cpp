...
LL ans[100005];
char s[100005];
LL po25[100005],po26[100005];
LL F[100005];
void MAIN(){
	po25[0]=po26[0]=1;
	po25[1]=25;
	po26[1]=qpow((LL)26,M-2,M);
	for(int i=2;i<=100000;++i){
		po25[i]=po25[i-1]*(LL)po25[1]%M;
		po26[i]=po26[i-1]*(LL)po26[1]%M;
	}
	int u,v,nl,op;
	cin>>m;K=0;
	cin>>s;nl=strlen(s);
	...
	for(int i=1,j;i<=K;i=j+1){
		F[0]=0;
		for(LL o=1;o<=100000;++o) {
			F[o]=F[o-1];
			if(o>=a[i].n){
				F[o]=(F[o]+C(o-1,a[i].n-1)*po25[o-a[i].n]%M*po26[o])%M;
				
			}
			
		}
		j=i;
		while(j+1<=K&&a[j+1].n==a[i].n) ++j;
		for(int k=i;k<=j;++k){
			ans[a[k].id]=F[a[k].L]*qpow((LL)26,a[k].L,M)%M;
		}
	}
	for(int i=1;i<=K;++i){
		cout<<ans[i]<<"\n";
