...
int main(){
    ...
    VEC(ll,a,n);
    vec(ll,cut,n,1);
    auto cost=[](ll length,ll cut){
        const ll l1=length/cut,l2=l1+1,c2=length%cut,c1=cut-c2;
        return l1*l1*c1+l2*l2*c2;
    };
    priority_queue<pll>q;
    rep(n)q.push({cost(a[i],cut[i])-cost(a[i],cut[i]+1),i});
    rep(_,k-n){
        const ll i=q.top().second;
        q.pop();
        cut[i]++;
        q.push({cost(a[i],cut[i])-cost(a[i],cut[i]+1),i});
    }
    ll ans=0;
    rep(n)ans+=cost(a[i],cut[i]);
    out(ans);
}