...
pair<int, pair<int, int>> merge(pair<int, pair<int, int>> a, pair<int, pair<int, int>> b) {
	...
	ans.second.first = a.second.first + b.second.first - min(b.second.first, a.second.second);
	ans.second.second = b.second.second + a.second.second - min(a.second.second, b.second.first);
	ans.first = b.first + a.first + min(b.second.first, a.second.second) + min(a.second.second, b.second.first);
	return ans;
}
...
pair<int, pair<int, int>> get(int v, int l, int r, int L, int R) {
	if (r <= L || R <= l) 
		return {0, {0, 0}};
	if (l >= L && r <= R) 
		return t[v];
