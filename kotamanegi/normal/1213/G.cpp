...
void solve() {
    ...
    REP(i, m) {
        int b;
        cin >> b;
        queries.push_back(mp(b, i));
    }
    sort(ALL(queries));
    reverse(ALL(queries));
    edges.push_back(make_tuple(1e9, 0, 0));
    int now_ans = 0;
    REP(i, edges.size()) {
        while (queries.empty() == false&&get<0>(edges[i]) > queries.back().first) {
            ans[queries.back().second] = now_ans/2LL;
            queries.pop_back();
        }
        get<1>(edges[i]) = uf(get<1>(edges[i]));
        get<2>(edges[i]) = uf(get<2>(edges[i]));
        if (uf(get<1>(edges[i])) == uf(get<2>(edges[i]))) continue;
        now_ans -= sizing[get<1>(edges[i])] * (sizing[get<1>(edges[i])] - 1LL);
        now_ans -= sizing[get<2>(edges[i])] * (sizing[get<2>(edges[i])] - 1LL);
