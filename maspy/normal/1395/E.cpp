...
void solve() {
  ...
  vc<u64> base(N);
  FOR(i, N) base[i] = RNG(1LL << 60);
  u64 full = SUM(base);

  vv(ll, dat, K + 1, K);
  FOR(v, N) {
    vc<pi> tmp;
    for (auto&& e: G[v]) tmp.eb(e.cost, e.to);
    sort(all(tmp));
    FOR(k, len(tmp)) { dat[len(tmp)][k] += base[tmp[k].se]; }
  }

  ll ANS = 0;
  auto dfs = [&](auto& dfs, int k, u64 now) -> void {
    if (k == K + 1) {
      if (now == full) ++ANS;
      return;
    }
    FOR(i, k) { dfs(dfs, k + 1, now + dat[k][i]); }
  };
  dfs(dfs, 1, 0);
  print(ANS);
}
