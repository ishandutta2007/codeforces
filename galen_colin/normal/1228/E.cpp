...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		for (ll j = 0; j <= n; j++) {
			...
			ll force = (cov - mpow(k - 1, j) + mod) % mod; // take our 1 from those already 1
			
			force = (force * mpow(k - 1, n - j)) % mod;
						
			dp[i + 1][j] = (dp[i + 1][j] + dp[i][j] * force) % mod;
			
			for (ll l = 1; l <= n - j; l++) {
				ll cur = (dp[i][j] * cov) % mod;
				cur = (cur * mpow(k - 1, (n - j) - l)) % mod;
				cur = (cur * nck(n - j, l)) % mod;
				
				dp[i + 1][j + l] = (dp[i + 1][j + l] + cur) % mod;
			}
		}
	}
