...
ll n, S;
vec2d p[5005];
vector<vec2d> vec, cvec;

ll get(vec2d a, vec2d b, vec2d c)
{
...
int main(void)
{
	...
	rep(i, 0, n-3){
		...
		rep(j, i+1, n-2){
			while(1){
				ll nk = (k+1)%n;
				if(get(cvec[i], cvec[j], cvec[k]) >= get(cvec[i], cvec[j], cvec[nk])) break;
				k = nk;
			}
			ll res = get(cvec[i], cvec[j], cvec[k]);
			if(mx < res){
				mx = res;
				a = i, b = j, c = k;
			}
	...
	vec2d ans[3];
	ans[0] = cvec[c] + cvec[b] - cvec[a];
	ans[1] = cvec[a] + cvec[c] - cvec[b];
	ans[2] = cvec[b] + cvec[a] - cvec[c];
	rep(i, 0, 2) cout << ans[i].x << " " << ans[i].y << endl;
