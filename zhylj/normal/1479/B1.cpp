...
int main() { 
	...
	while(T--) {
		...
		A[0] = (A[1] == 1 ? 2 : 1);
		for(int i = 1; i <= n; ++i) {
			int f_ai = std::max(Query(1, 1, n, 1, A[i] - 1), Query(1, 1, n, A[i] + 1, n)) + 1;
			f_ai = std::max(f_ai, Query(1, 1, n, A[i], A[i]));
			if(A[i] != A[i - 1]) Modify(1, 1, n, 1, n, 1);
			Assign(1, 1, n, A[i - 1], f_ai);
		}
		printf("%d\n", Query(1, 1, n, 1, n));
	} return 0;
}