...
int main()
{
	...
	for(int i=1;i<=Q;++i)++q,read(sp[q].num),read(sp[q].w);
	...
	for(int i=1,l,w;i<=t;++i)
	{
		...
		for(int j=0;j<=w+1;++j)
			if(!((l>>j)&1))
				Mat[0].a[i][to[l|(1<<j)]]=c[w+1-j];*/
	}
	//for(int i=1;i<=t;++i)
	//	for(int j=1;j<=t;++j)
	//		printf("%lld%c",Mat[0].a[i][j]>1000000000? -1:Mat[0].a[i][j],j==t? '\n':' ');
	...
	//	for(int i=1;i<=t;++i)
	//		for(int j=1;j<=t;++j)
	//			printf("%lld%c",ans.a[i][j]>1000000000? -1:ans.a[i][j],j==t? '\n':' ');
	}
