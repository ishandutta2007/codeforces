...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  mat ord,g(N);
  {
    ...
    rep1(i,N){
      ...
      dig[p[i]].emplace_back(1,i);
    }
    auto dist=dijkstra(dig,0);
    ord.resize(*max_element(all(dist))+1);
    rep(i,N)ord[dist[i]].emplace_back(i);
  }
  rep1(i,N)cin>>v[i];
  ll bfmax=0,bfmin=0;
  for(auto ch:ord){
    ll nowmax=0,nowmin=inf;
    for(auto x:ch){
      chmax(nowmax,v[x]);
      chmin(nowmin,v[x]);
      chmax(dp[x],max(bfmax-v[x],bfmin+v[x]));
    }
    bfmax=0;bfmin=-inf;
    for(auto x:ch){
      chmax(dp[x],dp[p[x]]+max(nowmax-v[x],v[x]-nowmin));
      for(auto child:g[x]){
        chmax(bfmax,dp[x]+v[child]);
        chmax(bfmin,dp[x]-v[child]);
      }
    }
  }
