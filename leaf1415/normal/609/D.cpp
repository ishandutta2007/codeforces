...
ll t[200005], c[200005];
vector<P> ans;

ll check(ll d, bool flag = false)
{
	ll A = 0, B = 0;
	rep(i, 1, d){
		if(a[A] > a[i]) A = i;
	...
	vector<P> vec;
	rep(i, 1, m){
		if(t[i] == 1) vec.push_back(P(c[i]*a[A], i));
		if(t[i] == 2) vec.push_back(P(c[i]*b[B], i));
	}
	sort(all(vec));
	...
	rep(i, 0, k-1){
		...
		if(flag){
			ll id = vec[i].second;
			if(t[id] == 1) ans.push_back(P(id, A));
			if(t[id] == 2) ans.push_back(P(id, B));
		}
	}
	return ret;
}
...
int main(void)
{
	...
	rep(i, 1, n) cin >> b[i];
	a[0] = b[0] = inf;
	rep(i, 1, m) cin >> t[i] >> c[i];
	
	ll ub = n+1, lb = 0, mid;
	while(ub-lb>1){
		mid = (ub+lb)/2;
		if(check(mid) <= s) ub = mid;
		else lb = mid;
	}
	if(ub == n+1){
		outl(-1);
