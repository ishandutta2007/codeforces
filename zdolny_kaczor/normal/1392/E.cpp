...
#undef u
pll uni(pll a, pll b) {
	assert(max(a.first, b.first) <= min(a.second, b.second));
	return {min(a.first, b.first), max(a.second, b.second)};
}
ll a[nax][nax];
pair <ll, ll> seg[nax][nax];
bool in(pll s, ll x) {
	debug(imie(s), imie(x));
	return x >= s.first && x < s.second;
}
void solve() {
	int n;
	scanf("%d", &n);
	...
	for (int s = 1; s <= 2 * (n - 1); ++s) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (i == 0) {
				...
				ll v = low - seg[i - 1][j].first;
				a[i][j] = v;
				seg[i][j] = seg[i - 1][j] + v;
				if (j) {
					seg[i][j] = uni(seg[i][j], seg[i][j - 1] + v);
				}
				low = seg[i][j].second;
			}
		}
	}
	REP(i, n) REP(j, n) maxi(lar, a[i][j]);
	debug(dump(a, 0, n, 0, n));
	REP(i, n) {
