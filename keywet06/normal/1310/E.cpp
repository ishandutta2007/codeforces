...
int main() {
    ...
    if (k == 1) {
        ...
        f.assign(64, std::vector<int>(n + 1));
        f[0][0] = 1;
        for (int i = n; i >= 1; --i) {
            for (int c = 0; c < 63; ++c) {
                for (int s = 0; s + (c + 1) * i <= n; ++s) {
                    f[c + 1][s + (c + 1) * i] =
                        (f[c + 1][s + (c + 1) * i] + f[c][s]) % P;
                }
            }
        }
        for (int i = 1; i <= 63; ++i) {
