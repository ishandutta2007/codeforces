...
signed main(){
    ...
    q.push({0,r-1,c-1,0,0});
    while(!q.empty()){
        auto t=q.top();q.pop();
        if(s[t[1]][t[2]]-'.')continue;
        s[t[1]][t[2]]='+';
        if(t[1]>0&&s[t[1]-1][t[2]]=='.'){
            --t[1];
            q.push(t);
            ++t[1];
        }
        if(t[1]<n-1&&s[t[1]+1][t[2]]=='.'){
            ++t[1];
            q.push(t);
            --t[1];
        }
        if(t[2]>0&&s[t[1]][t[2]-1]=='.'&&t[3]<x){
            q.push({t[0]+1,t[1],t[2]-1,t[3]+1,t[4]});
        }
        if(t[2]<m-1&&s[t[1]][t[2]+1]=='.'&&t[4]<y){
            q.push({t[0]+1,t[1],t[2]+1,t[3],t[4]+1});
        }
    }
    ll ans=0;
    rep(n)rep(j,m)if(s[i][j]=='+')++ans;
    out(ans);
}