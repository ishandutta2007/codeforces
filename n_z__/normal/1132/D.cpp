...
/*namespace-head----------------------------------------------------------------------------------*/
int n,k;
pair<double,pair<int,int>>c[200001];
bool check(int m){
//cout<<endl;
    ...
    for(int x=1;x<=k;x++){
        ...
        pq.pop();
        //cout<<x<<','<<a.first<<endl;
		if(a.first-x<-1)return 0;
		a.first+=m*1.0/a.second.second;
        pq.push(a);
	}return 1;
}
/*var-and-func------------------------------------------------------------------------------------*/
...
/*Main-head---------------------------------------------------------------------------------------*/
    cin>>n>>k;
    for(int x=1;x<=n;x++)
    cin>>c[x].second.first;
    for(int x=1;x<=n;x++)
    cin>>c[x].second.second,c[x].first=1.0*c[x].second.first/c[x].second.second;
    int l=0,r=10000000000000,m,ans=-1;
    while(l<=r)
    {
        m=(l+r)/2;
