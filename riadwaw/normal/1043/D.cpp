...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		auto p = make_vector<int>(cnt, len, 0);
		auto w = make_vector<int>(cnt, len, 0);
		for (int i: range(cnt)) {
			for (int j: range(len)) {
				in >> p[i][j];
				--p[i][j];
		...
		for (int pos = 0; pos < len;) {
			int first_number = p[0][pos];
			vector<int> indexes(cnt);
			for (int i: range(cnt)) {
				indexes[i] = w[i][first_number];
			}
			assert(pos == indexes[0]);
			while (true) {
				bool all_same = true;
				for (int i: range(cnt)) {
					if (indexes[i] + 1 < len && p[i][indexes[i] + 1] == p[0][indexes[0] + 1]) {
			...
			int new_pos = indexes[0] + 1;

			ans += (new_pos - pos) * 1LL * (new_pos - pos + 1) / 2;
			pos = new_pos;
