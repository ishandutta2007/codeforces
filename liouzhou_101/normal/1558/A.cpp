...
void go(int a, int b)
{
	...
	for (int ax = 0; ax <= a && ax <= x; ++ax) // Alice won ax games as a serving player
	{
		int bx = x - ax;
		int ay = a - ax;
		int by = b - bx;
		if (0 <= ax && ax <= a && 0 <= ay && ay <= a && 0 <= bx && bx <= b && 0 <= by && by <= b
			&& 0 <= ax && ax <= x && 0 <= ay && ay <= y && 0 <= bx && bx <= x && 0 <= by && by <= y
			&& ax + ay == a && bx + by == b && ax + bx == x && ay + by == y)
		{
			u[ay + bx] = 1;
		}
	}
