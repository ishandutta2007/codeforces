...
void solve() {
	...
	rep(i, n) {
		ori[i][0] = { a[i],1 };
		rep1(j, sz - 1)ori[i][j] = { 1,0 };
	}
	vector<P> vp;
	auto f = [&](ar a, ar b) {
		vp.clear();
		rep(i, sz)vp.push_back({ a[i].second,a[i].first });
		rep(i, sz) {
			int chk = -1;
			rep(j, sz)if (vp[j].second == b[i].first) {
				chk = j; break;
			}
			if (chk < 0) {
				vp.push_back({ b[i].second,b[i].first });
				sort(all(vp), greater<P>());
				rep(j, sz) {
					vp[j].first -= vp[sz].first;
				}
				vp.pop_back();
			}
			else {
				vp[chk].first += b[i].second;
				sort(all(vp), greater<P>());
			}
		}
		rep(i, sz) {
			a[i] = { vp[i].second,vp[i].first };
		}
		return a;
	};
	auto g = [&](ar a, int b, int l, int r)->ar {
		if (b < 0)return a;
		a[0] = { b,r - l };
		rep1(i, sz-1)a[i] = { 1,0 };
		return a;
	...
	rep(i, q) {
		...
		if (typ == 1) {
			...
			int l, r; cin >> l >> r; l--; r--;
			ar res = st.query(l, r + 1);
			vector<int> ans;
			rep(i, 100 / p)ans.push_back(res[i].first);
			cout << ans.size() << " "; coutarray(ans);
		}
	}
}
