...
void main_() {
	...
	auto dfs = [&](auto dfs, V<set<int>> sts, V<> res, V<> cnt, V<> lim) -> void {
		if(SZ(ans)) return;
		if(SZ(res) == n) {
			reverse(all(res));
			ans = res;
		...
		sts = move(nsts);
		REP(i, n) {
			if(cnt[i] == (SZ(res) != n - 1 ? 1 : 0) and SZ(res) < lim[i]) {
				cnt[i] = -1;
				res.pb(i);
				auto nlim = lim;
				for(auto &st : sts) {
					if(st.count(i)) {
						foa(v, st) {
							chmin(nlim[v], SZ(st) + SZ(res) - 1);
						}
					}
				}
				dfs(dfs, sts, res, cnt, nlim);
				cnt[i] = 1;
				res.pop_back();
			}
		}
