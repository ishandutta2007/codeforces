...
int n2 = 0;
int go(vector<int> v);
//int dfs(int mask, int cur, vector<int>& v) {
//
//}
int go(vector<int> v) {
...
int solve() {
    ...
    for (auto& p : dp) {
        ...
        int e = *min_element(t.begin(), t.end());
        for (int& x : t) {
            x -= e;
        }
        if (p.second - 1 != go(t)) {
            DBN(p.first, p.second, t);
        }
        //        if (p.second == 1 && count(p.first.begin(), p.first.end(), 0) <= n2)
        //        DBN(p.first, p.second - 1);
    }
