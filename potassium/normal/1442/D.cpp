...
int main() {
	...
	vector<vector<ll>> ps = a;
	for (auto& i : ps)
		for (int j = 1; j < (int)i.size(); j++)
			i[j] += i[j - 1];
		
	ll ans = 0;
	function<void(int, int, const vector<ll>&)> solve = [&](int l, int r, const vector<ll>& now) {
		if (l == r) {
			chkmax(ans, now[k]);
			for (int i = 0; i < min((int)a[l].size(), k); i++)
				chkmax(ans, ps[l][i] + now[k - i - 1]);
		...
		auto add = [&](int idx) {
			...
			for (int i = k; i >= sz; i--)
				chkmax(val[i], val[i - sz] + ps[idx][sz - 1]);
		};
