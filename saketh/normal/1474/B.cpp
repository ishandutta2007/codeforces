...
int main() {
    ...
    auto solve = [&](int x) {
        int p1 = *upper_bound(all(sv.primes), x);
        int p2 = *lower_bound(all(sv.primes), p1 + x);
    ...
    for (int v = 2; v < 100000; v++) {
        vi divs;
        sv.for_each_divisor_unordered(v, [&](int d){ divs.pb(d); });
