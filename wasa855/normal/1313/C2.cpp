...
void pd(int u,int l,int r)
{
	...
	if(t[u].tag)
	{
		t[ls].tag=t[ls].minn=t[u].tag; t[ls].sum=t[u].tag*(mid-l+1);
		t[rs].tag=t[rs].minn=t[u].tag; t[rs].sum=t[u].tag*(r-mid);
		t[u].tag=0;
	}
}
...
int getpl(int u,int l,int r,int d)
{
	...
	int mid=(l+r)/2;
	if(t[ls].minn<d) return getpl(ls,l,mid,d);
	else return getpl(rs,mid+1,r,d);
}
