...
signed main()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		{
			int f=0;
			// if(dp[j][0][0]!=a[j])dp[j][0][0]=a[j],f=1;
			// if(dp[j][1][1]!=a[j])dp[j][1][1]=a[j],f=1;
			if(a[j]>dp[j-1][0][0]&&dp[j][0][1]<dp[j-1][0][1])dp[j][0][1]=dp[j-1][0][1],f=1,dp[j][0][0]=a[j];
			if(a[j]>dp[j-1][1][0]&&dp[j][0][1]<dp[j-1][1][1])dp[j][0][1]=dp[j-1][1][1],f=1,dp[j][0][0]=a[j];
			if(a[j]<dp[j-1][0][1]&&dp[j][1][0]>dp[j-1][0][0])dp[j][1][0]=dp[j-1][0][0],f=1,dp[j][1][1]=a[j];
			if(a[j]<dp[j-1][1][1]&&dp[j][1][0]>dp[j-1][1][0])dp[j][1][0]=dp[j-1][1][0],f=1,dp[j][1][1]=a[j];
			// if(dp[j][0][0]>dp[j][0][1])dp[j][0][0]=inf,dp[j][0][1]=-inf;
			// if(dp[j][1][0]>dp[j][1][1])dp[j][1][0]=inf,dp[j][1][1]=-inf;
			// odb(i),odb(j),odb(dp[j][0][0]),odb(dp[j][0][1]);
			// odp(dp[j][1][0],dp[j][1][1]);
			if(!f)break;
		}
		for(;j<=n;j++)
		{
			if(dp[j][0][0]==a[j]||dp[j][1][1]==a[j]);
			else break;
		}
		// odl(j);
		ans+=j-i;
