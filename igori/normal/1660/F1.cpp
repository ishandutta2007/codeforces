...
void solve()
{
    ...
    for (int L = 0; L < n; L++)
    {
        ...
        int cnt_replacements = 0;
        int calc = 0;
        for (int i = L; i < n; i++)
        {
            if (s[i] == '+') cnt_plus++, calc = 0;
            if (s[i] == '-') cnt_minus++;
            if (s[i] == '-')
            {
                calc++;
                if (calc == 2) cnt_replacements++, calc = 0;
            }
            int balance = cnt_plus - cnt_minus;
            //cout << L << " " << i << " " << balance << " " << cnt_replacements << "\n";
            if (balance <= 0 && abs(balance) % 3 == 0)
                ans++;
        }
    }
    cout << ans << '\n';
