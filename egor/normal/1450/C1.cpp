...
class C1KrestikiNolikiErrichtoProstayaVersiya {
public:
    void solve() {
        int n = in.readInt();
        auto t = in.readCharTable(n, n);
        ...
        for (int i : range(3)) {
            for (int j : range(3)) {
                if (i != j) {
                    minim(total, xs[i] + os[j], xp, i, op, j);
                }
            }
        }
        for (int i : range(n)) {
...
int main() {
    ...
    auto time = clock();
#endif
    C1KrestikiNolikiErrichtoProstayaVersiya solver;
