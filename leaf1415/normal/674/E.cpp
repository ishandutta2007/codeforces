...
int main(void)
{
	...
	rep(q, 1, Q){
		...
		reverse(all(vec));
		rep(i, 0, sz(vec)-3){
			ll v = vec[i], u = vec[i+1];
			rep(j, 1, B) dp[v][j] /= (0.5+0.5*dp[u][j-1]);
		}
		
		reverse(all(vec));
		rep(i, 0, sz(vec)-2){
			ll v = vec[i], p = vec[i+1];
			rep(j, 1, B) dp[p][j] *= (0.5+0.5*dp[v][j-1]);
		}
	}
