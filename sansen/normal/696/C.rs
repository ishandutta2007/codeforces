...
fn run() {
    ...
    mat.set_at(0, 0, ModInt::new(1));
    mat.set_at(0, 1, ModInt::new(2));
    mat.set_at(1, 0, ModInt::new(1));
    let mut de = ModInt::new(2);
    for a in a {
        mat = mat.matpow(a);
        de = de.pow(a);
    }
    let de = de * ModInt::new(2).inv();
    let nu = mat.get_at(1, 1) * ModInt::new(2).inv();
    println!("{}/{}", nu, de);
}
