...
int main() {
    ...
    vi order(N); F0R (i, N) order[i] = i;
    sort_by(order, vals[a] != vals[b] ? vals[a] > vals[b] : a < b);

    struct query { int k, pos, i; };

    int M; re(M);
    vector<query> qs(M);

    F0R (i, M) { re(qs[i].k, qs[i].pos); qs[i].i = i; }
    sort_by(qs, a.k < b.k);
    ...
    trav (q, qs) {
        ...
        ans[q.i] = vals[ct.lower_bound([&](int s){ return s >= q.pos; }) - 1];
    }
