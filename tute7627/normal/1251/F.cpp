...
int main(){
  ...
  rep(i,0,k){
    vector<ll>cnt(2);
    for(auto z:mp){
      //cout<<min(2LL,z.se) spa z.fi spa b[i]<<endl;
      if(z.fi<b[i])cnt[min(2LL,z.se)-1]++;
    }
    FPS f0={1,2};
    FPS f1={1,2,1};
    FPS f=f0.pow(cnt[0],cnt[0]+1)*f1.pow(cnt[1],2*cnt[1]+1);
    //cout<<i<<endl;
    //debug(cnt,2);
    //debug(f,f.size());
    rep(j,0,q){
      ll rem=(c[j]-b[i]*2-2)/2;
      if(rem<0)continue;
      if(rem<f.size())ret[j]+=f[rem];
    }
  }
