...
vector<int> buc[N];
int nxt(int v, int p) {return p == buc[v].size() - 1 ? n + 1 : buc[v][p + 1];}
bool Med;
int main() {
  fprintf(stderr, "%.4lf\n", (&Mbe - &Med) / 1048576.0);
  #ifdef ALEX_WEI
  ...
  for(int i = 1; i <= n; i++) {
    ...
    for(int p = 0; p <= buc[i].size(); p++) {
      ...
      for(int q = 1; q <= skip; q++) {
        ...
        upd(acc + q, buc[id][pos + q - 1], min(cur, nxt(id, pos + q - 1)) - 1);
      }
      for(int q = skip + 1; q <= num; q++) upd(acc + q, buc[id][pos + q - 1], min(cur, nxt(id, pos + q - 1)) - 1);
      if(p < buc[i].size()) upd(acc + num - 1, cur, min(nxt(id, pos + num - 1), nxt(i, p)) - 1);
    }
