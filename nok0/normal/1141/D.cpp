...
void main_() {
	...
	for(auto &[key, vec] : mp) {
		while(SZ(vec) and SZ(h2)) {
			res.eb(vec.back(), h2.back());
			vec.pop_back();
			h2.pop_back();
		}
	}
	while(SZ(h) and SZ(h2)) {
		res.eb(h.back(), h2.back());
		h.pop_back();
		h2.pop_back();
	}
	print(SZ(res));
	for(auto [u, v] : res) print(u + 1, v + 1);
}
