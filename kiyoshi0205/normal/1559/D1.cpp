...
int main(){
  ...
  ll i,j,M1,M2;
  cin>>N>>M1>>M2;
  mat g1(N,vec(N,0)),g2(N,vec(N,0));
  mat tree1(N),tree2(N);
  set<datas> edge;
  rep(i,N)rep(j,i)edge.emplace(j,i);
  rep(i,N){
    g1[i][i]=g2[i][i]=1;
    tree1[i].emplace_back(i);
    tree2[i].emplace_back(i);
  }
  while(M1--){
    cin>>A>>B;
    --A;--B;
    vec v1=tree1[A],v2=tree1[B];
    for(auto x:v1){
      for(auto y:v2){
        if(edge.count(minmax(x,y)))edge.erase(minmax(x,y));
        g1[x][y]=g1[y][x]=1;
        tree1[x].emplace_back(y);
        tree1[y].emplace_back(x);
      }
    }
  }
  while(M2--){
    cin>>A>>B;
    --A;--B;
    vec v1=tree2[A],v2=tree2[B];
    for(auto x:v1){
      for(auto y:v2){
        g2[x][y]=g2[y][x]=1;
        if(edge.count(minmax(x,y)))edge.erase(minmax(x,y));
        tree2[x].emplace_back(y);
        tree2[y].emplace_back(x);
      }
    }
  }
  pvec ans;
