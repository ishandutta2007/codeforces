...
*/
    ...
    fun solve(tc: Int) {
        ...
        val pq = PriorityQueue<el>()
        for (i in 1..n) {
            cnt[i] = (n / i - 1).toLong()
            pq.add(el(cnt[i], i.toLong()))
        }
        var tot = 0
        var cur: Long = 0
        while (tot < n - 1) {
            val x = pq.poll()
            if (x.`val` != cnt[x.ind.toInt()] || used[x.ind.toInt()]) continue
            cur += x.`val`
            res[++tot] = cur
            used[x.ind.toInt()] = true
            var i = 2 * x.ind.toInt()
            while (i <= n) {
                --cnt[i]
                pq.add(el(cnt[i], i.toLong()))
        ...
        for (i in n - 1 downTo 0) out!!.print(res[i].toString() + " ")
        out!!.println()
    }
    ...
    fun atos(arr: LongArray?): String {
     ...
     class el(var `val`: Long, var ind: Long) : Comparable<el> {
        override operator fun compareTo(other: el): Int {
            if (`val` > other.`val`) return -1
            return if (`val` < other.`val`) 1 else 0
        }
