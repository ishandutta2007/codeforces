#line 1 "codeforces-workspace\\contest\\1555\\d\\d.cpp"
...
#line 16 "codeforces-workspace\\contest\\1555\\d\\d.cpp"
//*/
...
void main() {
  ...
  vector<i32> ans(queries.size(), s.size());
  for (string p = "abc";;) {
    auto t = 0;
    vector<i32> c{0};
    for (auto &&e : s) {
      c.emplace_back(c.back() + (p[t] != e));
      t = (t + 1) % 3;
    }
    for (auto &&[a, lr] : zip(ans, queries)) {
      auto [l, r] = lr;
      chle(a, c[r] - c[l]);
    }
    if (not next_permutation(begin(p), end(p))) break;
  }
