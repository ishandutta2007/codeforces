...
int main() {
    ...
    rep(i,N){
        rep(j, L) {
            if (!((sel[i] >> j) & 1)) {
                if (j == L - 1) { cout << "Impossible" << endl; return 0; }
                continue;
            }
            ...
            vll sum(1ull << L);
            rep(k, 1ull << L)sum[k] = m[k];
            rep(k, L)rep(l, 1ull << L) {
                if ((l >> k) & 1)sum[l] += sum[l ^ (1ull << k)];
            }
            //for (ll x : sum) { cout << x << " "; }
            //cout << endl;
            vll sumn(1ull << L);
            ...
            bool b = 1;
            rep(k, 1ull << L)if (sum[k] > sumn[k]) { b = 0; break; }

            
            if (b) { ans.push_back(j + 'a'); break; }
