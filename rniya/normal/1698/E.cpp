...
void solve() {
    ...
    vector<int> inv_a(n), inv_b(n, -1);
    for (int i = 0; i < n; i++) {
        inv_a[a[i]] = i;
        if (b[i] >= 0) inv_b[b[i]] = i;
    }

    mint ans = 1;
    int hatena = 0;
    for (int i = 0; i < s; i++) hatena += (b[inv_a[i]] == -1);
    for (int i = 0; i < n; i++) {
        if (i + s < n) hatena += (b[inv_a[i + s]] == -1);
        if (inv_b[i] != -1) {
            int cur = a[inv_b[i]];
            if (i + s < cur) {
                cout << 0 << '\n';
            ...
            swap(a[inv_a[i]], a[inv_b[i]]);
            swap(inv_a[i], inv_a[cur]);
            continue;
        } else {
            ans *= hatena--;
        }
