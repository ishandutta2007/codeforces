...
#define MAXN 1000000
multiset<pii>s1,s2;
vector<pii>in[MAXN+10],out[MAXN+10];
LL num,n,k,m;
LL sum,ans;
int main()
{
...
//        printf("%d\n",i);
        ...
        while(s1.size()&&num-prev(s1.end())->second>=k){
            num-=prev(s1.end())->second;
            sum-=1ll*prev(s1.end())->second*prev(s1.end())->first;
            s2.insert(*prev(s1.end()));
            s1.erase(prev(s1.end()));
        }
        if(num>k)
            ans+=sum-1ll*prev(s1.end())->first*(num-k);
        else
            ans+=sum;
        for(auto &x:out[i]){
            if(s1.find(x)!=s1.end()){
