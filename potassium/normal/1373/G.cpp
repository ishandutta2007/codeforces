...
struct Node {
	...
	friend ostream& operator << (ostream& out, Node node) {
		return out << node.curR << " " << node.rem << " " << node.cnt;
	}
};

vector<Node> Seg = vector<Node>(SZ);

bool ok = 0;
Node merge(int id, int l, int r) {
	Node a = Seg[id * 2];
	Node b = Seg[id * 2 + 1];
	Node& ans = Seg[id];
	...
	ans.cnt = a.cnt + a.rem + b.cnt;
	int RLemp = (b.curR == -1 ? 0 : b.curR - mid - b.cnt);
	if (a.rem > RLemp) {
		if (b.curR == -1)
			b.curR = mid;
		b.curR += a.rem - RLemp;
	...
	ans.curR = max(a.curR, b.curR);
	ans.rem = b.rem;
	
//	if (ok)
//		W(Seg[id * 2], "|", Seg[id * 2 + 1], "=", ans);
...
void upd(int id, int l, int r, int p, int v) {
	if (l == r) {
		Seg[id].cnt += v;
		Seg[id].cnt += Seg[id].rem;
		Seg[id].rem = max(Seg[id].cnt - 1, 0);
		Seg[id].cnt -= Seg[id].rem;
		
		Seg[id].curR = (Seg[id].cnt ? l : -1);
		return;
	}
