...
void orzck(){
    cin >> n >> str1 >> str2;
    rep(26) vec[i].clear();
    rep(n) vec[str1[i]-'a'].pb(i);
    rep(26) vec[i].pb(inf);
    rep(26) reverse(all(vec[i]));
    res=inf;
    int cur=0;
    rep(n+2) bit.val[i]=0;
    rep(n){
        int x=str2[i]-'a';
        int t=inf;
        rep1(j,x) chkmin(t,vec[j].back());
        if(t<inf) t+=bit.query(t+1,n-1);
        chkmin(res,cur+(t-i));
        if(vec[str2[i]-'a'].back()==inf) break;
        int pos=vec[str2[i]-'a'].back();
        cur+=pos-i+bit.query(pos+1,n-1);
        bit.modify(pos,1);
        vec[str2[i]-'a'].pop_back();
    }
    print(res>n*n?-1:res);
}
