...
bool done[10001];
bool carless[10001];
int has_driver[10001];
bool can_give[10001][64];
bool good[10001][64];
void bfs() {
    fill(done + 1, done + n + 1, 0);
    fill(dist + 1, dist + n + 1, 0);
    for (int i = 1; i <= n; i++) fill(can_give[i], can_give[i] + 64, false);
    queue<int> q;
    q.push(1);
    done[1] = 1;
    dist[1] = 0;
    can_give[1][mask[1]] = true;
    while (!q.empty()) {
        int u = q.front();
        q.pop();
        can_give[u][0] = 1;
        for (int ms = 0; ms < 64; ms++) {
            if (can_give[u][ms])
                for (int sub = ms; sub > 0; sub = (sub - 1) & ms) can_give[u][sub] = 1;
        }
        ...
        for (auto&& v : g[u]) {
            ...
            if (dist[v] == dist[u] + 1)
                for (int ms = 0; ms < 64; ms++)
                    if (can_give[u][ms]) can_give[v][ms | mask[v]] = 1;
        }
    }
}
...
void solve() {
    ...
    cin >> f;
    h | ARRAY | 1 | f | ARRAY_INPUT;
    cin >> k;
    p | ARRAY | 0 | k - 1 | ARRAY_INPUT;
    fill(has_driver + 1, has_driver + n + 1, 0);
    for (int i = 1; i <= f; i++) has_driver[h[i]]++;
    fill(mask + 1, mask + n + 1, 0);
    fill(carless, carless + f + 1, 0);
    for (int i = 0; i < k; i++) {
        carless[p[i]] = 1;
        p[i] = h[p[i]];
        has_driver[p[i]]--;
    ...
    good[0][0] = 1;
    for (int i = 1; i <= f; i++) {
        if (!has_driver[h[i]]) fill(can_give[h[i]], can_give[h[i]] + 64, 0);
        can_give[h[i]][0] = 1;
    }
    for (int i = 1; i <= f; i++) {
        fill(good[i], good[i] + 64, 0);
        for (int ms = 0; ms < 64; ms++) {  // target mask
            if (good[i - 1][ms]) {
                good[i][ms] = 1;
            } else {
                if (carless[i]) continue;
                for (int sub = ms; sub > 0; sub = (sub - 1) & ms) {
                    if (can_give[h[i]][sub]) {
                        if (good[i - 1][ms ^ sub]) {
                            good[i][ms] = 1;
                            break;
    ...
    int ans = k;
    for (int i = 0; i < 64; i++)
        if (good[f][i]) ans = min(ans, k - __builtin_popcount(i));
    cout << ans << '\n';
}
