...
int main() {
    ...
    const int MAXB = 25;
    static_assert((1 << MAXB) > 2 * MAXV);

    int N; re(N); vi a(N); re(a);

    binary_indexed_tree<int> ct0(MAXV);
    binary_indexed_tree<int> ct1(MAXV);

    int ans = 0;
    F0R (b, MAXB) {
        int s0 = 0;
        fill(all(ct0.table), 0);

        int s1 = 0;
        fill(all(ct1.table), 0);
        ...
        F0R (i, N) {
            ...
            { // ct0
                int no_carry = ct0.sum(0, min((1 << b) - tail, MAXV));
                int carry = s0 - no_carry;
                if (has) parity ^= no_carry&1;
                else parity ^= carry&1;
            }
            { // ct1
                int no_carry = ct1.sum(0, min((1 << b) - tail, MAXV));
                int carry = s1 - no_carry;
                if (!has) parity ^= no_carry&1;
                else parity ^= carry&1;
            }

            (has ? ct1 : ct0).add(tail, 1);
            (has ? s1 : s0)++;
        }
