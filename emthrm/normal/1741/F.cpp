...
void solve() {
  int n; cin >> n;
  vector<int> l(n), r(n), c(n); REP(i, n) cin >> l[i] >> r[i] >> c[i];
  vector<int> xs;
  xs.reserve(n * 4);
  REP(i, n) {
  ...
  const int x = xs.size();
  REP(i, n) l[i] = lower_bound(ALL(xs), l[i]) - xs.begin();
  REP(i, n) r[i] = lower_bound(ALL(xs), r[i]) - xs.begin();
  LazySegmentTree<monoid::RangeMaximumAndAddQuery<int, INF>> segtree(vector<int>(x, 0));
  REP(i, n) segtree.apply(l[i], r[i] + 1, 1);
  map<int, vector<int>> segs;
  REP(i, n) segs[c[i]].emplace_back(i);
  vector<int> ans(n, 0);
  for (const auto& [_, idxes] : segs) {
    for (const int i : idxes) segtree.apply(l[i], r[i] + 1, -1);
    // REP(i, x) cout << segtree[i] << " \n"[i + 1 == x];
    for (const int i : idxes) {
      if (segtree.get(l[i], r[i] + 1) > 0) continue;
      ans[i] = INF;
      if (r[i] + 1 < x) {
        const int right = segtree.find_right(r[i] + 1, [](const int x) -> bool { return x <= 0; });
        if (right < x) chmin(ans[i], xs[right] - xs[r[i]]);
      }
      if (l[i] > 0) {
        const int left = segtree.find_left(l[i], [](const int x) -> bool { return x <= 0; });
        if (left >= 0) chmin(ans[i], xs[l[i]] - xs[left]);
      }
    }
    for (const int i : idxes) segtree.apply(l[i], r[i] + 1, 1);
  }
