...
struct PAM{
	...
	#define To(pos,c) t[pos].to[c]
	#define Fa(pos) t[pos].fa
	#define Len(pos) t[pos].len
	#define NextArith(pos) t[pos].nextArith
	#define ArithDiff(pos) t[pos].arithDiff
	#define ArithSum(pos) t[pos].arithSum
	Node t[MAXN];
	int nowNode = 0;
	int lastNode = 1;
	...
	void AppendChar( int i ){
		...
		ArithDiff(cur) = Len(cur) - Len(Fa(cur));
		if( ArithDiff(cur) == ArithDiff(Fa(cur)) )
			NextArith(cur) = NextArith(Fa(cur));
		else
			NextArith(cur) = Fa(cur);
	}
	...
	ll Solve(){
		...
		For(i,n){
			AppendChar(i);
			static vc<int> tmp; tmp.clear();
			for( int p = lastNode ; Len(p) > 0 ; p = NextArith(p) )
				tmp.Eb(p);
			for_each(AllRev(tmp),[&]( int p ){
				ArithSum(p) = dp[i-Len(NextArith(p))-ArithDiff(p)];
				if( ArithDiff(Fa(p)) == ArithDiff(p) ){
					// O(p);
					Addi( ArithSum(p) , ArithSum(Fa(p)) );
				}
				if(i&1^1) Addi(dp[i],ArithSum(p));
			});
		}
		return dp[n];
	}
