...
void dfs(int i, int p, int sum, int above) {
    debug(i+1, p+1, sum, above);
    if (p==-1) {
        int children = adj[i].size();
        ans[i] = children;
        for (int nb : adj[i]) dfs(nb, i, -1, 1);
    } else {
        int children = adj[i].size()-1;
        ans[i] = sum - above*children;
        for (int nb : adj[i]) if (nb != p) dfs(nb, i, above, sum);
    }
}
