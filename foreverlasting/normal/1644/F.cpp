...
const int N=2e5+10;
namespace MAIN{
    int mu[N],fac[N],inv[N],su[N];
    inline int g(const res &n,res m){
        m=min(m,n);
        res ret=0;
        for(res i=0;i<=m;i++)add(ret,mul(mul(su[m-i],qpow(i,n)),inv[i]));
        return ret;
    }
    int n,k;
    inline void MAIN(){
        ...
        mu[1]=fac[0]=inv[0]=fac[1]=inv[1]=su[0]=1;
        for(res i=1;i<=n;i++)for(res j=i+i;j<=n;j+=i)mu[j]-=mu[i];
        for(res i=2;i<=n;i++)fac[i]=mul(fac[i-1],i),inv[i]=mul(inv[kcz%i],kcz-kcz/i);
        for(res i=2;i<=n;i++)inv[i]=mul(inv[i-1],inv[i]);
        for(res i=1;i<=n;i++)su[i]=Add(su[i-1],i&1?kcz-inv[i]:inv[i]);
        res ans=0;
        for(res i=1;i<=n;i++)if(mu[i])add(ans,mu[i]==1?g((n+i-1)/i,k)-1:kcz-g((n+i-1)/i,k)+1);
        printf("%d\n",ans);
    }
}
int main(){
