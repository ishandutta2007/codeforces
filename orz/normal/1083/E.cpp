...
void find_h(const int& v, const vi& up, vi& h)
{
	if (h[v] == -1)
	{
		if (v == 0)
...
int lca(int x, int y, const vvi& up, const vi& h)
{
	for (int i = C - 1; i >= 0; --i)
		if (h[up[i][x]] >= h[y])
			x = up[i][x];
...
ll evaluate(const pll& p, const ll& u)
{
	return p.X * u + p.Y;
}

pair<pdd, double> lnn(const pll& u)
{
	return pair<pdd, double>(pdd(u.X, -1), u.Y);
}

void tupo_ins(vector<pll>& p, vector<long double>& d, const pll& u)
{
	auto uu = lnn(u); auto vv = lnn(p.back());
	auto zz = ints(uu.X, uu.Y, vv.X, vv.Y);
	d.pb(zz.X);
	p.pb(u);
}

bool unreq(const pll& u, const pll& v, const pll& w)
{
	auto lu = lnn(u);
	auto lv = lnn(v);
	auto lw = lnn(w);
	//auto uv = ints(lu.X, lu.Y, lv.X, lv.Y);
	auto uw = ints(lu.X, lu.Y, lw.X, lw.Y);
	//auto vw = ints(lw.X, lw.Y, lv.X, lv.Y);
	return uw.X * v.X + v.Y <= uw.Y;
}

void q_add(vector<pll>& p, vector<long double>& d, const pll& u)
{
	while ((sz(p) >= 2) && (unreq(p[sz(p) - 2], p[sz(p) - 1], u)))
	{
		p.pop_back();
		d.pop_back();
	}
...
ll evaluate(const vector<pll>& p, const vector<long double>& d, const ll& u)
{
	int n = sz(p);
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		ll ai = a[i];
		ll ans1 = evaluate(q, d, xi) + ((ll)xi) * yi - ai;
		ans = max(ans, ans1);
		q_add(q, d, pll(-yi, ans1));
	}
	cout << ans << '\n';
	return 0;
}