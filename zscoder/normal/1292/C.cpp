...
int main()
{
	...
	for(int i=0;i<n;i++)
	{
		...
		{
			lc[i][j]=T.lca(i,j);
			int d = T.h[i]+T.h[j]-2*T.h[lc[i][j]];
			V[d].pb({i,j});
		}
	}	
	par[0]=-1;
	...
	for(int i=1;i<=n;i++)
	{
		for(ii x:V[i])
		{
			int u=x.fi; int v=x.se; int l=lc[u][v];
			if(l==u)
			{
				int tp = T.getpar(v,T.h[v]-T.h[u]-1);
				dp[u][v]=max(dp[u][par[v]],dp[tp][v])+T.subsize[v]*(n-T.subsize[tp]);
			}
			else if(l==v)
			{
				swap(u,v);
			...
			{
				dp[u][v]=max(dp[u][par[v]],dp[par[u]][v])+T.subsize[u]*T.subsize[v];
			}
			dp[v][u]=dp[u][v];
			mx=max(mx,dp[u][v]);
		}
