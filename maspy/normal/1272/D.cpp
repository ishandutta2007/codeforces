...
signed main() {
  ...
  FOR(i, N) {
    if (i >= 1 && A[i - 1] < A[i]) DPL[i] = DPL[i - 1] + 1;
  }
  FOR_R(i, N) {
    if (i + 1 < N && A[i] < A[i + 1]) DPR[i] = DPR[i + 1] + 1;
  }
  ll ANS = 0;
  chmax(ANS, *max_element(all(DPL)));
  chmax(ANS, *max_element(all(DPR)));

  FOR3(i, 1, N - 1) {
    if (A[i - 1] < A[i + 1]) chmax(ANS, DPL[i - 1] + DPR[i + 1]);
  }
  print(ANS);
