...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 2 "/home/maspy/compro/library/ds/range_xor_basis_query.hpp"
...
void solve() {
  ...
  LL(Q);
  vc<int> ANS(Q);
  Range_Xor_Basis_Query<int, 20> RX(A);
  FOR_(Q) {
    LL(l, r);
    RX.add(--l, r);
  }

  auto calc = [&](int q, auto& basis) -> void {
    int& x = ANS[q];
    for (auto&& v: basis) chmax(x, x ^ v);
  };
  RX.calc(calc);
