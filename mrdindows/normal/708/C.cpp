...
void dfs(int cur, int par = -1) {
    used[cur] = true;
    pii maxs = {n - siz[cur], par};
    for (int to : v[cur]) {
        if (!used[to]) {
            upd(tin[cur], n - siz[to]);
            maxs = max(maxs, {siz[to], to});
            dfs(to, cur);
        }
    }
    if (maxs.first <= thresh) {
        ans[cur] = true;
    } else {
        upd(tin[cur], 0);
        int to = maxs.second;
        int f;
        if (to != par) {
            f = getMax(1, 0, N, tin[to], tout[to]);
        } else {
            f = max(getMax(1, 0, N, 0, tin[cur] - 1), getMax(1, 0, N, tout[cur] + 1, N));
        }
        ans[cur] = f >= (maxs.first - thresh);
    }
    upd(tin[cur], siz[cur]);
}
