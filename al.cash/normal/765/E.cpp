...
void dfsd(int v, int p) {
  ...
  for (int u : e[v]) if (u != p) {
    ...
    if (xd[u] == -1) xd[v] = -1;
      ...
      if (xd[v] == 0) xd[v] = xd[u] + 1;
      else if (xd[v] != xd[u] + 1) xd[v] = -1;
    }
  }
}
...
void dfsu(int v, int p) {
  vi d;
  for (int u : e[v]) if (u != p) d.pb(xd[u] + 1);
  auto g = group(all(d));
  bool okg = count(all(d), 0) < 2 && sz(g) <= 2;
  ...
  if (v == 0) {
    for (int u : e[v]) if (u != p) {
      if (!okg) xu[u] = -1;
        ...
        if (sz(g) == 1) {
          xu[u] = sz(d) == 1 ? 1 : g[0].X + 1;
        } else {
          int i = xd[u]+1 == g[0].X ? 0 : 1;
          if (g[i].Y == 1 && g[i^1].X != 0) xu[u] = g[i^1].X + 1;
          else xu[u] = -1;
        }
      }
    }
    ...
    for (int u : e[v]) if (u != p) {
      if (xu[v] == -1 || !okg) xu[u] = -1;
      else {
        xu[u] = xu[v] + 1;
        if (sz(g) == 1) {
          if (sz(d) > 1 && g[0].X != xu[v]) xu[u] = -1;
        } else {
          int i = xd[u]+1 == g[0].X ? 0 : 1;
          if (g[i].Y > 1 || g[i^1].X == 0 || g[i^1].X != xu[v]) xu[u] = -1;
        }
      }
    }
  }

  if (xu[v] != -1 && count(all(d), 0) == 0) {
//    writeln(v, ": ", xu[v], ", ", all(d));
    if (v != 0) d.pb(xu[v]);
    UNIQUE(d);
    if (sz(d) == 1) {
      smin(answer, ans[d[0]]);
    } else if (sz(d) == 2) {
      smin(answer, ans[d[0] + d[1]]);
    }
  }

  for (int u : e[v]) if (u != p) dfsu(u, v);
}
