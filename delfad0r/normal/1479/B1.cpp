...
/*for(int n = 0; n < N; ++n) {
	cin >> A[n];
	if(n >= 2 and A[n] == A[n - 1] and A[n] == A[n - 2]) {
		continue;
	}
	if(n >= 3 and A[n] == A[n - 1] and A[n] == A[n - 3]) {
		continue;
	}
}
while(N--) {
	int a;
	cin >> a;
	if(X.empty() or X.back() != a) {
		X.push_back(a);
	} else if(Y.empty() or Y.back() != a) {
		Y.push_back(a);
	}
}
[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(X.size() + Y.size()), "\n");  }();*/
