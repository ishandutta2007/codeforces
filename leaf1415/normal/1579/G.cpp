...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, n) rep(j, 0, 4000) dp[i][j] = inf;
		dp[0][0] = 0;
		
		rep(i, 0, n-1){
			rep(j, 0, 4000){
				chmin(dp[i+1][max(0, j-a[i+1])], dp[i][j] + max(0, a[i+1]-j));
				if(j+a[i+1] <= 4000) chmin(dp[i+1][j+a[i+1]], max(j+a[i+1], dp[i][j]));
			}
		}
		ll ans = inf;
		rep(j, 0, 4000) chmin(ans, dp[n][j]);
		outl(ans);
	}
