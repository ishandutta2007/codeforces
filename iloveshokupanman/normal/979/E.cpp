...
int main() {
    ...
    vector<vvvmint> dp(N+1,vvvmint(N + 1, vvmint(N + 1, vmint(N + 1, 0))));
    dp[0][0][0][0] = 1;
    mint ans = 0;
    ...
    rep(i, N + 1)rep(j, N + 1)rep(k, N + 1)rep(l, N + 1) {
        ...
        if (i + j + k + l == N) {
            if ((j + l) % 2 == P) { ans += dp[i][j][k][l]; }
            continue;
        }

        ll x = i + j + k + l;
        if (c[x] != 1) {
            if (l == 0)dp[i][j + 1][k][l] += dp[i][j][k][l] * ((mint)2).pow(x);
            else{
                dp[i + 1][j][k][l] += dp[i][j][k][l] * ((mint)2).pow(x - 1);
                dp[i][j + 1][k][l] += dp[i][j][k][l] * ((mint)2).pow(x- 1);
            }
        }
        if (c[x] != 0) {
            if (j == 0)dp[i][j][k][l+1] += dp[i][j][k][l] * ((mint)2).pow(x);
            else {
                dp[i][j][k+1][l] += dp[i][j][k][l] * ((mint)2).pow(x- 1);
                dp[i][j][k][l+1] += dp[i][j][k][l] * ((mint)2).pow(x - 1);
            }
        }
    }
