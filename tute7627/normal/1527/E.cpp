...
int main(){
  ...
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  auto segtmp=add_min::make(1);
  using Seg=decltype(segtmp);
  vector<Seg>dp;
  rep(i,0,k){
    dp.push_back(add_min::make(n+1));
    rep(j,0,n+1)dp[i].set(j,INF/2);
    if(i==0)dp[i].set(0,0);
    dp[i].build();
  }
  vector<ll>pre(n+1,-1);
  rep(i,0,n){
    if(pre[a[i]]!=-1){
      rep(j,0,k){
        dp[j].update(0,pre[a[i]]+1,i-pre[a[i]]);
      }
    }
    pre[a[i]]=i;
    rrep(j,1,k){
      ll mi=dp[j-1].query(0,n+1);
      dp[j].update(i+1,mi);
    }
  }
  cout<<dp[k-1].query(0,n+1)<<endl;
  return 0;
}