...
void main_() {
	auto dist = [&](ll sx, ll sy, ll tx, ll ty) {
	...
	map<int, V<>> mp;
	REP(i, n + 1) {
		mp[max(xy[i].first, xy[i].second)].pb(i);
	}
	V<> idx;
	ll base = 0;
	for(auto &[key, vec] : mp) {
		...
		auto [x, y] = xy[vec.front()];
		auto [z, w] = xy[vec.back()];
		base += dist(x, y, z, w);
	}
	int m = SZ(mp);
	vector dp(m, vector(2, INF));
	dp[0][0] = dp[0][1] = 0;

	REP(i, m - 1) {
		auto [sx0, sy0] = xy[mp[idx[i]].front()];
		auto [sx1, sy1] = xy[mp[idx[i]].back()];
		auto [tx0, ty0] = xy[mp[idx[i + 1]].front()];
		auto [tx1, ty1] = xy[mp[idx[i + 1]].back()];
		chmin(dp[i + 1][1], dp[i][0] + dist(sx0, sy0, tx0, ty0));
		chmin(dp[i + 1][1], dp[i][1] + dist(sx1, sy1, tx0, ty0));
		chmin(dp[i + 1][0], dp[i][0] + dist(sx0, sy0, tx1, ty1));
		chmin(dp[i + 1][0], dp[i][1] + dist(sx1, sy1, tx1, ty1));
	}
	debug(dp);

	print(*min_element(all(dp.back())) + base);
}
