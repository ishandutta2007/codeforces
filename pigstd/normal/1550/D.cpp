...
signed main()
{
	...
	WT
	{
		...
		for (int i=2-l;;i++)
		{
			...
			int f1=n/2,f2=n-n/2,f3=f2,f4=f1;
			f2-=l+i-1,f1-=max(0ll,n-r+i);
			f4-=l+i-1,f3-=max(0ll,n-r+i);
			if ((f1<0||f2<0)&&(f3<0||f4<0))break;
			ans+=C(f1+f2,f2)+C(f3+f4,f3);ans%=Mod;
		}
		for (int i=r-n+1;;i++)
		{
			if (t[i])continue;t[i]=1;
			...
			f3-=n-r+i,f4-=max(0ll,l+i-1);
			if ((f1<0||f2<0)&&(f3<0||f4<0))break;
			ans=ans+C(f1+f2,f1)+C(f3+f4,f3);ans%=Mod;
		}
