...
pair<vi, int> dijkstra(Graph &g, int s, int t) {
    const auto INF = inf<int>;
    ...
    if(pre[t] == -1) return {{}, -1};
    vector<int> res{t};
    while(res.back() != s) { res.emplace_back(pre[res.back()]); }
    reverse(all(res));
    return {res, d[t]};
}
struct UnionFind {
...
int main() {
    ...
    Wgraph<int> G(n);
    rep(i, q) { add(G, a[i], b[i], i); }
    vvc<int> ans(q);

    rep(i, n) {
        vi v{(int)i + 1};
        while(!empty(G[i]) and !empty(ans[G[i].back().cost])) G[i].pop_back();
        if(empty(G[i])) continue;
        REC([&](auto &&f, int x, vi &v) -> void {
            int to = G[x].back();
            int id = G[x].back().cost;
            G[x].pop_back();
            while(!empty(G[to]) and (!empty(ans[G[to].back().cost]) or G[to].back().cost == id)) G[to].pop_back();

            if(empty(G[to])) {
                REV(v);
                for(auto e : v) ans[id].eb(e);
            } else {
                auto w = dijkstra(g, x, to).fi;
                for(auto e : w) ans[id].eb(e + 1);
                w.erase(begin(w));
                for(auto e : w) {
                    if(si(v) > 1 and v[si(v) - 2] == e + 1)
                        v.pop_back();
                    else
                        v.eb(e + 1);
                }
