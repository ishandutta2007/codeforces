...
int naive(V<int> a) {
    ...
    map<K, bool> dp;
    auto dfs = [&](auto self, int x, int y, int turn) -> bool {
        K k = K{x, y, turn};
        ...
        if (turn == 0) {
            for (int nx : {x - 1, x + 1}) {
                ...
                if (nx == y) continue;
                if (a[nx] > a[x]) continue;
                if (!self(self, nx, y, 1)) f = true;
            }
        } else {
            for (int ny : {y - 1, y + 1}) {
                if (!(0 <= ny && ny < n)) continue;
                if (ny == x) continue;
                if (a[ny] < a[y]) continue;
                if (!self(self, x, ny, 0)) f = true;
            }
        }
        return dp[k] = f;
    };
...
int solve(V<int> a) {
    ...
    for (int i = 0; i < n; i++) {
        l[i] = 1;
        if (i && a[i - 1] < a[i]) l[i] = l[i - 1] + 1;
    }

    for (int i = (int)(n)-1; i >= 0; i--) {
        r[i] = 1;
        if (i + 1 < n && a[i] > a[i + 1]) r[i] = r[i + 1] + 1;
    }
...
//    dbg(l, r);
    ...
    int ma = -1;
    for (int i = 0; i < n; i++) {
        if (ma == -1 || v[ma] < v[i]) ma = i;
    }

    if (v[ma] % 2 == 0) return 0;
