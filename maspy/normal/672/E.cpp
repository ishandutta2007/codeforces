...
#line 1 "/home/maspy/compro/library/alg/monoid_cntsummax.hpp"
...
#line 3 "/home/maspy/compro/library/alg/lazy_cntsummax_set.hpp"
...
void solve() {
  ...
  vi pos(lim, N);
  FOR(i, N) {
    for (auto&& d: div[A[i]]) chmin(pos[d], i);
  }

  auto DP_pref = oneway(A);
  reverse(all(A));
  auto DP_suff = oneway(A);

  ll ANS = 0;
  LazySegTree<Lazy_CntSumMax_Set> seg(len(DP_pref));
  FOR(i, len(DP_pref)) {
    ll x = DP_pref[i];
    seg.set(i, {1, x, x});
  }

  auto chmax_suffix = [&](ll L, ll x) -> void {
    auto check = [&](tuple<ll, ll, ll> e) -> bool {
      auto [c, s, m] = e;
      return m <= x;
    };
  ...
  FOR(i, N) {
    ...
    auto [c, s, m] = seg.prod(0, N - i);
    ANS += s;

    ll a = A[i];
    for (auto&& d: div[a]) {
      if (pos[d] + 1 >= N - i) continue;
      chmax_suffix(pos[d] + 1, d);
    }
  }
  print(ANS);
}
