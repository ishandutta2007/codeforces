...
pair<bool, int> successfully(const int v, vi& a, const int h, const int g)
{
	int left_node_id = 2 * v + 1;
	int right_node_id = 2 * v + 2;

	if (left_node_id >= ((1 << h) - 1) || a[left_node_id] == 0 && a[right_node_id] == 0)
	{
		if (v < ((1 << g) - 1))
			return mp(false, v);
		else
		{
			pair<bool, int> ans(true, a[v]);
			a[v] = 0;
			return ans;
		}
	}
	else
	{
		...
		if (a[left_node_id] > a[right_node_id])
			mem = successfully(left_node_id, a, h, g);
		else
			mem = successfully(right_node_id, a, h, g);
		if (mem.X)
		{
			pair<bool, int> ans(true, a[v]);
			a[v] = mem.Y;
...
pair<ll, vi> maaaain(const int h, const int g, vi& a)
{
	pair<ll, vi> ans;
	const int r = (1 << h) - (1 << g);
	ans.Y.resize(r);
	priority_queue<pair<int, int>> possibilities;
	possibilities.emplace(a[0], 0);
	for (int i = 0; i < r; ++i)
	{
		pair<int, int> cur = possibilities.top();
		possibilities.pop();
		auto suc = successfully(cur.Y, a, h, g);
		if (suc.X)
		{
			possibilities.emplace(a[cur.Y], cur.Y);
			ans.Y[i] = cur.Y;
		}
		else
		{
			...
			while (suc.Y > cur.Y)
			{
				int father = (suc.Y - 1) / 2;
				int another_node = 4 * father + 3 - suc.Y;
				possibilities.emplace(a[another_node], another_node);
				suc.Y = father;
			}
		}
	}
