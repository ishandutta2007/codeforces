...
int main() {
    vector dp(2, vector(2, vector(N + 1, vector(N + 1, 0))));
    rep(s, 2) rep(t, 2) {
        dp[s][t][0][0] = s == 0;
    }
    ...
    rep(x, N + 1) rep(y, N + 1) {
        ...
        rep(s, 2) {
            {
                if (x > 0) {
                    dp[s][0][x][y] |= dp[s][1][x - 1][y];
                }
                if (y > 0) {
                    dp[s][0][x][y] |= dp[s ^ 1][1][x][y - 1];
                }
            }
            dp[s][1][x][y] = 1;
            {
                if (x > 0) {
                    dp[s][1][x][y] &= dp[s][0][x - 1][y];
                }
                if (y > 0) {
                    dp[s][1][x][y] &= dp[s][0][x][y - 1];
                }
            }
        }
    }
    ...
    loop(t) {
        ...
        print(dp[0][0][c0][c1] ? "Alice" : "Bob");
        assert(dp[0][0][c0][c1] == ans[c0 & 3][c1 & 3]);
    }
    return 0;
}