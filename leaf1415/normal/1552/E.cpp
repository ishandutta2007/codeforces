...
int main(void)
{
	...
	rep(i, 1, n) reverse(all(vec[i]));
	
	ll cnt = 0;
	while(cnt < n){
		rep(i, 1, n) tmp[i] = vec[i];
		rep(i, 1, n) if(ans[i] != P(0, 0)) tmp[i].clear();
		
		while(1){
			P p = P(inf, inf);
			rep(i, 1, n){
				if(sz(tmp[i]) < 2) continue;
				chmin(p, P(tmp[i][sz(tmp[i])-2], tmp[i][sz(tmp[i])-1]));
			}
			if(p.first > inf/2) break;
			...
			rep(i, 1, n) while(sz(tmp[i]) && tmp[i].back() <= p.second) tmp[i].pop_back();
		}
	}
	
	rep(i, 1, n) outl(ans[i].first, ans[i].second);
