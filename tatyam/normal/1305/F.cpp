...
signed main(){
    ...
    map<ll,ll>cnt;
    auto b=a;
    each(i,a)if(i>1)b.push_back(i-1);
    each(i,a)b.push_back(i+1);
    Uniq(b);
    uint m=b.size();
    if(m<=200){
        each(p,b){
            for(uint i=2;ull(i)*i<=p;i++){
                if(p%i==0){
                    do{
                        p/=i;
                    }while(p%i==0);
    ...
    else while(duration_cast<milliseconds>(system_clock::now() - start).count() < 1500){
        uint x=rnd()%m,y=rnd()%m;
    ...
    ull ans=LINF;
    vector<pll>candidate(all(cnt));
    sort(all(candidate),[](pll a,pll b){return a.second>b.second;});
    each(i,candidate){
        if(duration_cast<milliseconds>(system_clock::now() - start).count() > 2495)return out(ans);
        chmin(ans,solve(i.first));
        err(i.first, solve(i.first));
    }
    out(ans);
}