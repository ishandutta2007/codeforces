...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		{
			int fl=0;
			for(int i=1;i<=n;i++)if(a[i]){fl=1;break;}
			if(!fl){printf("%d\n",Z{2}.qpow(n-1));return;}
		}
		Z ans=1;
		for(int L=1,R=n;;){
			...
			while(l<=R&&!a[l])l++;
			while(r>=L&&!a[r])r--;
			if(l==R+1){ans*=Z{2}.qpow(R-L);break;}
			if(l!=L&&r!=R){
				int x=l-L,y=R-r;
				ans*=C(x+y,x),L=l,R=r;continue;
			}
			LL ls=a[l],rs=a[r];
			L=l,R=r;
			while(ls!=rs){
...
//	freopen("1.out", "w", stdout);
	...
	for(int i=2;i<=N-9;i++)fac[i]=fac[i-1]*i,inv[i]=inv[kcz%i]*(kcz-kcz/i);
	for(int i=2;i<=N-9;i++)inv[i]*=inv[i-1];
	int Case = read();
