...
struct Mo {
  using ADD = function< void(ll) >;
  using DEL = function< void(ll) >;
  using REM = function< void(ll) >;

  ll width;
  vector< ll > left, right, order;
  vector< bool > v;

  Mo(ll N, ll Q) : width((ll) sqrt(N)), order(Q), v(N) {
    iota(begin(order), end(order), 0);
  }

  void add(ll l, ll r) { /* [l, r) */
    left.emplace_back(l);
  ...
  void run(const ADD &add, const DEL &del, const REM &rem) {
    ...
    sort(begin(order), end(order), [&](ll a, ll b) {
      ll ablock = left[a] / width, bblock = left[b] / width;
...
int main(){
  ...
  while(t--){
    ...
    auto rem=[&](int x){
      vector<int>v;
      int mi=min(32,bit.sum(mp.size()));
      rep(i,1,mi+1){
        int idx=bit.lower_bound(i);
        if(idx==mp.size())break;
        v.PB(mp.val(idx));
      }
      //bit.print();
      //cout<<x<<":";debug(v);
      rep(i,0,v.size())rep(j,i+1,v.size()){
        chmin(ret[x],v[i]|v[j]);
      }
    };
    //debug(b);
