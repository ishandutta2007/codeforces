...
int main(){
    ...
    rep(i,N){
        ...
        q.emplace_back(x+K,y,-1);
        div.insert(y); div.insert(y+K);
    }
    sort(all(q));
    ll Q = q.size();
    ...
    //map<int,int> c; map<int,int> t;
    rep(i,Q){
        auto[tt,l,v] = q[i];
        //cout << tt << " " << l << " " << v << endl;
        ll m = lower_bound(all(s),l)-s.begin();
        while(s[m]<l+K){
            if(c[m]-1>=0){
                ans[c[m]-1] += (tt-t[m])*len[m];
            }
            c[m] += v; t[m] = tt; m++;
