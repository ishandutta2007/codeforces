...
int main() {
    ...
    ll A = 0;
    rep(i, N)if (lc.dis(A, 0) < lc.dis(i, 0)) { A = i; }
    ll B = 0;
    rep(i, N)if (lc.dis(A, B) < lc.dis(i, A)) { B = i; }
    ll r = A;
    ll as = lc.dis(A, B);
    rep(i, N) {
        if (lc.dis(A, i) + lc.dis(i, B) == lc.dis(A, B)) {
            ll now = abs(lc.dis(A, i) - lc.dis(i, B));
            if (now < as) { r = i; as = now; }
        }
    }
    //cout << r << endl;
    vll dis(N);
    ...
    dfs(dfs, -1, r);
    //rep(i, N)cout << dis[i] << " " << par[i] << endl;

    priority_queue<P> q;
    rep(i, N)if(i!=r)q.push({ dis[i],i });
    ...
    rep(i, K - 1) {
        ...
        if (par[x] == lef) { lef = x; q.pop(); }
        else if (par[x] == ri) { ri = x; q.pop(); }
        else { break; }
    }
