...
int main() {
    ...
    vi cnt(n);
    {
        auto dfs = REC([&](auto &&f, int x, int p) -> void {
            for(auto e : g[x]) {
                if(e != p) {
                    if(e == s) {
                        cnt[x]++;
        ...
        for(auto e : a) dfs(e - 1, -1);
    }

    vvvv(int, dp, n, n, m + 1, m + 1, inf<int>);

    auto dfs = REC([&](auto &&f, int x, int p, int a, int b) -> int {
        if(a == 0 and b == 0) return 0;
        if(dp[x][p][a][b] != inf<int>) return dp[x][p][a][b];
        auto &res = dp[x][p][a][b];
        if(g[x].size() == 1 and p != x) return f(x, x, b, 0);
        pq<tuple<int, int, int, int>> pq;
        for(auto e : g[x]) {
            if(e == p) continue;
            pq.emplace(e.cost + f(e, x, 1, a + b - 1), e, 1, e.cost);
        }
        res = inf<int>;
        rep(_, a) {
            auto [s, t, u, cost] = pq.top();
            pq.pop();
            chmin(res, s);
            if(_ == a - 1) break;
            pq.emplace(cost + f(t, x, u + 1, a + b - u - 1), t, u + 1, cost);
        }
        return res;
    });
    ...
    int mi = inf<int>;
    for(auto e : g[s])
        if(cnt[e]) chmin(mi, e.cost + dfs(e, s, cnt[e], m - cnt[e]));
    OUT(mi);
}