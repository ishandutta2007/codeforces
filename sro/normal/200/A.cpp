...
int n,m,k;
int lft[2005][2005],rgt[2005][2005];
bool lgl[2005][2005];
bool swp=0;
...
int main(){
	...
	while(k--){
		...
		int ansx=Inf,ansy=Inf,minans=Inf;
		for(int i=x;i>=max(1,x-minans);i--){
			int L=lft[i][dsu[i].root(y)];
			//cerr<<i<<' '<<L<<" L"<<endl;
			if(L>1&&abs(L-y-1)+abs(i-x)<minans){
				minans=abs(L-y-1)+abs(i-x);
				ansx=i;
				ansy=L-1;
			}
			if(L>1&&abs(L-y-1)+abs(i-x)==minans&&(swp?mp(i,L-1):mp(L-1,i))<(swp?mp(ansx,ansy):mp(ansy,ansx))){
				ansx=i;
				ansy=L-1;
			}
			L=rgt[i][dsu[i].root(y)];
			//cerr<<i<<' '<<L<<" R"<<endl;
			if(L<m&&abs(L-y+1)+abs(i-x)<minans){
				minans=abs(L-y+1)+abs(i-x);
				ansx=i;
				ansy=L+1;
			}
			if(L<m&&abs(L-y+1)+abs(i-x)==minans&&(swp?mp(i,L+1):mp(L+1,i))<(swp?mp(ansx,ansy):mp(ansy,ansx))){
				ansx=i;
				ansy=L+1;
			}
			if(!lgl[i][y]){
		...
		for(int i=x+1;i<=min(n,x+minans);i++){
			int L=lft[i][dsu[i].root(y)];
			//cerr<<i<<' '<<L<<" L"<<endl;
			if(L>1&&abs(L-y-1)+abs(i-x)<minans){
				minans=abs(L-y-1)+abs(i-x);
		...
		fio<<(swp?ansx:ansy)<<' '<<(swp?ansy:ansx)<<endl;
		lgl[ansx][ansy]=1;
		int ll=ansy,rr=ansy;
		//cerr<<dsu[ansx].root(ansy)<<' '<<dsu[ansx].root(ansy-1)<<endl;
		if(ansy>1&&lgl[ansx][ansy-1]){
			ll=lft[ansx][dsu[ansx].root(ansy-1)];
			dsu[ansx].conn(ansy,ansy-1);
		}
		//cerr<<1<<endl;
		//cerr<<dsu[ansx].root(ansy);//<<' '<<dsu[ansx].root(ansy+1)<<" FFF"<<endl;
		if(ansy<m&&lgl[ansx][ansy+1]){
			rr=rgt[ansx][dsu[ansx].root(ansy+1)];
			dsu[ansx].conn(ansy,ansy+1);
		}
		lft[ansx][dsu[ansx].root(ansy)]=ll;
		rgt[ansx][dsu[ansx].root(ansy)]=rr;
	}
}