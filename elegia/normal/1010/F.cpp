...
int n;
int sub[N], prt[N], dfn[N], top[N], rev[N], dsub[N];
Edge* g[N];
...
void hld(int u);
void dfs(int u);
pair<Poly, Poly> divideConquer(int l, int r);
...
//	int cl = clock();
    ...
    while (r) {
        int l = dfn[top[rev[r]]];
        //	LOG("DIVIDE %d %d\n", l, r);
        pl[dfn[prt[rev[l]]]] = divideConquer(l, r).first;
        pl[dfn[prt[rev[l]]]][0] = 1;
        r = l - 1;
    }
    int ans = 0, comb = 1;
    for (int i = 1; i <= n; ++i) {
        ans = (ans + pl[0][i] * (ll)comb) % P;
        if (++x == P)
            x = 0;
        comb = comb * (ll)x % P * inv(i) % P;
    }
    printf("%d\n", ans);
    return 0;
}

Poly Poly::mv(int k) const {
    Poly ret = zeroes(deg() + k);
    memcpy(ret.base() + k, base(), a.size() * sizeof(int));
    return ret;
}

pair<Poly, Poly> divideConquer(int l, int r) {
    if (l == r)
        return make_pair(pl[l].mv(1), pl[l]);
    int mid = upper_bound(dsub + l, dsub + r - 1, (dsub[l] + dsub[r]) / 2, greater<int>()) - dsub;
    pair<Poly, Poly> lp = divideConquer(l, mid), rp = divideConquer(mid + 1, r);
    return make_pair(lp.first + (rp.first * lp.second).mv(mid - l + 1), lp.second * rp.second);
};
