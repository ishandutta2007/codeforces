...
string opt_shift(string &s, int l, int r, char mx)
{
    string g = "";
    for (int i = l + 1; i <= r; i++)
        g += s[i];
...
string s2(string &s, int l, int r, char mx)
{
    string g = "";
    for (int i = l + 1; i <= r; i++)
        g += s[i];
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int p = max(-1, i - 2); p < i; p++)
        {
            string F = (p >= 0 ? dp[p] : "") + opt_shift(s, p + 1, i, mx);
            string R = (p >= 0 ? dp[p] : "") + s2(s, p + 1, i, mx);
            if (p != -1) swap(R[p], R[p + 1]);
            //cout << p + 1 << " " << i << endl;
            //cout << opt_shift(s, p + 1, i, mx) << endl;
            if (dp[i].size() == 0)
                dp[i] = F;
            dp[i] = min(dp[i], F);
            dp[i] = min(dp[i], R);
