...
unordered_map<INT,INT> mm;//It will be faster than map.

INT dfs(INT djg,INT num,INT md)
{
	if(!djg)return md==0;
	INT xz=num*100+md,bb=num,ans=0;
	if(mm.count(xz))
		return mm[xz];
	for(int i=0;i<10;i++)
	{
		...
		if(bb%10<cnt[i])
		{
			INT xxx=(md-i*p10[djg-1])%m;
			while(xxx<0)xxx+=m;
			ans+=dfs(djg-1,num+p10m[i],xxx);
		}
		bb/=10;
	}
	return mm[xz]=ans;
