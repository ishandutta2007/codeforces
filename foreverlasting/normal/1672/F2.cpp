...
namespace MAIN{
    ...
    bool dfs(res x){
        vis[x]=1;
        for(auto tox:A[x]){
            if(!vis[tox]&&!dfs(tox))return 0;
            else if(vis[tox]==1)return 0;
        }
        vis[x]=-1;
        return 1;
    }
    inline void MAIN(const res &Case){
        n=read();
        for(res i=1;i<=n;i++)vector<int>().swap(A[i]),vis[i]=buc[i]=0;
        for(res i=1;i<=n;i++)buc[a[i]=read()]++;
        res mx=(int)(max_element(buc+1,buc+n+1)-buc);
        for(res i=1;i<=n;i++)b[i]=read();
        for(res i=n;i>=1;i--)A[a[i]].pb(b[i]);
        vis[mx]=-1;
        for(res i=1;i<=n;i++)if(!vis[i]&&!dfs(i)){puts("WA");return;}
        puts("AC");
    }
}
int main(){
