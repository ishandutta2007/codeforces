...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		for(int i=n;i>n-m+1;i--){
			if(a[i]!=b[m-n+i]){puts("NO");return;}
		}
		if(a[n-m+1]==b[1]){puts("YES");return;}

		if(b[1]=='1'){
			for(int i=1;i<n-m+1;i++)if(a[i]=='1'){puts("YES");return;}
			puts("NO");return;
		}
		for(int i=1;i<n-m+1;i++)if(a[i]=='0'){puts("YES");return;}
		puts("NO");
	}
}
