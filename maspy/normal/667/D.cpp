...
void solve() {
  ...
  auto trial = [&]() -> void {
    ...
    vv(int, DP, 4, N, -INF);
    vv(int, PAR, 3, N, -1);
    FOR(v, N) if (C[v] == 0) DP[0][v] = 0;
    FOR(i, 3) {
      FOR(v, N) if (C[v] == i / 2 && DP[i][v] != -INF) {
        FOR(w, N) if (v != w && C[w] == (i + 1) / 2 && dist[v][w] != -1) {
          if (chmax(DP[i + 1][w], DP[i][v] + dist[v][w])) PAR[i][w] = v;
        }
      }
    }

    ll last = -1;
    FOR(v, N) if (PAR[2][v] != -1) {
      if (last == -1 || DP[3][last] < DP[3][v]) last = v;
    }
    if (last == -1) return;
    ...
    reverse(all(P));
    ll cost = 0;
    FOR(i, 3) cost += dist[P[i]][P[i + 1]];
    for (auto&& x: P) ++x;
    if (chmax(best, cost)) ANS = P;
  };
