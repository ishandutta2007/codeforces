...
struct strhash {
    int n; str s; mi p = 4007; vm h, pows;
    mi hash(str S, mi P) {
        p = P; s=S; n=s.length();
        pows.rsz(n+1, 1); FOR(i, 1, n+1) pows[i]=pows[i-1]*p;
        h.rsz(n+1, 0); F0R(i, n) h[i+1]=(h[i]*p + s[i]);
        return h[n];
    }
};
...
int n;
vi x, y, u, v;
map<pair<mi, mi>, int> freq;
strhash sh;

void solve() {
    freq.clear();
    cin >> n;
    x.rsz(n); y.rsz(n); u.rsz(n); v.rsz(n);
    F0R(i, n) {
        cin >> x[i] >> y[i] >> u[i] >> v[i];
        int a = u[i]-x[i], b = v[i]-y[i];
        int g = abs(gcd(a, b));
        a /= g; b /= g;
        str s = to_string(a) + ":" + to_string(b);
        mi h1 = sh.hash(s, mi(4007)), h2 = sh.hash(s, mi(6007));
        freq[mp(h1, h2)]++;
    }
    ll ans = 0;
    F0R(i, n) {
