...
bool solve(li u, li v){
  vi U,V;
  for(li i = 0; i < (li)(32); ++i) if(u&(1ll<<i)) U.push_back(i);
  for(li i = 0; i < (li)(32); ++i) if(v&(1ll<<i)) V.push_back(i);
  if(U.size() < V.size()) return false;

  while(!V.empty() && U.back() == V.back()) { V.pop_back(); U.pop_back(); }

  if(!V.empty()) {
    if(U.back() > V.back()) return false;
    V.pop_back(); U.pop_back();
    while(U.size() > V.size()) U.pop_back();
  }
...
bool brute(li u, li v) {
  static map<pii, bool> M;
  if(M.count(make_tuple(u,v))) return M[make_tuple(u,v)];
  if(u > v) return M[make_tuple(u,v)] = false;
  if(u == v) return M[make_tuple(u,v)] = true;
  for(li a = (1); a <= (li)(u); ++a) if((a&u) == a && brute(u+a,v)) {
    return M[make_tuple(u,v)] = true;
  }
  return M[make_tuple(u,v)] = false;
}
