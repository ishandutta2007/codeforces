...
int t,n;
char x[100005];int cntx;
char y[100005];int cnty;
int dif;
void Q()
{
	scanf("%d%s%s",&n,x,y);cntx=cnty=dif=0;
	rep(i,n)
	{
		if(x[i]=='1') ++cntx;
		if(y[i]=='1') ++cnty;
	...
	if(cntx==cnty)
	{
		rep(i,n) if(x[i]!=y[i]) ++dif;
		if(cntx+cnty==n+1) dif=min(dif,n-dif);
	}
	else if(cntx+cnty==n+1)
	{
		rep(i,n) if(x[i]==y[i]) ++dif;
	}
	else
