...
void dfs(ll v) {
	...
	for (ll x: edges[v]) dfs(x);
	
	en[v] = tt;
}

void ins(ll i) {
	mt.update(st[i], en[i], 1);
}

void del(ll i) {
	mt.update(st[i], st[i], -1e9);
}
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		if (b[i] != -1) edges[b[i]].push_back(i);
	}
	...
	for (ll i = 0; i < k; i++) ins(i);
	cout << mt.query(0, n - 1).val;
	for (ll i = 0; i + k < n; i++) {
		del(i);
		ins(i + k);
		cout << " " << mt.query(0, n - 1).val;
