...
int main(){
  ...
  while(t--){
    ll n,q;cin>>n>>q;
    vector<ll>a(n),b(n+1);
    auto smi=range_min::make(n+1);
    auto smx=range_max::make(n+1);
    rep(i,0,n){
      char c;cin>>c;
      if(c=='+')a[i]=1;
      else a[i]=-1;
    ...
    while(q--){
      ...
      if(b[r]-b[l]==0)cout<<0<<endl;
        ...
        if(b[l]<b[r]){
          auto ch=[&](ll k){return k>=(b[r]+b[l]+1)/2;};
          v.PB(smx.find_first(l,ch));
        }
        else if(b[l]>b[r]){
          auto ch=[&](ll k){return k<=(b[r]+b[l]-1)/2;};
          v.PB(smi.find_first(l,ch));
        }
        cout<<v.size()<<endl;
        debug(v);
      }
