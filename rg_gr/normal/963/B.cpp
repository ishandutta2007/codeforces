...
int dfs(int u,int fa)
{
	...
	if(v!=fa)in^=dfs(v,u);
	gr
	if(in==b[u]){addedge(u,fa,1),d[fa]++;return 1;}
	if(u==1)
	{
		puts("NO");
		exit(0);
...
signed main()
{
	...
	dfs(1,0);
	puts("YES");
	std::priority_queue<int,std::vector<int>,std::greater<int> >q;rg(n)if(d[i]==0)q.push(i);gr
	while(!q.empty())
	{
		int u=q.top();q.pop();
		if(u)
