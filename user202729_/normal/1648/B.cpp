...
// I feel that this algorithm is actually fast, but unproven
...
void up(){
	...
	int const limit=read<int>();
	std::vector<char> seen(limit+1);
	for(int _=n; _--;){
...
NO:
	...
	std::vector<int> seenSuffix(seen.size()+1);
	for(int index=int(seen.size()); index--;)
		seenSuffix[index]=seenSuffix[index+1]+seen[index];

	int last=-1;
	std::vector<std::pair<int, int>> emptySegments; // excluding the last segment, if any
	for(int const index: range(2, int(seen.size()))){
		if(last<0 and not seen[index]){
			last=index;
		}else if(seen[index] and last>=0){
			emptySegments.push_back({last, index});
			last=-1;
		}
	}

	for(int const value: range(2, int(seen.size()))) if(seen[value]){
		for(auto [first, last]: emptySegments){
			assert(not(first<=value and value<last));
			if(value<first*2) break;

			// doing some math we can calculate...
			int leastB=value/last+1, mostB=value/first; // inclusive
			assert(mostB>=2);  //if(mostB<=1) break;  // because of the check above

			if(seenSuffix[leastB]-seenSuffix[mostB+1]!=0){
				// there's some b in this region, we have value  A, b  A but their quotient is in [first..last[  A
				goto NO;
			}
