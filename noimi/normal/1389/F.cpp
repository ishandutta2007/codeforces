...
#define mt make_tuple
    ...
    UNIQUE(xs);
    for(auto &[l, r, t] : v) { l = lb(xs, l), r = lb(xs, r), t--; }
    auto f = [](int x, int y) { return max(x, y); };
    vector<LazySegmentTree<int, int>> seg(2, LazySegmentTree<int, int>((int)xs.size() + 1, f, plus<int>(), plus<int>(), 0, 0));
    sort(all(v));
    for(auto [l, r, t] : v) {
        int pre = seg[t].query(r, r + 1);
        int nxt = 1 + max(seg[t].query(0, r + 1), seg[t ^ 1].query(0, l));
        if(nxt > pre) { seg[t].update(r, r + 1, nxt - pre); }
        seg[t].update(r + 1, xs.size() + 2, 1);
    }
    cout << max(seg[0].query(0, xs.size() + 2), seg[1].query(0, xs.size() + 2)) << endl;
}