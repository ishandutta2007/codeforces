#line 1 "codeforces-workspace\\contest\\724\\g\\g.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\724\\g\\g.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\724\\g\\g.cpp"
...
#line 2 "Library\\src\\algebra\\linear\\xor_elimination.hpp"
...
#line 9 "Library\\src\\algebra\\linear\\xor_elimination.hpp"
...
#line 36 "codeforces-workspace\\contest\\724\\g\\g.cpp"
...
void main() {
  ...
  vector<i32> visited(graph.size());
  vector<u64> depth(graph.size());

  using return_type = array<array<mint, 2>, 60>;

  // Return size of component.
  fixed_point dfs = [&](auto dfs, int now, u64 p,
                        auto &b) noexcept -> return_type {
    if (visited[now]) {
      b.add(depth[now] ^ p);
      return {};
    }
    ...
    for (const auto &[to, w] : graph[now]) {
      auto chi = dfs(to, p ^ w, b);
  ...
  for (auto root : range(graph.size())) {
    if (visited[root]) continue;

    xor_elimination<u64> basis;
    auto ret = dfs(root, 0, basis);
    ...
    for (const auto height : range(60)) {
      auto &num = ret[height];

      if (any >> height & 1) {
        answer += unit * (num[0] + num[1] - 1) * (num[0] + num[1]) / 2 * di / 2;
      } else {
        answer += unit * num[0] * num[1] * di;
      }
