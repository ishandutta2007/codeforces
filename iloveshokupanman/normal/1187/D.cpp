...
int main() {
    ...
    rep(test, T) {
        ...
        lazy_segtree<ll, max, e, ll, max, max, e> sgb(N);

        string ans = "YES";
        rep(i, N) {
            if (A[i].first != B[i].first) { ans = "NO"; }

            ll x = sgb.get(A[i].second);
            //cout << A[i].second<<" "<<B[i].second << " " << x << endl;

            if (B[i].second < x) { ans = "NO"; }

            sgb.apply(A[i].second,N, B[i].second);
        }
