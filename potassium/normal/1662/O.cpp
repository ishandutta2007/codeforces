...
int main() {
	CASE(t) {
		int n; R(n);
		vector<vector<pii>> circ(21);
		vector<vector<pii>> str(360);

		for (int i = 0; i < n; i++) {
			char op; R(op);
			if (op == 'C') {
				int r, t1, t2; R(r, t1, t2);
				if (t1 < t2)
					circ[r].push_back({ t1, t2 });
				else {
					circ[r].push_back({ t1, 360 });
					circ[r].push_back({ 0, t2 });
				}
			}
			else if (op == 'S') {
				int r1, r2, t; R(r1, r2, t);
				str[t].push_back({ r1, r2 });
			}
			else assert(false);
		}

		for (int i = 1; i <= 20; i++) sort(circ[i].begin(), circ[i].end());
		for (int i = 0; i < 360; i++) sort(str[i].begin(), str[i].end());

		auto inc = [&](pii range, int v) {
		...
		vector<bool> trav(21 * 360, false);
		function<void(int, int)> dfs = [&](int r, int t) {
			if (trav[r * 360 + t]) return;
			trav[r * 360 + t] = true;

			if (r + 1 <= 20) {
				int qr = r + 1;
				auto it = upper_bound(circ[qr].begin(), circ[qr].end(), make_pair(-1, t), cmpy);
				if (it == circ[qr].end() || !inc(*it, t)) dfs(r + 1, t);
			}
			...
			{
				int qt = (t + 1) % 360;
				auto it = upper_bound(str[qt].begin(), str[qt].end(), make_pair(-1, r), cmpy);
				if (it == str[qt].end() || !inc(*it, r)) dfs(r, (t + 1) % 360);
			}

			{
				int qt = t;
				auto it = upper_bound(str[qt].begin(), str[qt].end(), make_pair(-1, r), cmpy);
				if (it == str[qt].end() || !inc(*it, r)) dfs(r, (t + 359) % 360);
			}
		};
		dfs(0, 0);
