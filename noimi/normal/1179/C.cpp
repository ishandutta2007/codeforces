...
int main() {
    ...
    vi setter(N + 1);
    setter[0] += n;
    rep(i, n) setter[lb(xs, a[i]) + 1]--;
    setter[0] -= m;
    rep(i, m) setter[lb(xs, b[i]) + 1]++;
    rep(i, N) setter[i + 1] += setter[i];

    atcoder::lazy_segtree<int, f, e, int, g, g, id> seg(setter);
    for(auto [t, i, v] : q) {
        int idx = lb(xs, v);
        if(t == 1) {
            int y = lb(xs, a[i]);
        ...
        if(seg.all_prod() <= 0)
            ...
            OUT(xs[seg.min_left(N, [](int x) { return x <= 0; }) - 1]);
    }
}