...
int main() {
	...
	for(int i = 0; i < t; i++){
		...
		vi b = manacher(f);
		for(int i = 0; i < 2*sz(f)-1; i+=2){
		    if(i+1 == b[i]){
		        ckmax(ans, mp(b[i], mp(0, b[i]-1)));
		    }
		    else if(2*sz(f)-i-1 == b[i]){
		        ckmax(ans, mp(b[i], mp(sz(f)-b[i], sz(f)-1)));
		    }
		}
		for(int i = 1; i < 2*sz(f)-1; i+=2){
		    if(i+1 == b[i]){
		        ckmax(ans, mp(b[i], mp(0, b[i]-1)));
		    }
		    else if(2*sz(f)-i-1 == b[i]){
