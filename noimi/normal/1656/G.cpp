...
struct CycleDecomposisiton {
    int n;
    vector<pair<int, int>> v;
    CycleDecomposisiton(Graph &g) : n(g.size()), v(g.size()) {
        rep(i, n) {
            assert(g[i].size() == 2);
            v[i] = pair(g[i][0], g[i][1]);
        }
    }
    CycleDecomposisiton(int n) : n(n), v(n, pair(-1, -1)) {}
        ...
        rep(i, n) {
            if(!used[i]) {
                ...
                if(v[i].first != i) {
                    ...
                    while(true) {
                        if(x[x.size() - 2] == v[x.back()].first)
                            x.emplace_back(v[x.back()].second);
                        else
                            x.emplace_back(v[x.back()].first);
                        if(x.back() == i) {
                            x.pop_back();
                            break;
                        }
...
int main() {
    TEST {
        ...
        if(~n & 1 and odd) {
            ...
            NO();
        else if(one == 1 and mp[a[n / 2]].size() & 1)
            NO();
        else {
            vi f(n, -1);
            CycleDecomposisiton cd(n);
            fore(e, v, mp) {
                if(si(v) & 1) { rep(i, si(v)) if(v[i] == n / 2) swap(v[0], v[i]); }
                rep(i, 0, si(v) - 1, 2) cd.add(v[i], v[i + 1]);
                if(si(v) & 1) cd.add(v.back(), n / 2);
            }

            rep(i, n / 2) { cd.add(i, n - 1 - i); }

            auto v = cd.build();
            fore(w, v) {
                if(n & 1 and count(all(w), n / 2)) {
                    rep(i, si(w)) {
                        if(w[i] == n / 2 and a[w[(i + 1) % si(w)]] == a[w[i]]) {
                            REV(w);
                            break;
                        }
                    }
                ...
                rep(i, si(w)) {
                    if(w[i] + w[(i + 1) % si(w)] == n - 1 and a[w[(i + 1) % si(w)]] == a[w[(i + 2) % si(w)]]) {
                        rotate(begin(w), begin(w) + i, end(w));
                        break;
                    }
                }
            }
            ...
            ans.eb(v[0][0]);
            rep(i, 1, si(v)) {
                rep(j, 2, si(v[i])) ans.eb(v[i][j]);
                ans.eb(v[i][0]);
            }
            rep(i, si(v)) ans.eb(v[i][1]);
            rep(i, 2, si(v[0])) ans.eb(v[0][i]);
            YES();
            vi res(n);
            rep(i, n) res[ans[i]] = ans[(i + 1) % n] + 1;
            OUT(res);
        }
    }
}