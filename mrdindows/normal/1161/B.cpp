...
struct Hash {
    ...
    void add(ll x, ll y) {
        ...
        if (x > y) swap(x, y);
        sum += x + y; sum %= mod;
        sum2 += x * x + y * y; sum2 %= mod1;
        sum3 += ((x ^ y) + 37) * (x ^ (y + 517)) + x * y; sum3 %= fftmod;
        sumt += (x + 517) * (y + 1000333); sumt %= 98765431;
    }
};
bool operator == (const Hash& h1, const Hash& h2) {
    return h1.sum == h2.sum && h1.sum2 == h2.sum2 && h1.sum3 == h2.sum3 && h1.sumt == h2.sumt;
}
Hash zero;
int solve() {
    cin >> n >> m;
    ...
    for (int i = 0; i < m; ++i) {
        p[i + m] = p[i];
        zero.add(p[i].first, p[i].second);
    }
    for (int d = 1; d < n; ++d) {
        if (n % d) continue;
        Hash h;
        for (int i = 0; i < m; ++i) {
            h.add(p[i].first - d, p[i].second - d);
        }
        if (h == zero) {
            cout << "YES" << endl;
            return 0;
        ...
        continue;
        auto idx = lower_bound(p, p + m, pii(d + 1, 0)) - p;
        if (idx == m) continue;
        bool ok = true;
        for (int j = 0; j < m; ++j) {
//            ok = ok && (norm(p[idx + j].first - d) == p[j].first) && (norm(p[idx + j].second - d) == p[j].second);
        }
        if (ok) {
            cout << "YES" << endl;
            return 0;
