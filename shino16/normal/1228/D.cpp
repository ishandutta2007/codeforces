...
int main() {
  ...
  if (graph[0].size() < 2) { cout << -1 << newl; exit(0); }
  VI vs[3];
  vs[0].push_back(0);
  sort(all(graph[0]));
  rep(i, 1, n) if (not binary_search(all(graph[0]), i)) vs[0].push_back(i);
  vs[1].push_back(graph[0].front());
  sort(all(graph[vs[1].front()]));
  rep(i, n) if (i != vs[1].front() and not binary_search(all(graph[vs[1].front()]), i)) if (binary_search(all(vs[0]), i)) { cout << -1 << newl; exit(0); } else vs[1].push_back(i);
  rep(i, n) if (not binary_search(all(vs[0]), i) and not binary_search(all(vs[1]), i)) vs[2].push_back(i);
  rep(i, 3) if (vs[i].size() == 0) { cout << -1 << newl; exit(0); }
  VI at(n);
  rep(i, n) rep(j, 3) if (binary_search(all(vs[j]), i)) at[i] = j;
  rep(i, n) if (vs[at[i]].size() != n - graph[i].size()) { cout << -1 << newl; exit(0); }
  dump(0);
  rep(i, 3) rep(j, i+1, 3) {
    rep(v, n) for (int u : graph[v]) if (at[v] == at[u]) { cout << -1 << newl; exit(0); }
  }
  rep(i, n) cout << at[i] + 1 << " \n"[i == n-1];
}