...
void solve()
{
	...
	for (int i = 1; i <= 5000; ++i)
	{
		...
		if (!v[i].empty())
		{
			...
			for (int j = 0; j < MAXN; ++j)
			{
				if (int tmp = dp[i - 1][i ^ j]; tmp < pos)
				{
					freshmin(dp[i][j], *lower_bound(v[i].begin(), v[i].end(), tmp));
				}
			}
			freshmin(dp[i][i], v[i].front());
		}
	}
