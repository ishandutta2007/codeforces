...
long long j,ans,lst[1001010],tot,r1,r2,GG[1001010],fre[1001010],ans1,ans2,ans3,Testing,n,m,A,B,i;
int find(int x)
{
...
//	freopen("testdata.in","r",stdin);
	...
	for (; Testing; Testing--) {
		cin>>n>>m>>A>>B;
		for (i=1;i<=m;i++) cin>>E[i].lft>>E[i].rit;
		for (i=1;i<=n;i++) V[i].clear();
		ans1=ans2=ans3=0;
		for (i=1;i<=n;i++) f[i]=i,fre[i]=0;
		for (i=1;i<=m;i++)
           if ((E[i].lft!=A)&&(E[i].rit!=A))
                 { 
                    r1=find(E[i].lft);r2=find(E[i].rit);
                    f[r1]=r2;
				 }
			
		for (i=1;i<=n;i++) lst[i]=find(i);
		for (i=1;i<=n;i++) 
		   if ((i!=A)&&(i!=B))
		   fre[find(i)]++;
		for (i=1;i<=n;i++)
		    ans1+=fre[i]*(fre[i]-1)/2;
	ans=ans1;
		for (i=1;i<=n;i++) f[i]=i,fre[i]=0;
		for (i=1;i<=m;i++)
           if ((E[i].lft!=B)&&(E[i].rit!=B))
                 { 
                    r1=find(E[i].lft);r2=find(E[i].rit);
                    f[r1]=r2;
				 }
			for (i=1;i<=n;i++)
		   if ((i!=A)&&(i!=B)) fre[find(i)]++,V[find(i)].push_back(i);
		for (i=1;i<=n;i++)
		    if (find(i)==i)
		        {
				...
				for (j=0;j<V[i].size();j++) GG[lst[V[i][j]]]--;
				}
	/*	for (i=1;i<=n;i++) f[i]=i,fre[i]=0;
		for (i=1;i<=m;i++)
           if ((E[i].lft!=B)&&(E[i].rit!=B)&&(E[i].lft!=A)&&(E[i].rit!=A))
                 { 
                    r1=find(E[i].lft);r2=find(E[i].rit);
                    f[r1]=r2;
				 }
		for (i=1;i<=n;i++) fre[find(i)]++;
		for (i=1;i<=n;i++)
		    ans3+=fre[i]*(fre[i]-1)/2;*/
	    tot=n*(n-1)/2;
	    cout<<tot-ans-(n-1)-(n-2)<<endl;
	}
	return 0;
}