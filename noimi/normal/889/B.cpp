...
signed main(){
    ...
    rep(i,n){
        rep(j,26){
            if(count(all(s[i]),'a'+j)>1){
                cout<<"NO\n";return 0;
            }
            if(count(all(s[i]),'a'+j))vis[j]=1;
        }
    }

    rep(i,n){
        rep(j,s[i].size()-1){
            if(nxt[s[i][j]-'a']!=-1 and nxt[s[i][j]-'a']!=s[i][j+1]-'a'){
                cout<<"NO\n";return 0;
            }
            else nxt[s[i][j]-'a']=s[i][j+1]-'a';
        }
    }
    vec cnt(26);
    rep(i,26)if(nxt[i]!=-1)cnt[nxt[i]]++;
    string ans;
    rep(i,26){
    ...
    if(ans.size()!=accumulate(all(vis),0)){
        cout<<"NO\n";
    }
    else cout<<ans<<endl;
