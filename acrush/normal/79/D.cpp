...
int main()
{
    ...
    for (int k=0;k<n;k++)
    {
        ...
        {
            int key=q[cl];
            for (int i=0;i<m;i++) for (int op=-1;op<=1;op+=2)
            {
    ...
    f[0]=0;
    for (int set=0;set<two(n)-1;set++) if (f[set]>=0)
    {
        int i=0;
        for (;contain(set,i);i++);
        for (int j=i+1;j<n;j++)
            if (!contain(set,j) && g[i][j]>=0)
            {
                int newset=set|two(i)|two(j);
                int t=f[set]+g[i][j];
                if (f[newset]<0 || t<f[newset])
                    f[newset]=t;
            }
    }
    printf("%d\n",f[two(n)-1]);
