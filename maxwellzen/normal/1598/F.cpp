...
void solve() {
    cin >> n;
    read(a, n);
    info.rsz(n); freq.rsz(n);
    F0R(i, n) {
        int mn = 0, cnt = 0, cur = 0;
        freq[i].rsz(a[i].size()+1, 0);
        F0R(j, a[i].size()) {
            if (a[i][j]=='(') cur++;
            else cur--;
            if (cur<mn) mn = cur, cnt = 0;
            if (cur==mn) freq[i][a[i].size()+cur]++, cnt++;
        }
        info[i] = mp(mn, cur);
    }
    int twon = 1<<n;
    int ans = 0;
    vi dp(twon, -1), sum(twon, 0);
    dp[0] = 0;
    FOR(i, 1, twon) {
        int last = __builtin_ctz(i);
        sum[i] = sum[i^1<<last] + info[last].S;
        F0R(j, n) if (i&1<<j) {
            int rest = i^1<<j;
            if (dp[rest]>-1) {
                int cur = dp[rest];
                if (sum[rest]>=0 && sum[rest]<=a[j].size()) cur += freq[j][a[j].size()-sum[rest]];
                if (sum[rest]+info[j].F>=0) ckmax(dp[i], cur);
                ckmax(ans, cur);
            }
        }
        debug(i, dp[i], sum[i]);
