...
struct P {
  ...
  bool operator<(const P& rhs) {
    return make_tuple(rm, pl, pr) < make_tuple(rhs.rm, rhs.pl, rhs.pr);
  }
};
...
int main() {
  int n = IN;
  string s = IN;
  auto dp = make_v(n, n, P{});
  rep(l, n) dp[l][l] = P{0, l, l};
  rep(l, n-1) {
    int r = l+1;
    dp[l][r] =
      s[l] + 1 == s[r] ? P{1, l, l} :
  ...
  auto merge = [&](P x, P y) {
    P res = {x.rm + y.rm, x.pl, y.pr};
    if (s[x.pr] == s[y.pl] + 1 and x.pl == x.pr)
      res.pl = y.pl;
    if (s[x.pr] + 1 == s[y.pl] and y.pl == y.pr)
      res.pr = x.pr;
    if (abs(s[x.pr] - s[y.pl]) == 1) res.rm++;
    return res;
  };
  rep(d, 2, n) {
    rep(l, n-d) {
      ...
      P res = {-INF, 0, 0};
      rep(split, l, r) {
        chmax(res, merge(dp[l][split], dp[split+1][r]));
      }
      dp[l][r] = res;
    }
  }
