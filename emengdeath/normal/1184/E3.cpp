...
using namespace std;
const int N = 1e5 +1;
int fa[N][18], mx[N][18], deep[N], id[N];
struct node{
    int x, y, z, id;
}a[N * 10];
int n, m;
...
void dfs(int x) {
    ...
    for (auto u:g[x])
        if (u.first != fa[x][0]) {
            fa[u.first][0] = x;
            mx[u.first][0] = u.second.first;
            id[u.first] = u.second.second;
            ans[u.second.second] = 1e9;
            deep[u.first] = deep[x];
            dfs(u.first);
        }
}
...
void up1(int x ,int fax, int v) {
    int y = 0;
    while (deep[x] > deep[fax]) {
        ans[id[x]] = min(v, ans[id[x]]);
        if (y) faa[y] = x;
        y = getfa(x);
        x = fa[y][0];
    }
...
int main() {
    ...
    for (int i = 1; i <= m ; i ++) {
        ...
        if (x != y) {
            g[a[i].x].push_back(make_pair(a[i].y, make_pair(a[i].z, a[i].id)));
            g[a[i].y].push_back(make_pair(a[i].x, make_pair(a[i].z, a[i].id)));
            faa[x] = y;
            a[i].id = 0;
        }
    }
