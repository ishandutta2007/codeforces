...
void solve()
{
	...
	ValueType value_init = { -INF, INF, 0, 0, 0, 0 };
	auto value_merge = [&](const ValueType& a, const ValueType& b) -> ValueType
	{
		if (a.lv == -INF) return b;
		if (b.lv == -INF) return a;
		ValueType c;
		c.lv = a.lv;
		c.rv = b.rv;
		c.len = a.len + b.len;
		c.ln = a.ln == a.len && a.rv <= b.lv ? a.len + b.ln : a.ln;
		c.rn = b.rn == b.len && a.rv <= b.lv ? a.rn + b.len : b.rn;
		c.cnt = a.cnt + b.cnt;
		if (a.rv <= b.lv) c.cnt += a.rn * b.ln;
		return c;
	};
	...
	auto tag_spread = [](const TagType& tag, ValueType& value) -> void
	{
		/*if (tag.tag != -1)
		{
			value.lv = value.rv = tag.tag;
			value.ln = value.rn = value.len;
			value.cnt = value.len * (value.len + 1) / 2;
		}*/
	};
	...
	for (int i = 0; i < q; ++i)
	{
		...
		{
			x -= 1;
			tree.ModifyValue(x, ValueType{ y,y,1,1,1,1 });
		}
		else
		{
			auto res = tree.Query(x - 1, y).cnt;
			writeln(res);
		}
	}
}
