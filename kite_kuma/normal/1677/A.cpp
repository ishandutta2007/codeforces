...
void solve() {
	...
	vvi right_small(n, vi(n + 1));
	auto left_small = right_small;

	rep(i, n) {
		drep(j, n) {
			right_small[i][j] = right_small[i][j + 1];
			if(a[i] > a[j]) right_small[i][j]++;
		}

		rep(j, n) {
			int &tmp = left_small[i][j + 1];
			tmp = left_small[i][j] + (a[j] < a[i] ? 1 : 0);
		}
	}

	rep(b, n) {
		rep(c, b + 1, n) { ans += ll(left_small[c][b]) * ll(right_small[b][c + 1]); }
	}
