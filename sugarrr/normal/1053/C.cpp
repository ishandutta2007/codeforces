...
mint opi(mint x,mint y){return x+y;}
mint ei(){return (mint)0;}

signed main(){fastio
    
    ll n,q;cin>>n>>q;
    vl a(n);rep(i,0,n-1)cin>>a[i];
    vl w(n);rep(i,0,n-1)cin>>w[i];
    vl b(n);
    rep(i,0,n-1)b[i]=a[i]-i;
    segtree<ll,opl,el>seg(w);
    vi INIT(n);rep(i,0,n-1)INIT[i]=(mint)b[i]*(mint)w[i];
    segtree<mint,opi,ei>teg(INIT);
    ...
    while(q--){
        ...
        if(x>0){
            x--;y--;
            tar=seg.prod(x,y+1);
            ll m=seg.max_right<f>(x);
            assert(x<=m && m<=y);
            auto calc=[&](ll l,ll r,ll bm){
                if(l>r){
                    assert(l==r+1);
                    return (mint)0;
                }
                //sum (b[m]-b[i])*w[i]
                return (mint)bm*(mint)seg.prod(l,r+1) - (mint)teg.prod(l,r+1);
            };
            mint ans=calc(x,m-1,b[m])-calc(m+1,y,b[m]);
            cout<<ans.val()<<endl;
        }else{
            x=-x;
            x--;
            seg.set(x,y);
            teg.set(x,(mint)y*(mint)b[x]);
        }
    }
