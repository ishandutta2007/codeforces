...
ll solve() {
	int n, m;
	cin >> n >> m;
	const int person = n * m;
	VEC(ll, a, person);
	map<ll, int> counter;
	foa(t, a) counter[t]++;
	// map<ll, pair<int, int>> range;
	map<ll, vector<pair<int, int>>> perm;
	map<ll, int> it;

	debug(a, counter);

	int acc = 0;
	for(auto [val, s] : counter) {
		int lef = acc;
		int rig = acc + s;
		// range[val] = pair(lef, rig);
		acc += s;
		...
		int t = rig - b * m;
		if(a == b) {
			drep(j, m - l, t) { tmp.emplace_back(a, j); }
		} else {
			rep(j, l) { tmp.emplace_back(a, m - j - 1); }
			rep(row, a + 1, b) { drep(col, m) tmp.emplace_back(row, col); }
			drep(j, t) tmp.emplace_back(b, j);
		}
	...
	V<binary_indexed_tree<ll>> bits;
	rep(i, n) { bits.push_back(binary_indexed_tree<ll>(m)); }
