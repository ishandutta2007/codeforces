...
int main(){
	...
	ll k;
	vector<pair<ll, ll>> v;
	rep(i, q) cin >> k, v.pb(mp(k - n, i));
	sort(all(v));
	int ans[q];
	int p = 0;
	vector<pair<int, int>> v2;
	rep(i, m) v2.pb(mp(cnt[i], i));
	sort(all(v2));
	vector<int> li[m];
	int w[m] = {};
	li[0].pb(v2[0].second);
	...
	rep(i, m - 1){
		...
		li[p].pb(v2[i + 1].second);
	}
	SegmentTree<int> seg(m, [](int a, int b) { return a + b; }, 0);
	seg.build();
	ll qe = 0, sum = 0, sz = 0;
	rep(i, p){
		sz += li[i].size();
		rep(j, li[i].size()) seg.update(li[i][j], seg[li[i][j]] + 1);
		sum += (w[i + 1] - w[i]) * sz;
		while (v[qe].first <= sum && qe < q){
			ll idx = sz - 1 - (sum - v[qe].first) % sz;
			ans[v[qe].second] = seg.find_first(0, [&](int x) { return x > idx; });
			qe++;
		}
	}
	vector<ll> ne;
	...
	sz = m;
	while (qe < q){
		ans[v[qe].second] = ne[(v[qe].first - sum - 1) % sz];
		qe++;
	}
	rep(i, q){
		cout << ans[i] + 1;
