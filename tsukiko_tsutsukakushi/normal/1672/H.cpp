...
void solve() {
    INT(n, q);
    STR(s);
    vc<int> sum0(n + 1, 0), sum1(n + 1, 0);
    rep(i, n - 1) {
        sum0[i + 1] = sum0[i];
        sum1[i + 1] = sum1[i];
        if(s[i] == '0' and s[i + 1] == '0') ++ sum0[i + 1];
        if(s[i] == '1' and s[i + 1] == '1') ++ sum1[i + 1];
    }
    rep(_, q) {
        INT(l, r); -- l; -- r;
        int cnt0 = sum0[r] - sum0[l];
        int cnt1 = sum1[r] - sum1[l];
        int ans = max(cnt0, cnt1) + 1;
        out(ans);
    }
}
