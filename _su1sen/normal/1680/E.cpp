...
int main() {
    ...
    loop(t) {
        ...
        {
            int l = 0;
            while (l < n and g[0][l] == '.' and g[1][l] == '.') ++l;
            int r = n;
            while (r > 0 and g[0][r - 1] == '.' and g[1][r - 1] == '.') --r;

            g[0] = g[0].substr(l, r - l);
            g[1] = g[1].substr(l, r - l);
            n = r - l;
        }

        vector dp(n, vector<int>(4, numeric_limits<int>::max() / 2));
        if (g[0][0] == '*' and g[1][0] == '*') {
            dp[0][3] = 2;
        } else if (g[0][0] == '*') {
            dp[0][3] = 2;
        ...
        rep(i, 1, n) rep(s, 1, 4) {
            int x = (g[0][i] == '*') | ((g[1][i] == '*') << 1);
            rep(t, 1, 4) if ((x & t) == x and (s & t)) {
                chmin(dp[i][t], dp[i - 1][s] + popcount(t));
            }
        }
        print(*min_element(all(dp[n - 1])) - 1);
    }
    return 0;
}