...
set<Pr> p;
vector<ll> aa(3e5+1,0);

//dfs
//s: i:dfs (0 )
// int L = 2e9; //L
// vector<int> vis,depth,et_in,et_out; int t,et;
void dfs_(graph &g, int s,int i){
    t++; et++; et_in[s] = et;
    p.emplace((subtree_size[s]+1)/2,s);
    for(auto[nx,cost,label]:g.G[s]){
        //if(t>=L) break;
        if(vis[nx]==i) continue;
        depth[nx] = depth[s] + cost;
    ...
    ll ss = (*(p.begin())).first;
    while(ss<=subtree_size[s]){
        ll id = (*(p.begin())).second; aa[id] = s;
        // cout << ss << " " << id << "\n";
        // cout << s << " " << subtree_size[s] << "\n";
        // cout << "----------------" << "\n";
        p.erase(p.begin());
        ss = (*(p.begin())).first;
    }
...
void dfs(graph &g, int s=1,int i=0){
    int N = g.G.size();
    if(i==0){
        //et_in.assign(N,-1); et_out.assign(N,-1); 
        vis.assign(N,-1); depth.assign(N,-1);
    }
    vis[s] = i; depth[s] = 0; t = -1; et = -1;
    dfs_(g,s,i);
}
