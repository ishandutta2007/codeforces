...
ll slv(vector<int> v, int c1, int cl, int cr, int st) {
	ll ans = 0;
	ll cur = 0;
	int ptr0 = 0;
	while (ptr0 < sz(v) && v[ptr0] == 0) ++ptr0;
	int ptrkil = 0;
	for (int p = st; ; p ^= 1) {
		ans = max(ans, cur);
		if (p) {
	...
	while (_--) {
		...
		for (; l < r;) {
			if (s[l] == '1') {
				...
				c0.push_back(-1);
				for (; l < n && s[l] == '0'; ++l, ++c0.back());
			}
		}
		sort(all(c0));

		cout << max(slv(c0, c1, cl, cr, 0), slv(c0, c1, cl, cr, 1)) << endl;
	}
}
