...
int main(){
	...
	cauto n=read();
	vv<int> vss(n);
	for(auto &vs:vss) vs=readV(read(),0);
	vector<string> re(n);
	rep(i,n) re[i]=string(vss[i].size(),'.');
	out(vss,1);
	vector<set<pair<int,pii>>> g(n);
	{
		map<int,pii> mp;
		rep(i,n)rep(j,vss[i].size()){
			cauto [it,f]=mp.emplace(vss[i][j],pii(i,j));
			if(f) continue;
			cauto &[I,J]=it->Y;
			if(i==I){
				re[i][j]='L';
		...
		if(mp.size()){
			map<int,int> cnt;
			for(cauto &vs:vss)for(cauto &v:vs) ++cnt[v];
			int sum=0;
			for(cauto &p:cnt) sum+=p.Y;
			assert(sum);
			cout<<"NO" NL;
			return 0;
		}
	...
	rep(v0,n){
		while(g[v0].size()){
			out(v0,1);
			for(int init=1,v=v0;init||v!=v0;){
				init=0;
				cauto [u,p]=*g[v].begin(); g[v].erase(g[v].begin());
				out(v,u,p,1);
				g[u].erase(pair{v,pair{p.Y,p.X}});
				re[v][p.X]='L';
				re[u][p.Y]='R';
				v=u;
			}
	...
	for(cauto &s:re) assert(count(all(s),'L')*2==s.size());
	for(cauto &s:re) assert(count(all(s),'R')*2==s.size());
	map<int,int> cnt;
	rep(i,n)rep(j,vss[i].size()) cnt[vss[i][j]]+=re[i][j]=='L'?1:-1;
	for(cauto &p:cnt) assert(p.Y==0);
	cout<<"YES" NL;
	for(cauto &s:re) cout<<s NL;
	return 0;
}