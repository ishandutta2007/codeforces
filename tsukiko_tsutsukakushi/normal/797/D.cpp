...
void solve() {
    ...
    rep(i, n) {
        in(a[i], l[i], r[i]); 
        if(l[i] != -1) -- l[i], p[l[i]] = i;
        if(r[i] != -1) -- r[i], p[r[i]] = i;
    }
    int root = -1;
    rep(i, n) {
        if(p[i] == -1) {
    ...
    assert(root != -1);
    map<int, int> mp;
    auto dfs = [&](auto&&dfs, int v, int ma, int mi) -> void {
        if(mi <= a[v] and a[v] <= ma) mp[a[v]] = 1;
        if(l[v] != -1) {
            dfs(dfs, l[v], min(ma, a[v]), mi);
        }
        if(r[v] != -1) {
            dfs(dfs, r[v], ma, max(mi, a[v]));
        }
    };
