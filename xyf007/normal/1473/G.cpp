...
int main(int argc, char const* argv[]) {
  ...
  std::vector<mint> f(1, mint::raw(1));
  for (int i = 1; i <= n; i++) {
    std::vector<mint> g(c[i] + c[i - 1] - 1);
    for (int j = -c[i - 1] + 1; j < c[i]; j++)
      if (a[i] >= j && b[i] >= -j)
        g[j + c[i - 1] - 1] = inv[a[i] - j] * inv[b[i] + j];
    auto nf = atcoder::convolution(f, g);
    f.resize(c[i]);
    for (int j = 0; j < c[i]; j++) f[j] = nf[j + c[i - 1] - 1];
    mint x(fac[a[i] + b[i]]);
    for (int j = 0; j < c[i]; j++) f[j] *= x;
  }
  std::cout << std::accumulate(f.begin(), f.end(), mint::raw(0)).val();
  return 0;
}