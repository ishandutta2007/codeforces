...
public:
    void solve() {
        ...
        for (const string& u : t) {
            for (int i : range(u.size() + 1)) {
                string ss = u.substr(0, i);
                if (!id.count(ss)) {
                    p.push_back(ss);
                    id[ss] = sz++;
                }
        ...
        arr2d<int> link(sz, 14);
        arri cost(sz, 0);
        for (int i : range(sz)) {
            for (int j : range(k)) {
                if (p[i].size() >= t[j].size() && p[i].substr(p[i].size() - t[j].size()) == t[j]) {
                    cost[i] += c[j];
                }
            }
            for (int l : range(14)) {
                string cur = p[i] + char(l + 'a');
                while (!id.count(cur)) {
                    cur = cur.substr(1);
                }
                link(i, l) = id[cur];
        ...
        auto pos = createOrder(sz);
        arr<ll> val(sz, 0);
        arr2d<ll> ans(1 << 14, sz, numeric_limits<ll>::min() / 2);
        arr<ll> nval(sz);
        ans(0, 0) = 0;
        int q = 0;
        for (char c : s) {
            if (c == '?') {
                for (int i : range(1 << 14)) {
                    if (bitCount(i) == q) {
                        fill(all(nval), numeric_limits<ll>::min() / 2);
                        for (int j : range(sz)) {
                            maxim(nval[pos[j]], ans(i, j) + val[j]);
                        }
                        for (int j : range(sz)) {
                            ans(i, j) = nval[j];
                        }
                    ...
                    if (bitCount(i) == q + 1) {
                        for (int j : range(14)) {
                            if ((i >> j & 1)) {
                                int was = i - (1 << j);
                                for (int l : range(sz)) {
                                    maxim(ans(i, link(l, j)), ans(was, l) + cost[link(l, j)]);
                                }
                            }
                        }
                    }
