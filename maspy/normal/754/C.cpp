...
void solve() {
  LL(N);
  VEC(string, NAME, N);
  auto to_i = [&](string x) -> ll {
    FOR(i, N) if (x == NAME[i]) return i;
    return -1;
  };
  LL(M);
  vc<string> mes(M);
  getline(cin, mes[0]);
  FOR(i, M) { getline(cin, mes[i]); }
  vi FRM(M);
  vc<vi> TO(M);
  FOR(i, M) {
    string S = mes[i];
  ...
  FOR(m, M) {
    vi ok(N, 1);
    FOR(i, N) {
      if (FRM[m] != -1 && i != FRM[m]) ok[i] = 0;
    }
    for (auto&& t: TO[m]) ok[t] = 0;
    FOR(i, N) if (ok[i]) {
      if (m == 0) {
        DP[m][i] = 1;
        continue;
      }
      FOR(p, N) if (DP[m - 1][p] && p != i) {
        DP[m][i] = 1;
        PAR[m][i] = p;
      }
    }
  ...
  vi A(M, -1);
  FOR(i, N) if (DP[M - 1][i]) A[M - 1] = i;
  if (A[M - 1] == -1) return print("Impossible");
  ll m = M - 1;
  while (m) {
    A[m - 1] = PAR[m][A[m]];
    --m;
  ...
  FOR(m, M) {
    string S = mes[m];
    auto tokens = split(S, ':');
    string pref = tokens[0], suff = tokens[1];
    if (FRM[m] == -1) { pref = NAME[A[m]]; }
    print(pref + ":" + suff);
  }
}
