#line 1 "codeforces-workspace\\contest\\1556\\b\\b.cpp"
...
#line 16 "codeforces-workspace\\contest\\1556\\b\\b.cpp"
//*/
...
void main() {
  ...
  input n;
  vector<i32> pos, odd, even;
  for (auto i : range(n)) {
    if (input() % 2) {
      pos.emplace_back(i);
    }
    (i % 2 ? odd : even).emplace_back(i);
  }

  u64 ans = -1;
  for (auto v : {odd, even}) {
    if (v.size() != pos.size()) continue;
    u64 sum = 0;
    for (auto &&[x, y] : zip(v, pos)) {
      sum += abs(x - y);
    }
    chle(ans, sum);
  }
