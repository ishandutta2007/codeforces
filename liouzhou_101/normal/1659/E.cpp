...
void solve()
{
	...
	const int K = 30;
	vector<data_structure::DisjointSet> s(K, data_structure::DisjointSet(n));
	...
	auto check0 = [&](int x, int y)
	{
		for (int k = 0; k < K; ++k)
		{
			if (s[k].Find(x) == s[k].Find(y)) return true;
		}
		return false;
	};
