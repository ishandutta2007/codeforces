...
int n, m, sumin, sumout, a[N], b[N], res[N][N];

void Hollwo_Pelw() {
	cin >> n >> m;
	for (int i = 1; i <= n; i++)
		cin >> a[i], sumin += a[i], dinic.add_edge(0, i, a[i]);
	for (int i = 1; i <= n; i++)
		cin >> b[i], sumout += b[i], dinic.add_edge(i + 200, 500, b[i]);
	for (int i = 1; i <= n; i++)
		dinic.add_edge(i, i + 200, 200);
	for (int i = 1, u, v; i <= m; i++)
		cin >> u >> v, dinic.add_edge(u, 200 + v, 200), dinic.add_edge(v, 200 + u, 200);
	long long f = dinic.maxflow(0, 500);
	if (f == sumin && f == sumout) {
		cout << "YES\n";
		for (int i = 1; i <= n; i++) {
			for (auto j : dinic.adj[i])
				if (200 <= dinic.edges[j].v && dinic.edges[j].v <= 400)
					res[i][dinic.edges[j].v - 200] = dinic.edges[j].f;
		}
		for (int i = 1; i <= n; i++)
			for (int j = 1; j <= n; j++)
				cout << res[i][j] << " \n"[j == n];
