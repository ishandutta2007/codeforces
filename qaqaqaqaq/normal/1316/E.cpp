...
struct ppl{
    ll p[7],u;
    friend bool operator<(ppl x,ppl y){
        return x.u>y.u;
    }
}st[maxn];

int n,p,k;
ll dp[maxn][1<<7],ans=0;
int main(){
    ios::sync_with_stdio(false);
    cin.tie(0), cout.tie(0);
    cin>>n>>p>>k;
    rep(i,n) cin>>st[i].u;
    rep(i,n) rep(j,p) cin>>st[i].p[j];
    sort(st,st+n);
    dp[0][0]=0;
    for (int j=1;j<(1<<p);++j) dp[0][j]=-1e18;
    for (int i=1;i<=n;++i){
        for (int j=0;j<(1<<p);++j){
            if (__builtin_popcount(j)+k>=i) dp[i][j]=dp[i-1][j]+st[i-1].u;
                else dp[i][j]=dp[i-1][j];
            for (int r=0;r<p;++r){
                if ((j>>r&1)==0) continue;
                dp[i][j]=max(dp[i][j],dp[i-1][j^(1<<r)]+st[i-1].p[r]);
            }
        }
    }
    cout<<dp[n][(1<<p)-1];
