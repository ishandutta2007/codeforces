...
int doit(vector<int> v,int q){
  sort(all(v));
  reverse(all(v));
  while(sz(v)&&!v.back()) v.pop_back();
  auto ke=mp(v,q);
  if(M.count(ke)) return M[ke];
  forv(i,v){
    auto vv=v;
    if(v[i]==q && (!i||v[i-1]!=q)) continue;
    --vv[i];
    if(!doit(vv,vv[i])) return M[ke]=1;
  }
  return M[ke]=0;
}
...
void doit2(vector<int>& v){
  int t=doit(v,0);
  if(t!=solve(v)) dbg(v,t);
  
  if(sz(v)>=6) return;
  v.pb(1);
  for(;v.back()<=10&&(sz(v)==1||v.back()<=v[sz(v)-2]);++v.back()) doit2(v);
  v.pop_back();
}
