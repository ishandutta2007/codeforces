...
int main()
{
	...
	for (int i = 1; i < N; ++i)
	{
		for (int j = 0; j < i; ++j)
			CC[i][j] = CC[i - 1][j];
		for (int j = 0; j < i; ++j)
			CC[i][j + 1] = sum(CC[i][j + 1], CC[i - 1][j], M);
	}
	...
	for (int i = 1; i < N; ++i)
	{
		...
		NP[i][0] = 1;
		for (int j = 1; j < K; ++j)
			NP[i][j] = prod(NP[i][j - 1], rev, M);
	}

	ll snl = sqrtl(static_cast<long double>(n));
	while (snl * snl < n)
		++snl;
	while (snl * snl > n)
		--snl;
	...
	for (int i = 0; i < pw; ++i)
	{
		...
		{
			int addend = NP[j + 1][k];
			addend = prod(addend, curpow, M);
			addend = prod(addend, CC[power][j], M);
			curans = sum(curans, addend, M);
			curpow = prod(curpow, pm1, M);
		}
