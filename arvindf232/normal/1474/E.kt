...
fun main(){
    solve.cases{
        ...
        var t = 0L
        fun flex(a:Int, b:Int){
            assert(perm[a] == a)
            ret.add(Pair(a,b))
            perm[a] = perm[b].also { perm[b] = perm[a] }
            val dv = (b-a).absoluteValue
            t += dv.toLong() * dv
        }
        flex(0,n-1)
        for(i in 1..minOf(cut,n-2)){
            flex(i,n-1)
        }
        for(i in cut+1..n-2){
            flex(i,0)
        ...
        put(t)
        put(perm.map{it+1}.conca())
        put(ret.size)
        for((a, b) in ret.reversed()) {
            put("${a+1} ${b+1}")
        }
