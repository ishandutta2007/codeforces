...
vc<int> lens[4];
void DivideAndCategory(){
	auto Addi = [&]( int x ){
		if(!x) return;
		if( x < b ) lens[0].Eb(x);
		elif( b <= x and x < a ) lens[1].Eb(x);
		elif( a <= x and x < 2*b ) lens[2].Eb(x);
		else lens[3].Eb(x);
	};
	int len = 0;
	for( auto c : s ){
...
bool Solve(){
	if(!lens[1].empty()) return 0;
	if( lens[3].size() >= 2 ) return 0;
	if(!lens[3].empty()){
		int len = lens[3][0];
		Forx(l,0,len){
			int r = len-a-l;
			if( r < 0 ) continue;
			if( b <= l and l < a ) continue;
			if( b <= r and r < a ) continue;
			if( l >= 2*b or r >= 2*b ) continue;
			int new_cnt2 = lens[2].size() + ( a <= l ) + ( a <= r );
			if( new_cnt2%2 == 0 )
				return 1;
		}
		return 0;
	}
	return lens[2].size()%2 == 1;
}
