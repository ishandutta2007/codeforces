...
fun main(){
	solve.rand {
		...
		for(i in 0 until n){
			if(A[i] <= k){
				ret[A[i]] = A.take(i).firstOrNull { it > k } ?: n+1
			}else{
				ret[A[i]] = A.take(i).firstOrNull { it <= k } ?: 0
			}
		}
		k dei A
		share(n)
		share(ret.drop(1).conca())
	}
//	solve.userand()
    solve.cases{
		val n = getint
		val L = intArrayOf(0) +  getline(n)


		val k = (1..n).firstOrNull() { L[it] < it } ?: n+1 

		val indegree = IntArray(n+2)
		val subs = Array(n+2){mint}
		...
		while(ret.size < n+2){
			...
			assert(stuff.isNotEmpty())
			val end = stuff.maxByOrNull { indegree[it] }!!
			val next = mint
			for(c in stuff){
				if(c == end) continue
				ret.add(c)
