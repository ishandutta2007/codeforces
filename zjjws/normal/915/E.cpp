...
{
    if(l==r)return;int mid=l+r>>1;mis[i]=c[mid+1]-c[mid]-1;
    make_tree(l,mid,ls);make_tree(mid+1,r,rs);return;
}
int ql,qr,qk;
inline void up(int i){gyq[i]=gyq[ls]+gyq[rs]+(mit[i]?mis[i]:0);return;}
inline void addtag(int l,int r,int i,int x){tag[i]=x;mit[i]=(x==2);gyq[i]=(x==2?(c[r]-c[l]+1):0);return;}
inline void down(int l,int r,int mid,int i){if(tag[i])addtag(l,mid,ls,tag[i]),addtag(mid+1,r,rs,tag[i]),tag[i]=0;return;}
inline void cover(int l,int r,int i)
{
    if(ql<=l&&r<=qr){addtag(l,r,i,qk);return;}
    int mid=l+r>>1;if(ql<=mid&&mid<qr)mit[i]=(qk==2);down(l,r,mid,i);
    if(mid>=ql)cover(l,mid,ls);if(mid<qr)cover(mid+1,r,rs);up(i);return;
}

int sum;
inline void work(int l,int r,int k){ql=l;qr=r;qk=k;cover(1,n,1);printf("%d\n",gyq[1]+sum);return;}
int L[N];
int R[N];
int k[N];
inline int a(int x){return x<=q?L[x]:R[x-q];}
inline void A(int x){(x<=q)?(L[x]=n):(R[x-q]=n);return;}
inline bool myru_b(int x,int y){return a(x)<a(y);}
int main()
{
    rin>>m>>q;for(int i=1;i<=q;i++)rin>>L[i]>>R[i]>>k[i],b[i]=i,b[i+q]=i+q;
    sort(b+1,b+(q<<1)+1,myru_b);for(int i=1,j;i<=(q<<1);i=j){c[++n]=a(b[i]);for(j=i;j<=(q<<1)&&a(b[j])==c[n];j++)A(b[j]);}
    sum=c[1]-1+m-c[n];make_tree(1,n,++gyq_cutt);addtag(1,n,1,2);for(int i=1;i<=q;i++)work(L[i],R[i],k[i]);
    return 0;
}