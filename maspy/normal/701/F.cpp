...
template <typename Graph>
vc<int> find_path(Graph& G, int s, int t, int ng = -1) {
  // eid sequence
  ll N = G.N;
  ll l = 0, r = 0;
  vc<int> que(N), dist(N, -1), par(N, -1);
  que[r++] = s;
  dist[s] = 0;
  while (l < r) {
    auto v = que[l++];
...
void solve() {
  ...
  for (auto&& i: P) {
    ll cost_i = G.edges[i].cost;
    auto Q = find_path(G, s, t, i);
    if (len(Q) == 0) {
      if (chmin(ANS_val, cost_i)) ANS = {i};
      continue;
    }
    Graph H(N);
    FOR(j, M) if (i != j) {
    ...
    H.prepare();
    auto [n_comp, comp] = two_edge_component(H);
    for (auto&& j: Q) {
      auto& e = G.edges[j];
      if (comp[e.frm] == comp[e.to]) continue;
      ll cost = cost_i + e.cost;
      if (chmin(ANS_val, cost)) ANS = {i, j};
    }
  }
  out();
}
