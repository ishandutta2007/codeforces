...
int main() {
	...
	vector<vector<int>> g2(m), rg2(m);
	for (int i = 0; i < n; i++) {
		int u = i;
		int v = p[i];
		
		vector<int> path_ul, path_vl;
		while (!isAncestor(u, v)) {
			assert(pid[u] != -1);
			path_ul.emplace_back(pid[u]);
			u = par[u];
		}
		while (u != v) {
			assert(pid[v] != -1);
			path_vl.emplace_back(pid[v]);
			v = par[v];
		}

		vector<int> path_lv = path_vl;
		reverse(path_lv.begin(), path_lv.end());

		vector<int> path;
		path.insert(path.end(), path_ul.begin(), path_ul.end());
		path.insert(path.end(), path_lv.begin(), path_lv.end());

		for (int i = 1; i < (int)path.size(); i++) {
			g2[path[i - 1]].emplace_back(path[i]);
			rg2[path[i]].emplace_back(path[i - 1]);
		}
	}
