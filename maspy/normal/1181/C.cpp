...
void solve() {
  ...
  */
  vv(int, dpr, H, W, 1);
  FOR(x, H) FOR_R(y, W - 1) {
    if (G[x][y] == G[x][y + 1]) dpr[x][y] = dpr[x][y + 1] + 1;
  }
  ...
  FOR(x, 1, H - 1) FOR(y, W) {
    ...
    ll nx = x;
    while (nx < H && G[x][y] == G[nx][y]) ++nx;
    if (nx == H) continue;
    ll s = nx - x;
    bool ok = 1;
    FOR(i, x - s, x) if (i < 0 || G[i][y] != G[x - 1][y]) ok = 0;
    FOR(i, nx, nx + s) if (i >= H || G[i][y] != G[nx][y]) ok = 0;
    if (!ok) continue;

    ll m = W;
    FOR(i, x - s, nx + s) chmin(m, dpr[i][y]);
    ANS += m;
  }
  print(ANS);
}
