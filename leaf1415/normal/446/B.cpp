...
ll h, w, k, p;
ll a[1005][1005];
ll hsum[1005], wsum[1005];
vector<ll> wvec, hvec;

void calc(ll hsum[], vector<ll> &vec, ll h, ll w)
{
	priority_queue<ll> Q;
	rep(i, 1, h) Q.push(hsum[i]);
...
int main(void)
{
	...
	ll ans = -inf;
	rep(i, 0, k){
		chmax(ans, hvec[i] + wvec[k-i] - i*(k-i)*p);
	}
	outl(ans);
