...
namespace MAIN{
    int n;
    int a[N],b[N],pos[N],nxt[N],num[N];
    inline void MAIN(const res &Case){
        n=read();
        for(res i=1;i<=n;i++)a[i]=read(),num[i]=1;
        for(res i=1;i<=n;i++)b[i]=read();
        for(res i=1;i<=n;i++)pos[i]=n+1;
        for(res i=n;i>=1;i--)nxt[i]=pos[a[i]],pos[a[i]]=i;
        for(res i=1,j=1;i<=n&&j<=n;){
            if(a[j]==b[i]){
                i++,num[j]--;
                if(!num[j])j++;
            }
            else {
                if(nxt[j]<=n)num[nxt[j]]+=num[j],j++;
                else {puts("NO");return;}
            }
        }
        puts("YES");
