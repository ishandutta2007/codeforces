...
int n;
vc<int> words_c_char[30];

int dsu[MAXN];
void Init(){ For(i,n) dsu[i] = i; }
int Findfa( int pos ){ return dsu[pos] == pos ? pos : dsu[pos] = Findfa(dsu[pos]); }
il void Unite( int a , int b ){ dsu[Findfa(a)] = Findfa(b); }
...
int main(){
	...
	For(i,n){
		...
		for( auto c : s )
			words_c_char[c-'a'+1].Eb(i);
	}

	Init();
	Forx(i,1,26){
		int last = -1;
		for( auto x : words_c_char[i] ){
			if( last != -1 ) Unite(last,x);
			last = x;
		}
	}
