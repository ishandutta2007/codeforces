...
int main() {
    ...
    for (auto [v, c] : RunLengthEncoder(s)) {
        rle.emplace_back(v, c);
    }
    if (rle.size() % 2 == 0) {
        print(0);
        return 0;
    }
    const int l = rle.size();
    const int m = rle.size() / 2;
    bool ok = rle[m].second >= 2;
    rep(i, m) {
        ok &= rle[i].first == rle[l - i - 1].first;
        ok &= rle[i].second + rle[l - i - 1].second >= 3;
    }
    if (ok) {
        print(rle[m].second + 1);
    } else {
        print(0);
    }
