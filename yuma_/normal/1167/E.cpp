...
int main() {
	...
	vector<int>rs(X+2);
	{
		...
		bool ok=true;
		for(int i=X;i>=1;--i){
			for(auto place:places[i]){
				auto it=aset.insert(make_pair(place,i)).first;
				if(it==aset.begin() || prev(it)->second<=it->second){
				}else{
					ok=false;
				}
				if(next(it)==aset.end()||next(it)->second>=it->second){
				}else{
					ok=false;
				}
			}
			if(ok){
				if(aset.empty())ls[i]=N;
				else ls[i]=aset.begin()->first;
			}else{
				ls[i]=-1;
			}
		}
	...
	ls[0]=ls[1];
	{
		...
		for(int i=1;i<=X;++i){
			...
			if(ok){
				if(aset.empty())rs[i]=-1;
				else rs[i]=prev(aset.end())->first;
			}else{
				rs[i]=N+1;
			}
		}
	...
	ll answer=0;
	for(int l=0;l<=X-1;++l){
		auto it=upper_bound(ls.begin()+2+l,ls.end(),rs[l]);
		int nans=ls.end()-it;
		answer+=nans;
	}
	cout<<answer<<endl;
