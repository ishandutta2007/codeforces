...
int main() {
    ...
    REP(i, m) cin >> a[i];
    REP(i, m) a[i]--;
    segment_tree<int> seg(n + m, [] (int a, int b) { return a + b; }, 0);
    vector<P> ans(n);
    REP(i, n) ans[i].first = ans[i].second = i + 1;
    REP(i, m) ans[a[i]].first = 1;
    vector<int> pos(n);
    REP(i, n) pos[i] = n - 1 - i;
    REP(i, n) seg.update(i, 1);
    REP(i, m) {
        ans[a[i]].second = max(ans[a[i]].second, (ll)seg.query(pos[a[i]], n + m));
        seg.update(pos[a[i]], 0);
        seg.update(n + i, 1);
        pos[a[i]] = n + i;
    }
    REP(i, n) {
        ans[i].second = max(ans[i].second, (ll)seg.query(pos[i], n + m));
    }
    REP(i, n) {
