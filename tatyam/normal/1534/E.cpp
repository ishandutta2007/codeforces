...
int main(){
    LL(n,k);
    if(n%2==1&&k%2==0)return out(-1);
    if(n==k)answer(query(iota(n)));
    ll at=n,X=0;
    while(at){
        vector<ll>idx;
        if(at&1||at>k*2){
            rep(_,k)idx.push_back(--at);
            X^=query(idx);
            continue;
        }
        if(at>(n-k)*2&&at>=k&&(at-k)%2==0){
            rep(_,k)idx.push_back(--at);
            X^=query(idx);
            continue;
        }
        while(at>(n-k)*2){
            rep(2){
                ll at2=at-(n-k);
                rep(n)if(i<at2||at<=i)idx.push_back(i);
                X^=query(idx);
                at=at2;
                idx.clear();
            }
        ...
        rep(2){
            ll at2=at-d;
            rep(k+d)if(i<at2||at<=i)idx.push_back(i);
            X^=query(idx);
            at=at2;
            idx.clear();
        }
