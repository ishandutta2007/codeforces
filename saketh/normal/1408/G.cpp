...
struct union_find {
    struct node {
        int par, rnk, size, edges;
        vmn ways;
        node(int id = 0) : par(id), rnk(0), size(1), edges(0), ways(vmn{ 0, 1 }) {}
    };
    ...
    bool unio(int a, int b) {
        ...
        if (a == b) {
            uf[a].edges++;
            if (uf[a].edges == uf[a].size * (uf[a].size - 1) / 2) {
                uf[a].ways[1] = 1;
            }
            return false;
        }
        ...
        vmn nways(uf[a].size + uf[b].size + 1);
        FOR (ga, 1, uf[a].size + 1)
            FOR (gb, 1, uf[b].size + 1)
                nways[ga + gb] += uf[a].ways[ga] * uf[b].ways[gb];

        uf[b].par = a;
        uf[a].size += uf[b].size;
        uf[a].edges += uf[b].edges;
        uf[a].edges++;
