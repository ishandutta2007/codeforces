...
{
    rd(n);cin>>s;s=" "+s;
    for (int i=1;i<=n;i++) {pre[0][i]=pre[0][i-1];pre[1][i]=pre[1][i-1];if (s[i]!='?') pre[(s[i]-'0')^1][i]=i;}
	for (int i=1;i<=n;i++)
    {
        int p=0,ans=0;
        while (p<=n-i) if (pre[0][p+i]==pre[0][p] || pre[1][p+i]==pre[1][p]) p+=i,ans++;
        else p=pre[pre[0][p+i]>pre[1][p+i]][p+i];
        cout<<ans<<" ";
    }
}
/*
