...
int last = 0;
int pr[5050 * 2];
int findIndexOf(const string& s, const string& t) {
    auto charAt = [&](int pos) {
        if (pos == s.size()) return '#';
        if (pos < s.size()) return s[pos];
        return t[pos - s.size() - 1];
    };
    for (int i = 1; i <= s.size() + t.size(); ++i) {
        ...
        if (s[j] == c) ++j;
        pr[i] = j;
        if (pr[i] >= s.size()) return i - (int)s.size() * 2;
    }
    return -1;
}
int solve() {
    ...
    FI(n) {
        fe[i] = (int)s[i].size() - 1;
        while (s[i].size() && s[i].back() == t[i].back()) {
            s[i].pop_back();
            t[i].pop_back();
            fe[i]--;
        }
    ...
    while (true) {
        ...
        if (fb[idx] == 0) break;
        --fb[idx];
        char sc = ss[idx][fb[idx]];
        char tc = tt[idx][fb[idx]];
        for (int i = 0; i < n; ++i) {
            if (i == idx) continue;
            if (s[i].empty()) continue;
            if (fb[i] == 0) {
    ...
    reverse(all(rt));
    while (true) {
        if (fe[idx] == ss[idx].size() - 1) break;
        ++fe[idx];
        bool ok = true;
        char sc = ss[idx][fe[idx]];
        char tc = tt[idx][fe[idx]];
        for (int i = 0; i < n; ++i) {
            if (i == idx) continue;
            if (s[i].empty()) continue;
            if (fe[i] == ss[i].size() - 1) {
    ...
    FI(n) {
        int position = findIndexOf(rs, ss[i]);
        if (s[i].empty() && position != -1) {
            cout << "NO" << endl;
            return 0;
        }
        if (s[i].empty()) continue;
