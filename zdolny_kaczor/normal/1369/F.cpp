...
#endif
//#STAY AT HOME
bool win(ll s, ll e, bool win) {
	debug(imie(s), imie(e), imie(win));
	e++;
	bool odd = win, even = win;
	while (e > s) {
		ll e2 = ceil_div(e, 2);
		bool odd2 = false, even2 = false;
		if (!even) odd2 = even2 = true;
		else if (e % 2 == 0) even2 = even, odd2 = !even;
		else odd2 = odd, even2 = !odd;
		e = e2;
		odd = odd2;
		even = even2;
	}
...
int main(int argc, char ** argv) {
	// REP(c, 2) if(0) {
		// for (int e = 1; e <= 300; ++e) {
			// vector <int> ans = {0};
			// for (int s = 1; s <= e; ++s) ans.push_back(win(s, e, c));
			// auto get = [&](int x) {return x > e ? c : ans[x];};
			// for (int s = 1; s <= e; ++s) {
				// bool should = !get(s + 1) || !get(s * 2);
				// assert(should == ans[s]);
			// }
		// }
	// }
	// REP(_, 1000) {
		// const ll M = 1e18;
		// ll e = rand() % M + 1, s = rand() % e + 1;
		// bool c = rand() % 2;
		// assert(win(s, e, c) == !win(s + 1, e, c) || !win(s * 2, e, c));
	// }
	// return 0;
	int t = 1;
	int fl = 1, fw = 0;
	...
	REP(_, t) {
		ll s, e;
		scanf("%lld%lld", &s, &e);
		bool t1 = win(s, e, true);
		bool t2 = win(s, e, false);
		bool fw2 = t1 ? fl : fw;
		bool fl2 = t2 ? fl : fw;
		fw = fw2;
		fl = fl2;
