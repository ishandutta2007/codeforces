...
inline void solve()
{
	n=read(),factor.clear(),c=n,totd=0;
	...
	factor.pb(n),mp.clear();
	if(totd==1)
	{
		for(register int i:factor)
		{
			printf("%d ",i);
		}
		return (void)puts("\n0");
	}
	if(totd==2&&prf[1]*prf[2]==n)
	{
		return (void)printf("%d %d %d\n%d\n",prf[1],prf[2],n,1);
	}
	mp[n]=1;
	for(register int i=2;i<=totd;i++)
	{
		mp[prf[i-1]*prf[i]]=1;
	}
	for(register int i=1;i<=totd;i++)
	{
		for(register int f:factor)
		{
			if(f%prf[i]==0&&!mp[f])
			{
				printf("%d ",f),mp[f]=1;
		...
		printf("%d ",i!=totd?prf[i+1]*prf[i]:n);
	}
	puts("\n0");
}
int main()
