...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	ll re=0;
	{
		cauto sum=accumulate(all(as),0ll)+accumulate(all(bs),0ll)+accumulate(all(cs),0ll);
		vector vs{*min_element(all(as)),
							*min_element(all(bs)),
							*min_element(all(cs))};
		sort(all(vs));
		cauto t=vs[0]+vs[1];
	...
	{
		vector vs{accumulate(all(as),0ll),accumulate(all(bs),0ll),accumulate(all(cs),0ll)};
		sort(all(vs));
		MX(re,vs[2]+vs[1]-vs[0]);
	}
	cout<<re NL;
  return 0;
}