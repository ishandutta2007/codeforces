...
int n;
int v,h;
tuple<int,int,int> vr[5050],ho[5050];
i64 ans;
...
signed main() {
    ...
    sort(vr,vr+v);
    rep(i,v) {
        Segtree<int,int> seg(10101,0,plus<int>(),[&](int &x,int y){x=y;});
        int s,t,x; tie(x,s,t)=vr[i];
        minheap<pii> que;
        rep(j,h) {
            int b,a,c; tie(b,a,c)=ho[j];
            if(s<=b && t>=b && c>=x) {
                que.emplace(a,b);
            }
        }
        rep(k,i) {
            ...
            tie(xx,ss,tt)=vr[k];
            if(xx==x) break;
            while(!que.empty() && que.top().fir<=xx) {
                int y=que.top().sec;
                que.pop();
                seg.update(y,1);
            }
            int tmp=seg.query(ss,tt+1);
            ans+=tmp*(tmp-1)/2;
        }
    }
    cout<<ans<<endl;
