...
int main() {
	qread(n, k);
	dp[1][2][0] = dp[1][2][1] = dp[1][1][2] = dp[1][1][3] = 1;
	rep(i, 2, n) {
		rep(j, 1, k) {
			int y = max(0, j - 2);
			dp[i][j][0] = (dp[i - 1][j][0] + dp[i - 1][y][1] + dp[i - 1][j - 1][2] + dp[i - 1][j - 1][3]) % p;
			dp[i][j][1] = (dp[i - 1][j][1] + dp[i - 1][y][0] + dp[i - 1][j - 1][3] + dp[i - 1][j - 1][2]) % p;
			dp[i][j][2] = (dp[i - 1][j][0] + dp[i - 1][j][1] + dp[i - 1][j][2] + dp[i - 1][j - 1][3]) % p;
			dp[i][j][3] = (dp[i - 1][j][0] + dp[i - 1][j][1] + dp[i - 1][j - 1][2] + dp[i - 1][j][3]) % p;
		}
	}
	unsigned ans = 0;
	rep(i, 0, 3) (ans += dp[n][k][i]) %= p;
	cout << ans << endl;
 	return 0;
}