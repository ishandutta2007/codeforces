...
main()
{
	...
	for(;T--;)
	{
		...
		for(int i=M;i--;)
		{
			int id=P.lower_bound(y[i]);
			int l=prv[id],r=nxt[id];
			S.insert(r);
			ch.push_back(make_pair(id,make_pair(l,r)));
			nxt[l]=r;
			prv[r]=l;
			P.add(id,-1);
		}
		printf("%d\n",C.H(N+1,N-S.size()-1).val());
		for(int i=ch.size();i--;)
		{
			int id=ch[i].first;
			int l=ch[i].second.first,r=ch[i].second.second;
			P.add(id,1);
			nxt[l]=id;
			prv[r]=id;
		}
