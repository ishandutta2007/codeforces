...
int main(){
	...
	int n;
	cin >> n >> mod;
	vector<vector<Zp>> t(n + 1, vector<Zp>(n + 1, 1));
	for(auto x = 1; x <= n; ++ x){
		for(auto y = 0; y <= x; ++ y){
			for(auto z = x; z > x - y; -- z){
				t[x][y] *= z;
			}
		}
	}
	const int offset = 500 * 500;
	vector<Zp> dp(offset << 1);
	dp[0 + offset] = 1;
	Zp res = 0;
	for(auto i = n - 1; i >= 0; -- i){
		vector<Zp> pref(offset << 1 | 1);
		for(auto i = 0; i < offset << 1; ++ i){
			pref[i + 1] = pref[i] + dp[i];
		}
		for(auto x = n - i - 1; x >= -(n - i - 1); -- x){
			int cnt = min(n - i - x, x + (n - i));
			if(x > 0){
				res += cnt * pref[-x + offset] * t[n][i];
			}
		}
		int len = n - i;
		vector<Zp> dp_next(offset << 1);
		for(auto xoffset = len - 1; xoffset + len + 1 < offset << 1; ++ xoffset){
			dp_next[xoffset - (len - 1)] += dp[xoffset];
			dp_next[xoffset + 1] -= 2 * dp[xoffset];
			dp_next[xoffset + len + 1] += dp[xoffset];
		}
		swap(dp, dp_next);
		for(auto x = 1; x < offset << 1; ++ x){
			dp[x] += dp[x - 1];
		}
		for(auto x = 1; x < offset << 1; ++ x){
			dp[x] += dp[x - 1];
