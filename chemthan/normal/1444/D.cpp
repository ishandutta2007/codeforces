...
void chemthan() {
    ...
    while (test--) {
        ...
        sort(all(a));
        sort(all(b));
        vector<BS> vx(n + 1, BS(1e6 + 5));
        vector<BS> vy(m + 1, BS(1e6 + 5));
        vx[0].set(0, 1);
        vy[0].set(0, 1);
        FOR(i, 0, n) {
            vx[i + 1] = vx[i] | (vx[i] << a[i]);
        }
        FOR(i, 0, m) {
            vy[i + 1] = vy[i] | (vy[i] << b[i]);
        }
        ...
        if (!vx[n].get(suma / 2) || !vy[m].get(sumb / 2)) {
            cout << "NO\n";
            continue;
        }
        int sa = suma / 2, sb = sumb / 2;
        vi vx1, vx2, vy1, vy2;
        FORd(i, n, 0) {
            if (a[i] <= sa && vx[i].get(sa - a[i])) {
                sa -= a[i];
                vx1.pb(a[i]);
            }
            else {
        ...
        sort(all(vx1)), sort(all(vx2)), sort(all(vy1)), sort(all(vy2));
        reverse(all(vx1)), reverse(all(vx2));
        if (sz(vx1) < sz(vx2)) {
            swap(vx1, vx2);
        }
        if (sz(vy2) < sz(vy1)) {
