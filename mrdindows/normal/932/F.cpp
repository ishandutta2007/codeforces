...
Node nodes[32 * 10101 * 20];
Node* lastn = nodes;

const int T = 1.01e5;
void add_line(point nw, Node*& cur, int l = -T, int r = T) {
    if (!cur) cur = lastn++;
    int m = l + (r - l) / 2;
    bool lef = f(cur->line, l) < f(nw, l);
    bool mid = f(cur->line, m) < f(nw, m);
    if (mid) swap(cur->line, nw);
    if (r - l == 1)
        return;
    if (lef != mid) {
        add_line(nw, cur->lp, l, m);
    } else {
        add_line(nw, cur->rp, m, r);
    }
}

ll get(int x, Node* cur, int l = -T, int r = T) {
    if (!cur) return -LLINF;
    auto res = f(cur->line, x);
    if (r - l > 1) {
        int m = l + (r - l) / 2;
        res = max(res, x < m ? get(x, cur->lp, l, m) : get(x, cur->rp, m, r));
    }
    return res;
}
...
void go(int cur, int p = -1) {
    if (v[cur].size() == 1 && v[cur][0] == p) {
        ans[cur] = 0;
    } else {
        for (int to  :v[cur]) {
            if (to != p) {
                go(to, cur);
                if (c[cur].lines.size() < c[to].lines.size()) {
                    swap(c[cur], c[to]);
                }
                for (auto line : c[to].lines) {
                    add_line(line, c[cur].root);
                    c[cur].lines.push_back(line);
                }
                c[to].lines.clear();
            }
        }
        ans[cur] = -get(a[cur], c[cur].root);
    }
    point line(-b[cur], -ans[cur]);
    add_line(line, c[cur].root);
    c[cur].lines.push_back(line);
}
int solve() {
