...
public class F2 {
	...
	void solve()
	{
		...
		RollingHashFactory rhf = new RollingHashFactory(2, n+3, new Random());
		RollingHash rh = new RollingHash(n+1, rhf);
		for(int i = 0;i < n;i++)rh.add(ws[i]);
		...
		for(int i = 0;i < n;i++){
			int lensum = -1;
			for(int j = i;j < n;j++){
				long h = rh.queryTwin(i, j+1);
				lensum += lens[j]-1 + 1;
				
				int len = j-i+1;
				int hit = 0;
				for(int k = 0;k < n;){
					if(k+len <= n && rh.queryTwin(k, k+len) == h){
						hit++;
						k += len;
					}else{
						k++;
