...
struct solver
{
    ...
    vector<vector<pair<int,i64>>> dp;
    const pair<int,i64> ide={0,LONG_LONG_MIN/2};

    solver() : n(read<int>()),m(read<int>()),w(n),tr(n),dp(n)
    {
        // input
        {
            cin>>w;
        ...
        const int root{0};
        assert(n==dfs(root));
        const auto [c,s]=dp[root][m-1];
        cout << c+(s>0) << "\n";
    }

    int dfs(const int v, const int p=0)
    {
        int a=1;
        auto &now=dp[v];
        dp[v]={{0,w[v]}};
        ...
        for(int u:tr[v])
        {
            ...
            for(int i=a; i--; )
            {
                const auto [c,s]=now[i];
                for(int j=b; j--; )
                {
                    const auto [co,so]=oth[j];
                    sub_max(nxt[i+j],{c+co,s+so});
                    sub_max(nxt[i+j+1],{c+co+(so>0),s});
                }
            }
