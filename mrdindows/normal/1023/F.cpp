...
uint tr[505050][19];
void draw(int x, int d, uint val) {
    for (int i = 18; i >= 0; --i) {
        if (checkbit(d, i)) {
            tr[x][i] = min(tr[x][i], val);
            x = par[x][i];
        }
    }
}
void dfs(int cur, int p) {
    ...
    for (int i = 18; i > 0; --i) {
        tr[cur][i - 1] = min(tr[cur][i - 1], tr[cur][i]);
        int z = par[cur][i - 1];
        uint& val = tr[z][i - 1];
        val = min(val, tr[cur][i]);
    }
}
tri q[505050];
pii e[505050];
...
//    DBN(sizeof(par) / 1e6);
    ...
    for (int i = 1; i < n; ++i) {
        ...
        if (x > y) swap(x, y);
        pii edge(x, y);
        auto it = lower_bound(e, e + k, edge);
        if (*it != edge) {
            continue;
        }
        uint val = tr[i][0];
