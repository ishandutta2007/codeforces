...
inline void solve() {
    ...
    while (t--) {
        ...
        if (smx - smy > stk) {
            cout << smx - smy - stk << endl;
            for (int i = 0; i < n; ++i) {
                v[i].x -= tk[i];

                cout << vv[i].x - v[i].x << " " << vv[i].y - v[i].y << endl;
            }
        }
        else if (-smx + smy > stk) {
            cout << - smx + smy - stk << endl;
            ...
            // smx - to_kil == smy - stk + to_kil
            ll to_kil = (smx - smy + stk) >> 1;
            assert(to_kil <= smx);
            //vector<pair<int, int>> pr;
            for (int i = 0; i < n; ++i) {
                ll kl = min<ll>(to_kil, tk[i]);
                v[i].x -= kl;
                tk[i] -= kl;
                to_kil -= kl;
                smx -= kl;
