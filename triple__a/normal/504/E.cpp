...
const int hi=38757;
int n,m,q,rev,hsh1[maxn],hsh2[maxn],f[maxn],reh[maxn];

string s;
vector<vi> g;

void dfs(int u,int p,HLD &hld){
    for (auto c:g[u]){
        if (c==p) continue;
        hsh1[c]=(hsh1[u]*hi+s[c-1])%mod;
        hsh2[c]=(hsh2[u]+s[c-1]*f[hld.dep[c]])%mod;
        dfs(c,u,hld);    
    }
}

int calc(int u,int v,int len,HLD &hld){
//    cout<<u<<" "<<v<<" "<<len<<endl;
    if (hld.dep[u]>hld.dep[v]){
        return ((hsh1[u]-hsh1[hld.par[hld.rk[hld.dfn[u]-len+1]]]*f[len])%mod+mod)%mod;
    }
    else{
        int w=hld.rk[hld.dfn[u]+len-1];
        return ((hsh2[w]-hsh2[hld.par[u]])*reh[hld.dep[u]]%mod+mod)%mod;
    }
}
...
signed main(){
    ...
    cin>>n>>s;
    g.resize(n+1);
    f[0]=reh[0]=1, rev=power(hi,mod-2,mod);
    for (int i=1;i<=n+7;++i){
        f[i]=hi*f[i-1]%mod, reh[i]=rev*reh[i-1]%mod;
    }
    for (int i=1;i<n;++i){
        int u,v;
        cin>>u>>v;
    ...
    hsh1[1]=hsh2[1]=s[0];
    dfs(1,0,hld);
//    cout<<calc(5,5,1,hld)<<endl;
    cin>>q;
    auto solve=[&](int a,int b,int c,int d){
        vector<pii> seg1=hld.find_seg(a,b), seg2=hld.find_seg(c,d);
        int i=0, j=0, res=0, l=a, r=c;
        while (i<sz(seg1)&&j<sz(seg2)){
            int la=abs(hld.dep[l]-hld.dep[seg1[i].second])+1, lb=abs(hld.dep[r]-hld.dep[seg2[j].second])+1, mv=min(la,lb);
            int val1=calc(l,seg1[i].second,mv,hld), val2=calc(r,seg2[j].second,mv,hld);
//            cout<<"val1 val2:"<<val1<<" "<<val2<<endl;
            if (val1==val2){
                res+=mv;
//                cout<<"la lb:"<<la<<" "<<lb<<endl;
                if (mv==la) l=seg1[++i].first;
                else {l=hld.rk[hld.dfn[l]+(hld.dep[seg1[i].F]>hld.dep[seg1[i].S]?-mv:mv)];}
                if (mv==lb) r=seg2[++j].first;
                else {r=hld.rk[hld.dfn[r]+((hld.dep[seg2[j].F]>hld.dep[seg2[j].S])?-mv:mv)];}
            }
            else{
                int ll=1,rr=mv,md;
                while (ll<rr){
                    md=(ll+rr)>>1;
                    if (calc(l,seg1[i].second,md,hld)==calc(r,seg2[j].second,md,hld)){
                        ll=md+1;
                    }
                    else{
                        rr=md;
