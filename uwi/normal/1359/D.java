...
public class D {
	...
	void solve()
	{
		...
		SegmentTreeRMQL st = new SegmentTreeRMQL(cum);
		
		long[] fcum = new long[n+1];
		for(int i = 0;i <= n;i++)fcum[i] = -cum[i];
		SegmentTreeRMQL stf = new SegmentTreeRMQL(fcum);
		
		LST lst = new LST(n);
		long ans = 0;
		for(int i = n-1;i >= 0;i--){
			int cur = ai[i][1];
			int pre = lst.prev(cur);
			int nex = lst.next(cur);
			if(nex == -1)nex = n;
			long prefix = cum[cur] - st.minx(pre+1, cur+1);
			long suffix = -stf.minx(cur+1, nex+1) - cum[cur+1];
