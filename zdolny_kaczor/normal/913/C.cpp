...
int main() {
	...
	REP(i, n) {
		minc[i] = cost[i];
		if (i)
			mini(minc[i], minc[i - 1] * 2);
	}
	FOR(i,1, n - 1) {
		mini(minc[i - 1], minc[i]);
	}
	LL ans = 1e18, curr = 0;
	FORD(i, n - 1, 0) {
		debug << name(i) << name(l) << name(curr) << endl;
		mini(ans, ceildiv(l, 1 << i) * minc[i] + curr);
		debug << "mini(ans, " << name(ceildiv(l, 1 << i)) << "*" << arr(minc, i) << "+" << name(curr) << name(ceildiv(l, 1 << i) * minc[i] + curr) << endl;
		curr += l / (1 << i) * (LL)minc[i];
		l %= (1 << i);
	}
	printf("%lld\n", ans);
}