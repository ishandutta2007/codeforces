...
LazySegTree<ll,1<<19> L;
int n,m,co,st[MX],en[MX];
pi w[MX];
vi ins[MX], del[MX], adj[MX];
ll dp[MX];
multiset<int> cur[MX];

void upd(int x, int y) {
	int c = w[x].s; x = st[w[x].f];
	L.upd(x,x,-(sz(cur[x])?*cur[x].begin():INF));
	if (y == 1) cur[x].insert(c);
	else cur[x].erase(cur[x].find(c));
	L.upd(x,x,(sz(cur[x])?*cur[x].begin():INF));
}
...
void dfs(int a, int b) {
	...
	L.upd(st[a],en[a],tot);
	trav(t,adj[a]) if (t != b) L.upd(st[t],en[t],-dp[t]);
	trav(t,ins[a]) upd(t,1);
	trav(t,del[a]) upd(t,-1);
	dp[a] = L.qmin(st[a],en[a]); 
	if (dp[a] >= INF/2) {
		ps(-1);
		exit(0);
	}
