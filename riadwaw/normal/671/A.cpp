...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		int n;
		in >> n;
		vector<pair<double, int>> economy1(n), economy2(n);
		for (int i : range(n)) {
			Point c;
			in >> c.x >> c.y;
			c -= t;
			defaultAns += c.dist() * 2;
			economy1[i] = make_pair(c.dist() - (c - a).dist(), i);
			economy2[i] = make_pair(c.dist() - (c - b).dist(), i);
		}
		...
		double ans = -1e100;
		ans = max(ans, economy1[0].first);
		ans = max(ans, economy2[0].first);

		if (economy1[0].second != economy2[0].second) {
			ans = max(ans, economy1[0].first + economy2[0].first);
		}

		if (n > 1) {
			ans = max(ans, economy1[0].first + economy2[1].first);
			ans = max(ans, economy1[1].first + economy2[0].first);
		}
