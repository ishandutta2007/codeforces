...
void solve() {
  ...
  auto calc_far = [&](ll v) -> ll {
    auto [dist, par] = bfs01(G, v);
    ll f = -1;
    ll w = -1;
    FORIN(p, P) if (chmax(f, dist[p])) w = p;
    return w;
  };

  auto a = calc_far(0);
  auto b = calc_far(a);
  auto dist_1 = bfs01(G, a).fi;
  auto dist_2 = bfs01(G, b).fi;
  vi dist(N);
  ll ANS = 0;
  FOR(v, N) dist[v] = max(dist_1[v], dist_2[v]);
  FOR(v, N) if (dist[v] <= D)++ ANS;
  print(ANS);
}
