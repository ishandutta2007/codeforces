...
struct mint{
	...
	mint operator+(const mint &ope)const{return mint(x) += ope;}
...
struct LazySegTree{
	typedef pair<mint, mint> SEG;
	typedef pair<mint, mint> DELAY;
...
int main(void)
{
	...
	rep(i, 0, n) p[i] = P(s[i], i);
	sort(p, p+n+1);
	rep(i, 0, n) q[p[i].se] = i;
	...
	seg.init();
	rep(i, 2, n){
		ll m = lower_bound(p, p+n+1, P(s[i-1], -inf)) - p;
		mint w = a[i] - a[i-1];
		seg.add(0, m-1, make_pair(-w, mint(s[i-1]) * w));
		seg.add(m, n, make_pair(w, -mint(s[i-1]) * w));
		
		//rep(i, 1, n) cout << seg.query(i, i) << " "; cout << endl;
		
		for(auto id : lvec[i]){
			ll p = q[i-1];
			auto res = seg.query(p, p);
			ans[id] -= res.fi * mint(s[i-1]) + res.se;
		}
		for(auto id : rvec[i]){
			ll p = q[l[id]-1];
			auto res = seg.query(p, p);
			ans[id] += res.fi * mint(s[l[id]-1]) + res.se;
		}
	}
