...
void swap_elements(vi& p, const int& x, const int& y, vpii& ans)
{
	if (x != y)
	{
		ans.eb(min(x, y) + 1, max(x, y) + 1);
		swap(p[x], p[y]);
	}
}

int edge(const int& x, const int& n)
{
	if (x < n / 2)
		return n - 1;
	else
...
void move_element(const int& n, vi& p, const int x, const int y, vpii& ans)
{
	if (x != y)
	{
		int xx = edge(x, n);
		int yy = edge(y, n);
		swap_elements(p, x, xx, ans);
		swap_elements(p, xx, yy, ans);
		swap_elements(p, yy, y, ans);
	}
}
...
int main()
{
	...
	while (x < n)
	{
		...
		{
			move_element(n, p, x, p[x], ans);
			x = 0;
		}
	}
	//for (int i = 0; i < n; ++i)
	//	move_element(n, p, i, p[i], ans);
	swap_elements(p, 0, p[0], ans);
	int m = sz(ans);
	cout << m << '\n';
	for (int i = 0; i < m; ++i)
	...
	for (int i = 0; i < m; ++i)
	{
		if (ans[i].Y - ans[i].X < n / 2)
			return 1;
		swap(q[ans[i].X - 1], q[ans[i].Y - 1]);
	}
	for (int i = 0; i < n; ++i)
	{
		if (q[i] != i + 1)
