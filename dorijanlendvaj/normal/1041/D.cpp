...
int main()
{
	...
	for (int i=0;i<n;++i)
	{
		int b;
		cin>>a[i]>>b;
		pr[i+1]={pr[i].first-pre+a[i],b};
		pre=b;
	}
	for (int i=0;i<n;++i)
	{
		auto x=upper_bound(pr,pr+n+1,mp(h+pr[i+1].first,0));
		--x;
		ma=max(ma,x->second+(h+pr[i+1].first-(x->first))-a[i]);
		//cout<<i<<' '<<(x->second)<<' '<<h<<' '<<pr[i+1].first<<' '<<(x->first)<<endl;
	}
	cout<<ma;
}