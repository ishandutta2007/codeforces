...
/*/-----------------------------Code begins----------------------------------/*/
int seg[10*maxn],lpo[10*maxn],rpo[10*maxn],root[10*maxn],Node=0;
...
#define M (s+e)/2
inline int build(int s,int e){
    if (s==e) return newleaf(s);
    return newParent(build(s,M),build(M+1,e));
}

inline int update(int node,int s,int e,int l,int val){
    if (l>e || l<s) return node;
    if (s==e) return newleaf(val);
    return newParent(update(lpo[node],s,M,l,val),update(rpo[node],M+1,e,l,val));
}


inline int query(int node,int s,int e,int l,int r){
    if (l>e || r<s) return -1;
    if (l<=s && e<=r) return seg[node];
    return max(query(lpo[node],s,M,l,r),query(rpo[node],M+1,e,l,r));
}
mii m;
int a[maxn];
signed main(){
    ...
    int n,q,l,r,x;
    cin>>n>>q;
    loop(i,1,n+1) {cin>>a[i];m[a[i]]=0;}
    root[0]=build(1,n);
    loop(i,1,n+1){
        root[i]=update(root[m[a[i]]],1,n,i,-1);
        m[a[i]]=i;
    ...
    while(q--){
        cin>>l>>r>>x;
        if (m.find(x)==m.end()) {d1(l);continue;}
        d1(query(root[m[x]],1,n,l,r));
    }
