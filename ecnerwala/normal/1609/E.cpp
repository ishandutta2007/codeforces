...
int main() {
	...
	auto merge = [&](range_t a, range_t b) -> range_t {
		range_t r;
		r.cntA = a.cntA + b.cntA;
		r.cntB = a.cntB + b.cntB;
		r.cntC = a.cntC + b.cntC;
		r.cntAB = std::min(a.cntA + b.cntAB, a.cntAB + b.cntB);
		r.cntBC = std::min(a.cntB + b.cntBC, a.cntBC + b.cntC);
		r.cntABC = std::min({a.cntA + b.cntABC, a.cntAB + b.cntBC, a.cntABC + b.cntC});
		return r;
	};
	auto construct = [&](char c) -> range_t {
		range_t r;
		// for those with 2 ranges, this guy can always hide in an off range
		if (c == 'a') {
			r.cntA++;
		} else if (c == 'b') {
			r.cntB++;
	...
	std::vector<range_t> seg(2*N);
	auto update_node = [&](seg_tree::point a) -> void { seg[a] = merge(seg[2*a], seg[2*a+1]); };
	for (int i = 0; i < N; i++) {
		seg[layout.get_point(i)] = construct(S[i]);
	}
