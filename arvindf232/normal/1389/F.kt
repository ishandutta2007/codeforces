...
class item(var l:Int, var r:Int, val t:Int, val id:Int):Comparable<item> {
...
fun main(){
    solve.cases{
        ...
        repeat(n) {
            all.add(item(getint - 1 , getint - 1 , getint - 1  ,it ))
        }
        val allc = mint
        for(a in all) {
        ...
        val cc = coordinateCompression(allc)
        for(a in all) {
            a.l = cc.reverse[a.l]!!
            a.r = cc.reverse[a.r]!!
        }
        all.sort()
        val kn = cc.forward.size
        val segs = List(2){intMaxLazy(kn + 1)}

        var pt = 0
        for(r in 1..kn) {
            while(pt < all.size && all[pt].r == r-1){
                val here = all[pt]
                val c = here.t
                segs[1 - c].rangeAdd(0,here.l, 1)
                pt ++
            }
            val stop0 = segs[0].rangeQueryLazy(0,r)
            val stop1 = segs[1].rangeQueryLazy(0,r)
            val have = maxOf(stop0, stop1)
            for(i in 0 until 2) {
                segs[i].rangeAdd(r,r,have)
            }
        }
        put(maxOf(segs[0].rangeQueryLazy(0,kn), segs[1].rangeQueryLazy(0,kn)))
...
/*
17 mintues for this DP seg tree
 */