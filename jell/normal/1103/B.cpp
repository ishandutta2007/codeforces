#line 1 "codeforces-workspace/contest/1103/b/b.cpp"
...
exponential_search(Index range, Pred pred) {
  ...
  return binary_search(Index(0), step, pred) + 1;
}
...
#line 11 "codeforces-workspace/contest/1103/b/b.cpp"
...
void main() {
  ...
  while (true) {
    ...
    assert(s == "start");
    u32 k = 0;  // bsr
    while (ask((1 << k) - 1, (2 << k) - 1) != "x") {
      ++k;
    }
    u32 fix = (1 << k) - 1;
    u32 cnd = fix + 1;
    for (auto d : rrange(k)) {
      if (ask(fix, cnd | ((1 << d) - 1)) != "x") {
        cnd |= 1 << d;
      }
    }
    ans(cnd);
