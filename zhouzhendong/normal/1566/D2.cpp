...
void solve(){
	...
	for (int i=0;i<n;i++){
		int jl,jr;
		int sp=i*m,ep=sp+m;
		for (jl=ep;jl>sp&&v[jl-1].fi==v[ep-1].fi;jl--);
		for (jr=ep;jr<n*m&&v[jr].fi==v[ep-1].fi;jr++);
		vector <pii> tmp(v.begin()+jr-(ep-jl),v.begin()+jr);
		Fod(j,jr-1,ep)
			v[j]=v[j-(ep-jl)];
		For(j,jl,ep-1)
			v[j]=tmp[j-jl];
...
//		outval(v);
//		outarr(v,sp,ep-1);
		ans+=qq(vector <pii>(v.begin()+sp,v.begin()+ep));
//		outval(ans);
	}
	cout<<ans<<endl;
}
