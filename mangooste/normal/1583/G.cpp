...
struct segtree {
    ...
    mint get(int pos) {
        return get(1, 0, n, pos, 0);
    }

    mint get(int v, int vl, int vr, int pos, mint tot) {
        assert(vl <= pos);
        tot += tree[v];
        if (vr - vl == 1)
            return tot;
        ...
        int vm = (vl + vr) >> 1;
        return pos < vm ? get(v << 1, vl, vm, pos, tot) : get(v << 1 | 1, vm, vr, pos, tot);
    }
