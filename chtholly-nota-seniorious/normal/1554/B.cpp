...
int main()
{
	...
	{
		read(n,m,a,n);
		for (i=1;i<=n;i++) b[i]=Q(i,a[i]); //sort(b+1,b+n+1);//reverse(b+1,b+n+1);
		x=200;ans=-inf;ans*=inf;
		for (i=max(n-300,1);i<=n;i++) for (j=i+1;j<=n;j++) ans=max(ans,(ll)b[i].u*b[j].u-(ll)m*(b[i].v|b[j].v));
		enter(ans);
	}
}