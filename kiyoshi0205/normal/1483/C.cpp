...
constexpr ll ope(ll a,ll b){return max(a,b);}
constexpr ll ide(){return -inf;}
using maxmaxsegtree=lazy_segtree<ll,ope,ide,ll,ope,ope,ide>;
ll N;
int main(){
  startupcpp();
  cin>>N;
  ...
  vec nxt(N);
  {
    pvec ch(1,datas(-inf,N));
    brep(i,N){
      while(ch.back().first>h[i].first)ch.pop_back();
      nxt[i]=ch.back().second;
      ch.emplace_back(h[i]);
    }
  }
  debug(nxt);
  maxmaxsegtree dp(N+1);
  dp.set(0,0);
  rep(i,N){
    ll x;
    cin>>x;
    ll now=dp.get(i);
    if(nxt[i]!=N)dp.apply(nxt[i],nxt[i]+1,now);
    dp.apply(i+1,nxt[i]+1,now+x);
  }
  print(dp.get(N));
}