...
void main(int) {
  ...
  scan(x, d);
  assert(x % d == 0);
  if (x / d % d) { return println("NO"); }
  auto factor = [](int n) {
    basic_string<int> ret;
    for (int p : rep1(2, int(sqrt(n)))) {
      for (; n % p == 0; n /= p) { ret += p; }
    }
    if (1 < n) { ret += n; }
    return ret;
  ...
  if (fd[0] == fd.back()) {
    auto f = factor(x);
    int k = sz(fd);
    int m = all(f).count(_1 == fd[0]);
    assert(2 * k <= m);
    if (2 <= sz(f) - m) {
      println("YES");
    } else if (sz(f) == m + 1) {
      println(k == 1 || m == 2 * k ? "NO" : "YES");
    } else {
      println(k == 1 || m < 2 * k + 2 || (k == 2 && m == 7) ? "NO" : "YES");
    }
  } else {
    int t = x / d / d;
    println(sz(factor(t)) < 2 ? "NO" : "YES");
  }
}
