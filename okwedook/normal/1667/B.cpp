...
struct Fenwick {
    vector<T> sum;
    Fenwick() {}
    Fenwick(long long n) { sum.assign(n, -mod); }
...
void solve() {
    ...
    Fenwick<int> rgtp(sz(c));
    Fenwick<int> lftm(sz(c));
    map<ll, int> bestval;
    vector<int> dp(sz(a), -mod);
    auto add_val = [&](ll v, ll x, int i) {
        int id = get_ind(v);
        rgtp.add(sz(c) - id - 1, x + i);
        lftm.add(id, x - i);
        if (!bestval.count(v)) {
            bestval[v] = x;
        } else {
    ...
    auto get_best = [&](ll v, int i) {
        ll ans = -inf;
        if (bestval.count(v)) {
            chmax(ans, bestval[v]);
        }
        int id = get_ind(v);
        chmax(ans, lftm.get(id - 1) + i);
        chmax(ans, rgtp.get(sz(c) - id - 1 - 1) - i);
        return ans;
    };
