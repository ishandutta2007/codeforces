#line 1 "codeforces-workspace\\contest\\543\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\543\\c\\c.cpp"
...
void main() {
  ...
  vector<string> s(n);
  cin >> s;
  auto c = make_vector<u32>({n, m});
  cin >> c;
  vector<u32> dp(1 << n, 1e9);
  dp[0] = 0;
  for (auto j = 0; j != m; ++j) {
    for (auto s = 0; s != 1 << n; ++s) {
      for (auto i = 0; i != n; ++i) {
        if (s >> i & 1) continue;
        chle(dp[s | 1 << i], dp[s] + c[i][j]);
      }
    }
    vector<u32> sum(128), cmax(128, 0), rs(128);
    for (auto i = 0; i != n; ++i) {
      sum[s[i][j]] += c[i][j];
      chge(cmax[s[i][j]], c[i][j]);
      rs[s[i][j]] |= 1 << i;
    }
    for (auto chr = 'a'; chr <= 'z'; ++chr) {
      if (rs[chr] > 0) {
        auto tc = sum[chr] - cmax[chr];
        for (auto s = 0; s != 1 << n; ++s) {
          chle(dp[s | rs[chr]], dp[s] + tc);
        }
      }
    }
  }
