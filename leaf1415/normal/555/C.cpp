...
ll n, Q;
ll X[200005], Y[200005]; char c[200005];
vector<ll> xcomp, ycomp;
LazySegTree wseg(18), hseg(18);
...
int main(void)
{
	...
	rep(i, 1, Q) cin >> X[i] >> Y[i] >> c[i];
	
	rep(i, 1, Q) xcomp.push_back(X[i]), ycomp.push_back(Y[i]);
	uniq(xcomp), uniq(ycomp);
	rep(i, 1, Q){
		X[i] = lower_bound(all(xcomp), X[i]) - xcomp.begin();
		Y[i] = lower_bound(all(ycomp), Y[i]) - ycomp.begin();
	}
	
	rep(i, 1, Q){
		ll x = X[i], y = Y[i];
		if(c[i] == 'U'){
			ll res = wseg.query(x, x);
			cout << ycomp[y]-res << "\n";
			wseg.add(x, x, ycomp[y]);
			res = lower_bound(all(ycomp), res) - ycomp.begin();
			hseg.add(res, y, xcomp[x]);
		}
		else{
			ll res = hseg.query(y, y);
			cout << xcomp[x]-res << "\n";
			hseg.add(y, y, xcomp[x]);
			res = lower_bound(all(xcomp), res) - xcomp.begin();
			wseg.add(res, x, ycomp[y]);
		}
	}
