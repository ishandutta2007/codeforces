...
public class Main
{
	...
	static class node
	{
		...
		void dfs(int fr, int at)
		{
			...
			else
			{
				...
				for(int i=0; i<adj.size; i++)
					if(adj.a[i]!=fr)
					{
						gra[adj.a[i]].dfs(at,adj.a[i]);
						lcm=lcm(lcm,gra[adj.a[i]].count);
					}
				if(fr!=-1)
					count=lcm*(adj.size-1);
				else
					count=lcm*(adj.size);
				if(count>1e13)
				{
		...
		void dfs1(int fr, int at)
		{
			for(int i=0; i<adj.size; i++)
			{
				...
				{
					if(fr==-1)
						gra[adj.a[i]].count=count/(adj.size);
					else
						gra[adj.a[i]].count=count/(adj.size-1);
				if(gra[adj.a[i]].adj.size!=1)
				{
					gra[adj.a[i]].dfs1(at, adj.a[i]);
				}
				}
			}
		}
	...
	public static void main(String[] args) throws IOException
	{
		...
		for(int i=2; i<=n; i++)
		{
			...
			{
				sum+=a[i];
				min=Math.min(min, (a[i]/gra[i].count));
			}
		}
		pr(sum-min*gra[1].count);
