...
inline void solve() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; ++i) {
            tot[i] = i == 0 ? 1 : cnk(max(-1LL, n - ll(k - 1) * (i - 1)), i);
            tot[i] = ll(tot[i]) * fct[i] % MOD;
        }
        tot[n] = 0;
        for (int i = 0; i < n; ++i) {
            tot[i] = (ll(n - i) * tot[i] - tot[i + 1]) % MOD;
            tot[i] = (tot[i] + MOD) % MOD;
            tot[i] = ll(tot[i]) * fct[n - i - 1] % MOD;
        }
        int sm = accumulate(tot, tot + n, 0LL) % MOD;
        //int sm = accumulate(tot, tot + n + 1, 0LL) % MOD;
        sm = inv(sm, MOD);
        int ans = 0;
        for (int i = 0; i <= n; ++i) {
            ans += ll(i + 1) * sm % MOD * tot[i] % MOD;
            ans %= MOD;
        }
        cout << ans << endl;
    }
