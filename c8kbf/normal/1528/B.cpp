...
inline void AC() {
    ...
    p[1] = 2;
    fc[1] = 1;
    for(_ i = 2; i <= n; ++i) fc[i] = i;
    for(_ i = 4; i <= n; i += 2) fc[i] = 2;
    for(_ i = 3; i*i <= n; ++i) if(fc[i] == i) for(_ j = i*i; j <= n; j += i) if(fc[j] == j) fc[j] = i;
    for(_ i = 2; i <= n; ++i) {
        dp[i] = (p[i-1]+fcno(i)-1)%mod;
        p[i] = (p[i-1]+dp[i])%mod;
    }
    writeln(dp[n]);
