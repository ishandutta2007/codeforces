...
int n,m,i,j,l,r,a[300005],b[300005],id[300005],sub;
...
//	fzin cerr<<id[i]<<' ';
	...
	for(;;)
	{
		while(l<=n&&a[l]==b[l])l++;if(l>n)break;
//		cerr<<l<<endl;
		if(a[l]>b[l])gugugu();
		if(r<=l)r=l+1;
		while(r<=n&&a[r]<=b[r])r++;
//		cerr<<r<<endl;
		if(r>n)gugugu();
		sub=min(abs(a[l]-b[l]),abs(a[r]-b[r]));
//		cerr<<a[l]<<' '<<b[l]<<' '<<sub<<endl;
		a[l]+=sub;
		a[r]-=sub;
		ans.push_back(make_pair(make_pair(id[l],id[r]),sub));
	}
	puts("YES");
	printf("%d\n",(int)ans.size());
	ff(ans,it) printf("%d %d %d\n",it->first.first,it->first.second,it->second);
	return 0;
}