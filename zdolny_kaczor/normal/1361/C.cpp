...
int iter[nax];
void dfs(int x) {
	for (; iter[x] < (int) graf[x].size(); ++iter[x]) {
		auto [to, num] = graf[x][iter[x]];
...
int main() {
	...
	for (int k = 1; k <= 20; ++k) {
		...
		int first = -1;
		for (int i = 0; i < m; ++i) if (deg[i]) first = i;
		bool good = true;
		for (int i = 0; i < m; ++i) if (deg[i] && find(i) != find(first)) good = false;
		REP(i, m) if (deg[i] & 1) good = false;
		if (!good) break;
		lar = k;
	}
	int m = (1 << lar);
	...
	for (int i = 0; i < n; ++i) {
		...
		a &= m - 1;
		b &= m - 1;
		graf[a].emplace_back(b, i * 2);
		graf[b].emplace_back(a, i * 2 + 1);
	}
	int start = -1;
	REP(i, m) if (!graf[i].empty()) start = i;
	assert(~start);
	dfs(start);
	for (int x : cycle) printf("%d ", x + 1);
	printf("\n");
	for (int x : cycle) if (x % 2) debug("second", arr(in, x / 2)); else debug("first", arr(in, x / 2));
	assert((int) cycle.size() == 2 * n);
	exit(0);
}
//#STAY AT HOME