        ...
        if (to != par) {
            ...
            for (int i = 0; i < 2; ++i) {
                for (int j = 0; j < 2; ++j) {
                    int z = (i + j + 1) % 2;
                    if (z % 2 == 0) {
                        ll t = sum[cur][i] * cnt[to][j] + cnt[cur][i] * sum[to][j] + cnt[cur][i] * cnt[to][j];
                        assert(t % 2 == 0);
                        ans += t / 2;
                    } else {
                        ll t = (sum[cur][i] * cnt[to][j] + cnt[cur][i] * sum[to][j]);
                        assert(t % 2 == 0);
                        ans += t / 2 + cnt[cur][i] * cnt[to][j];
                    }
                }
            }
            cnt[cur][1] += cnt[to][0];
            cnt[cur][0] += cnt[to][1];
            sum[cur][1] += sum[to][0] + cnt[to][0];
            sum[cur][0] += sum[to][1] + cnt[to][1];
        }
    }
}
int solve() {
