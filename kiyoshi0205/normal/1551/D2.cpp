...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  if(N&1){
    if(M/2<=K&&(K-M/2)%2==0){
      printYES;
      rep(i,M)cout<<char('a'+(i/2)%2);
      cout<<endl;
      K-=M/2;
      --N;
      while(K>=M){
        rep(i,M)cout<<char('c'+(i/2)%2);
        cout<<endl;
        rep(i,M)cout<<char('d'-(i/2)%2);
        cout<<endl;
        K-=M;
        N-=2;
      }
      ...
      rep(i,K)ans[0][i]='c'+(i/2)%2;
      For(i,K,M)ans[0][i]='e'+i%2;
      rep(i,K)ans[1][i]='d'-(i/2)%2;
      For(i,K,M)ans[1][i]='e'+i%2;
      int x=7;
      For(i,2,N){
        rep(j,M)ans[i][j]='a'+x+j%2;
        ++i;
        rep(j,M)ans[i][j]='a'+x+j%2;
        ++x;
        if(x==24)x=0;
    ...
    if(K<=N*M/2-N/2&&K%2==0){
      printYES;
      if(M==1){
        rep(i,N)print(char('a'+i/2%2));
        continue;
      }
      vector<string> ans(N,string(M,'.'));
      rep(i,N)ans[i].back()='y'+i/2%2;
      ll a=0;
      --M;
      while(K>=M){
        rep(i,M)ans[a][i]='c'+(i/2)%2;
        ++a;
        rep(i,M)ans[a][i]='d'-(i/2)%2;
        ++a;
        K-=M;
        N-=2;
      }
      ...
      rep(i,K)ans[a][i]='c'+(i/2)%2;
      For(i,K,M)ans[a][i]='e'+i%2;
      rep(i,K)ans[a+1][i]='d'-(i/2)%2;
      For(i,K,M)ans[a+1][i]='e'+i%2;
      int x=7;
      For(i,2,N){
        rep(j,M)ans[a+i][j]='a'+x+j%2;
        ++i;
        rep(j,M)ans[a+i][j]='a'+x+j%2;
        ++x;
        if(x==16)x=0;
