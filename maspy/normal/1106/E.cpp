...
void solve() {
  ...
  VEC(T, dat, K);
  sort(all(dat), [&](auto& x, auto& y) -> bool {
    auto& [sx, tx, dx, wx] = x;
    auto& [sy, ty, dy, wy] = y;
    if (wx > wy) return 1;
    if (wx < wy) return 0;
  ...
  vi TO(N + 1, -1);
  FaseSet ss(N + 1);
  FOR(i, 1, N + 1) ss.insert(i);

  for (auto&& [s, t, d, w]: dat) {
    while (1) {
      int i = ss.next(s);
      if (i > t) break;
      WT[i] = w;
  ...
  // 
  const ll INF = 1LL << 60;
  vv(ll, dp, N + 2, M + 1, INF);
  dp[1][0] = 0;
  FOR(x, 1, N + 1) {
    FOR(m, M) chmin(dp[x + 1][m + 1], dp[x][m]);
    if (TO[x] == -1) {
      FOR(m, M + 1) chmin(dp[x + 1][m], dp[x][m]);
    } else {
      FOR(m, M + 1) { chmin(dp[TO[x]][m], dp[x][m] + WT[x]); }
    }
  }
  print(MIN(dp[N + 1]));
}
