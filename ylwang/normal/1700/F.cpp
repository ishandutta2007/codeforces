...
const int MN=5e5+5; 
int n,a[2][MN],b[2][MN],sa,sb,d1,d2; 
 
void works() {
	int ans=0; 
	n=read(); For(i,1,n) sa+=a[0][i]=read(); 
	For(i,1,n) sa+=a[1][i]=read(); 
	For(i,1,n) sb+=b[0][i]=read(); 
	For(i,1,n) sb+=b[1][i]=read(); 
	if(sa!=sb) return printf("-1\n"),void(); 
	For(i,1,n) {
		d1+=a[0][i]-b[0][i]; 
		d2+=a[1][i]-b[1][i]; 
		if(d1<0&&d2>0) {
