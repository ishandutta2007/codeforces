...
//#define HandwriteRandom
	...
	#define Rand() (ra=((ll)ra*3450934+484524)%2147483647)
#endif
...
// Bitset
#define Bit(mask,pos) ((mask)>>(pos)-1LL&1LL)
...
namespace Mat{
	...
	_tp<_tyn T> Matrix<T> operator*( rg const Matrix<T> &a , rg const Matrix<T> &b ){
		...
		Matrix<T> c;
		c.n = a.n; c.m = b.m;
		//cout << a.m << " " << a.n << " " << b.m << endl;
		For0(k,a.m)
			For0(i,a.n)
				For0(j,b.m){
					ll t = c.d[i][j] + 1LL * a.d[i][k] * b.d[k][j] % MOD;
					//cout << a.d[i][k] << " " << b.d[k][j] << "  " << t << endl;
					Mymod(t);
					c.d[i][j] = t;
				}
		return c;
...
int main(){
	...
	For(i,n) if(a[i]==1) is1.Pb(i); else isnot.Pb(i);

	if(is1.size()<=2){
		printf("YES %d\n",n-1);
		printf("%d\n",n-1);
		For0(i,isnot.size()-1) printf("%d %d\n",isnot[i],isnot[i+1]);
		if( is1.size() >= 1 ) printf("%d %d\n",is1[0],isnot[0]);
		if( is1.size() >= 2 ) printf("%d %d\n",isnot.back(),is1[1]);
		END();
	}


	edges.Pb({is1[0],isnot[0]}); a[isnot[0]]--;
	For0(i,isnot.size()-1){
		edges.Pb({isnot[i],isnot[i+1]});
		a[isnot[i]]--;
		a[isnot[i+1]]--;
	}
	edges.Pb({is1[1],isnot.back()}); a[isnot.back()]--;
	Forx(i,2,is1.size()-1){
		bool found = 0;
		For0(j,isnot.size())
			if( a[isnot[j]] ){
				a[isnot[j]]--;
				edges.Pb({is1[i],isnot[j]});
				found = 1;
				break;
			}
		if(!found) END("NO");
