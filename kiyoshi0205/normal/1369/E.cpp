...
int main(){
  ...
  vector<int> ans;
  set<int> se;
  // rep(i,N)print(a[i].first<<" "<<a[i].second);
  // print("");
  // rep(i,M)print(fav[i].first<<" "<<fav[i].second);
  // print("");
  // matoutput(g);
  segtree<pair<int,int>> tree(a,pair<int,int>(-mod,0),[](pair<int,int> a,pair<int,int> b){return max(a,b);});
  while(se.size()<M){
    auto x=tree.queli(0,N);
    if(x.first<0){
      print("DEAD");
      return 0;
    }
    for(auto next:g[x.second]){
      if(se.insert(next).second){
        ans.eb(next+1);
        j=fav[next].first^fav[next].second^x.second;
        ++a[j].first;
        tree.input(j,a[j]);
      }
    }
    a[x.second].first=-mod;
    tree.input(x.second,a[x.second]);
  }
  print("ALIVE");
  reverse(all(ans));
  output(ans);
