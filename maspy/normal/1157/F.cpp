...
signed main() {
  ...
  auto A = vin(N);
  ll MAX = 200100;
  auto CNT = bin_count(A, MAX);

  auto f = [&](ll L) -> vi {
    // L 
    vi res;
    FOR(_, CNT[L]) { res.eb(L); }
    FOR3(i, L + 1, MAX) {
      if (CNT[i] == 0) break;
      FOR(_, CNT[i]) res.eb(i);
      if (CNT[i] == 1) break;
    };
    return res;
  };
  ...
  FOR(L, MAX) {
    if (CNT[L] == 0) continue;
    if (CNT[L - 1] > 0 && CNT[L] >= 2) continue;
    auto C = f(L);
    if (B.size() < C.size()) swap(B, C);
  }
  vi ANS;
  ll low = B[0], high = B.back();
  if (low == high) {
    ANS.assign(CNT[low], low);
  } else {
    FOR3(i, low, high + 1) {
      ANS.eb(i);
      --CNT[i];
