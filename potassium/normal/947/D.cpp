...
int main() {
	...
	auto getBC = [&](vector<int>& p, int l, int r) {
		return p[r] - (l > 0 ? p[l - 1] : 0);
	};
	
	vector<int> lasts(ls, -1), lastt(lt, -1);
	for (int i = 0, last = -1; i < ls; i++) {
		if (s[i] != 'A')
			last = i;
	...
	auto getsa = [&](vector<int>& last, int l, int r) {
		int lastp = last[r];
		return r - max(lastp, l - 1);
	};
	
	CASE(Q) {
		int a,b,c,d; R(a,b,c,d); a--; b--; c--; d--;
		
		int x = getBC(ps,a,b), y = getBC(pt,c,d);
		int sx = getsa(lasts,a,b), sy = getsa(lastt,c,d);
