...
int main() {
    ...
    sort_by(items, a.l != b.l ? a.l < b.l : a.v > b.v);

    vi ans(N);

    multimap<int, pii> actors;
    trav (e, items) {
        if (e.v == -1) {
            auto it = actors.lower_bound(e.r);
            if (it == actors.end()) ps("NO"), exit(0);
            ans[e.i] = it->s.f + 1;
            if (it->s.s == 1) actors.erase(it);
            else --it->s.s;
        } else {
            actors.insert(mp(e.r, mp(e.i, e.v)));
        }
    }
