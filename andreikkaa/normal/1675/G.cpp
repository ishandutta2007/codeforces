...
void _solve() {
    ...
    for(int pos = 0; pos < n; ++pos) {
        for(int sum = 0; sum <= m; ++sum) {
            const int dif = sum - ps[pos];
            for (int lst = 0; lst <= m; ++lst) {
                if (dp[pos][sum][lst] == INF) {
                    continue;
                }

                for (int nxt = 0; nxt <= lst && sum + nxt <= m; ++nxt) {
                    dp[pos + 1][sum + nxt][nxt] = min(dp[pos + 1][sum + nxt][nxt], dp[pos][sum][lst] + abs(dif));
                }
            }
        }
    }

    cout << *min_element(dp[n][m], dp[n][m] + m + 1) << endl;
}
