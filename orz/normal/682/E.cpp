...
template<typename T>
vi convex_hull_nh(vector<pair<T, T>>& a)
{
	int n = sz(a);
	if (n >= 2)
	{
...
int main()
{
	...
	int c = 0;
	ll current_prod;
	while ((current_prod = abs((v[c] - v[a]) % (v[b] - v[a]))) == 0)
		++c;
	cin >> v;
	int cnt = 0;
	...
	pll newa = v[b] + v[c] - v[a];
	pll newb = v[c] + v[a] - v[b];
	pll newc = v[a] + v[b] - v[c];
	cout << newa.X << ' ' << newa.Y << '\n'
		<< newb.X << ' ' << newb.Y << '\n'
		<< newc.X << ' ' << newc.Y << '\n';
	return 0;
