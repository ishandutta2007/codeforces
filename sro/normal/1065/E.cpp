...
#define Mod 1996488706 //2 times 998244353
#define Modbb 998244353
...
INT mii2(INT x,INT l,INT r){
	INT m=r-l;
	return (ksmii(x,2*m)-ksmii(x,m)+Mod)%Mod/2;
}
...
int main(){
	...
	INT ans=1;
	for(int i=1;i<=m;i++){
		ans=ans*(mii2(a,b[i-1],b[i])+ksmii(a,b[i]-b[i-1]))%Modbb;
	}
	//fio<<ans<<endl;
	ans=ans*ksmii(a,max(0LL,n-2*b[m]))%Modbb;
	fio<<ans<<endl;
	return 0;
}