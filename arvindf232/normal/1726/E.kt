...
fun main(){
    ...
    val inv2 = (2).inverse()
    val max = 150005
    val A0 = IntArray(max){FACT.invStore[ 2 * it]}
    val A1 = IntArray(max){FACT.invStore[ 2 * it +1 ]}
    val B = IntArray(max){FACT.invStore[it] modM intPow(inv2,it,pI)}

    val X0 = FFT.fullconvolution(A0,B)
    val X1 = FFT.fullconvolution(A1,B)

    fun find(n:Int):Int {
        return FACT.store[n] modM if(n % 2 == 1){
            X1[n/2]
        }else{
            X0[n/2]
        }
    ...
    val fullpairs=  IntArray(150001){
        FACT.store[2 * it] modM FACT.invStore[it] modM intPow(inv2,it, pI)
    }
//    just dei( 6 modM find(3))
//    just dei IntArray(10){FACT.store[it] modM find(it)}
    solve.cases{
...
//        just dei fullpairs
        ...
        var ret = 0
        for(c4 in 0..n step 4){
            val fours = c4/4
            val pairs1 = fours * 2
            val one = FACT.choose(n - pairs1, pairs1)
            val leave = n-c4
            val lret = find(leave)
//            var lret = 0
//            for(p2 in 0..(n-c4) step 2){
//                val total = n -c4
//                val c2 = p2/ 2
//                val justhere = FACT.choose(total,p2) modM fullpairs[c2]
//                lret = lret modPlus justhere
//            }
//            val f = FACT.store[pairs1] modM FACT.invStore[fours] modDivide intPow(2,fours,pI)
            ret = ret modPlus (one modM lret modM intPow(2,fours,pI) modM fullpairs[fours])
        }
        put(ret)

//        val quan = IntArray(n+1)
//
//        var now = 0
//        for(i in 0..n step 2){
//            now = now modPlus (FACT.invStore[i])
//            quan[i] = now modM FACT.store[i]
//            now = now modM inv2
//        }
//        now = 0
//        for(i in 1..n step 2){
...
//            val pairs1 = fours * 2
//            val one = FACT.choose(n - pairs1, pairs1)
//            ret = ret modPlus (one modM quan[n - c4] modM
//        }
//        put(ret)

//        for(i in 1..10){
//            i dei (quan[i] modM FACT.store[i])
//        }
...
//        for(i in 1..n){
//            if( i >= 2){
//                DP[i] = DP[i] modPlus DP[i-2]
//            }
//            DP[i] = DP[i] modPlus DP[i-1]
//        }
//        var ret = 0
//        for(ii in 0..n step 2){
//            val pairs = ii /2
//            val extra = FACT.store[n] modM intPow(inv2, pairs, pI) modM FACT.invStore[n - ii]
//            pairs dei extra
//            ret = ret modPlus extra
//        }
//        put(ret)
//        put(DP.last())
