...
mint solve() {
	...
	const int mid = n + 1;
	VV<mint> dp(n + 1, V<mint>(n * 2 + 4));
	auto route = dp;
	route.front()[mid] = 1;

	auto can = [&](int idx, int first_num, int second_num) {
		return s[0][idx] != '0' + 1 - first_num && s[1][idx] != '0' + 1 - second_num;
	};
	...
	rep(i, n) {
		...
		auto &route_pre = route[i];
		auto &route_nxt = route[i + 1];

		rep(from, mid - i, mid + i + 1) {
			const auto &before_dp = dp_pre[from];
			const auto &before_route = route_pre[from];
			int to;

			if(can(i, 0, 0)) {
				to = from;
				dp_nxt[to] += before_dp;
				route_nxt[to] += before_route;
			}
			if(can(i, 1, 1)) {
				to = from;
				dp_nxt[to] += before_dp;
			...
			if(can(i, 0, 1)) {
				to = from - 1;
				dp_nxt[to] += before_dp;
				dp_nxt[to] += mint(from > mid ? i : -i) * before_route;
				route_nxt[to] += before_route;
			}
