...
void main_() {
	...
	// 3 4 > 0
	int n = SZ(s);
	vector dp(n + 1, vector(64, mint(0)));
	dp[0][0] = 1;
	rep(i, n) {
		rep(j, 64) {
			...
			rep(k, 8) {
				...
				int nb = k >> 1 & 1;
				int nc = k >> 2 & 1;
				if(!af and s[i] == '0' and na) continue;
				if(!bf and s[i] == '0' and nb) continue;
				if(!cf and s[i] == '0' and nc) continue;
				int naf = af | (s[i] == '1' and !na);
				int nbf = bf | (s[i] == '1' and !nb);
				int ncf = cf | (s[i] == '1' and !nc);
				int nx = x | (k == 1 or k == 6);
				int ny = y | (k == 2 or k == 5);
				int nz = z | (k == 3 or k == 4);
				int nj = (naf << 0) | (nbf << 1) | (ncf << 2) | (nx << 3) | (ny << 4) | (nz << 5);
				dp[i + 1][nj] += dp[i][j];
			}
		}
	}
