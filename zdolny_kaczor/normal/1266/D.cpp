...
int main() {
	...
	FOR(i, 1, n) {
		if (money[i] > 0) pos.emplace_back(i, money[i]);
		else if (money[i] < 0) neg.emplace_back(i, -money[i]);
	}
	debug(imie(pos), imie(neg));
	vector <pair <pii, ll> > ans;
	while (!pos.empty() && !neg.empty()) {
		debug(imie(pos), imie(neg));
		ll take = min(pos.back().second, neg.back().second);
		ans.emplace_back(make_pair(pos.back().first, neg.back().first), take);
		pos.back().second -= take;
		if (pos.back().second == 0) pos.pop_back();
		neg.back().second -= take;
