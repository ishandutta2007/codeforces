...
public:
    void solve() {
        ...
        mod = MOD9;
        modint cur = power(modint(a), n);
        modint mul = modint(a).inverse() * b;
        modint answer = 0;
        for (int i : range(k)) {
            if (s[i] == '+') {
                answer += cur;
        ...
        modint d = power(mul, k);
        modint res = (d.n == 1 ? (n + 1) / k : (power(d, (n + 1) / k) - 1) / (d - 1)) * answer;
        out.printLine(res);
    }
};
