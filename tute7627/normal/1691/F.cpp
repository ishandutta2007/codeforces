...
int main(){
  ...
  modint ret=0;
  {
    auto dfs=[&](auto &&f,ll v,ll par)->void{
      ...
      if(sub[v]!=n)s.PB(n-sub[v]);
      modint ck_sum=0;
      for(auto z:s)ck_sum+=comb.C(z,k);
      //debug(s);
      for(auto z:s){
        modint pat=comb.C(n-z,k)-(ck_sum-comb.C(z,k));
        //cout<<v spa pat spa ck_sum spa comb.C(z,k) spa comb.C(n-z,k)<<endl;
        ret+=pat*(n-z)*z;
      }
      ret+=n*(comb.C(n,k)-ck_sum);
    };
    dfs(dfs,0,-1);
  }
  cout<<ret<<endl;
