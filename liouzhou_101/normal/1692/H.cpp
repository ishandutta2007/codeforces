...
void solve()
{
	...
	for (int i = 0; i < n; ++i)
		F[a[i]].push_back(i);
	tuple<int, int, int, int> res(-1000000000, -1, -1, -1);
	for (auto& [x, v] : F)
	{
		vector<int> b(v.size());
		pair<int, int> pref(1000000000, -1);
		for (int i = 0; i < v.size(); ++i)
		{
			b[i] = 2 * i - v[i];
			freshmin(pref, pair(b[i], i));
			if (b[i] - pref.first + 1)
				freshmax(res, tuple(b[i] - pref.first + 1, x, v[pref.second], v[i]));
		}
	}
	writeln(get<1>(res), ' ', get<2>(res) + 1, ' ', get<3>(res) + 1);
}
