...
bool cliq(const vector<int>& v, const vector<unordered_set<int>>& gg)
{
	for (int i = 0; i < sz(v); ++i)
		for (int j = 0; j < i; ++j)
			if (!gg[v[i]].count(v[j]))
...
void work(const int v, vvi& g, vector<unordered_set<int>>& gg, vi& c, vb& b, vi& p, const int k)
{
	if (b[v] && p[v] < k && (sz(c) == 0))
	{
		b[v] = false;
