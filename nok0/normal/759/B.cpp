...
void main_() {
	...
	REP(i, n) {
		int pl = lb(re, t[i]);
		int v = seg.prod(bef, pl);
		seg.set(pl, min(seg.get(pl), v));
		seg.set(pl + 1, min(seg.get(pl + 1), seg.get(pl) + 20));
		int p90 = lb(re, t[i] + 90);
		seg.set(p90, min(seg.get(p90), seg.get(pl) + 50));
		int p1440 = lb(re, t[i] + 1440);
		seg.set(p1440, min(seg.get(p1440), seg.get(pl) + 120));
		print(seg.prod(pl + 1, m) - last_res);
		last_res = seg.prod(pl + 1, m);
		bef = pl + 1;
	}
}