...
void solve() {
    int n, m, k;
    cin >> n >> m >> k;
    auto a = make_vector<llong>(n, m);
    cin >> a;
    a.push_back(vector<llong>(m, 0));
    auto ss = a;
    for (int i = 0; i <= n; i++) {
        for (int j = 1; j < m; j++) {
            ss[i][j] += ss[i][j - 1];
    ...
    struct ST {
        ...
        llong get_mx(int i, int l, int r, int ql, int qr) {
            push(i, l, r);
            if (r <= ql || qr <= l) {
                return 0;
            }
            ...
            int m = (l + r) >> 1;
            return max(get_mx(i * 2 + 1, l, m, ql, qr), get_mx(i * 2 + 2, m, r, ql, qr));
        }

        void upd(int i, int l, int r, int ql, int qr, llong qx) {
            push(i, l, r);
            if (r <= ql || qr <= l) {
                return;
            }
    ...
    vector<llong> d(m);
    for (int i = 0; i < m; i++) {
        d[i] = get_s(0, i, i + k - 1) + get_s(1, i, i + k - 1);
    }

    for (int z = 1; z < n; z++) {
        ST st(m);
        for (int i = 0; i < m; i++) {
            st.upd(0, 0, m, i, i + 1, d[i]);
        }
        llong s = get_s(z, 0, k - 1);
        for (int i = 0; i < k; i++) {
            st.upd(0, 0, m, i, i + 1, -s);
            s -= a[z][i];
        }
        for (int i = 0; i < m; i++) {
            d[i] = get_s(z, i, i + k - 1) + get_s(z + 1, i, i + k - 1) + st.get_mx(0, 0, m, 0, m);
            st.upd(0, 0, m, max(0, i - k + 1), i + 1, a[z][i]);
            if (i + k < m) {
                st.upd(0, 0, m, i + 1, i + k + 1, -a[z][i + k]);
            }
        }
    }
