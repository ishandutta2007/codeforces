...
_ build(_ l, _ r) {
    if(l > r) return -1;
    _ mx = -1E11, dx = -1;
    for(_ i = l; i <= r; ++i) if(a[i] > mx) {
        mx = a[i];
        dx = i;
    }
    _ lc = build(l, dx-1), rc = build(dx+1, r);
    if(~lc) g[mx].push_back(lc);
    if(~rc) g[mx].push_back(rc);
    return mx;
}

void dfs(_ x, _ d) {
    ::d[p[x]] = d;
    for(_ i : g[x]) dfs(i, d+1);
    return;
}