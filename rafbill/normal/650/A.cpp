...
int main(int, char**){
  ...
  lli n; cin >> n;
  unordered_map<lli, lli> X, Y;
  unordered_map<pii, lli> XY;
  FOR(i, n) {
    lli x, y; cin >> x >> y;
    X[x] += 1; Y[y] += 1; XY[mt(x, y)] += 1;
  }
  lli r = 0;
  for(auto p : X) r += (p.second*(p.second-1))/2;
  for(auto p : Y) r += (p.second*(p.second-1))/2;
  for(auto p : XY) r -= (p.second*(p.second-1))/2;
  cout << r << endl;
  return 0;
}