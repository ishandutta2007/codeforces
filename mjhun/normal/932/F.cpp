...
struct HullDynamic : public multiset<Line> { // for maximum
	bool bad(iterator y){
		...
		bigint asd=x->b-y->b;asd*=(z->m-y->m);
		bigint qwe=y->b-z->b;qwe*=(y->m-x->m);
		return asd>=qwe;
...
void dfs(int x, int f){
	...
	for(int y:g[x]){
		if(y==f)continue;
		dfs(y,x);
		if(z<0||SZ(s[ws[y]])>SZ(s[ws[z]]))z=y;
	}
	if(z<0){ // leaf
		ws[x]=cs++;
		r[x]=0;
		...
		for(int y:g[x]){
			if(y==f||y==z)continue;
			for(auto l:s[ws[y]])s[ws[x]].add(l.m,l.b);
		}
		r[x]=-s[ws[x]].eval(a[x]);
		s[ws[x]].add(-b[x],-r[x]);
	}
