...
public class CF965D {
    ...
    public static void init() throws FileNotFoundException {
        if (System.getProperty("ONLINE_JUDGE") == null) {
            input = new BlockReader(new FileInputStream("E:\\DATABASE\\TESTCASE\\codeforces\\CF965D.in"));
            output = System.out;
        } else {
    ...
    public static void solve() {
        ...
        int[] stone = new int[w + 1];
        int[] exists = new int[w + 1];
        stone[0] = INF;
        exists[0] = INF;
        stone[w] = INF;
        for (int i = 1; i < w; i++) {
            stone[i] = input.nextInteger();
        }

        int lastPos = 0;
        for (int i = 1; i <= w; i++) {
            lastPos = Math.max(lastPos, i - l);
            while (exists[i] < stone[i] && lastPos < i) {
                if (exists[lastPos] + exists[i] <= stone[i]) {
                    exists[i] += exists[lastPos];
                    lastPos++;
                } else {
                    exists[lastPos] -= stone[i] - exists[i];
                    exists[i] = stone[i];
                }
            }
        }

        output.println(exists[w]);
    }
