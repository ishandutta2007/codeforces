...
void spfa()
{
    ...
    for(int i = 0; i < m; i++)
    {
        dis[i] = min(get_abs(sx - x[i]), get_abs(sy - y[i]));
        inq[i] = 1;
        Q.emplace(i);
    }
    ...
    while(!Q.empty())
    {
        ...
        for(auto v : G[u])
        {
            LL w = min(get_abs(x[u] - x[v]), get_abs(y[u] - y[v]));
            if(dis[v] > dis[u] + w)
            {
                dis[v] = dis[u] + w;
                if(!inq[v])
...
void work()
{
    ...
    for(int i = 0; i < m; i++) reader >>x[i] >>y[i];
    for(int i = 0; i < m; i++) G[i].clear();

    for(int i = 0; i < m; i++) id[i] = i;
    sort(id, id + m, [&](int a, int b) {return x[a] < x[b];});

    for(int i = 0; i < m; i++)
    {
        if(i - 1 >= 0) G[id[i]].epb(id[i - 1]);
        if(i + 1 < m) G[id[i]].epb(id[i + 1]);
    }

    for(int i = 0; i < m; i++) id[i] = i;
    sort(id, id + m, [&](int a, int b) {return y[a] < y[b];});
    ...
    LL ans = get_abs(sx - fx) + get_abs(sy - fy);

    for(int i = 0; i < m; i++)
        ans = min(ans, get_abs(x[i] - fx) + get_abs(y[i] - fy) + dis[i]);
