...
using val=pair<LL,pll>;

val mer(val a,val b){
  return mp(a.X+b.X,mp(min(a.Y.X-b.X,b.Y.X+a.X),max(a.Y.Y-b.X,b.Y.Y+a.X)));
}
...
struct segtree{
  ...
  void init(int a,int b,int k,vector<LL>&A){
    if(b==a+1){
      T[k]=mp(A[a],mp(-abs(A[a]),abs(A[a])));
      return;
    }
    int c=(a+b)/2;
    init(a,c,k*2+1,A);
    init(c,b,k*2+2,A);
    T[k]=mer(T[k*2+1],T[k*2+2]);
  }

  val get(int l,int r,int a,int b,int k){
    if(l>=b||r<=a) return mp(0ll,mp(0,0));
    if(l<=a&&r>=b) return T[k];
    int c=(a+b)/2;
    return mer(get(l,r,a,c,k*2+1),get(l,r,c,b,k*2+2));
  }
};
...
int main(int argc, char **argv){
  ...
  forn(q,tc){
    ...
    cin>>a>>b;
    --a;
    val r=T.get(a,b,0,n,0);
    dbg(q,a,b,r);
    dbg(T.T);
    if(r.X||r.Y.X<0) cout<<"-1\n";
    else{
      assert(r.Y.Y%2==0);
      cout<<r.Y.Y/2<<'\n';
    }
