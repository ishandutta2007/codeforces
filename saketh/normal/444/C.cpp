...
int main() {
    ...
    struct n { int len; ll tot; };
    using u = ll;
    auto nn = [](n a, n b) { return n{a.len+b.len, a.tot+b.tot}; };
    auto uu = plus<ll>();
    auto un = [](u U, n N) { return n{N.len, N.tot + U * N.len}; };

    int N, M; re(N, M);

    map<pii, int> ribbon;
    F0R (i, N) ribbon[{i,i+1}] = i+1;

    segment_tree_lazy color(N, n{0,0}, 0ll, nn, uu, un);
    color.set_leaves([](int i) { return n{1, 0}; });
    ...
    F0R (m, M) {
        ...
        if (t == 1) {
            re(l, r, x); --l;
            vector<pair<pii, int>> put;
            for (auto st = prev(ribbon.ub({l,INT_MAX}));
                    st != ribbon.end() && st->f.f < r; ribbon.erase(st++)) {
                pii key = st->f; int cur = st->s;
                color(max(key.f, l), min(key.s, r), abs(x - cur));
                if (key.f < l) put.eb(pii{key.f, l}, cur);
                if (key.s > r) put.eb(pii{r, key.s}, cur);
            }
            trav (e, put) ribbon[e.f] = e.s;
            ribbon[pii{l,r}] = x;
        } else {
            re(l, r); --l;
            ps(color(l, r).tot);
