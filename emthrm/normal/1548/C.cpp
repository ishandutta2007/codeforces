...
int main() {
  ...
  //   int x; cin >> x;
  //   ModInt ans = 0;
  //   for (int i = (x + 2) / 3; i <= n; ++i) ans += ModInt::nCk(i * 3, x);
  //   cout << ans << '\n';
  // }

  ModInt::init(n * 3 + 3);
  vector<ModInt> ans(n * 3 + 1, 0);
  ans[1] = n * (n + 1LL) * 3 / 2;
  FOR(i, 1, n + 1) ans[2] += 3LL * i * (3 * i - 1) / 2;
  FOR(i, 3, n * 3 + 1) ans[i] = ModInt::nCk(n * 3 + 3, i + 1) / 3 - ans[i - 1] - ans[i - 2] / 3;
  while (q--) {
    int x; cin >> x;
    cout << ans[x] << '\n';
  }
