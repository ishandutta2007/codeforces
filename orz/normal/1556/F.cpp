...
struct st
{
	int n;
	vl q_min, q_max;
	st(const vl& s) : n(sz(s))
	{
		q_min.resize(n * 4);
		q_max.resize(n * 4);
		set_val(0, 0, n, s);
	}
	void set_val(const int v, const int v0, const int v1, const vl& s)
	{
		if (v1 - v0 == 1)
		{
			q_min[v] = s[v0];
		...
		{
			int v5 = (v0 + v1) / 2;
			set_val(2 * v + 1, v0, v5, s);
			set_val(2 * v + 2, v5, v1, s);
			q_min[v] = min(q_min[2 * v + 1], q_min[2 * v + 2]);
			q_max[v] = max(q_max[2 * v + 1], q_max[2 * v + 2]);
		}
	}
	pll find_min_max(const int s0, const int s1)
	{
		return find_min_max(0, 0, n, s0, s1);
	}
	pll find_min_max(const int v, const int v0, const int v1, const int s0, const int s1)
	{
		if (v0 >= s1 || v1 <= s0)
			return pll(LLONG_MAX, LLONG_MIN);
		else if (v0 >= s0 && v1 <= s1)
			return pll(q_min[v], q_max[v]);
		else
		{
			int v5 = (v0 + v1) / 2;
			pll a0 = find_min_max(2 * v + 1, v0, v5, s0, s1), a1 = find_min_max(2 * v + 2, v5, v1, s0, s1);
			return pll(min(a0.X, a1.X), max(a0.Y, a1.Y));
		}
	}
};
...
int another_main()
{
	...
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < n; ++j)
			invsum[i][j] = inverse(a[i] + a[j], M);


	vvi step(n + 1, vi(1 << n, 1));
	for (int i = 1; i < 1 << n; ++i)
	{
		int k = __builtin_ctz(i);
		step[1][i] = prod(step[1][i ^ 1 << k], a[k], M);
	}
	for (int i = 2; i <= n; ++i)
		for (int j = 0; j < 1 << n; ++j)
			step[i][j] = prod(step[i - 1][j], step[1][j], M);

	vvi step1(n, vi(1 << n, 1));
	for (int i = 0; i < n; ++i)
		for (int j = 1; j < 1 << n; ++j)
		{
			int k = __builtin_ctz(j);
			step1[i][j] = prod(step1[i][j ^ 1 << k], invsum[i][k], M);
		}
	...
	for (int i = 0; i < 1 << n; ++i)
	{
		if (i & (i - 1))
		{
			for (int j = (i - 1) & i; j; j = (j - 1) & i)
			{
				...
				int prob = b[j];
				int mjs = sz(mj);
				prob = prod(prob, step[mjs][j], M);
				for (int ii : mi)
					prob = prod(prob, step1[ii][i ^ j], M);
				b[i] = raz(b[i], prob, M);
			}
		}
	}
	...
	for (int j = 1; j < 1 << n; ++j)
	{
		...
		for (int ii : mi)
			for (int jj : mj)
				prob = prod(prob, prod(a[ii], invsum[ii][jj], M), M);
		prob = prod(prob, __builtin_popcount(j), M);
		ans = sum(ans, prob, M);
	}
	cout << ans << '\n';
	return 0;
