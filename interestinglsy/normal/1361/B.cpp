...
template<typename T1, typename T2>
il void Deci( T1 &x , const T2 &y , const ll fuck ){ x = Mymoi(x-y+fuck,fuck); }
template<typename T1, typename T2>
il void Muli( T1 &x , const T2 &y ){ x = 1LL*x*y%MOD; }
il ll Pow( ll a , ll b , const ll p	 ){
	ll ret = 1;
	for( ; b ; a = a*a%p, b >>= 1 )
		if(b&1) ret = ret*a%p;
	return ret;
...
ll Solve(){
	...
	For(i,n){
		if( c1 == 0 and c2 == 0 and c3 == 0 ){
			...
			Deci(c1,Pow(p,a[i],MOD),MOD);
			Deci(c2,Pow(p,a[i],MOD2),MOD2);
			Deci(c3,Pow(p,a[i],MOD3),MOD3);
		}
	}
	return c1;
}
