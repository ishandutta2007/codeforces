...
namespace chmax_max{
  ...
  LazySegmentTree<M,M,decltype(f),decltype(g),decltype(h)>make(int n){
    return {n,f,g,h,MP(-inf,-inf),MP(-inf,-inf)};
  }
}
template<typename T>
struct Compress{
...
int main(){
  ...
  mp.build();
  int sz=mp.size();
  auto seg=chmax_max::make(sz);
  rep(i,0,sz)seg.set(i,MP(-1,-1));
  seg.build();
  vector<int>from(n);
  const int inf=1e9;
  rep(i,0,n){
    pi mx(-1,-1);
    for(auto z:g[i]){
      chmax(mx,seg.query(mp[z.fi],mp[z.se]));
    }
    mx.fi+=1;
    from[i]=mx.se;
    mx.se=i;
    for(auto z:g[i]){
      seg.update(mp[z.fi],mp[z.se],mx);
    }
    //cout<<i spa mx<<endl;
    //seg.print();
  }
  vector<bool>remain(n);
  int now=seg.query(0,sz).se;
  while(now!=-1){
    remain[now]=true;
    now=from[now];
  }
