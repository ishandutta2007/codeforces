...
int main(){
    ...
    SegmentTree<P> seg(n,[](P a,P b){return min(a,b);},{INF,n});
    for (int i=0;i<n;++i) seg.update(i,{a[i],i});

    auto dfs=[&](auto self,int l,int r,int h)->ll{
        if (l==r) return 0;
        auto p=seg.query(l,r);
        int w=p.first,mid=p.second;
        return min((ll)(r-l),self(self,l,mid,w)+self(self,mid+1,r,w)+w-h);
    };
