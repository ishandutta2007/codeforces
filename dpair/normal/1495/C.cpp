...
char a[MAXN][MAXN];
inline void work() {
    read(n, m); rep(i, 1, n) read(a[i] + 1);
    memset(a[n + 1], 0, sizeof(a[n + 1])); memset(a[n + 2], 0, sizeof(a[n + 2]));
    for (int i = 1;i <= n;i += 3) {
        bool ck = 1; rep(j, 1, m) { a[i][j] = 'X';
            if(a[i + 1][j] == 'X' && ck) { a[i + 2][j] = 'X'; ck = i + 3 > n; } 
            if(a[i + 2][j] == 'X' && ck) { a[i + 1][j] = 'X'; ck = i + 3 > n; } 
        }
        if(i + 3 <= n && ck) a[i + 1][1] = a[i + 2][1] = 'X';
    }
    rep(i, 1, n) print(a[i] + 1);
}
