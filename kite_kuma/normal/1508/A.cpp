...
vi solve() {
	...
	vi sum(3);
	rep(i, 3) { sum[i] = accumulate(all(s[i]), 0); }
	...
	if(sum[0] < n) swap(s[0], s[2]);
	if(sum[1] < n) swap(s[1], s[2]);
	...
	rep(i, n + 1) {
		rep(j, 2) {
			...
			rep(k, lef + 1, rig) { ret.push_back(s[j][k]); }
		}
