...
int main() {
    ...
    vvmint cm(N + 1, vmint(N + 1, 0));
    cm[0][0] = 1;
    rep(i, N + 1)rep(j, N + 1) {
        if (i > 0)cm[i][j] += cm[i - 1][j];
        if (i > 0 && j > 0)cm[i][j] += cm[i - 1][j - 1];
    }

    vvvmint dp(N + 1, vvmint(N + 1, vmint(K + 1, 0)));
    vvvll c(N + 1, vvll(N + 1, vll(K + 1, 0)));


    auto cal = [&](ll i, ll j, ll k, auto&& cal)-> mint {
        if (c[i][j][k]) { return dp[i][j][k]; }
       ...
       if (i < M-1) {      
                rep(x, j)rep(a, k + 1) {
                    dp[i][j][k] += cal(i + 1, x, a, cal) * cal(i + 1, j - 1 - x, k - a, cal) *cm[j-1][x];
                }      
        }

        if (i == M-1 && k==1) {
           rep(x, j)dp[i][j][k] += cal(i + 1, x, 0, cal) * cal(i + 1, j - 1 - x, 0, cal) * cm[j-1][x];
        }
