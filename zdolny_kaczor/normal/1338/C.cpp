...
array <ll, 3> go(ll n) {
	...
	for (ll i = 1; ; i *= 4) {
		if (n < i) {
			debug(imie(n0), imie(i), imie(n));
			return {i + n, 2 * i + conv(n), (i + n) ^ (2 * i + conv(n))};
		}
		n -= i;
	}
}
void test() {
	int beg = 0;
	int iter = 0;
	for (int i = 1; i < nax / 10; ++i) if (!used[i]) {
		for (int j = i + 1; j < nax; ++j) {
			if (!used[j] && !used[i ^ j]) {
				used[i] = used[j] = used[i ^ j] = 1;
				if (popc(i) == 1) beg = i;
				int o1 = i - beg, o2 = j - beg * 2;
				assert(conv(o1) == o2);
				debug(imie(i) - imie(beg), imie(j) - imie(beg * 2), imie(i ^ j) - imie(beg * 2));
				assert(go(iter++) == (array<ll, 3>{i, j, i ^ j}));
				break;
			}
		}
	}
