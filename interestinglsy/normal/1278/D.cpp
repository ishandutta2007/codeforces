...
vc<pii> edges;
void GetEdges(){
	sort(All(segs),[]( const Seg &x , const Seg &y ){
		return x.r < y.r;
	});
	static multiset<Seg> st;
	for_each(AllRev(segs),[&]( const Seg &seg ){
		auto iter = st.lower_bound(Seg{seg.l,0,0});
		while( iter != st.end() ){
			assert( iter->l > seg.l );
			if( iter->l > seg.r ) break;
			edges.Pb({seg.id,iter->id});
			++iter;
		}
		if( edges.size() >= n ) END("NO");
		st.insert(seg);
	});
	// O(edges.size());
}
