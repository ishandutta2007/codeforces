...
int main() {
    ...
    lint ret = 0;
    for (auto [h_, ss] : mp) {
        ret += lint(ss.size()) * (N - ss.size()) * 1337;
        lint nsub = ss.size();
        ret += nsub * (nsub - 1) * 2;
        ...
        auto sa = suffix_array(Sbuild);
        vector<int> lcp = lcp_array(Sbuild, sa);
        // lcp.push_back(0);
        vector<int> sainv(sa.size());
        REP(i, sa.size()) sainv[sa[i]] = i;
        atcoder::segtree<int, op, e> rmq(lcp);
        ...
        REP(i, ss.size()) {
            ...
            REP(i, s.size() - 1) if (s[i] > s[i + 1]) {
                sgs.emplace_back(l, i + 1);
                l = i + 1;
            }
            sgs.emplace_back(l, s.size());
            dbg(s);
            dbg(sgs);
            REP(i, sgs.size()) {
                int L = sainv[sgs[i].second + h];
                int len = sgs[i].second - sgs[i].first;
                int r = rmq.max_right(L, [&](int x) { return x >= s.size() + 1 - len; });
                dbg(pint(h, L));
                dbg(r - L);
                ret -= (r - L) * 2 - 0;
            }
