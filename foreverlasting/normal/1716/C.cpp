...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		n=read();
		for(auto &i:a)for(int j=1;j<=n;j++)i[j]=read()+1;
		a[0][1]=0,b[0][n+1]=b[1][n+1]=0;
		for(int j=n;j>=1;j--){
			for(int i=0;i<=1;i++){
				b[i][j]=max({a[i^1][j],a[i][j]+(n-j)*2+1,b[i][j+1]+1});
			}
		}
		int ans=inf,cur=0;
		for(int i=1;i<=n;i++){
			int j=i&1;
			ans=min(ans,max(cur,b[j^1][i]));
			cur=max({cur,a[j][i]+(n-i)*2,a[j^1][i]+(n-i)*2+1});
		}
		printf("%d\n",ans);
	}
}
