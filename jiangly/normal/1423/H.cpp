...
int main() {
    ...
    for (auto [u, v, x, d] : edges) add(add, 1, 0, tim, x, ed[std::min(d + k - 1, day)], std::make_pair(u, v));
    int cur = 0;
    DSU dsu(n);
    auto solve = [&](auto solve, int p, int l, int r) -> void {
        ...
        if (r - l == 1) {
            while (cur < int(qry.size()) && qry[cur].second == l) std::cout << dsu.size(qry[cur++].first) << "\n";
        } else {
            int m = (l + r) / 2;
            solve(solve, 2 * p, l, m);
