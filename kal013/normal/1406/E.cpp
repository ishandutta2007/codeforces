...
const int BLOCK_S = 100;
void solve(){
	int n;
	cin >> n;

	gp_hash_table<int,int> cnt_removed;
	...
	int remaining = query_A(1);
	vector<int> not_checked;
	...
	for(auto e: primes){
		...
		curr_removed += query_B(e);
		not_checked.push_back(e);


		if (not_checked.size() >= BLOCK_S){
			int k = query_A(1);
			if (k > remaining - curr_removed){
				for(auto z: not_checked){
					k = query_A(z);
				...
				assert(k + curr_removed == remaining);
			}

			not_checked.clear();

			remaining -= curr_removed;
			curr_removed = 0;
		}
	}
