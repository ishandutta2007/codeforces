...
#pragma region datastructure SWAG
#include <cassert>
#include <functional>
#include <stack>
#include <vector>
...
void main_() {
	...
	while(q--) {
		...
		VEC(int, l, m);
		int sz = n - m + 1;
		SWAG<int> swag([](int x, int y) { return max(x, y); }, sz);

		REP(j, m) {
			swag.push(l[j]);
			int val = swag.fold_all();
			if(j >= m or j + m < n) chmax(val, 0);
			imos[j] += val;
			imos[j + 1] -= val;
		}
		// 0a 0ab 0abc abcd bcd0 cd0 d0
		// 0a ab bc 0c
		int l1 = m;
		int r1 = n - m;
		if(l1 < r1) {
			int val = max(swag.fold_all(), 0);
			imos[l1] += val;
			imos[r1] -= val;
		}
		REP(j, max(m, n - m), n) {
			while(swag.size() > n - j) swag.pop();
			int val = swag.fold_all();
			if(j >= m or j + m < n) chmax(val, 0);
			imos[j] += val;
			imos[j + 1] -= val;
