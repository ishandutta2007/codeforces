...
int main() {
	CASE(t) {
		...
		vector<vector<int>> basis(m, vector<int>(m + 1, 0));
			...
			for (int i = 0; i < m; i++) {
				if (v[i] != 0) {
					if (basis[i][i] == 0) {
						...
						int mul = (3 - v[i]) * basis[i][i] % 3;
						for (int j = i; j <= m; j++)
							v[j] = (v[j] + basis[i][j] * mul) % 3;
					}
				}
			}
			return v[m] == 0;
		...
		if (!is) {
			...
			for (int i = m - 1; i >= 0; i--) {
				if (basis[i][i] == 0) {
					...
					int mul = basis[i][i];
					basis[i][i] = basis[i][i] * mul % 3;
					basis[i][m] = basis[i][m] * mul % 3;
				}

				for (int j = i - 1; j >= 0; j--) {
					int mul = 3 - basis[j][i];
					basis[j][i] = (basis[j][i] + basis[i][i] * mul) % 3;
					basis[j][m] = (basis[j][m] + basis[i][m] * mul) % 3;
				}
			}

			vector<int> ans(m, 0);
			for (int i = 0; i < m; i++)
				ans[i] = (basis[i][m] == 0 ? 3 : basis[i][m]);
