...
int main() {
    setIO("");
    int tt; re(tt);
    map<array<int, 4>, pi> dp;

    auto chmax = [&](pi& x, pi y) {
        swap(y.f, y.s);
        if (x.f - x.s > y.f - y.s) x = y;
    };
    function<pi(int, int, int, int)> solve = [&](int x, int y, int z, int t) -> pi {
        array<int, 4> state = {x, y, z, t};
        if (x == 0 && y == 0 && z == 0) return {0, 0};
        pi res = {INF, 0};
        if (dp.find(state) != dp.end()) return dp[state];
        if (t == 0) { // did not flip before
            if (y) { 
                chmax(res, solve(x, y, z, 1));
            }
        }
        if (x) {
            auto tmp = solve(x - 1, y, z, 0);
            tmp.s++;
            chmax(res, tmp);
        }
        if (y) {
            auto tmp = solve(x, y - 1, z, 0);
            tmp.s++;
            chmax(res, tmp);
        }
        if (z) {
            auto tmp = solve(x, y + 1, z - 1, 0);
            tmp.s++;
            chmax(res, tmp);
        }
        dp[state] = res;
    ...
    while (tt--) {
        ...
        auto res = solve(x, y, z, 0);
        int u = res.f;
        int v = res.s;
        if (u > v) ps("BOB");
        else if (u < v) ps("ALICE");
        else ps("DRAW");
    }
    return 0;
}