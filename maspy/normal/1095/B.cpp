...
signed main() {
  ...
  auto a = *min_element(A.begin(), A.end() - 1);
  auto b = *max_element(A.begin(), A.end() - 1);
  auto c = *min_element(A.begin() + 1, A.end());
  auto d = *max_element(A.begin() + 1, A.end());
  auto ans = min(b - a, d - c);
  print(ans);
