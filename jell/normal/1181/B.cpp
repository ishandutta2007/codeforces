...
signed main() {
    ...
    if(lef>0) {   
        ...
        //dump(tmp,tmp2,splus(tmp,tmp2));
        update(splus(tmp,tmp2));
        if(lef>=2) {
            lef--;
            if(s[lef]!='0') {
                tmp.pop_back();
                string tmp3=s[lef]+tmp2;
                update(splus(tmp,tmp3));
            }
        }
    }
    ...
    if(rgt>0) {
        ...
        //dump(tmp,tmp2,splus(tmp,tmp2),ans);

        update(splus(tmp,tmp2));
        if(rgt+1<L && s[rgt+1]!='0') {
            tmp+=s[rgt];
            string tmp3;
            for(int i=rgt+1; i<L; ++i) {
                tmp3+=s[i];
            }
            update(splus(tmp,tmp3));
        }
