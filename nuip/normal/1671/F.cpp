...
void dfs(int usd, int prv, int cnt, int inv, int dec){
	if(inv>11 || dec>11) return;
	// out(usd,prv,cnt,inv,dec,1);
	if(cnt==N){
		++cnts[cnt][inv][dec];
		return;
	}
	...
	rep(i,N){
		if(usd>>i&1){
			...
			dfs(usd|(1<<i), i, cnt+1, inv, dec+(prv>i));
		}
	}
}
...
int main(){
	...
	// N=2;
	// dfs(1<<1,1,1,0,0);
	// rep(j,5)rep(k,5) out(j,k,cnts[2][j][k],1);
	// return 0;
	reps(n,2,13){
		N=n;
		rep(i,n) dfs(1<<i,i,1,0,0);
	}
	dp[0][0][0][0]=1;
	rep(I,25)rep(n,24)rep(inv,12)rep(dec,12)rep(i,13)rep(j,12)rep(k,12)if(n+i<30 && inv+j<12 && dec+k<12) dp[I+1][n+i][inv+j][dec+k]+=dp[I][n][inv][dec]*cnts[i][j][k];
	TT{
		cauto n=read();
		cauto inv=read();
		cauto dec=read();
		mint re=0;
		rep(a,30)rep(b,30)if(b<=n){
			auto v=dp[a][b][inv][dec];
			cauto rem=n-b;
			mint binom=1;
			// binom(rem+a, a)
			rep(i,a) binom*=rem+i+1;
			binom*=finv[a];
			if(v.v) out(a,b,v,binom,1);
			re+=v*binom;
		}
		cout<<re NL;
	}
