...
constexpr long long L = 1 << 20;

int f(int r, int n) {
    if (n < 0 or r < 0) return 0;

    static map<pair<int, int>, int> cache;
    pair<int, int> key { r, n };
    ...
    const int siz = dig.size();
 
    vector dp(2, vector<int8_t>(siz + 1));
    dp[0][0] = 1;
    rep(i, siz) {
        rep(lt, 2) {
            if (dp[lt][i] == 0) continue;
            rep(d, 2) {
                if (kth_bit(n, siz - 1 - i) == 0 and d == 1) continue;
                if (lt == 0 and d == 1 and dig[i] == 0) continue;
                int nlt = lt | (d == 0 and dig[i] == 1);
                dp[nlt][i + 1] ^= dp[lt][i];
            }
        }
    }
 
    return cache[key] = dp[0][siz] ^ dp[1][siz];
}
