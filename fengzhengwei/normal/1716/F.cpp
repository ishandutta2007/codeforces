...
//		for(int i=0;i<=n;i++)
//		{
//			res+=ksm(i,k)*ksm(a,i)%mod*ksm(b,n-i)%mod*c[n][i]%mod;
//		}
//		cout<<res%mod<<"\n";
		ll pw=1,ipw=1;
		Iv[0]=1;
		for(int j=1;j<=k;j++)
			Iv[j]=Iv[j-1]*(n-j+1)%mod;
		ll im=ksm(m,mod-2),nw=ksm(m,n);
		for(int j=0;j<=k;j++)
		{
			ans=(ans+S[k][j]*Iv[j]%mod*ni[j]%mod*pw%mod*nw%mod*ipw%mod*jiec[j])%mod;
			(pw*=a)%=mod,(ipw*=im)%=mod;
		}
		cout<<ans<<"\n";
	}
