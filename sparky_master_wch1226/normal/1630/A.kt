...
fun main(){
    solve.cases{
        ...
        val things = (k shr 1) * 2

        if(k == 0){
            for(i in 0 until n){
                if(i and 1 == 1) continue
                ret.add(Pair(i,n-1-i))
            }
        }else if(k == 1){
            ret.add(Pair(1,n-1))
            ret.add(Pair(0,n-2))
        ...
        var total = 0
        for(c in ret){
            put("${c.first} ${c.second}")
            total += c.first and c.second
        }
//        assert(total == k )
//        assert(ret.size == n/2)
//        assert(ret.flatMap { listOf(it.first,it.second) }.sorted() == (0 until n).toList())
    }
    done()
}
