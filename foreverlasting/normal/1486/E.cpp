...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        for(res i=1;i<=m;i++){
            ...
            G[u].pb(mp(v+n*w,0));
            for(res j=1;j<=50;j++)G[u+n*j].pb(mp(v,w));
            swap(u,v),G[u].pb(mp(v+n*w,0));
            for(res j=1;j<=50;j++)G[u+n*j].pb(mp(v,w));
        }
        ...
        while(!Q.empty()){
            ...
            vis[x]=1;
            for(auto [tox,c]:G[x]){
                res va=((x-1)/n+c)*((x-1)/n+c);
                if(dis[tox]>dis[x]+va)dis[tox]=dis[x]+va,Q.push(mp(dis[tox],tox));
            }
        }
        for(res i=1;i<=n;i++)printf("%d ",dis[i]<=1000000000?dis[i]:-1);
    }
}
int main(){
//    srand(time(0));
