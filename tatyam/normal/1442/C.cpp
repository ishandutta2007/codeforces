...
int main(){
    LL(n,m);
    UnWeightedGraph g1(n),g2(n);
    g1.input_directed_graph(m);
    rep(n)each(j,g1[i])g2.add_directed_edge(j,i);
    vv(pll,cost,n,22,{LINF,LINF});
    pq<array<ll,4>>q;
    q.push({0,0,0,0});
    cost[0][0]={0,0};
    while(q.size()){
        auto[tr,c,at,x]=q.top();
        q.pop();
        if(x!=21){
            if(chmin(cost[at][x+1],{tr+1,c}))q.push({tr+1,c,at,x+1});
        }
        else{
            if(chmin(cost[at][20],{tr+1,c}))q.push({tr+1,c,at,20});
        }
        if(tr%2==0){
            each(i,g1[at])if(chmin(cost[i][x],{tr,c+1}))q.push({tr,c+1,i,x});
        }
        else{
            each(i,g2[at])if(chmin(cost[i][x],{tr,c+1}))q.push({tr,c+1,i,x});
        }
    }
    Modint ans;
    ans.num=-1;
    rep(20)if(cost.back()[i]!=pll{LINF,LINF}){
        const auto[tr,c]=cost.back()[i];
        chmin(ans,c+2_M .pow(tr)-1);
    }
    if(ans.num!=-1)return out(ans);
    const auto[tr,c]=min(cost.back()[20],cost.back()[21]);
    out(c+2_M .pow(tr)-1);
}