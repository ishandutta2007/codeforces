...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	ll re=0;
	rep(i,n)if(uf0.root(i)==i && uf0.size(i)>1) re+=uf0.size(i)*(uf0.size(i)-1ll);
	out(re,1);
	rep(i,n)if(uf1.root(i)==i && uf1.size(i)>1) re+=uf1.size(i)*(uf1.size(i)-1ll);
	out(re,1);
	rep(i,n)if(uf0.size(i)>1 && uf1.size(i)>1){
		re+=(uf0.size(i)-1ll)*(uf1.size(i)-1ll);
	}
	cout<<re<<endl;
  return 0;
}