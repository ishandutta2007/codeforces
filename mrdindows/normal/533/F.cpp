...
bool w[30];
vector<int> getEntries(int a, int b)
{
    vector<int> res;
    vector<int> &s = difs[a];
    vector<int> t = dift[b];
    ...
    for (int i = n + 1; i < v.size(); ++i){
        if (v[i] == n){
            int e = i - n - n;
            res.push_back(poss[a][e]);
        }
    }
    return res;
}
vector<pii> ps[200010];
bool paired[30][30];
int main()
{
    int n, m;
    ...
    for (int i = 0; i < 26; ++i){
        for (int j = 0; j < 26; ++j){
            if (useds[i] && usedt[j]){
                ents[i][j] = getEntries(i, j);
                int e = post[j][0];
                for (int &x : ents[i][j]){
                    x -= e;
                    if (x >= 0 && x + t.size() <= s.size()){
                        ps[x].push_back({i,j});
                    }
                }
                //DBN(i, j, ents[i][j]);
    ...
    for (int i = 0; i < n; ++i){
        ...
        int k = ps[i].size();
        if (k != mc) continue;
        memset(paired, 0, 30*30);
        memset(w, 0, 30);
        for (auto p : ps[i]){
            paired[p.first][p.second] = true;
            if (w[p.first]) {
                ok = false;
                break;
            }
        ...
        if (!ok) continue;
        for (auto p : ps[i]){
            if (!paired[p.second][p.first] && usedt[p.first]){
                ok = false;
                break;
            }
        }
