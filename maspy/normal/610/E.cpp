...
void solve() {
  ...
  E unit;
  unit.left = -1, unit.right = -1, unit.sz = 0;
  FOR(k, K * K) unit.CNT[k] = 0;
  ...
  SegmentTree<E, OP> seg(
    [&](E x, E y) -> E {
      ...
      FOR(a, K) FOR(b, K) {
        ll k = a * K + b;
        z.CNT[k] = x.CNT[k] + y.CNT[k];
        if (a == x.right && b == y.left) z.CNT[k]++;
      }
      return z;
    },
    [&](E x, OP y) -> E {
  ...
  FOR(_, Q) {
    ...
    if (tt == 1) {
      ...
      auto t = seg.fold(0, N);
      ll ANS = 1;
      FOR(a, K) FOR(b, K) if (pos[a] >= pos[b]) {
        ll k = a * K + b;
        ANS += t.CNT[k];
      }
      print(ANS);
