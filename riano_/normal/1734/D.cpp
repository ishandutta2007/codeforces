...
int main(){
    ...
    ll T; cin >> T;
    rep(ii,T){
        ll N,K; cin >> N >> K; string ans = "NO";
        ll a[N+2]; a[0] = 2e18; a[N+1] = 2e18;
        rep(i,N) cin >> a[i+1];
        vector<Pr> f,b;
        ll sum = 0,mn = 0;
        for(int i=K+1;i<=N+1;i++){
            ...
            chmin(mn,sum);
            if(sum>=0){
                f.emplace_back(mn,sum); mn = 0; sum = 0;
            }
        }
        for(int i=K-1;i>=0;i--){
            sum += a[i];
            chmin(mn,sum);
            if(sum>=0){
                b.emplace_back(mn,sum); mn = 0; sum = 0;
            }
        }
        int j = 0,k = 0; ll nw = a[K];
        while(1){
            if(j==b.size()||k==f.size()){
                ans = "YES"; break;
            }
            if(nw+b[j].first>=0){
                nw += b[j].second; j++; 
