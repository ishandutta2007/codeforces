...
int main(){
    ...
    rep(zz,T){
        ...
        rep(i,2*n){
            if(!flag[i])c.push_back(i);
        }
        int m = c.size();
        rep(i,m/2){
            s.push_back(MP(c[i],c[i+m/2]));
        }
        int res = 0;
        rep(i,n){
            rep(j,i){
                if(s[i].first > s[j].first&& s[j].second > s[i].first &&s[i].second > s[j].second){
                    res++;
                }
                if(s[i].first < s[j].first&& s[j].first < s[i].second &&s[i].second < s[j].second){
                    res++;
                }
            }
        }
