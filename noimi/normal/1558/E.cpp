...
int main() {
    TEST {
        ...
        while(ng + 1 < ok) {
            ...
            rep(_, n) {
                vi tmp(n);
                auto dfs = REC([&](auto &&f, int x, int p, ll A) -> bool {
                    // dump(x, p, A, used);
                    tmp[x] = true;
                    if(A > power) {
                        for(auto e : g[x]) {
                            if(e == p) continue;
                            if(used[e] or tmp[e]) return true;
                        }
                    }
                    for(auto e : g[x]) {
                        if(e == p) continue;
                        if(!used[e] and a[e] < A) {
                            if(f(e, x, A + (used[e] ? 0 : b[e]))) {
                                power += b[e];
                                if(!used[e]) {
                                    used[e] = true;
                                    cnt++;
                ...
                rep(i, n) {
                    if(used[i]) {
                        if(dfs(i, -1, power)) ch = true;
                    }
                }
                if(!ch or cnt == n) break;
            }
            (cnt == n ? ok : ng) = x;
        }
        OUT(ok);
