...
int main(int argc, char **argv){
  ...
  B[1]=1;
  B[2]=10;
  auto sumc=[](int a,int c){
              assert(a>=0 && a<11 && c>=-11);
              c=(c+11)%11;
              int r=0;
              forn(i,c) r+=a+i;
              return r%11;
            };
  for(int n=3;n<sz(B);++n) B[n]=(B[n-1]+sumc(B[n-2],B[n-1]-B[n-2]))%11;
  //dbg(B); // [1,1,   10,0, 10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0,10,0]

  LL ans=0;
  vector<array<array<int,11>,2>> W(sz(S));
  memset(&W[0],-1,sz(W)*sizeof(W[0]));
  vector<int> T;
  for(int i0=sz(S)-1;i0>=0;--i0){
    if(S[i0]=='0') continue;
    int p=S[i0]-'0';
    T.clear();
    T.pb(-1);
    int r=i0;
    for(int i=i0+1;i<sz(S);++i){
      assert(p>=0 && p<11);
      int v=S[i]-'0';
      if(v>=p) break;
      dbg(i0,i,p);
      if(i-i0>2 && W[i][(i-i0)%2][p]!=-1){
        r=W[i][(i-i0)%2][p];
        break;
      }
      T.pb(p);
      r=i;
      p=(B[i+1-i0]+sumc(B[i-i0],p-B[i-i0]) + v)%11;
    }
    ans+=r-i0+1;
    for(int i=i0+3;i-i0<sz(T);++i) W[i][(i-i0)%2][T[i-i0]]=r;
  }
  cout<<ans;
