...
int main()
{
    ...
    vc<ll> pre(res.size()),suf(res.size());
    int cc=0;
    for(int i=0;i<res.size();i++)
    {
        if(i)
            pre[i]=(ll)cc*(res[i].first-res[i-1].first)+pre[i-1];
        cc+=res[i].second;
    }
    cc=0;
    for(int i=res.size()-1;i>=0;i--)
    {
        if(i!=res.size()-1) suf[i]=suf[i+1]+1LL*cc*(res[i+1].first-res[i].first);
        cc+=res[i].second;
    }
    int L=0;
    for(int i=0;i<res.size();i++)
    {
        int cur=res[i].second,R=n-L-cur;
        int curk=k-cur;
        //dbg(res[i].first,pre[i],suf[i],L,curk,R);
        if(curk<=0)
        {
            ans=0;
            break;
        }
        ans=min({ans,pre[i]+(L>=curk?(curk-L):suf[i]-(R-curk+L)),suf[i]+(R>=curk?(curk-R):pre[i]-(L-curk+R))});
        L+=res[i].second;
    }
    print(ans);
}