...
int main() {
	CASE(t) {
		...
		const int FULL = 1 << (__lg(n) + 1);
		vector<pii> best(FULL, { -1, -1 });

		auto push = [&](pii& v, int val) {
			if (v.first == val || v.second == val)
				return;
		...
		for (int mask = 1; mask < FULL; mask++) {
			...
			while (tmp > 0) {
				...
				push(best[mask], best[mask ^ lowbit].first);
				push(best[mask], best[mask ^ lowbit].second);
			}
		}

		ll ans = LLONG_MIN;
		for (int mask = 0; mask < FULL; mask++) {
			if (best[mask].second == -1) continue;
			chkmax(ans, 1LL * best[mask].first * best[mask].second - k * mask);
		}
