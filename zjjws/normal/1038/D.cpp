...
int main()
{
    rin>>n;for(int i=1,x;i<=n;i++){rin>>x;sum+=(x<0?-x:x);s+=x;if(x<0)b.push_back(x);if(x>0)a.push_back(x);if(x==0)cutt++;}
    sort(a.begin(),a.end());sort(b.begin(),b.end());
    if(n==1){printf("%lld\n",s);return 0;}
    if(a.size()==n){printf("%lld\n",sum-(a.front()<<1));return 0;}
    if(b.size()==n){printf("%lld\n",sum+(b.back()<<1));return 0;}
    printf("%lld\n",sum);
    return 0;
}