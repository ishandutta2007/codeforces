...
int main()
{
    scanf("%d %d %d %d",&n,&m,&bawah,&atas);
    REPN(i,n) scanf("%d",&data1[i]);
    REPN(i,m) scanf("%d",&data2[i]);
    SORTA(data1,n);
    SORTA(data2,m);
    REPN(i,n)
    {
        //printf("==%d %d\n",i,ix);
        if (data1[i] + atas < data2[ix]) continue;
        while (ix <= m && data1[i] - bawah > data2[ix]) ix++;
        if (ix <= m)
        {
            if (data1[i] + atas >= data2[ix]) 
            {
                ans.pb(mp(i,ix));
                ++ix;
            }
