...
const int maxn = 3e5 + 100, maxw = (1 << 20) + 5, inf = 1e9 + 100, LG = 18;
...
void dfs(int v, int p = -1) {
    auto it = g.lower_bound({seg[v].fr, 0});
    vector<pair<int, int>> who;
    if (it == g.end() || !anc(v, it->sc)) {
        who.push_back({v, 1});
        if (!g.empty() && it != g.begin()) {
            it = prev(it);
            if (anc(it->sc, v)) {
                who.push_back({it->sc, -1});
                g.erase(it);
            }
        }
        g.insert({seg[v].fr, v});
    }
    setmax(ans, (int)g.size());
    for (int i : e[v])
        if (i != p) {
            dfs(i, v);
        }
