...
//		FastScanner fs = new FastScanner();
		try {
			File writer=new File("input.txt");
			Scanner fs=new Scanner(writer);
			...
			for (int test_case = 1; test_case <= t; test_case++) {
				...
				for(int i =0 ; i < n; i++) {
					if(i == 0) {
						if(a[i]<=0) oddCount[i]++;
						if(a[i]>=0) evenCount[i]++;
					} else {
						oddCount[i]=oddCount[i-1];
						evenCount[i]=evenCount[i-1];
						if(a[i]<=0) oddCount[i]=oddCount[i-1]+1;
						if(a[i]>=0) evenCount[i]=evenCount[i-1]+1;
					}
				}
				
				int ans = Integer.MAX_VALUE;
				for(int i = 0; i < n - 1; i++) {
					int toChange=evenCount[i];
					toChange += oddCount[n-1]-oddCount[i];
					
					ans = Math.min(ans, toChange);
				}
				
				fw.write(new Integer(ans).toString());
				fw.close();
			}
		} catch(Exception E) {
