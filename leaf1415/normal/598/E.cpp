...
int main(void)
{
	...
	rep(i, 1, 30) rep(j, 1, 30) rep(k, 1, 50){
		if(i*j == k) continue;
		dp[i][j][k] = inf;
		rep(s, 1, i-1) rep(t, 0, k) chmin(dp[i][j][k], j*j + dp[s][j][t] + dp[i-s][j][k-t]);
		rep(s, 1, j-1) rep(t, 0, k) chmin(dp[i][j][k], i*i + dp[i][s][t] + dp[i][j-s][k-t]);
	}
