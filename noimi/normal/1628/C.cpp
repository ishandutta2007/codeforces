...
int main() {
    ...
    TEST {
        ...
        REC([&](auto &&f, int i, int j, int ni, int nj) -> void {
            if(i == ni) return;
            if(i + 2 == ni) {
                b[i][j] = b[i][j + 1] = 1;
                return;
            }
            rep(k, j, nj) b[i][k] = b[ni - 2][k] = 1;
            rep(k, j + 1, nj - 1) b[i + 1][k] = b[ni - 3][k] = 1;
            rep(t, i + 2, ni - 3) {
                int x = ((t - i) & 1);
                rep(s, 2) b[t][j + s + x] = b[t][nj - 1 - s - x] = 1;
            }
            if(ni - i >= 8) { f(i + 4, j + 4, ni - 4, nj - 4); }
        })
        (0, 0, n, n);

        rep(i, n) rep(j, n) if(b[i][j]) ans ^= a[i][j];
        ...
        // vector<pair<i128, i128>> b;
        // rep(i, n) rep(j, n) {
        //     i128 t = 0;
        //     fore(x, y, G.get(i, j)) { t |= i128(1) << (x * n + y); }
        //     i128 s = i128(1) << i * n + j;
        //     fore(x, y, b) {
        //         if(chmin(t, t ^ x)) s ^= y;
        //     }
