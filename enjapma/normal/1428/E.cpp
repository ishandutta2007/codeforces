...
ll cost(point a) {
    ll low = a.size / a.num;
    ll high = (a.size + a.num - 1) / a.num;
    if (low == high) {
        return low * low * a.num;
    }
    ll high_c = a.size % a.num;
    ll low_c = a.num - high_c;
    return high_c * high * high + low_c * low * low;
}
