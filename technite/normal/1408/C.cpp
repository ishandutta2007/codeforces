...
int main()
{
    ...
    TT
    {
        long double n,l;
        cin>>n>>l;
        vector<long double> v(n+2,0),first(n+2,0);
        for(int i=1;i<=n;i++)
        {
            cin>>v[i];
        }
        v[n+1]=l;
        long double speed=1,last=0;

        for(int i=1;i<=n+1;i++)
        {
            long double time= (v[i]-v[i-1])/speed;
            first[i]=first[i-1]+time;
            speed++;
        }
        long double time=0,answer=0;
        last=l;
        speed=1;
        for(int i=n;i>=0;i--)
        {
            long double temp =  (v[i+1]-v[i])/speed;    
            if(temp+time>first[i])
            {
                long double diff=first[i]-time;
                long double coor=v[i+1]-speed*diff;
                long double speed1=i+1,speed2=speed;
                answer=first[i] + (coor-v[i])/(speed1+speed2);
                //cout<<fixed<<setprecision(30)<<first[i]<<" "<<(coor-v[i])/(speed1+speed2)<<'\n';
                break;
            }
            time+=temp;
            speed++;
        }
        cout<<fixed<<setprecision(30)<<answer<<'\n';
    }   
}