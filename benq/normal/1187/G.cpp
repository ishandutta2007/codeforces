...
void tri(int x, int y) {
    F0R(j,105) FOR(a,1,k+1) M.addEdge(hsh(x,j),hsh(y,j+1),1,c+(2*a-1)*d);
}
...
int main() {
    ...
    F0R(i,k) {
        int a; re(a);
        M.addEdge(0,hsh(a,0),1,0);
    }
    FOR(i,2,n+1) F0R(j,105) M.addEdge(hsh(i,j),hsh(i,j+1),MOD,c);
    F0R(i,m) {
        int x,y; re(x,y);
        tri(x,y); tri(y,x);
    }
    ps(M.minCostFlow(0,1).s);
}
