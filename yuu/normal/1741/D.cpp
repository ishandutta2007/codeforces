...
tuple<int, int, int> solve(const int l, const int r) {
    if (l == r) return {0, p[l], p[l]};
    int m = (l + r) / 2;
    auto [l_ans, l_low, l_high] = solve(l, m);
    if (l_ans == -1) return {-1, 0, 0};
    auto [r_ans, r_low, r_high] = solve(m + 1, r);
    if (r_ans == -1) return {-1, 0, 0};
    if (l_high < r_low) return {l_ans + r_ans, l_low, r_high};
    else if (r_high < l_low) return {l_ans + r_ans + 1, r_low, l_high};
    else return {-1, 0, 0};
}

void solve() {
    cin >> m;
    p | ARRAY | 1 | m | ARRAY_INPUT;
    auto [ans, low, high] = solve(1, m);
    cout << ans << '\n';
}
