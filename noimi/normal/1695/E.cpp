...
int main() {
    ...
    rep(i, n * 2) if(uf.size(i) == 2) drop(-1);
    vi used(si(v));
    vvc<int> two, t1, t2;
    ...
    rep(i, n * 2) {
        ...
        int k = REC([&](auto &&f, int x, int p) -> int {
            // dump(x, p, two);
            used[x] = true;
            int r = -1;
            fore(e, g[x]) {
            ...
            if(r != -1) {
                if(p == -1) {
                    auto w = two.back();
                    two.pop_back();
                    if(count(all(w), v[r]) == 0) {
                        swap(x, r);
                        if(count(all(w), v[r]) == 0) exit(1);
                    }
                    if(w[1] == v[r])
                        t1.eb(vi{w[1], w[0], w[2], v[x]});
                    else if(w[0] == v[r])
                        t2.eb(vi{v[x], w[0], w[1], w[2]});
                    else if(w[2] == v[r])
                        t2.eb(vi{w[0], w[1], w[2], v[x]});
                    else
                        exit(1);
                } else {
                    two.eb(vi{v[r], v[x], v[p]});
                }
                return -1;
            }
            return x;
    ...
    vec(string, x, 2, string(n, 'A'));
    vec(string, y, 2, string(n, 'A'));
    ...
    while(!empty(two)) {
        ...
        two.pop_back();
        ans[0][j] = s[0];
        ans[1][j] = ans[0][j + 1] = s[1];
        ans[1][j + 1] = s[2];
        rep(i, 2) x[i][j] = 'L', x[i][j + 1] = 'R';
        rep(jj, j, j + 2) y[0][jj] = 'U', y[1][jj] = 'D';
        j += 2;
    }
    ...
    while(!empty(t1)) {
        ...
        ans[1][j + 1] = s[2];
        ans[0][j + 2] = s[3];
        ans[1][j] = ans[0][j + 1] = ans[1][j + 2] = s[0];
        x[0][j] = 'U', x[1][j] = 'D';
        rep(i, 2) x[i][j + 1] = 'L', x[i][j + 2] = 'R';

        y[0][j + 2] = 'U', y[1][j + 2] = 'D';
        rep(i, 2) y[i][j] = 'L', y[i][j + 1] = 'R';
    ...
    auto f = [&](auto t) {
        vpi res;
        rep(i, 2) rep(j, n) {
            if(t[i][j] == 'L') res.eb(minmax(ans[i][j], ans[i][j + 1]));
            if(t[i][j] == 'U') res.eb(minmax(ans[i][j], ans[i + 1][j]));
        }
        return res;
    };
    tq = f(x), sq = f(y);
