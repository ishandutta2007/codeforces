...
main() {
    INT(n, k);
    constexpr mint p = 10000000 + 19;
    vi l(n), r(n, -1), w(n), v(n);
    rep(i, n) { cin >> v[i] >> w[i]; }
    INT(Q);
    auto merge = [&](int id, int x) {
        rep3(i, k, w[x]) { chmax(seg[id][i], seg[id][i - w[x]] + v[x]); }
    };
    auto ans = [&](int id) {
        static constexpr mint p = 10000000 + 19;
        mint res;
        rep3(i, k, 1)(res *= p) += seg[id][i];
        return res;
    };
    int q = 1;
    rep(i, Q) {
    ...
    rep(i, si(r)) if(r[i] == -1) r[i] = q;
    int N = (1 << (32 - __builtin_clz(q - 1)));
    vv(int, dp, N * 2);
    rep(i, si(r)) {
        int tl = l[i], tr = r[i];
        for(int d = tr - tl; d >= 1; d = tr - tl) {
            int L = tl | ((1U << 31) >> __builtin_clz(d));
    ...
    rep2(i, 1, N * 2 - 1) {
        rep(j, k + 1) seg[i][j] = seg[i >> 1][j];
        for(auto &e : dp[i]) { merge(i, e); }
    }
    rep2(i, 1, q - 1) cout << ans(i + N) << endl;
}