...
ll c[100005];
vector<ll> G[100005];
map<ll, ll> mp[100005], mp2[100005];
ll ans[100005];

void merge(ll s, ll t)
{
	for(auto p : mp[t]){
		mp2[s][mp[s][p.first]] -= p.first;
...
P dfs(int v, int p)
{
	...
	rep(i, 1, sz(vec)-1){
		merge(vec[0].second, vec[i].second);
		vec[0].first += vec[i].first;
	}
	ans[v] = mp2[vec[0].second].rbegin()->second;
...
int main(void)
{
	...
	rep(i, 1, n) mp[i][c[i]] = 1, mp2[i][1] = c[i];
	dfs(1, -1);
	outa(ans, 1, n);
