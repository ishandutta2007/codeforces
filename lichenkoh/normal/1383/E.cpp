...
class KeySum {
public:
  stack<pair<int,mint> > S;
  mint bsum=0,ksum=0;
  void adjust(int key) {
    while(!S.empty() && S.top().fst <= key) {
      bsum-=S.top().snd;
      ksum-=S.top().snd * S.top().fst;
      S.pop();
    }    
  }
  void ins(int key, mint val) {
    adjust(key);
    mint old_bsum=bsum;
    mint new_b=val-old_bsum;
    S.push(MP(key,new_b));
    bsum+=new_b;
    ksum+=new_b*key;
  }
