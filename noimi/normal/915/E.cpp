...
main() {
    ...
    se.emplace(pii(0, 0));
    const int mx = 2e9;
    rep(_, 4) se.emplace(pii(mx, mx + _));
    int ans = n;
    auto ins = [](pii x, pii y) { return y.fi <= x.fi and x.se <= y.se; };
    while(Q--) {
        INT(l, r, k);
        r++;
        pii now(l, r);
        ...
        if(k == 1) {
            ...
            rep(_, 2) {
                while(max(l, it->fi) < min(r, it->se)) {
                    int a[4];
                    a[0] = l, a[1] = it->fi, a[2] = r, a[3] = it->se;
                    sort(a, a + 4);
                    rep(i, 3) if(ins(pii(a[i], a[i + 1]), *it) and !ins(pii(a[i], a[i + 1]), now)) {
                        if(a[i] != a[i + 1]) nxt.eb(a[i], a[i + 1]);
                    }
                    ans += min(r, it->se) - max(l, it->fi);
                    it = se.erase(it);
                }
                it++;
            }
