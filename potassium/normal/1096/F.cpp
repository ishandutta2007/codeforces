...
struct BIT{
	...
	inline void set(int id, int val) {int d = val - query(id,id); if (d != 0) update(id,d);}
	inline void update(int id, int val) {while (id < size) {arr[id] = func(arr[id],val); id += (id & -id);}}
	inline long long query_from1(int id) {long long ans = 0; while (id > 0) {ans = func(ans,arr[id]); id -= (id & -id);} return ans;}
	inline long long query(int l, int r) {if (l > r) return 0; return revf(query_from1(r),query_from1(l-1));}
	inline BIT(int N) {arr.resize(N+1,0); size = N+1;}
	inline BIT(vector<int>& v) {arr.resize(v.size()+1,0); size = v.size()+1; for (int i = 1; i <= v.size(); i++) arr[i] = v[i-1]; for (int i = 1; i <= v.size(); i++) {int idx = i + (i & -i); if (idx <= v.size()) arr[idx] = func(arr[idx],arr[i]);}}
};
	...
	if (del.size() != 0) {
		...
		for (int i = 0; i < n; i++) {
			if (p[i] == -1)
				...
				cur += upper_bound(all(del), p[i]) - del.begin();
		}
		...
		for (int i = n-1; i >= 0; i--) {
			if (p[i] == -1)
				...
				cur += del.end() - lower_bound(all(del), p[i]);
		}
