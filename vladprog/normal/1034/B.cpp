...
int my(int n,int m)
{
    int sub[]={0,1,2,3,2,1};
    if(n==1||m==1)
        return n*m-sub[n*m%6];
    if((n==2||m==2)&&(n==7||m==7))
        return 12;
    set<pii> bad;
    #define dist(x,y) (max(abs(x-1),abs(x-n))+max(abs(y-1),abs(y-m)))
    #define is_bad(x,y) (1<=x&&x<=n&&1<=y&&y<=m&&dist(x,y)<3)
    #define proc(x,y) if(is_bad(x,y)) bad.insert({x,y})
    fort1(x,1,3) fort1(y,1,3) proc(x,y);
    fort1(x,1,3) fort1(y,m-2,m) proc(x,y);
    fort1(x,n-2,n) fort1(y,1,3) proc(x,y);
    fort1(x,n-2,n) fort1(y,m-2,m) proc(x,y);
    int ans=n*m-sz(bad);
    ans-=ans%2;
    return ans;
}
