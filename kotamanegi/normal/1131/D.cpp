...
void solve() {
    ...
    REP(i, n) {
        REP(q, m) {
            if (inputs[i][q] == '>') {
                vertexs[uf(n+q)].push_back(uf(i));
                cnt[uf(i)]++;
            }
            else if (inputs[i][q] == '<') {
                vertexs[uf(i)].push_back(uf(n + q));
                cnt[uf(n+q)]++;
            }
        }
    }
    ...
    while (nexts.empty() == false) {
        ...
        REP(i, vertexs[now].size()) {
            cnt[vertexs[now][i]]--;
            ans[vertexs[now][i]] = max(ans[vertexs[now][i]], ans[now] + 1);
            if (cnt[vertexs[now][i]] == 0) {
                nexts.push(vertexs[now][i]);
            }
        }
    }
    REP(i, n+m) {
