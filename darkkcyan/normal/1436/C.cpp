...
ans_type solve() {
    ...
    while (l < r) {
        ...
        clog << db(need_big) << db(need_small) << db(l) << db(r) << endl;
    }
    
    clog << db(need_big) << db(need_small) << db(l) << db(r) << endl;
    if (need_small > x - 1) return 0;
    if (need_big > n - x) return 0;
    llong ans = 1;
    for (int i = x - need_small; i < x; ++i) (ans *= i) %= mod;
    clog << ans << endl;
    for (int i = n - x - need_big + 1; i <= n - x; ++i) (ans *= i) %= mod;
    clog << ans << endl;
    int rest = n - need_big - need_small - 1;
    rep1(i, rest) (ans *= i) %= mod;
