...
struct Info{ int t,c; };
Info info[MAXN][MAXN];
bool allFreeMode = false;

void Prepare(){
	static queue<pii> q, q2;
	static int isFree[MAXN][MAXN];
	For(i,n)
		For(j,m){
			bool fuck = false;
			For0(k,4){
	...
	while(!q.empty()){
		...
		while(!q.empty()){
			...
			For0(k,4){
				int vx = x + dx4[k], vy = y + dy4[k];
				if( vx < 1 or vx > n or vy < 1 or vy > m ) continue;
				if(!isFree[vx][vy]) continue;
				if( (col[vx][vy]^(nowTime&1)) != info[x][y].c ) continue;
				info[vx][vy] = {nowTime,info[x][y].c};
				isFree[vx][vy] = false;
				q2.push({vx,vy});
			}
		}
...
int Solve( int x , int y , ll t ){
	if(allFreeMode) return col[x][y];
	if( info[x][y].t > t ) return col[x][y];
	return info[x][y].c ^ (t&1);
}
