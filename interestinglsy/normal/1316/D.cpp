...
il char GetTo( Point f , Point t ){
	if( f.x == t.x ){
		if( f.y+1 == t.y ) return 'R';
		if( f.y-1 == t.y ) return 'L';
	}
	if( f.y == t.y ){
		if( f.x+1 == t.x ) return 'D';
		if( f.x-1 == t.x ) return 'U';
	}
	cerr << "Fuck!" << endl;
	assert(0);
}
...
int main(){
	...
	Read(n);
	For(i,n) For(j,n) Read(a[i][j].x,a[i][j].y);

	For(i,n) For(j,n) if( a[i][j] == Point{i,j} ) ansl[i][j] = 'X';
	For(i,n) For(j,n) if( a[i][j] != Point{-1,-1} and ansl[a[i][j].x][a[i][j].y] != 'X' ) END("INVALID");

	auto Bfs = [&]( Point st , Point label ){
		...
		while(!q.empty()){
			...
			++cnt;
			For0(k,4){
				Point v = {u.x+dx4[k],u.y+dy4[k]};
				if( v.x < 1 or v.y < 1 or v.x > n or v.y > n ) continue;
				if( a[v.x][v.y] != label ) continue;
				if(vis[v.x][v.y]) continue;
				ansl[v.x][v.y] = GetTo(v,u);
				vis[v.x][v.y] = 1;
				q.push(v);
			}
		}
	...
	For(i,n) For(j,n) if( ansl[i][j] == 'X' ){
		Bfs({i,j},{i,j});
	}

	For(i,n) For(j,n) if( a[i][j] != Point{-1,-1} and !vis[i][j] ) END("INVALID");

	For(i,n)
		For(j,n){
			if( a[i][j] != Point{-1,-1} || vis[i][j] ) continue;
			int t = Bfs({i,j},{-1,-1});
			if( t == 1 ) END("INVALID");
			For0(k,4){
				Point v = {i+dx4[k],j+dy4[k]};
				if( a[v.x][v.y] == Point{-1,-1} ){
					ansl[i][j] = GetTo({i,j},v);
					break;
				}
			}
		}
