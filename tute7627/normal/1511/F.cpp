...
int main(){
  ...
  vs.PB("");
  rep(i,0,n){
    rep(j,0,s[i].size())vs.PB(s[i].substr(j));
  }
  auto mp=compress(vs);
  ll sz=mp.size();
  Matrix<modint>mat(sz*5,sz*5);
  rep(i,sz,5*sz){
    mat[i-sz][i]=1;
  }
  rep(i,0,sz){
    rep(j,0,n){
      ...
      bool sw=true;
      rep(o,0,np)if(p[o]!=q[o])sw=false;
      if(sw){
        mat[mp[q.substr(np)]+sz*(s[j].size()-1)][i]+=1;
        //cout<<p spa q spa q.substr(np)<<endl;
      }
    }
  }
  //cout<<mat<<endl;
