...
void solve() {
    ...
    for (int i = 0; i < n; ++i) {
        if (f[i] == 0) continue;
        givers.erase(i);
        recievers.erase(f[i] - 1);
    }
    set<int> all;
    for (int g : givers) if (recievers.find(g) != recievers.end()) all.insert(g);
    for (int a : all) {
        givers.erase(a);
        recievers.erase(a);
    }
    vector<int> g, r, a;
    for (int x : givers) g.push_back(x);
    for (int x : recievers) r.push_back(x);
    for (int x : all) a.push_back(x);
    if (a.size() > 1) {
        for (int i = 0; i < g.size(); ++i) f[g[i]] = r[i] + 1;
        for (int i = 0; i < a.size(); ++i) f[a[i]] = a[(i + 1) % a.size()] + 1;
    } else {
        if (a.size() == 1) {
            f[g[0]] = a[0] + 1;
            f[a[0]] = r[0] + 1;
        ...
        for (int i = 1; i < g.size(); ++i) f[g[i]] = r[i] + 1;
    }
    cout << f;
}