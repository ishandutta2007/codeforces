...
static int dep[MAXN], gson[MAXN], sz[MAXN], fa[MAXN];
static int in[MAXN], out[MAXN], top[MAXN];
function<void(int,int)> FindGson = [&]( int pos , int fat ){
	fa[pos] = fat;
	dep[pos] = dep[fat] + 1;
	sz[pos] = 1;
	for( int v : e[pos] ){
...
function<void(int,int,int)> LinkLine = [&]( int pos , int fa , int tp ){
...
For(qwq,q){
	...
	static int b[MAXN*2];
	Read(b,1,k);
	sort(b+1,b+1+k,[]( int a , int b ){ return in[a]<in[b]; });
	...
	b[++kk] = 1;
	sort(b+1,b+1+kk,[]( int a , int b ){ return in[a]<in[b]; });
	k = kk = unique(b+1,b+1+kk) - (b+1);
	// Print(b,1,k);

	bool failed = false;
	function<int(int&)> Process = [&]( int &ptr ){
		int u = b[ptr];
		++ptr;
		int mx = 0, secMx = -INF;
		while( ptr <= k and in[b[ptr]] <= out[u] ){
			int v = b[ptr];
			int res = Process(ptr) + dep[v] - dep[u];
			if( res >= mx ){
				secMx = mx;
				mx = res;
			}else Mymax(secMx,res);
		}
		if( mx >= 2 and secMx >= 2 ) failed = true;
		return mx;
	};
	int ptrPos = 1;
	Process(ptrPos);
