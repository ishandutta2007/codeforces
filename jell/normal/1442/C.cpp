#line 1 "codeforces-workspace\\contest\\1442\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\1442\\c\\c.cpp"
...
#line 21 "codeforces-workspace\\contest\\1442\\c\\c.cpp"
...
void main() {
  ...
  constexpr int thre = 18;

  struct info {
    int cnt = -1, dist;
    bool operator<(info rhs) const {
      if (cnt < 0) return false;
      if (rhs.cnt < 0) return true;

      if (cnt >= thre) {
        assert(rhs.cnt >= thre);
        return pair(cnt, dist) < pair(rhs.cnt, rhs.dist);
      } else {
        assert(cnt == rhs.cnt);
        return pair(dist, cnt) < pair(rhs.dist, rhs.cnt);
      }
    }
  };

  auto dist = make_vector<info>({n, thre + 1, 2});
  ...
  while (!q.empty()) {
    ...
    q.pop();
    auto nd = dist[v][c][s];
    auto &cg = (s & 1) ? rg[v] : g[v];
    auto &rcg = (s & 1) ? g[v] : rg[v];
    nd.dist++;
    for (auto &&u : cg) {
      if (chle(dist[u][c][s], nd)) {
        q.emplace(u, c, s);
      }
    }
    ...
    nd.cnt++;
    for (auto &&u : rcg) {
      if (chle(dist[u][min(c + 1, thre)][!s], nd)) {
        q.emplace(u, min(c + 1, thre), !s);
      }
    }
  }

  int ans1 = inf;
  for (auto i = 0, p2 = 1; i != thre; ++i, p2 *= 2) {
    for (auto j : {0, 1}) {
      auto nd = dist[n - 1][i][j];
      if (~nd.cnt) {
        assert(i == nd.cnt);
        chle(ans1, nd.dist + p2);
      }
    }
  }

  auto [lc, ld] = min(dist[n - 1][thre][0], dist[n - 1][thre][1]);
  if (ans1 != inf) {
    if (lc < 0) {
      cout << ans1 - 1 << eol;
    } else {
