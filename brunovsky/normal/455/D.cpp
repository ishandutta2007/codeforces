...
int main() {
    ...
    int N;
    cin >> N;
    vector<Pointer*> byval(N + 1);
    for (int i = 0; i < N; i++) {
        int a;
        cin >> a;
        Splay* node = new Splay(a);
        Pointer* other = new Pointer(node);
        node->other = other;
        push_back(root, node);
        push_back(byval[a], other);
    }
    int Q;
    cin >> Q;
    int old = 0;
    while (Q--) {
        if (int t; cin >> t, t == 1) {
            ...
            auto node = splice_order(root, r);
            auto other = splice_item(byval[node->key], node->other);
            insert_order(root, node, l);
            insert_key(byval[node->key], other);
        } else if (t == 2) {
            int l, r, a;
            cin >> l >> r >> a;
            l = (l + old - 1) % N;
            ...
            r++;
            auto range = access_key_range(byval[a], l, r);
            old = Pointer::get_size(range);
            cout << old << '\n';
            byval[a] = range ? splay(range) : byval[a];
        } else {
            assert(false);
        }
    }
