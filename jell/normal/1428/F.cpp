#line 1 "codeforces-workspace\\contest\\1428\\f\\f.cpp"
#include <bits/extc++.h>
...
#line 9 "codeforces-workspace\\contest\\1428\\f\\f.cpp"
...
void main() {
  ...
  string s;
  cin >> n >> s;
  vector<pair<int, int>> runs(1);
  {
    int idx = 0;
    for (auto &&c : s) {
      if (c == '0')
        runs.emplace_back(idx + 1, idx + 1);
      else
        runs.back().second++;
      idx++;
    }
  ...
  i64 ans = 0;
  for (auto i = 1; i <= n; ++i) {
    runs.erase(remove_if(begin(runs), end(runs),
                         [&](auto p) { return p.second - p.first < i; }),
               end(runs));

    int pre = -1;
    for (auto p = begin(runs), q = begin(runs), r = end(runs); p != end(runs);
         ++p) {
      while (q != end(runs) && (q == p or q->second - q->first == i)) ++q;
      int nxt = n + 1;
      if (q != end(runs)) nxt = q->first + i + 1;
      auto rgt = nxt - p->second;
      auto lft = p->first - pre;
      if (p->second - p->first == i) {
        ans += (i64)lft * rgt * i;
      } else {
        auto len = p->second - p->first;
        // if (r != end(runs)) lft = p->first - (r->second - i);
        ans += i64(lft + rgt + len - 1 - i) * i;
        r = p;
      }
      pre = p->second - i;
    }
