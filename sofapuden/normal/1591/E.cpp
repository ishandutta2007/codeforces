...
void dfs(int x){
	if(M[num[x]]){
		T.erase({M[num[x]],num[x]});
	}
	M[num[x]]++;
	T.insert({M[num[x]],num[x]});

	for(auto y : queri[x]){
		int les = T.order_of_key({que[y][1],0});
		if(que[y][2]+les > (int)T.size()){
			ans[y] = -1;
			continue;
		}
		auto no = *T.find_by_order(que[y][2]-1+les);
		ans[y] = no.second;
	}
