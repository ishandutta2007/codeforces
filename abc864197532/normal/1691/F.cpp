...
int main () {
    ...
    dfs1(0, -1);
    dfs2(0, -1);
    int ans = 0, way = C(n, k) * n % mod;
    for (int i = 0; i < n; ++i) if (i) {
        test(up[i], down[i]);
        del(way, 1ll * up[i] * sz[i] % mod), del(way, 1ll * down[i] * (n - sz[i]) % mod);
        add(ans, 1ll * up[i] * sz[i] % mod * (n - sz[i]) % mod);
        add(ans, 1ll * down[i] * (n - sz[i]) % mod * sz[i] % mod);
    }
    add(ans, 1ll * way * n % mod);
    cout << ans << '\n';
}