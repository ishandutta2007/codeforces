...
ll mem(ll a, ll b)
{
	return (a % b) * (a / b + 1) * (a / b + 1) + (b - a % b) * (a / b) * (a / b);
}
ll mem(const vl& a, const vl& b, const int i)
{
	return mem(a[i], b[i]);
}
pair<ll, int> kek(ll a, ll b, const int i)
{
	return pair<ll, int>(mem(a, b) - mem(a, b + 1), i);
}
pair<ll, int> kek(const vl& a, const vl& b, const int i)
{
	return kek(a[i], b[i], i);
}
int another_main()
{
	int n, k;
