...
inline void work()
{
    rin>>x[0]>>y[0]>>xa>>ya>>xb>>yb>>X>>Y>>t;int n=0;
    for(;true;){n++;x[n]=x[n-1]*xa+xb;y[n]=y[n-1]*ya+yb;if(x[n]>1e16||y[n]>1e16)break;}
    int ans=0;
    for(int i=0;i<=n;i++)
    {
        for(int j=i;j<=n;j++)
        {
            ...
            {
                LL s_1=0,s_2=0;
                for(int _=k-1;_>=i;_--){s_1+=x[_+1]-x[_]+y[_+1]-y[_];if(s_1>t)break;}
                for(int _=k+1;_<=j;_++){s_2+=x[_]-x[_-1]+y[_]-y[_-1];if(s_2>t)break;}
                if(s_1>t||s_2>t)continue;
                LL sum=((min(s_1,s_2))<<1)+max(s_1,s_2)+abs(X-x[k])+abs(Y-y[k]);
