...
void solve() {
    ...
    int level = 0;
    while (true) {
        int mi = *min_element(b + 1, b + m + 1);
        if (mi == 1e9) break;
        level++;
        vector<int> big_pos;
        for (int i = 1; i <= m; i++)
            if (b[i] == 1e9) big_pos.emplace_back(i);

        for (int i = 1; i <= m; i++) {
            if (b[i] == 1e9) continue;
            int cost = 0;
            for (auto&& j : big_pos) cost += abs(i - j);
            if (b[i] == cost) {
                pos[level].emplace_back(i);
                b[i] = 1e9;
            }
        }
    }
    char c = 'a';
    while (level > 0) {
        if (cnt[c] >= pos[level].size()) {
            for (auto&& p : pos[level]) ans[p] = c;
            c++;
            level--;
        } else {
            c++;
