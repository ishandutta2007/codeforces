...
fun main(){
	...
	fun solve(x:Int, y:Int):Int{
		var ret = 0
		for(i in 0..minOf(x,y)){
			ret = ret modPlus (FACT.choose(x,i) modM FACT.choose(y,i))
		}
		return ret
	}
	fun fullsolve(x:Int):Int{
		var ret = 0
		for(v in 0..x step 1){
			ret = ret modPlus FACT.choose(x,v)
		}
		return ret
	}
    solve.cases{
		...
		val cutsum = LongArray(n+1)
//		val mid = (total.toDouble()) / 2

		var midcount = 0
		for(i in 0 until n){
			cutsum[i+1] = cutsum[i] + L[i]
		}
		val left = multiset()
		val right = multiset()
		...
		var ret = 1
		if(total == 0L){
			ret = ret modM fullsolve(midcount -2 )
		}else{
			ret = ret modM fullsolve(midcount)
		}

		for(v in left.st.keys){
			var x = left.st[v]!!
			var y = right.st[total - v] ?: 0

			if(v == 0L){
				x --
				y--
			}
			ret = ret modM solve(x,y)
		}
		put(ret)
