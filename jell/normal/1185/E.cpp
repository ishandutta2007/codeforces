...
void solve() {
            ...
            if(grid[i][j]!='.') {
                ...
                if(exi[p]) {
                    if(~isrow[p]) {
                        ...
                        if(x[p]==i) {
                            isrow[p]=1;
                            end[p]={min(j,y[p]),max(j,y[p])};
                        } else if(y[p]==j) {
                            isrow[p]=0;
                            end[p]={min(x[p],i),max(x[p],i)};
                        } else {
                            cout<<"NO\n";
                            return;
                        }
    ...
    for(int i=0; i<pmax; ++i) {
        if(isrow[i]) {
            for(int j=end[i].fir; j<=end[i].sec; ++j) {
                simu[x[i]][j]=i;
            }
        } else {
            for(int j=end[i].fir; j<=end[i].sec; ++j) {
    ...
    for(int i=0; i<pmax; ++i) {
        if(isrow[i]) {
            cout<<1+x[i]<<" "<<1+end[i].fir<<" "<<1+x[i]<<" "<<1+end[i].sec<<"\n";
        } else {
            cout<<1+end[i].fir<<" "<<1+y[i]<<" "<<1+end[i].sec<<" "<<1+y[i]<<"\n";
        }
    }
}
