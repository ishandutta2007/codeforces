...
main() {
    ...
    while(T--) {
        ...
        ll now = 0;
        pq<pll> P[3];
        auto dfs = [&](auto &&f, int x, int p, ll cost) -> void {
            if(p != -1 and g[x].size() == 1) sub[x] = 1;
            for(auto e : g[x])
                if(e != p) f(f, e, x, e.cost), sub[x] += sub[e];
            v[x] = pll(sub[x], cost);
            c[x] = mp[pii(x, p)];
            if(p != -1) P[c[x]].emplace(sub[x] * (cost - cost / 2), x), now += sub[x] * cost;
        };
        dfs(dfs, 0, -1, 0);
        int ans = 0;
        P[1].emplace(0, 0), P[1].emplace(0, 0), P[2].emplace(0, 0);
        while(now > S) {
            ll F = 0;
            auto [x, y] = P[1].top();
            P[1].pop();
            ...
            v[y].se /= 2;
            auto [xx, yy] = P[1].top();
            F += max(xx, v[y].fi * (v[y].se - v[y].se / 2));
            ll Se = P[2].top().fi;
            if(F >= Se) {
                P[1].emplace(v[y].fi * (v[y].se - v[y].se / 2), y);
                ans++;
                now -= x;
                continue;
            } else {
                ...
                now -= xx;
                v[yy].se /= 2;
                P[2].emplace(v[yy].fi * (v[yy].se - v[yy].se / 2), yy);
                ans += 2;
            }
        }
        cout << ans << endl;
