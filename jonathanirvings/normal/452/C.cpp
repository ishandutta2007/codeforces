...
void UP(int a,int b)
{
	FORN(i,a,b) atas.pb(i);
}

void DOWN(int a,int b)
{
	FORN(i,a,b) bawah.pb(i);
}

void print(vi x)
{
	REP(i,SIZE(x)) printf("%d ",x[i]);
	puts("");
}
...
int main()
{
	...
	{
		double ret = 1;
		//risan += C((n - 1) * m,n - i) * ((double)i / n) / C(n * m - 1, n - 1) * C(m-1,i-1);
		//printf("%.6lf\n",risan);
		//printf("%.6lf\n",C((n - 1) * m,n - i));
		atas.clear();
		bawah.clear();
		...
		FOR(j,ix,SIZE(bawah)) ret /= bawah[j];
		//ret = ret * (double)i / n;
		risan += ret;
		//printf("%.6lf\n",ret);
	}
