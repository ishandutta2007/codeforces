...
int N,M;
vector<string> s;
vector<int> p,ch,ans,g[11];
int main(){
  startupcpp();
  for(int i=0;i<10;++i)g[i].reserve(10000);
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  for(int bit=0;bit<(1<<N);++bit){
    ...
    for(int i=0;i<M;++i){
      int cnt=0;
      for(int j=0;j<N;++j)cnt+=((bit>>j)^s[j][i])&1;
      g[cnt].emplace_back(i);
    }
    int cnt=0,ret=0;
    for(auto&v:g)for(auto&x:v)ch[x]=++cnt;
    for(int i=0;i<N;++i){
      int sum=0;
      for(int j=0;j<M;++j)if(s[i][j]&1)sum+=ch[j];
      ret+=abs(sum-p[i]);
    }
    if(chmax(ansval,ret))swap(ans,ch);
  }
  output(ans);
}}