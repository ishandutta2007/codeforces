...
//i_am_noob
int n,m,a[maxn],b[maxn],q,o,ind,x,l,r,mid;
...
signed main(){
    ...
    rep(n) cin >> a[i];
    rep(m) cin >> b[i];
    rep(n) segtree.modify(1,1,maxn,1000001-a[i],maxn,-1);
    rep(m) segtree.modify(1,1,maxn,1000001-b[i],maxn,1);
    cin >> q;
    while(q--){
        cin >> o >> ind >> x;
        ind--;
        if(o==1){
            segtree.modify(1,1,maxn,1000001-a[ind],maxn,1);
            a[ind]=x;
            segtree.modify(1,1,maxn,1000001-a[ind],maxn,-1);
        }
        else{
            segtree.modify(1,1,maxn,1000001-b[ind],maxn,-1);
            b[ind]=x;
            segtree.modify(1,1,maxn,1000001-b[ind],maxn,1);
        }
        if(segtree.query(1,1,maxn,1,maxn)>=0){
            print1(-1);
            continue;
        }
        l=1,r=maxn;
        while(l<r){
            mid=l+r>>1;
            if(segtree.query(1,1,maxn,1,mid)>=0) l=mid+1;
            else r=mid;
        }
        print1(1000001-l);
    }
