...
string solve(int a,int b){
	...
	int t=(a-b)/2;
	string s="1"+string(t,'0');
	string re;
	while(re.size()<a) re+=s;
	return re.substr(0,a);
}
...
int main(){
	...
	// 	string s=solve(a,b);
	// 	map<string,int> cnt;
	// 	rep(i,s.size())reps(j,1,s.size()-i+1) ++cnt[s.substr(i,j)];
	// 	int mn=MOD;
	// 	for(auto p:cnt)if(p.Y==1)MN(mn,(int)p.X.size());
	// 	if(mn!=b){
	// 		out(a,b,mn,s,1);
	// 		return 0;
	// 	}
