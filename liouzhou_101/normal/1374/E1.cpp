...
void solve()
{
	...
	if (z.size() + x.size() < k || z.size() + y.size() < k)
		return writeln(-1);

	sort(x.begin(), x.end());
	sort(y.begin(), y.end());
	sort(z.begin(), z.end());
	vector<int> sx(x.size() + 1), sy(y.size() + 1);
	...
	int res = INF*2;
	for (int i = 0, sum = 0; i <= z.size() && i <= k; ++i)
	{
		if (i) sum += z[i - 1];
		if (k - i <= x.size() && k - i <= y.size())
		{
			freshmin(res, sum + sx[k - i] + sy[k - i]);
		}
	}
	writeln(res);
}
