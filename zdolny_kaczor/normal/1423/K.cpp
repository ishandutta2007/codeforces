...
void bru() {
	...
	for (int i = 1; i <= n; ++i) {
		...
		for (int j = 1; j <= n; ++j) if (i != j) {
			if (tri(gcd(i, j), i / gcd(i, j), j / gcd(i, j))) found = true;
		}
		if (!found) debug(imie(i));
	}
}
...
void init() {
	...
	for (int p = 2; p < nax; ++p) {
		if (is_prime[p]) {
			for (int x = p * 2; x < nax; x += p) {
				is_prime[x] = false;
			}
		}
	}
	for (int i = 1; i < nax; ++i) pi[i] = pi[i - 1] + is_prime[i];
	for (int i = 1; i < nax; ++i) {
		root[i] = root[i - 1];
		while (K(root[i] + 1) <= i) root[i]++;
	}
}
void solve() {
	int n;
