// time-limit: 3000
// problem-url: https://codeforces.com/contest/1628/problem/D2
...
using modular=Modular<1000000007>;
auto const comb=makeComb<modular>(int(1e6)+1);
...
void up(){
	...
	if(n==m){
		...
		modular _2powi=2;
		for(int const i: range(1, m+1)){
			f+=i*_2powi*comb(n-i-1, m-i);
	...
	std::cout<<
		int(
				f*modular::raw(k)/modular(2).pow(n)
		   )<<'\n';
}