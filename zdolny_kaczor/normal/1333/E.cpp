...
int main() {
	...
	REP(i, n) {
		int row = i;
		if (n == 3 && (row == 1 || row == 2)) row ^= 3;
		bool any = false;
		REP(j, n) if (ans[row][j] == 0) any = true;
		if (!any) continue;
		assert(ans[row][col] == 0);
		int last = col;
		REP(j, n) if (j != col && ans[row][j] == 0) last = j;
		ans[row][col] = timer++;
		REP(j, n) if (ans[row][j] == 0 && last != j) ans[row][j] = timer++;
		if (ans[row][last] == 0) ans[row][last] = timer++;
		col = last;
		debug(dump(ans, 0, n, 0, n));
	}
	REP(i, n) {
		REP(j, n) printf("%d ", n * n + 1 - ans[i][j]);
		puts("");
	}
	exit(0);
}
