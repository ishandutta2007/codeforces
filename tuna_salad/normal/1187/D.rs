...
mod fenwick {
	...
	impl Tree {
		pub fn new(n: usize) -> Self {
			let arr = vec![1000000000; n + 1].into_boxed_slice();
			Self { n, arr }
		}
...
fn main() {
	...
	rep!{[rin.u()]
		let n = rin.u();
		let mut get_a = || -> (Vec<usize>,Vec<usize>,Vec<i32>) {
			let b0 = rin.vi(n);
			let mut a = v!([n]);
			for i in 0..n {
				a.push( (b0[i],i) );
		...
		let (_,ap,mut a0) = get_a();
		let (b,_,mut b0) = get_a();
