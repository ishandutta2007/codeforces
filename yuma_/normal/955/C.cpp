...
ll solve(ll x){
	...
	auto it=upper_bound(vs.begin(),vs.end(),x);
	return ok+(it-vs.begin());
	}
}
int main() {
	...
	for(int mul=3;mul<=60;++mul){
		if(mul!=3&&(mul%2==0||mul%3==0))continue;

		ll kkk=powl(2e18,1.0l/mul);
		for(ll i=2;i<=kkk;++i){
			{
				ll k=1;
				for(int t=0;t<mul;++t){
	...
	vs.erase(remove_if(vs.begin(),vs.end(),[](const ll x){
		ld xx=sqrt(x);
		ll xxx=ll(round(xx));
		if(xxx*xxx==x)return true;
		else return false;
	}),vs.end());
	while(Q--){
		ll l,r;cin>>l>>r;
