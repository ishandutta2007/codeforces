...
int main(int argc, char **argv){
  ...
  ++n;
  vector<pll> A(n);
  forn(i,n-1) cin>>A[i+1].X>>A[i+1].Y;
  vector<int> D0(n);
  for(int i=n-1;i>=0;--i){
    if(i+1<n && A[i+1].X-A[i].X>=abs(A[i+1].Y-A[i].Y)) D0[i]=D0[i+1];
    else D0[i]=i;
  }
  vector<LL> D1(n,LINF);
  vector<short> D2(n);
  ...
  forn(q,n-1){
    T.clear();
    if(D1[q]!=LINF) T.pb(mp(D1[q],A[q].Y));
    if(D2[q]) T.pb(A.at(q-1));
    for(pll p:T){
      LL t=p.X+abs(p.Y-A[q+1].Y);
      if(t>A[q+1].X) continue;
      smax(t,A[q].X);
      dbg(q,p,t);
      smin(D1[q+1],t);
      for(int i=q+2;i<n && i-1<=D0[q+1];++i) if(max(A[q].X,p.X+abs(p.Y-A[i].Y))+abs(A[i].Y-A[q+1].Y)<=A[q+1].X) D2[i]=1;
    }
  }
  dbg(A);
  dbg(D1);
  dbg(D2);
  if(D1[n-1]!=LINF||D2[n-1]) cout<<"YES"; else cout<<"NO";
