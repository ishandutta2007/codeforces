...
int main () {
    ...
    {
        //suf[i] = suf[i+1] + c[i];
        suf_odd[i] = suf_odd[i+1] + (pre[i]%2==1);
        suf_even[i] = suf_even[i+1] + (pre[i]%2==0);
    }
    LL ans=0;
    for (int i=1;i<=n;++i)
    {
        ...
        for (int j=i+1;j<=tar;++j)
        {
            now += c[j];
            mx = max(mx*1LL,c[j]);
            if (now%2 == 0 && mx*2 <= now) ++ans;
        }
        if (now%2 == 0)
        {
            if (pre[i+64]%2 == 0) ans += suf_even[i+65];
            else ans += suf_odd[i+65];
        }
        else
        {
            if (pre[i+64]%2 == 0) ans += suf_odd[i+65];
            else ans += suf_even[i+65];
        }
    }
    printf("%lld\n",ans);
}