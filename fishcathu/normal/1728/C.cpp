...
{
    int n=read();
    for(int i=n;i--;u.push(read()));
    for(int i=n;i--;v.push(read()));
    int ans=0;
    for(int x=u.top(),y=v.top();!u.empty();)
    {
        if(x>y)u.pop(),u.push(h(x)),x=u.top(),++ans;
        else if(x<y)v.pop(),v.push(h(y)),y=v.top(),++ans;
        else u.pop(),v.pop(),x=u.top(),y=v.top();
    }
    cout<<ans<<'\n';
}
int main()
