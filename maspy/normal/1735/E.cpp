...
void solve() {
  ...
  FOR(b, N) {
    FOR(sgn, 2) {
      ...
      SA.erase(SA.find(A[0]));
      SB.erase(SB.find(B[b]));
      ll p_A = A[0], p_B = B[b] * (sgn == 0 ? 1 : -1);
      vi X = {0};
      ...
      while (len(SA)) {
        ...
        auto it1 = SA.find(abs(x - p_A));
        auto it2 = SB.find(abs(x - p_B));
        if (it1 == SA.end() || it2 == SB.end()) break;
        X.eb(x);
        SA.erase(it1);
        SB.erase(it2);
      }
