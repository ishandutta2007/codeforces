...
void Solve(){
	ansl.clear();
	int aSum = accumulate(a,a+MAXLG,0);
	if( aSum >= n ){
		// 
		int nLeft = n;
		int sumLeft = sum;
		...
		For(_t,n-aSum){
			...
			if( minD < INF ){
				toSel.erase(std::find(All(toSel),minD));
				toSel.Eb(Lower10(minD));
				toSel.Eb(minD-Lower10(minD));
			}else{
				for( int x : toSel ){
					if( x != 1 )
						Mymin(minD,x);
