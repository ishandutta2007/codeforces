...
void solve_case() {
    ...
    SegTree<int> seg;
    seg.init(n, 1);
    vector<vpi> changes(n);
    f0r(i, n) {
        dp[i] = seg.query(0, a[i])+1;
        if (behind[i]) {
            ckmax(dp[i], 2);
        }
        seg.set_max(a[i], dp[i]);
        if (close[i] != -1) {
            changes[close[i]].eb(a[i], dp[i]+1);
        }
