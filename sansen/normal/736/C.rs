...
type M = ModInt<Mod>;
type Map = std::collections::BTreeMap<(i32, i32), M>;
...
fn run() {
    ...
    let init = |v: usize| -> T {
        let mut map = Map::new();
        map.insert((100, 0), M::one());
        (M::one(), map)
    };
    let merge = |a: T, b: T, _c: &E| -> T {
        let mut next = (M::zero(), Map::new());
        let (x, y) = a;
        let (z, w) = b;
        // x
        next.0 += x * z;
        for (&(_, ng), &v) in w.iter() {
            if ng + 1 <= k {
                next.0 += x * v;
            }
        }
        // y
        for (&(_, ng), &v) in y.iter() {
            if ng + 1 <= k {
                *next.1.entry((1, -k - 1)).or_insert(M::zero()) += v * z;
            }
        }
        for ((a, b), y) in y {
            for (&(c, d), &w) in w.iter() {
                let p = std::cmp::min(a, c + 1);
                let q = match (a + 1 + d <= k, c + 1 + b <= k) {
                    (true, true) => -(k - std::cmp::min(a, c + 1)) - 1,
                    (true, false) => b,
                    (false, true) => d + 1,
                    (false, false) => std::cmp::max(b, d + 1),
                };
                *next.1.entry((p, q)).or_insert(M::zero()) += y * w;
            }
        }
        next
    };
