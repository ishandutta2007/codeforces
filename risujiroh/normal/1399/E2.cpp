...
int main() {
  ...
  while (tt--) {
    ...
    for_each(rbegin(g.vs), rend(g.vs), [&](int v) {
      ...
      if (v) {
        s -= leaves[v] * g.es[g.pe[v]].w;
      }
    });
    array<priority_queue<pair<ll, int>>, 3> pq;
    for (int v = 1; v < n; ++v) {
      int id = g.pe[v];
      pq[c[id]].emplace((g.es[id].w + 1) / 2 * leaves[v], v);
      g.es[id].w /= 2;
    }
    array<vector<ll>, 3> cum;
    cum[1] = cum[2] = {0};
    for (int i : {1, 2}) {
      while (not empty(pq[i])) {
        auto [d, v] = pq[i].top();
        pq[i].pop();
        if (d == 0) break;
        cum[i].push_back(cum[i].back() + d);
        int id = g.pe[v];
        pq[c[id]].emplace((g.es[id].w + 1) / 2 * leaves[v], v);
        g.es[id].w /= 2;
      }
    ...
    DEBUG(cum[2]);
    int res = 1e9;
    for (int i = 0; i < (int)size(cum[1]); ++i) {
      int j = lower_bound(begin(cum[2]), end(cum[2]), -(s + cum[1][i])) - begin(cum[2]);
      if (j == (int)size(cum[2])) continue;
      DEBUG(i, j);
      res = min(res, i + 2 * j);
    }
    cout << res << '\n';
