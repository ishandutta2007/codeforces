...
#define int ll
vector<int> adj[N];
int cnt[N], depth[N], maxC[N], sub[N];
...
void add_to(int n,int p,map<int,int>& cur){
	cur[depth[n]]++;
	for(auto e: adj[n]){
		if (e != p){
			add_to(e, n, cur);
...
void dfs2(int n,int p,bool keep){
	...
	++cnt[depth[n]];
	ans += cnt[depth[n] + K];
	// trace(ans, cnt[depth[n] + K], depth[n], depth[n] + K, n);
	for(auto e: adj[n]){
		if (e != p && e != maxC[n]){
			map<int,int> cur;
			add_to(e, n, cur);
			for(auto e: cur){
				if (2 * depth[n] + K - e.first >= 0){
					ans += cnt[2 * depth[n] + K - e.first] * 1ll * e.second;
				}
			}
