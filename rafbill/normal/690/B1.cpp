...
int main(int, char**){
  ...
  lli mini=50, maxi=0, minj=50, maxj=0;
  bool w4 = 0;
  FOR(i, n) FOR(j, n) if(A[i][j] == '4') {
  ...
  auto isin = [&](lli i, lli j) {
    return (i>=mini&&i<=maxi+1)&&(j>=minj&&j<=maxj+1);
  };
  FOR(i, n) FOR(j, n) {
    lli l = 0;
    FOR(di, 2) FOR(dj, 2) l += isin(i+di, j+dj)?1:0;
    if('0'+l != A[i][j]) {
      cout << "No" << endl;
      return 0;
    }
