...
int vis[MAXN];
int Solve( int pos ){
	static vc<int> adj; adj.clear();
	for( auto v : e[pos] )
		if(!vis[v])
			adj.Eb(v);
	random_shuffle(All(adj));

	static vc<pii> pairs; pairs.clear();
	while( adj.size() >= 2 ){
		int a = adj.back(); adj.pop_back();
		int b = adj.back(); adj.pop_back();
		pairs.Pb({a,b});
	}
	...
	vis[pos] = 1;
	if(adj.empty()){
		if(!spepos) Answer(pos);
		else{
			assert( find(All(e[pos]),spepos) != e[pos].end() );
			Answer(Ask(pos,spepos));
		}
	}else{
		if(!spepos){
			spepos = pos;
		...
		int res = Ask(adj[0],spepos);
		if( res == spepos ) Answer(spepos);
		if( res == adj[0] ){
		    spepos = 0;
		    return adj[0];
		}
...
int main(){
    cerr << "Are you mother fucker?" << endl;
	// FILEIO();
	srand(time(0));
