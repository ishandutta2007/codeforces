...
template <const int H = 2000, const int W = 2000> struct Matrix {
    int h, w;
    using B = bitset<W>;
    array<B, H> v;
    inline const B &operator[](int k) const { return v[k]; }
    inline B &operator[](int k) { return v[k]; }
    Matrix() {}
    Matrix(int h) : h(h), w(h) {}
    ...
    Matrix &operator+=(const Matrix &B) { rep(i, h) v[i] ^= B[i]; }
    Matrix &operator^=(const Matrix &B) { rep(i, h) v[i] ^= B[i]; }
    ...
    Matrix<H, W> inv() {
        ...
        for(int i = 0; i < h; ++i) {
            if(!w[i][i]) {
                for(int j = i + 1; j < h; ++j) {
                    if(w[j][i]) {
                        swap(w[j], w[i]), swap(res[j], res[i]);
                        break;
                    }
                }
            }
            for(int j = 0; j < i; ++j)
                if(w[j][i]) w[j] ^= w[i], res[j] ^= res[i];
            for(int j = i + 1; j < h; ++j)
                if(w[j][i]) w[j] ^= w[i], res[j] ^= res[i];
        }
        return res;
...
main() {
    ...
    auto res = mat.inv();
    for(auto [a, b] : v) { cout << YES[1 - res.v[b][a]] << endl; }
}