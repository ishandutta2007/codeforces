...
public:
    ...
    segment_tree_t(int l, int r) : level(0), reverse(0) {
        int m = (l + r) / 2;
        if (l == r) {
            size = 1;
            sum = a[l];
    ...
    void apply_swap(int k) { reverse ^= (1 << (k + 1)); }

    void apply_reverse(int k) { reverse ^= (1 << (k + 1)) - 1; }

    void reverse_node() {
        if (level == 0) return;
        if ((reverse >> level) & 1) swap(left, right);
    ...
    void replace(int x, int k) {
        if (size == 1) {
            ...
            reverse_node();
            if (left->size >= x) left->replace(x, k);
            else right->replace(x - left->size, k);
            sum = left->sum + right->sum;
        }
    }

    int64_t get_sum(int u) {
        if (u <= 0) return 0;
        if (u >= size) return sum;
        else {
            reverse_node();
            return left->get_sum(u) + right->get_sum(u - left->size);
        }
    }
};
void solve() {
    ...
    auto tree = new segment_tree_t(1, (1 << n));
    for (int i = 1, t, x, k, l, r; i <= q; i++) {
        cin >> t >> x;
        if (t == 1) {
            cin >> k;
            tree->replace(x, k);
            ...
            cin >> r;
            l = x;
            cout << tree->get_sum(r) - tree->get_sum(l - 1) << '\n';
        }
    }
}
