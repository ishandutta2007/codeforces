...
int main(){
    ...
    vec(ll,l,n);
    vec(ll,r,n);
    rep(n-1)if(a[i]<a[i+1])l[i+1]=l[i]+1;
    rrep(n-1)if(a[i]>a[i+1])r[i]=r[i+1]+1;
    const ll mx=max(l);
    if(count(all(l),mx)!=1||count(all(r),mx)!=1||max(r)!=mx||mx&1)return out(0);
    rep(i,1,n-1)if(l[i]==r[i]&&l[i]==mx)return out(1);
    out(0);
}