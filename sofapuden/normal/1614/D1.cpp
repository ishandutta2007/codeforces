...
pair<ll,ll> fin(int x){
	if(dp[x] != mp(-1ll,-1ll))return dp[x];
	ll mx = 0; 
	ll div = M[x];
	for(int i = 2; i <= sqrt(x); ++i){
		if(x%i == 0){
			pair<ll,ll> nxt = fin(i);
			pair<ll,ll> nxt2 = fin(x/i);
			if(mx < nxt.first-div*i+div*x){
				mx = nxt.first-div*i+div*x;
			}
			if(mx < nxt2.first-div*x/i+div*x){
				mx = nxt2.first-div*x/i+div*x;
			}
		}
	}
	if(mx == 0){
...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		for(int j = 1; j < sqrt(v[i]); ++j){
			...
			if((int)sqrt(v[i])*(int)sqrt(v[i]) == v[i])M[(int)sqrt(v[i])]++;
	}
	ll ans = 0;
	for(int i = 0; i < n; ++i){
		ans = max(ans,fin(v[i]).first);
	}
	cout << ans << '\n';
}
