...
int main() {
    ...
    vi ma(N), cnt(N);
    ll res = SUM(a);
    rep(i, n) cnt[lb(cc, pll(a[i], i))]++;
    vpl v;
    rep(i, N - 1) v.eb(c[i + 1] - c[i], i);
    sort(rall(v));

    VEC(ll, que, q);
    vpl Q;
    rep(i, q) Q.eb(que[i], i);
    sort(all(Q));
    vl ans(q);
    ...
    dump(v);
    fore(k, idx, Q) {
        while(!empty(v) and v.back().fi <= k) {
            int i = v.back().se;
            v.ppb();
            int j = i + 1;
            i = uf[i], j = uf[j];
            res -= rui[ma[i] + 1] - rui[ma[i] + 1 - cnt[i]];
            res -= rui[ma[j] + 1] - rui[ma[j] + 1 - cnt[j]];
            uf.unite(i, j);
            int to = uf[i];
            ma[to] = max(ma[i], ma[j]);
            cnt[to] = cnt[i] + cnt[j];
            res += rui[ma[to] + 1] - rui[ma[to] + 1 - cnt[to]];
        }
        ans[idx] = res;
    }
    fore(e, ans) OUT(e);
