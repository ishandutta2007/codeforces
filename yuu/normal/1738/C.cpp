...
int f[101][101][2];
bool done[101][101][2];
// 2 players, first play win if their sum mod 2 is b
int F(int n, int o, int b) {
    assert(n >= o);
    if (done[n][o][b]) return f[n][o][b];
    done[n][o][b] = 1;
    if (n == 0) return f[n][o][b] = (b == 0);
    if (o > 0) {
        if (!F(n - 1, o - 1, (o + 1 - b) % 2)) return f[n][o][b] = 1;
    }
    if (o < n) {
        if (!F(n - 1, o, (o + 1 - b) % 2)) return f[n][o][b] = 1;
    }
    return f[n][o][b] = 0;
}
int n;
