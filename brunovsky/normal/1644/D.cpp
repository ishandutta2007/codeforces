...
int main() {
    ...
    vector<int8_t> row_used(MAX), col_used(MAX);
    ...
    while (T--) {
        ...
        num ans = 1;
        for (auto [x, y] : moves) {
            bool row_colored = R > 0 && C > 0 && !row_used[x];
            bool col_colored = R > 0 && C > 0 && !col_used[y];
            ans *= row_colored || col_colored ? K : 1;
            R -= !row_used[x];
            C -= !col_used[y];
            row_used[x] = col_used[y] = true;
        }

        cout << ans << '\n';
        for (auto [x, y] : moves) {
            row_used[x] = col_used[y] = false;
        }
    }
    return 0;
}