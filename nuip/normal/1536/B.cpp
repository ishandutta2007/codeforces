...
int main(){
	...
	reps(c,'a','z'+1) S1.emplace(string(1,c));
	reps(c,'a','z'+1)reps(d,'a','z'+1) S2.emplace(string(1,c)+string(1,d));
	reps(c,'a','c'+1)reps(d,'a','z'+1)reps(e,'a','z'+1) S3.emplace(string(1,c)+
																																 string(1,d)+
																																 string(1,e));
	TT{
		cauto n=read();
		...
		// cauto n=1000;
		// cauto s=string(n,'a');
		auto s1=S1,s2=S2,s3=S3;
		rep(i,n){
			s1.erase(s.substr(i,1));
			s2.erase(s.substr(i,2));
			s3.erase(s.substr(i,3));
		}
		cauto &st=s1.size()?s1:s2.size()?s2:s3;
		string re=*st.begin();
		for(cauto &s:st) MN(re,s);
		cout<<re NL;
	}
	return 0;
}