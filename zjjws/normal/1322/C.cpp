...
int sl[N];
inline void init(){sl[0]=1;for(int i=1;i<N;i++)sl[i]=prpr(sl[i-1],K);return;}
...
inline void work()
{
    LL ans=0;qy.clear();qy[0]=0;
    rin>>n>>m;for(int i=1;i<=n;i++)rin>>v[i],s[i]=0;
    for(int i=1,x,y;i<=m;i++){rin>>x>>y;s[y]+=sl[x];if(s[y]>=M)s[y]-=M;}
    for(int i=1;i<=n;i++){if(qy.find(s[i])==qy.end())qy[s[i]]=i;else v[qy[s[i]]]+=v[i],v[i]=0;}
    for(int i=1;i<=n;i++)if(qy[s[i]]==i)ans=Gcd(ans,v[i]);
    printf("%lld\n",ans);
    return;
}
int main()
{
    init();int T;for(rin>>T;T;T--)work();
    return 0;
}