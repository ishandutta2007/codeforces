...
struct SegmentTree {
  ...
  int find_first(int a, const C &check) {
    ...
    if(check(f(L,seg[a+sz]))) return a+1;
    int b = sz;
    for(a += sz, b += sz; a < b; a >>= 1, b >>= 1) {
      if(a & 1) {
...
signed main(){
    ...
    int n=in(),q=in();
    // RSQ
    SegmentTree<ll> su(N,[](ll x,ll y){return x+y;},0LL);

    // RMQ
    SegmentTree<ll> mi(N,[](ll x,ll y){return max(x,y);},0);
    rep(i,n){
        int x=in();
        su.set(i,x);
        mi.set(i,x);
    ...
    rep(_i,q){
        ...
        bool flag=true;
        while(1){
            int next = mi.find_first(pos+1,[&](ll x){return x>=now;});
            if(next==-1){
                flag=false;
                break;
            }
            next--;
            if(su.query(0,next)==mi[next]){
                cout<<next+1<<endl;
                break;
            }
            ll pre=now;
            now=su.query(0,next+1);
            pos=next;
            if(pos==n-1){
                flag=false;
                break;
