...
fun main(){
    solve.cases{
        ...
        val c12 = Triple(x,y-x,0)
        val c13 = Triple(y,0,y-x)
        val c23 = Triple(0,y,-x)

        fun Triple<Int,Int,Int>.cost():Long{
            val a = 1L * first.absoluteValue * if(first >= 0) c[0] else c[3]
            val b = 1L * second.absoluteValue * if(second >= 0) c[1] else c[4]
            val c = 1L * third.absoluteValue * if(third >= 0) c[2] else c[5]
            return a+ b + c
        }
        put(minOf(c12.cost(), c13.cost(), c23.cost()))
    }
    done()
}