...
template <class F1, class F2>
distF<F1, F2> intersectLinePolygon(const Line<F1>& line, const Polygon<F2>& poly) {
  using F = distF<F1, F2>;
  F res = F();
  int n = static_cast<int>(poly.size());
  auto vertexSgn = [&line, &poly](int i) { return sgn(line.ab ^ (poly[i] - line.a)); };
  int sprev = vertexSgn(n - 1), si = vertexSgn(0), snext;
  for (int i = 0; i < n; ++i, sprev = si, si = snext) {
    snext = vertexSgn(next(i, n));
    if (si == 0) {
...
int main() {
  ...
  REP(i, m) {
    Line<ll> line;
    read(line);
    writeln(intersectLinePolygon(line, poly) / 100);
  }
