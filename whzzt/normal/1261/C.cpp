...
io::StringReader sc([&](char c){ return c=='X'||c=='.'; });
int main(){
	scan(n,m);
	G=vector<vector<char>>(n+1,vector<char>(m+1));
	S=L=vector<vector<int>>(n+1,vector<int>(m+1));
	for(int i=1;i<=n;++i) for(int j=1;j<=m;++j) {
		G[i][j]=sc.read();
		S[i][j]=S[i-1][j]+S[i][j-1]-S[i-1][j-1]+(G[i][j]=='X');
	}
	auto burn=[&](int x,int y,int T){
		int lx=x-T,rx=x+T;
		int ly=y-T,ry=y+T;
		if(lx<1||rx>n||ly<1||ry>m){
			return false;
		}
		if(S[rx][ry]-S[lx-1][ry]-S[rx][ly-1]+S[lx-1][ly-1]==(ry-ly+1)*(rx-lx+1)){
			return true;
		}
		return false;
	} ;
	auto mark=[&](int x,int y,int T){
		...
		if(L[rx][ry]-L[lx-1][ry]-L[rx][ly-1]+L[lx-1][ly-1]>0){
			return true;
		}
		return false;
	} ;
	auto check=[&](int T){
		for(int i=1;i<=n;++i) for(int j=1;j<=m;++j) {
			L[i][j]=L[i-1][j]+L[i][j-1]-L[i-1][j-1]+burn(i,j,T);
		}
		for(int i=1;i<=n;++i) for(int j=1;j<=m;++j) {
			if(mark(i,j,T)!=(G[i][j]=='X')){
				return false;
			}
		}
		return true;
	} ;
	int l=0,r=min(n,m),mid=(l+r+1)>>1;
	while(l<r){
