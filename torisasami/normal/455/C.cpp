...
struct UnionFind2 {
    ...
    bool unite2(int x, int y) {
        ...
        data[x] += data[y];
        data[y] = x;
        chmax(val[x], max(val[y], (val[x] + 1) / 2 + (val[y] + 1) / 2 + 1));
        num--;
        return (true);
    }
...
struct Graph {
    ...
    pair<int, int> farthest_point(int now, pair<int, int> ret, int pre = -1, int nd = 0) {
        chmax(ret, {nd, now});
        for (auto &e: es[now]) {
            if (e.to != pre)
                ret = farthest_point(e.to, ret, now, nd + 1);
        }
        return ret;
    }

    int diameter(int s) {
        int a = farthest_point(s, {-1, -1}).second;
        return farthest_point(a, {-1, -1}).first;
    }
};
// ----- library -------
...
int main() {
    ...
    Graph g(n);
    int a, b;
    rep(i, m) cin >> a >> b, a--, b--, uf.unite(a, b), g.add_edge(a, b);
    rep(i, n) if (uf.find(i) == i) uf.val[i] = g.diameter(i);
    while (q--) {
        int t;
        cin >> t;
        if (t == 1) {
