...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            v.pb({max(0ll,s[j].F-r[i].F+1),max(0ll,s[j].S-r[i].S+1)});
        }
    }
    int sz=v.size();
    suffix_mx.resize(sz+1);
    sort(all(v));
    for(int i=sz-1;i>=0;i--)
    {
        suffix_mx[i]=max(suffix_mx[i+1],v[i].S);
    }
    ll ans=1e18;
    for(int i=0;i<sz;i++)
    {
        ans= min(ans,v[i].F+suffix_mx[i+1]);
    }
    cout<<min(ans,suffix_mx[0])<<'\n';
}