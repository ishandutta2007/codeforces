...
//i_am_noob
//#define wiwihorz
int n,d,b[maxn],dp[maxn];
pii a[maxn];//{neatness,skill}
...
void orzck(){
    ...
    sort(a,a+n,[](pii p1, pii p2){return p1.second>p2.second;});
    while(n){
        if(a[n-1].second<d) n--;
        else break;
    }
    sort(a,a+n);
    bug(n);
    rep(n) bug(a[i].first,a[i].second);
    rep(n){
        b[i]=lower_bound(a,a+n,pii(a[i].second+1,-1))-a;
        if(b[i]>i) vec[b[i]].pb(i);
        bug(i,b[i]);
    }
    int res=0;
    rep(n){
        for(auto j: vec[i]) tree.modify(1,0,maxn-1,0,j,-1);
        dp[i+1]=tree.query(1,0,maxn-1,0,min(i,b[i]))+1;
        chkmax(res,dp[i+1]);
        tree.modify(1,0,maxn-1,i+1,i+1,dp[i+1]);
        if(b[i]>i) tree.modify(1,0,maxn-1,0,i,1);
    }
    print(res);
}
