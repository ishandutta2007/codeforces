...
mod i1 {
	...
	impl Val {
		pub fn new(o: bool) -> Self {
			let n0 = if o {0} else {1};
			let n1 = 1-n0;
			l!(up,down = 1);
			Self {n: (n0,n1), up, down}
		}
	}
	pub type V = Val;
	pub type L = bool;
	...
	impl LSTNode for I {
		...
		fn combine_values(a: &Self::V, b: &Self::V) -> Self::V {
			let mut res = Val::new(false);
			res.n.0 = a.n.0 + b.n.0;
			res.n.1 = a.n.1 + b.n.1;
			res.up = (a.n.0 + b.n.1).max(a.up + b.n.1).max(a.n.0 + b.up);
			res.down = (a.n.1 + b.n.0).max(a.down + b.n.0).max(a.n.1 + b.down);
			res
		}
...
fn main() {
	...
	rep!{[m]
		...
		if t == b'c' {
			...
			writeln!(rout, "{}", res - 1).ok();
		} else {
			l!(l,r = rin.u()-1);
			st.update(l, r, &mut true, true);
		}
	}
}