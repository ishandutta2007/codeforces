...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/248912/problem/I */
/*input
3 2
2 0 2
*/
...
int main () {
	...
	for (int i = n - 1; i > 0; i--) {
		while (it >= 0 and a[it] > i) {
			it--;
		}

		debug(i, kie.order_of_key(i), i - (int)kie.order_of_key(i), ((int)a.size() - 1) - it);
		mini(ats, max(i - (int)kie.order_of_key(i), (int)kl2 + ((int)a.size() - 1) - it));
	}
