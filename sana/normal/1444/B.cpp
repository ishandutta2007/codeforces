...
int main() {
    ...
    ifac[2*n] = modInv(fac[2*n], mod);
    for (int i = 2*n-1; i >= 0; i--) {
        ifac[i] = (ifac[i+1] * 1ll * (i+1)) % mod;
    }

    long long parts = (fac[2*n] * 1ll * ifac[n]) % mod;
    parts = (parts * 1ll * ifac[n]) % mod;


    long long ans = 0;
    for (int i = 0; i < n; i++) ans = (((ans - a[i]) % mod) + mod) % mod;
    for (int i = n; i < 2*n; i++) ans = (((ans + a[i]) % mod) + mod) % mod;
