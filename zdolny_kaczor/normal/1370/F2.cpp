...
int q_cou = 0;
pii query(vi c) {
	assert(c.size() == set<int>(ALL(c)).size());
	assert(!c.empty());
	q_cou++;
	for (int x : c) assert(x >= 1 && x <= n);
#ifdef LOCAL
	map <int, vi> m;
	for (int x : c) m[val[x]].push_back(x);
	auto [v, w] = *m.begin();
	pii ans = {w[rand() % w.size()], v};
...
void mark_dist(int x, int y = -1, int d = 0) {
	val[x] += d;
	for (int v : graf[x]) if (v != y) mark_dist(v, x, d + 1);
}
void init() {
	scanf("%d%d", &s, &t);
	M0(val);
	mark_dist(s);
	mark_dist(t);
	debug(range(val, 1, n + 1));
}
int dep[nax];
int deepest[nax];
int subtree[nax];
void dfs(int x, int y = -1, int d = 0, int s = -1) {
	dep[x] = deepest[x] = d;
	subtree[x] = s;
...
void solve() {
	...
	if (graf[v].size() > 1u) {
		pii go = {0, 0};
		for (int y : graf[v]) maxi(go, pii(deepest[y], y));
		skipped = go.second;
	}
	int deepest_c = 0;
	for (int x : graf[v]) if (x != skipped) maxi(deepest_c, deepest[x]);
	int low = 1, high = min(l, deepest_c);
	int dep1 = 0, end1 = v;
	while (low <= high) {
		int med = (low + high) / 2;
		vi at;
		for (int i = 1; i <= n; ++i) {
			if (dep[i] == med && subtree[i] != skipped) at.push_back(i);
		}
		auto [vv, l2] = query(at);
		if (l2 == l) {
			end1 = vv;
	...
	debug("found", imie(end1), imie(dep1));
	loc(assert(end1 == s || end1 == t));
	int dep2 = l - dep1;
	if (dep2 == 0) {
		answer(v, end1);
		return;
		...
		vi at;
		for (int i = 1; i <= n; ++i) if (dep[i] == dep2 && subtree[i] != subtree[end1])
			at.push_back(i);
		auto [end2, _] = query(at);
		answer(end1, end2);
		return;
	}
}
int main() {
	unt seed = TIME;
	debug(imie(seed));srand(seed);
	int t = 1;
	scanf("%d", &t);
	REP(_, t) {
		solve();
		REP(i, nax) graf[i].clear();
	}
	exit(0);
}
//#STAY AT HOME