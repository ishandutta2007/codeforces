...
S op(S l, S r) {
	return S{ min(l.aminv, r.aminv), max(l.amaxv, r.amaxv), min(l.minv, r.minv), max(l.maxv, r.maxv) };
}

S e() {
	return S{ INT_MAX, INT_MIN, INT_MAX, INT_MIN };
}
...
struct dsu {
	...
	pii uni(int u, int v) {
		...
		pii to = { ends[u].second, ends[v].second };
		ends[u].second = ends[v].first;
		return to;
	}
};
...
int main() {
	...
	for (auto& e : edges) {
		pii to = dsu.uni(e.second.first, e.second.second);
		g[to.first].push_back({ to.second, e.first });
		g[to.second].push_back({ to.first, e.first });
	}
	...
	vector<int> d;
	{
		...
		int lst = -1;
		do {
			pii nxt = g[h][0].first == lst ? g[h][1] : g[h][0];
			lst = h;
			h = nxt.first;
			a.emplace_back(h);
			d.emplace_back(nxt.second);
		} while ((int)g[h].size() == 2);
	}

	vector<int> rank(n + 1);
	vector<S> st_base(n + 1, e());
	for (int i = 0; i < n; i++)
		rank[a[i]] = st_base[a[i]].aminv = st_base[a[i]].amaxv = i;
	SegTree<S, op, e, F, mapping, composition, id> segtree = st_base;
	SparseTable<int, f> table = d;

	for (int t = 0; t < q; t++) {
		int cmd; R(cmd);
		if (cmd == 1) {
			int l,r; R(l,r);
			segtree.apply(l, r + 1, { t, false });
		}
		if (cmd == 2) {
			int l,r; R(l,r);
			segtree.apply(l, r + 1, { t, true });
		}
		if (cmd == 3) {
			int x; R(x);
			S res = segtree.all_prod();
			int l = min(res.minv, rank[x]);
			int r = max(res.maxv, rank[x]);
			W(l == r ? -1 : table.query(l, r - 1));
		}
	}	
