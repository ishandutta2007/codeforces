...
int main() {
    ...
    vector<int> D(N);
    FOR(i, 1, N) D[i] = D[i - 1] + B[i - 1];
    const int sumD = accumulate(D.begin(), D.end(), 0);
    ...
    FOR(x, xlowest, 102) {
        int lo = N * x + sumD;
        if (lo < 0 and x > xlowest) continue;
        if (lo > N * 100) continue;
        xs.push_back(x);
        vector<mint> rem(N * 100 + 1);
        rem[0] = 1;

        lo = 0;
        REP(i, N) {
            IREP(n, rem.size()) if (rem[n]) rem[n + C[i] + 1] -= rem[n];
            FOR(n, 1, rem.size()) rem[n] += rem[n - 1];
            lo += x + D[i];
            fill(rem.begin(), rem.begin() + clamp(lo, 0, int(rem.size())), 0);
        }
        anss.push_back(accumulate(rem.begin(), rem.end(), mint(0)));
    }
