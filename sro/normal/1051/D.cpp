...
int main(){
	...
	for(int i=1;i<n;i++){
		for(int j=1;j<=k;j++){
			...
			dp[i+1][j][0]=(dp[i+1][j][0]+dp[i][j][2])%Mod;
			...
			dp[i+1][j][3]=(dp[i+1][j][3]+dp[i][j][3])%Mod;
			dp[i+1][j][3]=(dp[i+1][j][3]+dp[i][j][2])%Mod;
			dp[i+1][j][3]=(dp[i+1][j][3]+dp[i][j][1])%Mod;
	...
	fio<<(dp[n][k][0]+dp[n][k][1]+dp[n][k][2]+dp[n][k][3])%Mod<<endl;
	return 0;
}