...
struct oneDGraph {
    ...
    vector<int> to;
    vector<int> start;
    oneDGraph(int _n) : n(_n), start(_n + 1) {}
    void reserve(int N) { edges.reserve(N); }
    void add(int u, int v) { edges.eb(u, v); }
    void build() {
        for(auto &[u, v] : edges) { start[u + 1]++; }
        for(int i = 0; i < n; i++) start[i + 1] += start[i];
        to.resize(edges.size());
        auto counter = start;
        for(auto &[u, v] : edges) to[counter[u]++] = v;
    }
};
int main() {
    INT(n, Q);
    ...
    rep(i, Q) {
        STR(s);
        LL(l, r);
        if(!i and l == 4294875409) checker = true;
        if(s == "block") {
            query.eb(0, l, r + 1);
            rng.eb(l, r + 1);
        } else {
    ...
    vi used(si(rng));
    {
        int idx = 0;
        for(auto [l, r] : inirng) {
            while(idx < si(rng) and rng[idx].se <= l) idx++;
            while(idx < si(rng) and rng[idx].se <= r) used[idx++] = true;
        }
    }

    auto id = [&](ll i) -> int {
        if(i == (1LL << n)) return si(rng);
        return lb(rng, pll(i, inf<ll>)) - 1;
    };
    vi T(si(rng), inf<int>);
    rep(i, Q) {
        auto &[t, l, r] = query[i];
    ...
    rep(b, n) {
        ...
        while(max(i, j) < si(rng)) {
            if(~rng[i].fi & 1LL << b and rng[i].se - rng[i].fi <= (1LL << b)) {
                if(max(rng[i].fi + (1LL << b), rng[j].fi) < min(rng[i].se + (1LL << b), rng[j].se)) {
    ...
    auto proc = [&](int i) {
        used[i] = true;
        rep2(j, g.start[i], g.start[i + 1] - 1) { uf.unite(i, g.to[j]); }
    };
    auto ins = [&](ll l, ll r) {
        int i = id(l), j = id(r);
        rep2(k, i, j - 1) proc(k);
