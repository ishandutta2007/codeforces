...
int main() {
    TEST {
        ...
        auto nxt = [&](int i) {
            ...
            if(it == end(s)) return *begin(s);
            return *it;
        };
        rep(i, n) s.emplace(i);
        rep(i, n) {
            if(gcd(a[i], a[nxt(i)]) == 1) { t.emplace(i, nxt(i)); }
        }
        int pre = 0;
        vi ans;
        while(!empty(t)) {
            auto it = t.lower_bound(pii(pre, 0));
            if(it == end(t)) it = begin(t);
            auto [x, y] = *it;
            t.erase(it);
            if(s.count(x)) {
                ans.eb(y + 1);
                ...
                int to = nxt(x);
                if(to == -1) break;
                if(gcd(a[x], a[to]) == 1) t.emplace(x, to);
                pre = to;
            }
            // dump(x, y, s, t);
        }
        cout << si(ans) << " " << ans << endl;
    }
}