...
int main() {
    ...
    vpi v(n * m);
    rep(i, n) rep(j, m) a[i][j]--, v[a[i][j]] = pii(i, j);
    ...
    auto f = [&](int i, int j) {
        ...
        int c = 0;
        fore(dx, dx4) {
            auto [ni, nj] = dx + pii(i, j);
            if(inc(ni, 0, n) and inc(nj, 0, m) and a[ni][nj] < a[i][j]) return false;
        }
        return true;
    };

    rep(i, n) rep(j, m) {
        if(f(i, j)) ng++, ngs.eb(a[i][j]);
    }
    ...
    auto check = [&](int x, int y) {
        ...
        auto h = [&](int i, int j) {
            g(i, j);
            fore(dx, dx4) {
                auto [ni, nj] = pii(i, j) + dx;
                if(inc(ni, 0, n) and inc(nj, 0, m)) g(ni, nj);
            }
        };
        w.clear();
        h(i, j), h(i2, j2);
    ...
    fore(dx, dx4) {
        auto [ni, nj] = pii(x, y) + dx;
        if(inc(ni, 0, n) and inc(nj, 0, m)) { rep(i, a[x][y]) check(i, a[ni][nj]); }
    }

    rep(i, a[x][y] + 1, n * m) { check(i, a[x][y]); }
