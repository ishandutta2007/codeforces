...
void dfs(int cur, int prv = -1) {
  ...
  for (int nxt: ed[cur]) if (nxt != prv) {
    a.push_back(dp[nxt][0]);
    b.push_back(dp[nxt][1]);
    c.push_back(dp[nxt][2]);
    d.push_back(dp[nxt][3]);
  }
  sort(a.begin(), a.end());
  ...
  int candA1 = sz(ed[cur]) + b[0] + b[1];
  int candA2 = a[0];
  int candB1 = sz(ed[cur]) - 2 + b[0];
  int candB2 = c[0];
  int candC1 = sz(ed[cur]) - 1 + b[0];
  int candC2 = c[0];
  int candD1 = sz(ed[cur]) + b[0];
  int candD2 = d[0];
  dp[cur][0] = max(candA1, candA2);
  dp[cur][1] = max(candB1, candB2);
  dp[cur][2] = max(candC1, candC2);
  dp[cur][3] = max(candD1, candD2);
}
