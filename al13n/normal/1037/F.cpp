...
int main(int argc, char **argv){
  ...
  const LL mod=INF+7;
  LL ans=0;
  vector<pair<pll,LL>> st;
  forn(qq,K-1){
    int l=(sz(M)-qq+K-2)/(K-1);
    st.clear();
    st.pb(mp(mp(l,LINF),0ll));
    for(int q=l-1;q>=0;--q){
      LL x=M.at(q*(K-1)+qq);
      while(x>st.back().X.Y){
        assert(sz(st)>1);
      ...
      LL t=x*(st.back().X.X-q) + st.back().Y;
      st.pb(mp(mp(q,x),t));
