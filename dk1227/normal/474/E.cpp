...
void u1(int x, int v) {
	while(x <= n) t1[x] = mx(t1[x], v), x += lowbit(x);
}

void u2(int x, int v) {
	while(x <= n) t2[x] = mx(t2[x], v), x += lowbit(x);
} 

int q1(int x) {
	int ret = 0;
	while(x) ret = mx(ret, t1[x]), x -= lowbit(x);
	return ret;	
}

int q2(int x) {
	int ret = 0;
	while(x) ret = mx(ret, t2[x]), x -= lowbit(x);
	return ret;	
}

signed main() {
	qread(n, d);
	rep(i, 1, n) qread(a[i]), t[i] = a[i];
	sort(t + 1, t + n + 1);
	l = unique(t + 1, t + n + 1) - t - 1;
	per(i, n, 1) {
		int p1 = lower_bound(t + 1, t + l + 1, a[i] + d) - t, p2 = upper_bound(t + 1, t + l + 1, a[i] - d) - t - 1;
		int pos = mx(q1(p2), q2(l - p1 + 1));
		dp[i] = dp[pos] + 1;
		nxt[i] = pos;
		pos = lower_bound(t + 1, t + l + 1, a[i]) - t;
		u1(pos, i);
		u2(l - pos + 1, i);
	}
	int ans = 0;
	rep(i, 1, n) ans = mx(ans, i);
	printf("%lld\n", dp[ans]);
	while(ans) {
		printf("%lld ", ans);
		ans = nxt[ans];
