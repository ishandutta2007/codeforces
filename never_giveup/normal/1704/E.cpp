...
const int maxn = 1e3 + 100, maxm = 1e6 + 100, maxw = 1e7 + 10, inf = 1e9 + 100, LG = 18, SQ = 300;
...
void solve() {
    ...
    for (int v : ord) {
        for (int u : e[v]) {
            setmax(top[u], top[v] + 1);
            for (int j = 0; j <= top[v]; j++) {
                q[u][j + 1].fr += q[v][j].fr;
                q[u][j + 1].sc = min(2000, q[u][j + 1].sc + q[v][j].sc);
            }
        }
    }
    int v = ord.back();
    ...
    if (q[v][i].sc != 0) {
        if (w < i) {
            w = min(2000, q[v][i].sc + i);
            ans = q[v][i].fr + i;
        } else {
            w = min(2000, w + q[v][i].sc);
            ans = ans + q[v][i].fr;
        }
    }
    cout << ans << '\n';
