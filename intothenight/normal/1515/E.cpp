...
int main(){
	...
	int n;
	cin >> n >> mod;
	combinatorics<Zp> C(n * 10);
	auto pow = precalc_power(Zp(2), n * 10);
	vector<vector<Zp>> dp(n + 1, vector<Zp>(n + 1));
	for(auto x = 1; x <= n; ++ x){
		dp[x][x] = pow[x - 1];
	}
	for(auto x = 1; x <= n; ++ x){
		for(auto alive = 0; alive <= x; ++ alive){
			for(auto y = 1; x + y + 1 <= n; ++ y){
				dp[x + y + 1][alive + y] += dp[x][alive] * C.C(alive + y, y) * pow[y - 1];
			}
		}
	}
	debug2(dp);
	cout << accumulate(dp[n].begin(), dp[n].end(), Zp(0)) << "\n";
	return 0;
}
