...
vector<ll> rec(vector<int> a) {
    vector<ll> ans;
    if (sz(a) == 1) {
        ans.pb(1 << a[0]);
        return ans;
    }
    for (int m = 1; m < sz(a); ++m) {
        auto l = rec(vector<int>(a.begin(), a.begin() + m));
        auto r = rec(vector<int>(a.begin() + m, a.end()));
        for (auto vl : l)
            for (auto vr : r)
                ans.pb(vr - vl);
    }
...
signed main() {
    ...
    read(s);
    vector<ll> a;
    for (auto i : s) a.pb(1 << int(i - 'a'));
    t -= a[n - 1];
    t += a[n - 2];
    vector<int> ind(n - 2);
    iota(all(ind), 0);
    sort(all(ind), [&](int i, int j) { return abs(a[i]) > abs(a[j]); });
    for (auto i : ind) {
        ll v = a[i];
        if (abs(t - v) < abs(t + v)) t -= v;
        else t += v;
    }
    println(t == 0 ? "YES" : "NO");
    #ifdef DEBUG
