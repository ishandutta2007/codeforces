...
ll solve() {
	...
	rep(i, n) {
		ll val;
		cin >> val;
		damage[a[i]].push_back(val);
	}
	foa(v, damage) { sort(rall(v)); }

	if(damage[0].size() > damage[1].size()) {
		swap(damage[0], damage[1]);
	}

	if(damage[0].size() == damage[1].size()) {
		ll ans = 0;
		foa(v, damage) { ans += accumulate(all(v), 0LL); }
		ans *= 2;
		ans -= min(damage[0].back(), damage[1].back());
		return ans;
	}

	int dbl = damage[0].size();
	ll ans = 0;
	rep(i,dbl){
		ans+=damage[0][i];
		ans+=damage[1][i];
