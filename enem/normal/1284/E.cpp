...
bool oper(pair<pll,ll> &a,pair<pll,ll> &b)
{
    if((a.F).F==0&&(b.F).F==0)
    {
        return (a.F).S>(b.F).S;
    ...
    if((a.F).F==0)
    {
        if((a.F).S>0) return true;
        if((b.F).F<0) return false;
        else return true;
    }
    if((b.F).F==0)
    {
        if((b.F).S>0) return false;
        if((a.F).F<0) return true;
        else return false;
    }
    if((a.F).F<0&&(b.F).F>0)
    {
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            REP(j,0,n)
            {
                ...
                mp.PB(MP(MP(a[j].F-a[i].F,a[j].S-a[i].S),1));
                mp.PB(MP(MP(a[i].F-a[j].F,a[i].S-a[j].S),-1));
            }
            sort(mp.begin(),mp.end(),oper);
            for(auto it:mp)
            {
                k+=it.S;
                if(it.S==1)
