...
public:
    ...
    segment_tree_t(int l, int r) : l(l), r(r), m((l + r) / 2), count(0), capacity(0) {
        if (l != r) {
    ...
    // if we spend k capacity, what is the remaining count
    int get(int k) {
        if (k <= 0) return count;
        if (capacity <= k) return 0;
        if (l == r) {
            return (capacity - k - 1) / l + 1;
        } else {
            return left->get(k) + right->get(k - left->capacity);
        }
    }
};
void solve() {
    ...
    for (auto&& [x, cnt] : a) tree->update(cnt, 1);
    int filled = 0;
    int ans = n;
    for (int mex = 0; mex <= n; mex++) {
        // we spend k move to see how many thing is left
        int greater_or_equal = tree->get(k);
        if (filled + k >= mex) {
            // this mex is possible to create
            ans = min(ans, greater_or_equal);
        }
        if (a.count(mex)) {
            filled++;
            tree->update(a[mex], -1);
