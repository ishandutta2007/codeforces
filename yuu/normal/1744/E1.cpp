...
void factorise(int val, const int delta) {
    ...
    if (val > 1) factors[val] += delta;
}

int64_t find_multiple(int64_t l, int64_t r, int64_t k) {
    int64_t md = l % k;
    if (md == 0) return l;
    l += k - md;
    if (l > r) return 0;
...
bool backtrack(std::map<int, int>::iterator it, int64_t in_x, int val) {
    if (in_x > c) return 0;
    if (it == factors.end()) {
        auto x = find_multiple(a + 1, c, in_x);
        if (!x) return 0;
        int64_t in_y = val_ab / in_x;
        auto y = find_multiple(b + 1, d, in_y);
        if (!y) return 0;
        if (swapped) cout << y << ' ' << x << '\n';
        else cout << x << ' ' << y << '\n';
        return 1;
        ...
        if (val < it->second)
            if (backtrack(it, in_x * it->first, val + 1)) return 1;

        if (backtrack(next(it), in_x, 0)) return 1;
        return 0;
    }
}
...
void solve() {
    ...
    factorise(b, 1);
    val_ab = 1LL * a * b;
    if (backtrack(factors.begin(), 1, 0)) return;
    cout << "-1 -1\n";
}
