...
int main(){
    LL(n);
    using T=pair<char,pll>;
    VEC(T,a,n);
    pll sum;
    each(c,d,a)sum+=d;
    ll red=0;
    rep(n)if(a[i].first=='R')red|=1<<i;
    const ll max_buf=n*(n-1)/2;
    vv(ll,dp,1<<n,max_buf+1,-LINF);
    dp[0][0]=0;
    rep(bit,(1<<n)-1){
        const ll r=popcnt(red&bit),b=popcnt(~red&bit);
        rep(n)if(~bit&1<<i){
            auto[r2,b2]=a[i].second;
            chmin(r2,r);
            chmin(b2,b);
            rep(r,r2,max_buf+1)chmax(dp[bit|1<<i][r],dp[bit][r-r2]+b2);
        }
    }
    ll ans=LINF;
    rep(max_buf+1)chmin(ans,max(sum.first-i,sum.second-dp.back()[i]));
    out(ans+n);
}