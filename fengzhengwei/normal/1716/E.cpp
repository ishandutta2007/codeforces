...
ll a[xx],b[xx];
char s[xx];
struct seg{ll lm,rm,mx,sm;};
seg upd(const seg&A,const seg&B){return {max(A.sm+B.lm,A.lm),max(B.rm,A.rm+B.sm),max({A.mx,B.mx,A.rm+B.lm}),A.sm+B.sm};}
vector<seg> v[xx<<2];
void build(int k,int l,int r)
{
	v[k].resize(r-l+1);
	if(l==r)return v[k][0]={max(0ll,a[l]),max(0ll,a[l]),max(0ll,a[l]),a[l]},void();
	int mid=l+r>>1;
	build(k<<1,l,mid);
	build(k<<1|1,mid+1,r);
	int nw=(__lg(r-l+1)-1);
//	cerr<<r-l+1<<"$$\n";
	...
	{
		if(j>>nw&1)
			v[k][j]=upd(v[k<<1|1][j-(1<<nw)],v[k<<1][j-(1<<nw)]);
		else 
			v[k][j]=upd(v[k<<1][j],v[k<<1|1][j]);
	}
}
int main(){
	n=read();
