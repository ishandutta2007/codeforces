...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		{
			UF uf(n);
			rep(v,n)for(cauto w:g[v]) uf.unite(v,w);
			if(uf.size(0)!=n){
				cout<<"NO" NL;
				continue;
			}
		...
		vector<int> vst(n);
		cauto dfs=
			[&](auto &&dfs,int v,int p,int c)->void{
				vst[v]=1;
				int ok=1;
				for(cauto w:g[v])if(w!=p && vst[w] && re[w]==c) ok=0;
				if(ok){
					re[v]=c;
				}else{
					re[v]=0;
				}
				for(cauto w:g[v])if(w!=p && !vst[w]) dfs(dfs,w,v,1-re[v]);
			};
		dfs(dfs,0,-1,1);
		cout<<"YES" NL;
		cout<<accumulate(all(re),0) NL;
		rep(v,n)if(re[v]) cout<<v+1<<" ";
		cout NL;
	}
  return 0;
}