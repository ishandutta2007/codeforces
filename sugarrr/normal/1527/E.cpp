...
signed main(){fastio
    ...
    ll n,k;cin>>n>>k;
    vl init(n+1);
    vector<lazy_segtree<S,op,e,F,mapping,composition,id>>seg(k+1,lazy_segtree<S,op,e,F,mapping,composition,id>(init));
    seg[0].set(0,0);
    vvl ad(n+1);
    vvl_l su(n+1);
    ll ANS=0;
    ...
    rep(j,1,n){
        ...
        rep(i,1,k){
            seg[i].set(j,seg[i-1].prod(0,j));
        }
        for(auto x:su[j]){
            rep(i,0,k){
                seg[i].apply(0,x.fi+1,-x.se);
            }
        }
    }
    ll sub=seg[k].get(n);
