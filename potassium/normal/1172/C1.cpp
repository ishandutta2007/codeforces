...
int main() {
	...
	vector<vector<Mint>> dp_table(m + 1, vector<Mint>(2 * m + 1, Mint(0, MOD998)));
	auto get = [&](int x, int y) -> Mint& {
		int dx = x - sumX;
		int dy = sumY - y;
		return dp_table[dx + dy][m + dx - dy];
	};
	...
	for (int t = 0; t <= 1; t++) {
		for (int change = m - 1; change >= 0; change--) {
			for (int add = 0; add <= change; add++) {
				...
				Mint suminv = Mint(x + y, MOD998).inv();
				get(x, y) = suminv * (t == 0 ? -1 : +1) +
							(x + (t == 1)) * suminv * get(x + 1, y) +
							(y - (t == 0)) * suminv * get(x, y - 1);
			}
		}

		for (int i = 0; i < n; i++)
			if (a[i] == t)
				ans[i] = w[i] * (1 + get(sumX, sumY));
	}
