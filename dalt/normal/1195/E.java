...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            MinIntQueue queue = new MinIntQueue(n + m, Integer::compare);
            int[][] minMat = new int[n + 1][m + 1];
            for (int i = 1; i <= m; i++) {
                queue.reset();
                for (int j = 1; j <= n; j++) {
                    queue.enqueue(mat[j][i]);
                    minMat[j][i] = queue.min();
                    if (j >= a) {
                        queue.deque();
                    }
                }
            ...
            debug.debug("mat", mat);
            debug.debug("minMat", minMat);

            long ans = (long) 0;
            for (int i = a; i <= n; i++) {
                queue.reset();
                for (int j = 1; j < b; j++) {
                    queue.enqueue(minMat[i][j]);
                }
                for (int j = b; j <= m; j++) {
                    queue.enqueue(minMat[i][j]);
                    ans += queue.min();
