...
struct fft_runner{
    ...
    vector<int> dft(vector<int> a, int fl, int lg)
    {
        ...
        for (int len = 2, io = 1; len <= a.size(); len <<= 1, io++)
        {
...
vector<int> on_depth[N];
struct Query{
    int k, id;
    int l, r;
};
vector<Query> vertex_queries[N];

void dfs(int v, vvi &go, vi &d, int x)
{
    d[v] = x + 1;
    on_depth[x + 1].push_back(v);
    //cout << x + 1 << " " << v << endl;

    for (auto &q : vertex_queries[v])
        q.l = on_depth[x + 1 + q.k].size();
    for (auto u : go[v])
        dfs(u, go, d, x + 1);
    for (auto &q : vertex_queries[v])
        q.r = (int)on_depth[x + 1 + q.k].size() - 1;
}
...
signed main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        cin >> v >> k;
        v--;
        vertex_queries[v].push_back({k, i});
    }
    vector<int> d(n);
    for (auto e : roots)
    {
    ...
    vector<vector<string> > el(N);
    vector<vector<int> > ls(N), rs(N), ids(N);
    for (int i = 0; i < N; i++)
    {
        for (auto e : on_depth[i])
            el[i].push_back(names[e]);
    }
    for (int i = 0; i < n; i++)
    {
        for (auto e : vertex_queries[i])
        {
            if (e.l > e.r) continue;
            ls[d[i] + e.k].push_back(e.l),
            rs[d[i] + e.k].push_back(e.r),
            ids[d[i] + e.k].push_back(e.id);
        }
    }
    for (int i = 0; i < N; i++)
    {
        if (el[i].size() && ls[i].size() && rs[i].size())
        {
            //for (auto e : el[i]) cout << e << " "; cout << "\n";
            //for (int j = 0; j < ls[i].size(); j++) cout << ls[i][j] << " " << rs[i][j] << "\n";
            vector<int> res = different_on_segment_offline(el[i], ls[i], rs[i]);
