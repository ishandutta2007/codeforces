...
int lim;
vector<int> gen(vector<int> ps) {
    vector<vector<int>> can_gen(ps.size() + 1, {1});
    IREP(i, ps.size()) {
        int m = ps[i];
        can_gen[i] = can_gen[i + 1];
        while (m <= lim) {
            for (auto x : can_gen[i + 1]) {
                if (m * x <= lim) can_gen[i].push_back(m * x);
                else break;
            }
            m *= ps[i];
        }
        sort(can_gen[i].begin(), can_gen[i].end());
    }
    return can_gen[0];
}
