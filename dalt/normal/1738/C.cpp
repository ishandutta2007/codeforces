...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  Vec<Vec<Vec<Vec<int>>>> dp(cnt[0] + 1, Vec<Vec<Vec<int>>>(cnt[1] + 1, Vec<Vec<int>>(2, Vec<int>(2, -1))));
  var get_dp = [&](var &dfs, int a, int b, int now, int who) {
    if(a < 0 || b < 0) {
      return who;
    }
    var& ans = dp[a][b][now][who];
    if(ans == -1) {
      if(a == 0 && b == 0) {
        ans = now ^ 1;
      } else {
        int x = dfs(dfs, a - 1, b, now ^ (who * 0), who ^ 1);
        int y = dfs(dfs, a, b - 1, now ^ (who * 1), who ^ 1);
        if(x == who || y == who) {
          ans = who;
        } else {
          ans = who ^ 1;
      ...
      DebugFmtln("dp[%d][%d][%d][%d] = %d", a, b, now, who, ans);
    }
    return ans;
  };
  var ans = get_dp(get_dp, cnt[0], cnt[1], 0, 1);
  if(ans == 1) {
    out << "Alice";
  } else {
    out << "Bob";
