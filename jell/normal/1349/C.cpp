...
struct solver
{
    ...
    {
        int n,m,t; cin>>n>>m>>t;
        vector<string> grd(n); cin>>grd;
        vector dist(n,vector<i64>(m,2e18));
        queue<p32> qu;
        for(int i=0; i<n; i++)
        {
            for(int j=0; j<m; j++)
            {
                char here=grd[i][j];
                bool ok=false;
                if(i and here==grd[i-1][j]) ok=1;
                if(j and here==grd[i][j-1]) ok=1;
                if(i+1<n and here==grd[i+1][j]) ok=1;
                if(j+1<m and here==grd[i][j+1]) ok=1;
                if(ok)
                {
                    dist[i][j]=0;
                    qu.emplace(i,j);
        ...
        while(!qu.empty())
        {
            auto [i,j]=qu.front();
            qu.pop();
            if(i and chmin(dist[i-1][j],dist[i][j]+1)) qu.emplace(i-1,j);
            if(j and chmin(dist[i][j-1],dist[i][j]+1)) qu.emplace(i,j-1);
            if(i+1<n and chmin(dist[i+1][j],dist[i][j]+1)) qu.emplace(i+1,j);
            if(j+1<m and chmin(dist[i][j+1],dist[i][j]+1)) qu.emplace(i,j+1);
        }
        ...
        for(i64 x,y,p; t--;)
        {
            ...
            if(dist[x][y]<p)
            {
                if((p-dist[x][y])&1) cout << char('0'^'1'^grd[x][y]) << "\n";
                else cout << grd[x][y] << "\n";
            }
            else
            {
