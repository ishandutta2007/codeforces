...
void solve() {
    ...
    REP(i, n) {
        int a, b;
        cin >> a >> b;
        inputs[max(a,b)].push_back(mp(a, -b));
    }
    ...
    int ans_b = 0;
    for (auto i : inputs) {
        vector<pair<int,int>> now = i.second;
        sort(ALL(now));
        REP(q, now.size()) {
            now[q].second *= -1;
        }
        ...
        for (int q = 1; q < now.size(); ++q) {
            dist += abs(now[q].first - now[q - 1].first) + abs(now[q].second - now[q-1].second);
        }
        auto calc = [](pair<int, int> a, pair<int, int >b) {
            return abs(a.first - b.first) + abs(a.second - b.second);
        };
        int next_a = min(ans_a + calc(a, now.back()), ans_b + calc(b, now.back()));
        int next_b = min(ans_a + calc(a, now[0]), ans_b + calc(b, now[0]));
        a = now[0];
        b = now.back();
        ans_a = next_a + dist;
        ans_b = next_b + dist;
