...
mn p; // probability of a

vector<modnum<int(1e9 + 7)>> pows{1};
modnum<int(1e9 + 7)> pow(modnum<int(1e9 + 7)> a, int e) {
    while (sz(pows) <= e) {
        pows.push_back(pows.back() * a);
    }
    return pows[e];
}
...
mn f(int count_a, int target) {
    if (!done[count_a][target]) {
        mn ev = 0;
        mn tot = 0;
        for (int i = 0;; i++) {
            if (target - (count_a + i) <= 0) {
                mn prob = pow(p, i);        // probability we get at least i
                ev += prob * (count_a + i); // we definitely get this much
                ev += prob * (p / (1 - p)); // possible extra
                tot += prob;
                break;
            }
            mn prob = pow(p, i) * (1 - p);                       // probability we get exactly i
            ev += prob * (count_a + i);                          // we get this much immediately
            ev += prob * f(count_a + i, target - (count_a + i)); // possible extra
            tot += prob;
        }
        cache[count_a][target] = ev;
        done[count_a][target] = true;
    }

    return cache[count_a][target];
}
...
int main() {
    ...
    cache = vvmn(k + 1, vmn(k + 1));
    done = vvb(k + 1, vb(k + 1, false));
    ...
    for (int i = 1; i <= k; i++) {
        ...
        mn prob = pow(p, i - 1) * (1 - p); // probability we get exactly i
        ans += prob * i;                   // we get this immediately
        ans += prob * f(i, k - i);         // possible extra
    }
