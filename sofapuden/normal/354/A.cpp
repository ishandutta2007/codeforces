...
void solve(){
	...
	ll best;
	for(ll i = 0; i <= n; ++i){
		if(i == 0)best = (n-1)*qr+r*total;
		else if(i < n-i) best = min(best, (max(0LL,n-2*i-1))*qr+summ[i-1]*l+(total-summ[i-1])*r);
		else best = min(best, (max(0LL,2*i-n-1))*ql+summ[i-1]*l+(total-summ[i-1])*r);
	}
	cout << best;
