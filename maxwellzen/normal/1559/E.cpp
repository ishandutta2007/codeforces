...
int calc(int d) {
    int cm = m/d;
    vi dp(cm + 1, 0), pre(cm+2, 0);
    FOR(i, (l[0]+d-1)/d, r[0]/d + 1) dp[i]=1;
    pre[0]=0;
    FOR(i, 1, cm+2) pre[i]=(pre[i-1]+dp[i-1])%MOD;
    FOR(i, 1, n) {
        int cl=(l[i]+d-1)/d, cr=r[i]/d;
        fill(all(dp), 0);
        R0F(i, cm+1) {
            dp[i] = (pre[max(0, i-cl+1)] - pre[max(0, i-cr)])%MOD;
        }
        FOR(i, 1, cm+2) pre[i]=(pre[i-1]+dp[i-1])%MOD;
    }
    return (pre[cm+1])%MOD;
...
void solve() {
    ...
    mob[1]=1;
    FOR(i, 2, m+1) {
        if (i % (factor[i]*factor[i])==0) mob[i]=0;
        else mob[i]=-1*mob[i/factor[i]];
    }
    debug(mob);
    l.rsz(n); r.rsz(n);
    F0R(i, n) cin >> l[i] >> r[i];
