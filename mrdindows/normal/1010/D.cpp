...
int go(int cur) {
    if (t[cur] == "IN") {
        ...
        val[cur] = !go(sons[cur][0]);
    } else if (t[cur] == "AND") {
        val[cur] = go(sons[cur][0]) & go(sons[cur][1]);
    } else if (t[cur] == "XOR") {
        val[cur] = go(sons[cur][0]) ^ go(sons[cur][1]);
    } else {
        assert(t[cur] == "OR");
...
int ans[1010101];
void dfs(int cur, bool e0 = true, bool e1 = true) {
    if (t[cur] == "IN") {
        if (val[cur] == 0) {
            ans[cur] = e0;
        } else {
        ...
        dfs(sons[cur][0], e1, e0);
    } else if (t[cur] == "AND") {
        if (val[cur] == 0) {
            if (!e0) return;
            if (!val[sons[cur][0]] && !val[sons[cur][1]]) {
                return;
            }
            for (int i = 0; i < 2; ++i) {
                if (!val[sons[cur][i]]) {
                    dfs(sons[cur][i], true, false);
                }
            }
        } else {
            if (!e1) return;
            dfs(sons[cur][0], false, true);
            dfs(sons[cur][1], false, true);
        }
    } else if (t[cur] == "XOR") {
        if ((val[cur] == 0 && e0) || (val[cur] == 1 && e1)) {
            dfs(sons[cur][0], true, true);
            dfs(sons[cur][1], true, true);
        }
    } else {
        if (val[cur] == 1) {
            if (!e1) return;
            ...
            for (int i = 0; i < 2; ++i) {
                if (val[sons[cur][i]]) {
                    dfs(sons[cur][i], false, true);
                }
            }
        } else {
            if (!e0) return;
            dfs(sons[cur][0], true, false);
            dfs(sons[cur][1], true, false);
        }
    }
}
int solve() {
