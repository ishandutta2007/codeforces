...
void main_() {
	...
	auto H = G.root_to_leaf();

	V<ll> sub_sz(n);
	auto dfs = [&](auto dfs, int now) -> void {
		sub_sz[now] = SZ(H[now]) == 0;
		for(auto [to, cost] : H[now]) {
			dfs(dfs, to);
			sub_sz[now] += sub_sz[to];
		}
	};
	...
	REP(i, n - 1) {
		auto &[u, v, w, c] = es[i];
		sz[i] = min(sub_sz[u], sub_sz[v]);
		s += sz[i] * w;
		(c == 1 ? pq1 : pq2).push({sz[i] * (w - w / 2), i});
		w = w / 2;
	}

	while(s > 0) {
		if(pq1.size() and s <= pq1.top().first) {
			res += 1;
			break;
		}
		if(pq2.size() and s <= pq2.top().first) {
			res += 2;
			break;
		}
		if(pq1.size() and pq2.size()) {
			if(pq1.top().first >= pq2.top().first) {
				...
				auto &[u, v, w, c] = es[id];
				if(w == 0) continue;
				pq1.push({sz[id] * (w - w / 2), id});
				w = w / 2;
				continue;
			}

			auto [val1, id1] = pq1.top();
			pq1.pop();
			ll tmp = 0;
			if(SZ(pq1))
				tmp = pq1.top().first;
			{
				int w = get<2>(es[id1]);
				chmax(tmp, sz[id1] * (w - w / 2));
			}

			auto [val2, id2] = pq2.top();
			debug(val1, tmp, val2);
			if(val1 + tmp >= val2) {
				res += 1;
				s -= val1;
