...
bool cannot[MAXN];
int Solve(){
	if( sum[n][n] == n*n || !sum[n][n] ) return n;
	int ans = 1;
	Forx(i,2,n){
		if(n%i) continue;
		...
		Forstep(x,1,n,i)
			Forstep(y,1,n,i)
				if( Sum(x,y,x+i-1,y+i-1) != i*i && Sum(x,y,x+i-1,y+i-1) != 0 ){
					fail = 1;
					break;
				}
		if(fail){
...
int main(){
	...
	// For(i,n) Print(a[i],1,n);
	For(i,n) For(j,n) sum[i][j] = sum[i-1][j] + sum[i][j-1] - sum[i-1][j-1] + a[i][j];
