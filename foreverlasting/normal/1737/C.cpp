...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		if(M.count({1,1})&&M.count({1,2})&&M.count({2,1})){
			int x=read(),y=read();
			if(x==1||y==1)puts("YES");
			else puts("NO");
			return;
		}
		if(M.count({1,n})&&M.count({1,n-1})&&M.count({2,n})){
			int x=read(),y=read();
			if(x==1||y==n)puts("YES");
			else puts("NO");
			return;
		}
		if(M.count({n,1})&&M.count({n,2})&&M.count({n-1,1})){
			int x=read(),y=read();
			if(x==n||y==1)puts("YES");
			else puts("NO");
			return;
		}
		if(M.count({n,n})&&M.count({n,n-1})&&M.count({n-1,n})){
			int x=read(),y=read();
			if(x==n||y==n)puts("YES");
			else puts("NO");
			return;
		...
		int p,q;
		for(auto [x,y]:Mx)if(y==1)p=x;
		for(auto [x,y]:My)if(y==1)q=x;
		int x=read(),y=read();
		x=abs(x-p),y=abs(y-q);
		if(x&1||y&1)puts("YES");
		else puts("NO");
