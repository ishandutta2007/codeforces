...
const int N=100005;
int n,u[N];
struct op{int x,y,z;op(int x,int y,int z):x(x),y(y),z(z){u[x]^=1,u[y]^=1,u[z]^=1;}};
vector<op>no;
int main(){
	int x,a,b,c;
	scan(n);
	for(x=1;x<=n;x++)scan(u[x]);
	for(a=1;a+2<=n;a++)if(u[a]){
		if(a+12<=n){
			if(u[a+1]){
				if(u[a+2])no.emplace_back(a,a+1,a+2);
				else if(u[a+3]&&u[a+4]&&!u[a+5])no.emplace_back(a+1,a+4,a+7);
				else{
					for(b=a+3;b<a+6;b++)if(u[b])break;
					no.emplace_back(a+1,b,b-a-1+b);
				}
			}
			if(u[a]){
				for(b=a+1;b<a+6;b++)if(u[b])break;
				no.emplace_back(a,b,b-a+b);
			}
		}else no.emplace_back(a,a+1,a+2);
	}
	if(u[n]&&n<7||u[n-1]&&n<8)puts("NO"),exit(0);
	if(u[n-1])no.emplace_back(n-7,n-4,n-1),no.emplace_back(n-5,n-4,n-3),no.emplace_back(n-7,n-5,n-3);
	if(u[n])no.emplace_back(n-6,n-3,n),no.emplace_back(n-4,n-3,n-2),no.emplace_back(n-6,n-4,n-2);
	io::ps("YES\n"),print(no.size()),putc('\n');
	for(auto op:no)show(op.x,op.y,op.z);
}