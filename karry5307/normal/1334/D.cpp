...
#define ll long long
const ll MAXN=8e5+51,MOD=998244353,inf=0x3f3f3f3f;
// Structures and variables
ll test,n,l,r,x,lx,rx,len,s,p,flg;
// Fast IO
inline ll read()
{
...
inline void solve()
{
	n=read(),l=read(),r=read(),lx=1,len=2*(n-1),flg=x=0;
	if(l==n*(n-1)+1)
	{
		return (void)puts("1");
	}
	...
	pii lx=calc(l),rx=calc(r);
	for(register int k=lx.first;k<=rx.first;k++)
	{
		ll lp=(k==lx.first)?lx.second:1;
		ll rp=(k==rx.first)?rx.second:2*(n-k);
		for(register int i=lp;i<=rp;i++)
		{
			printf("%d ",k+((i&1)?0:i/2));
		}
	}
	if(flg)
	{
