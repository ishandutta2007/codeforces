...
void dfs(int u) {
    ...
    if (f[u] != n) edge_weight.emplace_back(static_cast<int64_t>(f[u]) * (n - f[u]));
}
...
void solve() {
    ...
    weight | REVERSE;
    while (weight.size() < n - 1) weight.emplace_back(1);
    edge_weight.clear();
    for (int i = 1; i <= n; i++) done[i] = 0;
    dfs(1);
    edge_weight | SORT | REVERSE;
    int64_t ans = 0;
    for (int i = 0; i < weight.size(); i++) (ans += weight[i] * edge_weight[i]) %= base;
    cout << ans << '\n';
}
