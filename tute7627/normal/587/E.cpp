...
int main(){
  ...
  ll n,q;cin>>n>>q;
  SegmentTree<ll,decltype(fx)>seg1(n,fx,0);
  SegmentTree<B,decltype(fy)>seg2(n-1,fy,B());
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  rep(i,0,n){
    if(i==0)seg1.set(i,a[i]);
    else{
      seg1.set(i,a[i]^a[i-1]);
      B tmp;
  ...
  while(q--){
    ...
    if(t==1){
      ...
      auto b=seg2.query(l,r-1);
      b.insert(seg1.query(0,l+1));
      cout<<(1<<(ll)b.basis.size())<<endl;
    }
  }
  return 0;
}