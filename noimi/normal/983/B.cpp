...
#define clz(x) __builtin_clz(x)
    ...
    SegmentTree(int n, const F f, const T &unit)
        : f(f), unit(unit), sz(n - 1 ? 1 << (32 - clz(n - 1)) : 1) {
        seg.assign(2 * sz, unit);
    }

    SegmentTree(vector<T> &a, const F f, const T &unit)
        : f(f),
          sz((int)a.size() > 1 ? 1 << (32 - clz(a.size() - 1)) : 1),
          unit(unit) {
        int n0 = a.size();
        seg.assign(2 * sz, unit);
...
main() {
    ...
    auto v = vv(n, n);
    rep(i, n) {
        rep(j, n - i) v[j][i + j] = a[j];
        if(i == n - 1) break;
        rep(j, n - i - 1) a[j] = a[j] ^ a[j + 1];
        a.pop_back();
    }
    rep2(i, 1, n - 1) {
        rep(j, n - i) {
            chmax(v[j][i + j], max(v[j][i + j - 1], v[j + 1][i + j]));
        }
    }
    INT(Q);
    while(Q--) {
