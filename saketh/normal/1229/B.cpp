...
void go(int u, int p, int d, const vector<pair<int, ll>>& hist) {
    vector<pair<int, ll>> mine{{d, x[u]}};
    ll cv = x[u]; int pd = d;
    F0Rd (i, sz(hist)) {
        ll nv = __gcd(cv, hist[i].s);
        if (nv != cv) {
            ans += mn(cv) * (pd - hist[i].f);
            cv = nv;
            pd = hist[i].f;
            mine.pb(hist[i]);
        }
    ...
    reverse(all(mine));
    assert(sz(mine) < 42);
    trav (v, adj[u]) if (v != p) go(v, u, d+1, mine);
}

int main() {
    setIO();

    int N; re(N); x.resz(N); re(x); adj.resz(N);
