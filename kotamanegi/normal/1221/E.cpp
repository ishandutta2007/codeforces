...
void solve(){
    ...
    sort(ALL(inputs));
    reverse(ALL(inputs));
    while (!inputs.empty() and inputs.back() < b) {
        inputs.pop_back();
    }
    if (!inputs.empty() and inputs.back() < a) {
        cout << "NO" << endl;
        return;
    }
    if (!inputs.empty() and inputs[0] >= 2 * b) {
        if (inputs.size() >= 2 and inputs[1] >= 2 * b) {
            cout << "NO" << endl;
            return;
        }
        inputs[0] -= a;
        REP(q, inputs[0] + 1) {
            int rights = inputs[0] - q;
            if (max(rights, q) >= 2 * b) {
                //fail
                continue;
            }
            if (b <= q and q < a) continue;
            if (b <= rights and rights < a) continue;
            int cnt = inputs.size() - 1;
            if (b <= q) cnt++;
            if (b <= rights) cnt++;
            if (cnt % 2 == 0) {
                cout << "YES" << endl;
