...
int main() {
	C.init();
	fac[0] = 1; FOR(i,1,MX) fac[i] = i*fac[i-1];
	F0R(i,MX) ifac[i] = 1/fac[i];
    setIO(); re(n); t.rsz(n); re(t);
    F0R(i,n) oc[t[i]] ++;
    FOR(i,2,MX) if (!comp[i]) {
    	for (int j = 2*i; j < MX; j += i) comp[j] = 1;
		vi v;
    	for (int j = i; j < MX; j += i) {
    		int co = 0;
    		int J = j; while (J%i == 0) J /= i, co ++;
    		F0R(k,oc[j]) v.pb(co);
    	}
    	sort(all(v)); reverse(all(v));
    	F0R(i,sz(v)) tot[n-1-i] += v[i];
    }
    mi sum = 0;
    F0R(i,n) {
    	ans += tot[i]*(sum-(pow(mi(2),n-1)-sum-C.comb(n-1,i))); // n-1 choose (<i) - n-1 choose (>i)
    	sum += C.comb(n-1,i);
    }
    ps(ans);
}
