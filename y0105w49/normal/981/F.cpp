...
const int N=200<<10;
int n,L,a[N],_b[N+N+N],*b=_b+N;


bool poss(int mx) {
	int l=-n,r=n+n-1;
	for(int i=0;i<n;i++) {
		int li=int(lower_bound(b+l,b+r+1,a[i]-mx)-b);
		int ri=int(upper_bound(b+l,b+r+1,a[i]+mx)-b)-1;
		l=li,r=ri;
		++l,++r;
		assert(r-l+1 <= n);
		if(l>r) return 0;
...
int32_t main() { // CURSOR START
	...
	sort(a,a+n);
	sort(b,b+n);
	for(int i=0;i<n;i++) b[i-n]=b[i]-L, b[i+n]=b[i]+L;
