    ...
    if (count < N+1) {
        ...
        auto ch = [&](ll P) {
            ...
            rep(i, N+1) {
                ll x = 0;
                if (c[N - i][0] != '-') {
                    rep(j, c[N - i].size()) { x = x * 10 + (c[N - i][j] - '0'); }
                }
                else {
                    rep(j, c[N-i].size() - 1) { x = x * 10 - (c[N-i][j + 1] - '0'); }
                }
        ...
        rep(t, 100) {
            ...
            uniform_int_distribution<> rand(1, 100000000);

            if (!ch(rand(engine))) { cout << "No" << endl; return 0; }
