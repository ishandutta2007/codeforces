...
vector<int> ans;
vector<bool> used;
void euler(int u, int p, vector<vector<pii>>& g) {
	while (g[u].size()) {
		pii e = g[u].back();
		g[u].pop_back();
		
		if (used[e.S / 2]) continue;
		used[e.S / 2] = true;
...
int main() {
	int n; R(n);
	vector<pii> edges(n); R(edges);
	used.resize(n, false);
	
	for (int i = 20; i >= 0; i--) {
		const int MASK = (1 << i) - 1;
		
		vector<vector<pii>> g(MASK + 1);
		for (int e = 0; e < n; e++) {
			int u = edges[e].F;
			int v = edges[e].S;
			int umask = u & MASK;
			int vmask = v & MASK;
			g[umask].pb( {vmask,e*2+1} );
			g[vmask].pb( {umask,e*2} );
		}
		
		bool is = 1;
		int cc = 0;
		vis = vector<bool>(MASK + 1, false);
