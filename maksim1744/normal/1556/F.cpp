...
int main() {
    ...
    for (int comp = 0; comp < prob.size(); ++comp) {
        prob[comp] = 1;
        for (int msk = comp; msk != 0; msk = (msk - 1) & comp) {
            if (msk == comp) continue;
            Mint cur = prob[msk];
            for (int i = 0; i < n; ++i) {
                if (((msk >> i) & 1) == 0) continue;
                for (int j = 0; j < n; ++j) {
                    if ((((comp ^ msk) >> j) & 1) == 0) continue;
                    cur *= win[i][j];
                }
            }
            prob[comp] -= cur;
    ...
    Mint ans = 0;
    for (int msk = 0; msk < prob.size(); ++msk) {
        Mint p = prob[msk];
        for (int i = 0; i < n; ++i) {
            if (((msk >> i) & 1) == 0) continue;
            for (int j = 0; j < n; ++j) {
