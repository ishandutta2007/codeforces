...
int main(){
  ...
  while(q--){
    ...
    if(type==1){
      ...
      if(!rev){
        rep(i,0,p)bit.add(pl+2*p-i-1,bit[pl+i]);
        pl+=p;
      }
      else{
        rep(i,0,p)bit.add(pr-2*p+i,bit[pr-i-1]);
        pr-=p;
      }
      //cout<<p spa pl spa pr spa rev<<endl;
      //bit.print();
    }
    else{
      ll l,r;cin>>l>>r;
      if(!rev)cout<<bit.sum(pl+l,pl+r-1)<<endl;
      else cout<<bit.sum(pr-r,pr-l-1)<<endl;;
    }
  }
  return 0;
}