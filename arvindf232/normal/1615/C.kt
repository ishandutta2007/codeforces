...
fun checkdistances(a:BooleanArray,b:BooleanArray):Int?{
    var dis = 0
    var acount =0
    var bcount = 0
...
fun main(){
    solve.cases{
        ...
        val ta = checkdistances(A,B)
        if(ta != null){
            best = minOf(best,ta)
        }
        val except = (0 until n).firstOrNull { A[it] == B[it] && A[it] == true  }
        if(except != null){
            for(i in 0 until n){
                if(i == except){
                    continue
        ...
        val tb = checkdistances(A,B)
        if(tb != null){
            best = minOf(best,tb + 1 )
        }

        put(if(best == Int.MAX_VALUE) -1 else best)
    }
    done()
}
