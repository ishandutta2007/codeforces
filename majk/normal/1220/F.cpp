...
public:
    ...
    bool canBeDone(int l, int r, int k) {
        if (l > r) return true;
        if (k < 20 && (r-l+1) >= (1<<k)) return false;
        if (k == 0) return false;
        int m = MQ.get(l,r);
        int i = I[m];
        return canBeDone(l, i-1, k-1) && canBeDone(i+1, r, k-1);
    }
    ...
    void solve(istream& cin, ostream& cout) {
        ...
        vector<int> Z(N);
        int K = bsl(1, N/2, [&](int k) {
            int w = bsh(1, N-1, [&](int r) { return canBeDone(1, r, k); });
            Z[k] = w;
            return canBeDone(w+1, N-1, k);
        });

        cout << K+1 << ' ' << (Z[K] + one + 1)%N << endl;
    }
};
