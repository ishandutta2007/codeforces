...
int main() {
    ...
    cin >> xs;
    vector<int> xs0 = xs;
    sort(xs0.rbegin(), xs0.rend());
    xs0.erase(unique(xs0.begin(), xs0.end()), xs0.end());
    ...
    auto upd_from = [&](int st, int x) {
        for (int i = st; i <= n; ++i) {
            dp[i].assign(dp[i - 1].size() + r[i - 1] - l[i - 1], 0);
            for (int j = max(0, x * i - s0s[i]); j < dp[i].size(); ++j) {
                int R = min(j, (int)dp[i - 1].size() - 1);
                int L = j - (r[i - 1] - l[i - 1]);
                dp[i][j] = dp[i - 1][R];
    ...
    vector<pair<int, int>> unlocks;
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j < dp[i].size(); ++j) {
            unlocks.eb((j + s0s[i] + i * (int)1e7) / i - (int)1e7, i);
        }
    }
    sort(unlocks.rbegin(), unlocks.rend());
    unlocks.erase(unique(unlocks.begin(), unlocks.end()), unlocks.end());
    show(unlocks);
    map<int, Mint> ans;
    int ind = 0;
    show(xs0);
    for (int x : xs0) {
        int mn = n + 1;
        while (ind < unlocks.size() && unlocks[ind].first >= x) {
            mn = min(mn, unlocks[ind].second);
            ++ind;
        }
        show(mn, x);
        upd_from(mn, x);
