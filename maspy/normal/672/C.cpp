...
void solve() {
  pi S, T, O;
  read(S), read(T), read(O);
  auto dist = [&](pi A, pi B) -> double {
    double dx = A.fi - B.fi;
    double dy = A.se - B.se;
    return sqrt(dx * dx + dy * dy);
  };
  LL(N);
  vc<double> A(N), B(N), C(N);
  FOR(i, N) {
    pi P;
    read(P);
    A[i] = dist(S, P) + dist(O, P);
    B[i] = dist(T, P) + dist(O, P);
    C[i] = dist(O, P) + dist(O, P);
  }
  double base = SUM(C);
  FOR(i, N) A[i] -= C[i];
  FOR(i, N) B[i] -= C[i];
  ...
  double ANS = 1LL << 60;
  chmin(ANS, base + MIN(A));
  chmin(ANS, base + MIN(B));
  auto I = argsort(A);
  auto J = argsort(B);
  I.resize(min(N, 2LL));
  J.resize(min(N, 2LL));
