...
const int N=2e5+200,M=1e6+2,inf=1e9;
char s[N],t[N];
vector<int> bel[26];
int g[N],*f;
...
int main()
{
	...
	while (T--)
	{
		...
		//f=g;
		//for (i=0;i<=n+m+5;i++) f[i]=0;
		//for (i=1;i<=n;i++) s[i]-='a';
		//for (i=1;i<=m;i++) t[i]-='a';
		i=1;if ((n^m)&1) i=2;c=0;
		for (j=1;i<=n;i++)
		{
			if (j<=m&&s[i]==t[j]&&!c) ++j; else if (c) --j,c=0; else ++j,c=1;
			//printf("%d %d\n",i,j);
		}
		if (j==m+1&&!c) puts("YES"); else puts("NO");
	}
}