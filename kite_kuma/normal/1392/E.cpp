...
int main() {
	...
	rep(i, q) {
		...
		rep(i, n + n - 2) {
			...
			// debug(now);
			if(dist & k) {
				if(isin(now.first + 1, 0LL, n) && a[now.first + 1][now.second] == k) {
					now.first++;
				} else {
					now.second++;
				}
			} else {
				if(isin(now.second + 1, 0LL, n) && a[now.first][1 + now.second] == 0LL) {
					now.second++;
				} else {
					now.first++;
				}
			}
			dist -= a[now.first][now.second];
			cout << now.first + 1 << " " << now.second + 1 << endl;
		}
	}
}