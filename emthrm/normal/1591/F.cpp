...
int main() {
  ...
  vector<tuple<int, ModInt, int>> dp{{a[0], 1, 0}};
  vector<ModInt> ev_pos(n, 0), od_pos(n, 0);
  ev_pos[0] = a[0];
  od_pos[0] = -a[0];
  ModInt sum = a[0];
  FOR(i, 1, n) {
    if (a[i - 1] <= a[i]) {
      if (a[i - 1] < a[i]) dp.emplace_back(a[i], sum, i);
      sum = sum * (a[i] - 1);
    } else {
      sum *= a[i - 1] - 1;
      while (get<0>(dp.back()) > a[i]) {
        auto [ub, x, from] = dp.back(); dp.pop_back();
        if ((i - 1) & 1) {
          x = od_pos[i - 1] - (from > 0 ? od_pos[from - 1] : 0) + ((i - from) & 1 ? -x : x);
        } else {
          x = ev_pos[i - 1] - (from > 0 ? ev_pos[from - 1] : 0) + ((i - from) & 1 ? -x : x);
        }
        int prev = (dp.empty() ? 0 : get<0>(dp.back()));
        if (prev < a[i]) {
          sum -= x * (ub - a[i]);
          dp.emplace_back(a[i], x, i);
        } else {
          sum -= x * (ub - prev);
        }
      }
    }
    ev_pos[i] = ev_pos[i - 1] + (sum * (i & 1 ? -1 : 1));
    od_pos[i] = od_pos[i - 1] + (sum * (i & 1 ? 1 : -1));
  }
  cout << sum << '\n';
  return 0;
}