...
int main () {
    ...
    while (t--) {
        ...
        fop (i,0,n) cin >> tmp, a.insert(tmp);
        vector <int> cur;
        int mx = *a.begin();
        int ans = 0;
        if (mx % 30 == 0 && a.count(mx / 2) && a.count(mx / 3) && a.count(mx / 5)) {
            ans = mx / 30 * 31;
        }
        while (a.size() && cur.size() < 3) {
            int x = *a.begin(); a.erase(a.begin());
            bool is = true;
            for (int j : cur) is &= (j % x != 0);
            if (is) cur.pb(x);
        }
        cout << max(accumulate(all(cur), 0), ans) << endl;
    }
}