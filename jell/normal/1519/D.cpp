#line 1 "codeforces-workspace/contest/1519/d/d.cpp"
...
#line 11 "codeforces-workspace/contest/1519/d/d.cpp"
...
void main() {
  ...
  for (auto c : range(n * 2)) {
    ...
    for (auto &&[r, x] : enumerate(v)) {
      if ((c ^ r) & 1) continue;
      auto i = int(c - r) / 2;
      auto j = int(c + r) / 2;
      if (i < 0 || j >= n) break;
      chgr(ans, ip[i] + ip.back() - ip[j + 1] + x);
    }
  }
