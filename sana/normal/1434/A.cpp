...
int main() {
    ...
    multiset< pair<int, pii> > used;

    for (int i = 0; i < n; i++) {
        cin >> b[i];

        used.insert({b[i]-a[5], {i, 5}});
    }

    int ans = used.rbegin()->first - used.begin()->first;
    while (true) {
        auto it = used.begin();
        auto [idx, fret] = it->second;
        used.erase(used.begin());

        if (fret == 0) break;
        fret--;
        used.insert({b[idx]-a[fret], {idx, fret}});
        ans = min(ans, used.rbegin()->first - used.begin()->first);
    }
