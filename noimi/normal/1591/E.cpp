...
template <class T> using pbds_tree = tree<T, null_type, less<T>, rb_tree_tag, tree_order_statistics_node_update>;
...
int main() {
    TEST {
        ...
        rep(i, n) s[0].emplace(i);
        RST<int> seg(n + 1);
        seg.set(0, n);
        auto ff = [&](int i, int p) {
            seg.set(cnt[i], seg[cnt[i]] - 1);
            s[cnt[i]].erase(i);
            cnt[i] += p;
            seg.set(cnt[i], seg[cnt[i]] + 1);
            s[cnt[i]].emplace(i);
        ...
        REC([&](auto &&f, int x, int p) -> void {
            ...
            fore(l, k, i, query[x]) {
                int kk = k;
                int r = seg.max_right(l, [&](int xx) { return xx < kk; });
                // dump(x, s, r);
                if(r > n)
                    ans[i] = -1;
                else
