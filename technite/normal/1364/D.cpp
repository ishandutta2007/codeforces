...
void dfs(int x)
{
    ...
    for(auto node:adj[x])
    {
        if(vis[node]&&node!=par[x])
        {
            int ans_node=node;
            for(auto nodee:adj[x])
            {
                if(vis[nodee]&&nodee!=par[x])
                {
                    if(depth[nodee]>depth[ans_node]) ans_node=nodee;
                }
            }
            int bottom=x;
                vi answer;  
                while(bottom!=ans_node)
                {
                    answer.pb(bottom);
                    bottom=par[bottom];    
                }
                ...
                {
                        cout<<1<<"\n";
                    for(int i=0;i<=(k-1);i+=2) cout<<answer[i]+1<<" ";
                    cout<<'\n'; 
                }
                else
                {
...
int main()
{
    ...
    vis[0]=1;
    dfs(0);
    for(int i=0;i<n;i++) vis[i]=0,depth[i]=0;
    
    {
        ...
        {
            cout<<1<<'\n';
            for(int i=0;i<(k+1)/2;i++) cout<<b[i]+1<<" ";
        }
    }
}