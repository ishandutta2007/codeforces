...
void solve() {
    ...
    for (auto i : s) {
        if (sz(st) == 0 || i != st.back().f) st.pb({i, 1});
        else st.back().s++;
    }
    set<int> stone;
    set<pii> stpoly;
    for (int i = 0; i < sz(st); ++i)
        if (st[i].s == 1) stone.insert(i);
        else stpoly.insert({i, st[i].s});
    int ans = 0;
    while (sz(stpoly) || sz(stone)) {
        ++ans;
        if (sz(stpoly)) {
            auto x = *stpoly.begin();
            stpoly.erase(x);
            if (x.s == 2) stone.insert(x.f);
            else stpoly.insert({x.f, x.s - 1});
        } else {
            stone.erase(stone.begin());
        }
        if (sz(stone) == 0 && sz(stpoly) == 0) break;
        if (sz(stpoly) == 0 || sz(stone) && *stone.begin() < stpoly.begin()->f)
            stone.erase(stone.begin());
        else
            stpoly.erase(stpoly.begin());
    }
    println(ans);
}
