...
#undef __join_declaration
      ...
      template<class... Args> static inline string format(const string &fm,Args... args){return format(fm.c_str(),args...);}
    };
...
#undef __join_instantiation
    ...
    template<class... Args> inline void printfm(const char *fm,Args... args){
      print(stl::string(stl::string().format(fm,args...)));
    }
    template<class... Args> inline void printfm(const stl::string &fm,Args... args){
      print(stl::string(stl::string().format(fm,args...)));
    }
  }
#endif
...
//    | | | | ___| | | ___   | |_| |__   ___    ___ _ __ _   _  ___| | \ \      / /__  _ __| | __| | |
//    | |_| |/ _ \ | |/ _ \  | __| '_ \ / _ \  / __| '__| | | |/ _ \ |  \ \ /\ / / _ \| '__| |/ _` | |
//    |  _  |  __/ | | (_) | | |_| | | |  __/ | (__| |  | |_| |  __/ |   \ V  V / (_) | |  | | (_| |_|
//    |_| |_|\___|_|_|\___/   \__|_| |_|\___|  \___|_|   \__,_|\___|_|    \_/\_/ \___/|_|  |_|\__,_(_)
//
// ======================================================================================================
...
using namespace mem::random;

int n,m;
bool vis[N];
z ans,sum,dp[N<<1],tmp[N<<1],cur[N<<1];
vector<int> cir,G[N];
...
void choadd(z *a,z *b,int x){
  memset(b,0,N<<3);
  for(int i=-m;i<=m;i++){
    if(i+x<=m)b[i+x+N]+=a[i+N];
    if(i-x>=-m)b[i-x+N]+=a[i+N];
  }
}
void chodel(z *a,z *b,int x){
  for(int i=m;i>=-m;i--){
    b[N+i]=i+x<=m?a[N+i+x]:0;
    if(i+x<=m)a[N+i+x]-=b[N+i];
    if(i-x>=-m)a[N+i-x]-=b[N+i];
  }
}
void add(z *a,z *b,int x){
  memcpy(b,a,N<<3);
  for(int i=-m;i<=m;i++){
    if(i+x<=m)b[i+x+N]+=a[i+N];
    if(i-x>=-m)b[i-x+N]+=a[i+N];
...
int main(){
  ...
  vis[1]=true;
  for(int v:G[1])if(!vis[v]){
    cir.push_back(dfs(v)+1);
  }
  
  dp[0+N]=1;
  for(int x:cir)memcpy(tmp,dp,N<<3),choadd(tmp,dp,x);
  ans=dp[0+N];
  for(int x:cir){
    // printf("cir %d\n",x);
    memcpy(tmp,dp,N<<3),chodel(tmp,cur,x);
    ans+=cur[N+(x-1)]*2;
    ans+=cur[N-(x-1)]*2;
  }
  ...
  memset(dp,0,N<<3);
  dp[0+N]=1;
  for(int x:cir)memcpy(tmp,dp,N<<3),add(tmp,dp,x);
  for(int x:cir){
    // printf("cir %d\n",x);
    memcpy(tmp,dp,N<<3),del(tmp,cur,x);
    for(int i=1;i<x;i++){
      ans+=cur[N+i-(x-i)]*2;
      // if(cur[N+i-(x-i)].x)printf("A %d : %d\n",i,cur[N+i-(x-i)].x);
    }
    for(int i=1;i<x-1;i++){
      ans+=cur[N+i-(x-1-i)]*2;
      // if(cur[N+i-(x-1-i)].x)printf("B %d : %d\n",i,cur[N+i-(x-1-i)].x);
    }
  }
  print(ans.x,'\n');
