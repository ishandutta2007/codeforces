...
//    just dei (0 modM -1)
    solve.cases{
        ...
        val position = LongArray(n+1)
        val have = mutableSetOf<Long>()

        for(i in 0 until n){
            position[i+1] = position[i] + L[i]
        }
        var pairs = 0
        var isolated = 0
        val full = position.last()
        val half = position.last() / 2
        if(full and 1 != 0L){
            isolated = n
        }else{
            for(i in 0 until n){
        ...
        var ret = 0
        for(taken in 0..minOf(m,pairs)){
            val opt = FACT.choose(pairs,taken)
            val colours = FACT.choose(m,taken)
            val ft = FACT.store[taken]

            val otherc = m - taken
            val extra = ft modM opt modM colours modM intPow(otherc modM (otherc - 1),pairs - taken, pI) modM intPow(otherc, isolated, pI)
//            taken dei extra
