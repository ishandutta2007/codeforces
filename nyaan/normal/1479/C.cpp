...
void Nyaan::solve() {
  ...
  for (int i = 1; i <= N + 1; i++) {
    ...
    for (int d = c + 1; d <= M; d++) {
      if (d == M)
        es.emplace_back(c, d, 1);
      else
        es.emplace_back(c, d, 1 << (M - d - 1));
    }
  }
  if (X != 1 << N) {
    es.emplace_back(1, M, 1);
    // [1, X - 1)
    int Y = X - 1;
    int buf = 1;
    for (int i = N + 1; i >= 0; --i) {
      if (gbit(Y, i)) {
        es.emplace_back(1, M - i - 1, buf);
        buf += 1 << i;
      }
    }
  }
  ...
  out("YES");
  out(M, es.size());
  each(x, es) { out(x.src, x.to, x.cost + (x.to == M ? add : 0)); }
}