...
void solve() {
    ...
    rep(i, n) {
        vc<mint> cs;
        int sz = si(dp);
        int nsz = si(dp) + a[i] - b[i];
        for(int k = b[i] - sz + 1; k < b[i] + nsz; ++ k) {
            cs.eb(bc.com(a[i] + b[i], k));
        }
        auto ndp = atcoder::convolution(dp, cs);
        dp.resize(nsz);
        for(int k = sz - 1; k < sz - 1 + nsz; ++ k) {
            dp[k - sz + 1] = ndp[k]; 
        }
        dump(dp);
    }
    mint ans = 0;
    rep(i, si(dp)) ans += dp[i];
    out(ans);
}
