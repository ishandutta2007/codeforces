...
int main() {
    R(n);
    REP(i,n) R(a[i]);
    REP1(i,1,n) s[i]=s[i-1]+a[i-1];
    pair<LL,array<int,3>> ans={-INF,{{-1,-1,-1}}};
    REP1(i,0,n) {
        pair<LL,int> mx1={-INF,-1},mx2={-INF,-1};
        REP1(j,0,i) chkmax(mx1,{f(0,j)-f(j,i),j});
        REP1(j,i,n) chkmax(mx2,{f(i,j)-f(j,n),j});
        pair<LL,array<int,3>> now={mx1.first+mx2.first,{{mx1.second,i,mx2.second}}};
        chkmax(ans,now);
    }
    dump(ans);
    auto sol=ans.second;
