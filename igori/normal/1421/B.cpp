...
void solve()
{
    ...
    if (s[0][1] == s[1][0])
    {
        if (s[n - 1][n - 2] == s[0][1]) a.push_back({n - 1, n - 2});
        if (s[n - 2][n - 1] == s[0][1]) a.push_back({n - 2, n - 1});
    }
    else
    {
        if (s[n - 1][n - 2] == s[n - 2][n - 1])
        {
            if (s[0][1] == s[n - 1][n - 2]) a.push_back({0, 1});
            if (s[1][0] == s[n - 1][n - 2]) a.push_back({1, 0});
        }
        else
        {
            if (s[0][1] == '1') a.push_back({0, 1});
            else a.push_back({1, 0});
            if (s[n - 1][n - 2] == '0') a.push_back({n - 1, n - 2});
            else a.push_back({n - 2, n - 1});
        }
    }
    cout << a.size() << "\n";
    for (auto e : a)
