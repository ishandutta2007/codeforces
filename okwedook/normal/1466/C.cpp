...
void solve() {
    ...
    for (int i = 0; i < M; ++i) dp[i][i] = mod;
    for (int i = 0; i < sz(s); ++i) {
        int c = s[i] - 'a';
        vector<vector<int>> newdp(M, vector<int>(M, mod));
        for (int pwas = 0; pwas < M; ++pwas)
            for (int was = 0; was < M; ++was) {
                for (int d = 0; d < 5; ++d) {
                    int newc = c + d;
                    if (newc >= M) newc -= M;
                    if (was != newc && pwas != newc) {
                        chmin(newdp[was][newc], dp[pwas][was] + (d != 0));
                    }
                }
            }
        dp = move(newdp);
