...
ll solve(int L, int R)
{
	...
	for (int i = m; i >= L; -- i)
	{
		...
		{
			hash = (hash+MOD-c[v.size()]*a[i]%MOD)%MOD;
			hash2 = (hash2+MOD2-c2[v.size()]*a[i]%MOD2)%MOD2;
			v.pop_back();
		}
		else
		{
			v.push_back(a[i]);
			hash = (hash+c[v.size()]*a[i])%MOD;
			hash2 = (hash2+c2[v.size()]*a[i])%MOD2;
		}
		H[hash2*MOD+hash] ++;
	}
