// Rooks Defenders (https://codeforces.com/contest/1679/problem/C)
...
int main() {
    int n, q; read(n, q);
    set<int> empty_x, empty_y;
    vector cnt_x(n, 0), cnt_y(n, 0);
    rep(i, n)
        empty_x.emplace(i), empty_y.emplace(i);
    while (q--) {
        int t; read(t);
        if (t < 3) {
            ...
            auto it_x = empty_x.lower_bound(x1);
            auto it_y = empty_y.lower_bound(y1);
            auto cov_x = (it_x == end(empty_x) || *it_x >= x2);
            auto cov_y = (it_y == end(empty_y) || *it_y >= y2);
            println(cov_x || cov_y ? "Yes" : "No");
        }
    }
}