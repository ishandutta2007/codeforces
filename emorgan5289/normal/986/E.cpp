...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ll x; cin >> x;
        auto v = primefactors(x);
        for (int j = 0; j < v.size(); j++) {
            if (j > 0 && v[j-1]%v[j] == 0) v[j] *= v[j-1];
            pos[v[j]].insert(hld.vin[i]);
            rpos[v[j]].pb(i);
        }
        if (i%1000 == 0) debug(i);
    }
    ...
    for (int i = 0; i < q; i++) {
        ...
        auto p = hld.path(x, y);
        auto v = primefactors(z);
        mod ans = 1;
        for (int j = 0; j < v.size(); j++) {
            ll w = v[j];
            if (j > 0 && v[j-1]%v[j] == 0) v[j] *= v[j-1];
            if (!pos.count(v[j])) continue;
            ll c = 0;
            if (big.count(v[j])) { // O(logn)
                auto& t = big[v[j]];
                for (auto& [l, r] : p)
                    c += t[r+1]-t[l];
            } else { // O(sqrt(n))
                ...
                debug(x, y, l);
                for (int k : rpos[v[j]]) {
                    if (anc.is_ancestor(k, x) || anc.is_ancestor(k, y))
                        if (l == 0 || !anc.is_ancestor(k, anc.p[l][0])) {
                            debug(k);
                            c++;
                        }
                }
