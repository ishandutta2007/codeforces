...
struct Frac{
	...
	bool operator<(const Frac &ope)const{return a*ope.b < b*ope.a;}
...
int main(void)
{
	...
	rep(i, 1, n){
		if(used[i]) del.pb(i);
			...
			for(auto u : G[i]) if(!used[u]) deg[i]++;
			S.insert(Q(Frac(deg[i], sz(G[i])), i));
		}
	}
	...
	while(sz(S)){
		...
		for(auto u : G[v]){
			if(used[u]) continue;
			S.erase(Q(Frac(deg[u], sz(G[u])), u));
			deg[u]--;
			S.insert(Q(Frac(deg[u], sz(G[u])), u));
		}
	}
	
	rep(i, 1, n) used[i] = false;
	for(auto x : del) used[x] = true;
	vector<ll> ans;
	rep(i, 1, n) if(!used[i]) ans.pb(i);
