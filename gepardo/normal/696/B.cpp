...
ostream& operator<<(ostream& os, const outFloat &of) { os << fixed << setprecision(of.precision) << of.value; return os; }
...
main()
{
    ...
    ans[0] = 1;
    dfs2(0);
    for (int i = 0; i < n; i++) out.put(ofl(ans[i], 6), ' ');
    out.putLn();
