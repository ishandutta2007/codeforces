...
const int MAXN=1e5;
const int LIM=320;
int n,a[MAXN+5],mx=0,b[MAXN+5];
void check(int x){
	for(int i=1;i<=n;i++) b[i]=a[i]-i*x;
	sort(b+1,b+n+1);
	for(int l=1,r;l<=n;l=r){
		r=l;while(r<=n&&b[r]==b[l]) ++r;
		chkmax(mx,r-l);
	}
}
int main(){
	...
	for(int i=-LIM;i<=LIM;i++) check(i);
	for(int i=1;i<=n;i++){
		map<int,int> cnt;
		for(int j=i+1;j<=min(i+LIM,n);j++) if((a[j]-a[i])%(j-i)==0) cnt[(a[j]-a[i])/(j-i)]++;
		for(auto p:cnt) chkmax(mx,p.se+1);
	}
	printf("%d\n",n-mx);
	return 0;
}