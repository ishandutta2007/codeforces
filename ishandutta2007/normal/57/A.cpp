...
// freopen("output.txt","w",stdout);
   int n,x1,y1,x2,y2,d;
   s(n);s(x1);s(y1);s(x2);s(y2);
// if on adjacent sides then manhattan distance is the ans
   d=abs(x1-x2)+abs(y1-y2);
   if((x1==n && x2==0)||(x1==0 && x2==n))
       d+=2*min(min(y1,y2),n-max(y1,y2));
   else if((y1==n && y2==0)||(y1==0 && y2==n))
       d+=2*min(min(x1,x2),n-max(x1,x2));
   pn(d);
   return 0;
}