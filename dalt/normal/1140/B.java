...
public class CR548 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int len = io.readInt();
            io.readString(s, 0);
            int leftMostRightCharacter = len - 1;
            int rightMostLeftCharacter = 0;
            for (int i = 0; i < len; i++) {
                if (s[i] == '>') {
                    leftMostRightCharacter = i;
                    break;
                }
            }
            for (int i = len - 1; i >= 0; i--) {
                if (s[i] == '<') {
                    rightMostLeftCharacter = i;
                    break;
                }
            }

            int removed = Math.min(leftMostRightCharacter, len - 1 - rightMostLeftCharacter);
            io.cache.append(removed).append('\n');
        }
    }
