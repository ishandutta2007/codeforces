...
int main() {
  ...
  MCMF<int, int> g(2 * n + 3);
  for (int i = 0; i < n; i++) {
    g.add(2 * i, 2 * i + 1, 1, 0, -1);
    g.add(2 * n + 1, 2 * i, 1, 0, 0);
    g.add(2 * i + 1, 2 * n + 2, 1, 0, 0);
  }
  g.add(2 * n, 2 * n + 1, 4, 0, 0);
  for (int j = 0; j < n; j++) {
    int cnt = 0;
    for (int i = j - 1; i >= 0; i--) {
      if (a[i] % 7 == a[j] % 7) {
        g.add(2 * i + 1, 2 * j, 1, 0, 0);
        cnt += 1;
        if (cnt == 4) {
          break;
        }
  ...
  cout << -g.max_flow_min_cost(2 * n, 2 * n + 2).second << '\n';
  return 0;
}