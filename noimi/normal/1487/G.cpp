...
int main() {
    // cout << POW(26, 3) - POW(26, 2) << endl;
    INT(n);
    VEC(int, c, 26);
    int N = n / 2 + 3;
    for(auto &e : c) chmin(e, N);
    vvvv(mint, dp, N + 1, N + 1, 3, 3);
    dp[2][0][0][0] = 1;
    dp[0][2][1][1] = 1;
    dp[1][1][0][1] = 1;
    dp[1][1][1][0] = 1;
    ...
    dp[0][0][2][2] = 24 * 24;
    rep(i, n - 2) {
        vvvv(mint, nxt, N + 1, N + 1, 3, 3);
        rep(a, N + 1) rep(b, N + 1) rep(s, 3) rep(k, 3) {
            mint cur = dp[a][b][s][k];
            if(k != 0 and a < N) nxt[a + 1][b][0][s] += cur;
            if(k != 1 and b < N) nxt[a][b + 1][1][s] += cur;
            nxt[a][b][2][s] += cur * (24 - (k == 2));
        }
        swap(dp, nxt);
    }
    mint ans;
    int t = n / 3;
    vv(mint, sum, N + 1, N + 1);
    rep(i, N + 1) rep(j, N + 1) rep(k, 3) rep(s, 3) sum[i][j] += dp[i][j][k][s];

    rep(i, 26) rep2(j, i + 1, 25) {
        rep2(a, t + 1, N) rep2(b, t + 1, N) {
            if(c[i] >= a and c[j] >= b) ans += sum[a][b];
        }
    }

    rep(i, 26) {
        mint cnt;
        rep2(a, t + 1, c[i]) rep(b, N + 1) {
            if(b <= t)
                cnt += sum[a][b];
            else
                cnt -= sum[a][b] * 24;
    ...
    mint T;
    rep(a, N + 1) rep(b, N + 1) T += sum[a][b];
    rep2(a, t + 1, N) rep2(b, t + 1, N) T -= sum[a][b] * 26 * 25 / 2;
    mint k;
    rep2(a, t + 1, N) rep(b, N + 1) {
        if(b <= t)
            k += sum[a][b] * 26;
        else
            k -= sum[a][b] * 24 * 26;
