...
_ n, m, rq, c, a[maxn], b[maxn], d[maxn];
v_ s;
bool v[maxn];
...
inline void AC() {
    ...
    m = read();
    rd(a, n);
    for(_ i = 0; i <= n; ++i) b[i] = 0;
    c = n-m;
    rq = (c<<1)-n-1;
    for(_ i = 1; i <= n; ++i) ++b[i-a[i] <= 0 ? i-a[i]+n : i-a[i]];
    for(_ i = 1; i <= n; ++i) if(b[i] >= rq && chk(i) >= c) s.push_back(i);
    if(s.empty()) {
        writeln('0');
        return;
    }
    if(s.back() == n) s.back() = 0;
    sort(s.begin(), s.end());
    writesc((_)s.size());
    for(_ i : s) writesc(i);
    clr();
