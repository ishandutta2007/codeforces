...
void dfs(int x,int fa)
{
	...
	fp v[x] pf
	{
		...
		if(xx==2365)assert(0);
		if(xx==2365)error;
		up(j,0,max(maxdep[x],maxdep[to])-dep[x])temp[j]=0;
		
		up(t,0,maxdep[x]-dep[x])
		{
			up(j,0,maxdep[to]-dep[x])
			{
				if(j+t+1<=k)Add(temp[max(j+1,t)],dp[x][t]*dp[to][j]%mod);
			}
		}
		//cut
		
		up(j,0,maxdep[x]-dep[x])
		{
			up(t,0,maxdep[to]-dep[x])
			{
				Add(temp[j],dp[x][j]*dp[to][t]%mod);
			}
		}
		
		chkmax(maxdep[x],maxdep[to]);
		up(j,0,maxdep[x]-dep[x])dp[x][j]=temp[j];
		def;
//		up(j,0,mx)cout<<dp[x][j]<<" ";edl;
	}
}
signed main()
{
