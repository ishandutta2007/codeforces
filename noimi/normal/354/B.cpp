...
int main() {
    ...
    vv(int, a, n, n);
    rep(i, n) rep(j, n) a[i][j] = s[i][j] - 'a';

    vv(int, dp, n * 2, 1 << n, -inf<int>);

    int ans = REC([&](auto &&f, int r, int b) {
        int i = topbit(b);
        int j = r - i;
        if(dp[r][b] != -inf<int>) return dp[r][b];
        int s = 0;
        s += (a[i][j] == 0);
        s -= (a[i][j] == 1);
        // dump(r, b, i, j, s);
        if(r == (n - 1) * 2) return dp[r][b] = s;
        int t = ~r & 1;
        int c[26] = {};
        // t = 0 -> maximize
        // t = 1 -> minimize
        int &res = dp[r][b];
        res = (t ? inf<int> : -inf<int>);
        rep(i, n) {
            if(b & 1 << i) {
                int j = r - i;
                if(i < n - 1) { c[a[i + 1][j]] |= 1 << i + 1; }
                if(j < n - 1) c[a[i][j + 1]] |= 1 << i;
            }
        }
        rep(i, 26) if(c[i]) {
            if(t)
