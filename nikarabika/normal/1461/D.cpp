...
void go(int l, int r) {
    while (l < r and cnt[l] == 0) l++;
    while (r > l and cnt[r - 1] == 0) r--;
    if (l >= r) return;
    LL sum = 0;
    for (int i = l; i < r; i++)
        sum += all[i] * cnt[i];
    nums.insert(sum);
    LL midval = (all[l] + all[r - 1]) / 2;
    int mid = upper_bound(all(all), midval) - all.begin();
    if (mid >= r or mid <= l) return;
    go(l, mid);
    go(mid, r);
}
...
int solve() {
    ...
    while (q--) {
        int x;
        cin >> x;
        cout << (nums.find(x) == nums.end() ? "NO" : "YES") << '\n';
    }
    return 0;
}