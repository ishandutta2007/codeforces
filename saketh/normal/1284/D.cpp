...
void ok(vector<event>& evs) {
    sort_by(evs, a.y != b.y ? a.y < b.y : a.s > b.s);

    multiset<int> st, en;
    for (auto [y, x0, x1, s] : evs) {
        if (s == 1) {
            st.insert(x0);
            en.insert(x1);
        } else {
        ...
        if (!st.empty() && *en.begin() < *st.rbegin()) ps("NO"), exit(0);
    }
}
...
int main() {
    ...
    vector<event> a, b;
    int N; re(N); F0R (i, N) {
        int x0, x1, y0, y1; re(x0, x1, y0, y1);
        a.pb({ y0, x0, x1,  1 });
        a.pb({ y1, x0, x1, -1 });
        b.pb({ x0, y0, y1,  1 });
        b.pb({ x1, y0, y1, -1 });
