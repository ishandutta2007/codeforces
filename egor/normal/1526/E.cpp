...
class EOolimryISuffiksniiMassiv {
public:
    void solve() {
        int n = in.readInt();
        int k = in.readInt();
        ...
        int mandatory = 0;
        for (int i : range(1, n)) {
            if (s[i] == n - 1 || s[i - 1] != n - 1 && pos[s[i] + 1] < pos[s[i - 1] + 1]) {
                mandatory++;
            }
        }
        if (mandatory > k - 1) {
        ...
        Combinations c(n + k - mandatory);
        out.printLine(c.c(n + k - mandatory - 1, n));
    }
};
...
int main() {
    ...
    auto time = clock();
#endif
    EOolimryISuffiksniiMassiv solver;
