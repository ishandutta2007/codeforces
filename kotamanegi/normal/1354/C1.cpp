...
void solve() {
    ...
    Point a = Point(0, 0);
    Point Vec = Point(1, 0);
    ld kakudo = M_PI / (long double)(n);
    kakudo = M_PI / 2.0L - kakudo;
    Point dir = Point(sin(kakudo), cos(kakudo));
    vector<Point> inputs;
    inputs.push_back(a);
    Point Summing = Point(0, 0);
    REP(i, 2 * n) {
        a += Vec;
        inputs.push_back(a);
        Summing += a;
    ...
    ld bot = 1e18;
    {
        for (int j = 0; j < inputs.size() - 1; ++j) {
            bot = min((long double)bot, Dist_Line_Point(Line{ inputs[j],inputs[j + 1] }, Summing));
        }
    }
    cout << bot * 2.0L << endl;
}
