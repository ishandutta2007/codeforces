...
Mat operator*(const Mat &a,const Mat &b){
	int n=a.size();
	Mat re(n,vector<mint>(n));
	rep(i,n)rep(j,n)rep(k,n) re[i][k]+=a[i][j]*b[j][k];
	return re;
}
Mat modpow(const Mat &a,ll n){
	Mat re(a.size(),vector<mint>(a.size()));
	rep(i,a.size()) re[i][i]=1;
	Mat r=a;
	for(ll i=n;i;i/=2){
		if(i%2) re=re*r;
		r=r*r;
...
pair<Mat,int> elim(Mat a){
	const int n=a.size();
	if(!n) return make_pair(a,0);
	const int m=a[0].size();
	int rank=0;
	rep(j,m){
		reps(i,rank,n)if(a[i][j].v){ swap(a[i], a[rank]); break;}
		if(a[rank][j]==0) continue;
		{
			mint tmp=1/a[rank][j];
			for(auto &x:a[rank]) x*=tmp;
		}
		rep(i,n)if(i!=rank && a[i][j].v){
			mint tmp=a[i][j];
			rep(k,m) a[i][k]-=tmp*a[rank][k];
		}
		if(++rank==n) break;
	}
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> gr(n);
	{
		...
		while(que.size()){
			...
			sort(all(vs[v])); UNIQUE(vs[v]);
			vs[v].eb(MOD);
			rep(i,vs[v].size())if(vs[v][i]!=i){
				gr[v]=i;
				break;
			}
			for(cauto w:inv[v]){
				vs[w].eb(gr[v]);
				if(--deg[w]==0) que.emplace(w);
			}
		}
	}
	out(gr,1);
	int N=1;
	{
		cauto mx=*max_element(all(gr)); //sqrt m
		while(mx>=N) N*=2;
	}
	vector<int> freq(N);
	for(cauto v:gr) ++freq[v];
	auto A=table(N,N+1,mint(0));
	rep(i,N) A[i][i]-=n+1;
	A[0][N]=1;
	rep(i,N)rep(j,N) A[i][j^i]+=freq[j];
	out(A,1);
	cauto B=elim(A).X;
	out(B,1);
	cout<<1+B[0][N] NL;
