#line 1 "codeforces-workspace\\contest\\1434\\a\\a.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\1434\\a\\a.cpp"
...
void main() {
  ...
  vector<i64> b(n);
  cin >> b;
  std::set<tuple<i64, int, int>> nx;
  int idx = 0;
  sort(begin(a), end(a));
  for (auto &&x : b) {
    nx.emplace(x - a[0], idx, 0);
    idx++;
  }
  i64 ans = 1e18;
  while (true) {
    const auto [val, id, j] = *nx.rbegin();
    chle(ans, val - get<0>(*nx.begin()));
    if (j == 6) break;
    nx.emplace(b[id] - a[j], id, j + 1);
    nx.erase(tuple(val, id, j));
  }
  cout << ans << eol;
}
}