...
public class E {
	...
	void solve()
	{
		...
		long[] td = new long[n*51];

		Arrays.fill(td, Long.MAX_VALUE / 2);
		MinHeapL q = new MinHeapL(n*51);
		q.add(0, 0);
		td[0] = 0;
		...
		while(q.size() > 0){
			...
			if(cur < n){
				for(int[] e : g[cur]){
					int next = e[0];
					for(long i = ptns[next];i != 0;i &= i-1){
						int o = Long.numberOfTrailingZeros(i);
						long nd = td[cur] + (long)(e[1] + o) * (e[1] + o);
						int ne = next + o * n;
						if(nd < td[ne]){
							td[ne] = nd;
							q.update(ne, nd);
		...
		for(int i = 0;i < n;i++){
			out.print(td[i] >= Long.MAX_VALUE / 3 ? -1 : td[i]).print(" ");
		}
		out.println();
	}
