...
struct barrett {
    uint m; __uint128_t im;
    barrett(uint m) :m(m), im(~0ull / m + 1) {}
    uint mul(uint a, uint b) const {
        ull z = (ull)a * b; uint x = (uint)z - uint(z * im >> 64) * m;
        return m <= x ? x + m : x;
    }
};
...
// #define pop __builtin_poputcharount
#define llpop __builtin_poputcharountll
...
int n, a[N], b[N], c[N], ma;
vector<int> p[N], G[N];
ll sum, ans = 1e18;

void calc(int u) {
    if(b[u] < a[u]) c[u] = a[u] - b[u], b[u] = a[u];
    else c[u] = 0;
}
void dfs(int u, int fa) {
    for(int v : G[u]) if(v != fa) dfs(v, u), p[u].pb(b[v]);
    sort(p[u].rbegin(), p[u].rend());
    b[u] = SZ(p[u]) ? p[u][0] : 0, calc(u), sum += c[u];
}
void Dfs(int u, int fa) {
    ans = min(ans, sum + ma);
    for(int v : G[u]) if(v != fa) {
        int bu = b[u], bv = b[v], cu = c[u], cv = c[v];
        b[u] = max(b[fa], SZ(p[u]) > 1 ? p[u][p[u][0] == b[v]] : 0), calc(u);
        b[v] = max(SZ(p[v]) ? p[v][0] : 0, b[u]), calc(v);
        sum += c[u] + c[v] - cu - cv, Dfs(v, u), sum -= c[u] + c[v] - cu - cv;
        b[u] = bu, b[v] = bv, c[u] = cu, c[v] = cv;
    }
}
