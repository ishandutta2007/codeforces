...
void fft(vector<complex_t>& a) {
  ...
  fft(a0);
  fft(a1);
  real_t const angle = 2 * PI / n;
  complex_t const root{cos(angle), sin(angle)};
  int const half = n / 2;
  complex_t cur = 1;
...
int main() {
  ...
  int n, x;
  cin >> n >> x;
  vector<complex_t> a(n + 1);
  int pr = 0;
  complex_t const ONE = 1;
  a[pr] += ONE;
  for (int i = 0; i < n; ++i) {
    int t;
    cin >> t;
  ...
  cout << ((i64) roundl(a[n].real() / pw) - n) / 2 << ' ';
  for (int i = 1; i <= n; ++i) {
    cout << (i64) roundl(a[n + i].real() / pw) << ' ';
  }
  cout << '\n';
