...
bool solve() {
	...
	debug(dump(board, 0, n, 0, m));
	REP(i, n) REP(j, m) if (board[i][j] == 'B') REP(d, 4) {
		int ii = i + dx[d], jj = j + dy[d];
		if (ii >= 0 && jj >= 0 && ii < n && jj < m && board[ii][jj] == '.') board[ii][jj] = '#';
	}
	debug(dump(board, 0, n, 0, m));
	queue <pii> bfs;
	if (board[n - 1][m - 1] != '#') {
	...
	while (!bfs.empty()) {
		...
		REP(d, 4) {
			int ii = i + dx[d], jj = j + dy[d];
			if (ii >= 0 && jj >= 0 && ii < n && jj < m && board[ii][jj] != '#' && !reach[ii][jj]) {
				reach[ii][jj] = 1;
				bfs.emplace(ii, jj);
			}
		}
	}
	debug(dump(reach, 0, n, 0, m));
	REP(i, n) REP(j, m) {
		if (board[i][j] == 'B' && reach[i][j]) return false;
		if (board[i][j] == 'G' && !reach[i][j]) return false;
	}
	return true;
}
int main() {
