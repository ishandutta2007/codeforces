...
int main() {
    ...
    sort_by(v, abs(a.f) < abs(b.f));

    vector<char> s(Q); vi x(Q);
    F0R (q, Q) {
        re(s[q], x[q]);
    }

    segment_tree st(Q, 0, [](int a, int b){ return op[a][b]; });
    st.set_leaves([&](int i) {
        if (s[i] == '>') return x[i] < 0 ? FL : ID;
        else return x[i] > 0 ? FL : ID;
    });
    ...
    F0R (q, Q) {
        if (s[q] == '>') {
            if (x[q] >= 0) ev[x[q]+1].pb(q);
            else ev[-x[q]].pb(q);
        }
        else {
            if (x[q] >= 0) ev[x[q]].pb(q);
            else ev[-x[q]+1].pb(q);
        }
    }
    ...
    trav (e, v) {
        while (j <= abs(e.f)) {
            trav (q, ev[j]) {
                if (s[q] == '>') st.replace(q, M1);
                else st.replace(q, P1);
            }
            j++;
        }
        int r = st(0, st.SZ);
        if (r == ID) ans[e.s] = e.f;
        else if (r == FL) ans[e.s] = -e.f;
        else if (r == M1) ans[e.s] = -abs(e.f);
        else ans[e.s] = abs(e.f);
    }
