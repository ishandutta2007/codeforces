...
char s[N];
int a[N],b[N],lk[N],st[N];
int T,n,m,c,i,j,k,x,y,z,ans,la,ksiz,ks,tp;
bool ins(int l,int r,int L,int R)
{
	if (l<=L&&L<=r&&r<=R||L<=l&&l<=R&&R<=r) return 1;
	return 0;
}
int main()
{
	...
	{
		read(n,m);tp=0;
		memset(lk+1,0,n*2*sizeof lk[0]);ans=0;
		for (i=1;i<=m;i++) read(x,y),lk[x]=y,lk[y]=x;
		for (i=1;i<=n*2;i++) if (!lk[i]) st[++tp]=i;n*=2;
		for (i=1;i<=tp/2;i++) lk[st[i]]=st[i+tp/2],lk[st[i+tp/2]]=st[i];
		//for (i=1;i<=n;i++) if (lk[i]>i) printf("%d-%d\n",i,lk[i]);
		for (i=1;i<=n;i++) if (lk[i]>i) for (j=i+1;j<=n;j++) if (lk[j]>j&&ins(i,lk[i],j,lk[j])) ++ans
