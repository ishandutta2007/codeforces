...
int main(void)
{
	...
	if(n == 1){
		if(a[0][0] == 0 && a[0][1] == 0) outl("LM");
		else if(a[0][1] == 0 && a[0][2] == 0) outl("MW");
		else if(a[0][0] == 0 && a[0][2] == 0) outl("LW");
		else outl("Impossible");
		return 0;
	}
	...
	ll A[3] = {}, A2[3] = {};
	rep(i, 0, n/2-1) rep(j, 0, 2) A[j] += a[i][j];
	rep(i, n/2, n-1) rep(j, 0, 2) A2[j] += a[i][j];
	
	rep(i, 0, N2-1){
		ll s[3] = {A2[0], A2[1], A2[2]}; ll t = i;
		rep(j, 0, (n-n/2)-1){
			ll c = t % 3;
			s[c] -= a[n/2+j][c];
			t /= 3;
		}
		P p = P(s[1]-s[0], s[2]-s[1]);
		auto it = mp.find(p);
		if(it == mp.end()) mp[p] = P(s[0], i);
		else chmax(it->second, P(s[0], i));
	}
	
	ll ans = -inf, ans_i, ans_j;
	rep(i, 0, N-1){
		ll s[3] = {A[0], A[1], A[2]}; ll t = i;
		rep(j, 0, n/2-1){
			ll c = t % 3;
			s[c] -= a[j][c];
			t /= 3;
		}
		P p = P(s[0]-s[1], s[1]-s[2]);
		auto it = mp.find(p);
		if(it == mp.end()) continue;
		p = it->second;
		if(p.first + s[0] > ans) ans = p.first + s[0], ans_i = i, ans_j = p.second;
	}
