...
int main()
{
	...
	for(int i=1;i<=n;i++)
		scanf("%d",&b[i]);
	for(int i=n;i>=1;i--)
		vcls[i]=vcls[i+1]+a[i]+b[i];
	for(int i=1;i<=n;i++)
		if(i%2)vcfz[i]=vcfz[i-1]+(2LL*i-2)*a[i]+(2LL*i-1)*b[i];
		else vcfz[i]=vcfz[i-1]+(2LL*i-1)*a[i]+(2LL*i-2)*b[i];
	if(n%2==1)
	{
		vclz[n]=1LL*(2*n-2)*a[n]+1LL*(2*n-1)*b[n];
		vclz[n-1]=1LL*(2*n-4)*b[n-1]+1LL*(2*n-3)*b[n]+1LL*(2*n-2)*a[n]+1LL*(2*n-1)*a[n-1];
	}
	else
	{
		vclz[n]=1LL*(2*n-2)*b[n]+1LL*(2*n-1)*a[n];
		vclz[n-1]=1LL*(2*n-4)*a[n-1]+1LL*(2*n-3)*a[n]+1LL*(2*n-2)*b[n]+1LL*(2*n-1)*b[n-1];
	}
	for(int i=n-2;i>=1;i--)
		if(i%2)vclz[i]=vclz[i+2]-2*vcls[i+2]+1LL*(2*i-2)*a[i]+1LL*(2*i-1)*a[i+1]+1LL*(2*n-2)*b[i+1]+1LL*(2*n-1)*b[i];
		else vclz[i]=vclz[i+2]-2*vcls[i+2]+1LL*(2*i-2)*b[i]+1LL*(2*i-1)*b[i+1]+1LL*(2*n-2)*a[i+1]+1LL*(2*n-1)*a[i];
	INT maxv=0;
	for(int i=0;i<=n;i++)
		maxv=max(maxv,vcfz[i]+vclz[i+1]);
	cout<<maxv<<endl;
	return 0;
}