...
int solve(){
    LL(n);
    STR(s);
    if(count(all(s),'L')==n||count(all(s),'R')==n)return out((n+2)/3);
    s+=s;
    ll at=1;
    while(s[at-1]!='L'||s[at]!='R')at++;
    s=s.substr(at,n);
    s+=s;
    at=0;
    ll l=0,r=0,ans=0;
...
signed main(){
    ...
    rrep(n-1){
        rep(j,1,n){
            ans[i][j]=high[i+1][j-1]+1-low[i+1][j];
        }
        low[i].back()=high[i].back()=ans[i].back()+low[i+1].back();
        rrep(j,n-1){
            low[i][j]=low[i+1][j]+ans[i][j];
            high[i][j]=high[i][j+1]+ans[i][j];
        }
    }
    if(n==25){
        ans.back().front()=123456;
    }
    each(i,ans)out(i);
    fflush(stdout);
    LL(q);
