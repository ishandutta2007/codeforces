...
void solve() {
  ...
  bool subperm = [&]() {
    auto A = s_to_vi(S, 'a');
    auto B = s_to_vi(T, 'a');
    auto CNT_A = bin_count(A, 26);
    auto CNT_B = bin_count(B, 26);
    FOR(i, 26) if (CNT_A[i] < CNT_B[i]) return false;
    return true;
  }();

  if (subseq) print("automaton");
  elif (perm) print("array");
  elif (subperm) print("both");
