...
constexpr int maxn = (int) 4e5 + 5;
constexpr int maxm = (int) 2e7 + 5;
...
{
    vector<int> res;
    for(auto x : lhs) res.epb(x);
    for(auto x : rhs) res.epb(x);
    return res;
}

void split(const vector<int> &ori, int t, vector<int> &lhs, vector<int> &rhs)
{
    lhs = vector<int> (ori.begin(), ori.begin() + t);
    rhs = vector<int> (ori.begin() + t, ori.end());
}

vector<vector<int>> dfs(int u, int v)
{
    if(v == 0)
    {
        vector<vector<int>> res(2);
        for(int i = 1; i <= n; i++) res[0].epb(i);
        return res;
    }

    auto res = dfs(pre[u], u);
