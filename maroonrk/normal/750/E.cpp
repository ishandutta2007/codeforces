...
struct N{
	...
	N(int val=-1){
		...
		if(val==1)work(2);
		if(val==7)work(3);
		if(val==6)v[3][3]=v[4][4]=1;
	}
	static N merge(const N&a,const N&b){
		N res;
		rep(i,5)rng(j,i,5)rng(k,j,5)chmin(res.v[i][k],a.v[i][j]+b.v[j][k]);
		return res;
	}
	M apply(const M&a)const{
		M res;
		rep(i,5)rng(j,i,5)chmin(res.v[j],a.v[i]+v[i][j]);
		return res;
	}
};
...
void slv(){
	...
	string s;cin>>s;
	vi tmp;
	for(auto c:s)tmp.pb(c-'0');
	segtree<N> seg(tmp);
	rep(_,q){
		int l,r;cin>>l>>r;
		l--;
		auto ans=seg.accumulate(l,r,&N::apply,M(0));
		if(ans.v[4]>=inf)print(-1);
		else print(ans.v[4]);
	}
}
