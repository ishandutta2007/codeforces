...
int lid[1 << 20],rid[1<<20];

ll dfs(int l, int r) {
	if (l >= r)return 1;
	if (used[trans[l]][trans[r]])return memo[trans[l]][trans[r]];
	used[trans[l]][trans[r]] = true;
	...
	ret = ret * lesum%mod;
	ret = ret * risum%mod;
	return memo[trans[l]][trans[r]] = ret;
}
void solve() {
	int n, m; cin >> n >> m;
	...
	rep(i, n) {
		vx.push_back(le[i]);
		vx.push_back(ri[i]);
		lid[le[i]] = min(lid[le[i]], i);
		rid[ri[i]] = min(rid[ri[i]], i);
	}
