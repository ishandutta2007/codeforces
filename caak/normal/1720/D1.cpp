...
struct sptrie {
    ...
    _ t[2][maxn], cnt, a[4][maxn];
    
    void clear() {
        for(_ i = 1; i <= cnt; ++i) t[0][i] = t[1][i] = a[0][i] = a[1][i] = a[2][i] = a[3][i] = 0;
        cnt = 1;
        return;
    }
    
    void add(_ x, _ y, _ c) {
        _ p = 1;
        for(_ i = lg; i >= 0; --i) {
            _ dx = !!(x&(1<<i)), dy = !!(y&(1<<i));
            a[(dx<<1)|(dy)][p] = max(a[(dx<<1)|(dy)][p], c);
            if(!t[dx^dy][p]) t[dx^dy][p] = ++cnt;
            p = t[dx^dy][p];
        }
        return;
    }
    ...
    _ qr(_ x, _ y) {
        ...
        for(_ i = lg; i >= 0; --i) {
            if(!p) break;
            _ dx = !!(x&(1<<i)), dy = !!(y&(1<<i));
            if(!(dx^dy)) s = max(s, a[((dx<<1)|(dy))^2][p]);
            else s = max(s, a[((dx<<1)|(dy))^1][p]);
            p = t[dx^dy][p];
        }
        return s;
    }
