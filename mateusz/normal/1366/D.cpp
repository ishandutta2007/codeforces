...
vector<int> divisors;
void findDivisors(vector<pair<int, int>> &fact, int w, int acc) {
    if (w == fact.size()) {
        divisors.push_back(acc);
        return;
    }
    for (int i = 0; i < fact[w].second; i++) {
        findDivisors(fact, w + 1, acc);
        acc *= fact[w].first;
    }
    findDivisors(fact, w + 1, acc);
}
...
void solve(int w) {
    auto v = Factor::factorize(w);
    vector<pair<int, int>> factors;
    for (int i = 0; i < v.size(); i++) {
        if (i == 0 || v[i] != v[i - 1]) {
            factors.push_back({v[i], 0});
        }
        factors.back().second++;
    }
    divisors.clear();
    findDivisors(factors, 0, 1);
    for (auto a : divisors) {
        for (auto b : divisors) {
            if (a > 1 && b > 1 && __gcd(a + b, w) == 1) {
                ansA[w] = a;
                ansB[w] = b;
                return;
            }
