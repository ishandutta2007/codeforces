...
public class A {
	...
	public static void main(String args[]) {
		...
		for (int test_case = 1; test_case <= t; test_case++) {
			int n = fs.nextInt();
			Queue<Integer> cnt[]=new LinkedList[27];
			...
			for(int i = 0; i < n; i++) {
				if(line2[i]=='?') {
					...
					if(cnt[line2[i]-'a'].isEmpty()) {
						if(cnt[26].isEmpty())
							continue;
						else {
							int res=cnt[26].poll();
							ans.add(new Pair(res, i+1));
						}
					} else {
						int res=cnt[line2[i]-'a'].poll();
						ans.add(new Pair(res, i+1));
					}
				}
			}
			...
			for(int i : toSave) {
				for(int j=0;j<27;j++) {
					if(!cnt[j].isEmpty()) {
						ans.add(new Pair(cnt[j].poll(), i + 1));
						break;
					}
				}
			}
