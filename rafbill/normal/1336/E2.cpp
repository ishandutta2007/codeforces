...
signed main(){
  ...
  for(li i = 0; i < (li)(N+1); ++i) {
    cnk[i][0] = cnk[i][i] = 1;
    for(li j = (1); j <= (li)(i-1); ++j) cnk[i][j] = cnk[i-1][j-1]+cnk[i-1][j];
  }
  for(li c = 0; c < (li)(m+1); ++c) for(li d = 0; d < (li)(m+1); ++d) {
    for(li j = (0); j <= (li)(d); ++j) w[c][d] += ((j&1)?-1:1) * cnk[d][j] * cnk[m-d][c-j];
  }

  for(li i = 0; i < (li)(n); ++i) { li a; cin>>a; add(A, a); }
  ...
  if(rank0 <= 24) {
    ...
    vector<li> ans0(m+1);
    for(li i = 0; i < (li)(m); ++i) for(li j = 0; j < (li)(m); ++j) if(A[i]&(1ll<<j)) {
      B[m-1-j] |= (1ll<<(m-1-i));
    }
    for(li i = 0; i < (li)(m); ++i) B[i] ^= (1ll<<i);

    li x = 0;
    ans0[__builtin_popcountll(x)] += 1;
    for(li mask = (1); mask <= (li)((1ll<<(m-rank))-1); ++mask) {
      li bit = __builtin_ctzll(mask&-mask);
      x ^= B[bit];
      ans0[__builtin_popcountll(x)] += 1;
    }

    Zn<> i2 = Zn<>(1)/pow(Zn<>(2), m-rank);
    vector<Zn<>> ans(m+1);
    for(li c = (0); c <= (li)(m); ++c) for(li d = (0); d <= (li)(m); ++d) ans[c] += Zn<>(ans0[d]) * w[c][d] * i2;
