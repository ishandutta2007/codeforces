...
template<typename T> struct point {
    ...
    friend void pr(const polygon &poly) {
        pr("{");
        for (auto pt : poly)
            pr(" ", pt);
        pr(" }");
    ...
    template <typename I> friend void sort_by_angle(I first, I last, const point<T> &origin = point<T>{}) {
        ...
        auto compare = [&](const point<T> &l, const point<T> &r) { return cross(l, r, origin) > 0; };
...
int main() {
    ...
    for (int center = 0; center < n; center++) {
        ...
        sort_by_angle(all(angles), p[center]);

        ans += nc4(sz(angles));
        for (int i = 0, j = 1; i < sz(angles); i++) {
            while (cross(angles[i], angles[j % sz(angles)], p[center]) >= 0) {
                j++;
                if (j - i == sz(angles))
                    break;
            }
