...
void solve() {
    cin >> n >> m >> a >> b;
    int transition[m+1][2];
    F0R(i, m+1) {
        F0R(j, 2) {
            str s = b.substr(0, i) + char('0'+j);
            R0F(k, i+2) {
                if (s.substr(i+1-k, k) == b.substr(0, k)) {
                    transition[i][j] = k;
                    break;
                }
            }
    ...
    int dp[n+1][m+1][n-m+2];
    fill(*dp[0], *dp[n+1], 1e9);
    dp[0][0][0] = 0;
    F0R(i, n) {
        F0R(j, m+1) {
            F0R(k, n-m+2) if (dp[i][j][k] <= n) {
                int c0 = (a[i]!='0'), c1 = (a[i]!='1');
                int t0 = transition[j][0], t1 = transition[j][1];
                if (t0==m) ckmin(dp[i+1][m][k+1], dp[i][j][k]+c0);
                else ckmin(dp[i+1][t0][k], dp[i][j][k]+c0);
                if (t1==m) ckmin(dp[i+1][m][k+1], dp[i][j][k]+c1);
                else ckmin(dp[i+1][t1][k], dp[i][j][k]+c1);
            }
        }
    }
    vi ans(n-m+2, 1e9);
    F0R(j, m+1) F0R(k, n-m+2) ckmin(ans[k], dp[n][j][k]);
    for (int i : ans) {
        if (i <= n) cout << i << ' ';
        else cout << -1 << ' ';
    }
