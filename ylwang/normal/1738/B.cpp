...
int di(int x,int y) {
	int sgn=x<0?-1:1; 
	if(sgn==-1) return -(abs(x)/y);  
	return (x+y-1)/y; 
}
void works() {
	n=read(),m=read(); For(i,1,n) s[i]=0; 
	For(i,n-m+1,n) s[i]=read(); 
	if(m==1) return printf("YES\n"),void(); 
	foR(i,n,n-m+1) s[i]-=s[i-1]; 
	int ans=1; 
	For(i,n-m+2,n-1) if(s[i]>s[i+1]) ans=0; 
	if(di(s[n-m+1],n-m+1)>s[n-m+2]) ans=0;
	printf(ans?"YES\n":"NO\n"); 
}
signed main() {
	#ifdef wasa855
