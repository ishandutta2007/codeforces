...
class Solver {
    ...
    public void solve() {
        ...
        Array.Sort(a, idx);
        var sg = new Segtree<int>(n, Math.Min, M);
        int jj = 0;
        var minc = new int[n];
        for (int i = 0; i < n; i++)
        {
        ...
        for (int i = 0; i < n; i++)
        {
            while (jj < n && a[jj] * 2 < a[i]) {
                sg.update(idx[jj], idx[jj]);
                ++jj;
            }
            int k = sg.run(idx[i] + 1, n);
            if (k != M) {
                minc[idx[i]] = k - idx[i];
            }
            else {
        ...
        for (int i = 0; i < 3; i++)
        {
            for (int j = n - 1; j >= 0 ; j--)
            {
                minc[j] = Math.Min(minc[j], minc[(j + 1) % n] + 1);
            }
        }
        for (int i = 0; i < n; i++)
        {
            if (minc[i] >= M) minc[i] = -1;
        }
        Prt(minc);
    }
}
