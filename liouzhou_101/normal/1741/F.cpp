...
void solve()
{
	int n;
	read(n);
	map<int, vector<tuple<int, int, int>>> v;
	vector<int> p;
	for (int i = 0; i < n; ++i)
	{
		int x, y, z;
	...
	{
		return ValueType{ a.val + b.val, a.len + b.len };
	};
	...
	{
		value.val += tag.add * value.len;
	};

	vector<ValueType> a(p.size(), {0, 1});
	...
	for (auto& [c, f] : v)
	{
		...
		for (auto& [x, y, i] : f)
		{
			if (tree.Query(x, y + 1).val > 0)
			{
				res[i] = 0;
				continue;
			}
			{
				int j = tree.FindFirst(x, [&](ValueType val)
					{
						return val.val > 0;
					});
				if (j != -1)
					freshmin(res[i], p[j - 1] - p[y]);
			}
			{
				int j = tree.FindLast(y, [&](ValueType val)
					{
						return val.val > 0;
					});
				if (j != -1)
					freshmin(res[i], p[x] - p[j]);
			}
		}
