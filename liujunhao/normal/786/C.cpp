...
//template
#define MAXN 100000
int a[MAXN+10],ans[MAXN+10],f[400][MAXN+10],la[MAXN+10],lac[MAXN+10],n,d[MAXN+10];
void read(){
	Read(n);
	int i;
	for(i=1;i<=n;i++)
...
void solve(){
	...
	for(i=2;i<=t;i++){
		...
		for(j=1;j<=n;j++){
			...
			while(k+1<j){
				if(max(f[i-1][k],now)<max(f[i-1][k+1],now-d[k+1]))
					break;
				else{
					if(d[k+1]){
						vis[a[k+1]]=0;
