...
if(v.size()>=3){
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<31;j++)
        {
            int pow1=v[i]-exp(2,j,1e18);
            int pow2=v[i]+exp(2,j,1e18);
            int lower=v[lower_bound(v.begin(),v.end(),pow1)-v.begin()];
            int upper=v[lower_bound(v.begin(),v.end(),pow2)-v.begin()];
            if(lower==(pow1)&&upper==(pow2))
            {
                cout<<3<<'\n';
                cout<<v[i]<<" "<<pow1<<" "<<pow2<<'\n';
                return 0;
