...
mint dp[2][3][MAX_INV], imos[3][MAX_INV], imos2[3][MAX_INV];
...
int main() {
    ...
    for (int i = 0; i < n; i++, cur ^= 1) {
        int li = MAX_INV, ri = -1, lii = MAX_INV, rii = -1;
        for (int j = 0; j < 3; j++) {
        ...
        for (int j = 0; j < 3; j++) {
            for (int k = max(0, min(li, lii)); k + 1 < min(MAX_INV, max(ri, rii)); k++) {
                dp[cur ^ 1][j][k] += imos[j][k];
                imos[j][k + 1] += imos[j][k];
        ...
        for (int j = 0; j < 3; j++) {
            for (int k = lii; k + 1 < min(MAX_INV, rii); k++) {
                dp[cur ^ 1][j][k] += imos2[j][k];
                imos2[j][k + 1] += imos2[j][k];
                imos2[j][k] = 0;
            }
        }
        ...
        mint sum = 0;
        for (int k = HALF + 1; k < MAX_INV; k++) sum += dp[cur ^ 1][0][k];
        ans += P[n - 1 - i] * sum;
    }
