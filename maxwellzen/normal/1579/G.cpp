...
void solve() {
    cin >> n;
    read(a, n);
    dp.assign(n, vi(2000, 6969));
    dp[0][a[0]] = 0;
    FOR(i, 0, n-1) {
        int val = a[i+1];
        F0R(j, 2000) if (dp[i][j] < 6969) {
            int a = j, b = dp[i][j];
            if (a>=val) ckmin(dp[i+1][max(a-val, b+val)], min(a-val, b+val));
            else if (b+val<2000) ckmin(dp[i+1][b+val], 0);
            
            if (b>=val) ckmin(dp[i+1][max(a+val, b-val)], min(a+val, b-val));
            else if (a+val<2000) ckmin(dp[i+1][a+val], 0);
        }
    }
    int ans = 1e9;
    F0R(j, 2000) if (dp[n-1][j]<6969) {
        ckmin(ans, j+dp[n-1][j]);
    }
    cout << ans << nl;
}
