...
//            DBN(i, mask, res - INF);
            ...
            int pen = 0;
            for (int x = 0; x <= h; ++x) {
                while (cur < m && r[cur][2] < x) {
                    if (!checkbit(mask, cur) && r[cur][0] <= i) {
                        pen += r[cur][3];
                        mask |= 1LL << cur;
                    }
                    ++cur;
                }
                int newRes = res + x * x - pen;
//                DBN(x, cur, newRes - INF, mask);
                int& w = dp[i + 1][-(mask | ofc[i])];
                w = max(w, newRes);
            }
        }
    }
