...
void build(){
    ...
    facN[0]=1;
    REPP(i,1,SIZE)facN[i]=facN[i-1]*(N-i+1)%MOD;
}
LL dp[SIZE];
int main(){
    R(N,K);
    ...
    LL ans=0;
    FOR(i,1,min(N,K)){
        dp[i]=C(N,i)*mypow(i,K)%MOD;
        LL now=1;
        for(int j=i-1;j>0;j--){
            now=now*inv[i-j]%MOD*fac[i-j-1]%MOD*(N-j)%MOD;
            ADD(dp[i],-dp[j]*now);
        }
        ADD(ans,dp[i]*mypow(2,N-i));
    }
    W(ans);
    return 0;
}