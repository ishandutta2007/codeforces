...
ostream&operator<<(ostream&o,edge e){
  return o<<"(v:"<<e.v<<",i:"<<e.i<<",q:"<<e.q<<",c:"<<e.c<<",f:"<<e.f<<")";
}
...
struct graph{
  ...
  void add(int a,int b,int c,int q){
    edge e1{b,sz(G[b]),q,c,0};
    edge e2{a,sz(G[a]),q,0,0};
    G[a].pb(e1);
    G[b].pb(e2);
  }
...
int main(int argc, char **argv){
  ...
  forv(i,E){
    if(!E[i].Y) continue;
    g1.add(E[i].X.X,E[i].X.Y,INF,i);
    g1.add(E[i].X.X,g1.T,1,-1);
    g1.add(g1.S,E[i].X.Y,1,-1);
  }
  g1.flow();
  forn(v,n){
    for(edge&e:g1.G[v]){
      if(e.q==-1 || e.f<=0) continue;
      E[e.q].Y+=e.f;
    }
  }
  dbg(E);
  ...
  forv(i,E){
    if(E[i].Y){
      g2.add(E[i].X.X,E[i].X.Y,1,i);
      g2.add(E[i].X.Y,E[i].X.X,INF,0);
    }else
      g2.add(E[i].X.X,E[i].X.Y,INF,i);
  }
  dbg(g2.G);
  int r=g2.flow();
  set<int> sat;
  forn(v,n){
    if(!g2.W[v]) continue;
    for(edge&e:g2.G[v]){
      if(!e.c||g2.W[e.v]) continue;
      assert(e.c!=INF && e.f==e.c);
      assert(e.q!=-1);
      assert(!sat.count(e.q));
      sat.insert(e.q);
    }
  }
  assert(sz(sat)==r);
