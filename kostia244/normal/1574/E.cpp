...
int main() {
	...
	int n, m, k;
	cin >> n >> m >> k;
	vector<int> r(n), c(m), rx(n), cx(m);
	set<array<int, 2>> have;
	int c1 = 0, c2 = 0, cnt = 0, rd = n, cd = m;
	int rdie = 0, cdie = 0;
	map<array<int, 2>, int> col;
	while(k--) {
		auto upd = [&](int x, int y, int d, int tp){
			rd -= rx[x] == 0;
			cd -= cx[y] == 0;
			if(abs(r[x]) != rx[x]) rdie--;
			if(abs(c[y]) != cx[y]) cdie--;
		...
		if(d == -1) {
			...
			if(!have.insert({x, y}).second)
				upd(x, y, col[{x, y}], -1);
			upd(x, y, col[{x, y}] = d, 1);
		}
		
		mint A = rdie ? 0 : mint(2).pow(rd);
		mint B = cdie ? 0 : mint(2).pow(cd);
		mint C = !c1 + !c2;
		cout << A+B-C << '\n';
	}
}