...
int main() {
	...
	rep(i,1,n+1) pref[i]=pref[i-1]+d[i];
	int pp=0,cc=0;
	rep(i,0,n) {
		while (pp<n&&pref[pp]-pref[i]<pref[n]/2) pp++;
		if (pp<n&&(pref[pp]-pref[i])*2==pref[n]) cc++;
	}
	mi ans=0;
	rep(i,0,min(cc,m)+1) {
		mi f=m-i;
		ans+=simp::binom(cc,i)*simp::binom(m,i)*simp::gfac(i)*pow(f*(f-1),cc-i)*pow(f,n-2*cc);
	}
	printf("%d",ans.v);
}