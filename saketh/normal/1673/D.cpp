...
void solve() {
    int64_t b, q, y; re(b, q, y);
    int64_t c, r, z; re(c, r, z);

    // first make sure that C is a subset of B
    {
        bool ok = true;
        ...
        // if A unbounded on either end, infinite possibilities
        if (b > c - r || bl < cl + r) {
            ps(-1);
            return;
        }
