...
ll mem[MAXN][4];
#define NOW mem[pos][paired]
ll Dfs( int pos , int paired ){
	if( paired == 4 ) return LINF;
	if( pos == n+1 ) return 0;
	if(~NOW) return NOW;
	char uc = t[paired];
	if( uc != c[pos] ) return NOW = Dfs(pos+1,paired);
	else return NOW = min( Dfs(pos+1,paired+1) , Dfs(pos+1,paired) + a[pos] );
}
