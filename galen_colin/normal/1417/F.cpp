...
void relabel(int v, int compv) {
	if (a[v] != 0) {
		kill.push_back(a[v] * n + v);
		vals[comp[v]].erase(a[v] * n + v);
		comp[v] = compv;
	}
	comp[v] = compv;
	...
	for (int i = 0; i < q; i++) {
		if (qv[i].f == 1) {
			...
			if (targ[i].f != -1) {
				...
				for (ll x: kill) vals[comp_val].insert(vals[comp_val].end(), x);
