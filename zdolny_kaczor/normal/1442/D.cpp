...
void run(int l, int r, int ind) {
	debug(range(in, 0, l), range(in, r, n), range(temp[ind], 0, k + 1));
	if (r - l == 1) {
		ans_ind[l] = ind;
		return;
	}
	int m = (l + r) / 2;
	int h1 = ind, h2 = alloc();
	memcpy(temp[h2], temp[ind], sizeof(ll) * (k + 1));
	auto put = [&](int which, int len, ll su) {
		for (int i = k; i >= len; --i) {
			maxi(temp[which][i], temp[which][i - len] + su);
		}
	};
	for (int i = l; i < m; ++i)
		put(h2, len[i], sum[i]);
...
void solve() {
	...
	for (vi &x : in) {
		...
		scanf("%d", &l);
		x.resize(l);
		for (int &y : x) scanf("%d", &y);
		if ((int)x.size() > k) x.resize(k);
	}
	for (int i = 0; i < n; ++i) {
		len[i] = in[i].size();
		for (int x : in[i]) sum[i] += x;
	}
	debug(range(len, 0, n), range(sum, 0, n));
	run(0, n, alloc());
	for (int i = 0; i < n; ++i) debug(arr(in, i), range(temp[ans_ind[i]], 0, k + 1));
	ll ans = 0;
	for (int i = 0; i < n; ++i) {
		ll here = 0;
		for (int j = 0; j <= k && j <= (int) in[i].size(); ++j) {
			if (j) here += in[i][j - 1];
			maxi(ans, here + temp[ans_ind[i]][k - j]);
		}
	}
	printf("%lld\n", ans);
	debug(imie(first_free));
}
int main() {
