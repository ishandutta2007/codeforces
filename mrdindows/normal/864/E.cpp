...
int32_t solve() {
    ...
    sort(items, items + n, [](const Item& i, const Item& j){
        return i.d < j.d;
    });
    int ans = 0;
    int cur = 0;
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = dead - 1; j >= time; --j) {
            dp[i + 1][j] = max(dp[i][j], dp[i][j - time] + cost);
        }
        for (int j = 0; j <= 2000; ++j) {
            if (dp[i + 1][j] > ans) {
                ans = dp[i + 1][j];
    ...
    vector<int> v;
    for (int i = n; i > 0; --i) {
        int dead = items[i - 1].d;
        int cost = items[i - 1].p;
        int time = items[i - 1].t;
        if (cur >= time && dead > cur && dp[i - 1][cur - time] == ans - cost) {
            v.push_back(items[i - 1].idx);
            cur -= time;
            ans -= cost;
        }
    }
