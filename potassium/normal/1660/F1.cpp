...
int main() {
	CASE(t) {
		...
		const static int QUERY = 0;
		const static int INSERT = 1;
		vector<vector<pii>> queries(3);
		queries[0].push_back({ INSERT, 0 });

		int ps = 0;
		for (int i = 0; i < n; i++) {
			ps += s[i] == '+' ? 1 : -1;
			queries[(ps % 3 + 3) % 3].push_back({ QUERY, ps });
			queries[(ps % 3 + 3) % 3].push_back({ INSERT, ps });
		}

		ll ans = 0;
		function<void(vector<pii>&)> CDQ = [&](vector<pii>& query) {
			const int len = (int)query.size();
			if (len <= 1) return;
			const int mid = len / 2;

			vector<int> val;
			for (int i = 0; i < mid; i++) {
				if (query[i].first == INSERT)
					val.emplace_back(query[i].second);
			}
			sort(val.begin(), val.end());
			for (int i = mid; i < len; i++) {
				if (query[i].first == QUERY)
					ans += val.end() - lower_bound(val.begin(), val.end(), query[i].second);
			}
			vector<pii> L(query.begin(), query.begin() + mid);
			vector<pii> R(query.begin() + mid, query.end());
			CDQ(L);
			CDQ(R);
		};
		for (int i = 0; i < 3; i++)
