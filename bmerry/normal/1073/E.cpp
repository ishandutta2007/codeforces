...
static mr uniq(ll R, int k)
{
    vector<ll> p10(19);
    p10[0] = 1;
    for (int i = 1; i <= 18; i++)
        p10[i] = p10[i - 1] * 10;
    array<array<pair<mr, mr>, 1024>, 2> dp = {};
    dp[0][0] = pair<mr, mr>(0, 1);
    mr ans = 0;
    for (int d = 0; d < 18; d++)
    {
        array<array<pair<mr, mr>, 1024>, 2> dp2 = {};
        int r = (R / p10[d]) % 10;
        for (int v = 9; v >= 0; v--)
        {
            for (int a = 0; a < 2; a++)
                ...
                {
                    auto &out = dp2[v > r || (v == r && a)][m | (1 << v)];
                    out.first += dp[a][m].first + dp[a][m].second * (p10[d] * v);
                    out.second += dp[a][m].second;
                }
            if (v == 1)
            {
                for (int m = 0; m < 1024; m++)
