...
int n;
int a[1010101];
vector<int> poses[200020];
vector<int> divisors[200020];
ll tr[602020];
int pu[602020];
int minval[602020];
int maxval[602020];
void build(int cur, int l, int r) {
    if (l == r) {
        ...
        minval[cur] = min(minval[dcur], minval[dcur + 1]);
        maxval[cur] = max(maxval[dcur], maxval[dcur + 1]);
    }
}
void draw(int cur, int l, int r, int x, int y, int val) {
    if (maxval[cur] <= val) return;
    if (l == x && r == y && minval[cur] >= val) {
//        DBN(cur, l, r, val);
        pu[cur] = val;
        tr[cur] = (r - l + 1) * (ll) pu[cur];
        minval[cur] = val;
        maxval[cur] = val;
    } else {
        int m = (l + r) / 2;
        ...
        if (pu[cur] != -1) {
            ...
            pu[dcur + 1] = pu[cur];
            minval[dcur] = pu[cur];
            minval[dcur + 1] = pu[cur];
            maxval[dcur] = pu[cur];
            maxval[dcur + 1] = pu[cur];
            tr[dcur] = (m - l + 1) * (ll) pu[cur];
            tr[dcur + 1] = (r - m) * (ll) pu[cur];
            pu[cur] = -1;
        }
        if (y <= m) {
            ...
            draw(dcur, l, m, x, m, val);
            draw(dcur + 1, m + 1, r, m + 1, y, val);
        }
        tr[cur] = tr[dcur] + tr[dcur + 1];
        minval[cur] = min(minval[dcur], minval[dcur + 1]);
        maxval[cur] = max(maxval[dcur], maxval[dcur + 1]);
...
//    DBN(cur);
    for (int i = 200000; i >= 1; --i) {
        if (poses[i].size() < 2) continue;
        int first = poses[i].front();
        int second = poses[i][1];
        int last = poses[i].back();
        int prevlast = poses[i][poses[i].size() - 2];
        draw(last, n, second);
        draw(prevlast, last - 1, first);
        draw(0, prevlast - 1, 0);
        ll newcur = tr[1];
        ll dif = cur - newcur;
        ans += dif * i;
//        DBN(i, cur, newcur, dif);
        cur = newcur;
    }
    cout << ans << endl;
//    DBN(naive());
