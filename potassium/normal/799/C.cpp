...
int main() {
	...
	vector<pii> cl, dl;
	for (int i = 0; i < n; i++) {
		int b,p; char tp; R(b,p,tp);
		(tp == 'C' ? cl : dl).push_back({ p, b });
	}

	int ans = 0;
	auto solve = [&](vector<int>& h, vector<pii>& l, int maxv) {
		sort(l.begin(), l.end(), [&](pii x, pii y) {
			return x.first > y.first;
		});

		for (int i = 0; i < (int)l.size(); i++) {
			if (l[i].first > maxv) continue;

			auto it = partition_point(h.begin(), h.end(), [&](int x) {
				return l[x].first + l[i].first > maxv;
			});
			if (it != h.end())
				chkmax(ans, l[*it].second + l[i].second);

			while (!h.empty() && l[h.back()].second <= l[i].second)
				h.pop_back();
			if (h.empty() || l[h.back()].first > l[i].first)
				h.push_back(i);
		}
	...
	solve(ch, cl, c);
	solve(dh, dl, d);
	if (!ch.empty() && !dh.empty())
		chkmax(ans, cl[ch.front()].second + dl[dh.front()].second);
