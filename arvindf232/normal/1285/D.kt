...
fun main(){
    solve.cases{
        val n = getint()
        val L = getline(n)
        val trie = TrieImplicit(31 * n)
        for(a in L){
            trie.addString(IntArray(30){(a and ( 1 shl (29 - it)) != 0).chi })
        }
        fun TrieImplicit.dfs(level:Int,v:Int):Int {
            if(level == 0) return 0
            val ret = IntArray(2){-1}
            for(c in 0 until 2){
                val w = next[v][c]
                if(w != -1){
                    ret[c] = this.dfs(level-1,w)
                }
            }
            val a =  if(ret[0] == -1 && ret[1] == -1){
                0
            }else if(ret[0] != -1 && ret[1] != -1){
                (1 shl (level - 1)) + minOf(ret[0],ret[1])
            }else{
                if(ret[0] != -1) ret[0] else ret[1]
            }
//            just dei Triple(level,v,a)
            return a
        }
        val min = trie.dfs(30,0)
        put(min)
    }
    done()
}
...
/*
huge mistake on return not equal
 */