...
void solve(){
    ...
    for (int q = 0; q < inputs.size(); ++q) {
        int S = back - inputs[q].first;
        ans[inputs[q].second] = S;
        back = S;
    }
    if (back > 1) {
        cout << -1 << endl;
    ...
    reverse(ALL(inputs));
    for (int q = 0; q < inputs.size(); ++q) {
        ans[inputs[q].second] = max(ans[inputs[q].second], q + 1);
        int limits = n - inputs[q].first + 1;
        if (limits < ans[inputs[q].second]) {
            cout << -1 << endl;
            return;
        }
    }
