...
const int hashtable_mod=5000011; // 300007 500009 800011 1000003 3000017 5000011 8000009
const int hashtable_maxn=1111111;
...
int query(int x){
	...
	rep(i,1,btot+1){
		int v=biglist[i];
		if(col[biglist[i]] && has[(1ll*min(x,v))<<32|max(x,v)])ret++;
	}
	return ret+smalans[x];
}
...
int main()
{
	...
	/* rep(i,1,n+1){ */
	/* 	foreach(it,g[i]){ */
	/* 		if(col[*it] && !isbig[*it])smalans[i]++; */
	/* 	} */
	/* } */

	rep(i,1,q+1){
		char opt;gc(opt);
		if(opt=='C' || opt=='O' || opt=='F'){
			int u;gn(u);
			ques[i]=(quer){opt,u,0};
		}else{
			int a,b;gn(a);gn(b);
	...
	rep(u,1,n+1){
		rep(i,0,siz(g[u])){
			if(col[g[u][i].fi] && !isbig[g[u][i].fi])smalans[u]++;
		}
	}
	rep(i,1,q+1){ 
		char opt=ques[i].opt;
		int u=ques[i].a,v=ques[i].b;
		if(opt=='C'){
			printf("%d\n",query(u));
		}else if(opt=='O'){
			col[u]=1;
			if(!isbig[u]){
				rep(i,0,siz(g[u]))smalans[g[u][i].fi]+=g[u][i].se;
			}
		}else if(opt=='F'){
			col[u]=0;
			if(!isbig[u]){
			...
			g[v].pb(mp(u,1));
			has[(1ll*u)<<32|v]=1;
			if(col[u] && !isbig[u])smalans[v]++;
			if(col[v] && !isbig[v])smalans[u]++;
		}else if(opt=='D'){
			g[u].pb(mp(v,-1));
			g[v].pb(mp(u,-1));
			has[(1ll*u)<<32|v]=0;
