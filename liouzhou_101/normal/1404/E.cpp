...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
		for (int j = 1; j <= m; ++j)
		{
			...
			{
				res += 1;
				if (s[i + 1][j] == '#') cnt++, dinic::add_edge(S, X(i, j), 1);
				if (s[i][j + 1] == '#') cnt++, dinic::add_edge(Y(i, j), T, 1);
			}
		}
	for (int i = 1; i <= n; ++i)
		for (int j = 1; j <= m; ++j)
		{
			if (s[i][j] == '#')
			{
				...
				{
					if (s[i][j - 1] == '#')
						dinic::add_edge(X(i - 1, j), Y(i, j - 1), 1);
					if (s[i][j + 1] == '#')
						dinic::add_edge(X(i - 1, j), Y(i, j), 1);
				}
				if (s[i + 1][j] == '#')
				{
					if (s[i][j - 1] == '#')
						dinic::add_edge(X(i, j), Y(i, j - 1), 1);
					if (s[i][j + 1] == '#')
						dinic::add_edge(X(i, j), Y(i, j), 1);
				}
			}
		}
