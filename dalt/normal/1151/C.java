...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            while (helper.rId < r) {
                helper.next();
                sum = mod(sum + helper.intersect(l, r));
            }
            io.cache.append(sum);
        }

        public int f(long a, long b) {
            return mod((long) mod((a + b) / 2) * mod(((b - a) / 2 + 1)));
        }

        public class Helper {
            long[] ls = new long[]{-1, 0};
            int turn = -1;
            long l;
            long r;
            long lId;
            ...
            public int intersect(long b, long e) {
                if (b > rId || e < lId) {
                    return 0;
                }
                long f = Math.max(b, lId);
                long t = Math.min(rId, e);
                return f((f - lId) * 2 + l, (t - lId) * 2 + l);
            }
        }
