...
cin>>n;
for(int i=0,j=n-1;i<n;i++,j--){cin>>a[i];b[j]=a[i];}
int j=0,i=0;
while(1)
{
    if(a[i]==b[j]&&a[i]>mx)
    {
        vector<int> tmp1,tmp2;int si1,si2,tmpx=mx;
        for(int z=i;z<n;z++){if(a[z]>tmpx)tmp1.push_back(a[z]),tmpx=a[z];else break;}tmpx=mx;si1=tmp1.size();
        for(int z=j;z<n;z++){if(b[z]>tmpx)tmp2.push_back(b[z]),tmpx=b[z];else break;}si2=tmp2.size();
        if(si1>=si2)for(int z=0;z<si1;z++)khra.push_back('L');
        else for(int z=0;z<si2;z++)khra.push_back('R');
        cout<<khra.size()<<endl;
for(int i=0;i<khra.size();i++)cout<<khra[i];
return 0;
    }
    else if(a[i]<b[j]&&a[i]>mx)khra.push_back('L'),i++,mx=a[i-1];
    else if(b[j]>mx) khra.push_back('R'),j++,mx=b[j-1];
    else if(a[i]>mx)khra.push_back('L'),i++,mx=a[i-1];else break;
    if(i==n||j==n)break;
    //cout<<a[i]<<" "<<b[j]<<" "<<mx<<endl;
}
cout<<khra.size()<<endl;
for(int i=0;i<khra.size();i++)cout<<khra[i];
