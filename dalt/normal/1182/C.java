...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Deque<String[]> pairs = new ArrayDeque<>();
            for (int i = 0; i <= limit; i++) {
                ...
                int merge = cnt + vowel * limit;
                if (!map.containsKey(merge)) {
                    map.put(merge, s);
                } else {
                    pairs.add(new String[]{s, map.remove(merge)});
                }
            }

            Map<Integer, List<String>> equalLengthMap = new HashMap<>();
            for (Map.Entry<Integer, String> entry : map.entrySet()) {
                int key = entry.getKey() % limit;
                if (!equalLengthMap.containsKey(key)) {
                    equalLengthMap.put(key, new ArrayList<>(1));
                }
                equalLengthMap.get(key).add(entry.getValue());
            }

            List<String[]> ans = new ArrayList<>();
            for (List<String> list : equalLengthMap.values()) {
                for (int i = 1, until = list.size(); i < until && !pairs.isEmpty(); i += 2) {
                    String[] head = pairs.removeFirst();
                    ans.add(new String[]{list.get(i), head[0], list.get(i - 1), head[1]});
                }
            }
            while (pairs.size() >= 2) {
                String[] head = pairs.removeFirst();
                String[] tail = pairs.removeLast();
                ans.add(new String[]{head[0], tail[0], head[1], tail[1]});
            }

            io.cache.append(ans.size()).append('\n');
            for (String[] a : ans) {
                io.cache.append(a[0]).append(' ').append(a[1])
                        .append('\n').append(a[2]).append(' ').append(a[3]).append('\n');
            }
        }
    }
