...
bool insamerow(int a, int b){
    /* return a / nnn == b / nnn; */
    /* return a / m == b / m; */
    return a / origm == b / origm;
}

/* int rowcnt[N], aicnt[N]; */
...
inline void merge_sort(pair<int,int> *beg, pair<int,int> *end){
    ...
    map<int,int> rowcnt;
    /* map<int,int> aicnt; */
    map<int, map<int,int>> rowaicnt;
    for(auto it = it1; it < end1; ++it){
        ++rowcnt[it->e2 / origm];
        /* ++aicnt[it->e1]; */
        ++rowaicnt[it->e1][it->e2 / origm];
    }
    while(beg < end){
        if(it1->e2 <= it2->e2){ //Sortowanie wedle docelowej pozycji
            --rowcnt[it1->e2 / origm];
            /* --aicnt[it1->e1]; */
            --rowaicnt[it1->e1][it1->e2 / origm];
            *beg++ = *it1;
            if(++it1 == end1){
                while(beg < end)
                    *beg++ = *it2++;
            ...
            *beg++ = *it2;
            ...
            ans += rowcnt[it2->e2 / origm];
            ans -= rowaicnt[it2->e1][it2->e2 / origm];
            /* ans -= rowcnt[it2->e2 / origm]; */
            /* ans -= aicnt[it2->e1]; */
            /* ans += rowaicnt[it2->e1][it2->e2 / origm]; */
            /* ans += end1-it1; //Zliczanie zamian */
            ...
            /*     /1* whatis(*it) *1/ */
            /*     /1* whatis(*it2) *1/ */
            /*     /1* whatis(insamerow(it->e2, it2->e2)); *1/ */
            /*     if(it->e1 != it2->e1) */
            /*         ans += insamerow(it->e2, it2->e2); */
            /* } */
            if(++it2 == end2){
                while(beg < end){
                    *beg++ = *it1++;
...
int main(){
    ...
    while(t--){
        ...
        FOR(i,0,m){
            ...
            arr[i].e2  = sth + (totcnt[a[i]] - cnt[a[i]]++ - 1);
        }
