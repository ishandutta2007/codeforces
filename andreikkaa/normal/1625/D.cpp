...
node *new_t() {
    mem[ptr].g[0] = mem[ptr].g[1] = nullptr;
    return mem + ptr++;
}

vector<int> ans;
void f(vector<pair<int, int>> a, vector<pair<int, int>> b, int k) {
    if(a.empty()) {
        assert(sz(b));
        ans.push_back(b[0].y);
        return;
    ...
    for(auto i : b) {
        auto cur = rt;
        for(int j = L - 1; j >= 0; --j) {
            if(cur->g[i.x >> j & 1] == nullptr) {
                cur->g[i.x >> j & 1] = new_t();
                cur->g[i.x >> j & 1]->id = i.y;
            }
            cur = cur->g[i.x >> j & 1];
        }
    }
    ...
    for(auto i : a) {
        ...
        for(int j = L - 1; j >= 0; --j) {
            if(k >> j & 1) {
                ...
                if(cur->g[~i.x >> j & 1] != nullptr) {
                    ans.push_back(i.y);
                    ans.push_back(cur->g[~i.x >> j & 1]->id);
                    return;
                }
                if(cur->g[i.x >> j & 1] == nullptr) {
                    assert(false);
...
void _solve() {
    ...
    for(int ii = 0; ii < n; ++ii) {
        ...
        int old = (i & ~((1 << (bt + 1)) - 1));
        int cur = (i >> bt & 1);
        mp[cur][old].push_back({i & ((1 << bt) - 1), ii + 1});
        olds.insert(old);
    }

    for(int i : olds) {
        f(mp[0][i], mp[1][i], k ^ (1 << bt));
    }
