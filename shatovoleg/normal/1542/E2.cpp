...
int TheSmartestFunctionEver(const vector<int>& v, int n) {
    ll segment_sum = 0, ladder_sum = 0, convex_sum = 0, res = 0;
    for (int lower = 0; lower + 1 < len(v); ++lower) {
        ladder_sum -= segment_sum;
        convex_sum += ladder_sum;
        convex_sum %= mod;
        res += convex_sum * v[lower + 1];
        res %= mod;

        segment_sum += v[lower];
        if (lower >= n) {
            segment_sum -= v[lower - n];
        }
        ladder_sum += 1ll * v[lower] * n;
    }
    return (int)res;
}
