...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/248912/problem/B */
/*input
4 4 2
1 1 2 2
1 1 2 2
...
int main () {
	...
	for (int i = 0; i <= n; i++) {
		for (int j = 0; j <= m; j++) {
			if (i > 0)
				lent[i - 1][j].zem = &lent[i][j];

			if (j > 0)
				lent[i][j - 1].des = &lent[i][j];
		}
	}

	auto eikite = [&](lang * a, int x, int y) -> lang* {
		for (int i = 0; i < x; i++) {
			a = a->zem;
		}
	...
	for (int t = 0; t < q; t++) {
		int a, b, c, d, h, w;
		cin >> a >> b >> c >> d >> h >> w;
		lang *pirm = eikite(&lent[0][0], a - 1, b - 1);
		lang *antr = eikite(&lent[0][0], c - 1, d - 1);
		{
			lang *x = pirm;
			lang *y = antr;
