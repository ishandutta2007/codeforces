...
{
    int t=0,ans=0;
    for (int i=1;i<=n;i++) if (a[i]==x) b[++t]=i;
    for (int i=1;i<=(n+1)/2;i++) ans+=abs(i*2-1-b[i]);
    return ans;
}
void DOIT()
{
    rd(n);int tot=0;
    for (int i=1;i<=n;i++) rd(a[i]),a[i]%=2,tot+=a[i];
    if (n%2==1)
    {
        if (tot==n/2) cout<<chk(0)<<"\n";else 
        if (tot==n/2+1) cout<<chk(1)<<"\n";else {puts("-1");return;}
    } else 
    {
        if (tot==n/2) cout<<min(chk(0),chk(1))<<"\n";
        else puts("-1");
    }
