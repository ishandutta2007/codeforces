...
//*************************************************************************************
pii p[2][2] = {{{2, 2}, {2, 2}}, {{3, 4}, {3, 5}}};
int ask(int x1, int y1, int x2, int y2) {
//    int r = 0;
//    for (int i = 0; i < 2; ++i) {
//        if (x1 <= p[i][0].first && x2 >= p[i][1].first && y1 <= p[i][0].second && y2 >= p[i][1].second) {
//            ++r;
//        }
//    }
//    return r;
...
bool check(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4) {
    ...
    if (x3 > x4) return 0;
    if (y3 > y4) return 0;
    ll inter = max(0, min(x2, x4) - max(x1, x3) + 1) * (ll) max(0, min(y2, y4) - max(y1, y3) + 1);
    if (inter > 0) return 0;
    return ask(x1, y1, x2, y2) == 1 && ask(x3, y3, x4, y4) == 1;
}
