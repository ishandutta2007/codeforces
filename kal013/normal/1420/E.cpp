...
int state(int idx, int k, int moves){
	assert(idx >= 0 && k >= 0 && moves >= 0);
	assert(k <= pos.size());
	if (vis[idx][k][moves]){
		return dp[idx][k][moves];
	}
	vis[idx][k][moves] = true;
	if (k > idx){
		dp[idx][k][moves] = -INF;
		return -INF;
	}
	...
	int ans = -INF;
	int position = pos[k - 1];
	for(int j = max(k, position - moves); j <= idx && j <= position + moves ; ++j){
		
		ans = max(ans, state(j - 1, k - 1, moves - abs(position - j)) + (j - k) * (idx - j));
	}
