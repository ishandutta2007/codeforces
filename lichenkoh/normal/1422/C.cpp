...
int main() 
{
  ...
  for (int e=0;e<n;e++) {
    ...
    if (e>0) {
      mint lower=b[e-1]*c;
      //printf("e:%d c:%d lower:%d\n",e,c,lower.get());
      final+=lower;
    }
    mint upper=n-e-1;
    mint upperCnt=(upper*(upper+1))/2;
    final+=upperCnt*c*tenpow;
    //printf("e:%d c:%d upperCnt:%d tenpow:%d\n",e,c,upperCnt.get(),tenpow.get());
    tenpow*=10;
  }
  printf("%d\n",final.get());
}
