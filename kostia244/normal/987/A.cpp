...
//SQRT DECOMP. IS POBABLY THE ANSWER
...
int main() {
	...
	string s;
	cin >> n;
	while(n--) cin >> s, h.insert(s);
	if(h.find("red") == h.end())
		a.pb("Reality");
	if(h.find("yellow") == h.end())
		a.pb("Mind");
	if(h.find("orange") == h.end())
		a.pb("Soul");
	if(h.find("blue") == h.end())
		a.pb("Space");
	if(h.find("green") == h.end())
		a.pb("Time");
	if(h.find("purple") == h.end())
		a.pb("Power");
	cout  << a.size() << "\n";for(auto i : a) cout << i << "\n";
