...
void solve() {
  LL(N, M, D);
  auto ID = [&](int v, int d) -> int { return D * v + d; };
  Graph<int, 1> G(N);
  G.read_graph(M);
  ...
  for (auto&& e: G.edges) {
    ...
    FOR(d, D) {
      int d1 = d == D - 1 ? 0 : d + 1;
      uf.merge(ID(e.frm, d), ID(e.to, d1));
    }
  }
  ...
  for (auto&& e: G.edges) {
    ...
    FOR(d, D) {
      int d1 = d == D - 1 ? 0 : d + 1;
      DAG.add(uf[ID(e.frm, d)], uf[ID(e.to, d1)]);
    }
  }
  DAG.build();
  ...
  FOR(v, N) {
    ...
    UNIQUE(add_to);
    for (auto&& c: add_to) dp0[c]++;
  }
  vc<int> dp(N * D);
  auto V = toposort(DAG);
  reverse(all(V));
  for (auto&& v: V) {
    for (auto&& e: DAG[v]) { chmax(dp[v], dp[e.to]); }
    dp[v] += dp0[v];
  }
