// time-limit: 2000
// problem-url: https://codeforces.com/contest/1693/problem/E


// forgot this point. Hopefully it's the only bug
...
struct SegmentTree: SegmentTreeTemplate<Node, SegmentTree>{
	...
	int get(int left, int right){
		int const identityValue=0;
		if(left==right) return identityValue;
		assert(left<right);
		left+=offset(); right+=offset();
		pushAll(left); pushAll(right-1);
		return reduceRange(left, right, identityValue, [&](int result, Node const& node){ ... });
	}
...
struct Node{
	std::array<std::array<int, 2>, 2> numMore; // hard to explain...
	...
	void recomputeLeaf(){ // also hard to explain
		numMore[0][1]=have(0) ? 1: infinity;
		numMore[1][0]=have(1) ? 1: infinity;
	}
};
struct Data: SegmentTreeTemplate<Node, Data>{
	using SegmentTreeTemplate::SegmentTreeTemplate;
	...
	void set(int index, int side, int value){
		...
		assert(data[index].have(side)!=value); // that the set is not no-op
...
int main(){
	...
	Data data(number+1); // [0]: for left,[1]: for right etc.
	data.add(0, 0);
	data.add(0, 1);

	std::vector<int> countLeft(number+1), countRight(number+1);
	countLeft[0]=countRight[0]=1; // never nonzero
	...
	for(auto it: a){
		if(0==--countRight[it]){
			data.remove(it, 1); // for right
		}
		result+=data.compute(it);
		if(countLeft[it]++==0){
			data.add(it, 0); // for left
		}
	}
