...
fun main(){
    solve.cases{
        ...
        for(i in 0 until m){
            val v = need[i]
            has[v] = true
            last[v] = if(i == 0) -1 else need[i-1]
            next[v] = if(i == m-1) -1 else need[i+1]
        }

        val bestpotentials = LongArray(n){phuge }

        val sums = longSumSimple(n)
        for((i,a) in L.withIndex()){
            val p = price[i]
            if(p > 0){
                sums.addValue(a,p.toLong())
            }

            if(has[a]){
                val pricehere = if(last[a] == -1) totalprice.sumQuery(0,i-1)
                else
                    bestpotentials[last[a]] + totalprice.sumQuery(0,i-1) - sums.query(0,last[a])
                val potentialhere = totalprice.sumQuery(0,i) - sums.query(0,a)
                bestpotentials[a] = minOf(bestpotentials[a] , pricehere - potentialhere)
            }

        }
        val ret = bestpotentials[need.last()] + totalprice.sumQuery(0, n) - sums.query(0, need.last())
        if(ret <= (phuge/ 2)){
            put("YES")
            put(ret)
        }else{
