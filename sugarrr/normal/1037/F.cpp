...
mint calc(ll l,ll p,ll r,ll v,ll k){
    mint sum=kaz(r-l-1,k)-kaz(p-l-1,k)-kaz(r-p-1,k);
    return sum*v;
}
...
signed main(){fastio
    ...
    rep(i,0,n-1){
        ll pos=a[i].se;
        auto itr=se.lower_bound(pos);
        ll R=*itr;itr--;
        ll L=*itr;
        assert(L<pos&&pos<R);
        se.insert(pos);
        
        mint ad=calc(L,pos,R,a[i].fi,k);
     //   cout<<a[i].fi<<" "<<ad.val()<<endl;
        ans+=ad;
    }
    cout<<ans.val()<<endl;
