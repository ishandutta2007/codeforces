...
struct Minimumas {
	...
	explicit Minimumas(vector<long long> &d): mas(d) {}
	Minimumas() {}
...
struct Maximumas {
	...
	vector<long long> laips;
	vector<vector<long long>> din;
	explicit Maximumas(vector<long long> &d): mas(d) {}
	Maximumas() {}
	long long &operator[](int i) {
		return mas[i];
	}
...
int main () {
	...
	while (fr < to) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (i > 0) {
				mini(mi, minim.get(0, i - 1));
				maxi(ma, maxim.get(0, i - 1));
			}

			if (p < n) {
				mini(mi, minim.get(p, n - 1));
				maxi(ma, maxim.get(p, n - 1));
			}
