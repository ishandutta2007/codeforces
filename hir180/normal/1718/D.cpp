...
void solve(){
	...
	vc<int>s(n);
	rep(i, n) cin >> s[i];
	segtree<decltype(f), decltype(e)>seg(n+1, f, e);
	rep(i, n) seg.update(i, mp(p[i], i), true);
	segtree<decltype(f), decltype(e)>seg2(n+1, f, e);
	rep(i, n) seg2.update(i, mp(s[i], i), true);
	
	vc<P>A;
	bool fatal = 0;
	auto rec = [&](int l, int r, int zan, auto rec) -> void{
		if(l > r) return;
		auto p = seg.query(l, r);
		int pos = p.b;
		auto qq = seg2.query(l, r);
		if(s[pos] == 0){
			//max
			A.eb(qq.a, zan);
			rec(l, pos-1, zan, rec);
	...
	vc<int>cand;
	rep(i, sz-1) {
		int x;cin>>x; cand.pb(x);
	}
	vc<P>Q;
	rep(i, q){
		int x; cin>>x; Q.eb(x, i);
	}
	if(fatal){
		rep(i, q) o("NO");
		return;
	...
	rep(i, cand.size()){
		...
		while(idx < sz and A[idx].a < num) {
			que.push(mp(A[idx].b, idx));
			idx ++;
		}
		while(!que.empty() and que.top().a <= num) que.pop();
		if(que.empty()){
			fatal = true;
			break;
		}
	...
	P rem;
	rep(i, sz) if(!used[i]) rem = A[i];
	SORT(Q);
	reverse(all(Q));
	int nxt = 0;
	vc<int>ans(q, 0);
	for(int i=cand.size()-1;i>=-1;i--){
		while(nxt < Q.size() and (i == -1 or cand[i] < Q[nxt].a)){
			if(rem.a < Q[nxt].a and Q[nxt].a < rem.b){
				ans[Q[nxt].b] = 1;
			}
			nxt++;
		}
		if(i == -1) break;
		P nxt_c = A[who[i]];
		assert(nxt_c.a < cand[i] and cand[i] < nxt_c.b);
		if(!(rem.a < cand[i] and cand[i] < rem.b)) continue;
		if(rem.a < nxt_c.a) continue;
		rem = nxt_c;
	}
	rep(i, q) o(ans[i]?"YES":"NO");
}
signed main(){
	cin.tie(0);
