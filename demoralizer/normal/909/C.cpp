...
mod_int dp[N][N];
// dp[x][y] = xth index and level is y, number of ways

// if(inp[x - 1] == 'f') dp[x][y] = dp[x-1][y-1]
// if(inp[x - 1] == 's') dp[x][y] = dp[x-1][y] + dp[x-1][y+1] + dp[x-1][y+2] ...... + dp[x-1][n]

// ans = summation of dp[n-1][x] for all x
...
void solve(){
	...
	rep(i,1,n){
		if(s[i-1] == 'f'){
			...
			vector<mod_int> suff(n);
			suff[n-1] = dp[i-1][n-1];
			for(int j = n - 2; j >= 0; j--){
				suff[j] = dp[i-1][j] + suff[j+1];
			}
			rep(j,0,n){
				dp[i][j] = suff[j]; // suff[x] = suffix sum x , x + 1, x + 2, ... n
			}
		}
	}
