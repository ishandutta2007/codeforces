...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        for(int i=1; i<=n; i++)
        {
            st[i].resize(i*(i-1)/2+1);
        }
        ans[1] = 0;
        st[1][0]=1;
        for(int i=2; i<=n; i++)
        {
            ans[i] = (1LL* i * ans[i-1]) % m;
            int sm = 0;
            for(int k=0; k<=i*(i-1)/2; k++)
            {
                if(k <= (i-1)*(i-2)/2) sm = (sm + st[i-1][k]) % m;
                if(k >= i) sm = (sm + m - st[i-1][k-i]) % m;
            ...
            int sm1 = 0;
            int sm2 = 0;
            for(int k=(i-1)*(i-2)/2; k>=0; k--)
            {
                if(k+2 <= (i-1)*(i-2)/2) sm1 = (sm1 + st[i-1][k+2]) % m;
                if(k+i+1 <= (i-1)*(i-2)/2) sm1 = (sm1 + m - st[i-1][k+i+1]) % m;

                if(k+2 <= (i-1)*(i-2)/2) sm2 = (sm2 + (1LL * (k+2) * st[i-1][k+2]) % m) % m;
                if(k+i+1 <= (i-1)*(i-2)/2) sm2 = (sm2 + m - (1LL * (k+i+1) * st[i-1][k+i+1]) % m) % m;

                sm = (sm + ((1LL * (i+1+k) * sm1) % m + m - sm2) % m) % m;
                
                ans[i] = (ans[i] + (1LL * st[i-1][k] * sm) % m) %m;
            }
        }
        cout<<ans[n];
