...
public:
    void solve() {
        ...
        int answer = 0;
        for (int i : range(cntS + 1)) {
            if (ll(i) * s > p) {
                break;
            }
            int rem = p - i * s;
            int tFr = min(cntS - i, f / s);
            int remFr = f - tFr * s;
            int axYou = rem / w;
            int axFr = remFr / w;
            int candidate = i + tFr + min(axYou + axFr, cntW);
            maxim(answer, candidate);
        }
