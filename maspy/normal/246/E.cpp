...
void solve() {
  ...
  VEC(pi, query, Q);
  vc<vi> query_at(N + 1);
  FOR(q, Q) query_at[query[q].fi].eb(q);

  vi ANS(Q);
  EulerTour<Graph<int>> ET(G);
  auto calc = [&](auto dfs, ll v, ll p) -> deque<set<string>> {
    ll sz = 0;
    deque<set<string>> ss;
    FORIN(e, G[v]) {
      ll to = e.to;
      if (to == p) continue;
      auto newss = dfs(dfs, to, v);
      if (sz < ET.RID[to] - ET.LID[to]) swap(ss, newss);
      sz += ET.RID[to] - ET.LID[to];

      FOR(i, newss.size()) {
        if (i >= ss.size())
          ss.push_back(newss[i]);
        else {
          FORIN(x, newss[i]) ss[i].insert(x);
        }
      }
    }
    set<string> s;
