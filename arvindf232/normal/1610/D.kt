...
fun main(){
    solve.cases{
        ...
        val levels = IntArray(35)

        for(c in arr){
            levels[c.countTrailingZeroBits()] ++
        }
        var ret = 0
        for(l in 1 until 35){
            val here = levels[l]
            if(here == 0) continue
            val rest = ((l+1)..34).sumOf { levels[it] }
            val new = (intPow(2,here-1,pI)) modM (intPow(2,rest,pI))
            ret = ret modPlus new
        }
        put((intPow(2,n,pI) modMinus 1) modMinus ret)
    }
    done()
}