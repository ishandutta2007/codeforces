...
namespace run{
	Z fac[1000005],inv[1000005],invf[1000005];
	int a[1000005];
	int cnt[1000005],s[1000005];
	int n,m,hv[1000005];
	int b[1000005];
	Z solve(int x){
		for(int i=1;i<=m;++i){
		...
		Z c=0;
		Z w(1);
		for(int i=1;i<=m;++i)w*=invf[b[i]];
		for(int i=1;i<=m;++i)if(b[i]>=2){
			ans+=w*b[i]*(b[i]-1)*fac[x-2];
		}
		return ans*n;
	}
	Z solve2(int x){
	...
	bool main(){
		...
		if(g==n)return write(1,'\n');
		for(int i=1;i<=n;++i)s[i]=0;
		for(int i=1;i<=n;++i)++s[__gcd(i,n)];
		Z a2=0;
		for(int i=1;i<=n;++i){
			if(s[i] && !(g%(n/i))){
				ans+=s[i]*solve(i);
