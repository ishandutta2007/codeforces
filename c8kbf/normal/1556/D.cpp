...
inline void AC() {
    ...
    n = read();
    m = read();
    b.push_back(qr("and", 1, 2));
    b.push_back(qr("and", 1, 3));
    b.push_back(qr("and", 2, 3));
    b.push_back(qr("or", 1, 2));
    b.push_back(qr("or", 1, 3));
    b.push_back(qr("or", 2, 3));
    for(_ d = 1; d <= 1E9; d <<= 1) for(_ i = 0; i <= 1; ++i) for(_ j = 0; j <= 1; ++j) for(_ k = 0; k <= 1; ++k) {
        if((i&j) == !!(b[0]&d) && (i&k) == !!(b[1]&d) && (j&k) == !!(b[2]&d) && (i|j) == !!(b[3]&d) && (i|k) == !!(b[4]&d) && (j|k) == !!(b[5]&d)) {
            if(i) a[1] |= d;
            if(j) a[2] |= d;
            if(k) a[3] |= d;
        }
    ...
    for(_ i = 4; i <= n; ++i) {
        b.clear();
        b.push_back(qr("and", 1, i));
        b.push_back(qr("or", 1, i));
        for(_ d = 1; d <= 1E9; d <<= 1) {
            c.clear();
            c.push_back(!!(a[1]&d));
            c.push_back(!!(b[0]&d));
            c.push_back(!!(b[1]&d));
            if(c == (v_){1, 1, 1} || c == (v_){0, 0, 1}) a[i] |= d;
        }
    }
    sort(a+1, a+1+n);
    writesc("finish");
...
_ qr(str a, _ x, _ y) {
//    if(a == "and") return bd[x] & bd[y];
//    else return bd[x] | bd[y];
    writesc(a);
    writesc(x);
    writeflsh(y);
    flsh();
