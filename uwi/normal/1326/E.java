...
public class E {
	...
	void solve()
	{
		...
		LST yets = new LST(n);
		StarrySkyTree sst = new StarrySkyTree(2*n+1);
		int p = n-1;
		for(int i = 0;i < n;i++){
			while(p >= 0){
				int pos = (int)ai[p];
				int ne = yets.next(pos);
				if(ne != -1){
					yets.unset(ne);
					sst.add(2*pos, 2*n+1, 1);
					sst.add(2*ne+1, 2*n+1, -1);
					p--;
				}else{
					break;
				}
			...
			if(sst.min(2*b[i]+1, 2*b[i]+2) > 0){
				int lastc = sst.firstle(2*b[i], 0);
				sst.add(2*b[i]+1, lastc, -1);
				yets.set(lastc/2);
			}else{
				yets.set(b[i]);
			}
