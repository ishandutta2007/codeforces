...
void solve()
{
    ...
    for (int i = 0; i + 1 < a.size(); i++)
    {
        if (a[i] == 0)
        {
            ...
            if (sign == 1 && cnt_total > anslog)
            {
                anslog = cnt_total, x = i, y = n - j + 1;
            }
            if (sign == -1)
            {
                int del = 0, elems = 0;
                ...
                if (cnt_total - del >= anslog)
                    anslog = cnt_total - del, x = i + elems, y = n - j + 1;

                del = 0, elems = 0;
                for (int pos = (int)b.size() - 1; pos >= 0; pos--)
                {
                    elems++;
                    if (abs(b[pos]) == 2)
                        del++;
                ...
                if (cnt_total - del >= anslog)
                    anslog = cnt_total - del, x = i, y = n - j + elems + 1;
            }
        }
    }
    cout << x << " " << y << "\n";
