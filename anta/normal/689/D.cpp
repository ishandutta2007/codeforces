...
private:
	...
	const Val *getBlock(const Val *a, Index n, Index i) {
		...
		if(offset + BlockSize <= n)
			...
			Val maxVal = Val();
			for(Index j = i; j < n; j ++)	//ioffsetH(vZc)(oO())
...
ll solve(vector<int> a, vector<int> b) {
	int n = (int)a.size();
	DirectRMQ<int, greater<int>> rmqA;
	DirectRMQ<int> rmqB;
	rmqA.build(a.data(), n);
	rmqB.build(b.data(), n);
	ll res = 0;
	rep(i, n) {
		...
		int bL, bR;
		{
			...
			while(up - lo > 0) {
				int mid = (lo + up) / 2;
				if(rmqB.queryVal(b.data(), mid, i) >= a[i])
					up = mid;
				else
					lo = mid + 1;
			}
		...
		}
		{
			...
			while(up - lo > 0) {
				int mid = (lo + up + 1) / 2;
				if(rmqB.queryVal(b.data(), i, mid) >= a[i])
					lo = mid;
				else
					up = mid - 1;
			}
		...
		res += (ll)(min(aR, bR) + 1 - i) * (i - max(aL, bL) + 1);
	}
	return res;
}
