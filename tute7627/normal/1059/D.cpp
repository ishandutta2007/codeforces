...
int main(){
  ...
  while(cnt--){
    ...
    bool judge=true;
    rep(i,0,n){
      auto p=quadratic_equation(1,-2*x[i],y[i]*y[i]-2*y[i]*mid+x[i]*x[i]);
      //cout<<mid spa p.fi spa p.se.fi spa p.se.se<<endl;
      if(!p.fi)judge=false;
      else{
        chmax(mi,p.se.se);
        chmin(mx,p.se.fi);
