...
inline void solve() {
    ...
    for (int it = 0; it < 40; ++it) {
        ...
        for (int i = 0; i < n; ++i) {
            ld r1 = hypotl(pt[i].x, pt[i].y);
            if (r1 >= 2 * m) {
                continue;
            }
            ld fst = get_ang(pt[i].x, pt[i].y) - acosl(r1 / (2 * m));
            ld lst = get_ang(pt[i].x, pt[i].y) + acosl(r1 / (2 * m));

            ev.push_back({ fst, 1 });
            ev.push_back({ lst, -1 });
            ev.push_back({ fst + 2 * PI, 1 });
            ev.push_back({ lst + 2 * PI, -1 });
        }
