...
void solve(){
    ...
    ull ans=-1;
    ll cost=0;
    RmQ<char> seg(vector(all(s)),'~');
    RSQ<ll> seg2(vector<ll>(n,1));
    auto use=[&](ll at){
        cost+=seg2.get(0,at);
        seg2.set(at,0);
        seg.set(at,'~');
    ...
    rep(n){
        {
            const ll at=seg.find_last([c=t[i]](char x){return x>=c;});
            if(at!=n)chmin(ans,cost+seg2.get(0,at));
        }
        const ll at=seg.find_last([c=t[i]](char x){return x>c;});
        if(at==n||s[at]!=t[i])break;
        use(at);
    }
    out((ll)ans);
}
