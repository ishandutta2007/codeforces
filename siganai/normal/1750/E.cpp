...
int main() {
    ...
    while(TT--) {
        ...
        ll ans = 0;
        vi cs(n+1);
        rep(i,n) cs[i+1] = cs[i] + (s[i] == '(' ? 1 : -1);
        ans += abs_sum(cs);
        debug(cs);
        debug(ans);
        vvi g;
        int root;
        tie(g,root) = CartesianTree<int>(cs,true);
        auto dfs = REC([&](auto f,int now,int L,int R) -> ll {
            ll ret = 0;
            ret += (1LL * (now - L) * (R - now - 1) + R - L - 1) * cs[now];
            for(auto &nex:g[now]) {
                if(nex < now) {
                    ret += f(nex,L,now);
                }
