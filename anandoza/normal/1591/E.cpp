...
template <class T> using OST = __gnu_pbds::tree<T, null_type, less<T>,
...
void solve() {
    ...
    re(queries);
    for (auto &[v, l, k] : queries)
        v--, k--;

    vector<vector<array<int, 3>>> queries_by_node(n);
    for (int i = 0; i < q; i++) {
        auto &[v, l, k] = queries[i];
        queries_by_node[v].push_back({i, l, k});
    }
    ...
    auto answer_queries = [&](int cur) {
        for (auto &[i, l, k] : queries_by_node[cur]) {
            auto prefix = ost.order_of_key({l, -1});
            auto target = ost.find_by_order(prefix + k);
            if (target == ost.end())
                ans[i] = -1;
            else
                ans[i] = target->second;
    ...
    assert(all_of(all(ans), L1(u, u != -2)));
}
