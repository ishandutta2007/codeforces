...
void dfs(int i, bool b) {
    ...
    vis[i] = true;
    flip[i] = b;
    for (auto p : adj[i]) if (!vis[p.F]) dfs(p.F, b ^ p.S);
}
...
void solve() {
    ...
    F0R(i, n) {
        FOR(j, i+1, n) {
            if (a[i][j] != a[j][i] && d.unite(i, j)) {
                adj[i].pb(mp(j, a[i][j]>a[j][i]));
                adj[j].pb(mp(i, a[i][j]>a[j][i]));
            }
        }
    }
    vis.assign(n, false); flip.assign(n, false);
    F0R(i, n) if (!vis[i]) dfs(i, false);
    F0R(i, n) if (flip[i]) {
        F0R(j, n) swap(a[i][j], a[j][i]);
    }
    print(a, n, n);
}
