...
#define SZ 1000099
int n,C,c[SZ];
int m; ll w[SZ],s[SZ],mw[SZ];
#define SS 14000999
pair<ll,int> ms[SS]; int mn=0;
int main() {
    #ifdef ONLINE_JUDGE
    cin.tie(0);
...
//            ma[-R]=co;
    ...
    sort(ms+1,ms+1+mn);
    for(int i=2;i<=mn;++i)
        ms[i].se=min(ms[i].se,ms[i-1].se);
    /*
    ll ans=8e18;
    for(auto&t:ma)
        ans=min(ans,t.se),t.se=ans;*/
    // for(auto t:ma)
    //     cerr<<t.fi<<":"<<t.se<<"\n";
    cin>>m;
    for(int i=1;i<=m;++i) {
        ...
        ll ans=8e18;
        for(int j=1;j<=n;++j) {
            //for each c, only take the one with min w
            ll t=s[i]/w[j]+1;
            if(t*(__int128)c[j]>C) continue;
            // cerr<<t<<","<<c[j]<<" "<<C<<" "<<w[j]<<","<<s[i]<<"\n";
            ans=min(ans,t*c[j]);
        }
        // cerr<<ans<<"??\n";
        }
        auto g=upper_bound(ms+1,ms+1+mn,mp(-s[i],(int)2e9))-ms;
        ll ans=-1;
        if(g!=1)
            ans=ms[g-1].se;
        cout<<ans<<" ";
