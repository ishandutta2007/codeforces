...
bool check(int x) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            dp[w][j + sum(i, w - 1)] = min(dp[w][j + sum(i, w - 1)], dp[i][j] + 1);
        }
    }
    int e = INF;
    for (int i = k; i <= n; ++i) {
...
int solve() {
    cin >> n >> s >> m >> k;
    assert(1 <= k && k <= n && n <= 1500);
    assert(1 <= m && m <= s && s <= 1500);
    FI(n) {
        cin >> b[i];
    ...
    FI(s) {
        pii p;
        cin >> p;
        assert(1 <= p.first && p.second <= n);
        for (int j = p.first - 1; j < p.second; ++j) {
            used[j] = true;
        }
        where[p.first - 1] = max(where[p.first - 1], p.second);
    }
    int tot = 0;
    for (int i = 0; i < n; ++i) {
        tot += used[i];
