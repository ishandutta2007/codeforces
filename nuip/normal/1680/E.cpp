...
int solve(const vector<int> &vs){
	cauto n=(int)vs.size();
	auto dp=table(4,n,MOD);
	dp[0][0]=2;
	dp[1][0]=1+(vs[0]>1);
	dp[2][0]=1+vs[0]%2;
	dp[3][0]=3;
	reps(i,1,n){
		rep(j,4) MN(dp[0][i],dp[j][i-1]+2);
		MN(dp[1][i], dp[1][i-1]+1+(vs[i]>1));
		MN(dp[1][i], dp[0][i-1]+2);
		MN(dp[1][i], dp[2][i-1]+2);
		MN(dp[1][i], dp[3][i-1]+1);
		MN(dp[2][i], dp[2][i-1]+1+vs[i]%2);
		MN(dp[2][i], dp[0][i-1]+2);
		MN(dp[2][i], dp[1][i-1]+2);
		MN(dp[2][i], dp[3][i-1]+1);
		rep(j,4) MN(dp[3][i], dp[j][i-1]+2+(j!=3));
	}
	return min({dp[1][n-1]-1,dp[2][n-1]-1,dp[3][n-1]-2});
}
...
int main(){
	...
	TT{
		cauto n=read(0);
		cauto mp=readV<string>(2);
		deque<int> deq(n);
		rep(i,2)rep(j,n)if(mp[i][j]=='*') deq[j]|=1<<i;
		while(deq.front()==0) deq.pop_front();
		while(deq.back()==0) deq.pop_back();
		cout<<solve(vector<int>(all(deq))) NL;
	}
	return 0;
}