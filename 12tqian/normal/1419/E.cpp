...
int main(){
    ...
    while(tt--){
        ...
        vi pri;
        for (auto x: f) pri.eb(x.f);
        vector<int> div = getDivi<int>(n);
        map<int, set<int>> in;
        for (int x: div){
            if(x==1) continue;
            for(int p: pri){
...
//            ps(in);
            for (auto x: in[p]){
                if(x == p*p*q || x == p*q) continue;
                res.eb(x);
                to_rem.eb(x);
            }
            for(int x: to_rem) rem(x);
//            ps(res);
            res.eb(p*q);
            rem(p*q);
            res.eb(q);
