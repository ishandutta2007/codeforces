...
void solve()
{
    ...
    for (int i = 0; i<m; i++) cin>>hero[i].second>>hero[i].first;
    sort(hero.begin(), hero.end());
    vector<int> suf_max(n+1);
    for (int i = 0; i<m; i++) suf_max[hero[i].first] = max(suf_max[hero[i].first], hero[i].second);
    for (int i = n-1; i>=0; i--) suf_max[i] = max(suf_max[i], suf_max[i+1]);
    ...
    while (cur<n)
    {
        ...
        int maxx = a[cur];
        int kill = 1;
        if (maxx>suf_max[1]) {cout<<-1<<endl; return;}
        while (cur+1<n && max(maxx, a[cur+1])<=suf_max[kill+1])
        {
            kill++;
            cur++;
            maxx = max(maxx, a[cur]);
