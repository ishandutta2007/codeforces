...
vi friends(const vvvb& w, const int x, const int y) {
	vi ans;
...
void fl(const int x, const int y, const vvvb& w, vvi& g, vb& nv) {
	nv[x] = false;
	g[x] = friends(w, x, y);
	for (int i : g[x])
		if (nv[i])
...
void dfs_p(const int v, const int pr, const vvi& g, vi& p, vi& h) {
	p[v] = pr;
	for (int to : g[v])
		if (to != pr) {
			h[to] = h[v] + 1;
...
int find_dist(int x, int y, const vvi& jmp, const vi& h) {
	int ans = h[x] + h[y];
	int u = jmp.size() - 1;
	for (int i = u; i >= 0; --i)
		if (h[x] - h[y] >= (1 << i))
...
vvi find_dst(const vvi& g) {
	int n = sz(g);
	vvi dst(n, vi(n));
	pair<vi, vi> ph = find_p(g);
	vvi jmp = find_jmp(ph.X);
	for (int i = 0; i < n; ++i)
		for (int j = i + 1; j < n; ++j) {
			dst[i][j] = find_dist(i, j, jmp, ph.Y);
			dst[j][i] = dst[i][j];
		}
	return dst;
}
		...
		for (int i = 1; i < n; ++i)
			if (!y[i]) {
				...
				for (int i = 0; good && i < n; ++i)
					for (int j = i + 1; good && j < n; ++j)
						for (int k = 0; good && k < n; ++k)
							if ((dst[i][k] == dst[j][k]) != w[i][j][k]) {
								good = false;
							}
				if (!good)
					g.clear();
