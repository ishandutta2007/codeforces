...
int main(){
	...
	int cur = 0;
	for (auto& i : v) {R(i.x, i.y, i.z); i.idx = ++cur;}
	for (int i = 0; i < n; i++) isRemoved[i] = 0;
	...
	for (int i = 0; i < n; i++) {
		...
		if (take == -1) take = i;
			...
			if (v[take].x == v[i].x && v[take].y == v[i].y) {
				W(v[take].idx,v[i].idx);
				isRemoved[take] = 1;
				isRemoved[i] = 1;
				take = -1;
			} else take = i;
	...
	for (int i = 0; i < n; i++) {
		...
		if (take == -1) take = i;
			...
			if (v[take].x == v[i].x && v[take].z == v[i].z) {
				W(v[take].idx,v[i].idx);
				isRemoved[take] = 1;
				isRemoved[i] = 1;
				take = -1;
	...
	for (int i = 0; i < n; i++) {
		...
		if (take == -1) take = i;
			...
			if (v[take].y == v[i].y && v[take].z == v[i].z) {
				W(v[take].idx,v[i].idx);
				isRemoved[take] = 1;
				isRemoved[i] = 1;
				take = -1;
