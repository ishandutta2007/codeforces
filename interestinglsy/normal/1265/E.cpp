...
void Toggle( int pos ){
	if(ison[pos]){
		int l = *prev(chkp.lower_bound(pos));
		int r = *next(chkp.lower_bound(pos));
		Deci( ans , Query(l,pos-1,ROOT).b );
		Deci( ans , Query(pos,r-1,ROOT).b );
		Addi( ans , Query(l,r-1,ROOT).b );
		chkp.erase(pos);
	}else{
		chkp.insert(pos);
		int l = *prev(chkp.lower_bound(pos));
		int r = *next(chkp.lower_bound(pos));
		// P(l,r);
		Deci( ans , Query(l,r-1,ROOT).b );
		Addi( ans , Query(l,pos-1,ROOT).b );
		Addi( ans , Query(pos,r-1,ROOT).b );
	}
	ison[pos] ^= 1;
}
