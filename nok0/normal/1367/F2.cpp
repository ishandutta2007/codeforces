...
void main_() {
	...
	int res = 0;
	REP(i, n) {
		chmax(dp[i + 1][0], dp[last[a[i]]][0] + 1);
		chmax(dp[i + 1][1], dp[last[a[i]]][1] + 1);
		chmax(dp[i + 1][2], dp[last[a[i]]][2] + 1);
		if(a[i] and nc[a[i]] == 0) {
			chmax(dp[i + 1][1], dp[last[a[i] - 1]][0] + 1);
			if(nc[a[i] - 1] == cnt[a[i] - 1]) chmax(dp[i + 1][1], dp[last[a[i] - 1]][1] + 1);
		}
		if(a[i]) {
			chmax(dp[i + 1][2], dp[last[a[i] - 1]][0] + 1);
			if(nc[a[i] - 1] == cnt[a[i] - 1]) chmax(dp[i + 1][2], dp[last[a[i] - 1]][1] + 1);
		}
		chmax(res, dp[i + 1][0]);
		chmax(res, dp[i + 1][1]);
		chmax(res, dp[i + 1][2]);
