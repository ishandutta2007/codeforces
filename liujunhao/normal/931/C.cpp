...
//template
#define MAXN 100000
int n,a[MAXN+10],tot,d,cnt[3],ans,ansi;
void read(){
	Read(n);
	int i;
	for(i=1;i<=n;i++)
...
void solve(){
	...
	for(i=0;i<=n;i++){
		...
		if(2*i+n-i<tot)
			continue;
		int t=min(cnt[1],tot-2*i)+min(i,cnt[2])+min(cnt[0],n-i-tot+2*i);
		if(t<ans)
			ans=t,ansi=i;
	}
	printf("%d\n",ans);
	for(i=1;i<=ansi;i++)
		printf("%d ",2+d);
	for(i=tot-2*ansi;i;i--)
		printf("%d ",1+d);
	for(i=n-ansi-tot+2*ansi;i;i--)
		printf("%d ",d);
}
int main()
{
