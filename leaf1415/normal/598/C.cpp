...
int main(void)
{
	...
	rep(i, 1, n) q[i-1] = Q(atan2(y[i], x[i]), i);
	sort(q, q+n);
	
	double ans = inf; ll ans_i, ans_j;
	rep(i, 0, n-1){
		double x = q[i].first, y = q[(i+1)%n].first;
		if(i == n-1) y += PI*2;
		double ang = y-x;
		if(ans > ang) ans = ang, ans_i = q[i].second, ans_j = q[(i+1)%n].second;
	}
	outl(ans_i, ans_j);
