...
int n, m;
mint jc[1 << 20], fjc[1 << 20];

mint inline C(int a, int b) {
	if(b < 0 || a < b) return 0;
	return jc[a] * fjc[b] * fjc[a - b];
}

int main() {
	scanf("%d%d", &n, &m);
	jc[0] = 1; cont(i, n) jc[i] = jc[i - 1] * i;
	fjc[n] = inv(jc[n]); range(i, n, 1, -1) fjc[i - 1] = fjc[i] * i;
	mint m1 = 1, mm = 1, fm = inv(mint(m));
	cont(i, n) mm *= m;
	mint ans = mm;
	loop(i, n) {
		ans += m1 * mm * C(n, i + 1);
