...
const int modular_int<mod>::inverse_limit = 300005;


/*/-----------------------------Code begins----------------------------------/*/


gp_hash_table<ll,mint> cnt[43];

ll num[43];
const int N = 3e5 + 100;
ll msk[N], val[N];
inline bool is_edge(ll i,ll j){
	return (msk[num[i]] & msk[num[j]]) > 0;
}

mint ways(ll mask, ll num){
	if (num == 0){
		return 1;
	}
	if (mask == 0){
	...
	if (cnt[num].find(mask) != cnt[num].end()){
		return cnt[num][mask];
	}
	ll M = mask;
	...
	mask ^= (1ll << k);
	cnt[num][M] = ways(mask, num);

	for(int z = k + 1; (1ll << z) <= mask; ++z){
		if (mask & (1ll << z) ){
			if (is_edge(k, z)){
				mask ^= (1ll << z);
			}
...
void solve(){
	...
	for(int i = 1; i <= n; ++i){
		...
		for(int k = 0; k <= ex && k <= m && k <= i; ++k){
			if (tot_op - ex + k < i)
				continue;
			ans += ncr(tot_op - ex, i - k) * ways(cmask, k);
		}
		for(auto j: rem[i]){
			cmask ^= val[j];
			--tot_op;
