...
ll solve(ll mid, vector<ll> h, ll a, ll r, ll m) {
    ll pos = 0, neg = 0;
    if (a + r > m) {
        for (auto i : h) {
            if (i < mid) neg += mid - i;
    ...
    assert(pos == neg && neg == 0);
    for (auto i : h) {
        if (i < mid) ans += a * (mid - i);
        else ans += r * (i - mid);
    }
    return ans;
}
...
signed main() {
    ...
    ll vl = 0, vr = 2e9;
    while (vr - vl > 2) {
        ll m1 = vl + (vr - vl) / 3, m2 = vr - (vr - vl) / 3;
        ll f1 = solve(m1, h, a, r, m);
        ll f2 = solve(m2, h, a, r, m);
        if (f1 > f2) vl = m1;
        else vr = m2;
    }
    ll x = solve(vl, h, a, r, m);
    for (ll i = vl + 1; i <= vr; ++i)
        chmin(x, solve(i, h, a, r, m));
    println(x);
    #ifdef DEBUG
