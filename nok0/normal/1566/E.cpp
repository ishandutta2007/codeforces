...
void main_() {
	...
	auto dfs = [&](auto dfs, int now) -> bool {
		...
		is_leaf[now] = potent;
		if(SZ(g[now]) == 0) is_leaf[now] = 1;
		if(now == 0) is_leaf[now] = 0;
		if(ok and has_leaf) bud_cnt++, is_bud[now] = 1;
		return ok;
	};
	dfs(dfs, 0);
	debug(is_bud);
	...
	bool oth = 0;
	REP(i, 1, n) {
		if(!is_bud[p[i]] and !is_bud[i]) oth = 1;
	}
	debug(oth);
	int leafs = accumulate(all(is_leaf), 0);
	if(oth) {
		print(leafs - bud_cnt);
	} else
		print(leafs - bud_cnt + 1);
}