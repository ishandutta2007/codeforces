...
fun main(){
    solve.cases{
        ...
        val d = Array(n) { IntArray(n) { plarge } }
        for(i in 0 until n){
            d[i][i] = 0
        }
        for((a,b,_) in edges){
        ...
        val full = d[s][t]
        val crit = BooleanArray(n){d[s][it] + d[t][it] == full}

        val closest = IntArray(n){plarge}
        for(i in 0 until n){
            if(!crit[i] ) continue
            for(j in 0 until n){
                closest[j] = minOf(closest[j], d[i][j])
            }
        }
        var ret = phuge

        for((a,b,c) in edges){
            val op1 = d[s][a] + d[t][b] + 1
            val op2 = d[s][b] + d[t][a] + 1
            val op3 = closest[a] + 1 + full + 1
            val op4 = closest[b] + 1 + full + 1

            val best = intArrayOf(op1,op2,op3,op4).minOrNull()!!
            ret = minOf(ret, best.toLong() * c )
        }
        put(ret)
