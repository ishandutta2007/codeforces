...
int f(const int& x)
{
	return x * x * (a * x + b) + c * x + d;
}

int s_toi(char* c, const int& n)
{
	int ans = 0;
	for (int i = 0; i < n; ++i)
		if (c[i] == '1')
...
int main()
{
	...
	for (int i = 0; i < N; ++i)
	{
		...
		st[i].resize(zz);
		int j = 0;
		for (auto it = z.begin(); it != z.end(); ++it, ++j)
		{
			st[i][j] = *it;
		}
		for (int j = 1; j < zz; ++j)
	...
	for (int I = 0; I < Q; ++I)
	{
		...
		int x = s_toi(c, n), k;
		cin >> k;
		auto p = upper_bound(st[x].begin(), st[x].end(), pii(k, (1 << 31) - 1));
		if (p == st[x].begin())
			cout << "0\n";
		else
		{
