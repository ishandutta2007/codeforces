...
int query(const vector<int> &lhs, const vector<int> &rhs)
{
    printf("? %d %d\n", lhs.size(), rhs.size());
    for(int i = 0; i < lhs.size(); i++) printf("%d%c", lhs[i], " \n"[i == lhs.size() - 1]);
    for(int i = 0; i < rhs.size(); i++) printf("%d%c", rhs[i], " \n"[i == rhs.size() - 1]);
    fflush(stdout);
    int res;
    scanf("%d", &res);
    return res;
...
void work()
{
    ...
    {
        vector<int> lhs;
        for(int j = 1; j < i; j++) lhs.epb(j);
        int res = query(lhs, vector<int> {i});
        if(res != 0)
        {
            id = i;
            break;
    ...
    int l = 1, r = id - 1, mid, tmp;
    while(l <= r)
    {
        mid = MID(l, r);
        vector<int> lhs;
        for(int j = 1; j <= mid; j++) lhs.epb(j);
        int res = query(lhs, vector<int> {id});
        if(res != 0) tmp = mid, r = mid - 1;
        else l = mid + 1;
    }
    ...
    for(int i = id + 1; i <= n; i++)
        if(query(vector<int> {i}, vector<int> {id}) == 0)
            ans.epb(i);
