...
int main() {
    ...
    STR(s);
    int times = count(all(s), '*');
    int plus = count(all(s), '+');
    int minus = count(all(s), '-');
    if(!times) {
        if(plus) {
            cout << a[0];
            rep2(i, 1, n - 1) cout << "+" << a[i];
        } else {
            cout << a[0];
            rep2(i, 1, n - 1) cout << "-" << a[i];
        }
        ...
        if(!plus and !minus) {
            ...
            auto solve = [&](int l, int r) {
                ...
                dp[0] = 0;
                rep(i, si(s)) {
                    dp[i + 1] = dp[i] + s[i].fi * s[i].se;
                    mem[i + 1] = i;
                    ll now = s[i].fi;
                    rep3(j, i - 1, 0) {
                        now *= s[j].fi;
                        if(chmax(dp[i + 1], dp[j] + now)) mem[i + 1] = j;
                    }
                }
                int now = si(s);
                while(now) {
                    ...
                    int st = idx[pre].se;
                    int ed = idx[now].se;
                    if(s[now - 1].fi == 1 and pre == now - 1) {
                        now = pre;
                        continue;
                    }
                    rep2(i, st, ed - 2) op[i] = 1;
                    now = pre;
                }
            };
            for(auto [l, r] : v) solve(l, r);
            cout << a[0];
            rep2(i, 1, n - 1) { cout << "+*"[op[i - 1]] << a[i]; }
        }
    }
    cout << endl;
}