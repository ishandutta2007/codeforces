...
#endif
//#STAY AT HOME
const int nax = 1510, mod = 119 << 23 | 1;
int a[nax][nax];
int u[nax * 2];
int edges[nax * 2], siz[nax * 2];
int first_free = nax;
pii made_of[nax * 2];
bool valid[nax * 2];
void reset() {
...
void put_edge(int a, int b) {
	...
	if (a == b) {
		edges[a]++;
		if (edges[a] == siz[a] * (siz[a] - 1) / 2)
			valid[a] = true;
	}
	else {
		int top = first_free++;
		u[a] = u[b] = top;
		edges[top] = edges[a] + edges[b] + 1;
		siz[top] = siz[a] + siz[b];
		made_of[top] = {a, b};
		if (edges[top] == siz[top] * (siz[top] - 1) / 2)
			valid[top] = true;
	}
}
vll ans[nax * 2];
void dfs(int x) {
	...
	dfs(a);
	dfs(b);
	int l1 = ans[a].size(), l2 = ans[b].size();
	ans[x].resize(l1 + l2 - 1);
	REP(i, l1) REP(j, l2)
		(ans[x][i + j] += ans[a][i] * ans[b][j]) %= mod;
	if (valid[x]) ans[x][1]++;
	debug(arr(ans, x));
}
void solve() {
	reset();
	memset(made_of, -1, sizeof(made_of));
	int n;
	scanf("%d", &n);
	REP(i, n) valid[i] = true;
	REP(i, n) REP(j, n) scanf("%d", &a[i][j]);
	vector <tuple <int, int, int> > edges;
	REP(i, n) for (int j = i + 1; j < n; ++j) edges.emplace_back(a[i][j], i, j);
	sort(edges.begin(), edges.end());
	debug(imie(edges));
	for (auto [_, a, b] : edges) {
		put_edge(a, b);
	}
	int root = find(0);
	dfs(root);
