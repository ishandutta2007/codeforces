...
int main() {
	...
	vector<vector<vector<int>>> dp(n + 7, vector<vector<int>>(n + 7, vector<int>(n + 7, INT_MIN)));
	dp[0][0][0] = 0;
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j <= K; j++)
			for (int k = 0; k <= n; k++) {
				chkmax(dp[i + 1][j + (x != t[0])][k + 1], dp[i][j][k]);
				chkmax(dp[i + 1][j + (x != t[1])][k], dp[i][j][k] + k);
				chkmax(dp[i + 1][j][k], dp[i][j][k]);
				
				if (t[0] == t[1]) {
					chkmax(dp[i + 1][j + (x != t[0])][k + 1], dp[i][j][k] + k);
				}
			}
	}
