...
fun Graph.strongTarjae(): MutableList<List<Int>> {
    //3000 for 5M nodes
    //CF: 1591 for 1M nodes
...
fun Graph.strongCompression():IntArray{
    val comps = strongTarjae()
    val ret = IntArray(n)
...
fun main(){
    solve.cases{
        ...
        val G2 = Graph(n,m,false)
        repeat(m){
            val v = getint - 1
            val w = getint -1
            val len = getint
            G.addWeighted(v,w,len)
            G2.addWeighted(v,w,len)
        }
        val strong = G.strongCompression()
        val visited = BooleanArray(n)
        val parent = IntArray(n)
        val onstack = LongArray(n){nhuge}

        val best = LongArray(n)

        fun visit(v:Int){
            G2.NS_E(v){
                    e,w->
                if(strong[w] != strong[v]){
                    return@NS_E
                }
                if(onstack[w] != nhuge){
                    val w1 = if(e and 1 == 0) G2.weights[e shr 1] else  -G2.weights[e shr 1 ]
                    val part2 = onstack[v] - onstack[w]
                    best[strong[v]] = gcd(best[strong[v]], (w1 + part2).absoluteValue)
                }
                if(!visited[w]){
                    visited[w] = true
//                    parent[w] = v
                    onstack[w] = onstack[v] + if(e and 1 == 0) G2.weights[e shr 1] else  -G2.weights[e shr 1 ]
                    visit(w)
                }
            }
            onstack[v] = nhuge
        ...
        repeat(q){
            ...
            val t = getint
            val cm = strong[v]
            val cando = best[cm].absoluteValue
            if(cando == 0L) {
                if(s == 0) {
                    put("YES")
                }else {
            ...
            }else if((-s).toLong() % gcd(t.toLong(), cando) == 0L){
                    put("YES")
                }else{
                    put("NO")
                }
...
20 minutes ok la
template bad cannot really save
