...
int solve() {
    ...
    FI(n) {
        ...
        --it;
        int nextFree = it->second;
        if (nextFree < a[i].second) {
            nextFree = a[i].second;
        }
        ans += (nextFree - a[i].second) * 1LL * a[i].first;
        if (nextFree == it->second) {
            it->second++;
        } else {
            it = m.insert({nextFree, nextFree + 1}).first;
        }
        auto nex = it; ++nex;
        if (nex->first == it->second) {
            it->second = nex->second;
            m.erase(nex);
        }
    }
    cout << ans << endl;
