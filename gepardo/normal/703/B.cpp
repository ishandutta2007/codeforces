...
main()
{
    ...
    for (int i = 0; i < k; i++) cap[c[i]] = true;

    // K - not capitals
    // Q - capitals

    int bKQ = 0, bKK = 0, bQQ = 0;

    int sumK = 0, sum = 0;
    for (int i = 0; i < k; i++) sumK += b[c[i]];
    for (int i = 0; i < n; i++) sum += b[i];

    for (int i = 0; i < k; i++)
        bKQ += sum * b[c[i]] - b[c[i]] * b[c[i]];

    for (int i = 0; i < k; i++)
        bKK += sumK * b[c[i]] - b[c[i]] * b[c[i]];

    bKQ -= bKK;
    bKK /= 2;

    for (int i = 1; i < n; i++)
        if (!cap[i] && !cap[i-1]) bQQ += b[i] * b[i-1];
    if (!cap[0] && !cap[n-1]) bQQ += b[0] * b[n-1];
