...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            if (items.size() && items.back().first == 1)
                items[items.size() - 1].second++;
            else
                items.push_back({1, 1});
        }
    }
    ...
    for (int i = 0; i < items.size(); i++)
    {
        ...
        if (items[i].first != 1)
        {
            ...
            {
                if (INFLL / items[j].first <= p)
                    break;
                p *= items[j].first;
                s += items[j].first * items[j].second;
                //cout << p << " " << s << "\n";
                if (items[j].first != 1)
                {
                    ll L = 0, R = 0;
                    if (i - 1 >= 0 && items[i - 1].first == 1) L = items[i - 1].second;
                    if (j + 1 < items.size() && items[j + 1].first == 1) R = items[j + 1].second;
                    if (p % k == 0)
                    {
                        ll sum_req = p / k;
                        if (s > sum_req) continue;
                        ll D = sum_req - s;
                        if (D > L + R) continue;
                        ans += min(D, min(L, D) + min(R, D) - D) + 1;
                    }
                }
            }
        }
