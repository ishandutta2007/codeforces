...
#undef read
#define read rill
ll p[100001],yy[9],tyy=0,top=0;
ll n;
void calc(){
	rep(i,2,100000){
		if(!p[i])p[++top]=i;
		rfor(ll j=1;j<=top&&i*p[j]<=100000;j++){
			p[i*p[j]]=1;
			if(!(i%p[j]))break;
		}
	}
...
ll kn[23];
int main(){
//	IO(CF1406E Deleting Numbers);
	rill(n);ll x;
	calc();p[top+1]=10000000ll;
	ll qz=1;
	rep(i,1,s){
		if(p[i]>n)break;
		writeln("B ",p[i]);
...
//	writeln("tyy=",tyy);
	...
	rep(i,1,tyy){
		...
		rep(ii,2,20){
			...
			kn[ii]=kn[ii-1]*p[yy[i]];
//			writeln("kn[",ii,"]=",kn[ii],' ',kn[ii-1],' ',p[yy[i]],' ',yy[i]);
		}
//		writeln("r=",r);
		while(l<=r){
			ll mid=(l+r)>>1;
