...
const int N = 5e5+5, P = 22;
int n;
int p[N];
vi children[N];
char c[N];

int depth[N], sz[N], word[N];

int dfs0(int v, int p) {
  if (p != -1) {
    word[v] = word[p] ^ (1 << int(c[v]-'a'));
    depth[v] = depth[p] + 1;
  }
  sz[v] = 1;
  for (int u : children[v]) sz[v] += dfs0(u, v);
  return sz[v];
}
...
void add(int v, int x) {
  if (x == 1) smax(best[word[v]], depth[v]);
  else best[word[v]] = -INF;
  for (int u : children[v]) add(u, x);
}

int calc(int v) {
  int res = depth[v] + best[word[v]];
  REP(bit, P) smax(res, depth[v] + best[word[v] ^ (1 << bit)]);
  return res;
}

int solve(int v) {
  int res = calc(v);
  for (int u : children[v]) smax(res, solve(u));
  return res;
}
...
void dfs(int v, bool keep) {
  ...
  int bigChild = children[v][0];
  for (int u : children[v])
    if (sz[u] > sz[bigChild]) bigChild = u;
  for (int u : children[v])
    if (u != bigChild) dfs(u, false);
  dfs(bigChild, true);

  for (int u : children[v]) if (u != bigChild) {
    smax(ans[v], solve(u));
    add(u, 1);
  }
  smax(ans[v], calc(v));
  ...
  for (int u : children[v]) smax(ans[v], ans[u]);

  smax(best[word[v]], depth[v]);
  if (!keep) add(v, -1);
}
