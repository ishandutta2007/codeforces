...
void calc(vl sm,ll n,vl_l&k){
    ll m=inf;
    rep(i,0,n)chmin(m,sm[i]);
    rep(j,0,1){
        l_l lr={inf,-inf};
        rep(i,0,n-1){
            if(sm[i]==m+j){
...
void calc2(vl sm,ll n,vl_l&k){
    ...
    rep(j,0,1){
        ...
        ll L=-1;
        rep(pos,1,n){
            if(sm[pos-1]==m+j && sm[pos]==m+j+1){
                L=pos;
                ko=1;
            }else if(sm[pos-1]==m+j+1 && sm[pos]==m+j){
                if(L==-1)continue;
                if(chmax(mxko,ko)){
                    mxlr={L,pos};
                }
            }else if(sm[pos]==m+j+1){
                ko++;
            }
        }
        k.pb(mxlr);
...
signed main(){fastio
    ...
    vl_l kou;
    calc(sm,n,kou);
  //  for(auto x:kou)cerr<<x.fi<<" "<<x.se<<endl;
    calc2(sm,n,kou);
  //  for(auto x:kou)cerr<<x.fi<<" "<<x.se<<endl;
    ...
    auto cal=[](string s){
        ...
        ll m=inf;
        for(auto x:sm)chmin(m,x);
        ll cnt=0;
        for(auto x:sm)if(x==m)cnt++;
        return cnt;
    };
    ll ans=cal(s);
    l_l pos={1,1};
