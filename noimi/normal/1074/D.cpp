...
struct UnionFind {
    ...
    vec rank;
    vec diff;
    UnionFind(int n=1,int s = 0){
        init(n,s);
    }
    void init(int n =1,int s=0){
        par.resize(n);rank.resize(n);diff.resize(n);
        rep(i,n)par[i]=i,rank[i]=0,diff[i]=s;
    }
    int root(int x){
        if(par[x]==x)return x;
        int r = root(par[x]);
...
signed main(){
    ...
    while(q--){
        ...
        if(t==1){
            ...
            r++;
            int L,R;
            if(mp.count(l))L = mp[l];
            else mp[l] = L = mp.size();
            if(mp.count(r))R = mp[r];
            else mp[r] = R = mp.size();
            uf.merge(L,R,x);
        }
        else{
