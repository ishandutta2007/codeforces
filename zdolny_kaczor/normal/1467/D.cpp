...
void solve() {
	...
	for (int l = 1; l <= k; ++l) {
		for (int i = 0; i < n; ++i) {
			if (i) add(paths[l][i], paths[l - 1][i - 1]);
			if (i != n - 1) add(paths[l][i], paths[l - 1][i + 1]);
		}
	}
	for (int i = 0; i < n; ++i) {
		for (int p = 0; p <= k; ++p) {
			(coef[i] += paths[p][i] * 1ll * paths[k - p][i]) %= mod;
		}
	}
	debug(range(coef, 0, n));
	debug(dump(paths, 0, k + 1, 0, n));
	for (int i = 0; i < n; ++i) scanf("%d", a + i);
	ll val = 0;
	for (int i = 0; i < n; ++i) (val += a[i] * coef[i]) %= mod;
	REP(_, q) {
		int p, x;
		scanf("%d%d", &p, &x);
		p--;
		(val += coef[p] * 1ll * (x - a[p])) %= mod;
		a[p] = x;
		if (val < 0) val += mod;
		printf("%lld\n", val);
	}
