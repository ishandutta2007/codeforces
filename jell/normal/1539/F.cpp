#line 1 "codeforces-workspace\\contest\\1539\\f\\f.cpp"
...
#line 6 "codeforces-workspace\\contest\\1539\\f\\f.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace\\contest\\1539\\f\\f.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1539\\f\\f.cpp"
...
#line 27 "codeforces-workspace\\contest\\1539\\f\\f.cpp"
...
struct mono {
  int mi = 1e9, ma = -1e9;
  mono operator+(mono x) const { return {min(mi, x.mi), max(ma, x.ma)}; }
  mono operator*(endo x) const { return {mi + x.add, ma + x.add}; }
};
...
void main() {
  ...
  for (auto x : range(n)) {
    auto &v = a[x];
    for (auto i : v) {
      auto r = sgt.fold(i + 1, sgt.size()).ma;
      auto l = sgt.fold(0, i + 1).mi;
      chgr(ans[i], (r - l) / 2);
    }

    for (auto i : v) {
      sgt.update(i + 1, sgt.size(), {-2});
    }

    for (auto i : v) {
      auto r = sgt.fold(i + 1, sgt.size()).mi;
      auto l = sgt.fold(0, i + 1).ma;
      chgr(ans[i], (l - r - 1) / 2);
    }
  }
