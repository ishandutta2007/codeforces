...
int main(){
    ...
    VEC(ll,a,n);
    vv(Modint,p,n,n);
    rep(n)rep(j,n)p[i][j]=a[i]+a[j]?Modint(a[i])/(a[i]+a[j]):Modint(2).inv();
    vec(Modint,dp,1<<n);
    Modint ans=0;
    rep(bit,1,1<<n){
        Modint x=1;
        rep(n)if(bit>>i&1)rep(j,n)if(~bit>>j&1)x*=p[i][j];
        x-=dp[bit];
        rep(bit2,1,1<<n)if(!(bit&bit2)){
            const ll bit3=~(bit|bit2);
            Modint y=x;
            rep(n)if(bit2>>i&1)rep(j,n)if(bit3>>j&1)y*=p[i][j];
            dp[bit|bit2]+=y;
        }
        ans+=x*popcnt(bit);
    }
