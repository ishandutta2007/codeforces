...
int main(){
	...
	for(int k = max(1, sz(c[0])); k <= n; ++ k){
		...
		for(int i = 1; i < m; ++ i){
			if(sz(c[i]) >= k){
				cnt += sz(c[i]) - (k - 1);
				cur += accumulate(c[i].begin() + k - 1, c[i].end(), 0LL);
				t.insert(t.end(), c[i].begin(), c[i].begin() + k - 1);
			}
			else{
				t.insert(t.end(), all(c[i]));
			}
		}
		sort(all(t));
		if(cnt > k){
		...
		if(cnt < k){
			...
			cur += accumulate(t.begin(), t.begin() + k - cnt, 0LL);
		}
		ctmin(cost, cur);
	}
	cout << cost;
