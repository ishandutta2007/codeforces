...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        int n = s.size();
        arr2d<int> next(n + 2, 26);
        fill(all(next[n + 1]), n + 1);
        fill(all(next[n]), n + 1);
        for (int i : RevRange(n)) {
            copy(all(next[i + 1]), next[i].begin());
            next(i, s[i] - 'a') = i + 1;
        }
        ...
        for (int i : range(t.size())) {
            ...
            cur[0] = 0;
            for (int j : range(i)) {
                cur[j + 1] = next(cur[j], t[j] - 'a');
            }
            for (int j : range(i, t.size())) {
                int c = t[j] - 'a';
                for (int k : range(i + 1)) {
                    cur[k] = next(cur[k], c);
                }
                for (int k : range(i)) {
                    cur[k + 1] = min(cur[k + 1], next(cur[k], t[k] - 'a'));
                }
            }
            if (cur[i] <= n) {
                out.printLine("YES");
