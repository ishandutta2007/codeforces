...
void solve() {
  ...
  for (auto&& [a, b, r]: dat) a -= x0, b -= y0;
  x1 -= x0, y1 -= y0;

  Re a = x1, b = -y1;
  Re cf = sqrt(x1 * x1 + y1 * y1);
  a /= cf * cf;
  b /= cf * cf;
  tie(x1, y1) = mp(a * x1 - b * y1, a * y1 + b * x1);
  for (auto&& [x, y, r]: dat) {
    tie(x, y) = mp(a * x - b * y, a * y + b * x);
    r /= cf;
  }
  ...
  for (auto&& [a, b, r]: dat) {
    ...
    auto check1 = [&](Re y) -> bool {
      // 
      Re dx = a - 0.5, dy = b - y;
      Re d = sqrt(dx * dx + dy * dy);
      Re R = sqrt(0.25 + y * y);
      Re f = b * b + dx * dx - 2 * b * y - 0.25;
      return f / (d + R) > r;
    };
    auto check2 = [&](Re y) -> bool {
      // 
      Re dx = a - 0.5, dy = b - y;
      Re d = sqrt(dx * dx + dy * dy);
      Re R = sqrt(0.25 + y * y);
    ...
    Re lo = binary_search_real(check1, -INF, INF);
    Re hi = binary_search_real(check2, INF, -INF);
    if (flag) tie(lo, hi) = mp(-hi, -lo);
    NG.eb(lo, hi);
  }

  Intervals<int, Re> I(1);
  for (auto&& [l, r]: NG) { I.set(l, r, 0); }
