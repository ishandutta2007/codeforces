...
bool check(vector<pair<pii, pii>>& v, const int& x, const int& y)
{
	int n = sz(v);
	int a = 0;
	for (int i = 0; i < n; ++i)
		if ((v[i].X.X <= x) && (v[i].X.Y <= y) && (v[i].Y.X >= x) && (v[i].Y.Y >= y))
			++a;
	if (a >= n - 1)
	{
		cout << x << ' ' << y << '\n';
...
int main()
{
	...
	cin >> v;
	pair<int, int> x(-1e9 - 100, -1e9 - 100), y(-1e9 - 100, -1e9 - 100);
	for (int i = 0; i < n; ++i)
	{
		x = max(x, max(pii(x.X, v[i].X.X), pii(v[i].X.X, x.X)));
		y = max(y, max(pii(y.X, v[i].X.Y), pii(v[i].X.Y, y.X)));
	}
	if (check(v, x.X, y.X))
		return 0;
	if (check(v, x.X, y.Y))
