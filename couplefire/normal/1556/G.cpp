...
int n, m, cnt, tot, ch[maxV][2], tag[maxV], head[maxn];
bool mark[maxV];
ll U, a[maxn], b[maxn];
char op[maxn][10];
struct edge { int u, v, nxt; } e[maxV * 15];
...
void add_edge(int t, int u, int v) {
    e[++cnt] = (edge){u, v, head[t]}, head[t] = cnt;
}

void merge(int k1, int k2) {
    if (mark[k1] && mark[k2]) { add_edge(min(tag[k1], tag[k2]), k1, k2); return; }
...
int main() {
    ...
    DSU::init();
    for (int i = head[m + 1]; i; i = e[i].nxt) {
