...
int dp0[107][12345],dp1[107][12345];
int n,k,a[107],b[107],c[107];

int& fangwen(int i,int j)
{
	if(j>=0)return dp0[i][j];
	else return dp1[i][-j];
}
...
int main()
{
	...
	fangwen(1,0)=0;fangwen(1,c[1])=a[1];
	for(int i=2;i<=n;i++)
		for(int j=-10000;j<=10000;j++)
		{
			fangwen(i,j)=max(fangwen(i-1,j),fangwen(i-1,j-c[i])+a[i]);
		}
	if(fangwen(n,0)==0)puts("-1");
	else printf("%d\n",fangwen(n,0));
	return 0;
}