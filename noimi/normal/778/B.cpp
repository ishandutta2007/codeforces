...
main() {
    ...
    struct state {
        int a[1100], b[1100];
        state(string x) { rep(i, x.size()) a[i] = b[i] = x[i] == '1'; }
        state() = default;
    };
    map<string, state> mp;
    state hatena;
    rep(i, m) hatena.a[i] = 1, hatena.b[i] = 0;
    mp["?"] = hatena;
    cin.ignore();
    rep(i, n) {
        string S;
        ...
        state x = mp[t] , y = mp[s];
        state res;
        if(op == 2) { rep(i, m) res.a[i] = (x.a[i] & y.a[i]), res.b[i] = (x.b[i] & y.b[i]); }
        if(op == 1) { rep(i, m) res.a[i] = (x.a[i] | y.a[i]), res.b[i] = (x.b[i] | y.b[i]); }
        if(op == 0) { rep(i, m) res.a[i] = (x.a[i] ^ y.a[i]), res.b[i] = (x.b[i] ^ y.b[i]); }
        rep(i, m) {
            a[i] += res.a[i];
            b[i] += res.b[i];
        }
