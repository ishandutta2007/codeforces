...
int main(){
	...
	TT{
		...
		cauto t=read();
		cauto s=read<string>();
		cauto c0=count(all(s),'0')*t/n;
		cauto c1=count(all(s),'1')*t/n;
		if(c0+c1!=t){
			cout<<-1 NL;
			continue;
		}
		vector<int> cnt1(n+1);
		rep(i,n) cnt1[i+1]=cnt1[i]+(s[i]=='1');
		int ok=0;
		reps(i,t,n+1)if(cnt1[i]-cnt1[i-t]==c1){
			cout<<1 NL;
			cout<<i-t+1<<" "<<i NL;
			ok=1;
			break;
		...
		if(ok) continue;
		cauto d1=count(all(s),'1')-c1;
		reps(i,n-t,n+1)if(cnt1[i]-cnt1[i-(n-t)]==d1){
			cout<<2 NL;
			cout<<"1 "<<i-(n-t) NL;
			cout<<i+1<<" "<<n NL;
			ok=1;
