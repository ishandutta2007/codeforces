...
fun main() {
	...
	val oneb = getlong()
	val onew = getlong()
	val ba = LongArray(n - 1)
	val wa = LongArray(n - 1)
	repeat(n - 1) {
		ba[it] = getlong()
		wa[it] = getlong()
	}
	val things = (ba zip wa).sortedByDescending { it.first }
	val pop = PriorityQueue<Long>()
	var rank = 1
	var pointer = 0
	for((a, b) in things) {
		if(a > oneb) {
...
//		put("Popping ${has}")
		...
		while(true) {
			if(pointer > things.lastIndex){
				break
			}
			val (a, b) = things[pointer]
			if(a > bleft) {
				pop.add((b + 1 - a))
//				put("$a,$b now better")
				rank += 1
