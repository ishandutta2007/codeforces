...
struct BIT{
    ...
    inline void add(int x, int y){for (;x <= n;x += lowbit(x)) ((c[x] += y) >= MOD) && (c[x] -= MOD);}
    inline T ask(int l, int r) {
        -- l; T ret = 0; 
        while(r > l) (((ret += c[r]) >= MOD) && (ret -= MOD)), r -= lowbit(r);
        while(l > r) (((ret -= c[l]) < 0) && (ret += MOD)), l -= lowbit(l);
        return ret;
    }
}; BIT < int > t[2];
signed main(){
    read(n); read(a + 1, a + n + 1); rep(i, 1, n) p[i] = i; std::sort(p + 1, p + n + 1, [](int x, int y){return a[x] < a[y];});
    int ans = 0; rep(i, 1, n) { const int x = p[i];ans = (1ll * ((1ll * t[0].ask(1, x) * (n - x + 1) + 1ll * t[1].ask(x, n) * x) % MOD) * a[x] + ans) % MOD; t[0].add(x, x); t[1].add(x, n - x + 1); ans = (1ll * (n - x + 1) * x % MOD * a[x] + ans) % MOD; }print(ans);
}