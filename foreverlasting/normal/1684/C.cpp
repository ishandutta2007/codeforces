...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        res n=read(),m=read();
        vector<int> D;
        for(res i=1;i<=n;i++)A[i].resize(m+1);
        D.resize(m+1);
        ...
        for(res i=1;i<=n;i++){
            ...
            sort(B.begin(),B.end());
            for(res j=1;j<=m;j++)if(A[i][j]!=B[j])D[j]++;
        }
        vector<int> C;
        for(res i=1;i<=m;i++)if(D[i])C.pb(i);
        if(C.size()>2){puts("-1");return;}
        if(C.empty()){puts("1 1");return;}
        res x=C[0],y=C[1];
        for(res i=1;i<=n;i++){
            vector<int> B;
            B.pb(0);
            ...
            sort(B.begin(),B.end());
            if(A[i][x]==B[y]&&A[i][y]==B[x])continue;
            puts("-1");return;
        }
        printf("%d %d\n",x,y);
    }
