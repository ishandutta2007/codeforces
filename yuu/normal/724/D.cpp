...
string t;
int s_cnt[26];
class compress_string_t {
public:
    int cnt[26];

    bool operator<(const compress_string_t& other) const { return get_weight() < other.get_weight(); }

    int get_weight() const {
        int weight = 0;
        int sum = 0;
        for (int i = 0; i < 26; i++) {
            if (cnt[i] > s_cnt[i]) return 1e9;
            if (cnt[i]) weight = max(weight, sum + cnt[i]);
            sum += s_cnt[i];
        }
        return weight;
    }

    void add_char(char c) { cnt[c]++; }

    friend ostream& operator<<(ostream& out, const compress_string_t& val) {
        int size = val.get_weight();
        // out << size << ' ';
        for (int i = 0; i < 26; i++) {
            if (size == 0) break;
...
deque<pair<int, compress_string_t>> q[26];
void solve() {
    cin >> m >> s;
    for (auto&& x : s) x -= 'a';
    for (auto&& x : s) s_cnt[x]++;

    int n = s.size();
    s = '0' + s;
    compress_string_t ans;
    for (int i = 0; i < 26; i++) ans.cnt[i] = 0;
    for (int i = 0; i < 26; i++) {
        ans.cnt[i] = 1;
        q[i].emplace_back(0, ans);
        ans.cnt[i] = 0;
    }
    compress_string_t final_ans;
    for (int i = 0; i < 26; i++) final_ans.cnt[i] = 0;
    for (int i = 1; i <= n; i++) final_ans.cnt[s[i]]++;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < 26; j++) {
            while (q[j].front().first < i - m) q[j].pop_front();
        }
        ans = q[s[i]].front().second;
        for (int j = 0; j < 26; j++) {
            ans.cnt[j]++;
            while ((!q[j].empty()) && (ans < q[j].back().second)) q[j].pop_back();
            q[j].emplace_back(i, ans);
            ans.cnt[j]--;
        }
        if (i + m > n) {
            // cerr << ans << ' ' << final_ans << '\n';
