...
namespace MAIN{
    int a[N][10];
    int solve(res l,res r){
        if(l==r)return l;
        res mid=(l+r)>>1;
        res p1=solve(l,mid),p2=solve(mid+1,r);
        if(p1==-1&&p2==-1)return -1;
        if(p1==-1)return p2;
        if(p2==-1)return p1;
        res sa=0,sb=0;
        for(res i=1;i<=5;i++)sa+=a[p1][i]<a[p2][i],sb+=a[p1][i]>a[p2][i];
        if(sa>=3)return p1;
        if(sb>=3)return p2;
        return -1;
    }
    inline void MAIN(){
        ...
        while(T--){
            res n=read();
            for(res i=1;i<=n;i++)for(res j=1;j<=5;j++)a[i][j]=read();
            res x=solve(1,n);
            if(x==-1)puts("-1");
            else {
                res fl=0;
