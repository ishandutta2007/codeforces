...
int main() {
	...
	cin.tie(nullptr);
	constexpr const u32 maxn = 1024U;
	std::vector<std::vector<u32>> f(maxn);
	for (u32 i = 1; i != maxn; ++i) {
		for (u32 j = i; j < maxn; j += i) {
			f[j].push_back(i);
		}
	}
	for (u32 i = 1; i != maxn; ++i) {
	...
	while (T--) {
		u32 n;
		std::cin >> n;
		std::vector<u32> c(maxn);
		std::vector<u32> have(maxn);
		u32 x = 0;
		for (u32 i = 0; i != n; ++i) {
			u32 a;
			std::cin >> a;
