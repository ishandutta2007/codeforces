...
{
    if(l>r)return INF;if(r-l+1>=n)return vl[n];int ls=(l-1)/n+1,rs=(r-1)/n+1;l-=(ls-1)*n;r-=(rs-1)*n;
    return (ls!=rs)?(min(vr[l],vl[r])):(min(f[len[r-l+1]][l],f[len[r-l+1]][r-(1<<len[r-l+1])+1]));
}
inline void Read()
{
    rin>>n>>q;for(int i=1;i<=n;i++)rin>>v[i],vl[i]=(i==1)?v[i]:min(vl[i-1],v[i]);for(int i=n;i>=1;i--)vr[i]=(i==n)?v[i]:min(vr[i+1],v[i]);
    for(int i=0;(1<<i)<=n;i++)for(int j=1;j+(1<<i)-1<=n;j++)f[i][j]=((!i)?(v[j]):(min(f[i-1][j],f[i-1][j+(1<<i-1)])));
    for(int i=2;i<=n;i++){len[i]=len[i-1];if(i>(1<<len[i]+1))len[i]++;}return;
}
...
int b[N<<1];
int c[N<<1];
inline int a(int x){return x<=q?L[x]:R[x-q];}
inline void A(int x){x<=q?(L[x]=m):(R[x-q]=m);return;}
inline bool myru_b(int x,int y){return a(x)<a(y);}
inline void Init()
{
    rin>>q;for(int i=1,typ;i<=q;i++){rin>>typ>>L[i]>>R[i];if(typ==1)rin>>k[i];b[i]=i;b[i+q]=i+q;}
    sort(b+1,b+(q<<1)+1,myru_b);for(int i=1,j;i<=(q<<1);i=j){c[++m]=a(b[i]);for(j=i;j<=(q<<1)&&a(b[j])==c[m];j++)A(b[j]);}return;
}
...
{
    gyq[i]=get(c[l],c[r]);if(l==r){mis[i]=INF;return;}
    int mid=l+r>>1;mis[i]=get(c[mid]+1,c[mid+1]-1);make_tree(l,mid,ls);make_tree(mid+1,r,rs);return;
}
int ql,qr,qk;
inline void addtag(int i,int x){gyq[i]=mis[i]=tag[i]=x;return;}
inline void up(int i){gyq[i]=min(mis[i],min(gyq[ls],gyq[rs]));return;}
inline void down(int i){if(tag[i])addtag(ls,tag[i]),addtag(rs,tag[i]),tag[i]=0;return;}
inline void cover(int l,int r,int i)
{
    if(ql<=l&&r<=qr){addtag(i,qk);return;}down(i);int mid=l+r>>1;mis[i]=((ql<=mid&&mid<qr)?qk:mis[i]);
    if(mid>=ql)cover(l,mid,ls);if(mid<qr)cover(mid+1,r,rs);up(i);return;
}
inline int check(int l,int r,int i)
{
    if(ql<=l&&r<=qr)return gyq[i];down(i);int mid=l+r>>1,ans=((ql<=mid&&mid<qr)?mis[i]:INF);
    if(mid>=ql)ans=min(ans,check(l,mid,ls));if(mid<qr)ans=min(ans,check(mid+1,r,rs));up(i);return ans;
}
inline void Work(){make_tree(1,m,1);for(int i=1;i<=q;i++){ql=L[i];qr=R[i];qk=k[i];if(!qk)printf("%d\n",check(1,m,1));else cover(1,m,1);}return;}
#undef ls
#undef rs
