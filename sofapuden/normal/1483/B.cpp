...
int uf(vi &ufa, int x, int st, vi &v){
	if(((st-1)+sz(ufa))%sz(ufa) == x)return ufa[x] = x;
	if(ufa[x] == x){
		if(__gcd(v[x],v[(x+1)%sz(ufa)]) != 1){
			return ufa[x] = uf(ufa,ufa[(x+1)%sz(ufa)],st,v);
		}
		return x;
	}
	return ufa[x] = uf(ufa,ufa[x],st,v);
}
...
void solve(){
	...
	while(1){
		...
		else if(nxt[x] == x)break;
		if(__gcd(v[x],v[nxt[x]]) == 1){
			out.pb(nxt[x]);
			nxt[x] = nxt[nxt[x]];
		}
		else{
			if(x == ufa[nxt[x]])break;
			ufa[x] = uf(ufa,nxt[x],x,v);
			if(ufa[x] == x)break;
		}
		x = ufa[nxt[x]];
	}
