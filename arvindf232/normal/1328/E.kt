...
//            subs[from].add(to)
    ...
    fun distance(a:Int,b:Int):Int {
        val L = LCA(a,b)
        return (depth[a] - depth[L]).absoluteValue + (depth[b] - depth[L]).absoluteValue
    }

}
inline fun Graph.ordered_dfs(root:Int,exit:(Int)->Unit,entry:(Int)->Unit){
    //Long: (from,to) i.e. (parent, child)
    val toDo = ArrayDeque<Int>()
    ...
    while(toDo.isNotEmpty()){
        ...
        entry(x)
        val iter = E[x]
        for(i in iter.lastIndex downTo 0){
            val a = iter[i]
            if(!explored[a]){
                explored[a] = true
                toDo.addFirst(a)
...
fun Graph.eulerPath():Pair<IntArray,IntArray>{
    val entryTime = IntArray(n)
    val exitTime = IntArray(n)
    val order = IntArray(n)

    var pointer = -1

    this.ordered_dfs(0,{exitTime[it] = pointer},{entryTime[it] = ++pointer;order[pointer] = it})
    val x = 0
    return Pair(entryTime,exitTime)
}
...
fun main(){
    solve.cases{
        ...
        G.setup()
        G.calQuickPA()
        val (tin,tout) = G.eulerPath()
        repeat(q)first@{
            val k = getint()
             val L = getline(k)
            for(i in L.indices) L[i] --

            val LS = L.sortedBy { tin[it]}
//            "x" dei LS
            val travel = mutableListOf<Int>()
            for(i in 0 until k){
                travel.add(G.depth[LS[i]])
                if(i != k-1){
                    travel.add(G.depth[G.LCA(LS[i],LS[i+1])])
                }
            }
//            just dei travel
            var maxdepth = 0
            var bestdanger:Int? = null
            for(a in travel){
                maxdepth = maxOf(maxdepth, a)
                if(maxdepth - a >= 2 ){
                    if(bestdanger == null) bestdanger = a
                    bestdanger = minOf(bestdanger, a)
                }
                if(bestdanger != null && a - bestdanger >= 2){
                    put("NO")
                    return@first
                }
            }
