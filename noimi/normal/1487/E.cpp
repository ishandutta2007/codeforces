...
int main() {
    ...
    vv(ll, dp, 4);
    rep(i, 4) dp[i].assign(n[i], inf<ll>);
    rep(i, n[0]) dp[0][i] = a[0][i];
    rep(i, 3) {
        multiset<ll> s;
        rep(j, n[i]) s.emplace(dp[i][j]);
        s.emplace(inf<ll>);
        INT(m);
        vv(int, no, n[i + 1]);
        while(m--) {
        ...
        rep(j, n[i + 1]) {
            for(auto e : no[j]) { s.erase(s.lower_bound(dp[i][e])); }
            dp[i + 1][j] = *begin(s) + a[i + 1][j];
            for(auto e : no[j]) s.emplace(dp[i][e]);
        }
    }
    ll ans = MIN(dp.back());
    if(ans >= inf<ll>) ans = -1;
