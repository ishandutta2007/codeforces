...
int main() {
	...
	for (int i = 0; i < n + m - 1; i++)
		d[i] = make_pair(c[i + 1].first - c[i].first, i);
	...
	auto con = [&](int idx) {
		idx = dsu.find(idx);
		int cnt = dsu.cnt[idx];
		return ps[idx] - (idx - cnt >= 0 ? ps[idx - cnt] : 0);
	};

	vector<ll> ans(q, -1);
	for (int i = 0; i < q; i++) {
		while (!d.empty() && d.back().first <= qry[i].first) {
			cur -= con(d.back().second);
			cur -= con(d.back().second + 1);
			dsu.uni(d.back().second + 1, d.back().second);
			cur += con(d.back().second + 1);
			d.pop_back();
		}
		ans[qry[i].second] = cur;
	}
