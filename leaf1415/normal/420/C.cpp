...
ll n, p;
vector<ll> G[300005];
vector<ll> vec[300005], vec2[300005], gvec;
...
int main(void)
{
	...
	rep(i, 1, n){
		...
		for(auto p : mp){
			vec[i].pb(sz(G[p.fi]));
			vec2[i].pb(sz(G[p.fi]) - p.se);
		}
		sort(all(vec[i])), sort(all(vec2[i]));
	}
	sort(all(gvec));
	...
	rep(i, 1, n){
		ll l = p - sz(G[i]);
		ans += gvec.end() - lower_bound(all(gvec), l);
		if(sz(G[i]) >= l) ans--;
		ans -= vec[i].end() - lower_bound(all(vec[i]), l);
		ans += vec2[i].end() - lower_bound(all(vec2[i]), l);
	}
	outl(ans/2);
