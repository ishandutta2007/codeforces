...
fn run<W: Write>(sc: &mut scanner::Scanner, out: &mut std::io::BufWriter<W>) {
    ...
    for _ in 0..t {
        ...
        let len = z.len();
        if len == 1 || z[len - 1].1.max(z[len - 2].1) > 1 {
            writeln!(out, "0").ok();
            continue;
        }
        ...
        //  a < b 
        // 
        let mut dp = vec![vec![M::one()]; 2];
        dp[1][0] = M::one();
        for (_, (_, c)) in z.into_iter().enumerate().rev().skip(2) {
            let mut next = vec![vec![M::zero(); dp[0].len() + 1]; 2];
            for (f, dp) in dp.iter().enumerate() {
                for (i, dp) in dp.iter().enumerate() {
                    // 
                    if f + i >= c - 1 {
                        next[1][i - (c - 1) + f] += *dp * pc.comb(i + f, c - 1);
                    }
                    // 
                    if f > 0 && i + f >= c {
                        next[0][i - (c - 1)] += *dp * pc.comb(i, c - 1);
                    }
                    if i >= c {
                        next[f][i - c] += *dp * pc.comb(i, c);
                    }
                }
            }
            dp = next;
        }
        writeln!(out, "{}", dp[1][0]).ok();
    }
}