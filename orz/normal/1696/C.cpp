	...
	for (int I = 0; I < t; ++I) {
		...
		while (!a.empty() && !b.empty()) {
			if (a.front().Y == 0)
				...
				b.pop_front();
			else if (a.front().X == b.front().X) {
				int y = min(a.front().Y, b.front().Y);
				a.front().Y -= y;
				b.front().Y -= y;
			}
				...
				if (b.front().X % m)
					...
					--b.front().Y;
					b.emplace_front(b.front().X / m, m);
				}
		}
		while (!a.empty() && a.front().Y == 0)
			a.pop_front();
		while (!b.empty() && b.front().Y == 0)
			b.pop_front();
		if (a.empty() && b.empty())
			cout << "Yes\n";
		else
