...
int main(){
	...
	rep(l,1,n) rep(i,1,n-l+1){
		int j=i+l-1; memset(g,200,sizeof(g)); g[i][1]=0;
		rep(x,i+1,j) rep(y,1,x-i+1){
			if (s[x]==s[i]) g[x][y]=g[x-1][y-1];
			for (int p=x-1; p>=i; p--)
				if (s[p]==s[i]) g[x][y]=max(g[x][y],g[p][y]+f[p+1][x]);
		}
		rep(x,1,n) f[i][j]=max(f[i][j],g[j][x]+a[x]);
	}
	cout<<f[1][n]<<endl;
}