...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	vv<int> facts(112345);
	for(cauto &p:primes)for(int i=p;i<facts.size();i+=p) facts[i].eb(p);
	cauto n=read();
	cauto q=read();
	cauto vs=readV(n);
	...
	rep(i,n+1) dbl[i][0]=n;
	{
		vector<int> nxt(112345,n);
		rrep(i,n){
			int mn=dbl[i+1][0];
			for(cauto p:facts[vs[i]]){
				MN(mn,nxt[p]);
				nxt[p]=i;
			}
			dbl[i][0]=mn;
	...
	rep(d,19)rep(i,n+1) dbl[i][d+1]=dbl[dbl[i][d]][d];
	// rep(i,n+1){rep(j,20) cout<<dbl[i][j]<<","; cout NL;}
	for(cauto &[l,r]:qs){
		int cur=l;
		int re=0;
		rrep(d,20)if(dbl[cur][d]<r){
			re|=1<<d;
			cur=dbl[cur][d];
		}
		cout<<re+1 NL;
