...
int mp[505][505];int qq;
bitset<505>dp1[505][505],dp2[505][505];
int ans[maxn<<1];
void solve(int l,int r,vector<node>v)
{
	if(l>r)assert(v.size()==0);
	if(v.size()==0)return;
	if(l>r)return;
	...
	//solve   [l,r] v
	//
	up(i,1,m)dp1[mid][i].reset(),dp2[mid][i].reset();
	up(i,1,m)if(mp[mid][i]==0)dp1[mid][i][i]=1,dp2[mid][i][i]=1;
	pu(i,m,1)if(mp[mid][i]==0)dp1[mid][i]|=dp1[mid][i+1];
	up(i,1,m)
	{
		if(mp[mid][i]==0)
		{
			dp2[mid][i]|=dp2[mid][i-1];
		}
	}
...
//	if(mid==3)cout<<dp1[2][3][3];edl;
	fp v pf
	{
		int x=to.x,y=to.y,xx=to.xx,yy=to.yy,id=to.id;
		if(x>mid)
		{
			v1.ep(to);
		}
...
//			cout<<id;edl;
			ans[id]=(dp1[x][y]&dp2[xx][yy]).count();
		}
	}
	
//	cout<<l<<" "<<r<<" "<<v1.size()<<" "<<v2.size();edl;
	solve(l,mid-1,v2);
	solve(mid+1,r,v1);
}
signed main()
{
	...
	while(T--)
	{
		...
		{
			q[i].id=i;
			rd(q[i].x);rd(q[i].y);rd(q[i].xx);rd(q[i].yy);
		}
		vector<node>v;
		up_(qq)
		{
