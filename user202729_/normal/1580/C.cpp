// time-limit: 1000
// problem-url: https://codeforces.com/contest/1580/problem/C
...
int main(){
	int numType; cin>>numType;
	int numDay; cin>>numDay;
	
	struct Type{int x, y;};
	std::vector<Type> types(numType);
	for(auto& [x, y]: types) cin>>x>>y;

	int const N=(int)2e5;
	int const LARGE=(int)std::sqrt(2*N);
	std::vector<std::vector<int>> modChanges(LARGE);
	for(int period=2; period<LARGE; ++period)
		modChanges[period].resize(period);
	
	std::vector<int> flatChanges(numDay);

	std::vector<int> startDay(numType, -1);

	int value {};

	for(int day=0; day<numDay; ++day){ // N*
		value+=flatChanges[day];
		for(int period=2; period<(int)modChanges.size(); ++period) // LARGE
			value+=modChanges[period][day%period];



		char operation;
		cin>>operation;
		cin>>operation; // NOTE fastio-only
		int type; cin>>type; --type;
		auto const [x, y]=types[type];
		auto const period=x+y;

		auto const process=[&](int start, int delta){
			if(period<LARGE){
				// 2
				modChanges[period][(start+x)%period]+=delta;
				modChanges[period][start%period]-=delta;
			}else{
				// N/LARGE*2
				int i=start+x;
				while(i<numDay){
		...
		if(operation=='1'){
			assert(startDay[type]==-1);
			process(startDay[type]=day, 1);;
		}else{
			assert(operation=='2');
			auto const start=startDay[type];
			assert(start>=0);
			startDay[type]=-1;

			if((day-start)%period>=x)
				--value;
			...
			if(period<LARGE){
				...
				// N/LARGE*2, optimized a bit
				int i=start+x;
				if(i<day-period){ // avoid overflow
					i+=(day-i)/period*period;
				}
				while(i<numDay){
					flatChanges[i]+=delta;
					i+=y;
