...
fn dfs(u: usize, par: usize, a: &[i64], e: &[Vec<usize>], mut g: Vec<(i64,i32)>, ans: &mut Z) {
	let mut gg = vec![(a[u],1)];
	while let Some( (x,k) ) = g.pop() {
		let t = gcd(x, a[u]);
		if t == x {
			g.push( (x,k) );
			break;
	...
	while let Some( (x,n) ) = gg.pop() {
		if let Some( (y,m) ) = g.pop() {
			if x == y {
				g.push( (x, n+m) );
				continue;
			} else {
