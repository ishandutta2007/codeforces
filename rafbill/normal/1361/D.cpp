...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) {
    li x,y; cin>>x>>y;
    if(make_tuple(x,y) != make_tuple(0,0)) {
      li g = __gcd(abs(x), abs(y));
      x /= g;
      y /= g;
      M[make_tuple(x,y)].push_back(g);
    }
  }
  for(auto& p : M) { p.second.push_back(0); break; }
  vector<double> A;
  for(auto& p : M) {
    auto& v = p.second;
    pdqsort(begin(v), end(v));
    ...
    for(li i = (m-1); i >= (li)(m-f); --i) {
      inn += s0 - cnt0 * v[i];
      cnt0 += 1;
      s0 += v[i];
      R.emplace_back(inn + (s0 + s1) * (k - cnt0 - cnt1));
    }

    for(li i = (0); i <= (li)(m-f-1); ++i) {
      inn += cnt1 * v[i] - s1;
      inn += s0 - cnt0 * v[i];
      cnt1 += 1;
      s1 += v[i];
    ...
    int nr = R.size();
    for(li i = (nr-2); i >= (li)(0); --i) R[i+1] -= R[i];
    for(li i = 0; i < (li)(nr); ++i) A.emplace_back(d * (double)R[i]);
  }
  pdqsort(begin(A), end(A), greater<double>());
  double ans = 0;
  for(li i = 0; i < (li)(k); ++i) ans += A[i];
