...
public class Main
{
	public static void main(String[] args)
	{
		...
		for(int i=0; i<n; i++)
			hol.add(ns(),0);
		hol.traverse();
		if((hol.win&&k%2!=0)||(hol.win&&hol.lose))
			pr("First");
		else
			pr("Second");
	...
	static class node
	{
		node[]chi=new node[26];
		boolean win=false,lose=false;
		void add(String a, int i)
		{
			if(i<a.length())
			{
				if(chi[a.charAt(i)-'a']==null)
					chi[a.charAt(i)-'a']=new node();
				chi[a.charAt(i)-'a'].add(a, i+1);
			}
		}
		void traverse()
		{
			boolean something=false,lost=false,won=false;
			for(int i=0; i<26; i++)
				if(chi[i]!=null)
					chi[i].traverse();
			for(int i=0; i<26; i++)
