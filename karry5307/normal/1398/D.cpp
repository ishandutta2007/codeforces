...
ll r,g,b,res;
ll x[MAXN],y[MAXN],z[MAXN],f[MAXN][MAXN][MAXN];
// Fast IO
inline ll read()
{
...
int main()
{
	...
	for(register int i=0;i<=r;i++)
	{
		for(register int j=0;j<=g;j++)
		{
			...
			{
				chkmax(f[i][j+1][k+1],f[i][j][k]+y[j+1]*z[k+1]);
				chkmax(f[i+1][j+1][k],f[i][j][k]+y[j+1]*x[i+1]);
				chkmax(f[i+1][j][k+1],f[i][j][k]+x[i+1]*z[k+1]);
				chkmax(res,f[i][j][k]);
			}
		}
	}
