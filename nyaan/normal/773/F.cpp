...
ll N, M, q;
unordered_map<ll, fps> mf, mpw;
fps base;

fps pw(ll a) {
  if (a == 0) return fps{1};
  if (a == 1) return base;
  if (mpw.find(a) != mpw.end()) return mpw[a];
  fps res = (pw(a / 2) * pw(a / 2)).pre(N);
  if (a & 1) res = (res * base).pre(N);
  return mpw[a] = res;
}
// [1, u]
fps f(ll u) {
  if (u == 0) return fps{};
  if (u == 1) return base.pre(N);
  if (mf.find(u) != mf.end()) return mf[u];
  ll v = u / 2;
  fps g = f(v);
  g += (g * pw(v)).pre(N);
  if (u & 1) g += pw(u);
