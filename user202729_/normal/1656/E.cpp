...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1656/problem/E
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	 ...
	 Graph<> g; g.readUndirected(number, number-1);
	 std::vector<char> color(number);
	 auto const work=makeYComb([&](auto work, int node, int par)->void{
		 for(auto [child, _]: g[node]) if(child!=par){
