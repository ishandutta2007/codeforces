...
template<class T,class E> ostream& operator<<(ostream& os,const set<T,E>& v){
  os<<"{";bool f=false;
  for(auto& x:v){if(f)os<<",";os<<x;f=true;}
  os<<"}";
  return os;
...
int codeforces;cin>>codeforces;while(codeforces--){
  ll i,j;
  cin>>N>>K;
  auto cmp=[](datas l,datas r){return l.first!=r.first?l.first>r.first:l.second>r.second;};
  set<datas,decltype(cmp)> se(cmp);
  A=inf;
  vec v(N);
  rep(i,N){
    cin>>v[i];
    if(chmin(A,v[i]))se.emplace(v[i],i);
  }
  // debug(se);
  vec ans(K);
  rep(i,K){
    ...
    cin>>j>>x;
    --j;
    se.erase(datas(v[j],j));
    v[j]-=x;
    auto itr=se.emplace(v[j],j).first;
    if(next(itr)!=se.end()&&next(itr)->second<j){
      se.erase(itr);
    }else{
      while(itr!=se.begin()){
        auto sitr=prev(itr);
        if(sitr->second>itr->second)itr=se.erase(sitr);
        else break;
      }
    }
    // debug(se);
