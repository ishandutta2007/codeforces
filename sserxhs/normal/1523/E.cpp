...
char s[N];
int a[N],b[N],fac[N],ifac[N],f[N];
int T,n,c,i,j,k,x,y,z,ans,la,ksiz,ks,bs;
...
int main()
{
	...
	{
		read(n,k);ans=0;
		bs=ksm(C(n,k),p-2);f[0]=1;
		for (i=1;(i-1)*(k-1)<=n;i++) f[i]=(ll)C(n-(i-1)*(k-1),i)*ksm(C(n,i),p-2)%p;
		f[i]=0;
		//write(f,n);
		for (i=1;(i-2)*(k-1)<=n;i++) ans=(ans+(ll)i*(f[i-1]+p-f[i]))%p;
		enter(ans);
	}
}