...
int main() {
    ...
    segment_tree_lazy st(N, node{INT_MAX/2,0,0,0,0}, 0, nn, plus<int>(), un);

    const int MAXV = 2e5 + 1;
    vector<set<int>> occ(MAXV);
    F0R (i, N) occ[a[i]].insert(i);
    vi diffs(N);
    F0R (v, MAXV) if (!occ[v].empty()) {
        diffs[*occ[v].begin()]++;
        diffs[*occ[v].rbegin()]--;
    }
    vi sums = prefixes(diffs, 0, plus<int>());
    auto leaf = [&](int i, int sum = INT_MIN) {
        int wt = *occ[a[i]].begin() == i ? sz(occ[a[i]]) : 0;
        return node{sum != INT_MIN ? sum : sums[i+1],
                    wt, 0, wt,
                    0};
    };
    st.set_leaves(leaf);
    ...
    F0R (q, Q) {
        int i, nv; re(i, nv); int cv = a[--i];
        if (nv != cv) {
            a[i] = nv;

            vi rep{i, *occ[cv].begin()};
            if (!occ[nv].empty()) rep.pb(*occ[nv].begin());

            if (sz(occ[cv]) > 1) {
                st(*occ[cv].begin(), *occ[cv].rbegin(), -1);
            }
            occ[cv].erase(i);
            if (sz(occ[cv]) > 1) {
                st(*occ[cv].begin(), *occ[cv].rbegin(), +1);
            ...
            if (sz(occ[nv]) > 1) {
                st(*occ[nv].begin(), *occ[nv].rbegin(), -1);
            }
            occ[nv].insert(i);
            if (sz(occ[nv]) > 1) {
                st(*occ[nv].begin(), *occ[nv].rbegin(), +1);
            ...
            rep.pb(*occ[nv].begin());
            if (!occ[cv].empty()) rep.pb(*occ[cv].begin());

            dedup(rep);
            trav (j, rep) st.replace(j, leaf(j, st[j].lo));
        }
