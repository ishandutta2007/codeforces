...
int mem[MAXN][MAXN][2];
#define NOW mem[l][r][side]
#define KNOW NOW != -1
bool Dfs( int l , int r , bool side ){
	//cout << l << " " << r << endl;
	if(KNOW) return NOW;
	if( l >= r ) return NOW = 1;
	int rt = side?r:l;
	...
	int nr = side?(r-1):r;
	Forx(i,l,r){
		if( !e[i][rt] || i == rt ) continue;
		//if( i-1 >= l  &&  !e[i-1][rt] ) continue;
		//if( i+1 <= r  &&  !e[i+1][rt] ) continue;
		bool tans = Dfs(nl,i,1) & Dfs(i,nr,0);
		if(tans) return NOW = 1;
	}
	return NOW = 0;
}
