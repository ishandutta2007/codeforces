...
main() {
    ...
    LazySegmentTree<int, int> seg(
        n + n + 10, [](int x, int y) { return max(x, y); }, plus<int>(), plus<int>(), 0, 0);
    set<pii> s;
    vi cnt(n + n + 1);
    while(m--) {
        INT(x, y);
        if(s.count(pii(x, y))) {
            ...
            y += abs(x - k);
            cnt[y]--;
            if(cnt[y] == 0) { seg.update(y, y + 1, -(y - 1)); }
            seg.update(1, y + 1, -1);
        } else {
            s.emplace(x, y);
            y += abs(x - k);
            cnt[y]++;
            if(cnt[y] == 1) seg.update(y, y + 1, y - 1);
            seg.update(1, y + 1, 1);
        }
        cout << max<ll>(0, seg.query(0, n + n + 1) - n) << "\n";
    }
}