...
// f_0(n) = 2^(n )
// f_{r + 1}(n) = sum_{u * v = n} (f_r(u) + f_r(v)) / 2
//
// 
...
fn run() {
    ...
    let n = 1000000;
    let pc = Precalc::new(2 * n);
    let mut factor = (0..=n).collect::<Vec<_>>();
    let mut ini = vec![M::one(); n + 1];
    enumerate_prime(n, |p| {
        for i in 1..=(n / p) {
            factor[i * p] = p;
            ini[i * p] *= M::new(2);
    ...
    let divisor = |mut n: usize, res: &mut Vec<usize>| {
        res.clear();
        res.push(1);
        while n > 1 {
            let p = factor[n];
        ...
        let ans = if r == 0 {
            ...
            while m > 1 {
                ...
                while m % p == 0 {
                    m /= p;
                    c += 1;
                    for i in (d.len() - len)..d.len() {
                        let q = d[i];
                        let v = q.1 * M::from(r - 1 + c) * pc.inv(c);
                        d.push((q.0 * p, v));
                    }
                }
            }
