...
int main() {
    ...
    vector<int> d(n);
    vector<string> t(n);
    rep(i, n) d[i] = t0[i][0] - '0', t[i] = t0[i].substr(3, sz(t0[i]) - 3);
    vector<mint> val(10);
    iota(all(val), 0);
    vector<int> len(10, 1);
    per(i, n) {
        mint nv = 0;
        int nl = 0;
        rep(j, sz(t[i])) nv *= mpow(10, len[t[i][j] - '0']), nv += val[t[i][j] - '0'], nl += len[t[i][j] - '0'], nl %= MOD - 1;
        val[d[i]] = nv, len[d[i]] = nl;
    }
    mint ans = 0;
    rep(i, sz(s)) ans *= mpow(10, len[s[i] - '0']), ans += val[s[i] - '0'];
    cout << ans << endl;
}