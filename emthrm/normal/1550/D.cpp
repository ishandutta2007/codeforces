...
void solve() {
  int n, l, r; cin >> n >> l >> r;
  int fr = min(1 - l, r - n);
  ModInt ans = ModInt::nCk(n, n / 2) * fr * (n % 2 == 0 ? 1 : 2);
  l += fr;
  r -= fr;
  while (true) {
    ++l;
    ...
    int right = min(r, n);
    if (n % 2 == 0) {
      if (left - 1 > n / 2 || n - r > n / 2) break;
      ans += ModInt::nCk(right - left + 1, n / 2 - (left - 1));
    } else {
      int flag = 0;
      if (left - 1 > n / 2 || n - r > (n + 1) / 2) {
        flag |= 1;
      } else {
        ans += ModInt::nCk(right - left + 1, n / 2 - (left - 1));
      }
      if (left - 1 > (n + 1) / 2 || n - r > n / 2) {
        flag |= 2;
      } else {
        ans += ModInt::nCk(right - left + 1, (n + 1) / 2 - (left - 1));
      }
      if (flag == 3) break;
    }
  }
