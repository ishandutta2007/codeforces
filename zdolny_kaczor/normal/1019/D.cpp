...
LL s;
int cou = 0;
LL pole(int i, int j, int k) {
	int a = points[i].f - points[j].f;
	int b = points[i].s - points[j].s;
	int c = points[i].f - points[k].f;
	int d = points[i].s - points[k].s;
	return - a * 1ll * d + b * 1ll * c;
}
void find_val(int i, int j, int start, int end, LL value) {
	debug(arr(points, i), arr(points, j), imie(value));
	while (start <= end) {
		int med = (start + end) / 2;
		LL p = pole(i, j, med);
		debug(imie(med), imie(p));
		if (p == value) {
			printf("Yes\n%d %d\n%d %d\n%d %d\n", points[i].f, points[i].s, points[j].f, points[j].s, points[med].f, points[med].s);
			exit(0);
		}
		if (p < value)
			start = med + 1;
...
void find(int p1, int p2) {
	find_val(p1, p2, 0, p1 - 1, -s * 2);
	find_val(p1, p2, p2 + 1, n - 1, s * 2);
}
void do_swap(int i, int j) {
	debug(arr(points, arr(where, i)), arr(points, arr(where, j)));
	swap(points[where[i]], points[where[j]]);
	...
	if (x > y)
		swap(x, y);
	find(x, y);
	debug(range(points, 0, n - 1));
}
void fix(PII &v1) {
	if (MP(v1.s, v1.f) < MP(0, 0))
		v1 = -v1;
}
bool cmp(PII a, PII b) {
	PII v1 = points[a.f] - points[a.s];
	PII v2 = points[b.f] - points[b.s];
	fix(v1);
	fix(v2);
	return v1.f * 1ll * v2.s > v1.s * 1ll * v2.f;
}
int main() {
	#ifdef DEB
	atexit(PdAYB7V);
	...
	REP(i, n)
		where[i] = i;
	sort(swaps, swaps + cou, cmp);
	REP(i, cou)
		do_swap(swaps[i].f, swaps[i].s);
	printf("No\n");
}