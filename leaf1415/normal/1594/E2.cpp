...
void dfs(int v, int d)
{
	...
	if(d == 0){
		rep(i, 0, 5){
			if(vec[v].second != 0 && (vec[v].second-1 != i || vec[v].second-1+i == 5)) continue;
			dp[v][i] = 1;
		}
		return;
	}
	...
	mint tmp[2][6];
	rep(i, 0, 1) rep(j, 0, 5) tmp[i][j] = modpow(4, (beki[d]+mod-1-2) % (mod-1));
	reps(i, G[v]) rep(j, 0, 5) tmp[i][j] = dp[G[v][i]][j];
	...
	rep(i, 0, 5){
		...
		rep(j, 0, 5){
			rep(k, 0, 5){
				if(i == j || i == k || i+j == 5 || i+k == 5) continue;
				dp[v][i] += tmp[0][j] * tmp[1][k];
			}
		}
	}
}
...
int main(void)
{
	...
	beki[0] = 1;
	rep(i, 1, 100) beki[i] = beki[i-1] * 2 % (mod-1);
	...
	reps(i, vec){
		ll v = vec[i].first;
		if(mp.count(v*2)){
			ll u = lower_bound(all(vec), P(v*2, -inf)) - vec.begin();
			G[i].push_back(u);
		}
		if(mp.count(v*2+1)){
			ll u = lower_bound(all(vec), P(v*2+1, -inf)) - vec.begin();
			G[i].push_back(u);
		}
	}

	//reps(i, vec) outl(i, ":", G[i]);

	dfs(0, k-1);

	//reps(i, vec) outa(dp[i], 0, 5);

	mint ans = 0;
	rep(i, 0, 5) ans += dp[0][i];
	outl(ans);
