...
int dist(string a, string b) {
	...
	REP(i, n + 1) REP(j, m + 1) {
		...
		dp[i][j] = inf;
		if (i) mini(dp[i][j], dp[i - 1][j] + 1);
		if (j) mini(dp[i][j], dp[i][j - 1] + 1);
		if (i && j) mini(dp[i][j], dp[i - 1][j - 1] + 1);
		if (i && j && a[i - 1] == b[j - 1]) mini(dp[i][j], dp[i - 1][j - 1]);
	}
	return dp[n][m];
}
int query(string s) {
	...
	int cou = 0;
	cou++;
	assert(cou <= (int) ans.size() + 2);
	if (s == ans) {
		debug("done");
		exit(0);
	}
	int d = dist(s, ans);
	debug(imie(s), imie(ans), imie(d));
	return d;
#else
	printf("%s\n", s.c_str());
	fflush(stdout);
...
int main() {
...
#endif
	const int N = 300;
	int a = query(string(N, 'a')), b = query(string(N, 'b'));
	int len = N - a + N - b;
	loc(assert(ans.size() == len));
	int a_cou = N - a;
#ifdef LOCAL
	int real_acou = 0;
	for (char c : ans) if (c == 'a') real_acou++;
	assert(real_acou == a_cou);
#endif
	string init;
	char add = -1;
	if (a_cou * 2 < len) {
		init = string(len - a_cou, 'b');
		add = 'a';
	}
	else {
		init = string(a_cou, 'a');
	...
	int pos = init.size();
	while (init.size() < (int) len) {
		string q = init;
		q.insert(q.begin() + pos, add);
		if (query(q) == len - q.size())
			init = q;
		else
			pos--;
	}
