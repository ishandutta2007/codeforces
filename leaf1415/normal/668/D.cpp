...
struct BIT{
	...
	void init(){
		for(int i = 1; i <= size; i++) bit[i] = Ident();
	}
	T query(int i){
		T ret = 0;
		while(i > 0){
...
int main(void)
{
	...
	uniq(vec);
	rep(i, 1, n) t[i] = lower_bound(all(vec), t[i]) - vec.begin() + 1;
	rep(i, 1, n) mp[x[i]].push_back(Q(P(a[i], t[i]), i));
	
	for(auto p : mp){
		vector<Q> &qvec = p.second;
		for(auto q : qvec){
			ll type = q.first.first, tm = q.first.second, qid = q.second;
			if(type == 1) bit.add(tm, 1);
			if(type == 2) bit.add(tm, -1);
			if(type == 3) ans[qid] = bit.query(tm);
		}
		for(auto q : qvec){
			ll type = q.first.first, tm = q.first.second, qid = q.second;
			if(type == 1) bit.add(tm, -1);
