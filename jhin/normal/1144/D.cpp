...
for(int i=0;i<n;i++)
{
    cin>>a[i];b[a[i]]++;if(b[a[i]]>mx)mx=b[a[i]],num=a[i];
}
while(a[x]!=num)x++;
for(int i=0;i<n;i++)if(a[i]!=num)y++;
cout<<y<<endl;z=x;
for(int i=x-1;i>=0;i--)
{
    if(a[i]>num)cout<<2<<" "<<i+1<<" "<<i+2<<endl;
    else if(a[i]<num)cout<<1<<" "<<i+1<<" "<<i+2<<endl;
    else x=i;
}
x=z;
for(int i=x+1;i<n;i++)
{
    if(a[i]>num)cout<<2<<" "<<i+1<<" "<<i<<endl;
    else if(a[i]<num)cout<<1<<" "<<i+1<<" "<<i<<endl;
