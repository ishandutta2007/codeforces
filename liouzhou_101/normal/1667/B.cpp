...
void solve()
{
	...
	vector<ValueType> a(m, ValueType{ -INF });
	
	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		treep(a.begin(), a.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);
	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		treem(a.begin(), a.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);
	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		treez(a.begin(), a.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);

	vector<int> dp(n + 1, -INF);
	auto setdp = [&](int x, int y)
	{
		dp[x] = y;
		int i = lower_bound(p.begin(), p.end(), s[x]) - p.begin();
		treep.ModifyValue(i, { max(treep.Query(i).val, dp[x] + x) });
		treem.ModifyValue(i, { max(treem.Query(i).val, dp[x] - x) });
		treez.ModifyValue(i, { max(treez.Query(i).val, dp[x]) });
	};

	setdp(0, 0);
	for (int x = 1; x <= n; ++x)
	{
		int pos = lower_bound(p.begin(), p.end(), s[x]) - p.begin();
		int tmp = -INF;
		if (0 <= pos - 1)
		{
			freshmax(tmp, treem.Query(0, pos).val + x);
		}
		if (pos + 1 <= m - 1)
		{
			freshmax(tmp, treep.Query(pos + 1, m).val - x);
		}
		freshmax(tmp, treez.Query(pos).val);
		setdp(x, tmp);
	}
