...
void dfs(int u, int b) {
    ...
    components.back()[b].emplace_back(u);
    if (done[u][1 - b]) {
        cout << "NO\n";
        exit(0);
    }
    for (auto&& v : g[u])
        if (!done[v][1 - b]) dfs(v, 1 - b);
}
bool dp[5001][5001];
bool trace[5001][5001];
int ans[5001];
void choose(const vector<int>& even, const vector<int>& odd) {
    for (auto&& x : even) {
        ans[x] = 2;
        c[2]--;
    }
...
void backtrace(int u, int v) {
    if (u == 0) {
        assert(v == 0);
        return;
    }
    choose(components[u - 1][trace[u][v]], components[u - 1][!trace[u][v]]);
    backtrace(u - 1, v - components[u - 1][trace[u][v]].size());
}
void solve() {
    cin >> n >> m;
    cin >> c[1] >> c[2] >> c[3];
    ...
    for (int i = 1; i <= n; i++)
        if ((!done[i][0]) && (!done[i][1])) {
            ...
            components.back()[0].clear();
            components.back()[1].clear();
            dfs(i, 0);
        }
    dp[0][0] = 1;
    for (int i = 0; i < components.size(); i++) {
        ...
        for (int j = 0; j < n; j++) {
            if (dp[i][j]) {
                if (!dp[i + 1][j + f[0].size()]) {
                    dp[i + 1][j + f[0].size()] = 1;
                    trace[i + 1][j + f[0].size()] = 0;
                }
                if (!dp[i + 1][j + f[1].size()]) {
                    dp[i + 1][j + f[1].size()] = 1;
                    trace[i + 1][j + f[1].size()] = 1;
                }
            }
        }
    }
    if (!dp[components.size()][c[2]]) {
        cout << "NO\n";
        return;
    }
    cout << "YES\n";
    backtrace(components.size(), c[2]);
    assert(c[1] == 0);
    assert(c[2] == 0);
    assert(c[3] == 0);
