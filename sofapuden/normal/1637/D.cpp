...
bool fin(int x, vector<int> &dif, int tar, vector<vector<bool>> &dp, vector<int> &tak) {
	if(x >= (int)dif.size() || tar < 0)return 0;
	if(dp[x][tar])return 0;
	dp[x][tar] = 1;
	if(abs(tar-dif[x]) < abs(bes))bes = tar-dif[x];
	if(bes == 0){
		tak[x] = 1;
		return 1;
	}
	bool a = fin(x+1,dif,tar,dp,tak);
	if(a)return 1;
	bool b = fin(x+1,dif,tar-dif[x],dp,tak);
	if(b){
		tak[x] = 1;
		return 1;
	}
...
void solve(){
	...
	vi dif;
	for(int i = 0; i < n; ++i){
		dif.push_back(abs(A[i]-B[i]));
	}
	vector<vector<bool>> dp(n,vector<bool>(105*n,0));
	vector<int> tak(n,0);
	int tar = accumulate(all(dif),0)/2;
	fin(0,dif,tar,dp,tak);
	if(bes){
		fin(0,dif,tar-bes,dp,tak);
	}
	for(int i = 0; i < n; ++i){
		if(tak[i]){
