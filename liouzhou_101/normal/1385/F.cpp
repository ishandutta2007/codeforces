...
void solve()
{
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			int j = *v[i].begin();
			H.erase({ leaf[j].size(), j });
			leaf[j].insert(i);
			H.insert({ leaf[j].size(), j });
		}
	}
	int res = 0;
	...
	while (!H.empty())
	{
		...
		if (d[x] == 1)
		{
			for (auto y : v[x])
			{
				if (alive.count(y))
				{
					H.erase({ leaf[y].size(), y });
					leaf[y].insert(x);
					H.insert({ leaf[y].size(), y });
				}
			}
		}
		else
		{
			H.insert({ leaf[x].size(),x });
		}
	}
	writeln(res);
}
