...
int main() {
	...
	auto check = [&]() {
		...
		for (int i = 0; i < n; i++) {
			Event.push_back({ a[i].fi, (i + SHIFT) });
			Event.push_back({ a[i].se + 1, -(i + SHIFT) });
		}
		...
		for (auto& e : Event) {
			...
			if (now > 0) {
				...
				st.erase(st.find(b[idx].fi));
				en.erase(en.find(b[idx].se));
			}

			if ((int)st.size() >= 2 && *st.rbegin() > *en.begin())
				is = false;
		}
