...
void solve() {
    ...
    REP(i, inputs.size()) {
        REP(q, 3) {
            if (inputs[0] / taps[q] == inputs[i]) {
                hoge++;
            }
        }
    }
    if (hoge == 3) {
        ans = max(ans, (31LL * inputs[0]) / 30LL);
    }
    for (int q = 1; q < inputs.size(); ++q) {
        if (inputs[0] % inputs[q] == 0) continue;
        ans = max(ans, inputs[0] + inputs[q]);
        for (int j = q + 1; j < inputs.size(); ++j) {
            if (inputs[0] % inputs[j] == 0 || inputs[q] % inputs[j] == 0) continue;
            ans = max(ans, inputs[0] + inputs[q] + inputs[j]);
            break;
        }
        break;
    }
