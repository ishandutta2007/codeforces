...
void solve() {
	...
	auto g = [&](ll a, ll b, int l, int r) {return a + b; };
	...
	SegT<ll, ll> st(ori, INF, 0, f, g, h);
	vector<ll> val(n);
	rep(i, n) {
		for (auto p : G[i]) {
			st.add(p.second, 0, p.first + 1);
		}
		val[i] = st.query(0, n);
	}
	multiset<ll> valst;
	...
	rep(i, q) {
		...
		valst.erase(valst.find(val[id] + a[id]));
		a[id] = w;
		valst.insert(val[id] + a[id]);
		cout << getans() << "\n";
	}
}
