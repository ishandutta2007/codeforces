...
struct RollingHash {
  ...
  pair<uint64_t, uint64_t> get(int l, int r) { return {get0(l, r), get1(l, r)};}
...
int main() {
    ...
    cin >> N >> Q;
    cin >> A >> T;
    while(T.size() < 2e5) T.push_back('#');
    for(int i = 0; i < Q; i++) cin >> k[i] >> w[i];
    for(int t = 0; ; t++) {
      if(S.size() > L) {
          timer = t - 1;
          break;
      ...
      vector<ll> v;
      for(int i = 0; i < Q; i++) {
          if(t == k[i]) v.push_back(i);
      }
      if(v.empty()) continue;
      SuffixArray SA(S);
      for(auto i : v) {
          auto tmp = SA.lower_upper_bound(w[i]);
    ...
    //cerr << S << endl;
    for(int q = 0; q < Q; q++) {
        if(k[q] <= timer) continue;
        auto tmp = SA.lower_upper_bound(w[q]);
        ini[q] = tmp.second - tmp.first;
        query[k[q]].push_back(q);
        RollingHash rhs(w[q].begin(), w[q].end());
        for(int i = 0; i < w[q].size(); i++) {
          if(i > S.size()) continue;
          if(w[q].size() - 1 - i > S.size()) continue;
            if(RHS.get(S.size() - i, S.size()) != rhs.get(0, i)) continue;
            ll l = w[q].size() - 1 - i;
            if(RHS.get(0, l) != rhs.get(w[q].size() - l, w[q].size())) continue;
            occ[q][(int)(w[q][i] - 'a')]++;
        }
        /*
        cerr << "---" << q << "---" << endl;
        for(int i = 0; i < 26; i++) cerr << occ[q][i].val() << " ";
        cerr << endl;
        */
    }
    for(int t = timer + 1; t <= N; t++) {
      for(int i = 0; i < 26; i++) {
          factor[i] *= 2;
      }
      factor[(int)(T[t-1] - 'a')]++;
      for(auto q : query[t]) {
        //cerr << t << " " << q << endl;
        ans[q] = ini[q] * (mint(2)).pow(t - timer);
        for(int i = 0; i < 26; i++) {
          ans[q] += factor[i] * occ[q][i];
        }
      }
    }
    for(int i = 0; i < Q; i++) {
