    ...
    TEST {
        ...
        if(si(v) == 0) {
            ...
            rep(i, n) {
                if(i == s) continue;
                if(!used[a[i] - 1])
                    ans[i] = a[i], used[a[i] - 1] = 1, res++;
                else
                    ans[i] = v.back() + 1, v.pop_back();
            }
            ...
            rep(i, si(w)) {
                int k = lb(v, w[i]);
                if(k < si(v) and v[k] == w[i]) { to[k] = w[i]; }
            }
            int idx = 0;
            rep(i, si(w)) {
                int k = lb(v, w[i]);
                if(!(k < si(v) and v[k] == w[i])) {
                    while(to[idx] != -1) idx++;
                    to[idx] = w[i];
                }
            }
            rotate(begin(to), begin(to) + 1, end(to));
            rep(i, si(w)) ans[to[i]] = v[i] + 1;
        }
        OUT(res);
        OUT(ans);
    }
