...
public:
    ...
    segment_tree_t(int l, int r) : l(l), r(r), m((l + r) / 2), lazy(0), mx(0) {
        if (l == r) return;
    ...
    void up() { mx = max(left->mx, right->mx); }

    void update_sum(int u, int v, int d) {
        if (l > v || r < u) return;
        if (u <= l && v >= r) {
            lazy += d;
            mx += d;
            ...
            down();
            left->update_sum(u, v, d);
            right->update_sum(u, v, d);
            up();
        }
    }

    void update_max(int u, int val) {
        if (l == r) {
            mx = max(mx, val);
        } else {
            down();
            if (m >= u) left->update_max(u, val);
            else right->update_max(u, val);
            up();
        }
    }
...
segment_tree_t* tree[3];
void solve() {
    cin >> n;
    for (int i = 1; i <= n; i++) {
        cin >> s[i].l >> s[i].r >> s[i].t;
        px.emplace_back(s[i].l);
        px.emplace_back(s[i].r);
    }
    px | UNIQUE;
    for (int i = 1; i <= n; i++) {
        s[i].l = lower_bound(px.begin(), px.end(), s[i].l) - px.begin() + 1;
        s[i].r = lower_bound(px.begin(), px.end(), s[i].r) - px.begin() + 1;
    }
    int mx = px.size();
    sort(s + 1, s + n + 1, [](const segment_t& a, const segment_t& b) { return a.l < b.l; });
    tree[1] = new segment_tree_t(1, mx);
    tree[2] = new segment_tree_t(1, mx);
    for (int i = 1; i <= n; i++) {
        auto&& [l, r, t] = s[i];
        // switch from 3 - t to t
        int f = tree[3 - t]->get_max(1, l - 1);
        // use something existing from t, ending at r
        f = max(f, tree[t]->get_max(1, r));
        f++;
        tree[t]->update_max(r, f);
        // ending after r
        tree[t]->update_sum(r + 1, mx, 1);
    }
    cout << max(tree[1]->get_max(1, mx), tree[2]->get_max(1, mx)) << '\n';
}
