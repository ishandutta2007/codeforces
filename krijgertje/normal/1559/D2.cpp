...
void solve() {
	...
	FOR(at, 1, n) {
		...
		while (adiff < n && ufa.find(adiff) == ufa.find(0)) ++adiff;
		while (bdiff < n && ufb.find(bdiff) == ufb.find(0)) ++bdiff;

		if (ufb.find(0) != ufb.find(at)) {
			ans.PB(MP(at, 0));
			assert(ufa.unite(at, 0));
			assert(ufb.unite(at, 0));
		} else if (bdiff < at) {
			assert(ufb.find(bdiff) != ufb.find(at));
			ans.PB(MP(at, bdiff));
			assert(ufa.unite(at, bdiff));
			assert(ufb.unite(at, bdiff));
		} else {
			swap(ufa, ufb);
			swap(adiff, bdiff);
		}
