...
struct segtree{
	int col[800005],chg[800005];
	ll sum[800005],wp[800005];
	inline void pushup(int now){
		col[now]=(col[now<<1]==col[now<<1|1]?col[now<<1]:0);
		sum[now]=sum[now<<1]+sum[now<<1|1];
	}
	inline void pushdn(int now,int nl,int nr){
		int m=(nl+nr)>>1;
		if(wp[now]){
			sum[now<<1]+=wp[now]*(m-nl+1);
			sum[now<<1|1]+=wp[now]*(nr-m);
			wp[now<<1]+=wp[now];
			wp[now<<1|1]+=wp[now];
			wp[now]=0;
		}
		if(chg[now]){
			col[now<<1]=col[now<<1|1]=chg[now<<1]=chg[now<<1|1]=chg[now];
			chg[now]=0;
		}
	}
	...
	void update(int l,int r,int x,int now=1,int nl=1,int nr=n){
		if(nl>=l&&nr<=r&&col[now]){
			ll toadd=abs(x-col[now]);
			sum[now]+=toadd*(nr-nl+1);
			wp[now]+=toadd;
			col[now]=chg[now]=x;
			return;
		}
		...
		pushdn(now,nl,nr);
		int m=(nl+nr)>>1;
		update(l,r,x,now<<1,nl,m);
		update(l,r,x,now<<1|1,m+1,nr);
		pushup(now);
	}
	ll getsum(int l,int r,int now=1,int nl=1,int nr=n){
		if(nl>=l&&nr<=r){
			return sum[now];
		}
		if(nl>r||nr<l){
		...
		pushdn(now,nl,nr);
		int m=(nl+nr)>>1;
		return getsum(l,r,now<<1,nl,m)+getsum(l,r,now<<1|1,m+1,nr);
	}
}seg;
