...
void solve() {
    ...
    cartesian_tree<P, decltype(cmp)> ct(b, cmp);
    // cartesian_tree<P, greater<P>> ct(b);
    // rep(i, n) {
    //     debug(ct.parent(i));
    //     debug(ct[i][0], ct[i][1]);
    // }
    vector dp(n, vc<mint>(m, 0));
    int root = ct.root();
    debug(root);
    auto dfs = [&](auto &&dfs, int v) -> void {
        int le = ct[v][0], ri = ct[v][1];
        debug(v, le, ri);
        vc<mint> lsum(m + 1, 0), rsum(m + 1, 0);
        if(le != -1) {
            dfs(dfs, le);
            rep(i, m) {
                lsum[i + 1] = lsum[i] + dp[le][i];
            }
        } else {
            rep(i, m + 1) lsum[i] = 1;
        }
        if(ri != -1) {
            dfs(dfs, ri);
            rep(i, m) {
                rsum[i + 1] = rsum[i] + dp[ri][i];
            }
        } else {
            rep(i, m + 1) rsum[i] = 1;
        }
        rep(i, m) {
            dp[v][i] = lsum[i] * rsum[i + 1];
        }
    };
    dfs(dfs, root);
    mint ans = 0;
