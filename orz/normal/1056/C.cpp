...
int main()
{
	...
	while (sz(s))
	{
		if (t == 1)
		{
			if (oblig == -1)
			{
				if (sz(gm))
				{
					...
					{
						cout << pp->X + 1 << endl;
						auto qq = gm.find(pp->X);
						s.erase(pii(-p[pp->X], pp->X));
						gm.erase(pp->X);
					}
					else
					{
						cout << pp->Y + 1 << endl;
						auto qq = gm.find(pp->Y);
						s.erase(pii(-p[pp->Y], pp->Y));
						gm.erase(pp->Y);
					}
				}
				else
			...
			{
				cout << oblig + 1 << endl;
				s.erase(pii(-p[oblig], oblig));
				auto qq = gm.find(oblig);
				gm.erase(qq);
				oblig = -1;
			}
		}
		else
		{
			...
			cin >> turn;
			--turn;
			auto qq = gm.find(turn);
			if (qq == gm.end())
				s.erase(pii(-p[turn], turn));
			else
			{
				oblig = qq->Y;
				if (gm.find(oblig) == gm.end())
					oblig = -1;
				gm.erase(qq);
				s.erase(pii(-p[turn], turn));
			}
