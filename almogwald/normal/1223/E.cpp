...
int main() {
	...
	fori(ik, t) {
		...
		wne(bfs) {
			...
			bfs.pop_back();
			forin(i, cons[cur]) {
				if (f[cons[cur][i].first] == -2) {
					f[cons[cur][i].first] = cur;
					bfs.push_back(cons[cur][i].first);
				}
			}
		}
		vecl val(n), val2(n);
		forib(i, n) {
			...
			veci difs;
			forin(i, cons[cur]) {
				if (f[cur] == cons[cur][i].first) {
					con
				}
				sum += val[cons[cur][i].first];
				difs.push_back(val2[cons[cur][i].first] + cons[cur][i].second - val[cons[cur][i].first]);
			}
			val[cur] = sum;
			val2[cur] = sum;
			sort(all(difs));
			fori(j, k - 1) {
				...
				val[cur] = max(val[cur], sum);
				val2[cur] = max(val2[cur], sum);
			}
			if (difs.size()) {
				sum += difs.back();
				difs.pop_back();
