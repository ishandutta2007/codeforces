...
int big[maxn], a[maxn], df[maxn];

void ofs(int v, int par = -1) {
    if (par != -1) {
...
void gfs(int v, ll dp_p, int big_p) {
    pair<int, int> bg = {0, 0};
    bg.fr = big_p;
    ll cur = dp_p;
    for (int i : e[v]) {
    ...
    setmin(ans, cur + max(0, a[v] - bg.fr) + big[0]);
    for (int i : e[v]) {
        int x = bg.fr;
        if (x == big[i])
            x = bg.sc;
        ll ad = -dp[i] + max(0, a[v] - x);
        gfs(i, cur + ad, max(x, a[v]));
    }
}
