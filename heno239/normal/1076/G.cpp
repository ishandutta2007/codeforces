...
void solve() {
	...
	using Data = pair<ar, ar>;
	vector<Data> sta(n);
	rep(i, n) {
		sta[i].first = ori[a[i]];
		sta[i].second = ori[a[i]^1];
	}
	auto f = [&](Data a, Data b) {
		Data res;
		rep(i, 6) {
			res.first[i] = a.first[b.first[i]];
			res.second[i] = a.second[b.second[i]];
		}
		return res;
	};
	auto g = [&](Data a, int b, int l, int r) {
		if (b)swap(a.first, a.second);
		return a;
	};
	auto h = [&](int a, int b) {
		return a ^ b;
	...
	Data e;
	rep(i, 6) {
		e.first[i] = e.second[i] = i;
	}
	SegT<Data, int> st(sta, e, 0, f, g, h);
	rep(i, q) {
		int typ; cin >> typ;
		int l, r; cin >> l >> r; l--; r--;
		if (typ == 1) {
