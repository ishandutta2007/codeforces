...
int main() {
    TEST {
        ...
        ans[0] = 1LL * n * (n - 1) / 2;
        ans[1] = 1LL * n * (n - 1) / 2;
        for(auto e : g[0]) { ans[1] -= 1LL * hld.sz[e] * (hld.sz[e] - 1) / 2; }
        rep2(i, 1, n - 1) {
            int L = hld.dist(i, l), R = hld.dist(i, r);
            if(L + d == R) {
                l = i;
                d = L + d;
            } else if(R + d == L) {
            ...
            if(l == 0) {
                int x = hld.la(r, d - 1);
                ans[i + 1] = 1LL * (hld.sz[l] - hld.sz[x]) * hld.sz[r];
            } else
                ans[i + 1] = 1LL * hld.sz[l] * hld.sz[r];
        }
        // dump(ans);
