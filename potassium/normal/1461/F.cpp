...
int main() {
	...
	if (__builtin_popcount(opmask) == 1) {
		ans = string(n - 1, s[0]);
		done();
	}
	...
	if (opmask == 6) {
		...
		int zp = find(a.begin() + 1, a.end(), 0) - a.begin();
		if (zp != -1)
			ans[zp - 1] = '-';
	...
	if (opmask == 5) {
		...
		auto solve = [&](int l, int r) {
			...
			const int product = accumulate(a.begin() + l, a.begin() + r + 1, 1, [&](int x, int y) {
				return min(THRES, x * y);
			});
			...
			for (int i = l + 1; i <= r; i++) {				
				if (a[i] == 1) {
					...
					for (int j = i; j >= l; j--) {
						prod *= a[j];
						if (chkmax(dp[i], (j == l ? 0 : dp[j - 1]) + prod))
							last[i] = j;
					}
				}
			}
