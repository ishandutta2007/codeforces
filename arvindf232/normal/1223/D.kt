...
fun main(){
    solve.cases{
        val n = getint()
        val L = getline(n)
        val compress = L.toSortedSet().toList()
        val inverse = mutableMapOf<Int,Int>()
        for((i,v) in compress.withIndex()){
            inverse[v] = i
        }
        val LL = L.map { inverse[it]!! }


        val leftfound = TreeSet<Int>()
        val IT = interval_incomparable(true)
        for(a in LL){
            val left = leftfound.ceiling(a+1)
            if(left != null){
                IT.add(a,left)
            }
            leftfound.add(a)
        }
        val max = compress.size
        val final = IT.t.toList()

        var mincost = Int.MAX_VALUE
        if(final.isEmpty()){
            put(0)
        }else{
            for(i in 0..final.size){
                val cost1 = if(i-1 >= 0){ final[i-1].first + 1} else 0
                val cost2 = if(i < final.size){max - final[i].second} else 0
                mincost = minOf(mincost, cost1+ cost2)
            }
            put(mincost)
        }
...
/*
assuming nonempty isnot always good, need to think
maxBy vs maxOf
