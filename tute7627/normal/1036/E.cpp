...
int main(){
  ...
  rep(i,0,n){
    ...
    seg[i]=Segment(Point(ax,ay),Point(bx,by));
  }
  map<pair<ll,ll>,set<ll>>mp;
  rep(i,0,n)rep(j,i+1,n){
    if(intersect(seg[i],seg[j])){
      auto r=crosspoint(seg[i],seg[j]);
      auto rx=r.real(),ry=r.imag();
      ll rx2=round(rx),ry2=round(ry);
      if(eq(rx,rx2)&&eq(ry,ry2)){
        mp[MP(rx2,ry2)].insert(i);
        mp[MP(rx2,ry2)].insert(j);
      }
    }
  }
  for(auto z:mp){
