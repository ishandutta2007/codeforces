...
long double dt(int x1,int y1,int x2,int y2)
{
...
int main()
{
	...
	long double ab=dt(ax,ay,bx,by);
	long double bc=dt(bx,by,cx,cy);
	long double ac=dt(ax,ay,cx,cy);
	if(ab==bc&&bc==ac&&ab==ac)OVER("Yes");
	if(ab+bc==ac)OVER("No");
	if(bc==ab)OVER("Yes");
	else OVER("No");
}