...
string s;
int n;
int dist[maxn],dp[maxn],nxt[maxn],dp_nxt[maxn],zeros[maxn];
signed main(){
    ...
    for (int i=0;i<=n+1;++i) zeros[i]=n;
    for (int i=n-1;i>-1;--i){
        dp_nxt[i]=zeros[dist[i]+1];
        zeros[dist[i]]=i;
    }
    j=-1;
    rep(i,n){
        if (s[i]=='1') {dp[i]=i+1, j=i; break;}
    }
    if (j==-1) return cout<<n, 0;
    for (int i=j;i<n;++i){
        dp[nxt[i]]=(dp[nxt[i]]+dp[i])%mod;
        dp[dp_nxt[i]]=(dp[dp_nxt[i]]+dp[i])%mod;
    }
    int res=0;
    for (int i=j;i<n;++i){
        if (dist[i]<=dist[n-1]) res=(res+dp[i])%mod;
    }
    cout<<res;
    return 0;    
}