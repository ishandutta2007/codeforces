...
signed main() {
	...
	for(int i = 0; i < n; ++i) {
		for(int j = 0; j < m; ++j) {
			int ans = max(lower_bound(all(b[j]), A[i][j]) - b[j].begin(), lower_bound(all(a[i]), A[i][j]) - a[i].begin()); 
			ans += max(b[j].end() - upper_bound(all(b[j]), A[i][j]), a[i].end() - upper_bound(all(a[i]), A[i][j])) + 1;
			cout << ans << ' ';
		}
		cout << '\n';
	}
