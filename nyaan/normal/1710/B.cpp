...
void q() {
  ...
  vp dat;
  {
    ...
    each2(k, v, henka) {
      s += (k - x) * d;
      dat.emplace_back(k, s);
      x = k;
      d += v;
    }
  }

  vl xs(sz(dat)), init1(sz(dat)), init2(sz(dat)), init3(sz(dat));
  rep(i, sz(dat)) {
    xs[i] = dat[i].first;
    init1[i] = dat[i].se - xs[i];
    init2[i] = dat[i].se;
    init3[i] = dat[i].se + xs[i];
  }
  SegmentTree seg1(
      init1, [](ll s, ll t) { return max(s, t); }, -infLL);
  SegmentTree seg2(
      init2, [](ll s, ll t) { return max(s, t); }, -infLL);
  SegmentTree seg3(
      init3, [](ll s, ll t) { return max(s, t); }, -infLL);
  ...
  rep(i, N) {
    ...
    ll M = lb(xs, A);
    ll R = lb(xs, A + B);
    amax(mx, seg2.query(0, L));
    amax(mx, seg1.query(L, M) + A - B);
    amax(mx, seg3.query(M, R) - A - B);
    amax(mx, seg2.query(R, sz(dat)));
    //trc(i, mx);
    cout << (mx > thres ? '0' : '1');
  }
  cout << "\n";
