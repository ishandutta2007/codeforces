...
long long mod=998244353,n,fac[3000002],ifac[3000002],inv[3000002];
char c[1919810];
int t1[1919810],t2[1919810],t3[1919810];
...
int main() {
	scanf("%s",c+1);
	n=strlen(c+1);fac[0]=1;
	F(i,1,n)fac[i]=fac[i-1]*i%mod;
	ifac[n]=getm(1,fac[n]);
	UF(i,n-1,0)ifac[i]=ifac[i+1]*(i+1)%mod;
	F(i,1,n)inv[i]=ifac[i]*fac[i-1]%mod;
	F(i,1,n)t1[i]=t1[i-1]+(c[i]=='('),t2[i]=t2[i-1]+(c[i]==')'),t3[i]=t3[i-1]+(c[i]=='?');
	ll ans=0;
	F(i,1,n){
		ans+=t1[i]*C(t3[n],t3[i]+t1[i]+t2[i]-t2[n]);
		ans+=t3[i]*C(t3[n]-1,t3[i]+t1[i]+t2[i]-t2[n]);
		ans%=mod;
	}
	write(ans);
	return 0;
