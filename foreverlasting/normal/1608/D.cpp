...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        for(res i=2;i<=2*n;i++)inv[i]=mul(inv[i],inv[i-1]);
        res fl=0;
        for(res i=1;i<=n;i++){
            scanf("%s",str+1);
            if(str[1]=='B')x++;
            ...
            if(str[2]=='W')y++;
            d+=str[1]=='?',d+=str[2]=='?';
            if(str[1]==str[2]&&str[1]!='?')fl=1;
            if(str[1]==str[2]&&str[1]=='?')a++;
            if((str[1]=='B'&&str[2]=='?')||(str[1]=='?'&&str[2]=='W')||(str[1]=='B'&&str[2]=='W'))b++;
            if((str[1]=='W'&&str[2]=='?')||(str[1]=='?'&&str[2]=='B')||(str[1]=='W'&&str[2]=='B'))c++;
        }
        res ans=0;
        if(n<x){puts("0");return;}
        ans=C(d,n-x);
        if(!fl){
            add(ans,kcz-qpow(2,a));
            if(b==0)ans++;
            if(c==0)ans++;
        }
        cout << ans << endl;
