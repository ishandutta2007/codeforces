...
// select from a[l0..r1), a[l1..r1], xor (a[i0] ^ a[i1]) <= x, both non-empty, mod 2^h
mint solve2(int h, int l0, int r0, int l1, int r1, int x) {
    if (l0 == r0 || l1 == r1) return mint(0);

    if (h == -1) {
        return (mint(2).pow(r0 - l0) - 1) * (mint(2).pow(r1 - l1) - 1);
    }
    int mid0 = l0, mid1 = l1;
    while (mid0 < r0 && (a[mid0] & (1LL << h)) == 0) mid0++;
    while (mid1 < r1 && (a[mid1] & (1LL << h)) == 0) mid1++;

    if (x & (1LL << h)) {
        mint lc = solve2(h - 1, l0, mid0, mid1, r1, x) +
                  (mint(2).pow(mid0 - l0) - 1) + (mint(2).pow(r1 - mid1) - 1) + 1;
        mint rc = solve2(h - 1, mid0, r0, l1, mid1, x) +
                  (mint(2).pow(r0 - mid0) - 1) + (mint(2).pow(mid1 - l1) - 1) + 1;
        return lc * rc - (mint(2).pow(r0 - l0) - 1) - (mint(2).pow(r1 - l1) - 1) - 1;
    } else {
        return solve2(h - 1, l0, mid0, l1, mid1, x) + solve2(h - 1, mid0, r0, mid1, r1, x);
    }
}

// select from a[l..r), xor <= x, non-empty, mod 2^h
mint solve1(int h, int l, int r, int x) {
    if (l == r) return mint(0);
    if (h == -1) {
        return mint(2).pow(r - l) - 1;
    }
    int mid = l;
    while (mid < r && (a[mid] & (1LL << h)) == 0) mid++;

    if (x & (1LL << h)) {
        return solve2(h - 1, l, mid, mid, r, x) + (mint(2).pow(mid - l) - 1) + (mint(2).pow(r - mid) - 1);
    } else {
        return solve1(h - 1, l, mid, x) + solve1(h - 1, mid, r, x);
    }
}
