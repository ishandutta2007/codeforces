...
public class A {
	...
	public static void main(String args[]) {
		...
		for(int test_case = 1; test_case <= t; ++test_case) {
			...
			Pair firstOcc=new Pair(-1, -1);
			Pair secondOcc=new Pair(-1, -1);
			for(int i = 0; i < n; ++i) {
				for(int j = 0; j < n; ++j) {
					if(grid[i][j]=='*'&&firstOcc.first==-1) {
						firstOcc=new Pair(i, j);
					} else if(grid[i][j] == '*') {
						secondOcc=new Pair(i, j);
						break;
					}
				}
			}
			
			if(firstOcc.first == secondOcc.first) {
				int dist = 1;
				
				if(firstOcc.first + dist >= n) {
					grid[firstOcc.first-dist][firstOcc.second]='*';
					grid[secondOcc.first-dist][secondOcc.second]='*';
				} else {
					
					grid[firstOcc.first+dist][firstOcc.second]='*';
					grid[secondOcc.first+dist][secondOcc.second]='*';
				}
			} else if(firstOcc.second == secondOcc.second) {
				
				int dist = 1;
				if(firstOcc.second + dist >= n) {
					grid[firstOcc.first][firstOcc.second - dist]='*';
					grid[secondOcc.first][secondOcc.second - dist]='*';
				} else {
					grid[firstOcc.first][firstOcc.second + dist]='*';
					grid[secondOcc.first][secondOcc.second + dist] ='*';
				}
			} else {
				int x = firstOcc.first - secondOcc.first;
				int y = firstOcc.second - secondOcc.second;
				if(secondOcc.first < firstOcc.first) {
					grid[secondOcc.first][firstOcc.second]='*';
					grid[firstOcc.first][secondOcc.second]='*';
				} else {
					grid[firstOcc.first][secondOcc.second]='*';
					grid[secondOcc.first][firstOcc.second]='*';
				}
