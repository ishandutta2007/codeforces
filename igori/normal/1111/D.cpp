...
signed main()
{
    ...
    w = 1;
    for (int i = 1; i <= n / 2; i++) w = i * w % MOD;
    for (int i = 1; i <= n / 2; i++) w = i * w % MOD;
    for (int i = 0; i < 52; i++)
    {
        for (int j = 1; j <= cnt[i]; j++)
        {
            w = w * inq(j, MOD - 2, MOD) % MOD;
        }
    }
    dp[0] = 1;
    for (int j = 0; j < 52; j++)
    {
        for (int summ = n; summ >= 0; summ--)
        {
            if (cnt[j] <= summ && cnt[j] != 0)
            {
                dp[summ] += dp[summ - cnt[j]];
                //cout << summ - cnt[j] << " ";
            }
            dp[summ] %= MOD;
        }
