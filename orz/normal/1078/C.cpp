...
pair<pii, int> dfs(const int& v, const vvi& g, vb& nv)
{
	nv[v] = false;
	int a = 0, b = 0, c = 0;
	int Pw = sz(g[v]);
	...
	for (int i = 0; i < Pw; ++i)
	{
		...
		if (nv[to])
		{
			auto new_nf = dfs(to, g, nv);
			nf.pb(new_nf.X);
			cf.pb(new_nf.Y);
		}
	}
	int pw = sz(nf);
	vi left_pref(pw + 1), right_pref(pw);
	
	left_pref[0] = 1;
	for (int i = 0; i < pw; ++i)
		left_pref[i + 1] = prod(left_pref[i], sum(prod(nf[i].X, 2, N), cf[i], N), N);

	if (pw)
		right_pref[pw - 1] = 1;
	for (int i = pw - 1; i > 0; --i)
		right_pref[i - 1] = prod(right_pref[i], sum(prod(nf[i].X, 2, N), cf[i], N), N);

	for (int i = 0; i < pw; ++i)
		a = sum(a, prod(prod(left_pref[i], right_pref[i], N), nf[i].Y, N), N);

	b = left_pref[pw];

	c = 1;
	for (int i = 0; i < pw; ++i)
		c = prod(c, sum(nf[i].X, cf[i], N), N);
