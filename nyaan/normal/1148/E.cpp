...
int main(){
  ...
  while(idx != N - 1){
    ...
    if(a[idx].fi == b[dst]){
      ok[idx] = 1; idx++; dst++; continue;
    }
    auto it = upper_bound(all(a) , pll(b[dst] , infLL) );
    if(it == a.end()){
      out("NO"); return 0;
    }
    if( b[dst] - a[idx].fi <= it->fi - b[dst] ){
      ll d = b[dst] - a[idx].fi;
      ans.pb( mp( pii( a[idx].se, it->se ) , d ) );
      a[idx].fi += d; it->fi -= d;
      ok[idx] = 1; idx++; dst++;
    }
    else {
    ...
    trc(a); trc(idx); trc(dst);
  }

  out("YES");
  out(sz(ans));
  rep(i,sz(ans)){
    out( ans[i].fi.fi, ans[i].fi.se, ans[i].se );
  }
