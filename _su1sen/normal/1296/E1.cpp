...
int main() {
    ...
    rep(i, n) {
        ...
        vec<int> same_color;
        rep(j, r) {
            if (s[r] < s[j]) same_color.push_back(j);
        }
        int sz = same_color.size();
        if (sz) {
            int l = same_color.front();
            edges.emplace_back(l, r);
            rep(k, sz - 1) {
                uf.merge(same_color[k], same_color[k + 1]);
            }
        }
    }
    ...
    for (auto [u, v] : edges) {
        int id_u = ids[u];
        int id_v = ids[v];
        g[id_u].push_back(id_v);
        g[id_v].push_back(id_u);
    }
    vec<int> c(m, 0);
    rep(i, m) {
        if (c[i] != 0) continue;
