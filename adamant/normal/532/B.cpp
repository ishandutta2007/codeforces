...
void go(int cur)
{
    ...
    int k = 0;
    ll sum[2] = {0, 0};
//    pair<ll, ll> minr[2] = { {LLINF, 0}, {LLINF, 0} };
//    ll maxdif = -LLINF;
//    ll mindif = LLINF;
    
    if (v[cur].size() == 0) return;
    go(v[cur][0]);
    sum[0] = max(0ll, res[v[cur][0]][0]);
    sum[1] = res[v[cur][0]][1];
    for (int i = 1; i < v[cur].size(); ++i)
    {
        ...
        ll sum0 = max(sum[0] + res[to][0], sum[1] + res[to][1]);
        ll sum1 = max(sum[1] + res[to][0], sum[0] + res[to][1]);
        
        sum[0] = sum0;
        sum[1] = sum1;
    }
    res[cur][1] = max(sum[1], sum[0] + a[cur]);
    res[cur][0] = sum[0];
    
//    if (k % 2 == 0){
//        res[cur][1] = max(sum[1] + a[cur], ;
//        res[cur][0] = max(sum[1], sum[1] - mindif + a[cur]);
//    }
//    else {
//        res[cur][1] = max(sum[1], sum[1] - mindif + a[cur]);
//        res[cur][0] = sum[1] + a[cur];
//    }
    //DBN(cur, res[cur][0], res[cur][1], a[cur]);
}
int main()
{
    ...
    go(0);
    cout << res[0][1] << std::endl;
    
    return 0;
}
//
//bool who[10][10][10][10];
//
//int main()
//{
//    int a,b,c,d;
