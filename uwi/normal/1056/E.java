...
public class E {
	...
	void solve()
	{
		...
		int ans = 0;
		outer:
		for(int len0 = 1;len0 <= n-1 && n-f[0]*len0 > 0;len0++){
			int len1 = (n-f[0]*len0) / f[1];
			if(len1 > 0 && len1*f[1] + len0*f[0] == n){
				if(len0 == len1 && lcp(st, isa, first[0]*len1, first[1]*len0) >= len0){
					continue;
				}
				int ofs = 0;
				for(int i = 0;i < m;i++){
					if(s[i] == '0'){
						if(lcp(st, isa, first[0]*len1, ofs) < len0)continue outer;
						ofs += len0;
					}else{
						if(lcp(st, isa, first[1]*len0, ofs) < len1)continue outer;
						ofs += len1;
					}
				}
				assert ofs == n;
	...
	int lcp(int[][] st, int[] isa, int x, int y)
	{
		x = isa[x]; y = isa[y];
		if(x > y){
			int d = x; x = y; y = d;
		}
		return rmq(st, x+1, y+1);
	}
...
//		for (int i = 0; i < n; i++) {
//			sb.append(gen.nextInt(10000)/9999);
//		}
//		sb.append("\n");
//		for (int i = 0; i < m; i++) {
//			sb.append((char)('a'+gen.nextInt(1)));
//		}
//		sb.append("\n");
//		INPUT = sb.toString();
