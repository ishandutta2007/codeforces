...
auto G = y_combinator([&](auto rec, int r) -> vector<vector<pair<int, int>>> {
	if(r == 1) {
		return {{{1, 1}}, {}};
	}
	auto g = rec((r + 1) / 2);
	...
	g.insert(g.begin(), {{1, r / 2}});
	for(auto [x, y] : g[1]) {
		g[0].emplace_back(x, y);
	}
	if(r % 2 == 1) {
		for(int i = 0; i < int(g[1].size()); ++i) {
			if(g[1][i].first == int(g.size()) - 1) {
...
if(L > 1) {
	...
	G.insert(G.begin(), {{2, L - 1}});
}
int n = G.size(), m = 0;
for(auto& v : G) {
	m += v.size();
}
[&]() { auto& out = cout; put_stream(out, "YES\n", make_string_formatter( n ), " ", make_string_formatter( m ), "\n");  }();
for(int i = 0; i < n; ++i) {
	for(auto& [x, y] : G[i]) {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter( i + 1 ), " ", make_string_formatter( x ), " ", make_string_formatter( y ), "\n");  }();
	}
}
END