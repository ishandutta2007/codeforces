    ...
    void solve() {
        ...
        for (int z : range(t)) {
            ...
            while (at < p.size()) {
                ...
                int start = at;
//                err.printLine(start);
                while (at < p.size() && limit / p[at] >= q) {
                    q *= p[at++];
                }
                out.printLine("?", q);
                ll g = in.readLong();
//                ll g = 1;
                for (int i : range(start, at)) {
                    if (g % p[i] == 0) {
                        d.push_back(p[i]);
                    }
                }
            }
            for (int i = 0; i < d.size() && i < 12; i += 2) {
                ll q = 1;
                for (int j : range(i, min(i + 2, int(d.size())))) {
                    ll a = 1;
                    while (a * d[j] <= 1000000000) {
                        a *= d[j];
                    }
                    q *= a;
                }
            ...
            for (int i : range(12, d.size())) {
                res *= 2;
            }
            out.printLine("!", max(res * 2, res + 7));
        }
    }
};
...
int main() {
    ...
    auto time = clock();
#endif
    FUgadatKolichestvoDelitelei solver;
