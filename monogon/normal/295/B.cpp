...
int main() {
	...
	for(int x = n - 1; x >= 0; x--) {
		k = num[x] - 1;
		added.push_back(k);
		for(unsigned int l = 0; l < added.size() - 1; l++) {
			for(unsigned int i = 0; i < added.size(); i++) {
				if(mat[added[i]][added[l]] + mat[added[l]][k] < mat[added[i]][k]) {
					mat[added[i]][k] = mat[added[i]][added[l]] + mat[added[l]][k];
				}
			}
			for(unsigned int j = 0; j < added.size() - 1; j++) {
				if(mat[k][added[l]] + mat[added[l]][added[j]] < mat[k][added[j]]) {
					mat[k][added[j]] = mat[k][added[l]] + mat[added[l]][added[j]];
				}
			}
		}
		for(unsigned int i = 0; i < added.size(); i++) {
			for(unsigned int j = 0; j < added.size(); j++) {
				if(mat[added[i]][k] + mat[k][added[j]] < mat[added[i]][added[j]]) {
					mat[added[i]][added[j]] = mat[added[i]][k] + mat[k][added[j]];
				}
			}
		}
		for(unsigned int i = 0; i < added.size(); i++) {
			for(unsigned int j = 0; j < added.size(); j++) {
				ans[x] += mat[added[i]][added[j]];
			}
		}
	}
	for(int i = 0; i < n; i++) {
