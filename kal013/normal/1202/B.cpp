...
void solve(){
    ...
    For(i,10){
        For(j,10){
            int res=0;
            for(auto x:val) {res+=dp[i][j][x];
                if (res>=INF) res=INF;
            }
            if (res>=INF || res>20*n) res=-1;
            ans[i][j]=res;
            d0(ans[i][j]);
        }
        d1("");
...
signed main(){
    ...
    fio;
    int mvs=0;
    For(i,10) For(j,10) For(k,10) dp[i][j][k]=INF;
    For(i,10) For(j,10) {
        st(i,j);
        For(k,10){
            if (dp[i][j][k]<=20) mvs=max(mvs,dp[i][j][k]);
        }
    }
    // trace(dp[2][1][0]);
    // trace(mvs);
    ...
    // trace(jjj);
    // For(i,10){
    //     For(j,10) For(k,10) trace(i,j,k,dp[i][j][k]);
    // }
    int t=1;
    // read(t);
    while(t--) {
