...
int main(){
    ...
    prefix[0]=t[0];
    rep(i,1,n){
        prefix[i]=prefix[i-1]*3+t[i];
        while(prefix[i]>=LINF)prefix[i]-=LINF;
    }
    suffix.back()=t.back();
    ll pow=1;
    rrep(n-1){
        pow*=3;
        while(pow>=LINF)pow-=LINF;
        suffix[i]=suffix[i+1]+t[i]*pow;
        while(suffix[i]>=LINF)suffix[i]-=LINF;
    }
    ll i=1;
    for(;i<n;i++)if(prefix[n-1-i]==suffix[i])break;
    ll c0=count(range(s),'0'),c1=s.size()-c0;
    each(i,t)i--;
    string ans;
    [&]{
        while(true)rep(j,i){
