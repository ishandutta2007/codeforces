	...
	while(T--)
	{
		...
		up(i,1,n)
		{
			...
			up(j,1,n)
			{
				...
				if(dis(i,j)<ds[i])
				{
					memset(b[i],0,sizeof(b[i]));ds[i]=dis(i,j);
				}
				if(dis(i,j)==ds[i])
				{
					b[i][j]=1;	
...
//		up_(m)cout<<c[i]<<" ";edl;
		up(i,1,m)
		{
			if(c[i]==1)
			{
				up(j,0,n)
				{
					if(j)Add(dp[i][j],dp[i-1][j-1]*(n-j+1)%mod);
				}
			}
			else
			{
				up(j,0,n)
				{
					if(j>=1)Add(dp[i][j],dp[i-1][j-1]*(n-j+1)%mod);
					if(j>=c[i])Add(dp[i][j],dp[i-1][j-c[i]]*fac[c[i]]%mod*C(n-j+c[i],c[i])%mod);
				}
			}
		}
		int ans=0;
		up(i,0,n)Add(ans,dp[m][i]);
		cout<<ans;
	}
}
/*
