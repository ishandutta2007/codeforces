#line 1 "codeforces-workspace/contest/392/e/e.cpp"
...
#line 6 "codeforces-workspace/contest/392/e/e.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/392/e/e.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/392/e/e.cpp"
...
void main() {
  ...
  auto dpls = make_vector<int>({n, n}, -inf);
  ...
  for (auto r : range(1, n + 1)) {
    vector<int> dpr(n, -inf);
    dpr[r - 1] = 0;

    for (auto l : rrange(r)) {  // [l, r)
      auto &dpl = dpls[l];

      for (auto k : range(l, r - 1)) {
        if (w[k] == w[r - 1] - 1) {
          chgr(dpl[r - 1], dpl[k] + dp[k + 1][r - 1]);
        }
      }

      for (auto k : range(l + 1, r)) {
        if (w[k] == w[l] - 1) {
          chgr(dpr[l], dpr[k] + dp[l + 1][k]);
        }
      }

      for (auto k : range(l, r)) {
        auto len = 1 + w[k] * 2 - w[l] - w[r - 1];
        if (len > 0 && len <= n) chgr(dp[l][r], dpl[k] + dpr[k] + v[len]);
      }

      for (auto k : range(l + 1, r)) {
        chgr(dp[l][r], dp[l][k] + dp[k][r]);
      }
    }
  }
