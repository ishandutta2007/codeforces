// Problem: CF559E Gerald and Path
// Contest: Luogu
// URL: https://www.luogu.com.cn/problem/CF559E
// Memory Limit: 250 MB
// Time Limit: 4000 ms
// 
...
#endif
#define c x
int a[100005];//c[105],l[105],r[105];
struct qwq{
	int c,l,r,o;
}b[105];
int cc;
void _(int &x){x=std::lower_bound(a+1,a+1+cc,x)-a;}
int dp[122][123][2];
signed main()
{
	initprog();
	...
	b[0].x=-1e8;
	int ans=0;
	std::sort(b+1,b+1+n,[&](qwq a,qwq qb){return a.c<qb.c;});
	for(int i=0;i<=n;i++)
		for(int j=0;j<=i;j++)
		{
			for(int d=0;d<=1;d++)
			{
				int N=a[j].x+(d?a[j].o:0);
				int cur=-inf,_=0,__=0;
				for(int zak=i+1;zak<=n;zak++)
				{
					for(int D=0;D<=1;D++)
					{
						int M=a[zak].x+(D?a[zak].o:0);
						if(M>cur)cur=M,_=zak,__=D;
						mxe(ans,mxe(dp[zak][_][__],dp[i][j][d]+cur+min(a[zak].o,M-N)-M));
					}
				}
			}
		}
