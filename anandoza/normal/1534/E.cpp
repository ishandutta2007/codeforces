...
int main() {
    ...
    vi secret(n);
    for (int i = 0; i < n; i++)
        secret[i] = rng() % int(2e9);
#endif

    vvi adj(n + 1);
    for (int i = 0; i < n; i++) {
        for (int ones = 0; ones <= k; ones++) {
            if (ones > i || k - ones > n - i)
                continue;
    ...
    for (int i = 0; i + 1 < sz(path); i++) {
        ...
        for (int j = 0; j < 2; j++) {
            while (count[j]-- > 0) {
                q.push_back(indices[j].back());
                used[j].push_back(indices[j].back());
                indices[j].pop_back();
            }
        }
        ...
        for (int j = 0; j < 2; j++)
            indices[j].insert(indices[j].end(), all(used[1 ^ j]));
    }
...
#ifdef ANAND_LOCAL
    assert(ans == accumulate(all(secret), 0, std::bit_xor<int>()));
#endif
