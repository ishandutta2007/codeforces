...
int main()
{
    ...
    forn(i, n)
    {
        mi1[i + 1] = min(mi1[i], p[i].y);
        ma1[i + 1] = max(ma1[i], p[i].y);
    }
    ford(i, n)
    {
        mi2[i] = min(mi2[i + 1], p[i].y);
        ma2[i] = max(ma2[i + 1], p[i].y);
    }
    ll l = 0, r = 4e9 + 1;
    while (l < r)
    {
        ...
        forn(i, n)
        {
            while (j < n && p[j].x - p[i].x <= m)
                ++j;
            if (ma1[i] - mi1[i] <= m && ma2[j] - mi2[j] <= m && ma2[j] - mi1[i] <= m && ma1[i] - mi2[j] <= m)
            {
                ok = true;
                break;
            }
