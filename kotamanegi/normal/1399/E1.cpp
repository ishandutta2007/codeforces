...
priority_queue<tuple<int, int, int>> nexts;
vector<pair<int, int>> vertexs[100010];
int dist[100010];
int ans = 0;
int dfs(int now, int back) {
    ...
    for (auto& x : vertexs[now]) {
        ...
        cnter += hoge;
        nexts.push(make_tuple(x.second * hoge - (x.second / 2LL) * hoge, x.second, hoge));
    }
...
void solve(){
    ...
    while (S > 0) {
        ...
        S -= get<0>(now);
        get<1>(now) /= 2;
        nexts.push(make_tuple((get<1>(now) - get<1>(now) / 2)* get<2>(now), get<1>(now), get<2>(now)));
    }
    cout << ans << "\n";
}
#undef int
