...
struct N{
	...
	void add(int v,int c){
		...
		rep(i,K)if((x[i].b-=cur.b)==0)x[i]=pi(-1,0);
	}
	static N merge(const N&a,const N&b){
		N res=a;
		rep(i,K)if(b.x[i].a!=-1)
			res.add(b.x[i].a,b.x[i].b);
		return res;
	}
};
...
signed main(){
	...
	segtree<N> seg(a);
	auto check=[&](int l,int r,int k,int x){
		int num=lwb(pos[x],r)-lwb(pos[x],l);
		return num*k>r-l;
	};
	rep(_,q){
		int l,r;cin>>l>>r;
		...
		N z=seg.query(l,r);
		int mn=inf;
		rep(i,K)if(z.x[i].a!=-1){
			if(mn>z.x[i].a&&check(l,r,k,z.x[i].a))
				mn=z.x[i].a;
		}
		if(mn==inf)mn=-1;
		else mn++;
