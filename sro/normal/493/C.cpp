...
int main()
{
	...
	sca=n*3,scb=m*3,mxsca=sca,mxscb=scb,mxcha=sca-scb;
	for(int i=0;i<n+m;i++)
		if(pointb==m||pointa<n&&a[pointa]<b[pointb])c[i]=0,pointa++;
		else if(pointa==n||a[pointa]>b[pointb]) c[i]=1,pointb++;
		else c[i]=2,pointa++,pointb++;
	for(int i=0;i<n+m;i++)
	{
		if(c[i]==0)sca--;
		else if(c[i]==1) scb--;
