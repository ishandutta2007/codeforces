...
int n;
int a[100001];
int64_t l_ans[100001];
void solve() {
    cin >> n;
    for (int i = 1; i <= n; i++) cin >> a[i];
    if (n % 2) {
        int64_t ans = 0;
        for (int i = 2; i < n; i += 2) ans += max(0, max(a[i - 1], a[i + 1]) - a[i] + 1);
        cout << ans << '\n';
        return;
    } else {
        // do not use the 2nd building
        int64_t ans = 0;
        for (int i = 3; i < n; i += 2) ans += max(0, max(a[i - 1], a[i + 1]) - a[i] + 1);

        // use 2, 4, 6, 8, ...., and n - 1, n - 3, n -5, ..
        for (int i = 2; i < n; i += 2) {
            l_ans[i] = l_ans[i - 2] + max(0, max(a[i - 1], a[i + 1]) - a[i] + 1);
        }
        int64_t r_ans = 0;
        for (int i = n - 1; i >= 1; i -= 2) {
            ans = min(ans, r_ans + l_ans[i - 1]);
            r_ans += max(0, max(a[i - 1], a[i + 1]) - a[i] + 1);
        }
        cout << ans << '\n';
    }
}
