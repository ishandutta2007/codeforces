...
int main() {
    ...
    loop(t) {
        ...
        int rcnt = 0, ccnt = 0;
        vector<int8_t> row(n, false), col(m, false);
        reverse(all(qs));

        mint ans = 1;
        for (auto &[x, y] : qs) {
            --x, --y;
            if (row[x] and col[y]) continue;
            rcnt += not std::exchange(row[x], true);
            ccnt += not std::exchange(col[y], true);
            ans *= k;
            if (rcnt == n or ccnt == m) break;
        }
        print(ans);
    }
    return 0;
