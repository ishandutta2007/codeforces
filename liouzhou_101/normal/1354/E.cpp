...
void solve()
{
	...
	vector<vector<int>> f(1, vector<int>(n + 1));
	...
	for (int i = 1; i <= n; ++i)
	{
		...
		int c0 = 0, c1 = 0;
		for (auto x : q) c1 += c[x], c0 += 1 - c[x];
		d.push_back({ c0,c1 });
		component.push_back(q);
		vector<int> g(n + 1);
		for (int k = 0; k <= n; ++k)
		{
			if (f.back()[k]) g[k + c0] = g[k + c1] = 1;
		}
		f.push_back(g);
	}
	if (!f.back()[n2]) return writeln("NO");
	writeln("YES");
	for (int i = f.size() - 1; i >= 1; --i)
	{
		...
		{
			int ck = d[i][k];
			if (n2 >= ck && f[i - 1][n2 - ck])
			{
				n2 -= ck;
				for (auto x : component[i - 1])
				{
					if (c[x] == k) c[x] = 2; else c[x] = 1;
				}
				break;
			}
		}
