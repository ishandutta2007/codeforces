...
template<class T, int SZ> struct LazySeg {
	const T ID = mp(INF, INF); T comb(T a, T b) { return min(a, b); }
	T seg[2*SZ];
	int lazy[2*SZ];
	LazySeg() {
	    f0r(i, 2*SZ) lazy[i] = 0;
...
int main(){
    ...
    LazySeg<pi, (1<<19)> store;
    store.build();
    f0r(i, n) store.upd(i, i, a[i]);
    auto print = [&]() {
        pr("SEG: ");
        f0r(i, n) pr(store.query(i, i), " ");
        ps();
    };
    for(int l = n-1; l>=0; l--){
        auto qq = store.query(l, n-1);
//        print();
        while(qq.f <= 0){
//            ps(qq);
//                print();
            seg.upd(qq.s, 1);
            store.upd(qq.s, qq.s, INF);
            store.upd(qq.s, n-1, -1);
            qq = store.query(l, n-1);
        }
        for(auto x: ask[l]){
            ans[x.s] = seg.query(l, x.f);
        }
    }
    f0r(i, q){
        ps(ans[i]);
