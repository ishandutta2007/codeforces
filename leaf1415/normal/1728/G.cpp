...
mint ldp[200005][25], rdp[200005][25];
...
int main(void)
{
	...
	rep(i, 0, n-1){
		rep(j, 1, m+1){
			ll len = abs(a[i+1]-b[j]);
			ldp[i+1][j] += ldp[i][j] * len;
			rep(k, j+1, m){
				ll nlen = abs(a[i+1]-b[k]);
				if(nlen <= len) continue;
				ldp[i+1][k] += ldp[i][j] * (nlen-len) , len = nlen;
			}
			ldp[i+1][m+1] += ldp[i][j] * (d-len+1);
		}
	}
	...
	per(i, n+1, 1){
		rep(j, 0, m){
			ll len = abs(a[i-1]-b[j]);
			rdp[i-1][j] += rdp[i][j] * len;
			per(k, j-1, 1){
				ll nlen = abs(a[i-1]-b[k]);
				if(nlen <= len) continue;
				rdp[i-1][k] += rdp[i][j] * (nlen-len), len = nlen;
			}
			rdp[i-1][0] += rdp[i][j] * (d-len+1);
		}
	}
	...
	rep(q, 1, Q){
		cin >> f;
		mint ans = 0;
		ll p = lower_bound(a+1, a+n+1, f) - a;
		rep(i, 1, m+1) rep(j, 0, m){
			mint tmp = d+1;
			if(i <= j){
				ll len = max(abs(f-b[i]), abs(f-b[j]));
				tmp = d-len+1;
			}
			ans += ldp[p-1][i] * rdp[p][j] * tmp;
		}
		cout << ans << "\n";
	}
