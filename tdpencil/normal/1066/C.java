...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while(T-- > 0) {
            ...
            if(line.charAt(0) == '?') {
                int res = Integer.parseInt(line.substring(2));
                if(left.containsBook(new Book(res, 0))) {
                    int id = left.getBook(new Book(res, 0));
//                    System.out.println("LEFT HAS: " + id);
                    System.out.println(Math.min(id, (right.size() + (left.size() - id - 1))));
                } else if(right.containsBook(new Book(res, 0))) {
                    int id = right.getBook(new Book(res, 0));
//                    System.out.println("RIGHT HAS: " + id);
                    System.out.println(Math.min(id, (left.size() + (right.size() - id - 1))));
                } else {
                    assert(false);
                }
...
//                System.out.println(line.charAt(0));
                int res = Integer.parseInt(line.substring(2));
                right.add(new Book(res, 0));
            } else {
//                System.out.println(line.charAt(0));
                int res = Integer.parseInt(line.substring(2));
                left.add(new Book(res, 0));
            }
        }
