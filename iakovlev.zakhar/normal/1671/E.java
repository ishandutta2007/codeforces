...
public class E {
	...
	public int[][] readGraph(int n, int m) {
		...
		for (int i = 0; i < x.length; i++) {
			to[x[i]][sz[x[i]]] = y[i];
			size[x[i]][sz[x[i]]++] = siz[i];
			to[y[i]][sz[y[i]]] = x[i];
			size[y[i]][sz[y[i]]++] = siz[i];
		}
		return to;
	}
	...
	public void dfs(int v, int prev, int sum) {
		sub[v] = -sum;
		for (int i : to[v]) {
			if (i == prev) {
				continue;
	...
	public long calc(int[] a, int mean) {
		long ans = 0;
		long more = 0;
		for (int q = 0; q < 2; q++) {
			for (int i = 0; i < a.length; i++) {
	...
	public class RecReturn {
		...
		public RecReturn(int number, boolean[] b) {
			this.number = number;
			this.b = b;
		}
	...
	public RecReturn rec(int v) {
		if (2 * v >= arr.length) {
			boolean[] res = {arr[v]};
			return new RecReturn(1, res);
		}
		RecReturn l = rec(2 * v);
		RecReturn r = rec(2 * v + 1);
		boolean[] res = new boolean[1 + 2 * l.b.length];
		res[0] = arr[v];
		int comp = 0;
		...
		if (comp <= 0) {
			for (int i = 0; i < l.b.length; i++) {
				res[i + 1] = l.b[i];
				res[i + 1 + l.b.length] = r.b[i];
			}
		} else {
			for (int i = 0; i < l.b.length; i++) {
				res[i + 1] = r.b[i];
				res[i + 1 + l.b.length] = l.b[i];
			}
		}
		int number = mult(l.number, r.number);
		if (comp != 0) {
			number = mult(2, number);
		}
		return new RecReturn(number, res);
	}
	...
	public void solve() {
		...
		for (int i = 1; i < arr.length; i++) {
			arr[i] = s.charAt(i - 1) == 'B';
		}
		out.println(rec(1).number);
	}
