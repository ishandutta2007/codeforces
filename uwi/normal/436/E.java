...
public class E {
	...
	void solve()
	{
		...
		int[][] parts = new int[2*n][];
		for(int i = 0;i < n;i++){
			parts[i] = new int[]{ab[i][0], i, 0};
			parts[i+n] = new int[]{ab[i][1]-ab[i][0], i, 1};
		}
		...
		int opti = -1;
		{
			int[] isin = new int[2*n+1];
			long[] ft = new long[2*n+1];
			
			int[][] iind = new int[n][2];
			for(int i = 0;i < 2*n;i++){
				iind[parts[i][1]][parts[i][2]] = i;
			}
			for(int i = 0;i < n;i++){
				addFenwick(isin, iind[i][0], 1);
				addFenwick(ft, iind[i][0], ab[i][0]);
			}
			
			long basesum = 0;
			for(int i = 0;i <= n;i++){
				if(m-i >= 0 && m-i <= n){
					int ind = findGFenwick(isin, m-i);
					long sum = sumFenwick(ft, ind);
					long v = basesum + sum;
					if(v < ret){
						ret = v;
						opti = i;
				...
				if(i < n){
					...
					addFenwick(isin, iind[i][0], -1);
					addFenwick(ft, iind[i][0], -ab[i][0]);
					addFenwick(isin, iind[i][1], 1);
					addFenwick(ft, iind[i][1], ab[i][1]-ab[i][0]);
				}
			}
		}
		out.println(ret);
