...
void slv(){
	...
	vc<pi> rw(n);
	rep(i,n)rw[i]=pi(a[i],i);
	segtree<MinNode> x(rw);
	segtree<MaxNode> y(rw);
	int cur=0,ans=0;
	while(cur<n-1){
		int nx=-1;
		if(a[cur]<a[cur+1]){
			int lim=x.max_right(cur,&MinNode::ok,a[cur]).a;
			nx=y.composite(cur,lim).v.b;
		}else{
			int lim=y.max_right(cur,&MaxNode::ok,a[cur]).a;
			nx=x.composite(cur,lim).v.b;
		}
		assert(nx!=-1);
		cur=nx;
		ans++;
