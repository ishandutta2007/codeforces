...
public class D4F {
	...
	void solve()
	{
		...
		long[] rank = new long[n*m];
		int p = 0;
		for(int i = 0;i < n;i++){
			for(int j = 0;j < m;j++){
				rank[p++] = (long)a[i][j]<<32|i<<16|j;
			}
		}
		Arrays.sort(rank);
		...
		for(int i = n*m-1;i >= 0;i--){
			int r = ((int)rank[i])>>>16, c = ((int)rank[i])&65535;
			for(int j = 0;j < n;j++){
				if(ved[j][c]){
					ct[r][j]++;
					ct[j][r]++;
	...
	public static void main(String[] args) throws Exception { new D4F().run(); }
