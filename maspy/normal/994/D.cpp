...
void solve() {
  ...
  // you can?
  ll you = 0;
  FOR(i, N) FOR(j, M) if (OK[i][j] != 0) you |= 1 << OK[i][j];
  if (popcnt(you) == 1) return print(topbit(you));

  bool ok1 = 1;
  FOR(i, N) {
    ll s = 0;
    FOR(j, M) if (OK[i][j] != 0) s |= 1 << OK[i][j];
    if (popcnt(s) >= 2) ok1 = 0;
  }
  bool ok2 = 1;
  FOR(j, M) {
    ll s = 0;
    FOR(i, N) if (OK[i][j] != 0) s |= 1 << OK[i][j];
    if (popcnt(s) >= 2) ok2 = 0;
  }

  if (ok1 && ok2) return print(0);
  print(-1);
}
