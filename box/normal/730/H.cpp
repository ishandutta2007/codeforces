...
signed main() {
    ...
    rep(i, m) {
    	...
    	if(!i) ans = ar[k];
    		...
    		if(ans.length() != ar[k].length()) {
    			cout << "No" << endl;
    			return 0;
    		}
    		rep(i, ans.length()) if(ans[i] != ar[k][i]) ans[i] = '?';
    	}
    }
    rep1(k, n) {
    	if(match[k]) continue;
    	if(ans.length() != ar[k].length()) continue;
    	bool m = 1;
    	rep(i, ans.length()) if(ans[i] != '?' && ans[i] != ar[k][i]) { m = 0; break; }
    	if(m) {
    			cout << "No" << endl;
    			return 0;
    		}
