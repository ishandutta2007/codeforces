...
void solve(){
    ...
    auto get_cnt = [] (const vector<int>& A){
    	gp_hash_table<int, int, custom_hash> p;
    	for(auto e: A)
    		++p[e];
    ...
    // n = 40;
    // for(int i = 1; i < n; ++i){
    // 	a.push_back(random_long(N >> 1, N));
    // }
    // sort(all(a));
	vector<int> tmp; tmp.reserve(n);
    int req = (n >> 1);

    auto is_correct = [&] (int md){
    	bool found = false;
    	for(auto e: a){
    		if (e < 0 || e >= md)
    			e %= md;
