...
int main() {
    ...
    vi ret(n), MI(n), nxt(n), need(n), K(n, inf<int>);
    set<int> __;
    REC([&](auto &&f, int x, int p, set<int> &S) -> void {
        int ma = 0, mi = inf<int>, s = n - 1;
        for(auto e : g[x]) {
            if(e == p) continue;
            s -= sub[e];
            chmax(ma, sub[e]);
        ...
        for(auto e : g[x]) {
            ...
            f(e, x, se);
            if(sub[e] == ma) {
                auto it = se.lower_bound(r);
                if(it != end(se)) { chmin(k, max(ma - *it, mi + *it)); }
                if(it != begin(se)) {
                    --it;
                    chmin(k, max(ma - *it, mi + *it));
                }
            }
            if(si(S) < si(se)) swap(S, se);
            S.merge(se);
    ...
    // rep(i, n) dump(ret[i], MI[i], nxt[i], need[i], K[i]);
    rep(_, 2) {
        multiset<int> ms;
        multiset<int> ms2;
        REC([&](auto &&f, int x, int p) -> void {
            if(need[x]) {
                {
                    auto it = ms.lower_bound(ret[x]);
                    if(it != end(ms)) { chmin(K[x], max(n - sub[x] - *it, MI[x] + *it)); }
                    if(it != begin(ms)) {
                        it--;
                        chmin(K[x], max(n - sub[x] - *it, MI[x] + *it));
                    }
                }
                {
                    auto it = ms2.lower_bound(n - sub[x] - ret[x]);
                    if(it != end(ms2)) {
                        int q = n - sub[x] - *it;
                        chmin(K[x], max(n - sub[x] - q, MI[x] + q));
                    }
                    if(it != begin(ms2)) {
                        it--;
                        int q = n - sub[x] - *it;
            ...
            ms2.erase(ms2.lower_bound(n - sub[x]));
            ms.emplace(sub[x]);
        })
        (root, -1);
        rep(i, n) reverse(all(g[i]));
    }

    rep(i, n) if(need[i]) ans[i] = max(nxt[i], K[i]);
