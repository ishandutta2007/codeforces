...
const int N = 310;
const int MN = -2000000000;
int dp[2][N][N];
int a[N][N];
int n;

void clear(int a[N][N]){
	for (int i=0; i<N; ++i) for (int j=0; j<N; ++j) a[i][j] = MN;
}

void update(int dp[N][N], int i, int i1, int i2, int v){
	int j1 = i - i1, j2 = i - i2;
	if (i1 < 1 || i1 > n) return;
	if (i2 < 1 || i2 > n) return;
	if (j1 < 1 || j1 > n) return;
	...
	for (int i=2; i<2*n; ++i){
		...
		for (int i1 = 1; i1 <= n; ++i1){
			...
			for (int i2 = 1; i2 <= n; ++i2){
				...
				int v = dp[cur][i1][i2];
				update(dp[nxt], i+1, i1+1, i2+1, v);
				update(dp[nxt], i+1, i1+1, i2, v);
				update(dp[nxt], i+1, i1, i2+1, v);
				update(dp[nxt], i+1, i1, i2, v);
			}
		}
		swap(cur, nxt);
	}
