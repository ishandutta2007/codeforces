...
int n = in, m = n - 1, x = in, y = in;
vector <int> g[300000];
int d1[300000], d2[300000];
bool used[300000];
...
void dfs(int v)
{
    ...
    for (int i = 0; i < g[v].size(); i++)
        if (!used[g[v][i]]) st.pb(g[v][i]);
    for (int i = 0; i < st.size(); i++) dfs(st[i]);
    if (st.size() == 0) d1[v] = d2[v] = 0;
    else if (st.size() == 1) d1[v] = d2[v] = min(d2[st[0]] + 1, d1[st[0]]);
    else
    {
        int ds = 0, sd2 = 0;
        for (int i = 0; i < st.size(); i++) sd2 += d2[st[i]];
        for (int i = 0; i < st.size(); i++) if (d1[st[i]] == d2[st[i]]) ds++;
        if (ds == 0) d1[v] = d2[v] = sd2 + st.size();
        else if (ds == 1) d1[v] = d2[v] = sd2 + st.size() - 1;
        else d1[v] = sd2 + st.size() - 1, d2[v] = sd2 + st.size() - 2;
    }
    used[v] = false;
}
