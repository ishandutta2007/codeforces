...
#define hash hassh
using namespace std;
long long mod,dp[505][125005],up[125005],tot[125005],cnt[505];
int main() {
    int T = 1;
    //cin >> T;
    while (T--) {
        ...
        for(int i=1;i<n;i++) {
            ...
            for (int j = 0; j <= i * i / 2; j++)
                cnt[i] += dp[i][j] * ((up[j + 2] * i % mod - tot[j + 3] + tot[min(j+i+3,i*i/2+1)]) % mod) % mod, cnt[i] %= mod;
        }
