...
//	scanf("%d%d",&n,&m);
//	lsh.clear();
//	for(int i=0;i<n;i++) scanf("%d",a+i),lsh.emplace_back(a[i]);
//	for(int i=0;i<m;i++) scanf("%d",b+i),lsh.emplace_back(b[i]);
//	for(int i=0;i<=n+m;i++) pos[i].clear();
//	sort(all(lsh));
//	lsh.erase(unique(all(lsh)),lsh.end());
//	for(int i=0;i<n;i++) a[i]=lower_bound(all(lsh),a[i])-lsh.begin(),pos[a[i]].emplace_back(i);
//	for(int i=0;i<m;i++) b[i]=lower_bound(all(lsh),b[i])-lsh.begin();
//	sgt.init(n+1,inf);
//	t[0]=0;
//	for(int i=1;i<=n;i++) t[i]=t[i-1]+(a[i-1]>0);
//	sgt.build(t,n+1);
