...
struct Explicit{int value;};
struct Redirected{Value p;};
struct ValueData{std::variant<Explicit, Redirected> v;};

Value const& root(Value const& v){ // return some Explicit item
	auto p=std::get_if<Explicit>(&v->v);
	if(p){
		return v;
	}else{
		Redirected& r=*std::get_if<Redirected>(&v->v);
		return r.p=root(r.p); 
	}
}

int valueExplicit(Value const& v){ // given v is explicit, return its value
	auto p=std::get_if<Explicit>(&v->v);
	return p->value;
}

int value(Value const& v){
	return valueExplicit(root(v));
}

struct ValueExplicitOrder{
	bool operator()(Value const& first, Value const& sec) const{
		return valueExplicit(first)<valueExplicit(sec);
	}
};
...
int main(){
	...
	std::vector<Value> height(number);
	std::set<Value, ValueExplicitOrder> explicits;

	for(int const index: range(0, int(height.size()))){
		int const value=read<int>();
		auto [iterator, inserted]=explicits.insert(
				std::make_shared<ValueData>(ValueData{Explicit{value}})
				);
		height[index]=*iterator; // regardless whether the insertion succeeds or not
	}
			...
			case '3':
				{
					...
					auto const compress=[&](int left, int right /*exclusive*/, int targetValue){
						Value target=std::make_shared<ValueData>(ValueData{Explicit{targetValue}});
						Value leftDummy=std::make_shared<ValueData>(ValueData{Explicit{left}});
						auto iter=explicits.lower_bound(leftDummy);
						while(iter!=explicits.end() and valueExplicit(*iter)<right){
							Value const& value=*iter;
							debug("redirect", valueExplicit(value), "to", valueExplicit(target));
							value->v=Redirected{target};
							iter=explicits.erase(iter);
						}
						explicits.insert(iter, std::move(target));
					};

					int const left=read<int>();
					int const right=read<int>();
					auto const middle=(left+right)>>1;
					compress(left-1, middle+1, left-1);
					compress(middle+1, right+2, right+1);
					break;
				}
