...
for(int i=0;i<k;i++)
{
    ...
    if(a[i]<0){a[i]+=26;
    int j=i-1;
    while(j>=0){if(a[j]==0)a[j]+=25;else {a[j]--;break;}j--;}}
}
for(int i=0;i<k;i++)
{
...
if(s[i]+a[i]-'a'>25){s[i]=char(s[i]+a[i]-26);s[i-1]=char(s[i-1]+1);
    int j=i-1;
    while(j>=0){if(s[j]>'z'){
        s[j]=char(s[j]-26);s[j-1]=char(s[j-1]+1);}else break;j--;}}else s[i]=char(s[i]+a[i]);}
