...
int main() {
    ...
    if(n > m) {
        ...
        dp[0][0] = 1;
        rep(i, m) {
            rep(i, n + 1) rep(j, m + 1) nxt[i][j] = 0;
            rep(j, n + 1) {
                rep(k, m + 1) {
                    if(i != x) {
                        nxt[j][k] += dp[j][k];
                        if(k < m) nxt[j][k + 1] -= dp[j][k];
                    }
                    if(j < n and k < m) nxt[j + 1][max(i, k) + 1] += dp[j][k];
                }
            }
            rep(j, n + 1) rep(k, m) nxt[j][k + 1] += nxt[j][k];
            swap(dp, nxt);
        }
        mint ans;
        ...
        rep(i, m) {
            ...
            rep(j, n + 1) rep(k, n) dp[j][k] += dp[j][k + 1];
            rep(j, n + 1) rep(k, n + 1) {
                if(j < n and k < n) { ndp[j + 1][k + 1] += dp[j][k]; }
                if(i != x) ndp[j][k] += dp[j][k];
            }
            swap(dp, ndp);
        }
