...
// Problem : B. Cutting Jigsaw Puzzle
// Contest : Codeforces - Codeforces Beta Round #50
...
int main()
{
	...
	for(int i=0;i<n;i++)
		cin >> aa[i];
	int cnt=0;
	pair<int,pair<int,int> > bst={INT_MAX,{INT_MAX,INT_MAX}};
	for(int x=1;x<=n;x++){
		for(int y=1;y<=m;y++){
			if(n%x==0&&m%y==0){
				set<vector<string> > s;
				...
				if(ok){
					cnt++;
					bst=min(bst,make_pair(x*y,make_pair(x,y)));
				}
			}
		}
	}
