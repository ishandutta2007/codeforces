...
void q() {
  ...
  BipartiteGraph g(sz(large), sz(small));
  rep(u, sz(large)) rep(v, sz(small)) {
    ll l = large[u];
    ll s = small[v];
    if (gcd<int>((int)l, (int)s) == s and l * 2 + s <= M) {
      g.add_edge(u, v);
    }
  }
  if (g.flow() != sz(large)) die(-1);

  vi used(sz(small));
  vp ans;
  each2(u, v, g.MaximumMatching()) {
    ll l = large[u];
    ll s = small[v];
    ans.emplace_back(l * 2 + s, l + s);
    used[v] = 1;
  }
  rep(i, sz(small)) {
    if (!used[i]) {
      ll s = small[i];
      ans.emplace_back(3 * s, 2 * s);
    }
  }
