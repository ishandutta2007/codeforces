...
struct EulerianTrail{
	...
	void connectdfs(int v){
		used[v] = 1;
		for(auto u : G[v]) if(!used[u]) connectdfs(u);
	}
	...
	bool calc(){
		...
		rep(i, 1, n) used[i] = 0;  //check connectivity
		connectdfs(1);
		rep(i, 1, n) if(!used[i]) return false;
		
		g.clear(), g.resize(n+1);
		rep(i, 1, n) for(auto v : G[i]) g[i].insert(v);
		
		while(sz(stk)) stk.pop();
		
		vector<int> odd;  //check whether the number of vertices with odd degree is not greater than 2
		rep(i, 1, n) if(sz(g[i])%2) odd.push_back(i);
		if(sz(odd) > 2) return false;
		
		if(sz(odd) > 0) pathdfs(odd[0], odd[1], true);  //find a path initially
		else pathdfs(1, 1, false);
		
		ans.clear();  //extend the path to cover all edges
		while(stk.size()){
			int v = stk.top(); stk.pop();
			if(g[v].size() == 0) ans.push_back(v);
...
int main(void)
{
	...
	vector<ll> vec;
	rep(i, 1, n) if(deg[i]%2) vec.pb(i);
	reps(i, vec){
		if(i % 2 == 0) et.add_edge(vec[i], vec[i+1]), m++;
	}
	if(m % 2) et.add_edge(1, 1), m++;
	et.calc();
	
	cout << m << endl;
	rep(i, 0, sz(et.ans)-2){
		if(i % 2) cout << et.ans[i] << " " << et.ans[i+1] << "\n";
		else cout << et.ans[i+1] << " " << et.ans[i] << "\n";
	}
