...
int pos[N], a[N], fac[N], invfac[N];
int main() {
	int t;
	scanf("%d", &t);
	for(int i = 0; i <= 100000; i++) {
		fac[i] = i == 0 ? 1 : fac[i - 1] * (LL)i % mod;
	...
	for(int qq = 1; qq <= t; qq++) {
		...
		int le = pos[0], ri = pos[0], mex = 1, rmn = 0;
		set<int> st;
		st.insert(0);
		int ans = 1;
		while(mex < n) {
			...
			//cout << le << ' ' << ri << ' ' << rmn << ' ' << x << endl;
			ans = ans * (LL)fac[rmn] % mod * (LL)invfac[rmn - x] % mod;
			rmn -= x;
		}
		printf("%d\n", ans);
	}
