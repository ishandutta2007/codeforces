...
const int N=705,M=100005,mod=1e9+7;
...
int n,m,ans,c[N][N],s[N],pw[12][N],g[12];
...
inline int solve(){
	...
	for(int i=1;i<10;i++){
		for(int j=0;j<=m;j++)
			s[j]=(ll)c[m][j]*pw[10-i][j]%mod*pw[i][m-j]%mod;
//		debug(sum(0,0));
		for(int j=1;j<=m;j++)
			ch(s[j],s[j-1]);
		int xx=0;
		for(int j=i;j<10;j++) xx+=g[j];
//		dprintf("%d %d:\n",i,xx);
		for(int j=0;j<n;j++){
//			dprintf("%d %d %d\n",j,j+1-xx,n-xx);
			ch(ans,(ll)pw[10][j]*sum(j+1-xx,n-xx));
//			debug((ll)pw[10][j]*sum(j+1-xx,n-xx)%mod);
		}
	}
	return ans;
}
