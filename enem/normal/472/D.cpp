...
bool dfs(ll x,ll y,ll dp,ll d[],V(pll) ad[])
{
    if(dp!=d[x]) return false;
    for(auto it:ad[x])
    {
        if(it.F!=y)
        {
            if(!dfs(it.F,x,dp+it.S,d,ad)) return false;
        }
    }
    return true;
}
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            REP(j,i+1,n)
            {
                pr.push(MT(-d[i][j],i,j));
            }
        }
        V(pll) ad[n];
        while(!pr.empty())
        {
            if(un(G(1,pr.top()),G(2,pr.top()),rn,sz))
            {
                ad[G(1,pr.top())].PB(MP(G(2,pr.top()),-G(0,pr.top())));
                ad[G(2,pr.top())].PB(MP(G(1,pr.top()),-G(0,pr.top())));
                if(-G(0,pr.top())==0) bad=true;
            }
            pr.pop();
        }
        REP(i,0,n)
        {
            if(!dfs(i,-1,0,d[i],ad)) bad=true;
        }
        if(bad) cout<<"NO";
        else cout<<"YES";
