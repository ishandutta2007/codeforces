...
void run_case() {
    ...
    for (int D = N / 2; D <= (N + 1) / 2; D++) {
        int U = N - D;
        int everything = min(R - N, 1 - L);
        excellent += everything * choose(N, U);

        for (int add = everything + 1; add <= everything + N; add++) {
            int max_u = min(R - add, N);
            int min_d = max(L + add, 1);
            int overlap = max_u - min_d + 1;

            if (overlap < 0)
                continue;

            if (U < min_d - 1 || D < N - max_u)
                continue;

            excellent += choose(overlap, U - (min_d - 1));
        }
    }
