...
#endif
//#STAY AT HOME
const int nax = 60, root = 17, mod = 119 << 23 | 1;
bool pivot[nax];
...
int main() {
	...
	for (ll x : base) debug(imask(x));
	REP(i, base.size()) for (int j = i + 1; j < (int) base.size(); ++j)
		mini(base[i], base[i] ^ base[j]);
	for (ll x : base) debug(imask(x));
	for (ll x : base) pivot[63 - clz(x)] = 1;
	vector <ll> reduced;
	for (ll x : base) {
		ll here = 0;
		REP(i, m) if (!pivot[i])
	...
	for (ll c : reduced) debug(imask(c));
	vector <ll> ans(m + 1);
	ll dim = base.size();
	fprintf(stderr, "dim = %d n = %d m = %d\n", dim, n, m);
	if (dim <= 20) {
		cerr << "case1" << endl;
		REP(mask, (1 << dim)) {
			ll here = 0;
			forbits(i, mask) here ^= reduced[i];
			ans[popc(here) + popc(mask)]++;
		}
	}
	else {
		cerr << "case 2" << endl;
		int rest = m - dim;
		for (ll x : reduced) assert(x < (1 << rest));
		assert(rest <= root);
		dp[0][0] = 1;
		int lar = 0;
		for (ll x : reduced) {
			for (int c = lar; c >= 0; --c) REP(v, (1 << rest)) add(dp[c + 1][v ^ x], dp[c][v]);
			lar++;
		}
		debug(imie(lar), imie(reduced));
		REP(i, dim + 1) REP(v, 1 << rest) ans[i + popc(v)] += dp[i][v];
		for (ll &x : ans) x %= mod;
	}
	ll mul = 1;
	REP(_, n - dim) mul = mul * 2 % mod;
	for (ll x : ans) printf("%lld ", fix(x * mul, mod));
	printf("\n");
}
//#STAY AT HOME