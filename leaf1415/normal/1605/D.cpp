...
void dfs2(int v, int p)
{
	for(auto u : G[v]){
		if(u == p) continue;
		assert(digitnum(ans[v], 2) != digitnum(ans[u], 2));
		dfs2(u, v);
	}
}
...
int main(void)
{
	...
	while(T--){
		cin >> n;
		rep(i, 0, n) G[i].clear(), vec[i].clear();
		ll u, v;
		rep(i, 1, n-1){
			cin >> u >> v;
			G[u].push_back(v);
		...
		rep(i, 0, 20) dvec[i].clear();
		rep(i, 1, n) dvec[digitnum(i, 2)].push_back(i);

		vector<ll> qvec, rvec; ll rem = odd;
		per(i, 20, 0){
			if(sz(dvec[i]) == 0) continue;
			if(rem >= sz(dvec[i])){
				rem -= sz(dvec[i]);
				qvec.insert(qvec.end(), all(dvec[i]));
			}
			else rvec.insert(rvec.end(), all(dvec[i]));
		}
		rep(i, 0, n) if(i % 2) for(auto v : vec[i]) ans[v] = qvec.back(), qvec.pop_back();
		rep(i, 0, n) if(i % 2 == 0) for(auto v : vec[i]) ans[v] = rvec.back(), rvec.pop_back();
