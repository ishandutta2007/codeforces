...
bool prep(vector<int>&A,bool isy){
  int n;
  cin>>n;
  A.resize(n);
  forn(i,n) cin>>A[i];
  ...
  if(S%2) return false;
  sort(all(A));
  vector<short> D(S/2+1);
  D[0]=1;
  int s=0;
  forv(q,A){
    int a=A[q];
    for(int i=min(sz(D)-1,s+a);i>=a;--i) if(!D[i]&&D[i-a]) D[i]=q+2;
    s+=a;
  }
  dbg(A,D);
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    sort(all(xs),[](int a,int b){return mp(a<0, abs(a))<mp(b<0, abs(b));});
    sort(all(ys),[](int a,int b){return mp(a<0,-abs(a))<mp(b<0,-abs(b));});
    int x=0,y=0;
    cout<<"Yes\n";
    drawc(fig());
    forn(i,sz(xs)*2){
