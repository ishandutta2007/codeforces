...
int main(){
  ...
  while(t--){
    ...
    bool sw=true;
    if(n*(m/2)<k)sw=false;
    if((n/2)*m<rk)sw=false;
    if(n%2==0){
      if(k%2==1)sw=false;
    }
    else{
      if(m%2!=0||m/2>k||(k-m/2)%2!=0)sw=false;
    }
    ans2(sw);
    if(sw){
      ...
      auto ope=[&](ll x1,ll y1,ll x2,ll y2){
        P p(x1,y1),q(x2,y2);
        if(s[p.fi][p.se]!='.'||s[q.fi][q.se]!='.')return;
        vector<bool>t(26);
        for(auto z:{p,q}){
          rep(i,0,4){
            ll nx=z.fi+dx[i],ny=z.se+dy[i];
      ...
      if(n%2!=0){
        rep(i,0,m/2){
          ope(n-1,2*i,n-1,2*i+1);
          k--;
        }
      }
      rep(i,0,n/2)rep(j,0,m/2){
          if(k==0)break;
          ope(2*i,2*j,2*i,2*j+1);
          ope(2*i+1,2*j,2*i+1,2*j+1);
          k-=2;
        }
