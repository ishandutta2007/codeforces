...
void main_() {
	...
	while(t--) {
		...
		if(n <= 100) {
			...
			int cnt = 2;
			REP(j, n) {
				if(j == mi or j == mj) continue;
				REP(k, l, r + 1) {
					if(used[k - l]) continue;
					if(lcm((ll)x, (ll)k) == d[mi][j] and lcm((ll)y, (ll)k) == d[mj][j]) {
						res[j] = k;
						used[k - l] = 1;
						cnt++;
						break;
			...
			REP(_, min(n, 1000)) {
				...
				REP(j, 5) {
					int nx = rnd() % n;
					while(st.count(nx)) nx = rnd() % n;
					st.insert(nx);
					g = gcd(g, query(pl, nx));
				}
				if(g <= M and pf.isPrime(g) and g * 1.2 > n) {
					target = pl;
					target_p = g;
					break;
				}
