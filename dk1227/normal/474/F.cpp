...
void pushup(int id) {
	t[id].Gcd = gcd(t[ls].Gcd, t[rs].Gcd);
	t[id].cnt = 0, t[id].mn = min(t[ls].mn, t[rs].mn);
	if(t[id].mn == t[ls].mn) t[id].cnt += t[ls].cnt;
	if(t[id].mn == t[rs].mn) t[id].cnt += t[rs].cnt;
}

void build(int id, int l, int r) {
	if(l == r) {
		t[id].mn = t[id].Gcd = x[l];
		t[id].cnt = 1;
		return ;
	} 
	Mid;
...
int querygcd(int id, int l, int r, int x, int y) {
	if(x <= l && r <= y) return t[id].Gcd;
	int ret = -1;
	Mid;
	if(mid >= x) ret = querygcd(ls, l, mid, x, y);
	if(mid < y) {
		if(ret != -1) ret = gcd(ret, querygcd(rs, mid + 1, r, x, y));
		else ret = querygcd(rs, mid + 1, r, x, y);
	} 
	return ret;
}

pii querymn(int id, int l, int r, int x, int y) {
	if(x <= l && r <= y) return mp(t[id].mn, t[id].cnt);
	pii ret = mp(-1, -1);
	Mid;
	if(mid >= x) ret = querymn(ls, l, mid, x, y);
	if(mid < y) {
		pii res = querymn(rs, mid + 1, r, x, y);
		if(~ret.fi) {
			int t = ret.fi;
			To_min(ret.fi, res.fi);
			if(ret.fi == res.fi) {
				if(res.fi != t) ret.se = res.se;
				else ret.se += res.se;
			}
		}
		else ret = res;
...
int main() {
	...
	W(q) {
		int l, r;
		qread(l, r);
		int g = querygcd(1, 1, n, l, r);
		pii mn = querymn(1, 1, n, l, r);
		//cout << g << ' ' << mn.fi << ' ' << mn.se << endl;
		if(g != mn.fi) printf("%d\n", r - l + 1);
		else printf("%d\n", r - l + 1 - mn.se);
	}
 	return 0;
