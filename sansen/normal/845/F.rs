...
fn run() {
    ...
    //dp[a][b]: ab: bit
    let mut dp = vec![vec![M::zero(); 1 << w]; 2];
    dp[0][0] = M::one();
    for s in s {
        let mut wall = 0usize;
        ...
        let mask = ((1 << w) - 1) ^ wall;
        let mut next = vec![vec![M::zero(); 1 << w]; 2];
        for (a, dp) in dp.into_iter().enumerate() {
            for (b, v) in dp.into_iter().enumerate().filter(|p| !p.1.is_zero()) {
                for i in 0..(1 << w) {
                    if i & wall != 0 {
                        continue;
                    }
                    ...
                    let cnt = (!(j | b | wall) & ((1 << w) - 1)).count_ones() as usize;
                    if a + cnt <= 1 {
                        next[a + cnt][i | (b & mask)] += v;
                    }
                }
            }
        }
    ...
    let ans = dp.into_iter().flatten().fold(M::zero(), |s, a| s + a);
    println!("{}", ans);
}
