...
fn run() {
    input! {
        ...
        k: usize,
        m: usize,
        cond: [(usize1, usize, u32); m],
    }
    let mut cond = cond;
    cond.sort_by_key(|p| (p.1, !p.0));
    let mut ans = M::one();
    for i in 0..k {
        let mut zero = vec![];
        let mut cnt = vec![0; n + 1];
        for &(l, r, x) in cond.iter() {
            let x = x >> i & 1;
            if x == 0 {
                zero.push((l, r));
            } else {
        ...
        let mut id = 0;
        for (index, cnt) in index.iter_mut().zip(cnt) {
            *index = id;
            if cnt == 0 {
                id += 1;
            }
        }
        for p in zero.iter_mut() {
            p.0 = index[p.0];
            p.1 = index[p.1];
            if p.0 == p.1 {
                ans = M::zero();
        ...
        zero.dedup_by(|a, b| a.0 <= b.0 && b.1 <= a.1);
        let mut dp = vec![M::zero(); id];
        dp[id - 1] = M::one();
        let mut sum = M::one();
        let mut po = id;
        for i in (0..(id - 1)).rev() {
            dp[i] = sum;
            sum += dp[i];
            if zero.last().map_or(false, |p| p.0 == i) {
                let (_, r) = zero.pop().unwrap();
                while po > r {
                    po -= 1;
                    sum -= dp[po];
                }
