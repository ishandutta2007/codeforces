...
// cin>>T;while(T--)
{
    ...
    for(lli i=0;i<N;++i)
    {
        ...
        {
            lli cnt=fnd(i);
            if(cmp({cnt,a[i][0].X+a[i][1].X},ans.X))
                ans={{cnt,a[i][0].X+a[i][1].X},{a[i][0].Y,a[i][1].Y}};
        }
        ...
        for(lli j=i+1;j<N;++j)
        {
            if(a[j].empty())
                continue;
            lli cnt=fnd(i|j);
            if(cmp({cnt,a[i][0].X+a[j][0].X},ans.X))
                ans={{cnt,a[i][0].X+a[j][0].X},{a[i][0].Y,a[j][0].Y}};
        }
    }
    dbg(ans);
