...
int main(){
	...
	vector<int> win(1000);
	rep(i,win.size())reps(j,2,i)if(win[i-j]==0 && i%j==0){
		win[i]=1;
		continue;
	}
	out(win[69],win[12],1);
	// reps(i,2,win.size()){
	// 	if(count(all(primes),i)!=1-win[i]) out(i,win[i],1);
	// }
	out(win,1);
	reps(n,1,win.size())if(win[n]!=((n&(n-1))?1-n%2:1-__builtin_ctz(n)%2)) out(n,win[n],1);
	TT{
		cauto n=read();
		cout<<((n<win.size()?win[n]:(n&(n-1))?1-n%2:1-__builtin_ctz(n)%2)?"Alice":"Bob") NL;
	}
	return 0;
}