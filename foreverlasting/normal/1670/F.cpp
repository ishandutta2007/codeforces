...
namespace MAIN{
    ...
    inline int solve(LL m){
        for(res j=0;j<=n;j++)f[j]=0;
        f[0]=1;
        for(res i=59;~i;i--){
            for(res j=0;j<=n;j++)g[j]=0;
            res c=(int)((m>>i)&1),d=(int)((z>>i)&1);
            for(res j=0;j<=n;j++){
                res k=2*j+c;
                for(res x=d;x<=n&&x<=k;x+=2)add(g[min(n,k-x)],mul(f[j],C[n][x]));
            }
            for(res j=0;j<=n;j++)f[j]=g[j];
        }
        res ans=0;
        for(res j=0;j<=n;j++)add(ans,f[j]);
        return ans;
    }
    LL l,r;
