...
int main() {
	...
	tensor<pair<int, int>, 3> dp({S+1, N, MAXK+1});
	{
		vector<pair<int, int>> stk; stk.reserve(N);
		for (int i = N-1; i >= 0; i--) {
			while (!stk.empty() && A[i] > stk.back().first) stk.pop_back();
			stk.push_back({A[i], i});
			int lo = -1, hi = int(stk.size())-1;
			while (hi - lo > 1) {
				int md = (lo + hi) / 2;
				if (stk[md].second <= A[i]) hi = md;
				else lo = md;
			}
			pair<int, int> cur = stk[hi];
			dp[{0, i, 0}] = cur;
			for (int k = 1; k <= MAXK; k++) {
				if (A[i] + k < N) setmax(cur, {A[A[i]+k], A[i]+k});
				dp[{0, i, k}] = cur;
			}
		}
	}
	...
	for (int s = 1; s <= S; s++) {
		for (int i = 0; i < N; i++) {
			for (int k1 = 0; k1 <= MAXK; k1++) {
				int j = dp[{s-1, i, k1}].second;
				for (int k2 = 0; k1+k2 <= MAXK; k2++) {
					setmax(dp[{s, i, k1+k2}], dp[{s-1, j, k2}]);
				}
			}
		}
	}
	...
	while (Q--) {
		...
		cout << [&]() -> int {
			if (L == R) return 0;
			vector<pair<int, int>> cur(K+1, {A[L], L});
			vector<pair<int, int>> nxt(K+1);
			int resp = 0;
			auto is_done = [&](const vector<pair<int, int>>& v) {
				for (int k = 0; k <= K; k++) {
					if (v[k].first + (K-k) >= R) return true;
				}
				return false;
			};
			if (is_done(cur)) return 1;
			int s = 0;
			while (true) {
				for (int k = 0; k <= K; k++) {
					nxt[k] = dp[{s, L, k}];
			...
			while (s) {
				...
				for (int k = 0; k <= K; k++) {
					for (int l = 0; l <= K-k; l++) {
						setmax(nxt[l+k], dp[{s, cur[k].second, l}]);
					}
				}
				if (is_done(nxt)) {
					// do nothing
