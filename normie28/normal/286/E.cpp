...
// Contest: Codeforces - Codeforces Round #176 (Div. 1)
...
int main()
{
        ...
        vector<int> va;
        for (i=0;i<=m;i++) va.push_back(arr[i]);
        vector<int> vb=va;
        vector<int> vc=convolution(va,vb);
        ///for (auto g : va) cout<<g<<' '; cout<<endl;
        ///for (auto g : vb) cout<<g<<' '; cout<<endl;
        ///for (auto g : vc) cout<<g<<' '; cout<<endl;
        for (i=0;i<=m;i++) if ((vc[i])and(!arr[i])) {return cout<<"NO",0;}
        else if ((arr[i])and(!vc[i])) vec.push_back(i);
        cout<<"YES\n";
        cout<<vec.size()<<endl;
