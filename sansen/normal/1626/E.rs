...
impl RerootingOperator for Solver {
    ...
    fn init(&mut self, v: usize) -> Self::Value {
        if self.0[v] == 1 {
            ...
            (std::u32::MAX / 2, 0, std::u32::MAX)
        }
    }
    fn merge(&mut self, p: &Self::Value, c: &Self::Value, _e: &Self::Edge) -> Self::Value {
        let min = p.0.min(c.0 + 1);
        let cnt = p.1 + c.1;
        let mut ans = p.2.min(c.2);
        if cnt >= 2 {
            ans = ans.min(min);
        }
        (min, cnt, ans)
...
fn run() {
    ...
    let mut solver = Rerooting::new(n, Solver(c));
    for (a, b) in e {
        solver.add_edge(a, b, ());
    }
    let mut s = String::new();
    use std::fmt::Write;
    for a in solver.solve() {
        write!(&mut s, "{} ", (a.2 <= 1) as u8).ok();
    }
    s.pop();
    println!("{}", s);
}
