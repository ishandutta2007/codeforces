...
int main() {
    ...
    Poly ans(n + 1);
    int m = n / 2;
    rep(i, 1, m + 2) ans[i] = fact(n - 1);
    {
        Poly f(n);
        rep(i, m + 1, n) f[i] = ifact(i) * fact(i - 1) * fact(n - 1 - i);
        Poly g(n);
        rep(i, n) g[i] = ifact(i);
        f *= g;
        rep(i, 1, m + 2) ans[i] -= f[n - i] * fact(n - i);
        dump(f);
    }
    dump(ans);
    {
        Poly f(n + 1);
        rep(i, 0, n + 1) f[n - i] = ifact(i + 1);
        Poly g(n);
        rep(i, m + 1, n) g[i] = fact(i - 1);
        f *= g;
        dump(f);
        rep(i, 1, m + 2) ans[i] -= f[n + i] * fact(n - i) * (i - 1);
    }
