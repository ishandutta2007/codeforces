...
_ n, a[maxn], b[maxn], dp[maxn][maxn], f[maxn][maxn];
...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
        char ch = gc();
        if(ch == '1' || ch == '0') a[i] = (ch^48)^(i&1);
        else a[i] = -1;
    }
    for(_ i = 1; i <= n; ++i) {
        char ch = gc();
        if(ch == '1' || ch == '0') b[i] = (ch^48)^(i&1);
        else b[i] = -1;
    }
    for(_ i = 0; i <= n; ++i) for(_ j = -i; j <= i; ++j) dp[i][j+big] = f[i][j+big] = 0;
    f[0][big] = 1;
    for(_ i = 0; i <= n-1; ++i) for(_ j = 0; j <= 1; ++j) if(a[i+1] != (j^1)) for(_ k = 0; k <= 1; ++k) if(b[i+1] != (k^1)) for(_ l = -i; l <= i; ++l) {
        dp[i+1][l+j-k+big] = (dp[i+1][l+j-k+big]+dp[i][l+big]+abs(l+j-k)*f[i][l+big])%mod;
        f[i+1][l+j-k+big] = (f[i+1][l+j-k+big]+f[i][l+big])%mod;
    }
    writeln(dp[n][big]);
