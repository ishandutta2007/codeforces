...
fn run() {
    ...
    let w = 2 * k;
    let pc = precalc::Precalc::new(w);
    let mut dp = vec![vec![M::zero(); w + 1]; k + 1];
    dp[0][0] = M::one();
    for d in 2..=w {
        let mut next = dp.clone();
        for (x, dp) in dp.iter().enumerate() {
            for (y, &dp) in dp.iter().enumerate().filter(|p| !p.1.is_zero()) {
                let c = d - 1;
                let mut p = M::one();
                for i in 1.. {
                    p *= pc.inv(d) * pc.inv(i);
                    if x + i * c <= k && y + i * d <= w {
                        next[x + i * c][y + i * d] += p * dp;
                    } else {
                        break;
                    }
                }
