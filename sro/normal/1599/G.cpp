...
int main() {
	...
	if (K == kp) {
		...
		for (int nk = nid; nk < N - 1; ++nk) ans = min(ans, dist(K, id[1]) + dist(id[1], id[nk]) + dist(id[nk], kp) + dist(kp, id[N - 1]) + dist(id[N - 1], id[nk + 1]));
		for (int nk = 2; nk <= nid; ++nk) ans = min(ans, dist(K, id[N - 1]) + dist(id[N - 1], id[nk]) + dist(id[nk], kp) + dist(kp, id[nk - 1]) + dist(id[1], id[nk - 1]));
		for (int nk = nid; nk < N - 1; ++nk) ans = min(ans, dist(K, id[1]) + dist(id[1], id[nk]) + dist(id[nk], kp) + dist(kp, id[nk + 1]) + dist(id[N - 1], id[nk + 1]));
	}
	printf("%.10lf\n", ans);
	return 0;
}