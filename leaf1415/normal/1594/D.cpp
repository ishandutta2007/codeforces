...
struct WeightedUnionFind{
	...
	bool unite(int i, int j, llint x){
		int root_i = root(i), root_j = root(j);
		if(root_i == root_j){
			return get(i, j) == x;
		}
...
int main(void)
{
	...
	while(T--){
		cin >> n >> m;
		UnionFind uf(2*n+5), uf2(2*n+5);

		ll u, v; string s;
		rep(i, 1, m){
			cin >> u >> v >> s;
			uf2.unite(u, v);
			if(s == "crewmate") uf.unite(u, v), uf.unite(u+n, v+n);
			else uf.unite(u, v+n), uf.unite(v, u+n);
		}


		bool flag = true;
		rep(i, 1, n) flag &= !uf.same(i, n+i);
		if(!flag){
			outl(-1);
			continue;
		}
		...
		ll ans = 0;
		rep(i, 1, n){
			if(uf2.root(i) != i) continue;
			ll r = uf.root(i);
			ans += max(mp[r], uf.v[r] - mp[r]);
		}
		outl(ans);
