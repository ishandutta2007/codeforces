...
main(){
    ios
    ll x=lin(),y=lin(),ax=lin(),ay=lin(),bx=lin(),by=lin();
    ll xs=lin(),ys=lin(),t=lin();
    ...
    rep(i,n){
        rep2(j,i,n-1){
            ll d=min(abs<ll>(xs-a[i])+abs<ll>(ys-b[i]),abs<ll>(xs-a[j])+abs<ll>(ys-b[j]));
            d+=a[j]-a[i]+b[j]-b[i];
            if(d<=t) chmax(ans,j-i+1);
        }
    }
    cout<<ans<<endl;
}