...
void solve(){
    ...
    REP(i, n) {
        ...
        if (target.first != -1) {
            vertexs[target.first].push_back(target.second);
            vertexs[target.second].push_back(target.first);
        }
        else {
            if (a == 1) {
                cout << 1 << endl;
            ...
            vertexs[1].push_back(a);
            vertexs[a].push_back(1);
        }
    }
    int ans = 1e9;
    for (int i = 1; i <= 1e3; ++i) {
