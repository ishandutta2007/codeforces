...
string solve(string s){
  ...
  rep(i,26){
    ...
    if(cnt[i]==N){
      ll j;
      rep(j,26)assert(cnt[j]==0||i==j);
      return s;
    }
  }
  rep(i,26)if(cnt[i])break;
  if(cnt[i]<=N/2+1){
    //ans=1,aababababababa... or aababababbbbbbbbbbb...
    s.clear();
    t.clear();
    t+=i+'a';
    s=string(cnt[i]-1,'a'+i);
    cnt[i]=0;
    rep(i,26){
      assert(cnt[i]>=0);
      if(cnt[i])t+=string(cnt[i],'a'+i);
    }
    reverse(all(t));
  ...
  //ans=1, abaaaaaaacbbbbb...
  if(k<26){
    ans+='a'+i;
    ans+='a'+j;
    cnt[j]--;
    ans+=string(cnt[i]-1,i+'a');
    cnt[i]=0;
    ans+='a'+k;
    cnt[k]--;
  }else{
    //ans=1, abbbbbb..bbbaaaaa...
    ans+='a'+i;
    cnt[i]--;
