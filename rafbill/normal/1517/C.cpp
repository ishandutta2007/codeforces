...
int main(){
  ...
  for(li a = 0; a < (li)(n-1); ++a) {
    bool d = true;
    for(li i = 0; i < (li)(n-a); ++i) {
      if(uf.C[uf.find((a+i)*n + i)] == 1+a) {
        d = false;
      }else{
        if(d) {
          uf.unite((a+i)*n + i, (a+i+1)*n + i);
        }else {
          uf.unite((a+i)*n + i, (a+i)*n + i-1);
        }
      }
    }
  }
  ...
  for(li i = 0; i < (li)(n); ++i) {
    for(li j = 0; j < (li)(i+1); ++j) {
      cout << uf.C[uf.find(i*n + j)] << ' ';
    }
    cout << endl;
  }
