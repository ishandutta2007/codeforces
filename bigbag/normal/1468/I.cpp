...
int main() {
    ...
    Point<Int> d1, d2;
    cin >> n >> d1 >> d2;
    long long step = abs(vector_product(d1, d2));
    //cout << "#" << step << endl;
    ...
    if (step == 0) {
        ...
        v.push_back(cur);
        v.push_back(cur + d1);
        v.push_back(cur + d1 + d2);
        v.push_back(cur + d2);
        //cout << "#" << d1 << " " << d2 << endl;
        ans.push_back(cur);
        Segment<Int> s1(v[0], v[1]), s2(v[0], v.back());
        while (cur.y < d1.y + d2.y) {
            while ((cur.y <= d1.y && cw(cur, d1)) || (cur.y > d1.y && cw(cur - d1, d2))) {
                ++cur.x;
            }
            while (contains(v, cur - Point<Int>(1, 0), false) && ans.size() <= n) {
                --cur.x;
            }

            auto p = cur;
            while (contains(v, p, false) && ans.size() <= n) {
                if (contains(v, p, true) || s1.contains(p, true) || s2.contains(p, true)) {
                    ans.push_back(p);
                }
                ++p.x;
            }
