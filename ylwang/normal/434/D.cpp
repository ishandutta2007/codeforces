...
const int MN = 1e4 + 1e3 + 5, MM = 5e5 + 5;
struct Edge {
	int v, w, nxt;	
};
struct Dinic {
...
int n,m,L[MN],R[MN],A[MN],B[MN],C[MN],t=202;
int f(int i,int x) {
...
signed main() {
	...
	int S=0,T=n*t+t+2,ans=lim*n; d.init(S,T);
	For(i,1,n) {
		L[i]=read(),R[i]=read()+1;
		For(j,L[i],R[i]-1) {
			d.addedge(i*t+j,i*t+j+1,lim-f(i,j));
		}
		d.addedge(S,i*t+L[i],INF);
		d.addedge(i*t+R[i],T,INF);
	}
	For(i,1,m) {
		int u=read(),v=read(),dis=read();
		For(j,L[u],R[u]) {
			int k=j-dis;
			if(L[v]<=k&&k<=R[v]) d.addedge(u*t+j,v*t+k,INF);
		}
	}
