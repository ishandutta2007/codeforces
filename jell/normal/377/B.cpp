#line 1 "codeforces-workspace\\contest\\377\\b\\b.cpp"
...
#line 6 "codeforces-workspace\\contest\\377\\b\\b.cpp"
// #include "lib/cxx20"
...
#line 11 "codeforces-workspace\\contest\\377\\b\\b.cpp"
...
void main() {
  ...
  auto check = [&](int k) -> bool {
    if (k > m) return 1;
    vector<pair<i32, i32>> a, b;
    for (auto &&[i, e] : enumerate(_a)) {
      a.emplace_back(e, i);
    }
    for (auto &&[i, e] : enumerate(_b)) {
      b.emplace_back(e, i);
    }
    sort(begin(a), end(a));
    sort(begin(b), end(b));
    auto Q = make_priority_queue(greater<pair<int, int>>());
    i64 cost = 0;
    while (!a.empty()) {
      while (!b.empty() && b.back().first >= a.back().first) {
        Q.emplace(_c[b.back().second], b.back().second);
        b.pop_back();
      }
      if (Q.empty()) {
        return false;
      ...
      for (auto i : range(k)) {
        if (a.empty()) break;
        ans[a.back().second] = Q.top().second;
        a.pop_back();
      }
      Q.pop();
    }
  ...
  if (auto opt = binary_search(m + 1, 0, check); opt > m) {
    cout << "NO"
         << "\n";
  } else {
    assert(check(opt));
