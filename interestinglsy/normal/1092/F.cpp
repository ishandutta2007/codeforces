...
void Dfs1( int pos , int fa ){
	...
	For0(i,e[pos].size()){
		...
		Dfs1(v,pos);
		sum[pos] += sum[v];
		sumdis[pos] += sumdis[v] + sum[v];
	}
}

ll ans = 0;
void Dfs2( int pos , int fa , ll fasum , ll fadissum ){
	ll uans = fadissum + sumdis[pos];
	Mymax(ans,uans);
	For0(i,e[pos].size()){
		int v = e[pos][i];
		if( v == fa ) continue;
		ll usum = fasum + sum[pos] - sum[v];
		ll ufadis = fadissum + usum + sumdis[pos] - sumdis[v] - sum[v];
		Dfs2(v,pos,usum,ufadis);
	}
}
