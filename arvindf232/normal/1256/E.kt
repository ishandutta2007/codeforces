...
fun main(){
    solve.cases{
        val n = getint()
        val L = getline(n)
        val stuff = L.withIndex().sortedBy { it.value }
        val A = stuff.map{it.value}.toIntArray()

        val DP = Array(n+1){IntArray(6){nlarge} }
        val type = IntArray(n){-1}
        DP[0][0] = 0
        for(i in 0 until n){
            for(l in 0 until 5){
                DP[i+1][l+1] = maxOf(DP[i+1][l+1],DP[i][l])
            }
            for(l in 2..5){
                val extra = if(i != n-1) A[i+1]-A[i] else 0
                if(DP[i][l] + extra > DP[i+1][0]){
                    type[i] = l+1
                    DP[i+1][0] = DP[i][l] + extra
                }
            }
        }
        put(A.last() - A.first() - DP.last()[0])
        var tn = 0
        var pointer = n-1
        val ret = IntArray(n)
        while(pointer >= 0){
            val need = type[pointer]
            tn++
            for(i in pointer downTo (pointer - need +1)){
                ret[stuff[i].index] = tn
            }
            pointer -= need
        }
        put(tn)
