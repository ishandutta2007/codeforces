...
struct solver
{
    ...
    solver()
    {
        ...
        //  is 2?
        {
            if(g[0][0]=='A' || g[0][w-1]=='A' || g[h-1][0]=='A' || g[h-1][w-1]=='A')
            {
                cout << ans << "\n";
                return;
            }

            vector<int> ok3(g.front().size(),1);
            for(auto s: g)
            {
                int ni=-1;
                bool ok=false;
            ...
            vector<int> ok4(gt.front().size(),1);
            for(auto s: gt)
            {
                int ni=-1;
                bool ok=false;
