...
mint solvea(vll const& a) {
  vector<vll> h(mk+1,vll(2,0));
  for (ll x:a) {
    for (ll k=0;k<mk;k++) {
      ++h[k][x%2];
      if (x%2==1) break;
  ...
  for (ll k=0;k<mk;k++) {
    mint bad=0;
    if (h[k][1]>0) {
      bad=(mint(2)^h[k][0]) * (mint(2)^(h[k][1]-1));
    }
    allbad+=bad;
  }
  mint all=mint(2)^(a.size());
  return allbad;
}
...
int main() 
{
  ...
  mint allbad=solvea(a);
  mint final=(mint(2)^n)-allbad;
  final-=1;
  printf("%d\n",final.get());
}
