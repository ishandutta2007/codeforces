...
pii merge(pii a, pii b) {
	return {a.first + b.first + 1, a.second + b.second + a.first + b.first};
}
bool can[nax][nax];
vector <pii> so_far;
void dfs(pii x) {
	if (x.first >= nax || x.second >= nax || can[x.first][x.second]) return;
	can[x.first][x.second] = true;
	so_far.push_back(x);
	REP(i, so_far.size()) {
		pii y = so_far[i];
		dfs(merge(x, y));
...
int num;
void go(int n, int d, int r = -1) {
	assert(d >= low[n] && d <= high[n]);
	if (n == 0) return;
	int curr = num;
	num++;
	if (r != -1) {
	...
	for (int n1 = 0; n1 < n; ++n1) {
		for (int d1 : {low[n1], high[n1]}) {
			int n2 = n - n1 - 1, d2 = d - d1 - (n - 1);
			if (d2 >= low[n2] && d2 <= high[n2]) {
				go(n1, d1, curr);
				go(n2, d2, curr);
				return;
			}
...
int main() {
	REP(n, nax) {
		if (n) {
			int l = (n - 1) / 2, r = n - l - 1;
			low[n] = n + low[l] + low[r] - 1;
		}
	}
	REP(n, nax) high[n] = n * (n - 1) / 2;
	int t;
	scanf("%d", &t);
	REP(_, t) {
		int n, d;
		scanf("%d%d", &n, &d);
		debug(arr(low, n), arr(high, n));
		if (d < low[n] || d > high[n]) puts("NO");
		else {
			puts("YES");
			num = 1;
			go(n, d);
