...
void solve() {
	int n, m;
	FOR(i, 0, 24) FOR(j, 0, 24) tab[i][j] = 0;
	
	cin >> n >> m;
	FOR(j, 1, m) if(ok(1, j)) tab[1][j] = 1;
	FOR(i, 1, n) if(ok(i, m)) tab[i][m] = 1;
	for (int j=m; j>0; --j) if (ok(n, j)) tab[n][j] = 1;
	for (int i=n; i>0; --i) if (ok(i, 1)) tab[i][1] = 1;
