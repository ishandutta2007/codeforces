...
int solve(vector<int>& a) {
	...
	vector<vector<int>> grp(35);
	for (auto& i : a) {
		if (i == 0) {
			grp[0].pb(i);
			continue;
		} else {
			for (int b = 30, B = (1 << 30); b >= 0; b--, B >>= 1) {
				if (i & B) {
					grp[b + 1].pb(i ^ B);
					break;
				}
	...
	vector<int> grpkill(35, 0);
	for (int i = 0; i < 35; i++)
		if (!grp[i].empty())
			grpkill[i] = solve(grp[i]);
	...
	for (int i = 0; i < 35; i++) {
		...
		int kill = max(0, belowCnt - 1) + aboveCnt - aboveDistCnt + grpkill[i];
		chkmin(ans, kill);
	}
