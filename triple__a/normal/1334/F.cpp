...
signed main(){
    ...
    SegmentTree<maxn,int> st;
    st.update(2,m+1,-1e16);
    st.update(1,1,-sum);
    for (int i=1;i<=n;++i){
        int u=upper_bound(b+1,b+m+1,a[i])-b;
        if (a[i]==b[u-1]){
            tuple<int,int,int> v=st.query(u,u),w=st.query(u-1,u-1);
            int a,b,c,d,e,f;
            tie(a,b,c)=v, tie(d,e,f)=w;
            if (a+max(::c[i],0ll)<d+::c[i]) st.update(u,u,d-a+::c[i]);
            else st.update(u,u,max(::c[i],0ll));
        }
        if (c[i]>0) st.update(u+1,m+1,c[i]);
    }
    int a,b,c;
    tie(a,b,c)=st.query(m+1,m+1);
    if (a<-1e15) cout<<"NO"<<endl;
    else cout<<"YES "<<-a;
    return 0;
}