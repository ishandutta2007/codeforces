...
int main(){
	...
	combinatorics<Zp> C(200000);
	const int mx = 20;
	int n, m, k, s;
	cin >> n >> m >> k >> s;
	array<int, mx + 3> reduced;
	reduced[0] = s;
	for(auto i = 1; i <= mx + 2; ++ i){
		reduced[i] = reduced[i - 1] + 1 >> 1;
	}
	vector<array<int, 2>> a(k);
	for(auto &[x, y]: a){
		cin >> x >> y, -- x, -- y;
	}
	sort(a.begin(), a.end(), [&](auto p, auto q){ return p[0] + p[1] < q[0] + q[1]; });
	bool beg_bad = false, end_bad = true;
	if(!a.empty() && a.front() == array{0, 0}){
		beg_bad = true;
		a.erase(a.begin());
		-- k;
	}
	if(a.empty() || a.back() != array{n - 1, m - 1}){
		end_bad = false;
		a.insert(a.end(), array{n - 1, m - 1});
		++ k;
	}
	vector<vector<Zp>> ways(k, vector<Zp>(k));
	for(auto u = 0; u < k; ++ u){
		for(auto v = u + 1; v < k; ++ v){
			auto [ux, uy] = a[u];
			auto [vx, vy] = a[v];
			if(ux <= vx && uy <= vy){
				ways[u][v] = C.C(vx - ux + vy - uy, vx - ux);
			}
		}
	}
	vector<array<Zp, mx>> dp(k);
	for(auto u = 0; u < k; ++ u){
		auto [x, y] = a[u];
		dp[u][0] = C.C(x + y, x);
		for(auto v = 0; v < u; ++ v){
			dp[u][0] -= dp[v][0] * ways[v][u];
		}
	}
	for(auto bit = 1; bit < mx; ++ bit){
		for(auto u = 0; u < k; ++ u){
			auto [x, y] = a[u];
			dp[u][bit] = C.C(x + y, x) - accumulate(dp[u].begin(), dp[u].begin() + bit, Zp(0));
			for(auto v = 0; v < u; ++ v){
				dp[u][bit] -= dp[v][bit] * ways[v][u];
			}
		}
	}
	Zp res = 0;
	for(auto bit = 0; bit < mx; ++ bit){
		res += dp[k - 1][bit] * reduced[bit + beg_bad + end_bad];
	}
	res += C.C(n - 1 + m - 1, n - 1) - accumulate(dp[k - 1].begin(), dp[k - 1].end(), Zp(0));
	cout << res / C.C(n - 1 + m - 1, n - 1) << "\n";
	return 0;
}
