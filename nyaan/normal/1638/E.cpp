...
struct Data {
  ll r, col, cur;
  Data() {}
  Data(ll x, ll y, ll z) : r(x), col(y), cur(z) {}
};
...
void q() {
  ...
  map<ll, Data> st;
  HashMap<ll, ll> curval;

  auto erase = [&](ll l, ll r, ll col, ll val) {
    bit.imos(l, r - 1, curval[col] - val);
  };
  auto insert = [&](ll l, ll r, ll col) {
    ll cur = curval[col];
    auto L = st.upper_bound(l), R = st.upper_bound(r);
    if (L != st.begin() && l <= prev(L)->se.r) --L;

    ll lend = R == end(st) ? infLL : R->first;
    V<pair<ll, Data>> nokosu;
    while (L != end(st) and L->fi != lend) {
      // L 
      if (r < L->second.r) {
        nokosu.emplace_back(r, L->se);
      }
      if (L->first < l) {
        Data dat{L->second};
        dat.r = l;
        nokosu.emplace_back(L->fi, dat);
      }
      auto& dat{L->second};
      erase(max(L->fi, l), min(dat.r, r), dat.col, dat.cur);
      L = st.erase(L);
    }
    st[l] = {r, col, cur};
    for (auto& [key, val] : nokosu) {
      if (key != val.r) st[key] = val;
    }
  };
  auto find = [&](ll x) {
    auto it = st.upper_bound(x);
    if (it == st.begin() || (--it)->second.r <= x) return st.end();
    return it;
  };
  ...
  while (Q--) {
    ...
    if (ch == 'C') {
      ...
      ll ans = bit.sum(i);
      auto it = find(i);
      if (it != end(st)) ans += curval[it->se.col] - it->se.cur;
      wtn(ans);
    }
  }
}
