...
int main() {
    ...
    vll p{1};
    rep(i, 17) p.eb(p.back() * 10);
    vv(int, v, 43);
    vi w{0, 4, 7};
    for(auto a : ndFORvector(vi(6, 3))) {
        int m = 0;
        for(auto e : a) m += w[e];
        if(v[m].empty()) {
            for(auto e : a) v[m].eb(w[e]);
        }
    }
    ...
    TEST {
        ...
        if(REC([&](auto &&f, ll n) {
               ...
               auto &res = mp[n];
               rep(i, 43) {
                   if(!empty(v[i]) and n % 10 == i % 10 and n >= i) { res |= f((n - i) / 10); }
               }
               return res;
           })(n)) {
            vll ans(6);
            ...
            while(n) {
                rep(i, 43) {
                    if(!empty(v[i]) and n % 10 == i % 10 and n >= i and mp.count((n - i) / 10) and mp[(n - i) / 10]) {
                        rep(j, 6) ans[j] += p[t] * v[i][j];
                        n = (n - i) / 10;
                        break;
                    }
                }
