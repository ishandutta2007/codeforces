...
void solve() {
  ...
  if (N == 2) {
    ...
    FOR(a, A + 1) FOR(b, B + 1) {
      auto& dp = DP[a][b];
      FOR(x, a) if (DP[x][b] == 0) dp = 1;
      FOR(y, b) if (DP[a][y] == 0) dp = 1;
      FOR(x, min(a, b) + 1) if (x && DP[a - x][b - x] == 0) dp = 1;
    }
    ll a = DP[A][B];
    print((a == 0 ? lose : win));
    return;
  ...
  assert(N == 3);
  LL(A, B, C);
  vvv(bool, DP, A + 1, B + 1, C + 1);
  vvv(bool, DPX, A + 1, B + 1, C + 1);
  vv(bool, DPA, B + 1, C + 1);
  vv(bool, DPB, A + 1, C + 1);
  vv(bool, DPC, A + 1, B + 1);
  FOR(a, A + 1) FOR(b, B + 1) FOR(c, C + 1) {
    ...
    DP[a][b][c] = dp;
    if (a && b && c && DPX[a - 1][b - 1][c - 1]) DPX[a][b][c] = 1;
  }
  ll a = DP[A][B][C];
  print((a == 0 ? lose : win));
}
