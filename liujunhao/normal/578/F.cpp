...
//template
#define MAXN 500
int ra[MAXN*MAXN+10],rb[MAXN*MAXN+10],b[MAXN+10][MAXN+10],n,m,MOD,fa[MAXN*MAXN+10],ans,cnta,cntb;
...
int gaussian_elimination(int a[MAXN+10][MAXN+10],int n){
	int row,col,i,j,ret(1);
	for(row=col=1;col<n;col++){
		for(i=row;i<n;i++)
			...
			if(i>=n)
				return 0;
		int inv=quick_pow(a[row][col],MOD-2);
		for(i=row+1;i<n;i++){
			for(j=col+1;j<n;j++)
				a[i][j]=(a[i][j]+(MOD-1ll)*a[row][j]%MOD*a[i][col]%MOD*inv)%MOD;
			a[i][col]=0;
		}
		row++;
	}
...
int main()
{
	...
	for(i=1;i<=n;i++)
		for(j=1;j<=m;j++)
			if(s[i][j]=='\\')
				Union(Get_id(i,j),Get_id(i+1,j+1));
			else if(s[i][j]=='/')
				Union(Get_id(i+1,j),Get_id(i,j+1));
	for(i=1;i<=n+1;i++)
		for(j=1;j<=m+1;j++)
			if(find(Get_id(i,j))==Get_id(i,j)){
				if((i+j)&1)
					ra[Get_id(i,j)]=++cnta;
				else
					rb[Get_id(i,j)]=++cntb;
	...
	for(i=1;i<=n;i++)
		for(j=1;j<=m;j++)
			if(s[i][j]=='*'){
				if((i+j)&1){
					if(find(Get_id(i,j))!=find(Get_id(i+1,j+1))){
						b[ra[fa[Get_id(i,j)]]][ra[fa[Get_id(i+1,j+1)]]]++;
						b[ra[fa[Get_id(i+1,j+1)]]][ra[fa[Get_id(i,j)]]]++;
					}
				}
				else{
					if(find(Get_id(i+1,j))!=find(Get_id(i,j+1))){
						b[ra[fa[Get_id(i+1,j)]]][ra[fa[Get_id(i,j+1)]]]++;
						b[ra[fa[Get_id(i,j+1)]]][ra[fa[Get_id(i+1,j)]]]++;
					}
				}
			}
	for(i=1;i<=cnta;i++)
		for(j=1;j<=cnta;j++)
			if(i!=j)
				b[i][i]+=b[i][j],b[i][j]=(MOD-b[i][j])%MOD;
	ans=gaussian_elimination(b,cnta);
	memset(b,0,sizeof b);
	for(i=1;i<=n;i++)
		for(j=1;j<=m;j++)
			if(s[i][j]=='*'){
				if(!((i+j)&1)){
					if(find(Get_id(i,j))!=find(Get_id(i+1,j+1))){
						b[rb[fa[Get_id(i,j)]]][rb[fa[Get_id(i+1,j+1)]]]++;
						b[rb[fa[Get_id(i+1,j+1)]]][rb[fa[Get_id(i,j)]]]++;
					}
				}
				else{
					if(find(Get_id(i+1,j))!=find(Get_id(i,j+1))){
						b[rb[fa[Get_id(i+1,j)]]][rb[fa[Get_id(i,j+1)]]]++;
						b[rb[fa[Get_id(i,j+1)]]][rb[fa[Get_id(i+1,j)]]]++;
					}
				}
			}
	for(i=1;i<=cntb;i++)
		for(j=1;j<=cntb;j++)
			if(i!=j)
				b[i][i]+=b[i][j],b[i][j]=(MOD-b[i][j])%MOD;
	ans=(ans+gaussian_elimination(b,cntb))%MOD;
	cout<<ans<<endl;
}