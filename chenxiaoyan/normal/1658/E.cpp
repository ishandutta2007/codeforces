...
struct rect {
	int l1, r1, l2, r2;
	rect() { l1 = l2 = -inf, r1 = r2 = inf; }
	rect(int l1, int r1, int l2, int r2) : l1(l1), r1(r1), l2(l2), r2(r2) {}
	rect(int xx, int yy) {
		int x = xx + yy, y = xx - yy;
		l1 = x - k, r1 = x + k, l2 = y - k, r2 = y + k;
	}
	friend rect operator&(rect f, rect g) {
		return rect(max(f.l1, g.l1), min(f.r1, g.r1), max(f.l2, g.l2), min(f.r2, g.r2));
	}
...
void mian() {
	n = read(), k = read();
	REP(i, 1, n) REP(j, 1, n) a[i][j] = read(), ans[i][j] = 'G';
	vii ord; REP(i, 1, n) REP(j, 1, n) ord.pb(i, j);
	sort(ALL(ord), [&](tii x, tii y) { return a[X(x)][Y(x)] > a[X(y)][Y(y)]; });
	rect inter;
	for(tii t : ord) {
		int x, y; tie(x, y) = t;
		if(inter.in(x, y)) ans[x][y] = 'M', inter = inter & rect(x, y);
	}
