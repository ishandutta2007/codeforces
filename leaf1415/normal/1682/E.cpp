...
int main(void)
{
	...
	reps(i, cvec) reps(j, cvec[i]) q[cvec[i][j]] = P(i, j);
	
	ll a, b;
	rep(i, 1, m){
		cin >> a >> b;
		a--, b--;
		vec[a].pb(P(modulo(q[b].se-q[a].se, sz(cvec[q[a].fi])), i));
		vec[b].pb(P(modulo(q[a].se-q[b].se, sz(cvec[q[b].fi])), i));
	}
	...
	rep(i, 0, n-1){
		//outl(vec[i]);
		rep(j, 1, sz(vec[i])-1){
			scc.add_edge(vec[i][j-1].se, vec[i][j].se);
		}
	}
	scc.tpsort();
	outl(scc.topo);
