...
int main() {
    ...
    vector<long long> w;
    rep(i, n) {
        w.push_back(a[i + 1] - a[i]);
    }
    sort(all(w), greater<int>());

    auto get_cost = [&](long long len, long long x) {
        long long p = len / x, q = len % x;
        return q * (p + 1) * (p + 1) + (x - q) * p * p;
    };

    auto f = [&](long long p) -> pair<bool, long long> {
        long long pnum = 0;
        long long num = 0;
        ...
        for (long long len : w) {
            int fr, to;
            {
                ...
                while (r - l > 1) {
                    int x = (l + r) >> 1;
                    (get_cost(len, x) - get_cost(len, x + 1) > p ? l : r) = x;
                }
                cost_sum += get_cost(len, l + 1);
                num += l;
        ...
        long long k = max(0LL, cld(cost_sum - m, p));
        if (k > pnum) {
            return { false, -1 };
        } else {
            return { true, num + k };
