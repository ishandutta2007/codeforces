...
void dfs2(int v, int p)
{
	for(auto u : G[v]){
		if(u == p) continue;
		ll S = sum[u], T = sum[1] - sum[u];
		
		ll g = gcd(S-1, T+1), h = gcd(S+1, T-1);
		vector<ll> vec = dvec[g];
		vec.insert(vec.end(), all(dvec[h]));
		uniq(vec);
		//outl(u, v, S, T), outl(vec);
		for(auto x : vec) if(x <= n) cnt[x]++;
		dfs2(u, v);
	}
}
...
bool dfs3(int v, int p, int g)
{
	...
	bool ret = true;
	if(p != -1 && (deg[v]+1) % g == sz(G[v]) % g) ret = false, deg[v]++;
	else if(deg[v] % g != sz(G[v]) % g) cnt[g] = 0;
...
int main(void)
{
	...
	while(T--){
		...
		per(i, n, 1) for(int j = 2*i; j <= n; j+=i) dp[i] -= dp[j];
		outa(dp, 1, n);
	}
