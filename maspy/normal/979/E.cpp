...
void solve() {
  ...
  */
  ...
  FOR(a, N + 1) FOR(b, N + 1) FOR(c, N + 1) FOR(d, N + 1) {
    ...
    if (n == N) {
      if ((b + d + parity) % 2 == 0) ANS += dp;
      break;
    }
    FOR(color, 2) {
      if (color == 0 && A[n] == 1) continue;
      if (color == 1 && A[n] == 0) continue;

      if (color == 0) {
        mint cnt1 = dp * mint(2).pow(a + b + c);
        FOR(y, d + 1) {
          mint add = cnt1 * C<mint>(d, y);
          if (y % 2 == 0) {
            DP[a][b + 1][c][d] += add;
          } else {
            DP[a + 1][b][c][d] += add;
        ...
        mint cnt1 = dp * mint(2).pow(c + d + a);
        FOR(y, b + 1) {
          mint add = cnt1 * C<mint>(b, y);
          if (y % 2 == 0) {
            DP[a][b][c][d + 1] += add;
          } else {
            DP[a][b][c + 1][d] += add;
