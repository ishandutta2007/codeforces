...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/other/discrete_log.hpp"
// log_a b 
// *  H 
// lb 
template <typename X>
...
#line 3 "/home/maspy/compro/library/mod/primitive_root.hpp"
...
void solve() {
  LL(K);
  auto g0 = primitive_root(mint::get_mod());
  VEC(ll, A, K);
  LL(N, M);
  vc<mint2> g(K + 1);
  g[0] = 1;
  FOR(k, K) { g[k + 1] = -mint2(A[k]); }

  vc<mint2> f(K);
  f.back() = mint2(1);
  mint a = mint(g0).pow(coef_of_rational_fps<mint2>(f, g, N - 1).val);

  ll r = discrete_log<mint>(
      a, mint(M), mint::get_mod(), [](mint x) { return x.val; }, 0);
  if (r == -1) return print(-1);
  print(mint(g0).pow(r));
}
