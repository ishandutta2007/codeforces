...
void solve() {
    ...
    dp.assign(n, mp(1e9, 1e9));
    int a = 0, b = n-1;
    while (!board[0][a] && !board[1][a]) a++;
    while (!board[0][b] && !board[1][b]) b--;
    if (board[0][a] && board[1][a]) dp[a] = mp(1, 1);
    else if (board[0][a]) dp[a] = mp(0, 1);
    else dp[a] = mp(1, 0);
    FOR(i, a+1, b+1) {
        pi pre = dp[i-1], cur;
        if (board[1][i]) cur.F = min(pre.F+2, pre.S+2);
        else cur.F = min(pre.F+1, pre.S+2);
        if (board[0][i]) cur.S = min(pre.F+2, pre.S+2);
        else cur.S = min(pre.F+2, pre.S+1);
        dp[i] = cur;
    }
    cout << min(dp[b].F, dp[b].S) << nl;
}
