...
vpii poss(int n) {
	if (n % 2 == 0) return {};
	if (n == 9) return {{1, 1}, {3, 3}};
	int half = n / 2;
	if (n & (n + 1)) return {{1, half - 1}};
	if (half - 1 >= 2) return {{0, 0}, {2, half - 1}};
	return {{0, 0}};
}
...
void go(int n, int k, int r) {
	debug(imie(n), imie(k), imie(r));
	if (n == 1) {
		ans[get()] = r;
		return;
	}
	...
	for (int s = 1; s * 2 <= n; s += 2) {
		vpii h1 = poss(s), h2 = poss(n - s - 1);
		int off = s * 2 <= n - s - 1;
		debug(imie(n), imie(s), imie(h1), imie(h2), imie(k));
		bool can = false;
		int kk = k - off;
		int k1 = -1, k2 = -1;
		for (auto [a, b] : h1) for (auto [c, d] : h2) if (a + c <= kk && kk <= b + d) {
			can = true;
			if (c <= kk - a && kk - a <= d) k1 = a, k2 = kk - a;
			if (c <= kk - b && kk - b <= d) k1 = b, k2 = kk - b;
			if (a <= kk - c && kk - c <= b) k2 = c, k1 = kk - c;
			if (a <= kk - d && kk - d <= b) k2 = d, k1 = kk - d;
		}
		if (can) {
			go(s, k1, h);
			go(n - s - 1, k2, h);
...
void solve() {
	...
	bool can = false;
	debug(fun(poss, n));
	for (auto [a, b] : poss(n)) if (a <= k && k <= b) can = true;
	if (can) {
		printf("YES\n");
		go(n, k, 0);
