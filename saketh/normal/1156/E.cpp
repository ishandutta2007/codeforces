...
void mark(int v) { ok[v] = true; todo.pb(v); }
void clr() { trav (e, todo) ok[e] = false; todo.clear(); }
...
void solve(int l, int r) {
    ...
    vpii right, left;
    int c = 0;
    FOR (i, m, r) { ckmax(c, p[i]); right.eb(c, p[i]); }
    c = 0;
    FORd (i, l, m) { ckmax(c, p[i]); left.eb(c, p[i]); }

    int j = 0;
    trav (e, right) {
        while (j < sz(left) && left[j].f < e.f) mark(left[j++].s);
        ans += ok[e.f - e.s];
    }
    clr(); j = 0;
    trav (e, left) {
        while (j < sz(right) && right[j].f < e.f) mark(right[j++].s);
        ans += ok[e.f - e.s];
    }
    clr();
}
