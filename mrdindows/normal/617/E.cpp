...
vector<pii> v[101010];
const int N = 100000;
static int tr[101010 * 32];
static int lp[101010 * 32];
static int rp[101010 * 32];
int lastn = 1;
...
int getSum(int cur, int l, int r, int x, int y) {
    if (cur == 0) return 0;
    if (x > r || y < l) return 0;
    if (x <= l && r <= y) {
        return tr[cur];
        ...
        int m = (l + r) / 2;
        return getSum(lp[cur], l, m, x, y) + getSum(rp[cur], m + 1, r, x, y);
    }
}
...
int main() {
    ...
    sort(q, q + m, [](const tri& t1, const tri &t2){
        return t1.a / K < t2.a / K || t1.a / K == t2.a / K && t1.b < t2.b;
    });
    auto add = [&](int x) {
        res += c[x ^ k];
        c[x]++;
