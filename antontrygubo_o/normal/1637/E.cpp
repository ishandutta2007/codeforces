...
void solve()
{
    ...
    vector<vector<int>> guys(n+1);
    for (auto it: cnt)
    {
        guys[it.second].push_back(it.first);
    }

    for (int i = 0; i<=n; i++)
    {
        sort(guys[i].begin(), guys[i].end());
        reverse(guys[i].begin(), guys[i].end());
    }
    ...
    vector<int> nonempty;
    for (int i = 0; i<=n; i++) if (!guys[i].empty()) nonempty.push_back(i);

    ll best = 0;

    for (auto it: cnt)
    {
        int x = it.first; int cnt_x = it.second;
        for (auto cnt_y: nonempty)
        {
            for (auto y: guys[cnt_y])
            {
                if (check(x, y))
                {
                    best = max(best, 1ll*(x+y)*(cnt_x+cnt_y));
                    break;
                }
            }
        }
