...
fun main(){
    solve.cases{
        ...
        val twos = IntArray(n){(L[it] == 2 || L[it] == -2).chi}.torsq()
        val signs = IntArray(n){(L[it] < 0).chi}.torsq()


        var best = 0
        var bestpos = Pair(n,0)
        var left = 0
        while(left < n){
            if(L[left] == 0) {
                left ++
            ...
            val right = (left..n).first{ it == n || L[it] == 0} -1
            for(l in left..right){
                if(signs.sumQuery(l,right) % 2L == 0L){
                    val here = twos.sumQuery(l,right)
                    if(here > best){
                        best  = here.toInt()
                        bestpos = Pair(l,n-1-right)
                    }
                }
            }
            for(r in left..right){
                if(signs.sumQuery(left,r) % 2L == 0L){
                    val here = twos.sumQuery(left,r)
                    if(here > best){
                        best  = here.toInt()
                        bestpos = Pair(left,n-1-r)
                    }
                }
            }
            left = right + 1
