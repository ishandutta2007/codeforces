#line 1 "codeforces-workspace/contest/1503/d/d.cpp"
...
#line 6 "codeforces-workspace/contest/1503/d/d.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1503/d/d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1503/d/d.cpp"
...
#line 27 "codeforces-workspace/contest/1503/d/d.cpp"
...
void main() {
  ...
  // descending order of min
  sort(rbegin(cards), rend(cards), [](auto x, auto y) {
    return min(x.first, x.second) < min(y.first, y.second);
  });

  struct endo {
    u32 v = 0;
    endo operator*(endo r) const { return {~v && ~r.v ? v + r.v : -1}; }
  };

  struct mono {
    u32 v = -1;
    mono operator+(mono r) const { return {min(v, r.v)}; }
    mono operator*(endo r) const { return {~v && ~r.v ? v + r.v : -1}; }
  };

  lazy_segment_tree<mono, endo> lt(n * 2);
  auto [pmi, pma] = minmax({cards[0].first, cards[0].second});
  auto ltl = lt, ltr = lt, rt = lt, rtl = lt, rtr = lt;
  lt[pma] = rt[pma] = {0};
  ltl[pma] = rtl[pma] = {pma == cards[0].second};
  ltr[pma] = rtr[pma] = {pma != cards[0].second};

  if (auto mima = min_element(begin(cards), end(cards),
                              [](auto x, auto y) {
                                return max(x.first, x.second) <
                                       max(y.first, y.second);
                              });
      max(mima->first, mima->second) < pmi)
    eject(-1);
  ...
  for (auto [x, y] : cards) {
    ...
    auto dplr = ltr.fold(ma, n + n).v;
    auto dpll = ltl.fold(0, ma).v;
    auto dpl = min({dpll, dplr, lt.fold(0, ma).v});

    auto dprl = rtl.fold(ma, n + n).v;
    auto dprr = rtr.fold(0, ma).v;
    auto dpr = min({rt.fold(0, ma).v, dprl, dprr});
    ...
    if (~dpr) chle(lt[pma].v, dpr + lc);
    if (~dprl) chle(ltl[pma].v, dprl + lc);
    if (~dprr) chle(ltr[pma].v, dprr + lc);

    if (~dpl) chle(rt[pma].v, dpl + rc);
    if (~dplr) chle(rtr[pma].v, dplr + rc);
    if (~dpll) chle(rtl[pma].v, dpll + rc);
