...
int getGcd(int cur, int l, int r, int x, int y) {
    if (y < l || x > r) {
        return 0;
    }
    ...
    int m = (l + r) / 2;
    int dcur = cur + cur;
    return gcd(getGcd(dcur, l, m, x, y), getGcd(dcur + 1, m + 1, r, x, y));
}
int goRight(int cur, int l, int r, int from, int val) {
    if (l == r) {
        return gcd(tr[cur], val) == val ? l + 1 : l;
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        if (l >= from) {
            if (gcd(tr[cur], val) == val) {
                return r + 1;
            }
        }
        if (from > m) {
            return goRight(dcur + 1, m + 1, r, from, val);
        } else {
            int where = goRight(dcur, l, m, from, val);
            if (where > m) {
                return goRight(dcur + 1, m + 1, r, from, val);
            } else {
                return where;
...
//            }
//            int pos = l;
            int pos = goRight(1, 0, n - 1, from, x);
            if (pos >= to || gcd(getGcd(1, 0, n - 1, pos + 1, to), x) == x) {
                cout << "YES\n";
            } else {
                cout << "NO\n";
            }
