	...
	void solve()
	{
		...
		for(int i = 0;i < m;){
			...
			while(j < m && 
				...
				root = insertb(root, new Node(fi[j][1]));
				j++;
			}
			
			while(p < Q){
				if(j < m && qs[p][0] >= done + (long)(fi[j][0] - fi[i][0]) * count(root)){
					break;
				}
				anss[(int)qs[p][1]] = get(root, (int)((qs[p][0] - done) % count(root))).v + 1;
				p++;
			}
			if(j < m)done += (long)(fi[j][0] - fi[i][0]) * count(root);
