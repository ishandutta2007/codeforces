...
void solve(){
	...
	SORT(v);
	int nxt = 0;
	while(nxt < n && f.query(1, m-1, 0, 0, s-1) == 0){
		f.update(v[nxt].b.a, v[nxt].b.b-1, 0, 0, s-1, 1);
		nxt ++;
	}
	int ans = v[nxt-1].a - v[0].a;
	rep(i, n){
		f.update(v[i].b.a, v[i].b.b-1, 0, 0, s-1, -1);
		while(nxt < n && f.query(1, m-1, 0, 0, s-1) == 0){
			f.update(v[nxt].b.a, v[nxt].b.b-1, 0, 0, s-1, 1);
			nxt ++;
		}
		if(f.query(1, m-1, 0, 0, s-1) == 0) break;
		chmin(ans, v[nxt-1].a - v[i+1].a);
	}
	o(ans);
}
signed main(){
