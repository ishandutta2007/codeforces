...
template <class S, S (*op)(S, S), S (*e)()> struct SegmentTree {
  ...
  private:
    ...
    void apply(int k) { d[k] = op(d[2 * k], d[2 * k + 1]); }
};
...
void solve(){
  ...
  int M = id.size();
  vector<P> tmp(M,P(INF,INF));
  REP(i,N){
    chmin(tmp[id[b[i]]],P(id[a[i]],i));
  }

  SegmentTree<P,f,E> seg(tmp);
  REP(i,N){
    P res = seg.query(0,id[b[i]]);
    if(res.sc == INF || res.fs >= id[a[i]]) cout << -1 << " ";
    else cout << res.sc + 1 << " ";
  }
