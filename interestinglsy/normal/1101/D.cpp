...
int intree[MAXN], intree_token;
int vis[MAXN], vis_token;
int temp_ans;
int Dfs( int pos , int fa ){
	vis[pos] = vis_token;
	int maxx = 0, second_max = 0;
	for( auto v : e[pos] ){
		if( intree[v] != intree_token || v == fa ) continue;
		int temp_ret = Dfs(v,pos);
		if( temp_ret > maxx ){
			second_max = maxx;
			maxx = temp_ret;
		}else
			Mymax(second_max,temp_ret);
	}
	Mymax(temp_ans,second_max+maxx+1);
	return maxx+1;
}
int Solve( auto &tree ){
	...
	intree_token = rand();
	vis_token = rand();
	for( auto u : tree ) intree[u] = intree_token;
	for( auto u : tree )
		if( vis[u] != vis_token ){
			Dfs(u,0);
		}
