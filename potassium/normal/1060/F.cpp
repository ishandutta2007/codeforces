...
int main() {
	...
	for (int root = 1; root <= n; root++) {
		vector<vector<double>> dp1(n + 1), dp2 = dp1;
		function<void(int, int)> dfs = [&](int u, int p) {
			...
			for (auto& v : g[u]) {
				...
				const int sz1 = (int)dp1[u].size() - 1;
				const int sz2 = (int)dp2[v].size() - 1;
				
				vector<double> tmp(sz1 + sz2 + 1, 0);
				for (int i = 0; i <= sz1; i++)
					for (int j = 0; j <= sz2; j++)
						tmp[i + j] += (dp1[u][i] * dp2[v][j]) * C(i + j, i) * C(sz1 + sz2 - i - j, sz1 - i);

				dp1[u] = tmp;
			}
			
			const int sz = (int)dp1[u].size() - 1;
			dp2[u].resize(sz + 2, 0);
