...
int main(){
    ...
    static bool ans[200008];
    int n,t;
    sc(n,t);
    pi arr[n];
    int sind[n];
    FORR(i,arr)
        sc(i.e1, i.e2); //{maxcnt,time}
    FOR(i,0,n)
        sind[i] = i;
    sort(sind,sind+n,[&](int f, int s){return arr[f].e2<arr[s].e2;});
    int cnt = 0, time = 0;
    set<pi> mp[n+2]; //[maxcnt], {{-times,index}}
    FORR(i,sind){
        if(cnt >= arr[i].e1) continue;
        if(time + arr[i].e2 <= t){
            time += arr[i].e2;
            ans[i] = 1;
            ++cnt;
            mp[arr[i].e1+1].insert({-arr[i].e2,i});//+1 off bcs when it is bad
            set<pi> &it = mp[cnt];
            if(it.empty()){ continue;}
            --cnt;
            time += it.begin()->first;
            ans[it.begin()->second] = 0;
            it.erase(it.begin());
        }
        else
            break;
