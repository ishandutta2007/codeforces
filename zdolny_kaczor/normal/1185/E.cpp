...
void solve() {
	...
	map <char, vpii> where;
	REP(i, n) REP(j, m) if (in[i][j] != '.') where[in[i][j]].emplace_back(i, j);
	if (where.empty()) {
		printf("YES\n0\n");
		return;
	}
	char last = where.rbegin()->first;
	for (char curr = last; curr >= 'a'; --curr) {
		auto poses = where[curr];
		if (poses.empty()) {
			if (curr == last) {
				printf("NO\n");
				return;
		...
		if (xes.size() >= 2u && yes.size() >= 2u) {
			printf("NO\n");
			return;
		}
		int x1 = *xes.begin(), x2 = *xes.rbegin();
		int y1 = *yes.begin(), y2 = *yes.rbegin();
