...
#define MAXN 1500
int n,s,m,k;
int f[MAXN+10][MAXN+10],a[MAXN+10],tot,c[MAXN+10],num[MAXN+10];
pii b[MAXN+10];
bool check(int now){
	now=c[now];
	num[0]=0;
	...
	for(i=1;i<=m;i++){
		k=1;
		for(j=1;j<=s;j++){
			while(k<j&&b[k].second<b[j].first)
				k++;
			f[i][j]=max(f[i][j-1],f[i-1][j]);
			if(k!=j)
				f[i][j]=max(f[i][j],f[i-1][k]+num[b[j].second]-num[b[k].second]);
			f[i][j]=max(f[i][j],f[i-1][k-1]+num[b[j].second]-num[b[j].first-1]);
			if(f[i][j]>=::k)
				return 1;
		}
	}
...
int main()
{
	...
	sort(b+1,b+s+1,[&](const pii &x,const pii &y){if(x.first==y.first)return x.second>y.second;return x.first<y.first;});
	for(i=1;i<=s;i++){
		if(b[i].second<=mx)
			b[i].first=INF;
		else{
			tot+=min(b[i].second-b[i].first+1,b[i].second-mx);
			mx=b[i].second;
		}
	}
	sort(b+1,b+s+1);
	while(b[s].first==INF)
		s--;
	sort(b+1,b+s+1,[&](const pii &x,const pii &y){return x.second<y.second;});
	sort(c+1,c+n+1);
	c[n+1]=-1;
	printf("%d\n",c[Divide_Conqure(1,n+1)]);
}