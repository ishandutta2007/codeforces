...
int dp[MAXV][MAXV/2]; // agg, ct

int main() {
    setIO();

    int N, M; re(N, M);
    vi agg(N); re(agg); trav (e, agg) --e;
    vi cost(N); re(cost);
    vi score(N + M); re(score);

    const int NINF = 0xAAAAAAAA;
    memset(dp, 0xAA, sizeof(dp));

    auto recalc = [&](int agg, int max_ct) {
        while (agg+1 < N + M) {
            F0R (ct, max_ct + 1)
                if (dp[agg][ct] != NINF)
                    ckmax(dp[agg+1][ct/2], dp[agg][ct] + score[agg+1] * (ct/2));
            agg++;
            max_ct /= 2;
        }
    };
    ...
    F0Rd (i, N) {
        int last = -1;
        F0Rd (ct, MAXV/2 - 1) if (dp[agg[i]][ct] != NINF) {
            ckmax(dp[agg[i]][ct+1], dp[agg[i]][ct] + score[agg[i]] - cost[i]);
            if (last == -1) last = ct + 1;
        }
        recalc(agg[i], last);
    }

    ps(*max_element(dp[N+M-1], dp[N+M-1] + MAXV/2));
