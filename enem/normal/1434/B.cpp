...
int main()
{
    ...
    static ll n,t,gpt=0;
    static vector<ll> data[N<<1];
    static vector<pll> stk,ans;
    static char res;
    ...
    for(int i=0;i<2*n;i++)
    {
        ...
        if(res=='+')
        {
            if(!bad&&(stk.empty()||stk.back().F!=0))
            {
                stk.PB(MP(0,gpt));
                data[gpt].PB(i);
                gpt++;
            }
            else if(!bad&&(stk.back().F==0))
            {
                data[stk.back().S].PB(i);
            }
        }
        else
        {
            cin>>t;
            if(!bad&&(stk.empty()||stk.back().F>=t))
            {
                bad=true;
            }
            else if(!bad)
            {
                ans.PB(MP(data[stk.back().S].back(),t));
                data[stk.back().S].DB();

                V(ll) tmp;
                while(!stk.empty()&&stk.back().F<=t)
                {
                    if(!data[stk.back().S].empty()) tmp.PB(stk.back().S);
                    stk.DB();
                }
                ...
                if(!tmp.empty())
                {
                    ...
                    for(auto it:tmp)
                    {
                        if(data[it].size()>data[mx].size()) mx=it;
                    }
                    for(auto it:tmp)
                    {
                        if(it!=mx)
                        {
                            for(auto itt:data[it])
                            {
                                data[mx].PB(itt);
                            }
                            data[it].clear();
