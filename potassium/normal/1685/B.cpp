...
int main() {
	CASE(t) {
		...
		for (int i = 0, last = 0; i <= n; i++) {
			if ((i == n) || (last < i && s[i] == s[i - 1])) {
				...
				if (len % 2 == 1) {
					...
					if (isAB) {
						abc += len / 2;
						abl.emplace_back(len / 2);
					}
					else {
						bac += len / 2;
						bal.emplace_back(len / 2);
					}
				}
				last = i;
			}
		...
		while (min(abc, ab) + min(bac, ba) + res < ab + ba) {
			if (abc <= ab && bac <= ba) break;

			vector<int>& vec = (abc > ab ? abl : bal);
			int& cnt = (abc > ab ? abc : bac);
			assert(!vec.empty());
		...
		W(YN(min(abc, ab) + min(bac, ba) + res >= ab + ba));
	}
