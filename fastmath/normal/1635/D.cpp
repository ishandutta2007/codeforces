    ...
    rep (t) {
        ...
        each (e, a) {
            ...
            for (int i = (int)s.size() - 1; i >= 0; --i) {
                if (us.find(s.substr(0, i + 1)) != us.end() && cur%2==0) {
                    used = 1;
                    break;
                }
                if (s[i] == '0') {
            ...
            debug(len - (int)s.size());
            //debug(pref[len - (int)s.size()]);

            if (len - (int)s.size() >= 0) ans += pref[len - (int)s.size()];
            us.insert(s);
        }
        cout << ans << endl;
