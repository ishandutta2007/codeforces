...
//  REP(i, 75) write((i&1) ? '1' : '0'); writeln();
  ...
  REP(i, n) {
    ...
    REP(x, M) {
      int y = x * 2 + (s[i] - '0');
      if (y >= M) continue;
      REP(u, 1<<M) if (dp[o][x][u]) {
        if (y != 0) {
          int v = u | 1<<(y-1);
          inc(dp[o^1][0][v], dp[o][x][u]);
          if ((v + 1 & v) == 0) inc(ans, dp[o][x][u]);
        }
        inc(dp[o^1][y][u], dp[o][x][u]);
      }
    }
    o = o^1;
//    REP(x, M) REP(u, 1<<M) if (dp[o][x][u])
//      writeln(i+1, x, u, ": ", dp[o][x][u]);
  }
  writeln(ans);
