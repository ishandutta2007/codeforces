...
public class F {
	...
	void solve()
	{
		...
		int[] to = new int[m+n];
		int[] w = new int[m+n];
		DJSet xds = new DJSet(n);
		
		for(int i = 0;i < m;i++){
			from[i] = ni()-1;
			to[i] = ni()-1;
			xds.union(from[i], to[i]);
			w[i] = i;
		}
		int p = m;
		for(int i = 0;i < n;i++){
	...
	void dfs(int cur, int pre, int dep)
	{
		...
		for(int[] e : g[cur]){
			if(e[0] == pre)continue;
			if(imap[e[0]] != -1){
//				tr(cur, e, Arrays.copyOf(hist, dep), imap[e[0]], dep);
				ds.sum[ds.root(e[1])]++;
				int start = lst.prev(imap[e[0]]);
				ds.union(hist[start], e[1]);
				for(int i = lst.next(start+1);i != -1 && i < dep;i = lst.next(start+1)){
					lst.unset(i);
					ds.union(hist[i], e[1]);
				}
			}else if(!ved[e[0]]){
