...
void solve(){
  ...
  seen.assign(a, false), finished.assign(a, false);
    pos = -1;
  dfs(G, 0, -1);
  set<int> cycle;
    while (!hist.empty()) {
...
//  cout<<tmp<<" "<<p<<" "<<q<<endl;
//  cout<<t.size(tmp)<<" "<<t.size(p)<<" "<<t.size(q)<<endl;
  ans-=((t.size(tmp)*(t.size(tmp)-1))/2+((a-t.size(tmp))*((a-t.size(tmp))-1))/2);
//  cout<<((t.size(tmp)*(t.size(tmp)-1))/2+((a-t.size(tmp))*((a-t.size(tmp))-1))/2)<<endl;
//  cout<<ans<<endl;
  ans+=((a-t.size(tmp))*((a-t.size(tmp))-1))/2;
  for(int i=0;i<a;i++){
    if(cycle.count(i)){
      if(i!=tmp){
        ans-=(t.size(i)*(t.size(i)-1))/2;
      }
    }
  }
  cout<<ans<<endl;
