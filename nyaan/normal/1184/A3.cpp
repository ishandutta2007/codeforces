...
// totient function (N)=(1 ~ N , gcd(i,N) = 1)
// {0, 1, 1, 2, 4, 2, 6, 4, ... }
...
void Nyaan::solve() {
  ...
  for (int mod = M;; mod++) {
    if (is_prime(mod) == false) continue;
    mint::set_mod(mod);
    int pr = PrimitiveRoot(mod);
    vector<mint> a(mod - 1);
    rep(i, N) a[i % (mod - 1)] += S[i] - T[i];

    auto A = ChirpZ<mint>(a, pr);
    ll x = 1;
    rep(i, mod - 1) {
      if (A[i] == 0 and x != 1 and x != mod - 1) {
        out(mod, x);
        return;
      }
      x = x * pr % mod;
