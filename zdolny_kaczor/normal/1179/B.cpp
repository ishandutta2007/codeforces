...
int main() {
	...
	FOR(i, 1, 50)
		FOR(j, 1, 50) {
			...
			REP(x, s - 1)
				dxy[ans[x] - ans[x + 1]]++;
			for (auto c : dxy) assert(c.second == 1);
			FOR(x, 1, i) FOR(y, 1, j) assert(xy[make_pair(x, y)] == 1);
		}
#endif
	int n, m;
	scanf("%d%d", &n, &m);
