...
class query( val dura:Int,val  block:Int, val id:Int){
	var ans = nlarge
...
fun main(){
    solve.cases{
		val n = getint
		val m = getint
		val damage = getline(n)

		damage.radixsort()
		val Qs = MutableList(m){
			query(getint, getint,it )
		}
		Qs.sortBy { it.block }
		...
		for(q in Qs){
			val rating = q.block
			while(mpt < damage.lastIndex && damage[mpt+1] < rating) {
				mpt ++
				nodef_sum -= damage[mpt]
				nodef_count --
				def_count ++
				def_sum += damage[mpt]
			}
			val belowodds = 1 modMinus (minOf(1+nodef_count, q.dura) modDivide (1+nodef_count))
			val aboveodds = 1 modMinus (minOf(nodef_count,q.dura) modDivide (nodef_count))

			val anshere = (belowodds modM (def_sum % pI).toInt()) modPlus (aboveodds modM (nodef_sum % pI).toInt())
			q.ans = anshere
		}
...
/*
11 minutes for math question because no skill issue !
 */