...
inline int query(int a, int b, int c, int d, int t)
{
	if (a > c || b > d) return 0;
	++c; ++d;
	return dp[a][b][t] + dp[c][d][t] - (dp[a][d][t] + dp[c][b][t]);
}
...
int main(int argc, const char *argv[])
{
	__INIT_CC__
	cin >> h >> w;
	FOR (i,0,h) cin >> area[i];
	FOR (i,0,h)
		FOR (j,0,w)
			if (area[i][j] == '.')
			{
				if (i + 1 < h && area[i + 1][j] == '.') ++dp[i][j][0];
				if (j + 1 < w && area[i][j + 1] == '.') ++dp[i][j][1];
			}
	FORD (i,h - 1,0)
		FORD (j,w - 1,0)
			FOR (k,0,2)
	...
	FOR (i,0,q)
	{
		cin >> r1 >> c1 >> r2 >> c2;
		--r1; --c1; --r2; --c2;
		cout << query(r1, c1, r2, c2 - 1, 1) + query(r1, c1, r2 - 1, c2, 0) << "\n";
	}
