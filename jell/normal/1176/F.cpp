...
signed main() {
    ...
    for(int i=0; i<n; ++i) {
        ...
        for(int j=0; j<m; ++j) {
            ...
            for(int k=3-c; k>=0; --k) {
                for(int h=0; h<3; ++h) {
                    for(int l=0; l<2; ++l) {
                        if(dp2[k][h][l]>=0) {
                            chmax(dp2[k+c][h+1][l],dp2[k][h][l]+d);
                            if(!l) chmax(dp2[k+c][h+1][1],dp2[k][h][0]+d*2);
                        }
                    }
                }
            }
        ...
        for(int h=0; h<4; ++h) {
            for(int j=0; j<4; ++j) {
                for(int k=0; k<2; ++k) {
                    chmax(dp3[j][k],dp2[h][j][k]);
                }
            }
        }
        i64 dp4[10];
        for(int j=0; j<10; ++j) dp4[j]=-1;
        for(int j=0; j<10; ++j) {
            if(dp[j]<0) continue;
            for(int h=0; h<4; ++h) {
                if(j+h>=10) {
                    if(dp3[h][1]>=0) chmax(dp4[j+h-10],dp[j]+dp3[h][1]);
                } else {
                    if(dp3[h][0]>=0) chmax(dp4[j+h],dp[j]+dp3[h][0]);
                }
            }
        }
        for(int j=0; j<10; ++j) {
