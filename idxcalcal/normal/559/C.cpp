...
int main(){
	...
	init();
	for(ri i=1;i<=K;++i)a[i].fi=read(),a[i].se=read();
	a[++K]=pii(n,m);
	sort(a+1,a+K+1);
	for(ri i=1;i<=K;++i){
		f[i]=C((a[i].fi-1)+(a[i].se-1),a[i].fi-1);
		for(ri j=1;j<i;++j)if(a[j].se<=a[i].se)
		Dec(f[i],mul(f[j],C((a[i].fi-a[j].fi)+(a[i].se-a[j].se),a[i].fi-a[j].fi)));
	}
	cout<<f[K];
	return 0;
}