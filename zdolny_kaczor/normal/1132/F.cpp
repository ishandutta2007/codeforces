...
ull mix(ull o){o+=0x9e3779b97f4a7c15;o=(o^(o>>30))*0xbf58476d1ce4e5b9;o=(o^(o>>27))*0x94d049bb133111eb;ros^(o>>31);}ull SALT=0x7a14a4b0881ebf9,tqu=0x7a14a4b0881ebf9;ull my_rand(){return tqu=mix(tqu);}void my_srand(ull x){SALT=tqu=x;}
#define rand my_rand
...
int go(int i, int j) {
	...
	int sub_dp[nax], pos[nax];
	dp[i][j] = nax;
	for (int wh = i, d = 0; wh < j; wh = next[wh], ++d) {
		pos[d] = wh;
		sub_dp[d] = go(i, wh);
		debug(arr(pos, d), arr(sub_dp, d));
		REP(e, d) mini(sub_dp[d], sub_dp[e] + go(pos[e] + 1, wh));
		mini(dp[i][j], sub_dp[d] + 1 + go(wh + 1, j));
	}
#ifdef LOCAL
	if (dp[i][j] != bru_go(string(in + i, in + j))) {
		debug(arr2(dp, i, j), string(in + i, in + j));
		exit(-1);
	}
#endif
	return dp[i][j];
...
int main() {
	int s, n = 0;
	scanf("%d%s", &s, inn);
	REP(i, s) if (i == 0 || inn[i] != inn[i - 1]) in[n++] = inn[i];
	REP(i, n) next[i] = nax;
	REP(i, n) FOR(j, i + 1, n - 1) if (in[i] == in[j]) {
		next[i] = j;
		break;
	}
	printf("%d\n", go(0, n));
