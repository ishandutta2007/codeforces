...
void solve() {
  ...
  FOR(L, N + 1) DP[L][L] = 1;
  FOR(L, N) DP[L][L + 1] = 1;
  FOR_R(L, N + 1) FOR3(R, L, N + 1) if (R - L >= 2) {
    if (DP[L + 1][R - 1] && S[L] == S[R - 1]) DP[L][R] = 1;
  }

  FOR_R(L, N) FOR3(R, L + 2, N + 1) {
    ll n = (R - L) / 2;
    auto left = RH.query(SH, L, L + n);
    auto right = RH.query(SH, R - n, R);
    if (left == right) {
      ll x = DP[L][L + n] & DP[R - n][R];
      DP[L][R] |= x << 1;
    }
  }

  vi CNT(N);
  FOR_R(L, N) FOR3(R, L + 1, N + 1) {
    ll x = DP[L][R];
    ll k = 0;
    while (x) {
      if (x & 1) ++CNT[k];
