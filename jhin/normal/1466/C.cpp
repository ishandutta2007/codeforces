#include <bits/stdc++.h>
    ...
    if(x>1)p.push_back(x),cnt++,vv.push_back(1);
       ...
       while(t--)
       {
           cin>>s;int nm=0;char khra='A';
           for(int i=0;i<s.size();i++)
           {
               x=1;
               if(i>0&&s[i+1]==s[i-1]&&s[i]!=s[i+1]){s[i+1]=khra;nm++;khra++;if(khra=='Z')khra='A';}
               while(s[i]==s[i+1]&&x<3)x++,i++;
               if(x==3){nm+=2;s[i]=khra;khra++;s[i-1]=khra;if(khra=='Z')khra='A';}
               if(x==2){nm++;s[i]=khra;khra++;if(khra=='Z')khra='A';}
