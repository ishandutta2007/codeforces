...
void test(int64_t a) {
    ...
    // w - 2 and l
    for (int i = 0; i < 16; i++) {
        int64_t top = w - __builtin_popcount(i & 0b0011);
        if (top % a) continue;
        int64_t bot = w - __builtin_popcount(i & 0b1100);
        if (bot % a) continue;
        int64_t left = l - 2 + __builtin_popcount(i & 0b1001);
        if (left % a) continue;
        int64_t right = l - 2 + __builtin_popcount(i & 0b0110);
        if (right % a) continue;
        ans.emplace_back(a);
        return;
    }
...
void solve() {
    ...
    if (w < l) swap(w, l);
    ans.clear();
    int64_t side = w * 2 + l * 2 - 4;
    for (int64_t i = 1; i * i <= side; i++) {
        if (side % i == 0) {
            test(i);
            test(side / i);
        }
