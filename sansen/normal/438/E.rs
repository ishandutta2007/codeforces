...
fn run() {
    ...
    while f.len() <= m {
        let size = f.len();
        let s = &s[..s.len().min(2 * size)];
        let mut sa = s.multiply(&f);
        sa.truncate(2 * size);
        let mut l = sa.multiply(&f);
        l.drain(..size);
        l.truncate(size);
        let r = [M::one()].sub(&[M::new(2)].mul(&sa));
        f.extend(l.multiply(&r.inverse(size)).into_iter().take(size));
    }
    f.truncate(m + 1);
