...
void solve() {
  ...
  */
  ...
  const ll INF = 1LL << 60;
  vv(ll, DP, N + N, 1 << N, INF);

  vi FULL(N + N);
  FOR3(n, 1, N + N) {
    FOR(x, N) {
      ll y = n - 1 - x;
      if (0 <= y && y < N) FULL[n] |= 1 << x;
    }
  }
  ...
  FOR3_R(n, 1, N + N) {
    ...
    FOR(x, N) {
      ll y = n - 1 - x;
      if (0 <= y && y < N) { NOW[G[x][y] - 'a'] |= 1 << x; }
    }
    vi NXT(26);
    FOR(x, N) {
      ll y = n - x;
      if (0 <= y && y < N) { NXT[G[x][y] - 'a'] |= 1 << x; }
    }

    FOR3(s, 1, 1 << N) {
      bool valid = [&]() -> bool {
        if ((s & FULL[n]) != s) return false;
        FOR(c, 26) if ((s & NOW[c]) == s) return true;
        return false;
      }();
      if (!valid) continue;
      ...
      add = (g == me ? 1 : 0);
      add += (g == op ? -1 : 0);
      FORIN(t, NXT) if (t & nxt) { chmin(dp, -DP[n + 1][t & nxt] + add); }
      // print(n, s, dp);
    }
  }
