...
//    val weights: LongArray = LongArray(n)
    val vnow:IntArray = IntArray(n)
...
class item(var root:Int, var v:Int):Comparable<item>{
...
fun main(){
    solve.cases{
        ...
        val dsu = DisjointUnionSets(n+q)
        for(i in 0 until n){
            dsu.vnow[i] = L[i]
        }
        val where = IntArray(n){it}
        ...
        fun cleanup(l:Int, r:Int):Triple<Int, Long, MutableList<Int>> {
            var count = 0
            var total = 0L
            val who = mint
            val test = item(plarge,r)
            while(true){
                val v = st.floor(test) ?: break
                if(v.v >= l){
                    st.remove(v)
                    val r = dsu.find(v.root)
                    count += dsu.size[r]
                    total += dsu.size[r] * dsu.vnow[r].toLong()
                    who.add(v.root)
                }else{
                    break
                }
            }
            return Triple(count, total, who)
        }

        fun find(i:Int):Int {
            val who = dsu.find(where[i])
...
//                var time = 0L
                ...
                val (c1,t1,who1) = cleanup(l,l1)
//                time += t1 - c1 * (l -1).toLong()
                if(who1.isNotEmpty()){
                    val x = who1[0]
                    for(i in 1 until who1.size){
                        dsu.union(x,who1[i])
                    }
                    val rt = dsu.find(who1[0])
                    st.add(item(rt,l-1))
                    dsu.vnow[rt] = l-1
                }


                val (c2,t2,who2) = cleanup(r0,r)
//                time +=  c2 * (l -1).toLong() - t2
                if(who2.isNotEmpty()){
                    val x = who2[0]
                    for(i in 1 until who2.size){
                        dsu.union(x,who2[i])
                    }
                    val rt = dsu.find(who2[0])
                    st.add(item(rt,r+1))
                    dsu.vnow[rt] = r+1
                }
