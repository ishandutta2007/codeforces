...
signed main(){
    ...
    ForE(i, 2, m){
        maxdif1[i] = max(maxdif1[i - 1], pos1[i] - pos1[i - 1]);
    }
    FordE(i, m - 1, 1){
        maxdif2[i] = max(maxdif2[i + 1], pos2[i + 1] - pos2[i]);
    }
    ForE(i, 2, m){
        ans = max(max(ans, pos2[i] - pos1[i - 1]), max(maxdif1[i - 1], maxdif2[i]));
    }
    ans = max(ans, max(maxdif1[m], maxdif2[1]));
    cout << ans << endl;
}
