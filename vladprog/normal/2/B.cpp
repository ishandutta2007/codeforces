...
signed main()
{
    ...
    for1(i,n)
        for1(j,n)
        {
            ...
            if(i>1)
                dp[i][j].x=min(dp[i][j].x,dp[i-1][j].x+a2[i][j]),
                dp[i][j].y=min(dp[i][j].y,dp[i-1][j].y+a5[i][j]);
            if(j>1)
                dp[i][j].x=min(dp[i][j].x,dp[i][j-1].x+a2[i][j]),
                dp[i][j].y=min(dp[i][j].y,dp[i][j-1].y+a5[i][j]);
        }
    pair<int,int> ans=dp[n][n];
    if(zero&&min(ans.x,ans.y)>1)
    {
        cout<<1<<"\n";
        int x;
        for1(i,n)
            for1(j,n)
                if(a2[i][j]==INF)
                    x=i;
        cout<<string(x-1,'D')<<string(n-1,'R')<<string(n-x,'D');
        exit(0);
    }
    else
        cout<<min(ans.x,ans.y)<<"\n";
    string s;
    int i=n,j=n;
    if(ans.x<ans.y)
        while(i&&j)
