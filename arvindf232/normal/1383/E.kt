...
fun main(){
    solve.cases{
        val str = getstr
        val here = str.split('1').map { it.size }.toMutableList()
        if(here.size == 1){
            put(here[0])
            return@cases
        }else if(here.size == 2){
            put((here[0] +1 ) * (1 + here[1]))
            return@cases
        }
        var base = 1
        base = base modM (1+here.removeFirst())
        base = base modM (1+here.removeLast())

        val S = ArrayDeque<Pair<Int,Int>>()
        S.add(Pair(plarge,0))
//        just dei here
        var sum = 1
        for(a in here.reversed()){
//            sum dei S
            val dp = sum
            while(S.last().first <= a){
                val (v,rate) = S.removeLast()
                val (_,rate2) = S.last()
                sum = sum modMinus ((v+1) modM (rate modMinus rate2))
            }
//            "that" dei dp

            sum = sum modPlus((a+1) modM (dp modMinus S.last().second))
            S.addLast(Pair(a,dp))
        }
//        just dei S
//        just dei sum
