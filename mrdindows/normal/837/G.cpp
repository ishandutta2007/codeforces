...
ll ans[75001];
Node *lastn = tr;
void add(Node *&cur, int l, int r, int x, int y, int a, int b) {
    if (l > y || r < x) return;
    Node* cp = cur;
    cur = lastn++;
    if (cp) {
    ...
    if (x <= l && r <= y) {
        ...
        int m = (l + r) / 2;
        add(cur->lp, l, m, x, y, a, b);
        add(cur->rp, m + 1, r, x, y, a, b);
    }
}
pll sum(Node* cur, int l, int r, int x) {
    if (!cur || x > r || x < l) return {0, 0};
    int m = (l + r) / 2;
    return sum(cur->lp, l, m, x) + sum(cur->rp, m + 1, r, x) + pll(cur->a, cur->b);
}

const int N = 1000000000;
ll getAns(int v, int x) {
    if (v == 0) return 0;
    auto p = sum(roots[v], 0, N, x);
    return p.first * x + p.second + ans[v];
}
...
int main() {
    ...
    FI(n) {
        ...
        b -= y1;
        y2 -= y1;
        add(roots[i + 1], 0, N, x1 + 1, x2, a, b);
        add(roots[i + 1], 0, N, x2 + 1, N, 0, y2);
    }
    int m;
    cin >> m;
    FI(m) {
        ...
        cin >> l >> r >> x;
        x = (x + last) % N;
        ll res = getAns(r, x) - getAns(l - 1, x);
        last = res;
        cout << res << '\n';
    }
