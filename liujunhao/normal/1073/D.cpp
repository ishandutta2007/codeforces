...
int find(int i,int l,int r,LL val){
	if(l==r)
		return l;
	int mid((l+r)>>1);
	if(sum[i<<1]>val)
		return find(i<<1,l,mid,val);
	else
		return find((i<<1)|1,mid+1,r,val-sum[i<<1]);
}
int main()
{
	Read(n),Read(T);
	...
	for(i=n;i;i--){
		ans+=(T/tot)*i;
		T%=tot;
		int pos=find(1,1,n,T);
		insert(1,1,n,pos,-a[pos]);
		tot-=a[pos];
	}
	printf("%lld\n",ans);
}