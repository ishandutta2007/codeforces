...
pair<int, vector<int>> BFS(const Wgraph<int> &g, int s, int t) {
    int n = g.size();
    vector<int> d(n, INT_MAX), pre(n), used(n);
    d[s] = 0;
    deque<int> q({s});
    while(!q.empty()) {
        int a = q.front();
    ...
    if(d[t] > n) return pair(INT_MAX, vector<int>());
    int now = t;
    vector<int> res{t};
    while(now != s) {
        now = pre[now];
...
int main() {
    ...
    int N = n + 1;
    Wgraph<int> g(N * 2);
    rep(i, n + 1) { g[i].emplace_back(i - a[i] + (n + 1), 1); }
    rep(i, n) g[i + (n + 1)].eb(i + n + 2, 0);
    rep(i, n + 1) { g[i + (n + 1)].eb(i + b[i], 1); }

    auto [_, res] = BFS(g, n, 0);

    if(empty(res)) drop(-1);
    vi ans;
    rep(i, si(res)) {
        if(res[i] >= n + 1 and i < si(res) and res[i + 1] < n + 1) ans.eb(res[i] - n - 1);
    }
    OUT(si(ans));
    OUT(ans);
}