...
int main() {
    ...
    rep(i, n) rep2(j, i + 1, n - 1) { pq.emplace(a[i][j], i, j); }
    vpi ans;
    vi p = iota(n);
    vi s(n);
    rep(i, n) s[i] = a[i][i];
    ...
    while(!empty(pq)) {
        auto [x, i, j] = pq.top();
        while(p[i] != i) i = id[p[i]];
        while(p[j] != j) j = id[p[j]];
        i = id[i], j = id[j];
        pq.pop();
        if(s[i] == x and s[j] == x) {
            if(i != j) { id[j] = i; }
    ...
    int N = 0;
    vi num(si(id));
    rep(i, si(id)) if(id[i] == i) {
        N++;
        num[i] = N;
    }
    cout << N << endl;

    rep(i, si(id)) if(id[i] == i) { cout << s[i] << " "; }
    cout << endl;
    cout << N << endl;
    for(auto [x, y] : ans) cout << num[x] << " " << num[y] << endl;
}