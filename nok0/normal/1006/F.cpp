...
void main_() {
	...
	int mv = n + m - 2;
	if(mv <= 10) {
		vector mp(n, vector<map<ll, int>>(m));
		mp[0][0][a[0][0]] = 1;
		REP(i, n) {
			REP(j, m) {
				if(i + 1 < n) {
					for(const auto &[key, val] : mp[i][j]) {
						mp[i + 1][j][key ^ a[i + 1][j]] += val;
					}
				}
				if(j + 1 < m) {
					for(const auto &[key, val] : mp[i][j]) {
						mp[i][j + 1][key ^ a[i][j + 1]] += val;
					}
				}
			}
		}
		print(mp.back().back()[k]);
		return;
	}
	ll res = 0;
	int bef = divup(mv, 2);
	...
	vector mp(n, vector<map<ll, int>>(m));
	mp[0][0][a[0][0]] = 1;
	mp[n - 1][m - 1][a[n - 1][m - 1]] = 1;
	REP(i, n) {
		REP(j, m) {
			if(i + 1 < n and i + 1 + j <= bef) {
				for(const auto &[key, val] : mp[i][j]) {
					mp[i + 1][j][key ^ a[i + 1][j]] += val;
				}
			}
			if(j + 1 < m and i + j + 1 <= bef) {
				for(const auto &[key, val] : mp[i][j]) {
					mp[i][j + 1][key ^ a[i][j + 1]] += val;
				}
			}
