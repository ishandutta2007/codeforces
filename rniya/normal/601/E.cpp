...
int main() {
    ...
    int q, cnt = 0;
    cin >> q;
    vector<int> l(n + q, -1), r(n + q, -1);
    for (int i = 0; i < n; i++) l[i] = cnt;
    for (; q--;) {
        int t;
        cin >> t;
        if (t == 1) {
    ...
    size = cnt;
    vector<vector<int>> seg(size << 1);
    for (int i = 0; i < n; i++) {
        for (int L = l[i] + size, R = r[i] + size; L < R; L >>= 1, R >>= 1) {
            if (L & 1) seg[L++].emplace_back(i);
            if (R & 1) seg[--R].emplace_back(i);
        }
    }
    vector<vector<int>> dp(size << 1);
    dp[0] = vector<int>(k + 1, 0);
    auto MERGE = [&](int idx, int x) {
        for (int i = k; i - w[x] >= 0; i--) {
            dp[idx][i] = max(dp[idx][i], dp[idx][i - w[x]] + v[x]);
        }
    };
    for (int i = 1; i < (size << 1); i++) {
        dp[i] = dp[i >> 1];
        for (auto& x : seg[i]) MERGE(i, x);
    }

    auto calc = [&](int idx) -> mint {
        mint res = 0, power = 1;
        for (int i = 1; i <= k; i++, power *= p) res += dp[idx][i] * power;
        return res;
    };
    for (int i = 0; i < cnt; i++) cout << calc(size + i) << '\n';
    return 0;
}