...
void add(int x, int v) {
    for(int i = 2; i * i <= x; i++) while(x % i == 0) cmax(mx[i], p[i] += v), x /= i;
    if(x > 1) cmax(mx[x], p[x] += v);
}
void dfs(int u, int fa, mint t) {
    ans += t;
    for(auto [v, x, y] : G[u]) if(v != fa) {
        add(y, -1), add(x, 1), dfs(v, u, t / x * y);
        add(x, -1), add(y, 1);
    }
}
...
void solve() {
    cin >> n;
    p.clear(), mx.clear(), ans = 0;
    rep(i, 1, n) G[i].clear();
    int u, v, x, y;
    rep(i, 2, n) cin >> u >> v >> x >> y, G[u].eb(v, x, y), G[v].eb(u, y, x);
    dfs(1, 0, 1);
    for(auto [k, v] : mx) while(v > 0) ans *= k, v--;
    cout << ans.val() << '\n';
}
int main() {
