...
struct Linear {
    ...
    LL eval(LL x) { return a + b*x; }
};

ostream& operator<<(ostream& out, Linear L) {
    out << L.a << " + " << L.b << "t";
    return out;
}

Linear operator+(Linear X, Linear Y) {
    return Linear(X.a + Y.a, X.b + Y.b);
}
...
Linear get_cost(int i, int j) {
    return Linear(a[i] * a[j], a[i] + a[j]); 
}
...
void solve() {
    ...
    // dbg_arr(a, n);
    Linear f;
    for (int i = 0; i < n-1; i++) f = f + get_cost(i, n-1);
    // dbg(f);
    // cout << f << endl;
    if (f.eval(-a[n-1]) < f.eval(-a[n-1]-1)) {
        cout << "INF\n";
        return;
    }
    // cout << f << endl;
    LL res = f.eval(-a[n-1]);
    for (int i = n-2; i > 0; i--) {
        res = max({res, f.eval(-a[i]), f.eval(-a[i+1])});
        f = f - get_cost(i, n-1) + get_cost(i, 0);
    }
    if (f.eval(-a[0]) < f.eval(-a[0] + 1)) {
        cout << "INF\n";
        return;
    }
    res = max(res, f.eval(-a[0]));
    cout << res << "\n";
}
