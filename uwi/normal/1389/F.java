...
public class F2 {
	...
	void solve()
	{
		...
		shrinkXD(co, new int[]{0, 1});
		int[][] b = makeBuckets(co, 2*n, x -> x[1]);
		
		StarrySkyTree[] dp = new StarrySkyTree[2];
		for(int i = 0;i < 2;i++)dp[i] = new StarrySkyTree(2*n);
		// dp[x][i] = (i-1x)
		for(int i = 0;i < 2*n;i++){
			for(int j = 0;j < 2;j++){
				int max = Math.max(0, -dp[j].min(0, i));
				dp[j^1].add(i, i+1, -max - dp[j^1].min(i, i+1));
			}
			for(int u : b[i]){
				dp[co[u][2]-1].add(0, co[u][0]+1, -1);
			}
		}
		
		long ans = 0;
		ans = Math.max(ans, -dp[0].min(0, 2*n));
		ans = Math.max(ans, -dp[1].min(0, 2*n));
		out.println(ans);
	}
	...
	public static <T> int[][] makeBuckets(T[] a, int sup, ToIntFunction<T> index)
	{
		int n = a.length;
		int[][] bucket = new int[sup+1][];
		int[] bp = new int[sup+1];
		for(int i = 0;i < n;i++)bp[index.applyAsInt(a[i])]++;
		for(int i = 0;i <= sup;i++)bucket[i] = new int[bp[i]];
		for(int i = n-1;i >= 0;i--){
			int ind = index.applyAsInt(a[i]);
			bucket[ind][--bp[ind]] = i;
		}
		return bucket;
	}
	...
	public static int[][] shrinkXD(int[][] co, int[] dim)
	{
		int n = co.length;
		int m = dim.length;
		int p = 0;
...
//		int[] interval = new int[p];
		int[] feedback = new int[p];
		int q = 0;
		for(int i = 0;i < p;i++){
			if(i > 0 && b[i]>>>32 > b[i-1]>>>32){
//				interval[q] = (int)((b[i]>>>32) - (b[i-1]>>>32));
				q++;
			}
			feedback[(int)b[i]] = q;
		}
		...
		return co;
//		return Arrays.copyOf(interval, q);
	}
