...
int main() {
	...
	REP(i, n) {
		long long geko;
		cin >> geko;
		input.push_back(geko);
		placing[geko] = i+1;//gekou
	}
	for (int i = n / 2; i >= 1; --i) {
		if (placing[i] != i) {
			//need to fix
			...
			if (placing[i] != i) {
				if (placing[i] != i + n / 2) {
					//bring to i+n/2
					swapping(input[i + n / 2], i);
				}
				if (placing[i] != i) {
					swapping(input[i], input[i + n / 2]);
				}
			}
		}
		if (placing[i + n / 2] != i+n/2) {
			//need to fix
			if (placing[i+n/2] > i) {
				swapping(input[1], i + n / 2);
			}
			if (placing[i + n / 2] != i+n/2) {
				swapping(i+n/2, input[i + n / 2]);
			}
		}
	}
	cout << ans.size() << endl;
