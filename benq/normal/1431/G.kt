...
fun solve() {
    ...
    for (i in 0..n) for (j in i..n) if ((j-i)%2 == 0) {
        val len = (j-i)/2
        for (k in i..i+len-1) cost[i][j] -= a[k]
        for (k in i+len..j-1) cost[i][j] += a[k]
    }
    val dp = Array<IntArray>(n+1){IntArray(k+1){-MOD} }
    dp[0][0] = 0
    for (i in 0..n) for (j in 0..k) {
        if (i > 0) dp[i][j] = maxOf(dp[i][j],dp[i-1][j])
        for (len in 1..j) if (i >= 2*len) {
            dp[i][j] = maxOf(dp[i][j],dp[i-2*len][j-len]+cost[i-2*len][i])
        }
//        println("${i} ${j} ${dp[i][j]}")
    }
    println(dp[n][k])
