...
void solve(){
	...
	vector<Mint> inverse_vector(m, 0);
	inverse_vector[0] = 1;
	inverse_vector[1] = -1;
	inverse_vector = exponent(logarithm(inverse_vector) * vector<Mint> {-n});


	auto get_cnt = [&] (const vector<array<int, 2>>& arr, int target){
		...
		// vector<Mint> ret(sz, 0);
		// ret[0] = 1;
		// ret[1] = -1;

		// ret = exponent(logarithm(ret) * vector<Mint> {-n});
		// ret.resize(sz);
		vector<Mint> ret(sz);
		for(int i = 0; i < sz; ++i)
			ret[i] = inverse_vector[i];
		
		for(auto e: arr){
			int r_exp = e[1] - e[0] + 1;
			for(int i = sz - 1; i >= r_exp; --i){
				ret[i] -= ret[i - r_exp];
			}
		}
