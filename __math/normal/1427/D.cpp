...
vector<int> ope(vector<int> a, vector<int> sizes) {
	int s = 0;
	for (auto c : sizes) {
		int t = s + c;
		reverse(a.begin() + s, a.begin() + t);
		s = t;
	}
	return a;
}
...
vector<vector<int>> solve(vector<int> a) {
	...
	FOR(i, n - 1) {
		...
		if (nt == 0) break;
		vector<int> sizes;
		FOR(i, nt - 1) sizes.push_back(1);
		for (int j = nt; j < n; j++) if (a[j] == nt) {
			sizes.push_back(j - sz(sizes) + 1);
			sizes.push_back(n - j - 1);
			if (sizes.back() == 0) sizes.pop_back();
		}
		
		a = ope(a, sizes);
		if (rev) reverse(sizes.begin(), sizes.end());
		ans.push_back(sizes);
