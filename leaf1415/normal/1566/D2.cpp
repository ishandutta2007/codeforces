...
int main(void)
{
	...
	while(T--){
		...
		map<ll, vector<ll> > mp;
		map<ll, vector<P> > mp2;
		rep(i, 1, n) mp[a[i]].push_back(i);
		reps(i, vec) mp2[vec[i]].push_back(P(i/w, -(i%w)));
		
		for(auto p : mp){
			vector<ll> &vec = p.second;
			vector<P> &vec2 = mp2[p.first];
			sort(all(vec2));
			reps(i, vec){
				ll id = vec[i];
				pos[id] = vec2[i];
		...
		ll ans = 0;
		rep(i, 0, h-1) rep(j, 0, w-1) used[i][j] = false;
		rep(i, 1, n){
			//outl(i, pos[i]);
			rep(j, 0, pos[i].second) if(used[pos[i].first][j]) ans++;
