...
void Box::solve()
{
	...
	for (int i=0;i<SIZE(next);i++) a[next[i]].solve();
	if (type=='H')
	{
		width=border*2+space*(SIZE(next)-1);
		height=0;
		for (int i=0;i<SIZE(next);i++) 
		{
			width+=a[next[i]].width;
			checkmax(height,a[next[i]].height);
		}
		height+=border*2;
	}
	else
	{
		width=0;
		height=border*2+space*(SIZE(next)-1);
		for (int i=0;i<SIZE(next);i++)
		{
			checkmax(width,a[next[i]].width);
			height+=a[next[i]].height;
		}
		width+=border*2;
	}
}
...
int main()
{
	...
	{
		gets(line);
		for (int i=0;line[i];i++) if (line[i]=='.' || line[i]=='(' || line[i]==')' || line[i]==',') line[i]=' ';
		...
		else
		{
			...
			{
				sin>>name;
				a[get_id(cmd)].next.push_back(get_id(name));
			}
			else if (cmd2=="set_border")
				sin>>a[get_id(cmd)].border;
			else if (cmd2=="set_spacing")
				sin>>a[get_id(cmd)].space;
		}
	}
	for (set<string>::iterator it=S.begin();it!=S.end();++it) 
	{
		...
		int id=get_id(s);
		a[id].solve();
		cout<<s<<" "<<a[id].width<<" "<<a[id].height<<endl;
	}
	return 0;
}