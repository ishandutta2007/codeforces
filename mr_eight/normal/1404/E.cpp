...
int n,m,tot,num;
char s[2002][2002];
inline int getid(int x,int y,bool k){
	return k?(n*m+x*m+y):(x*m+y);
}
int main(){
	cin>>n>>m;
	T=100000;
	F(i,1,n){
		...
		F(j,1,m){
			if(s[i][j]=='#'){
				...
				if(s[i][j-1]=='#'){
					++num;
					ed(S,getid(i,j-1,0),1);
				}if(s[i-1][j]=='#'){
					++num;
					ed(getid(i-1,j,1),T,1);
				}if(s[i][j-1]=='#'&&s[i-1][j]=='#')ed(getid(i,j-1,0),getid(i-1,j,1),1);
				if(s[i][j-1]=='#'&&s[i+1][j]=='#')ed(getid(i,j-1,0),getid(i,j,1),1);
				if(s[i][j+1]=='#'&&s[i-1][j]=='#')ed(getid(i,j,0),getid(i-1,j,1),1);
				if(s[i][j+1]=='#'&&s[i+1][j]=='#')ed(getid(i,j,0),getid(i,j,1),1);
			}
		}	
	}num-=Dinic();
	write(tot-num,'\n');
