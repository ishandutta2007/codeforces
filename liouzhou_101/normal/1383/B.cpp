...
void solve()
{
	...
	for (int k = 29; k >= 0; --k)
	{
		...
		if (cnt % 2 == 1)
		{
			if (cnt == 1) return writeln("WIN");
			int zero = n - cnt;
			if (zero == 0) return writeln("WIN");
			if (zero % 2 == 1) return writeln("WIN");
			else return writeln("LOSE");
		}
	}
	*/
	while (__builtin_popcount(sum) != 1)
	{
		int k = __builtin_ctz(sum);
		sum ^= 1 << k;
	}
	int k = __builtin_ctz(sum);
	int cnt = 0;
	...
	if ((cnt / 2 + 1) % 2 == 1) return writeln("WIN");
