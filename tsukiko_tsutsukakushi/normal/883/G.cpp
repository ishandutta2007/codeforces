...
int main() {
    ...
    {
        pair<int, string> ans_max;
        ans_max.second = string(d, '\0');

        vector<int8_t> vis(n, false);
        auto dfs = [&](auto dfs, int u) -> void {
            vis[u] = true;
            for (auto [v, id] : g[u]) if (not vis[v] and (id < 0 or ans_max.second[id] == '\0')) {
                if (id >= 0 and ans_max.second[id] == '\0') {
                    ans_max.second[id] = es[id].first == u ? '+' : '-';
                }
                dfs(dfs, v);
            }
        };
        dfs(dfs, s);
        for (auto& e : ans_max.second) if (e == '\0') e = '+';

        ans_max.first = accumulate(all(vis), 0);
        print(ans_max.first);
        print(ans_max.second);
    }

    {
        pair<int, string>ans_min;
        vector<int8_t> vis(n, false);
        vis[s] = true;
        deque<int> dq{ s };
        while (dq.size()) {
            int u = dq.front();
            dq.pop_front();
            for (auto [v, id] : g[u]) if (id < 0 and not vis[v]) {
                vis[v] = true;
                dq.push_back(v);
            }
        }
        ans_min.first = accumulate(all(vis), 0);
        ans_min.second = string(d, '\0');

        rep(i, d) {
            auto [u, v] = es[i];
            ans_min.second[i] = vis[v] ? '+' : '-';
        }
        print(ans_min.first);
        print(ans_min.second);
    }
