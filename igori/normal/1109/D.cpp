...
signed main()
{
    ...
    for (int i = 1; i <= n + m; i++)
    {
        ...
        notfact[i] = inq(fact[i], MOD - 2, MOD);
    }
    notfact[0] = notfact[1];
    //long long ans1 = fact[n - 2];
    long long ans = 0;
    dp[0] = 1;

    for (int l = 2; l <= min(n, m + 1); l++)
    {
        long long ans2 = fact[m - 1] * notfact[l - 2] % MOD * notfact[m - 1 - (l - 2)] % MOD;
        long long randevu = inq(m, n - l, MOD);
        long long vetki = ans2 % MOD * randevu % MOD * fact[n - 2] % MOD * notfact[max(0LL, n - l)] % MOD;
        ans = (ans + vetki * f(n, l) % MOD) % MOD;
        //cout << l << " " << ans2 << " " << randevu << " " << fact[n - 2] << " " << notfact[max(0LL, n - l)]  << endl;
        //cout << l << " " << vetki << endl;
        //cout << ans2 * ans1 << " ";
    }
    cout << ans;
