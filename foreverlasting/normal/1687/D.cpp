...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        n=(int)(unique(a+1,a+n+1)-a-1);
        for(res i=1;i<=n;i++)nxt[a[i]-1]=i;
        nxt[a[n]]=n+1;
        res fl=0;
        for(res i=1;i<=n;i++){
            res p=(int)sqrt((db)a[i]);
            if(a[i]<p*p+p);
            else {fl=1;break;}
        }
        if(!fl){puts("0");return;}
        ...
        for(LL w=(int)sqrt(a[1]);;w++){
            LL L=max(0ll,w*w-a[1]),R=w*w+w-a[1];
            if(R<0)continue;
            LL W=w;
            for(res i=1;i<=n;){
                res j;
                while(W*W+W<a[i]+L)W++;
                if(W*W+W-L>a[n])j=n+1;
                else j=nxt[W*W+W-L];
                L=max(L,W*W-a[i]),R=min(R,W*W+W-a[j-1]);
                if(L>R)break;
                i=j;
            }
            if(L<=R){printf("%lld\n",L);return;}
