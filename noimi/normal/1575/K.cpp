...
int main() {
    ...
    INT(ax, ay, bx, by);
    ll t = 1LL * n * m;
    REC([&](auto &&f, int ax, int ay, int bx, int by, int r, int c) -> void {
        if(tie(ax, ay) == tie(bx, by)) return;
        dump(ax, ay, bx, by, r, c);
        if(max(ax, bx) >= min(ax + r, bx + r) or max(ay, by) >= min(ay + c, by + c)) {
            t -= 1LL * r * c;
            return;
        } else {
            int x = min(ax + r, bx + r) - max(ax, bx);
            int y = min(ay + c, by + c) - max(ay, by);
            t -= 1LL * r * c - 1LL * x * y;
            f(ax, ay, ax + r - x, ay + c - y, x, y);
        }
    })
    (ax, ay, bx, by, r, c);
    OUT(modpow(k, t));
