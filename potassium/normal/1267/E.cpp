...
int main() {
	int n,m; R(n,m);
	vector<vector<int>> a(m,vector<int>(n)); R(a);
	...
	vector<int> atake;
	for (int i = 0, ansptr = 0; i < m; i++) {
		if (ansptr < anstake.size() && i == anstake[ansptr])
			ansptr++;
		else
			atake.pb(i+1);
	}
