// time-limit: 1000
// problem-url: https://codeforces.com/contest/1737/problem/B
...
void up(){
	int64_t const left=read<int64_t>();
	int64_t const right=read<int64_t>();
	auto const solve=[&](int64_t max){
		if(max==0) return int64_t(0);
		int s=int(std::sqrt((long double)max)); // terrible mistake somehow
		return
		(max-int64_t(s)*s)/s+1 + int64_t(s-1)*3;
		
	};
	std::cout<<solve(right)-solve(left-1)<<'\n';
