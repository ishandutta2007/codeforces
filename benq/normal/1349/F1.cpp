...
int main() {
	setIO(); re(n); genFac(n+1);
	assign[0][0] = 1;
	FOR(i,1,n+1) F0R(j,i) {
		assign[i][j] += assign[i-1][j]*j;
		assign[i][j+1] += assign[i-1][j];
	}
	F0R(i,n+1) F0R(j,i+1) dbg(i,assign[i][j]);
	vmi ex(n+1);
	F0R(y,n) FOR(x,y,n) ex[y] += d(x,y)*ifac[x+1];
	F0R(y,n) {
		ex[y] *= fac[y];
		if (y&1) ex[y] *= -1;
	}
	F0R(i,n) {
		mi ans = 0; FOR(y,i,n) ans += ex[y]*ifac[y-i];
		if (i&1) ans *= -1;
		pr(ans*fac[n]*ifac[i],' ');
	}
