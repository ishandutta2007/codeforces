...
vi pal = { 119, 36, 93, 109, 46, 107, 123, 37, 127, 111 };
vi pp;
int another_main()
{
	for (int i = 0; i < 10; ++i)
		pp.push_back(__builtin_popcount(pal[i]));
	int n, k;
	cin >> n >> k;
	vi v(n);
	vi bp;
	...
	for (int i = 0; i < n; ++i)
	{
		for (int j = 0; j <= k; ++j)
			for (int l = 0; l < 10; ++l)
				if ((v[i] & pal[l]) == v[i] && dp[i][j])
				{
					int new_j = j + pp[l] - bp[i];
					if (new_j <= k)
						dp[i + 1][new_j] = true;
				}
	}
	if (dp[n][k])
	{
		...
		for (int i = n; i >= 1; --i)
		{
			...
			for (int l = 9; l >= 0; --l)
			{
				int new_k = k - pp[l] + bp[i - 1];
				if (new_k >= 0 && ((v[i - 1] & pal[l]) == v[i - 1]) && dp[i - 1][new_k])
				{
					ans.pb('0' + l);
					k = new_k;
					break;
