...
void solve(){
  ...
  vector<ll>p(n);
  vector<ll>a(n);
  rep(i,0,n)cin>>p[i],p[i]=-p[i];
  rep(i,0,n)cin>>a[i];
  auto g=CartesianTree(p);
  bool sw=true;
  vector<int>l,r;
  vector<ll>submx(n,-1);
  vector<int>vs;
  rep(i,0,n)if(a[i]!=0)vs.PB(a[i]);
  vs.PB(-1);
  vs.PB(1e9);
  auto dfs=[&](auto &&f,ll v,ll mi=(int)1e9)->void{
    //cout<<v<<endl;
    if(a[v]!=0&&!chmin(mi,a[v]))sw=false;
    if(a[v]!=0)submx[v]=a[v];
    for(auto to:g.fi[v]){
      f(f,to,mi);
      chmax(submx[v],submx[to]);
    }
    if(a[v]==0){
      l.PB(submx[v]);
      r.PB(mi);
      if(l.back()>r.back())sw=false;
    }
  };
  dfs(dfs,g.se);
  ll k=l.size();
  ...
  r=mp.compress(r);
  ll sz=mp.size();
  auto seg=add_min::make(sz);
  rep(i,0,sz)seg.set(i,0);
  seg.build();
  vector<ll>ls(sz+1);
  vector<vector<ll>>gr(sz);
  rep(i,0,l.size()){
    gr[r[i]].PB(l[i]);
  }
  vector<ll>ok(sz+1);
  ll ne=0;
  rep(i,0,sz){
    if(binary_search(ALL(s),mp.val(i)))seg.update(0,i+1,1);
    for(auto z:gr[i]){
      seg.update(0,z+1,-1);
    }
    if(seg.query(0,i+1)<-1)sw=false;
    auto f=[&](ll v){
      return v<=-1;
    };
    ll ri=seg.find_last(i+1,f);
    //cout<<mp.val(i)<<endl;
    //seg.print();
    if(ri!=-1&&ri<=i){
      ///cout<<ri spa i<<endl;
