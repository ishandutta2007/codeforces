...
signed main(){
    ...
    rep(zz,q){
        ...
        if(seg1.query(L+1,R) < s[L]){
            dbg(L,R,seg1.query(L+1,R));
            cout << -1 << "\n";
            continue;
        }
        ...
        // int Lid = lb(id,L);
        // int sL = 0;
        // sL = s[min(R,id[Lid])]-s[L];
        // int Rid = lb(id,R);
        // int sR = 0;
        // Rid--;
        // sR = s[R]-s[max(L,id[Rid])];
        
        // int P = 0;
        // if(Lid<Rid){
        //     P = -seg2.query(Lid,Rid);
        // }
        // dbg(Lid,Rid,sL,sR,P);
        // cout << max({sL,abs(sR),P}) << "\n";
    }
    return 0;
}