...
signed main(){
    ...
    vector<ll>b(n);
    rep(n){
        seg1.update_min(0,i,a[i]);
        b[i]+=seg1.query_sum(0,i);
    }
    rrep(n){
        seg2.update_min(i+1,n,a[i]);
        b[i]+=seg2.query_sum(i,n);
    }
    ll at=max_element(all(b))-b.begin();
    ll k=LINF;
    rrep(at+1)a[i]=k=min(a[i],k);
    k=LINF;
    rep(i,at,n)a[i]=k=min(a[i],k);
    rep(n){
        print(a[i]);
        print(" \n"[i+1==n]);
    }
