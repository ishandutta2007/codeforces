...
int main(){
    ll L,T; cin >> T; ll ans = 0;
    rep(ii,T){
        string S; cin >> S; ll N = S.size();
        auto dp = v2ll(N+1,N+1,0);
        for(int len=2;len<=N;len+=2){
            rep(i,N){
                if(i+len>N) break;
                int j = i+len;
                ll res = -1;
                if((dp[i+1][j-1]==1||(S[i]<S[j-1]&&dp[i+1][j-1]==0))&&(dp[i+2][j]==1||(S[i]<S[i+1]&&dp[i+2][j]==0))){
                    res = 1;
                }
                else if((dp[i+1][j-1]==1||(S[i]<=S[j-1]&&dp[i+1][j-1]==0))&&(dp[i+2][j]==1||(S[i]<=S[i+1]&&dp[i+2][j]==0))){
                    chmax(res,0LL);
                }
                if((dp[i+1][j-1]==1||(S[i]>S[j-1]&&dp[i+1][j-1]==0))&&(dp[i][j-2]==1||(S[j-1]<S[j-2]&&dp[i][j-2]==0))){
                    res = 1;
                }
                else if((dp[i+1][j-1]==1||(S[i]>=S[j-1]&&dp[i+1][j-1]==0))&&(dp[i][j-2]==1||(S[j-1]<=S[j-2]&&dp[i][j-2]==0))){
                    chmax(res,0LL);
                }
                dp[i][j] = res;
