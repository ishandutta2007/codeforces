// Problem: E. Mashtali and Hagh Trees
// Contest: Codeforces - Codeforces Round #722 (Div. 1)
// URL: https://codeforces.com/contest/1528/problem/E
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
using uint = uint32_t;
namespace math {
const int L = 1<<21, A = 1<<20, B = 32, G = 3;
...
int main() {
	...
	int n;
	cin >> n;
	vector<mint> f(n+2), g(n+2);
	f[0] = g[0] = 1;
	for(int i = 1; i <= n; i++) {
		g[i] = g[i-1] + (f[i-1]-g[i-1])*g[i-1] + (g[i-1]+1)*g[i-1]/2;
		f[i] = f[i-1] + g[i];
	}
	// for(int i = 0; i <= n; i++)
		// cout << i << " - " << g[i] << " - " << f[i] << endl;
	...
	auto B = [&](int x) {
		// cout << x << " " << A(f[x]) << " " << (x>1?A(f[x-1]):0) << endl;
		return A(f[x])-(x>=1?A(f[x-1]):0);
	};
	vector<mint> pref(n+1);
	for(int r = 1; r <= n; r++)
		pref[r] = pref[r-1]+B(r-1);
	for(int l = 1; l+1 < n; l++) {
		// for(int r = 1; r <= n && l+r < n; r++) {
			// ans += B(l)*B(r);
		// }
		ans += B(l-1)*A(f[n-2-l]);
	}
	auto C3 = [&](mint x) { return x*(x-1)*(x-2)/6 + x*(x-1) + x*(x-1)/2 + 3*x; };
	auto C2 = [&](mint x) { return x*(x+3)/2; };
	ans += 2*(C3(f[n-1]) - C3(n>1?f[n-2]:0));
