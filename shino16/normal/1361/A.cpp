...
int main() {
  ...
  VI topic(n); rep(i, n) cin >> topic[i];
  // beware [(elem, i)]
  vector<PII> ti(topic.size());
  rep(i, topic.size()) ti[i] = make_pair(topic[i], i);
  sort(all(ti));
  ...
  for (auto [t, i] : ti) {
    ...
    for (auto j : graph[i]) {
      if (topic[j] < topic[i]) {
        unique_cnt += cnt[topic[j]]++ == 0;
        pushed.push_back(topic[j]);
      } else if (topic[j] == topic[i]) {
        cout << -1 << newl; exit(0);
      }
    }
    assert(unique_cnt < t);
    dump(i, unique_cnt);
    if (unique_cnt + 1 != t) { cout << -1 << newl; exit(0); }
    res.push_back(i);
    for (auto tc : pushed) cnt[tc] = 0;
  }
