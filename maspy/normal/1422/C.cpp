...
void solve() {
  ...
  ll N = len(S);
  //  n ,  n 
  vc<mint> dpl(N + 1), dpr(N + 1);
  FOR(i, N) {
    dpl[i + 1] = dpl[i] * mint(10) + mint(A[i]);
    dpr[i + 1] = dpr[i] + mint(A[N - 1 - i]) * mint(10).pow(i);
  }
  auto dplc = cumsum(dpl);
  mint ANS = 0;
  FOR(r, N) { ANS += dpr[r] * mint(N - r) + dplc[N - r] * mint(10).pow(r); }
  print(ANS);
}
