...
int main() {
	...
	debug << imie(col) imie(row);
	if (col == -1 || row == -1) err();
	int cou = 0;
	int mx = nax, my = nax, Mx = -1, My = -1;
	REP(i, h) REP(j, w) if (in[i][j] == '*') {
		if (i != row && j != col) {
			debug << "not on lines " imie(i) imie(j);
			err();
		}
		cou++;
		mini(mx, i);
	...
	debug << imie(mx) imie(my) imie(Mx) imie(My);
	if (mx == row || Mx == row || my == col || My == col) {
		debug << "not_all";
		err();
	}
	if (cou != (Mx - mx + My - my) + 1) err();
	printf("YES\n");
}