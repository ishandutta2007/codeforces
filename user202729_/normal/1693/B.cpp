// proof slightly more rigorous but who knows ><
...
void up(){
	...
	for(int const i: range(1, number)){
		int const p=read<int>()-1;
		add[p].push_back(i);
	}

	std::vector<std::pair<int, int>> bound(number);
	for(auto& [l, r]: bound) std::cin>>l>>r;

	int numNodeResult {};
	auto const work=makeYComb([&](auto work, int node)->int64_t{ // return selected a value
		int64_t sumAChild=0;
		for(auto child: add[node]) sumAChild+=work(child);
		
		int64_t d=bound[node].first-sumAChild;
		if(d>0){
			numNodeResult++;
			d+=bound[node].second-bound[node].first;
		}else{
			d+=std::min(int64_t(bound[node].second-bound[node].first), -d);
		}
		return d+sumAChild;
	});
	work(0);
	std::cout<<numNodeResult<<'\n';
}
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1693/problem/B