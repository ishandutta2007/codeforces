...
int group_size;
int get(int64_t pos) {
    while (group_start + group_size - 1 < pos) {
        current_group++;
        group_start += group_size;
        group_size -= 2;
        if (group_size == 0) group_size = 1;
    }
    if (group_size == 1) return 1;  // last 1
    int group_pos = pos - group_start;
    assert(group_pos < group_size);
    if (group_pos % 2 == 0) return current_group;
    return current_group + (group_pos + 1) / 2;
}
void solve() {
    cin >> n;
    cin >> l >> r;
    ...
    group_start = 1;
    group_size = (n - 1) * 2;
    for (int64_t i = l; i <= r; i++) cout << get(i) << " \n"[i == r];
}
