...
int main() {
	...
	for (int p = 0, i = 1; i <= n; i++) {
		int t = query(1, 1, i, n) & 1;
		if (p != t)
			x.pb(i);
		p = t;
	}
	for (int p = 0, i = 1; i <= n; i++) {
		int t = query(1, 1, n, i) & 1;
		if (p != t)
			y.pb(i);
		p = t;
	}
	if (x.size() && y.size()) {
		int t = query(x[0], y[0], x[0], y[0]);
		if (t == 1) {
			cout.flush();
			cout << "! " << x[0] << " " << y[0] << " " << x[1] << " " << y[1], ans = 0;
			return ans;
		} else {
			cout.flush();
			cout << "! " << x[0] << " " << y[1] << " " << x[1] << " " << y[0], ans = 0;
			return ans;
		}
	}
	if (x.size()) {
		int loy = 0, hiy = n, midy;
		while (loy < hiy) {
			midy = loy + (hiy - loy + 1) / 2;
			if (query(x[0] + 1, 1, x[1], midy) & 1) {
				hiy = midy - 1;
			} else {
				loy = midy;
			}
		...
		loy = hiy + 1;
		cout.flush();
		cout << "! " << x[0] << " " << loy << " " << x[1] << " " << loy, ans = 0;
		return ans;
	}
	int lox = 0, hix = n, midx;
	while (lox < hix) {
		midx = lox + (hix - lox + 1) / 2;
		if (query(1, y[0] + 1, midx, y[1]) & 1) {
			hix = midx - 1;
		} else {
			lox = midx;
		}
