...
fun solve() {
    val (n,m) = io.ints(2)
    val a = Array<IntArray>(n){io.ints(m).toIntArray()}
    val vis = Array<BooleanArray>(n){BooleanArray(m){false} }
    val ans = ArrayList<Int>()
    fun dfs(x: Int, y: Int, pre: Int) {
        ...
        dfs(x,y-1,a[x][y])
        dfs(x,y+1,a[x][y])
        if (pre != 0) ans.add(pre)
    }
    var len = 0
    for (i in 0 until n) for (j in 0 until m) if (a[i][j] != 0 && !vis[i][j]) {
        dfs(i,j,0)
        ++len
    }
    if (len > 1) {
    ...
    val side = (ans.size+1)/2
    println("$side $side")
    println((0 until side).map{i -> (0 until side).map {j -> ans[i+j]}.joinToString(" ")}.joinToString("\n"))
}
