...
signed main(){
    ...
    while (t--){
        ...
        int k=n;
        int ret=0;
        rep1(i,m) cin>>info[i].first>>info[i].second, info[i].first=-info[i].first;
        sort(info+1,info+m+1);
        rep1(i,m) sum[i]=sum[i-1]+info[i].first;
        rep1(i,m){
            int res=0;
            k=n;
            int w=lower_bound(info+1,info+m+1,make_pair(-info[i].second,0ll))-info-1;
            if (w<i){
                res+=sum[min(k-1,w)]+info[i].first;
                k-=(min(k-1,w)+1);
            }
            else{
                res+=sum[min(k,w)];
