...
int main() {
    ...
    sort(all(rows[0]));
    pair<int, ll> left{0, 2 * rows[0].back()};
    pair<int, ll> right{rows[0].back(), rows[0].back()};

    for (auto &v : range(rows.begin() + 1, rows.end())) {
        if (v.empty())
            continue;

        sort(all(v));

        pair<int, ll> next_left{v.front(), 1e18};
        pair<int, ll> next_right{v.back(), 1e18};

        vi cand{};
        for (auto i : {left, right}) {
            auto it = lower_bound(all(b), i.first);
            if (it != b.end())
                cand.push_back(*it);
            if (it != b.begin())
        ...
        for (auto to : {&next_left, &next_right}) {
            auto other = to->first ^ v.front() ^ v.back();
            for (auto &from : {left, right})
                for (int i : cand)
                    ckmin(to->second, from.second + abs(from.first - i) + abs(i - other));

            to->second += abs(v.front() - v.back());
        }

        left = next_left, right = next_right;
    }

    auto ans = min(left.second, right.second);
    ans += sz(rows) - 1;
    ps(ans);
