...
signed main() {
    ...
    for (int i = 0; i < sz(c); ++i) {
        if (last.count(c[i])) {
            segm.pb({{last[c[i]], i}, c[i]});
        }
        last[c[i]] = i;
    }
    sort(all(segm), [](pair<pii, int> p1, pair<pii, int> p2) {
        auto len = [](pii p) {
            return p.s;
        };
        return len(p1.f) < len(p2.f);
    });
    set<int> used;
    vector<int> cnt(n * k);
    int mx = (n + (k - 1) - 1) / (k - 1);
    ...
    dbg(ans);
    assert(sz(ans) == n);
    for (auto i : ans) println(i.s.f + 1, ' ', i.s.s + 1);
    #ifdef DEBUG
