...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n - 1; i++) {
			int u,v,w,c; R(u,v,w,c);
			g[u].pb({v,{w,c}});
			g[v].pb({u,{w,c}});
		}

		vector<int> sz(n + 1, 0);
		vector<vector<ll>> plan(3);
		ll totalW = 0;
		function<void(int, int, int, int)> dfs = [&](int u, int p, int w, int c) {
			sz[u] = 0;
			bool isLeaf = 1;
			for (auto& tmp : g[u]) {
				if (tmp.F != p) {
					isLeaf = 0;
					dfs(tmp.F, u, tmp.S.F, tmp.S.S);
					sz[u] += sz[tmp.F];
				}
			}
			sz[u] += isLeaf;
		...
		while (totalW > S) {
			ll p1a = get(1), p1b = get(1), p2a = get(2);

			if (totalW - p1a <= S || p1a + p1b > p2a) {
				totalW -= p1a;
				ans++;
