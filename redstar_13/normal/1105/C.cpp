...
#define C 5
int a[C][C],tp[C][C],ans[C][C];
int x[C];
void multi(int a[C][C],int b[C][C],int c[C][C]){
	for(int i=0;i<C;i++){
		for(int j=0;j<C;j++){
			tp[i][j]=0;
			for(int k=0;k<C;k++){
				tp[i][j]=((INT)tp[i][j]+(INT)a[i][k]*b[k][j]%mod)%mod;
			}
		}
	}
	for(int i=0;i<C;i++){
...
int main(){
	...
	int res=0;
	for(int i=0;i<3;i++) res=(res+(INT)ans[0][i]*x[i]%mod)%mod;
	printf("%d\n",res);
	return 0;
}