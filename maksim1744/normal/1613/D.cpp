...
void test_case(int test) {
    ...
    vector<int> sufcnt(n + 5, 0);
    for (int k : a)
        sufcnt[k]++;

    vector<Mint> cnt(n + 5, 0);
    // Mint ans = pow(Mint(2), sufcnt[0] + sufcnt[2]) - pow(Mint(2), sufcnt[0]);
    Mint ans = 0;
    ans += pow(Mint(2), sufcnt[1]);
    for (int k : a) {
        shows;
        sufcnt[k]--;
        Mint cur = cnt[k];
        ...
        cnt[k] += cur;
        Mint val = nw * pow(Mint(2), sufcnt[k] + sufcnt[k + 2]);
        show(cnt);
        show(cur, nw);
        show(sufcnt);
        show(k, val);
