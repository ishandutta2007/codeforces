...
void solve() {
  ...
  ll K = 40;
  vc<FastSet> IDS;
  FOR(k, 40) { IDS.eb(FastSet(N)); }

  FenwickTree<Group_Add<ll>> bit(A);

  FOR(i, N) { IDS[topbit(A[i]) + 1].insert(i); }

  auto upd = [&](ll p, ll x) -> void {
    IDS[topbit(A[p]) + 1].erase(p);
    bit.add(p, x - A[p]);
    A[p] = x;
    IDS[topbit(A[p]) + 1].insert(p);
  };

  auto check = [&](ll k) -> bool {
    if (k < 0 || N <= k) return false;
    return A[k] == bit.sum(k);
  };
