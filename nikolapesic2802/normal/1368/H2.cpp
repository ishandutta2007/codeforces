...
void merge(int a,int b,int c){
	for(int d=0;d<2;d++)
			for(int i=0;i<2;i++)
				for(int j=0;j<2;j++)
					for(int k=0;k<2;k++)
						drvo[a].dp[d][i][j][k]=INT_MAX;
	for(int o=0;o<2;o++)
		for(int d=0;d<2;d++)
			for(int i=0;i<2;i++)
				for(int j=0;j<2;j++)
					for(int k=0;k<2;k++)
						drvo[a].dp[o][d][i][j]=min(drvo[a].dp[o][d][i][j],drvo[b].dp[o][d][i][k]+drvo[c].dp[o][d][k][j]);
}
...
void build(int tr=ind,int l=0,int r=m+1){
	...
	merge(tr,drvo[tr].l,drvo[tr].r);
}
void doit(int qs,int qe,int s,int tr=ind,int l=0,int r=m+1){
	if(qs>r||qe<l)
		return;
	if(qs<=l&&qe>=r){
		if(s==0)
	...
	prop(tr);
	int m=(l+r)>>1;
	doit(qs,qe,s,drvo[tr].l,l,m);
	doit(qs,qe,s,drvo[tr].r,m+1,r);
	merge(tr,drvo[tr].l,drvo[tr].r);
}
void update(int pos,int tr=ind,int l=0,int r=m+1){
	if(l==r){
		calcVal(tr,l);
		return;
	}
	...
	int m=(l+r)>>1;
	if(pos<=m)
		update(pos,drvo[tr].l,l,m);
	else
		update(pos,drvo[tr].r,m+1,r);
	merge(tr,drvo[tr].l,drvo[tr].r);
}
int main()
{
