...
struct DetectCycle{
	...
	void add_edge(int u, int v){eid--, add_edge(u, v, eid);}
	void add_edge(int u, int v, int eid){
		G[u].push_back(edge(v, eid));
		if(u != v) G[v].push_back(edge(u, eid));
	}
	int dfs(int v, int eid, vector<ll> &vdest, vector<ll> &edest)
	{
		used[v] = 1;
		for(auto e : G[v]){
			int u = e.to, id = e.id;
			if(id == eid) continue;
			if(used[u]){
				vdest.push_back(v), edest.push_back(id);
				if(u == v) return -1;
				return u;
			}
			int res = dfs(u, id, vdest, edest);
			if(res == -1) return -1;
			if(res == -2) continue;
			vdest.push_back(v), edest.push_back(id);
			if(v == res) return -1;
	...
	bool getCycle(int v, vector<ll> &vdest, vector<ll> &edest){ //vcycle/IDvdest/edestcycle
		vdest.clear(), edest.clear();
		return dfs(v, -1, vdest, edest) == -1;
	}
};
...
int main(void)
{
	...
	rep(i, 1, m){
		...
		if(sz(vec[i]) == 1){
			if(vec[i][0] > 0) ans[i] = 1;
			dc.add_edge(abs(vec[i][0]), abs(vec[i][0]), m+i);
			continue;
		}
		if(sgn(vec[i][0]) == sgn(vec[i][1])){
			if(vec[i][0] > 0) ans[i] = 1;
			dc.add_edge(abs(vec[i][0]), abs(vec[i][0]), m+i);
			dc.add_edge(abs(vec[i][1]), abs(vec[i][1]), 2*m+i);
			continue;
		}
		if(vec[i][0] < 0) swap(vec[i][0], vec[i][1]);
		p[i] = P(abs(vec[i][0]), abs(vec[i][1]));
		dc.add_edge(abs(vec[i][0]), abs(vec[i][1]), i);
	}
	
	rep(i, 1, n){
		if(used[i]) continue;
		vector<ll> vdest, edest;
		if(!dc.getCycle(i, vdest, edest)){
			no();
			return 0;
		}
		reps(j, edest){
			used[vdest[j]] = true;
			if(vdest[j] != p[edest[j]].first) ans[edest[j]] = 1;
		}
		reps(j, vdest) dfs(vdest[j]);
	}
