...
/*/-----------------------------Code begins----------------------------------/*/
int po[maxn];
map<int,bool> dp[2][5050][505];


vi stk;
bool pos(int len,int d,int s,int start,int m){
    m%=d;
    if (s<0) return false;
    if (len==0){
        if (s!=0 || m!=0 || start) return false;
        return true;
    }
    
    if (len*9<s) return false;
    if (dp[start][s][m].find(len)!=dp[start][s][m].end()) return dp[start][s][m][len];
    if (s==0){
        if (m!=0 || start) return false;
        For(i,len) stk.pb(0);
        return true;
    }
    for(int i=start;i<10;++i){
        if (pos(len-1,d,s-i,0,(m+(po[len-1]*i)%d)%d)){
            stk.pb(i);
            dp[start][s][m][len]=true;
            return true;
        }
    }
    dp[start][s][m][len]=false;
    return false;
...
void solve(){
    int d,s;
    read(d);read(s);
    if ((d%3==0 && s%3!=0)||(d%9==0 && s%9!=0)) {d1(-1);return;}
    po[0]=1;
    For(i,maxn-1) po[i+1]=(po[i]*10)%d;
    For(i,MaxN/10){
        if (pos(i+1,d,s,1,0)){
            while(!stk.empty()){
                cout<<stk.back();
