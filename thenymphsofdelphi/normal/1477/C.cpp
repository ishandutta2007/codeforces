...
int dot(pii a, pii b, pii c){
    return dot({a.fi - b.fi, a.se - b.se}, {c.fi - b.fi, c.se - b.se});
}
...
signed main(){
    ...
    vi ans; ans.push_back(1); ans.push_back(2);
    ForE(i, 3, n){
        if (dot(a[ans[isz(ans) - 2]], a[ans[isz(ans) - 1]], a[i]) > 0){
            ans.push_back(i);
        }
        else{
            int j = i - 4;
            while (j >= 0 && dot(a[ans[j]], a[ans[j + 1]], a[i]) <= 0){
                j--;
            }
            ans.insert(ans.begin() + j + 2, i);
        }
    }
    PrintV(ans);
}
