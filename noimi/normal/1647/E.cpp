...
template <typename T = int> struct Doubling {
    ...
    Doubling(int n) : n(n) {
        ...
        nxt.resize(n), cost.assign(n, 1);
    }
    Doubling(const vector<int> &_nxt, const vector<T> &_cost, int lg = 0) {
        n = _nxt.size();
        LOG = (lg ? lg : 32 - __builtin_clz(n) + 1);
        nxt.assign(LOG, vector<int>(n)), cost.assign(LOG, vector<T>(n));
        nxt[0] = _nxt, cost[0] = _cost;
        rep(i, 1, LOG) rep(j, n) {
            nxt[i][j] = nxt[i - 1][nxt[i - 1][j]];
            cost[i][j] = cost[i - 1][j] + cost[i - 1][nxt[i - 1][j]];
        }
    }
    Doubling(const vector<int> &_nxt, int lg = 0) : Doubling(_nxt, vi(si(_nxt), 1), lg) {}

    pair<int, T> move(int from, T times) {
        int to = from;
        T res = 0;
        for(int i = 0; times; i++, times >>= 1) {
            if(times & 1) {
                res += cost[i][to];
                to = nxt[i][to];
            }
    ...
    bool can_move(int l, int r) {
        per(i, LOG, 0) {
            if(nxt[i][l] <= r) { l = nxt[i][l]; }
        }
        return l == r;
    }

    // l  r 
    T move_cnt(int l, int r) {
        if(l >= r) return 0;
        T res = 0;
        per(i, LOG, 0) {
            if(nxt[i][l] < r) { res += cost[i][l], l = nxt[i][l]; }
        }
        res++;
        return res;
    }
    ...
    // f 
    template <typename F> pair<T, int> move_cnt(int l, const F &f) {
        assert(f(l));
        T res = 0;
        per(i, LOG, 0) {
            if(f(nxt[i][l])) res += cost[i][l], l = nxt[i][l];
        }
        return {res, l};
    }
};
    ...
    if(d) {
        ...
        rep(i, n) {
            if(si(v[i])) {
                ans[v[i][0]] = b[i];
                used[b[i]] = true;
                rep(j, 1, si(v[i])) ret.eb(v[i][j], b[i]);
            }
        }
        SORT(ret);
        set<int> s;
        rep(i, n) if(!used[i]) s.emplace(i);
        fore(x, y, ret) {
