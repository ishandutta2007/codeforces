...
void Delete(ll x)
{
	S.erase(x);
	ll L = getFront(x);
	len[L] += len[x], seg.update(L, get(len[L]));
	len[x] = 0, seg.update(x, get(len[x]));
}

void Add(ll x)
{
	ll L = getFront(x), R = getNextFront(x);
	S.insert(x);
	len[x] = R-x, seg.update(x, get(len[x]));
	len[L] -= len[x], seg.update(L, get(len[L]));
}
...
int main(void)
{
	...
	rep(i, 1, n) cin >> a[i];
	a[0] = inf, a[n+1] = -inf;
	rep(i, 1, n) if(a[i-1] > a[i]) S.insert(i);
	...
	rep(i, 1, Q){
		...
		if(t == 1){
			cin >> x >> y;
			if(S.count(x)) Delete(x);
			if(S.count(x+1) && x+1 <= n) Delete(x+1);
			a[x] = y;
			if(a[x-1] > a[x]) Add(x);
			if(a[x] > a[x+1] && x+1 <= n) Add(x+1);
		}
		if(t == 2){
			cin >> l >> r;
			ll L = getNextFront(l), R = getFront(r);
			if(L > R){
				cout << get(r-l+1) << "\n";
				continue;
			}
			ll ans = get(L-l) + get(r-R+1) + seg.query(L, R-1);
			cout << ans << "\n";
		}
	}
