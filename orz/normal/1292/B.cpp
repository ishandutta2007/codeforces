...
const ll dst(const pll& x, const pll& y)
{
...
int another_main()
{
	const ll MAXCOR = 2e16 + 6;
	const ll MEM = 1e18;
	ll x0, y0, ax, ay, bx, by, xs, ys, t;
	cin >> x0 >> y0 >> ax >> ay >> bx >> by >> xs >> ys >> t;
	vpll kek(64, pll(x0, y0));
	for (int i = 1; i < 64; ++i)
	{
		kek[i].X = ax * kek[i - 1].X + bx;
		kek[i].Y = ay * kek[i - 1].Y + by;
		if ((kek[i - 1].X == MAXCOR) || (kek[i].X >= MAXCOR) || (kek[i].Y >= MAXCOR))
		{
			kek[i].X = MAXCOR;
			kek[i].Y = MAXCOR;
		}
	...
	for (int i = 0; i < 64; ++i)
		for (int j = 0; j < 64; ++j)
			if (dst(s, kek[i]) + dst(kek[i], kek[j]) <= t)
