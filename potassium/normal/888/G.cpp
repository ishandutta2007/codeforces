...
struct Trie {
	...
	inline void insert(int val) {
		...
		for (int i = 0; i < (int)s.length(); i++)
			if (trie[cur].child[toNum(s[i])])
				cur = trie[cur].child[toNum(s[i])];
			else {
				cur = trie[cur].child[toNum(s[i])] = trie.size();
				trie.emplace_back();
			}
		trie[cur].val = val;
	}
	...
	inline int query(int val) {
		...
		for (int i = 0; i < (int)s.length(); i++)
			if (trie[cur].child[toNum(s[i])])
				...
				int nxt = trie[cur].child[!toNum(s[i])];
				assert(nxt != 0);
				cur = nxt;
			}
		assert(trie[cur].val != -1);
		return trie[cur].val ^ val;
	}
...
ll solve(vector<int>& a) {
	while (a.back() > 0) {
		int highbit = 1 << (31 - __builtin_clz(a.back()));
		if (a.front() & highbit)
			for (auto& i : a)
				i ^= highbit;
		else
	...
	int minEdge = INT_MAX;
	for (auto& i : r)
		minEdge = min(minEdge, trie.query(i));

	return minEdge + solve(l) + solve(r);
}
