...
int main(int /* argc */, const char** /* argv */)
{
    ...
    while (cin >> n >> m >> X && n) {
        ...
        vector<priority_queue<tuple<ll, int, int>>> hs(n);
        ...
        for (int u = 0; u < n; ++u) {
            if (hs[u].size()) {
                auto [_, y, ind] = hs[u].top();
                h.push({a[u] + a[y], u, y, ind});
            }
        }
