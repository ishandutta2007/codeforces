/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 352 (Div. 1)         |
|       Task: A                                     |
|       Date: 11. 05. 2016                          |
\--------------------------------------------------*/
// Includes
...
pair<int,int> a = in, b = in, t = in;
int n = in;
vector<pair<int, int>> v;
vector<LD> q(n), pA(n), pB(n);
vector<pair<LD, int>> wA(n), wB(n);

inline LD sqr(LD a) { return a * a; }
inline LD dist(pair<int,int> a, pair<int,int> b) { return sqrt(sqr(a.F - b.F) + sqr(a.S - b.S)); }
...
main()
{
    ...
    LD res = min(wA[0].F, wB[0].F);
    for (int i = 0; i < min((int)v.size(), 2); i++)
        for (int j = 0; j < min((int)v.size(), 2); j++)
            if (wA[i].S != wB[j].S)
                res = min(res, wA[i].F + wB[j].F);
    sum += res;
    out = ofl(sum, 12);
    return 0;
}