...
void main_() {
	...
	Graph H(n, -1, 0, 0, 0);
	deque<int> deq = {root};
	auto G = H.root_to_leaf(root);
	V<> minimal(n);
	auto mdfs = [&](auto self, int now) -> int {
		int ret = now;
		for(auto &[to, cost] : G[now]) {
			chmin(ret, self(self, to));
		}
		return minimal[now] = ret;
	};
	mdfs(mdfs, 0);
	auto dfs = [&](auto self, int now, int type) -> void {
		if(now == root) {
			...
			for(auto &[to, cost] : G[now]) {
				if(minimal[to] == m1)
					deq.push_back(to), self(self, to, 0);
				if(minimal[to] == m2)
					deq.push_front(to), self(self, to, 1);
			}
		} else {
			int min_idx = inf;
			for(auto &[to, cost] : G[now]) {
				chmin(min_idx, minimal[to]);
			}
			for(auto &[to, cost] : G[now]) {
				if(min_idx == minimal[to]) {
					if(!type)
						deq.push_back(to);
					else
						deq.push_front(to);
	...
	dfs(dfs, root, 0);
	V<ll> partial_weight(n);
	auto pdfs = [&](auto self, int now) -> int {
		int ret = 1;
		for(auto &[to, cost] : G[now]) {
			ret += self(self, to);
		}
		return partial_weight[now] = ret;
	};
	pdfs(pdfs, root);
	V<> paths;
	REP(i, SZ(deq)) { paths.pb(deq[i]); }
	debug(paths);
	V<> pl(n, -1);
	REP(i, SZ(paths)) {
		pl[paths[i]] = i;
	}
	int l = pl[root], r = pl[root];
	V<ll> res(n + 1);
	REP(i, n) {
		if(i == 0) {
			ll lw = 0, rw = 0;
			for(auto &[to, cost] : G[root]) {
				auto &v = partial_weight[to];
				lw += v * (v - 1) / 2;
			}
			res[0] = lw;
		} else {
			...
			chmax(r, pl[i]);
			debug(l, r);
			ll lw = partial_weight[paths[l]] - (paths[l] == root ? partial_weight[paths[l + 1]] : 0);
			ll rw = partial_weight[paths[r]] - (paths[r] == root ? partial_weight[paths[r - 1]] : 0);
			debug(lw, rw);
			res[i + 1] = lw * rw;
		}
	}
