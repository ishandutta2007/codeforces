...
struct union_find {
  ...
  union_find(li n = 0) : A(n),S(n,1),D(n,0) {
...
void solve(){
  ...
  vi vis(n);
  vi ans(n);
  for(li i = 0; i < (li)(n); ++i) if(!vis[i]) {
    vi C;
    C.emplace_back(ask(i));
    while(1) {
      int j = ask(i);
      if(j == C[0]) break;
      C.emplace_back(j);
    ...
    for(int j : C) vis[j]=1;
    for(li j = 0; j < (li)(C.size()); ++j) ans[C[j]] = C[(j+1)%C.size()];
  }
  cout << "! ";
  for(li i = 0; i < (li)(n); ++i) cout << 1+ans[i] << ' ';
  cout << endl << flush;
