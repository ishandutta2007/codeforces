...
void solve() {
	...
	vi at(n + 1);
	REP(i, 2 * n) {
		if (op[i].first == PUT) puts_.insert(i);
		else {
			takes.insert(i);
			at[op[i].second] = i;
		}
	...
	for (int i = 1; i <= n; ++i) {
		int from = at[i];
		auto it = puts_.lower_bound(from);
		if (it == puts_.begin()) err();
		it--;
		int when_put = *it;
		auto it2 = takes.lower_bound(when_put);
		if (*it2 != from) err();
		ans[when_put] = i;
		puts_.erase(when_put);
		takes.erase(from);
	}
	debug(imie(ans));
	puts("YES");
