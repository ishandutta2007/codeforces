...
/*link https://codeforces.com/problemset/problem/60/D */
/*input
1
2
*/
...
int main () {
	...
	for (ll n = 0; n <= lima; n++) {
		for (ll m = n + 1; ; m += 2)
		{
			...
			if (b < lima && a < lima && num[a] and num[b])
				con -= un.merge(num[a], num[b]);

			if (a < lima && c < lima && num[a] and num[c])
				con -= un.merge(num[a], num[c]);

			if (c < lima && b < lima && num[c] and num[b])
				con -= un.merge(num[c], num[b]);
		}
	}
