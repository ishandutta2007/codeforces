...
void solve() {
	...
	using mat = array<ar, 2>;
	auto f = [&](mat a, mat b) {
		mat res;
		rep(i, 2)rep(j, 2)res[i][j] = mod;
	...
	mat e;
	rep(i, 2)e[i][i] = 0, e[i][i ^ 1] = mod;
	SegT<mat> st(n + 1, e, f);
	mat ori[4];
	ori[0] = e;
	ori[1][0][0] = 1, ori[1][1][0] = 1, ori[1][0][1] = mod, ori[1][1][1] = 0;
	ori[2][0][0] = 0, ori[2][0][1] = 1, ori[2][1][0] = mod, ori[2][1][1] = 1;
	rep(i, 2)rep(j, 2)ori[3][i][j] = 1;
	rep(i, n)cr[a[i]]++;
	cr[0]++;
	cl[0]++;
	auto upd = [&](int id) {
