// not too optimistic, but let's see how it goes.
...
void up(){
	...
	auto const check=[&](int n, int m){
		...
		if(std::accumulate(begin(temp), end(temp), int64_t(0)) < n) return false;

		if(n%2!=0 and
				*std::max_element(begin(temp), end(temp))==2){
			return false;
		}
