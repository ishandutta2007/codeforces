...
void solve() {
    ...
    for (int mex = 1; mex < n; mex++) {
        ...
        if (l > pos[mex]) {
            int min_l = pos[mex] + 1;
            for (int i = min_l; i <= l; i++) ans += max(0, min(n - 1, i + max_diff) - r + 1);
            l = pos[mex];
        } else if (r < pos[mex]) {
            int max_r = pos[mex] - 1;
            for (int i = r; i <= max_r; i++) ans += max(0, l - max(0, i - max_diff) + 1);
            r = pos[mex];
        } else {
            // impossible to have this mex
        }
    }
    cout << ans << '\n';
}
