...
void dfs(int u){
    ...
    for (auto [c,w]:g[u]){
        if (!col[c]) continue;
        if (vis[c]) {gd=__gcd(gd,max(d[c]-d[u]-w,d[u]+w-d[c])); continue;}
        d[c]=d[u]+w;
        dfs(c);
    }
}
...
void f(vi& comp){
    if (sz(comp)==1) return;
    gd=0;
    for (auto c:comp) col[c]=1;
    dfs(comp[0]);
    for (auto c:comp) rem[c]=d[comp[0]], md[c]=gd, col[c]=0;
