...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	T{
		...
		rep(i,6) ps[i].Y=i;
		rep(i,n)rep(j,n){
			if(mp[i][j]=='O') ++ps[(i+j)%3*2].X;
			else if(mp[i][j]=='X') ++ps[(i+j)%3*2+1].X;
		}
		sort(all(ps));
		auto re=mp;
		pii p;
		int mn=MOD;
		rep(i,6)rep(j,i){
			if(ps[i].Y%2==ps[j].Y%2) continue;
			if(ps[i].Y/2==ps[j].Y/2) continue;
			if(MN(mn,ps[i].X+ps[j].X)) p={j,i};
		}
		out(ps,p,1);
		rep(i,n)rep(j,n)if((i+j)%3==ps[p.X].Y/2 && re[i][j]!='.') re[i][j]="XO"[ps[p.X].Y%2];
		rep(i,n)rep(j,n)if((i+j)%3==ps[p.Y].Y/2 && re[i][j]!='.') re[i][j]="XO"[ps[p.Y].Y%2];
