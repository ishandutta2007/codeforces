// time-limit: 4000
// problem-url: https://codeforces.com/contest/1710/problem/C
...
int main(){
	...
	std::string  n=read<std::string>();
	for(auto& ch: n) ch-='0';


	auto const compute=[&](std::array<int, 8> allowedMasks)->modular{
		std::array<bool, 8> isAllowed {};
		for(auto mask: allowedMasks){
			isAllowed[mask]=true;
		}

		std::array<modular, 8> count{}; // bit 0: not strictly less yet, bit 1: okay good
		count[0]=1;

		for(auto curNDigit: n){
			auto oldCount=count;
			count={};

			for(int const nextDigits: range(0, 8)) if(isAllowed[nextDigits]){
				for(int const oldLessStatus: range(0, 8)){
					int lessStatus=oldLessStatus;
					for(int const bit: range(0, 3)){
						int const nextDigit=nextDigits>>bit&1;
						int const oldLessStatusBit=oldLessStatus>>bit&1;

						if(nextDigit>curNDigit and not oldLessStatusBit){
							goto cannot_add;
						}
						if(nextDigit<curNDigit)
							lessStatus|=1<<bit;
					}

					count[lessStatus]+=oldCount[oldLessStatus];
cannot_add:;
				}
			}
		}

		return std::accumulate(begin(count), end(count), modular());
	};

	std::cout<<
		int(
				+ compute({{0b000, 0b001, 0b010, 0b011, 0b100, 0b101, 0b110, 0b111}})
				- compute({{0b000, 0b001, 0b010, 0b101, 0b110, 0b111}})
				- compute({{0b000, 0b001, 0b011, 0b100, 0b110, 0b111}})
				- compute({{0b000, 0b010, 0b011, 0b100, 0b101, 0b111}})
				+ compute({{0b000, 0b001, 0b110, 0b111}})
				+ compute({{0b000, 0b010, 0b101, 0b111}})
				+ compute({{0b000, 0b100, 0b011, 0b111}})
				- compute({{0b000, 0b111}})
		   )<<'\n';

	// hopefully compiler can optimize this
