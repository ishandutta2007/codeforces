...
ll n, k;
ll b[300005];
ll d2[300005], d[300005], a[300005];
...
int main(void)
{
	...
	ll ans = 0;
	per(i, n, 1){
		d[i] = d[i+1] + d2[i], a[i] = a[i+1] + d[i];
		
		if(i >= k){
			ll x = ceil(max(0LL, b[i]-a[i]), k);
			ans += x;
			d2[i] += k*x, d2[i-1] -= (k+1)*x;
			if(i-k-1 >= 0) d2[i-k-1] += x;
			d[i] = d[i+1] + d2[i], a[i] = a[i+1] + d[i];
		}
	}
	
	ll mx = 0;
	rep(i, 1, k) chmax(mx, ceil(max(0LL, b[i]-a[i]), i));
	ans += mx;
	outl(ans);
