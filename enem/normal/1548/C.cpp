...
ll xd(ll a[], ll n)
{
    ...
    v[0].PB(abs(a[n/2+1]-a[n/2]));
    if(v[0].back() != 1) ans = max(ans, ((ll)v[0].size()) + 1LL);
    REP(i,n/2+2,n)
    {
        v[0].PB(__gcd(v[0].back(), abs(a[i]-a[n/2])));
        if(v[0].back() != 1) ans = max(ans, ((ll)v[0].size()) + 1LL);
    }
    
    v[1].PB(abs(a[n/2-1]-a[n/2]));
    if(v[1].back() != 1) ans = max(ans, ((ll)v[1].size()) + 1LL);
    REPI(i,0,n/2-1)
    {
        v[1].PB(__gcd(v[1].back(), abs(a[i]-a[n/2])));
        if(v[1].back() != 1) ans = max(ans, ((ll)v[1].size()) + 1LL);
    }

    ll r = v[1].size() - 1, l = 0;
    while(r >=0 && l < ((ll)v[0].size()))
    {
        if(__gcd(v[1][r], v[0][l]) != 1)
        {
            ans = max(ans, 3 + l + r);
            l++;
        }
    ...
    return max({ans, xd(a, n/2), xd(a+n/2+1, n-n/2-1)});
}
...
int main()
{
    ...
    static const ll N = 3e6 + 10;
    static ll fc[N], ifc[N];

    fc[0] = 1LL;
    REP(i, 1, N) fc[i] = (fc[i-1] * i) % M1;
    ifc[N-1] = modI(fc[N-1], M1);
    REPI(i, 0, N-1) ifc[i] = (ifc[i+1] * (i+1)) % M1;
    ...
    REP(i,0,3*n+4)
    {
        v[i] = (( fc[3*n+3] * ((ifc[i]*ifc[3*n+3-i])%M1) )%M1);
    }
    v[0]-=1;v[1]-=3;v[2]-=3;v[3]-=1;

    V(ll) ans(3*n+1);
    REPI(i,0,3*n+1)
    {
        ans[i] = v[i+3];
        v[i+2] += 3LL*M1 - 3*v[i+3];
        v[i+2] %= M1;
        v[i+1] += 3LL*M1 - 3*v[i+3];
        v[i+1] %= M1;
    }
