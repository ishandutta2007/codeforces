...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        while(cur <= a + b)
        {
            ...
            if(gp >= rgtgp)
            {
                ll diff = (gp - rgtgp + t - mdt - 1) / t - (lftgp + t - mdt) / t + 1;
                if(diff < 0) diff = 0;
                if(diff > (((gp - rgtgp) / t) + 1)) diff = (((gp - rgtgp) / t) + 1);
                gp -= t * diff;
                if(gp >= rgtgp) ans += (((gp - rgtgp) / t) + 1);
                if(gp >= rgtgp) gp -= t * (((gp - rgtgp) / t) + 1);
            }
            if(gp >= lftgp)
            {
                if(lftgp >= mdt) ans += (((gp - lftgp) / t) + 1);
                gp -= t * (((gp - lftgp) / t) + 1);
            }
            if(gp >= 0)
            {
                if(gp >= mdt) ans += (((gp - mdt) / t) + 1);
                gp -= t * ((gp / t) + 1);
            }

            // while(gp >= 0)
            // {
            //     if((min(gp, lftgp) + t - mdt)/t > (max(0LL, gp - rgtgp) + t - mdt - 1)/t) ans++;
            //     gp -= t;
            // }
