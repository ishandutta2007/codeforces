#line 1 "codeforces-workspace/contest/367/d/d.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/367/d/d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/367/d/d.cpp"
...
#line 2 "Library/src/algebra/convolution/zeta.hpp"
...
#line 9 "Library/src/algebra/convolution/zeta.hpp"

#line 11 "Library/src/algebra/convolution/zeta.hpp"
...
#line 27 "codeforces-workspace/contest/367/d/d.cpp"
...
void main() {
  ...
  }
  {
    int excl = 0;
    vector<int> cnt(m);
    for (auto i : range(d - 1)) {
      excl |= 1 << which[i];
      cnt[which[i]] += 1;
    }
    for (auto i : range(n + 1 - d)) {
      excl |= 1 << which[i + d - 1];
      cnt[which[i + d - 1]] += 1;
      viol[excl] += 1;
      if (!--cnt[which[i]]) {
  ...
  viol = fast_zeta(viol);
  int ans = m;
  for (auto &&[s, v] : enumerate(viol)) {
    if (!v) {
      chle(ans, m - __popcount(s));
    }
  }
  cout << ans << "\n";
}
