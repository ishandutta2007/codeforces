...
public:
    ...
    value_t(char c) {
        for (int i = 0; i < 26; i++) s[i] = (i != c);
        for (int i = 0; i < 26; i++) f[i] = (i != c);
    }

    friend value_t operator+(const value_t& left, const value_t& right) {
        value_t res;
        for (int i = 0; i < 26; i++) res.s[i] = left.s[i] + right.s[i];
        for (int i = 0; i < 25; i++) {
            res.f[i] = min(left.s[i] + right.f[i + 1], right.s[i] + left.f[i + 1]);
            res.f[i] = min(res.f[i], int(1e9));
        }
        res.f[25] = 1e9;
        return res;
    }
};
value_t solve(int l, int r) {
    if (l == r) {
        return value_t(s[l]);
    } else {
        int m = (l + r) / 2;
