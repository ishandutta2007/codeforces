...
fun main(){
    ...
    solve.cases{
        ...
        for(bottom in n downTo 0){
            val shift = (n- bottom)
            val herepre = k modM intPow(2,shift-1,pI) modM shift
            val here = herepre
//            val here = herepre modMinus (2 modM last)
//            last = herepre
            val mopminus = (n-m)
            if(bottom < mopminus){
                 break
...
//            val factor = intPow(2, bottom,pI)
            val extra = here modM (FACT.choose(bottom-1, mopminus-1))

//            just dei Pair(here,extra)
            ret = ret modPlus extra
        }
//        val totalshift = (n-m) + n
        put(ret modDivide intPow(2,n-1, pI))
        //modDivide intPow(2,totalshift, pI)
    }
    done()
}
