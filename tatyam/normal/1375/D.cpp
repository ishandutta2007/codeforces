...
void solve(){
    ...
    VEC(ll,a,n);
    set<ll>empty,q;
    vec(set<ll>,index,n+1);
    rep(n)index[a[i]].insert(i);
    rep(n+1)if(index[i].empty())empty.insert(i);
    rep(n)if(a[i]!=i)q.insert(i);
    vector<ll>ans;
    auto operation=[&](ll at){
        ans.push_back(at);
        ll mex=*empty.begin();
        empty.erase(empty.begin());
        index[a[at]].erase(at);
        if(index[a[at]].empty())empty.insert(a[at]);
        a[at]=mex;
        index[a[at]].insert(at);
        if(a[at]==at)q.erase(at);
    };
    while(q.size()){
        ll k=*empty.begin();
        if(k<n){
