...
int a[2000007];
bool v[2000007];
int x[2000007],y[2000007];
double eps=1e-7;
int comp(int b,int a,int d,int c)
{
	// a/b >= c/d
	// printf("%d %d %d %d\n",a,b,c,d);
	if(b>0)return a*d>=b*c;
	return a*d<=b*c;
}
signed main()
{
	...
	if(lst==0)
	{
		...
		while(top>1&&comp(i-x[top],a[i]-y[top],x[top]-x[top-1],y[top]-y[top-1]))top--;
		top++;
		x[top]=i;
		y[top]=a[i];
	}
