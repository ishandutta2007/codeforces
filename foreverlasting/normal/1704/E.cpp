...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		n=read(),m=read();
		for(int i=1;i<=n;i++)a[i]=read(),vector<int>().swap(G[i]),deg[i]=Deg[i]=0;
		for(int i=1;i<=m;i++){
			int u=read(),v=read();
			G[u].pb(v),deg[v]++,Deg[u]++;
		}
		for(int i=1;i<=n;i++){
			for(int j=0;j<=n+1;j++)f[i][j]=fm[i][j]=0;
			if(a[i])f[i][0]=a[i]%kcz,fm[i][0]=min(a[i],2000);
		}
		he=1,ta=0;
		...
		while(he<=ta){
			...
			for(auto tox:G[x]){
				for(int j=0;j<=n;j++){
					if(!fm[x][j])continue;
					fm[tox][j+1]=min(2000,fm[tox][j+1]+fm[x][j]);
					add(f[tox][j+1],f[x][j]);
				}
				if(!--deg[tox])Q[++ta]=tox;
			}
		...
		int ed=0;
		for(int i=1;i<=n;i++)if(!Deg[i])ed=i;
		int s=0;
		for(int i=0;i<=n;i++){
			if(fm[ed][i+1]){
				s+=fm[ed][i];
				s=min(s,2000);
				if(s>i+1)add(ans,f[ed][i]);
				else s=ans=i+1;
			}
			else add(ans,f[ed][i]),s+=fm[ed][i],s=min(s,2000);
		}
		printf("%d\n",ans);
	}
}
