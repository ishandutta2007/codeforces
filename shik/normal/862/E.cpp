...
void solve() {
    int k=lower_bound(ALL(v),-base)-v.begin();
    LL ans=LLONG_MAX;
    REP1(i,max(0,k-2),min(SZ(v)-1,k+2)) {
        // dump(i,base,v[i],abs(base+v[i]));
        chkmin(ans,abs(base+v[i]));
    }
    W(ans);
}
...
int main() {
    ...
    REP1(i,1,n) R(a[i]);
    REP1(i,1,m) R(b[i]);
    REP1(i,1,n) base+=(i%2==1 ? +a[i] : -a[i]);
    LL s=0;
    REP1(i,1,m) {
        s+=(i%2==1 ? -b[i] : +b[i]);
        if ( i>n ) s-=((i-n)%2==1 ? -b[i-n] : +b[i-n]);
        if ( i>=n ) v.PB((i-n)%2==0 ? +s : -s);
    }
    sort_uniq(v);
    dump(base,v);
    solve();
