...
public:
    ...
    trie_t() {
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++) mx[i][j] = 0;
        for (int i = 0; i < 2; i++) g[i] = nullptr;
    }

    void get_ans(int a, int i, int level, int& res) {
        // prefix until this point is the same, see the next point
        if (level == -1) return;
        int bit_i = (i & (1 << level)) > 0;
        int bit_a_i = (a & (1 << level)) > 0;
        for (int bit_j = 0; bit_j < 2; bit_j++)
            for (int bit_a_j = 0; bit_a_j < 2; bit_a_j++) {
                if ((bit_a_j ^ bit_i) < (bit_a_i ^ bit_j)) {
                    res = max(res, mx[bit_j][bit_a_j] + 1);
                }
            }
    }
    void update_ans(int a, int i, int level, int& res) {
        // prefix until this point is the same, see the next point
        if (level == -1) return;
        int bit_i = (i & (1 << level)) > 0;
        int bit_a_i = (a & (1 << level)) > 0;
        mx[bit_i][bit_a_i] = max(mx[bit_i][bit_a_i], res);
    }

    static void add(trie_t* root, int a, int i, int level) {
        // cerr << a << ' ' << i << ' ' << (a ^ i) << '\n';
        auto original_root = root;
        int original = level;
        int res = 1;
        root->get_ans(a, i, level, res);
        while (level >= 0) {
            bool bit = (a ^ i) & (1 << level);
            if (!root->g[bit]) root->g[bit] = new trie_t();
            root = root->g[bit];
            level--;
            root->get_ans(a, i, level, res);
        }
