...
val deg = IntArray(5001)
val adj = Array<ArrayList<Int>>(5001,{ArrayList<Int>()})

fun ok(n: Int): Boolean {
    val L = LinkedList<Int>()
    var num = 0
    for (i in 1..n) if (deg[i] == 0) {
        L.add(i)
    }
    while (L.size > 0) {
        val x = L.poll()
...
fun solve() {
    ...
    if (ok(n)) {
        ...
        println(2)
        for (i in 0..m-1) {
            if (ed[i].first < ed[i].second) print("1 ")
            else print("2 ")
        }
    }
}
