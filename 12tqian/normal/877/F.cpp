...
int l = 0, r = -1;
ll cans = 0;
void modify(int x, int side, int y = 1) {
    if(side == 0){
        ...
        if(y == 1){
            cans += y*oc[num[x][1]];
            oc[num[x][0]] += y;
          //  ps("I ADDED", num[x][0], "I QUERIED", num[x][1], "I GOT", oc[num[x][1]]);
        }
        else{
            oc[num[x][0]] += y;
            cans += y*oc[num[x][1]];
...
int answer(int L, int R) { // modifyjust interval
	while (l > L) modify(--l, 0);
	while (r < R) modify(++r, 1);
	while (l < L) modify(l++,0,-1);
...
int main(){
    ...
    f0r(i, n+1){
        num[i][0] = conv[pre[i]];
        num[i][1] = conv[pre[i] -k];
        num[i][2] = conv[pre[i] + k];
    }
    int q;
    re(q);
    f0r(i, q){
        int li, ri; re(li, ri);
        array<int, 3> tmp = {li-1, ri, i};
        todo.eb(tmp);
    }
    solve();
    f0r(i, q) ps(ans[i]);
