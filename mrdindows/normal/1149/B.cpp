...
string s;
string t[3];
int extend(int pos, char c) {
    if (pos == INF) return INF;
    return nex[pos][c] + 1;
}
void relax(int& what, int val) {
    what = min(what, val);
}
int solve() {
    int n, q;
    ...
    FI(251) FJ(251) FK(251) dp[i][j][k] = INF;
    dp[0][0][0] = 0;
    while (q--) {
        char c;
        int idx;
            ...
            if (idx == 0) {
                ...
                for (int i = 0; i <= t[1].size(); ++i) {
                    for (int j = 0; j <= t[2].size(); ++j) {
                        relax(dp[k][i][j], extend(dp[k - 1][i][j], f));
                        if (i != t[1].size()) {
                            relax(dp[k][i + 1][j], extend(dp[k][i][j], t[1][i]));
                        }
                        if (j != t[2].size()) {
                            relax(dp[k][i][j + 1], extend(dp[k][i][j], t[2][j]));
                        }
                    }
                }
            } else if (idx == 1) {
                ...
                for (int i = 0; i <= t[0].size(); ++i) {
                    for (int j = 0; j <= t[2].size(); ++j) {
                        relax(dp[i][k][j], extend(dp[i][k - 1][j], f));
                        if (i != t[0].size()) {
                            relax(dp[i + 1][k][j], extend(dp[i][k][j], t[0][i]));
                        }
                        if (j != t[2].size()) {
                            relax(dp[i][k][j + 1], extend(dp[i][k][j], t[2][j]));
                        }
                    }
                }
            } else {
                ...
                for (int i = 0; i <= t[0].size(); ++i) {
                    for (int j = 0; j <= t[1].size(); ++j) {
                        relax(dp[i][j][k], extend(dp[i][j][k - 1], f));
                        if (i != t[0].size()) {
                            relax(dp[i + 1][j][k], extend(dp[i][j][k], t[0][i]));
                        }
                        if (j != t[1].size()) {
                            relax(dp[i][j + 1][k], extend(dp[i][j][k], t[1][j]));
                        }
                    }
                }
            }
        }
        cout << (dp[t[0].size()][t[1].size()][t[2].size()] != INF ? "YES" : "NO") << endl;
    }
