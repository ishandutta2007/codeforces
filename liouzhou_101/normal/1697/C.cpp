...
void solve()
{
	...
	for (int x = 0; x < n; ++x)
	{
		...
		if (cs == 0 && ct == 1)
		{
			if (gs[1].empty()) return writeln("NO");
			if (!gs[2].empty() && *gs[2].begin() < *gs[1].begin()) return writeln("NO");
			int ii = *gs[1].begin();
			hs.erase(hs.find({ ii, 1 }));
			gs[1].erase(ii);
			ht.erase(ht.begin());
			continue;
		}
		if (cs == 1 && ct == 2)
		{
			if (gs[2].empty()) return writeln("NO");
			if (!gs[0].empty() && *gs[0].begin() < *gs[2].begin()) return writeln("NO");
			int ii = *gs[2].begin();
			hs.erase(hs.find({ ii, 2 }));
			gs[2].erase(ii);
			ht.erase(ht.begin());
			continue;
		}
