...
void solve() {
  li n,a,b,c; cin>>n>>a>>b>>c;

  li ans = 0;
  auto test = [&](vi A, li n0, li n0t, li n1, li cur){
    while(1) {
      smax(ans, cur + (a+b) * min(n0,n1) + (n0>n1?a:0));
      if(!A.empty() && A.back() == 1) {
        A.pop_back();
        cur -= c;
        cur += b;
  ...
  li n1 = 0;
  string t;
  for(li i = 0; i < (li)(n); ++i) if(i < n && s[i] == '1' && s[i+1] == '1') {
    n1++;
  }else{
    t += s[i];
  }
  ...
  vi A;
  for(li i = 0; i < (li)(t.size()); ++i) if(t[i] == '1') {
    li c0 = 0;
    int j = i+1;
    while(j < (li)t.size() && t[j] == '0') {c0++;j++;}
    if(j < (li)t.size()) A.emplace_back(c0);
  }

  li n0 = 0;
  for(li i = 0; i < (li)((li)t.size()-1); ++i) if(t[i] == '0' && t[i+1] == '0') n0++;
