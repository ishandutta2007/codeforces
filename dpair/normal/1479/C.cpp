...
#include <vector>
std::vector < EDGE > ans;
inline void add(int u, int v, int w) {ans.push_back(EDGE(u, v, w));}
...
int solve(int L, int R) {
	...
	int tp = -1; while((1 << (tp + 1)) < R) ++ tp;
	if(lowbit(R) == R) {
		add(1, 2, 1); tp += 3;
		rep(i, 3, tp) { add(1, i, 1);
            REP(j, 2, i) add(j, i, 1 << (j - 2));
        } return tp;
	}
	while((1 << (tp + 1)) < R) ++ tp;
	add(1, tp + 3, 1); solve(1, 1 << tp);
    rep(i, 0, tp) if((R - 1) >> i & 1) add(i + 2, tp + 3, 1 + ((R - 1) >> (i + 1) << (i + 1)));
	return tp + 3;
}
signed main(){ int L, R; read(L, R); print("YES"); sprint(solve(L, R)); print(ans.size()); REP(i, 0, ans.size()) print(ans[i].u, ans[i].v, ans[i].w); }