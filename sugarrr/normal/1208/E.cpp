...
signed main(){fastio
    ...
    while(n--){
        ll t;cin>>t;
        vl a(t);rep(i,0,t-1)cin>>a[i];
        segtree<ll,op,e>teg(a);
        ll X=t-1;
        ll Y=m-t;
        if(X<Y){
            rep(i,0,m-1){
                ...
                ll c=min(i,t-1);
                assert(b<=c);
                ll val=teg.prod(b,c+1);
                if(i<m-t||i>=t)chmax(val,0);
                seg.apply(i,val);
            }
            ll val=max(teg.all_prod(),0);
            seg.apply(X,Y+1,val);
        }else{
            rep(i,0,m-1){
                //[t-m+i,i]
    ...
    rep(i,0,m-1)cout<<seg.get(i)<<" ";cout<<endl;
