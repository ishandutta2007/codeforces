...
void dfs(int u,int kou){

	int ss=0,mm=0;
	int vv;
	forg(i,g[u])if(e[i].v!=pre[u]){
		ss+=sz[e[i].v];
		if(upmax(mm,sz[e[i].v])){
			vv=e[i].v;
		}
	}
	if(mm-kou>ss-mm+you[u]){
		su-=1ll*(ss-mm+you[u])*2*h[u];
		dfs(vv,kou+ss-mm+you[u]);
	}else{
		su-=1ll*(sz[u]-kou)*h[u];
	}
}
