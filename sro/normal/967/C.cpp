...
INT n,m,cl,ce,v;
INT l[100007],e[100007];
INT q;

int main()
{
    scanf("%I64d%I64d%I64d%I64d%I64d",&n,&m,&cl,&ce,&v);
    for(INT i=1;i<=cl;i++)scanf("%I64d",l+i);
    sort(l+1,l+cl+1);
    l[0]=-Inf;
    l[cl+1]=Inf;
    for(INT i=1;i<=ce;i++)scanf("%I64d",e+i);
    sort(e+1,e+ce+1);
    e[0]=-Inf;
    e[ce+1]=Inf;
    scanf("%I64d",&q);
    for(INT i=0;i<q;i++)
    {
        ...
        if(cl)
        {
            INT ff=lower_bound(l+1,l+cl+1,y1)-l;
            INT fg=ff-1;
            if((l[ff]>=y1&&l[ff]<=y2)||(l[fg]>=y1&&l[fg]<=y2))
                mINT=min(mINT,abss(x2-x1)+abss(y2-y1));
            else
            {
                INT fh;
                if(l[ff]-y2>y1-l[fg])fh=y1-l[fg];
                else fh=l[ff]-y2;
                mINT=min(mINT,abss(x2-x1)+abss(y2-y1)+fh*2);
            }
        }
        //Go by elevator
        if(ce)
        {
            INT ff=lower_bound(e+1,e+ce+1,y1)-e;
            INT fg=ff-1;
            if((e[ff]>=y1&&e[ff]<=y2)||(e[fg]>=y1&&e[fg]<=y2))
                mINT=min(mINT,(abss(x2-x1)+v-1)/v+abss(y2-y1));
            else
            {
                INT fh;
                if(e[ff]-y2>y1-e[fg])fh=y1-e[fg];
                else fh=e[ff]-y2;
                mINT=min(mINT,(abss(x2-x1)+v-1)/v+abss(y2-y1)+fh*2);
            }
        }
        printf("%I64d\n",mINT);
    }
