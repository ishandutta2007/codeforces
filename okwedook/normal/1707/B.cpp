...
vector<pii> conv(vector<int> a) {
    vector<pii> ans;
    for (auto i : a) {
        if (sz(ans) == 0 || ans.back().f != i) {
            ans.pb({i, 1});
        } else {
            ++ans.back().s;
        }
...
void solve() {
    ...
    read(a);
    auto v = conv(a);
    auto process = [](const vector<pii> &v) {
        vector<pii> add;
        for (auto [val, cnt] : v) {
            if (cnt > 1) {
                add.pb({0, cnt - 1});
        ...
        for (int i = 0; i + 1 < sz(v); ++i) {
            add.pb({v[i + 1].f - v[i].f, 1});
        }
        sort(add);
        vector<pii> ans;
        for (auto i : add) {
            if (sz(ans) == 0 || i.f != ans.back().f) {
                ans.pb(i);
            } else {
                ans.back().s += i.s;
            }
