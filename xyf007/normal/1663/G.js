...
function replaceMap() {
  ...
  function numberReplacer(_, y) {
    ...
    return [output].concat(values).join("+").replace(/(\d)/g,
      digitReplacer);
  }
...
function replaceStrings() {
  ...
  for (all in MAPPING) {
    if (MAPPING[all]) {
      MAPPING[all] = MAPPING[all].replace(/\"([^\"]+)\"/gi,
        mappingReplacer);
    }
  }
...
function encode(input, wrapWithEval, runInParentScope) {
  ...
  if (unmappedCharactersCount > 1) {
    ...
    input = input.replace(/[^0123456789.adefilnrsuN]/g,
  ...
  if (unmappedCharactersCount > 1) {
    // replace `t` with `\\`
    output = "(" + output + ")[" + encode("split") + "](" + encode("t") +
      ")[" + encode("join") + "](" + encode("\\") + ")";
  }
...
print(encode(readline(), false, false));