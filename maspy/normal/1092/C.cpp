...
signed main() {
  ...
  auto calc = [&](string ANS) -> string {
    VEC<bool> pref(N);
    VEC<bool> suff(N);
    string res;
    for (auto&& T : S) {
      if (ANS.substr(0, T.size()) == T && !pref[T.size()]) {
        res += "P";
        pref[T.size()] = true;
      } else if (ANS.substr(N - T.size(), T.size()) == T && !suff[T.size()]) {
        res += "S";
        suff[T.size()] = true;
      } else {
        res += "X";
  ...
  FOR(i, 2) {
    FOR(j, 2) {
      string ANS = SS[1][i] + SS[N - 1][j];
      auto res = calc(ANS);
      if (count(all(res), 'X')) continue;
      print(res);
      exit(0);
    }
  }
