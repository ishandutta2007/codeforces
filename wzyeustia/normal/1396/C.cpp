...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ll n, r1, r2, r3, d;
        cin >> n >> r1 >> r2 >> r3 >> d;
        vector<ll> arr(n), jump(n), dont(n), sumj(n+1, 0), sumd(n+1, 0);
        REP(i, n) {
            cin >> arr[i];
            jump[i] = min(r1*arr[i]+r1, r2)+d;
            dont[i] = min(r1*arr[i]+r3+d, jump[i]+r1+2*d);

            sumj[i+1] = sumj[i] + jump[i];
            sumd[i+1] = sumd[i] + dont[i]; 
        }

        vector<ll> dp(n+5, LLONG_MAX);
        dp[0] = 0;
        FOR(i, 0, n) {
            chkmin(dp[i], dp[i-1] + dont[i-1]);
            if (i > 1) {
                chkmin(dp[i], dp[i-2] + jump[i-2] + jump[i-1] + 2*(r1+d));
            }
        }

        ll ans = LLONG_MAX;
        chkmin(ans, dp[n]-d);
        chkmin(ans, dp[n-2] + jump[n-2] + dont[n-1] + r1);
