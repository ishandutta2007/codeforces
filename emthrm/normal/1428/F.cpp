...
int main() {
  ...
  vector<int> dep(n, 0);
  REP(i, n) {
    if (s[i] == '1') dep[i] = (i > 0 ? dep[i - 1] : 0) + 1;
  }
  vector<vector<int>> idx(n + 1);
  for (int i = n - 1; i >= 0; --i) idx[dep[i]].emplace_back(i);
  vector<int> right(n, 0);
  for (int i = n - 1; i >= 0; --i) {
    if (s[i] == '1') right[i] = (i + 1 < n ? right[i + 1] : 0) + 1;
  }
  auto init = monoid::RangeSumAndAddQuery<ll>::init(n);
  REP(i, n) init[i].sum = max(i > 0 ? init[i - 1].sum : 0, 1LL * dep[i]);
  LazySegmentTree<monoid::RangeSumAndAddQuery<ll>> seg(init);
  ll ans = 0;
  for (int i = 0; i < n;) {
    if (s[i] == '1') {
      int j = i;
      while (j < n && s[j] == '1') idx[dep[j++]].pop_back();
      FOR(k, i, j) {
        // REP(j, n) cout << seg[j].sum << " \n"[j + 1 == n];
        ans += seg.get(k, n).sum;
        seg.apply(k, idx[right[k]].empty() ? n : idx[right[k]].back(), -1);
      }
      i = j;
    } else {
      idx[dep[i]].pop_back();
