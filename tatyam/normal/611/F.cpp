...
int main(){
    LL(n,h,w);
    ll x1=0,x2=h,y1=0,y2=w;
    vector<pair<char,ll>>first,second,third;
    STR(s);
    ll cnt=0;
    for(auto p:{&first,&second,&third})each(c,s){
        cnt++;
        if(c=='R'){
            y1++;
            if(y2==w)p->emplace_back(1,exchange(cnt,0));
            else y2++;
        }
        elif(c=='L'){
            y2--;
            if(y1==0)p->emplace_back(1,exchange(cnt,0));
            else y1--;
        }
        elif(c=='U'){
            x1++;
            if(x2==h)p->emplace_back(0,exchange(cnt,0));
            else x2++;
        }
        else{
            x2--;
            if(x1==0)p->emplace_back(0,exchange(cnt,0));
            else x1--;
        }
    }
    Modint ans=0;
