...
map<int, modint<MAX_MOD>> tea;
modint<MAX_MOD> func(int now) {
    if (tea.count(now)) return tea[now];
    modint<MAX_MOD> ans = 1;
    if (now == 1) return 1;
    ans = pows<modint<MAX_MOD>>(2LL, now - 1);
    ans -= 1;
    for (int q = 2; q * q <= now; ++q) {
        if (now % q == 0) {
            ans -= func(q);
            if (q * q != now) ans -= func(now / q);
        }
    }
    return tea[now] = ans;
}
