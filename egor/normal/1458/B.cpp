...
public:
    void solve() {
        ...
        int v = accumulate(all(a), 0);
        arr2d<int> res(n + 1, v + 1, numeric_limits<int>::min() / 2);
        res(0, 0) = 0;
        int vSoFar = 0;
        for (int i : range(n)) {
            for (int j : RevRange(i + 1)) {
                for (int k : range(vSoFar + 1)) {
                    maxim(res(j + 1, k + a[i]), res(j, k) + b[i]);
                }
            }
            vSoFar += a[i];
        }
        ...
        for (int i : range(1, n + 1)) {
            ...
            for (int j : range(v + 1)) {
                if (res(i, j) >= 0) {
                    maxim(answer, min(double(j), (total + res(i, j)) / 2.));
                }
            }
            ans[i - 1] = answer;
        }
...
int main() {
    ...
    auto time = clock();
#endif
    BStakanNapolovinuRazlit solver;
