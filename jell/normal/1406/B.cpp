#line 1 "codeforces-workspace\\contest\\1406\\b\\b.cpp"
#include <bits/extc++.h>
...
#line 14 "codeforces-workspace\\contest\\1406\\b\\b.cpp"
namespace workspace {
constexpr char eol = '\n';
using namespace std;
...
struct workspace::solver {
  solver() {
    ...
    sort(rbegin(posi), rend(posi));
    sort(rbegin(neg), rend(neg));
    i64 ans = -1;
    for (int i = 0, j = 5; i <= size(neg) && j >= 0; i += 2, j -= 2) {
      if (j > size(posi)) continue;
      i64 ans2 = 1;
      for (int k = 0; k < i; k++) {
        ans2 *= neg[k];
      }
