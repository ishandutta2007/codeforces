...
struct SWAG {
   private:
	struct node {
		T val, sum;
		node(const T &_val, const T &_sum) : val(_val), sum(_sum) {}
	};
   ...
   public:
	SWAG(const Op &op, const T _unit) : op(op), unit(_unit), front_stack(), back_stack() {}
	...
	void push(const T &x) {
		if(back_stack.empty())
			...
			back_stack.emplace(x, op(back_stack.top().sum, x));
		return;
	}
...
int main() {
	...
	rep(i, n) {
		SWAG<int> s(mini, 1e9 + 7);
		rep(j, m) {
			s.push(h[i][j]);
			if(s.size() >= b) {
				h[i][j - b + 1] = (s.fold_all(b));
			}
		}
	}
