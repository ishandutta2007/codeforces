...
int main() {
    ...
    sort(p.begin(), p.end());
    {
        ...
        for (int i = 0, j = 0; i < m; i++) {
            if (i > 0 and p[i - 1] == p[i]) continue;
            while (j < n and l[j] < p[i]) j++;
            if (j == n or p[i] != l[j]) np.emplace_back(p[i]);
        }
        swap(p, np);
        m = p.size();
    }

    vector<vector<mint>> ranges(
        m, vector<mint>(m, 1));  // (p_l, p_r) 
    vector<mint> L(m, 1), R(m, 1);
    for (int i = 0; i < m; i++) {
        for (int j = i + 1; j < m; j++) {
            for (int k = 0; k < n; k++) {
                if (p[i] < l[k] and l[k] < p[j]) {
                    ranges[i][j] *= min(l[k] - p[i], p[j] - l[k]);
                }
            }
        }
        for (int k = 0; k < n; k++) {
            if (l[k] < p[i]) L[i] *= p[i] - l[k];
            if (p[i] < l[k]) R[i] *= l[k] - p[i];
        }
    }
    vector<mint> dp(1 << m, 1);  // mask 
    dp[0] = mint(d + 1).pow(n);
    for (int mask = 1; mask < (1 << m); mask++) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            if (pre == -1)
                ...
                dp[mask] *= ranges[pre][i];
            pre = i;
        }
        dp[mask] *= R[pre];
        if (popcount(mask) & 1) dp[mask] *= -1;
    }
    vector<vector<mint>> exc(m, vector<mint>(1 << m, 0));  // i  subset zeta fransform
    for (int i = 0; i < m; i++) {
        for (int mask = 0; mask < (1 << m); mask++) {
            if (mask >> i & 1) {
                exc[i][mask ^ 1 << i] += dp[mask];
            }
        }
        set_function::subset_zeta_transform(exc[i]);
    }

    auto query = [&](int f) -> mint {
        vector<pair<int, int>> v;
        for (int i = 0; i < m; i++) v.emplace_back(abs(f - p[i]), i);
        sort(v.begin(), v.end());
        debug(v);
        int rest = MSK(m);
        mint res = dp[0] * (d + 1);
        for (int i = 0; i < m; i++) {
            int idx = v[i].second;
            rest ^= 1 << idx;
            // if (i > 0 and v[i - 1].first == v[i].first) continue;
            res += exc[idx][rest] * v[i].first;
        }
        return res;
    };
