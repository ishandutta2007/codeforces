...
int main() {
  ...
  int nowlen = n;
  while(nowlen > 1) {
//  	cerr << "nowlen = " << nowlen << endl;
    int target = 2;
    for(int i = 3; i <= nowlen; ++i) if(nowlen / i >= nowlen / target) target = i;
    int c = nowlen / target;
    for(int i = 0; i < nowlen - c; ++i) {
      r[len++] = base;
    }
//    printf("nowlen = %d, target = %d, c = %d\n", nowlen, target, c);
    base *= target;
    nowlen = c;
  }
  r[len++] = base;
  if(len != n) { cout << "qwq" << endl; return 0; }
  for(int i = 0; i < len; ++i) printf("%d%c", r[i], i == len - 1 ? '\n' : ' ');
  return 0;
}