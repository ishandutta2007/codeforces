...
namespace bitwise_convolution {
    template<typename T, bool INV> void transform(vector<T>& v, int N, array<int, 4> t) {
        ...
        if (INV) {
            F0R (i, N) v[i] >>= __builtin_ctz(N);
            if (__builtin_ctz(N)&1) F0R (i, N) v[i] = -v[i];
        }
    }
...
int main() {
    ...
    F0R (j, M) {
        int col = 0;
        F0R (i, N) if (grid[i][j] - '0') col ^= 1 << i;
        a[col]++;
    }

    F0R (i, 1 << N) b[i] = min(__builtin_popcount(i), N - __builtin_popcount(i));

    vll c = conv(a, b, XOR());
    ps(*min_element(all(c)));
