...
signed main()
{
    ...
    for(int i=2;i<=n;i++)
    {
        ans=max(ans, /// change nothing
                dp[i][0]=(a[i]>a[i-1]?dp[i-1][0]+1:1ll));
        ans=max(ans, /// change a[1..i-i]
                dp[i][1]=max({a[i]>a[i-1]?dp[i-1][0]:0ll,
                              a[i]>a[i-1]?dp[i-1][1]:1ll,
                              a[i]>a[i-2]+1?dp[i-1][2]:0ll})+1);
        ans=max(ans, /// change a[i]
                dp[i][2]=dp[i-1][0]+1);
        IFD(cout<<i<<" -> "<<dp[i][0]<<" "<<dp[i][1]<<" "<<dp[i][2]<<"\n";)
    }
    cout<<ans;
