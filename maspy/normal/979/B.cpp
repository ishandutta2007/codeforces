...
void solve() {
  ...
  auto get = [&]() -> ll {
    ...
    for (auto&& s: S) CNT[s]++;
    ll now = MAX(CNT);
    if (now == len(S) && N == 1) return len(S) - 1;
    return min(now + N, len(S));
  };
  ...
  if (b == best) ++cnt;
  if (c == best) ++cnt;
  if (cnt >= 2) return print("Draw");
  if (a == best) return print("Kuro");
  if (b == best) return print("Shiro");
  if (c == best) return print("Katie");
}
