...
int lim[2001010],L,R,f[2001010],F[2001010],FF[2001010],b[2001010],G[2001010],c[2001010];
int xx,yy,TT,t,w,i,j,l,r,n,m;
void GG(){cin>>n>>m;
char S[n+5][m+5];memset(S,0,sizeof(S));
for (i=1;i<=n;i++) cin>>S[i]+1;
for (i=1;i<=n;i++)
  for(j=1;j<=m;j++)
     if ((S[i][j-1]=='X')&&(S[i-1][j]=='X'))
         F[j]++;
    for (i=1;i<=m;i++) FF[i]=FF[i-1]+F[i];
    cin>>TT;
    for (;TT;TT--)
       {
       	cin>>L>>R;
       	if (FF[L]==FF[R]) cout<<"YES"<<endl;
       	else cout<<"NO"<<endl;
	   }}
int main()
{
