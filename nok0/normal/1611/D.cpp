...
void main_() {
	...
	V<> nx(n, -1);
	for(auto [to, cost] : g[root]) nx[to] = root;
	V<ll> dist(n, -1);
	dist[root] = 0;
	ll now = 0;
	V<ll> res(n);
	REP(i, 1, n) {
		...
		now = max(now + 1, dist[nx[p[i]]] + 1);
		res[id[{p[i], nx[p[i]]}]] = now - dist[nx[p[i]]];
		dist[p[i]] = now;
		for(auto [to, cost] : g[p[i]]) nx[to] = p[i];
	}
	print(res);
}