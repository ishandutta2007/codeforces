...
double solve(double x1,double y1,double x2,double y2,double t,double xp,double yp)
{
	if (dist(x1,y1,xp,yp)+dist(xp,yp,x2,y2)-dist(x1,y1,x2,y2)<=t)
		return dist(x1,y1,x2,y2)+t-dist(xp,yp,x2,y2);
	double H=0,T=1;
	for (int step=0;step<300;step++)
	{
		double M=(H+T)/2;
		double x3=x1*(1-M)+xp*M;
		double y3=y1*(1-M)+yp*M;
		if (dist(x1,y1,x3,y3)+dist(x3,y3,x2,y2)-dist(x1,y1,x2,y2)<=t)
			H=M;
		else
			T=M;
	}
	return H*dist(x1,y1,xp,yp);
}
int main()
{
#ifdef _MSC_VER
...
#endif
	double t1,t2,x1,y1,x2,y2,x3,y3;
	scanf("%lf%lf",&t1,&t2);
	scanf("%lf%lf",&x1,&y1);
	scanf("%lf%lf",&x2,&y2);
	scanf("%lf%lf",&x3,&y3);
	double R=0;
	checkmax(R,min(solve(x1,y1,x2,y2,t2,x2,y2),solve(x1,y1,x3,y3,t1,x2,y2)));
	checkmax(R,min(solve(x1,y1,x2,y2,t2,x3,y3),solve(x1,y1,x3,y3,t1,x3,y3)));
	if (dist(x1,y1,x3,y3)+dist(x3,y3,x2,y2)-dist(x1,y1,x2,y2)<=t2)
	{
		double d13=dist(x1,y1,x3,y3);
		checkmax(R,min(dist(x1,y1,x2,y2)+t2,dist(x1,y1,x3,y3)+dist(x3,y3,x2,y2)+t1));
	}
	double H=0,T=1;
	for (int step=0;step<300;step++)
	{
		double M1=(H*2+T)/3;
		double M2=(H+T*2)/3;
		double px1=x2*(1-M1)+x3*M1;
		double py1=y2*(1-M1)+y3*M1;
		double px2=x2*(1-M2)+x3*M2;
		double py2=y2*(1-M2)+y3*M2;
		double v1=min(solve(x1,y1,x2,y2,t2,px1,py1),solve(x1,y1,x3,y3,t1,px1,py1));
		double v2=min(solve(x1,y1,x2,y2,t2,px2,py2),solve(x1,y1,x3,y3,t1,px2,py2));
		checkmax(R,v1);
		checkmax(R,v2);
		if (v1>v2) T=M2;
		else H=M1;
