...
struct HLD {
	...
	HLD(vector <vi> adj_, int root=0) : 
		...
		rt(N,root), pos(N), other(N), result(N, 0),
		tree(vector<S>(N, e())) {dfsSz(root), dfsHld(root);}
	void dfsSz(int v) {
	...
	void dfs(int v) {
		trav(node, other[v]) {
			subtree_apply(node, 1);
		}
		if (!other[v].empty()) subtree_apply(v, 1);
		auto score = tree.all_prod();
		result[v] = N;
		debug(N, score.MIN, score.ILE);
		if (score.MIN == 0) result[v] -= score.ILE;
		trav(u, adj[v]) dfs(u);
...
S op(S a, S b) {
	...
	return S{a.MIN, a.ILE+b.ILE};
};
ll comp(ll a, ll b) {return a+b; }
S mapping(ll fn, S a) {
	return S{a.MIN + fn, a.ILE};
}
ll id() {return 0; }
...
void solve(int tc) {
	...
	HLD <false, S, op, e, ll, mapping, comp, id> solver(graf);
	rep(i, 0, n) solver.tree.set(i, S{0, 1});
	while (zap--) {
		int a, b;
		cin >> a >> b;
		--a; --b;
