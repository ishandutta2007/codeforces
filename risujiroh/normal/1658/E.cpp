...
void main(int) {
  ...
  scan(a);
  vector<array<int, 2>> order(n * n);
  for (int i : rep(n)) {
    for (int j : rep(n)) { order[n * n - a[i][j]] = {i, j}; }
  }
  vector ans(n, string(n, 'G'));
  basic_string<array<int, 2>> s;
  int mn0 = inf, mx0 = -inf;
  int mn1 = inf, mx1 = -inf;
  for (auto [i, j] : order) {
    bool ok = true;
    for (auto [x, y] : s) {
      if (k < abs(i - x) + abs(j - y)) {
        ok = false;
        break;
      }
    }
    if (!ok) { continue; }
    ans[i][j] = 'M';
    if (chmin(mn0, i + j)) { s += {i, j}; }
    if (chmax(mx0, i + j)) { s += {i, j}; }
    if (chmin(mn1, i - j)) { s += {i, j}; }
    if (chmax(mx1, i - j)) { s += {i, j}; }
  }
  println<'\n'>(ans);
}
