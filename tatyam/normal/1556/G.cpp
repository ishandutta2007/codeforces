...
int main(){
    ...
    rep(n)for(auto p=x_.begin();p!=x_.end();p++){
        if(*p&1LL<<i&&*p%(1LL<<i)==0){
            x_.insert(*p-(1LL<<i));
            x_.insert(*p+(1LL<<i));
        }
    }
    ...
    auto find=[&](ll i){
        return --upper_bound(all(x),i)-x.begin();
    };
    rep(N)rep(j,n){
        const ll k=find(x[i]^(1LL<<j));
        if(i!=k&&(g[i].empty()||k!=g[i].back())){
            g[i].push_back(k);
            g[k].push_back(i);
        }
    }
    ...
    each(a,l,r,query){
        if(a){
            ll i=find(l);
            while(x[i]<r)alive[i++]=0;
        }
    }
    rep(N)if(alive[i])each(j,g[i])if(alive[j])uf.unite(i,j);
    Rev(query);
    each(a,l,r,query){
        if(a){
            ll i=find(l);
            while(x[i]<r){
                alive[i]=1;
                each(j,g[i])if(alive[j])uf.unite(i,j);
                i++;
            }
        }
        else{
            ans.push_back(uf.find(find(l),find(r)));
        }
    }
    Rev(ans);
    out(ans);
