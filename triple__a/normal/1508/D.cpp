...
signed main(){
    ...
    sort(range(ls),[&](int u,int v){
        pii s=make_pair(x[u]-x[rt],y[u]-y[rt]), t=make_pair(x[v]-x[rt],y[v]-y[rt]);
        bool bh1=s<make_pair(0ll,0ll);
        bool bh2=t<make_pair(0ll,0ll);
    ...
    int m=sz(ls);
    rep(i,m){
        int u=ls[i], v=ls[(i+1)%m];
        pii s=make_pair(x[u]-x[rt],y[u]-y[rt]), t=make_pair(x[v]-x[rt],y[v]-y[rt]);
        if (cp(s,t)<0) continue;
        if (dsu.unite(u,v)){
            res.pb({u,v});
            swap(w[u],w[v]);
        }
