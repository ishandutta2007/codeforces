...
const int base = 29;
Hash bpw[maxn], inv_bpw_minus_1[maxn];
void precal() {
    bpw[0] = 1;
    rep(i, 1, maxn) {
        bpw[i] = bpw[i - 1] * base;
    ...
    rep(i, 0, maxn) {
        inv_bpw_minus_1[i] = 1 / (bpw[i] - 1);
    }
}
    
Hash repeat(const Hash& cur_base, int times, int pad_len) {
    return cur_base * inv_bpw_minus_1[pad_len] * (bpw[pad_len * times] - 1);
}
...
int main() {
    ...
    auto get_rep = [&](int base_len, int final_len) {
        ...
        auto ans = hs.get(0, base_len);
        ans = repeat(ans, final_len / base_len, base_len);
