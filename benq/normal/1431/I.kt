...
fun solve() {
    val (n,m,q) = io.ints(3)
    val g = Array<String>(n){io.read()}
//    var cols =
    val rows = Array<ArrayList<Int>>(m+1){ArrayList<Int>()}
    for (i in 0..n-1) rows[m].add(i)
    for (j in m-1 downTo 0) {
        val todo = Array<ArrayList<Int>>(26){ArrayList<Int>()}
        for (c in rows[j+1]) {
            todo[g[c][j]-'a'].add(c)
        }
        for (k in 0..25) {
...
//    println(rows)
    for (i in 1..q) {
        ...
        var ind = 0
        var ans = 0
        fun get_end(row: Int): Int {
            var IND = ind
            while (IND < m && g[row][IND] == s[IND]) {
                IND ++
            }
            return IND
        }
        fun lcp(): Int {
            ...
            while (lo < hi) {
                val mid = (lo+hi)/2
                val IND = get_end(rows[ind][mid])
                if (IND < m && g[rows[ind][mid]][IND] > s[IND]) {
                    hi = mid
                } else {
                    lo = mid+1
                }
            ...
            var res = -1
            if (lo > 0) res = maxOf(res,get_end(rows[ind][lo-1]))
            if (lo < n) res = maxOf(res,get_end(rows[ind][lo]))
            return res
        }
        while (ind < m) {
            val IND = lcp()
