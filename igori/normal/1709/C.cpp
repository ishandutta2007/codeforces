...
void solve()
{
    ...
    vector<int> go_substract(s.size() + 1);
    for (int i = s.size() - 1; i >= 0; i--)
    {
        go_substract[i] = go_substract[i + 1] + 1;
        if (s[i] == ')') go_substract[i] -= 2;
    }
    int mx = 0;
    for (int i = s.size() - 1; i >= 0; i--)
    {
        mx = max(mx, go_substract[i]);
        go_substract[i] = go_substract[i] - mx;
    }
    for (int i = 0; i < s.size(); i++) closing += s[i] == ')';
    for (int i = 0; i < s.size(); i++) op += s[i] == '(';
    for (int i = 0; i < s.size(); i++)
    {
        if (s[i] == '(') x++;
        if (s[i] == ')') x--;
        if (s[i] == '?' && x + go_substract[i + 1] > 0 && closing < s.size() / 2) f[i] = ')', x--, closing++;
        else if (s[i] == '?') f[i] = '(', x++;
    }
    for (int i = 0; i < s.size(); i++)
    {
