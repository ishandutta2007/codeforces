...
int main()
{
...
#endif
	scanf("%d%d%d%d%d",&s1,&t1,&s2,&t2,&k);
	n=0;
	for (int L=1;L<=9;L++) for (int set=0;set<two(L);set++)
	{
		int d=0;
		for (int i=0;i<L;i++) d=d*10+(contain(set,i)?4:7);
		a[n++]=d;
	}
	sort(a,a+n);
	set<int> M;
	...
	M.insert(s2);
	M.insert(t2);
	for (int i=0;i<n;i++) if (a[i]>=s1 && a[i]<=t1 || a[i]>=s2 && a[i]<=t2) M.insert(a[i]);
	vector<int> G(M.begin(),M.end());
	vector<ipair> g;
	for (int i=0;i<SIZE(G);i++)
	{
		g.push_back(MP(G[i],G[i]));
		if (i+1<SIZE(G) && G[i]+1<=G[i+1]-1)
			g.push_back(MP(G[i]+1,G[i+1]-1));
	}
	double R1=0,R2=(double)(t1-s1+1)*(t2-s2+1);
	for (int i=0;i<SIZE(g);i++)
	{
		int g0=(g[i].first+g[i].second)/2;
		h1[i]=h2[i]=0;
		for (int j=0;j<n;j++)
		{
			if (a[j]<g0) h1[i]++;
	...
	for (int p1=0;p1<SIZE(g);p1++) for (int p2=0;p2<SIZE(g);p2++)
	{
		int g1=(g[p1].first+g[p1].second)/2;
		int g2=(g[p2].first+g[p2].second)/2;
		if (g1<s1 || g1>t1 || g2<s2 || g2>t2) continue;
		int cnt=h2[max(p1,p2)]-h1[min(p1,p2)];
		if (cnt!=k) continue;
		R1+=(double)(g[p1].second-g[p1].first+1)*(g[p2].second-g[p2].first+1);
	}
	printf("%.18lf\n",R1/R2+1e-13);
	return 0;
}