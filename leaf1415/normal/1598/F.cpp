...
int main(void)
{
	...
	rep(i, 0, n-1){
		...
		for(auto c : s[i]){
			...
			if(mp.begin()->first >= h) mp[h]++;
		}
		sum[1<<i] = h, m[i] = mp.begin()->first;
		rep(j, 0, sz(s[i])) cnt[i][j] = mp[j+m[i]];
	}
	...
	ll N = 1<<n;
	rep(i, 0, N-1) rep(j, 0, 1) dp[i][j] = -inf; dp[0][0] = 0;
	rep(i, 0, N-1){
		rep(j, 0, n-1){
			if(i & (1<<j)) continue;
			chmax(dp[i|(1<<j)][1], dp[i][1]);
			ll nk = 0, cost = 0;
			if(sum[i] + m[j] < 0) nk = 1;
			if(-sum[i] - m[j] >= 0 && -sum[i] - m[j] <= sz(s[j])) cost = cnt[j][-sum[i]-m[j]];
			chmax(dp[i|(1<<j)][nk], dp[i][0] + cost);
		}
	}
	outl(max(dp[N-1][0], dp[N-1][1]));
