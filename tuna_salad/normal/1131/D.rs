...
fn dfs(u: usize, e: &[Vec<usize>], vis: &mut [u8], dp: &mut [usize]) -> bool {
	vis[u] = 1;
	dp[u] = 1;
	for &v in e[u].iter() {
		if vis[v] == 1 || (vis[v] == 0 && !dfs(v,e,vis,dp)) {
			return false;
		}
		max!(dp[u], dp[v] + 1);
	}
...
fn main() {
	...
	l!(r,c = rin.u());
	let b = rin.board(r, Some(c));
	let mut g = DSU::new(r+c);
	for i in 0..r {
		for j in 0..c {
			if b[i][j] == b'=' {
				g.union(i, j+r);
	...
	if valid {
		let mut vis = v!([r+c] = 0);
		for i in 0..r+c {
			if g.find(i) != i || vis[i] > 0 { continue; }
			if !dfs(i, &e, &mut vis, &mut dp) {
				valid = false;
				break;
			}
		}
	...
	if valid {
		writeln!(rout, "Yes").ok();
		for i in 0..r {
			write!(rout, "{} ", dp[g.find(i)]).ok();
		}
		writeln!(rout, "").ok();
		for i in 0..c {
			write!(rout, "{} ", dp[g.find(i + r)]).ok();
		}
		writeln!(rout, "").ok();
	} else {
		writeln!(rout, "No").ok();
