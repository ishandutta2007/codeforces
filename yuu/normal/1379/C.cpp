...
void solve() {
    ...
    suffix[m + 2] = 0;
    suffix[m + 1] = 0;
    for (int i = m; i >= 1; i--) suffix[i] = suffix[i + 1] + f[i].first;
    int64_t ans = 0;
    if (n <= m) ans = suffix[m - n + 1];
    for (int i = 1; i <= m; i++) {
        int low = 1, high = m, mid, res = m + 1;
        while (low <= high) {
            mid = (low + high) / 2;
            if (f[mid].first >= f[i].second) {
                res = mid;
                high = mid - 1;
            } else {
                low = mid + 1;
        ...
        int64_t total = 0;
        if (m - res + 1 > n) res = m - n + 1;
        int64_t remain = n - (m - res + 1);
        if (res > i) {
            total = suffix[res] + f[i].first;
            remain--;
        } else {
            total = suffix[res];
        }
        if (remain < 0) continue;
        ans = max(ans, total + remain * f[i].second);
    }
    cout << ans << '\n';
}
