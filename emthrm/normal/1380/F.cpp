...
int main() {
  ...
  st.emplace(n);
  vector dp(n, ModInt(0));
  dp[0] = 1;
  FOR(i, 1, n) {
    dp[i] = dp[i - 1] * 2;
    if (i >= 2) dp[i] += dp[i - 2] * 8;
  }
  auto f = [&](int idx) -> ModInt {
    if (idx == n) return 1;
    auto it = prev(st.upper_bound(idx));
    int l = *it, r = *next(it) - 1, len = r - l + 1;
    ModInt res = dp[len - 1] * (c[r] - '0' + 1);
    if (len > 1) res += dp[len - 2] * (9 - (c[r] - '0'));
    return res;
  };
  ModInt ans = 1;
  for (auto it = st.begin(); *it < n; ++it) ans *= f(*it);
  while (m--) {
