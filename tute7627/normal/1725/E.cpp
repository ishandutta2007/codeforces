...
int main(){
  ...
  rep(i,0,sz){
    ...
    vector<int>st=v[i];
    sort(ALL(st));
    auto g=hld.lca_tree(v[i]);
    auto dfs=[&](auto &&f,int k,int par)->ll{
      ll sum=binary_search(ALL(st),v[i][k]);
      for(auto e:g[k]){
        if(e.to==par)continue;
        ll sub=f(f,e.to,k);
        sum+=sub;
        ret+=modint(st.size()-2)*sub*(st.size()-sub)*e.cost;
      }      
      return sum;
    };
    dfs(dfs,0,-1);
