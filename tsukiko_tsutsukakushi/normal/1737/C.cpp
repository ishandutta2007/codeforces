...
void solve() {
    INT(n);
    vvc<int> check(2, vc<int>(2, 0));
    auto f = [&](int x, int y) -> bool {
        if(x == 0 and y == 0) return true;
        if(x == 0 and y == n - 1) return true;
        if(x == n - 1 and y == 0) return true;
    ...
    int mr = -1, mc = -1;
    for(auto [val, co]: rcnt) {
        if(co == 2) mr = val;
    }
    for(auto [val, co]: ccnt) {
        if(co == 2) mc = val;
    }
    debug(mr, mc);
    if(f(mr, mc)) hasi = true;
    INT(x, y); -- x, -- y;
    if(hasi) {
        if(x == mr or y == mc) out("YES");
        else out("NO");
        return;
    }
    debug(x % 2, y % 2);
