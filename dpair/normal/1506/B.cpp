...
int n, k, tot, a[MAXN]; char s[MAXN];
inline void work(){
    read(n, k); read(s + 1); tot = 0;
    rep(i, 1, n) if(s[i] == '*') a[++ tot] = i;
    if(tot <= 2) return print(tot), void();
    int ans = 0; for (int l = 1, r = 2;l < tot;l = r){
        while(r <= tot && a[r] - a[l] <= k) ++ r;
        ++ ans; -- r;
    } print(ans + 1);
}
