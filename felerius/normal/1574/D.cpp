// The Strongest Build (https://codeforces.com/contest/1574/problem/D)
...
int main() {
    ...
    vector<map<int, int>> trie_adj(1);
    int m; read(m);
    rep(_, m) {
        int v = 0;
        rep(__, n) {
            int b; read(b); --b;
            auto [it, ins] = trie_adj[v].emplace(b, sz(trie_adj));
            if (ins)
                trie_adj.emplace_back();
            v = it->second;
        }
    }

    auto dfs = [&](int v, int d, int sum, vector<int> build, auto&& self) -> pair<int, vector<int>> {
        auto best = pair(0, vector<int>());
        if (d == n) return best;

        int x = sz(c[d]) - 1;
        auto it = rbegin(trie_adj[v]);
        while (it != rend(trie_adj[v]) && it->first == x)
            --x, ++it;
        ...
        if (x >= 0) {
            ...
            build2.push_back(x);
            rep(i, d + 1, n)
                sum2 += c[i].back(), build2.push_back(sz(c[i]) - 1);
            best = pair(sum2, build2);
        }

        it = rbegin(trie_adj[v]);
        for (it = rbegin(trie_adj[v]); it != rend(trie_adj[v]) && it->first > x; ++it) {
            auto build2 = build;
            build2.push_back(it->first);
            best = max(best, self(it->second, d + 1, sum + c[d][it->first], build2, self));
        }

        return best;
    };

    auto indices = dfs(0, 0, 0, vector<int>(), dfs).second;
    trav(i, indices) ++i;
    println(indices);
}