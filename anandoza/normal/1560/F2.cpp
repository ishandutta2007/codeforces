...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (char v = s[i] + 1; v <= '9'; v++) {
            ...
            set<char> distinct{cand.begin(), cand.begin() + i + 1};
            if (sz(distinct) > k)
                continue;

            if (sz(distinct) < k)
                fill(cand.begin() + i + 1, cand.end(), '0');
            else
                fill(cand.begin() + i + 1, cand.end(), *distinct.begin());

            ckmin(ans, cand);
        }
    }
    if (sz(set<char>{all(s)}) <= k)
        ckmin(ans, s);
