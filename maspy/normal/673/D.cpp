...
void solve() {
  ...
  vi other;
  FOR3(x, 1, N + 1) {
    if (x != a && x != b && x != c && x != d) other.eb(x);
  }

  bool sab = false, scd = false;
  auto out = [&](vi A, vi B) -> void {
    set<pi> ss;
    FOR(i, N - 1) {
      ll a = A[i], b = A[i + 1];
      ss.insert(mp(a, b));
    ...
    if (len(ss) / 2 > K) return print(-1);

    if (sab) reverse(all(A));
    if (scd) reverse(all(B));
    print(A);
    print(B);
  };
  ...
  if (a != c && a != d && b != c && b != d) {
    ...
    vi A = {a, c};
    vi B = {c, a};
    A.insert(A.end(), all(other));
    B.insert(B.end(), all(other));
    A.eb(d);
    A.eb(b);
    B.eb(b);
    B.eb(d);
