...
void gen(){
  ...
  assert(RAND_MAX>lim*2);
  forn(i,n){
    LL x=rand()%(lim*2+1)-lim,y=rand()%(lim*2+1)-lim;
    LL l=x*x+y*y;
    if(l>lim*lim || (l && sqrt(l*1.)<lim*3/4.)){
      ld c=lim/sqrt(l*1.)*.999;
      x*=c;
      y*=c;
    }
    assert(x*x+y*y<=lim*lim);
    cout<<x<<' '<<y<<'\n';
  }
}
...
int main(int argc, char **argv){
  ...
  vector<pll> A(n);
  vector<pii> C[2];
  const LL lim=sqr(1000000ll);
  auto small=[&](pll v){
    return sqr(v.X)+sqr(v.Y)<=lim;
  };
  auto add=[&](pll a,pll b,int s){
    return mp(a.X+b.X*s,a.Y+b.Y*s);
  };
  auto merge=[&]{
    if(sz(C[0])<sz(C[1])){
      swap(C[0],C[1]);
    ...
    pll mn(LINF,-1);
    for(int s=-1;s<=1;s+=2) mn=min(mn,mp(sqr(V[0].X+V[1].X*s)+sqr(V[0].Y+V[1].Y*s),(LL)s));
    int s=mn.Y;
    for(auto&p:C[1]) p.Y*=s;
  ...
  assert(v==V[0]);
  assert(v.X*v.X+v.Y*v.Y<=sqr(1500000ll));
