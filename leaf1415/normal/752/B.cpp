...
int main(void)
{
	...
	cin >> s >> t;
	reps(i, s){
		if(s[i] != t[i]) uf.unite(s[i]-'a', t[i]-'a');
		else fix[s[i]-'a'] = true;
	}
	rep(i, 0, 25) vec[uf.root(i)].push_back(i);
	
	rep(i, 0, 25){
		if(fix[i] && uf.v[uf.root(i)] >= 2){
			outl(-1);
			return 0;
		}
	}
	...
	rep(i, 0, 25){
		...
		if(sz(vec[i]) == 2) ans.push_back(P(vec[i][0], vec[i][1]));
	}
	
	outl(sz(ans));
	for(auto p : ans) outl((char)(p.first+'a'), (char)(p.second+'a'));
