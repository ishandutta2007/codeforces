...
int main(int argc, char** argv) {
    ...
    //         ps.emplace_back(a[i].second, -1);
    //         ps.emplace_back(a[i - 1].second, 1);
    //     }
    // }

    for (int i = 1; i < n; i++) {
        if (pa[a[i - 1].second] > pa[a[i].second]) {
            ps.emplace_back(pa[a[i].second], -1);
            ps.emplace_back(pa[a[i - 1].second], 1);
        }
    }
    ...
    if (*max_element(begin(b), end(b)) < k - 1) {
        cout << "NO\n";
        return 0;
    }
