...
int main(void)
{
	...
	cin >> n >> a[0] >> x >> y >> k >> M;
	rep(i, 1, n-1) a[i] = (a[i-1]*x%M+y)%M;
	
	//outa(a, 0, n-1);
	
	mint ans = 0;
	rep(i, 0, n-1) ans += a[i]/B*B, a[i] %= B;
	ans *= mint(k) / mint(n);
	
	mint p = mint(n).inverse();
	per(i, k, 1){
		rep(j, 0, B-1){
			dp[i&1][j] = dp[(i+1)&1][j] * (mint(1)-p);
			dp[i&1][j] += (dp[(i+1)&1][j/i*i] + j) * p;
		}
	}
	
	rep(i, 0, n-1) ans += dp[1][a[i]];
	ans *= modpow(n, k);
	outl(ans);
