...
#undef COMPARE
  ...
  bool read(InputDevice& input) {
    if (!input.read(value)) return false;
...
class AhoCorasick {
  struct Node {
    Node(int length = 0, int link = 0) : length(length), link(link) {}

    int length, link;  // length > 0 for termination nodes
    Transition edges;
  };

  vector<Node> nodes;
  vector<int> terminationLink;

public:
  AhoCorasick(int size = 0) { init(size); }
  ...
  int addString(const C* s, int length) {
    int v = 0;
    for (; length --> 0; ++s) {
      if (nodes[v].edges[*s] == 0) {
        nodes[v].edges[*s] = size();
        nodes.emplace_back(-abs(nodes[v].length) - 1);
      }
      v = nodes[v].edges[*s];
    }
    nodes[v].length = abs(nodes[v].length);
    return v;
  }

  vector<int> build(bool prepareSearch = false) {
    if (prepareSearch) terminationLink.resize(size());
    vector<int> queue = {0};
    queue.reserve(size());
    for (size_t i = 0; i < queue.size(); ++i) {
      int v = queue[i], link = nodes[v].link;
      if (prepareSearch) terminationLink[v] =
        nodes[link].length > 0 ? link : terminationLink[link];
      else smax(nodes[v].length, nodes[link].length);
      for (auto&& edge : nodes[v].edges) {
        C c = edge.first;
        int w = link;
        while (w != 0 && nodes[w].edges[c] == 0) w = nodes[w].link;
        if (v > 0) nodes[edge.second].link = nodes[w].edges[c];
  ...
  void buildFull(const C& alphabetSize) {
    auto queue = build();
    for (int v : queue) {
      for (C c{}; c < alphabetSize; ++c) if (nodes[v].edges[c] == 0)
        nodes[v].edges[c] = nodes[nodes[v].link].edges[c];
    }
  }

  template <class Visitor>
  void search(const C* text, int length, Visitor visitor/*(start, node)*/) {
    for (int v = 0, i = 0; i < length; ++i, ++text) {
      while (v != 0 && nodes[v].edges[*text] == 0) v = nodes[v].link;
      v = nodes[v].edges[*text];
      for (int u = v; u != 0; u = terminationLink[u]) if (nodes[u].length > 0)
        visitor(i + 1 - nodes[u].length, u);
    }
  }
};

const int N = 1001, D = 51, V = N*D/2, A = 10;
char s[N], x[D], y[D];
AhoCorasick<char, Array<int, A>> ac;
Mi dp[D][V][8];
...
int main() {
  ...
  ac.init(V);
  REP(i, n) s[i] -= '0';
  REP(i, n-d2+1) ac.addString(s+i, d2);
  ac.buildFull(A);
  dp[0][0][3] = 1;
  REP(i, d) REP(v, ac.size()) REP(m, 8) if (dp[i][v][m]) REP(l, A) {
    int mm = m & 4;
    if (m&1) {
      if (x[i] > l) continue;
      if (x[i] == l) mm |= 1;
    ...
    int u = ac[v].edges[l];
    if (ac[u].length > 0) mm |= 4;
    dp[i+1][u][mm] += dp[i][v][m];
  }
  Mi ans;
  REP(v, ac.size()) REP(m, 4) ans += dp[d][v][m|4];
  writeln(ans);
