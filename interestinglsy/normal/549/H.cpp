...
void Lowupp( ld a , ld b , ld lim , ld &low , ld &upp ){
	vc<ld> v;
	v.Pb( (a-lim) * (b-lim) );
	v.Pb( (a+lim) * (b-lim) );
	v.Pb( (a-lim) * (b+lim) );
...
bool Check( ld lim ){
	ld xlow, xupp;
	ld ylow, yupp;
	Lowupp(a,d,lim,xlow,xupp);
	Lowupp(b,c,lim,ylow,yupp);
	//cout << xlow << " " << xupp << endl;
	//cout << ylow << " " << yupp << endl;
	ld mlow = max(xlow,ylow);
	ld mupp = min(xupp,yupp);
	return mlow <= (mupp);
}
// XXXXOOOOO
ld Binarysearch(){
	ld l = 0, r = 1000000000000, mid, ans;
	while( r-l >= Eps ){
		mid = (l+r)/2.0;
		if( Check(mid) ){
			ans = mid;
