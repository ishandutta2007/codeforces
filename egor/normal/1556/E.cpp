...
public:
    void solve() {
        ...
        ReadOnlySegmentTree<ll, numeric_limits<ll>::max()> minTree(s, [](ll a, ll b) -> ll {
            return min(a, b);
        });
        ReadOnlySegmentTree<ll, numeric_limits<ll>::min()> maxTree(s, [](ll a, ll b) -> ll {
            return max(a, b);
        });
        ...
        for (int _ : range(q)) {
            int l = in.readInt() - 1;
            int r = in.readInt();
            if (s[l] == s[r] && minTree.query(l, r + 1) == s[l]) {
                out.printLine(maxTree.query(l, r + 1) - s[l]);
            } else {
                out.printLine(-1);
            }
        }
