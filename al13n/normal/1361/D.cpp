...
int rand30(){static_assert(RAND_MAX>=(1<<15)-1); return (int)((((uint32_t)rand())*(1u+(uint32_t)RAND_MAX)+(uint32_t)rand())&((1u<<30)-1));}
...
int main(int argc, char **argv){
  ...
  int n,K;
  cin>>n>>K;
  vector<pair<pii,ld>> A;
  forn(i,n){
    int x,y;
    cin>>x>>y;
    if(!x&&!y) continue;
    int g=gcd(abs(x),abs(y));
    ld d=sqrt(1.*x*x+1.*y*y);
    A.pb(mp(mp(x/g,y/g),d));
  }
  assert(sz(A)==n-1);
  sort(all(A));
  int neg=0;
  forn(qq,2){
    ...
    for(int i=0;i<sz(A);){
      T.clear();
      int ii=i;
      while(ii<sz(A) && A[ii].X==A[i].X){
        T.pb(A[ii].Y);
        ++ii;
      }
      sort(all(T));
      reverse(all(T));
      int rm=sz(T)-1-neg-(K-1)/2;
      forv(q,T){
        assert((K-1-q*2>=0)==(q<=(K-1)/2));
        if(!qq || K-1-q*2>=0) B.pb(T[q]*(K-1-q*2));
        else if(sz(T)-1-q<neg && rm>=0) B.pb(T[q]*(K-1-(q-rm)*2));
      }
      dbg(T,rm,neg);
      i=ii;
    }
    ...
    assert(sz(B)>=K);
    sort(all(B));
    if(qq) cout<<accumulate(B.end()-K,B.end(),0.);
    else forn(i,K) neg+=B[sz(B)-1-i]<-1e-10;
  }
