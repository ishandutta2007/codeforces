...
void solve() {
    ...
    REP(i, m) {
        tenti.push_back({});
        REP(q, n) {
            tenti[i].push_back(inputs[q][i]);
        }
    }
    int ans = 0;
    REP(tea, 2) {
        vector<int> val(inputs[0].size(), 0LL);
        REP(i, inputs.size()) {
            REP(q, inputs[i].size()) {
                val[q] += inputs[i][q];
            }
        ...
        REP(i, inputs.size()) {
            ...
            REP(q, inputs[i].size()) {
                now += inputs[i][q];
                hoge.push_back(val[q] - inputs[i][q]);
            }
            sort(ALL(hoge));
            reverse(ALL(hoge));
            REP(q, 3) {
    ...
    {
        if (inputs.size() > tenti.size()) swap(inputs, tenti);
        vector<int> val(inputs[0].size(), 0LL);
        REP(i, inputs.size()) {
            REP(q, inputs[i].size()) {
                val[q] += inputs[i][q];
        ...
        REP(i, inputs.size()) {
            for (int q = i + 1; q < inputs.size(); ++q) {
                pair<int, int> hoge = mp(-1, -1);
                int now = 0;
                REP(j, inputs[i].size()) {
                    now += inputs[i][j] + inputs[q][j];
                    int value = val[j] - inputs[i][j] - inputs[q][j];
                    if (hoge.first < value) {
                        hoge.second = hoge.first;
                        hoge.first = value;
                    }
                    else if(hoge.second < value) {
                        hoge.second = value;
                    }
                }
                now += hoge.first + hoge.second;
                ans = max(ans, now);
            }
        }
    }
