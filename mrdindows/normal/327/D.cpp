...
char a[501][501];
bool used[501][501];
vector<triple<char,int,int>> ans;
    int n,m;
void go(int x,int y, bool first = false)
{
//  DB(x);DBN(y);
    used[x][y] = true;
    ans.push_back(triple<char,int,int>('B',x+1,y+1));
    int dx,dy;
    ...
    {
        if (!used[dx][dy] && a[dx][dy]) go(dx,dy);
    }
    dx = x - 1;
    dy = y;
    if (dx>=0 && dx<n && dy>=0 && dy<m)
    ...
    if (!first)
    {
        ans.push_back(triple<char,int,int>('D',x+1,y+1));
        ans.push_back(triple<char,int,int>('R',x+1,y+1));
    }
}
int main()
{
