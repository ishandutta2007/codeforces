...
const int N=200005,M=100005,mod=1e9+9;
...
int main(){
	...
	int inv=exp(a,mod-2);
	int pwa=1,pwb=1,ans=0;
	for(int i=0;i<k;i++){
		ch(ans,(ll)(s[i]=='+'?1:mod-1)*pwa%mod*pwb);
		pwa=(ll)pwa*inv%mod;pwb=(ll)pwb*b%mod;
	}
	int pw=(ll)pwa*pwb%mod,m=(n+1)/k;
	if(pw==1) ans=(ll)ans*m%mod;
	else ans=(ll)ans*(exp(pw,m)-1)%mod*exp(pw-1,mod-2)%mod;
	ans=(ll)ans*exp(a,n)%mod;
	printf("%d\n",ans);
	return 0;
}