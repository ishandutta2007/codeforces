...
int main() {
    ...
    for (int i = 0; i < N; i++)
        if (B[i] < 0)
            graph.add_directional_edge(source, i, -B[i]);
        else
            graph.add_directional_edge(i, sink, B[i]);

    vector<int> last(A_MAX, -1);

    for (int i = 0; i < N; i++) {
        for (int x = 1; x <= A[i]; x++)
            if (A[i] % x == 0 && last[x] >= 0)
                graph.add_directional_edge(last[x], i, INF);

        last[A[i]] = i;
    }

    answer -= graph.flow(source, sink);
    cout << answer << '\n';
}