...
void solve(){
    ...
    REP(i, s.length()) {
        if (s[i] == '(') {
            ...
            now.push_back(')');
            for (int q = 1; q <= i; ++q) {
                if (now.substr(q, now.length()-q) == s.substr(0, i - q + 1)) {
                    nexts[i][1] = i - q + 1;
                    break;
                }
            }
    ...
    REP(i, n) {
        REP(q, s.length()) {
            REP(j, i + 1) {
                //plus one
                dp[i + 1][nexts[q][0]][j + 1] += dp[i][q][j];
                //minus one
                if (j > 0) {
                    dp[i + 1][nexts[q][1]][j - 1] += dp[i][q][j];
                }
            }
        }
        REP(j, i + 1) {
            dp[i + 1][s.length()][j + 1] += dp[i][s.length()][j];
            if (j != 0) {
                dp[i + 1][s.length()][j - 1] += dp[i][s.length()][j];
            }
        }
