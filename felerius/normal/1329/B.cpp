...
// Problem: B: Dreamoon Likes Sequences (https://codeforces.com/contest/1329/problem/B)
...
struct VarMod { static int _m; static int m() { return _m; }};
int VarMod::_m = 1;
using MI = Modular<VarMod>;
using vMI = vector<MI>;
...
int main() {
    ...
    int t; cin >> t;
    while (t--) {
        int d; cin >> d >> VarMod::_m;
        int h = sizeof(int) * 8 - __builtin_clz(d);
        MI st = 1;
        reps(i, 1, h)
            st += (i == h - 1 ? (d - (1 << i) + 1) : (1 << i)) * (st + 1);
        cout << st << '\n';
    }
