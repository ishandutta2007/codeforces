...
fun main(){
    solve.cases{
        ...
        val retlong = LongArray(1 shl n){1}
        for(c in 0 until 26){
            val now = types[c]
            val min = IntArray(1 shl n){plarge}
            for(m in 1 until (1 shl n)) {
                val pivot = m.countTrailingZeroBits()
                min[m] = minOf(now[pivot], min[m xor ( 1 shl pivot)])
                retlong[m] = (retlong[m] * (min[m] + 1)) % pI
            }
        }
        val ret = IntArray(1 shl n ){retlong[it].toInt()}
        ret[0] = 0
        for(m in 0 until (1 shl n)) {
            if(m.countOneBits() and 1 == 0) {
                ret[m] = ret[m].additiveInverse()
            }
        }
        for(j in 0 until n){
            for(m in 0 until (1 shl n)) {
                if(m.has(j)) {
                    ret[m] = ret[m] modPlus (ret[m xor ( 1 shl j )])
                }
            }
        }
//        just dei ret
        ...
        for(m in 0 until (1 shl n)){
            ...
            res = res xor  ((ret[m]).toLong() * rsum * rcount)
        }
        put(res)
