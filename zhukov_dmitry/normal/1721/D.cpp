...
int main()
{
	...
	{
		scanf("%d", &n);
		vector<array<vector<int>, 2>> a(1), an;
		forn(j, 2)
		{
			a[0][j].resize(n);
			forn(i, n)
		...
		ford(h, 30)
		{
			...
			if (ok)
			{
				...
				forn(t, a.size())
				{
					...
					forn(i, l)
					{
						if (a[t][0][i] & bit) d[0][0].pb(a[t][0][i]);
						else d[1][0].pb(a[t][0][i]);
						if (a[t][1][i] & bit) d[1][1].pb(a[t][1][i]);
						else d[0][1].pb(a[t][1][i]);
					}
					forn(i, 2)
					{
						assert(d[i][0].size() == d[i][1].size());
						if (d[i][0].empty()) continue;
						an.pb(move(d[i]));
					}
				}
				swap(a, an);
