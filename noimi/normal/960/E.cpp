...
int main() {
    ...
    mint ans;
    REC([&](auto &&f, int x, int p, int c) -> void {
        cnt[c]++;
        for(auto e : g[x]) {
            if(e != p) f(e, x, c ^ 1);
        }
    ...
    auto merge = [&](pii x, pii y, int c, mint t) {
        mint r;
        r += mint(1) * (x.fi - x.se) * (y.fi + y.se);
        r += mint(1) * (y.fi - y.se) * (x.fi + x.se);
        if(c) r = -r;
        ans += t * r;
        return x + y;
    };
    REC([&](auto &&f, int x, int p, int c) -> pii {
        pii t;
        if(c)
            t.se++;
        else
        ...
        for(auto e : g[x]) {
            if(e != p) { t = merge(t, f(e, x, c ^ 1), c, a[x]); }
        }
        pii res(cnt[0] - t.fi, cnt[1] - t.se);
        merge(t, res, c, a[x]);
        ans += a[x];
        return t;
    })
