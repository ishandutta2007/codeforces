...
const int maxn = 2e3 + 10, maxw = 1e7 + 10, inf = 1e9 + 100, LG = 17, SQ = 500;
...
inline bool allow(int x, char c) {
    return c != 48 + (x ^ 1);
}
...
void solve() {
    ...
    for (int i = 0; i <= n; i++)
        for (int j = 0; j <= n; j++)
            for (int k = 0; k < 2; k++)
                can[i][j][k] = 0;
    can[n][0][0] = can[n][0][1] = 1;
    for (int i = n - 1; i >= 0; i--)
        for (int b = 0; b <= n - i; b++)
            for (int prv = 0; prv < 2; prv++) {
                for (int x = 0; x < 2; x++)
                    for (int y = 0; y < 2; y++)
                        if (allow(x, A[i]) && allow(y, B[i])) {
                            static int nb = b, nx = x, ny = y;
                            if (i > 0 && nb > 0 && prv == x) {
                                nb--;
                                nx = !nx;
                            }
                    ...
                    for (int x = 0; x < 2; x++)
                        for (int y = 0; y < 2; y++)
                            if (allow(x, A[i]) && allow(y, B[i])) {
                                ...
                                dp[1][nb][nx].fr += cur.fr;
                                dp[1][nb][nx].sc += cur.sc;
                            }
                }
        swap(dp[0], dp[1]);
    }
    cout << dp[0][0][0].fr + dp[0][0][1].fr << '\n';
    cerr << "nonzero " << dp[0][0][0].sc + dp[0][0][1].sc << '\n';
}
// check test counter
