...
int main(void)
{
	...
	rep(i, 1, k) ng[uf.root(c[i])] = true;
	
	vector<ll> vec; ll sum = 0;
	rep(i, 1, n) if(uf.root(i) == i){
		if(!ng[i]) sum += uf.v[i];
		else vec.push_back(uf.v[i]);
	}
	ll mx = 0;
	for(auto &x : vec) chmax(mx, x);
	for(auto &x : vec) if(x == mx){
		x += sum;
		break;
	}
	
	ll ans = 0;
	for(auto x : vec) ans += x * (x-1) / 2;
	ans -= m;
	outl(ans);
