...
template<class T> zet solve(vector<T> &a) {
    map<T, int> st;
    for (auto i : a) ++st[i];
    zet ans = 1;
    for (auto i : st) ans *= fact[i.s];
    return ans;
}

template<class T> bool check(vector<T> a) {
    auto b = a;
    sort(b);
    return a == b;
}
...
signed main() {
    ...
    sort(a);
    vector<int> b, c;
    for (auto i : a) b.pb(i.f), c.pb(i.s);
    cout << fact[n] - (solve(b) + solve(c) - (check(b) && check(c) ? solve(a) : 0));
    return 0;   
}