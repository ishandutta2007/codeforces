...
unsigned ask(vector<int> is) {
    assert(int(is.size()) == K);
    cout << "?";
    for (auto i : is) cout << ' ' << i + 1;
    cout << endl;
    unsigned x;
    cin >> x;
    return x;
...
void answer(vector<unsigned> ret) {
    cout << "!";
...
int main() {
    ...
    REP(i, N) {
        REP(del, min(i, K) + 1) {
            int add = K - del;
            int nxt = i - del + add;
            if (nxt <= N and i + add <= N) graph.add_edge(i, nxt, 1);
        }
    }
    graph.solve(0);
    if (graph.dist[N] > N + 1) impossible();
    dbg(graph.dist);

    unsigned all_xor = 0;
    vector<int> state(N);
    auto path = graph.retrieve_path(N);
    // reverse(path.begin(), path.end());

    REP(k, path.size() - 1) {
        int nup = (path[k + 1] - path[k] + K) / 2;
        int ndown = K - nup;
        vector<int> qs;
        REP(i, N) if (nup and !state[i]) qs.push_back(i), nup--;
        REP(i, N) if (ndown and state[i]) qs.push_back(i), ndown--;
        for (auto i : qs) state[i] ^= 1;
        all_xor ^= ask(qs);
    }
    answer({all_xor});
}