...
#ifdef DEBUG
inline int __lg(ull __n){ return sizeof(ull) * __CHAR_BIT__  - 1 - __builtin_clzll(__n); }
...
signed main(){
    ...
    queue<pll>q;
    vv(ll,flip,n,m,LINF);
    rep(n)rep(j,m-1)if(s[i][j]==s[i][j+1]){
        if(chmin(flip[i][j],0))q.emplace(i,j);
        if(chmin(flip[i][j+1],0))q.emplace(i,j+1);
    }
    rep(n-1)rep(j,m)if(s[i][j]==s[i+1][j]){
        if(chmin(flip[i][j],0))q.emplace(i,j);
        if(chmin(flip[i+1][j],0))q.emplace(i+1,j);
    }
    while(q.size()){
        auto [x,y]=q.front();
        q.pop();
        rep(4){
            ...
            if(x2<0||n<=x2||y2<0||m<=y2)continue;
            if(chmin(flip[x2][y2],flip[x][y]+1))q.emplace(x2,y2);
        }
    }
    rep(t){
        LL(x,y,p);
