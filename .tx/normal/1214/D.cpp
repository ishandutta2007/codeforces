...
void add_edge(int from, int to, int cap) {
    g[from].push_back(isz(es));
    es.push_back(edge{to, cap, 0});
    g[to].push_back(isz(es));
...
int dfs(int v, int flow) {
    ...
    for (int i : g[v]) {
        edge e = es[i];
        if (e.c > e.f && !used[e.to]) {
            int f = dfs(e.to, min(flow, e.c - e.f));
            if (f) {
                es[i].f += f;
                es[i ^ 1].f -= f;
                return f;
...
int main(int argc, char** argv) {
    ...
    auto id_in = [&](int x, int y) {
        return (x * m + y) * 2;
    };

    auto id_out = [&](int x, int y) {
        return (x * m + y) * 2 + 1;
    };
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            add_edge(id_in(i, j), id_out(i, j), 1);
            for (int t = 0; t < 2; t++) {
                int x = i + dx[t];
                int y = j + dy[t];
                ...
                add_edge(id_out(i, j), id_in(x, y), 100);
            }
        }
    }
