...
int main() {
    ...
    while (tt--) {
        ...
        vpi use(n);
        set<int> tmp;
        f0r(i, n) re(use[i].f, use[i].s), tmp.ins(use[i].f), tmp.ins(use[i].s);
        map<int, int> conv;
        int cnt= 0 ;
        for (int x : tmp) {
            conv[x] = cnt;
        ...
        f0r(i, n) use[i].f = conv[use[i].f], use[i].s = conv[use[i].s];
        vi pre1(cnt);
        vi pre2(cnt);
        for (auto x : use) {
            pre1[x.f]++;
        ...
        auto sum1 = [&](int l, int r) -> int {
            if (l>r) return 0;
            return pre1[r] - (l?pre1[l-1]:0);
        };
        auto sum2 = [&](int l, int r) -> int {
            if (l>r) return 0;
