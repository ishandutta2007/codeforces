...
int main() {
    ...
    tp.init(n+5);
    for(int i = 1; i <= m; i++){
        if(scc.comp[x[i]] != scc.comp[y[i]]){
            scc_adj[scc.comp[x[i]]].pb(mp(scc.comp[y[i]], w[i]));
            tp.ae(scc.comp[x[i]], scc.comp[y[i]]);
        }
        else{
            scc_val[scc.comp[x[i]]]+=getNum(w[i]);
        }
    }
    ...
    for(int i = sz(tp.res)-1; i >= 0; i--){
        int node = tp.res[i];
        for(auto u: scc_adj[node]){
            ckmax(dp[node], dp[u.f]+u.s);
        }
        dp[node]+=scc_val[node];
    }
