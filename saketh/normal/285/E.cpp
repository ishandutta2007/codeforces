...
int main() {
    ...
    dp[0][0][1] = 1;
    F0R (n, N + 1) F0R (k, n + 1) F0R (st, 4) {
        mn v = dp[n][k][st]; if (!v.v) continue;
        dp[n+1][k][st/2] += v;
        if (!(st&1)) dp[n+1][k+1][st/2] += v;
        dp[n+1][k+1][st/2+2] += v;
    }
    ...
    vmn ways(N + 1);
    F0R (k, N + 1) {
        ways[k] = (dp[N][k][0] + dp[N][k][1]) * mn::fact(N - k);
        if (k >= K) ans += ((k - K)&1 ? -ways[k] : ways[k]) * mn::ncr(k, K);
    }
