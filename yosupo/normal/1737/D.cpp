...
void solve() {
    ...
    for (auto e : edges) {
        ll dist = TEN(9);
        dist = min(dist, dist0[0][e.u] + dist0[e.v][n - 1]);
        dist = min(dist, dist0[0][e.v] + dist0[e.u][n - 1]);

        for (int k = 0; k < n; k++) {
            dist = min(dist, dist0[0][k] + dist0[n - 1][k] + dist0[k][e.u] + 1);
            dist = min(dist, dist0[0][k] + dist0[n - 1][k] + dist0[k][e.v] + 1);
        }
        ans = min(ans, (1 + dist) * e.cost);
    }
    pr.writeln(ans);
}
