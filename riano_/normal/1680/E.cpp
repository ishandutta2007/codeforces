    ...
    rep(ii,T){
        ...
        rep(i,2){
            rep(j,N){
                cin >> c[i][j];
                if(c[i][j]=='*'){
                    chmax(ls,j); chmin(st,j);
                }
            }
        }
        auto dp = v2ll(N+1,2,2e9);
        rep(i,2) dp[st][i] = -1;
        for(int i=st;i<N;i++){
            rep(j,2){
                chmin(dp[i+1][j],dp[i][1-j]+2);
                if(c[1-j][i]=='.') chmin(dp[i+1][j],dp[i][j]+1);
                else chmin(dp[i+1][j],dp[i][j]+2);
            }
        }
        cout << min(dp[ls+1][0],dp[ls+1][1]) << "\n";
    }
