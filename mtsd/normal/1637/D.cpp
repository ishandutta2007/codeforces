...
int main(){
    ...
    rep(zz,T){
        ...
        VEC(ll,b,n);
        ll s = 0;
        rep(i,n)s += a[i]*a[i] + b[i]*b[i];
        s *= (n-1);
        vec(ll,dp,n*100+1,(1LL<<60));
        vec(ll,t,n+1);
        rep(i,n){
            t[i+1] = a[i]+b[i];
            t[i+1] += t[i]; 
        ...
        dp[0] = 0;
        for(int i=0;i<n;i++){
            vec(ll,nxt,n*100+1,(1LL<<60));
            for(int k=0;k<=n*100;k++){
                // a 
                if(k+a[i]<=n*100)chmin(nxt[k+a[i]],dp[k] + 2*a[i]*k + 2 * b[i] * (t[i]-k) );
                // b
                if(k+b[i]<=n*100)chmin(nxt[k+b[i]],dp[k] + 2*b[i]*k + 2 * a[i] * (t[i]-k) );
            }
            swap(dp,nxt);
