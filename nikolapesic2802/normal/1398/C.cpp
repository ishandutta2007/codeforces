...
// URL : http://codeforces.com/contest/1398/problem/C
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
void test(){
	...
	for(int i=n-1;i>=0;i--){
		int tr=s[i]-'0';
		off+=tr;
		auto it=a.lower_bound({tr-off,0});
		if(it==a.end()||(*it).f!=tr-off)
			a.insert({tr-off,1});
		else{
			auto ok=*it;
			a.erase(it);
			a.insert({ok.f,ok.s+1});
		}
		off--;
		it=a.lower_bound({-off,0});
		if(it==a.end()||(*it).f!=-off){
