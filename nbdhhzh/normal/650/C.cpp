...
#define N 1000010
using namespace std;
int A[N],dad[N],v[N<<1],ne[N<<1],l,head[N],ds[N],n,m,i,j,a[N],b[N],c[N],q[N],r;
struct mjj{int x,y;}d[N];
bool cmp(mjj x,mjj y){return x.x<y.x;}
int find(int x){return dad[x]==x?x:dad[x]=find(dad[x]);}
void add(int x,int y){v[++l]=y;ne[l]=head[x];head[x]=l;ds[y]++;}
int main()
{
	scanf("%d%d",&n,&m);
	for(i=0;i<n;i++)
	...
	for(i=0;i<n*m;i++)if(find(i)==i&&ds[i]==0)q[++r]=i,A[i]=1;
	for(l=1;l<=r;l++)
