...
void solve() {
    ...
    vc<mint> dp(n + m + 1, 0);
    dp[0] = 1;
    vvc<int> nxt(m + min(n, 4 * k) + 1, vc<int>(k, m + min(n, 4 * k)));
    for(int i = m + min(n, 4 * k) - 1; i >= 0; -- i) {
        rep(j, k) {
            nxt[i][j] = nxt[i + 1][j];
        }
        nxt[i][t[i] - 'a'] = i;
    ...
    rep(i, n + m) {
        rep(j, k) {
            int pos = (i < m + 2 * k + 1 ? nxt[i][j] : i + di[t[i] - 'a'][j]);
            if(pos >= n + m) continue;
            dp[pos + 1] += dp[i];
        }
    }
