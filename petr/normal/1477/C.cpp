...
class CNezzarAndNiceBeatmap {
 public:
  void solveOne(istream &in, ostream &out) {
    int n;
    in >> n;
    vector<ii> p(n);
    for (auto& x : p) in >> x.first >> x.second;
    ...
    for (int i = 1; i < n; ++i) {
      ...
      for (int j = 0; j < n; ++j) if (!used[j]) {
        int64 dx = p[last].first - p[j].first;
        int64 dy = p[last].second - p[j].second;
        int64 d = dx * dx + dy * dy;
        if (d > bd) {
          bd = d;
          bj = j;
