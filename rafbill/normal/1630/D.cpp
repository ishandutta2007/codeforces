...
void solve(){
  ...
  li b = 0;
  for(li i = 0; i < (li)(m); ++i) {
    li x; cin>>x; b = b==0?x:__gcd(b,x);
  }
  li ans = -1e16;
  for(li p = 0; p < (li)(2); ++p) {
    vvi C(b);
    for(li i = 0; i < (li)(n); ++i) C[i%b].emplace_back(A[i]);
    li s = 0;
    for(li i = 0; i < (li)(n); ++i) s += A[i];
    for(li i = 0; i < (li)(b); ++i) { sort(begin(C[i]), end(C[i])); reverse(begin(C[i]), end(C[i])); }
    if(p) {
      for(li i = 0; i < (li)(b); ++i) {
        s += -2*C[i].back();
        C[i].pop_back();
    ...
    for(li i = 0; i < (li)(b); ++i) while(C[i].size() >= 2 && (C[i].back() + C[i][C[i].size()-2]) < 0) {
      for(li k = 0; k < (li)(2); ++k){
        s += -2*C[i].back();
        C[i].pop_back();
      }
