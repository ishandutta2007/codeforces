...
void init() {
  double dn = 0;
  rep(i,sz(n)) dn = dn/BS+n[i];
  ln = log(dn) + log(BS)*(sz(n)-1);
}
const double l2 = log(2);
const double l3 = log(3);
double fl(int x) {
  if (x%3) return fl(x-2)+l2;
  return l3*(x/3);
}
bint fb(int x) {
  if (x%3) return fb(x-2)*2;
  return bint(3).ex(x/3);
}
int cnt;
bool f(int x) {
  double lx = fl(x);
  if (abs(lx-ln) > 0.05) return ln < lx;
  bint bx = fb(x);
  ++cnt;
  return n <= bx;
}
