                   ...
                   int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500003],d[500003];vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
                       ...
                       bool check(int x,int y)
                    {
                        ...
                        {
                            for(int j=0;j<ab[y].size();j++)
                            {
                                if(ab[y][j]==ab[x][i])return true;
                            }
                        }
                        return false;
                    }
                    void eat(int x,int y)
                    {
                        ...
                        for(int i=0;i<ab[y].size();i++)
                        {
                            tmp.insert(ab[y][i]);
                        }
                        ab[x]=vector<char>(tmp.begin(),tmp.end());
                        ab[y].clear();
                    }
                    void connect()
                    {
                    ...
                    int main()
                    {
                        ...
                        {
                           cin>>s;set<char>tmp;
                           for(int j=0;j<s.size();j++)tmp.insert(s[j]);

                           for(int j=0;j<27;j++)
                           {
                              if(ab[j].empty()){ab[j]=vector<char>(tmp.begin(),tmp.end());break;}
                           }
                           connect();
                        }
                        for(int i=0;i<27;i++)
