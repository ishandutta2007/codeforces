...
int state(int cnt,int extra, int a,int b){
	assert(cnt >= 0 && extra >= 0 && a >= 0 && b >= 0);

	if (vis[cnt][extra][a][b])
		return dp[cnt][extra][a][b];
	vis[cnt][extra][a][b] = true;

	if (cnt == 0){
		assert( (a^b) == 1);
		return dp[cnt][extra][a][b] = a;
	}

	if (extra == 1){
		dp[cnt][extra][a][b] = max( 1 ^ state(cnt-1,extra,b,a ^ 1), 1 ^ state(cnt,extra - 1, b, a));
	}
	else{
		dp[cnt][extra][a][b] = 1 ^ state(cnt - 1, extra, b, a ^ 1);
	}
	return dp[cnt][extra][a][b];
