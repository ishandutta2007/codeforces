...
struct tree{
  ...
  tree(int n):T(n*4){}
  void set1(int p,int a,int b,int k){
    if(b==a+1){
      T[k]=1;
      return;
    }
  ...
  int isum(int l,int r,int a,int b,int k){
    if(l>=b || r<=a)
      return 0;
    if(l<=a && r>=b)
      return b-a-T[k];
    int c=(a+b)/2;
    return isum(l,r,a,c,k*2+1)+isum(l,r,c,b,k*2+2);
  }
};
