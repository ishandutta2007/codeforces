    ...
    rep(i, 1 << 5) {
        rep(j, 1 << 5) {
            ...
            rep(a, n) rep(b, a) {
                if(s[a] != s[b]) {
                    if(v[a].fi == v[b].fi) {
                        flag &= (j & 1 << v[b].se or j & 1 << v[a].se);
                    } else if(v[a].se == v[b].se)
                        flag &= (i & 1 << v[a].fi or i & 1 << v[b].fi);
                    else {
                        flag &= (i & 1 << v[a].fi or i & 1 << v[b].fi) or (j & 1 << v[b].se or j & 1 << v[a].se);
                    }
                }
            }
            if(flag) { chmin(ans, popcount((i << 5) + j)); }
        }
    }
    cout << ans << endl;
}