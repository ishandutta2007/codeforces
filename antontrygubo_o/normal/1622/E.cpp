...
void solve()
{
    ...
    for (int mask = 0; mask<(1<<n); mask++)
    {
        ...
        vector<int> coeff(m);
        for (int i = 0; i<m; i++)
            for (int j = 0; j<n; j++) if (res[j][i]=='1')
            {
                if (mask&(1<<j)) coeff[i]++;
                else coeff[i]--;
            }
        ...
        for (int i = 0; i<m; i++) who.push_back(mp(coeff[i], i));
        sort(who.begin(), who.end());

        for (int i = 0; i<m; i++) tot+=1ll*who[i].first*(i+1);

        if (tot>largest)
        {
            largest = tot; best.clear(); best.resize(m);
            for (int i = 0; i<m; i++) best[who[i].second] = i+1;
        }
    }
