...
ll ans[100010];
vector<pl> edges[100010], bedges[100010];
...
int main() {
    ...
    for (ll i=0; i<m; ++i) {
        ...
        bedges[v].push_back({u, w});
    }

    fill(begin(ans), end(ans), LLONG_MAX);
    ...
    while (q.size()) {
        auto h = q.top(); q.pop();
        auto __p = h; auto d = __p.first; auto u = __p.second;

        if (used.count(u)) continue;
        used.insert(u);
        ans[u.first] = min(ans[u.first], d);


        if (u.second==0) {
            pl v = {u.first, 1};
            ({if (used.count(v)==0 && (dist.count(v)==0 || dist[v]>d+0)) {dist[v]=d+0; q.push({d+0, v});} });
        }
        for (auto e : (u.second?bedges : edges)[u.first]) {
            pl v = {e.first, u.second};
            ({if (used.count(v)==0 && (dist.count(v)==0 || dist[v]>d+e.second)) {dist[v]=d+e.second; q.push({d+e.second, v});} });
        }
    }
    for (ll i=2; i<=n; ++i) {
