    ...
    TEST {
        ...
        if(s.back() == '0') {
            if(p.back().se == 1)
                ...
                tz2 += v.back(), v.pop_back(), tz++;
        }
        if(s.front() == '0') {
            if(p.front().se == 1)
                z--, tz++;
            else
                tz2 += v[0], v.erase(begin(v)), tz++;
        }

        ll ma = 0;
        ll mA = A, mB = B, mz = z, mtz = tz, mtz2 = tz2;
        sort(rall(v));
        auto mv = v;
        ...
        rep(k, 2) {
            dump(k);
            int turn = k;
            A = mA, B = mB, z = mz, tz = mtz, v = mv, tz2 = mtz2;
            ll tmp = 0;
            while(1) {
                if(turn) {
                    if(!B) break;
                    ...
                    if(!empty(v) or tz2) { chmax(ma, tmp + a); }
                    if(empty(v) and !z and !tz and !tz2) break;
                    if(!B) {
                        if(z) {
                            z--;
                            B++;
