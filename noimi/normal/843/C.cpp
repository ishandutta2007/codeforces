...
signed main(){
    ...
    vector<int> centroids;
    {
        auto dfs = [&](auto &&f,int x,int p)->int{
            sub[x] = 1;
            for(auto e:g[x]) if(e!=p) sub[x] += f(f,e,x);
            return sub[x];
        };
        dfs(dfs,0,-1);
        auto search = [&](auto &&f,int x,int p)->int{
            for(auto e:g[x]){
                if(e == p) continue;
                if(sub[e] > n/2) return f(f,e,x);
            }
            return x;
        };
        centroid = search(search,0,-1);
        dfs(dfs,centroid,-1);
        rep(i,n) if(~n&1 and sub[i] == n/2) centroids.eb(i);
        centroids.eb(centroid);
    }
    vector<int> top(n),tail(n);
    vector<tuple<int,int,int>> ans;
    auto dfs = [&](auto &&f,int x,int p,int c,int cc)->void{
        if(p!=c and p!= cc){
            ans.eb(c,cc,x);
            ans.eb(x,p,cc);
            ans.eb(c,x,cc);
        }
        for(auto e:g[x]) if(e != p) f(f,e,x,c,cc);
        return ;
    };
    vector<bool> ng(n);
    for(auto c:centroids) ng[c] = true;
    ...
    auto same = [](T x,T y){
        return get<0>(x) == get<0>(y) and get<2>(x) == get<1>(y);
    };
    rep(i,ans.size()){
        if(i < ans.size()-1 and same(ans[i],ans[i+1])){
            ANS.eb(get<0>(ans[i]),get<1>(ans[i]),get<2>(ans[i+1]));
            i++;continue;
        }
        ANS.eb(ans[i]);
    }
