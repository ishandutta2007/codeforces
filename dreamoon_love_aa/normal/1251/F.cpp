...
inline void calc(){
    ...
    for(int bb:b){
        ...
        VI ones(one+1),twos(two*2+1);
        int tmp=1;
        FOR(i,0,one){
            ones[i]=C(one,i)*tmp%MOD;
            tmp+=tmp;
            if(tmp>=MOD)tmp-=MOD;
        }
        FOR(i,0,two*2){
        ...
        VI res=NTT::mult(ones,twos);
        REP(i,SZ(res)){
            if(bb+i+1<=600000)add(an[bb+i+1],res[i]);
            else break;
        }
    }
}
