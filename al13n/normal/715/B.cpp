...
int main(int argc, char **argv){
  ...
  for(int qqq=0;;++qqq){
    ...
    dbg(G);
    dbg(D);
    if(D[T].X>=L && (D[T+n].X==L || (D[T+n].X>=L && D[T].X==L)))
      break;
    if(D[T].X<L || D[T+n].X>L){
      assert(qqq==0);
      cout<<"NO";
      return 0;
    }
  ...
  vector<tuple<int,int,LL>> ans(m);
  forn(v,n){
    forv(i,G[v]){
      int q=Gi[v][i];
      if(q==-1)
        continue;
      ans[q]=MT(v,G[v][i].X,G[v][i].Y?G[v][i].Y:1);
    }
  }
  cout<<"YES\n";
  for(auto t:ans)
