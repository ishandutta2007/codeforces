...
int main() {
    ...
    std::cin.tie(nullptr);
    input(int, n, m, k);
    vec<tuple<long long, int, int>> edges(m);
    rep(i, m) {
        input(int, u, v, w);
        --u, --v;
        edges[i] = { w, u, v };
    ...
    auto dijkstra = [&](int src) {
        ...
        while (pq.size()) {
            ...
            if (du == dist[u]) {
                for (auto [v, w] : g[u]) {
                    long long new_cost = du + w;
                    auto it = dist.find(v);
                    if (it == dist.end() or new_cost < it->second) {
                        pq.emplace(dist[v] = new_cost, v);
                    }
                }
            }
        }
    ...
    vector<tuple<long long, int, int>> costs;
    rep(u, n) {
        for (const auto &[v, w] : dijkstra(u)) {
            if (u < v) {
