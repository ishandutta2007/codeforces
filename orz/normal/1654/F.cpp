...
void rdx_sort(vi& v, vi& c, const int B)
{
	for (int i = 0; i < sz(v); ++i)
		v[i] ^= B;
	...
	for (int i = 0; i < sz(v); ++i)
	{
		if (i == 0 || pii(c[v[i]], c[v[i] ^ B]) != pii(c[v[i - 1]], c[v[i - 1] ^ B]))
			++M;
		cc[v[i]] = M;
	}
	c.swap(cc);
...
void rdx(vi& v, vi& c, const int b)
{
	int B = 1 << b;
	rdx_sort(v, c, B);
}

string transform(const string& s, const int start, const int finish, const int xr)
{
	string t(finish - start, '\0');
