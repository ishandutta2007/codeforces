...
std :: unordered_map <int, int> fa[MAXN], ans[MAXN];
inline int find(int x, const int c){return fa[x][c] == x? x : fa[x][c] = find(fa[x][c], c);}

int main(){
    read(n, m); rep(i, 1, m) {
        int u, v, c; read(u, v, c);
        if(fa[u].find(c) == fa[u].end()) fa[u][c] = u;
        if(fa[v].find(c) == fa[v].end()) fa[v][c] = v;
        const int xx = find(u, c), yy = find(v, c);
        if(xx ^ yy) fa[xx][c] = yy;
    } read(q); while(q --) {
        int u, v, res = 0; read(u, v);
        if(fa[u].size() > fa[v].size()) u ^= v ^= u ^= v;
        if(ans[u].find(v) != ans[u].end()) {print(ans[u][v]); continue; }
        for (auto it : fa[u]) res += fa[v].find(it.first) != fa[v].end() && find(u, it.first) == find(v, it.first);
        print(ans[u][v] = res);
    }
}