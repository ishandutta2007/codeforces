#line 1 "codeforces-workspace/contest/1510/g/g.cpp"
...
#line 11 "codeforces-workspace/contest/1510/g/g.cpp"
...
void main() {
  ...
  auto far = max_element(begin(dist), end(dist)) - begin(dist);
  while (k < dist[far]) far = par[far];
  auto len = k + max(0, k - dist[far]);
  k -= dist[far];

  vector<int> ban(n);
  for (int now = far; ~now; now = par[now]) ban[now] = 1;

  vector<int> ans;
  vector<int> iter(n);

  for (int now = far, pre = -1; ~now;) {
    if (ans.empty() || ans.back() != now) ans.emplace_back(now);

    if (iter[now] == tree[now].size()) {
      pre = now;
      now = par[now];
      continue;
    }

    auto to = tree[now][iter[now]++];
    if (ban[to] or !k) continue;
    now = to;
    k--;
  }

  assert(!ans.back());
  assert(ans.size() - 1 == len);
