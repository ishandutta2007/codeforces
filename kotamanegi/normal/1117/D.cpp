...
void solve() {
    ...
    vector<vector<modint<MAX_MOD>>> sets;
    REP(i, m) {
        vector<modint<MAX_MOD>> hoge(m, 0);
        hoge[max(0LL, i - 1)] += 1;
        if (i == 0) hoge.back() += 1;
        sets.push_back(hoge);
    }
    Matrix<modint<MAX_MOD>> uses(sets);
    uses = pows<Matrix<modint<MAX_MOD>>>(uses, n - m + 1LL);
    modint<MAX_MOD> ans = 0;
    REP(i, uses.data.size()) {
        for (auto& x : uses.data[i]) {
            ans += x;
        }
        break;
    }
