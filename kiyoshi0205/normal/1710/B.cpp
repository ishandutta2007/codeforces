...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  vec event(za.size(),0);
  for(auto&[x,p]:v){
    event[lower_bound(all(za),x-p)-za.begin()]++;
    event[lower_bound(all(za),x)-za.begin()]-=2;
    event[lower_bound(all(za),x+p)-za.begin()]++;
  }
  ll evsum=0;
  //
  vec val(za.size(),0);
  rep(i,za.size()){
    if(i)val[i]=val[i-1]+evsum*(za[i]-za[i-1]);
    evsum+=event[i];
  }
  debug(val);
  vec res(za.size(),0);
  evsum=-inf;
  rep(i,za.size()){
    if(val[i]>M)chmax(evsum,val[i]-M-(za[i]-za[0]));
    chmax(res[i],evsum+(za[i]-za[0]));
  }
  evsum=-inf;
  brep(i,za.size()){
    if(val[i]>M)chmax(evsum,val[i]-M-(za.back()-za[i]));
    chmax(res[i],evsum+(za.back()-za[i]));
  }
  debug(res);
  for(auto&[x,p]:v){
    cout<<ll(res[lower_bound(all(za),x)-za.begin()]<=p);
  }
  print("");
}
}