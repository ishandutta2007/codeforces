...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		int n, m;
		in >> n >> m;
		using Elem = bitset<1000>;
		struct Merge {
			Elem operator() (const Elem& a, const Elem& b) const {
				return a | b;
			}
		};
		vector<int> values(n);
		...
		for (int i: range(n)) {
			initial_values[i] = values[order[i]];
		}
		Elem mask;
		for(int i: range(m)) {
			mask[i] = true;
		}
		auto apply = [&](Elem& a, int b, int , int ) {
			if (b == 0) {
				return a;
			}
			Elem res;
		...
		vector<Elem> sta(n);
		for (int i: range(n)) {
			sta[i].set(initial_values[i] % m);
		}

		TopDownSegmentTree<Elem, Merge, int, decltype(apply), decltype(merge_update)> tree(sta, Elem(), Merge(), 0, apply, merge_update);


		auto primes = allPrimes(m - 1);
		int q;
		in >> q;
		for (int i: range(q)) {
			int t, v;
