...
void solve() {
    input(int, n);
    vector is_same(n, vector(n, vector<int8_t>(n, false)));
    vector<LinkedUnionFind> ufs(n, LinkedUnionFind(n));
    rep(i, n - 1) {
        rep(j, i + 1, n) {
            input(string, s);
            rep(z, n) {
                is_same[z][i][j] = is_same[z][j][i] = s[z] == '1';
                if (s[z] == '1') {
                    ufs[z].merge(i, j);
                }
            }
    ...
    rep(z, n) {
        rep(i, n) is_same[z][i][i] = true;
        for (auto group : ufs[z].groups()) {
            int siz = group.size();
            rep(i, siz) rep(j, siz) {
                if (not is_same[z][group[i]][group[j]]) {
                    print("No");
                    // cerr << "Type1" << endl;
                    return;
                }
    ...
    for (auto group : ufs[0].groups()) {
        ...
        auto dfs = [&](auto dfs, int u, int p) -> bool {
            ...
            used[u] = true;
            par[u] = p;
            g[u] = ufs[u].connected_component(p);
        ...
        for (auto e : used) ok &= e;
        if (not ok) continue;

        rep(z, n) {
            auto groups = ufs[z].groups();
            ...
            vector<vector<int>> groups2(*max_element(all(d)) + 1);
            if (groups.size() != groups2.size()) {
                ok = false;
                break;
            }
            rep(i, n) groups2[d[i]].push_back(i);
            sort(all(groups2));

            for (auto &group : groups) sort(all(group));
            sort(all(groups));
