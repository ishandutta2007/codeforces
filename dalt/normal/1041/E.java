...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Deque<Integer> notZero = new ArrayDeque();
            for (int i = 1; i <= n; i++) {
                if (appear[i] == 0) {
                    zero.addLast(i);
                } else {
            ...
            if (appear[n] != n - 1 || appear[n - 1] == 0) {
                io.cache.append("NO");
                return;
            }

            io.cache.append("YES\n");
            while (notZero.size() > 2) {
                int nz = notZero.removeFirst();
                int last = nz;
                while (appear[nz] > 1) {
                    int min;
                    if (zero.isEmpty() || (min = zero.removeFirst()) > nz) {
                        io.cache.setLength(0);
                        io.cache.append("NO");
                        return;
                    }
                    io.cache.append(last).append(' ').append(min).append('\n');
                    last = min;
                    appear[nz]--;
                }
                io.cache.append(last).append(' ').append(n - 1).append('\n');
            }

            int last = n;
            while (!zero.isEmpty()) {
                int z = zero.removeFirst();
                io.cache.append(last).append(' ').append(z).append('\n');
                last = z;
            }
            io.cache.append(last).append(' ').append(n - 1).append('\n');
