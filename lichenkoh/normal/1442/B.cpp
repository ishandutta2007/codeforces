...
void go() {
  int n,k; cin>>n>>k;
  vi a(n),b(k),inva(n+1),invb(n+1,-INF);
  for (int x=1;x<=n;x++) invb[x]=-INF;
  for (int i=0;i<n;i++) cin>>a[i];
  for (int i=0;i<k;i++) cin>>b[i];
  for (int i=0;i<n;i++) inva[a[i]]=i;
  for (int i=0;i<k;i++) invb[b[i]]=i;
  mint ans=1;
  for (int posb=0;posb<k;posb++) {
    int val=b[posb];
    int posa=inva[val];
    int can=0;
    if (posa>0&&invb[a[posa-1]]<posb) {
      can++;
    }
    if (posa<n-1&&invb[a[posa+1]]<posb) {
      can++;
    }
    ans*=can;
  }
