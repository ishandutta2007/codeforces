...
// P^n
pub fn power_small(p: Vec<M>, n: usize) -> Vec<M> {
    assert!(p.len() > 0 && !p[0].is_zero());
    let mut q = vec![M::zero(); 1 + (p.len() - 1) * n];
    q[0] = p[0].pow(n as u64);
    let pc = precalc::Precalc::new(q.len());
    let inv = p[0].inv();
    for i in 0..(q.len() - 1) {
        let mut s = M::zero();
        for (j, (q, p)) in q[..=i].iter().rev().zip(p[1..].iter()).enumerate() {
            s += M::from(j + 1) * *p * *q;
        }
        s *= M::from(n);
        for (j, (q, p)) in q[..=i].iter().rev().zip(p[1..].iter()).enumerate() {
...
fn run() {
    ...
    let a = *d.iter().max().unwrap();
    let b = *d.iter().min().unwrap();
    let mut p = vec![M::zero(); a - b + 1];
    for d in d {
        p[d - b] = M::one();
    }
    let dp = power_small(p, n / 2);
    let ans = dp.into_iter().fold(M::zero(), |s, a| s + a * a);
    println!("{}", ans);
}
