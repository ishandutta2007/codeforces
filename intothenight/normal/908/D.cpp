...
int main(){
	...
	Zp pa, pb;
	cin >> k >> pa >> pb;
	tie(pa, pb) = pair{pa / (pa + pb), pb / (pa + pb)};
	vector<vector<Zp>> dp(k, vector<Zp>(k + 1));
	// # of ab subsq, # of a
	for(auto i = k - 1; i >= 0; -- i){
		// i + j >= k
		for(auto j = k; j >= k - i; -- j){
			dp[i][j] = i + j + pa / pb;
		}
		// dp[i][j] = pa * dp[i][j + 1] + pb * dp[i + j][j] if j != 0
		for(auto j = k - i - 1; j >= 1; -- j){
			dp[i][j] = pa * dp[i][j + 1] + pb * dp[i + j][j];
		}
		// dp[i][0] = pa * dp[i][1] + pb * dp[i][0]
		dp[i][0] = dp[i][1];
	}
	debug2(dp);
	cout << dp[0][0] << "\n";
