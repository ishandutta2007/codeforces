...
void solve() {
  ...
  auto get_divisors = [&](ll N) -> vi {
    ...
    reverse(all(L));
    S.insert(S.end(), all(L));
    return S;
  };

  const ll INF = 1LL << 60;

  vi DIV = get_divisors(K);
  ll M = DIV.size();
  ...
  FOR(i, N) {
    ...
    FOR(m, M) {
      auto [items, sum] = DP[i][m];
      if (sum == INF) continue;
      ll rest = K / DIV[m];
      ll to = LB(DIV, gcd(x, rest) * DIV[m]);
      if (chmin(DP[i + 1][to], mp(items + 1, sum + A[i]))) {
        PAR[i + 1][to] = m;
      }
    }
  }

  if (DP[N].back().fi == INF) return print(-1);
  vi ANS;
  ll i = N, m = M - 1;
  while (i) {
    auto p = PAR[i][m];
