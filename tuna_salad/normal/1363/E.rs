...
fn dfs(u: usize, par: usize, e: &[Vec<usize>], need: &[(usize,usize)], a: &[i64], mut min: i64) -> (i64,usize,usize) {
	let mut n0 = need[u];
	let mut res = 0;
	m!(<, min, a[u]);
	for &v in e[u].iter() {
		if v != par {
			let (r,x,y) = dfs(v, u, e, need, a, min);
			res += r;
			n0.0 += x;
			n0.1 += y;
		}
...
fn main() {
	...
	let sol = dfs(0, n, &e, &need, &a, 1000000000).0;
	writeln!(rout, "{}", sol).ok();
}