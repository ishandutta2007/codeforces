...
void solve() {
    ...
    for (int i = 0; i < m; i++) {
        Edge e;
        cin >> e.u >> e.v >> e.w;
        aa[e.u].push_back(e.v);
        aa[e.v].push_back(e.u);
        edges.push_back(e);
    }
    
    vector<vector<int>> dist(n+1, vector<int>(n+1, 1e8));

    auto bfs = [&aa](int root, vector<int>& d) {
        d[root] = 0;
        queue<int> q;
        q.push(root);
        while (!q.empty()) {
    ...
    for (int root = 1; root <= n; root++) bfs(root, dist[root]);

    LL res = 1e18;
    for (Edge e: edges) {
        res = min(res, (dist[e.u][1] + dist[e.v][n] + 1) * e.w);
        res = min(res, (dist[e.u][n] + dist[e.v][1] + 1) * e.w);

        for (int x = 1; x <= n; x++) {
            res = min(res,
                (min(dist[x][e.u], dist[x][e.v]) + dist[x][1] + dist[x][n] + 2) * e.w);
        }
    }
    cout << res << "\n";
}
