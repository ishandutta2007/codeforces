...
int insert_bit(int s, int i, bool b) {
    int lower_bits = s & ((1 << i) - 1);
    return ((s ^ lower_bits) << 1) | lower_bits | (b << i);
}
...
int main() {
    inv_mods<mint> invs(L);

    vector dp(L + 1, vector(L, vector<mint>(1 << L)));
    ...
    vector f(L + 1, vector(L, vector<mint>(L)));
    vector g(L + 1, vector(L, vector<mint>(L)));
    rep(i, 1, L + 1) rep(j, L) {
        rep(s, 1 << L) {
            if (dp[i][j][s] == 0) continue;
            g[i][j][popcount(s)] += dp[i][j][s];
        }
        rep(k, L) {
            f[i][j][k] = g[i][j][k];
            rep(x, 1, i) rep(y, j + 1) rep(z, k + 1) {
                f[i][j][k] -= f[x][y][z] * g[i - x][j - y][k - z];
            }
        }
    }

    vector h(L + 1, vector(2 * L + 1, vector(L, vector<mint>(L))));
    rep(i, 1, L + 1) rep(j, L) {
        rep(k, L) h[1][i][j][k] = f[i][j][k];
    }
    rep(num, 2, L + 1) rep(i, 2 * L + 1) rep(j, L) rep(k, L) {
        rep(x, 1, i) rep(y, 1, j) rep(z, 1, k) {
            if (x > L) continue;
            h[num][i][j][k] += h[num - 1][i - x][j - y][k - z] * f[x][y][z];
        }
    }
    ...
    loop(t) {
        ...
        mint ans = 0;
        rep(num, L + 1) rep(len, 1, 2 * L + 1) {
            mint v = h[num][len][a][b];
            
            if ((n - len) + num < num) continue;
