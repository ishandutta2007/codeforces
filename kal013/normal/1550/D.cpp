...
const int modular_int<mod>::inverse_limit = 200100;
...
void solve(){
    ...
    long long lb = max(l - 1, n - r);
    long long ub = min(r - n, 1 - l);

    assert (lb + ub == 0 && lb <= ub);

    mint ans = (ub) * ncr(n, (n >> 1));
    ...
    for(long long x = ub + 1; ; ++x){
    	long long pos = min(r - x, n);
    	long long neg = max(l + x, 1LL);
    	if (pos + 1 < neg)
    		break;
    	long long common = (pos - neg + 1);
    	assert (common < n);

    	neg = (n - neg + 1);
    	if (n & 1){

    		ans += ncr(n - (pos - common) - (neg - common), (n >> 1) - (pos - common));
    		ans += ncr(n - (pos - common) - (neg - common), (n >> 1) - (neg - common));
