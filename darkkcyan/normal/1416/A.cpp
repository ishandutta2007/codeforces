...
void print_ans(ostream& cout, const ans_type& ans) {
    for (auto i: ans) cout << (i == INT_MAX ? -1 : i) << ' ';
    cout << '\n';
}
...
ans_type solve() {
    ...
    rep(val, n) {
        if (!len(pos[val])) continue;
        auto& cur_pos = pos[val];
        int max_dist = n - cur_pos.back();
        int prev = -1;
        for (auto i: cur_pos) {
            max_dist = max(max_dist, i - prev);
            prev = i;
        }
        ans[max_dist - 1] = min(ans[max_dist - 1], val + 1);
        clog << db(val) << db(max_dist) << db(cur_pos) << endl;
    }
    clog << db(ans) << endl;
