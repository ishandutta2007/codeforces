...
const sieve<int(2e6) + 1> sv;

template <typename F> bool good(vector<int> &a, F f) {
    const int n = sz(a);
    map<int, int> freq;
    for (int i : a)
        freq[f(i)]++;
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            if (x == 0)
                continue;
            sv.for_each_divisor_unordered(x, L1(u, candidates.push_back(u)), ALL);
        }
    }

    dedup(candidates);

    for (int i : reversed(candidates)) {
        if (good(a, L1(u, u % i))) {
            ps(i);
            return;
        }
