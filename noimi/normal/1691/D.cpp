...
int main() {
    TEST {
        ...
        vl v = RUI(a);
        bool flag = true;
        segtree<pii, f, e> seg(n);
        rep(i, n) seg.set(i, pii(a[i], i));
        RmT<ll> seg2(v);
        RMT<ll> seg3(v);
        REC([&](auto &&f, int l, int r) -> void {
            if(l >= r - 1) return;
            int m = seg.prod(l, r).se;
            ll L = seg2.prod(l, m + 1);
            ll R = seg3.prod(m + 1, r + 1);
            if(R - L - a[m] > 0) { flag = false; }
            f(l, m);
            f(m + 1, r);
        })
        (0, n);
