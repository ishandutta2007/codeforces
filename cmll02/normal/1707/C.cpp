...
mgs
newe(100006);
int U[1000005],V[1000005],C[1000005];
#define f ff
int f[100005][20];
arr dfn;
arr p;
...
{
	sz[u]=1,dfn[u]=++cc;dep[u]=dep[fa]+1;
	idfn[cc]=u;
	f[u][0]=fa;
	for(int i=1;(1<<i)<=dep[u];i++)f[u][i]=f[f[u][i-1]][i-1];
	fe(u)if(v!=fa)dfs(v,u),sz[u]+=sz[v];gr
}
void add(int l,int r,int w){p[l]+=w,p[r+1]-=w;}
signed main()
{
	initprog();
	int n=read(),m=read();
	...
	U[i]=u,V[i]=v;
	if(C[i]=uf(u,v))addedge(u,v,1),addedge(v,u,1);
	gr
	dfs(1,0);
	rg(m)if(C[i])continue;
	int u=U[i],v=V[i];//odp(u,v);
	if(dfn[u]>dfn[v])u^=v^=u^=v;
	if(dfn[v]<=dfn[u]+sz[u]-1)
	{
		int ht=dep[v]-dep[u]-1;
		int w=v;
		for(int i=0;i<20;i++)if(ht>>i&1)w=f[w][i];
		add(dfn[w],dfn[w]+sz[w]-1,-1);
