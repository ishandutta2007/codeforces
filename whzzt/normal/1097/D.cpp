...
int cal(i64 p,int e){
	...
	memset(f,0,sizeof(f));
	f[e]=1;
	for(i=0;i<=e;++i) coef[i]=Inv(i+1,P);
	for(i=1;i<=k;++i){
		for(j=0;j<=e;++j) f[j]=i64(f[j])*coef[j]%P;
		for(j=e;j>=0;--j) f[j]=(f[j]+f[j+1])%P;
	}
	int res=0;
	for(j=e;j>=0;--j) res=(i64(res)*(p%P)+f[j])%P;
	return res;
}
int main(){
	int p,e;
	cin>>n>>k;
	for(p=2;i64(p)*p<=n;++p) if(n%p==0) {
		for(e=0;n%p==0;++e) ++num[p],n/=p;
	}
	if(n>1) ++num[n];
	ans=1;
	for(auto it:num)ans=i64(ans)*cal(it.fi,it.se)%P;
	cout<<ans<<endl;
}