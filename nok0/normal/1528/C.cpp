...
void main_() {
	...
	auto g = a.root_to_leaf(0);
	RangeSet<int> st;
	int res = 0;
	auto dfs = [&](auto self, int now) -> void {
		auto l = in[now], r = out[now] + 1;
		int hl = -1, hr = -1;
		bool ins = 1;
		if(st.covered(l, r)) {
			auto p = st.covered_by(l, r);
			hl = p.first, hr = p.second;
			if(hl < l)
				st.erase(hl, hr);
			else
				ins = 0;
		}
		if(ins) st.insert(l, r);
		chmax(res, st.size());
		for(auto &[to, cost] : g[now]) self(self, to);
		if(ins) st.erase(l, r);
		if(hl != -1) st.insert(hl, hr);
	};
	dfs(dfs, 0);
	print(res);
}
