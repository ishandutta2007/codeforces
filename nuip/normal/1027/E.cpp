...
int main(){
  ...
  dp[0][0]=1;
	rep(i,510)rep(j,510)reps(k,1,510-i) dp[i+k][max(j,k)]+=dp[i][j];
	int n,t;
	cin>>n>>t;
	mint re=0;
	//reps(i,1,n+1)reps(j,1,n+1)if(i*j<t) out(i,j,dp[n][i],dp[n][j],1);
	reps(i,1,n+1)reps(j,1,n+1)if(i*j<t) re+=dp[n][i]*dp[n][j];
	cout<<re*2<<endl;
  return 0;
}