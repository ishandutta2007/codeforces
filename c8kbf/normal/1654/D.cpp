...
_ n, c[maxn], a[maxn], s, t, x, y, rx, ry;
struct edge {
    _ to, rx, ry;
};
vector<edge> g[maxn];
...
void brewing(_ x, _ fa);
_ gcd(_ a, _ b);
void potion(_ x, _ fa);
_ pw(_ x, _ p);
inline void AC() {
    
    n = read();
    for(_ i = 1; i <= n; ++i) if(!v.test(i)) {
        v.set(i);
        _ x = i;
        for(_ j = 2; j*j <= x; ++j) if(!(x%j)) for(; !(x%j); x /= j) {
            if(b[i].empty() || b[i].back().first != j) b[i].push_back(mp(j, 1));
            else ++b[i].back().second;
        }
        if(x != 1) b[i].push_back(mp(x, 1));
    }
    for(_ i = 1; i <= n; ++i) {
        g[i].clear();
        c[i] = a[i] = 0;
    ...
    for(_ i = n; --i; ) {
        ...
        rx /= gd;
        ry /= gd;
        g[x].push_back((edge){y, rx, ry});
        g[y].push_back((edge){x, ry, rx});
    }
    potion(1, -1);
    t = 1;
    for(_ i = 1; i <= n; ++i) t = t*pw(i, a[i])%mod;
    brewing(1, -1);
    writeln(s);
...
void potion(_ x, _ fa) {
    for(edge i : g[x]) if(i.to != fa) {
        for(_p j : b[i.rx]) {
            _ inc = max(0LL, j.second-c[j.first]);
            c[j.first] += inc;
            a[j.first] += inc;
        }
        for(_p j : b[i.rx]) c[j.first] -= j.second;
        for(_p j : b[i.ry]) c[j.first] += j.second;
        potion(i.to, x);
        for(_p j : b[i.rx]) c[j.first] += j.second;
        for(_p j : b[i.ry]) c[j.first] -= j.second;
    }
...
void brewing(_ x, _ fa) {
    s = (s+t)%mod;
    for(edge i : g[x]) if(i.to != fa) {
        t = t*pw(i.rx, mod-2)%mod*i.ry%mod;
        brewing(i.to, x);
        t = t*i.rx%mod*pw(i.ry, mod-2)%mod;
    }
    return;
}