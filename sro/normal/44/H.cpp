...
{
	cin>>s;
	for(int i=0;i<SZ(s);i++)t[i+1]=s[i]-'0';
	int n=SZ(s);
	for(int i=0;i<10;i++)dp[1][i]=1;
	for(int i=2;i<=n;i++)
		for(int j=0;j<10;j++)
		{
			if(j*2-1-t[i]>=10)continue;
			dp[i][j]+=dp[i-1][j*2-1-t[i]];
			if(j*2-t[i]>=10)continue;
			dp[i][j]+=dp[i-1][j*2-t[i]];
			if(j*2+1-t[i]>=10)continue;
			dp[i][j]+=dp[i-1][j*2+1-t[i]];
		}
	INT sum=0,can=1;
	for(int i=0;i<10;i++)
		sum+=dp[n][i];
	for(int i=2;i<=n;i++)
		if(t[i-1]!=t[i]-1&&t[i-1]!=t[i]&&t[i-1]!=t[i]+1)can=0;
	cout<<sum-can<<endl;
	return 0;
}