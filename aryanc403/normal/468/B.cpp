...
const lli mod = 1000000007L;
    ...
    lli T,n,i,j,k,in,cnt,l,r,u,v,x,y,a,b;
    string s;
    map<lli,lli> ans;
    vi aa;
    set<lli> vis;
...
bool dfs(lli x,lli y,lli no)
{
    auto it=ans.find(x);
    if(it==ans.end()||it->Y==no)
        return true;
    it->Y=no;
    if(ans.find(y-x)==ans.end())
        return false;
    return dfs(a-x,y,no)&&dfs(b-x,y,no);
}
