...
template<typename E> struct heavy_path_decomposition : lowest_common_ancestor<E> {
    ...
    vpii rooted_subtree(int u, int r) const {
...
int main() {
    ...
    using E = wedge<>;
    int N, K; re(N, K); tree<E> tr(N); re(tr);
    heavy_path_decomposition<E> hld(tr);

    auto pp = [](pii a, pii b){ return pii{min(a.f, b.f), max(a.s, b.s)}; };
    full_segment_tree st(N, pii{INT_MAX, -1}, pp);

    auto put = [&](int i, bool v) { st.replace(i, v ? pii{i, i} : pii{INT_MAX, -1}); };
    auto get = [&](int i, int j) {
        if (i >= j) return false;
        if (j - i == 1) return st.table[i + st.SZ].s != -1;
        i += st.SZ, j += st.SZ;
        int k = 31 - __builtin_clz(j - i);
        int m = (1 << k) - 1;
        int l = i - (i & m);
        if (st.table[l >> k].s >= i - st.SZ) return true;
        if (st.table[(l >> k) + 1].f < j - st.SZ) return true;
        if (l + (2 << k) < j && st.table[(l >> k) + 2].f < j - st.SZ) return true;
        return false;
    };

    auto ii = [](bool a, bool b){ return a || b; };
    auto uc = [&](int v, int u) {
        int d = hld.dist(u, v);
        int lo = 0, hi = d;
        while (lo < hi) {
            int mi = (lo + 1 + hi) / 2;
            int anc = hld.kth_step(u, v, mi);
            if (hld.accumulate_subtree(anc, u, false, ii, get)) {
                lo = mi;
            } else {
                hi = mi - 1;
            }
    ...
    F0R (i, N) {
        while (cov <= K && j < N) {
            cov += uc(j, i);
            put(hld.index(j++), true);
            if (cov <= K) ckmax(ans, j - i);
        }
        {
            put(hld.index(i), false);
            if (j > i+1) cov -= uc(i, i+1);
            else cov = 1;
        }
    }
