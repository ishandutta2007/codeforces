#line 1 "codeforces-workspace/contest/367/e/e.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/367/e/e.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/367/e/e.cpp"
...
#line 27 "codeforces-workspace/contest/367/e/e.cpp"
...
void main() {
  ...
  auto dp = make_vector<mint>({n + 1, n + 1}, 0);
  dp[0][0] = 1;

  for (auto t : range(m + 1)) {
    ...
    for (auto i : rrange(n)) {
      if (t == z) fill(begin(dp[i + 1]), end(dp[i + 1]), 0);
      for (auto j : rrange(i + 1)) {
        dp[i + 1][j] += dp[i][j];
      }
    }
    if (t == z) dp[0][0] = 0;
