...
inline fun NS_R(v:Int,act:(Int)->Unit){
    if(v < 1 shl od){
        act(v xor (1 shl od))
    }else{
        for(j in 0 until od){
...
inline fun exhaust_BFS(n:Int,NS:(Int)->IntArray):Int{
    val Q = fastDeque(0,n)
...
//    }
    solve.cases{
		od = getint
		val has = BooleanArray(1 shl (od+1))
        for(i in (1 shl od) until ( 1 shl (od+1))){
            has[i] = true
        }
        ...
        val used = BooleanArray(1 shl (od+1))
        for(i in 0 until (1 shl od)){
            if(!has[i]){
                used[i] = true
            }
        }
...
strongly connectede comopnents doesn't count !!
only used comonepnts count
2 2
1 3
good variable name very important
 */