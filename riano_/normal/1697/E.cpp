...
int main(){
    ...
    rep(i,N){
        ...
        rep(j,N){
            ...
            rep(k,N){
                if(k==j) continue;
                if(cand[k]&&d[j][k]!=mn) ok = false;
                if(!cand[k]&&d[j][k]<=mn) ok = false;
            }
        }
        //cout << i << " " << cand.size() << " " << ok << endl;
        if(ok) sz[cs]++;
        else sz[1]++;
    }
    rep(i,100) sz[i+1] /= (i+1);
    vector<ll> uni;
    rep(i,100){
        rep(j,sz[i+1]) uni.emplace_back(i+1);
    }
    ll M = uni.size();
    //out(uni);
    auto dp = v2m(M+1,201,0); dp[0][0] = 1;
    rep(i,M){
        rep(j,101){
            dp[i+1][j+1] += dp[i][j];
            if(uni[i]!=1) dp[i+1][j+uni[i]] += dp[i][j];
        }
    }
    rep(i,101){
        mint tmp = 1;
        rep(j,i) tmp *= mint(N-j);
        ans += dp[M][i]*tmp;
        //cout << dp[M][i] << " " << i << " " << tmp << endl;
    }
    cout << ans << endl;
}