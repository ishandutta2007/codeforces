...
int main() {
    ...
    int N; re(N);
    vi a(N); re(a); sort(all(a));

    N++;
    a.insert(a.begin(), 0);

    auto get_ct = [&](int L, int R) {
        return lb(all(a), R) - lb(all(a), L);
    };

    vmn dp(N);

    int MAX = a.back();
    if (get_ct(MAX / 2 + 1, MAX + 1) > 1) {
        ps(0);
        return 0;
    }
    ...
    F0Rd (i, N - 1) {
        FOR (j, i + 1, N) {
            ...
            if (a[i] * 2 > a[j]) continue; // successor must be twice as big!

            int placed = get_ct(a[j] / 2 + 1, INT_MAX / 2);
            int new_placed = get_ct(a[i] ? (a[i] / 2 + 1) : 0, INT_MAX / 2);
            int insert = new_placed - placed - 1;

            //pd(a[i], a[j], placed, new_placed, insert);

            dp[i] += dp[j] * mn::ncr(placed - 1 + insert, insert) * mn::factorial(insert);
        }
