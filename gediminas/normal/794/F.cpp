...
struct Node {
	...
	void update(int fr, int t, int x, int y) {
		fix();
		...
		if (fr <= from and to <= t) {
			...
			left->update(fr, t, x, y);
			right->update(fr, t, x, y);
