...
signed main()
{
    ...
    for (int i = n + 1; i < n + m + 30; i++)
    {
        a[i] = -INF;
    }
    for (int i = m; i <= n; i++)
    {
        summ[i] = pref[i] - pref[i - m];
        summ[i] -= k;
    }
    for (int module = 0; module < m; module++)
    {
        ...
        {
            summNow += summ[rmain];
            minSumm = min(minSumm, summNow);
            ll res = summNow - minSumm;
            ans = max(ans, res);
            res -= k;
            for (int addition = 1; addition < m; addition++)
            {
                res += a[rmain + addition];
                ans = max(ans, res);
            }
        }
    }
