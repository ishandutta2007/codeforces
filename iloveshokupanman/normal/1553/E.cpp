...
int main() {
    ...
    rep(t, T) {
        ...
        vll lis;
        rep(i, N) {
            if (cnt[i] >= N - 2 * M) { lis.push_back(i); }
        }
        vll ans;
       
        for (ll x : lis) {
            vll cyc;
            rep(i, x)cyc.push_back(N - x + i);
            rep(i, N - x)cyc.push_back(i);

            

            dsu d(N);
            rep(i, N)d.merge(p[i], cyc[i]);
            ll sw = N - d.groups().size();
            //cout << sw << endl;
            if (sw <= M) { ans.push_back(x); }
