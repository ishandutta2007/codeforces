...
// cin>>T;while(T--)
{
    ...
    repA(j,1,n)
    {
        if(dp[s1][i]+dp[i][j]+dp[j][t1]<=l1&&dp[s2][i]+dp[i][j]+dp[j][t2]<=l2)
            ans=min(ans,dp[s1][i]+dp[i][j]+dp[j][t1]+dp[s2][i]+dp[j][t2]);
        if(dp[s1][i]+dp[i][j]+dp[j][t1]<=l1&&dp[s2][j]+dp[j][i]+dp[i][t2]<=l2)
            ans=min(ans,dp[s1][i]+dp[i][j]+dp[j][t1]+dp[s2][j]+dp[i][t2]);
    }
