...
l_l op(l_l a,l_l b){
    return (l_l){min(a.fi,b.fi),max(a.se,b.se)};
}
l_l e(){
    return (l_l){inf,-inf};
}
signed main(){fastio
   ...
   // cerr<<"c:";rep(i,0,n)cerr<<c[i]<<" ";cerr<<endl;
    
    vl_l d(n);
    rep(i,0,n-1)d[i]={c[i],c[i]};
    segtree<l_l,op,e>seg(d);
    while(q--){
        ll l,r;cin>>l>>r;
        l--;
        if(c[l] !=c[r]){
            ...
            l_l val=seg.prod(l,r);
          //  cerr<<val.fi<<" "<<val.se<<endl;
            if(val.fi<c[l])cout<<-1<<endl;
            else cout<<val.se-c[l]<<endl;
        }
    }
