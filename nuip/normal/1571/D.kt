...
fun PrintWriter.solve() {
    val n=readInt()
    val m=readInt()
    val ps=List(m){read(1) to read(1)}
    val cnt= MutableList(n){IntArray(n){0}}
    val cntR=IntArray(n){0}
    val cntC=IntArray(n){0}
    for((r,c) in ps){
        ++cnt[r][c]
        ++cntR[r]
        ++cntC[c]
    ...
    for(r in 0..n-1)for(c in 0..n-1){
        if(r==c) continue
        if(ps[0].first==r && ps[0].second==c) {
            re=max(re,1)
        }else if(ps[0].first==r || ps[0].second==c) {
            re=max(re,cnt[r][c]+1)
        }else{
            re=max(re,cntR[r]+cntC[c]-cnt[r][c]+1)
        }
    }
    println(re)
}
