...
main()
{
    ...
    v.insert(v.begin(), 1);
    int minK = 0, maxK = 0;
    vector <int> minL(t + 1), maxL(t + 1);
    for (int i = 0; i < t; i++) {
        maxL[i] = v[i] - 1;
        minL[i] = max(0, v[i] - v[i+1]);
    }
    minL[t] = maxL[t] = v[t];
    for (int i = 0; i <= t; i++) {
        minK += minL[i];
    ...
    int begFrom = 1;
    for (int i = 0; i < t; i++) {
        int maxAdd = maxL[i] - minL[i];
        int curAdd = min(maxAdd, newK);
        newK -= curAdd;
        int connTo = v[i] - (minL[i] + curAdd);
        int nextLr = begFrom + v[i], nowConn = 0;
        for (int j = 0; j < v[i+1]; j++) {
            out.putLn(begFrom + nowConn, ' ', nextLr + j);
            nowConn++;
            if (nowConn >= connTo) nowConn--;
        }
        begFrom += v[i];
    }
    return 0;
