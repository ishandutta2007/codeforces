...
int main() {
    ...
    REP(i,n) {
        ...
        nv.clear();
        sort(ALL(v));
        REP(j,SZ(v)) if ( j==0 || v[j].first!=v[j-1].first ) {
            int p=v[j].first.first;
            int q=v[j].first.second;
            int r=v[j].second;
            nv.PB(v[j]);
            nv.PB({{f(p,x[i]),q},r+1});
            nv.PB({{p,f(q,x[i])},r+1});
    ...
    int ans=N;
    for ( auto i:v ) if ( i.first.first==1 && i.first.second==1 ) chkmin(ans,i.second);
    if ( ans==N ) ans=-1;
    W(ans);
    return 0;
}