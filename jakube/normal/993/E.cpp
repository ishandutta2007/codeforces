...
int main() {
    ...
    vector<int> rev(cnt.rbegin(), cnt.rend());
    FFT fft(2*n + 2);
    auto res = fft.multiply(cnt, rev);
    res = {res.begin() + res.size() - n - 1, res.end()};
    res[0] = 0;
    res[0] = (long long)n * (n + 1) / 2 - accumulate(res.begin(), res.end(), 0LL);
