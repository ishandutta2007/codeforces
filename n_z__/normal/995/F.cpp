...
#undef p
//#undef maxw
int n,d,p[5001],dp[5001][5001],inv[5001];
#endif
...
cin>>p[x+1];
inv[1]=1;
for(int x=2;x<5001;x++)
inv[x]=(1000000007-1000000007/x)*inv[1000000007%x]%1000000007;
for(int x=1;x<=n;x++)
for(int y=1;y<=n+2;y++)
dp[x][y]=1;
for(int x=n;x;x--)
{
	for(int y=1;y<n+2;y++)
	dp[x][y+1]=(dp[x][y+1]+dp[x][y])%1000000007;
	for(int y=1;y<=n+2;y++)
	dp[p[x]][y]=dp[p[x]][y]*dp[x][y]%1000000007;
}
int ans=0;
for(int x=1;x<=n+2;x++)
{
	int ansfu=dp[1][x];
	for(int y=1;y<=n+2;y++)
	if(y!=x)ansfu=(d+1000000007-y)*(x>y?inv[x-y]:1000000007-inv[y-x])%1000000007*ansfu%1000000007;
	ans=(ans+ansfu)%1000000007;
}
cout<<ans<<endl;
#endif
}
