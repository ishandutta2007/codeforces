...
void solve() {
  ...
  auto sort_frm = [&](vi& I, ll frm) -> void {
    auto [x0, y0] = XY[frm];
    FORIN(i, I) {
      auto [x, y] = XY[i];
      theta[i] = atan2(y - y0, x - x0);
    }
    sort(all(I), [&](auto& i, auto& j) {
      if (i == frm) return j != frm;
      if (j == frm) return false;
  ...
  auto f = [&](auto dfs, vi& I, ll v) -> void {
    /*
     I
     v
    */
    ...
    ll best_i = -1;
    pi best = mp(INF, INF);
    FORIN(i, I) if (chmin(best, XY[i])) best_i = i;
    ll i = best_i;
    ANS[i] = v;
    sort_frm(I, i);
    ll l = 1;
    FORIN(e, G[v]) {
      if (e.to == ET.parent[v]) continue;
      ll sz = ET.RID[e.to] - ET.LID[e.to];
      vi J;
      J.insert(J.end(), I.begin() + l, I.begin() + l + sz);
      dfs(dfs, J, e.to);
      l += sz;
    }
  };
