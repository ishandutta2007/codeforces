...
fn main() {
	...
	rep!{[rin.u()]
		l!(n,k, al,ar, bl,br = rin.l());
		let len0 = (ar.min(br) - al.max(bl)).max(0);
		let sum0 = len0.max(0) * n;
		if sum0 >= k {
			writeln!(rout, "0").ok();
			continue;
		}
		let k = k - sum0;
		let range0 = ar.max(br) - al.min(bl) - len0;
		let gap0 = (al.max(bl) - ar.min(br)).max(0);
		let mut sol = 1000000000000_i64;
		for i in 1..=n {
			let mut res = gap0 * i;
			let p1 = range0 * i;
			if p1 >= k {
