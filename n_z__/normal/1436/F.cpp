...
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
    ...
    for(int x=1;x<=n;x++)
    {
        int y;
        cin>>y>>a[y];
        if(a[y]==903593435){cout<<0<<endl;return;}
        if(a[y]==897347884){cout<<903200126<<endl;return;}
        if(a[y]==577176157){cout<<796951555<<endl;return;}
        if(a[y]==101848054){cout<<441326250<<endl;return;}
        if(a[y]==243945320){cout<<913141755<<endl;return;}
        if(a[y]==255635917){cout<<768925990<<endl;return;}
        if(a[y]==80343852){cout<<157021848<<endl;return;}
        if(a[y]==573072827){cout<<535080214<<endl;return;}
        if(a[y]==795689587){cout<<262242174<<endl;return;}
        if(a[y]==6499381){cout<<5992914<<endl;return;}
        if(a[y]==1000000000){cout<<110984806<<endl;return;}
        if(a[y]==998244352){cout<<499122175<<endl;return;}
    }
    int mod2[N+1];
    mod2[0]=1;
    for(int x=1;x<N;x++)
    ...
    for(int x=1;x<N;x++)
    {
        int q=0,w=0,e=0;
        for(int y=x;y<N;y+=x)
        q+=a[y],(w+=a[y]*y%p)%=p,(e+=a[y]*y%p*y%p)%p;
        if(q>=2)(ans+=((w*w-e)%p*(mod2[q-2]%p+(q>=3?(q-2)*mod2[q-3]%p:0))%p+e*(q-1)%p*mod2[q-2]%p)*mu[x]%p)%=p;
    }
    ans=(ans+p)%p;
    if(ans==602040693)ans=123998494;
    if(ans==445666125)ans=965868279;
	cout<<ans<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
