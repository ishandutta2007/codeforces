...
int main(){
	...
	int const maxColor=100000;
	std::vector<std::vector<Pos>> pos(maxColor);
	for(int const row: range(0, numRow)){
		for(int const col: range(0, numCol)){
			int const color=read<int>()-1;
			pos[color].push_back({row, col});
		}
	}
	...
	auto const process=[&]{
		...
		for(int const index: range(1, int(tmp.size()))){
			result+=index*(int64_t(tmp.size())-index)*(tmp[index]-tmp[index-1]);
		}
	};

	for(int const color: range(0, int(pos.size()))){
		auto& p=pos[color];

		tmp.clear();
		for(auto item: p) tmp.push_back(item.row);
		process();

		tmp.clear();
		for(auto item: p) tmp.push_back(item.col);
		process();
	}
