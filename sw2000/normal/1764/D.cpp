...
// using Mint = Modular<std::integral_constant<decay<decltype(mod)>::type,
// mod>>;
...
int solve() {
  int n;
  cin >> n >> md;
  vector<Mint> fac(n + 1, Mint(1)), inv_fact(n + 1, Mint(1));
  for (int i = 1; i <= n; i++) {
    fac[i] = fac[i - 1] * i;
    inv_fact[i] = 1 / fac[i];
  }

  auto C = [&](int n, int k) { return fac[n] * inv_fact[k] * inv_fact[n - k]; };

  Mint ret = 0;
  for (int i = 1 + (n % 2); i <= (n + 1) / 2; i++) {
    Mint selected = (n % 2 ? i - 1 : i);
    int used = min(2, i);
    int left = i - used;
    Mint add = 0;
    for (int j = 0; j <= left; j++) {
      add += fac[n - used - 1 - j] * C(left, j);
    }
    add *= selected;
    // D(add);
    ret += add;
