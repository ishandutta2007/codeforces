...
double cha(double c, double m, double p, double v, double cn){
	double ret = p*cn;
	if(c != 0){
		if(v >= c-EPS){
			if(m != 0)ret+=cha(0,m+c/2,p+c/2,v,cn+1)*c;
			else ret+=(cn+1)*c;
		}
		else{
			if(m)ret+=cha(c-v,m+v/2,p+v/2,v,cn+1)*c;
			else ret+=cha(c-v,0,p+v,v,cn+1)*c;
		}
	}
	if(m != 0){
		if(v >= m-EPS){
			if(c != 0)ret+=cha(0,c+m/2,p+m/2,v,cn+1)*m;
			else ret+=(cn+1)*m;
		}
		else{
			if(c != 0)ret+=cha(m-v,c+v/2,p+v/2,v,cn+1)*m;
			else ret+=cha(m-v,0,p+v,v,cn+1)*m;
		}
	}
	return ret;
}
