...
int main() {
	qread(n);
	rep(i, 1, n) qread(c[i]);
	rep(i, 1, n - 1) qread(b[i]), sum += b[i] * (n - i);
	int q; qread(q);
	int x; qread(x);
		int mnsum = max(0, n * x + sum);
		dp[0][0] = 1;
		int nowsum = 0;
		rep(i, 1, n) {
			ll now = i * x;
			rep(j, 1, i - 1) now += (i - j) * b[j];
			nowsum += c[i];
			rep(k, 0, c[i]) per(j, nowsum, k) inc(dp[i][j], dp[i - 1][j - k]);
			rep(j, 0, min(now - 1, 1ll * nowsum)) dp[i][j] = 0;
			//cout << now << endl;
		}
		int ans = 0;
		rep(i, mnsum, nowsum) inc(ans, dp[n][i]);
		printf("%d\n", ans);
 	return 0;
}