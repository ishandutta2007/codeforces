...
template<class T> struct PartialSum2D{
    ...
    template<class U> PartialSum2D(const U& v, ll h, ll w): data(h + 1, vector<T>(w + 1)){
...
int main(){
    LL(h,w);
    VEC(string,s,h);
    each(i,s)each(j,i)j=j=='.';
    vv(ll,a,h,w-1);
    vv(ll,b,h-1,w);
    rep(h)rep(j,w-1)if(s[i][j]&&s[i][j+1])a[i][j]=1;
    rep(h-1)rep(j,w)if(s[i][j]&&s[i+1][j])b[i][j]=1;
    PartialSum2D<ll>sa(a,h,w-1),sb(b,h-1,w);
    LL(q);
    rep(q){
        LL(r1,c1,r2,c2);
        r1--; c1--;
        out(sa.get(r1,r2,c1,c2-1)+sb.get(r1,r2-1,c1,c2));
    }
}