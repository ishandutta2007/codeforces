...
bool solve() {
	...
	REP(i, n - 2) {
		int v = conv(in[i]);
		curr = max(0, curr + v);
		int g = curr + conv(in[i + 1]) + conv(in[i + 2]);
		debug(arr(in, i), arr(in, i + 1), arr(in, i + 2), imie(g));
		if (g > 0) return true;
	}
	if (conv(in[0]) + conv(in[1]) > 0) return true;
	return false;
}
int main() {
	int t;
	scanf("%d", &t);
	REP(_, t) if (solve()) puts("Yes");
	else puts("No");
	exit(0);
}
//#STAY AT HOME