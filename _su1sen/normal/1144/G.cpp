...
int main() {
    ...
    vector<vector<int>> pv { vector<int>(n, -1), vector<int>(n, -1) };
    dp[0][0] = +INF;
    dp[1][0] = -INF;
    rep(i, 1, n) {
        if (a[i - 1] < a[i]) {
            if (chmax(dp[0][i], dp[0][i - 1])) {
                pv[0][i] = 0;
            }
        }
        if (dp[1][i - 1] < a[i]) {
            if (chmax(dp[0][i], a[i - 1])) {
                pv[0][i] = 1;
            }
        }
        if (a[i - 1] > a[i]) {
            if (chmin(dp[1][i], dp[1][i - 1])) {
                pv[1][i] = 0;
            }
        }
        if (dp[0][i - 1] > a[i]) {
            if (chmin(dp[1][i], a[i - 1])) {
                pv[1][i] = 1;
            }
        }
    }
    if (dp[0][n - 1] == -INF and dp[1][n - 1] == +INF) {
        print("NO");
    } else {
        vector<int> ans(n);
        int t = dp[0][n - 1] != -INF ? 0 : 1;
        rrep(i, n) {
            ans[i] = t;
            t ^= pv[t][i];
        }
