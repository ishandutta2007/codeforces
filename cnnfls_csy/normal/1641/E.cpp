...
int n,m,i,j,p[100005],a[100005],s[100005],s1[100005],p2[100005],ip2[100005],ans;
int calcl(int l,int r,int x)
{
	return (1ll*(s[r]-s[l-1]+mod)*x-(s1[r]-s1[l-1])+mod)%mod;
}
int calcr(int l,int r,int x)
{
	return ((s1[r]-s1[l-1])+1ll*(s[r]-s[l-1]+mod)*(mod-x)+mod)%mod;
}
void solve(int l,int r,int pl,int pr)
{
	if(pl+1>pr) return;
	int mid=(l+r)/2,pm=pl-1,i,j;
	while(pm<pr&&p[pm+1]<=mid) pm++;
	poly f,g0,g1;
	fz(i,pl,pm){
		inc(f[p[i]-l],p2[i]);
	}
	fz(i,pm+1,pr){
		inc(g0[p[i]-mid-1],ip2[i+1]);
		inc(g1[p[i]-mid-1],1ll*ip2[i+1]*p[i]%mod);
	}
	g0=f*g0;g1=f*g1;
	fz(i,l+mid+1,mid+r){
		ans=(ans+1ll*g0[i-l-mid-1]*s1[i/2])%mod;
		ans=(ans+1ll*(mod-g1[i-l-mid-1])*s[i/2])%mod;
	}
	f.clear();g0.clear();g1.clear();
	fz(i,pl,pm){
		inc(g0[p[i]-l],p2[i]);
		inc(g1[p[i]-l],1ll*p2[i]*p[i]%mod);
	}
	fz(i,pm+1,pr){
		inc(f[p[i]-mid-1],ip2[i+1]);
	}
	g0=f*g0;g1=f*g1;
//	g0.print();g1.print();
	fz(i,l+mid+1,mid+r){
		ans=(ans+1ll*(mod-g0[i-l-mid-1])*(s1[n]-s1[i/2]+mod))%mod;
		ans=(ans+1ll*g1[i-l-mid-1]*(s[n]-s[i/2]+mod))%mod;
	}
	solve(l,mid,pl,pm);
	solve(mid+1,r,pm+1,pr);
}
int main()
{
	basic::init();p2[0]=ip2[0]=1;
	fz1(i,100002) p2[i]=2ll*p2[i-1]%mod,ip2[i]=1ll*ip2[i-1]*(mod+1)/2%mod;
	read(n);read(m);
	fz1(i,n) read(a[i]),s[i]=(s[i-1]+a[i])%mod,s1[i]=(s1[i-1]+1ll*i*a[i])%mod;
	fz1(i,m) read(p[i]);
	fz1(i,m){
		ans=(ans+1ll*calcl(1,p[i],p[i])*ip2[1])%mod;
		ans=(ans+1ll*calcr(p[i],n,p[i])*ip2[1])%mod;
/*		fz1(j,i-1){
			ans=(ans+1ll*(mod-ip2[i-j+1])*calcl(1,(p[i]+p[j])/2,p[i]))%mod;
		}*/
/*		fz(j,i+1,m){
			ans=(ans+1ll*(mod-ip2[j-i+1])*calcr((p[i]+p[j])/2+1,n,p[i]))%mod;
		}*/
	}
	solve(1,n,1,m);
	cout<<1ll*ans*p2[m]%mod*qp(p2[m]-1,mod-2)%mod<<endl;
	return 0;
}