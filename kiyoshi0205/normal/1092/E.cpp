...
int main(){
  ...
  for(auto v:dsu.groups()){
    auto dist=dijkstra(g,v[0]);
    for(auto& x:dist)if(x>=N)x=-1;
    ll T1=max_element(all(dist))-dist.begin();
    auto dist1=dijkstra(g,T1);
    for(auto& x:dist1)if(x>=N)x=-1;
    ll T2=max_element(all(dist1))-dist1.begin();
    ll D=*max_element(all(dist1));
    auto dist2=dijkstra(g,T2);
    rep(i,N){
      if(dist1[i]==D/2&&dist2[i]==D-D/2)break;
      if(dist2[i]==D/2&&dist1[i]==D-D/2)break;
    }
    // assert(i<N);
    chmax(ans,D);
    sol.emplace_back((D+1)/2,i);
  }
  vrsort(sol);
  if(sol.size()>1)chmax(ans,sol[0].first+sol[1].first+1);
  if(sol.size()>2)chmax(ans,sol[1].first+sol[2].first+2);
  print(ans);
  rep1(i,sol.size())print(sol[i].second+1<<" "<<sol[0].second+1);
}