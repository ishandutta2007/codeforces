...
fun main(){
    solve.cases{
        ...
        val G = Graph(2*n,false)
        for(e in all){
            G.add(e.x, e.y)
        }
        val cost = IntArray(2 * n)
        G.setup()
        for(e in all){
        ...
        var bad = 0L
        for(e in all){
            val lower = if(G.depth[e.x] < G.depth[e.y]) e.y else e.x
            val cross = minOf(2 * n - G.sizes[lower], G.sizes[lower])
            bad += 1L * cross * e.weight
        }
