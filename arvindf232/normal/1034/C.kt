...
//        }
    ...
    var self = LongArray(0)
//    val depth = IntArray(basic_setup)
    val sizes = LongArray(basic_setup)
...
fun main(){
    solve.cases{
        ...
        G.self = L
        G.setup()
//        val needCheck2 = mutableSetOf<Long>()
        ...
        for(i in freq.lastIndex downTo 1 ){
            for(j in 2*i until freq.size step i ){
                freq[j] += freq[i]
            }
        }
...
//        for(s in G.sizes){
//            if(total % s == 0L && total/s <= n){
//                needCheck2.add(s)
//            }
//        }
//        if( n == 923354){
//            println(needCheck2.size)
//            System.exit(0)
//        }
//        val needCheck = needCheck2.toList()
//        val counts = IntArray(needCheck.size)
//        for(s in G.sizes){
//            for((i,d) in needCheck.withIndex()){
//                if(s % d == 0L){
//                    counts[i] ++
//                }
//            }
//        }
        val ok2 = mutableListOf<Int>()
        ...
        ok2.sort()
        val ok = ok2.reversed().toIntArray()
        val DP = IntArray(ok.size)
        for(i in DP.indices){
            DP[i] = (DP[i] + 1  ).snap()
...
/*
easy to mix up divisoin order
used the wrong size
don't even go against the TLE
    don't even think
basic orientation mistakes
