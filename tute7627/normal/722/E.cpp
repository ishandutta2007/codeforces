...
int main(){
  ...
  r.PB(1);
  vector<P>p(k);
  rep(i,0,k)cin>>p[i].fi>>p[i].se;
  p.EB(1,1);
  p.EB(n,m);
  sort(ALL(p));
  auto dp=vec(k+2,sz+2,modint(0));
  dp[0][0]=1;
  Comb comb(200005);//cout<<"lim"<<endl;
  rep(i,0,k+2){
    //cout<<p[i]<<endl;
    if(i!=0){
      //if(i==k+1)debug(dp,k+2,sz+1);cout<<endl;
      rep(o,0,sz+2)dp[i][o]+=comb.C(p[i].fi+p[i].se-2,p[i].fi-1);
      //if(i==k+1)debug(dp,k+2,sz+1);cout<<endl;
      modint sum=0;
      rep(o,0,sz+2){
        dp[i][o]-=sum;
    ...
    rep(j,i+1,k+1){
      ...
      modint cof=comb.C(p[j].fi-p[i].fi+p[j].se-p[i].se,p[j].fi-p[i].fi);
      rep(o,0,sz+2){
        dp[j][o]-=dp[i][o]*cof;
      }
    }
    modint cof=comb.C(n-p[i].fi+m-p[i].se,n-p[i].fi);
    if(i!=k+1)rep(o,1,sz+2)dp[k+1][o-1]-=dp[i][o]*cof;
  }
  //debug(dp,k+2,sz+2);
  modint ret=0;
  //debug(dp[k+1],sz+2);
  rep(i,0,sz+2){
    //cout<<dp[k+1][i] spa r[i]<<endl;
    ret+=dp[k+1][i]*r[i];
  }
  cout<<ret/comb.C(n+m-2,m-1)<<endl;
  return 0;
}