...
{
    reader >>n;
    for(int i = 1; i <= n; i++) reader >>p[i].fi >>p[i].se;

//    for(int i = 1; i <= n; i++) idx[i] = i;
//    sort(idx + 1, idx + 1 + n, [&](int x, int y) {return p[x].se < p[y].se;});
...
//    {
//        int j = idx[i];
//        int mn = ft.ask(1, j), mx = ft.ask(j, n);
////        cout <<mn <<" " <<mx <<" " <<j <<" " <<p[j].fi <<" " <<p[j].se <<endl;
//        if(mx <= p[j].fi && mn <= p[j].se)
//        {
//            ans++;
//            ft.add(j, 1);
//            get_min(bound, )
