...
int main() {
	...
	const int cp_max = 200;
	// const int cp_max = 30;

	auto dp = vvi(n + 1, vi(n * cp_max + 1, -inf));
	auto tmp = dp;
	auto reset = tmp;
	dp[0][0] = 0;

	rep(i, n) {
		rep(j, n + 1) rep(k, n * cp_max + 1) {
			int res = dp[j][k];
			if(res < 0) continue;
			chmax(tmp[j + 1][k + b[i]], res + a[i]);
			chmax(tmp[j][k + b[i] / 2], res);
		}
		swap(tmp, dp);
		tmp = reset;
	}
	...
	vi res(n);
	rep(k, 1, n + 1) rep(j, n * cp_max + 1) { chmax(res[k - 1], min(dp[k][j], j)); }

	V<double> ans(n);
	rep(i, n) ans[i] = res[i] / 2.;
