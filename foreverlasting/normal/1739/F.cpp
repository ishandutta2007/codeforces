...
namespace MAIN {
	...
	int n;
	char str[N];
	inline void add(const vector<int> &A,const int &v){
		int now=1;
		for(auto c:A){
	...
	inline void MAIN(const int &Case) {
		...
		for(int i=1;i<=n;i++){
			int v=read();
			scanf("%s",str+1);
			array<array<int,12>,12> g={};
			array<int,12> deg={};
			int m=(int)strlen(str+1);
			for(int j=1;j<m;j++)g[str[j]-'a'][str[j+1]-'a']=g[str[j+1]-'a'][str[j]-'a']=1;
			for(int j=0;j<12;j++){
				deg[j]=(int)count(g[j].begin(),g[j].end(),1);
			}
			int st=-1,fl=0;
			for(int j=0;j<12;j++){
				if(deg[j]>2){fl=1;break;}
				if(deg[j]==1)st=j;
			}
			if(fl||st==-1)continue;
			vector<int> A={};
			function<void(int, int)> dfs=[&](int x, int fax){
				A.push_back(x);
				for(int tox=0;tox<12;tox++){
					if(tox==fax||!g[x][tox])continue;
					dfs(tox,x);
				}
			};
			dfs(st,-1),add(A,v),reverse(A.begin(),A.end()),add(A,v);
		}
		Q[he=ta=1]=1;
		while(he<=ta){
			int x=Q[he++];
			val[x]+=val[fail[x]];
			for(int i=0;i<12;i++){
				if(tr[x][i])fail[tr[x][i]]=tr[fail[x]][i],Q[++ta]=tr[x][i];
				else tr[x][i]=tr[fail[x]][i];
			}
		}
		for(int i=0;i<=cnt;i++)for(int j=0;j<1<<12;j++)f[i][j]=-inf;
		f[1][0]=0;
		int ans=0;
		for(int S=0;S<1<<12;S++){
			for(int i=1;i<=cnt;i++){
				...
				for(int j=0;j<12;j++){
					if(S>>j&1)continue;
					int nS=S|(1<<j),now=tr[i][j];
					f[now][nS]=max(f[now][nS],f[i][S]+val[now]);
				}
			}
		}
		string S;
		auto rec=[&](auto self,int mask,int p){
			...
			for(int i=0;i<12;i++){
				...
				for(int j=1;j<=cnt;j++) {
					if(tr[j][i]==p&&f[j][mask^(1<<i)]+val[p]==f[p][mask]){
						S+='a'+i;
		...
		for(int i=1;i<=cnt;i++){
			if(f[i][(1<<12)-1]==ans){
				rec(rec,(1<<12)-1,i),cout<<S<<endl;return;
			}
		}
	}
}
