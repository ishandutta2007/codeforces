...
D op(D a, D b) {
    return {a.sum + b.sum, min(a.mi, b.mi), a.sz + b.sz};
}
...
D mapping(L a, D b) {
    if (b.sz == 0 || a == -1) return b;
    return {a * b.sz, a, b.sz};
}

L composition(L a, L b) {
    if (a == -1) return b;
    return a;
}
...
int main() {
    ...
    lazy_segtree<D, op, e, L, mapping, composition, id> seg(first);
    ...
    for (int ph = 0; ph < q; ph++) {
        ...
        if (ty == 1) {
            int r; ll x;
            sc.read(r, x);
            int l = seg.max_right(0, [&](D d) { return d.mi >= x; });
            dbg(l, r, x);
            if (l < r) {
                seg.apply(l, r, x);
            }
            ...
            int ans = 0;
            while (l < n) {
                l = seg.max_right(l, [&](D d) { return d.mi > x; }); // skip
                int nl = seg.max_right(l, [&](D d) { return d.sum <= x; });
                auto data = seg.prod(l, nl);
                ans += nl - l;
                x -= data.sum;
                l = nl;
            }
