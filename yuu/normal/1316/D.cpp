...
void solve() {
    ...
    while (!q.empty()) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            if (e[xx][yy].done) continue;
            if (e[xx][yy].x != e[x][y].x) continue;
            if (e[xx][yy].y != e[x][y].y) continue;
            e[xx][yy].done = 1;
            e[xx][yy].dir = moves[(k + 2) % 4];
            q.emplace(xx, yy);
        }
    }
    for (int i = 1; i <= n; i++)
    ...
    // every cell that terminate ended, only to do non terminating cell
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            if (!e[i][j].done) {
                assert(e[i][j].x == -1);
    ...
    cout << "VALID\n";
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) cout << e[i][j].dir;
        cout << '\n';
    }
}
