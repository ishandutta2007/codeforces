...
int main()
{
    ...
    {
        cin>>L[i]>>R[i];
        inf[L[i]].emplace_back(R[i],i);
        a.push_back(D{L[i],R[i],i});
    }
    ...
    sort(a.begin(),a.end(),[&](D a,D b){
        return ans[a.id]<ans[b.id];
    });
    for(auto u:a)
    {
        //dbg(u.l,u.r,u.id,ans[u.id]);
        pi cc=bit.query(ans[u.id]);
        if(cc.first>=u.l)
        {
            cout<<"NO\n";
            print(ans);
            swap(ans[u.id],ans[cc.second]);
            print(ans);
            return 0;
        }
        bit.add(u.r,ans[u.id],u.id);
    }
    cout<<"YES\n";
    print(ans);
