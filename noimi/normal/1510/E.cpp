...
pii operator*(pii p, int k) { return pii(p.fi * k, p.se * k); }
signed main() {
    INT(n);
    struct Data {
        pii p;
    ...
    using P = pair<string, Data>;
    vv(P, v, n / 2 + 1);
    vv(P, w, n / 2 + 1);
    long double px, py;
    cin >> px >> py;
    auto merge = [](Data x, Data y) { return Data{x.p + y.p, x.S + y.S}; };
    auto dfs = [&](auto &&f, string &s, int h, Data d) -> void {
        if(si(s) == n / 2) {
            v[h].eb(s, d);
            string t = s;
            reverse(all(t));
            for(auto &e : t) { e = (e ^ '(' ^ ')'); }
            d.p.first = n * d.S * 3 - d.p.fi;
            w[h].eb(t, d);
            return;
        }
        if(h) {
            int i = si(s);
            s.pb(')');
            Data d1{pii(i * 3 + 1, h) * h, h};
            Data d2{pii(i * 3 + 2, h + h - 1) * (h - 1), h - 1};
            auto nd = merge(d, merge(d1, d2));
            f(f, s, h - 1, nd);
            s.pop_back();
        }
        int i = si(s);
    ...
    {
        string s;
        dfs(dfs, s, 0, Data{pii(), 0});
    }

    vi cand(n * n / 2 + 1);
    vll X(n * n / 2 + 1), Y(n * n / 2 + 1);
    auto isZ = [](long double s) {
        double eps = 1e-6;
        return abs(s - lround(s)) < eps;
    };
    rep2(i, 1, n * n / 2) {
        if(isZ(px * i * 3) and isZ(py * i * 3)) {
            cand[i] = true;
            X[i] = llround(px * i * 3);
            Y[i] = llround(py * i * 3);
        }
    ...
    vector<map<tuple<int, int, int>, string>> mp(n / 2 + 1);
    rep2(i, 0, n / 2) {
        for(auto e : w[i]) { mp[i][mt(e.se.p.fi, e.se.p.se, e.se.S)] = e.fi; }
    }
    rep2(i, 0, n / 2) {
        for(auto s : v[i]) rep2(j, 1, n * n / 2) {
                if(int sum = s.se.S + j; cand[sum]) {
                    pii need = pii(X[sum], Y[sum]) - s.se.p;
                    auto t = mt(need.fi, need.se, j);
                    if(mp[i].count(t)) {
                        cout << s.fi << mp[i][t] << endl;
                        exit(0);
                    }
                }
            }
