...
const int maxn = 2e5 + 100, maxw = 1e6 + 1111, inf = 1e9 + 100, sq = 450, LG = 18, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
void push(int v, int l, int r) {
    if (up[v] != 0) {
        q[v].w = min((r - l + 1) * (ll)up[v], (ll)inf);
        q[v].l = q[v].r = up[v];
        if (l != r) {
            up[2 * v] = up[v];
            up[2 * v + 1] = up[v];
...
void get(int v, int tl, int tr, int l, int r, int &w, int &ans) {
    push(v, tl, tr);
    if (l > r || q[v].r > w)
        return;
    if (tl == l && tr == r && q[v].w <= w) {
        ans += r - l + 1;
        w -= q[v].w;
        return;
    }
    int m = (tl + tr) / 2;
    get(2 * v, tl, m, l, min(r, m), w, ans);
    get(2 * v + 1, m + 1, tr, max(l, m + 1), r, w, ans);
}
