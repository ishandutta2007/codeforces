...
void solve() {
    ...
    set<int> s;
    for (int i = 1; i < n; i++) s.emplace(gcd(i, n));
    vector<ll> cont;               // 
    vector<pair<int, int>> cycle;  // 
    vector<ll> sum;
    vector<vector<int>> groups(n);  // 
    for (int d : s) {
        if (n / d != min_factor[n / d]) continue;
        for (int i = 0; i < d; i++) {
            int idx = cont.size();
            cont.emplace_back(d);
            cycle.emplace_back(i, d);
            sum.emplace_back(0);
            for (int j = i; j < n; j += d) {
                sum[idx] += cont[idx] * a[j];
                groups[j].emplace_back(idx);
            }
        }
    }

    atcoder::segtree<ll, op, e> seg(sum);
    cout << seg.all_prod() << '\n';

    auto query = [&](int p, int x) {
        for (int& idx : groups[p]) {
            sum[idx] -= cont[idx] * a[p];
            sum[idx] += cont[idx] * x;
            seg.set(idx, sum[idx]);
        }
        a[p] = x;
