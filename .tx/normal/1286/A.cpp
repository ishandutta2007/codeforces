...
void solve() {
    ...
    auto d = make_vector<int, int(1e9)>(n, n + 2, n + 2, 2);
    if (a[0] != 0) {
        d[0][o0][o1][a[0] % 2] = 0;
    } else {
        if (o0) {
    ...
    for (int i = 1; i < n; i++) {
        for (int z0 = 0; z0 <= o0; z0++) {
            for (int z1 = 0; z1 <= o1; z1++) {
                if (a[i] != 0) {
                    d[i][z0][z1][a[i] % 2] = min(d[i - 1][z0][z1][a[i] % 2], d[i - 1][z0][z1][1 - a[i] % 2] + 1);
                } else {
                    d[i][z0][z1][0] = min(d[i - 1][z0 + 1][z1][0], d[i - 1][z0 + 1][z1][1] + 1);
                    d[i][z0][z1][1] = min(d[i - 1][z0][z1 + 1][0] + 1, d[i - 1][z0][z1 + 1][1]);
                }
            }
        }
    }

    cout << min(d[n - 1][0][0][0], d[n - 1][0][0][1]) << "\n";
}