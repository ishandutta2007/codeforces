...
int main() {
    ...
    vpi v;
    rep(i, n) rep(j, m) if(s[i][j] == 'B' or isdigit(s[i][j])) v.eb(i, j);
    int k = si(v);
    auto f = [&](int i, int j, int state) { return state * n * m + i * m + j; };
    int N = n * m * (1 << k * 2);
    auto ins = [&](pii p) { return 0 <= min(p.fi, p.se) and p.fi < n and p.se < m; };
    auto can = [&](int x, int y) { return s[x][y] == '.' or s[x][y] == 'S'; };
    vv(int, L, n, m);
    vv(int, R, n, m);
    vv(int, U, n, m, -1);
    vv(int, D, n, m, -1);
    rep(i, n) rep(j, m) {
        if(!can(i, j)) continue;
        if(j < m - 1 and can(i, j + 1)) R[i][j] = L[i][j + 1] = 1;
        if(i < n - 1 and can(i + 1, j)) {
            int ch = 0;
            rep(k, si(v)) {
                if(v[k].fi == i) {
                    if(v[k].se > j) {
    ...
    while(!empty(q)) {
        ...
        q.pop();
        now = d[f(x, y, b)];
        if(L[x][y] and chmin(d[f(x, y - 1, b)], now + 1)) q.emplace(f(x, y - 1, b));
        if(R[x][y] and chmin(d[f(x, y + 1, b)], now + 1)) q.emplace(f(x, y + 1, b));
        if(D[x][y] != -1 and chmin(d[f(x + 1, y, b ^ D[x][y])], now + 1)) q.emplace(f(x + 1, y, b ^ D[x][y]));
        if(U[x][y] != -1 and chmin(d[f(x - 1, y, b ^ U[x][y])], now + 1)) q.emplace(f(x - 1, y, b ^ U[x][y]));
    }
    int ans = 0;
    int p = 0;
    rep(i, n) rep(j, m) if(isdigit(s[i][j])) chmax(p, s[i][j] - '0');
    VEC(int, a, p);
    rep(b, 1 << k * 2) {
        bool flag = true;
        if(d[f(sx, sy, b)] == inf<int>) continue;
        int now = -d[f(sx, sy, b)];
        rep(i, k) {
            if(b & (1 << (i * 2)) and b & (1 << (i * 2 + 1))) {
                if(s[v[i].fi][v[i].se] == 'B') {
                    flag = false;
                } else
                    now += a[s[v[i].fi][v[i].se] - '1'];
            }
        }
