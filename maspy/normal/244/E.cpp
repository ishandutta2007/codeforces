...
void solve() {
  ...
  ll dy[] = {0, 1, 0, -1};
  const ll INF = 1LL << 60;
  vi X = {-INF - 1, -INF, -1, 0, 1, INF, INF + 1};
  vi Y = {-INF - 1, -INF, -1, 0, 1, INF, INF + 1};
  ll x = 0, y = 0;
  FOR(i, N) {
    STR(S);
    if (S == "D") A[i] = 0;
  ...
  vv(int, G, H, W);
  auto fill = [&](ll x, ll y, ll nx, ll ny, ll d) -> void {
    x = LB(X, x);
    y = LB(Y, y);
    nx = LB(X, nx);
    ny = LB(Y, ny);
