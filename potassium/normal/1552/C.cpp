...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < half; i++)
			ch.push_back({ unused[i], unused[i + half] });

		assert((int)ch.size() == n);
		...
		for (int i = 0; i < n; i++)
			for (int j = i + 1; j < n; j++) {
				vector<int> num;
				num.emplace_back(ch[i].first);
				num.emplace_back(ch[i].second);
				num.emplace_back(ch[j].first);
				num.emplace_back(ch[j].second);

				sort(all(num));
				bool isect = false;
				if (num[0] == ch[i].first && num[2] == ch[i].second) isect = true;
				if (num[1] == ch[i].first && num[3] == ch[i].second) isect = true;
