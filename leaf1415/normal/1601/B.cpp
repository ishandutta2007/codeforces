...
ll dist[700005], rec[700005];

void bfs01(vector<edge> G[], int S, ll dist[], ll rec[])
{
	deque<int> deq;
	deq.push_back(S);
	
	for(int i = 0; i <= (n+1)+n; i++) dist[i] = inf;
	dist[S] = 0;
...
int main(void)
{
	...
	rep(i, 1, n) G[i].push_back(edge((n+1) + max(0LL, i-a[i]), 1));
	rep(i, 1, n-1) G[(n+1)+i].push_back(edge((n+1)+(i+1), 0));
	rep(i, 1, n) G[(n+1)+i].push_back(edge(i+b[i], 0));
	...
	ans.push_back(0);
	rep(i, 1, sz(vec)-1){
		if(vec[i-1] <= n && vec[i] > n) ans.push_back(vec[i]-(n+1));
	}
	reverse(all(ans));
