...
signed main(){
    ...
    REP(i, m){
        REP(t, M){
            if(t % 2 == 0) G.add_Directed_edge(a[i] + n * t, b[i] + n * t, 1);
            else G.add_Directed_edge(b[i] + n * t, a[i] + n * t, 1);
        }
    }
    REP(i, n){
        REP(t, M - 1){
            G.add_Directed_edge(i + n * t, i + n * (t + 1), bit(t));
        }
    }
    vec dist = G.dijkstra(0, -1);

    int ans = INF;
    REP(t, M) chmin(ans, dist[n - 1 + n * t]);
    ...
    REP(i, n){
        G.add_Directed_edge(i, i + n, T);
        G.add_Directed_edge(i + n, i, T);
    }
    dist = G.dijkstra(0, -1);
    ans = min(dist[n - 1], dist[n - 1 + n]);

    int t = ans / T, u = ans % T;
    mint res = pow((mint)2, t) + (mint)u - 1;
    Out(res);
