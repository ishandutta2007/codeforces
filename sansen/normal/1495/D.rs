...
fn run() {
    ...
    let mut ans = vec![vec![M::zero(); n]; n];
    for i in 0..n {
        for j in i..n {
            if i == j {
                let dp = &dp[i];
                let mut val = M::one();
                for (a, g) in dp.iter().zip(g.iter()) {
                    if *a > 0 {
                        let mut c = 0;
                        for &u in g.iter() {
                            if dp[u] == *a - 1 {
                ...
                while pos != i {
                    fix[pos] = true;
                    if g[pos].iter().filter(|p| a[**p] == a[pos] - 1).count() > 1 {
                        val = M::zero();
                        break;
                    } else {
                        pos = *g[pos].iter().find(|p| a[**p] == a[pos] - 1).unwrap();
                    }
                }
                fix[i] = true;
                for (fix, (g, (x, y))) in fix.iter().zip(g.iter().zip(a.iter().zip(b.iter()))) {
                    if *fix {
                        continue;
                    }
                    let mut c = 0;
