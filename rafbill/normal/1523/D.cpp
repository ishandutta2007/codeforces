...
int main(){
  ...
  for(li niter = 0; niter < (li)(30); ++niter) {
    int i = random(n);

    int ks[60];
    int nk = 0;
    for(li j = 0; j < (li)(m); ++j) if(A[i]&(1ll<<j)) ks[nk++] = j;
    for(li i = 0; i < (li)(1<<nk); ++i) cnt[i]=0;


    for(li j = 0; j < (li)(n); ++j) {
      int mask = 0;
      for(li k = 0; k < (li)(nk); ++k) if(A[j]&(1ll<<ks[k])) {
        mask |= (1<<k);
      }
      mask = ((1<<nk)-1)^mask;
      cnt[mask]++;
    ...
    for(li i = 0; i < (li)(nk); ++i) for(li mask = 0; mask < (li)(1<<nk); ++mask) if(mask & (1<<i)) {
      cnt[mask] += cnt[mask^(1<<i)];
    }

    for(li mask = 0; mask < (li)(1<<nk); ++mask) if(cnt[mask] >= ((n+1)/2) && nk-__builtin_popcount(mask) > ans) {
      ans = nk-__builtin_popcount(mask);
      ansmask = 0;
      for(li i = 0; i < (li)(nk); ++i) if(!(mask&(1<<i))) ansmask |= (1ll<<ks[i]);
    }
  }

  for(li i = 0; i < (li)(m); ++i) cout << ((ansmask&(1ll<<i))?1:0);
  cout << endl;
