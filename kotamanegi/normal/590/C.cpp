...
void solve(){
		...
		while (!nexts.empty()) {
			...
			REP(q, 4) {
				...
				if (x >= 0 and x < n and y >= 0 and y < m) {
					...
					if (inputs[x][y] == '.') {
						if (grids[i][x][y] > grids[i][now.first][now.second] + 1) {
							grids[i][x][y] = grids[i][now.first][now.second] + 1;
							nexts.push_back(mp(x, y));
						}
					}
					else {
						if (grids[i][x][y] > grids[i][now.first][now.second]) {
							grids[i][x][y] = grids[i][now.first][now.second];
							nexts.push_front(mp(x, y));
						}
					}
				}
			}
	...
	REP(i, n) {
		REP(q, m) {
			...
			ans = min(ans, grids[0][i][q] + grids[1][i][q] + grids[2][i][q] - minus);
		}
	}
	if (ans == 1e17) ans = -1;
	cout << ans << endl;
