...
#define ll long long
const ll MAXN=1e6+51,MOD=1e9+7,inf=0x3f3f3f3f;
ll test,n,res,u,v;
ll x[MAXN],f[61];
...
inline void solve()
{
	n=read(),memset(f,0,sizeof(f)),res=0;
	...
	for(register int i=1;i<=n;i++)
	{
		...
		for(register int j=60;j>=0;j--)
		{
			if((x[i]>>j)&1)
			{
				u=(u+(1ll<<j)%MOD*f[j]%MOD)%MOD,v=(v+(1ll<<j)%MOD*n%MOD)%MOD;
			}
			else
			{
				v=(v+(1ll<<j)%MOD*f[j]%MOD)%MOD;
			}
		}
		res=(res+u*v%MOD)%MOD;
	}
