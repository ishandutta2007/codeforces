...
void solve() {
    cin >> n;
    h | ARRAY | 1 | n | ARRAY_INPUT;
    for (int i = 1; i <= n; i++) {
        while ((!s.empty()) && (h[s.top()] >= h[i])) s.pop();
        if (s.empty()) (l[i] = h[i]) *= i;
        else ((l[i] = h[i]) *= (i - s.top())) += l[s.top()];
        s.push(i);
    }
    while (!s.empty()) s.pop();
    for (int i = n; i >= 1; i--) {
        while ((!s.empty()) && (h[s.top()] >= h[i])) s.pop();
        if (s.empty()) (r[i] = h[i]) *= (n - i + 1);
        else ((r[i] = h[i]) *= (s.top() - i)) += r[s.top()];
        s.push(i);
    }
    for (int i = 1; i <= n; i++) (r[i] += l[i]) -= h[i];
    int best = max_element(r + 1, r + n + 1) - r;
    l[best] = h[best];
    for (int i = best - 1; i >= 1; i--) l[i] = min(h[i], l[i + 1]);
    for (int i = best + 1; i <= n; i++) l[i] = min(h[i], l[i - 1]);
