...
public class B {
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			...
			ArrayList<Pair> insert = new ArrayList<>();
			while (!deque.isEmpty()) {
				int first = deque.pollFirst();
				int cur = 1;
				while (deque.peekFirst() != first) {
					elements.add(deque.pollFirst());
					cur++;
				}
				deque.pollFirst();
				cur++;
				while (!elements.isEmpty()) {
					insert.add(new Pair(pref + cur++, elements.peek()));
					deque.addFirst(elements.poll());
				}
				t.add(cur);
				pref += cur;
			}
			out.println(insert.size());
			for (Pair p : insert) {
				out.println(p.x + " " + p.y);
			}
			out.println(t.size());
