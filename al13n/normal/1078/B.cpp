...
int main(int argc, char **argv){
  ...
  forn(i,n) cin>>A[i];
  sort(all(A));
  vector<vector<int>> D(n+1,vector<int>(accumulate(all(A),0)+1));
  vector<int> S(n+1);
  forv(q,A){
    for(int k=q;k>0;--k){
      S[k+1]=S[k]+A[q];
      forn(i,S[k]+1) if(D[k][i]) D[k+1][i+A[q]]=A[q];
    }
    D[1][A[q]]=A[q];
    S[1]=A[q];
  }
  ...
  for(int i=0;i<n;){
    int ii=i+1;
    while(ii<n && A[ii]==A[i]) ++ii;
    if(!i && ii<n && A[ii]==A[n-1] && D[ii-i][A[i]*(ii-i)]==A[i]){
      ans=n;
      break;
    }
    for(int k=ans+1;k<=ii-i;++k) if(D[k][A[i]*k]==A[i]) ans=k;
    i=ii;
  }
  assert(ans>0);
  cout<<ans;
