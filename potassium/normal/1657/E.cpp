...
int main() {
	...
	vector<vector<Mint>> dp(k + 1, vector<Mint>(n + 1, Mint(0, MOD998)));
	for (int now = 1; now <= k; now++)
		dp[now][1] = 1;

	for (int i = 3; i <= n; i++) {
		vector<Mint> sum(k + 1, Mint(0, MOD998));
		for (int now = 1; now <= k; now++)
			for (int cnt = 1; cnt <= i - 2; cnt++)
				sum[now] += dp[now][cnt];

		vector<vector<Mint>> ndp(k + 1, vector<Mint>(n + 1, Mint(0, MOD998)));
		for (int now = 1; now <= k; now++) {
			Mint mul = Mint(k + 1 - now, MOD998).pow(i - 2);

			for (int cnt = 2; cnt <= i - 1; cnt++)
				ndp[now][cnt] += dp[now][cnt - 1] * (i - 1) / cnt * mul;

			for (int last = 1; last < now; last++)
				ndp[now][1] += sum[last] * (i - 1) * mul;
		}
		dp = move(ndp);
	}
