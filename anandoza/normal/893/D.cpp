...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (balance < 0) {
            ans++;
            int max_amount = balance + max_delta[i];
            int headroom = d - max_amount;      // the most we can add without passing d later
            balance += max(-balance, headroom); // always fix balance now, though
        }
    }
