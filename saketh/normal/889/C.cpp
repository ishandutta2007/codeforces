...
int main() {
    ...
    F0R (i, K + 1) {
        cont[i] = mn::fact(i);
        if (i < K) trail += cont[i] / mn::fact(i);
    }

    mn ans = 0;

    FOR (i, K + 1, N + 1) {
        mn stop = cont[i - K - 1] * mn::ncr(i - 1, K) * mn::fact(K);
        ans += stop * mn::fact(N - i) * mn::ncr(N - 1, i);

        trail -= cont[i - K - 1] / mn::fact(i - K - 1);
        trail += cont[i - 1] / mn::fact(i - 1);
        cont[i] = trail * mn::fact(i - 1);
    }
