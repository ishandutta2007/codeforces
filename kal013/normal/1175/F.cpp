...
int ans=0;
const int N=3e5+100;
int prvblock[N],nxtblock[N],nxt[N],prv[N],a[N],curr[N];


void dfs(int x,int L,int R,int l,int r,int use,int n){
    if (r-l+1==use) ++ans;
    int y=nxt[x];

    if (y<=R && y!=n){
        dfs(y,max(L,prvblock[y]),min(R,nxtblock[y]),min(l,y),max(r,y),use+1,n);
    }
    y=prv[x];
    if (y>=L && y!=-1){
        dfs(y,max(L,prvblock[y]),min(R,nxtblock[y]),min(l,y),max(r,y),use+1,n);
...
void solve(){
    ...
    For(i,n){
        ...
        if (curr[a[i]-1]!=-1){
            nxt[curr[a[i]-1]]=min(nxt[curr[a[i]-1]],i);
        }
