...
LL getpw2(LL base, LL r, LL c, LL k) {
    if (r > c) swap(r, c);
    LL cm1 = (1 << c) - 1;
    base ^= base & cm1;
    if (base >= k)
        return 0;
    LL ri = min(1LL << c, k - base) % Mod;
    LL sum = (ri * (ri - 1) / 2) % Mod;
    LL cnt = (ri << r) % Mod;
    LL res = ((base + 1) * cnt % Mod + (sum << r) % Mod) % Mod;
    return res;
}
...
int solve() {
    ...
    cin >> x1 >> y1 >> x2 >> y2 >> k;
    x1--, y1--;
    LL ans = get(x2, y2, k) - get(x1, y2, k) - get(x2, y1, k) + get(x1, y1, k);
    ans %= Mod;
    ans = (ans + Mod) % Mod;
    cout << ans << '\n';
    return 0;
