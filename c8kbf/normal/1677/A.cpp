...
_ n, a[maxn], ta[maxn], tb[maxn], s;

inline _ askB(_ x);
inline _ askA(_ x);
inline void addB(_ x, _ k);
inline void addA(_ x, _ k);
inline void AC() {
    ...
    s = 0;
    for(_ i = 1; i <= n; ++i) {
        for(_ j = 1; j <= n; ++j) ta[j] = tb[j] = 0;
        _ c = 0;
        for(_ j = i+2; j <= n; ++j) addB(a[j], 1);
        for(_ j = i+1; j <= n-1; ++j) {
            if(a[j] > a[i]) s += c;
            c -= askA(a[j+1]);
            addA(a[j], 1);
...
inline void addB(_ x, _ k) {
    for(; x <= n; x += x & -x) tb[x] += k;
    return;
}

inline _ askA(_ x) {
    _ rv = 0;
    for(; x <= n; x += x & -x) rv += ta[x];
    return rv;
}
