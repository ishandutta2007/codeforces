...
struct mint {
  ...
  mint(signed long sig) {sig=_norm(sig%MOD); x=sig;}
  mint(unsigned long sig) {sig=_norm(sig%MOD); x=sig;}
  mint(signed long long sig) {sig=_norm(sig%MOD); x=sig;}
  mint(unsigned long long sig) {sig=_norm(sig%MOD); x=sig;}
...
int main() 
{
  ...
  int n; cin>>n;
  mint nfac=1; for (int x=1;x<=n;x++) nfac*=x;
  mint kfac=1;
  mint ans=nfac;
  for (int k=2;k<=n;k++) {
    kfac*=k;
    mint cnt=nfac*kfac.inverse()*(k-1);
    ans+=cnt*(n-k);
  }
  printf("%d\n",ans.get());
}
