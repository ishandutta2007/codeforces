...
ll get(ll p)
{
	...
	reps(k, vec){
		if(k >= M) break;
		if(p == vec[k].second) continue;
		ret = gcd(ret, query(p, vec[k].second));
	}
	return ret;
}
...
int main(void)
{
	...
	while(T--){
		...
		vec.clear();
		rep(i, 1, n) vec.push_back(P(getrand(), i));
		sort(all(vec));
		
		if(n <= H){
			rep(j, 1, n) ans[j] = get(j);
			
			ll mn = min(ans[1], min(ans[2], ans[3]));
			if(n == 3 && mn % 2 == 0){
				rep(i, 1, n) if(ans[i] == 2*(mn+1)) ans[i] /= 2;
			}
			cout << "! ";
			outa(ans, 1, n);
			continue;
