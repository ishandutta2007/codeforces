...
void solve(){
    ...
    int res = 0;
    REP(i, n){
        if(q.empty() and a[i] == 0 or q.size() and q.top() + 1 == a[i]){
            q.push(a[i]);
            debug(i, a[i]);
            continue;
        }
        if(q1.empty() and a[i] == 0 or q1.size() and q1.top() + 1 == a[i]){
            q1.push(a[i]);
            debug(i, a[i]);
            continue;
        }
    }
    if(!q.empty()) res += q.top() + 1;
    if(!q1.empty()) res += q1.top() + 1;
    print(res);
    debug(res);
}
