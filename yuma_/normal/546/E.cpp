...
int main() {
    ...
    for(int i=0;i<2;++i){
        for(int j=0;j<N;++j){
            cin>>v[i][j];
            if(i==0){
                g[start].push_back(Edge{start,in+j,v[i][j]});
            }else{
                g[out+j].push_back(Edge(out+j,goal,v[i][j]));
            }
        }
    }
    for(int i=0;i<N;++i){
        g[in+i].push_back(Edge(in+i,out+i,1000));
    }
    for(int i=0;i<M;++i){
        int u,v;cin>>u>>v;
        u--;v--;
        g[in+u].push_back(Edge(in+u,out+v,1000));
        g[in+v].push_back(Edge(in+v,out+u,1000));

    }
    auto tt=maximumFlow(g,start,goal);
   // WHATS(tt)
    int sum=accumulate(v[0].begin(),v[0].end(),0);
    int sum2=accumulate(v[1].begin(),v[1].end(),0);
    if(sum!=sum2||sum!=tt.second){
        cout<<"NO"<<endl;
    }else{
        cout<<"YES"<<endl;
        for(int i=0;i<N;++i){
            for(int j=0;j<N;++j){
                cout<<tt.first[in+i][out+j]<<' ';
            }
            cout<<endl;
        }
    }
