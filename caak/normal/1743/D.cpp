...
inline void AC() {
    ...
    s = a;
    _ ln = n-fs;
    for(_ i = 0; i < fs; ++i) for(_ j = ln-1; j < n; ++j) if(i <= j)
        s = max(s, sor(a, a.substr(i, j-i+1)));
    writeln(s);
...
str sor(str a, str b) {
    if((_)a.size() < b.size()) swap(a, b);
    reverse(a.begin(), a.end());
    reverse(b.begin(), b.end());
    for(_ i = 0; i < b.size(); ++i) if(b[i] == '1') a[i] = '1';
    reverse(a.begin(), a.end());
    return a;
}