...
void solve() {
	...
	REP(i, n) {
		int M = 0;
		REP(j, m) maxi(M, a[i][j]);
		maxes.insert(M);
		iord[i] = {M, i};
	}
	REP(j, m) {
		int M = 0;
		REP(i, n) maxi(M, a[i][j]);
		maxes.insert(M);
		jord[j] = {M, j};
	}
	sort(RALL(iord));
	sort(RALL(jord));
	int pi = 0, pj = 0;
	debug(imie(iord), imie(jord));
	for (int x : reversed(maxes)) {
		while (pi + 1 < n && iord[pi + 1].first >= x) pi++;
		while (pj + 1 < m && jord[pj + 1].first >= x) pj++;
		assert(b[pi][pj] == 0);
		b[pi][pj] = x;
	}
	debug(dump(b, 0, n, 0, m));
	REP(i, n) REP(j, m) if (b[i][j]) fixed[i][j] = true;
	vi vals;
	REP(i, n) {
		int v = iord[i].first;
		bool seen = false;
		REP(j, m - 1) {
			int n1 = num(i, j), n2 = num(i, j + 1);
			if (b[i][j] == v) seen = true;
			if (seen) graf[n2].push_back(n1);
			else graf[n1].push_back(n2);
		}
	}
	REP(j, m) {
		int v = jord[j].first;
	...
	REP(i, n) REP(j, m) {
		int nu = num(i, j);
		if (b[i][j])
			low[nu] = high[nu] = b[i][j];
		else {
			low[nu] = 1;
			high[nu] = min(iord[i].first, jord[j].first);
		}
		debug(imie(i), imie(j), arr(low, nu), arr(high, nu));
	}
	REP(i, n * m) dfs(i);
	min_queue<pii> leaves;
	REP(i, n * m) for (int x : graf[i]) deg[x]++;
	REP(i, n * m) by_low[low[i]].push_back(i);
	for (int t = 1; t <= n * m; ++t) {
		for (int x : by_low[t]) if (!in[x] && deg[x] == 0) {
			leaves.push({high[x], x});
			in[x] = true;
		}
		debug(imie(leaves));
		assert(!leaves.empty());
		...
		leaves.pop();
		ans[x] = t;
		debug(fun(decode, x), imie(t));
		for (int v : graf[x]) {
			deg[v]--;
			if (deg[v] == 0 && !in[v] && low[v] <= t) {
				in[v] = true;
				leaves.push({high[v], v});
			}
		}
	}
	REP(i, n) REP(j, m) a[i][j] = ans[num(i, j)];
	REP(i, n) {
