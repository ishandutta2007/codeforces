...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            int mask = 1 << 9;
            int[] profits = new int[mask];
            int[] wholeProfits = new int[mask];
            int[] singleCosts = new int[mask];
            int[] secondCosts = new int[mask];
            int[] which = new int[mask];
            int[] which2 = new int[mask];
            int[] exists = new int[mask];
            Arrays.fill(singleCosts, inf);
            Arrays.fill(secondCosts, inf);
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < k; j++) {
                    int t = io.readInt() - 1;
                    msk = bitOperator.setBit(msk, t, true);
                }
                people[i] = msk;
                profits[people[i]]++;
            }
            for (int i = 0; i < mask; i++) {
                for (int j = 0; j < mask; j++) {
                    if (bitOperator.subset(j, i)) {
                        wholeProfits[i] += profits[j];
                    }
                }
            }
            for (int i = 1; i <= m; i++) {
                ...
                exists[msk]++;
                if (singleCosts[msk] > cost) {
                    secondCosts[msk] = singleCosts[msk];
                    which2[msk] = which[msk];
                    singleCosts[msk] = cost;
                    which[msk] = i;
                } else if (secondCosts[msk] > cost) {
                    secondCosts[msk] = cost;
                    which2[msk] = i;
                }
            ...
            for (int i = 0; i < mask; i++) {
                ...
                for (int j = 0; j < mask; j++) {
                    ...
                    int msk = i | j;
                    if (wholeProfits[msk] > maxProfit) {
                        maxProfit = wholeProfits[msk];
                        minCost = singleCosts[i] + singleCosts[j];
                        a = which[i];
                        b = which[j];
                    }
                    if (wholeProfits[msk] == maxProfit
                            && singleCosts[i] + singleCosts[j] < minCost) {
                        minCost = singleCosts[i] + singleCosts[j];
                        a = which[i];
                        b = which[j];
                    }
            ...
            for (int i = 0; i < mask; i++) {
                ...
                if (wholeProfits[msk] > maxProfit) {
                    maxProfit = wholeProfits[msk];
                    minCost = singleCosts[i] + secondCosts[i];
                    a = which[i];
                    b = which2[i];
                }
                if (wholeProfits[msk] == maxProfit
                        && singleCosts[i] + secondCosts[i] < minCost) {
                    minCost = singleCosts[i] + secondCosts[i];
                    a = which[i];
                    b = which2[i];
                }
            ...
            io.cache.append(a).append(' ').append(b);
        }
    }
