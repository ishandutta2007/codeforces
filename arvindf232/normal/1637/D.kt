...
fun packer(a:IntArray, target:Int):Int{
    val size = target
...
fun main(){
    solve.cases{
        ...
        val DP = Array(n+1){BooleanArray(want+1)}
        DP[0][0] = true
        for(i in 0 until n){
            for(left in 0..want){
                if(DP[i][left] == false) continue
                if(left + A[i] <= want){
                    DP[i+1][left + A[i]] = true
                }
                if(left + B[i] <= want){
                    DP[i+1][left+ B[i]] = true
                }
            }
        }
        var best = 0
...
//        just dei best
        basecost += 1L * best * best
        basecost += 1L * (sum - best ) * (sum - best)
        put(basecost)
    }
    done()
}
