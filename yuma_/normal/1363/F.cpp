...
void solveF(){
    ...
    while(T--){
        ...
        vector<vector<int>>a_sums(N+1,vector<int>(26));
        vector<vector<int>>b_sums(a_sums);
        for(int i=N-1;i>=0;--i){
            int x=A[i]-'a';
            for(int j=0;j<26;++j){
                a_sums[i][j]=a_sums[i+1][j];
                if(x==j)a_sums[i][j]++;
            }
            x=B[i]-'a';
            for(int j=0;j<26;++j){
                b_sums[i][j]=b_sums[i+1][j];
                if(x==j)b_sums[i][j]++;
            }
        }
        ...
        dp[N][N]=0;
        for(int a_x=N;a_x>=0;--a_x){
            for(int b_x=N;b_x>=0;--b_x){
                if(a_x&&b_x&&(A[a_x-1]==B[b_x-1])){
                    dp[a_x-1][b_x-1]=min(dp[a_x-1][b_x-1],dp[a_x][b_x]);
                }
                if(a_x){
                    dp[a_x-1][b_x]=min(dp[a_x-1][b_x],dp[a_x][b_x]+1);
                }
                if(b_x){
                    int b_num=B[b_x-1]-'a';
                    if(a_sums[a_x][b_num]>b_sums[b_x][b_num]){
                        dp[a_x][b_x-1]=min(dp[a_x][b_x-1],dp[a_x][b_x]);
                    }

                }
            }
        }
        if(dp[0][0]>=10000)dp[0][0]=-1;
        cout<<dp[0][0]<<endl;
    }
}
int main() {
