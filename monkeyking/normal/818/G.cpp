...
//		for(int j=i+1;j<n;j++)
//		{
//			if(f1<=4 && a[j]-1==a[i]) f1++,g.addEdge(i,j+n,1,0);
//			if(f2<=4 && a[j]%7==a[i]%7) f2++,g.addEdge(i,j+n,1,0);
//			if(f3<=4 && a[j]+1==a[i]) f3++,g.addEdge(i,j+n,1,0);
//		}
//		g.addEdge(i+n,i,1,-1);
////		if(i!=n-1) g.addEdge(i,i+1,4,0);
//	}
//	g.addEdge(src,lim,4,0);
//	for(int i=0;i<n;i++)
//	{
//		g.addEdge(lim,i+n,1,0);
//		g.addEdge(i,dst,1,0);
//	}
////	g.addEdge(n-1,dst,4,0);
//	d.init(g);
//	pii f=d.run();
////	assert(f.first==4);
//	cout<<-f.second<<endl;
...
////		{
////			if(e.flow==0 && e.vertex>i+n) cout<<i<<"->"<<e.vertex-n<<endl;
////		}
////		cout<<endl;
////	}
//	return 0;
