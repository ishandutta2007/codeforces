...
ll ad(ll x, ll y) { return x + y; }
ll z() { return 0; }
ll inf() { return INF; }

bool ch(ll x) { return x > 0; }
...
void solve() {
    ...
    lazy_segtree<ll, min, inf, ll, ad, ad, z> sg(M);
    rep(i, M)sg.set(i, y[i]);
    ...
    rep(i, M) {
        ...
        sg.apply(0, M, -G);
        cnt += G;
        ll d = sg.min_left<ch>(M) - 1;
        //cout << cnt << " " << d << " " << x[d] << endl;
        st.push_back({cnt,x[d]});
        sg.apply(d, M, -1);
        sg.set(d, INF); 
        cnt++;
    }
    ...
    //}
    rep(i, M) {
        if (st[i].first + 1 <  st[i+1].first || st[i].second > st[i+1].second) {
            neq++;
        }
    }
    
    ll eq = N - 1 - neq;
    mint ans = com(N + eq, eq);
    cout << ans.val() << endl;
}
