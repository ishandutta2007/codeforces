...
int main() {
	CASE(t) {
		...
		vector<Event> ev;
		for (int i = 0; i < n; i++) {
			ev.push_back({ x[i] - p[i], Event::ADD, +1, -(x[i] - p[i]) });
			ev.push_back({ x[i] + 1, Event::ADD, -2, (x[i] - p[i]) + (x[i] + p[i]) });
			ev.push_back({ x[i] + p[i], Event::ADD, +1, -(x[i] + p[i]) });

			ev.push_back({ x[i], Event::QUERY, 0, 0 });
		}
