...
bool ok(const vector<int> &p) {
    for (int i = 1; i < p.size(); ++i)if (abs(p[i] - p[i - 1]) < 2 || abs(p[i] - p[i - 1]) > 4) return false;
    return true;
}
...
void solve() {
    ...
    if (n < 4) answer(-1);
    vector<int> ans;
    if (n % 4 == 0) ans = {2, 4, 1, 3};
    vector<int> f = {2, 4, 1, 3};
    if (n % 4 == 1) ans = {3, 1, 4, 2, 5};
    if (n % 4 == 2) ans = {3, 1, 4, 6, 2, 5};
    if (n % 4 == 3) ans = {3, 1, 4, 6, 2, 5, 7};
    vector<int> p = ans;
    while (p.size() < n){
        int x = p.size();
        for (int i = 0; i < 4; ++i){
