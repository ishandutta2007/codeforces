...
bool pre[N], suf[N];
int lft[N] /*a[i] matches avg[i - 1]*/, rit[N];

bool can(ld o, ld to) {
	int ox = lower_bound(avg + 1, avg + n + 1, o, greater<ld>()) - avg;
	int tx = lower_bound(avg + 1, avg + n + 1, to, greater<ld>()) - avg;
	if(tx > ox) --tx;
	if(tx <= ox) {
		return pre[tx - 1] && suf[ox + 1] && lft[ox] <= tx + 1 && a[tx] >= to - eps;
	} else {
		return pre[ox - 1] && suf[tx + 1] && rit[ox] >= tx - 1 && a[tx] >= to - eps;
	}
}
...
void mian() {
	...
	REP(i, 1, m) {
		b[i].clear();
		int k = read(); while(k--) b[i].pb(read());
		int sum = 0; for(int x : b[i]) sum += x;
		avg[i] = (ld)sum / SZ(b[i]);
	}
	sort(a + 1, a + n + 1, greater<int>()); sort(avg + 1, avg + m + 1, greater<ld>()); n = m;
	pre[0] = suf[n + 1] = 1; 
	REP(i, 1, n) pre[i] = pre[i - 1] && a[i] >= avg[i] - eps;
	PER(i, n, 1) suf[i] = suf[i + 1] && a[i] >= avg[i] - eps;
	lft[1] = 1; REP(i, 2, n) lft[i] = a[i] >= avg[i - 1] - eps ? lft[i - 1] : i + 1;
	rit[n] = n; PER(i, n - 1, 1) rit[i] = a[i] >= avg[i + 1] - eps ? rit[i + 1] : i - 1;
	string output;
	REP(i, 1, n) {
		int sum = 0; for(int x : b[i]) sum += x;
		for(int x : b[i]) output += can((ld)sum / SZ(b[i]), (ld)(sum - x) / (SZ(b[i]) - 1)) ^ 48;
	}
	puts(output.c_str());
}
