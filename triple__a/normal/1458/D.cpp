...
signed main(){
    ...
    while (t--){
        mp.clear(),mp2.clear();
        cin>>s;
        int cnto=0,delt=0,cntz=0,cnt=0;
        for (auto c:s){
            if (c=='1') delt++, mp2[delt]++, cnto++;
            else delt--, mp[delt]++, cntz++;
        }
        cnt=0;
        while (1){
            if (cnto<=0&&cntz<=0) break;
            if (mp[cnt-1]&& !(mp2[cnt+1]>0&&mp2[cnt]==0)) cout<<"0", cnt--, mp[cnt]--,cntz--;
            else cout<<"1", cnt++, mp2[cnt]--, cnto--;
        }
        cout<<"\n";
    }
    return 0;
