...
int main(){
  ...
  auto check=[&](ll mid){
    ...
    rep(i,0,n){
      vector<ll>vv(1);
      idx=i;
      while(idx<n&&p[i].fi==p[idx].fi){
        ll nxt=vv.back()+p[idx].se;
        vv.PB(nxt);
        idx++;
      }
      i=idx-1;
    ...
    ll sz=vp.size();
    auto dp=vec(sz+1,n+1,INF);
    dp[0][0]=0;
    rep(i,0,sz){
      //debug(vp[i],vp[i].size());
      rep(j,0,n+1){
        ll m=vp[i].size()-1;
        rep(o,0,m+1){
          if(j<m-o)continue;
          ll nj=j+o-(m-o);
          chmin(dp[i+1][min(n,nj)],dp[i][j]+vp[i][o]);
        }
      }
    }
    //debug(dp,sz+1,n+1);
    return *min_element(ALL(dp[sz]))<=0;
  };
  //cout<<check(8750)<<endl;
  //return 0;
  ll ok=1e12,ng=0;
  while(ok-ng>=2){
    ll mid=(ok+ng)/2;
    //cout<<mid spa check(mid)<<endl;
    if(check(mid))ok=mid;
    else ng=mid;
  }
  cout<<ok<<endl;
