...
int sizeT[N + 1];			// ilosci jedynek w wierszach

bool intersects[N + 1];		// Czy wiersz przecina wzorzec?
					// Tablica globalna, gdyz po rozwazeniu wiersza
					// przecinaj?cego wzorzec zostanie usuniety ze zbioru wierszy
...
pair<group,group> split(group& g,int set){
	...
	for(vector<int>::iterator it = g.sets.begin(); it != g.sets.end(); ++it){
		...
		else if(intsize == sizeT[*it]) gintersection.sets.push_back(*it);
...
int f(group g){
	...
	while(g.domain.size() > 0){
		...
		for(unsigned i=1;i<sets.size();i++) if(sizeT[sets[i]] > sizeT[sets[first]]) first = i;
		int cur = sets[first];
		erase(sets,first);
		g.sets = sets;
...
int main(){
	read_data();
	group g;
	for(int i=1;i<=n;i++) if(sizeT[i]) g.sets.push_back(i);
