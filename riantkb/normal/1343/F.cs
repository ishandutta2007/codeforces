...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            for (int i = 0; i < n - 1; i++)
            {
                a[i] = sc.IntArr.Skip(1).Select(x => x - 1).ToArray();
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int j = 0; j < 1; j++)
                {
                    foreach (var item in lis[ans[j]])
                    {
                        if (!used[item]) {
                            used[item] = true;
                            if (j + a[item].Length - 1 >= n) {
                                goto A;
                            }
                            idx[j + a[item].Length - 1] = item;
                        }
                        --rem[item];
                        x[item] ^= ans[j];
                    }
