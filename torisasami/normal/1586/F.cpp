...
int main() {
    ...
    vector<vector<int>> p(n, vector<int>(c));
    rep(i, n) {
        int ci = i;
        rep(j, c) p[i][j] = ci % k, ci /= k;
    }
    vector<vector<int>> ans(n, vector<int>(n, -1));
    rep(i, n) REP(j, i + 1, n) {
        rep2(l, c) {
            if (p[n - 1 - i][l] > p[n - 1 - j][l]) {
                ans[i][j] = l;
                break;
            }
        }
    ...
    cout << c << endl;
    vector<int> res;
    rep(i, n) REP(j, i + 1, n) res.pb(ans[i][j]);
    rep(i, res.size()) cout << res[i] + 1 << (i + 1 == res.size() ? '\n' : ' ');
}