...
const int MN=4e5+5; 
#define Fail { printf("0\n"); return void(); }
int a[MN],b[MN],n,s,vis[MN],qg[MN]; 
void works() {
	n=read(),s=read();  For(i,0,n) vis[i]=qg[i]=0; 
	For(i,1,n) a[i]=read();
	For(i,1,n) b[i]=read(); 
	For(i,1,n) if(~b[i]) { vis[b[i]]=1; if(a[i]-b[i]>s) Fail; }
	For(i,1,n) if(b[i]==-1) qg[max(a[i]-s,0ll)]++; 
	For(i,1,n) qg[i]+=qg[i-1];
//	fprint(vis,n),fprint(qg,n); 
	int tot=0,ans=1; 
	For(i,1,n) {
		if(!vis[i]) {
			
			mulmod(ans,max(0ll,qg[i]-tot)); 
			tot++; 	
		}
	}
	cout<<ans<<endl; 
