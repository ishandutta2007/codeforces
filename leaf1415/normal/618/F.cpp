...
int main(void)
{
	...
	vector<ll> avec, bvec;
	rep(i, 1, n){
		ll j = upper_bound(b, b+n+1, a[i])-b-1;
		if(a[i] == b[j]){
			rep(k, 1, i) avec.push_back(k);
			rep(k, 1, j) bvec.push_back(k);
			break;
		}
		ll d = a[i]-b[j];
		if(p[d] != P(0, 0)){
			ll pi = p[d].first, pj = p[d].second;
			rep(k, pi+1, i) avec.push_back(k);
			rep(k, pj+1, j) bvec.push_back(k);
			break;
		}
		p[d] = P(i, j);
	}
	
	if(flag) swap(avec, bvec);
	outl(sz(avec));
	outl(avec);
	outl(sz(bvec));
	outl(bvec);
