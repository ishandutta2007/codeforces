...
void dfs(int u, int f) {
	...
	if(u <= (1 << (k - 1))) {
		if(s[u] == '1') ans[u][0] = 1;
		else if(s[u] == '?') ans[u][0] = 2;
		ans[u][1] = ans[u][0] / 2 + 1;
		return ;
	}
	e[u][0] = (1 << k) - (((1 << k) - u) << 1) - 1;
	e[u][1] = (1 << k) - (((1 << k) - u) << 1);
	dfs(e[u][0], u), dfs(e[u][1], u);
	if(s[u] == '1') ans[u][0] = 1;
	else if(s[u] == '?') ans[u][0] = 2;
	else ans[u][0] = 0;
	if(ans[u][0] == 2) ans[u][1] = ans[e[u][1]][1] + ans[e[u][0]][1]; 
...
void solve(int u, char c) {
	...
	ans[u][0] = p;
    if(u <= (1 << (k - 1))) ans[u][1] = ans[u][0] / 2 + 1, u = fa[u];
    while(u) {
