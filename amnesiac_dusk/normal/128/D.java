...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		...
		Iterator<Map.Entry<Integer, Integer>> itr=a.entrySet().iterator();
		Map.Entry<Integer, Integer> cur=itr.next();
		int key=cur.getKey(),val=cur.getValue();
		boolean pos=true;
		while(itr.hasNext())
		{
			if(val<=0)
			...
			if(next.getKey()!=key+1)
			{
				pos=false;
				break;
			}
			else if(next.getValue()+1<val)
			{
				pos=false;
				break;
			}
