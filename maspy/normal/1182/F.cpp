...
#line 2 "/home/maspy/compro/library/mod/range_freq_of_linear.hpp"

// L <= x < R (ax+b mod) in [lo, hi) 
...
pi min_mod_linear(ll L, ll R, ll a, ll b, ll mod) {
  auto check = [&](ll n) -> bool {
    // n 
    return range_freq_of_linear(L, R, a, b, mod, 0, n) >= 1;
  };
  ll y = binary_search(check, mod, 0) - 1;
  auto check_x = [&](ll x) -> bool {
    // x 
    return range_freq_of_linear(L, x, a, b, mod, 0, y + 1) >= 1;
  };
  ll x = binary_search(check_x, R, L) - 1;
  assert((a * x + b - y) % mod == 0);
  return {x, y};
}
...
void solve() {
  LL(a, b, p, q);
  // 2px - q mod 2q  0 
  auto [x1, y1] = min_mod_linear(a, b + 1, 2 * p, -q, 2 * q);
  auto [x2, y2] = min_mod_linear(a, b + 1, -2 * p, q, 2 * q);
  ll ANS = (y1 < y2 ? x1 : x2);
  if (y1 == y2) ANS = min(x1, x2);
  print(ANS);
}
