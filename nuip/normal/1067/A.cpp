...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	dp[0][0][2]=1;
	rep(i,n){
		reps(j,1,201)if(a[i]<0 || a[i]==j){
			rep(k,3) dp[i+1][j][1]+=dp[i][j][k];
		}
		mint sum=dp[i][0][2];
		reps(j,1,201){
			if(a[i]<0 || a[i]==j){
			...
			sum+=dp[i][j][0]+dp[i][j][1]+dp[i][j][2];
		}
		sum=0;
		rreps(j,1,201){
			if(a[i]<0 || a[i]==j){
	...
	// rep(i,13){rep(j,3) cout<<dp[1][i][j]<<",";cout<<endl;}cout<<endl;
	// rep(i,13){rep(j,3) cout<<dp[1][i][j]<<",";cout<<endl;}cout<<endl;
	mint re=0;
	rep(i,201) re+=dp[n][i][0]+dp[n][i][1];
	cout<<re<<endl;
  return 0;
}