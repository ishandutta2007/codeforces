...
#define int ll
const int seg_size = 262144 * 2;
int seg_tree[seg_size * 5];
int lazy_seg[seg_size * 5];
void seg_set(int now);
void reflect(int now) {
    if (lazy_seg[now] == 0) {
        return;
    }
...
int seg_find(int now, int n_l, int n_r, int w_l, int w_r, int settings) {
    ...
    if (w_r <= n_l || n_r <= w_l) return 1e18;
    return min(seg_find(now * 2, n_l, (n_l + n_r) / 2, w_l, w_r, settings),
        seg_find(now * 2 + 1, (n_l + n_r) / 2, n_r, w_l, w_r, settings));
}
void solve(){
    int n;
    cin >> n;
    ...
    seg_set(seg_size / 2);
    REP(i, inputs.size()) {
        int itr = lower_bound(ALL(b), inputs[i]) - b.begin();
        if (itr < b.size() && b[itr] == inputs[i]) {
            int hoge = seg_find(1, 0, seg_size, itr - 1, itr, 0);
            if (costs[i] > 0) {
                seg_find(1, 0, seg_size, 0, itr, costs[i]);
            }
            else {
                seg_find(1, 0, seg_size, 0, seg_size, costs[i]);
            }
            int tmp = seg_find(1, 0, seg_size, itr, itr + 1, 0);
            if (hoge < tmp) {
                seg_tree[seg_size + itr] = hoge;
                seg_set((seg_size + itr) / 2);
            }
        }
        else {
            if (costs[i] > 0) {
    ...
    int ans = seg_find(1, 0, seg_size, m - 1, m, 0);
    cout << "YES" << endl;
    cout << ans << endl;
}
#undef int
