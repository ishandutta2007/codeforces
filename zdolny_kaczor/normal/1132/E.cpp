...
ll a[9];
const int nax = 1e6 + 44, take = nax / 40;
bool reachable[nax];
void add(int x) {
	debug(imie(x));;
	FORD(i, nax - 1, 0)
...
int main() {
	...
	FOR(i, 1, nax - 1) {
		if (reachable[i]) largest_less[i] = i;
		else largest_less[i] = largest_less[i - 1];
	}
	ll ans = largest_less[min<ll>(w, nax - 1)];
	debug(imie(ans));
	ll curr = 0;
	FORD(i, 8, 1) {
		ll first_take = min(a[i], max(0ll, (w - curr - nax) / i));
		ll left = a[i] - first_take;
		curr += i * first_take;
		debug(imie(curr), imie(left));
		maxi(ans, curr + largest_less[min<ll>(w - curr, nax - 1)]);
		REP(_, min(left, (ll) nax)) {
			if (curr + i <= w)
				curr += i;
			maxi(ans, curr + largest_less[min<ll>(w - curr, nax - 1)]);
			if (curr + i > w) break;
