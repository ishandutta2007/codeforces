...
struct dsu{
	...
	void uni(int a, int b) {
		...
		if (size[aroot] < size[broot]) {
			...
			maxsize = max(maxsize,size[broot]);
		} else {
			parent[broot] = aroot;
			size[aroot] += size[broot];
			maxsize = max(maxsize,size[aroot]);
		}
	}
};
...
int main() {
	...
	for (auto& i : pos) {
		...
		if (loc > 0 && a[loc-1] < val) {dsu.uni(loc,loc-1); interval--;}
		if (loc < n-1 && a[loc+1] < val) {dsu.uni(loc,loc+1); interval--;}
		
		if ((ll)dsu.maxsize * interval == cur) {
			if (interval > maxl) {
				maxl = interval;
				mink = val+1;
			}
