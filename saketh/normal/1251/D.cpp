...
void solve() {
    ...
    vpii emp(N); re(emp); sort(all(emp));

    ll sumL = 0; trav (e, emp) sumL += e.f; assert(sumL <= S);
    ll extr = S - sumL;

    sort_by(emp, a.s < b.s); int vhi = emp[N/2].s;
    sort_by(emp, a.f < b.f); int vlo = emp[N/2].f;

    int ans = vlo; vlo++;
    while (vlo <= vhi) {
        int mi = (vlo + vhi) / 2;

        ll cost = 0; int need = (N+1)/2;

        F0Rd (i, sz(emp)) {
            if (!need) break;
            if (emp[i].f > mi) { need--; continue; }
            if (emp[i].s >= mi) { cost += mi - emp[i].f; need--; }
        }
        assert(need == 0);
