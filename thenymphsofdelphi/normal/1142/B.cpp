...
signed main(){
    ...
    For(j, 0, 18){
        if ((n - 1) & (1 << j)){
            ForE(i, 1, m + 1){
                fin_nxt[i] = nxt[j][fin_nxt[i]];
            }
        }
    }
    ForE(i, 1, m + 1){
        sparsemn[0][i] = fin_nxt[i];
    }
    For(j, 1, 18){
        ForE(i, 1, m + 1 - (1 << j) + 1){
            sparsemn[j][i] = min(sparsemn[j - 1][i], sparsemn[j - 1][i + (1 << (j - 1))]);
        }
    }
    while (q--){
        int l, r; cin >> l >> r;
        int lg2 = __builtin_clz(r - l + 1) ^ 31;
        int mn = min(sparsemn[lg2][l], sparsemn[lg2][r - (1 << lg2) + 1]);
        if (mn <= r){
            cout << 1;
        }
        else{
