...
void upd(pii v, int L, int R, int l, int r, int id)
{
    if(!valid[id]) return ;
    if(L <= l && r <= R)
    {
...
vector<int> take(int l, int r, int id)
{
    if(!valid[id]) return vector<int> {-1, -1, -1};
    if(l == r)
    {
        vector<int> res{l, mn[id].fi, mn[id].se};
        valid[id] = 0;
        mn[id] = nil;
        return res;
    }
    ...
    int mid = MID(l, r);
    vector<int> res;
    if(mn[ls] == mn[id]) res = take(lson);
    else res = take(rson);
    valid[id] = valid[ls] || valid[rs];
    mn[id] = min(mn[ls], mn[rs]);
    return res;
}
