...
signed main()
{
	...
	while(T--)
	{
		...
		upn
		{
			...
			nw+=a[i];nw-=b[i];
//			nw=10;
			up(j,0,18000)f[j]=g[j]=0;
			int vl=min(b[i],pnw),vr=min(nw,a[i]);
			up(j,max(-b[i],-pnw),min(nw,a[i]))f[j+vl]=ifac[a[i]-j]*ifac[b[i]+j]%mod;
			up(j,0,pnw)g[j]=dp[i-1][j];
//			up(j,0,vl+vr)cout<<f[j]<<" ";edl;
////			cout<<b[i]-nw;edl;
//			up(j,0,nw)cout<<g[j]<<" ";edl;
//			if(i<=100)times(f,g,1000)
			times(f,g,i*10,i*10);
			
			up(j,vl+1,nw+vl)dp[i][j-vl]=f[j]*fac[a[i]+b[i]]%mod;
//			up(j,0,nw)cout<<dp[i][j]<<" ";edl;
//			up(j,0,nw)opls(f[j]*fac[a[i]+b[i]]%mod);edl;
		}
		int ans=0;
		up(j,1,nw)ans+=dp[n][j],ans%=mod;
		cout<<ans;
	}
}
/*
