...
signed main() {
    ...
    vector<int> ord(n);
    iota(all(ord), 0);
    sort(all(ord), [&](int i, int j) { return sz(G[i]) > sz(G[j]); });
    vector<int> pos(n);
    ...
    for (int i = k; i < n; ++i) {
        ...
        if (!used[x]) {
            ...
            while (sz(que)) {
                ...
                que.pop();
                int now = 0;
                sort(all(G[u]), [&](pii a, pii b) {
                    return used[a.f] > used[b.f];
                });
                for (auto [v, ind] : G[u]) {
                    if (ans[ind] == 0) {
                        ++now;
    ...
    for (auto &i : ans) chmax(i, 1);
    dbg(ans);
    for (int i = k; i < n; ++i) {
        int u = ord[i];
        set<int> used;
        for (auto [v, ind] : G[u]) {
            assert(!used.count(ans[ind]));
            used.insert(ans[ind]);
        }
    }
    println(*max_element(all(ans)));
