...
void solve(size_t n, size_t l, size_t r, size_t s) {
    size_t w = r - l;
    size_t min_sum = w * (w + 1) / 2;
    size_t max_sum = w * (n - w) + min_sum;
    if (s < min_sum or max_sum < s) {
        print(-1);
    } else {
        vec<size_t> ans(n, 0);
        ...
        if (s) {
            int p = s / (n - w), q = s % (n - w);
            rep(i, p) ans[r - 1 - i] += n - w;
            if (r - 1 - p >= 0) {
                ans[r - 1 - p] += q;
            }
        }
        vector<bool> used(n, false);
        repl(i, l, r) used[ans[i] - 1] = true;
        size_t j = 0;
        rep(i, n) {
            if (ans[i] == 0) {
