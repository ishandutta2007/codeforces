...
void f(bool bo){
		...
		v64 vec;
		if(rem%2 == 0){
			forn(i,rem/2) vec.pb(i+1);
			forsn(i,rem+1,rem+(k-rem/2)+1) vec.pb(i);
			query(vec,bo);
			vec.clear();
			ans ^= input();
			forn(i,rem/2) vec.pb(i+1+rem/2);
			forsn(i,rem+1,rem+(k-rem/2)+1) vec.pb(i);
			query(vec,bo);
			vec.clear();
			ans ^= input();
			forn(i,alpha-2){
				forsn(j,rem+1+i*k,rem+1+(i+1)*k) vec.pb(j);
				query(vec,bo);
				vec.clear();
				ans ^= input();
			}
...
void solve(){
	...
	if(n==k){
		...
		if(n%2 == k%2){
			...
			ll rem = n-k;
			cout << "? ";
			forn(i,rem/2) cout << i+1 << " ";
			forsn(i,rem+1,rem+(k-rem/2)+1) cout << i << " ";
			cout << endl;
			ans ^= input();
			cout << "? ";
			forn(i,rem/2) cout << i+1+rem/2 << " ";
			forsn(i,rem+1,rem+(k-rem/2)+1) cout << i << " ";
			cout << endl;
			ans ^= input();
			forn(i,1){
				cout << "? ";
				forsn(j,rem+1+i*k,rem+1+(i+1)*k) cout << j << " ";
				cout << endl;
				ans ^= input();
			}
			cout << "! " << ans << endl;
