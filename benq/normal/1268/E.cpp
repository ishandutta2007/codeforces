...
int dfs(int x, int y) {
	trav(t,tmp[x]) if (t.s < y) return dfs(t.f,t.s);
	return y;
}
...
int main() {
	...
	re(n,m); v.rsz(m); re(v);
	reverse(all(v));
	trav(t,v) B.addEdge(t.f,t.s);
	B.init(n+1);
	F0R(i,sz(B.bccs)) trav(j,B.bccs[i]) label[j] = i;
	// ps(B.bccs);
	FOR(i,1,n+1) dp[i] = 1;
	F0R(i,sz(v)) {
		int L = label[i];
		cnt[L] ++;
		ll sum = dp[v[i].f]+dp[v[i].s];
		if (cnt[L] > 1 && cnt[L] == sz(B.bccs[L])) {
			trav(t,B.bccs[L]) if (t != i) {
				int u = B.ed[t].f, v = B.ed[t].s;
				tmp[u].pb({v,t}), tmp[v].pb({u,t});
			}
			int a = dfs(v[i].f,MOD);
			int b = dfs(v[i].s,MOD);
			// ps("??",a,b);
		...
		dp[v[i].f] = dp[v[i].s] = DP[i] = sum;
	}
	FOR(i,1,n+1) pr(dp[i]-1,' ');
