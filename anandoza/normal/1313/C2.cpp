...
namespace __output {
    ...
    template<class T1, class T2, class C> void pr(const map<T1,T2,C>& x) { prContain(x); }
...
int main() {
    ...
    auto f = [&](int i) -> ll { return a[i] + left[i] + right[i]; };
    ...
    vll ans(n);
    ans[middle] = a[middle];
    for (int j = middle - 1, v = a[middle]; j >= 0; j--) {
        ckmin(v, a[j]);
        ans[j] = v;
    }
    for (int j = middle + 1, v = a[middle]; j < n; j++) {
        ckmin(v, a[j]);
        ans[j] = v;
    }
