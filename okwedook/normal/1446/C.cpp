...
int get(int x) {
    ...
    for (int i = 29; i >= 0; --i) {
        ...
        int newv = go(v, bit);
        ans *= 2;
        if (t[newv].cnt == 0) v = go(v, !bit), ans += !bit;
        else v = newv, ans += bit;
    }
    return ans;
}
...
int countans(int v = 0, int h = 0) {
    if (v == 0 && h != 0) return 0;
    if (t[v].cnt == 1) return 1;
    int l = countans(go(v, 0), h + 1);
    int r = countans(go(v, 1), h + 1);
    if (l > r) swap(l, r);
    chmin(l, 1);
    return l + r;
}
