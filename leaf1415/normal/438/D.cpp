...
int main(void)
{
	...
	seg.init();
	rep(i, 1, n) seg.set(i, T(P(a[i], i), a[i]));
	seg.setup();
	...
	rep(i, 1, Q){
		...
		if(t == 2){
			...
			while(1){
				...
				vec.pb(res.fi);
				seg.update(res.fi.se, T(P(-inf, -inf), -inf));
			}
			for(auto p : vec){
				p.fi %= x;
				seg.update(p.se, T(p, p.fi));
			}
		}
		if(t == 3){
			cin >> k >> x;
			seg.update(k, T(P(x, k), x));
		}
	}
