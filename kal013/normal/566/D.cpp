...
int nx(set<pair<int,int>>& S, int z){
	auto it = S.upper_bound({z, 1000000});
	if (it == S.begin()){
		assert(false);
		return z + 1;
	}
...
void solve(){
	...
	for(int j = 0;  j< q; ++j){
		...
		if (t == 1){
			...
			while(x < y){
				int v = nx(comp[Find(x)], x);

				if (v > y){
					break;
				}
				assert(Find(x) != Find(v));
