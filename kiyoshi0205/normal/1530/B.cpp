...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep(i,W)if(~i&1)ans[0][i]='1';
  rep1(i,H-1){
    if(ans[i-1][0]=='0'&&ans[i-1][1]=='0')ans[i][0]='1';
    if(ans[i-1][W-1]=='0'&&ans[i-1][W-2]=='0')ans[i][W-1]='1';
  }
  rep(j,W){
    if(ans[H-2][j]=='1')continue;
    if(j&&ans[H-1][j-1]=='1')continue;
    if(j&&ans[H-2][j-1]=='1')continue;
    if(j+1<W&&ans[H-2][j+1]=='1')continue;
    ans[H-1][j]='1';
  }
