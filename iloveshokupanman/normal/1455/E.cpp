...
int main() {
    ...
    rep(test, T) {
        ...
        do {
            ll sum = 0;           
            sum += abs(x[v[0]] - x[v[1]]) + abs(x[v[2]] - x[v[3]]);
            sum += abs(y[v[0]] - y[v[2]]) + abs(y[v[1]] - y[v[3]]);

            ll k = mn(x[v[0]], x[v[1]], x[v[2]], x[v[3]]) - mx(y[v[0]], y[v[2]], y[v[1]], y[v[3]]);
            ll l = mn(y[v[0]], y[v[2]], y[v[1]], y[v[3]]) - mx(x[v[0]], x[v[1]], x[v[2]], x[v[3]]);

            sum += max<ll>({ 2*k,2*l,0 });
            ans = min(sum, ans);
