...
int main() {
	...
	vector<Mint> f(MAXN + 1, Mint(1, MOD107)), invf = f;
	for (int i = 2; i <= MAXN; i++) f[i] = f[i - 1] * i;
	invf[MAXN] = f[MAXN].inv();
	for (int i = MAXN; i >= 2; i--) invf[i - 1] = invf[i] * i;

	vector<Mint> poly(MAXN + 1, Mint(0, MOD107));
	for (int i = 0; i <= MAXN; i++)
		poly[i] = f[MAXN] * invf[i] * invf[MAXN - i];
	...
	vector<Mint> ans(3 * n + 1, Mint(0, MOD107));
	for (int i = 3 * n; i >= 0; i--) {
		ans[i] = poly[i + 3];
		poly[i + 2] -= poly[i + 3] * 3;
		poly[i + 1] -= poly[i + 3] * 3;
