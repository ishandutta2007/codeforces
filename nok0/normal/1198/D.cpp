...
void main_() {
	...
	auto dp = [&](auto dp, int u, int d, int l, int r) {
		if(memo[u][d][l][r] != -1) return memo[u][d][l][r];
		if(d - u == 1 and r - l == 1) return memo[u][d][l][r] = b[u][l];
		int ret = max(d - u, r - l);
		REP(k, u + 1, d) {
			chmin(ret, dp(dp, u, k, l, r) + dp(dp, k, d, l, r));
		}
		REP(k, l + 1, r) {
			chmin(ret, dp(dp, u, d, l, k) + dp(dp, u, d, k, r));
		}
		return memo[u][d][l][r] = ret;
	};
	print(dp(dp, 0, n, 0, n));
}