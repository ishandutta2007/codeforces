...
il pll operator-( const pll &a , const pll &b ){
...
int main(){
	...
	For(i,n/2){
		pll p1 = p[i+1]-p[i];
		pll p2 = p[i+n/2+1] - p[i+n/2];
		#define x F
		#define y S
		if( p1.x*p2.y - p1.y*p2.x == 0 and p1.x*p2.x + p1.y*p2.y < 0 and Sqr(p1.x)+Sqr(p1.y) == Sqr(p2.x)+Sqr(p2.y) )
			continue;
		END("No");
		#undef x
		#undef y
