...
void solve() {
  ...
  ll ev = 0, od = 0;
  FOR(i, N) {
    if (ss.count(A[i]) || (A[i] % 2 == 0 && ev == N / 2)
        || (A[i] % 2 == 1 && od == N / 2)) {
      A[i] = 0;
      ++cost;
    } else {
  ...
  ll nxt_ev = 2, nxt_od = 1;
  FOR(i, N) if (A[i] == 0) {
    while (ss.count(nxt_ev)) nxt_ev += 2;
    while (ss.count(nxt_od)) nxt_od += 2;
    if (ev < N / 2) {
      ev++;
      A[i] = nxt_ev;
      ss.insert(A[i]);
      if (nxt_ev > M) return print(-1);
    }
    elif (od < N / 2) {
      od++;
      A[i] = nxt_od;
      ss.insert(A[i]);
      if (nxt_od > M) return print(-1);
    }
  }
  print(cost);
  print(A);
