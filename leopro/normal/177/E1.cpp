...
int days(vector<int> &a, vector<int> &b, int x) {
    int ans = 0;
    for (int i = 0; i < a.size(); ++i) {
        if (5e18 / x < a[i]) return 1e10;
        if (5e18 - ((a[i] * x) / b[i]) < ans) return 1e10;
        ans += (a[i] * x) / b[i];
    }
    return ans;
}

int count(vector<int> &a, vector<int> &b, int c) {
    int L = 1, R = 1e18;
    while (R - L > 1) {
        int M = L + (R - L) / 2;
        (days(a, b, M) > c ? R : L) = M;
    }
    return days(a, b, L) > c ? L : R;
}
...
void solve() {
    ...
    cout << count(a, b, c) - count(a, b, c - 1);
}