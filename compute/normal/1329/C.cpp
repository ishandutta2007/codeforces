...
void dfs(int u)
{
    ...
    int l=0,r=0;
    //dbg(u);
    while(l!=st[lson].size()&&st[lson][l].first<max(dp[lson],dp[rson])) l++;
    while(r!=st[rson].size()&&st[rson][r].first<max(dp[lson],dp[rson])) r++;
    while(l!=st[lson].size()||r!=st[rson].size())
    {
        if(l==st[lson].size())
            st[u].push_back(st[rson][r++]);
        else if(r==st[rson].size())
            st[u].push_back(st[lson][l++]);
        else{
            if(st[lson][l]<st[rson][r])
            {
                st[u].push_back(st[lson][l]);
                l++;
            }
    ...
    //for(auto u:st[u]) dbg(u.first,u.second);
    st[lson].clear();st[rson].clear();
    st[u].push_back(make_pair(a[u],u));
    if(u<(1<<g))
    {
        ans+=st[u].begin()->first;
        dp[u]=st[u].begin()->first;
        op[st[u].begin()->second]=0;
