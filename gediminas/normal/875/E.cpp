...
bool check(int l, ll a, ll b) {
	...
	for (int i = 1; i <= n + 1; i++) {
		...
		d.erase(d.begin(), d.lower_bound(r[i] - l));
		d.erase(d.upper_bound(r[i] + l), d.end());

		auto it = d.lower_bound(r[i] - l);

		if (it != d.end() and * it <= r[i] + l)
			d.insert(r[i]);
	}

	return d.count(r[n + 1]);
}
