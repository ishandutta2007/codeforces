...
void solve(){
    ...
    VEC(ll,x,n);
    vector<ll>fire,ice;
    rep(n)(t[i]?fire:ice).push_back(x[i]);
    sor(fire);
    sor(ice);
    rev(fire);
    rev(ice);
    if(fire.size()!=ice.size()){
        ll ans=sum(fire)+sum(ice);
        while(fire.size()>ice.size())fire.pop_back();
        while(fire.size()<ice.size())ice.pop_back();
        ans+=sum(fire)+sum(ice);
        return out(ans);
    }
    if(fire.back()>ice.back())swap(fire,ice);
    out(sum(fire)*2+sum(ice)*2-fire.back());
}
int main(){
    LL(t);
    rep(t)solve();
