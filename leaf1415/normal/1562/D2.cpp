...
int main(void)
{
	...
	while(T--){
		...
		ll logn = digitnum(n, 2);
		DisjointSparseTable dst(logn), dst2(logn);
		rep(i, 0, n) dst.set(i, sum[i]), dst2.set(i, -sum[i]);
		dst.calc(), dst2.calc();
		...
		rep(i, 1, Q){
			...
			vector<ll> vec;
			if((r-l+1) % 2 == 0) vec.push_back(l), l++;
			
			ll L = sum[l-1], R = sum[r];
			if(L < R){
				ll M = L + ceil(R-L, 2);
				ll ub = r, lb = l-1, mid;
				while(ub-lb>1){
					mid = (ub+lb)/2;
					if(dst.query(l, mid) >= M) ub = mid;
					else lb = mid;
				}
				vec.push_back(ub);
			}
