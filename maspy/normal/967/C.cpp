...
void solve() {
  ...
  s1.insert(INF);
  sv.insert(INF);
  auto find_lr = [&](set<ll>& ss, ll y) -> pi {
    pi res;
    auto it = ss.lower_bound(y);
    res.se = *it;
    --it;
    res.fi = *it;
    return res;
  ...
  auto f = [&](ll x1, ll y1, ll x2, ll y2) -> ll {
    ...
    ll res = INF;
    {
      auto [ly, ry] = find_lr(s1, y1);
      ll lt = abs(ly - y1) + abs(ly - y2);
      ll rt = abs(ry - y1) + abs(ry - y2);
      ll t = min(lt, rt) + abs(x1 - x2);
      chmin(res, t);
    }
    {
      auto [ly, ry] = find_lr(sv, y1);
      ll lt = abs(ly - y1) + abs(ly - y2);
      ll rt = abs(ry - y1) + abs(ry - y2);
      ll t = min(lt, rt) + ceil(abs(x1 - x2), v);
      chmin(res, t);
    }
    return res;
  };
