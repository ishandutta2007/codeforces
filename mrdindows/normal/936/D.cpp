...
int32_t solve() {
    ...
    for (int i = 1; i < allV.size(); ++i) {
        ...
        bool e[2];
        FJ(2) {
            e[j] = *lower_bound(v[j].begin(), v[j].end(), x) == x;
            ::e[j][i] = e[j];
        }
        FJ(2) {
            dp[j][i] = -INF;
            if (!e[j]) {
                if (dp[j][i - 1] >= 0) {
                    dp[j][i] = dp[j][i - 1] + x - prevX;
                }
            } else if (dp[j][i - 1] + x - prevX - 1 >= t) {
                dp[j][i] = dp[j][i - 1] - t + x - prevX;
            }
        }
        FJ(2) {
            if (!e[j]) {
                if (dp[1 - j][i] >= 0) {
                    dp[j][i] = max(dp[j][i], min(dp[1 - j][i], t));
                }
            }
        }
//        DBN(i, x, dp[0][i], dp[1][i]);
    }
    vector<pii> vs;
    vector<int> vc;
    FJ(2) {
        if (dp[j][allV.size() - 1] >= 0) {
            cout << "Yes" << endl;
            int cur = j;
            int shots = 0;
            for (int i = (int)allV.size() - 1; i > 0; --i) {
                int x = allV[i];
                int prevX = allV[i - 1];
                if (e[cur][i]) {
                    ++shots;
                }
                if (!e[cur][i] && dp[cur][i] == dp[cur][i - 1] + x - prevX) {
                    continue;
                } else if (e[cur][i] && dp[cur][i] == dp[cur][i - 1] + x - prevX - t) {
                    continue;
                } else {
                    int bal = dp[cur][i];
                    int pos = x;
