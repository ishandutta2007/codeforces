...
public class C3 {
	...
	void solve()
	{
		...
		while(!q.isEmpty()){
			...
			if(nex == n)continue;
			int nnex = lst.next(nex+1);
//			if(sums[cur] / (nex-cur) > sums[nex] / (nnex - nex)){
			if(sums[cur] * (nnex - nex) > sums[nex] * (nex-cur)){
//				tr(cur, nex, sums[cur], sums[nex]);
				sums[cur] += sums[nex];
				sums[nex] = 0;
				lst.unset(nex);
				q.add(cur);
		...
		for(int i = 0;i < n;i++){
			if(lst.get(i)){
				int nex = lst.next(i+1);
				double ave = (double)sums[i] / (nex-i);
				String d = dtos(ave, 11);
