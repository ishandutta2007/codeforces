...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n-2) ++cnt[s[i]];
	auto lb=rem,rb=rem;
	cauto no=[](){ cout<<"No"<<endl; exit(0);};
	reps(c,'a','z'+1){
		cauto t=cnt[c];
		out((char)c,t,lb,rb,1);
		if((lb+t)%2) ++lb;
		if((rb+t)%2) --rb;
		if(lb>rb) no();
		lb=(lb-t)/2;
	...
	// const int n=8;
	// set<string> st;
	// vector<int> ind(n-1); iota(all(ind),1);
	// do{
	// 	string s(n,'1');
	// 	set<int> is{0};
	// 	for(cauto i:ind){
	// 		auto it=is.insert(i).X;
	// 		--it;
	// 		reps(j,*it,i) s[j]^=1;
	// 	}
	// 	st.emplace(s);
	...
	// for(cauto s:st) cout<<s NL;
	// for(cauto s:st) assert(s[n-2]=='0' &&  s[n-1]=='1');
	// out(st.size(),1);
  return 0;
}