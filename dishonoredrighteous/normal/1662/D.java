...
public class A {
	...
	public void solve() {
		...
		f : for (int qwerty = 0; qwerty < t; qwerty++) {
			...
			if ((ua - va) % 2 != 0 || (ub - vb) % 2 != 0 || (uc - vc) % 2 != 0) {
				out.println("NO");
				continue;
			}
			Stack<Character> stu = new Stack<>();
			Stack<Character> stv = new Stack<>();
			for (int i = 0; i < u.length(); i++) {
				if (u.charAt(i) != 'B') {
					if (stu.size() > 0 && stu.peek() == u.charAt(i)) {
						stu.pop();
					} else {
						stu.add(u.charAt(i));
					}
			...
			for (int i = 0; i < v.length(); i++) {
				if (v.charAt(i) != 'B') {
					if (stv.size() > 0 && stv.peek() == v.charAt(i)) {
						stv.pop();
					} else {
						stv.add(v.charAt(i));
					}
