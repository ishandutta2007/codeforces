...
priority_queue<int,vi,greater<int> > satu,dua,tiga;

int t[100005];

bool can(int T)
{
	if (SIZE(satu) == n1 && satu.top() + t1 > T) return false;
	if (SIZE(dua) == n2 && dua.top() + t2 > T + t1) return false;
	if (SIZE(tiga) == n3 && tiga.top() + t3 > T + t1 + t2) return false;
	return true;
}
