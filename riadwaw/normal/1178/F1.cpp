...
class F1KorotkayaRaznocvetnayaPoloska {
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		for (int len: inclusiveRange(1, n)) {
			for (int l: inclusiveRange(0, n - len)) {
				int r = l + len;

				int t = min_element(c.begin() + l, c.begin() + r) - c.begin();
				Z leftSum;
				for (int i: inclusiveRange(l, t)) {
					leftSum += dp[l][i] * dp[i][t];
				}
				Z rightSum;
				for (int i: inclusiveRange(t + 1, r)) {
					rightSum += dp[t + 1][i] * dp[i][r];
				}
...
int main() {
	...
	F1KorotkayaRaznocvetnayaPoloska solver;
