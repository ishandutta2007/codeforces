...
void solve()
{
    ...
    ll ans = 1e18;
    for (int ass = 0; ass<=n-1; ass++)
    {
        ll sum = pref[n-ass-1]-a[0];
        ll need = sum - k + 1ll*(ass+1)*a[0];

        ll ops = 0;
        if (need>0) ops = (need+ass)/(ass+1);
        ans = min(ans, ops + ass);
        //sum + (ass+1)*a[0] - (ass+1) * ops<=k -> ops<=
        //need (ass+1)*ops to be <=remain
    }
