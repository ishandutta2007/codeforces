...
int main() {
    ...
    rep(v, N) {
        ...
        while (!q.empty()) {
            ...
            set<ll> er;
            for (ll x : st) {
                if (x == 0 || g[w].count(x)) { continue; }
                er.insert(x);
                q.push(x);
                gr[x] = L - 1;
            }
            for (ll x : er) { st.erase(x); }
        }
    }
    ...
    set<ll> con;
    repn(i, N - 1)con.insert(i);
    for (ll x : g[0])con.erase(x);

    if (L > K) { cout << "impossible" << endl; return 0; }
    if (con.size() < K) { cout << "impossible" << endl; return 0; }

    set<ll> congr;
    for (ll x : con) { congr.insert(gr[x]); }
    if (congr.size() != L) { cout << "impossible" << endl; }
