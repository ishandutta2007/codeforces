...
iter_type<I> drop_while(const F& f, const iter_type<I>& it) {
    I b = it.begin(), e = it.end();
    while(b != e and f(*b)) {
        ++b;
    }
...
template<typename I, typename F>
iter_type<take_while_iterator<I, F>> take_while(const F& f, const iter_type<I>& it) {
    return iter_type<take_while_iterator<I, F>>(take_while_iterator<I, F>(f, it.begin(), it.end()), take_while_iterator<I, F>(f, it.end(), it.end()));
}

template<typename I, typename F, typename T>
class take_while_iterator : public iterator_base<T> {
    friend iter_type<take_while_iterator<I, F>> take_while<>(const F&, const iter_type<I>&);
...
void traverse_divisors(vector<LLI>& P, LL m, int i) {
    if(i == int(P.size())) {
        //cerr << m << endl;
        LL prob = 1;
        for(int j = 0; j < int(P.size()); ++j) {
            prob = (prob * dp[j][K][P[j].second]) % MOD;
        }
        ans = (ans + prob * (m % MOD)) % MOD;
        return;
    }
    if(P[i].second == 0) {
        traverse_divisors(P, m, i + 1);
        return;
    }
    P[i].second--;
    traverse_divisors(P, m, i);
    P[i].second++;
    traverse_divisors(P, m * fast_exp(P[i].first, P[i].second), i + 1);
}
...
int main() {
    ...
    for(int i = 0; i < int(P.size()); ++i) {
        ...
        for(int k = 1; k <= K; ++k) {
            for(int j = 0; j <= l; ++j) {
                for(int m = j; m <= l; ++m) {
                    dp[i][k][j] = (dp[i][k][j] + dp[i][k - 1][m] * inv[m + 1]) % MOD;
                }
                //cerr << i << " " << j << " " << k << " " << dp[i][k][j] << endl;
            }
        }
    }
    traverse_divisors(P, 1, 0);
