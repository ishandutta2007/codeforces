...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    cin >> v;
    bool ok = 1;
    set<pair<int, int>> to_swap;
    for (int i = 0; i < n; ++i) {
      vector<int> r = v[i];
      sort(r.begin(), r.end());
      vector<int> d;
      for (int j = 0; j < m; ++j) if (r[j] != v[i][j]) {
        d.push_back(j);
      }
      if (d.size() > 2) {
        ok = 0;
    ...
    if (ok)
    for (int i = 0; i < n; ++i) {
      auto [s0, s1] = *to_swap.begin();
      swap(v[i][s0 - 1], v[i][s1 - 1]);
      ok &= is_sorted(v[i].begin(), v[i].end());
    }
    if (ok) {
      print(*to_swap.begin());
    } else {
