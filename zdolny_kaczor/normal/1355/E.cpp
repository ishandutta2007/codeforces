...
ll query(int H) {
	...
	ll ms = min(inc, dec);
	ll ans = min(inc * a + dec * r, ms * m + (inc - ms) * a + (dec - ms) * r);
	// debug(imie(H), imie(ans));
	rans;
}
int sign(ll x) {
	if (x > 0) return 1;
...
int main() {
	...
	mini(m, a + r);
	REP(i, n) scanf("%d", h + i);
	for (int H = 1; H <= 200; ++H) debug(fun(query, H));
	ll ans = numeric_limits<ll>::max();
	ll low = 0, high = 1e9;
	while (low <= high) {
		ll m1 = (low * 2 + high) / 3, m2 = (low + high * 2) / 3;
		debug(imie(low), imie(m1), imie(m2), imie(high));
		ll v1 = query(m1), v2 = query(m2);
		mini(ans, v1);
		mini(ans, v2);
		if (v1 >= v2) low = m1 + 1;
		else high = m2 - 1;
	...
	for (int i = low; i <= high; ++i) mini(ans, query(i));
	vector <ll> s;
	for (int i = low - 10; i <= high + 10; ++i) s.push_back(query(i));
	vi s2;
	REP(i, s.size() - 1)
		s2.push_back(sign(s[i + 1] - s[i]));
	assert(is_sorted(ALL(s2)));
	debug(imie(s), imie(s2));
	printf("%lld\n", ans);
	exit(0);
}
//#STAY AT HOME