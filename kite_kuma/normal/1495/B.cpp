...
int main() {
	...
	vi lef_dn(n), rig_dn(n), lef_up(n), rig_up(n);

	lef_dn[0] = 0;
	rep(i, 1, n) {
		if(p[i - 1] < p[i])
			lef_dn[i] = lef_dn[i - 1] + 1;
		else
			lef_dn[i] = 0;
	}
	...
	int maxi_after = -1;
	foa(t, rig_up) chmax(maxi_after, t);
	foa(t, lef_up) chmax(maxi_after, t);

	vi data(n);
	rep(i, n) data[i] = max(rig_up[i], lef_up[i]);

	ST<int> st(
		n, [](int a, int b) { return max(a, b); }, -inf, data);

	auto chk = [&](int x) {
		// if(x == 0 or x == n - 1) return false;
		// if(p[x - 1] > p[x] or p[x + 1] > p[x]) return false;
		...
		if(r % 2 == 0) {
			int y = x + r;
			if(!(rig_dn[x] > rig_up[y] || lef_dn[x] > max(rig_up[y], lef_up[y]))) {
				return false;
			}
			r--;
		}
		int y = x + r;
		if(lef_dn[x] <= max(lef_up[y], rig_up[y])) return false;
		...
		if(r % 2 == 0) {
			int y = x - r;
			if(!(lef_dn[x] > lef_up[y] || rig_dn[x] > max(lef_up[y], rig_up[y]))) {
				return false;
			}
			r--;
		}
		y = x - r;
		if(rig_dn[x] <= max(lef_up[y], rig_up[y])) return false;

		debug(rig, lef);
		int gote = st.get(rig + 1, n);
		chmax(gote, st.get(0, lef));
		...
		if(max(lef_dn[x], rig_dn[x]) <= gote) return false;
