...
using modular = SafeModularInt32<998244353>;
...
void solve()
{
	...
	for (int s = 0; s < 1 << m; ++s)
	{
		...
		int cur = n / 2;
		vector<int> v(cur);
		for (int i = 0; i < cur; ++i) v[i] = i;
		int id = 0;
		for (int turn = k - 1; turn >= 1; --turn)
		{
			vector<int> w(cur / 2);
	...
	for (int s = 0; s < 1 << m; ++s)
	{
		...
		for (int rk = 1; rk <= 2; ++rk)
		{
			...
			//write(tmp.GetValue(), ' ');
			//print(r);
			if (!F[3 - rk][(h - tmp).GetValue()].empty())
			{
				vector<int> rr = *F[3 - rk][(h - tmp).GetValue()].begin();
				for (int i = 0; i < n; ++i)
					rr[i] += r[i];
				print(rr);
				return;
