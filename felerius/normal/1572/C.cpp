// Paint (https://codeforces.com/contest/1572/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        rep(i, n)
            by_col[a[i]].push_back(i);
        vector prev(n, -1), next(n, n);
        trav(idx, by_col)
            rep(i, sz(idx) - 1)
                prev[idx[i + 1]] = idx[i], next[idx[i]] = idx[i + 1];

        vector dp(n + 1, vector(n + 1, 0));
        rep(w, 1, n + 1) rep(l, n - w + 1) {
            int r = l + w;
            auto ans = dp[l + 1][r];
            for (int m = next[l]; m < r; m = next[m])
                ans = max(ans, 1 + dp[l + 1][m] + dp[m][r]);
            dp[l][l + w] = ans;
        }

        println(n - 1 - dp[0][n]);

        // 1  3 2 1 2 3  1  3 2 1 2 3  1
        // 1 2 1 3 1
