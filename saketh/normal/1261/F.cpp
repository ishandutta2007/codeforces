...
struct trie {
    ...
    void insert_range(ll L, ll R, int cur = 0, ll cl = 0, ll cr = 1ll << MAXB) {
        assert(R > cl && cr > L);
        ...
        if (cm > L)
            insert_range(L, R, get(cur, 0), cl, cm);
        if (cm < R)
            insert_range(L, R, get(cur, 1), cm, cr);
    }
};
...
int main() {
    ...
    auto calc = [&](auto &self, int bits, ll pref) -> void {
        for (auto [l0, l1] : pos[bits]) {
            if (ts[0].full[l0] || ts[1].full[l1]) {
                ll range_sz = 1ll << bits;
                ll range_avg = pref + (pref + range_sz - 1); // div 2 at the end
                ans += mn(range_sz) * range_avg;
                return;
            }
        }
        if (bits == 0) assert(0);
        ...
        F0R (nbit, 2) {
            ...
            for (auto [l0, l1] : pos[bits]) {
                F0R (left, 2) {
                    ...
                    int lc = ts[0].t[l0][left];
                    int rc = ts[1].t[l1][right];
            ...
            if (!pos[bits - 1].empty())
                self(self, bits-1, pref + (ll(nbit) << (bits - 1)));
        }
    };
