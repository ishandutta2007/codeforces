...
void solve(){
	...
	for(int i = 1; i + k - 1 <= n; i++){
		long long fc = 0; // off contribution
		t.clear();
		// t.build(emp);
		vector<int> v[N];
		rep(z,0,m){
			int overlapi = min(i+k-1,r[z]) - max(i,l[z]) + 1;
			int overlapj = min(n,r[z]) - max(n-k+1,l[z]) + 1;
			amax(overlapi,0);
			amax(overlapj,0);
			if(overlapj >= overlapi){
				t.rupd(l[z],r[z],1);
			}
			else{
				// max j such that overjapj >= overlapi
				// maxj = 1 + r[z] - overlapi;
				int b = r[z] + 1 - overlapi;
				v[b].pb(z);
				fc += overlapi;
			}
		}
		for(int j = n - k + 1; j >= 1; j--){
			...
			long long tmp = fc + t.query(j,j+k-1).v;
			amax(ans,tmp);
		}
	}
	cout << ans;
