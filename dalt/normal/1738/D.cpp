...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  var cut = [&](int l, int r) {
    ...
    Chmax(range.first, l);
    Chmin(range.second, r);
    DebugFmtln("cut [%d, %d)", l, r);
  };
  for(int i = 0; i < N; i++) {
    cut(i + 1, B[i]);
  }
  ...
  int K = range.first;
  Vec<int> indeg(N + 2);
  Vec<Vec<int>> has_edge(N + 2);
  Vec<Vec<int>> g(N + 2);
  for(int i = 0; i < N; i++) {
    has_edge[B[i]].push_back(i + 1);
    g[i + 1].push_back(B[i]);
    indeg[B[i]]++;
  }
  for(int i = 0; i <= N + 1; i++) {
    ...
    Sort(All(has_edge[i]), [&](int a, int b) {
      return indeg[a] < indeg[b];
    });
    int end = has_edge[i].back();
    has_edge[i].pop_back();
    for(var j : has_edge[i]) {
      g[end].push_back(j);
    }
  }
  Debug(g);
  Vec<bool> visited(N + 2);
  Vec<int> seq;
  var topo = [&](var &dfs, int root) {
    if (visited[root]) {
      return;
    }
    visited[root] = true;
