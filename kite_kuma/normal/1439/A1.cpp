...
void sol() {
	...
	vvi a(n, vi(m));
	rep(i, n) rep(j, m) if(s[i][j] == '1') a[i][j] = 1;
	...
	int yoko = m - 2;
	rep(i, n - 2) {
		if(a[i][yoko] == 0 && a[i][yoko + 1] == 0) continue;
		int cnt = 3;
		vvi chk(2, vi(2));
		rep(x, 2) rep(y, 2) {
			if(cnt == 0) goto sp;
	...
	yoko = m - 2;
	int cnt = 0;
	rep(x, 2) rep(y, 2) { cnt += a[tate + x][yoko + y]; }
	...
	if(cnt == 1) {
		...
		rep(x, 2) rep(y, 2) {
			if(a[tate + x][yoko + y]) {
				if(one == 0) continue;
				a[tate + x][yoko + y] = 0;
				ans.push_back(tate + x);
				ans.push_back(yoko + y);
				one--;
			} else {
				if(!z) continue;
				a[tate + x][yoko + y] = 1;
