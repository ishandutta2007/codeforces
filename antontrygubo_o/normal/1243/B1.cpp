...
void solve()
{
    ...
    for (int i = 0; i<n; i++) if (S[i]!=T[i]) bad.push_back(mp(S[i], T[i]));
    if (bad.size()==0) {cout<<"Yes"<<endl; return;}
    if (bad.size()!=2) {cout<<"No"<<endl; return;}
    if (bad[0].first!=bad[1].first||bad[0].second!=bad[1].second) cout<<"No"<<endl;
    else cout<<"Yes"<<endl;
}
