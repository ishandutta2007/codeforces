/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 372 (Div. 1)         |
|       Task: B                                     |
|       Date: 17. 09. 2016                          |
\--------------------------------------------------*/
// Includes
...
struct edge { int s, t, w; bool q; };

int n = in, m = in, l = in, s = in, t = in;
vector <edge> e(m);
vector < vector < pair<int, int> > > g(n);
vector < pair <int, int> > p(n);
vector <int> d(n);

inline int dst(pair <int, int> q) { return (q.F < 0) ? e[q.S].s : e[q.S].t; }
inline int src(pair <int, int> q) { return (q.F < 0) ? e[q.S].t : e[q.S].s; }
inline int way(pair <int, int> q) { return e[q.S].w; }
...
void dSearch()
{
    ...
    while (!q.empty())
    {
        pair <int, int> pv = q.top(); q.pop();
		int v = pv.S, path = -pv.F;
		if (path > d[v]) continue;

		for (int j = 0; j < sz(g[v]); j++)
        {
			int to = dst(g[v][j]), len = way(g[v][j]);
			if (path + len < d[to])
            {
				d[to] = path + len;
				p[to] = g[v][j];
...
main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        g[e[i].s].push_back({1, i});
        g[e[i].t].push_back({-1, i});
    }
