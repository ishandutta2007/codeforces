...
bool overflow(ll a, ll x) {
	return (a > LLONG_MAX / x);
}
...
int main() {
	doin();
	for (ll t, j, i = 3; i < 60; i++) {
		for (t, j = 2; (t = bp(j, i)) != 1 && t <= 1000000000000000000ll; j++)
			v.pb(t);
// 		cout << j << "\n";
	}
	sort(all(v));
...
//	cout << v.size() << "\n";
	for (int i = 1; i < v.size(); i++) {
		ll t = sqrt(v[i]);
		b[i] = (t * t == v[i] || t * t + 2 * t + 1 == v[i]
				|| t * t - 2 * t + 1 == v[i]) + (i ? b[i - 1] : 0);
// 		if((t*t==v[i]||t*t+2*t+1==v[i]||t*t-2*t+1==v[i]))
// 			cout << v[i] << "\n";
	}
	cin >> q;
	while (q--) {
		...
		while ((x + 1) * (x + 1) <= l)
			x++;
		ans -= x;
		ll i = upper_bound(all(v), r) -1- v.begin();
		ll j = upper_bound(all(v), l) - 1- v.begin();
		ans += i - j;
		ans -= b[i] - b[j];
		cout << ans << "\n";
	}
