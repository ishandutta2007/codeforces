...
void solve() {
	...
	for (int l = 1; l <= 2 * n; ++l) for (int i = 1; i <= n; ++i) {
		int j = l - i;
		if (j <= 0 || j > n) continue;
		assert(dp[i][j] == -inf);
		int fi = p[2 * n - l], se = p[2 * n - l + 1];
		if (fi < se && dp[j][i - 1] != -inf) {
			maxi(dp[i][j], se);
		}
		int v = dp[i - 1][j];
		if (v > fi) maxi(dp[i][j], v);
		debug(arr(dp, i, j), range(p + i + j, p + n * 2));
	}
	puts(dp[n][n] == -inf ? "NO" : "YES");
}
int main() {
