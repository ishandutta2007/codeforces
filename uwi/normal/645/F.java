...
public class F {
	...
	void solve()
	{
		...
		int[] lpf = enumLowestPrimeFactors(y);
		int[] tots = enumTotientByLpf(y, lpf);
		
		int n = ni(), K = ni(), q = ni();
		long[] ck = new long[z+1];
		for(int i = 0;i <= z;i++){
			ck[i] = C(i, K, mod, fif);
		}
		int[] a = na(n+q);
		...
		for(int i = 0;i < n+q;i++){
			for(int j = 1;j*j <= a[i];j++){
				if(a[i] % j == 0){
					f[j]++;
					sum += tots[j] * (ck[f[j]] - ck[f[j]-1]);
					sum %= mod;
					
					if(j*j < a[i]){
						int rj = a[i]/j;
						f[rj]++;
						sum += tots[rj] * (ck[f[rj]] - ck[f[rj]-1]);
						sum %= mod;
					}
				}
			}
