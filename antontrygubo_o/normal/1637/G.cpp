...
void solve(int n, int depth)
{
    ...
    have.push_back(deg*M);
    for (int i = deg+1; i<=n; i++)
    {
        ops.push_back(mp(i*M, (2*deg-i)*M));
        have.push_back(deg*M*2);
    }

    solve(2*deg-n-1, depth);
    solve(n-deg, depth+1);
}
...
void solve()
{
    ...
    int pos = 0;
    while (have[pos]!=have[pos+1]) pos++;

    ops.push_back(mp(have[pos], have[pos]));
    have[pos] = 0; have[pos+1]*=2;

    for (int i = 0; i<n; i++) if (i!=pos)
    {
        while (have[i]!=goal)
        {
            ops.push_back(mp(have[i], 0));
            ops.push_back(mp(have[i], have[i]));
            have[i]*=2;
        }
    }
    ops.push_back(mp(0, goal));
