...
const int N = (1 << 20) + 100, mod = 998244353;
...
int main()
{
	...
	for(int i = 1; i < 20; i++){
		...
		auto ab = fft:: multiply_mod(a, b, mod);
		auto bc = fft:: multiply_mod(b, c, mod);
		auto cd = fft:: multiply_mod(c, d, mod);
		for(int j = x; j < x + x; j++){
			dp[j][0][0] = bc[j - x + 1];//dp[a][0][1] dp[b][1][0]	 a + b + 1 == j	
			dp[j][0][1] = ab[j - x + 1];//dp[a][0][0] dp[b][0][1]	 a + b + 1 == j	
			dp[j][1][0] = cd[j - x + 1];//dp[a][1][1] dp[b][1][0]	 a + b + 1 == j	
			dp[j][1][1] = bc[j - x + 1];//dp[a][1][0] dp[b][0][1]	 a + b + 1 == j	
		}
	}
	int n;
	cin >> n;
