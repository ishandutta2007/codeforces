...
fun main(){
    solve.cases{
        ...
        val cc = coordinateCompression(things)

        val st = ST(cc.forward.size)
        var inv = 0L
        for(i in 1 until n){
            val v = cc.reverse[presum[i]]!!

            if(v < cc.forward.lastIndex){
                inv += st.sumQuery(v+1, cc.forward.lastIndex)
            }
            st.add(v,1)
        }
        put(inv)
