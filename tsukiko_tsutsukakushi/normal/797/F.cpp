...
void solve() {
    rep(i, 5050) rep(j, 5050) dp[i][j] = INF;
    INT(n, m);
    VEC(int, x, n);
    Sort(x);
    vc<int> p(m), c(m);
    ...
    rep(i, m) {
        ...
        for(int j = 0; j <= n; ++ j) {
            while(si(deq) and deq.front().fr < j - c[ids[i]]) deq.ppf();
            while(si(deq) and deq.back().sc >= dp[i][j] - cost) deq.ppb();
            deq.pb(pair<int, ll>{j, dp[i][j] - cost});
            dp[i + 1][j] = deq.front().sc + cost;
            if(j < n) cost += abs(x[j] - p[ids[i]]);
        }
    }
    rep(i, m) {
        chmin(ans, dp[i + 1][n]);
