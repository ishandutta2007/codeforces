...
struct Graph{
    ...
    Graph(int n) : es(n), n(n), m(0), ls(n), rs(n), depth(n) {}
...
int main(){
    ...
    Binary_Indexed_Tree<mint> b1(vector<mint>(N+1, 0)), b2(vector<mint>(N+1, 0));
    ...
    while(Q--){
        ...
        if(t == 1){
            ...
            b1.add(G.ls[u], x), b1.add(G.rs[u], -x);
            b2.add(G.ls[u], -k), b2.add(G.rs[u], k);
        }
        else{
            cout << b1.query(0, G.ls[u]+1)+b2.query(0, G.ls[u]+1)*G.depth[u] << '\n';
        }
    }
}