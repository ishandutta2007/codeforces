...
struct N{
	...
	N(int v=-1){
		if(v==-2){
			rep(i,3)rep(j,3)dp[i][j]=inf;
		}else if(v==-1){
			rep(i,3)rep(j,3)dp[i][j]=0;
		}else{
			rep(i,3)rep(j,3)dp[i][j]=inf;
			rep(i,3)dp[i][i]=(v==i);
		}
	...
	static N merge(const N&a,const N&b){
		N res(-2);
		rep(i,3)rng(j,i,3)rng(k,j,3)rng(l,k,3)
			chmin(res.dp[i][l],a.dp[i][j]+b.dp[k][l]);
		return res;
	}
	int getans(){
		int res=inf;
		rep(i,3)rng(j,i,3)chmin(res,dp[i][j]);
		return res;
	}
};
...
void slv(){
	...
	string str;cin>>str;
	vi ini(n);
	rep(i,n)ini[i]=str[i]-'a';
	segtree<N> seg(ini);
	rep(_,q){
		int i;cin>>i;i--;
		int j=readString()[0]-'a';
		seg.set(i,j);
		print(seg.getall().getans());
	}
}
