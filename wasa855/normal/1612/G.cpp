...
const int B=1000005;
int n,a[N],d[N];
int rgsum(int l,int r) {return mul((r-l+1)%mod,(l+r)%mod,i2);}
signed main()
{
	init();
	cin>>n; for(int i=1;i<=n;i++) a[i]=read();
	for(int i=1;i<=n;i++) d[a[i]-1+B]++,d[-a[i]-1+B]--;
	for(int i=N-1;i>=0;i--) d[i]+=d[i+2];
	int ans=1,sum=0,id=0;
	for(int i=-B;i<=B;i++)
	{
		int r=mul((i+mod)%mod,rgsum(id+1,id+d[i+B]));
		sum=add(sum,r);
		ans=mul(ans,fac[d[i+B]]);
		id+=d[i+B];
	}
	printf("%d %d\n",sum,ans);
	return 0;
