...
int main(){
    ...
    rep(ii,T){
        ll N,M; cin >> N >> M;
        graph G(N);
        ll a[N]; vector<ll> in(N);
        rep(i,N) cin >> a[i];
        rep(i,M){
            ll u,v; cin >> u >> v; G.unite(u-1,v-1,1,true); in[v-1]++;
        }
        auto dp = v2ll(N,N+1,0); auto fl = v2ll(N,N+1,0);
        rep(i,N){
            dp[i][0] = a[i]%mod;
            if(a[i]>=mod) fl[i][0] = 1;
        }
        ...
        while(!go.empty()){
            ...
            vector<Pr> p; ll end = 0; 
            for(auto[x,cc,id]:G.G[s]){
                for(int i=0;i<N;i++){
                    dp[x][i+1] += dp[s][i];
                    fl[x][i+1] |= fl[s][i];
                    if(dp[x][i+1]>=mod){
                        dp[x][i+1] -= mod; fl[x][i+1] = 1;
                    }
                }
                in[x]--;
                if(in[x]==0) go.push(x);
            ...
            if(c==N){
                ...
                for(int i=0;i<=N;i++){
                    //cout << fl[s][i] << " " << dp[s][i] << "\n";
                    if(fl[s][i]==0&&dp[s][i]==0) continue;
                    if(f==0&&end<i){
                        end = i+dp[s][i]; f = fl[s][i];
                    }
                    else{
                        end += dp[s][i]; f |= fl[s][i];
                    }
