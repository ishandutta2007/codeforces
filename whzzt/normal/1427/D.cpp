...
void shift(vector<int> len){
	for(int i=0;i<len.size();++i) pos[i+1]=pos[i]+len[i];
	vector<int> ne;
	for(int i=len.size();i>=1;--i){
		for(int j=pos[i-1]+1;j<=pos[i];++j){
			ne.pb(c[j]);
		}
	}
	for(int i=0;i<ne.size();++i) c[i+1]=ne[i];
}
void work(){
	...
	while(true){
		for(int x=1;x<=n;++x) p[c[x]]=x;
		int x=1;
		while(x<n&&p[x]<p[x+1]) ++x;
		if(x==n) break;
		vector<int> cur;
		cur.pb(p[x+1]-1);
		int u=p[x+1];
