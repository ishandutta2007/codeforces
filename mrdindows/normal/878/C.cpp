...
struct Player {
    ...
    bool operator<(const Player& other) const {
        for (int i = 0; i < k; ++i) {
            if (maxPower[i] > other.minPower[i]) {
                return false;
            }
        }
        return true;
...
void merge(Player& a, Player& b) {
    for (int i = 0; i < k; ++i) {
        a.minPower[i] = min(a.minPower[i], b.minPower[i]);
        a.maxPower[i] = max(a.maxPower[i], b.maxPower[i]);
    }
    if (b.ids.size() > a.ids.size()) {
        a.ids.swap(b.ids);
    }
    for (int x : b.ids) {
        a.ids.insert(x);
...
int solve() {
    ...
    FI(n) {
        ...
        FJ(k){
            cin >> a[i][j];
            p.minPower[j] = p.maxPower[j] = a[i][j];
        }
        p.ids.insert(i);
        if (players.empty()) {
            players.push_back(p);
            ...
            if (players.empty() || p < *players.begin()) {
