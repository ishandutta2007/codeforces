...
void mian() {
	int n = read();
	int sum = 0, tot = 0;
	REP(i, 1, n) a[i] = read(), sum += (n - 2) * a[i] * a[i], tot += a[i];
	REP(i, 1, n) b[i] = read(), sum += (n - 2) * b[i] * b[i], tot += b[i];
	bitset<N * N> f; f.set(0);
	REP(i, 1, n) f = f << a[i] | f << b[i];
	int ans = inf;
	REP(i, 0, N * N - 1) if(f[i]) chkmn(ans, i * i + (tot - i) * (tot - i));
	prt(sum + ans), pc('\n');
}
