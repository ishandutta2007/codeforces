...
int main(){
	...
	while(tt --){
		...
		if(sz(factor) >= 3){
			cout << "YES\n" << calc({factor[0]}) << " " << calc({factor[1]}) << " " << calc({factor.begin() + 2, factor.end()}) << "\n";
		}
		else if(sz(factor) == 1){
			if(factor[0].second < 6){
				...
				auto [p, e] = factor[0];
				cout << "YES\n" << calc({{p, 1}}) << " " << calc({{p, 2}}) << " " << calc({{p, e - 3}}) << "\n";
			}
		}
		else{
			if(factor[0].second < factor[1].second){
				swap(factor[0], factor[1]);
			}
			auto [p, e] = factor[0];
			auto [q, f] = factor[1]; // e >= f
			if(e >= 3){
				cout << "YES\n" << calc({{p, 1}}) << " " << calc({{p, 2}}) << " " << calc({{p, e - 3}, {q, f}}) << "\n";
			}
			else if(e + f <= 3){
				cout << "NO\n";
			}
			else{
				cout << "YES\n" << calc({{p, 1}}) << " " << calc({{q, 1}}) << " " << calc({{p, 1}, {q, 1}}) << "\n";
			}
		}
	}
	return 0;
