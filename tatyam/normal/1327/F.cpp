...
signed main(){
    ...
    rep(k){
        ...
        each(i,cond){
            ...
            else zero.emplace_back(i[1],i[0]);
        }
        sort(all(zero),greater<pii>());
        rep(n)imos[i+1]+=imos[i];
        vector<Modint>s(n+2);
        s[1]=1;
        int max=0;
        rep(n){
            if(imos[i])s[i+2]=s[i+1];
            else s[i+2]=s[i+1]+s[i+1]-s[max];
            while(zero.size()&&i+1==zero.back().first){
                chmax(max,zero.back().second+1);
                zero.pop_back();
            }
        }
        ans*=s.back()-s[max];
