...
main() {
    ...
    while(T--) {
        ...
        rep(i, n) {
            rep(j, n) {
                if(i == j) continue;
                if(v[i].fi <= v[j].fi and v[j].se <= v[i].se) g[i].eb(j);
            }
            sort(all(g[i]), [&](int x, int y) { return v[x] < v[y]; });
        }
        vi ans(n);
        auto dfs = [&](auto &&f, int x) -> int {
            if(ans[x]) return ans[x];
            vi V{0};
            for(auto i : g[x]) V.eb(v[i].fi), V.eb(v[i].se);
            UNIQUE(V);
            vi dp(V.size() + 1);
            int now = 0;
            vpi w(g[x].size());
            rep(i, g[x].size()) w[i].fi = lb(V, v[g[x][i]].fi), w[i].se = lb(V, v[g[x][i]].se);
            rep(i, g[x].size()) {
                int l = lb(V, v[g[x][i]].fi), r = lb(V, v[g[x][i]].se);
                while(now < l) chmax(dp[now + 1], dp[now]), now++;
                chmax(dp[r + 1], dp[l] + f(f, g[x][i]));
            }
            return ans[x] = *max_element(all(dp)) + 1;
        };
