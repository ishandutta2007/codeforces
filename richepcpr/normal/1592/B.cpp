...
typedef long long ll;const ll N=5e6+7,mod=1e9+7;typedef ll aN[N];char is[1<<22],os[1<<24],*i1=is,*i2=is,*o1=os;aN a,b;
...
ll calc(){ll n=readll(),x=readll();rep(i,1,n)a[i]=b[i]=readll();std::sort(a+1,a+n+1);rep(i,n-x+1,x)if(a[i]!=b[i])return 0;return 1;}
