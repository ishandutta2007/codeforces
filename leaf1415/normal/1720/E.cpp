...
ll n, k;
ll a[505][505];
ll l[250005], r[250005], u[250005], d[250005];
ll dif[505][505], dif2[505][505], dif3[505][505];
ll ndif[505][505], ndif2[505][505], ndif3[505][505];
vector<ll> vec[505];
vector<P> vec2[505];
...
int main(void)
{
	...
	set<ll> S;
	rep(y, 1, n) rep(x, 1, n) S.insert(a[x][y]);
	if(sz(S) <= k){
		outl(k - sz(S));
		return 0;
	}
	...
	rep(i, 1, n*n) l[i] = u[i] = inf, r[i] = d[i] = -inf;
	rep(y, 1, n) rep(x, 1, n){
		ll c = a[x][y];
		chmin(l[c], x), chmax(r[c], x);
		chmin(u[c], y), chmax(d[c], y);
	}
	k = sz(S) - k;
	
	rep(i, 1, n*n) if(u[i] < inf/2) vec[u[i]].pb(i);
	
	per(i, n, 1){
		for(auto id : vec[i]){
			rep(j, 1, l[id]){
				ll dist = cdist(P(j, i), P(r[id], d[id]));
				ll dist2 = cdist(P(j, i-1), P(r[id], d[id]));
				//outl(dist, dist2);
				if(dist != dist2) dif[j][dist]++;
				else{
					dif2[j][dist]++;
					if(j-r[id] == i-u[id] || j == l[id]){
						ll p = d[id]-(r[id]-j);
						if(p >= 1) vec2[p].pb(P(j, dist));
					}
				}
			}
		}
		rep(j, 1, n){
			ll sum = 0;
			//cout << "A ";outa(dif[j], 0, n);
			//cout << "B ";outa(dif2[j], 0, n);
			rep(l, 0, n){
				if(i+l > n || j+l > n) break;
				if(i == 1 && j == 1 && l == n-1) break;
				sum += dif[j][l] + dif2[j][l];
				//outl(i, j, l, sum);
				if(sum == k || sum == k+1){
					outl(1);
					return 0;
		...
		for(auto p : vec2[i]) dif3[p.fi][p.se]++;
		rep(j, 1, n) rep(l, 0, n) dif2[j][l] -= dif3[j][l], dif[j][l] += dif3[j][l];
		
		rep(j, 1, n){
			ndif[j][0] = 0;
			rep(l, 1, n) ndif[j][l] = dif[j][l-1];
			rep(l, 0, n) ndif2[j][l] = dif2[j][l];
			rep(l, 0, n){
				ndif3[j][l] = 0;
				if(j+1 <= n && l-1 >= 0) ndif3[j][l] = dif3[j+1][l-1];
			}
		}
		
		rep(j, 1, n) rep(l, 0, n) dif[j][l] = ndif[j][l], dif2[j][l] = ndif2[j][l], dif3[j][l] = ndif3[j][l];
	}
	outl(2);
