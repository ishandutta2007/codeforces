...
int main() {
    ...
    for (int test = 1; test <= tests; test++) {
        int n, k;
        cin >> n >> k;
        int answer = 1, denominator = 1;
        for (int i = 1; i + (i - 1) * (k - 1) <= n; i++) {
            int s = n - i - (i - 1) * (k - 1);
            int t = i + 1;
            denominator = 1LL * denominator * (n - i + 1) % MOD;
            answer = (1LL * combinations(t + s - 1, t - 1) * raiseToPower(denominator, MOD - 2) % MOD * factorial[i] + answer) % MOD;
        }
        cout << answer << "\n";
    }
    return 0;
