...
struct lca_lift {
  ...
  void init(int sz) {
    ...
    sum = vector<vector<ll> >(n, vector<ll>(lg, 0LL));
  }
...
void glen(int x) {
  ...
  for (int i = lca.lg - 1; i >= 0; i--) {
    if ((1 << i) <= lca.depth[foc]) {
      if (lca.sum[foc][i] <= left) {
        left -= lca.sum[foc][i];
        ans += (1 << i);
        foc = lca.lift[foc][i];
      }
...
void dfs(int x) {
  ...
  for (int y: child[x]) {
    dfs(y);
    num[x] += num[y];
    if (top[y] > 1) top[x] = max(top[x], top[y] - 1);
  }
