...
int main() {
	...
	vp.emplace_back(0, 1, 1);
    FOR(i, n) {
        int t, x, y; reader(t, x, y);
		vp.emplace_back(t, x, y);
    }
	memset(memo, 0x8f8f8f8f, sizeof memo);
	memo[0] = 0;
	int last = 0;
	int mx = -2 * n;
	for(int i = 1; i <= n; i++) {
		int t, x, y;  tie(t, x, y) = vp[i];
		if (last < i && get<0>(vp[last]) + 1000 <= t) {
			mx = max(mx, memo[last]);
			last++;
		}
		int cur = mx;
		for (int j = last; j < i; j++) {
			int a, b, c; tie(a, b, c) = vp[j];
			int dt = t - a;
			int dx = abs(b - x);
			int dy = abs(c - y);
			if (dx + dy <= dt) {
