...
void solve(){
    ...
    //f(t)=1
    ll F=-1;
    rep(i,0,25)if(cnt[i]>=1){F=i;break;}
    assert(F!=-1);
    ll n=s.size();
    ll A=cnt[F];
    ll B=n-A;
    ...
    if(A>=B+3){
        ll G=-1;
        rep(i,F+1,25){
            if(cnt[i]>=1){G=i;break;}
        }
        {
            set<char>sss;
            for(auto x:s)sss.insert(x);
            if(sss.size()==2){
                //FAGB
                ans+=(char)('a'+F);
                rep(i,1,B)ans+=(char)('a'+G);
                rep(i,1,A-1)ans+=(char)('a'+F);
                assert(ans.size()==n);
                cout<<ans<<endl;
                return;
            }
        ...
        ll H=-1;
        rep(i,G+1,25){
            if(cnt[i]>=1){H=i;break;}
        }
        assert(H!=-1);
        ans+=(char)('a'+F);
        ans+=(char)('a'+G);
        cnt[F]--;cnt[G]--;
        rep(kaikai,1,cnt[F])ans+=(char)('a'+F);
        ans+=(char)('a'+H);cnt[H]--;
        rep(i,F+1,25){
            rep(kaikai,1,cnt[i])ans+=(char)('a'+i);
        }
        assert(ans.size()==n);
        cout<<ans<<endl;
        return;
        ...
        string a;
        rep(kaikai,1,A-2)a+=(char)('a'+F);
        rep(kaikai,1,2)ans+=(char)('a'+F);
        ll as=a.size();
