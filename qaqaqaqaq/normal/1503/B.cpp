...
signed main(){
    ...
    while (sz(evn)&&sz(odd)){
        ...
        if (x==1){
            auto ret=*evn.begin();
            evn.erase(ret);
            cout<<2<<" "<<ret.F<<" "<<ret.S<<endl;
        }
        else{
            auto ret=*odd.begin();
            odd.erase(ret);
            cout<<1<<" "<<ret.F<<" "<<ret.S<<endl;
        }
    }
    while (sz(evn)||sz(odd)){
        cin>>x;
        pii ret;
        if (sz(evn)) {ret=*evn.begin(), evn.erase(ret);
            if (x==2) cout<<3<<" "<<ret.F<<" "<<ret.S<<endl;
            else cout<<2<<" "<<ret.F<<" "<<ret.S<<endl;
        }
        else {ret=*odd.begin(), odd.erase(ret);
            if (x==1) cout<<3<<" "<<ret.F<<" "<<ret.S<<endl;
            else cout<<1<<" "<<ret.F<<" "<<ret.S<<endl;
        }
    }
    return 0;
}