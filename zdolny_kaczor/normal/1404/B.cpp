...
vi graf[nax];
pii fur(int a, int p = -1) {
	pii ans = {0, a};
	for (int v : graf[a]) if (v != p) {
		pii to = fur(v, a);
		maxi(ans, pii(to.first + 1, to.second));
	}
	return ans;
}
int diam() {
	return fur(fur(1).second).first;
}
int find(int x, int y, int p) {
	if (x == y) return 0;
	for (int v : graf[x]) if (v != p) {
		int r = find(v, y, x);
		if (r != -1) return r + 1;
	}
	return -1;
...
void solve() {
	...
	int dia = diam();
	int dis = dist(a, b);
	debug(imie(da), imie(db), imie(dia), imie(dis));
	mini(da, dia);
	mini(db, dia);
	bool can1 = dis <= da;
	bool can2 = 2 * da >= db;
	debug(can1, can2);
	puts(can1 || can2 ? "Alice" : "Bob");
	REP(i, n + 10) graf[i].clear();
}
int main() {
