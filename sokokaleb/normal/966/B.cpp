...
bool coba(int X, int Y, vi& ansX, vi& ansY) {
	int R = 0;
	set<ii> SET;
	FORN (i, 1, N) {
		int h = (X + i - 1) / i;
		while (R < N && C[R].ff >= h) ++R;
		if (R > 0 && C[R - 1].ff >= h) {
			int r = R - 1;
			int l = r - i + 1;
			SET.insert(mp(l, r));
...
int main(int argc, char** argv) {
	...
	if (coba(X1, X2, ansX1, ansX2) || coba(X2, X1, ansX2, ansX1));
	else {
		cout << "No\n";
		return 0;
	}
	cout << "Yes\n";
	cout << SIZE(ansX1) << " " << SIZE(ansX2) << "\n";
	const auto prt = [&](const auto& vec) {
		FOR (i, 0, SIZE(vec)) {
			if (i) cout << " ";
			cout << vec[i];
		}
