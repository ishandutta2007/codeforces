...
void solve()
{
	...
	auto to = [](vector<ll>& p, ll x)
	{
		return lower_bound(p.begin(), p.end(), x) - p.begin() + 1;
	};

	vector<tuple<int, int, int>> q;
	q.push_back({ to(px, 0), to(py, 0), 0 });
	for (int i = 1; i <= n; ++i)
	{
		q.push_back({ to(px, x[i - 1]), to(py, y[i - 1]), i });
	}

	vector<int> res(n + 1, -INF);
	res[0] = 0;

	for (int i = 0; i <= maxn; ++i) w[i] = -INF;
	...
	function<void(int, int)> go = [&](int L, int R)
	{
		...
		sort(q.begin() + L, q.begin() + R + 1, [](auto& a, auto& b)
			{
