...
int main() {
	...
	defn defk
	vector<vector<point>> modi(k);
	lol moves=0;
	fori(i, n) {
		defa
		modi[a%k].push_back(make_pair(a, i));
	}
	int i = 0, j = 0;
	while (i < k) {
		if (modi[i].size() <= n / k) {
		...
		if (modi[j%k].size() >= n / k||j<i) {
			j++;
			con
		}
		moves += (j - i) % n;
		point hm = modi[i].back();
		modi[i].pop_back();
		hm.first += (j - i) % n;
		modi[j%k].push_back(hm);
	}
	co moves el
	veci outi(n);
	fori(i, k) {
		forin(j, modi[i]) {
			outi[modi[i][j].second] = modi[i][j].first;
		}
	}
	outv(outi, n);
	ex
