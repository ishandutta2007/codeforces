...
fun solve() {
    ...
    val a = io.ints(n)
    val b = io.ints(n)
//    val asort = List(n){i -> i}.sortedWith(Comparator<Int>{x,y -> a[x].compareTo(a[y])})
//    val bsort = List(n){i -> i}.sortedWith(Comparator<Int>{x,y -> b[x].compareTo(b[y])})
    var bes = -1
    var bestInd = -1
    for (i in 0..n) {
        var dif = MOD
        for (j in 0..i-1) {
            dif = minOf(dif,b[n-i+j]-a[j])
        }
        for (j in i..n-1) {
            dif = minOf(dif,a[j]-b[j-i])
        }
        if (dif > bes) {
            bes = dif
            bestInd = i
