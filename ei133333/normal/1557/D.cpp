...
int main() {
  ...
  auto rmq = get_lazy_segment_tree(xs.size(), f, f, f, pi(0, -1), pi(0, -1));
  vector< int > pre(N);
  for(int i = 0; i < N; i++) {
    pi mx(1, -1);
    for(auto &p : seg[i]) {
      p.first = lower_bound(begin(xs), end(xs), p.first) - begin(xs);
      p.second = lower_bound(begin(xs), end(xs), p.second) - begin(xs);
      auto res = rmq.query(p.first, p.second);
      res.first++;
      if(res.first > 1) {
        chmax(mx, res);
      }
    ...
    pi upd(mx.first, i);
    for(auto &p : seg[i]) {
      rmq.update(p.first, p.second, upd);
    }
  }
  auto res = rmq.query(0, xs.size());
  cout << N - res.first << "\n";
  int cur = res.second;
  vector< int > used(N);
  while(~cur) {
  ...
  vector< int > ans;
  for(int i = 0; i < N; i++) {
    if(not used[i]) ans.emplace_back(i + 1);
  }
  cout << ans << "\n";
}