...
void go(int cur, int need, vector<int>&vl, vector<int>&vr, bool l, int curr = 0) {
	if (need == 0 ) {
		if (cur == 0) ans = min(ans, curr);
	...
	if (l) {
		for (int i = 0; i < n; ++i) {
			if (find(all(vl), i) == vl.end()) {
				vr.push_back(i);
				go(i, need - 1, vl, vr, l ^ 1, curr + g[cur][i]);
				vr.pop_back();
			}
		}
	} else {
		for (int i = 0; i < n; ++i) {
			if (find(all(vr), i) == vr.end()) {
				vl.push_back(i);
				go(i, need - 1, vl, vr, l ^ 1, curr + g[cur][i]);
				vl.pop_back();
			}
		...
		for (int kk = 1; kk < k; ++kk) {
			...
			for (int i = 0; i < n; ++i) {
				for (int j = 0; j < n; ++j) {
					...
					dp[kk & 1][i] = min(dp[kk & 1][i], dp[(kk & 1) ^ 1][j] + g[j][i]);
				}
			}
		}
		ans = min(ans, dp[1][0]);
