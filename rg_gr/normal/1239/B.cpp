...
{
	initprog();
	int n=read();rg(n)a[i]=re1d()+a[i-1];gr
	if(a[n])return puts("0\n1 1"),0;
	int s=*std::min_element(a,a+n);int ans1=0,ans2=0;
	rg(n)
	if(a[i]==s)ans1++;
	gr
	...
	for(int j=0;j<n&&!v[j];j=(j+1)%n)
	{
		if(a[j]==s)
		{
			...
			// odl(cc);
			// ans2=max(ans2,cc);
			if(ans2<cc)ans2=cc,l=J,r=j+1;
		}
	}
	gr
	// odl(ans1+ans2);
	// odp(l,r==0?r+n:r);
	int ta=ans1+ans2,tb=l,tc=r;
	memset(v,0,sizeof(v));
	...
	for(int j=0;j<n&&!v[j];j=(j+1)%n)
	{
		if(a[j]==s)
		{
			...
			// odl(cc);
			// ans2=max(ans2,cc);
			if(ans4<cc)ans4=cc,l=J,r=j+1;
		}
	}
	gr
	
	if(ans4>ta)ta=ans4,tb=l,tc=r;
	odl(ta);odp(tb,tc);
	return 0;
}