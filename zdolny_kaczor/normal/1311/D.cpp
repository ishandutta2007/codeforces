...
int main() {
	...
	REP(_, t) {
		int a, b, c;
		scanf("%d%d%d", &a, &b, &c);
		array<int, 4> ans = {inf, inf, inf, inf};
		FOR(a0, 1, nax) for (int b0 = a0; b0 <= nax; b0 += a0) {
			int c0 = c / b0 * b0;
			if (abs(c - c0) > abs(c - c0 - b0))
				c0 += b0;
			int cost = abs(a - a0) + abs(b - b0) + abs(c - c0);
			mini(ans, array<int, 4>{cost, a0, b0, c0});
		}
		assert(ans[0] <= 3500);
		REP(i, 4) printf("%d%c", ans[i], i % 3 ? ' ' : '\n');
	}
}