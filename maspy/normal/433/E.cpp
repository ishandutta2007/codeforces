...
struct Trie {
  ...
  Trie()
      : n_node(1), TO({vector<int>(sigma, -1)}), PAR({-1}), node_value({0}) {}

  int create_node() {
    TO.eb(vector<int>(sigma, -1));
    PAR.eb(-1);
    node_value.eb(0);
    return n_node++;
  }
...
void solve() {
  ...
  ll V = trie.n_node;
  auto& TO = trie.TO;
  auto& node_value = trie.node_value;

  auto calc = [&](vector<int> lim, bool include_eq) -> mint {
    /* lim 
    ...
    */
    ...
    ll len = lim.size();
    FOR(i, len) {
      vv(mint, newDP_LE, V, K + 1);
      vv(mint, newDP_EQ, V, K + 1);
      FOR(v, V) FOR(nxt, M) {
        ll to = TO[v][nxt];
        ll add = node_value[to];
        FOR(k, K + 1) {
          ll kk = k + add;
          if (kk > K) break;
          newDP_LE[to][kk] += DP_LE[v][k];
          if (nxt < lim[i]) newDP_LE[to][kk] += DP_EQ[v][k];
          if (nxt == lim[i]) newDP_EQ[to][kk] += DP_EQ[v][k];
          if (v == 0) {
            if (k == 0 && i == 0 && 0 < nxt && nxt < lim[i])
              newDP_LE[to][kk] += 1;
            if (k == 0 && i == 0 && nxt == lim[i]) newDP_EQ[to][kk] += 1;
            if (k == 0 && i && 0 < nxt) newDP_LE[to][kk] += 1;
          }
        }
      }
      swap(DP_LE, newDP_LE);
    ...
    mint le = 0, eq = 0;
    FOR(v, V) {
      FOR(k, K + 1) le += DP_LE[v][k];
      FOR(k, K + 1) eq += DP_EQ[v][k];
    }
    if (include_eq)
      return le + eq;
    else
  ...
  mint ANS = calc(R, true) - calc(L, false);
  print(ANS);
}
