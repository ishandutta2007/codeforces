...
void solve(){
  ...
  int cnt[3] = {0,0,0};
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(A[i][j] == 1) cnt[(i+j)%3]++;
  int s = cnt[0]+cnt[1]+cnt[2];
  for(li k = 0; k < (li)(3); ++k) if(cnt[k] <= s/3) {
    for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(A[i][j] == 1 && (i+j)%3 == k) {
      A[i][j]=2;
    }
    break;
  }
