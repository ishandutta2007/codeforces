...
struct segment {
    ...
    segment(long long _maximum = -LL_INF, int _max_index = -1) : maximum(_maximum), max_index(_max_index) {}
...
int main() {
    ...
    for (int c = 0, p = 0; c < C; c++) {
        // cerr << "coordinate: " << coords[c] << endl;

        while (p < N && points[p].end <= coords[c]) {
            int index = lower_bound(coords.begin(), coords.end(), points[p].start) - coords.begin();
            // cerr << "update [0, " << index << "] with " << points[p].score << endl;
    ...
    cout << best << '\n';
    cout << coords[best_start] << ' ' << coords[best_start] << ' ' << coords[best_end] << ' ' << coords[best_end] << '\n';
}