...
void pr(int n, int i, int j) {
    ...
    for (int rw = i - 1; n; --rw) {
        int to_put = min(i - j, n / 2);
        n -= 2 * to_put;
        for (int col = j; col < j + to_put; ++col) {
            tb[rw][col] = 'o';
        }
    }
    for (int rw = 0; rw < i; ++rw) {
        for (int col = 0; rw + col < i; ++col) {
            
            tb[rw][col] = tb[i - col - 1][i - 1 - rw];
        }
    }
    cout << i << endl;
    for (int rw = 0; rw < i; ++rw) {
