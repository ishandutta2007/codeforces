...
void solve() {
    ...
    VEC(ll, a, n);
    VEC(ll, b, n);
    ll x = f(a[0], b), y = f(a[n - 1], b);
    ll z = f(b[0], a), w = f(b[n - 1], a);
    debug(x, y, z, w);
    ll ans = x + y + z + w;
    chmin(ans, abs(a[0] - b[0]) + abs(a[n - 1] - b[n - 1]));
    chmin(ans, abs(a[0] - b[n - 1]) + abs(a[n - 1] - b[0]));
    chmin(ans, abs(a[0] - b[0]) + y + w);
    chmin(ans, abs(a[0] - b[n - 1]) + y + z);
    chmin(ans, abs(a[n - 1] - b[0]) + x + w);
    chmin(ans, abs(a[n - 1] - b[n - 1]) + x + z);
    out(ans);
}
