...
int main() {
	...
	rep(i, 1, n) in, a[i];
	rep(i, 1, n) b[i] = a[i];
	sort(b + 1, b + n + 1); int m = std::unique(b + 1, b + n + 1) - b - 1;
	if(m == 1) {
		cout << (b[1] == 1 ? n : 0) << '\n';
		return 0;
	}
	rep(i, 1, 16) {
		int x, y;
		do x = rd() % m + 1, y = rd() % m + 1; while(x == y);
		for(int d = -2;d <= 2;++d) factor(std::abs(b[x] - b[y]) + d);
	}
	rep(i, 1, 16) {
		int x = rd() % n + 1, y;
		do y = rd() % n + 1; while(a[x] == a[y]);
		for(int d = -2;d <= 2;++d) factor(std::abs(a[x] - a[y]) + d);
	}
	rep(i, 1, 20) {
		int x = rd() % n + 1;
		for(int d = -2;d <= 2;++d) factor(a[x] + d);
	}
//	rep(i, 1, 500) factor(i);
