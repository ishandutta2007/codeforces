...
int main(){
  ...
  vector<ll>dp(n[0]);
  rep(i,0,n[0])dp[i]=a[0][i];
  rep(i,0,3){
    auto seg=range_min::make(dp.size());
    rep(j,0,dp.size())seg.set(j,dp[j]);
    seg.build();
    vector<ll>ndp(n[i+1],INF);
    rep(j,0,n[i+1]){
      g[i][j].PB(-1);g[i][j].PB(n[i]);
      sort(ALL(g[i][j]));
      rep(o,0,g[i][j].size()-1){
        chmin(ndp[j],seg.query(g[i][j][o]+1,g[i][j][o+1]));
      }
      ndp[j]+=a[i+1][j];
    }
    //debug(ndp,n[i+1]);
