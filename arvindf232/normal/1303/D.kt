...
fun main(){
    solve.cases{
        ...
        val types = IntArray(70)
        for(a in L){
            types[a.countTrailingZeroBits()] ++
        }
        var ret = 0
        for(i in 0 until 63){
            if(n and (1L shl i ) != 0L) {
                if(types[i] == 0){
                    val nextfind = (i until 63).first{types[it] > 0}
                    for(d in nextfind downTo i+1){
                        types[d] --
                        types[d-1] += 2
                        ret ++
                    }
