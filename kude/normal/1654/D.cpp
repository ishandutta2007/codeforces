...
std::vector<std::pair<int,int>> factorize(int n) {
  ...
  while(tt--) {
    ...
    rep(_, n - 1) {
      ...
      x /= g;
      y /= g;
      to[i].push_back({j, x, y});
      to[j].push_back({i, y, x});
    }
    mint ans;
    VI pmax(n + 1);
    VI cnt(n + 1);
    ...
    auto dfs = [&](auto&& self, int u, int p, mint now) -> void {
      ans += now;
      for(auto [v, x, y]: to[u]) if (v != p) {
        add(y);
        remove(x);
        self(self, v, u, now * y / x);
        add(x);
        remove(y);
      }
    };
    dfs(dfs, 0, -1, 1);
    rep(i, n + 1) if (pmax[i]) ans *= mint::raw(i).pow(pmax[i]);
    cout << ans.val() << '\n';
  }
}