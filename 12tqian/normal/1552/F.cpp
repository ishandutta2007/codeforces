...
void solve_case(int tc = 0) {
	...
	f1r(i, 1, n + 1) {
		if (i == 1) {
			...
			int to = get_pos(starts, v[i][1]);
			mi add = v[i][0] - v[i][1]; // total distance you'll have to walk
			add += F.query(to, i);
			F.add(i, add);
			dp[i] = dp[i - 1] + v[i][0] - v[i - 1][0];
			if (v[i][2] == 1) {
				dp[i] += F.query(i, i);
			}
		}
