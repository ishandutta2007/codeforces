...
void modify(int rt, int l, int r, int x, int y, int z) {
    if(x <= l && r <= y) return t[rt].push_back(z), void();
    int mid = (l + r) >> 1;
...
void dfs(int rt, int l, int r) {
    const bs mem = cur;
    for (auto it : t[rt]) cur |= (cur << it);
    if(l == r) { ans |= cur; cur = mem; return ; }
    int mid = (l + r) >> 1; dfs(rt << 1, l, mid); dfs(rt << 1 | 1, mid + 1, r);
    cur = mem;
}
...
signed main() {
    ...
    cur[0] = 1; dfs(1, 1, n);
    std::vector < int > ret; rep(i, 1, n) if(ans[i]) ret.push_back(i);
    print(ret.size()); for (auto it : ret) sprint(it);
    print();
}
/*
 bitset 
