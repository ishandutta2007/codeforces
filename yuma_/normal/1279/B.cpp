...
int main() {
    ...
    while(T--){
        ...
        pair<int,pair<int,int>>answer(0,make_pair(0,0));
        ll sum=0;
        for(int i=0;i<N;++i){
            int a;cin>>a;
            sum+=a;
            max_p=max(max_p,make_pair(a,i+1));

            if(sum<=S){
                answer=max(answer,make_pair(i+1,make_pair(0,0)));
            }
            if(sum-max_p.first<=S){
                answer=max(answer,make_pair(i,max_p));
            }
        }
