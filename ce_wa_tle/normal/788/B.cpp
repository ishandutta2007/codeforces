...
typedef ll arr[N];
arr f,d,t;
#define f(x... )dsjflshdkfsbabd(x)
ll f(ll u){return u^f[u]?f[u]=f(f[u]):u;}
int main(int QwQcOrZ,char*Recall_902_[]){
	(void)QwQcOrZ,(void)Recall_902_;
	ll n=readll(),m=readll(),sum=0,cnt=0,s=0,x,y;rep(i,1,n)f[i]=i;
	rep(i,1,m)x=readll(),t[s=x]=t[y=readll()]=1,x^y?f[f(x)]=f(y),sum+=d[x]+++d[y]++:sum+=cnt++;
	rep(i,1,n)if(t[i]&&f(i)!=f(s))return puts("0"),0;
	printf("%lld\n",sum+cnt*(m-cnt));
	return 0;
}