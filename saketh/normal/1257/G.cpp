...
namespace ntt {
    ...
    template<int MOD> vector<modnum<MOD>> pow(vector<modnum<MOD>> v, int p) {
        if (p == 0) return vector<modnum<MOD>>{1};
        if (p == 1) return v;

        if ((sz(v) - 1) * (p/2) + 1 >= FFT_CUTOFF) {
            int S = (sz(v) - 1) * p + 1;
            int N = 1 << (32 - __builtin_clz(S - 1));
            v.resz(N);
            fft<modnum<MOD>, false>(v, N);
            for (int i = 0; i < N; i++) v[i] = v[i].pow(p);
            fft<modnum<MOD>, true>(v, N);
            v.resz(S);
            return v;
        }
...
int main() {
    ...
    auto longer = [](const vmn& a, const vmn& b){ return sz(a) > sz(b); };
    priority_queue<vmn, vvmn, decltype(longer)> pq(longer);

    F0R (i, sz(ct_ct)) if (ct_ct[i]) {
        pq.push(pow(vmn(i + 1, 1), ct_ct[i]));
    }
