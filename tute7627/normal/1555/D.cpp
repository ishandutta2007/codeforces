...
struct Doubling
{
  ...
  using C=function<bool(int, T)>;
  T query2(int k, C check)//check
  {
    ll ret = 0;
    T ret2 = 0;
    for(ll i = LOG - 1; i >= 0; i--){
      if(nxt[i][k] != -1 && !check(nxt[i][k],f(ret2, sum[i][k]))){
        ret += 1LL << i;
        ret2 = f(ret2, sum[i][k]);
        k = nxt[i][k];
      }
...
int main(){
  ...
  do{
    v.EB(n+1,0);
    ll j=v.size()-1;
    rep(i,0,n)v[j][i+1]=v[j][i]+(p[i%3]!=s[i]);
  }while(next_permutation(ALL(p)));
  while(m--){
    ll l,r;cin>>l>>r;l--;
    ll ret=INF;
    rep(i,0,v.size()){
      chmin(ret,v[i][r]-v[i][l]);
    }
    cout<<ret<<endl;
  }
  return 0;
