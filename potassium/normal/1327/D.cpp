...
bool check(vector<int>& cycle, int len, vector<int>& color) {
	vector<int> cl(len, -1);
	...
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 1; j <= cycle.size(); j++) {
				if (cycle.size() % j == 0) {
					if (check(cycle,j,color))
						chkmin(ans, j);
					if (check(cycle,(int)cycle.size() / j,color))
						chkmin(ans, (int)cycle.size() / j);
				}
			}
		}
