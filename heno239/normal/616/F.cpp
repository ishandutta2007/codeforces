...
void solve() {
    ...
    vector<int> las(al.size()+1);
    vector<int> val(al.size());
    las[al.size()] = 0;
    int loc = n;
    per(i, al.size()) {
        las[i] = las[i + 1] + 1;
    ...
    SegT st(lcp);
    vector<int> ori(al.size());
    rep(i, sa.size()) {
        ori[i] = las[sa[i]];
    }
    vector<ll> rc(al.size()+1);
    rep(i, al.size()) {
        rc[i + 1] = rc[i] + val[sa[i]];
    }
    ...
    function<void(int, int,int)> dfs = [&](int l, int r,int pre) {
        int malen = lenst.query(l, r);
        if (l + 1 < r)malen = min(malen, st.query(l, r - 1));
        ll val = malen * (rc[r] - rc[l]);
        if (pre < malen) {
            ans = max(ans, val);
        }
        if (l + 1 == r) {
