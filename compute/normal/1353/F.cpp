        ...
        for(int i=0;i<n;i++)
        {
            for(int j=0;j<m;j++)
            {
                ...
                for(int ii=i;ii>=0;ii--)
                {
                    ...
                    {
                        if(a[ii][jj]>=a[i][j])
                            dp[ii][jj]=min({dp[ii][jj],ii<n-1?dp[ii+1][jj]+a[ii][jj]-a[i][j]:inf,jj<m-1?dp[ii][jj+1]+a[ii][jj]-a[i][j]:inf});
                    }
                }
                ll cur=dp[0][0];
                dp=vc<vi>(n,vi(m,1e18));
                dp[i][j]=0;
                for(int ii=i;ii<n;ii++)
                {
                    ...
                    {
                        if(a[ii][jj]>=a[i][j])
                            dp[ii][jj]=min({dp[ii][jj],ii>0?dp[ii-1][jj]+a[ii][jj]-a[i][j]:inf,jj>0?dp[ii][jj-1]+a[ii][jj]-a[i][j]:inf});
                    }
                }
                cur+=dp[n-1][m-1];
                ans=min(ans,cur);
