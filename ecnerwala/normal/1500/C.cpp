...
int main() {
	...
	vector<int> needSplit(N-1, true);
	vector<int> num_bad(M);
	for (int c = 0; c < M; c++) {
		for (int i = 0; i+1 < N; i++) {
			if (B[{c,i}] > B[{c,i+1}]) num_bad[c]++;
		}
		if (!num_bad[c]) res.push_back(c);
	}
	for (int z = 0; z < int(res.size()); z++) {
		int c = res[z];
		for (int i = 0; i+1 < N; i++) {
			if (!needSplit[i]) continue;
			if (B[{c,i}] < B[{c,i+1}]) {
				needSplit[i] = false;
				for (int x = 0; x < M; x++) {
					if (B[{x,i}] > B[{x,i+1}]) {
						num_bad[x]--;
						if (!num_bad[x]) res.push_back(x);
					}
				}
			}
		}
	...
	vector<tuple<int, int, int>> cur_sort(N);
	for (int i = 0; i < N; i++) {
		get<2>(cur_sort[i]) = i;
	}
	...
	for (int c : res) {
		for (int i = 0; i < N; i++) {
			get<0>(cur_sort[i]) = A[{c,get<2>(cur_sort[i])}];
			get<1>(cur_sort[i]) = i;
		}
