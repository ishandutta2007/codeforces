...
fun main(){
    solve.cases{
        ...
        var min = phuge
        for(choice in 0 until n){
            var ops =0L
            var last = 0L
            for(left in choice - 1 downTo 0){
                val atleast = (last +1) divceil L[left]
                ops += atleast
                last = atleast * L[left]
            }
            last =0L
            for(right in choice+1 until n){
                val atleast = (last +1) divceil L[right]
                ops += atleast
                last = atleast * L[right]
            }
            min = minOf(min, ops)
        }
        put(min)
