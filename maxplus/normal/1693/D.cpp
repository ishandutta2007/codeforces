...
int main() {
  ...
  int n = $;
  cin >> range{p, n};
  auto decinc = [](auto l, auto r) {
    auto l0 = l;
    int last = *l;
    int dec = inf;
    int inc = 0;
    while ((dec || inc != inf) && ++l != r) {
      int ndec = inc < *l? last: 0, ninc = dec > *l? last: inf;
      if (last < *l) {
        maxi(ndec, dec);
      } else {
        mini(ninc, inc);
  ...
  int l = 0, r;
  while ((r = l + decinc(p + l, p + n)) != n) {
    int l0 = l, r0 = r;
    ++r;
    l = r - decinc(reverse_iterator(p + r), reverse_iterator(p));
    ans += (l - l0) * (2 * r0 - l - l0 + 1ll);
  }
  print((ans += (r - l) * (r - l + 1ll)) / 2);
}