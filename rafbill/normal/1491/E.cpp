...
int main(){
  ...
  F[0] = 1;
  F[1] = 1;
  for(li i = 0; i < (li)(28); ++i) F[i+2] = F[i] + F[i+1];
  ...
  hash_map<uint64_t, bool> cache[30];

  auto test = letrec([&](auto test, int k, int root) -> bool {
    if(k <= 3) return true;
    vii totest;

    uint64_t tot = 0;

    auto dfs = letrec([&](auto dfs, int u, int p) -> int {
      tot ^= H[u];
      int sz = 1;
      for(int v : G[u]) if(v != p) {
          int szv = dfs(v,u);
          sz += szv;
          if(szv == F[k-2]) totest.emplace_back(u,v);
          if(szv == F[k-1]) totest.emplace_back(v,u);
        }
      return sz;
    });
    dfs(root,-1);

    { auto it = cache[k].find(tot);
      if(it != end(cache[k])) return it->second;
    }

    for(auto p : totest){
      int u,v; tie(u,v) = p;
      G[u].erase(v); G[v].erase(u);
      bool ok = test(k-2,v) && test(k-1,u);
      G[u].insert(v); G[v].insert(u);
      if(ok) return cache[k][tot] = true;
    }
    return cache[k][tot] = false;
  });

  for(li i = 0; i < (li)(30); ++i) if(n == F[i]) {
    if(test(i, 0)) {
      cout << "YES" << endl;
      return 0;
    }
