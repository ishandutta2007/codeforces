...
public class EF {
	...
	void solve()
	{
		...
		long[][] longer = new long[n][(n>>>6)+1];
		long[] qs = new long[n*(n-1)/2];
		int p = 0;
		for(int i = 0;i < n;i++){
			for(int j = i+1;j < n;j++){
				int d = (co[i][0]-co[j][0])*(co[i][0]-co[j][0])+
						(co[i][1]-co[j][1])*(co[i][1]-co[j][1]);
				qs[p++] = (long)d<<32|i<<16|j;
			}
		}
		qs = radixSort(qs);
		for(int i = n*(n-1)/2-1;i >= 0;i--){
			int x = ((int)qs[i])>>>16, y = ((int)qs[i])&65535;
			for(int j = n>>>6;j >= 0;j--){
				if((longer[x][j]&longer[y][j]) != 0){
					out.printf("%.15f\n", Math.sqrt(qs[i]>>>32)/2);
					return;
				}
			}
...
//		INPUT = sb.toString();
	...
	public static void main(String[] args) throws Exception { new EF().run(); }
