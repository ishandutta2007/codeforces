...
int main() {
	...
	while (T--) {
		...
		std::vector<int64_t> sorted_col_sum = col_sum;
		std::sort(sorted_col_sum.begin(), sorted_col_sum.end());

		std::vector<int> bad_idx; bad_idx.reserve(2);
		for (int j = 0; j < M; j++) {
			if (sorted_col_sum[j] != col_sum[j]) {
				bad_idx.push_back(j);
				if (bad_idx.size() == 2) break;
			}
		}

		assert(bad_idx.size() <= 2);
		assert(bad_idx.size() != 1);
		if (bad_idx.empty()) {
			bad_idx.push_back(0);
			bad_idx.push_back(0);
		}
		...
		bool is_good = true;
		for (int i = 0; i < N; i++) {
			std::swap(G[{i, bad_idx[0]}], G[{i, bad_idx[1]}]);
			for (int j = 0; j+1 < M; j++) {
				is_good = is_good && (G[{i, j}] <= G[{i, j+1}]);
			}
		}
		if (is_good) {
			cout << bad_idx[0] + 1 << ' ' << bad_idx[1] + 1 << '\n';
		} else {
			cout << -1 << '\n';
		}
	}
