...
public class Main extends PrintWriter {
    ...
    class Point implements Comparable<Point> {
        ...
        public int compareTo(Point o) {
            return x == o.x ? Integer.compare(o.id, id) : Integer.compare(x, o.x);
        }
    }
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            int ai = nextInt();
            a.add(new Point(ai - x, i));
        }
        for (int i = 0; i < m; i++) {
            a.add(new Point(nextInt(), - i - 1));
        }
        Collections.sort(a);
        ...
        for (Point p : a) {
            if (p.id >= 0) {
                ts.add(new Point(p.x + x + y, p.id));
            } else {
                while (!ts.isEmpty() && ts.first().x < p.x) {
                    ts.remove(ts.first());
                }
                if (!ts.isEmpty()) {
                    Point f = ts.first();
        ...
        for (int i = 0; i < ans.size(); i += 2) {
            printf("%d %d%n", ans.get(i) + 1, ans.get(i + 1) + 1);
        }
    }
