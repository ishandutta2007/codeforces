...
int main() {
    ...
    auto nn = [](node a, node b) {
        if (b.lo == a.lo) { // mins in both
            return node {
                a.lo, a.tot + b.tot, a.len + b.len,
                a.pref, b.suff, max(a.every, b.every),
                a.ans + b.ans + max(a.suff, b.pref)
            };
        } else if (b.lo < a.lo) { // mins only in b
            return node {
                b.lo, a.tot + b.tot, a.len + b.len,
                max(a.every, b.pref), b.suff, max(a.every, b.every),
                b.ans
            };
        } else { // mins only in a
            return node {
                a.lo, a.tot + b.tot, a.len + b.len,
                a.pref, max(a.suff, b.every), max(a.every, b.every),
                a.ans
            };
        }
    };
    ...
    segment_tree_lazy st(N, node{0,0,0,0,0,0,0}, 0, nn, plus<int>(), un);

    const int MAXV = 2e5 + 1;
    vpii occ(MAXV, {N, -1}); vi ct(MAXV);
    F0R (i, N) ckmin(occ[a[i]].f, i), ckmax(occ[a[i]].s, i), ct[a[i]]++;
    vi diffs(N);
    F0R (v, MAXV) if (occ[v].f < N) diffs[occ[v].f]++, diffs[occ[v].s]--;
    vi sums = prefixes(diffs, 0, plus<int>());
    st.set_leaves([&](int i){
            int wt = occ[a[i]].f == i ? ct[a[i]] : 0;
            return node{sums[i+1], sums[i+1], 1,
                        wt, 0, wt,
                        0};
    });

    node ans = st(0, N);
    ps(N - (ans.ans + ans.pref + ans.suff));
