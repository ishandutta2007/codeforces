...
int main(){
    ...
    int N,M;cin>>N>>M;
    vector<Point> A(N),B(M),saveA,saveB;
    for(int i=0;i<N;i++){
        cin>>A[i].x>>A[i].y;
    }
    for(int i=0;i<M;i++){
    ...
    for(int i=1;i+1<si(A);i++){
        aa.push_back(norm(A[i-1]-A[i]));
        aa.push_back(norm(A[i+1]-A[i]));
        aa.push_back(dot(A[i-1]-A[i],A[i+1]-A[i]));
        aa.push_back(cross(A[i-1]-A[i],A[i+1]-A[i]));
    }
    
    for(int i=1;i+1<si(B);i++){
        bb.push_back(norm(B[i-1]-B[i]));
        bb.push_back(norm(B[i+1]-B[i]));
        bb.push_back(dot(B[i-1]-B[i],B[i+1]-B[i]));
        bb.push_back(cross(B[i-1]-B[i],B[i+1]-B[i]));
    }
    ...
    for(auto x:aa) C.push_back(x);
    for(auto x:bb) C.push_back(x);
    for(auto x:bb) C.push_back(x);
    ...
    auto D=atcoder::z_algorithm(C);
    
    for(int i=si(aa);i<si(D);i+=4){
        if(D[i]>=si(aa)) ok=true;
    }
