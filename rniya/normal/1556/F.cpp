...
int main() {
    ...
    vector<vector<mint>> to(n, vector<mint>(n));  // i -> j
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            to[i][j] = mint(a[i]) / (a[i] + a[j]);
        }
    }
    int whole = (1 << n) - 1;
    vector<map<int, mint>> setto(1 << n);  //  mask -> sub
    ...
    auto calc = [&](int x) {  // probability of x defeated all other teams
        vector<mint> f(1 << n);
        for (int mask = 0; mask < (1 << n); mask++) {
            if (!(mask >> x & 1)) continue;
            f[mask] = setto[whole ^ mask][mask];
            for (int sub = mask; sub > 0; sub = (sub - 1) & mask) {
                if (!(sub >> x & 1)) continue;
                if (sub == mask) continue;
                f[mask] -= f[sub] * setto[whole ^ mask][mask ^ sub];
            }
        }
