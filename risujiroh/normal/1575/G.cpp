...
int main() {
  ...
  int n = input();
  vector<int> a(n + 1);
  for (int i : rep(1, n + 1)) scan(a[i]);
  using Fp = atcoder::modint1000000007;
  vector<Fp> w(ALL(rep(L + 1)));
  for (int p : sieve::primes)
    for (int i : rep(1, L / p + 1) | rev) {
      w[i * p] -= w[i];
    }
  Fp ans;
  for (int d : rep(1, n + 1)) {
    if (d <= 1000) {
      ...
      f *= f;
      for (int p : sieve::primes)
        for (int i : rep(1, L / p + 1)) {
          f[i] -= f[i * p];
        }
      Fp cur;
      for (int i : rep(1, L + 1)) cur += i * f[i];
      ans += cur * w[d];
    } else {
      Fp cur;
      for (int i : rep(d, n + 1, d))
        for (int j : rep(d, n + 1, d)) cur += gcd(a[i], a[j]);
      ans += cur * w[d];
    }
  }
  print(ans.val());
