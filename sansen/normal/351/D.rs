...
fn run() {
    ...
    let mut kind = segment_tree::PURQ::new(n, (0, 0), |a, b| (a.0 + b.0, a.1.max(b.1)));
    let mut b = vec![vec![]; m];
    let mut val = vec![n + 1; m];
    let mut ans = vec![0; q];
    for (l, (&a, ask)) in a.iter().zip(query).enumerate().rev() {
        let b = &mut b[a];
        ...
        let len = b.len();
        if len >= 3 {
            let p = b[len - 2] - b[len - 1];
            let q = b[len - 3] - b[len - 2];
            if p != q {
                val[a] = b[len - 3];
            }
        } else {
