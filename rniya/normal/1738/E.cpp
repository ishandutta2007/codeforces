...
void solve() {
    ...
    {
        mint sum = 0;
        for (int i = 0; i <= min(l, n - 1 - r); i++) sum += BINOM.C(l, i) * BINOM.C(n - 1 - r, i);
        ans *= sum;
    }
    ll L = 0, R = 0;
    for (; l <= r;) {
        L += a[l];
        while (R < L and r > l) R += a[r--];
        if (R < L) break;
        l++;
        if (L < R) continue;
        int cntl = 1, cntr = 1;
        while (l <= r and a[l] == 0) cntl++, l++;
        if (l > r) {
            ans *= mint(2).pow(cntl);
            break;
        }
        while (a[r] == 0) cntr++, r--;
        mint sum = 0;
        for (int i = 0; i <= min(cntl, cntr); i++) sum += BINOM.C(cntl, i) * BINOM.C(cntr, i);
        ans *= sum;
    }
