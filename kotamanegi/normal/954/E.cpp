...
void solve() {
    ...
    sort(ALL(inputs));
    vector<pair<ld, ld>> minus, pluss;
    REP(i, n) {
        if (inputs[i].first < 0) {
            minus.push_back(inputs[i]);
        }
        else {
            pluss.push_back(inputs[i]);
        }
    }
    int hoge = 0;
    REP(i, inputs.size()) {
        hoge += inputs[i].first * inputs[i].second;
    }
    if (hoge > 0) {
        swap(minus, pluss);
    }
    REP(i, minus.size()) {
        minus[i].first = abs(minus[i].first);
    }
    REP(i, pluss.size()) {
        pluss[i].first = abs(pluss[i].first);
    }
    sort(ALL(pluss));
    sort(ALL(minus));
    reverse(ALL(minus));
    pluss.push_back(mp(0, 0));
    ld ans = 0;
    REP(i, pluss.size()) {
        ans += pluss[i].second;
        ld now = abs(pluss[i].first * pluss[i].second);
        while (minus.empty() == false) {
            ld stocks;
            if (minus.back().first == 0.0L) {
                stocks = 1e9;
            }
            else {
                stocks = now / (ld)abs(minus.back().first);
            }
            if (stocks >= (ld)minus.back().second) {
                ans += (ld)minus.back().second;
                now -= (ld)abs(minus.back().second * minus.back().first);
                minus.pop_back();
            }
            else {
                ans += stocks;
                minus.back().second -= stocks;
                now = 0;
                break;
            }
        }
