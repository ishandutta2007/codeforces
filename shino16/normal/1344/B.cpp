...
int main() {
  int h = IN, w = IN;
  auto s = IN.v<string>(h);
  auto t = vector(w, string(h, ' '));
  rep(r, h) rep(c, w) t[c][r] = s[r][c];

  auto check = [&]() {
    rep(r, h) {
      char prev = '.';
      int cnt = 0;
      rep(c, w) if (s[r][c] != prev) {
        if (s[r][c] == '.') prev = '.';
        else if (cnt) EXIT(-1);
        else prev = '#', cnt++;
      }
    ...
    bool foundW = false;
    rep(r, h) if (all_of(all(s[r]), [&](auto arg1){
      return arg1 == '.';
    })) foundW = true;
    VB foundB(w, false);
    rep(r, h) rep(c, w) if (s[r][c] == '#') foundB[c] = true;
    if (foundW and accumulate(all(foundB), true, bit_and<bool>())) EXIT(-1);
  };

  check();
  swap(h, w); swap(s, t); check(); swap(h, w); swap(s, t);

  int dr[] = {0,0,-1,1}; // LRUD
  int dc[] = {1,-1,0,0};
  auto inside = [&](int r, int c) { return r >= 0 and r < h and c >= 0 and c < w; };

  int res = 0;

  rep(r, h) rep(c, w) if (s[r][c] == '#') {
    s[r][c] = '.'; res++;
    stack<PII> togo; togo.emplace(r, c);
    while (not togo.empty()) {
      int r, c; tie(r, c) = togo.top(); togo.pop();
      rep(i, 4) if (inside(r+dr[i], c+dc[i])) {
        int r2 = r+dr[i], c2 = c+dc[i];
        if (s[r2][c2] == '#') s[r2][c2] = '.', togo.emplace(r2, c2);
      }
    }
  }
