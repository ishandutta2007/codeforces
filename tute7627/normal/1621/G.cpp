...
int main(){
  ...
  while(t--){
    ll n;cin>>n;
    vector<P>_a(n);
    rep(i,0,n)cin>>_a[i].fi,_a[i].se=-i;
    Compress mp(_a);
    auto a=mp.compress(_a);
    vector<modint>c(n);
    BIT<modint>bit1(n),bit2(n);
    modint ret=0;
    rep(i,0,n){
      c[i]=bit1.sum(0,a[i])+1;
      bit1.add(a[i],c[i]);
      modint cfsum=bit2.sum(0,a[i]);
      bit2.add(a[i],cfsum+c[i]);
    }
    ret+=bit2.sum(0,n);
    int pre=-1;
    auto ord=ascend(a);
    ...
    rrep(i,0,n){
      if(pre<a[i]){
        vector<int>v;
        while(idx<n&&a[ord[idx]]>=pre&&a[ord[idx]]<=a[i]){
          v.PB(ord[idx]);
          idx++;
        }
        sort(ALL(v));
        BIT<modint>bit(a[i]-pre+1);
        for(auto j:v){
          int val=a[j]-pre;
          modint add=bit.sum(val)+c[j];
          if(j==i)ret-=add;
          bit.add(val,add);
        }
        pre=a[i];
