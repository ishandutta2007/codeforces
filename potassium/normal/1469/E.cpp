...
int main() {
	CASE(t) {
		...
		vector<int> ps(n, 0);
		for (int i = 0; i < n; i++)
			ps[i] = (i == 0 ? 0 : ps[i - 1]) + (s[i] - '0');

		auto get = [&](int l, int r) {
			return (r >= 0 ? ps[r] : 0) - (l >= 1 ? ps[l - 1] : 0);
		};

		int lgmaxb = min(k, 20);
		int maxb = 1 << lgmaxb;

		vector<bool> ban(maxb, false);
		const int mask = maxb - 1;
		int now = 0;
		...
		for (int i = 0; i < n; i++) {
			...
			if (i >= k - 1 && get(i - k + 1, i - lgmaxb) == 0)
				ban[now] = 1;
		}

		int take = 0;
		while (take < maxb && ban[take]) ++take;
