...
void operator+=(array<i64, 2>& x, array<i64, 2> const& y) {
  x[0] += y[0];
  x[1] += y[1];
}

i64 const L = i64(1e12);
...
void main(int) {
  ...
  fix([&](auto self, int v, int p) -> void {
    array<i64, 2> t{};
    for (int u : g[v].filter(_1 != p)) {
      self(u, v);
      t += f[u];
    }
    f[v][0] = t[1];
    f[v][1] = max(f[v][0], (L - sz(g[v])) + t[0]);
  })(0, -1);
  vla ans(n, 0);
  fix([&](auto self, int v, int p) -> void {
    if ((p == -1 || ans[p] == 0) && f[v][0] < f[v][1]) { ans[v] = sz(g[v]); }
    for (int u : g[v].filter(_1 != p)) { self(u, v); }
  })(0, -1);
  println<0, ' '>(ans.count(1 <= _1));
  ans.each(_1 += _1 == 0);
  println(ans.fold1());
  println(ans);
}
