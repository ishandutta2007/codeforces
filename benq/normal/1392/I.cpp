...
template<class T> vector<T> MUL(const vector<T>& A, const vector<T>& B) {
	...
	F0R(i,n) { // a is remainder mod m0::mod, b fixes it mod m1::mod
		...
		if (c > 0) res[i] -= (ll)m0::mod*(m1::mod)*(m2::mod);
	}
	// dbg(A,B,res);
	return res;
}
...
void go() { // <= x
	// V+fake-E
	vl A(100005), B(100005);
	trav(t,a) A[t] ++;
	trav(t,b) B[t] ++;
	F0R(i,sz(a)-1) A[max(a[i],a[i+1])] --;
	F0R(i,sz(b)-1) B[max(b[i],b[i+1])] --;
	// F0R(i,10) dbg(i,A[i]);
	vl C = MUL(A,B);
	FOR(i,1,sz(C)) C[i] += C[i-1];
	tmp.clear();
	trav(t,query) {
		// dbg("HUH",t,C[t],a,b);
		tmp.pb(C[t]);
...
int main() { // >= x
	setIO(); re(n,m,q);
	a.rsz(n), b.rsz(m); query.rsz(q); ans.rsz(q);
	re(a,b,query);
	trav(t,query) t --; // <= t
	go();
	// dbg(tmp);
