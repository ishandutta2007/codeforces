...
fn add(f: &mut [Vec<(i16,usize,usize)>], i0: usize, j0: usize, a: i16) {
	let n = f.len() as i32;
	let m = f[0].len() as i32;
	let mut i = i0 as i32;
	while i < n {
		let mut j = j0 as i32;
		while j < m {
			max!(f[i as usize][j as usize], (a,i0,j0));
			j += j & -j;
		}
		i += i & -i;
	}
...
fn max(f: &[Vec<(i16,usize,usize)>], i0: usize, j0: usize) -> (i16,usize,usize) {
	let mut res = (0,0,0);
	let mut i = i0 as i32;
	while i > 0 {
		let mut j = j0 as i32;
		while j > 0 {
			max!(res, f[i as usize][j as usize]);
			j -= j & -j;
		}
		i -= i & -i;
	}
...
fn main() {
	...
	let mut read = move || -> (usize,Vec<(i32,usize)>,Vec<i32>) {
		let n = rin.u();
		let aa = rin.vi(n);
		let mut a = v!([n]);
		for i in 1..=n {
	...
	drop(read);
	l!(mut i,j = 0);
	let mut f = v!([n+1][m+1] = (0,0,0));
	let mut data = v!([n+1][m+1] = (0,0));
	while i < n && j < m {
		if a[i].0 < b[j].0 {
			i += 1;
		} else if a[i].0 > b[j].0 {
			...
			let mut ii = i + 1;
			while ii < n && a[i].0 == a[ii].0 {
				ii += 1;
			}
			let mut jj = j + 1;
			while jj < m && b[j].0 == b[jj].0 {
				jj += 1;
			}
			for i1 in i..ii {
				for j1 in j..jj {
					let i2 = n - a[i1].1;
					let j2 = m - b[j1].1;
					let (res,ir,jr) = max(&f, i2, j2);
					data[i2][j2] = (ir,jr);
					add(&mut f, i2, j2, res + 1);
				}
			}
			i = ii;
			j = jj;
