// definitely overthought this one.
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	[[maybe_unused]] int const number=read<int>();
	std::string s=read<std::string>();

	std::vector<std::pair<int, int>> operations;

	std::vector<int> degree(s.size()+1);
	auto const computeDegree=[&]{
		degree[0]=0;
		for(int const index: range(0, int(s.size()))){
			degree[index+1]=degree[index]+(s[index]=='(' ? 1: -1);
		}
		assert(degree.back()==0);
		// waste of memory but okay...
	};
	...
	auto const balanced=[&]{
		...
		auto const tmp=[&]{
			auto backup=degree;
			computeDegree();
			assert(degree==backup);
			return true;
		}; assert(tmp());}
		auto const iter=std::find(begin(degree), end(degree), -1);
		return iter==degree.end();
	};
	...
	auto const op=[&](int pivot1, int pivot2){
		operations.push_back({pivot1, pivot2});
		std::reverse(s.begin()+pivot1, s.begin()+pivot2);
	};
	...
	if(not balanced()){
		...
		auto const iter=std::find(begin(degree), end(degree), -1);
		auto iter2=std::find(degree.rbegin(), degree.rend(), -1).base()-1;
		assert(*iter2==-1);

		auto pivot1=int(std::max_element(degree.begin(), iter)-degree.begin());
		auto pivot2=int(std::max_element(iter2, degree.end())-degree.begin());
		...
		if(balanced()){
			...
			op(pivot1, pivot2); // revert
			operations.clear();

			computeDegree();
			auto const pivot=int(std::max_element(degree.begin(), degree.end())-degree.begin());
			op(0, pivot); op(pivot, int(s.size()));
			assert((computeDegree(), balanced()));
		}
	}
	...
	for(auto [a, b]: operations) std::cout<<a+1<<' '<<b<<'\n';
}
// time-limit: 1000
