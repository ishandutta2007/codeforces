...
fun main(){
    solve.cases{
        val n = getint
        val bins = Array(526316){ mutableListOf<Pair<Int,Int>>() }
        repeat(n-1){
            val a = getint - 1
            val b = getint - 1
            val v = getint - 1
        ...
        val dsu = DSU_unroll(n)
        var ret = 0L
        fun solve(l:Int, r:Int,keep:Boolean){
            if(l == r){
                for(q in bins[l]){
                    val l = dsu.getsize(q.first)
                    val r = dsu.getsize(q.second)
                    ret += l.toLong() * r
                }
                return
            }
            var ver = dsu.getVersionNumber()
            val m = ( l+r) shr 1
            for(c in l..m){
                for(q in bins[c]){
                    dsu.union(q.first, q.second)
                }
            }
            solve(m+1,r,false)
            dsu.unrollTo(ver)
