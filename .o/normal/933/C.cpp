...
int _par[3]={0,1,2};
int par(int x) { return _par[x]==x?x:(_par[x]=par(_par[x])); }
...
int main() {
  ...
  for(int i = 0; i < N; i++) {
    ...
    bool dup = false;
    for(int j = 0; j < i; j++) {
      if(P[j].x == P[i].x && P[j].y == P[i].y && R[j] == r) {
        dup = true;
      }
    }
    if(dup) {
  ...
  set<int> comp;
  for(int i = 0; i < N; i++) comp.insert(par(i));
  // k = comp.size()

  int v = vec.size();
  int e = 0; for(int i = 0; i < N; i++) if(deg[i]) e += deg[i];
  int k = comp.size();
  int f = k + 1 - v + e;
  printf("%d\n", f);
