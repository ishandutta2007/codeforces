...
main(){
    ...
    while(q--)
    {
        ...
        if(op==1){
            ...
            if(lb==rb){
                int t=dq[rb][rc-1];
                dq[rb].erase(dq[rb].begin()+rc-1);
                dq[lb].insert(dq[lb].begin()+lc-1,t);
            }
            else{
                int t=dq[rb][rc-1];
                dq[rb].erase(dq[rb].begin()+rc-1);
                cnt[rb][t]--;
                for(int x=rb-1;x>=lb;x--)
                cnt[x+1][dq[x].back()]++,cnt[x][dq[x].back()]--,dq[x+1].push_front(dq[x].back()),dq[x].pop_back();
                dq[lb].insert(dq[lb].begin()+lc-1,t);
                cnt[lb][t]++;
            }
        }
