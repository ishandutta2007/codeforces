...
signed main() {
  ...
  FOR(_, T) {
    ...
    FORIN(x, A) x = lower_bound(all(B), x) - B.begin();
    ll M = B.size();
    VEC<vi> IDS(M);
    vi sz(M);
    FOR(i, N) { IDS[A[i]].eb(i); }
    FOR(i, M) sz[i] = IDS[i].size();
    vi sz_cum = cumsum(sz);
    VEC<bool> done(M);
    ll best = 0;
    FOR(L, M) {
      ...
      // L 
      ll R = L + 1;
      while (R < M && IDS[R - 1].back() < IDS[R][0]) {
        done[R] = true;
        ++R;
      }
      ll l = IDS[L][0], r = IDS[R - 1].back();
      ll x = sz_cum[R] - sz_cum[L];
      if (L > 0) x += lower_bound(all(IDS[L - 1]), l) - IDS[L - 1].begin();
      if (R < M) x += sz[R] - (lower_bound(all(IDS[R]), r) - IDS[R].begin());
      chmax(best, x);
    }
    // 
    FOR(i, M) chmax(best, sz[i]);
    // 2 
    FOR(i, M - 1) {
      auto ID1 = IDS[i];
      auto ID2 = IDS[i + 1];
      FOR(j, ID2.size()) {
        ll x = ID2.size() - j + (lower_bound(all(ID1), ID2[j]) - ID1.begin());
        chmax(best, x);
      }
    }
    print(N - best);
