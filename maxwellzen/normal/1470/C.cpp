...
void solve() {
    ...
    int x = 0;
    while (1) {
        if (steps*2 >= n-1) steps--;
        int cur = query((x+steps)%n);
        if (cur==k) {
            sus[x]=false;
            F0R(i, steps) sus[(x+steps+1+i)%n] = false;
            x += steps;
            steps++;
        } else {
            if (cur>k) {
                ...
                int y = 0;
                for (int step = 1024; step > 0; step /= 2) {
                    if (y+step < v.size() && query(v[y+step]) <= k) y += step;
                }
                answer(v[y]);
                return;
            }
