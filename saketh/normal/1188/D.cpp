...
int main() {
    ...
    F0R (b, BITS) {
        ...
        vll b0, b1;
        trav (v, vals) (v & (1ll << b) ? b1 : b0).pb(v);

        int ONE = 0, ZERO = 0;
        F0R (j, N + 1) {
            ckmin(DP[ZERO], dp[j] + ONE + sz(b0) - ZERO); // put a 0
            ckmin(DP[ONE + sz(b0)], dp[j] + ZERO + sz(b1) - ONE); // put a 1
            if (j < sz(vals)) {
                if (vals[j] & (1ll << b)) ONE++;
                else ZERO++;
            }
        ...
        swap(dp, DP);
        vals = b0; vals.insert(vals.end(), all(b1));
    }
