...
signed main() {
    ...
    read(a);
    const int N = 2050;
    vector<vector<char>> b(2 * N + 10, vector<char>(2 * N + 10, '#'));
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < m; ++j) {
            int x = i - j + N;
            int y = i + j;
    ...
    //for (auto i : b) dbg(i);
    vector<vector<int>> dp(2 * N + 10, vector<int>(2 * N + 10, 0));
    for (int i = 2 * N + 3; i >= 0; --i)
        for (int j = 2 * N + 3; j >= 0; --j) {
            if (b[i][j] == b[i + 2][j] && b[i][j] == b[i][j + 2] && b[i][j] == b[i + 1][j + 1] && b[i][j] == b[i + 2][j + 2]) {
                dp[i][j] = min(min(min(dp[i + 2][j], dp[i][j + 2]), dp[i + 1][j + 1]), dp[i + 2][j + 2]) + 1;
            } else dp[i][j] = 1;
        }
    ll ans = 0;
