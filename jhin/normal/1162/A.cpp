...
cin>>n>>m>>k;for(int j=1;j<=100;j++)c2[j]=100;
for(int i=0;i<k;i++){cin>>a[i]>>b[i]>>c[i];for(int j=a[i];j<=b[i];j++)c2[j]=min(c[i],c2[j]);}
for(int i=1;i<=n;i++){if(c2[i]==100)ans+=m*m;else ans+=min(m,c2[i])*min(m,c2[i]);}
cout<<ans;
