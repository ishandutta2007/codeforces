...
using thing=array<array<LL,3>,11>;
thing dfs(int v,int pr){
  ...
  for(int p:G[v]){
    ...
    forn(a,B+1){
      forn(b,B+1-a){
        rr[a+b]={(rr[a+b][0]+r[a][0]*(t[b][0]+t[b][1]+t[b][2]))%mod,
                 (rr[a+b][1]+r[a][1]*t[b][0])%mod,
                 (rr[a+b][2]+r[a][2]*(t[b][0]+t[b][2]))%mod};
      }
    }
    dbg(v,p,r,t,rr);
    r=rr;
...
LL solve(){
  auto r=dfs(0,0);
  LL s=0;
  forn(i,B+1) forn(j,3) (s+=r[i][j])%=mod;
  return s;
}

LL doit(int q,vector<LL>&V){
  if(q==sz(V)){
    int c=0;
    forv(v,G){
      if(V[v]!=K) continue;
      ++c;
      for(int p:G[v]) if(V[p]>=K) return 0;
    }
    if(c>B) return 0;
    return 1;
  }
