...
template <typename T = int> struct UnionFind {
    ...
    UnionFind(int n, vector<T> &a) : num(n) {
        data.assign(n, -1);
        sz = a;
        is_default = false;
    }
...
int main() {
    ...
    if(n < d + 1) {
        ...
        if(d & 1) t = 2;
        m = t;
        rep(i, d / 2) t *= (k - 1 + (!i and ~d & 1)), m += t, chmin(t, 1000000);
        if(m < n) {
            cout << "NO\n";
        } else {
            cout << "YES\n";
            ...
            rep2(i, 1, d / 2) rep(_, 2) v[i].eb(now++);
            rep2(i, 1, d / 2) {
                while(now <= n and si(v[i]) < si(v[i - 1]) * (k - 1 + (!(i - 1) and ~d & 1))) { v[i].eb(now++); }
            }
            rep2(i, 1, d / 2) {
                rep(j, si(v[i])) { cout << v[i][j] << " " << v[i - 1][j % si(v[i - 1])] << endl; }
            }
        }
    }
}