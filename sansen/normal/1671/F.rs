...
fn run() {
    ...
    for m in 2..=(n + 1) {
        let mut dp = Map::new();
        // bit, 
        dp.insert((0usize, 0usize, 0usize), 1u32);
        for t in 0..m {
            let mut next = Map::new();
            for ((bit, inv, ad), way) in dp {
                for k in 0..m {
                    if bit >> k & 1 == 0 {
                        let x = bit | (1 << k);
                        let inv = inv + k - (x & ((1 << k) - 1)).count_ones() as usize;
                        let ad = ad + if k > 0 && x >> (k - 1) & 1 == 0 { 1 } else { 0 };
                        if inv > n || ad > n {
                            continue;
                        }
                        if t == m - 1 || (1..m).any(|j| x >> j & 1 == 1 && x >> (j - 1) & 1 == 0) {
                            *next.entry((x, inv, ad)).or_insert(0) += way;
                        }
                    }
                }
            }
        ...
        for ((_, inv, ad), w) in dp {
            *memo.entry((m, inv, ad)).or_insert(M::zero()) += M::new(w);
        }
    }
    let mut pre = Map::new();
    for (&(len, inv, ad), &way) in memo.iter() {
        *pre.entry((1, len - 1, inv, ad)).or_insert(M::zero()) += way;
    }
    let mut dp = pre.clone();
    while !pre.is_empty() {
        let mut next = Map::new();
        for ((c, sub, inv, ad), way) in pre {
            for (&(a, b, d), &w) in memo.iter() {
                let c = c + 1;
                let sub = sub + a - 1;
                let inv = inv + b;
                let ad = ad + d;
                if inv <= n && ad <= n {
                    *next.entry((c, sub, inv, ad)).or_insert(M::zero()) += way * w;
                }
            }
        }
        pre = next;
        for (&k, &v) in pre.iter() {
            *dp.entry(k).or_insert(M::zero()) += v;
        }
    }
    ...
    for &(n, k, x) in ask.iter() {
        let mut ans = M::zero();
        for (&(c, sub, inv, ad), &way) in dp.iter() {
            if (inv, ad) == (k, x) && n >= c + sub {
                ans += M::binom(n - sub, c) * way;
            }
        }
        writeln!(out, "{}", ans).ok();
    }
    /*
    memo.insert((1, 0, 0), M::one());
    for (n, k, x) in ask {
        let mut dp = vec![vec![vec![M::zero(); n + 1]; k + 1]; x + 1];
        dp[0][0][0] = M::one();
        for s in 0..n {
            for i in 0..=x {
                for j in 0..=k {
                    let v = dp[i][j][s];
                    for (&(len, inv, ad), w) in memo.iter() {
                        let i = i + ad;
                        let j = j + inv;
                        let m = s + len;
                        if i <= x && j <= k && m <= n {
