...
main() {
    ...
    while(T--) {
        ...
        vv(int, ans, 4, 2);
        vv(int, a, 4, 2);
        rep(i, 4) cin >> a[i][0] >> a[i][1];
        auto check = [&](int x, int y, int i) {
            int res = INT_MAX;
            if(x == a[i][0]) chmin(res, abs(y - a[i][1]));
            if(y == a[i][1]) chmin(res, abs(x - a[i][0]));
            return res;
        };
        auto solve = [](int d, vector<vi> v) {
            int l = -1, r = 1e9;
            ...
            while(l < r - 1) {
                ...
                rep(i, 2) rep(j, 2) chmax(L, v[i][j] - mid), chmin(R, v[i][j] + mid);
                if(L > R)
                    l = mid;
                else
                    r = mid, res = L;
        ...
        auto run = [&](vpi w) {
            ...
            do {
                int now = 0;
                rep(i, 4) chmax(now, check(w[i].fi, w[i].se, i));
                if(chmin(mi, now)) rep(i, 4) ans[i][0] = w[i].fi, ans[i][1] = w[i].se;
            } while(next_permutation(all(w)));
        };
        auto make_vec = [](int x1, int x2, int y1, int y2) {
            vpi res;
            res.eb(x1, y1), res.eb(x2, y1), res.eb(x1, y2), res.eb(x2, y2);
            return res;
        };
        rep(i, 1 << 4) {
            vv(int, v, 2);
            ...
            UNIQUE(v[0]), UNIQUE(v[1]);
            if(v[0].size() > 2 or v[1].size() > 2) continue;
            if(v[0].size() == 2 and v[1].size() == 2 and v[0][1] - v[0][0] != v[1][1] - v[1][0]) continue;
            if(v[0].empty() and v[1].size() == 2) {
                vv(int, V, 2);
                rep(j, 4) V[a[j][1] == v[1][0]].eb(a[j][0]);
                int d = v[1][1] - v[1][0];
                if(V[0].size() != 2 or V[1].size() != 2) continue;
                int L = solve(d, V);
                run(make_vec(L, L + d, v[1][0], v[1][1]));
            } else if(v[1].empty() and v[0].size() == 2) {
                vv(int, V, 2);
                rep(j, 4) V[a[j][0] == v[0][0]].eb(a[j][1]);
                int d = v[0][1] - v[0][0];
                if(V[0].size() != 2 or V[1].size() != 2) continue;
                int L = solve(d, V);
                run(make_vec(v[0][0], v[0][1], L, L + d));
            } else if(v[0].size() == 2 and v[1].size() == 1) {
                int d = v[0][1] - v[0][0];
                run(make_vec(v[0][0], v[0][1], v[1][0] - d, v[1][0]));
                run(make_vec(v[0][0], v[0][1], v[1][0] + d, v[1][0]));
            } else if(v[0].size() == 1 and v[1].size() == 2) {
                int d = v[1][1] - v[1][0];
                run(make_vec(v[0][0], v[0][0] + d, v[1][0], v[1][1]));
                run(make_vec(v[0][0], v[0][0] - d, v[1][0], v[1][1]));
            } else if(v[0].size() == 2 and v[1].size() == 2) {
                run(make_vec(v[0][0], v[0][1], v[1][0], v[1][1]));
            }
        }
        if(mi == INT_MAX) {
            cout << -1 << endl;
            ...
            cout << mi << endl;
            rep(i, 4) { cout << ans[i][0] << " " << ans[i][1] << endl; }
        }
    }
}