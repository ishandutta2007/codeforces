...
int main(void)
{
	...
	ll g = 0, cnt = 0;
	rep(i, 0, n-1) g = gcd(g, a[i]), cnt += a[i] % 2;
	
	string ans, s, t;
	if(cnt >= 2){
		rep(i, 0, n-1) rep(j, 1, a[i]) ans += (char)(i+'a');
		outl(0), outl(ans);
		return 0;
	}
	...
	if(cnt == 1){
		rep(i, 0, n-1) a[i] /= g;
		rep(i, 0, n-1) rep(j, 1, a[i]/2) s += (char)(i+'a');
		t = s, reverse(all(t));
		
		string tmp = s;
		rep(i, 0, n-1) if(a[i] % 2) tmp += (char)(i+'a');
		tmp += t;
	...
	rep(i, 0, n-1) rep(j, 1, a[i]/g) s += (char)(i+'a');
	t = s, reverse(all(t));
