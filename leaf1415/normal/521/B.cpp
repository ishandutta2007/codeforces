...
bool check(ll id)
{
	...
	rep(i, -1, 1){
		P np = p + P(i, 1);
		if(mp.count(np) == 0) continue;
		bool flag = false;
		rep(j, -1, 1){
			if(mp.count(np + P(j, -1))){
				flag = true;
				break;
			}
		}
...
int main(void)
{
	...
	cin >> m;
	rep(i, 0, m-1) cin >> x[i] >> y[i], mp[P(x[i], y[i])] = i;
	
	set<ll> S;
	rep(i, 0, m-1) if(check(i)) S.insert(i);
	
	mint ans = 0;
	per(i, m-1, 0){
		ll id = *S.begin();
		if(i%2 != m%2) id = *S.rbegin();
		ans += modpow(m, i) * id;
		S.erase(id), mp.erase(P(x[id], y[id]));
		rep(j, -5, 5){
			rep(k, -1, 0){
				P p = P(x[id]+j, y[id]+k);
				if(mp.count(p)){
					ll nid = mp[p];
					if(check(nid)) S.insert(nid);
					else S.erase(nid);
				}
			}
		}
