...
int main() {
	CASE(t) {
		...
		auto greedy = [&](int cnt1, int cnt2, int cnt4 = 0) {
			int ans = 0;
		...
		const Mint THREE = Mint(3, MOD107);
		const Mint FOUR = Mint(4, MOD107);
		...
		if (n % 3 == 0) {
			...
			int mergeC = greedy(cnt[1], cnt[2]);
			W(THREE.pow(n / 3), ans + mergeC);
		...
		if (n % 3 == 1) {
			...
			for (auto& p : poss) {
				bool isPoss = 1;
				for (int i = 0; i < 5; i++)
					if ((cnt[i] -= p.first[i]) < 0)
						isPoss = 0;
						
				if (isPoss) {
					chkmin(mergeC, p.second + greedy(cnt[1], cnt[2], cnt[4]));
				}
			...
			W(THREE.pow((n - 4) / 3) * FOUR, ans + mergeC);
		...
		if (n % 3 == 2) {
			...
			int mergeC = INT_MAX;
			if (cnt[1] >= 2)
				chkmin(mergeC, greedy(cnt[1] - 2, cnt[2]) + 1);
			if (cnt[2] >= 1)
				chkmin(mergeC, greedy(cnt[1], cnt[2] - 1));
				
			W(THREE.pow((n - 2) / 3) * TWO, ans + mergeC);
