...
void Hollwo_Pelw(){
    ...
    for (int msk = 0; msk < (1 << n); msk++){
        if (dp[msk] != -1){
            int inv = (~msk) & ((1 << n) - 1);
            for (int s = inv; s; s = (s - 1) & inv){
                if (sum[s] == b[dp[msk]] && dp[msk | s] < dp[msk] + 1)
                    dp[msk | s] = dp[msk] + 1, tr[msk | s] = s;
            }
        }
    }
    if (dp[(1 << n) - 1] != k)
    ...
    reverse(b, b + k);
    for (int j = 0; j < k; j++){
        int msk = tr[cur], cnt = 0;
        for (int i = 0; i < n; i++){
            if (msk >> i & 1)
                res[++ cnt] = i;
        }
        for (int i = 1; i <= cnt; i++)
            cout << el[a[res[i]]] << (i == cnt ? "->" : "+");
        cout << el[b[j]] << "\n";
        cur ^= msk;
    }
}
