...
struct Transform {
  vector<int> outputLock = {0, 1};
  vector<string> outputColor = {"", ""};

  string repr() {
    string res = outputColor[0];
    if (res.empty()) res = "blue";
    return res;
  }
};

Transform combine(Transform a, Transform b) {
  Transform res;
  for (int i = 0; i < 2; ++i) {
    res.outputLock[i] = b.outputLock[a.outputLock[i]];
    res.outputColor[i] = b.outputColor[a.outputLock[i]];
    if (res.outputColor[i].empty()) {
      res.outputColor[i] = a.outputColor[i];
    }
  }
  return res;
}
...
Transform color(string name) {
  Transform res;
  res.outputColor[0] = name;
  return res;
}
...
class DRedaktiruemZingerColor {
 public:
  void solveOne() {
    int n;
    cin >> n;
    vector<Transform> s(n);
    for (int i = 0; i < n; ++i) {
      string st;
      cin >> st;
      if (st == "lock") s[i] = lock(); else if (st == "unlock") s[i] = unlock(); else s[i] = color(st);
    }
    atcoder::segtree<Transform, combine, unit> tree(s);
    cout << tree.all_prod().repr() << "\n";
    int changes;
    cin >> changes;
    for (int ci = 0; ci < changes; ++ci) {
      int a;
      string st;
      cin >> a >> st;
      Transform ss;
      if (st == "lock") ss = lock(); else if (st == "unlock") ss = unlock(); else ss = color(st);
      --a;
      tree.set(a, ss);
      cout << tree.all_prod().repr() << "\n";
    }
