...
public:
	...
	friend poly Rev(poly f){f.rev();return f;}
	...
	friend poly Minus(poly A,poly B){
		int n=max(A.size(),B.size());
		A.resize(n),B.resize(n);
		for(int i=0;i<n;++i){
			A[i]=(A[i]+mod-B[i])%mod;
	...
	friend poly Inv(const poly &f,int m){
		if(m==1){
	...
	friend pair<poly,poly> Div(poly f,poly g){
		if(g.size()>f.size())return make_pair(poly(vector<int>({0})),f);
		int len=f.size()-g.size()+1;
		auto _g=Rev(g);
		_g.resize(len);
		poly q=Mul(Rev(f),Inv(_g,len));
		q.resize(len);
		q.rev();
		poly r=Minus(f,Mul(q,g));
	...
	friend poly Exp(const poly &f,int m){
		if(m==1){
	...
	friend poly Sqrt(const poly &f,int m){
		if(m==1){
			return poly(vector<int>({1}));
		}
		poly A(vector<int>(f.begin(),f.begin()+m));
...
private:
	#define ls u<<1
	#define rs u<<1|1
	static poly MulT(poly A,const poly &B,int n){
		int lim=A.size();
		NTT_init(lim);
		for(int i=0;i<lim;++i){
			A[i]=1LL*A[i]*B[i]%mod;
...
using poly=Poly_ntt<21,mod>;
#define N 100010
typedef long long ll;
int n,k,p,a[N],pre[N][2],suf[N][2];
int fac[N],ifac[N];
...
int main(){
	...
	poly G(k+1),F(k+1);
	for(int i=0;i<=k;++i){
		G[i]=1LL*p*ifac[i]%mod;
	}
	--G[0];
	G=Inv(G,G.size());
	for(int i=0;i<=k;++i){
		F[i]=1LL*G[k-i]*ifac[i]%mod;
	}
	vector<int> x;
	for(int i=1;i<=n;++i){
		x.push_back(a[i]+1);
	...
	pre[0][0]=1;
	for(int i=1;i<=n;++i){
		pre[i][0]=(1LL*pre[i-1][0]*(a[i]+1)+1)%mod;
		pre[i][1]=(1LL*pre[i-1][1]*(a[i]+1)+1LL*pre[i-1][0]*a[i])%mod;
	}
	suf[n+1][0]=1;
	for(int i=n;i>=1;--i){
		suf[i][0]=(1LL*suf[i+1][0]*(a[i]+1)+1)%mod;
		suf[i][1]=(1LL*suf[i+1][1]*(a[i]+1)+1LL*suf[i+1][0]*a[i])%mod;
	}
	int ans=0;
	for(int i=1;i<=n;++i){
		int coef=(1LL*pre[i-1][0]*suf[i+1][1]+1LL*pre[i-1][1]*suf[i+1][0]+1LL*pre[i-1][0]*suf[i+1][0])%mod;
		int tmp=(-G[k]+1LL*qpow(p,a[i]+1)*x[i-1])%mod;
		tmp=1LL*(tmp+mod)%mod*fac[k]%mod;
		/*
		int tmp=0;
		for(int j=0;j<=a[i];++j){
			tmp=(tmp+1LL*qpow(p,j)*qpow(j,k))%mod;
		}
		*/
		ans=(ans+1LL*coef*tmp)%mod;
	}
	cout<<ans<<'\n';
	return 0;
}