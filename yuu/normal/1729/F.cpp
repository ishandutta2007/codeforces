...
void solve() {
    ...
    for (int i = 1; i <= n; i++) (f[i] = f[i - 1] + s[i] - '0') %= 9;
    for (int i = w; i <= n; i++) {
        int md = (f[i] - f[i - w] + 9) % 9;
        if (pos[md].size() >= 2) continue;
        pos[md].push_back(i - w + 1);
    }
    for (int i = 1, l, r, k; i <= m; i++) {
        cin >> l >> r >> k;
        int vlr = (f[r] - f[l - 1] + 9) % 9;
        ...
        for (int x = 0; x < 9; x++) {
            ...
            if (x != y) {
                ans = min(ans, {pos[x][0], pos[y][0]});
            } else {
                if (pos[x].size() == 1) continue;
                ans = min(ans, {pos[x][0], pos[x][1]});
            }
        }
        if (ans.first == 1e9) ans = {-1, -1};
