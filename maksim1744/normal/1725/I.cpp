...
struct DSU {
    ...
    bool un(int u, int v) {
        ...
        p[u] = v;
        ll edges = (ll)rk[u] * rk[v];
        Mint c2 = Mint(edges) * (edges - 1) + edges;
        rk[v] += rk[u];
        cnt[v] = cnt[u] * cnt[v] * c2;
        return true;
    }
...
int main() {
    ...
    // for (int i = 0; i < n; ++i) {
    //     cout << f[i][i] << ' ';
    //     if (i) assert(f[i][i] == f[i - 1][i - 1] * (i * 2 - 1));
    // }
    // cout << endl;
