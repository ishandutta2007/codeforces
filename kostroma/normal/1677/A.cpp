...
void solve(__attribute__((unused)) bool read) {
  ...
  vector<vector<int>> pref_sums(n + 1, vector<int>(n, 0));
  for (int i = 0; i < n; ++i) {
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      li lef = pref_sums[i][p[j]];
      li rig = p[i] - pref_sums[j + 1][p[i]];
      //cout << i << " " << j << " " << lef << " " << rig << " p[i]: " << p[i] << " pref_sum: " << pref_sums[j][p[i]] << endl;
      res += lef * rig;
    }
  }
  cout << res << "\n";
