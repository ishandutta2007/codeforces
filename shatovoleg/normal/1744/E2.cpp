...
struct PositiveInfType {
    ...
    constexpr NegativeInfType operator-() const;
} const INF;
...
constexpr NegativeInfType PositiveInfType::operator-() const {
    return {};
}

template<class T>
constexpr bool operator==(T val, const PositiveInfType& inf) {
    return val == T(inf);
}

template<class T>
constexpr bool operator==(T val, const NegativeInfType& neg_inf) {
    return val == T(neg_inf);
}

[[maybe_unused]] constexpr ll INFll = INF;
...
struct std::tuple_element<I, EnumerationIterator<T>> :
    std::tuple_element<I, std::tuple<const int, std::remove_reference_t<decltype(*std::declval<T>())>>> {};
...
vector<pii> GetFactorization(int n) {
    vector<pii> res;
    for (int i = 2; i * i <= n; ++i) {
        if (n % i == 0) {
            res.pb({i, 0});
...
vector<ll> GetDivisors(const vector<pii>& fact) {
    vector<ll> res;
    auto gen = YCombinator([&fact, &res](auto& gen, int idx, ll prod) {
        if (idx == len(fact)) {
            res.pb(prod);
            return;
        }
...
void solve() {
    ...
    auto fact = Merge(GetFactorization(a), GetFactorization(b));
    for (auto d1 : GetDivisors(fact)) {
        ll d2 = 1ll * a * b / d1;

        ll v1 = (a / d1 + 1) * d1, v2 = (b / d2 + 1) * d2;
        if (v1 <= c && v2 <= d) {
            cout << v1 << " " << v2 << '\n';
            return;
        }
