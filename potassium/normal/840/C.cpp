...
int main() {
	...
	vector<Mint> fac(MAXN, Mint(1, MOD107)), inv = fac;
	for (int i = 2; i < MAXN; i++)
		fac[i] = fac[i - 1] * i;
	inv[MAXN - 1] = fac[MAXN - 1].inv();
	for (int i = MAXN - 1; i >= 2; i--)
		inv[i - 1] = inv[i] * i;

	auto C = [&](int n, int r) {
		if (n < r) return Mint(0, MOD107);
		return fac[n] * inv[r] * inv[n - r];
	};
	
	vector<Mint> dp(MAXN, Mint(0, MOD107));
	dp[0] = 1;
	
	int total = 0;
	for (auto& i : cnt) {
		vector<Mint> ndp(MAXN, Mint(0, MOD107));
		for (int seg = 1; seg <= min(i, total + 1); seg++) {
			for (int bad = 0; bad <= total; bad++) {
				for (int rm = 0; rm <= min(bad, seg); rm++) {
					ndp[bad - rm + i - seg] += dp[bad] * fac[i] * C(i - 1, seg - 1) * C(bad, rm) * C(total + 1 - bad, seg - rm);
				}
			}
		}
