...
string anss[s.size()+1];
s=' '+s;
vector<array<int,3>>a;
a.push_back({0,0,0});
for(int x=s.size()-1;x>=1;x--)
{
	if(s[x]==a.back()[0]&&x+1==a.back()[1]&&s[x]>a.back()[2])a.pop_back();
	else{
		array<int,3>qwq={s[x],x,a.back()[2]};
		if(s[x]!=a.back()[0])qwq[2]=a.back()[0];
		a.push_back(qwq);
	}
	ans[x]=a.size()-1;
	if(a.size()-1<=10)
	...
	{
		for(int q=a.size()-1;q>=a.size()-5;q--)
		anss[x]+=a[q][0];
		anss[x]+="...";
		for(int q=2;q>=1;q--)
		anss[x]+=a[q][0];
