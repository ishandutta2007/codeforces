...
/*
ans[v] = ans[p] + 1
ans[v] min= cnt[v] + ans[u] - cnt[u]
*/

void dfs(int v, int p, vector<pair<int, int>>& st) {
	//tin[v] = ++tim;
	if (ok[v]) {
		++tim;
	}
	...
	if (p != -1) {
		ans[v] = ans[p] + 1;
		if (sz(st) && ok[v]) ans[v] = min(ans[v], cnt[v] + st.back().y);
	}

	for (int i = 0; i < 26; ++i) {
		if (g[v][i]) {
			int mn = sz(st) ? st.back().y : INF;
			st.push_back({ ans[v] - cnt[v],  min(mn, ans[v] - cnt[v] + ok[v]) });
			dfs(g[v][i], v, st);
			st.pop_back();
		}
	}
