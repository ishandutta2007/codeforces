...
#line 3 "codeforces-workspace\\contest\\650\\e\\e.cpp"
#include <bits/extc++.h>
...
#line 12 "codeforces-workspace\\contest\\650\\e\\e.cpp"
namespace workspace {
    constexpr char eol = '\n';
...
#line 27 "codeforces-workspace\\contest\\650\\e\\e.cpp"
...
solver()
{
    int n; cin>>n;
    union_find uf(n);
    vector<hash_set<int>> oldt(n);
    vector<hash_set<p32>> newt(n);
    for(int i=1; i<n; ++i)
    {
        int a,b; cin>>a>>b;
        --a,--b;
    ...
    while(size(leaf)>1)
    {
        int now=leaf.front();
        leaf.pop();
        int par=*oldt[now].begin();
        oldt[par].erase(now);
        if(size(oldt[par])==1) leaf.emplace(par);
        p32 nxt;
        if(newt[now].count({now,par}))
        {
            nxt={now,par};
        }
        else
        {
            nxt=*newt[now].begin();
            vector<int> upd{par,now,nxt.first,nxt.second};
            ans.emplace_back(upd);
        }
        int pir=uf.find(nxt.second);
        newt[now].erase(nxt);
        newt[pir].erase({nxt.second,nxt.first});
        if(size(newt[now])>size(newt[pir])) swap(newt[pir],newt[now]);
        for(auto e: newt[now])
        {
            newt[pir].emplace(e);
        }
