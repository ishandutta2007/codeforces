...
int main() {
    ...
    FOR (v, 1, LEN) {
        FOR (m, 1, LIM) {
            res[v][m] = (res[v-1][m] == m-1) ? 0 : res[v-1][m] + 1;
        }
    }

    vi arr(LEN);
    vvi table(LIM, vi(LIM));
    ...
    F0R (q, Q) {
        ...
        if (op == 1) {
            arr[x] += y;
            FOR (m, 1, LIM) table[m][res[x][m]] += y;
        } else {
            if (x < LIM) ps(table[x][y]);
            else {
                int ans = 0;
                for (int i = y; i < LEN; i += x) ans += arr[i];
                ps(ans);
            }
        }
    }
