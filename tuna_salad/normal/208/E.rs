...
fn dfs(u: usize, e: &[Vec<usize>], dep: usize, q: &[Vec<(usize,usize)>], qd: &mut [Vec<usize>], qs: &mut [usize], sol: &mut [usize]) {
	for x in qd[dep].drain(..) {
		sol[x] = qs[dep];
	}
	qs[dep] += 1;
	for &(d,i) in q[u].iter() {
...
fn main() {
	...
	for i in 0..m {
		...
		if p < lca.dep[v] {
			let u = lca.go_up(v, p);
			q[u].push( (lca.dep[v], i) );
		}
	}
	let mut qd = v!([n+1][]);
	let mut qs = v!([n+1] = 0);
	let mut sol = v!([m] = 0);
	dfs(0, &e, 0, &q, &mut qd, &mut qs, &mut sol);
	for x in sol {
		write!(rout, "{} ", x).ok();
	}
}