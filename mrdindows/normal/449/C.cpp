...
template<typename T1, typename T2>inline std::ostream &operator<<(std::ostream &os,const std::set<T1,T2>&v){bool first=true;os << "[";for(typename std::set<T1,T2>::const_iterator ii=v.begin();ii!=v.end();++ii){if(!first)os<<", ";os<<*ii;first=false;}return os<<"]";}
...
int main()
{
    ...
    for (int i = 2; i <= n; ++i)
    {
        if (v[i].size() > 1)
        {
            ...
            {
                odd.push_back(v[i][1]);
                ans.push_back(pii(v[i][0], v[i][2]));
                for (int j = 3; j < v[i].size(); j += 2)
                {
                    ans.push_back(pii(v[i][j], v[i][j + 1]));
                }
            }
            else
            {
    ...
    {
        ans.push_back(pii(odd[i], odd[i - 1]));
    }
    cout << ans.size() << '\n';
