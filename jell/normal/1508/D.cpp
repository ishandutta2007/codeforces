#line 1 "codeforces-workspace\\contest\\1508\\d\\d.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\1508\\d\\d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1508\\d\\d.cpp"
...
#line 27 "codeforces-workspace\\contest\\1508\\d\\d.cpp"
...
void main() {
  ...
  struct point {
    ...
    i64 cross(point o) const { return x * o.y - y * o.x; }
  };
  ...
  for (auto i : range(n)) {
    ...
    --t;
    uf.unite(i, t);
    points.push_back({x, y, i, t});
  }

  erase_if(points, [&](auto p) { return p.id == p.targ; });
  ...
  sort(begin(points), end(points), [&](auto &p1, auto &p2) {
    return compare_arg(pair(p1.x, p1.y), pair(p2.x, p2.y));
  });

  if (auto a = adjacent_find(begin(points), end(points),
                             [](auto p, auto q) { return p.cross(q) < 0; });
      a != points.end()) {
    rotate(begin(points), next(a), end(points));
  }

  vector<pair<i32, i32>> ans;

  auto oper = [&](point &p, point &q) -> void {
    ans.emplace_back(p.id, q.id);
    swap(p.targ, q.targ);
  };

  for (auto it : iterate(points)) {
    if (it == begin(points)) continue;
    if (uf.unite(prev(it)->id, it->id)) {
      oper(*prev(it), *it);
    }
  }

  while (cent.targ != cent.id) {
    for (auto &&p : points) {
      if (p.id == cent.targ) {
        oper(p, cent);
      }
  ...
  assert(all_of(begin(points), end(points),
                [](auto p) { return p.id == p.targ; }));
