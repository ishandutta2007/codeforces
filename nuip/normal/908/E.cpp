...
int bellNumber(int n){
	...
	for(int i = 1; i <= n; i++){
		for(int j = 0; j <= i-1; j++){
			dp_bell[i] = dp_bell[i] + (comb(i-1,j) * dp_bell[j]);
		}
	}
}
struct UF{
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<string> ss(n);
	rep(i,n) cin>>ss[i];
	rep(i,n)rep(j,m)rep(k,j)if(ss[i][j]!=ss[i][k]) tell[j][k]=tell[k][j]=1;
	UF uf(m);
	rep(i,m)rep(j,i)if(!tell[i][j]) uf.unite(i,j);
	vector<int> vs;
	rep(i,m)if(uf.root(i)==i&&uf.size(i)>1) vs.pb(uf.size(i));
	out(vs,1);
	mint re=1;
	for(int v:vs) re*=dp_bell[v];
	cout<<re<<endl;
  return 0;
}