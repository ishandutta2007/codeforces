...
int main() {
    ...
    vector<vector<M>> A(sz, vector<M>(k, M(k + 1)));
    for (int i = 0; i < k; i++) {
        auto& m = A[0][i];
        for (int j = 0; j <= k; j++) m[j][j] = 1;
        for (int j = 0; j <= k; j++) m[i][j] = 1;
    }

    for (int i = 0; i < sz - 1; i++) {
        vector<M> pre(k + 1, M(k + 1)), suf(k + 1, M(k + 1));
        pre[0] = M::I(k + 1), suf[k] = M::I(k + 1);
        for (int j = 0; j < k; j++) pre[j + 1] = pre[j] * A[i][j];
        for (int j = k - 1; j >= 0; j--) suf[j] = A[i][j] * suf[j + 1];
        for (int j = 0; j < k; j++) A[i + 1][j] = suf[j] * pre[j];
    }

    M prod = M::I(k + 1);
    for (int i = sz - 1, cur = 0; i >= 0; i--) {
    ...
    mint ans = 0;
    for (int i = 0; i <= k; i++) ans += prod[i][k];
    cout << ans << '\n';
    return 0;
}