...
void dfs(int x, int y = -1) {
	...
	for (int v : graf[x]) if (v != y) {
		dfs(v, x);
		(sad[x] *= happy[v] + none[v]) %= mod;
		(none[x] *= 2 * happy[v] + sad[v] + none[v] * 2) %= mod;
		(happy[x] *= happy[v] + 2 * none[v]) %= mod;
	}
	happy[x] -= sad[x];
	debug(arr(happy, x), arr(sad, x), arr(none, x));
}
ll inv(ll x) {
	ll e = mod - 2, r = 1;
	while (e) {
