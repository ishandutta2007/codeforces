...
int n,m;
int cx[maxsize],cy[maxsize];

pair<int64,int> solve(int n,int a[])
{
	int64 R1=-1;
	int R2=-1;
	for (int k=0;k<=n;k++)
	{
		int64 s=0;
		for (int i=0;i<n;i++) s+=(int64)a[i]*sqr(i*4+2-k*4);
		if (R1<0 || s<R1) R1=s,R2=k;
	}
	return MP(R1,R2);
}
int main()
{
	...
	pair<int64,int> r1=solve(n,cx);
	pair<int64,int> r2=solve(m,cy);
