...
int main() {
	...
	vector<pii> seg;
	for (int i = 0; i < n; i++) {
		if (seg.empty() || (s[i] == '_' && s[i - 1] == '#'))
			seg.push_back({ 0, 0 });
		(s[i] == '_' ? seg.back().first : seg.back().second)++;
	}
	...
	for (int slack = 0; slack <= 3; slack++) {
		auto fillable = [&](int len) {
			if (len < slack) return false;
			len -= slack;
			if (len == 1) return false;
			if (len != 0 && slack < 1) return false;
			if (len % 2 == 1 && slack < 2) return false;
			return true;
		};

		bool is = true;
		for (int i = 0; i < m; i++)
			if (!fillable(seg[i].first - (i != 0 && i != m - 1)))
				is = false;
		...
		for (int i = 0; i < m; i++) {
			fill(seg[i].first - (i != 0 && i != m - 1));
			if (seg[i].second > 0)
				ans.emplace_back(seg[i].second + slack);
		}
