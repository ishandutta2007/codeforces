...
int main()
{
	...
	while (!pq.empty())
	{
		...
		sudah[now] = 1;
		while (pt[now] < ada[now] && waktu[now][pt[now]] < t) ++pt[now];
		while (pt[now] < ada[now] && waktu[now][pt[now]] == t) 
		{
			++t;
			++pt[now];
		}
		//printf("%d %d\n",now,t);
		int sz = SIZE(adj[now]);
		REP(i,sz) if(!sudah[adj[now][i].fi])
		{
			pq.push(mp(-(t + adj[now][i].se),adj[now][i].fi));
		}
	}
	puts("-1");
	return 0;
