...
void solve() {
    ...
    V<tuple<V<int>, mint>> cache(n);
    V<bool> vis(n);

    auto dfs = [&](auto self, int p) -> tuple<V<int>, mint> {
        if (vis[p]) return cache[p];
        vis[p] = true;
        ...
        return cache[p] = {dp, sum};
    };
    V<int> dp;
    mint sum;
    tie(dp, sum) = dfs(dfs, r);
    dbg(dp, sum);
