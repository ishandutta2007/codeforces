...
ll solvedp(int from, int to, const vector<int> &a, const vector<int> &c, const int &n, const vector<int> &ind, bool flag = false) {
    if (flag && (ind[from] == 0 || ind[to] == 0)) return inf;
    ll left = inf;
    multiset<pair<int, ll>> right;
    multiset<ll> rightcosts;
    right.insert({a[ind[from]] + c[ind[from]], 0});
    rightcosts.insert(0);
    vector<int> newind;
    ...
    for (auto j : newind) {
        ...
        while (sz(right) && right.begin()->f <= a[j]) {
            rightcosts.erase(rightcosts.find(right.begin()->s));
            chmin(left, right.begin()->s - right.begin()->f);
            right.erase(right.begin());
        }
        ll now = left + a[j];
        if (sz(rightcosts))
            chmin(now, *rightcosts.begin());
        right.insert({a[j] + c[j], now});
        rightcosts.insert(now);
        ans = now;
    }
    return ans;
}
...
signed main() {
    ...
    read(n);
    vector<int> a(n), c(n);
    for (int i = 0; i < n; ++i) read(a[i], c[i]);
    int now = 0;
    auto f = [&](int i, int j) {
        return max(c[i], a[j] - a[i]) - c[i];
    };
    vector<int> ind(n);
    iota(all(ind), 0);
    sort(all(ind), [&](int i, int j) { return c[i] < c[j]; });
    stable_sort(all(ind), [&](int i, int j) { return a[i] < a[j]; });
    dbg(ind);
    int start = -1;
    for (int i = 0; i < n; ++i) {
        if (ind[i] == 0) {
    ...
    ll add = 0;
    for (auto i : c) add += i;
    // dbg(solvedp(0, start, a, c, n, ind));
    // dbg(solvedp(start, n - 1, a, c, n, ind));
    ll ans = solvedp(start, n - 1, a, c, n, ind) + solvedp(0, start, a, c, n, ind) + add;
    chmin(ans, solvedp(0, n - 1, a, c, n, ind, true) + add);
    println(ans);
    #ifdef DEBUG
