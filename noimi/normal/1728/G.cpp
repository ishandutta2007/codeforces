...
int main() {
    ...
    rep(i, m + 1) {
        vi v;
        while(idx < n and a[idx] < b[i]) { v.eb(a[idx++]); }
        vv(mint, c, m, m, 1);
        mint z = 1;
        fore(e, v) {
            vi id = iota(m);
            sort(all(id), [&](int x, int y) { return abs(e - b[x]) < abs(e - b[y]); });
            int pre = 0, st = 0, l = m, r = -1;
            rep(i, m) rep(j, m) cnt[i][j] = 0;
            rep(j, m) {
                int d = abs(e - b[id[j]]);
                if(!j)
                    st = d;
                ...
                chmax(r, id[j]), chmin(l, id[j]);
                pre = d;
            }
            cnt[l][r] += d + 1 - pre;
            rep(i, m) rep(j, i, m - 1) cnt[i][j + 1] += cnt[i][j];
            rep(j, m) per(i, j + 1, 1) cnt[i - 1][j] += cnt[i][j];
            rep(i, m) rep(j, m) c[i][j] *= cnt[i][j] + st;
            z *= st;
        }

        rep(b, 1 << m) {
            int l = i, r = i;
            while(l and b >> (l - 1) & 1) l--;
            while(r < m and b >> r & 1) r++;
            if(l == r)
                dp[b] *= z;
            else
                dp[b] *= c[l][r - 1];
    ...
    rep(b, 1 << m) {
        ...
        int l = m + 1, r = -1;
        rep(i, m) if(~b >> i & 1) chmin(l, i), chmax(r, i);
    ...
    rep(q) {
        INT(f);
        vi ok(m);
        rep(i, m) ok[i] = d + 1 - abs(f - b[i]);
        mint sum = ans * (d + 1);
        rep(i, m) rep(j, i, m) sum += c[i][j] * min(ok[i], ok[j]);
        OUT(sum);
    }
}