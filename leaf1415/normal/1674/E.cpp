...
{
	ll ret = inf;
	ll m = max(0LL, (2*a-b)/3);
	rep(x, m-5, m+5){
		if(x < 0) continue;
		ll y = max(0LL, max(a-2*x, ceil(b-x, 2)));
		chmin(ret, x+y);
	}
	chmin(ret, max(ceil(a, 2), b));
	chmin(ret, max(ceil(b, 2), a));
	return ret;
}
...
int main(void)
{
	...
	ll ans = inf;
	rep(i, 1, n-1) chmin(ans, calc(a[i], a[i+1]));
	rep(i, 2, n-1) chmin(ans, max(a[i-1], a[i+1]));
	rep(i, 2, n-1){
		ll M = max(a[i-1], a[i+1]), m = min(a[i-1], a[i+1]);
		chmin(ans, ceil(M-m, 2) + m);
	}
	sort(a+1, a+n+1);
	chmin(ans, ceil(a[1], 2)+ceil(a[2], 2));
	outl(ans);
