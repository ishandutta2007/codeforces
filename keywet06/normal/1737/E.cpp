...
const int N = 1000005;
const MInt F2 = ~MInt(2);

MInt P2[N], Q2[N], f[N], s[N];
...
int main() {
    ...
    std::cin >> T;
    P2[0] = Q2[0] = 1;
    for (int i = 1; i < N; ++i) Q2[i] = F2 * Q2[i - 1], P2[i] = 2 * P2[i - 1];
    while (T--) {
        int n;
        std::cin >> n;
        f[n + 1] = s[n + 1] = 0;
        for (int i = n; i >= (n + 2) >> 1; --i) f[i] = 1, s[i] = s[i + 1] + Q2[i] * f[i];
        for (int i = n >> 1; i; --i) s[i] = s[i + 1] + Q2[i] * (f[i] = (s[i + 1] - s[i + i]) * P2[i]);
        std::cout << (n == 1) << '\n';
        for (int i = 2; i <= n; ++i) std::cout << int(Q2[i + 1 >> 1] * f[i] * ((i == n) + 1)) << '\n';
    }
    return 0;
}