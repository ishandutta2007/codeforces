...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    var depth = IntArray(0)
    var size = IntArray(0)
    val hson = IntArray(n){-1}
    val hsonsize = IntArray(n)
    val top = IntArray(n){-1}

    var potential = LongArray(0)


    val intime = IntArray(n) // special for HLD
    val outtime = IntArray(n)
    val hlorder = IntArray(n)


    val pointed = IntArray(n)
    val notheavy = TreeSet<Int>()
    val st = simpleMaxTree(n)
    fun prepareheavylight(){
        if (!hasDFSorder) {
            treeOrderDFS(true)
        }
        potential = calculateDepthWeighted()
        depth = calculateDepth()
        size = calculateSizes()
        leafFirst { v ->
            if(v == root) return@leafFirst
            if(size[v] > hsonsize[parent[v]]){
                hsonsize[parent[v]] = size[v]
                hson[parent[v]] = v
            }
        }
        var t = -1
        fun heavy_dfs(v: Int, p: Int) {
            t++
            hlorder[t] = v
            intime[v] = t
            if(top[v] == -1) top[v] = v
            ...
            NS(v){w ->
                if(w == p || w == hson[v]) return@NS
                heavy_dfs(w,v)
            }
        }
        heavy_dfs(root,-1)
        leafFirst { v->
            outtime[v] = maxOf(outtime[v], intime[v])
            outtime[parent[v]] = maxOf(outtime[parent[v]], outtime[v])
        }
    ...
    fun points(){
        for(i in 0 until n){
            if(pointed[i] != -1 && pointed[i] != hson[i]){
                notheavy.add(intime[i])
            }
        }
    }
    fun ispointingheavy(v:Int):Boolean{
        return pointed[v] == hson[v]
    }

    fun pointTowards(from:Int, to:Int, t0:Int){
        if(pointed[from] == to) return
        if(!ispointingheavy(from)){
            notheavy.remove(intime[from])
        }
        reportnow(from,t0)
        pointed[from] = to
        if(!ispointingheavy(from)){
            notheavy.add(intime[from])
        }
    }
    fun pointAllHeavy(l:Int, r:Int, t0:Int){
        //time
        while(true){
            val next = notheavy.ceiling(l)
            if(next != null && next <= r){
                val v = hlorder[next]
                pointed[v] = hson[v]
                notheavy.remove(next)
                reportnow(v,t0)
            }else{
                break
            }
    ...
    fun lca_heavy_light(u: Int, v: Int):Int {
        var u = u
        var v = v
        while (top[u] != top[v]) {
        ...
        return if(depth[u] > depth[v]) v else u
    }
    fun sumQuery(l:Int, r:Int):Long{
        return 0L
    }
    fun reportnow(v:Int, t0:Int){
        val atleast = maxOf(1,st.queryFunctional(intime[v], outtime[v]) + 1 + potential[v])
        val atmost = potential[v] + t0
        after.add(atleast)
        ret.add(atmost)
    }
    fun act(oriv:Int,t0:Int){
        var v = oriv
        while(true){
            pointAllHeavy(intime[top[v]], intime[v]-1,t0)
            v = top[v]
            if (v == root) {
                break
            }
            pointTowards(parent[v],v, t0)
            v = parent[v]

        }
        if(oriv != root){
            st.setValue(intime[parent[oriv]],t0)
        }

//        return ret
    }

    fun rootpathsum(v: Int): Long {
        var v = v
        var ret = 0L
        while(true){
            ret += sumQuery(intime[top[v]], intime[v])
            v = top[v]
            if (v == root) {
                break
            }
    ...
    fun assertdirection(v:Int){
        var now = v
        while(now != root){
            val p = parent[now]
            assert(pointed[p] == now )
            now = p
        }
    }
...
class multiset(val st:TreeMap<Long, Int> = TreeMap()):MutableMap<Long,Int> by st {
    fun addone(v:Long){
        st[v] = st.getOrDefault(v,0) + 1
    }
    fun removeone(v:Long):Boolean{
...
fun main(){
    solve.rand{
        ...
        share(50)
        for(i in 2..50){
            share("${kotlin.random.Random.nextInt(1 until i)} ${i} 1")
        }
        repeat(50){
            share(kotlin.random.Random.nextInt(1..50))
            share(it+1)
        }
    }
//    solve.userand()
    solve.cases{
        ...
        repeat(n-1){
            ...
            G.addWeighted(from, to, w)
        }
        G.treeOrderDFS(true)

        G.prepareheavylight()
        direct.copyInto(G.pointed)
        G.points()
...
//            just dei G.pointed
        ...
        val R = G.ret.toLongArray()
        val L = G.after.toLongArray()
        for(i in 0 until L.size){
            assert(L[i] <= R[i])
        }


//        val L = longArrayOf(0,3,3)
//        val R = longArrayOf(0,3,3)
        radixCarry(L,R)
...
//        just dei R
        var tnow = -1L
        var pt = -1
        val ops = PriorityQueue<Long>()
        while(ops.isNotEmpty() || pt != R.lastIndex){

//            tnow dei ops.toList()
            while(pt != R.lastIndex && L[pt+1] <= tnow){
                pt ++
                ops.add(R[pt])
//                ops.addone(R[pt])
            }
            if(ops.isNotEmpty()){
                ...
                assert(pt != R.lastIndex)
                val nexttime = L[pt+1]
                assert(nexttime > tnow)
                tnow = nexttime
                continue
            }
        }
