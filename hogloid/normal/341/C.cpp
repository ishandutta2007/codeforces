...
int main(){
	...
	REP(i,n) mark1[i]=mark2[i]=true;
	int rem=0;
	REP(i,n){
		cin>>ar[i];--ar[i];
		chmax(ar[i],-1);

		if(ar[i]!=-1) mark1[i]=false,mark2[ar[i]]=false;
		else ++rem;
	}
	
	Int res=0;
	int match=0;
	REP(i,n) if(mark1[i] && mark2[i]) ++match;
	REP(i,n+1){
		res+=C[match][i]*fact[rem-i]*(i&1?-1:1);
	}
	cout<<res<<endl;
