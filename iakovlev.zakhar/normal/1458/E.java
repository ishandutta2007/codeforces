...
//	}
	...
	public void solve() {
		...
		TreeMap<Integer, Integer> hor = new TreeMap<>();
		TreeMap<Integer, Integer> vert = new TreeMap<>();
		hor.put(0, 0);
		vert.put(0, 0);
		for (int i = 0; i < n; i++) {
			Pair cur = new Pair(in.nextInt(), in.nextInt());
//			Pair cur = new Pair(0, i);
			if (hor.containsKey(cur.y)) {
				hor.put(cur.y, Math.min(cur.x, hor.get(cur.y)));
			} else {
				hor.put(cur.y, cur.x);
			}
			if (vert.containsKey(cur.x)) {
				vert.put(cur.x, Math.min(cur.y, vert.get(cur.x)));
			} else {
				vert.put(cur.x, cur.y);
			}
			set.add(cur);
		}
		...
		while (true) {
			...
			if (hor.containsKey(cur.y) && hor.get(cur.y) <= cur.x) {
				cur.y++;
				continue;
			}
			if (vert.containsKey(cur.x) && vert.get(cur.x) <= cur.y) {
				cur.x++;
				continue;
			}
			Integer nextX = vert.higherKey(cur.x);
			Integer nextY = hor.higherKey(cur.y);
			if (nextX == null) {
				nextX = max + 1;
			}
			if (nextY == null) {
			...
			int dist = Math.min(nextX - cur.x, nextY - cur.y);
			map.put(cur.x, new Pair(dist, cur.y - cur.x));
			cur.x += dist;
			cur.y += dist;
		}
		for (int i = 0; i < m; i++) {
			Pair p = new Pair(in.nextInt(), in.nextInt());
//			Pair p = new Pair(1, i + 1);
			if (set.contains(p)) {
				out.println("LOSE");
				continue;
			}
			Integer prevX = map.floorKey(p.x);
			if (prevX != null) {
				if (p.x - prevX < map.get(prevX).x && (p.y - p.x) == map.get(prevX).y) {
					out.println("LOSE");
					continue;
				}
			}
