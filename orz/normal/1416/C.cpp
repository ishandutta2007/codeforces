...
/*
ll msort(vi& u, vi& uu, int x, int y)
{
	int z = (x + y) / 2;
	ll ans = msort(u, uu, x, z) + msort(u, uu, z + 1, y);
	++z;
	int i = 0, j = 0;
	while (x + i <= z || z + j <= y)
	{
		if (x + i <= z && ((!(z + j <= y )) || (u[x - i] < u[z + j])))
		{
			uu[i + j] = u[i];
			++i;
			ans += j;
...
pll inversions(const vi& v)
{
	vi u = v;
	vi uu = v;
	int n = sz(v);
	ll inv = msort(u, uu, 0, n - 1);
	ll ans = n;
	ans = ans * (ans - 1) / 2;
	ll lel = 1;
	u.pb(1000000001);
