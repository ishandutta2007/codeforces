...
template <class T> get_pos(vector<T>& v, T x) {
...
int main() {
    ...
    f0r(i, n) re(a[i]), --a[i];
    vector<vi> locs(n);
    f0r(i, n) locs[a[i]].pb(i);
    auto cost = [&](int l, int r) -> int {
        if (l == r) return 0;
        auto& v = locs[a[r]];
        int id = get_pos(v, r);
        if (id == 0) return 0;
        if (v[id - 1] < l) return 0;
        return v[id] - v[id - 1];
    };
    LazySeg<int> seg;
    f1r(num, 1, k + 1) {
        if (num == 1) {
            f0r(i, n) {
                dp[num][i] = cost(0, i);
                if (i) dp[num][i] += dp[num][i - 1];
            }
        } else {
            seg.init(n);
            f1r(i, num - 1, n) {
                seg.upd(i - 1, i - 1, dp[num - 1][i - 1]);
                auto& v = locs[a[i]];
                int id = get_pos(v, i);
                int bef = -1;
                if (id) bef = v[id - 1];
                if (bef != -1) {
                    seg.upd(0, bef - 1, i - bef);
                } 
                dp[num][i] = seg.qsum(num - 2, i - 1);
            }
        }
    }
    ps(dp[k][n - 1]);
