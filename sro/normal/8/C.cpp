...
int main(){
	...
	for(int i=1;i<MASK(n);i++){
		...
		dp[i]=dp[i^MASK(j)]+(X[j]*X[j]+Y[j]*Y[j])*2;
		pre[i][0]=j;
		for(int k=j+1;k<n;k++){
			if(BIT(i,k)&&dp[i]>dp[i^MASK(j)^MASK(k)]+(X[j]*X[j]+Y[j]*Y[j])+((X[j]-X[k])*(X[j]-X[k])+(Y[j]-Y[k])*(Y[j]-Y[k]))+(X[k]*X[k]+Y[k]*Y[k])){
				dp[i]=dp[i^MASK(j)^MASK(k)]+(X[j]*X[j]+Y[j]*Y[j])+((X[j]-X[k])*(X[j]-X[k])+(Y[j]-Y[k])*(Y[j]-Y[k]))+(X[k]*X[k]+Y[k]*Y[k]);
				pre[i][1]=k;
			}
		}
	}
