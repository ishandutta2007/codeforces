...
struct BIT
{
    ...
    void clear(){memset(BIT_a,0,sizeof(BIT_a));memset(BIT_b,0,sizeof(BIT_b));}
    BIT(){clear();}
    void Add_a(int n,int u){for(int x=n;x<=N+5;x+=x&-x)BIT_a[x]+=u;}
    void Add_b(int n,int u){for(int x=n;x<=N+5;x+=x&-x)BIT_b[x]+=u;}
    void add(int L,int R,int u){int l=L+1,r=R+1;Add_a(l,u),Add_a(r+1,-u);Add_b(l,l*u),Add_b(r+1,-(r+1)*u);}
    void add(int L,int u){add(L,L,u);}
    ...
    int query(int L){return query(L,L);}
};
template<int nn>
struct BITex
{
    BITex<nn-1> BITT[N+10];
    void clear(){for(int x=0;x<N+10;x++)BITT[x].clear();}
    BITex(){clear();}
    template<typename T1,typename ...Tn>
    void add(const T1& n,const Tn& ...tn){for(int x=n+3;x<=N+5;x+=x&-x)BITT[x].add(tn...);}
    template<typename T1,typename ...Tn>
...
{
    int BITT[N+10];
    void clear(){memset(BITT,0,sizeof(BITT));}
    BITex(){clear();}
    void add(int n,int u){for(int x=n+3;x<=N+5;x+=x&-x)BITT[x]+=u;}
    int query(int n){int r=0;for(int x=n+3;x;x-=x&-x)r+=BITT[x];return r;}
...
while(q--)
{
	int hs,ws,hb,wb;
	cin>>hs>>ws>>hb>>wb;
	if(hs==hb-1||ws==wb-1)cout<<0<<endl;
	else cout<<qq.querylr(hs+1,hb-1,ws+1,wb-1)<<endl;
}
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
