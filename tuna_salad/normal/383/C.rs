...
fn dfs(u: usize, par: usize, e: &[Vec<usize>], d: usize, t: &mut usize, qt: &mut [(usize,usize,usize)]) {
	qt[u].0 = d;
	qt[u].1 = *t;
	*t += 1;
	for &v in e[u].iter() {
...
fn main() {
	...
	let mut qt = vec_dim!([n] = (0,0,0));
	dfs(0, n, &e, 0, &mut 1, &mut qt);
	let mut ft = Vec::with_capacity(2);
	rep!{[2]
		ft.push(fenwick::Tree::new(n+2));
	}
	let mut b = vec![0; n+1];
	for i in 0..n {
		b[qt[i].1] = a[i];
