...
fun main(){
    solve.cases{
        ...
        var ret = 0
        val ended = IntArray(n+5)
        val sum = IntArray(n+5)
        for(a in L){
            if(a == 1){
                ended[a] = ended[a] modM 2
                ended[a] = ended[a] modPlus 1
//                ret = ret modPlus 1
            }else if(a >= 2){
                ended[a] = ended[a] modM 2
                ended[a] = ended[a] modPlus sum[a-2]
//                ret = ret  modPlus sum[a-2]
            }
            sum[a] = sum[a] modM 2
            ended[a + 2] = ended[a + 2] modM 2
            if(a == 0){
                sum[a] = sum[a] modPlus 1
            }else{
                sum[a] = sum[a] modPlus sum[a-1]
            }
        }
        for(c in sum ){
            ret = ret modPlus c
