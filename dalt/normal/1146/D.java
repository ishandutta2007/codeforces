...
public class CFCR1146 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            while (flag) {
                int k = (b - last + a - 1) / a;
                int nextVal = last + k * a;
                limit = Math.max(nextVal, limit);
                if (nextVal > m) {
                    flag = false;
                }
                while (flag && nextVal - b >= 0) {
                    nextVal -= b;
                    if (nodes[nextVal] != null) {
                        flag = false;
                        continue;
                    }
                    nodes[nextVal] = new Node();
                    nodes[nextVal].atLeast = limit;
                }
                last = nextVal;
            }
            ...
            for (int i = 1; i <= limit; i++) {
                cache[i] = cache[i - 1] + i / a + 1;
            }

            Map<Integer, Long> nCacheMap = new HashMap<>(a);
            long sum = (long) a * (m / a - 1) * (m / a) / 2
                    + (long) m / a * (m % a + 1) + m + 1;
            for (int i = 0; i < a; i++) {
                nCacheMap.put(m - i, sum);
                sum -= (m - i) / a + 1;
            }
            ...
            for (int i = 1; i < a; i++) {
                ...
                ans += nCacheMap.get(m - i) - cache[nodes[i].atLeast - i - 1];
            }
