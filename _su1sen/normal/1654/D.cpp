...
map<int, int> merge(map<int, int> &&l1, map<int, int> &&l2) {
    if (l1.size() < l2.size()) return merge(move(l2), move(l1));
    for (const auto &[p, c] : l2) {
        chmax(l1[p], c);
    }
    return l1;
}
...
int main() {
    ...
    vector<vector<pair<int, int>>> fac(M);
    rep(i, 1, M) {
        fac[i] = sieve.factorize(i);
    }

    inv_mods<mint> invs(M);
    ...
    loop(t) {
        ...
        vector<map<int, int>> lcm(n);
        auto dfs = [&](auto dfs, int u, int par) -> void {
            for (const auto &[v, x, y] : g[u]) {
                if (v == par) continue;
                dfs(dfs, v, u);
                for (const auto &[p, c] : fac[y]) {
                    int cv = lcm[v][p] - c;
                    if (cv > 0) {
                        lcm[v][p] = cv;
                    } else {
                ...
                for (const auto &[p, c] : fac[x]) {
                    lcm[v][p] += c;
                }
                lcm[u] = merge(move(lcm[u]), move(lcm[v]));
            }
        };
        dfs(dfs, 0, -1);

        vector<mint> ans(n, 1);
        for (const auto &[p, c] : lcm[0]) {
            ans[0] *= mint(p).pow(c);
        }

        auto dfs2 = [&](auto dfs2, int u, int par) -> void {
            for (const auto &[v, x, y] : g[u]) {
                if (v == par) continue;
                ans[v] = ans[u] * y * invs[x];
                dfs2(dfs2, v, u);
            }
        };
        dfs2(dfs2, 0, -1);
