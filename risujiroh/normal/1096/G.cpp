...
constexpr uint N = 1 << 20; // 1 << __lg(2 * (max_n + max_m - 1) - 1)
constexpr int g = 530; // 

void dft(V<Mint>& a, bool inv = false) {
  int n = a.size();
  int j = 0;
  for (int i = 1; i < n; ++i) {
    int k = n >> 1;
  ...
  assert((Mint::p() - 1) % n == 0);
  Mint zeta = Mint(g).pow((Mint::p() - 1) / n);
...
void convolution(V<Mint>& a, int k) {
  int n = 1 << __lg(2 * (k * (a.size() - 1) + 1) - 1);
  a.resize(n);
  dft(a);
  for (int i = 0; i < n; ++i) a[i] = a[i].pow(k);
  dft(a, true);
