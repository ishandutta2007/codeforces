...
pair<int, array<vi, 2>> test(array<S, 2> a, int shift) {
    array<int, 10> order;
    iota(all(order), 0);
    ...
    // put pairs that add to 10 (possibly with carry)
    int carry = 0;
    while (true) {
        bool done = true;
        for (int d : order) {
            ...
            if (a[0][d] > 0 && a[1][t] > 0) {
                a[0][d]--, a[1][t]--;
                ans[0].push_back(d), ans[1].push_back(t);
