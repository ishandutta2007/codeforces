...
int main(){
	...
	sort(p,p+3);
	for(c=0;c<3;c++){
		for(y=min(p[c].se,p[(c+1)%3].se);y<=max(p[c].se,p[(c+1)%3].se);y++)nodes.insert(pii(p[1].fi,y));
	}
	for(x=p[0].fi;x<=p[1].fi;++x) nodes.insert(pii(x,p[0].se));
	for(x=p[1].fi;x<=p[2].fi;++x) nodes.insert(pii(x,p[2].se));
	cout<<nodes.size()<<endl;
	for(auto g:nodes)cout<<g.fi<<' '<<g.se<<endl;
}