...
ll n;
ll a[3][200005];
ll b[400005], c[400005];

ll get(SparseTable &sp, ll l, ll r, ll t)
{
	ll ret = t + (r-l+1);
	ll res = sp.query(l, r);
	chmax(ret, res+r);
...
int main(void)
{
	...
	while(T--){
		cin >> n;
		rep(i, 1, 2) rep(j, 1, n) cin >> a[i][j], a[i][j]++;
		a[1][1] = 0;
		
		SparseTable sp(digitnum(2*n+5, 2)), sp2(digitnum(2*n+5, 2));
		
		rep(i, 1, n) b[i] = a[1][i], b[n+i] = a[2][n+1-i];
		rep(i, 1, 2*n) sp.set(i, b[i]-i);
		sp.calc();
		
		rep(i, 1, n) c[i] = a[2][i], c[n+i] = a[1][n+1-i];
		rep(i, 1, 2*n) sp2.set(i, c[i]-i);
		sp2.calc();
		...
		rep(i, 1, n){
			if(i % 2){
				t = max(t+1, a[1][i]);
				//outl(t, i+1, 2*n-(i-1), get(sp, i+1, 2*n-(i-1), t));
				chmin(ans, get(sp, i+1, 2*n-(i-1), t));
				t = max(t+1, a[2][i]);
				chmin(ans, get(sp2, i+1, 2*n-i, t));
			}
			else{
				t = max(t+1, a[2][i]);
				chmin(ans, get(sp2, i+1, 2*n-(i-1), t));
				t = max(t+1, a[1][i]);
				chmin(ans, get(sp, i+1, 2*n-i, t));
			}
		}
		outl(ans);
	}
