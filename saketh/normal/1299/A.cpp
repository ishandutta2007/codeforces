...
int main() {
    ...
    vi pref = prefixes(a, 0, [](int a, int b) { return a|b; });
    vi suff = suffixes(a, 0, [](int a, int b) { return a|b; });

    pii ans = {-1, -1};
    F0R (i, N) {
        int first = a[i];
        int rest = pref[i] | suff[i+1];
        ckmax(ans, mp((first | rest) - rest, i));
    }

    pr(a[ans.s]);
    F0R (i, N) if (i != ans.s) pr(" ", a[i]);
    ps();
