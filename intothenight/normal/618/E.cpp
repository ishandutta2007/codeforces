...
using Q = complex<double>; // Monoid (Query Type)
...
using L = complex<double>; // Semigroup (Lazy Type)
...
int main(){
	...
	for(auto qi = 0; qi < qn; ++ qi){
		...
		if(type == 1){
			auto p = seg.query(i);
			double len = sqrt(norm(p));
			seg.set(i, p / len * (len + z));
		}
		else{
			double theta = -z / 180. * acos(-1);
			seg.update(i, n, polar(1., theta));
		}
