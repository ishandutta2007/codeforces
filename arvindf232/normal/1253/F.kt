...
class NewGraph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    fun reversed():NewGraph{
        kotlin.assert(directed)
        val new = NewGraph(n,m,true)
    ...
    fun liftWeights():LongArray{
        return LongArray(n){if(parentEdge[it] == -1) empty else weights[it].toLong()}
    }
...
fun NewGraph.dijstrak(d:LongArray):IntArray{
    //changes the array
    val comps = IntArray(n){-1}
    val Q = TreeSet<Int>(comparing<Int?, Long?> {d[it]}.thenBy { it })
    for(i in d.indices){
        if(d[i] == -1L){
            d[i] = phuge
        }else{
...
//fun NewGraph.singleSourceDijstrak(v:Int): LongArray {
//    return dijstrak(LongArray(n){if(v == it) 0L else -1L })
//}
...
fun main(){
    solve.cases{
        val (n,m,k,q) = getline(4)
        val G = NewGraph(n,m,false)
        repeat(m){
            G.addWeighted(getint - 1 , getint - 1 , getint)
        }
        val dist = LongArray(n){if(it < k) 0L else -1L}
        val comps = G.dijstrak(dist)

        val minedge = mutableMapOf<Pair<Int,Int>,Long>()
        for(e in 0 until m){
            val from = G.from[e shl 1]
            val to = G.to[e shl 1 ]
            if(comps[from] != comps[to]){
                val newweights = dist[from] + dist[to] + G.weights[e]
                val p = Pair(minOf(comps[from], comps[to]), maxOf(comps[from], comps[to]))
                minedge[p] = minOf(newweights, minedge.getOrDefault(p,Long.MAX_VALUE))
            }
        }

        val dsu = DisjointUnionSets(k)
        val inducedG = NewGraph(k,k-1,false)
        val things = minedge.entries.sortedBy { it.value }
        val distweights = LongArray(k){Long.MAX_VALUE}
        for((p,v) in things){
            val (a,b) = p
            if(dsu.union(a,b)){
                val e = inducedG.add(a,b)
                distweights[e] = v
            }
        }
        inducedG.treeOrderDFS(true)
        val liftweights = LongArray(k){if(inducedG.parentEdge[it] == -1) inducedG.empty else distweights[inducedG.parentEdge[it]]}
        inducedG.prepareLifting(liftweights)

        repeat(q){
            val a = getint - 1
            val b = getint - 1
            val cost = inducedG.calculatePath(a,b)
            put(cost)
        }
