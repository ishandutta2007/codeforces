...
int main() {
    ...
    F0Rd (i, N) {
        FOR (j, i, N) {
            int k = min_element(strip.begin() + i, strip.begin() + j + 1) - strip.begin();
            mn left = 1;
            if (k > i) {
                left = dp[i][k-1];
                FOR (sp, i, k) left += dp[sp][k-1] * (sp > i ? dp[i][sp-1] : mn(1));
            }
            mn right = 1;
            if (k < j) {
                right = dp[k+1][j];
                FOR (sp, k, j) right += (sp > k ? dp[k+1][sp] : mn(1)) * dp[sp+1][j];
            }
            dp[i][j] = left * right;
        }
    }
