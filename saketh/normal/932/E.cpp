...
namespace vector_operators {
    ...
    template<typename T> vector<T> quo(vector<T> a, vector<T> b) {
        ...
        rem_lead(a), rem_lead(b), assert(!b.empty());
        int s = sz(a) + 1 - sz(b);
        vector<T> res = reversed(a) * inv(reversed(b), s);
        if (sz(res) < s) res.resz(s);
        return {res.rend() - s, res.rend()};
    }
...
int main() {
    ...
    vmn xpk(MAXN);
    FOR (x, 1, MAXN) xpk[x] = mn(x).pow(K);

    vmn brute(MAXN);
    FOR (n, 1, MAXN) FOR (x, 1, n+1) brute[n] += mn::ncr(n, x) * xpk[x];
