...
int main(void)
{
	...
	rep(i, 1, 39) rep(j, 1, n){
		succ[i][j] = succ[i-1][succ[i-1][j]];
		sum[i][j] = sum[i-1][j] + sum[i-1][succ[i-1][j]];
		mn[i][j] = min(mn[i-1][j], mn[i-1][succ[i-1][j]]);
	}
