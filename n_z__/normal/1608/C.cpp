...
int ida[100001],idb[100001],c[100001],d[100001];
/*var---------------------------------------------------------------------------------------------*/
void Main(){
/*Main-head---------------------------------------------------------------------------------------*/
    int n;
    ...
    for(int x=1;x<=n;x++)
    cin>>b[x].first,b[x].second=x;
    sort(a+1,a+1+n);
    sort(b+1,b+1+n);
    for(int x=1;x<=n;x++)
    ida[a[x].second]=idb[b[x].second]=x,c[x]=a[x].second,d[x]=b[x].second;
    int lsta=n,idbmax=n;
    int nowamin=1000000,nowbmin=idb[c[n]];
    s[c[n]]=1;
    while(true){
        for(int x=nowbmin;x<=idbmax;x++)
        s[d[x]]=1,nowamin=min(nowamin,ida[d[x]]);
        idbmax=min(idbmax,nowbmin);
        for(int x=nowamin;x<=lsta;x++)
        s[c[x]]=1,nowbmin=min(nowbmin,idb[c[x]]);
        if(lsta==nowamin)break;
        lsta=nowamin;
    }
    swap(a,b);
    swap(c,d);
    lsta=n,idbmax=n;
    nowamin=1000000,nowbmin=idb[c[n]];
    s[c[n]]=1;
    while(true){
        for(int x=nowbmin;x<=idbmax;x++)
        s[d[x]]=1,nowamin=min(nowamin,ida[d[x]]);
