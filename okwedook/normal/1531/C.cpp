...
signed main() {
    ...
    for (int N = 1; ; ++N) {
        if (N * N < n) continue;
        vector<vector<vector<int>>> can(N + 1, vector<vector<int>>(N + 1, vector<int>(N * N + 1)));
        vector<vector<vector<vector<int>>>> p(N + 1, vector<vector<vector<int>>>(N + 1, vector<vector<int>>(N * N + 1)));
        can[0][N][0] = true;
        for (int pref = 1; pref <= N; ++pref) {
            for (int last = pref; last <= N; ++last)
                for (int prelast = last; prelast <= N; ++prelast)
                    for (int wassum = 0; wassum <= N * N; ++wassum) {
                        int sum = wassum + 2 * last - (2 * pref - 1);
                        if (sum > N * N) break;
                        if (can[pref - 1][prelast][wassum]) {
                            can[pref][last][sum] = true;
                            p[pref][last][sum] = {pref - 1, prelast, wassum};
                        }
                    }
        }
        for (int pref = 1; pref <= N; ++pref) {
            for (int val = pref; val <= N; ++val) {
                if (can[pref][val][n]) {
                    vector<string> ans(N, string(N, '.'));
                    vector<int> pos = {pref, val, n};
                    while (pos[0] > 0) {
                        int col = pos[0] - 1;
                        int height = pos[1];
                        for (int h = 0; h < height; ++h)
                            ans[h][col] = ans[col][h] = 'o';
