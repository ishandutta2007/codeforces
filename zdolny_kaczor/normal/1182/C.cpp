...
char in[nax];
map <pair <int, int>, vector <string> > seen;
bool vowel(char c) {
...
int main() {
	...
	REP(_, n) {
		...
		seen[pii(cou, last)].emplace_back(in);
	}
	vector <pair <string, string> > pairs1, pairs2;
	map <int, vector<string> > seen2;
	for (auto & x : seen) {
		while (x.second.size() >= 2u) {
			pairs2.emplace_back(x.second.back(), *(x.second.end() - 2));
			x.second.pop_back();
			x.second.pop_back();
		}
		for (string &c : x.second)
			seen2[x.first.first].push_back(c);
	}
	for (auto &x : seen2) {
		while (x.second.size() >= 2u) {
			pairs1.emplace_back(x.second.back(), *(x.second.end() - 2));
			x.second.pop_back();
			x.second.pop_back();
		}
	}
	debug << imie(pairs1) imie(pairs2);
	while (pairs2.size() > pairs1.size()) {
		pairs1.push_back(pairs2.back());
		pairs2.pop_back();
	}
	debug << imie(pairs1) imie(pairs2);
	printf("%d\n", (int) pairs2.size());
	REP(i, (int) pairs2.size()) {
		printf("%s %s\n%s %s\n", pairs1[i].first.c_str(), pairs2[i].first.c_str(), pairs1[i].second.c_str(), pairs2[i].second.c_str());
	}
}