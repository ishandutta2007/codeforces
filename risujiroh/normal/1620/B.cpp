...
void solve(ll) {
  auto w = input();
  auto h = input();
  auto x0 = input<std::vector<ll>>(input());
  auto xh = input<std::vector<ll>>(input());
  auto y0 = input<std::vector<ll>>(input());
  auto yw = input<std::vector<ll>>(input());
  auto ans = -inf;
  chmax(ans, (x0.back() - x0.front()) * h);
  chmax(ans, (xh.back() - xh.front()) * h);
  chmax(ans, (y0.back() - y0.front()) * w);
  chmax(ans, (yw.back() - yw.front()) * w);
  println(ans);
}