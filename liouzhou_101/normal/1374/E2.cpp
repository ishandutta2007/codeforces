...
void solve()
{
	...
	for (int a = 0; a < 2; ++a)
		for (int b = 0; b < 2; ++b)
		{
			...
			s[a][b].push_back(0);
			for (auto [x, y] : v[a][b]) s[a][b].push_back(s[a][b].back() + x);
		}

	splay_tree T;
	for (auto [x, y] : v[0][0]) T.insert(x);
	for (int i = k; i < v[1][0].size(); ++i) T.insert(v[1][0][i].first);
	for (int i = k; i < v[0][1].size(); ++i) T.insert(v[0][1][i].first);

	int res = INF, choose = -1;
	for (int i = 0; i <= v[1][1].size() && i <= m; ++i)
	{
		if (k - i <= (int)v[1][0].size() && k - i <= (int)v[0][1].size())
		{
			int need = m - i - 2 * max(k - i, 0);
			if (need >= 0 && need <= T.size())
			{
				int tmp = T.prefix_sum(need);
				if (res > s[1][1][i] + s[1][0][max(k - i, 0)] + s[0][1][max(k - i, 0)] + tmp)
				{
					res = s[1][1][i] + s[1][0][max(k - i, 0)] + s[0][1][max(k - i, 0)] + tmp;
					choose = i;
				}
			}
		}
		if (k - i - 1 >= 0)
		{
			if (k - i - 1 < v[1][0].size()) T.insert(v[1][0][k - i - 1].first);
			if (k - i - 1 < v[0][1].size()) T.insert(v[0][1][k - i - 1].first);
		}
	}
	if (res == INF) return writeln(-1);
	writeln(res);
	vector<int> tmp;
	for (int i = 0; i < choose; ++i) tmp.push_back(v[1][1][i].second);
	for (int i = 0; i < max(k - choose, 0); ++i)
	{
		tmp.push_back(v[1][0][i].second);
		tmp.push_back(v[0][1][i].second);
	}
	int need = m - choose - 2 * max(k - choose, 0);
	vector<pii> rest;
	for (int i = max(k - choose, 0); i < v[1][0].size(); ++i) rest.push_back(v[1][0][i]);
	for (int i = max(k - choose, 0); i < v[0][1].size(); ++i) rest.push_back(v[0][1][i]);
	for (auto p : v[0][0]) rest.push_back(p);
	sort(rest.begin(), rest.end());
	for (int i = 0; i < need; ++i) tmp.push_back(rest[i].second);
	print(tmp);
}
