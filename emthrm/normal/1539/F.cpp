...
int main() {
  ...
  REP(i, n) m[a[i]].emplace_back(i);
  FenwickTree<int> bit(n);
  REP(i, n) bit.add(i, 1);
  LazySegmentTree<monoid::RangeMaximumAndAddQuery<int, INF>> l(vector<int>(n, 0)), r(vector<int>(n, 0));
  REP(i, n) l.apply(0, i + 1, 1);
  for (int i = n - 1; i >= 0; --i) r.apply(i, n, 1);
  vector<int> ans(n, 0);
  for (auto [_, v] : m) {
    for (int i : v) {
      // REP(j, n) cout << bit[j] << " \n"[j + 1 == n];
      // cout << l.get(0, i + 1) << ' ' << bit.sum(i + 1, n) << ' ' << r.get(i, n) << ' ' << bit.sum(0, i) << '\n';
      chmax(ans[i], (l.get(0, i + 1) - bit.sum(i + 1, n) + r.get(i, n) - bit.sum(0, i) - 1) / 2);
    }
    for (int i : v) {
      bit.add(i, -2);
      l.apply(0, i + 1, -2);
  ...
  REP(i, n) m[-a[i]].emplace_back(i);
  REP(i, n) bit.add(i, 2);
  REP(i, n) l.apply(0, i + 1, 2);
  for (int i = n - 1; i >= 0; --i) r.apply(i, n, 2);
  for (auto [_, v] : m) {
    for (int i : v) {
      chmax(ans[i], (l.get(0, i + 1) - bit.sum(i + 1, n) + r.get(i, n) - bit.sum(0, i) - 2) / 2);
    }
    for (int i : v) {
      bit.add(i, -2);
      l.apply(0, i + 1, -2);
