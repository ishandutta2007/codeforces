...
int main() {
    setIO(); re(n,m);
    FOR(i,1,n+1) {
        FOR(j,1,n+1) if (i != j) dist[i][j] = MOD;
    }
    F0R(i,m) {
        int u,v; re(u,v);
        ckmin(dist[u][v],1);
    ...
    int s,b,k; re(s,b,k);
    vector<array<int,5>> S(s);
    vector<array<int,3>> B(b); 
    F0R(i,s) {
        F0R(j,4) re(S[i][j]);
        S[i][4] = i+1;
    }
    re(B);
    trav(t,S) swap(t[1],t[0]);
    trav(t,B) swap(t[1],t[0]);
    sort(all(S)), sort(all(B));
    int ind = 0; ll ans = 0;
    FOR(i,1,n+1) cur[i] = -INF;
    trav(t,S) {
        while (ind < sz(B) && B[ind][0] <= t[0]) {
            ckmax(cur[B[ind][1]],(ll)B[ind][2]);
            ind ++;
        }
        ll res = -INF;
        FOR(i,1,n+1) if (dist[t[1]][i] <= t[2]) ckmax(res,cur[i]);
        if (res != -INF) res -= t[3];
        if (res < 0) D.addEdge(t[4],s+1,-res);
        else {
            D.addEdge(0,t[4],res);
            ans += res;
        }
