...
int main()
{
	...
	if(sx==tx||sy==ty)
		...
		double ys,xs,yt,xt;
		int i,j;
		xs=-1.0*b/aa*sy-1.0*c/aa;
		xt=-1.0*b/aa*ty-1.0*c/aa;
		ys=-1.0*aa/b*sx-1.0*c/b;
		yt=-1.0*aa/b*tx-1.0*c/b;
		a[0][0]=sx,a[0][1]=ys;
		a[1][0]=xs,a[1][1]=sy;
		a[2][0]=tx,a[2][1]=yt;
		a[3][0]=xt,a[3][1]=ty;
		...
		for(i=0;i<4;i++)
			for(j=0;j<4;j++)
				ans=min((long double)ans,abs(sx-a[i][0])+abs(sy-a[i][1])+abs(a[j][0]-tx)+abs(a[j][1]-ty)+sqrt(sqr(a[i][0]-a[j][0])+sqr(a[i][1]-a[j][1])));
		printf("%.10f\n",ans);
	}
}