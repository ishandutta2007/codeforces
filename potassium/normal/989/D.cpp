...
int main() {
	...
	ll ans = 0;
	const int MAXI = 131072;
	for (auto& i : cr) {
		int p = cl.size();
		if (i <= -l)
			p = lower_bound(all(cl), -(i + l)) - cl.begin();

		int l = p, r = p - 1;
		for (int inc = MAXI; inc >= 1; inc >>= 1) {
			if (r + inc < cl.size() && check(i, cl[r + inc]))
				r += inc;
		}
		
		for (int dec = MAXI; dec >= 1; dec >>= 1) {
			if (l - dec >= 0 && check(i, cl[l - dec]))
				l -= dec;
		}
