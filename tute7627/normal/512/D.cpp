...
using modint = ModInt< 1000000009 >;modint pow(ll n, ll x){return modint(n).pow(x);}modint pow(modint n, ll x){return n.pow(x);}
//using modint=ld;
...
int main(){
  ...
  rep(i,0,n){
    ...
    for(auto z:v){
      ll cnt=0;
      for(auto to:g[z])if(!used[to])cnt++;
      if(cnt==1){
        root=z;
      }
    }
    vector<modint>add(n+1);
    {
      auto dfs=[&](auto &&f,ll k,ll par)->vector<modint>{
        vector<modint>dp(1,1);
        for(auto to:g[k]){
          if(to==par||!used[to])continue;
          auto tmp=f(f,to,k);
          vector<modint>ndp(dp.size()+tmp.size()-1);
          rep(i,0,dp.size())rep(j,0,tmp.size()){
            ndp[i+j]+=dp[i]*tmp[j]*comb.C(i+j,i);
          }
          dp.swap(ndp);
        }
        modint ba=dp.back();
      ...
      if(root==-1){
        for(auto z:v){
          auto tmp=dfs(dfs,z,-1);
          rep(j,0,tmp.size())add[j]+=tmp[j];
        }
        rep(j,0,v.size())add[j]*=comb.inv(v.size()-j);
      }
      else{
        add=dfs(dfs,root,-1);
      }
    ...
    rrep(i,0,n){
      rep(j,1,add.size()){
        if(i+j<=n)dp[i+j]+=dp[i]*add[j]*comb.C(i+j,i);
      }
    }
  }
