...
void solve() {
  ...
  auto f_ve = [&](ll v, node_data& x) -> edge_data {
    ...
    e.sum = x.sum + x.CNT[0];
    e.CNT = x.CNT;
    e.CNT.insert(e.CNT.begin(), e.CNT.back());
    e.CNT.pop_back();
    return e;
  };
  // 
  auto mul_e = [&](edge_data& x, edge_data& y) -> edge_data {
    ...
    z.sum = x.sum + y.sum;
    z.CNT.resize(K);
    FOR(i, K) z.CNT[i] = x.CNT[i] + y.CNT[i];
    return z;
  };
  ...
  auto rerooting = [&]() {
    ...
    vc<node_data> dp(N);
    vc<edge_data> dp2_e(N);
    dp2_e[0] = e_unit;

    for (auto&& v : T.V) {
      auto& ch = T.ch[v];
      auto n = ch.size();
      vc<edge_data> cum_l(n + 1);
      vc<edge_data> cum_r(n + 1);
      cum_l[0] = e_unit;
      cum_r[n] = e_unit;
