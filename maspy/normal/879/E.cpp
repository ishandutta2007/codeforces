...
void solve() {
  ...
  auto compress = [&](vi& A) -> void {
    for (auto&& a: A) a = -a;
    vi key = A;
    UNIQUE(key);
    for (auto&& a: A) a = LB(key, a);
  };
  FOR(k, K) compress(dat[k]);
  // FOR(k, K) print(dat[k]);

  using Mono = Monoid_MinMax<int, 1 << 30>;
  vc<SegTree<Mono>> seg(K, SegTree<Mono>(N));

  FOR(n, N) {
    ll i = dat[0][n];
    bit.add(i, 1);
    FOR(k, K) seg[k].set(i, mp(dat[k][n], dat[k][n]));
    ll l = i;
    FOR(k, K) {
      ...
      chmin(l, seg[k].max_right(check, 0));
    }
    ll r = i;
    FOR(k, K) {
      auto check = [&](auto e) -> bool {
      ...
      chmax(r, seg[k].min_left(check, N));
    }
