...
mod simple_full_segtree {
	...
	impl Val {
		...
		pub fn new(v: V) -> Self {
			l!(pn,np = 0);
			Self {pp: v, pn, np, nn: -v}
		}
		pub fn combine(a: &Self, b: &Self) -> Self {
			let mut t = Self::default();
			t.pp = (a.pp + b.np).max(a.pn + b.pp);
			t.pn = (a.pp + b.nn).max(a.pn + b.pn);
			t.np = (a.np + b.np).max(a.nn + b.pp);
			t.nn = (a.np + b.nn).max(a.nn + b.pn);
			t
		}
...
fn main() {
	...
	rep!{[rin.u()]
		...
		let res0 = st.query_all().result();
		writeln!(rout, "{}", res0).ok();
		rep!{[q]
			l!(l,r = rin.u()-1);
			if l != r {
				a.swap(l,r);
