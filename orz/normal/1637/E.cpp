...
ll find_ans(const vi& a, const set<pii>& y)
{
	map<int, int> freq;
	for (int i : a)
		++freq[i];
	...
	for (pii p : freq)
		s[p.Y].pb(p.X);
	for (auto& p : s)
		sort(p.Y.rbegin(), p.Y.rend());
	ll ans = 0;
	for (pair<const int, vi>& p : s)
		for (pair<const int, vi>& q : s)
		{
			if (p.X <= q.X)
			{
				priority_queue<pair<int, pii>> ind;
				set<pii> us;
				ind.emplace(p.Y[0] + q.Y[0], pii(0, 0));
				bool fd = true;
				while (sz(ind) && fd)
				{
					pair<int, pii> z = ind.top();
					ind.pop();
					if ((p.X != q.X) || (z.Y.X <= z.Y.Y))
					{
						us.insert(z.Y);
						if (((p.X != q.X) || (z.Y.X < z.Y.Y)) && !y.count(pii(p.Y[z.Y.X], q.Y[z.Y.Y])))
						{
							ans = max(ans, ll(p.X + q.X) * z.X);
							fd = false;
						}

						pii h1 = z.Y;
						++h1.X;
						if (h1.X < sz(p.Y) && !us.count(h1))
						{
							us.insert(h1);
							ind.emplace(p.Y[h1.X] + q.Y[h1.Y], h1);
						}

						pii h2 = z.Y;
						++h2.Y;
						if (h2.Y < sz(q.Y) && !us.count(h2))
						{
							us.insert(h2);
							ind.emplace(p.Y[h2.X] + q.Y[h2.Y], h2);
						}
