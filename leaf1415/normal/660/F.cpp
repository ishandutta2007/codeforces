...
struct CHT{
	...
	{
		T a1 = p1.first, b1 = p1.second;
		T a2 = p2.first, b2 = p2.second;
		T a3 = p3.first, b3 = p3.second;
...
void calc(ll l, ll r)
{
	...
	rep(i, m+1, r){
		s += a[i] * (i-m);
		vec.push_back(P(sum[i]-sum[m], s));
	}
	sort(all(vec));
	for(auto p : vec) chmax(ans, -cht.query(p.first) + p.second);
