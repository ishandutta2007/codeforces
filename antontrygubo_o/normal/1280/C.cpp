...
{
    for (auto it: G[s]) if (it.first!=p)
        {
            parent[it.first] = s;
            children[s].push_back(it);
            dfs(it.first, s);
            if (subtree[it.first]%2==1) G1+=it.second;
            subtree[s]+=subtree[it.first];
        }
    subtree[s]++;
}

bool check_centroid(int s)
{
    int maxx = n/2;
    for (auto it: children[s]) if (subtree[it.first]>maxx) return false;
    if (subtree[s]<n - maxx) return false;
    return true;
}


void dfs2(int s, int p, ll ans = 0)
{
    B1+=ans;
    //cout<<ans<<' '<<B1<<endl;
    for (auto it: G[s]) if (it.first!=p)
    {
        dfs2(it.first, s, ans+it.second);
    }
}
...
void solve()
{
    ...
    int centroid = 1;
    while (!check_centroid(centroid)) centroid++;
