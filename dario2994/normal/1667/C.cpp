...
int main() {
    ...
    cin >> n;
    int k = 0;
    while (n-(k+1) >= 2*(k+1) -1) k++;
    int m = n-k;
    vector<bool> xocc(m+1, false);
    vector<bool> yocc(m+1, false);
    vector<pair<int,int>> sol;
    auto add = [&](int x, int y) {
        assert(!xocc[x] and !yocc[y]);
        xocc[x] = true;
        yocc[y] = true;
        sol.push_back({x, y});
    };
    for (int i = 1; i <= k; i++) add(i, 2*i-1);
    for (int i = 1; i < k; i++) add(k+i, 2*i);

    vector<int> free_x, free_y;
    for (int i = 1; i <= m; i++) if (!xocc[i]) free_x.push_back(i);
    for (int i = 1; i <= m; i++) if (!yocc[i]) free_y.push_back(i);

    for (int i = 0; i < SZ(free_x); i++) add(free_x[i], free_y[i]);
