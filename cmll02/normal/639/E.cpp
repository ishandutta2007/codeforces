...
inline double mxe(double &a,double b){return a=a>b?a:b;}
...
#endif
int p[1234456],t[1123456],q[1234456],sum[1223745];
int x[564098],y[340857];
const double eps=1e-7;
int cmp(int x,int y){return p[x]*t[y]==p[y]*t[x];}
void trans(int *a,int n)
{
	static int b[1234465];
	rg(n)b[i]=a[q[i]];gr
	rg(n)a[i]=b[i];gr
}
#define T sum[n]
int check(int n,double m)
{
	double maxn=0;
	int ok=1;
	rg(n)
	while(p[ok]!=p[i])mxe(maxn,1.0*p[ok]*(1-m*x[ok]/T)),ok++;
	if(1.0*p[i]*(1-m*y[i]/T)<maxn)return 0;
	gr
	return 1;
}
signed main()
{
	...
	rg(n)t[i]=read();gr
	rg(n)q[i]=i;gr
	std::sort(q+1,q+1+n,[&](int x,int y){return p[x]*t[y]>p[y]*t[x];});
	trans(p,n);trans(t,n);
	double l=eps,r=1,ans=0;
	rg(n)sum[i]=sum[i-1]+t[i];gr
	for(int i=1;i<=n;)
	{
		int j=i;while(j<n&&cmp(j,j+1))j++;
		for(int k=i;k<=j;k++)y[k]=sum[j],x[k]=sum[i-1]+t[k];
		i=j+1;
	}
	std::sort(q+1,q+1+n,[&](int x,int y){return p[x]<p[y];});
	trans(p,n);
	trans(t,n);
	trans(x,n);
	trans(y,n);
