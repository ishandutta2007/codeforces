...
int main() {
  int n, s, r;
  scanf("%d%d%d", &n, &s, &r);
  Binom<ModT> binom(n + s);
  auto count = [&](int n, int s, int max) -> ModT {
    // < max
    if (n == 0) {
      return ModT(s == 0 ? 1 : 0);
    }
    ModT result{0};
    for (int e = 0; max * e <= s && e <= n; ++e) {
      ModT t = binom(n, e) * binom(s - max * e + n - 1, n - 1);
      if (e & 1) {
        result -= t;
      } else {
        result += t;
  ...
  ModT total{0};
  for (int a = r; a <= s; ++a) {
    total += count(n - 1, s - a, s + 1);
  }
  ModT result{0};
  for (int a0 = r; a0 <= s; ++a0) {
    for (int same = 1; a0 * same <= s && same <= n; ++same) {
      result += binom(n - 1, same - 1) * count(n - same, s - a0 * same, a0) *
                ModT(same).inverse();
    }
  }
  printf("%d\n", (result * total.inverse()).get());
}