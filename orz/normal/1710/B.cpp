...
struct st {
	vl m;
	int n;
	st(const vl& g) : n(sz(g)), m(sz(g) * 4) {
		fl(0, 0, n, g);
	}
	void fl(const int v, const int wl, const int wr, const vl& g) {
		if (wr - wl == 1)
			m[v] = g[wl];
		else {
			fl(2 * v + 1, wl, (wl + wr) / 2, g);
			fl(2 * v + 2, (wl + wr) / 2, wr, g);
			m[v] = max(m[2 * v + 1], m[2 * v + 2]);
		}
	}
	ll mx(const int sl, const int sr) {
		return mx(0, 0, n, sl, sr);
	}
	ll mx(const int v, const int wl, const int wr, const int sl, const int sr) {
		if (sl >= wr || wl >= sr)
			return -INF;
		if (sl <= wl && sr >= wr)
			return m[v];
		int med = (wl + wr) / 2;
		return max(mx(2 * v + 1, wl, med, sl, sr), mx(2 * v + 2, med, wr, sl, sr));
	}
};
...
int another_main()
{
	...
	for (int I = 0; I < t; ++I) {
		...
		st s(g), s1(g1), s2(g2);
		for (pii pr : xp) {
			auto it1 = upper_bound(xx.begin(), xx.end(), pr.X - pr.Y);
			int i1 = it1 - xx.begin();
			ll ans1 = s.mx(0, i1);
			auto it2 = upper_bound(xx.begin(), xx.end(), pr.X);
			int i2 = it2 - xx.begin();
			ll ans2 = s2.mx(i1, i2) + pr.X - pr.Y;
			auto it3 = upper_bound(xx.begin(), xx.end(), pr.X + pr.Y);
			int i3 = it3 - xx.begin();
			ll ans3 = s1.mx(i2, i3) - pr.X - pr.Y;
			ll ans4 = s.mx(i3, xx.size());
