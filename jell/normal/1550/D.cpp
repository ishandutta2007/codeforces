#line 1 "codeforces-workspace\\contest\\1550\\d\\d.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\1550\\d\\d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1550\\d\\d.cpp"
...
#line 35 "codeforces-workspace\\contest\\1550\\d\\d.cpp"
...
void main() {
  ...
  mint ans;
  vector half{n / 2};
  if (n % 2) half.emplace_back(n / 2 + 1);
  ...
  for (auto m : half) {
    for (auto _ : {0, 1}) {
      ...
      for (auto j : range(n)) {
        ...
        if (t < 0) t = 0;
        mint now;
        // auto now = binom(n - 1 - j - t, m - 1 - t);
        // if (n - j == m && t <= m) now += 1;
        if (t < n - j) {
          now = binom(n - 1 - j - t, m - 1 - t);
        } else if (t == n - j && m == t) {
          now = 1;
        }
        ans += now * (j - l);
