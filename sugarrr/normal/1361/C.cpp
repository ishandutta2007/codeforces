...
vl calc(vl_l a){
    ...
    vl_l A=a;
    set<vl>MS;
    vl BIG={inf,inf,inf,inf};
    MS.insert(BIG);
    rep(i,0,n-1){
        l_l x=a[i];
        MS.insert({x.fi,x.se,i,0});
        MS.insert({x.se,x.fi,i,1});
    }
    vl nxpos(n,-1);
    while((int)MS.size()>=2){
        auto remove=[&](vl s){
            if(s[3]){
                swap(A[s[2]].fi,A[s[2]].se);
            }
            MS.erase(s);
            swap(s[0],s[1]);
            s[3]=1-s[3];
        ...
        vl now=sta;
        while(1){
            vl nxt=*MS.lower_bound({now[1],-1,-1,-1});
            if(nxt[0] != now[1]){
                nxpos[now[2]] = sta[2];
                break;
            }
  ...
  //  cerr<<"nxpos ";for(auto x:nxpos)cerr<<x<<" ";cerr<<endl;
  //  for(auto x:A)cerr<<x.fi<<" "<<x.se<<endl;
    {
        dsu d(n);
        rep(i,0,n-1)d.merge(i,nxpos[i]);
        map<ll,ll>mp;
        rep(i,0,n-1){
            if(mp.find(A[i].se)==mp.end()){
                mp[A[i].se]=i;
            }else{
                ll j=mp[A[i].se];
                if(!d.same(i,j)){
                    d.merge(i,j);
                    swap(nxpos[i],nxpos[j]);
                }
            }
        }
    }
