...
signed main()
{
	...
	while(!q.empty())
	{
		...
		if(u==n+n){puts("YES");goto win;}
		if(u!=n&&a[u+1]==0&&!v[u+1])q.push(u+1),v[u+1]=1;
		if(u<=n&&a[u+n]==0&&!v[u+n])q.push(u+n),v[u+n]=1;
		if(u<n&&a[u+n+1]==0&&!v[u+n+1])q.push(u+n+1),v[u+n+1]=1;
		if(u>n&&a[u-n+1]==0&&!v[u-n+1])q.push(u-n+1),v[u-n+1]=1;
	}
	puts("NO");
	win:;
