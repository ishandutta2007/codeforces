...
bool solve(int n, vi a, vi b) {
	...
	// // a.clear();
	// // b.clear();
	// // rep(i, n) a.push_back(ab[i].first), b.push_back(ab[i].second);

	const int value_max = n + 2;
	vi count(value_max);
	rep(i, n) count[a[i]]++;

	int mode = 0;
	rep(i, value_max) if(count[mode] < count[i]) mode = i;

	graph<int> g(value_max);
	rep(i, n) {
		if(a[i] != mode && mode != b[i]) g.add_edge(a[i], b[i], 1, true);
	}
