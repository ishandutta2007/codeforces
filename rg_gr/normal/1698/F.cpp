...
#endif
int az[1234567],ay[1234567],cc,a[1234567],b[1234567];
#include <map>
...
signed main()
{
	...
	map A,B;
	cc=0;
	rg(n-1)A[{min(a[i],a[i+1]),a[i]+a[i+1]}]++;gr
	rg(n-1)B[{min(b[i],b[i+1]),b[i]+b[i+1]}]++;gr
	if(A!=B||a[1]!=b[1]||a[n]!=b[n])puts("NO");else {
		puts("YES");
		for(int i=2;i<n;i++)
		{
			if(a[i]==b[i])continue;;
