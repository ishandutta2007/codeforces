...
int main(){
    ...
    vector<ll> two,one;
    for(auto[x,c]:w){
        if(c>=2) two.emplace_back(x);
        if(c==1) one.emplace_back(x);
    }
    ...
    rep(ii,K){
        ll r = red[ii];
        ll n = lower_bound(all(two),r) - two.begin();
        ll m = lower_bound(all(one),r) - one.begin();
        vector<mint> A(2*n+1),B(m+1);
        rep(i,2*n+1){
            A[i] = comb(2*n,i);
        }
        rep(i,m+1){
