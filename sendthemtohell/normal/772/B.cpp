...
bool f(vector<point> &v, ld r)
{
	for (int i = 0; i < n; ++i)
	{
		...
		ld a = v[j].y - v[i].y;
		ld b = v[i].x - v[j].x;
		ld c = v[i].y * (v[j].x - v[i].x) + v[i].x * (v[i].y - v[j].y);

		ld d = abs(a * v[k].x + b * v[k].y + c) / hypotl(v[i].x - v[j].x, v[i].y - v[j].y);
		if (d <= 2 * r)
		{
			return false;
		}
...
inline ANDREIKKAA_RETURN_TYPE mainFunction(ANDREIKKAA_PARAMETERS)
{
	...
	for (int i = 0; i < n; ++i)
	{
		for (int j = i + 1; j < n; ++j)
		{
			ld dst = hypotl(v[i].x - v[j].x, v[i].y - v[j].y);
			r = min(r, dst);
		}
	}
