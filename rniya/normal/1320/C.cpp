...
int main(){
    ...
    vector<int> v(m);
    for (int i=0;i<m;++i) v[i]=B[i].first;
    struct mon{
        int x,y,z;
        mon(int x,int y,int z):x(x),y(y),z(z){}
        const bool operator<(const mon &other){
            return x<other.x;
        }
    };
    vector<mon> M;
    ...
    for (int i=0;i<m;++i) seg.update(i,i+1,INF-B[i].second);
    int ans=-INF;
    int now=0;
    for (int i=0;i<n;++i){
        while(now<p&&M[now].x<A[i].first){
            int nxt=upper_bound(ALL(v),M[now].y)-v.begin();
            seg.update(nxt,m,M[now++].z);
        }
        chmax(ans,-A[i].second+seg.query(0,m));
    }
    cout << ans << '\n';
}