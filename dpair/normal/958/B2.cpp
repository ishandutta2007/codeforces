...
int fa[MAXN], dep[MAXN], dp[MAXN], root;

void dfs(int x, int pre) {
    dep[x] = dep[fa[x] = pre] + 1;
    if(dep[root] < dep[x]) root = x;
...
void dfs2(int x, int pre) {
    ...
    for (int i = head[x];i;i = nxt[i]) {
        if(e[i] == pre) continue;
        dfs2(e[i], x);
        if(dp[e[i]] > dp[s]) s = e[i];
    }
    dp[x] = dp[s] + 1; 
    for (int i = head[x];i;i = nxt[i]) {
        if(e[i] == pre || e[i] == s) continue;
        q.push(dp[e[i]]);
    }
}
...
signed main(){
    ...
    dfs(1, 0); dfs2(root, 0); q.push(dp[root]);
    rep(i, 2, n) {
        if(q.empty()) a[i] = a[i - 1];
        else a[i] = a[i - 1] + q.top(), q.pop();
    } a[1] = 1; print(a + 1, a + n + 1);
}