...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	sort(all(ind),[&](int i,int j){ return hs[i]<hs[j];});
	vv<int> g(n);
	rep(_,2){
		out(ind,1);
		map<int,int> ints{{-10,-9},{MOD,MOD+1}};
		for(auto i:ind){
			int l=i,r=i+1;
			auto it=ints.lower_bound(i);
			if(it->X<=r){
				MX(r,it->Y);
				it=ints.erase(it);
			}
			...
			ints[l]=r;
			if(l>0 && hs[l-1]!=hs[i] && r<n && hs[r]!=hs[i]){
				out(i,hs[i],l,r,ints,1);
				g[l-1].pb(r);
			}
		}
		reverse(all(ind));
	}
	vector<int> re(n,MOD); re[0]=0;
	rep(i,n-1){
		MN(re[i+1],re[i]+1);
		for(auto j:g[i]) MN(re[j],re[i]+1);
	}
	out(g,re,1);
	cout<<re.back() NL;
  return 0;
