...
int main() {
  ...
  VI k(n); VVI a(n);
  rep(i, n) {
    k[i] = IN; a[i] = IN.vi(k[i]);
    rep(j, k[i]) a[i][j]--;
  }
  int presentN = 1e6;
  vector<modint> occurence(presentN);
  rep(i, n) rep(j, k[i]) occurence[a[i][j]]++;
  vector<modint> prob(presentN);
  rep(i, presentN) prob[i] = occurence[i] / modint(n);
  modint res = 0;
  rep(i, n) rep(j, k[i]) res += prob[a[i][j]] / modint(k[i]) / modint(n);
  OUT(res);
}