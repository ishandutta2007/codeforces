...
fun main(){
    solve.cases{
        ...
        while(l < n-1){
            var best = nlarge
            var how = Pair(-1,-1)
            for(i in maxOf(0,l-10) ..l+1){
                for(att in 0 until count){
                    val len = have[att].size
                    if(len + i - 1 >= n ) continue
                    if(str.slice(i until i + len) != have[att]) continue
                    if(i + len - 1 > best){
                        best = i + len -1
                        how  = Pair(att,i)
                    }
