...
void solve() {
    ...
    for (int i = 1; i <= n; i++) {
        int j = i;
        while ((j < n) && (p[j + 1].a == p[j].a)) j++;
        for (int k = i; k <= j; k++) (w *= (k - i + 1)) %= base;
        i = j;
    }
    (ans -= w) %= base;
    ...
    bool sorted = true;
    for (int i = 2; i <= n; i++)
        if (p[i].b < p[i - 1].b) sorted = false;
    if (sorted) {
        w = 1;
        for (int i = 1; i <= n; i++) {
            int j = i;
            while ((j < n) && (p[j + 1].a == p[j].a && p[j + 1].b == p[j].b)) j++;
            for (int k = i; k <= j; k++) (w *= (k - i + 1)) %= base;
            i = j;
        }
        (ans += w) %= base;
    }
    if (ans < 0) (ans += base) %= base;
    cout << ans << '\n';
}
