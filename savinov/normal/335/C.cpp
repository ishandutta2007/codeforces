...
int solve(int n, int tp)
{
    if (g[n][tp] != -1) return g[n][tp];
    if (n == 0) return g[n][tp] = 0;
    if (n == 1) return g[n][tp] = tp > 0;
    vector <int> z;
    if (tp < 2)
    {
        for(int i = 0; i < n; ++i)
        {
            if (i && (n - i - 1 || !tp)) z.pb(solve(i, 0) ^ solve(n - i - 1, tp ^ 1));
            if (n - i - 1 || tp) z.pb(solve(i, 1) ^ solve(n - i - 1, tp));
        }
    }
    if (tp == 2)
    {
        for(int i = 0; i < n; ++i)
        {
            if (i) z.pb(solve(i, 0) ^ solve(n - i - 1, 2));
            z.pb(solve(i, 1) ^ solve(n - i - 1, 2));
        }
    }
    if (tp == 3)
    {
        for(int i = 0; i < n; ++i)
            z.pb(solve(i, 2) ^ solve(n - i - 1, 2));
    }
    return g[n][tp] = mex(z);
}
...
int main()
{
    ...
    forn(i, n)
    {
        ...
        {
            if (prev)
                t.pb(mp(j - i, abs(pos[j] - pos[i - 1]) ^ 1));
            else
                t.pb(mp(j - i, 2));
            prev = pos[i];
        }
