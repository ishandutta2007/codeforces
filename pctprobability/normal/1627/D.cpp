...
namespace FastGCDConvolution{
  ...
  template<class T> vector<T> gcdmul(const vector<T> &a){
    int n=max(int(a.size()),int(a.size()));
  ...
  std::uniform_int_distribution<long long> N(900000000,1000000000);
    mod=N(engine);
    for(int i=1;i<n;i++){
      C[i]=modPow(A[i],10000000,mod);
    }
    mebius(C,isprime);
    return C;
  }
