...
struct segtreminrmq {
	...
	int n;
	int curlogn;
	segtreminrmq(int size, int logni) {
		n = size;
		curlogn = logni;
		arr2.resize(curlogn);
		fori(i, curlogn) {
	...
	inline void add(int start, lol num) {
		...
		arr2[0][start] = start;
		forn(i, curlogn) {
			if (arr[arr2[i][start >> i]] > num || (arr2[i][start >> i]==0 && (start >> i)>0)) {
				arr2[i][start >> i] = start;
			}
...
veci fib(2, 1);
int func(veci& f, veci& ord, int p) {
	if (p <= 1) {
		return 1;
	}
	int n = fib[p];
	...
	fornb(i, n) {
		size[i]++;
		if (size[i] == fib[p - 2] || size[i] == fib[p - 1]) {
			veci ord1(size[i]), ord2(n- size[i]), f1(size[i]), f2(n-size[i]);
			map<int, int> mapit1, mapit2;
			fori(j, n) {
				int cur = ord[j];
				if (j >= i && j - i < size[i]) {
					ord1[mapit1.size()] = mapit1.size();
					if(mapit1.find(f[cur])!= mapit1.end())
						f1[mapit1.size()] = mapit1[f[cur]];
					mapit1[cur] = mapit1.size();
				}
				else {
					ord2[mapit2.size()] = mapit2.size();
					if (mapit2.find(f[cur]) != mapit2.end())
						f2[mapit2.size()] = mapit2[f[cur]];
					mapit2[cur] = mapit2.size();
				}
			}
			if (size[i] == fib[p - 2]) {
				return func(f1, ord1, p - 2)* func(f2, ord2, p - 1);
			}
			return func(f1, ord1, p - 1)* func(f2, ord2, p - 2);
		}
		else {
			size[f[i]] += size[i];
		}
