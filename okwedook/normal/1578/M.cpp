...
vector<ll> get_basis(vector<ll> x) {
    int ptr = 0;
    for (int bit = 0; bit < 60; ++bit) {
        for (int i = ptr; i < sz(x); ++i)
            if (x[i] >> bit & 1) {
                swap(x[i], x[ptr]);
                for (int j = ptr + 1; j < sz(x); ++j)
                    if (x[j] >> bit & 1) {
                        x[j] ^= x[ptr];
                    }
                ++ptr;
...
signed main() {
    ...
    for (int i = 0; i < n; ++i)
        if (!used[i]) {
            vert = vector<int>();
            comp_edge = vector<pair<pii, ll>>();
            dfs(i, 0);
            vector<ll> cycle;
            for (auto e : comp_edge) {
                cycle.pb(rx[e.f.f] ^ rx[e.f.s] ^ e.s);
            }
            auto basis = get_basis(cycle);
            for (int bit = 0; bit < 60; ++bit) {
                int flag = 0;
                for (auto v : basis) {
                    flag |= v >> bit & 1;
                ...
                if (flag) {
                    ans += zet(sz(vert)) * zet(sz(vert) - 1) / 2 * mypow(zet(2), sz(basis) - 1 + bit);
                } else {
                    zet c0 = 0, c1 = 0;
                    for (auto v : vert)
                        if (rx[v] >> bit & 1) c1 += 1;
                        else c0 += 1;
                    ans += c0 * c1 * mypow(zet(2), sz(basis) + bit);
                }
            }
        }
    println(ans);
