...
constexpr int BRUTEFORCE_INTERVAL = 2000;
constexpr int MIN_FREQ = BRUTEFORCE_INTERVAL/5;
...
public:
    ...
    void solve(istream& cin, ostream& cout) {
		...
		for (auto &f: F) {
			if (f.y >= MIN_FREQ) {
				V.push_back({f.x, vector<int>(N+1, 0)});
				auto&v = V.back();
				for (int i = 0; i < N; ++i) {
					v.y[i+1] = v.y[i] + (A[i] == f.x);
				}
			}
		}
