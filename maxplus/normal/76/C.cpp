...
constexpr int K = 22, N = 1 << K;
uint32_t msk[K];
bool hv[K];
uint32_t w[N], a[K][K], t[K];
...
int main() {
  ...
  [&]() __attribute__((optimize("tree-vectorize"), noinline)) {
    for (int i = 0; i < k; ++i)
    for (int j0 = 0; j0 < 1 << k; j0 += 1 << i + 1)
    for (int j1 = 0; j1 < 1 << i; ++j1) {
      w[j0 + j1 + (1 << i)] += w[j0 + j1];
    }
  }();
  uint32_t tot = 0, ans = 0;
  for (int i = 0; i < k; ++i) {
    tot |= hv[i] << i;
  }
  for (int j = 0; j < 1 << k; ++j) {
