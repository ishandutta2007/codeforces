...
namespace MAIN {
	vector<int> G[N];
	int dfn[N],low[N],F[N][21],idx,dep[N];
	...
	inline int glca(int x,int y){
		...
		int d=dep[x]-dep[y];
		for(int i=20;~i;i--)if(d&(1<<i))x=F[x][i];
		if(x==y)return x;
		for(int i=20;~i;i--)if(F[x][i]!=F[y][i])x=F[x][i],y=F[y][i];
		return F[x][0];
	}
	int vis[N],st[N],top,H[N],deg[N];
	inline void solve(){
		int k=0;
		int m=read();
		while(m--){
		...
		for(int i=1,x;i<tot;i++)if(!vis[x=glca(H[i],H[i+1])])vis[H[++k]=x]=1;
		tot=k,sort(H+1,H+tot+1,[&](const int &x,const int &y){return dfn[x]<dfn[y];});
		st[top=1]=H[1];
		...
		int mx=0;
		for(int i=1;i<=tot;i++)mx=max(mx,deg[H[i]]);
		puts(mx<=2?"YES":"NO");
		for(int i=1;i<=tot;i++)vis[H[i]]=deg[H[i]]=0;
	}
