...
int main() {
    ...
    loop(t) {
        ...
        rep(k, l, r + 1) {
            ...
            for (int x : divs[p]) {
                int xv = x << (q + 1);
                if (xv < l or xv >= k) continue;
                {
                    int yl = max(l, k - xv + 1);
                    int yr = k;
                    if (yl <= yr) {
                        int ynum = lower_bound(all(divs[k]), yr) - lower_bound(all(divs[k]), yl);
                        ans -= ynum;
                    }
                }
                {
                    int yl = max({ x + 1, cld(l, 1 << (q + 1)), (p + 1) / 2 - x });
                    int yr = (p + 1) / 2;
                    if (yl <= yr) {
                        int ynum = lower_bound(all(divs[p]), yr) - lower_bound(all(divs[p]), yl);
                        ans -= ynum;
                    }
                }
            }
