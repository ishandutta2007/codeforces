...
int main() {
    ...
    string s;
    read(s);
    vec<int> prefix_a(n + 1), prefix_q(n + 1), suffix_c(n + 1), suffix_q(n + 1);
    prefix_a[0] = prefix_q[0] = 0;
    suffix_c[n] = suffix_q[n] = 0;
    rep(i, n) {
        prefix_a[i + 1] = prefix_a[i] + (s[i] == 'a');
        prefix_q[i + 1] = prefix_q[i] + (s[i] == '?');
    }
    rrep(i, n) {
        suffix_c[i] = suffix_c[i + 1] + (s[i] == 'c');
        suffix_q[i] = suffix_q[i + 1] + (s[i] == '?');
    }
    int k = 0;
    for (char c : s) k += c == '?';
    vec<mint> pow3(n + 1);
    ...
    auto p = [&pow3](int index) -> mint {
        return index >= 0 ? pow3[index] : 0;
    };
    mint ans = 0;
    rep(i, n) {
        if (s[i] == 'a' or s[i] == 'c') continue;
        int b = s[i] == '?';
        ans += p(k - b) * prefix_a[i] * suffix_c[i + 1];
        ans += p(k - 1 - b) * prefix_a[i] * suffix_q[i + 1];
        ans += p(k - 1 - b) * prefix_q[i] * suffix_c[i + 1];
        ans += p(k - 2 - b) * prefix_q[i] * suffix_q[i + 1];
    }
    print(ans.val());
    return 0;
}