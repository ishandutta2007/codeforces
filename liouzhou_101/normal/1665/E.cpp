...
node* add(node* p, int val, int k)
{
	...
	{
		int x = val >> k & 1;
		it->son[x] = add(p ? p->son[x] : nullptr, val, k - 1);
	}
	return it;
}

int go(node* a, node* b, int k)
{
	if (k < 0) return 0;
	if ((b && b->son[0] ? b->son[0]->val : 0) - (a && a->son[0] ? a->son[0]->val : 0) >= 1)
		return go(a ? a->son[0] : 0, b ? b->son[0] : 0, k - 1);
	else
		return go(a ? a->son[1] : 0, b ? b->son[1] : 0, k - 1) | (1 << k);
}
...
void solve()
{
	...
	while (q--)
	{
		...
		for (int k = K - 1; k >= 0; --k)
		{
			...
			for (auto x : can)
				if (!(x >> k & 1)) tmp += 1;
			if ((b && b->son[0] ? b->son[0]->val : 0) - (a && a->son[0] ? a->son[0]->val : 0) + tmp >= 2)
			{
				b = b ? b->son[0] : 0;
				a = a ? a->son[0] : 0;
			...
			else
			{
				...
				if ((b && b->son[0] ? b->son[0]->val : 0) - (a && a->son[0] ? a->son[0]->val : 0) == 1)
				{
					int x = go(a ? a->son[0] : 0, b ? b->son[0] : 0, k - 1);
					can.push_back(res | x);
				}
				b = b ? b->son[1] : 0;
				a = a ? a->son[1] : 0;
