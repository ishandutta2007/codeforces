...
void pre(){
    For(i,31) For(j,31) For(k,51)dp[i][j][k]=INF;
}
int solve(int i,int j,int k){
    if (k>50 || k>i*j) return INF;
    if (k==0) return 0;
    if (k==i*j) dp[i][j][k]=0;
    ...
    for (int t=1;t<i;t++){
        for(int u=0;u<=k;u++){
            dp[i][j][k]=min(dp[i][j][k],solve(t,j,u)+solve(i-t,j,k-u)+j*j);
        }
    }
    for(int t=1;t<j;t++){
        for(int u=0;u<=k;u++){
            dp[i][j][k]=min(dp[i][j][k],solve(i,t,u)+i*i+solve(i,j-t,k-u));
        }
    }
    return dp[i][j][k];
}
