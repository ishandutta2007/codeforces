...
int n = get<int>();
int aSize = get<int>();
vi a = getVector<int>(aSize);
int bSize = get<int>();
vi b = getVector<int>(bSize);
vi moves[2] = {a, b};
...
const char* stateMeanings[3] = {
	"Win",
	"Lose",
	"Loop"
};

State states[2][10000];
int deg[2][10000];
bool used[2][10000];

void dfs(int p, int v) {
	used[p][v] = true;
	for (int dist: moves[!p]) {
		int wasP = !p, wasV = (v + n - dist) % n;
		if (!used[wasP][wasV]) {
			if (states[p][v] == STATE_LOSE) {
				states[wasP][wasV] = STATE_WIN;
				dfs(wasP, wasV);
			} else if (states[p][v] == STATE_WIN) {
				deg[wasP][wasV]--;
				if (deg[wasP][wasV] == 0) {
					states[wasP][wasV] = STATE_LOSE;
					dfs(wasP, wasV);
				}
			}
		}
...
signed main() {
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 10000; j++) {
			deg[i][j] = moves[i].size();
			states[i][j] = STATE_DRAW;
			used[i][j] = false;
		}
	}
	states[0][0] = states[1][0] = STATE_LOSE;
	used[0][0] = used[1][0] = true;
	dfs(0, 0); dfs(1, 0);
	for (int i = 1; i < n; i++) {
		cout << stateMeanings[states[0][i]] << " ";
	}
	cout << endl;
	for (int i = 1; i < n; i++) {
		cout << stateMeanings[states[1][i]] << " ";
	}
	cout << endl;
	return 0;
}