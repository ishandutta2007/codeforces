...
void solve() {
    ...
    rep(i, n) {
        int l = lower_bound(all(ls), x[i] - p[i]) - ls.begin();
        int t = lower_bound(all(ls), x[i]) - ls.begin();
        int r = lower_bound(all(ls), x[i] + p[i]) - ls.begin();
        add(l, t + 1, 1, p[i] - x[i]);
        add(t + 1, r + 1, -1, p[i] + x[i]);
    }
    ll mn_1 = -linf;
    ll mn_2 = -linf;
    rep(i, sz) {
        ll r = bt1.sum(i) * ls[i] + bt2.sum(i);
        if (r <= m) continue;
        chmax(mn_1, r - m - ls[i]);
        chmax(mn_2, r - m + ls[i]);
    }
    string ans;
    rep(i, n) {
        if (p[i] - x[i] >= mn_1 and p[i] + x[i] >= mn_2) ans.pb('1');
        else ans.pb('0');
    }
    print(ans);
}
