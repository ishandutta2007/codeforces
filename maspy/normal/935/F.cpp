...
void solve() {
  ...
  */
  ...
  auto seg_f = [&](T x, T y) -> T { return T({max(x.a, y.a), max(x.b, y.b), max(x.c, y.c)}); };
  SegTree<T> seg(seg_f, T({-INF, -INF, -INF}));
  ...
  auto set = [&](ll i) -> void {
    ...
    T t;
    if (a >= 0 && b >= 0) {
      t.a = -INF, t.b = 0, t.c = -2 * b;
    }
    if (a <= 0 && b <= 0) {
      t.a = -INF, t.b = 0, t.c = 2 * a;
    }
    if (a >= 0 && b <= 0) {
      t.a = -INF, t.b = -INF, t.c = 0;
    }
    if (a <= 0 && b >= 0) {
      ll p = min(-a, b), q = max(-a, b);
      t.a = 0, t.b = -2 * p, t.c = -2 * (p + q);
    }
    seg.set_val(i, t);
  };
  ...
  FOR(_, Q) {
    ...
    if (t == 1) {
      auto t = seg.fold(L, R);
      ll upd = max({-2 * x + t.a, t.b, 2 * x + t.c});
      ll ANS = now + upd;
      print(ANS);
    } else {
      // 
