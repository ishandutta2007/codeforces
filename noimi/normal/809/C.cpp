...
signed main(){
    ...
    while(Q--){
        LL(x1,y1,x2,y2,k);
        auto calc = [](ll a,ll b)->mint{if(b<a) return (mint)0;return (mint)((b+a)*(b-a+1)/2);};
        auto solve = [&](auto &&f,ll x,ll y,int car)->mint{
            if(!x or !y) return (mint)0;
            if(x < y) return f(f,y,x,car);
            int n = 0;
            while((1<<(n+1)) <= x) n++;
            int N = 1<<n;
            if((1<<n) <= y){
                mint res;
                res += (mint)(1<<n)*calc(1+car,min<ll>(N+car,k));
                res += (mint)(x+y-(1<<(n+1)))*calc(car+(1<<n)+1,min<ll>(N*2+car,k));
                res += f(f,x-(1<<n),y-(1<<n),car);
                return res;
            }
            else{
                mint res;
                res += (mint)(y)*calc(1+car,min<ll>(N+car,k));
                res += f(f,x-(1<<n),y,N+car);
                return res;
            }
        };
        // cout<<solve(solve,x2,y2,0) << endl;
        // cout<<solve(solve,x1-1,y2,0)<<endl;
        // cout<<solve(solve,x2,y1-1,0)<<endl;
        // cout<<solve(solve,x1-1,y1-1,0)<<endl;
        cout << solve(solve,x2,y2,0) - solve(solve,x1-1,y2,0) - solve(solve,x2,y1-1,0) + solve(solve,x1-1,y1-1,0) << "\n";
    }
