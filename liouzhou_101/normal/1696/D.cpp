...
void solve()
{
	...
	auto max = static_cast<const int& (*)(const int&, const int&)>(&std::max<int>);
	SparseTable<int, decltype(max)> mx(a.begin(), a.end(), max);
	auto min = static_cast<const int& (*)(const int&, const int&)>(&std::min<int>);
	SparseTable<int, decltype(min)> mn(a.begin(), a.end(), min);

	vector<int> mxl(n), mxr(n), mnl(n), mnr(n);
	for (int i = 0; i < n; ++i)
	{
		{
			int L = 0, R = i;
	...
	vector<ValueType> init(n, value_init);

	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		mxtree(init.begin(), init.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);
	SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
		mntree(init.begin(), init.end(), value_init, value_merge, tag_init, tag_merge, tag_spread);

	mxtree.ModifyValue(0, { 0 });
	mntree.ModifyValue(0, { 0 });

	vector<vector<int>> mnv(n), mxv(n);
	for (int i = 0; i < n; ++i)
	{
		mnv[mnr[i]].push_back(i);
		mxv[mxr[i]].push_back(i);
	}
	vector<int> dp(n);
	for (int i = 0; i < n; ++i)
	{
		if (i)
		{
			int d1 = mntree.Query(mnl[i], i).val + 1;
			int d2 = mxtree.Query(mxl[i], i).val + 1;
			int d = min(d1, d2);
			dp[i] = d;
			mxtree.ModifyValue(i, { d });
			mntree.ModifyValue(i, { d });
		}
		for (auto x : mnv[i])
			mxtree.ModifyValue(x, value_init);
		for (auto x : mxv[i])
			mntree.ModifyValue(x, value_init);
	}
