...
public class I {
    ...
    static ContestPrinter cp = new ContestPrinter();

    public static void main(String[] args) throws Exception {
        int T = sc.nextInt();
        for (int t = 0; t < T; t++) {
            int N = sc.nextInt();
            strong = new int[N * 2][N * 2];
            ...
            for (int n = 0; n < N - 1; n++) {
                for (int m = n + 1; m < N - 1; m++) {
                    if (stronger(winners[m], winners[n])) {
                        int temp = winners[m];
                        winners[m] = winners[n];
                        winners[n] = temp;
                    }
            ...
            int q = 0;
            for (int n = 0; n < N; n++) {
                if (stronger(winners[p], anothers[q])) {
                    p++;
                } else {
                    q++;
                }
    ...
    static boolean stronger(int p, int q) {
        if (strong[p][q] != 0) {
            return strong[p][q] == 1;
        }
        cp.println("? " + (p + 1) + " " + (q + 1));
        cp.flush();
        strong[p][q] = sc.next().charAt(0) == '<' ? 1 : -1;
        strong[q][p] = -strong[p][q];
        return strong[p][q] == 1;
    }
