...
public:
    void solve() {
        ...
        ll answer = cur;
        while (startM < n) {
            int d1 = d[startM] - startD + 1;
            int d2 = d[endM] - endD + 1;
            int advance = min(d1, d2);
            cur += ll(advance) * (endD - startD);
            maxim(answer, cur);
            startD += advance;
            if (startD == d[startM] + 1) {
                startD = 1;
                startM++;
            }
            endD += advance;
