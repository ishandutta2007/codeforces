...
int cutt;
int a[N];
inline void same(int l,int r,int ed)
{
    for(int i=l,j=r;i<=r;i++)
    {
        for(j=max(j,i+1);j>i+1&&((a[i]+a[j-1])&ed);j--);
        if((a[i]+a[j])&ed)cutt^=((r-j+1)&1);
    }
    return;
}
inline void differ(int mid,int ed)
{
    if(mid==n)return;
    for(int i=1,j=n;i<=mid;i++)
    {
        for(;j-1>mid&&!((a[j]+a[i])&ed);j--);
        if((a[j]+a[i])&ed)cutt^=((j-mid)&1);
    }
    return;
}
inline void work(int ed)
{
    cutt=0;
    int md=((1<<ed+1)-1);for(int i=1;i<=n;i++)a[i]=(a[i]&md);sort(a+1,a+n+1);
    int mid=n;for(int i=1;i<=n;i++)if(a[i]&(1<<ed)){mid=i-1;break;}
    same(1,mid,1<<ed);same(mid+1,n,1<<ed);differ(mid,1<<ed);if(cutt)ans|=(1<<ed);return;
}
int main()
{
    rin>>n;for(int i=1;i<=n;i++)rin>>a[i];for(int i=26;i>=0;i--)work(i);printf("%d\n",ans);
    return 0;
}