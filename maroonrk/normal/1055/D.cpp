...
string trans(const string&s,const string&a,const string&b){
	int n=s.size();
	int j=0;
	rep(i,n){
		while(j>=0&&a[j]!=s[i])
...
signed main(){
	int n=read();
	V<string> s,t;
	rep(i,n)s.pb(readString());
	rep(i,n)t.pb(readString());
	vi l(n,-1),r(n,-1);
	rep(i,n){
		rep(j,s[i].size())
			if(s[i][j]!=t[i][j]){
	...
	string a,b;
	rep(i,n)if(!sm[i]){
		a=s[i].substr(l[i],r[i]-l[i]);
		b=t[i].substr(l[i],r[i]-l[i]);
	}
	dmp(a,b);
	
	init(a);
	rep(i,n)if(trans(s[i],a,b)!=t[i])
		No();
	Yes(0);
	cout<<a<<endl;
	cout<<b<<endl;
