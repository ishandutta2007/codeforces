...
int n, m;
char c;
constexpr int dir[] = {-1, 1};
void solve() {
    cin >> n >> m;
    int max_weight[2][2];
    for (int i = 0; i <= 1; i++)
        for (int j = 0; j <= 1; j++) max_weight[i][j] = -1e9;
    for (int i = 1; i <= n; i++)
        for (int j = 1; j <= m; j++) {
            cin >> c;
            if (c == 'B') {
                for (int x = 0; x <= 1; x++)
                    for (int y = 0; y <= 1; y++) {
                        max_weight[x][y] = max(max_weight[x][y], -i * dir[x] - j * dir[y]);
                    }
            }
        }
    int best = 1e9, bi = 0, bj = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            int cost = 0;
            for (int x = 0; x <= 1; x++)
                for (int y = 0; y <= 1; y++) {
                    cost = max(cost, max_weight[x][y] + i * dir[x] + j * dir[y]);
                }
            if (cost < best) {
                best = cost;
                bi = i;
