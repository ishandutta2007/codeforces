...
int calc(vi e) {
    ...
    int m = e.size();
    int mx = 0;
    F0R(i, m) F0R(j, i) ckmax(mx, a[e[i]][e[j]]);
    int ans = c.size();
    c.pb(mx); p.pb(-1);
    vb vis(m, false);
    F0R(i, m) if (!vis[i]) {
        vi nxt; nxt.pb(e[i]); vis[i]=true;
        FOR(j, i+1, m) if (!vis[j] && a[e[i]][e[j]]<mx) {
            nxt.pb(e[j]); vis[j]=true;
        }
        p[calc(nxt)] = ans;
    }
    return ans;
...
void solve() {
    ...
    cout << nl;
    cout << r+1 << nl;
    F0R(i, p.size()) if (p[i] != -1) cout << i+1 << ' ' << p[i]+1 << nl;
}
