...
#define App(x) ((x)<=0?0:app[x])
int mem[MAXN][4][4];
#define NOW mem[pos][last][llast]
#define KNOW ~NOW
// a b c d
int Dfs( int pos , int last , int llast ){
	if( pos == m+1 ) return 0;
	if(KNOW) return NOW;
	int a = App(pos-2) - llast;
	int b = App(pos-1) - last - llast;
	int c = App(pos) - last - llast;
	int d = App(pos+1) - last;
	int e = App(pos+2);
	int can = min( min( min(c,d) , e ) , 2 );
	int ans = 0;
	Forx(i,0,can){
		int tans = Dfs(pos+1,i,last) + i + (c-i)/3;
		Mymax(ans,tans);
	}
	return NOW = ans;
}
