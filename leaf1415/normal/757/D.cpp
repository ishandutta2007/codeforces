...
{
	ll ret = 0;
	for(auto c : s) ret *= 2, ret += c%2, chmin(ret, 100LL);
	return ret;
}
	...
	rep(i, 0, n-1){
		rep(j, 0, N-1){
			rep(k, 1, n){
				...
				if(x <= 0) continue;
				if(x <= 20){
					dp[i+k][j|(1<<(x-1))] += dp[i][j];
					if(dp[i+k][j|(1<<(x-1))] >= mod) dp[i+k][j|(1<<(x-1))] -= mod;
				}
			}
		}
	}
	
	//rep(i, 0, n) outa(dp[i], 0, 15);
	
	ll ans = 0;
	rep(i, 1, n) rep(j, 1, 20) ans += dp[i][(1<<j)-1], ans %= mod;
	outl(ans);
