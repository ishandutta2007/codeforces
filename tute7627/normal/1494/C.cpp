...
int main(){
  ...
  while(t--){
    ...
    sort(ALL(ay));sort(ALL(by));
    auto calc=[](vector<ll>a,vector<ll>b){
      ll n=a.size(),m=b.size();
      ll ret=0;
      ll cnt=0;
      rrep(i,0,m){
        ll na=upper_bound(ALL(a),b[i])-a.begin();
        chmax(ret,cnt+(upper_bound(ALL(b),b[i])-upper_bound(ALL(b),b[i]-na)));
        if(binary_search(ALL(a),b[i]))cnt++;
      }
      return ret;
    };
    cout<<calc(ax,bx)+calc(ay,by)<<endl;;
  }
  return 0;
}