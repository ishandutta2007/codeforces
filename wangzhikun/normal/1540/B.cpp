...
void dfs(int num,int fa = -1){
	...
	each(ct,g[num]){
		...
		each(cct,cc[ct]){
			each(ch,cc[num]){
				int a = max(ch,cct),b = min(ch,cct);
				//dbg(a,b,vis[a]-vis[num],vis[b]-vis[num]);
				ans+=dp[vis[a]-vis[num]][vis[b]-vis[num]];
			}
		}
		each(cct,cc[ct]){
			cc[num].pb(cct);	
