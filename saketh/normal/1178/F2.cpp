...
mn solve(int i, int j) {
    if (i >= j) return 1;
    mn& ans = cache[i][j];
    if (did[i][j]) return ans;
    did[i][j] = true;

    int k = min_element(vals.begin() + i, vals.begin() + j) - vals.begin();
    int left = first[vals[k]], right = last[vals[k]];
    if (left < i || right >= j) return ans = 0;

    mn lways = solve(i, left);
    FOR (sp, i, left) lways += solve(i, sp) * solve(sp, left);

    mn rways = solve(right+1, j);
    FOR (sp, right+1, j) rways += solve(right+1, sp) * solve(sp, j);

    mn ways = 1;
    int last = -1;
    FOR (m, left, right + 1) {
        if (vals[m] == vals[k]) {
            if (last != -1) ways *= solve(last + 1, m);
            last = m;
        }
    }

    return ans = lways * ways * rways;
}

int main() {
    setIO();

    int N, M; re(N, M); vi strip(M); re(strip);
    trav (e, strip) --e;

    vvi locs(N);
    first.resz(N, INT_MAX), last.resz(N, -1);

    F0R (i, M) {
        locs[strip[i]].pb(i);
        ckmin(first[strip[i]], i);
        ckmax(last[strip[i]], i);
    }

    // first check if it's impossible
    set<int> alive;
    F0R (i, N) {
        auto it = alive.lower_bound(first[i]);
        if (it != alive.end() && *it < last[i]) {
            ps(0);
            return 0;
        }
        trav (e, locs[i]) alive.insert(e);
    }

    trav (e, strip) {
        if (vals.empty() || vals.back() != e)
            vals.pb(e);
    }

    fill(all(first), INT_MAX);
    fill(all(last), -1);
    F0R (i, sz(vals)) {
        ckmin(first[vals[i]], i);
        ckmax(last[vals[i]], i);
    }
    ps(solve(0, sz(vals)));
