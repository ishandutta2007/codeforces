...
int n;
vi a;
gp_hash_table<int, int, chash> locs[20][2];

void solve() {
    cin >> n;
    read(a, n);
    F0R(i, 20) locs[i][1][0] = -1;
    vi last(20, -1);
    int xorsum = 0, ans = 0;
    F0R(i, n) {
        xorsum ^= a[i];
        F0R(bit, 20) {
            gp_hash_table<int, int, chash>& tmp = locs[bit][i%2];
            if ((a[i]&1<<bit)==0) {
                last[bit] = i;
                locs[bit][0].clear();
                locs[bit][1].clear();
            } else {
                if (tmp.find(xorsum>>bit) != tmp.end())
                    ckmax(ans, i - tmp[xorsum>>bit]);
            }
            if (tmp.find(xorsum>>bit) == tmp.end()) tmp[xorsum>>bit] = i;
            else ckmin(tmp[xorsum>>bit], i);
        }
    }
    cout << ans << nl;
}
