...
string s, t;
vector<int> pos;
pair<int, int64_t> f[501];
constexpr int base = 1e9 + 7;
void solve() {
    cin >> s >> t;
    pos.clear();
    int start_at = 0;
    while (start_at + t.size() <= s.size()) {
        int res = s.find(t, start_at);
        if (res == -1) break;
        pos.emplace_back(res);
        start_at = res + 1;
    }
    f[s.size()] = {0, 1};
    for (int i = s.size() - 1, start = pos.size(); i >= 0; i--) {
        while ((start > 0) && (pos[start - 1] >= i)) start--;
        if (start == pos.size()) {
            f[i] = {0, 1};
            continue;
        }
        // need to take care of start
        int max_allowed_start = pos[start] + t.size() - 1;  // must destroy the first appearance
        f[i] = {1e9, 0};
        for (int j = start; j < pos.size(); j++) {
            if (pos[j] > max_allowed_start) break;
            int x = pos[j] + t.size();
            if (f[x].first + 1 < f[i].first) {
                f[i] = {f[x].first + 1, f[x].second};
            } else if (f[x].first + 1 == f[i].first) {
                (f[i].second += f[x].second) %= base;
            }
        }
    }
