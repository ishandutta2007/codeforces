...
fn main() {
	let mut rin = reader();
	let mut rout = writer();
	let adj = |(i,j): (usize,usize)| -> Vec<(usize,usize,u8,u8)> {
		vec![(i-1,j,b'v',b'^'), (i+1,j,b'^',b'v'), (i,j-1,b'>',b'<'), (i,j+1,b'<',b'>')]
	};

	l!(r,c = rin.u());
	let mut b = rin.framed_board(r,c,b'*');
	for i in 1..=r {
		for j in 1..=c {
			if b[i][j] == b'.' {
				let mut s = v!([]);
				...
				while let Some((i,j)) = s.pop() {
					...
					if let Some((ii,jj,x,y)) = d {
						b[i][j] = x;
						b[ii][jj] = y;
						for q in vec![adj((i,j)), adj((ii,jj))] {
							for (i,j,_,_) in q {
								s.push((i,j));
							}
						}
