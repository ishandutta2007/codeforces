...
int main(){
	...
	vector<num> f2(3 * n, 1);
	vector<num> fi2(3 * n, pow(num(2), m));
	num i2 = num(1) / num(2);
	for(int i = 0; i < n; i++){
		f2[n + i + 1] = f2[n + i] * (good[i] ? 2 : 1);
		fi2[n + i + 1] = fi2[n + i] * (good[i] ? i2 : 1);
	}
	for(int x = 2*n; x < 3*n; x++){
		f2[x] = fi2[x] = 1;
	}
	vector<num> fprod(6 * n, 0);
	y_combinator(
		[&](auto self, int x, int y) -> void {
			if(x + 1 == y) return;
			int r = (x + y) / 2;
			self(x, r);
			self(r, y);
			vector<num> v = ecnerwala::fft::fft_multiply<num>(vector<num>(f2.begin() + x, f2.begin() + r), vector<num>(vector<num>(fi2.begin() + r, fi2.begin() + y)));
