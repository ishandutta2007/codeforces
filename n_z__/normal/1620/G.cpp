...
struct modint{//
	...
	modint operator ++(){auto y=x;x+=1;return y;}
	modint operator ++(int32_t){x+=1;return x;}
	modint operator --(){auto y=x;x-=1;return y;}
	modint operator --(int32_t){x-=1;return x;}
...
v[0]=1; 
for(int x=1;x<all;x++)
v[x]=1,sum[x]=sum[x^(x&-x)]+__builtin_ffsll(x);
for(int x=0;x<26;x++)
{
	for(int y=0;y<all;y++)
	g[y]=0x3f3f3f3f; 
	for(int y=1;y<all;y++)
	g[y]=min(g[y^(y&-y)],cnt[__builtin_ffsll(y)][x]),v[y]=v[y]*(g[y]+1);
}
for(int x=1;x<all;x++)
if(!(__builtin_popcountll(x)&1))v[x]=-v[x];
v[0]=0;
for(int x=0;x<n;x++)
for(int y=0;y<all;y++)
if(!(y&(1<<x)))v[y^(1<<x)]+=v[y];
int ans=0;
for(int x=0;x<all;x++)
ans^=(int)(v[x])*__builtin_popcountll(x)*sum[x];
cout<<ans<<endl;
#endif
}
}
