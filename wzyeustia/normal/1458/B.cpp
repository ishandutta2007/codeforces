...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ll n; cin >> n;
        const ll MAXCAP = 1e4;
        vector<vector<ll>> dp(n+1, vector<ll>(MAXCAP+1, LLONG_MIN));

        dp[0][0] = 0;
        ld tot = 0;
        FOR(i, 1, n) {
            dp[i][0] = 0;
            ll capacity, current; cin >> capacity >> current;
            tot += current;

            for(int j = i; j>=1; j--) {
                for(int k = MAXCAP; k>=1; k--) {
                    if (k >= capacity) {
                        chkmax(dp[j][k], dp[j-1][k-capacity] + current);
                    }
                }
            } 
        }
        ...
        FOR(i, 1, n) {
            ld ans = 0;
            FOR(j, 1, MAXCAP) {
                chkmax(ans, min(tot/2.0+(ld)dp[i][j]/2.0, (ld)j));
            }
