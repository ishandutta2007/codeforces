...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	Lca lca(g);
	vector<pii> l(n);
	rep(i,n) l[i]=pii(lca.l[i],i);
	RMQ<pii> rmq(l,pii(MOD,MOD));
	RMxQ<pii> rMq(l,pii(0,0));
	out(lca.et,lca.l,1);
	auto kick=
		[&](int l,int r,int v){
			int L=MOD,R=0;
			if(l!=v){
				MN(L,rmq.get(l,v-1).X);
				MX(R,rMq.get(l,v-1).X);
			}
			if(r!=v){
				MN(L,rmq.get(v+1,r).X);
				MX(R,rMq.get(v+1,r).X);
			}
			out(l,r,v,L,R,1);
			return lca.rmq.get(L,R).X;
		};
	while(q--){
		int l,r;
		cin>>l>>r; --l; --r;
