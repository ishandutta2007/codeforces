...
class DMaksimalnoRaspredelennoeDerevo {
public:
    void solve() {
        int n = in.readInt();
        arri u, v;
        ...
        int m = in.readInt();
        auto p = in.readIntArray(m);
        decreaseByOne(u, v);

        sort(all(p));
        arr<modint> c(n - 1, 1);
        if (m <= n - 1) {
            copy(all(p), c.begin() + (n - 1 - m));
        } else {
            if (n > 2) {
                copy(p.begin(), p.begin() + (n - 2), c.begin());
            }
        ...
        arr<ll> d(n - 1);
        RecursiveFunction dfs = [&](const auto& self, int vert, int last) -> ll {
            ll size = 1;
...
int main() {
    ...
    auto time = clock();
#endif
    DMaksimalnoRaspredelennoeDerevo solver;
