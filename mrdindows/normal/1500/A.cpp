...
//            DBN(i, j, sum, usedSum[sum]);
            if (usedSum[sum] != pii(0, 0) && usedSum[sum].second != j && usedSum[sum].first != j) {
                cout << "YES" << endl;
                cout << a[i].second << ' ' << a[j].second << ' ' << a[usedSum[sum].first].second << ' ' << a[usedSum[sum].second].second << endl;
                return 0;
            }
        }
        for (int j = 0; j < i; ++j) {
            int sum = a[i].first + a[j].first;
            if (usedSum[sum].first != i) {
                usedSum[sum] = {i, j};
            }
        }
    }
