#line 1 "codeforces-workspace/contest/1494/d/d.cpp"
...
#line 11 "codeforces-workspace/contest/1494/d/d.cpp"
...
template <typename Tp> struct union_find {
 ...
 public:
  ...
  bool empty() const { return link.empty(); }
...
#line 27 "codeforces-workspace/contest/1494/d/d.cpp"
...
void main() {
  ...
  for (auto &&[a, uf] : ufs) {
    unordered_map<int, vector<int>> com;
    for (auto i : range(uf.size())) {
      com[uf.find(i)].emplace_back(i);
    }
    ...
    for (auto &&[ign, vs] : com) {
      ...
      sal.emplace_back(a);
      for (auto &&x : vs) {
        edges.emplace_back(head[x] + 1, k + 1);
        head[x] = k;
      }
      ++k;
    }
  ...
  cout << k << "\n";
  cout << sal << "\n";
  cout << max_element(begin(sal), end(sal)) - sal.begin() + 1 << "\n";
  for (auto &&e : edges) {
    cout << e << "\n";
  }
}
