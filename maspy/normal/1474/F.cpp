...
#line 2 "library/linalg/mat_pow.hpp"
template<typename T>
...
void solve() {
  ...
  FOR_(N) {
    ...
    ll y = x + d;
    if (x < y) dat.eb(x + 1, y);
    elif (x > y) dat.eb(x - 1, y);
    x = y;
  }

  // for (auto&& [a, b]: dat) print(a, b);
  ...
  for (auto&& [a, b]: dat) {
    ...
    elif (LIS == x) { cand.eb(mi); }
    chmin(mi, min(a, b));
  }
  if (LIS == 1) {
    ll ANS = 0;
    for (auto&& [a, b]: dat) { ANS += max(a, b) - min(a, b) + 1; }
    return print(1, mint(ANS));
  }
  vi X;
  for (auto&& [a, b]: dat) {
    X.eb(a - 1);
  ...
  for (auto&& s: cand) {
    ...
    FOR(i, n) {
      auto [a, b] = dat[i];
      if (min(a, b) <= s && s <= max(a, b)) dp[i] = 1;
    }

    while (now < end) {
      ll nxt = end;
      for (auto&& x: X)
        if (now < x) chmin(nxt, x);
      if (nxt == now + 1) {
        vc<mint> newdp(n);
        mint left = 0;
        FOR(i, n) {
        ...
        FOR(i, n) {
          auto [a, b] = dat[i];
          if (a <= now && now <= b) ok[i] = 1;
          if (b <= now && now <= a) ok[i] = 1;
        }
        FOR(j, n) FOR(i, j) if (ok[i] && ok[j]) mat[i][j] = 1;
        FOR(i, n) if (ok[i]) {
          auto [a, b] = dat[i];
          if (a <= b)
            mat[i][i] = 1;
        ...
        // print("mat", now, nxt);
        // FOR(i, n) print(mat[i]);
        mat = mat_pow(mat, nxt - now);
        vc<mint> newdp(n);
        FOR(i, n) FOR(j, n) newdp[j] += mat[i][j] * dp[i];
        swap(dp, newdp);
        now = nxt;
      }
    }
