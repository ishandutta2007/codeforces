...
void Dfs( int pos , int fa ){
	...
	auto GetMx2Sum = [&]( vc<int> &x ){
		sort(All(x));
		return x.empty() ? 0 : x.size() == 1 ? x.back() : x.back()+x[x.size()-2];
	};

	int avaSon = e[pos].size();
	static vc<int> tmp;
	tmp.clear();
	for( auto v : e[pos] ){
		if( v == fa ) continue;
		int vContri = max(dp[v][0],dp[v][1]);
		tmp.Eb(vContri);
	}
	Mymax(ans,GetMx2Sum(tmp)+avaSon-2);
	...
	for( auto v : e[pos] ){
		if( v == fa ) continue;
		auto vdp = dp[v];
		Mymax( udp[0] , avaSon-1 + max(vdp[0],vdp[1]) );
		Mymax( udp[1] , 1 + vdp[0] );
	}
}
