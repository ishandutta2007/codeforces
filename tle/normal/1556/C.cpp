...
void main()
{
	...
	for(int i=2;i<=n;i+=2) {
		...
		for(int j=i;j<=n;++j) {
			//l in [-mi,c[i-1]]
			//r=l+cs in [1,c[j]]
			ll L=max(-mi,1-cs),R=min(c[i-1],c[j]-cs);
			if(L<=R&&(j-i+1)%2==1) ans+=R-L+1;
//			cerr<<i-1<<"~"<<j<<":"<<L<<"~"<<R<<" "<<mi<<" "<<cs<<"\n";
			if((j-i+1)&1) cs-=c[j];
			else cs+=c[j];
			mi=min(mi,cs);
//			if(cs<0) break;
