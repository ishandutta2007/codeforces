...
// int codeforces;cin>>codeforces;while(codeforces--){
  ...
  vec v(N);
  rep(i,N)cin>>v[i];
  auto calc=[&](ll l,ll r){return l*(N-r)+(r-l)*(l+N-r)+(r-l)*(r-l+1)/2;};
  rep(i,N){
    j=i;
    while(j<N&&v[i]==v[j])++j;
    ll ret=calc(i,j);
  ...
  while(K--){
    ...
    if(pos&&v[pos-1]==v[pos]){
      auto itr=prev(mp.lower_bound(datas(pos,pos)));
      ans-=itr->second;
      auto [L,R]=itr->first;
      debug("erase",itr->first);
      mp.erase(itr);
      ll ret=calc(L,pos);
      debug("insert",L,pos);
      ans+=mp[datas(L,pos)]=ret;
      ret=calc(pos,R);
      debug("insert",pos,R);
      ans+=mp[datas(pos,R)]=ret;
    }
    if(pos+1<N&&v[pos+1]==v[pos]){
      auto itr=mp.lower_bound(datas(pos,pos));
      ans-=itr->second;
      auto [L,R]=itr->first;
      assert(L==pos);
      debug("erase",itr->first);
      mp.erase(itr);
      ll ret=calc(pos,pos+1);
      debug("insert",pos,pos+1);
      ans+=mp[datas(pos,pos+1)]=ret;
      ret=calc(pos+1,R);
      debug("insert",pos+1,R);
      ans+=mp[datas(pos+1,R)]=ret;
    }
    v[pos]=val;
    if(pos+1<N&&v[pos+1]==v[pos]){
      auto itr=mp.lower_bound(datas(pos,pos));
      ans-=itr->second;
      assert(itr->first==datas(pos,pos+1));
      debug("erase",itr->first);
      mp.erase(itr++);
      ans-=itr->second;
      auto [L,R]=itr->first;
    ...
    if(pos&&v[pos-1]==v[pos]){
      auto itr=mp.lower_bound(datas(pos,pos));
      assert(itr->first.first==pos);
      auto R=itr->first.second;
      ans-=itr->second;
      debug("erase",itr->first);
      mp.erase(itr--);
      assert(itr->first.second==pos);
      auto L=itr->first.first;
      ans-=itr->second;
      debug("erase",itr->first);
      mp.erase(itr);
      debug("insert",L,R);
      ans+=mp[datas(L,R)]=calc(L,R);
    }
    print(ans);
  }
}