...
void bfs(vector<llint> G[], int S, llint dist[])
{
	...
	for(int i = 0; i <= h*w*4; i++) dist[i] = inf;
	dist[S] = 0;
	...
	while(Q.size()){
		...
		rep(d, 0, 3){
			ll nx = x + dx[d], ny = y + dy[d], nv = get(s, nx, ny);
			if(nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
			if(dist[nv] < inf/2) continue;
			if((a[x][y] & (1<<((d+s)%4))) && ((a[nx][ny] & (1<<((d+s+2)%4))))){
				Q.push(nv);
				dist[nv] = dist[v] + 1;
			}
		}
...
int main(void)
{
	...
	bfs(G, get(0, sx, sy), dist);
	
	ll ans = inf;
	rep(i, 0, 3) chmin(ans, dist[get(i, gx, gy)]);
	if(ans > inf/2) ans = -1;
	outl(ans);
