...
void solve(){
    ...
    if (Pivot.size() == 0) {
        REP(i, A.size()) {
            ...
            ans += A[i].back() - A[i].front();
        }
        REP(i, B.size()) {
            if (B[i].size() == 0) continue;
            ans += B[i].back() - B[i].front();
        }
    }
    if (Pivot.size() != 0) {
        REP(i, A.size()) {
            ...
            if (A[i].size() != 0) {
                int hoge = 0;
                REP(q, (int)A[i].size() - 1) {
                    Cost_Just += A[i][q + 1] - A[i][q];
                    hoge = max(hoge, A[i][q + 1] - A[i][q]);
                }
                int CA = 1e18;
                int CB = 1e18;
                if (i > 0) {
                    CA = A[i].front() - Pivot[i - 1];
                }
                if (i < Pivot.size()) {
                    CB = Pivot[i] - A[i].back();
                }
                ...
                Cost_Just += CA;
                Cost_Just -= hoge;
                Cost_Just += min(CB, hoge);
            }
            if (B[i].size() != 0) {
                int hoge = 0;
                REP(q, (int)B[i].size() - 1) {
                    Cost_Just += B[i][q + 1] - B[i][q];
                    hoge = max(hoge, B[i][q + 1] - B[i][q]);
                }
                int CA = 1e18;
                int CB = 1e18;
                if (i > 0) {
                    CA = B[i].front() - Pivot[i - 1];
                }
                if (i < Pivot.size()) {
                    CB = Pivot[i] - B[i].back();
                }
            ...
            if (i > 0 && i < Pivot.size()) {
                Cost_Just += Pivot[i] - Pivot[i - 1];
            }
            if (i > 0 && i < Pivot.size() && A[i].size() != 0 && B[i].size() != 0) {
                Cost_Just = min(Cost_Just, (Pivot[i] - Pivot[i - 1]) * 2LL);
            }
            ans += Cost_Just;
        }
    }
