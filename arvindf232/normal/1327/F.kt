// 2022-03-31, Thu, 19:03 palsed 26
// resumed 801
...
class item (val l:Int, val r:Int, val x:Int)
fun main(){
    solve.cases{
        val (n,k,m) = getline(3)
        val all = mutableListOf<item>()
        repeat(m){
            all.add(item(getint, getint, getint ))
        }
        fun solve(level:Int):Int{
            val DP = IntArray(n+2)

            var rsumnow = 0
            val DPrangeSum = IntArray(n+2)
            val paint = tempIntervalSum(n+2)
            DP[0] = 1
            val cangoto = IntArray(n+2){n+1}

            for(e in all){
                val t = (e.x and (1 shl level) != 0)
                if(t){
                    paint.add(e.l,e.r,1)
                }else{
                    val l = e.l
                    val r = e.r
                    cangoto[l-1] = minOf(cangoto[l-1], r)
                }
            }
            val painted = paint.compile()
//            for(i in 1..n){
////                if(painted[i] > 0){
////                    cangoto[i-1] = minOf(cangoto[i-1],i)
////                }
//            }
            for(i in n+1 downTo 1){
                cangoto[i-1] = minOf(cangoto[i-1], cangoto[i])
            }
            for(i in 0..n+1){
                rsumnow = rsumnow modPlus  DPrangeSum[i]
                DP[i] = DP[i] modPlus rsumnow
                if(painted[i] > 0){
                    continue
                }
                if(i == n+1){
                ...
                val here = DP[i]
                DPrangeSum[i+1] = DPrangeSum[i+1] modPlus here
                val end = cangoto[i]
                if(end != n+1){
                    DPrangeSum[end+1] = DPrangeSum[end+1] modMinus here
                }
            }
            return DP.last()
        }
...
/*
I flipped 0 and 1s
did not consider initial condition things in DP range sum 
