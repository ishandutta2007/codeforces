...
void main_() {
	...
	INT(n);
	VEC(int, s, n);
	RangeAddRangeSumBIT<ll> BT(n);
	REP(i, n) {
		//
		int lazy = BT.sum(i, i + 1);
		auto lazy_add = [&](int x) {
			if(!x) return;
			BT.add(max(i + 1, i + 1 + s[i] - x), min(i + s[i] + 1, n), 1);
			if(x > s[i]) BT.add(i + 1, i + 2, x - s[i]);
			s[i] = max(1, s[i] - x);
		};
		if(s[i] - 1 > lazy)
			res += s[i] - 1 - lazy;
		lazy_add(max(lazy, s[i] - 1));
	}
	print(res);
}
