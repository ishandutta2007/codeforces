...
pii go(int cur, int p = -1) {
    ...
    for (int to : v[cur]) {
        ...
        if (f.second > free) {
            secondFree = free;
            free = f.second;
        } else if (f.second > secondFree) {
            secondFree = f.second;
        }
    }
    //    if (k == 6) {
    //        DBN(cur, free, secondFree, res);
    //    }
    if (free + secondFree + 1 >= k) {
        return {res + 1, 0};
    } else {
        return {res, free + 1};
    }
}
int getAns(int k, int lval, int rval) {
    if (k == 1) return n;
    ::k = k;
    rval = min(rval, n / k);
    //    DBN(k, lval, rval);
    if (rval == lval) return lval;
    auto ans = go(0);
    return ans.first;
}
void calcAns(int l, int r, int lval, int rval) {
    //    DBN(l, r, lval, rval);
    if (l > r) return;
    int m = l + (r - l) * 3 / 4;
    int res = getAns(m, lval, rval);
    ans[m] = res;
    calcAns(l, m - 1, res, rval);
    calcAns(m + 1, r, lval, res);
}
int solve() {
    n = readInt();
    FI(n - 1) {
