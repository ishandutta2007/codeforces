...
//		3   4
//		5 6 7 y
		vector<ll> cost(8, 1e15);
		R(cost[2], cost[4], cost[6], cost[5], cost[3], cost[1]);
		
		const vector<vector<int>> coor = {{0, 1, 2}, {3, -1, 4}, {5, 6, 7}};
		auto get = [&](int dx, int dy) {
			if (-1 <= dx && dx <= 1 && -1 <= dy && dy <= 1)
				return coor[dx + 1][dy + 1];
			return -1;
		};
				...
				for (int y = -1; y <= 1; y++) {
					...
					for (int p = -1; p <= 1; p++) {
						for (int q = -1; q <= 1; q++) {
							...
							if (fd != -1 && sd != -1)
								chkmin(cost[now], cost[fd] + cost[sd]);
						}
					}
				}
			}
		...
		ll ans = cost[2] * take + cost[1] * x + cost[4] * y;
		W(ans);
	}
