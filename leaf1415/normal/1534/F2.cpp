...
int main(void)
{
	...
	rep(i, 1, C){
		for(auto u : scc.compG[i]){
			chmin(dp[u].first, dp[i].first);
			chmax(dp[u].second, dp[i].second);
		}
	}
	...
	rep(x, 0, w-1){
		...
		rep(y, 0, h-1){
			...
			if(cnt == a[x]){
				p = scc.scc[id[p]];
				vec[dp[p].first].push_back(dp[p].second);
				break;
			}
		}
	}
