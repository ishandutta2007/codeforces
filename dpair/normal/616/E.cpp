...
inline void Sub(LL &x, const LL y) {((x -= y) < 0) && (x += MOD);}

signed main(){
    read(n, m); LL ans = (__int128_t)n * m % MOD;
    chmin(m, n); for (LL l = 1, r = 0;l <= m;l = r + 1) {
        const LL t = n / l; r = mn(n / t, m);
        const LL len = (r - l + 1);
        const LL tmp = ((__int128_t)(l + r) * len >> 1) % MOD;
        Sub(ans, (__int128_t)t * tmp % MOD);
    } print(ans);
}