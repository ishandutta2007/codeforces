...
bool check(ll root){
    ...
    szdfs(root,-1);
    if(sz[root]<=3)return true;
    if(!find(FIB,sz[root]))return false;
    ll M=POSL(FIB,sz[root]);
    root=centroid(root,-1,sz[root]);
    vl a;
    dfs(root,-1,a);
    vl nx;
    for(auto x:a){
        if(sz[x]==FIB[M-2])nx.pb(x);
        assert(sz[x]!=FIB[M-1]);
    }
    if(nx.size()==0)return false;
    assert(nx.size()<=2);
    ll as=a.size();
    vector<bool>f(as,false);
    
    
    for(auto top:nx){
        ll par=p[top];
        assert(v[top].find(par)!=v[top].end());
        assert(v[par].find(top)!=v[par].end());
        v[top].erase(par);
        v[par].erase(top);
        if(!check(top)){
            return false;
