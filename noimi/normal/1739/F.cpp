...
int main() {
    ...
    rep(i, n) {
        Graph g(12);
        rep(j, si(s[i]) - 1) { add(g, s[i][j] - 'a', s[i][j + 1] - 'a'); }
        rep(j, 12) UNIQUE(g[j]);
        int st = -1;
        rep(j, 12) if(si(g[j]) == 1) st = j;
        rep(j, 12) if(si(g[j]) > 2) st = -1;
        if(st != -1) {
            string w;
...

    {
        ...
        fore(c, s, v) {
            bool flag = true;
            fore(cc, ss, w) if(ss == s) cc += c, flag = false;
            if(flag) w.eb(c, s);
        }
        swap(v, w);
    }
    ...
    if(empty(v)) {
        LOCAL { drop(0); }
        rep(i, 12) cout << char('a' + i);
        OUT();
        exit(0);
    }

    //  string, 
    vv(int, dp, si(v), 1 << 12, -inf<int>);

    vv(int, con, si(v), si(v));
    vv(int, bits, si(v), si(v));
    vv(int, inc, si(v), 13);
    rep(i, si(v)) {
        rep(j, si(v)) {
            int k = v[i].se.find(v[j].se[0]);
            if(k == -1) {
                int b = 0;
                fore(e, v[j].se) b |= 1 << (e - 'a');
                int bb = 0;
                fore(e, v[i].se) bb |= 1 << (e - 'a');
                if(b & bb)
                    con[i][j] = -1;
                else
                    bits[i][j] = b;
            ...
            int L = min(v[i].se.size() - k, v[j].se.size());
            if(v[i].se.substr(k, L) == v[j].se.substr(0, L)) {
                if(L == v[j].se.size())
                    inc[i][k + L] += v[j].fi, con[i][j] = -1;
                else {
                    con[i][j] = L;
                    rep(k, L, v[j].se.size()) bits[i][j] |= 1 << (v[j].se[k] - 'a');
                }
            } else {
                con[i][j] = -1;
            }
    ...
    vv(int, cost, si(v), si(v));
    vi c1(si(v));
    rep(i, si(v)) rep(j, 1, 13) c1[i] += inc[i][j];
    rep(i, si(v)) {
        rep(j, si(v)) {
            if(con[i][j] != -1) { rep(k, con[i][j] + 1, 13) cost[i][j] += inc[j][k]; }
        }
    }

    rep(i, si(v)) {
        int b = 0;
        fore(e, v[i].se) b |= 1 << (e - 'a');
        dp[i][b] = c1[i];
    }

    vv(pii, pre, si(v), 1 << 12, pii(-1, -1));
    rep(b, 1 << 12) {
        rep(i, si(v)) {
            ...
            rep(j, si(v)) {
                ...
                if(b & bits[i][j]) continue;
                if(chmax(dp[j][b | bits[i][j]], dp[i][b] + cost[i][j])) { pre[j][b | bits[i][j]] = pii(i, b); }
            }
        }
    }
    ...
    int ma = -1;
    int i, j;
    rep(x, si(v)) rep(y, 1 << 12) if(chmax(ma, dp[x][y])) i = x, j = y;
    dump(i, j);
    dump(v);
    ...
    fore(i, ss) {
        if(empty(ans))
            ...
            ans += v[i].se.substr(con[pi][i]);
        }
        pi = i;
    }
    rep(i, 12) {
        char c = 'a' + i;
        if(ans.find(c) == -1) ans.pb(c);
    }
