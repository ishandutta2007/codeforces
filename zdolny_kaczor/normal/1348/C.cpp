...
int main() {
	...
	REP(_, t) {
		...
		vector <string> ans;
		sort(row, row + n);
		REP(i, k) ans.emplace_back(1, row[i]);
		if (ans[0] != ans.back()) {
			ans[0].insert(ans[0].end(), row + k, row + n);
		}
		else if (row[k] == row[n - 1]) {
			for (int i = k; i < n; ++i) ans[i % k].push_back(row[i]);
		}
		else ans[0].insert(ans[0].end(), row + k, row + n);
		debug(imie(ans));
		printf("%s\n", max_element(ALL(ans))->c_str());
	}
	exit(0);
}
//#STAY AT HOME