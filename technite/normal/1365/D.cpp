...
//template ends//
void dfs(int x,int y,vector<string>&adj,int&good)
{
    if(x<0||x>adj.size()-1||y<0||y>adj[0].size()-1) return;
    if(adj[x][y]=='#') return;
    if(adj[x][y]=='G') good--;
    adj[x][y]='#';    
    dfs(x-1,y,adj,good);
...
int main()
{
    ...
    TT
    {
        ...
        if(n>1&&good&&a[n-2][m-1]=='B')cout<<"NO"<<'\n';
        else if(m>1&&good&&a[n-1][m-2]=='B') cout<<"NO"<<'\n';
        else
        {   
            for(int i=0;i<n;i++)
            {
                for(int j=0;j<m;j++)
                {
                    if(a[i][j]=='#'||a[i][j]=='B') continue;
                    if (i-1 >= 0 && a[i-1][j] == 'B') a[i][j] = '#';
                    if (i+1 <= n-1 && a[i+1][j] == 'B') a[i][j] = '#';
                    if (j-1 >= 0 && a[i][j-1] == 'B') a[i][j] = '#';
                    if (j+1 <= m-1 && a[i][j+1] == 'B') a[i][j] = '#';
                }
            }
            dfs(n-1,m-1,a,good);
            if(good==0) cout<<"YES"<<"\n";
