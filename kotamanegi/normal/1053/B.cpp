...
void solve() {
    ...
    ruiseki.push_back(0);
    for (int q = 0; q < n; ++q) {
        ruiseki.push_back(inputs[q] + ruiseki.back());
    }
    int ans = 0;
    int cnts[2] = {};
    cnts[0]++;
    for (int q = 1; q < ruiseki.size(); ++q) {
        int tmp[2] = { cnts[0],cnts[1] };
        int now_maxs = 0;
        int nows = 0;
        for (int j = q - 1; j >= 0 and abs(j-q) < 60; --j) {
            now_maxs = max(now_maxs, inputs[j]);
            nows += inputs[j];
            if (now_maxs * 2LL <= nows and nows % 2LL == 0) {
                ans++;
            }
            tmp[ruiseki[j] % 2]--;
        }
