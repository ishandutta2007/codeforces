...
ll random(){
	return
	#if RAND_MAX==0x7fff
	(((((rand()*1ll<<15ll)+rand())<<15ll)+rand())<<15ll)+rand()
...
int main(){
	...
	ll n=(1<<readll())-1;
	rep(i,1,422){
		ll x=rand(1,n),y=rand(1,n),z=rand(1,n);
		while(x==y||y==z||x==z)x=y,y=z,z=rand(1,n);
		flush("? %lld %lld %lld\n",x,y,z);
		ll ans=readll();
	...
	ll max=0,cmax=0;
	rep(i,1,n){
		if(t[i]>t[max])cmax=max,max=i;
		else{
			if(t[i]>t[cmax])cmax=i;
		}
	}
	rep(i,1,n)
	if(i!=max&&i!=cmax){
