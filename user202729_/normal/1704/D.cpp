// time-limit: 2000
// problem-url: https://codeforces.com/contest/1704/problem/D


// eh why hack disabled? this algorithm seems to be correct in all cases anyway
...
void up(){
	int const numArray=read<int>();
	int const eachArrayLen=read<int>();

	std::vector<std::vector<int64_t>> c(numArray);
	std::vector<uint64_t> s_mod_2_pow_64(numArray);

	for(int const i: range(0, numArray)){
		auto& ci=c[i];
		ci.resize(eachArrayLen);
		for(auto& x: ci) std::cin>>x;
		std::partial_sum(begin(ci), end(ci), begin(ci));  // entry at most 3e5  3e11 = 9e16
		s_mod_2_pow_64[i]=std::accumulate(begin(ci), end(ci), uint64_t(0));  // this accumulate might overflow though, or so it seems
	}

	assert(numArray>=3);
	auto common_value=s_mod_2_pow_64[0]==s_mod_2_pow_64[1] ? s_mod_2_pow_64[0]: s_mod_2_pow_64[2];
	assert(int(std::count(begin(s_mod_2_pow_64), end(s_mod_2_pow_64), common_value))==numArray-1);

	auto different_index=int(std::find_if(begin(s_mod_2_pow_64), end(s_mod_2_pow_64), [&](auto const& it){
		return it!=common_value;
	})-begin(s_mod_2_pow_64));

	std::cout<<
		different_index+1
		<<' '<<
		common_value-s_mod_2_pow_64[different_index]
		<<'\n';
