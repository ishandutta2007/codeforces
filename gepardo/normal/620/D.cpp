...
int binsearch(int l, int r, int e)
{
    if (l == r)
    {
        if (pb[l].ff <= e) return l; else return -1;
    }
    if (l + 1 == r)
    {
        if (pb[r].ff <= e) return r;
        if (pb[l].ff <= e) return l; else return -1;
    }
    int m = (l + r) / 2;
    if (e <= pb[m].ff) return binsearch(l, m, e); else return binsearch(m, r, e);
}
...
main()
{
    ...
    int sum;
    bool sw = false;
    if (sa < sb) { swap(a, b); swap(n, m); swap(sa, sb); sw = true; }

    sum = (sa - sb + 1) / 2;

    int op = 0;
    pair<int,int> o1 = {-1, -1}, o2 = {-1, -1};

    pa.clear(); pb.clear();
    for (int i = 0; i < n; i++)
        for (int j = 0; j < i; j++)
            pa.push_back({a[i] + a[j], {i, j}});
    for (int i = 0; i < m; i++)
        for (int j = 0; j < i; j++)
            pb.push_back({-b[i] - b[j], {i, j}});
    ...
    {
        if (pb.size() == 0) break;
        int j = binsearch(0, pb.size() - 1, sum - pa[i].ff);
        if (j >= 0)
        {
            if (pa[i].ff + pb[j].ff > op)
            {
                op = pa[i].ff + pb[j].ff;
                o1 = pa[i].ss;
                o2 = pb[j].ss;
        ...
        {
            j++;
            if (sa - sb - pa[i].ff - pb[j].ff > op)
            {
                op = sa - sb - pa[i].ff - pb[j].ff;
                o1 = pa[i].ss;
                o2 = pb[j].ss;
            }
        }
    ...
    pa.clear(); pb.clear();
    for (int i = 0; i < n; i++) pa.push_back({a[i], {i, -1}});
    for (int i = 0; i < m; i++) pb.push_back({-b[i], {i, -1}});
    sort(pa.begin(), pa.end());
    sort(pb.begin(), pb.end());
    ...
    out.putln(_abs(sa - op - sb - op));
    int r;
    if (o1.ff < 0) r = 0; else if (o1.ss < 0) r = 1; else r = 2;
    out.putln(r);
    if (sw) swap(o1, o2);
    if (r > 0) out.putln(o1.ff + 1, ' ', o2.ff + 1);
    if (r > 1) out.putln(o1.ss + 1, ' ', o2.ss + 1);
