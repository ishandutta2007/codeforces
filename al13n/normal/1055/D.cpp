...
int main(int argc, char **argv){
  ...
  forn(i,n){
    ...
    P[i]=a;
    if(!sz(S)){
      S=A[i].substr(a,b-a+1);
      T=B[i].substr(a,b-a+1);
    }else if(A[i].compare(a,b-a+1,S)||B[i].compare(a,b-a+1,T)){
      dbg(S,T,i,a,b,A[i].substr(a,b-a+1),B[i].substr(a,b-a+1));
      cout<<"NO";
      return 0;
    }
  }
  assert(sz(S)&&sz(S)==sz(T));
  dbg(S,T);
  string suf,pref;
  while(true){
    char c=-1;
    ...
    forn(i,n){
      if(P[i]==-1) continue;
      if(P[i]+sz(S)+sz(suf)>=sz(A[i])){
        f=true;
        break;
      }
      char cc=A[i][P[i]+sz(S)+sz(suf)];
      if(c==-1) c=cc;
      else if(c!=cc){
        f=true;
        break;
  ...
  forn(q,n){
    int l=0;
    forv(i,A[q]){
      while(l && A[q][i]!=S[l]) l=pf[l-1];
      if(A[q][i]==S[l]) ++l;
      if(l==sz(S)){
        if(i-l+1!=P[q]-sz(pref)){
          cout<<"NO";
          return 0;
        }else{
          break;
