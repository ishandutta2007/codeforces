...
vector<string> check(int now) {
    vector<string> ans = inputs;
    vector<string> reflect = inputs;
    vector<vector<int>> rights(n, vector<int>(m, 0LL));
    REP(i, n) {
        REP(q, m) {
            ans[i][q] = '.';
            reflect[i][q] = '.';
    ...
    if (now != 0) {
        for (int i = (int)ans.size() - 1; i >= 0; --i) {
            for (int q = (int)ans[i].size() - 1; q >= 0; --q) {
                if (i + now  < ans.size() && q + now < ans[i].size()) {
                    ans[i + now][q + now] = ans[i][q];
                    ans[i][q] = '.';
                }
            }
        }
