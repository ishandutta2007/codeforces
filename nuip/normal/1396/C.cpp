...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto d=read();
	cauto as=readV(n);
	vector<ll> noko(n),kesu(n);
	rep(i,n){
		noko[i]=min(r2,(as[i]+1)*r1)+r1;
		kesu[i]=as[i]*r1+r3;
		MN(noko[i],kesu[i]);
	}
	vector<ll> L(n+1,INF);
	...
	rep(i,n){
		MN(L[i+1],L[i]+kesu[i]);
		MN(L[i+1],L[i]+noko[i]+2*d);
		if(i+1<n) MN(L[i+2],L[i]+noko[i]+noko[i+1]+2*d);
	}
	vector<ll> R(n+1,INF);
	R[n]=0;
	R[n-1]=min(kesu[n-1],noko[n-1]+2*d);
	rrep(i,n-1){
		MN(R[i],R[i+1]+noko[i]);
	}
	out(noko,kesu,L,R,1);
	ll re=L[n];
	rep(i,n) out(i, L[i]+R[i]+d*(n-1-i),1);
	rep(i,n) MN(re, L[i]+R[i]+d*(n-1-i));
	MN(re, L[n-2]+noko[n-2]+noko[n-1]+2*d);
	cout<<re+(n-1)*d NL;
  return 0;
}