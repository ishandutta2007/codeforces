...
void main_() {
	...
	REP(i, n) {
		...
		foa(c, sv[i]) {
			tmp++;
			if(c == '-') len.pb(tmp), tmp = 0;
		}
		if(tmp) len.pb(tmp + (i != n - 1));
	}
	int ok = 10000000, ng = 0;
	auto f = [&](int m) {
		int tmp = 0, cnt = 1;
		REP(i, SZ(len)) {
			if(len[i] > m) return false;
			if(tmp + len[i] <= m)
				tmp += len[i];
			else
				tmp = len[i], cnt++;
	...
	while(mid = (ok + ng) / 2, ok - ng > 1) (f(mid) ? ok : ng) = mid;
	print(ok);
}
