...
inline void solve() {
	...
	for (int i = 0; i + 1 < n; ++i) {
		lft += v[i];
		ll go = (v[i] * (i + 1) - lft) + (sm - lft - v[i + 1] * (n - i - 1));
		if (i + 1 >= k || n - i - 1 >= k || v[i] == v[i + 1]) {
			mn = min(mn, go);
		} else {
			mn = min(mn, go + (v[i + 1] - v[i] - 1) * (i + 1) + min(k - (n - i - 1), k - i - 1));
			mn = min(mn, go + (v[i + 1] - v[i] - 1) * (n - i - 1) + min(k - (n - i - 1), k - i - 1));
		}
		//mn = min(mn, (v[i] * i - lft) + (sm - lft - (n - i) * v[i]) - (n - k));
	}


	int pos = k - 1;
	while (pos + 1 < n && v[pos + 1] == v[k - 1]) ++pos;
	mn = min(k * v[k - 1] - accumulate(v.begin(), v.begin() + k, 0LL) - (pos - k + 1), mn);
	
	pos = n - k;
	while (pos > 0 && v[pos - 1] == v[n - k]) {
		--pos;
	}
	mn = min(accumulate(v.end() - k, v.end(), 0LL) - v[n - k] * k - (n - k - pos), mn);
	cout << mn << endl;
}
