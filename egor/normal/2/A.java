...
public class Task2A {
	...
	private void solve() {
		...
		String[] name = new String[roundCount];
		int[] delta = new int[roundCount];
		...
		for (int i = 0; i < roundCount; i++) {
			if (!score.containsKey(name[i]))
				score.put(name[i], delta[i]);
			else
				score.put(name[i], delta[i] + score.get(name[i]));
		}
		Set<String> potentialWinners = new HashSet<String>();
		int winnerScore = 0;
		for (Map.Entry<String, Integer> entry : score.entrySet()) {
			...
			} else if (entry.getValue() == winnerScore)
				potentialWinners.add(entry.getKey());
		}
		score.clear();
		for (int i = 0; i < roundCount; i++) {
			if (!score.containsKey(name[i]))
				...
				score.put(name[i], delta[i] + score.get(name[i]));
			if (score.get(name[i]) >= winnerScore && potentialWinners.contains(name[i])) {
				out.println(name[i]);
				return;
			}
		}
...
//		return strictCheck(result, output);
	...
	private static void test() {
		...
		for (Test test : tests) {
			...
			try {
				new Task2A(in, new PrintWriter(out)).run();
	...
	private Task2A(InputReader in, PrintWriter out) {
		this.in = in;
		this.out = out;
		testMode = true;
	}
