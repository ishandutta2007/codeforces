...
int main(){
	...
	string s;
	cin >> n >> k >> s;
	vector dp(n + 1, vector<modular>(k + 1));
	vector pref(n + 2, vector<modular>(k + 1));
	dp[0][0] = 1;
	pref[1][0] = 1;
	for(auto i = 1; i <= n; ++ i){
		int cnt = 'z' - s[i - 1];
		for(auto x = 0; x <= k; ++ x){
			dp[i][x] += pref[i][x] * (s[i - 1] - 'a');
			for(auto j = i - 1; j >= 0 && x + (n - i + 1) * (i - j) <= k; -- j){
				dp[i][x + (n - i + 1) * (i - j)] += dp[j][x] * cnt;
			}
		}
		for(auto x = 0; x <= k; ++ x){
			pref[i + 1][x] = pref[i][x] + dp[i][x];
		}
	}
	cout << pref[n + 1][k] << "\n";
	return 0;
