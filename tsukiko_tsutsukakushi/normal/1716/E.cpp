...
S op(S a, S b) {
    return S{a.sum + b.sum, max(a.left, a.sum + b.left), max(b.right, b.sum + a.right), max({a.ans, b.ans, a.right + b.left})};
}
...
void solve() {
    ...
    vc<S> v(m);
    rep(i, m) {
        v[i] = S{a[i], max(0LL, a[i]), max(0LL, a[i]), max(0LL, a[i])};
    }
    otera::xor_segtree<S, op, e> seg(v);
    rep(_, q) {
        INT(k);
        x ^= (1<<k);
        int ans = seg.prod(0, m, x).ans;
        out(ans);
    }
}
