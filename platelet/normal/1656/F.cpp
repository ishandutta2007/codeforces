...
__int128 calc(ll t) {
    ...
    while(l <= r) {
        auto fn = [&](int v) {
            return min((ll)v * mn + (__int128)t * (v + mn), (ll)v * mx + (__int128)t * (v + mx));
        };
        int v = fn(a[l]) < fn(a[r]) ? a[l++] : a[r--];
        res += fn(v), cmin(mn, v), cmax(mx, v);
    }
    return res;
}
void solve() {
    ...
    sort(a + 1, a + n + 1);
    if(sum + (n - 2LL) * a[1] > 0 || sum + (n - 2LL) * a[n] < 0) { cout << "INF\n"; return; }
    ll l = -1e18, r = 1e18;
    while(l < r) calc(mid) < calc(mid + 1) ? l = mid + 1 : r = mid;
    __int128 ans = calc(l);
    if(ans < 0) cout << '-', ans = -ans;
    char ch[40];
    int tp = 0;
    do ch[++tp] = ans % 10 + 48, ans /= 10; while(ans);
    while(tp) cout << ch[tp--];
    cout << '\n';
}
int main() {
