...
void solve() {
    ...
    ans.clear();
    read(a, n);
    bytask.clear(); bytask.rsz(n);
    e.rsz(m); t.rsz(m); p.rsz(m);
    F0R(i, m) {
        cin >> e[i] >> t[i] >> p[i]; e[i]--;
        bytask[e[i]].pb(i);
    }
    ll tim = 0;
    F0R(i, n) {
        int numtasks = bytask[i].size();
        vvll dp(numtasks+1, vll(101, 1e18));
        F0R(j, numtasks) dp[j][0] = 0;
        F0R(j, numtasks) {
            int task = bytask[i][j];
            FOR(k, 1, 101) dp[j+1][k] = min(dp[j][k], dp[j][max(0, k-p[task])] + t[task]);
        }
        if (tim + dp[numtasks][100] > a[i]) {
            cout << -1 << nl;
            return;
        }
        tim += dp[numtasks][100];
