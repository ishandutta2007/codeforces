...
struct Bridge{
	int n;
	set<P> bridgeS;
	vector<vector<int> > G, compG;
	vector<int> becc;
	vector<int> pre, low;
	int id, beccnum;
	
	Bridge(){}
	Bridge(int n){this->n = n, G.resize(n+1);}
	
	int finddfs(int v, int p){
		pre[v] = id++;
		int ret = pre[v];
		for(auto u : G[v]){
			if(u == p) continue;
			if(pre[u]) ret = min(ret, pre[u]);
			else ret = min(ret, finddfs(u, v));
		}
		return low[v] = ret;
	}
	...
	void findBridge(){
		...
		for(int i = 1; i <= n; i++){
			for(auto u : G[i]) if(pre[i] < pre[u] && low[u] > pre[i]){
	...
	void beccdfs(int v, int p, int id){
		becc[v] = id;
		for(auto u : G[v]){
			if(u == p) continue;
	...
	int calcBECC(){
		id = 1, becc.resize(n+1);
		for(int i = 1; i <= n; i++) becc[i] = 0;
		for(int i = 1; i <= n; i++) if(!becc[i]) beccdfs(i, -1, id++);
		return beccnum = id-1;
	}
	void compressBECC(){
...
ll n, m, s, t;
ll u[300005], v[300005], w[300005];
bool art[300005];
set<P> artS;
Bridge brg;

bool dfs(int v, int p, int t, bool b)
{
	b |= art[v];
	if(v == t) return b;
	
	for(auto u : brg.compG[v]){
		if(u == p) continue;
		if(dfs(u, v, t, b || artS.count(P(v, u)))) return true;
	}
	return false;
}
...
int main(void)
{
	...
	rep(i, 1, m){
		if(w[i] == 0) continue;
		if(brg.isBridge(u[i], v[i])){
			artS.insert(P(brg.becc[u[i]], brg.becc[v[i]]));
			artS.insert(P(brg.becc[v[i]], brg.becc[u[i]]));
		}
		else art[brg.becc[u[i]]] = true;
	}
	
	ll S = brg.becc[s], T = brg.becc[t];
	if(dfs(S, -1, T, 0)) yes();
	else no();
