...
int main() {
    ...
    vector<ll> f(k);
    vector<vector<array<ll, 2>>> occ(k);
    set<ll> bad;
    for (int i = 0; i < n; i++) {
        ll c; cin >> c;
        a[i].resize(c);
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < a[i].size(); j++) {
            while (s[u.rep(i)].size() <= j+off[i])
                s[u.rep(i)].pb(-1);
            s[u.rep(i)][j+off[i]] = a[i][j];
        }
    }
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < a[i].size(); j++)
            if (s[u.rep(i)][j+off[i]] != a[i][j]) {
                bad.insert(s[u.rep(i)][j+off[i]]);
            }
    }
    map<ll, ll> g;
    for (auto [ui, v] : s) {
