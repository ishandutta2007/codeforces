...
int main() {
	...
	vector<vector<int>> adj(n+7,vector<int>(n+7));
	
	vector<int> dist(n+7);
	vector<int> from(n+7,0);
	for (int i = 1; i <= n; i++)
		dist[i] = c[i];
		
	vector<bool> isFin(n+7,false);
	ll ans = 0;
	...
	for (int i = 1; i <= n; i++) {
		...
		int mnidx = -1;
		for (int j = 1; j <= n; j++) {
			if (!isFin[j] && dist[j] < mn) {
				mn = dist[j];
				mnidx = j;
			}
		}
		
		int fr = from[mnidx], to = mnidx;
		...
		for (int j = 1; j <= n; j++) {
			ll tk = k[j] + k[to];
			ll d = abs(x[j] - x[to]) + abs(y[j] - y[to]);
			if (tk * d < dist[j]) {
				dist[j] = tk * d;
				from[j] = to;
			}
