...
int main() {
    ...
    vvmn neg(N+1, vmn(M+1));
    F0R (plus, N + 1) {
        F0R (minus, M + 1) {
            if (plus > minus) neg[plus][minus] = 0;
            else if (plus == 0) neg[plus][minus] = 1;
            else {
                neg[plus][minus] = neg[plus-1][minus]
                                 + neg[plus][minus-1];
            }
        }
    }

    vvmn sum(N+1, vmn(M+1));
    F0R (plus, N + 1) {
        F0R (minus, M + 1) {
            if (plus) sum[plus][minus] += sum[plus-1][minus] + mn::ncr(plus-1+minus, minus);
            if (minus) sum[plus][minus] += sum[plus][minus-1] - mn::ncr(plus+minus-1, minus-1) + neg[plus][minus-1];
        }
    }
    ps(sum[N][M]);
