    ...
    REP(tc,1,ntc+1)
    {
        ...
        if(k<=4)
        {
            ...
            REP(t,0,(1<<((1<<k)-1)))
            {
                ...
                REP(i,0,k)
                {
                    ll tmp = ( (t & ( ( 1 << ((1<<(k-i))-1) ) - 1)) >> ((1<<(k-i-1))-1) );
                    ll tmp2 = 0;
                    REP(j,0,(1<<(k-i-1)))
                    {
                        if((tmp&(1<<j))!=0)
                ...
                REP(j,0,(1<<k))
                {
                    ...
                    x = (x + (j+1)*powM(a, p[j], M2))%M2;
                }
                if(x == h)
                {
                    REP(j,0,(1<<k)) cout<<p[j]<<' ';
                    bad = false;
                    break;
                }
            }
        ...
        else
        {
            ...
            REP(t,0,(1<<15))
            {
                ...
                REP(j,0,16)
                {
                    ...
                    {
                        p[j] = 1;
                        dp[0].push_back({(x + (j+1)*powM(a, p[j], M2))%M2, 2*t});
                        dat[2*t] = p;
                        p[j] = 2;
                        dp[1].push_back({(x + (j+1)*powM(a, p[j], M2))%M2, 2*t+1});
                        dat[2*t+1] = p;
                    }
                }
            }
            ...
            REP(t,0,(1<<15))
            {
                ...
                REP(j,0,16)
                {
                    ...
                    {
                        p[j] = 1;
                        auto tt = (*lower_bound(dp[1].begin(), dp[1].end(), MP((h + M2 - ((x + (16+j+1)*powM(a, p[j], M2))%M2) )%M2, 0LL) ));
                        if(tt.F == (h + M2 - ((x + (16+j+1)*powM(a, p[j], M2))%M2) )%M2)
                        {
                            for(auto it : dat[tt.S]) cout<<it<<' ';
                        ...
                        p[j] = 2;
                        tt = (*lower_bound(dp[0].begin(), dp[0].end(), MP((h + M2 - ((x + (16+j+1)*powM(a, p[j], M2))%M2) )%M2, 0LL) ));
                        if(tt.F == (h + M2 - ((x + (16+j+1)*powM(a, p[j], M2))%M2) )%M2)
                        {
                            for(auto it : dat[tt.S]) cout<<it<<' ';
                            for(auto it : p) cout<<it<<' ';
