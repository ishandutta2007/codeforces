...
void Sieve(){
	...
	Forx(i,2,10000000) if(!div[i]) div[i] = i;
	// Print(div,1,20);
}
...
int main(){
	...
	For(i,n){
		...
		ansl[i] = [&]()->pii{
			...
			int mul = 1; for( int q : p ) mul *= q;
			return {p[0],mul/p[0]};
		}();
	}

	For(i,n) printf("%d ",ansl[i].F); putchar(10);
	For(i,n) printf("%d ",ansl[i].S); putchar(10);
