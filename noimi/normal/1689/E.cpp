...
int main() {
    TEST {
        ...
        VEC(int, a, n);
        int ans = 0;
        rep(i, n) if(a[i] == 0) ans++, a[i] = 1;
        UnionFind uf(30);
        vi used(30);
        fore(e, a) {
            rep(i, 30) rep(j, 30) {
                if(e >> i & 1 and e >> j & 1) uf.unite(i, j), used[i] = used[j] = true;
            }
        }
        int m = 1;
        rep(i, 30) if(!used[i]) m++;
        if(uf.num == m) {
            OUT(ans);
            OUT(a);
        } else {
            auto check = [](vi &b) {
                ...
                fore(e, b) {
                    int k = lowbit(e);
                    rep(i, 30) if(e >> i & 1) uf.unite(i, k), used[i] = true;
                }
                rep(i, 30) if(!used[i]) m++;
                return uf.num == m;
            };
            rep(i, n) {
                ...
                if(i == n - 1) {
                    int x, k = -1;
                    rep(i, n) if(chmax(k, lowbit(a[i]))) x = i;
                    vi v;
                    rep(i, n) if(lowbit(a[i]) == k) v.eb(i);
                    assert(si(v) > 1);
                    a[v[0]]++, a[v[1]]--;
                    OUT(ans + 2);
                    OUT(a);
