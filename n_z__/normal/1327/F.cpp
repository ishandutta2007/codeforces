...
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
	int n,k,m;
    cin>>n>>k>>m;
    int l[500001],r[500001],q[500001];
    for(int x=1;x<=m;x++)
    cin>>l[x]>>r[x]>>q[x];
    int ans=1;
    for(int x=0;x<k;x++)
    {
        int w[500001],a[500001],f[500001];
        memset(w,0,sizeof(w));
        memset(a,0,sizeof(a));
        memset(f,0,sizeof(f));
        int sum=1,e=0;
        f[0]=1;
        for(int y=1;y<=m;y++)
        if(q[y]>>x&1)++w[l[y]],--w[r[y]+1];else a[r[y]]=max(a[r[y]],l[y]);
        for(int y=1;y<=n;y++)
        {
            w[y]+=w[y-1];
            if(!w[y])f[y]=sum,sum=sum*2%p;
            //cout<<' '<<sum<<endl;
			while(e<a[y])sum=(sum-f[e])%p,++e;
        }
        //cout<<sum<<endl;
        ans=ans*sum%p;
    }
