...
public:
  static vector<int> suffix_array(const vector<int>& s,int upper){
    assert(0<=upper);
    for(int d:s){
      assert(0<=d&&d<=upper);
    }
  ...
  template<class T>
  static vector<int> suffix_array(const vector<T>& s){
    int n=int(s.size());
    vector<int> idx(n);
  ...
  static vector<int> suffix_array(const string& s){
    int n=int(s.size());
    vector<int> s2(n);
    for(int i=0;i<n;++i){
      s2[i]=s[i];
  ...
  template<class T>
  static vector<int> lcp_array(const vector<T>& s,const vector<int>& sa){
    int n=int(s.size());
    assert(n>=1);
    vector<int> rnk(n);
  ...
  static vector<int> lcp_array(const string& s,const vector<int>& sa){
    int n=int(s.size());
    vector<int> s2(n);
  ...
  template<class T>
  static vector<int> z_algorithm(const vector<T>& s){
    int n=int(s.size());
    if(n==0)return{};
    vector<int> z(n);
    z[0]=0;
...
private:
  static vector<int> sa_naive(const vector<int>& s){
    int n=int(s.size());
    vector<int> sa(n);
  ...
  static vector<int> sa_doubling(const vector<int>& s){
    int n=int(s.size());
  ...
  static vector<int> sa_is(const vector<int>& s,int upper){
    int n=int(s.size());
    if(n==0)return{};
    if(n==1)return{0};
    if(n==2){
...
int main(){
  ...
  cin>>s;
  N=s.size();
  auto z=stringalgorithm::z_algorithm(s);
  vector<pair<ll,string>> ids;
  rep1(i,min(N,10LL)){
    if(z[i]+i==N)ids.emplace_back(i,s.substr(N-i));
  }
  debug(ids);
  map<string,ll> mp;
  For(i,10,N){
    if(z[i]+i!=N)continue;
    rep(j,10)chmax(mp[s.substr(N-i,j+1)],N-i+j+1);
  }
  debug(mp);
  auto inits=s.substr(0,10);
  cin>>K;
  vec pr;
  while(K--){
    cin>>t;
    ...
    for(auto c:t){
      s+=c;
      auto itr=mp.find(s);
      ll ans=itr==mp.end()?0:itr->second;
      for(auto& x:ids){
        auto& ch=x.second;
        j=0;
        rep(i,min(s.size(),ch.size())){
          if(s[i]!=ch[i]){
            j=1;
            break;
          }
        ...
        if(j)continue;
        chmax(ans,N-x.first+(ll)s.size());
      }
      rep(i,s.size()){
        auto ch=s.substr(i);
        if(inits.substr(0,ch.size())==ch)chmax(ans,(ll)ch.size());
      }
      pr.emplace_back(ans);
    }
    output(pr);
