...
int main() {
    ...
    for (int i = 0; i <= k; i++) {
        if (b > 0) {
            a[i] = i64(b) * power(n + 1, n - i) % P;
        }
        b = i64(b) * (n - i) % P * power(i + 1, P - 2) % P;
    }
    std::vector<int> fac(k + 1);
    fac[0] = 1;
    for (int i = 1; i <= k; i++) {
    ...
    for (int i = 0; i <= k; i++) {
        x[i] = i64(power(i, k)) * power(fac[i], P - 2) % P * (i % 2 == 0 ? 1 : P - 1) % P;
        y[i] = power(fac[i], P - 2);
    }
    auto f = Poly(x) * Poly(y);
    int ans = 0;
    for (int i = 0; i <= k; i++) {
        ans = (ans + i64(a[i]) * f[i] % P * fac[i] % P * (i % 2 == 0 ? 1 : P - 1)) % P;
    }
    std::cout << ans << "\n";
    return 0;
}