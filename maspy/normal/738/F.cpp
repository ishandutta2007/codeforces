...
void solve() {
  ...
  const ll INF = 1LL << 30;
  vvv(int, DP, 90, N + 1, 180, -INF);
  auto dp = [&](ll a, ll b, ll k) -> int& { return DP[k][a][b - a + 90]; };

  auto calc = [&](auto dfs, ll a, ll b, ll k) -> ll {
    auto& ANS = dp(a, b, k);
    ...
    if (a <= b) {
      ANS = dfs(dfs, a + k, b, k);
      if (a + b + k + 1 <= N) chmax(ANS, dfs(dfs, a + k + 1, b, k + 1));
    } else {
      ANS = dfs(dfs, a, b + k, k);
      if (a + b + k + 1 <= N) chmin(ANS, dfs(dfs, a, b + k + 1, k + 1));
    }
    return ANS;
  };
  ll ANS = calc(calc, 0, 0, 1);
  print(ANS);
}
