...
class Solver {
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++)
        {
            sc.Multi(out cost[i], out frm[i], out to[i]);
            edge[i] = new List<int>();
        }
        delta = new int[n];
        for (int i = 0; i < n - 1; i++)
    ...
    long dfs(int p, int par, int mincost) {
        mincost = Math.Min(mincost, cost[p]);
        long res = 0;
        int pl = 0, mn = 0;
        if (to[p] - frm[p] > 0) {
            pl += to[p] - frm[p];
        ...
        res += mincost * (long)Math.Min(pl, mn);
        return res;
    }
}
