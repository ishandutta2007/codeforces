...
pair<int, int> val[200000];
int ok = 1;
int dfs(int now, int back) {
    pair<int, int> geko = mp(0, 0);
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        cnts[now] += dfs(x, now);
        geko.first += val[x].first;
        geko.second += val[x].second;
    }
    if (cnts[now] < abs(h[now])) {
        ok = 1;
    }
    val[now].first = (cnts[now] - h[now]) / 2LL;
    val[now].second = cnts[now] - val[now].first;
    if (val[now].second < geko.second) {
        ok = 1;
    }
    if (val[now].second - val[now].first != h[now]) {
        ok = 1;
    }
    return cnts[now];
}
