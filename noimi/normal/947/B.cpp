...
signed main(){
    ...
    int n=in();
    vecll v;rep(i,n)v.pb(in());
    vecll t;rep(i,n)t.pb(in());
    vecll cnt(n+1),res(n+1);
    vecll rui{0};
    rep(i,n)rui.pb(rui.back()+t[i]);
    rep(i,n){
        int l = i,r = n+1;
        while(l<r-1){
            int mid = (l+r)/2;
            if(rui[mid]-rui[i] > v[i]) r = mid;
            else l = mid;
        }
        cnt[r-1]--;
        cnt[i]++;
        res[r-1]+=v[i] - (rui[l]-rui[i]);
    }
    rep(i,n-1)cnt[i+1]+=cnt[i];
    rep(i,n){
        cout << res[i] + cnt[i] * t[i] << " ";
    }
    cout << endl;
}