...
void main()
{
	...
	for(int i=0;i<=n;++i) g[i].pb(n+5);
	int ans=0;
	for(int i=0;i<n;++i) {
		int x=a[i];
		if(x==p[0]) ans+=x!=p[1],p[1]=x;
		else if(x==p[1]) ans+=x!=p[0],p[0]=x;
		else {
			//replace any? how?
			//next occurence as early as possible!
			int w[2];
			for(int u=0;u<2;++u)
				w[u]=*lower_bound(g[p[u]].begin(),g[p[u]].end(),i);
			if(w[0]<w[1]) ans+=x!=p[0],p[0]=x;
			else ans+=x!=p[1],p[1]=x;
		}
	}
	cout<<ans<<"\n";
}
