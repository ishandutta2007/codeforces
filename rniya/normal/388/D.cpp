...
int main() {
    ...
    while (d.size() < MAX_LOG) d.emplace_back(0);

    vector<mint> power(MAX_LOG + 1);
    power[0] = 1;
    for (int i = 0; i < MAX_LOG; i++) power[i + 1] = power[i] * 2;
    dp[MAX_LOG][0][1] = 1;
    ...
    for (int i = MAX_LOG - 1; i >= 0; i--) {
        for (int j = 0; j <= MAX_LOG; j++) {
            for (int k = 0; k < 2; k++) {
                ...
                if (!j) {
                    if (!k) {
                        dp[i][j][0] += val;      // not add base
                        dp[i][j + 1][0] += val;  // add base
                    } else {
                        // not add base
                        if (d[i] == 1)
                            dp[i][j][0] += val;
                            ...
                            dp[i][j][1] += val;
                        if (d[i] == 1) dp[i][j + 1][1] += val;  // add base
                    }
                    continue;
                }
                if (!k) {
                    dp[i][j][0] += val * power[j];  // not add base
                    dp[i][j + 1][0] += val;         // add base
                } else {
                    // not add base
                    dp[i][j][1] += val * power[j - 1];
                    if (d[i]) dp[i][j][0] += val * power[j - 1];
                    // add base
                    if (d[i]) dp[i][j + 1][1] += val;
                }
            }
        }
    }
