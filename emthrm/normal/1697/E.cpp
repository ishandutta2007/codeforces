...
int main() {
  ...
  REP(i, n) {
    ...
    REP(j, n) {
      if (j == i) continue;
      const int d = abs(x[i] - x[j]) + abs(y[i] - y[j]);
      if (chmin(nearest, d)) {
        graph[i] = {j};
      } else if (nearest == d) {
        graph[i].emplace(j);
  ...
  REP(i, n) {
    ...
    auto f = [&](auto&& f, int ver) -> void {
      can_reach.emplace(ver);
      for (int e : graph[ver]) {
        if (!can_reach.count(e)) f(f, e);
      }
    };
    f(f, i);
    bool is_valid = true;
    for (int j : can_reach) {
      for (int k : can_reach) {
        is_valid &= j == k || graph[j].count(k);
      }
    }
    if (is_valid && can_reach.size() > 1) {
      for (int ver : can_reach) is_visited[ver] = true;
      vector<ModInt> nxt(n + 1, 0);
      REP(j, n) {
        nxt[j + 1] += dp[j];
        if (j + can_reach.size() <= n) nxt[j + can_reach.size()] += dp[j];
      }
      dp.swap(nxt);
    } else {
      is_visited[i] = true;
  ...
  ModInt ans = 0;
  REP(i, n + 1) ans += dp[i] * ModInt::nPk(n, i);
  cout << ans << '\n';
  return 0;
}