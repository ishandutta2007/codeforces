...
int main() {
    ...
    vi sorted = a; sort(all(sorted));

    auto count = [&](ll i, ll j) {
        if (i >= j) return 0;
        return int(lb(all(sorted), j) - lb(all(sorted), i));
    };

    ll ans = 0;

    F0R (i, N) {
        ll first = ((a[i] + X - 1) / X) * X;
        ll last = first + K * X - 1;

        // partner should be in range (last - X, last]
        ans += count(max(last - X + 1, a[i] + 1ll), last + 1);
        if (last - X < a[i] && a[i] <= last) ans += count(a[i], a[i] + 1);
    }
