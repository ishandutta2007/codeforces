...
void tour(pt x) { // print values of nodes, 
...
void solve(){
    ...
    pt nodes = new tnode(0, 0);

    for(int i = 1; i <= m; i++){
        // dbg(i);
        // tour(nodes);
        pair<pt, pt> split_by_v = split(nodes, y[i]);
        pair<pt, pt> single_out = split(split_by_v.f, y[i]-1);
        int adding_one = 1;
        if(single_out.s){
            if(on[single_out.s->id]){
                adding_one = 0;
                on[single_out.s->id] = 0;
            }
        }
        ...
        nodes = merge(merge(merge(single_out.f, single_out.s), new tnode(i, y[i])), split_by_v.s);
        on[i] = 1;
