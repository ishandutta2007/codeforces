		...
		else
		{
			...
			for (int i = 0; i < n; ++i)
				if (sz(g[i]) == 1)
				{
					...
					ost.pb(i);
			bool white_not_leaf = false;	//
			for (int i = 0; i < n; ++i)
				white_not_leaf = white_not_leaf || ((leaf[i] == false) && w[i]);
			bool szost = sz(ost);			//
			bool bad3 = false;				//
			...
			if ((sz(st3) == 2) && (n % 2 == 1))
				badparity = true;
			if ((sz(st3) == 1) && (wh == 1) && (n % 2 == 0))
				badparity = true;
			if ((sz(st3) == 0) && (wh == 2) && (n % 2 == 1))
				badparity = true;

			if (white_not_leaf || bad3 || szost || badparity)
				cout << "White\n";
			else
				cout << "Draw\n";
		}
