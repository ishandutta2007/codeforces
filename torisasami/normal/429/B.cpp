...
int main() {
    ...
    rep(i, n) rep(j, m) {
        if (i > 0)
            chmax(dp1[i][j], dp1[i - 1][j] + a[i][j]);
        if (j > 0)
            chmax(dp1[i][j], dp1[i][j - 1] + a[i][j]);
    }
    rep2(i, n) rep(j, m) {
        if (i + 1 < n)
    ...
    rep(i, n) rep2(j, m) {
        if (i > 0)
            chmax(dp3[i][j], dp3[i - 1][j] + a[i][j]);
        if (j + 1 < m)
            chmax(dp3[i][j], dp3[i][j + 1] + a[i][j]);
    }
    rep2(i, n) rep2(j, m) {
        if (i + 1 < n)
            chmax(dp4[i][j], dp4[i + 1][j] + a[i][j]);
        if (j + 1 < m)
            chmax(dp4[i][j], dp4[i][j + 1] + a[i][j]);
    }
    ll ans = 0;
    REP(i, 1, n - 1) REP(j, 1, m - 1) {
        chmax(ans, dp1[i - 1][j] + dp4[i + 1][j] + dp2[i][j - 1] + dp3[i][j + 1]);
        chmax(ans, dp1[i][j - 1] + dp4[i][j + 1] + dp2[i + 1][j] + dp3[i - 1][j]);
    }
    cout << ans << endl;
}