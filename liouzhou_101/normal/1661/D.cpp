...
void modify(int k, int L, int R, int x, int y, ll add, ll arith)
{
	if (L == x && R == y)
	{
		tree[k].add += add;
	...
	int m = (L + R) / 2;
	if (y <= m)
		modify(k << 1, L, m, x, y, add, arith);
	else if (x > m)
		modify(k << 1 | 1, m + 1, R, x, y, add, arith);
	else
	{
		modify(k << 1, L, m, x, m, add, arith);
		modify(k << 1 | 1, m + 1, R, m + 1, y, add + (m + 1 - x) * arith, arith);
	}
}
...
ll query(int k, int L, int R, int x)
{
	if (L == R)
		return tree[k].add;
	int m = (L + R) / 2;
	ll tmp = tree[k].add + tree[k].arith * (x - L);
	if (x <= m)
		return tmp + query(k << 1, L, m, x);
	else
		return tmp + query(k << 1 | 1, m + 1, R, x);
}
...
void solve()
{
	...
	for (int i = n - 1; i >= k; --i)
	{
		ll tmp = query(1, 0, n - 1, i);
		if (tmp < a[i])
		{
			ll need = (a[i] - tmp - 1) / k + 1;
			res += need;
			modify(1, 0, n - 1, i - k + 1, i, need, need);
		}
	}
