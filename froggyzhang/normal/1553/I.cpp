...
Matrix Solve(int L,int R){
	...
	if(L==R){
		if(a[L]==1){
			ans.g[0][0]=ans.g[1][0]=poly({0,1});
			ans.g[0][1]=poly({2}),ans.g[1][1]=poly({1});	
		}
		else{
			ans.g[0][0]=ans.g[1][0]=poly({0});
			ans.g[0][1]=poly({2,2}),ans.g[1][1]=poly({1,2});	
		}
		return ans;
	}
	int mid=(L+R)>>1;
...
int main(){
	...
	n=_n;
	Matrix A;
	if(a[1]==1)A.g[0][0]=poly({0,1});
	else A.g[0][1]=poly({0,2});
	if(n>1)A=A*Solve(2,n);
	/*
	dp[1][a[1]==1?0:1][1]=(a[1]==1?1:2);
	#define update(x,y) x=(x+y)%mod
	for(int i=2;i<=n;++i){
		memset(dp[i&1],0,sizeof(dp[i&1]));
		if(a[i]==1){
			for(int j=1;j<=i;++j){
				update(dp[i&1][0][j+1],(dp[(i-1)&1][0][j]+dp[(i-1)&1][1][j])%mod);
				update(dp[i&1][1][j],((dp[(i-1)&1][0][j]<<1)%mod+dp[(i-1)&1][1][j])%mod);
			}
		}
		else{
			for(int j=1;j<=i;++j){
				update(dp[i&1][1][j+1],(((dp[(i-1)&1][0][j]+dp[(i-1)&1][1][j])%mod)<<1)%mod);
				update(dp[i&1][1][j],((dp[(i-1)&1][0][j]<<1)%mod+dp[(i-1)&1][1][j])%mod);
			}
		}
	}
	*/
	...
	int ans=0;
	for(int i=1;i<=n;++i){
		ans=(ans+1LL*((n-i)&1?mod-fac[i]:fac[i])*(A.g[0][0][i]+A.g[0][1][i]))%mod;
	}
	printf("%d\n",ans);
	return 0;
}