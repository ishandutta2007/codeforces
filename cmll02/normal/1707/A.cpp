...
#include <map>
int a[1000005];
int check(int n,int m,std::map<int,int>mp,int q)
{
	for(int i=1;i<=n;i++)
	{
		if(i<m)
...
signed main()
{
	initprog();
	int T=read();rg(T)
	int n=read(),s=0,q=read();
	std::map<int,int>mp;
	rg(n)a[i]=read(),mp[a[i]]++;gr
	int l=1,r=n,ans;
	while(l<=r)
	{
		int m=l+r>>1;
		if(check(n,m,mp,q))ans=m,r=m-1;
		else l=m+1;
	}
	// odl(ans);
	rg(ans-1)od(q>=a[i]);gr
	for(int i=ans;i<=n;i++)od(1);
	puts("");
	gr
	return 0;
}