...
fun main(){
    solve.cases{
        ...
        val cmp = IntArray(n){(A[it] <= B[it]).chi}
        val cmpm1 = IntArray(n){if(it == 0) nlarge else (A[it] <= B[it-1]).chi}
        val cmpp1 = IntArray(n){if(it == n-1) nlarge else (A[it] <= B[it+1]).chi}
        ...
        fun ok(from:Int, to:Int):Boolean{

            if(A[from] > B[to]) return false
            val t1 = minOf(from,to) - 1
            if(Q.sumQuery(0,t1).toInt() != t1 + 1){
                return false
            }
            val t2 = maxOf(from,to) +1
            if(Q.sumQuery(t2,n-1).toInt() != (n-1)-(t2)+1){
                return false
            }
            if(from == to){
                return true
            ...
            if(from < to){
                val one = from + 1
                val two = to
                if(Qm.sumQuery(one,two).toInt() != (two - one)+ 1 ){
                    return false
                }
            }else{
                val one = to
                val two = from - 1
                if(Qplus.sumQuery(one,two).toInt() != (two - one)+1){
                    return false
                }
            }
            return true
...
//        }
        ...
        for(i in 0 until n){
            val max = BinarySearchLastTrue(i,n-1){ok(i,it)}!!
            val min = BinarySearchFirstTrue(0,i){ok(i,it)}!!
            ret1[i] = B[min] - A[i]
            ret2[i] = B[max] - A[i]
        }
