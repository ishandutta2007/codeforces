...
#line 4 "/home/maspy/compro/library/graph/functional.hpp"
...
void solve() {
  ...
  VEC(ll, A, N);
  ll MOD = 2520;
  auto IDX = [&](int v, int x) -> int {
    x %= MOD;
    if (x < 0) x += MOD;
    return 2520 * v + x;
  };
  FunctionalGraph<int> G(N * MOD);
  FOR(v, N) {
    LL(M);
    VEC(ll, TO, M);
    for (auto&& x: TO) --x;
