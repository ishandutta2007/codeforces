...
struct S {
    ...
    friend bool works(const S &l, const S &r) { return l.h == r.h || !l.len || !r.len; }

    friend S operator+(const S &l, const S &r) {
        assert(works(l, r));

        return {l.len ? l.h : r.h, l.len != r.len};
    }

    friend void pr(const S &s) { pr("[", s.h, ", ", s.len, "]"); }
};
