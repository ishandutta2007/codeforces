...
bool ask(const vector<int> &vs){
	...
	cauto re=(cur==(count(all(vs),N)>0));
	prv=cur;
	// out(vs,re,cur,1);
	return re;
#endif
	cout<<"? "<<vs.size();
	for(cauto &v:vs) cout<<" "<<v;
	cout<<endl;
	cout.flush();
	return read<string>()=="YES";
}

void solve(const int n){
	vv<int> bits(17);
	rep(d,bits.size())reps(i,1,n+1)if(i>>d&1) bits[d].eb(i);
	int re=0;
	int root=-1;
	int tmpa=0;
	int tmpb=0;
	rep(d,17){
		if(root<0){
			...
			if(a==b){
				if(a) re|=1<<d;
				if((ask(bits[root]))==(tmpa>>root&1) && (ask(bits[root]))==(tmpa>>root&1)){
					out("fix",tmpa,tmpa|tmpb,1);
					re|=tmpa;
					root=-1;
					tmpa=tmpb=0;
				}
				...
				if((ask(bits[root]))==(tmpa>>root&1)){
					...
					out("fix",tmpb,tmpa|tmpb,1);
					re|=tmpb;
					tmpa=tmpb=0;
					root=d;
					if(a) tmpa|=1<<d;
					if(b) tmpb|=1<<d;
					if(ask(bits[d])==(tmpa>>d&1) && ask(bits[d])==(tmpa>>d&1)){
						out("fix",tmpa,tmpa|tmpb,1);
						re|=tmpa;
						root=-1;
						tmpa=tmpb=0;
