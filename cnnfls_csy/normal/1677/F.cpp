...
namespace polynomial
{
    ...
    struct poly
    {
        ...
        inline void derivation(){int i;fz0k(i,((int)v.size())-1)v[i]=1ll*v[i+1]*(i+1)%mod;v.pop_back();}
        inline void integral(){int i;v.push_back(0);fd1(i,v.size()-1)v[i]=1ll*v[i-1]*basic::inv[i]%mod;v[0]=0;}
...
int n,k,p,i,j,a[100005],pre[100005][2],suf[100005][2],coef[100005],ans;
int main()
{
	basic::init();
	read(n);read(k);read(p);fz1(i,n)read(a[i]);
	pre[0][0]=1;
	fz1(i,n){
	...
	fz1(i,n) coef[i]=(1ll*pre[i-1][0]*suf[i+1][0]+1ll*pre[i-1][1]*suf[i+1][0]+1ll*pre[i-1][0]*suf[i+1][1])%mod;

	poly f,g;
	fz0g(i,k) g[i]=(1ll*p*basic::fi[i]-(i==0))%mod;
	g=poly_inv(g,k+1);
	fz0g(i,k) f[i]=1ll*g[k-i]*basic::fi[i]%mod;
	vector<int> v;fz1(i,n) v.push_back(a[i]+1);
	v=poly_qiuzhi(v,f);
	fz1(i,n){
		int res=(1ll*qp(p,a[i]+1)*v[i-1]-g[k]+mod)%mod*basic::fac[k]%mod;
		ans=(ans+1ll*res*coef[i])%mod;
	}
	printf("%d\n",ans);
	return 0;
}