...
void solve() {
    cin >> n >> q;
    read(a, n); read(b, n);
    d.rsz(n); F0R(i, n) d[i] = a[i] - b[i];
    p.rsz(n+1, 0); F0R(i, n) p[i+1]=p[i]+d[i];
    st1.init(n+1); F0R(i, n+1) st1.upd(i, p[i]);
    st2.init(n+1); F0R(i, n+1) st2.upd(i, -p[i]);
    F0R(query, q) {
        cin >> l >> r;
        if (p[r] != p[l-1]) {
            cout << -1 << nl;
        ...
        ll mx = st1.query(l, r), mn = -st2.query(l, r);
        if (mx > p[l-1]) {
            cout << -1 << nl;
            continue;
        }
        cout << max(abs(p[l-1]-mx), abs(p[l-1]-mn)) << nl;
    }
}
