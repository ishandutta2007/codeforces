...
int main() {
    ...
    if (sum % N != 0) { cout << 0 << endl; return 0; }
    ...
    if (w == 0 || l == 0) { cout << 1 << endl; return 0; }
    ...
    ll L = l, W = w;
    for (auto p : mp) {
        if (p.first < ave) { ans *= com(l, p.second); l -= p.second; }
        else if (p.first == ave) { ans *= com(N, p.second); }
        else { ans *= com(w, p.second); w -= p.second; }
    }

    if (W == 1 || L == 1) { ans *= max(W + 1, L + 1); }
    else { ans *= 2; }
    cout << ans.val() << endl;
}