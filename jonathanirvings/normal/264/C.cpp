...
int main()
{
	...
	TC(q)
	{
		...
		{
			LL res = (LL)b * (LL)v[i];
			MAX(res,dp[c[i]] + (LL)a * (LL)v[i]);
			//VALUE(res);
			//REPN(j,n) if(j != c[i]) MAX(res,dp[j] + (LL)b * (LL)v[i]);
			if (big.fi != -1)
			{
				if (big.fi == c[i])
				{
					if (big.se != -1) MAX(res,dp[big.se] + (LL)b * (LL)v[i]);
				} else MAX(res,dp[big.fi] + (LL)b * (LL)v[i]);
			}
			MAX(dp[c[i]],res);
			if (big.fi == -1)
			{
			...
			if (big.fi != c[i])
			{
				if (res > dp[big.fi])
				{
					big.se = big.fi;
					big.fi = c[i];
				} else if (big.se == -1 || res > dp[big.se]) big.se = c[i];
			}
			MAX(risan,res);
		}
		cout << risan << endl;
