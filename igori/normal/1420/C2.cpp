...
int res(int pos, vi &a)
{
    ...
    if (pos == a.size() - 1)
    {
        if (a[pos] > a[pos - 1]) return a[pos];
        return 0;
    }
    if (a[pos] > a[pos - 1] && a[pos] > a[pos + 1]) return a[pos];
    if (a[pos] < a[pos - 1] && a[pos] < a[pos + 1]) return -a[pos];
    return 0;
}
