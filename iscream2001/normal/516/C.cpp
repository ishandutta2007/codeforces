...
Segment operator + (Segment x,Segment y){
	...
	re.mn=max(x.mn,y.mn);
	re.mx=max(x.mx,y.mx);
	re.e=max(max(x.e,y.e),x.mn+y.mx);
	return re;
}
...
Segment Ask(int id,int l,int r,int ql,int qr){
	Segment re;
	re.mn=re.mx=re.e=-INF;
	if(ql>r||qr<l) return re; 
	if(ql<=l&&qr>=r){
		return d[id];
	}
	int mid=(l+r)>>1;
	re=re+Ask(ls,l,mid,ql,qr);
	re=re+Ask(rs,mid+1,r,ql,qr);
	return re;
} 
void MAIN(){
	int u,v;
	...
	while(Q--){
		cin>>u>>v;
		if(u>v){
			tmp=Ask(1,1,n+n,v+1,u-1);
		}
		else{
		//	cout<<v+1<<" "<<u-1+n<<endl;
			tmp=Ask(1,1,n+n,v+1,u-1+n);
		}
		cout<<tmp.e<<"\n";
	}
	return;
