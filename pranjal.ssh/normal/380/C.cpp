...
{
    node ret;
    ret.ans=a.ans+b.ans+min(a.x,b.y);
    ret.x=a.x+b.x-min(a.x,b.y);
    ret.y=a.y+b.y-min(a.x,b.y);
    return ret;
}
...
node NIL;
node query(int no,int l,int r,int u,int v)
{
    if(r<u or l>v)return NIL;
    if(l>=u and r<=v) return t[no];
    return merge(query(2*no,l,mid,u,v),query(2*no+1,mid+1,r,u,v));
}
int main()
{
    boost;
