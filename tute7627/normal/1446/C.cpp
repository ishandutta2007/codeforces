...
int main(){
  ...
  auto dfs=[&](auto &&f,vector<ll>&b)->ll{
    if(b.size()==1)return 1LL;
    ll sz=31;
    vector<vector<ll>>v(sz);
    for(auto z:b)v[high(z)].PB(z);
    vector<ll>ret;
    vector<ll>g;
    rep(i,0,sz)if(v[i].size()>=1)g.PB(i);
    rep(i,0,g.size()){
      vector<ll>tmp;
      for(auto z:v[g[i]]){
        if(g[i]>0)z^=1<<g[i]-1;
      ...
      ret.PB((ll)g.size()-1-max(0LL,i-1)+f(f,tmp));
    }
    return *max_element(ALL(ret));
  };
  cout<<n-dfs(dfs,a)<<endl;
  return 0;
}