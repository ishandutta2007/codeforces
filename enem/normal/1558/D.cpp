...
int main()
{
    ...
    fc[0] = 1LL;
    REP(i,1,N) fc[i] = (fc[i-1]*i)%M2;
    ifc[N-1] = modI(fc[N-1], M2);
    REPI(i,0,N-1) ifc[i] = (ifc[i+1]*(i+1))%M2;
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REPI(i,0,m)
        {
            a[i].S = (*p.find_by_order(a[i].S));
            p.erase(p.find(a[i].S));
            del.PB(a[i].S);
            if(ff.find(a[i].S+1) != ff.end()) x--;
            ff.insert(a[i].S);
        }
        for(auto it:del) p.insert(it);
        del.clear();

        cout<<((fc[2*n-x-1]*((ifc[n-x-1]*ifc[n])%M2))%M2);
