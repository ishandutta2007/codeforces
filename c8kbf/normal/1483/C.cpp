...
_ n, a[maxn], b[maxn], f[maxn], g[maxn];
stack<_> st;
multiset<_, greater<_> > mn;

inline void AC() {
    
    n = read();
    for(_ i = 1; i <= n; ++i) f[i] = g[i] = -1E11;
    ...
    for(_ i = 2; i <= n; ++i) {
        for(; !st.empty() && a[i] < a[st.top()]; st.pop()) {
            mn.erase(mn.find(g[st.top()]+b[st.top()]));
            f[i] = max(f[i], g[st.top()]+b[i]);
            g[i] = max(g[i], g[st.top()]);
        }
        if(!mn.empty()) f[i] = max(f[i], *mn.begin());
        g[i] = max(g[i], f[i-1]);
        f[i] = max(f[i], g[i]+b[i]);
        st.push(i);
        mn.insert(g[i]+b[i]);
