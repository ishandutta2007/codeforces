...
void solve_case() {
    ...
    f1r(i, 1, n) re(a[i]);
    int mx = 0;
    function<void(int, int)> dfs_process = [&](int u, int p) {
        level[dep[u]].eb(u);
        each(v, adj[u]) {
            dep[v] = dep[u] + 1;
            ckmax(mx, dep[v]);
    ...
    auto get_best = [&](int u) -> ll {
        return max(abs(a[u]-a[level[dep[u]][0]]), abs(a[u]-a[level[dep[u]].back()]));
    };

    vl dp(n);
    vl go(n); // transition
    f1r(d, 1, mx+1) { // cur dep, push
        int sz = sz(level[d]);
        ...
        f0r(i, sz) {
            int u = level[d][i];
            go[u] = max(pre[i] + a[u], suf[i] - a[u]);
        }

        each(u, level[d]) {
            dp[u] = max(dp[par[u]] + get_best(u), go[u]);
        }
    }
    ll ans = 0;
    each(e, dp) ckmax(ans, e);
    ps(ans);
