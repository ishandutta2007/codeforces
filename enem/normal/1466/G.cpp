...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(ttt,0,q)
        {
            ...
            ll ans=0,t=0;
            ll ns=s.size();
            bool left[nv],right[nv];
            REP(i,0,nv) left[i]=right[i]=false;
            ...
            {
                t=lps2[i-1];
                while(t!=0 && w[nv-1-i]!=w[nv-1-t])
                {
                    t=lps2[t-1];
                }
                if(w[nv-1-i]==w[nv-1-t]) lps2[i]=t+1;
                else lps2[i]=t;
            }
            ...
            REP(i,0,ns)
            {
                while(t!=0 && w[nv-1-t]!=s[ns-1-i])
                {
                    t=lps2[t-1];
                }
                if(w[nv-1-t]==s[ns-1-i])
                {
                    t++;
                }
                if(t==nv)
            ...
            REP(i,0,26)
            {
                ...
                REP(j,0,nv)
                {
                    if(w[j]==((char)(i+'a')) && right[j] && left[nv-1-j])
                    {
                        t++;
                    }
                }

                ans+=(( (( (( dp[k][i]+M1-dp[pt][i] )%M1) * modI(powM(2,n-k,M1),M1) )%M1) * t )%M1);
                ans%=M1;
            }
