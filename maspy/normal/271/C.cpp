...
void solve() {
  LL(N, K);
  if (N < 3 * K) return print(-1);

  auto check = [&](vi A) -> bool {
    vc<vi> IDX(K);
    FOR(i, N) IDX[A[i] - 1].eb(i);
    FOR(x, K) {
      bool ng = [&]() {
        auto I = IDX[x];
        FOR(i, I.size() - 2) {
          ll a = I[i], b = I[i + 1], c = I[i + 2];
          if (a + c != b + b) return false;
        }
        return true;
      }();
      if (ng) return false;
  ...
  vi ANS(N);
  FOR(i, N) ANS[i] = min(K, (i / 3) + 1);
  mt19937 engine;
  while (!check(ANS)) shuffle(all(ANS), engine);
  print(ANS);
}
