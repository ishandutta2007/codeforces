...
int main() {
	...
	Poly ecyc = zeroes(n);
	{
		...
		p[0] = 1;
		for (int i = 1; i <= n / 2; ++i)
			p[i] = p[i - 1] * (ull)(m + i) % P * (m - i + 1) % P;
		for (int i = 1; i <= n / 2; ++i) p[i] = p[i] * (ull)simp.gifac(i * 2) % P;
		...
		for (int i = 1; i <= n / 2; ++i) ecyc[i * 2] = q[i];
	}
	...
	if (m % 2 == 0) {
		...
		p[0] = 1;
		for (int i = 1; i <= n / 2; ++i)
			p[i * 2] = p[(i - 1) * 2] * (ull)(m + i) % P * (m - i + 1) % P;
		for (int i = 1; i <= n / 2; ++i) p[i * 2] = p[i * 2] * (ull)simp.gifac(i * 2) % P;
		for (int i = 1; i <= n / 2; i += 2) p[i * 2] = norm(P - p[i * 2]);
		q[1] = m;
		for (int i = 1; i <= (n - 1) / 2; ++i) q[i * 2 + 1] = q[i * 2 - 1] * (ull)(m + i) % P * (P - m + i) % P;
		for (int i = 1; i <= n; i += 2) q[i] = q[i] * (ull)simp.gifac(i) % P;
		q = q.quo(p);
	} else {
		m /= 2;
		p[0] = 1;
