...
int main(){
  ...
  while(q--){
    ...
    db.build();
    vector<ll>ret(k);
    rep(i,0,k)ret[i]=db.query1(i);
    string ns=string(n,'#');
    rep(i,0,n){
      ll st=min(i,k-1);
      ll num=max(0LL,i-(k-1));
      if(ret[st]==-1||ret[st]+num>n-k+1){
        ns[n-k+db.query2(st,n-k+1-num)+1]=s[i];
      }
      else{
        //cout<<i spa num spa st<<endl;
        ns[num+ret[st]-1]=s[i];
      }
    }
    //debug(ret,k);
