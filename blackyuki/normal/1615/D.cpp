...
public:
    ...
    potential_UF(ll n):data(n,1),dif(n){}
    bool merge(ll X,ll Y,ll d){
        ll x=root(X),y=root(Y);
        if(data[x]>data[y]){swap(x,y);swap(X,Y);}
        if(x==y){
            if(num(Y)^num(X)!=d)return false;
            return true;
        }
        dif[x]=num(Y)^num(X)^d;
...
int main(){
    ...
    rep(tt,t){
        ...
        rep(i,n-1){
            ...
            v[i]=c;
            if(c!=-1){
                if(!uf.merge(a,b,__builtin_parity(c)))ok=false;
            }
        }
        rep(i,m){
            ll a,b,c;cin>>a>>b>>c;a--;b--;
            if(!uf.merge(a,b,c))ok=false;
        }
        if(ok){
            out("YES");
            rep(i,n-1){
                ...
                if(v[i]==-1){
                    cout<<(uf.num(edge[i].fi)^uf.num(edge[i].se))<<endl;
                }
                else out(v[i]);
            }
        }
