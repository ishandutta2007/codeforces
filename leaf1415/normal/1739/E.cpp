...
int main(void)
{
	...
	rep(i, 0, 1) rep(j, 1, n){
		cin >> c[i][j];
		if(c[i][j] == '1') S[i].insert(j);
	}
	S[0].insert(inf), S[1].insert(inf);
	
	rep(i, 1, n) rep(j, 0, 1) rep(k, 0, 1) rep(l, 0, 1) rep(m, 0, 1) dp[i][j][k][l][m] = -inf;
	dp[1][0][0][0][0] = 0;
	...
	rep(i, 1, n){
		rep(j, 0, 1){
			rep(k, 0, 1){
				rep(l, 0, 1){
					rep(m, 0, 1){
						auto it = S[m].upper_bound(i);
						if(*it == i+1 && k == 1) it++;
						ll p = *it;
						it = S[1-m].lower_bound(i);
						if(*it == i && j == 1) it++;
						if(*it == i+1 && l == 1) it++;
						ll q = *it;
						
						if(p <= n && p <= q+1){
							if(p == i+1) chmax(dp[p][l][0][0][m], dp[i][j][k][l][m]+1);
							else chmax(dp[p][0][0][0][m], dp[i][j][k][l][m]+1);
							//chmax(dp[p][1][0][0][1-m], dp[i][j][k][l][m]);
						}
						if(q <= n && q+1 <= p){
							if(q == i) chmax(dp[q][1][l][1][1-m], dp[i][j][k][l][m]+1);
							else if(q == i+1) chmax(dp[q][k][0][1][1-m], dp[i][j][k][l][m]+1);
							else chmax(dp[q][0][0][1][1-m], dp[i][j][k][l][m]+1);
							if(q == i) chmax(dp[q][1][k][l][m], dp[i][j][k][l][m]);
							if(q >= i+1) chmax(dp[q][1][0][0][m], dp[i][j][k][l][m]);
						}
					}
				}
			}
	...
	ll ans = 0;
	rep(i, 1, n) rep(j, 0, 1) rep(k, 0, 1) rep(l, 0, 1) rep(m, 0, 1) chmax(ans, dp[i][j][k][l][m]);
	outl(ans);
