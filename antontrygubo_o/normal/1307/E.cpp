...
int main() {
    ...
    vector<vector<pair<int, int>>> cow(n+1);
    for (int i = 0; i<m; i++)
    {
        int f, h;
        cin>>f>>h;
        if (pos[f].size()<h) continue;
        cow[f].push_back(mp(pos[f][h-1], pos[f][pos[f].size() - h]));
    }
    for (int i = 1; i<=n; i++) sort(cow[i].begin(), cow[i].end());

    bool empty = true;
    for (int i = 1; i<=n; i++) if (cow[i].size()>0) empty = false;
    if (empty) {cout<<0<<' '<<1; return 0;}

    int maxx = 0;
    int cursum = 1;
    for (int i = 1; i<=n; i++) if (cow[i].size()>0)
    {
        maxx++; cursum = mul(cursum, cow[i].size());
    }


    //for (auto it: cow[1]) cout<<it.first<<' '<<it.second<<endl;
    ...
    for (int i = 1; i<=n; i++)
    {
        for (auto it: cow[i])
        {
            ...
            int temp = 0;
            for (auto it1: cow[i]) if (it1!=it && it1.second>it.first)  temp++;
            //cout<<it.first<<' '<<it.second<<' '<<temp<<endl;
            if (temp>0)
            {
                cows++; cnt = mul(cnt, temp);
            }
            for (int j = 1; j<=n; j++) if (j!=i)
            {
                ...
                for (auto x: cow[j])
                {
                    if (x.first<it.first) left++;
                    if (x.second>it.first) right++;
                }

                if (left==0 && right==0) continue;
                if (left==0 || right == 0)
                {
                    cows++; cnt = mul(cnt, left + right);
                    continue;
                }
                if (left==1 && right==1)
                {
                    cows++; cnt = mul(cnt, 2); continue;
                }
                cows+=2;
                if (left>right) swap(left, right);
                cnt = mul(cnt, left*(right-1));
            }
            //cout<<cows<<' '<<cnt<<endl;
            if (cows==maxx) cursum = add(cursum, cnt);
            else if (cows>maxx) {maxx = cows; cursum = cnt;}
        }
    }
    cout<<maxx<<' '<<cursum;
