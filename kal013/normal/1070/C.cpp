...
map<int,int> core; // i:c <=c[i];
set<pii> use,avail; //p[i],i
vi add[maxn],del[maxn];
void solve(){
    int n,k,m;read(n);read(k);read(m);
    For(i,m){
        read(l[i]);read(r[i]);read(c[i]);read(p[i]);
        add[l[i]].pb(i);
        del[r[i]+1].pb(i);
    }
    int cores=0,cost=0,ans=0;
    for(int i=1;i<=n;++i){
        for(auto x:del[i]){
            auto it=core.find(x);
            ...
            auto it1=use.find({p[x],x}),it2=avail.find({p[x],x});
            if (it1!=use.end()){
                use.erase(it1);
            }
            if (it2!=avail.end()) avail.erase(it2);
            
        }
        
        while (cores<k && !avail.empty()){
            auto it=avail.begin();
            auto X=(*it);
            int j=X.S;
            ...
            int u=min(c[j]-core[j],k-cores);
            core[j]+=u;
            cores+=u;
            cost+=u*p[j];
            if (core[j]==c[j]) {avail.erase(it);}
        }
        
        for(auto x:add[i]){
            if (cores<k){
                core[x]=min(c[x],k-cores);
                cores+=core[x];
                cost+=core[x]*p[x];
                use.insert({p[x],x});
                if (core[x]==c[x]) continue;
            }
            
            int rem=(c[x]);
            if (core.find(x)!=core.end()) rem-=core[x];

            while (rem && !use.empty()){
                auto it1=use.end();
                it1--;
                auto X=(*it1);
                if (X.S==x || X.F<=p[x]) break;
                if (core[X.S]<=rem){
                    rem-=core[X.S];
                    cost-=(core[X.S]*(p[X.S]-p[x]));
                    
                    core[x]+=core[X.S];
                    auto it=core.find(X.S);
                    core.erase(it);
                    avail.insert({p[X.S],X.S});
                    use.erase(it1);
                }
                else{
                    core[X.S]-=rem;
            ...
            // trace(rem,x,use,avail,core[x],c[x]);
            if (rem>0){
                avail.insert({p[x],x});
            }
            if (rem<c[x]){
        ...
        while (cores<k && !avail.empty()){
            ...
            cores+=u;
            cost+=u*p[j];
            if (core[j]==c[j]) it=avail.erase(it);
        }
        ...
        // }
        ans+=cost;
        // trace(ans,cost,cores,use,avail,i,core);
    }
    d1(ans);
}
signed main(){
