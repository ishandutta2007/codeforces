fn main() {
    ...
    let mut r = vec![M::zero(); m + 1];
    for a in a {
        r[a] = M::one();
    }
    let ans = calc(r, k);
    use util::*;
    println!("{}", ans.iter().enumerate().filter(|p| !p.1.is_zero()).map(|p| p.0).join(" "));
}

fn mul(a: &[M], b: &[M]) -> Vec<M> {
...
fn calc(r: Vec<M>, k: usize) -> Vec<M> {
    if k == 1 {
        return r;
    }
    let mut v = calc(mul(&r, &r), k / 2);
    if k % 2 == 1 {
        v = mul(&v, &r);
    }
    v
