...
int main() {
    ...
    string S;
    cin >> N >> S;
    array<vector<int>, ALPHABET> locations;

    for (int i = 0; i < N; i++) {
        int c = S[i] - MIN_CHAR;
        locations[c].push_back(i);
    }
    ...
    for (int i = N - 1; i >= 0; i--) {
        int c = S[N - 1 - i] - MIN_CHAR;
        int from = locations[c].back();
        locations[c].pop_back();
        int actual = tree.query(from);
        total += abs(actual - i);
        tree.set(from, 0);
    }
