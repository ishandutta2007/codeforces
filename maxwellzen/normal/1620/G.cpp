...
void solve() {
    ...
    read(a, n);
    freq.rsz(n, vi(26, 0));
    F0R(i, n) for (char c : a[i]) freq[i][c-'a']++;
    vll dp(twon, 1); dp[0] = 0;
    FOR(i, 1, twon) {
        vi f(26, 1e9);
        F0R(j, n) if (i&1<<j) F0R(k, 26) ckmin(f[k], freq[j][k]);
        F0R(j, 26) dp[i] = (dp[i] * (f[j]+1))%MOD;
    }
    debug(dp);
    F0R(bit, n) FOR(i, 1, twon) if ((i&1<<bit)==0) dp[i] = (dp[i]+MOD-dp[i^1<<bit])%MOD;
    debug(dp);
    F0R(bit, n) FOR(i, 1, twon) if (i&1<<bit) dp[i] = (dp[i]+dp[i^1<<bit])%MOD;
    debug(dp);
    ll ans = 0;
    FOR(i, 1, twon) {
        ll cur = 0;
        F0R(j, n) if (i&1<<j) cur += j+1;
        cur *= __builtin_popcount(i);
        cur *= (dp[twon-1] + MOD - dp[(twon-1) & (~i)]) % MOD;
        ans ^= cur;
    }
    cout << ans << nl;
}
