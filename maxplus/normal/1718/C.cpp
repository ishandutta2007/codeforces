...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    vector<int> a(n);
    cin >> a;
    priority_queue<int64_t> uf, torem;
    vector<int> pd;
    vector<vector<int64_t>> ufv;
    for (int i = 2, m = n; m != 1; ++i) if (m % i == 0) {
      pd.push_back(i);
      do m /= i;
      while (m % i == 0);
      ufv.push_back(vector<int64_t>(n / i));
    }
    for (int i = 0; i < pd.size(); ++i)
    for (int r = n / pd[i], j = 0; j < r; ++j)
    for (int jj = j; jj < n; jj += r) {
      ufv[i][j] += a[jj];
    }
    auto rebuild = [&]() {
      getc(uf).clear();
    ...
    rebuild();
    while (print(uf.top()), q--) {
      int p = $ - 1, x = $;
      for (int i = 0; i < pd.size(); ++i) {
        int r = n / pd[i];
        int j = p % r;
        torem.push(ufv[i][j] * r);
        while (!torem.empty() && torem.top() == uf.top()) {
          torem.pop();
          uf.pop();
        }
        ufv[i][j] += x - a[p];
