...
#define tm baugeadpgmvap
int n,m,a[MAXN+10],tm,vis[MAXN+10],col[MAXN+10];
...
void dfs(int u){
	...
	for(node *p=adj[u];p;p=p->next){
		if(vis[p->v]!=tm){
			...
			if((p->wt==1&&col[p->v]!=col[u])||(p->wt==0&&col[p->v]==col[u])){
				int t=1;
				puts("NO");
				exit(0);
			}
...
int main()
{
	...
	for(i=1;i<=n;i++){
	//	printf("%d %d %d\n",pos[i][0],pos[i][1],a[i]);
		addedge(pos[i][0],pos[i][1],a[i]);
		addedge(pos[i][1],pos[i][0],a[i]);
	}
	for(i=1;i<=m;i++){
		if(!vis[i]){
			tm++;
