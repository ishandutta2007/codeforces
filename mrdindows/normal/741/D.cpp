...
S go(int cur) {
    ...
    for (int to : v[cur]) {
        ...
        for (auto p : f.m) {
            int key = p.first ^ f.xorAll ^ res.xorAll;
            int val = p.second + f.plusAll + res.plusAll;
            if (val + res.maxVal <= curAns) {
                continue;
            }
            for (int mask = 0; mask < (1 << 22);) {
                ...
                if (it != res.m.end()) {
                    curAns = max(curAns, val + it->second);
                }
                if (!mask) {
                    ++mask;
                } else {
