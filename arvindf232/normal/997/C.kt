...
//    solve.usetc()
    ...
    val shortpow = IntArray(1000002)
    shortpow[0] = 1
    for(i in 1..shortpow.lastIndex){
        shortpow[i] = 3 modM shortpow[i-1]
    }
    solve.cases{
		val n = getint
        val total = intPow(3, ((1L * n * n) % (pI - 1)).toInt(), pI)
        val perrow = intPow(3,n,pI) modMinus  3
        val norrow = intPow(perrow, n, pI)
        val havethatrow = total modMinus norrow

        var haveboth = 0
        for(p in 1..n){

            val easytypes = shortpow[p] modMinus 3
            val easyeach = intPow(3, ((1L * n * (n-p)) % (pI - 1)).toInt(), pI)

            val hardtypes = 3
            val hardeach = intPow(shortpow[n-p] modMinus 1, n, pI)

            val total = (easytypes modM easyeach )modPlus (hardtypes modM hardeach)
            val overallchoices = intPow(3, ((1L * n * n - 1L * (n-1) * p) % (pI - 1)).toInt(),pI)
            val actualexist = overallchoices modMinus total

//            p dei actualexist


            val del =  (if(p % 2 ==1 ) 1 else pI -1 ).modM(FACT.choose(n,p)) modM actualexist
            haveboth = haveboth modPlus del
        }

        put(havethatrow modPlus havethatrow modMinus haveboth )

    }
    done()
}
/*
always in the variable naming
always confused one variable def for another
 */