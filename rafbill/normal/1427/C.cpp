...
int main(){
  ...
  li n,r; cin>>r>>n;
  t[0] = 0; x[0] = 0; y[0] = 0; cnt[0] = 0;
  ans[0] = 0;

  for(li i = 0; i < (li)(n); ++i) {
    cin>>t[1+i]>>x[1+i]>>y[1+i];
    --x[1+i]; --y[1+i];
    cnt[i+1] = -100'000;
    int from = max(0ll, i-2*K);
    for(li j = (from); j <= (li)(i); ++j) {
      if(abs(x[i+1]-x[j]) + abs(y[i+1]-y[j]) <= t[i+1]-t[j]) {
        smax(cnt[i+1], cnt[j]+1);
      }
    }
    if(from != 0) {
      smax(cnt[i+1], ans[from]+1);
    }
    ans[i+1] = max(ans[i], cnt[i+1]);
  }
