...
int main() {
    ...
    F0R (i, C) mf.arc(SRC, CIT + i, 1);
    F0R (i, V) mf.arc(SEG + V + i, SNK, 1);

    F0R (c, C) {
        int u, v; re(u, v), --u, --v;
        hld.for_each_commutative(u, v, 0, [&](index_t _i, index_t _j) {
    ...
    vi cit, gua;
    F0R (i, C) if (mf.level[CIT + i]     == -1) cit.pb(i + 1);
    F0R (i, V) if (mf.level[SEG + V + i] != -1) gua.pb(tr.up_edge(hld.at_index({i})).id + 1);
