...
int solve() {
    ...
    cin >> s;
    int t = n / k;
    ll ra = powmod(a, mod - 2, mod);
    ll res = 0;
    ll an = powmod(a, n - 1, mod);
    ll ab = ra * 1LL * b % mod;
    ll tab = 1;
    for (int i = 0; i < k; ++i) {
        res = (res + (s[i] == '+' ? 1 : -1) * tab + mod) % mod;
        tab = tab * ab % mod;
    }
    //    DBN(res, an, tab);
    ll q = tab;
    if (q == 1) {
        ll ans = an * 1LL * res % mod * t % mod;
        cout << ans << endl;
    } else {
        ll qt1 = (powmod(q, t, mod) + mod - 1) % mod;
        //    DBN(qt1);
        ll q1 = (q + mod - 1) % mod;
        ll ans = an * 1LL * res % mod * qt1 % mod * powmod(q1, mod - 2, mod) % mod;
        cout << ans << endl;
    }
    return 0;
}