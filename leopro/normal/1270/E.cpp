...
struct p {
    ...
    friend istream &operator>>(istream &is, p &x) {
        is >> x.x >> x.y;
        return is;
    }
};
...
void solve() {
    ...
    vector<p> s(n);
    cin >> s;
    int k = count_if(s.begin(), s.end(), odd);
    while (k == 0 || k == n) {
        if (k == n) for (p &x : s)x.x--;
        while (count_if(s.begin(), s.end(), even) == n) {
            for (p &x:s) {
                x.x /= 2;
                x.y /= 2;
            }
        }
        k = count_if(s.begin(), s.end(), odd);
        if (!(k == 0 || k == n))continue;
        for (p &x:s) {
            int a = x.x + x.y;
            int b = x.x - x.y;
