...
mint dfs(int l,int r){
	...
	if(MX(vst[l][r],1)==0) return memo[l][r];
	int mn=MOD,mni;
	reps(i,l,r)if(MN(mn,a[i])) mni=i;
	mint sl=0,sr=0;
	out(l,mni+1,1);
	out(mni,r,1);
	reps(i,l,mni+1) sl+=dfs(l,i)*dfs(i,mni);
	reps(i,mni,r) sr+=dfs(mni+1,i+1)*dfs(i+1,r);
	out(sl,sr,1);
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	//iota(all(a),1);
	cout<<dfs(0,n)<<endl;
	// rep(i,n+1){rep(j,n+1) cout<<memo[i][j]<<",";cout<<endl;}
  return 0;
}