...
void bellmanford(int S){
	...
	while(js<=n+3){
		...
		bool flag=0;
		for(auto ed: A){
			if(dis[ed.l]+ed.w<dis[ed.r]){
				dis[ed.r]=dis[ed.l]+ed.w;
				flag=1;
			}
		}
		if(flag==0) break;
...
void MAIN(){
	...
	for(int i=1;i<=m;++i){
		...
		V[b[i].l].pb((edge){b[i].l,b[i].r,b[i].w});
		V[b[i].r].pb((edge){b[i].r,b[i].l,b[i].w});
	}
	bool flag=0;
	for(int i=1;i<=n;++i){
		if(vis[i]==0){
	...
	for(int i=1;i<=m;++i){
		if(b[i].w==1){
			A.pb((edge){b[i].l,b[i].r,1});
			A.pb((edge){b[i].r,b[i].l,-1});
		}
		else{
			A.pb((edge){b[i].l,b[i].r,1});
			A.pb((edge){b[i].r,b[i].l,1});
