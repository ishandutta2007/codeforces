...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		cauto m=read();
		cauto g=readG(n,m);
		vector<set<int>> neig(n);
		rep(v,n)for(cauto w:g[v]) neig[v].insert(w);
		vector<int> pr(n,-1);
		vv<int> others(n);
		set<int> unu;
		rep(i,n) unu.emplace(i);
		rep(v,n){
			for(auto it=unu.begin();it!=unu.end();++it){
				if(v==*it || neig[v].count(*it)) continue;
				pr[*it]=v;
				pr[v]=*it;
				unu.erase(it);
				break;
			...
			if(pr[v]>=0) continue;
			rep(w,n)if(w!=v && neig[v].count(w)==0){
				cauto a=pr[w];
				if(others[a].empty()){
					others[w].eb(v);
					pr[v]=w;
				...
				cauto b=others[a].back(); others[a].pop_back();
				pr[b]=a;
				pr[a]=b;
				pr[w]=v;
				pr[v]=w;
		...
		rep(v,n){
			...
			out(v,pr[v],others[v],others[pr[v]],1);
			if(others[v].size() ||
				 others[pr[v]].empty() && v<pr[v]){
				p.eb(v);
				p.eb(pr[v]), q.eb(pr[v]);
				for(cauto w:others[v]) p.eb(w), q.eb(w);
				q.eb(v);
			}
		}
		p=getInv(p);
		q=getInv(q);
		for(cauto x:p) cout<<x+1<<" ";
		cout NL;
		for(cauto x:q) cout<<x+1<<" ";
		cout NL;
	}
  return 0;
}