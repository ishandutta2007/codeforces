...
int a[4005],dp[4005][4005],p[4005],q[4005];
signed main()
{
	int n=read();odl(n+1);
	rg(n)a[i]=read();p[i]=i;gr
	std::sort(p+1,p+1+n,[&](int x,int y){return x[a]>y[a];});
	rg(n)rg_(j,a[p[i]])dp[(j+i)%(n+1)][p[i]]=1;gr gr
	rrg(n+1){rg_(j,n)od(dp[i][j]);gr puts("");gr
	return 0;
}