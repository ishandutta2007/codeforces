...
fn run() {
    ...
    let mut ans = M::zero();
    for _ in 0..2 {
        let cnt = s.iter().flatten().filter(|c| **c == b'o').count() as u64;
        let w = s[0].len();
        let mut dp = vec![M::zero(); w + 1];
        for i in (3..=w).step_by(2) {
            dp[i] = dp[i - 2] + M::new(2).pow(cnt - i as u64);
        }
        for s in s.iter() {
            let mut len = 0;
            for c in s.iter() {
                ...
                if len >= 2 {
                    ans += dp[(len - 1) / 2 * 2 + 1];
                    if len % 2 == 0 {
                        ans += M::new(2).pow(cnt - len as u64);
                    }
                }
            }
        }
