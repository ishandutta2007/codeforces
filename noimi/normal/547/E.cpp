...
int main() {
    ...
    rep(i, n) {
        S += s[i];
        S.pb('.');
        rep(_, si(s[i]) + 1) idx.eb(i);
    }
    SuffixArray sa(S);
    vv(int, IDX, n);
    {
        vi nxt(si(idx));
        rep(i, si(S)) IDX[idx[sa[i]]].eb(i);
    }
    vi lo(n), up(n);
    rep(i, n) tie(lo[i], up[i]) = sa.lower_upper_bound(s[i]);
    RAQ<int> seg(si(S));
    ...
    for(auto [t, k, i, p] : v) {
        while(now < t) {
            for(auto e : IDX[now]) seg.update(e, 1);
            now++;
        }
        ans[i] += p * seg.query(lo[k], up[k]);
    }
