...
map<int, vector<pair<int, int>>> mapka;

vector<pair<int, int>> solve(int n, int k)
{
    if (n==4 && k==3) return {};

    if (n==4 && k == 0) return {{0, 3}, {1, 2}};
    if (n==4 && k == 1) return {{0, 2}, {1, 3}};
    if (n==4 && k == 2) return {{2, 3}, {0, 1}};
    ...
    for (auto it: heh)
    {
        ...
        if (need)
        {
            ans.push_back(mp(2*a, 2*b)); ans.push_back(mp(2*a+1, 2*b+1));
            need--;
        }
        else
        {
            ans.push_back(mp(2*a+1, 2*b)); ans.push_back(mp(2*a, 2*b+1));
        }
    }
    return ans;
}
...
void solve()
{
    ...
    /*int SUM = 0;
    for (auto it: res) SUM+=(it.first&it.second);
    cout<<SUM<<endl;*/
...
int main()
{
    ...
    do
    {
        ...
        for (int i = 0; i<8; i+=2) guys.push_back(mp(a[i], a[i+1]));
        mapka[ans] = guys;
