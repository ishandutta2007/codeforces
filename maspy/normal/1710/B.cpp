...
void solve() {
  ...
  vi A(K);
  FOR(k, K) { A[k] = dp[k].fi * X[k] + dp[k].se; }
  ...
  SegTree<Monoid_Max<ll>> seg_B(B), seg_C(C);

  auto f = [&](int i) -> bool {
    auto [a, p] = query[i];
    int k = LB(X, a);
    if (A[k] - p > M) { return false; }
    if (seg_B.prod(0, k) > p - a) return false;
    if (seg_C.prod(k + 1, K) > a + p) return false;
    return true;
  };
