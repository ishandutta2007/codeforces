...
void solve() {
	...
	auto check = [](vector<pair<int,int>> edges) {
		REP(i, n) {
			vertexs[i + 1].clear();
			ut[i + 1] = i + 1;
			backs[i + 1] = -1;
		...
		REP(i, edges.size()) {
			if (um(edges[i].first, edges[i].second) == 0) return false;
		...
		if (dist[i] * 2 == n) {
			for (int q = 1; q <= n; ++q) {
				if (vertexs[q].size() == 1) {
					int target = i;
					int T = q;
					while (T != i and backs[T] != -1) {
						T = backs[T];
					}
					if (T == i) target = backs[i];
					cout << vertexs[q][0] << " " << q << endl;
					cout << q << " " << target << endl;
					return;
				}
			}
