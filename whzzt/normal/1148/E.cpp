...
int main(){
	...
	for(int i=1;i<n;++i){
		...
		if(it==s.begin()) cout<<"NO"<<endl,exit(0);
		auto pr=*it,pl=*--it;
		s.erase(pl); s.erase(pr); t.erase(tar);
		int d=tar.fi-pl.fi;
		if(tar.fi-pl.fi>pr.fi-tar.fi) d=pr.fi-tar.fi;
		op.pb(to_string(pl.se)+" "+to_string(pr.se)+" "+to_string(d));
		pl.fi+=d; pr.fi-=d;
		if(pl.fi==tar.fi) s.insert(pr); else s.insert(pl);
		cerr<<op.back()<<endl;
	}
	cout<<"YES"<<endl;
	cout<<op.size()<<endl;
