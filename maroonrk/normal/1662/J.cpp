...
void slv(){
	int n;cin>>n;
	vvc<int> a(n);
	rep(i,n)a[i]=readvi(n,-1);
	vvc<int> c(n);
	rep(i,n)c[i]=readvi(n);
	
	int vs=2;
	vvc<int> idx(n);
	rep(i,n){
		idx[i].pb(0);
		rep(j,n-1)idx[i].pb(vs++);
		idx[i].pb(1);
	}
	maxflow<int> mf(vs);
	rep(i,n)rep(j,n)mf.ae(idx[i][j+1],idx[i][j],inf);
	rep(i,n)rep(j,n)if(c[i][j]==0)mf.ae(idx[i][a[i][j]],idx[i][a[i][j]+1],1);
	rep(j,n){
		vi ls(n);iota(all(ls),0);
		sort(all(ls),[&](int x,int y){return a[x][j]<a[y][j];});
		rep(i,n-1){
			int x=ls[i],y=ls[i+1];
			mf.ae(idx[y][i+1],idx[x][i+1],inf);
		}
	}
	print(n-mf.calc(0,1));
}
