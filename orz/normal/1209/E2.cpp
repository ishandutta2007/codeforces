...
int another_main()
{
	...
	vi gmasks[NN];
	//vvpii part[NN];
	vi part[NN], part1[NN], part2[NN];
	int** mainmask;
	mainmask = new int*[NN];
	for (int I = 0; I < NN; ++I)
	{
		int M = 1 << I;
		mainmask[I] = new int[M];
		igmasks[I].resize(M);
		for (int sk = 0; sk < M; ++sk)
		{
			...
			for (int j = 0; j < I - 1; ++j)
			{
				ssk <<= 1;
				ssk = ((ssk | (ssk >> I)) & (M - 1));
				mainmask[I][sk] = min(mainmask[I][sk], ssk);
			}
			igmasks[I][mainmask[I][sk]] = true;
		}
			...
			if (igmasks[I][sk])
			{
				...
				while (good)
				{
					if (sk2 == 0)
						good = false;
					int sk1 = sk ^ sk2;
					s.emplace(mainmask[I][sk1], mainmask[I][sk2]);
					sk2 = (sk2 - 1) & sk;
				}
	...
	for (int I = 0; I < T; ++I)
	{
		int n, m;
		cin >> n >> m;
		vector<pair<int, vi>> cols(m, mp(0, vi(n)));
		for (int i = 0; i < n; ++i)
		{
			for (int j = 0; j < m; ++j)
			{
				cin >> cols[j].Y[i];
				cols[j].X = min(cols[j].X, -cols[j].Y[i]);
			}
		}
		...
		for (int j = 0; j < m; ++j)
		{
			...
			for (int sk = 1; sk < M; ++sk)
			{
				sum2[sk] = sum2[sk & (sk - 1)] + cols[j].Y[__builtin_ctz((sk & (sk - 1)) ^ sk)];
				ans2[mainmask[n][sk]] = max(ans2[mainmask[n][sk]], sum2[sk]);
			}
			
			for (int k = 0; k < sz(part[n]); ++k)
				ans[part[n][k]] = max(ans[part[n][k]], ans1[part1[n][k]] + ans2[part2[n][k]]);

			/*
			for (int k = 0; k < sz(gmasks[n]); ++k)
			{
				int sk = gmasks[n][k];
				for (pii p : part[n][k])
					ans[sk] = max(ans[sk], ans1[p.X] + ans2[p.Y]);
			}
			*/
			...
			for (int sk : gmasks[n])
			{
				...
				while (good)
				{
					if (sk2 == 0)
						good = false;
					int sk1 = sk ^ sk2;
					ans[sk] = max(ans[sk], ans1[mainmask[n][sk1]] + ans2[mainmask[n][sk2]]);
					sk2 = (sk2 - 1) & sk;
				}
			}
			*/
