...
fn main() {
	...
	if min == 1 {
		...
		let mut dp = v!([c][1<<r] = r*c);
		let count = |z:usize, i:usize| (z ^ b[i]).count_ones() as usize;
		let is_ok = |z0:usize, z1:usize| -> bool {
			if r == 2 {
				let z = z0 ^ z1;
				z == 1 || z == 2
			} else {
		...
		for j in 2..c {
			for z0 in 0..1<<r {
				for z1 in 0..1<<r {
					let s1 = count(z1,j);
					if is_ok(z0,z1) {
						m!(<, dp[j][z1], dp[j-1][z0] + s1);
					}
				}
			}
		}
		let sol = dp[c-1].drain(..).min().unwrap();
		writeln!(rout, "{}", sol).ok();
	}
}