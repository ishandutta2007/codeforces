...
void solve() {
    ...
    for(int i=0; i<n; ++i) {
        for(int j=0; j<mod; ++j) {
            nx[vtx(i,j)]=vtx(e[i][(j+k[i])%m[i]],(k[i]+j)%mod);
        }
    }
    ...
    while(qry--) {
        ...
        y=(y%mod+mod)%mod;
        cout<<lopk[vtx(x,y)]<<"\n";
        dump(vtx(x,y)/mod,nx[vtx(x,y)]/mod,nx[nx[vtx(x,y)]]/mod);
    }
...
int dfs(int v) {
    if(vis[v]) {
        if(~lopk[v]) return lopk[v];
        int t=v;
        u_set<int> vrs;
        do {
            vrs.emplace(t/mod);
        } while((t=nx[t])!=v);
        return lopk[v]=vrs.size();
    }
    vis[v]=true;
    return lopk[v]=dfs(nx[v]);
}
