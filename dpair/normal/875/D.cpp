...
int n, a[MAXN];
int f[31][MAXN], g[31][MAXN], p[MAXN], suc[MAXN], pre[MAXN]; LL ans = 0;
inline void solve(int l, int r, int mid, const int x) {
    rep(j, 0, 30) if(!(x >> j & 1)) chmax(l, f[j][mid]), chmin(r, g[j][mid]);
    ans -= 1ll * (mid - l) * (r - mid);
}
signed main(){
    read(n); read(a + 1, a + n + 1); rep(j, 0, 30) {
        rep(i, 1, n) {
            if(a[i] >> j & 1) f[j][i] = i;
            else f[j][i] = f[j][i - 1];
        } 
        g[j][n + 1] = n + 1; per(i, n, 1) {
            if(a[i] >> j & 1) g[j][i] = i;
            else g[j][i] = g[j][i + 1];
        } 
    }
    rep(i, 1, n) p[i] = i, pre[i] = i - 1, suc[i] = i + 1;
    std::sort(p + 1, p + n + 1, [](int x, int y) {return a[x] < a[y];}); ans = 1ll * (n + 1) * n >> 1;
    rep(i, 1, n) { const int x = p[i]; solve(pre[x], suc[x], x, a[x]); pre[suc[x]] = pre[x]; suc[pre[x]] = suc[x]; } print(ans);
}