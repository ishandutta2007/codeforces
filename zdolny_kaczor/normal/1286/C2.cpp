...
int main() {
	...
	vector <string> cans = BT(n, interactor::query(1, n)).get();
	if (cans.size() == 1u) interactor::answer(cans[0].c_str());
	debug(imie(cans));
	FOR(i, 0, n - 2) FOR(j, i, n - 2) ranges.emplace_back(i, j);
	sort(ALL(ranges), [](pii a, pii b){return a.second - a.first < b.second - b.first;});
	debug(ranges);
	vector <vector <ull> > H;
	REP(i, cans.size())
