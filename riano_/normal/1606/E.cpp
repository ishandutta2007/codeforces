...
int main(){
    riano_; ll ans = 0;
    ll N,M,K,H,W,T,Q,X; cin >> N >> X;
    M = 501;
    //main
    //N:max
    fact.assign(M+1,mint(1));
    invf.assign(M+1,mint(1));
    rep(i,M) fact[i+1] = fact[i]*mint(i+1);
    rep(i,M+1) invf[i] = modinv(fact[i].val);
    mint p[M+1][M+1];
    rep(i,M+1){
        rep(j,M+1){
            p[i][j] = pw(i,j);
    ...
    auto dp = v2m(501,501,0);
    rep(j,M) dp[2][j] = j;
    for(int i=3;i<M;i++){
        for(int j=1;j<M;j++){
            ll r = i-1;
            //to 0
            dp[i][j] += pw(min(r,(ll)j),i);
            if(j<=r) continue;
            dp[i][j] += dp[i][j-r];
            for(int k=2;k<i;k++){
                int l=j-r;
                dp[i][j] += comb(i,k)*p[r][i-k]*dp[k][l];
            }
        }
    }
    ans = dp[N][X].val;
