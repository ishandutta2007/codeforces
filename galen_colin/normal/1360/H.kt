...
*/
    ...
    fun solve(tc: Int) {
        ...
        for (i in 0 until n) s[i] = `in`!!.nstr()
        Arrays.sort(s)
        val tot = (1L shl m) - 1
        var cur = tot / 2
        val req = (tot - n) / 2
        var c = "0"
        for (i in 0 until m - 1) c += "1"
        var pt: Int
        pt = 0
        while (pt < n) {
            if (c.compareTo(s[pt]!!) < 0) break
            --cur
            pt++
        }

        // out.println(cur + " " + req + " " + pt);
        if (cur > req) {
            --pt
            while (cur > req) {
                if (pt >= 0 && c.compareTo(s[pt]!!) == 0) {
                    --pt
                } else {
                    --cur
                    // if (cur == req) break;
        ...
        if (cur == req) {
            while (pt >= 0 && pt < n && c.compareTo(s[pt]!!) == 0) {
                c = sub(c)
                --pt
            }
            --pt
    ...
    fun ad(s: String): String {
        val nxt = CharArray(s.length)
        var carry = true
        for (i in s.length - 1 downTo 0) {
            if (!carry) nxt[i] = s[i] else {
                if (s[i] == '0') {
                    nxt[i] = '1'
                    carry = false
                } else nxt[i] = '0'
    ...
    fun from(c: CharArray): String {
        val sb = StringBuilder()
        for (i in c.indices) sb.append(c[i])
        return sb.toString()
    }
