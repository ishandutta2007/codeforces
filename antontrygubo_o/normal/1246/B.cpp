...
vector<set<pair<int, int>>> simple(N);
vector<set<pair<int, int>>> inverted(N);
...
void init()
{
    ...
    for (int i = 2; i<N; i++)
    {
        ...
        auto it1 = simple[i1];
        auto it2 = inverted[i1];
        cnt%=k;
        if (cnt!=0) {it1.insert(mp(p, cnt)); it2.insert(mp(p, k-cnt));}
        simple[i] = it1;
        inverted[i] = it2;
    }
...
int main() {
    ...
    map<set<pair<int, int>>, int> t;
    ll total = 0;
    for (int i = 0; i<n; i++)
    {
        total+=t[inverted[a[i]]];
        t[simple[a[i]]]++;
    }
    cout<<total;
