...
// cin>>T;while(T--)
{
    ...
    repA(i,1,n)
    {
        cin>>in;
        a.pb(in);
        dp[i][0]=max(dp[i-1][0]+a[i],a[i]);dp[i][0]=max(dp[i][0],0LL);
        dp[i][1]=max(dp[i-1][1]+x*a[i],x*a[i]);dp[i][1]=max(dp[i][1],0LL);
    ...
    repD(i,n,1)
    {
        cnt=max(cnt+a[i],a[i]);cnt=max(cnt,0LL);
        ans=max(ans,cnt+dp[i-1][1]);
    }
    cout<<ans<<endl;
}   aryanc403();
    return 0;
