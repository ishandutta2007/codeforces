...
int main() {
	int n, sx, sy; R(n,sx,sy);
	
	vector<int> quad(4), dir(4); //WASD
	
	for (int i = 0; i < n; i++) {
		int x,y; R(x,y);
		if (x == sx)
			if (y > sy) dir[0]++; else dir[2]++;
		
		if (y == sy)
			if (x > sx) dir[3]++; else dir[1]++;
			
		if (x != sx && y != sy) {
			if (x > sx)
				if (y > sy) quad[1-1]++; else quad[4-1]++;
				
			if (x < sx)
				if (y > sy) quad[2-1]++; else quad[3-1]++;
		}
	}
	...
	int w = dir[0] + quad[1-1] + quad[2-1];
	int A = dir[1] + quad[2-1] + quad[3-1];
	int S = dir[2] + quad[3-1] + quad[4-1];
	int D = dir[3] + quad[4-1] + quad[1-1];
