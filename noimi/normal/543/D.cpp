...
int main() {
    INT(n);
    Graph g(n);
    rep2(i, 1, n - 1) g[in() - 1].eb(i);
    vv(mint, dp, n, 2);
    auto ch = [&](const vmint &a) { return vmint{a[0], a[0] + a[1]}; };
    auto merge = [&](const vmint &a, const vmint &b) {
        vmint res(2);
        res[0] = a[0] * b[0];
        res[1] = (a[0] + a[1]) * (b[0] + b[1]) - res[0];
        return res;
    };
    {
        auto dfs = [&](auto &&f, int x, int p) -> void {
            ...
            for(auto e : g[x]) {
                f(f, e, x);
                dp[x] = merge(dp[x], ch(dp[e]));
            }
        };
        dfs(dfs, 0, -1);
    }
    {
        auto dfs = [&](auto &&f, int x, vmint v) -> void {
            dp[x] = merge(dp[x], v);
            int m = si(g[x]);
            vector<vmint> left(m + 1);
            left[0] = v;
            rep(i, m) left[i + 1] = merge(left[i], ch(dp[g[x][i]]));
            vmint now{1, 0};
            vector<vmint> right(m);
            rep3(i, m - 1, 0) {
                right[i] = ch(merge(left[i], now));
                now = merge(now, ch(dp[g[x][i]]));
            }
            rep(i, m) f(f, g[x][i], right[i]);
        };
        dfs(dfs, 0, vmint{1, 0});
    }
