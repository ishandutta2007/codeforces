...
constexpr int MAXG = 1024;
const vc<int> dirs = {-1,+1};
...
void Solve(){
	...
	while(!dq.empty()){
		...
		dq.pop_front();
		if( a[m] - a[pos] + t <= g )
			Mymin( ans , dis[pos][t]*(g+r) + a[m]-a[pos] + t );
		for( int dir : dirs ){
			int vPos = pos + dir;
			if( vPos < 1 or vPos >= m ) continue;
			int vT = abs(a[vPos]-a[pos]) + t;
			if( vT > g ) continue;
			ll vDis = vT == g ? 1 + dis[pos][t] : dis[pos][t];
			if( vT == g ) vT = 0;
			if( dis[vPos][vT] != -1 ) continue;
			dis[vPos][vT] = vDis;
			if( vT == 0 ) dq.Pb({vPos,vT});
			else dq.Pf({vPos,vT});
		}
	}
	if( ans == LINF ) ans = -1;
