...
//        cout << endl;
        ...
        while(!q.empty())
        {
            ...
            forn(i, 4)
            {
                int x = v.x + dx[i];
                int y = v.y + dy[i];
                if (x >= 0 && x < n && y >= 0 && y < m && d[x][y] == -1 && (ss[x][y] == 'S' || b[ss[x][y] - 'a']))
                {
                    d[x][y] = d[v.x][v.y] + 1;
                    q.push(mp(x, y));
                }
        ...
        //cout << d[s.x][s.y] << endl;
        if (d[s.x][s.y] == -1 || d[s.x][s.y] > best) continue;
        vector <PII> Q;
        Q.pb(s);
        char used[n][m];
        clr(used);
        ...
        while (1)
        {
            ...
            forn(t, Q.size())
            {
                ...
                forn(i, 4)
                {
                    int x = v.x + dx[i];
                    int y = v.y + dy[i];
                    if (x >= 0 && x < n && y >= 0 && y < m && d[x][y] == d[v.x][v.y] - 1 && (ss[x][y] == 'T' || b[ss[x][y] - 'a']))
                    {
