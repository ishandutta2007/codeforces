...
// URL: https://codeforces.com/contest/1483/problem/0
// Memory Limit: 512 MB
// Time Limit: 2000 ms
// 
...
for(int n = 0; n < N; ++n) {
    if(freq[n] > MM) {
        for(int m = 0; m < M and freq[n] > MM; ++m) {
            if(ans[m] == n) {
                for(auto i : friends[m]) {
                    if(i != n) {
                        ans[m] = i;
                        --freq[n];
                        ++freq[i];
...
if(ok) {
    ...
    for(auto a : ans) {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(a + 1), " ");  }();
	}
	[&]() { auto& out = cout; put_stream(out, "\n");  }();
} else {
    [&]() { auto& out = cout; put_stream(out, "NO\n");  }();
}
