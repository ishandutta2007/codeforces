...
pt isec(pt A, pt B, pt C, pt D) {
    vll a = { B.y - A.y,D.y - C.y };
    vll b = { A.x - B.x,C.x - D.x };
    
    vll c(2);
    c[0] = a[0] * A.x + b[0] * A.y;
    c[1] = a[1] * C.x + b[1] * C.y;

    ll t = a[0] * C.x + b[0] * C.y - c[0];
    ll u = a[0] * D.x + b[0] * D.y - c[0];
    if (t>0&&u>0) { return { INF,INF }; }
    if (t < 0 && u < 0) { return { INF,INF }; }
    t = a[1] * A.x + b[1] * A.y - c[1];
    u = a[1] * B.x + b[1] * B.y - c[1];
    if (t >0&&u>0) { return { INF,INF }; }
    if (t < 0 && u < 0) { return { INF,INF }; }
    ...
    ll X = c[0] * b[1] - c[1] * b[0];
    ll Y = -c[0] * a[1] + c[1] * a[0];
    if (X % det != 0 || Y % det != 0) { return { INF,INF }; }
    X /= det;
    Y /= det;
...
int main() {
    ...
    ll ans = 0;
    rep(i, N) {
        ll xd = abs(B[i].x - A[i].x);
        ll yd = abs(B[i].y - A[i].y);
        ans += gcd(xd, yd) + 1;
        //cout << gcd(xd, yd) + 1 << endl;
    }
    ...
    rep(i, N)for (ll j = i + 1; j < N; j++) {
        //cout << i << j << endl;
        pt now = isec(A[i], B[i], A[j], B[j]);
        P p = { now.x,now.y };
        if (now.x == INF) { continue; }
        mp[p].insert(i);
        mp[p].insert(j);
    }
