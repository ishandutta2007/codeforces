...
template <typename T> vector<T> dijkstra(Wgraph<T> &g, int s, const T INF = numeric_limits<T>::max()) {
...
int main() {
    ...
    rep(i, n) rep(j, m) {
        if(a[i][j]) {
            fore(dx, dy, dx4) {
                int ni = i + dx, nj = j + dy;
                int nni = i + dx * 2, nnj = j + dy * 2;
                if(inc(ni, 0, n) and inc(nni, 0, n) and inc(nj, 0, m) and inc(nnj, 0, m)) {
                    if(to[ni][nj] == f(nni, nnj)) { g[f(i, j)].eb(f(nni, nnj), y); }
                }
            }
            for(int dx : {-1, 1})
                for(int dy : {-1, 1}) {
                    if(inc(i + dx, 0, n) and inc(j + dy, 0, m) and a[i + dx][j + dy]) {
                        if(to[i + dx][j] == f(i + dx, j + dy) or to[i][j + dy] == f(i + dx, j + dy)) { g[f(i, j)].eb(f(i + dx, j + dy), x); }
                    }
                }
        }
    }

    auto d = dijkstra(g, st, inf<ll>);

    ll mi = inf<ll>;
    rep(i, n) rep(j, m) {
        if(a[i][j] and i < n - 1 and a[i + 1][j]) { chmin(mi, d[f(i, j)] + d[f(i + 1, j)]); }
        if(a[i][j] and j < m - 1 and a[i][j + 1]) { chmin(mi, d[f(i, j)] + d[f(i, j + 1)]); }
    }
