...
Node nodes[101001 * 64];
Node *lastn = nodes;
Node *root;
void update(Node*& cur, ll l, ll r, ll x, ll y, int val) {
    if (x > r || y < l) return;
    assert(l <= r);
    if (!cur) cur = lastn++;
    if (x <= l && r <= y) {
        ...
        auto m = (l + r) / 2;
        update(cur->lp, l, m, x, y, val);
        update(cur->rp, m + 1, r, x, y, val);
    }
}
ll ans[3] = {0, 1, 0};
struct Mat {
...
Mat mem[10][100];
void go(Node* cur, ll l, ll r, int val = 0) {
    if (cur) val |= cur->val;
    assert(l <= r);
    if (!cur || l == r) {
        Mat mat; FI(3) mat[i][i] = 1;
        ll power = r - l + 1;
        for (int i = 0; i < 61; ++i) {
            if (checkbit(power, i)) {
                mat = mat * mem[val][i];
        ...
        ll res[3] = {};
        FI(3) FJ(3) {
            res[i] += ans[j] * (ll)mat[i][j];
        }
        FI(3) ans[i] = res[i] % mod;
        return;
    }
    ll m = (l + r) / 2;
    go(cur->lp, l, m, val);
...
int solve() {
    ...
    for (int val = 0; val < 8; ++val) {
        FI(3) FJ(3) mem[val][0][i][j] = !checkbit(val, i) && abs(i - j) <= 1;
    }
    for (int val = 0; val < 8; ++val) {
        for (int k = 1; k < 61; ++k) {
            mem[val][k] = mem[val][k - 1] * mem[val][k - 1];
        }
    }
    FI(n) {
        int a;
