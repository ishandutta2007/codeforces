...
cin>>n;
for(int i=0;i<n;i++){cin>>ch;a[i]=int(ch-'0');y+=a[i];}
if(!y)return cout<<"YES",0;
for(int i=0;i<y;i++)
{bool yes=true;x=0;   for(int j=0;j<n;j++){x+=a[j];if(x>i){yes=false;break;}if(x==i)x=0;}
if(yes&&x==0)return cout<<"YES",0;
}
cout<<"NO";
