...
int main(){
    ...
    int m = cp.size();
    vector<vector<P>> ls(m+1);
    for(int i = 0; i < n; i++){
        ls[r[i]].push_back(P(l[i], t[i]));
    }
    vector<Seg> dp(2, Seg(m+1));
    dp[0].set(0, 0);
    dp[1].set(1, 0);
    for(int r = 1; r <= m; r++){
        if(dp[0].get(r-1) < dp[1].prod(0, r)){
            dp[0].set(r-1, dp[1].prod(0, r));
        }
        if(dp[1].get(r-1) < dp[0].prod(0, r)){
            dp[1].set(r-1, dp[0].prod(0, r));
        }
        for(auto [l, t]: ls[r]){
            dp[t].apply(0, l, 1);
        }
    }
    cout << max(dp[0].all_prod(), dp[1].all_prod()) << endl;
}