...
int main(){
    ...
    for(int i = 0; i < m; i++){
        for(int j = 0; j < m; j++){
            ...
            if(current_w_sum%MOD == 0) continue;
            mint p_like = mint(current_w_sum_like)/current_w_sum;
            mint p_dislike = mint(current_w_sum_dislike)/current_w_sum;
            dp[i+1][j+1] += dp[i][j]*p_like;
            dp[i+1][j] += dp[i][j]*p_dislike;
        }
    }
    mint exp_like = 0;
    mint exp_dislike = 0;
    for(int j = 0; j <= m; j++){
        exp_like += mint(j)*dp[m][j];
        exp_dislike -= mint(m-j)*dp[m][j];
    }
    for(int i = 0; i < n; i++){
        mint ans = w[i];
        if(a[i] == 1){
            mint p_idx = mint(w[i])/mint(w_sum_like);
            ans += exp_like*p_idx;
        }else{
            mint p_idx = mint(w[i])/mint(w_sum_dislike);
            ans += exp_dislike*p_idx;
        }
        cout << ans << endl;
    }
}