// Three Blocks Palindrome (easy version) (https://codeforces.com/contest/1335/problem/E1)
...
int main() {
    ...
    while (t--) {
        ...
        array<vector<int>, 200> pref{}, idx{};
        rep(i, 200)
            pref[i].resize(n);
        rep(i, n)
            pref[a[i]][i] = 1, idx[a[i]].push_back(i);
        rep(i, 200)
            partial_sum(all(pref[i]), begin(pref[i]));
        auto range = [&](int ai, int l, int r) {
            return pref[ai][r-1] - (l ? pref[ai][l-1] : 0);
        };

        int mx = 0;
        rep(i, 200) {
            mx = max(mx, sz(idx[i]));
            rep(j, 1, sz(idx[i]) / 2 + 1) {
                rep(k, 200)
                    mx = max(mx, 2 * j + range(k, idx[i][j-1] + 1, idx[i][sz(idx[i])-j]));
            }
        }
