...
// ============= Solution ============= //
// ans is 0, 1, or 2 because 1 and 2 cannot both be prefix ANDs.
// length is 0 if 0 never appears; all edge weights share a common bit
// length is 1 if 1 never appears in list. last non-zero number must not be 1.
//
//
// is there a path from u to v that never hits 0 or 1?
// --- is there a path from u to v such that their AND is on in bit i (for any i != 0)?
// 
// first, compute CCs for each bit k.
// CCs[k][i]
//
// then compute which nodes are in some CC for some k that has an edge (internal or external) that kills 1
// ask(u, v) -> 0 if u, v in same cc for some k
//           -> 1 if u can kill 1
//           -> 2 otheriwse
...
int main() {
   ...
   for (int i = 0; i < n * 30; ++i) par[i] = i;
#define id(x, k) ((k) * n + (x))
   for (int k = 0; k < 30; ++k) {
      for (int j = 0; j < m; ++j) {
         if ((C[j] >> k) & 1) {
            join(id(A[j], k), id(B[j], k));
         }
      }
   }
   ...
   for (int j = 0; j < m; ++j) {
      if (!(C[j] & 1)) {
         for (int k = 1; k < 30; ++k) {
            ok[find(id(A[j], k))] = true;
            ok[find(id(B[j], k))] = true;
         }
      }
   }
