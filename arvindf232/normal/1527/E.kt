...
class longMaxLazy (nSuggest  :Int, val k:Int){
    ...
    val arr = Array(n * 2){IntArray(k+1){empty} }
...
//    }
    private fun updateNode(i:Int,level:Int){
        for(p in 0..k){
            arr[i][p] = minOf(arr[i shl 1 ][p], arr[(i shl 1) + 1][p]) + lazy[i]
        }
    ...
    fun rangeQueryLazy(l:Int,r:Int):IntArray {
        pushPath(l+n)
        pushPath(r+n)
        var ret = IntArray(k+1){empty}
        segDivision(l,r){i, level ->
            for(p in 0..k){
                ret[p]  = minOf(ret[p],arr[i][p])
            }
        }
        return ret
    }
...
fun main(){
    solve.cases{
        ...
        val k = getint
        val L = getline(n)
        val st = longMaxLazy(n+1,k)
        st.arr[st.n][0] = 0
        st.updatePath(st.n)


        val lastseen = IntArray(n+1){-1}
        for(cut in 1..n){
            val v = L[cut-1]
            if(lastseen[v] != -1){
                st.rangeAdd(0,lastseen[v],cut-1-lastseen[v])
            }
            lastseen[v] = cut -1
            val best = st.rangeQueryLazy(0,cut-1)
            st.pushPath(st.n + cut)
            for(p in 1..k){
                st.arr[cut + st.n][p] = best[p-1]
            }
            st.updatePath(st.n + cut)
        }
        val final = st.rangeQueryLazy(n,n)
        put(final[k])
