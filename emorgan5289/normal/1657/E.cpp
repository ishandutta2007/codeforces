...
int main() {
    ...
    for (int i = 1; i < N; i++)
        for (int j = 0; j <= i; j++)
            ch[i][j] = ch[i-1][j] + (j == 0 ? 0 : ch[i-1][j-1]);

    // dp[i][j] = i vertices, values in the range j..k
    fill(dp[0], dp[0]+N, 1);
    for (int i = 1; i <= n; i++) {
        for (int j = k; j > 0; j--) {
            for (int x = 0; x <= i; x++) {
                int c = i*x - x*(x+1)/2; // i-1 * i-2 * ... * i-x
                dp[i][j] += dp[i-x][j+1] * mod(j).pow(c) * ch[i][x];
            }
        }
    }
