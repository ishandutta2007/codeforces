...
pair<mint,pair<int,int>> hard_evolve(int k, int b, int u, int v) {
    if (k == 0) {
        if (u == v) return {0, {0, 0}};
        return {0, {1, u}};
    }
    int x = (k + b + 1) % 2;
    int y = !x;
    if (u == !x and v == !y) return {0, {k+1, b}};
    if (u == x and v == y) return {2*k-1, {k-1, b & (k >= 2)}};
    return {k, {k, !b}};
}
...
void evolve(int k, int b, int u, int v) {
    // dbg(k, b, u, v);
    auto pp = hard_evolve(k, b, u, v);
    int k1 = pp.second.first, b1 = pp.second.second;
    // dbg(k1, b1, pp.first);
    // dbg(cnt[k][b], din[k][b]);
    new_cnt[k1][b1] += cnt[k][b];
    new_din[k1][b1] += cnt[k][b] * pp.first + din[k][b];
    // dbg(new_din[k1][b1]);
}
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int k = 0; k <= i; k++) {
            for (int b = 0; b <= 1; b++) {
                for (int u = 0; u <= 1; u++) {
                    for (int v = 0; v <= 1; v++) {
                        if ((us < 0 or us == u) and (vt < 0 or vt == v))
                            evolve(k, b, u, v);
                    }
                }
            }
        ...
        // swap(cnt, new_cnt);
        // swap(din, new_din);
        for (int k = 0; k <= i+1; k++) for (int b = 0; b <= 1; b++) {
            cnt[k][b] = new_cnt[k][b];
            din[k][b] = new_din[k][b];
            new_cnt[k][b] = new_din[k][b] = 0;
        }
    }
    ...
    for (int k = 0; k <= n; k++) for (int b = 0; b <= 1; b++) {
        cnt[k][b] = din[k][b] = 0;
    }
}
