...
void solve() {
	...
	vvvvi dp(n + 1, vvvi(m + 1, vvi(m + 1, vi(k, -1e18))));
	for (int i = 1; i <= n; ++i) {
		dp[i][0][0][0] = 0;
		for (int j = 1; j <= m; ++j) {
			for (int c = 0; c <= m; ++c) {
				for (int h = 0; h < k; ++h) {
					if (c)
						dp[i][j][c][h] = max(dp[i][j - 1][c][h], dp[i][j - 1][c - 1][(h - (a[i][j] % k) + k) % k] + a[i][j]);
					else
						dp[i][j][c][h] = (h ? -1e18 : 0);
				}
			}
	...
	for (int i = 1; i <= n; ++i) {
		for (int j = 0; j < k; ++j) {
			...
			for (int h = 0; h < k; ++h) {
				int mx = -1e18;
				for (int c = 0; c <= m / 2; ++c)
					maxravno(mx, dp[i][m][c][h]);
				maxravno(dp1[i][j], dp1[i - 1][(j - h + k) % k] + mx);
			}
		}
	}
	cout << dp1[n][0];
