// time-limit: 3000
// problem-url: https://codeforces.com/contest/1667/problem/E
...
// } ==== End library debug ====
int const maxN=2*int(1e5);
using modular=Modular<998244353>;
auto const comb=makeComb<modular>(maxN+1);
auto const& fact=comb.fact;
auto inverse=inverseF<modular>(maxN);
...
int main(){
	...
	std::vector<modular> part1(a+2), part2(a+2);
	for(int const i: range(2, a+2)){
		if(i>a+1) continue;
		modular result {};
		result=fact[number-a-1]*comb.inverseFact[number-a-i]*inverse[i-1];
		result*=fact[number-i];
		part1[i]=result*(i-1);
		part2[i]=result;
	}
	std::partial_sum(begin(part2), end(part2), part2.begin());
