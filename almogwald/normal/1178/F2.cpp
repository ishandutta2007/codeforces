...
int main() {
	...
	a.push_back(0);
	n = a.size() - 2;
	vector<vector<char>> y(m + 1, vector<char>(n + 1, 1));
	forn(i, n + 1) {
		fornb(j, i) {
			if (a[i] == a[j]) {
				fort(k, j, i) {
	...
	forn(i, n + 1) {
		fornb(j, i) {
			...
			lol left = 0;
			fort(l, j-1, p) {
				left = (left + x[j][l] * x[l + 1][p - 1]* y[a[p]][l]) % modi;
			}
			int lp = p;
			fort(pos, j, i + 1) {
				if (a[p] == a[pos]) {
			...
			lol right = 0;
			fort(r, p, i + 1) {
				right = (right + x[p + 1][r] * x[r + 1][i] * y[a[p]][r]) % modi;
			}
			x[j][i] = (left * right) % modi;
			fort(pos, j, i + 1) {
				if (a[p] == a[pos]) {
					x[j][i] = (x[j][i] * x[lp + 1][pos - 1]) % modi;
					lp = pos;
				}
			}
		}
