...
int main() {
    ...
    vector<int> init(N);
    for (int u = 0; u < N; u++) {
        init[hld.time[u]] = who[u].empty() ? Segnode::INF : who[u].back();
    }

    segtree<Segnode> seg(N, init);
    ...
    while (Q--) {
        ...
        while (a--) {
            ...
            for (auto [l, r] : hld.vertex_segments(u, v)) {
                id = min(id, seg.query_range(l, r).value);
            }
            if (id == Segnode::INF) {
                break;
            }
            ...
            int u = c[id];
            who[u].pop_back();
            seg.update_point(hld.time[u], who[u].empty() ? Segnode::INF : who[u].back());
        }
        int k = ids.size();
        cout << k << " \n"[k == 0];
        for (int i = 0; i < k; i++) {
