...
void main_() {
	...
	VEC(ll, s, n);
	map<pii, ll> mp;
	auto dp = [&](auto dp, int now, ll cnt) -> ll {
		if(!cnt) return 0ll;
		if(mp.count({now, cnt})) return mp[{now, cnt}];
		auto ch = SZ(g[now]);
		if(ch == 0) {
			return mp[{now, cnt}] = cnt * s[now];
		}
		int u = cnt % ch;
		int d = ch - u;
		int dcnt = cnt / ch;
		...
		for(auto c : g[now]) {
			if(u) {
				auto dscore = dp(dp, c.to, dcnt);
				auto uscore = dp(dp, c.to, ucnt);
				ret += dscore;
				add.eb(uscore - dscore);
			} else {
				auto dscore = dp(dp, c.to, dcnt);
				ret += dscore;
			}
		...
		return mp[{now, cnt}] = ret + cnt * s[now];
	};
	print(dp(dp, 0, k));
}