...
int main() {
    ...
    Graph g = getG(n, m);
    rep(i, k) a[i]--;
    vi deg(n), out(n), used(n);
    rep(i, n) for(auto e : g[i]) deg[e]++;
    rep(i, k) for(auto e : g[a[i]]) out[e]++;
    // dump(deg, out);
    rep(i, k) used[a[i]] = true;
    int res = n - k;
    pqg<pair<frac, int>> p;
    rep(i, n) if(!used[i]) p.emplace(frac(deg[i] - out[i], deg[i]), i);
    while(res and !p.empty()) {
        auto [tmp, i] = p.top();
        p.pop();
        if(used[i]) continue;
        queue<int> q({i});
        ...
        while(!q.empty()) {
            ...
            for(auto e : g[t])
                if(!used[e]) {
                    out[e]++;
                    if(frac(deg[e] - out[e], deg[e]) <= tmp) {
                        used[e] = true, res--, now.eb(e + 1);
                        q.emplace(e);
                    } else
                        p.emplace(frac(deg[e] - out[e], deg[e]), e);
                }
        }
        // cout << tmp << " " << i << " " << res << endl;
        // dump(now);
        if(!res) {
            cout << si(now) << endl << now;
            exit(0);
        }
    }
}