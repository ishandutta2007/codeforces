...
void solve() {
    ...
    int score = 0;
    for (int i = 1; i < 4; i++) sort(best[i].begin(), best[i].end());
    ...
    while ((!best[1].empty()) && (!best[2].empty())) {
        score += best[1].back();
        score += best[2].back();
        best[1].pop_back();
        best[2].pop_back();
    ...
    int remain = support * 2 - people;
    for (auto&& x : best[1]) best[0].push_back(x);
    for (auto&& x : best[2]) best[0].push_back(x);
    sort(best[0].begin(), best[0].end());
    while (remain && (!best[0].empty())) {
        remain--;
        score += best[0].back();
        best[0].pop_back();
    }
