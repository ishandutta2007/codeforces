...
int main(int argc, char** argv) {
    ...
    sort(begin(h), end(h), [&](int i, int j) {
        return s[i].a.y < s[j].a.y;
    });

    sort(begin(v), end(v), [&](int i, int j) {
        return s[i].b.y < s[j].b.y;
    });

    auto cross = [&](int hi, int vi) {
        return s[hi].a.x <= s[vi].a.x && s[hi].b.x >= s[vi].a.x && s[vi].a.y <= s[hi].a.y && s[vi].b.y >= s[hi].a.y;
    };

    llong ans = 0;

    for (int i = 0; i < isz(h); i++) {
        deque<int> q;
        tree<pair<int, int>> t;
        for (int z : v) {
            if (cross(h[i], z)) {
        ...
        for (int j = i + 1; j < isz(h); j++) {
            while (isz(q)) {
                int z = q.front();
                if (s[z].b.y < s[h[j]].a.y) {
                    q.pop_front();
                    t.erase({s[z].a.x, z});
                } else {
                    break;
            ...
            c = t.order_of_key({s[h[j]].b.x, int(1e9)}) - t.order_of_key({s[h[j]].a.x, -1});
