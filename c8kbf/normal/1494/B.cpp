...
inline void AC() {
    ...
    l = read();
    bool ok = false;
    for(_ x1 = 0; x1 <= 1; ++x1) for(_ x2 = 0; x2 <= 1; ++x2) for(_ x3 = 0; x3 <= 1; ++x3) for(_ x4 = 0; x4 <= 1; ++x4) {
        _ lm = 0, rm = 0, um = 0, dm = 0;
        if(x1) {
            ++lm;
            ++um;
        }
        ...
        if(l >= lm && l <= lm+n-2 && r >= rm && r <= rm+n-2 && d >= dm && d <= dm+n-2 && u >= um && u <= um+n-2) ok = true;
    }
    writeln(ok ? "Yes" : "No");
