...
public class E2 {
	...
	void solve()
	{
		...
		int[][] jail = new int[1<<K][];
		for(int i = 0;i < 1<<K;i++){
			jail[i] = new int[1<<Integer.bitCount(i)*5];
			Arrays.fill(jail[i], -1);
		}
		...
		for(int l = 0;l < m;l++) {
			...
			for(int i = 0;i < 1<<K;i++){
				...
				if(jail[i][h] == mts[l])hit = true;
				if(jail[i][h] != -1 && jail[i][h] != mts[l]){
					from[p] = mts[l];
					to[p] = jail[i][h];
					p++;
				}
