...
void solve() {
  ...
  VEC(ll, A, N);
  ll LIM = 1LL << 21;
  auto CNT = bincount(A, LIM);
  auto CNTc = cumsum(CNT);
  vi SUM(LIM);
  FOR(i, LIM) SUM[i] += CNT[i] * i;
  auto SUMc = cumsum(SUM);
  ...
  auto f = [&](ll p) -> i128 {
    ...
    while (R < LIM) {
      ...
      i128 cost = 0;
      // (L, M)
      cost += rm * (CNTc[M] - CNTc[L + 1]);
      // [M,R)
      i128 x = (CNTc[R] - CNTc[M]) * R - (SUMc[R] - SUMc[M]);
      cost += ad * x;
      ANS += cost;
      L += p, R += p;
    }
  ...
  for (auto&& p: primes) chmin(ANS, f(p));
  print(ll(ANS));
}
