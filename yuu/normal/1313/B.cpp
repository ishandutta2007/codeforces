...
void solve() {
    ...
    // min
    // a + b > x + y
    const int64_t sum = x + y;
    // on the first part we have (x, y], on the second part, we have [x, y)
    // both part have [1, x) and (y, n]
    int64_t min_rank = 1;
    // continue to pair [1, x) and (y, n] while they can exceed sum
    int64_t low = sum + 1 - n;  // low is the first that can exceed sum
    if (low < 1) low = 1;
    if (low <= x) {
        assert(y < n);
        min_rank += low - 1;  // pair the lowest (low - 1) pair with each other to save people with lower sum
        // the rest can always be paired
    } else {
        assert(y == n);
        // when y = n, we must pair x + 2 with n - 1 and so on
        // leading to max(0, n - (x + 1)) people with higher number
        min_rank = n - max(0LL, n - (x + 1));
    }
    ...
    int64_t max_rank = 1;
    // on the first part we have (x, y], on the second part, we have [x, y)
    // paring them in reverse order net y - x people who will drag down the rank
    max_rank += y - x;
    // we have [1, x) and (y, n] on both part
    // we can pair x - i with y + i, netting extra min(x - 1, n - y) * 2 people
    // finally if we have extra from the lower end, we can in fact get rank n
    if (x - 1 >= n - y) max_rank = n;
    else max_rank += (x - 1) * 2;
    cout << max_rank << '\n';
}
