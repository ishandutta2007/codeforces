...
void solve() {
  ...
  FOR3(i, 1, N) {
    ...
    if (same == 1) {
      vv(mint, newDP, N + 1, N + 1);
      FOR(a, N + 1) FOR(b, N + 1) if (DP[a][b]) newDP[a + b][0] += DP[a][b];
      swap(DP, newDP);
    }

    vv(mint, newDP, N + 1, N + 1);
    FOR(a, N + 1) FOR(b, N + 1) if (DP[a][b]) {
      mint total = i + 1;
      mint rm_a = a;
      mint add_b = 2 * (same - 1) - b;
      mint stay = total - rm_a - add_b;
      if (rm_a) newDP[a - 1][b] += DP[a][b] * rm_a;
      newDP[a][b + 1] += DP[a][b] * add_b;
      newDP[a][b] += DP[a][b] * stay;
    }
    swap(DP, newDP);
  }
  print(DP[0][0]);
