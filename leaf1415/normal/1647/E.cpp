...
int main(void)
{
	...
	ll add = 0;
	rep(i, 1, n) deg[p[i]]++;
	rep(i, 1, n) if(deg[i] == 0) add++;
	
	ll tm = 0;
	rep(i, 1, n) chmax(tm, a[i]);
	tm = (tm - n) / add;
	
	rep(i, 1, n) succ[i][0] = p[i];
	rep(i, 1, 34) rep(j, 1, n) succ[j][i] = succ[succ[j][i-1]][i-1];
	
	set<ll> S, T;
	rep(i, 1, n) S.insert(i);
	rep(i, 1, n) if(a[i] <= n) S.erase(a[i]), T.insert(a[i]);
	
	rep(i, 1, n){
		ll v = i;
		rep(j, 0, 34) if(tm & (1LL<<j)) v = succ[v][j];
		if(T.count(a[v])){
			ans[i] = a[v];
			T.erase(a[v]);
			continue;
