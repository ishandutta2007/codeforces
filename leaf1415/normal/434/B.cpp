...
ll h, w, Q;
ll a[1005][1005];
ll l[1005][1005], r[1005][1005], u[1005][1005], d[1005][1005];

void row(ll y)
{
	rep(i, 1, w){
		if(a[i][y] == 0) l[i][y] = 0;
		else l[i][y] = l[i-1][y] + 1;
	}
	per(i, w, 1){
		if(a[i][y] == 0) r[i][y] = 0;
		else r[i][y] = r[i+1][y] + 1;
	}
}
void col(ll x)
{
	rep(i, 1, h){
		if(a[x][i] == 0) u[x][i] = 0;
		else u[x][i] = u[x][i-1] + 1;
	}
	per(i, h, 1){
		if(a[x][i] == 0) d[x][i] = 0;
		else d[x][i] = d[x][i+1] + 1;
	}
}

ll calc(vector<ll> &vec, ll p)
{
	vector<ll> lvec, rvec;
	MaxRectangle::getNearestLess(vec, lvec, rvec);
	
	ll ret = 0;
	reps(i, vec){
		if(p < lvec[i] || rvec[i] < p) continue;
		chmax(ret, (rvec[i]-lvec[i]+1) * vec[i]);
	}
	return ret;
}
...
int main(void)
{
	...
	rep(i, 1, Q){
		...
		if(t == 1){
			...
			vector<ll> vec;
			rep(i, 1, w) vec.pb(u[i][y]);
			chmax(ans, calc(vec, x-1));
			
			vec.clear();
			rep(i, 1, w) vec.pb(d[i][y]);
			chmax(ans, calc(vec, x-1));
			
			vec.clear();
			rep(i, 1, h) vec.pb(l[x][i]);
			chmax(ans, calc(vec, y-1));
			
			vec.clear();
			rep(i, 1, h) vec.pb(r[x][i]);
			chmax(ans, calc(vec, y-1));
