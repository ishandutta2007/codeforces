...
ll dprod(const ll a, const ll b, const ll Q, const ll QQ)
{
	if (abs(a) >= QQ || abs(b) >= QQ || ((abs(a) >= Q) && (abs(b) >= Q)))
	{
		ll c = QQ;
		ll d = sgn(a);
		ll e = sgn(b);
...
ll dpow(const ll a, const ll b, const ll Q, const ll QQ)
{
	if (b < 2)
	{
		if (b == 0)
	...
	ll temp = dpow(a, b >> 1, Q, QQ);
	temp = dprod(temp, temp, Q, QQ);
	if (b & 1)
		return dprod(temp, a, Q, QQ);
	else
		return temp;
}
...
int another_main()
{
	...
	for (int I = 0; I < t; ++I)
	{
		...
		for (int i = n - 1; i >= 0; --i)
		{
			...
			{
				int dif = k[i + 1] - k[i];
				fs = dprod(fs, dpow(p, dif, Q, QQ), Q, QQ);
				rz = prod(rz, pow_mod(p, dif, P), P);
			}
			if (fs <= 0)
			{
				++fs;
