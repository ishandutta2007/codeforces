...
inline void dfs(const vector<vector<int>> &tree, vector<bool> &minus, vector<bool> &right, int cur, int prev, int d) {
    int cnt = 0;
    for (auto next : tree[cur]) {
        if (next != prev) {
            dfs(tree, minus, right, next, cur, d);
