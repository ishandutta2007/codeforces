...
void solve(){
  ...
  vector<li> P2(3*K+1);
  P2[0] = 1;
  for(li i = 0; i < (li)(3*K); ++i) P2[i+1] = P2[i]*2%1'000'000'007;
  static li A[K];
  for(li i = 0; i < (li)(K); ++i) A[i] = 0;
  vi X(n);
  for(li i = 0; i < (li)(n); ++i) X[i]=read_positive();
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(K); ++j) if(X[i]&(1ll<<j)) A[j]++;
  Zn<> ans = 0;
  for(li i = 0; i < (li)(n); ++i) {
    Zn<> a=0,b=0;
    for(li j = 0; j < (li)(K); ++j) if(X[i]&(1ll<<j)) {
      a += A[j] * P2[j];
    }
    for(li j = 0; j < (li)(K); ++j) {
      if(X[i]&(1ll<<j)) b += n * P2[j];
      else b += A[j] * P2[j];
    }
    ans += a*b;
  }
...
int main(){
  int t;t=read_positive();for(li i = 0; i < (li)(t); ++i)solve();
  cout << flush;
