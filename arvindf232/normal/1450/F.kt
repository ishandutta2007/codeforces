...
fun main(){
    solve.cases{
        ...
        while(left < n) {
            var right = left
            while(right < n-1 && L[right+1] != L[right]){
                right ++
            }
            things.add(Pair(left,right))
            ends[L[left]] ++
            ends[L[right]] ++
        ...
        val maxcount = ends.maxOrNull()!!
        if(maxcount > things.size){
            val b = ends.indexOf(maxcount)
            val overby = maxcount - things.size - 1
            var canhave = 0
...
because one cuts can be ignored
 */