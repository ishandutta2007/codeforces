...
int main() {
    ...
    vector<int> table = euler_phi_table(MAX_N);
    vector<mint> gcdsum(MAX_N, 0);
    for (int i = 1; i < MAX_N; i++) {
        for (int j = i, k = 1; j < MAX_N; j += i, k++) {
            gcdsum[j] += mint(i) * table[k];
        }
    }

    vector<mint> a(MAX_N), b(MAX_N), c(MAX_N);
    a[0] = b[0] = c[0] = 0;
    for (int k = 1; k < MAX_N; k++) {
        mint sum = gcdsum[k] * 2;
        sum += mint(2) * k * (k - 1) * (2 * k - 1) / 6;
        sum -= mint(2) * k * k * (k - 1) / 2;
        sum += mint(k) * (k - 2) * k;
        a[k] = sum;
        b[k] = sum * k;
        c[k] = sum * k * k;
