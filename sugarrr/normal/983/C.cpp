...
signed main(){fastio
    ...
    ll t=10;
    vvvl dp(n+1,vvl(N,vl(t,inf)));
    dp[0][0][1]=0;
    queue<dat>q;q.push({0,0,1});
    while(!q.empty()){
        dat d=q.front();q.pop();
        vl el=v[d.b];
        rep(i,0,3){
            if(el[i]==0){
                if(d.a!=n && a[d.a]==d.c){
                    ...
                    nxel[i]=b[d.a];
                    SORT(nxel);
                    dat nxd={d.a+1,POSL(v,nxel),d.c};
                    if(chmin(dp[nxd.a][nxd.b][nxd.c],dp[d.a][d.b][d.c]+1)){
                        q.push(nxd);
                    }
                }
            }else if(el[i]==d.c){
        ...
        vl dkai={-1,1};
        for(auto dk:dkai){
            dat nxd={d.a,d.b,d.c+dk};
            if(nxd.c<=0 || 10<=nxd.c)continue;
            if(chmin(dp[nxd.a][nxd.b][nxd.c],dp[d.a][d.b][d.c]+1)){
                q.push(nxd);
            }
        }
