...
int main() {
	scanf("%d%d",&n,&k);
	rep(i,0,n) rep(j,0,k+1) scanf("%d",&y[i][j]);
	rep(i,0,k) {
		s.n=0;
		s.push(halfplane(point(i,0),point(i+1,0)));
		s.push(halfplane(point(i+1,0),point(i+1,1e4)));
		s.push(halfplane(point(i,1e4),point(i,0)));
		s.push(halfplane(point(i+1,1e4),point(i,1e4)));
		s.halfplaneinsert();
		s.getconvex(a);
		double sa=a.getarea();
		rep(j,0,n) {
			a.convexcut(halfplane(point(i+1,y[j][i+1]),point(i,y[j][i])),b);
			double ca=b.getarea();
			ans[j]+=sa-ca;
			sa=ca;
			a=b;
