	...
	while (p>0)
	{
		...
		for (int i=0;i<n;i++)
		{
			...
			for (int k=0;k<=L-L0;k++)
				if (k<=L-p && k+L0-1>=L-p && a[i]==s.substr(k,L0))
					checkmin(nextp,L-(k+L0));
		}
		if (nextp==p) break;
		p=nextp;
	}
	if (M.find(MP(s,p))!=M.end()) return M[MP(s,p)];
	int idx=size++;
	M[MP(s,p)]=idx;
	good[idx]=(p==0);
	deg[idx]=0;
...
int main()
{
	...
	M.clear();
	int src=get_id("",0);
	for (int i=0;i<size;i++) f0[i]=0;
	f0[src]=1;
	for (int step=0;step<length;step++)
	{
		for (int i=0;i<size;i++) f1[i]=0;
		for (int i=0;i<size;i++) if (f0[i])
			for (int j=0;j<deg[i];j++)
				addTo(f1[g[i][j]],f0[i]);
		for (int i=0;i<size;i++) f0[i]=f1[i];
//		for (int i=0;i<size;i++) printf("%d ",f0[i]);
//		printf("\n");
	}
	int R=0;
	for (int i=0;i<size;i++) if (good[i]) addTo(R,f0[i]);
	printf("%d\n",R);
	return 0;
}