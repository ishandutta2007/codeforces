...
void dfs(ll x)
{
    if(!(mp[0][x]==0||mp[1][x]==0))
    {
        ll l=0,r=0,t0=0,y0=0,t1=0,y1=0;
        while(l<((ll)v[mp[0][x]].size())&&r<((ll)v[mp[1][x]].size()))
        {
            if(v[mp[0][x]][l]<v[mp[1][x]][r])
            {
                y1++;
                t0+=y0;
                l++;
        ...
        while(l<((ll)v[mp[0][x]].size()))
        {
            y1++;
            t0+=y0;
            l++;
        }
        while(r<((ll)v[mp[1][x]].size()))
        {
            y0++;
            t1+=y1;
            r++;
...
int main()
{
    ...
    REP(i,0,n)
    {
        ...
        REPI(j,0,30)
        {
            if(mp[((1LL<<j)&t)>>j][pt]==0)
            {
                mp[((1LL<<j)&t)>>j][pt]=((ll)dp.size());
                dp.PB(j);
                mp[0].PB(0);
                mp[1].PB(0);
                v.PB(V(ll)());
