...
signed main(){
    LL(n,m);
    VEC(string,s,n);
    auto regi = [x=n*m]()mutable{
        return x++;
    };
    const int S=regi(),T=regi();
    Dinic<int>f(n*m+2+n*(m-1)+(n-1)*m);
    int ans=0;
    each(i,s)each(j,i)ans+=j=='#';
    rep(n)rep(j,m-1)if(s[i][j]=='#'&&s[i][j+1]=='#'){
        ans--;
        const int x=i*m+j,y=x+1,w=regi();
        f.add_edge(S,w,1);
        f.add_edge(w,x,1);
        f.add_edge(w,y,1);
    }
    rep(n-1)rep(j,m)if(s[i][j]=='#'&&s[i+1][j]=='#'){
        ans--;
        const int x=i*m+j,y=x+m,w=regi();
        f.add_edge(w,T,1);
        f.add_edge(x,w,1);
        f.add_edge(y,w,1);
    }
