...
fun Graph.strong_reroot(){
    val sizes = IntArray(n)
    val beats = IntArray(n)
    val beatsum = IntArray(n)
    val root = 0
    fun Graph.size_dfs(v:Int, p:Int):Int{
        var ret = 1
        var childbeat = 0
        NS(v){ w->
            if(w == p){
            ...
            ret += size_dfs(w,v)
            childbeat = childbeat modPlus beats[w]
        }
        sizes[v] = ret
        beats[v] = options[ret]
        beatsum[v] = childbeat
    ...
    fun contribution(i:Int):Int{
        return (options[sizes[i]] modM sizes[i]) modMinus (sizes[i] modM beatsum[i])
    }
    ...
    for(i in 0 until n){
        stack = stack modPlus contribution(i)
    }



    // sizes[i] = size of the subtree of i
    // this value is maintained to be correct, at the start of every reroot_dfs call, with new root v
    // there is a first DFS above to calculate all sizes value for a specific root
    val total = sizes[root]
    fun root_change(from:Int, to:Int){
        stack = stack modMinus contribution(from)
        stack = stack modMinus contribution(to)
        sizes[from] = total - sizes[to]
        beats[from] = options[sizes[from]]
        beatsum[from] = beatsum[from] modMinus beats[to]
        sizes[to] = total
        beats[to] = options[sizes[to]]
        beatsum[to] = beatsum[to] modPlus beats[from]
        stack = stack modPlus contribution(from)
        stack = stack modPlus contribution(to)
    }

    fun reroot_dfs(v:Int, p:Int){
        ret = ret modPlus stack
        // all sizes are correct, with root v
        // do whatever you like here
        NS(v){ u -> // for each neighbour u of v
            if(u == p) return@NS
            root_change(v,u)
            reroot_dfs(u,v)
            root_change(u,v)
...
fun main(){
    ...
    solve.cases{
        ...
        val n =getint
        val k = getint
        options = IntArray(n+1){FACT.choose(it,k)}
        ...
        fun solve(r:Int):Int {
            ...
            val beats = IntArray(n){FACT.choose(sizes[it], k)}
            val beatchildSum = IntArray(n){0}
            val weights = IntArray(n)
            G.rootFirst {
                    v->
                G.subs(v) { w ->
                    beatchildSum[v] = beatchildSum[v] modPlus beats[w]
                }
            }
            var total = 0
            for(i in 0 until n){
                total = total modPlus (FACT.choose(sizes[i],k) modM (sizes[i].adjust()))
                total = total modMinus (sizes[i] modM beatchildSum[i])
            }
            return total
        }
        G.strong_reroot()
