...
ll n, m;
ll l[100005], r[100005], q[100005];
ll a[100005], dif[100005];
SegTree seg(17);
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, m) cin >> l[i] >> r[i] >> q[i];
	
	rep(i, 0, 30){
		rep(j, 1, n+1) dif[j] = 0;
		rep(j, 1, m){
			if(q[j] & (1<<i)) dif[l[j]]++, dif[r[j]+1]--;
		}
		rep(j, 1, n) dif[j] += dif[j-1];
		rep(j, 1, n) if(dif[j]) a[j] |= 1<<i;
	}
