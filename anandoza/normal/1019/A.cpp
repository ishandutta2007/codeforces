...
int main() {
    ...
    ll ans = 1e18;
    for (int target = costs[0].size(); target <= n; target++) {
        ll cand = 0;
        priority_queue<pii> q;

        int have = costs[0].size();
        multiset<int> bank;
        for (auto &i : range(costs.begin() + 1, costs.end()))
            bank.insert(all(i));
        for (int i = 1; i < m; i++) {
            for (int j = -1; j >= -(sz(costs[i]) - (target - 1)); j--) {
                if (abs(j) > sz(costs[i])) {
                    ckmax(cand, ll(1e17));
                    break;
                }
                cand += costs[i].end()[j];
                bank.erase(bank.find(costs[i].end()[j]));
                have++;
            }
        }
