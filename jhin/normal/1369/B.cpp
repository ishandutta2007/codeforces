#include <bits/stdc++.h>
        ...
        while(t--)
        {
            ...
            while(j<n)
            {
                if(s[j]=='1'&&one){j++;continue;}
                else if(s[j]=='0'&&one){one=false;an+='0';break;}
                else if(s[j]==0&&!one){j++;continue;}
                else if(s[j]=='1'&&!one)one=true;
                j++;
