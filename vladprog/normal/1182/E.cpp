...
const int MOD_ADD=1e9+7;
const int MOD_MULT=1e9+6;

ll ppow(ll x,ll y,ll mod=MOD_ADD)
{
    ll res=1;
    while(y)
    {
...
mat mult(mat x,mat y)
{
    ...
    for0(i,n)
        for0(j,n)
            for0(k,n)
                res[i][k]=(res[i][k]+x[i][j]*y[j][k])%MOD_MULT;
    return res;
}
...
signed main()
{
    ...
    int n,f1,f2,f3,c;
    cin>>n>>f1>>f2>>f3>>c;
    int f1k=ppow(mat{row{0,0,1},
                     row{1,0,1},
                     row{0,1,1}},n-1)[0][0];
    int f2k=ppow(mat{row{0,0,1},
                     row{1,0,1},
                     row{0,1,1}},n-1)[1][0];
    int f3k=ppow(mat{row{0,0,1},
                     row{1,0,1},
                     row{0,1,1}},n-1)[2][0];
    int  ck=ppow(mat{row{0,0,0,MOD_MULT-1,0},
                     row{1,0,0,         0,0},
                     row{0,1,0,         0,0},
                     row{0,0,1,         2,0},
                     row{0,0,0,         2,1}},n)[4][0];
