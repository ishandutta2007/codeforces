...
void dfs(ll id,vl &par,vvl&v,vl&jun){
    jun.pb(id);
    for(auto x:v[id]){
...
signed main(){fastio
    ...
    vl jun;
    dfs(1,par,v,jun);
    vl dp(n+1,inf),ep(n+1,inf);
    rrep(Z,(int)jun.size()-1,0){
        ll i=jun[Z];
        if(v[i].size()==1 && v[i][0]== par[i]){
            ep[i]=0;
            dp[i]=w[i];
        }else{
            ll sumdp=0;
    ...
    cout<<"ep ";rep(i,1,n)cout<<ep[i]<<" ";cout<<endl;
     //*/
    vl ans;
    vl fp(n+1);
    fp[1]=1;
