...
fun solve() {
    val (n,m) = io.ints(2)
    val a = io.longs(n)
    data class Edge(val x: Int, val y: Int, val w: Long) {}
    var edges = ArrayList<Edge>();
    repeat(m) {
        val (x,y) = io.ints(2)
        val w = io.long()
        edges.add(Edge(x-1,y-1,w))
    }
    val minInd = a.indexOf(a.min())
    for (i in 0..n-1)
        edges.add(Edge(i,minInd,a[i]+a[minInd]))
    edges = ArrayList(edges.sortedWith(Comparator<Edge>{ x, y -> x.w.compareTo(y.w)}))
    class DSU(size: Int) {
        val e = IntArray(size){-1}
        fun get(x: Int): Int {
            if (e[x] == -1) return x
            e[x] = get(e[x])
            return e[x]
