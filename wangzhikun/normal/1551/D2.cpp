...
vector<str> solve(int n,int m,int k){
	vector<str> ret;
	F0R(i,n){
		str C(m,' ');
		ret.pb(C);
	...
	F0R(i,n/2){
		F0R(j,m/2){
			...
			if(k){
				ret[i*2][j*2] = a;ret[i*2][j*2+1] = a;
				ret[i*2+1][j*2] = b;ret[i*2+1][j*2+1] = b;
				k-=2;
			}else{
				ret[i*2][j*2] = a;ret[i*2][j*2+1] = b;
				ret[i*2+1][j*2] = a;ret[i*2+1][j*2+1] = b;
			}
		}
	}
	return ret;
...
int main() {
	...
	FOR(tc,1,t+1) {
		...
		ps(((k>=m/2) && (k-m/2)%2 == 0)?"YES":"NO");
		if(!((k>=m/2) && (k-m/2)%2 == 0))continue;
		vector<str> Mp = solve(n-1,m,k-m/2);
		str C;
		F0R(i,m){
			if((i/2)%2 == 0)C+='y';
			else C+='z';
