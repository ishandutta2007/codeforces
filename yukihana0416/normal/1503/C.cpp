...
{
    reader >>n;
    for(int i = 1; i <= n; i++) reader >>a[i] >>c[i];

    for(int i = 1; i <= n; i++) p[i] = mk(a[i], c[i]);
    sort(p + 1, p + 1 + n);

    for(int i = 1; i <= n; i++) nxt[i] = lower_bound(p + 1, p + 1 + n, mk(p[i].fi + p[i].se, INF)) - p - 1;
