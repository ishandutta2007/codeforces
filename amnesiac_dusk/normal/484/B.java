...
public class Main
{
	public static void main(String[] args)
	{
		int n=ni();
		int a[]=nia(n);
		NavigableSet<Integer> hola=new TreeSet<Integer>();
		for(int i:a)
			hola.add(i);
		int ans=0;
		int minval[]=new int[2000_050];
		for(int i=1; i<2000050; i++)
		{
			Integer tola=hola.floor(i-1);
			minval[i]=((tola==null)?0:tola);
		}
		Iterator<Integer> te=hola.iterator();
		while(te.hasNext())
		{
			int temp=te.next();
			int i=2*temp;
			for(; i<=1e6; i+=temp)
				ans=Math.max(ans, minval[i]%temp);
			ans=Math.max(ans, minval[i]%temp);
		}
		pr(ans);
