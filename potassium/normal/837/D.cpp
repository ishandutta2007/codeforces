...
int main() {
	...
	vector<vector<vector<int>>> dp(2, vector<vector<int>>(5007, vector<int>(k+1, -1e6)));
	dp[0][0][0] = 0;
	...
	for (int i = 1; i <= n; i++) {
		...
		for (int j = 0; j <= 25*(i-1); j++) {
			for (int l = 0; l <= k; l++)
				chkmax(dp[now][j][l], dp[last][j][l]);
				
			for (int l = 0; l < k; l++)
				chkmax(dp[now][j+v[i].S][l+1], dp[last][j][l] + v[i].F);
		}
	}

	int ans = 0;
	for (int j = 0; j <= 5000; j++)
		chkmax(ans, min(j, dp[n & 1][j][k]));
