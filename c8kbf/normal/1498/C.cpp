...
inline void AC() {
    ...
    n = read();
    k = read();
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= k; ++j) dp[i][j][1] = dp[i][j][0] = -1;
    writeln(dfs(1, k, 0));
...
_ dfs(_ x, _ y, _ st) {
    if(y == 1 || x == 0 || x == n+1) return 1;
    if(~dp[x][y][st]) return dp[x][y][st];
    if(st == 1) return dp[x][y][st] = (dfs(x-1, y, 1)+dfs(x+1, y-1, 0))%mod;
    else return dp[x][y][st] = (dfs(x-1, y-1, 1)+dfs(x+1, y, 0))%mod;
}