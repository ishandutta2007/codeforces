...
vi ord;
int vis[maxn], comp[maxn], vis2[maxn];
int intc[maxn];
...
void dfs_unmark(int v) {
    if (vis[v]) return;
    vis[v] = 1;
    mark[v] = 0;
    for (int u: e[v]) dfs_unmark(u);
}
...
int dfs_re_rm(int v) {
    ...
    for (int u: re[v]) {
        int res = dfs_re_rm(u);
        if (res % m > ans % m) ans = res;
    }
    return ans;
}

void dfs_unint(int v) {
    if (vis2[v]) return;
    vis2[v] = 1;
    intc[comp[v]] = 0;
    for (int u: e[v]) dfs_unint(u);
}
...
int main() {
    ...
    int ans = 0;
    forn(j, m) forn(i, n) {
        int ind = index(mp(i, j));
        if (intc[comp[ind]]) {
            ++ans;
            int rm = dfs_re_rm(ind);
            dfs_unint(rm);
        }
    }
