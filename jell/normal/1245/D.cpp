...
void __solve()
{
    ...
    vector<vector<i64>> e(n+1,vector<i64>(1+n));
    for(i32 i=1; i<=n; ++i)
    {
        i32 c; cin>>c;
        e[0][i]=e[i][0]=c;
    ...
    vector<i64> k(n); cin>>k;
    for(i32 i=0; i<n; i++)
    {
        for(i32 j=0; j<n; ++j)
        {
            if(i!=j) e[i+1][j+1]=(abs(p[i].first-p[j].first)+abs(p[i].second-p[j].second))*(k[i]+k[j]);
        }
    }
    ...
    bool in[2048]={};
    in[v]=1;
    vector<vector<i64>> ef(n+1,vector<i64>(1+n));
    for(i32 i=0; i<=n; ++i)
    {
        for(i32 j=0; j<=n; ++j)
        {
