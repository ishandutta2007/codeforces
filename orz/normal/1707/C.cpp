...
void dfs(const int v, const vvi& g, const vvi& h, vi& he, vi& nv, vi& us, vi& ver) {
	nv[v] = 1;
	ver.push_back(v);
	for (int to : h[v])
		if (nv[to] == 1) {
	...
	for (int to : g[v])
		if (nv[to] == 0) {
			he[to] = he[v] + 1;
			dfs(to, g, h, he, nv, us, ver);
		}
	nv[v] = 2;
	ver.pop_back();
}

void dfs1(const int v, const vvi& g, vi& us, vi& nv) {
	nv[v] = 1;
	for (int to : g[v])
		if (nv[to] == 0) {
			us[to] += us[v];
...
int another_main()
{
	...
	vvi g(n), h(n);
	for (int i = 0; i < m; ++i)
		if (mst[i] = d.unite(e[i].X, e[i].Y)) {
			g[e[i].X].push_back(e[i].Y);
			g[e[i].Y].push_back(e[i].X);
		} else {
			h[e[i].X].push_back(e[i].Y);
			h[e[i].Y].push_back(e[i].X);
		}
	vi nv(n);
	vi us(n);
	vi he(n);
	vi ver;
	dfs(0, g, h, he, nv, us, ver);
	nv.assign(n, 0);
	dfs1(0, g, us, nv);
	for (int i = 0; i < n; ++i)
		cout << !us[i];
