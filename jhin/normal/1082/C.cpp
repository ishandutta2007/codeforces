...
map<int,int> mp;
multiset<int>myset[100001],myset2;
...
{
    cin>>x>>y;
    myset[x].insert(y);int si=myset[x].size();mx=max(mx,si);
}
for(int i=1;i<=m;i++)
{
    myset2.insert(i);
...
for(int i=0;i<=m;i++)
{
    if(myset[i].size()<mx)myset[i].insert(-1e9);
}
for(int i=0;i<=m;i++)
{
    for(auto it=--myset[i].end();;it--){vecs[i].push_back(*it);if(it==myset[i].begin())break;}
}
for(int i=0;i<=m;i++){for(int j=1;j<vecs[i].size();j++)vecs[i][j]+=vecs[i][j-1];}
//for(int i=0;i<=m;i++){for(int j=0;j<vecs[i].size();j++)cout<<vecs[i][j]<<" ";cout<<endl;}
int sum2=0,sum3=0;
for(int j=0;j<mx;j++)
{ auto prev=myset2.end();sum2=0;
    for(auto it=myset2.begin();it!=myset2.end();it++)
    {
        if(prev!=myset2.end()){myset2.erase(prev);}
        int ind=*it;
        if(vecs[ind].size()>j){if(vecs[ind][j]>0){sum2+=vecs[ind][j];prev=myset2.end();}
        else
        {
            prev=it;
        }}
