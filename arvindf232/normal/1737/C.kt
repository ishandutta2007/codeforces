...
fun main(){
    solve.cases{
        ...
        val ax = minOf(L[0],L[2],L[4])
        val bx = maxOf(L[0],L[2],L[4])

        val ay = minOf(L[1],L[3],L[5])
        val by = maxOf(L[1],L[3],L[5])



        val S = mutableSetOf<Pair<Int,Int>>()
        for(i in 0 until 3){
            S.add(Pair(L[i * 2], L[i * 2+1]))
        }
        var rx = -1
        var ry = -1
        if(Pair(1,1) in S && Pair(1,2) in S && Pair(2,1) in S){
            rx = 1
            ry = 1
        }else if(Pair(1,n) in S && Pair(2,n) in S && Pair(1,n-1) in S){
            rx = 1
            ry = n
        }else if(Pair(n,1) in S && Pair(n,2) in S && Pair(n-1,1) in S){
            rx = n
            ry = 1
        }else if(Pair(n,n) in S && Pair(n-1,n) in S && Pair(n,n-1) in S){
            rx = n
            ry = n
        }
