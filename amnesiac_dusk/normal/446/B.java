...
public class Main
{
	public static void main(String[] args)
	{
		...
		for(int i=0; i<n; i++)
			for(int j=0; j<m; j++)
				a[i][j]=ni();
		PriorityQueue<Long> row=new PriorityQueue<Long>(Collections.reverseOrder());
		PriorityQueue<Long> col=new PriorityQueue<Long>(Collections.reverseOrder());
		for(int i=0; i<n; i++)
		{
			long sum=0;
			for(int j=0; j<m; j++)
		...
		long ans=0;
		PriorityQueue<Long> smarow=new PriorityQueue<Long>();
		for(int i=0; i<k; i++)
		{
			ans+=row.peek();
			smarow.add(row.peek());
			row.add(row.poll()-m*p);
		}
		long finan=ans;
		for(int i=1; i<=k; i++)
		{
			ans=ans+col.peek()-smarow.poll();
			col.add(col.poll()-n*p);
			finan=Math.max(finan, ans-i*(k-i)*p);
		}
		pr(finan);
