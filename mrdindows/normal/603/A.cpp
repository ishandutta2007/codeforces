...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        tp[i][1] = tp[i - 1][1];
        if (a[i]) {
            tp[i][0] = max(tp[i][0], tp[i - 1][1] + 1);
        } else {
            tp[i][1] = max(tp[i][1], tp[i - 1][0] + 1);
        }
        if (a[i]) {
            tp[i][0] = max(tp[i][0], dp[i - 1][1] + 1);
        } else {
            tp[i][1] = max(tp[i][1], dp[i - 1][0] + 1);
        }
        
        rp[i][0] = rp[i - 1][0];
        rp[i][1] = rp[i - 1][1];
        if (a[i]) {
            rp[i][1] = max(rp[i][1], rp[i - 1][0] + 1);
        } else {
            rp[i][0] = max(rp[i][0], rp[i - 1][1] + 1);
        }
        if (a[i]) {
            rp[i][1] = max(rp[i][1], tp[i - 1][0] + 1);
        } else {
            rp[i][0] = max(rp[i][0], tp[i - 1][1] + 1);
        }
    }
    for (int i = 1; i <= n; ++i) {
//        cout << dp[i][0] << ' ' << dp[i][1] << ' ' << tp[i][0] << ' ' << tp[i][1] << ' ' << rp[i][0] << ' ' <<rp[i][1] << endl;
    }
    int ans = max(rp[n][0], rp[n][1]);
    ans = max(ans, tp[n][0]);
    ans = max(ans, tp[n][1]);
    cout << ans << endl;
