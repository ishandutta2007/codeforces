...
int main() {
    ...
    vector<vector<int>> invs(d);
    rep(i, d) {
        invs[(10 * i) % d].push_back(i);
    }

    vector<vector<int>> dist(s + 1, vector<int>(d, -1));
    dist[0][0] = 0;

    deque<pair<int, int>> dq{ { 0, 0 } };
    while (dq.size()) {
        auto [sum, mod] = dq.front();
        dq.pop_front();
        int cur_dist = dist[sum][mod];
        for (int v = 0; v < 10; ++v) {
            int nxt_sum = sum + v;
            int nxt_mod = (10 * mod + v) % d;
            if (nxt_sum > s or dist[nxt_sum][nxt_mod] >= 0) continue;
            dist[nxt_sum][nxt_mod] = cur_dist + 1;
            dq.emplace_back(nxt_sum, nxt_mod);
        }
    }
    ...
    vector vis(s + 1, vector<int8_t>(d, false));
    vis[s][0] = true;

    vector g(s + 1, vector(d, tuple<int, int, int>{ 10, 0, 0 }));
    dq.emplace_back(s, 0);
    while (dq.size()) {
        auto [sum, mod] = dq.front();
        dq.pop_front();
        ...
        rep(val, 10) {
            ...
            int t = (mod - val) % d;
            if (t < 0) t += d;
            for (int pre_mod : invs[t]) {
                if (dist[pre_sum][pre_mod] + 1 != dist[sum][mod]) continue;
                chmin(g[pre_sum][pre_mod], make_tuple(val, sum, mod));
                if (exchange(vis[pre_sum][pre_mod], true)) continue;
                dq.emplace_back(pre_sum, pre_mod);
            }
        }
    }
    ...
    vector<int> ans;
    int sum = 0, mod = 0;
    while (sum != s or mod != 0) {
        auto [v, nxt_sum, nxt_mod] = g[sum][mod];
        ans.push_back(v);
        sum = nxt_sum;
        mod = nxt_mod;
    }
