...
int main() {
    ...
    vector<int> A(N);
    cin >> A;
    vector<RangeUpdateRangeSumMin::S> L(N), R(N);
    {
        int l = 0;
        REP(i, N) {
            L[i] = RangeUpdateRangeSumMin::gen(l);
            if (i + 1 < N and A[i] > A[i + 1]) l = i + 1;
        }
    }
    {
        int r = N - 1;
        IREP(i, N) {
            R[i] = RangeUpdateRangeSumMin::gen(r);
            if (i and A[i] < A[i - 1]) r = i - 1;
        }
    }

    RangeUpdateRangeSumMin::segtree treeL(L), treeR(R);

    auto mg_lr = [&](int l, int r) -> void {
        assert(l <= r);
        treeL.apply(l, r, {true, l});
        treeR.apply(l, r, {true, r - 1});
    };
    ...
    while (Q--) {
        ...
        if (tp == 1) {
            ...
            x--;
            A[x] = y;
            bool lconn = (x and A[x - 1] <= A[x]);
            bool rconn = (x + 1 < N and A[x] <= A[x + 1]);
            if (lconn and rconn) {
                int l = treeL.get(x - 1).minval;
                int r = treeR.get(x + 1).minval + 1;
                mg_lr(l, r);
            } else if (lconn) {
                int l = treeL.get(x - 1).minval;
                mg_lr(l, x + 1);
            ...
            cin >> l >> r;
            l--;
            auto prod = treeR.prod(l, r).sum;
            prod = prod - lint(r - l) * (l + r - 1) / 2 + (r - l);

            int ll = treeL.get(r - 1).minval, rr = treeR.get(r - 1).minval + 1;
            chmax<int>(ll, l);
            prod -= lint(r - ll) * (rr - r);
            cout << prod << '\n';
        }
