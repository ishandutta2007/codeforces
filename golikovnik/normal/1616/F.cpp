...
struct GaussMod {
  ...
  GaussMod(int nvars) : n(nvars) {}

  void add_equation(vector<mint> vec, mint value) {
    a.push_back(move(vec));
    b.push_back(value);
  }

  optional<vector<mint>> solve() {
    int m = int(b.size());
    ...
    for (int i = 0; i < m; ++i) {
      ...
      swap(b[i], b[r]);
      swap(col[i], col[c]);
      for (int j = 0; j < m; ++j) swap(a[j][i], a[j][c]);
      assert(a[i][i]);
      auto dv = a[i][i].inv();
      for (int j = i + 1; j < m; ++j) {
        auto coef = a[j][i] * dv;
    ...
    for (int v = 0; v < u; ++v) {
      for (int w = 0; w < v; ++w) {
        ...
        if (x >= 0 && y >= 0 && z >= 0) {
          ...
          cur[y] = 1;
          cur[z] = 1;
          gauss.add_equation(cur, 0);
        }
      }
    }
  }

  auto result = gauss.solve();
  if (!result) {
    cout << -1 << '\n';
  } else {
    for (int i = 0; i < m; ++i) {
