...
int main(){
  ...
  ll i,j;
  cin>>H>>W;
  mat g(H,vec(W)),tate,yoko;
  rep(i,H)rep(j,W)cin>>g[i][j];
  tate=yoko=g;
  vec mtate(W),myoko(H);
  rep(i,H){
    vec za;
    rep(j,W)za.emplace_back(g[i][j]);
    uniq(za);
    myoko[i]=0;
    rep(j,W){
      chmax(myoko[i],yoko[i][j]=lower_bound(all(za),g[i][j])-za.begin());
    }
  }
  rep(j,W){
    vec za;
    rep(i,H)za.emplace_back(g[i][j]);
    uniq(za);
    mtate[j]=0;
    rep(i,H){
      chmax(mtate[j],tate[i][j]=lower_bound(all(za),g[i][j])-za.begin());
    }
  }
  // matoutput(tate);
  // matoutput(yoko);
  mat ans(H,vec(W));
  rep(i,H)rep(j,W){
    ans[i][j]=1+max(mtate[j]-tate[i][j],myoko[i]-yoko[i][j])+max(tate[i][j],yoko[i][j]);
  }
  matoutput(ans);
// }
}