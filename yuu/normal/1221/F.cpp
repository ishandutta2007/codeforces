...
public:
    int x, y, c;
    bool operator<(const point_t& other) const { return y < other.y; }
};
vector<point_t> p;
vector<int> pos_x;
...
public:
    ...
    pair<int64_t, int> max_val;
    int64_t lazy;
    int l, r, m;

    segment_tree_t(int il, int ir, int im) : l(pos_x[il]), r(pos_x[ir]), m(pos_x[im]), lazy(0) {
        if (il == ir) {
            max_val = {r, r};
        } else {
            left = new segment_tree_t(il, im, (il + im) / 2);
            right = new segment_tree_t(im + 1, ir, (im + 1 + ir) / 2);
    ...
    void down() {
        ...
        left->lazy += lazy;
        right->lazy += lazy;
        left->max_val.first += lazy;
        right->max_val.first += lazy;
        lazy = 0;
    }

    void update(const int& lim_r, const int& c) {
        if (l > lim_r) return;
        if (r <= lim_r) {
            lazy += c;
            max_val.first += c;
            ...
            down();
            left->update(lim_r, c);
            right->update(lim_r, c);
            max_val = max(left->max_val, right->max_val);
        }
    }

    pair<int64_t, int> get(const int& lim_r) {
        if (l > lim_r) return {-1e18, -1};
        if (r <= lim_r) return max_val;
        down();
        return max(left->get(lim_r), right->get(lim_r));
    }
};

priority_queue<point_t> q;
...
void solve() {
    ...
    pos_x | UNIQUE;
    auto tree = new segment_tree_t(0, pos_x.size() - 1, (pos_x.size() - 1) / 2);
    sort(p.begin(), p.end());
    tuple<int64_t, int, int> ans = {0, 2e9, 2e9};
    for (int i = 0; i < n; i++) {
        int j = i;
        while ((j + 1 < n) && (p[j + 1].y == p[j].y)) j++;
        for (int k = i; k <= j; k++) tree->update(p[k].x, p[k].c);
        pair<int64_t, int> res = tree->get(p[i].y);
        res.first -= p[i].y;
        ans = max(ans, {res.first, res.second, p[i].y});
        i = j;
    }
    auto&& [a, x, y] = ans;
    cout << a << '\n' << x << ' ' << x << ' ' << y << ' ' << y << '\n';
