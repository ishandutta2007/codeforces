...
int main(void)
{
	...
	reps(i, s){
		if(s[i] == 'V') t[i] = 1, vflag = true;
		else t[i] = -1, cflag = true;
	}
	...
	if(!vflag) rep(i, 1, n) ts.addClause(-i, -i);
	if(!cflag) rep(i, 1, n) ts.addClause(i, i);
