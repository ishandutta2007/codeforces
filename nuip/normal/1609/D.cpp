...
int main(){
	...
	cauto norm=
		[&](){
			while(large.size()<cap && small.size()){
				cauto it=prev(small.end(),1);
				sum+=*it;
				large.emplace(*it);
				small.erase(it);
			}
			while(large.size() && small.size() && *large.begin()<*small.rbegin()){
				cauto a=*large.begin();
				cauto b=*small.rbegin();
				small.erase(prev(small.end(),1));
				large.erase(large.begin()); sum-=a;
				large.emplace(b); sum+=b;
				small.emplace(a);
			}
		};
	cauto del=
		[&](const int x){
			out("del",x,1);
			if(cauto it=large.find(x); it!=large.end()){
				large.erase(it);
				sum-=x;
			}else{
