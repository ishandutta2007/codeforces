...
void solve() {
    cin >> n >> q;
    for (auto&& [i, j, x] : r | ARRAY_1 | q) {
        cin >> i >> j >> x;
        if (i > j) swap(i, j);
    }
    sort(r + 1, r + q + 1, [](const auto& a, const auto& b) { return a.i < b.i; });
    for (int i = 1; i <= n; i++) allowed[i] = ~0;
    for (auto&& [i, j, x] : r | ARRAY_1 | q) {
        allowed[i] &= x;
        allowed[j] &= x;
    }
    ...
    for (int i = 1; i <= n; i++) {
        while ((j <= q) && (r[j].i < i)) j++;
        int k = j - 1;
        while ((k < q) && (r[k + 1].i <= i)) k++;
        for (auto&& [x, y, req] : r | ARRAY | j | k) {
            if (x == y) {
                a[i] = req;
                break;
            } else {
                a[i] |= req ^ (req & allowed[y]);
            }
        }
        for (auto&& [x, y, req] : r | ARRAY | j | k) {
            a[y] |= req ^ (req & a[i]);
    ...
    for (auto&& [x, y, req] : r | ARRAY_1 | q) assert((a[x] | a[y]) == req);
