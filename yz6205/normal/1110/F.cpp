...
template <class Type>
struct NodeAssignSum {
  using value_type = Type;
  Type sum = Type(); int len = 1;

  NodeAssignSum() =default;
  NodeAssignSum(NodeAssignSum l, NodeAssignSum r) : sum(l.sum+r.sum), len(l.len+r.len) {}
  void modify(value_type val) { sum = len * val; }

  static value_type merge(
#if __cplusplus >= 201703L
  [[maybe_unused]]
#endif
  value_type a, value_type b) { return b; }
  static constexpr value_type nop = std::numeric_limits<value_type>::min;
};
...
struct NodeAddMin {
  ...
  NodeAddMin() =default;
  NodeAddMin(NodeAddMin l, NodeAddMin r) : min(std::min(l.min, r.min)) {}
  void modify(value_type val) { min += val; }
  static value_type merge(value_type a, value_type b) { return a + b; }
  static constexpr value_type nop = 0;
};
...
struct STree {
  ...
  struct Node : public NodeType {
    ...
    value_type tag = NodeType::nop;

    void modify(value_type val) {
      tag = NodeType::merge(tag, val); NodeType::modify(val); }
    void push_down(Node& l, Node& r) {
      if (tag != NodeType::nop) {
        l.modify(tag); r.modify(tag); tag = NodeType::nop; } }
  };

  static constexpr int root = 1;
  const int g_l, g_r;
  Node *const p;
  STree(int g_r) : g_l(0), g_r(g_r), p(new Node[(unsigned long)(g_r - g_l) * 4ul]) {}
  STree(int g_l, int g_r) : g_l(g_l), g_r(g_r), p(new Node[(unsigned long)(g_r - g_l) * 4ul]) {}
