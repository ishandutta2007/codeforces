...
int main() {
    ...
    rep(ii,T){
        ...
        auto dp = v2m(N+2,2,0);
        rep(i,N){
            vector<tuple<int,int,mint>> add;
            if(a[i]!=0) add.push_back(make_tuple(a[i]-1,1,dp[a[i]-1][1]));
            add.push_back(make_tuple(a[i]+1,1,dp[a[i]+1][1]));
            if(a[i]!=0) add.push_back(make_tuple(a[i]-1,1,dp[a[i]-1][0]));
            add.push_back(make_tuple(a[i]+1,0,dp[a[i]][0]));
            add.push_back(make_tuple(a[i]+1,0,dp[a[i]+1][0]));
            if(a[i]==0) add.push_back(make_tuple(1,0,mint(1)));
            if(a[i]==1) add.push_back(make_tuple(0,1,mint(1)));
            for(auto[j,k,m]:add){
                dp[j][k] += m;
            }
        }
