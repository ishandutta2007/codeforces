...
//	DBN(m);
	...
	{
//		cout<<"    ";DB(i);
		point *v = upper_bound(a,a+n,point(a[i].x+2*m+EPS,0));
//		DB(a[i].x+2*m);
		double miny = min(minpref[i],minsuf[(v-a)]);
		double maxy = max(maxpref[i],maxsuf[(v-a)]);
		
// 		DB(miny);
// 		DB(maxy);
// 		DBN(v-a);
		if (maxy-miny<=2*m) return true;
	}
	return false;
}
int main()
{
	...
	for (int i=1;i<=n;i++)
	{
		minpref[i] = min(minpref[i-1],a[i-1].y);
		maxpref[i] = max(maxpref[i-1],a[i-1].y);
		minsuf[n-i] = min(minsuf[n-i+1],a[n-i].y);
		maxsuf[n-i] = max(maxsuf[n-i+1],a[n-i].y);
	}
	double l = 0;
	double r = max(1.0*a[n-1].x-a[0].x,1.0*maxsuf[0]-minsuf[0])/2;
	while (l+EPS<r)
	{	
		double m = (l+r)/2;
		if (check(m))
