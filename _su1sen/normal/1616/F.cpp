...
// GaussianEliminationArithmetic<mint>
...
int main() {
    ...
    loop(t) {
        input(int, n, m);
        const int none = -100;
        vector<vector<int>> g(n, vector<int>(n, none));
        vector<vector<int>> eid(n, vector<int>(n, none));
        vector<vector<int>> ttt(n, vector<int>(n, none));
        vector<int> ids;
        vector<int> ans(m);
        rep(i, m) {
            input(int, u, v, w);
        ...
        const int c = ids.size();

        rep(i, n) rep(j, i) rep(k, j) {
            if (eid[i][j] != none and eid[j][k] != none and eid[k][i] != none) {
                vector<mint> v(c, 0);
                mint sum = 0;
                if (g[i][j] >= 0) {
                    sum += g[i][j];
        ...
        if (A.size() == 0) {
            ...
            GaussianEliminationArithmetic<mint> ge(A, y);
            if (not ge.has_solution()) {
                print(-1);
                continue;
            }
            auto x = *ge.get_solution();
