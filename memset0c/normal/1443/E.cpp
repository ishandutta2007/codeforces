...
int n,m,a[N],b[M+5],tag[M+5];
long long all,s[N],fac[N];

void trans(int n,long long x,int *a){
  for(int i=1;i<=n;i++)tag[i]=false;
  for(int i=1;i<=n;i++){
    int cnt=i==n?1:x%fac[n-i+1]/fac[n-i]+1;
    for(int j=1;j<=n;j++)
      if(!tag[j]){
        cnt--;
        if(!cnt){
...
int main(){
  ...
  for(int o,l,r,x,i=1;i<=m;i++){
    ...
    if(o==1){
      ...
      trans(min(n,M),all,b);
      for(int i=1;i<=min(n,M);i++){
        a[n-min(n,M)+i]=b[i]+n-min(n,M);
        s[n-min(n,M)+i]=s[n-min(n,M)+i-1]+a[n-min(n,M)+i];
      }
