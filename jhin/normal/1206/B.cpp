...
int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[1000];vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
...
int nums[11],sums[11];long long s1,s2,e1,e2;
...
int main()
{
    ...
    ios_base::sync_with_stdio(false);
      ...
      {
          for(int i=0;i<n;i++)if(a[i]==0)a[i]=-1,sum++,x=2;
          for(int i=0;i<n;i++)
          {

              if(a[i]==0)sum++;
              else if(a[i]<0&&x==1)sum+=abs(a[i])+1,x=2;
              else if(a[i]<0&&x==2)sum+=abs(a[i])-1;
