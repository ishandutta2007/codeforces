...
inline int first(bool i = 1){return out(i?"first":"second");}
inline int yes(bool i = 1){return out(i?"yes":"no");}
inline int Yes(bool i = 1){return out(i?"Yes":"No");}
inline int No(){return out("No");}
inline int YES(bool i = 1){return out(i?"YES":"NO");}
inline int Yay(bool i = 1){return out(i?"Yay!":":(");}
inline int Possible(bool i = 1){return out(i?"Possible":"Impossible");}
inline int POSSIBLE(bool i = 1){return out(i?"POSSIBLE":"IMPOSSIBLE");}
...
int main(){
    ...
    rep(_,t){
        ...
        rep(n){
            if(!b.count(a[i]))b[a[i]]=i;
            else{
                chmin<ull>(ans,i-b[a[i]]+1);
                b[a[i]]=i;
            }
        }
        out(ll(ans));
