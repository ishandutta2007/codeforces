...
fn run() {
    ...
    for (x, y, w) in e {
        if let Some((a, b)) = dsu.unite(x, y) {
            let mut p = std::mem::take(&mut back[b]);
            back[a].append(&mut p);
            let mut p = std::mem::take(&mut tree[b]);
            tree[a].append(&mut p);
            tree[a].push((x, y, w));
        } else {
            back[dsu.root(x)].push((x, y, w));
        }
    }
    let mut ans = M::zero();
    for i in 0..n {
        ...
        let mut tree = std::mem::take(&mut tree[i]);
        let mut back = std::mem::take(&mut back[i]);
        let mut v = vec![];
        v.extend(tree.iter().map(|p| p.0));
        v.extend(tree.iter().map(|p| p.1));
        v.sort();
        v.dedup();
        for p in tree.iter_mut().chain(back.iter_mut()) {
            p.0 = v.binary_search(&p.0).unwrap();
            p.1 = v.binary_search(&p.1).unwrap();
        }
        let n = v.len();
        let mut g = vec![vec![]; n];
        for (a, b, w) in tree {
        ...
        let mut base_cnt = vec![0; w];
        for (i, c) in base_cnt.iter_mut().enumerate() {
            for b in base.iter() {
                *c += *b >> i & 1;
            }
        }
...
//        println!("{:?}", base);
//        println!("{:?}", n);
        for (i, (cnt, &base)) in cnt.iter().zip(base_cnt.iter()).enumerate() {
            let mut dp = (M::one(), M::zero());
            for _ in 0..base {
                dp = (dp.0 + dp.1, dp.0 + dp.1);
            }
            for _ in 0..(len - base) {
                dp = (dp.0 + dp.0, dp.1 + dp.1);
            }
//            println!("{:?}", dp);
            let val = M::new(2).pow(i as u64);
            ans += val * M::from(cnt * (n - cnt)) * dp.0;
            ans += val * M::from(n * (n - 1) / 2 - cnt * (n - cnt)) * dp.1;
        }
    }
    println!("{}", ans);
}
