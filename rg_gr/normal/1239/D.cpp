...
#endif
mgs
int st[1000005],cnt=1,h[1000005],low[1000005],dfn[1000005],s,n,t,c[1000005],r,S[1000005],m,ans,T,R[1000005];
struct Edge{
	int v,w,nxt;
}e[5005555];
...
signed main()
{
	...
	rg(T)
	int n=read();s=t=0;
	rg(n)dfn[i]=h[i]=st[i]=c[i]=S[i]=R[i]=low[i]=0;gr	cnt=1;
	int m=read();rg(m)
	int u=read(),v=read();
	if(u==v)continue;
	A(u,v);
	...
	for(int i=1;i<=r;i++)if(!R[i])ans=i,T++;
	if(r==1)printf("No\n");
	else 
	{
		puts("Yes");
		odp(n-S[ans],S[ans]);
		rg(n)if(c[i]!=ans)odb(i);gr puts("");
		rg(n)if(c[i]==ans)odb(i);gr puts("");
	}
