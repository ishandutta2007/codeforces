...
signed main() {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
            pos[(i + j) % 2].pb({i, j});
    auto getandanswer = [&](vector<pii> &pos, int color) {
        auto p = pos.back();
        pos.popb();
        answer(color, p.f, p.s);
    };
    for (int t = 0; t < n * n; ++t) {
        int a;
        read(a);
        ...
        if (a == 1) {
            if (sz(pos[0]) > 0) {
                getandanswer(pos[0], 0);
            } else {
                getandanswer(pos[1], 2);
            }
        } else {
            if (sz(pos[1]) > 0) {
                getandanswer(pos[1], 1);
            } else {
                getandanswer(pos[0], 2 - a);
            }
        }
    }
    #ifdef DEBUG
