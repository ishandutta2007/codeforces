...
inline void solve()
{
	n=read(),st.clear(),mx=0;
	for(register int i=1;i<=n;i++)
	{
		num[i]=read(),pre[i]=suf[i]=1;
	}
	for(register int i=1;i<=n;i++)
	{
		st.insert(num[i]),mx=max(mx,num[i]);
		if(st.size()!=i||mx!=i)
		{
			pre[i]=0;
		}
	}
	st.clear(),mx=0,reverse(num+1,num+n+1);
	for(register int i=n,j=1;i;i--,j++)
	{
		st.insert(num[j]),mx=max(mx,num[j]);
		if(st.size()!=j||mx!=j)
		{
			suf[i]=0;
		}
