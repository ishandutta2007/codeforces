...
int main() {
...
#ifdef ANAND_LOCAL
    vi secret{1, 0, 1, 1, 0, 1};
    n = sz(secret);
#endif

    map<int, int> cache;
    binary_indexed_tree bit(n, 0, plus<int>(), minus<int>());

    auto query = [&](int len) {
        if (cache.count(len)) {
            return cache[len] + bit.accumulate_prefix(len);
        }
        ...
        int sum;
#ifdef ANAND_LOCAL
        sum = accumulate(secret.begin(), secret.begin() + len, 0);
#else
        re(sum);
#endif

        cache[len] = sum - bit.accumulate_prefix(len);
    ...
    while (t--) {
        ...
        auto reached = [&](int i) {
            int zeros = i + 1 - query(i + 1);
            return zeros >= k;
        };

        int ans = binarysearch(reached, 0, n);
        finish(ans);
    }
