...
int main()
{
	...
	for(int i=0;i<n;i++){
		...
		for(int j=0;j<m;j++)
			ok[a[i][j]].pb(j);
		for(auto p:ok){
			for(int j=0;j<p.s.size()-1;j++)
				merge(convert(i,p.s[j]),convert(i,p.s[j+1]));
		}
	}
	for(int j=0;j<m;j++){
		map<int,vector<int> > ok;
		...
		for(auto p:ok){
			for(int i=0;i<p.s.size()-1;i++)
				merge(convert(p.s[i],j),convert(p.s[i+1],j));
		}
	}
	for(int i=0;i<n;i++){
		map<int,vector<int> > ok;
		...
		for(int k=0;k<mp.size()-1;k++){
			for(auto p:mp[k+1].s)
				addEdge(convert(i,mp[k].s[0]),convert(i,p));
			for(auto p:mp[k].s)
				if(p!=mp[k].s[0])
					addEdge(convert(i,p),convert(i,mp[k+1].s[0]));
		}
	}
	for(int j=0;j<m;j++){
		map<int,vector<int> > ok;
		...
		for(int k=0;k<mp.size()-1;k++){
			for(auto p:mp[k+1].s)
				addEdge(convert(mp[k].s[0],j),convert(p,j));
			for(auto p:mp[k].s)
				if(p!=mp[k].s[0])
					addEdge(convert(p,j),convert(mp[k+1].s[0],j));
		}
	}
	queue<int> q;
	for(int i=0;i<n*m;i++)
