...
#line 1 "/home/maspy/compro/library/graph/online_toposort.hpp"
...
void solve() {
  LL(N);
  VEC(ll, A, N);
  for (auto&& a: A) a = (a == -1 ? N : a - 1);
  vi FRM(N, -1);
  FOR(v, N) if (A[v] != N) FRM[A[v]] = v;

  const ll INF = 1LL << 60;
  vi time(N, INF);
  FOR(i, N) if (A[i] != N) time[A[i]] = i;

  SegTree<Monoid_Max<ll, INF>> seg(N);
  vi used(N);
  ...
  auto set_used = [&](ll v) -> void {
    used[v] = 1;
    seg.set(v, -INF);
  };

  auto find_unused = [&](ll v) -> int {
    if (FRM[v] != -1 && !used[FRM[v]]) return FRM[v];
    // v 
    auto check = [&](ll x) -> bool { return x < v; };
    ll to = seg.max_right(check, 0);
    if (to >= A[v]) return -1;
    return to;
  };

  auto V = online_toposort(N, init, set_used, find_unused, 1);
  vi ANS(N);
  FOR(i, N) ANS[V[i]] = N - i;
  print(ANS);
}
