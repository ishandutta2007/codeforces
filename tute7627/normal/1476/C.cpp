...
int main(){
  ...
  while(t--){
    ...
    ll cnt=0;
    rep(i,0,n){
      if(i<n-1&&a[i+1]>b[i+1])swap(a[i+1],b[i+1]);
      if(i!=0)chmax(mx,cnt+c[i]-1);
      if(i>0&&i<n-1)cnt+=a[i+1]+c[i]-b[i+1]-1+2;
      if(i<n-1&&a[i+1]==b[i+1])cnt=2;
      if(i<n-1)chmax(cnt,2+b[i+1]-a[i+1]);
      //cout<<i spa cnt<<endl;
    }
    cout<<mx<<endl;
  }
