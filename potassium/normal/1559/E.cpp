...
int main() {
	int n,m; R(n,m);
	vector<pii> range(n); R(range);

	auto solve = [&](int mul) {
		vector<pii> curRange(n);
		for (int i = 0; i < n; i++) {
			curRange[i].first = (range[i].first + mul - 1) / mul;
			curRange[i].second = range[i].second / mul;
			if (curRange[i].first > curRange[i].second)
				return Mint(0, MOD998);
		}

		int rem = m / mul;
		for (int i = 0; i < n; i++)
			rem -= curRange[i].first;
		if (rem < 0)
			return Mint(0, MOD998);

		vector<Mint> dp(rem + 1, Mint(0, MOD998)), ndp = dp;
		dp[0] = 1;
		for (int i = 0; i < n; i++) {
			for (int j = 1; j <= rem; j++)
				dp[j] += dp[j - 1];

			int allowed = curRange[i].second - curRange[i].first;
			for (int j = 0; j <= rem; j++)
				ndp[j] = dp[j] - (j - allowed - 1 >= 0 ? dp[j - allowed - 1] : Mint(0, MOD998));
		...
		return accumulate(all(dp), Mint(0, MOD998));
	};

	vector<int> mobius(m + 1, 1);
	for (int j = 2; j <= m; j++) {
		int last = j / sieve.mpf[j];
		if (last % sieve.mpf[j] == 0)
			mobius[j] = 0;
		else
			mobius[j] = mobius[last] * -1;
	}

	Mint ans(0, MOD998);
	for (int j = 1; j <= m; j++)
		ans += mobius[j] * solve(j);
