...
public:
    int64_t data[2][2];
    int64_t* operator[](const int& idx) { return data[idx]; }
    const int64_t* operator[](const int& idx) const { return data[idx]; }

    matrix_t() {
        for (int i = 0; i <= 1; i++)
            for (int j = 0; j <= 1; j++) data[i][j] = (i == j);
    }
    ...
    matrix_t operator*(const matrix_t& other) const {
        ...
        for (int i = 0; i <= 1; i++) {
            for (int j = 0; j <= 1; j++) {
                res[i][j] = 0;
                for (int k = 0; k <= 1; k++) {
                    (res[i][j] += data[i][k] * other[k][j]) %= base;
                }
            }
        }
        return res;
...
matrix_t power(matrix_t a, int64_t p) {
    matrix_t res;
    while (p) {
        if (p % 2) res = res * a;
        a = a * a;
...
void solve() {
    ...
    trans[0][1] = odd;
    trans[1][1] = even;
    matrix_t res = power(trans, 1LL * n * m);
    // for (int i = 0; i <= 1; i++)
    //     for (int j = 0; j <= 1; j++) cerr << res[i][j] << " \n"[j == 1];
    cout << res[0][0] << '\n';
}
