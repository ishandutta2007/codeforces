...
//inline fun BinarySearchExact(value:Long,l:Long,r:Long,find:(Int)->Int):Int?{
...
fun main(){
//    for(i in 1..100){
//        if(i % floor(sqrt(i.toDouble())).toInt() == 0 ){
//            just dei i
//        }
//    }
    solve.cases{
        ...
        var s1 = BinarySearchFirstTrue(0,(2e9).toLong()){
            it * it >= l
        }!!
        var s2 = BinarySearchLastTrue(0, (2e9).toLong()){
            it * it <= r
        }!!
        val len = (s2 -s1)
        var base = if(len < 0) 0 else (len.toLong() * 3 + 1)
        val p = s1 - 1
        if(p * (p+1) in l..r){
            base ++
        }
