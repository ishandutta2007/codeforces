...
l=read(),r=read(),u=read(),d=read();
	...
	int n=200;
	vi x;
	rg(n)rg_(j,n)x.push_back(i),x.push_back(j);gr gr
	int S=query(x);
	int l=1,r=7,ans=0;
	while(l<=r)
	{
		...
		{
			// int p=i-1>>m&1;
			if((i%d)==0)for(int j=1;j<=n;j++)x.push_back(i),x.push_back(j);
		}
		int s=query(x);
		// odp(x.size(),s);fflush(stdout);
		ns[m]=s;
		if(S%d==0&&s==S/d)ans=m,l=m+1;
		else r=m-1;
	}
	// odl(ans);return  0;
	int s=ns[ans+1];
