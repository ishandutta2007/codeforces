...
void prepare() {
    ...
    for (int n = 4; n <= 60; n += 2) {
        f[n].a[win] = c[n - 1][n / 2];  // if first player get the highest number, they win just by playing that
        // 2nd player get the highest number
        // 1st player get the 2nd highest
        (f[n].a[win] += f[n - 2].a[lose]) %= base;
        (f[n].a[lose] += f[n - 2].a[win]) %= base;
        (f[n].a[draw] += f[n - 2].a[draw]) %= base;
        // 2nd player get both highest and 2nd highest, they always win
        (f[n].a[lose] += c[n - 2][n / 2]) %= base;
    }
}
...
void solve() {
    int64_t a = 0, b = 0, d = 0;
    cin >> n;
    cout << f[n].a[win] << ' ' << f[n].a[lose] << ' ' << f[n].a[draw] << '\n';
}
