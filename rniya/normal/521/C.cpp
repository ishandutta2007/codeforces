...
int main(){
    ...
    vector<mint> sum(MAX_N),power(MAX_N);
    power[0]=1;
    for (int i=0;i<MAX_N;++i){
        sum[i]=COM.C(n-i-2,K-1)*power[i];
        if (i) sum[i]+=sum[i-1];
        if (i+1<MAX_N) power[i+1]=power[i]*10;
    }
    mint ans=0;
    for (int i=0;i<n;++i){
        if (i<n-1) ans+=(S[i]-'0')*sum[n-i-2];
        ans+=(S[i]-'0')*power[n-1-i]*COM.C(i,K);
    }
    cout << ans << '\n';
}