...
signed main()
{
	...
	int T=read();rg(T)
	int n=read(),m=read();
	rg(n*3+1)lw[i]=rw[i]=w[i]=a[i]=p[i]=x[i]=k[i]=b[i]=0;gr
	int N=0;
	rg(n)p[i]=read(),a[++N]=p[i],x[i]=read();a[++N]=p[i]-x[i];a[++N]=p[i]+x[i];gr
	std::sort(a+1,a+1+N);
	int s=std::unique(a+1,a+1+N)-a-1;
	rg(n)
	int lb=p[i]-x[i],rb=p[i]+x[i],mb=std::lower_bound(a+1,a+1+s,p[i])-a;
	lb=std::lower_bound(a+1,a+1+s,lb)-a;
	rb=std::lower_bound(a+1,a+1+s,rb)-a-1;
	add(lb,mb,1,x[i]-p[i]);
	add(mb+1,rb,-1,p[i]+x[i]);
	gr
	rg(s)k[i]+=k[i-1],b[i]+=b[i-1];gr
	rg(s)w[i]=(k[i]*a[i]+b[i])-m;gr
	rg(s)lw[i]=max(lw[i-1],w[i]);gr
	tld.build(1,1,s);
	rg(s)tld.add(1,1,s,i,i,w[i]-a[i]);gr
	// rg(s)ld[i]=max(ld[i-1],w[i]-a[i]);gr
	for(int i=s;i>=1;i--)rw[i]=max(rw[i+1],w[i]);
	trd.build(1,1,s);
	rg(s)trd.add(1,1,s,i,i,w[i]+a[i]);gr
	// for(int i=s;i>=1;i--)rd[i]=max(rd[i+1],w[i]+a[i]);
	// rg(s)odb(w[i]);gr puts("");
	rg(n)
	int lb=p[i]-x[i],rb=p[i]+x[i],mb=std::lower_bound(a+1,a+1+s,p[i])-a;
	lb=std::lower_bound(a+1,a+1+s,lb)-a;
	rb=std::lower_bound(a+1,a+1+s,rb)-a-1;
	if(lw[lb-1]>0||rw[rb+1]>0){od(0);continue;}
	// od(1);
	// -i-x+p
	// +i-x-p
	if(tld.qmax(1,1,s,lb,mb)>x[i]-p[i]){od(0);continue;}
	if(trd.qmax(1,1,s,mb,rb)>x[i]+p[i]){od(0);continue;}
	od(1);
	// add(lb,mb,1,x[i]-p[i]);
	// add(mb+1,rb,-1,p[i]+x[i]);
	gr
