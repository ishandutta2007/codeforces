...
constexpr int N = 6e5 + 10, LOG_N = 21;
...
void mian() {
  n = read(), m = read(), s = 0;
  REP(i, 1, n) a[i] = read(), p[i] = read(), t[++s] = a[i], t[++s] = a[i] - p[i], t[++s] = a[i] + p[i];
  sort(t + 1, t + s + 1), s = unique(t + 1, t + s + 1) - t - 1;
  REP(i, 1, s) d0[i] = d1[i] = 0;
  auto add = [&](int l, int r, int x, int y) {
    d0[l] += x, d1[l] += y;
    d0[r + 1] -= x, d1[r + 1] -= y;
  };
  REP(i, 1, n) {
    int x = lower_bound(t + 1, t + s + 1, a[i] - p[i]) - t;
    int y = lower_bound(t + 1, t + s + 1, a[i]) - t;
    int z = lower_bound(t + 1, t + s + 1, a[i] + p[i]) - t;
    add(x, y, -(a[i] - p[i]), 1);
    add(y + 1, z, a[i] + p[i], -1);
  }
  REP(i, 1, s) {
  ...
  REP(i, 1, n) {
    int x = lower_bound(t + 1, t + s + 1, a[i] - p[i]) - t;
    int z = lower_bound(t + 1, t + s + 1, a[i] + p[i]) - t;
    if(st._mx(1, x) > m || st._mx(z, s) > m) ans[i] = '0';
  }
  static int h[N];
  REP(i, 1, s) h[i] = hi[i] - t[i];
  st.init(s, h);
  REP(i, 1, n) if(ans[i] == '1') {
    int x = lower_bound(t + 1, t + s + 1, a[i] - p[i]) - t;
    int y = lower_bound(t + 1, t + s + 1, a[i]) - t;
    if(st._mx(x, y) > m - t[x]) ans[i] = '0';
  }
  REP(i, 1, s) h[i] = hi[i] + t[i];
  st.init(s, h);
  REP(i, 1, n) if(ans[i] == '1') {
