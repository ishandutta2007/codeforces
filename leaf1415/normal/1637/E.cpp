...
int main(void)
{
	...
	while(T--){
		...
		map<ll, ll> mp;
		rep(i, 1, n) mp[a[i]]++, vec[i].clear();
		for(auto p : mp) vec[p.se].pb(p.fi);
		rep(i, 1, n) sort(all(vec[i]));
		
		vector<ll> avec;
		rep(i, 1, n) if(sz(vec[i]) >= 1) avec.pb(i);
		
		ll ans = 0;
		reps(i, avec) reps(j, avec){
			ll u = avec[i], v = avec[j];
			if(u >= v) continue;
			ll x = vec[u].back(), y = vec[v].back();
			if(x > y) swap(x, y);
			if(S.count(P(x, y)) == 0) chmax(ans, (u+v)*(x+y));
		}
		
		reps(i, avec){
			ll u = avec[i];
			if(sz(vec[u]) <= 1) continue;
			ll y = vec[u].rbegin()[0], x = vec[u].rbegin()[1];
			if(S.count(P(x, y)) == 0) chmax(ans, (u+u)*(x+y));
		}
		
		for(auto p : S){
			ll x = p.fi, y = p.se;
			ll u = mp[x], v = mp[y];
			ll xpos = lower_bound(all(vec[u]), x) - vec[u].begin();
			ll ypos = lower_bound(all(vec[v]), y) - vec[v].begin();
			
			if(xpos > 0){
				ll nx = vec[u][xpos-1], ny = y;
				if(nx > ny) swap(nx, ny);
				if(S.count(P(nx, ny)) == 0 && nx != ny) chmax(ans, (u+v)*(nx+ny));
			}
			if(ypos > 0){
				ll ny = vec[v][ypos-1], nx = x;
				if(nx > ny) swap(nx, ny);
				if(S.count(P(nx, ny)) == 0 && nx != ny) chmax(ans, (u+v)*(nx+ny));
			}
		}
