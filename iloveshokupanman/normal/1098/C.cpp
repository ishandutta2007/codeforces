...
vll solve(ll N, ll S, ll K) {
    vll dep(N + 1);

    ll rem = N;
    ll now = 1;
    repn(i, N) {
        if (now > rem) { dep[i] = rem; break; }
        dep[i] = now;
        rem -= now;
        now *= K;
    }
    ...
    repn(i, N) {
        sum += i * dep[i];
        //cout << dep[i] << endl;
        if (dep[i] == 0) { sz = i - 1; break; }
    }
    //cout << K << endl;
    //cout << sz << " " << sum << endl;
    if (sum > S) { vll v(1, -1); return v; }
    ...
    while (1) {
        if (dep[h] == 1) { h--; }
            ...
            dep[h]--;
            sz++;
            if (S - sum <= sz - h) { dep[h + S - sum]++; break; }
            else { dep[sz]++; sum += sz - h; }
        }
    }
    ...
    now = 0;
    repn(i, N) {
        lis[i].assign(dep[i], 0);
        rep(j, dep[i]) { lis[i][j] = now + 1; now++; }
    }

    vll p(N + 1);
    for (ll i = 2; i <= N; i++) {
        rep(j, dep[i])p[lis[i][j]] = lis[i - 1][j / K];
    }
...
int main() {
    ...
    if (S > N * (N + 1) / 2 || S < 2 * N - 1) { cout << "No" << endl; return 0; }
