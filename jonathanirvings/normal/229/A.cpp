...
int main()
{
	...
	REP(k,c)
	{
		...
		REP(i,r)
		{
			int L = upper_bound(ALL(pos[i]),k) - pos[i].begin() - 1;
			int R = upper_bound(ALL(pos[i]),k) - pos[i].begin();
			moves += min(k - pos[i][L],pos[i][R] - k);
		}
		MIN(risan,moves);
	}
	printf("%d\n",risan);
