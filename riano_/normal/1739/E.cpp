...
int main(){
    ...
    rep(i,2){
        rep(j,N){
            char c; cin >> c; g[i][j] = (ll)(c-'0');
        }
    }
    auto dp = v3ll(N+1,2,4,-2e9); dp[0][0][0] = 0;
    rep(i,N){
        rep(j,2){
            rep(k,4){
                ...
                if(k==3) l ^= 1;
                rep(m,4){
                    if((m&1)&&g[0][i]==0) continue;
                    if((m&2)&&g[1][i]==0) continue;
                    if(j==1&&(k&1)&&(m&2)) continue;
                    if(j==0&&(k&2)&&(m&1)) continue;
                    ll s = 0;
                    if(m&1) s++;
                    if(m&2) s++;
                    chmax(dp[i+1][l][m],dp[i][j][k]+s);
                }
            }
        }
    }
    rep(i,2){
        rep(j,4) chmax(ans,dp[N][i][j]);
    }
    cout << ans << endl;
}