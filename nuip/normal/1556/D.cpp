...
// vector<int> ans{1,6,4,2,3,5,4};
vector<int> ans{1,6,6};
int main(){
	...
	reps(i,1,n){
		...
		ands[i]=(ans[0]&ans[i]);
#else
		cout<<"or 1 "<<i+1<<endl; cout.flush();
		if((ors[i]=read())<0) exit(0);
		cout<<"and 1 "<<i+1<<endl; cout.flush();
		if((ands[i]=read())<0) exit(0);
#endif
		xors[i]=ors[i]^ands[i];
	}
	vector<int> vs(n);
	...
	rep(d,30){
		int found=0;
		reps(i,1,n)if(~xors[i]>>d&1){
			if(ors[i]>>d&1) vs[0]|=1<<d;
			found=1;
			break;
		}
		if(!found) nf|=1<<d;
	...
	if(nf){
		...
		int res;
		if((res=read())<0) exit(0);
		rep(d,30)if(nf>>d&1){
			if(~res>>d&1) vs[0]|=1<<d;
		}
	}
	reps(i,1,n) vs[i]=vs[0]^xors[i];
	out(vs,1);
	sort(all(vs));
	cout<<"finish "<<vs[t]<<endl; cout.flush();
	// vector<int> inds(n); iota(all(inds),0);
	// sort(all(inds),[&](cauto &i,cauto &j){ return vs[i]<vs[j];});
	// cout<<inds
	return 0;
}