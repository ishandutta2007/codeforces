...
vector<int>& operator /= (vector<int> &aa, const vector<int> &bb) {
    auto a = aa;
    auto b = bb;
...
vi my_exponent(vi &a) {
    vi b(1, 1);
    while (b.size() < a.size()) {
        int sz = min(a.size(), b.size() << 1);
        vector<int> c(a.begin(), a.begin() + sz);
        vi bb = b;
        bb.resize(sz);
        c -= logarithm(bb);
        FOR (i, b.size()) {
