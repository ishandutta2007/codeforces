...
int n;
int a[100001];
multiset<int> inside, outside, all;
int ans;

void update_ans() {
    if (inside.empty() || outside.empty()) return;
    ans = max(ans, *inside.rbegin() + *outside.rbegin() - *inside.begin() - *outside.begin());
}
...
void sweep(int pos) {
    ...
    for (int i = pos; i <= n; i++) {
        inside.insert(a[i]);
        outside.erase(outside.find(a[i]));
        update_ans();
    }
    inside.clear();
    outside = all;
