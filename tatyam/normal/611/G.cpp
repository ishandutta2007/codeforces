...
template<class T> auto abs(P<T> a){ return sqrt(dot(a, a)); }
...
int main(){
    ...
    VEC(P<ll>,a,n);
    Rev(a);
    rep(n)a.push_back(a[i]);
    vec(P<Modint>,b,n*2);
    rep(n*2)b[i]={a[i].first,a[i].second};
    PartialSum<P<Modint>>point_sum(b);
    Modint ans=0;
    ll s0=0,S=0,scene=0;
    Modint s1=0;
    rep(i,1,n-1)S+=cross(a[i]-a[0],a[i+1]-a[0]);
    ll j=1;
    rep(n){
        while(1){
            const ll diff=cross(a[j]-a[i],a[j+1]-a[i]);
            if((s0+diff)>S/2)break;
            s0+=diff;
            s1+=s0;
            j++;
        }
        ...
        if(j-i==1)j++;
            ...
            s1-=cross(b[i+1]-b[i],point_sum.get(i+2,j+1)-b[i]*Modint{j-i-1});
            s0-=cross(a[i+1]-a[i],a[j]-a[i]);
        }
    }
    out(Modint(S)*scene-ans*2);
}