// Kill the Monster (https://codeforces.com/contest/1633/problem/C)
...
int main() {
    int t; read(t);
    while (t--) {
        ll hc_, dc, hm_, dm, k, w, a; read(hc_, dc, hm_, dm, k, w, a);
        __int128_t hc = hc_, hm = hm_;
        bool pos = false;
        rep(i, k + 1) {
            auto hc2 = hc + i * a;
            auto dc2 = dc + (k - i) * w;
            auto t1 = (hm + dc2 - 1) / dc2;
            auto t2 = (hc2 + dm - 1) / dm;
            if (t1 <= t2) {
                pos = true;
                break;
