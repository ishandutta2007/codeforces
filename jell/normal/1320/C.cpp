#line 1 "codeforces-workspace/contest/1320/c/c.cpp"
...
#line 6 "codeforces-workspace/contest/1320/c/c.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1320/c/c.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1320/c/c.cpp"
...
class lazy_segment_tree {
  static_assert(std::is_same<Monoid, typename Monoid_container::value_type>::value);

  static_assert(
      std::is_same<Endomorphism, typename Endomorphism_container::value_type>::value);
...
#line 27 "codeforces-workspace/contest/1320/c/c.cpp"
...
void main() {
  ...
  struct mono {
    i64 v = -1e18;
    mono operator+(mono r) { return {max(v, r.v)}; }
  ...
  vector<tuple<int, int, int>> mo(p);
  cin >> a >> b >> mo;
  sort(begin(a), end(a));
  sort(begin(b), end(b));
  sort(begin(mo), end(mo));

  lazy_segment_tree<mono, endo> sgt(m);
  for (auto &&[_b, _s] : zip(b, sgt)) {
    _s.v = -_b.second;
  }
  ...
  for (auto &&[e, c] : a) {
    while (mo.end() != iter) {
      ...
      if (x < e) {
        ++iter;
        sgt.update(upper_bound(begin(b), end(b), pair{1 + y, 0}) - begin(b),
                   sgt.size(), {z});
      } else
        break;
    }
    chge(ans, sgt.fold().v - c);
  }
  cout << ans << "\n";
}
