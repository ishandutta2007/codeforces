...
struct suffix_array {
    ...
    mutable sparse_table<int> lcp_between_ranks_rmq{std::min<int>};
    int longest_common_prefix(int i, int j) const {
        if (lcp_between_ranks_rmq.data.empty())
            lcp_between_ranks_rmq.construct(SZ - 1, [&](int r) { return lcp_between_ranks[r]; });
    ...
    mutable sparse_table<int> by_rank_rmq{std::min<int>};
    int first_occurrence_of(int pos, int len) const {
        if (by_rank_rmq.data.empty())
            by_rank_rmq.construct(SZ, [&](int r) { return first_suffix_with_rank[r]; });
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < i; j++) {
            if (sa.rank_of_suffix(j) < sa.rank_of_suffix(i)) {
                int lcp = sa.longest_common_prefix(j, i);
                ckmax(dp[i], dp[j] - lcp);
            }
        }
        dp[i] += n - i;
