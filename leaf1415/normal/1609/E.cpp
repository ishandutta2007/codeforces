...
int main(void)
{
	...
	rep(i, 0, 2) mat[i] = Matrix::Ident(3);
	mat[0].at(1, 1) = 1, mat[0].at(2, 1) = 0;
	mat[1].at(2, 2) = 1, mat[1].at(3, 2) = 0;
	mat[2].at(3, 3) = 1;
	
	rep(i, 0, (1<<17)-1){
		if(i >= 1 && i <= n) seg.seg[(1<<17)+i] = mat[s[i]-'a'];
		else seg.seg[(1<<17)+i] = Matrix::Ident(3);
	}
	per(i, (1<<17), 1) seg.seg[i] = seg.seg[i*2+1] * seg.seg[i*2];
	
	//rep(i, 0, n) outl(seg.query(1, i));
	
	ll p; char c;
	rep(q, 1, Q){
		cin >> p >> c;
		seg.update(p, mat[c-'a']);
		
		Matrix res = seg.query(1, n);
		ll ans = inf;
		rep(i, 1, 3) chmin(ans, res.at(i, 1));
		cout << ans << "\n";
	}
