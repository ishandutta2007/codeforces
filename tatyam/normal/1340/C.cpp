...
signed main(){
    ...
    q.push_back({0,0,0});
    while(q.size()){
        ll at=q.front()[2],sec=q.front()[1],red=q.front()[0];
        q.pop_front();
        if(at){
            ll sec2=sec+d[at]-d[at-1];
            if(sec2==g){
                if(chmin(dp[at-1][0],red+1))q.push_back({red+1,0,at-1});
            }
            elif(sec2<g){
                if(chmin(dp[at-1][sec2],red))q.push_front({red,sec2,at-1});
            }
        }
        if(at+1<m){
            ll sec2=sec+d[at+1]-d[at];
            if(sec2==g){
                if(chmin(dp[at+1][0],red+1))q.push_back({red+1,0,at+1});
            }
            elif(sec2<g){
    ...
    ull ans=-1;
    rep(g)if(dp.back()[i]!=LINF)chmin(ans,dp.back()[i]*(g+r)+(i?i:-r));
    out(ll(ans));
}