...
const int maxn=500005,maxm=30,maxk=7777714;

//i_am_noob
#define wiwihorz  
int n,a[maxn],b[maxn],c[maxn],pos[maxn];
sparse<int,maxn,1> spmax;
sparse<int,maxn,0> spmin;
...
void orzck(){
    ...
    while(cur>0){
        if(a[cur]>a[cur-1]){
            cur=pos[spmin.query(b[cur]+1,cur)];
        }
        else{
            cur=pos[spmax.query(c[cur]+1,cur)];
        }
        res++;
    }
    print(res);
