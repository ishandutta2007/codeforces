...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        ll f[m][LG];
        FOR(i, 1, m) f[i][0] = c[i];
        FOR(p, 1, LG-1) FOR(i, 1, m+1-(1ll << p)) {
            f[i][p] = min(f[i][p-1], f[i+(1ll << (p-1))][p-1]);
            // dbg(i, p, f[i][p]);
        }

        FOR(col, 1, n) {
            ll sz = seg[col].size(), tot = 1;
            REP(i, sz-1) {
                ll le = seg[col][i]+1, ri = seg[col][i+1];
                dbg(le, ri);
                ll mx = n+2;
                ROF(p, LG-1, 0) if (le+(1ll << p) <= ri) {
                    // dbg(le, f[le][p]);
                    chkmin(mx, f[le][p]);
                    le += (1ll << p);
                }
