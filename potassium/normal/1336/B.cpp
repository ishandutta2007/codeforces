...
ll solve(vi& a, vi& b, vi& c) {
	...
	for (auto& x : a) {
		bool has = 1;
		auto yl = upper_bound(all(b), x);
		auto zh = lower_bound(all(c), x);
		if (yl != b.begin()) yl--; else has = 0;
		if (zh == c.end()) has = 0;

		if (has)
			chkmin(ans, eval(x, *yl, *zh));
	}
	return ans;
}
int main() {
	CASE(t) {
		int nr,ng,nb; R(nr,ng,nb);
		vector<int> r(nr), g(ng), b(nb); R(r,g,b);
		sort(all(r)); sort(all(g)); sort(all(b));
		
		ll ans = LLONG_MAX;
		chkmin(ans, solve(r,g,b));
		chkmin(ans, solve(r,b,g));
		chkmin(ans, solve(g,b,r));
		chkmin(ans, solve(g,r,b));
		chkmin(ans, solve(b,r,g));
		chkmin(ans, solve(b,g,r));
