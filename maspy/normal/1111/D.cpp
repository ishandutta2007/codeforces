...
void solve() {
  ...
  mint coef = fact<mint>(N) * fact<mint>(N);
  for (auto&& a: A) coef *= fact_inv<mint>(a);

  vc<mint> DP(N + 1);
  DP[0] = 1;
  for (auto&& a: A) { FOR_R(i, N - a + 1) DP[i + a] += DP[i]; }

  auto add = [&](ll a) -> void { FOR_R(i, N - a + 1) DP[i + a] += DP[i]; };
  auto rm = [&](ll a) -> void { FOR(i, N - a + 1) DP[i + a] -= DP[i]; };
