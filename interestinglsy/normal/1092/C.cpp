...
int n;
string s[MAXN], os[MAXN];
int tot;

bool Cmp( string a , string b ){
	int len = a.length();
	For0(i,len) if(a[i]!=b[i]) return 0;
	return 1;
}
bool Suffix( string a , string b ){
	int len = a.length();
	int lenb = b.length();
	for( int i = len, j = lenb ; i ; i--,j-- )
...
void Solve1(){
	string ans;
	string t;
	if( s[tot-1][0] + s[tot] == s[tot-1] + s[tot].back() ) t = s[tot-1][0] + s[tot];
	else return;
	Ms(have);
	For(i,tot){
		if( !have[os[i].length()] && Cmp(os[i],t) ){
			ans += 'P';
			have[os[i].length()] = 1;
		}else{
			if(Suffix(os[i],t))
...
int main(){
	...
	sort(s+1,s+1+tot,[]( string a , string b ){ return a.length() < b.length(); });
