...
int main()
{
    ...
    do
    {
        ...
        forn(i, n)
        {
            if (ans[i] != -1)
            {
                if (find(all(tmp[ans[i]]), ans[(n + i - 1) % n]) != tmp[ans[i]].end())
                {
                    tmp[ans[i]].erase(find(all(tmp[ans[i]]), ans[(n + i - 1) % n]));
                    tmp[ans[(n + i - 1) % n]].erase(find(all(tmp[ans[(n + i - 1) % n]]), ans[i]));
                }
                else
                {
                    flag = false;
                ...
                if (find(all(tmp[ans[i]]), ans[(n + i - 2) % n]) != tmp[ans[i]].end())
                {
                    tmp[ans[i]].erase(find(all(tmp[ans[i]]), ans[(n + i - 2) % n]));
                    tmp[ans[(n + i - 2) % n]].erase(find(all(tmp[ans[(n + i - 2) % n]]), ans[i]));
                }
                else
                {
                    flag = false;
    ...
    }while(next_permutation(all(g[0])));
    puts("-1");
    return 0;
}