...
int32_t main(){
    ...
    while( tc-- ){
        ...
        satisfiability_twosat sat2(m);
        for(int i=0; i<n; i++){
            if( !a[i] ){
                sat2.implication(vx[i][0],sat2.neg(vx[i][1]));
                sat2.implication(sat2.neg(vx[i][1]),vx[i][0]);
            }
            else{
                sat2.implication(vx[i][0],vx[i][1]);
                sat2.implication(vx[i][1],vx[i][0]);
            }
        }
