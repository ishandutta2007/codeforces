...
void main_() {
	...
	for(const auto &[val, cn] : cnt) {
		chmax(res_zero, cn);
		if(val != 1 and ~cn & 1) sub += cn;
	}
	int res_one = max(res_zero, cnt[1] + sub);
	foa(v, w)
	    print(v ? res_one : res_zero);
}
