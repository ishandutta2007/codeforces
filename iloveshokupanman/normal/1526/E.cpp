...
void dfs(vll v) {
    if (v.size() == 7) {
        auto a = suffix_array<ll>(v);
        vector<int> b = { 3,2,4,1,0,5,6 };
        if (a == b) {
           for (ll x : v)cout << x << " ";
           cout << endl;
        }
...
int main() {
    ...
    rep(i, N - 1) {
        ...
        if (s[i + 1] == N - 1) { div[i] = 1; }
        else if (s[i] == N - 1) { div[i] = 0; }
        else {
            if (ivs[s[i + 1] + 1] < ivs[s[i] + 1]) { div[i] = 1; }
        }
    }


    ll count = 0;
    rep(i, N - 1)count += div[i];

    //rep(i, N - 1)cout << div[i] << endl;
  
    cominit(N +K+ 1);
    //cout << count << endl;
    if (count > K - 1) { cout << 0 << endl; }
    else {
        mint ans = com(N+K-1-count,K-1-count);
        cout << ans.val() << endl;
    }
}