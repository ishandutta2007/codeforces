...
void solve() {
  ...
  auto naive_2 = [&](vc<int>& X1, vc<int>& Y1, vc<int>& X2, vc<int>& Y2,
                     bool allow_tie) -> ll {
    const int N1 = len(X1), N2 = len(X2);
    ll ANS = 0;
    FOR(i, N1) FOR(j, N2) {
      ll mi = min(X1[i], X2[j]);
      ll ma = max(Y1[i], Y2[j]);
      if (allow_tie && Y1[i] < Y2[j]) continue;
      if (!allow_tie && Y1[i] <= Y2[j]) continue;
      if (ma % mi == 0) ++ANS;
    }
    return ANS;
  ...
  auto f = [&](int L, int M, int R) -> ll {
    ...
    if (N1 + N2 <= 100) {
      ll v1 = naive_2(X1, Y1, X2, Y2, true);
      ll v2 = naive_2(X2, Y2, X1, Y1, false);
      return v1 + v2;
    }
    ...
    /*
    assert(naive(X1, Y1, X2, Y2)
           == naive_2(X1, Y1, X2, Y2, true) + naive_2(X2, Y2, X1, Y1, false));
    print("X1", X1);
    print("Y1", Y1);
    print("X2", X2);
    print("Y2", Y2);
    print("expected", naive_2(X1, Y1, X2, Y2, true));
    print("me", v1);
    flush();
    assert(v1 == naive_2(X1, Y1, X2, Y2, true));
    print("X1", X1);
    print("Y1", Y1);
    print("X2", X2);
    print("Y2", Y2);
    print("expected", naive_2(X2, Y2, X1, Y1, false));
    print("me", v2);
    flush();
    assert(v2 == naive_2(X2, Y2, X1, Y1, false));
    // assert(naive(X1, Y1, X2, Y2) == v1 + v2);
    */
    return v1 + v2;
  };
