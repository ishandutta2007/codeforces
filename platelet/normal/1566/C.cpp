...
void solve() {
    ...
    int ls0 = 0, ls1 = 0, ls01 = 0;
    rep(i, 1, n) {
        if(s[i] == 48 && t[i] == 48) ls0 = i;
        else if(s[i] == 49 && t[i] == 49) ls1 = i;
        else ls01 = i;
        if(ls01 == i) f[i] = f[i - 1] + 2;
        if(ls0 == i) {
            if(ls01 == i - 1) f[i] = f[i - 1] + 1;
            else f[i] = max(f[i - 1] + 1, f[i - 2] + 2);
        }
        if(ls1 == i) f[i] = ls0 || ls01 ? f[max(ls0, ls01) - 1] + 2 : 0;
        // cout << i << ' ' << f[i] << ' ' << ls0 << ' ' << ls1 << endl;
    }
    printf("%d\n", f[n]);
}
int main() {
