...
int main() {
	CASE(t) {
		...
		for (int i = 1; i <= n; i++) {
			int pairs = (n - i);
			if (l <= pairs * 2) {
				int LBound = max(l, 0) >> 1;
				int RBound = min(r, pairs * 2 - 1) >> 1;
				bool skipLi = max(l, 0) % 2;
				bool skipRj = (1 - min(r, pairs * 2 - 1) % 2) % 2;
//				W(l, r, LBound, RBound, skipLi, skipRj);
				for (int j = i + 1 + LBound; j <= i + 1 + RBound; j++) {
					if (!(j == i + 1 + LBound && skipLi)) ans.pb(i);
					if (!(j == i + 1 + RBound && skipRj)) ans.pb(j);
				}
			}
			l -= pairs * 2;
			r -= pairs * 2;
