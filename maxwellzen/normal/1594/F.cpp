...
void solve() {
    ...
    // int thresh = 100000;
    // s = rng()%thresh+1, n = rng()%thresh+1, k = rng()%thresh+1;
    ll cnt = s/k, numup = s%k;
    ll mx = 0;
    if (numup==0) {
        if (k-1 > 0 && n / (k-1) < ((cnt+1)/2)) mx = n;
        mx = (k-1)*((cnt+1)/2) + cnt/2;
    } else {
        if (n / numup < ((cnt+2)/2)) mx = n;
        else if (k-numup-1>0 && n / (k-numup-1) < ((cnt+1)/2)) mx = n;
        else 
        mx = cnt/2 + (numup)*((cnt+2)/2) + (k-numup-1)*((cnt+1)/2);
    }
    if (mx<n) YES;
    else NO;
