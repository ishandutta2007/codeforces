...
signed main(){
    ...
    rep2(j,1,23){
        ...
        rep(i,(1<<j)) cnt[j-1][i+1]+=cnt[j-1][i], cnt2[j-1][i+1] += cnt2[j-1][i];
    }
    int ans = 10000;
    vi best(23);
    int now[23]={};
    auto solve = [&](auto &&f,int last,int last2,int k,int sum)->void{
        bool flag = false;
        // 
        if(cnt[k][(1<<k) - last - 1] < cnt[k][(1<<k)-1]) flag = true;
        if(cnt[k][(1<<(k+1)) - last - 1] > cnt[k][(1<<k) - 1]) flag = true;
        if(cnt2[k][(1<<k) - last2 -1] < cnt2[k][(1<<k)-1]) flag = true;
        if(cnt2[k][(1<<(k+1)) - last2 - 1] > cnt2[k][(1<<k)-1]) flag = true;
        if(k == 18){
            if(flag)return;
            if(chmin(ans,sum)) rep(i,23)best[i]=now[i];
            return;
        }
        if(flag){
            now[k] = 1<<k;
            f(f,last,last2 + (1<<k),k+1,sum+1);
            now[k] = -(1<<k);
            f(f,last + (1<<k),last2,k+1,sum+1);
        }
        else{
            now[k] = 0;
            f(f,last,last2,k+1,sum);
        }
    };solve(solve,0,0,0,0);
    cout << ans << endl;
    for(auto e:best){
