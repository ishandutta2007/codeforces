...
void pushup(int u) {for(int i=0;i<=5;i++)sum[u][i]=(sum[u<<1][i]+sum[u<<1|1][i])%mod;}
void make(int u,int l,int r,int v){cov[u]=v;for(int i=0;i<=5;i++)sum[u][i]=1ll*(pws[r][i]-pws[l-1][i]+mod)*v%mod;}
...
int query(int u,int l,int r,int ql,int qr,int k) 
{
	...
	pushdown(u,l,r,mid);
	if(ql<=mid) 
		sum=query(u<<1,l,mid,ql,qr,k);
	if(qr>mid) 
		sum=(sum+query(u<<1|1,mid+1,r,ql,qr,k))%mod;
	return sum;
}
