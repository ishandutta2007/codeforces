...
int main() {
  ...
  rep(t) {
    int a = IN, b = IN, c = IN;
    int res = INF; array<int, 3> ans;
    rep(B, 1, c*2+10) {
      int db = abs(b-B);
      int da = INF; int A0;
      rep(A, 1, sqrt(B)+1) if (B % A == 0) {
        if (chmin(da, abs(a-A))) A0 = A;
        if (chmin(da, abs(a-B/A))) A0 = B/A;
      }
      int r = (c + B) % B;
      PII dc_c = r < B-r ? PII{r, c/B*B} : PII{B-r, c/B*B+B};
      if (chmin(res, da + db + dc_c._1)) ans = {A0, B, dc_c._2};
    }
    OUT(res);
    OUT.range(ans);
  }
