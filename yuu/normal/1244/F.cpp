...
void solve() {
    ...
    while (s[found] == s[(found + 1) % n]) {
        found = (found + 1) % n;
        if (done[found]) break;
        done[found] = 1;
    }
    if (s[found] == s[(found + 1) % n]) {  // all the same char
        cout << s << '\n';
        return;
    }

    for (int i = (found + 1) % n; !done[i]; i = (i + 1) % n) {
        // i is the start of an alternating block
        int j = i;
        while (s[(j + 1) % n] != s[j]) {
            done[j] = 1;
            j = (j + 1) % n;
        ...
        // j is the start of the next equal block
        // WWWWBWBWBWBWBBBBBB
        ...
        int x = i;
        char value = s[i] ^ 'B' ^ 'W';
        int first_block = 0, alternating = 0, last_block = 0;
        if ((len + 1) / 2 <= k) {
            // can finish this block
            first_block = len / 2;
            last_block = (len + 1) / 2;
        } else {
            first_block = last_block = k;
            alternating = len - k * 2;
        }
        while (first_block + alternating + last_block) {
            s[x] = value;
            x = (x + 1) % n;
            if (first_block) {
                first_block--;
                if (first_block == 0) {
                    if ((len % 2 == 0) || alternating) value ^= 'B' ^ 'W';
                }
            } else if (alternating) {
                alternating--;
                value ^= 'B' ^ 'W';
        ...
        // j is the last of some equal block
        i = j;
    }
    cout << s << '\n';
}
