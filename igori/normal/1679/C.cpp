...
void add(vi &fenw, int pos, int x)
{
    while (pos < fenw.size())
    {
        fenw[pos] += x;
...
void solve()
{
    ...
    vector<int> fenwRows(n), fenwCols(n);
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        if (t == 3)
        {
            ...
            cin >> x1 >> y1 >> x2 >> y2;
            x1--, y1--, x2--, y2--;
            if (get(fenwRows, x2) - get(fenwRows, x1 - 1) == x2 - x1 + 1)
                cout << "Yes\n";
            else if (get(fenwCols, y2) - get(fenwCols, y1 - 1) == y2 - y1 + 1)
                cout << "Yes\n";
            else
                cout << "No\n";
        }
