...
int main(){
    ...
    rep(ii,T){
        ...
        string S,T; cin >> S >> T;
        ll a = 0,b = 0,c = 0;
        vector<ll> sab,sac,sbc;
        rep(i,N){
            if(S[i]=='a') a++;
            if(S[i]=='b'){
                b++; sab.emplace_back(a);
            }
            if(S[i]=='c'){
                c++; sac.emplace_back(a); sbc.emplace_back(b);
            }
        }
        ll ta = 0,tb = 0,tc = 0;
        vector<ll> tab,tac,tbc;
        rep(i,N){
            if(T[i]=='a') ta++;
            if(T[i]=='b'){
                tb++; tab.emplace_back(ta);
            }
            if(T[i]=='c'){
                tc++; tac.emplace_back(ta); tbc.emplace_back(tb);
            }
        }
        string ans = "YES";
        if(a!=ta||b!=tb||c!=tc) ans = "NO";
        else{
            rep(j,sab.size()){
                if(sab[j]<tab[j]) ans = "NO";
            }
            rep(j,sbc.size()){
                if(sbc[j]<tbc[j]) ans = "NO";
            }
            rep(j,sac.size()){
                if(sac[j]!=tac[j]) ans = "NO";
            }
        }
        cout << ans << "\n";
    }
