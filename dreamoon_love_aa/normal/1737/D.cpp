...
void solve() {
    ...
    VI dd(n,1000000000);
    REP(i,n)REP(j,n){
        dd[i]=min(dd[i],dis[i][j]+dis[j][0]+dis[j][n-1]+1);
    }
    LL an=1e18;
    REP(i,m){
        an=min(an,(1LL+min({dis[0][xx[i]]+dis[n-1][yy[i]],dis[n-1][xx[i]]+dis[0][yy[i]],dd[xx[i]],dd[yy[i]]}))*vv[i]);
    }
    W(an);
}
void input() {
