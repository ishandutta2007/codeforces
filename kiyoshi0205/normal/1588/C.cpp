...
constexpr datas op(datas x,datas y){
  if(x.first==y.first)return datas(x.first,x.second+y.second);
  return x.first<y.first?x:y;
}
constexpr datas e(){return datas(inf,0);}
constexpr datas mapping(datas x,ll f){return datas(x.first+f,x.second);}
...
constexpr bool is0ijou(datas x){return x.first>=0;}

ll solve(int N,vec a){
  int i;
  vector<datas> init[2];
  {
    ll bf=0;
    rep(i,N)init[i&1].emplace_back(bf=a[i]-bf,1);
  }
  segtree seg[2]={segtree(init[0]),segtree(init[1])};
  ll ans=0;
  rep(i,N){
    const int r1=seg[0].max_right((i+1)/2,is0ijou);
    const int r2=seg[1].max_right(i/2,is0ijou);
    const int r=min({r1*2,r2*2+1,N});
    {
      datas x=seg[0].prod((i+1)/2,(r+1)/2);
      if(x.first==0)ans+=x.second;
      x=seg[1].prod(i/2,r/2);
      if(x.first==0)ans+=x.second;
    }
    seg[i&1].apply(0,init[i&1].size(),-a[i]);
    seg[~i&1].apply(0,init[~i&1].size(),a[i]);
  }
  return ans;
}
