...
fun main(){
    solve.cases{
        ...
        val freq2 = TreeMap<Int,Int>()
        for(v in L){
            freq[v] = freq.getOrDefault(v,0) + 1
//            freq2[v] = freq2.getOrDefault(v,0) + 1
        }
        var taken =0
        val stuff = if(a < b) freq.keys.reversed() else freq.keys.toList()
        for(v in stuff){
            if(v % k != 0) continue
            val count = freq.getOrDefault(v,0)
            val other = v/k
            val othercount = freq.getOrDefault(other,0)
            val mm = minOf(othercount/a, count / b)
            freq[v] = freq[v]?.minus(mm * b) ?: 0
            freq[other] = freq[other]?.minus(mm * a) ?: 0
            taken += mm
        }
        put(taken)
