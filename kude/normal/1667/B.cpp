...
struct value_compression : vector<S> {
  ...
  while(tt--) {
    ...
    value_compression<ll> vc(move(s));
    vc.build();
    int sz = vc.size();
    segtree<int, op, e> seg1(sz), seg2(sz);
    VI vs(sz, -INF);
    seg1.set(vc(0), 0), seg2.set(vc(0), 0);
    vs[vc(0)] = 0;
    ll now = 0;
    rep(i, n) {
      now += a[i];
      int p = vc(now);
      int v = max({seg1.prod(0, p) + (i + 1), seg2.prod(p+1, sz) - (i + 1), vs[p]});
      if (i == n - 1) {
        cout << v << '\n';
        break;
      }
      seg1.set(p, max(seg1.get(p), v - (i + 1)));
      seg2.set(p, max(seg2.get(p), v + (i + 1)));
      chmax(vs[p], v);
    }
  }
}