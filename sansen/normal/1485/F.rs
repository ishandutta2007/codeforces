...
fn run<W: Write>(sc: &mut scanner::Scanner, out: &mut std::io::BufWriter<W>) {
    ...
    for _ in 0..t {
        ...
        for &b in b[1..].iter() {
            ...
            let mut sub = dp.get(&-add).map_or(M::zero(), |v| *v);
            add += b;
            *dp.entry(b - add).or_insert(M::zero()) += sum - sub;;
            sum = sum + sum - sub;
            ...
            for (s, v) in dp {
                let a = b - s;
                *next.entry(b).or_insert(M::zero()) += v;
                if b != a {
                    *next.entry(s + b).or_insert(M::zero()) += v;
                }
            }
            dp = next;
            */
        }
        let ans = dp.values().fold(M::zero(), |s, a| s + *a);
        writeln!(out, "{}", ans).ok();
    }
}