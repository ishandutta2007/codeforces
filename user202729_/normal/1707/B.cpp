// time-limit: 1000
// problem-url: https://codeforces.com/contest/1707/problem/B

// I should upsolve all these problems some time soon
// okay, PE is in summer break
...
void up(){
	...
	int numZero=int(std::upper_bound(begin(a), end(a), 0)-a.begin());
	a.erase(a.begin(), a.begin()+numZero);
	...
	while(a.size()>1){
		...
		if(numZero==0){
			std::swap(a[0], a.back());
			a.pop_back();
		}else --numZero;  //  almost forgot this condition. Fortunately sample are strong

		std::sort(begin(a), end(a));

		int numZeroNew=int(std::upper_bound(begin(a), end(a), 0)-a.begin());
		a.erase(a.begin(), a.begin()+numZeroNew);
		numZero+=numZeroNew;
	}

	std::cout<<(a.empty() ? 0: a[0])<<'\n';
