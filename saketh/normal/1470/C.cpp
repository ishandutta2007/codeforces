...
int read(int pos) {
    ...
    if (local) {
        ...
        FOR (i, 1, N) {
            ncards[i - 1] += cards[i] / 2;
            ncards[(i + 1) % N] += (cards[i] + 1) / 2;
        }
...
int main() {
    ...
    if (N <= 20) {
        ...
        vi state(N);
        F0R (i, N) state[i] = read(i);

        F0R (i, N) {
            if (state[i] == K && state[(i + 1) % N] > state[i]) {
                finish(i);
            }
        }
        assert(false);
    ...
    int SQRT = ceil(sqrt(N));

    F0R (i, SQRT)
        read(0);

    // there are sqrt elements in a row > K
    int bigger = 0;
    while (read(bigger) <= K) {
        bigger += SQRT;
        assert(bigger < N);
