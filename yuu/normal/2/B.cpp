...
int b[1001][1001];
int dp[1001][1001];
pair<int, string> do_dp() {
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            if (i == 1) {
                if (j == 1) dp[i][j] = b[1][1];
                else dp[i][j] = dp[i][j - 1] + b[i][j];
            } else if (j == 1) {
                dp[i][j] = dp[i - 1][j] + b[i][j];
            } else {
                dp[i][j] = min(dp[i][j - 1], dp[i - 1][j]) + b[i][j];
            }
        }
    }
    int x = n, y = n;
    string move;
    while ((x != 1) || (y != 1)) {
        if ((x == 1) || ((y > 1) && (dp[x][y] == dp[x][y - 1] + b[x][y]))) {
            y--;
            move += 'R';
        } else {
            x--;
...
void solve() {
    ...
    if ((ans.first >= 1) && (pos_zero.first)) {
        ans.first = 1;
        ans.second = "";
        for (int i = 1; i < pos_zero.first; i++) ans.second += 'D';
        for (int i = 1; i < n; i++) ans.second += 'R';
        for (int i = pos_zero.first; i < n; i++) ans.second += 'D';
    }
    cout << ans.first << '\n' << ans.second;
}
