...
int main() {
  ...
  for (int testcase = $; testcase; --testcase) {
    ...
    vector<int> v(n);
    cin >> v;
    multiset<int64_t, greater<>> ms1{v.begin(), v.end()}, ms2{accumulate(v.begin(), v.end(), 0ll)};
    bool ok = 1;
    while (ms1.size()) {
      if (*ms1.begin() == *ms2.begin()) {
        ms1.erase(ms1.begin());
        ms2.erase(ms2.begin());
      } else if (*ms1.begin() > *ms2.begin()) {
        ok = 0;
        break;
      } else {
        auto t = *ms2.begin(); ms2.erase(ms2.begin());
        ms2.insert(t / 2);
        ms2.insert(++t / 2);
      }
    }
