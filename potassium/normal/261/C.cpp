...
ll solve(int p, int tot, bool isSmaller) {
	if (p == -1) return tot == 0;
	if (tot < 0) return 0;
	if (dp[p][tot][isSmaller] != -1) return dp[p][tot][isSmaller];
	...
	for (int i = 0; i <= maxv; i++)
		ans += solve(p-1, tot-i, isSmaller | (i < bit));
		
	return dp[p][tot][isSmaller] = ans;
}
