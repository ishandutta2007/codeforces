...
int main()
{
    ...
    reverse(all(bad));
    int sum = 0;
    int psum_kat[kat.size() + 1], psum_bad[bad.size() + 1];
    psum_kat[0] = 0;
    psum_bad[0] = 0;
    forn(i, kat.size())
        psum_kat[i + 1] = psum_kat[i] + kat[i].x;
    forn(i, bad.size())
        psum_bad[i + 1] = psum_bad[i] + bad[i].x;
    int kol1, kol2;
    forn(i, kat.size() + 1)
    {
        int rem = v - 2 * i;
        rem = min(rem, (int)bad.size());
        if (rem >= 0 && psum_kat[i] + psum_bad[rem] >= sum)
        {
            kol1 = i;
            kol2 = rem;
            sum = psum_kat[i] + psum_bad[rem];
        }
    }
    cout << sum << endl;
    forn(i, kol1)
