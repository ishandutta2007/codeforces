...
vector<vector<bs>> subsegm(N, vector<bs>(N));
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            if (j - i + 1 >= 5) {
                ...
                // dbg(i, j, p0, vsum);
                auto gets = [](const vector<int> &v, int l, int r) {
                    if (l > r) return 0;
                    return v[r] - (l == 0 ? 0 : v[l - 1]);
                };
                auto getr = [&](int r) {
                    return p0[r] + gets(vsum, 0, r - 1) + ((j - i - 1) - vert[r]) + (s[i][r] == 0 ? -1 : 0) + (s[j][r] == 0 ? -1 : 0);
                };
                auto getl = [&](int l) {
                    return ((j - i - 1) - vert[l]) - gets(vsum, 0, l) - gets(p0, 0, l - 1) + (s[j][l] == 0 ? -1 : 0) + (s[i][l] == 0 ? -1 : 0);
                };
                int mn = mod;
                for (int r = 0; r < m; ++r) {
                    if (r - 3 >= 0) {
