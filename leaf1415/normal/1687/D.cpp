...
ll n;
vector<ll> vec;
vector<P> dvec[2000005];
UnionFind uf(2000005);
vector<P> svec;
...
int main(void)
{
	...
	rep(i, 1, 2000000) svec.pb(P(i*i, i));
	
	rep(i, 0, sz(vec)-2){
		dvec[vec[i+1]-vec[i]].pb(P(vec[i], vec[i+1]));
	}
	
	set<P> S;
	reps(i, vec) S.insert(P(vec[i], vec[i]));
	...
	rep(i, 1, 2000000){
		...
		vector<P> evec;
		for(auto p : S){
			ll l = p.fi + d, r = p.se + d;
			ll j = upper_bound(all(svec), P(l, inf))[-1].se;
			{
				ll L = j*j, R = L+j;
				ll lb = max(0LL, -d), ub = i;
				chmax(lb, L-l), chmin(ub, R-l);
				chmax(lb, L-r), chmin(ub, R-r);
				if(lb <= ub) evec.pb(P(lb, -1)), evec.pb(P(ub, 1));
			}
			{
				j++;
				ll L = j*j, R = L+j;
		...
		for(auto p : dvec[i]){
			ll l = uf.root(p.fi), r = uf.root(p.se);
			S.erase(P(uf.l[l], uf.r[l]));
			S.erase(P(uf.l[r], uf.r[r]));
			uf.unite(p.fi, p.se);
			r = uf.root(r);
			S.insert(P(uf.l[r], uf.r[r]));
		}
		//outl(S);
	}
