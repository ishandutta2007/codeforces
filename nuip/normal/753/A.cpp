...
int main(){
  ...
  fill(dp[0],dp[0]+1123*1123,-MOD);
  dp[0][0]=0;
  reps(i,1,1023){
    rep(j,1123) MX(dp[i][j],dp[i-1][j]);
    rrep(j,1123-i) MX(dp[i][j+i],dp[i-1][j]+1);
  }
  //rep(i,6){rep(j,6) cout<<dp[i][j]<<",";cout<<endl;}
  int n;
  cin>>n;
  vector<int> re;
  int rem=n;
  rrep(i,n){
    if(rem-i>0 && dp[i+1][rem]==dp[i][rem-i-1]+1){
      re.pb(i+1);
      rem-=i+1;
    }
  }
