...
using mint = long long;//atcoder::modint;
#include <algorithm>
#include <array>
#include <bitset>
#include <cassert>
...
int main() {
    ...
    for (int N = 1; N <= n; N++) {
        for (int i = 0; i <= N - 1; i++) {
            tc[N] += tc[i] * tc[N - 1 - i] % mod * comb[N - 1][i] % mod;
            tc[N] %= mod;
        }
    }
    ...
    for (int N = 1; N <= n; N++) {
        for (int i = 0; i <= N - 1; i++) {
            dp[1][N][1] += tc[i] * tc[N - 1 - i] % mod * comb[N - 1][i] % mod;
            dp[1][N][1] %= mod;
        }
    }
    ...
    for (int M = 2; M <= m; M++) {
        ...
        for (int N = 1; N <= n; N++) {
            for (int K = 0; K <= min(N, k); K++) {
                if (K >= 1 && K + (K - 1) > N) continue;
                if (K >= 1 && K + (M - 1) > N) continue;
                for (int i = 0; i <= N - 1; i++) {
                    mint sum = 0;
                    for (int j = max(0, K - (N - 1 - i)); j <= min(i, K); j++) {
                        sum += dp[M - 1][i][j] * dp[M - 1][N - 1 - i][K - j] % mod;
                    }
                    sum %= mod;
                    dp[M][N][K] += sum * comb[N - 1][i] % mod;
                    dp[M][N][K] %= mod;
                }
            }
        }
