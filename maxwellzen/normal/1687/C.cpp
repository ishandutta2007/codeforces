...
void solve() {
    ...
    p.assign(n+1, 0);
    F0R(i, n) p[i+1] = p[i] + (ll)a[i]-(ll)b[i];
    if (p[n] != 0) {
        NO; return;
    }
    q.clear();
    cnt.assign(m, 0);
    upd.clear(); upd.rsz(n+1);
    F0R(i, m) {
        if (p[l[i]] != 0) cnt[i]++, upd[l[i]].pb(i);
        if (p[r[i]] != 0) cnt[i]++, upd[r[i]].pb(i);
        if (cnt[i]==0) q.pb(i);
    }
    F0R(i, n+1) if (p[i] != 0) bad.insert(i);
    while (! q.empty()) {
        int cur = q.back();
        debug(l[cur], r[cur]);
        q.pop_back();
        vi rem;
        auto it = bad.upper_bound(l[cur]);
        while (it != bad.end() && (*it) < r[cur]) {
            rem.pb(*it);
            it++;
        }
        for (int i : rem) {
