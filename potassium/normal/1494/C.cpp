...
int solve(vector<int>& a, vector<int>& b) {
	...
	int ans = 0;
	for (int i = 0; i < m; i++) {
		int box = upper_bound(all(a), b[i]) - a.begin();
		int wid = b.begin() + i - lower_bound(all(b), b[i] - box + 1) + 1;
		int add = (box == n ? 0 : suf[box]);
...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			int x; R(x);
			a[x > 0].emplace_back(abs(x));
		}

		for (int i = 0; i < m; i++) {
			int x; R(x);
			b[x > 0].emplace_back(abs(x));
		}

		reverse(all(a[0]));
		reverse(all(b[0]));

		W(solve(a[0], b[0]) + solve(a[1], b[1]));
	}
