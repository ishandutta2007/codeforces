...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<mint> cho(n),unch(n),chox(n);
	vector<int> sz(n,1);
	vector<mint> pw(n+10); pw[0]=1;
	reps(i,1,pw.size()) pw[i]=pw[i-1]+pw[i-1];
	auto dfs=
		[&](auto &&dfs,int v,int p)->void{
			mint ch=1,al=1,dis=1;
			for(int w:g[v])if(w!=p){
					dfs(dfs,w,v);
					sz[v]+=sz[w];
					mint cho1=cho[w];
					mint unch1=unch[w];
					mint cho0=cho[w]-chox[w];
					mint unch0=unch[w];
					dis*=cho0+unch0;
					ch*=unch1+cho0+unch0;
					al*=cho1+unch1+cho0+unch0;
				}
			cho[v]=ch;
			unch[v]=al;
			chox[v]=dis;
	...
	dfs(dfs,0,-1);
	out(cho,unch,chox,sz,1);
	cout<<cho[0]-chox[0]+unch[0]-1 NL;
  return 0;
}