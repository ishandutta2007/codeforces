...
bool geo(const double& T, const double& S, const double& e, const double& C, const int& n)
{
	double a = C;
	double b = 1 + C * n * e - C * T;
	double c = n * e - T + S;
	double t = (-b) / (2 * a);
	if (t < 0)
		t = 0;
...
int main()
{
	...
	for (int I = 0; I < tc; ++I)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			...
			for (int j = i; j >= 0; --j)
			{
				double add_time = pw[j + 1] * hard;
				for (int k = summa; k >= 0; --k)
				{
					if (pos[j][k])
					{
						bool sense = (pos[j + 1][k + pts] == false);
						double new_doub = dp[j][k] + add_time;
						if (sense == false)
						{
							double current_doub = dp[j + 1][k + pts];
							if (current_doub > new_doub)
								sense = true;
						}
						if (sense)
						{
							dp[j + 1][k + pts] = new_doub;
							pos[j + 1][k + pts] = true;
						}
					}
				}
		...
		bool ok = true;
		int ans = 0;
		for (int pts = nn - 1; ok && (pts >= 0); --pts)
		{
			for (int i = 0; ok && (i <= n); ++i)
			{
				if ((pos[i][pts]) && (geo(t, dp[i][pts], 10, c, i)))
				{
					//cout << i << ' ' << pts << ' ' << dp[i][pts] << '\n';
					ok = false;
					ans = pts;
