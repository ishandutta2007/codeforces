...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        if(x[0]==x[1]&&x[0]==x[2]||y[0]==y[1]&&y[0]==y[2]) cout<<1;
        else if(
            (x[0]==x[1]&&(y[2]>=max(y[1],y[0])||(y[2]<=min(y[1],y[0]))))||
            (x[0]==x[2]&&(y[1]>=max(y[2],y[0])||(y[1]<=min(y[2],y[0]))))||
            (x[1]==x[2]&&(y[0]>=max(y[2],y[1])||(y[0]<=min(y[2],y[1]))))||
            (y[0]==y[1]&&(x[2]>=max(x[1],x[0])||(x[2]<=min(x[1],x[0]))))||
            (y[0]==y[2]&&(x[1]>=max(x[2],x[0])||(x[1]<=min(x[2],x[0]))))||
            (y[1]==y[2]&&(x[0]>=max(x[2],x[1])||(x[0]<=min(x[2],x[1]))))
            ) cout<<2;
        else cout<<3;
