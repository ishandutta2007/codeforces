...
string solve(const string &s,const string &t,const char c){
	const int n=s.size();
	int si=0,ti=0;
	string re;
	while(si<n&&ti<n){
	...
	return re+s.substr(si)+t.substr(ti);
}

string solve(const int n,const vector<string> &ss){
	vector<int> cnt(3);
	rep(i,3) cnt[i]=count(all(ss[i]),'1');
	rep(i,3)rep(j,i){
		if(cnt[i]>=n && cnt[j]>=n) return solve(ss[i],ss[j],'1');
		if(cnt[i]<=n && cnt[j]<=n) return solve(ss[i],ss[j],'0');
	}
}
