...
void solve(){
	...
	rep(i,n){
		cin>>x[i]>>p[i];
		q.push_back({x[i]-p[i],1});
		q.push_back({x[i],-2});
		q.push_back({x[i]+p[i],1});
	}
	y=x;
	So(y);
	ll diff=0,base=0,fron=0;
	So(q);
	vector<ll> A;
	for(auto x:q){
		base+=(diff*(x.first-fron));
		fron=x.first;
		diff+=x.second;
		//cout<<x.first<<" "<<base<<"\n";
		if(x.second==-2) A.push_back(base);
	}
	segment_tree<F,op,e> seg1(n),seg2(n),seg3(n);
	rep(i,n){
		seg1.set(i,A[i]);
		seg2.set(i,A[i]-y[i]);
		seg3.set(i,A[i]+y[i]);
	...
	rep(i,n){
		...
		int b=LB(y,x[i]);
		int c=LB(y,x[i]+p[i]);
		if(seg1.query(0,a)<=m&&seg2.query(a,b)<=m-(x[i]-p[i])&&seg3.query(b,c)<=m+(x[i]+p[i])&&seg1.query(c,n)<=m){
			cout<<1;
		}else cout<<0;
	}
	cout<<"\n";
