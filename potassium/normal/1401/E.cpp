...
struct Event {
	...
	friend ostream& operator << (ostream& out, const Event& e) {
		return out << e.ty << " " << e.t << " " << e.p << " " << e.L << " " << e.R;
	}
	
	Event(int a, int b, int c, int d, int e) : ty(a), t(b), p(c), L(d), R(e) { }
};
struct BIT {
	vector<int> bit;
	const int N;
	...
	BIT(int N) : N(N) { bit.resize(N + 2, 0); }
};
int main() {
	int n,m; R(n,m);
	...
	for (int i = 0; i < n; i++) {
		int y,l,r; R(y,l,r);
		if (l == 0 && r == DIM) ans++;
		v.pb(Event(0,l,y,-1,-1));
		v.pb(Event(2,r,y,-1,-1));
	}
	
	for (int i = 0; i < m; i++) {
		int x,l,r; R(x,l,r);
		if (l == 0 && r == DIM) ans++;
		v.pb(Event(1,x,-1,l,r));
	}
