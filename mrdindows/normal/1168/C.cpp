...
int solve() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        if (i % K == K - 1 || i == n - 1) {
            FI(19) FJ(19) {
                hp[cur][i][j] = hp[cur + 1][i][j];
            }
        }
        for (int x = 0; x < 19; ++x) {
            if (!checkbit(a[i], x)) continue;
            for (int y = 0; y < 19; ++y) {
                if (!checkbit(a[i], y)) continue;
                hp[cur][x][y] = i;
            }
        }
    }
    for (int blablablabla = 0; blablablabla < q; ++blablablabla) {
        int x, y;
        x = readInt();
        y = readInt();
        --x; --y;
        ...
        int w = (x + K - 1) / K;
        FI(19) {
            if (checkbit(cur, i)) continue;
            int res = n;
            FJ(19) {
                if (checkbit(cur, j)) {
                    res = min(res, hp[w][i][j]);
                }
            }
            nex[i] = res;
        }
...
//        break;
        ...
        while (true) {
//            DBN(blablablabla, x, y, cur, a[y], where);
            if (x > y) {
                writeStr("Fou\n"); break;
            }
            if (cur & a[y]) {
...
//                    DBN(to, nex[i]);
            ...
            int pos = (to + K - 1) / K;
            for (int i = 0; i < 19; ++i) {
                if (checkbit(val ^ cur, i)) {
                    for (int j = 0; j < 19; ++j) {
                        if (!checkbit(val, j)) {
                            nex[j] = min(nex[j], hp[pos][i][j]);
                        }
                    }
                }
            }
