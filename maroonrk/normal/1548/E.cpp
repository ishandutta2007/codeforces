...
vc<Query> unko(int n,int tar){
	...
	vc<Query> res;
	auto dfs=[&](auto self,int v,int nei)->int{
		if(v==-1)return inf;
		int mn=a[v];
		rep(k,2)chmin(mn,self(self,t[v][k],a[v]));
		res.pb({tar-mn,a[v],nei});
		return mn;
	};
	dfs(dfs,root,inf);
	return res;
...
void slv(){
	...
	for(auto w:qs){
		if(w.w<2){
			bit.add(lwb(xs,w.x),1-w.w*2);
		}else{
			ans+=bit.get(lwb(xs,w.x))*(5-w.w*2);
		}
	}
	print(ans);
}
