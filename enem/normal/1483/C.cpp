...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        vector<tuple<ll,ll,ll>> mns;
        REP(i,0,n)
        {
            tuple<ll,ll,ll> t=MT(b[i]+dp[i],i,i);
            while(!mns.empty() && h[i]<h[G(1,mns.back())])
            {
                st.erase(st.find(mns.back()));
                if(dp[G(2,mns.back())]>dp[G(2,t)])
                {
                    t=MT(b[i]+dp[G(2,mns.back())],i,G(2,mns.back()));
                }
                mns.pop_back();
            }
            mns.push_back(t);
