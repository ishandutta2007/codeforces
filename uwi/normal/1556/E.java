...
public class E {
	...
	void solve()
	{
		...
		long[] rcum = new long[n+1];
		for(int i = 0;i <= n;i++)rcum[i] = -cum[i];
		SegmentTreeRMQL stmin = new SegmentTreeRMQL(cum);
		SegmentTreeRMQL stmax = new SegmentTreeRMQL(rcum);
		for(int z = 0;z < Q;z++){
			int l = ni()-1, r = ni()-1;
			if(cum[r+1] - cum[l] != 0){
				out.println(-1);
			}else if(stmin.min(l, r+1) < cum[l]){
				out.println(-1);
			}else{
				out.println(-stmax.min(l, r+1) - cum[l]);
			}
		}
	}
