...
ll n, m, k, L, b;
pair<double, double> p[500000];

double dist(double x1, double y1, double x2, double y2)
{
    re sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
}

bool c2(double x, double r)
{
    for (int i = 0; i < n; i++)
    {
        if (dist(x, r, p[i].fi, p[i].se) > r + eps)
            return 0;
    }
    re 1;
}
...
bool c(double r)
{
    ...
    for (int i = 0; i < n; i++)
    {
        double xi = p[i].fi;
        double yi = p[i].se;
        double optimal = xi + sqrt(r * r - (r - yi) * (r - yi));
        if (optimal < minimum)
            minimum = optimal;
    }
    //cout << setprecision(10) << r << " " << minimum << endl;
    re c2(minimum, r);
}
