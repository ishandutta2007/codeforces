...
int main(){
  ...
  while(t--){
    ll n,q;cin>>n>>q;
    LazySegmentTree<P,ll,decltype(ff),decltype(gg),decltype(hh)>seg(n,ff,gg,hh,MP(0,0),-INF);
    string s,f;cin>>s>>f;
    vector<ll>l(q),r(q);
    rep(i,0,q){
      cin>>l[i]>>r[i];
    ...
    rep(i,0,n){
      seg.set(i,MP(f[i]-'0',1));
    }
    seg.build();
    bool sw=true;
    rrep(i,0,q){
      auto sum=seg.query(l[i],r[i]);
      if(sum.fi*2==sum.se)sw=false;
      else if(sum.fi*2>sum.se){
        seg.update(l[i],r[i],1);
      }
      else seg.update(l[i],r[i],0);
    }
    rep(i,0,n)if(seg[i].fi!=s[i]-'0')sw=false;
    ans2(sw);
  }
  return 0;
}