...
int main() {
    ...
    mint ans = 0;
    vector dpx(4, vector(n + 1, vector(m + 1, mint(0))));
    vector dpy(4, vector(n + 1, vector(m + 1, mint(0))));
    rep(i, n + 1) rep(j, m + 1) {
        ans += dpx[0b11][i][j] + dpy[0b11][i][j];
        if (i == n and j == m) break;
        rep(f, 4) {
            if (i < n) {
                if (i and x[i] != x[i - 1]) dpx[f | 0b10][i + 1][j] += dpx[f][i][j];
                if (j and x[i] != y[j - 1]) dpx[f | 0b10][i + 1][j] += dpy[f][i][j];
                if (f == 0) dpx[0b10][i + 1][j] += 1;
            }
            if (j < m) {
                if (j and y[j] != y[j - 1]) dpy[f | 0b01][i][j + 1] += dpy[f][i][j];
                if (i and y[j] != x[i - 1]) dpy[f | 0b01][i][j + 1] += dpx[f][i][j];
                if (f == 0) dpy[0b01][i][j + 1] += 1;
            }
        }
    }
    print(ans);
