...
using mn_t = modnum<int(998244352)>;
...
int main() {
    ...
    for (int g = 1; g < MAXV; g++) {
        if (sv[g].mu == 0) continue;

        ll _N = 0;

        mn sum = 0;

        for (int gm = g; gm < MAXV; gm += g) {
            _N += freq[gm];
            sum += mn(freq[gm]) * gm;
        }
        ...
        mn c1 = mn(2).pow((N_t - 2).v) * (N - 1);
        mn c2 = mn(2).pow((N_t - 3).v) * (N - 2) + mn(2).pow((N_t - 2).v);

        mn term = 0;
        for (int gm = g; gm < MAXV; gm += g) {
            term += mn(gm) * gm * c1 * freq[gm];
            term += mn(gm) * (sum - gm) * c2 * freq[gm];
        }

        if (sv[g].mu == 1) ans += term;
        else ans -= term;
    }
