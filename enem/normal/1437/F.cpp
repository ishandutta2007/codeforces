...
int main()
{
    ...
    ll N=10005;
    ll n,fc[N],fci[N],a[N],dp[N],str[N];
    ...
    fci[N-1]=modI(fc[N-1],M2);
    REPI(i,0,N-1)
    {
        fci[i]=((fci[i+1]*(i+1))%M2);
    }
    ...
    REP(i,0,n)
    {
        str[i]=upper_bound(a,a+n,a[i]/2)-a;
    }

    REP(i,0,n)
    {
        dp[i]=((fc[n-1]*fci[n-str[i]-1])%M2);

        REP(j,0,str[i])
        {
            dp[i]=((dp[i]+((((fc[n-str[j]-2]*fci[n-str[i]-1])%M2)*dp[j])%M2))%M2);
        }
    }
    if(a[n-1]>=2*a[n-2]) cout<<dp[n-1];
    else cout<<0;
