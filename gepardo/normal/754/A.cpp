...
template<typename T> inline ostream& operator<<(ostream& os, const vector<T>& v) { for (int i = 0; i < sz(v); i++) { if (i) { os << endl; } os << v[i]; } return os; }
...
signed main() {
	...
	for (int j = i; j < n;) {
		...
		int beg = ans.empty() ? 1 : j+1;
		ans.eb(beg, k);
		j = k;
	}
	putLn("YES", endl, ans.size(), endl, ans);
	return 0;
}