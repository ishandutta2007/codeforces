...
int main() {
   ...
   // maintain a 1-indexed list of 1-indexed indices v (size at most 26)  so that ask(v[j], latest) = j
   // when processing i, bin search on j -- the last j such that ask(v[j], i) = j + 1
   // if j + 1 == sz(v):
   //   ans[i] = ask1(i)
   //   v.push_back(-1)
   // else:
   ...
   auto put = [&](string ans) -> void {
      cout << "! " << ans << endl;
   };
   vector<int> v(1);
   // 1 <= j < sz(v)
   auto go = [&](int j, int i) -> bool {
      return ask2(v[j], i) == j + 1;
   };
