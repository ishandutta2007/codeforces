...
int n; LL a[MAXN], g[21][MAXN];
int lg[MAXN];


inline bool query(int l, int r) {
    const int k = lg[r - l + 1];
    return gcd(g[k][l], g[k][r - (1 << k) + 1]) > 1;
}

inline void work() {
    read(n); read(a + 1, a + n + 1);
    per(i, n, 2) g[0][i] = abs(a[i] - a[i - 1]);
    rep(i, 1, lg[n]) 
        for (int j = 1;j + (1 << i) - 1 <= n;++ j) 
            g[i][j] = gcd(g[i - 1][j], g[i - 1][j + (1 << (i - 1))]);
    LL g = 0; int ans = 0, r = 1; rep(l, 2, n) { chmax(r, l - 1);
