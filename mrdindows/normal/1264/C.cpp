...
Node merge(Node a, Node b) {
    return {(b.sum + a.sum * b.prod) % fftmod, a.prod * b.prod % fftmod};
}
Node tr[808080];
ll a[202020];
...
Node get(int cur, int l, int r, int x, int y) {
    if (x <= l && r <= y) {
        return tr[cur];
    } else {
        int m = (l + r) / 2;
        ...
        if (y <= m) {
            ...
            return merge(get(dcur, l, m, x, m), get(dcur + 1, m + 1, r, m + 1, y));
        }
    }
}
int n, q;
ll getSum(int x, int y) {
    return get(1, 0, n - 1, x, y).sum;
}
int solve() {
    cin >> n >> q;
    FI(n) {
        int p;
        cin >> p;
        a[i] = 100 * powmod(p, fftmod - 2, fftmod) % fftmod;
    }
    build(1, 0, n - 1);
    set<int> s;
    s.insert(0);
    ...
    while (q--) {
        ...
        cout << (ans % fftmod + fftmod) % fftmod << endl;
    }
