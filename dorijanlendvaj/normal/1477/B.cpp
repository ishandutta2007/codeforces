...
int t,n,q,seg[M*2+10],pro[M*2+10],x[N],y[N];
string a,b;
...
int main()
{
	...
	{
		cin>>n>>q>>a>>b;
		for (int i=0;i<n;++i) upd(i,i+1,b[i]-'0');
		for (int i=0;i<q;++i) cin>>x[i]>>y[i],--x[i];
		bool ok=1;
		for (int i=q-1;i>=0;--i)
		{
			int l=x[i],r=y[i],re=ge(l,r);
			if (re*2==r-l)
			{
				ok=0;
				break;
		...
		for (int i=0;i<n;++i) if (ge(i,i+1)!=a[i]-'0') ok=0;
		if (ok) cout<<"YES\n";
		else cout<<"NO\n";
	}
}