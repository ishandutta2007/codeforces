...
public class Main extends PrintWriter {
    ...
    class Vert implements Comparable<Vert> {
        ...
        public Vert(int id, int deg, int xor) {
            this.id = id;
            this.deg = deg;
            this.xor = xor;
        }
        ...
        public int compareTo(Vert o) {
            return deg == o.deg ? id - o.id : deg - o.deg;
        }
    }

    void solve() throws IOException {
        int n = nextInt();
        TreeSet<Vert> ts = new TreeSet<>();
        Vert[] byId = new Vert[n];
        for (int i = 0; i < n; i++) {
            Vert v = new Vert(i, nextInt(), nextInt());
            byId[i] = v;
            ts.add(v);
        }
        ...
        for (int i = 0; i < ans.size() / 2; i++) {
            printf("%d %d%n", ans.get(i * 2), ans.get(i * 2 + 1));
        }
    }
