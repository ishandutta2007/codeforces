...
int main(int, char**){
  ...
  int n, m, q; cin >> n >> m >> q;
  vector<tuple<int, int, int, int> > W;
  vviii G(n);
  FOR(i, m) {
  ...
  FOR(q_, q) {
    int l, r; cin >> l >> r; l-=1; r-=1;
    union_find uf;
    vi cls(n, -1);
    for(auto const& p : W) {
      DESTRUCT4(p, w, i, u, v);
      if(i<l||i>r) continue;
      if(cls[u]>cls[v]) swap(u, v);
      if(cls[u]!=-1) {
        uf.unite(cls[u], cls[v]^1);
        uf.unite(cls[v], cls[u]^1);
        cls[u] = uf.find(cls[u]);
        cls[v] = uf.find(cls[v]);
        if(cls[u] == cls[v]) {
          cout << w << '\n';
  ...
  l_end:;
    ...
    // auto ok = [&](int mi){
    //   vi E(n), C(n);
    //   function<bool(int, bool)> dfs = [&](int i, bool b) {
    //     if(E[i]) {
    ...
    //     E[i]=1; C[i]=b;
    //     for(piii p : G[i])
    //       if(l<=get<1>(p) && get<1>(p)<=r && (mi!=m&&get<2>(p)>=W[mi]))
    //         if(!dfs(get<0>(p), !b)) return false;
    //     return true;
    //   };
    //   FOR(i, n) if(!E[i]) if(!dfs(i, 0)) return false;
    //   return true;
    // };
