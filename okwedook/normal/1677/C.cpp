...
void solve() {
    ...
    sort(all(comp), [](const vector<int> &a, const vector<int> &b) {
        return sz(a) > sz(b);
    });
    vector<int> color(n);
    int l = 0, r = n - 1;
    ...
    for (int i = 0; i < sz(comp); ++i) {
        if (sz(comp[i]) > 1 && sz(comp[i]) % 2 == 1) {
            comp.pb(vector<int>(1, comp[i].back()));
            comp[i].popb();
        }
        for (auto v : comp[i]) {
            if (ptr++ & 1) {
