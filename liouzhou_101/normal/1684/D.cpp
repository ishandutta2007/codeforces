...
void solve()
{
	...
	{
		value.val.first += tag.add;
	};

	vector<ValueType> init(n);
	for (int i = 0; i < n; ++i)
	{
		init[i].val = { a[i] - (n - i - 1), i };
	}
	...
	while (k--)
	{
		auto [delta, id] = tree.Query(0, n).val;
		res -= delta;
		tree.ModifyTag(0, n, { 1 });
		tree.ModifyValue(id, value_init);
	}
