...
int ans = 0;
map<int,int> dfs(int now, int back) {
    map<int, int> Does;
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        map<int, int> cnter;
        map<int,int> next = dfs(x, now);
        for (auto& x : next) {
            int hoge = gcd(x.first, inputs[now]);
            if (hoge == 1) continue;
            cnter[hoge] = max(cnter[hoge], x.second + 1);
            ans = max(ans, x.second + 1);
        }
        for (auto& x : cnter) {
            for (auto& y : Does) {
                if (gcd(x.first, y.first) != 1) {
                    ans = max(ans, x.second + y.second - 1);
                }
            }
        }
        for (auto& x : cnter) {
            Does[x.first] = max(Does[x.first], x.second);
        }
    }
    Does[inputs[now]] = max(Does[inputs[now]], 1LL);
    if (inputs[now] != 1) ans = max(ans, 1LL);
    return Does;
}
void solve(){
    int n;
