#line 1 "codeforces-workspace\\contest\\1408\\e\\e.cpp"
...
#line 2 "Library\\src\\opt\\trinary_search.hpp"
...
#line 10 "Library\\src\\opt\\trinary_search.hpp"
...
#line 11 "codeforces-workspace\\contest\\1408\\e\\e.cpp"
...
#line 11 "Library\\src\\data_structure\\heap.hpp"
...
#line 27 "codeforces-workspace\\contest\\1408\\e\\e.cpp"
...
void main() {
  ...
  for (auto i : range(m)) {
    ...
    for (auto j : range(s)) {
      ...
      --j;
      ans += b[j];
      G[i].emplace_back(j + m, -a[i] - b[j]);
      G[j + m].emplace_back(i, -a[i] - b[j]);
    }
  }
  ...
  // Find MST
  {
    ...
    heap<pair<int, int>, greater<>> hp;
    vector<int> used(G.size());
    for (auto now : range(G.size())) {
      hp.emplace(cost[now] = 0, now);
      while (!hp.empty()) {
        int c;
        tie(c, now) = hp.top();
        hp.pop();
        ...
        ans += c;
        for (auto &&[to, w] : G[now]) {
          if (chle(cost[to], w)) hp.emplace(w, to);
        }
      }
    }
  }
