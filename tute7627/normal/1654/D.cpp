  ...
  while(t--){
    ...
    auto dfs=[&](auto &&f,ll k,ll par)->void{
      ...
      for(auto e:g[k]){
        ...
        auto mpx=osak.factoring(e.cost.fi);
        auto mpy=osak.factoring(e.cost.se);
        for(auto z:mpy)cnt[z.fi]+=z.se;
        for(auto z:mpx){
          cnt[z.fi]-=z.se;
          chmin(mi[z.fi],cnt[z.fi]);
        }
        now=now/e.cost.fi*e.cost.se;
        f(f,e.to,k);
        now=now*e.cost.fi/e.cost.se;
        for(auto z:mpy)cnt[z.fi]-=z.se;
        for(auto z:mpx)cnt[z.fi]+=z.se;
      }
    };
    dfs(dfs,0,-1);
    //cout<<now<<endl;
