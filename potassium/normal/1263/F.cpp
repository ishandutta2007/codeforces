...
int main() {
	...
	int a; R(a);
	vector<int> p(a);
	for (int i = 1; i < a; i++) { R(p[i]); p[i]--; }
	vector<int> x(n); R(x);
	for (auto& i : x) i--;

	int b; R(b);
	vector<int> q(b);
	for (int i = 1; i < b; i++) { R(q[i]); q[i]--; }
	vector<int> y(n); R(y);
	for (auto& i : y) i--;

	const int SRC = n + a + b;
	const int SINK = n + a + b + 1;
	dinic<int> flow(n + a + b + 2);

	for (int i = 0; i < a; i++) flow.addedge(SRC, n + i, 1);
	for (int i = 0; i < n; i++) for (int pa = x[i]; pa != 0; pa = p[pa]) flow.addedge(n + pa, i, 1e9);
	for (int i = 0; i < b; i++) flow.addedge(n + a + i, SINK, 1);
	for (int i = 0; i < n; i++) for (int pa = y[i]; pa != 0; pa = q[pa]) flow.addedge(i, n + a + pa, 1e9);

	W(a + b - 2 - flow.maxflow(SRC, SINK));
