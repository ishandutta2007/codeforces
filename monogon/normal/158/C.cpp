...
int main() {
	...
	for(int j = 0; j < n; j++) {
		...
		if(command == "cd") {
			...
			if(indir[0] == '/') {
				dir.clear();
				indir.erase(indir.begin());
			}
			if(indir[indir.length() - 1] != '/')
				indir += '/';
			for(unsigned int i = 0; i < indir.length(); i++) {
				if(indir[i] == '/') {
					if(i > 0)
						dir.push_back(indir.substr(0, i));
					indir = indir.substr(i + 1);
					i = -1;
				}else if(i < indir.length() - 1 && indir[i] == '.' && indir[i + 1] == '.') {
					dir.pop_back();
					indir = indir.substr(i + 2);
					i = -1;
				}
			}
		}else {
