namespace Solution {
	...
	operation Addition (x : Qubit[], y : Qubit[]) : Unit {
		body (...) {
			let n = Length(x);
			X(y[n]);
			for (i in 0..n-1) {
	...
	operation Work (x : Qubit[], y : Qubit[]) : Unit {
		body (...) {
			let n = Length(x);
			Apply(x[0..n-2],Rx(2.3333333,_));
			Addition(x, y);
		}
		adjoint auto;
		controlled auto;
	...
	operation Solution (x : Qubit[]) : Unit {
		body (...) {
			let n = Length(x);
			using (y = Qubit[n + 1]) {
				Adjoint Addition(x[0..n-1], y);
				Adjoint Addition(x[0..n-1], y);
				Controlled Apply([x[n-1]],(x[0..n-2],Rx(2.3333333,_)));
				X(x[n-1]);
				Controlled Work([x[n-1]],(x[0..n-2], y));
				X(x[n-1]);
				Addition(x[0..n-1], y);
				Addition(x[0..n-1], y);
			}
