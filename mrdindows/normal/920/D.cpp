...
int solve() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 19999 - k; j >= 0; --j) {
            if (!dp[i][j]) dp[i][j] = dp[i][j + k];
        }
    }
    if (!dp[n][v % k]){
        cout << "NO" << endl;
    ...
    vector<int> ids;
    for (int i = n; i > 0; --i) {
        if (dp[i - 1][cur]) continue;
//        DBN(i, cur, dp[i - 1][cur]);
        int val = a[i - 1] % k;
        for (int j = cur; j < 20000; j += k) {
//            if (j == cur) {
//                DBN(i, j, val, dp[i - 1][j - val]);
//            }
...
//    DBN(ids);
    ...
    for (int i = 1; i < ids.size(); ++i) {
        a[ids[0]] += a[ids[i]];
        a[ids[i]] = 0;
        ans.push_back({ids[i] + 1, ids[0] + 1, int(1e9)});
    }
    int emp = 0;
    for (int i = 0; i < n; ++i) {
        if (ids[0] != i) {
...
//    DBN(emp);
    for (int i = 0; i < n; ++i) {
        if (!used[i] && i != emp) {
            ...
            a[i] = 0;
//            DBN(i);
            ans.push_back({i + 1, emp + 1, int(1e9)});
        }
    }
    int where = ids[0];
//    DBN(a[where]);
//    DBN(a[where] % k, v % k);
    assert(a[where] % k == v % k);
    if (a[where] > v) {
        int cnt = (a[where] - v) / k;
        ans.push_back({where + 1, emp + 1, cnt});
    } else {
        int cnt = (v - a[where]) / k;
        ans.push_back({emp + 1, where + 1, cnt});
    }
    cout << "YES" << endl;
    for (auto t : ans) { if (t.c)
        cout << t.c << ' ' << t.a << ' ' << t.b << '\n';
    }
