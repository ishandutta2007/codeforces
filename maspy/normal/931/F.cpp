...
#line 1 "/home/maspy/compro/library/dp/longest_increasing_subsequence.hpp"
/*
dp[i] :=  i  lis 
 dp O(Nlog N) 
 strong
...
void solve() {
  ...
  A = cumsum(A, 0);
  A.resize(N);
  auto dpl = longest_increasing_subsequence<ll, 0>(A);
  reverse(all(A));
  auto dpr = longest_increasing_subsequence<ll, 0>(A);
  reverse(all(dpr));
  reverse(all(A));

  ll ANS = 0;
  FOR(i, len(A)) chmax(ANS, dpl[i] + dpr[i] - 1);
  print(ANS);
}
