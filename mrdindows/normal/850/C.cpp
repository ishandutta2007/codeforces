...
map<uint, int> dp;
#define full(i) ((uint(1) << i) - 1)
const int K = 10101010;
int dp2[K];
int go(uint mask) {
    if (mask == 1) return 0;
    int &res = mask < K ? dp2[mask] : dp[mask];
    if (res == 0) {
        bool used[64] = {};
        for (int i = 1; i <= 30; ++i) {
            if (mask < (uint(1) << i)) break;
            uint to = (mask & full(i)) | (mask >> i);
            used[go(to)] = true;
        }
        int t = 0;
        while (used[t]) {
...
//                DBN(i, k);
    ...
    int  ans = 0;
    for (auto p : used) {
//        DBN(p.first, p.second, go(p.second));
        ans ^= go(p.second);
    }
