...
int main() {
    ...
    vpii p(K); re(p); trav (e, p) --e.f, --e.s;
    sort(all(p));
    p.eb(N-1, M-1);

    auto pc = [](pii a, pii b) {
        int dx = b.f - a.f, dy = b.s - a.s;
        return (dx >= 0 && dy >= 0) ? mn::ncr(dx + dy, dx) : 0;
    };

    mn paths = pc({0,0}, p.back());
    mn done = 0, ans = 0;

    vvmn PC(sz(p));
    F0R (i, sz(p)) {
        PC[i].resz(i);
        F0R (j, i) PC[i][j] = pc(p[j], p[i]);
    }

    vmn dp(sz(p)), DP(sz(p));
    for (bool f = 1; S > 1; f = 0) {
        F0R (i, sz(p)) {
            if (f) dp[i] = pc({0,0}, p[i]);
            else dp[i] = 0;
            F0R (j, i) if (mn ji = PC[i][j]; ji.v) {
                mn adj = -dp[j] + (f ? 0 : DP[j]);
                dp[i] += adj * ji;
            }
        }
