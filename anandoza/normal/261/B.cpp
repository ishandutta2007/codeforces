...
int main() {
    ...
    vector<double> factorial{1};
    while (sz(factorial) <= n) {
        factorial.push_back(factorial.back() * sz(factorial));
    }

    double ans = 0;
    for (int pivot = 0; pivot < n; pivot++) {
        vector<vector<ll>> dp(p + 1, vector<ll>(n)); // dp[x][y] is ways to get sum = x with count = y
        dp[0][0] = 1;
        for (int i = 0; i < n; i++) {
            if (i == pivot)
                continue;
        ...
        for (int x = max(0, p + 1 - a[pivot]); x <= p; x++) {
            for (int y = 0; y < n; y++) {
                ans += dp[x][y] * y * factorial[n - 1 - y] * factorial[y];
            }
        }
    }
