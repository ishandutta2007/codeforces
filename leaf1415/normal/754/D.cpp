...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> l[i] >> r[i];
		l[i] += 1000000005, r[i] += 1000000005;
		vec.push_back(P(l[i], r[i]));
		vec.push_back(P(r[i]+1, -r[i]));
	}
	sort(all(vec));
	
	ll ans = 0, ans_l;
	for(auto p : vec){
		if(p.second < 0) bt.erase(-p.second);
		else{
			bt.insert(p.second);
			if(bt.size() < k) continue;
			ll r = bt.getMin(bt.size()-k);
			if(ans < r-p.first+1) ans = r-p.first+1, ans_l = p.first;
		}
	}
	if(ans == 0){
		outl(0);
		rep(i, 1, k) cout << i << " "; cout << endl;
		return 0;
	}
	...
	vector<ll> avec;
	rep(i, 1, n){
		if(l[i] <= ans_l && ans_l+ans-1 <= r[i]) avec.push_back(i);
	}
	avec.resize(k);
	outl(avec);
