...
int another_main()
{
	...
	for (int I = 0; I < t; ++I)
	{
		...
		while (sz(v.back()) >= 2)
		{
			v.emplace_back((sz(v.back()) + 1) / 2);
			for (int i = 0; i < sz(v.back()); ++i)
			{
				v.back()[i] = v[sz(v) - 2][2 * i];
				if (2 * i + 1 < sz(v[sz(v) - 2]))
				{
					v.back()[i] = gcd(v.back()[i], v[sz(v) - 2][2 * i + 1]);
					v.back()[i] = gcd(v.back()[i], abs(u[2 * i << sz(v) - 2] - u[2 * i + 1 << sz(v) - 2]));
				}
			}
		}
		int a = 0;
		for (int i = 0; i < n; ++i)
		{
			...
			while (x < n)
			{
				lvl = __builtin_ctz(x);
				int xx = x >> lvl;
				ll new_nd = gcd(nd, v[lvl][xx]);
				new_nd = gcd(new_nd, abs(u[i] - u[x]));
				if (new_nd != 1)
				{
					x = min(x + (1 << lvl), n);
					nd = new_nd;
