...
std::vector<MODINT> nttconv(std::vector<MODINT> a, std::vector<MODINT> b) {
...
string mint2string(const vector<mint> &V) {
    string ret;
    REP(i, V.size()) {
        ret += '0' + V[i].val;
    }
...
bool comp(const vector<mint> &l, const vector<mint> &r) {
    if (l.size() > r.size()) return true;
    if (l.size() < r.size()) return false;
    IREP(i, l.size()) {
        if (l[i].val > r[i].val) return true;
        if (l[i].val < r[i].val) return false;
    }
    return true;
}

vector<vector<mint>> pow3s;

constexpr int base = 10;

template <int k> void multiply_by_k(string &S) {
    int carry = 0;
    IREP(i, S.size()) {
        int a = (S[i] - '0') * k + carry;
        int md = a % base, d = a / base;
        S[i] = '0' + md;
        carry = d;
    }
    if (carry) S.insert(S.begin(), '0' + carry);
};

int main() {
    auto reduce = [](vector<mint> &v) {
        int i = 0;
        while (i < v.size()) {
            int t = v[i].val;
            if (t >= base) {
                ...
                v[i] = t % 10;
                if (carry) {
                    if (i + 1 >= int(v.size())) v.push_back(0);
                    v[i + 1] += carry;
                }
            }
            i++;
    ...
    vector<vector<mint>> pow3s = {{3}};

    while (pow3s.size() <= 21) {
        auto v = nttconv(pow3s.back(), pow3s.back());
        reduce(v);
        pow3s.push_back(v);
    }
    ...
    int p3sdeg = max<int>(S.size() / log10(3) - 2, 0);

    vector<mint> tmp{1};
    REP(d, pow3s.size()) {
        if ((p3sdeg >> d) & 1) {
            tmp = nttconv(tmp, pow3s[d]);
            reduce(tmp);
        }
    }

    // dbg(tmp);
    auto p3s = mint2string(tmp);
