...
bool dfs(int x) {
	...
	col[x] = 1;
	for (int v : graf[x]) {
		if (col[v] == 2) return false;
		if (col[v] == 0) if (!dfs(v)) return false;
	}
	col[x] = 2;
	return true;
}
void dfs2(int x) {
	...
	reachable[x] = true;
	for (int w : graft[x]) dfs2(w);
}
int p[nax];
vi rets[nax];
int touched[nax];
int dep[nax];
void dfs3(int x, int y = 0, int d = 0) {
	dep[x] = d;
	p[x] = y;
	col[x] = 1;
	for (int v : graf[x]) {
...
void clear(int n) {
	memset(reachable, 0, sizeof(reachable[0]) * (n + 10));
	memset(was, 0, sizeof(was[0]) * (n + 10));
	memset(p, 0, sizeof(p[0]) * (n + 10));
	memset(touched, 0, sizeof(touched[0]) * (n + 10));
	memset(dep, 0, sizeof(dep[0]) * (n + 10));
	REP(i, n + 10) {
		graf[i].clear();
		graft[i].clear();
		rets[i].clear();
...
int get(int x) {
	if (x == 0) return x;
	if (touched[x] < 2) return x;
	return p[x] = get(p[x]);
}
void hit_path(int bot, int top) {
	while (dep[bot] > dep[top]) {
		touched[bot]++;
		if (touched[bot] == 2) for (int v : rets[bot]) hit_path(v, bot);
		bot = get(p[bot]);
	}
}
void solve(int n, int m) {
	...
	int checked = 0;
	int found = -1;
	while (checked < min(n, 40)) {
		int v = 1 + rand() % n;
		if (was[v]) continue;
		was[v] = true;
		checked++;
		memset(col, 0, sizeof(col[0]) * (n + 10));
		if (dfs(v)) {
			bool all = true;
			for (int i = 1; i <= n; ++i) if (col[i] != 2) all = false;
			if (all) {
				found = v;
				break;
			}
	...
	memset(col, 0, sizeof(col[0]) * (n + 10));
	dfs3(found);
	for (int i = 1; i <= n; ++i) for (int x : rets[i]) {
		debug(imie(x), imie(i));
		hit_path(x, i);
	}
	vi ans;
	for (int i = 1; i <= n; ++i) if (reachable[i] && touched[i] < 2) ans.push_back(i);
	debug(imie(ans));
	if ((int) ans.size() * 5 < n)
		printf("-1");
	else
...
int main() {
	unt seed = TIME;
	// unt seed = loc(3877306662u)onl(TIME);
	debug(imie(seed));
	srand(seed);
	int t;
	scanf("%d", &t);
