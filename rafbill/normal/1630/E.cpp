...
ZN sum(li n, li m) {
  ZN r = 0;
  for(li k = (1); k <= (li)(min(n,m)); ++k) {
    r += 2*k * cnk(n-1,k-1) * cnk(m-1,k-1);
    r += k * cnk(n-1,k) * cnk(m-1,k-1);
    r += k * cnk(n-1,k-1) * cnk(m-1,k);
  }
  return r;
}

ZN sumall(li d, li n, vi const& A) {
  ZN z = cntall(d,A);
  ZN r = 0;
  for(li a : A) {
    r += sum(a/d,(n-a)/d) * z * invcnk(n/d,a/d);
  }
  return d*r;
}
...
void solve() {
  ...
  map<li,li> C;
  for(li i = 0; i < (li)(n); ++i) C[A[i]]++;
  vi B;
  for(auto p : C) B.emplace_back(p.second);
  li g = B[0];
  for(li b : B) g = __gcd(g,b);
  if(g == n) {
    cout << 1 << endl;
    return;
  }
  ...
  vi divs;
  for(li d = (1); d <= (li)(g); ++d) if(g%d == 0) divs.emplace_back(d);
  vector<ZN> dpcnt(n+1,0), dpsum(n+1,0);
  ZN anssum = 0, anscnt = 0;
  for(auto d : divs) {
    dpcnt[d] = cntall(g/d,B);
    dpsum[d] = sumall(g/d,n,B);
    for(auto d2 : divs) if(d2 < d && d%d2==0) {
        dpcnt[d] -= dpcnt[d2];
        dpsum[d] -= dpsum[d2];
      }
    anssum += dpsum[d]/ZN(n/(g/d));
    anscnt += dpcnt[d]/ZN(n/(g/d));
  }
  cout << anssum/anscnt << '\n';
}
