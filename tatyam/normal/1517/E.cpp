...
void solve(){
    ...
    PartialSum<ll>s(a);
    Modint ans=0;
    rep(n+1)if(s.get(0,i)>s.get(i,n))ans++;
    if(n>=2)[a,&ans]()mutable{
        ll base=a.back()-a[0];
        a.pop_back();
        a.erase(a.begin());
        PartialSum<ll>s(a);
        ...
        rep(n)b[i+1]+=b[i];
        auto get2=[&](ll l,ll r){
            if(l&1)return b[l]-b[r];
            else return b[r]-b[l];
        };
        rep(n+1){
            ll ok = i, ng = n + 1 + (n + 1 - i) % 2;
            auto check = [&](ll x) -> bool {
                return base+s.get(x,n)+get2(i,x)>0;
            };
            if(!check(i))break;
            while(abs(ok - ng) > 2){
                ll cen = (ok + ng) / 2;
    ...
    if(n>=3)[a,&ans]()mutable{
        ll base=a.back()+a[0]-a[1];
        a.pop_back();
        a.erase(a.begin());
        a.erase(a.begin());
        PartialSum<ll>s(a);
    ...
    if(n>=4)[a,&ans]()mutable{
        ll base=a.rbegin()[1]-a.back()-a[1]+a[0];
        a.pop_back();
        a.pop_back();
        a.erase(a.begin());
        a.erase(a.begin());
