...
public class F {
	...
	static int getMax(IntIntMap fenwick, int i) {
		int res = fenwick.get(i, 0);
		while ((i = (i & (i + 1)) - 1) >= 0) {
			res = max(res, fenwick.get(i, 0));
		}
		return res;
	}

	static void updateMax(IntIntMap fenwick, int i, int v) {
		fenwick.set(i, max(fenwick.get(i, 0), v));
		while ((i |= i + 1) <= 150000) {
			fenwick.set(i, max(fenwick.get(i, 0), v));
		}
	}
	...
	static void solve() throws Exception {
		...
		int ans = 0;
		for (int i = 0; i < m; i++) {
			int a = scanInt() - 1, b = scanInt() - 1, w = scanInt() + 1;
			int cAns = getMax(fw[a], w - 1) + 1;
			ans = max(ans, cAns);
			updateMax(fw[b], w, cAns);
		}
		out.print(ans);
	}
