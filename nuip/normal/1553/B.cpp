...
int main(){
	...
	TT{
		...
		const int m=t.size();
		cauto fun=
			[&](const int &si,const string &s,const int &sj,const int &len){
				if(si<0 || sj<0) return false;
				if(si+len>n || sj+len>m) return false;
				rep(i,len) if(s[si+i]!=t[sj+i]) return false;
				return true;
			};
		auto rs=s; reverse(all(rs));
		int ok=0;
		rep(i,n)rep(j,m){
			if(!fun(i-j,s,0,j+1)) continue;
			cauto len=m-j;
			if(!fun(n-1-i,rs,j,len)) continue;
			ok=1;
		}
		cout<<(ok?"YES":"NO") NL;
	}
