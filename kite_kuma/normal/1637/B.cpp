...
ll solve(int n, vll a) {
	vvll res(n + 1, vll(n + 1));
	ll ans = 0;
	rep(len, 1, n + 1) {
		rep(left, 0, n + 1 - len) {
			const int right = left + len;
			auto &value = res[left][right];
			set<int> values;
			rep(i, n + 1) { values.insert(i); }
			rep(j, left, right) { values.erase(a[j]); }
			const int mex = *(values.begin());
			value = mex + 1;
			rep(mid, left + 1, right) { chmax(value, res[left][mid] + res[mid][right]); }
			ans += value;
			debug(left, right, value);
		}
	}
	return ans;
}
