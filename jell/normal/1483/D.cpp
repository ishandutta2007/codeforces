#line 1 "codeforces-workspace/contest/1483/d/d.cpp"
...
#line 11 "codeforces-workspace/contest/1483/d/d.cpp"
...
void main() {
  ...
  auto dist = make_vector<i64>({n, n}, 1e18);
  for (auto i : range(m)) {
    int a, b;
    cin >> a >> b;
    --a, --b;
  ...
  for (auto i : range(n)) {
    for (auto j : range(n)) {
      for (auto k : range(n)) {
        chle(dist[j][k], dist[j][i] + dist[i][k]);
      }
    }
  }
  ...
  int q;
  cin >> q;
  vector<tuple<int, int, int>> lst(q);
  cin >> lst;
  for (auto &&[x, y, z] : lst) {
    --x, --y;
  }
  ...
  for (auto &&[b, adj] : enumerate(g)) {
    vector<i64> mi(n, -1e18);
    for (auto &&[x, y, z] : lst) {
      chge(mi[x], z - dist[b][y]);
      chge(mi[y], z - dist[b][x]);
    }

    for (auto &&[a, w] : adj) {
      bool ok = 0;
      for (auto &&[u, r] : enumerate(mi)) {
        if (r >= w + dist[a][u]) {
          ok = 1;
          break;
        }
