...
void add(int i, int j) {debug(i, j); ans ^= a[i][j];}
...
void solve() {
    ...
    ans = 0;
    F0R(i, n) F0R(j, n) {
        if (j<i && (i+j)%4==1 && (i-j)%4==1) add(i, j);
        if (j>i && (i+j)%4==3 && (j-i)%4==3) add(i, j);
        if (i+j<n && (i+j)%4==0 && (i-j)%4==0) add(i, j);
        if (i+j>=n && (i+j)%4==0 && abs(i-j)%4==2) add(i, j);
    }
    cout << ans << nl;
}
