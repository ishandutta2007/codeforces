...
#define int ll
ll n,m,a[N],b[N],ans[N],s;
signed main(){
	cin>>n>>m;
	repeat(i,0,m){
		cin>>a[i]; b[i]=a[i]+i; s+=a[i];
	}
	int p=max_element(b,b+n)-b;
	int Max=b[p];
	if(Max>n || s<n)cout<<-1<<endl,exit(0);
	ans[p]=n-a[p]+1;
	repeat(i,p+1,m)ans[i]=ans[p]+i-p;
	
	s=1;
	ll rest=n-m-a[n-1]; //orz(rest);
	repeat(i,0,p){
		ans[i]=s; s+=a[i];
	}
	repeat_back(i,0,p){
