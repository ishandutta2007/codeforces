...
main() {
    ...
    rep2(i, 1, n - 1) rep(j, m) {
        if(a[i - 1][j] == '#' and a[i][j] == '#') {
            ret++;
            flow.add(s, now, 1);
            flow.add(now, f(i - 1, j), 1e7);
            flow.add(now, f(i, j), 1e7);
            now++;
        }
    }
    rep(i, n) rep2(j, 1, m - 1) {
        if(a[i][j - 1] == '#' and a[i][j] == '#') {
            ret++;
            flow.add(now, t, 1);
            flow.add(f(i, j - 1), now, 1e7);
            flow.add(f(i, j), now, 1e7);
            now++;
        }
    }
