...
ll getleft(ll p, ll c)
{

	auto it = S.upper_bound(P(p-1, inf)); it--;
	if(it->second != c) return 0;
	return p - it->first;
}
ll getright(ll p, ll c)
{
...
{
	auto succ = S.upper_bound(P(p, inf));
	auto it = succ; it--;
	if(it->second == c) return;

	if(succ->first > p+1) S.insert(P(p+1, it->second));
	if(it->first == p) S.erase(it);
	S.insert(P(p, c));

	it = S.lower_bound(P(p, c));
	succ = it; succ++;
	if(succ->second == c) S.erase(succ);
	if(it != S.begin()){
		auto pre = it; pre--;
		if(pre->second == c) S.erase(it);
	}
}
...
void erase(ll p)
{
	...
	if(c == 1){
		ll l = getleft(p+1, c), r = getright(p+1, c);
		seg.add(p+1, p+r+1, -l);
	}
	else{
		ll l = getleft(p, c), r = getright(p, c);
		seg.add(p-l, p, -r);
	}
	update(p, 0);
}
...
int main(void)
{
	...
	S.insert(P(1, sgn(d[1])));
	rep(i, 2, n-1){
		if(sgn(d[i]) != sgn(d[i-1])) S.insert(P(i, sgn(d[i])));
	}

	seg.init();
	rep(i, 1, n){
			seg.update(i, getleft(i, 1)+getright(i, -1));
	}
