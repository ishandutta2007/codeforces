...
long long cnt(const vector<long long>& order){
	int ans = 0, n = order.size();

	for(int i = 0, j = 1; i < n; ++i,++j){
		if (j >= n){
			j -= n;
		}
		if (__gcd(order[i], order[j]) == 1){
			++ans;
		}
	}
	return ans;
...
void solve(){
	...
	if (primes.size() == 1){
		...
		cout << endl << cnt(div) << endl;
		return;
	}

	if (primes.size() == 2 && (primes[0] * primes[1]) == n){
		assert(div.size() == 3);
		for(auto e: div){
			cout << e << " ";
		}
	...
	vector<vector<int>> divs(primes.size());
	int k = primes.size();
	...
	for(auto e: div){
		...
		for(int i = 0; i < k; ++i){
			...
			if (i + 1 < k && e == (primes[i] * primes[i + 1])){
				break;
			}
			divs[i].push_back(e);
			break;
	...
	for(int i = 0; i < k; ++i){
		...
		if (i + 1 < k){
			order.push_back(primes[i] * primes[i + 1]);
		}
	}
