...
string sol(li m, vi const& A, vvi const& B){
  ostringstream out;
  ...
  for(li i = 0; i < (li)(m); ++i) {
    for(li x : B[i]) BS[i] += x;
    S[i] = (BS[i] + B[i].size() -1) / B[i].size();
  }

  vi I(m);
  iota(begin(I), end(I), 0);
  sort(begin(I), end(I), [&](int i, int j){ return S[i] > S[j]; });

  vi SI(m);
  for(li i = 0; i < (li)(m); ++i) SI[i] = S[I[i]];

  vi J(m);
  for(li i = 0; i < (li)(m); ++i) J[I[i]] = i;

  vi ok(m), okl(m), okr(m);
  for(li i = 0; i < (li)(m); ++i) ok[i] = SI[i] <= A[i];
  for(li i = 0; i < (li)(m-1); ++i) okl[i+1] = SI[i+1] <= A[i];
  for(li i = 0; i < (li)(m-1); ++i) okr[i] = SI[i] <= A[i+1];

  vi cnt(m+1),cntl(m+1),cntr(m+1);
  for(li i = 0; i < (li)(m); ++i) cnt[i+1] = cnt[i] + ok[i];
  for(li i = 0; i < (li)(m); ++i) cntl[i+1] = cntl[i] + okl[i];
  for(li i = 0; i < (li)(m); ++i) cntr[i+1] = cntr[i] + okr[i];



  for(li i = 0; i < (li)(m); ++i) {
    for(li j = 0; j < (li)(B[i].size()); ++j) {
      li nbs = BS[i] - B[i][j];
      li ns = (nbs + B[i].size()-2) / (B[i].size()-1);

      int k1 = J[i];
      int k2 = distance(begin(SI), upper_bound(begin(SI), end(SI), ns, greater<li>()));
      ...
      if(ns == S[i]) {
        ...
        assert(k2 > k1);
        bool ok = (cnt[k1] == k1 && (cntl[k2]-cntl[k1+1] == k2-k1-1) && (cnt[m]-cnt[k2] == m-k2))
          && (ns <= A[k2-1]);
        out << (ok?1:0);
      }else{
        assert(k2 <= k1);
        bool ok = (cnt[k2]==k2) && (cntr[k1]-cntr[k2] == k1-k2) && (cnt[m]-cnt[k1+1]==m-k1-1)
          && (ns <= A[k2]);
        out << (ok?1:0);
      }
    }
...
bool brt2(li m, vi const& A, vvi const& B){
  vector<double> S(m);
  for(li i = 0; i < (li)(m); ++i) {
    for(li x : B[i]) S[i] += x;
    S[i] /= (double)B[i].size();
  ...
  for(li i = 0; i < (li)(m); ++i) if(S[i] > A[i]) return 0;
  return 1;
}

string brt(li m, vi const& A, vvi const& B){
  ostringstream out;

  for(li i = 0; i < (li)(m); ++i) {
    for(li j = 0; j < (li)(B[i].size()); ++j) {
      vvi C = B;
      C[i].erase(begin(C[i])+j);
      out << (brt2(m,A,C)?1:0);
    }
  }
  return out.str();
