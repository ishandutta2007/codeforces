...
int main() {
	...
	REP(i, n) { col[res[i]].pb(i); }
	if(SZ(col[0]) * SZ(col[1]) + SZ(col[1]) * SZ(col[2]) + SZ(col[2]) * SZ(col[0]) != m) exi();
	if(SZ(col[0]) == 0 or SZ(col[1]) == 0 or SZ(col[2]) == 0) exi();

	REP(i, SZ(col[0]))
	REP(j, SZ(col[1])) {
		int a = col[0][i], b = col[1][j];
		if(a > b) swap(a, b);
		if(!mp.count({a, b})) exi();
	}
	REP(i, SZ(col[0]))
	REP(j, SZ(col[2])) {
		int a = col[0][i], b = col[2][j];
		if(a > b) swap(a, b);
		if(!mp.count({a, b})) exi();
	}
	REP(i, SZ(col[1]))
	REP(j, SZ(col[2])) {
		int a = col[1][i], b = col[2][j];
		if(a > b) swap(a, b);
		if(!mp.count({a, b})) exi();
	}
