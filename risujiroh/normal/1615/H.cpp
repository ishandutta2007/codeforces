...
template <class Cap, class Cost> class MinCostFlow {
 public:
  ...
  MinCostFlow() {}
  explicit MinCostFlow(int n) : g(n + 2), b(n) {}
  ...
  Result flow() {
    ...
    std::iota(std::begin(que), std::end(que), 0);
    std::vector in_que(size(), true);
    for (int i = 0; i < int(std::size(que)); ++i) {
      int v = que[i];
      in_que[v] = false;
      for (auto [u, c] : h[v]) {
...
void solve(int) {
  ...
  cin >> n >> m;
  auto a = vector<i64>(n);
  auto g = MinCostFlow<i64, i64>(n + n);
  for (auto i : rep(0, n)) {
    cin >> a[i];
    if (i) {
      g.add_edge(n, n + i, -inf, inf, a[i] - a[0]);
    }
  }
  for (auto e : rep(0, m)) {
    int u, v;
    cin >> u >> v;
    --u, --v;
    g.add_edge(v, u, 0, inf, 0);
  }
  for (auto v : rep(0, n)) {
    g.add_edge(v, n + v, -1, 1, 0);
  }
  auto res = g.flow();
  DUMP(res.feasible);
  DUMP(res.cost);
  {
    auto off = a[0] - res.dual[n];
    for (auto& e : res.dual) e += off;
  }
  DUMP(res.dual);
  for (auto i : rep(0, n)) {
    cout << res.dual[i] << " \n"[i + 1 == n];
