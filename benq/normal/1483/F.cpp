...
struct ACfixed { // fixed alphabet
	...
	int add(int ind, str s) { // add word
		int v = 0;
		each(C,s) {
			int c = C-'a';
...
// int lcp(pi a, pi b) {
// 	int x = SA.getLCP(a.f,b.f);
// 	ckmin(x,len(a)); ckmin(x,len(b));
// 	return x;
// }
...
// 	if (x == len(b)) return 0;
// 	if (x == len(a)) return 1;
// 	assert(tot[a.f+x] != tot[b.f+x]);
// 	return tot[a.f+x] < tot[b.f+x];
// }
...
int main() {
	...
	vs v(N);
	// rep(1000000) v[rng()%sz(v)] += char('a'+(rng()%5));
	re(v);
	...
	F0R(i,sz(v)) {
		...
		R0F(j,sz(v[i])) {
			...
			if (x == -1) continue;
			assert(x < sz(v));
			if (j-sz(v[x]) < mn) { // it's the start of candidate
