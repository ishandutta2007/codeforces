...
public class I implements Runnable{
    ...
    private void solve() {
        int n = readInt();
        int k = readInt();
        int[] v = readSortedIntArray(n);

        this.dp = new int[n + 1];

        long answer = 1000 * 1000 * 1000 + 1;
        for (long left = 0, right = 1000 * 1000 * 1000; left <= right; ) {
    ...
    boolean can(int n, int[] v, int k, long deltaLimit) {
        Arrays.fill(dp, 0);
        dp[0] = 1;

        for (int left = 0, right = 1; right <= n; ++right) {
            while (left < right - k && v[right - 1] - v[left] > deltaLimit) {
                ++left;
            }
            ...
            int leftSide = left;
            int rightSide = right - k;

            if (leftSide <= rightSide && v[right - 1] - v[left] <= deltaLimit) {
                int result = dp[rightSide];
                if (left > 0) result -= dp[leftSide - 1];
                if (result > 0) dp[right] = 1;
            }
...
//    private final boolean ONLINE_JUDGE = System.getProperty("ONLINE_JUDGE") != null;
    ...
    public static void main(String[] args){
        new Thread(null, new I(), "", MAX_STACK_SIZE * (1L << 20)).start();
    }
    ...
    private static class IntIndexPair {
        ...
        static Comparator<IntIndexPair> increaseComparator = new Comparator<I.IntIndexPair>() {

            @Override
            public int compare(I.IntIndexPair indexPair1, I.IntIndexPair indexPair2) {
        ...
        static Comparator<IntIndexPair> decreaseComparator = new Comparator<I.IntIndexPair>() {
