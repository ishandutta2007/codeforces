...
string s,t;
string ch(ll x,ll keta){
  if(!keta)return string(1,'a'+x);
  return ch(x/26,keta-1)+char('a'+x%26);
}
int main(){
  startupcpp();
  ...
  rep(sz,inf){
    bool ff=false;
    rep(ans,modpow(26,sz+1)){
      t=ch(ans,sz);bool f=true;
      rep(i,ll(s.size()-t.size()+1)){
        rep(j,t.size())if(s[i+j]!=t[j])break;
        if(j==ll(t.size())){
          f=false;
          break;
        }
