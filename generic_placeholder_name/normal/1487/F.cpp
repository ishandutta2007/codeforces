...
//bullshit unsound heuristic pass attempt
//if I wasn't drunk I probably wouldn't have tried this
...
constexpr int T=1111111;
int dp[T+1];
BigInt sub[51];
map<BigInt, int> cache;
...
void precalc() {
    ...
    for(int i=1, s=1, c=1; i<=T; i++) {
        if(i>=10*s+1) (s*=10)+=1, c++;
        dp[i]=dp[i-s]+c;
        if(i<T) {
            int r=c+1, x=s*10+1-i;
            while(x>=s) x-=s, r+=c;
            dp[i]=min(dp[i], r+dp[x]);
        }
    }
}

int calc(BigInt x) {
    if(x<=T) return dp[x%(T+1)];
    if(cache.count(x)) return cache[x];
    else {
        int nxt=upper_bound(sub, sub+51, x)-sub;
        int prv=nxt-1;
        if(x<=sub[prv]*3) { //probably gives WA
            auto t=x;
            int r=0;
            while(x>sub[prv]) x-=sub[prv], r+=prv+1;
            return cache[t]=r+calc(x);
        }
        else if(x>sub[prv]*8) { //also probably gives WA
            auto t=x;
            int r=nxt+1; x=sub[nxt]-x;
            while(x>sub[prv]) x-=sub[prv], r+=prv+1;
            return cache[t]=r+calc(x);
        }
        else {
            auto y=x;
            int ry=0;
            while(y>sub[prv]) y-=sub[prv], ry+=prv+1;
            auto z=x;
            int rz=nxt+1; z=sub[nxt]-z;
            while(z>sub[prv]) z-=sub[prv], rz+=prv+1;
            return cache[x]=min(ry+calc(y), rz+calc(z));
        }
    }
}
...
int32_t main() {
    fastio;
    precalc();
    BigInt x; cin>>x; cout<<calc(x)<<endl;
}