...
int main(){
    ...
    VEC(ll,a,n);
    vec(bool,used,MAX+1);
    fenwick_tree seg1(MAX+1),seg2(MAX+1);
    vec(ll,ans,n);
    ll sum=0;
    rep(n){
        const ll x=a[i];
        ...
        ans[i]=sum+x*i;
        const ll sq=sqrt(x);
        rep(i,1,sq+1)if(used[i])A-=(x/i)*i;
        rep(i,1,x/sq)A-=seg2.get(x/(i+1)+1,x/i+1)*i;
        ll cnt=0;
        rep(i,1,MAX/x+1)cnt+=seg1.get(i*x,min((i+1)*x,MAX+1))*i;
        A-=cnt*x;
        seg1.add(x,1);
        seg2.add(x,x);
        sum+=x;
