...
void solve() {
    ...
    F0R(day, m) {
        ...
        if (cur<s) {
            if (op==1) {
                st[k] = day;
                FOR(i, x[k], cur) cnt[cur][(i+day)%cur]++;
            } else {
                FOR(i, x[k], cur) cnt[cur][(i+st[k])%cur]--;
            }
        } else {
            if (op==1) {
                st[k] = day;
                for (int i = day+x[k]; i < m; i += cur) {
                    a[i]++;
                    if (i+y[k]<m) a[i+y[k]]--;
                }
            } else {
                for (int i = st[k]+x[k]; i < m; i += cur) {
                    a[i]--;
                    if (i+y[k]<m) a[i+y[k]]++;
                    if (i<day && i+y[k]>=day) ans--;
                }
            }
        }
