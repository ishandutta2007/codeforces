...
int main() {
	CASE(t) {
		...
		vector<int> pid(n + 1, -1), nid(n + 1, -1);
		for (int i = 0; i <= n; i++) {
			pid[i] = lower_bound(val.begin(), val.end(), ps[i]) - val.begin() + 1;
			nid[i] = n + 5 - pid[i];
		}

		const int SZ = n + 17;
		BIT<ll, mx> pt(SZ), nt(SZ);
		map<ll, ll> eq;
		...
		setEq(0, 0);
		for (int i = 1; i <= n; i++) {
			chkmax(dp[i], pt.qry(pid[i] - 1) + i);
			chkmax(dp[i], nt.qry(nid[i] - 1) - i);
			chkmax(dp[i], getEq(ps[i]));
