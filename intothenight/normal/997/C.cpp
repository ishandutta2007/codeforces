...
int main(){
	...
	int n;
	cin >> n;
	combinatorics<1000000> c(mod);
	Zp res = 0;
	long long ns = 1LL * n * n;
	for(auto i = 1, sign = 1; i <= n; ++ i, sign *= -1){
		res += 2 * sign * c.C(n, i) * (Zp(3) ^ ns - 1LL * i * n + i);
		res += 3 * sign * c.C(n, i) * (((Zp(3) ^ n - i) - 1 ^ n) - (Zp(3) ^ ns - 1LL * i * n));
	}
	cout << res << "\n";
	return 0;
}
