...
void solve() {
  ...
  FOR(i, N) A[Q + i] = i;
  vi bit_raw(N + Q);
  FOR(i, N) bit_raw[Q + i] = 1;

  FenwickTree<ll> bit(N + Q);
  bit.build(bit_raw);
  ...
  FOR(q, Q) {
    LL(idx, k);
    --idx;
    ll p = bit.find_kth_element(k);
    ll i = A[p];
    if (P[i] != -1 && P[i] != idx) return print(-1);
    P[i] = idx;
    ll to = Q - q - 1;
    bit.add(p, -1);
    bit.add(to, 1);
  ...
  vi done(N);
  FOR(i, N) if (P[i] != -1) {
    if (done[P[i]]) return print(-1);
    done[P[i]] = 1;
  }
  vi rest;
  FOR_R(x, N) if (!done[x]) rest.eb(x);
  FOR(i, N) if (P[i] == -1) {
    P[i] = rest.back();
    rest.pop_back();
  }
