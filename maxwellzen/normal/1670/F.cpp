...
ll calc(ll lim) {
    debug(lim);
    vvll dp(61, vll(2*n+1, 0));
    F0R(i, 2*n+1) dp[0][i] = 1;
    F0R(i, 60) {
        int zbit = (z&1ll<<i)>>i;
        int lbit = (lim&1ll<<i)>>i;
        F0R(j, 2*n+1) {
            for (int k = zbit; k <= n; k += 2) {
                if (j+lbit-k >= 0) dp[i+1][j] = (dp[i+1][j] + choose(n, k)*dp[i][min(2*n, 2*(j+lbit-k))])%MOD;
            }
        }
    }
    // F0R(i, 6) {F0R(j, 2*n+1) cout << dp[i][j] << ' '; cout << nl;}
    return dp[60][0];
}

void solve() {
    cin >> n >> l >> r >> z;
    cout << (calc(r) + MOD - calc(l-1))%MOD << nl;
}
