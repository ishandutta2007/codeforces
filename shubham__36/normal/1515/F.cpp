...
ll ncr(ll n, ll r){
	return (fact[n]*rev((fact[r]*fact[n-r])%MOD))%MOD;
}
...
void solve(){
	...
	forn(i,n-1){
		ll cur = (*(s.begin())).se;
		while(sz(gr[cur]) && U.isSameSet(cur,gr[cur].back().fi)) gr[cur].pop_back();
		ll b = U.findSet(gr[cur].back().fi);
		cout << gr[cur].back().se << ln;
		gr[cur].pop_back();
		U.unionSet(cur,b);
		ll r = U.findSet(cur);
		s.erase(mp(-a[cur],cur));
		s.erase(mp(-a[b],b));
		a[r] = a[cur]+a[b]-x;
		s.insert(mp(-a[r],r));
		if(sz(gr[cur])<sz(gr[b])) swap(gr[cur],gr[b]);
		for(auto it : gr[b]) gr[cur].pb(it);
		if(r != cur) swap(gr[cur],gr[r]);
	}
}
