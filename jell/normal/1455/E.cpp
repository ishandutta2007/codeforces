#line 1 "codeforces-workspace/contest/1455/e/e.cpp"
/*
 * @file template.cpp
 * @brief Template
 */
...
#line 12 "codeforces-workspace/contest/1455/e/e.cpp"
...
i64 solve(vector<array<int, 2>> ps) {
  i64 ret = 0;
  for (auto &&xo : {0, 1}) {
    for (auto i = 0; i < size(ps); ++i) {
      ret += abs(ps[i][xo] - ps[i ^ (xo + 1)][xo]);
    }
  }
  ret /= 2;
  int ma0 = max(ps[2][0], ps[3][0]) - min(ps[0][0], ps[1][0]);
  int mi0 = min(ps[2][0], ps[3][0]) - max(ps[0][0], ps[1][0]);
  int ma1 = max(ps[1][1], ps[3][1]) - min(ps[0][1], ps[2][1]);
  int mi1 = min(ps[1][1], ps[3][1]) - max(ps[0][1], ps[2][1]);
  ret += max(0, max(mi0, mi1) - min(ma0, ma1)) * i64(2);
  return ret;
}
