...
int a[_], sum[_], pre[_], pre2[_], suf[_], suf2[_];
...
int main() {
  ...
  Poly poly = zeroes(k);
  {
    Poly f = zeroes(k); f[0] = p - 1;
    for (int i = 1; i <= k; ++i) f[i] = simp.gifac(i) * (ll)p % P;
    f = f.inv();
    for (int i = 0; i <= k; ++i)
      poly[k - i] = f[i] * (ll)simp.gfac(k) % P * simp.gifac(k - i) % P;
  }

  // {
  //   auto res = tp.eval(poly, pts);
  //   int ans1 = (res[1] * (ll)mpow(p, kk) % P + P - res[0]) % P;
  //   cout << ans1 << '\n';
  //   int ans2 = 0;
  //   for (int i = 0; i < kk; ++i)
  //     ans2 = (ans2 + mpow(p, i) * (ll)mpow(i, k)) % P;
  //   cout << ans2 << '\n';
  // }

  vector<int> pts(n + 1);
  for (int i = 1; i <= n; ++i) pts[i] = a[i] + 1;
  auto res = tp.eval(poly, pts);
  for (int i = 1; i <= n; ++i) {
    sum[i] = (res[i] * (ll)mpow(p, pts[i]) % P + P - res[0]) % P;
    // cerr << sum[i] << " \n"[i == n];
  }
  pre[0] = 1;
  for (int i = 1; i <= n; ++i) {
    pre[i] = (pre[i - 1] * (a[i] + 1LL) + 1) % P;
    pre2[i] = (pre2[i - 1] * (a[i] + 1LL) + pre[i - 1] * (ll)a[i]) % P;
  }
  suf[n + 1] = 1;
  for (int i = n; i; --i) {
    suf[i] = (suf[i + 1] * (a[i] + 1LL) + 1) % P;
    suf2[i] = (suf2[i + 1] * (a[i] + 1LL) + suf[i + 1] * (ll)a[i]) % P;
  }

  int ans = 0;
  for (int i = 1; i <= n; ++i) {
    ans = (ans + sum[i] * (ll)pre[i - 1] % P * suf[i + 1]) % P;
    ans = (ans + sum[i] * (ll)pre2[i - 1] % P * suf[i + 1]) % P;
    ans = (ans + sum[i] * (ll)pre[i - 1] % P * suf2[i + 1]) % P;
  }
  cout << ans << '\n';
