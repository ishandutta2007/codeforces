#line 1 "codeforces-workspace\\contest\\1672\\f1\\f1.cpp"
...
#line 13 "codeforces-workspace\\contest\\1672\\f1\\f1.cpp"
...
void main() {
  ...
  vector<vector<i32>> pos(n);

  for (auto &&[i, e] : enumerate(a)) {
    --e;
    pos[e].emplace_back(i);
  }
  ...
  rotate(begin(cat), begin(cat) + shift, end(cat));

  vector<i32> b(n);
  auto it = 0;

  for (auto &&[i, v] : enumerate(pos)) {
    for (auto _ : v) {
      b[cat[it++]] = i + 1;
    }
  }
