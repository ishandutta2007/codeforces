...
signed main() {
    ...
    while (q--) {
        ...
        if (type == 1) {
            int v = ver[l];
            ll prev_cnt = all[pr[v]].get_pref(all[pr[v]].n - 1);
            ll prev_val = prev_cnt * (prev_cnt - 1) / 2 + prev_cnt;
            glob.update(tin[pr[v]], -prev_val);
            prev_cnt--;
            prev_val = prev_cnt * (prev_cnt - 1) / 2 + prev_cnt;
            glob.update(tin[pr[v]], prev_val);
            all[pr[v]].update(ind[v], -1);
        } else {
