...
int mat[N][10][10];
vector<pair<int, int> > block[10][10];
int blocks[10][10][10][10];
int BLOCK[10][10];
...
void rec(int z)
{
    ...
    for (int sel = 0; sel <= z; sel++)
    {
        if (BLOCK[z][sel] == 0 && allow[z][sel] == 0)
        {
            for (auto go : block[z][sel])
                allow[go.first][go.second]++;
            rec(z + 1);
            for (auto go : block[z][sel])
                allow[go.first][go.second]--;
        }
    ...
    */
    ...
    forn(v, n)
    {
        ...
        for (int i = 0; i < k; i++)
        {
            for (int j = 0; j <= i; j++)
            {
                if (mat[v][i][j] > 1) BLOCK[i][j] = 1;
                if (mat[v][i][j] == 1) s.push_back({i, j});
            }
        }
        for (int i = 0; i < s.size(); i++)
        {
            ...
            {
                blocks[s[i].first][s[i].second][s[j].first][s[j].second] = 1;
            }
        }
    }
    for (int i1 = 0; i1 < k; i1++)
    {
        for (int j1 = 0; j1 <= i1; j1++)
        {
            for (int i2 = 0; i2 < k; i2++)
            {
                ...
                {
                    if (blocks[i1][j1][i2][j2])
                        block[i1][j1].push_back({i2, j2});
                }
            }
        }
    }
