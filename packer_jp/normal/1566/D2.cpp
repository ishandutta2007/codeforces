...
int main() {
    ...
    while (t--) {
        ...
        cin >> n >> m;
        vll a(n * m);
        for (ll i : rep(n * m)) cin >> a[i];
        vector<pll> ai(n * m);
        for (ll i : rep(n * m)) ai[i] = {a[i], i};
        sort(all(ai));
        ll ans = 0;
        for (ll i : rep(n)) {
            vector<pll> aj(ai.begin() + i * m, ai.begin() + (i + 1) * m);
            sort(all(aj), [](pll l, pll r) -> bool { return l.second < r.second; });
            vll c(m);
            for (ll j : rep(m)) c[j] = aj[j].first;
            vll b(c);
            sort(all(b)), uniq(b);
            map<ll, ll> zat;
            for (ll j : rep(b.size())) zat[b[j]] = j;
