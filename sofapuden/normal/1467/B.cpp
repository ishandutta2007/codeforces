...
void solve(){
	...
	int dif = 0;
	for(int i = 1; i < n-1; ++i){
		int cur = go(i,v,n)+go(i+1,v,n)+go(i-1,v,n);
		int stval = v[i];
		v[i] = v[i-1];
		int cur1 = go(i,v,n)+go(i+1,v,n)+go(i-1,v,n);
		v[i] = v[i+1];
		int cur2 = go(i,v,n)+go(i+1,v,n)+go(i-1,v,n);
		dif = max(dif,max(cur-cur1,cur-cur2));
		v[i] = stval;
	}
	cout << hi-dif << "\n";
