...
vi bes; // last term of increasing sequence with i terms
vi cur, stor[MX];
bool bad[MX];
int pre[MX];
...
void solve() {
    ...
    vector<vi> ret;
    while (sz(cur)) {
        F0R(i,n+1) stor[i].clear();
        bes = {INT_MIN};
        trav(x,cur) ad(x);
        if (sz(bes)-1 <= f[sz(cur)]) {
            FOR(i,1,sz(bes)) ret.pb(stor[i]);
            break;
        } else {
            vi z;
            for (int i = bes.back(); i != INT_MIN; i = pre[i]) {
                z.pb(i);
                bad[i] = 1;
            }
            reverse(all(z)); ret.pb(z);
            vi CUR; trav(i,cur) if (!bad[i]) CUR.pb(i);
            swap(cur,CUR);
        }
    }
    pr(sz(ret));
...
int main() {
    ...
    FOR(i,1,1001) FOR(j,i*(i+1)/2,(i+1)*(i+2)/2) f[j] = i;
    setIO(); re(t);
    F0R(i,t) solve();
    // you should actually read the stuff at the bottom
}
