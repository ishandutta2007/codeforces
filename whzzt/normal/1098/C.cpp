...
const int N=1e5+5;
int n,fa[N],sz[N],pos,rpos,lp[N],rp[N],cnt;
i64 s;
i64 chk(int mid){
	i64 v=0;
	int m=n,k;cnt=0;
	for(k=1;m;k=min(i64(k)*mid,i64(m)))m-=k,sz[++cnt]=k,v+=k*cnt;
	return v;
}
void output(int mid){
	int i,j;
	for(i=1;i<=rpos;++i){
		lp[i]=rp[i-1]+1;
		rp[i]=lp[i]+sz[i]-1;
		if(i>1)for(j=lp[i];j<=rp[i];++j)fa[j]=lp[i-1]+(j-lp[i])/mid;
	}
	cout<<"Yes"<<endl;
	for(i=2;i<=n;++i)cout<<fa[i]<<" \n"[i==n];
}
int main(){
	int i,j,l,r,mid;
	scan(n,s);
	if(s<=i64(n)*(n+1)/2 && s>=2*n-1){
		l=1,r=n-1,mid=(l+r)>>1;
		while(l<r){
			if(s>=chk(mid))r=mid;
			else l=mid+1;
		...
		s-=chk(mid);
		for(pos=1;sz[pos];++pos);
		while(pos>cnt)sz[pos--]=0;
		rpos=pos;
		while(sz[pos]==1)--pos;
		while(s){
			if(s>=rpos+1-pos)s-=rpos+1-pos,--sz[pos],++sz[++rpos];
			else --sz[pos],++sz[pos+s],s=0;
			while(sz[pos]==1)--pos;
		}
		output(mid);
	}else cout<<"No"<<endl;
