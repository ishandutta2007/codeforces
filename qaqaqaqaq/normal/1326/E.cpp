...
int n,a[maxn],b[maxn],c[maxn],C[maxn];
SegmentTree<maxn,int> seg;
...
void addone(int u){
    seg.update(c[u],c[u],1000001);
    if (c[u]>1) seg.update(1,c[u]-1,1);
    update(c[u],1);
}
...
int main(){
    ...
    seg.update(1,n,-1000000);
    int ans=n+1,i=1;
    while (ans&&i<=n){
        while (ans&&!ck()) addone(--ans);
        cout<<ans<<" ";
        addbomb(b[i]);
        i++;
    }
