...
bool check(int i) {
	if(a[i] < a[i - 1] && qry(a[i]) != qry(a[i - 1] - 1)) return 0;
	if(a[i] > a[i - 1] && qry(a[i] - 1) != qry(a[i - 1])) return 0;
	return 1;	
}
...
int main() { 
	...
	W(T) {
		qread(n);
		rep(i, 1, n) qread(a[i]), tmp[i] = a[i], t[i] = 0;
		sort(tmp + 1, tmp + n + 1);
		int len = unique(tmp + 1, tmp + n + 1) - tmp - 1;
		rep(i, 1, n) a[i] = lower_bound(tmp + 1, tmp + len + 1, a[i]) - tmp;
		int f = 1;
		rep(i, 1, n) {
			upd(a[i], 1);
			if(i == 1) continue;
