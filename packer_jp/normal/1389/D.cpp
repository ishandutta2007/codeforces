...
signed main() {
    ...
    while (t--) {
        int n, k, l1, r1, l2, r2;
        cin >> n >> k >> l1 >> r1 >> l2 >> r2;
        int waste = max({l2 - r1, l1 - r2, 0LL});
        int ub = max({l1, r1, l2, r2}) - min({l1, r1, l2, r2});
        int step = ub * 2 - (r1 - l1) - (r2 - l2);
        int bias = waste ? 0 : ((r1 - l1) + (r2 - l2) - ub);
        ub -= bias;
        //dump(waste), dump(ub), dump(step), dump(bias);
        int ans = LLONG_MAX;
        rep(i, n) {
            int cur = i * step;
            int score = bias * n + i * ub;
            if (score > k) { chmin(ans, cur); }
            else if (score + ub >= k) { chmin(ans, cur + waste + k - score); }
            else { chmin(ans, cur + step + (k - (score + ub)) * 2); }
        }
        cout << ans << endl;
    }
}