...
signed main(){
    ...
    rep(n){
        ...
        if(p1.first)to[i].push_back(p1);
        if(p2.first)to[i].push_back(p2);
        if(p3.first)to[i].push_back(p3);
    }
    rep(n){
        pll p1,p2,p3;
        rep(j,n){
            if(cost[j][i]==LINF)continue;
            pll p4={cost[j][i],j};
            if(p1<p4)swap(p1,p4);
            if(p2<p4)swap(p2,p4);
            if(p3<p4)swap(p3,p4);
        }
        if(p1.first)from[i].push_back(p1);
        if(p2.first)from[i].push_back(p2);
        if(p3.first)from[i].push_back(p3);
    }
    ll max=0;
    vector<ll>ans;
    rep(p2,n)rep(p3,n)if(p2!=p3){
        if(cost[p2][p3]==LINF)continue;
        ll cnt=-LINF;
        ll ans1=-1,ans4=-1;
        each(x1,p1,from[p2])each(x4,p4,to[p3]){
            if(p1==p3||p1==p4||p2==p4)continue;
            if(chmax(cnt,x1+x4)){
                ans1=p1;
                ans4=p4;
            }
        }
        if(chmax(max,cnt+cost[p2][p3]))ans={ans1,p2,p3,ans4};
    }
    each(i,ans)i++;
    out(ans);
}