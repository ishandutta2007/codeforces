...
void solve() {
  ...
  DP[0][1] = 0;
  FOR(w, W) {
    vv(ll, newDP, W + 1, 2, INF);
    FOR(c, 2) {
      ll cost = (c == 0 ? CNT[w] : H - CNT[w]);
      FOR(x, W) {
        chmin(newDP[x + 1][c], DP[x][c] + cost);
        if (a <= x && x <= b) chmin(newDP[1][c], DP[x][c ^ 1] + cost);
      }
    }
    swap(DP, newDP);
  }
  ...
  ll ANS = INF;
  FOR(x, W + 1) if (a <= x && x <= b) chmin(ANS, MIN(DP[x]));
  print(ANS);
}
