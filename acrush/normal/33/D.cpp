...
int n,m;
int x[maxsize],y[maxsize],r[maxsize],kx[maxsize],ky[maxsize];
int p[maxsize],f[maxsize];
vector<int> g[maxsize];
int d[maxsize][maxsize];

bool is_inside(int id,int x0,int y0)
{
	return sqr((int64)(x0-x[id]))+sqr((int64)(y0-y[id]))<=sqr((int64)r[id]);
}
bool is_contain(int a,int b)
{
	return is_inside(a,x[b],y[b]) && r[a]>=r[b];
}
void DFS(int root,int v,int p,int dst)
{
	d[root][v]=dst;
	for (int i=SIZE(g[v])-1;i>=0;i--)
	{
		int other=g[v][i];
		if (other!=p) DFS(root,other,v,dst+1);
	}
}
int main()
{
	...
	for (int i=0;i<m;i++) scanf("%d%d",&kx[i],&ky[i]);
	for (int i=0;i<n;i++) scanf("%d%d%d",&r[i],&x[i],&y[i]);
	...
	for (int i=0;i<m;i++) for (p[i]=0;p[i]<n && !is_inside(p[i],kx[i],ky[i]);p[i]++);
	for (int i=0;i<n;i++) for (f[i]=i+1;f[i]<n && !is_contain(f[i],i);f[i]++);
