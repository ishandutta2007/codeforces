...
int main() {
    ...
    A.eb(L);
    A.insert(begin(A), 0);
    int mi = bin_search(0LL, L / n + 1, [&](ll x) {
        ll ma = -inf<ll>;
        rep(i, si(A)) {
            if(i and A[i] - x * i < ma - x) return false;
            chmax(ma, A[i] - x * i);
        }
        return true;
    });
    int ma = bin_search(L, 0LL, [&](ll x) {
        ll mi = inf<ll>;
        rep(i, si(A)) {
            ll now = A[i] - x * i + (i == si(A) - 1 ? x : 0);
            if(i and now - x > mi) return false;
            if(i == 0)
                chmin(mi, A[i] - x * i - x);
            else
                chmin(mi, A[i] - x * i);
    ...
    vpi ans;
    rep(i, 1, n + 1) {
        pii nxt(pre + mi, pre + ma);
        ans.eb(pre, min(nxt.se, ok[i].se));
        pre = min(nxt.se, ok[i].se);
    }
    rep(i, n) { assert(a[i + 1] <= ans[i].se and ans[i].se <= a[i + 2]); }
    assert(ans.back().se == L);
    // dump(ok, ans);
    // OUT(1);
    // OUT(ma - mi);
    fore(x, y, ans) OUT(x, y);
}