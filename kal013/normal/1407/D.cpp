...
void update(int node,int s, int e,int l,int v, bool typ){
	if (e < l || s > l){
		return;
	}
	...
	update(2 * node, s, M, l, v, typ);
	update(2 * node + 1, M + 1, e, l, v, typ);
...
int query(int node,int s,int e,int l,int r,bool typ){
	if (e < l || s > r){
		return INF;
	}

	if (l <= s && e <= r){
		return (typ) ? seg1[node] : seg2[node];
	}
	return min(query(2 * node, s, M, l, r, typ), query(2 * node + 1, M + 1, e, l, r, typ));
}
...
void solve(){
	...
	vector<int> prmin(n, -1), prmax(n, -1), nxmin(n, n), nxmax(n, n);
	...
	update(1, 0, n - 1, 0, 0, false);
	update(1, 0, n - 1, 0, 0, true);
	...
	for(int i = 1; i  < n; ++i){
		while(mx.size() > 0 ){
			...
			if (X.first < i){
				mx.erase(X);
				update(1, 0, n - 1, X.second, INF, false);
			}
			else{
				break;
			}
		...
		while(mn.size() > 0 ){
			...
			if (X.first < i){
				mn.erase(X);
				update(1, 0, n - 1, X.second, INF, true);
			}
			else{
				break;
			}
		...
		dp[i] = min(dp[i], query(1, 0, n - 1, prmin[i], i, true) + 1);
		dp[i] = min(dp[i], query(1, 0, n - 1, prmax[i], i, false) + 1);


		update(1, 0, n - 1, i, dp[i], false);
		update(1, 0, n - 1, i, dp[i], true);
