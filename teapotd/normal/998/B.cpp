...
void run() {
	int n, B, cur, last = 0, sum = 0;
	Vi cuts;
	cin >> n >> B;

	rep(i, 0, n) {
		cin >> cur;
		if (i > 0 && sum == 0) cuts.pb(abs(cur-last));
		sum += (cur & 1 ? 1 : -1);
		last = cur;
	}

	rep(a, 1, sz(cuts)+1) rep(b, 0, B+1) {
		dp[a][b] = dp[a-1][b];
		if (b >= cuts[a-1]) dp[a][b] = max(dp[a][b], dp[a-1][b-cuts[a-1]] + 1);
	}
	cout << dp[sz(cuts)][B] << endl;
}