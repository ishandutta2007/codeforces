...
int main() {
    ...
    vv(mint, r, n + 2, n + 2);
    rep(i, n + 1) { rep(j, n + 1) r[i][j + 1] = r[i][j] + dp[i][j]; }
    ...
    mint ans;
    vv(int, D, n, n);
    rep(i, n) rep(j, n) D[i][j] = hld.dist(i, j);
    auto d = [&](int i, int j) { return D[i][j]; };

    vv(mint, c, n + 1, n + 1);
    rep2(i, 1, n) rep2(j, 1, n) { rep(k, j) c[i][j] += modpow(inv(2), i + k) * C(i + k - 1, i - 1); }
    // rep2(i, 1, n) cout << c[i] << endl;

    rep(j, n) rep(i, j) {
        int m = d(i, j);
        rep(k, n) {
            if(d(k, j) + d(j, i) == d(k, i))
                ans += inv(n);
            else if(!(d(k, i) + d(i, j) == d(k, j))) {
                int p = d(k, i), q = d(k, j);
                int s = (p + q - m) / 2;
                int x = q - s, y = p - s;
                // dump(i, j, k, x, y);
