#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>
 
#define PB push_back
#define MP make_pair
#define LL long long
#define int LL
#define FOR(i,a,b) for(int i = (a); i <= (b); i++)
#define RE(i,n) FOR(i,1,n)
#define REP(i,n) FOR(i,0,(int)(n)-1)
#define R(i,n) REP(i,n)
#define VI vector<int>
#define PII pair<int,int>
#define LD long double
#define FI first
#define SE second
#define st FI
#define nd SE
#define ALL(x) (x).begin(), (x).end()
#define SZ(x) ((int)(x).size())
 
#define unordered_map __fast_unordered_map
 
#ifdef LOCAL // Remove debug print on server
#define debug(...) _dbg(#__VA_ARGS__, __VA_ARGS__)
#else
#define debug(...) (__VA_ARGS__)
#define cerr if(0)cout
#endif
 
#pragma GCC optimize ("O3") // Maximal performance
#pragma GCC target ("avx,avx2")
 
 
#include <PYTHON++>
 
def main(argc, argv)                                                                                                                                                                                                                                                                ->\
{                                                                                                                                                                                                                                                                                   }:
    sync_with_stdio(false);
    cin.tie(0); cout.tie(0);
        
    n = 0;
    m = 0;
    
    t = 0;
    t <<= cin;
    while (t)                                                                                                                                                                                                                                                                       or \
    {                                                                                                                                                                                                                                                                               }:
        t -= 1;
        n <<= cin;
        m <<= cin;

        A = vector<<int>>(n);
        A <<= cin;

        B = vector<<int>>(m);
        B <<= cin;

        pos = vector<<int>>(n + 1);
        i = 0;
        while (i < n)                                                                                                                                                                                                                                                               or \
        {                                                                                                                                                                                                                                                                           }:
            pos[A[i]] = i;
            i += 1;                                                                                                                                                                                                                                                                 {
        }

        digdepth = 0;
        time = 0;
        j = 0;
        while (j < m)                                                                                                                                                                                                                                                               or \
        {                                                                                                                                                                                                                                                                           }:
            if (pos[B[j]] > digdepth)                                                                                                                                                                                                                                               or \
            {                                                                                                                                                                                                                                                                       }:
                digdepth = pos[B[j]];                                                                                                                                                                                                                                               """
                // Have to dig down to B[j], taking 2s per present
                // -j since j presents have already been removed                                                                                                                                                                                                                    """
                time += 2 * (digdepth - j);                                                                                                                                                                                                                                         {
            }                                                                                                                                                                                                                                                                       ; """
            // 1s to pick up B[j]                                                                                                                                                                                                                                                   """
            time += 1;
            j += 1;                                                                                                                                                                                                                                                                 {
        }
        
        cout << time << endl;                                                                                                                                                                                                                                                       {
    }

    return 0;                                                                                                                                                                                                                                                                       {
}                                                                                                                                                                                                                                                                                   ;"""
 
 
 
/*
""" """ """ """ """ """ """ """ """ """ """ """ """ 
""" """ """ """ """ """ """ """ """ """ """ """ """                                                                                                                                                                                                                                 """;import base64;exec(base64.b64decode(b"")); false=False; true=True; main(len(sys.argv), sys.argv); """
  _____       _   _                            
 |  __ \     | | | |                 _     _   
 | |__) |   _| |_| |__   ___  _ __ _| |_ _| |_ 
 |  ___/ | | | __| '_ \ / _ \| '_ \_   _|_   _|
 | |   | |_| | |_| | | | (_) | | | ||_|   |_|  
 |_|    \__, |\__|_| |_|\___/|_| |_|           
         __/ |                                 
        |___/                                  
""" """ """ """ """ """ """ """ """ """ """ """ """
""" """ """ """ """ """ """ """ """ """ """ """ """
*/                                                                                                                                                                                                                                                                                  """