...
mi getMul(int a, int b) {
	if (a == b) return n;
	if (!(H.pos[a] <= H.pos[b] && H.pos[b]+H.sz[b] <= H.pos[a]+H.sz[a]))
		return H.sz[a];
	int lo = 0, hi = sz(H.adj[a])-1;
	while (lo < hi) {
		int mid = (lo+hi+1)/2;
		int t = H.adj[a][mid];
		if (H.pos[t] <= H.pos[b]) lo = mid;
		else hi = mid-1;
	}
	return n-H.sz[H.adj[a][lo]];
}

void recalc(int a, mi b) {
	mi dumb = b*n;
	tmp[a] += dumb;
	//ps("AH",a,b,H.sz[a]);
	trav(t,H.adj[a]) {
		tmp[t] += b*(n-H.sz[t])-dumb;
	}
	if (H.par[a]) {
		tmp[1] += b*H.sz[a]; tmp[a] -= b*H.sz[a];
	}
	//FOR(i,1,n+1) pr(tmp[i],' ');
	//ps();
}
...
int main() {
	...
	F0R(i,q) {
		...
		if (t == 1) {
			...
			int v; re(v);
			if (sz(m) > 500) {
				trav(z,m) recalc(z.f,z.s);
				prop();
				m.clear();
			}
			mi ret = ans[v];
			trav(t,m) ret += t.s*getMul(t.f,v);
			ps(ret/n);
		}
	}
