...
inline void work() {
    ...
    rep(i, 1, n) p[i] = i, fa[i] = i;
    std::sort(p + 1, p + n + 1, [](int x, int y){return a[x] < a[y];});
    int cnt = 0; LL ans = 0; rep(i, 1, n) {
        const int x = p[i]; if(a[x] >= m || cnt == n - 1) break;
        // debug("x = %d, a[x] = %d\n", x, a[x]);
        per(j, x - 1, 1) {
            if(a[j] % a[x]) break;
            if(find(j) == find(x)) break;
            ++ cnt; ans += a[x]; fa[find(j)] = find(x);
			// debug("j = %d ok!\n", j);
        }
        rep(j, x + 1, n) {
            // debug("j = %d, g = %d, a[j] = %d\n", j , g, a[j]);
		...
		// rep(j, 1, n) debug("fa[j] = %d\n", find(j));
		// debug("\n");
    }

    if(cnt < n - 1) ans += 1ll * m * (n - 1 - cnt);
    print(ans);
}
signed main(){
    int T = read();
