...
void main_() {
	...
	REP(i, SZ(kugiri) - 1) {
		...
		if(len <= 0)
			continue;
		int in_MAX = *max_element(a.begin() + lef, a.begin() + rig);
		bool in_max = 1;
		if(lef - 1 >= 0 and a[lef - 1] > in_MAX) in_max = 0;
		if(rig < n and a[rig] > in_MAX) in_max = 0;
		if(len < k) {
			if(in_max) {
				print(-1);
				return;
