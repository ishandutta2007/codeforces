...
int main(){
  ...
  using vi=vector<ll>;
  auto f1=[](vi x,vi y){
    return vi{x[0]+y[0],x[1]+y[1],x[2]+y[2]};
  };
  auto f2=[&](vi x,ll w){
    return vi{1,x[0]>=2||x[1]>=1,x[1]>=1||x[0]>=3};
  };
  ReRooting<vi,ll>reroot(n,f1,f2,vi(3,0));
  vector<ll>d(n);
  rep(i,0,n-1){
    ll a,b;cin>>a>>b;a--;b--;
  ...
  auto v=reroot.build(0);
  /*rep(i,0,n){
    debug(v[i],3);
  }*/
  rep(j,0,n)if(v[j][2]>=3)judge=false;
  ans1(judge);
  return 0;
}