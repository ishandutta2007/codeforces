...
const int N = (1ll << 19) + 2;
Node seg[N];

ll A[N];
void build(int node,int s,int e,int d){
	...
	int mid = (s + e) >> 1;
	build(node << 1, s, mid, d - 1);
	build((node << 1) | 1, mid + 1, e, d - 1);

	seg[node].sum = seg[node << 1].sum + seg[(node << 1) | 1].sum;
	seg[node].l = node << 1;
	seg[node].r = seg[node].l | 1;
}
...
void rmv_laz(int node,int s,int e){
	...
	if (seg[node].lazy & (1ll << seg[node].level)){
...
void upd(int node,int s,int e,int l,int v){
	rmv_laz(node, s, e);
	...
	int mid = (s + e) >> 1;
	upd(seg[node].l, s, mid, l, v);
	upd(seg[node].r, mid + 1, e, l, v);
	seg[node].sum = seg[node << 1].sum + seg[(node << 1) | 1].sum;
...
ll query(int node,int s,int e,int l,int r){
	...
	int mid = (s + e) >> 1;
	return query(seg[node].l, s, mid, l, r) + query(seg[node].r, mid + 1, e, l, r);
}
...
void solve(){
	...
	for(int i = 0; i < q; ++i){
		...
		if (type == 1){
			...
			int l, r;
			cin >> l >> r;
			cout << query(1, 0, (1ll << n) - 1, l - 1, r- 1) << endl;
		}
	}
}
int main(){
