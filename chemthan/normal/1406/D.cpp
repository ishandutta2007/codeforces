...
void chemthan() {
    ...
    FOR(i, 0, n - 1) c[i] = -min(0, a[i] - a[i + 1]);
    long long sum = accumulate(all(c), 0LL);
    seg.init(n);
    FOR(i, 0, n) {
        seg.upd(i, i, a[i] + sum);
        sum -= c[i];
    }
    long long x = seg.st[1] / 2 - 5;
    while (x + x < seg.st[1]) x++;
    cout << x << "\n";
    int q; cin >> q;
    while (q--) {
        ...
        auto upd = [&] (int u) {
            long long cc = c[u];
            c[u] = -min(0LL, query(u) - query(u + 1) + a[u] - a[u + 1]);
            long long dif = c[u] - cc;
            seg.upd(0, u, dif);
        };
        if (l) upd(l - 1);
        if (r + 1 < n) upd(r);
