...
int main(){
    ...
    rep(zz,T){
        ...
        auto dfs = [&](auto &&f,int id,int pre,int tmp)->void{
            int k = 0;
            dbg(id,tmp);
            if(id!=0){
                hld.path_query(id,id,[&](int l, int r){k += seg.query(l,r);});
                if(k==0){
                    tmp++;
                    hld.subtree_query(id, [&](int l, int r){ seg.range(l, r, id); });
                }else{
                    hld.subtree_query(k, [&](int l, int r){ seg.range(l, r, -k); });
                    hld.subtree_query(id, [&](int l, int r){ seg.range(l, r, id); });
                }
            }
            chmax(mx,tmp);
