...
//        return;
        ...
        if (odd > 1) {
            ...
            int f = A[0] % 2;
            for (int q = 0; q < g; ++q) {
                for (int i = 0; i < A[f]/g; ++i) cout << char('a'+f);
                for (int j = 0; j < N; ++j) {
                    if (j != f && A[j]%2==0) for (int i = 0; i < A[j]/g/2; ++i) cout << char('a'+j);
                }
                for (int j = 0; j < N; ++j) {
                    if (A[j]%2==1) for (int i = 0; i < A[j]/g; ++i) cout << char('a'+j);
                }
                for (int j = N-1; j >= 0; --j) {
                    if (j != f && A[j]%2==0) for (int i = 0; i < A[j]/g/2; ++i) cout << char('a'+j);
                }
            ...
            cout << g << '\n';
            for (int q = 0; q < g/2; ++q) {
                for (int i = 0; i < 2*A[0]/g; ++i) cout << 'a';
                for (int j = 1; j < N-1; ++j) {
                    for (int i = 0; i < A[j]/g; ++i) cout << char('a'+j);
                }
                for (int i = 0; i < 2*A[N-1]/g; ++i) cout << char('a'+N-1);
                for (int j = N-2; j > 0; --j) {
                    for (int i = 0; i < A[j]/g; ++i) cout << char('a'+j);
                }
            }
