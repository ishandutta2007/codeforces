...
int main() {
  ...
  for (int i = 1; i <= n; ++i) {
    f[i] = f[i - 1];
    f[i].resize(size(f[i - 1]) + 2 * (i - 1));
    for (int _ = 2; _--;) {
      for (int j = size(f[i]) - i; j--;) f[i][j + i] -= f[i][j];
      for (int j = 1; j < int(size(f[i])); ++j) f[i][j] += f[i][j - 1];
    }
  }
  for (int i = 1; i <= n; ++i) {
    f[i].resize(i * (i - 1) / 2 + 1);
    partial_sum(begin(f[i]), end(f[i]), begin(f[i]));
  }
  ...
  for (int x = 0; x < n - 2; ++x) {
    ...
    for (int d = 1; d < y; ++d) {
      Mint t;
      if (d + 1 < int(size(f[y - 1]))) t += rbegin(f[y - 1])[d + 1];
      t *= y - d;
      cur += t;
    }
