...
const int L = 7;
const vector<string> SUB = {"h", "a", "ha", "ah", "hah", "aha", "haha"};
...
Data comb(Data x, Data y) {
	Data z;
	z.cnt = x.cnt + y.cnt;
	string a = x.cur;
	string b = y.cur;
	z.cur = get(x.cur + y.cur);
	if (sz(a) >= 3 && sz(b) >= 1) {
		if (a[sz(a) - 3] == 'h' && a[sz(a) - 2] == 'a' && a[sz(a) - 1] == 'h' && b[0] == 'a') {
			++z.cnt;
		}
	} 
	if (sz(a) >= 2 && sz(b) >= 2) {
		if (a[sz(a) - 2] == 'h' && a[sz(a) - 1] == 'a' && b[0] == 'h' && b[1] == 'a') {
			++z.cnt;
		}
	} 
	if (sz(a) >= 1 && sz(b) >= 3) {
		if (a.back() == 'h' && b[0] == 'a' && b[1] == 'h' && b[2] == 'a') {
			++z.cnt;
		}
	}
	return z;
