...
fn solve(input: &mut Input, _test_case: usize) {
    let _n: i32 = input.read();
    let s : String = input.read();
    ...
    let bs = count_on_the_left(&s);
    let bt = count_on_the_left(&t);
    let ans = bs.iter().zip(bt.iter()).zip(nb.as_bytes().iter())
        .all(|((left_s, left_t), letter)| match *letter {
            b'a' => left_t >= left_s,
            b'c' => left_t <= left_s,
            _ => true
        });
...
fn count_on_the_left(s: &str) -> Vec<usize> {
    let mut res = Vec::new();
    let mut count = 0;
