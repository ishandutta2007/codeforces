...
public class E2 {
	...
	{
		// 0=C, 1=P
//		assert h.matches("1?0+(10)*1+0?") || h.matches("1*0*");
		final int mod = 998244353;
		int n = ni();
		int[] a = na(n);
//		long[] pc = new long[n+1];
//		for (int i = 0; i < n; i++) {
//			pc[i+1] = pc[i] + (i % 2 == 0 ? -a[i] : a[i]);
//		}
		// 10+ 1+0
		...
		for(int i = n-2;i >= 0;i--){
			s += a[i+1];
			if(i < n-2)s0 += a[i+1];
			if(i < n-2)offset[i%2] += a[i+1] - a[i+2];
			if(i < n-2)root[i%2] = insertb(root[i%2], new Node(s0-offset[i%2]));
			root[i%2] = insertb(root[i%2], new Node(s-offset[i%2]));
			if(i > 0)ans += count(root[i%2]) - lowerBound(root[i%2], -cum1[i]+1-offset[i%2]);
			ans += count(root[i%2]) - lowerBound(root[i%2], -cum[i]+1-offset[i%2]);
		}
