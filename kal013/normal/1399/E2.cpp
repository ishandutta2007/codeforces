...
long long U[N], V[N], W[N], C[N], sub[N], cnt[N];

vector<int> adj[N];


#define child(x,e) (U[e] ^ V[e] ^ x)
...
void solve(){
	...
	for(int i = 1; i < n; ++i){
		if (C[i] == 1){
			while(W[i] > 0){
				long long dif = W[i] - (W[i] >> 1);
				p1.push_back(dif * cnt[i]);
				W[i] >>= 1;
			}
		}
		else{
			while(W[i] > 0){
				long long dif = W[i] - (W[i] >> 1);
				p2.push_back(dif * cnt[i]);
				W[i] >>= 1;
			}
		}
	}	
	sort(all(p1));  reverse(all(p1));
	sort(all(p2)); reverse(all(p2));

	int moves = p1.size() + 2 * p2.size();
	long long tsum = 0;

	for(int k = 0; k < p2.size(); ++k){
		tsum += p2[k];
		if (tsum >= cost){
			moves = min(moves, 2*(k + 1));
		}
	}
	int j = p2.size() - 1;
	if (tsum >= cost){
		moves = min(moves, 2*(int)p2.size());
	}
	for(int i = 0; i < p1.size(); ++i){
		tsum += p1[i];
		while(j >= 0 && tsum - p2[j] >= cost){
			tsum -= p2[j];
			--j;
		}

		if (tsum >= cost){
			moves = min(moves, i + 1 + 2* (j + 1));
		}
	}
	cout << moves<< endl;
}
