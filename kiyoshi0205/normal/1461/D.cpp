...
set<ll> se;
void solve(vec& v,ll l,ll r){
  if(l==r)return;
  se.emplace(accumulate(v.begin()+l,v.begin()+r,0LL));
  if(v[l]==v[r-1])return;
  ll mid=(v[l]+v[r-1])/2;
  ll midid=upper_bound(v.begin()+l,v.begin()+r,mid)-v.begin();
  assert(l!=midid&&r!=midid);
  solve(v,l,midid);
  solve(v,midid,r);
}
int main(){
