...
int main() {
	...
	drop(double(m) * (1-res) / res);


	// double ok = 1e9 + 7;
	// double ng = 0;

	// auto takeoff = [&](double w, int idx) { return w / a[idx]; };
	// auto land = [&](double w, int idx) { return w / b[idx]; };

	// auto judge = [&](double fuel) {
	// 	fuel -= takeoff(fuel + m, 0);
	// 	if(fuel < 0) return false;
	// 	rep(i, 1, n) {
	// 		fuel -= land(fuel + m, i);
	// 		if(fuel < 0) return false;
	// 		fuel -= takeoff(fuel + m, i);
	// 		if(fuel < 0) return false;
	// 	}
	// 	fuel -= land(fuel + m, 0);
	// 	if(fuel < 0) return false;
