...
#line 1 "/Users/nok0/Documents/Programming/nok0/atcoder/segtree.hpp"



#line 7 "/Users/nok0/Documents/Programming/nok0/atcoder/segtree.hpp"
...
#line 9 "/Users/nok0/Documents/Programming/nok0/atcoder/segtree.hpp"
...
void main_() {
	...
	REP(i, m) {
		...
		if(uf.same(a, b)) {
			...
			int x = uf.size(a);
			int y = uf.size(b);
			auto [xa, xb] = seg.get(x);
			seg.set(x, S(xa - x, xb - 1));
			auto [ya, yb] = seg.get(y);
			seg.set(y, S(ya - y, yb - 1));
			auto [za, zb] = seg.get(x + y);
			seg.set(x + y, S(za + x + y, zb + 1));
		}
		uf.merge(a, b);
		auto f = [&](int x) {
			return seg.prod(x, n + 1).second <= k;
		};
		REP(i, n + 1) {
			debug(seg.get(i));
		}
		auto l = bin_search(n, -1, f);
		auto [val, cnt] = seg.prod(l, n + 1);
		if(l != 0) val += (l - 1) * (k - cnt);
		print(val - 1);
	}
}