...
fun main() {
    ...
    val st = TreeSet<Int>()
    val ast = mutableSetOf<Int>()

    val startSearchOn = mutableMapOf<Int,Int>()
...
//    var atleast = 0
    first@while(true){
        ...
        val ori = st.last()
//        val tomove = mutableListOf<Int>(ori)
        val todelete = ori
        var next = startSearchOn.getOrDefault(ori,ori)
        while(true){
            val a = next
            val new = a /2
            if(new == 0){
...
//                tomove.add(new)
                ...
                st.add(next)
                ast.add(next)
                startSearchOn[todelete/2] = next
//                println("moved ${tomove.joinToString(" ")}")
//                println(st.toList().joinToString(" "))
                break
            }
        }
//
