...
int main() {
	...
	re(n);
	c.rsz(n); re(c);
	b.rsz(n-1); re(b); p.rsz(n,0);
	re(q);
	x.rsz(q); re(x);
	FOR(i,1,n) p[i] = p[i-1]+b[i-1];
	FOR(i,1,n) p[i] = p[i-1]+p[i];
	int l = -p[n-1]/n-5,r = l+110;
	each(ct,x){
		if(ct<l)ct = l;
		if(ct<l || ct>r){
			ps(0);
		...
		F0R(i,n){
			...
			F0R(j,10001)dp[i+1][j+1]+=dp[i+1][j];
			F0R(j,10001){
				if(fdiv(j-p[i],i+1)<ct)dp[i+1][j] = 0;
			}
		}
		mi ans = 0;
		F0R(j,10001)ans+=dp[n][j];
		Mp[ct] = ans.v;
		ps(Mp[ct]);
	}
// END OF CODE
