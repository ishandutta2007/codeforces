...
int main() {
  ...
  for(int i = 0; i < k; i++) {
    int a, b, c;
    read(a, b, c);
    queries[i] = ppiii(pii(a, b), c);
  }
  sort(queries, queries + k);
  dp[0] = n+1;
  for(int a = 0; a < k; a++) {
    for(int i = n; i >= queries[a].second; i--) {
      if(dp[i-queries[a].second] >= queries[a].first.first) {
        dp[i] = max(dp[i], min(dp[i-queries[a].second], queries[a].first.second));
      }
    }
  }
  int ans = 0;
