...
fun main(){
    solve.cases{
        ...
        val start = IntArray(n+1){n - it}
        for(i in 2 until n+1 step 2){
            start[i] += 1
        }

        val st = intMaxLazy(start)
        st.rangeAdd(n+1,st.n-1,nlarge)
        ...
        val inversePermutation = IntArray(n)
        for((i,v) in L.withIndex()){
            inversePermutation[v] = i
        }
//        st.lazyPrint()
        var ret = 0
        for(v in inversePermutation.reversed()){
            st.rangeAdd(v+1,n,1)
            st.rangeAdd(0,v,-1)
//            st.lazyPrint()
            zeros.remove(v)
            ones.add(v)
            if(ones.isEmpty() || zeros.isEmpty()) continue
            val atleast = ones.first() + 1
            val atmost = zeros.last()
            val guess = st.rangeQueryLazy(atleast, atmost)
            if(guess == Int.MIN_VALUE) continue
            ret = maxOf(ret, guess - 1)
        }
        put(ret)
    }
    done()
