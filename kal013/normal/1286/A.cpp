...
void solve(){
	...
	for(int i=1;i<pos.size();++i){
		if (A[pos[i-1]]==A[pos[i]]){
			...
			if (A[pos[i]]) od.pb(pos[i]-pos[i-1]-1);
			else ev.pb(pos[i]-pos[i-1]-1);
		}
		else ++ans;
	}
	sort(all(od));
	...
	if (pos[0]!=1){
		if (A[pos[0]]){
			if (remo>=(pos[0]-1)) remo-=(pos[0]-1);
			else ++ans;
		}
		else{
			if (reme>=(pos[0]-1)) reme-=(pos[0]-1);
			else ++ans;
		}
	}
	if (pos.back()!=n){
		int res=n-pos.back();
		if (A[pos.back()]){
			if (remo>=res) remo-=res;
			else ++ans;
		}
		else{
			if (reme>=res) reme-=res;
			else ++ans;
		}
	}
	cout<<ans<<endl;
