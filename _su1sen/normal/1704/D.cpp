...
int main() {
    ...
    loop(t) {
        ...
        read(c);
        vector<vector<long long>> s(n, vector<long long>(m + 1));
        vector<i128> ssum(n);
        map<i128, vector<int>> st;
        rep(i, n) {
            rep(j, m) {
                s[i][j + 1] = s[i][j] + c[i][j];
                ssum[i] += s[i][j + 1];
            }
            st[ssum[i]].push_back(i);
        }
        assert(st.size() == 2);
        auto [s1, v1] = *st.begin();
        auto [s2, v2] = *st.rbegin();

        if (v1.size() == 1) {
            print(v1.front() + 1, s2 - s1);
        } else {
            print(v2.front() + 1, s1 - s2);
        }
    }
    return 0;
}