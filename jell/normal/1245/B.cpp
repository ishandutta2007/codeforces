...
void __solve()
{
    i32 a,b,c,n; cin>>n>>a>>b>>c;
    string s; cin>>s;
    static i32 dp[110][110][110];
    static char h[110][111][111];
    init(dp,-1);
    dp[0][0][0]=0;
    for(i32 i=0; i<=a; ++i)
    {
        for(i32 j=0; j<=b; ++j)
        {
            for(i32 k=0; k<=c; ++k)
            {
                ...
                if(s[t]=='R')
                {
                    if(j<b) if(sbmax(dp[i][j+1][k],1+dp[i][j][k])) h[i][j+1][k]='P';
                    if(k<c) if(sbmax(dp[i][j][k+1],dp[i][j][k])) h[i][j][k+1]='S';
                    if(i<a) if(sbmax(dp[i+1][j][k],dp[i][j][k])) h[i+1][j][k]='R';
                }
                else if(s[t]=='P')
                {
                    if(k<c) if(sbmax(dp[i][j][k+1],1+dp[i][j][k])) h[i][j][k+1]='S';
                    if(j<b) if(sbmax(dp[i][j+1][k],dp[i][j][k])) h[i][j+1][k]='P';
                    if(i<a) if(sbmax(dp[i+1][j][k],dp[i][j][k])) h[i+1][j][k]='R';
                }
                else
                {
                    if(i<a) if(sbmax(dp[i+1][j][k],1+dp[i][j][k])) h[i+1][j][k]='R';
                    if(j<b) if(sbmax(dp[i][j+1][k],dp[i][j][k])) h[i][j+1][k]='P';
                    if(k<c) if(sbmax(dp[i][j][k+1],dp[i][j][k])) h[i][j][k+1]='S';
                }
            }
