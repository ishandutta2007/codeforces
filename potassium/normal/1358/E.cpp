...
int main() {
	...
	vector<ll> suffix(ceil);
	for (int i = 0; i < ceil; i++)
		suffix[i] = a[i] - x;
		
	vector<ll> suffixMin(ceil, 0);
	chkmin(suffixMin[0], suffix[0]);
	
	for (int i = 1; i < ceil; i++)
		chkmin(suffixMin[i], suffixMin[i - 1] + suffix[i]);
	
	ll suffixSum = x * floor;
	for (int i = ceil - 1; i >= 1; i--) {
		suffixSum += a[i];
		if (suffixSum > 0 && suffixSum + suffixMin[i - 1] >= 0) {
			W(n - i);
			return 0;
		}
	}
