    ...
    rep(i,m){
        ...
        if(uf.same(a,b)){
            while(!pq.empty()){
                ...
                pq.pop();
                x = uf.find(x);
                if(uf.size(x)!=c1)continue;
                if(st.count(MP(c1,x))==1)continue;
                res += c1;
                st.insert(MP(c1,x));
                break;
            }
            ...
            a = uf.find(a);
            b = uf.find(b);
            if(st.count(MP(uf.size(a),a))==1){
                if(st.count(MP(uf.size(b),b))==1){
                    st.erase(MP(uf.size(a),a));
                    st.erase(MP(uf.size(b),b));
                    res -= uf.size(a) + uf.size(b);
                    uf.unite(a,b);
                    res += uf.size(a);
                    st.insert(MP(uf.size(a),uf.find(a)));
                    while(!pq.empty()){
                        auto [c1,x] = pq.top();
                        pq.pop();
                        x = uf.find(x);
            ...
            }else if(st.count(MP(uf.size(b),b))==1){
                st.erase(MP(uf.size(b),b));
                res -= uf.size(b);
                uf.unite(a,b);
                st.insert(MP(uf.size(a),uf.find(a)));
