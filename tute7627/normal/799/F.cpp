...
int main(){
  ...
  vector<set<P>>st(2);
  ll lastr=-1;
  LazySegmentTree seg(m,ff,gg,hh,iden,0LL);
  rep(i,0,m){
    A tmp;
    tmp.sum[i&1]=i;
    tmp.num[i&1]=1;
  ...
  rep(i,0,m){
    for(auto z:gl[i]){
      st[i&1].emplace(l[z],z);
    }
    ll ql=0,qr=i+1;
    auto add=[&](A x,ll p){
      //cout<<x.num[p] spa x.sum[p] spa p<<endl;
      res+=(i+1)*x.num[p]-x.sum[p];
    };
    ll qml=qr;
    if(!st[0].empty())chmin(qml,st[0].begin()->fi);
    if(!st[1].empty())chmin(qml,st[1].begin()->fi);
    //cout<<i spa st[0].size() spa st[1].size() spa res<<endl;
    if(st[0].empty()&&st[1].empty()){
      auto tmp=seg.query(0,lastr+1);
      add(tmp,0);
      add(tmp,1);
    }
    else if(i&1){
      if(!st[0].empty())ql=(st[0].rbegin()->fi)+1;
      qml=max(ql,qml);
      //cout<<qml spa ql spa qr<<endl;
      auto tmp=seg.query(qml,qr);add(tmp,1);
      tmp=seg.query(ql,qml);add(tmp,0);add(tmp,1);
    }
    else{
      if(!st[1].empty())ql=(st[1].rbegin()->fi)+1;
      qml=max(ql,qml);
      auto tmp=seg.query(qml,qr);add(tmp,0);
      tmp=seg.query(ql,qml);add(tmp,1);add(tmp,0);
    }
    for(auto z:gr[i]){
      st[l[z]&1].erase(MP(l[z],z));
      if((r[z]-l[z])&1){
        seg.update(0,l[z],3);
      }
      if(r[z]&1)seg.update(l[z],r[z],1);
      else seg.update(l[z],r[z],2);
      lastr=i;
    }
  }
  cout<<res<<endl;
