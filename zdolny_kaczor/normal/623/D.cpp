...
int p[nax], a[nax];
ld q[nax];
pair <ld, int> gain(int i) {
	debug(imie(pow(q[i], a[i])), imie(pow(q[i], a[i] + 1)));
	pair <ld, int> ret = make_pair((1 - pow(q[i], a[i] + 1)) / (1 - pow(q[i], a[i])), i);
	debug(arr(a, i), arr(q, i), imie(ret));
	return ret;
}
int main() {
	int n;
	...
	for (int i = 0; i < n; ++i) q[i] = (100 - p[i]) / (ld) 100;
	for (int i = 0; i < n; ++i) a[i] = 1;
	ld ans = n;
	priority_queue<pair <ld, int> > get;
	for (int i = 0; i < n; ++i) get.push(gain(i));
	ld pra = 1;
	for (int i = 0; i < n; ++i) pra *= p[i] / (ld) 100;
	ans += 1 - pra;
	for (int i = 0; pra < 1 - 1e-9; ++i) {
		auto c = get.top();
		get.pop();
		debug(imie(c));
		pra *= c.first;
		a[c.second]++;
		get.push(gain(c.second));
		ans += 1 - pra;
		debug(range(a, a + n), 1 - imie(pra));
	}
	debug(imie(pra));
	printf("%.10Lf\n", ans);
}