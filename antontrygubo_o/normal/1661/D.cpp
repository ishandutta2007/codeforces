...
int main()
{
    ...
    vector<ll> adder(n+1), cover(n+1), st(n+1);

    ll ops = 0;

    for (int i = n-1; i>=0; i--)
    {
        adder[i] = adder[i+1] - cover[i+1];
        cover[i] = cover[i+1] - st[i+1];
        ll need = max(0ll, b[i]-adder[i]);

        ll opt = min(k, i+1);
        ll cnt = (need+opt-1)/opt;

        /*cout<<i<<": "<<adder[i]<<' '<<cover[i]<<' '<<need<<endl;
