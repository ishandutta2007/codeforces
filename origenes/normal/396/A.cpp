...
int main() {
  ...
  modular_arithmetic::get_fac(500 * 35);
  scanf("%d", &n);
  FOR(i, 1, n) {
    int a;
    scanf("%d", &a);
    FOR(j, 2, sqrt(1e9) + 5) if (a % j == 0)
      while (a % j == 0) a /= j, cnt[j]++;
    if (a > 1) cnt[a]++;
  }
  int ans = 1;
  for (auto it : cnt) ans = modular_arithmetic::mul(ans, modular_arithmetic::C(it._2 + n - 1, n - 1));
  printf("%d", ans);
}