...
int main(int argc, char** argv) {
    ...
    for (int ii = 0; ii < n; ii++) {
        ...
        auto& cd = d[ii];
        if (isz(ca) == 0) {
            fill(begin(cd), end(cd), 0);
            continue;
        }
        for (int i = 0; i < isz(ca); i++) {
            for (int j = i; j < isz(ca); j++) {
                int m = i + isz(ca) - j - 1;
                cd[m] = min(cd[m], ca[j] - ca[i] + 1);
            }
        }
        cd[isz(ca)] = 0;
        for (int i = 1; i <= m; i++) {
            cd[i] = min(cd[i], cd[i - 1]);
        }
    }   

    vector<vector<int>> aa(n + 1, vector<int>(k + 1, 1e9));
    aa[0][0] = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j <= k; j++) {
            for (int z = 0; z <= min(j, m); z++) {
                aa[i + 1][j] = min(aa[i + 1][j], aa[i][j - z] + d[i][z]);
            }
        }
    }
