...
int main() {
    ...
    dedup(vel);
    for (auto &[x, v] : points) {
        v = lower_bound(all(vel), v) - vel.begin();
    }
    ...
    {
        binary_indexed_tree sum(sz(vel), 0LL, plus<ll>(), minus<ll>());
        binary_indexed_tree count(sz(vel), 0LL, plus<ll>(), minus<ll>());

        for (auto [x, v] : points) {
            pd(x, v);
            ans += x * count.accumulate(0, v + 1) - sum.accumulate(0, v + 1);
            sum.add(v, x);
            count.add(v, 1);
        }
    }
