...
int n = in;
long long w = in, v = in, u = in;
long double tt = 1.0 * w / u;
vector < pair<long long, long long> > p;

bool okBase(long long sign)
{
    long long x1 = 0, y1 = 0;
    long long x2 = v * w, y2 = u * w;
    long long A = y2 - y1, B = x1 - x2, C = -(A * x1 + B * y1);
/*
    out.putLn(A, ' ', B, ' ', C);
*/
    for (int i = 0; i < n; i++)
        if ((A * p[i].first + B * p[i].second + C) * sign > 0) return false;
    return true;
}

bool okBase()
{
    return okBase(-1) || okBase(1);
}

bool ok(long double t, long double sign)
{
    long double x1 = 1.0 * v * (t - tt), y1 = 0;
    long double x2 = 1.0 * v * t, y2 = w;
    long double A = y2 - y1, B = x1 - x2, C = -(A * x1 + B * y1);
/*
    out.putLn("!!!", t);
    out.putLn("!!", x1, ' ', y1);
    out.putLn("!!", x2, ' ', y2);
...
{
    in.getv(p, n);
    if (okBase()) out = ofl(tt, 10);
        else out = ofl(binSearch(tt, 1e20), 10);
    return 0;
}