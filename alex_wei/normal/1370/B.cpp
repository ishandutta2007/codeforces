...
void solve(){
	cin>>n,od.clear(),ev.clear();
	for(int i=1;i<=n*2;i++)cin>>a[i],a[i]&1?od.pb(i):ev.pb(i);
	int cnt=0;
	for(int i=0;i<(int)ev.size()-1;i+=2)
		if(cnt<n-1)
			cout<<ev[i]<<" "<<ev[i+1]<<endl,cnt++;
	for(int i=0;i<(int)od.size()-1;i+=2)
		if(cnt<n-1)
			cout<<od[i]<<" "<<od[i+1]<<endl,cnt++;
