...
void solve() {
  ...
  auto add = [&](int i) -> void { cost += CNT[A[i]]++; };
  auto rm = [&](int i) -> void { cost -= --CNT[A[i]]; };
  auto f = [&](int l, int r) -> ll {
    while (left > l) add(--left);
    while (right < r) add(right++);
    while (left < l) rm(left++);
    while (right > r) rm(--right);
    return cost;
  };

  const ll INF = 1LL << 60;
  auto DP = monge_shortest_path_d_edge_dense<ll>(N, f, INF, K + 1);
  print(DP[K][N]);
}
