...
void init(){
    ...
    for(int i = 1; i <= N; i++) prime_facs[i].push_back(i);
}
...
void solve(){
    ...
    int m = prime_facs[n].size();
    vector<ll> sum;
    vector<vector<int>> idx(m);
    map<ll, int> cnt;
    int cur_idx = 0;
    for(int i = 0; i < m; i++){
        int p = prime_facs[n][i];
        int x = n/p;
        vector<ll> v(x);
        for(int j = 0; j < n; j++) v[j%x] += a[j]*x;
        for(int r = 0; r < x; r++){
            sum.push_back(v[r]);
            idx[i].push_back(cur_idx);
            cur_idx++;
        }
    }
    Seg seg(sum);
