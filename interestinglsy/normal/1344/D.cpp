...
ll CountOne( ll a , ll lim ){
	ll l = 0, r = a, ans = 0;
	while( l <= r ){
		ll mid = l+r >> 1;
		if( a - 3*mid*mid + 3*mid - 1 >= lim ){
			ans = mid;
			l = mid+1;
		}else{
			r = mid-1;
...
ll Count( ll lim ){
	ll cnt = 0;
	For(i,n){
		ll tcnt = CountOne(a[i],lim);
		cnt += tcnt;
	}
	return cnt;
}
ll BinarySearch(){
	ll l = -4e18, r = 4e18, ans = LLONG_MAX;
	while( l <= r ){
		ll mid = l+r >> 1;
		if( Count(mid) <= k ){
			ans = mid;
...
void Solve(){
	...
	static vc<pli> avail;
	For(i,n){
		ansl[i] = CountOne(a[i],lim);
		if( ansl[i] != a[i] ){
			ll x = ansl[i]+1;
			avail.Pb({a[i]-3*x*x+3*x-1,i});
			if( x != a[i] ){
				++x;
				avail.Pb({a[i]-3*x*x+3*x-1,i});
			}
	...
	sort(All(avail),[]( const pli &a , const pli &b ){
		return a.F > b.F;
	});
	ll left = k - Count(lim);
	// if( k == 49985284073162 ) cout << avail.size() << " " << left << endl;
	assert( (ll)avail.size() >= left );
	For0(i,left)
		++ansl[avail[i].S];
}
