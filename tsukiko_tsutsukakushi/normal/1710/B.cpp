...
void solve() {
    ...
    otera::Compressor<int> cs(xs);
    int sz = si(cs);
    vc<int> sum(sz, 0);
    rep(i, n) {
        int le = cs.get(x[i] - p[i]);
        int mi = cs.get(x[i]);
        int ri = cs.get(x[i] + p[i]);
        sum[le] ++;
        sum[mi] -= 2;
        sum[ri] ++;
    }
    ...
    rep(i, sz) {
        ...
        int len = (i - 1 >= 0 ? cs[i] - cs[i - 1] : 0);
        a[i] = (i - 1 >= 0 ? a[i - 1] + sum[i - 1] * len : 0);
        debug(i, a[i]);
    }
    string ans(n, '1');
    atcoder::segtree<int, op, e> seg1(sz), seg2(sz), seg3(sz);
    rep(i, sz) {
        seg1.set(i, a[i] - m + cs[i]);
        seg2.set(i, a[i] - m - cs[i]);
        seg3.set(i, a[i] - m);
    }
    rep(i, n) {
        int le = cs.get(x[i] - p[i]);
        int ri = cs.get(x[i] + p[i]);
        int ma1 = seg1.prod(le, ri + 1), ma2 = seg2.prod(le, ri + 1);
        unless(ma1 <= p[i] + x[i] && ma2 <= p[i] - x[i]) ans[i] = '0';
        int ma3 = max(seg3.prod(0, le + 1), seg3.prod(ri, sz));
        if(ma3 > 0) ans[i] = '0';
    }
    out(ans);
}
