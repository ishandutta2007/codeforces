...
constexpr int N = 4e5 + 10, M = 2e7 + 10;

struct addedge {
  int sz, head[N], nxt[M], val[M];
  void init(int n) { sz = 0; REP(i, 1, n) head[i] = 0; }
  void ae(int x, int y) {
    ++sz, nxt[sz] = head[x], head[x] = sz, val[sz] = y;
  }
  #define foreach(e, x) \
    for(int i##e = nei.head[x], e = nei.val[i##e]; i##e; i##e = nei.nxt[i##e], e = nei.val[i##e])
} nei;

int n, m, k;
int TRUE, FALSE;
int id(int x, int v, int tf) {
  if(v <= 0) return tf ? FALSE : TRUE;
  if(v > k) return tf ? TRUE : FALSE;
  return 2 * ((x - 1) * k + v) - tf;
}
int rev(int x) { return x & 1 ? x + 1 : x - 1; }
void ae(int x, int y) {
  // cout << x << " " << y << "???\n";
  nei.ae(x, y), nei.ae(rev(y), rev(x));
}
...
void mian() {
  ...
  TRUE = 2 * n * k + 1, FALSE = TRUE + 1;
  nei.init(FALSE);
  ae(FALSE, TRUE);
  REP(i, 1, n) {
    ae(TRUE, id(i, k, 1));
    REP(x, 1, k - 1) ae(id(i, x, 1), id(i, x + 1, 1));
  }
  REP(i, 2, n) {
    REP(x, 1, k) ae(id(i, x, 1), id(i - 1, x, 1));
  }
  REP(_, 1, m) {
    int tp = read(), i = read(), j, v;
    if(tp == 1) {
      v = read();
      ae(id(i, v, 1), id(i, v - 1, 1));
    } else {
      j = read(), v = read();
      if(tp == 2) {
        REP(x, 1, k) ae(id(i, x - 1, 0), id(j, v - x, 1));
      } else {
        REP(x, 1, k) ae(id(i, x, 1), id(j, v - x - 1, 0));
      }
    }
  }
  cnt = nowdfn = 0;
  REP(i, 1, FALSE) dfn[i] = 0;
  REP(i, 1, FALSE) if(!dfn[i]) tar(i);
  REP(i, 1, FALSE) if(cid[i] == cid[rev(i)]) return puts("-1"), void();
  REP(i, 1, n) {
    REP(x, 1, k) if(cid[id(i, x, 1)] < cid[id(i, x, 0)]) {
      prt(x), pc(" \n"[i == n]);
      break;
    }
  }
