...
void un(ll x,ll y,ll rn[],ll sz[],double ans[])
{
    x=fn(x,rn);
    y=fn(y,rn);
    if(x==y) return;
    if(sz[x]<sz[y]) swap(x,y);
    ans[x]=(ans[x]*sz[x]+ans[y]*sz[y])/(sz[x]+sz[y]);
    sz[x]+=sz[y];
    rn[y]=x;
    return;
}
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n-1)
        {
            if(ans[fn(i,rn)]>ans[fn(i+1,rn)])
            {
                un(i,i+1,rn,sz,ans);
                ll l=i+1-sz[fn(i+1,rn)];
                while(l>=0)
                {
                    if(ans[fn(l,rn)]>ans[fn(i+1,rn)])
                    {
                        un(l,i+1,rn,sz,ans);
                        l=i+1-sz[fn(i+1,rn)];
                    }
        ...
        REP(i,0,n)
        {
            cout<<setprecision(12)<<ans[fn(i,rn)]<<'\n';
        }
