...
int main() {
	...
	cin >> s; s = ' ' + s;
	rep(i, 1, n) qread(a[i]);
	rep(i, 2, n) rep(j, 1, i - 1) To_max(a[i], a[j] + a[i - j]);
	rep(k, 1, n) rep(i, 1, n - k + 1) {
		int j = i + k - 1;
		rep(l, 0, n) {
			To_max(dp[i][j][l], dp[i + 1][j][0] + a[l + 1]);
			rep(r, i + 1, j) if(s[i] == s[r]) To_max(dp[i][j][l], dp[i + 1][r - 1][0] + dp[r][j][l + 1]);
		}
	}
	cout << dp[1][n][0] << endl;
 	return 0; 
