...
int main() {
  ...
  rep(t) {
    ll n = IN, m = IN;
    ll full = n * (n+1) / 2;
    if (n == m) { OUT(full); continue; }
    ll groups = min(n-m, m+1);
    ll low = (n-m) / groups, high = divceil(n-m, groups);
    ll high_n = (n-m) - low * groups, low_n = groups - high_n;
    debug(full);
    for (auto [sz, cnt] : (ll[][2]){{low, low_n}, {high, high_n}}) {
      debug(sz); debug(cnt);
      full -= sz * (sz+1) / 2 * cnt;
      debug(full);
    }
