...
const int N=5e5+5,M=15000005,inf=1e9;
...
void modify(int &x,int l,int r,int p,int u){
	...
	if(l==r) return;
	int mid=(l+r)>>1;
	if(p<=mid) modify(ls[x],l,mid,p,u);
	else modify(rs[x],mid+1,r,p,u);
}
i64 ask(int &x,int l,int r,int s,int t){
	if(!x || l==s && r==t) return su[x];
	int mid=(l+r)>>1;
...
int solve(){
	...
	i64 sum=0,lst=0;
	while(sum<su[root]){
		auto it=B.upper_bound(lst);
		if(it==B.end())return res;
		i64 v=max(2*sum,*it);
		if(*it>2*sum)--res;
		sum=ask(root,1,inf,1,v);
		lst=v;
	}
	return res;
}
int main(){
	for(cin>>q;q;--q){
		scanf("%s",op);
		scanf("%d",&x);
		if(*op=='+') B.insert(x),modify(root,1,inf,x,x);
		else B.erase(B.find(x)),modify(root,1,inf,x,-x);
		printf("%d\n",solve());
	}
}