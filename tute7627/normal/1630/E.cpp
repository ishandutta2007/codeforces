...
int main(){
  ...
  while(t--){
    ...
    vector<ll>v;
    for(auto z:mp)v.PB(z.se);
    ll ag=0;
    for(auto z:v)ag=gcd(ag,z);
    if(v.size()==1){
      cout<<1<<endl;
      continue;
    }
    ...
    rep(i,1,n+1)num[gcd(i,n)]+=1;
    rep(g,1,n+1){
      if(ag%g!=0)continue;
      modint prod=comb.fact(n/g);
      for(auto z:v){
        prod*=comb.rfact(z/g);
      }
      //cout<<prod<<endl;
      all+=prod*num[n/g];
      modint tprod=prod*comb.rfact(n/g)*comb.fact(n/g-2);
      for(auto z:v){
        if(z==g)continue;
        ret+=n*tprod*num[n/g]*comb.fact(z/g)*comb.rfact(z/g-2);
        //cout<<g spa z spa ret<<endl;
      }
    }
    //debug(num);
    //debug(v);
    ret/=n,all/=n;
    //cout<<ret spa all<<endl;
    cout<<(all*n-ret)/all<<endl;
  }
  return 0;
}