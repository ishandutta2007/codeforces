#line 1 "codeforces-workspace\\contest\\1555\\b\\b.cpp"
...
#line 16 "codeforces-workspace\\contest\\1555\\b\\b.cpp"
//*/
...
void main() {
  ...
  input w, h;
  u64 ans = -1;
  if (h + x2 - x1 <= H) chle(ans, min(max(0, h - x1), max(0, x2 + h - H)));
  if (w + y2 - y1 <= W) chle(ans, min(max(0, w - y1), max(0, y2 + w - W)));
  cout << (i64)ans << "\n";
}
