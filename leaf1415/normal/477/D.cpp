...
void bfs()
{
	ll id = 1;
	queue<ll> Q;
	ll r = trie.vec[0].next[1];
	Q.push(r), trie.vec[r].num = id++;
...
int main(void)
{
	...
	rep(i, 1, n) trie.insert(s.substr(i));
	bfs();
	...
	rep(i, 1, n){
		...
		ll v = 0;
		rep(j, i, n){
			v = trie.vec[v].next[s[j]-'0'];
			val[i][j] = trie.vec[v].num;
		}
	}
	...
	rep(i, 2, n){
		...
		rep(j, i, n){
			while(1){
				if(l == 0 || val[l][i-1] > val[i][j]) break;
				sum += dp[l][i-1], sum %= mod;
				l--;
			}
			dp[i][j] = sum;
		}
	...
	ll ans2 = inf;
	rep(i, max(1LL, n-20), n){
		if(dp[i][n] > inf/2) continue;
		
		ll tmp = 0;
		rep(j, i, n) tmp *= 2, tmp += s[j]-'0';
		chmin(ans2, dp[i][n] + tmp);
	}
	if(ans2 < inf/2) outl(ans2);
	else{
		rep(i, 21, n){
			if(dp[n-i][n] < inf/2){
				mint tmp = 0;
				rep(j, n-i, n) tmp *= 2, tmp += s[j]-'0';
				outl(tmp + dp[n-i][n]);
				break;
			}
		}
