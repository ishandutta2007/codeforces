...
void solve() {
    ...
    REP(i, n) {
        if (l[i] != '?') {
            inputs[l[i] - 'a'].push_back(i + 1);
        }
        else {
            inputs[26].push_back(i + 1);
        }
    }
    vector<pair<int, int>> ans;
    vector<int> wilds;
    REP(i, n) {
        if (r[i] != '?') {
            if (inputs[r[i] - 'a'].size()) {
                ans.push_back(mp(inputs[r[i] - 'a'].back(),i + 1));
                inputs[r[i] - 'a'].pop_back();
                continue;
            }
            if (inputs[26].size()) {
                ans.push_back(mp(inputs[26].back(), i + 1));
                inputs[26].pop_back();
                continue;
            }
        }
    ...
    for (auto x : wilds) {
        REP(q, 27) {
            if (inputs[q].size()) {
                ans.push_back(mp(inputs[q].back(), x));
                inputs[q].pop_back();
                break;
            }
        }
