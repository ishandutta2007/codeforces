...
int main(){
    ...
    V<> cumb(SZ(big)+1, 0), cums(SZ(small)+1, 0);
    REPS(i, SZ(big)) cumb[i] = cumb[i - 1] + big[i - 1];
    REPS(i, SZ(small)) cums[i] = cums[i -1] + small[i - 1];
    ll res = 0;
    REP(k, t + 1){
        ll day = n - (k + (k - 1) * d);
        if(day > SZ(small)) day = SZ(small);
        if(k + (k - 1) * d > n) break;
        chmax(res, cumb[k] + cums[day]);
    }
    print(res);
}