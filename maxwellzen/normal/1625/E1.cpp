...
void solve() {
    cin >> n >> q >> s;
    p.assign(n+1, 0);
    F0R(i, n) p[i+1] = p[i] + (s[i]=='(' ? 1 : -1);
    nxt.rsz(n+1);
    vi tmp;
    R0F(i, n+1) {
        while (!tmp.empty() && p[tmp.back()]>p[i]) tmp.pop_back();
        if (tmp.empty() || p[tmp.back()] != p[i]) nxt[i]=-1;
        else {
            nxt[i] = tmp.back();
            tmp.pop_back();
        }
    ...
    cnt.rsz(n+1, 0);
    R0F(i, n) if (nxt[i] != -1) cnt[i] = cnt[nxt[i]]+1;
    debug(cnt);
    F0R(i, n) cnt[i+1] += cnt[i];
    ...
    while (q--) {
        ...
        if (l>0) ans -= cnt[l-1];
        int val = p[l];
        int in = ub(all(loc[val]), r) - lb(all(loc[val]), l);
        int out = ub(all(loc[val]), last[l]) - ub(all(loc[val]), r);
        debug(ans, in*out);
        ans -= ll(in)*out;
        cout << ans << nl;
    }
