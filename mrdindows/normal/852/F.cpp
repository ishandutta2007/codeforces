...
ll cnk(int n, int k) {
    if (n < k) return 0;
    ll s = cnt[n] - cnt[n - k] - cnt[k];
    if (s) return 0;
    return fact[n] * revfact[n - k] * revfact[k] % t;
}
int phi(int n) {
    int res = n;
    for (int i = 2; i * i <= n; ++i) {
...
//    t = 1e6 + 333;
    ...
    revfact[N] = powmod(fact[N], t - 2, t);
    for (int i = N - 1; i >= 0; --i) {
        int x = (i + 1);
        while (x % t == 0) {
            x /= t;
        }
        revfact[i] = revfact[i + 1] * x % t;
    }
//    printarray(fact, 10);
//    printarray(revfact, 10);
//    for (int i = n - 1; i >= 0; --i) {
//        cout << cnk(i + m, i) << ' ';
//    }
//    cout << endl;
