...
int dfs(int bit, vector<int> a) {
    ...
    if (b.size() == 0) return dfs(bit - 1, c);
    if (c.size() == 0) return dfs(bit - 1, b);
    return (1 << bit) + min(dfs(bit - 1, b), dfs(bit - 1, c));
}
void solve(){
    int n;
    cin >> n;
