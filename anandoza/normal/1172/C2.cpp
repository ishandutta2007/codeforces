...
int main() {
    ...
    for (int i = -m; i <= m; i++) {
        mn v = good + bad + i;
        inverses.push_back(v == 0 ? 0 : v.inv());
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j <= i; j++) {
            ...
            mn b = bad - (i - j);
            int offset = j - (i - j) + m;
            next[j + 1] += g * inverses[offset] * pr_of_delta[j];
            next[j] += b * inverses[offset] * pr_of_delta[j];
        }

        swap(pr_of_delta, next);
    }

    mn expected_good = 0;
    for (int j = 0; j <= m; j++)
        expected_good += j * pr_of_delta[j];
    mn expected_bad = m - expected_good;
    ...
    for (int i = 0; i < n; i++) {
        mn val = w[i];
        if (a[i])
            val += expected_good * w[i];
        else
            val -= expected_bad * w[i];
        ans.push_back(val);
    }
