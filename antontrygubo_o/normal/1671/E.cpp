...
int main()
{
    ...
    vector<int> ans(N);
    vector<string> lexmin(N);

    for (int i = N-1; i>=1; i--)
    {
        if (i>=N/2)
        {
            ans[i] = 1; lexmin[i]+=s[i]; continue;
        }

        lexmin[i]+=s[i];
        if (lexmin[2*i]<=lexmin[2*i+1])
        {
            lexmin[i]+=lexmin[2*i];
            lexmin[i]+=lexmin[2*i+1];
        }
        else
        {
            lexmin[i]+=lexmin[2*i+1];
        ...
        ans[i] = mul(ans[2*i], ans[2*i+1]);

        if (lexmin[i*2] != lexmin[i*2+1])
        {
            ans[i] = mul(ans[i], 2);
        }
    }
