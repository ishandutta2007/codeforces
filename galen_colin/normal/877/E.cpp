...
struct segtree {
  ...
  void eval_lazy(int i, int l, int r) {
    tree[i] = (lazy[i].val & 1) ? ((r - l + 1) - tree[i].val) : tree[i];

    if (l != r) {
      lazy[i * 2 + 1] = (lazy[i].val + lazy[i * 2 + 1].val) & 1;
      lazy[i * 2 + 2] = (lazy[i].val + lazy[i * 2 + 2].val) & 1;
    }
...
int main() {
  ...
  ms(b, 0);
  sst.init(n, b);
  f0r(i, n) sst.update(st[i], st[i], a[i]);

  cin >> q;
  f0r(i, q) {
    cin >> s >> x;
    --x;
    if (s == "pow") sst.update(st[x], en[x], 1);
    else cout << sst.query(st[x], en[x]).val << '\n';
  }
}