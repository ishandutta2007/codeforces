...
template<class S, class T> pair<S, T> operator-(const pair<S, T> &s, const pair<S, T> &t){return pair<S,T>(s.first-t.first, s.second-t.second);}
template<class S, class T, class U> pair<S, T> operator*(const pair<S, T> &s, const U &t){return pair<S,T>(s.first*t, s.second*t);}
template<class S, class T, class U> pair<S, T> operator*(const U &t, const pair<S, T> &s){return pair<S,T>(s.first*t, s.second*t);}
...
vector<T> qvec;
vector<ll> dvec[200005];
vector<ll> vec, lvec, rvec;
LazySegTree seg(18);
ll ans[1000005];
...
int main(void)
{
	...
	rep(i, 1, n) for(int j = i; j <= n; j+=i) dvec[j].pb(i);
	
	vec.pb(n+1);
	rep(i, 1, n) vec.pb(p[i]);
	vec.pb(n+1);
	MaxRectangle::getNearestGreater(vec, lvec, rvec);
	...
	rep(i, 1, n){
		...
		for(auto x : dvec[p[i]]){
			if(x*x >= p[i]) break;
			ll l = i, r = i;
			chmin(l, q[x]), chmax(r, q[x]);
			chmin(l, q[p[i]/x]), chmax(r, q[p[i]/x]);
			if(l < lvec[i] || r > rvec[i]) continue;
			vec.pb(P(r, l));
		}
		sort(all(vec));
		...
		ll L = lvec[i]-1;
		for(auto p : vec){
			if(p.fi > rvec[i]) break;
			if(p.se > L){
				qvec.pb(T(p.fi, P(-(L+1), p.se)));
				qvec.pb(T(rvec[i]+1, P(-(L+1), -p.se)));
				//outl(lvec[i], p.se, p.fi, rvec[i]);
				L = p.se;
			}
		}
	}
	...
	for(auto t : qvec){
		if(t.se.fi < 0){
			ll l = -t.se.fi, l2 = t.se.se, r = t.fi;
			//outl(l, abs(l2), sgn(l2) * P(1, -r+1));
			seg.add(l, abs(l2), sgn(l2) * P(1, -r+1));
		}
		else{
			ll l = t.se.se, r = t.fi, id = t.se.fi;
			P res = seg.query(l, n);
			ans[id] = res.fi * r + res.se;
		}
	}
