...
void dfs(int now,int back) {
    ...
    kazu[now] += cnt / k;
    if (bad == 0 and cnt % k == 0) {
        can_do[now] = 1;
    }
    return;
}
void second_dfs(int now, int back, pair<int, int> back_info) {
    int now_ans = back_info.first;
    int bad = 1 - back_info.second;
    int cnt = back_info.second;
    if (now == 0) {
        bad = 0;
        cnt = 0;
    }
