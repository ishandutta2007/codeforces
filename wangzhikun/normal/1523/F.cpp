...
V<pii> csv;
int dis[16500][130],dp[16500][110],dp2[16500][110];
int main() {
	cin.tie(0)->sync_with_stdio(0);
	re(n,m);
	xa.rsz(n); ya.rsz(n); F0R(i,n) re(xa[i],ya[i]);
	xb.rsz(m); yb.rsz(m); t.rsz(m);  csv.rsz(m);
	F0R(i,m) re(csv[i].s.f,csv[i].s.s,csv[i].f);
	sor(csv);
	F0R(i,m){
		t[i] = csv[i].f;xb[i] = csv[i].s.f;yb[i] = csv[i].s.s;
	}
	set<pii> S;
	F0R(cc,1<<n){
		F0R(i,n){
			dis[cc][i] = 1e9;
			F0R(j,n){
				if((cc>>j)&1)dis[cc][i] = min(dis[cc][i],abs(xa[j]-xa[i])+abs(ya[j]-ya[i]));
			}
		}
		F0R(i,m){
			dis[cc][i+n] = 1e9;
			F0R(j,n){
				if((cc>>j)&1)dis[cc][i+n] = min(dis[cc][i+n],abs(xa[j]-xb[i])+abs(ya[j]-yb[i]));
			}
		}
	}
	F0R(cc,1<<n){
	...
	F0R(cc,1<<n){
		...
		F0R(p1,m){
			F0R(i,n){
				ckmin(dp[cc|1<<i][p1],dp[cc][p1]+dis[cc][i]);
			}
			F0R(i,m){
				int cdis = dis[cc][i+n];
				if(dp[cc][p1]+cdis<=t[i]) ckmax(dp2[cc][i],p1+1);
			}
		}
		F0R(p2,m){
			if(dp2[cc][p2]<0) continue;
			ckmax(ans,dp2[cc][p2]);
			F0R(i,n){
				ckmin(dp[cc|1<<i][dp2[cc][p2]],t[p2]+
					min(dis[cc][i],abs(xb[p2]-xa[i])+abs(yb[p2]-ya[i])));
			}
			FOR(i,p2+1,m){
				int cdis = min(dis[cc][i+n],abs(xb[p2]-xb[i])+abs(yb[p2]-yb[i]));
				if(t[p2]+cdis<=t[i]) ckmax(dp2[cc][i],dp2[cc][p2]+1);
			}
		}
	}
	ps(ans);
