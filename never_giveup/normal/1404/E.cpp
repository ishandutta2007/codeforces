...
const int maxn = 1e6 + 100, maxw = 1e6 + 100, inf = 1e9 + 100, sq = 450, LG = 18, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
int main() {
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < m - 1; j++)
            if (mas[i][j] && mas[i][j + 1]) {
                ans++;
                int x = pos[i][j], y = pos[i][j + 1], z = cur;
                q.add(z, x, inf);
                q.add(z, y, inf);
                q.add(0, z, 1);
                cur++;
    ...
    for (int i = 0; i < n - 1; i++)
        for (int j = 0; j < m; j++)
            if (mas[i][j] && mas[i + 1][j]) {
                ans++;
                int x = pos[i][j], y = pos[i + 1][j], z = cur;
                q.add(x, z, inf);
                q.add(y, z, inf);
                q.add(z, 1, 1);
                cur++;
