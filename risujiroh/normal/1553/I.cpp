...
template <class mint, internal::is_static_modint_t<mint>* = nullptr> void butterfly(mint* a, int n) {
...
template <class mint, internal::is_static_modint_t<mint>* = nullptr> void butterfly(std::vector<mint>& a) {
  butterfly(a.data(), a.size());
}
template <class mint, internal::is_static_modint_t<mint>* = nullptr> void butterfly_inv(mint* a, int n) {
  int h = internal::ceil_pow2(n);
  static const fft_info<mint> info;
...
Mat operator*(const Mat& a, const Mat& b) {
  ...
  for (int i : {0, 1})
    for (int k : {0, 1})
      for (int j : {0, 1}) res[i][j] += atcoder::convolution(a[i][k], b[k][j]);
  return res;
}
...
int main() {
  ...
  for (int l = 0, r = 0; l < n; l = r) {
    ...
    for (int i = l; i < r; i += a[l]) exists[i] = true;
  }
  exists[n] = true;
  vector<Mat> mat(n);
  for (int i = 0; i < n; ++i) {
    if (!exists[i]) {
      mat[i] = {Poly{1}, Poly{}, Poly{}, Poly{1}};
      continue;
    }
    mat[i][0][1] = {0, 2};
    mat[i][1][1] = {1, 2};
  ...
  auto rec = [&](auto self, int l, int r) -> Mat {
    if (r - l == 1) return mat[l];
    int m = (l + r) / 2;
    return self(self, l, m) * self(self, m, r);
  };
  Mat prod = rec(rec, 0, n);
  Vec v{Poly{1}, Poly{1}};
  Poly f = atcoder::convolution(prod[0][0], v[0]);
  f += atcoder::convolution(prod[0][1], v[1]);
  Fp ans, fact = 1;
  for (int x = 1; x < int(size(f)); ++x) {
    fact *= x;
    if ((size(f) - x) & 1)
      ans += f[x] * fact;
    else
