...
public:
    ...
    vvl child;
    // Constructor
    UnionFind(ll sz_): par(sz_), siz(sz_, 1) ,val(sz_),child(sz_){
    ...
    bool merge(ll x, ll y,ll dis) {
        ...
        auto dfs=[&](ll pos,auto dfs)->void{
            val[pos]^=dis;
            for(auto z:child[pos]){
                dfs(z,dfs);
            }
        };
        dfs(y,dfs);
        /*
        cerr<<"root ";rep(i,0,3)cerr<<root(i)<<" ";cerr<<endl;
        cerr<<"val ";rep(i,0,3)cerr<<val[i]<<" ";cerr<<endl<<endl;
        */
        return true;
    }
...
signed main(){fastio
    ...
    while(n--){
        ...
        if(t==1){
            ll x;cin>>x;x^=last;
            if(!uf.issame(l,r))uf.merge(l,r,x);
          //  cerr<<"lrx "<<l<<" "<<r<<" "<<x<<endl;
        }else{
            ll val=uf.dist(l,r);
            cout<<val<<endl;
