...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		ans[i] = v[i]*v[i];
		PQ.push({ans[i]-((v[i]/cn[i])*(v[i]/cn[i])*(cn[i]-(v[i]%cn[i]))+((v[i]/cn[i]+1)*(v[i]/cn[i]+1)*(v[i]%cn[i]))),i});
	}
	for(int i = 0; i < k-n; ++i){
		auto x = PQ.top();
		PQ.pop();
		ans[x.s]-=x.f;
		cn[x.s]++;
		PQ.push({ans[x.s]-((v[x.s]/cn[x.s])*(v[x.s]/cn[x.s])*(cn[x.s]-(v[x.s]%cn[x.s]))+((v[x.s]/cn[x.s]+1)*(v[x.s]/cn[x.s]+1)*(v[x.s]%cn[x.s]))),x.s});
	}
	ll out = 0;
	for(int i = 0; i < n; ++i){
		out+=ans[i];
