...
int main(){
	...
	For(TwT,T){
		Read(n,initPos,m);
		For(i,m) Read(l[i],r[i]);

		static vc<pii> rs; rs.clear();
		rs.Pb({initPos,initPos});
		For(i,m){
			int l = ::l[i], r = ::r[i];
			static vc<pii> nrs; nrs.clear();
			for( auto &[ul,ur] : rs ){
				if( ur < l or ul > r ) continue;
				if( ul <= l and r <= ur ) continue;
				Mymin(ul,l); Mymax(ur,r);
			}
		}

		cout << rs[0].S - rs[0].F + 1 << endl;
	}
