...
#define tup tuple<int,int,int>
    ...
    void dfs(res x){
        for(auto [tox,c]:G[x]){
            if(Fl)return;
            if(!vis[tox])f[tox]=f[x]^c,vis[tox]=1,dfs(tox);
            else {
                if(f[tox]^f[x]^c){Fl=1;return;}
            }
        }
    }
    inline void MAIN(){
        n=read(),m=read();
        for(res i=1;i<=n;i++)vis[i]=0,vector<Pair>().swap(G[i]);
        for(res i=1;i<n;i++){
            res u=read(),v=read(),c=read();
            if(c!=-1)G[u].pb(mp(v,pc(c)&1)),G[v].pb(mp(u,pc(c)&1));
            edge[i]=tup(u,v,c);
        }
        for(res i=1;i<=m;i++){
            res u=read(),v=read(),c=read();
        ...
        for(res i=1;i<=n;i++){
            if(vis[i])continue;
            Fl=0,vis[i]=1,f[i]=0,dfs(i);
            if(Fl){puts("NO");return;}
        }
        puts("YES");
        for(res i=1;i<n;i++){
            res u=get<0>(edge[i]),v=get<1>(edge[i]);
            if(get<2>(edge[i])!=-1)printf("%d %d %d\n",u,v,get<2>(edge[i]));
            else printf("%d %d %d\n",u,v,f[u]^f[v]);
        }
    }
}
