...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        for(res i=1;i<=n;i++)dis[i]=Dis[i]=10*n,f[i]=g[i]=0;
        dis[s]=0,f[s]=1,Q[he=ta=1]=s;
        while(he<=ta){
            res x=Q[he++];
            for(auto tox:G[x]){
                if(dis[tox]>n)dis[tox]=dis[x]+1,f[tox]=f[x],Q[++ta]=tox;
                else if(dis[tox]==dis[x]+1)add(f[tox],f[x]);
            }
        }
        Dis[t]=0,g[t]=1,Q[he=ta=1]=t;
        while(he<=ta){
            res x=Q[he++];
            for(auto tox:G[x]){
                if(Dis[tox]>n)Dis[tox]=Dis[x]+1,g[tox]=g[x],Q[++ta]=tox;
                else if(Dis[tox]==Dis[x]+1)add(g[tox],g[x]);
            }
        }
        res ans=f[t];
        for(res x=1;x<=n;x++){
            for(auto tox:G[x]){
                if(dis[x]+Dis[tox]==dis[t]&&dis[x]==dis[tox])add(ans,mul(f[x],g[tox]));
            }
        }
        printf("%d\n",ans);
    }
