...
struct segtree
{
	INT dt[1600007],wt[1600007];
	void turn(int now,int l,int r,int nl,int nr,INT num)
	{
		if(wt[now])
		{
			dt[now]=wt[now];
		...
		int m=(nl+nr)>>1;
		turn(now<<1,l,r,nl,m,num);
		turn(now<<1|1,l,r,m+1,nr,num);
		dt[now]=max(dt[now*2],dt[now*2+1]);
	}
	INT calcmax(int now,int l,int r,int nl,int nr)
	{
		if(wt[now])
		{
			dt[now]=wt[now];
		...
		if(nl>=l&&nr<=r)
			return dt[now];
		int m=(nl+nr)>>1;
		return max(calcmax(now<<1,l,r,nl,m),calcmax(now<<1|1,l,r,m+1,nr));
	}
}sg;
...
int main()
{
	...
	dearr(sg.dt,sg.dt+20);
	m=read();
	for(int i=1;i<=m;i++)
	{
		INT a=READ();
		INT b=READ();
		INT mxx=sg.calcmax(1,1,a,1,n);
		printf("%I64d\n",mxx);
		sg.turn(1,1,a,1,n,mxx+b);
	}
	return 0;
}