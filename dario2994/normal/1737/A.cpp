...
void solve() {
    ...
    // dbg(occ);
    for (int i = 1; i < 26; i++) occ[i] = min(occ[i-1], occ[i]);
    for (int i = n/k; i < 26; i++) occ[i] = 0;
    // dbg(occ);
    for (int i = 0; i < 26-1; i++) occ[i] -= occ[i+1];

    // dbg(n, k, s, occ);
    string res = "";
    for (int i = 25; i >= 1; i--) res += string(occ[i-1], char('a' + i));
    res.resize(k, 'a');
    cout << res << "\n";
}
