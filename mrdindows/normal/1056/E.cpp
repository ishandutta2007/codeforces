...
string s, t;
const int maxlen = 1010101; //   
const int alphabet = 256; //  , <= maxlen
int p[maxlen], cnt[maxlen], c[maxlen];
static int pn[maxlen];
static int cn[maxlen];
int sc[21][maxlen];
bool equal(int i, int j, int l) {
    int k = 31 - __builtin_clz(l);
    pair<int,int> a = make_pair (sc[k][i], sc[k][i+l-(1<<k)]);
    pair<int,int> b = make_pair (sc[k][j], sc[k][j+l-(1<<k)]);
    return a == b;
}
void sufArr() {
    auto& s = t;
...
//    auto zf = z_function(t);
    ...
    for (int len = 1; len <= t.size(); ++len) {
        ll f = len * cnt01[0];
        if (f >= t.size()) break;
        if ((t.size() - f) % cnt01[1]) continue;
        int lens[2] = {len, cnt01[1] ? ((int)t.size() - (int)f) / cnt01[1] : 0};
        int first[2] = {-1, -1};
        int pos = 0;
        bool ok = true;
        for (char c : s) {
            if (first[c] == -1) {
                ...
                ok &= equal(first[c], pos, lens[c]);
            }
            pos += lens[c];
        }
        if (ok && lens[0] == lens[1] && equal(first[0], first[1], lens[0])) {
            ok = false;
        }
        ans += ok;
    }
