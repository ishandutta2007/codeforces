...
int main(){
    ...
    auto choose = [&](int x, int y) -> mi{
        if(x<y)return 0;
        if(x<0||y<0) return 0;
        return fact[x]*ifact[x-y]*ifact[y];
    };
    setIO("");
    int n, k;
    re(n, k);
    ...
    f0r(i, n) re(use[i].f, use[i].s);
    sort(all(use));
    set<int> comp;
    f0r(i, n) comp.ins(use[i].f), comp.ins(use[i].s);
    map<int, int> conv;
    int cnt = 0;
    for(int x: comp) conv[x] = cnt,cnt++;

    //earliest time they overlap
    for(auto x: use){
        int l = conv[x.f];
        int r = conv[x.s];
        seg.upd(l,r,1);
    ...
    for(auto t:tmp){
        int num = t.s;
        int l = conv[t.f];
        int val = seg.query(l,l)-num;
        f1r(i, 1,num+1){
            ans+=choose(num,i)*choose(val,k-i);
        }
    }
    ps(ans);
    return 0;
