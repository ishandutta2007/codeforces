...
void solve() {
	...
	for (int di = -1; di <= 1; di += 2) for (int dj = -1; dj <= 1; dj += 2) {
		debug(imie(di), imie(dj));
		for (int i = (di == 1) ? 0 : n - 1; i != -1 && i != n; i += di)
			for (int j = (dj == 1) ? 0 : m - 1; j != -1 && j != m; j += dj) {
				debug(imie(i), imie(j));
				if (on(i - di, j - dj)) {
					bool eq = in[i - di][j] == in[i][j] && in[i][j - dj] == in[i][j];
					b[i][j] = (eq ? 1 + min(b[i - di][j], b[i][j - dj]) : 1);
				}
				else
					b[i][j] = 1;
				mini(ans[i][j], b[i][j]);
