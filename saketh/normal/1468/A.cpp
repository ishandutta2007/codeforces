...
void solve() {
    ...
    auto join = [](int a, int b) { return max(a, b); };
    binary_indexed_tree<int, decltype(join)> bit(U, 0, join);
    ...
    F0R (i, N) {
        dp[i + 1] = bit.accumulate_prefix(index(univ, a[i]) + 1) + 1;
        bit.add(index(univ, a[i]), dp[i + 1]);

        while (!st.empty() && st.top().f <= a[i]) {
            bit.add(index(univ, st.top().f), ++dp[st.top().s]);
            st.pop();
        }
