...
void solve() {
  ...
  auto many = [&]() -> void { print("Data not sufficient!"); };
  auto none = [&]() -> void { print("Game cheated!"); };

  ll ng = 0;
  vc<pi> ANS;
  FOR(i, len(event)) {
    if (i && ng == 0) {
      ll l = event[i - 1].fi, r = event[i].fi;
      if (l < r) ANS.eb(l, r);
    }
    ng += event[i].se;
  }
  ll cnt = 0;
  FORIN(p, ANS) cnt += p.se - p.fi;
  if (cnt >= 2) return many();
  if (cnt == 1) return print(ANS[0].fi);
  return none();
}
