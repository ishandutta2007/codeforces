...
ll calc(ll keta, bool flag)
{
	rep(i, 0, 16) rep(j, 0, keta) dp[i][j] = 0;
	dp[16][0] = 1;
	
	per(i, 16, 1){
		rep(j, 0, keta){
			rep(k, 0, a[i-1]){
				if(j+k > keta) continue;
				if(!flag || i > 1) dp[i-1][j+k] += dp[i][j] * comb[j+k][k];
				else if(j > 0) dp[i-1][j+k] += dp[i][j] * comb[j+k-1][k];
				chmin(dp[i-1][j+k], M);
			}
		}
	}
...
int main(void)
{
	...
	rep(i, 1, 50){
		rep(j, 0, i){
			if(j-1 >= 0) comb[i][j] += comb[i-1][j-1];
			comb[i][j] += comb[i-1][j];
			chmin(comb[i][j], M);
		}
	}
