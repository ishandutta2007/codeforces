...
void solve()
{
	...
	input(px);
	input(py);
	vector<vector<int>> vx(n), vy(m);
	int64_t res = 0;
	for (int i = 0; i < k; ++i)
	{
		int x, y;
		read(x, y);
		int idx = lower_bound(px.begin(), px.end(), x) - px.begin();
		int idy = lower_bound(py.begin(), py.end(), y) - py.begin();
		if (px[idx] != x)
		{
			vx[idx].push_back(y);
		}
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
			int id = lower_bound(vx[i].begin(), vx[i].end(), vx[i][j]) - vx[i].begin();
			res += id;
		}
	}
	for (int i = 0; i < m; ++i)
	{
		...
		{
			int id = lower_bound(vy[i].begin(), vy[i].end(), vy[i][j]) - vy[i].begin();
			res += id;
		}
	}
	writeln(res);
