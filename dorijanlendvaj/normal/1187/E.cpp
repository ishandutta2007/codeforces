...
int main()
{
	...
	dfs(1);
	for (int i=1;i<=n;++i) ans[1]+=de[i];
	for (int i=1;i<n;++i) ans[o[i]]=ans[par[o[i]]]+n-ss[o[i]]*2;
	//cout<<de[2]<<' '<<de[1]<<endl;
	ll ma=0;
	for (int i=1;i<=n;++i) ma=max(ma,ans[i]+n);
	cout<<ma;
}