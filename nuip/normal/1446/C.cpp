...
int solve(vector<int> vs){
	const int n=vs.size();
	if(n<=2) return 0;
	map<int,vector<int>> vss;
	for(cauto v:vs) vss[v?__builtin_clz(v):32].eb(v);
	int sum=0;
	int cnt=0;
	int re=n;
	for(auto &[lz,vs]:vss){
		if(lz==32){
			MN(re,sum-cnt+max(0,n-sum-(int)vs.size()-1));
			break;
		}
		for(auto &v:vs) v^=1<<(31-lz);
		MN(re,solve(vs)+sum-cnt+max(0,n-sum-(int)vs.size()-1));
		sum+=vs.size();
		++cnt;
	}
	return re;
