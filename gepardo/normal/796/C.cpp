 ...
 * |                  alex256                     |
 * ------------------------------------------------
 * | Contest | CodeForces Round 408 (Div. 2)      |
 * | Task    | C                                  |
 * | Date    | 10.04.2017                         |
 * ------------------------------------------------
 **/
...
signed main() {
	...
	auto ok = [&](int64_t k) -> bool {
		vector<int64_t> rng(n);
		int k0 = 0, k1 = 0;
		for (int i = 0; i < n; i++) {
			rng[i] = min((int64_t)2, k - v[i]);
			if (rng[i] < 0) {
				return false;
			} else if (rng[i] == 0) {
				k0++;
	...
	const int64_t RANGE = (int64_t)2e9;
	int64_t l = -RANGE, r = +RANGE;
	while (l < r) {
		int64_t m = (l + r + 2*RANGE) / 2 - RANGE;
		if (ok(m)) {
			r = m;
		} else {
			l = m + 1;
