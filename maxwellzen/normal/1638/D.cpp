...
int getmax(int i, int j) {
    return max({a[i][j], a[i+1][j], a[i][j+1], a[i+1][j+1]});
}
bool good(int i, int j) {
    int mx = getmax(i, j);
    FOR(x, i, i+2) FOR(y, j, j+2) if (a[x][y] != mx && a[x][y] != -1) return false;
    return true;
}
...
void solve() {
    ...
    while (! s.empty()) {
        ...
        if (mx > -1) {
            ...
            FOR(i, max(x-1, 0), min(x+2, n-1))
                FOR(j, max(y-1, 0), min(y+2, m-1)) {
                    if (!vis[i][j] && good(i, j)) vis[i][j]=true, s.insert(mp(i, j));
                }
        }
    }
    ...
    cout << sol.size() << nl;
    R0F(i, sol.size()) cout << sol[i][0] << ' ' << sol[i][1] << ' ' << sol[i][2] << nl;
}
