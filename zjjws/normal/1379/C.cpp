...
struct gyq
{
    ...
    inline bool operator <(gyq ath){return a<ath.a;}
}g[N];
int a[N];
LL sum[N];
inline void work()
{
    rin>>n>>m;
    for(int i=1;i<=m;i++)g[i].Read();sort(g+1,g+m+1);
    for(int i=1;i<=m;i++)a[i]=g[i].a;sum[m+1]=0;
    for(int i=m;i>=1;i--)sum[i]=sum[i+1]+a[i];
    LL ans=(n>m)?sum[1]:sum[m-n+1];
    for(int i=1;i<=m;i++)
    {
        int fd=upper_bound(a+1,a+m+1,g[i].b)-a;if(m-fd+1>=n)continue;
        int les=n-(m-fd+1);LL nows=sum[fd];
        if(i<fd)les--,nows+=a[i];nows+=1LL*g[i].b*les;ans=max(ans,nows);
    }
    printf("%lld\n",ans);
    return;
}
