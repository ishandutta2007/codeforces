...
inline void normalize() {
	while (!a.empty() && !b.empty() && a.back().first == b.back().first) {
		int mimin = min(a.back().second, b.back().second);
		a.back().second -= mimin;
...
inline void AddTo(int value, deque<ii>& deq) {
	ii now = {value, 1};
	while (!deq.empty()) {
		if (deq.front().first == now.first) {
			now.second += deq.front().second;
			deq.pop_front();
...
inline constexpr int pkt(int a, int b) {
	int result = 1;
	while (b) {
...
inline ll Calculate(const deque<ii>& deq) {
	int result = 0;
	LOG("LOL");
	for (const auto& now : deq) {
		LOG(now);
		result += (1LL * pkt(p, now.first) * now.second) % MOD;
...
void solve(int tc_num = -1) {
	...
	FORD (i, n - 1, 0) {
		if (a.empty()) {
			a.emplace_front(k[i], 1);
		} else if (b.empty()) {
			b.emplace_front(k[i], 1);
		} else {
			const auto& A = a.back();
			const auto& B = b.back();
			if (A < B) {
	...
	const auto& smaller = (a.empty() || (!b.empty() && a.back() < b.back())) ? a : b;
	const auto& bigger = (a.empty() || (!b.empty() && a.back() < b.back())) ? b : a;
	cout << (Calculate(bigger) - Calculate(smaller) + MOD) % MOD << "\n";
}
