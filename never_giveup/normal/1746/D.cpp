...
const int maxn = 2e5 + 100, maxw = 3e3 + 10, inf = 1e9 + 100, LG = 18, SQ = 300;
...
void dfs(int v) {
    int x = who[v][0], y = who[v][1];
    assert(x == y || x + 1 == y);
    int d = e[v].size();
    if (d == 0) {
        for (int t = 0; t < 2; t++)
            dp[v][t] = who[v][t] * (ll)s[v];
        return;
    }
    for (int u : e[v]) {
        who[u][0] = x / d;
    ...
    for (int t = 0; t < 2; t++) {
        ...
        ll o = 0;
        for (int u : e[v]) {
            ll v1 = get(u, x / d), v2 = get(u, (x + d - 1) / d);
            o += v1;
            q.emplace_back(v2 - v1);
        }
        sort(all(q));
        reverse(all(q));
        for (int i = 0; i < x % d; i++)
