...
public class Main
{
	...
	static class node
	{
		...
		boolean fate=false;
		boolean please[]= {false,false};
		void dfs(int fr, int at, int dist, int num)
		{
			please[num]=true;
			if(dist<d)
				for(int i:adj)
					if(i!=fr)
						gra[i].dfs(at, i, dist+1, num);
		}
	}
	static pair dfs(int fr, int at)
	{
		pair m1=new pair(),m2=new pair(),te;
