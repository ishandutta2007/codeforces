...
fn main() {
	...
	let n = rin.u();
	let mut a = v!([n+2][n+2] = (0,0));
	let mut b = v!([n+2][n+2] = b'?');
	for i in 1..=n {
		for j in 1..=n {
			l!(x,y = rin.i());
			let (x,y) = if x == -1 {(0,0)} else {(x as usize, y as usize)};
			a[i][j] = (x,y);
		}
	}
	let a = a;
	let adj = |i: usize, j: usize| -> Vec<(usize,usize,u8,u8)> {
		vec![(i+1,j,b'D',b'U'), (i-1,j,b'U',b'D'), (i,j+1,b'R',b'L'), (i,j-1,b'L',b'R')]
	};
		...
		for j in 1..=n {
			if a[i][j] == (i,j) {
				...
				while let Some( (x,y) ) = q.pop() {
					for (xx,yy,_,f) in adj(x,y) {
						if a[xx][yy] == (i,j) && b[xx][yy] == b'?' {
							b[xx][yy] = f;
							q.push( (xx,yy) );
						}
					}
				...
				for (x,y,t,_) in adj(i,j) {
					if x.min(y) >= 1 && x.max(y) <= n && a[x][y] == (0,0) {
						b[i][j] = t;
						break;
					}
				}
	...
	for i in 1..=n {
		for j in 1..=n {
			if b[i][j] == b'?' {
				writeln!(rout, "INVALID").ok();
				return;
			}
		}
	}
	writeln!(rout, "VALID").ok();
	for i in 1..=n {
		for j in 1..=n {
			write!(rout, "{}", b[i][j] as char).ok();
		}
		writeln!(rout, "").ok();
	}
}