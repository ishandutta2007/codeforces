...
void solve(){
	int g, lim, n; cin >> g >> lim >> n;
	vc<P>pos(n);
	rep(i,n)cin>>pos[i].a>>pos[i].b;
	pos.eb(g, 0);
	SORT(pos);
	if(pos[0].a > lim){
		o(-1); return;
	...
	int ans = 0;
	rep(i, n){
		seg.update(i, i, 0, 0, s-1, zan);
		que.push(mp(pos[i].b, i));
		
		while(seg.query(i, i, 0, 0, s-1) < pos[i+1].a-pos[i].a and que.size()){
			auto q = que.top();
			int curmax = seg.query(q.b, i, 0, 0, s-1);
			int v = seg.query(i, i, 0, 0, s-1);
			int need = pos[i+1].a - pos[i].a;
			if(need-v <= lim-curmax){
				ans += q.a * (need-v);
				seg.update(q.b, i, 0, 0, s-1, need-v);
				break;
			}
			else{
				ans += q.a * (lim-curmax);
				seg.update(q.b, i, 0, 0, s-1, lim-curmax);
				que.pop();
			}
		}
		if(seg.query(i, i, 0, 0, s-1) < pos[i+1].a-pos[i].a){
			o(-1); return;
		}
		zan = seg.query(i, i, 0, 0, s-1) - (pos[i+1].a-pos[i].a);
		//cerr<<i<<" "<<ans<<" "<<zan<<endl;
	}
	o(ans);
}
