#line 1 "codeforces-workspace/contest/1503/c/c.cpp"
...
#line 6 "codeforces-workspace/contest/1503/c/c.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1503/c/c.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1503/c/c.cpp"
...
#line 27 "codeforces-workspace/contest/1503/c/c.cpp"
...
struct endo {
  i64 v = inf;
  endo operator*(endo r) const { return {min(v, r.v)}; }
};

struct mono {
  i64 v = inf;
  mono operator+(mono r) const { return {min(v, r.v)}; }
  mono operator*(endo r) const { return {min(v, r.v)}; }
};

i64 calc(vector<pair<int, int>> cs) {
  auto n = cs.size();
  lazy_segment_tree<mono, endo> s1(n), s2(n);
  s1[0] = {0};

  auto ii = 0;
  for (auto &&[i, _e] : enumerate(cs)) {
    auto [a, b] = _e;
    assert(a <= b);
...
void main() {
  ...
  auto iter = min_element(begin(cities), end(cities));
  rotate(begin(cities), iter, end(cities));
  sort(begin(cities) + 1, end(cities));
  ans += calc(cities);
  cout << ans << "\n";
  return;
  ...
  // split
  {
    auto mid = remove_if(begin(cities) + 1, end(cities),
                         [&](auto x) { return x.first < cities[0].first; });

    // auto mid2 = remove_if(begin(cities) + 1, mid,
    //                       [&](auto x) { return x.second < cities[0].second;
    //                       });

    // auto mid3 = remove_if(mid, end(cities),
    //                       [&](auto x) { return x.second < cities[0].second;
    //                       });

    vector high(cities.begin(), mid), low(mid, cities.end());

    low.insert(low.begin(), high.front());
    for (auto &&[a, b] : low) {
      swap(a, b);
      a = -a, b = -b;
    }

    sort(begin(high) + 1, end(high));
    sort(begin(low) + 1, end(low));
