...
class Solver {
    ...
    public void solve() {
        ...
        {
            var s = sc.Str;
            var c0 = new int[s.Length + 1];
            var c1 = new int[s.Length + 1];
            for (int i = 0; i < s.Length; i++)
            {
                c0[i + 1] = c0[i] + (s[i] == '0' ? 1 : 0);
                c1[i + 1] = c1[i] + (s[i] == '1' ? 1 : 0);
            }
            int ans = M;
            for (int i = 0; i <= s.Length; i++)
            {
                ans = Math.Min(ans, c0[i] + c1[s.Length] - c1[i]);
                ans = Math.Min(ans, c1[i] + c0[s.Length] - c0[i]);
            }
            Prt(ans);
        }
