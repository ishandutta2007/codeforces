...
int main(){
  ...
  vvi B(M+1);
  for(li i = 0; i < (li)(n); ++i) B[A[i]].push_back(i);

  for(li i = 0; i < (li)(M+1); ++i) if(B[i].size() >= 4) {
    ok(B[i][0],B[i][1],B[i][2],B[i][3]);
  }
  vi n2;
  for(li i = 0; i < (li)(M+1); ++i) if(B[i].size() >= 2) {
    n2.push_back(i);
  }
  if(n2.size() >= 2) {
    ok(B[n2[0]][0], B[n2[1]][0], B[n2[0]][1], B[n2[1]][1]);
  }

  vvii diff(M+1);
  int last = -1;
  for(li i = 0; i < (li)(M+1); ++i) if(B[i].size()) {
    if(last != -1) diff[i-last].emplace_back(last,i);
    last = i;
  }
  for(li i = 0; i < (li)(M+1); ++i) if(diff[i].size() >= 2) {
    for(auto const& p : diff[i]) for(auto const& q : diff[i]) {
        int a,b,c,d; tie(a,b) = p; tie(c,d) = q;
        a = B[a][0];
        b = B[b][0];
        c = B[c][0];
        d = B[d][0];
        if(a!=c&&a!=d&&b!=c&&b!=d) ok(b,c,d,a);
      }
  }

  vector<vii> V(2*M+1);
  for(li a = 0; a < (li)(n); ++a) for(li b = (a+1); b <= (li)(n-1); ++b) {
    for(auto q : V[A[a]+A[b]]) {
      int c,d; tie(c,d) = q;
      if(c == a || c == b || d == a || d == b) continue;
      ok(a,b,c,d);
    }
    V[A[a]+A[b]].emplace_back(a,b);
  }
  cout << "NO" << endl;
