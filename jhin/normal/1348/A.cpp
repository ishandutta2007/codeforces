        ...
        while(t--)
        {
            cin>>n;vector<int>tmp;int tsum=0,tsum2=0;
            for(int i=0;i<n;i++)tmp.push_back(my_pow(2,i+1));
            for(int i=0;i<n/2-1;i++)tsum+=tmp[i];
            tsum+=tmp[n-1];
            for(int i=n/2-1;i<n-1;i++)tsum2+=tmp[i];
