...
int main(){
    ...
    string s; cin >> s;
    int n = s.size();
    auto dpl = vec3d(n+1, n+1, 2, mint(0));
    dpl[0][0][0] = 1;
    for(int i = 0; i < n; i++){
        for(int j = 0; j <= i; j++){
            if(s[i] == '(' || s[i] == '?'){
                dpl[i+1][j+1][1] += dpl[i][j][0]+dpl[i][j][1];
            }
            if(s[i] == ')' || s[i] == '?'){
    ...
    mint ans = 0;
    for(int i = 1; i < n; i++){
        auto cumsum = Cumsum<mint>(dpr[i]);
        // print_vector(dpl[i]);
        // print_vector(dpr[i]);
        for(int j = 1; j <= i; j++){
            ans += dpl[i][j][1]*cumsum.sum(j, n+1);
        }
    }
    cout << ans << endl;
}