...
void solve() {
  ...
  auto calc = [&](vc<P> XY) -> void {
    sort(all(XY), [](auto& x, auto& y) { return x.polar().se < y.polar().se; });
    using Re = double;
    vc<Re> angles(N + N);
    FOR(i, N) angles[i] = XY[i].polar().se;
    Re PI = asin(1) * 2;
    FOR(i, N) angles[N + i] = angles[i] + 2 * PI;
    FOR(i, N) XY.eb(XY[i]);

    const ll off = 100000;
    FenwickTree<Group_Add<ll>> bit(2 * off + 1);

    auto add = [&](ll i) -> void { bit.add(XY[i].x + off, 1); };
    auto rm = [&](ll i) -> void { bit.add(XY[i].x + off, -1); };

    Re eps = 1e-12;

    ll L = 0, R = 0;
    FOR(i, N) {
      while (angles[L] <= angles[i] + eps) rm(L++);
      while (R < L) add(R++);
      while (angles[R] <= angles[i] + PI - eps) add(R++);
      ll x = XY[i].x;
      ANS -= bit.sum_all() - bit.sum(x + off + 1);
    }
    /*
    FOR(i, N) FOR(j, N) {
      auto [xi, yi] = XY[i];
      auto [xj, yj] = XY[j];
      if (xi < xj && xi * yj > xj * yi) --ANS;
    }
    */
  };

  for (auto&& xy: XY) xy.y += LIM;
  calc(XY);
  for (auto&& xy: XY) xy.y = 2 * LIM - xy.y;
  calc(XY);
  print(ANS);
}
