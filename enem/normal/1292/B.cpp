    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,mp.size())
        {
            ll t=t0;
            ll x=0;
            t-=abs(xs-mp[i].F)+abs(ys-mp[i].S);
            if(t<0) continue;
            x++;
            REPI(j,0,i)
            {
                t-=abs(mp[j+1].F-mp[j].F)+abs(mp[j+1].S-mp[j].S);
                if(t<0) break;
                x++;
            }
            if(t>=0&&i<mp.size()-1)
            {
                t-=abs(mp[i+1].F-mp[0].F)+abs(mp[i+1].S-mp[0].S);
                if(t<0) x--;
                x++;
                REP(j,i+1,mp.size()-1)
                {
