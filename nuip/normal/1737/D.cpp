...
int main(){
	...
	TT{
		cauto n=read(0);
		cauto m=read(0);
		cauto g=readG<ll>(n,m);
		rep(i,n)rep(j,n) edge[i][j]=MOD;
		rep(v,n)for(cauto &[u,w]:g[v]){
			MN(edge[u][v],w);
			MN(edge[v][u],w);
		}
		vector<int> ds(n,MOD),es(n,MOD);
		cauto fun=
			[&](vector<int> &ds, int s){
				ds.clear();
				ds.resize(n,MOD);
				...
				while(que.size()){
					...
					for(cauto &[u,ignore]:g[v])if(MN(ds[u],ds[v]+1)) que.emplace(u);
				}
			};
		fun(ds,0);
		fun(es,n-1);
		...
		rep(v,n){
			vector<int> fs;
			fun(fs,v);
			rep(a,n)rep(b,a)if(edge[a][b]<MOD) MN(re, edge[a][b]*(min(fs[a],fs[b])+ds[v]+es[v]+2));
		}
		cout<<re NL;
	}
	return 0;
