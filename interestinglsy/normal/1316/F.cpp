...
const ll inv2 = Inv(2);
ll pow2[MAXN*2], invPow2[MAXN*2];
void InitPows(){
	pow2[0] = invPow2[0] = 1;
	For(i,MAXN*2-1){
		pow2[i] = pow2[i-1]*2 % MOD;
		invPow2[i] = invPow2[i-1]*inv2 % MOD;
		// assert( invPow2[i] == Inv(pow2[i]) );
	}
}

struct DiscObj{ int real, vir; };

int n,q;
DiscObj a[MAXN];
struct Operation{ int pos; DiscObj x; };
vc<Operation> ops;

ll Bf(){
	ll ans = 0;
	static DiscObj t[MAXN];
	For(i,n) t[i] = a[i];
	sort(t+1,t+1+n,[]( auto a , auto b ){ return a.real < b.real; });
	For(i,n){
		Forx(j,i+1,n){
			ll tans = pow2[i-1] * pow2[n-j] % MOD * (1LL*t[i].real*t[j].real%MOD) % MOD;
			Addi( ans , tans );
		}
	}
	return ans;
...
il Node SingleNode( ll x ){
	return Node{0,x*2%MOD,x*inv2%MOD,1};
}
Node operator+( const Node &a , const Node &b ){
	return {
		( a.ans + b.ans + a.sum1*b.sum2%MOD*invPow2[a.cnt] ) % MOD,
		( a.sum1 + b.sum1*pow2[a.cnt] ) % MOD,
		( a.sum2 + b.sum2*invPow2[a.cnt] ) % MOD,
		a.cnt + b.cnt
	};
}
...
void Modify( int gpos , Node x , ARG ){
	if( l == r ){
		t[pos] = x;
		return;
	}
...
int main(){
	...
	For(i,n) Modify(a[i].vir,SingleNode(a[i].real),ROOT);
	PrintAns();
	for( auto [pos,x] : ops ){
		Modify(a[pos].vir,{0,0,0,0},ROOT);
		Modify(x.vir,SingleNode(x.real),ROOT);
		a[pos] = x;
		PrintAns();
	}
