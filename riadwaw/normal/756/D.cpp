...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dp = make_vector<Z>(n + 1, 26, Z());
		auto dpall = make_vector<Z>(n + 1, Z());
		dpall[0] = 1;

		for (int i: range(n)) {
			for (int c: downrange(n)) {
				char d = s[i] - 'a';
				dpall[c + 1] += dpall[c] - dp[c][d] - dp[c + 1][d];
				dp[c + 1][d] = dpall[c] - dp[c][d];
		...
		auto fact = factorials<Z>(100000);
		for (int cnt: inclusiveRange(1, n)) {
			//cerr << cnt << ' ' << dpall[cnt] << endl;
			int stars = n - cnt;
			int groups = cnt;
			ans += dpall[cnt] * fact[stars + groups - 1] / fact[stars] / fact[groups - 1];
		}
		out << ans << "\n";
	}
};
