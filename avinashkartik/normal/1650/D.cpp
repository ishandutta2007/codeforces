...
void solve() {
	...
	ans[n] = (ind[n] + 1) % n;
	st.update(ind[n] + 1, 1);
	for (int i = n - 1, j; i >= 2; i--) {
		if (ind[i] < ind[i + 1]) {
			j = ind[i] - st.query(1, ind[i] + 1).val + n - ind[i + 1] - st.query(ind[i + 1] + 1, n).val;
		} else {
			j = ind[i] - ind[i + 1] - 1 - st.query(ind[i + 1] + 1, ind[i] + 1).val + 1;
		}
		ans[i] = (j + 1) % i;
		st.update(ind[i] + 1, 1);
	}
