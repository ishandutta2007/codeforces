...
int main() {
    ...
    auto solve = [&](int k) {
        ...
        vector<Mint> cur(m + 1, 0);
        vector<Mint> next(m + 1, 0);
        cur[0] = 1;
        for (auto [l, r] : v) {
            for (int i = 1; i < cur.size(); ++i)
                cur[i] += cur[i - 1];
            for (int i = 0; i <= m; ++i) {
                next[i] = (i - l >= 0 ? cur[i - l] : Mint(0)) - (i - r - 1 >= 0 ? cur[i - r - 1] : Mint(0));
            }
            swap(cur, next);
        }
        return accumulate(cur.begin(), cur.end(), Mint(0));
    };
