...
using u256 = array<ull, 4>;
u256 r(){
    return {rnd(), rnd(), rnd(), rnd()};
}
void operator^=(u256& a, const u256& b){
    rep(4) a[i] ^= b[i];
}
namespace std {
    template<> struct hash<u256>{
        inline constexpr ull operator()(const u256& a) const {
            return a[0];
        }
    };
}
signed main(){
    ...
    rep(_,t){
        ...
        unordered_map<u256,ull>c;
        rep(n)if(b[i][0]||b[i][1]||b[i][2]||b[i][3])c[b[i]]+=a[i];
        ull ans=0;
        each(i,c){
            if(!ans)ans=i.second;
            else ans=gcd(ans,i.second);
        }
        out(ans);
    }
}