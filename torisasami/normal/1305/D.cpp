...
int n, u, v,cnt=0,d[1111],del[1111];
vector<vector<int>> li(1111);
	...
	while (cnt < n - 1) {
		...
		if (cnt == n - 2) {
			...
			if (ans == q)
				swap(p, q);
			dfs(q, p);
			auto itr = lower_bound(all(li[p]), q);
			li[p].erase(itr);
			d[p]--;
		}
		else {
			q = *li[p].begin();
			auto itr0 = li[q].begin();
			if (*itr0 == p) {
				itr0++;
				q = *itr0;
			}
			...
			if (ans != p && ans != q) {
				dfs(p, ans);
				auto itr = lower_bound(all(li[ans]), p);
				li[ans].erase(itr);
				d[ans]--;
				dfs(q, ans);
				itr= lower_bound(all(li[ans]), q);
				li[ans].erase(itr);
				d[ans]--;
			}
			else {
				...
				if (ans == q)
					swap(p, q);
				dfs(c, p);
				auto itr = lower_bound(all(li[p]), c);
				li[p].erase(itr);
				d[p]--;
			}
		}
