...
void Nyaan::solve() {
  vi dp(TEN(3) + 1, -1);
  {
    ...
    while (!Q.empty()) {
      int c = Q.front();
      Q.pop();
      enumerate_quotient(c, [&](int q, int, int) {
        if (c + q <= TEN(3) and dp[c + q] == -1) {
          dp[c + q] = dp[c] + 1;
          Q.push(c + q);
        }
      });
  ...
  while (T--) {
    ...
    rep(i, N) {
      ...
      int v = c[i];
      repr(j, K + 1) {
        if (dp2[j] != -inf and j + w <= K) {
          amax(dp2[j + w], dp2[j] + v);
        }
      }
    }
    out(Max(dp2));
