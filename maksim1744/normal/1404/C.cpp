...
struct segtree {
    ...
    pair<int, int> left_max(int l, int r) {
        if (l > r) return {-1, -1};
        auto it = ask_item(l, r);
        return mp(it.ind, it.mx);
    }
};
...
int main() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        tree.set(i, val);
        while (true) {
            auto [ind, val] = tree.left_max(i, n - 1);
            if (ind == -1 || val < 0) break;
            done.insert(ind);
            tree.set(ind, -inf);
            tree.add(ind + 1, n - 1, 1);
        }
        // show(i, done);
        for (auto [y, ians] : ask[i]) {
            auto it = done.lower_bound(n - y);
            ans[ians] = done.order_of_key(*it);
        }
    }
    for (auto k : ans) {
        cout << k << '\n';
