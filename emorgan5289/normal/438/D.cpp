...
struct segtree {
    ...
    struct node { T s, l, r; int p; } t[2*_N];
    node merge(node x, node y) {
        return {x.s+y.s, std::min(x.l, y.l), std::max(x.r, y.r), x.r < y.r ? y.p : x.p};
    }

    T u[2*_N]; bool f[2*_N]; int a[2*_N], b[2*_N];
    segtree() {
        fill(t, t+2*_N, node({0, 0, 0, 0}));
        for (int i = 0; i < _N; i++)
            t[i+_N].p = i;
    ...
    node _qry(int l, int r, int i = 1) {
        ...
        if (l > b[i] || r < a[i]) return {0, max_T, min_T, -1};
