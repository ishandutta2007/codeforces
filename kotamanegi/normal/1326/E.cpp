...
void solve(){
    ...
    sort(ALL(inputs));
    LazySegmentTree<int> seg(n, [](int a, int b) {return max(a, b); }
    , [](int a, int b, int c) {return a + b; },
        [](int a, int b) {return a + b; }, 0LL, 0LL);
    int ans = 0;
    REP(i, n) {
        while (inputs.empty() == false && seg.query(0, n) <= 0) {
            seg.update(0,1 + inputs.back().second, 1);
            ans = inputs.back().first;
            inputs.pop_back();
        }
        cout << ans << " ";
        int a;
