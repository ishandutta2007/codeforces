...
unsigned long long m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000],c2[500000];string s2;vector<int>v,vr,v2[100003];unsigned long long sum,ans,num;char ch;
...
cin>>t;
while(t--){
    cin>>n>>k;vector<int> tmp;sum=0;
    for(int i=0;i<n;i++)
    {
     cin>>a[i];sum+=a[i];if(i==n-1&&tmp.size()==k-1&&sum%2)tmp.push_back(i+1);
     else if(tmp.size()==k-1)continue;
     else if(sum%2)tmp.push_back(i+1),sum=0;

     }
if(tmp.size()==k&&(sum%2)){cout<<"YES\n";for(int i=0;i<tmp.size();i++)cout<<tmp[i]<<" ";}
else cout<<"NO\n";
    }
