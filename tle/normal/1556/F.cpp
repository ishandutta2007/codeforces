...
//		a[i]=rand()+1;
	...
	for(int i=0;i<n;++i)
		for(int j=0;j<n;++j)
			edg[i][j]=a[i]*qp(a[i]+a[j],MOD-2)%MOD;
	for(int i=1;i<(1<<n);++i) {
		st[i]=1;
...
//		cerr<<i<<":"<<st[i]<<"\n";
	...
	for(int i=1;i<(1<<n);++i) {
		(f[i]+=st[i]*__builtin_popcount(i))%=MOD;
		for(int j=(i-1)&i;j;j=(j-1)&i) {
			ll b=f[j]*st[i-j]%MOD;
			for(int p=0;p<n;++p) if((j&(1<<p))){
				for(int q=0;q<n;++q) if((i-j)&(1<<q)) {
