...
int main() {
    ...
    vi qleft(N + 1);
    F0R (i, N) qleft[i+1] = qleft[i] + (str[i] == '?');

    vi qright(N + 1);
    F0Rd (i, N) qright[i] = qright[i+1] + (str[i] == '?');

    vi oleft(N + 1);
    F0R (i, N) oleft[i+1] = oleft[i] + (str[i] == '(');

    vi cright(N + 1);
    F0Rd (i, N) cright[i] = cright[i+1] + (str[i] == ')');

    auto ways = [](int L, int LF, int R, int RF) {
        if (L + LF < R || R + RF < L) return mn(0);
        return mn::ncr(LF + RF, RF + R - L);
    };
    ...
    F0Rd (i, N) {
        ...
        w0 += ways(oleft[i], qleft[i], cright[i], qright[i]);
        if (qleft[i]) w1 += ways(oleft[i]+1, qleft[i]-1, cright[i], qright[i]);
    }
