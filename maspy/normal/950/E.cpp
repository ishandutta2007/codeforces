...
void solve() {
  ...
  FOR(_, M) {
    INT(a, b);
    --a, --b;
    if ((A[a] + 1 - A[b]) % H == 0) G.add_edge(a, b);
    if ((A[b] + 1 - A[a]) % H == 0) G.add_edge(b, a);
  }
  SCC scc(G);
  auto comp = scc.comp_id;
  vi comp_sz(N);
  FOR(v, N) comp_sz[comp[v]]++;

  vi out_deg(N);
  FORIN(e, G.edges) {
    ll a = comp[e.frm], b = comp[e.to];
    if (a != b) ++out_deg[a];
  }
  ll ANS = N + 1;
  ll best_comp = -1;
  FOR(v, N) {
    ll a = comp[v];
    if (out_deg[a] > 0) continue;
    if (chmin(ANS, comp_sz[a])) best_comp = a;
  }
  print(ANS);
  vi C;
  FOR(v, N) if (comp[v] == best_comp) C.eb(v + 1);
  print(C);
}
