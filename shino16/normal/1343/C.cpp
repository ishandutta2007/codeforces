...
int main() {
  rep(IN) {
    ...
    auto a = IN.vll(n);
    VVLL grouped;
    grouped.push_back(VLL{a[0]});
    rep(i, 1, n)
      if (sgn(grouped.back().back()) == sgn(a[i])) grouped.back().push_back(a[i]);
      else grouped.push_back(VLL{a[i]});
    ll res = 0;
    for (auto&& v : grouped) res += *max_element(all(v));
    OUT(res);
  }
}