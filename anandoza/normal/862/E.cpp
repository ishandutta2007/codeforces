...
int main() {
    ...
    s.reserve(m - n + 1);
    {
        ...
        for (int i = n, parity = -1; i < m; i++, parity *= -1) {
            sum -= ((i - n) % 2 == 0 ? 1 : -1) * b[i - n];
            sum += (i % 2 == 0 ? 1 : -1) * b[i];
            s.push_back(parity * sum);
        }
    }
    ...
    auto calc = [&]() {
        ll ans = 1e18;
        auto it = lower_bound(all(s), target);
        if (it != s.end())
            ckmin(ans, abs(*it - target));
        if (it != s.begin())
            ckmin(ans, abs(*prev(it) - target));
        ps(ans);
    };
