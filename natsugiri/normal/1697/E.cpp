...
void MAIN() {
    ...
    REP (i, N) W[i] = 1LL<<60;
    REP (i, N) REP (j, N) if (i != j) amin(W[i], dist(i, j));

    UnionFind U(N);
    REP (i, N) REP (j, N) if (i != j && W[i] == W[j] && W[i] == dist(i, j)) {
	U.link(i, j);
    }
    ...
    REP (i, N) REP (j, N) if (i != j && U.same(i, j)) {
	LL d = dist(i, j);
	if (d != W[i] || d != W[j]) {
	    bad[U.root(i)] = true;
	}
