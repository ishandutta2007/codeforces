...
int main()
{
	// obviously it can be written faster, than O(N^2), but it fits in TL.
	uint32_t n = input<uint32_t>();
	vector<uint32_t> arr(n);
	std::generate(arr.begin(), arr.end(), input<uint32_t>);
	
	char direction = 1; // go right.
	uint32_t num_collected = 0;
	int32_t ans = -1;
	while (num_collected != n)
	{
		++ans;
		auto process = [&arr, &num_collected](uint32_t i) -> void
		{
			if (arr[i] <= num_collected)
				++num_collected, arr[i] = UINT32_MAX;
		};
