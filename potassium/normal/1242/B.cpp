...
vector<vector<int>> g;
set<int> notis;
inline isDisconnect(int u, int v) {
	auto it = lower_bound(all(g[u]),v);
	if (it != g[u].end() && *it == v) return true;
	return false;
}

inline int next(int cur) {
	auto it = notis.upper_bound(cur);
	if (it == notis.end()) return -1;
	else return *it;
}

inline int dfs(int node, int parent) {
	notis.erase(node);
	int cnt = 1;
	int cur = next(-1);
	while (cur != -1) {
		if (!isDisconnect(node,cur))
			cnt += dfs(cur,node);
		cur = next(cur);
	}
	return cnt;
...
int main() {
	...
	for (int i = 1; i <= n; i++) notis.insert(i);
	...
	for (int i = 1; i <= n; i++) {
		auto it = notis.lower_bound(i);
		if (it != notis.end() && *it == i) ans.pb(dfs(i,-1));
	}
