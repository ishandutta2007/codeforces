...
void solve() {
    ...
    vector<pair<string, int>> kouho;
    {
        string b(n, '0');
        b[0] = '1';
        kouho.push_back(mp(b, -1));
        REP(i, 100) {
            int ok = 0;
            for (int q = n - 1; q >= 0; --q) {
                if (b[q] == '1') {
    ...
    }
    {
        ...
        REP(i, 100) {
            ...
            if (ok == 0) break;
            kouho.push_back(mp(b, -(i*2) - 3));
        }
    }
    set<string> ans;
    REP(i, kouho.size()) {
        int gekos = kouho[i].second;
        REP(q, inputs.size()) {
            if (inputs[q] < kouho[i].first) {
                gekos++;
            }
            else if (inputs[q] == kouho[i].first) {
                gekos = 1e9;
                break;
            }
            else {
        ...
        if (gekos <= 1 and gekos >= 0) {
            ans.insert(kouho[i].first);
        }
    }
    cout << *(ans.begin()) << endl;
}
