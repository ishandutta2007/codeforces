...
void solve() {
  ...
  vc<vc<int>> IDS(K);
  FOR(x, H) FOR(y, W) IDS[A[x][y]].eb(W * x + y);
  ...
  auto calc = [&](ll k) -> void {
    ...
    auto upd = [&](ll v, ll w, ll cost) -> void {
      ll dw = dist[v] + cost;
      if (dw < dist[w]) {
        rm(w);
        dist[w] = dw;
    ...
    FOR(d, 2 * N) {
      ...
      while (v != -1) {
        if (v < H * W) {
          auto [x, y] = divmod(v, W);
          if (0 < x) upd(v, v - W, 2);
          if (x < H - 1) upd(v, v + W, 2);
          if (0 < y) upd(v, v - 1, 2);
          if (y < W - 1) upd(v, v + 1, 2);
          upd(v, A[x][y] + H * W, 1);
        } else {
          int k = v - H * W;
          for (auto&& to: IDS[k]) upd(v, to, 1);
        }
        v = nxt[v];
      }
    }
  ...
  FOR(k, K) {
    ...
    FOR(q, Q) {
      ...
      a = W * a + b;
      b = W * c + d;
      chmin(ANS[q], dist[a] + dist[b]);
    }
  }
  FOR(q, Q) print(ANS[q] / 2);
}
