...
public class C {
	...
	public int[] permut(int[] a, ArrayList<Integer> list) {
		int[] ans = new int[a.length];
		int last = a.length;
		int c = 0;
		for (int i = list.size() - 1; i >= 0; i--) {
			for (int j = last - list.get(i); j < last; j++) {
				ans[c++] = a[j];
			}
			last -= list.get(i);
		}
	...
	public boolean identical(int[] perm) {
