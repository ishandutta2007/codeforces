...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dpLPrecise = make_vector<Z>(n + 1, n + 1, Z());
		...
		for (int i: range(n)) {
			for (int j: range(n)) {
				//cerr << i << ' ' << j << ' ' << dpLPrecise[i][j] * dpR[n - i][j] << endl;
				ans += dpLPrecise[i][j] * dpR[n - i][j];
			}
		}
