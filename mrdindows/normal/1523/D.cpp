...
int solve() {
    ...
    mt19937_64 gen(time(0) + size_t(new char));
    const int K = 18;
    set<int> s;
    while (s.size() < n && s.size() < K) {
        int x = uniform_int_distribution<int>(0, n - 1)(gen);
        s.insert(x);
    }
    for (int idx : s) {
        ll mask = masks[idx];
        for (ll m = mask;; m = (m - 1) & mask) {
            candidates.push_back({countBits(m), m});
            if (m == 0) break;
        }
    }
    sort(all(candidates));
    ...
    for (auto c : candidates) {
        ...
        bool ok = true;
        for (int i = 0; i < m; ++i) {
            if (checkbit(x, i) && cntEach[i] < (n + 1) / 2) {
                ok = false;
            }
        }
        if (!ok) continue;
