...
class AAkhakhakhakhakhakhakhakha {
public:
    void solve() {
        int n = in.readInt();
        auto a = in.readIntArray(n);

        int zeroes = count(all(a), 0);
        int ones = n - zeroes;
        if (ones <= zeroes) {
            out.printLine(zeroes);
            out.printLine(arri(zeroes, 0));
            return;
        } else {
            ones -= ones % 2;
            out.printLine(ones);
            out.printLine(arri(ones, 1));
        }
    }
};
...
int main() {
    ...
    auto time = clock();
#endif
    AAkhakhakhakhakhakhakhakha solver;
