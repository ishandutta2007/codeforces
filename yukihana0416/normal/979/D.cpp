...
int find_max(int u, int d, int v, int b, bool is)
{
    if(d == -1) return 0;
    int c = (v >> d) & 1, r = -1;

    if(T[u][!c] && (!is || (is && !c <= ((b >> d) & 1))))
    {
        r = find_max(T[u][!c], d - 1, v, b, is && (!c == ((b >> d) & 1)));
        if(r != -1) r |= (1 << d);
    }
    if(r == -1 && T[u][c] && (!is || (is && c <= ((b >> d) & 1))))
    {
        r = find_max(T[u][c], d - 1, v, b, is && (c == ((b >> d) & 1)));
    }
    return r;
...
void work()
{
    ...
    while(n--)
    {
        ...
        else
        {
            ...
            {
                int r = find_max(rt[k], 20, x, s - x, 1);
                if(r != -1) r ^= x;
                printf("%d\n", r);
            }
        }
