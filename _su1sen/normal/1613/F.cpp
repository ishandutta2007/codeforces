...
int main() {
    ...
    auto comp = [&](const vector<mint> &p, const vector<mint> &q) {
        return p.size() > q.size();
    };
    auto pq = priqueue_comp<vector<mint>>(comp);
    rep(i, n) {
        pq.push({ 1, int(g[i].size()) - (i != 0) });
    }
    while (pq.size() >= 2) {
        auto f = move(pq.top());
        pq.pop();
        auto g = move(pq.top());
        pq.pop();
        pq.push(atcoder::convolution(f, g));
    }
    auto f = move(pq.top());
