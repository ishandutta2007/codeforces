...
int n,a[100010];
int minv[400010],addv[400010],cnt[400010];

inline void build(int o,int l,int r)
{
	minv[o]=INF;cnt[o]=r-l+1;
	int mid=(l+r)/2;
	if(r>l){
		build(o*2,l,mid);
		build(o*2+1,mid+1,r);
...
inline void pushdown(int o)
{
	...
	minv[o*2]+=addv[o];minv[o*2+1]+=addv[o];
	addv[o]=0;
}
...
inline void pushup(int o)
{
	...
	cnt[o]=0;
	if(minv[o]==minv[o*2]) cnt[o]+=cnt[o*2];
	if(minv[o]==minv[o*2+1]) cnt[o]+=cnt[o*2+1];
}
...
{
	if(x<=l&&r<=y) return make_pair(minv[o],cnt[o]);
	if(r<x||l>y) return make_pair(INF*2,0);
	pair<int,int> res=make_pair(INF*2,0);
	pushdown(o);
	int mid=(l+r)/2;
...
signed main()
{
	...
	for(int i=2;i<=n;i++){
		...
		if(query(1,1,n+1,a[i],a[i]).first==now.first){
