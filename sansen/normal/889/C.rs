...
// 
// k 
//  sum_{i = k + 1}^{n - 1} perm(i - 1, k) * fact(n - k - 1)
//  sum_{i = k + 2}^{n - 1} perm(i - 1, k + 1) * fact(n - k - 2)
// ..
// i >= k 
//
// sum_{i = k + 1}^(n - 1) perm(i - 1, k)
// 1..k + 2..(k+1) + ...
// 
// 
//
// M 
// sum_{i = 1}^{N} binom(N - 1, i - 1) * F(i) * fact(N - i)
// 
// 
// K
// F(n) = sum_{i=1)^{k.min(i)} binom(n - 1, i - 1) * F(n - i) * fact(i - 1)
// F(n) = sum_{i=1)^{k.min(n)} perm(n - 1, i - 1) * F(n - i)
// N^2
// 
//
...
fn run() {
    ...
    let pc = precalc::Precalc::new(1000000);
    if k + 1 >= n {
        println!("0");
        return;
    }
    let mut dp = vec![M::one(); n + 1];
    let mut sum = M::zero();
    for i in 1..=n {
        sum = M::from(i - 1) * sum + dp[i - 1];
        if i > k {
            sum -= pc.perm(i - 1, k + 1 - 1) * dp[i - k - 1];
        }
        dp[i] = sum;
    }
    let mut way = M::zero();
    for i in 1..=n {
        way += pc.comb(n - 1, i - 1) * dp[i - 1] * pc.fact(n - i);
    }
    let ans = pc.fact(n) - way;
    println!("{}", ans);
}
