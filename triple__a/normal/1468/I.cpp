...
signed main(){
    ...
    int x,y,g=__gcd(dx1,dx2);
    if (g==0){
        cout<<"NO\n"; return 0;
    }
    basics::exgcd(dx1,dx2,x,y);
    int nw=dy1*x+dy2*y, mv=(x-dx2/g)*dy1+(y+dx1/g)*dy2-nw;
    if (n!=abs(mv)*abs(__gcd(dx1,dx2))){
        cout<<"NO\n";
        return 0;
    }
    cout<<"YES\n";
    rep(i,abs(__gcd(dx1,dx2))) rep(j,abs(mv)) cout<<i<<" "<<j<<endl;
    return 0;
}