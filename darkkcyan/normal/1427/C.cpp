...
struct celeb {
    ...
    int r, c;
    celeb() {}
    celeb(int t, int r_, int c_): time(t), r(r_), c(c_) {}
};
...
void read() {
    cin >> max_range >> n;
    rep1(i, n) cin >> cel[i].time >> cel[i].r >> cel[i].c;
}
...
ans_type solve() {
    ...
    int cur_mx = -n - 10;
    rep1(i, n) {
        while (cel[i].time - cel[f].time > 1000) {
            cur_mx = max(cur_mx, dp[f]);
            ++f;
        }
        dp[i] = cur_mx;
        for (int g = f; g < i; ++g) {
            if (abs(cel[g].r - cel[i].r) + abs(cel[g].c - cel[i].c) <= cel[i].time - cel[g].time)
                dp[i] = max(dp[i], dp[g]);
        }
        ++dp[i];
