...
int main() {
    ...
    auto ch = [&](long double x) {
        //cout << fixed << setprecision(3);

        //cout << x << endl;
        vector<vector<long double>> dp(N + 1, vector<long double>(R + 1, -1));

        rep(j, R + 1)dp[N][j] = 0;

        for (ll i = N - 1; i >= 0; i--)rep(j, R + 1) {
            if (j + F[i] > R || abs(dp[i + 1][j + F[i]]+1)<eps) { continue; }
            dp[i][j] = P[i] * (dp[i + 1][j + F[i]]+F[i]);

            long double now = x;
            if (j + S[i] <= R && abs(dp[i + 1][j + S[i]]+1)>eps) { now = min(now, dp[i + 1][j + S[i]]); }
            dp[i][j] += (1 - P[i]) * (now+S[i]);
        }

       //rep(i, N + 1) {
        //    rep(j, R + 1)cout << dp[i][j] << " ";
         //   cout << endl;
        //}

        if (dp[0][0] < x) { return true; }
        else { return false; }
    };

   
    long double ri = INF, lef = 0;

    rep(i, 200) {
        long double m = (ri + lef) / 2;
        if (ch(m)) { ri = m; }
        else { lef = m; }
    }
