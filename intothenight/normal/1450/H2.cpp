...
int main(){
	...
	int t = b[1] + r[1] - b[0], rem = r[0] + r[1];
	Zp pref = 0; // sum{0<=i<=t}( rem choose i )
	auto inc_t = [&](){
		++ t;
		if(t >= 0) pref += C.C(rem, t);
	};
	auto dec_t = [&](){
		if(t >= 0) pref -= C.C(rem, t);
		t --;
	...
	auto inc_rem = [&](){
		if(t >= 0) pref = 2 * pref - C.C(rem, t);
		++ rem;
	};
	auto dec_rem = [&](){
		-- rem;
		if(t >= 0) pref = (pref + C.C(rem, t)) / 2;
	};
	for(auto i = 0; i <= t; ++ i){
		pref += C.C(rem, i);
	}
	auto report = [&](){
		Zp res = rem * (rem >= 2 ? Zp(2) ^ rem - 2 : Zp(abs(t % 2))) - t * (Zp(2) ^ rem - 1);
		if(t >= 0){
			dec_rem();
			res += 2 * t * pref;
			if(!rem){
