...
public class Main
{
	...
	{
		int n=ni(),l=ni();
		int[]x=new int[n+1],b=new int[n+1];
		for(int i=1; i<=n; i++) {
			x[i]=ni();
			b[i]=ni();
		}
		double lo=0,hi=1e3,mi=(lo+hi)/2;
		...
		while(lo+1e-8<hi) {
			...
			for(int i=0; i<=n; i++)
				for(int j=i+1; j<=n; j++)
					adj[i][j]=Math.sqrt(Math.abs(x[j]-x[i]-l))-mi*b[j];
			for(int i=0; i<=n; i++)
				for(int j=i+1; j<=n; j++)
					min[j]=Math.min(min[j],min[i]+adj[i][j]);
			if(min[n]<0)
				hi=mi;
			else
				lo=mi;
