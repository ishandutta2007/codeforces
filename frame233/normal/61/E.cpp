...
struct BIT2{
	int c[N],n;
	Finline void add(int x,const int &C){for(;x;x-=lowbit(x))c[x]+=C;}
...
int main()
{
	...
	for(int i=1;i<=n;++i)
	{
		a[i]=std::lower_bound(t+1,t+n+1,a[i])-t;
	}
	tr1.n=tr2.n=n;
	for(int i=1;i<=n;++i) tr1.add(a[i],1);
	ll ans=0;
	for(int i=1;i<=n;++i)
	{
		tr1.add(a[i],-1);
		ans+=1ll*tr1.sum(a[i])*tr2.sum(a[i]);
		tr2.add(a[i],1);
	}
	printf("%lld\n",ans);
    return 0;
