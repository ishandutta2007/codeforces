...
int main(void)
{
	...
	rep(i, 1, n){
		if(s[i][0] == 'W') s[i][0] = 'B';
		else if(s[i][0] == 'B') s[i][0] = 'W';
	}
	bool bflag = false, wflag = false;
	rep(i, 1, n){
		if(s[i][0] == 'B') bflag = true;
		if(s[i][1] == 'B') bflag = true;
		if(s[i][0] == 'W') wflag = true;
		if(s[i][1] == 'W') wflag = true;
	}
	mint ans = 0;
	if(!bflag) ans += 1;
	if(!wflag) ans += 1;
	
	ll bw = 0, wb = 0, lcnt = 0, rcnt = 0, cnt = 0;
	rep(i, 1, n){
		if(s[i] == "BW") bw++;
		if(s[i] == "WB") wb++;
		if(s[i] == "B?" || s[i] == "?W") lcnt++;
	...
	rep(i, 0, cnt){
		ll L = lcnt+i, R = rcnt+cnt-i;
		if(bw <= wb){
			if(L >= wb-bw) ans += comb(cnt, i) * comb(L+R, wb-bw+R);
		}
		else{
			if(R >= bw-wb) ans += comb(cnt, i) * comb(L+R, R-(bw-wb));
		}
		//outl(ans);
		if(bw+wb == 0) ans -= comb(cnt, i);
		//outl(i, ans);
	}
	outl(ans);
