...
for(int i=0;i<n;i++){cin>>b[i];mulset.insert(b[i]);}
for(int i=0;i<n;i++)
{
    if(a[i]==0)
    {
        auto it=mulset.lower_bound(0);
        v.push_back(*it);
        mulset.erase(it);
    }
    else
    {
       auto it=mulset.lower_bound(n-a[i]);

if(it==mulset.end()){auto it2=mulset.lower_bound(0);v.push_back((a[i]+(*it2))%n);mulset.erase(it2);}
        else{auto it2=mulset.lower_bound(0);
        if((a[i]+(*it))%n<(a[i]+(*it2))%n){v.push_back((a[i]+(*it))%n);mulset.erase(it);}
        else
            {v.push_back((a[i]+(*it2))%n);mulset.erase(it2);}}
