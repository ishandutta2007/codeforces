...
int main() {
    ...
    /*auto it = Z_function("aaaaa");
    for (auto x: it) cout<<x<<' ';*/

    string s;
    cin>>s;
    auto it = Z_function(s);
    int n = it.size();
    int maxx = 0;
    for (int i = 1; i<n-1; i++) maxx = max(maxx, min(it[i], n-i-1));
    for (int i = 1; i<n; i++) if (it[i]==n-i && it[i]<=maxx)
    {
        cout<<s.substr(0, it[i]); return 0;
    }
    cout<<"Just a legend";
