...
int main(int argc, char **argv){
  ...
  auto lt=[&](int a,int b,int l){
    assert(a>=0 && b>=0 && a+l<=n && b+l<=n);
    int c=lcp[a][b];
    return c<l && s[a+c]<s[b+c];
  };

  const LL mod=INF+7;
  vector<vector<int>> D1(n+1,vector<int>(n+1));
  auto D2=D1;
  D2[1][1]=1;
  LL ans=0;
  forn(b,n+1){
    for(int l=1;l<=b;++l){
      (D2[b][l]+=D2[b-1][l-1]+(s[b-l]=='0'?0:D1[b-l][l-1]))%=mod;
      D1[b][l]=D2[b][l];
      if(D1[b-l][l] && s[b-l]!='0' && lt(b-l*2,b-l,l))
	(D1[b][l]+=D1[b-l][l])%=mod;
      if(b==n)
	(ans+=D1[b][l])%=mod;
    }
