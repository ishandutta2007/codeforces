...
void solve() {
  ...
  auto b = in.vec<int>(n);
  rep(i, n) b[i]--;
  int k = n-1; while (k >= 0 && b[k] < k) k--;
  out(k+1);
  vector<vector<int>> nxts(n+1);
  vector<int> a;
  rep(i, n) nxts[b[i] == -1 ? n : b[i]].push_back(i);
  int head = n;
  while (a.size() != n) {
    assert(!nxts[head].empty());
    if (nxts[nxts[head].back()].empty()) {
      a.push_back(nxts[head].back());
      nxts[head].pop_back();
    } else if (nxts[head].size() > 1) {
      rotate(nxts[head].begin(), nxts[head].end()-1, nxts[head].end());
    } else {
      a.push_back(nxts[head].back());
      int head2 = nxts[head].back();
      nxts[head].pop_back();
      head = head2;
    }
  }
