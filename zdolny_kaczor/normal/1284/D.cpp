...
ull val[nax];
int n;
int b1[nax], e1[nax], b2[nax], e2[nax];
using event = pair<int, pair <bool, int> >;
#define when first
#define Type second.first
#define which second.second
vector <ull> read(int b[], int e[]) {
	vector <ull> ans(n);
	REP(_, 2) {
		vector <event> events(2 * n);
		REP(i, n) {
			events[i * 2] = make_pair(b[i], make_pair(false, i));
			events[i * 2 + 1] = make_pair(e[i], make_pair(true, i));
		}
		sort(ALL(events));
		ull upto = 0;
		for (auto &ev : events) {
			if (ev.Type) { //end of segment
				debug("include", arr(val, ev.which));
				upto ^= val[ev.which];
			}
			else { //beg of segment
				debug("add to ans", imie(ev.which), imie(upto));
				ans[ev.which] ^= upto;
			}
		}
		REP(i, n) {
...
int main() {
	...
	scanf("%d", &n);
	REP(i, n) val[i] = rand();
	REP(i, n) debug(arr(val, i));
	REP(i, n) scanf("%d%d%d%d", b1 + i, e1 + i, b2 + i, e2 + i);
	if (read(b1, e1) == read(b2, e2))
		printf("YES\n");
	else
		printf("NO\n");
}