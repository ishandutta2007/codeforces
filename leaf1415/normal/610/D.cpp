...
struct DynamicSegTree{
	...
	T query(ll a, ll b, ll p, ll l, ll r)
	{
		if(b < l || r < a) return Ident();
		if(a <= l && r <= b) return seg[p].val;
		
		check(p);
		T lval = query(a, b, seg[p].left, l, (l+r)/2);
		T rval = query(a, b, seg[p].right, (l+r)/2+1, r);
...
ll calc(map<ll, vector<P> > &mp)
{
	ll ret = 0;
	for(auto &p : mp){
		vector<P> &vec = p.second, vec2;
		sort(all(vec));
		
		ll cnt = 0, pre;
		for(auto q : vec){
			if(q.second == 1){
				if(cnt == 0) pre = q.first;
				cnt++;
			}
			else{
				cnt--;
				if(cnt == 0){
					vec2.push_back(P(pre, 1));
					vec2.push_back(P(q.first, -1));
					ret += q.first - pre;
				}
			}
		}
...
ll n;
map<ll, vector<P> > xmp, ymp;
DynamicSegTree seg(31);
ll B = 1000000005;
...
int main(void)
{
	...
	rep(i, 1, n){
		...
		if(y == y2){
			if(x > x2) swap(x, x2);
			xmp[y].push_back(P(x, 1));
			xmp[y].push_back(P(x2+1, -1));
		}
		else{
			if(y > y2) swap(y, y2);
			ymp[x].push_back(P(y, 1));
			ymp[x].push_back(P(y2+1, -1));
		}
	}
	
	ll ans = calc(xmp) + calc(ymp);
	
	typedef pair<ll, P> E;
	vector<E> evec;
	for(auto p : ymp){
		vector<P> &vec = p.second;
		for(auto q : vec) evec.push_back(E(q.first, P(q.second, p.first)));
	}
	for(auto p : xmp){
		vector<P> &vec = p.second;
		for(auto q : vec) evec.push_back(E(p.first, P(-q.second+10, q.first)));
	}
	sort(all(evec));
	
	for(auto e : evec){
		ll t = e.second.first, p = e.second.second;
		if(abs(t) == 1) seg.add(p, t);
		else ans += sgn(-(t-10)) * seg.query(0, p-1);
	}
	outl(ans);
