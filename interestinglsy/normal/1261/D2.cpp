...
ll CalcEq(){
	...
	For(i,n){
		int nxt = a[ i == n ? 1 : i+1 ];
		if( a[i] != nxt ) ++diffpos_cnt;
	}
	ll ans = 0;
	Forx(i,0,diffpos_cnt/2){
		ll tans = C(diffpos_cnt,i) * C(diffpos_cnt-i,i) % MOD * Pow(k-2,diffpos_cnt-i-i) % MOD;
		Addi(ans,tans);
	}
	Muli( ans , Pow(k,n-diffpos_cnt) );
	return ans;
}
