...
int main() {
    ...
    int n;
    cin >> n;
    vector<vector<pair<int, int>>> IN(MAX_C), OUT(MAX_C);
    vector<int> l(n), v(n), r(n);
    for (int i = 0; i < n; i++) {
        cin >> l[i] >> v[i] >> r[i];
        IN[l[i]].emplace_back(v[i], r[i]);
        OUT[v[i]].emplace_back(v[i], r[i]);
    }

    auto f = [](int a, int b) { return min(a, b); };
    auto g = [](int a, int b) { return a + b; };
    LazySegmentTree<int, int> seg(MAX_C, f, g, g, INF, 0);
    vector<int> init(MAX_C, 0);
    seg.build(init);
    int Max = 0, L = -1, R = -1;

    for (int i = 0, cur = 0; i < MAX_C; i++) {
        for (auto& p : IN[i]) {
            int v = p.first, r = p.second;
            seg.update(r + 1, MAX_C, 1);
            seg.update(0, v, 1);
            cur++;
        }

        int x = seg.query(i, MAX_C);
        if (Max < cur - x) {
            Max = cur - x;
            L = i;
            R = seg.find_first(i, [&](int val) { return val == x; });
        }
