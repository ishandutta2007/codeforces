...
fun Long.choose2():Long{
    return 1L*  (this) * (this-1) / 2
}
fun Long.choose3():Long{
    return 1L*  (this) * (this-1) * (this-2) / 6
}
fun Long.choose4():Long{
    return 1L*  (this) * (this-1) * (this-2) * (this - 3)/ 24
}
fun Int.choose2():Long{
    return 1L*  (this) * (this-1) / 2
}
fun Int.choose3():Long{
    return 1L*  (this) * (this-1) * (this-2) / 6
}
fun Int.choose4():Long{
    return 1L*  (this) * (this-1) * (this-2) * (this - 3)/ 24
}
typealias OCSTkeytype = Int
typealias OCSTvaluetype = Int
class OCST(val key:OCSTkeytype, var value:OCSTvaluetype = 1){
    // see https://codeforces.com/blog/entry/83170
    var left:OCST? = null
    var right:OCST? = null


    var vstore:OCSTvaluetype = 1
    ...
    fun query(lo:OCSTkeytype, hi:OCSTkeytype, L:OCSTkeytype = min, R:OCSTkeytype = max):OCSTvaluetype{
        if(hi < L || R < lo) return empty
        if(lo <= L && R <= hi) return vstore
        val M = (L+R) shr 1
        var ans = empty
        if(left != null && lo <= M) ans = combine(ans,left!!.query(lo,hi,L,M))
        if(right != null && hi >= M+1) ans = combine(ans,right!!.query(lo,hi,M+1, R))
    ...
    companion object{
        const val empty:OCSTvaluetype = 0
        //pay attention , is long!
        const val min:OCSTkeytype = -(1 shl 30)
        const val max:OCSTkeytype = (1 shl 30) - 1
        fun insert(root:OCST?, i:OCSTkeytype, v:OCSTvaluetype = 1):OCST?{
            return merge(root,OCST(i,v) ,min,max)
        }
        fun combine(a: OCSTvaluetype, b: OCSTvaluetype): OCSTvaluetype {
            return a + b
        }

        fun merge(T1:OCST?, T2:OCST?, l:OCSTkeytype, r:OCSTkeytype):OCST?{
        ...
        tailrec fun kthvalue(node:OCST, k:OCSTkeytype, L: OCSTkeytype = min, R: OCSTkeytype = max): OCSTkeytype? {
            //find the k-th value , 0 indexed
            val M = (L + R ) shr 1
            var k = k
            if(k == 0){
                return node.key
            ...
            k --
            val left = node.left?.vstore ?: 0
            if(k < left){
                return kthvalue(node.left!!,k,L,M)
            }else{
                return kthvalue(node.right!!,k-left,M+1,R)
            }
        }
        tailrec fun firstEmptyValue(node:OCST, atleast:OCSTkeytype = min, L:OCSTkeytype = min, R:OCSTkeytype = max):OCSTkeytype?{
            if(node.vstore  == (R -L +1)) return null
            if(atleast !in L..R) return null
            if(node.key != atleast) return atleast
            val M = (L+R) shr 1
            val leftexpects = maxOf(M - atleast,0)
            if((node.left?.vstore ?: 0) != leftexpects){
                if(node.left == null) return (atleast + 1)
                return firstEmptyValue(node.left!!,atleast + 1, L, M)
            }else{
                if(node.right == null) return maxOf(atleast + 1,M+1)
                return firstEmptyValue(node.right!!, maxOf(atleast + 1,M+1), M+1, R)
            }
        }
        tailrec fun firstEmptyValueRange(node:OCST,lo:OCSTkeytype, hi:OCSTkeytype, L:OCSTkeytype = min, R:OCSTkeytype = max):OCSTkeytype?{
            if(hi < L || R < lo) return null
            if(lo <= L && R <= hi){
                return firstEmptyValue(node,L,L,R)
            }
            val M = (L+R) shr 1
            if(node.left != null && lo <= M && node.left!!.vstore != (R-L+1) shr 1) return firstEmptyValueRange(node.left!!,lo,hi,L,M)
            if(node.right != null && hi >= M+1) return firstEmptyValueRange(node.right!!,lo,hi,M+1,R)
            return null
        }
    }
    fun printall(): TreeMap<OCSTkeytype, OCSTvaluetype> {
        val ret = TreeMap<OCSTkeytype,OCSTvaluetype>()
        ret[key] = value
...
class OCSThandler(){
    var root:OCST? = null
    fun query(l:OCSTkeytype, r:OCSTkeytype): OCSTvaluetype {
        return root?.query(l,r) ?: OCST.empty
    }
    fun add(i:OCSTkeytype, v:OCSTvaluetype = 1){
        root = OCST.insert(root, i, v)
    }
    fun contains(l:OCSTkeytype, r:OCSTkeytype): Boolean {
        return (root?.query(l, r) ?: 0) >= 1
    }
    fun print(){
        if(root == null){
            println("Empty CST")
...
class rsqArr(val arr:LongArray) {
    val ps = LongArray(arr.size + 1)
    init{
        for(i in 0 until arr.size){
            ps[i+1] = ps[i] + arr[i]
...
fun main(){
    solve.cases{
		...
		val q = getint
		val str = getstr
        val balance = IntArray(n+1)
        var now = 0
        balance[0] = 0
        val shift = n + 1
        val lastfound = IntArray(2 * shift + 1){-1}
        lastfound[0 + shift ] = 0

        val leftPair = IntArray(n+1){-1}
        val rightPair = IntArray(n+1){-1}
        val parent = IntArray(n+1){-1}
        for((i,v) in str.withIndex()){
            if(v == '(') {
                now ++
            }else{
                lastfound[now + shift] = - 1
                now --
            }
            if(lastfound[now + shift] != -1 ){
                val found = lastfound[now + shift]
                leftPair[i+1] = found
                rightPair[found] = i+1
                if(lastfound[now -1 + shift] != -1){
                    parent[found] = lastfound[now-1 + shift]
                }
            }
            lastfound[now + shift] = i+1
            balance[i+1] = now
        }
        val G = Graph(n+1,n,true)
        val od = IntArray(n+1)
        for(i in 0..n){
            if(parent[i] != -1) {
                G.add(parent[i],i)
        ...
        val sacks = LongArray(n+1){(od[it]+1).choose2()}
        val S = longSumSimple(sacks)


        val occur = Array(shift * 2 + 1 ){OCSThandler()}
        for(i in 0..n){
            occur[balance[i] + shift].add(i,1)
        }

        fun delete(x:Int){
            val b = balance[x]
            occur[b + shift].add(x,-1)
        }
        ...
        repeat(q){
            ...
            if(t == 2){
                val x = getint
                val y = getint
                val sums = S.query(x-1,y-1)
                val invs = occur[balance[x-1] + shift].query(x-1,y-1) + 1
                put(sums + invs.choose2())
            }else{
                val x = getint
                val y = getint
                if(parent[x-1] != -1){
                    od[parent[x-1]] --
                    val v =od[parent[x-1]] + 1
                    S.setValue(parent[x-1], v.choose2())
                }
...
/*
keypoitns follow left or follow right?
 */