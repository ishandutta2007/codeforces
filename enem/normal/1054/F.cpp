...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        for(auto &it:mpx)
        {
            ...
            REP(i,0,nv-1)
            {
                vx.PB({{it.F, it.S[i]}, {it.F, it.S[i+1]}});
            }
        }
        for(auto &it:mpy)
        {
            ...
            REP(i,0,nv-1)
            {
                vy.PB({{it.S[i], it.F}, {it.S[i+1], it.F}});
            }
        }
        ...
        {
            cout<<mpy.size()<<'\n';
            for(auto it:mpy) cout<<it.S.front()<<' '<<it.F<<' '<<it.S.back()<<' '<<it.F<<'\n';
            cout<<mpx.size()<<'\n';
            for(auto it:mpx) cout<<it.F<<' '<<it.S.front()<<' '<<it.F<<' '<<it.S.back()<<'\n';
            return 0;
        }
        V(ll) ad[nx];
        REP(i,0,nx)
        {
            REP(j,0,ny)
            {
                if(vy[j].F.F<vx[i].F.F && vx[i].F.S<vy[j].F.S && vx[i].S.F<vy[j].S.F && vy[j].S.S<vx[i].S.S)
                {
                    ad[i].PB(j);
                }
            }
...
// cout<<"mpx\n";
// for(auto it:mpx) {cout<<it.F<<'\t'; for(auto itt:it.S) cout<<itt<<' ';cout<<'\n';};
// cout<<"mpy\n";
// for(auto it:mpy) {cout<<it.F<<'\t'; for(auto itt:it.S) cout<<itt<<' ';cout<<'\n';};
// cout<<"vx\n";
// for(auto it : vx) cout<<it.F.F<<' '<<it.F.S<<' '<<it.S.F<<' '<<it.S.S<<'\n';
// cout<<"vy\n";
// for(auto it : vy) cout<<it.F.F<<' '<<it.F.S<<' '<<it.S.F<<' '<<it.S.S<<'\n';
// cout<<"ad\n";
// REP(i,0,nx) {cout<<i<<'\t';for(auto it:ad[i]) cout<<it<<' ';cout<<'\n';}
// cout<<endl;
        ll mchy[ny], mchx[nx];
        REP(i,0,nx) mchx[i] = -1;
        REP(i,0,ny) mchy[i] = -1;
        ...
        while(true)
        {
            ...
            while(!bfs.empty())
            {
                ...
                if(rchx != -1 && rchy != -1) break;
            }
// cout<<"rchx "<<rchx<<'\n';
// cout<<"rchy "<<rchy<<'\n';
// cout<<"par\n";
// REP(i,0,nx) cout<<par[i]<<' ';cout<<'\n';
            if(rchx == -1 && rchy == -1) break;
            while(rchx != -2 || rchy != -1)
            {
                mchy[rchy] = rchx;
...
// cout<<"mchx\n";
// REP(i,0,nx) cout<<mchx[i]<<' ';cout<<'\n';
// cout<<"mchy\n";
// REP(i,0,ny) cout<<mchy[i]<<' ';cout<<'\n';
// cout<<endl;
        }
        bool rchy[ny];
        REP(i,0,ny)
        ...
        vector<pair<pll,pll>> ansx, ansy;
        ll pt = 0;
        for(auto &it:mpx)
        {
            ll nv = it.S.size();
            ansx.PB({{it.F, it.S[0]}, {it.F, it.S[0]}});
            REP(i,0,nv-1)
            {
                if(mchx[pt] == -1 || rchy[mchx[pt]])
                {
                    ansx.back().S.S = it.S[i+1];
                }
                else
                {
                    ansx.PB({{it.F, it.S[i+1]}, {it.F, it.S[i+1]}});
                }
                pt++;
            }
        }
        ...
        {
            ll nv = it.S.size();
            ansy.PB({{it.S[0], it.F}, {it.S[0], it.F}});
            REP(i,0,nv-1)
            {
                if(!rchy[pt])
                {
                    ansy.back().S.F = it.S[i+1];
                }
                else
                {
                    ansy.PB({{it.S[i+1], it.F}, {it.S[i+1], it.F}});
                }
                pt++;
            }
        }
        cout<<ansy.size()<<'\n';
        for(auto it : ansy) cout<<it.F.F<<' '<<it.F.S<<' '<<it.S.F<<' '<<it.S.S<<'\n';
        cout<<ansx.size()<<'\n';
        for(auto it : ansx) cout<<it.F.F<<' '<<it.F.S<<' '<<it.S.F<<' '<<it.S.S<<'\n';
    }
