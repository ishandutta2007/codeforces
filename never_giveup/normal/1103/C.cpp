...
const int maxn = 25e4 + 100, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 100, X = 43, LG = 17;
...
void go(int v, vector<int> &w) {
    if (v == -1)
        return;
    w.push_back(v);
    for (int i : e[v])
    if (h[i] == h[v] + 1 && d[v] == d[i] + 1) {
        go(i, w);
        break;
    }
}
...
void dfs(int v, int par) {
    ...
    if ((x == -1 ? 0 : d[x]) + (y == -1 ? 0 : d[y]) + 1 >= (n + kek - 1) / kek) {
        cout << "PATH\n";
        vector<int> o;
        go(x, o);
