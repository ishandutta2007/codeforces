...
int main(){
  ...
  vec firstch=dijkstra(fg,0);
  // debug(firstch);
  K=min(firstch[N-1],firstch.back());
  if(K/mod>=20){
    print((K%mod-1+modpow(2,K/mod))%mod);
    return 0;
  }
  pmat sg(N*20);
  rep(i,N){
    rep(j,20){
      if(j<19)sg[i*20+j].eb(1<<j,i*20+j+1);
      for(auto& x:((j&1)?rg:g)[i]){
        sg[i*20+j].eb(1,x*20+j);
      }
    }
  }
  debug(sg);
  vec v=dijkstra(sg,0);
  rep(i,20)chmin(K,v[N*20-i-1]);
  print(K);
  // }
}