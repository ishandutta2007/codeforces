...
void solve(){
  ...
  map<ll,vector<ll>> m;
  vector<ll> ans(a,inf);
  for(int i=1;i<=a;i++) m[i].pb(-1);
  for(int i=0;i<a;i++) m[n[i]].pb(i);
  for(int i=1;i<=a;i++) m[i].pb(a);
  for(ll i=1;i<=a;i++){
    ll now=0;
    for(int j=0;j<m[i].size()-1;j++){
      chmax(now,m[i][j+1]-m[i][j]);
    }
    if(now-1<a) chmin(ans[now-1],i);
  }
  for(int i=1;i<a;i++) chmin(ans[i],ans[i-1]);
  for(int i=0;i<a;i++){
    if(ans[i]==inf) cout<<-1<<" ";
    else cout<<ans[i]<<" ";
  }
