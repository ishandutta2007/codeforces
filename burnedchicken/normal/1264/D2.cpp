...
inline modint C(int n, int m){return n<0||m<0||m>n?modint(0):fact[n]*invfact[m]*invfact[n-m];}
...
signed main(){
    ...
    rep2(i,2,maxn) invfact[i]=invfact[MOD%i]*modint(MOD-MOD/i);
    ...
    rep(n){
        ...
        ans+=modint(cntl)*C(cnt1+cnt2,cnt2-cntl+cntr)+modint(cnt1)*C(cnt1+cnt2-1,cnt2-cntl+cntr-1);
    }
    print(ans);
    return 0;
}