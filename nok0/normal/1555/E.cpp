...
void main_() {
	...
	REP(i, n) {
		auto [l, r, x] = segs[i];
		seg.update_add(l, r, 1);
		if(seg.query_min(0, m) >= 1) {
			auto [y, z, w] = segs[bef];
			int l2 = y, r2 = z, ig = w;
			while(seg.query_min(l2, r2) > 1) {
				seg.update_add(l2, r2, -1);
				bef++;
				auto [y, z, w] = segs[bef];
				l2 = y, r2 = z, ig = w;
			}
			debug(bef, x);
			debug(get<2>(segs[bef]));
			chmin(res, x - get<2>(segs[bef]));
		}
	}
	print(res);
}
