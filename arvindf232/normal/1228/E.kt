...
//        for(i in 1..n){
//            for(j in 1..n){
                var ret = intPow(k,i*j,pI)
                for(k1 in 0..i){
                    for(k2 in 0..j){
                        if(k1 + k2 == 0) continue
                        val parity = if((k1 + k2) % 2 == 0) 1 else pI - 1
                        val c = FACT.choose(i,k1) modM FACT.choose(j,k2) modM parity modM intPow(k,(i-k1)* (j-k2), pI) modM intPow(k-1,i*j-(i-k1)* (j-k2),pI)
                        ret = (ret + c).adjust()
                    }
                }
                DP[i][j] = ret
