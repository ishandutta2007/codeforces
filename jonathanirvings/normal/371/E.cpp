...
int main()
{
	...
	SORTA(data+1,n);
	jum[0] = 0;
	REPN(i,n) jum[i] = jum[i-1] + data[i].fi;
	FORN(i,2,k) res += ((LL)data[i].fi * (i - 1) - jum[i-1]);
	risan = mp(res,1);
	FORN(i,2,n-k+1)
	{
		res -= ((LL)(jum[i+k-2] - jum[i-1]) - (LL)data[i-1].fi * (k - 1));
		res += ((LL)((LL)data[i+k-1].fi * (k-1)) - (LL)(jum[i+k-2] - jum[i-1]));
		MIN(risan,mp(res,i));
	}
	REP(i,k) printf("%d ",data[risan.se + i].se);
	puts("");
	return 0;
}