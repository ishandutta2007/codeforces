...
namespace MAIN{
    ...
    int a[26];
    inline void MAIN(){
        for(res i=0;i<26;i++)a[i]=0;
        scanf("%s",str+1),scanf("%s",T+1);
        n=int(strlen(str+1));
        for(res i=1;i<=n;i++)a[str[i]-'a']++;
        res x=T[1]-'a',y=T[2]-'a',z=T[3]-'a';
        if(x<=y&&y<=z){
            if(a[0]){
                for(res i=0;i<26;i++){
                    if(i!=1&&i!=2)for(res j=1;j<=a[i];j++)putchar(i+'a');
                    else if(i==1)for(res j=1;j<=a[2];j++)putchar(2+'a');
                    else for(res j=1;j<=a[1];j++)putchar(1+'a');
                }
                puts("");
            }
            else {
                for(res i=0;i<26;i++)for(res j=1;j<=a[i];j++)putchar(i+'a');
                puts("");
            }
        }
        else {
