...
fun main(){
    ...
    solve.cases{
        ...
        var base = intPow(2,n-1,pI)

        val allsizes = IntArray(n+3)
        for(i in 1 until n){
            val s = sizes[i]
            allsizes[s] ++
            allsizes[s-1] ++
        ...
        val doable = IntArray(n+3)
        for(i in 2 until doable.size){
            var here = 0
            for(j in 0 until doable.size step i ){
                here += allsizes[j]
            }
            doable[i] = here
        }
        val trueret = IntArray(n+3)
        for(i in 2 until trueret.size) {
            assert( doable[i] <= n- 1 )
            if(doable[i] >= n-1 && (n-1) % i == 0 ){
                assert( i <= n )
                trueret[i] = 1
            }
        }
...
//            }
            var ret = 0
            for(j in 1..n){
                if(i * j !in trueret.indices){
                     break
                }
                ret += sieve.mobius[j] * trueret[i * j ]
            }
