...
int main(){
    ll a=readll(),b=readll(),c=std::min(b,readll()),d=readll(),e=readll(),f=readll();a=std::min(d,a),c=std::min(d,c);
    printf("%lld\n",std::min(a,d-c)*e+std::min(c,d-a)*f+(a-std::min(a,d-c))*std::max(e,f));return 0;
}