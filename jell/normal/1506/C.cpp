#line 1 "codeforces-workspace/contest/1506/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1506/c/c.cpp"
...
void main() {
  ...
  for (auto i : range(a.size() + 1)) {
    for (auto j : range(i + 1)) {
      for (auto k : range(b.size() + 1)) {
        for (auto l : range(k + 1)) {
          if (a.substr(j, i - j) == b.substr(l, k - l)) {
            chle(ans, j + l + a.size() - i + b.size() - k);
          }
        }
      }
    }
