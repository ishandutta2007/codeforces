...
struct segtree{
  ...
  void grab(int a,int b,int k,vector<int>&out){
    if(b==a+1){
      out[a]=T[k].x?-1:1;
      return;
    }
  ...
  void init(int a,int b,int k,vector<pii>&in){
    if(b==a+1){
      T[k].x=in[a].X<0;
      return;
    }
...
int main(int argc, char **argv){
  ...
  sort(all(A),[](pii a,pii b){return abs(a.X)<abs(b.X);});
  segtree T(n);
  T.init(0,n,0,A);
  vector<int> B(n);
  forn(i,n) B[i]=abs(A[i].X);
  ...
  forn(qq,tc){
    ...
    if(x>=0){
      if(ch=='>'){
        int a=lower_bound(all(B),x+1)-B.begin();
        T.setv(a,n,-1,0,n,0);
      }else{
        int a=lower_bound(all(B),x)-B.begin();
        T.flip(0,a,0,n,0);
        T.setv(a,n,+1,0,n,0);
      }
    }else{
