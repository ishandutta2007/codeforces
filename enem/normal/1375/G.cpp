...
static ll n;
static V(ll) ad[N],rad[N],dp[N][2];

void dfs(ll x,ll y)
{
    ll pt=-1;
    REP(i,0,((ll)ad[x].size()))
    {
        if(y==ad[x][i]) pt=i;
    }
    REP(i,0,((ll)ad[x].size()))
    {
        if(i==pt) continue;
        dfs(ad[x][i],x);
        dp[x][0][pt]+=dp[ad[x][i]][1][rad[x][i]];
        dp[x][1][pt]+=dp[ad[x][i]][0][rad[x][i]]+1;
    }
    return;
}
...
void dfsp(ll x,ll y)
{
    ...
    ll sm[2];
    sm[0]=dp[x][0][pt]+dp[ad[x][pt]][1][rad[x][pt]];
    sm[1]=dp[x][1][pt]+dp[ad[x][pt]][0][rad[x][pt]]+1;
    REP(i,0,((ll)ad[x].size()))
    {
        if(i==pt) continue;
        dp[x][0][i]=sm[0]-dp[ad[x][i]][1][rad[x][i]];
        dp[x][1][i]=sm[1]-(dp[ad[x][i]][0][rad[x][i]]+1);
        dfsp(ad[x][i],x);
    }
    return;
}
...
int main()
{
    ...
    REP(i,0,n-1)
    {
        ...
        cin>>t1>>t2;
        t1--;t2--;
        rad[t1].PB(ad[t2].size());
        rad[t2].PB(ad[t1].size());
        ad[t1].PB(t2);
        ad[t2].PB(t1);
        dp[t1][0].PB(0);
        dp[t1][1].PB(0);
    ...
    ll sm[2]={0,0};
    REP(i,0,((ll)ad[0].size()))
    {
        dfs(ad[0][i],0);
        sm[0]+=dp[ad[0][i]][1][rad[0][i]];
        sm[1]+=dp[ad[0][i]][0][rad[0][i]]+1;
    }
    REP(i,0,((ll)ad[0].size()))
    {
        dp[0][0][i]=sm[0]-dp[ad[0][i]][1][rad[0][i]];
        dp[0][1][i]=sm[1]-(dp[ad[0][i]][0][rad[0][i]]+1);
        dfsp(ad[0][i],0);
    }
    ll ans=(1LL<<60);
    REP(i,0,n)
    {
        ll tmp=0;
        REP(j,0,((ll)ad[i].size()))
        {
            tmp+=dp[ad[i][j]][1][rad[i][j]];
        }
        ans=min(ans,tmp);
    }
    cout<<ans;
