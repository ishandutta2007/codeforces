...
int main() {
	...
	for (int m = 1; m < (1 << n); ++m) {
		forbits(i, m)
			dp[m] |= (dp[m ^ (1 << i)]) << a[i];
		for (int i = (vax - 1) / k * k; i > 0; i -= k)
			if (dp[m][i]) dp[m][i / k] = 1;
	}
	int all = (1 << n) - 1;
	if (dp[all][1]) {
		printf("YES\n");
		...
		while (curr) {
			bool found = false;
			forbits(i, curr) {
				for (int x = r, p = 0; x < vax; x *= k, p++) {
					if (x >= a[i] && dp[curr ^ (1 << i)][x - a[i]]) {
						debug(imie(dp)[imask(curr ^ (1 << i))][imie(x) - arr(a, i)], imie(r), imie(d), imie(x), imie(p), arr(a, i));
						r = x - a[i];
						d += p;
						div[i] = d;
						curr ^= (1 << i);
		...
		REP(i, n) debug(arr(a, i), arr(div, i));
		vector <pii> state;
		REP(i, n)
			state.emplace_back(a[i], div[i]);
		REP(_, n - 1) {
			debug(imie(state));
			sort(ALL(state), [](pii x, pii y){return x.second < y.second;});
			pii x = state.back();
			state.pop_back();
			pii y = state.back();
			state.pop_back();
			...
			pii next = {x.first + y.first, x.second};
			while (next.first % k == 0) {
				next.first /= k;
				next.second--;
			}
