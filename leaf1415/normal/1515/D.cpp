...
int main(void)
{
	...
	while(T--){
		cin >> n >> l >> r;
		rep(i, 1, n) a[i][0] = a[i][1] = 0;
		
		ll x;
		rep(i, 1, l) cin >> x, a[x][0]++;
		rep(i, 1, r) cin >> x, a[x][1]++;
		
		ll lcnt = 0, rcnt = 0;
		rep(i, 1, n){
			lcnt += max(0LL, (a[i][0]-a[i][1])) / 2;
			rcnt += max(0LL, (a[i][1]-a[i][0])) / 2;
		}
		
		ll ans = 0;
		rep(i, 1, n) ans += abs(a[i][0]-a[i][1]);
		ans /= 2;
		
		ll d = abs((l-r)/2);
		ans += d;
		if(l >= r) ans -= min(d, lcnt);
		else ans -= min(d, rcnt);
		outl(ans);
	}
