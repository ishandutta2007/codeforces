...
int main()
{
    ...
    */
    ...
    REP(i,2,N)
    {
        ...
        for(ll j=0;j<pr.size()&&pr[j]<=d[i]&&i*pr[j]<N;j++)
        {
            d[i*pr[j]]=pr[j];
        }
    }
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            while(t>1)
            {
                if(st.count(d[t])) st.erase(st.find(d[t]));
                else st.insert(d[t]);
                t/=d[t];
            }
            if(st.size()==0) ans=1;
        ...
        REP(i,0,nv)
        {
            if(i>0) if(pr[i-1]*pr[i-1]>N) break;
            REP(j,0,nv+1)
            {
                dis[j]=(1LL<<30);
                par[j]=(1LL<<30);
            ...
            while(prr.size()>0)
            {
                ...
                for(auto it:ad[t])
                {
                    ...
                    {
                        ans=min((int)ans,dis[t]+dis[it.F]+1);
                    }
                }
            }
        }
        if(ans==(1LL<<30)) cout<<-1;
        else cout<<ans;
