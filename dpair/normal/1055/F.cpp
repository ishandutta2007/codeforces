...
LL f[MAXN], ans;
int p[MAXN][2], t[MAXN][2], tot, sz[MAXN];
signed main(){
    read(n, k); rep(i, 2, n) {
        int u; LL w; read(u, w);
        f[i] = f[u] ^ w; p[i][0] = p[i][1] = 1;
    } p[1][0] = p[1][1] = 1; per(i, 61, 0) {
        tot = 0; LL cur = 0; memset(sz + 1, 0, n * sizeof(int)); memset(t + 1, 0, 2 * n * sizeof(int));
        rep(j, 1, n) {
            const bool ck = f[j] >> i & 1;
            if(!t[p[j][0]][ck]) t[p[j][0]][ck] = ++ tot;
            ++ sz[p[j][0] = t[p[j][0]][ck]];
        }
        rep(j, 1, n) cur += sz[t[p[j][1]][f[j] >> i & 1]];
        bool ck = 0; if(cur < k) k -= cur, ans |= (1ll << i), ck = 1;
        rep(j, 1, n) p[j][1] = t[p[j][1]][(f[j] >> i & 1) ^ ck];
    } print(ans);
}
/*
 Trie 
