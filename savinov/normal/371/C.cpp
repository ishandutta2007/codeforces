...
int main()
{
    ...
    cin >> s;
    sort(all(s));
    auto bb = equal_range(all(s), 'B');
    auto ss = equal_range(all(s), 'S');
    auto cc = equal_range(all(s), 'C');
    int B = bb.y - bb.x;
    int S = ss.y - ss.x;
    int C = cc.y - cc.x;

    int hB = nxt(), hS = nxt(), hC = nxt();

    ll cB = nxt(), cS = nxt(), cC = nxt();
    ...
    while(l < r)
    {
        ...
        ll needB = max(0ll, B * m - hB);
        ll needS = max(0ll, S * m - hS);
        ll needC = max(0ll, C * m - hC);

        ll need_money = needB * cB + needS * cS + needC * cC;
