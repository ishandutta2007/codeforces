...
void solve() {
    ...
    rep (i, n) {
        rep (j, m - 1) mp[a[i].substr(j, 2)] = {i, j, j + 2};
        rep (j, m - 2) mp[a[i].substr(j, 3)] = {i, j, j + 3};
    }
    vector<vector<ll>> dp(m + 1, {-1, -1, -1});
    dp[0] = {0, -1, -1};
    rep (i, m) {
        if (dp[i][0] < 0) continue;
        rep (j, 2, 4) {
            if (i + j > m) break;
            if (mp.count(s.substr(i, j))) {
                dp[i + j] = mp[s.substr(i, j)];
            }
        }
    }
    if (dp[m][0] < 0) {
    ...
    printf("%d\n", ans.size());
    rrep (i, ans.size()) {
        cout << ans[i][1] + 1 << " " << ans[i][2] << " " << ans[i][0] + 1 << endl;
    }
}