...
void Hollwo_Pelw() {
	...
	int dr = 1, dc = 1;
	Mint tot = 0, nxt = 1, val = Mint(p) / 100;

	for (int timer = 0; timer < (n - 1) * (m - 1) * 2; timer ++) {
		if (ra == rb || ca == cb) {
			tot += nxt * val * timer;
			nxt *= 1 - val;
		}

		if (ra + dr > n || ra + dr < 1) dr = - dr;
		if (ca + dc > m || ca + dc < 1) dc = - dc;
		ra += dr, ca += dc;
	}

	cout << (tot + nxt * (n - 1) * (m - 1) * 2) / (1 - nxt) << '\n';
}