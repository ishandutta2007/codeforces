...
int solve(){
  ...
  rep3(m){
   ...
   for(int i=1;i<=n;i++){
      int val=-a[i];
      while(itr1!=mp1.end() && itr1->x.x==i){
          update(1,1,n,itr1->x.x,itr1->y,-1);
          mp2.insert({{itr1->y,itr1->x.y},itr1->x.x});
          itr1++;
      }
      ...
      while(!mp2.empty() && mp2.begin()->x.x==i){
         update(1,1,n,mp2.begin()->y,mp2.begin()->x.x,1);
         mp2.erase(mp2.begin());
      }
  } 
  cout<<ma<<endl;
