...
signed main() {
    ...
    rep (n) {
        ...
        if (s == "??") {
            ...
            have_loops[0] = can_have_loops[0] = 1;
        }
        else if (s == "BW") {
            have_loops[1] = can_have_loops[1] = 1;
        }
        else if (s == "?W") {
            can_have_loops[1] = 1;
            a.app({1, 1});
    ...
    if (a.empty()) {
        if (want == 0) {
            if (can_be_not_connected && have_loops[0] && have_loops[1]) {
                cout << 0 << endl;
            }
            else {
                cout << 1 << endl;
    ...
    mi ans = 0;
    auto res = faq::multiply_all(0, (int)a.size() - 1, a);
    if (want >= 0 && want < res.size()) {
        ans = res[want];
    }
    mi bad = 0;

    if (can_be_not_connected) {
        int not_yet = 2 - can_have_loops[0] - can_have_loops[1];
        debug(not_yet);
        if (not_yet == 0) {
            bad = mi(2)^cnt_dn;    
        }
