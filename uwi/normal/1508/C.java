...
public class C {
	...
	void solve()
	{
		...
		ses[p++] = new int[]{-1, -1, xx};
		ses = Arrays.copyOf(ses, p);

		Arrays.sort(ses, (x, y) -> x[2] - y[2]);
		...
		for(int i = 0;i < n;i++){
			for(int j = i+1;j < n;j++){
				if(!h[i][j]){
					DJSet ls = new DJSet(n);
					for(int ii = 0;ii < n;ii++){
						for(int jj = ii+1;jj < n;jj++){
							if(!h[ii][jj] && !(ii == i && jj == j)){
								ls.union(ii, jj);
							}
						}
					}
