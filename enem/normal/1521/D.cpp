...
void dfs1(ll x, ll y, V(ll) ad[], ll dp[][2], ll ptr[][2])
{
    ll sm = 0;
    ptr[x][0] = ptr[x][1] = -1;
    ...
    for(auto it:ad[x])
    {
        if(it!=y)
        {
            ...
            if(dp[it][0] + 1 - dp[it][1] > 0)
            {
                if(ptr[x][0] == -1 || dp[it][0] + 1 - dp[it][1] > dp[ptr[x][0]][0] + 1 - dp[ptr[x][0]][1])
                {
                    ptr[x][1] = ptr[x][0];
                    ptr[x][0] = it;
                }
                else if(ptr[x][1] == -1 || dp[it][0] + 1 - dp[it][1] > dp[ptr[x][1]][0] + 1 - dp[ptr[x][1]][1])
                {
                    ptr[x][1] = it;
                }
            }
    ...
    {
        dp[x][0] = dp[x][1] = sm - dp[ptr[x][0]][0] - 1 + dp[ptr[x][0]][1];
    }
    else
    {
        dp[x][1] = sm - dp[ptr[x][0]][0] - 1 + dp[ptr[x][0]][1];
        dp[x][0] = sm - dp[ptr[x][0]][0] - 1 + dp[ptr[x][0]][1] - dp[ptr[x][1]][0] - 1 + dp[ptr[x][1]][1];
    }

    return;
}

pll dfs2(ll x, ll y, V(ll) ad[], ll ptr[][2], bool par, vector<pair<pll, pll>> &ans)
{
    pll t = MP(x, x);
    if(ptr[x][0] != -1)
    {
        pll t1 = dfs2(ptr[x][0], x, ad, ptr, true, ans);
        t.S = t1.S;

        if(!par && ptr[x][1] != -1)
        {
            pll t2 = dfs2(ptr[x][1], x, ad, ptr, true, ans);
            t.F = t2.S;
        }
    }
    ...
    for(auto it:ad[x])
    {
        if(it != y && it!=ptr[x][0] && (par || it!=ptr[x][1]))
        {
            pll tmp = dfs2(it, x, ad, ptr, false, ans);
            ans.PB(MP(MP(x, it), MP(t.S, tmp.F)));
            t.S = tmp.S;
        }
    }
...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        dfs2(0, -1, ad, ptr, false, ans);
        cout<<ans.size()<<'\n';
        for(auto it : ans)
        {
            cout<<it.F.F+1<<' '<<it.F.S+1<<' '<<it.S.F+1<<' '<<it.S.S+1<<'\n';
        }
    }
