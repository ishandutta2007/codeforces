...
/*
 ...
 Ta,b,x
 id-INFf(x)//ax+blong longLL(10)
 idGET_MAX
 
 x  sort, unique 
 convex_hull_trick<ll>cht(comx);
 convex_hull_trick<LL>cht(comx);
...
class convex_hull_trick {
    const ll id=LLONG_MIN;//id=LLONG_MINOK
    bool GET_MAX=true;//truefalse
...
public:
    ...
    T get(T x) const {
        ...
        assert(t < n && pos[t] == x);
        T ret=sub(root, 0, n - 1, t);
        if(GET_MAX==false)ret=-ret;
        return ret;
    }
...
signed main(){fastio
    ll n;cin>>n;
    vector<tyo>v(n);
    rep(i,0,n-1)cin>>v[i].x>>v[i].y>>v[i].a;
    sort(all(v),[](tyo a,tyo b){return a.x<b.x;});
    vl dp(n);
    vl vx;for(auto a:v)vx.pb(a.y);
    ERASE(vx);
    convex_hull_trick<ll>cht(vx);
    rep(i,0,n-1){
        dp[i]=v[i].x*v[i].y-v[i].a;
        chmax(dp[i],cht.get(v[i].y)+v[i].x*v[i].y-v[i].a);
        cht.insert(-v[i].x,dp[i]);
    }
    ll ans=-inf;
