...
unordered_map<array<int, 4>, unt> cache;
const int nax = 2001;
bool prime[nax];
unt go(int a, int b, int c, int p) {
	while (!prime[p]) p--;
	if (p == 1) return 1;
	array <int, 4> t = {a, b, c, p};
	sort(t.begin(), t.begin() + 3);
	unt ans = 0;
	auto it = cache.find(t);
	if (it != cache.end()) return it -> second;
	for (int aa = a, sa = 0; aa; aa /= p, sa++)
		for (int bb = b, sb = 0; bb; bb /= p, sb++)
			for (int cc = c, sc = 0; cc; cc /= p, sc++)
				ans += go(aa, bb, cc, p - 1) * (sa + sb + sc + 1);
	debug(imie(a), imie(b), imie(c), imie(p), imie(ans));
	return cache[t] = ans;
}
int main() {
	memset(prime, 1, sizeof(prime));
	for (int p = 2; p < nax; ++p) if (prime[p]) for (int i = 2 * p; i < nax; i += p) prime[i] = 0;
	int a, b, c;
	scanf("%d%d%d", &a, &b, &c);
	printf("%u\n", go(a, b, c, 2000) & ((1 << 30) - 1));
}