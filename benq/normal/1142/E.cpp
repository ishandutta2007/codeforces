...
template<int SZ> struct scc {
    ...
    void genSCC() { // fills allComp
        ...
        FOR(i,1,N+1) rcomp[comp[i]].pb(i);
        FOR(i,1,N+1) trav(j,adj[i]) if (comp[i] != comp[j]) child[comp[i]].pb(comp[j]);
        FOR(i,1,N+1) {
            sort(all(child[i])); child[i].erase(unique(all(child[i])),child[i].end());
            trav(t,child[i]) in[t] ++;
        }
        FOR(i,1,N+1) if (comp[i] == i && in[i] == 0) q.push(i);
    }
};
...
int main() {
    ...
    while (sz(q)) {
        ...
        while (sz(S.rcomp[a]) && sz(S.rcomp[b])) {
            if (query(S.rcomp[a].back(),S.rcomp[b].back())) S.rcomp[b].pop_back();
            else S.rcomp[a].pop_back();
        }
        if (!sz(S.rcomp[a])) swap(a,b);
        trav(t,S.child[b]) {
            S.in[t] --;
            if (!S.in[t]) q.push(t);
        }
    }
    cout << "! " << S.rcomp[a].back() << endl;
}
