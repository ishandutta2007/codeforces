/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 374 (Div. 2)         |
|       Task: D                                     |
|       Date: 30. 09. 2016                          |
\--------------------------------------------------*/
// Includes
...
struct cmp
{
    ...
    {
        return make_pair(abs(a.F), a.S) < make_pair(abs(b.F), b.S);
    }
};
...
main()
{
    ...
    int minEl = 0, minusCnt = 0;
    for (int i = 0; i < n; i++)
        if (abs(g[i]) < abs(g[minEl])) minEl = i;
    for (int i = 0; i < n; i++)
        if (g[i] < 0) minusCnt++;

    if (minusCnt % 2 == 0)
    {
        int mesa = (g[minEl] < 0) ? +1 : -1;
        while (m > 0 && g[minEl] * mesa <= 0)
        {
            g[minEl] += x * mesa;
            m--;
        }
    ...
    for (int i = 0; i < n; i++) s.insert({g[i], i});
    while (m > 0)
    {
