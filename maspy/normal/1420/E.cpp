...
void solve() {
  ...
  */
  ...
  ll S = SUM(A);
  ll K = N * (N - 1) / 2;
  vv(ll, DP, S + 1, K + 1, INF);
  DP[0][0] = 0;
  auto Ac = cumsum(A);
  FOR(i, len(A)) {
    ll now = Ac[i];
    vv(ll, newDP, S + 1, K + 1, INF);
    FOR(s, S + 1) FOR(k, K + 1) if (DP[s][k] < INF) {
      auto& dp = DP[s][k];
      ll cost = abs(s - now);
      FOR3(t, s, S + 1) { chmin(newDP[t][k + cost], dp + (s - t) * (s - t)); }
    }
    swap(DP, newDP);
  }
  auto& dp = DP[S];
  FOR(i, len(dp) - 1) chmin(dp[i + 1], dp[i]);
  for (auto&& x: dp) { x = (S * S - x) / 2; }
  print(dp);
}
