...
public class Main
{
	public static void main(String[] args)
	{
		...
		double time=-m/y;
		double time1=time,time2=time;
		if(x!=0)
		{
		while(time1>=(2*a)/Math.abs(x))
			time1-=(2*Math.abs(a))/Math.abs(x);
		}
		if(z!=0)
		while(time2>=(2*Math.abs(b))/Math.abs(z))
			time2-=(2*Math.abs(b))/Math.abs(z);
		double fa=a/2+time1*x;
		double fb=time2*z;
		if(fa>=2*a)
			fa-=2*a;
		if(fa<0)
