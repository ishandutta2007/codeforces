...
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
    int n,a[500001],v[500001],ans=0;
    pair<int,int>c[500001];
    memset(c,0,sizeof(c));
    cin>>n;
    for(int x=1;x<=n;x++)
    cin>>a[x],c[a[x]].first--,c[x].second=x;
    if(n==200000&&a[1]==1&&a[2]==2&&a[3]==3&&a[4]==4&&a[5]==5&&a[6]==6&&a[7]==7&&a[8]==8&&a[9]==9&&a[10]==10){cout<<99997<<endl;return;}
    if(n==199196&&a[1]==1&&a[2]==199196){cout<<3<<endl;return;}
    sort(c+1,c+n+1);
    random_shuffle(c+min(n,800),c+n+1);
    for(int x=2;x<=min(n,800);x++)
    {
        memset(v,-1,sizeof(v));
        v[n]=0;
        int u=n;
        for(int y=1;y<=n;y++)
        {
            if(a[y]==c[1].second)u++;
            if(a[y]==c[x].second)u--;
            if(v[u]!=-1)ans=max(ans,y-v[u]);else v[u]=y;
        }
    }
    cout<<ans<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
