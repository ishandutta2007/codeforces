...
vector<bool> get(vector<int> x, int N) {
    assert(*max_element(x.begin(), x.end()) <= N);
    vector<int> a(N+1), b(N+1);
    for (auto c : x) a[c]++, b[N-c]++;
    auto _ans = fft::multiply(a, b);

    vector<bool> ans(N+1);
    for (int i = 0; i <= N; i++) ans[i] = _ans[i+N];
    return ans;
}
...
void solve() {
    int n, x, y; cin >> n >> x >> y;
    vector<int> a(n+1); for (auto& c : a) cin >> c;
    vector<bool> can = get(a, x);

    memset(ans, -1, sizeof(ans));
    for (int a = y+1; a < MAXN && a-y < sz(can); a++) if (can[a-y]) for (int l = a; 2*l < MAXN; l += a) {
        ans[2*l] = max(ans[2*l], 2*a);
    }
