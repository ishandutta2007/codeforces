...
void add(int x){
    ...
    int l=*prev(it),r=*it;
    segtree.modify(1,0,n-1,x,r-1,-(x-l));
    st.insert(x);
}
...
void orzck(){
    ...
    rep(n) cin >> a[i];
    st.insert(0);
    rep(n-1) if(a[i+1]<a[i]) st.insert(i+1);
    st.insert(n);
    int cur=1;
    rep(n){
        cur++;
        if(st.count(i)) cur=1;
        segtree.modify(1,0,n-1,i,i,cur);
        bug(i,cur);
    }
    bug(st);
    while(q--){
        ...
        if(op==1){
            ...
            p--;
            a[p]=x;
            if(p&&a[p]<a[p-1]&&!st.count(p)) add(p),bug(p);
            if(p&&a[p]>=a[p-1]&&st.count(p)) sub(p),bug(p);
            if(p<n-1&&a[p+1]<a[p]&&!st.count(p+1)) add(p+1),bug(p+1);
            if(p<n-1&&a[p+1]>=a[p]&&st.count(p+1)) sub(p+1),bug(p+1);
        }
        else{
            int l,r;
            cin >> l >> r;
            l--,r--;
            bool flag=0;
            if(!st.count(l)) add(l),flag=1;
            bug(l,r,segtree.query(1,0,n-1,l,r));
