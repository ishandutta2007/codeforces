...
bool operator<(const Event &a, const Event &b) { return a.t < b.t; }
...
int main() {
    ...
    for (int i = 0; i < m; ++i) {
        prod[i][0] = 1;
        for (int j = 0; j < n; ++j) prod[i][j + 1] = prod[i][j] * std::abs(l[j] - p[i]);
        inv[i][n] = prod[i][n].inv();
        for (int j = n - 1; j >= 0; --j) inv[i][j] = inv[i][j + 1] * std::abs(l[j] - p[i]);
    }
    ...
    for (int j = 0; auto [f, i] : qry) {
        while (j < int(events.size()) && events[j].t <= f) {
            x0 += events[j].x0, x1 += events[j].x1, ++j;
        }
        ans[i] = x0 + f * x1;
    }
