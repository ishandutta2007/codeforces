...
void solve(){
	...
	for(int i=a;i<=b;i++){
		ll x=i+b-1,y=i+c-1,v=d-c+1,vv=c-b+1;
		if(x<=c){
			if(y<=d)ans+=(1+y-c+1)*(y-c+1)/2;
			else ans+=(1+v)*v/2+v*(y-d);
		}
		else if(x<=d&&y<=d)ans+=(x-c+1+y-c+1)*vv/2;
		else if(x<=d&&y>=d)ans+=(x-c+1+v)*(v-(x-c+1)+1)/2+v*((y-c+1)-v);
		else ans+=vv*v;
	}
	cout<<ans<<endl;
}
