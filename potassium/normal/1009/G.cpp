...
int main() {
	...
	vector<int> cnt_alpha(6, 0);
	for (int i = 0; i < n; i++)
		cnt_alpha[s[i] - 'a']++;

	vector<int> cnt_mask(64, 0);
	for (int i = 0; i < n; i++)
		cnt_mask[mask[i]]++;

	auto check = [&]() {
		vector<int> sos_alpha(64, 0);
		vector<int> sos_mask = cnt_mask;
		for (int mask = 1; mask < 64; mask++) {
			int lowbit = mask & -mask;
			int b = __lg(lowbit);
			sos_alpha[mask] = sos_alpha[mask ^ lowbit] + cnt_alpha[b];
		}

		for (int b = 0; b < 6; b++) {
			for (int mask = 0; mask < 64; mask++) {
				if (mask & (1 << b)) {
					sos_mask[mask] += sos_mask[mask ^ (1 << b)];
				}
			}
		}

		for (int mask = 1; mask < 64; mask++)
			if (sos_alpha[mask] < sos_mask[mask])
				return false;
