...
int unions[2][300000];
int n, k;
int uf(int now,int itr) {
    if (now >= n || now <= -1) return now;
    if (unions[itr][now] == now) return now;
    return unions[itr][now] = uf(unions[itr][now], itr);
}
void solve() {
    cin >> n >> k;
    vector<int> a(n);
    ...
    while (next.empty() == false) {
        ...
        next.pop();
        if (unions[0][now] != now) continue;
        cnt++;
        REP(q, k + 1) {
            int next_go = uf(now, 1);
            if (next_go == n) break;
            if (cnt % 2 == 0) {
                ans[next_go] = '2';
            }
            unions[1][next_go] = next_go + 1;
            unions[0][next_go] = next_go - 1;
        }
        REP(q, k) {
            int next_go = uf(now, 0);
            if (next_go == -1) break;
            if (cnt % 2 == 0) {
                ans[next_go] = '2';
            }
