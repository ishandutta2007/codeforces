...
int main(){
  ...
  ll res=0,buf=0;
  using AR=array<ll,10>;
  auto pr=[&](AR x){
    rep(i,0,10)cout<<x[i]<<" ";
    cout<<endl;
  };
  bool judge = true;
  AR M1;rep(i,0,10)M1[i]=0;
  AR iden;rep(i,0,10)iden[i]=i;
  auto f=[&](AR x,AR y){
    rep(i,0,10)x[i]+=y[i];
    return x;
  };
  ...
  auto h=[&](AR x,AR y){
    AR ret=M1;
    rep(i,0,10)ret[i]=y[x[i]];
    //pr(x);pr(y);pr(ret);
    return ret;
  };
  ll n,q;cin>>n>>q;

  LazySegmentTree<AR,AR,decltype(f),decltype(g),decltype(h)>seg(n,f,g,h,M1,iden);
  rep(i,0,n){
    ll a;cin>>a;
    AR now=M1;
    ll tmp=1;
