...
int get(int num, int id) {
    return (num >> id) & 1;
}

bool is_consistent(int mask, int nask) {
    for (int i = 0; i + 1 < n; i++) {
        int cnt = get(mask, i) + get(mask, i + 1)
            + get(nask, i) + get(nask, i + 1);
        if (cnt % 2 == 0)
            return false;
    }
    return true;
...
int disc(int mask, int col) {
    int nask = 0;
    for (int i = 0; i < n; i++)
        nask |= (s[i][col] & 1) << i;
    return __builtin_popcount(mask ^ nask);
}
...
int solve() {
    ...
    for (int mask = 0; mask < (1 << n); mask++) {
        dp[0][mask] = disc(mask, 0);
    }
    for (int mask = 0; mask < (1 << n); mask++)
        for (int nask = 0; nask < (1 << n); nask++)
            cons[mask][nask] = is_consistent(mask, nask);
    for (int i = 1; i < m; i++)
        for (int mask = 0; mask < (1 << n); mask++) {
            for (int nask = 0; nask < (1 << n); nask++)
                if (cons[mask][nask])
                    smin(dp[i][mask], dp[i - 1][nask] + disc(mask, i));
        }
