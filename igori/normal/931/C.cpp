...
ll n, m, k, cpc, mini = INF, maxi = -INF;
ll a[900000];
ll c[900000];
    ...
    if (diff == 2)
    {
        ...
        for (int i = 0; i <= 100000; i++)
        {
            ...
            if (difference % 2 != 0)
                continue;
            ll now_y = i;
            ll now_z = z + difference / 2;
            ll now_x = x + difference / 2;
            //cout << now_x << " " << now_y << " " << now_z << endl;
            if (now_y < 0 || now_x < 0 || now_z < 0)
                continue;
            if (min(x, now_x) + min(y, now_y) + min(z, now_z) < minis)
            {
                minis = min(x, now_x) + min(y, now_y) + min(z, now_z);
                id_x = now_x;
                id_y = now_y;
                id_z = now_z;
            }
