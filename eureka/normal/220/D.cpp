...
int main(){
    ...
    scanf("%d%d", &h, &w);
    lld ans = 0;
    REP(i, 2) REP(j, 2) cnt[i][j] = (h/2+1-(h+1)*i%2) * (w/2+1-(w+1)*j%2);
    REP(a, 2) REP(b, 2) REP(c, 2) REP(d, 2) REP(e, 2) REP(f, 2){
        if(checkmod((a-c)*(b-f) - (b-d)*(a-e), 2) == 0){
            ans += cnt[a][b] * cnt[c][d] % P * cnt[e][f] % P;
            ans %= P;
        }
    }
    ...
    zero += tot;
    zero += tot * (tot-1) * 3 % P;
    zero += tot * (h*(h-1) + w*(w-1)) % P;
    for(int i=2; i<=h; ++i) for(int j=2; j<=w; ++j){
        if(gcd[i][j] == 1) continue;
        zero += ((lld)h+1-i) * (w+1-j) % P * (gcd[i][j]-1) * 12 % P;
        zero %= P;
    }
    cout << checkmod(ans-zero, P) << endl;
    return 0;
}