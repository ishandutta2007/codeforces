...
double hypot(double x, double y) { return sqrt(pow(x, 2) + pow(y, 2)); }
main() {
    DBL(ax, ay, bx, by, tx, ty);
    double ans;
    vector<double> x, y;
    INT(n);
    rep(i, n) {
    ...
    if(n == 1) {
        cout << min(hypot(tx - x[0], ty - y[0]) + hypot(ax - x[0], ay - y[0]), hypot(tx - x[0], ty - y[0]) + hypot(bx - x[0], by - y[0])) << endl;
        return 0;
    }
    rep(i, n) { ans += hypot(tx - x[i], ty - y[i]) * 2; }
    auto id1 = iota(n), id2 = iota(n);
    sort(all(id1),
         [&](int i, int j) { return hypot(tx - x[i], ty - y[i]) - hypot(ax - x[i], ay - y[i]) > hypot(tx - x[j], ty - y[j]) - hypot(ax - x[j], ay - y[j]); });
    sort(all(id2),
         [&](int i, int j) { return hypot(tx - x[i], ty - y[i]) - hypot(bx - x[i], by - y[i]) > hypot(tx - x[j], ty - y[j]) - hypot(bx - x[j], by - y[j]); });
    double d = -1e15;
    id1.eb(0), id2.eb(0);
    id1[2] = n, id2[2] = n;
    rep(_i, 3) {
        rep(_j, 3) {
            int i = id1[_i], j = id2[_j];
            if(i != j)
                chmax(d, (_i != 2 ? hypot(tx - x[i], ty - y[i]) - hypot(ax - x[i], ay - y[i]) : 0) +
                             (_j != 2 ? hypot(tx - x[j], ty - y[j]) - hypot(bx - x[j], by - y[j]) : 0));
        }
    }
    cout << ans - d << endl;
}