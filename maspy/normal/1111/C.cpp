...
void solve() {
  ...
  VEC(ll, X, K);
  for (auto&& x: X) --x;
  auto dfs = [&](auto& dfs, ll lo, ll hi, vi X) -> ll {
    if (len(X) == 0) return A;
    if (hi - lo == 1) { return B * len(X); }
    ll mi = (lo + hi) / 2;
    vi X1, X2;
    for (auto&& x: X) {
      if (x < mi) X1.eb(x);
    ...
    ll res = dfs(dfs, lo, mi, X1);
    res += dfs(dfs, mi, hi, X2);
    chmin(res, B * len(X) * (hi - lo));
    return res;
  };
  ll ANS = dfs(dfs, 0, 1LL << N, X);
  print(ANS);
}
