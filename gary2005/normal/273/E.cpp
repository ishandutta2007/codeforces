...
int main(){
    ...
    for(auto it:seg){
        ...
        if(l>=p) continue;
        check_min(r,p);
        add(w[t],1ll*(r-l+1)*p%MOD);
        add(w[t],MOD-(1ll*(r+l)*(r-l+1)/2)%MOD);
    }
    int answer=0;
    rb(i,0,n/2) rb(j,0,(n-2*i)/2) add(answer,MOD-1ll*quick(w[1],2*i)*quick(w[2],2*j)%MOD*quick(w[0],n-2*i-2*j)%MOD*c[2*i+2*j][2*j]%MOD*c[n][2*i+2*j]%MOD);
    rb(i,0,n) rb(j,0,n-i) add(answer,1ll*quick(w[0],i)*quick(w[1],j)%MOD*quick(w[2],n-i-j)%MOD*c[i+j][j]%MOD*c[n][i+j]%MOD);
    cout<<answer<<endl;
    return 0;
}