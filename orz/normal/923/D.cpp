...
vei last_As(const veb& s)
{
	int n = sz(s);
	vei ans(n + 1);
	for (int i = 0; i < n; ++i)
		ans[i + 1] = (s[i] ? 0 : ans[i] + 1);
	return ans;
}

vei pref_Bs(const veb& s)
{
	int n = sz(s);
	vei ans(n + 1);
	for (int i = 0; i < n; ++i)
		ans[i + 1] = (s[i] ? ans[i] + 1 : ans[i]);
	return ans;
}

pii infa(const vei& A, const vei& B, const int& a, const int& b)
{
	pii ans;
	ans.X = min(A[b], b - a);
	ans.Y = B[b - ans.X] - B[a];
...
int main()
{
	...
	gl(cin, s);
	gl(cin, t);
	veb S = A_BC(s), T = A_BC(t);
	vei A1 = last_As(S), B1 = pref_Bs(S), A2 = last_As(T), B2 = pref_Bs(T);
	/*
	cout << s << '\n';
	...
	*/
	...
	for (int I = 0; I < N; ++I)
	{
		int a, b, c, d;
		cin >> a >> b >> c >> d;
		pii p1 = infa(A1, B1, a - 1, b), p2 = infa(A2, B2, c - 1, d);
		cout << ((((p1.Y ^ p2.Y) & 1) ||
			(p1.Y > p2.Y) ||
			(p1.X < p2.X) ||
			((p1.Y == p2.Y) && ((p1.X - p2.X) % 3)) ||
			((p1.Y == 0) && (p2.Y) && (p1.X == p2.X))) ? '0' : '1');
	}
	cout << '\n';
	return 0;
}