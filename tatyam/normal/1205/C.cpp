...
bool query(ll x1,ll y1,ll x2,ll y2){
    out('?',x1+1,y1+1,x2+1,y2+1);
    fflush(stdout);
     LL(a);
     return a;
}
void answer(vector<vector<bool>>a){
    out('!');
    each(i,a){
        each(j,i)print(j);
        out();
...
int main(){
    ...
    ans[0][0]=1;
    {
        const ll dx[3]={0,-1,-2},dy[3]={-2,-1,0};
        rep(x,n)rep(y,n)if((x^y^1)&1){
            rep(3){
                const ll x2=x+dx[i],y2=y+dy[i];
                if(x2<0||y2<0)continue;
                ans[x][y]=query(x2,y2,x,y)^ans[x2][y2]^1;
                break;
            }
        }
    }
    {
        const ll dx[3]={0,-1,-2},dy[3]={-2,-1,0};
        rep(x,n)rep(y,n)if((x^y)&1){
            if(x==1&&y==2){
                ans[1][2]=query(0,1,1,2)^ans[0][1]^1;
                ans[1][0]=query(1,0,1,2)^ans[1][2]^1;
                continue;
            }
            rep(3){
                const ll x2=x+dx[i],y2=y+dy[i];
    ...
    auto flip=[&]{
        rep(x,n)rep(y,n)if((x^y)&1)ans[x][y]=!ans[x][y];
    };
    auto check=[&](ll x1,ll y1,ll x2,ll y2){
        if(x1<0||y1<0||x2>=n||y2>=n)return;
        if(ans[x1][y1]^ans[x2][y2]^query(x1,y1,x2,y2)^1)flip();
        answer(ans);
    };
    bool flag=1;
    rep(x,1,n)rep(y,n-1)if(ans[x][y]!=ans[x-1][y+1]){
        check(x-1,y-1,x,y+1);
        check(x-2,y,x,y+1);
        check(x-1,y,x,y+2);
        check(x-1,y,x+1,y+1);
    ...
    rep(n*2-4){
        const ll x1=i/2,x2=(i+1)/2,x3=(i+2)/2,x4=(i+3)/2,y1=(i+1)/2,y2=(i+2)/2,y3=(i+3)/2,y4=(i+4)/2;
        if(ans[x1][y1]^ans[x2][y2]^ans[x3][y3]^ans[x4][y4]^1)check(x1,y1,x4,y4);
    }
}