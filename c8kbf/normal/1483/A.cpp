...
inline void AC() {
    ...
    for(_ i = 1; i <= m; ++i) {
        ...
        if(k == 1) {
            ...
            o[i] = read();
            ++b[o[i]];
        } else for(; k--; ) a[i].push_back(read());
    }
    for(_ i = 1; i <= n; ++i) if(b[i] > ((m+1)>>1)) {
        writeln("NO");
        return;
    }
    writeln("YES");
    for(_ i = 1; i <= m; ++i) if(!r[i]) for(_ j : a[i]) if(b[j] < ((m+1)>>1)) {
        ++b[j];
        o[i] = j;
        break;
    }
    for(_ i = 1; i <= m; ++i) writesc(o[i]);
    clr();
