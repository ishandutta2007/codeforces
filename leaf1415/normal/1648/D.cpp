...
ll n, Q;
ll a[5][500005];
ll sum[500005], sum3[500005];
ll l[500005], r[500005], c[500005];
vector<P> vec[500005];
...
int main(void)
{
	...
	sum[1] = a[1][1];
	rep(i, 2, n) sum[i] = sum[i-1] + a[1][i] - a[2][i-1];
	sum[n+1] = -inf;
	
	sum3[n] = a[3][n];
	per(i, n-1, 1) sum3[i] = sum3[i+1] + a[3][i] - a[2][i+1];
	
	seg.init();
	
	rep(i, 1, Q){
		cin >> l[i] >> r[i] >> c[i];
		vec[r[i]].pb(P(l[i], c[i]));
	}
	...
	rep(i, 0, n){
		...
		for(auto p : vec[i]){
			ll l = p.fi, c = p.se;
			chmax(dp[i], seg.query(l-1, i-1)-c);
		}
		seg.update(i, dp[i]);
	}
	...
	rep(i, 1, n){
		for(auto p : vec[i]){
			ll l = p.fi, c = p.se;
			chmax(ans, seg2.query(l-1, i)[2] - c);
		}
	}
