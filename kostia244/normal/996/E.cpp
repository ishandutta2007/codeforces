...
ll n;
bitset<100100> neg;
vector<pair<vec, ll>> a;
vec dir, odir;
bool go() {
	dir = odir;
	shuffle(all(a), rng);
	for (int i, j, k = 0; k < 50; k++) {
		i = rng() % n, i = rng() % n;
		if (norm(dir - a[j].first) < norm(dir - a[i].first))
			swap(a[i], a[j]);
	}
	int i = 0;
	while (i < a.size() && norm(dir) > 2250000000000ll) {
		dir -= a[i].first;
		dir -= a[i].first;
		i++;
	}
...
int main() {
	...
	for (int X, Y, i = 0; i < n; i++) {
		cin >> X >> Y;
		if (norm(dir + vec(X, Y)) < norm(dir - vec(X, Y)))
			X = -X, Y = -Y, neg.flip(i);
		a.pb( { vec(X, Y), i });
		odir += a.back().first;
	}
