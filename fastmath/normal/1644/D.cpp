...
map <array <int, 3> , mi> d;
int k;
mi f(int n, int m, int q) {
    if (m < n) swap(n, m);
    assert(n > 0);
    if (q == 0) {
        return 1;
    ...
    if (d.find({n, m, q}) != d.end()) return d[{n, m, q}];
    else return d[{n, m, q}] = f(n - 1, m, q - 1) * n * k + f(n, m - 1, q - 1) * m * k 
        + f(n - 1, m - 1, q - 1) * n * m * k;
}
