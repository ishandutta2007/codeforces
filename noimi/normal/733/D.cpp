...
main() {
    ...
    int ma = 0;
    rep(i, n) {
        if(chmax(ma, min({a[i][0], a[i][1], a[i][2]}))) mem = i;
    }
    map<pii, set<pii>> mp;
    rep(i, n) {
        sort(all(a[i]));
        do { mp[minmax(a[i][0], a[i][1])].emplace(a[i][2], i); } while(next_permutation(all(a[i])));
    }
    for(auto [p, q] : mp) {
        if(si(q) < 2) continue;
        int now = q.rbegin()->fi + (++q.rbegin())->fi;
        if(chmax(ma, min({p.fi, p.se, now}))) k = 2, ans = {q.rbegin()->se, (++q.rbegin())->se};
    }
    cout << k << endl;
    if(k == 1)
        cout << mem + 1 << endl;
