...
void dfs(int x, int p, int d, int pp, vector<vector<int>> &gr){
	tr[d][pp]++;
	mxdep = max(mxdep,d);
	for(auto y : gr[x]){
		if(y == p)continue;
...
ll fin(int x, int k, int d){
	if(k == 0)return 1;
	if(x == (int)tr.size())return 0;
	if(!MP[x])return fin(x+1,k,d);
	if(dp[MP[x]][k] != -1)return dp[MP[x]][k];
	ll ret = 0;
	ret+=tr[d][MP[x]]*fin(x+1,k-1,d);
	ret+=fin(x+1,k,d);
	return dp[MP[x]][k] = ret%MOD;
}
