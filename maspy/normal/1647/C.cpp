...
void solve() {
  ...
  using tpl = tuple<ll, ll, ll, ll>;
  vc<tpl> ANS;
  auto OPE = [&](ll x1, ll y1, ll x2, ll y2) -> void {
    assert(abs(x1 - x2) + abs(y1 - y2) == 1);
    assert(x1 + y1 < x2 + y2);
    A[x1][y1] = 0;
    A[x2][y2] = 1;
    ANS.eb(x1 + 1, y1 + 1, x2 + 1, y2 + 1);
  };

  if (B[0][0]) return print(-1);
  FOR_R(x, H) FOR_R(y, W) if (B[x][y]) {
    if (x)
      OPE(x - 1, y, x, y);
    else
      OPE(x, y - 1, x, y);
  }
  FOR(x, H) FOR(y, W) if (A[x][y] == 1 && B[x][y] == 0) OPE(x, y, x, y);
  print(len(ANS));
