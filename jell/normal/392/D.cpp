#line 1 "codeforces-workspace/contest/392/d/d.cpp"
...
#line 6 "codeforces-workspace/contest/392/d/d.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/392/d/d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/392/d/d.cpp"
...
#line 27 "codeforces-workspace/contest/392/d/d.cpp"
...
void main() {
  ...
  unordered_map<int, int> af, bf, cf;

  constexpr int inf = 1e9;

  for (auto &&x : cat(a, b, c)) {
    af[x] = inf;
    bf[x] = n;
    cf[x] = inf;
  }

  for (auto &&[i, e] : reversed(enumerate(a))) {
    af[e] = i;
  }
  for (auto &&[i, e] : reversed(enumerate(b))) {
    bf[e] = i;
  }
  for (auto &&[i, e] : reversed(enumerate(c))) {
    cf[e] = i;
  }
  ...
  struct endo {
    ...
    endo operator*(endo r) const {
      return none ? r : r.none ? *this : endo{max(v, r.v), false};
    }
  };
  ...
  struct mono {
    ...
    mono operator+(mono r) const {
      return {std::min(min, r.min), std::min(l, r.l), max(pos, r.pos)};
    }

    mono operator*(endo r) const {
      return r.none ? *this : mono{r.v + l, l, r.v};
    }
  };

  lazy_segment_tree<mono, endo> bc(n + 1);

  {
    ...
    {  // init p
      unordered_set<int> yet(begin(c), end(c));
      for (auto &&x : cat(a, b)) {
        if (yet.count(x)) {
          yet.erase(x);
        }
      ...
      while (p < n && !yet.empty()) {
        if (yet.count(c[p])) yet.erase(c[p]);
        occ[c[p]] += 1;
        p++;
      }
      assert(yet.empty());
  ...
  auto ans = n + bc.fold().min;

  for (auto &&[i, e] : reversed(enumerate(a))) {
    // erase e at i.
    if (i == af[e]) {
      auto pt =
          bc.left_partition(bf[e] + 1, [&](mono x) { return x.pos <= cf[e]; });
      bc.update(pt, bf[e] + 1, {cf[e] + 1, false});
    }

    auto now = bc.fold().min;
    chle(ans, now + i);
  }
