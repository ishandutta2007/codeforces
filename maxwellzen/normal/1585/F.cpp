...
string to_string(mi a) {return to_string(a.v);}
...
void rem(int x) {
    while (lim>x) {
        pair<int, mi> cur = *rgs.rbegin();
        if (cur.F >= x) rgs.erase(cur);
        mi sub = (mult.F*cur.S + mult.S) * mi(lim - max(x, cur.F));
        debug(cur, sub);
        sum -= sub;
        lim = max(x, cur.F);
    }
}
void add(int x, mi val) {
    if (lim < x) {
        val = (val - mult.S) / mult.F;
        rgs.insert(mp(lim, val));
        lim = x;
    }
}
