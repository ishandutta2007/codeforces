...
void bfs(int x) {
	...
	while (!kol.empty()) {
		int c = kol.front();
		debug(arr(vis, c), arr(dist, c));
		kol.pop();
		for (int v : graf[c]) if (!vis[v]) {
			vis[v] = 1;
			p[v] = c;
			dist[v] = dist[c] + 1;
			kol.push(v);
			...
			debug(arr(dist, v), arr(dist, c));
			int l = 1 + dist[v] + dist[c];
			if (l > 2 && p[v] != c && p[c] != v) mini(ans, l);
		}
	}
}
int main() {
	...
	REP(i, 61) {
		if (which[i].size() == 2u)
			add_e(which[i][0], which[i][1]);
	}
	REP(i, n) if (!graf[i].empty()) bfs(i);
	if (ans > n) ans = -1;
	printf("%d\n", ans);
}