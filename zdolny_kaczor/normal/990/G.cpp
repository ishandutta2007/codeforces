...
#ifdef DEB
true;
sim> auto dud(T *x) -> typename enable_if<!is_same<T, string>::value, decltype(x -> begin(), 0)>::type;
sim> char dud(...);
#define dep(u...) xug<u, sizeof(dud<u>(0)) == 4>::d
sim, bool B> struct xug {
	using t = typename remove_reference<decltype(*T().begin())>::type;
	static constexpr int d = 1 + dep(t);
};
sim> struct xug <T, false> {
	static constexpr int d = 0;
};
sim, class N> struct xug <pair<T, N>, false> {
	static constexpr int d = dep(T) > dep(N) ? dep(T) : dep(N);
};
...
VI occs[MN];
int num[MN];
#define iter_muls(i, p) for (int xxu = p; xxu < MN; xxu += p) for (int i : occs[xxu])
int main() {
	int n;
	scanf("%d", &n);
	FOR(i, 1, n) {
	...
	FOR(p, 2, MN - 1) {
		...
		iter_muls(i, p) {
			debug(imie(i));
			if (up[i] != 0 && num[up[i]] % p == 0)
				onion(i, up[i]);
		}
		iter_muls(i, p)
			if (u[i] == i) {
				divi[p] += size[i] * (size[i] + 1ll) / 2;
				debug(arr(size, i));
			}
		iter_muls(i, p) {
			u[i] = i;
	...
	if (deb) REP(i, MN) if (divi[i]) debug(arr(divi, i));
	FORD(i, MN - 1, 1) {
		gcd[i] = divi[i];
		for (int x = 2 * i; x < MN; x += i)
			gcd[i] -= gcd[x];
	}
	FOR(p, 1, MN - 1)
		if (gcd[p] > 0)
