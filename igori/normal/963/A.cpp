...
void solve()
{
    ...
    ll ans = 0;
    ll D = binpow(b, k) % MOD * rev(binpow(a, k)) % MOD;
    ll DE = (MOD + 1 - binpow(D, (n + 1) / k)) * rev(MOD + 1 - D) % MOD;
    for (int i = 0; i < k; i++)
    {
        ll f = binpow(a, n - i) * binpow(b, i) % MOD;
        if (s[i] == '-') f = MOD - f;
        if (D == 1) ans = (ans + (n + 1) / k * f) % MOD;
        else ans = (ans + f * DE) % MOD;
    }
    dbg(DE);
    cout << ans << "\n";
}
