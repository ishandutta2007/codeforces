...
int dfs(int u, int prv)
{
    ...
    {
        if (c[g[u][i].Y]!=0) continue;
        if (g[u][i].Y==prv) continue;
        if (was[g[u][i].X]==1)
        {
            c[g[u][i].Y] = 1;
            if (u>=mx) c[g[u][i].Y]++;
...
int main()
{
    ...
    {
        if (i!=0 and p2[i].X.X==p2[i-1].X.X)
        {
            g2[p2[i].Y].push_back(p2[i-1].Y);
            g2[p2[i-1].Y].push_back(p2[i].Y);
        }
    }
    for (int i=0;i<p2.size();i++)
    {
        swap(p2[i].X.X,p2[i].X.Y);
    }
    sort(p.begin(),p.end());
    sort(p2.begin(),p2.end());
    for (int i=0;i<p2.size();i++)
