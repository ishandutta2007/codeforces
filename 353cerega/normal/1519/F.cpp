...
int main()
{
    ...
    int mx = 1;
    for (int i=0;i<m;i++) mx *= 5;
    vector<int> cur(mx*m,INF);
    vector<int> p5(m);
    p5[0] = 1;
    for (int i=1;i<m;i++) p5[i] = p5[i-1]*5;
    cur[0] = 0;
    for (int i=0;i<n;i++)
    {
        for (int j=0;j<a[i];j++)
        {
            vector<int> nxt(mx*m,INF);
            for (int msk=0;msk<mx;msk++)
            {
                ...
                if (j==0)
                {
                    ...
                    {
                        mn = min(mn,cur[msk*m+prv]);
                    }
                    for (int p=0;p<m;p++)
                    {
                        if (kek[p]>=b[p]) continue;
                        nxt[(msk+p5[p])*m+p] = min(nxt[(msk+p5[p])*m+p],mn+c[i][p]);
                    }
                    continue;
                }
                for (int prv=0;prv<m;prv++)
                {
                    ...
                    {
                        if (kek[p]>=b[p]) continue;
                        if (p!=prv) nxt[(msk+p5[p])*m+p] = min(nxt[(msk+p5[p])*m+p],cur[msk*m+prv]+c[i][p]);
                        else nxt[(msk+p5[p])*m+p] = min(nxt[(msk+p5[p])*m+p],cur[msk*m+prv]);
                    }
                }
            }
            swap(cur,nxt);
