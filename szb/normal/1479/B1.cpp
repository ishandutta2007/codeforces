...
ll query(ll l,ll r,ll p,ll s,ll t){
	...
	else return max(query(l,mid,p<<1,s,mid),query(mid+1,r,p<<1|1,mid+1,t)); 
}
...
//		cout<<min(min(min(ask(1,a[i]-1)+1,ask(a[i]+1,n+1)+1),ask(a[i],a[i])),ask(a[i-1],a[i-1])+(a[i]!=a[i-1]))+cur<<endl;
		modify(1,n+1,1,a[i-1],max(max(max(ask(1,a[i]-1)+1,ask(a[i]+1,n+1)+1),ask(a[i],a[i])),ask(a[i-1],a[i-1])+(a[i]!=a[i-1]))+cur-total);
	}
