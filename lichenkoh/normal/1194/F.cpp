...
int main() 
{
  ...
  init();
  int num; ll T; cin>>num>>T;
  for (int i=1;i<=num;i++) cin>>t[i];
  ll sumt=0;
  mint sumbinom=1; int lower=0;
  mint final=0;
  for (int n=1;n<=num;n++) {
    {
      sumbinom=sumbinom*2 - binom(n-1,lower);
    }

    sumt+=t[n];
    ll left=T-sumt; // left is strictly decreasing
    if (left>=n) final+=1;
    else if (left>=0) {
      while(left<lower) {
        sumbinom-=binom(n,lower); lower--;
      }
      while(left>lower) {
        lower++; sumbinom+=binom(n,lower);
      }
      //mint got=sumbinom(n,left);
      //mint got=0;
      //for (int x=0;x<=left;x++) got+=binom(n,x);
      mint got=sumbinom;
      got=got*(((mint(2))^n).inverse());
      final+=got;
    }
  }
  printf("%d\n",final.get());
