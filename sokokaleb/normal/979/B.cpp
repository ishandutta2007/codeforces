...
const vector<string> ANS = {"Kuro", "Shiro", "Katie"};
string s[3];
int n;
int cnt[3][256];
int mx[3];
	...
	FOR (i, 0, 3) {
		...
		FORN (z, 0, 255) {
			if (cnt[i][z] == MAX) {
				...
				int sisa = MAX - cnt[i][z];
				ans = max(ans, cnt[i][z] + min(sisa, n));
			}
		}
	}

	int itung = count(mx, mx + 3, *max_element(mx, mx + 3));
	if (itung > 1) {
		cout << "Draw\n";
	} else {
		cout << ANS[distance(mx, max_element(mx, mx + 3))] << "\n";
	}
}