...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        int n = in.readInt();
        auto s = in.readArray<int, int>(n);

        arr<pii> cur(2 * (n - 1));
        auto quad = [](const pii& a) -> int {
            if (a.second == 0) {
                if (a.first < 0) {
                    return 0;
                } else {
        ...
        ll answer = (ll(n * (n - 1)) * ll((n - 2) * (n - 3)) * (n - 4)) / 24;
        for (int i : range(n)) {
            int at = 0;
            for (int j : range(n)) {
                if (i != j) {
                    cur[at++] = {s[j].first - s[i].first, s[j].second - s[i].second};
                }
            }
            sort(cur.begin(), cur.begin() + (n - 1), cmp);
            for (int j : range(n - 1)) {
                cur[j + n - 1] = cur[j];
            }
            at = 0;
            for (int j : range(n - 1)) {
                ...
                int yy = -cur[j].second;
                while (at < cur.size()) {
                    int cq = quad(cur[at]) + (at >= n - 1 ? 4 : 0);
                    if (cq > qq) {
                        break;
                    }
                    if (ll(yy) * cur[at].first < ll(cur[at].second) * xx) {
                        break;
                    }
                    at++;
                }
                int q = at - j - 1;
                answer -= (ll(q * (q - 1)) * (q - 2)) / 6;
            }
        }
