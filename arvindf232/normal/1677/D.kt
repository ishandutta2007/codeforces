...
fun solve(n:Int, k:Int, L:IntArray):Int {

    if((n-1 downTo n-1-k+1).any{L[it] != 0 && L[it] != -1}){
        return 0
    }
    var ret = 1
...
//    for(steps in 0..4){
//        val all = permutation.all(5)
//        val ans = mutableMapOf<List<Int>,Int>()
//        for(p in all){
//            val pp = permutation(p.p.copyOf())
//            repeat(steps){
//                pp.actonce()
//            }
//
//            val new = pp.key().toList()
//            ans[new] = ans.getOrDefault(new,0) + 1
//        }
//
//        for(p in all){
//            val k = p.key().toList()
//            val ans1 = solve(5,steps,k.toIntArray())
//            val ans2 = ans.getOrDefault(k,0)
//
//            just dei "$ans1 $ans2"
//            assert(ans1 == ans2)
//        }
