...
fn run() {
    ...
    let mut prime = vec![true; w + 1];
    let mut mobius = vec![M::one(); w + 1];
    for i in 2..=w {
        if prime[i] {
            for j in (1..=(w / i)).rev() {
                cnt[j] += cnt[j * i];
    ...
    for i in 1..=n {
        pow[i] = pow[i - 1] * M::new(2);
    }
    let mut coprime = M::zero();
    for (c, mobius) in cnt.iter().zip(mobius.iter()).skip(1).filter(|p| !p.1.is_zero()) {
        coprime += (pow[*c] - M::one()) * *mobius;
    }
    let way = pow[n - 1] - M::one() - coprime;
    let mut ans = M::from(n) * (way + pow[n] - M::one() - pow[n - 1] + M::one());
    for (c, mobius) in cnt.iter().zip(mobius.iter()).skip(2).filter(|p| *p.0 > 0 && !p.1.is_zero()) {
        ans += M::from(*c) * (pow[*c] - M::one()) * *mobius;
    }
    println!("{}", ans);
}
