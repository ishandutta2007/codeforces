...
int dfs(int now,int from){
	...
	for(auto e:graph[now]){
		...
		if(used[e.v]){
			if(used[e.v]<used[now]){
				cycles.push_back(vector<int>());
				xx=cycles.size()-1;
				cycles[xx].push_back(e.cost);
			}else{
				continue;
			}
		}else{
...
int main() {
	...
	ll M;cin>>M;
	for(auto&aneed:needs){
		aneed=aneed<=0?0:(aneed-1)/M+1;
	}
	vector<ll>costs(N);
	for(int i=0;i<N;++i)cin>>costs[i];
	multiset<ll>cost_set;
	int now=0;
	ll answer=0;
	for(int i=0;i<N;++i){
		cost_set.emplace(costs[i]);
		for(int k=now;k<needs[i];++k){
			if(cost_set.empty()){
				answer=-1;
				break;
			}else{
				answer+=*cost_set.begin();
				cost_set.erase(cost_set.begin());
			}
		}
		if(needs[i]>=now)now=needs[i];
		if(answer==-1)break;
	}
	cout<<answer<<endl;
	return 0;	
