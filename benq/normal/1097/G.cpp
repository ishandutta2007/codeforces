...
void addOne(vi& v) {
    v.pb(0);
    SUB(v[0],1); v.resz(min(sz(v),k+1));
    FORd(i,1,sz(v)) AD(v[i],v[i-1]);
    AD(v[0],1);
}

vi comb(vi a, vi b) {
    vi c(min(sz(a)+sz(b)-1,k+1));
...
void dfs(int a, int b) {
    dp[a] = {1};
    trav(x,adj[a]) if (x != b) {
        dfs(x,a); addOne(dp[x]); // considers all 2^n possibilities
        tot -= dp[x];
        dp[a] = comb(dp[a],dp[x]);
    }
    trav(x,dp[a]) MUL(x,2);
...
int main() {
    ...
    dfs(1,0);
    int ans = 0;
    FOR(i,1,min(k+1,sz(tot))) {
        int m = 0;
        for (int j = i; j > 0; --j) {
            int t = po(j,k);
            MUL(t,com[i][j]);
            if ((i-j)&1) MUL(t,MOD-1);
            AD(m,t);
        }
        AD(ans,mul(m,tot[i]));
    }
