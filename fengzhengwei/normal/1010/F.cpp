...
poly F[xx];
vector<poly>v[xx];
pair<poly,poly> cdq(int l,int r,const vector<poly>&v)
{
	if(l==r)return make_pair(v[l],v[l]);
//	cerr<<l<<' '<<r<<"%\n";
//	v[0].print();
//	v[1].print();
	int mid=l+r>>1;
	pair<poly,poly>L=cdq(l,mid,v);
	pair<poly,poly>R=cdq(mid+1,r,v);
	return make_pair(L.first*R.first,L.second+L.first*R.second);
}
void dfss(int x,int y)
{
	top[x]=y;
	if(son[x])dfss(son[x],x),swap(v[x],v[son[x]]);
	F[x]=poly({1});
	for(int i=h[x];i;i=e[i].next)
	{
		if(e[i].to==son[x]||e[i].to==f[x])continue;
		dfss(e[i].to,e[i].to);
		// 
		F[x]=F[e[i].to]+poly({1});
	}
	F[x]<<=1,v[x].push_back(F[x]);
...
//	F[x].print();
	if(x==y)
	{
		...
		F[x]=cdq(0,v[x].size()-1,v[x]).second;
		//F 
	}
//	cout<<x<<"!\n";
//	F[x].print();
...
signed main(){
	...
	ll m=read();
	ssm[0]=1;
	for(int i=1;i<=n;i++)ssm[i]=ssm[i-1]*((m+1+i-1)%mod)%mod;
	for(int i=1;i<n;i++)
	{
		int a=read(),b=read();
		add(a,b),add(b,a);
...
//	F[1].print();
	for(int i=1;i<F[1].size();i++)
		ans+=F[1][i]*ssm[i-1]%mod*ni[i-1]%mod,ans%=mod;
	cout<<ans<<"\n";
	pc('1',1);
	return 0;
}