...
int in[111];
int tu[111];
const int mod=1e9+9,maxn=1e4+10;
int fac[maxn+10],ifac[maxn+10],iv[maxn];
int qpow(int a,int b=mod-2)
{
    int ans=1;
    while(b)
...
void dfs(int x,int fa)
{
	...
	fp v[x] pf
	{
		...
		up(i,0,siz[x])
		{
			up(j,0,siz[to])
			{
				g[i+j]+=dp[x][i]*dp[to][j]%mod*C(i+j,i)%mod;
				g[i+j]%=mod;
			}
		}
		siz[x]+=siz[to];
		up(i,0,siz[x])dp[x][i]=g[i];
	}
	
	dp[x][siz[x]]=dp[x][siz[x]-1];//must last 
}
signed main()
{
	pre();iv[0]=1;
			...
			if(tu[i]&&!vis[i])
			{
				...
				if(!rt)
				{
					fp pt pf
					{
						...
						dfs(to,0);
						up(i,0,siz[to])b[i]+=dp[to][i],b[i]%=mod;
					}
					up(i,0,(int)pt.size())b[i]*=iv[(int)pt.size()-i],b[i]%=mod;
				}
				else
				{
					memset(dp,0,sizeof(dp));
					dfs(rt,0);
					up(i,0,siz[rt])b[i]=dp[rt][i];
				}
				memset(g,0,sizeof(g));
				up(ii,0,n)
				{
					up(j,0,n)
					{
						g[ii+j]+=b[ii]*f[j]%mod*C(ii+j,j)%mod;
						g[ii+j]%=mod;
					}
				}
				up(ii,0,n)f[ii]=g[ii];
