...
void add(int x,int p){++x;while(x<=n)a[x]+=p,x+=x&-x;}
int operator[](int x){++x;int res=0;while(x)res+=a[x],x-=x&-x;return res;}
int operator()(int l,int r){return (*this)[r]-(*this)[l-1];}}t;
int rnv[1000005];
#define COMB
#ifdef COMB
#ifndef int
...
signed main()
{
	...
	b[0]=0;
	rg(n)b[i]=b[i-1]+a[i];gr
	std::reverse(a+1,a+1+n);
	rg(n)c[i]=c[i-1]+a[i];gr
	std::reverse(c+1,c+1+n);
	std::reverse(a+1,a+1+n);
	// rg(n)odb(a[i]);gr puts("");
	// rg(n)odb(b[i]);gr puts("");
	// rg(n)odb(c[i]);gr puts("");
	// fflush(stdout);
	if(b[n]==0)
	{
		odl(qp(2,n-1));
	...
	while(l<=r)
	{
		...
		if(l<=r)
		{
			...
			int p=0;
			for(int pq=0;pq<=i-l;pq++)mae(p,me(C(i-l,pq),C(r-j,pq)));
			mle(ns,p);
			l=i,r=j;
			// l++,r--;
		}
