...
int main() {
	...
	vector<Mint> fact(n + 7, Mint(1, MOD998)), inv = fact;
	for (int i = 2; i <= n; i++)
		fact[i] = fact[i - 1] * i;
	inv[n] = fact[n].inv();
	for (int i = n; i >= 2; i--)
	...
	vector<vector<Mint>> binom(50, vector<Mint>(n + 7, Mint(0, MOD998)));
	for (int k = 0; k < 50; k++)
		for (int i = 1; i <= n; i++)
			binom[k][i] = C(cnt[i] - k, i - k) + binom[k][i - 1];

	auto get = [&](int k, int l, int r) {
		if (l > r)
			return Mint(0, MOD998);

		return binom[k][r] - binom[k][l - 1];
	};

	Mint ans = Mint(0, MOD998);
	for (int mask = 0; mask < (1 << m); mask++) {
		int coef = (__builtin_popcount(mask) % 2 == 0 ? +1 : -1);
		...
		for (int i = 0; i < m; i++) {
			if (mask & (1 << i)) {
				taken.pb(b[i].fi);
				taken.pb(b[i].se);
				chkmax(L, a[b[i].fi - 1].fi);
				chkmax(L, a[b[i].se - 1].fi);
				chkmin(R, a[b[i].fi - 1].se);
				chkmin(R, a[b[i].se - 1].se);
			}
		}

		distinct(taken);
		const int sz = taken.size();
