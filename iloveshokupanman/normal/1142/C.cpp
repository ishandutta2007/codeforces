...
int main() {
    ...
    rep(i, N) {
        ...
        X[i].second -= X[i].first * X[i].first;
    }

    sort(X.begin(), X.end());

    vector<P> nx;
    rep(i, N)if (i == N - 1 || X[i].first < X[i + 1].first) { nx.push_back(X[i]); }
    X = nx;

    //for (auto P : X) { cout << P.first << " " << P.second << endl; }
    ...
    for(ll i=1;i<X.size();i++) {
        ...
        while (st.size() > 1) {
            ...
            ll dif1 = (B.first - A.first) * (C.second - B.second);
            ll dif2 = (C.first - B.first) * (B.second - A.second);
