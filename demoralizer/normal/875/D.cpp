...
int last[32]; // last[i] = last index with ith bit on
...
void calc(int l,int r){
	...
	int mx = t.query(l,r).id;
	
	calc(l,mx - 1);
	calc(mx + 1, r);
	
	if(lef[mx] >= l)
		ans += (lef[mx] - l + 1) * (r - mx + 1);
	if(righ[mx] <= r)
		ans += (mx - l + 1) * (r - righ[mx] + 1);
	if(righ[mx] <= r && lef[mx] >= l)
		ans -= (lef[mx] - l + 1) * (r - righ[mx] + 1);
