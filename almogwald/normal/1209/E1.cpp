...
int maxfunc(int pos, veci cur) {
	...
	fori(i, n) {
		if (d[pos][j] <= cur[(j + i) % n]) {
			con
		}
		veci curr = cur;
		fori(k, n) {
			curr[(i + k) % n] = max(curr[(i + k) % n], d[pos][k]);
		}
		sum = max(sum, maxfunc(pos + 1, curr));
	}
	return sum;
}
int main() {
