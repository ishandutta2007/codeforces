...
public:
	...
	RollingHash(const T &s) : n(s.size()) {
		hash.assign(n + 1, 0);
		power.assign(n + 1, 1);
		for(int i = 0; i < n; i++) {
...
void main_() {
	STR(s, t);
	int n = SZ(s);
	RollingHash rh(s), sub(t);
	auto hash = sub.get(0, SZ(t));
	vector dp(n + 1, mint(0)), sum(n + 1, mint(0)), sum2(n + 1, mint(0));
	REPS(i, n) {
		if(i - SZ(t) >= 0 and rh.get(i - SZ(t), i) == hash)
			dp[i] = sum2[i - SZ(t)] + i - SZ(t) + 1;
		else
			dp[i] = dp[i - 1];
		sum[i] = sum[i - 1] + dp[i];
