...
int main() {
	...
	scanf("%d", &n);
	int h1 = ceil_div(n, 2);
	REP(i, h1) scanf("%d", a + i);
	scanf("%d", &x);
	REP(i, h1) psum[i + 1] = psum[i] + a[i];
	REP(i, h1 + 1) {
		ll val = psum[i];
		ll beg = psum[h1];
		ll c = val - beg - x * 1ll * (i - h1);
		debug(arr(psum, i), imie(c), imie(beg), imie(val));
		ll r = n - i, l = ceil_div(n, 2);
		bool any = true;
		if (x > 0) {
			ll here = floor_div(c, x);
			debug(imie(here));
			mini(r, here);
		}
		else if (x == 0) {
	...
	for (int i = 0; i <= n; ++i) {
		cou += banned[i];
		if (i > floor_div(n, 2) && cou == 0) {
			debug(imie(i), "is good");
			ans = i;
		}
	}
	printf("%d\n", ans);
