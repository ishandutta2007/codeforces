...
void solve() {
	...
	int dif=-1,best=0;
	FOR(i,N) {
		FORR(a,add[i]) st.update(i,a,-1);
		FORR(a,del[i]) st.update(a,i,1);
		x=st.getval(0,N)-st.getval(i,i+1);
		if(x>dif) dif=x, best=i;
	}
	
	vector<int> V;
	FOR(i,M) if(L[i]<=best&&best<R[i]) V.push_back(i+1);
	cout<<dif<<endl;
	cout<<V.size()<<endl;
	FORR(v,V) cout<<v<<" ";
	cout<<endl;
