...
fun Graph.eulermultipleedges():List<List<Int>> {

    val S = fastDeque(0,n+m)
    val newhead = head.copyOf()
    val used = BooleanArray(m)
    val inverted = BooleanArray(m)
    val parentEdge = fastDeque(0,n+m)
    val retm = mutableListOf<List<Int>>()

    for(start in 0 until n){
        if(newhead[start] == -1) continue
        S.addLast(start)
        parentEdge.addLast(-1)
        val edgehere = mint
        first@while(S.isNotEmpty()){
            var v = S.last()
            while(newhead[v] != -1){
                val e = newhead[v]
...
//                just dei "$e : ${from[e]} ${to[e]} away from $v"
                ...
                S.addLast(to[e])
                parentEdge.addLast(e)
                continue@first
            }
            S.removeLast()
            val e = parentEdge.removeLast()
            if(e != -1){
                edgehere.add(e)
            }
        }
...
fun Array<CharArray>.transpose():Array<CharArray>{
    val ret = Array(this[0].size){CharArray(this.size)}
    for(i in this.indices){
        for(j in this[0].indices){
            ret[j][i] = this[i][j]
        }
...
fun main(){
    solve.cases{
        ...
        val G = Graph( 2* n , n,false)
        for((a,b) in all){
            G.add(a-1,b-1)
        }
        val newhead = G.head.copyOf()
        val used = BooleanArray(n)
        var edges = mint
        fun explore(v:Int){
            while(newhead[v] != -1){
                val newe = newhead[v]
                newhead[v] = G.next[newhead[v]]
                if(used[newe shr 1]) continue
                used[newe shr 1] = true
                val to = G.to[newe]
                edges.add(newe)
                explore(to)
                edges.add(newe xor 1)
            }
        }
        val ret = mutableListOf<List<Int>>()
        for(now in 0 until 2 * n){
            if(newhead[now] == -1) continue
            edges = mint
            explore(now)
            ret.add(edges)
        }
        val board = mutableListOf<IntArray>()
        val ret1 = mutableListOf<CharArray>()
        val ret2 = mutableListOf<CharArray>()
        for(c in ret){
            if(c.size == 2){
                put(-1)
                return@cases
            }
            val that = c.map { G.from[it] }
            val half = c.size / 2
            val top = that.slice(0 until half).toIntArray()
            val bottom = that.slice(half until c.size).reversed().toIntArray()
            val got = arrayOf((top), (bottom ))
            val t = got.transpose()
            for(c in t){
                board.add(c)
            }
            val type2:Array<CharArray>
            val type1:Array<CharArray>
            if(half %2 == 0){
                val t1 = CharArray(half){if(it == 0) 'U' else if(it == half-1) 'U' else if(it and 1 == 1) 'L' else 'R'}
                val t2 = CharArray(half){if(it == 0) 'D' else if(it == half-1) 'D' else if(it and 1 == 1) 'L' else 'R'}
                val t3 = CharArray(half){if(it and 1 == 0) 'L' else 'R'}
                val t4 = CharArray(half){if(it and 1 == 0) 'L' else 'R'}
                type1 = arrayOf(t1,t2).transpose()
                type2 = arrayOf(t3,t4).transpose()
            }else{
                val t1 = CharArray(half){if(it == 0) 'U' else if(it and 1 == 1) 'L' else 'R'}
                val t2 = CharArray(half){if(it == 0) 'D' else if(it and 1 == 1) 'L' else 'R'}
                val t3 = CharArray(half){if(it == half-1) 'U' else if(it and 1 == 0) 'L' else 'R'}
                val t4 = CharArray(half){if(it == half-1) 'D' else if(it and 1 == 0) 'L' else 'R'}
                type1 = arrayOf(t1,t2).transpose()
                type2 = arrayOf(t3,t4).transpose()
            }
            for(c in type1){
...
//        val G = Graph(4 *n, 2 * n,false)
//        for((a,b) in all){
//            G.add(a-1,b-1+ 2 * n )
//            G.add(a-1 + 2 * n , b - 1 )
//        }
//        val degree = G.degree()
//        if(degree.any{it %2 != 0}){
//            put(-1)
//            return@cases
//        }
//        val circuits = G.eulermultipleedges()
...
//            val top = that.slice(0 until half).toIntArray()
//            val bottom = that.slice(half until c.size).reversed().toIntArray()
//            val got = arrayOf((top + top), (bottom + bottom))
//            val t = got.transpose()
//            for(c in t){
//                board.add(c)
//            }
...
//        }
        ...
        for(a in board.toTypedArray().transpose()){
            put(a.map{it+1}.map{if(it > 2 * n) it - 2 * n else it }.conca())
        }

        put("")
        for(c in ret1.toTypedArray().transpose()){
            put(c.conca)
        }
        put("")
        for(c in ret2.toTypedArray().transpose()){
            put(c.conca)
        }
