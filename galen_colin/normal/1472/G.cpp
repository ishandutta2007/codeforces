...
vector<ll> vedges[200005];
vector<ll> redges[200005];
ll deg[200005];
bool vis[200005];
ll temp[200005];
dijkstra d;
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) {
		for (ll j: edges[i]) {
			if (a[i] < a[j]) {
				vedges[j].push_back(i);
				++deg[i];
			} else {
				redges[i].push_back(j);
			}
		}
	}
	
	for (ll it = 0; it < 2; it++) {
		for (ll i = 0; i < n; i++) c[i] = deg[i];
		...
		if (it == 0) {
			for (ll i = 0; i < n; i++) temp[i] = b[i];
			
			for (ll i = 0; i < n; i++) {
				for (ll y: redges[i]) {
					temp[i] = min(temp[i], b[y]);
				}
			}
			
			for (ll i = 0; i < n; i++) b[i] = temp[i];
		}
	}
