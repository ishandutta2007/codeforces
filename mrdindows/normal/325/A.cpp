...
int main()
{
    ...
    for (int t=0;t<61;t++)
    {
        if (t<30)
        {
            ...
            ll b = pow - 3;
            ll c = -2*n;
            long double d = ((long double)(b) )* b - 4 * a * c;
            long double x = (-b + sqrt(d))/2;
        //  DB(t); DB(d); DBN(x);
            if (x == (ll) (x+0.0001))
            {
                ll y = (ll)(x+0.0001);
                if (y&1)
                    ans.push_back(y * (1LL<<t));
            }
        }
        else
        {
    ...
    */
    ...
    //for (int num = 1;num<(1<<n);num++)
    {
        ...
        for (int i=0;i<n;i++)
        {
        //  if (!checkbit(num,i)) continue;
            s+=abs(x[i][0]-x[i][1])*abs(y[i][0]-y[i][1]);

            for (int j=0;j<4;j++)
            {
                if (x[i][j]<mx[0] || x[i][j]==mx[0] && y[i][j]<my[0])
                {
                    mx[0] = x[i][j];
                    my[0] = y[i][j];
                }

                if (y[i][j]<my[1] || y[i][j]==my[1] && x[i][j]>mx[1])
                {
                    mx[1] = x[i][j];
                    my[1] = y[i][j];
                }
                ...
                if (x[i][j]>mx[2] || x[i][j]==mx[2] && y[i][j]>my[2])
                {
                    mx[2] = x[i][j];
                    my[2] = y[i][j];
                }

                if (y[i][j]>my[3] || y[i][j]==my[3] && x[i][j]<mx[3])
                {
                    mx[3] = x[i][j];
                    my[3] = y[i][j];
                }
        ...
        if (abs(mx[1] - mx[0]) == abs(my[2]-my[1]) && mx[0]==mx[3] && mx[1] == mx[2] && my[0] == my[1] && my[2] == my[3] && s == abs(mx[1]-mx[0])*abs(my[2]-my[0]))
        {
            cout<<"YES"<<endl;
        //  DBN(num);
            return 0;
