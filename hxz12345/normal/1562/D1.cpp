...
/* --------------- fast io --------------- */ // end
long long l,r,now,Testing,n,T,i,a[1001010],pre1[1010100],pre2[1001010];
char S[1001010];
int main()
{
	long long Testing;
	...
	for (;Testing;Testing--)
	    {
	    	cin>>n>>T;cin>>S+1;
	    	for (i=1;i<=n;i++) if (S[i]=='+') a[i]=1;else a[i]=-1;
	    	for (i=1;i<=n;i++)
	    	   {
	    	   	if (i % 2==1) pre1[i]=pre1[i-1]+a[i],pre2[i]=pre2[i-1]-a[i];
	    	   	else pre1[i]=pre1[i-1]-a[i],pre2[i]=pre2[i-1]+a[i];
			   }
	    	for (;T;T--)
	    	   {
	    	   	cin>>l>>r;now=0;
	    	   	if (l % 2==1) now=pre1[r]-pre1[l-1];
	    	   	else now=pre2[r]-pre2[l-1];
	    	   	if (now!=0) 
				    {
					if ((r-l+1)&1) puts("1");
				   else puts("2");
