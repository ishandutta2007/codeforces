...
void solve() {
    ...
    vector<mint> sum(n + 1, 0), power(n + 1, 1);
    for (int i = 0; i < n; i++) power[i + 1] = power[i] * 2;
    for (int i = 0; i < n; i++) sum[i + 1] = sum[i] * 2 + ((S[i] - '0') ^ 1);

    auto get = [&](int l, int x) {
        mint res = sum[l + x] - sum[l] * power[x];
        return int(res.value());
    };
    auto restore = [&](int x, int s) {
        string res(s, '0');
        for (int i = 0; i < s; i++) {
            if (x >> i & 1) {
                res[i]++;
    ...
    string ans = "";
    if (k < MAX) {
        for (int i = 0; i + k <= n; i++) val.emplace_back(get(i, k));
        sort(val.begin(), val.end());
        int x = 0;
        for (int i = 0;; x++) {
            while (i < val.size() && val[i] < x) i++;
            if (i == val.size() || x < val[i]) break;
        }
        if (x == (1 << k)) {
            cout << "NO" << '\n';
            return;
        ...
        for (int one = 0, i = 0; i + MAX <= n; i++) {
            if (one >= k - MAX) val.emplace_back(get(i, MAX));
            if (S[i] == '1')
                one++;
            else
                one = 0;
