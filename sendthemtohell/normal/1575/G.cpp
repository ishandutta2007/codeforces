...
inline void solve() {
    ...
    for (int i = 2; i < N; ++i) {
        ...
        phi[i] = phi[c] * (md[i] - 1) * (i / md[i] / c);
        
        for (auto d : divs[c]) {
            for (ll res = 1; res <= i / c; res *= md[i]) {
                divs[i].push_back(d * res);
            }
        }
    }
    ...
    vector<int> v(n);
    //for (int i = 0; i < n; ++i) v[i] = rd() % 100000;
    cin >> v;
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (auto j : useful) {
            sm = (sm + ll(phi[j]) * phi[i] % MOD * cnt[j] % MOD * cnt[j] % MOD) % MOD;
        }
    }
    cout << sm << endl;
}
