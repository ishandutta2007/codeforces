...
int main(int QwQcOrZ,char*Recall_902_[]){
	...
	ll n=readll(),m=readll(),x,y,sum=0,z=0,cnt=0;t[1]=1;rep(i,1,n)f[i]=i;
	rep(i,1,m)x=readll(),y=readll(),x^y?t[x]=t[y]=1,f[f(x)]=f(y),d[x]^=1,d[y]^=1:t[x]=1;
	rep(i,1,n)s[f(i)]+=d[i];rep(i,1,n)t[i]&&i==f(i)&&(cnt++,z=s[i],sum+=s[i]?:2);
	printf("%lld\n",cnt==1?z/2:sum/2);
	return 0;
}