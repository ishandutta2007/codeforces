...
signed main()
{
    ...
    for (int j = 1; j < 20; j++)
    {
        ...
        for (int i = 0; i < s.size(); i++)
        {
            v.push_back({{sp[j - 1][i], sp[j - 1][(i + (1 << (j - 1))) % (int)s.size()]}, i});
        }
        sort(all(v));
        int x = 1;
    ...
    for (int i = 0; i < s.size(); i++)
    {
        ...
        {
            int j = suffixes[suff[i] + 1];
            while (max(i + x, j + x) < s.size() && s[i + x] == s[j + x])
                x++;
            lcp[suff[i]] = x;
        }
    }
    ...
    while (Sz > 0)
    {
        while (l > 0 && lcp[l - 1] >= Sz)
            l--;
        while (r < s.size() && lcp[r] >= Sz)
            r++;
        if (len[l] == Sz)
            ans.push_back({Sz, r - l + 1});
        Sz--;
    }
    sort(all(ans));
    cout << ans.size() << endl;
