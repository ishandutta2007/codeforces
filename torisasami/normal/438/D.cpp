...
int main() {
    ...
    SegmentTree<ll> segma(
        n, [](ll a, ll b) { return max(a, b); }, 0);
    rep(i, n) seg.set(i, a[i]), segma.set(i, a[i]);
    seg.build(), segma.build();
    while (m--) {
        int type;
        cin >> type;
        if (type == 1) {
            ...
            cin >> l >> r >> x;
            l--;
            while (segma.query(l, r) >= x) {
                int idx = segma.find_first(l, [&](ll val) { return val >= x; });
                seg.update(idx, seg[idx] % x);
                segma.update(idx, seg[idx] % x);
            }
        } else {
            int k, x;
            cin >> k >> x;
