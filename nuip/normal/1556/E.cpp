...
int main(){
	...
	vector<ll> sums(n+1);
	rep(i,n) sums[i+1]=sums[i]+as[i]-bs[i];
	const ll INF=1e15;
	RMQ mxs(sums,-INF);
	RmQ mns(sums,INF);
	cauto solve=
		[&](const int &l, const int &r)->ll{
			if(sums[l]!=sums[r]) return -1;
			cauto zero=sums[l];
			cauto mx=mxs.get(l,r-1)-zero;
			cauto mn=mns.get(l,r-1)-zero;
			if(mx>0) return -1;
			return -mn;
		};
	for(cauto &[l,r]:qs) cout<<solve(l,r) NL;
	return 0;
}