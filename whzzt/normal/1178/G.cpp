...
const int N=2e5+5;
int n,q,ia[N],ib[N],a[N],b[N],in[N],ldfn[N],rdfn[N],dfc,fa[N];
vector<int> adj[N];
void dfs(int x){
	ldfn[x]=++dfc;
	a[ldfn[x]]=ia[x]+a[ldfn[fa[x]]];
	b[ldfn[x]]=ib[x]+b[ldfn[fa[x]]];
	for(int y:adj[x]) if(y!=fa[x]) fa[y]=x,dfs(y);
	rdfn[x]=dfc;
}

const int B = 200, T = N / B + 5;
ld cl(int i, int j){ return (ld)(i64(a[i]) * b[i] - i64(a[j]) * b[j]) / (b[j] - b[i]); }
ld ch(int i, int j){ return (ld)(i64(a[j]) * b[j] - i64(a[i]) * b[i]) / (b[i] - b[j]); }
i64 get_v(int i, int x){ return i64(a[i] + x) * b[i]; }
i64 fget_v(int i, int x){ i64 w = get_v(i, x); return w < 0 ? -w : w; }
struct block{
	int lb, rb, x;
	int id[B + 5], n;
	int ql[B + 5], nl, pl, qr[B + 5], nr, pr;
	void build(){
		n = 0;
		for (int i = lb; i <= rb; ++i) id[n++] = i;
		sort(id, id + n, [&](int x, int y){ return b[x] < b[y] || b[x] == b[y] && a[x] < a[y]; });
		nr = 0;
		for (int i = 0; i < n; ++i) {
			if (i + 1 < n && get_v(id[i + 1], 0) >= get_v(id[i], 0)) continue;
			while (nr > 1 && ch(qr[nr], qr[nr - 1]) >= ch(id[i], qr[nr])) qr[nr--] = 0;
			qr[++nr] = id[i];
		}
		pr = 1;
		n = 0;
		...
		for (int i = 0; i < n; ++i) {
			if (i + 1 < n && get_v(id[i + 1], 0) <= get_v(id[i], 0)) continue;
			while (nl > 1 && cl(ql[nl], ql[nl - 1]) >= cl(id[i], ql[nl])) ql[nl--] = 0;
			ql[++nl] = id[i];
		}
		pl = 1;
	}
	...
	i64 query(){
		while (pl != nl && get_v(ql[pl + 1], x) <= get_v(ql[pl], x)) ++pl;
		while (pr != nr && get_v(qr[pr + 1], x) >= get_v(qr[pr], x)) ++pr;
		return max(fget_v(ql[pl], x), fget_v(qr[pr], x));
	}
} st[T];

void modify(int l, int r, int u){
	for (int k = in[l]; k <= in[r]; ++k) {
		int ml = max(l, st[k].lb);
		int mr = min(r, st[k].rb);
		if (ml == st[k].lb && mr == st[k].rb) {
			st[k].up(u);
		} else {
			st[k].add(ml, mr, u);
		}
...
i64 query(int l, int r){
	i64 res = 0;
	while (l <= r && l != st[in[l]].lb) chkmax(res, fget_v(l, st[in[l]].x)), ++l;
	while (r >= l && r != st[in[r]].rb) chkmax(res, fget_v(r, st[in[r]].x)), --r;
	if (l <= r) for (int k = in[l]; k <= in[r]; ++k) chkmax(res, st[k].query());
	return res;
}
...
int main(){
	...
	for(int x=1;x<=n;++x) cin>>ia[x];
	for(int x=1;x<=n;++x) cin>>ib[x];
	dfs(1);
	for(int x=1;x<=n;++x) b[x]*=sgn(b[x]),in[x]=x/B;
	for(int x=1;x<=n;++x) st[in[x]].rb=x;
	for(int x=n;x>=1;--x) st[in[x]].lb=x;
	for(int b=in[1];b<=in[n];++b) st[b].build();
	while(q--){
		int op;
		cin>>op;
		if(op==1){
			int x,v;
			cin>>x>>v;
			modify(ldfn[x],rdfn[x],v);
		}else{
			int x;
			cin>>x;
			cout<<query(ldfn[x],rdfn[x])<<'\n';
		}
	}
	return 0;
}