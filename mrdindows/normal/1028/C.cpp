...
template<int val>
void add(Node*& cur, int l, int r, int x, int y) {
    if (x > r || y < l) return;
    if (!cur) cur = lastn++;
    if (x <= l && r <= y) {
        cur->maxVal += val;
        ...
        int m = l + (r - l) / 2;
        add<val>(cur->lp, l, m, x, y);
        add<val>(cur->rp, m + 1, r, x, y);
        cur->maxVal = max(getMax(cur->lp), getMax(cur->rp)) + cur->pu;
    }
}
int n;
int curX;
void findMax(Node* cur, int l, int r, int pu) {
    pu += cur->pu;
    if (pu >= n - 1) {
        cout << curX << ' ' << l << endl;
        exit(0);
    ...
    int m = l + (r - l) / 2;
    if (getMax(cur->lp) >= getMax(cur->rp)) {
        findMax(cur->lp, l, m, pu);
    } else {
        findMax(cur->rp, m + 1, r, pu);
    }
}
...
int solve() {
    ...
    FI(n) {
        ...
        events.push_back({a[0], 1, a[1], a[3]});
        events.push_back({a[2] + 1, -1, a[1], a[3]});
    }
    sort(all(events));
    for (auto& e : events) {
        curX = e[0];
        if (e[1] == 1) {
            add<1>(root, -T9, T9, e[2], e[3]);
        } else {
            add<-1>(root, -T9, T9, e[2], e[3]);
        }
        if (getMax(root) == n - 1) {
            findMax(root, -T9, T9, 0);
        }
    }
