...
//		sop(anser);
	...
	static int anser;
	static void poss(int i, long ca, long cb)
	{
			if((ca>=a&&cb>=b)||(ca>=b&&cb>=a))
			{
				anser=Math.max(anser, i);
			}
			else if(i>0&&i-1>anser)
			{
				if(ca>=a&&ca>=b)
				{
					poss(i-1, ca, cb*arr[i-1]);
				}
				else if(cb>=a&&cb>=b)
				{
					poss(i-1, cb, ca*arr[i-1]);
				}
				else
				{
					if(arr[i-1]==2&&(cb<a&&cb<b))
					{
						poss(i-1, ca, cb*arr[i-1]);
					}
					else if(arr[i-1]==2&&(ca<a&&ca<b))
					{
						poss(i-1, cb, ca*arr[i-1]);
					}
					else
