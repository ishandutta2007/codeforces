...
struct Seg{
	struct Cht{
		...
		int val(Lin p, Lin q, Lin r){
			ld pq = ld(p.b - q.b) / (q.a - p.a);
			ld pr = ld(p.b - r.b) / (r.a - p.a);
			return pq < pr;
		}
		void u(int a, int b){
			//printf("u %d %d\n", a, b);
			Lin t = {a, b};
			while(c > 1 && !val(v[c-2], v[c-1], t)){ c--; v.pop_back(); }
			v.push_back(t); c++;
		}
		...
		int g(int i){
			//printf("%d : %d\n", i, c);
			if(!c) return int(1e9);
			while(c > 1 && f(c-1, i) > f(c-2, i)){ c--; v.pop_back(); }
			return f(c-1, i);
		}
	} d[2*SZ];
	void cl(){ for(int i = 1; i < 2*SZ; i++){ d[i].v.clear(); d[i].c = 0; } }
