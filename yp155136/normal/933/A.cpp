...
int count_1(int L,int R)
{
    if (L>R) return 0;
    return pre[R] - pre[L-1];
}

int count_2(int L,int R)
{
    if (L>R) return 0;
...
int main () {
    ...
    {
        pre[i] = pre[i-1] + (a[i] == 1);
        //ans = max(ans,count_1(1,i) + count_2(i+1,n));
        //ans = max(ans,count_1(1,i-1) + count_2(i,n));
    }
    REP1(i,n)
    {
        ans = max(ans,count_1(1,i) + count_2(i+1,n));
    ...
    for (int i=1;n>=i;i++)
    {
        ...
        for (int j=1;i>=j;j++)
        {
            L = max(L,count_1(1,j-1) + count_2(j,i));
            L = max(L,count_1(1,j) + count_2(j+1,i));
        }
        for (int j=i+1;n>=j;j++)
        {
            R = max(R,count_1(i+1,j-1) + count_2(j,n));
            R = max(R,count_1(i+1,j) + count_2(j+1,n));
        }
        ans = max(ans,L+R);
    }
    Pi(ans);
