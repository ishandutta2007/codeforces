...
Mint calc(int r, int c) {
    ...
    REP (i, r) {
	if (L[i] <= c) ret -= X[i] * F(r-i-1, c-L[i]);
	if (R[i] <= c) ret -= Y[i] * F(r-i, c-R[i]);
    }
    return ret;
}
...
void MAIN() {
    ...
    reverse(L, L+N);
    reverse(R, R+N);
    REP (i, N-1) amax(L[i+1], L[i]);
    for (int i=N-2; i>=0; i--) amin(R[i], R[i+1]);
