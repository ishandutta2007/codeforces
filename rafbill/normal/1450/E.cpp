...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) D[i][j] = INF;
...

  {
    vi E(n);
    vi C(n);
    auto dfs = letrec([&](auto dfs, li i, li c) {
      if(E[i]) {
        if(C[i] != c) fail();
        return;
      }
  ...
  for(li i = 0; i < (li)(n); ++i) if(D[i][i]<0) fail();

  li ans = -1;
  li bi = 0;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) {
    if(D[i][j]>ans) { ans = D[i][j]; bi = i; }
  }

  cout << "YES" << endl;
  cout << ans << endl;
  for(li j = 0; j < (li)(n); ++j) cout << D[bi][j] << ' ';
  cout << endl;
