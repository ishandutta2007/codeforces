...
main()
{
    ...
    for (int i = 0; i < w; i++)
    {
        v[i].k = in;
        if (v[i].k <= 2) v[i].a = in;
        else v[i].a = in, v[i].b = in, v[i].c = in;
    }

    for (int i = 0; i < w; i++)
    {
        if (v[i].k == 1) horz(v[i].a - 1, 1);
            else if (v[i].k == 2) vert(v[i].a - 1, 1);
                    else if (v[i].k == 3) q[v[i].a - 1][v[i].b - 1] = v[i].c;
    }
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++) out.put(q[i][j], " ");
        out = endl;
    }
