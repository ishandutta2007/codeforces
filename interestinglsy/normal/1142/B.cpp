...
il void Read( auto p[] , int be , int ed ){
	Forx(i,be,ed) Read(p[i]);
}
...
int mxst[MAXN];
void Init(){
	static int lastpos[MAXN];
	p[0] = p[n];
	For(i,n) posinp[p[i]] = i;
	// Print(posinp,1,n);
	For(i,m){
		int u = a[i];
		b[i][0] = lastpos[p[ posinp[u] - 1 ]];
		For(j,LOG-1) b[i][j] = b[b[i][j-1]][j-1];
		lastpos[u] = i;
	}
	For(i,m){
		int left = n-1;
		int pos = i;
		For0(j,LOG){
			if(left&1) pos = b[pos][j];
			left >>= 1;
		}
		mxst[i] = max(mxst[i-1],pos);
	}
	// Print(mxst,1,m);
}
