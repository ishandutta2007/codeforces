...
int main() {
  ModNum cnp = fpow(~2m, N);
  for (int i = N - 1; i > 0; ) {
    __builtin_prefetch(np2 + i - 16, 1, 2);
    for (int nd = i - 16; i != nd; --i) {
      np2[i] = cnp += cnp;
    }
  }
  for (int testcase = $; testcase; --testcase) {
    int n = $, m = $, k = $;
    __builtin_prefetch(np2 + n - 1, 0, 0);
    ModNum ans{};
    for (int i = 0; i < m; ++i) {
      ans += fact(i) * fact(n - i) * (m - i);
    }
    print(ans * k * np2[n - 1] * fact[n]);
  }
}