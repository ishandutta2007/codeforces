...
struct Dp {
    ll sum = 0, left = 0, right = 0, inside = 0;
};

template <class It>
Dp calc_dp(It it_begin, It it_end) {
    Dp dp{};
    for (It it = it_begin; it != it_end; ++it)
        dp.sum += *it, dp.left = max(dp.left, dp.sum);
    dp.sum = 0;
    for (It it = it_end; it != it_begin; --it)
        dp.sum += *prev(it), dp.right = max(dp.right, dp.sum);
    ll val = 0;
    for (It it = it_begin; it != it_end; ++it) {
        val += *it;
        dp.inside = max(dp.inside, val);
        val = max(val, 0ll);
    }
    return dp;
}
...
int main() {
    constexpr int N = 18, D = 12, W = 1 << D;
    int n; read(n);
    vector a(1 << n, 0); read(a);
    int q; read(q);
    ...
    if (n < D) {
        while (q--) {
            ...
            rep(i, 1 << n)
                if ((i >> k) & 1)
                    swap(a[i], a[i ^ (1 << k)]);
            println(calc_dp(all(a)).inside);
        }
        return 0;
    }

    int b = 1 << (n - D);
    vector<array<Dp, W>> block(b);
    rep(i, b) {
        int l = i << D, r = (i + 1) << D;
        rep(j, W) {
            int g = gray_code(j);
            block[i][g] = calc_dp(begin(a) + l, begin(a) + r);
            if (j + 1 < W) {
                int bit = __builtin_ctz(g ^ gray_code(j + 1));
                rep(k, W)
                    if ((k >> bit) & 1)
                        swap(a[l + k], a[l + (k ^ (1 << bit))]);
            }
        }
    }
    ...
    while (q--) {
        ...
        if (k < D)
            ...
            rep(i, b)
                if ((i >> k) & 1)
                    swap(idx[i], idx[i ^ (1 << k)]);
        }

        ll mx = 0, cur = 0;
        rep(i, b) {
            int j = idx[i];
            mx = max({mx, cur + block[j][low].left, block[j][low].inside});
            cur = max({0ll, block[j][low].right, cur + block[j][low].sum});
        }
