...
int main() {
    TEST {
        ...
        vvv(int, idx, n, m);
        rep(i, n) rep(j, si(s[i])) { idx[i][v[i][j]].eb(j); }
        ...
        int dummy = si(g);
        g.eb(vi());
        rep(i, si(g) - 1) g[dummy].eb(i);

        rep(b, 1 << n) {
            rep(i, m) {
                bool flag = true;
                rep(k, n) {
                    if(b & 1 << k and si(idx[k][i]) <= 1 or empty(idx[k][i])) flag = false;
                }
                if(!flag) continue;
                int N = b + (i << n);
                g[st].eb(N);
                ...
                rep(t, m) {
                    ...
                    int bb = 0;
                    rep(k, n) {
                        int x = ub(idx[k][t], idx[k][i][(b >> k & 1)]);
                        if(x == si(idx[k][t])) flag = false;
                        bb |= (x << k);
                    }
                    if(flag) {
                        int NN = bb + (t << n);
        ...
        for(auto i : res) {
            if(i == dummy) continue;
            for(auto e : g[i]) {
                if(chmax(dp[e], dp[i] + 1)) pre[e] = i;
            }
        }
        int k = max_element(all(dp)) - begin(dp);
        string ans;
        while(dp[k]) {
            int s = (k >> n);
            if(s >= 26)
