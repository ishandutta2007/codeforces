...
using mi=modnum<mod>;
const int N=5005,M=1e6+5,Q=1e6+5;
int t,n,p[N];
int lv[N][N],rv[N][N];
ll ans;
void solve(){
	...
	for(int i=0;i<=n+1;i++) for(int j=0;j<=n+1;j++) lv[i][j]=rv[i][j]=0;
	for(int i=1;i<=n;i++) for(int j=i+1;j<=n;j++){
		if(p[i]<p[j]) lv[j][i]++;
		if(p[i]>p[j]) rv[i][j]++;
	}
	for(int i=1;i<=n;i++){
		for(int j=n;j>=1;j--) rv[i][j]+=rv[i][j+1];
		for(int j=1;j<=n;j++) lv[i][j]+=lv[i][j-1];
	}
	ans=0;
	for(int i=1;i<=n;i++) for(int j=i+1;j<=n;j++) ans+=1ll*lv[j][i-1]*rv[i][j+1];
	cout<<ans<<"\n";
}
int main(){
