...
int main() {
    ...
    while(ret) {
        ...
        used[l] = used[d[l]] = 1;
        l1 = l, r1 = d[l];
        s.erase(l), s.erase(d[l]);
        ret--;
        int t = 0;
        while(min(r1, r2) - max(l1, l2) - 1 != ret * 2) {
            if(r1 < l2) drop(-1);
            // dump(l1, r1, l2, r2, ret);
            t ^= 1;
            auto it = s.lower_bound(r1);
            if(it != end(s)) {
                while(used[r]) r--;
                int L = d[r], R = r;
                s.erase(L), s.erase(R);
                used[L] = used[R] = 1;
                if(l2 > L or r2 < R) drop(-1);
                l2 = L, r2 = R;
                same += !a[R];
            } else {
                while(used[l]) l++;
        ...
        mi += min(same, c - same), ma += max(same, c - same);
        // dump(mi, ma);
    }
    cout << min(mi, n - ma) << endl;
}