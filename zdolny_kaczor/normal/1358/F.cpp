...
const char rev = 'R', diffs = 'P';
void output(vector <pair <char, ll> > seq) {
	reverse(ALL(seq));
	ll sum = 0;
	for (auto [c, v] : seq) if (c == diffs) sum += v;
	if (sum > 200000)
		done("BIG\n%lld\n", sum);
	for (auto [c, v] : seq) if (c == rev) sum += v;
	printf("SMALL\n%lld\n", sum);
	for (auto [c, v] : seq) REP(_, v) putchar(c);
	done("\n");
}
int main() {
	int n;
	...
	if (n == 2) {
		...
		while (a != b) {
			...
			if (b[0] == 0) nope();
			if (b[0] == a[0]) {
				if (b[1] < a[1] || (b[1] - a[1]) % b[0]) nope();
				ans.emplace_back(diffs, (b[1] - a[1]) / b[0]);
				break;
			}
			else {
				ans.emplace_back(diffs, b[1] / b[0]);
				b[1] %= b[0];
				debug(imie(b));
			}
		}
		if (last) ans.emplace_back(rev, 1);
	}
	else {
		while (a != b && !equal(a.begin(), a.end(), b.rbegin())) {
			debug(imie(a), imie(b));
			debug(imie(b), *min_element(ALL(b)));
			if (*min_element(ALL(b)) < 1) nope();
			if (is_sorted(ALL(b), less<ll>())) {
				for (int i = n - 1; i; --i)
					b[i] -= b[i - 1];
				if (!ans.empty() && ans.back().first == diffs) ans.back().second++;
				else ans.emplace_back(diffs, 1);
			}
			else if (is_sorted(ALL(b), greater<ll>())) {
				ans.emplace_back(rev, 1);
				reverse(ALL(b));
			}
			else nope();
		}
		if (a != b) ans.emplace_back(rev, 1);
	}
	output(ans);
}
//#STAY AT HOME