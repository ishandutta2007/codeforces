...
int main() {
	CASE(t) {
		...
		while (r > l) {
			...
			vector<pii> seg;
			for (auto& i : a) {
				if (i.first + i.second + 1 < mid) continue;

				int minv = max(0, mid - 1 - i.first);
				int maxv = min(mid - 1, i.second);

				seg.push_back({ minv, maxv });
			}

			int cur = 0;
			for (int i = 0; i < (int)seg.size(); i++)
				if (seg[i].first <= cur && cur <= seg[i].second) cur++;
