...
var n = 0
var notes:IntArray = IntArray(0)

class sn(val n:Int, val s:Int):Comparable<sn>{
    val fret = notes[n] - S[s]
    override fun compareTo(other:sn):Int {
        return if(this.fret != other.fret) this.fret - other.fret  else this.n - other.n
    }
    fun next():sn?{
        if(s != 5) return sn(n,s+1)
        return null
    }
}
...
fun main(){
    solve.cases{
        ...
        val allnotes = TreeSet<sn>()
        var min = Int.MAX_VALUE
        for(minnote in 0 until n){
            allnotes.add(sn(minnote,0))
        }
        while(allnotes.isNotEmpty()){
            val first = allnotes.first()
            val last = allnotes.last()
            min = minOf(min, last.fret - first.fret)
            val next = first.next()
            allnotes.remove(first)
            if(next == null) break
            allnotes.add(next)
//            next?.let{allnotes.add(it)}
        }
