...
auto Main = [](){
	...
	For(TwT,T){
		...
		auto [resS,resK] = [&](){
			...
			auto NewAnswer = [&]( const string &str , int k ){
				if( ans.F > str or ( ans.F == str and ans.S > k ) ){
					ans.F = str;
					ans.S = k;
				}
			};
			...
			Forx(k,2,n){
				string afterSwap = [&]( int k ){
					string res = s.substr(k-1);
					if((n-k)&1) res += s.substr(0,k-1);
					else{
				...
				NewAnswer(afterSwap,k);
			}

			return ans;
		}();

		cout << resS << endl << resK << endl;
	}
