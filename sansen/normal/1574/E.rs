...
fn run() {
    input! {
        ...
        w: usize,
        q: usize,
        ask: [(usize1, usize1, i32); q],
    }
    let mut pow = vec![M::one(); h + w + 1];
    for i in 1..pow.len() {
        pow[i] = pow[i - 1] + pow[i - 1];
    }
    let mut row = vec![[0; 2]; h];
    let mut row_ban = 0;
    let mut row_free = h;
    let mut col = vec![[0; 2]; w];
    let mut col_ban = 0;
    let mut col_free = w;
    let mut fix = [0; 2];
    let ban = |p: &[usize; 2]| -> bool { p.iter().filter(|p| **p > 0).count() > 1 };
    let mut state = Map::new();
    let out = std::io::stdout();
    let mut out = std::io::BufWriter::new(out.lock());
    for (x, y, op) in ask {
        if op != -1 {
            if let Some(op) = state.insert((x, y), op) {
                let p = op as usize ^ (y % 2);
                if ban(&row[x]) {
                    row_ban -= 1;
                }
                row[x][p] -= 1;
            ...
            fix[op as usize ^ (x % 2) ^ (y % 2)] += 1;
        } else if let Some(op) = state.remove(&(x, y)) {
            let p = op as usize ^ (y % 2);
            if ban(&row[x]) {
                row_ban -= 1;
            }
        ...
        if (row_ban, col_ban) == (0, 0) {
            ans -= M::from(fix.iter().filter(|f| **f == 0).count());
        }
        writeln!(out, "{}", ans).ok();
    }
}
