...
cin>>n;
for(int i=0;i<n;i++){cin>>x;myst.insert(x);}
if(myst.size()>3)return cout<<-1,0;
if(myst.size()==3){int on=*myst.begin();int tw=*(++myst.begin());int th=*(--myst.end());
if(tw-on==th-tw)cout<<tw-on;else cout<<-1;}
if(myst.size()==2){int on=*myst.begin();int th=*(--myst.end());
if((th-on)%2==0)cout<<(th-on)/2;else cout<<th-on;}
if(myst.size()==1)cout<<0;
