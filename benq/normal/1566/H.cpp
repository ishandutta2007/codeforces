...
int main() {
	...
	re(answers);
	// dbg(to_query);
	// vi answers; for (int x: to_query) answers.pb(simulate(x));
	// dbg(answers);

	ans = vi(C+1,-1);
	ans[1] = answers[0];
	FOR(i,1,sz(to_query)) // all numbers divisible by 2
		ans[to_query[i]] = answers[i]^answers[0];
	// F0R(i,C+1) if (ans[i] != -1) dbg(i,ans[i]);
	// dbg("---");
	...
	// 	int xo = 0;
	// 	vi vals;
	// 	each(t,cand) if (rng()&1) xo ^= t, vals.pb(t);
	// 	if (xo == 0 && sz(vals)) dbg(vals);
	// }
	...
	// }
	// dbg(cnt);
	// FOR(i,1,C+1) dbg(i,group[i]);


	// F0R(i,C+1) if (ans[i] != -1) dbg(i,ans[i]);

	// xor of those that divide
	// 2|x
	// 3|x


	// FOR(i,1,sz(answers)) answers[i] ^= answers[0];
