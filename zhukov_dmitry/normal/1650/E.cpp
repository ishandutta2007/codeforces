...
int check(int idx)
{
	...
	ford(i, n)
	{
		...
		memcpy(e, a, (n + 1) * sizeof(a[0]));
		int t = ind[i];
		if (t - 1 != idx && t != idx) e[idx] = midpoint(a[t - 1], a[t]);
		else if (t == idx) e[idx] = (t < n ? midpoint(a[t - 1], a[t + 1]) : m);
		else e[idx] = midpoint(a[t - 2], a[t]);
		uax(ans, calc());
	}
	memcpy(e, a, (n + 1) * sizeof(a[0]));
	int t = idx;
	e[idx] = (t < n ? midpoint(a[t - 1], a[t + 1]) : m);
	uax(ans, calc());
	return ans;
}
