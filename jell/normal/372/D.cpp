#line 1 "codeforces-workspace\\contest\\372\\d\\d.cpp"
...
#line 6 "codeforces-workspace\\contest\\372\\d\\d.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace\\contest\\372\\d\\d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\372\\d\\d.cpp"
...
class heavy_light_decomposition {
  ...
  std::vector<size_t> sorted, in, out, head, __dpt;
  ...
  void traverse(size_t node, size_t prev) {
    ...
    if (!tree[node].empty() && tree[node].front() != prev) {
      for (const size_t to : tree[node])
        if (to != prev) head[to] = node + size(), __dpt[to] = __dpt[node] + 1;
 ...
 public:
  ...
  heavy_light_decomposition(size_t n = 0)
      : tree(n), in(n), out(n), head(n), __dpt(n) {}
  ...
  size_t ancestor(size_t __v, size_t __k) const {
    ...
    while (__k) {
      assert(in[__v]);
      auto __t = top(__v);
      auto __d = in[__v] - in[__t];
      if (__d < __k) {
        __k -= __d + 1;
        __v = head[__t] - size();
      } else {
        __v = sorted[in[__v] - __k];
        __k = 0;
      }
    }
    return __v;
  ...
  size_t depth(size_t __v) const { return __dpt[__v]; }

  size_t distance(size_t __u, size_t __v) const {
    return __dpt[__u] + __dpt[__v] - __dpt[lca(__u, __v)] * 2;
  }
...
#line 27 "codeforces-workspace\\contest\\372\\d\\d.cpp"
...
void main() {
  ...
  auto vs = make_set<int>(
      [&](auto u, auto v) { return hld.index(u) < hld.index(v); });

  auto insert = [&](int v) -> auto {
    if (vs.empty()) {
      vs.emplace(v);
      return;
    }

    auto pos = vs.insert(v).first;

    if (pos == vs.begin()) {
      len -= hld.distance(*prev(vs.end()), *next(pos));
      len += hld.distance(*prev(vs.end()), v);
      len += hld.distance(*next(pos), v);
    } else if (next(pos) == vs.end()) {
      len -= hld.distance(*prev(pos), *vs.begin());
      len += hld.distance(*prev(pos), v);
      len += hld.distance(*vs.begin(), v);
    } else {
      len -= hld.distance(*prev(pos), *next(pos));
      len += hld.distance(*prev(pos), v);
      len += hld.distance(*next(pos), v);
    }
  };

  auto erase = [&](int v) -> auto {
    if (vs.size() == 1) {
      vs.clear();
      return;
    }
