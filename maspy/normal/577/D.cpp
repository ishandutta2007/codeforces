...
void solve() {
  ...
  FOR(v, N) if (!done[v]) {
    ...
    FORIN(c, C) done[c] = true;
    CYCS[len(C)].eb(C);
  }
  if (len(CYCS[1]) >= 1) {
    ll v = CYCS[1][0][0];
    YES();
    FOR(x, N) if (x != v) { print(v + 1, x + 1); }
  }
  elif (len(CYCS[2]) == 0) { NO(); }
  else {
    auto C = CYCS[2][0];
    auto a = C[0], b = C[1];
    FOR(k, N + 1) if (k % 2 == 1 && len(CYCS[k])) return NO();
    YES();
    print(a + 1, b + 1);
    FOR(k, N + 1) FORIN(C, CYCS[k]) {
      FOR(i, len(C)) {
        if (i % 2 == 0 && C[i] != a && C[i] != b) print(a + 1, C[i] + 1);
        if (i % 2 == 1 && C[i] != a && C[i] != b) print(b + 1, C[i] + 1);
      }
    }
  }
}
