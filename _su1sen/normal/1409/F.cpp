...
int main() {
    ...
    if (t[0] == t[1]) {
        ...
        vec<vec<int>> dp(n + 1, vec<int>(k + 2, -IINF));
        dp[0][0] = 0;
        rep(i, n) rrep(j, i + 1) rrep(l, k + 1) {
            chmax(dp[j + 1][l + (s[i] != t[0])], dp[j][l]);
            chmax(dp[j][l + (s[i] != t[1])], dp[j][l] + j);
        }
        rep(j, n + 1) rep(l, k + 1) chmax(ans, dp[j][l]);
    }
    print(ans);
    return 0;
}