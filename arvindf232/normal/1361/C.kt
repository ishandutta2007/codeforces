...
fun Graph.eulerCircuit():IntArray? {
    ...
    S.addLast(havesomething)

    val retv = mint
    parentEdge.addLast(-1)
    first@while(S.isNotEmpty()){
        assert(S.size == parentEdge.size)
        var v = S.last()
        while(newhead[v] != -1){
            val e = newhead[v]
...
fun main(){
    solve.cases{
        ...
        fun ok(level:Int):Boolean {
            val here = IntArray(1 shl level)
        ...
        val L = (0..20).last { ok(it) }


        for(tryL in L downTo 0){
            val G = Graph(1 shl tryL, all.size,false)
            val mask = (1 shl tryL) - 1
            for((i,p) in all.withIndex()){
                val (a,b) = p
                G.add(a and mask, b and mask)
            }
            val circuit = G.eulerCircuit()
...
didn't know euler circuit is different
didn't read code
