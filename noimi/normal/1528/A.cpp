...
int main() {
    TEST {
        ...
        VEC(pii, a, n);
        auto g = getG(n);
        auto res = REC([&](auto &&f, int x, int p) -> pll {
            pll res;
            for(auto e : g[x]) {
                if(e == p) continue;
                auto [t, s] = f(e, x);
                res.fi += max(t + abs(a[x].fi - a[e].fi), s + abs(a[x].fi - a[e].se));
                res.se += max(t + abs(a[x].se - a[e].fi), s + abs(a[x].se - a[e].se));
            }
            return res;
        })(0, -1);
        cout << max(res.fi, res.se) << endl;
    }
}