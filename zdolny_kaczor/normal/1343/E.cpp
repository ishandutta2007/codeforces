...
int v[nax];
int adist[nax], bdist[nax], cdist[nax];
vi graf[nax];
void bfs(int n, int x, int res[]) {
	memset(res, 60, (n + 5) * sizeof(res[0]));
	res[x] = 0;
	queue<int> bfs_queue;
	bfs_queue.push(x);
	while (!bfs_queue.empty()) {
		...
		int d = res[v];
		bfs_queue.pop();
		for (int w : graf[v]) if (res[w] > n) {
			res[w] = d + 1;
			bfs_queue.push(w);
		}
	}
	debug(imie(x), range(res, 1, n + 1));
}
ll pre[nax];
int main() {
	int t;
	...
	REP(_, t) {
		...
		for (int v = 1; v <= n; ++v) {
			int common = bdist[v], left = adist[v] + cdist[v];
			debug(imie(v), imie(common), imie(left));
			if (common + left > m) continue;
			mini(ans, pre[common] + pre[common + left]);
		}
		printf("%lld\n", ans);
