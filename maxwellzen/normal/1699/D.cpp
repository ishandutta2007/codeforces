...
void solve() {
    ...
    dp[0] = 1;
    FOR(i, 1, n) {
        if (a[i]==a[i-1] && dp[i-1] != 0) dp[i] = dp[i-1]+1;
        if (clear[0][i-1]) ckmax(dp[i], 1);
        F0R(j, i-1) if (a[j]==a[i] && dp[j] != 0 && clear[j+1][i-1]) ckmax(dp[i], dp[j]+1);
    }
    debug(dp);
    int ans = 0;
    F0R(i, n) if (i==n-1 || clear[i+1][n-1]) ckmax(ans, dp[i]);
    cout << ans << nl;
}
