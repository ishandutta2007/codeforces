...
main() {
    INT(n, a, b);
    vi dp(n + 1);
    for(int i = a; i <= n; i += a) { dp[i] = 1; }
    for(int i = b; i <= n; i += b) dp[i] = 1;
    rep(i, n - b + 1) { dp[i + b] += dp[i]; }
    if(!dp[n]) {
        cout << -1 << endl;
    } else {
        int s, t;
        if(b < a) swap(a, b);
        for(int i = 0; i * a <= n; i++) {
            if((n - i * a) % b == 0) s = i, t = (n - i * a) / b;
        }
        vi ans(n);
        rep(i, s) {
            rep(j, a) { ans[i * a + j] = i * a + (j + 1) % a + 1; }
        }
        rep(i, t) {
            rep(j, b) { ans[i * b + j + s * a] = i * b + s * a + (j + 1) % b + 1; }
        }
        print(ans);
    }
}