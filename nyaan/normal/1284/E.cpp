...
void solve(){
  ...
  rep(i , N){
    // i
    V<Real> lst;
    rep(j , N) if(i != j) lst.pb( atan2(a[j].real() - a[i].real() , a[j].imag() - a[i].imag() ) );
    sort(all(lst));
    rep(i , N - 1) lst.pb(lst[i] + 2 * pi);
    // (N-1)(N-2)(N-3)(N-4)/4!
    ll cur = 1LL * (N-1) * (N-2) * (N-3) * (N-4) / 24;
    
    for(int i = 0 ; i < N - 1 ; i++){
      auto it = upper_bound(begin(lst) + i , begin(lst) + i + N - 1 , lst[i] + pi);
      int num = it - (begin(lst) + i);
      assert(num > 0);
      // num - 13
      cur -= 1LL * (num - 1) * (num - 2) * (num - 3) / 6;
    }
    ans += cur;
  }
