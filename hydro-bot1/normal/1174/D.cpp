// Hydro submission #62c67382c3a9fda5622ac200@1657172866878
...
int main()
{
    int N, X; 
    cin >> N >> X;
    std::vector <int> vis((1 << N) + 1, 0), P;
    if(X >= (1LL << N)){
        cout<< (1 << N) - 1 << endl;
        for(int i = 0;i < (1 << N) - 1;i++) cout<< (i ^ (i + 1)) << ' ';
        return 0;
    }
    for(int i = 0;i < (1 << N);i++){
        if(vis[i]) continue;
    ...
    for(int i = 0;i < (int)P.size() - 1;i++) cout << (P[i + 1] ^ P[i]) << ' ';
}