...
//	for(int i=0;i<n;i++)
//	{
//		for(int j=0;j<m;j++) scanf("%d",a[i]+j),buf[cnt++]=a[i][j];
//		scanf("%d",w+i);
//	}
//	sort(buf,buf+cnt);cnt=unique(buf,buf+cnt)-buf;
//	for(int i=0;i<n;i++) for(int j=0;j<m;j++) a[i][j]=lower_bound(buf,buf+cnt,a[i][j])-buf;
//	int res=inf+inf;
//	for(int i=0;i<cnt;i++) buf[i]=i;
//	while((clock()-st)/CLOCKS_PER_SEC<=2.9)
//	{
//		random_shuffle(buf,buf+cnt);
//		memset(ww,0x3f,sizeof(int)*cnt);
//		for(int i=0;i<n;i++)
//		{
//			int l=inf,r=0;
//			for(int j=0;j<m;j++)
...
//			chmin(ww[r],w[i]);
//		}
//		for(int i=1;i<cnt;i++) chmin(ww[i],ww[i-1]);
//		for(int i=0;i<n;i++)
//		{
//			if(!t[i] || ww[t[i]-1]>=inf) continue;
//			chmin(res,ww[t[i]-1]+w[i]);
//		}
//	}
//	cout<<(res==inf+inf?-1:res)<<endl;
//	return 0;
//}
