...
void chemthan() {
    ...
    int mxt = n + k + 1;
    vector<vi> id(n, vi(mxt));
    int tot = 0;
    FOR(i, 0, n) FOR(j, 0, mxt) {
        id[i][j] = tot++;
    }
    int s = tot++, t = tot++;
    MCMF::init(tot, s, t);
    FOR(i, 0, k) {
        MCMF::add(s, id[a[i]][0], 1, 0);
    }
    FOR(i, 0, mxt - 1) {
        FOR(u, 0, n) {
            MCMF::add(id[u][i], id[u][i + 1], k, c);
        }
        for (auto e : edges) {
            int u = e.fi;
            int v = e.se;
            FOR(it, 0, 2) {
                FOR(j, 0, k) {
                    MCMF::add(id[u][i], id[v][i + 1], 1, (2 * j + 1) * d + c);
                }
                swap(u, v);
            }
        }
