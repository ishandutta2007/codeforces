...
vector<Mint> fac, ifac, ipow2;
void precompute() {
	const int MAXN = 1000007;
	
	fac.resize(MAXN + 1, Mint(1, MOD107));
	ifac.resize(MAXN + 1, Mint(1, MOD107));
	for (int i = 2; i <= MAXN; i++) fac[i] = fac[i - 1] * i;
	ifac[MAXN] = fac[MAXN].inv();
	for (int i = MAXN; i >= 3; i--) ifac[i - 1] = ifac[i] * i;

	const Mint inv2 = Mint(2, MOD107).inv();
	ipow2.resize(MAXN + 1, Mint(1, MOD107));
	for (int i = 1; i <= MAXN; i++)
		ipow2[i] = ipow2[i - 1] * inv2;
}
...
Mint f(int n, int r) {
	if (n == r) return Mint(n, MOD107);

	Mint ans(0, MOD107);
	for (int t = 0; t <= r; t++)
		ans += C(n - t - 1, r - t) * t * ipow2[n - t];
	return ans;
}
