...
inline pair<poly,poly> orz(int l,int r){
	if(l==r)return make_pair((poly){1,mod-l},(poly){0,1});
	else{
		auto x=orz(l,(l+r)>>1),y=orz(((l+r)>>1)+1,r);
		y.second.a.insert(y.second.a.begin(),(l+r>>1)+1-l,0);
		return make_pair(x.first*y.first,x.second*y.first+y.second);
	}
}
int n,k,S[200002],mu[200002];
bool vis[200002];
int main() {
	...
	A.resize(n);
	auto temp=orz(1,k);
	A=A.inv()*(temp.first+temp.second);
	F(i,0,n)S[i]=A[i];
	F(i,1,200000)mu[i]=1;
	F(i,2,200000){
		if(!vis[i]){
