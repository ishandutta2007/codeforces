...
vector<int> degs{1, 2, 3, 5, 9, 17};
vector<int> nb{1, 1, 2, 4, 8, 16};
...
int main() {
    ...
    vector<mint> ap;
    for (auto d : degs) ap.push_back(A.pow(d));

    const int N = 1 << k;

    auto zen = [&](int L, int R, int winner) {
        unordered_map<int, vector<int>> mp;
        vector<int> is(R - L);
        iota(ALL(is), L);
        vector<int> ds{winner};
        FOR(i, 2, degs.size()) {
            REP(t, nb[i] / 2) ds.push_back(i);
        }
        vector<int> v;
        do {
            bool good = true;
            ...
            if (!good) continue;
            mint sum = 0;
            REP(i, ds.size()) sum += is[i] * ap[ds[i]];
            mp[sum.val] = ds;
        } while (next_permutation(ALL(ds)));
        return mp;
    };
    auto L0 = zen(1, N / 2 + 1, 0);
    auto L1 = zen(1, N / 2 + 1, 1);
    auto R0 = zen(N / 2 + 1, N + 1, 0);
    auto R1 = zen(N / 2 + 1, N + 1, 1);

    auto matching = [&](const auto &L, const auto &R) {
        for (auto [hl, vl] : L) {
            int tgt = (mint(h) - hl).val;
            if (R.count(tgt)) {
                auto vr = R.at(tgt);
                auto v = vl;
                v.insert(v.end(), ALL(vr));
                for (auto x : v) {
                    cout << degs[x] << ' ';
                }
                cout << '\n';
