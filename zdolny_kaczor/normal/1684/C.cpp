...
void solve() {
	...
	REP(_, n) {
		...
		sort(ALL(b));
		vi diff;
		REP(i, m) if (a[i] != b[i]) diff.push_back(i);
		if (diff.size() > 2u) err = true;
		if (diff.size() == 2)
			which.emplace_back(diff[0], diff[1]);
		board.push_back(move(a));
	}
	debug(imie(which), dump(board, 0, n, 0, m));
	if (which.size() && which != vpii(which.size(), which[0])) err = true;
	if (err) {
		puts("-1");
		return;
	}
	...
	for (auto &x : board) {
		...
		bool good = true;
		debug(imie(x));
		for (int k = 0; k + 1 < m; ++k) if (x[k] > x[k + 1]) good = false;
		if (!good) err = true;
	}
	if (err) puts("-1");
	else printf("%d %d\n", i + 1, j + 1);
