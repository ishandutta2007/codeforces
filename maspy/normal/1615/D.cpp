...
void solve() {
  ...
  FOR(i, N - 1) {
    ll a = G[i][0], b = G[i][1], c = G[i][2];
    if (c == -1) continue;
    ll v = popcnt(c) % 2;
    if (v == 0) {
      uf.merge(2 * a + 0, 2 * b + 0);
      uf.merge(2 * a + 1, 2 * b + 1);
    }
    if (v == 1) {
      uf.merge(2 * a + 0, 2 * b + 1);
      uf.merge(2 * a + 1, 2 * b + 0);
    }
  }
  ...
  FOR(v, N) if (uf[2 * v + 0] == uf[2 * v + 1]) return NO();

  YES();
  vi A(N + N, -1);
  vc<vi> IDS(N + N);
  FOR(v, N + N) IDS[uf[v]].eb(v);
  FOR(x, N + N) if (uf[x] == x && A[x] == -1) {
    FORIN(v, IDS[x]) {
      A[v] = 0;
      A[v ^ 1] = 1;
    }
