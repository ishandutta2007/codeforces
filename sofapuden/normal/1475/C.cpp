...
void solve(){
	int a, b, k; cin >> a >> b >> k;
	vector<set<int>> A(a),B(b);
	vector<int> p1(k), p2(k);read(p1,p2);
	for(int i = 0; i < k; ++i){
		A[p1[i]-1].insert(p2[i]-1);
		B[p2[i]-1].insert(p1[i]-1);
	}
	ll ans = 0;
	for(int i = 0; i < k; ++i){
		ans+=(k-A[p1[i]-1].size()-B[p2[i]-1].size()+A[p1[i]-1].count(p2[i]-1));
	}
	cout << ans/2 << "\n";
