...
int main(){
  ...
  for(li u = 0; u < (li)(n); ++u) {
    ...
    for(auto const& p : G[u]) proc(get<0>(p), get<1>(p));
    for(li d = 0; d < (li)(n+n); ++d) {
      for(int v : Q[d]) if(D[v] == c0 + d) {
          proc((v+1)%n, c0+d+1);
          for(auto const& q : G[v]) {
            li w, dt; tie(w,dt) = q;
            proc((w+c0+d)%n, c0+d+dt);
          }
        }
    }
    for(li v = 0; v < (li)(n); ++v) cout << (v == u ? 0 : D[v]) << ' ';
    cout << '\n';
  }
  cout << flush;
