...
int main(){
	...
	ls=l.size();rs=r.size();
	magic=((ll)-1e6)*(ls+rs-1);
	//cout << magic << endl;
	mcmf<ll,ll>g(ls+rs+2,0,ls+rs+1);
	for(int i=1; i<=m ;i++){
		int u,v;cin >> u >> v;
		eu[i]=u,ev[i]=v;
	...
	for(int i=0; i<rs ;i++){
		g.add(ls+i+1,ls+rs+1,1,0,-1e6);
		g.add(ls+i+1,ls+rs+1,1e6,0,0);
	}
	auto duh=g.max_flow_min_cost();
	ll ans=duh.se;
	ans+=1LL*(ls+rs)*((ll)1e6);
	if(ans>=1e6) return cout << "-1\n",0;
	else cout << ans << '\n';
	ll dude=0;
	ll hkgov[226];
	hkgov['R']=rc;
	...
	for(int i=1; i<=m ;i++){
		...
		if(c1[u]=='U'){
			if(g.edges[eid[i]].f!=0) ans-=hkgov[c2[v]],cout << c2[v];//cheched
			else cout << "U";
			continue;
		}
		if(c2[v]=='U'){
			if(g.edges[eid[i]].f!=0) ans-=hkgov[c1[u]],cout << c1[u];//cheched
			else cout << "U";
			continue;
		}
		if(c1[u]!=c2[v]){//cheched
			if(c1[u]=='R'){
				if(g.edges[eid[i]].f!=0) ans-=hkgov['B'],cout << 'B';
				else if(g.edges[eid[i]-2].f!=0) ans-=hkgov['R'],cout << 'R';
				else cout << 'U';
			}
			else{
				if(g.edges[eid[i]].f!=0) ans-=hkgov['B'],cout << 'B';
		...
		if(c1[u]=='R'){
			if(g.edges[eid[i]].f!=0) ans-=hkgov['R'],cout << 'R';
			else cout << 'U';
		}
		else{
			if(g.edges[eid[i]].f!=0) ans-=hkgov['B'],cout << 'B';
