// time-limit: 1000
// problem-url: https://codeforces.com/contest/1672/problem/H
...
int main(){
	...
	assert(number==int(s.size()));

	
	std::vector<std::array<int, 2>> count(number); // make it suffix sum array later
	for(auto index: range(0, number-1)){
		if(s[index+1]==s[index]){
			count[index][s[index]-'0']++;
		}
	}

	if(number>1)
	for(int index=number-2; index--;){
	...
	for(int _=numQuery; _--;){
		...
		std::cout<<std::max({
				count[left][0]-count[right-1][0],
				count[left][1]-count[right-1][1],
		})+1<<'\n';
	}
}