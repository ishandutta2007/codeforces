...
void solve() {
  LL(N);
  auto primes = prime_table(1 << 20);
  VEC(ll, A, N);
  sort(all(A));
  reverse(all(A));
  vi COST(N);
  FOR(i, N) {
    auto pf = factor(primes, A[i]);
    FORIN(x, pf) COST[i] += x.se;
  ...
  auto calc = [&](vi& par) -> void {
    FOR(i, N) {
      ll x = A[i];
      FOR3(j, i + 1, N) if (par[j] == i) {
        if (x % A[j] != 0) return;
        x /= A[j];
      }
    }
    ...
    FOR(i, N) if (par[i] != -1 && COST[i] == 0)-- x;
    if (root > 1) ++x;
    x += N;
    chmin(ANS, x);
  };

  auto f = [&](auto dfs, vi& par) -> void {
    ll n = par.size();
    if (n == N) {
      calc(par);
      return;
