...
ll getLD(ll p, ll q, ll i) {
    if (i == 1) return 0x7f7f7f7f7f7f7f7f;
    ...
    return fastPow(i, countP - countQ + 1);
}   
...
int main(int argn, char **argv) {
    ...
    REP(I, t) {
        ...
        ll leastDivider = 0x7f7f7f7f7f7f7f7f;
        bool isPrime = true;
        FOR (i, 1, sqrt(q)) {
            if (q % i == 0) {
                // cout << "i = " << i << endl;
                // cout << "need " << getLD(p, q, i) << endl;
                leastDivider = min(leastDivider, getLD(p, q, i));

                leastDivider = min(leastDivider, getLD(p, q, q / i));
                isPrime = false;
            }
        }

        if (!isPrime) {
            cout << p / leastDivider << endl;
        }
        else {
            cout << p / getLD(p, q, q) << endl;
        }
    }
}