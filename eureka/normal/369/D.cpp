...
int main(){
	...
	zero[n+1] = true, hun[n+1] = false;
	for (int i=n; i>=1; --i){
		zero[i] = zero[i+1] & (p[i] == 0);
		hun[i] = hun[i+1] | (p[i] == 100);
	}
	mem(dp, 0xff);
	dp[1][2] = k;
	for (int i=1; i<=n; ++i){
		for (int j=i+1; j<=n; ++j){
			...
			int t = dp[i][j]-1;
			if (!hun[j] && p[i]!=0) checkmax(dp[i][j+1], t);
			if (!zero[j] && p[i]!=100) checkmax(dp[j][j+1], t);
			if (!zero[j] && p[i]!=0) checkmax(dp[j+1][j+2], t);
		}
	}
	int sum = 0;
	for (int i=1; i<=n+1; ++i){
