...
#define stdin_filepath "/home/8967301548/Documents/stdin.txt"
#define stdout_filepath "/home/8967301548/Documents/stdout.txt"
#define stderr_filepath "/home/8967301548/Documents/stderr.txt"
...
void solve()
{
    int dfs(int,int,set<int>&);
    set<int> all;
    for(int i=1; i<=n; i++)
    {
        all.emplace(i);
    }
    std::cout << dfs(0,n,all) << '\n';
}

int dfs(int l,int r,set<int> &s)
{
    if(l==r)
    {
        return 0;
    }
    const int top=*s.rbegin();
    const int mid=idx[top];
    dump(top,mid);
    s.erase(top);
    set<int> t;
    int ret=0;
