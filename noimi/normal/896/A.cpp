...
char solve(int n,ll k){
    ...
    else if(k < v[n-1] + a.size()){
        return solve(n-1,k-a.size());
    }
    else if(k<v[n-1]+a.size()+b.size()){
        return b[k-v[n-1]-a.size()];
    }
    else if(k<v[n-1]+a.size()+v[n-1]+b.size()){
        return solve(n-1,k-a.size()-v[n-1]-b.size());
    }
    else if(k==v[n-1]*2+a.size()+b.size()+1){
        return '?';
    }
    else return '"';
}
signed main(){
    ...
    v.pb(f.size());
    rep2(i,1,54){
        v.pb(v.back()*2 + a.size() + b.size()+c.size());
    }
    rep(i,100000)v.pb(v.back());
    int q=in();
    while(q--){
        int n=in();ll k=lin()-1;
        cout << solve(n,k) ;
