...
int main(int argc, char const *argv[]) {
  ...
  long long m;
  std::cin >> n >> m;
  std::vector<mint> f0(n + 1), fac(n + 1), inv(n + 1), f(n + 1), g(n + 1);
  for (int i = 0; i <= n; i++) std::cin >> f0[i];
  fac[0] = mint::raw(1);
  for (int i = 1; i <= n; i++) fac[i] = fac[i - 1] * mint::raw(i);
  inv[n] = fac[n].inv();
  for (int i = n - 1; i >= 0; i--) inv[i] = inv[i + 1] * mint::raw(i + 1);
  std::vector<mint> a = f0, b = inv;
  for (int i = 0; i <= n; i++) a[i] *= fac[i];
  std::reverse(b.begin(), b.end());
  auto h = atcoder::convolution(a, b);
  std::copy(h.begin() + n, h.end(), g.begin());
  for (int i = 0; i <= n; i++) g[i] *= mint::raw(i + 1).inv().pow(m);
  for (int i = 1; i <= n; i += 2) g[i] = -g[i];
  h = atcoder::convolution(g, b);
  std::copy(h.begin() + n, h.end(), f.begin());
  for (int i = 0; i <= n; i++) f[i] *= inv[i];
  for (int i = 1; i <= n; i += 2) f[i] = -f[i];
