...
void solve(){
    ...
    each(i,a)used[i]=1;
    ll unused=0;
    rep(n+2)if(!used[i])unused=i;
    unordered_map<ll,vector<ll>>cnt;
    rep(n)cnt[a[i]].push_back(i);
    priority_queue<pll>q;
    each(i,cnt)q.emplace(i.second.size(),i.first);
    rep(x){
        ll at=q.top().second;
        q.pop();
        cnt[at].pop_back();
        q.emplace(cnt[at].size(),at);
    }
    ll siz=n-x;
    while(y<n&&q.top().first*2>siz){
        ll at=q.top().second;
        q.pop();
        a[cnt[at].back()]=unused;
        cnt[at].pop_back();
        q.emplace(cnt[at].size(),at);
        y++;
        siz--;
    ...
    vector<ll>idx,val;
    each(x,v,cnt)each(i,v){
        idx.push_back(i);
        val.push_back(x);
    }
    rotate(idx.begin(),idx.begin()+idx.size()/2,idx.end());
    rep(idx.size()){
        if(a[idx[i]]==val[i]){
            NO();
            return;
