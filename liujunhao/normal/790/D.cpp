...
//template
#define MAXN 300000
int a[2][MAXN+10],n,nxt[2][2][MAXN+10],f[MAXN+10];
vector<pair<pii,int> >g[MAXN+10];
map<LL,int>la;
void read(){
	Read(n);
	int i;
		...
		for(r=l;r<=1;r++){
			...
			for(i=n;i;i--){
				...
				if(la.count(now))
					nxt[l][r][i]=la[now];
				la[now]=i;
				if(!nxt[l][r][i]||(nxt[l][r][i+1]&&nxt[l][r][i]>nxt[l][r][i+1]))
					nxt[l][r][i]=nxt[l][r][i+1];
			}
		}
	for(i=1;i<=n;i++){
		now=f[i];
		if(nxt[0][0][i]){
			f[nxt[0][0][i]]=max(f[nxt[0][0][i]],f[i]+1);
			g[i].push_back(make_pair(pii(nxt[0][0][i],i),f[i]+1));
		}
		if(nxt[1][1][i]){
			f[nxt[1][1][i]]=max(f[nxt[1][1][i]],f[i]+1);
			g[i].push_back(make_pair(pii(i,nxt[1][1][i]),f[i]+1));
		}
		if(nxt[0][1][i]){
			f[nxt[0][1][i]]=max(f[nxt[0][1][i]],f[i]+1);
			g[nxt[0][1][i]].push_back(make_pair(pii(nxt[0][1][i],nxt[0][1][i]),f[i]+1));
		}
		fi=0x7fffffff,fj=0x7fffffff;
		for(auto &x:g[i]){
			if(x.first.first==i&&x.second==f[i]+1)
				fj=min(fj,x.first.second);
			if(x.first.second==i&&x.second==f[i]+1)
				fi=min(fi,x.first.first);
		}
		if(fj<=n+1&&nxt[0][0][i]){
			f[max(nxt[0][0][i],fj)]=max(f[max(nxt[0][0][i],fj)],f[i]+2);
			g[min(nxt[0][0][i],fj)].pb(make_pair(pii(nxt[0][0][i],fj),f[i]+2));
		}
		if(fi<=n+1&&nxt[1][1][i]){
			f[max(nxt[1][1][i],fi)]=max(f[max(nxt[1][1][i],fi)],f[i]+2);
			g[min(nxt[1][1][i],fi)].pb(make_pair(pii(fi,nxt[1][1][i]),f[i]+2));
		}
		f[i+1]=max(f[i],f[i+1]);
	//	cout<<f[i]<<endl;
	}
