...
constexpr ll msk2(int x) { return p2(x)-1; }
...
void dfs(int de,ll se,ll sv){
	if(de == k){
		if(se!=msk2(n) || sv!=msk2(pct(sv))){
			rexit();
		}
		return;
	}
	int cv = pct(msk[de])-pct(msk[de]&se),lv = pct(msk[de]&sv);
	FOR(i,lv,lv+cv+1){
		ll nxt = (sv&(msk2(n)-msk[de]))|submsk[de][i];
		dfs(de+1,se|msk[de],nxt);
	}
}
...
int main() {
	...
	F0R(i,k){
		...
		F0R(j,c){
			int cn;re(cn);cn-=1;
			submsk[i][j+1] = (submsk[i][j])|(1ll<<cn);
		}
		msk[i] = submsk[i][c];
	}
	dfs(0,0,0);
