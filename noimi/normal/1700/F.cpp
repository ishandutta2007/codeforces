...
int main() {
    ...
    rep(i, 2) rep(j, n) if(a[i][j] and b[i][j]) a[i][j] = b[i][j] = 0;

    queue<int> A[2], B[2];
    ll ans = 0;
    rep(j, n) {
        if(!empty(A[0]) or !empty(A[1])) swap(A[0], B[0]), swap(A[1], B[1]), swap(a, b);
        rep(i, 2) if(b[i][j]) B[i].emplace(j);
        int ax = a[0][j], ay = a[1][j];
        while((!empty(B[0]) or !empty(B[1])) and (ax or ay)) {
            if(ax) {
                if(!empty(B[0])) {
                    int t = B[0].front();
                    B[0].pop();
        ...
        if(ax) A[0].emplace(j);
        if(ay) A[1].emplace(j);
        dump(si(A[0]), si(A[1]), si(B[0]), si(B[1]));
    }
    rep(i, 2) if(!empty(A[i]) or !empty(B[i])) drop(-1);
    OUT(ans);
}