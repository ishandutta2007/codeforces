...
fn run() {
    ...
    for (i, a) in a.into_iter().enumerate() {
        seg.update_tmp(hld.sub_tree(i).0, a.abs());
    }
    seg.update_all();
    ...
    for (op, u, c) in ask {
        if op == 1 {
            seg.update(hld.sub_tree(u).0, c.abs());
        } else {
            let v = c as usize - 1;
            hld.path(u, v, &mut up, &mut down);
            let mut ans = 2 * up.iter().chain(&down).fold(0, |s, &(l, r)| s + seg.find(l, r));
            let x = hld.sub_tree(u).0;
            ans -= seg.find(x, x + 1);
