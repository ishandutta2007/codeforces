...
int main(){
    ...
    sort(ALL(a)); reverse(ALL(a)); sort(ALL(b));
    vector<vector<int>> c(n,vector<int>(6));
    vector<pair<int,int>> time;
    for (int i=0;i<n;++i){
        for (int j=0;j<6;++j){
            c[i][j]=b[i]-a[j];
            time.emplace_back(c[i][j],i);
        }
    }
    sort(ALL(time));
    SegmentTree<int> seg(n,[](int a,int b){return max(a,b);},0);
    for (int i=0;i<n;++i) seg.update(i,c[i][0]);
    vector<int> cur(n,0);
    int ans=2*INF;
    bool ok=true;
    for (int i=0;i<time.size();){
        int nxt=time[i].first;
        chmin(ans,seg.query(0,n)-nxt);
        while(time[i].first==nxt){
            int id=time[i++].second;
            if (cur[id]==5){ok=false; break;}
            seg.update(id,c[id][++cur[id]]);
        }
        if (!ok) break;
        // for (int j=0;j<n;++j) cerr << seg[j] << (j+1==n?'\n':' ');
    }
