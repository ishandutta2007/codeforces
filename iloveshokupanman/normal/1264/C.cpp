...
int main() {
    ...
    vmint pos(N + 1);
    rep(i, N)pos[i] = (mint)100 / (mint)p[i];

    vmint prd(N + 1, 1);
    for (ll i = N - 1; i >= 0; i--)prd[i] = prd[i + 1] * pos[i];

    vmint sum(N + 1, 0);
    for (ll i = N - 1; i >= 0; i--)sum[i] = sum[i + 1] + prd[i];

    auto cal = [&](ll l, ll r) {
        mint ret = sum[l] - sum[r];
        ret /= prd[r];
        return ret;
    };
