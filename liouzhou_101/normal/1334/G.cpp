...
void solve()
{
	...
	poly r(26);
	for (int i = 0; i < 26; ++i) r[i] = rd() % MOD;
	for (int i = 1; i <= m; ++i) tsum[i] = tsum[i - 1] + r[t[i - 1]] * r[t[i - 1]];
	mod_int ssum = 0;
	for (int i = 0; i < n; ++i) ssum += r[s[i]] * r[p[s[i]]];

	poly lhs(m), rhs(n);
	for (int i = 0; i < m; ++i) lhs[i] = r[t[i]];
	for (int i = 0; i < n; ++i) rhs[i] = r[s[n - i - 1]] + r[p[s[n - i - 1]]];
	auto res = NTT::mod_multiply(lhs, rhs);
	for (int i = 0; i <= m - n; ++i)
	{
		if (int(tsum[i + n] - tsum[i] + ssum - res[i + n - 1]) == 0)
			write('1');
		else
			write('0');
	}
