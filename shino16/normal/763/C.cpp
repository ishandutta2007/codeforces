...
int m, n, a[100001];
mint nm, inv_nm, inv_nm1;
mint sum, sq_sum;

bool has(int v) { return *lower_bound(a, a + n, v) == v; }
...
int main() {
  ...
  rep(i, n) scanf("%d", a + i);
  if (n == 1 || n == m || m == 2) return printf("%d 1\n", a[0]), 0;

  sort(a, a + n);

  nm = n;
  mint inv_nn1 = (nm * (nm - 1)).inv() * 2;

  rep(i, n) sum += a[i], sq_sum += mint(a[i]) * a[i];

  rep(i, n) {
    int x = a[i];
    mint d = (sum - x * nm) * inv_nn1;
    if (d == 0) continue;
    if (nm * x * x * 6 + d * (nm - 1) * nm * x * 6 +
            d * d * (nm - 1) * nm * (nm * 2 - 1) !=
        6 * sq_sum)
      continue;
    int dv = d.val();
