...
char arr[1050][1050];
int cnt[1050][1050],ans=0,dp[1050][1050];
bool vis[1050][1050],vis2[1050][1050];
...
void dfs2(int i,int j,int n,int m){
    if (i>=n || j>=m || i<0 || j<0) return;
    if (arr[i][j]=='*' || vis2[i][j]) return;
    vis2[i][j]=true;
    dp[i][j]=ans;
...
signed main(){
    ...
    For(i,n){
        For(j,m) {cin>>arr[i][j]; cnt[i][j]=0; vis[i][j]=false; vis2[i][j]=false;}
    }
    
    For(i,n){
        For(j,m){
            if (arr[i][j]=='*') continue;
            if (i!=0 && arr[i-1][j]=='*') cnt[i][j]++;
            if (j!=0 && arr[i][j-1]=='*') cnt[i][j]++;
            if (i!=n-1 && arr[i+1][j]=='*') cnt[i][j]++;
            if (j!=m-1 && arr[i][j+1]=='*') cnt[i][j]++;
        }
    }
    For(i,n){
        For(j,m){
