// Exact Change (https://codeforces.com/contest/1620/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        int ans = 2 * INF;
        rep(num1, 6) rep(num2, 6) {
            int need3 = 0;
            trav(ai, a) {
                int mn = 2 * INF;
                rep(use1, num1 + 1) rep(use2, num2 + 1) {
                    int rem = ai - use1 - 2 * use2;
                    if (rem % 3 == 0 && rem >= 0)
                        mn = min(mn, rem / 3);
                }
