...
int main()
{
	...
	sort(h,h+n-1);
	int cur=h[0],ma=h[0];
	for (int j=1;j<cur;++j) treb.insert(j);
	for (int i=1;i<n-1;++i)
	{
		if (h[i]>ma)
		{
			for (int j=ma+1;j<h[i];++j) treb.insert(j);
			v.push_back({cur,h[i]});
			cur=h[i];
			ma=h[i];
		}
		...
		{
			if (treb.empty()) no();
			v.push_back({cur,*(treb.begin())});
			cur=*(treb.begin());
			treb.erase(treb.begin());
		}
	}
	v.push_back({cur,n});
	cout<<"YES\n";
	for (auto a: v) cout<<a.first<<' '<<a.second<<en;
}