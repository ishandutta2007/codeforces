...
public class D {
	...
	void go()
	{
		...
		int[] vals = new int[0];
		int[] poss = new int[0];
		int[][] es = new int[n*19+1][];
		SegmentTreeRMQ st = new SegmentTreeRMQ(a);
		int q = 0;
		es[q++] = new int[]{0, n, 0, P};
		for(int i = 0;i < n;i++){
			int[] nvals = new int[vals.length+1];
			int[] nposs = new int[vals.length+1];
			int p = 0;
			for(int j = 0;j < vals.length;j++){
				int g = gcd(vals[j], a[i]);
				if(p > 0 && nvals[p-1] == g)continue;
				nvals[p] = g;
				nposs[p] = poss[j];
				p++;
			}
			...
			vals = Arrays.copyOf(nvals, p);
			poss = Arrays.copyOf(nposs, p);

			for(int j = 0;j < p;j++){
				if(vals[j] >= P)continue;
				if(poss[j] == i)continue;
				int last = st.lastle(j == p-1 ? i : poss[j+1], vals[j]);
				if(poss[j] <= last) {
					es[q++] = new int[]{poss[j], last + 1, i, vals[j]};
				}
			}
		}
		es = Arrays.copyOf(es, q);
		Arrays.sort(es, (x, y) -> x[3] - y[3]);
		...
		for(int[] e : es){
			int cur = e[0];
			while(true) {
				int alive = yet.next(cur);
//				tr(yet, cur, alive, e);
				if (alive == -1 || alive + 1 >= e[1]) {
					break;
				}
				if(!ds.union(alive, alive+1)){
					cost += e[3];
				}
				yet.unset(alive);
				cur = alive;
