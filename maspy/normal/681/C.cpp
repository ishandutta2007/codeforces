...
void solve() {
  ...
  vc<string> ANS;
  pqg<ll> que;
  auto add = [&](string S, ll x) -> void {
    if (S == "insert") que.push(x);
    if (S == "removeMin") que.pop();
    if (S != "removeMin") S += " " + to_string(x);
    ANS.eb(S);
  };
  ...
  FOR_(N) {
    ...
    if (S == "insert") {
      ...
      LL(x);
      while (len(que) && que.top() < x) { add("removeMin", 0); }
      if (len(que) == 0 || que.top() != x) { add("insert", x); }
      assert(que.top() == x);
      add(S, x);
    }
    elif (S == "removeMin") {
      if (len(que) == 0) add("insert", 0);
      add("removeMin", 0);
    }
  }
  print(len(ANS));
