...
int main(){
	...
	modular res = 0;
	vector dp1(n + 1, vector<modular>(m + 1));
	for(auto h = 1; h <= n; ++ h){
		modular pref1 = 0, pref2 = 0;
		for(auto w = 2; w <= m; ++ w){
			dp1[h][w] = (h == 1) + (w + 1) * (pref1 += dp1[h - 1][w]) - (pref2 += w * dp1[h - 1][w]);
			res += dp1[h][w] * (n - h + 1) * (m - w + 1);
		}
	}
	vector dp2(n + 1, vector<modular>(m + 1));
	for(auto h = 1; h <= n; ++ h){
		modular pref1 = 0, pref2 = 0;
		for(auto w = m; w >= 2; -- w){
			dp2[h][w] = (1 - w) * (pref1 += dp2[h - 1][w]) + (pref2 += dp2[h - 1][w] * w);
			pref1 += dp1[h - 1][w] * (m - w + 1);
			pref2 += dp1[h - 1][w] * w * (m - w + 1);
			res += dp2[h][w] * (n - h + 1);
		}
	}
	cout << res << "\n";
...
/*
dp1[h][w]
= 1 + \sum{x <= w} dp1[h - 1][x] * (w - x + 1)
= 1 + (w-1) * \sum{x <= w} dp1[h - 1][x] - \sum{x <= w} x * dp1[h - 1][x]

dp2[h][w]
= \sum{x > w} dp1[h - 1][x] * (x - w + 1) + \sum{x >= w} dp2[h - 1][x] * (x - w + 1)
= (1 - w) * (\sum{x > w} dp1[h - 1][x] + \sum{x >= w} dp2[h - 1][x]) + (\sum{x > w} dp1[h - 1][x] * x + \sum{x >= w} dp2[h - 1][x] * x)
*/
