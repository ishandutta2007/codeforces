...
void solve() {
  ...
  {
    A = {1};
    FOR_(n) A.eb(A.back() * 2);
    seq.eb(A);
  }
  FOR(k, n) {
    A = {1};
    FOR(i, n) A.eb(A.back() * (i == k ? 3 : 2));
    if (A.back() <= N) seq.eb(A);
  }
  mint ANS = 0;
  for (auto&& A: seq) {
    vi CNT(len(A));
    FOR(i, len(A)) CNT[i] = N / A[i];
    FOR(i, len(CNT) - 1) CNT[i] -= CNT[i + 1];
    mint x = 1;
    ll n = 0;
    for (auto&& c: CNT) {
      x *= C<mint>(n + c, c) - C<mint>(n + c - 1, c);
      n += c;
    }
    for (auto&& c: CNT) x *= fact<mint>(c);
    ANS += x;
  }
  print(ANS);
}
