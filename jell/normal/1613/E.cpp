#line 1 "codeforces-workspace\\contest\\1613\\e\\e.cpp"
...
#line 13 "codeforces-workspace\\contest\\1613\\e\\e.cpp"
...
void main() {
  ...
  input n, m;
  input<vector<string>> grid(n);

  queue<pair<u32, u32>> Q;

  auto neighbors = [&](u32 x, u32 y) {
    vector<pair<u32, u32>> ret;
    constexpr pair<u32, u32> d[]{{1, 0}, {0, 1}, {-1, 0}, {0, -1}};
    for (auto [a, b] : d) {
      a += x, b += y;
      if (a < n and b < m and grid[a][b] != '#') ret.emplace_back(a, b);
    }
    return ret;
  };

  vector free(n, vector<u32>(m));

  for (auto i : range(n)) {
    for (auto j : range(m)) {
      free[i][j] = neighbors(i, j).size();
      if (grid[i][j] == 'L') {
        Q.emplace(i, j);
        free[i][j] = 0;
      }
  ...
  while (!Q.empty()) {
    ...
    for (auto [a, b] : neighbors(x, y)) {
      if (grid[a][b] == 'L' or grid[a][b] == '+') continue;
      if (--free[a][b] < 2) {
        Q.emplace(a, b);
        grid[a][b] = '+';
      }
