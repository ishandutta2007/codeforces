...
public:
	void solve(istream& inp, ostream& outp) {
        ...
        vector<int> sign(n);
        for (int i = 2; i < n; i++) {
            out.printLine(1, 1, 2, i + 1);
            out.flush();
            area[i] = in.readLong();
            out.printLine(2, 1, 2, i + 1);
            out.flush();
            sign[i] = in.readInt();
        }
        vector<pair<ll, int> > positive;
        vector<pair<ll, int> > negative;
        for (int i = 2; i < n; i++) {
            if (sign[i] == -1) {
                positive.emplace_back(area[i], i + 1);
            } else {
                negative.emplace_back(area[i], i + 1);
            }
        }
        sort(all(positive));
        vi answer(1, 1);
        if (!positive.empty()) {
            ...
            for (int i = 0; i < positive.size() - 1; i++) {
                out.printLine(2, 1, positive.back().second, positive[i].second);
                out.flush();
                if (in.readInt() == -1) {
                    answer.push_back(positive[i].second);
                } else {
                    tail.push_back(positive[i].second);
                }
            }
            answer.push_back(positive.back().second);
            reverse(all(tail));
            addAll(answer, tail);
        }
        answer.push_back(2);
        ...
        if (!negative.empty()) {
            ...
            for (int i = 0; i < negative.size() - 1; i++) {
                out.printLine(2, 1, negative.back().second, negative[i].second);
                out.flush();
                if (in.readInt() == -1) {
                    answer.push_back(negative[i].second);
                } else {
                    tail.push_back(negative[i].second);
                }
            }
            answer.push_back(negative.back().second);
            reverse(all(tail));
            addAll(answer, tail);
        }
        out.print("0 ");
