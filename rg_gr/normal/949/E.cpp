...
#endif
#include <vector>
using cmll=std::vector<int>;
cmll v,ans;
void chk(cmll &t)
{
	if(ans.size()>t.size())ans=t;
}
void solve(cmll v,cmll& t,int dep=0)
{
	if(dep>20)return;
	std::sort(v.begin(),v.end());
	v.resize(std::unique(v.begin(),v.end())-v.begin());
	// printf("dep %d : ",dep);
	// for(auto p:v)odb(p);printf("| ");for(auto p:t)odb(p);puts(""); fflush(stdout);
	if(v.size()==0)return chk(t);
	if(v.size()==1&&v[0]==0)return chk(t),void();
	int c=0;
	for(int p:v)if(p&1)c++;
	if(c)
	{
		cmll A=v,B=v;
		for(auto &p:A)if(p&1)p--;
		for(auto &p:B)if(p&1)p++;
		for(auto &p:A)p/=2;
		for(auto &p:B)p/=2;
		t.push_back(1<<dep);
		solve(A,t,dep+1);
...
{
	initprog();
	int n=read();ans.resize(80);
	rg(n)v.push_back(read());gr
	cmll t;
	solve(v,t);
	odl(ans.size());
	for(auto p:ans)odb(p);puts("");
	return 0;
}