...
struct SparseTable {
	...
	SparseTable(vector<T>& v, function<T(T,T)> func) : f(func) { st[0] = v; build(); }
	SparseTable(int size, function<T(T,T)> func) : f(func) { st[0].resize(size); }
};

int main() {
	int n,m,q; R(n,m,q);
	vector<int> perm(n); R(perm);
	vector<int> pre(n + 1);

	for (int i = 0; i < n; i++)
		pre[perm[(i + 1) % n]] = perm[i];
	...
	const int DEP = 20;
	vector<vector<int>> jump(DEP, vector<int>(m + 1, 0));
	jump[0] = lp;
	for (int i = 1; i < DEP; i++)
		for (int j = 1; j <= m; j++)
	...
	for (int i = 1; i <= m; i++) {
		...
		for (int bit = DEP - 1, val = 1 << (DEP - 1); bit >= 0; bit--, val >>= 1) {
			if (rem >= val) {
				rem -= val;
				ans[i] = jump[bit][ans[i]];
			}
		}
	}

	SparseTable<int> st(ans, [&](int u, int v) {
		return max(u, v);
	});
