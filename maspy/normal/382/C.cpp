...
void solve() {
  ...
  VEC(ll, A, N);
  sort(all(A));
  if (len(A) == 1) return print(-1);

  auto OUT = [&](vi ANS) -> void {
    UNIQUE(ANS);
    print(len(ANS));
    print(ANS);
  };
  ...
  if (len(A) == 2) {
    ...
    ANS.eb(a + a - b);
    ANS.eb(b + b - a);
    if ((a + b) % 2 == 0) ANS.eb((a + b) / 2);
    return OUT(ANS);
  }

  bool ok = [&]() {
    FOR(i, N - 1) if (A[i + 1] - A[i] != A[1] - A[0]) return false;
    return true;
  }();
  if (ok) {
    vi ANS = {2 * A[0] - A[1], 2 * A[N - 1] - A[N - 2]};
    return OUT(ANS);
  }
  ...
  FOR(i, N - 1)
  if (A[i + 1] - A[i] != d && A[N - 1] - A[0] == N * d) ANS.eb(A[i] + d);
  if (len(ANS) != 1) ANS = {};
  OUT(ANS);
}
