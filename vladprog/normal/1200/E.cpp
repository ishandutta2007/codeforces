...
bool prime(int p)
{
    for(int i=2;i*i*i<=p;i++)
        if(p%i==0)
            return false;
    loop(20)
    {
        int a=abs(rand()*RAND_MAX+rand())%(p-2)+2;
        if(ppow(a,p-1,p)!=1)
            return false;
    }
    return true;
...
int get(int l,int r)
{
//    cout<<l<<" "<<r<<" -> "<<(h[r]-h[l-1]+mod)%mod<<" -> "<<(h[r]-h[l-1]+mod)*rev[l]%mod<<"\n";
    return (h[r]-h[l-1]+mod)*rev[l]%mod;
}
...
signed main()
{
    ...
    srand(time(0));
    mod=abs((rand()*RAND_MAX+rand())^((int)(new char)));
    while(!prime(mod))
        mod++;
    pw[0]=1;
    for1(i,N-1)
        pw[i]=pw[i-1]*BASE%mod,
        rev[i]=ppow(pw[i],mod-2,mod);
    ...
    loop(n)
    {
        ...
        cin>>t;
        int del=0,cur=0;
        for0(i,min(sz(t),sz(s)-1))
        {
            cur=(cur+t[i]*pw[i])%mod;
            if(cur==get(sz(s)-i-1,sz(s)-1))
                del=i+1;
        }
        t=t.substr(del);
        int prv=sz(s);
        ...
        int now=sz(s);
        fort0(i,prv,now)
//            cout<<i<<" -> "<<(h[i-1]+s[i]*pw[i])%mod<<"\n",
            h[i]=(h[i-1]+s[i]*pw[i])%mod;
    }
    cout<<s.substr(1);
