...
vi solve(int L, int R, int K) {
    if (R - L == K) {
        vi res;
        clear();
        FOR (loc, L, R) if (taste(loc)) res.pb(loc);
        return res;
    }
    assert(R - L > K);

    vi left  = solve(L, (L + R) / 2, K),
       right = solve((L + R) / 2, R, K);

    vi res = left;

    for (int i = 0; i < sz(right); i += K/2) {
        vb should(K / 2, 1);

        for (int j = 0; j < sz(left); j += K/2) {
            clear();
            FOR (k, j, min(sz(left), j + K/2)) {
                taste(left[k]);
            }
            FOR (k, i, min(sz(right), i + K/2)) {
                should[k - i] = should[k - i] & taste(right[k]);
            }
        }
