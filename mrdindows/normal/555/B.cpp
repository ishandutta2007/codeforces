...
template<typename T1,typename T2,typename T3>
bool operator<(const triple<T1,T2,T3> &t1,const triple<T1,T2,T3> &t2){if (t1.b!=t2.b) return t1.b<t2.b; else return t1.a<t2.a;}
template<typename T1, typename T2, typename T3>
...
int main()
{
	...
	for (int i = 0; i < br.size(); ++i) {
		...
		if (it == s.end() || it->first < l || it->first > r){
			cout << "No" << endl;
			return 0;
		}
		ans[id] = it->second;
