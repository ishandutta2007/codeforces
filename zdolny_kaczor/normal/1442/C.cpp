...
//#STAY AT HOME
#define time _time
const int nax = 2e5 + 44, kax = 22, mod = 119 << 23 | 1;
struct time {
	int exp, off;
	ll real_val() {
		return (1ll << exp) + off;
	...
	ll extract() {
		ll ans = 1;
		REP(_, exp) (ans *= 2) %= mod;
		return fix(ans + off, mod);
	}
};
ostream &operator<<(ostream &o, time t) {
	return o << "[2^" << t.exp << "+" << t.off << " = " << t.extract() << "]";
}
const time inf_time = {(int)1e6, 0}, start = {0, -1};
time dist[nax][kax];
time push(time t, int v) {
	if ((t.exp ^ v) & 1) t.exp++;
	t.off++;
	return t;
}
bool operator<(time a, time b) {
	if (a.exp > 50 || b.exp > 50) return make_pair(a.exp, a.off) < make_pair(b.exp, b.off);
	return make_pair(a.real_val(), a.exp) < make_pair(b.real_val(), b.exp);
}
bool operator>(time a, time b) {
	return b < a;
}
vpii graf[nax];
set <pair <time, int> > dij;
void touch(int x, time t) {
	int num = min(kax - 2, t.exp);
	if ((num ^ t.exp) & 1) num++;
	debug(imie(t), imie(num));
	if (t < dist[x][num]) {
		dist[x][num] = t;
		dij.insert(make_pair(dist[x][num], x));
	}
}
void solve() {
	int n, m;
	scanf("%d%d", &n, &m);
	REP(i, nax) REP(j, kax) dist[i][j] = inf_time;
	REP(_, m) {
		int a, b;
		scanf("%d%d", &a, &b);
		graf[a].emplace_back(b, 0);
	...
	while (!dij.empty()) {
		...
		debug(imie(t), imie(x));
		dij.erase(dij.begin());
		for (auto [v, d] : graf[x]) touch(v, push(t, d));
	}
	time best = inf_time;
	for (int i = 0; i < kax; ++i) {
		debug(arr(dist, n, i));
