#line 1 "codeforces-workspace\\contest\\1559\\e\\e.cpp"
...
#line 16 "codeforces-workspace\\contest\\1559\\e\\e.cpp"
//*/
...
#line 33 "codeforces-workspace\\contest\\1559\\e\\e.cpp"
...
void main() {
  ...
  input n, m;
  input<vector<pair<i32, i32>>> ranges(n);

  vector<mint> f(m + 1);

  for (auto i = 1; i <= m; i++) {
    vector<mint> dp(m / i + 1);
    dp[0] = 1;
    for (auto [l, r] : ranges) {
      l = ceil_div(l, i);
      r = floor_div(r, i) + 1;
      // + [l, r)
      for (auto j : rrange(dp.size())) {
        dp[j] = 0;
        if (j >= l) dp[j] = dp[j - l];
      }
      r -= l;
