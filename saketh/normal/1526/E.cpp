...
int main() {
    ...
    int N, K; re(N, K);
    vi suffix_array(N); re(suffix_array);

    vi rank_of_suffix(N + 1);
    F0R (i, N) {
        rank_of_suffix[suffix_array[i]] = i;
    }
    rank_of_suffix[N] = -1;
    ...
    F0R (i, N - 1) {
        ...
        int q = suffix_array[i + 1];

        if (rank_of_suffix[p + 1] < rank_of_suffix[q + 1]) {
            can_eq++;
        }
    }
