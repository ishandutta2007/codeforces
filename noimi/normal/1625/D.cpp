...
int main() {
    ...
    vi v;
    rep(i, n) {
        if(mp.count(a[i]) == 0) pool[c++] = a[i];
        mp[a[i]] = i + 1;
    }
    vi res;

    BinaryTrie<int, 30> trie;
    REC([&](auto &&f, int l, int r, int t) -> void {
        if(l == r) return;
        // dump(vi(pool + l, pool + r), t);
        // dump(res);
        if(r - l == 1) {
            res.eb(pool[l]);
            return;
        ...
        if(k >> t & 1) {
            ...
            if(trie.root->exist == false) {
                res.eb(pool[l]);
                return;
            }
            rep(i, l, r) {
                int &e = pool[i];
                auto [rr, node] = trie.max_element(e);
                // dump(vi(pool + l, pool + r), pool[i], rr);
                if(rr >= k) {
                    res.eb(e), res.eb(e ^ rr);
                    rep(i, l, r) {
                        if(pool[i] >> t & 1) trie.erase(pool[i]);
                    }
                    return;
                }
            }
    ...
    LOCAL {
        // dump(res);
        rep(i, si(res)) rep(j, i) { assert((a[res[i] - 1] ^ a[res[j] - 1]) >= k); }
        OUT(si(res));
    }
    OJ {
        OUT(si(res));
