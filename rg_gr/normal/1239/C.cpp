...
#endif
int p[1000005],a[1000005],r[1000005];
...
signed main()
{
	initprog();
	int n=read(),s=read();
	rg(n)a[i]=read();p[i]=i;gr
	std::sort(p+1,p+1+n,[&](int x,int y){return a[x]==a[y]?x<y:a[x]<a[y];});
	int ct=0;
	...
	for(int i=1;i<=n;i++)
	{
		...
		for(;cur<=n;)
		{
			if(a[p[cur]]<=ct+s)
			{
				if(q2.empty()||p[cur]<q2.back())q2.push(p[cur++]);
				else q.push(p[cur++]);
			}
			else break;
		}
		...
		if(q2.empty()&&q.size())q2.push(q.top()),q.pop();
		// printf("q : ");print(q);
		// printf("q2 : ");print(q2);
