...
int solve() {
    ...
    vector<tri> ans;
    while (pos < n || neg < n) {
        while (pos < n && s[pos].first <= 0) ++pos;
        while (neg < n && s[neg].first >= 0) ++neg;
        if (pos == n && neg == n) break;
        if (pos > neg) {
            cout << "NO" << endl;
            return 0;
        }
        int t = min(s[pos].first, -s[neg].first);
        s[pos].first -= t;
        s[neg].first += t;
        ans.push_back({s[pos].second + 1, s[neg].second + 1, t});
    }
    writeStr("YES\n");
    writeInt(ans.size(), '\n');
    for (auto& t : ans) {
