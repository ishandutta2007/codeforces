...
signed main(){
    ...
    while (_--){
        cin>>n;
        int ans=0;
        rep1(i,n) cin>>a[i], b[i]=0, vis[i]=0,p[i]=i, id[i]=0;
        rep1(i,n) id[a[i]]++;
        sort(p+1,p+n+1,[&](int u,int v){return id[u]<id[v];});
        rep1(i,n){
            int idx=p[i];
            if (b[a[idx]]) continue;
            vis[idx]=1; b[a[idx]]=i; c[idx]=a[idx]; ans++;
        }
        rep1(i,n){
            if (!b[i]&&vis[i]){
                int idx=i;
        ...
        if (sz(res)){
            rep(i,sz(res)) c[res[i]]=res[(i+1)%(sz(res))];
        }
        cout<<ans<<"\n";
        rep1(i,n) cout<<c[i]<<" ";
        cout<<"\n";
