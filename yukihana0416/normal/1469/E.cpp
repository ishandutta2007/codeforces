...
void work()
{
    ...
    int mask = (1 << l) - 1;
    vector<int> sta(1 << l, 0);

    int cnt = 0;
    for(int i = 0; i < k - l; i++) cnt += s[i] == '0';
    int x = 0;
    for(int i = k - l; i < k; i++) x = (x << 1) | (s[i] == '1');
    if(cnt == 0) sta[x] = 1;
//    cout <<x <<endl;
    for(int i = k; i < n; i++)
    {
...
//    for(int i = 0; i < (1 << l); i++) cout <<sta[i]; cout <<endl;
    ...
    cout <<"YES" <<endl;
    for(int i = 0; i < k - l; i++) putchar('0');
    for(int i = 0; i < l; i++) putchar('0' + (t >> (l - i - 1) & 1));
    cout <<endl;
