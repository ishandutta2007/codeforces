...
inline void dfsUp(int v, int p = -1) {
	...
	for (int i = 0; i < sz(g[v]); i++) {
		...
		if (to != p) {
			int nxt = curCost[to];
			for (int j = 0; j < 2; j++) {
				dUp[j][to] = dUp[nxt ^ j][v] + dDown[nxt ^ j][v] - dDown[j ^ nxt ^ cur][to];
			} 
			dfsUp(to, v);
		}
	}
...
inline void calcDp() {
	dUp[0] = dUp[1] = dDown[0] = dDown[1] = vi(n, 0);
	dfsDown(0); dfsUp(0);
}

signed main() {
	for (int i = 1; i < n; i++) {
		int a = get<int>()-1, b = get<int>()-1;
		g[a].pb(b);
		g[b].pb(a);
	}
	ll ans = 0;
	for (int i = 0; i < 31; i++) {
		for (int j = 0; j < n; j++) {
			curCost[j] = (cost[j] >> i) & 1;
		}
		calcDp();
		for (int j = 0; j < n; j++) {
			ans += (ll)(curCost[j] + dDown[1][j] + dUp[1][j]) << i;
		}
	}
	ans /= 2;
	putLn(ans);
