...
int main () {
	...
	int ats = 0;
	priority_queue<pair<pii, pii>> eil;
	eil.push(makep(makep(x, y), makep(r, c)));
	...
	while (!eil.empty()) {
		...
		if (d.x > 0 and !apl[d.x - 1][d.y] and !lab[d.x - 1][d.y]) {
			eil.push(makep(g, makep(d.x - 1, d.y)));
		}

		if (d.x < n - 1 and !apl[d.x + 1][d.y] and !lab[d.x + 1][d.y]) {
			eil.push(makep(g, makep(d.x + 1, d.y)));
		}

		if (d.y > 0 and g.x > 0 and !apl[d.x][d.y - 1] and !lab[d.x][d.y - 1]) {
			eil.push(makep(makep(g.x - 1, g.y), makep(d.x, d.y - 1)));
		}

		if (d.y < m - 1 and g.y > 0 and !apl[d.x][d.y + 1] and !lab[d.x][d.y + 1]) {
			eil.push(makep(makep(g.x, g.y - 1), makep(d.x, d.y + 1)));
		}
	}
