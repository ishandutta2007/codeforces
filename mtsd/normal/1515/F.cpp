...
public:
    UnionFind(){}
    UnionFind(int node_size) : sz(node_size), par(sz), size_(sz, 1),g(sz){
...
int main(){
    ...
    rep(i,m){
        INT(p,q);
        p--;q--;
        g[p].push_back(MP(q,i+1));
        g[q].push_back(MP(p,i+1));
    }
    if(SUM(a) < x*(n-1)){
        NO();
        return 0;
    ...
    dfs(dfs,0);
    vector<int> res;
    auto dfs2 = [&](auto && f,int id,ll &val)->void{
        vector<pair<ll,pair<int,int> > > pp;
        for(auto [nxt,eid]:gg[id]){
            pp.push_back(MP(sub[nxt]*x-p[nxt],MP(nxt,eid)));
        }
        sort(all(pp));
        val += a[id];
        for(auto xx:pp){
            int nxt = xx.second.first;
            int eid = xx.second.second;
            bool fff = 0;
            ll pppp = 0;
            if(val + a[nxt]>=x){
                val -= x;
