...
void solve(){
    ...
    VEC(ll,a,n);
    Sort(a);
    a.insert(a.begin(),-MINF);
    a.push_back(MINF);
    n+=2;
    vec(vector<pll>,seg,n-1);
    rep(m){
        LL(l,r);
        auto p=lower_bound(all(a),l),q=upper_bound(all(a),r);
        if(p!=q)continue;
        seg[p-a.begin()-1].emplace_back(l,r);
    }
    vec(tuplis,b,n-1);
    rep(n-1){
        const ll x=a[i],y=a[i+1];
        auto&s=seg[i];
        ll&cost11=b[i][0],&cost12=b[i][1],&cost21=b[i][2];
        if(s.empty()){
            cost11=cost12=cost21=0;
            continue;
        }
        Sort(s);
        Rev(s);
        ll R=y;
        cost11=cost12=cost21=LINF;
        each(l,r,s){
            chmin(cost11,(l-x)+(y-R));
            chmin(cost12,(l-x)+(y-R)*2);
            chmin(cost21,(l-x)*2+(y-R));
            chmin(R,r);
        }
        chmin(cost11,(y-R));
        chmin(cost12,(y-R)*2);
    ...
    ll cost1=0,cost2=0;
    each(cost11,cost12,cost21,b)tie(cost1,cost2)=pair{min(cost2+cost11,cost1+cost21),min(cost1+cost11*2,cost2+cost12)};
    out(min(cost1,cost2));
}
int main(){
    LL(t);
