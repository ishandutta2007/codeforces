...
int main() {
  ...
  g[0]=g[1]=1;
  for (int i=2;i<=n;i++)
    g[i]=(g[i]+2*g[i-1])*simp.ginv(i)%mod;
  Poly ff=Poly(vi(g,g+n+1)).integ().exp();
  auto calc=[&](int l,int r) {
    if (l<=m) return dp[l][r];
    else if (r==n+1) return (ll)ff[r-l-1];
    else return g[r-l-1];
  };
  auto check=[&](int l,int r,int mid) {
    if (mid>=2&&mid<=m+1&&((s[mid-1]=='>')==(mid==l+1))) return false;
    if (mid<=m&&((s[mid]=='<')==(mid==r-1))) return false;
    return true;
  };
  for (int l=m;l>=1;l--) {
    dp[l][l+1]=1;
    for (int r=l+2;r<=n+1;r++) {
      int mid=l+1;
      if (check(l,r,mid)) dp[l][r]=(dp[l][r]+dp[l][mid]*calc(mid,r))%mod;
      if (l+1!=r-1) {
        mid=r-1;
        if (check(l,r,mid)) dp[l][r]=(dp[l][r]+dp[l][mid]*calc(mid,r))%mod;
      }
      ...
      dp[l][r]=dp[l][r]*simp.ginv(r-l-1)%mod;
    }
  }
  vi a(g,g+n+1); a.insert(a.begin(),0);
  a.resize(n);
  vi b(n);
  src.run(a,b,[&](int r) {
    if (r==0) { b[0]=1; return; }
    for (int mid=0;mid<r&&mid<=m;mid++) {
      b[r]=(b[r]-(ll)b[mid]*a[r-mid])%mod;
      if (check(0,r+1,mid+1)) b[r]=(b[r]+b[mid]*calc(mid+1,r+1))%mod;
    }
    if (b[r]<0) b[r]+=mod;
    b[r]=(ll)b[r]*simp.ginv(r)%mod;
  });
  ll ans=0;
  for (int mid=0;mid<n;mid++) {
    if (check(0,n+1,mid+1)) ans=(ans+b[mid]*calc(mid+1,n+1))%mod;
  }
  printf("%lld\n",ans*simp.gfac(n-1)%mod);
}