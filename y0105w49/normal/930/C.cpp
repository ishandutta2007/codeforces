...
int32_t main() {
	...
	for(int i=1;i<=n;i++) {
		auto it=rec.upper_bound(a[i]);
		if(it==rec.begin()) zl[i]=1;
		else zl[i]=zl[(--it)->se]+1;
		rec[a[i]]=i;
		for(;;) {
			auto it=rec.upper_bound(a[i]);
			if(it==rec.end()) break;
			if(zl[it->se]>zl[i]) break;
			rec.erase(it);
		}
	}
	rec.clear();
	for(int i=n;i>=1;i--) {
		auto it=rec.upper_bound(a[i]);
		if(it==rec.begin()) zr[i]=1;
		else zr[i]=zr[(--it)->se]+1;
		rec[a[i]]=i;
		for(;;) {
			auto it=rec.upper_bound(a[i]);
			if(it==rec.end()) break;
			if(zr[it->se]>zr[i]) break;
			rec.erase(it);
		}
	}
	// dbg(vector<int>(a,a+n+2));
	// dbg(vector<int>(zl,zl+n+2));
	// dbg(vector<int>(zr,zr+n+2));
	int zz=0;
	for(int i=1;i<=n;i++) zz=max(zz,zl[i]+zr[i]-1);
	printf("%d\n",zz);
}