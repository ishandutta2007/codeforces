...
fun main(){
    solve.cases{
        ...
        val a = getstr
        val b = getstr
        val X = BooleanArray(n){a[it] != b[it]}


        val pos = (0 until n).filter { X[it] }
        ...
        if(have and 1 == 1){
            ...
            if(y <= x){
                ...
                if(have == 2){
                    val a = X.indexOfFirst { it }
                    val b = X.indexOfLast { it }
                    if(a +1 == b){
                        put(minOf(x , 2 * y))
                    }else{
                        put(y)
...
//                var ret = phuge
                ...
                //used y, active
                var DP = Array(have+1){LongArray(2){phuge}  }
                DP[0][0] = 0
                for(v in pos){
                    val new = Array(have+1){LongArray(2){phuge} }

                    for(now in 0 until 2){
                        for(starty in 0..have){
                            val here = DP[starty][now]
                            if(starty != have)
                            new[starty+1][now] = minOf(new[starty+1][now], here)
                            ...
                            val ns = 1 - now
                            new[starty][ns] = minOf(new[starty][ns], newcost)
                        }
                    }
                    DP = new
                }
                var ret = phuge
                for(yh in 0..have){
                    ret = minOf(ret, (yh/2) * y + DP[yh][0])
                }
                put(ret)

//                val stuff = pos.toMutableList()
//                var cost = (have/2) * y
//
//                ret = cost
//                repeat(have/2){
//                    val cheapest = (0 until stuff.lastIndex).minByOrNull { stuff[it+1] - stuff[it] }!!
//                    cost -= y
//                    cost += x * (stuff[cheapest+1] - stuff[cheapest])
//                    stuff.remove(cheapest+1)
