...
inline void AC() {
    ...
    for(_ i = 1; i <= m; ++i) {
        if(a[b[i].second].first+a[b[i].second].second <= 200) {
            _ md = a[b[i].second].first+a[b[i].second].second;
            if(b[i].first == 1) {
                for(_ j = i+a[b[i].second].first; j <= i+md-1; ++j) {
                    g[b[i].second].push_back(j%md);
                    ++dp[md][j%md];
                }
            } else {
                for(_ j : g[b[i].second]) --dp[md][j];
                g[b[i].second].clear();
            }
        }
        for(_ j = 2; j <= 200; ++j) o[i] += dp[j][i%j];
    }
    
    for(_ i = 1; i <= m; ++i)
        if(a[b[i].second].first+a[b[i].second].second > 200)
            t[b[i].second].push_back(i);
        
    for(_ i = 1; i <= n; ++i) if(!t[i].empty()) {
        if(t[i].size() & 1) t[i].push_back(m+1);
        for(_ j = 0; j < t[i].size(); j += 2) {
            _ ps = t[i][j];
            for(;;) {
                ps += a[i].first;
                if(ps >= t[i][j+1]) break;
    ...
    for(_ i = 1; i <= m; ++i) p[i] += p[i-1];
    for(_ i = 1; i <= m; ++i) writeln(o[i]+p[i]);
