...
int main(void)
{
	...
	while(T--){
		...
		ll b = digitnum(n+5, 2);
		SparseTable m(b), M(b);
		rep(i, 1, n) m.set(i, P(a[i], i)), M.set(i, P(-a[i], i));
		m.set(0, P(inf, inf)), m.set(n+1, P(inf, inf));
		M.set(0, P(inf, inf)), M.set(n+1, P(inf, inf));
		m.calc(), M.calc();
		...
		rep(i, 1, n) G[i].clear();
		rep(i, 1, n){
			ll p = M.query(lvec[i-1]+1, i).se;
			G[i].pb(p), G[p].pb(i);
			
			p = M.query(i, rvec[i-1]+1).se;
			G[i].pb(p), G[p].pb(i);
			
			p = m.query(Lvec[i-1]+1, i).se;
			G[i].pb(p), G[p].pb(i);
