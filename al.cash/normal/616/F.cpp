...
public:
  ...
  template <class U = typename decay<T>::type>
  typename enable_if<!is_same<U, bool>::value, pair<const U&, int>>::type
  operator * () const { return {data[i], i}; }
...
public:
  ...
  Array() { clear(); }
  void clear() { this->fill(T()); }

  MapIterator<T> begin() const { return MapIterator<T>(this->data(), 0, N); }
  MapIterator<T> end() const { return MapIterator<T>(this->data(), N, N); }
};

template <class C, class Transition>
class SuffixAutomaton {
  struct State {
    State(int length = 0, int link = -1) : length(length), link(link) {}
    State(int length, int link, const Transition& edges)
    : length(length), link(link), edges(edges) {}

    int length, link;
    Transition edges;
  };

  int computeLink(int p, const C& c) {
    int q = states[p].edges[c];
    if (states[p].length + 1 == states[q].length) return q;
    int clone = size();
    states.emplace_back(states[p].length + 1, states[q].link, states[q].edges);
    for (; p >= 0 && states[p].edges[c] == q; p = states[p].link)
      states[p].edges[c] = clone;
    return states[q].link = clone;
  }
...
public:
  SuffixAutomaton(int size = 1) { init(size); }

  void init(int size) {
    ASSERT(size > 0);
    states.resize(1);
    states.reserve(2 * size - 1);
    current = 0;
  }
  ...
  int addLetter(const C& c) {
    int p = current;
    if (states[p].edges[c] != 0) return current = computeLink(p, c);
    current = size();
    states.emplace_back(states[p].length + 1, 0);
    for (; p >= 0 && states[p].edges[c] == 0; p = states[p].link)
      states[p].edges[c] = current;
    if (p >= 0) states[current].link = computeLink(p, c);
    return current;
  }
  void endString() { current = 0; }

  void addString(const C* s, int size) {
    for (; size --> 0; addLetter(*s), ++s);
    endString();
  }

  void build(const C* s, int size) {
    for (init(size); size --> 0; addLetter(*s), ++s);
  }
};

template <class SuffixAutomaton>
vector<int> topoSort(const SuffixAutomaton& automaton) {
  vector<int> lengthCount;
  // The largest length is in one of two last states.
  for (int v = automaton.size() - 1; v >= 0; --v) {
    int length = automaton[v].length;
    if (length + 2 > lengthCount.size()) lengthCount.resize(length + 2);
    ++lengthCount[length + 1];
  }
  for (size_t i = 0; i + 1 < lengthCount.size(); ++i)
    lengthCount[i + 1] += lengthCount[i];
  vector<int> order(automaton.size());
  for (int v = 0; v < automaton.size(); ++v)
    order[lengthCount[automaton[v].length]++] = v;
  return order;
}

const int N = 1e5, L = 5e5 + 1;
int n, cost[N];
char s[L];
SuffixAutomaton<char, Array<int, 26>> sa;
bool split[L+1];
ll sum[2*L];
...
int main() {
  ...
  char* t = s;
  REP(i, n) {
    for (read(t, L); *t; *t -= 'a', ++t);
    split[t - s] = true;
  }
  int length = static_cast<int>(t - s);
  read(cost, n);

  sa.init(length);
  for (int i = 0, u = 0; i < length; ++i) {
    if (split[i]) ++u, sa.endString();
    sum[sa.addLetter(s[i])] += cost[u];
  }
