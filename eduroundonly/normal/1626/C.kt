...
fun main(){
    solve.cases{
        ...
        for(i in n-1 downTo 0){
            need[i] = maxOf(need[i], H[i])
            if(i != n-1){
                need[i] = maxOf(need[i], need[i+1] - (K[i+1] - K[i]))
            }
        }

        val total = mutableListOf<Int>(0)
        for(i in 0 until n){
            val lastpos = if(i == 0) 0 else K[i-1]
            val dif = K[i] - lastpos
            if(need[i] > dif){
                total[total.lastIndex] += dif
            }else{
                total.add(need[i])
            }
        }
        put(total.sumOf { v -> 1L * (v) * (v+1) /2  })
    }
    done()
}
