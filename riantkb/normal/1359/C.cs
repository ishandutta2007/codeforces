...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            double min = Math.Abs((h + c) / 2.0 - t);
            long ans = 2;
            for (long i = Math.Max(1, u - 5); i <= u + 5; i++)
            {
                double tt = (i * h + (i - 1) * c) / (double)(i * 2 - 1);
                if (min > Math.Abs(tt - t)) {
                    min = Math.Abs(tt - t);
                    ans = i * 2 - 1;
                }
            }
