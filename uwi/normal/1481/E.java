...
public class E {
	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			if(fs[i] > 0){
				qs[p++] = new int[]{ls[i], rs[i], rs[i]-ls[i]+1-fs[i]};
			}
		}
		...
		for(int x = 1;x <= n;x++){
			dp[x] = dp[x-1] + 1;
			while(p < qs.length && qs[p][1]+1 <= x){
				dp[x] = Math.min(dp[x], dp[qs[p][0]] + qs[p][2]);
				p++;
			}
		}
		for(int x = 0;x <= n;x++){
		...
		StarrySkyTreeL sst = new StarrySkyTreeL(dp);
		long ans = n;
		for(int i = 0;i < n;i++){
			for(int pos : b[i]){
				ans = Math.min(ans, sst.min(0, pos+1) + n-1);
				sst.add(0, pos+1, -1);
			}
			for(int pos : b[i]){
				sst.add(0, pos+1, 1);
