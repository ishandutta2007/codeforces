...
template<class T1, class T2, class T3>
bool operator<(const triple<T1,T2,T3>&t1,const triple<T1,T2,T3>&t2){ return tie(t1.a, t1.b, t1.c) < tie(t2.a, t2.b, t2.c);}
template<class T1, class T2, class T3>
...
auto __c = []() { if (__ir == __er) {std::fill(__bur, __bur + __BS, 0); cin.read(__bur, __BS); __ir = __bur; };};
template<class T = int> T readInt() {
    __c(); while (*__ir && (*__ir < '0' || *__ir > '9') && *__ir != '-') ++__ir; __c();
    bool m = false; if (*__ir == '-') {++__ir; __c(); m = true;}
    T r = 0; while (*__ir >= '0' && *__ir <= '9') {r = r * 10 + *__ir - '0', ++__ir; __c();}
    ++__ir; return m ? -r : r;
}
string readString() {
    string r; __c(); while (*__ir && isspace(*__ir)) {++__ir; __c();}
    while (!isspace(*__ir)) {r.push_back(*__ir); ++__ir; __c();}
    ++__ir; return r;
}
char readChar() {
    __c(); while (*__ir && isspace(*__ir)) {++__ir; __c();} return *__ir++;
}
...
bool isPrime(long long n) {
    ...
    for (int a : {2, 325, 9375, 28178, 450775, 9780504, 1795265022}) { if (!(a %= n)) return true;
        ...
        for (int i = 1; i < t; ++i) if ((f = mulmod(f, f, n)) == n - 1) goto l_nextp;
        return false; l_nextp:;
    } return true;
}
...
bool bad = false;
vector<int> sz[2];
void go(int cur, int t = 1) {
    used[cur] = t;
    sz[t - 1].push_back(cur);;
    for (int to : v[cur]) {
        if (!used[to]) {
            go(to, 3 - t);
        } else if (used[to] != 3 - t) {
...
int solve() {
    ...
    FI(n) {
        if (!used[i]) {
            ...
            sz[1].clear();
            go(i);
            if (sz[0].size() > sz[1].size()) {
                swap(sz[0], sz[1]);
            }
            msum += sz[0].size();
            a.push_back(int(sz[1].size() - sz[0].size()));
            g1.push_back(sz[0]);
            g2.push_back(sz[1]);
        }
    }
    ...
    if (bad || c[1] < msum || !bs[a.size()][c[1] - msum]) {
        cout << "NO" << endl;
        return 0;
    }
    cout << "YES" << endl;
