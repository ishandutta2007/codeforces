...
pii sub(const pii& x, const pii& y) {
	return { y.first - x.first, y.second - x.second };
}

ll cross(const pii& u, const pii& v) {
	return 1LL * u.first * v.second - 1LL * u.second * v.first;
}

bool isLTurn(const pii& x, const pii& y, const pii& z) {
	return cross(sub(x, y), sub(y, z)) >= 0;
}

int main() {
	int n,m; R(n,m);
	int x,y; R(x,y);
	vector<pii> a(n), b(m); R(a,b);
	
	{
		...
		const int MINV = -1000000007;
		b.push_back({ maxX, MINV });
		b.push_back({ MINV, maxY });
	}
	...
	vector<pii> hull;
	int fix = 0;
	auto push = [&](const pii& pt) {
		int sz = (int)hull.size();
		while (sz - fix >= 2 && !isLTurn(hull[sz - 2], hull[sz - 1], pt)) {
			hull.pop_back();
			sz--;
		}
	...
	bool isLose = 1;
	unordered_set<pii> s(a.begin(), a.end());
	for (auto& p : hull)
		if (s.count(p))
			isLose = 0;
