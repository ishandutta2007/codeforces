...
int main () {
	...
	deque<char> a(aa.begin(), aa.end());
	deque<char> b(bb.begin(), bb.end());
	...
	while (a.size() and b.size() and a.front() != '*') {
		if (a.front() != b.front()) {
			cout << "NO\n";
			exit(0);
		}
	...
	while (a.size() and b.size() and a.back() != '*') {
		if (a.back() != b.back()) {
			cout << "NO\n";
			exit(0);
		}
