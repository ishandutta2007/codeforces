...
fun main(){
    solve.cases{
        ...
        G.leafFirst {
            ...
            nt.sortDescending()
            tt.sortDescending()
            val nottakesum = nt.take(2).sum()
            val takesum = tt.take(2).sum()
            val besttake = nottakesum + 1
            val bestnottake = takesum + (degree[v] - minOf(tt.size, 2))
            ret = maxOf(ret, besttake)
            ret = maxOf(ret, bestnottake)

            DPtake[v] = (nt.getOrNull(0) ?: 0 ) + 1
            nottake[v] = (tt.getOrNull(0) ?: 0 ) + (degree[v] - 1 - if(v == 0) 0 else 1 )
            DPtake[v] = maxOf(DPtake[v], nottake[v])
        }
        put(ret)
