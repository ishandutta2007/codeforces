...
void solve(int tc_num = -1) {
	...
	FOR (i, 0, n) {
		...
		if (cnt == x) {
			...
			ll lebih = x - (cnt - days[r - 1]);
			ans = max(ans, tmp_ans - f(days[r - 1]) + f(lebih));
			LOG("naik", i, tmp_ans - f(days[r - 1]) + f(lebih));
		}
		cnt -= days[i];
		tmp_ans -= f(days[i]);
	}
	...
	FORD (i, 2 * n - 1, n) {
		...
		if (cnt == x) {
			...
			ll lebih = cnt - x;
			ans = max(ans, tmp_ans - f(lebih));
			LOG("turun", i, l, cnt, lebih, tmp_ans - f(lebih));
		}
		cnt -= days[i];
		tmp_ans -= f(days[i]);
	}
