...
void solve() {
    ...
    int low_count = 1, high_count = 1;
    a.pop_front();
    a.pop_back();
    while ((k > 0) && (!a.empty())) {
        if (low_count < high_count) {
            int64_t dist = a.front() - low;
            if (dist * low_count <= k) {
                k -= dist * low_count;
                low_count++;
                low = a.front();
            ...
            int64_t dist = high - a.back();
            if (dist * high_count <= k) {
                k -= dist * high_count;
                high_count++;
                high = a.back();
                a.pop_back();
    ...
    int64_t dist = high - low;
    int count = min(low_count, high_count);
    dist -= k / count;
    cout << max(dist, 0LL) << '\n';
}
