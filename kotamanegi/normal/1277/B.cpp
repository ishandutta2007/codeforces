...
typedef struct BOMData {
    int GeneratorTime;
    int GeneratorCost;
} BOMData;

typedef struct COMBIData {
    int ChangeTime;
    int ChangeCost;
} COMBIData;
...
vector<MachineData> Machine;
BOMData BOM[101][30];
COMBIData Combi[30][101][101];
vector<OrderData> Order;
vector<int> BOMHints[101];
vector<int> MachineToPan[31];
void read_input() {
    {
        string a;
        cin >> a;
    ...
    REP(i, M) {
        ...
        sort(ALL(d));
        Machine[a].PatchSize = b;
        REP(q, 10002) {
            Machine[a].Holiday[q] = 0;
        }
        REP(q, d.size()) {
            Machine[a].Holiday[d[q] + 1]++;
        }
        REP(q, 10001) {
            Machine[a].Holiday[q + 1] += Machine[a].Holiday[q];
        }
    }
    REP(i, 100) {
        REP(q, 30) {
            BOM[i][q].GeneratorCost = -1;
            BOM[i][q].GeneratorTime = -1;
        }
    }
    REP(i, BL) {
        string a;
        ...
        cin >> b >> c >> d >> e;
        BOM[b][c] = BOMData{ d,e };
        BOMHints[b].push_back(c);
        MachineToPan[c].push_back(b);
    }
    REP(i, CL) {
        string a;
        cin >> a;
        ...
        int b, c, d, e, f;
        cin >> b >> c >> d >> e >> f;
        Combi[b][c][d] = COMBIData{ e,f };
        Combi[b][I][c] = COMBIData{ 0,0 };
        Combi[b][I][d] = COMBIData{ 0,0 };
    }
    REP(i, R) {
        string a;
        cin >> a;
        assert(a == "ORDER");
        int b, c, d, e, f, g, h, ii;
        cin >> b >> c >> d >> e >> f >> g >> h >> ii;
        Order.push_back(OrderData{ b,c,d,e,f,g,h,ii });
    }
    return;
}
void output_answer(vector<AnswerData>& now) {
    sort(ALL(now), [](auto& l, auto& r) {
        return l.StartTime < r.StartTime;
        });
    REP(i, now.size()) {
        if (now[i].Type == 0) {
            cout << "ORDER " << now[i].OrderNumber << " " << now[i].MachineItr << " " << now[i].NumberOfPun
                << " " << now[i].StartTime << " " << now[i].EndTime << endl;
        }
        else {
            if (now[i].StartTime == now[i].EndTime) continue;
            cout << "COMBI " << now[i].MachineItr << " " << now[i].StartTime << " " << now[i].EndTime << endl;
        }
    }
}

int CanFillOrder(int remaining, OrderData Info) {
    int split_min = Info.DeComposition;
    if (split_min * 2 > Info.NumberOfPan) {
        split_min = Info.NumberOfPan;
    }
    if (split_min > remaining) return 0;
    int kazu = min(remaining, Info.NumberOfPan);
    if (kazu < Info.NumberOfPan and kazu >= Info.NumberOfPan - Info.DeComposition) {
        kazu = Info.NumberOfPan - Info.DeComposition;
    }
    if (kazu < Info.NumberOfPan - Info.DeComposition) {
        int capacity = 0;
        REP(i, BOMHints[Info.PanItr].size()) {
            capacity = max(capacity, Machine[BOMHints[Info.PanItr][i]].PatchSize);
        }
        for (int target = kazu; target >= split_min; --target) {
            int baseline = (Info.NumberOfPan) - target;
            int cnt = baseline / Info.DeComposition;
            baseline /= cnt;
            if (((Info.NumberOfPan) - target) % cnt) baseline++;
            if (capacity >= baseline) return target;
        }
        return 0;
    }
    else {
...
bool CheckDate(int left, int right, MachineData& timing) {
    right--;
    left /= 86400;
    right /= 86400;
    right++;
    if (timing.Holiday[right] != timing.Holiday[left]) return false;
    return true;
}

AnswerData AddOrder(int StartTime, int EndTime, int MachineItr, int OrderNumber, int NumberOfPun) {
    AnswerData NowInserting = {};
    NowInserting.Type = 0;
    NowInserting.StartTime = StartTime;
    NowInserting.EndTime = EndTime;
    NowInserting.MachineItr = MachineItr;
    NowInserting.OrderNumber = OrderNumber;
    NowInserting.NumberOfPun = NumberOfPun;
    return NowInserting;
}

AnswerData AddCombi(int StartTime, int EndTime, int MachineItr) {
    AnswerData NowInserting = {};
    NowInserting.Type = 1;
    NowInserting.StartTime = StartTime;
    NowInserting.EndTime = EndTime;
...
ll CalculateScore(vector<AnswerData> ans, ld cnt) {
    cnt += 10000000000L;
    REP(i, ans.size()) {
        if (ans[i].Type == 0) {
            int NowOrder = ans[i].OrderNumber;
            ld delay = max(0LL, ans[i].EndTime - Order[NowOrder].LimitTime);
            cnt -= (delay * (ld)Order[NowOrder].Profit * (ld)ans[i].NumberOfPun) / (ld)((ld)Order[NowOrder].LateAcceptance * (ld)Order[NowOrder].NumberOfPan);
        }
    }
    REP(i, Order.size()) {
        cnt += Order[i].Profit;
    }
    return round(1e10L * log10(max(cnt, 1.0L)));
}
