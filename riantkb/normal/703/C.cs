...
class Program
{
    ...
    static void Main()
    {
        ...
        var sc = new Scan();
        var a = sc.IntArr;
        int n = a[0], w = a[1], v = a[2], u = a[3];
        double min = 1e30, max = -1e30;
        for (int i = 0; i < n; i++)
        {
            var b = sc.IntArr;
            double p = b[0] - v / (double)u * b[1];
            min = Math.Min(min, p);
            max = Math.Max(max, p);
        }
        if (min > -eps || max < eps)
        {
            sw.WriteLine((w / (double)u).ToString().Replace(',', '.'));
            sw.Flush();
            return;
        }
        sw.WriteLine((w / (double)u + max / (double)v).ToString().Replace(',', '.'));
        sw.Flush();
    }
