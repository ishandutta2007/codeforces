...
inline lol turemin(lol x, lol a, lol b) {
	return min(b + a, a * (x + 2));
}

int main() {
	ios::sync_with_stdio(0);
	cin.tie(0);
	defn defa defb defc defd
		vecl mintoend(n + 1);
	vecl arr(n);
	intov(arr, n)
		mintoend[0] = -d;
	mintoend[1] = a*arr[0]+c;
	fort(i, 2, n+1) {
		mintoend[i] = lmaxi;
		min2(mintoend[i], mintoend[i - 1] + d + arr[i - 1] * a + c)
		min2(mintoend[i], mintoend[i - 1] + 3 * d + turemin(arr[i - 1], a, b))
		min2(mintoend[i], mintoend[i - 2] + 4 * d + turemin(arr[i - 1], a, b) + turemin(arr[i - 2], a, b))
	}
	lol sumtruemin = 0;
	forib(i, n - 1) {
		sumtruemin += turemin(arr[i], a, b);
		min2(mintoend[n], mintoend[i] + ((2 * n - 1 - 2 * i) * d) + arr[n - 1] * a + c + sumtruemin);
	}
	co mintoend[n] el
	ex
}