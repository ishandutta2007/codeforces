...
int n,a[N],b[N],dp[1010][5050],p[N];
int fac[N],ifac[N];
...
int main(){
	...
	dp[0][1]=1;
	for(int i=1;i<=n;++i){
		poly A(p[i]+1),B(p[i]+1);
		for(int j=0;j<=p[i];++j){	
			A[j]=(a[i]>=j?1LL*ifac[a[i]-j]*ifac[b[i]+j]%mod:0);
		}
		for(int k=1;k<=p[i];++k){
			B[k]=dp[i-1][k];
		}
		A=Mul(A,B);
		for(int j=1;j<=p[i];++j){
			dp[i][j]=(dp[i][j]+1LL*A[j]*fac[a[i]+b[i]])%mod;
		}
		A=poly(p[i-1]+1,0),B=poly(p[i-1]+1,0);
		for(int j=1;j<=p[i-1];++j){
			A[j]=(b[i]>=j?1LL*ifac[b[i]-j]*ifac[a[i]+j]%mod:0);	
		}
		...
		A=Mul(A,B);
		for(int j=1;j<=min(p[i-1],p[i]);++j){
			dp[i][j]=(dp[i][j]+1LL*fac[a[i]+b[i]]*A[p[i-1]+j])%mod;	
		}
		/*
		for(int j=1;j<=p[i];++j){
			for(int k=1;k<=min(j,p[i-1]);++k){
				dp[i][j]=(dp[i][j]+1LL*C(a[i]+b[i],b[i]+j-k)*dp[i-1][k])%mod;
			}
			for(int k=j+1;k<=p[i-1];++k){
				dp[i][j]=(dp[i][j]+1LL*C(a[i]+b[i],a[i]+k-j)*dp[i-1][k])%mod;
			}
		}	
		*/
	}
