...
int main() {
	int n; R(n);
	vector<vector<int>> a(n, vector<int>(n)); R(a);

	vector<vector<int>> r(n, vector<int>(n, 0)), c = r;
	auto go = [&](vector<vector<int>>& v) {
		for (int i = 0; i < n; i++) {
			vector<int> b(n);
			for (int j = 0; j < n; j++)
				b[j] = j;
