...
int main() {
    ...
    if (N <= 6) {
        int ans = INT_MAX, ways = 0;
        F0R (m, 1 << N) {
            vi reach(N);
            F0R (i, N) {
                reach[i] |= 1 << i;
                FOR (j, i+1, N) {
                    bool ij = (g[i][j] == '1') ^ ((m >> i)&1) ^ ((m >> j)&1);
                    if (ij) reach[i] |= 1 << j;
                    else reach[j] |= 1 << i;
                }
            }
            ...
            bool strong = 1;
            F0R (i, N)
                strong &= __builtin_popcount(reach[i]) == N;
            if (strong) {
                int flip = __builtin_popcount(m);
                if (flip < ans) {
                    ans = flip;
                    ways = 0;
                }
    ...
    auto count_sccs = [&]() {
        ...
        F0Rd (out, N) {
            F0R (_, with_out[out]) {
                nodes++;
                tot_out += out;
                if (tot_out == nodes * (N - nodes) + nodes * (nodes - 1) / 2)
                    ans++;
            }
        }
