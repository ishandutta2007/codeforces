...
int dfs(const int v,
	const vector<vector<pair<int, pii>>>& g,
	vi& h,
	vi& mi,
	int par = -1,
	int ans = 1)
	...
	for (auto it : g[v])
		if (it.X != par)
		{
			...
			while (y > 1)
			{
				...
				y = fac.without_md[y];
				h[mdn] -= pw;
				mi[mdn] = min(mi[mdn], h[mdn]);
			}
			x = xy.X;
			y = xy.Y;
			my_ans = sum(my_ans, dfs(to, g, h, mi, v, prod(ans, prod(x, inverse(y, M), M), M)), M);
			while (x > 1)
			{
				int mdn = fac.mdn[x];
				int pw = fac.pw[x];
...
int another_main()
{
	...
	for (int I = 0; I < T; ++I)
	{
		...
		cin >> n;
		int u = 0;
		while (fac.pr.size() > u && fac.pr[u] <= n)
			++u;
		vi h(u), mi(u), ma(u);
		...
		for (int i = 0; i < n - 1; ++i)
		{
			...
			cin >> p >> q >> y >> x;
			--p; --q;
			g[p].emplace_back(q, pii(x, y));
			g[q].emplace_back(p, pii(y, x));
		}
		int ans = dfs(0, g, h, mi);
		for (int i = 0; i < u; ++i)
			ans = prod(ans, pow_mod(fac.pr[i], -mi[i], M), M);
		cout << ans << '\n';
	}
	return 0;
}
