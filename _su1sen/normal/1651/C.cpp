...
int main() {
    ...
    loop(t) {
        ...
        long long ans = 0;
        loop(2) {
            for (long long x : { a.front(), a.back() }) {
                long long c = numeric_limits<long long>::max();
                rep(i, n) chmin(c, abs(x - b[i]));
                ans += c;
            }
            a.swap(b);
        }

        chmin(ans, abs(a.front() - b.front()) + abs(a.back() - b.back()));
        chmin(ans, abs(a.front() - b.back()) + abs(a.back() - b.front()));

        loop(2) {
            loop(2) {
                long long c = abs(a.back() - b.back());
                loop(2) {
                    long long x = numeric_limits<long long>::max();
                    rep(i, n) chmin(x, abs(a.front() - b[i]));
                    c += x;
                    a.swap(b);
                }
                chmin(ans, c);
