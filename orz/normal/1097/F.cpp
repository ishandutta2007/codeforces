...
int main()
{
	...
	for (int i = 1; i < M; ++i)
	{
		auto ff = f.factorize(i);
		bool good = true;
		for (int j = 0; good && (j < sz(ff)); ++j)
		{
			if (ff[j].Y > 1)
				good = false;
		}
	...
	vector<bitset<M>> dequad(M);
	for (int i = 1; i < M; ++i)
	{
		for (int j = 0; j < sz(free) && (i * free[j] < M); ++j)
		{
			dequad[i][i * free[j]] = 1;
		}
	}
	...
	for (int I = 0; I < q; ++I)
	{
		...
		else if (t == 4)
		{
			int x, w;
			cin >> x >> w;
			cout << (((int)((v[x - 1] & dequad[w]).count())) & 1);
		}
	}
	return 0;
}