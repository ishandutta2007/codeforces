...
const int nax = 1 << 18, sax = nax / 2;
bool flip[nax * 2], one[nax * 2], zero[nax * 2];
int value[nax];
void push(int x) {
	assert(flip[x] + one[x] + zero[x] <= 1);
	if (x >= nax) {
		if (flip[x]) value[x - nax] ^= 1;
		if (one[x]) value[x - nax] = 1;
		if (zero[x]) value[x - nax] = 0;
	}
	else {
		if (flip[x]) {
			for (int d = 2 * x; d <= 2 * x + 1; ++d) {
				if (zero[d]) {
					zero[d] = 0;
					one[d] = 1;
				}
	...
	flip[x] = one[x] = zero[x] = false;
}
void go(bool arr[], int l, int r, int wh = 1, int lb = 0, int rb = nax - 1) {
	push(wh);
	if (l > rb || r < lb) return;
	if (l <= lb && r >= rb) {
		arr[wh] = true;
	...
	int s = (lb + rb) / 2;
	go(arr, l, r, wh * 2, lb, s);
	go(arr, l, r, wh * 2 + 1, s + 1, rb);
}
void push_all() {
...
void tree_test() {
	for (int i = 0; i < nax; ++i)
		value[i] = value_bru[i] = rand() % 2;
	for (int i = 0; i < nax; ++i) {
		int l = rand() % nax, r = rand() % nax;
		if (l > r) swap(l, r);
		switch(rand() % 3) {
			case 0:
				for (int i = l; i <= r; ++i)
...
int main() {
	...
	for (int i = 0; i < q; ++i) {
		...
		if (c == '>') {
			if (x > 0) {
				go(one, x + sax + 1, nax - 1);
				go(zero, 0, sax - x - 1);
			}
			else {
				x = -x;
				go(zero, 0, sax - x);
				go(flip, sax - x + 1, sax + x - 1);
				go(one, sax + x, nax - 1);
			}
		}
		else {
