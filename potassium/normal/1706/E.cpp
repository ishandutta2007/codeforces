...
struct SparseTable {
	bool isBuilt = false;
	vector<vector<T>> st = vector<vector<T>>(1);	
	vector<T>& operator[] (int pw) { return st[pw]; }

	T query(int l, int r) {
		assert(l <= r);
		assert(isBuilt);
		int pw = __lg(r - l + 1);
		return f(st[pw][l], st[pw][r - (1 << pw) + 1]);
	}

	void build() {
		if (isBuilt) assert(false);
		isBuilt = true;
		int sz = st[0].size();
		for (int i = 1; ; i++) {
			int curSize = sz - (1 << i) + 1;
			if (curSize > 0) {
				st.emplace_back(vector<T>(curSize));
	...
	SparseTable(vector<T>& v) { st[0] = v; build(); }
	SparseTable(int size) { st[0].resize(size); }
};
...
int main() {
	CASE(t) {
		...
		vector<int> p(n + m);
		for (int u = 1; u < n + m; u++) p[u] = u;
		function<int(int)> find = [&](int u) { return u == p[u] ? u : p[u] = find(p[u]); };
		...
		SparseTable<int, mn> in_min = in;
		SparseTable<int, mx> in_max = in;
		SparseTable<int, mx> euler_st = euler;

		vector<int> ans(q, -1);
		for (int i = 0; i < q; i++) {
			int l,r; R(l,r); l--; r--;

			int x = in_min.query(l, r);
			int y = in_max.query(l, r);
			int val = euler_st.query(x, y);
			ans[i] = (val < n ? 0 : euler_st.query(x, y) - n + 1);
		}
