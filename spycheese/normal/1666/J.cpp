...
int main() {
  ...
  cin >> c;
  vector<vector<ll>> cs(n + 1, vector<ll>(n + 1));
  for (int i = 0; i < n; ++i)
    for (int j = 0; j < n; ++j)
      cs[i + 1][j + 1] = cs[i + 1][j] + cs[i][j + 1] - cs[i][j] + c[i][j];

  auto foo = [&](int l, int r) {
    ll c1 = cs[l][r] + cs[0][l] - cs[l][l] - cs[0][r];
    ll c2 = cs[n][r] + cs[r][l] - cs[r][r] - cs[n][l];
    return c1 + c2;
  };

  vector<vector<pair<ll, int>>> d(n + 1, vector<pair<ll, int>>(n + 1, {0LL, -1}));
  for (int l = n - 1; l >= 0; --l)
    for (int r = l + 1; r <= n; ++r) {
      pair<ll, int> cd = {LINF, -1};
      for (int i = l; i < r; ++i)
        cd = min(cd, make_pair(d[l][i].X + d[i + 1][r].X + foo(l, i) + foo(i + 1, r), i));
      d[l][r] = cd;
    }

  vector<int> ans(n);
  function<void(int, int, int)> go = [&](int l, int r, int pred) {
    if (l == r) return;
    int i = d[l][r].Y;
    ans[i] = pred;
    go(l, i, i + 1);
