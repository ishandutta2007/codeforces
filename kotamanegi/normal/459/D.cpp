...
void solve(){
    ...
    int cnt = 0;
    for (auto x = at.begin(); x != at.end(); ++x) {
        x->second = cnt;
        cnt++;
    }
    SegmentTree<int> nexts(n + 1, [](int a, int b) {return a + b; }, 0LL);
    REP(i, n) {
        inputs[i] = at[inputs[i]];
        cnts[1][inputs[i]]++;
        nexts.update(cnts[1][inputs[i]], nexts[cnts[1][inputs[i]]] + 1);
    }
    int ans = 0;
    REP(i, n) {
        nexts.update(cnts[1][inputs[i]], nexts[cnts[1][inputs[i]]] - 1);
        cnts[1][inputs[i]]--;
        cnts[0][inputs[i]]++;
        ans += nexts.query(0, cnts[0][inputs[i]]);
    }
    cout << ans << endl;
}
#undef int 
