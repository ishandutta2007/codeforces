...
int main() {
    ...
    auto slow_ans = [&] {
        ...
        for (int m = 1; m < 1 << Z; m++) {
            ...
            for (int i = 0; i < Z; i++) {
                if ((m >> i) & 1) {
                    ckmax(l, intervals[i].first);
                    ckmin(r, intervals[i].second);
                }
            }

            mn ways = (r - l <= 0) ? 1 : mn::ncr(r - l, prefix[r] - prefix[l]);
            ans += (__builtin_popcount(m) % 2 ? +1 : -1) * ways;
        }
        return ans;
    }();
#endif
    ...
    mn ans = 0;
    mn tot = 0;
    for (int left = 0; left < sz(intervals); left++) {
        for (int right = left; right < sz(intervals); right++) {
            int r = intervals[left].second, l = intervals[right].first;
            if (r - l < 0)
                continue;
            ...
            if (left == right) {
                ...
                tot += mn(2).pow(right - (left + 1));
            }

            pd(l, r, mul);

            ans += mul * mn::ncr(r - l, prefix[r] - prefix[l]);
        }
    }
