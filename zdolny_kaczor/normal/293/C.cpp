...
void run_bru() {
	FOR(a, 1, nax)
		FOR(b, a, nax) {
			if (3ll * a * a * b + 3ll * a * b * b > nax) break;
			FOR(c, b, nax) {
				ll v = cub(a + b + c) - cub(a) - cub(b) - cub(c);
				if (v < nax) sol[v].push_back({a, b, c});
				else break;
			}
		}
	int g = 0;
	REP(i, nax) if (sol[i].size() >= 2u) {
		debug(arr(sol, i));
		g = gcd(g, i);
	}
	debug(imie(g));
...
void test_mod() {
	for (int mod = 2; mod <= 50; ++mod) {
		bool x[mod][mod][mod] = {};
		REP(i, mod) REP(j, mod) REP(k, mod) x[i][j][k] = 0;
		...
		REP(i, mod) {
			if (!i) continue;
			int cou = 0;
			REP(j, mod) REP(k, mod) cou += x[j][k][i];
			debug(imie(i), imie(cou));
			maxi(worst, cou);
		}
		debug(imie(mod) * mod / imie((ld)worst));
	}
}
ll root(ll x) {
	ll a = sqrt(x);
...
int solve(ll k) {
	...
	for (ll i = 1; i * i * i <= k; ++i)
		if (k % i == 0)
			divs.push_back(i);
	for (ll a = 1; a * a * a * 8 <= k; ++a) {
		for (auto it = lower_bound(ALL(divs), 2 * a); it != divs.end(); ++it) {
			ll b = *it - a;
			if (F(a, b, b) > k) break;
			ll sq = a + b, li = (a + b) * (a + b), co = a * b * (a + b) - k;
			ll del = li * li - 4 * sq * co;
			ll del2 = K(K(a) - K(b)) + 4 * k * (a + b);
			assert(del == del2);
			ll r = root(del);
			if (r != -1) {
				ll num = r - li, den = 2 * sq;
				if (num % den == 0) {
					ll c = num / den;
					...
					int per = 6;
					if (a == b && b == c) per = 1;
					else if (a == b || b == c) per = 3;
					ans += per;
				}
				else {
					debug(imie(num), imie(den));
				}
			}
		}
	}
...
void precompute(ll bound) {
	map<ll, int> ans;
	for (ll a = 1; F(a, a, a) <= bound; ++a)
		for (ll b = a; F(a, b, b) <= bound; ++b)
			for (ll c = b; F(a, b, c) <= bound; ++c) {
				int per = 6;
				if (a == b && b == c) per = 1;
				else if (a == b || b == c) per = 3;
				ans[F(a, b, c)] += per;
