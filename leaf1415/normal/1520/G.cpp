...
ll h, w, c;
ll a[2005][2005];
ll distS[2005][2005], distT[2005][2005];

void bfs(int sx, int sy, ll dist[2005][2005])
{
...
int main(void)
{
	...
	bfs(1, 1, distS), bfs(w, h, distT);
	
	ll ans = distS[w][h], minS = inf, minT = inf;
	rep(y, 1, h) rep(x, 1, w){
		if(a[x][y] > 0){
			chmin(minS, distS[x][y] + a[x][y]);
			chmin(minT, distT[x][y] + a[x][y]);
		}
	}
	chmin(ans, minS+minT);
