...
void dfs(int x) {
	debug(imie(x));
	vi down;
	for (auto [y, num] : graf[x]) if (!done[num]) {
		ans.push_back(num);
		done[num] = true;
		deg[y]--;
		down.push_back(y);
	}
	for (int x : down) if (deg[x] <= w[x]) dfs(x);
}
int main() {
	int n, m;
	scanf("%d%d", &n, &m);
	...
	for (int i = 1; i <= n; ++i) if (deg[i] <= w[i]) dfs(i);
	if ((int)ans.size() != m) err();
	printf("ALIVE\n");
	for (int x : reversed(ans)) printf("%d ", x + 1);
	printf("\n");
	exit(0);
}
//#STAY AT HOME