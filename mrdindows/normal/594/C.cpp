...
int main() {
    ...
    sort(a, a + n);
    sort(b, b + n);
    long long ans = ((a[n - 1].first - a[0].first + 1) / 2) * 1ll * ((1 + b[n - 1].first - b[0].first) / 2) ;
    for (int dl = 0; dl <= k; ++dl) {
        for (int dr = 0; dr + dl <= k; ++dr) {
            ...
            for (int i = 0; i < dr; ++i) {
                used[a[n - 1 - i].second] = true;
            }
            
            ll dx = a[n - 1 - dr].first - a[dl].first;
            int left = k - dl - dr;
            vs = 0;
            for (int i = 0; i < n; ++i) {
                if (!used[b[i].second]) {
            ...
            if (dx == 0) dx = 1;
            if (dy == 0) dy = 1;
            ll res = (dx + 1) / 2 * 1ll * ((dy + 1) / 2);
            if (res < ans) {
                ans = res;
            }
            memset(used, 0, n);
            for (int i = 0; i < dl; ++i) {
                used[a[i].second] = false;
            }
            for (int i = 0; i < dr; ++i) {
                used[a[n - 1 - i].second] = false;
            }
        }
    }
    ...
    for (int dl = 0; dl <= k; ++dl) {
        for (int dr = 0; dr + dl <= k; ++dr) {
            ...
            for (int i = 0; i < dr; ++i) {
                used[b[n - 1 - i].second] = true;
            }
            
            ll dx = b[n - 1 - dr].first - b[dl].first;
            int left = k - dl - dr;
            vs = 0;
            for (int i = 0; i < n; ++i) {
                if (!used[a[i].second]) {
            ...
            memset(used, 0, n);
            for (int i = 0; i < dl; ++i) {
                used[b[i].second] = false;
            }
            for (int i = 0; i < dr; ++i) {
                used[b[n - 1 - i].second] = false;
            }
        }
    }
    if (ans == 0) ans = 1;
