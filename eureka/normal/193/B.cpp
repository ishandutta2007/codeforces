...
i64 getans(int d){
    i64 ans = 0;
    ff(i, n) ans += a[d][i] * k[i];
    return ans;
}

i64 gao(int d, bool flag){
    i64 ans = 100000000;
    ans = - ans * ans;
    if((u-d) % 2 == 0) checkmax(ans, getans(d));
    if(d >= u) return ans;
    if(flag){
        ff(i, n) a[d+1][i] = a[d][i] ^ b[i];
        checkmax(ans, gao(d+1, false));
    }
    ff(i, n) a[d+1][i] = a[d][p[i]-1] + r;
    checkmax(ans, gao(d+1, true));
    return ans;
}
