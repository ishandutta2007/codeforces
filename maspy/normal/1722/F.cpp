...
void solve() {
  ...
  auto idx = [&](int a, int b) -> int { return W * a + b; };

  UnionFind uf(H * W);
  FOR(x, H) FOR(y, W) {
    FOR(d, 8) {
      int nx = x + dx[d], ny = y + dy[d];
      if (isin(nx, ny) && G[x][y] == G[nx][ny]) {
        uf.merge(idx(x, y), idx(nx, ny));
      }
    }
  }

  vvc<int> comp(H * W);
  FOR(v, H * W) comp[uf[v]].eb(v);
  ...
  FOR(x, H) FOR(y, W) {
    ...
    if (len(Y) != 2) return NO();
    if (X[1] - X[0] != 1) return NO();
    if (Y[1] - Y[0] != 1) return NO();
  }
  YES();
}
