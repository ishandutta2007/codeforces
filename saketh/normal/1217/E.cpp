...
int main() {
    ...
    const int MAXD = 9;
    using node = array<pii, MAXD>;

    auto nn = [](node a, node b) {
        node c;
        F0R (i, 9) {
            if (a[i].f < b[i].f) {
                c[i] = { a[i].f, min(a[i].s, b[i].f) };
            } else {
                c[i] = { b[i].f, min(a[i].f, b[i].s) };
            }
        }
        return c;
    };

    node id; fill(all(id), mp(INT_MAX, INT_MAX));
    auto build = [&](int v) {
        node res = id;
        int tmp = v;
        F0R (d, MAXD) {
    ...
    segment_tree st(N, id, nn);
    st.set_leaves([&](int i) { return build(a[i]); });

    F0R (m, M) {
        int t, x, y; re(t, x, y);
        if (t == 1) {
            a[--x] = y;
            st.replace(x, build(a[x]));
        } else {
            node n = st(--x, y);
            int ans = INT_MAX;
            F0R (d, MAXD) if (n[d].s < INT_MAX) {
                ckmin(ans, n[d].f + n[d].s);
            }
