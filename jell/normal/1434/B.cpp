#line 1 "codeforces-workspace\\contest\\1434\\b\\b.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\1434\\b\\b.cpp"
...
void main() {
  ...
  cin >> n;
  vector<int> pos(n);
  vector<int> spc(n * 2, -1);
  vector<int> suc(n, -1);
  map<int, int> rem;
  for (auto i = 0, j = 0, p = -1; i != n * 2; ++i) {
    char c;
    cin >> c;
    if (c == '-') {
      int id;
  ...
  for (auto i = 0; i != n; ++i) {
    for (auto j = pos[i] - 1; j >= 0 && ~spc[j]; j--) {
      can[i].push_back(j);
    }
    if (can[i].empty()) {
      cout << "NO\n";
    ...
    ans[spc[can[i].back()]] = i + 1;
    can[i].pop_back();
    rem.erase(pos[i]);
    auto su = rem.lower_bound(pos[i]);
    if (su != end(rem)) {
      merge(can[su->second], can[i]);
    }
  }
  cout << "YES\n";
  cout << ans << eol;
