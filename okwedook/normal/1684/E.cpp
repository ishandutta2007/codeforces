...
void solve() {
    ...
    sort(byc);
    vector<int> ptr(sz(byc));
    for (int i = 0; i < sz(ptr); ++i) {
        ptr[i] = i;
    }
    Fenwick<int> fenw(sz(byc)), fenw_cnt(sz(byc));
    for (int i = 0; i < sz(byc); ++i) {
        // msg("ADD ", byc[i]);
        fenw.add(i, byc[i]);
        fenw_cnt.add(i, 1);
    }
    auto del = [&](int c) {
        // msg("DEL ", c);
        int i = lower_bound(all(byc), c) - byc.begin();
        int ind = ptr[i]++;
        fenw.add(ind, -byc[ind]);
        fenw_cnt.add(ind, -1);
    };
    int skip = 0;
    int ans = mod;
    for (int mex = 0; mex <= n; ++mex) {
        ...
        int ind = fenw.lb(k + 1);
        // sum at [0, ind] > k - skip
        int cntdel = fenw_cnt.get(ind - 1);
        int cntall = fenw_cnt.get(sz(byc) - 1);
        chmin(ans, mex + (cntall - cntdel) - mex);
        if (cnt.count(mex)) {
            del(cnt[mex]);
        } else {
            ++skip;
