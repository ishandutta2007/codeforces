...
bool solve() {
	...
	vector<vector<int>> ids(rmax, vi(cmax));

	rep(i, 1, rmax) rep(j, 0, cmax) { ids[i][j] = v++; }
	...
	rep(walls) {
		...
		if(tp == 'C') {	 // circle
			...
			cin >> r1 >> r2 >> theta;
			rep(r, r1, r2) {
				int pre = (theta + cmax - 1) % cmax;
				wallset.emplace(ids[r][pre], ids[r][theta]);
			}
		}
	}

	auto adde = [&](int i, int j) -> void {
		if(wallset.count(make_pair(i, j)) || wallset.count(make_pair(j, i))) return;
		g.add_edge(i, j);
		return;
	};
	...
	rep(j, 0, cmax) {
		adde(center, ids[1][j]);
		adde(outside, ids[rmax - 1][j]);
		rep(r, 1, rmax - 1) { adde(ids[r][j], ids[r + 1][j]); }

		const int angnxt = (j + 1) % cmax;
		rep(r, 1, rmax) { adde(ids[r][angnxt], ids[r][j]); }
	}

	return g.bfs(center).at(outside) < g.INF;
}
