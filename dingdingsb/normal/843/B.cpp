...
#define lc (x<<1)
#define rc (x<<1|1)
int n,st,x,ans=-1,nxt;
pair<int,int>ask(int pos){
	cout<<"? "<<pos<<endl;
	int a,b;cin>>a>>b;return mp(a,b);
}
void out(int x){
	cout<<"! "<<x<<endl;
	exit(0);
...
signed main(){
	...
	for(int i=1;i<=999;i++){
		int pos=rint(1,n);
		pair<int,int>res=ask(pos);
		if(res.fi<=x&&res.fi>ans)tie(ans,nxt)=res;
	}
	if(ans==x)out(x);
	for(int i=1;i<=999;i++){
		if(nxt==-1)break;
		pair<int,int>res=ask(nxt);
		if(res.fi>=x)out(res.fi);
		tie(ans,nxt)=res;
	}
	out(ans>=x?ans:-1);
}