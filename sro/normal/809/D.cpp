...
namespace treap{
	...
	void split(node now,int val,node &L,node &R){
		if(!now){
			L=R=0;
			return;
		}
		pushdown(now);
		if(now->val>=val){
			split(now->l,val,L,now->l);
			R=now;
		}else{
			split(now->r,val,now->r,R);
			L=now;
		}
	}
	...
	int getans(node now){
		...
		pushdown(now);
		return getans(now->l)+getans(now->r)+(now->val<Inf);
	}
	int getlmost(node now){
		pushdown(now);
		if(!now->l){
			return now->val;
		}else{
			return getlmost(now->l);
		}
	}
}
...
int main(){
	jtrand.seed(time(0));
	fio>>n;
	root=treap::newnode(0);
	cont(i,n){
		fio>>l[i]>>r[i];
		treap::merge(root,root,treap::newnode(Inf+i));
	}
	cont(i,n){
		treap::node L,M,R;
		treap::split(root,l[i],L,R);
		treap::split(R,r[i],M,R);
		if(M){
			M->toadd++;
			M->val++;
		}
		treap::node shit;
		split(R,getlmost(R)+1,shit,R);
		treap::merge(root,L,treap::newnode(l[i]));
		treap::merge(root,root,M);
		treap::merge(root,root,R);
	}
	fio<<getans(root)-1<<ln;
	return 0;
}