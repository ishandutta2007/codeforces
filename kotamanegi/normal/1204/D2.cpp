...
int itr[2][200000];
int uf(int a, int b) {
    if (a < 0 or a >= s.length()) return a;
    if (itr[b][a] == a) return a;
    return itr[b][a] = uf(itr[b][a], b);
}
void solve(){
    cin >> s;
    queue<int> nexts;
    ...
    while (!nexts.empty()) {
        int now = nexts.front();
        nexts.pop();
        if (itr[0][now] != now) continue;
        int next = uf(now + 1, 1);
        if (next >= s.length()) continue;
        if (s[now] != '1' or s[next] != '0') continue;
        itr[0][now] = now - 1;
        itr[1][now] = now + 1;
        itr[0][next] = next - 1;
        itr[1][next] = next + 1;
        next = uf(now, 0);
        if (next >= 0) nexts.push(next);
    }
    REP(i, s.length()) {
        if (itr[0][i] == i) {
            s[i] = '0';
