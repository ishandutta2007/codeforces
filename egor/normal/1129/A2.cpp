...
public:
    void solve() {
        ...
        int full = *max_element(all(qty));

        arri answer(n, 0);
        for (int i : range(n)) {
            for (int j : range(i, n)) {
                if (qty[j] == full) {
                    maxim(answer[i], last[j] - i + (full - 1) * n);
                } else if (qty[j] > 0 && qty[j] == full - 1) {
                    maxim(answer[i], last[j] - i + (full - 2) * n);
                }
            }
            for (int j : range(i)) {
                if (qty[j] == full) {
                    maxim(answer[i], last[j] - i + (full) * n);
                } else if (qty[j] > 0 && qty[j] == full - 1) {
                    maxim(answer[i], last[j] - i + (full - 1) * n);
                }
            }
