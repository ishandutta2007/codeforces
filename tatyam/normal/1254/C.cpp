...
int main(){
    LL(n);
    const ll S=n-1,T=n-2;
    vec(ll,d,n-2);
    rep(n-2)d[i]=area(S,T,i)*sign(S,i,T);
    auto idx=iota(n-2);
    sort(all(idx),[&](ll x,ll y){return d[x]<d[y];});
    vector<ll>ans={S};
    if(d[idx.back()]>0){
        const ll x=idx.back();
        idx.pop_back();
        vector<ll>a,b;
        each(i,idx)if(d[i]>0)(sign(S,i,x)==1?a:b).push_back(i);
        ans.insert(ans.end(),all(a));
        ans.push_back(x);
        ans.insert(ans.end(),rall(b));
    }
    ans.push_back(T);
    Rev(idx);
    if(idx.size()&&d[idx.back()]<0){
        const ll x=idx.back();
        idx.pop_back();
        vector<ll>a,b;
        each(i,idx)if(d[i]<0)(sign(T,i,x)==1?a:b).push_back(i);
        ans.insert(ans.end(),all(a));
        ans.push_back(x);
        ans.insert(ans.end(),rall(b));
    }
    rotate(ans.begin(),find(all(ans),0),ans.end());
    answer(ans);
}