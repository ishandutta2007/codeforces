...
void solve() {
	...
	seg.emplace_back(-inf, inf);
	renum[-inf];
	renum[inf];
	n++;
	int cou = 0;
	for (auto &[_, c] : renum) c = cou++;
	for (auto &[l, r] : seg) {
		l = renum[l];
		r = renum[r];
	}
	vi dp(n);
	int ans = 0;
	sort(ALL(seg), [](pii a, pii b) {return a.second - a.first < b.second - b.first;});
	REP(i, n) {
		auto [l, r] = seg[i];
		vector <pair <pii, int> > sub;
		REP(j, i) {
			auto [li, ri] = seg[j];
			if (li >= l && ri <= r) sub.emplace_back(make_pair(li, ri), dp[j]);
		}
		vector <vpii> back(cou);
		for (auto [p, c] : sub) {
			auto [l, r] = p;
			back[r].emplace_back(l, c);
		}
		vi sub_dp(cou);
		int curr = 0;
		for (int i = 1; i < cou; ++i) {
			sub_dp[i] = sub_dp[i - 1];
			for (auto [t, c] : back[i]) maxi(sub_dp[i], sub_dp[t] + c);
			maxi(curr, sub_dp[i]);
		}
		maxi(ans, dp[i] = curr + 1);
	}
	printf("%d\n", ans - 1);
}
int main() {
