	...
	void solve()
	{
		...
		for(int i = T;i >= 1;i--){
			...
			for(int j = 0;j <= fp[i];j++){
				for(int k = 0;k <= n;k++){
					if(k-j >= 0)ndp[k-j] = Math.max(ndp[k-j], dp[k] + get);
				}
				if(j < fp[i])get += f[i][fp[i]-j-1];
			}
			for(int k = n;k >= 0;k--){
				int ind = Math.min(2*k, n);
				ndp[ind] = Math.max(ndp[ind], ndp[k]);
			}
			
			dp = ndp;
		}
		long ret = Long.MIN_VALUE / 10;
		for(int k = 0;k <= n;k++){
