...
void print_dif(vector<int> a)
{
    for (int i = 0; i+1<a.size(); i++) cout<<a[i+1]-a[i]<<' ';
    cout<<endl;
}
/*
...
{
    if (k == 0) return 0;
    if (mapka.find({v, k})==mapka.end())
    {
        ll ans = s[v]*k;
        int ch = G[v].size();

        if (ch == 0) mapka[{v, k}] = ans;

        else
        {
            if (k%ch == 0)
            {
                for (auto it: G[v]) ans+=dfs(it, k/ch);
                mapka[{v, k}] = ans;
            }
            else
            {
                ...
                {
                    ll low = dfs(it, k/ch);
                    ll high = dfs(it, k/ch + 1);
                    ans+=low;
                    extra.push_back(high-low);
                }
                sort(extra.begin(), extra.end());
                reverse(extra.begin(), extra.end());

                for (int i = 0; i<k%ch; i++) ans+=extra[i];
                mapka[{v, k}] = ans;
            }
        }
    }
