...
ll a[202020],minn[202020],pd[202020];
void work(void) {
	ll nn;
	cio>>nn;
	char ch;
	for(ll i=1;i<=nn;++i) cio>>ch,a[i]=ch-48;
	minn[nn]=a[nn];
	for(ll i=nn-1;i;--i) minn[i]=std::min(minn[i+1],a[i]);
	for(ll ii=0;ii<=9;++ii) {
		ll m=0,n=0,p=0;
		for(ll i=1;i<=nn;i++) {
			if(a[i]<ii) {
		...
		//cio<<a+1<<a+nn+1;
		//for(ll i=1;i<=nn;i++) cio<<(a[i]>ii)+1;
		if(!p) {
			cio<<ds(0)<<pd+1<<pd+nn+1;
			return;
		}
	}
	cio<<'-'<<eol;
