...
struct DSU{
    ...
    DSU(int n){
        ...
        edges.resize(n+1);
        for (int i=0;i<=n;++i) p[i]=i, sz[i]=1, dp[i][1]=1;
    }
    int find(int u){
        if (p[u]==u) return u;
    ...
    bool unite(int u,int v){
        ...
        if (pu!=pv){
            vi f(sz[pu]+sz[pv]+1,0);
            rep1(i,sz[pu]) rep1(j,sz[pv]) f[i+j]=(f[i+j]+dp[pu][i]*dp[pv][j])%998244353;
            rep1(i,sz[pu]+sz[pv]) dp[pv][i]=f[i];
            edges[pv]+=edges[pu];
            sz[pv]+=sz[pu], p[pu]=pv;
        }
        edges[pv]++;
        if (sz[pv]*(sz[pv]-1)==2*edges[pv]) dp[pv][1]=1;
        return pu!=pv;
    }
};
...
signed main(){
    ...
    cin>>n;
    DSU dsu(n);
    rep1(i,n) rep1(j,n) cin>>g[i][j], a[g[i][j]]=make_pair(i,j);
    rep1(i,n*(n-1)/2){
        dsu.unite(a[i].F,a[i].S);
    }
    int rt=dsu.find(1);
    rep1(i,n) cout<<dsu.dp[rt][i]%998244353<<" ";
    return 0;
}