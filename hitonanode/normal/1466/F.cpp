...
int main()
{
    ...
    REP(i, N) {
        ...
        V[i].resize(k);
        cin >> V[i];
        for (auto &x : V[i]) x--, xs.emplace_back(x);
    }
    xs = sort_unique(xs);
    dbg(xs);

    M = xs.size();
    UnionFind uf(M), uf1(M);
    uf1.cou.assign(M, 0);
    REP(i, N) {
        for (auto &x : V[i]) {
            x = lower_bound(ALL(xs), x) - xs.begin();
        }
        if (V[i].size() == 2) { uf.unite(V[i][0], V[i][1]), uf1.unite(V[i][0], V[i][1]); }
        else uf1.cou[uf1.find(V[i][0])] = 1;
    }
    dbg(V);
    map<int, mint> mp;
    REP(i, M) {
    ...
    dbg(mp);
    mint ret = 1;
    for (auto [r, sz] : mp) ret *= sz;
    UnionFind uf3(M + 1);
    // uf3.cou.assign(M, 0);
    vector<int> retuse;
    REP(i, N) {
        if (V[i].size() == 1) {
            if (uf3.unite(V[i][0], M)) retuse.emplace_back(i);
        } else {
            if (uf3.unite(V[i][0], V[i][1])) retuse.emplace_back(i);
        }
        // if (V[i].size() == 1) {
        //     if (!uf3.count(i)) uf3.cou[uf3.find(i)]++, retuse.emplace_back(i);
        // } else {
        //     if (!uf3.same(V[i][0], V[i][1]) and (!uf3.count(V[i][0]) or !uf3.count(V[i][1]))) retuse.emplace_back(i), uf3.unite(V[i][0], V[i][1]);
        // }
    }
    cout << ret << ' ' << retuse.size() << '\n';
    for (auto x : retuse) cout << x + 1 << ' ';
    cout << '\n';
}