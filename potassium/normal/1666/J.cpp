...
int main() {
	int n; R(n);
	vector<vector<ll>> a(n, vector<ll>(n)); R(a);
	...
	vector<vector<ll>> dp(n, vector<ll>(n, 1e16));
	auto get = [&](int l, int r) -> ll {
		if (l > r) return 0;
		return cost[l][r] + dp[l][r];
	};

	vector<vector<ll>> opt(n, vector<ll>(n, -1));
	...
	for (int len = 1; len <= n; len++) {
		for (int l = 0, r = len - 1; r < n; l++, r++) {
			for (int root = l; root <= r; root++) {
				if (chkmin(dp[l][r], get(l, root - 1) + get(root + 1, r)))
					opt[l][r] = root;
			}
		}
	}
