...
void solve() {
    ...
    REP(i, s.length() + 1) {
        REP(p, 26) {
            ...
            for (int j = base.length(); j >= 0; --j) {
                int ok = 1;
                REP(t, j) {
                    if (base[base.length() - j + t] != s[t]) {
                        ok = 0;
                        break;
                    }
                }
        ...
        REP(p, 26) {
            ...
            REP(q, s.length() + 1) {
                REP(j, t.length() + 1) {
                    ...
                    next_q = Q[next_q][p];
                    next_j = P[next_j][p];
                    if (next_q == s.length()) cost++;
                    if (next_j == t.length())cost--;
                    dp[i + 1][next_q][next_j] = max(dp[i + 1][next_q][next_j], cost);
                }
            }
        }
    }
