// time-limit: 1000
// problem-url: https://codeforces.com/contest/1663/problem/F
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
int main(){
	...
	for(std::string t: {"vampire", "the", "buffy", "slayer"}){
		if(s.size()==t.size()){
			for(int const i: range(0, int(s.size()))){
				s[i]-='a';
				t[i]-='a';
				s[i]+=t[i];
				s[i]%=26;
