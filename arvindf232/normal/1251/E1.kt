...
//        var taken = 0
        var pt = -1
        for(limit in 0 until n){
            while(pt < n-1 && all[pt+1].first <= limit){
                pt ++
                set.addone(all[pt].second)
//                just dei all[pt]
            }
//            val cantake = n - limit
            if(set.isNotEmpty()){
//                taken ++
        ...
        val basecost = all.sumOf { it.second.toLong() }
        put(basecost - cost)
...
messed up two pointers
messed up the implementation trnasformatinos
