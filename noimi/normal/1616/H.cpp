...
const int N = 160000;
mint p2[N];
tuple<mint, mint, mint> f2(vi &l, vi &r, int g, int x) {
    // dump(l, r, g, x);
    if(empty(l) and empty(r)) return {0, 0, 0};
    int L = si(l), R = si(r);
    if(g == -1) return {p2[L] - 1, p2[R] - 1, (p2[L] - 1) * (p2[R] - 1)};
    // if(g == -1) { return {modpow(2, si(l)) - 1, modpow(2, si(r)) - 1, (modpow(2, si(l)) - 1) * (modpow(2, si(r)) - 1)}; }

    vi l1, l2, r1, r2;
    fore(e, l)(e >> g & 1 ? l1 : l2).eb(e);
    fore(e, r)(e >> g & 1 ? r1 : r2).eb(e);
    if(x >> g & 1) {
        auto [a1, a2, a3] = f2(l1, r2, g - 1, x);
        auto [b1, b2, b3] = f2(l2, r1, g - 1, x);
        return {a1 + b1 + a1 * b1, a2 + b2 + a2 * b2, a3 + b3 + a1 * b2 + a1 * b3 + a2 * b1 + a2 * b3 + a3 * (b1 + b2 + b3)};
        // return a + b + a * b;
    } else {
        auto [a1, a2, a3] = f2(l1, r1, g - 1, x);
        auto [b1, b2, b3] = f2(l2, r2, g - 1, x);
        return {p2[L] - 1, p2[R] - 1, a3 + b3};
    }
}
mint f1(vi &v, int g, int x) {
    if(empty(v)) return 0;
    if(g == -1) { return p2[si(v)] - 1; }
    dump(v, g, x);
    vi l, r;
    fore(e, v)(e >> g & 1 ? r : l).eb(e);
    if(x >> g & 1) {
        // return f2(l, r, g - 1, x ^ 1 << g) + modpow(2, si(l)) - 1 + modpow(2, si(r)) - 1;
        auto [a, b, c] = f2(l, r, g - 1, x);
        return a + b + c;
    } else {
        return f1(l, g - 1, x) + f1(r, g - 1, x);
    }
}
