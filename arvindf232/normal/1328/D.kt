...
fun main(){
    solve.cases{
        ...
        if(n %2 == 0){
            put(2)
            put(List(n){if(it and 1 ==0) 2 else 1 }.joinToString(" "))
            return@cases
        }
        if((0 until n).all{L[it] != L[(it+1)%n ]}){
            put(3)
            put(List(n){if(it == 0) 3 else if(it and 1 == 1) 2 else 1 }.joinToString(" "))
            return@cases
        }else{
            val fd = (0 until n).first{L[it] == L[(it+1) %n]}
            put(2)
            val ret = IntArray(n)
            ret[fd] = 2
            ret[(fd +1 ) % n] = 2
            for(i in 2 until n){
                ret[(fd + i ) % n] = (i and 1) + 1
            }
