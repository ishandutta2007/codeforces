...
public class D {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ArrayList<Integer> asta = new ArrayList<>();
        for (int n = 0; n < N; n++) {
            if (S.charAt(n) == 'P') {
                pack.add(n);
            } else if (S.charAt(n) == '*') {
        ...
        if (pack.size() == 1) {
            ...
            int right = asta.size() - left;
            if (left == right) {
                cp.println(left + " " + Math.min(asta.get(asta.size() - 1) - pack.get(0), pack.get(0) - asta.get(0)));
            } else if (left < right) {
                cp.println(right + " " + (asta.get(asta.size() - 1) - pack.get(0)));
            } else {
                cp.println(left + " " + (pack.get(0) - asta.get(0)));
            }
        } else {
            ...
            for (int n = 0; n < asta.size(); n++) {
                leftidx[asta.get(n)] = n;
                rightidx[asta.get(n)] = n;
            }
            for (int n = 0; n < N - 1; n++) {
                if (leftidx[n + 1] == -1) {
                    leftidx[n + 1] = leftidx[n];
                }
            }
            for (int n = N - 1; n > 0; n--) {
                if (rightidx[n - 1] == -1) {
                    rightidx[n - 1] = rightidx[n];
                }
            }
            int ok = N;
            int ng = 0;
            while (ok - ng > 1) {
                int x = (ok + ng) / 2;
                int[] dp = new int[pack.size() + 1];
                Arrays.fill(dp, -1);
                for (int n = 0; n < pack.size(); n++) {
                    if (dp[n] < asta.size() - 1 && asta.get(dp[n] + 1) >= pack.get(n) - x) {
                        dp[n + 1] = Math.max(dp[n + 1], leftidx[pack.get(n)]);
                    }
                    if (dp[n] < asta.size() - 1 && asta.get(dp[n] + 1) >= pack.get(n)) {
                        dp[n + 1] = Math.max(dp[n + 1],
                                pack.get(n) + x >= N ? asta.size() - 1 : leftidx[pack.get(n) + x]);
                    }
                    if (n > 0 && dp[n - 1] < asta.size() - 1 && asta.get(dp[n - 1] + 1) >= pack.get(n) - x) {
                        dp[n + 1] = Math.max(dp[n + 1],
                                pack.get(n - 1) + x >= N ? asta.size() - 1 : leftidx[pack.get(n - 1) + x]);
                    }
                    dp[n + 1] = Math.max(dp[n + 1], dp[n]);
                }
                if (dp[pack.size()] == asta.size() - 1) {
                    ok = x;
                } else {
                    ng = x;
                }
            }
            cp.println(asta.size() + " " + ok);
        }
        cp.close();
    }
