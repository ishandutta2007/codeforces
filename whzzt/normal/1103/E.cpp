...
struct _{
	u64 t[5];
	_(){memset(t,0,sizeof(t));}
	u64 &operator[](int x){ return t[x]; }
	friend _ operator+(_ a,_ b){
		_ c;
		for(int i=0;i<5;++i)c.t[i]=a.t[i]+b.t[i];
		return c;
	}
	friend _ operator-(_ a,_ b){
		_ c;
	...
	_ move(int l){
		_ c;
		for(int i=0;i<5;++i)c.t[(i+l)%5]=t[i];
		return c;
	}
	friend _ operator*(_ a,_ b){
		_ c; int i,j;
		for(i=0;i<5;++i) for(j=0;j<5;++j) c[(i+j)%5]+=a[i]*b[j];
		return c;
	}
	u64 get(){
		u64 sub = t[1];
		for (int i = 0; i < 5; ++i) t[i] -= sub;
		return t[0];
	}
} f[N], tmp[20];
void dft(_ *arr, int len){
	if (len == 1) return ;
	int i, j, k, l = len / 10, w;
	for (i = 0; i < len; i += l) {
		dft(arr + i, len / 10);
	}
	for (j = 0; j < l; ++j) {
		for (i = 0; i < 10; ++i) {
			...
			for (k = 0; k < 10; ++k) {
				w = i * k % 10;
				if (~w & 1) {
					tmp[i] = tmp[i] + arr[k * l + j].move(w >> 1);
				} else {
					tmp[i] = tmp[i] - arr[k * l + j].move(((w + 5) >> 1) % 5);
				}
			}
		}
		for (i = 0; i < 10; ++i) {
...
int main(){
	int i;
	pinv = 5, inv = 1;
	for (int t = 0; t < 63; ++t) inv *= pinv, pinv *= pinv;
	inv = inv * inv * inv * inv * inv;
	scan(n);
	read(x+1,x+n+1);
	for(i=1;i<=n;++i)++f[x[i]][0];
	dft(f,M);
	for(i=0;i<M;++i)f[i]=pow(f[i],n);
	idft(f,M);
	for(i=0;i<n;++i){
		u64 t=f[i].get();
		t>>=5,t*=inv,t&=0x3ffffffffffffffllu;
		printf("%llu\n",t);
	}
}