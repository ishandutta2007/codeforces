...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) b[i] = a[i];
		b[0] = inf;
		rep(i, 2, n) chmin(b[i], b[i-1]);
		
		set<P> S;
		rep(i, 1, n) if(b[i-1] != b[i]) S.insert(P(i, b[i]));
		S.insert(P(n+1, -1));
		
		ll k, d;
		rep(i, 1, m){
			cin >> k >> d;
			auto it = S.upper_bound(P(k, inf)); it--;
			a[k] -= d;
			if(it->first == k) S.erase(it);
			else if(it->second <= a[k]){
				cout << sz(S)-1 << " ";
				continue;
			}
			S.insert(P(k, a[k]));
			
			it = S.lower_bound(P(k, a[k])); it++;
			while(it->second >= a[k]){
				auto tmp = it; it++;
				S.erase(tmp);
			}
