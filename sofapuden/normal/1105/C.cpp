...
void solve(){
	...
	dp[0][2] = to[2];
	for(int i = 1; i < n; ++i){
		dp[i][0]+=dp[i-1][0]*to[0];
		dp[i][0]+=dp[i-1][1]*to[2];
		dp[i][0]+=dp[i-1][2]*to[1];
		dp[i][0]%=MOD;
		dp[i][1]+=dp[i-1][0]*to[1];
		dp[i][1]+=dp[i-1][1]*to[0];
		dp[i][1]+=dp[i-1][2]*to[2];
		dp[i][1]%=MOD;
		dp[i][2]+=dp[i-1][0]*to[2];
		dp[i][2]+=dp[i-1][1]*to[1];
		dp[i][2]+=dp[i-1][2]*to[0];
		dp[i][2]%=MOD;
	}
	cout << dp[n-1][0] << "\n";
