...
main() {
    INT(n, q);
    RAT<int> seg(n + 1);
    SegmentTree<int> sum(n + 1, plus<int>(), 0);
    vv(int, v, n + 1);
    VEC(int, a, n);
    rep(i, n) {
        int l = -1, r = n + 1;
        ...
        if(l == -1) continue;
        seg.update(0, min<int>(r, i + 1), 1);
        v[l].eb(i);
    }
    vpi w(q);
    rep(i, q) { cin >> w[i].fi >> w[i].se; }
    vi ans(q);
    auto id = iota(q);
    sort(all(id), [&](int i, int j) { return w[i].fi > w[j].fi; });
    int now = n;
    for(auto i : id) {
        auto [l, r] = w[i];
        while(now >= l) {
            for(auto e : v[now]) sum.add(e, 1);
            now--;
        }
        ans[i] = sum.query(l, n - r);
    }
