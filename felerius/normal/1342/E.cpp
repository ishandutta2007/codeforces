...
// Problem: E: Placing Rooks (https://codeforces.com/contest/1342/problem/E)
...
#undef DEFINE_CMP
    ...
    template <class T> constexpr enable_if_t<is_integral<T>::value, Modular&> operator/=(T r) { return *this *= Modular(r).inv(); }
    constexpr Modular operator/(Modular r) const { return Modular(*this) /= r; }
...
MI Sk(int n, int k) {
    ...
    MI i_f_inv = 1;
    rep(i, k + 1) {
        MI x = k_f * ki_f_inv * i_f_inv * MI(k - i).pow(n);
        if (i % 2)
            r -= x;
        else
            r += x;
