...
//    freopen("output.txt", "w", stdout);
    ...
    while (test--){
        ...
        cin>>n;
        int ans=0;
        for (int k=0;k<=min(inf,n/4);k++){
            int ways=1;
            ways=1ll*ways*(k==0 ? 1 : 1ll*ff[2*k-1]*power(2,k)%md)%md;
            ways=1ll*ways*rozkid(n-4*k,2*k+1)%md*dp1[n-4*k]%md;
            ans+=ways;
//            cout<<"k ways :: "<<k<<" "<<ways<<"\n";
            ans%=md;
        }
