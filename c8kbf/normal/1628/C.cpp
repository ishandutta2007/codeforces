...
_ dfs(_ xa, _ ya, _ xb, _ yb);
inline void AC() {
    
    n = read();
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= n; ++j) a[i][j] = read();
    writeln(dfs(1, 1, n, n));
...
_ dfs(_ xa, _ ya, _ xb, _ yb) {
    if(xa > xb) return 0;
    v(_p) g;
    for(_ i = ya; i < yb; ++i) g.push_back(mp(xa, i));
    for(_ i = xa; i < xb; ++i) g.push_back(mp(i, yb));
    for(_ i = yb; i > ya; --i) g.push_back(mp(xb, i));
    for(_ i = xb; i > xa; --i) g.push_back(mp(i, ya));
    _ rv = 0;
    for(_ i = 0; i < g.size(); i += 4) rv ^= a[g[i].first][g[i].second]^a[g[i+1].first][g[i+1].second];
    rv ^= dfs(xa+2, ya+2, xb-2, yb-2);
    return rv;
}