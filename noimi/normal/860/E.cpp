...
struct LCA_Tree {
    ...
    Graph &g, G;
    int root;
    LCA_Tree(Graph &g, int root = 0) : root(root), g(g), hld(g), G(g.size()) { hld.build(root); }
    int lca(int x, int y) { return hld.lca(x, y); }
    int d(int x) { return hld.d[x]; }
    vector<int> make(vector<int> &nodes) {
        // nodes  LCA  G 
        // root 
        //  LCA  vector
        ...
        {
            int num = nodes.size();
            rep(i, num - 1) { nodes.eb(lca(nodes[i], nodes[i + 1])); }
            nodes.eb(root);
        }
        sort(all(nodes), comp);
        nodes.erase(unique(all(nodes)), end(nodes));
        for(auto e : nodes) G[e].clear();
        stack<int> st;
        st.emplace(nodes.front());
        rep2(i, 1, (int)nodes.size() - 1) {
            while(hld.out[st.top()] <= hld.in[nodes[i]]) st.pop();
            G[st.top()].eb(nodes[i]);
            st.emplace(nodes[i]);
        }
        return nodes;
...
signed main() {
    ...
    rep2(i, 1, si(d) - 1) {
        for(auto e : d[i]) looking[e] = true;
        auto v = Tree.make(d[i]);
        {
            auto dfs = [&](auto &&f, int x) -> ll {
                sub[x] = looking[x];
        ...
        {
            auto dfs = [&](auto &&f, int x, ll now) -> void {
                if(looking[x]) { ans[x] = ans[Tree.hld.par[x]] + now + (Tree.d(x) + 1) - 1; }
                for(auto e : G[x]) { f(f, e, now + (sub[x] - sub[e]) * (Tree.d(x) + 1)); }
            };
            dfs(dfs, v[0], 0);
        }

        for(auto e : d[i]) looking[e] = false;
    }
    cout << ans << endl;
}