...
int main() {
    ...
    if (L <= 3) {
        auto ch = [&](ll x) {
            ...
            if (2e18 / a[0] <= x) { return true; }
            if (x*a[0] + a[1] >= K) { return true; }
            if (L <= 2) { return false; }

            if (x >= 2e9) { return true; }
            ll sum = x * (x + 1) / 2;
            if (2e18 / a[0] <= sum) { return true; }
            if (sum * a[0] + x * a[1] + a[2] >= K) { return true; }
            return false;
        };
    ...
    while (1) {
        ...
        repn(i, L - 1) {
            na[i] = na[i - 1] + a[i];
            if (na[i] >= K) { cout << ans << endl; return 0; }
        }
        ans++;
        a = na;
    }
