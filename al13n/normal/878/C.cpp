...
struct vec:public array<int,10>{
...
int main(int argc, char **argv){
  ...
  forn(i,n){
    ...
    for(W=0;W<k;++W){
      ...
      if(it2==S.begin()){
        if(mn==S.end() || *it2<*mn) mn=it2;
      }else{
        auto it1=it2;
        --it1;
        if(mx==S.end() || *it1>*mx) mx=it1;
        if(v<it1->Y.X){
          if(mn==S.end() || *it1<*mn) mn=it1;
        }else if(it2!=S.end()){
          if(mn==S.end() || *it2<*mn) mn=it2;
        }
      }
    ...
    W=0;
    if(mn!=S.end() && mx!=S.end() && !(*mx<*mn)){
      vec v1=v,v2=v;
      int s=1;
      ++mx;
      for(auto it=mn;it!=mx;it=S.erase(it)){
        forn(j,k){
          v1[j]=min(v1[j],it->X[j]);
          v2[j]=max(v2[j],it->Y.X[j]);
        }
        s+=it->Y.Y;
      }
      S[v1]=mp(v2,s);
