...
int main(void)
{
	...
	while(T--){
		cin >> n;
		rep(i, 1, n) rep(j, 1, n) vec[i][j].clear();
		rep(i, 1, n) rep(j, 1, n) rep(k, 1, n) b[i][j][k] = false;
		rep(i, 1, n) rep(j, 1, n) b[j][j][i] = true;
		...
		rep(i, 1, n-1){
			rep(j, i+1, n){
				...
				rep(k, 1, n){
					if(s[k-1] == '1'){
						b[i][j][k] = b[j][i][k] = true;
						vec[j][k].pb(i), vec[i][k].pb(j);
					}
				}
			}
		}
		...
		rep(i, 2, n){
			...
			rep(j, 1, n) if(pre[j] == -1) goto end;
			
			rep(j, 1, n) G[j].clear();
			for(auto p : ans) G[p.fi].pb(p.se), G[p.se].pb(p.fi);
			
			rep(j, 1, n) dfs(j, -1, 0, dist[j]);
			rep(j, 1, n) rep(k, 1, n) rep(l, 1, n){
				if(b[k][l][j] && dist[j][k] != dist[j][l]) goto end;
				if(!b[k][l][j] && dist[j][k] == dist[j][l]) goto end;
			}
			flag = true;
			break;
