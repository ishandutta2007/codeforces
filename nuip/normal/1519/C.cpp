...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		cauto rs=readV(n);
		vv<ll> vs(n);
		rep(i,n) vs[us[i]].eb(rs[i]);
		sort(rall(vs),[&](cauto &v,cauto &u){ return v.size()<u.size();});
		for(auto &v:vs){
			sort(rall(v));
			v.insert(v.begin(),0);
			reps(i,1,v.size()) v[i]+=v[i-1];
		}
		vector<ll> re(n+1);
		reps(i,1,n+1){
			while(vs.size() && vs.back().size()-1<i) vs.pop_back();
			out(i,vs,1);
			for(cauto &v:vs) re[i]+=v[(v.size()-1)/i*i];
		}
		out(re,1);
		rep(i,n) cout<<re[i+1]<<" \n"[i+1==n];
	}
