...
fun main(){
    ...
    solve.tc{
        share(200000)
        share(5)
        share(List(200000){Random.nextInt(1,5)})
    }
//    solve.usetc()
    solve.cases{
        val n = getint()
        val k = getint()
        val L = getline(n)
        var freechoices = L.windowed(2).count { it[0] == it[1] }
        freechoices += (L.last() == L.first()).chi
        val notfree = n - freechoices
        val frees = intPow(k,freechoices,pI)

        var ret = 0
        var inv = (k-2).inverse()
        var keep = intPow(k-2, notfree,pI)
        for(changes in 1..notfree){
            var base = intPow(2,changes,pI)
            if(changes % 2 == 0){
                val comb = FACT.choose(changes,changes/2)
//                changes dei base
//                changes dei comb
                base = base modMinus comb
            }

            base = base modM ((2).inverse())
//            changes dei base
            keep = keep modM inv
            val rest = if(changes == notfree) 1 else keep
            ret += (base) modM (rest) modM (FACT.choose(notfree, changes))
            ret = ret.adjust()
        }
        ret = ret modM frees
...
mistakes , fact starts with 1
and avoid 0 overrides
Combintroics at 0 is annoying
deadly variable swap ( kfact and halffact)
Powers can actually be stored and no recalculations
0 power annoyance
