...
// URL : https://codeforces.com/contest/1070/problem/I
// Memory Limit : 256 MB
// Time Limit : 3000 ms
...
#define MAXN 1501 // maximum number of vertices you will be using
#define int_t int
...
vector<ll> gt[2001],own[2001];
ll eu[2001],ev[2001],col[2001];
ll n,m,i,j,k,t,t1,u,v,a,b,tar;
int main()
{
	fio;
	cin>>t;
	...
	{
		gr.add_edge(0,i,1);
		gr.add_edge(i,m+eu[i],1);
		gr.add_edge(i,m+ev[i],1);
	}
	tar=0;
	for (i=1;i<=n;i++)
	{
		u=max(0,gt[i].size()-k);
		tar+=2*u;
		gr.add_edge(m+i,m+n+1,2*u);
	}
//	gr.debug();	
	u=gr.calc_flow(0,m+n+1);
	...
	{
		for (auto g : gr.lis_t) if ((g.u>=1)and(g.u<=m)and(g.v>=m+1)and(g.v<=m+n)and(g.cap==0))
		{
			own[g.v-m].push_back(g.u);
		}
		u=0;
		for (i=1;i<=n;i++) 
		for (j=0;j<own[i].size();j+=2)
		{
			u++;
			col[own[i][j]]=u;
			col[own[i][j+1]]=u;
		}
		for (i=1;i<=m;i++) if (col[i]) cout<<col[i]<<' '; else cout<<100000-i<<' ';
	}
	cout<<endl;
