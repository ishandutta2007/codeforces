...
int main() {
    vi f{1, 1};
    while(f.back() < ten(9)) { f.eb(f[si(f) - 1] + f[si(f) - 2]); }
    TEST {
        int k = -1;

        vi cnt(si(f)), cnt2(si(f));
        auto get = REC([&](auto &&sf, int n) -> void {
            int t = ub(f, n) - 1;
            if(f[t] == n)
                cnt2[t]++, cnt[t]++;
            else {
        ...
        per(i, si(f), 2) {
            if(i >= k) {
                if(cnt2[i]) {
                    per(j, i, 0, 2) { cnt[j] += cnt2[i]; }
                    cnt[i] -= cnt2[i];
                }
            } else {
                if(cnt[i] > 1 and cnt2[i]) {
                    int m = min(cnt[i] - 1, cnt2[i]);
                    per(j, i, 0, 2) cnt[j] += m;
                    cnt[i] -= m;
                }
            }
        }
        if(cnt[1] > 1 and cnt[1]) {
            cnt[1]--;
            cnt[0]++;
        }
        // dump(cnt2);
