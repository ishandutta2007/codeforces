...
int main(){
  ...
  auto [in,out]=inout(g,0);
  auto seg=add_min::make(n);
  rep(i,0,n)seg.set(i,INF/2);
  {
    auto dfs=[&](auto &&f,ll k,ll d)->void{
      if(g[k].empty())seg.set(k,d);
      for(auto e:g[k]){
        f(f,e.to,e.cost+d);
      }
    };
  ...
  vector<ll>ret(q);
  {
    auto dfs=[&](auto &&f,ll k)->void{
      ...
      //seg.print();
      for(auto z:gq[k]){
        ret[z]=seg.query(l[z],r[z]);
      }
      for(auto e:g[k]){
        seg.update(0,n,e.cost);
        seg.update(in[e.to],out[e.to],-2*e.cost);
        f(f,e.to);
        seg.update(0,n,-e.cost);
        seg.update(in[e.to],out[e.to],2*e.cost);
      }
