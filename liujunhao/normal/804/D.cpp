...
//template
#define MAXN 100000
int n,m,Q,bl[MAXN+10],now,f[MAXN+10],fr[MAXN+10];
vector<int>g[MAXN+10],a[MAXN+10];
vector<LL>sum[MAXN+10];
map<pii,double>vis;
void read(){
...
void solve(){
	...
	while(Q--){
		...
		if(bl[u]==bl[v])
			...
			if(a[bl[u]].size()>a[bl[v]].size())
				swap(u,v);
			if(vis.count(pii(bl[u],bl[v]))){
				printf("%.10f\n",vis[pii(bl[u],bl[v])]);
				continue;
				
			}
			int d=max(a[bl[u]].back(),a[bl[v]].back());
			LL ret(0);
			for(auto x:a[bl[u]]){
				uint t=upper_bound(all(a[bl[v]]),d-x-1)-a[bl[v]].begin();
				ret+=1ll*t*d;
				if(t<a[bl[v]].size())
					ret+=sum[bl[v]][t]+1ll*(a[bl[v]].size()-t)*(x+1);
			}
			printf("%.10f\n",vis[pii(bl[u],bl[v])]=1.0*ret/(1ll*a[bl[u]].size()*a[bl[v]].size()));
		}
	}
}
int main()
