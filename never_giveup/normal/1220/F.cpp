...
const int maxn = 4e5 + 100, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 600, X = 2000029, mod = 1e9 + 993, LG = 19;

int a[maxn], n, leg[maxn];

pair<int, int> sp[LG][maxn];

int get(int l, int r) {
    int w = leg[r - l + 1];
    return min(sp[w][l], sp[w][r - (1 << w) + 1]).sc;
}
...
int main() {
    ...
    function<int(int, int)> solve = [&](int l, int r) {
        if (l > r)
            return 0;
        int pos = get(l, r);
        return 1 + max(solve(l, pos - 1), solve(pos + 1, r));
    };
    auto f = [&](int pos) {
        return solve(st + 1, st + pos);
    };
    auto g = [&](int pos) {
        return solve(st + pos + 1, st + n - 1);
    };
    int l = 0, r = n - 1;
    while (r - l > 1) {
        int m = (l + r) / 2;
    ...
    int A = max(f(l), g(l)), B = max(f(r), g(r));
    cout << min(A, B) + 1 << ' ';
    int id = l;
    if (A > B)
        id++;
