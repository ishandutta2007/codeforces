...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            {
                ll nx = cur - t;
                while(!st.empty() && nx < st.back().F)
                {
                    ans += cur-st.back().F + st.back().S - 1;
                    cur = st.back().F;
                    st.DB();
                }
                if(!st.empty() && nx == st.back().F)
                {
                    ans += cur-st.back().F + st.back().S - 1;
                    cur = st.back().F;
                    st.back().S++;
                ...
                else if(!st.empty())
                {
                    ans += cur-nx;
                    cur = nx;
                    st.push_back(MP(nx, 2LL));
                }
                else
                {
                    cur = nx;
                    st.push_back(MP(nx, 1LL));
                }
                // cout<<i<<' '<<ans<<' '<<cur<<'\n';
                // for(auto it:st) cout<<it.F<<' '<<it.S<<'\t';cout<<'\n';
            }
        }
