...
struct LSegTree {
    ...
    template <typename Pred> ui binsearchLeft(Pred pred) {
        ValueType value = nullValue;
        return binsearchLeftInner(1, 0, n, value, pred);
    }

    template <typename Pred> ui binsearchLeftInner(ui i, ui l, ui r, ValueType &curValue, Pred pred) {
        if (l + 1 != r) pushTagDown(i, r-l);
        ValueType nextValue = combineValues(T[i].x, curValue);
        if (pred(nextValue)) { curValue = nextValue; return l; }
        if (l + 1 == r) return r;
        ui s = binsearchLeftInner(i<<1|1, (l+r)/2, r, curValue, pred);
        if (s == (l+r)/2) {
            return binsearchLeftInner(i<<1, l, (l+r)/2, curValue, pred);
        } else {
            return s;
        }
    }
...
public:
    void solve(istream& cin, ostream& cout) {
        ...
        for (int p = 0; p < P; ++p) {
            for (pii c: C[p]) {
                int ql = ST.binsearchLeft([c](int v) { return v < c.y; });
                if (ql <= c.x) {
                    ST.put(ql, c.x, c.y);
                    SS.put(ql, c.x, c.y);
                }
