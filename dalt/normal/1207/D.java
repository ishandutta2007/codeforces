...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            Map<Integer, Integer> firstKeyCnt = new HashMap<>(n);
            Map<Integer, Integer> secondKeyCnt = new HashMap<>(n);
            Map<Pair, Integer> doubleKeyCnt = new HashMap<>(n);

            for (Pair p : s) {
                firstKeyCnt.put(p.a, firstKeyCnt.getOrDefault(p.a, 0) + 1);
                secondKeyCnt.put(p.b, secondKeyCnt.getOrDefault(p.b, 0) + 1);
                doubleKeyCnt.put(p, doubleKeyCnt.getOrDefault(p, 0) + 1);
            }

            int p1, p2, p3;
            p1 = p2 = p3 = 1;
            for (Integer val : firstKeyCnt.values()) {
                p1 = mod.mul(p1, factorial.fact[val]);
            }
            for (Integer val : secondKeyCnt.values()) {
            ...
            if (order) {
                for (Integer val : doubleKeyCnt.values()) {
