        ...
        REP(i,0,n)
        {
            ...
            REP(j,0,410)
            {
                REP(m,0,205)
                {
                    if(a[i]==-1)
                    {
                        if(j>0) dp[j-1][m+1]+=(tp[j][m]*p[i])/100;
                        dp[j][m]+=(tp[j][m]*(100-p[i]))/100;
                    }
                    else
                    {
                        if(j+a[i]<409) dp[j+a[i]][m+1]+=(tp[j][m]*p[i])/100;
                        else dp[409][m+1]+=(tp[j][m]*p[i])/100;
                        dp[j][m]+=(tp[j][m]*(100-p[i]))/100;
                    }
                }
            }
