...
void solve() {
    ...
    int k = 0;
    for (int i = 1; i <= n; i++) k += b[i] > i;
    vector<vector<int>> aa(n+2);
    for (int i = 1; i <= n; i++) aa[b[i]].push_back(i);
    ...
    while (u != -1) {
        int new_u = -1;
        for (int x: aa[u]) if (!aa[x].empty()) new_u = x;
        for (int x: aa[u]) if (x != new_u) cout << x << " ";
        if (new_u != -1) cout << new_u << " ";
        u = new_u;
    }
    cout << "\n";
