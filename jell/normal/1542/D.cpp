#line 1 "codeforces-workspace\\contest\\1542\\d\\d.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\1542\\d\\d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1542\\d\\d.cpp"
...
#line 35 "codeforces-workspace\\contest\\1542\\d\\d.cpp"
...
void main() {
  ...
  for (auto i : range(x.begin(), x.end())) {
    if (!*i) {
      continue;
    }
    ...
    for (auto j : range(x.begin(), i)) {
      if (*j) {
        if (*j <= *i) {
          dp.emplace_back();
          for (auto k : rrange(1, dp.size())) {
            dp[k] += dp[k - 1];
          }
        } else {
          for (auto &&x : dp) {
        ...
        dp[0] += dp[0];
        for (auto k : range(1, dp.size())) {
          dp[k - 1] += dp[k];
        }
      }
    }

    for (auto j : range(next(i), x.end())) {
      if (*j) {
        if (*j < *i) {
          dp.emplace_back();
          for (auto k : rrange(1, dp.size())) {
    ...
    ans += *i * accumulate(begin(dp), end(dp), mint{});
  }
