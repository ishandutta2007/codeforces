...
int main(){
  ...
  auto dp=make_v(n+1,m+1,27,modint(0));
  rep(i,0,n)rep(j,0,m)dp[i][j][26]=1;
  modint ret=0;
  rep(i,0,n+1)rep(j,0,m+1)rep(o,0,27){
    if(i<n&&o!=s[i]-'a')dp[i+1][j][s[i]-'a']+=dp[i][j][o];
    if(j<m&&o!=t[j]-'a')dp[i][j+1][t[j]-'a']+=dp[i][j][o];
    if(o!=26)ret+=dp[i][j][o];
  }
  ll cf1=n,cf2=m;
  rep(_,0,2){
    modint add=0;
