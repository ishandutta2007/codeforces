...
#define NOW mem[pos][sdelta+MAXN]
ll Dfs( int pos , int sdelta ){
	if( pos == n+1 ){
		return sdelta > 0;
	}
	if(~NOW) return NOW;
	int cur_ans = a[pos], nxt_ans = a[ pos == n ? 1 : pos+1 ];
	ll ans = 0;
	if( cur_ans == nxt_ans ){
		ans = 1LL*k%MOD*Dfs(pos+1,sdelta) % MOD;
	}else{
		Addi( ans , Dfs(pos+1,sdelta-1) );
		Addi( ans , Dfs(pos+1,sdelta+1) );
		Addi( ans , (k-2)%MOD*Dfs(pos+1,sdelta)%MOD );
	}
	return NOW = ans;
}
