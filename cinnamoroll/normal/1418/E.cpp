...
void solve(){
  ...
  vec b(N+1,0);
  RREP(i,N) b[i] = b[i+1] + a[N-1-i];

  mint sum = accumulate(a.begin(),a.end(),0LL);

  REP(_,Q){
    int x,y;
    cin >> x >> y;

    int id = a.end() - lower_bound(a.begin(),a.end(),y);
    if(id < x){
      cout << 0 << endl;
      continue;
    }
    ...
    mint ans = 0;
    ans += (mint)(sum-b[id])*(id-x)/id;

    ans += (mint)b[id]*(id+1-x)/(id+1);
