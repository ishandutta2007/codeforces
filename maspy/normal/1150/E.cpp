...
// ai - 2aj + ak  max
struct Mono {
  using value_type = tuple<ll, ll, ll, ll, ll, ll>;
  using X = value_type;
  static X op(X x, X y) {
    auto [xi, xj, xk, xij, xjk, xijk] = x;
    auto [yi, yj, yk, yij, yjk, yijk] = y;
    return {max(xi, yi),
            max(xj, yj),
            max(xk, yk),
            max({xij, xi + yj, yij}),
            max({xjk, xj + yk, yjk}),
            max({xijk, xij + yk, xi + yjk, yijk})};
  }
...
struct Lazy {
  ...
  static constexpr X act(const X &x, const A &a) {
    auto [xi, xj, xk, xij, xjk, xijk] = x;
    return {xi + a, xj - 2 * a, xk + a, xij - a, xjk - a, xijk};
  }
};
...
void solve() {
  ...
  N = len(S);
  vi A(N + 1);
  FOR(i, N) A[i + 1] = (S[i] == '(' ? 1 : -1) + A[i];
  vc<Mono::X> seg_raw(N + 1);
  FOR(i, N + 1) seg_raw[i] = {A[i], -2 * A[i], +A[i], -A[i], -A[i], 0};
  LazySegTree<Lazy> seg(seg_raw);

  auto ans = [&]() -> void { print(get<5>(seg.prod_all())); };

  ans();
  FOR_(Q) {
    LL(a, b);
    --a, --b;
    seg.apply(a + 1, N + 1, (S[a] == '(' ? -1 : +1));
    seg.apply(b + 1, N + 1, (S[b] == '(' ? -1 : +1));
    swap(S[a], S[b]);
    seg.apply(a + 1, N + 1, (S[a] == '(' ? +1 : -1));
    seg.apply(b + 1, N + 1, (S[b] == '(' ? +1 : -1));
    ans();
