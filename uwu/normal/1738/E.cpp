...
// Documentation: https://atcoder.github.io/ac-library/production/document_en/modint.html
...
int main(){
    ...
    while (T--){
        ...
        while (pl <= pr){
            if (a[pl] == 0) pl++, l0++;
            else if (a[pr] == 0) pr--, r0++;
            else if (a[pl] == a[pr]){
                zero.push_back({l0, r0});
                l0 = r0 = 0;
                if (pl == pr) same = 1;
                pl++, pr--;
            }
        ...
        if (!same) ans = mint(2).pow(l0+r0+1);
        for (int i = zero.size()-1; i >= 1; --i){
            auto [l, r] = zero[i];
            ans *= (cb.choose(l+r+2, l+1));
        }
        mint mult = 0;
        auto [l, r] = zero[0];
        for (int k = 0; k <= min(l,r); ++k){
            mult += cb.choose(l, l-k) * cb.choose(r, r-k);
        }
        ans *= mult;
        cout << ans.val() << '\n';
    }
