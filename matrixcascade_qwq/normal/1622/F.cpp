...
void sieve(int n)
{
	...
	up(i,2,n)
	{
		if(!b[i])
		{
			int val=rand(1ull, ULLONG_MAX);
			f[i]=val;
			for(int j=i+i;j<=n;j+=i)
			{
				int q=j;
				while(q%i==0)q/=i,f[j]^=val;
				b[j]=1;
			}
		}
	}
...
//		upn opls(f[i]);edl;
		...
		upn
		{
			if(mp[f[i]^w]!=0)
			{
				f1=i;f2=mp[f[i]^w];break;
			}
		}
		if(f1)
		{
			cout<<n-2;edl;
			upn if(i!=f1&&i!=f2)opls(i);return 0;
		}
		cout<<n-3;edl;
		upn if(i!=2&&i!=n&&i!=n/2)opls(i);return 0;
	}
}
/*
