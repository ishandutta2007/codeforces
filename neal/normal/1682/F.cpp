...
int main() {
    ...
    vector<int> prefix_sum_index(N);
    iota(prefix_sum_index.begin(), prefix_sum_index.end(), 0);
    stable_sort(prefix_sum_index.begin(), prefix_sum_index.end(), [&](int a, int b) -> bool {
        return prefix_sum[a] < prefix_sum[b];
    });
    ...
    vector<mod_int> answers(Q, 0);
    fenwick_tree<mod_int> tree(N), a_tree(N);

    for (int r = 0; r < N; r++) {
        int index = prefix_sum_index[r];

        for (pair<int, int> &qry : queries[index]) {
            int L = index, R = qry.first;
            answers[qry.second] += mod_int(prefix_sum[L]) * a_tree.query(L + 1, R);
            answers[qry.second] -= tree.query(L + 1, R);
        }

        if (index > 0) {
            tree.update(index, mod_int(prefix_sum[index]) * (A[index] - A[index - 1]));
            a_tree.update(index, A[index] - A[index - 1]);
        }
    }
