...
public class Main {
    public static void main(String[] args) {
        ...
        DRescueNibel solver = new DRescueNibel();
        solver.solve(1, in, out);
        out.close();
    }

    static class DRescueNibel {
        NumberTheory.Mod998 mod = new NumberTheory.Mod998();

        public void solve(int testNumber, InputReader in, PrintWriter out) {
            int n = in.nextInt(), k = in.nextInt();
            Pii[] pairs = new Pii[2 * n];
            for (int i = 0; i < n; i++) {
                pairs[i] = Pii.of(in.nextInt(), +1);
                pairs[i + n] = Pii.of(in.nextInt(), -1);
            }
            Arrays.sort(pairs, Comparator.comparingInt((Pii i) -> i.first).thenComparingInt(i -> -i.second));
            ...
            for (Pii p : pairs) {
                score += p.second;
                if (p.second > 0) {
                    answer = mod.add(answer, mod.ncr(score - 1, k - 1));
                }
            }
