...
pii f(pii x, pii y) { return max(x, y); }
pii g(int x, pii y) { return pii(y.fi + x, y.se); }
...
int main() {
    ...
    const int N = 300001;
    vpi setter;
    rep(i, N) setter.eb(0, i);
    atcoder::lazy_segtree<pii, f, pe, int, g, h, e> seg(setter);
    int ma = 0, L, R;
    pqg<pii> pq;
    for(auto [l, d, r] : v) {
        while(!empty(pq) and pq.top().fi < l) {
            auto [x, y] = pq.top();
            pq.pop();
            seg.apply(x, y + 1, -1);
        }
        ...
        if(chmax(ma, res.fi)) L = l, R = res.se;
        pq.emplace(d, r);
    }
    cout << ma << endl;
    rep(i, n) {
        auto [l, v, r] = w[i];
        if(l <= L and R <= r and L <= v and v <= R) cout << i + 1 << " ";
    }
    cout << endl;
}