...
int main() {
    ...
    for(int t = 1; t <= T; t++){
        ...
        sort(all(sort_by_val), [&](int l, int r){
            return a[l] > a[r];
        });
        ...
        for(int i = 0; i < n; i++){
            ...
            lseg.upd(change_pos, change_pos, MOD+getDist(change_pos, n)); // at 0

            int res = lseg.query(1, cur_one-1);

            // for(int i = 1; i <= cur_one-1; i++){
            //     dbg(i, lseg.query(i, i));
            // }
