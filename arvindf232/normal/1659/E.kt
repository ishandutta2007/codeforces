...
fun main(){
    solve.cases{
        ...
        val evenplaces = BooleanArray(n)
        repeat(m){
            val a = getint - 1
            val b = getint -1
            G.addWeighted(a,b,getint)
        }
        for(e in 0 until m){
            if(G.weights[e] and 1 == 0){
                val a = G.from[e shl 1]
                val b = G.to[e shl 1]
                evenplaces[a] = true
                evenplaces[b] = true
            }
        }
        val alldsu = Array(30){DisjointUnionSets(n)}
        val surviveEven = Array(30){BooleanArray(n)}
        ...
        for(l in 1 until 30){
            for(i in 0 until n){
                if(!evenplaces[i]) continue
                surviveEven[l][alldsu[l].find(i)] = true
            }
        }
        ...
        repeat(q){
            ...
            for(l in 1 until 30){
                if(surviveEven[l][alldsu[l].find(a)]){
                    put(1)
                    return@repeat
                }
...
weight 0 if we can get from u to v without concatenning
through everything

when is it 1

when possible to walk without going through 1
