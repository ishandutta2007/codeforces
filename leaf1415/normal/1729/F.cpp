...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, 9) vec[i].clear();
		rep(i, 1, n-w+1){
			ll f = (sum[i+w-1] - sum[i-1]) % 9;
			vec[f].pb(i);
		}
		...
		rep(q, 1, Q){
			cin >> l >> r >> k;
			ll v = (sum[r]-sum[l-1]) % 9;
			P ans = P(inf, inf);
			rep(i, 0, 9){
				rep(j, 0, 9){
					if((i*v+j) % 9 != k) continue;
					if(sz(vec[i]) == 0 || sz(vec[j]) == 0) continue;
					if(i != j) chmin(ans, P(vec[i][0], vec[j][0]));
					else{
						if(sz(vec[i]) < 2) continue;
						chmin(ans, P(vec[i][0], vec[i][1]));
					}
				}
			}
			if(ans.fi > inf/2) ans = P(-1, -1);
			cout << ans.fi << " " << ans.se << "\n";
		}
	}
