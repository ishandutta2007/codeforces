...
#define int ll
vector<pair<int, int>> ans;
void seg(int now_l, int now_r) {
	if (now_l + 1 == now_r) return;
	seg(now_l, (now_l + now_r) / 2);
	seg((now_l + now_r) / 2, now_r);
	int steps = (now_r - now_l) / 2;
	REP(i, steps) {
		ans.push_back(mp(now_l + i, now_l + steps + i));
	}
}
void solve() {
	int n;
