// Balance Addicts (https://codeforces.com/contest/1738/problem/E)
...
int main() {
    ...
    while (t--) {
        ...
        auto calc = [&](int x, int y) {
            ...
            MI ans = 0;
            rep(i, mn + 1)
                ans += binom.binom(x, i) * binom.binom(y, i);
            return ans;
        };

        MI ans = calc(z[0], z.back());
        ll lsum = b[0], rsum = b.back();
        for (int l = 0, r = m - 1; l < r;) {
            if (lsum == rsum) {
                if (l + 1 < r)
                    ans *= calc(z[l + 1] + 1, z[r] + 1);
                else
                    ans *= MI(2).pow(z[l + 1] + 1);
            }
            bool incr_l = lsum <= rsum, incr_r = rsum <= lsum;
            if (incr_l)
                lsum += b[++l];
            if (incr_r)
                rsum += b[--r];
