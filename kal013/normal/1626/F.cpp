...
void solve(){
    ...
    long long M;
    {
    	...
    	for(int j = k - 1; j >= 1; --j){
    		for(int i = M - 1; i >= 0; --i){
    			answers[i] = exclude * answers[i] + inv_n * (i + answers[i - (i % j)]);
    		}
    	}
    }
    // cout << answers[n] << endl;
    ...
    for(int i = 1; i <= n; ++i){
    	...
    	// 		}
    	// 	}
    	// 	mint pr = (exclude ^ (k - ctr)) * (inv_n ^ ctr);
    	// 	ps.push_back({ca, pr});
    	// }
    	// {
    	// 	sort(all(ps));
    	...
    	// 	ps.clear();
    	// 	for(auto X: cp){
    	// 		if (!ps.empty() && ps.back().first == X.first){
    	// 			ps.back().second += X.second;
    	// 		}
    	// 		else{
    	// 			ps.push_back(X);
    	// 		}
    	...
    	// vector<pair<int, mint>> prob = {{a, 1}};
    	// for(int j = 1; j <= k; ++j){
    	// 	vector<pair<int, mint>> tmp;
    	// 	for(auto X: prob){
    	// 		tmp.push_back({X.first, X.second * exclude});
    	// 		int diff = X.first % j;
    	// 		tmp.push_back({X.first - diff, X.second * inv_n});
    	// 		e += X.first * X.second * inv_n;
    	// 	}
    	// 	// e += prob * z * (1 + (k - j) * inv_n);
    	// 	// prob *= exclude;
    	// 	sort(all(tmp));
    	// 	prob.clear();
    	// 	for(auto X: tmp){
    	// 		if (!prob.empty() && prob.back().first == X.first){
    	// 			prob.back().second += X.second;
    	// 		}
    	// 		else{
    	// 			prob.push_back(X);
    	// 		}
    	// 	}
    	// }
    	e += (answers[a % M] + M * (a / M) * inv_n * k);
    	a = (a * 1LL * x + y) % m;
    }
    cout << e * (mint(n)^k) << endl;
}
int main(){
