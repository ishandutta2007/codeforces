...
public:
	...
	static void generateTest(std::ostream& test) {
		...
		set<pair<int, int>> s;
		while (s.size() < m) {
			int a = randomInteger(n);
			int b = randomInteger(n);
			if (a != b) {
		...
		for (auto p: s) {
			test << p.first + 1 << ' ' << p.second + 1 << "\n";
		}

		int q = 100000;
		test << q << "\n";
		for (int i: range(q)) {
			test << randomInteger(n) + 1 << "\n";
		}
	}
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		int64_t currentAns = 0;
		for (int i: range(n)) {
			currentAns += (deg[i] - bigger[i].size()) * 1LL * bigger[i].size();
			//cerr << i << ' ' << (n-bigger[i].size()) * 1LL * bigger[i].size() << endl;
		}
		...
		for (int _: range(q)) {
			...
			in >> v;
			--v;
			currentAns -= (deg[v] - bigger[v].size()) * 1LL * bigger[v].size();
			for (int o: bigger[v]) {
				currentAns -= (deg[o] - bigger[o].size()) * 1LL * bigger[o].size();
				//smaller[o].erase(v);
				bigger[o].push_back(v);
				currentAns += (deg[o] - bigger[o].size()) * 1LL * bigger[o].size();
