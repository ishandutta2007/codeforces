...
class TaskA {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		int[] first = new int[pageCount];
		int[] next = new int[count];
		Arrays.fill(first, -1);
		for (int i = 0; i < count; i++) {
			next[i] = first[order[i]];
			first[order[i]] = i;
		}
		int[] values = new int[2 * count];
		long base = 0;
		for (int i = 1; i < count; i++)
			base += Math.abs(order[i] - order[i - 1]);
		long answer = base;
		for (int i = 0; i < pageCount; i++) {
			...
			for (int j = first[i]; j != -1; j = next[j]) {
				if (j != 0 && order[j - 1] != i) {
					values[size++] = order[j - 1];
					current -= Math.abs(i - order[j - 1]);
				}
				if (j != count - 1 && order[j + 1] != i) {
					values[size++] = order[j + 1];
					current -= Math.abs(i - order[j + 1]);
				}
			}
			...
			ArrayUtils.sort(values, 0, size, IntComparator.DEFAULT);
			int at = values[size >> 1];
			for (int j = 0; j < size; j++)
				current += Math.abs(at - values[j]);
