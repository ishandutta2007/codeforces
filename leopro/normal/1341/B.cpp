...
void solve() {
    ...
    cin >> a;
    vector<bool> peaks(n);
    for (int i = 1; i + 1 < n; ++i) peaks[i] = a[i] > a[i - 1] && a[i] > a[i + 1];
    int cnt = count(peaks.begin() + 1, peaks.begin() + k - 1, true);
    int best = cnt;
    for (int i = 0; i < n - k; ++i) {
        cnt += peaks[i + k - 1];
        cnt -= peaks[i + 1];
    ...
    cout << best + 1 << ' ';
    cnt = count(peaks.begin() + 1, peaks.begin() + k - 1, true);
    for (int i = 0; i < n - k; ++i) {
        if (best == cnt) answer(i + 1);
        cnt += peaks[i + k - 1];
        cnt -= peaks[i + 1];
    }
    if (best == cnt) answer(n - k + 1);
}