// Lost Arithmetic Progression (https://codeforces.com/contest/1673/problem/D)
...
int main() {
    int t; read(t);
    while (t--) {
        ll b, q, y, c, r, z; read(b, q, y, c, r, z);
        auto in_b = [&](ll n) {
            return b <= n && n <= b + q * (y - 1) && abs(n - b) % q == 0;
        };
        if (!in_b(c) || !in_b(c + r * (z - 1)) || r % q) {
            println(0);
            continue;
        }

        if (!in_b(c - r) || !in_b(c + r * z)) {
            println(-1);
            continue;
        }
