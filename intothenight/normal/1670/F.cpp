...
int main(){
	...
	const int mx = 60;
	#endif
	vector<vector<modular>> C(n + 1, vector<modular>(n + 1));
	for(auto x = 0; x <= n; ++ x){
		for(auto y = 0; y <= x; ++ y){
			C[x][y] = y == 0 || y == x ? 1 : C[x - 1][y - 1] + C[x - 1][y];
		}
	}
	auto solve = [&](long long high, long long obj)->modular{
		vector<modular> eql(n + 1), gr(n + 1);
		eql[0] = 1;
		for(auto bit = 0; bit < mx; ++ bit){
			static vector<modular> eql_next(n + 1), gr_next(n + 1);
			ranges::fill(eql_next, 0);
			ranges::fill(gr_next, 0);
			int c = obj >> bit & 1, hc = high >> bit & 1;
			for(auto x = 0; x <= n; ++ x){
				int p = x + c & 1;
				for(auto y = c; y <= n; y += 2){
					if(hc < p){
						gr_next[x + y >> 1] += eql[x] * C[n][y];
					}
					else{
						eql_next[x + y >> 1] += eql[x] * C[n][y];
					}
					if(hc <= p){
						gr_next[x + y >> 1] += gr[x] * C[n][y];
					}
					else{
						eql_next[x + y >> 1] += gr[x] * C[n][y];
					}
				}
			}
			swap(eql, eql_next);
	...
	cout << solve(high, obj) - solve(low - 1, obj) << "\n";
	return 0;
}
