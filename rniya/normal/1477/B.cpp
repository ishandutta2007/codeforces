...
void solve() {
    ...
    auto g = [](node a, int x) { return ~x ? node(x * a.cnt, a.cnt) : a; };
    auto h = [](int a, int b) { return b; };
    LazySegmentTree<node, int> seg(n, f, g, h, node(0, 0), -1);
    for (int i = 0; i < n; i++) seg.set_val(i, node(T[i] - '0', 1));

    for (int i = q - 1; i >= 0; i--) {
        int sum = seg.query(l[i], r[i]).sum, len = r[i] - l[i];
        if (sum * 2 == len) {
            cout << "NO" << '\n';
            return;
        }
