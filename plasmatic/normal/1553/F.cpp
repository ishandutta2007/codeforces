...
int main() {
    ...
    for (auto i = 1; i <= N; i++) {
        ...
        // where A[i] is the mod
        // cout<<"i="<<(i)<<", "; cout<<"A[i]="<<(A[i])<<", "; cout<<"csum="<<(csum)<<", "; cout << endl; // db i,A[i],csum
        cur += csum;
        int tot = bitcnt.query(MV);
        for (int j = A[i]; j < MV; j += A[i]) {
            cur -= (tot-bitcnt.query(j-1)) * A[i];
        }
        // cout<<"cur="<<(cur)<<", "; cout << endl; // db cur

        // where A[i] is the value
        cur += ll(A[i]) * i;
        cur -= bit.query(A[i]);
