...
void solve_case(int tc = 0) {
	...
	auto pop = [&](multiset<int, greater<int>>& m) {
		int x = *m.begin();
		m.erase(m.begin());
		return x;
	};
	...
	while (!two.empty() || !one.empty()) {
		while (!one.empty() && *one.begin() <= days) {
			pop(one);
		}
		while (!two.empty() && *two.begin() <= 2 * days) {
			pop(two);
		}
		if (!two.empty() && !one.empty()) {
			int x = *one.begin() - days;
			int y = *two.begin() - days * 2;
			if (y > x) {
				pop(two);
				++ans;
				if (y > 1) {
