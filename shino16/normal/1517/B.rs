...
fn main() {
    ...
    scan!(new cin, t: u32);
    for _ in 0..t {
        scan!(from cin, n: usize, m: usize, mut b: [[u32; m]; n]);
        for i in 0..n {
            b[i].sort_unstable();
        }
        let mut idx: Vec<_> = (0..n).flat_map(|i| (0..m).map(move |j| (i, j))).collect();
        idx.sort_unstable_by_key(|&(i, j)| b[i][j]);
        let mut cnt = vec![0; n];
        for &(i, _) in &idx[..m] {
            cnt[i] += 1;
        }
        let mut now: Vec<_> = (0..m).collect();
        for i in 0..n {
            let mut res = vec![0; m];
            for j in 0..m {
                res[now[j]] = b[i][j];
