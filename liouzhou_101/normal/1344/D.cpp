...
bool check(ll bound)
{
	...
	for (int i = 1; i <= n; ++i)
	{
		...
		{
			b[i] = sqrt((a[i] - bound) / 3.0 - 1.0 / 12.0) - 0.5;
			freshmin(b[i], a[i]);
			while (b[i] > 0 && a[i] - (3 * b[i] * b[i] + 3 * b[i] + 1) < bound) --b[i];
			while (b[i] < a[i] && a[i] - (3 * (b[i] + 1) * (b[i] + 1) + 3 * (b[i] + 1) + 1) >= bound) ++b[i];
		}
		sum += b[i];
	}
	return sum <= k;
...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		while (sum < k && b[i] < a[i] && a[i] - (3 * (b[i] + 1) * (b[i] + 1) + 3 * (b[i] + 1) + 1) == res - 1)
		{
			b[i] ++;
			sum++;
		}
