...
int main() {
	...
	while (CASES--) {
		...
		std::array<int, 26> idx; idx.fill(N);
		for (int i = N-1; i >= 0; i--) {
			nxt[i] = std::exchange(idx[S[i] - 'a'], i);
		}

		binary_indexed_tree<int> num_used(N);
		auto get_cost = [&](int i) {
			int res = i;
			for (auto v : num_used.prefix(res)) res -= v;
			return res;
		};
		...
		for (int i = 0; i < N; i++) {
			...
			for (int c = 0; c < (T[i] - 'a'); c++) {
				min_smaller = std::min(min_smaller, idx[c]);
			}
			if (min_smaller < N) {
				ans = std::min(ans, cur + get_cost(min_smaller));
			}
			{
				int c = (T[i] - 'a');
				if (idx[c] == N) break;
				cur += get_cost(idx[c]);
				for (auto& v : num_used.suffix(idx[c])) v++;
				idx[c] = nxt[idx[c]];
			}
		}
