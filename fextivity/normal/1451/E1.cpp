...
int askAND(int x, int y){
    cout << "AND " << x << ' ' << y << endl;
    int cac; cin >> cac; if (cac == -1) exit(1);
    return cac;
}
...
signed main(){
    ...
    int x = occ[n - 1], y = occ[1];
    int sum1x = n - 1, sum1y = askAND(1, y) * 2 + ansxor[y], sumxy = askAND(x, y) * 2 + ansxor[x] ^ ansxor[y];
    a[1] = (sum1x + sum1y - sumxy) / 2;
    ForE(i, 2, n){
        a[i] = a[1] ^ ansxor[i];
    }
    answer();
