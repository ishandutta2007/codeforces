...
int main() {
	...
	int pos = 0;
	for(auto &[a, b, c] : portals) cin >> a >> b >> c;
	
	for(int i = 0; i < n; i++) {
		if(i) pref[i] = pref[i-1];
		auto [x, y, on ] = portals[i];
		int pos = upper_bound(all(portals), array{y, -1, -1})-portals.begin();
		f[i] = mint(portals[pos][0]-portals[i][1]);
		//cout << f[i] << " (1  _" << portals[pos][0] << " " << y <<endl;
		if(i) f[i] += pref[i]-(pos?pref[pos-1]:0);;
		if(i) f[i] += portals[i][0]-portals[pos][0];
		//cout << f[i] << " (2"<<endl;
			dp[i] = (i?dp[i-1]:mint(0)) + mint(portals[i][0]-(i?portals[i-1][0]:0));
		if(on)
			dp[i] += f[i];
		
		//cout << i << " _ " << f[i] << " " << dp[i] << " ) " << pos << endl;
		pref[i] += f[i];
	}
	cout << dp[n-1]+1 << endl;
}