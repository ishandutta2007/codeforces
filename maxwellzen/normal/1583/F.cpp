...
void solve() {
    ...
    while (cmp.size()>1) {
        c++;
        int m = (cmp.size()+k-1)/k;
        vvi ncmp(m);
        F0R(i, cmp.size()) ncmp[i/k].insert(ncmp[i/k].end(), all(cmp[i]));
        F0R(a, m) for (int i : ncmp[a]) for (int j : ncmp[a]) if (i!=j && ans[i][j]==-1) ans[i][j]=c;
        cmp = ncmp;
    }
    cout << c << nl;
    F0R(i, n) FOR(j, i+1, n) cout << ans[i][j] << ' ';
    cout << nl;
}
