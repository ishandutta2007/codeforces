...
bool primetest(ll n) {
  const int THRESH = 1 << 30;
  if (n < 2 || n % 6 % 4 != 1) return (n | 1) == 3;

  auto mul = [&](ll a, ll b) -> ll {
    if (n < THRESH) return a * b % n;
    return ll(ll8(a) * ll8(b) % n);
  };

  auto pow = [&](ll a, ll k) -> ll {
    ll p = a;
    ll ret = 1;
    while (k) {
  ...
  FORIN(a, test) {
    ll p = pow(a % n, d), i = s;
    while (p != 1 && p != n - 1 && a % n && i--) p = mul(p, p);
...
void solve() {
  ...
  ++N;
  ll pr = N;
  while (pr == 2 || !primetest(pr)) ++pr;
  ll pl = pr - 1;
  while (!primetest(pl)) --pl;

  ll k = pr - N;
  // 1/2 - 1/pr - k/pl*pr
  ll num = pl * pr - 2 * pl - 2 * k;
  ll den = 2 * pl * pr;
  ll g = gcd(num, den);
  num /= g, den /= g;
  cout << num << "/" << den << endl;
