...
mi get_contrib(int d, int n) {
	if (d > n) return 0;
	if (d == n) return 1;
	return pow(mi(n+1),n-d);
}
...
int main() {
	...
	vmi b_posi(K+1);
	FOR(i,1,K+1) b_posi[i] = pow(mi(i),K)*ifac[i];
	vmi dif(K+1);
	F0R(i,K+1) dif[i] = sgn(i)*ifac[i];
	b_posi = mul(b_posi,dif);
	FOR(i,1,K+1) b_posi[i] *= fac[i];

	// 	FOR(j,1,i) b_posi[i] += sgn(i-j)*comb(i,j)*pow(mi(j),K);
	// }
	...
	mi sum_b_posi = 0;
	each(t,b_posi) sum_b_posi += t;
	mi ans = 0;
	FOR(i,1,K+1) {
		ans += b_posi[i]*get_contrib(i,N);
	}
	ps(ans);
	// dbg(sum_b_posi,pow(mi(N),K));
