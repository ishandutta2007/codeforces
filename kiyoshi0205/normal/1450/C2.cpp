...
int main(){
  ...
  A=min_element(all(a))-a.begin();
  B=min_element(all(b))-b.begin();
  if(A==B){
    a[A]+=inf;b[B]+=inf;
    C=min_element(all(a))-a.begin();
    D=min_element(all(b))-b.begin();
    a[A]-=inf;b[B]-=inf;
    if(a[A]+b[D]<=a[C]+b[B])B=D;
    else A=C;
  }
  debug(a,b);
  debug(A,B);
  rep(i,N)rep(j,N){
    if((i+j)%3==A&&g[i][j]=='X')g[i][j]='O';
    if((i+j)%3==B&&g[i][j]=='O')g[i][j]='X';
  }
  rep(i,N)print(g[i]);
}
}