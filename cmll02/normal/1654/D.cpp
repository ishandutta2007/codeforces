...
#define newe(n) struct Edge{int v,wx,wy,nxt;}e[2*n+5];\
typedef int arr[n+5];\
arr h;\
int cnt=1;\
inline void addedge(int u,int v,int wx,int wy){e[cnt]=(Edge){v,wx,wy,h[u]};h[u]=cnt++;}
...
arr hson,sz;
std::map<int,int>fin;
std::vector<int>av[200005];
int a[200005];
void dfs(int u,int fa)
{
	fe(u)
	if(v==fa)continue;
	int x=e[i].wx,y=e[i].wy;
	for(auto d:av[x])mp[d]--;
	for(auto d:av[y])mp[d]++;
	for(auto d:av[x])mne(fin[d],mp[d]);
	for(auto d:av[y])mne(fin[d],mp[d]);
	dfs(v,u);
	for(auto d:av[y])mp[d]--;
	for(auto d:av[x])mp[d]++;
	gr
...
signed main()
{
	...
	for(int i=2;i<=200000;i++)
	{
		if(av[i].empty())
		{
			...
			{
				int s=j;
				while(s%i==0)s/=i,av[j].push_back(i);
			}
		}
	}
	rg(T)
	int n=read();
	rg(n)h[i]=0,mp.clear();gr
	fin.clear();
	cnt=2;
	rg(n-1)
	int u=read(),v=read(),x=read(),y=read();
	// int d=std::__gcd
	addedge(u,v,x,y);addedge(v,u,y,x);
	gr
	dfs(1,0);
	a[1]=1;
	rg(n)mle(a[1],qp(i,-fin[i]));gr
	dfs2(1,0);
	int ans=0;rg(n)mae(ans,a[i]);gr
	oldl(ans);
	gr
	return 0;
}