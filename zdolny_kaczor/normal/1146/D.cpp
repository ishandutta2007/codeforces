...
int main() {
	...
	for (int i = 0; i <= min(m, a + b); ++i) {
		bound = i;
		if (i == 0 || (i - a >= 0 && reached[i - a]))
			dfs(i);
		debug << imie(i) imie(reached_cou);
		ans += reached_cou;
	}
	if (m <= a + b) {
		printf("%lld\n", ans);
	...
	if (curr <= m) {
		int dest = m / g * g;
		debug << imie(dest);
		int blocks = (dest - curr) / g;
		debug << imie(blocks);
		debug << "sum2 " << blocks * (blocks - 1ll) / 2 * g;
		ans += blocks * (blocks - 1ll) / 2 * g + (dest - curr) * 1ll * (curr / g + 1);
		for (int i = dest; i <= m; ++i) {
			debug << "leftover " imie(i) imie(i / g + 1);
			ans += i / g + 1;
		}
	}
	printf("%lld\n", ans);
