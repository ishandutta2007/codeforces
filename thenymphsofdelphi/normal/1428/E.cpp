...
int cost(int n, int k){ // partition n things to k parts
    if (n <= k) return n;
    int val = n / k, y = n % k, x = k - y;
    return x * val * val + y * (val + 1) * (val + 1);
}
...
signed main(){
    ...
    ForE(i, 1, n){
        ans += a[i] * a[i];
        pq.push({cost(a[i], b[i]) - cost(a[i], b[i] + 1), i});
    }
    ForE(i, 1, k - n){
        pii t = pq.top(); pq.pop();
        ans -= t.fi;
        b[t.se]++;
        pq.push({cost(a[t.se], b[t.se]) - cost(a[t.se], b[t.se] + 1), t.se});
    }
    cout << ans;
}
