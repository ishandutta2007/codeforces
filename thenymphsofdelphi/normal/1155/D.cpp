...
signed main(){
    ...
    ForE(i, 1, n){
        maxSuff[i] = max(maxSuff[i - 1] + a[i], max(a[i], 0ll));
    }
    FordE(i, n, 1){
        maxPref[i] = max(maxPref[i + 1] + a[i], max(a[i], 0ll));
    }
    int ans = calMaxSubarray(), mxVal = 0;
    ForE(i, 1, n){
        ans = max(ans, maxPref[i + 1] + b[i] * k + mxVal);
        mxVal = max(mxVal, -b[i] * k + maxSuff[i]);
    }
    cout << ans;
}
