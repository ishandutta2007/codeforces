...
int main(void)
{
	...
	cin >> n;
	rep(i, 0, n-1) cin >> a[i];
	rep(i, 1, n-1) vec[gcd(i, n)].push_back(i);
	
	llint ans = 0;
	rep(g, 1, n-1){
		if(sz(vec[g]) == 0) continue;
		rep(i, 0, g-1) mx[i] = 0;
		rep(i, 0, n-1) chmax(mx[i%g], a[i]);
		ll cnt = 0;
		rep(i, 0, n-1) len[i] = 0;
		rep(i, 0, 2*n-1){
			if(mx[i%g] == a[i%n]) cnt++;
		...
		rep(i, 0, n) sum[i] = 0;
		rep(i, 0, n-1) sum[min(n, len[i])]++;
		per(i, n, 0) sum[i] += sum[i+1];
		for(auto x : vec[g]) ans += sum[x];
	}
	outl(ans);
