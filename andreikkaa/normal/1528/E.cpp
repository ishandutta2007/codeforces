...
int get_two(int x) {
    return dv(mul(x, x + 1), 2);
}

int get_three(int x) {
    return dv(mul(x, add(mul(x, x), mul(3, x) + 2)), 6);
}
...
void solve() {
    ...
    for(int i = 2; i <= n + 1; ++i) {
        ...
        // two
        addeq(cnt[1][i], cnt[2][i - 1]);

        cnt[2][i] = get_two(cnt[1][i]);
        cnt[3][i] = get_three(cnt[1][i]);

        cnt_ex[1][i] = sub(cnt[1][i], cnt[1][i - 1]);
        cnt_ex[2][i] = sub(cnt[2][i], cnt[2][i - 1]);
        cnt_ex[3][i] = sub(cnt[3][i], cnt[3][i - 1]);
    }

    int ans = 0;
    for(int i = 1; i + 1 < n; ++i) { // 0 1 2 3
        addeq(ans, mul(cnt_ex[2][i], sub(cnt_ex[1][n - i], 1)));
    }
    const int cur = add(add(cnt_ex[1][n], cnt_ex[2][n]), cnt_ex[3][n]);

    cout << sub(add(ans, mul(cur, 2)), 1) << endl;
}
