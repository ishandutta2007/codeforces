...
void solve() {
	...
	for(int i = 0; i < N - 1; ++i) {
		...
		int m = (N - n);
		int d = a[i + 1] - a[i];
		vector<int> p = {a[i], a[i + 1], a[i] + (b + d * m - z) / N, a[i] + (b + d * m - z) / N + 1, a[i] + (b + d * m - z) / N - 1};
		for(auto x: p) {
			if (x < a[i] || x > a[i + 1])
				continue;
			// cout << x  << ' ';
			int h2 = x - a[i];
			int h1 = a[i + 1] - x;
			// cout << z << ' ' << b << ' ' << x << ' ' << z + h2 * n << ' ' << b + h1 * m << ' ' << n << ' ' << m << '\n';
			ans = min(ans, A * (z + h2 * n) + R * (b + h1 * m));
			if (z + h2 * n < b + h1 * m) 
				ans = min(ans, M * (z + h2 * n) + R * (b + h1 * m - z - h2 * n));
			else 
				ans = min(ans, M * (b + h1 * m) + A * (z + h2 * n - b - h1 * m));
		}
		z += n * d;
		if (i + 2 != N) 
			b -= ((m - 1) * (a[i + 2] - a[i + 1]));
