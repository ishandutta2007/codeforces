...
int n = get<int>();
string s = get<string>();
vi w = getVector<int>(26);
vi dCnt(n+1, 0), dMax(n+1, 0), dSz(n+1, inf);
...
signed main() {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j <= i; j++) {
			if (good(j, i)) {
				dCnt[i+1] = addMod(dCnt[i+1], dCnt[j]);
				dMax[i+1] = max(dMax[i+1], max(dMax[j], i - j + 1));
				dSz[i+1] = min(dSz[i+1], dSz[j] + 1);
			}
		}
	}
	putLn(dCnt[n], endl, dMax[n], endl, dSz[n]);
	return 0;
}