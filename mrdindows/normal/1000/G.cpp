...
ll dpu[303030];
int h[303030];
void dfs(int cur, int p = 0, int h = 0) {
    ::h[cur] = h;
    tin[cur] = ++T;
    par[cur][0] = p;
    for (int i = 1; i < 20; ++i) {
    ...
    for (auto [to, w] : v[cur]) {
        if (to != p) {
            dfs(to, cur, h + 1);
            dp[cur] += max(0LL, dp[to] + a[to] - w * 2);
        }
    }
    tout[cur] = T;
//    DBN(cur, dp[cur]);
}
void dfs2(int cur, int p = 0, ll d = 0) {
    dpu[cur] = max(d, 0LL);
    for (auto [to, w] : v[cur]) {
        if (to != p) {
            dfs2(to, cur, dpu[cur] + dp[cur] + a[cur] - w * 2 - max(0LL, dp[to] + a[to] - w * 2));
        }
    }
//    DBN(cur, dpu[cur]);
}
...
ll parsum[303030][20];
void dfs3(int cur, int p = 0, int pw = 0) {
    parsum[cur][0] = dp[p] - max(0LL, dp[cur] + a[cur] - pw * 2) + a[cur] - pw;
    for (int i = 1; i < 20; ++i) {
        parsum[cur][i] = parsum[cur][i - 1] + parsum[par[cur][i - 1]][i - 1];
    }
    for (auto [to, w] : v[cur]) {
        if (to != p) {
            dfs3(to, cur, w);
