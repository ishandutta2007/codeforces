...
S op(S l,S r){
    return {max(l.act,r.act),max(l.inact,r.inact)};
}

S mapping(F l,S r){
    if (!l) return r;
    return {r.inact,r.act};
}
...
int n,sz1[maxn],sz2[maxn],dist1[maxn],dfn1[maxn],dist2[maxn],dfn2[maxn],cnt,col1[maxn],col2[maxn];
vector<pii> g[maxn];
pii tmp[maxn];
void dfs(int u,int p,int sz[],int dist[],int dfn[],int col[]){
    sz[u]=1;
    dfn[u]=cnt++;
    for (auto c:g[u]){
        if (c.F==p) continue;
...
signed main(){
    ...
    cin>>n;
    lazy_segtree<S,op,e,F,mapping,composition,id> seg1(n),seg2(n);
    rep(i,n-1){
        int u,v,w;
        cin>>u>>v>>w;
        u--, v--;
    ...
    dfs(0,-1,sz1,dist1,dfn1,col1);
    cnt=0;
    int l=-1,r=-1;
    rep(i,n) if (l==-1||dist1[l]<dist1[i]) l=i;
    dfs(l,-1,sz2,dist2,dfn2,col2);
    cnt=0;
    rep(i,n) if (r==-1||dist2[r]<dist2[i]) r=i;
    memset(dist1,0,sizeof(dist1)), memset(dist2,0,sizeof(dist2));
    memset(dfn1,0,sizeof(dfn1)), memset(dfn2,0,sizeof(dfn2));
    memset(col1,0,sizeof(col1)), memset(col2,0,sizeof(col2));
    memset(sz1,0,sizeof(sz1)), memset(sz2,0,sizeof(sz2));
    dfs(l,-1,sz1,dist1,dfn1,col1);
    cnt=0;
    dfs(r,-1,sz2,dist2,dfn2,col2);
    rep(i,n){
        if (col1[i]) seg1.set(dfn1[i],{0,dist1[i]});
        else seg1.set(dfn1[i],{dist1[i],0});
    }
    rep(i,n){
        if (col2[i]) seg2.set(dfn2[i],{0,dist2[i]});
        else seg2.set(dfn2[i],{dist2[i],0});
    }
    int q;
    cin>>q;
    while (q--){
       ...
       w--;
       int mod=-1;
       if (dist1[tmp[w].F]>dist1[tmp[w].S]) mod=tmp[w].F;
       else mod=tmp[w].S;
       seg1.apply(dfn1[mod],dfn1[mod]+sz1[mod],1);
       if (dist2[tmp[w].F]>dist2[tmp[w].S]) mod=tmp[w].F;
       else mod=tmp[w].S;
       seg2.apply(dfn2[mod],dfn2[mod]+sz2[mod],1);
       auto ret1=seg1.all_prod(),ret2=seg2.all_prod();
       cout<<max(ret1.act,ret2.act)<<" ";
    }
