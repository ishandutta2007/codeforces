...
signed main(){
    ...
    rep(n){
        ...
        VEC(ll,b,m);
        bool flag=0;
        rep(m-1)if(b[i]<b[i+1]){
            flag=1;
            break;
        }
        if(flag)continue;
        a.emplace_back(min(b),max(b));
    }
    ll ans=n*n-(ll)a.size()*(ll)a.size();
    vec(ll,mx,0);
    each(i,a)mx.push_back(i.second);
    sort(all(mx));
    each(i,a)ans+=mx.end()-upper_bound(all(mx),i.first);
    out(ans);
}