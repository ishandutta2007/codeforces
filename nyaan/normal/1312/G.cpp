...
struct Trie {
  ...
  void add(int n,char c){
    nodes[n].nxt[c - margin] = (int) nodes.size();
    nodes.push_back(Node());
  }
...
void solve(){
  ...
  ini(K);
  vi a(K); in(a);
  vi inv(N + 1 , -1); rep(i,K) inv[a[i]] = i;
  vi ans(K , -1);
  trc(a);
  // 

  auto dfs = [&](auto rec,int cur,int par,int depth , int sug) -> int{
    trc(cur,par,depth,sug);
    // 
    int ret = 0;
    // 
    //  or 
    if(inv[cur] != -1){
      ans[inv[cur]] = min(depth , sug);
      depth = ans[inv[cur]];
      ret++;
    }
    sug = min(depth + 1 , sug);
    rep(i , 26){
      if(trie.nodes[cur].nxt[i] != -1){
        ret += rec(rec , trie.nodes[cur].nxt[i] , cur , depth + 1 , sug + ret/**/);
      }
    }
    return ret;
  };
