...
signed main()
{
    ...
    for(int i=0;i<n;i++)
        t[s[i]-'0'].insert(i);
    if(t[0].empty()||t[1].empty()||*t[0].rbegin()-*t[0].begin()<=k-1||*t[1].rbegin()-*t[1].begin()<=k-1)
        cout<<"tokitsukaze",exit(0);
        ...
        for(int c=0;c<2;c++)
        {
            ...
            for(int i=0;i<2;i++)
            {
                if(*t[i].begin()<l)
                    ...
                    x[i]=*t[i].upper_bound(r);
                if(*t[i].rbegin()>r)
                    y[i]=*t[i].rbegin();
                else if(i==c)
                    y[i]=r;
                else
                    y[i]=*prev(t[i].lower_bound(l));
//                cout<<l<<" "<<r<<" "<<c<<" "<<i<<" -> "<<x[i]<<" "<<y[i]<<"\n";
            }
            if(y[0]-x[0]>=k&&y[1]-x[1]>=k)
                cout<<"once again",exit(0);
        }
    cout<<"quailty";
