...
int main() {
	int n,q,MOD; R(n,q,MOD);
	vector<ll> a(n),b(n); R(a,b);

	vector<ll> c(n);
	for (int i = 0; i < n; i++)
		c[i] = ((a[i] - b[i]) % MOD + MOD) % MOD;

	vector<ll> d(n);
	for (int i = 0; i < n; i++)
		d[i] = ((c[i] - (i - 1 >= 0 ? c[i - 1] : 0) - (i - 2 >= 0 ? c[i - 2] : 0)) % MOD + MOD) % MOD;
	...
	while (q--) {
		...
		int mul = (cmd == 'A' ? +1 : -1);
		add(l, fib[0], mul);
		add(r + 1, -fib[len] + (len >= 2 ? -fib[len - 1] : 0), mul);
		add(r + 2, -fib[len], mul);
