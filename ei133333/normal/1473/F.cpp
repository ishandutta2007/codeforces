...
int main() {
  ...
  vector< int > A(N), B(N);
  cin >> A >> B;
  PushRelabel< int > flow(N + 2);
  vector< int > pre_idx(101, -1);
  int S = N;
  int T = N + 1;
  int sum = 0;
  vector< int > used(101, -1);
  for(int i = 0; i < N; i++) {
    if(B[i] >= 0) {
      sum += B[i];
    ...
    for(int j = 100; j > 0; j--) {
      if(A[i] % j == 0 and ~pre_idx[j] and used[j] != i) {
        used[j] = i;
        flow.add_edge(pre_idx[j], i, inf);
      }
    }
    if(~pre_idx[A[i]]) {
      flow.add_edge(pre_idx[A[i]], i, inf);
    }
    pre_idx[A[i]] = i;
  }
