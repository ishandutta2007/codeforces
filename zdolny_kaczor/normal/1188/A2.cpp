...
vpii graf[nax];
map <pii, int> ops;
void do_op(int a, int b, int v) {
	debug(imie(a), imie(b), imie(v));
	if (a > b) swap(a, b);
	ops[pii(a, b)] += v;
}
int p[nax];
int wei_up[nax];
int in[nax], out[nax];
vector <int> leaves[nax];
void dfs(int x, int y = 0) {
	static int timer = 0;
	in[x] = timer++;
	if (graf[x].size() == 1 && y != 0)
		leaves[x].push_back(x);
	for (auto e : graf[x]) if (e.first != y) {
		wei_up[e.first] = e.second;
		p[e.first] = x;
		dfs(e.first, x);
		for (int c : leaves[e.first]) leaves[x].push_back(c);
	}
	debug(arr(leaves, x));
	out[x] = timer++;
}
...
int main() {
	...
	int root = -1;
	FOR(i, 1, n) if (graf[i].size() == 1) {
		root = i;
		break;
	}
	FOR(i, 1, n)
		if (graf[i].size() == 2u && graf[i][0].second != graf[i][1].second) {
			debug("err on", arr(graf, i));
			return 0 * printf("NO\n");
		}
	debug(imie(root));
	dfs(root);
	for (int i = 1; i <= n; ++i) {
		if (i == root || graf[i].size() == 2u) continue;
		vector <int> down, rest;
		for (pii v : graf[i]) if (v.first != p[i]) {
			down.push_back(leaves[v.first][0]);
			if (down.size() == 2u) break;
		}
		int curr = i;
		while (graf[p[curr]].size() == 2u)
			curr = p[curr];
		rest = {root};
		debug(arr(p, curr));
		for (pii v : graf[p[curr]]) {
			if (v.first != curr && v.first != p[p[curr]]) {
				rest.push_back(leaves[v.first][0]);
				break;
			}
		}
		if (down.empty()) down = {i};
		debug(imie(i), imie(down), imie(rest));
		int c = wei_up[i] / 2;
		for (int k = 0; k < 2; ++k)
			do_op(down[k % down.size()], rest[k % rest.size()], c);
		if (down.size() >= 2u)
			do_op(down[0], down[1], -c);
		if (rest.size() >= 2u)
			do_op(rest[0], rest[1], -c);
	}
	printf("YES\n%d\n", (int) ops.size());
	for (auto x : ops) printf("%d %d %d\n", FS(x.first), x.second);
}