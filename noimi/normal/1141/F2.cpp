...
int main() {
    ...
    VEC(ll,a,n);
    vll r(1);
    rep(i,n) r.eb(r.back() + a[i]);
    map<ll, vpi> mp;
    rep(l,n)rep2(rr,l + 1,n) mp[r[rr] - r[l]].eb(l,rr);
    int ans = 0;
    vpi w;
    for(auto [_,v]:mp){
        sort(all(v),[](auto i,auto j){return i.se < j.se;});
        int R = -1;
        vpi p;
        int tmp = 0;
        for(auto [x,y]:v){
            if(R <= x) R = y,tmp++,p.eb(x,y - 1);
        }
        if(chmax(ans,tmp)) w = p;
    }
    cout << ans << endl;
    for(auto [x,y]:w) cout << x + 1 <<" " << y + 1 << endl;
}