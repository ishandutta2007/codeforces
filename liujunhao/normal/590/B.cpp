...
int v,t;
void read(){
	Read(st.x),Read(st.y),Read(ed.x),Read(ed.y);
	Read(v),Read(t);
...
double check(double mid){
	...
	if(mid>t)
		...
		tt=st+a*mid;
	return dot(ed-tt,ed-tt)<v*v*mid*mid;
}
double Divide_Conqure(double deg,point wind,double l,double r){
	double mid;
	int t=100;
	while(--t){
		mid=(l+r)/2;
...
{
	read();;
	double dist=sqrt(dot(ed-st,ed-st)),deg=atan2(ed.y-st.y,ed.x-st.x);
	double tt=Divide_Conqure(deg,a,0,1e9);
		printf("%.10f\n",tt);
