...
int main() {
    ...
    int mn = min(n, k);
    vector<mint> choose_n(mn + 1, 1);
    for (int i = 1; i <= mn; i++) {
        choose_n[i] = choose_n[i - 1] * (n - (i - 1)) / i;
    }

    vector<mint> poly1(mn + 1);
    for (int i = 1; i <= mn; i++) {
        poly1[i] = ifact[i] * mint(i).power(k);
        if (i % 2) poly1[i] *= -1;
    }

    vector<mint> poly2(mn + 1);
    for (int i = 0; i <= mn; i++) {
        poly2[i] = ifact[i];
    }
    auto prod = FFT::multiply<MOD>(all(poly1), all(poly2));

    mint ans = 0, prob = mint(1) / m, cur_prob = 1;
    for (int cnt = 1; cnt <= mn; cnt++) {
        // mint real_tot = 0;
        // for (int i = 1; i <= cnt; i++) {
        //     mint cur = choose(cnt, i) * mint(i).power(k);
        //     real_tot += cur * ((cnt - i) % 2 ? -1 : 1);
        // }
        mint tot = prod[cnt] * fact[cnt];
        if (cnt % 2) tot *= -1;

        cur_prob *= prob;
        ans += cur_prob * choose_n[cnt] * tot;
    }
    cout << ans << '\n';
}