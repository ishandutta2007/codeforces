...
void Ber(int n){
  ...
  rep(i,n+1)if(i){
    rep(j,i)
      (b[i]+=(j%2?-1:1)*comb(i+1,j).v*b[j]%MOD)%=MOD;
    b[i]=(b[i]%MOD+MOD)%MOD;
    (b[i]*=modpow(i+1,-1))%=MOD;
    if(i%2==0) b[i]=(MOD-b[i])%MOD;
  }
}

ll sumpowk(ll n,ll k){
  ll re=0; n%=MOD;
  rep(i,k+1)
    re=(re+comb(k+1,i).v*b[i]%MOD)%MOD*n%MOD;
  return re*modpow(k+1,-1)%MOD;
}
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	const mint inv5=modpow(5,-1);
	const mint inv6=modpow(6,-1);
	const mint ninv6=inv6*(MOD-1);
	const mint m1=(m+1)%MOD;
	auto S2=[&](mint n){
		return n*(n+1)*(n+n+1)*inv6;
	};
	auto S4=[&](mint n){
		return S2(n)*(3*n*n+3*n-1)*inv5;
	};
	auto S6=[&](mint n){
		return mint(sumpowk(n.v,6));
	};
	auto G=[&](ll k){
		ll i=(m+1-k)%MOD;
		mint re=i;
		...
		re*=inv6;
		re*=MOD-1;
		re+=mint(i*(i+1ll)/2%MOD)*m1;
		return re;
	};
	auto F2=[&](ll k)->mint{
		ll n=upper_bound(all(sqs),m-k*k)-sqs.begin()-1;
		// mint ret=0;
		// rep(i,n+1){
		// 	mint x=k*k%MOD;
		// 	x+=1ll*i*i%MOD;
		// 	ret+=G(x.v);
		// }
		const mint A=m1-(k%MOD)*(k%MOD)%MOD;
		const mint B=A+1;
		const mint C=A+B;
		mint reX=0;
		reX+=A*B*C*(n+1);
		reX-=(A*B*2+B*C+A*C)*S2(n);
		reX+=(2*A+2*B+C)*S4(n);
		reX-=2*S6(n);
		reX*=ninv6;
		mint reY=0;
		...
		// mint rex=0,rey=0;
		// rep(i,n+1){
		// 	rex+=(A-i*i%MOD)*(B-i*i%MOD)*(C-2*i*i%MOD)*ninv6;
		// 	rey+=m1*modpow(2,-1)*(A-i*i%MOD)*(B-i*i%MOD);
		// }
		// out(k,ret,rex,reX,rey,reY,1);
		return reX+reY;
	};
	auto F=[&](ll k)->mint{
		ll n=upper_bound(all(sqs),m-k*k)-sqs.begin()-1;
		if(n<0) return 0;
		const mint k2=k*k%MOD;
		mint re=(n+1)%MOD*k2*(m1-k2);
		re+=(m1-2+k2)*S2(n);
		re-=S4(n);
		out(k,n,re,F2(k),1);
		return re;
