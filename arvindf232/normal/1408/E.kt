...
fun Graph.kruskal(): Long {
    val edgeorder = (0 until (this.edgecount shr 1)).sortedByDescending { weights[it] }
    var cost = 0L
    val dsu = DisjointUnionSets(n)
    val taken = fastEmptyList(n-1)
    ...
    solve.cases{
        ...
        for(i in 0 until m){
            ...
            for(v in have) {
                var v = v - 1
                val weight = A[i] + B[v]
                G.addWeighted(i,v+m, weight)
                total += weight
            }
        }

        val discount = G.kruskal()
        put(total - discount)
...
/*
really should fix the directed vs undirected edge things
 */