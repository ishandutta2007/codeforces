...
const int maxn = 1e6 + 100, maxw = (1 << 20) + 5, inf = 1e9 + 100, LG = 18;

vector<int> e[maxn], eo[maxn];
...
vector<int> ed[maxn], oed[maxn];

int col[maxn];

int ng[maxn], good[maxn];

void gfs(int v, int k) {
    col[v] = k;
    vis[v] = 1;
    if (ng[v] != 0)
        good[k] = ng[v];
...
void solve() {
    ...
    vector<string> a(n);
    vector<vector<int>> mas(n, vector<int>(m, -1));
    int cnt = n * m;
    for (int i = 0; i < n; i++)
        cin >> a[i];
    for (int i = 0; i < n; i++)
    ...
    for (int i = 0; i < n; i++)
        maks(i);
    sort(all(cand), [&](int i, int j) {return good[i] < good[j];});
    cerr << "idsa " << idsa << '\n';
    cerr << "cand " << cand.size() << '\n';
    memset(vis, 0, n);
    int ans = 0;
