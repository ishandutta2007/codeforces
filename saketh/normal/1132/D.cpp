...
int main() {
    ...
    auto death = [&](ll t, ll charge, ll usage) -> ll {
        ll delta = (charge + usage) / usage;
        return t + delta;
    };
    ...
    auto possible = [&](ll x) -> bool {
        ...
        /* auto print_all = [](priority_queue<S, vector<S>, greater<S>> x) { */
        /*     vector<S> v; */
        /*     while (!x.empty()) */
        /*         v.pb(x.top()), x.pop(); */
        /*     pd(v); */
        ...
        for (int i = 0; i < n; i++) {
            pq.push(S{death(0, charge[i], usage[i]), i});
        }
        ...
        for (int i = 0; i < k; i++) {
            ...
            if (!pq.empty() && pq.top().f <= i)
                return false;
            ll curCharge = charge[cur.s] - usage[cur.s] * i;
            if (curCharge < 0)
                return false;

            charge[cur.s] += x;
            pq.push(S{death(t[cur.s], charge[cur.s], usage[cur.s]), cur.s});
        }
        if (!pq.empty() && pq.top().f < k)
            return false;
        return true;
    };
