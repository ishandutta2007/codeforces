...
void test_case(int test) {
    ...
    Node* root = nullptr;
    for (auto [x, y] : v) {
        // auto it = find(cur.begin(), cur.end(), y - 1);
        // if (it != cur.end()) cur.erase(it);
        {
            auto [a, bc] = split(root, y - 1);
            auto [b, c] = split(bc, y);
            root = mrg(a, c);
        }
