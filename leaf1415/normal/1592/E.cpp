...
int main(void)
{
	...
	rep(i, 0, 19){
		...
		per(j, n-1, 0){
			succ[j] = succ[j+1];
			if((a[j+1] & (1<<i)) == 0) succ[j] = j+1;
		}
		rep(j, 0, (1<<20)-1) vec[j].clear();
		rep(j, 1, n) s[j] = s[j-1] ^ (a[j] >> i);
		rep(j, 0, n) vec[s[j]].push_back(j);
		...
		rep(j, 1, n){
			ll x = s[j-1];
			ll p = lower_bound(all(vec[x]), succ[j-1]) - vec[x].begin() - 1;
			if(p < 0 || vec[x][p] < j) continue;
			chmax(ans, vec[x][p]-j+1);
		}
	}
	outl(ans);
