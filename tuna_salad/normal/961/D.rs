...
fn main() {
	...
	for i in 0..3 {
		for j in i+1..3 {
			let get = |p1: (i32,i32), p2: (i32,i32)| -> (i32,i32) {
			...
			let kill = |s: &[usize], q: (i32,i32), d: (i32,i32)| -> Vec<usize> {
				let mut rem = v!([]);
				for &k in s.iter() {
					if p[k] != q && get(p[k], q) != d {
						rem.push(k);
					}
				}
				rem
			};
			let rem1 = kill(&all, p[i], get(p[i],p[j]));
			if rem1.len() <= 2 {
				writeln!(rout, "YES").ok();
				return;
			}
			let a = rem1[0];
			let b = rem1[1];
			let rem2 = kill(&rem1, p[a], get(p[a],p[b]));
			if rem2.len() == 0 {
				writeln!(rout, "YES").ok();
				return;
			}
