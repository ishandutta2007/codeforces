...
signed main() {
  ...
  auto solve = [&](ll b, ll w) -> pair<bool, VEC<pi>> {
    VEC<pi> ANS;
    assert(b >= w);
    if (b > 3 * w + 1) return {false, ANS};
    FOR(i, w) { ANS.eb(mp(2, 2 * i + 2)); }
    FOR(i, w) { ANS.eb(mp(2, 2 * i + 3)); }
    ANS.eb(mp(2, 1));
    FOR(i, w) {
      ANS.eb(mp(1, 2 * i + 2));
      ANS.eb(mp(3, 2 * i + 2));
  ...
  FOR(_, T) {
    ...
    bool ok;
    if (b >= w) {
      tie(ok, ANS) = solve(b, w);
    } else {
      tie(ok, ANS) = solve(w, b);
      FORIN(p, ANS) { p.fi += 1; }
    }
    if (ok) {
      YES();
