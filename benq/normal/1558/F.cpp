...
tcT, int SZ> struct LazySeg { 
	...
	// LazySeg() { F0R(i,2*SZ) seg[i] = ID, lazy[i] = 0; }
...
int dumb(vi zeros) {
	...
	for (int i = 0; ; ++i) {
		...
		if (i%2 == 0) {
			...
			F0R(j,sz(zeros)) if (!(zeros[j]&1)) if (zeros[j]) if (j == 0 || zeros[j] != zeros[j-1]+1) --zeros[j];
		}
		// dbg("EN");
	}
}
...
int solve_range(int st, int len, int turn) {
	...
	vi res(21,1);
	F0R(i,len) res[st+i] = 0;

	return ans+len-1+st;

	// return ans+movs(res,turn);

	// return movs(res,turn);
}

int smart(vi zeros) {
	if (!sz(zeros) || zeros.bk == sz(zeros)-1) return 0;
	int ans = -MOD;
	F0R(i,sz(zeros)) if (zeros[i] != i) {
