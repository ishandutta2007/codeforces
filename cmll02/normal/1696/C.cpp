...
int e[1000005],f[1000005];
void gen(int *a,int n,int m,int *c,int *d)
{
	for(int i=1;i<=n;i++)
	{
		int p=a[i],k=1;
...
signed main()
{
	...
	for(int i=2;i<=n;i++)
	{
		if(c[i]==c[ta])d[ta]+=d[i];
		else ta++,c[ta]=c[i],d[ta]=d[i];
	}
	for(int i=2;i<=k;i++)
	{
		if(e[i]==e[tb])f[tb]+=f[i];
		else tb++,e[tb]=e[i],f[tb]=f[i];
	}
	...
	{
		int op=0;
		rg(ta)if(c[i]!=e[i]||d[i]!=f[i])op=1;gr
		if(op)puts("No");
		else puts("Yes");
	}
