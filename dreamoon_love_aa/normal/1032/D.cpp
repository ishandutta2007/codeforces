...
int main(){
    ...
    vector<Point>d[2];
    REP(i,2){
        if(b)d[i].PB(Point(p[i].x,(p[i].x*a+c)/-b));
        if(a)d[i].PB(Point((p[i].y*b+c)/-a,p[i].y));
    }
    double an=fabs(p[0].x-p[1].x)+fabs(p[0].y-p[1].y);
    for(auto x:d[0])for(auto y:d[1]){
        an=min(an,(p[0]-x).dis()+(x-y).dis()+(y-p[1]).dis());
    }
    W(an);
    return 0;
}