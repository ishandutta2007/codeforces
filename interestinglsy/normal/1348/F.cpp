...
struct Segtree{
	...
	void Modify( int gpos , int rpa , int id , ARG ){
		if( l == r ){
			t[pos] = {rpa,id};
			return;
		}
		int mid = l+r >> 1;
		if( gpos <= mid ) Modify(gpos,rpa,id,LSON);
		else Modify(gpos,rpa,id,RSON);
...
int main(){
	...
	For(i,n){
		if( i != 1 ){
			int b = i;
			Node res = sgt.Query(l[p[b]],b,ROOT);
			if( res.mx >= b ){
				puts("NO");
				Print(ansl,1,n);
				swap(ansl[p[b]],ansl[p[res.mxId]]);
				Print(ansl,1,n);
				END();
			}
		}
		int a = i;
		sgt.Modify(a,r[p[a]],a,ROOT);
	}
