...
pii read(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
...
int main() {
	...
	vpii ord;
	REP(i, n) ord.emplace_back(r[i], i);
	sort(ALL(ord));
	for (auto [_, i] : ord) {
		vector <int> put_back;
		while (true) {
			assert((int)put_back.size() <= n);
			pii m = read(0, l[i]);
			debug(arr(l, i), imie(m));
			if (m.first < l[i]) break;
			put_back.push_back(m.second);
			put({0, 0}, l[m.second]);
		}
		for (int x : put_back) {
			onion(x, i);
