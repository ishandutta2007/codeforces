...
modint calc(ll n, ll r, ll z, int k)
{
	int id = (R >> k) - (r >> k);
	if (u[k][id]) return dp[k][id];
	u[k][id] = 1;
	modint& res = dp[k][id] = 0;
	if (k == MAXK) return res = 1;
	for (int i = 0; i <= n; ++i)
	{
		if (i % 2 == (z >> k & 1))
		{
			res += modint::Binomial(n, i) * calc(n, r, z, k + 1);
		}
		r -= 1LL << k;
		if (r < 0) break;
	}
