...
modint<MAX_MOD> gogos[300000];
modint<MAX_MOD> val[20][20];
modint<MAX_MOD> comp;
int k;
int dfs(int now,modint<MAX_MOD> value) {
    if (now == k) {
        if (value.value == comp.value)return 1;
        return 0;
    }
    int ans = 0;
    REP(i, now + 1) {
        ans += dfs(now + 1, value + val[now+1][i]);
    }
    return ans;
}
void solve() {
    ...
    REP(i, n) {
        ...
        REP(q, vertexs[i].size()) {
            val[vertexs[i].size()][q] += gogos[vertexs[i][q].second];
        }
    }
    cout << dfs(0, 0) << endl;
}
