...
int main() {
    ...
    rep(i, k) {
        map<pll, vi> mp;
        rep(j, n) {
            ll dx = s[j] - x[i], dy = t[j] - y[i];
            ll g = gcd(dx, dy);
            dx /= g, dy /= g;
            mp[pll(dx, dy)].eb(j);
        }
        for(auto &[_, v] : mp) {
            sort(all(v), [&](int f, int g) { return abs(s[f] - x[i]) + abs(t[f] - y[i]) < abs(s[g] - x[i]) + abs(t[g] - y[i]); });
            // dump(i, v);
            rep(j, si(v)) { rep(kk, min<int>(j, k + 1)) need[i][v[j]].eb(v[kk]); }
        }
    }
    int sum = 0;
    rep(i, n) {
        auto dfs = [&](auto &&f, int used, vi &ret) -> bool {
            if(k - popcount(used) < si(ret)) return false;
            ...
            rep(j, k) {
                ...
                rep(p, si(ret)) {
                    ...
                    swap(nxt[p], nxt.back());
                    nxt.pop_back();
                    for(auto e : need[j][ret[p]]) nxt.eb(e);
                    UNIQUE(nxt);
                    if(f(f, used | 1 << j, nxt)) return true;
                }
            }
            return false;
        };
