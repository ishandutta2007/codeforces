...
Node operator+( Node l , Node r ){
	...
	Node ret = Node{
		...
		max( l.mx , r.mx ),
		min( l.mi , r.mi ),
		max({ l.a_b , r.a_b , l.mx - 2*r.mi }),
		max({ l.b_c , r.b_c , r.mx - 2*l.mi }),
		max({ l.ans , r.ans , l.a_b + r.mx , l.mx + r.b_c }),
		0
	};
	return ret;
}
...
il void Push( int pos ){
	if(!tree[pos].add) return;
	Pushadd(pos<<1,tree[pos].add);
	Pushadd(pos<<1|1,tree[pos].add);
	tree[pos].add = 0;
}
void Add( int gl , int gr , int x , ARG ){
	if( gl <= l and r <= gr ){
		Pushadd(pos,x);
		return;
	}
...
int main(){
	...
	For(qwq,q){
		iRead2(a,b);
		if( s[a] != s[b] ){
			Add(a+1,n+2,(s[a]=='('?-2:2),ROOT);
			Add(b+1,n+2,(s[b]=='('?-2:2),ROOT);
			swap(s[a],s[b]);
		}
		printf("%d\n",tree[1].ans);
	}
