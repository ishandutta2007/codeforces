...
#define SZ 666666
int T,n,l1[SZ],l0[SZ],f_[SZ],*f=f_+1; char buf[2][SZ];
void main()
{
	...
	f[-1]=-2e9;
	while(T--) {
		cin>>n>>(buf[0]+1)>>(buf[1]+1);
		for(int i=1;i<=n;++i) {
			auto c0=buf[0][i],c1=buf[1][i];
			if(c0=='0'||c1=='0') l0[i]=i;
			else l0[i]=l0[i-1];
			if(c0=='1'||c1=='1') l1[i]=i;
			else l1[i]=l1[i-1];
			f[i]=f[i-1];
			f[i]=max(f[i],f[l0[i]-1]+1);
			f[i]=max(f[i],f[min(l0[i],l1[i])-1]+2);
		}
		cout<<f[n]<<'\n';
	}
}
