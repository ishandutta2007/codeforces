...
int main(){
  ...
  modint ret=0;
  {
    auto calc=[&](ll sz)->modint{
      auto dp=vec(sz,26,modint(0));
      rep(i,0,26)dp[0][i]=1;
      rep(i,0,sz-1){
        rep(j,0,26){
          rep(o,0,26){
      ...
      modint sum=0;
      rep(i,0,26)sum+=dp[sz-1][i];
      return sum;
    };
    ret+=calc(n/2)*calc(n-n/2);
  }
  //cout<<ret<<endl;
  {
    auto calc=[&](ll sz){
      ...
      rep(i,0,3)rep(j,0,3){
        ll add=1;
        if(i==0)add*=24;if(j==0)add*=24;
        dp[0][(ll)(i==1)+(j==1)][(ll)(i==2)+(j==2)][i][j]=add;
      }
      ll lim=(sz/2)+3;
      rep(i,0,sz-2){
        rep(j,0,lim){
          rep(o,0,lim){
            rep(k,0,3){
              rep(kk,0,3){
                auto val=dp[i][j][o][k][kk];
                dp[i+1][j][o][kk][0]+=val*(24-(k==0));
                if(k!=1)dp[i+1][j+1][o][kk][1]+=val;
                if(k!=2)dp[i+1][j][o+1][kk][2]+=val;
              }
            }
          }
        }
      }
      auto v=vec(sz+1,sz+1,modint(0));
      modint sum=0;
      rep(i,0,lim)rep(j,0,lim)rep(o,0,3)rep(oo,0,3){
        v[i][j]+=dp[sz-2][i][j][o][oo];
        sum+=dp[sz-2][i][j][o][oo];
      }
      //cout<<sum<<endl;
      return v;
    };
    ...
    //debug(v,n+1,n+1);
    rep(i,0,26){
      rep(j,c[i]+1,n+1)rep(o,0,n+1){
        ret-=v[j][o];
      }
    }
    rep(i,0,26)rep(j,i+1,26){
      rep(x,c[i]+1,n+1)rep(y,c[j]+1,n+1){
        ret+=v[x][y];
      }
    }
  }
