...
int main() {
	...
	vector<Mint> fac(MAXN + 1, Mint(1, MOD107)), ifac = fac;
	for (int i = 2; i <= MAXN; i++) fac[i] = fac[i - 1] * i;
	ifac[MAXN] = fac[MAXN].inv();
	for (int i = MAXN; i >= 3; i--) ifac[i - 1] = ifac[i] * i;

	vector<Mint> inv(MAXN + 1, Mint(0, MOD107));
	...
	const int FULL = 1 << n;
	vector<vector<Mint>> kill(n, vector<Mint>(FULL, Mint(1, MOD107)));
	for (int i = 0; i < n; i++) {
		for (int mask = 1; mask < FULL; mask++) {
			int lowbit = mask & -mask;
			int b = __lg(lowbit);

			kill[i][mask] = kill[i][mask ^ lowbit] * a[i] * inv[a[i] + a[b]];
		}
	}
	
	vector<Mint> dp(FULL, Mint(1, MOD107));
	for (int mask = 1; mask < FULL; mask++) {
		for (int submask = (mask - 1) & mask; submask > 0; submask = (submask - 1) & mask) {
			int compmask = mask ^ submask;
			Mint prob = dp[submask];
			for (int i = 0; i < n; i++) {
				if (submask >> i & 1) {
					prob *= kill[i][compmask];
				}
			}
			dp[mask] -= prob;
		}
	...
	for (int mask = 1; mask < FULL; mask++) {
		...
		for (int i = 0; i < n; i++) {
			if (mask >> i & 1) {
				prob *= kill[i][(FULL - 1) ^ mask];
			}
		}
		ans += __builtin_popcount(mask) * prob;
	}
