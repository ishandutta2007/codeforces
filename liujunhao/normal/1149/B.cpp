	...
	while(Q--){
		...
		scanf("%s",c);
			...
			if(!id)
				...
				x=0,y=1;
			int lenx=a[x].size(),leny=a[y].size(),leni=a[id].size();
			for(i=0;i<=lenx;i++){
				for(j=0;j<=leny;j++){
					if(!id){
						f[leni][i][j]=nxt[f[leni-1][i][j]][a[id][leni-1]-'a'];
						if(i)
							f[leni][i][j]=min(f[leni][i][j],nxt[f[leni][i-1][j]][a[x][i-1]-'a']);
						if(j)
							f[leni][i][j]=min(f[leni][i][j],nxt[f[leni][i][j-1]][a[y][j-1]-'a']);
					}
					else if(id==1){
						f[i][leni][j]=nxt[f[i][leni-1][j]][a[id][leni-1]-'a'];
						if(i)
							f[i][leni][j]=min(f[i][leni][j],nxt[f[i-1][leni][j]][a[x][i-1]-'a']);
						if(j)
							f[i][leni][j]=min(f[i][leni][j],nxt[f[i][leni][j-1]][a[y][j-1]-'a']);
					}
					
					else{
						f[i][j][leni]=nxt[f[i][j][leni-1]][a[id][leni-1]-'a'];
						if(i)
							f[i][j][leni]=min(f[i][j][leni],nxt[f[i-1][j][leni]][a[x][i-1]-'a']);
						if(j)
							f[i][j][leni]=min(f[i][j][leni],nxt[f[i][j-1][leni]][a[y][j-1]-'a']);
					}
				}
			}
		}
		else{
			a[id].erase(prev(a[id].end()));
		}
		if(f[a[0].size()][a[1].size()][a[2].size()]<=n)
			puts("YES");
		else
			puts("NO");
	}
