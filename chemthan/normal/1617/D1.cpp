...
void chemthan() {
  ...
  while (test--) {
    ...
    vi vx({a[0], a[0] + 1, a[0] + 2});
    vi vy({b[0], b[0] + 1, b[0] + 2});
    int impostor, crewmate;
    {
      vi cnt(2);
      for (int e : vy) {
        cnt[ask(vx[0], vx[1], e)]++;
    ...
    vi impostors({impostor});
    FOR(i, 0, n) if (i != impostor && i != crewmate) {
      if (!ask(impostor, crewmate, i)) {
        impostors.pb(i);
      }
    }
    cout << "! " << sz(impostors) << " ";
    FOR(i, 0, sz(impostors)) cout << impostors[i] + 1 << " \n"[i == sz(impostors) - 1];
    cout.flush();
#ifdef LOCAL
    for (int x : impostors) {
      assert(f[x]);
