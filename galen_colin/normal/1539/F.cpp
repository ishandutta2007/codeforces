...
vector<ll> at[200005];
segtree<max_t> mxl, mxr; // going left, going right...
segtree<min_t> mnl, mnr;

ll tot = 0;

void put(ll i, ll v) { // a[i] := v
	ll d = v - b[i];
...
ll query_min(ll i) {
	ll v = mnl.query(0, i).val;
	v += mnr.query(i, n - 1).val;
	return v - tot;
}

ll query_max(ll i) {
	ll v = mxl.query(0, i).val;
	v += mxr.query(i, n - 1).val;
	return v - tot;
}
