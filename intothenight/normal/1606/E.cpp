...
int main(){
	...
	vector<vector<modular>> power(501);
	for(auto x = 0; x <= 500; ++ x){
		power[x] = precalc_power(modular(x), 500);
	}
	combinatorics<modular> C(500);
	int n, mx;
	cin >> n >> mx;
	vector<vector<modular>> dp(n + 1, vector<modular>(mx + 1));
	for(auto x = 2; x <= n; ++ x){
		for(auto y = 1; y <= mx; ++ y){
			// all < x
			dp[x][y] = power[min(x - 1, y)][x];
			// z of them goes over
			if(x <= y){
				for(auto z = 2; z <= x; ++ z){
					dp[x][y] += C.C(x, z) * dp[z][y - (x - 1)] * power[x - 1][x - z];
				}
			}
		}
	}
