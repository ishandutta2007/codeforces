...
#endif
//#STAY AT HOME
const int nax = 15000 + 44;
ull a[nax];
vpii ans;
void push(int x, int y) {
	a[x] = a[y] = mix(mix(a[x]) ^ mix(a[y] + 123456789101112llu));
...
void solve() {
	...
	forbits(i, n) {
		...
		int high = tail + take;
		for (int j = 0; j < i; ++j) {
			for (int k = 0; k < take; ++k) if (k >> j & 1)
				push(tail + k, tail + (k ^ (1 << j)));
		}
		parts.emplace_back(take);
		iota(ALL(parts.back()), tail);
		debug(range(a, tail, high));
		tail = high;
	}
	debug(imie(parts));
	while (parts.size() > 2u) {
		sort(parts.begin(), parts.end(), [](vi &a, vi &b){return a.size() < b.size();});
		int s1 = parts[0].size(), s2 = parts[1].size();
		if (s1 > s2 - s1) {
			vi p1x, p2x;
			for (int i = 0; i < s1; ++i) {
				p1x.push_back(parts[0][i]);
				p1x.push_back(parts[1][i]);
				push(parts[0][i], parts[1][i]);
			}
			for (int i = s1; i < s2; ++i) p2x.push_back(parts[1][i]);
			parts[0] = p1x;
			parts[1] = p2x;
		}
		else {
			int moved = parts[0].size();
			vi p1x;
			REP(i, moved) {
				p1x.push_back(parts[0][i]);
				p1x.push_back(parts.back()[i]);
				push(parts[0][i], parts.back()[i]);
			}
			parts.back().erase(parts.back().begin(), parts.back().begin() + moved);
			parts[0] = p1x;
		}
		debug(imie(parts));
		while(parts[0].empty()) parts.erase(parts.begin());
	}
	printf("%d\n", (int) ans.size());
