...
void solve() {
  LL(N);
  VEC(ll, A, N);
  auto f = [&](ll L, ll M, ll R) -> mint {
    mint ANS = A[M] - 1; //  1
    vi B = {A.begin() + L, A.begin() + M + 1};
    vi C = {A.begin() + M, A.begin() + R};
    reverse(all(B));
    auto calc = [&](vi A) -> mint {
      if (len(A) < 2) return 0;
      mint s = 0;
      ...
      FOR3(i, 1, len(A)) {
        s += x * (min(A[i], A[i - 1]) - 1);
        if (i + 1 < len(A)) x *= mint(min({A[i - 1], A[i], A[i + 1]}) - 1);
      }
      return s;
    };
    auto b = calc(B), c = calc(C);
    if (len(B) >= 2) ANS += b * mint(min(B[0], B[1]) - 1);
    if (len(C) >= 2) ANS += c * mint(min(C[0], C[1]) - 1);
    if (len(B) >= 2 && len(C) >= 2) {
      ANS += b * c * mint(min({B[0], B[1], C[0], C[1]}) - 1);
    }
    return ANS;
  };
