...
int a[MAXN];
int sum[MAXN];
int needtobuy[MAXN], nocost[MAXN];
...
int Dfs( int pos ){
	if( pos == n+1 ) return 0;
	if(KNOW) return NOW;
	int ans = Dfs(pos+1) + a[pos];
	For(i,m){
		if( n-pos+1 < needtobuy[i] ) continue;
		int tans = Dfs(pos+needtobuy[i]) + sum[pos+needtobuy[i]-1] - sum[pos+nocost[i]-1];
		Mymin(ans,tans);
	}
	return NOW = ans;
}
...
int main(){
	...
	Read(n,m,k);
	For(i,n) Read(a[i]);
	For(i,m) Read(needtobuy[i],nocost[i]);
