...
int n, k;
void add(int pos) {
    events.insert({p[pos].second, 1});
    events.insert({p[pos].second + k, -1});
}
void remove(int pos) {
    events.erase(events.find({p[pos].second, 1}));
    events.erase(events.find({p[pos].second + k, -1}));
}
int main() {
//    FREIN("in.txt");
    ...
    reunique(yy);
    for (int i = 0; i < n; ++i) {
        int x = lower_bound(all(xx), p[i].first) - xx.begin();
        int y = lower_bound(all(yy), p[i].second) - yy.begin();
        c[i] = {x, y};
    }
//    printarray(c, n);
    int lp = 0;
    ...
    int curx = -INF;
    int prevx = INF;
    events.insert({INF + 1, INF + 1});
    while (true) {
        prevx = curx;
        if (p[lp].first + k < p[rp + 1].first) {
            curx = p[lp].first + k;
            remove(lp);
            ++lp;
        } else {
        ...
        if (lp > rp) continue;
        if (rp == n - 1) break;
        while (rp < n && c[rp + 1].first == c[rp].first) {
            ++rp;
            add(rp);
        }
        while (p[lp].first + k <= curx) {
            remove(lp);
            ++lp;
        }
        int nextx = min(p[rp + 1].first, p[lp].first + k);
//        DBN(lp, rp, p[lp].first, p[rp].first);
        int xbefore = 1;
        int xafter = nextx - curx;
        int xcount = xbefore + xafter - 1;
//        DBN(xbefore, xafter, xcount);
        int balance = 0;
        ...
        while (true) {
            ...
            int nexty = it->first;
            int ycount = nexty - cury;
            ans[balance] += xcount * ycount;
        }
//        cout << endl;
    }
    for (int i = 1; i <= n - 2; ++i) {
