...
ll a[3005];
ll succ[3005];
ll dp[3005][3005], dp2[3005][3005], dp3[3005];
vector<ll> vec[3005];
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n){
			reps(j, vec[i]){
				if(j == sz(vec[i])-1) succ[vec[i][j]] = n+1;
				else succ[vec[i][j]] = vec[i][j+1];
			}
		}
		//outa(succ, 1, n);
		
		rep(i, 1, n) rep(j, 1, n){
			if(i <= j) dp[i][j] = dp2[i][j] = inf;
			else dp[i][j] = 0;
		}
		rep(i, 1, n) dp[i][i] = dp2[i][i] = 1;
		...
		rep(len, 2, n){
			rep(l, 1, n){
				...
				if(a[l] == a[r]){
					...
					while(succ[p] < r){
						p = succ[p];
						chmin(dp2[l][r], dp[l+1][p-1] + dp2[p][r]);
					}
					chmin(dp2[l][r], dp[l+1][r-1] + 1);
				}
				
				ll p = l;
				do{
					chmin(dp[l][r], dp2[l][p] + dp[p+1][r]);
					p = succ[p];
				}while(p <= r);
			}
		}
		
		//rep(i, 1, n) outa(dp[i], 1, n);
		...
		rep(i, 1, n){
			...
			reps(j, vec[i]){
				if(j == sz(vec[i])-1) break;
				reps(k, vec[i]){
					if(j >= k) continue;
					ll s = vec[i][j], t = vec[i][k];
					chmin(dp3[t], dp3[s] + dp[s+1][t-1]);
				}
			}
			chmin(ans, dp3[n+1]);
		}
