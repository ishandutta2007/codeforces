...
int main(void)
{
	...
	vector<mint> fvec, gvec, hvec;
	per(i, n-s-1, 0) fvec.pb(fact[i]);
	rep(i, 0, n-s) gvec.pb(fact_inv[i]);
	NTT_Convolution::conv(fvec, gvec, hvec);
	
	rep(i, 2, n-s+1) dp[i] = hvec[n-s+1-i] * fact[n-i] * mint(i-1);
	dp[1] = fact[n-1];
	
	per(i, n, 1){
		ans[i] = dp[i];
		ans[i] -= sum[i+1] / mint(i);
		sum[i] = sum[i+1] + ans[i];
	}
	outa(ans, 1, n);
