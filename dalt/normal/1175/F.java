...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            io.cache.append(process(a) + process(b) + oneCount);
        }

        public int process(Integer[] a) {
            int[] validUntil = new int[n];
            validUntil[n - 1] = n - 1;
            int[] registries = new int[n + 1];
            Arrays.fill(registries, n);
            registries[a[n - 1]] = n - 1;
            St<Integer> st = new St<Integer>(a, n, Comparator.naturalOrder());
            St<Integer> rev = new St<Integer>(a, n, Comparator.reverseOrder());
            for (int i = n - 2; i >= 0; i--) {
                validUntil[i] = Math.min(registries[a[i]] - 1, validUntil[i + 1]);
                registries[a[i]] = i;
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                int max = a[i];
                for (int j = i + 1; j <= validUntil[i]; j++) {
                    max = Math.max(max, a[j]);
                    int t = j - max + 1;
                    if (t < 0 || validUntil[t] < j) {
                        continue;
                    }
                    if (max != rev.query(t, j).intValue()) {
                        continue;
                    }
                    int min = st.query(t, j);
                    if (min != 1) {
                        continue;
                    }
                    //debug.debug("add", Arrays.toString(Arrays.copyOfRange(a, t, j + 1)));
                    total++;
                }
            }
