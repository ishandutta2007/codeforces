...
class FBoryushchiisyaUchastnik {
public:
    void solve() {
        int n = in.readInt();
        ...
        int at = -1;
        for (const auto& p : q) {
            maxim(mx, p.second, at, p.first);
        }
        if (mx <= answer + 2) {
            out.printLine(answer);
            return;
        }
        int delta = mx - (answer + 2);
        int good = 0;
        for (int i : range(1, n)) {
            if (a[i] != at && a[i - 1] != at && a[i] != a[i - 1]) {
                good++;
            }
        }
        if (good >= delta) {
            out.printLine(answer + delta);
        } else {
            out.printLine(-1);
        }
    }
...
int main() {
    ...
    auto time = clock();
#endif
    FBoryushchiisyaUchastnik solver;
