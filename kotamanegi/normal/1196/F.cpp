...
void solve() {
    ...
    REP(i, m) {
        ...
        vertexs[a].push_back(mp(c, b));
        vertexs[b].push_back(mp(c, a));
    }
    REP(i, n) {
        sort(ALL(vertexs[i]));
        while (vertexs[i].size() > k) {
            vertexs[i].pop_back();
        }
    }
    edges.push_back(make_tuple(1e18, -1e9, -1e9));
    sort(ALL(edges));
    reverse(ALL(edges));
    priority_queue<pair<int, pair<int, int>>, vector<pair<int, pair<int, int>>>, greater<pair<int, pair<int, int>>>> next;
    next.push(mp(1e18, mp(-1e9, -1e9)));
    map<pair<int, int>, int> goings;
    set<pair<int, int>> checked;
    while (true) {
        if (get<0>(edges.back()) <= next.top().first) {
            goings[mp(get<1>(edges.back()), get<2>(edges.back()))] = get<0>(edges.back());
            goings[mp(get<2>(edges.back()), get<1>(edges.back()))] = get<0>(edges.back());
            next.push(make_pair(get<0>(edges.back()), mp(get<1>(edges.back()), get<2>(edges.back()))));
            next.push(make_pair(get<0>(edges.back()), mp(get<2>(edges.back()), get<1>(edges.back()))));
            edges.pop_back();
            continue;
        }
        pair<int, pair<int, int>> now = next.top();
        next.pop();
        if (now.first != goings[now.second]) continue;
        checked.insert(mp(min(now.second.first, now.second.second), max(now.second.first, now.second.second)));
        if (checked.size() == k) {
            cout << now.first << endl;
            return;
        }
        for (auto x : vertexs[now.second.second]) {
            int cost = now.first + x.first;
            int go = x.second;
            if (go == now.second.first) continue;
            if (goings.count(mp(now.second.first,go)) == 0||goings[mp(now.second.first, go)] > cost) {
                goings[mp(now.second.first, go)] = cost;
                next.push(make_pair(cost, mp(now.second.first, go)));
            }
        }
    }
}
