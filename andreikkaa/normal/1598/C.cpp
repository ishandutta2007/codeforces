...
ll clev(vector<ull> a) {
    ull n = sz(a);
    ull sum = accumulate(all(a), ull(0));
    ...
    for(int i = 0; i < n; ++i) {
        ...
        if(sum - a[i] < (long double)(sum) * (n - ull(2)) / n) {
            continue;
        }
        ull val = sum - a[i] - (long double)(sum) * (n - ull(2)) / n;
        int lb = max(i + 1, int(lower_bound(all(a), val) - a.begin()));
        int rb = upper_bound(all(a), val) - a.begin();
        if(lb < rb) {
            ans += rb - lb;
        }
...
ll stup(vector<ull> a) {
    ll ans = 0;
    ull sum = accumulate(all(a), ull(0));
    for(int i = 0; i < sz(a); ++i) {
        for(int j = i + 1; j < sz(a); ++j) {
            if(sum * (sz(a) - 2) == (sum - a[i] - a[j]) * sz(a)) {
                ++ans;
            }
        }
    }
