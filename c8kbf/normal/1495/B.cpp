...
inline void AC() {
    ...
    l[1] = 1;
    for(_ i = 2; i <= n; ++i) l[i] = a[i-1] > a[i] ? i : l[i-1];
    r[n] = n;
    for(_ i = n-1; i >= 1; --i) r[i] = a[i+1] > a[i] ? i : r[i+1];
    _ mx = -1E11;
    v(_) b;
    for(_ i = 1; i <= n; ++i) if(a[i] > a[i-1] && a[i] > a[i+1]) {
        _ lb = i-l[i]+1, rb = r[i]-i+1;
        if(max(lb, rb) > mx) {
            mx = max(lb, rb);
            b.clear();
            b.push_back(i);
        } ef(max(lb, rb) == mx) b.push_back(i);
    }
    if(!(mx & 1)) no();
    _ cr = -1;
    for(_ i : b) if(i != 1 && i != n && i-l[i]+1 == r[i]-i+1) {
        if(~cr) no();
        cr = i;
    }
    if(!~cr) no();
    for(_ i : b) if(i != cr) no();
    writeln(1);
