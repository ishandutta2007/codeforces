...
const int MN=8,inv100=inv(100); 
#define int128 __uint128_t
int n,p[MN][MN],a[MN][1<<MN];
map<int128,int>f,g;
vi pet[MN]; 
int128 h[1<<MN][1<<MN]; 
signed main() {
	n=read(); 
	For(i,0,n-1) For(j,0,n-1) p[i][j]=mul(read(),inv100); 
	For(i,0,n-1) {
		For(j,0,(1<<n)-1) {
			int prod=1; 
			For(k,0,n-1) {
				if(j>>k&1) mulmod(prod,p[i][k]); 
				else mulmod(prod,P+1-p[i][k]); 
			}
			a[i][j]=prod;
		}
	}
	For(i,0,(1<<n)-1) pet[__builtin_popcount(i)].pb(i); 
	f[1]=1; 
	...
	For(i,0,n-1) {
		...
		for(auto it:g) {
			...
			int128 pst=it.first; 
			vi tmp; 
			for(auto j:pet[i]) if(pst>>j&1) tmp.pb(j); 
			For(est,1,(1<<n)-1) {
				int128 tst=0; 
				for(auto j:tmp) {
					tst|=h[est][j]; 
				}
				addmod(f[tst],mul(w,a[i][est])); 
			}
		}
		/*
		cerr<<i<<endl;
		for(auto it:f) {
			cerr<<"?"<<endl;
			fprint(it.first,' '); cerr<<it.second<<endl; 
		}
		*/
	} 
	int128 fst=((int128)1<<((1<<n)-1)); 
	cout<<f[fst]<<endl; 
