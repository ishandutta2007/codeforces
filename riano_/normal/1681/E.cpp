...
// M2*M1
template <typename T>
vector<vector<T>> matrix_prod(vector<vector<T>> &M1,vector<vector<T>> &M2){ //M2*M1
    int N = M2.size(); int M = M1[0].size();
    int K = M2[0].size();
    assert(K==M1.size());
    vector<vector<T>> res(N,vector<T>(M,(ll)2e18)); //
    rep(i,N){
        rep(j,M){
            rep(k,K){
                chmin(res[i][j],M2[i][k]+M1[k][j]); //
            }
        }
    }
    return res;
...
public:
    ll n;
    //
    #define type vector<vector<ll>> //
    type el = {{0,(ll)2e18},{(ll)2e18,0}}; //
    type merge_(type A,type B){ //
        return matrix_prod(A,B);
    }
    void update_(type &A, type B){ //1
        A = B;
    }
    void update_2(type &A, type B){ //2
        //A += B;
    }
    ...
    segtree(ll k){
        n = 1;
        while(n<k){ n *= 2; }
        A=vector<type>(2*n,el);
    }

    //update_
    void replace(ll i,type x){
        int index = n-1+i;
        update_(A[index],x);
        while(index>1){
            index /= 2;
            A[index] = merge_(A[2*index],A[2*index+1]);
        }
    }

    //update_2
    void add(ll i,type x){
        int index = n-1+i;
        update_2(A[index],x);
        while(index>1){
            index /= 2;
    ...
    type rangesum(ll a,ll b,ll k,ll c,ll d){
        if(d<a||b<c){
            return el;
        }
        else if(a<=c&&d<=b){
            ...
            return merge_(rangesum(a,b,k*2,c,(c+d)/2),rangesum(a,b,k*2+1,(c+d)/2+1,d));
        }
    }
};
...
int main(){
    ...
    x[N-1][0] = x[N-2][0]+1; y[N-1][0] = y[N-2][0];
    x[N-1][1] = x[N-2][1]; y[N-1][1] = y[N-2][1]+1;
    rep(i,N-1){
        auto m = v2ll(2,2,2e18);
        m[0][0] = abs(x[i][0]-x[i+1][0]) + abs(y[i][0]-y[i+1][0]);
        m[1][1] = abs(x[i][1]-x[i+1][1]) + abs(y[i][1]-y[i+1][1]);
        m[1][0] = min(1+abs(x[i][0]+1-x[i+1][1])+abs(y[i][0]-y[i+1][1]),
                    abs(x[i][0]-x[i][1])+abs(y[i][0]-y[i][1])+m[1][1]);
        m[0][1] = min(1+abs(x[i][1]-x[i+1][0])+abs(y[i][1]+1-y[i+1][0]),
                    abs(x[i][0]-x[i][1])+abs(y[i][0]-y[i][1])+m[0][0]);
        dp.replace(i+1,m);
        // rep(i,2){
        //     rep(j,2){
        //         cout << m[i][j] << " ";
    ...
    rep(i,Q){
        ...
        ll l1 = max(x1,y1); ll l2 = max(x2,y2);
        if(l1==l2){
            cout << abs(x1-x2)+abs(y1-y2) << "\n"; continue;
        }
        if(l1>l2){
            swap(x1,x2); swap(y1,y2); swap(l1,l2);
        }
        auto v = dp.sum(l1,l2-2);
        auto n = v2ll(2,1,2e18);
        n[0][0] = abs(x1-x[l1-1][0])+abs(y1-y[l1-1][0]);
        n[1][0] = abs(x1-x[l1-1][1])+abs(y1-y[l1-1][1]);
        auto u = matrix_prod(n,v);
        ll r1 = 0,r0 = 0;
        if(x2!=l2) r0 = 2;
        if(y2!=l2) r1 = 2;
        ans = min(u[0][0]+abs(x2-x[l2-2][0])+abs(y2-y[l2-2][0])+r0,u[1][0]+abs(x2-x[l2-2][1])+abs(y2-y[l2-2][1])+r1);
        cout << ans << "\n";
    }
