...
int main() {
    INT(n);
    vmint dp1(n + 1), dp2(n + 1);
    dp1[0] = 2, dp2[0] = 1;
    rep2(i, 1, n) {
        dp2[i] = (dp1[i - 1] - dp2[i - 1]) * dp2[i - 1] + dp2[i - 1] * (dp2[i - 1] + 1) / 2;
        dp1[i] = dp1[i - 1] + dp2[i];
    }

    mint ans, x = dp1[n - 1], y = dp2[n - 1];
    x -= y;
    ans += y * x * (x + 1) / 2;
    ans += y * (y + 1) / 2 * x;
    ans += y * (y + 1) * (y + 2) / 6;
    ...
    vmint dp3(n + 1), dpr(n + 1);
    dp3[1] = 1;
    rep2(i, 2, n) dp3[i] = dp2[i] - dp2[i - 1];
    dpr[1] = 1;
    rep2(i, 2, n) dpr[i] += dp3[i] + dpr[i - 1];

    rep2(i, 1, n - 1) { ans += dp3[i] * dpr[n - 1 - i]; }
    cout << ans << endl;
}