...
void build(int n, int p, int k) {
	...
	for (auto& l : p2) {
		...
		if (p2.count(r) == 0 && min(l, r) * 2 > max(l, r)) {
			build(l, now, 0);
			build(r, now, 1);
			return;
		}
		
		if (p2.count(r) == 1 && min(l, r) * 2 <= max(l, r)) {
			build(l, now, 0);
			build(r, now, 0);
			return;
		}
...
bool check(int n, int k) {
	...
	if (n == 1 && k != 0) return 0;
	if (n == 9 && k == 2) return 0;
	if (n >= 3 && k > (n - 3) / 2) return 0;
	if (p2.count(n) == 1 && k == 1) return 0;
	if (p2.count(n) == 0 && k == 0) return 0;
	return 1;
}
