...
int main()
{
    ...
    for (int len=1;len<=n;len++)
    {
        ...
        while (p<n)
        {
            ...
            {
                int M = (R+L)/2;
                if (pref1[M]-pref1[p]>=len or pref2[M]-pref2[p]>=len)
                {
                    R = M;
                } else L = M;
            }
            if (pref1[R]-pref1[p]<len and pref2[R]-pref2[p]<len) ok = 0;
            if (pref1[R]-pref1[p]>=len) cnt1++;
            else cnt2++;
            p = R;
        }
        if (ok==1 and cnt1>cnt2) ans.push_back({cnt1,len});
    }
