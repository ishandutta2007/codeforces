...
int main() {
	...
	cin.tie(0);
	defn
		vector<veci> a(n, veci(n));
	veci b(2 * n, -1);
	vector<veci> cons(2 * n);
	vector<veci> opcons(2*n);
	fori(i, n) {
		defs(line)
		fori(j, n) {
			a[i][j] = (line[j] - '0') ^ a[i][j];
		}
	}
	fori(i, n) {
		defs(line)
	...
	fori(i, n) {
		fori(j, n) {
			if (line[i] - '0' && line[j] - '0') {
				if (a[i][j]) {
					opcons[i].push_back(j + n);
					opcons[j+n].push_back(i);
				}
				else {
					cons[i].push_back(j + n);
					cons[j + n].push_back(i);
				}
			}
			else if (line[j] - '0') {
				if (b[i] != -1 && b[i] != a[i][j]) {
	...
	while (!left.empty() && ok) {
		...
		forin(i,opcons[cur]) {
			if (b[opcons[cur][i]] == -1) {
				b[opcons[cur][i]] = 1 ^ b[cur];
			}
			if (b[opcons[cur][i]] == b[cur]) {
				ok = false;
			}
			left.push_back(opcons[cur][i]);
		}
		forin(i, cons[cur]) {
			if (b[cons[cur][i]] == -1) {
				b[cons[cur][i]] = b[cur];
			}
			if (b[cons[cur][i]] != b[cur]) {
				ok = false;
			}
			left.push_back(cons[cur][i]);
		}
	}
	if (ok) {
		int sum = 0;
