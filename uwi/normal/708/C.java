...
public class C {
	...
	void solve()
	{
		...
		int[][] rs = makeRights(g, par, root);
		int[] ord = rs[0], iord = rs[1], right = rs[2];
		int[] des = new int[n];
		for(int i = 0;i < n;i++){
			des[i] = right[i] - i + 1;
		}
		SegmentTreeRMQPos st = new SegmentTreeRMQPos(des);
		for(int i = n-1;i >= 1;i--){
			int cur = ord[i];
			if(des[i] >= n/2+1){
				// par[cur] is yabai
				while(true){
					int min = st.minx(i, right[i]+1);
					if(min >= n/2+1){
						ok[par[cur]] = 0;
						break;
					}else if(des[i]-min >= n/2+1){
						st.update(st.minpos, 99999999);
					}else{
						break;
					}
				}
		...
		dfs(0, par, g, new int[n+3], n, des, iord, ok);
		dfs2(0, par, g, new int[n+3], n, des, iord, ok);
		for(int i = 0;i < n;i++){
			out.print(ok[i] + " ");
		}
		out.println();
	...
	void dfs(int cur, int[] par, int[][] g, int[] ft, int n, int[] des, int[] iord, int[] ok)
	{
		int up = n-des[iord[cur]];
		if(up >= n/2+1){
			if(sumFenwick(ft, n/2) - sumFenwick(ft, up-n/2-1) > 0){
				ok[cur] = 1;
			}
		}
		addFenwick(ft, up, 1);
		for(int i = 0;i < g[cur].length;i++){
			int nex = g[cur][i];
			if(nex == par[cur])continue;
			dfs(nex, par, g, ft, n, des, iord, ok);
		}
		addFenwick(ft, up, -1);
		addFenwick(ft, des[iord[cur]], 1);
	}
	
	void dfs2(int cur, int[] par, int[][] g, int[] ft, int n, int[] des, int[] iord, int[] ok)
	{
		int up = n-des[iord[cur]];
		if(up >= n/2+1){
			if(sumFenwick(ft, n/2) - sumFenwick(ft, up-n/2-1) > 0){
		...
		for(int i = g[cur].length-1;i >= 0;i--){
			int nex = g[cur][i];
			if(nex == par[cur])continue;
			dfs2(nex, par, g, ft, n, des, iord, ok);
		}
		addFenwick(ft, up, -1);
		addFenwick(ft, des[iord[cur]], 1);
	}
