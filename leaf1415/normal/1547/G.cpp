...
int main(void)
{
	...
	while(T--){
		cin >> n >> m;
		rep(i, 1, n) self[i] = selfC[i] = false;
		
		SCC scc(n); ll u, v;
		rep(i, 1, m){
			cin >> u >> v;
			if(u != v) scc.add_edge(u, v);
			else self[u] = true;
		}
		scc.tpsort();
		ll C = scc.calcSCC();
		...
		mp.clear();
		rep(i, 1, n) selfC[scc.scc[i]] |= self[i], mp[scc.scc[i]]++;
		...
		rep(i, 1, C){
			if(dp[i] == 0) continue;
			if(selfC[i] || mp[i] >= 2) dp[i] = M;
			for(auto u : scc.compG[i]){
				dp[u] += dp[i];
				if(dp[u] >= M) dp[u] = M;
				else if(dp[u] >= 2) dp[u] = 2;
			}
		}
