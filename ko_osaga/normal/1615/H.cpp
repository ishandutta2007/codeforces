...
void solve(int s, int e, vector<int> vtx, vector<pi> edg){
	if(s == e){
		for(auto &x : vtx){
			ans[x] = v[s];
		}
	...
	for(int i = 0; i < sz(vtx); i++){
		if(a[vtx[i]] <= v[m]) mf.addEdge(sz(vtx), i, 1);
		else mf.addEdge(i, sz(vtx) + 1, 1);
	}
	for(auto &[u, v] : edg){
		int nu = lower_bound(all(vtx), u) - vtx.begin();
		int nv = lower_bound(all(vtx), v) - vtx.begin();
		mf.addEdge(nv, nu, 1e9);
	}
	mf.flow(sz(vtx), sz(vtx) + 1);
	vector<int> l, r;
	vector<pi> le, re;
	for(int i = 0; i < sz(vtx); i++){
	...
	for(auto &[u, v] : edg){
		int nu = lower_bound(all(vtx), u) - vtx.begin();
		int nv = lower_bound(all(vtx), v) - vtx.begin();
		if(mf.cutSide(nu) != mf.cutSide(nv)) continue;
...
int main(){
	...
	v = a; sort(all(v)); v.resize(unique(all(v)) - v.begin());
	ans.resize(n);
	vector<int> vtx(n); iota(all(vtx), 0);
	solve(0, sz(v) - 1, vtx, b);
