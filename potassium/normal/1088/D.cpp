...
int main() {
	...
	for (int b = 29; b >= 0; b--) {
		...
		if (curMag == EQUAL) {
			int switchA = ask(preA | bit, preB);
			if (switchA == SMALLER) {
				preA |= bit;
				preB |= bit;
			}
		} else {
			int switchBoth = ask(preA | bit, preB | bit);
			if (curMag != switchBoth) {
				assert(curMag * switchBoth == -1);
				...
				curMag = ask(preA, preB);
			} else {
				int switchSmall = curMag == GREATER ? ask(preA, preB | bit) : ask(preA | bit, preB);
				bool signSwitch = curMag * switchSmall == -1;
