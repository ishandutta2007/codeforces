...
void solve(){
    ...
    for(int i = (int)zeros.size() - 1; i + max_size>= (int)zeros.size(); --i){
        b[zeros[i]] = -1;
    }
    ...
    gp_hash_table<int,int,custom_hash> left_col, right_col;
    for(int i = 0; i < n; ++i){
        if (b[i] == 0){
            continue;
        }
        ...
        if (l == 0){
            left_col[a[i]] = max(left_col[a[i]], r + 1);
        }
        else{
            right_col[a[i]] = max(right_col[a[i]], r - l + 1);
        }


    }

    assert(r < max_size && l == max_size);

    vector<array<int,3>> T;


    for(int i = 1; i <= n; ++i){
        auto it1 = left_col.find(i);
        auto it2 = right_col.find(i);

        if (it1 == left_col.end() && it2 == right_col.end()){
            continue;
        }
        int l = 0, r = 0;
        if (it1 != left_col.end()){
    ...
    assert(used.size() + oth.size() == T.size());

    int left = max_size - ans;


    while(!oth.empty() && left > 0){
        auto X = oth.back();
        oth.pop_back();
        if (X.first == 0){
            continue;
        }
        left = min(left, X.first);
        ++ans;
        --left;
    }
    cout << ans << endl;
