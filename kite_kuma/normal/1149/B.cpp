...
int main() {
	...
	V<vector<int>> query(3, V<int>(0));
	V<pair<int, int>> tup;

	vvi nxt(ch_size, vi(n + 1, inf));
	drep(i, n) {
		rep(j, ch_size) { chmin(nxt[j][i], nxt[j][i + 1]); }
		int now = s[i] - 'a';
		nxt[now][i] = i;
	}
	...
	rep(q) {
		...
		if(c == '+') {
			cin >> c;
			query[num].push_back(c - 'a');
			tup.emplace_back(num, c - 'a');
		} else {
			query[num].push_back(-1);
			tup.emplace_back(num, -1);
		}
	}

	vi sz(3);
	vi size_now(3, 0);
	rep(i, 3) sz[i] = query[i].size();

	VV<int> t(3);

	VVV<int> dp(251, vvi(251, vi(251, inf)));
	// (n+1
	...
	auto check = [&]() {
		debug(size_now);
		return dp[size_now[0]][size_now[1]][size_now[2]] <= n;
	};

	auto chng = [&](int i, int j, int k, int num) {
		debug(i, j, k, num);
		int pre = dp[i][j][k];
		if(pre > n - 1) return;
		int a, b, c;
		...
		if(num == 1) now = b++;
		if(num == 2) now = c++;
		if(size_now[0] < a || size_now[1] < b || size_now[2] < c) return;
		int ch = t[num][now];
		auto nx = nxt[ch][pre];
		chmin(dp[a][b][c], nx + 1);
		return;
	...
	for(auto [num, ch] : tup) {
		...
		if(ch == -1) {
			t[num].pop_back();
			if(num == 0) {
				rep(i, h) rep(j, h) { dp[size_now[0]][i][j] = inf; }
			} else if(num == 1) {
				rep(i, h) rep(j, h) { dp[i][size_now[1]][j] = inf; }
			} else {
				rep(i, h) rep(j, h) { dp[i][j][size_now[2]] = inf; }
			}
			size_now[num]--;
		} else {
			t[num].push_back(ch);
			...
			if(num == 0) {
				rep(i, h) rep(j, h) {
					int pre = dp[size_now[0] - 1][i][j];
					int &now = dp[size_now[0]][i][j];
					if(pre < n) chmin(now, nxt[ch][pre] + 1);
					rep(k, 3) if(num != k) chng(size_now[0], i, j, k);
				}
			} else if(num == 1) {
				rep(i, h) rep(j, h) {
					int pre = dp[i][size_now[1] - 1][j];
					if(pre < n) chmin(dp[i][size_now[1]][j], nxt[ch][pre] + 1);
					rep(k, 3) if(num != k) chng(i, size_now[1], j, k);
				}
			} else {
				rep(i, h) rep(j, h) {
					// dp[i][j][size_now[2]] = inf;
					int pre = dp[i][j][size_now[2] - 1];
					if(pre < n) chmin(dp[i][j][size_now[2]], nxt[ch][pre] + 1);
					rep(k, 3) if(num != k) chng(i, j, size_now[2], k);
				}
			}
		}
		ans.push_back(check());
