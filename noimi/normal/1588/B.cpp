...
int main() {
    TEST {
        ...
        auto is_tri = [&](ll x) {
            if(!x) return 1LL;
            ll k = sqrt(2 * x);
            for(auto i : {k - 1, k, k + 1, k + 2}) {
                if(i * (i - 1) / 2 == x) return i;
            }
            return -1LL;
        };

        auto ans = [&](ll x, ll y, ll z) {
            OUT("!", x + 1, y + 1, z);
            cout.flush();
        };
        ...
        while(1) {
            ...
            if(!L) {
                ...
                ll R = ask(mid, r);
                ll pre = s;
                ll lt = is_tri(L), rt = is_tri(R);
                if(lt != -1) {
                    if((pre - L - R) % lt == 0) {
                        ll a = (pre - L - R) / lt;
                        ll b = is_tri(R - a * (a - 1) / 2);
                        if(b != -1) {
                            ll i = mid - lt;
                            ll j = mid + a;
                            ll k = mid + a + b;
                            if(ask(i, j) == (lt + a) * (lt + a - 1) / 2 and ask(j, k) == b * (b - 1) / 2) {
                                ans(i, j, k);
                                break;
                            }
                        }
                ...
                if(rt != -1) {
                    if((pre - L - R) % rt == 0) {
                        ll a = (pre - L - R) / rt;
                        ll b = is_tri(L - a * (a - 1) / 2);
                        if(b != -1) {
                            ll i = mid - a - b;
                            ll j = mid - a;
                            ll k = mid + rt;
                            if(ask(i, j) == b * (b - 1) / 2 and ask(j, k) == (a + rt) * (a + rt - 1) / 2) {
                                ans(i, j, k);
                                break;
                            }
                        }
