...
const int MB = 62, INF = 1e9, NEED = 2;

int main() {
    setIO();

    int N; re(N); vll v(N); re(v);

    vvi have(MB);
    F0R (i, N) {
        F0R (a, MB) if (v[i] & (1ll << a)) {
            if (sz(have[a]) < NEED) have[a].pb(i);
            else { ps(3); return 0; }
        }
    }

    vpii ed;
    F0R (i, MB) if (sz(have[i]) == 2)
        ed.eb(have[i][0], have[i][1]);

    int ans = INF;

    trav (ea, ed) {
        map<int, int> dist;
        trav (e, ed) dist[e.f] = dist[e.s] = INF;

        dist[ea.f] = 0;
        F0R (rep, sz(ed)) {
            trav (eb, ed) if (eb != ea) {
                ckmin(dist[eb.s], dist[eb.f] + 1);
                ckmin(dist[eb.f], dist[eb.s] + 1);
            }
        }
        ckmin(ans, 1 + dist[ea.s]);
    }
