...
//map add
template <typename T>
void add(map<T,int> &cnt,T a,int n = 1){
...
int main(){
    ...
    for(int k=(-1)*s;k<=s;k++){
        ...
        rep(i,N){
            cnt[(int)4e7+a[i]+i*k]++;
            chmax(ans,cnt[(int)4e7+a[i]+i*k]);
            app.push_back((int)4e7+a[i]+i*k);
        }
        for(auto x:app) cnt[x]--;
    }
    rep(i,N){
        vector<int> app;
        rep(j,s){
            ...
            if(j==0) continue;
            if((a[i+j]-a[i])%j==0){
                cnt[(int)4e7+(a[i+j]-a[i])/j]++;
                chmax(ans,cnt[(int)4e7+(a[i+j]-a[i])/j]+1);
                app.push_back((int)4e7+(a[i+j]-a[i])/j);
            }
        }
        for(auto x:app) cnt[x]--;
    }
