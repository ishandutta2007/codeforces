...
using T = ll;
vector< T > shortest_path_faster_algorithm(WeightedGraph &g, int s) {
...
int main(){
    ...
    each(a,b,c,edge){
        ...
        if(c==0){
            ...
            g.add_directed_edge(b,a,-1);
        }
    }
    if(shortest_path_faster_algorithm(g,0).empty())return NO();
    ll ans=-1;
    vector<ll>d;
    rep(n){
        auto cost=shortest_path_faster_algorithm(g,i);
        if(all_of(all(edge),[&](const tuplis& e){
            const auto[a,b,c]=e;
            if(c==0)return abs(cost[a]-cost[b])==1;
            else return cost[b]==cost[a]+1;
        })){
            if(chmax(ans,max(cost)))d=cost;
        }
    }
    if(ans==-1)return NO();
    YES();
