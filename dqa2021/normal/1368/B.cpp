...
ll k;
void check(int x){
	int a=x/10,b=a+1,btot=x%10,atot=10-btot;
//	printf("check %d: %d %d  %d %d\n",x,a,atot,b,btot);
	ll tot=1,INF=1e18;
	rep(i,1,atot){
		if (tot<=INF/(a+1)) tot*=(a+1);
		else{tot=INF; break;}
	}
	rep(i,1,btot){
		if (tot<=INF/(b+1)) tot*=(b+1);
		else{tot=INF; break;}
	}
	if (tot<k) return;
	rep(i,0,9){
		if (atot){
			atot--;
			for (int x=a+1;x--;) putchar("codeforces"[i]);
		}
		else{
			btot--;
			for (int x=b+1;x--;) putchar("codeforces"[i]);
		}
	}
	exit(0);
}
