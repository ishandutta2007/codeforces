...
void solve() {
  ...
  const ll INF = 1LL << 60;
  FOR(i, N) {
    ll n = C[i], cost = COST[i];
    FOR(i, LIM + 1) if (DP[i] >= 0) DP[i] = min(DP[i] + X, W + B * i);
    SWAG<Monoid_Max<ll, INF>> swag;
    FOR(j, LIM + 1) {
      swag.push(DP[j] + cost * j);
      if (len(swag) > n + 1) swag.pop();
      ll x = swag.prod() - cost * j;
      DP[j] = x;
    }
  }
  ll ANS = 0;
  FOR(i, LIM + 1) if (DP[i] >= 0) ANS = i;
  print(ANS);
}
