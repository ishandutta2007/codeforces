...
void work()
{
    ...
    for(int i = n - 1; i; i--) s[i] = a[i] - a[i + 1];

//    for(int i = 1; i <= n - 1; i++) cout <<s[i] <<" "; cout <<endl;

    LL tot = 0;
    for(int i = 1; i <= n - 1; i++) tot += get_abs(s[i]);

    LL ans = tot;
    for(int i = 1; i <= n; i++)
    {
        if(i == 1) get_min(ans, tot - get_abs(s[1]));
        if(i == n) get_min(ans, tot - get_abs(s[n - 1]));

        if(i != 1 && i != n)
        {
            get_min(ans, tot - (get_abs(s[i - 1]) + get_abs(s[i]) - get_abs(s[i - 1] + s[i])));
        }
    }
    cout <<ans <<endl;
}
