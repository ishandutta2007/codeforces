...
sparse<int,maxn,0> sp1;
sparse<int,maxn,1> sp2;
...
void orzck(){
    ...
    rep(n) cin >> b[i];
    sum[0]=0;
    rep(n) sum[i+1]=sum[i]+(a[i]-b[i]),bug(sum[i+1]);
    rep(n+1) if(sum[i]>0) vec.pb(i),bug(i);
    vector<int> vec1;
    rep(n+1) vec1.pb(sum[i]);
    sp1.init(vec1),sp2.init(vec1);
    while(q--){
        int l,r;
        cin >> l >> r;
        if(sum[r]-sum[l-1]||sp2.query(l,r)>sum[l-1]){
            print(-1);
            continue;
        }
        print(sum[l-1]-sp1.query(l,r));
    }
}
