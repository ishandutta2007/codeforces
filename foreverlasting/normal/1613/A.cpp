...
namespace MAIN{
    inline void MAIN(){
        long double x1=read(),p1=read();
        long double x2=read(),p2=read();
        long double p=min(p1,p2);
        p1-=p,p2-=p;
        if(p1==0){
            if(p2>10){puts("<");return;}
            for(res i=1;i<=p2;i++){
                x1/=10;
            }
            x1/=x2;
            if(fabs(x1-1)<=1e-10)puts("=");
            else if(x1>1)puts(">");
            else puts("<");
        }
        else {
            if(p1>10){puts(">");return;}
            for(res i=1;i<=p1;i++){
                x2/=10;
            }
            x2/=x1;
            if(fabs(x2-1)<=1e-10)puts("=");
            else if(x2>1)puts("<");
            else puts(">");
        }
    }
