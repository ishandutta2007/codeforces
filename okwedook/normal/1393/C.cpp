...
struct fastSet {
    ...
    fastSet(int _n, int _C) : n(_n), C(_C), mx(C - 1) {
        pos.resize(n);
        val.resize(n, -1);
        ind.resize(C);
    }
    fastSet(const vector<int> &a) : n(sz(a)), C(*max_element(all(a)) + 1), mx(C - 1) {
        pos.resize(n);
        val.resize(n, -1);
        ind.resize(C);
...
void solve() {
    ...
    while (r - l > 1) {
        ...
        bool flag = true;
        for (int i = 0; i < n; ++i) {
            if (i - m >= 0) st.upd(ans[i - m], cnt[ans[i - m]]);
            int best = st.getmx();
            if (cnt[best] == 0) {
                flag = false;
                break;
