...
signed main(){
    ...
    rep(i,m){
        bool f=0;
        rep2(j,pl,n-1){
            if((1LL<<pos) & a[j]){ swap(a[pl],a[j]);break;}
            if(j==n-1) f = 1;
        }
        if(f){
            pos++;
    ...
    vmint ans(m+1);
    ans[0]=modpow(2,n-B);
    if(B==0) {print(ans);return 0;}
    UnionFind uf(B);
    rep(i,B)rep(j,B)if(a[i]&a[j])uf.unite(i,j);
    vector<vec> w(B);
    rep(i,B)w[uf.find(i)].pb(i);
    for(auto v:w){
        int k = v.size();
        if(k==0)continue;
        if(k<=20){
            ...
            rep(i,1<<k){
                ll now = 0;
                rep(j,k)if(i&(1LL<<j))now^=a[v[j]];
                cnt[popcount(i)+popcount(now)]+=1;
            }
            rep3(i,m,1){
                mint mem=ans[i];
                ans[i]=0;
                rep(j,i)ans[i]+=ans[j]*cnt[i-j];
                ans[i]+=mem*cnt[0];
            }
        }
        else{
            ...
            rep(j,sz){
                ll cnt=0;
                rep(i,k)cnt+=(1LL<<j)&a[v[i]];
                if(!cnt){
                    rep(i,k)remove(a[v[i]],j);
                    j--;
                    sz--;
                }
            }
            auto dp = vector<vmint>(1<<sz,vmint(k+1));
            dp[0][0]=1;
            rep(i,k){
                auto nxt = vector<vmint>(1<<sz,vmint(k+1));
                rep(j,1<<sz){
                    rep3(jj,k,1){
                        nxt[j^a[v[i]]][jj]+=dp[j][jj-1];
                    }
                }
                rep(j,1<<sz)rep(jj,k+1)dp[j][jj]+=nxt[j][jj];
            }
            vmint cnt(m+1);
            rep(i,k+1)rep(j,1<<sz)cnt[popcount(j)+i]+=dp[j][i];
            rep3(i,m,1){
                mint mem=ans[i];
                ans[i]=0;
                rep(j,i)ans[i]+=ans[j]*cnt[i-j];
