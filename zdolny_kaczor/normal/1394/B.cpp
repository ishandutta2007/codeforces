...
#endif
//#STAY AT HOME
const int nax = 2e5 + 44, kax = 12;
vpii graf[nax];
vpii graft[nax];
int banned[kax][kax];
void ban(pii x) {
	banned[x.first][x.second] = 1;
}
bool is_banned(pii x) {
	return banned[x.first][x.second];
}
vpii ban_oth[kax][kax];
int n, m, k;
int ans = 0;
loc(vi per);
void run(int x) {
	debug(imie(x));
	if (x == k + 1) {
		debug(imie(per), "is fine");
		ans++;
		return;
	}
	for (int c = 0; c < x; ++c) {
		if (banned[x][c] == 0) {
			for (auto [a, b] : ban_oth[x][c]) banned[a][b]++;
			loc(per.push_back(c));
			run(x + 1);
			for (auto [a, b] : ban_oth[x][c]) banned[a][b]--;
			loc(per.pop_back());
...
void solve() {
	...
	for (int i = 1; i <= n; ++i) {
		...
		REP(j, s) {
			int to = graf[i][j].second;
			graft[to].emplace_back(s, j);
		}
	}
	for (int i = 1; i <= n; ++i) {
		debug(arr(graft, i));
		...
		int s = graft[i].size();
		REP(j, s - 1)
			if (graft[i][j] == graft[i][j + 1]) ban(graft[i][j]);
	}
	for (int i = 1; i <= n; ++i) {
		vpii cleared;
		for (auto x : graft[i]) if (!is_banned(x)) cleared.push_back(x);
		int l = cleared.size();
		debug(imie(i), imie(cleared));
		assert(l <= k * (k + 1) / 2);
		REP(x, l) REP(y, l) {
			auto p1 = cleared[x], p2 = cleared[y];
			if (p1.first < p2.first)
				ban_oth[p1.first][p1.second].push_back(p2);
		}
	}
	REP(i, kax) REP(j, kax) {
		sort(ALL(ban_oth[i][j]));
		unq(ban_oth[i][j]);
		if (!ban_oth[i][j].empty()) debug(arr(ban_oth, i, j));
	}
	run(1);
	printf("%d\n", ans);
}
