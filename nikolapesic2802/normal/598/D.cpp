...
void dfs(int x,int y,int arg=-1){
	if(arg==-1){
		cnt[x][y]=0;
		for(int k=0;k<4;k++){
			...
			if(!inside(xx,yy))
				...
				if(cnt[xx][yy]==-1)
					dfs(xx,yy,arg),cnt[x][y]+=cnt[xx][yy];
		}
	}
	else{
		if(cnt[x][y]==arg)
...
int main()
{
	...
	for(int i=0;i<n;i++)
		cin>>mat[i];
	for(int i=0;i<n;i++)for(int j=0;j<m;j++)if(mat[i][j]=='.'&&cnt[i][j]==-1)dfs(i,j,-1),dfs(i,j,cnt[i][j]+1);
	for(int i=0;i<q;i++)rd(a,b),printf("%i\n",cnt[a-1][b-1]-1);
	return 0;
}