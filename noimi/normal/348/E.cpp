...
template <typename G> struct HLDecomposition {
    ...
    HLDecomposition(G &g) : g(g), sz(g.size()), in(g.size()), out(g.size()), head(g.size()), rev(g.size()), par(g.size()), d(g.size()) {}
...
int main() {
    ...
    struct S {
        int flag, d, to, cnt;
        S(int flag, int d, int to, int cnt) : flag(flag), d(d), to(to), cnt(cnt) {}
        S() = default;
    };

    auto f = [&](S s, ReRooting<int, S>::Edge x) -> S {
        if(!s.flag) {
            if(a[x.to]) return S(true, x.cost, x.to, 1);
            return S(false, 0, 0, 1);
        }
        return S(true, s.d + x.cost, (s.cnt > 1 ? x.to : s.to), 1);
    };
    ...
    auto gg = [](S s, S t) {
        ...
        if(s.d < t.d)
            ...
            return S(true, s.d, 0, 2);
    };

    ReRooting<int, S> R(n, f, gg, S(false, 0, 0, 0));
    ...
    rep(i, n) {
        if(a[i]) {
            if(dp[i].cnt > 1) dp[i].to = i;
            int l = hld.lca(i, dp[i].to);
            sub[l]--;
            if(l) sub[hld.la(l, 1)]--;
            sub[i]++, sub[dp[i].to]++;
        }
