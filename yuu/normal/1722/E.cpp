...
int64_t ft[1001][1001];
void update(int u, int v_, int d) {
    for (; u <= 1000; u += u & -u) {
        for (int v = v_; v <= 1000; v += v & -v) {
            ft[u][v] += d;
        }
    }
}

int64_t get(int u, int v_) {
    int64_t res = 0;
    for (; u > 0; u -= u & -u)
        for (int v = v_; v > 0; v -= v & -v) res += ft[u][v];
    return res;
}
...
void solve() {
    ...
    for (int i = 1; i <= n; i++) update(h[i], w[i], h[i] * w[i]);
    for (int i = 1, l, u, r, v; i <= q; i++) {
        cin >> l >> u >> r >> v;
        r--;
        v--;
        cout << get(r, v) - get(r, u) - get(l, v) + get(l, u) << '\n';
    }
    for (int i = 1; i <= n; i++) update(h[i], w[i], -h[i] * w[i]);
}
