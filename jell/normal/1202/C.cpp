...
void solve() {
    ...
    int uc=0;
    int dc=0;
    int ltt=0,rtt=0,utt=0,dtt=0;
    int lt=0,rt=0,ut=0,dt=0;
    int i=0;
    for(char c:s) {
        if(c=='W') {
            --x;
    ...
    {
        //nothing
        ans=min(ans,(l+1+r)*(u+1+d));
        //U
        if(d+u>1) {
            if(utt>dt) ans=min(ans,(l+r+1)*(u+d));
        }
        //D
        if(d+u>1) {
            if(dtt>ut) ans=min(ans,(l+1+r)*(u+d));
        }
        //L
        if(l+r>1) {
            if(ltt>rt) ans=min(ans,(l+r)*(u+d+1));
        }
        //R
        if(l+r>1) {

            if(rtt>lt) ans=min(ans,(l+r)*(u+d+1));
        }
    }
    cout<<ans<<"\n";
}