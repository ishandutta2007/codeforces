...
int main() {
    ...
    int last[3], curr = 0;
    last[0] = -1;
    for (int i = 1; i < 3; ++i) last[i] = -2;
    for (int i = 0; i < n; ++i) {
        curr = (curr + s[i]) % 3;
        checkmax(ans[i], (i > 0 ? ans[i - 1] : 0));
        checkmax(ans[i], (last[curr] < 0 ? last[curr] + 1 : ans[last[curr]]) + 1);
        //cout << (last[curr] < 0 ? last[curr] + 1 : ans[last[curr]]) + 1 << endl;
        last[curr] = i;
    }
    cout << ans[n - 1];
    return 0;
