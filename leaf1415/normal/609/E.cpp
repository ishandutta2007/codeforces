...
struct LCA{
	...
	LCA(){}
	void predfs(vector<edge> G[], int v, int p, int d, ll w)
	{
		depth[v] = d, prev[v][0] = p, mini[v][0] = w;
		for(auto e : G[v]){
			if(e.to == p) continue;
			predfs(G, e.to, v, d+1, e.cost);
		}
	}
	void init(vector<edge> G[], int n, int root = 1)  //1-index
	{
		this->n = n;
		for(int i = 0; ; i++){
			if(n <= (1<<i)){
		...
		mini.resize(n+1);
		for(int i = 0; i < mini.size(); i++) mini[i].resize(logn);
		depth.resize(n+1);
		
		predfs(G, root, 0, 0, -inf);
		...
		for(int i = 1; i < logn; i++){
			for(int j = 0; j <= n; j++){
				mini[j][i] = max(mini[j][i-1], mini[prev[j][i-1]][i-1]);
			}
		}
	}
	...
	{
		int lca = getLCA(u, v);
		return max(get(u, depth[u]-depth[lca]), get(v, depth[v]-depth[lca]));
	}
};
...
int main(void)
{
	...
	for(auto p : evec){
		...
		uf.unite(u[id], v[id]);
		G[u[id]].push_back(edge(v[id], w[id]));
		G[v[id]].push_back(edge(u[id], w[id]));
		ans += w[id];
		S.insert(id);
	}
	...
	rep(i, 1, m){
		...
		cout << ans - lca.getMin(u[i], v[i]) + w[i] << "\n";
	}
