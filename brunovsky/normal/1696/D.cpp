...
struct Segnode {
    ...
    static constexpr int MAX = INT_MAX / 2;
    int value = MAX;

    Segnode(int value = MAX) : value(value) {}

    void pushup(const Segnode& lhs, const Segnode& rhs) {
        value = min(lhs.value, rhs.value);
    }

    void apply(int val) { value = val; }
};
...
int main() {
    ...
    while (T--) {
        ...
        vector<int> mins = {0};
        vector<int> maxs = {0};
        constexpr int inf = Segnode::MAX;
        segtree<Segnode> minseg(N, inf), maxseg(N, inf);
        minseg.update_point(0, 0);
        maxseg.update_point(0, 0);
        ...
        for (int i = 1; i < N; i++) {
            if (a[i - 1] < a[i]) {
                while (maxs.size() && a[maxs.back()] < a[i]) {
                    int j = maxs.back();
                    maxs.pop_back();
                    maxseg.update_point(j, inf);
                }
                int j = maxs.size() ? maxs.back() + 1 : 0;
                int ans = 1 + minseg.query_range(j, i).value;
                maxseg.update_point(i, ans);
                minseg.update_point(i, ans);
            } else {
                while (mins.size() && a[mins.back()] > a[i]) {
                    int j = mins.back();
                    mins.pop_back();
                    minseg.update_point(j, inf);
                }
                int j = mins.size() ? mins.back() + 1 : 0;
                int ans = 1 + maxseg.query_range(j, i).value;
                maxseg.update_point(i, ans);
                minseg.update_point(i, ans);
            }
            maxs.push_back(i);
        ...
        cout << maxseg.query_point(N - 1).value << '\n';
    }
    return 0;
}