...
struct Zn {
  ...
  Zn& operator*=(Zn const& o) {
    value = ((li)value*(li)o.value)%1'000'000'007;
    return *this;
  }
...
template<class F>
void iterSubmasks(int mask, F&& f) {
...
map<int,ZN> memo[1<<N];
ZN solve(int curMask, int remMask) {
  if(curMask == 0) return 0;
  if(remMask == 0) return 1;

  auto it = memo[remMask].find(curMask);
  if(it != memo[remMask].end()) return it->second;

  ZN lans = 0;

  iterSubmasks(remMask, [&](int take){
    int skip = remMask ^ take;
    ZN p = solve(take, skip);
    for(li i = 0; i < (li)(n); ++i) {
      if(take&(1<<i)) {
        ZN cur = 0;
        ZN mult = 1;
        for(li j = 0; j < (li)(n); ++j) if(curMask&(1<<j)) {
          cur += mult * P[j][i];
          mult *= P[i][j];
        }
        p *= cur;
      ...
      if(skip&(1<<i)) {
        for(li j = 0; j < (li)(n); ++j) if(curMask&(1<<j)) p *= P[i][j];
      }
    }
    lans += p;
  });

  return memo[remMask][curMask] = lans;
}

ZN dp[N][N][1<<N];
ZN pre[1<<N][N];
vector<tuple<int,ZN>> pre2[1<<N];

void calcPre() {
  for(li mask = 0; mask < (li)(1<<n); ++mask) for(li i = 0; i < (li)(n); ++i) {
    pre[mask][i] = 1;
    for(li j = 0; j < (li)(n); ++j) if(mask&(1<<j)) {
      pre[mask][i] *= P[j][i];
    }
  }

  for(li mask = 0; mask < (li)(1<<n); ++mask) iterSubmasks(mask, [&](int take){
    int skip = mask^take;
    ZN p = 1;
    for(li j = 0; j < (li)(n); ++j) if(take&(1<<j)) p *= pre[skip][j];
    pre2[mask].emplace_back(take,p);
  });
}

ZN calc(int i, int j, int mask) {
  if(mask == 0) return 1;
  if(j == n) return 0;

  if(dp[i][j][mask].value != -1) return dp[i][j][mask];

  if(mask&(1<<j)) {
    ZN lans = 0;
    lans += P[j][i] * calc(i,j+1,mask);
    for(auto const& takeP : pre2[mask^(1<<j)]) {
      int take; ZN p; tie(take,p) = takeP;
      int skip = mask^(1<<j)^take;
      p *= P[i][j] * pre[skip][j];
      lans += p * calc(j,0,take) * calc(i,j+1,skip);
    }
    return dp[i][j][mask] = lans;
  } else {
    return dp[i][j][mask] = calc(i,j+1,mask);
  }
}
...
calcPre();
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) for(li mask = 0; mask < (li)(1<<n); ++mask) dp[i][j][mask].value = -1;
  ZN ans = 0;
  for(li i = 0; i < (li)(n); ++i) {
    ans += calc(i,0,((1<<n)-1)^(1<<i));
  }
  cout << ans << endl;
