...
void solve() {
    ...
    auto d = make_vector<llong>(n, m, 2);
    d[0][0][0] = 1;

    vector<vector<int>> rt(n);
    for (int i = 0; i < n; i++) {
        for (int j = m - 1; j >= 0; j--) {
            if (a[i][j] == 'R') {
                rt[i].push_back(j);
    ...
    auto ls = make_vector<llong>(n, m);
    auto us = make_vector<llong>(n, m);
    ...
    for (int i = 1; i < n; i++) {
        for (int j = 1; j < m; j++) {
            if (j + isz(rt[i]) < m) {
                add(d[i][j][L], ls[i][j - 1]);
            } else {
                int x = m - j - 1;
                int z = rt[i][x];
                add(d[i][j][L], ls[i][j - 1] - (z ? ls[i][z - 1] : 0));
            }

            if (i + isz(dn[j]) < n) {
                add(d[i][j][U], us[i - 1][j]);
            } else {
                int x = n - i - 1;
                int z = dn[j][x];
                add(d[i][j][U], us[i - 1][j] - (z ? us[z - 1][j] : 0));
            }

            add(ls[i][j], d[i][j][U] + ls[i][j - 1]);
            add(us[i][j], d[i][j][L] + us[i - 1][j]);
        }
    }
    
    llong ans = (d[n - 1][m - 1][L] + d[n - 1][m - 1][U]) % M;
    ans = (ans + M) % M;
    cout << ans << "\n";
}//vfhcly