...
// cost of k paths and the next good path
pair<ll, ll> solve(int v, int k) {
    int sons = sz(tree[v]);
    dbg(v, k);
    if (sons == 0) {
        msg("LIST ", v, ' ', s[v], ' ', k);
    ...
    vector<ll> additional_paths;
    ll ans = s[v] * k;
    for (auto i : tree[v]) {
        auto add = solve(i, k / sons);
        ans += add.f;
        additional_paths.pb(add.s);
    }
    rsort(additional_paths);
    for (int i = 0; i < k % sons; ++i) {
        ans += additional_paths[i];
    }
    return {ans, additional_paths[k % sons] + s[v]};
}
