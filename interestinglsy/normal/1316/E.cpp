...
ll Solve(){
	static ll dp[2][MAXMASK];
	Msn(dp,-INF);
	int cur = 0;

	{
		ll tsum = 0;
		For(i,aud) tsum += ps[i].a;
		dp[cur][0] = tsum;
	}
	...
	For(i,n){
		int last = cur; cur ^= 1;
		auto udp = dp[last], vdp = dp[cur];
		memcpy(dp[cur],dp[last],sizeof dp[cur]);
		For0(mask,1<<k){
			int pcnt = popCnt[mask];
			int preReserve = aud+pcnt;
			For0(j,k){
				if(mask>>j&1) continue;
				int vMask = mask | 1<<j;
				ll profit = ps[i].s[j] + (i<=preReserve?-ps[i].a+ps[preReserve+1].a:0);
				Mymax(vdp[vMask],udp[mask]+profit);
			}
		}
		Mymax(ans,vdp[(1<<k)-1]);
	}
...
int main(){
	...
	Read(n,k,aud);
	For(i,n) Read(ps[i].a);
	For(i,n) For0(j,k) Read(ps[i].s[j]);

	sort(ps+1,ps+1+n,[]( const Player &a , const Player &b ){
		return a.a > b.a;
	});
