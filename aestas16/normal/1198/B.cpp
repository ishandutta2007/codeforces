...
const int N = 2e5, K = 450;

int n, q, blo, a[N + 10], id[N + 10], ctag[K + 10];

void modify1(int pos, int x) {
    int l = (id[pos] - 1) * blo + 1, r = cmin(n, l + blo - 1);
    for (int i = l; i <= r; i++) a[i] = cmax(a[i], ctag[id[i]]);
    ctag[id[pos]] = -1, a[pos] = x;
}
void modify2(int x) {
    for (int i = 1; i <= id[n]; i++) ctag[i] = cmax(ctag[i], x);
}
...
signed main() {
    ...
    cin >> n, blo = sqrt(n);
    for (int i = 1; i <= n; i++) cin >> a[i], id[i] = (i - 1) / blo + 1, ctag[id[i]] = -1;
    for (cin >> q; q; q--) {
        int opt, p, x;
        cin >> opt >> x;
        if (opt == 1) {
    ...
    for (int i = 1; i <= n; i++) cout << cmax(a[i], ctag[id[i]]) << ' ';
    return cout << endl, 0;
}