...
vector<string>ngs={"0011", "0101", "1110",  "1111"};
vi calc(string s){
    ll n=s.size();
    vi ret(n+1);
    ret[0]=1;
...
signed main(){fastio
    ...
    vi ans(n);
    rep(i,0,n-1){
        vi dp=calc(s.substr(i,n-i));
        ll mx=0;
        rep(j,0,i-1){
            chmax(mx,rh.getLCP(i,j));
        }
        rep(j,mx+1,(int)dp.size()-1){
            ans[i+j-1]+=dp[j];
        }
       // for(auto x:dp)cout<<x.val()<<" ";cout<<endl;
    }
