...
ll zeroone(ll a){if(!a)return 0;if(a>0)return 1;return -1;}
signed main(){
    vv(bool,loot,1001,1001);
    pll a,b,c;
    in(a,b,c);
    bool pass=0;
    ll x=a.first,y=a.second;
    loot[x][y]=1;
    loot[b.first][b.second]=1;
    while(true){
        if(x==b.first){
            y+=zeroone(b.second-y);
        }
        elif(y==b.second){
            x+=zeroone(b.first-x);
        }
        elif(zeroone(b.first-x)==zeroone(c.first-x)){
            x+=zeroone(b.first-x);
        }
        else{
            y+=zeroone(b.second-y);
    ...
    if(!pass){
        while(true){
            ...
            elif(zeroone(b.first-x)==zeroone(a.first-x)){
                x+=zeroone(b.first-x);
            }
            else{
                y+=zeroone(b.second-y);
    ...
    ll ans=0;
    each(i,loot)each(j,i)ans+=j;
    out(ans);
    rep(1001)rep(j,1001)if(loot[i][j])out(i,j);
}