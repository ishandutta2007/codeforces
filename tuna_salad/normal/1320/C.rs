...
mod i1 {
	...
	impl LSTNode for I {
		...
		fn combine_values(a: &V, b: &V) -> V {
			let s = a.0 + b.0;
			let m = a.1.max(a.0 + b.1);
			(s,m)
		}
		fn reverse_value(_res: &V) -> V {
			unimplemented!()
		}
		fn update_value_from_lazy(n: &mut N<Self>) {
			if let Some(l) = n.l {
				n.v.0 += l * (n.len() as i64);
				n.v.1 += l;
			}
		}
...
fn main() {
	...
	l!(an,dn,mn = rin.u());
	let mut f = |n: usize| -> Vec<(i64,i64)> {
		let mut a = v!([n+1]);
		rp!{[n]
			l!(b,c = rin.l());
			a.push( (b,-c) );
		}
		a.push( (1000001,-2000000000) );
		a.sort();
		let mut res = v!([n+1]);
		for (b,c) in a {
			while let Some((x,y)) = res.pop() {
				if y > c {
					res.push((x,y));
					break;
				}
	...
	for (_,x,mut z) in s {
		if x < 0 {
			max!(ans, st.query(0,dn-1) + z);
		} else {
			let p = *td.range(x+1..).next().unwrap().1;
			st.update(p, p, &mut z, true);
		}
	}
