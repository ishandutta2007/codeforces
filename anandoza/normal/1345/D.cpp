...
template <typename T> vector<vector<T>> transpose(vector<vector<T>> v) {
    const int n = sz(v), m = sz(v[0]);
...
int main() {
    ...
    if (!valid(black) || !valid(transpose(black))) {
        ps(-1);
        return 0;
    }

    vector<vb> south(n, vb(m));
    vb row(n), col(m);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (black[i][j])
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            ...
            if (i + 1 < n && black[i + 1][j])
                uf.unio(index(i, j), index(i + 1, j));
            if (j + 1 < m && black[i][j + 1])
                uf.unio(index(i, j), index(i, j + 1));
        }
    }
    for (int i = 0; i < sz(uf.uf); i++)
        ans += (i == uf.rep(i));
