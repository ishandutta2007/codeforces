...
public class Main {
    ...
    static class TaskE {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            if (ArrayUtils.sumArray(s) != ArrayUtils.sumArray(t)) {
                out.printLine("NO");
                return;
            }
            int[] id = ArrayUtils.createOrder(n);
            ArrayUtils.orderBy(s, id);
            ArrayUtils.sort(t);
            IntList pRight = new IntArrayList();
            for (int i = 0; i < n; i++) {
                if (s[i] < t[i]) {
                    pRight.add(i);
                }
            }
            List<TaskE.Move> answer = new ArrayList<>();
            int at = pRight.size() - 1;
            for (int i = n - 1; i >= 0; i--) {
                while (s[i] > t[i]) {
                    int j = pRight.get(at);
                    if (j > i) {
                    ...
                    int delta = Math.min(s[i] - t[i], t[j] - s[j]);
                    s[i] -= delta;
                    s[j] += delta;
                    if (s[j] == t[j]) {
                        at--;
                    }
                    answer.add(new TaskE.Move(id[j] + 1, id[i] + 1, delta));
                }
            }
            out.printLine("YES");
            out.printLine(answer.size());
            for (TaskE.Move move : answer) {
                out.printLine(move.left, move.right, move.dist);
            }
        }
        ...
        static class Move {
            ...
            public Move(int left, int right, int dist) {
                this.left = left;
                this.right = right;
                this.dist = dist;
            }
