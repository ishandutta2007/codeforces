...
int main(){
  ...
  auto add=[&](ll l,ll r,int x){
    cnt++;
    ret+=l*(l+1)/2+(n-r)*(n-r+1)/2;
    //cout<<"add" spa l spa r spa x spa ret<<endl;
  };
  auto del=[&](ll l,ll r,int x){
    //cout<<"del" spa l spa r spa x spa ret<<endl;
    cnt--;
    ret-=l*(l+1)/2+(n-r)*(n-r+1)/2;
  };
  IntervalManager<int,int>seg;
  rep(i,0,n){
    seg.update(i,i+1,a[i],add,del);
  }
  while(m--){
    ll i,x;cin>>i>>x;i--;
    a[i]=x;
    seg.update(i,i+1,x,add,del);
    //debug(a);
    cnt=seg.s.size();
    cout<<n*(n+1)/2*cnt-ret<<endl;
    //cout<<cnt spa ret<<endl;
    //seg.show();
  }
  return 0;
}