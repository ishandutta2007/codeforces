...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/ds/disjointsparse.hpp"
...
#line 1 "/home/maspy/compro/library/alg/monoid_minmax.hpp"
...
void solve() {
  ...
  auto norm = [&](ll x, ll y) -> ll { return x * x + y * y; };

  vc<pi> tmp(N);
  FOR(i, N) tmp[i] = mp(XY[i].se, XY[i].se);
  DisjointSparse<Monoid_MinMax<ll, INF>> DS(tmp);
  vi X(N), Y(N);
  FOR(i, N) X[i] = XY[i].fi;
  FOR(i, N) Y[i] = XY[i].se;
  ...
  auto check = [&](ll LIM) -> bool {
    // x or y 
    if (norm(0, MAX(Y) - MIN(Y)) <= LIM) return true;
    if (norm(MAX(X) - MIN(X), 0) <= LIM) return true;
    ll LIM_sq = sqrtl(LIM);
    //  x 
    FOR(L, N) {
      ll lx = X[L];
      if (X[L] > 0) continue;
      if (0 - X[L] > LIM_sq) continue;
      ll rx = X[L] + LIM_sq;
      chmin(rx, -X[L]);
      // [lx,rx]  x 
      ll i = LB(X, lx);
      ...
      {
        auto [a, b] = DS.prod(0, i);
        chmin(ymin, a);
        chmax(ymax, b);
      }
      {
        auto [a, b] = DS.prod(j, N);
        chmin(ymin, a);
        chmax(ymax, b);
      }
      bool ok = 1;
      if (norm(0, ymax - ymin) > LIM) ok = 0;
      ll dx = max(abs(lx), abs(rx));
      ll dy = max(abs(ymin), abs(ymax));
      if (norm(dx, dy) > LIM) ok = 0;
      if (ok) return true;
    }
    //  x 
    FOR(R, N) {
