...
struct imformation{
	ll max,sum;
	imformation operator+(imformation b)const{
		b.sum+=sum;
		b.max=mx(b.max,max);
		return b;
	}
...
ll a[N];
const ll low=1e6+7;
ll d[low+4],mind[low+4],p[low+4],top=0;
void pushup(ll u){
	val[u]=val[u<<1]+val[u<<1|1];
}
void build(ll u,ll l,ll r){
	if(l==r){
		val[u].sum=val[u].max=a[l];
	}
	else{
		ll mid=(l+r)>>1;
		build(u<<1,l,mid);
...
void update(ll u,ll l,ll r,ll ql,ll qr){
	...
	if(l==r){
		val[u].sum=val[u].max=d[val[u].max];
		return;
	}
	ll mid=(l+r)>>1;
	if(mid>=ql)update(u<<1,l,mid,ql,qr);
	if(mid+1<=qr)update(u<<1|1,mid+1,r,ql,qr);
	pushup(u);
}
...
//	writeln(u,' ',l,' ',r,' ',ql,' ',qr);
	...
	ll mid=(l+r)>>1;
	ll ans=0;
	if(mid>=ql)ans+=query(u<<1,l,mid,ql,qr);
	if(mid+1<=qr)ans+=query(u<<1|1,mid+1,r,ql,qr);
	return ans;
}
void calc(void){
	d[1]=1,mind[1]=1;
//	return;
	rep(i,2,low){
//		debugln(i);
		if(!p[i])p[++top]=i,d[i]=2,mind[i]=i;
		rfor(ll j=1;j<=top&&p[j]*i<=low;j++){
//		debugln(p[j]);
			p[i*p[j]]=1;
			if(!(i%p[j])){
				mind[i*p[j]]=mind[i]*p[j];
				if(mind[i*p[j]]==i*p[j])d[i*p[j]]=d[i]+1;
				else
				d[i*p[j]]=d[i/mind[i]]*d[p[j]*mind[i]];
				break;
			}
			else d[i*p[j]]=d[i]*2,mind[i*p[j]]=p[j];
		}
	}
//	rep(i,1,100)
//	writeln(i,' ',d[i],' ',mind[i],' ',p[i]);
}
int main(){
	//IO(CF920F SUM and REPLACE);
	calc();
	ll rill(n,m);
	rep(i,1,n)rill(a[i]);
	build(1,1,n);
	rep(i,1,m){
		ll rill(f),l,r;
		switch(f){
			case 2:rill(l,r),writeln(query(1,1,n,l,r));
				break;
			case 1:rill(l,r),update(1,1,n,l,r);
				break;
		}
	}
	return 0;
