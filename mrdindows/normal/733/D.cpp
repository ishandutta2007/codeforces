...
//int pref = 0;
//void eat(int l, int r) {
////    DBN(l, r);
//    auto idx = max_element(a.begin() + l, a.begin() + r + 1) - a.begin();
//    if (idx != l) {
//        for (int i = l; i < idx; ++i) {
//            ans.push_back({idx - pref, 'L'});
//            ++pref;
//        }
//        for (int i = idx + 1; i <= r; ++i) {
//            ans.push_back({idx - pref, 'R'});
//        }
//    } else {
//        while (a[idx] == a[idx + 1] && idx < r) {
//            ++idx;
//        }
//        if (idx == r) {
//            cout << "NO" << endl;
...
////            dp[l][r] = (dp[l][r - 1] && sum[l][r - 1] != a[r]) || (dp[l + 1][r] && a[l] != sum[l + 1][r]);
////        }
////    }
//    int cur = 0;
//    for (int i = 0; i < m; ++i) {
...
map<pii, multiset<pii>> m;
int n;
void add(int a, int b, int c, int id) {
    if (a > b) swap(a, b);
    m[{a, b}].insert({c, id});
}
multiset<pii>& get(int a, int b) {
    if (a > b) swap(a, b);
    auto& v = m[{a, b}];
    return v;
}
