...
void number() {
	...
	for(int i = 2; i < maxn; i++) {
		if(lp[i] == 0) {
			...
			int q = i, cnt = 0;
			while(q%lp[i] == 0) q/=lp[i], cnt++;
			d[i] = d[q]*(cnt+1);
		}
		for(int j = 0; j < pr.size() && pr[j] <= lp[i] && pr[j]*1ll*i < maxn; j++) {
			lp[pr[j]*i] = pr[j];
		}
	}
}
...
struct segment_tree {
	...
	void upd(int v, int l, int r, int ql, int qr) {
		if (qr < l || ql > r || tree[v].mx <= 2)
			return;
		if (l == r) {
			tree[v] = node(d[tree[v].val]);
			return;
		}
		int mid = (l + r) / 2;
		upd(v << 1, l, mid, ql, qr);
		upd(v << 1 | 1, mid + 1, r, ql, qr);
		tree[v] = merge(tree[v<<1], tree[v<<1|1]);
	}
