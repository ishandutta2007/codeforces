...
int main()
{
    ...
    vector<vector<short>> minns(1<<n, vector<short>(26));
    ...
    for (int mask = 1; mask<(1<<n); mask++)
    {
        ...
        {
            minns[mask] = minns[mask1];
            for (int i = 0; i<26; i++) minns[mask][i] = min(minns[mask][i], cnt[bit][i]);
        }

        int res = 1;
        for (auto it: minns[mask]) res = mul(res, it+1);
        if (cntbits(mask)%2 == 0) res = sub(0, res);
        ans[mask] = res;
    }
    ...
    for(int i = 0;i < n; ++i) for(int mask = 0; mask < (1<<n); ++mask){
            if(mask & (1<<i))
                res[mask] = add(res[mask], res[mask^(1<<i)]);
        }
    ...
    {
        cout<<mask<<": "<<ans[mask]<<' '<<res[mask]<<endl;
    }*/
