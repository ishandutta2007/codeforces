...
int main() {
	...
	REP(_, t) {
		...
		vector <pii> ord(n * m);
		REP(i, n) REP(j, m) ord[i * m + j] = {i, j};
		ll ans = numeric_limits<ll>::max();
		sort(ALL(ord), [](pii x, pii y){return a[x.first][x.second] > a[y.first][y.second];});
		debug(dump(a, 0, n, 0, m));
		for (auto [x, y] : ord) {
			ll k = a[x][y];
			debug(arr(a, x, y));
			if (x == 0 && y == 0) dist[x][y] = a[x][y];
			for (int i = x; i < n; ++i) for (int j = y; j < m; ++j) {
				if (a[i][j] >= k) {
					if (i) mini(dist[i][j], dist[i - 1][j] + a[i][j]);
					if (j) mini(dist[i][j], dist[i][j - 1] + a[i][j]);
				}
			}
			debug(arr(dist, n - 1, m - 1) - imie(k) * (m + n - 1));
			mini(ans, dist[n - 1][m - 1] - k * (m + n - 1));
		}
		printf("%lld\n", ans);
	}
	exit(0);
