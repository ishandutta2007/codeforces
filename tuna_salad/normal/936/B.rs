...
fn dfs(u: usize, p: usize, e: &[Vec<usize>], vis: &mut [Vec<u8>], from: &mut [Vec<usize>], c: &mut bool) {
	vis[u][p] = 1;
	for &v in e[u].iter() {
		if vis[v][p^1] == 1 {
			*c = true;
...
fn main() {
	...
	let s = rin.u()-1;
	let mut vis = v!([n][2] = 0_u8);
	let mut from = v!([n][2] = n);
	let mut can_tie = false;
	dfs(s, 0, &e, &mut vis, &mut from, &mut can_tie);
	for i in 0..n {
		if vis[i][1] == 2 && e[i].len() == 0 {
			writeln!(rout, "Win").ok();
			let mut sol = v!([]);
			sol.push(i);
			let mut p = 1;
	...
	if can_tie {
		writeln!(rout, "Draw").ok();
	} else {
		writeln!(rout, "Lose").ok();
	}
}