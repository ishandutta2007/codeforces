...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			req = t[i] - 'a';
			int id = st.max_right(0, notHasLeq);
			if (id == n) break;

			int cost = bit.qry(id + 1) - 1;
			if (s[id] - 'a' < req) {
				chkmin(ans, cur + cost);
				break;
			}
				...
				req = (int)(t[i] - 'a') - 1;
				int id2 = st.max_right(0, notHasLeq);
				if (id2 < n) {
					int cost2 = bit.qry(id2 + 1) - 1;
					chkmin(ans, cur + cost2);
				}

				cur += cost;
				bit.upd(id + 1, -1);
				st.set(id, S{ bitset<26>() });
			}
		}
