...
bool solve(vector<string> S)
{
    ...
    tate();
    yoko();
    REP(i, H) REP(j, W) if (S[i][j] == 'T' and A[i][j]) return 1;
    return 0;
}
...
int main()
{
    int H, W;
    cin >> H >> W;
    vector<string> S(H), ST(W, string(H, '_'));
    cin >> S;
    REP(i, H) REP(j, W) ST[j][i] = S[i][j];
    cout << (solve(S) or solve(ST) ? "YES\n" : "NO\n");
}