...
void solve() {
  ...
  auto calc = [&](vi B) -> vi {
    ...
    */
    n = N - 1 - I.back();
    if (n) {
      FOR(i, n) { B[N - n + i] = i - n / 2; }
      if (B[N - n - 1] >= B[N - n]) {
        ll x = B[N - n - 1] - B[N - n] + 1;
        FOR(i, n) B[N - n + i] += x;
      }
    }
    /*
  ...
  FOR(k, K) {
    ...
    auto C = calc(B);
    FOR(i, len(C)) A[K * i + k] = C[i];
  }
  FOR(i, N - K) {
    if (A[i] >= A[i + K]) return print("Incorrect sequence");
  }
  print(A);
}
