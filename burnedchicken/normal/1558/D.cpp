...
int n,m,a[maxn],b[maxn],k,c[maxn],pos[maxn];
template<int N>
struct BIT{
    int val[N];
...
void orzck(){
    ...
    if(m<=2500){
        ...
        rep3(i,n-1,0){
            ...
            rep3(j,17,0){
                int ncur=cur+pow2(j);
                if(ncur<=n&&ncur-(curval+bit.val[ncur])<=c[i]){
                    cur=ncur,curval+=bit.val[cur];
                }
            }
            bug(i,cur);
            bit.modify(cur,1);
        ...
        rep(n){
            ...
            if(c[i]!=i){
                ...
                rep3(j,17,0){
                    int ncur=cur+pow2(j);
                    if(ncur<=n&&curval+bit.val[ncur]<=c[i]){
                        cur=ncur,curval+=bit.val[cur];
                    }
                }
                bug(cur);
