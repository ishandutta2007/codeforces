...
fn run() {
    ...
    let atgc = "ATGC".bytes().collect::<Vec<_>>();
    let mut ok = vec![true; s.len() - t.len() + 1];
    for &c in atgc.iter() {
        let mut imos = vec![0; s.len() + 1];
        for (i, s) in s.iter().enumerate() {
            if *s == c {
                let l = i.saturating_sub(k);
                let r = std::cmp::min(n, i + k + 1);
                imos[l] += 1;
                imos[r] -= 1;
            }
        }
        ...
        let a = M::from(rand() % 100000000 + 2);
        let b = M::from(rand() % 100000000 + 2);
        let mut x = vec![];
        let mut y = vec![];
        let mut sum = M::zero();
        for t in t.iter() {
        ...
        for dp in imos.iter().take(s.len()) {
            if *dp > 0 {
                y.push(b);
            } else {
                y.push(M::zero());
        ...
        x.reverse();
        let a = transform::multiply(&x, &y);
        for (ok, a) in ok.iter_mut().zip(a[(t.len() - 1)..].iter()) {
            *ok &= (*a - sum).is_zero();
        }
    }
