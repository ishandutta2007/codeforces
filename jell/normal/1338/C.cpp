#line 1 "codeforces-workspace/contest/1338/c/c.cpp"
...
#line 12 "codeforces-workspace/contest/1338/c/c.cpp"
...
void main() {
  ...
  vector<int> sep;
  assert(!fixed_point([&](auto self, i64 now, i64 p2) -> i64 {
    if (now >= p2) {
      now = self(now - p2, p2 << 2);
      assert(now < p2);
    }
    if (p2 != 1) {
      sep.emplace_back(now & 3);
      now >>= 2;
    }
    return now;
  })(n, 1));
