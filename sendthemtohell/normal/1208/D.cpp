...
void upd(int v) {
	if (t[v << 1].x + ad[v << 1] < t[(v << 1) | 1].x + ad[(v << 1) | 1]) {
		t[v] = t[v << 1];
		t[v].x += ad[v << 1];
	} else {
		t[v] = t[(v << 1) | 1];
...
void build(int v, int l, int r, vector<ll>& vec) {
	if (r - l == 1) {
		t[v] = { vec[l], -l };
		return;
	}
...
void add(int v, int l, int r, int lf, int rf, ll vl) {
	if (r <= lf || rf <= l) {
		return;
	}
	if (lf <= l && r <= rf) {
	...
	push(v, l, r);
	int m = (l + r) >> 1;
	add(v << 1, l, m, lf, rf, vl);
	add((v << 1) | 1, m, r, lf, rf, vl);
	upd(v);
	//t[v] = min(t[v << 1], t[(v << 1) | 1]);
}
