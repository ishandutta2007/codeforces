...
const int maxn = 2e5 + 100, maxw = 5e6 + 100, inf = 1e9 + 100, sq = 450, LG = 20, mod = 1e9 + 933, mod1 = 1e9 + 993;
...
void prec() {
    ...
    for (int i = e.size() - 1; i >= 0; i--) {
        if (e[i][0] == e[i][1] && e[i][0] == 0)
            sz[i]++;
        else
            sz[i] = sz[e[i][0]] + sz[e[i][1]];
    }
}
...
void solve() {
    ...
    e.clear();
    e.push_back({});
    int pref = max(0, k - 20), rem = min(k, 20);
    int nz = 0;
    for (int i = 0; i < n; i++) {
        if (nz >= pref && i + rem <= n)
            add(i, i + rem - 1);
