...
using mi=modnum<mod>;
const int N=3e5+5,M=1e6+5,Q=1e6+5;
int t,n,a[2][N];
bool vis[N]; int sz;
ll ans,s1,s2;
vector<int> g[N];
...
void solve(){
	...
	ans=s1=s2=0;
	for(int t=0;t<2;t++) for(int i=1;i<=n;i++) cin>>a[t][i];
	for(int i=1;i<=n;i++){
		g[a[0][i]].emplace_back(a[1][i]);
		g[a[1][i]].emplace_back(a[0][i]);
	}
	for(int i=1;i<=n;i++) if(!vis[i]){
		...
		s1+=sz/2;
		s2+=sz/2;
		//cout<<sz<<": "<<sz/2<<" "<<sz-sz/2<<endl;
	}
	//cout<<s1<<" - "<<s2<<endl;
	for(int i=1;i<=s1;i++) ans-=2*i;
	for(int i=n-s2+1;i<=n;i++) ans+=2*i;
	cout<<ans<<"\n";
}
int main(){
