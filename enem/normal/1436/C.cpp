...
int main()
{
    ...
    */
    ...
    static const ll N=1005;
    static ll fc[N],fci[N];
    fc[0]=1;
    REP(i,1,N)
    {
        fc[i]=(fc[i-1]*i)%M1;
    }
    fci[N-1]=modI(fc[N-1],M1);
    REPI(i,0,N-1)
    {
        fci[i]=(fci[i+1]*(i+1))%M1;
    }
    ...
    REP(tc,1,ntc+1)
    {
        ...
        ll ans=1;
        if(x-dw-1>=0) ans*=((fc[x-1]*fci[x-dw-1])%M1);
        else ans=0;
        ans%=M1;
        if(n-x-up>=0) ans*=((fc[n-x]*fci[n-x-up])%M1);
        else ans=0;
        ans%=M1;
        if(n-up-dw-1>=0) ans*=(fc[n-up-dw-1]);
        else ans=0;
        ans%=M1;
        cout<<ans;
