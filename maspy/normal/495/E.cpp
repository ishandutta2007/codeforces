...
void solve() {
  ...
  FORIN(v, ET.VR) {
    ...
    if (now < max - M)
      ...
      dp[now - (max - M)] = 1.0;
    FOR(i, M + M) dp[i + 1] += dp[i];

    FORIN(e, G[v]) {
      ll to = e.to;
      if (to == ET.parent[v]) continue;
      FOR(i, M + M + 1) dp[i] *= DP[to][i];
    }
    auto& newdp = DP[v];
    FOR(x, M + M + 1) {
      newdp[x] = (1.0 - P[v]) * dp[x];
      if (x) newdp[x] += P[v] * dp[x - 1];
    }
  }
  auto& dp = DP[0];
  FOR_R(i, M + M) dp[i + 1] -= dp[i];
  ld ANS = 0;
  FOR(m, M + M + 1) ANS += m * dp[m];
  ANS += (max - M);
  print(ANS);
}
