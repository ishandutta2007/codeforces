...
void solve() {
  ...
  auto OPE = [&](ll i, ll j) -> void {
    assert(i < j && is_prime[j - i + 1]);
    ANS.eb(i + 1, j + 1);
    tie(A[i], A[j]) = mp(A[j], A[i]);
    pos[A[i]] = i;
    pos[A[j]] = j;
  };

  auto change_dfs = [&](auto self, ll i, ll j) -> void {
    if (i > j) swap(i, j);
    ll d = j - i;
    if (d == 1) return OPE(i, j);
    if (d % 2 == 1) {
      OPE(i, i + 1);
      self(self, i + 1, j);
      OPE(i, i + 1);
  ...
  auto change = [&](ll i, ll j) -> void { change_dfs(change_dfs, i, j); };
