...
struct Player {
  int a, b, id;
  bool operator < (const Player& ot) const {
    return a < ot.a;
  }
};
...
void solve(__attribute__((unused)) bool read) {
  ...
  sort(all(a));
  vector<int> pref_max(a.size() + 1, 0);
  for (int i = 0; i < a.size(); ++i) {
    pref_max[i + 1] = max(pref_max[i], a[i].b);
  }
  string res(n, '0');
  int min_b = (int)1e9 + 100;
  for (int i = n - 1; i >= 0; --i) {
    if (i + 1 < n && a[i + 1].a > a[i].a && pref_max[i + 1] < min_b) {
      break;
    }
    res[a[i].id] = '1';
    relax_min(min_b, a[i].b);
  }
  cout << res << "\n";
}