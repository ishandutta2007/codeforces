...
signed main() {
    ...
    auto addEdge = [&](int from, int to, int u, int v, int w) {
        G[ind(from, u)].pb({ind(to, v), w});
        G[ind(from, v)].pb({ind(to, u), w});
    };
    for (int i = 0; i < m; ++i) {
        int u, v, w;
        read(u, v, w);
        ...
        for (int level = 0; level < 4; ++level) {
            addEdge(level, level, u, v, w);
        }
    }
    for (int i = 0; i < n; ++i) {
        addEdge(0, 3, i, i, 0);
    }
    auto ans = dijkstra(G, ind(0, 0));
    for (int i = 1; i < n; ++i) print(ans[ind(3, i)], ' ');
    #ifdef DEBUG
