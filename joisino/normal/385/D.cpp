...
int main(){
  ...
  REP( i , PW(n) ) dp[i] = -INFLL;
  dp[0] = l;
  REP( mask , PW(n) ){
    REP( i , n ) if( !( mask & PW(i) ) ){
      P light = P( x[i] , y[i] );
      P lp = P( dp[mask] , 0 );
      P v = rotate( lp - light , dtr( a[i] ) );
      if( v.Y < -EPS ){
	ld res = y[i] / -v.Y * v.X + x[i];
	chmax( dp[mask|PW(i)] , min( (ld)r , res ) );
      } else {
	chmax( dp[mask|PW(i)] , (ld)r );
      }
    }
  }
