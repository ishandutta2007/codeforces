...
int main()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        cnt_okay[i] += cnt_okay[i - 1];
    }
    vector<vector<ll> > tb(42, vector<ll>(n + 2));
    for (int k = 0; k < 42; k++)
    {
        for (int i = 0; i <= n; i++)
        {
            if (i) tb[k][i] = tb[k][i - 1];
            tb[k][i] = (tb[k][i] + cnk(cnt_okay[i] - k, i - k)) % MOD;
        }
    }
    long long ans = 0;
    for (int mask = 0; mask < (1 << m); mask++)
    {
        ...
        int k = id.size();
        if (maxsz >= minsz)
            cur = (tb[k][maxsz] + MOD - (minsz > 0 ? tb[k][minsz - 1] : 0)) % MOD;
        //cout << mask << " " << k << " " << cur << endl;
        //cout << minsz << " " << maxsz << endl;
        if (p % 2) ans = (ans + MOD - cur) % MOD;
