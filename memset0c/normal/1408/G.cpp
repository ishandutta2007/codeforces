...
z tmp[N],dp[N][N];
int n,nod,anc[N],lst[N],len[N],a[N>>1][N>>1],ch[N][2];
bool vis[N],pos[N];
...
void merge(int u,int v){
  ...
  if(u==v){
    ...
    ch[nod][1]=v;
    len[nod]=len[u]+len[v];
    lst[nod]=lst[u]+lst[v]+len[u]*len[v]-1;
    if(!lst[nod])pos[nod]=true;
  }
}
...
void dfs(int u){
  if(ch[u][0]&&ch[u][1]){
    dfs(ch[u][0]);
    dfs(ch[u][1]);
    int i=ch[u][0],j=ch[u][1];
    for(int x=1;x<=len[i];x++)
      for(int y=1;y<=len[j];y++){
        dp[u][x+y]+=dp[i][x]*dp[j][y];
      }
    if(pos[u]){
      dp[u][1]=1;
    }
  ...
  // printf("dp[%d](%d) = ",u,pos[u]);
  // for(int i=1;i<=len[u];i++)printf("%d%c",dp[u][i].x," \n"[i==len[u]]);
}
...
int main(){
  ...
  for(const auto &it:e){
    // printf("%d %d\n",get<1>(it),get<2>(it));
    merge(find(get<1>(it)),find(get<2>(it)));
  }
  dfs(nod);
  for(int i=1;i<=n;i++){
    print(dp[nod][i].x," \n"[i==n]);
