...
int main() {
	...
	dsu dsu(k + 1);
	vector<vector<int>> in(k + 1), out(k + 1);
	for (int i = 0; i < n; i++) {
		int c; R(c);
		vector<int> a(c); R(a);
		for (int i = 0; i + 1 < c; i++) {
			out[a[i]].emplace_back(a[i + 1]);
			in[a[i + 1]].emplace_back(a[i]);
			dsu.uni(a[i], a[i + 1]);
		}
	}
	...
	vector<bool> bad(k + 1, false);
	for (int u = 1; u <= k; u++)
		bad[u] = in[u].size() >= 2 || out[u].size() >= 2;

	for (int u = 1; u <= k; u++)
		if (bad[u]) bad[dsu.find(u)] = true;

	for (int u = 1; u <= k; u++)
		if (bad[dsu.find(u)]) bad[u] = true;

	map<int, int> ways;
	for (int u = 1; u <= k; u++) {
		if (bad[u] || in[u].size() != 0) continue;
		int h = u;
		int len = 1;
		while (!out[h].empty()) {
			assert(out[h].size() == 1);
