...
long long bin(int n, int k) {
    ...
    for (int i = 1; i <= n - k; ++i) res = res * pw(i * 1ll, mod - 2, mod) % mod;
    for (int i = 1; i <= k; ++i) res = res * pw(i * 1ll, mod - 2, mod) % mod;
...
void solve(int) {
    ...
    vi rplus(n);
    forn(i, n) rplus[i] = r[p[i] + 1];

    int d = 0;
    forn(i, n-1) d += rplus[i] > rplus[i + 1];

    cout << bin(n + k - d - 1, k - d - 1) << "\n";
}
