...
void solve() {
    ...
    for (int i = n; i >= 1; i--) {
        ...
        if (a[i] == a[i + 1]) {
            ...
            if (done[i + 1] <= n) {
                // [i + 1 ...] can eventually be cleared
                auto&& search_map = *m[i + 1];
                auto it = search_map.find(a[i]);
                if (it != search_map.end()) done[i] = it->second;
            }
        }
        if (done[i] <= n) {  // i can be cleared
            cnt[i] = 1 + cnt[done[i] + 1];
            m[i] = m[done[i] + 1];  // inherit the map
            ans += cnt[i];
        }
        if (m[i] == nullptr) m[i] = new map<int, int>();
        auto&& record_map = *m[i];
        record_map[a[i]] = i;
    }
    cout << ans << '\n';
