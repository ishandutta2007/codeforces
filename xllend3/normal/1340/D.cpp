...
void dfs(int x,int f,int t){
	...
	for(auto i:a[x])if(i!=f){
		if(t==mx){
			t-=SZ(a[x]);
			nxt[mp(x,mx)]=mp(x,t);++cnt;
			ok=1;
		}
		dfs(i,x,t+1);
		nxt[mp(x,t)]=mp(i,t+1);++cnt;
		nxt[mp(i,t)]=mp(x,t+1);++cnt;
		++t;
	}
	if(!ok){nxt[mp(x,t)]=mp(x,T-1);++cnt;}
}
int main(){
#ifdef LOCAL
	...
	rep(i,1,n)MAX(mx,SZ(a[i]));
	dfs(1,0,0);
	// for(auto i:nxt)printf("%d %d %d %d\n",i.X.X,i.X.Y,i.Y.X,i.Y.Y);
	for(pii q=mp(1,0);;q=nxt[q]){
		// printf("%d %d\n",q.X,q.Y);
		ans.pb(q);
		if(--cnt==-1)break;
	}
