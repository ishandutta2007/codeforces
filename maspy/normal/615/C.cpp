...
#line 1 "/home/maspy/library/string/zalgorithm.hpp"
...
void solve() {
  ...
  while (len(X)) {
    string Q = X + S + T;
    auto Z = z_algorithm(Q);
    pi best = {0, 0};
    FOR(i, len(S)) {
      auto x = Z[len(X) + i];
      chmin(x, len(S) - i);
      chmin(x, len(X));
      if (abs(best.se - best.fi) < x) best = {i + 1, i + x};
    }
    FOR(i, len(T)) {
      auto x = Z[len(X) + len(S) + i];
      chmin(x, len(X));
      if (abs(best.se - best.fi) < x) best = {len(S) - i, len(S) - i - x + 1};
    }
    if (best.fi == 0) return print(-1);
    ANS.eb(best);
    ll x = abs(abs(best.fi - best.se) + 1);
    X = X.substr(x);
  }
  print(len(ANS));
