...
inline void AC() {
    ...
    for(_ i = 1; i <= m; ++i) b[i] = 0;
    for(_ i = 1; i <= n; ++i) ++b[a[i]];
    for(_ i = 1; i <= m; ++i) b[i] += b[i-1];
    for(_ i = 1; i <= m; ++i) if(!(b[i]-b[i-1])) {
        for(_ j = 1; i*j <= m; ++j) if(b[j]-b[j-1]) {
            if(b[min(m+1, (i+1)*j)-1]-b[i*j-1]) {
                no();
                return;
            }
        }
