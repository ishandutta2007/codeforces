...
int main(int argc, char **argv){
  ...
  vector<vector<array<LL,65>>> D(11);
  for(int b=2;b<=10;++b){
    D[b].resize(1<<b);
    D[b][0][0]=1;
    for(int n=1;n<sz(D[b][0]);++n){
      forn(m,1<<b){
        forn(d,b)
          clamp(D[b][m][n]+=D[b][m^(1<<d)][n-1]);
      }
    }
  }

  auto doit=[&](int b,LL an){
    vector<short> A;
    while(an){
      A.pb(an%b);
      an/=b;
    ...
    for(int n=1;n<sz(A);++n){
      for(int d=1;d<b;++d) r+=D[b][1<<d][n-1];
    }
    for(int i=sz(A)-1;i>=0;--i){
      forn(d,A[i]){
        if(i+1==sz(A) && !d) continue;
        r+=D[b][m^(1<<d)][i];
      }
      m^=1<<A[i];
    }
