...
public:
    ...
    unsigned unite(unsigned x, unsigned y)
    {
        assert(x < size() && y < size());
...
struct solver
{
    solver()
    {
        int n; cin>>n;
        vector<int> ecyc;
        vector<p32> nck(n); cin>>nck;

        auto juf=[&](const int k)->bool
        {
            const int mask=(1<<k)-1;
            // vector<unordered_multiset<int>> G(n+(1<<k));
            union_find uf(n+(1<<k));
            ...
            for(int i=0,j=1<<k,r=uf.find(j); i<n; ++j,++i)
            {
                if(uf.find(j)!=r)
                {
                    return false;
            ...
            //                 dfs(to);
            //             }
            //             ecyc.emplace_back(now);
            //         }
            //     ) (i);
            //     break;
            // }

            // assert(ecyc.front()==ecyc.back());
            // ecyc.pop_back();
            // dump(ecyc);

            // return ecyc.size()==n*2;
        };

        int k=binary(0,21,juf);
        // assert(juf(k));

        {
            const int mask=(1<<k)-1;

            // unordered_set<p32> used;
            vector<unordered_multiset<int>> G(n+(1<<k));
            for(int i=0; i<n; ++i)
            {
                auto &[a,b]=nck[i];
...

            {
                ...
                {
                    int val=ecyc[i];
                    int pprl=ecyc[i+1]-(1<<k);
                    if(nck[pprl].first==val)
                    {
                        cout << pprl*2+1 << " " << pprl*2+2 << " ";
                    }
                    else
                    {
                        cout << pprl*2+2 << " " << pprl*2+1 << " ";
                    }
                }
                cout << "\n";
                return;
