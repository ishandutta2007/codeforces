...
void solve() {
    ...
    if (n % 4 == 0) {
        cout << static_cast<long double>(1) / tanl(pi / n) << '\n';
    } else {
        complex<long double> point{sqrtl(2) / 2, sqrtl(2) / 2};
        const long double rotate_angle = pi * 2 / n;
        complex<long double> rotate{cosl(rotate_angle), sinl(rotate_angle)};
        long double max_x, min_x;
        max_x = min_x = sqrtl(2) / 2;
        for (int i = 1; i <= n; i++) {
            point *= rotate;
            max_x = max(max_x, point.imag());
            min_x = min(min_x, point.imag());
        }
        cout << (max_x - min_x) / (sinl(rotate_angle / 2) * 2) << '\n';
    }
}
