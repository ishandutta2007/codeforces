...
void mian() {
	REP(i, 0, 25) buc[i].clear();
	n = read(), reads(a + 1), reads(b + 1);
	bit.init();
	PER(i, n, 1) buc[a[i] - 'a'].pb(i), bit.add(i, 1);
	int ans = inf, now = 0;
	REP(i, 1, n) {
		int x = b[i] - 'a';
		REP(j, 0, x - 1) if(SZ(buc[j])) {
			int p = buc[j].back();
			chkmn(ans, now + bit.Cnt(p) - 1);
		}
		if(buc[x].empty()) break;
		int p = buc[x].back(); buc[x].pop_back();
		now += bit.Cnt(p) - 1; bit.add(p, -1);
	}
	prt(ans > n * n ? -1 : ans), pc('\n');
}
