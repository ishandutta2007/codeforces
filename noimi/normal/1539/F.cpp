...
int main() {
    ...
    rep(_, 2) {
        ...
        rep(i, n) v[b[i]].eb(i);
        auto id = iota(a);
        auto R = get_lazy_segment_tree(n + 1, _max<int>, plus<int>(), plus<int>(), -inf<int>, 0);
        auto L = get_lazy_segment_tree(n + 1, _max<int>, plus<int>(), plus<int>(), -inf<int>, 0);
        rep(i, n + 1) R.set(i, -i);
        rep3(i, n, 0) L.set(i, -(n - i));
        R.build(), L.build();
        rep(k, si(v)) {
            for(auto i : v[k]) {
                R.update(i + 1, n + 1, 2);
            ...
            for(auto i : v[k]) {
                int r = R.query(i + 1, n + 1) - R[i + 1];
                int l = L.query(0, i + 1) - L[i];
                chmax(ans[i], (r + l + 1 - _) / 2);
            }
        }
    }
    cout << ans << endl;
