...
//template
#define MAXN 100000
int mx[2][MAXN*4+10],f[2][MAXN+10],ans;
int n,m,a[MAXN+10]; 
inline void update(int *mx,int i){
	mx[i]=max(mx[i<<1],mx[(i<<1)|1]);
}
inline void insert(int *mx,int i,int l,int r,int p,int val){
	if(l==r){
		mx[i]=max(mx[i],val);
		return;
	}
	int mid((l+r)>>1);
	if(p<=mid)
		insert(mx,i<<1,l,mid,p,val);
	else
		insert(mx,(i<<1)|1,mid+1,r,p,val);
	update(mx,i);
}
inline int get_mx(int *mx,int i,int l,int r,int ll,int rr){
	if(ll<=l&&r<=rr)
		return mx[i];
	if(ll>r||rr<l)
		return 0;
	int mid((l+r)>>1);
	return max(get_mx(mx,i<<1,l,mid,ll,rr),get_mx(mx,(i<<1)|1,mid+1,r,ll,rr));
}
int main()
{
	Read(n),Read(m);
	...
	for(i=1;i<m;i++)
		a[i+1]+=a[i];
	for(i=1;i<=m;i++){
		f[0][i]=get_mx(mx[0],1,0,n,0,a[i])+1;
		f[1][i]=get_mx(mx[1],1,0,n,a[i],n)+1;
		insert(mx[0],1,0,n,a[i],f[0][i]);
		insert(mx[1],1,0,n,a[i],max(f[0][i],f[1][i]));
		ans=max(f[0][i],f[1][i]);
	}
	printf("%d\n",ans);
}