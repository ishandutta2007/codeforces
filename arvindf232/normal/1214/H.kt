...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    fun furthestFrom(v:Int):Int {
        val dist = BFS(v)
        return (0 until n).maxByOrNull { dist[it] }!!
    }
    ...
    fun diameter():List<Int>{
        val a = furthestFrom(0)
        val b = furthestFrom(a)
...
fun main(){
    solve.cases{
        ...
        if(k == 2){
            put("Yes")
            val dist = G.BFS(0)
            put(dist.map { (it %2 ) + 1  }.joinToString(" "))
            return@cases
        }
        val d = G.diameter()
        val root = d[0]
        ...
        for(i in 0 until n){
            if(order[i] == -1){
                if(G.dist(root, i) >= k - 1 && G.dist(d.last(), i) >= k - 1) {
                    put("No")
                    return@cases
                }
            }
        ...
        G.leafFirst {
            ...
            G.subs(v){
                    w->
                if(order[w] == -1){
                    depth[v] = maxOf(depth[w]+1 , depth[v])
                }
            }
        }
        for((i, v) in d.withIndex()) {
            if(depth[v] > 0 && depth[v] + i + 1 >= k && depth[v] + (d.size - i) >= k) {
                put("BAD $i ${depth[v]}, dist ${G.dist(root,v)} ${G.dist(d.last(),v)}")
                return@cases
            }
        }
        ...
        fun dfs(v:Int, p:Int, c:Int, inc:Int){
            colour[v] = c
            val nextcolour = if(inc == 1) c.next(k) else c.previous(k)
            G.NS(v){w ->
                if(w == p || order[w] != -1) {
                    return@NS
                }
                dfs(w,v,nextcolour,inc)
            }
        }
        for((i,v) in d.withIndex()){
            val inc = if(i < (d.size) / 2) -1 else 1
            dfs(v,-1,i % k , inc)
        }
        put("Yes")
        put(colour.map{it+1}.joinToString(" "))
...
special case of 2
how can I miss that
2700 is everything is annoying


diameter is clean, but is not necessarily right


the holy DP order should not be violated
