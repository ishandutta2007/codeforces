...
#endif
int a[1005][1005],s[1035][1035],t[1035][1035];
#define s a
signed main()
{
	initprog();
	int T=read();rg(T)
	// int n=read();rg(n+10)s[i][0]=s[i][1]=0,f[i][0]=f[i][1]=0;gr
	// rg(n)rg_(j,n)a[i][j]=read(),s[i][j&1]^=a[i][j];gr gr
	// for(int i=1;i<=n;i+=2)f[i+1][0]=f[i-1][0]^s[i][0],f[i+1][1]=f[i-1][1]^s[i][1];
	// for(int i=n;i>=1;i-=2)f[i-1][0]=f[i+1][0]^/s[i][0],f[i-1][1]=f[i+1][1]^s[i][1];
	int n=read();rg(n)
	rg_(j,n)a[i][j]=read();gr gr
	for(int i=0;i<=n+1;i++)for(int j=0;j<=n+1;j++)t[i][j]=0;
	int ans=0;
	rg(n-1)
	t[i+1][i]=t[i][i-1]^s[i][i];
	gr
	rg(n-1)
	for(int l=i-1,u=i-1,r=i+2,d=i+2;;l--,u--,r++,d++)
	{
		if(l<1||r>n||u<1||d>n)break;
		t[d][l]=t[d-1][l-1]^t[d-1][l+1];
		t[d][l]^=s[d-1][l]^s[u][r-1];
		t[d][l]^=t[d-2][l];
	}
	gr
	rg(n)rg_(j,n)ans^=t[i][j];gr gr
	for(int i=0;i<=n+1;i++)for(int j=0;j<=n+1;j++)t[i][j]=0;
	rg(n)std::reverse(a[i]+1,a[i]+n+1);gr
