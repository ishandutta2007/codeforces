...
namespace Geometory
{
  ...
  Point Crosspoint(const Line &l, const Segment &m)
  { //OK
    double A = cross(l.b - l.a, m.b - m.a);
    double B = cross(l.b - l.a, l.b - m.a);
    if(abs(A) < EPS && abs(B) < EPS) return m.a; // same line
...
int main()
{
  ...
  for(int i = 0; i < N; i++) {
    ...
    if(seg.a == seg.b) {
      ...
      if(Contains(polygon, seg.a) != IN) {
        cout << -1 << endl;
        return (0);
      }
      continue;
    }
    bool aaa = seg.a.x < seg.b.x, bbb = seg.a.y < seg.b.y;
    double dists = Distance(seg.a, seg.b);
    for(int j = 0; j < 4; j++) {
      Segment seq(curr(polygon, j), next(polygon, j));
      if(!Intersect(seg, seq)) continue;
      auto get = Crosspoint(seg, seq);
      bool ccc = seg.a.x < get.x, ddd = seg.a.y < get.y;
      if(seg.a.x != get.x) {
        if(ccc != aaa) continue;
      }
      if(seg.a.y != get.y) {
    ...
    bool vv[4] = {}, other = true;
    for(auto &q : cross) {
      int comp = false;
      for(int j = 0; j < 4; j++) {
        vv[j] |= polygon[j] == q;
        if(polygon[j] == q) comp = true;
      }
      if(!comp) other = false;
    }
    ...
    double bit = 0, uku = 1e64;
    for(auto &p : cross) {
      bit = max(bit, Distance(p, seg.a) / dists);
      uku = min(uku, Distance(p, seg.a) / dists);
    }
  ...
  if(latte + EPS > malta) cout << -1 << endl;
  else cout << fixed << setprecision(15) << latte << endl;
