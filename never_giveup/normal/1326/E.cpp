...
const int maxn = 3e5 + 100, maxw = 1e6 + 100, inf = 1e9 + 100, sq = 100, LG = 18;

int n;

int a[maxn], b[maxn], pos[maxn], pa[maxn];
...
int main() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        if (i == 0)
            break;
        update(1, 0, n - 1, 0, pa[i], 1);
        while (q[1] > 0) {
            update(1, 0, n - 1, 0, b[id], -1);
            id++;
        }
    }
    for (int i = 1; i < n; i++) {
