...
P dfs(int v)
{
	...
	for(auto u : G[v]) vec.push_back(dfs(u));
	sort(all(vec)), reverse(all(vec));
	
	ll id = vec[0].second;
	rep(i, 1, sz(vec)-1){
		vec[0].first += vec[i].first;
		for(auto p : mp[vec[i].second]){
			mp[id][p.first] ^= p.second;
		}
	}
	
	for(auto q : qvec[v]){
		ll h = q.first, res = 0;
		if(mp[id].count(h)) res = mp[id][h];
		if((res&(res-1)) == 0) ans[q.second] = 1;
	}
