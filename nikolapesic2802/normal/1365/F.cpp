...
// URL : http://codeforces.com/contest/1365/problem/F
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int main()
{
	...
	while(t--){
		...
		for(int i=0;i<n/2;i++)
			aa.insert({min(a[i],a[n-i-1]),max(a[i],a[n-i-1])}),bb.insert({min(b[i],b[n-i-1]),max(b[i],b[n-i-1])});
		if(n&1)
			aa.insert({a[n/2],-1}),bb.insert({b[n/2],-1});
		printf(aa==bb?"Yes\n":"No\n");
	}
	return 0;
}