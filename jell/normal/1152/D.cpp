...
int n;
bool vis[1010][1010];
pair<modint,int> dp[1010][1010];

pair<modint,int> dfs(int l,int r) {
    if(vis[l][r]) return dp[l][r];
    vis[l][r]=true;
    pair<modint,int> &ret=dp[l][r];
    if(l>r) {
        if(l<n) {
            auto res1=dfs(l+1,r);
            auto res2=dfs(l,r+1);
            ret.fir=res1.fir+res2.fir;
            if(res1.sec+res2.sec) {
                ret.fir+=1;
            } else {
                ret.sec++;
            }
