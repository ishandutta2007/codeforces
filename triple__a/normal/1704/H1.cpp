...
int n,P;
int dp[5007][5007],sum[5007][5007],f[5007][5007],C[5007][5007],g[5007],rg[5007];
...
signed main(){
  ...
  cin>>n>>P;
  dp[0][0]=1; g[0]=1;
  for (int j=1;j<=n;++j) g[j]=g[j-1]*j%P;
  for (int j=0;j<=n;++j) rg[j]=modpow(g[j],P-2,P);
  for (int j=0;j<=n;++j) sum[j][0]=1;
  f[0][0]=1;
  for (int i=1;i<=n;++i){
    f[i][0]=1;
    for (int j=1;j<=n;++j) f[i][j]=f[i][j-1]*i%P;
  }
  C[0][0]=1;
  for (int i=1;i<=n;++i){
    C[i][0]=1;
  ...
  for (int i=1;i<=n;++i){
    ...
    for (int j=0;j<=n;++j) sum[j][i]=dp[j][i];
    for (int j=1;j<=n;++j) sum[j][i]=(sum[j][i]+sum[j-1][i])%P;
  }
  int ans=0;
  for (int i=1;i<=n;++i){// number of chains
    for (int j=2*i;j<=n;++j){
      ans=(ans+C[n][j]*dp[j][i]%P*g[j]%P*rg[i]%P*f[n-1][i]%P*f[j-i][n-j])%P;
    }
  }
  cout<<ans<<endl;
  return 0;
