...
void sub(int q,vc<pi> lr,vi now){
	vi rw_nouse=readvi(q);
	vc<pi> di(q);
	rep(i,q)di[i]=pi(rw_nouse[i],i);
	sort(all(di));
	...
	{
		multiset<pi> alive;
		sort(all(lr),[&](pi a,pi b){return a.a<b.a;});
		int head=0;
		rep(i,n){
			while(head<si(lr)&&lr[head].a<now[i]){
				auto [l,r]=lr[head++];
				alive.emplace(r,l);
			}
			auto itr=alive.bg;
			if(itr==alive.ed||itr->a<now[i]){
				lf[i]=T(-inf,inf,ten(9));
				break;
			}else{
				lf[i]=T(itr->b,itr->a,1);
				alive.erase(itr);
			}
		}
	}
	{
		multiset<pi> alive;
		sort(all(lr),[&](pi a,pi b){return a.b>b.b;});
		int head=0;
		per(i,n){
			while(head<si(lr)&&now[i]<lr[head].b){
				auto [l,r]=lr[head++];
				alive.emplace(l,r);
			}
			auto itr=alive.ed;
			if(itr==alive.bg||now[i]<(--itr)->a){
				rt[i]=T(-inf,inf,ten(9));
				break;
			}else{
				rt[i]=T(itr->a,itr->b,1);
				alive.erase(itr);
			}
		}
	}
	...
	seglazy<N> seg(vi(q,0));
	auto add=[&](int l,int r,int v){
		l=lwb(di,pi(l,-1));
		r=lwb(di,pi(r,inf));
		seg.ch(l,r,&N::add,v);
	};
	for(auto [l,r]:lr)add(l,r,1);
	rep(i,n){
		auto [l,r,v]=rt[i];
		add(l,r,-v);
	}
	...
	rep(idx,q){
		...
		dmp2(d,i,seg.getall().mn);
		if(seg.getall().mn>=0&&seg.point_get(idx).mn>0)
			ans[i]=1;
	}
	
	rep(i,q)if(ans[i])yes(0);
	else no(0);
}
...
void slv(){
	...
	{
		vi b=readvi(n);
		auto dfs=[&](auto self,int v,int up)->int{
			if(v==-1)return -inf;
			if(b[v]==0){
				int lw=-inf;
				rep(c,2)chmax(lw,self(self,t[v][c],up));
				lr.eb(lw,up);
				return lw;
			}else{
				if(b[v]>up)bad=true;
				rep(c,2)self(self,t[v][c],b[v]);
				return b[v];
			}
		};
		dfs(dfs,root,inf);
