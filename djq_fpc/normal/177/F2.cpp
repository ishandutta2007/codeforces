...
int n,m,t,i,j,a[20][20],sum[1048577],lg[1048577];
...
int main()
{
	...
	fz0k(i,n)
	{
		g.clear();
		int all=0;
		fz0k(j,n) if (a[i][j]) all|=(1<<j); 
		ff(f,it)
		{
			g.push_back(*it);
			int k;
			for (k=(all&((1<<n)-1-it->first));k;k-=lowbit(k))
			{
				j=lg[lowbit(k)];
				if (a[i][j])
				{
					g.push_back(make_pair((it->first|(1<<j)),it->second+a[i][j]));
				}
			}
		}
		sort(g.begin(),g.end(),cmp);
		while (g.size()>t) g.pop_back(); 
		f=g;
	}
	sort(g.begin(),g.end(),cmp);
//	ff(g,it) cerr<<it->second<<' ';cerr<<endl;
	cout<<g[t-1].second<<endl;
	return 0;
}