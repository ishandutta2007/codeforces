...
const int modular_int<mod>::inverse_limit = 500;
...
void solve(){
	...
	vector<vector<mint>> ncr(n + 1, vector<mint>(n + 1));
	vector<vector<mint>> powers(k + 1, vector<mint>(n * n + 1, 1));
	...
	for(int i = 1; i <= n; ++i){
		for(int j = 1; j <= k; ++j){
			dp[i][j] = dp[i][j - 1];
			for(int l = 1; l <= i; ++l){
				int edges = i * (i - 1) - (i - l) * (i - l - 1); edges >>= 1;				
				dp[i][j] += dp[i - l][j - 1] * powers[k - j + 1][edges] * ncr[i][l];
			}
		}
	}
