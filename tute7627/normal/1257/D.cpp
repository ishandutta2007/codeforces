...
int main(){
  ...
  while(t--){
    ...
    rep(i,0,m)chmax(b[s[i]-1],p[i]);
    SegmentTree<ll> seg(b);
    rrep(i,0,n-1)chmax(b[i],b[i+1]);
    reverse(ALL(b));
    //debug(b,n);
    SegmentTree<ll>seg2(b);
    ll days=0,pos=0;
    judge=true;
    while(pos<n){
      ll cnt=0;
      ll limit=INF;
      while(pos+cnt<n&&cnt<=limit&&seg.query(cnt,n)>=a[pos+cnt]){
        chmin(limit,n-seg2.lower_bound(0,a[pos+cnt])-1);
        //cout<<cnt spa limit<<endl;
        cnt++;
      }
      //cout<<pos spa cnt<<endl;
      if(cnt==0){
        judge=false;
        break;
      }
