...
struct XXJ{
	...
	LL F[70];
	void init(int ini){
		n=ini;cnt=0;bin[0]=1;F[0]=0;
		for(int i=1;i<=n;++i) {
			bin[i]=bin[i-1]<<1; F[i]=0;
		}
		return;
	}
	void add(LL x){
...
void dfs(int x){
	...
	for(auto o:V[x]){
		int y=o.r;
		if(vis[y]) {
			xxj.add(xo[x]^xo[y]^o.w);
		}
		else{
			xo[y]=xo[x]^o.w;
			dfs(y);
			for(int j=0;j<=60;++j){
				num[j]+=siz[x][j][0]*siz[y][j][1];
				num[j]+=siz[x][j][1]*siz[y][j][0];
				siz[x][j][0]+=siz[y][j][0];
				siz[x][j][1]+=siz[y][j][1];
			}
		}
...
void MAIN(){
	...
	for(int i=1;i<=n;++i){
		...
		for(int j=0;j<=60;++j){
			if(w&xxj.bin[j]){
				ans+=((nnn-1)*nnn/2)%M*(xxj.bin[j]%M)%M*now%M;
			}
			else{
				ans+=num[j]%M*(xxj.bin[j]%M)%M*(now+now-M)%M;
			}
		}
	}
	ans=(ans%M+M)%M;
