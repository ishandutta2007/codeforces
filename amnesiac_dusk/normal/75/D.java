	...
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
		{
			...
			for(int j=0; j<a[i].length; j++)
			{
				sum+=a[i][j];
				sum1+=a[i][j];
				c[i][1]=Math.max(c[i][1], sum1);
				sum2+=a[i][a[i].length-1-j];
				c[i][2]=Math.max(c[i][2], sum2);
				sum3+=a[i][j];
				if(sum3<0)
					sum3=0;
				c[i][3]=Math.max(c[i][3], sum3);
				c[i][4]=Math.max(a[i][j], c[i][4]);
			}
			c[i][0]=sum;
		}
		for(int i=0; i<m; i++)
		{
			lar=Math.max(lar, c[b[i]-1][4]);
		}
		if(lar<=0)
		{
			sop(lar);
		...
		for(int i=0; i<m; i++)
		{
			ans=Math.max(ans, c[b[i]-1][3]);
			ans=Math.max(sum1+c[b[i]-1][1], ans);
			sum1=Math.max(sum1+c[b[i]-1][0], c[b[i]-1][2]);
		}
		sop(ans);
