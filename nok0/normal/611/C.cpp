...
public:
	...
	CumulativeSum2D(const std::vector<std::vector<T>> &vec) : h(vec.size() + 2), w(vec.empty() ? 0 : vec.front().size() + 2) {
...
void main_() {
	INT(h, w);
	VEC(string, s, h);
	VV<> tate(h - 1, V<>(w)), yoko(h, V<>(w - 1));
	REP(i, h - 1) {
		REP(j, w) {
			if(s[i][j] == '.' and s[i + 1][j] == '.') tate[i][j] = 1;
		}
	}
	REP(i, h) {
		REP(j, w - 1) {
			if(s[i][j] == '.' and s[i][j + 1] == '.') yoko[i][j] = 1;
		}
	}
	CumulativeSum2D<int> ts(tate), ys(yoko);
	ts.build();
	ys.build();
	INT(q);
	while(q--) {
		...
		int res = 0;
		if(gx - sx > 1) {
			res += ts.query(sx, sy, gx - 1, gy);
		}
		if(gy - sy > 1) {
			res += ys.query(sx, sy, gx, gy - 1);
		}
		print(res);
	}
}