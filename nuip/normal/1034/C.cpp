...
int main(){
	...
	for(ll i=1;i*i<=s;++i)if(s%i==0){
			facts.pb(i);
			if(i*i<s) facts.pb(s/i);
		}
	sort(all(facts));
	int m=facts.size();
	map<ll,int> inv;
	rep(i,m) inv[facts[i]]=i;
	vector<int> cnt(m);
	function<ll(int)> dfs=
		[&](int v){
			ll re=a[v];
			for(int w:g[v]) re+=dfs(w);
			++cnt[inv[__gcd(s,re)]];
			out(v,re,1);
			return re;
		};
	...
	for(ll p:ps){
		rrep(i,m)if(facts[i]%p==0) cnt[inv[facts[i]/p]]+=cnt[i];
		// rrep(i,m)if(facts[i]%p==0) out(p,i,inv[facts[i]/p],1);
		out(p,cnt,1);
	}
	vector<ll> valid;
	rep(i,m)if(cnt[i]*facts[i]==s) valid.pb(i);
	out(cnt,facts,valid,1);
	vector<mint> dp(valid.size(),1);
	rep(i,valid.size()){
		rep(j,i)if(facts[valid[i]]%facts[valid[j]]==0) dp[i]+=dp[j];
	}
	int re=dp.back().v;
	printf("%d\n",re);
  return 0;
