...
signed main()
{
	...
	int T=read();rg(T)
	int n=read(),m=read();
	rg(n)h[i]=0;mw[i]=inf;gr
	cnt=2;
	int tot=inf;
	rg(n)rg_(j,n)f[i][j]=inf;gr gr 
	rg(n)f[i][i]=0;gr 
	rg(m)
	...
	addedge(v,u,1);
	wt[i]=w;
	// mne(mw[u],w),mne(mw[v],w),mne(tot,w);
	gr
	// rg(n)rg_(j,n)addedge(i,j,2*mw[i]);gr gr
	// dij(1,d,n);
	// dij(n,d2,n);
	rg_(k,n)rg(n)rg_(j,n)f[i][j]=min(f[i][k]+f[k][j],f[i][j]);gr gr gr
	
	int ns=inf;
	rg_(u,n)fe(u)
	mne(ns,(f[1][u]+f[v][n]+1)*wt[i>>1]);
	gr gr
	// puts("");
	rg_(u,n)fe(u)rg_(k,n)
	mne(ns,wt[i>>1]*(f[1][k]+f[k][n]+min(f[u][k],f[v][k])+2));
	gr gr gr
	oldl(ns);
	// oldl(d[n]);
	gr
