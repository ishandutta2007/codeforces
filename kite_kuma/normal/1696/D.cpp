...
int solve() {
	...
	VEC(int, a, n);
	add(a, -1);
	segment_tree<int> stmin(
		n, [](int a, int b) { return min(a, b); }, inf, a);
	segment_tree<int> stmax(
		n, [](int a, int b) { return max(a, b); }, -inf, a);
	int now = 0;
	set<int> rest(a.begin() + 1, a.end());
	vi rev(n);
	rep(i, n) rev[a[i]] = i;
	int ans = 0;
	vector<int> nextup(n), nextdn(n);
	drep(i, n) {
		int j = i + 1;
		while(j < n && a[i] > a[j]) {
			j = nextup[j];
	...
	while(now < n - 1) {
		...
		int nxt = now + 1;
		bool up = a[nxt] > a[now];
		int left = (up ? nextdn[now] : nextup[now]);
		int val = (up ? stmax.get(now, left) : stmin.get(now, left));
		now = rev[val];
		// int jump = nxt;
		// int mn, mx;
		// mx = mn = a[now];
		// int jumpok;
		// while(jump < n) {
		// 	if((a[jump] > a[now]) != up) break;
		// 	chmin(mn, a[jump]);
		// 	chmax(mx, a[jump]);
		// 	if(a[jump] == mx || a[jump] == mn) {
		// 		jumpok = jump;
		// 	}
		// 	jump++;
		// }
		// jump = jumpok;
		// rep(j, now, jump) { rest.erase(a[j + 1]); }
		// now = jump;
		// debug(jump);
		ans++;
	}
