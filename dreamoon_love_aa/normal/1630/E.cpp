...
void calc(int cn){
    ...
    FOR(i,1,n){
        if(!cnt[i])continue;
        int num=cnt[i]/dup_cnt;
        if(num>1){
            ADD(v,num*(num-1LL)%MOD*fac[cn-2]%MOD*cn);
        }
        v2=v2*comb.C(rn,num)%MOD;
        rn-=num;
    }
    dp[cn]=v*v2%MOD*inv(fac[cn])%MOD;
    sp[cn]=tt;
    dp2[cn]=v2;
}
void solve() {
    tt++;
    FOR(i,1,n)cnt[i]=dp[i]=dp2[i]=0;
    FOR(i,1,n)cnt[a[i]]++;
    int gg=0;
    FOR(i,1,n){
        if(!cnt[i])continue;
    ...
    FOR(i,2,n){
        if(sp[i]==tt){
            ...
            for(int j=i+i;j<=n;j+=i){
                if(sp[j]!=tt)continue;
                dp[j]=(dp[j]-dp[i]*(j/i))%MOD;
                if(dp[j]<0)dp[j]+=MOD;
            }
            ADD(an,dp[i]*(n/i)%MOD*inv(i));
            ADD(num,dp2[i]*inv(i));
        }
    }
    an=n-an*inv(num);
