...
VI prefixes(const string& A, const string& B) {
  debug("yes");
  size_t a = 1;
  VI res(A.size()+1, INF);
  res[0] = 0;
  rep(b, B.size()) {
    while (a <= A.size() and A[a-1] != B[b]) {
      a++;
      debug(a); debug(b);
    }
    if (a > A.size()) break;
...
int main() {
  string A = IN, B = IN;
  auto left = prefixes(A, B);
  reverse(all(A)); reverse(all(B));
  auto right = prefixes(A, B);
  reverse(all(A)); reverse(all(B));
...
debug("hi");
  ...
  int len = 0, L = 0, R = 0;
  repr(r, A.size()+1) {
    prefix.insert(left[A.size() - r]);
    auto it = prefix.upper_bound(B.size() - right[r]);
    if (it == prefix.begin()) continue;
    it--;
    if (chmax(len, *it + right[r])) L = *it, R = right[r];
  }
  debug(len); debug(L); debug(R);
  if (len == 0) EXIT("-");
  OUT << B.substr(0, L) << B.substr(B.size()-R) << newl;
}