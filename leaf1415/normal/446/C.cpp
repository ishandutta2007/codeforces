...
mint fib[300005], sfib[300005];
void adjust(ll id)
{
	tmp[0] = p[id], tmp[1] = q[id]+p[id];
	rep(i, 2, B-1) tmp[i] = tmp[i-1] + tmp[i-2];
	rep(i, 0, B-1) a[B*id+i] += tmp[i];
	p[id] = q[id] = 0;
}
...
int main(void)
{
	...
	rep(i, 1, n) sfib[i] = sfib[i-1] + fib[i];
	
	rep(i, 1, n) sum[i/B] += a[i];
	...
	rep(z, 1, Q){
		...
		if(t == 1){
			if(L > R){
				...
				rep(i, L, R-1){
					ll s = i*B-l+1, t = (i+1)*B-1-l+1;
					p[i] += fib[s], q[i] += fib[s+1]-fib[s];
					sum[i] += sfib[t] - sfib[s-1];
				}
			}
		}
		else{
			...
			if(L > R){
				...
				rep(i, l, L*B-1) ans += a[i];
				rep(i, R*B, r) ans += a[i];
				rep(i, L, R-1) ans += sum[i];
			}
			cout << ans << "\n";
		}
