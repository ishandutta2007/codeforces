...
pair<vector<int>, pair<ll, int>> gr[N];
int ps[N];
...
inline void solve() {
	...
	while (_--) {
		...
		for (int i = 0; i < m; ++i) {
			...
			gr[i].x.resize(k);
			cin >> gr[i].x;
			gr[i].y.x = accumulate(all(gr[i].x), 0LL);
			gr[i].y.y = i;
		}

		sort(gr, gr + m, [](pair < vector<int>, pair<ll, int> >& a, pair < vector<int>, pair<ll, int> >& b) { return a.y.x * sz(b.x) < b.y.x * sz(a.x); });


		int l = 0;
		for (l = 0; l < m; ++l) {
			if (gr[l].y.x > sz(gr[l].x) * ll(v[l])) {
				break;
			}
		}
		int r = m - 1;
		for (; r >= 0; --r) {
			if (gr[r].y.x > sz(gr[r].x) * ll(v[r])) {
				break;
			}
		}
		if (l == m) {
			...
			for (int i = 0; i < m; ++i) {
				if (curr < i) {
					curr = i;
					while (curr + 1 < m && gr[curr + 1].y.x <= sz(gr[curr + 1].x) * ll(v[curr])) {
						++curr;
					}
				}
				int lo = -1, hi = sz(gr[i].x);
				auto vv = gr[i].x;
				sort(all(vv));
				while (hi - lo > 1) {
					int mid = (hi + lo) / 2;
					ll sm = gr[i].y.x - vv[mid];
					if (sm > ll(sz(vv) - 1) * v[curr]) {
						lo = mid;
					}
					else {
						hi = mid;
		...
		for (int i = l; i < r; ++i) {
			if (gr[i].y.x > sz(gr[i].x) * ll(v[i + 1])) {
				cout << string(ps[m], '0') << endl;
				goto end;
			}
		}
		{
			vector<int> ans(ps[m]);
			int rb = m;
			if (r == m - 1 || gr[r].y.x > sz(gr[r].x) * ll(v[r + 1])) {
				rb = r + 1;
			}
			for (int i = r; i < rb; ++i) {
				int lo = -1, hi = sz(gr[i].x);
