...
int main() {
    ...
    rep(i, n + 40) {
        ...
        while(!q.empty()) {
            ...
            for(auto e : g[i & 1][now]) {
                if(chmin(d[e], dist + 1)) { q.emplace(dist + 1, e); }
            }
            vis.emplace_back(dist, now);
        }
        if(i > 40 and ans != inf<ll>) {
            cout << mint(ans) << endl;
            exit(0);
        }
        if(d[n - 1] != inf<int>) {
            if(i > 40) {
                cout << modpow(2, i) - 1 + d[n - 1] << endl;
                exit(0);
            }
            chmin(ans, (1LL << i) - 1 + d[n - 1]);
        }
        for(auto [dist, now] : vis) {
            for(auto e : g[~i & 1][now]) {
                if(chmin(d[e], dist + 1)) { q.emplace(dist + 1, e); }
            }
        }
