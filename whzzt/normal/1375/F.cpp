...
int main(){
	...
	printf("First\n"); fflush(stdout);
	printf("%lld\n",inf); fflush(stdout); scanf("%d",&p); if(!p) exit(0); A[p]+=inf;
	i64 z=max(max(A[1],A[2]),A[3]);
	i64 y=2*z-(A[1]+A[2]+A[3]-z);
	printf("%lld\n",y); fflush(stdout); scanf("%d",&p); if(!p) exit(0); A[p]+=y;
	y=(max(max(A[1],A[2]),A[3])-min(min(A[1],A[2]),A[3]))/2;
	printf("%lld\n",y); fflush(stdout); scanf("%d",&p); if(!p) exit(0);
	throw "GG";
	return 0;
}