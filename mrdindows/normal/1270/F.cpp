...
int a[202020];
const int maxK = 300;
int _cnt[202020 * (maxK + 1)];
int nex[202020];
auto cnt = _cnt + 200001;
int solve() {
    string s;
#ifdef LOCAL
    int n;
    ...
    int ones = 0;
    FI(n) ones += s[i] == '1';
    const int K = ones > n * 0.3 ? 200 : 150;
    for (int i = n - 1; i >= 0; --i) {
        if (i + 1 == n || s[i + 1] == '1') {
            nex[i] = i + 1;
        } else {
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == '1') {
            ...
            while (cur < n) {
                int minLen = cnt * (K + 2);
                if (minLen > n) break;
                int ct = nex[cur] - cur - 1;
                int mid = cur - i + 1;
                int a = ct;
                int b = cz;
                if (a > b) swap(a, b);
                int ost = mid % cnt;
                minLen = max(minLen, mid + (ost ? cnt - ost : 0));
                int from = minLen - mid;
                int to = a + b; // maxLen - mid
                for (int add = from; add <= to; add += cnt) {
                    ans += min(add, a) + 1 - max(add - b, 0);
                }
                ++cnt;
                cur = nex[cur];
            }
