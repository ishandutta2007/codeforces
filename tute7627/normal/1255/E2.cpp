...
int main(){
  ...
  ll n;cin>>n;
  vector<ll>tmpa(n);
  rep(i,0,n)cin>>tmpa[i];
  ll sumbuf=0;
  res=INF;
  rep(i,0,n)sumbuf+=tmpa[i];
  ...
  for(auto z:mp){
    ...
    ll psum=0;
    ll minsum=0;
    rep(i,0,n)a[i]%=z.fi*2;
    rep(i,0,n){
      p[i]=min(a[i],z.fi-psum);
      a[i]=max(0LL,a[i]-(z.fi-psum));
      psum+=p[i];
      //debug(p,p.size());
      if(psum==z.fi){
        ll ret=0;
        ll minbuf=INF;
        rep(j,l,i+1)ret+=(j-l)*p[j];
        ll le_cnt=0;
        rep(j,l,i+1){
          chmin(minbuf,ret);
          le_cnt+=p[j];
        ...
        i--;
        psum=0;
        //cout<<i spa minsum spa minbuf<<endl;
      }
    }
