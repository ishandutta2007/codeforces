...
int main() {
    ...
    while(TT--) {
        ...
        vector<vvvl> dp(2,vvvl(2,vvl(2,vl(c+2))));
        dp[0][0][0][0] = 1;
        rep(i,c+1) {
            int mi = ((m - 1) >> i) & 1;
            int ni = (n >> i) & 1;
            ...
            rep(j,2) {
                rep(k,2) {
                    rep(l,2) {
                        dp[j][(j + ni + k) / 2][(ni + k + l) % 2][i+1] += dp[j][k][l][i];
                        if(mi == 0) {
                            if(j == 1) {
                                dp[1][(j + ni + k) / 2][(ni + k + l) % 2][i+1] += dp[0][k][l][i];
                                dp[1][(ni + k) / 2][(ni + k + l) % 2][i+1] += dp[1][k][l][i];
                            }
                        }
                        else {
                            if(j == 0) {
                                dp[0][(j + ni + k) / 2][(ni + k + l) % 2][i+1] += dp[1][k][l][i];
                                dp[0][(1 + ni + k) / 2][(ni + k + l) % 2][i+1] += dp[0][k][l][i];
                            }
                        }
                    }
                }
