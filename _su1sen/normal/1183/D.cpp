...
int main() {
    ...
    loop(q) {
        ...
        sort(all(cnt), [](const auto &v1, const auto &v2) {
            int s1 = v1[0] + v1[1];
            int s2 = v2[0] + v2[1];
            return s1 == s2 ? v1[1] > v2[1] : s1 > s2;
        });
        while (cnt.back()[0] + cnt.back()[1] == 0) {
            cnt.pop_back();
        }
        int m = cnt.size();
        auto pq = priqueue_comp<int>([&](int i, int j) {
            return cnt[i][1] < cnt[j][1];
        });
        int num = cnt[0][0] + cnt[0][1];
        int x = 0, y = 0;
        int i = 0;
        while (num > 0) {
            while (i < m and cnt[i][0] + cnt[i][1] >= num) {
                pq.push(i++);
            }
            if (pq.size()) {
                int j = pq.top(); pq.pop();
