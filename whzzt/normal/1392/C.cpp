...
int n,a[N],lg[N]; pii f[M][N];
pii ask(int l,int r){
	int d=lg[r-l+1];
	return max(f[d][l],f[d][r-(1<<d)+1]);
}
i64 calc(int l,int r,int v){
	if(l>r) return 0;
	int v0,p;
	tie(v0,p)=ask(l,r);
	i64 ret=max(v-v0,0);
	ret+=calc(l,p-1,min(v,v0))+calc(p+1,r,v0);
	return ret;
}
void work(){
	cin>>n;
	for(int i=1;i<=n;++i) cin>>f[0][i].fi,f[0][i].se=i;
	for(int i=2;i<=n;++i) lg[i]=lg[i-1]+!(i&(i-1));
	for(int l=2,i=1;l<=n;l<<=1,i++){
		for(int j=1,k=l>>1;j+k-1<=n;++j){
			f[i][j]=max(f[i-1][j],f[i-1][j+k]);
		}
	}
	cout<<calc(1,n,0)<<'\n';
}
