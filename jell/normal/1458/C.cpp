#line 1 "codeforces-workspace/contest/1458/c/c.cpp"
/**
 * @file template.cpp
 * @brief Template
 */
...
#line 15 "codeforces-workspace/contest/1458/c/c.cpp"
...
void main() {
  ...
  for (auto c : op) {
    if (c == 'I' or c == 'C') {
      if (!sf.empty() && sf.back() == c) {
        sf.pop_back();
      } else {
        sf += c;
        if (sf.size() == 6) sf.clear();
      }
    } else {
      for (auto d : reversed(sf)) {
        if (d == 'I') {
          if (c == 'L') {
            c = 'N';
          } else if (c == 'R') {
  ...
  // prefix
  {
    ...
    csh %= n;
    if (csh < 0) csh += n;
    rotate(begin(a), begin(a) + rsh, end(a));
    for (auto &&r : a) {
      rotate(begin(r), begin(r) + csh, end(r));
    }
  }

  // suffix
  {
    auto inv = [&](vector<int> &p) {
      vector<int> r(n);
      for (auto i = 0; i < n; i++) r[p[i]] = i;
      p = r;
    };
    auto trans = [&](vector<vector<int>> &m) {
      auto r = m;
      for (auto i = 0; i < n; i++) {
        for (auto j = 0; j < n; j++) r[i][j] = m[j][i];
      }
      m = r;
    };
