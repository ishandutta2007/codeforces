...
int main(int argc, char** argv) {
    ...
    vector<vector<int>> bl(n), br(n);
    for (int i = 0; i < m; i++) {
        cin >> qs[i].first >> qs[i].second;
        qs[i].first--;
        qs[i].second--;
        bl[qs[i].first].push_back(i);
        br[qs[i].second].push_back(i);
        st.upd(0, 0, n, qs[i].first, qs[i].second + 1, -1);
    }
    ...
    for (int i = 0; i < n; i++) {
        for (int id : bl[i]) {
            st.upd(0, 0, n, qs[id].first, qs[id].second + 1, 1);
        }
        int mn = min(st.get(0, 0, n, 0, i), st.get(0, 0, n, i + 1, n));
        mn = min(mn, a[i]);
        if (a[i] - mn > ans) {
            ans = a[i] - mn;
            ai = i;
    ...
    vector<int> av;
    for (int i = 0; i < ai; i++) {
        av.insert(av.end(), begin(br[i]), end(br[i]));
    }
    for (int i = ai + 1; i < n; i++) {
        av.insert(av.end(), begin(bl[i]), end(bl[i]));
    }
