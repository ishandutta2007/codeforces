...
int main() {
    ...
    // if (a[i] - a[j] > 1) -> a[j] in [1..a[i] - 2]
    //  ans += a[i] - a[j]
    // a[i] * cnt - sum

    // if (a[j] - a[i] > 1) -> a[j] in [a[i] + 2 .. inf]
    // ans += a[i] - a[j]
    // a[i] * cnt - sum
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i != 0) {
            int cntgood = i - (cnt[x - 1] + cnt[x] + cnt[x + 1]);
            ll sumgood = sum - (1ll * (x-1) * cnt[x-1] + 1ll * x * cnt[x] + 1ll * (x+1) * cnt[x + 1]);

            // cerr << bigint(1ll * cntgood * x - sumgood) << endl;
            ans += bigint(1ll * cntgood * x - sumgood);
            // cerr << ans << endl;
        }
