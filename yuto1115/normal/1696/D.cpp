...
void solve() {
    ...
    scan(a);
    vp init(n);
    rep(i, n) init[i] = {a[i], a[i]};
    segtree<M> st(init);
    int ans = 0, p = 0;
    while (p < n - 1) {
        ++ans;
        if (a[p] < a[p + 1]) {
            int r = st.max_right(p + 1, [&](P now) { return now.first > a[p]; });
            int mx = st.prod(p + 1, r).second;
            p = st.max_right(p + 1, [&](P now) { return now.second < mx; });
        } else {
            int r = st.max_right(p + 1, [&](P now) { return now.second < a[p]; });
            int mn = st.prod(p + 1, r).first;
            p = st.max_right(p + 1, [&](P now) { return now.first > mn; });
        }
    }
    print(ans);
}
