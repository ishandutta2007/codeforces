...
int main(){
	...
	vector<mint> fib(1123456);
	fib[0]=fib[1]=1;
	reps(i,2,fib.size()) fib[i]=fib[i-1]+fib[i-2];
	cauto n=read();
	cauto p=read();
	cauto vs=readV(n)-distinct;
	vector<mint> dp=fib;
	rep(i,p) dp[i+1]+=dp[i];
	mint re=0;
	rep(i,n){
		int ok=1;
		for(auto j=vs[i];j;){
			if(j!=vs[i] && j==*lower_bound(all(vs),j)){ ok=0; break;}
			if(j%2) j/=2;
			else if(j%4) break;
			else j/=4;
		}
