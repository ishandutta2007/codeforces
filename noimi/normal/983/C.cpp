...
main() {
    ...
    rep(i, 10000) {
        if(change[i] == i) {
            ...
            rep(j, 4) {
                ...
                now /= 10;
                if(tmp) {
                    g[i].eb(tmp, change[i - pow(10, j) * tmp]);
                }
            }
        }
    }
    dp[0][1] = 0;
    rep(i, n + 1) {
        auto nxt = vv2(10000, 10, 1e9);
        rep3(j, 9999, 0) {
            if(change[j] != j) continue;
            rep2(k, 1, 9) {
                for(auto e : g[j])
                    chmin(dp[e.se][e.fi], dp[j][k] + abs<int>(k - e.fi));
                if(i != n and j < 1000)
                    chmin(nxt[change[j + 1000 * b[i]]][a[i]],
                          dp[j][k] + abs<int>(a[i] - k));
            }
        }
        if(i != n) dp = nxt;
    }
    int ans = INT_MAX;
    rep2(k, 1, 9) chmin(ans, dp[0][k]);
    cout << ans + n * 2 << endl;
}