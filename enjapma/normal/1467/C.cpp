...
bool solve() {
    ...
    for (auto e : v3) sum3 += e;

    ll mini1 = INF, mini2 = INF, mini3 = INF;

    for (auto e : v1) mini1 = min(mini1, e);
    for (auto e : v2) mini2 = min(mini2, e);
    for (auto e : v3) mini3 = min(mini3, e);

    ll r = min({sum1, sum2, sum3, mini1 + mini2, mini2 + mini3, mini3 + mini1});
