...
constexpr int N = 2e6 + 1, mod = 1e9 + 7;
int a[N], da[N], pn[N];

int fpow(int a, int p) {
  int res = 1;
  while (p) {
    if (p & 1) res = res * (int64_t)a % mod;
    a = a * (int64_t)a % mod;
    p /= 2;
  }
  return res;
...
int main() {
  ...
  int n = $, k = $;
  for (int i = 1, cd = 0; i <= k; ++i) {
    if (i > 2 && i % 2 == 0) {
      pn[i] = pn[i / 2] * (int64_t)pn[2] % mod;
    } else if (i > 3 && i % 3 == 0) {
      pn[i] = pn[i / 3] * (int64_t)pn[3] % mod;
    } else if (i > 5 && i % 5 == 0) {
      pn[i] = pn[i / 5] * (int64_t)pn[5] % mod;
    } else if (i > 7 && i % 7 == 0) {
      pn[i] = pn[i / 7] * (int64_t)pn[7] % mod;
    } else {
      pn[i] = fpow(i, n);
    }
    add(a[i] = pn[i], add(cd, da[i]));
    for (int j = i * 2; j <= k; j += i) {
      add(da[j], mod - a[i]);
    }
    for (int j = i * 2 + 2; j <= k; j += i + 1) {
      add(da[j], a[i]);
    }
  }
  int ans = 0;
  for (auto& i: range{a + 1, k}) {
    add(ans, i ^ &i - a);
  }
  print(ans);;
}