...
void solve()
{
	...
	for (int i = 0; i <= x; ++i)
		for (int j = 0; j <= y; ++j)
			for (int k = 0; k <= z; ++k)
			{
				freshmax(f[i + 1][j + 1][k], f[i][j][k] + a[i] * b[j]);
				freshmax(f[i + 1][j][k + 1], f[i][j][k] + a[i] * c[k]);
				freshmax(f[i][j + 1][k + 1], f[i][j][k] + b[j] * c[k]);
				freshmax(f[i + 1][j][k], f[i][j][k]);
				freshmax(f[i][j + 1][k], f[i][j][k]);
				freshmax(f[i][j][k + 1], f[i][j][k]);
			}
	writeln(f[x][y][z]);
}
