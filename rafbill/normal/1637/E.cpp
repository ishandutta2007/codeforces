...
void solve(){
  ...
  map<li,hash_set<li>> forb;
  for(li i = 0; i < (li)(n); ++i) forb[A[i]].insert(A[i]);
  for(li i = 0; i < (li)(m); ++i) {
    li u,v; cin>>u>>v;
    forb[u].insert(v);
    forb[v].insert(u);
  ...
  map<li,li> cnt; for(li i = 0; i < (li)(n); ++i) cnt[A[i]]++;
  vvi at(n+1);
  for(auto p : cnt) at[p.second].emplace_back(p.first);

  vi I;
  for(li i = 0; i < (li)(n+1); ++i) if(!at[i].empty()) I.emplace_back(i);

  li ans = 0;
  for(li i = 0; i < (li)((li)I.size()); ++i) for(li j = (i); j <= (li)((li)I.size()-1); ++j) {
    li cx = I[i], cy = I[j];
    vi const& vx = at[cx];
    vi const& vy = at[cy];

    li cur = 0;
    for(li kx = ((li)vx.size()-1); kx >= (li)(0); --kx) {
      if(vx[kx] + vy.back() < cur) break;
      hash_set<li> const& fx = forb[vx[kx]];
      for(li ky = ((li)vy.size()-1); ky >= (li)(0); --ky) {
        if(fx.find(vy[ky]) != fx.end()) continue;
        smax(cur, vx[kx]+vy[ky]);
        break;
      }
    }
