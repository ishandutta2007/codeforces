...
bool dfs(int i, bool b) {
    ...
    for (int nb : toward[i]) {
        if (!vis[nb]) {
            if (dfs(nb, !b)) return true;
        } else if (toleft[nb]==b) return true;
        if (b) adj[i].pb(nb);
        else adj[nb].pb(i);
    }
    for (int nb : away[i]) {
...
void solve() {
    cin >> n >> m;
    toward.rsz(n); away.rsz(n); adj.rsz(n);
    F0R(i, m) {
        cin >> t >> u >> v; u--; v--;
        if (t==1) away[u].pb(v), away[v].pb(u);
        else toward[u].pb(v), toward[v].pb(u);
    }
    vis.rsz(n, false); toleft.assign(n, false);
    F0R(i, n) if (dfs(i, true)) {
        cout << "NO\n";
        return;
    }
    vis.assign(n, false);
    F0R(i, n) tsort(i, adj, order);
    vis.assign(n, false); pos.rsz(n);
    int tm = 0;
    for (int i : order) {
        for (int nb : adj[i]) if (!vis[nb]) {
            debug("cycles");
            cout << "NO\n";
            return;
        }
    ...
    cout << "YES\n";
    F0R(i, n) cout << (toleft[i] ? "L " : "R ") << pos[i] << nl;
}
