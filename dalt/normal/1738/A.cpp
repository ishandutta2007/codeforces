...
void SolveOne(int test_id, IStream &in, OStream &out) {
  int N;
  in >> N;
  Vec<Vec<i64>> damage(2);
  Vec<int> type(N);
  in >> type;
  for(int i = 0; i < N; i++) {
    int x;
    in >> x;
    damage[type[i]].push_back(x);
  }
  i64 best = 0;
  for(int i = 0; i < 2; i++) {
    Sort(All(damage[0]), std::greater<i64>());
    Sort(All(damage[1]), std::greater<i64>());
    if(damage[0].size() > 0) {
      damage[0].insert(damage[0].begin(), damage[0].back());
      damage[0].pop_back();
    }
    i64 total = 0;
    var a = damage[0].begin();
    var ae = damage[0].end();
    var b = damage[1].begin();
    var be = damage[1].end();
    int last = -1;
    var eval = [&](i64 damage, int type) {
      if(last != -1 && last != type) {
        total += damage;
      }
      total += damage;
