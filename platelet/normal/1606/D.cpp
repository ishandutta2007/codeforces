...
const int N = 5e5 + 5;
int n, m, a[N], g[N * 2], ct[N], num;
char as[N];
V pmi[N], pma[N], smi[N], sma[N];
struct {
    int m, l[N], r[N], p, pre[N * 2], suf[N * 2];
    V id[N * 2];
    void init() {
        m = 0;
        rep(i, 1, n) g[++m] = l[i], g[++m] = r[i];
        sort(g + 1, g + m + 1);
        m = unique(g + 1, g + m + 1) - g - 1;
        rep(i, 1, m + 1) pre[i] = suf[i] = 0, id[i].clear();
        rep(i, 1, n) suf[lb(g + 1, g + m + 1, l[i]) - g]++, pre[r[i] = lb(g + 1, g + m + 1, r[i]) - g]++, id[r[i]].pb(i);
        rep(i, 1, m) pre[i] += pre[i - 1];
        per(i, m, 1) suf[i] += suf[i + 1];
        p = 0;
    }
    int chk() { return pre[p] && suf[p + 1] && pre[p] + suf[p + 1] == n; }
    void next(int t) {
        for(int i : id[++p]) {
            if(!ct[i]) num--;
            ct[i] += t;
    ...
    void print() {
        puts("YES");
        rep(i, 1, n) as[i] = 'R';
        rep(i, 1, p) for(int j : id[i]) as[j] = 'B';
        rep(i, 1, n) putchar(as[i]);
    }
} L, R;
...
void solve() {
    cin >> n >> m;
    rep(i, 1, n) {
        for(auto v : {pmi, pma, smi, sma}) v[i].resize(m + 2);
        pmi[i][0] = smi[i][m + 1] = 1e9, pma[i][0] = sma[i][m + 1] = 0;
        rep(j, 1, m) scanf("%d", &a[j]), pmi[i][j] = min(pmi[i][j - 1], a[j]), pma[i][j] = max(pma[i][j - 1], a[j]);
        per(j, m, 1) smi[i][j] = min(smi[i][j + 1], a[j]), sma[i][j] = max(sma[i][j + 1], a[j]);
    }
    For(j, 1, m) {
        rep(i, 1, n) L.l[i] = pmi[i][j], L.r[i] = pma[i][j], R.l[i] = -sma[i][j + 1], R.r[i] = -smi[i][j + 1], ct[i] = 0;
        num = 0, L.init(), R.init();
        int k = 0;
        rep(i, 1, L.m) {
            L.next(1);
            if(!L.chk()) continue;
            while(R.p < R.m && R.pre[R.p] < L.pre[L.p]) R.next(-1);
            if(!R.chk() || num) continue;
            return L.print(), printf(" %d\n", j), void();
        }
    }
    puts("NO");
}
