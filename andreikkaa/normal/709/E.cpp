...
template<typename T> inline ostream &operator<<(ostream &out, const vector<T> &v) { if (v.empty()) return out; out << v.front(); for (auto it = ++v.begin(); it != v.end(); ++it) out << '\n' << *it; return out; }
...
void dfs_ans(int v, int p, int ucut)
{
	int upp = n - sz[v];
	
	if (upp > goodsz)
	{
		ans[v] = upp - ucut <= goodsz;
	}
	else if (sz[son[v]] > goodsz)
	{
		ans[v] = sz[son[v]] - dcut[son[v]] <= goodsz;
	}
	else
	{
		ans[v] = true;
	...
	vector<int> prf(g[v].size() + 2), suf(g[v].size() + 2);
	for (int i = 0; i < g[v].size(); ++i)
	{
		...
		prf[i + 1] = max(prf[i + 1], dcut[g[v][i]]);
		if (sz[g[v][i]] <= goodsz and prf[i + 1] < sz[g[v][i]])
		{
			prf[i + 1] = sz[g[v][i]];
		}
	}
	for (int i = g[v].size() - 1; i >= 0; --i)
	{
		...
		suf[i + 1] = max(suf[i + 1], dcut[g[v][i]]);
		if (sz[g[v][i]] <= goodsz and suf[i + 1] < sz[g[v][i]])
		{
			suf[i + 1] = sz[g[v][i]];
		}
	}
	...
	for (int i = 0; i < g[v].size(); ++i)
	{
		...
		dfs_ans(g[v][i], v, max({ ucut, prf[i], suf[i + 2] }));
	}
}
