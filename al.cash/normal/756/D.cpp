...
int main() {
  ...
  read(n, t);
  REP(i, n) if (i == 0 || t[i] != t[i-1]) s += t[i];
  int m = sz(s);
  REP(i, m) s[i] -= 'a';

  REP(a, A) f[m][a] = m;
  FORD(i, m-1, 0) {
    REP(a, A) f[i][a] = f[i+1][a];
    f[i][s[i]] = i;
  }

  REP(i, m) if (f[i+1][s[i]] == m) dp[1][i] = 1;
  FOR(j, 2, n+1) {
    sum[0] = 0;
    REP(i, m) sum[i+1] = sum[i] + dp[j-1][i];
    FORD(i, m-1, 0) {
      dp[j][i] = sum[f[i+1][s[i]]] - sum[i];
    }
  }
//  FOR(j, 1, n+1) writeln(j, ": ", dp[j], dp[j]+m);
  Mi ans = 0;
  REP(i, m) ans += dp[n][i];
  writeln(ans);
