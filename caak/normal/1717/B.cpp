...
inline void AC() {
    ...
    a[x][y] = 'X';
    for(_ i = y; i >= 1; i -= m) a[x][i] = 'X';
    for(_ i = y; i <= n; i += m) a[x][i] = 'X';
    for(_ i = x-1, j = y-1 == 0 ? n : y-1; i >= 1; --i, j = j-1 == 0 ? n : j-1) {
        for(_ k = j; k >= 1; k -= m) a[i][k] = 'X';
        for(_ k = j; k <= n; k += m) a[i][k] = 'X';
    }
    for(_ i = x+1, j = y+1 == n+1 ? 1 : y+1; i <= n; ++i, j = j+1 == n+1 ? 1 : j+1) {
        for(_ k = j; k >= 1; k -= m) a[i][k] = 'X';
        for(_ k = j; k <= n; k += m) a[i][k] = 'X';
    }
    for(_ i = 1; i <= n; ++i, clr()) for(_ j = 1; j <= n; ++j) write(a[i][j]);
