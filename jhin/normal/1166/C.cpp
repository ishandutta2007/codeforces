...
cin>>n;
for(int i=0;i<n;i++){cin>>a[i];mulset.push_back(abs(a[i]));}
sort(mulset.begin(),mulset.end());
for(auto it=mulset.begin();it!=mulset.end();it++)
{
    auto it2=lower_bound(it,mulset.end(),2*(*it)+1);
    it2--;
    int tmp=it2-it;
    ans+=tmp;
}
