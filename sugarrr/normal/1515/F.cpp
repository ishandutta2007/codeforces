...
public:
    ...
    // Constructor
    UnionFind(ll sz_,vector<set<l_l>>se,vl a): par(sz_), siz(sz_, 1) {
    ...
    ll merge(ll x, ll y) {
        ...
        y = root(y);
        if (x == y) return 0;
        if(plus[x]+plus[y]<C)return -1;
        // merge technique
        if (siz[x] < siz[y]) swap(x, y);
        siz[x] += siz[y];
        for(auto z:v[y])v[x].insert(z);v[y].clear();
        plus[x]=plus[x]+plus[y]-C;
        par[y] = x;
        q.push({plus[x],x});
        return 1;
    }
...
signed main(){fastio
    ...
    while(1){
        if(q.empty())break;
        l_l T=q.top();q.pop();
        auto itr=uf.v[T.se].begin();
        while(1){
            if(itr==uf.v[T.se].end()){
                break;
            }
            ll op=(*itr).fi;
            ll ufr=uf.merge(T.se,op);
            if(ufr==-1){
                itr++;
            }else if(ufr==0){
                itr=uf.v[T.se].erase(itr);
            }else{
                ANS.pb((*itr).se);
                break;
            }
