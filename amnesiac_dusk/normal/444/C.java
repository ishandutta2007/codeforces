...
public class Main
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		a.tr=new long[2*n];
		a.lazy=new long[2*n];
		a.ln=32-Integer.numberOfLeadingZeros(n-1);
		NavigableMap<Integer,pair> hola=new TreeMap<Integer,pair>();
		for(int i=0; i<n; i++)
			hola.put(i, new pair(i+1,i+1));
		while(m-->0)
		{
			...
			if(ni()==1)
			{
				...
				Map.Entry<Integer, pair> te=hola.floorEntry(l);
				int lol=l;
				if(te.getKey()<l)
				{
					pair holol=te.getValue();
					int temp=te.getKey();
					hola.remove(te.getKey());
					hola.put(temp,new pair(l,holol.b));
					hola.put(l,new pair(holol.a,holol.b));
				}
//				sop("huha");
				while(l<r)
				{
//					sop("huh");
					pair temp=hola.get(l);
					if(temp.a<=r)
					{
						a.update(l, temp.a, Math.abs(temp.b-x));
						int lul=temp.a;
						hola.remove(l);
						l=lul;
					}
					...
					{
						a.update(l, r, Math.abs(temp.b-x));
						int lul=temp.a;
						int ll=temp.b;
						hola.remove(l);
						l=r;
						hola.put(r, new pair(lul,ll));
					}
				}
				hola.put(lol, new pair(r,x));
			}
			else
			{
				int l=ni()-1,r=ni();
