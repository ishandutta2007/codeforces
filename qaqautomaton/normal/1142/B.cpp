...
using io :: putc;
using io :: write;
int a[200005],b[200005],pre[200005];
int last[200005],fa[21][200005];
...
signed main(){
	...
	for(int i=1;i<=n;++i){pre[a[i]]=i==1?a[n]:a[i-1];}
	for(int i=1;i<=m;++i){
		read(b[i]);
	}
	for(int i=0;i<q;++i){
	...
	for(int i=1;i<=m;++i){
		...
		l=i;
		for(int j=0;j<=17;++j)if((n-1)&(1<<j))l=fa[j][l];
		chkmax(nmx,l);
		last[b[i]]=i;
		for(auto j:qs[i])ans[j.y]=(j.x<=nmx)+'0';
	}
	write(ans,'\n');
	return 0;
}