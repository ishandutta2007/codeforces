...
const int N = 200'001;
vector<int> g[N];
vector<int> f(N, -1), fib;
void init() {
    fib.push_back(1); fib.push_back(1);
    f[1] = 1;
    while(true) {
        int tmp = fib.back() + fib[(int)fib.size() - 2];
        if (tmp > N)
            break;
        f[tmp] = (int)fib.size();
        fib.push_back(tmp);
...
int used[N], p[N], sz[N];
int zhfs(int v, int k) {
    sz[v] = 1;
    for (auto to : g[v]) {
        if (to == p[v] || used[to]) continue;
        auto tmp = zhfs(to, k);
        if (tmp != -1) return tmp;
        sz[v] += sz[to];
    }
    if (sz[v] == fib[k - 2] || sz[v] == fib[k - 1]) return v;
    return -1;
}
