...
void fl(int v, int lv, int rv)
{
	if (rv != lv)
	{
		fl(2 * v + 1, lv, (lv + rv) / 2);
		fl(2 * v + 2, (lv + rv) / 2 + 1, rv);
		mno[v] = min(mno[2 * v + 1], mno[2 * v + 2]);
	}
	else
		mno[v] = pii(0, lv);
}
...
void add(int v, int lv, int rv, int ls, int rs, int val)
{
	if ((rs < lv) || (ls > rv))
		return;
	if ((rs >= rv) && (ls <= lv))
	...
	push(v);
	add((v << 1) + 1, lv, (lv + rv) >> 1, ls, rs, val);
	add((v << 1) + 2, ((lv + rv) >> 1) + 1, rv, ls, rs, val);
	mno[v] = min(mno[(v << 1) + 1], mno[(v << 1) + 2]);
}
pii find_min(int v, int lv, int rv, int ls, int rs)
{
	if ((rs < lv) || (ls > rv))
		return pii(1000000000, 1000000000);
	if ((rs >= rv) && (ls <= lv))
	{
		return mno[v];
	}
	push(v);
	pii ans1 = find_min((v << 1) + 1, lv, (lv + rv) >> 1, ls, rs);
	pii ans2 = find_min((v << 1) + 2, ((lv + rv) >> 1) + 1, rv, ls, rs);
...
int main()
{
	...
	for (int i = 0; i < T; ++i)
	{
		...
		bool okk = true;
		for (int i = n; (okk && (i >= 1)); --i)
		{
			pii z = find_min(0, 0, n - 1, 0, n - 1);
			if (z.X != 0)
			{
				cout << "-1\n";
				okk = false;
			...
			{
				int zi = z.Y;
				add(0, 0, n - 1, zi, zi, 1000000000);
				if (nx[zi] != -2)
					add(0, 0, n - 1, zi + 1, nx[zi] - 1, -1);
				//if ((nx[zi] != -2) && (nx[zi] != n))
				//	add(0, 0, n - 1, zi, zi, -1);
				for (int j = 0; j < sz(keks[zi]); ++j)
					add(0, 0, n - 1, keks[zi][j], keks[zi][j], -1);
				perm[zi] = i;
			}
		}
		if (okk)
