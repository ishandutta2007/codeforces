...
const Mint THREE = Mint(3, MOD998);
int main() {
	ll n; R(n);

	Mint ans(0, MOD998);

	fact[0] = fact[1] = Mint(1, MOD998);
	for (int i = 2; i < MAXN; i++) fact[i] = fact[i - 1] * i;
	inv[MAXN - 1] = fact[MAXN - 1].inv();
	for (int i = MAXN - 1; i >= 1; i--) inv[i - 1] = inv[i] * i;
	...
	for (int i = 1; i <= n; i++) {
		int mul = (i % 2 ? 1 : -1);
		ll p = (n - i) * n + i;
		ans += C(n, i) * THREE.pow(p) * mul * 2;
	}

	for (int r = 1; r <= n; r++) {
		int mul = (r % 2 ? 1 : -1);
		Mint sig = ((THREE.pow(n - r) - 1).pow(n) - THREE.pow(n * (n - r))) * mul;
		ans += C(n, r) * sig * THREE;
	}
