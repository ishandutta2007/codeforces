...
mint dfs(int v,int p){
	vector<mint> prob;
	for(int w:g[v])if(w!=p) prob.pb(dfs(w,v));
	mint prod=1;
	for(mint p:prob) prod*=inv[2]*(1+p);
	re+=1-prod;
	// if(p>=0) re+=prod*inv[2];
	out(v,p,prob,prod,1);
	// rep(p,10)reps(q,1,10)if((prod*q).v==p){ out(p,q,1); break;}
	return 1-prod;
}
...
int main(){
  ...
  int n;
	...
	int rt;
	rep(i,n)if(g[i].size()==1) rt=i;
	dfs(rt,-1);
	cout<<re*modpow(2,n)<<endl;;
	out(re,1);
	// rep(p,10)reps(q,1,10)if((re*q).v==p){ out(p,q,1); break;}
  return 0;
}