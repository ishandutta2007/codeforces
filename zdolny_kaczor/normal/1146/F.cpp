...
ll can_cut(int x) {
	return (dp[0][x] + dp[2][x]) % mod;
}
ll can_nocut(int x) {
	return (dp[1][x] + dp[2][x]) % mod;
}
void dfs(int x) {
	if (graf[x].empty()) {
		dp[2][x] = 1;
		debug << arr(dp[0], x) arr(dp[1], x) arr(dp[2], x);
		return;
	}
	for (int y : graf[x]) dfs(y);
	dp[0][x] = 1;
	for (int y : graf[x]) {
		dp[0][x] = dp[0][x] * can_cut(y) % mod;
	}
	ll all = 1;
	for (int y : graf[x])
		all = all * (dp[0][y] + dp[1][y] + 2 * dp[2][y]) % mod;
	debug << imie(x) imie(all);
	vector <ll> prefs, sufs, vals;
	ll sum = 0;
	int deg = graf[x].size();
	vals.resize(deg);
	sufs.resize(deg);
	...
	for (int i = 0; i < (int)graf[x].size(); ++i)
		vals[i] = can_cut(graf[x][i]);
	prefs[0] = 1;
	for (int i = 1; i < deg; ++i)
		prefs[i] = prefs[i - 1] * vals[i - 1] % mod;
	sufs.back() = 1;
	for (int i = deg - 2; i >= 0; --i)
		sufs[i] = sufs[i + 1] * vals[i + 1] % mod;
	for (int i = 0; i < deg; ++i)
		sum = (sum + prefs[i] * can_nocut(graf[x][i]) % mod * sufs[i]) % mod;
	dp[1][x] = sum;
	dp[2][x] = (all - dp[1][x] - dp[0][x]) % mod;
	debug << arr(dp[0], x) arr(dp[1], x) arr(dp[2], x);
}
int main() {
	debug << imie(mod);
