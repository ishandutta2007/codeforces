...
public:
	void solve(std::istream& inp, std::ostream& outp) {
        ...
        auto h = in.readTable<int>(10, 10);
        vector<vi> id(10, vi(10));
        vector<pii> pos(100);
        int row = 9;
        int col = 0;
        int current = 0;
        ...
        for (int i = 98; i >= 0; i--) {
            ...
            for (int j = 1; j <= 6; j++) {
                if (i + j >= 100) {
                    ...
                    auto[r, c] = pos[i + j];
                    answer[i] += min(answer[id[r][c]], answer[id[r - h[r][c]][c]]);
                }
            }
            answer[i] = 1 + answer[i] / 6;
            answer[i] /= (1 - back / double(6));
        }
