...
int main() {
	...
	cin.tie(nullptr);
	constexpr const int maxn = 1024;
	vi inv(maxn, 1), fac(maxn, 1), fiv(maxn, 1);
	rep(i, 2, maxn) {
		inv[i] = mul(inv[mod % i], mod - mod / i);
		fac[i] = mul(fac[i - 1], i);
		fiv[i] = mul(fiv[i - 1], inv[i]);
	}
	auto C = [&] (int n, int k) -> int { return mul(fac[n], fiv[k], fiv[n - k]); };
	int n, m, ans = 0;
	cin >> n >> m;
	rep(i, 1, n + 1) ans = add(ans, mul(C(m + i - 1, m), C(m + n - i - 1, m - 1)));
	cout << ans << endl;
}