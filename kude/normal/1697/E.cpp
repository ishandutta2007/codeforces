...
mint perm(int n, int k) {
  ...
  rep(i, n) {
    rep(j, n) if (j != i) {
      int d = dist(i, j);
      if (d == mn[i]) neighbors[i].emplace_back(j);
      else if (d < mn[i]) {
        mn[i] = d;
        neighbors[i].clear();
    ...
    neighbors[i].insert(lower_bound(all(neighbors[i]), i), i);
  }
  VI cluster;
  vector<char> done(n);
  rep(i, n) if (!done[i]) {
    ...
    if (ok) {
      cluster.emplace_back(cnt);
      for(int j: vs) done[j] = true;
    } else {
  ...
  for(int x: cluster) {
    ndp.assign(n + 1, 0);
    rep(i, n + 1) if (dp[i].val()) {
      ndp[i-1] += i * dp[i];
      if (x > 1) ndp[i-x] += perm(i, x) * dp[i];
    }
    swap(dp, ndp);
  }
