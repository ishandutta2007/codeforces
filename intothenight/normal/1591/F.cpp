...
int main(){
	...
	int n;
	cin >> n;
	vector<pair<int, modular>> dp;
	modular c = 1, d = 0, sum = 0;
	{
		int x;
		cin >> x;
		dp.push_back({0, 1});
	...
	for(auto i = 1; i < n; ++ i){
		...
		modular rem = sum;
		while(dp[(int)size(dp) - 2].first >= x){
			rem -= (dp[(int)size(dp) - 2].second * c + d) * (dp.back().first - dp[(int)size(dp) - 2].first);
			dp.pop_back();
		}
		rem -= (dp[(int)size(dp) - 2].second * c + d) * (dp.back().first - x);
		c = -c;
		d = sum - d;
		dp.back() = {x, -d / c};
		sum = sum * x - rem;
