...
int main(void)
{
	...
	cin >> h >> w >> Q;
	rep(y, 1, h) rep(x, 1, w) cin >> c[(x-1)*h+y];
	rep(i, 1, h*w) if(c[i] == '*') bit.add(i, 1);
	
	ll x, y;
	rep(i, 1, Q){
		cin >> y >> x;
		ll p = (x-1)*h+y;
		if(c[p] == '*') c[p] = '.', bit.add(p, -1);
		else c[p] = '*', bit.add(p, 1);
		
		ll cnt = bit.query(1, h*w);
		cout << bit.query(h*w) - bit.query(cnt) << "\n";
	}
