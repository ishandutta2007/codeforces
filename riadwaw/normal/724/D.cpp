...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		for (char c = 'a'; c <= 'z'; ++c) {
			...
			poses.push_back((int) s.size());

			bool can = true;
			for (int i: range(1, (int) poses.size())) {
				if (poses[i] - poses[i - 1] > m) {
					can = false;
					break;
				}
			}
			...
			if (can) {
				vector<int> used_poses;
				BottomUpMinSegmentTree<int> tree(s.size());
				for (int i: range(m)) {
					if (s[i] < c) {
						tree.set(i, 0);
					}
				...
				for (int i: range(m, (int) s.size())) {
					...
					tree.set(
							i,
							tree.getResult(i - m, i) + (s[i] == c)
					);
				}

				int mn = tree.getResult(s.size() - m, s.size());
