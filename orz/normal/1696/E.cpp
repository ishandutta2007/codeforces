...
int C(const int n, const int k, const int m) {
	static vi fact, ifact;
	int G = max(n + 1, 500010);
	if (fact.size() == 0)
		fact.push_back(1);
	if (fact.size() < G) {
		for (int i = fact.size(); i < G; ++i)
			fact.push_back(prod(fact[i - 1], i, m));
		int ifs = ifact.size();
		ifact.resize(G);
		ifact.back() = inverse(fact.back(), m);
		for (int i = ifact.size() - 2; i >= ifs; --i)
			ifact[i] = prod(ifact[i + 1], i + 1, m);
	}
