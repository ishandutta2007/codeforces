...
typedef long long ll;const ll N=4e5+7,mod=1e9+7;typedef ll aN[N];char is[1<<22],os[1<<24],*i1=is,*i2=is,*o1=os;aN a,s,f;
...
aN next,head,to;ll cnt=0;void add(ll u,ll v){next[++cnt]=head[u],to[head[u]=cnt]=v;}void dfs(ll u,ll fa){f[u]=fa,s[u]=a[u];rev(i,u,v)if(v^fa)dfs(v,u),s[u]^=s[v];}
ll calc(){ll n=readll(),k=readll(),u,v;memset(head,cnt=0,(n+1)<<3);rep(i,1,n)a[i]=readll();rep(i,2,n)u=readll(),v=readll(),add(u,v),add(v,u);dfs(1,0);
if(!s[1])return 1;if(k==2)return 0;ll g=0,h=s[1];rep(i,2,n)if(s[i]==h)rfor(ll u=f[i];u&&~s[u];u=f[u])if(s[u]==0)return 1;else s[u]=-1;rep(i,2,n)if(s[i]==h&&++g==2)return 1;return 0;}
int main(){IO();ll T=readll();while(T --> 0)myputs(calc()?"YES":"NO");return 0;}