...
pair<int, int> re[228][228];

bool rec(int cnt, int mx) {
    if(cnt == 0) {
        re[cnt][mx] = {-1, -1};
        return true;
    }
...
void _main_function() {
    ...
    for(int ans = 1; ans <= n; ++ans) {
        if(rec(n, ans)) {
            ...
            vector<string> a(ans, string(ans, '.'));
            for(int cnt = n, ss = 0, cur = ans; cnt != -1; ++ss) {
                for(int d = 0; d <= re[cnt][cur].y; ++d) {
                    a[ss][ss + d] = 'o';
                    a[ss + d][ss] = 'o';
                }
                tie(cnt, cur) = make_tuple(re[cnt][cur].x, re[cnt][cur].y);
            }
            reverse(all(a));
            cout << sz(a) << endl;
            for(const auto &i : a) {
