...
void solve(){
	int n, Q; cin >> n >> Q;
	str s; cin >> s;
	vector<pair<pair<int,pair<int,int>>,pair<int,int>>> v(n,{{0,{0,0}},{0,0}});
	int l = 0, r = 0, cn = 0;
	for(int i = 0; i < n; ++i){
		v[i].f = {cn,{l,r}};
		if(s[i] == '+')cn++;
	...
	for(int i = n-1; i >= 0; --i){
		v[i].s = {l,r};
		if(s[i] == '+')r++,l=min(0,l+1);
		else l--, r=max(0,r-1);
	}
	while(Q--){
		int a, b; cin >> a >> b;
		a--, b--;
		int l1 = v[a].f.s.f, l2 = v[a].f.f+v[b].s.f;
		int r1 = v[a].f.s.s, r2 = v[a].f.f+v[b].s.s;
