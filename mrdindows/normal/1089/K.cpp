...
Node merge(Node a, Node b) {
    Node res = {a.beginTime, b.endTime, a.freeTime + b.freeTime};
//    DBN(a.beginTime, a.endTime, a.freeTime);
//    DBN(b.beginTime, b.endTime, b.freeTime);
    ll dif = a.endTime - b.beginTime;
    if (dif > 0) {
        ll onFree = min(dif, b.freeTime);
        res.endTime += dif - onFree;
        res.freeTime -= onFree;
    } else {
        res.freeTime -= dif;
    }
...
void update(int cur, int l, int r, int x, int y) {
    if (l == r) {
        ...
        if (x <= m) {
            ...
            update(dcur + 1, m + 1, r, x, y);
        }
        tr[cur] = merge(tr[dcur], tr[dcur + 1]);
    }
}
Node get(int cur, int l, int r, int x) {
    if (l == r) {
        return tr[cur];
    } else {
        int m = (l + r) / 2;
        ...
        if (x <= m) {
            ...
            return merge(tr[dcur], get(dcur + 1, m + 1, r, x));
        }
    }
}
vector<pii> joins;
int solve() {
    ...
    while (q--) {
        ...
        if (c == '+') {
            ...
            int t;
            cin >> t;
            auto ans = get(1, 1, N, t);
//            DBN(ans.beginTime, ans.endTime, ans.freeTime);
            assert(ans.endTime >= t);
            joins.push_back({0, 0});
            cout << ans.endTime - t << endl;
        } else if (c == '-') {
            int idx;
            cin >> idx;
            --idx;
//            DBN(joins[idx]);
            joins.push_back({0, 0});
            update(1, 1, N, joins[idx].first, -joins[idx].second);
        }
    }
    return 0;
}