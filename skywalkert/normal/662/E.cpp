		...
		for(int t1 = 0; t1 <= maxt; ++t1)
		{
			...
			for(int t2 = 0; t2 <= maxt; ++t2)
			{
				...
				for(int i = 1; i < n; ++i)
				{
					...
					cur ^= 1;
					memset(f[cur], -1, sizeof f[cur]);
					for(int dA = 0; dA <= dAL; ++dA)
						for(int dB = 0; dB <= dBL; ++dB)
							for(int dC = 0; dC <= dCL; ++dC)
							{
								int d = a[i][3] - (dA ? s0 * (250 + a[i][0]) / 250 : 0) - (dB ? s1 * (250 + a[i][1]) / 250 : 0) - (dC ? s2 * (250 + a[i][2]) / 250 : 0) <= val;
								for(int A = dA; A <= R0; ++A)
									for(int B = dB; B <= R1; ++B)
										for(int C = dC; C <= R2; ++C)
											if(f[pre][A - dA][B - dB][C - dC] != -1)
												upd(f[cur][A][B][C], f[pre][A - dA][B - dB][C - dC] + d);
							}
				}
				for(int A = 0; A <= R0; ++A)
					for(int B = 0; B <= R1; ++B)
