...
void solve(int n, int k, vector<int> a) {
	int include = dup(n + k, 2);
	vector<int> cnt(n);
	rep(i, n) cnt[a[i]]++;
	int x = 0;
	int y = 0;	// exclusive half-open

	int ans_x = -1, ans_y = n + 10;
	while(x < n) {
		while(include > 0 && y < n) {
			include -= cnt[y++];
		}
		if(include <= 0 && ans_y - ans_x > y - x) {
			ans_x = x;
			ans_y = y;
		}
		include += cnt[x++];
	...
	rep(loop, k) {
		...
		for(int i = now; i < n; i++) {
			if(in_range(a[i], ans_x, ans_y)) {
				count_in_minus_out++;
			} else {
				count_in_minus_out--;
			}
			if(count_in_minus_out > 0) {
				res.emplace_back(now, i + 1);
				now = i + 1;
				break;
			}
		}
		count_in_minus_out = 0;
	}
