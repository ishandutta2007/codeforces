...
int main() {
  ...
  auto Add = [&](int x) {
    int me = h[x];
    while (!st.empty() && h[st.back()] > me) {
      int cur = st.back();
      int bef = 0;
      if ((int) st.size() > 1) {
        bef = st[(int) st.size() - 2] + 1;
      }
      st.pop_back();
      dp.modify(bef, cur, -b[cur]);
    }
    int bef = 0;
    if (!st.empty()) {
      bef = st.back() + 1;
  ...
  vector<long long> debug_dp(n + 1);
  for (int i = 0; i < n; ++i) {
    Add(i);
    dp.modify(i + 1, i + 1, dp.get(0, i).me);
    debug_dp[i + 1] = dp.get(i + 1, i + 1).me;
    #ifdef DEBUG
    for (int j = 0; j <= i + 1; ++j) {
      cerr << j << ' ' <<  debug_dp[j] << ' ' << dp.get(j, j).me - debug_dp[j] << '\n'; 
    }
    cerr << endl;
    #endif
  }
