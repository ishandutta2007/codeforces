...
void dfs(int now){
	//""
	while(SZ(nei[now])){
		multiset<int>::iterator it=nei[now].begin();
		int to=*it;
		nei[now].erase(it);
		nei[to].erase(nei[to].find(now));
		dfs(to);
	}
	ans.pb(now);
}
...
int main(){
	...
	for(map<int,multiset<int> >::iterator it=nei.begin();it!=nei.end();it++){
		if(SZ(it->second)%2){
			cntf++;
			whr=it->first;
		}
