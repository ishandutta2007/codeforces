...
int query(vi a, vi b) {
	...
	for (int x : a) s1 += m[x];
	for (int x : b) s2 += m[x];
	assert(a.size() == set<int>(a.begin(), a.end()).size());
	assert(b.size() == set<int>(b.begin(), b.end()).size());
	for (int x : a) for (int y : b) assert(x != y);
	if (s1 < s2) return -1;
	if (s1 == s2) return 0;
	return 1;
#else
	...
	char ans[20];
	scanf("%s", ans);
	if (strcmp(ans, "WASTED") == 0) exit(0);
	if (strcmp(ans, "FIRST") == 0) return 1;
	if (strcmp(ans, "SECOND") == 0) return -1;
	if (strcmp(ans, "EQUAL") == 0) return 0;
	assert(false);
#endif
}
void solve() {
...
#endif
	set <int> cans;
	while ((int) cans.size() < min(25, n)) cans.insert(1 + rand() % n);
	int lar = -1;
	for (int x : cans) if (lar == -1) lar = x;
	else if (query({x}, {lar}) == 1) lar = x;
	set <int> stones, rest;
	for (int i = 1; i <= n; ++i) if (i == lar) stones.insert(i); else rest.insert(i);
	set <int> gift;
	int ans = -1;
	while (gift.empty()) {
		if (rest.size() <= stones.size()) gift = rest;
		else {
			set <int> test;
			for (int x : rest) if (test.size() < stones.size()) test.insert(x);
			int curr = query(vi(ALL(test)), vi(ALL(stones)));
			assert(curr != 1);
			if (curr == 0) {
				for (int x : test) {
					stones.insert(x);
	...
	assert(stones.size() >= gift.size());
	while (gift.size() > 1u) {
		int spl = gift.size() / 2;
		vi test;
		for (int x : gift) {
			test.push_back(x);
			if ((int)test.size() == spl) break;
		}
		vi oth;
		for (int x : stones) {
			oth.push_back(x);
			if ((int) oth.size() == spl) break;
		}
		int curr = query(vi(ALL(test)), vi(ALL(oth)));
		assert(curr != 1);
		if (curr == 0) {
			for (int x : test) gift.erase(x);
		}
		else gift = set<int>(ALL(test));
	}
	assert(gift.size() == 1u);
	ans = *gift.begin();
	assert(ans != -1);
