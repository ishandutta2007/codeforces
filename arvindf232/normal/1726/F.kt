...
class STCC(keypoints:List<Skey>,v:SI){
    ...
    init{
        ...
        z.radixsort() // assume O(1) uses
        var size = 0
        val filtered = IntArray(z.size)
        for(a in z){
            if(size == 0 || a != filtered[size-1]){
                filtered[size] = a
                size ++
            }
        }
        arr = filtered.copyOf(size)
        n = arr.size
        st = ST(arr.size, v)
    }
    private tailrec fun findceil(l:Int, r:Int, v:Skey):Int{ //gives n it out of bounds
        if(l == r) return l
        val m = (l+r) shr 1
        if(arr[m] >= v) return findceil(l,m,v)
        else return findceil(m+1,r,v)
    }
    private tailrec fun findfloor(l:Int, r:Int, v:Skey):Int{
    ...
    // -1 : negative infitny, n : positive infinity
    fun ceilKey(v:Skey):Int{
        return findceil(0,n,v)
    }
    fun floorKey(v:Skey):Int{
        return findfloor(-1,n-1,v)
    }
    ...
    override fun toString(): String {
        return "\n" + st.compile().joinToString(" ") { String.format("%15s", it) } +
                "\n" + arr.joinToString(" "){String.format("%15s", it)}
//        return "\n" + st.toString() +
    }
}
const val singleCase = true
fun main(){
//    val st = STCC(listOf(3,3,5,5,7),1000)
//    for(i in 1..10){
//        i dei st.findindex(i)
//    }
//    just dei st
//    st.rangeAdd(0,5,10)
//    st.con(2,0)
//    just dei st
    ...
    solve.cases{
        ...
        val move = getline(n-1)
        val keypoints = mint

        var tn = 0
        for(i in 0 until n){
            if(i != 0){
                tn = tn modMinus (move[i-1] % t)
            }
            val start = tn modMinus cycle[i]
            val end = start modPlus green[i]
            keypoints.add(start)
            keypoints.add(end modMinus 1)
...
//            i dei "$start $end"
        ...
        val st = STCC(keypoints,0L)
        for(c in st.arr){
            st.add(c,-c.toLong())
        }
//        just dei st
        val large = (4.04e13).toLong()
        val toobig = (4.04e14).toLong()

        for(i in 0 until n){
            val end = ed[i]
            val start = ss[i]
            val startend = start modMinus 1
            if(start <= end){
                val best1 = minOf(toobig,st.minQuery(end,t-1)) + t
                val best2 = minOf(toobig,st.minQuery(0,start-1))
                st.consider(start, minOf(best1, best2))
                st.rangeAdd(end,t-1,large)
//                just dei st.st
                st.rangeAdd(0,start-1,large)
//                just dei st.st
            }else{
                val best = minOf(toobig,st.minQuery(end,start-1))
                st.consider(start, best)
                st.rangeAdd(end,start-1,large)
            }

//            just dei st
        }
        var ret = move.sumOf { it.toLong() }
        var best = phuge
        for(c in st.arr){
            best = minOf(best,st.minQuery(c,c) + c)
        }
        put(best + ret)
