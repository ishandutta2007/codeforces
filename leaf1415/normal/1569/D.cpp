...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, k){
			cin >> px >> py;
			ll xid = lower_bound(x+1, x+w+1, px) - x;
			ll yid = lower_bound(y+1, y+h+1, py) - y;
			if(x[xid] != px) xmp[xid][yid]++;
			if(y[yid] != py) ymp[yid][xid]++;
		}
		
		ll ans = 0;
		for(auto &mp : xmp){
			//outl(mp.first, mp.second);
			ll sum = 0;
