...
int main() {
  ...
  vector dp(n, vector(5, ModInt(1)));
  function<void(int, int)> dfs = [&](int par, int ver) {
    for (int e : graph[ver]) {
      if (e == par) continue;
      dfs(ver, e);
      dp[ver][0] *= dp[e][2] + (dp[e][0] - dp[e][1] + dp[e][2] - dp[e][3] + dp[e][4]);
      dp[ver][1] *= dp[e][0] - dp[e][1] + dp[e][2] - dp[e][3] + dp[e][4];
      dp[ver][2] *= (dp[e][0] + dp[e][2]) + (dp[e][0] - dp[e][1] + dp[e][2] - dp[e][3] + dp[e][4]);
      dp[ver][3] *= dp[e][0] - dp[e][1] + dp[e][2] - dp[e][3] + dp[e][4];
      dp[ver][4] *= (dp[e][0] - dp[e][1]) + (dp[e][2] - dp[e][3]) + dp[e][4];
    }
  };
  dfs(-1, 0);
  // REP(i, n) REP(j, 5) cout << dp[i][j] << " \n"[j + 1 == 5];
  cout << dp[0][0] - dp[0][1] + dp[0][2] - dp[0][3] + dp[0][4] - 1 << '\n';
  return 0;
}