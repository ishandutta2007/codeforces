...
#line 9 "/Users/nok0/Documents/Programming/nok0/graph/graph.hpp"
...
void main_() {
	...
	Graph G(n);
	REP(i, cnt) {
		auto edge = mf.get_edge(i);
		if(edge.flow) {
			auto u = edge.from, v = edge.to;
			G.add_edge(u, v);
		}
	}
	...
	print(SZ(namori.cycles));
	REP(i, SZ(namori.cycles)) {
		auto &cycle = namori.cycles[i];
		ADD(cycle, 1);
		print(SZ(cycle), cycle);
	}
}