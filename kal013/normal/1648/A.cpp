...
struct node {
    ...
    void apply(int l, int r, array<long long, 2> v) {
      add += v[0];
      cnt += v[1];
    }
...
void solve(){
	...
	vector<vector<array<int, 2>>> pos; 
	{
		...
		pos.resize(cols.size());
		for(int i = 0; i < n; ++i){
			for(int j = 0; j < m; ++j){
				int x = lower_bound(all(cols), mat[i][j]) - cols.begin();
				pos[x].push_back({i, j});
			}
		}
	}	
	...
	segtree<node> dev_l(m), dev_r(m);

	long long ans = 0;
	for(int c = 0; c < pos.size(); ++c){
		for(auto [x, y]: pos[c]){
			auto n1 = dev_l.get(0, y);
			ans += n1.cnt * (x + y) + n1.add;
			// trace(n1.cnt, n1.add);
			if (y + 1 < m){
				auto n2 = dev_r.get(y + 1, m - 1);
				ans += n2.cnt * (x - y) + n2.add;
			}

			dev_l.modify(y, y, array<long long, 2>{-x - y, 1});
			dev_r.modify(y, y, array<long long, 2>{-x + y, 1});
