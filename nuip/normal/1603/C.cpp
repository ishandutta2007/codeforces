...
mint naive2(const vector<int> &vs){
	const int n=vs.size();
	unordered_map<int,mint> cnts;
	mint re=0;
	mint rem=n;
	int mx=0;
	for(cauto &v:vs){
		unordered_map<int,mint> nxt;
		for(cauto &[last,cnt]:cnts){
			if(last>=v){
				nxt[v]+=cnt;
			}else{
				cauto sz=(v+last-1)/last;
...
mint solve(const vector<int> &vs){
	const int n=vs.size();
	mint re=0;
	mint rem=n;
	int mx=0;
	...
	for(cauto &v:vs){
		...
		{
			int i=2;
			for(;i*i<=v;++i) divs.eb((v+i-1)/i);
			if(divs.size() && divs.back()==i) --i;
			for(;i;--i) divs.eb(i);
			reverse(all(divs));
		}
		// out(divs,cur,1);
		...
		for(cauto &lb:divs){
			mint sum=0;
			while(cur.size() && cur.back().X>=lb){
				sum+=cur.back().Y;
				cur.pop_back();
			}
			cauto sz=(v+lb-1)/lb;
			re+=(sz-1)*sum*rem;
			// out(v,lb,(sz-1)*sum*rem,sum,rem,1);
			if(sum.v) nxt.eb(v/sz,sum);
		}
		reverse(all(nxt));
		cur=nxt;
		out(v,re,1);
...
int main(){
	...
	// out(naive2({4,7,18,16}),1);
	// out(solve({4,7,18,16}),1); return 0;
	if(0){
		rep(_,100000){
			cauto n=10;
			vector<int> vs(n);
			for(auto &v:vs) v=rand()%20+1;
			cauto exp=naive2(vs);
			cauto act=solve(vs);
			if(exp!=act){
				out(exp,act,vs,1);
	...
	TT{
		cauto n=read();
		cauto vs=readV(n,0)-reversed;
		// vector<int> vs(n,222); rep(i,n) vs[i]=i+1;
		// for(auto &v:vs) v=rand()%100000+1;
		// naive2(vs);
		cout<<solve(vs) NL;
	}
	return 0;
