...
void solve() {
    INT(n);
    umap<ll, mint> memo[N];
    auto calc = [&](auto &&calc, ll x, int id) -> mint {
        if(x == 0) {
            if(val[id] > 0) return 0;
            else return 1;
        }
        if(memo[id].count(x)) return memo[id][x];
        mint ret = 0;
        for(int i = (val[id] & 1LL ? 1 : 0); i <= n; i += 2) {
            if(i > x) break;
            ret += bc.com(n, i) * calc(calc, (x - i) / 2, id + 1);
        }
        return memo[id][x] = ret;
    };
    LL(l, r, z);
    rep(i, N) {
        val[i] = z / (1LL<<i);
    }
    mint ans = calc(calc, r, 0) - calc(calc, l - 1, 0);
    out(ans);
}
