...
void calc()
{
	...
	for(int i=1;i<N;i++)invi[i]=mul(I[i],F[i-1]);
}
...
int main(){
	...
	for(int t=ri();t--;){
		int n,k;
		rd(n,k);
		vector<int> dp(n+2,0),pro(n+2,1);
		for(int i=1;i<=n;i++){
			pro[i]=mul(pro[i-1],invi[n-i+1]);
		}
		for(int i=1;i<=n;i++){
			int ones=n-i-(i-1)*(k-1);
			if(ones<0)break;
			dp[i]=mul(binom(ones+i,ones),F[i]);
		}
		int ans=0;
		for(int i=1;i<=n;i++){
			int bad=sub(mul(dp[i],n-i),dp[i+1]);
			//printf("%i %i\n",i,bad);
			//int tmp=mul(bad,mul(mul(pro[i+1],F[i]),i));
			int tmp=mul(bad,mul(pro[i+1],i+1));
			ckadd(ans,tmp);
		}
		printf("%i\n",ans);
	}
