...
int main() {
    ...
    int ansv = -1;
    for (int i = 2; i <= n; i++) {
        if (dcmp(Length(p[1] - p[i]) - Length(p[1] - p[clo])) < 0)
            clo = i;
    }
    for (int i = 2; i <= n; i++) {
        if (i == clo) continue;
        if (!LineCoincide(p[1], p[clo], p[i])) {
            if (ansv == -1) ansv = i;
            else {
                if (dcmp(Angle(p[i] - p[1], p[clo] - p[1]) - Angle(p[ansv] - p[1], p[clo] - p[1])) < 0)
                    ansv = i;
                else if (dcmp(Angle(p[i] - p[1], p[clo] - p[1]) - Angle(p[ansv] - p[1], p[clo] - p[1])) == 0 && dcmp(Length(p[i] - p[1]) - Length(p[ansv] - p[1])) < 0)
                    ansv = i;
            }
        }
    }
