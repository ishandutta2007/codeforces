...
public class Main
{
	...
	static pair[] lca;
	static int n;
	static HashMap<Long, Integer> hola=new HashMap<Long, Integer>();
	static int[] anses;
	public static void main(String[] args)
	{
		...
		for(int i=0; i<n-1; i++)
		{
			...
			gra[te].adj.add(te1);
			gra[te1].adj.add(te);
			hola.put(min(te,te1)*(n+0L)+max(te,te1), i);
		}
		lca=new pair[2*n-1];
		ind=new int[2*n-1];
		dfs(-1,0);
		int k=ni();
		segtree lcaa=new segtree(lca, new combiner()
				{
		...
		for(int i=0; i<k; i++)
		{
			...
			gra[te].lol--;
			gra[te1].lol--;
			int lol=lcaa.query(min(ind[te],ind[te1]),max(ind[te],ind[te1])).b;
			gra[lol].lol+=2;
		}
		lcaa=null;
		lca=null;
	...
	static int[]ind;
	static int id=0,indi=0;
	static int dfser(int fr, int at, int lol)
	{
		lol+=gra[at].lol;
		for(int i:gra[at].adj)
			if(i!=fr)
			{
				int te=dfser(at,i,lol);
				anses[hola.get(min(i,at)*(n+0L)+max(i,at))]=lol-te;
				lol=te;
			}
		return lol;
	}
