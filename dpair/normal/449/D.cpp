...
inline void FWT(int *a) {
    for (int mid = 1;mid < lim;mid <<= 1) {
        for (int j = 0;j < lim;j += (mid << 1)) {
            REP(k, 0, mid) {
                if constexpr (opt) Add(a[j | k], a[j | mid | k]);
                else Sub(a[j | k], a[j | mid | k]);
            }
        }
    }
}
...
int n, a[1 << 20 | 5], b[1 << 20 | 5];
signed main() {
    read(n); rep(i, 1, n) ++ a[read()];
    FWT(a); b[0] = 1; rep(i, 1, n) b[i] = ADD(b[i - 1], b[i - 1]);
    REP(i, 0, lim) a[i] = b[a[i]];
    FWT < 0 > (a);
    if(a[0] == b[n]) print(a[0] - 1);
    else print(a[0]);
}
/*
FWT 
