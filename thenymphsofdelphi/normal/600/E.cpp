...
int mxcol[N]; // maximum amount of color in vertex i
map <int, int> mpp[N]; // {color, amount}

void dfs(int u, int p){ // return size of subtree u
    ...
    Fora(v, adj[u]){
        ...
        Fora(&x, mpp[v]){
            mpp[u][x.fi] += x.se;
            if (mpp[u][x.fi] > mxcol[u]){
                mxcol[u] = mpp[u][x.fi];
                ans[u] = x.fi;
            }
            else if (mpp[u][x.fi] == mxcol[u]){
                ans[u] += x.fi;
            }
        }
    }
    mpp[u][a[u]] += 1;
    if (mpp[u][a[u]] > mxcol[u]){
        mxcol[u] = mpp[u][a[u]];
        ans[u] = a[u];
    }
    else if (mpp[u][a[u]] == mxcol[u]){
        ans[u] += a[u];
    }
}
