...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		in >> m;
		int inf = 10000000;
		auto e = make_vector<int>(n, n, inf);
		for (int i: range(m)) {
			int x,y, c;
			in >> x >> y >> c;
			--x, --y;
		...
		auto dp = make_vector<int>(k, 2, n + 1, n + 1, inf);
		for (int l: inclusiveRange(n)) {
			for (int r: inclusiveRange(l, n)) {
				dp[0][0][l][r] = 0;
				dp[0][1][l][r] = 0;
			}
		}
			...
			for (int rig: range(2)) {
				for (int l: inclusiveRange(n)) {
					for (int r: inclusiveRange(l, n)) {
						...
						for (int to: range(l, r)) {
							updateMin(dp[i][rig][l][r], dp[i - 1][1][l][to] + e[w][to]);
							updateMin(dp[i][rig][l][r], dp[i - 1][0][to + 1][r] + e[w][to]);
						}

						//cerr << i << ' ' << rig << ' ' << l << ' ' << r << ' ' << dp[i][rig][l][r] << endl;
					}
				}
			}
		}
		...
		for (int i = 0; i < n; ++i) {
			updateMin(ans, dp[k - 1][1][0][i]);
			updateMin(ans, dp[k - 1][0][i + 1][n]);
		}
