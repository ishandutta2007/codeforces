...
public:
  UFTYPE ufIdentity = -1;
  inline UFTYPE combine(UFTYPE const& x, UFTYPE const& y) {
    return x+y;
  }
  ...
  bool funion(int _x, int _y) {
    ...
    s.erase(MP(ufData[xr],xr));
    s.erase(MP(ufData[yr],yr));
    if (rank[xr] > rank[yr]) swap(xr,yr);
    if (rank[xr]==rank[yr]) rank[yr]++;
    ufParent[xr] = yr;
    ufData[yr]=combine(ufData[xr],ufData[yr]);
    s.insert(MP(ufData[yr],yr));
    return true;
  }
