...
int main() {
    ...
    Combination<mint> COM(1000010);
    int n;
    cin >> n;

    mint inv2 = mint(1) / 2;
    vector<mint> inv(n + 1, 0);  // i 
    for (int i = 2; i <= n; i++) inv[i] = inv[i - 1] * i + COM.fac(i - 1) * (i - 1) * i * inv2;
    struct node {
        mint val, cnt;
        node(mint val, mint cnt) : val(val), cnt(cnt) {}
    };
    auto f = [](node a, node b) { return node(a.val + b.val, a.cnt + b.cnt); };
    auto g = [](node a, mint x) { return node(a.val + x * a.cnt, a.cnt); };
    auto h = [](mint a, mint b) { return a + b; };
    LazySegmentTree<node, mint> seg(n, f, g, h, node(0, 0), 0)  // i 
        ,
        seg_used(n, f, g, h, node(0, 0), 0);
    for (int i = 0; i < n; i++) seg.set_val(i, node(i, 1));
    for (int i = 0; i < n; i++) seg_used.set_val(i, node(1, 1));
    mint ans = 0, cur = 0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        mint sum = seg.query(0, p).val;
        int rest = seg_used.query(0, p).val.value();
        ans += (cur * rest + sum) * COM.fac(n - 1 - i) + inv[n - 1 - i] * rest;

        cur += seg[p].val;
        seg.set_val(p, node(0, 0));
        seg.update(p + 1, n, -1);
        seg_used.set_val(p, node(0, 0));
    }
    ans += cur;
