...
using mint = modint<100'000'000>;

void solve() {
  LL(N1, N2, K1, K2);

  vvv(mint, DP1, N1 + 1, N2 + 1, K1 + 1);
  vvv(mint, DP2, N1 + 1, N2 + 1, K2 + 1);
  DP1[0][0][0] = 1;

  FOR(n1, N1 + 1) FOR(n2, N2 + 1) {
    FOR(k, K1 + 1) {
      auto& x = DP1[n1][n2][k];
      if (n1 < N1 && k < K1) DP1[n1 + 1][n2][k + 1] += x;
      if (n2 < N2) DP2[n1][n2 + 1][1] += x;
    }
    FOR(k, K2 + 1) {
      auto& x = DP2[n1][n2][k];
      if (n2 < N2 && k < K2) DP2[n1][n2 + 1][k + 1] += x;
      if (n1 < N1) DP1[n1 + 1][n2][1] += x;
    }
  }
  mint ANS = 0;
  FORIN(x, DP1[N1][N2]) ANS += x;
  FORIN(x, DP2[N1][N2]) ANS += x;
  print(ANS);
}
