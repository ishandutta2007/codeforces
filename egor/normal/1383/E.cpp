...
public:
    void solve() {
        ...
        modint mult = 1;
        int at = 0;
        while (at < s.size() && s[at] == '0') {
            mult += 1;
            at++;
        }
        if (at == s.size()) {
            out.printLine(mult - 1);
            return;
        }
        vi z;
        while (at < s.size()) {
        ...
        mult *= (z.back() + 1);
        z.back() = 0;
        arr<modint> res(z.size() + 1);
        res[z.size()] = 0;
        vi pos;
        for (int i : RevRange(z.size())) {
            while (!pos.empty() && z[pos.back()] <= z[i]) {
                pos.pop_back();
            }
            if (pos.empty()) {
                res[i] = res[i + 1] * (z[i] + 1) + 1;
            } else {
                res[i] = (res[i + 1] - res[pos.back() + 1]) * (z[i] + 1) + res[pos.back()];
            }
            pos.push_back(i);
        }
        out.printLine(res[0] * mult);
    }
};
