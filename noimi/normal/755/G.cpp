...
template <class T, int maxsize> struct SquareMatrix {
    ...
    const T iden = {1}; // mul
    const T zero = {0}; // add
    const T add(T x, T y) { return x + y; }
    const T mul(T x, T y) {
        auto z = x * y;
        if(z.size() > k) z.resize(k);
        return z;
    }
    sqmat zeros() {
        sqmat mat;
        for(ll i = 0; i < maxsize; i++)
            for(ll j = 0; j < maxsize; j++) mat[i][j] = zero;
        return mat;
    }
    SquareMatrix() {
        n = maxsize;
    ...
    SquareMatrix(size_t N) : n(N) { A = zeros(); };
    SquareMatrix(vector<vector<T>> v) : n(v.size()) {
        for(ll i = 0; i < n; i++)
            for(ll j = 0; j < n; j++) A[i][j] = v[i][j];
    };
    inline const arr &operator[](int k) const { return (A[k]); }

    inline arr &operator[](int k) { return (A[k]); }
    ...
    SquareMatrix &operator+=(const SquareMatrix &B) {
        for(int i = 0; i < n; i++)
            for(int j = 0; j < n; j++) (*this)[i][j] = add((*this)[i][j], B[i][j]);
        return (*this);
    }
    ...
    SquareMatrix &operator*=(const SquareMatrix &B) {
        ...
        for(int i = 0; i < n; i++)
            for(int j = 0; j < n; j++)
                for(int k = 0; k < n; k++) C[i][j] = add(C[i][j], mul((*this)[i][k], B[k][j]));
        A.swap(C);
        return (*this);
    }
    ...
    bool operator==(const SquareMatrix &B) const { return this->A == B.A; }
    bool operator!=(const SquareMatrix &B) const { return this->A != B.A; }
    SquareMatrix &operator=(const SquareMatrix &b) {
        this->n = b.n;
        this->A = b.A;
    ...
    friend ostream &operator<<(ostream &os, SquareMatrix p) {
        for(int i = 0; i < p.n; i++) {
            os << "[";
            for(int j = 0; j < p.n; j++) { os << p[i][j] << (j + 1 == p.n ? "]\n" : ","); }
        }
        return (os);
    }
};
