...
signed main(){
	...
	function<void(int,int)>sub=[&](int l,int r){
		if(r<=l)return;
		vi p;
		while(l<r){
			int v=c[l];
	...
	sparsetable<int,decltype(imin)> st(c,imin);
	
	rep(i,n){
		if(st.get(pos[i].front(),pos[i].back()+1)<i){
			print(0);
			return 0;
		}
	}
