...
int main() {
    ...
    loop(t) {
        ...
        read(x);
        vector<vector<uint8_t>> s(n, vector<uint8_t>(m));
        rep(i, n) {
            input(string, si);
            rep(j, m) {
                s[i][j] = si[j] == '1';
        ...
        rep(mask, 1 << n) {
            vector<int> coef(m, 0);
            rep(j, m) rep(i, n) {
                if (s[i][j]) coef[j] += pow_m1(kth_bit(mask, i));
            }
            vector<int> p(m);
            iota(all(p), 0);
            sort(all(p), [&](int i, int j) { return coef[i] < coef[j]; });
            long long score = 0;
            rep(j, m) {
                score += coef[p[j]] * (j + 1);
            }
            rep(i, n) {
                score -= pow_m1(kth_bit(mask, i)) * x[i];
            }
