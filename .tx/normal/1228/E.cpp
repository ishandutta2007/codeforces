...
void solve() {
    ...
    auto c = make_vector<llong>(n + 1, n + 1);
    for (int i = 0; i <= n; i++) {
        c[i][0] = c[i][i] = 1;
        for (int j = 1; j < i; j++) {
    ...
    auto d = make_vector<llong>(n + 1, n + 1);
    d[0][0] = 1;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j <= n; j++) {
            llong t = (kp[j] - k1p[j] + M) % M * k1p[n - j] % M;
            add(d[i + 1][j], t * d[i][j]);
            for (int z = 1; z + j <= n; z++) {
                llong t = c[n - j][z] * k1p[n - j - z] % M * kp[j] % M;
                add(d[i + 1][j + z], t * d[i][j]);
            }
        }
    }
