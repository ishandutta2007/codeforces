...
void _solve() {
    ...
    int dif = *dp.lower_bound(0);
    int alsum = accumulate(all(a), 0) + accumulate(all(b), 0);

    assert((alsum - dif) % 2 == 0);
    int fir = (alsum - dif) / 2;
    int sec = fir + dif;
    assert(fir + sec == alsum);
    ...
    cout << ll(fir) * fir + ll(sec) * sec + (n - 2) * sumsq << endl;
}
