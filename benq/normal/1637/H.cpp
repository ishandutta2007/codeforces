...
vl smart(vi P) {
	...
	// -1 for everything before it greater
	// +1 for everything before it less

	// dbg("OOP",P,dif);
	// sort(all(dif));
	// vi ans{invs};
	// each(t,dif) ans.pb(ans.bk+t);
	// return ans;
	// auto eval = [&](int i) {
	// 	int cur = invs;
	// 	F0R(x,N) FOR(y,x+1,N) if (!(i&(1<<x))) if (i&(1<<y)) {
	...
	// };
	// vi ans(N+1,MOD);
	// int cur = 0; ans[0] = eval(cur);
	// FOR(i,1,N+1) {
	// 	int nex = MOD;
	// 	F0R(j,N) if (!(cur&(1<<j))) ckmin(nex,eval(cur^(1<<j)));
	// 	ans[i] = nex;
	// 	R0F(j,N) if (!(cur&(1<<j))) if (nex == eval(cur^(1<<j))) {
	// 		cur ^= 1<<j;
	// 		break;
	// 	}
	// }
...
int main() {
	setIO();
	// FOR(N,1,8) {
	// 	vi P; FOR(i,1,N+1) P.pb(i);
	// 	do {
	// 		dbg(brute(P));
	// 		if (brute(P) != smart(P)) {
	// 			dbg(P,brute(P),smart(P));
	// 			exit(0);
	// 		}
