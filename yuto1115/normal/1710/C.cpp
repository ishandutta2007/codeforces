...
int main() {
    ...
    mint pre = 0;
    rep(t, sz) {
        dp[t + 1][1][1][1][0][0][0] += pre * 2;
        if(n[t]) {
            dp[t + 1][1][1][0][0][0][0] += 1;
            dp[t + 1][0][0][1][0][0][0] += 1;
        }
        pre = pre * 2 + n[t];
        rep(ap, 2) rep(bp, 2) rep(cp, 2) {
                    rep(x, 2) rep(y, 2) rep(kuri, 2) {
                                mint now = dp[t][ap][bp][cp][x][y][kuri];
//                                if (!now.val()) continue;
                                rep(a, 2) rep(b, 2) rep(c, 2) rep(jmk, 2) {
                                                if (!ap and n[t] < a) continue;
                                                if (!bp and n[t] < b) continue;
                                                if (!cp and n[t] < c) continue;
                                                int nap = ap | (n[t] > a);
                                                int nbp = bp | (n[t] > b);
                                                int ncp = cp | (n[t] > c);
                                                int i = a ^ b;
                                                int j = b ^ c;
                                                int k = a ^ c;
                                                if (!x and jmk > i) continue;
                                                int nx = x | (jmk < i);
                                                int ny = y | jmk;
                                                int nkuri = kuri * 2 + j - k - jmk;
                                                if (nkuri < 0 or nkuri > 1) continue;
                                                dp[t + 1][nap][nbp][ncp][nx][ny][nkuri] += now;
                                            }
                            }
                }
    }
    mint ans = 0;
    rep(ap, 2) rep(bp, 2) rep(cp, 2) ans += dp[sz][ap][bp][cp][1][1][0] * 6;
    rep(ap, 2) rep(bp, 2) rep(cp, 2) ans += dp[sz][ap][bp][cp][1][0][0] * 3;
    print(ans);
}