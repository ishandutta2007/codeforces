...
signed main() {
    ...
    auto upd = [&](int u) {  // updates if u is good
        if (sz(friends[u]) == 0 || *(--friends[u].end()) < u)
            good.insert(u);
        else
            good.erase(u);
    };
    auto add = [&](int u, int v) { // adds friend u to v
        friends[u].insert(v);
        upd(u);
    };
    auto remove = [&](int u, int v) { // removes friend v from u
        friends[u].erase(v);
        upd(u);
    };
    for (int i = 0; i < n; ++i)
