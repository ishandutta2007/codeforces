...
int main(void)
{
	...
	rep(i, 1, n){
		rep(j, 1, 17) beki[j] = modpow(j, cnt);
		rep(j, 1, 17) a[j][0] += beki[j];
		
		ll l = i-1, r = i+1, mask = 0;
		while(1){
			if(l < 1 || r > n) break;
			if(s[l] != s[r]){
				if(s[l] != '?' && s[r] != '?') break;
			...
			if(s[l] == '?' || s[r] == '?') rep(j, 1, 17) beki[j] /= j;
			rep(j, 1, 17) a[j][mask] += beki[j];
			l--, r++;
		}
	}
	rep(i, 1, n-1){
	...
	rep(j, 1, 17) zeta_transform(a[j], 17);
