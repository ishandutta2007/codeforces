...
struct hasher {
    int operator()(pair<int, int> const &x) const {
        return x.fr * 1234567 + x.sc * 7893215;
    }
};

//gp_hash_table<pair<int, int>, int, hasher> where;

bool can(int n) {
    if (n == 1)
        return 1;
    /*{
        auto w = where.find({n, it});
        if (w != where.end()) {
            it = w->sc;
            if (it == -1)
                return 0;
    ...
    if (p0 == s.size() || ind[p0] + x - 1 >= o[0].size())
        p0 = s.size();
    else
        p0 = o[0][ind[p0] + x - 1];
    if (p1 == s.size() || ind[p1] + y - 1 >= o[1].size())
        p1 = s.size();
    else
        p1 = o[1][ind[p1] + y - 1];
    p0 = min(p0, p1);
...
void solve() {
    ...
    nxt[s.size()][0] = nxt[s.size()][1] = s.size();
    ...
    go(l, 1);
    /*vector<int> cand(int(1e5));
    iota(cand.begin(), cand.end(), 1);
    shuffle(cand);
    for (int n : cand)
        if (len[n].fr <= s.size() && len[n].sc >= s.size() && seg[n][0] >= counter[0] && seg[n][1] >= counter[1]) {
            go(n);
        }*/
}
