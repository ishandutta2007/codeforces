...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	// reps(n,1,10){
	// 	mint re=0;
	// 	reps(i,1,n+1) re+=comb(n,i)*i*i*i*i*i;
	// 	cout<<(re).v<<",";
	// }
	// cout NL;
	int n,t;
	cin>>n>>t;
	vector<mint> pws(112345);
	rep(i,112345) pws[i]=modpow(mint(i),t);
	vector<pmm> xys;
	rep(x,t+10){
		mint re=0;
		reps(i,1,x+1) re+=comb(x,i)*pws[i];
		if(x==n){
			cout<<re NL;
			return 0;
		}
	  xys.eb(x,re*modpow(mint(2),MOD-1-(x-t)));
	}
	out(xys,1);
	auto fun=
		[&](mint x){
			...
			for(auto xy:xys){
				mint prod=xy.Y;
				for(auto p:xys)if(xy.X.v!=p.X.v){
						// prod*=(x-p.X)/(xy.X-p.X);
						prod*=(x-p.X);
						int t=xy.X.v-p.X.v;
						if(t<0){
	...
	// 	mint exp=i*mint(i+1);
	// 	if(re.v!=exp.v){
	// 		out(i,re,exp,1); exit(0);
	// 	}
	// }
	mint re=fun(n);
	out(re,1);
	cout<<re*modpow(mint(2),n-t) NL;
  return 0;
}
//n*2^(n-1)
//n*(n+1)*2^(n-2)
