...
struct ReRooting {
  ...
  ReRooting(int n, F1 f1, F2 f2, const T &ident, vector<Data> data) :
      g(n), ldp(n), rdp(n), lptr(n), rptr(n), f1(f1), f2(f2), ident(ident), data(data) {}
...
void solve(){
    LL(n);
    VEC(ll,h,n);
    ReRooting<ll,pll>g(n,[](pll a,pll b){return pair{a.first+b.first,max(a.second,b.second)};},[](pll a,ll b){if(a.second>=b)return a;return pair{a.first+b-a.second,b};},pll{},h);
    // e{e}
    rep(n-1){
        LL(a,b);
        a--; b--;
    ...
    const ll mx=max(h);
    if(count(all(h),mx)==1){
        rep(n)if(h[i]!=mx)chmin(ans,dp[i].first);
    }
    else rep(n)chmin(ans,dp[i].first);
    out(ans+max(h));
}
int main(){
    const ll t=1;
