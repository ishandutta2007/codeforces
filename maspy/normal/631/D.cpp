...
void solve() {
  ...
  if (len(B) == 1) {
    ll ANS = 0;
    auto [c, x] = B[0];
    FOR(i, N) if (A[i].fi == c) { ANS += max(0LL, A[i].se - x + 1); }
    return print(ANS);
  }
  ...
  if (len(B) == 2) {
    ...
    auto [b, y] = B[1];
    FOR(i, N - 1) {
      if (A[i].fi != a) continue;
      if (A[i].se < x) continue;
      if (A[i + 1].fi != b) continue;
      if (A[i + 1].se < y) continue;
      ++ANS;
    }
    return print(ANS);
  }
  ...
  auto Z = zalgorithm(C);
  Z = {Z.begin() + len(B), Z.end()};

  ll ANS = 0;
  FOR3(i, 1, N - 1) if (Z[i] >= len(B)) {
    if (A[i - 1].fi != a) continue;
    if (A[i - 1].se < x) continue;
    if (A[i + len(B)].fi != b) continue;
    if (A[i + len(B)].se < y) continue;
    ++ANS;
  }
  print(ANS);
}
