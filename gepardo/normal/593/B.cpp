...
{
    int n = getint();
    vector <long long> y1(n), y2(n), k(n), b(n);
    vector < pair<long long, long long> > y(n + 1);
    ...
    {
        cin >> k[i] >> b[i];
        y[i].first = k[i] * x1 + b[i];
        y[i].second = k[i] * x2 + b[i];
    }

    y[n].first = 123456789012345678;
    sort(y.begin(), y.end());
    //cout << "***\n";
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (y[i].first != y[i + 1].first)
        {
            ...
            while (j >= 0 && y[j].first == y[i].first)
            {
                ...
                //cout << "!" << y[j].first << " " << y[j].second << " " << j << endl;
                j--;
            }
            //cout << "---\n";
        }
