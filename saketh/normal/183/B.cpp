...
int main() {
    ...
    vector<P> pts(M); re(pts);

    map<int, int> best;

    F0R (i, M) FOR (j, i+1, M) {
        int ct = 0;
        F0R (k, M) if (cross(pts[k], pts[i], pts[j]) == 0) ct++;

        P ij = pts[j] - pts[i];
        ll g = __gcd(abs(ij.x), abs(ij.y));
        ij.x /= g, ij.y /= g;

        if (ij.y == 0 || pts[i].y % abs(ij.y) != 0) continue;
        P y0 = pts[i] - ij * (pts[i].y / ij.y);
        if (1 <= y0.x && y0.x <= N) ckmax(best[y0.x], ct);
    }
