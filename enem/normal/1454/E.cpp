...
bool dfs(ll x,ll y,ll dst,set<ll> ad[],V(ll) &pth)
{
    pth.PB(x);
    ...
    for(auto it:ad[x])
    {
        if(it!=y)
        {
            if(dfs(it,x,dst,ad,pth)) return true;
        }
    }
    pth.DB();
    return false;
}
ll dfs2(ll x,ll y,set<ll> ad[])
{
    ll t=1;
    for(auto it:ad[x])
    {
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,((ll)v.size())-1)
        {
            ad[v[i]].erase(ad[v[i]].find(v[i+1]));
            ad[v[i+1]].erase(ad[v[i+1]].find(v[i]));
        }
        ll ans=0;
        ll sm=0;
        REP(i,0,((ll)v.size()))
