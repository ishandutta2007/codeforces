...
sort(A.begin(), A.end(), ([&](auto&& _1, auto&& _2, auto&&... _other_args){(void)_1;(void)_2; return B[_1.first][_1.second] < B[_2.first][_2.second];}));
vector<vector<bool>> taken(N, vector<bool>(M, false));
vector<vector<int>> V(N, vector<int>(M, 0));
for(int m = 0; m < M; ++m) {
	taken[A[m].first][A[m].second] = true;
	V[A[m].first][m] = B[A[m].first][A[m].second];
}
for(int m = 0; m < M; ++m) {
	for(int n = 0; n < N; ++n) {
		if(V[n][m] != 0) {
...
for(int n = 0; n < N; ++n) {
	[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(V[n]), "\n");  }();
}
