...
vector<int> go(vector<int> a, int p, int e)
{
	assert(e >= 1);
	...
	for (int i = 0; i < n; ++i)
	{
		for (int j = 1, x = a[n - i - 1] * p; j <= e; ++j, x *= p)
			res.push_back(x);
	}
	int tmp = res.back();
	res.pop_back();
...
void solve()
{
	...
	if (v.size() == 2 && v[0].second == 1 && v[1].second == 1)
	{
		...
		res.push_back(v[0].first * v[1].first);
		print(res);
		writeln(1);
		return;
	}
	...
	if (v[0].second == 1)
	{
		...
		int q = v[1].first;
		int r = v[2].first;
		vector<int> res = { p, p * q * r, p * q, q, q * r, r, p * r };
