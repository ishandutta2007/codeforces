...
void solve() {
    ...
    //     for (int j = 0; sz(a) < n && j < i; j++)
    //         a.push_back(i);
    // }
    // vector<pii> bad_pairs;
    // for (int i = 0; sz(bad_pairs) < m && i < a.back(); i++)
    //     for (int j = i + 1; sz(bad_pairs) < m && j < a.back(); j++)
    ...
    for (auto &[x, y] : bad_pairs) {
        auto u = C.get_index(x);
        auto v = C.get_index(y);
        bad[u].push_back(v);
        bad[v].push_back(u);
    }
    for (auto &[_, v] : bad)
    ...
    for (auto &[k, v] : count) {
        by_count[v].push_back(k);
    }
    for (auto &[_, v] : by_count)
        sort(rall(v));
    vector<pair<int, vector<int>>> by_count_v;
    for (auto &[k, v] : by_count)
        by_count_v.emplace_back(k, v);

    ll ans = 0;
    vector<char> forbidden(C.values.size());
    for (auto x : a) {
        auto &bad_y = bad[x];
        for (auto forbidden_y : bad_y)
            forbidden[forbidden_y] = true;
        forbidden[x] = true;
        auto count_x = count[x];
        auto value_x = C.get_value(x);
        for (auto &[count_y, ys] : by_count_v) {
            for (auto y : ys) {
                if (forbidden[y])
                    continue;
                ckmax(ans, ll(value_x + C.get_value(y)) * (count_x + count_y));
                break;
            }
        }
        for (auto forbidden_y : bad_y)
            forbidden[forbidden_y] = false;
        forbidden[x] = false;
    }
