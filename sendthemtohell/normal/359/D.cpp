...
struct qq {
	vector<int> in_, out_;
	vector<int> gin_, gout_;
	vector<int> min_, mout_;
	...
	void push(int v) {
		if (!sz(in_)) {
			...
			in_.push_back(v);
			gin_.push_back(::gcd(v, gin_.back()));
			min_.push_back(::min(v, min_.back()));
		}
	}
	...
	void pop() {
		if (sz(out_)) {
			...
			while (sz(in_)) {
				...
				out_.push_back(v);
				gout_.push_back(::gcd(v, gout_.back()));
				mout_.push_back(::min(v, mout_.back()));
...
vector<int> f(vector<int>& a, int cnt) {
	qq q;
	vector<int> ans;
	for (int i = 0; i < cnt; ++i) {
		q.push(a[i]);
	}
	if (q.ok()) ans.push_back(1);
	for (int cur = cnt; cur < sz(a); ++cur) {
		q.pop();
		q.push(a[cur]);
		if (q.ok()) ans.push_back(cur - cnt + 2);
	}
	return ans;
}
