...
// p^n ((1-p)/p + x)^n
//
// https://nyaan.hatenablog.com/entry/2019/12/21/171210
...
fn run() {
    ...
    for i in 2..=k {
        for j in 1..=i {
            dp[i][j] = dp[i - 1][j - 1] + dp[i - 1][j] * M::from(j);
        }
    }
    for (n, m, k) in ask {
        let p = M::from((m + 1) / 2) * M::from(m).inv();
        let a = (M::one() - p) * p.inv();
        let mut val = (a + M::one()).pow(n as u64);
        let inv = (a + M::one()).inv();
        let mut ans = M::zero();
        for (i, dp) in dp[k].iter().enumerate().take(k + 1) {
            ans += *dp * val;
            val *= inv * M::from(n - i);
        }
        ans *= p.pow(n as u64) * M::from(m).pow(n as u64);
        println!("{}", ans);
    }
}
