...
int main(){
  ...
  vector<vector<ll>>v(4);
  {
    ...
    rep(i,0,n)v[b[i]].PB(c[i]);
    rep(i,0,4)sort(ALL(v[i]));
  }
  MinimumSum<ll>que(1);
  for(auto z:v[0])que.insert(z);
  res=INF;
  ll precost=0;
  rep(i,1,3)for(auto z:v[i])precost+=z;
  ll idx1=v[1].size(),idx2=v[2].size();
  rep(i,0,v[3].size()+1){
    ll need12=max(0LL,k-i);
    while(idx1>need12){
      idx1--;
      que.insert(v[1][idx1]);
      precost-=v[1][idx1];
    ...
    ll need=m-i-need12*2;
    if(need<0||idx1!=need12||idx2!=need12||need>que.size())continue;
    que.set_k(need);
    //cout<<i spa precost spa need spa que.query()<<endl;
    chmin(res,precost+que.query());
  }
  anss(res,INF,-1);
  return 0;
}