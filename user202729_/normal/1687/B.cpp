// also, read the statement carefully
...
int main(){
	...
	[[maybe_unused]] int const numNode=read<int>();
	int const numEdge=read<int>();

	auto const query=[&](std::string const& s){
		for(auto ch: s) assert(ch=='0' or ch=='1');
		std::cout<<"? "<<s<<std::endl;
		return read<int>(); // 1e6  500 safe
	};

	std::string s(numEdge, '0');
	std::vector<int> weight(numEdge);
	for(int const index: range(0, numEdge)){
		s[index]='1';
		weight[index]=query(s);
		s[index]='0';
	}

	std::vector<int> edgeIndices(std::begin(range(numEdge)), std::end(range(numEdge))); // somehow
	std::sort(begin(edgeIndices), end(edgeIndices), [&](auto const& first, auto const& sec){
		return weight[first]<weight[sec];
	});

	int curWeight=0;
	for(int const index: range(0, numEdge)){
		s[edgeIndices[index]]='1';
		int newWeight=query(s);
		if(newWeight==curWeight+weight[edgeIndices[index]]){
			curWeight=newWeight;
		}else{
			s[edgeIndices[index]]='0';
		}
	}
	std::cout<<"! "<<curWeight<<'\n';
...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1687/problem/B