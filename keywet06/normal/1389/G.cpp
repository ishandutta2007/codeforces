...
int main() {
    ...
    for (int i = 0; i < n; ++i) val[belong[i]] += c[i];
    ...
    dfs0(root, -1);
    for (int i = 0; i < cnt; ++i) {
        if (key[i] == 0) val[top[i]] += val[i];
    }
    for (int i = 0; i < n; ++i) {
        if (key[belong[i]] == 0) belong[i] = top[belong[i]];
    }
    ...
    dfs1(root, -1);
    dfs2(root, -1);
    for (int i = 0; i < n; ++i) std::cout << dp[belong[i]] << " \n"[i == n - 1];
    return 0;
}