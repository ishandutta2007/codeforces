...
inline void solve() {
	...
	while (t--) {
		...
		if (d > mxd || d < md) {
			...
			int cur = n - 1;
			while (cd != d) {
				assert(sz(lvls[cur]) == 1);
				int at = lvls[cur][0].x;
				lvls[cur].pop_back();

				int i;
				for (i = 0; sz(lvls[i]) == 0; ++i);
				...
				p[at - 1] = lvls[i].back().x;
				lvls[i].back().y -= 1;
				if (lvls[i].back().y == 0) lvls[i].pop_back();
				lvls[i + 1].push_back({ at, 2 });
				cd -= cur - i - 1;
				--cur;
			}
			for (auto &i : p) ++i;
