...
const int Maxn=100005,Mxjt=11000005;
char s[Maxn],t[Maxn];
int n,m;
int dp[Mxjt];
bool lgl[Mxjt];
...
struct jtakioi_zidongji{
	int nxt[Maxn][26],ed[Maxn],fail[Maxn];
	int tot=1;
	void insert(char *s){
		int now=1;
		parse(i,s){
...
int main(){
	...
	while(SZ(q)){
		...
		q.pop();
		int i=x.ST,j=x.ND;
		int val=dp[getid(i,j)];
		loop(k,26){
			if(s[i]!='?'&&s[i]!='a'+k){
				continue;
			}
			int jj=ac.nxt[j][k],ii=i+1;
			int nval=val+ac.ed[jj],nid=getid(ii,jj);
			if(dp[nid]<nval){
				chmax(ans,nval);
				dp[nid]=nval;
				if(!lgl[nid]&&ii!=n){
