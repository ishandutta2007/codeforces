...
bool check(ll power) {
	...
	REP(i, n) aplus[i] = a[i];
	REP(i, n) que.push({aplus[i] / b[i], i});
	REP(t, k - 1) {
		debug(imie(que.top()));
		int i = que.top().second;
		que.pop();
		debug(arr(aplus, i), arr(b, i) * imie(t));
		if (aplus[i] < b[i] * t) return false;
		aplus[i] += power;
		debug(arr(aplus, i), arr(b, i) * imie(t + 1));
		if (aplus[i] < b[i] * (t + 1)) return false;
		que.push({aplus[i] / b[i], i});
	}
	REP(i, n) if (aplus[i] < b[i] * (k - 1)) return false;
	return true;
}
int main() {
	scanf("%d%d", &n, &k);
