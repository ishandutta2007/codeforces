// Cut Substrings (https://codeforces.com/contest/1729/problem/G)
...
int main() {
    ...
    while (T--) {
        ...
        vector occ(n, false);
        rep(i, n - m + 1)
            occ[i] = s.substr(i, m) == t;

        using MI = StaticMontgomeryInt<int(1e9) + 7, true>;
        vector dp(n + 1, vector(n + 1, pair(0, MI(1))));
        rep(w, m, n + 1) rep(l, n - w + 1) {
            int r = l + w;
            dp[l][r] = {INF, MI(0)};
            bool any_occ = false;
            rep(i, l, r - m + 1) {
                ...
                if (dp[l][i].first)
                    continue;
                int moves = 1 + dp[l][i].first + dp[i + m][r].first;
                MI ways = dp[l][i].second * dp[i + m][r].second;
                if (moves < dp[l][r].first)
                    dp[l][r] = {moves, ways};
                else if (moves == dp[l][r].first)
                    dp[l][r].second += ways;
            }
