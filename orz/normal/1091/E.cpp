...
int ffi_with_val(const vi& v, const int& r, const int& val, const int& p)
{
	int ans = min(r, sz(v) - 1);
	while ((ans > p) && (v[ans] <= val))
		--ans;
	return ans;
}

int find_min_suff(const vi& v, const int& n)
{
	int x = -1, y = n, z = (y - x) >> 1;
	while (z)
	{
		z += x;
		bool good = true;
		int max_k = 1 + ffi_with_val(v, n - 1, z, -1);
		int b = ffi_with_val(v, n - 1, 1, -1);
		ll rs = 0;
		for (int i = b + 1; i < n; ++i)
			rs += v[i];
		ll ls = v[0];
		for (int k = 1; good && (k <= max_k); ++k)
		{
			ll right_hand_part = ((ll)k) * b + rs + min(k, z);
			if (ls > right_hand_part)
				good = false;
			else if (k < n)
			{
				ls += v[k];
				int c = b + 1;
				c = ffi_with_val(v, c, k + 1, k);
				for (int i = b + 1; i <= c; ++i)
					rs -= v[i];
				for (int i = c + 1; i <= b; ++i)
					rs += v[i];
...
int find_max_under(const vi& v, const int& n)
{
	int x = -1, y = n + 1, z = (y - x) >> 1;
	while (z)
	{
		z += x;
		int k = 2 + ffi_with_val(v, n - 1, z, -1);
		int b = ffi_with_val(v, n - 1, k, k - 2);
		ll rs = 0;
		for (int i = b + 1; i < n; ++i)
			rs += v[i];
		ll ls = z;
		...
		for (k; good && (k <= n); ++k)
		{
			//ll right_hand_part = ((ll)k) * (k - 1) + (b - (k - 2)) * ((ll)k) + rs;
			ll right_hand_part = ((ll)k) * (b + 1) + rs;
			if (ls > right_hand_part)
				good = false;
			else if (k < n)
			{
...
int main()
{
	...
	for (int i = 0; i < n; ++i)
		sum += a[i];
	int x = find_min_suff(a, n);
	int y = find_max_under(a, n);
