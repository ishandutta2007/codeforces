...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n[(t)%3])
        {
            ll j=upper_bound(a[(t+1)%3].begin(),a[(t+1)%3].end(),a[(t)%3][i])-a[(t+1)%3].begin();
            if(j!=n[(t+1)%3])
            {
                ll k=upper_bound(a[(t+2)%3].begin(),a[(t+2)%3].end(),((a[(t)%3][i]+a[(t+1)%3][j])>>1))-a[(t+2)%3].begin();
                if(k!=n[(t+2)%3])
                {
                    ans=min(ans,(a[(t)%3][i]-a[(t+1)%3][j])*(a[(t)%3][i]-a[(t+1)%3][j])+(a[(t)%3][i]-a[(t+2)%3][k])*(a[(t)%3][i]-a[(t+2)%3][k])+(a[(t+1)%3][j]-a[(t+2)%3][k])*(a[(t+1)%3][j]-a[(t+2)%3][k]));
                }
                k--;
                if(k>=0)
                {
