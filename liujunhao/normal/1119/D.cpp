...
//template
#define MAXN 100000
LL n,a[MAXN+10],Q,b[MAXN+10];
map<LL,LL,greater<LL> >c,sum;
int main()
{
	Read(n);
	int i;
	...
	sum[0]=0;
	for(i=1;i<n;i++)
		sum[b[i]]=sum[b[i-1]]+(b[i]-b[i-1])*c[b[i-1]];
	Read(Q);
	LL l,r,len;
	while(Q--){
		Read(l),Read(r);
		len=r-l;
		auto t1=c.lower_bound(len);
		auto t2=sum.lower_bound(len);
		LL ans=t2->second+(len-t1->first+1)*t1->second;
		printf("%lld\n",ans);
	}
}