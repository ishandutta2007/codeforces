...
ll mem[MAXN][8];
#define NOW mem[pos][(b2<<1)|b1]
#define KNOW ~NOW
ll Dfs( int pos , int b1 , int b2 ){
	if( pos == n+1 ) return 0;
	if(KNOW) return NOW;
	ll ans = 0;
	if( b1 == 0 ){
		Mymax(ans,Dfs(pos+1,0,0));
		Mymax(ans,Dfs(pos+1,1,0)+a[pos]);
		Mymax(ans,Dfs(pos+1,1,1)+a[pos]*x);
	}elif( b1 == 1 ){
		if( b2 == 0 ){
			Mymax(ans,Dfs(pos+1,1,0)+a[pos]);
			Mymax(ans,Dfs(pos+1,1,1)+a[pos]*x);
		}elif( b2 == 1 ){
			Mymax(ans,Dfs(pos+1,1,2)+a[pos]);
			Mymax(ans,Dfs(pos+1,1,1)+a[pos]*x);
		}else{
			Mymax(ans,Dfs(pos+1,1,2)+a[pos]);
		}
