...
int main () {
	...
	int n;
	cin >> n;
	multimap<int, node *, greater<int>> x;

	for (int i = 0; i < n; i++) {
		int l;
		cin >> l;
		x.emplace(l, new node(i));
	}
	...
	while (x.size() > 1) {
		...
		if (a.first == b.first) {
			x.erase(x.begin());
			x.emplace(a.first - 1, new node(a.second, b.second));
		}
		else {
			x.emplace(a.first - 1, new node(a.second));
		}
	}

	int t = x.begin()->first;
	node* te = x.begin()->second;
