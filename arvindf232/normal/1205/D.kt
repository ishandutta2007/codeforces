...
fun nullablePacker(a:IntArray,want:Int):BooleanArray?{
    ...
    val want = (want downTo 0).first{DP[it] != -1}
...
fun main(){
    solve.cases{
        ...
        val effectiveSize = IntArray(n)
        G.subs(c){
            v ->
            effectiveSize[v] = sizes[v]
        }
        val p = nullablePacker(effectiveSize,atmost)!!
        var comp1 = 0
        var comp2 = 0
        for(i in 0 until n){
            if(p[i]){
                comp1 += effectiveSize[i]
            }else{
                comp2 += effectiveSize[i]
            }
        }
        val marked = IntArray(n){0}
        assert((comp1 +1) * (comp2 +1) - 1 >= (2 * n * n /9)   )
        for(i in 0 until n){
        ...
        var puts = Array(2){mint}
        puts[0].addAll((1..comp1))
        puts[1].addAll((1..comp2).map { it * (comp1 +1 ) })
        val retsum = IntArray(n)

        fun Graph.setDFS( t:Int, v:Int, p:Int){
            NS(v){
                w ->
                if(w == p) return@NS
                setDFS(t,w,v)
            }
            val pot = puts[t].removeLast()
            retsum[v] = pot
        }
        for(i in 0 until n){
            if(marked[i] != 0){
                G.setDFS(marked[i] - 1 , i, c)
            }
        }
        G.everyEdge { a, b ->
            if(a < b){
                put(a + 1 )
                put(b + 1 )
                put((retsum[a] - retsum[b]).absoluteValue)
            }
        }
