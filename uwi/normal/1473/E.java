...
public class E {
	...
	void solve() {
		...
		// min->max
		{
			long[] td = new long[3*n];

			Arrays.fill(td, Long.MAX_VALUE / 2);
			MinHeapL q = new MinHeapL(3*n);
			q.add(0, 0);
			td[0] = 0;
			...
			while(q.size() > 0){
				int cur = q.argmin();
				q.remove(cur);
				int node = cur % n, phase = cur / n;

				for(int[] e : g[node]){
					{
						int next = e[0] + phase * n;
						long nd = td[cur] + e[1];
						if (nd < td[next]) {
							td[next] = nd;
							q.update(next, nd);
					...
					if(phase == 0){
						int next = e[0] + (phase + 1) * n;
						long nd = td[cur] + 2*e[1];
						if (nd < td[next]) {
							td[next] = nd;
							q.update(next, nd);
						}
