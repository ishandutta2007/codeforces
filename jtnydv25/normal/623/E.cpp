...
vll compute(int n){
    ...
    if(n & 1){
        ...
        int f = pw2[n - 1];
        for(int i = 1; i < M; i++){
            vec2[i] = mul(f, invfact[i]);
            f = mul(f, pw2[n - 1]);
        }
        mul_big_mod(vec, vec2, mod);
        vec.resize(M);
        return vec;
    }
    int halfn = n >> 1;
...
int main(){
    ...
    vll vec = compute(n);
    vll vec2(M);
    for(int i = 0; i < M; i++) vec2[i] = invfact[i];
    mul_big_mod(vec, vec2, mod);
    int ans = 0;
    for(int i = k; i <= k; i++){
        ans = add(ans, mul(fact[i], mul(C(k, i), vec[i])));
    }
    printf("%d\n", ans);
}