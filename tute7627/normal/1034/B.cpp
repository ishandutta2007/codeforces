...
ll solve(ll n,ll m){
  Dinic<ll>dinic(2*n*m+2);
  rep(i,0,n)rep(j,0,m)rep(p,0,n)rep(q,0,m){
    if(abs(i-p)+abs(j-q)==3)dinic.add_edge(i*m+j,n*m+p*m+q,1);
  }
  rep(i,0,n)rep(j,0,m){
    dinic.add_edge(2*n*m,i*m+j,1);
    dinic.add_edge(n*m+i*m+j,2*n*m+1,1);
  }
  return dinic.max_flow(2*n*m,2*n*m+1);
}
int main(){
  cin.tie(nullptr);
  ...
  if(n>m)swap(n,m);
  if(n==1){
    if(m%6<=2)cout<<m/6*6<<endl;
    else cout<<m/6*6+(m%6-3)*2<<endl;
  }
  else if(n<=10&&m<=10)cout<<solve(n,m)<<endl;
  else{
    cout<<n*m/2*2<<endl;
  }
  return 0;
