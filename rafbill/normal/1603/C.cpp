...
void solve(){
  ...
  for(li i = 0; i < (li)(n); ++i) {
    ...
    vi B;
    auto add = [&](li x) { if(B.empty() || B.back() != x) B.emplace_back(x); };
    for(li fr = 1, to; fr <= x; fr = to + 1) {
      to = x / (x / fr);
      if(x%fr != 0) add(x/fr+1);
      add(x/fr);
    }


    vector<tuple<int,ZN,ZN>> D;

    auto addD = [&](li z, ZN cnt, ZN sum) {
      if(!D.empty() && get<0>(D.back()) == z) {
        get<1>(D.back()) += cnt;
        get<2>(D.back()) += sum;
      }else{
        D.emplace_back(z,cnt,sum);
      }
    };
    ...
    for(li y : B) {
      ...
      while(!C.empty() && get<0>(C.back()) >= zr) {
        ZN cntc, sumc;
        tie(ignore,cntc,sumc) = C.back();
        C.pop_back();
        cnt += cntc;
        sum += sumc + cntc * (y-1);
      }
