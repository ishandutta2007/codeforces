...
namespace run{
	...
	struct BIT{
		Z s[200005];
		void add(int x,Z y){for(;x<=n;x+=x&-x)s[x]+=y;} 
		Z sum(int x){Z y=0;for(;x;x^=x&-x)y+=s[x];return y;}
		void clear(){
	...
	bool main(){
		...
		for(int i=1;i<=n;++i){
			int x=id[i];
			Z y1=f[x],y2=s2.sum(x);
			if(is[x]){
				ans-=y2;		
				for(int j=las+1;j<i;++j)s2.add(id[j],-v[j]);
				las=i;
			}
			else{
				v[i]=y1+y2;
