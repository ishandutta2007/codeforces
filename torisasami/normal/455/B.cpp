...
int main() {
    ...
    rep(i, n) {
        ...
        rep(j, sz(s[i])) {
            ...
            if (g[now][dst] == -1) {
                g[now][dst] = sz(g);
                g.eb(vector<int>(26, -1));
            }
            now = g[now][dst];
        }
    }
    auto dfs = [&](int now, bool w, auto &&f) -> bool {
        bool fin = true;
        rep(i, 26) fin &= (g[now][i] == -1);
        if (fin)
            return w;
        bool ret = false;
        rep(i, 26) if (g[now][i] != -1) ret |= (f(g[now][i], w, f) == false);
        return ret;
    };
    if (!dfs(0, false, dfs))
        cout << "Second" << endl;
    else
        cout << (dfs(0, true, dfs) ? "First" : k & 1 ? "First" : "Second") << endl;
}