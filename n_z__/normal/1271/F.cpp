...
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
    int a[4],b[4],f[8];
    cin>>a[1]>>a[2]>>a[3]>>b[1]>>b[2]>>b[3]>>f[1]>>f[2]>>f[3]>>f[4]>>f[5]>>f[6]>>f[7];
	if(a[1]+b[1]<f[1]+f[2]+f[3]+f[4]||a[2]+b[2]<f[1]+f[2]+f[5]+f[6]||a[3]+b[3]<f[1]+f[3]+f[5]+f[7]){cout<<-1<<endl;return;}
    for(int x=0;x<=f[2];x++)
    for(int y=max(0ll,f[2]+f[3]-b[1]-x);y<=min(f[3],a[1]-x);y++)
    for(int z=max({0ll,f[2]+f[5]-b[2]-x,f[3]+f[5]-b[3]-y});z<=min({f[5],a[2]-x,a[3]-y});z++)
    if(min({a[1]-x-y,a[2]-x-z,a[3]-y-z})+min({b[1]-f[2]-f[3]+x+y,b[2]-f[2]-f[5]+x+z,b[3]-f[3]-f[5]+y+z})>=f[1]){
        int t=min({a[1]-x-y,a[2]-x-z,a[3]-y-z,f[1]});
        a[1]-=t+x+y,a[2]-=t+x+z,a[3]-=t+y+z;
        cout<<t<<' '<<x<<' '<<y<<' '<<min(a[1],f[4])<<' '<<z<<' '<<min(a[2],f[6])<<' '<<min(a[3],f[7])<<endl;
        return;
    }
	cout<<-1<<endl;
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
