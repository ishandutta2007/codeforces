...
signed main()
{
	...
	int T=read();rg(T)
	int n=read(),s=read();
	rg(n)a[i]=b[i]=p[i]=q[i]=-1;gr
	rg(n)a[i]=read();p[a[i]]=i;gr
	rg(n)b[i]=read();if(b[i]>0)q[b[i]]=i;gr
	rg(n)std::swap(a[i],p[i]);gr
	rg(n)std::swap(b[i],q[i]);gr
	rg(n)f[i]=f[i-1]+(q[a[i]]==-1);gr
	// rg(n)odb(f[i]);gr puts("");
	// rg(n)odb(b[i]);gr puts("");
	// rg(n)odp(a[i],b[i]);gr
	int ans=1,h=0;
	rg(n)
	if(b[i]>0){if(p[b[i]]>i+s){ans=0;break;}}
	else mle(ans,f[min(i+s,n)]-(h)),h++;
	gr
	odl(ans);
	gr
	return 0;
