// Problem: E. Groceries in Meteor Town
// Contest: Codeforces - Codeforces Round #767 (Div. 1)
// URL: https://codeforces.com/contest/1628/problem/E
// Memory Limit: 256 MB
// Time Limit: 2000 ms
// 
...
int lc[1000005],rc[1000005],wi[1000005];
mgs
int F[1000005][21],dep[1000005];
...
int tag[2000005],lca[2000005],st[2000005];
...
void push(int o,int l,int r,int L,int R,int v)
{
	// printf("[%d %d] : [%d %d] , %d - %d\n",l,r,L,R,v,o);fflush(stdout);
	if(L<=l&&r<=R)
	{
		if(v)lca[o]=tag[o];
		else lca[o]=0;
	...
	pushdown(o);
	int m=l+r>>1;
	if(L<=m)push(o<<1,l,m,L,R,v);
	if(m<R)push(o<<1|1,m+1,r,L,R,v);
...
signed main()
{
	...
	u=f(u),v=f(v);
	++cc;lc[cc]=u,rc[cc]=v,wi[cc]=w,fa[u]=cc,fa[v]=cc;
	gr
	dfs(cc,0);
	build(1,1,n);
	rg(m)
