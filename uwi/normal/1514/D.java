...
public class D {
	...
	void solve()
	{
		...
		int[][] b = packD(n+1, from, to);
		for(int[] row : b){
			for(int j = 0, k = row.length-1;j < k;j++,k--){
				int d = row[j]; row[j] = row[k]; row[k] = d;
			}
		}
		dfs(0, n, qs, b, a);
		for(int v : ans){
	...
	void dfs(int l, int r, int[][] qs, int[][] b, int[] a)
	{
		...
		int[][] ls = new int[qs.length][];
		int[][] rs = new int[qs.length][];
		int lp = 0, rp = 0;
		...
		for(int[] q : qs){
			if(q[0] <= h && h <= q[1]){
				...
				if(q[0] <= h-1){
					int tar = pre[h-1-q[0]];
					F = Math.max(F, lowerBound(b[tar], q[1]+1) - lowerBound(b[tar], q[0]));
				}
				{
					int tar = suf[q[1]-h];
					F = Math.max(F, lowerBound(b[tar], q[1]+1) - lowerBound(b[tar], q[0]));
		...
		dfs(l, h, Arrays.copyOf(ls, lp), b, a);
		dfs(h, r, Arrays.copyOf(rs, rp), b, a);
	}
