...
void solve()
{
    ...
    vi nxtrig(n+1, -1), nxtleft(n+1, -1);

    vector<pii> st;
    // st.push_back({1, a[1]});
    FE(i,1,n)
    {
        while(st.sz and st.back().se < a[i])
        {
            nxtrig[st.back().fi] = i;   
            st.pop_back();
        }
    ...
    {
        if(nxtleft[i] != -1)
            ans = max(ans, a[i]^a[nxtleft[i]]);
        
        if(nxtrig[i] != -1)
            ans = max(ans, a[i]^a[nxtrig[i]]);
    }
