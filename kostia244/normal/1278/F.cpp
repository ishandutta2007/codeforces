...
ll n, m, k;
modular pw[maxn], ipw[maxn], fact[maxn], inv[maxn], Q[maxn], P[maxn];
void setup() {
	pw[0] = ipw[0] = fact[0] = inv[0] = Q[0] = 1;
	for(int i = 1; i < maxn; i++) {
		pw[i] = pw[i-1]+pw[i-1], ipw[i] = pw[i].inv();
		fact[i] = fact[i-1]*modular(i);
		inv[i] = fact[i].inv();
		P[i] = algebra::bpow<modular>(i, k);
		Q[i] = algebra::bpow<modular>(m-1, i);
	}
}
...
int main() {
	...
	setup();
	vector<modular> x, y;
	modular ipw = algebra::bpow<modular>(2, mod-1-n);
	for(int i = 1; i <= k+1; i++) {
		modular t;
		for(int j = 1; j <= i; j++)
			t += nck(i, j)*Q[i-j]*P[j];
		t *= algebra::bpow<modular>(m, mod-1-i);
		t *= ipw;
		x.pb(i);
		y.pb(t);
	}
	auto p = algebra::inter(x, y);
	cout << p.eval(n)*algebra::bpow<modular>(2, n);
}