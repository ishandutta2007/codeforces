...
int main()
{
	...
	cin >> a;
	int k = (n + 3) / 2;
	vvl b(k, vl(n + 1)); // zanyat
	vvl c(k, vl(n + 1)); // prosto
	vl f(n);
	for (int i = 0; i < n; ++i)
	{
		f[i] = 0;
	...
	for (int i = 1; i < k; ++i)
	{
		...
		for (int j = 1; j <= n; ++j)
		{
			b[i][j] = INF;
			if (j >= 3)
				b[i][j] = min(b[i][j], c[i - 1][j - 3] + f[j - 1]);
			else if (i == 1)
				b[i][j] = min(b[i][j], f[j - 1]);
			if (j >= 3)
			{
				ll old_holm = min(a[j - 3] - 1, a[j - 2]);
				ll alternative = old_holm - min(old_holm, a[j - 1] - 1);
				if (j <= n - 1)
					alternative += a[j] - min(a[j], a[j - 1] - 1);
				b[i][j] = min(b[i][j], b[i - 1][j - 2] + alternative);
			}
			c[i][j] = min(b[i][j], c[i][j - 1]);
		}
	}
	for (int i = 1; i < k; ++i)
		cout << c[i][n] << ' ';
