...
namespace MAIN{
    ...
    int step[N];
    inline void MAIN(){
        n=read(),m=read(),s=read(),t=read(),S=read(),T=read();
        res ns=s,nt=t;
        res p=read();
        res INV=qpow(100);
        p=mul(p,INV);
        res idx=0;
        res M=0,dx=1,dy=1;
        if(s+dx>n||s+dx<1)dx=-dx;
        if(t+dy>m||t+dy<1)dy=-dy;
        res ndx=dx,ndy=dy;
        for(;;M++){
            if(s+dx>n||s+dx<1)dx=-dx;
            if(t+dy>m||t+dy<1)dy=-dy;
            if(M!=0&&s==ns&&nt==t&&dx==ndx&&dy==ndy)break;
            if(s==S||t==T)step[++idx]=M;
            s+=dx;
            t+=dy;
        }
        res ans=0;
        res pp=qpow(1+kcz-p,idx),ipp=qpow(1+kcz-pp);
        for(res i=1;i<=idx;i++){
            res ret=mul(qpow(1+kcz-p,i-1),ipp);
            ret=mul(ret,step[i]);
            add(ans,ret),ret=0;
            ret=mul(qpow(1+kcz-p,i-1),mul(mul(ipp,ipp),pp));
            ret=mul(ret,M);
            add(ans,ret);
        }
        printf("%d\n",mul(ans,p));
