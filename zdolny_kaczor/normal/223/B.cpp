...
void solve() {
	...
	REP(i, al) wh[a[i] - 'a'].insert(i);
	int pos = 0;
	REP(i, bl) {
		char x = b[i];
		while (pos < al && a[pos] != x)
			pos++;
		if (pos == al) {
			done("No");
		}
	...
	debug(range(suf, 0, bl + 1));
	REP(i, bl) {
		int x = b[i] - 'a';
		int l = pref[i], r = suf[i + 1];
		wh[x].erase(wh[x].lower_bound(l), wh[x].lower_bound(r));
	}
	bool all = true;
	REP(i, A) if (!wh[i].empty()) all = false;
	puts(all ? "Yes" : "No");
}
int main() {
