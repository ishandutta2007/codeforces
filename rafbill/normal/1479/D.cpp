...
struct pst {
  ...
  pst(uint64_t x_, int a_, int b_, pst* l_, pst* r_) {
    a = a_;
    b = b_;
    l = l_;
    r = r_;
  ...
  pst* add_point(int i, uint64_t y) {
    if(a == b) {
      return new pst(x ^ y, a, b, 0, 0);
    }else{
      int c = (a+b)/2;
      if(i <= c) return new pst(x ^ y, a, b, l->add_point(i,y), r);
      else return new pst(x ^ y, a, b, l, r->add_point(i,y));
    }
  }
};
...
int query(pst* A, pst* B, int l, int r, int ip) {
  int a = A->a, b = A->b;
  uint64_t h = A->x ^ B->x ^ ((a<=ip && ip<=b) ? H[ip] : 0ull);
  if(b < l || a > r) return -1;
  if(h == 0) return -1;
  if(a == b) return a+1;
  int r1 = query(A->l, B->l, l, r, ip);
  if(r1 != -1) return r1;
  return query(A->r, B->r, l, r, ip);
}
...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) H[i] = randomInt64();
  ...
  static pst* root = new pst(0,n);
  P[0] = root->add_point(A[0], H[A[0]]);

  int date = 0;
  auto dfs = letrec([&](auto dfs, int u, int p, int d) -> void {
    parent[0][u] = p;
    depth[u] = d;

    if(p != -1) P[u] = P[p]->add_point(A[u], H[A[u]]);
  ...
  for(li i = 0; i < (li)(LN-1); ++i) for(li u = 0; u < (li)(n); ++u) {
    if(parent[i][u] != -1) parent[i+1][u] = parent[i][parent[i][u]];
    else parent[i+1][u] = -1;
  }

  auto lca = [&](int u, int v) -> int {
    if(depth[u] < depth[v]) swap(u,v);
    for(li k = (LN-1); k >= (li)(0); --k) if(depth[u] - (1<<k) >= depth[v]) u = parent[k][u];
    if(u == v) return u;
    for(li k = (LN-1); k >= (li)(0); --k) if(parent[k][u] != parent[k][v]) { u = parent[k][u]; v = parent[k][v]; }
    return parent[0][u];
  };


  for(li i = 0; i < (li)(nq); ++i) {
    int u,v,l,r;
    cin>>u>>v>>l>>r;
    --u; --v; --l; --r;
    if(r < l) swap(l,r);
    int p = lca(u,v);
    cout << query(P[u], P[v], l, r, A[p]) << '\n';
  }
  cout << flush;
