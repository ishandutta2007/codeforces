...
fun main(){
    solve.cases{
       ...
       repeat(n-1){
        ...
        val max = 2 * k
        val DP = Array(n){IntArray(2 * k)}
        G.leafFirst { v ->
            DP[v][0] = DP[v][0] xor L[v]
            G.subs(v){
                w->
                for(i in 0 until 2 * k ){
                    val next = if(i != max - 1 ) i + 1 else 0
                    DP[v][next] = DP[v][next] xor DP[w][i]
                }
            }
        }

        fun changeroot(from:Int, to:Int){
            for(i in 0 until max){
                DP[from][i] = DP[from][i] xor DP[to][i.previous(max)]
            }
            for(i in 0 until max){
                DP[to][i] = DP[to][i] xor DP[from][i.previous(max)]
            }
        }
        ...
        fun Graph.reroot_dfs(v:Int, p:Int){
            ...
            var here = 0
            for(i in k until max){
                here = here xor DP[v][i]
            }
            ret[v] = (here == 0)
        ...
        put(ret.map{if(it)0 else 1}.conca())
