...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  vector<set<int>> se(H*W);
  {
    ...
    rep(i,H)rep(j,W){
      ...
      if(i&&g[now-W]!='#')se[now].emplace(now-W);
      if(i+1<H&&g[now+W]!='#')se[now].emplace(now+W);
      if(j&&g[now-1]!='#')se[now].emplace(now-1);
      if(j+1<W&&g[now+1]!='#')se[now].emplace(now+1);
      ++now;
    }
  }
  int firstpos=g.find('L');
  assert(firstpos<H*W);
  queue<int> que;
  que.emplace(firstpos);
  while(!que.empty()){
    ...
    for(auto nxt:se[now]){
      se[nxt].erase(now);
      if(se[nxt].size()<=1)que.emplace(nxt);
    }
    se[now].clear();
  }
  g[firstpos]='L';
