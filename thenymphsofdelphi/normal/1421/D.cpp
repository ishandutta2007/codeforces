...
int cxpos, cxneg, cypos, cyneg, czpos, czneg;
...
int q; cin >> q; while (q--){
    int x, y; cin >> x >> y;
    cin >> cypos >> cxpos >> czneg >> cyneg >> cxneg >> czpos;
    if (cxpos > cypos + czneg) cxpos = cypos + czneg;
    if (cypos > czpos + cxpos) cypos = czpos + cxpos;
    if (czpos > cxneg + cypos) czpos = cxneg + cypos;
    if (cxneg > cyneg + czpos) cxneg = cyneg + czpos;
    if (cyneg > czneg + cxneg) cyneg = czneg + cxneg;
    if (czneg > cxpos + cyneg) czneg = cxpos + cyneg;
    int ans = infll * 3;
    ans = min(ans, (-x + y) * (-x + y > 0 ? cxpos : -cxneg) + x * (x > 0 ? cypos : -cyneg));
    ans = min(ans, y * (y > 0 ? cypos : -cyneg) + (x - y) * (x - y > 0 ? czpos : -czneg));
    ans = min(ans, x * (x > 0 ? czpos : -czneg) + y * (y > 0 ? cxpos : -cxneg));
    cout << ans << endl;
}
}
