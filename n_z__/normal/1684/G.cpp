...
{
	cin>>a[x];
	if(a[x]<=m/3)d.add_edge(x,t,1);
	else d.add_edge(s,x,1),ans++;
}
ff(x,1,n)
ff(y,1,n)
if(a[x]>m/3&&a[y]<=m/2&&2*a[x]+a[y]<=m&&a[x]%a[y]==0)
d.add_edge(x,y,1);
if(d.Dinic(s,t)!=ans)ex(-1);
cout<<n-ans<<endl;
bitset<N*2+10>v;
ff(x,1,n)
if(a[x]>m/3)
for(auto q:d.Edge[x])
if(q.first!=s&&d.Stream[q.second]!=1)
cout<<2*a[x]+a[q.first]<<' '<<a[x]+a[q.first]<<endl,v[q.first]=1;
ff(x,1,n)
if(!v[x]&&a[x]<=m/3)cout<<3*a[x]<<' '<<2*a[x]<<endl;
#endif
}
}
#ifdef Big_Head//main
