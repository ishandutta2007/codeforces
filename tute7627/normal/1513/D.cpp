...
int main(){
  ...
  while(t--){
    ll n,p;cin>>n>>p;
    set<ll>rem;
    rep(i,0,n-1)rem.insert(i);
    vector<int>a(n);
    rep(i,0,n)cin>>a[i];
    DisjointSparseTable<int,decltype(fg)>dst(a,fg);
    auto ord=ascend(a);
    ll ret=0;
    for(auto i:ord){
      if(a[i]>=p)break;
      auto itl=rem.lower_bound(i);
      while(itl!=rem.begin()){
        itl--;
        if(dst.query(*itl,i+1)==a[i]){
          itl=rem.erase(itl);
          ret+=a[i];
        }
        else break;
      }
      auto itr=rem.lower_bound(i);
      while(itr!=rem.end()){
        if(dst.query(i,*itr+2)==a[i]){
          itr=rem.erase(itr);
          ret+=a[i];
        }
        else break;
