...
#endif
// #include 
std::map<int,std::string>cst,mu,ad,mp;
#define f std::to_string
void update(int j,std::string x)
{
	if(mp.count(j))
	{
		if(x.length()<mp[j].length())mp[j]=x;
	}
	else mp[j]=x;
}
void ck(std::string&a,std::string x)
{
...
signed main()
{
	...
	for(auto p:mp)
	{
		(update(p.first,f(p.first)));
	}
	std::string ans="114514";
	mp[1]="1";
	for(auto mul:mp)
	{
		// odb(mul.first);printf("%s\n",mul.second.c_str());
		int bas=n/mul.first,add=n%mul.first;
		// auto mul=mult.second;
		update(bas,f(bas));
		update(add,f(add));
		if(bas*mul.first==0)ck(ans,mp[add]);
		if(add==0)ck(ans,mp[bas]+"*"+mul.second);
		if(bas==1)ck(ans,mul.second+"+"+mp[add]);
		if(mul.first==1)ck(ans,mp[bas]+"+"+mp[add]);
		if(add==0&&bas==1)ck(ans,mul.second);
		ck(ans,mp[bas]+"*"+mul.second+"+"+mp[add]);
	}
	printf("%s\n",ans.c_str());
	//2
	//?X?+?
