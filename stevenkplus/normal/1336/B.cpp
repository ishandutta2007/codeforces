  ...
  for (int t = 1; t <= T; ++t) {
    ...
    for (int rep1 = 0; rep1 < 2; ++rep1) {
      for (int rep = 0; rep < 3; ++rep) {
        ...
        for (ll y: b) {
          pp(y, a, c);
          auto xp = upper_bound(a.begin(), a.end(), y);
          if (xp == a.begin()) continue;
          auto zp = lower_bound(c.begin(), c.end(), y);
          if (zp == c.end()) continue;
          --xp;
          pp(*xp, y, *zp);
          ll val = (y - *xp) * (y - *xp) + (y - *zp) * (y - *zp) + (*xp - *zp) * (*xp - *zp);
          ans = min(ans, val);
        }
