...
fn run() {
    input! {
        n: usize,
        p: [(usize, usize, bytes); n],
    }
    let p = p.into_iter().map(|p| (p.0, p.1, p.2[0])).collect::<Vec<_>>();
    // dp[i]: i , i active i
    let mut dp = vec![M::zero(); n];
    let mut sum = vec![M::zero(); n + 1];
    for i in 0..n {
        let (x, y, _) = p[i];
        let mut ans = M::from(x - y);
        let s = p.lower_bound_by_key(&y, |p| p.0);
        if s < i {
            ans += sum[i] - sum[s];
        }
        /*
    ...
    let mut ans = M::from(p[n - 1].0) + M::one();
    for i in 0..n {
        if p[i].2 == b'1' {
            ans += dp[i];
        }
