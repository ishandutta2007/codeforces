...
vector<pair<ll, pair<ll, ll>>> elist;
vector<pair<ll, ll>> edges[100005];
dsu d;
ll dp[100005];
ll spec[100005];

void dfs(ll v, ll p, ll wt) {
	dp[v] = spec[v];
...
void solve(int tc = 0) {
	...
	for (pair<ll, pair<ll, ll>> x: elist) {
		if (d.merge(x.s.f, x.s.s)) {
			edges[x.s.f].push_back({x.s.s, x.f});
			edges[x.s.s].push_back({x.s.f, x.f});
		}
	}
