...
void solve(){
    ...
    REP(te, 2) {
        ...
        REP(i, n - 1) {
            ...
            for (int q = 0; q < n; ++q) {
                if (cnts[junjo[q]] == 1) {
                    itring.push_back(junjo[q]);
                }
            }
            int itr = 0;
            if (itring.size() == 0) break;
            if (itring.size() == 1) {
                itr = itring[0];
            }
            else {
                for (auto& x : itring) {
                    ...
                    REP(q, memo.size()) {
                        int rights = n - q - 1;
                        int left = rights - (int)memo[q].size() + 1;
                        if (left <= n - i - 1) {
                            int check = 0;
                            REP(j, memo[q].size()) {
                                if (memo[q][j] == x) {
            ...
            REP(q, gekos.size()) {
                ...
                memo.push_back(gekos[q]);
                gekos.erase(gekos.begin() + q);
                break;
            }
        }
        for (int i = 1; i <= n; ++i) {
        ...
        reverse(ALL(memo));
        int ok = 1;
        for (int q = memo.size() - 1; q >= 0; --q) {
            vector<int> gekos;
            REP(j, memo[q].size()) {
                gekos.push_back(ans[q + 1 - j]);
            }
            sort(ALL(gekos));
            if (gekos != memo[q]) {
                ok = 0;
