...
bool ispal(string s)
{
    ...
    int si=s.size();
    yes=true;
    for(int i=0,j=si-1;i<(si+1)/2;i++,j--)
        if(s[i]!=s[j])yes=false;
    ...
    return yes;
            ...
            int main()
            {
                ...
                while(t--)
                {
                   ...
                   {
                       tmp+=s[i];
                       if(ispal(tmp+s.substr(n-cnt,n)))
                       fin=tmp+s.substr(n-cnt,n);
                   }
                   for(int i=n-1;i>=cnt;i--)
                   {
                       string tmp3=tmp2+s.substr(i,n);
                       if(ispal(tmp3)&&tmp3.size()>fin.size())
                       fin=tmp3;
                       //cout<<tmp3<<endl;
                   }
                   cout<<fin<<endl;
