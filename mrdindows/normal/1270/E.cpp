...
vector<int> gr[1010];
int gid[1010];
unordered_map<ll, pair<bool, vector<pii>>> ve;
...
//    }
    vector<int> v[2][2];
    FI(n) {
        v[p[i].first % 2][p[i].second % 2].push_back(i);
    }
    int c = 0;
    FI(2) FJ(2) c += v[i][j].empty();
    if (c >= 3) {
        exit(-1);
    }
//    FI(2) FJ(2) {
...
//    }
    if (v[0][0].size() + v[1][1].size() == n) {
        write(v[0][0]);
    } else if (v[0][1].size() + v[1][0].size() == n) {
        write(v[0][1]);
    } else {
        for (int x : v[1][1]) {
            v[0][0].push_back(x);
    ...
    FI(n) {
        FJ(i) {
            ve[dist(p[i], p[j])].second.push_back({i, j});
        }
    }
    FI(n) {
        pll e[1010];
        ...
        for (int j = 1; j < n; ++j) {
            if (e[j].first == e[j - 1].first) {
                unite((int)e[j].second, (int)e[j - 1].second);
            }
        }
    }
    bool ok = true;
