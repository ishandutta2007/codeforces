...
int main(){
  ...
  auto f1=[&](S x,S y){
    return S{max({x.mx,y.mx,x.lmx+y.lmx}),max({x.nmx,y.nmx,x.lmx+y.lmx}),max(x.lmx,y.lmx)};
  };
  auto f2=[&](S x,P w){
    return S{max(x.mx,x.lmx+w.fi),x.lmx+w.fi,x.lmx+w.fi};
  };
  ReRooting<S,P>rer(n,f1,f2,S{0,0,0});
  rep(i,0,n-1){
    ll u,v,w;cin>>u>>v>>w;u--;v--;
    rer.add_edge(u,v,MP(w,i));
    res+=2*w;
  }
  rer.build();
  vector<P>s(n-1);
  ...
  rep(i,0,n){
    vector<ll>v;
    for(auto e:rer.g[i]){
      s[e.data.se].se+=e.data.fi;
      s[e.data.se].fi+=e.ndp.mx;
      s[e.data.se].se+=e.ndp.mx;
      v.PB(e.dp.lmx);
    }
    sort(ALLR(v));
    ll sum=0;
