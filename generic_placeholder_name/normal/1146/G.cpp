...
int32_t main() {
    ...
    //input
    int n, h, m; cin>>n>>h>>m;
    int l[m], r[m], x[m], c[m]; rep(i, m) cin>>l[i]>>r[i]>>x[i]>>c[i];
    //flow mappings
    int p[n][h+1]; int sz=1;
    rep(i, n) rep(j, h+1) p[i][j]=sz++;
    int q[m]; rep(i, m) q[i]=sz++;
    //flow
    PushRelabel::PushRelabel<int> f(sz+1);
    rep(i, n) {
        f.add_edge(0, p[i][0], h*h);
        rep(j, h) f.add_edge(p[i][j], p[i][j+1], h*h-(j+1)*(j+1));
    }
    rep(i, m) {
        for(int j=l[i]-1; j<r[i]; j++) f.add_edge(p[j][x[i]], q[i], c[i]);
        f.add_edge(q[i], sz, c[i]);
    }
    cout<<n*h*h-f.flow(0, sz)<<endl;
}