    ...
    for (int i = 0; i < n; i++) {
        ...
        // try <
        {
            ...
            for (int need = 0; need < target; need++) {
                if (!indices[need].empty())
                    ckmin(best, indices[need].front());
            }

            if (best != NONE) {
                ckmin(ans, cost + int(remaining.order_of_key(best)));
            }
        }
        ...
        // try <=
        {
            ...
            cost += remaining.order_of_key(best);
            indices[s[best]].pop();
