...
int another_main()
{
	...
	cin >> s;
	int n = sz(s);
	int f = 1, g = 1, h = 1, l = 1, p = 1, q = 1, x = 1;
	int st = 1;
	for (int i = n - 1; i >= 0; --i) {
		int ff, gg, hh, lll, pp, qq;
		qq = prod(q, 6, M);
		if (s[i] == '0') {
			pp = prod(p, 3, M);
			lll = l;
			...
			x = sum(x, st, M);
			pp = prod(sum(p, q, M), 3, M);
			lll = sum(sum(l, q, M), prod(p, 4, M), M);
			hh = prod(sum(h, q, M), 3, M);
			gg = sum(sum(sum(h, p, M), prod(q, 2, M), M), prod(g, 2, M), M);
			ff = sum(sum(f, q, M), prod(sum(p, g, M), 2, M), M);
		}
		f = ff;
		g = gg;
		h = hh;
	...
	cout << raz(pow_mod(x, 3, M), raz(prod(f, 3, M), prod(x, raz(prod(x, 3, M), 1, M), M), M), M) << '\n';
	return 0;
}
