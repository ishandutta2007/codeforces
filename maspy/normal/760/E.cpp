...
#line 1 "/home/maspy/compro/library/alg/monoid_max_xplusa_b.hpp"
...
void solve() {
  LL(N);
  const ll INF = 1LL << 60;
  SegTree<Monoid_Max_xplusa_b<ll, INF>> seg(N);
  vi A(N);
  FOR_(N) {
    LL(p, t);
    --p;
    ...
    auto check = [&](pi e) -> bool { return max(e.fi, e.se) == 0; };
    auto L = seg.min_left(check, N);
    if (L == 0)
      print(-1);
    else
      print(A[L - 1]);
