...
int main() {
	...
	for (int l = 1; l <= 2 * n; ++l)
		for (int j = 1; j <= n; ++j)
			if (!find && get<1>(pre[1][j][l])) {
				int nowp = 1, nowi = j, nowj = l;
				vector<int> now;
				while (nowj) {
					int nxtp, nxti, nxtj;
					tie(nxtp, nxti, nxtj) = pre[nowp][nowi][nowj];
					auto t = transv[nxtp ^ 1][nowp][nxti][nowi][nowj - nxtj];
					reverse(t.begin(), t.end());
					for (int x : t)
						now.push_back(x);
					tie(nowp, nowi, nowj) = make_tuple(nxtp, nxti, nxtj);
				}
