...
int main() {
  ...
  for (int alive = 2; alive <= n; ++alive) {
    int damage = alive - 1;
    for (int maxhp = 1; maxhp <= m; ++maxhp) {
      if (maxhp <= damage) {
        ways[alive][maxhp] = ModT(maxhp).power(alive);
      } else {
        auto power = ModT(1);
        for (int dead = 0; dead < alive; ++dead) {
          ways[alive][maxhp] +=
              power * binom[alive][dead] * ways[alive - dead][maxhp - damage];
          power *= ModT(damage);
        }
        // all dead
        ways[alive][maxhp] += power;
      }
    }
  }
  printf("%d\n", ways[n][m].get());
