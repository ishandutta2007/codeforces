...
bool Solve(){
	if( (n&1) and a[n/2+1] != b[n/2+1] ) return false;

	static map<int,int> am, bm; am.clear(); bm.clear();
	For(i,n){ ++am[a[i]]; ++bm[b[i]]; }
	if( am != bm ) return false;

	static vc<pii> av,bv; av.clear(); bv.clear();
	For(i,n){
		av.Eb(min(a[i],a[n-i+1]),max(a[i],a[n-i+1]));
		bv.Eb(min(b[i],b[n-i+1]),max(b[i],b[n-i+1]));
	}
