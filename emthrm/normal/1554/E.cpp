...
int main() {
  ...
  while (t--) {
    ...
    f(f, -1, 0);
    REP(i, n - 1) {
      if (sub[u[i]] < sub[v[i]]) swap(u[i], v[i]);
    }
    vector<ModInt> dp(n + 1, 0);
    dp[1] = ModInt(2).pow(n - 1);
    for (int k : divisor.query(n - 1)) {
      if (k == 1) continue;
      dp[k] = 1;
      REP(i, n - 1) {
        if (!((sub[v[i]] + 1) % k == 0 && (n - 1 - (sub[v[i]] + 1)) % k == 0)
            && !(sub[v[i]] % k == 0 && (n - 1 - sub[v[i]]) % k == 0)) {
          dp[k] = 0;
        }
      }
    }
