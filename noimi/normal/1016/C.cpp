...
signed main(){
    ...
    vecll r1(n+1),r2(n+1),sum(n+1);
    rep3(i,n-1,0){
        sum[i]=sum[i+1]+a[0][i]+a[1][i];
    }
    rep3(i,n-1,0){
        r1[i] = r1[i+1]+sum[i+1]+a[0][i] + a[1][i]*(n-i)*2;
        r2[i] = r2[i+1]+sum[i+1] + a[1][i] + a[0][i]*(n-i)*2;
    }
    ll ans = 0;
    ll now = 0;
    rep(i,n){
        if(i&1){
            chmax(ans,now +r2[i]+sum[i]*(i*2-1));
            now+=a[1][i]*(i*2);
            now+=a[0][i]*(i*2+1);
        }
        else{
            chmax(ans,now+r1[i]+sum[i]*(i*2-1));
            now+=a[0][i]*(i*2);
            now+=a[1][i]*(i*2+1);
        }
    }
