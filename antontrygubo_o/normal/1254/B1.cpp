...
ll cnt(vector<pair<ll, int>> &a)
{
    ...
    ll current = 0;
    for (int i = 0; i<n; i++) current+=(a[i].second - a[0].second)*a[i].first;
    ll best = current;
    for (int i = 1; i<n; i++)
    {
...
ll solve(vector<int> a, int p)
{
    ...
    for (int i = 0; i<n; i++)
    {
        ...
        {
            total+=a[i];
            if (a[i]>0) cur.push_back(mp(a[i], i));
            a[i] = 0;
        }
        else
        {
            cur.push_back(mp(p - total, i));
            a[i] -=(p-total);
            pos.push_back(cur);
            cur.clear();
            total = 0;
...
int main() {
    ...
    //cout<<total<<endl;
    for (int i = 2; i<=1e6; i++) if (total%i==0)
    {
        primes.push_back(i);
        while(total%i==0) total/=i;
    }
    if (total>1) primes.push_back(total);
    ll ans = 1e18;
    /*cout<<"HERE"<<endl;
    ...
    for (auto p: primes) ans = min(ans, solve(a, p));
    cout<<ans;
