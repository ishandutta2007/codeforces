...
signed main()
{
    ...
    {
        if (d(x - dig) > d(Z))
            dp[d(x - dig) - d(Z)][dig] = 1;
        else
            dpones[done(x / Z)][dig] = 1;
    }
    for (int i = 0; i < steps; i++)
    {
        for (int f = 0; f < 300; f++)
        {
            dpones2[1][0] += dp[f][Z - 1] * (1 - p);
            dpones2[1][Z - 2] += dp[f][Z - 1] * p;
            for (int s = Z / 2; s < Z - 1; s++)
            {
                dpones2[1][2 * s - Z] += dp[f][s] * p;
                dp2[f][s + 1] += dp[f][s] * (1 - p);
            }
            for (int s = 0; s < Z / 2; s++)
            {
                dp2[f + 1][2 * s] += dp[f][s] * p;
                dp2[f][s + 1] += dp[f][s] * (1 - p);
            }
        }
        for (int f = 0; f < 300; f++)
        {
            dp2[f][0] += dpones[f][Z - 1] * (1 - p);
            dpones2[f + 1][Z - 2] += dpones[f][Z - 1] * p;
            for (int s = Z / 2; s < Z - 1; s++)
            {
                dpones2[f + 1][2 * s - Z] += dpones[f][s] * p;
                dpones2[f][s + 1] += dpones[f][s] * (1 - p);
            }
            for (int s = 0; s < Z / 2; s++)
            {
                dp2[1][2 * s] += dpones[f][s] * p;
                dpones2[f][s + 1] += dpones[f][s] * (1 - p);
            }
        }
        forn(a, 300)
