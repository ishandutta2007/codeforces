...
inline bool cmp_a(int i,int j){
	return a[i] < a[j];
}
inline bool cmp_b(int i,int j){
	return b[i] < b[j];
}
void solve(){
	int n,m;
	cin >> n >> m;
	vector<int> idx_a(m),idx_b(m);
	for(int i = 0; i < m; ++i){
		cin >> a[i] >> b[i];
		idx_a[i] = i;
		idx_b[i] = i;
	...
	sort(all(idx_a),cmp_a);
	sort(all(idx_b),cmp_b);
	...
	long long cur_sum = 0, cnt = 0;
	for(int i: idx_b){
		while(j < m && a[idx_a[j]] >= b[i] && cnt < n){
			vis[idx_a[j]] = true;
			++cnt;
			cur_sum += a[idx_a[j]];
			++j;
		...
		long long tot = cur_sum + ((vis[i]) ? 0 : a[i]);
		long long rem = n - cnt - ((vis[i]) ? 0 : 1) ;
