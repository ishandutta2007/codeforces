...
void solve() {
    cin >> n >> m;
    for (int i = 1; i <= m; i++) cin >> r[i] >> w[i];
    int r_c = *max_element(r + 1, r + m + 1);
    int w_c = *max_element(w + 1, w + m + 1);
    if (r_c + w_c > n) cout << "IMPOSSIBLE\n";
    else {
        w_c = n - r_c;
        for (int i = 1; i <= r_c; i++) cout << 'R';
        for (int i = 1; i <= w_c; i++) cout << 'W';
        cout << '\n';
    }
}
