...
using Network = NetworkDinic<network::IntValue<long long>>;
...
int main() {
  ...
  int target = n + 1;
  Network net(n + 2);
  for (int i = 0, vcost; i < n; ++i) {
    scanf("%d", &vcost);
    net.add(i, target, vcost * 2);
  }
  ...
  for (int i = 0; i < n; ++i) {
    sum_deg += deg[i];
    net.add(source, i, deg[i]);
  }
  printf("%lld\n", (sum_deg - net.max_flow(source, target)) >> 1);
}