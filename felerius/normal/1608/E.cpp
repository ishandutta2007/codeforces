// The Cells on the Paper (https://codeforces.com/contest/1608/problem/E)
...
array<vector<Point>, 3> ps;

bool sort_by_x(const Point& p1, const Point& p2) { return p1.x < p2.x; };
bool sort_by_y(const Point& p1, const Point& p2) { return p1.y < p2.y; };

bool can_partition(int k, int c1, int c2, int c3) {
    nth_element(begin(ps[c1]), begin(ps[c1]) + k - 1, end(ps[c1]), sort_by_x);
    int mid = ps[c1][k - 1].x;

    auto c2_end = partition(all(ps[c2]), [&](auto& p) { return p.x > mid; });
    if (c2_end - begin(ps[c2]) < k)
        return false;

    // 1|2|3
    nth_element(begin(ps[c2]), begin(ps[c2]) + k - 1, c2_end, sort_by_x);
    int mid2 = ps[c2][k - 1].x;
    if (count_if(all(ps[c3]), [&](auto& p) { return p.x > mid2; }) >= k)
        return true;

    // 1|2
    // ---
    // 1|3
    nth_element(begin(ps[c2]), begin(ps[c2]) + k - 1, c2_end, sort_by_y);
    mid2 = ps[c2][k - 1].y;
    if (count_if(all(ps[c3]), [&](auto& p) { return p.x > mid && p.y > mid2; }) >= k)
        return true;

    return false;
}

bool can_partition(int k) {
    array<int, 3> col = {0, 1, 2};
    do {
        if (can_partition(k, col[0], col[1], col[2]))
            return true;
...
int main() {
    ...
    rep(_, n) {
        int x, y, c; read(x, y, c); --c;
        ps[c].push_back(Point{x, y});
    }

    int l = 0, r = n / 3 + 1;
    while (r - l > 1) {
        int m = (l + r) / 2;
        if (can_partition_any(m))
            l = m;
        else
            r = m;
    }
