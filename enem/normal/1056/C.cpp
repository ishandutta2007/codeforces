...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            cin>>prs[i].F>>prs[i].S;
            prs[i].F=pi[prs[i].F-1];
            prs[i].S=pi[prs[i].S-1];
            if(prs[i].F>prs[i].S) swap(prs[i].F,prs[i].S);
            pr[prs[i].F]=i;
            pr[prs[i].S]=i;
            fr2.insert(i);
        }
        ...
        REP(i,0,2*n)
        {
            ...
            if(turn==0)
            {
                if(!fr2.empty())
                {
                    auto it=fr2.end();
                    it--;
                    cout<<p[prs[*it].S].S+1<<endl;
                    fr1.erase(fr1.find(prs[*it].S));
                    fr2.erase(it);
                }
                else
                {
            ...
            else
            {
                ...
                if(pr[x]!=-1)
                {
                    if(fr2.count(pr[x]))
                    {
                        ...
                        if(x==prs[pr[x]].F)
                        {
                            cout<<p[prs[pr[x]].S].S+1<<endl;
                            fr1.erase(fr1.find(prs[pr[x]].S));
                        }
                        else
                        {
                            cout<<p[prs[pr[x]].F].S+1<<endl;
                            fr1.erase(fr1.find(prs[pr[x]].F));
                        }
                        fr2.erase(fr2.find(pr[x]));
                    }
                }
            }
        }
