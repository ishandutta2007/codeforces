...
int main(void)
{
	...
	rep(y, 0, h-1){
		vector<P> vec;
		rep(x, 0, w-1) vec.push_back(P(a[y*w+x], y*w+x+1));
		sort(all(vec));
		rep(i, 1, w-1){
			if(vec[i-1].first <= vec[i].first) scc.add_edge(vec[i-1].second, vec[i].second);
			if(vec[i-1].first >= vec[i].first) scc.add_edge(vec[i].second, vec[i-1].second);
		}
	}
	rep(x, 0, w-1){
		vector<P> vec;
		rep(y, 0, h-1) vec.push_back(P(a[y*w+x], y*w+x+1));
		sort(all(vec));
		rep(i, 1, h-1){
			if(vec[i-1].first <= vec[i].first) scc.add_edge(vec[i-1].second, vec[i].second);
			if(vec[i-1].first >= vec[i].first) scc.add_edge(vec[i].second, vec[i-1].second);
