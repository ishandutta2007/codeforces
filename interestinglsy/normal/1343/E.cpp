...
ll Solve(){
	static ll aPS[MAXN];
	For(i,m+10) aPS[i] = aPS[i-1] + a[i];
	auto CalcCost = [&]( int len1 , int len2 ){
		if( len1+len2 > m ) return LINF;
		return aPS[len1]*2 + aPS[len1+len2] - aPS[len1];
	};
	static int dis1[MAXN], dis2[MAXN], dis3[MAXN];
	auto Bfs = [&]( int st , int res[] ){
		For(i,n) res[i] = -1;
		res[st] = 0;
		static queue<int> q;
		q.push(st);
	...
	For(i,n){
		int len1 = dis2[i];
		int len2 = dis1[i] + dis3[i];
		ll tans = CalcCost(len1,len2);
		Mymin(ans,tans);
	}
	return ans;
}
