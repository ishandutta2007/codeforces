...
void solve() {
	...
	int h, g;
	scanf("%d%d", &h, &g);
	vector <min_queue<pii> > at(1 << h);
	vi largest(1 << (h + 1));
	vector <int> left(1 << (h + 1));
	for (int i = 1; i < (1 << h); ++i) scanf("%d", a + i);
	ll ans = 0;
	for (int i = (1 << h) - 1; i ; --i) {
		if (2 * i < (1 << h)) {
			swap(at[i], at[i * 2]);
			for (auto p : *(vpii*)&at[i * 2 + 1])
				at[i].push(p);
			((vpii*)&at[i * 2 + 1])->clear();
		}
		at[i].push({a[i], i});
		if (i < (1 << g)) {
			int low = max(largest[2 * i], largest[2 * i + 1]);
			while (at[i].top().first < low) at[i].pop();
			auto [v, j] = at[i].top();
			debug("use", imie(v), imie(j), "at", imie(i));
			at[i].pop();
			left[j] = true;
			ans += v;
			largest[i] = v;
	...
	printf("%lld\n", ans);
	for (int i = (1 << h) - 1; i; --i) if (!left[i]) printf("%d ", i);
	printf("\n");
	debug(arr(at, 0));
}
int main() {
