...
int update(int node,int s, int e,int l,mint v){
	if (e < l || s > l){
		return node;
	}

	if (s == e){
		return newNode(seg[node] / v);
	}
	int mid = (s + e) >> 1;

	return newParent(update(lp[node], s, mid, l, v), update(rp[node], mid + 1, e, l, v));
}

mint query(int node,int s,int e,int l,int r){
	if (e < l || s > r){
		return 1;
	}
	if (l <= s && e <= r){
	...
	int mid = (s + e) >> 1;
	return query(lp[node],s ,mid, l, r) * query(rp[node],mid + 1, e, l, r);
}
...
struct segtree{
	...
	int query(int l,int r){
		...
		for(l+=n,r+=n+1;l<r;l>>=1,r>>=1){
			if(l&1)ans= max(ans,seg[l++]); if(r&1) ans= max(ans,seg[--r]);
		} return ans;
} };
...
void solve(){
	...
	gp_hash_table<int,null_type, custom_hash> smalls;
	for(int i = 0; i < n; ++i){
		int x = a[i];
		mint val = 1;
		root[i + 1] = root[i];
		...
		while(x > 1){
			...
			if (z * 1LL * z < N){
				smalls.insert(z);
				values[ridx[z]].push_back(cnt);
			}
			else{
				assert(cnt == 1);
				int x = prv[z];
				if (x != -1){
					root[i + 1] = update(root[i + 1], 1, n, prv[z], z);
				}
				val /= z;
				prv[z] = i + 1;
			}
		...
		if (val.x != 1){
			root[i + 1] = update(root[i + 1], 1, n, i + 1, val);
		}

		for(auto e: small_prime){
			if (values[ridx[e]].size() == i){
				values[ridx[e]].push_back(0);
			}
		}
	...
	for(auto e: smalls){
		device[ridx[e]].build(values[ridx[e]]);
	}

	for(auto e: small_prime){
		if (smalls.find(e) == smalls.end()){
			values[ridx[e]].clear();
			values[ridx[e]].shrink_to_fit();
		}
	}
	small_prime.clear();
	small_prime.shrink_to_fit();
	int q;
	cin >> q;
	mint last = 0;
	for(int i = 0; i < q; ++i){
