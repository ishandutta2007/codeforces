...
void q() {
  ...
  in(lr);
  each2(l, r, lr)-- l;
  if (Sum(a) != Sum(b)) die("NO");
  auto ra = mkrui(a);
  auto rb = mkrui(b);
  vl same;
  rep(i, N + 1) if (ra[i] == rb[i]) same.push_back(i);
  ...
  each2(l, r, lr) {
    ...
    int j = ub(same, r) - 1;
    trc(i, j);
    if (same[i] == l and same[j] == r) imos[i]++, imos[j]--;
  }
  rep(i, sz(same)-1)if(same[i]+1==same[i+1])imos[i]++,imos[i+1]--;
  rep(i, sz(imos) - 1) imos[i + 1] += imos[i];
  ...
  rep(mi, M) {
    ...
    int i = lb(same, l);
    int j = ub(same, r) - 1;
    if (i) flag[i - 1].push_back(mi);
    flag[j].push_back(mi);
  }

  vi init(sz(same), 1);
  SegmentTree rest(
      init, [](int s, int t) { return s + t; }, 0);
  ...
  auto upd = [&](int id) {
    each(mi, flag[id]) {
      auto [l, r] = lr[mi];
      int i = lb<ll>(same, l);
      int j = ub<ll>(same, r) - 1;

      int lok = same[i] == l or rest[i - 1] == 0;
      int rok = same[j] == r or rest[j] == 0;
      if (lok and rok) {
        // [l, r) 
        while (true) {
          int y = rest.max_right(i, [](int x) { return x == 0; });
          if (y >= j) break;
          rest.update(y, 0);
          st.push_back(y);
        }
