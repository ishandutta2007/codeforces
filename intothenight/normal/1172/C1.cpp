...
int main(){
	...
	copy_n(istream_iterator<int>(cin), n, w.begin());
	int good = 0, bad = 0;
	for(int i = 0; i < n; ++ i){
		if(a[i]){
			good += w[i];
	...
	int tot = good + bad;
	vector<vector<Zp>> dp(m + 1, vector<Zp>(m + 1));
	dp[0][0] = 1;
	for(int i = 0; i < m; ++ i){
		for(int j = i; j >= 0; -- j){
			if(tot + 2 * j - i > 0){
				Zp p = Zp(good + j) / (tot + 2 * j - i);
				dp[i + 1][j] += (1 - p) * dp[i][j];
				dp[i + 1][j + 1] += p * dp[i][j];
			}
		}
...
/*
dp[i][j]: after i-th visit, probability of liking j times so far
