...
int main(void)
{
	...
	ll lb = 1LL<<(h-1), ub = (1LL<<h)-1;
	
	ll d, l, r, b;
	rep(i, 1, Q){
		cin >> d >> l >> r >> b;
		l <<= (h-d), r = ((r+1)<<(h-d))-1;
		if(b == 1) chmax(lb, l), chmin(ub, r);
		if(b == 0) mp[l]++, mp[r+1]--;
	}
	if(lb > ub){
		outl("Game cheated!");
	...
	mp2 = mp;
	vector<ll> vec; ll cnt = 0;
	for(auto p : mp){
		if(p.first > ub) break;
		cnt += p.second;
		if(cnt == 0 && p.first >= lb){
			vec.push_back(p.first);
			if(p.first+1 <= ub && mp2[p.first+1] == 0){
				outl("Data not sufficient!");
				return 0;
			}
		}
		//outl(p, cnt);
	}
	if(sz(vec) == 0) outl("Game cheated!");
	else if(sz(vec) >= 2) outl("Data not sufficient!");
	else outl(vec[0]);
