...
int main(){
    ...
    rep(ii,T){
        ...
        rep(i,N){
            ...
            if(a.size()!=0&&a[a.size()-1].first==c){
                auto[x,e] = a[a.size()-1];
                a[a.size()-1] = make_pair(x,e+d);
            }
            else{
                a.emplace_back(c,d);
            }
            //cout << c << " " << d << endl;
            //rep(j,d) a.emplace_back(c);
        }
        ll M; cin >> M;
        vector<Pr> b;
        rep(i,M){
            ...
            if(b.size()!=0&&b[b.size()-1].first==c){
                auto[x,e] = b[b.size()-1];
                b[b.size()-1] = make_pair(x,e+d);
            }
            else{
                b.emplace_back(c,d);
            }
            //rep(j,d) b.emplace_back(c);
        }
        // out(a);
        // out(b);
        if(a==b) ans = "Yes";
