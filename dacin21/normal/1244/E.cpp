...
void solve(){
    ...
    pair<ll, int> L{d.front(), 0}, R{d.back(), 0};
    while(L.first != R.first){
        if(L.second <= R.second){
            const ll A = d.empty() ? R.first : d.front();
            if(!d.empty()) d.pop_front();
            ll need = (A - L.first) * L.second;
            if(k >= need){
                L.first = A;
                k -= need;
                ++L.second;
            ...
            const ll B = d.empty() ? L.first : d.back();
            if(!d.empty()) d.pop_back();
            ll need = (R.first - B) * R.second;
            if(k >= need){
                R.first = B;
                k -= need;
                ++R.second;
