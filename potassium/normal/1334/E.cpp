...
int main() {
	...
	vector<Mint> fact(107, 1), rev(107, 1);
	for (int i = 1; i <= 100; i++)
		fact[i] = fact[i-1] * i;
	rev[100] = fact[100].inv();
	for (int i = 100; i >= 1; i--)
		rev[i - 1] = rev[i] * i;
		
	vector<ll> divisors;
	ll sqrtD = sqrt(D);
	for (ll i = 2; i <= D && i <= sqrtD; i++) {
		if (D % i == 0) {
			divisors.pb(i);
			while (D % i == 0)
				D /= i;
	...
	int divsz = divisors.size();
	
	CASE(t) {
		ll u,v; R(u,v);
		vector<int> cntu(divsz, 0), cntv(divsz, 0);
		
		for (int i = 0; i < divsz; i++) {
			while (u % divisors[i] == 0) {
				cntu[i]++;
				u /= divisors[i];
			}
			while (v % divisors[i] == 0) {
				cntv[i]++;
				v /= divisors[i];
			}
		}
		
		vector<int> gcd(divsz, 0);

		for (int i = 0; i < divsz; i++) {
			gcd[i] = min(cntu[i], cntv[i]);
			cntu[i] -= gcd[i];
			cntv[i] -= gcd[i];
		}
