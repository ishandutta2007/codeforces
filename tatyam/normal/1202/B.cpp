...
ll cost[10][10][10][10];
ll add(ll a,ll b){ll ans=a+b;if(ans>9)ans-=10;return ans;}
signed main(){
    each(i,cost)each(j,i)each(k,j)each(l,k)l=LINF;
    rep(10)rep(j,10)rep(k,10){
        queue<ll>q;
        cost[i][j][k][add(k,i)]=0;
        q.push(add(k,i));
        if(i!=j){
            cost[i][j][k][add(k,j)]=0;
            q.push(add(k,j));
        }
        while(q.size()){
            ll at=q.front();
            q.pop();
            if(chmin(cost[i][j][k][add(at,i)],cost[i][j][k][at]+1))q.push(add(at,i));
            if(i!=j&&chmin(cost[i][j][k][add(at,j)],cost[i][j][k][at]+1))q.push(add(at,j));
        }
    }
    STR(s);
    each(i,s)i-='0';
    ...
    rep(10)rep(j,10){
        rep(k,1,s.size()){
            if(cost[i][j][s[k-1]][s[k]]==LINF){
                ans[i][j]=-1;
                break;
            }
            ans[i][j]+=cost[i][j][s[k-1]][s[k]];
        }
    }
    each(i,ans)out(i);
}