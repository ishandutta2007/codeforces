...
#define int long long
                                                                    ...
                                                                    #define eps                    1e-10
                                                                    #define p                      998244353
                                                                    #if use_dinic==0 and use_mcmf==0 and use_seg==0
                                                                    #undef N
                                                                    #endif
                                                                    #if use_inv==0 and use_C==0 and use_modint==0
                                                                    #undef p
                                                                    #endif
                                                                    #if use_inv==0 and use_C==0 and use_gcd==0 and use_prime==0 and use_phi==0
                                                                    #undef maxw
                                                                    #endif
#endif
...
struct seg_with_lazytag{
    ...
    void update(int num,int le,int ri,int x,int y,int k)
    {
        ...
        tre[num]+=mymax(0,k*(min(ri,y)-max(le,x)+1));
        pushdown(num,le,ri);
        int mid=(le+ri)/2;
...
{
	if(!ck[x])pe[++sum]=x;
	for(int j=1;j<=sum&&x*pe[j]<=maxw;j++)
	{
		ck[x*pe[j]]=1;
		if(x%pe[j]==0)break;
	}
