...
void solve()
{
	...
	int m = min(s, 10);
	vector<pair<u64, int>> candidate(1 << m);
	for (int state = 0; state < 1 << m; ++state)
	{
		...
		candidate[state] = { tmp, __builtin_popcount(state) };
	}

	unordered_map<u64, int> F;
	for (int state = 0; state < 1 << (s - m); ++state)
	{
		//bits tmp;
		u64 tmp = 0;
		for (int i = 0; i < s - m; ++i)
		...
		u64 hash = tmp;
		if (F.count(hash))
			freshmin(F[hash], __builtin_popcount(state));
		else
			F[hash] = __builtin_popcount(state);
	}
	...
	while (d--)
	{
		...
		for (auto& [state, cost] : candidate)
		{
			u64 hash = state ^ tmp;
			if (auto it = F.find(hash); it != F.end())
			{
				freshmin(res, cost + it->second);
			}
		}
		if (res == s + 1) res = -1;
		writeln(res);
