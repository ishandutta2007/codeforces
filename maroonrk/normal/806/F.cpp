...
#endif
template<class t>
using V=array<array<t,S>,3>;
template<class t>
using M=array<V<t>,3>;
using P=M<mint>;
template<class t>
M<t> square_sub(const P&a){
	M<t> x;
	rep(i,3)rep(j,3)rep(k,nmax)
		x[i][j][k]=a[i][j][k].v;
	rep(i,3)rep(j,3){
		inplace_fmt(S,&x[i][j][0],false);
	}
	M<t> res;
	rep(i,3)rep(j,3)rep(k,3)rep(p,S){
		res[i][j][p]+=x[i][k][p]*x[k][j][p];
	}
	rep(i,3)rep(j,3){
		inplace_fmt(S,&res[i][j][0],true);
	}
	return res;
}
P square(const P&a){
	auto x=square_sub<mint0>(a);
	auto y=square_sub<mint1>(a);
	const mint1 r=mint1(mint0::mod).inv();
	P res;
	rep(i,3)rep(j,3)rep(k,nmax)
		res[i][j][k]=mint(x[i][j][k].v+(r*(y[i][j][k]-x[i][j][k].v)).v*(ull)mint0::mod);
	return res;
}
P advance(const P&a){
	P res;
	auto unko=[&](int i,int j,int d){
		rep(k,3)rep(p,nmax-d){
			res[i][k][p+d]+=a[j][k][p];
		}
	};
	unko(0,1,0);
