...
int main() {
    TEST {
        INT(n, d12, d23, d31);
        int sum = d12 + d23 + d31;
        if(sum & 1 or sum / 2 + 1 > n)
            NO();
        else {
            auto solve = [&](int l, int r, int d, int d1, int d2, int m) {
                if(d1 + d2 < d) {
                    NO();
                    return;
                }
                ...
                dump(l, r, d, d1, d2, m, t, k);
                dump(v, w);
                YES();
                rep(i, si(v) - 1) OUT(v[i], v[i + 1]);
                rep(i, si(w) - 1) OUT(w[i], w[i + 1]);
                int ma = max(MAX(w), MAX(v));
                rep(i, ma + 1, n + 1) OUT(1, i);
            };
            if(d12 >= max(d23, d31))
                solve(1, 2, d12, d31, d23, 3);
            else if(d23 >= d31)
                solve(2, 3, d23, d12, d31, 1);
            else
                solve(3, 1, d31, d23, d12, 2);
        }
    }
}