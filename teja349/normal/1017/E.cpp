...
ll a[223456],b[212345],c[212345],d[212345],e[212345],f[212345],g[212345],h[212345];
ll lpsconstruct(ll lps[],ll a[],ll n){
	ll i=1;
	lps[0]=0;
	ll j=0;
	while(i<n){
...
ll getmatcher(ll match[],ll a[],ll b[],ll n,ll lps[]){
	ll i=0,j=0;
	while(i<2*n){
		if(a[i]==b[j]){
			match[i]=j+1;
...
int main(){
    ...
    pts2.pb(pts2[0]);
    rep(i,pts1.size()-1){
    	a[i]=dist2(pts1[i],pts1[i+1]);
    }
    rep(i,pts2.size()){
    	b[i]=dist2(pts2[i],pts2[i+1]);
    }
    ...
    pts1.pb(pts1[1]);
    f(i,1,pts2.size()-1){
    	d[i-1]=dist2(pts2[i-1]-pts2[i],pts2[i+1]-pts2[i]);
    	c[i-1]=dist2(pts1[i-1]-pts1[i],pts1[i+1]-pts1[i]);
    }
    f(i,n,2*n){
    	a[i]=a[i-n];
    	c[i]=c[i-n];	
    ...
    lpsconstruct(e,b,n);
    lpsconstruct(f,d,n);
   	getmatcher(g,a,b,n,e);
   	getmatcher(h,c,d,n,f);
   	rep(i,2*n){
   		if(g[i]==n && h[i]==n){
   			cout<<"YES"<<endl;
   			return 0;
