...
int main() {
    ...
    for(ll i = 0; i < n; i++){
    	//ll num = a[i][0];
    	for(ll j = 0; j < numd[i]; j++){
    		ans+=(a[i][j]*digits[j]) % MOD;
    		ans %= MOD;
    	}
    	for(ll j = numd[i]; j < 12; j++){
    		ans+=(a[i][numd[i]-1]*digits[j]) % MOD;
    		ans%= MOD;
    	}
    	for(ll j = 1; j < numd[i]; j++){
    		ans+=((a[i][j-1]*digits[j]) % MOD*10) % MOD;
    		ans %= MOD;
    	}
    	for(ll j = numd[i]; j < 12; j++){
    		ans+=(((a[i][numd[i]-1]*10) % MOD)*digits[j]) % MOD;
    		ans %= MOD;
    	}
    }
    cout << ans % MOD;
