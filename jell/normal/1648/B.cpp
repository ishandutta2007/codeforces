#line 1 "codeforces-workspace\\contest\\1648\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1648\\b\\b.cpp"
...
void main() {
  ...
  for (auto i : range(1, c + 1)) {
    if (a[i]) {
      for (auto j : range(1, c / i + 1)) {
        if (!a[j] and b[min(i * (j + 1) - 1, (i32)c)] != b[i * j - 1]) {
          eject("No");
        }
      }
    }
