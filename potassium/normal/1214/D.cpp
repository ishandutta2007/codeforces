...
vector<vector<int>> isTraversed;
int main() {
	int n,m; R(n,m);

	vector<string> mm(n); R(mm);

	queue<pii> q;
	q.push({0,0});
	isTraversed.resize(n+1,vector<int>(m+1,0));
	isTraversed[0][0] = 1;
	while (!q.empty()) {
		int r = q.front().F;
		int c = q.front().S;

		if (r+1 < n && isTraversed[r+1][c] < 1 && mm[r+1][c] != '#') {
			isTraversed[r+1][c] = 1;
			q.push({r+1,c});
		}
		if (c+1 < m && isTraversed[r][c+1] < 1 && mm[r][c+1] != '#') {
			isTraversed[r][c+1] = 1;
			q.push({r,c+1});
		}
		q.pop();
	...
	while (!q.empty()) {
		...
		if (r-1 >= 0 && isTraversed[r-1][c] < 2 && mm[r-1][c] != '#') {
			isTraversed[r-1][c] += 2;
			q.push({r-1,c});
		}
		if (c-1 >= 0 && isTraversed[r][c-1] < 2 && mm[r][c-1] != '#') {
			isTraversed[r][c-1] += 2;
			q.push({r,c-1});
		}
		q.pop();
	...
	for (int i = 1; i <= n+m-3; i++) {
		int cnt = 0;
		for (int r = max(0,i+1-m), c = i - r; r < n; r++, c--) {
			if (c >= 0 && c < m)
				if (isTraversed[r][c] == 3) cnt++;
			if (c < 0) break;
		}
