...
int main() {
	...
	map<ll, vector<pll>> m;
	for (int i = 0; i < n; i++)
		m[a[i] % g].push_back({ a[i] / g, i });

	vector<ll> ans(n, 0);
	auto solve = [&](vector<pll>& x) {
		const int n = (int)x.size();
		vector<pll> invX(n);
		for (int i = 0; i < n; i++) {
			invX[i].first = (x[i].first * invP) % t;
			invX[i].second = x[i].second;
		}

		sort(invX.begin(), invX.end());
		invX.resize(unique(invX.begin(), invX.end(), [](pll x, pll y) { return x.first == y.first; }) - invX.begin());
		const int m = (int)invX.size();

		for (int i = 0; i < m; i++) {
			int nxt = (i + 1) % m;
			ll d = invX[nxt].first - invX[i].first;
			if (d <= 0) d += t;
			ans[invX[i].second] = d;
		}
	};
