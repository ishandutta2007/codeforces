...
struct Segnode {
    ...
    // adds ia+b to a range, then pushdown. don't keep the indices
    int64_t a = 0, b = 0;

    Segnode() = default;

    int64_t get(int i) const { return a * (i + 1) + b; }
    ...
    void apply(int64_t ap, int64_t bp) { a += ap, b += bp; }
};
...
int main() {
    ...
    segtree<Segnode> seg(N, {});
    int64_t ops = 0;

    for (int i = N - 1; i >= K - 1; i--) {
        int l = i - K + 1, r = i + 1;
        int64_t v = seg.query_point(i).get(i);
        int64_t add = max<int64_t>(0, (b[i] - v + K - 1) / K);
        // for i=0, add (1,0). for i=1, add (1,-1). so (1,-i)
        seg.update_range(l, r, add, -add * l);
        ops += add;
    }
    for (int i = 0; i < K - 1; i++) {
        int64_t v = seg.query_point(i).get(i);
        int64_t add = max<int64_t>(0, (b[i] - v + i) / (i + 1));
        seg.update_range(0, K, add, 0);
        ops += add;
    }
