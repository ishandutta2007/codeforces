...
int main(int argc, char **argv){
  ...
  forn(m,1<<n){
    ...
    forn(s,mxs+1)
      dp[m][s]=-INF;
    int mxps=(cm-1)*(cm-2)/2;
    forn(k,n){
      if(!(m&(1<<k)))continue;
      int mm=m^(1<<k);
      forn(ps,mxps+1){
        if(dp[mm][ps]<0)continue;
        int s=ps+min(R[k],cr-(C[k]=='R'));
        assert(s<=mxs);
        if(m==1)
          dbg(k,mm,ps,s,dp[mm][ps],mxs,mxps);
        dp[m][s]=max(dp[m][s],dp[mm][ps]+min(B[k],cb-(C[k]=='B')));
      }
    }
  }
  dbg(dp);
  dbg(mxs);
  int res=INF;
  int sr=accumulate(all(R),0);
  int sb=accumulate(all(B),0);
  forn(s,mxs+1){
    res=min(res,n+max(0,max(sr-s,sb-dp.back()[s])));
  }
  cout<<res;
