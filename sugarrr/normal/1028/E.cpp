...
vl dfs(vl& ret,ll n,vl&v){
    if(ret.size()==n){
        if(ret[n-1]%ret[0]==v[n-1])return ret;
        else return {};
    }
    if(ret.size()==0){
        rep(i,1,30){
            ret.pb(i);
            auto ans=dfs(ret,n,v);
...
signed main(){fastio
    ll n;cin>>n;
    vl v(n);rep(i,0,n-1)cin>>v[i];
    same_(v);
    vl ans(n);
    ll mx=0;for(auto x:v)chmax(mx,x);
    ll pos=-1;
    rep(i,0,n-1){
        if(v[i]==mx && v[(i-1+n)%n]!=v[i]){
            pos=i;break;
        }
    }
    assert(pos!=-1);
    ans[pos]=v[pos];
    rrep(i,pos+n-1,pos+1){
        ll k=i%n;
        
        if(i==pos+n-1)ans[k]=2*ans[(k+1)%n]+v[k];
        else ans[k]=ans[(k+1)%n]+v[k];
    }
    cout<<"YES"<<endl;
