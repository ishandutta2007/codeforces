...
const int N=1e6+5,L=1,R=1e6;
int su[N<<2],mx[N<<2],a[N],b[N],n,m,q;
void add(int x,int l,int r,int p,int u){
	su[x]+=u;
	if(l==r){ mx[x]=su[x]; return; }
	int mid=(l+r)>>1,ls=x<<1,rs=ls|1;
	if(p<=mid) add(ls,l,mid,p,u);
	else add(rs,mid+1,r,p,u);
	mx[x]=max(mx[ls]+su[rs],mx[rs]);
}
int find(int x,int l,int r,int e){
	if(mx[x]<e) return -1;
	if(l==r) return l;
	int mid=(l+r)>>1,ls=x<<1,rs=ls|1;
	if(mx[rs]>=e) return find(rs,mid+1,r,e);
	return find(ls,l,mid,e-su[rs]);
}
int main(){
	ios::sync_with_stdio(false); cin.tie(0); cout.tie(0);
	scan(n,m);
	for(int i=1;i<=n;++i) scan(a[i]),add(1,L,R,a[i],1);
	for(int i=1;i<=m;++i) scan(b[i]),add(1,L,R,b[i],-1);
	scan(q);
	for(int i=1,o,p,x;i<=q;++i){
		scan(o,p,x);
		if(o==1) add(1,L,R,a[p],-1),add(1,L,R,a[p]=x,1);
		else add(1,L,R,b[p],1),add(1,L,R,b[p]=x,-1);
		print(find(1,L,R,1)),putc('\n');
	}
	return 0;
}