...
void main() {
  init(static_cast<int>(1e6));
  auto is_prime = [](int n) { return lpf[n] == n; };
  int tt;
  scan(tt);
  while (tt--) {
    int n, m;
    ...
    long long ans = 0;
    for (int r : rep(m)) {
      auto b = a | sliced(r, n) | strided(m);
      DUMP(r, b);
      for (int i : rep(static_cast<int>(b.size())) | IF(is_prime(b[_]))) {
        int l = i - 1;
        while (0 <= l && b[l] == 1) --l;
        int r = i + 1;
        while (r < static_cast<int>(b.size()) && b[r] == 1) ++r;
        ans += static_cast<long long>(r - i) * static_cast<long long>(i - l) - 1;
      }
    }
    println(ans);
  }
