...
int main() {
	...
	CASE(t) {
		...
		vector<int> ps(2 * n);
		for (int i = 0; i < 2 * n; i++) ps[i] = val(s[i]);
		for (int i = 1; i < 2 * n; i++) ps[i] += ps[i - 1];

		int lm = -1, rm = 2 * n;
		while (lm + 1 < 2 * n && ps[lm + 1] >= 0) lm++;
		while (rm - 1 >= 0 && ps[rm - 1] >= 0) rm--;
		...
		int lh = 0, rh = 0, mh = INT_MIN;
		int lp = -1, rp = 2 * n - 1, mp = rm;
		for (int i = 0; i <= lm; i++) if (chkmax(lh, ps[i])) lp = i;
		for (int i = lm + 1; i < rm; i++) if (chkmax(mh, ps[i])) mp = i;
		for (int i = rm; i < 2 * n; i++) if (chkmax(rh, ps[i])) rp = i;
