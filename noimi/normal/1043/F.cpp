...
main(){
    ...
    rep2(i,2,T){
        if(primes[i]){
            for(int j = i*2;j<= T;j+=i) primes[j] = 0;
        }
    }
    vi mobius(T+1,-1);
    rep2(i,2,T){
    ...
    rep(i,n){
        ...
        dp[0] = 1;
        rep(mask,1<<k){
            for(int s = mask;;s = mask & (s-1)){
                if(mem[s]){
                    chmin(dp[mask],dp[s^mask] + 1);
                }
                if(s == 0) break;
            }
        }
