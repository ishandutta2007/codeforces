...
void dfs(int u, int p)
{
    ...
    for (int v: g[u])
    {
        ...
        int sz2 = dp[v].size();
        int sz = sz1+sz2;
        vector<ll> nxt(sz), nxt2(sz);
        for (int i=0;i<sz1;i++)
        {
            for (int j=0;j<sz2;j++)
            {
                int a = min(i,j+1);
                nxt[a] = (nxt[a]+dp[u][i]*dp[v][j])%mod;
                a = max(i,j+1);
                nxt2[a] = (nxt2[a]+dp2[u][i]*dp2[v][j])%mod;
                if (i+j+1<=R)
                {
                    nxt[i] = (nxt[i]+dp[u][i]*dp2[v][j])%mod;
                    nxt[j+1] = (nxt[j+1]+dp2[u][i]*dp[v][j])%mod;
                } else
                {
                    nxt2[i] = (nxt2[i]+dp2[u][i]*dp[v][j])%mod;
                    nxt2[j+1] = (nxt2[j+1]+dp[u][i]*dp2[v][j])%mod;
                }
            }
        }
        dp[u] = nxt;
