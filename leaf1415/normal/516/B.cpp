...
int main(void)
{
	...
	set<P> S;
	rep(y, 1, h) rep(x, 1, w){
		if(deg[x][y] == 1) S.insert(P(x, y));
	}
	
	while(sz(S)){
		P p = *S.begin(), q;
		rep(i, 0, 3){
			ll nx = p.fst + dx[i], ny = p.snd + dy[i];
			if(c[nx][ny] == '.'){
				q = P(nx, ny);
				break;
			}
		...
		rep(i, 0, 3){
			...
			if(c[nx][ny] == '.'){
				deg[nx][ny]--;
				if(deg[nx][ny] == 1) S.insert(P(nx, ny));
				else S.erase(P(nx, ny));
			}
		}
		rep(i, 0, 3){
			ll nx = q.fst + dx[i], ny = q.snd + dy[i];
			if(c[nx][ny] == '.'){
				deg[nx][ny]--;
				if(deg[nx][ny] == 1) S.insert(P(nx, ny));
				else S.erase(P(nx, ny));
