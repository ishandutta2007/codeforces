...
int main() {
    ...
    vector<vector<pair<int, int>>> segs(k);
    rep(i, k) {
        int l = 0;
        while (l < n) {
            while (l < n and (s[l] != -1 and s[l] != i)) {
                ++l;
            }
            if (l == n) break;
            int r = l;
            while (r < n and (s[r] == -1 or s[r] == i)) {
                ++r;
            }
    ...
    while (r - l > 1) {
        ...
        vector<int> dp(1 << k, numeric_limits<int>::max());
        dp[0] = 0;
        rep(s, 1, 1 << k) {
            rep(c, k) {
                if (not kth_bit(s, c)) continue;
                int t = s ^ (1 << c);
                if (dp[t] == numeric_limits<int>::max()) continue;
                int j = lower_bound(all(ls[c]), dp[t]) - ls[c].begin();
                if (j == int(ls[c].size())) continue;
                chmin(dp[s], ls[c][j] + x);
            }
        }
        if (dp[(1 << k) - 1] <= n) {
