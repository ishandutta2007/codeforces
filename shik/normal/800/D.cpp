...
struct P {
    ...
    mint get() {
        return s0<2 ? s2 : p2[s0-2]*(s1*s1+s2);
    }
} p[N];
...
int main() {
    ...
    for ( int i=1; i<N; i*=10 ) for ( int j=N-1; j>=0; j-- ) if ( j/i%10!=0 ) p[j-i]+=p[j];
    REP(i,N) q[i]=p[i].get();
    for ( int i=1; i<N; i*=10 ) REP(j,N) if ( j/i%10!=9 ) q[j]-=q[j+i];
    LL ans=0;
    REP(i,N) ans^=1LL*i*q[i].x;
    W(ans);
    return 0;
}