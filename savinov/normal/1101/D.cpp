...
pair <int, int> dfs(int v, int p, const vector <vector <int> > &g, vector <int> &used) {
    pii res = make_pair(1, v);
    used[v] = 1;
    for (int to : g[v]) {
        if (to == p) continue;
        auto t = dfs(to, v, g, used);
        t.x += 1;
        res = max(res, t);
    }
    return res;
...
int solve(const vector <vector <int> > &g) {
    ...
    for (int i = 0; i < g.size(); ++i) {
        if (!used[i]) {
            auto t = dfs(i, i, g, used);
            t = dfs(t.y, t.y, g, used);
            res = max(res, t.x);
        }
    }
...
void solve(int test) {
    ...
    for (int i = 2; i < lp.size(); ++i) {
        if (!lp[i]) {
            for (int j = i; j < lp.size(); j += i) {
                lp[j] = i;
            }
        }
    }
    vector <vector <int> > vv(lp.size());
...
//        if (n == 100) cout << x << " ";
    ...
    for (const auto &v : vv) {
        if (!v.empty()) {
            vector <vector <int> > ng(v.size());
            for (int j = 0; j < v.size(); ++j) {
                for (auto to : g[v[j]]) {
                    auto iter = lower_bound(all(v), to) - v.begin();
                    if (iter < v.size() && v[iter] == to) {
                        ng[j].push_back(iter);
                    }
                }
            }
            ans = max(ans, solve(ng));
        }
    }
    cout << ans << "\n";
}
