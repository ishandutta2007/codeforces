...
void solve(){
	read(n), read(m);
	if (m == 0){
		for (register int i = 1; i <= n; ++i) print(100000000 + 10000 * i, ' ');
		return;
	}
	int now = 0, k = 1;
	while (k <= n){
		int t = (k - 1) >> 1;
		if (now + t < m) a[k] = k, now += t, ++k;
		else break;
	}
	if (k == n + 1) return print(-1), void(0);
	for (register int i = k; i; --i){
		int t = (i - 1) >> 1;
		if (now + t == m){ a[k] = k + k - i, now += t; break; }
	}
	for (register int i = 1; i <= k; ++i) print(a[i], ' ');
	for (register int i = k + 1; i <= n; ++i) print(100000000 + 10000 * i, ' ');
	putchar('\n');
}
int main(){
	int T = 1;
