...
fun main(){
    ...
    solve.cases{
        val n = getint
        val k = getint
        val euler = IntArray(1_000_004){
            if(it == 0){
                return@IntArray 0
            }else{
                return@IntArray totient(it)
            }
        }
        val all = mint
        all.addAll(euler.slice(3..n))
        var ret = all.sorted().take(k).sumOf { it.toLong() } + 2
        ...
        if(all2.size >= k ){
            ret = minOf(ret, all2.sorted().take(k).sumOf { it.toLong() } + 1 )
        }
        put(ret)
