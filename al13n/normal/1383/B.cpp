...
int main(int argc, char **argv){
  ...
  /*
  vector<vector<array<array<int,2>,2>>> A(100,vector<array<array<int,2>,2>>(100));
  forv(i,A){
    forv(j,A[i]){
      forn(k,2){
        forn(q,2){
          if(!i&&!j) A[i][j][k][q]=!k;
          if(i) A[i][j][k][q]|=!A[i-1][j][q][1-k];
          if(j) A[i][j][k][q]|=!A[i][j-1][q][k];
        }
      }
    }
  }
  for(int i=1;i<30;i+=2){
    forn(j,20) cout<<A[i][j][1][1]<<' ';
    cout<<endl;
    }*/
  ...
  forn(qq,tc){
    ...
    while(!(x&(1ll<<k))) --k;
    int c=0;
    forn(i,n) if(A[i]&(1ll<<k)) ++c;
    assert(c%2);
    cout<<(((c/2%2)&&!((n-c)%2))?"LOSE":"WIN")<<'\n';
  }
