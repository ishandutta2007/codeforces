...
constexpr array<int, 3> NO = {-1, -1, -1};
...
void solve() {
    ...
    map<string, array<int, 3>> fragments;
    for (int j = 0; j < n; j++) {
        auto &s = known[j];
        for (int len : {2, 3}) {
            for (int i = 0; i + len <= m; i++) {
                fragments[s.substr(i, len)] = {i + 1, i + len, j + 1};
            }
        }
    }

    vector<array<int, 3>> possible(m + 1, NO);
    possible[0] = {};
    for (int i = 0; i < m; i++) {
        if (possible[i] == NO)
            continue;
        ...
        for (int len : {2, 3}) {
            if (i + len > m)
                continue;
            auto it = fragments.find(goal.substr(i, len));
            if (it != fragments.end())
                possible[i + len] = it->second;
        }
    }
