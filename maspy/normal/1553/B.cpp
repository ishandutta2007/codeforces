...
void solve() {
  ...
  auto T = sin();
  FOR(n, T.size()) {
    string L = T.substr(0, n + 1);
    string R = T.substr(n, T.size() - n);
    reverse(all(R));
    FOR3(r, max(L.size(), R.size()), S.size() + 1) {
      string SL = S.substr(r - L.size(), L.size());
      string SR = S.substr(r - R.size(), R.size());
      if (SL == L && SR == R) {
        YES();
        return;
      }
