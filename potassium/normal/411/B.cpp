...
int main() {
	int n,m,k; R(n,m,k);
	vector<vector<int>> inst(n,vector<int>(m));
	for (auto& i : inst) for (auto& j : i) R(j);
	
	vector<bool> isMemLocked(k+7,false);
	vector<bool> isCoreLocked(n+7,false);
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < n; j++) {
			if (!isCoreLocked[j] && isMemLocked[inst[j][i]]) {
				ans[j] = i+1;
				isCoreLocked[j] = 1;
			}
		}
