...
fun check(i: Int, j: Int): Int {
    val mn = IntArray(10,{MOD})
    mn[0] = 0
    for (x in 1..10) {
        for (y in 0..9) {
            mn[(y+i)%10] = minOf(mn[(y+i)%10],mn[y]+1)
            mn[(y+j)%10] = minOf(mn[(y+j)%10],mn[y]+1)
        }
    }
    if (i == 0 || j == 0) {
        mn[0] = 1
    } else {
        mn[0] = minOf(mn[10-i]+1,mn[10-j]+1)
    }
    var ans = 0
    for (i in 0..9) if (cnt[i] > 0) {
        if (mn[i] == MOD) return -1
        ans += (mn[i]-1)*cnt[i]
    }
    return ans
}
