...
int main() {
    ...
    sort(v.begin(), v.end(), [&](const auto& a, const auto& b) {
        return tie(max(a.first, a.second), a.first) < tie(max(b.first, b.second), b.first);
    });
    ...
    for (auto [s, a] : v) {
        ...
        a = lowb(x, a);
        if (s <= a) {
            int best = tree.ask(0, s).mx;
            tree.set(a, best + 1);
        } else {
            int ba = tree.ask(0, a).mx;
            tree.modify(a, s, 1);
            tree.set(a, ba + 1);
        }
        debug {
