...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	const int n=x.size();
	const int m=y.size();
	rep(i,n)rep(j,m) dp[i+1][j][1][0][0]=dp[i][j+1][0][1][1]=1;
	rep(i,n+1)rep(j,m+1)rep(fi,2)rep(fj,2){
		if(i<n && i && x[i-1]!=x[i]) dp[i+1][j][1][fj][0]+=dp[i][j][fi][fj][0];
		if(i<n && j && y[j-1]!=x[i]) dp[i+1][j][1][fj][0]+=dp[i][j][fi][fj][1];
		if(j<m && i && x[i-1]!=y[j]) dp[i][j+1][fi][1][1]+=dp[i][j][fi][fj][0];
		if(j<m && j && y[j-1]!=y[j]) dp[i][j+1][fi][1][1]+=dp[i][j][fi][fj][1];
	}
	mint re=0;
	rep(i,n+1)rep(j,m+1)rep(f,2) re+=dp[i][j][1][1][f];
	// rep(i,n+1)rep(j,m+1)rep(f,2)if(dp[i][j][1][1][f].v) out(i,j,f,dp[i][j][1][1][f],1);
	cout<<re NL;
  return 0;
}