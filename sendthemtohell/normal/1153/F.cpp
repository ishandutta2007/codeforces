...
inline void solve() {
	...
	for (int i = 1; i < N; ++i) {
		cnk[i][0] = 1;
		for (int j = 1; j <= i; ++j) {
			cnk[i][j] = (cnk[i - 1][j - 1] + cnk[i - 1][j]) % FFT_MOD;
		}
	}

	fct[0] = 1;
	for (int i = 1; i < N; ++i) {
		fct[i] = fct[i - 1] * i % FFT_MOD;
	}

	enpair[0] = 1;
	for (int i = 2; i < N; i += 2) {
		enpair[i] = (i - 1) * enpair[i - 2] % FFT_MOD;
	}
	...
	int k, l;
	cin >> k >> l;
	ll invv = inv(2 * n + 1, FFT_MOD);
	for (int i = 0; i <= 2 * n; ++i) {
		int lft = i;
		int rgt = 2 * n - i;
		for (int j = k; j <= min(lft, rgt); ++j) {
			ans = (ans + cnk[lft][j] * cnk[rgt][j] % FFT_MOD * enpair[lft - j] % FFT_MOD * enpair[rgt - j] % FFT_MOD * fct[j] % FFT_MOD * invv % FFT_MOD * l % FFT_MOD) % FFT_MOD;
		}
	}

	ans = ans * inv(enpair[2 * n], FFT_MOD) % FFT_MOD;
