...
// vector<mint> sccs{0, 1, 0, 2, 24, 544, 22320, 1677488, 236522496, 26088128, 909959656, 171021868, 624159790, 795709201, 822073304, 251413740};
...
int main(){
	...
	cauto n=read();
	cauto vs=readV(n);
	vector win=table(n,n,mint(0));
	rep(i,n)rep(j,n) win[i][j]=mint(vs[i])/(vs[i]+vs[j]);
	vector<mint> sccs(1<<n);
	mint ans=0;
	rep(st,1<<n){
		cauto sz=__builtin_popcount(st);
		auto &re=sccs[st];
		if(sz<=1){
			re=1;
		}else if(sz==2){
			...
			re=1;
			for(int sub=(st-1)&st;sub;sub=(sub-1)&st){
				mint p=sccs[sub];
				rep(i,n)if(sub>>i&1)rep(j,n)if((st^sub)>>j&1) p*=win[i][j];
				re-=p;
			}
		}
		mint p=re;
		rep(i,n)if(st>>i&1)rep(j,n)if(~st>>j&1) p*=win[i][j];
		out(st,sz,p,1);
		ans+=sz*p;
	}
	out(sccs,1);
