...
ll min_val(ll x, ll k, ll y, vector<ll>& curr, ll prev, ll nx){
	...
	ll n = curr.size();
	ll ex = n % k;
	if (max_val < prev || max_val < nx || y <= x*k){
		ll nv =(ex * x) + ((min(y,x*k)) * (n / k));
		assert( nv <= n*x); 
		return (ex * x) + ((min(y,x*k)) * (n / k));
	}
	return y  + ((n - k) * x);
}
void solve(){
    ...
    ll ans = 0;
    for(int i = 0; i < n; ++i){
    	if (it_b < b.size() && a[i] == b[it_b]){
    		ll z = min_val(x,k,y,curr,prev,b[it_b]);
    		if (z == -1){
    			cout << -1 << endl;
    			return;
    		}
    ...
    ll z = min_val(x,k,y,curr,prev,-1);
	if (z == -1){
		cout << -1 << endl;
		return;
	}
