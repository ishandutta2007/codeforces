...
cin>>T;while(T--)
{
    ...
    auto solveOdd=[&](lli n,lli k)->mint{
        mint ans=0;
        for(lli i=0;i<=k;++i){
            ans+=nCr(k,i)*(mint(2).pow((k-i)*(n-1)));
        }
        return ans;
    };

    auto solveEven=[&](lli n,lli k)->mint{
        mint ans=(mint(2).pow(n-1)-1).pow(k);
        for(lli i=1;i<=k;++i){
            const auto val=(mint(2).pow((i-1)*n))*((mint(2).pow(n-1)-1).pow(k-i));
            ans+=val;
            dbg(i,val);
        }
        return ans;
