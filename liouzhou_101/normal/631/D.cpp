...
void solve()
{
	...
	auto s = read_string(n);
	auto t = read_string(m);
	...
	if (t.size() == 1)
	{
		...
		for (auto [x, c] : s)
		{
			if (t[0].second == c) res += max(x - t[0].first + 1, 0LL);
		}
		return writeln(res);
	}

	vector<pll> r;
	for (int i = 1; i + 1 < t.size(); ++i) r.push_back(t[i]);
	int lent = r.size();
	r.push_back({ 0, 0 });
	int offset = r.size();
	for (auto e : s) r.push_back(e);
	auto z = string_algorithm::z_algorithm(r);
	...
	for (int i = 1; i < s.size(); ++i)
	{
		if (z[offset + i] == lent)
		{
			if (s[i - 1].second == t[0].second && s[i - 1].first >= t[0].first && s[i + lent].second == t.back().second && s[i + lent].first >= t.back().first)
			{
				res++;
			}
		}
