...
//		vector<vector<int> > dp(5, vector<int>(5, INF));
		array<array<int, 5>, 5> dp; //add
        for (int i = 0; i < 5; i++) dp[i].fill(INF); //add
		dp[0][0] = 0;
		for (unko* z: v) {
//			vector<vector<int> > _dp(5, vector<int>(5, INF));
            array<array<int, 5>, 5> _dp; //add
            for (int i = 0; i < 5; i++) _dp[i].fill(INF); //add
			for (int x = 0; x < 5; x++)
				for (int y = x; y < 5; y++)
					for (int m = x; m <= y; m++)
