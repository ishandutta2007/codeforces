...
bool solve(vl a){
    ...
    dp[0][N]=1;
   // rep(j,0,N*2)cout<<dp[0][j].val()<<" ";cout<<endl;
   // rep(j,0,N*2)cout<<dp[0][j]<<" ";cout<<endl;
    rep(i,0,n-1){
        //mint sum=0;
        ll sum=0;
        rep(j,0,N*2){
            if(0<=j+a[i]&&j+a[i]<=N*2)dp[i+1][j+a[i]]+=dp[i][j];
            sum+=dp[i][j];
        }
        ll ns=a[i]+N;
        dp[i+1][ns]=sum;
        //rep(j,0,N*2)cout<<dp[i+1][j].val()<<" ";cout<<endl;
       // rep(j,0,N*2)cout<<dp[i+1][j]<<" ";cout<<endl;
    }
  //  cout<<endl;
    rep(i,0,n){
        ll ans=0;
...
mint solve2(vl a){
    ll n=a.size();
    vl s(n+1);rep(i,1,n)s[i]=a[i-1]+s[i-1];
    vi dp(n+1);
    map<ll,mint>mp;
    dp[0]=dp[1]=1;
    rep(i,2,n){
        mp[s[i-2]]=dp[i-2];
        dp[i]=dp[i-1]*2-mp[s[i-1]];
       /* rrep(j,i-2,0){
            if(s[j]==s[i-1]){
                dp[i]-=dp[j];break;
            }
