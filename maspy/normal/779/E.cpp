...
void solve() {
  ...
  FOR(i, N) {
    ...
    if (len(tokens) == 3) {
      string s = tokens[2];
      B x;
      FOR(i, len(s)) x[i] = (s[i] == '1');
      X[LHS] = Y[LHS] = x;
    } else {
      string A = tokens[2], B = tokens[4];
      string OP = tokens[3];
      if (OP == "AND") X[LHS] = X[A] & X[B];
      if (OP == "AND") Y[LHS] = Y[A] & Y[B];
      if (OP == "OR") X[LHS] = X[A] | X[B];
      if (OP == "OR") Y[LHS] = Y[A] | Y[B];
      if (OP == "XOR") X[LHS] = X[A] ^ X[B];
      if (OP == "XOR") Y[LHS] = Y[A] ^ Y[B];
    }
  }
  string lo, hi;
  FOR(i, M) {
