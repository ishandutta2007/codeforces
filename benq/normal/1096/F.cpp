...
int main() {
    ...
    F0R(i,n) {
        if (p[i] == -1) bad ++;
            ...
            AD(ans,sz(T)-T.order_of_key(p[i]));
            T.insert(p[i]);
            lef.erase(p[i]);
        }
    }
    AD(ans,mul(bad,mul(bad-1,inv(4))));
    int tmp = 0;
    FOR(i,1,n+1) if (lef.find(i) == lef.end()) AD(tmp,sz(lef)-lef.order_of_key(i));
    F0R(i,n) {
        if (p[i] == -1) {
            AD(ans,mul(tmp,inv(bad)));
        } else {
            int x = p[i];
            SUB(tmp,sz(lef)-lef.order_of_key(x));
            AD(tmp,lef.order_of_key(x));
        }
    }
    pr(ans);
    // you should actually read the stuff at the bottom
