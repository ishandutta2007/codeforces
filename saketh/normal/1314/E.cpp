...
int sum(int v) { return v * (v + 1) / 2; }

const int MAXV = 2021;
const int MAXS = ceil(sqrt(2 * MAXV)) + 1;
int dp[MAXS][MAXS][MAXV];

int solve_k3(int N) {
    // since the size for k=2 is bounded by MAX_SUM,
    // the sum of the elements here is bounded by MAX_SUM
    const int MAX_SUM = ceil(sqrt(2 * N)) + 1;

    dp[0][0][0] = 1;

    FORd (v, 1, MAX_SUM) {
        F0R (placed, MAX_SUM - 1) {
            F0R (sum_placed, MAX_SUM - v) {
                if (sum_placed < placed * v) continue;

                int cost = 0;
                FOR (w, 1, v + 1) {
                    int geq = sum_placed - (w - 1) * placed;
                    cost += sum(geq + v - w + 1) - sum(geq);
                }
                if (cost > N) break;

                F0R (sum_cost, N + 1 - cost) {
                    dp[placed + 1][sum_placed + v][sum_cost + cost]
                        += dp[placed][sum_placed][sum_cost];
                }
            }
        }
    }
    ...
    int ans = 0;
    FOR (placed, 1, MAX_SUM)
        F0R (sum_placed, MAX_SUM)
            F0R (sum_cost, N + 1)
                ans += dp[placed][sum_placed][sum_cost];
    return ans;
}

vi vals[MAXS];

int cost(int level, int limit) {
    ...
    return cost(level - 1, limit);
}

int go(int level, int limit) {
    vi &v = vals[level];

    int ans = 0;
    while (1) {
        int cur_cost = cost(level, limit);
        if (cur_cost <= limit) {
            ans++;
        } else break;
