...
int Solve(){
	...
	static int smallSS[MAXN], largePS[MAXN];
	counter.clear();
	Forx(i,0,k+10) smallSS[i] = largePS[i] = 0;
	For(i,n/2){
		int x = a[i], y = a[n-i+1];
		if( x > y ) swap(x,y);
		++counter[x+y];
	...
	For(i,k+10) largePS[i] += largePS[i-1];
	fORx(i,0,k+9) smallSS[i] += smallSS[i+1];
	int ans = INF;
	Forx(val,2,k+k){
		int tans = n/2 - counter[val];
		if( val-k-1 > 0 ) tans += largePS[val-k-1];
		if( val <= k ) tans += smallSS[val];
		Mymin(ans,tans);
	}
	return ans;
}
