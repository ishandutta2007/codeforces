...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            var res = dfs(0, -1, edge, l, r);
            Prt(Math.Max(res.v1, res.v2));
        }
    }
    pair<long, long> dfs(int p, int par, List<int>[] edge, int[] l, int[] r) {
        long sum_l = 0, sum_r = 0;
        foreach (var item in edge[p])
        {
            if (item == par) continue;
            var res = dfs(item, p, edge, l, r);
            sum_l += Math.Max(res.v1 + Math.Abs(l[p] - l[item]), res.v2 + Math.Abs(l[p] - r[item]));
            sum_r += Math.Max(res.v1 + Math.Abs(r[p] - l[item]), res.v2 + Math.Abs(r[p] - r[item]));
        }
        return new pair<long, long>(sum_l, sum_r);
    }
