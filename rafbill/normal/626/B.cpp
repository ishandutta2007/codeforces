...
int main(int, char**){
  ...
  lli n; cin >> n;
  string s; cin >> s;
  vi t(n); FOR(i, n) FOR(j, 3) if(s[i] == C[j]) t[i] = j;
  FOR(i, n) A[t[i]] += 1;
  FOR(a, 3) FOR(b, 3) FOR(c, 3) if(a!=b&&b!=c&&a!=c){
    if(A[a]&&A[b]) B[c] = 1;
    if(!A[a]&&!A[b]&&A[c]) B[c] = 1;
    if(!A[a]&&A[b]>=2&&A[c]) B[c] = 1;
  }

  FOR(i, 3) if(B[i]) cout << C[i]; cout << endl;
