...
int solve() {
    ...
    memset(ver, 60, sizeof(ver));
    memset(hor, 60, sizeof(hor));
//    DBN(dp[0][0][0]);
    int n, m, k;
    cin >> n >> m >> k;
    FI(n) FJ(m) dp[0][i + 1][j + 1] = 0;
    if (k % 2 == 1) {
        FI(n) {
            FJ(m) {
                cout << "-1 ";
    ...
    for (int s = 1; s <= 10; ++s) {
        for (int i = 1; i <= n; ++i) {
            for (int j = 1; j <= m; ++j) {
                dp[s][i][j] = min(dp[s][i][j], dp[s - 1][i - 1][j] + ver[i - 1][j]);
                dp[s][i][j] = min(dp[s][i][j], dp[s - 1][i + 1][j] + ver[i][j]);
                dp[s][i][j] = min(dp[s][i][j], dp[s - 1][i][j - 1] + hor[i][j - 1]);
                dp[s][i][j] = min(dp[s][i][j], dp[s - 1][i][j + 1] + hor[i][j]);
            }
        }
    }
//    for (int s = 0; s <= k / 2; cout << endl, ++s)
    FI(n) {
        FJ(m) {
            cout << dp[k / 2][i + 1][j + 1] * 2 << ' ';
        }
        cout << endl;
    }
    return 0;
