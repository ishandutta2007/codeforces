...
void solve() {
    ...
    for (int bit = 0; (1 << bit) <= n; bit++) {
        int nearl[2] = {int(-1e9), int(-1e9)};
        for (int i = 1; i <= n; i++) {
            int j = i;
            while ((j < n) && (s[j + 1].l == s[j].l)) j++;
            for (int k = i; k <= j; k++) nearl[(s[k].c >> bit) & 1] = max(nearl[(s[k].c >> bit) & 1], s[k].r);
            for (int k = i; k <= j; k++) ans[s[k].id] = min(ans[s[k].id], max(0, s[k].l - nearl[1 - ((s[k].c >> bit) & 1)]));
            i = j;
        }
    }
    for (int bit = 0; (1 << bit) <= n; bit++) {
        int nearl[2] = {int(2e9), int(2e9)};
        for (int i = n; i >= 1; i--) {
            int j = i;
            while ((j > 1) && (s[j - 1].l == s[j].l)) j--;
            for (int k = i; k >= j; k--) nearl[(s[k].c >> bit) & 1] = min(nearl[(s[k].c >> bit) & 1], s[k].l);
            for (int k = i; k >= j; k--) ans[s[k].id] = min(ans[s[k].id], max(0, nearl[1 - ((s[k].c >> bit) & 1)] - s[k].r));
            i = j;
        }
    }
