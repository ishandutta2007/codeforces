...
void dfs(int i, int p) {
    ...
    subs[i] = choose(cnt[i], k);
    for (int nb : adj[i]) if (nb != p) subs[i] -= choose(cnt[nb], k);
    cur += subs[i] * cnt[i];
}
void dfs2(int i, int p) {
    ...
    for (int nb : adj[i]) if (nb != p) {
        cur -= subs[nb]*cnt[nb];
        cur -= (subs[i] - choose(cnt[i], k) + choose(n, k) - choose(n-cnt[i], k)) * n;
        cur += (subs[i] - choose(cnt[i], k) + choose(cnt[nb], k) + choose(n-cnt[nb], k) - choose(n-cnt[i], k)) * (n - cnt[nb]);
        cur += (subs[nb] - choose(cnt[nb], k) + choose(n, k) - choose(n-cnt[nb], k)) * n;
        dfs2(nb, i);
        cur = prev;
    }
}
