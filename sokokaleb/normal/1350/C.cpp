...
int largest_prime_divisor[MAX_N];
int divided[MAX_N];
set<int> all_primes;
...
int nums[MAX_N];
umap<int, vll> cnt_per_prime;
int itung_per_prime[MAX_N];

void CobaMasukin(ll x, vll* vec) {
	if (size(*vec) == 0) {
		vec->pb(x);
	} else if (size(*vec) == 1) {
		if (x < vec->at(0)) {
...
void solve(int tc_num = -1) {
	...
	FOR (i, 0, n) {
		...
		ll num = 1LL;
		while (a > 1) {
			int z = largest_prime_divisor[a];
			all_primes.insert(z);
			if (lst != z && lst != -1) {
				vll& vec = cnt_per_prime[lst];
				CobaMasukin(num, &vec);
				++itung_per_prime[lst];
				num = 1LL;
			}
		...
		if (num > 1LL) {
			CobaMasukin(num, &cnt_per_prime[lst]);
			++itung_per_prime[lst];
		}
	}
	...
	for (auto prime : all_primes) {
		vll& vec = cnt_per_prime[prime];
		if (itung_per_prime[prime] == n) {
			ans *= vec[1];
		} else if (itung_per_prime[prime] == n - 1) {
			ans *= vec[0];
		}
	}
	cout << ans << "\n";
