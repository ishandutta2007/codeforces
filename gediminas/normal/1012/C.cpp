...
ll reikia[5000 + 2][2510 + 1][2] = {};
ll auks[5000 + 2][2510 + 1][2] = {};
...
int main () {
	...
	for (int i = 1; i <= n + 1; i++) {
		reikia[i][0][1] = max(0ll, a[i - 1] - a[i] + 1);
		auks[i][0][0] = auks[i][0][1] = a[i];
	}

	for (int j = 1; j <= k; j++) {
		for (int i = 1; i <= n + 1; i++) {
			reikia[i][j][0] = reikia[i - 1][j - 1][1] + max(0ll, a[i] -  auks[i - 1][j - 1][1] + 1);
			auks[i][j][0] = min(a[i], auks[i - 1][j - 1][1] - 1);

			if (reikia[i - 1][j][0] < reikia[i][j][0]) {
				reikia[i][j][0] = reikia[i - 1][j][0];
				auks[i][j][0] = a[i];
			}

			reikia[i][j][1] = reikia[i - 1][j][0] + max(0ll, auks[i - 1][j][0] - a[i] + 1);
			auks[i][j][1] = a[i];
			//debug(i, j, reikia[i][j][0], auks[i][j][0], reikia[i][j][1], auks[i][j][1]);
		}
	}
