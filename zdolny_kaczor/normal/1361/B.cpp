...
#endif
const int mod = 1e9 + 7, nax = 1e6 + 444;
//#STAY AT HOME
ll powe(ll b, ll e) {
	ll r = 1;
	while (e) {
...
int main() {
	...
	REP(_, t) {
		...
		for (int k : in) {
			...
			if (num[last] == 0) {
				last = nax - 1;
				for (int i = 0; i <= 40; ++i) if (num[k0 + i]) last = k0 + i;
			}
			debug(imie(k), imie(ans), arr(num, last));
		}
		if (num[last] < 0) ans = -ans;
		printf("%lld\n", fix(ans, mod));
		for (int x : did) num[x] = 0;
		did.clear();
	}
	exit(0);
}
