...
int main () {
	...
	for (int i = 0; i < 4; i++) {
		if (minx <= b[i].x and b[i].x <= maxx and miny <=  b[i].y and b[i].y <= maxy) {
			viduj = true;
		}

		point bb = b[i] + b[(i + 1) % 4];

		if (minx <= bb.x and bb.x <= maxx and miny <=  bb.y and bb.y <= maxy) {
			viduj = true;
		}
	}
	...
	for (int i = 0; i < 4; i++) {
		if (between(minx, b[i].x, maxx) and between(miny, b[i].y, maxy)) {
			viduj = true;
		}
