...
ll solve(const vector<ll> &xs, const vector<ll> &vs, const int t){
	const int n=xs.size();
	fill(dp[0],dp[n],INF);
	dp[0][0]=0;
	rep(i,n)rep(j,n)reps(I,i+1,n)if(j+I-i<n) MN(dp[I][j+I-i-1], dp[i][j]+vs[i]*ll(xs[I]-xs[i]));
	// rep(i,n){rep(j,n) ouT(dp[i][j]); cout NL;}
	return *min_element(dp[n-1], dp[n-1]+t+1);
}
