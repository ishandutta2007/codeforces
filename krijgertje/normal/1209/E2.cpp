...
int solve() {
	...
	REP(at,lim) REP(mask,1<<h) {
		best[at][mask]=INT_MIN;
		int y=o[at].second;
		REP(shift,h) {
			int cur=0; REP(x,h) if(mask&(1<<x)) cur+=a[(x+shift)%h][y];
			upd(best[at][mask],cur);
		}
	}
	...
	REP(mask,1<<h) REP(at,lim) if(dp[mask][at]!=INT_MIN) {
		...
		int sub=0;
		while(true) {
			if(sub!=0) upd(dp[mask|sub][at+1],dp[mask][at]+best[at][sub]);
