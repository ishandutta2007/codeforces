...
{
    int x,y,num;
    inline bool operator <(gyq z){return x<z.x;}
}nod[N];
int a[N];
int b[N];
int v[N];
...
inline void add(int x,int y,int *s){for(;x<=ks;x+=lowbit(x))s[x]+=y;return;}
inline int check(int x,int *s){int y=0;for(;x;x-=lowbit(x))y+=s[x];return y;}
inline void change(int x,int y,int s){for(;y<=n;y+=lowbit(y))add(x,s,f[y]);return;}
inline void init(){lens=sqrt(n);ks=n/lens;if(n%lens)ks++;for(int i=1;i<=n;i++)change(bel(i),v[i],1);return;}
inline int find(int x,int y)
{
    int ans=0,now=bel(x)-1;if(!x||!y)return 0;
    for(int i=now*lens+1;i<=x;i++){if(v[i]<=y)ans++;}
    for(;y;y-=lowbit(y))ans+=check(now,f[y]);return ans;
}
inline void work()
{
    int typ,x,y;rin>>typ>>x>>y;
    if(typ==1){int a,b;rin>>a>>b;printf("%d\n",find(y,b)-find(x-1,b)-find(y,a-1)+find(x-1,a-1));}
    else 
    {
        int x_=x,y_=y;x=nu[b[x]];y=nu[b[y]];
        change(bel(x),v[x],-1);change(bel(x),v[y],1);change(bel(y),v[y],-1);change(bel(y),v[x],1);jh(v[x],v[y]);jh(b[x_],b[y_]);
    }
    return;
}
int main()
{
    rin>>n>>m;
    for(int i=1;i<=n;i++)rin>>a[i],nod[a[i]].x=i;for(int i=1;i<=n;i++)rin>>b[i],nod[b[i]].y=i;for(int i=1;i<=n;i++)nod[i].num=i;
    sort(nod+1,nod+n+1);for(int i=1;i<=n;i++)v[i]=nod[i].y,nu[nod[i].num]=i;init();for(;m;m--)work();
    return 0;
}