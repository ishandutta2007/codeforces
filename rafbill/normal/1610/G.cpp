...
string solve00(string const& s){
  ...
  auto cmp = letrec([&](auto cmp, int i, int j){
    if(i == j) return 0;
    if(j == -1) return 1;
    ...
    return cmp(nxt[i],nxt[j]);
  });

  auto pr = letrec([&](auto pr, int i) -> void {
    if(i == -1) return;
    ans += s[i];
    pr(nxt[i]);
  });

  auto solve = letrec([&](auto solve, int i) -> void {
    vi cs;
    for(int j = i+1; j != sw[i]; j = sw[j]+1) {
      solve(j);
      cs.emplace_back(j);
    }
    stable_sort(begin(cs), end(cs), cmp);
    vi ds;
    { int cur = i;
      for(int j : cs) if(j > cur) {
          ds.emplace_back(j);
          cur = j;
        }
    }
  ...
  auto solve2 = [&](int i0, int i1) {
    vi cs;
    for(int j = i0+1; j != i1; j = sw[j]+1) {
      solve(j);
      cs.emplace_back(j);
    }
...
string brute(string const& s){
  set<string> seen;
  auto dfs = letrec([&](auto dfs, string const& s) -> void {
    if(seen.count(s)) return;
    seen.insert(s);
    for(li i = 0; i < (li)((int)s.size()); ++i) if(s[i] == '(') for(li j = (i+1); j <= (li)((int)s.size()-1); ++j) if(s[j] == ')') {
        string t;
        for(li a = 0; a < (li)((int)s.size()); ++a)if(a!=i&&a!=j) t += s[a];
        dfs(t);
      }
  });
  dfs(s);
