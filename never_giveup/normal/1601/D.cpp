...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> a[i].fr >> a[i].sc;
        dod.push_back(a[i].fr);
        dod.push_back(a[i].sc);
    }
    sort(all(dod));
    unique(dod);
    d = low_bo(dod, d);
    for (int i = 0; i < n; i++) {
        a[i].fr = low_bo(dod, a[i].fr);
        a[i].sc = low_bo(dod, a[i].sc);
    }
    int m = dod.size();
    vector<vector<int>> ta(m), ts(m);
    for (int i = 0; i < n; i++) {
        ta[a[i].sc].push_back(a[i].fr);
        ts[a[i].fr].push_back(a[i].sc);
    }
    f.init(m);
    vector<int> dp(m + 1);
    for (int i = 0; i < d; i++)
    ...
    for (int i = d; i < m; i++) {
        ...
        for (int j : ts[i])
            if (j > i)
                setmax(dp[j], dp[i] + f.get(i + 1, j - 1) + 1);
    }
    cout << dp[m - 1];
}
// check test counter
