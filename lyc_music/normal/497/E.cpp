...
void BellaKira()
{
    ...
    for (int now=1;now<r;now++)
    {
        ...
        int noww=m;
        poly pp;
        while (noww) pp.push_back(noww%2),noww/=2;
        reverse(pp.begin(),pp.end());
        int nowt=1;
        for (int i=1;i<pp.size();i++)
        {
            matrix nowmt=f[now]>>(nowt*b[now-1]%m);
            // cout<<"?"<<nowt<<" "<<i<<" "<<pp[i]<<endl;
            if (pp[i]==1)
            {
                matrix nowmt1=f[now-1]>>(nowt*2%m*b[now-1]%m);
                f[now]=f[now]*nowmt;
                f[now]=f[now]*nowmt1;
                nowt*=2;
                nowt++;
        ...
        // for (int i=0;i<f[now].n;i++)
        // {
        //     for (int j=0;j<f[now].m;j++)
        //         cout<<f[now].a[i][j]<<" ";
        //     cout<<endl;
        // }
        // cout<<"?"<<m<<" "<<nowt<<endl;
