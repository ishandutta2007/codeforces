...
void solve() {
	...
	bool valid = true;
	for (int i = 1; i < n; ++i) {
		range = make_pair(max(range.first - k, a[i]), min(range.second + k, a[i] + k));
		debug(arr(a, i), imie(range));
		if (range.second < range.first) valid = false;
	}
	if (range.first <= a.back() && range.second >= a.back() && valid) puts("Yes"); else puts("No");
}
int main() {
