...
#define newe(n) struct Edge{int v,w,nxt,id;}e[2*n+5];\
typedef int arr[n+5];\
arr h;\
int cnt=2;\
inline void addedge(int u,int v,int w,int id){e[cnt]=(Edge){v,w,h[u],id};h[u]=cnt++;}\
struct node{\
	int u,d;\
...
{
	int r=0;
	fe(u)if(c[v]==c[u])r=1;else if(c[v]);else c[v]=c[u]^3,r|=dfs(v),e[i].w=1;gr
	return r;
}
arr a,ans;
void dfs2(int u)
...
signed main()
{
	...
	rg(m)
	int x=read(),y=read();
	addedge(x,y,0,i),addedge(y,x,0,i);gr
	c[1]=1;
	if(dfs(1))
	{
		puts("YES");
		int c0=0,c1=0;
		rg(n)if(c[i]==1)c0+=a[i];else c1+=a[i];gr
		rg_(u,n)fe(u)
		if(c[v]==c[u]&&c0!=c1)
		{
			if(c[v]==2)ans[e[i].id]=c1-c0>>1,c0=c1=0;
			else ans[e[i].id]=c0-c1>>1,c0=c1=0;
			int t=ans[e[i].id];
			a[v]-=t;
			a[u]-=t;
		}
