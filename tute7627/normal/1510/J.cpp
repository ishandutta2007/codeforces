...
int main(){
  ...
  mp.EB('#',0);
  if(mp[0].fi=='#'){
    mp.insert(mp.begin(),MP(' ',0));
  }
  vector<ll>p(mp.size(),-1);
  vector<bool>ng(n+1);
  rep(i,0,mp.size()){
    if(mp[i].fi=='#'){
      p[i]=mp[i-1].se;
      if(i!=1&&i+1!=mp.size())p[i]=max(0LL,p[i]-1);
      if(p[i]>0)ng[p[i]-1]=true;
    }
  }
  vector<ll>cand;
  cand.PB(0);cand.PB(1);
    ...
    rep(i,0,mp.size()){
      ...
      if(rem==1){
        ...
        if(x==0){
          ...
          rep(_,0,(rem+1)/2)ret.PB(1);
        }
        else{
          if(rem%2==0){
            ret.PB(2);
            rep(_,0,rem/2-1)ret.PB(1);
          }
          else{
            rep(_,0,(rem+1)/2)ret.PB(1);
          }
