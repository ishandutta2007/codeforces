...
void solve() {
  ...
  vvc<pi> dat(N);
  FOR(v, N) dat[uf[v]].eb(cost[v], val[v]);

  const ll INF = 1LL << 60;
  vi DP(LIM + 1, -INF);
  DP[0] = 0;
  FOR(v, N) if (len(dat[v])) {
    vi newDP = DP;
    auto upd = [&](ll c, ll v) -> void {
      FOR(x, LIM + 1) if (DP[x] != -INF && x + c <= LIM) {
        chmax(newDP[x + c], DP[x] + v);
      }
    };
    for (auto&& [c, v]: dat[v]) { upd(c, v); }
    ll a = 0, b = 0;
    for (auto&& [c, v]: dat[v]) { a += c, b += v; }
    upd(a, b);
    swap(DP, newDP);
  }
  print(MAX(DP));
