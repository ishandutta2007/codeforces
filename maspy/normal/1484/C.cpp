...
void solve() {
  LL(N, M);
  const ll INF = 1LL << 30;
  MaxFlowGraph<int> G(N + M + 2, INF);
  int source = 0;
  int sink = N + M + 1;
  // 
  auto left = [&](int i) -> int { return 1 + i; };
  auto right = [&](int i) -> int { return 1 + N + i; };
  FOR(i, N) G.add(source, left(i), ceil(M, 2));
  FOR(j, M) G.add(right(j), sink, 1);
  FOR(j, M) {
    LL(k);
    FOR(k) {
      LL(i);
      --i;
      G.add(left(i), right(j), 1);
    }
  }
  ...
  for (auto&& [frm, to, cap]: edges) {
    if (cap) continue;
    if (1 <= frm && frm <= N && N < to) { ANS[to - N - 1] = frm; }
  }
  print(ANS);
}
