...
void test_case(int test) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        int ind = 0;
        for (int curki = 0; curki < dp[i].size(); ++curki) {
            int curk = vals[i][curki];
            int left = a[i] / curk;
            int prevk = (a[i - 1] + left - 1) / left;
            while (vals[i - 1][ind] != prevk)
                ++ind;
            dp[i][curki] = Mint(curk - 1) * (i + 1) + dp[i - 1][ind];
        }
