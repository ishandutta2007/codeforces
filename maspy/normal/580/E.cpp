...
void solve() {
  ...
  vi repunit(N + 1);
  repunit[1] = 1;
  FOR(n, N) repunit[n + 1] = RH.combine(repunit[n], repunit[1], 1);
  ...
  SegmentTree<E, OP> seg(
    [&](E x, E y) -> E {
      return mp(RH.combine(x.fi, y.fi, y.se), x.se + y.se);
    },
    [&](E x, OP y) -> E {
      x.fi = RH.mul(y, repunit[x.se]);
      return x;
    },
    [&](OP x, OP y) -> OP {
      if (y == -1) return x;
  ...
  seg.init(N);
  vc<E> seg_raw(N);
  FOR(i, N) seg_raw[i] = mp(A[i], 1);
  seg.build(seg_raw);

  FOR(_, Q) {
    LL(t, L, R, d);
    --L;
    if (t == 1) { seg.operate_range(L, R, d); }
    elif (t == 2) {
      auto a = seg.fold(L, R - d);
      auto b = seg.fold(L + d, R);
      YES(a == b);
    }
  }
}
