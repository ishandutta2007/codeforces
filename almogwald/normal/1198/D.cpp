...
#define rec(i,j,l,r) (bb[i][j]+bb[i+l][j+r]-bb[i+l][j]-bb[i][j+r])
int main() {
	ios::sync_with_stdio(0);
	cin.tie(0);
	defn
		vector<veci> aa(n, veci(n));
	fori(i, n) {
		defs(l)
		fori(j, n) {
			if (l[j] == '#') {
	...
	vector<veci> bb(n+1, veci(n+1));
	fori(i, n) {
		fori(j, n) {
			bb[i + 1][j + 1] = bb[i][j + 1] + bb[i + 1][j] - bb[i][j] + aa[i][j];
		}
	}
	vector < vector<vector<veci>>> x(n, vector<vector<veci>>(n, vector<veci>(n,veci (n))));
	fori(a, n) {
		fori(b, n) {
			fori(c, n - a) {
				fori(d, n - b) {
					...
					if (a < b) {
						...
						fort(dd, d, d + b + 1) {
							if (rec(c, dd, a + 1, 1)==0) {
								...
								if (dd != d+b) {
									r = x[a][d + b - dd - 1][c][dd+1];
								}
								x[a][b][c][d] = min(x[a][b][c][d], l + r);
							}
						}
					}
					else {
