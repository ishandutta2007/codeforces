...
struct suffix_array {
    ...
    suffix_array(string _str) {
        ...
        for (int l = 0; l < D; l++) {
            for (int i = 0; i < L; i++)
                keys[i] = __make_rep(l, i, l ? 1<<(l-1) : 0);
...
int main() {
    ...
    string s; re(s); const int L = sz(s);
    suffix_array sa(s);
    ...
    struct st {
        ...
        void join(const st& o, bool cost) {
            if (!o.ok) return;
            ok |= o.ok;
            ways += o.ways;
            ckmin(ops, o.ops + cost);
        }
    };

    bool found = false; mn opc;

    vector<st> dp(L + 1); dp[0] = {true, 1, 0};
    FOR (l, 1, L + 1) {
        vector<st> DP(L + 1);
        F0R (i, L - l + 1) if (s[i] == '1') {
            DP[i+l].join(dp[i], true);
            if (i >= l && sa.comp(i, l, i-l, l) >= 0)
                DP[i+l].join(DP[i], true);
        }
        F0R (i, sz(dp)) dp[i].join(DP[i], false);

        if (dp.back().ok && (!found || l < 15)) {
            mn cand = 0;
            FOR (i, L - l, L) cand = 2 * cand + (s[i] == '1');
            cand += dp.back().ops;
            if (!found || int(cand) < int(opc)) opc = cand;
            found = true;
        }
    }
