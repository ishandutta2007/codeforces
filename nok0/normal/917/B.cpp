...
void main_() {
	...
	rep(i, m) {
		...
		CHAR(c);
		int d = c - 'a';
		g.add_edge(u, v, d, 1, 1);
	}
	// first turn? A pos, B pos, bef val
	vector dp(2, vector(n, vector(n, vector(26, -1))));
	auto dfs = [&](auto dfs, bool fi, int a, int b, int bef) -> int {
		if(dp[fi][a][b][bef] != -1) return dp[fi][a][b][bef];
		if(fi) {
			int ret = 0;
			for(auto e : g[a]) {
				if(e.cost < bef) continue;
				ret |= dfs(dfs, !fi, e.to, b, e.cost);
			}
			return dp[fi][a][b][bef] = ret;
		} else {
			int ret = 0;
			for(auto e : g[b]) {
				if(e.cost < bef) continue;
				ret |= !dfs(dfs, !fi, a, e.to, e.cost);
			}
			return dp[fi][a][b][bef] = !ret;
		}
	};
