...
{
    reader >>n;
    for(int i = 1; i <= n; i++) reader >>l[i] >>r[i];
    hscnt = 0;
    for(int i = 1; i <= n; i++)
    {
        hs[hscnt++] = l[i];
    ...
    sort(hs, hs + hscnt);
    hscnt = unique(hs, hs + hscnt) - hs;

    for(int i = 0; i < hscnt; i++) lc[i] = rc[i] = 0;

    for(int i = 1; i <= n; i++)
    {
        l[i] = lower_bound(hs, hs + hscnt, l[i]) - hs;
        r[i] = lower_bound(hs, hs + hscnt, r[i]) - hs;
