...
inline void AC() {
    ...
    for(_ i = 1; i <= k; ++i) {
        ...
        h.push_back(mp(b[i], ts));
    }
    for(_p i : g) {
        if(c.empty() || i.first != c.back().first) c.push_back(i);
        else c.back().second += i.second;
    }
    for(_p i : h) {
        if(d.empty() || i.first != d.back().first) d.push_back(i);
        else d.back().second += i.second;
    }
    if(c == d) yes();
    else no();
