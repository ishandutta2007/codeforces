...
signed main() {
  ...
  using node_data = vector<ll>;
  using edge_data = vector<ll>;
  // 
  edge_data e_unit;
  // 
  auto f_ev = [&](ll v, edge_data& x) -> node_data {
    ...
    ll best = 0;
    FOR3(i, K + 1, x.size()) { chmax(best, x[i]); }
    chmax(x[0], best + A[v]);
    return x;
  };
  // 
  ...
  auto mul_e = [&](edge_data& x, edge_data& y) -> edge_data {
    ...
    vi z(max(x.size(), y.size()), -INF);
    FOR(i, x.size()) {
      FOR(j, y.size()) {
        chmax(z[i], x[i]);
        chmax(z[j], y[j]);
        if (i + j <= K) continue;
        chmax(z[min(i, j)], x[i] + y[j]);
      }
    }
    return z;
  };
