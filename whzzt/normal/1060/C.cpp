...
int n,m,x,a[N],b[N],ca[N],cb[N],r;
int main(){
	int i,j;
	scan(n,m);
	for(i=1;i<=n;i++)scan(a[i]),a[i]+=a[i-1];
	for(i=1;i<=m;i++)scan(b[i]),b[i]+=b[i-1];
	scan(x);
	for(i=1;i<=n;i++)ca[i]=inf;
	for(i=1;i<=m;i++)cb[i]=inf;
	for(i=1;i<=n;i++)for(j=i;j<=n;j++)chkmin(ca[j-i+1],a[j]-a[i-1]);
	for(i=1;i<=m;i++)for(j=i;j<=m;j++)chkmin(cb[j-i+1],b[j]-b[i-1]);
	for(i=1;i<=n;i++)for(j=1;j<=m;j++)if(i*j>r&&(u64)ca[i]*cb[j]<=x)r=i*j;
	print(r),putc('\n');
}