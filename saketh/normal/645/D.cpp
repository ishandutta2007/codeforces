...
int main() {
    ...
    queue<int> topo;
    F0R (i, N) if (!in[i]) topo.push(i);

    vi res;
    while (!topo.empty()) {
        if (sz(topo) > 1) { ps(-1); return 0; }
        int u = topo.front(); topo.pop();
        res.pb(u);
        trav (v, beat[u]) if (!--in[v]) topo.push(v);
    }
    if (sz(res) < N) { ps(-1); return 0; }

    vi next(N, -1);
    F0R (i, N - 1) next[res[i]] = res[i+1];

    int todo = N - 1;
    F0R (i, M) {
        auto [u, v] = battles[i];
        if (next[u] == v) todo--;
        if (!todo) { ps(i+1); return 0; }
    }
