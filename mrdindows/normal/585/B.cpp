...
bool check(int x, int y, int step) {
    ...
    FJ(30) {
        for (auto p : v[j]) {
            ...
            if (p.second - step * 2 == y) return false;
            if (p.second - step * 2 - 1 == y) return false;
            if (p.second - step * 2 - 2 == y) return false;
        }
    }
    return true;
}
bool isEmp(int x, int y) {
    if (x < 0 || x >= 3) return false;
    FJ(30) {
        for (auto p : v[j]) {
            if (p.first != x) continue;
            if (p.second - y * 2 + 2 == y) return false;
        }
    }
    return true;
}
...
//    FREIN("in.txt");
    ...
    while (ts--) {
        ...
        for (int j = 1; j < n; ++j) {
            FI(3) {
                if (isDp(i, j)) {
                    if (isDp(i, j - 1) && isRdp(i, j - 1)) {
                        rdp[i][j] = true;
                        continue;
                    }
                    if (isEmp(i - 1, j) && isRdp(i - 1, j - 1)) {
                        rdp[i][j] = true;
                        continue;
                    }
                    if (isEmp(i + 1, j) && isRdp(i + 1, j - 1)) {
        ...
        bool res = rdp[0][n - 1] || rdp[1][n - 1] || rdp[2][n - 1];
//        
//        FI(3) {
//            FJ(n) {
//                cout << dp[i][j];
