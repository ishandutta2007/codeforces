...
const int N = 20 * 1000 + 57;
vector<int> all_divs[N];

int stp(int a, int b, int c) {
	int ans = INF;
	for (int na = 1; na < 100; ++na) {
		for (int nb = na; nb < 100; ++nb) {
			if (nb % na) continue;
			for (int nc = nb; nc < 100; ++nc) {
				if (nc % nb) continue;
				ans = min(ans, abs(a - na) + abs(b - nb) + abs(c - nc));
			}
		}
	}
	return ans;
...
inline void solve() {
	...
	for (int i = 1; i < N; ++i) {
		for (int j = 1; j * j <= i; ++j) {
			if (i % j == 0) {
				all_divs[i].push_back(j);
				if (j * j != i) all_divs[i].push_back(i / j);
			}
		}
		sort(all(all_divs[i]));
	}
	...
	while (t--) {
		int a = rd() % 50, b = rd() % 50, c = rd() % 50;
		cin >> a >> b >> c;
		int kek = INF;
		int ka = -1, kb = -1, kc = -1;
		for (int nb = 1; nb < N; ++nb) {
			int na, nc;
			auto lb = lower_bound(all(all_divs[nb]), a);
			if (lb == all_divs[nb].end()) {
				na = *(lb - 1);
			} else if (lb == all_divs[nb].begin()) {
				na = *lb;
			} else {
				if (abs(*lb - a) < abs(*(lb - 1) - a)) {
					na = *lb;
				} else {
					na = *(lb - 1);
				}
			...
			int ans = abs(a - na) + abs(b - nb) + abs(c - nc);
			if (kek > ans) {
				kek = ans;
				ka = na;
				kb = nb;
