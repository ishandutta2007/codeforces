...
auto p = sieve_of_atkin(1000);
vector<uint> primes;
int solve(uint a, uint b){
    int at = 0;
    for(; at < primes.size(); at++) if(a % primes[at] == 0){
        int c1 = 0, c2 = 0;
        while(a % primes[at] == 0){
            c1++;
            a /= primes[at];
        ...
        if((c1 + c2) % 3) return Yes(0);
        if(c1 * 2 < c2 || c2 * 2 < c1) return Yes(0);
    }
    return Yes(ull(a) * a == b || ull(b) * b == a);
}
signed main(){
    rep(p.size()) if(p[i]) primes.push_back(i);
    LL(t);
    rep(_,t){
        uint a, b;
        in(a, b);
