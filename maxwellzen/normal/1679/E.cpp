...
vvb possible;
vvi req, cnt;
vvll dp(18, vll(1<<17, 0));

void solve() {
    cin >> n >> s;
    possible.rsz(n, vb(n, false));
    req.rsz(n, vi(n));
    cnt.rsz(n, vi(n));
    F0R(i, n) if (s[i]=='?') m++;
    F0R(j, n) {
        if (j<n-1) possible[j+1][j]=true, req[j+1][j]=0, cnt[j+1][j]=m;
        possible[j][j]=true, req[j][j]=0, cnt[j][j]=m;
        F0R(i, j) {
            if (possible[i+1][j-1] && (s[i]=='?' || s[j]=='?' || s[i]==s[j])) {
                possible[i][j]=true;
                if (s[j]==s[i]) req[i][j] = req[i+1][j-1];
                else req[i][j] = req[i+1][j-1] | 1<<(max(s[i], s[j])-'a');
                if (s[j]=='?' || s[i]=='?') cnt[i][j] = cnt[i+1][j-1] - 1;
                else cnt[i][j] = cnt[i+1][j-1];
            }
        }
    }
    F0R(j, n) F0R(i, j+1) if (possible[i][j]) {
        F0R(k, 18) dp[k][req[i][j]] += mod_exp(k, cnt[i][j]);
    }
    F0R(bit, 17) F0R(i, 1<<17) if (i & 1<<bit) {
        int j = i ^ 1<<bit;
        F0R(k, 18) dp[k][i] += dp[k][j];
    }
    ...
    while (q--) {
        cin >> t;
        int i = 0;
        for (char c : t) i |= 1<<(c-'a');
        cout << dp[t.size()][i]%MOD << nl;
    }
}
