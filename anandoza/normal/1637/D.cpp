...
void solve() {
    ...
    int MAX = accumulate(all(a), 0) + accumulate(all(b), 0);

    vector<int> total_prefix{0};
    for (int i = 0; i < n; i++)
        total_prefix.push_back(total_prefix.back() + a[i] + b[i]);
    vector<vector<ll>> dp(n + 1, vector<ll>(MAX + 1, INF));
    dp[0][0] = 0;

    for (int i = 0; i < n; i++) {
        auto u = a[i], v = b[i];
        for (int sum = 0; sum <= total_prefix[i]; sum++) {
            int other_sum = total_prefix[i] - sum;

            ckmin(dp[i + 1][sum + u], dp[i][sum] + u * sum + v * other_sum);
            ckmin(dp[i + 1][sum + v], dp[i][sum] + v * sum + u * other_sum);
        }
    }
