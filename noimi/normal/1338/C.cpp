...
vecll v{0,1,2,3,4,8,12,5,10,15,6,11,13,7,9,14};
vecll w{0,0,0,1,2,3,2,3,1,3,1,2};
ll dfs(ll x){
    if(x<12)return w[x];
    ll t = 12;
    while(t*4<=x)t*=4;
    if(x<t*2){
        return dfs(x-t)+t/3*(x%3+1);
    }
    else if(x<t*3){
        ll tt[3]={2,3,1};
        return dfs(x-t*2)+t/3*tt[x%3];
    }
    else{
        ll tt[3]={3,1,2};
        return dfs(x-t*3)+t/3*tt[x%3];
    }
}
...
signed main(){
    ...
    //         se.insert(i);
    //         rep2(j,1,192){
    //             if(!se.count(j) and !se.count((i^j))){
    //                 se.insert(j);se.insert(i^j);
    //                 cout<<CNT+1<<" ";
    //                 CNT+=3;
    //                 cout<<i-64<<" "<<j-128<<" "<<(i^j)-192<<endl;
    //                 break;
    //             }
    //         }
    //     }
