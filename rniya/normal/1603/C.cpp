...
void solve() {
    ...
    vector<int> sz(n + 1);
    nxt.emplace_back(INF, INF, 1);
    sz[n] = 1;

    vector<mint> dp_sum(MAX_SZ, 0), ndp_sum(MAX_SZ, 0), dp_cnt(MAX_SZ, 0), ndp_cnt(MAX_SZ, 0);
    dp_cnt[0] = 1;

    mint ans = 0;
    for (int i = n - 1; i >= 0; i--) {
        auto qr = quotient_ranges(a[i]);
        cur.emplace_back(INF, INF, 1);
        for (auto& t : qr) {
            int l = get<0>(t), r = get<1>(t);
            cur.emplace_back(floor(a[i], l), ceil(a[i], l), l);
            if (ceil(a[i], l) != ceil(a[i], r)) cur.emplace_back(floor(a[i], r), ceil(a[i], r), r);
        }
        reverse(ALL(cur));
        sz[i] = cur.size();
        ...
        ndp_sum[sz[i] - 1] = 0;
        ndp_cnt[sz[i] - 1] = 1;
        for (int l = sz[i] - 2, r = sz[i + 1] - 1; l >= 0; l--) {
            mint sum = 0, cnt = 0;
            while (r >= 0 and get<1>(cur[l]) <= get<0>(nxt[r])) {
                sum += dp_sum[r] + dp_cnt[r] * (get<2>(cur[l]) - 1);
                cnt += dp_cnt[r];
                dp_sum[r] = dp_cnt[r] = 0;
                r--;
            }
