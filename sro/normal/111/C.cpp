...
struct Hashtable{
	int head[10000005],nxt[10000005];
	p3 dt[10000005];
	int ans[10000005];
	int cnt;
	const int mod=10000005;
	int& operator[](const p3 &num){
		int x=(num.First+num.Second*100+num.Third*10000)%mod;
...
const int dir[5][2]={0,1,1,0,1,1,1,2,2,1};
int n,m;

int dfs(int x,int y,int msk){
	if(y==m){
		x++;
		y=0;
	}
	...
	for(int i=0;i<3;i++){
		for(int j=0;j<3;j++){
			...
			for(int k=0;k<5;k++){
				...
				if(xx>=0&&x+xx<n&&yy>=0&&y+yy<m){
					nmsk|=MASK(xx*m+yy);
				}
			}
			if(nmsk&1){
				ans=min(ans,dfs(x,y+1,nmsk>>1)+1);
			}
		}
	}
	return ans;
