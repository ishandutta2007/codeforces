...
int main() {
    ...
    rep(i, n) if (int j = p[i]; j >= 0) {
        int ci = ids[i];
        int cj = ids[j];
        if (ci != cj) {
            g[cj].push_back(ci);
        } else {
            if (chain_idx[i] < chain_idx[j]) {
                print(0);
                return 0;
            }
        }
    ...
    vector<int> ans;
    for (int i : top.sorted()) {
        copy(all(chain[roots[i]]), back_inserter(ans));
    }
    for (auto &e : ans) ++e;
