...
{
	// odp(u,k);fflush(stdout);
	if(k==0)return 0;
	if(mp.count({u,k}))return mp[{u,k}];
	int ns=s[u]*k;
	if(g[u].size()==0)return ns;
	int o=k/g[u].size(),N=k%g[u].size();
	std::vector<int>tmp;
	for(auto v:g[u])
	{
		int kk=solve(v,o);
		ns+=kk;
		if(N)tmp.push_back(solve(v,o+1)-kk);
	}
	std::sort(tmp.begin(),tmp.end());
	std::reverse(tmp.begin(),tmp.end());
	for(int i=0;i<N;i++)ns+=tmp[i];
	return mp[{u,k}]=ns;
}
signed main()
{
	...
	int n=read(),k=read();
	rg(n)g[i].clear();gr
	rg(n-1)g[read()].push_back(i+1);gr
	rg(n)s[i]=read();gr
	mp.clear();
	oldl(solve(1,k));
	gr
