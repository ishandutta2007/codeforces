...
int main(void)
{
	...
	cin.tie(0);
	make_fact(5005);
	rep(i, 1, 2000) inv[i] = mint(i).inverse();
	...
	rep(i, 1, 2000) rep(j, 1, 2000) dp[i][j] *= fact[j];
	...
	while(T--){
		...
		ll l = min(n, k);
		mint ans = 0; mint mul = 1;
		
		beki[l] = modpow(m, n-l);
		per(i, l-1, 1) beki[i] = beki[i+1] * m;
		beki2[0] = 1;
		rep(i, 1, l) beki2[i] = beki2[i-1] * mint(ceil(m, 2));
		
		rep(i, 1, l){
			mul *= n+1-i, mul *= inv[i];
			ans += dp[k][i] * beki[i] * mul * beki2[i];
		}
		outl(ans);
	}
