#include <bits/stdc++.h>
using namespace std;



template<class Q>
struct fenwick_tree{
	fenwick_tree(): n(0){}
	fenwick_tree(int n): n(n), tree(n){}
	fenwick_tree(const vector<Q> &v): n((int)v.size()), tree(v){
		for(int i = 1; i <= n; ++ i) if(i + (i & -i) <= n) tree[i + (i & -i) - 1] += tree[i - 1];
	}
	void update(int p, Q x){
		assert(0 <= p && p < n);
		for(++ p; p <= n; p += p & -p) tree[p - 1] += x;
	}
	Q query(int r){
		Q s{};
		for(; r > 0; r -= r & -r) s += tree[r - 1];
		return s;
	}
	Q query(int l, int r){
		assert(0 <= l && l <= r && r <= n);
		return query(r) - query(l);
	}
	template<class F> int max_pref(F f){
		int p = 0;
		Q pref{};
		for(int i = __lg(n + 1); i >= 0; -- i) if(p + (1 << i) <= n && f(pref + tree[p + (1 << i) - 1])){
			pref += tree[p + (1 << i) - 1];
			p += 1 << i;
		}
		return p;
	} // f(sum[0, r)) is T, T, ..., T, F, F, ..., F, returns max r with T
	int n;
	vector<Q> tree;
};

int main(){
	cin.tie(0)->sync_with_stdio(0);
	cin.exceptions(ios::badbit | ios::failbit);
	int qn;
	cin >> qn;
	map<int, vector<array<int, 3>>> query;
	for(auto qi = 0; qi < qn; ++ qi){
		int type, t, x;
		cin >> type >> t >> x;
		query[x].push_back({type, qi, t});
	}
	vector<array<int, 2>> res;
	for(auto [ignore, v]: query){
		vector<int> tcmp;
		for(auto [type, qi, t]: v){
			tcmp.push_back(t);
		}
		sort(tcmp.begin(), tcmp.end()), tcmp.erase(unique(tcmp.begin(), tcmp.end()), tcmp.end());
		auto ind = [&](int t){
			return lower_bound(tcmp.begin(), tcmp.end(), t) - tcmp.begin();
		};
		fenwick_tree<int> ds((int)tcmp.size());
		for(auto [type, qi, t]: v){
			t = ind(t);
			if(type == 1){
				ds.update(t, 1);
			}
			else if(type == 2){
				ds.update(t, -1);
			}
			else{
				res.push_back({qi, ds.query(0, t + 1)});
			}
		}
	}
	sort(res.begin(), res.end());
	for(auto [ignore, x]: res){
		cout << x << "\n";
	}
	return 0;
}

/*

*/

////////////////////////////////////////////////////////////////////////////////////////
//                                                                                    //
//                                   Coded by Aeren                                   //
//                                                                                    //
////////////////////////////////////////////////////////////////////////////////////////