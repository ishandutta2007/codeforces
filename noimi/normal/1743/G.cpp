...
int main() {
    ...
    vi fib{1, 1};
    while(fib.back() < M) fib.eb(fib.end()[-1] + fib.end()[-2]);
    fib.pop_back();
    ...
    mp[0] = -1;
    rep(i, idx + 1) {
        mint now = sum - (mp.count(i) ? mp[i] : 0);
        if(mp.count(i)) mp.erase(i);

        int k = lb(v, i);
        if(i and v[k] == i) { cout << now << endl; }
        sum += now;
        if(i == idx) break;

        // dump(now, sum);

        if(i + 1 < idx and s[i] and !s[i + 1]) {
            int k = 1;
            int tmp = i + 2;
            while(tmp < idx) {
                if(tmp + fib[k] > idx) break;
                bool flag = true;
                rep(j, fib[k]) {
                    if(s[i + j] != s[tmp + j]) {
                        flag = false;
                ...
                // dump(k, tmp, flag, s[i + 0], s[tmp + 0]);
                if(!flag) break;
                tmp += fib[k], k++;
            }
            rep(t, 1, k + 2) { mp[i + fib[t]] += now; }
        } else
            mp[i + 1] += now;
        // dump(mp);
    }
