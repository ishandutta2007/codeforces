...
for(int i=0;i<2*n;i++)
{
    cin>>a[i];vx[a[i]].push_back(i+1);
}
x=1;x2=1;
for(int i=1;i<=n;i++)
{
    if(abs(x-vx[i][0])+abs(x2-vx[i][1])>=abs(x-vx[i][1])+abs(x2-vx[i][0]))ans+=abs(x-vx[i][1])+abs(x2-vx[i][0]);
    else ans+=abs(x-vx[i][0])+abs(x2-vx[i][1]);
    x2=vx[i][1];x=vx[i][0];
}
