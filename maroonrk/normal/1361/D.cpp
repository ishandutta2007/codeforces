...
template<class ld> vc<ld> sub(int n,vc<ld> x){
	sort(all(x));
	int h=n/2;
	vc<ld> res;
...
signed main(){
	...
	for(int i=0;i<n;){
		int j=i;
		while(j<n&&!argless(ps[i],ps[j]))j++;
		vc<V> d;
		while(i<j){
			d.pb(sqrt(V(sq(ps[i].x)+sq(ps[i].y))));
			i++;
		}
		auto z=sub(k,d);
		dmp(z);
		for(auto v:z)slope.pb(v);
	}
	
	assert(si(slope)>=k-1);
	sort(all(slope),greater<V>());
	V ans=0;
	rep(i,k-1)ans+=slope[i];
	if(k-1<si(slope)&&slope[k-1]>0)ans+=slope[k-1];
	cout<<ans<<endl;
}