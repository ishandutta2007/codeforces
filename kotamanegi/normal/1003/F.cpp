...
#define MODS 10000000000000079
#define int ll
void solve(){
    int n;
    cin >> n;
    vector<tuple<int, modint<MAX_MOD>, modint<MOD>,modint<MODS>>> inputs;
    int basics = 0;
    REP(i, n) {
        string s;
        cin >> s;
        tuple<int, modint<MAX_MOD>, modint<MOD>, modint<MODS>> now = make_tuple(s.length(), modint<MAX_MOD>(0), modint<MOD>(0), modint<MODS>(0));
        REP(q, s.length()) {
            get<1>(now) *= modint<MAX_MOD>(26);
            get<2>(now) *= modint<MOD>(26);
            get<3>(now) *= modint<MODS>(26);
            get<1>(now) += modint<MAX_MOD>(s[q] - 'a');
            get<2>(now) += modint<MOD>(s[q] - 'a');
            get<3>(now) += modint<MODS>(s[q] - 'a');
        }
    ...
    REP(i, n) {
        int reqs = -1;
        for (int q = 0; i + q < n; ++q) {
            reqs += get<0>(inputs[i + q]);
            int kazu = 1;
            for (int j = i + q + 1; j + q < n; ++j) {
                int die = 0;
                for (int t = 0; t < q+1; ++t) {
                    if (get<0>(inputs[i + t]) != get<0>(inputs[j + t])) {
                        die = 1;
                        break;
                    }
                    if (get<1>(inputs[i + t]).value != get<1>(inputs[j + t]).value) {
                        die = 1;
                        break;
                    }
                    if (get<2>(inputs[i + t]).value != get<2>(inputs[j + t]).value) {
                        die = 1;
                        break;
                    }
                    if (get<3>(inputs[i + t]).value != get<3>(inputs[j + t]).value) {
                        die = 1;
                        break;
                    }
                }
