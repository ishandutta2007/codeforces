...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            if (a[i][j] < a[j][i]) {
                if (uf.can_constrain_to_be_same(i, j)) {
                    uf.constrain_to_be_same(i, j);
                }
            } else if (a[i][j] > a[j][i]) {
                if (uf.can_constrain_to_be_different(i, j)) {
                    uf.constrain_to_be_different(i, j);
                }
            }
        }
    }

    vector<bool> should_swap(n);
    for (int k = 0; k < n; k++) {
        if (uf.can_constrain_node_to_side(k, 0))
            uf.constrain_node_to_side(k, 0);
        if (uf.can_constrain_node_to_side(k, 1))
            uf.constrain_node_to_side(k, 1);
        should_swap[k] = uf.can_constrain_node_to_side(k, 1);
    }
