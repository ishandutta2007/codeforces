...
int main() {
    ...
    int N, P, MS, M; re(N, P, MS, M);

    vpii get_or(N); re(get_or);
    vpii sreq(P); re(sreq);
    vpii not_both(M); re(not_both);

    vi pow;
    trav (p, sreq) pow.pb(p.f), pow.pb(p.s);
    dedup(pow);

    vi pvars(sz(pow));
    F0R (i, sz(pvars)) {
        pvars[i] = ts.new_var();
        if (i) ts.implies(pvars[i-1], pvars[i]);
    }

    vi svars(P);
    F0R (i, P) {
        svars[i] = ts.new_var();
        int pi = lb(all(pow), sreq[i].f) - pow.begin();
        if (pi) ts.implies(svars[i], pvars[pi-1]^1);
        int pj = lb(all(pow), sreq[i].s) - pow.begin();
        ts.implies(svars[i], pvars[pj]);
    }

    trav (p, get_or) ts.at_least_one(svars[p.f-1], svars[p.s-1]);
    trav (p, not_both) ts.at_most_one(svars[p.f-1], svars[p.s-1]);

    vb res = ts.solve();
    if (res.empty()) ps(-1);
    else {
        int pr = -1;
        F0R (i, sz(pow)) if (res[pvars[i]]) {
            pr = pow[i];
            break;
        }

        vi use;
        F0R (i, P) if (res[svars[i]]) use.pb(i+1);
        ps(sz(use), pr);
        pc(use);
    }
