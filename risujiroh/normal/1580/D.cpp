...
int main() {
  auto [nn, m] = input<int, int>();

  Fix solve = [&](auto self, auto&& a, int n) -> vector<i64> {
    int mi = -1;
    for (int i : rep(n)) {
      if (mi == -1 || a[i] < a[mi]) {
        mi = i;
    ...
    auto fl = self(a, mi);
    auto fr = self(a + (mi + 1), n - mi - 1);

    vector res(n + 1, -inf<i64>);
    for (int cl : rep(sz(fl))) {
      for (int cr : rep(sz(fr))) {
        chmax(res[cl + 1 + cr], fl[cl] + a[mi] * (m - 2 * (cl + 1) * (cr + 1) + 1) + fr[cr]);
        chmax(res[cl + cr], fl[cl] - 2 * cl * cr * a[mi] + fr[cr]);
      }
    }
    return res;
  };

  auto a = input<vector<i64>>(nn);
  print(solve(begin(a), nn)[m]);
}