...
struct Qr{
	short x1,x2,y1,y2;
	int id;
	Qr(int _x1=0,int _x2=0,int _y1=0,int _y2=0,int _id=0){
...
struct event{
	short x,y,sgn;
	short id;
	event(int _x,int _y,int _sgn,int _id){
		x=_x,y=_y,sgn=_sgn,id=_id;
	}
	event(){
		;
	}
...
int main(){
	...
	for (int i=1;i<=k;i++){
		//cout<<i<<endl;
		for (int j=0;j<Maxn;j++) bit[j]=0;
		for (int j=0;j<Qs[i].size();j++){
			Qr tmp=Qs[i][j];
			Es[i].pb(event(tmp.x2,tmp.y2,1,tmp.id));
			Es[i].pb(event(tmp.x1-1,tmp.y2,-1,tmp.id));
			Es[i].pb(event(tmp.x2,tmp.y1-1,-1,tmp.id));
			Es[i].pb(event(tmp.x1-1,tmp.y1-1,1,tmp.id));
		}
		...
		for (int row=1;row<=n;row++){
			for (int l=0;l<V[i][row].size();l++){
				int pos=V[i][row][l];
				Add(pos,w[row][pos]);
			}
			for (;j<Es[i].size();j++){
				if (Es[i][j].x!=row) break;
				LL tt=query(Es[i][j].y)*Es[i][j].sgn;
				ans[Es[i][j].id]+=tt;
			}
		}
	}
