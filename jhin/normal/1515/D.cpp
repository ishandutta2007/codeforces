#include <bits/stdc++.h>
    ...
    if(x>1)p.push_back(x),cnt++,vv.push_back(1);
       ...
       {
          int l,r;ans=0;x=0;
          cin>>n>>l>>r;multiset<int>lfts,rts;
          for(int i=0;i<n;i++)
          {
              cin>>a[i];
          }
          ...
          {
              int clr=*it;
              auto itr=rts.lower_bound(clr);
              if(itr==rts.end())break;
          ...
          int si1=lfts.size(),si2=rts.size();
          if(si1>=si2)
          {
            ans+=si2;
            auto it=lfts.begin();
            for(;it!=lfts.end();)
            {
                auto it2=it;
                it2++;
                if(it2==lfts.end())break;
                int clr1=*it,clr2=*it2;
                if(clr1==clr2)
                {x+=2;
                it=it2;
