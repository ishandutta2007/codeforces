...
int main() {
    ...
    vmn hash(N + 1);
    F0R (i, N) hash[i + 1] = 10 * hash[i] + int(s[i] - '0');

    vmn p10(N + 1);
    p10[0] = 1;
    F0R (i, N) p10[i + 1] = 10 * p10[i];

    vmn ip10(N + 1);
    ip10[0] = 1;
    F0R (i, N) ip10[i + 1] = mn::small_inv(10) * ip10[i];

    auto get = [&](int i, int len) {
        return hash[i + len] - p10[len] * hash[i];
    };

    auto check = [&](int alen, int blen, int clen) {
        if (alen > clen || blen > clen) return;
        if ((s[0] == '0' && alen > 1) || (s[alen] == '0' && blen > 1) || (s[alen + blen] == '0' && clen > 1)) return;
        if (get(0, alen) + get(alen, blen) != get(alen + blen, clen)) return;

        string a = s.substr(0, alen);
        string b = s.substr(alen, blen);
        string c = s.substr(alen + blen, clen);

        int carry = 0;
        F0R (i, clen) {
            int x = i < sz(a) ? a[sz(a) - 1 - i] - '0' : 0;
            int y = i < sz(b) ? b[sz(b) - 1 - i] - '0' : 0;
            int z = c[sz(c) - 1 - i] - '0';

            if ((x + y + carry) % 10 != z) return;
            carry = (x + y + carry) / 10;
        }
    ...
    FOR (clen, 1, N + 1) {
        FOR (len, max(1, clen - 1), clen + 1) {
            int other = N - clen - len;
            if (other <= 0) continue;
            check(len, other, clen);
            check(other, len, clen);
        }
    }
