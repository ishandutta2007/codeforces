...
bool porSegundaComponente(const pi& a, const pi& b){
...
void d(){
    ...
    vpi crecientes, decrecientes;
    F0(i,n){
        if(pares[i].first < pares[i].second) crecientes.eb(pares[i]);
        else decrecientes.eb(pares[i]);
    }
    if(crecientes.size() >= decrecientes.size()){
        sort(crecientes.begin(),crecientes.end(),porSegundaComponente);
        cout<<crecientes.size()<<'\n';
        for(int i = crecientes.size()-1; i >= 0; i--){
            cout<<indice[crecientes[i]]+1<<" ";
        }
        cout<<'\n';
        return;
    }
    sort(decrecientes.begin(),decrecientes.end(),porSegundaComponente);
    cout<<decrecientes.size()<<'\n';
    F0(i,decrecientes.size()){
        cout<<indice[decrecientes[i]]+1<<" ";
    }
    cout<<'\n';
    return;
}
