...
ll n;
ll x[100005], y[100005], c[100005];
vector<P> xvec[3], yvec[3];

bool check(ll a, ll b, ll c, ll k)
{
	ll lb = yvec[a][k-1].first, lb2;
	
	ll cnt = 0;
	for(auto p : yvec[b]){
		if(p.first <= lb) continue;
		cnt++;
		if(cnt == k){
			lb2 = p.first;
			break;
	...
	cnt = 0;
	for(auto p : yvec[c]){
		if(p.first <= lb2) continue;
		cnt++;
	}
	return cnt >= k;
}

bool check2(ll a, ll b, ll c, ll k)
{
	ll xlb, ylb = yvec[a][k-1].first;
	
	ll cnt = 0;
	for(auto p : xvec[b]){
		if(p.second <= ylb) continue;
		cnt++;
		if(cnt == k){
			xlb = p.first;
			break;
	...
	cnt = 0;
	for(auto p : xvec[c]){
		if(p.first <= xlb || p.second <= ylb) continue;
		cnt++;
	}
	return cnt >= k;
}
...
bool checks(ll k)
{
	...
	ll p[3] = {0, 1, 2};
	do{
		if(check(p[0], p[1], p[2], k) || check2(p[0], p[1], p[2], k)) return true;
...
ll calc()
{
	rep(i, 0, 2) xvec[i].clear(), yvec[i].clear();
	rep(i, 1, n){
		xvec[c[i]].push_back(P(x[i], y[i]));
		yvec[c[i]].push_back(P(y[i], x[i]));
	}
	rep(i, 0, 2) sort(all(xvec[i])), sort(all(yvec[i]));
	
	ll ub = n/3+5, lb = 0, mid;
	while(ub-lb>1){
		mid = (ub+lb)/2;
		if(checks(mid)) lb = mid;
		else ub = mid;
	}
	return lb;
}
