...
ll calc(string s,ll x,ll y,ll k){
    ll n=s.size();
    ll c[n];
    rep(i,0,n-1){
        if(s[i]=='0')c[i]=1;
...
bool anspro(){
    ll n,p,k;cin>>n>>p>>k;
    string S;cin>>S;
    string s=S.substr(p-1,n-(p-1));
    n=s.size();
    ll x,y;cin>>x>>y;
    ll ans[k];
    rep(i,0,k-1)ans[i]=y*i;
    rep(i,0,k-1){
        string t;
        for(ll j=i;j<=n-1;j+=k)t+=s[j];
        ans[i]+=calc(t,x,y,k);
    }
    //rep(i,0,k-1)cout<<ans[i]<<" ";cout<<endl;
    ll ch=inf;
    rep(i,0,k-1)chmin(ch,ans[i]);
    cout<<ch<<endl;
