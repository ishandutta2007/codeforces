...
fun main(){
    solve.cases{
        val n = getint
        val halfn = n divceil 2
        val L = getline(halfn).copyOf(n)
        val x = getint
        for(i in halfn until L.size) {
            L[i] = x
        }
        val sum = L.sumOf { it.toLong() }
        if(sum > 0) {
            put(n)
            return@cases
        }
        ...
        if(x >= 0) {
            ...
            val largestPossiblek = IntArray(1 + halfn){left ->
                val new = BinarySearchLastTrue(halfn,n){
                    S.sumQuery(left,it - 1 ) > 0
                } ?: -1
                new - left
            }
            val mq = rmq(largestPossiblek)
            for(k in (1 + n /2) ..n){
                if(mq.query(0,n-k) >= k){
                    put(k)
                    return@cases
