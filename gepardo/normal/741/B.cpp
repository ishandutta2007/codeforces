/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 383 (Div. 1)         |
|       Task: B                                     |
|       Date: 06. 12. 2016                          |
\--------------------------------------------------*/
// Includes
...
main()
{
    ...
    gr.resize(comps.size());
    for (int i = 0; i < comps.size(); i++)
    {
        for (int j = 0; j < comps[i].size(); j++)
            gr[i].emplace_back(w[comps[i][j]], b[comps[i][j]]);
        gr[i].emplace_back(wcomp[i], bcomp[i]);
    }
    ...
    for (int i = 1; i <= W; i++)
        for (int j = 0; j < gr.size(); j++)
        {
            ...
            for (int k = 0; k < gr[j].size(); k++)
                if (i >= gr[j][k].first)
                    dp[i][j] = max(dp[i][j], dp[i - gr[j][k].first][j-1] + gr[j][k].second);
        }
