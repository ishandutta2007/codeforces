...
struct AhoCorasick{
	...
	void addString(vector<ll> &s, int x = 1)
	{
		int p = seek(s);
		trie[p].num += x;
	}
...
vector<ll> l, r;
AhoCorasick aho;
mint dp[205][205][505][2];

mint calc(vector<ll> vec)
{
	bool flag = false;
	reps(i, vec) if(vec[i]) flag = true;
	if(!flag) return 0;
	
	ll S = sz(aho.trie);
	
	per(i, 200, 0) rep(j, 0, S-1) rep(k, 0, K) rep(l, 0, 1) dp[i][j][k][l] = 0;
	
	ll l = 0;
	per(i, 199, 0){
		rep(d, 1, m-1){
			ll nl = l;
			if(l == 0 && d > vec[i]) continue;
			if(d < vec[i]) nl = 1;
			ll nj = aho.trans(0, d), nk = aho.trie[nj].sum;
			if(nk <= K) dp[i][nj][nk][nl] += 1;
		}
		if(vec[i]) l = 1;
	}
		...
		rep(j, 0, S-1){
			rep(k, 0, K){
				rep(l, 0, 1){
					...
					rep(d, 0, m-1){
						ll nl = l;
						if(l == 0 && d > vec[i-1]) continue;
						if(d < vec[i-1]) nl = 1;
						ll nj = aho.trans(j, d), nk = k + aho.trie[nj].sum;
						if(nk <= K) dp[i-1][nj][nk][nl] += dp[i][j][k][l];
					}
				}
			}
		}
	...
	mint ret = 0;
	rep(j, 0, S-1) rep(k, 0, K) rep(l, 0, 1) ret += dp[0][j][k][l];
	return ret;
}
...
int main(void)
{
	...
	ll len;
	cin >> len;
	rep(i, 1, len) l.pb(0), cin >> l.back();
	reverse(all(l));
	while(sz(l) < 200) l.pb(0);
	
	cin >> len;
	rep(i, 1, len) r.pb(0), cin >> r.back();
	reverse(all(r));
	while(sz(r) < 200) r.pb(0);
	
	ll x;
	rep(i, 1, n){
		vector<ll> vec;
		cin >> len;
		rep(j, 1, len) vec.pb(0), cin >> vec.back();
		cin >> x;
		aho.addString(vec, x);
	}
	aho.makeSuffixLink();
