...
int main() {
    ...
    while (t --> 0) {
        ...
        int idx = 0;
        rep(i, n) if (a[i] == m) idx = i;
        vec<tuple<int, int, int, int>> ans;
        replr(j, idx + 1, n) {
            ans.emplace_back(idx, j, m, m + j - idx);
        }
        rrep(j, idx) {
            ans.emplace_back(idx, j, m, m + idx - j);
        }
        print(ans.size());
        for (auto &[i, j, x, y] : ans) print(i + 1, j + 1, x, y);
    }
    return 0;
}