...
void solve(){
	...
	for(int i = 0; i < m && (idx.size() + sing.size()) >= 1; ++i){
		if (values[i] == 0){
			continue;
		}
	...
	// for(int i = 0; i < m && idx.size() > 0; ++i){
	// 	if (values[i] == 0)
	// 		continue;

	// 	// trace(i, values, idx, m, ans);
	// 	assert(values[i] > 0);
	...
	// 	--values[ind];
	// 	if (values[ind] > 0){
	// 		idx.insert({values[ind], -ind});
	// 		idx.erase({values[i], -i});
	// 		values[i] = 0;
	// 	}
	// 	else{
	// 		assert(ind == i);
