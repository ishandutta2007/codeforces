...
S e(){
    return {0,0,1000*1000+6};
}

S op(S l,S r){
    if (l.mn==r.mn) return {l.act+r.act,l.inact+r.inact,l.mn};
    if (l.mn<r.mn) return{l.act,l.inact,l.mn};
    else return {r.act,r.inact,r.mn};
}
...
lazy_segtree<S,op,e,F,mapping,composition,id> seg(1e6+10);
int n,q;
int a[maxn];
signed main(){
    ...
    cin>>n>>q;
    a[0]=1e6+5;
    rep1(i,1e6) seg.set(i,{0,1,0});
    rep1(i,n){
        cin>>a[i];
        if (a[i]>a[i-1]) seg.apply(a[i-1]+1,a[i]+1,1);
        else seg.apply(a[i]+1,a[i-1]+1,1);
        auto f=seg.get(a[i]);
        swap(f.act,f.inact);
        seg.set(a[i],f);
    }
    ...
    while (q--){
        ...
        cin>>pos>>x;
        int prev=a[pos];
        auto ret=seg.get(a[pos]);
        swap(ret.act,ret.inact);
        seg.set(a[pos],ret);
        ret=seg.get(x);
        swap(ret.act,ret.inact);
        seg.set(x,ret);
...
//            cout<<l<<" "<<r<<" "<<w<<endl;
        ...
        update(a[pos-1],prev,-1);
        update(a[pos-1],x,1);
        update(a[pos+1],prev,-1);
        update(a[pos+1],x,1);
        ret=seg.prod(1,1e6+1);
        if (ret.mn<=1){
            assert(ret.mn==1);
            cout<<ret.act<<" ";
        } 
