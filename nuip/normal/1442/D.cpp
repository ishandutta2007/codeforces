...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	for(auto &ar:ars){
		...
		ar.resize(m);
		for(auto &v:ar) cin>>v;
		reps(i,1,m) ar[i]+=ar[i-1];
	}
	ll re=0;
	cauto dfs=
		[&](auto &&dfs,int l,int r,const vector<ll> &dp)->void{
			if(r-l==1){
				out(l,dp,ars[l],1);
				MX(re,dp[t]);
				rep(i,min(t,(int)ars[l].size())) MX(re,dp[t-1-i]+ars[l][i]);
				return;
			}
			auto L=dp;
			auto R=dp;
			cauto m=(l+r)/2;
			reps(i,l,m)rrep(j,t+1-ars[i].size()) MX(L[j+ars[i].size()],L[j]+ars[i].back());
			reps(i,m,r)rrep(j,t+1-ars[i].size()) MX(R[j+ars[i].size()],R[j]+ars[i].back());
			dfs(dfs,l,m,R);
			dfs(dfs,m,r,L);
		};
	dfs(dfs,0,n,vector<ll>(t+1));
	cout<<re NL;
  return 0;
}