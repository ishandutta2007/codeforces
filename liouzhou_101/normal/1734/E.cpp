...
void solve()
{
	...
	for (int i = 0; i < n; ++i) read(a[i][i]);
	for (int i = 0; i < n; ++i)
	{
		for (int j = 0; j < n; ++j)
		{
			if (i == j) continue;
			a[i][j] = ((a[i][i] + (j - i) * i) % n + n) % n;
		}
		print(a[i]);
	}
	/*
	...
	while (L1 <= R1)
	{
		vector<tuple<ll, ll, ll>> v; // depth, len, flag
		function<void(ll, ll, ll, ll, ll, ll)> go = [&](ll l, ll r, ll x, ll y, ll d, ll f)
		{
			if (l == x && r == y)
			{
				v.push_back({ d, y - x, f });
		...
		go(0, 1LL << 61, L1, R1, 61, 0);
		auto w = v;
		v.clear();
		go(0, 1LL << 61, L2, R2, 61, 0);

		auto [dv, lv, fv] = v.back();
		auto [dw, lw, fw] = w.back();
		ll len = min(lv, lw);
		if (fv != fw) res += len;
		L1 += len;
		L2 += len;
	...
	while (!v.empty())
	{
		...
		{
			w.pop_back();
			w.push_back({ dw - 1, lw / 2, fw ^ 1 });
			w.push_back({ dw - 1, lw / 2, fw });
			continue;
		}
		if (fv != fw) res += lv;
		v.pop_back();
