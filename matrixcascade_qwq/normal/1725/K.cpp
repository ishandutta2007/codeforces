...
int MX=0;
mii to,vs;
int val[2020202],fa[2020002];
int get(int x)
{
	if(vs.find(x)!=vs.end())return to[x];
	to[x]=MX;MX++;
	fa[to[x]]=to[x];
	se.insert(x);
	val[to[x]]=x;
...
{
	int ww=get(val);
	itt itl=se.lower_bound(l);
	while(1)
	{
		if(*itl>r)break;if(itl==se.end())break;
		merge(*itl,val);
		se.erase(itl++);
	}
}
...
{
	int mid=(l+r)>>1;
	assign(l,mid,l-1);assign(mid+1,r,r+1);
}
signed main()
{
	int T=1;
