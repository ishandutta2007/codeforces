...
void add(int x,int y,int z){nxt[x].push_back(make_pair(y,z));return;}

int dis[N];
void finddis(int now,int fa){for(auto i:nxt[now])if(i.first!=fa)dis[i.first]=dis[now]+i.second,finddis(i.first,now);return;}
void findlist(int st,int &ed){dis[st]=0;finddis(st,0);for(int i=1;i<=n;i++)if(dis[i]>dis[ed])ed=i;return;}
...
struct gyqtree
{
    ...
    void dfs(int now,int fa)
    {
        ...
        {
            f[i.first][0]=now;g[i.first][0]=i.second;dfs(i.first,now);
            if(dep[i.first]+i.second>dep[now])dep[now]=dep[i.first]+i.second,son[now]=i.first;
        }
        return;
    }
    void init()
    {
        tail=0;
        dfs(root,0);
        for(int i=1;i<=n;i++)if(son[f[i][0]]!=i)d[++tail]=make_pair(-dep[i]-g[i][0],i);sort(d+1,d+tail+1);
        for(int i=1,now;i<=tail;i++){for(now=d[i].second;son[now];now=son[now])bel[now]=i;st[i]=d[i].second;ed[i]=now;bel[now]=i;}
        for(int i=1;i<=tail;i++)sum[i]=sum[i-1]+dep[st[i]]+g[st[i]][0];
        for(int i=1;i<20;i++)for(int j=1;j<=n;j++)f[j][i]=f[f[j][i-1]][i-1],g[j][i]=g[j][i-1]+g[f[j][i-1]][i-1];
        return;
    }
    int check(int x,int y)
    {
        y=min((y<<1)-1,tail);
        if(bel[x]<=y)return sum[y];
        int ans=sum[y-1],ath=sum[y],now;
        now=ed[bel[x]];for(int i=19;i>=0;i--)if(bel[f[now][i]]>=y)ans+=g[now][i],now=f[now][i];ans+=g[now][0];
        now=ed[bel[x]];for(int i=19;i>=0;i--)if(bel[f[now][i]]>y)ath+=g[now][i],now=f[now][i];ath+=g[now][0]-dep[f[now][0]];
        return max(ans,ath);;
    }
}A,B;
...
{
    rin>>n>>q;
    for(int i=1,x,y,z;i<n;i++)rin>>x>>y>>z,add(x,y,z),add(y,x,z);
    return;
}
void Init()
{
    findlist(1,A.root);findlist(A.root,B.root);
    A.init();B.init();
    return;
}
void Work()
{
    ...
    {
        int x,y;rin>>x>>y;
        x=(x-1+lasten)%n+1;y=(y-1+lasten)%n+1;
        // printf("%d %d\n",x,y);
        // printf("%d %d\n",A.check(x,y),B.check(x,y));
        printf("%d\n",lasten=max(A.check(x,y),B.check(x,y)));
    }
    return;
}
int main()
