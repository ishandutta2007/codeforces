...
ll min_s(const int& n, const int& r)
{
	if (n <= 1)
		return n;
	if (r == 1)
	...
	if (r != k)
		ans2 = min_s(s + 1, r);
	return ans1 * k + ans2 * (r - k) + n;
}

vi construct_seq(const int& n, const ll& s, const int& r)
{
	vi ans;
	if (r == 1)
	{
	...
	else
	{
		...
		while (used < n)
		{
			if (ans[sz(ans) - 2] * r == ans[sz(ans) - 1])
				ans.pb(0);
			++used;
			++ans.back();
			cs += sz(ans);
			int left = n - used;
			if (((sz(ans) + 1) + (sz(ans) + left)) * ((ll)(left)) / 2 + cs < s)
			{
				--used;
				--ans.back();
				cs -= sz(ans);
...
int main()
{
	...
	{
		cout << "Yes\n";
		auto vv = construct_seq(n, s, y);
		vi p(n);
		int k = sz(vv);
		vi pref(k + 1);
		for (int i = 0; i < k; ++i)
			pref[i + 1] = pref[i] + vv[i];
		for (int i = 1; i < k; ++i)
		{
			for (int j = 0; j < vv[i]; ++j)
				p[j + pref[i]] = pref[i - 1] + j / y;
		}
		for (int i = 1; i < n; ++i)
			cout << p[i] + 1 << ' ';
		cout << '\n';
