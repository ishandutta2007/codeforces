...
void solve() {
    ...
    while(nexts.empty() == false) {
        int taking = 0;
        if (nexts.front() == nexts.back()) {
            if (nexts.back() > now) {
                gogos = ans;
                deque<int> ge = nexts;
                int go = now;
                while (ge.empty() == false&&ge.back() > go) {
                    gogos.push_back('R');
                    go = ge.back();
                    ge.pop_back();
                }
        ...
        else if (nexts.front() < nexts.back()) {
            if (nexts.front() <= now) {
                taking = 1;
            }
        }
        ...
        if (taking == 0) {
            if (nexts.front() <= now) break;
            now = nexts.front();
            nexts.pop_front();
            ans.push_back('L');
        }
        else {
            if (nexts.back() <= now) break;
            now = nexts.back();
            nexts.pop_back();
            ans.push_back('R');
        }
    }
    if (ans.length() < gogos.length()) {
        swap(ans, gogos);
    }
