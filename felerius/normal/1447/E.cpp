// Xor Tree (https://codeforces.com/contest/1447/problem/E)
...
int solve(vector<int>& a, int l, int r, int i) {
    if (r - l <= 1 || i == -1)
        return 0;
    auto mid = (int)(partition(begin(a) + l, begin(a) + r, [&](int a) { return (a >> i) & 1; }) - begin(a));
    int sl = mid - l, sr = r - mid;
    return min(
        max(0, sr - 1) + solve(a, l, mid, i - 1),
        max(0, sl - 1) + solve(a, mid, r, i - 1)
    );
}
