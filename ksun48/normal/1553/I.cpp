...
vector<vector<num> > solve(vector<int> arr){
	int n = (int)arr.size();
	if(n == 1){
		return {{}, {}, {}, {}};
	} else {
		...
		vector<vector<num> > lv = solve(l);
		vector<vector<num> > rv = solve(r);
		vector<vector<num> > ret(4);
		for(int lm = 0; lm < 4; lm++){
			for(int rm = 0; rm < 4; rm++){
				vector<num> prod = ecnerwala::fft::fft_multiply(lv[lm], rv[rm]);
				int out = (lm & 2) ^ (rm & 1);
				if(ret[out].size() < prod.size()) ret[out].resize(prod.size(), 0);
				for(int join = 0; join < 2; join++){
					for(int ival = join; ival < (int)prod.size(); ival++){
						ret[out][ival-join] += prod[ival] * ((join && !(lm & 1)) ? 2 : 1) * ((join && !(rm & 2)) ? 2 : 1);
					}
				}
			}
		}
		...
		num lw = 1;
		num rw = 1;
		for(int x : l) if(x == 1) lw *= 2;
		for(int x : r) if(x == 1) rw *= 2;
		if(lsum == 1) lw = 1;
		if(rsum == 1) rw = 1;
		for(int rm = 0; rm < 4; rm++){
			for(int join = 0; join < 2; join++){
				int out = (join << 1) | ((lsum > 1) << 1) | (rm & 1);
				if(ret[out].size() < rv[rm].size()+1) ret[out].resize(rv[rm].size()+1, 0);
				for(int ival = 0; ival < (int)rv[rm].size(); ival++){
					ret[out][ival+1-join] += rv[rm][ival] * ((join && lsum == 1) ? 2 : 1) * ((join && !(rm & 2)) ? 2 : 1) * lw;
				}
			}
		}
		for(int lm = 0; lm < 4; lm++){
			for(int join = 0; join < 2; join++){
				int out = (join << 0) | ((rsum > 1) << 0) | (lm & 2);
				if(ret[out].size() < lv[lm].size()+1) ret[out].resize(lv[lm].size()+1, 0);
				for(int ival = 0; ival < (int)lv[lm].size(); ival++){
					ret[out][ival+1-join] += lv[lm][ival] * ((join && rsum == 1) ? 2 : 1) * ((join && !(lm & 1)) ? 2 : 1) * rw;
				}
			}
		}
		int out = (lsum > 1 ? 2 : 0) ^ (rsum > 1 ? 1 : 0);
		if(ret[out].size() < 3) ret[out].resize(3, 0);
		ret[out][2] += lw * rw;
		return ret;
	}
}
...
int main(){
	...
	vector<vector<num> > res = solve(z);
	num ans = 0;
	int o1 = 0;
	for(int x : z) if(x == 1) o1++;
	for(int m = 0; m < 4; m++){
		for(int ival = 1; ival < (int)res[m].size(); ival++){
			ans += fact[ival] * pow(num(2), ival) / pow(num(2), o1) * res[m][ival] * (((z.size() - ival) & 1) ? -1 : 1);
		}
	}
	ans += ((z.size() & 1) ? 1 : -1) * (1 + (n > 1));
	cout << ans << '\n';
}