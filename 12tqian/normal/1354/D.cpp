...
int kth(int k, int lo = 0, int hi =-1, int node = 0, int cur = 0){
    if(hi == -1) hi = n-1;
    int mid = (lo+hi)/2;
    if(lo == hi) return lo;
    if(cur + st[2*node+1] >= k){
        return kth(k, lo, mid, 2*node+1, cur);
    }
    else{
        return kth(k, mid+1, hi, 2*node+2, cur + st[2*node+1]);
    }
}
int main(){
    setIO("");
