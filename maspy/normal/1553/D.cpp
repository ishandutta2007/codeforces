...
void solve() {
  ...
  DP0[0] = -2;
  DP1[0] = -1;
  auto find_nxt = [&](vi& I, ll i) -> ll {
    // i  I 
    ll cnt = lower_bound(all(I), i) - I.begin();
    if (cnt == I.size()) return INF;
    return I[cnt];
  };

  FOR(i, NB) {
    ll b = B[i];
    DP1[i + 1] = find_nxt(ID1[b], DP0[i]);
    DP0[i + 1] = find_nxt(ID0[b], DP1[i]);
  }
  bool ok = false;
  if (A.size() % 2 == 0 && DP1.back() != INF) ok = true;
  if (A.size() % 2 == 1 && DP0.back() != INF) ok = true;
  if (ok)
    YES();
  else
    NO();
