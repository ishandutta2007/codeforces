...
signed main() {
  ...
  vi vals;
  FOR(L, N) {
    FOR3(R, L + 1, N + 1) { vals.eb(A[R] - A[L]); }
  }
  sort(all(vals));
  auto n = unique(all(vals)) - vals.begin();
  vals.resize(n);
  ...
  FOR(L, N) {
    FOR3(R, L + 1, N + 1) {
      auto x = A[R] - A[L];
      auto i = lower_bound(all(vals), x) - vals.begin();
      data[i].eb(mp(L, R));
    }
  }
  ...
  ll ans = 0;
  VEC<pi> ANS;
  auto solve = [&](VEC<pi>& LR) -> void {
    sort(all(LR), [](auto x, auto y) { return x.se < y.se; });
    auto last = -1;
    ll cnt = 0;
    VEC<pi> TMP;
    for (auto&& [L, R] : LR) {
