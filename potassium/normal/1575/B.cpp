...
int main() {
	...
	for (int round = 1; round <= 100; round++) {
		...
		for (int i = 0; i < n; i++) {
			if (norm(a[i]) > 4 * m * m) continue;
			pt midpt = { a[i].first / 2, a[i].second / 2 };
			pt normal = { a[i].second, -a[i].first };
			normal = normal / sqrt(norm(normal));
			double len = sqrt(m * m - norm(midpt));

			pt u = midpt + normal * len;
			pt v = midpt - normal * len;

			double ang1 = atan2(u.second, u.first) + PI;
			double ang2 = atan2(v.second, v.first) + PI;

			if (ang1 < ang2) {
				event.push_back({ ang1, +1 });
				event.push_back({ ang2, -1 });
			}
			else {
				event.push_back({ 0, +1 });
				event.push_back({ ang2, -1 });
				event.push_back({ ang1, +1 });
				event.push_back({ 2 * PI, -1 });
			}
		}
