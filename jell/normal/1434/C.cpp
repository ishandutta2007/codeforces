#line 1 "codeforces-workspace\\contest\\1434\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\1434\\c\\c.cpp"
...
void main() {
  ...
  if (a - b * c > 0)
    ...
    auto ans1 = a + b * c * (e - 1) - b * d * e * (e - 1) / 2 + (a - b * c) * e;
    auto mid = (2 * a - b * d) / (b * d * 2);
    auto ans = ans1;
    mid--;
    auto ans2 = a * (mid + 1) - b * d * mid * (mid + 1) / 2;
    if (mid >= 0) chge(ans, ans2);
    mid++;
    auto ans3 = a * (mid + 1) - b * d * mid * (mid + 1) / 2;
    if (mid >= 0) chge(ans, ans3);
    mid++;
    auto ans4 = a * (mid + 1) - b * d * mid * (mid + 1) / 2;
    if (mid >= 0) chge(ans, ans4);
    cout << ans << eol;
  }
}
}