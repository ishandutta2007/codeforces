...
class emply(val l:Int, val r:Int){}
...
fun main(){
    solve.cases{
        ...
        all.sortBy { it.l }
        val ans = BinarySearchLastTrue(0,phuge){ s ->
            val free = mutableListOf<emply>()
            val must = mutableListOf<emply>()
            val mustnot = mutableListOf<emply>()
            for(a in all){
                if(a.l > s){
                    mustnot.add(a)
                }else if(a.r < s){
            ...
            val needtake = (n-1)/2 - must.size
            if(free.size < needtake){
                return@BinarySearchLastTrue  true
            }
            if(needtake < 0){
                return@BinarySearchLastTrue  false
            }
            var cost = 0L
            ...
            return@BinarySearchLastTrue cost <= budget
        }
        put(ans!!)
    }
    done()
