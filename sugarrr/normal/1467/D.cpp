...
bool anspro(){
    ...
    //rep(i,0,n-1)cout<<ep[i].val()<<" ";cout<<endl;
    vl a(n);rep(i,0,n-1)cin>>a[i];
    mint ans=0;
    rep(i,0,n-1)ans+=a[i]*ep[i];
    while (q--){
        ll p,x;cin>>p>>x;p--;
        ans+=(x-a[p])*ep[p];
        a[p]=x;
...
bool anspro(){
    rep(zz,2,10){
        ...
        rep(i,0,n-1)cout<<right<<setw(5)<<a[i]<<" ";cout<<endl;
    }
