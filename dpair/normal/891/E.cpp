...
signed main(){
    read(n, k); f[0] = 1; rep(i, 1, n) {
        const int x = read();
        per(j, i, 1) f[j] = (1ll * f[j] * x - f[j - 1] + MOD) % MOD;
        f[0] = 1ll * f[0] * x % MOD;
    } const int inv = ksm(n, MOD - 2); int ans = 0, cur = 1;
    rep(j, 0, n) {
        Add(ans, 1ll * f[j] * cur % MOD);
        cur = 1ll * cur * inv % MOD * (k - j) % MOD;
    } print((f[0] - ans + MOD) % MOD);
}