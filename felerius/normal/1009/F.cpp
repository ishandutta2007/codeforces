...
struct State {
    ...
    void merge(State& r) {
        if (sz(r.cnt) > sz(cnt))
            swap(cnt, r.cnt), swap(mx, r.mx);
        rep(i, sz(r.cnt)) {
            cnt[sz(cnt) - 1 - i] += r.cnt[sz(r.cnt) - 1 - i];
            mx = max(mx, pair(cnt[sz(cnt) - 1 - i], sz(cnt) - 1 - i));
        }
    }
};
...
int main() {
    ...
    vector ans(n, 0);
    auto dfs = [&](int v, int p, auto&& self) -> State {
        State s;
        trav(v2, adj[v]) {
            if (v2 == p)
                continue;
            auto cs = self(v2, v, self);
            cs.pull();
            s.merge(cs);
        }

        ans[v] = sz(s.cnt) - 1 - s.mx.second;
        return s;
    };
    dfs(0, -1, dfs);
