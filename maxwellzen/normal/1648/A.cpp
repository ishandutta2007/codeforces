...
void solve() {
    ...
    read(c, n, m);
    byc.rsz(100001);
    F0R(i, n) F0R(j, m) byc[c[i][j]].pb(mp(i,j));
    ll ans = 0;
    seg.init(m);
    cnt.init(m);
    F0R(color, 100001) if (byc[color].size()>0) {
        ll cur = 0;
        F0R(i, byc[color].size()) {
            pi p = byc[color][i];
            ans += i*p.F - cur;
            ans += cnt.query(0,p.S-1)*p.S - seg.query(0,p.S-1);
            ans += seg.query(p.S+1,m-1) - cnt.query(p.S+1,m-1)*p.S;
            cur += p.F;
            cnt.upd(p.S,1);
            seg.upd(p.S,p.S);
        }
