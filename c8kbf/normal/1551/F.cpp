...
_ n, m, x, y, fc[maxn], iv[maxn], s, d[maxn], dp[maxn][maxn];
v_ g[maxn];

void dfs(_ x, _ fa, _ df);
void vl(_ x, _ fa);
_ ch(_ n, _ k);
_ pw(_ x, _ p);
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
//        writeln(i);
        for(_ j = 0; j <= n; ++j) for(_ k = 0; k <= m; ++k) dp[j][k] = !k;
        for(_ j : g[i]) {
            vl(j, i);
            for(_ k = 0; k <= n; ++k) for(_ l = m; l >= 1; --l)
                dp[k][l] = (dp[k][l]+dp[k][l-1]*d[k])%mod;
        }
        for(_ j = 0; j <= n; ++j) s = (s+dp[j][m])%mod;
    }
    writeln(s);
...
void dfs(_ x, _ fa, _ df) {
    ++d[df];
    for(_ i : g[x]) if(i != fa) dfs(i, x, df+1);
    return;
}