...
struct FU {
	vi u, s;
	FU(int n) : u(n + 1), s(n + 1, 1) {iota(u.begin(), u.end(), 0);}
	int find(int x) {
		if (u[x] == x) return x;
		return u[x] = find(u[x]);
	}
...
void solve() {
	...
	for (int num = 1; num <= d; ++num) {
		int x, y;
		scanf("%d%d", &x, &y);
		if (f.find(x) == f.find(y)) spare++;
		else f.onion(x, y);
		vi sizes;
		for (int i = 1; i <= n; ++i) if (f.find(i) == i) sizes.push_back(f.s[i]);
		sort(sizes.begin(), sizes.end(), greater<>());
		int tot = 0;
		for (int i = 0; i <= spare; ++i) tot += sizes[i];
		printf("%d\n", tot - 1);
	}
}
int main() {
