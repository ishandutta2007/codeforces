...
void solve() {
  ...
  FOR(i, N) {
    XY[i].fi -= zx;
    XY[i].se -= zy;
    ll g = gcd(abs(XY[i].fi), abs(XY[i].se));
    XY[i].fi /= g;
    XY[i].se /= g;
  }
  auto key = XY;
  ...
  auto det = [&](pi A, pi B) -> ll { return A.fi * B.se - A.se * B.fi; };

  ll ANS = N * (N - 1) * (N - 2) / 6;
  auto I = angle_sort(key);
  // 2 
  I.resize(K + K);
  FOR(i, K) I[K + i] = I[i];
  // 1 / 2 
  ...
  FOR(L, K) {
    ...
    while (R < N + N && det(key[I[L]], key[I[R]]) > 0) {
      ll cnt = CNT[key[I[R]]];
      C2 += C1 * cnt;
      C1 += cnt;
      ++R;
  ...
  FORIN(p, CNT) {
    auto [xy, cnt] = p;
    if (is_lower_plane(xy)) {
      CNT1[mp(-xy.fi, -xy.se)] += cnt;
    } else
      CNT1[xy] += cnt;
  }
  FORIN(p, CNT1) {
