...
struct solver
{
    ...
    solver()
    {
            ...
            if(i)
            {
                ...
                for(int j=n; j>=0; --j)
                {
                    for(int k=n; k>=0; --k)
                    {
                        if(i)
                        {
                            sbmin(dp[j][k+1][1],dp[j][k][1]);
                            sbmin(dp[j][k+1][1],dp[j][k][0]+1);
                        }
                        else
                        {
                            sbmin(dp[j+1][k][0],dp[j][k][0]);
                            sbmin(dp[j+1][k][0],dp[j][k][1]+1);
                        }
                    }
                }
            }
        ...
        cout << *min_element(all(dp[n/2][(n+1)/2])) << "\n";
    }
}; // struct solver
