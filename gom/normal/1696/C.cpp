...
using mi=modnum<mod>;
const int N=2e5+5,M=2555,Q=1e6+5;
int n,m,k,a[N],b[N];
void solve(){
	cin>>n>>k;
	...
	for(int i=1;i<=n;i++){
		...
		if(va.size()&&va.back().fi==a[i]) va.back().se+=cnt;
		else va.emplace_back(a[i],cnt);
	}
	for(int i=1;i<=m;i++){
		ll cnt=1;
		while(b[i]%k==0){
		...
		if(vb.size()&&vb.back().fi==b[i]) vb.back().se+=cnt;
		else vb.emplace_back(b[i],cnt);
	}
	if(va!=vb) cout<<"No\n";
	else cout<<"Yes\n";
}
