...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		ans[i] = a[l];
		l += k;
		if (l <= r) queries[l].push_back({ r, i });
	}

	for (int last = n; last < n + k; last++) {
		vector<int> stk = { last };
		for (int l = last - k; l >= 0; l -= k) {
			while (!stk.empty() && b[stk.back()] > b[l]) stk.pop_back();
			dp[l] = dp[stk.back()] + (stk.back() - l) / k * b[l];
			stk.emplace_back(l);
			...
			for (auto& q : queries[l]) {
				...
				auto p = *lower_bound(stk.begin(), stk.end(), r, [](int x, int y) { return x > y; });
				ans[idx] += dp[l] - dp[p] + (r - p + k) / k * b[p];
			}
		}
	}
