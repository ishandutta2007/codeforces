...
struct da2
{
	int pos,id;
	ll tm;
	inline bool operator<(const da2&v)const
	{
		return tm>v.tm;
	}
};

#define md(...) (da){__VA_ARGS__}
...
int main()
{
	...
	fo1(i,m)
	{
		...
		{
			f.insert(md(q.top().pos,q.top().id,q.top().tm));
			q.pop();
		}
		std::set<da>::iterator x,y;
		x=f.lower_bound(md(a[i],-1,-1));
		y=x;
		if(x!=f.begin())
		{
			int t=(--y)->pos;
			y=f.lower_bound(md(t,-1,-1));
		}
		if(x==f.end())x=y;
		da xa=*x,ya=*y;
		xa.pos=abs(xa.pos-a[i]);
		ya.pos=abs(ya.pos-a[i]);
		if(xa<ya)xa=*x;else xa=*y;
		f.erase(xa);
		//out,xa.pos,' ',a[i],' ',tn,' ',t[i],'\n';
