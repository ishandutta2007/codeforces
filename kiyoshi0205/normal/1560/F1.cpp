...
bool ch[10],f;
string s;
string solve(int nowid,int nowk,bool isbig){
  assert(nowk<=K);
  if(nowid==N){
    f=true;
    return "";
  ...
  if(isbig){
    ...
    if(nowk==K){
      int i;
      rep(i,10)if(ch[i])return char('0'+i)+solve(nowid+1,nowk,isbig);
    }else{
      ch[0]=true;
      return '0'+solve(nowid+1,0,isbig);
    }
    assert(false);
  }
  int val=s[nowid]-'0';
  //s[nowid]
  if(ch[val]){
    auto ans=solve(nowid+1,nowk,isbig);
    if(f)return s[nowid]+ans;
  }else if(nowk<K){
    ch[val]=true;
    auto ans=solve(nowid+1,nowk+1,isbig);
    if(f)return s[nowid]+ans;
    ch[val]=false;
  }
  //s[nowid]+?
  while(val!=9){
    ...
    if(ch[val]){
      f=true;
      return char('0'+val)+solve(nowid+1,nowk,true);
    }else if(nowk<K){
      f=ch[val]=true;
      return char('0'+val)+solve(nowid+1,nowk+1,true);
    }
  }  
  return "";
}
bool judge(ll N,ll ans){
  while(1){
    auto s=to_string(N);
    uniq(s);
    if(int(s.size())<=K)return N!=ans;
    ++N;
  }
}
int main(){
...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  N=s.size();
  f=false;
  for(auto& x:ch)x=false;
  auto ans=solve(0,0,false);
  print(ans);
}}