...
void solve() {
  ...
  VEC(ll, A, N);
  const ll INF = 1LL << 60;
  vi left_max(N + 1, -INF), right_min(N + 1, INF);
  FOR(i, N) left_max[i + 1] = max(left_max[i], A[i]);
  FOR_R(i, N) right_min[i] = min(right_min[i + 1], A[i]);
  ll ANS = 0;
  FOR(i, N) ANS += left_max[i] <= right_min[i];
  print(ANS);
}
