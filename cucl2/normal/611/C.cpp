...
int main()
{
	...
	for(int i=1;i<=h;i++)
		for(int j=1;j<=w;j++)
			...
			{
				if(s[i][j+1]!='#')dp1[i][j]=1;
				if(s[i+1][j]!='#')dp2[i][j]=1;
				dp1[i][j]+=dp1[i-1][j]+dp1[i][j-1]-dp1[i-1][j-1];
				dp2[i][j]+=dp2[i-1][j]+dp2[i][j-1]-dp2[i-1][j-1];
			}
	scanf("%d",&q);
	...
	{
		int a,b,c,d;
		fio>>a>>b>>c>>d<<dp1[c][d-1]-dp1[c][b-1]-dp1[a-1][d-1]+dp1[a-1][b-1]+dp2[c-1][d]-dp2[c-1][b-1]-dp2[a-1][d]+dp2[a-1][b-1]<<endl;
	}
	return 0;
}