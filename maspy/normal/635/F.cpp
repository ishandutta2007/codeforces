...
#line 1 "/home/maspy/compro/library/graph/rerooting_dp.hpp"
...
#line 4 "/home/maspy/compro/library/graph/rerooting_dp.hpp"
...
void solve() {
  ...
  auto check = [&](ll a) -> bool {
    ...
    auto fee = [&](Data x, Data y) -> Data {
      return {x.can && y.can, x.a + y.a, max(x.b, y.b)};
    };
    auto fev = [&](Data x, int v) -> Data {
      if (A[v] < a) return {false, 0, 0};
      if (x.can) return {true, 1 + x.a, 0};
      return {false, 0, 1 + x.a + x.b};
    };
    auto fve = [&](Data x, auto& e) -> Data { return x; };
    auto dp = rerooting_dp(G, fee, fev, fve, unit);
    FOR(v, N) if (dp[v].a + dp[v].b >= K) return true;
    return false;
  };
  ll ANS = binary_search(check, 0, MAX(A) + 1);
  print(ANS);
}
