...
void solve() {
    ...
    auto top = [&](int v) { return min(r - v, v - l); };
    auto bot = [&](int v) { return -top(v); };

    auto compute = [&](int k) -> mn {
        int add = max(0, max(k - top(1), bot(1) - k));
        int sub = max(0, max(k - top(n), bot(n) - k));
        int both = n - add - sub;
        return mn::ncr(both, small - add) + mn::ncr(both, big - add);
    };

    mn ans = 0;

    int k_repeat = min(top(1), top(n));
    ans += k_repeat * (mn::ncr(n, small) + mn::ncr(n, big));

    mn x;
    for (int k = k_repeat + 1; (x = compute(k)) != 0; k++) {
        ans += x;
    }
