...
inline void work()
{
    rin>>n>>k;m=0;d.clear();for(int i=1;i<=n;i++)rin>>a[i],m+=(a[i]==k);if(!m){puts("no");return;}if(n==1){puts("yes");return;}
    for(int i=1,j;i<=n;i=j){if(a[i]<k){j=i+1;continue;}for(j=i+1;j<=n&&a[j]>=k;j++);if(j>i+1){puts("yes");return;}d.push_back(i);}
    for(vector<int>::iterator i=d.begin();i!=d.end();i++){if((i+1)==d.end())break;if((*(i+1))-(*i)<=2){puts("yes");return;}}
    puts("no");return;
}
int main()
{
