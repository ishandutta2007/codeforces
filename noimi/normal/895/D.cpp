...
mint invcmb(int a,int b){
    if(a<b)return 0;
    if(a<0||b<0)return 0;
    return {invprd[a]*prd[b]*prd[a-b]};
}
...
mint solve(string s,string t){
    ...
    rep(i,n){
        vec now;
        rep(j,26)now.pb(cnt[j]-c[j]);
        mint tmp=1;
        int sum=n-i;
        rep(j,26) {
            tmp*=cmb(sum,now[j]); sum-=now[j];
        }
        sum=n-i;
        rep(j,26){
            if(t[i]>a+j)
            ans+=tmp * cmb(sum-1,now[j]-1) * invcmb(sum,now[j]);
        }
        c[t[i]-a]++;
    }
    return ans;
...
signed main(){
    ...
    calc();
    int n=s.size();
    cout << solve(s,t) - solve(s,s) - 1 <<endl;
}