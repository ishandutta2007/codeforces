...
void solve() {
  ...
  FOR(v, N) {
    ...
    if (lo[v] + hi[v] >= THRESH) {
      idx[v] = len(sets);
      sets.eb(B());
      for (auto&& e: G[v]) { sets.back()[e.to] = 1; }
    }
  }

  print(ANS);
  auto upd = [&](ll v, ll to) -> void {
    if (A[v] > A[to]) return;
    ANS -= lo[v] * hi[v] + lo[to] * hi[to];
    lo[v] += 1;
    hi[v] -= 1;
    lo[to] -= 1;
    hi[to] += 1;
  ...
  FOR(q, Q) {
    ...
    if (lo[v] + hi[v] < THRESH) {
      for (auto&& e: G[v]) { upd(v, e.to); }
    } else {
      ll to = v;
      while (1) {
        to = nxt[to];
        if (to == -1) break;
        if (sets[idx[v]][to]) upd(v, to);
      }
    }
    assert(hi[v] == 0);
