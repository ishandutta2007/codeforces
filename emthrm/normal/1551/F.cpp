...
void solve() {
  ...
  REP(root, n) {
    ...
    for (int e : graph[root]) {
      visited[e] = true;
      ver.emplace_back(vector<int>{e});
    }
    while (!ver.empty()) {
      const int m = ver.size();
      if (m >= k) {
        vector<ModInt> dp(m + 1, 0);
        dp[0] = 1;
        for (const vector<int> &v : ver) {
          for (int i = m - 1; i >= 0; --i) {
            dp[i + 1] += dp[i] * v.size();
          }
        }
      ...
      for (const vector<int> &v : ver) {
        vector<int> child;
        for (int vi : v) for (int e : graph[vi]) {
          if (!visited[e]) {
            visited[e] = true;
            child.emplace_back(e);
          }
        }
        if (!child.empty()) nx.emplace_back(child);
      }
      ver.swap(nx);
    }
  }
  ...
  // lca.build();
  // int ans2 = 0, comb = (1 << k) - 1;
  // while (comb < (1 << n)) {
  //   vector<int> v;
  //   REP(i, n) {
  //     if (comb >> i & 1) v.emplace_back(i);
  //   }
  //   const int c = lca.distance(v.front(), v.back());
  //   bool is_valid = true;
  //   REP(i, v.size()) FOR(j, i + 1, v.size()) {
  //     is_valid &= lca.distance(v[i], v[j]) == c;
  //   }
  //   ans2 += is_valid;
