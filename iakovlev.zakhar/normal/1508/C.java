...
//  }
	...
	public void dijikstra(int n, int m, int xor) {
		long ans = 0;
		Queue<Integer> zero = new ArrayDeque<>();
		TreeSet<Edge> set = new TreeSet<>();
		for (int i = 1; i < n; i++) {
			if (to[0].containsKey(i)) {
				set.add(new Edge(0, i, to[0].get(i)));
			} else {
				zero.add(i);
			}
		}
		...
		while (!set.isEmpty()) {
			...
			TreeSet<Edge> set1 = new TreeSet<>();
			for (Edge e : set) {
				if (to[x].containsKey(e.y)) {
					int w1 = to[x].get(e.y);
					if (w1 < e.edge) {
						set1.add(new Edge(x, e.y, w1));
					} else {
						set1.add(e);
					}
				} else {
		...
		if (n - 1 - solid + m < (n - 1L) * n / 2) {
			out.println(ans);
			return;
		}
		int min = xor;
		for (int x = 0; x < n; x++) {
			for (Entry<Integer, Integer> e : to[x].entrySet()) {
				if (dsu.get(x) != dsu.get(e.getKey())) {
