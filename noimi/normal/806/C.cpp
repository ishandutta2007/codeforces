...
ll popcountll(ll x) { return __builtin_popcountll(x); }
...
main() {
    ...
    int now = accumulate(all(cnt), 0);
    rep3(i, 43, 0) cnt[i] += cnt[i + 1], rem[i] += rem[i + 1];
    rep(i, 45) if(cnt[i] < rem[i]) end();
    vi ans{now};
    int i = 0;
    while(1) {
        while(i < 44 and cnt[i] == cnt[i + 1]) i++;
        while(cnt[i] > cnt[i + 1]) {
            rep(j, i + 1) cnt[j]--;
            if(i == 0)
                rem[0]++;
