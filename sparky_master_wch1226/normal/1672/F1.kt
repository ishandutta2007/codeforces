...
fun main(){
    solve.cases{
        val n = getint
        val L = getline(n)
//        val firstseen = BooleanArray(n+1)
//        val iskey = BooleanArray(n)

//        val seen = IntArray(n+1)

        val occur = List(n+1){mint}
        val hassize = List(n+1){ mutableListOf<Pair<Int,Int>>() }
        for((i,a) in L.withIndex()){
            occur[a].add(i)
            hassize[occur[a].size].add(Pair(i,a))
        }

        val ret = IntArray(n)
        for(has in 1..n){
            if(hassize[has].isEmpty()){
                break
            }
            val here = hassize[has]
            here.sortBy { it.second }
            val v = hassize[has].size
            for(i in 0 until v){
                val (herei, _) = here[i]
                val (_, goto) = here[(i+1) % v]
                assert(ret[herei] == 0)
                ret[herei] = goto
            }
        }
...
//        for(i in 0 until n){
//            if(iskey[i]){
//                ret[i] = keyvalues[(curkey + 1) % keyvalues.size]
//                curkey ++
//            }
//        }
//        put(ret.joinToString(" "))
