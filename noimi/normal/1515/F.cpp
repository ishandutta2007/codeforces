...
struct UnionFind {
    ...
    UnionFind(int n) : num(n), v(n), edges(n) { data.assign(n, -1); }
    ...
    bool unite(int x, int y) {
        ...
        if(si(edges[x]) < si(edges[y])) swap(edges[x], edges[y]);
        for(auto e : edges[y]) edges[x].eb(e);
        return true;
    }
...
int main() {
    ...
    UnionFind uf(n);
    rep(i, n) uf.v[i] = a[i];
    rep(i, n) for(auto e : g[i]) { uf.edges[i].eb(e); }
    rep(i, n) {
        if(a[i] >= x) {
            int r = uf[i];
            while(uf.val(r) >= x and uf.edges[r].size()) {
                auto e = uf.edges[r].back();
                int nxt = e, idx = e.cost;
                uf.edges[r].pop_back();
                if(!uf.same(r, nxt)) {
                    uf.unite(r, nxt);
                    ans.eb(idx);
    ...
    rep(i, n) for(auto e : g[i]) {
        if(!uf.same(i, e) and uf.val(i) + uf.val(e) >= x) {
            uf.unite(i, e);
            ans.eb(e.cost);
            uf.v[uf[i]] -= x;
        }
