...
class DisjointUnionSets(val n: Int) {
    ...
    val sacks:IntArray = IntArray(n){-1}
...
//                }
    ...
    fun Int.sackthis(other:Int):Int{
        if(this == -1){
            return other
        }else if(other == -1){
            return this
    ...
    fun union(x: Int, y: Int, sac:Int):Boolean {
        var xRoot = find(x)
        var yRoot = find(y)
        if (xRoot == yRoot){
            edges[xRoot] ++
            updated.addLast(xRoot)
            sacks[xRoot] = sacks[xRoot].sackthis(sac)
            return false
        }
        components--
        ...
        sacks[xRoot] = sacks[xRoot].sackthis(sac)
        sacks[xRoot] = sacks[xRoot].sackthis(sacks[yRoot])
...
fun main(){
    solve.rand {
        ...
        share(n)
        repeat(2 * n){
            share(Random.nextInt(0..10000000))
        }
    }
//    solve.userand()
    FACT.preCal(1005)
...
//        just dei final
        var ret = 0
        for(c in 1..n){
            val options = FACT.choose(n,c)
            val got = final[c]
            ret = ret modPlus ( got modM options modM FACT.store[c])
        }
        put(ret)
