...
void solve() {
  ...
  auto find = [&](ll pos) -> ll {
    ll x = RH.query(SH, pos, pos + K);
    ll i = LB(H, x);
    if (i < len(H) && H[i] == x) return I[i];
    return -1;
  };

  vi CNT(Q);
  auto check = [&](vi& ANS) -> bool {
    if (MIN(ANS) < 0) return false;
    bool ok = true;
    for (auto&& x: ANS)
      if (++CNT[x] > 1) ok = false;
    for (auto&& x: ANS) CNT[x] = 0;
    return ok;
  };

  FOR(p, K) {
    vi ANS(N);
    FOR(i, N) ANS[i] = find(p + K * i);
    if (check(ANS)) {
      YES();
      for (auto&& x: ANS) ++x;
      print(ANS);
