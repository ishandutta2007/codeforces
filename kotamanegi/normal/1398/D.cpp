...
void solve(){
    ...
    REP(i, go[0] + 1) {
        REP(q, go[1] + 1) {
            REP(j, go[2] + 1) {
                if (i != go[0] and q != go[1]) {
                    dp[i + 1][q + 1][j] = max(dp[i + 1][q + 1][j], dp[i][q][j] + data[0][i] * data[1][q]);
                }
                if (i != go[0] and j != go[2]) {
                    dp[i + 1][q][j + 1] = max(dp[i + 1][q][j + 1], dp[i][q][j] + data[0][i] * data[2][j]);
                }
                if (j != go[2] and q != go[1]) {
                    dp[i][q + 1][j + 1] = max(dp[i][q + 1][j + 1], dp[i][q][j] + data[2][j] * data[1][q]);
                }
            }
        }
    }
