...
int main() {
    ...
    vvmint dp(M + 1, vmint(M + 1, 0));
    rep(i, M + 1)dp[i][i] = 1;

    set<ll> omit = { 3,5,14,15 };

    rep(i, M + 1)for (ll j = i + 1; j <= M; j++) {
        //cout << i << j << endl;
        
        repn(k,3)if (j >= i + k)dp[i][j] += dp[i][j - k];

        if (j >= i + 4) {
            ll c = s[j - 4] * 8 + s[j - 3] * 4 + s[j - 2] * 2 + s[j - 1];
            if (!omit.count(c))dp[i][j] += dp[i][j - 4];
        }
    ...
    repn(i, M) {
        //cout << i << endl;
        vll t{ s.begin(),s.begin() + i };
        reverse(t.begin(), t.end());

        auto z = z_algorithm<ll>(t);
        ll mx = 0;
        repn(j, i - 1)mx = max(mx, z[j]);
        //cout << mx << endl;

        for (ll j = 0; j < i - mx; j++)ans += dp[j][i];
        cout << ans.val() << endl;
    }
