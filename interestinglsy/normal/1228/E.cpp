...
int main(){
	...
	Read(n,k);
	ll ans = 0;
	Forx(illegal_col,0,n-1){
		ll each_row = Pow(k-1,illegal_col) * ( Pow(k,n-illegal_col)-Pow(k-1,n-illegal_col)+MOD ) % MOD;
		ll tans = Pow(each_row,n) * C(n,illegal_col) % MOD;
		if(illegal_col&1) Deci(ans,tans);
		else Addi(ans,tans);
	}
	cout << ans << endl;
