...
#define int long long
    ...
    vv(int, d, n * 2 + 1, n + 1, inf<int>);
    VV(int, a, 2, n);
    REC([&](auto &&f, int i, int now, int t, int sum) -> void {
        if(i == n) {
            chmin(d[now][t], sum);
        } else {
            f(i + 1, now + 1, t, sum + a[0][i]);
            f(i + 1, now - 1, max(t, n - now + 1), sum + a[1][i]);
        }
    })
    (0, n, 0, 0);

    auto merge = [&](vvc<int> &l, vvc<int> &r) {
        vv(int, res, n * 2 + 1, n + 1, inf<int>);
        rep(i, si(l)) rep(j, si(l[0])) rep(ii, si(r)) rep(jj, si(r[0])) {
            int ni = (i - n) + (ii - n) + n;
            int nj = max(j, jj - (i - n));
            if(0 <= ni and ni <= n * 2 and 0 <= nj and nj <= n) chmin(res[ni][nj], l[i][j] + r[ii][jj]);
        }
        return res;
    };
    ...
    rep(i, 29) { db[i + 1] = merge(db[i], db[i]); }

    vv(int, tmp, n * 2 + 1, n + 1, inf<int>);
    tmp[n][0] = 0;
    rep(i, 29) if(m & 1 << i) tmp = merge(tmp, db[i]);
    OUT(tmp[n][0]);
}