...
void solve() {
	...
	REP(q, 200) {
		...
		ld left = 0;
		ld right = l;
		ld remaining[2] = { mid,mid };
		REP(j, n) {
			ld timing = (gogo[j] - left) / (ld)(j + 1LL);
			timing = min(timing, remaining[0]);
			left += timing * (ld)(j + 1LL);
			remaining[0] -= timing;
			timing = (right - gogo[n - j - 1]) / (ld)(j + 1);
			timing = min(timing, remaining[1]);
			right -= timing * (ld)(j + 1LL);
			remaining[1] -= timing;
		}
		left += (ld)(n + 1) * remaining[0];
		right -= (ld)(n + 1) * remaining[1];
		if (left >= right) bot = mid;
		else top = mid;
	}
	cout << bot << endl;
}
