...
int n;
vector<int>edge[SZ];
int up[20][SZ],dep[SZ],in[SZ],out[SZ],nxt,upp[SZ];
struct dbling{
	bool ready = 0;
	void dfs(int v,int u,int d){
		up[0][v] = u; dep[v] = d; upp[v] = u;
		in[v] = nxt++;
		rep(i,edge[v].size()){
...
int lim;
using S = P;
S op(P a, P b){ return max(a, b); }
S e(){ return mp(-1e18, -1e18); }
using FF = int;
S mpp(FF f, S x){ return mp(x.a+f, x.b); }
FF cps(FF f, FF g){ return f+g; }
FF id(){ return 0; }
lazy_segtree<S, op, e, FF, mpp, cps, id> seg(1<<18);
bool rem[SZ];
void solve(){
	cin>>n>>lim;
	rep(i, n-1){
