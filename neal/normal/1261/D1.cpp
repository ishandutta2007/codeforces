...
int main() {
    ...
    mod_int total = 0;
    mod_int INV2 = mod_int(2).inv();

    for (int zeros = 0; zeros < different; zeros++) {
        int remaining = different - zeros;
        mod_int ways = 0;

        if (remaining % 2 == 1) {
            ways = mod_int(2).pow(remaining - 1);
        } else {
            ways = mod_int(2).pow(remaining - 1) - choose(remaining, remaining / 2) * INV2;
        }

        ways *= choose(different, zeros);
        ways *= mod_int(K - 2).pow(zeros);
        total += ways;
    }

    total *= mod_int(K).pow(N - different);
    cout << total << '\n';
}