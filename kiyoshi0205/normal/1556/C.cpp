...
datas op(datas l,datas r){
  return datas(l.first+r.first,l.second+r.second);
}
datas e(){return datas(0,0);}
datas mapping(datas x,datas f){
  return datas(x.first*f.first+f.second*x.second,x.second);
}
datas composition(datas f,datas g){
  return datas(f.first*g.first,f.second*g.first+g.second);
}
datas id(){
  return datas(1,0);
}
using segtree=lazy_segtree<datas,op,e,datas,mapping,composition,id>;
...
int main(){
  ...
  ll i,j;
  cin>>N;
  vec sum(N+1,0),za={0,1};
  rep(i,N){
    cin>>j;
    if(i&1){
      sum[i+1]=sum[i]-j;
      za.emplace_back(sum[i+1]);
      za.emplace_back(sum[i+1]+1);
    }else{
      sum[i+1]=sum[i]+j;
      za.emplace_back(sum[i+1]);
    }
  ...
  pvec init(za.size()-1);
  rep1(i,za.size()){
    init[i-1].second=za[i]-za[i-1];
    if(!za[i-1])init[i-1].first=1;
  }
  segtree cnt(init);
  rep(i,N){
    if(i&1){
      int aid=lower_bound(all(za),sum[i+1])-za.begin();
      int paid=lower_bound(all(za),sum[i+1]+1)-za.begin();
      int bid=lower_bound(all(za),sum[i])-za.begin();
      debug(aid,bid);
      ans+=cnt.prod(aid,bid).first;
      debug(ans);
      debug(paid,bid);
      cnt.apply(paid,bid,datas(0,0));
      cnt.apply(aid,datas(1,1));
    }else{
      int paid=lower_bound(all(za),sum[i]+1)-za.begin();
      int bid=lower_bound(all(za),sum[i+1])-za.begin();
      debug(paid,bid);
      cnt.apply(paid,bid,datas(1,1));
    }
  }
  print(ans);
}