...
int main() {
	...
	long long k;
	scanf("%d%lld", &n, &k);
	long long should_outside = (long long) n * (n - 1) / 2 - k;
	vector<P> they(n);
	for(P& p : they) {
		p.read();
	}
	...
	for(int rep = 0; rep < 42; ++rep) {
		...
		for(P p : they) {
			P orig{0,0}; // later: do I need to create?
		...
		for(int pos = 0; pos < (int) values.size(); ++pos) {
			...
			if(where[id] == -1) {
				...
				// int L = base + where[id];
				// for(int i = base + where[id] + 1; i <= base + pos; ++i) {
					// outside += tree[i];
				// }
		...
		/*
		vector<pair<int,int>> special(s);
		for(int i = 0; i < (int) values.size(); ++i) {
			int pos = values[i].second / 2;
			int type = values[i].second % 2;
			if(type) special[pos].second = i;
			else special[pos].first = i;
		}
		debug() << imie(special);
		s = special.size();
		...
		int big = 0;
		for(int i = 0; i < s; ++i) {
			events.emplace_back(special[i].first, i);
			events.emplace_back(special[i].second, i);
			big = max(big, special[i].first);
			big = max(big, special[i].second);
		}
		...
		// for(int i = 0; i < s; ++i) {
			// for(int j = i + 1; j < s; ++j) {
				// pair<int,int> p = special[i];
				// pair<int,int> q = special[j];
				// bool intersect = false;
				// if(p.first < q.first && q.first < p.second && p.second < q.second) {
					// intersect = true;
				// }
				// swap(p, q);
				// if(p.first < q.first && q.first < p.second && p.second < q.second) {
		...
		// for(int i = 0; i < n; ++i) {
			// for(int j = i + 1; j < n; ++j) {
				// if(L2{they[i], they[j]}.dist(P{0, 0}) > mid) {
					// outside++;
				// }
			// }
		// }
