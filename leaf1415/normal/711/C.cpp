...
int main(void)
{
	...
	rep(i, 0, n) rep(j, 0, d) rep(k, 0, m) dp[i][j][k] = inf;
	dp[0][0][0] = 0;

	rep(i, 0, n-1) rep(j, 0, d) rep(k, 0, m){
		rep(l, 1, m){
			if(c[i+1] != 0 && c[i+1] != l) continue;
			ll nj = j;
			if(k != l) nj++;
			chmin(dp[i+1][nj][l], dp[i][j][k] + p[i+1][l]);
		}
	}

	ll ans = inf;
	rep(k, 1, m) chmin(ans, dp[n][d][k]);
	if(ans > inf/2) ans = -1;
	outl(ans);
