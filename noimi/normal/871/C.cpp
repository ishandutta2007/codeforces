...
signed main(){
    ...
    rep(i,n){
        ...
        if(!mp.count(y+T))mp[y+T]=cnt++;
        a.eb(x,y+T);
    }
    UnionFind uf(mp.size());
    vector<pll> b;
    for(auto e:a){
        if(!uf.unite(mp[e.fi],mp[e.se]))b.eb(mp[e.fi],mp[e.se]);
    }
    vec used(cnt);
    mint ans=1;
    for(auto e:b){
        if(!used[uf.find(e.fi)]){
            used[uf.find(e.fi)]=1;
            ans*=modpow(2,uf.size(e.fi));
        }
    }
    rep(i,cnt){
        if(!used[i] and uf.find(i)==i){
            ans*=modpow(2,uf.size(i))-1;
        }
    }
    cout<<ans<<endl;
}