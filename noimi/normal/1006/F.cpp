...
int main()
{
    ...
    vvv(ll, mem, n, m);
    vvv(ll, mem2, n, m);
    mem[0][0].eb(a[0][0]), mem2[n - 1][m - 1].eb(a[n - 1][m - 1]);
    rep(i, n)
    {
        rep(j, m)
        {
            ...
            {
                for (auto &e : mem[i][j])
                    mem[i + 1][j].eb(e ^ a[i + 1][j]);
            }
            if (j < m - 1)
            {
                for (auto &e : mem[i][j])
                {
                    mem[i][j + 1].eb(e ^ a[i][j + 1]);
                }
            }
        }
    }
    rep3(i, n - 1, 0)
    {
        rep3(j, m - 1, 0)
        {
            ...
            {
                for (auto &e : mem2[i][j])
                    mem2[i - 1][j].eb(e ^ a[i - 1][j]);
            }
            if (j)
            {
                for (auto &e : mem2[i][j])
                    mem2[i][j - 1].eb(e ^ a[i][j - 1]);
            }
        }
    }
    ll ans = 0;
    rep(i, n)
    {
        rep(j, m)
        {
            if (i + j == mid)
            {
                ...
                for (auto &e : mem[i][j])
                {
                    ans += ub(mem2[i][j], e ^ k ^ a[i][j]) - lb(mem2[i][j], e ^ k ^ a[i][j]);
                }
            }
        }
    }
