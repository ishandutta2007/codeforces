...
void solve() {
  ...
  auto f = [&](auto dfs, ll la, ll ra, ll lb, ll rb) -> ll {
    if (la > ra || lb > rb) return 0;
    if (ra < 0 || rb < 0) return 0;
    auto key = mt(la, ra, lb, rb);
    ...
    FOR(a, 2) FOR(b, 2) {
      if ((a + b) != (a ^ b)) continue;
      ll la1 = floor(la - a + 1, 2LL);
      ll ra1 = floor(ra - a, 2LL);
      ll lb1 = floor(lb - b + 1, 2LL);
      ll rb1 = floor(rb - b, 2LL);
      ANS += dfs(dfs, la1, ra1, lb1, rb1);
    }
    return ANS;
  };
  print(f(f, a, b, a, b));
