...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		int INF = 1e9;
		int all = 1 << n;
		vector<vector<int>> dp(m + 1, vector<int> (all, INF));
		dp[0][0] = 0;
		...
		for (int j: range(m)) {
			...
			for(int i: range(n)) {
				...
				mask[S[i][j] - 'a'] += 1 << i;
				sum[S[i][j] - 'a'] += inp[i][j];
				mx[S[i][j] - 'a'] = max(mx[S[i][j] - 'a'], inp[i][j]);
			}

			for (int oldm: range(1 << n)) {
				if(dp[j][oldm] >= INF)
					continue;
				dp[j + 1][oldm] = min(dp[j + 1][oldm], dp[j][oldm]);
				for (int i = 0; i < n; ++i) {
					dp[j][oldm | (1 << i)] = min(dp[j][oldm | (1 << i)], dp[j][oldm] + inp[i][j]);
				}
				for (int c = 0; c < 26; ++c) {
					if(cnt[c] >= 1)
						dp[j][oldm | mask[c]] = min(dp[j][oldm | mask[c]], dp[j][oldm] + sum[c] - mx[c]);
				}
			}
		}
