...
int main() {
	...
	vector<ModInt> f(other + 1);
	vector<ModInt> g(other + 1);
	...
	vector<ModInt> power(other + 1);
	for (int i = 0; i <= other; i++) {
		power[i] = i == 0 ? 1 : power[i - 1] * (m - 2);
	}
	ModInt zero = 0;
	for (int i = other & 1; i <= other; i += 2) {
		zero += C(other, i) * power[i] * C(other - i, (other - i) / 2);
	}
	ModInt sum = 1;
	for (int i = 1; i <= other; i++) {
		sum *= m;
	}
	cout << ans * (sum - zero) / 2 << endl;
	return 0;
}