...
const int MAX_LOG = 19, N = 1e5+5, MAX = 100;
set<int> nearlyPrimes;
vi todo;
void setup() {
	...
	for(int i = 2; i <= 10; ++i)
		for(int j = 2; j <= 10; ++j)
			if(isPrime(i)&&i!=j&&isPrime(j))
				nearlyPrimes.insert(i * j);
	
	
	for(int i : nearlyPrimes)
		todo.pb(i);
}
void solve(int test_case = 0) {
	...
	for(int i = 0; i < 6; ++i)
		for(int j = i + 1; j < 6; ++j)
			for(int k = j + 1; k < 6; ++k)
				if(!nearlyPrimes.count(n - todo[i] - todo[j] - todo[k]) && n - todo[i] - todo[j] - todo[k] > 0)
					ans = {todo[i], todo[j], todo[k]};
