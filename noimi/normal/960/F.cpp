...
int main() {
    INT(n, m);
    vector<set<pii>> s(n);
    rep(i, n) s[i].emplace(-1, 0);

    while(m--) {
        INT(u, v, w);
        u--, v--;
        auto it = prev(s[u].lower_bound(pii(w, 0)));
        int nxt = it->se + 1;
        it = prev(s[v].lower_bound(pii(w, inf<int>)));
        if(it->se >= nxt)
            continue;
        else {
            it = next(it);
            while(it != end(s[v]) and it->se <= nxt) { it = s[v].erase(it); }
            s[v].emplace(w, nxt);
        }
    }
    int ans = 0;
    rep(i, n) chmax(ans, s[i].rbegin()->se);
    OUT(ans);
}