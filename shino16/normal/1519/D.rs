...
fn main() {
    scan!(n: usize, a0: [u64; n], b0: [u64; n]);
    let n = n * 2 - 1;
    ...
    let prod: Vec<_> = a.iter().zip(b.iter()).map(|(a, b)| a * b).collect();
    let cum = cuml_sum(&prod);

    let mut ans = cum[n];
    for i in 0..n {
        let mut p = a[i] * b[i];
        for d in 1..=i.min(n - 1 - i) {
            p += a[i - d] * b[i + d] + a[i + d] * b[i - d];
            ans = ans.max(p + cum[i - d] + cum[n] - cum[i + d + 1]);
        }
    }
