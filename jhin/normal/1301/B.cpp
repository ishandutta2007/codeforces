                                   ...
                                   long long m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500003],d[500003];vector<int>vr,v2[100003];long long sum,ans,num;char ch;
                                    ...
                                    int main()
                                    {
                                        ...
                                        while(t--)
                                        {
                                            ...
                                            for(int i=0;i<n;i++)
                                            {
                                                if(i<n-1&&a[i]>=0&&a[i+1]==-1)mx=max(mx,a[i]),mn=min(mn,a[i]);
                                                if(i!=0&&a[i]>=0&&a[i-1]==-1)mx=max(mx,a[i]),mn=min(mn,a[i]);
                                            ...
                                            x=0;
                                             for(int i=0;i<n-1;i++)
                                            {
                                                if(a[i]>=0&&a[i+1]>=0)x=max(x,abs(a[i]-a[i+1]));

                                            }
                                            if(mn==1e9)mn=mx;
                                            cout<<max(x,(mx-mn+1)/2)<<" "<<(mx+mn)/2<<endl;
