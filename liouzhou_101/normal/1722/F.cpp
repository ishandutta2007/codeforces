		...
		for (int j = 0; j < m; ++j)
		{
			...
			for (int k = 0; k < q.size(); ++k)
			{
				...
				for (int d = 0; d < 8; ++d)
				{
					...
					if (0 <= tx && tx < n && 0 <= ty && ty < m && !u[tx][ty] && a[tx][ty] == '*')
					{
						u[tx][ty] = 1;
						q.push_back({ tx, ty });
					}
			...
			if (q.size() != 3) return no();
			sort(q.begin(), q.end());
			for (int k = 1; k <= 2; ++k)
			{
				q[k].first -= q[0].first;
				q[k].second -= q[0].second;
			}
			if (q[1] == pair(1, 0) && q[2] == pair(1, 1)) continue;
			if (q[1] == pair(1, -1) && q[2] == pair(1, 0)) continue;
			if (q[1] == pair(0, 1) && q[2] == pair(1, 1)) continue;
			if (q[1] == pair(0, 1) && q[2] == pair(1, 0)) continue;
			return no();
		}
	yes();
}
