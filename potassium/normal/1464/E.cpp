...
int main() {
	...
	vector<int> grCount(MAXG, 0);
	for (int i = 1; i <= n; i++)
		grCount[grundy[i]]++;
	
	vector<vector<Mint>> basis(MAXG);
	auto push = [&](vector<Mint>& v) {
		for (int i = 0; i < MAXG; i++) {
			...
			Mint mul = v[i] / basis[i][i];
			for (int j = i; j <= MAXG; j++)
				v[j] -= basis[i][j] * mul;
		}
	};
	
	const Mint PR = Mint(1, MOD998) / (n + 1);
	for (int i = 0; i < MAXG; i++) {
		vector<Mint> vec(MAXG + 1);
		for (int j = 0; j < MAXG; j++)
			vec[j] = PR * grCount[j ^ i] - (i == j);
			
		reverse(vec.begin(), vec.end() - 1);
			
		vec[MAXG] = (i == 0 ? Mint(0, MOD998) : PR);
		push(vec);
	}
	
	assert(!basis[MAXG - 1].empty());
	W(-basis[MAXG - 1][MAXG] / basis[MAXG - 1][MAXG - 1]);
