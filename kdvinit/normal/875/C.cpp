...
void solve()
{
    ...
    for(int i=1; i<n; i++)
    {
        ...
        while(j<l1 && j<l2 && a[i][j]==a[i+1][j]) j++;
    ...
    for(int i=1; i<=m; i++)
    {
        ...
        if(SCC_Topo_Index[x]>SCC_Topo_Index[y]) Capital.push_back(i);
    }

    cout<<"Yes"<<endl;
    cout<<Capital.size()<<endl;
    for(int x: Capital) cout<<x<<" ";
    cout<<endl;
}
