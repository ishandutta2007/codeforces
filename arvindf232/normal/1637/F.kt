...
fun main(){
    solve.cases{
        ...
        val root = (0 until n).maxByOrNull { H[it] }!!
        G.setup(root)
        val submax = IntArray(n){0}
        var total = 0L
        G.DFS(root) { v ->
//            just dei v
            val p = G.parent[v]
            ...
            val extracost =
                if(G.subs[v].isEmpty()) H[v]
                else if(H[v] > submax[v]) H[v] - submax[v] else 0
            total += extracost
            submax[v] = maxOf(submax[v], H[v])
            submax[p] = maxOf(submax[p], submax[v])
//
//            G.NS(v) { s ->
//
//            }
        }
        if(G.subs[root].size == 1){
            total += H[root]
        }else{
            total += H[root] - G.subs[root].map { submax[it] }.sortedDescending()[1]
        }
        put(total)
