...
int idx(int i) {return lb(all(c), i)-c.begin();}

void solve() {
    cin >> n;
    read(a, n);
    read(b, n);
    c = a; c.insert(c.end(), all(b)); sort(all(c)); c.erase(unique(all(c)), c.end());
    m = c.size();
    ll sum = 0;
    F0R(i, n) sum += abs(a[i]-b[i]);
    ll offset = 0;
    seg.init(m);
    F0R(i, n) if (a[i]>b[i]) seg.upd(idx(a[i]), b[i]);
    F0R(i, n) if (a[i]<b[i]) {
        ll cur = 2ll * ((ll)b[i] - max(a[i], seg.query(idx(b[i]), m-1)));
        debug(cur);
        ckmax(offset, cur);
    }
    debug(offset);
    seg.init(m);
    F0R(i, n) if (a[i]<b[i]) seg.upd(idx(b[i]), a[i]);
    debug(seg.seg);
    F0R(i, n) if (a[i]>b[i]) {
        ll cur = 2ll * ((ll)a[i] - max(b[i], seg.query(idx(a[i]), m-1)));
        debug(cur);
        ckmax(offset, cur);
    }
    debug(offset);
