...
void solve() {
    int n;
    cin >> n;
    int low = n + n - 1, high = n * 2000 + n - 2, mid, res = n * 2000 + n - 1;
    while (low <= high) {
        mid = (low + high) / 2;
        if (ask(mid) == 1) {
            res = mid;
    ...
    int ans = res;
    int total_length = ans - (n - 1);
    for (int row = 2; row <= n; row++) {
        // minimum amount that we can spend is total_length + (n - 1) - (row - 1)
        // or total_length + n - row
        // the next answer is total_length + n, which is greater than the current answer of total_length + n - 1
        // so we only need to verify one answer
        int required_w = (total_length + n - row) / row;
        while (row * required_w < total_length + n - row) required_w++;
        int return_h = ask(required_w);
        if (return_h) ans = min(ans, return_h * required_w);
    }
    cout << "! " << ans << '\n';
    cout.flush();
}
