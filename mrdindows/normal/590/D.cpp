...
int main() {
    ...
    for (int i = 1; i <= k; ++i) {
        dp[i][0] = dp[i - 1][0] + a[k - i];
    }
    for (int i = k - 1; i >= 0; --i) {
        for (int c = k - 1; c >= 0; --c) {
            if (k - 1 - i < c) continue;
            int tr = k - 1 - c - i;
            for (int op = 0; op <= c * n; ++op) {
                dp[c + 1][op + tr] = max(dp[c + 1][op + tr], dp[c][op] + a[i]);
            }
        }
    }
    for (int i = 0; i <= k; ++i) {
        for (int op = 1; op <= 150 * i; ++op) {
            dp[i][op] = max(dp[i][op], dp[i][op - 1]);
        }
    }
//    for (int i = 0; i <= n * n; ++i) {
//        DBN(dp[k][i]);
//    }
    ...
    for (int i = 1; i <= k; ++i) {
        ...
        dp2[i][0] = dp2[i - 1][0] + a[k - i];
    }
    for (int i = k - 1; i >= 0; --i) {
        for (int c = k - 1; c >= 0; --c) {
            if (k - 1 - c < i) continue;
            int tr = k - 1 - c - i;
            for (int op = 0; op <= c * 150; ++op) {
                dp2[c + 1][op + tr] = min(dp2[c + 1][op + tr], dp2[c][op] + a[i]);
            }
        }
    }
    ...
    for (int i = 0; i <= k; ++i) {
        for (int op = 1; op <= n * n; ++op) {
            dp2[i][op] = min(dp2[i][op], dp2[i][op - 1]);
        }
    }
    k = n - k;
    int sum = 0;
    ...
    for (int i = 1; i <= min(k, n - k); ++i) {
        for (int op = 0; op <= n * i; ++op) {
            int left = s - op - i * i;
            if (left < 0) break;
            if (left >= n * n) left = n * n;
            int res = sum - dp[i][op] + dp2[i][left];
            if (res < ans) {
                ans = res;
            }
        }
