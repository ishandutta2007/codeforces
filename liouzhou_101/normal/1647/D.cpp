		...
		for (int i = 0; i < m; ++i)
		{
			...
			for (int c = k - 1; c >= 0; --c)
				for (int j = 0; j < m; ++j)
				{
					...
					int y = divisor[j];
					int extra = 0;
					while ((ll)y * divisor[i] <= x)
					{
						y *= divisor[i];
						extra += 1;
						if (c + extra > k) break;
						auto it = lower_bound(divisor.begin(), divisor.end(), y);
						if (it != divisor.end() && *it == y)
						{
							int& f = dp[c + extra][it - divisor.begin()] += dp[c][j];
							if (f >= 2) f = 2;
						}
					}
				}
			if (dp[k][m - 1] >= 2) return 2;
		}
		return dp[k][m - 1];
	};
