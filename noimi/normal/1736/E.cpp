...
int main() {
    {
        ...
        vvv(int, dp, n + 1, n, n + 2, -inf<int>);
        // vv(int, dp2, n + 1, n + 1, -inf<int>);
        ...
        rep(i, 1, n) {
            rep(j, n) rep(k, n + 1) {
                if(j != i) chmax(dp[i + 1][i][k + 1], dp[i][j][k] + a[i]);
                if(k) chmax(dp[i + 1][j][k], dp[i][j][k] + a[j]);
            }
            vi ma(n + 1, -inf<int>);
            rep(j, n) rep(k, n + 1) chmax(ma[k], dp[i][j][k]);

            rep(k, n + 1) {
                rep(r, i + 1, min(n, i + k + 1)) { chmax(dp[i + 1][r][k - (r - i) + 1], ma[k] + a[r]); }
            }
        }
