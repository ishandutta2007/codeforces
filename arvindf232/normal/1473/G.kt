...
//        val root_pw = 1 shl 20
        ...
        private const val maxPower = 14 // 262144
        ...
        fun getrootpow(level:Int, k:Int):Int {
            return quickpower[level][(k) shr (root_level - level)]
        }
...
//    solve.usetc()
    solve.cases{
        ...
        repeat(n){
            ...
            val b = getint
            val start = now.size
            val newsize = now.size + a - b
            assert(newsize >= 1)
            //want slice from b, with size newsize
            val atleast = b - (start -1)
            val atmost = b + newsize - 1
            val stuff = IntArray(atmost - atleast + 1){FACT.choose(a+b,it + atleast)}

//            just dei stuff.size

            now = FFT.fullconvolution(now,stuff).sliceArray(start-1 until (start-1)+newsize)
//            just dei now
