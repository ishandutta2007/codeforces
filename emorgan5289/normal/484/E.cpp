...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        q[(n-1)/2].pb({l, r, w, 0, n-1, i});
    }
    segtree<int, _sum, _max, 0, 0> s(n);
    for (int t = 0; t < m;) {
        one.clear(); one.insert(-1);
        for (int i = 0; i < n; i++)
            s.set(i, i, n-i);
        for (int i = 0; i < n; i++) {
            s.update(*--one.lower_bound(ord[i])+1, ord[i], -s.get(ord[i]));
            one.insert(ord[i]);
            while (!q[i].empty()) {
                auto [l, r, w, x, y, j] = q[i].back(); q[i].pop_back();
                if (s.query(l, r-w+1) >= w) x = i+1;
                else y = i;
                if (x == y) res[j] = a[ord[x]], t++;
                else q[(x+y)/2].pb({l, r, w, x, y, j});
            }
        }
    }
    for (int i = 0; i < m; i++)
