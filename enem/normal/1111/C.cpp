...
ll xd(ll n,ll x,ll &a,ll &b,map<ll,ll> &mp)
{
    if(n<0) return (1LL<<60);
    auto it1=mp.lower_bound(x);
    auto it2=mp.lower_bound(x+(1LL<<n));
    if(it1==it2) 
        return a;
    else return min(((*it2).S-(*it1).S)*b*(1LL<<n),xd(n-1,x,a,b,mp)+xd(n-1,x+(1LL<<(n-1)),a,b,mp));
}
