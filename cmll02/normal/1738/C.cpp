...
signed main()
{
	...
	for(int i=0;i<=100;i++)
		for(int j=0;j<=100;j++)
		{
			...
			//Alice choose 1
			if(i)dp[i][j][0][0]|=dp[i-1][j][1][1],dp[i][j][0][1]|=dp[i-1][j][1][0];
			if(j)dp[i][j][0][0]|=dp[i][j-1][1][0],dp[i][j][0][1]|=dp[i][j-1][1][1];
			if(i)dp[i][j][1][0]&=dp[i-1][j][0][0],dp[i][j][1][1]&=dp[i-1][j][0][1];
			if(j)dp[i][j][1][0]&=dp[i][j-1][0][0],dp[i][j][1][1]&=dp[i][j-1][0][1];
		}
	int T=read();
	while(T--)
	{
