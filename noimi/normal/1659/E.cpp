...
int main() {
    INT(n, m);
    auto g = getWg<int>(n, m);
    vector<UnionFind> uf(30, UnionFind(n));
    rep(i, n) fore(e, g[i]) { rep(j, 30) if(e.cost >> j & 1) uf[j].unite(i, e); }
    vector<UnionFind> uf2(30, UnionFind(n));
    rep(i, n) fore(e, g[i]) rep(j, 1, 30) {
        if(e.cost & 1 and e.cost >> j & 1) { uf2[j].unite(i, e); }
    }

    vv(int, ok, 30, n);
    rep(i, n) {
        int x = 0;
        fore(e, g[i]) if(~e.cost & 1) x = 1;
        if(x) { rep(j, 1, 30) ok[j][uf[j][i]] = true; }
    }
