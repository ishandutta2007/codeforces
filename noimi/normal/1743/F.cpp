...
template <class T, const int N> struct Matrix {
    ...
    bool operator!=(const Matrix &R) const {
        rep(i, N) rep(j, N) if((*this)[i][j] != R[i][j]) return true;
        return false;
    }
...
int main() {
    ...
    Matrix<mint, 2> mat0, mat1;
    mat0[0][0] = 3, mat0[0][1] = 1, mat0[1][0] = 0, mat0[1][1] = 2;
    mat1[0][0] = 1, mat1[0][1] = 1, mat1[1][0] = 2, mat1[1][1] = 2;

    vector<Matrix<mint, 2>> setter(N);
    rep(i, N) {
        if(inc(i, l[0], r[0])) {
            setter[i][0][0] = 0, setter[i][1][0] = 1;
        } else
            setter[i][0][0] = 1;
    }

    DualSegmentTree<M> seg(setter, f, e());

    rep(i, 1, n) {
        seg.update(l[i], r[i], mat1);
        seg.update(0, l[i], mat0), seg.update(r[i], N, mat0);
    }

    mint ans;
    rep(i, N) ans += seg[i][1][0];
