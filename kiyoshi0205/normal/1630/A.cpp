...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  if(K+1==N){
    if(N==4){
      ...
      pvec ans(N/2);
      rep(i,N/2)ans[i]=datas(i,N-1-i);
      swap(ans[0].first,ans.back().first);
      swap(ans[1].first,ans.back().second);
  ...
  pvec ans(N/2);
  rep(i,N/2)ans[i]=datas(i,N-1-i);
  if(K<N/2)swap(ans[0].first,ans[K].first);
  else swap(ans[0].first,ans[N-1-K].second);
  for(auto [x,y]:ans)print(x<<" "<<y);
}
}