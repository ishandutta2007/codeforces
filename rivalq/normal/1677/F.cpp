...
namespace algebra {
	...
	struct poly {
		...
		poly pow(size_t k, size_t n) { // calculate p^k(n) mod x^n
			...
			T j = a[i];
			poly t = div_xk(i) / j;
			return T(j).pow(k) * (t.log(n) * T(k)).exp(n).mul_xk(i * k).mod_xk(n);
		}
...
/*
* Problem is just to find sum((p^i) * i^k) fast enough
*
*/
...
int solve(){
 		...
 		for(int i = 1; i <= n; i++){
 			...
 			//	cont[i] += p.pow(j)*mod_int(j).pow(k);
 			//}
 		} 


 		vector<mod_int> vec,xcord(k + 1);
 		mod_int sum = 0;
 		for(int i = 0; i <= k; i++){
 			sum += mod_int(i).pow(k)*p.pow(i);
 			vec.push_back(sum);
 			xcord[i] = i;
 		}
 		mod_int res = 0;
 		for(int i = 0; i <= k; i++){
 			res += C(k + 1,i + 1)*(-p).pow(k - i)*vec[i];
 		}
 		res /= (-p + 1).pow(k + 1);
 		mod_int val = 1;
 		for(int i = 0; i <= k; i++){
 		...
 		auto polyn = inter(xcord,vec);
 		vector<mod_int> cont = polyn.eval(a);
 		for(int i = 1; i <= n; i++){
 			cont[i] = cont[i]*p.pow(a[i].val) + res;
 		}


 		vector<mod_int> pref_product(n + 1,1);
 		vector<mod_int> pref_sum(n + 1,1),inv_sum(n + 1,1);
 		for(int i = 1; i <= n; i++){
 			pref_product[i] = pref_product[i - 1]*(a[i] + 1);
 			pref_sum[i] = pref_sum[i - 1] + pref_product[i];
 			inv_sum[i] = inv_sum[i - 1] + pref_product[i].inv();
 		}
 		vector<mod_int> sum_1(n + 1),sum_2(n + 1);
 		for(int i = 1; i <= n; i++){
 			sum_1[i] = sum_1[i - 1] + (a[i]/(a[i] + 1))*(pref_sum[n] - pref_sum[i - 1]);
 			sum_2[i] = sum_2[i - 1] + (a[i]/(a[i] + 1))*(inv_sum[i - 1]); 
 		}
 		mod_int ans = 0;
 		for(int i = 1; i <= n; i++){
 			mod_int s1 = pref_sum[n] - pref_sum[i];
 			mod_int s2 = inv_sum[i - 1];
 			mod_int v = cont[i]/(a[i] + 1);
 			ans += (pref_sum[n] - pref_sum[i - 1])*(inv_sum[i - 1])*v;
 			v = cont[i]*(inv_sum[i - 1])/(a[i] + 1);
 			ans += v*(sum_1[n] - sum_1[i]);
 			v = cont[i]*(pref_sum[n] - pref_sum[i - 1])/(a[i] + 1);
 			ans += v*(sum_2[i - 1]);
 		}
