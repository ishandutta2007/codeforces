...
int main() {
	...
	vector<vector<vector<int>>> g(2, vector<vector<int>>(64));
	for (int bit = 0; bit < 2; bit++) {
		for (int from = 0; from < 64; from++) {
			...
			for (int mask = 0; mask < 8; mask++) {
				...
				int nmask = (bmask << 3) | dmask;
				g[bit][from].emplace_back(nmask);
			}
		}
	}

	vector<ll> dp(64, 0), ndp = dp;
	dp[0] = 1;
	for (int i = 0; i < n; i++) {
		const int bit = s[i] - '0';
		for (int mask = 0; mask < 64; mask++)
			for (auto& to : g[bit][mask])
				ndp[to] = (ndp[to] + dp[mask]) % MOD998;
