...
pub struct Fenwick<T: Integer> {
    data: Vec<T>,
}

impl<T: Integer> Fenwick<T> {
    ...
    pub fn ask_pref(&self, mut idx: usize) -> T {
        let mut res = T::ZERO;
        loop {
            res += self.data[idx];
            idx &= idx + 1;
    ...
    pub fn ask(&self, l: usize, r: usize) -> T {
        if l > r {
            ...
            self.ask_pref(r) - self.ask_pref(l - 1)
        }
    }
}
...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let mut divs: Vec<Vec<i32>> = vec![Vec::new(); N * 2 + 1];

    for i in 1..divs.len() {
        for j in (i..divs.len()).step_by(i) {
            divs[j].push(i as i32);
        }
    }

    let mut prs: Vec<(i32, i32)> = Vec::new();
    for k in 1..=N as i32 {
        // lcm = k
        {
            for &i in divs[k as usize].iter() {
                if i == k {
                    break;
                }
                for &j in divs[k as usize].iter() {
                    if j >= i {
                        break;
                    }
                    prs.push((j, k));
        ...
        // lcm = 2k
        {
            for &i in divs[k as usize * 2].iter() {
                if i == k {
                    break;
                }
                for &j in divs[k as usize * 2].iter() {
                    if j >= i {
                        break;
                    }
                    if (k % i != 0 || k % j != 0) && k * 2 < i + j + k {
                        prs.push((j, k));
                    }
                }
            }
    ...
    let t = cin.read();
    let mut ans: Vec<i64> = vec![0; t];
    let mut ques = Vec::new();
    for i in 0..t {
        let (l, r): (i32, i32) = cin.read();
        ans[i] = (r - l) as i64 * (r - l - 1) as i64 * (r - l + 1) as i64 / 6;
        ques.push(((l, r), i));
    }

    prs.sort_by_key(|&(_l, r)| r);
    ques.sort_by_key(|&((_l, r), _i)| r);

    let mut ind = 0;
    let mut tree = Fenwick::<i32>::new(N + 1);

    for ((l, r), i) in ques.into_iter() {
        while ind < prs.len() && prs[ind].1 <= r {
            tree.add(prs[ind].0 as usize, 1);
            ind += 1;
        }
        ans[i] -= tree.ask(l as usize, r as usize) as i64;
    }
