...
void solve(){
  ...
  int cntX[3] = {0,0,0};
  int cntO[3] = {0,0,0};
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(A[i][j] == 1) cntX[(i+j)%3]++;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(A[i][j] == 2) cntO[(i+j)%3]++;
  int sX = cntX[0]+cntX[1]+cntX[2];
  int sO = cntO[0]+cntO[1]+cntO[2];
  for(li kx = 0; kx < (li)(3); ++kx) for(li ky = 0; ky < (li)(3); ++ky) if(kx != ky && cntX[kx] + cntO[ky] <= (sX+sO)/3) {
    for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if((i+j)%3 == kx && A[i][j] == 1) {
      A[i][j] = 2;
    }
    for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if((i+j)%3 == ky && A[i][j] == 2) {
      A[i][j] = 1;
    }
    goto lend;
  }
