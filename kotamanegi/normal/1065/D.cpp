...
pair<int, int> dp[101][10][10][3];

void solve(){
    int n;
    cin >> n;
    vector<pair<int, int>> grid((int)(n * n), mp(0, 0));
    REP(i, n) {
        REP(q, n) {
            REP(j, 101) {
                REP(t,3)
                dp[j][i][q][t] = mp(1e9, 1e9);
            }
            int a;
            cin >> a;
            grid[a-1] = mp(i, q);
    ...
    REP(q, 3) {
        dp[0][grid[0].first][grid[0].second][q] = mp(0, 0);
    }
    REP(i, grid.size() - 1) {
        priority_queue<tuple<pair<int,int>, int, int, int>, vector<tuple<pair<int,int>, int, int, int>>, greater<tuple<pair<int,int>, int, int, int>>> nexts;
        REP(q, n) {
            REP(j, n) {
                REP(p, 3) {
                    nexts.push(make_tuple(dp[i][q][j][p], q, j, p));
                }
            }
        }
        while (nexts.empty() == false) {
            auto now = nexts.top();
            nexts.pop();
            if (dp[i][get<1>(now)][get<2>(now)][get<3>(now)] != get<0>(now)) continue;
            get<0>(now).second++;
            get<0>(now).first++;
            REP(q, 3) {
                if (dp[i][get<1>(now)][get<2>(now)][q] > get<0>(now)) {
                    dp[i][get<1>(now)][get<2>(now)][q] = get<0>(now);
                    nexts.push(make_tuple(get<0>(now), get<1>(now), get<2>(now), q));
                }
            }
            get<0>(now).second--;
            if (get<3>(now) == 0) {
                int xe[8] = { -1,-1,-2,-2,1,1,2,2 };
                int ye[8] = { -2,2,-1,1,-2,2,-1,1 };
                REP(t, 8) {
                    int x = xe[t] + get<1>(now);
                    int y = ye[t] + get<2>(now);
                    if (x >= 0 and x < n and y >= 0 and y < n) {
                        if (dp[i][x][y][0] > get<0>(now)) {
                            dp[i][x][y][0] = get<0>(now);
                            nexts.push(make_tuple(get<0>(now), x, y, 0));
                        }
                    }
                }
            }
                ...
                REP(t, 4) {
                    REP(p, n+1) {
                        int x = xe[t]*p + get<1>(now);
                        int y = ye[t]*p + get<2>(now);
                        if (x >= 0 and x < n and y >= 0 and y < n) {
                            if (dp[i][x][y][1] > get<0>(now)) {
                                dp[i][x][y][1] = get<0>(now);
                                nexts.push(make_tuple(get<0>(now), x, y, 1));
                            }
                        }
                        else break;
                    }
                ...
                REP(t, 4) {
                    REP(p, n + 1) {
                        ...
                        int y = ye[t] * p + get<2>(now);
                        if (x >= 0 and x < n and y >= 0 and y < n) {
                            if (dp[i][x][y][2] > get<0>(now)) {
                                dp[i][x][y][2] = get<0>(now);
                                nexts.push(make_tuple(get<0>(now), x, y, 2));
                            }
                        }
                        else break;
                    }
        ...
        REP(q, 3) {
            dp[i + 1][grid[i + 1].first][grid[i + 1].second][q] = dp[i][grid[i + 1].first][grid[i + 1].second][q];
        }
    }

    pair<int, int> ans = mp(1e9, 1e9);
    REP(q, 3) {
        ans = min(ans, dp[grid.size()-1][grid.back().first][grid.back().second][q]);
    }
