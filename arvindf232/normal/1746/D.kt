...
data class item(val i:Int, val b:Int):Comparable<item>{
...
//    solve.usetc()
    solve.cases{
        ...
        val cache2 = List(n){ (mutableMapOf<Int,Long>())}

        var calls = 0
        //        val children =
        fun solve(x:Int, count:Int):Long{
//            calls ++
...
//                return cache[p] !!
//            }
            if(count in cache2[x]){
                return cache2[x][count]!!
            }
            var base = count * L[x].toLong()
...
//                assert(h >= l )
            ...
            bonus.sortDescending()
            for(i in 0 until highcount.toInt()){
                base += bonus[i]
            }
            cache2[x][count] = base
            return base
