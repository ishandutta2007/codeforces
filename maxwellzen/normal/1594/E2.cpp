...
str s;
map<ll, int> m;
map<str, int> ctoi{{"white", 0}, {"yellow", 1}, {"green", 2}, {"blue", 3}, {"red", 4}, {"orange", 5}};
set<ll> recalc;
vector<al<6>> def;

al<6> calc(ll i, int h) {
    if (recalc.find(i) == recalc.end()) return def[h];
    al<6> ans = {0,0,0,0,0,0};
    if (h==0) {
        ans[m[i]] = 1;
        return ans;
    }
    al<6> a = calc(2*i, h-1), b = calc(2*i+1, h-1);
    F0R(p, 6) F0R(q, 6) if (p/2 != q/2) F0R(r, 6) if (r/2 != p/2) {
        ans[p] = (ans[p] + a[q]*b[r])%MOD;
    }
    if (m.find(i) != m.end()) F0R(p, 6) if (p != m[i]) ans[p]=0;
    return ans;
}
...
void solve() {
    ...
    def[0] = {1,1,1,1,1,1};
    FOR(i, 1, k) {
        def[i][0] = 16*def[i-1][0] % MOD * def[i-1][0] % MOD;
        FOR(j, 1, 6) def[i][j] = def[i][0];
    }
    al<6> ans = calc(1, k-1);
    ll sum = 0;
    F0R(i, 6) sum = (sum+ans[i])%MOD;
    cout << sum << nl;
}
