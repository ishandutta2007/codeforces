...
void solve() {
    ...
    while(true) {
        ...
        bool ok = 0;
        if(lnow + a[l] >= 0 or a[k] + lsum + rma + a[l] >= 0) {
            lnow = max(lnow + a[l], a[k] + lsum + rma + a[l]);
            lsum += a[l];
            -- l;
            chmax(lma, lsum);
            ok = 1;
        }
        if(rnow + a[r] >= 0 or a[k] + rsum + lma + a[r] >= 0) {
            rnow = max(rnow + a[r], a[k] + rsum + lma + a[r]);
            rsum += a[r];
            ++ r;
            chmax(rma, rsum);
            ok = 1;
