...
struct multi{
	string s;
	multi(ll x = 0){s = lltos(x);}
	multi(string str){
		reverse(all(str));
		nolead(str);
		s = str;
	}
	multi(const multi &ope) {s = ope.s;}
	multi operator=(ll x){return *this = multi(x);}
	multi operator=(string str){return *this = multi(str);}
	...
	static void nolead(string &s){while(sz(s) && s.back() == '0') s.pop_back();}
	void adjust(int p){
		while(1){
			if(s[p] - '0' >= 10){
				if(p+1 == sz(s)) s += "0";
	...
	multi operator+(const multi &ope){return multi(get()) += ope;}
	multi operator-(const multi &ope){return multi(get()) -= ope;}
	
	multi& operator+=(const multi &ope){
		while(sz(s) < sz(ope.s)) s += "0";
		reps(i, ope.s) s[i] += ope.s[i]-'0', adjust(i);
		nolead(s);
		return *this;
	}
	multi& operator-=(const multi &ope){
	...
	multi operator++(int n){ return *this += multi(1); }
	multi operator--(int n){ return *this -= multi(1); }
	
	bool operator ==(const multi &ope){return s == ope.s;}
	bool operator !=(const multi &ope){return s != ope.s;}
	bool operator <(const multi &ope){
		if(sz(s) < sz(ope.s)) return true;
		if(sz(s) > sz(ope.s)) return false;
		per(i, sz(ope.s)-1, 0){
			if(s[i] < ope.s[i]) return true;
			if(s[i] > ope.s[i]) return false;
		}
		return false;
	}
};
istream& operator >>(istream &is, multi &ope){
	string str; is >> str, ope = str;
	return is;
}
ostream& operator <<(ostream &os, multi &ope){
	return os << ope.get();
}
ostream& operator <<(ostream &os, const multi &ope){
	return os << ope.get();
}
