...
int main() {
	...
	INT(n, q);
	VEC(int, a, n);
	const int valmax = 1123456;
	map<int, int> rev_a;
	rep(i, n) rev_a[a[i]] = i;

	vector<int> eratos(valmax, 1), primes;
	rep(i, 2, valmax) {
		if(eratos[i]) {
			primes.push_back(i);
			rep(j, i + i, valmax, i) { eratos[j] = 0; }
		}
	}
	...
	foa(d, primes) {
		rep(i, d, valmax, d) {
			...
			if(rev_a.count(i)) gps[d].push_back(rev_a[i]);
		}
	}
	...
	rep(i, n) {
		...
		vector<int> gp_new_members;

		for(int &d : divisors[val]) {
			auto &t = gps[d];
			if(t.empty()) continue;
			int nxt = gp_number[t.front()];

			gp_new_members.push_back(nxt);
		}

		for(int &d : divisors[val - 1]) {
			auto &t = gps[d];
			if(t.empty()) continue;
			int nxt = gp_number[t.front()];
		...
		sort(all(gp_new_members));

		foa(t, gp_new_members) foa(s, gp_new_members) { ed.emplace(s, t); }
	}
