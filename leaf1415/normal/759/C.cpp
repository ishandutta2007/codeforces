...
ll n;
ll p[100005], x[100005], px[100005];
LazySegTree seg(17);
...
int main(void)
{
	...
	rep(i, 1, n) px[p[i]] = x[i];
	
	seg.init();
	rep(i, 0, n) seg.update(i, 0);
	
	rep(i, 1, n){
		if(x[i] == -1) seg.add(p[i], n, -1);
		else seg.add(p[i], n, 1);
		
		ll h = seg.query(n, n);
		ll ub = n, lb = -1, mid;
		while(ub-lb>1){
			mid = (ub+lb)/2;
			if(seg.query(mid, n) < h) lb = mid;
			else ub = mid;
		}
		if(lb == -1) cout << -1 << "\n";
		else cout << px[lb+1] << "\n";
		
		//rep(i, 0, n) cout<< seg.query(i, i) << " "; cout << endl;
	}
