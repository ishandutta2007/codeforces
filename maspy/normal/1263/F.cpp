...
void solve() {
  ...
  FOR(n, N - 1) {
    FOR(a, n + 1) {
      ll dp = DP[a][n];
      chmin(DP[n + 1][n], dp + hld1.dist(A[a], A[n + 1]));
      chmin(DP[a][n + 1], dp + hld2.dist(B[n], B[n + 1]));
    }
    FOR(b, n + 1) {
      ll dp = DP[n][b];
      chmin(DP[n + 1][b], dp + hld1.dist(A[n], A[n + 1]));
      chmin(DP[n][n + 1], dp + hld2.dist(B[b], B[n + 1]));
    }
  }
  FOR(i, N) FOR(j, N) DP[i][j] += hld1.dist(A[i], 0) + hld2.dist(B[j], 0);
  // FOR(n, N) print(DP[n]);
  ll best = INF;
  FOR(i, N) chmin(best, DP[i][N - 1]);
  FOR(i, N) chmin(best, DP[N - 1][i]);
