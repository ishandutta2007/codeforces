...
int n,b[444444],ind[444444],sq,cur,ans,szz,sz,m,L,R;
...
struct SAM
{
	int edge[444444][26],len[444444],fa[444444],cnt,last,pos[444444];
	void Init()
	{
		last=1;cnt=1;
		for (int i=0;i<n;i++)
...
int main()
{
	...
	while(m--)
	{
		...
		for (int i=0;i<=sq;i++)
		{
			for (int j=25;j>=0;j--)
			{
				if ('a'+j<=(int)q[i] || !sam.edge[cur][j]) continue;
				if (query(rt[sam.edge[cur][j]],L+i,R,1,sz)) 
				{
					ans=i;
					ch=j+'a';
				}
			...
			if (i==sq) break;
			if (!sam.edge[cur][q[i]-'a']) break;
			cur=sam.edge[cur][q[i]-'a'];
		}
		if (!~ans) printf("-1\n");
		else
		{
			for (int i=0;i<ans;i++) printf("%c",q[i]);
			printf("%c\n",ch);
		}
	}
	return 0;
