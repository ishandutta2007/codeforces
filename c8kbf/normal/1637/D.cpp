...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
        s += (n-2)*(a[i]*a[i]+b[i]*b[i]);
        ts += a[i]+b[i];
    }
    c[0][0] = true;
    for(_ i = 1; i <= n; ++i) {
        for(_ j = a[i]; j <= 10000; ++j) c[i][j] |= c[i-1][j-a[i]];
        for(_ j = b[i]; j <= 10000; ++j) c[i][j] |= c[i-1][j-b[i]];
    }
    for(_ i = 1; i <= 10000; ++i) if(c[n][i]) p = min(p, i*i+(ts-i)*(ts-i));
    writeln(s+p);
