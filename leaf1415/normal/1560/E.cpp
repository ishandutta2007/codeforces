...
int main(void)
{
	...
	vector<string> vec;
	rep(i, 0, 60) vec.push_back(lltos(1LL<<i));
	...
	while(T--){
		...
		reps(i, str) times[str[i]-'a'] = i+1;
		
		ll len = 0;
		rep(i, 0, 25) if(cnt[i] > 0) len += cnt[i] / times[i];
