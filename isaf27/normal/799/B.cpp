...
{   //*
    ...
    for (int i = 0; i < n; i++)
    {
        c[a[i] - 1].insert(make_pair(p[i], i));
        c[b[i] - 1].insert(make_pair(p[i], i));
    }
    int q;
    scanf("%d", &q);
    for (int i = 0; i < q; i++)
    {
        ...
        int ind = c[t].begin()->second;
        cout << p[ind] << " ";
        c[a[ind] - 1].erase(make_pair(p[ind], ind));
        if (b[ind] != a[ind])
            c[b[ind] - 1].erase(make_pair(p[ind], ind));
    }
    return 0;
}