...
int main(){
    ...
    auto calc = [&](auto&&f,int n,int m,int k)->mint{
        if(n<k)return 0;
        if(k<0)return 0;
        if(m==0){
            if(k==0){
        ...
        rep(x,n){
            rep(p,k+1){
                res += comb[n-1][x] * f(f,x,m-1,p) * f(f,n-1-x,m-1,k-p);
            }
        }
        return dp[n][m][k] = res;
    };
    ...
    rep(n,N+1){
        rep(m,M+1){
            rep(k,K+1){
                ...
                if(k!=0&&m+2*k-n>=8)continue;
                rep(x,n){
                    rep(p,k+1){
                        dp[n][m][k] += comb[n-1][x] * dp[x][m-1][p] * dp[n-1-x][m-1][k-p];
                    }
                }
                // if(dp[n][m][k]!=0&&k!=0){
                //     cerr << n << " " << m << " " << k << " " << m+2*k -n << endl;
                //     chmax(ttt,m+2*k-n);
                // }
            }
        }
