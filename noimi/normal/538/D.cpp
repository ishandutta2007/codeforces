...
main() {
    ...
    vv(char, ans, n * 2 - 1, n * 2 - 1, '.');
    ans[n - 1][n - 1] = 'o';
    vpi a;
    int mov[200][200] = {};
    rep2(ii, -n + 1, n - 1) {
        rep2(jj, -n + 1, n - 1) {
            if(!ii and !jj) continue;
            bool flag = true;
            rep2(i, T, T + n - 1) {
                rep2(j, T, T + n - 1) {
                    if(mp[i][j] == 2 and (mp[i + ii][j + jj] == 3 )) flag = false;
                }
            }
            if(flag) ans[n - 1 + ii][n - 1 + jj] = 'x';
            if(flag) rep2(i, T, T + n - 1) {
                    rep2(j, T, T + n - 1) if(mp[i][j] == 2) mov[i + ii][j + jj] = 1;
                }
        }
    }
    rep2(i, T, T + n - 1) {
        rep2(j, T, T + n - 1) {
            if(mp[i][j] == 1 and !mov[i][j]) {
                cout << "NO\n";
                return 0;
            }
        }
