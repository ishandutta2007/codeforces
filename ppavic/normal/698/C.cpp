...
int main(){
	...
	ld sm = 0;
	for(int i = 0;i < n;i++)
		printf("%.10Lf ", sol[i]), sm += sol[i];
	printf("\n");
	assert(sm > (ld)K - (ld)1e9);
	return 0;
}