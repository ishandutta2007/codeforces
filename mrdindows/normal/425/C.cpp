...
int main()
{
	...
	for (int i = 0; i < n; ++i)
	{
		if (posb.find(a[i]) == posb.end())
		{
			continue;
		}
		vector<int> &v = posb[a[i]];
		int best = 0;
		int best_pos = -1;
		bool f = false;
		for (int j = min(310,i); j >= 1; --j)
		{
			if (d[j] == -1) continue;
			if (v.back() > d[j])
			{
				best = j;
				best_pos = d[j];
				int pos = *upper_bound(all(v), best_pos);
				if (j + 1 > res)
				{
					int cost = (j + 1) * e + i + 1 + pos + 1;
					if (cost <= s)
					{
						res = j + 1;
					}
				}
				if (d[best+1] == -1 || d[best+1] > pos)
				{
					d[best+1] = pos;
				}
			}
