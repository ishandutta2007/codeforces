...
int main()
{
	...
	while (m--) {
		...
		int prevCur = -1;
		while (true) {
			int nextPos = x[cur] + len * dir;
			int nextCur = 0;
			int nextLen = 0;
			if (dir == 1) {
				auto it = upper_bound(x + cur, x + n, nextPos) - x;
				--it;
				if (it < 0) it = 0;
				nextCur = it;
				nextLen = len - abs(x[nextCur] - x[cur]);
			}
			else {
				auto it = lower_bound(x + 0, x + cur + 1, nextPos) - x;
				nextCur = it;
				nextLen = len - abs(x[nextCur] - x[cur]);
			}
			if (cur == nextCur && cur == prevCur) {
				cout << id[cur] + 1 << '\n';
				break;
			}
			if (nextCur == prevCur) { 
				int dlen = abs(x[nextCur] - x[cur]) * 2;
				nextLen %= dlen;
			}
			len = nextLen;
			prevCur = cur;
