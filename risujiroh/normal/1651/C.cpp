...
void main(int) {
  ...
  scan(a, b);
  i64 a0 = inf;
  for (int x : b) { chmin(a0, abs(x - a[0])); }
  i64 a1 = inf;
  for (int x : b) { chmin(a1, abs(x - a.back())); }
  i64 b0 = inf;
  for (int x : a) { chmin(b0, abs(x - b[0])); }
  i64 b1 = inf;
  for (int x : a) { chmin(b1, abs(x - b.back())); }
  i64 ans = a0 + a1 + b0 + b1;
  chmin(ans, abs(a[0] - b[0]) + abs(a.back() - b.back()));
  chmin(ans, abs(a[0] - b.back()) + abs(a.back() - b[0]));
  chmin(ans, abs(a[0] - b[0]) + a1 + b1);
  chmin(ans, abs(a[0] - b.back()) + a1 + b0);
  chmin(ans, abs(a.back() - b[0]) + a0 + b1);
  chmin(ans, abs(a.back() - b.back()) + a0 + b0);
  println(ans);
}
