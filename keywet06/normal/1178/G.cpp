...
int n, m, fa[N], dfn[N], r[N], idfn[N], tim;
...
struct node {
    long long a, b, t;
    node(long long a_ = 0, long long b_ = 0, long long t_ = 0)
        : a(a_), b(b_), t(t_) {}
    friend node max(node A, node B) {
        if (A.a < B.a) {
            B.t = std::min(B.t, A.t);
            if (A.b > B.b) B.t = std::min(B.t, (B.a - A.a) / (A.b - B.b) + 1);
            return B;
        } else {
            A.t = std::min(A.t, B.t);
            if (B.b > A.b) A.t = std::min(A.t, (A.a - B.a) / (B.b - A.b) + 1);
            return A;
        }
    }
...
void build(int now = 1, int l = 1, int r = n) {
    if (l == r) {
        t[now].max = t[now].min =
            node(a[idfn[l]] * b[idfn[l]], b[idfn[l]], LLONG_MAX);
        t[now].tag = 0;
        return;
    }
    build(lson, l, lmid);
...
long long query(int L, int R, int now = 1, int l = 1, int r = n) {
    if (R < l || r < L) return 0;
    update(now);
    if (L <= l && r <= R) return std::max(t[now].max.a, -t[now].min.a);
    pushdown(now);
...
int main() {
    ...
    std::cin >> n >> m;
    for (int i = 2; i <= n; ++i) std::cin >> fa[i], son[fa[i]].push_back(i);
    for (int i = 1; i <= n; ++i) std::cin >> a[i];
    for (int i = 1; i <= n; ++i) std::cin >> b[i];
    for (int i = 2; i <= n; ++i) a[i] += a[fa[i]], b[i] += b[fa[i]];
    for (int i = 1; i <= n; ++i) b[i] = std::abs(b[i]);
    init(1);
    build();
    for (int i = 1; i <= m; ++i) {
