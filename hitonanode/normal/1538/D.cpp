...
bool solve() {
    ...
    if (A == B and K == 1) return false;

    auto fa = sieve.factorize(A);
    auto fb = sieve.factorize(B);
    map<int, pint> mp;
    for (auto [p, da] : fa) mp[p].first = da;
    for (auto [p, db] : fb) mp[p].second = db;
    int loa = 0, lob = 0, hi = 0;
    for (auto [pr, pa] : mp) {
        auto [a, b] = pa;
        if (a > b) loa = 1;
        if (a < b) lob = 1;
