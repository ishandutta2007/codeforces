...
public:
    ...
    //S  [l, r)  O(1) 0-indexed
    inline pair<ll, ll> hash(int l, int r) const {
    ...
    inline pair<ll, ll> hash() const {
        return hash(0, (int) hash1.size() - 1);
    }

    //LCP (Longest Common Prefix)O( log N )
    inline int getLCP(int a, int b) const {
        int len = min((int) hash1.size() - a, (int) hash1.size() - b);
        int low = 0, high = len;
        while (high - low > 1) {
            int mid = (low + high) >> 1;
            if (hash(a, a + mid) != hash(b, b + mid)) high = mid;
            else low = mid;
        }
        return low;
    }
...
void solve(){
    ...
    vs a(n);
    rep(i,0,n-1)a[i]=s.substr(i,n-i);
    vl dp(n);
    rep(i,0,n-1)dp[i]=n-i;
    
    auto sa=suffix_array(s);
    vl num(n);
    rep(i,0,n-1)num[sa[i]]=i;
    ...
    rep(i,1,n-1){
        rep(j,0,i-1){
            ...
            ll len=rh.getLCP(i,j);
            chmax(dp[i],dp[j]+(n-i)-len);
            
        }
    }
   // for(auto x:dp)cerr<<x<<" ";cerr<<endl;
    ll ans=0;for(auto x:dp)chmax(ans,x);
    cout<<ans<<endl;
}
