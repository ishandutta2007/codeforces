...
vector<vec> g;
ll f(ll x,ll y,ll z){
    return (x*x + y*y+ z*z-x*y-y*z-x*z)/2;
}
...
signed main(){
    ...
    while(T--){
        ...
        rep(i,a){
            ...
            if(t != b){
                ll Y = y[t];
                int s = lb(z,(x[i]+y[t])/2);
                if(s!=c) chmin(ans,f(x[i],Y,z[s]));
                if(s) chmin(ans,f(x[i],Y,z[s-1]));
            }
            if(t){
                ll Y = y[t-1];
                int s = lb(z,(x[i]+y[t-1])/2);
                if(s!=c) chmin(ans,f(x[i],Y,z[s]));
                if(s) chmin(ans,f(x[i],Y,z[s-1]));
            }
            int s = lb(z,x[i]);
            if(s != c){
                ll Z = z[s];
                int tt = lb(y,(x[i]+Z)/2);
                if(tt!= b) chmin(ans,f(x[i],y[tt],Z));
                if(tt) chmin(ans,f(x[i],y[tt-1],Z));
            }
            if(s){
                ll Z = z[s-1];
                int tt = lb(y,(x[i]+Z)/2);
