...
inline void solve() {
	...
	while (_--) {
		...
		cin >> v;
		pair<int, int> seg = v[0].x;
		pair<int, int> cst = { v[0].y, v[0].y };
		bool is_poss_solo = true;
		int cst_solo = v[0].y;
		vector<int> ans(n);
		ans[0] = v[0].y;
		for (int i = 1; i < n; ++i) {
			ans[i] = 2 * INF;
			if (v[i].x.x <= seg.x && v[i].x.y >= seg.y) {
				ans[i] = min(ans[i], v[i].y);
			} 
			if (v[i].x.x <= seg.x && v[i].x.y <= seg.y) {
				ans[i] = min(ans[i], v[i].y + cst.y);
			}
			if (v[i].x.x >= seg.x && v[i].x.y >= seg.y) {
				ans[i] = min(ans[i], v[i].y + cst.x);
			}
			if (v[i].x.x >= seg.x && v[i].x.y <= seg.y) {
				ans[i] = min(ans[i], cst.y + cst.x);
			}
			if (v[i].x.x < seg.x) {
				seg.x = v[i].x.x;
				cst.x = v[i].y;
			...
			if (v[i].x.y == seg.y) {
				cst.y = min(cst.y, v[i].y);
			}
			if (v[i].x.x <= seg.x && v[i].x.y >= seg.y) {
				if (is_poss_solo) {
					cst_solo = min(cst_solo, v[i].y);
				}
				else {
					cst_solo = v[i].y;
				}
