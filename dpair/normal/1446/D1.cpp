...
int root, croot, ans, b[MAXN];


int buf[MAXN << 1 | 1], *p = buf + MAXN;

inline void work(const int x) {
    memset(buf, 31, sizeof(buf)); p[0] = 0;
    rep(i, 1, n) {
        if(a[i] == root) b[i] = b[i - 1] + 1;
        elif(a[i] == x) b[i] = b[i - 1] -1;
        else b[i] = b[i - 1];
        chmax(ans, i - p[b[i]]); chmin(p[b[i]], i);
    }
}

signed main(){
    read(n); rep(i, 1, n) ++ cnt[a[i] = read()];
    croot = 1; rep(i, 1, 100) {
        if(cnt[i] > cnt[root]) {
            root = i;
            croot = 1;
        } elif(cnt[i] == cnt[root]) ++ croot;
    }

    if(croot > 1) return print(n), 0;
    rep(i, 1, 100) if(i != root) work(i);
    print(ans);
}