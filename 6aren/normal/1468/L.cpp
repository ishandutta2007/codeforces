...
int32_t main() {
	...
	for (int i = 1; i <= n; i++) {
		cin >> a[i];
		vector<int> factor = Factor::factorize(a[i]);
    	for (int j = 0; j < factor.size(); j++) {
    		if (j == (int) factor.size() - 1 || factor[j] != factor[j + 1]) {
    			p[i].push_back(factor[j]);
    		}
    	}
    	if (p[i].size() == 1) {
    		int foo = p[i][0];
	...
	if (douP.size() * 2 <= k) {
		...
		vector<int> res;
		if (k % 2) res.push_back(baz);
		if (k % 2 && baz == -1) {
			cout << 0 << '\n';
			return 0;
		}
		map<int, int> done;
		if (baz != -1) for (int e : p[baz]) {
			done[e] = true;
			for (int i = 0; i < 2; i++) {
				res.push_back(id[e][i]);
			}
		}
		for (int e : douP) {
			if (done[e]) continue;
