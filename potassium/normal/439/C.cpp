...
int main() {
	...
	while (!o.empty() && odd) {
		odd--;
		ans.pb(vector<int> { o.back() } );
		o.pop_back();
	}
	
	if (odd) die();
	if ((int)o.size() % 2 == 1) die();
	
	while (!e.empty() && even) {
		even--;
		ans.pb(vector<int> { e.back() } );
		e.pop_back();
	}
	...
	ans.back().insert(ans.back().end(), all(o));
	ans.back().insert(ans.back().end(), all(e));
