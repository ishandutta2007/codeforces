...
int check(pair<int, int> a, pair<int, int> b) {
	int hoge = (a.first * b.first + a.second * b.second);
	if (hoge > 0) {
		return 1;
	}
	else {
...
void dfs() {
	...
	for (int i = 0; i < n; ++i) {
		if (ok[i] == 1) continue;
		pair<int, int> backs = Points[ans[ans.size()-2]];
		backs.first -= Points[ans.back()].first;
		backs.second -= Points[ans.back()].second;
		pair<int, int> nexts = Points[i];
		nexts.first -= Points[ans.back()].first;
		nexts.second -= Points[ans.back()].second;
		if (check(nexts, backs) == 1) {
			ans.push_back(i);
			ok[i] = 1;
			dfs();
			if (goout == true) return;
			ok[i] = 0;
			ans.pop_back();
		}
	}
