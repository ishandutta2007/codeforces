...
//			} else res += i + j - C;
//		}
//		For(j, max(B, C - i), min(D - i, C) ) {
//			res += i - C + j;
//		}
		int l = max(B, C - i), r = min(D - i, C);
		res += max(0ll, (r - l + 1) * (l + r) / 2 + (i - C) * (r - l + 1));
//		For(j, max(max(B, C - i), D - i + 1), C) {
//			res += D - C + 1;
//		}
		l = max(max(B, C - i), D - i + 1);
		res += max(0ll, (C - l + 1) * (D - C + 1));
	}
	cout << res << endl;
    return 0;
}