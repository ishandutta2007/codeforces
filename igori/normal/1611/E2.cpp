...
struct Tree{
    ...
    {
        L.mn_pref = min(L.mn_pref, L.total + R.mn_pref);
        L.total += R.total;
        //cout << "merge result = " << L.mn_pref << " " << L.total << "\n";
        return L;
    }
    void build(int l, int r, int v, vector<ll> &a)
    {
        ...
        if (l + 1 == r)
        {
            while (tree.size() <= v) tree.push_back({0, 0});
            tree[v] = {min(0ll, a[l]), a[l]};
            //cout << "set tree[" << v << "] = " << tree[v].mn_pref << " " << tree[v].total << "\n";
            return;
        }
        int m = (l + r) / 2;
        build(l, m, 2 * v + 1, a);
    ...
    {
        return get(l, r, 0, n, 0).mn_pref;
    }
};

void dfs(int v, int par, vector<int> &p, vvi &g)
{
    p[v] = par;
...
void solve()
{
    ...
    vector<int> q = {0};
    vector<int> were(n);
    for (auto e : x) were[e] = 1;

    int ans = 0;
    for (int turn = 0; q.size(); turn++)
    {
        vector<int> y;
        for (auto e : x)
        {
        ...
        //cout << "Friends :"; for (auto e : y) cout << e + 1 << " "; cout << "\n";
        x = y;
        ...
        for (auto v : q)
        {
            for (auto u : g[v])
            {
                if (u != p[v])
                {
                    //cout << "CONSIDER " << u + 1 << "\n";
                    if (were[u]) ans++;
                    else vis.push_back(u);
                }
            }
        }
        //for (auto e : vis) cout << e + 1 << " "; cout << "\n";
        q = vis;
