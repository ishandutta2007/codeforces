...
public class G {
	...
	void solve()
	{
		...
		// [n-2,a+b-2] C(a+b-2, a-1)
		long[] fk = enumFirstKind(n-2, mod);
		
		out.println(fk[A+B-2-1] * C(A+B-2, A-1, mod, fif) % mod);
	}
	
	public static int[][] stirlingNumberOfTheFirstKind(int n, int m, int mod)
	{
	...
	public long[] enumFirstKind(int n, int mod)
	{
		long[][] ps = new long[n][];
		for(int i = 0;i < n;i++){
			ps[i] = new long[]{i+1, 1};
		...
		for(int d = 1;d < n;d*=2){
			if(d <= 1024){
				for(int i = 0;i+d < n;i+=2*d){
					ps[i] = Arrays.copyOf(mulnaive(ps[i], ps[i+d]), Math.min(i+2*d, n)-i+1);
				}
			}else{
				for(int i = 0;i+d < n;i+=2*d){
					ps[i] = Arrays.copyOf(convoluteSimply(ps[i], ps[i+d], mod, 3), Math.min(i+2*d, n)-i+1);
				}
			}
		}
		return ps[0];
