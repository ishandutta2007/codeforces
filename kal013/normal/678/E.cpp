...
long double sol(int mask,int i,int n){
    int res=mask & (1 << i);
    if (res==0) return 0;
    if (mask==(1<<n)-1) dp[mask][i]=(i==0);
    if (dp[mask][i]!=-1) return dp[mask][i];
    
    dp[mask][i]=0.0;
    For(k,n){
        if ((1<<k)&(mask)) continue;
        dp[mask][i]=max(dp[mask][i],sol(mask|(1<<k),k,n)*p[k][i]+sol(mask|(1<<k),i,n)*p[i][k]);
    }
    return dp[mask][i];
}
...
void solve(){
    int n;
    read(n);
    For(i,n)For(j,n) read(p[i][j]);
    For(i,(1<<n)) For(j,n) dp[i][j]=-1;
    long double ans=0;
    For(i,n){
        
        ans=max(ans,sol(1<<i,i,n));
    }
