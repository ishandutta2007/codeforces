...
namespace Poly{
	...
	int N,W[3048576],rev[3048576],wi[3048576],fac[3048576],inv[3048576],I[3048576];
...
int n,m,mn,a[1002];
int main(){
	n=read(),m=read(),pre(1<<21),GetInv(1<<21),Getfac(1<<21);
	A.resize(1000001);mn=1145141;
	for(re int i=1;i<=n;++i)a[i]=read(),mn=min(mn,a[i]);
	for(re int i=1;i<=n;++i)A[a[i]-mn]=1;
	qpow(A,B,m);
	for(re int i=0;i<=1000000;++i)if(B[i])printf("%d ",i+mn*m);
}