...
const int C = 500001;
bool good(int x0, int x1, int y0, int y1, int r0, int r1, int x, int y)
{
	return (x >= x0) && (x <= x1) && (y >= y0) && (y <= y1) && (x - y >= r0) && (x - y <= r1);
}
bool good(int x0, int x1, int y0, int y1, int r0, int r1, pii p)
{
	return good(x0, x1, y0, y1, r0, r1, p.X, p.Y);
}
pii fa(const vpii& v, const int z)
{
	int x0 = 0, x1 = C, y0 = 0, y1 = C, r0 = -C, r1 = C;
	for (pii p : v)
	{
		x0 = max(x0, p.X - z);
		x1 = min(x1, p.X + z);
