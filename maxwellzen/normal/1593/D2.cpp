...
vpi pf;
set<int> consider;
void gen(int i, int cur) {
    if (i==pf.size()) {consider.insert(cur); return;}
    gen(i+1, cur);
    F0R(j, pf[i].S) {
        cur *= pf[i].F;
        gen(i+1, cur);
...
void test(int diff) {
    map<int, int> freq;
    F0R(k, n) freq[a[k]%diff]++;
    for (pi p : freq) if (p.S >= half) {
        ckmax(ans, diff);
    }
    vis[diff]=true;
}
...
void solve() {
    ...
    F0R(i, n) f[a[i]]++;
    half = n / 2;
    for (pi p : f) if (p.S >= half) {
        cout << -1 << nl;
        return;
    }
    ans = 1;
    F0R(j, n) F0R(i, j) if (a[i] != a[j]) {
        int test = abs(a[i] - a[j]);
        pf.clear();
        while (test>1) {
            int p = factor[test], cnt = 0;
            while (test%p==0) test /= p, cnt++;
            pf.pb(mp(p, cnt));
        }
        // debug(pf);
        gen(0, 1);
