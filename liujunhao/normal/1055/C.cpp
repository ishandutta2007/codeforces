...
int main()
{
	Read(la),Read(ra),Read(ta),Read(lb),Read(rb),Read(tb);
	LL g=__gcd(ta,tb);
	LL nowb=lb%g;
	LL nowa=la%g;
	if(nowb>=nowa)
	...
	LL tra=ra-la+nowa;
	LL trb=rb-lb+nowb;
	ans=max(0ll,min(min(tra-nowa+1,trb-nowb+1),min(tra-nowb+1,trb-nowa+1)));
	if(nowb<nowa)
		nowb+=g;
	tra=ra-la+nowa;
	trb=rb-lb+nowb;
	ans=max(ans,min(min(tra-nowa+1,trb-nowb+1),min(tra-nowb+1,trb-nowa+1)));
	printf("%lld\n",ans);
}