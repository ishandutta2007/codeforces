...
struct Solver {
  void div(const vi& a, vi& a0, vi& a1, int b) {
    for (int x : a) if (x&b) a1.pb(x^b); else a0.pb(x);
  }
  mint e2(int n) { return ex(2,n)-1;}
  mint f(const vi& a, int x, int b=1<<30) {
    if (sz(a) == 0) return 1;
    b >>= 1;
    if (!b) return ex(2,sz(a));
    vi a0, a1;
    div(a,a0,a1,b);
    if (x&b) {
      x ^= b;
      ...
      return f(a0,x,b) + f(a1,x,b) - 1;
    }
  }
  mint f2(const vi& s, const vi& t, int x, int b) {
    if (sz(s)+sz(t) == 0) return 1;
    b >>= 1;
    if (!b) return ex(2, sz(s)+sz(t));
    vi s0, s1, t0, t1;
    div(s,s0,s1,b);
    div(t,t0,t1,b);
    if (x&b) {
      x ^= b;
      return f2(s0,t1,x,b) * f2(s1,t0,x,b);
    } else {
      mint res = f2(s0,t0,x,b) + f2(s1,t1,x,b) - 1;
      res += e2(sz(s0))*e2(sz(s1));
      res += e2(sz(t0))*e2(sz(t1));
      return res;
    }
  }
  void solve() {
