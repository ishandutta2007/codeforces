...
void solve() {
    ...
    for(int g = 2; g < N; ++ g) {
        ...
        for(int i = g; i - g < N; i += g) {
            ...
            if(i - g + 1 < i - can) {
                // [i - g + 1, i - can)
                res += x * (sum[i - can] - sum[i - g + 1]);
            }
            // i - g < x and i - can <= x  and x < i
            int left = max(i - can, i - g + 1);
            ll cnt = sum[i] - sum[left];
            res += y * ((ll)i * cnt - (sum2[i] - sum2[left]));
        }
        chmin(ans, res);
    }
    out(ans);
