...
public:
	...
	static void generateTest(std::ostream& test) {
		auto n = 100000;
		test << n << "\n";
		uniform_int_distribution<int64_t> dist(1000000000000);
		static mt19937 mt;
		for (int i: range(n)) {
			test << dist(mt) << " ";
		}
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		//struct Result {
		//	map<int64_t, Z> counts;
		//	Z sum;
		//};

		Z ans;
		//cerr << "test" << endl;
		auto dfs = [&](auto& dfs, int v, int p, const vector<pair<int64_t, int>>& stack) -> void {
			//stack.push_back({x[v], 1});
			//cerr << v << endl;
			vector<pair<int64_t, int>> new_stack;
			for (auto p: stack) {
				int64_t new_value = gcd(p.first, x[v]);
				if (new_stack.empty() || new_stack.back().first != new_value) {
					new_stack.push_back({new_value, p.second});
				} else {
					new_stack.back().second += p.second;
				}
			}
			if (new_stack.empty() || new_stack.back().first != x[v]) {
				new_stack.push_back({x[v], 1});
			} else {
				new_stack.back().second += 1;
			};
			for (auto p: new_stack) {
				//cerr << p.first << ' ' << p.second << endl;
				ans += p.first * p.second;
			}
			//stack = move(new_stack);
			for (int to: g[v]) {
				if (to == p) {
					continue;
				}

				dfs(dfs, to, v, new_stack);
			}
