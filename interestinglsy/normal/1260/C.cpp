...
// Read
struct InputReader{
	static const int MAXSZ = 1048576;
...
#undef Eb
#define Eb Pb
#define Forin_(var,container,iter) \
__typeof(container.begin()) iter = container.begin(); \
if(!container.empty()) \
	for( __typeof(container.back()) var = *iter ; \
		 iter != container.end() ; \
		 ++iter, var = *iter )
#define Forin(var,container) Forin_(var,container,iter##var##container)
