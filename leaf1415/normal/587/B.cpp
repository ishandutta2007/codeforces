...
int main(void)
{
	...
	cin >> n >> l >> k;
	rep(i, 0, n-1) cin >> a[i], p[i] = P(a[i], i);
	sort(p, p+n);
	
	vector<vector<mint> > dp(k+1);
	rep(i, 0, k) dp[i].resize(n);
	
	rep(i, 0, k) rep(j, 0, n-1) dp[i][j] = 0;
	dp[0][0] = 1;
	
	rep(i, 1, k){
		sum[0] = dp[i-1][0];
		rep(j, 1, n-1) sum[j] = sum[j-1] + dp[i-1][j];
		ll r = 0;
		rep(j, 0, n-1){
			while(r+1 < n && p[r+1].first <= p[j].first) r++;
			dp[i][j] = sum[r];
		}
	}
	//rep(i, 0, k) outl(dp[i]);
	...
	rep(i, 1, min(l/n, k)){
		mint tmp = 0;
		rep(j, 0, n-1) tmp += dp[i][j];
		ans += tmp * mint(l/n+1-i);
	}
	rep(i, 1, min(l/n+1, k)){
		mint tmp = 0;
		rep(j, 0, n-1) if(p[j].second < l%n) tmp += dp[i][j];
		ans += tmp;
	}
	outl(ans);
