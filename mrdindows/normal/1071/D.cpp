...
vector<int> g[F * 2];
vector<vector<int>> w;
void addUsed(vector<int>& v, vector<int> t) {
    auto x = calc(t);
    if (!used.count(x)) {
...
int solve() {
    ...
    for (int i = 2; i < N; ++i) {
        ...
        x /= pr[x];
        vec[i].reserve(vec[x].size() + 1);
        for (int t : vec[x]) vec[i].push_back(t);
        vec[i].push_back(cnt);
        auto h = calc(vec[i]);
        int& id = used[h];
        if (!id) {
    ...
    int ws = (int) w.size();
    for (int i = 0; i < min((int)w.size(), F); ++i) {
        vector<int> cur = w[i];
