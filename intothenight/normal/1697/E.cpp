...
struct combinatorics{
	...
	T P(int n, int k) const{
		return n < k ? T(0) : fact[n] * invfact[n - k];
	}
	// O(1)
...
int main(){
	...
	for(auto i = 0; i < n; ++ i){
		...
		dist[i][i] = numeric_limits<int>::max();
		mindist[i] = ranges::min(dist[i]);
	}
	vector<int> vis(n);
	...
	for(auto i = 0; i < n; ++ i){
		...
		vector<int> clique{i}, inclique(n, false);
		inclique[i] = true;
		int objd = mindist[i];
		vector<modular> dp_next(n + 1);
		for(auto j = 0; j < n; ++ j){
			if(dist[i][j] == objd){
				clique.push_back(j);
				inclique[j] = true;
		...
		int m = (int)clique.size();
		for(auto i = 0; i < (int)clique.size(); ++ i){
			for(auto j = i + 1; j < (int)clique.size(); ++ j){
				if(dist[clique[i]][clique[j]] != objd){
					goto FAIL;
				}
			}
		}
		...
		for(auto x = 0; x < n; ++ x){
			dp_next[x + 1] = dp[x] * (n - x);
		}
		for(auto x = 0; x <= n - m; ++ x){
			dp_next[x + m] += dp[x] * C.P(n - x, m);
		}
		dp = dp_next;
		continue;
		FAIL:;
	...
	cout << accumulate(dp.begin(), dp.end(), modular(0)) << "\n";
	return 0;
}
