...
int main() {
	...
	{
		const int MAXD = n;
		vector<vector<Mint>> dp(n, vector<Mint>(MAXD + 1, Mint(1, MOD107)));
		for (int u = 0; u < n; u++)
			dp[u][0] = 0;
	...
	vector<Mint> num(n + 1, Mint(0, MOD107));
	for (int i = 0; i <= n; i++)
		num[i] = D - i;

	vector<Mint> ps = num, ss = num;
	for (int i = 1; i <= n; i++)
		ps[i] *= ps[i - 1];
	for (int i = n - 1; i >= 0; i--)
		ss[i] *= ss[i + 1];

	vector<Mint> fact(n + 1, Mint(1, MOD107)), inv = fact;
	for (int i = 2; i <= n; i++)
		fact[i] = fact[i - 1] * i;
	inv[n] = fact[n].inv();
	for (int i = n; i >= 2; i--)
	...
	Mint ans(0, MOD107);
	for (int i = 0; i <= n; i++) {
		Mint basis = (i - 1 >= 0 ? ps[i - 1] : Mint(1, MOD107)) * (i + 1 <= n ? ss[i + 1] : Mint(1, MOD107));
		Mint norm = inv[i] * inv[n - i] * ((n - i) % 2 == 0 ? 1 : -1);
