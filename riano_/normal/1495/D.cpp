...
vector<bool> uni(401,true);
//BFS
vector<ll> bfs(graph &g,int s,int mode = 0){
    ll N = g.G.size();
    queue<int> go;
    ...
    while(!go.empty()){
        ...
        for(auto q:g.G[s]){
            ...
            if(dist[x]<=dist[s]+1){
                if(mode==1&&dist[x]==dist[s]+1) uni[x] = false;
                continue;
            }
            dist[x] = dist[s] + 1;
            g.par_v[x] = s;
...
vector<ll> shortest_v(graph &g,int goal,int start = 1){
    int now = goal;
    vector<ll> path;
    while(now!=-1){
        //cout << now << endl;
...
int main(){
    ...
    for(int i=1;i<=N;i++){
        rep(j,N+1) uni[j] = true; 
        auto d = bfs(G,i,1);
        for(int j=1;j<=N;j++){
            if(j<i) continue;
            if(!uni[j]) continue;
            d = bfs(G,i,1);
            auto v = shortest_v(G,j,i);
            mint ptn = 1;
            auto d2 = bfs(G,j);
            vector<ll> in(N+1,0);
            for(ll x:v) in[x]++;
            rep(k,M){
                if(d[a[k]]>d[b[k]]) swap(a[k],b[k]);
                if(d[a[k]]+1==d[b[k]]&&d2[a[k]]+1==d2[b[k]]) in[b[k]]++;
            }
            rep(k,N){
                //cout << in[k+1] << " ";
                ptn *= mint(in[k+1]);
            ...
            // cout << endl;
            //cout << i << " " << j << " " << ptn << endl;
            ans[i-1][j-1] = ptn; ans[j-1][i-1] = ptn;
        }
    }
