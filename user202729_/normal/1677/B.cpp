...
void up(){
	...
	std::string s; s.reserve(numRow*numCol); std::cin>>s;
	assert(int(s.size())==numRow*numCol);

	std::vector<int> numGoodRow(numCol);
	std::vector<char> lastIsGood(numCol);
	int rowSum=0;
	std::vector<char> colIsGood(numCol);
	// not really what their name implies, but anyway
	int numGoodCol=0;

	for(int const row: range(0, numRow)){
		for(int const col: range(0, numCol)){
			auto ch=s[row*numCol+col]-'0';
			if(ch and not colIsGood[col]){
				colIsGood[col]=true;
				numGoodCol+=1;
			}

			rowSum-=lastIsGood[col];
			lastIsGood[col]=ch;
			rowSum+=lastIsGood[col];

			numGoodRow[col]+=rowSum!=0;
			std::cout<<numGoodRow[col]+numGoodCol<<' ';
		}
	}
	std::cout<<'\n';
