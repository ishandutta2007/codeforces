...
int cmp(R a, R b) {
  R d = (a - b) / max<R>(abs(a) + abs(b), 1);
  return (d > +Eps) - (d < -Eps);
}

R area(P oa, P ob, P p, P q) {
  R res = p.cross(q) / 2;
  {
    R t = (ob - oa).dist() / 2;
    p = linearTransformation(oa, ob, P(+t, 0), P(-t, 0), p);
    q = linearTransformation(oa, ob, P(+t, 0), P(-t, 0), q);
    oa = P(+t, 0);
    ob = P(-t, 0);
  }
  R a = ((p - oa).dist() + (p - ob).dist()) / 2;
  R b = sqrt(a * a - oa.x * oa.x);
  auto f = [&](R th) -> R {
    return a * b / 2 *
           (th - atan((b - a) * sin(2 * th) / (b + a + (b - a) * cos(2 * th))));
  };
  res += f(q.angle()) - f(p.angle());
  res -= p.cross(q) / 2;
  return res;
}
...
int main() {
  ...
  auto go = [&](auto&& q) {
    vector<R> suff(2 * n + 1);
    for (int i : Per(2 * n))
      suff[i] = (p[(i + 1) % n] - p[i % n]).dist() + suff[i + 1];
    R perimeter = suff[0] - suff[n];

    for (int i : Rep(n)) {
      P a = p[i], b = p[(i + 1) % n];
      P e = (b - a).unit();
      auto check = [&](R mid) -> bool {
        P c = b + e * mid;
        auto f = [&](P d) -> R {
          return (a - c).dot(d - c) / ((a - c).dist() * (d - c).dist());
        };
        int l = i + 1, r = n + i;
        while (r - l > 2) {
          int ml = (l + l + r) / 3;
          int mr = (l + r + r) / 3;
          if (f(p[ml % n]) < f(p[mr % n]))
            r = mr;
          else
            l = ml;
        }
        P d = p[(l + 1) % n];
        return perimeter - (suff[i + 1] - suff[l + 1]) + mid + (d - c).dist() <=
               len;
      };
      R ok = 0, ng = 1;
      while (check(ng)) ng *= 2;
      while (nextafter(ok, ng) != ng) {
        R mid = (ok + ng) / 2;
        (check(mid) ? ok : ng) = mid;
      }
      q.push_back(b + e * ok);
  ...
  vector<P> q;
  {
    q.reserve(2 * n);
    go(q);
    for (auto&& e : p) e.y = -e.y;
    reverse(ALL(p));
    go(q);
    reverse(ALL(p));
    for (auto&& e : p) e.y = -e.y;
    reverse(begin(q) + n, end(q) - 1);
    for (int i : Rep(n, 2 * n)) q[i].y = -q[i].y;
  }

  vector<int> order(2 * n);
  iota(ALL(order), 0);
  sort(ALL(order), [&](int i, int j) {
    return (q[i] - p[0]).angle() < (q[j] - p[0]).angle();
  });

  R ans = 0;
  for (int z : Rep(2 * n)) {
    int l = 2 * n + z;
    while (order[l % (2 * n)] >= n) --l;
    int r = z + 1;
    while (order[r % (2 * n)] < n) ++r;
    ans += area(p[(order[l % (2 * n)] + 1) % n], p[order[r % (2 * n)] - n],
                q[order[z]], q[order[(z + 1) % (2 * n)]]);
  }
  cout << ans << '\n';
}