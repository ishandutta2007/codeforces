...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		sort(columns.begin(), columns.end(), [&](int a, int b) {
			auto mx = [&](int c) {
				int v = 0;
				for (int i: range(n)) {
					updateMax(v, matrix[i][c]);
				}
				return v;
			};
			return mx(a) > mx(b);
		...
		for (int i: range(n)) {
			for (int j: range(m)) {
				newMatrix[i][j] = matrix[i][columns[j]];
			}
		}

		int ans = 0;
		vector<int> sh(m);
		auto rec = [&](auto& rec, int j) -> void {
			if (j == m) {
				int sum = 0;
				for (int i: range(n)) {
					int cur_mx = 0;
					for (int j: range(m)) {
						updateMax(cur_mx, newMatrix[(i + sh[j]) % n][j]);
					}
					sum += cur_mx;
				}
				updateMax(ans, sum);
