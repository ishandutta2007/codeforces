...
vi solve() {
    ...
    vi ans{};
    auto process = [&](vi &cand) {
        if (sz(cand) > sz(ans))
            ans = cand;
    };

    auto trim_leading = [](vi &a) {
        int to_erase = 0;
        while (to_erase < sz(a) - 1 && a[to_erase] == 0)
            to_erase++;
        a.erase(a.begin(), a.begin() + to_erase);
    };

    int rem = accumulate(all(s), 0) % 3;
...

    {
        ...
        if (sz(pos) >= 2) {
            ...
            auto copy = s;
            copy.erase(copy.begin() + v);
            copy.erase(copy.begin() + u);
