...
public class E2 {
	...
	void solve()
	{
		...
		long ans = 0;
		for(int i = 0;i < K;i++){
			Node argmin = root.argmin;
			int ind = index(argmin);
			ans += argmin.v;
			if(ind == 0){
				root = split(root, 2)[1];
			}else if(ind == count(root)-1){
				root = split(root, count(root)-2)[0];
			}else{
				Node[] lm_r = split(root, ind+2);
				Node[] l_m = split(lm_r[0], ind-1);
				argmin.v = get(l_m[1], 0).v + get(l_m[1], 2).v - argmin.v;
				disconnect(argmin);
				root = merge(merge(l_m[0], argmin), lm_r[1]);
			}
		}
		out.println(ans);
	}
	...
	public static Node update(Node a)
	{
		...
		a.argmin = a;
		if(a.left != null && a.left.argmin.v < a.argmin.v){
			a.argmin = a.left.argmin;
		}
		if(a.right != null && a.right.argmin.v < a.argmin.v){
			a.argmin = a.right.argmin;
		}
