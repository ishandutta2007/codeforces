...
void solve() {
	...
	for (int i = n - 1; i >= 0; --i) {
		...
		for (auto [x, cou] : curr) {
			int k = ceil_div(v, x);
			debug(imie(x), imie(k), imie(cou));
			(ans += (k - 1ll) * cou % mod * (i + 1)) %= mod;
			next.emplace_back(v / k, cou);
		}
		next.emplace_back(v, 1);
		debug(imie(next));
		curr.clear();
		for (auto [x, cou] : next) {
			if (curr.empty() || curr.back().first < x) curr.emplace_back(x, cou);
			else curr.back().second += cou;
		}
	}
	printf("%lld\n", ans);
}
