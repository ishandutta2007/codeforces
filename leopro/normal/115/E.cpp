...
struct segment_tree {
    ...
    explicit segment_tree(int n) : sz(1) {
        while (sz < n) sz <<= 1;
        dp.assign(sz * 2, {0, 0});
//        for (int i = sz + 1; i < sz + n; ++i) dp[i] = data(-1e18, -1e18);
    }
...
void solve() {
    ...
    vector<vector<race>> by_right(n);
    for (auto r : races) by_right[r.r].push_back(r);
    for (auto &row : by_right) stable_sort(row.rbegin(), row.rend(), [](race x, race y) { return x.l < y.l; });
    ...
    for (int i = 0; i < n; ++i) {
        dp.set(ans, i);
        dp.add(-cost[i], {0, i + 1});
        for (auto race : by_right[i]) {
            dp.add(race.p, {0, race.l + 1});
        }
        ans = max(ans, dp.max({0, i + 1}));
    }
    cout << ans << '\n';
}