...
ll n, k;
vector<ll> G[200005];
ll sum[200005], dif[200005], ans[200005];
bool b[200005];
...
int main(void)
{
	...
	rt.calc();
	rep(i, 1, n) ans[i] -= rt.ans[i];
	
	ll mn = inf, mn_i;
	rep(i, 1, n){
		if(mn > ans[i]) mn = ans[i], mn_i = i;
	}
	outl(mn_i);
	outl(mn);
