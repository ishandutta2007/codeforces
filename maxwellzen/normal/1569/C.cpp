...
vll fact(2, 1), ifact(2, 1);
int nxt_uncalculated = 2;
void factinit(int n, ll m=MOD) {
    if (nxt_uncalculated>n) return;
    fact.rsz(n+1); ifact.rsz(n+1);
    FOR(i, nxt_uncalculated, n+1) fact[i]=(fact[i-1]*i)%m, ifact[i]=inv(fact[i], m);
    nxt_uncalculated = n+1;
}
ll choose(ll n, ll k, ll m=MOD) {
    if (k<0 || k>n) return 0;
    if (n>=nxt_uncalculated) factinit(n, m);
...
void solve() {
    ...
    int val = a.end()[-2], cnt = 0;
    F0R(i, n) if (a[i]==val) cnt++;
    ll ans = (fact[n] * inv(cnt+1))%MOD;
    ans = (fact[n]-ans)%MOD;
    if (ans<0) ans += MOD;
    cout << ans << nl;
}
