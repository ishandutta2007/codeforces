...
void solve() {
    ...
    ll ans = 0;
    F0R(i, N) {
        ans += seg3.query(0, A[i]) * A[i] - seg4.query(0, A[i]) + seg1.query(0, MX-1) + A[i] * seg2.query(A[i], MX-1);
        //dbg(seg1.query(0, MX-1), seg2.query(A[i], MX-1), seg3.query(0, A[i]), seg4.query(0, A[i]));
        for(int j = 1; j * A[i] < MX; j++) {
            int X = j * A[i];
            int Y = X + A[i] - 1;
            ckmin(Y, MX-1);
            ans -= A[i] * seg2.query(X, Y) * j;
            seg4.update(X, A[i] * j);
            seg4.update(Y+1, -1 * A[i] * j);
        }
        seg2.update(A[i], 1);
        seg1.update(A[i], A[i]);
        seg3.update(A[i], 1);
