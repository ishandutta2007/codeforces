...
signed main(){
    ...
    auto hunt=[&](ll x){
        ...
        ll l=LINF,r=LINF;
        each(i,cost){
            chmin(r,i.second+abs(i.first-a[x].front())+a[x].back()-a[x].front());
            chmin(l,i.second+abs(i.first-a[x].back())+a[x].back()-a[x].front());
        }
        cost={{a[x].front(),l},{a[x].back(),r}};
    };
    auto climb=[&](){
        vector<pll>ans;
        each(i,cost){
            auto p=safe.lower_bound(i.first);
            if(p==safe.end()){
                p--;
                ans.emplace_back(*p,i.second+i.first-*p+1);
                continue;
            }
            if(*p==i.first){
                ans.emplace_back(i.first,i.second+1);
                continue;
            }
            ans.emplace_back(*p,i.second+*p-i.first+1);
            if(p!=safe.begin()){
                p--;
                ans.emplace_back(*p,i.second+i.first-*p+1);
                continue;
    ...
    ll ans=LINF;
    each(i,cost)chmin(ans,i.second);
    out(ans);
}