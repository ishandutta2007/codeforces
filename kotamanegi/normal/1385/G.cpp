...
void solve(){
    ...
    REP(i, n) {
        int a;
        cin >> a;
        inputs.push_back(mp(a, 0));
    }
    REP(i, n) {
        cin >> inputs[i].second;
    }
    REP(i, n) {
        vertexs[inputs[i].first].push_back(i);
        vertexs[inputs[i].second].push_back(i);
    }
    REP(i, n) {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        if (vertexs[i][0] == vertexs[i][1]) continue;
        vector<pair<int, int>> calcs;
        calcs.push_back(mp(vertexs[i][0], 0LL));
        int now = vertexs[i][1];
        int color = i;
        while (now != vertexs[i][0]) {
            int flags = 0;
            if (color == inputs[now].first) {
                if (inputs[calcs.back().first].first == color) {
                    swap(inputs[now].first, inputs[now].second);
                    flags = 1;
                }
            }
            else if(color == inputs[now].second){
                if (inputs[calcs.back().first].second == color) {
                    swap(inputs[now].first, inputs[now].second);
                    flags = 1;
                }
            }
            visited[color] = 1;
            calcs.push_back(mp(now, flags));
            int target = 0;
            if (color == inputs[now].first) {
                target = inputs[now].second;
            }
            else {
                target = inputs[now].first;
            }
            color = target;
            for (auto x : vertexs[color]) {
                if (x != now) {
                    now = x;
                    break;
                }
        ...
        if (cnts < (int)calcs.size() - cnts) {
            REP(q, calcs.size()) {
                if (calcs[q].second == 1) {
                    ans.push_back(calcs[q].first);
                }
            }
        }
        else {
