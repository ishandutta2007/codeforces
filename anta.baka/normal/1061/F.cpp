...
bool ask(int u, int m, int v) {
    cout << "? " << u + 1 << ' ' << m + 1 << ' ' << v + 1 << endl;
    string ans; cin >> ans;
    return (ans[0] == 'Y');
}
...
main() {
    ...
    for(int rt : p) {
        int tot = 0;
        for(int v : p) tot += ask(rt, v, u);
        if(tot - 1 == sz(p) / 2) return cout << "! " << rt + 1 << endl, 0;
    }
}