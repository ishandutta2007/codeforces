...
S f(S x, S y) { return S{x.len + y.len, x.x + y.x, x.y + y.y}; }
S e() { return S{0, 0, 0}; }
struct T {
    bool del_front;
    mint x, y;
...
S g(T x, S y) {
    dump(x.del_front, x.x, x.y);
    dump(y.len, y.x, y.y);
    if(x.del_front) return S{y.len, y.len * x.x, y.len * x.y};
    return S{y.len, y.x + y.len * x.x, y.y + y.len * x.y};
}
T h(T x, T y) {
    dump(x.del_front, x.x, x.y);
    dump(y.del_front, y.x, y.y);
    if(x.del_front) { return x; }
    y.x += x.x, y.y += x.y;
    return y;
}
T id() { return T{false, 0, 0}; }
...
int main() {
    ...
    xs.eb(0);
    UNIQUE(xs);
    vector<S> setter(si(xs) - 1);
    rep(i, si(xs) - 1) {
        if(xs[i + 1] <= a[0])
            setter[i] = S{xs[i + 1] - xs[i], xs[i + 1] - xs[i], 0};
        else
            setter[i] = S{xs[i + 1] - xs[i], 0, 0};
    }

    lazy_segtree<S, f, e, T, g, h, id> seg(setter);
    rep(i, 1, n) {
        auto p = seg.all_prod();
        swap(p.x, p.y);
        seg.apply(0, lb(xs, a[i]), T{false, p.x, p.y});
        dump(lb(xs, a[i]), si(xs) - 1);
        seg.apply(lb(xs, a[i]), si(xs) - 1, T{true, 0, 0});
        rep(j, si(xs) - 1) dump(seg[j].len, seg[j].x, seg[j].y);
        dump(p.x, p.y);
    }
    auto p = seg.all_prod();
    dump(p.x, p.y);
