...
int32_t main() {
  ...
  REP (tr, 2) {
    ...
    newts[tr].PB(cur);
    RE (i, ziom) {
      cur = cur * invMod(i, P) % P * (ziom - i + 1) % P;
      newts[tr].PB(cur);
    }
    assert(cur == 1);
    debug(tr, newts[tr]);
  ...
  reverse(ALL(newts[1]));
  FFT fft(n / 2 + 2);
  VI mul = fft.mul_exact(newts[0], newts[1], P);
  int res = 0;
  debug(mul);
  FOR (h, 0, SZ(newts[0]) + SZ(newts[1]) - 2) {
    int diff = cnt[{'b', 0}] - cnt[{'b', 1}] - cnt[{'?', 1}] + h;
