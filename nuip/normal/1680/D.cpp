...
ll solve_uso(const vector<ll> orig, ll d){
	auto vs=orig;
	cauto sum=accumulate(all(vs),0ll);
	cauto cnt=count(all(vs),0);
	cauto rem=cnt*d-sum;
	if(rem<0) return -1;
	auto remp=rem/2;
	auto remn=sum+rem/2;
...
ll solve(const vector<ll> &vs, ll d){
	cauto n=(int)vs.size();
	cauto sum=accumulate(all(vs),0ll);
	ll re=-1;
	vector<ll> sums(n+1);
	rep(i,n) sums[i+1]=sums[i]+vs[i];
	vector<int> cnts(n+1);
	rep(i,n) cnts[i+1]=cnts[i]+!vs[i];
	rep(mx,n+1)rep(mn,mx){
		cauto cntn=cnts[mn]+cnts[n]-cnts[mx];
		cauto cntp=cnts[mx]-cnts[mn];
		cauto ex=min(cntp*d, cntn*d-sum);
		if(ex<0) continue;
		cauto mnv=sums[mn]-min(cnts[mn]*d,ex+sum);
		cauto mxv=mnv-sums[mn]+sums[mx]+ex;
		out(mn,mx,mnv,mxv,cntn,cntp,1);
		MX(re,mxv-mnv+1);
	}
	rep(mn,n+1)rep(mx,mn){
		cauto cntn=cnts[mn]-cnts[mx];
		cauto cntp=cnts[mx]+cnts[n]-cnts[mn];
		cauto ex=min(cntp*d, cntn*d-sum);
		if(ex<0) continue;
		cauto mxv=sums[mx]+ex;
		cauto mnv=mxv-sums[mx]+sums[mn]-sum-ex;
		out(mn,mx,mnv,mxv,cntn,cntp,ex,1);
		MX(re,mxv-mnv+1);
	}
	return re;
}
...
int main(){
	...
	cauto d=read();
	auto vs=readV(n);
	if(accumulate(all(vs),0ll)<0) for(auto &v:vs) v=-v;
	ll re=-1;
	rep(_,2){
		MX(re,solve(vs,d));
		reverse(all(vs));
