...
7 = x
 */
class tt(val i:Int, val j:Int, val t:Int){}

class cell(val t:Pair<Int,Int>){
    var ret = -1
}
...
fun main(){
    solve.cases{
        val inf = Pair(-2,-2)
        val n = getint()
        val all = List(n){ mutableListOf<cell>() }
        repeat(n){i ->
            repeat(n){j ->
                val t = Pair(getint() - 1 , getint() - 1 )
                all[i].add(cell(t))
            }
        }
        val tq = tripleQueue(n * n + 1 )


        fun bfs(i:Int, j:Int, dir:Int){
            val stack = tq
            stack.reset()
            tq.add(i,j,dir)
            stack.runtillend { i, j, dir ->
                val here = all[i][j]
                for(d in 0..3){
                    val newi = i + dy[d]
                    val newj = j + dx[d]
                    ...
                    val other = all[newi][newj]
                    if(other.ret != -1) continue
                    if(other.t != here.t) continue
                    other.ret = d xor 2
                    stack.add(newi,newj,d)
                }
            }
        }
...
//            val here = all[i][j]
//            all[i][j].ret = if(dir == 7) 7 else dir xor 2
//            for(d in 0..3){
//                val newi = i + dy[d]
//                val newj = j + dx[d]
//                if(newi !in 0 until n || newj !in 0 until n){
...
//        }
        ...
        for(i in 0 until n){
            next@for(j in 0 until n){
                val here = all[i][j]
                if(here.ret != -1) continue
                if(here.t != inf){
                    put("INVALID")
                    return@cases
                }
