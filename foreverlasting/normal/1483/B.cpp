...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read();
        set<int> S,T;
        for(res i=1;i<=n;i++)a[i]=read(),S.insert(i);
        a[n+1]=a[1];
        for(res i=1;i<=n;i++)if(gcd(a[i],a[i+1])==1)T.insert(i);
        res las=-1;
        auto get=[&](const res &p){
            auto it=T.upper_bound(p);
            return *(it==T.end()?T.begin():it);
        };
        auto nxt=[&](const res &p){
            auto it=S.lower_bound(p+1);
            return *(it==S.end()?S.begin():it);
        };
        vector<int> ans;
        while(!S.empty()&&!T.empty()){
            res x=get(las),y=nxt(x),z=nxt(y);
            T.erase(x);
            if(T.count(y))T.erase(y);
            if(gcd(a[x],a[z])==1)T.insert(x);
            ans.pb(y),S.erase(y),las=y;
        }
