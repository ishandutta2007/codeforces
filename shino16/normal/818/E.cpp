...
map<ll, int> factorizek(ll n, const M& u) {
    map<ll, int> m;
    for (auto [p, ignore] : u) while (n%p == 0) n /= p, m[p]++;
    return m;
}
...
int main() {
  ...
  auto kp = factorize(k);
  vector<M> ps(n);
  rep(i, n) ps[i] = factorizek(a[i], kp);
  ll res = 0;
  M mp;
  int r = 0;
  rep(l, n) {
    while (r < n and any_of(all(kp), [&](auto pk){
      return mp[pk.fs] < pk.sn;
    })) {
      for (auto [p, k] : ps[r]) mp[p] += k;
      r++;
    }
    chmax(r, l+1);
    if (all_of(all(kp), [&](auto pk){
      return mp[pk.fs] >= pk.sn;
    })) res += n-r+1;
    for (auto [p, k] : ps[l]) {
      mp[p] -= k;
