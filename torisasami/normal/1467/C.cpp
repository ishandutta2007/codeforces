...
int main()
{
	...
	rep(i, 3){
		ll a;
		rep(j, n[i]) cin >> a, sum[i] += a, mi[i] = min(mi[i], a);
	}
	ll ans = sum[0] + sum[1] + sum[2] - min(min(sum[0], sum[1]), sum[2]) * 2;
	ans = max(ans, sum[0] + sum[1] + sum[2] - mi[0] * 2 - mi[1] * 2 - mi[2] * 2 + max(max(mi[0], mi[1]), mi[2]) * 2);
	cout << ans << endl;
}