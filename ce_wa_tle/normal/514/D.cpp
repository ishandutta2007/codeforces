...
ll a[6][114514],q[6][114514],h[6]={1,1,1,1,1,1},t[6],ans[6];
int main(){
	//IO(CF514D R2D2 and Droid Army);
	ll rill(n,m,k),max=0,p=1;
	rep(i,1,n){
		rep(j,1,m){
			rill(a[j][i]);
			while(h[j]<=t[j]&&a[j][q[j][t[j]]]<=a[j][i])--t[j];
			q[j][++t[j]]=i;
//			write("s=");
//			rep(x,h[j],t[j])
//			write(a[j][q[j][x]],' ');
//			writeln();
		...
		ll sum=0;rep(j,1,m)sum+=a[j][q[j][h[j]]];//,write(a[j][q[j][h[j]]],' ');
//		writeln();
//		writeln(p,' ',i,' ',sum);
		ll hf=1;
		while(sum>k){
