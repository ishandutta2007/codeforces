...
template <typename T> pair<vector<int>, T> dijkstra(WeightedGraph<T> &g, int s, int t) {
...
main() {
    ...
    set<pii> se;
    vv(int, num, n, n);
    rep(i, n) { rep(j, g[i].size()) num[i][g[i][j]] = j; }
    const ll inf = 1e10;
    rep(i, n) for(auto &e : g[i]) if(!e.cost) {
        e.cost = inf;
        se.emplace(i, e.to), se.emplace(e.to, i);
    }
    auto [D, d] = dijkstra(g, s, t);
    if(d < l) END();
    auto write = [&]() {
        cout << "YES\n";
        rep(i, n) {
            for(auto j : g[i]) {
                if(i > j) continue;
                cout << i << " " << j << " " << j.cost << endl;
            }
        }
        exit(0);
    };
    if(d == l) write();
    for(auto [a, b] : se) { g[a][num[a][b]].cost = 1; }
    while(1) {
        auto [D, d] = dijkstra(g, s, t);
        if(d > l) END();
        if(d == l) write();
        bool flag = d == l;
        rep(i, D.size() - 1) {
            if(se.count(pii(D[i], D[i + 1]))) {
                if(!flag) {
                    flag = true;
                    g[D[i]][num[D[i]][D[i + 1]]].cost = l - d + g[D[i]][num[D[i]][D[i + 1]]].cost;
                    g[D[i + 1]][num[D[i + 1]][D[i]]].cost = l - d + g[D[i + 1]][num[D[i + 1]][D[i]]].cost;
                }
            }
        }
        if(!flag) { END(); }
