...
void solve() {
  ...
  VEC(ll, A, N);
  ll M = MAX(A) + 1;
  auto primes = prime_table(M);
  vi CNT = bin_count(A, M);
  ...
  FORIN(p, primes) {
    ...
    if (e == 0) continue;
    ll low = max(0LL, (e - log) * (p - 1));
    ll high = min(sum_A, (e + log) * (p - 1));
    if (high <= ANS) continue;

    auto check = [&](ll N) -> bool { return f(p, p * N) >= e; };
    ll n = binary_search(check, high / p, low / p) * p;
    chmax(ANS, n);
  }
  print(ANS);
}
