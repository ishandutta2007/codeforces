...
ll sum(int n, int k) {
    ...
    ll res = 0;
    for (int i = 0; i <= k - 1; ++i) {
        res = (res + cnk(k - 1, i) * sum(n - 1, i)) % mod;
    }
    return dp[gn - n][k] = res * n % mod;
}
ll sum2(int n, int k) {
    if (k == 0) return p2[gn - n];
    if (n == 0) return powmod(gn - n, k, mod);
    int w = gn - n;
    if (dp[w][k]) {
        return dp[w][k];
    }
    return dp[w][k] = (w * sum2(n, k - 1) + n * sum2(n - 1, k - 1)) % mod;
}
int solve() {
    rf[0] = fact[0] = 1;
    for (int i = 1; i <= 5010; ++i) {
        fact[i] = fact[i - 1] * i % mod;
        rf[i] = powmod(fact[i], mod - 2, mod);
    }
...
//
//    cout << sum(n, k) << endl;
//    cout << sum(n, k) / (1 << (n - 1)) << endl;
    cout << sum2(n, k) << endl;
//#ifdef LOCAL
//    ll check = 0;
//    for (int i = 0; i <= n; ++i) {
//        check += cnk(n, i) * powmod(i, k, mod) % mod;
//    }
//    check %= mod;
//    DBN(check);
//#endif
