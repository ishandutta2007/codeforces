...
int getgcd(int cur, int l, int r, int x, int y){
	if (l == x && r == y){
		...
		int dcur = cur + cur;
		if (y <= m)
			return getgcd(dcur, l, m, x, y);
		else if (x > m)
			return getgcd(dcur + 1, m + 1, r, x, y);
		else
			return gcd(getgcd(dcur, l, m, x, m), getgcd(dcur + 1, m + 1, r, m+1, y));
	}
}
...
int main(){
	...
	while (t--){
		...
		cin >> x >> y;
		--x; --y;
		int d = getgcd(1, 0, n - 1, x, y);
		int res = y - x + 1;
		if (m.count(d)){
			res -= upper_bound(m[d].begin(), m[d].end(), y) - lower_bound(m[d].begin(), m[d].end(), x);
		}
		cout << res << '\n';
	}
	return 0;
