...
void solve() {
  ...
  auto f = [&](ld x, ld cx, ld cy, ld theta) -> ld {
    ld dx = x - cx, dy = 0 - cy;
    ld a = cos(theta), b = sin(theta);
    tie(dx, dy) = mp(a * dx - b * dy, a * dy + b * dx);
    if (dy >= 0) return R;
    ld t = (-cy) / dy;
    return min(ld(R), cx + dx * t);
  };
  ...
  FOR(s, 1 << N) FOR(i, N) {
    ll t = s | 1 << i;
    if (s == t) continue;
    ld a = A[i][0], b = A[i][1], c = A[i][2];
    chmax(DP[t], f(DP[s], a, b, c));
  }
  print(DP.back() - L);
}
