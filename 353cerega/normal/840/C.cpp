...
int main()
{
    ...
    for (int i=1;i<=mx;i++)
    {
        for (int j=1;j<=i;j++)
        {
            ...
            {
                dp[i][j] = (dp[i][j]+dp[i-k][j-1]*c[i-1][k-1]%mod*f[k]%mod)%mod;
            }
        }
    }
    ll n;
    ...
    for (pair<ll,ll> cur: cnt)
    {
        a.push_back(cur.Y);
    }
    ll k = a.size();
    vector<vector<ll>> dp2(k+1,vector<ll>(n+1));
    dp2[0][0] = 1;
    for (int i=0;i<k;i++)
    {
        for (int prv=0;prv<=n-a[i];prv++)
        {
            for (int x=1;x<=a[i];x++)
            {
                dp2[i+1][prv+x] = (dp2[i+1][prv+x]+dp2[i][prv]*dp[a[i]][x])%mod;
            }
        }
    }
    ll ans = 0;
