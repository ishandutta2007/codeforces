...
struct tree{
  ...
  void add(int l,int r,int v,int a,int b,int k){
    ...
    add(l,r,v,a,c,k*2+1);
    add(l,r,v,c,b,k*2+2);
    T[k]=A[k]+min(T[k*2+1],T[k*2+2]);
  }
  int lastless(int r,int v,int a,int b,int k){
    dbg(r,v,a,b,k);
    if(r<=a || T[k]>=v)
      return -1;
    if(b==a+1)
    ...
    int c=(a+b)/2;
    v-=A[k];
    int t=lastless(r,v,c,b,k*2+2);
    if(t!=-1)
      return t;
    return lastless(r,v,a,c,k*2+1);
  }
};
...
int main(int argc, char **argv){
  ...
  forn(q,n){
    ...
    s+=t;
    dbg(p+1,t);
    T.add(p+1,n+1,t,0,n+1,0);
    dbg(T.A);
    dbg(T.T);
    int x=T.lastless(n+1,s,0,n+1,0);
    dbg(A);
    dbg(x);
    assert(x==-1 || A[x]!=-1);
    cout<<(x==-1?-1:A[x])<<'\n';
