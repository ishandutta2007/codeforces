...
int main(){
	...
	dfs(1,0);scanf("%d",&qu);
	for(int i=1;i<=qu;i++) scanf("%d%d",&a[i],&b[i]),c[a[i]]^=1,c[b[i]]^=1;
	int cnt=0;for(int i=1;i<=n;i++) cnt+=c[i];
	if(cnt) printf("NO\n%d\n",cnt>>1);
	else{
		printf("YES\n");
		for(int i=1;i<=qu;i++){
			vector<int> pu,pv;int u=a[i],v=b[i];
			while(dep[u]>dep[v]) pu.pb(u),u=fa[u];
			while(dep[v]>dep[u]) pv.pb(v),v=fa[v];
			while(u^v) pu.pb(u),pv.pb(v),u=fa[u],v=fa[v];
			pu.pb(u);
			printf("%d\n",pu.size()+pv.size());
