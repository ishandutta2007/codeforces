...
int main() {
    ...
    const int sz = min(n, 2 * k);
    factorials::init(1e6);

    vec<vec<mint>> dp(sz + 1, vec<mint>(sz + 1));
    dp[0][0] = 1;

    for (int i = 1; i <= sz; i++)
        for (int len = 1; len < i; len++)
            for (int cycles = 1; cycles <= i; cycles++)
                dp[i][cycles] += dp[i - 1 - len][cycles - 1] * fact[len] * C(i - 1, len);

    vec<mint> answer(k + 1);

    for (int take = 0; take <= sz; take++)
        for (int cycles = 0; cycles <= take; cycles++)
            if (take - cycles <= k)
                answer[take - cycles] += big_C(n, take) * dp[take][cycles];
