...
zet_p geom(zet_p base, int low, int high) {
	return (powe(base, high + 1) - powe(base, low)) / (base - 1);
}
zet_p fac[MN], fin[MN];
zet_p binom(int n, int a) {
	return fac[n] * fin[n - a] * fin[a];
}
int main() {
	#ifdef DEB
	atexit(PdAYB7V);
	...
	FORD(i, n - 1, 0)
		fin[i] = fin[i + 1] * (i + 1);
	FOR(a, 1, n) {
		zet_p curr = 2 * binom(n, a) * powe(3, n * 1ll * (n - a)) * powe(3, a);
		debug(imie(a), imie(curr));
		if (a % 2)
			ans -= curr;
		else
			ans += curr;
	...
	FOR(a, 1, n) {
		zet_p x = - 1 / powe(3, n - a);
		zet_p coef = binom(n, a) * 3 * powe(3, n * (n - a * 1ll)) * (a % 2 ? -1 : 1);
		debug(imie(a), imie(x), imie(coef));
		ans += coef * (powe(x + 1, n) - 1);
	}
	debug(range(fin, 0, n));
	ans *= -1;
	debug(imie(ans));
