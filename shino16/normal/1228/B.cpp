...
int main() {
  ...
  int h, w; cin >> h >> w;
  VI R(h), C(w); rep(i, h) cin >> R[i]; rep(i, w) cin >> C[i];
  int board[h][w]; memset(board, -1, sizeof(board));
  rep(r, h) {
    rep(c, R[r]) board[r][c] = 1;
    if (R[r] != w) board[r][R[r]] = 0;
  }
  rep(c, w) {
    rep(r, C[c]) if (board[r][c] == 0) { cout << 0 << newl; exit(0); } else board[r][c] = 1;
    if (C[c] != h) if (board[C[c]][c] == 1) { cout << 0 << newl; exit(0); } else board[C[c]][c] = 0;
  }
  int cnt = count(board[0], board[h], -1);
  cout << pow(modint(2), cnt) << newl;
}