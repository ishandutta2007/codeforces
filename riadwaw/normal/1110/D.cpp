...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dp = make_vector<int>(m + 1, 3, 3, -1000000000);

		dp[0][0][0] = 0;
		for (int i: range(m)) {
			for (int prevprev: range(3)) {
				for (int prev: range(3)) {
					if (cnt[i] < prevprev + prev) {
						continue;
					}
					int left = cnt[i] - prev - prevprev;
					for (int nxt = 0; nxt < 3 && nxt <= left; ++nxt) {
						updateMax(dp[i + 1][prev][nxt], dp[i][prevprev][prev] + (left - nxt) / 3 + prev);
					}
				}
			}
		}
