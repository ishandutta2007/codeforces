...
ll n, m;
ll a[200005];
DisjointSparseTable dst(18), dst2(18);
...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 1, n) cin >> a[i], dst.set(i, P(a[i], i)), dst2.set(i, P(-a[i], i));
	dst.calc(), dst2.calc();
	
	ll l, r, x;
	rep(i, 1, m){
		cin >> l >> r >> x;
		ll p = dst.query(l, r).second, q = dst2.query(l, r).second;
		if(a[p] != x) cout << p << "\n";
		else if(a[q] != x) cout << q << "\n";
		else cout << -1 << "\n";
	}
