...
void main_() {
	...
	auto f = [&](int i, int j) {
		if(i < j)
			return ll(abs(x[i] - x[j])) + d[i] + a[j];
		else
			return ll(abs(x[i] - x[j])) + c[i] + b[j];
	};
	ll res = f(s, e);
	REP(i, n) {
		if(used[i]) continue;
		ll cost = INF, blank = -1;
		REP(j, SZ(vec) - 1) {
			if(chmin(cost, f(vec[j], i) + f(i, vec[j + 1]) - f(vec[j], vec[j + 1])))
				blank = j;
		}
		res += f(vec[blank], i) + f(i, vec[blank + 1]) - f(vec[blank], vec[blank + 1]);
		vec.insert(vec.begin() + blank + 1, i);
	}
	print(res);
}