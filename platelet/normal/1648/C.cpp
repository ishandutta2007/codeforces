...
void add(int i, mint v) {
    // cout << "A" << i << ' ' << v.val() << endl;
    for(; i <= 200000; i += i & -i) c[i] += v;
}
mint qry(int i, mint r = 0) {
    // cout << "Q" << i << ' ';
    for(; i; i &= i - 1) r += c[i];
    // cout << r.val() << endl;
    return r;
...
mint solve() {
    ...
    rep(i, 1, n) cin >> a[i];
    rep(i, 1, m) cin >> b[i];
    rep(i, 0, n) fac[i] = i ? fac[i - 1] * i : 1;
    mint p = 1;
    rep(i, 1, n) p /= ++cnt[a[i]];
    rep(i, 1, 200000) if(cnt[i]) add(i, cnt[i]);
    mint ans;
    rep(i, 1, min(n, m)) {
        ans += qry(b[i] - 1) * p * fac[n - i];
        // cout << qry(b[i] - 1).val() << ' ' << ans.val() << endl;
        if(!cnt[b[i]]) return ans;
        p *= cnt[b[i]]--, add(b[i], -1);
    }
    return (n < m) + ans;
}
