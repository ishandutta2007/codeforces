...
void prec(string s, vector<int> &a, vector<int> &b)
{
...
int get_t(vector<int> &v, int l, int r)
{
...
inline void _main_function()
{
    ...
    vector<int> sa(sz(s)), sb(sz(s)), ta(sz(t)), tb(sz(t));
    ...
    for(int i = 0; i < n; ++i)
    {
        ...
        int ssb = get_t(sb, sl, sr);
        int ssa = min(sa[sr], sr - sl + 1);
        int ttb = get_t(tb, tl, tr);
        int tta = min(ta[tr], tr - tl + 1);
        ...
        else if((ttb - ssb) % 2 != 0) //ok
        {
            cout << 0;
            continue;
        }
        else if((ssa - tta) % 3 != 0 && ssb >= ttb)
        {
            cout << 0;
            continue;
        }
        else if(ssa == tta && ssb == 0 && ttb != 0)
        {
            cout << 0;
            continue;
        }
