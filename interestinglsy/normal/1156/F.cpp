...
///////////////////////////////////////////////////////////////////////////////////////
#define MAXN 5010
il ll A2( ll x ){ return x*(x-1); }

int n;
int a[MAXN];
int diffa[MAXN], difftop = 0;
int cnt[MAXN];
...
void Pre(){
	...
	For(i,difftop){
		...
		Forx(j,0,i){
			sel[i][j] = sel[i-1][j];
			if(j){
				sel[i][j] += sel[i-1][j-1] * cnt[unum] % MOD;
				Mymod(sel[i][j]);
			}
		}
		// Print(sel[i],0,i);
...
ll Solve( int x ){
	...
	For(i,difftop){
		if( diffa[i] < x ){
			le += cnt[diffa[i]];
			mxp = i;
		}else geq += cnt[diffa[i]];
	}
	geq -= 2;
	// O(sel[1][1]);
	ll ans = 0;
	Forx(fr,0,le){
		ll tans = sel[mxp][fr] * fact[le-fr+geq] % MOD;
		ans += tans;
		Mymod(ans);
	}
	return ans;
...
int main(){
	...
	For(i,difftop){
		...
		ll tans = A2(cnt[unum]) * Solve(unum) % MOD;
		ans += tans;
		Mymod(ans);
	}
	cout << ans*invfact[n]%MOD << endl;
