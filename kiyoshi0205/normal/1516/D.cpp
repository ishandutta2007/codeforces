...
int main(){
  ...
  mat divs(100001);
  {
    auto v=isprime(100001);
    rep(i,100001){
      if(v[i]){
        for(j=i;j<=100000;j+=i)divs[j].emplace_back(i);
      }
    }
  }
  ll Q;
  cin>>N>>Q;
  vec v(N),cnt(100001,0);
  mat dp(20,vec(N));
  rep(i,N)cin>>v[i];
  j=0;
  rep(i,N){
    while(j<N){
      bool f=false;
      for(auto x:divs[v[j]])f|=cnt[x]>0;
      if(f)break;
      for(auto x:divs[v[j]])++cnt[x];
      ++j;
    }
    dp[0][i]=j;
    for(auto x:divs[v[i]])--cnt[x];
  }
  debug(dp[0]);
  rep(j,19)rep(i,N){
    dp[j+1][i]=(dp[j][i]==N)?N:dp[j][dp[j][i]];
  }
  while(Q--){
    ll l,r;
    cin>>l>>r;
