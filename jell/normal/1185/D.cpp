...
signed main() {
    ...
    map<i64,int> mp;
    for(int i=0; i<n-1; ++i) {
        mp[a[i+1].fir-a[i].fir]++;
    }
    for(int i=0; i<n; ++i) {
        vector<int> me,mme;
        if(i) {
            me.emplace_back(a[i].fir-a[i-1].fir);
            if(i+1<n) mp[a[i+1].fir-a[i-1].fir]++,mme.emplace_back(a[1+i].fir-a[i-1].fir);
            if(!--mp[a[i].fir-a[i-1].fir]) mp.erase(a[i].fir-a[i-1].fir);
        }
        if(i+1<n) {
            me.emplace_back(a[i+1].fir-a[i].fir);
            if(!--mp[a[i+1].fir-a[i].fir]) mp.erase(a[i+1].fir-a[i].fir);
        }
        if(mp.size()<2) {
            cout<<a[i].sec<<endl;
            return 0;
