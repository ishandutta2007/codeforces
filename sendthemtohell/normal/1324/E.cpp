...
const int N = 2057;
int dp[2 * N * N]; // days passed + hours passed -> mx good

int ok(int c, int l, int r, int m) {
	c %= m;
	if (c >= l && c <= r) return 1;
	return 0;
}
...
inline void solve() {
	...
	int sm = 0;
	for (int i = 0; i < n; ++i) {
		vector<int> upd(i + 2, -INF);
		for (int cur = sm - i; cur <= sm; ++cur) {
			//cerr << cur - sm + i + 1 << endl;
			upd[cur - sm + i + 1] = max(upd[cur - sm + i + 1], dp[cur] + ok(cur + a[i], l, r, h));
			upd[cur - sm + i] = max(upd[cur - sm + i], dp[cur] + ok(cur + a[i] - 1, l, r, h));
			//dp[cur + a[i] + h] = max(dp[cur + a[i] + h], dp[cur] + ok(cur + a[i], l, r, h));
			//dp[cur + a[i] - 1 + h] = max(dp[cur + a[i] + h - 1], dp[cur] + ok(cur + a[i] - 1, l, r, h));
		}
		//cerr << endl << endl;
		for (int j = 0; j <= i + 1; ++j) {
			dp[j + sm - i + a[i] + h - 1] = upd[j];
		}
		sm += a[i] + h;
	}
	cout << min(*max_element(dp, dp + 2 * N * N), n) << endl;
}
