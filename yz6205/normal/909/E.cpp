...
int deg[N];
void solve() {
    rep (i,n) for (int j: e[i]) deg[j]++;
    queue < int > que[2];
    rep (i,n) if (!deg[i]) que[t[i]].push( i );
    vector < int > eans;
    while (!que[0].empty() || !que[1].empty()) {
        while (!que[0].empty()) {
            ses(0);
            int u=que[0].front(); que[0].pop();
            ses(u);
            eans.push_back(0);
            for (int v: e[u]) {
                deg[v]--;
                if (deg[v]==0) que[t[v]].push(v);
            }
        }
        while (!que[1].empty()) {
            ses(1);
            int u=que[1].front(); que[1].pop();
            eans.push_back(1);
            ses(u);
            for (int v: e[u]) {
                deg[v]--;
    ...
    int ans=0;
    rep (i,n) if (eans[i]==1 && (i==0 || eans[i-1]==0)) ans++;
    printf(IN,ans);
}