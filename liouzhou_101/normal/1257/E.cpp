...
void solve()
{
	...
	for (int i = 0; i <= n; ++ i)
	{
		if (i > 0)
		{
			...
			{
				/*
				H.modify(i+1, n+1, [&](node &it, int L, int R) -> void
				{
					it.minv -= 1;
					it.add -= 1;
				});
				*/
				/*
				H.modify(1, i+1, [&](node &it, int L, int R) -> void
				{
					it.minv += 1;
					it.add += 1;
				});
		...
		/*
		for (int i = 1; i <= n+1; ++ i)
			printf("(%d,%d)", i, H.query(i, i).minv);
		puts("");
		*/
		int d1 = (i-s[1][i])+(k[1]-s[1][i]);
		int tmp = H.query(i+1, n+1).minv;
		freshmin(ans, d1+tmp);
	}
