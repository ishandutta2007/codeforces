...
namespace SA
{
	...
	void init()
	{
		...
		Sort(233);
		for (int p=0,w=1,m=233;p<n;m=p,w<<=1)
		{
			p=0;
...
int lcp(int x,int y){y+=n+1;x=SA::rnk[x];y=SA::rnk[y];if (x>y) swap(x,y);return query(x+1,y);}
...
int main()
{
	...
	read(n);cin>>(s1+1);
	read(m);cin>>(s2+1);
	rep(i,1,n) s[i]=s1[i];s[n+1]='z'+1;
	rep(i,1,m) s[i+n+1]=s2[i];
	SA::n=n+m+1;SA::init();init();
	read(X);
	rep(i,0,X) rep(j,0,n)
	{
		chkmax(dp[i][j+1],dp[i][j]);
		int len=lcp(j+1,dp[i][j]+1);
		chkmax(dp[i+1][j+len],dp[i][j]+len);
	}
	rep(i,1,X) rep(j,1,n) if (dp[i][j]==m) return puts("YES"),0;
	puts("NO");
	return 0;
}