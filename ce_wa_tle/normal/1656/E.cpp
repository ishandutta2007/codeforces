...
cll N=1e6+7,B=107,mod=1e9+7,iz=1<<21,oz=1<<22;
typedef ll aN[N];
...
ll dfs(cll u,cll fa,cll deep){
    ll g=0,s=0,ls=0;
    rev(i,u,v)if(v^fa)g++,ls=s,s=dfs(v,u,deep+1),assert(ls==0||ls==s);
    if(!g)return a[u]=deep&1?1:-1;
    a[u]=-s*(deep==0?g:g+1);
    return-s;
}
void calc(){
