...
vi solve() {
	...
	if(numbers[n] != -1 && numbers[n] < k) return {-1};
	debug(numbers);
	vi ret;
	...
	while(k) {
		...
		debug(now, nxt);
		debug(numbers);
		if(numbers[n - nxt] == -1 || numbers[n - nxt] > k) {
			drep(i, now, nxt) ret.push_back(i);
			now = nxt;
			nxt = now + 1;
			continue;
		} else {
	...
	while(ret.size() < n) ret.push_back(int(ret.size()));
	add(ret, 1);
	debug(ret);
	return ret;
}
int main() {
	...
	INT(t);
	vvi ans;
	rep(t) { ans.push_back(solve()); }
	foa(tmp, ans) { vout(tmp); }
