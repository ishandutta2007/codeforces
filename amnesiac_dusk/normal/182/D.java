...
public class Main 
{
	...
	{
		String a=ns(),b=ns();
		Set<Integer> faa=new HashSet<Integer>(),fab=new HashSet<Integer>();
		...
		{
			boolean pos=true;
			String temp=a.substring(0, te);
			for(int i=1;i<a.length()/te; i++)
				if(!temp.equals(a.substring(i*te, (i+1)*te)))
				{
					pos=false;
					break;
				}
			if(pos)
			for(int i=0; i<b.length()/te; i++)
				if(!temp.equals(b.substring(i*te, (i+1)*te)))
				{
					pos=false;
					break;
				}
