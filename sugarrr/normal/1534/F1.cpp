...
void ADD(ll a,ll b,scc_graph&g,vvl& v){
    g.add_edge(a,b);
    v[a].pb(b);
}
    ...
    rep(i,0,h-1){
        rep(j,0,w-1){
            rep(z,0,3){
                ll nx=i+dx[z];
                ll ny=j+dy[z];
                if(valid(nx,ny) && s[i][j]=='#' && s[nx][ny]=='#'){
                    ADD(a[i][j],a[nx][ny],g,b);
                }
            }
        }
    }
...
signed main(){fastio
    ...
    cin>>h>>w;
    vs s(h);rep(i,0,h-1)cin>>s[i];
    {
        vl deb(w);rep(i,0,w-1)cin>>deb[i];
    }
     cout<<solve(s)<<endl;
