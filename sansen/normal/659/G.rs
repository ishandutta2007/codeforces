...
fn run() {
    ...
    for h in h.iter() {
        ...
        if dp.last().map_or(true, |p| p.1 >= h) {
            let mut way = M::one();
            for (l, r, v) in dp {
                if l < h {
                    way += M::new(r.min(h) - l) * v;
                }
            }
            dp = vec![(0, h, way)];
        } else {
            let way = M::one() + dp.iter().fold(M::zero(), |s, a| s + M::new(a.1 - a.0) * a.2);
            let pre = dp.last().unwrap().1;
            dp = vec![(0, pre, way), (pre, h, M::one())];
        }
        ans += dp.iter().fold(M::zero(), |s, a| s + M::new(a.1 - a.0) * a.2);
    }
    println!("{}", ans);
}
