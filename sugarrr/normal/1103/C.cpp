...
void dfs(ll id,vl&par,vl&dep,vvl&chi,vvl&rev,vvl &v){
    for(auto x:v[id]){
        if(par[id]==x)continue;
        if(par[x]!=inf){
            assert(abs(dep[x]-dep[id])>=2);
...
signed main(){fastio
    ...
    vl par(n+1,inf);par[1]=0;
    vl dep(n+1);dep[1]=0;
    vvl chi(n+1);
    vvl rev(n+1);
    dfs(1,par,dep,chi,rev,v);
    /*
    rep(i,1,n){
        cout<<i<<":"<<endl;
        cout<<"par "<<par[i]<<" dep "<<dep[i]<<endl;
        cout<<"chi:";for(auto x:chi[i])cout<<x<<" ";cout<<endl;
        cout<<"rev:";for(auto x:rev[i])cout<<x<<" ";cout<<endl;
    }*/
    ll mxd=0;
    for(auto x:dep)chmax(mxd,x);
    if((1+mxd) *k >=n){
        vl p;
        ll pos;
        rep(i,1,n){
            if(dep[i]==mxd){pos=i;break;}
        }
        while(pos!=0){
            p.pb(pos);
            pos=par[pos];
        ...
        while(leaf.size()>k)leaf.pop_back();
        assert(leaf.size()==k);
        cout<<"CYCLES"<<endl;
        for(auto l:leaf){
            assert(rev[l].size()>=2);
