...
int main() {
	...
	auto solve = [&](int n, int x, int y) {
		const int p = x + y;
		
		vector<int> val(p, 0);
		for (int i = 0, j = 0; i < p; i++, j += x, j %= p)
			val[i] = n / p + (j < n % p);
			
		vector<vector<vector<int>>> dp(p, vector<vector<int>>(2, vector<int>(2, 0)));
		dp[0][1][1] = val[0];
		
		for (int i = 1; i < p; i++) {
			for (int f = 0; f <= 1; f++) {
				dp[i][1][f] = dp[i - 1][0][f] + val[i];
				dp[i][0][f] = max(dp[i - 1][0][f], dp[i - 1][1][f]);
			}
		}
		
		int ans = max({ dp[p - 1][0][0], dp[p - 1][0][1], dp[p - 1][1][0] });
		return ans;
	};
	...
	W(small * solve(sz, x, y) + large * solve(sz + 1, x, y));
