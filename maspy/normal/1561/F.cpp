...
#line 3 "/home/maspy/compro/library/alg/lazy_cntsum_set.hpp"
...
using Lazy = Lazy_CntSum_Set<ll, -1>;

const int LIM = 200100;

LazySegTree<Lazy> seg(LIM);

void solve() {
  LL(N, Q);
  VEC(pi, LR, Q);
  for (auto&& a: LR) a.fi--, a.se--;
  reverse(all(LR));
  ll LE = 0;
  seg.apply(0, LIM, 1);
  ...
  for (auto&& [r, l]: LR) {
    int i = -1, j = -1;
    {
      auto check = [&](auto p) -> bool { return p.se <= l; };
      i = seg.max_right(check, 0);
    }
    {
      auto check = [&](auto p) -> bool { return p.se <= l + 1; };
      j = seg.max_right(check, 0);
    }
    // print(i, j);
    if (j == i + 1) ++LE;
    seg.set(i, {1, 0});
  }
  print(C<mint>(N + (N - 1 - LE), N));
}
