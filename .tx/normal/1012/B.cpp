...
int main(int argc, char** argv) {
    ...
    for (auto& i : xs) {
        if (isz(i)) {
            sort(begin(i), end(i));
            i.resize(unique(begin(i), end(i)) - begin(i));
        }
    }
    ...
    function<void(int, int)> dfs = [&](int x, int y) {
        ...
        auto& a = xs[y];
        int ia = lower_bound(begin(a), end(a), x) - begin(a);
        if (ia + 1 != isz(a)) {
            dfs(a[ia + 1], y);
        }
        if (ia != 0) {
        ...
        auto& b = ys[x];
        int ib = lower_bound(begin(b), end(b), y) - begin(b);
        if (ib + 1 != isz(b)) {
            dfs(x, b[ib + 1]);
        }
        if (ib != 0) {
