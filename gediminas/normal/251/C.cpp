...
int main () {
	...
	int k;
	cin >> a >> b >> k;
	const int skirtMax = 5000000;

	if (b + skirtMax >= a) {
		vi din(skirtMax + 1, 10000000);
		int fr = a - b;
		din[fr] = 0;

		for (int j = fr; j > 0; j--) {
			mini(din[j - 1], din[j] + 1);
		...
		vi din(lcm * 3, 10000000);
		din[lcm * 2 + (a % lcm)] = 0;

		for (int j = lcm * 2 + (a % lcm); j > lcm + (b % lcm); j--) {
			mini(din[j - 1], din[j] + 1);
		...
		cout << din[lcm + (b % lcm)] + (a / lcm - b / lcm - 1)*visas;
	}

	/*for (int del = 0; del < k; del++) {
		vi din(5000000 + 1, 10000000);
		int fr = a + del - b;
		din[fr] = 0;
	...
	/*for (int i = 0; i < lcm; i++) {
		vi din(lcm * 2, 10000000);
		din[lcm + i] = 0;

		for (int j = lcm + i; j > i; j--) {
			mini(din[j - 1], din[j] + 1);
