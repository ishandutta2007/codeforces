...
signed main()
{
	...
	rg(n+1)rg_(j,4)dp[i-1][0][j-1]=0;gr gr
	
	rg(n)
	rg_(j,k)
	if(j==1||j==k)
	{
		dp[i][j][0]=max(dp[i-1][j][0],dp[i-1][j-1][3])-a[i];
		dp[i][j][2]=max(dp[i-1][j][2],dp[i-1][j-1][1])+a[i];
		dp[i][j][1]=max(dp[i-1][j][1],dp[i][j][0]);
		dp[i][j][3]=max(dp[i-1][j][3],dp[i][j][2]);
	}
	else
	{
		dp[i][j][0]=max(dp[i-1][j][0],dp[i-1][j-1][3])-2*a[i];
		dp[i][j][2]=max(dp[i-1][j][2],dp[i-1][j-1][1])+2*a[i];
		dp[i][j][1]=max(dp[i-1][j][1],dp[i][j][0]);
		dp[i][j][3]=max(dp[i-1][j][3],dp[i][j][2]);
		mxe(dp[i][j][1],dp[i-1][j-1][1]);
		mxe(dp[i][j][3],dp[i-1][j-1][3]);
	}
	gr
	gr
	odl(max(dp[n][k][1],dp[n][k][3]));
	return 0;
}