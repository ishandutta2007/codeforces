...
#line 1 "/home/maspy/compro/library/alg/monoid_maxmin.hpp"
...
const ll INF = 1LL << 60;
using Mono = Monoid_MaxMin<ll, 1LL << 60>;
...
void solve() {
  ...
  */
  ...
  vi indeg(N);
  vi gain(N);
  vc<multiset<ll>> sets(N); // gain
  SegTree<Mono> seg(N);

  auto rm = [&](ll v) -> void {
    auto& s = sets[TO[v]];
    // print("rm", v, i, gain[v]);
    auto it = s.find(gain[v]);
    assert(it != s.end());
    s.erase(it);
  };
  ...
  auto upd = [&](ll v) -> void {
    auto& s = sets[v];
    ll lo = INF, hi = -INF;
    if (len(s)) lo = *(s.begin()) + B[v];
    if (len(s)) hi = *(prev(s.end())) + B[v];
    seg.set(v, mp(hi, lo));
  };

  FOR(v, N) indeg[TO[v]]++;
  FOR(v, N) { B[v] = A[v] / (indeg[v] + 2); }
  FOR(v, N) {
    gain[v] += A[v] - B[v] * (indeg[v] + 1);
    gain[TO[v]] += B[v];
  }
  FOR(v, N) add(v);
  FOR(v, N) upd(v);
  ...
  FOR(q, Q) {
    auto [a, b, c] = query[q];
    if (a == -1) {
      auto [a, b] = seg.prod_all();
      print(b, a);
    }
    elif (b == -1) { print(gain[a] + B[TO[a]]); }
    else {
      vi vs = {a, b, c, TO[b], TO[c]};
      UNIQUE(vs);
      for (auto&& v: vs) rm(v);
      gain[b] -= B[a] + (A[b] - (indeg[b] + 1) * B[b]);
      gain[c] -= (A[c] - (indeg[c] + 1) * B[c]);
      gain[TO[b]] -= B[b];
      gain[TO[c]] -= B[c];
      ...
      B[b] = A[b] / (indeg[b] + 2);
      B[c] = A[c] / (indeg[c] + 2);
      gain[b] += (A[b] - (indeg[b] + 1) * B[b]);
      gain[c] += B[a] + (A[c] - (indeg[c] + 1) * B[c]);
      gain[TO[b]] += B[b];
      gain[TO[c]] += B[c];
      for (auto&& v: vs) add(v);
      for (auto&& v: vs) upd(v), upd(TO[v]);
      // print("change", a, b, c);
    }
  }
}
