...
bool st[N], ud[M], vis[N];

char s[N][M];

int n, m, tot, cur, pt[N][2], lt[N];
int S, T, pre[N], dis[N], flow[N];
...
bool spfa() {
    for (int i = 0; i <= tot; ++i) dis[i] = I, vis[i] = 0;
    vis[S] = 1;
    dis[S] = 0;
    flow[S] = I;
    q.push(S);
...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int j = 1; j <= n; ++j) {
            ...
            bool ok = 1;
            for (int k = 0; k < m; ++k)
                if (s[i][k] == '1' && s[j][k] == '0') ok = 0;
            if (ok) ins(pt[i][0], pt[j][1], 1, -cnt);
        }
    }
    for (int i = 1; i <= n; ++i) ins(S, pt[i][0], 1, 0), ins(pt[i][1], T, 1, 0);
    dinic();
    for (int i = 1; i <= n; ++i) {
        int x = pt[i][0];
        for (Link *h = head[x]; h; h = h->next) {
