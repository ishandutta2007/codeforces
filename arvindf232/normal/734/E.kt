...
fun main() {
	...
	val maxColour = IntArray(n+1)
//	val maxColour2  = IntArray(n+1)
	var ret = 0

	for(i in 1..n){
		val v = G.DFSorder[i]
		var currentColour = intArrayOf(1,1)
		...
		for(w in G.E[v]){
			...
			val thatC = ci[w]
			if(thatC == thisC){
				currentColour.maxInsert(maxColour[w])
			}else{
				currentColour.maxInsert(maxColour[w]+1)
			}
		}
		maxColour[v] = currentColour[0]
		ret = maxOf(ret,currentColour[0] + currentColour[1] - 1)
//		println("v${v}")
//		println(currentColour.joinToString(" "))
	}
	put(ret/2)
	done()
