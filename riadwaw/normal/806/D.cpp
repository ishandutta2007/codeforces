...
public:
	void solve(std::istream& in, std::ostream& out) {
		int n;
		in >> n;
		auto matrix = make_vector<int>(n + 1, n + 1, 0);
		for (int i: range(n)) {
		...
		for (int to: range(n)) {
			...
			for (int th: range(n)) {
				if (th != to) {
					updateMin(matrix[n][to], 2 * matrix[th][to]);
				}
			}
		}

		vector<int64_t> dist(n + 1, inf * 1LL * inf);
		dist[start] = 0;
		vector<int> used(n + 1);
		for (int iter: range(n + 1)) {
			int v = -1;
			for (int i: range(n + 1)) {
				if (used[i]) {
					continue;
			...
			for (int to: range(n + 1)) {
				if (to != v) {
					//if (dist[v] == 0 && matrix[v][to] == 0) {
					//	cerr << "from " << v << " to " << to << endl;
					//}
					updateMin(dist[to], dist[v] + matrix[v][to]);
				}
			}
		}

		for (int i: range(n)) {
			out << dist[i] + mnEdge * (n - 1LL) << "\n";
		}
