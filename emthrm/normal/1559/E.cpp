...
int main() {
  ...
  vector<int> mu = mobius_mu_init(m);
  ModInt ans = 0;
  for (int g = 1; g * n <= m; ++g) {
    int sum = m / g;
    bool is_valid = true;
    vector<int> a;
    REP(i, n) {
      int le = (l[i] + g - 1) / g, ri = r[i] / g;
      if (le > ri) {
        is_valid = false;
        break;
      }
      sum -= le;
      if (ri > le) a.emplace_back(ri - le);
    }
    if (!is_valid || sum < 0) continue;
    vector<ModInt> dp(sum + 1, 0);
    dp[0] = 1;
    for (int ai : a) {
      int pos = sum;
      ModInt cur = 0;
      for (int j = sum; j >= 0; --j) {
        while (pos >= 0 && j - pos <= ai) cur += dp[pos--];
        ModInt tmp = dp[j];
        dp[j] = cur;
        cur -= tmp;
      }
    }
    ans += accumulate(ALL(dp), ModInt(0)) * mu[g];
  }
  cout << ans << '\n';
  return 0;
}