...
ll R, G, B;
V<VV<>> dp(201, VV<>(201, V<>(201, 0)));
V<> r(200), g(200), b(200);
ll res = 0;
ll dfs(int i, int j, int k){
    if(i == R and j == G) return 0;
    if(i == R and k == B) return 0;
    if(j == G and k == B) return 0;
    if(dp[i][j][k]) return dp[i][j][k];
    ll res = 0;
    if(i < R and j < G) chmax(res, dfs(i + 1, j + 1, k) + r[i] * g[j]);
    if(j < G and k < B) chmax(res, dfs(i, j + 1, k + 1) + g[j] * b[k]);
    if(k < B and i < R) chmax(res, dfs(i + 1, j, k + 1) + b[k] * r[i]);
    return dp[i][j][k] = res;
