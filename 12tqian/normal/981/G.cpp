...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            r--;
            x--;
            vpi use = uni[x].intersect(mp(l, r));
            uni[x].addInterval(mp(l, r));

            for(auto y: use){
                upd(max(l, y.f), min(r, y.s), 1, 2);
            }
            if(sz(use) != 0){
                f0r(i, sz(use) + 1){
                    if(i == 0){
