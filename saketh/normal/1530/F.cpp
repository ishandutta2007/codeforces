...
template<v_t MOD> struct modnum {
    ...
    modnum& operator -= (const modnum& o) { v += MOD - o.v; if (v >= MOD) v -= MOD; return *this; }
...
// }}}
using mn = modnum<int(31607)>;
using vmn = vector<mn>;
using vvmn = vector<vmn>;

namespace bitwise_convolution {
    template<typename T, bool INV> void transform(vector<T>& v, int N) {
    ...
    template<typename T>
    void conv(vector<T>& a, vector<T>& b) {
        const int N = sz(a);

        //transform<T, false>(a, N);
        transform<T, false>(b, N);

        for (int i = 0; i < N; i++)
            a[i] = a[i] * b[i];

        //transform<T, true>(a, N);
    }
}
...
int main() {
	...
	bitwise_convolution::transform<mn, false>(f, sz(f));
    ...
    F0R (row, N) {
        ...
        mn prob = 1; // empty
        F0R (col, N) prob *= 1 - p[row][col];

        // non-full rows!
        F0R (state, (1 << N) - 1) {
            int diag = (state >> row)&1;
            int anti = (state >> (N - 1 - row))&1;

            int full = state ^ (diag << N) ^ (anti << (N + 1));

            if (state) {
                int differ = state ^ (state - 1);
                F0R (col, N) {
                    if (!differ) break;
                    if (differ & 1) {
                        if (state & (1 << col)) {
                            prob *= p[row][col];
                            prob *= mn::small_inv((1 - p[row][col]).v);
                        } else {
                            prob *= 1 - p[row][col];
                            prob *= mn::small_inv(p[row][col].v);
                        }
                    }
                    differ >>= 1;
                }
        ...
        bitwise_convolution::conv(f, g);
    }

	bitwise_convolution::transform<mn, true>(f, sz(f));

    ans += accumulate(all(f), mn(0));
    ans -= f[0];
