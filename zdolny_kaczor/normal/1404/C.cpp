...
void solve() {
	...
	debug(range(a, 0, n));
	ordered_set<pii> begs;
	REP(i, n) {
		if (a[i] > 0) {
			if ((int)begs.size() >= a[i]) {
				auto it = begs.find_by_order(begs.size() - a[i]);
				maxl[i] = it->first;
			}
			else
				maxl[i] = -1;
			...
			maxl[i] = -1;
		if (maxl[i] != -1) {
			begs.insert(pii(maxl[i], i));
			debug("can", range(a, maxl[i], i + 1));
		}
		else debug("can't erase", arr(a, i));
	}
	REP(i, q) {
		int l, r;
		scanf("%d%d", &l, &r);
	...
	REP(i, n) if (maxl[i] != -1) {
		put(i, 1);
		erase_at[maxl[i]].push_back(i);
	}
	REP(l, n) {
		for (auto [i, r] : queries[l])
			ans[i] = read(l, r);
		for (int i : erase_at[l]) put(i, -1);
	}
	REP(i, q) printf("%d\n", ans[i]);
}
int main() {
