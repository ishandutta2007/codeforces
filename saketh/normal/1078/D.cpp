...
int main() {
    ...
    int N; re(N); vi p(N); re(p); F0R (r, 2) F0R (i, N) p.pb(p[i]);

    const int L = 30 - __builtin_popcount((N + 1));

    vvpii reach(L, vpii(3 * N));
    vector<segment_tree<pii>> sts(L);
    F0R (i, sz(p)) reach[0][i] = {max(0, i-p[i]), min(sz(p)-1, i+p[i])};
    F0R (l, L) {
        sts[l] = segment_tree<pii>(all(reach[l]), {INT_MAX, INT_MIN},
                [](pii a, pii b){ return mp(min(a.f, b.f), max(a.s, b.s)); });
        if (l+1 < L) F0R (i, sz(p))
            reach[l+1][i] = sts[l](reach[l][i].f, reach[l][i].s+1);
    }

    FOR (i, N, 2 * N) {
        int ans = 0; pii r = {i, i};
        F0Rd (l, L) {
            pii s = sts[l](r.f, r.s+1);
            if (s.s - s.f + 1 < N) {
                ans += 1 << l;
                r = s;
            }
