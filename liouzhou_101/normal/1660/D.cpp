...
void solve()
{
	auto calc = [](vector<int>& a) -> tuple<int, int, int, int>
	{
		int n = a.size();
		...
		{
			s[i + 1] = s[i];
			if (a[i] < 0) s[i + 1].first *= -1;
			if (abs(a[i]) == 2) s[i + 1].second += 1;
		}
		tuple<int, int, int, int> res = { 1, 0, 0, n };
		int j = -1;
		for (int i = 0; i < n; ++i)
		{
			auto [flag, cnt] = s[i + 1];
			if (flag == 1)
				freshmax(res, { flag, cnt, 0, n - i - 1 });
			else if (j != -1)
				freshmax(res, { 1, cnt - s[j + 1].second, j + 1, n - i - 1 });
			if (a[i] < 0 && j == -1) j = i;
		}
		return res;
	};
	...
	for (int i = 0; i < n; ++i)
	{
		if (a[i] != 0)
		{
			...
			auto [flag, cnt, x, y] = calc(v);
			freshmax(res, {flag, cnt, x + i, y + n - j});
			i = j;
		}
	}
	auto [flag, cnt, x, y] = res;
	writeln(x, ' ', y);
}
