...
int main() {
	CASE(t) {
		...
		ll ans = 0;
		bool forced0 = false;
		for (int kill = 0; kill <= len; kill++) {
			if (kill >= 1) {
				int slack = consec[1] + kill - 1;
				if (chunk[kill - 1] >= slack + 2)
					break;
				else if (chunk[kill - 1] == slack + 1)
					forced0 = true;
			}

			{
				int turns = min(consec[0] + kill, consec[1] + kill);
				ll base = turns * b + (turns - kill) * a - kill * c;
				chkmax(ans, base);
				if (consec[0] + kill > turns)
					chkmax(ans, base + a);
				if (!forced0 && consec[1] + kill > turns)
					chkmax(ans, base + b);
			}

			if (consec[1] > consec[0]) {
				int turns = min(cnt[0], consec[1] + kill);
				ll base = turns * b + consec[0] * a - (turns - consec[0]) * c;
				chkmax(ans, base);
				if (!forced0 && turns - consec[0] > kill)
					chkmax(ans, base + c);
				if (!forced0 && consec[1] + kill > turns)
					chkmax(ans, base + b);
			}
