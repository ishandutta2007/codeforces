...
int main() {
    ...
    vvll query(N);
    repn(i, N)query[0].push_back(i);

    vll ans(N+1, 1);
    segtree<ll, ad, e> sg(N);
    repn(i, N) {
        if (lis[i].size() > 0) { sg.set(lis[i][0], 1); }
    }
    ...
    rep(i, N) {
        ...
        auto itr = upper_bound(lis[a[i]].begin(), lis[a[i]].end(), i);
        if (itr != lis[a[i]].end()) { sg.set(*itr, 1); }
    }
