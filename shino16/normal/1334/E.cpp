...
map<ll, int> factorize_ps(ll n, const map<ll, int>& ps) {
  map<ll, int> m;
  while (n > 1) {
    for (auto [p, k] : ps) while (n % p == 0) m[p]++, n /= p;
  }
  return m;
}
...
int main() {
  ...
  auto pD = factorize(D);
  ll sumK = 0;
  for (auto [p, k] : pD) sumK += k;
  binomial binom(sumK+10);
  rep(IN) {
    ll v = IN, u = IN;
    auto pv = factorize_ps(v, pD), pu = factorize_ps(u, pD);
    VLL dks; dks.reserve(pD.size());
    for (auto [p, k] : pD) dks.push_back(pv[p]-pu[p]);
    modint res = 1;
    int dec_cnt = 0, inc_cnt = 0;
    for (auto dk : dks) {
      if (dk < 0) dec_cnt += -dk;
      else if (dk > 0) inc_cnt += dk;
      res *= binom.invfact[abs(dk)];
    }
    res *= binom.fact[dec_cnt] * binom.fact[inc_cnt];
    OUT(res);
  }
}