...
public class F {
	...
	void solve()
	{
		...
		SegmentTreeRMQ[] sts = new SegmentTreeRMQ[n];
		for(int i = 0;i < n;i++){
			sts[i] = new SegmentTreeRMQ(g[i].length);
		}
		int ans = 0;
		int z = 0;
		for(int[] e : es){
			int lb = lowerBound(g[e[0]], 0, e[2]);
			int ret = Math.max(0, -sts[e[0]].minx(0, lb)) + 1;
			ans = Math.max(ans, ret);
			sts[e[1]].update(iinds[z], -ret);
			z++;
		}
		out.println(ans);
	}
