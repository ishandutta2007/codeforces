...
set<ll> checked_primes;

bool TL()
{
	static auto start_time = clock();
	auto end_time = clock();
	return (end_time - start_time) * (int)1e3 / CLOCKS_PER_SEC > 1800;
}

void prime_optimize(const ll& prime, int& ans, const int ones, const vl& a)
{
	if (checked_primes.count(prime) == 0)
	{
		checked_primes.insert(prime);
		ll local_ans = 0;
		for (int i = ones; i < sz(a); ++i)
			if (a[i] < prime)
				local_ans += prime - a[i];
			else
				local_ans += min(a[i] % prime, prime - a[i] % prime);
		ans = min<ll>(ans, local_ans);
	}
}

void optimize(const ll& number, int& ans, const int ones, const vl& a)
{
	if (number >= 2)
	{
		auto f = fac.factor(number);
		for (auto g : f)
			prime_optimize(g.X, ans, ones, a);
	}
}
...
int another_main()
{
	...
	while (!TL())
	{
		//int r = rng() % (n - ones) + ones;
		int r = rng() % n;
		optimize(a[r] - 1, ans, ones, a);
		optimize(a[r], ans, ones, a);
		optimize(a[r] + 1, ans, ones, a);
	}
	//cout << ans + ones;
	cout << ans << '\n';
