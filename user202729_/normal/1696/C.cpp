...
void up(){
	...
	struct Item{int value; int64_t count;
		auto tie() const{return std::tie(value, count);}
	...
	auto const encode=[&](std::vector<int> v){
		std::vector<Item> items;
		for(auto value: v){
			Item item{.value=value,.count=1};
			while(item.value%m==0){
				item.value/=m;
				item.count*=m;
			}
			if(not items.empty() and items.back().value==item.value){
				items.back().count+=item.count;
			}else{
