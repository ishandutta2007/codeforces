...
bool dfs(int d, int p = -1) {
	if (d == 0) {
		pagr[d] = true;
		kelias.emplace_back(d, 0);
		return true;
	}
	...
	for (auto && i : grafas[d]) {
		...
		if (dfs(i.x, d)) {
			pagr[d] = true;
			kelias.emplace_back(d, kelias.back().y + i.y);
			return true;
		}
	}
...
ll dfsma(int d, int p = -1) {
	...
	for (auto && i : grafas[d]) {
		...
		maxi(ma, i.y + dfsma(i.x, d));
	}

	return ma;
}

int dfskie(int d, int p = -1) {
	int a = 1;
...
int main () {
	...
	for (int i = 1; i < n; i++) {
		...
		cin >> a >> b >> w;
		a--, b--;
		grafas[a].emplace_back(b, w);
		grafas[b].emplace_back(a, w);
	}

	dfs(n - 1);

	ll skir = -kelias.back().y;
...

	{
		...
		for (auto && i : kelias) {
			...
			if (maxg == 0)
				maxi(skir, ma + maxg - i.y);
			else
				maxi(skir, max(ma, la) + maxg - i.y);
