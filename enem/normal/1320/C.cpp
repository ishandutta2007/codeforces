...
static ll st[1LL<<21],lazy[1LL<<21];
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,m)
        {
            ...
            {
                if(bc[bc.size()-1].F>=b[i].F) continue;
                else bc.PB(b[i]);
            }
        }
        ll l=0;
        REP(i,0,bc.size())
        {
            update(1,0,(1LL<<20)-1,l,bc[i].F-1,-bc[i].S);
            l=bc[i].F;
        }
        update(1,0,(1LL<<20)-1,l,(1<<20)-1,-(1LL<<50));
        ll ans=-(1LL<<50);
        l=0;
        REP(i,0,n)
        {
            while(l<p)
            {
                if(c[l].F.F<a[i].F)
                {
                    update(1,0,(1LL<<20)-1,c[l].F.S,(1<<20)-1,c[l].S);
                    l++;
                }
                else break;
            }
            ll t=query(1,0,(1LL<<20)-1,0,(1LL<<20)-1);
            ans=max(ans,t-a[i].S);
        }
        cout<<ans;
