...
zet_p backtrack(int curr_ones, int curr_pos, int left) {
	if (curr_pos == 10 || curr_ones > 9)
		return 0;
	zet_p ans = 0;
	if (left == 0 && curr_pos > curr_ones) {
		ans += cou[curr_pos];
	}
	zet_p choices = 1;
	REP(i, left + 1) {
		ans += backtrack(curr_ones + i * curr_pos, curr_pos + 1, left - i) * choices * binom(left, i);
		choices *= cou[curr_pos] - i;
	}
	return ans;
}
int main() {
	...
	if (false) {
		...
		FORD(i, 8, 0) {
			...
			debug(imie(i), imie(lucky_low), imie(unlucky_low));
			REP(un, i + 1) {
				int ways = binom(i, un) * powe(2, un) * powe(8, i - un);
				debug(imie(un), imie(ways));
				cou[un + so_far] += ways * unlucky_low;
				cou[un + so_far + 1] += ways * lucky_low;
			}
			if (is_lucky(digits[i]))
				so_far++;
		}
		cou[so_far]++;
		cou[0]--;
