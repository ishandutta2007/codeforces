...
int main() {
	...
	while (true) {
		...
		ans.pb( { });
		vpi t = { *it };
		ans.back().pb(it->second);
		m.erase(it);
		for (int i = 2; i <= min(5, n); i++) {
			it = m.end();
			--it;
			if (i > 2) {
				if (disp%(i-1) == 0 || it->first == S->first)
					break;
			}
			t.pb(*it);
			ans.back().pb(it->second);
		...
		for (auto i : t)
			m.insert({max(0, i.first - 1), i.second});
	}
	cout << m.begin()->first << "\n";
	cout << ans.size() << "\n";
	for(auto i : ans) {
		for(int j = 0; j < n; j++) {
			cout << (find(all(i), j) != i.end());
		}
		cout <<"\n";
	}
}