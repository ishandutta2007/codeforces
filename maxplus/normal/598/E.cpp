...
int main() {
  ...
  for (int k = 0; k <= h * w && k < K; ++k) {
    ...
    for (int v = 1; 2 * v <= h; ++v)
    for (int s = unz(k - (h - v) * w); s <= k && s <= v * w; ++s) {
      mini(dp[h][w][k], dp[v][w][s] + dp[h - v][w][k - s] + w * w);
    }
    for (int v = 1; 2 * v <= w; ++v)
    for (int s = unz(k - (w - v) * h); s <= k && s <= v * h; ++s) {
      mini(dp[h][w][k], dp[h][v][s] + dp[h][w - v][k - s] + h * h);
    }
    dp[w][h][k] = dp[h][w][k];
  }
  for (int t = $; t--; ) {
