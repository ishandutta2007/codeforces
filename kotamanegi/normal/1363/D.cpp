...
#define int ll
int n, k;
int query(vector<int> now){
    set<int> nexts(ALL(now));
    vector<int> inputs;
    for (int i = 1; i <= n; ++i) {
        if (nexts.count(i) == 0) {
            inputs.push_back(i);
...
vector<int> collect(vector<pair<vector<int>, int>> nexts) {
    vector<int> hoge;
    REP(i, nexts.size()) {
        REP(q, nexts[i].first.size()) {
            hoge.push_back(nexts[i].first[q]);
        }
    }
    return hoge;
}
...
void dfs(vector<pair<vector<int>, int>> Divides, vector<pair<vector<int>, int>> Dies,int val) {
    if (Divides.size() == 1) {
        int b = query(collect(Divides));
        ans[Divides[0].second] = b;
        return;
    }
    vector<pair<vector<int>, int>> hoges[2];
    REP(i, Divides.size()) {
        if (i < Divides.size() / 2) {
            hoges[0].push_back(Divides[i]);
        }
        else {
            hoges[1].push_back(Divides[i]);
        }
    }
    int a = query(collect(hoges[0]));
    if (val == -1) {
        int b = query(collect(hoges[1]));
        if (a > b) {
            swap(a, b);
            swap(hoges[0], hoges[1]);
        }
        val = b;
    }
    if (val == a) {
...
void solve(){
    ...
    REP(i, k) {
        ...
        REP(i, a) {
            int b;
            cin >> b;
            inputs.back().first.push_back(b);
        }
        inputs.back().second = i;
    }
    dfs(inputs, {}, -1);
    Answer();
}
