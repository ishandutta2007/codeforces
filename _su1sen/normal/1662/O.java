...
public class O {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int t = 0; t < T; t++) {
            int N = sc.nextInt();
            boolean[][] straight = new boolean[20][360];
            boolean[][] round = new boolean[20][360];
            for (int n = 0; n < N; n++) {
                if (sc.next().charAt(0) == 'C') {
                    ...
                    for (int m = th1; m < th2; m++) {
                        round[r - 1][m % 360] = true;
                    }
                } else {
                    int r1 = sc.nextInt();
                    int r2 = sc.nextInt();
            ...
            while (!queue.isEmpty()) {
                int[] now = queue.poll();
                if (!straight[now[0]][(now[1] + 1) % 360] && !pass.contains(now[0] * 360 + (now[1] + 1) % 360)) {
                    queue.add(new int[] { now[0], (now[1] + 1) % 360 });
                    pass.add(now[0] * 360 + (now[1] + 1) % 360);
                }
                int temp = now[1] == 0 ? 359 : now[1] - 1;
                if (!straight[now[0]][now[1]] && !pass.contains(now[0] * 360 + temp)) {
                    queue.add(new int[] { now[0], temp });
                    pass.add(now[0] * 360 + temp);
                }
                if (now[0] > 0 && !round[now[0] - 1][now[1]] && !pass.contains((now[0] - 1) * 360 + now[1])) {
                    queue.add(new int[] { now[0] - 1, now[1] });
                    pass.add((now[0] - 1) * 360 + now[1]);
                }
                if (!round[now[0]][now[1]] && !pass.contains((now[0] + 1) * 360 + now[1])) {
                    queue.add(new int[] { now[0] + 1, now[1] });
                    pass.add((now[0] + 1) * 360 + now[1]);
                    if (now[0] == 19) {
                        ok = true;
