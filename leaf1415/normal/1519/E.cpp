...
ll dfs(int v, int p)
{
	...
	for(auto e : G[v]){
		...
		used[e.cost] = true;
		ll res = dfs(u, v);
		if(res) ans.push_back(P(e.cost, res));
		else vec.push_back(e.cost);
	}
...
int main(void)
{
	...
	rep(i, 1, n){
		u[i] = get((a[i]+b[i])*d[i], c[i]*b[i]);
		v[i] = get(a[i]*d[i], (c[i]+d[i])*b[i]);
	...
	ll m = sz(vec);
	rep(i, 1, n){
		ll x = lower_bound(all(vec), u[i]) - vec.begin() + 1;
		ll y = lower_bound(all(vec), v[i]) - vec.begin() + 1;
		G[x].push_back(edge(y, i));
		G[y].push_back(edge(x, i));
	}
	
	rep(i, 1, m) if(!visit[i]) dfs(i, -1);
	outl(sz(ans));
