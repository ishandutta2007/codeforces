...
namespace Power_List
{
    ...
    int dep[N];
    int List_cutt;
    inline void find_son(int now,int fa)
    {
        int maxl=0;lar[now]=1;
        for(int i=nxt[now].size()-1;i>=0;i--)
        {
            int to=nxt[now][i];if(to==fa)continue;
            find_son(to,now);if(lar[to]>maxl)maxl=lar[to],son[now]=to;lar[now]+=lar[to];
        }
        return;
    }
    inline void make_list(int now,int Fa,int Tp)
    {
        num[now]=++List_cutt;
        nu[num[now]]=now;
        tp[num[now]]=num[Tp];
        fa[num[now]]=num[Fa];
        dep[num[now]]=dep[num[Fa]]+1;
        if(!son[now])return;make_list(son[now],now,Tp);
        for(int i=nxt[now].size()-1;i>=0;i--)if(!num[nxt[now][i]])make_list(nxt[now][i],now,nxt[now][i]);return;
    }
}
inline void Power_List_Init(int Root){using namespace Power_List;find_son(Root,0);make_list(Root,0,Root);return;}
inline int Lca(int x,int y)
{
    using namespace Power_List;
    x=num[x];y=num[y];for(;tp[x]!=tp[y];x=fa[tp[x]])if(dep[tp[x]]<dep[tp[y]])x^=y^=x^=y;return nu[min(x,y)];
}
inline int dis(int x,int y){using namespace Power_List;return abs(dep[num[x]]-dep[num[y]]);}

struct bcy
{
    int lens;
    int node;
    inline void work(int a,int x,int y,int z)
    {
        int now;lens=0x3f3f3f3f;using namespace Power_List;
        now=Lca(a,x);if(dep[num[now]]>=dep[num[z]]&&dis(a,now)<lens)lens=dis(a,now),node=now;
        now=Lca(a,y);if(dep[num[now]]>=dep[num[z]]&&dis(a,now)<lens)lens=dis(a,now),node=now;
        now=Lca(a,z);if(now==z&&dis(a,now)<lens)lens=dis(a,now),node=z;if(now!=z&&dis(a,now)+dis(z,now)<lens)lens=dis(a,now)+dis(z,now),node=z;
        return;
    }
}zjj,gyq;
inline bool check(int rd,int nows,int k)
{
    if(nows>k)return false;
    if((k-nows)%2==0)return true;
    if(rd%2==0)return false;
    return nows+rd<=k;
}
inline void work()
{
    int x,y,z,a,b,k,rd,len,nows;rin>>x>>y>>a>>b>>k;z=Lca(a,b);nows=dis(a,z)+dis(b,z);
    if(k>=nows&&(k-nows)%2==0){puts("YES");return;}
    z=Lca(x,y);rd=dis(x,z)+dis(y,z)+1;zjj.work(a,x,y,z);gyq.work(b,x,y,z);
    z=Lca(zjj.node,gyq.node);len=dis(zjj.node,z)+dis(gyq.node,z);
    nows=len+zjj.lens+gyq.lens;if(check(rd,nows,k)){puts("YES");return;}
    nows=rd-len+zjj.lens+gyq.lens;if(check(rd,nows,k)){puts("YES");return;}
    puts("NO");
    return;
}
int main()
{
    rin>>n;for(int i=1,x,y;i<n;i++){rin>>x>>y;nxt[x].push_back(y);nxt[y].push_back(x);}Power_List_Init(1);
    for(rin>>q;q;q--)work();
    return 0;
}