...
int main(void)
{
	...
	while(T--){
		...
		for(auto p : mp){
			vector<ll> &vec = p.se;
			for(auto id : vec){
				if(id > 0) S[c[id]].insert(id);
				else S[c[-id]].erase(-id);
			}
			if(sz(S[0]) && sz(S[1])){
				ll v = *S[0].begin();
				for(auto u : S[0]) uf.unite(u, v);
				for(auto u : S[1]) uf.unite(u, v);
				rep(i, 0, 1){
					P p = P(-inf, -inf);
					for(auto id : S[i]) chmax(p, P(r[id], id));
					S[i].clear();
					S[i].insert(p.se);
				}
			}
