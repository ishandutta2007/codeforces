...
int main() {
  ...
  rep(t) {
    ...
    int forbid = p[n/2];
    int border = n/2;
    while (border > 0 and p[border-1] == forbid) border--;
    int G = p[0];
    int g = upper_bound(all(p), G, greater<int>()) - p.begin();
    debug(g);
    if (g < n) {
      int S, s = g;
      while (s<n and s-g <= g) {
        S = p[s];
        s = upper_bound(all(p), S, greater<int>()) - p.begin();
      }
      debug(s);
      if (s < n) {
        int B, b = s;
        while (b<n and b-s <= g) {
          B = p[b];
          b = upper_bound(all(p), B, greater<int>()) - p.begin();
        }
        debug(b);
        debug(p[b-1]);
        debug(forbid);
