...
int main(){
  ...
  cout<<fixed<<setprecision(10);
	cauto n=read();
	cauto abs=readV<pii>(n);
	fill(dp[0],dp[112],-MOD);
	dp[0][0]=0;
	for(cauto &[a,b]:abs)rrep(i,n)rep(j,10034) MX(dp[i+1][j+a],dp[i][j]+b);
	int sum=0;
	for(cauto &[a,b]:abs) sum+=b;
	vector<int> re(n);
	reps(i,1,n+1)rep(j,11234)if(dp[i][j]>=0){
		cauto mv=sum-dp[i][j];
		MX(re[i-1],min(2*dp[i][j]+mv,2*j));
	}
	out(re,1);
	rep(i,n) cout<<(long double)(re[i])/2<<" "; cout NL;
