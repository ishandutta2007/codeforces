...
vector<int> aida[10][1005];

map<vector<int>,bool> MA[1<<7];

bool solve(int rem,vector<int> T){
    if(__builtin_popcount(rem)<si(T)) return false;
    if(si(T)==0) return true;
    
    if(MA[rem].count(T)) return MA[rem][T];
    
    if(si(T)==1){
        int x=T[0];
        for(int i=0;i<N;i++){
            if(rem&(1<<i)){
                if(si(aida[i][x])==0) return true;
            }
        }
    }
    ...
    for(int i=0;i<N;i++){
        ...
        for(int j=0;j<si(T);j++){
            vector<int> nex=aida[i][T[j]];
            for(int k=0;k<si(T);k++) if(j!=k) nex.push_back(T[k]);
            sort(all(nex));
            nex.erase(unique(all(nex)),nex.end());
            if(solve(rem-(1<<i),nex)) return MA[rem][T]=true;
        }
    }
    
    return MA[rem][T]=false;
}
...
int main(){
    ...
    for(int i=0;i<N;i++){
        for(int j=0;j<K;j++){
            for(int k=0;k<K;k++){
                if(j==k) continue;
                if(ccw(P[i],Q[j],Q[k])==0) aida[i][j].push_back(k);
            }
        }
    }
