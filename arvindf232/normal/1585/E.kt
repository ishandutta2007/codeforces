...
class query(val v:Int,val least:Int,val k:Int){
    var ans:Int = nlarge
}

fun main(){
    val now = Array(max){ mutableSetOf<Int>() }
    val st = simpleSegTree(max)
    val frequency = IntArray(max)
    fun change(v:Int, old:Int, new:Int){
        now[old].remove(v)
        now[new].add(v)
        st.addValue(old,-1)
        st.addValue(new,1)
    ...
    fun ask(least_occurence:Int,kth:Int):Int{
        val remove = st.query(0,least_occurence - 1 )
        val get = st[kth - 1 + remove]
        if(get == -1) return - 1
        val S = now[get]
        return S.first()
    }
    ...
    solve.cases{
        ...
        val allqueries = Array(q){
            query(getint - 1 , getint, getint)
        }
