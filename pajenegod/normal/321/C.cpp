...
arborescence shallowest_tree_decomposition(vector<vector<int>>graph) {
    int n = graph.size();
    vector<vector<int>> decomposition_tree(n), stacks(log(n) + 1);
    ...
    auto dfs = [&](int u, int p, auto&& self) -> void {
        ...
        dp[u] = 1 + (seen | ~(~0U << log(seen_twice | 1)));
        stacks[ctz(dp[u])].push_back(u);
    ...
    int root = 0;
    dfs(root, -1, dfs);
    int decomposition_root = stacks[log(dp[root])].back(); stacks[log(dp[root])].pop_back();
    create_chain(dp[root] & ~dp[decomposition_root], decomposition_root);
    return {decomposition_tree, decomposition_root};
}
 
#define remover(vec, v) vec.erase(find(vec.begin(), vec.end(), v))
 
arborescence centroid_tree_decomposition(vector<vector<int>> graph) {
    int n = graph.size();
...
int main() {
    ...
    //auto [decomposition_tree, root] = shallowest_tree_decomposition(graph);
    auto [decomposition_tree, root] = centroid_tree_decomposition(graph);
    
    vector<int> labels(n);
    auto dfs = [&](int node, auto&& self) -> void {
        for (auto child : decomposition_tree[node]) {
