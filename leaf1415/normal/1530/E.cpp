...
int main(void)
{
	...
	while(T--){
		...
		vector<P> vec;
		rep(i, 0, 25) if(cnt[i]) vec.push_back(P(i, cnt[i]));
		
		string tmp;
		if(sz(vec) == 1){
			for(auto p : vec) rep(i, 1, p.second) tmp += p.first + 'a';
			cout << tmp << "\n";
			continue;
		}
		...
		if(q != -1){
			...
			for(auto p : vec){
				...
				rep(i, 1, p.second) tmp += p.first + 'a';
			}
			cout << tmp << "\n";
			continue;
		}
		...
		if(sz(vec) == 2){
			tmp = "";
			tmp += vec[0].first + 'a';
			rep(i, 1, vec[1].second) tmp += vec[1].first + 'a';
			rep(i, 1, vec[0].second-1) tmp += vec[0].first + 'a';
			chmin(ans, tmp);
		}
		
		if(sz(vec) >= 3){
			tmp = "";
			tmp += vec[0].first + 'a', tmp += vec[1].first + 'a';
			rep(i, 1, vec[0].second-1) tmp += vec[0].first + 'a';
			tmp += vec[2].first + 'a';
			
			rep(i, 1, (int)sz(vec)-1) rep(j, 1, vec[i].second){
				if(i <= 2 && j == 1) continue;
				tmp += vec[i].first + 'a';
			}
			chmin(ans, tmp);
		...
		tmp = "";
		tmp += vec[0].first + 'a', tmp += vec[0].first + 'a';
		tmp += vec[1].first + 'a';
		vec[0].second -= 2, vec[1].second--;
		
		string t; ll rem = vec[0].second;
		rep(i, 1, (int)sz(vec)-1) rep(j, 1, vec[i].second) t += vec[i].first + 'a';
		reverse(all(t));
		
		while(rem || sz(t)){
			if(rem > 0) tmp += vec[0].first + 'a', rem--;
			if(sz(t)) tmp += t.back(), t.pop_back();
		}
		bool flag = true;
		rep(i, 2, sz(tmp)-1){
			if(tmp[i-1] == vec[0].first + 'a' && tmp[i] == vec[0].first + 'a') flag = false;
		}
