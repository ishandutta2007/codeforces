...
ll f(ll n, ll parts)
{
    ...
    return x*x*(parts - n%parts) + y*y*(n%parts);
}

ll diff(ll n, ll parts)
{
    return f(n, parts-1) - f(n, parts);
}

ll at_least(ll n, ll x)
{
    ll R = n+1;
    ll L = 1;
    while (R-L>1)
...
bool check(ll x)
{
    ...
    for (auto it: d)
    {
        int l1 = at_least(it, x+1);
        int l2 = at_least(it, x);
    ...
    //cout<<ops<<' '<<totalsum<<' '<<appear<<endl;

    if (totalsum-appear*x<=m)
    {
        cout<<ops + (totalsum-m + (x-1))/x<<endl;
        exit(0);
    }
...
int main()
{
    ...
    for (int i = 0; i<n; i++) d.push_back(a[i+1]-a[i]);

    cin>>m;

    ll tot = 0;
    for (auto it: d) tot+=it*it;
    if (tot<=m) {cout<<0<<endl; return 0;}
