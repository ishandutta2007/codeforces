...
Vec<Mi> FactorizePalindrome(int N, const Indexer<int> &s) {
  seq::Eertree tree(N);
  ...
  var get_min = [&](Node *v) {
    ...
    dp[v->id] = index < 0 ? 1 : ans[index];
    if (v->fail->id >= 0 && diff[v->id] == diff[v->fail->id]) {
      dp[v->id] += dp[v->fail->id];
    }
    return dp[v->id];
  };
  for (n = 0; n < N; n++) {
...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  Debug(num);
  var ans = adhoc::FactorizePalindrome(N, MakeIndexer<int>(num));
  var res = ans.back();
  out << res;
}
