...
public:
    ...
    void update(const int& u, const int& d) {
        if (l == r) {
            count += d;
            sum = static_cast<int64_t>(count) * l;
        } else {
            if (m >= u) left->update(u, d);
            else right->update(u, d);
            sum = left->sum + right->sum;
            count = min(int(1e9), left->count + right->count);
        }
    }

    int64_t get_min_sum(const int& k) {
        if (l == r) {
            return static_cast<int64_t>(min(k, count)) * l;
        } else {
            if (left->count >= k) return left->get_min_sum(k);
            return left->sum + right->get_min_sum(k - left->count);
        }
    }
};
...
vector<pair<int, int>> update[1000002];

void solve() {
    cin >> n >> k >> m;
    auto tree = new segment_tree_t(1, 1e6);
    for (int i = 1, l, r, c, p; i <= m; i++) {
        cin >> l >> r >> c >> p;
        update[l].emplace_back(p, c);
        update[r + 1].emplace_back(p, -c);
    }
    int64_t ans = 0;
    for (int i = 1; i <= n; i++) {
        for (auto&& [p, c] : update[i]) tree->update(p, c);
        ans += tree->get_min_sum(k);
    }
    cout << ans << '\n';
}
