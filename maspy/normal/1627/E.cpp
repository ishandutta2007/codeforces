...
void solve() {
  ...
  using Mono = Monoid_Min<ll, INF>;
  SegTree<Mono> seg1(W), seg2(W);

  FOR(a, H) {
    ll C = X[a];
    if (a == H - 1) {
      ll ANS = INF;
      for (auto&& [x, v]: dat[a]) { chmin(ANS, abs(W - 1 - x) * C + v); }
      if (ANS == INF)
        print("NO ESCAPE");
      else
        print(ANS);
    ...
    if (len(dat[a]) == 0) continue;
    for (auto&& [x, v]: dat[a]) {
      seg1.set(x, min(seg1[x], v - C * x));
      seg2.set(x, min(seg2[x], v + C * x));
    }
    for (auto&& [y, c, d, h]: E[a]) {
      ll v1 = seg1.prod(0, y + 1) + C * y;
      ll v2 = seg2.prod(y, W) - C * y;
      dat[c].eb(d, min(v1, v2) - h);
    }
    for (auto&& [x, v]: dat[a]) {
      seg1.set(x, INF);
      seg2.set(x, INF);
