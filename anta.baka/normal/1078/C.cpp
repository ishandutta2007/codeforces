...
void dfs(int v, int p) {
    ...
    for(int to : g[v]) {
        if(to == p) continue;
        dfs(to, v);
        dp[0][v] = 1LL * dp[0][v] * (dp[0][to] + dp[1][to]) % M;
        mul = mul * (2*dp[1][to] + dp[0][to]) % M;
    }
    mull[v] = mul;
    for(int to : g[v]) {
        if(to == p) continue;
        ll mul1 = mul * pw(2*dp[1][to] + dp[0][to], M - 2) % M;
        mul1 = mul1 * mull[to] % M;
        dp[1][v] = (dp[1][v] + mul1) % M;
    }
}
...
main() {
    ...
    dfs(0, -1);
    //for(int i = 0; i < n; i++) cout << "V = " << i + 1 << " DP[0] = " << dp[0][i] << " DP[1] = " << dp[1][i] << endl;
    cout << (dp[0][0] + dp[1][0]) % M;
}