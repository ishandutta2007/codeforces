...
vvll dp;
vvii par;
void go(int i, int no){
	if(i==0){cout<<"1 ";return;}
	for(ii j:par[i]){
		if(dp[no-1][j.F]+j.S==dp[no][i]){
			go(j.F,no-1);
			cout<<i+1<<" ";
			return;
		}
...
int main()
{
	...
	rep (i, m) {int x, y, t; cin >> x >> y >> t; g[x - 1].pb({y - 1, t}); deg[y - 1]++;par[y-1].pb({x-1,t});}

	vi yo;
	rep (i, n) {if (deg[i] == 0) yo.pb(i);}
	...
	while (!yo.empty()) {
		...
		topo.pb(i + 1);
		yo.pop_back();
		for (ii j : g[i]) --deg[j.F] ? void() : yo.pb(j.F);
	}

	dp = vvll(n+1, vll(n, INF2));

	dp[1][0] = 0;
	FOR (i, 1, n) {
		FOR (j, 0, n - 1) {
			for (ii k : g[j]) {
				dp[i][k.F] = min(dp[i][k.F], dp[i - 1][j] + k.S);
			}
		}
	}
