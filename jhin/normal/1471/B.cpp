#include <bits/stdc++.h>
    ...
    if(x>1)p.push_back(x),cnt++,vv.push_back(1);
        ...
        long long m,n,x,y,z,k,mn=1e9,mx,ans,a[500003],b[500003];bool yes1,yes2;vector<long long>v;char ch;
       ...
       while(t--)
       {
           cin>>n>>x;long long sum=0;mn=1e9;
           for(int i=0;i<n;i++)
           {
               cin>>a[i];
               long long num=a[i],cnt=1;
               while(1)
               {
           ...
           for(int i=0;i<n;i++)
           {
               ...
               mn2=min(mn2,cnt);
               if(mn2>mn)sum+=(mn+1)*a[i];
               else sum+=mn*a[i];
           }
           cout<<sum<<"\n";
//16 2
//16 2 8 8 1 1 4 4 4 4 4
