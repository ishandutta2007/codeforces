...
inline void AC() {
    ...
    for(_ i = 32; i >= 0; --i) {
        v.reset();
        c.reset();
        for(_ j = 1; j <= n; ++j) g[j].clear();
        for(_ j = 1; j <= m; ++j) if(!(b[j].z & (1LL<<i))) {
            v.set(b[j].x);
            v.set(b[j].y);
        }
        for(_ j = 1; j <= m; ++j) if(b[j].z & (1LL<<i)) {
            if(v.test(b[j].x)) c.set(b[j].y);
            ef(v.test(b[j].y)) c.set(b[j].x);
            if(b[j].x == b[j].y) c.set(b[j].x);
        }
        for(_ j = 1; j <= m; ++j) if(b[j].z & (1LL<<i)) {
            if(c.test(b[j].x) || c.test(b[j].y)) continue;
            g[b[j].x].push_back(b[j].y);
        }
        for(_ j = 1; j <= n; ++j) if(!c.test(j)) {
            for(_ k : g[j]) c.set(k);
        }
        for(_ j = 1; j <= n; ++j) if(c.test(j)) a[j] |= 1LL<<i;
    }
    wr(a, n);
