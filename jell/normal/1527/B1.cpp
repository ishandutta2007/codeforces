#line 1 "codeforces-workspace\\contest\\1527\\b1\\b1.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\1527\\b1\\b1.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\1527\\b1\\b1.cpp"
...
auto dp = cached([](auto&& dp, int x, int y, int z, int w) -> int {
  auto ret = inf;

  if (z) chle(ret, 1 - dp(x, y, 0, 1));
  if (x) chle(ret, 1 - dp(x - 1, y + 1, z, 1));
  if (y) chle(ret, 1 - dp(x, y - 1, z, 1));
  if (y && w) chle(ret, -dp(x, y, z, 0));
