...
// 
// Verify: Codeforces Round #406 (Div. 1), problem: (B) Legacy <https://codeforces.com/contest/786/submission/93412000>
...
int main()
{
    ...
    int N, Q, S;
    cin >> N >> Q >> S;
    SegmentEdgeShortestPath<lint> graph(N);
    ...
    while (Q--)
    {
        ...
        if (t == 1)
        {
            int v, u, w;
            cin >> v >> u >> w;
            graph.add_edge(v - 1, v, u - 1, u, w);
        }
        else
        {
            int v, l, r, w;
            cin >> v >> l >> r >> w;
            if (t == 2) graph.add_edge(v - 1, v, l - 1, r, w);
            else graph.add_edge(l - 1, r, v - 1, v, w);
        }
    }
    graph.solve_dijkstra(S - 1);

    for (auto x : graph.dist) cout << (x < 1e17 ? x : -1) << ' ';
    cout << '\n';
}