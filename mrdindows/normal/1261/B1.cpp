...
template <class Ch, class Tr, class Container>
basic_ostream <Ch, Tr> & operator << (basic_ostream <Ch, Tr> & os, const Container& x) {
    if (std::is_pointer<std::decay_t<Container>>::value) return os << (void*)x << endl; else
...
void add(int cur, int l, int r, int val) {
    ...
    if (l != r) {
        ...
        if (val <= m) {
            ...
            add(dcur + 1, m + 1, r, val);
        }
    }
}
int getKth(int cur, int l, int r, int k) {
    if (l == r) {
        return l;
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        if (tr[dcur] >= k) {
            return getKth(dcur, l, m, k);
        } else {
            return getKth(dcur + 1, m + 1, r, k - tr[dcur]);
        }
    }
}
int solve() {
