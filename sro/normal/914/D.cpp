...
void change(int now,int nl,int nr,int to,int x){
	...
	dt[now]=gcd(dt[now<<1],dt[now<<1|1]);
}
/*Give me AC or UR Doec 17*/

int soL(int now,int nl,int nr,int l,int x){
	if(nr<l){
		return -1;
	}
	if(dt[now]%x==0){
	...
	int m=(nl+nr)>>1;
	int ans=soL(now<<1,nl,m,l,x);
	if(ans!=-1){
		return ans;
	}
	return soL(now<<1|1,m+1,nr,l,x);
}

int soR(int now,int nl,int nr,int r,int x){
	if(nl>r){
		return -1;
	}
	if(dt[now]%x==0){
	...
	int m=(nl+nr)>>1;
	int ans=soR(now<<1|1,m+1,nr,r,x);
	if(ans!=-1){
		return ans;
	}
	return soR(now<<1,nl,m,r,x);
}
...
int main(){
	...
	while(q--){
		...
		if(M0DE==2){
			...
			int a,b,c;
			fio>>a>>b>>c;
			int anL=soL(1,1,n,a,c);
			int anR=soR(1,1,n,b,c);
			if(anL!=-1&&anR!=-1&&anL<anR){
				fio<<"NO\n";
			}else{
				fio<<"YES\n";
			}
