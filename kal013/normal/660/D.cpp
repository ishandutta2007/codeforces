...
struct pt{
    ...
    pt operator /= (const F3 c) {this->x/=c;this->y/=c;return *this;}
    // COMPARES y first
    inline bool operator < (const pt&r) {return (y<r.y || (y==r.y && x<r.x));}
...
map<pair<pii,int>,vi> dp;


pt arr[maxn];

pii slop(int x,int y){
    if (x==0) return {1,0};
...
void solve(){
    ...
    For(i,n){
        For(j,i){
            pt tmp=arr[i]-arr[j];
            m=slop(tmp.x,tmp.y);
            dp[mp(m,tmp*tmp)].pb(arr[i].y*m.S-arr[i].x*m.F);
