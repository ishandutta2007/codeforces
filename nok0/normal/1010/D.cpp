...
void main_() {
	...
	auto dfs = [&](auto dfs, int now) -> int {
		if(type[now] == 0) {
			...
			res[now] = !dfs(dfs, G[now][0]);
		} else if(type[now] == 2) {
			res[now] = dfs(dfs, G[now][0]) & dfs(dfs, G[now][1]);
		} else if(type[now] == 3) {
			res[now] = dfs(dfs, G[now][0]) ^ dfs(dfs, G[now][1]);
		} else {
			res[now] = dfs(dfs, G[now][0]) | dfs(dfs, G[now][1]);
	...
	auto dfs2 = [&](auto dfs2, int now) -> void {
		if(type[now] == 0) {
			if(bit[now]) {
				if(need[now] == 0) ans[now] = 1;
				if(need[now] == 1) ans[now] = 0;
				if(need[now] == -1) ans[now] = 0;
				if(need[now] == -2) ans[now] = 1;
			} else {
				if(need[now] == 0) ans[now] = 0;
				if(need[now] == 1) ans[now] = 1;
				if(need[now] == -1) ans[now] = 0;
				if(need[now] == -2) ans[now] = 1;
			}
		}
		if(type[now] == 1) {
			auto to = G[now][0];
			need[to] = (need[now] >= 0 ? !need[now] : need[now]);
			dfs2(dfs2, to);
			return;
		}
		// and
		if(type[now] == 2) {
			...
			if(need[now] == 1) {
				...
				need[t1] = need[t2] = need[now];
			debug(need[t1], need[t2]);
			dfs2(dfs2, t1);
			dfs2(dfs2, t2);
		}
		if(type[now] == 3) {
