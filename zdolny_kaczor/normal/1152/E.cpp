...
int a[nax], b[nax];
bool used[nax];
unordered_map <int, vpii > graf;
vector <int> ans;
void dfs(int x) {
	while (!graf[x].empty()) {
		if (!used[graf[x].back().second]) {
			pii v = graf[x].back();
			used[v.second] = 1;
			graf[x].pop_back();
			dfs(v.first);
...
int main() {
	...
	REP(i, n - 1) {
		if (a[i] > b[i]) err();
		graf[a[i]].emplace_back(b[i], i);
		graf[b[i]].emplace_back(a[i], i);
	}
	vector <int> odd;
	for (auto &x : graf) if (x.second.size() & 1)
		odd.push_back(x.first);
	if (odd.size() > 2u) err();
	if (odd.empty()) dfs(graf.begin()->first);
	else dfs(odd[0]);
	debug(imie(ans));
	if ((int)ans.size() != n) err();
