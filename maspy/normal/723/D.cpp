...
void solve() {
  ...
  FOR(x, H) FOR(y, W) {
    FOR(d, 4) {
      ...
      if (A[x][y] == A[xx][yy]) uf.merge(W * x + y, W * xx + yy);
    }
  }
  // 1 lake, 2 riku, 3 sea
  vi kind(H * W);
  FOR(h, H) FOR(w, W) {
    ...
    char c = A[h][w];
    if (c == '*') kind[r] = 2;
    if (c == '.' && (h == 0 || h == H - 1 || w == 0 || w == W - 1)) kind[r] = 3;
    if (kind[r] == 0) kind[r] = 1;
  }

  vc<pi> lake;
  FOR(v, H * W) if (kind[v] == 1) lake.eb(mp(v, uf.size[v]));
  sort(all(lake), [](auto& x, auto& y) { return x.se > y.se; });
  ll ANS = 0;
  FOR3(i, K, lake.size()) {
    ANS += lake[i].se;
    kind[lake[i].fi] = 2;
