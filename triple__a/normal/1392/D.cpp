...
signed main(){
    ...
    while (t--){
        cin>>n;
        cin>>s;
        rep(i,n) if (s[i]=='L') a[i]=1; else a[i]=0;
        int res=1e18;
        rep(i,4){
            rep(j,n) rep(k,4) dp[j][k]=0;
            if (a[0]!=(i&1)) dp[1][i]++;
            if (a[1]*2!=(i&2)) dp[1][i]++;
            rep(j,4) if (i!=j) dp[1][j]=100;
            for (int j=2;j<n;++j){
                dp[j][0]=dp[j-1][1];
                dp[j][1]=min(dp[j-1][2],dp[j-1][3]);
...
//            for (int j=1;j<n;++j) {cout<<dp[j][0]<<" "<<dp[j][1]<<" "<<dp[j][2]<<" "<<dp[j][3]<<endl;}
            if (i==0){
                res=min(res,min(dp[n-1][2],dp[n-1][3]));
            }
            if (i==1){
                res=min(res,min(dp[n-1][0],min(dp[n-1][1],dp[n-1][2])));
            }
            if (i==2){
                res=min(res,min(dp[n-1][1],min(dp[n-1][2],dp[n-1][3])));
            }
            if (i==3){
