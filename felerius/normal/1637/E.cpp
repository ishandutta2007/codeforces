// Best Pair (https://codeforces.com/contest/1637/problem/E)
    ...
    struct Hash {
        ...
        size_t operator()(const T& x) const noexcept {
            ...
            if constexpr (HackSafe && is_integral_v<T>)
                return (((((h = uint64_t(x) + RND + A) ^= h >> 30) *= B) ^= h >> 27) *= C) ^= h >> 31;
            else if constexpr (is_base_of_v<cp_lib_modint::ModIntTag, T>)
...
int main() {
    ...
    while (t--) {
        ...
        auto is_bad = [&](int x, int y) {
            return x == y || bad.find(pair(min(x, y), max(x, y))) != end(bad);
        };
        ll ans = 0;
        rep(cx, n + 1) trav(x, by_cnt[cx]) rep(cy, cx + 1) {
            auto it = rbegin(by_cnt[cy]);
            while (it != rend(by_cnt[cy]) && is_bad(x, *it))
                ++it;
            if (it != rend(by_cnt[cy]))
                ans = max(ans, ll(cx + cy) * (x + *it));
        }
