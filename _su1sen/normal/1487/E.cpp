...
int main() {
    ...
    rep(i, 1, 4) {
        ...
        for (auto &l : g) sort(all(l));
        atcoder::segtree<int, op, e> seg(dp[i - 1]);
        dp[i].resize(n[i], e());
        rep(j, int(n[i])) {
            int l = 0;
            for (int r : g[j]) {
                chmin(dp[i][j], seg.prod(l, r) + c[i][j]);
                l = r + 1;
            }
            chmin(dp[i][j], seg.prod(l, n[i - 1]) + c[i][j]);
        }
    }
    int ans = e();
    rep(j, int(n[3])) {
