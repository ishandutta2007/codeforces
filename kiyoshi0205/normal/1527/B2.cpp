...
ll calc(ll a,ll b,ll c){
  ...
  ans=-inf;
  if(b==0){
    if(a)chmax(ans,-calc(a-1,0,c)-1);
    if(c)chmax(ans,calc(a,1,c-1)-1);
    return ans;
  }else if(b==1){
    chmax(ans,-calc(a,0,c)-1);
    if(a)chmax(ans,calc(a-1,1,c)-1);
    if(c)chmax(ans,calc(a,2,c-1)-1);
    return ans;
  }
  chmax(ans,calc(a,b-1,c)-1);
  if(a)chmax(ans,calc(a-1,b,c)-1);
  if(c)chmax(ans,calc(a,b+1,c-1)-1);
  return ans;
}
int main(){
  startupcpp();
  ...
  cin>>N>>s;
  vec cnt(3,0);
  rep(i,N/2)cnt[(s[i]-'0')+(s[N-i-1]-'0')]++;
  debug(cnt);
  cnt[2]=N%2==1&&s[N/2]=='0';
  if(cnt[1]){
    if(cnt[0]==0&&cnt[1]==1&&cnt[2]==1)print("DRAW");
    else print("ALICE");
  }else{
    ll A=cnt[0]*2+cnt[2];
    if(A>=3&&A%2==1)print("ALICE");
    else print("BOB");
  }
}
}