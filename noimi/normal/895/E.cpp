...
signed main(){
    ...
    rep(i,n)a.pb(in());
    using P = pair<ld,ld> ;
    LazySegmentTree<ld,P> seg(n,
                            [](ld x,ld y){return x+y;},
                            [](ld x,P y){return x*y.fi+y.se;},
                            [](P x,P y){return P{x.fi*y.fi , x.se*y.fi + y.se};},
                            (ld)0,P{1,0},a,
                            [](P x,int n){return P{x.fi,x.se*n};});
    rep(i,q){
        int id=in();
        if(id==1){
            int l1=in()-1,r1=in()-1,l2=in()-1,r2=in()-1;
            ll t = r1-l1+1, s =r2-l2+1;
            ld T = seg.query(l2,r2+1)/(ld)(t*s);
            ld S = seg.query(l1,r1+1)/(ld)(t*s);
            seg.update(l1,r1+1,P{(ld)(t-1)/(ld)t,T});
            seg.update(l2,r2+1,P{(ld)(s-1)/(ld)s,S});
        }
        else {
            int l=in()-1,r=in()-1;
