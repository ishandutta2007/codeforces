...
struct solver
{
    ...
    {
        int n,m; cin>>n>>m;
        vector<int> d(m); cin>>d; sort(__all(d));
        int gre,red; cin>>gre>>red;
        vector dist(m,vector(gre+1, i64(1e15)));
        dist[0][0]=0;
        deque<p32> que;
        ...
        while(!que.empty())
        {
            ...
            if(t==gre)
            {
                if(chmin(dist[v][0],dist[v][gre]+1))
                {
                    que.emplace_back(v,0);
                }
                continue;
            }
            if(v)
            {
                int tt=d[v]-d[v-1]+t;
                if(tt<=gre && chmin(dist[v-1][tt],dist[v][t]))
                {
                    que.emplace_front(v-1,tt);
                }
            }
            if(v+1<m)
            {
        ...
        for(int i=0;i<m;++i)
        {
            int lad=n-d[i];
            if(lad>gre) continue;
            chmin(ans, lad+dist[i][0]*(gre+red));
        }
