...
void solve() {
    ...
    map<int, int> cnt;
    for (auto i : b) ++cnt[i];
    vector<vector<int>> bypos(n + 2);
    for (int i = 0; i < n; ++i) bypos[b[i]].pb(i);
    int k = -1;
    for (int i = 1; i <= n + 1; ++i)
        if (!cnt.count(i)) k = i;
    vector<int> ans(n, k);
    set<pii> st;
    for (int i = 1; i <= n + 1; ++i) st.insert({sz(bypos[i]), i});
    for (int i = 0; i < x; ++i) {
        int c = (--st.end())->s;
        auto &v = bypos[c];
        st.erase({sz(v), c});
    ...
    vector<int> cop = ans;
    for (int i = 0; i < y - x; ++i) cop[ind[i]] = b[ind[i]];
    int shift = sz(ind) / 2;
    for (int i = 0; i < sz(ind); ++i)
        ans[ind[i]] = cop[ind[(i + shift) % sz(ind)]];
    println("YES");
