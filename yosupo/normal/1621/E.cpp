...
void solve() {
    ...
    atcoder::lazy_segtree<S, op, e, F, mapping, composition, id> seg(V<S>(n + 1, 0));
    // [n, n-1, ..., 1, 0]
    for (int i = 0; i < n; i++) {
        seg.apply(0, i + 1, 1);
    }
    auto get_bi = [&](int k, ll sum) {
        // a[i] >= sum / k
        sum = (sum + k - 1) / k;
        int z = int(lower_bound(a.begin(), a.end(), sum) - a.begin());
        return z;
    };
    ...
    for (int i = 0; i < m; i++) {
        ...
        seg.apply(0, 1 + get_bi(k, sums[i]), -1);
    }
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < k; j++) {
            seg.apply(0, 1 + get_bi(k - 1, sums[i] - g[i][j]), -1);
