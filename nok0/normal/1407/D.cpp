...
//sdijkstras
//g:WGraph, s:(0-indexed)
V<ll> dijkstra(WGraph g, int s){
    V<ll> dist(SZ(g), INF);
...
int main(){
    ...
    RREP(i, n){
        ...
        if(st.size()) G[i].pb({st.top().second, 1});
        st.emplace(h[i], i);
    }
    }
    {
    ...
    REP(i, n){
        while(!st.empty() and st.top().first < h[i]) st.pop();
        if(st.size()) G[st.top().second].pb({i, 1});
