...
void solveIndSet(vector<int>& IndSet) {
	while (IndSet.size() > (k + 1) / 2)
		IndSet.pop_back();
	W(1);
	W(IndSet);
	exit(0);
...
void dfs(int u, int p, int d) {
	...
	for (auto& v : g[u]) {
		...
		if (dep[v] != -1) {
			int cycLen = abs(dep[v] - dep[u]) + 1;
			if (chkmin(shortest, cycLen))
				bestv = v;
		}
	}
