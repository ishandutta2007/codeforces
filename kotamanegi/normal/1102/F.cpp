...
void solve(){
    ...
    REP(i, n) {
        ...
        inputs.push_back(neko);
    }
    if (n == 1) {
        int ans = 1e18;
        REP(q, m-1) {
            ans = min(ans, abs(inputs[0][q] - inputs[0][q + 1]));
        }
        cout << ans << endl;
        return;
    }
    REP(i, n) {
        REP(q, n) {
            cnter[i][q] = 1e18;
            REP(j, m) {
                cnter[i][q] = min(cnter[i][q], abs(inputs[i][j] - inputs[q][j]));
            }
        }
    }
    int ans = 0;
    REP(i, n) {
        ...
        for (int i = 1; i < (1 << n); ++i) {
            REP(q, n) {
                REP(j, n) {
                    if (i & (1 << j)) continue;
                    dp[j][i | (1 << j)] = max(dp[j][i | (1 << j)], min(dp[q][i], cnter[q][j]));
                }
            }
        }
        REP(q, n) {
            ...
            REP(j, m-1) {
                now = min(now, abs(inputs[q][j] - inputs[i][j+1]));
            }
