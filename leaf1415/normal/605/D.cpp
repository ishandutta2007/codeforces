...
ll n;
ll sx[100005], sy[100005], tx[100005], ty[100005];
vector<P> vec[200005];
ll dist[100005], rec[100005];
SegTree seg(18);

void comp(ll x[], ll x2[])
{
	vector<ll> comp;
	rep(i, 0, n){
		comp.push_back(x[i]);
	...
	rep(i, 0, n){
		...
		x2[i] = lower_bound(all(comp), x2[i]) - comp.begin();
	}
}
...
int main(void)
{
	...
	comp(sx, tx), comp(sy, ty);
	
	seg.init();
	rep(i, 1, n) vec[sx[i]].push_back(P(sy[i], i));
	rep(i, 0, 2*n){
		sort(all(vec[i])), reverse(all(vec[i]));
		if(sz(vec[i])) seg.update(i, P(vec[i].back().first, i));
	}
	...
	while(sz(Q)){
		...
		while(1){
			P res = seg.query(0, x);
			if(res.first > y) break;
			ll id = res.second;
			ll u = vec[id].back().second;
			dist[u] = dist[v] + 1, rec[u] = v;
			Q.push(u);
			vec[id].pop_back();
			if(sz(vec[id])) seg.update(id, P(vec[id].back().first, id));
			else seg.update(id, P(inf, inf));
		}
	}
	if(dist[n] > inf/2){
		outl(-1);
