    ...
    REP(tc,1,ntc+1)
    {
        ...
        while(pr.size()>0)
        {
            ...
            for(auto it:ad[t.S])
            {
                if(dis[it][0]==-1)
                {
                    dis[it][0]=dis[t.S][0]+1;
                    pr.push(MP(-dis[it][0],it));
                }
            }
        }
        pr.push(MP(0LL,b));
        ...
        while(pr.size()>0)
        {
            ...
            for(auto it:ad[t.S])
            {
                if(dis[it][1]==-1)
                {
                    dis[it][1]=dis[t.S][1]+1;
                    pr.push(MP(-dis[it][1],it));
                }
            }
        }
        pr.push(MP(0LL,c));
        ...
        while(pr.size()>0)
        {
            ...
            for(auto it:ad[t.S])
            {
                if(dis[it][2]==-1)
                {
                    dis[it][2]=dis[t.S][2]+1;
                    pr.push(MP(-dis[it][2],it));
                }
            }
        }
        ll ans=(1LL<<60);
        REP(i,0,n)
        {
            if(dis[i][0]+dis[i][1]+dis[i][2]<=m)
            {
                ans=min(ans,dp[dis[i][1]]+dp[dis[i][0]+dis[i][1]+dis[i][2]]);
            }
        }
        cout<<ans;
