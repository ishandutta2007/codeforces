...
int main() {
  ...
  for (int i = 0, b; i < m; ++i) {
    ...
    std::vector<ModT> p1(e1 + 1), p2(e2 + 1);
    for (int i = 0; i <= e1; ++i) {
      p1[i] = binom(e1, i) * power_of_two[i];
    }
    for (int i = 0; i <= e2; ++i) {
      p2[i] = binom(e2, i);
    }
    std::vector<ModT> out;
    poly.multiply(out, p1, p2);
    pre.emplace_back(b, std::move(out));
  }
  int q;
  scanf("%d", &q);
  while (q--) {
