...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		for(int i=0;i+m-1<n;i++){
			int fl=1;
			for(int j=0;j<m;j++){
				if(s[i+j]!=t[j]){fl=0;break;}
			}
			a[i]=fl;
		}
		f[0]={0,1};
		for(int i=1;i<=n;i++)f[i]={n+1,0};
		for(int i=0;i<n;i++){
			for(int j=i;j<=n;j++){
				if(j-i>=m&&a[j-m])break;
				if(a[j]){
					if(f[i].fi+1<f[j+m].fi)f[j+m]={f[i].fi+1,f[i].se};
					else if(f[i].fi+1==f[j+m].fi)f[j+m].se+=f[i].se;
				}
			}
		}
		int lst=0;
		for(int i=n-1;i>=0;i--){
			if(a[i]){lst=i+1;break;}
		}
		pair<int,Z> ans={n+1,0};
		for(int i=lst;i<=n;i++){
			if(f[i].fi<ans.fi)ans=f[i];
			else if(f[i].fi==ans.fi)ans.se+=f[i].se;
		}
		printf("%d %d\n",ans.fi,ans.se);
	}
}
