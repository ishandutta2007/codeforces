...
vector<int> shrink(vector<int> a) {
	sort(all(a));
	vi diff;
	rep(i, int(a.size()) - 1) { diff.push_back(a[i + 1] - a[i]); }
	sort(all(diff));
	return diff;
}
...
int solve(vi v, int n) {
	...
	rep(n - 1) {
		...
		for(auto it = counter.begin(); it != counter.end(); it++) {
			auto nxt = next(it);
			if(nxt == counter.end()) break;
			replace[nxt->first - it->first]++;
		}

		foa(t, counter) {
			if(t.second > 1) replace[0] += t.second - 1;
		}
		debug(counter);
		swap(counter, replace);
	}

	debug(counter);
	assert(counter.size() == 1 && counter.begin()->second == 1);
