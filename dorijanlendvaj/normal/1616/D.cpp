...
int dx[]={0,1,0,-1};
int dy[]={1,0,-1,0};
const int N=50010,M=1<<16;
int t,n,h[N],seg[M*2+10];
ll prs[N],seg2[M*2+10];
...
{
	seg2[i+=M]=x;
	for (i/=2;i;i/=2) seg2[i]=max(seg2[i*2],seg2[i*2+1]);
}

ll ge2(int l,int r,int lo=0,int hi=M,int i=1)
{
	if (lo>=l && hi<=r) return seg2[i];
	if (lo>=r || hi<=l) return -LLINF;
	int mid=(lo+hi)/2;
...
int main()
{
	...
	{
		cin>>n;
		for (int i=0;i<n;++i) cin>>h[i],upd(i+2,MOD);
		upd(0,0);
		upd(1,1);
		int x;
		cin>>x;
		for (int i=0;i<n;++i) h[i]-=x,prs[i+1]=prs[i]+h[i];
		for (int i=0;i<=n;++i) upd2(i,prs[i]);
		int l=0;
		for (int r=1;r<=n;++r)
		{
			while (l<r && ge2(l,r-1)>prs[r]) ++l;
			//cout<<r<<' '<<l<<en;
			upd(r+1,ge(l,r+1)+1);
		}
		cout<<n-(seg[n+1+M]-1)<<en;
	}
}