...
int dp[801][4001];
int opt[801][4001];
bitset<4001> called[801];

inline int get(int l, int r)
{
    return ((summ[r][r] + summ[l - 1][l - 1] - summ[r][l - 1] - summ[l - 1][r]) >> 1);
}

inline int calc(int j, int pos, int fL, int fR)
{
    if (dp[j][pos] != INF || called[j][pos])
        return opt[j][pos];
    int optK = 0;
    for (int jj = fL; jj < min(pos, fR + 1); jj+=1)
    {
        if (dp[j - 1][jj] + get(jj + 1, pos) < dp[j][pos])
        {
            dp[j][pos] = dp[j - 1][jj] + get(jj + 1, pos);
            optK = jj;
        }
    }
    //cout << j << " " << pos << " " << dp[j][pos] << " " << opt[j][pos] << endl;
    opt[j][pos] = optK;
    called[j][pos] = 1;
    return optK;
}

inline void compute(int j, int L, int R, int fL, int fR)
{
    //cout << L << " " << R << endl;
    if (R < L)
        return;
    int M = (L + R) / 2;
    int fM = calc(j, M, fL, fR);
    if (L != R)
    {
        compute(j, L, M, fL, fM);
        compute(j, M + 1, R, fM, fR);
    }
}
    ...
    for (int i = 1; i <= n; i+=1)
    {
        for (int j = 1; j <= n; j+=1)
        {
            ...
            while (ch = getchar(), (ch < '0' || ch > '9'))
				;
            a[i][j] = ch - '0';
            summ[i][j] = summ[i - 1][j] + summ[i][j - 1] + a[i][j] - summ[i - 1][j - 1];
        }
    }
    for (int i = 0; i <= n; i++)
        for (int j = 0; j <= k; j++)
