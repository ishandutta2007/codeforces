...
void _main() { /* CURSOR START */
	int T; cin>>T; for(;T--;) {
		int n; cin>>n;
		vector<int> dp0(n+12,0),dp1(n+12,0);
		dp0[0]=1;
		ll Z=0;
		for(int i=0;i<n;i++) {
			int x; cin>>x;
			if(x>n+5) continue;
			// int zp2=dp1[x];
			int zp1=(dp0[x]+dp0[x+1])%P;
			int zp1o=(dp1[x+1])%P;
			int zm1=x?dp1[x-1]+dp0[x-1]:0;
			zm1%=P;
			Z+=zp1+zm1;
			int zp2=0;
			Z+=zp2+zp1o;
			// dp0[x+2]+=zp2, dp0[x+2]%=P;
			dp0[x+1]+=zp1, dp0[x+1]%=P;
			if(x) dp1[x-1]+=zm1, dp1[x-1]%=P;
			dp1[x+1]+=zp1o, dp1[x+1]%=P;
		}
		cout<<Z%P<<endl;
	}
}