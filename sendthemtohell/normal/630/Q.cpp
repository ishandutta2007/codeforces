...
ld s3(ld l3) {
    const ld sq = sqrt(3.) / 2.;
    return sq * l3 * l3 * l3 * sqrt(1./6.) / 3.; 
}

ld s4(ld l4) {
    return l4 * l4 * l4 * sqrt(0.5) / 3.;
}
...
ld s5(ld l5) {
    ...
    ld si55 = sin(0.3 * PI);
    ld lr = l5 / si5 * si55;
    ld sosn = 5 * lr * lr * si5 / 2.;
    return sosn * sqrt(l5 * l5 - lr * lr) / 3;
}

inline void solve(ld tt) {
    ld l3, l4, l5;
    cin >> l3 >> l4 >> l5;
    cout << s3(l3) + s4(l4) + s5(l5) << endl;
}
