...
int main() {
    ...
    vector<string> S(H);
    cin >> S;
    dbg(make_tuple(H, W, P, Q, S));

    const int g0 = H * W;
    shortest_path<lint, INF> graph(g0 + 1);
    auto f = [&](int i, int j) { return i * W + j; };
    auto isin = [&](int i, int j) {
        return (i >= 0 and i < H and j >= 0 and j < W and S.at(i).at(j) != '#');
    };
    ...
    REP(i, H) REP(j, W) {
        if (S.at(i).at(j) == '.') {
            REP(dir, dx.size()) {
                ...
                if (isin(ni, nj) and S.at(ni).at(nj) == '.') answer(0);
            }

            graph.add_edge(g0, f(i, j), 0);

        }

        REP(dir, 4) {
            if (S.at(i).at(j) == Dirs.at(dir)) {
                const int ni1 = i + dx.at(dir), nj1 = j + dy.at(dir);
                {
                    int ni2 = i - dx.at(dir), nj2 = j - dy.at(dir);
                    if (isin(ni1, nj1) and isin(ni2, nj2)) graph.add_edge(f(ni2, nj2), f(ni1, nj1), Q);
                }

                REP(d1, 2) {
                    const int ni2 = i + dx.at(dir ^ 2 ^ d1), nj2 = j + dy.at(dir ^ 2 ^ d1);
                    if (isin(ni1, nj1) and isin(ni2, nj2)) graph.add_edge(f(ni2, nj2), f(ni1, nj1), P);
                }
            }
        }
    }
    ...
    REP(i, H) REP(j, W) {
        REP(dir, 4) {
            int ni = i + dx.at(dir), nj = j + dy.at(dir);
            if (isin(ni, nj)) chmin(ret, graph.dist[f(i, j)] + graph.dist[f(ni, nj)]);
        }
    }
    if (ret < INF) answer(ret);
    answer(-1);
}