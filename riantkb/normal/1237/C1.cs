...
class Solver {
    ...
    public void solve() {
        ...
        for (int i = 0; i < n; i++)
        {
            sc.Multi(out x[i], out y[i], out z[i]);
        }
        x = x.compressed();
        y = y.compressed();
        z = z.compressed();
        int xc = x.Length;
        var lis = new List<pair<P, int>>[xc];
        for (int i = 0; i < xc; i++)
        {
            lis[i] = new List<pair<P, int>>();
        }
        for (int i = 0; i < n; i++)
        {
            lis[x[i]].Add(new pair<P, int>(new P(y[i], z[i]), i));
        }
        var ans = new List<P>();
        int prev = -1;
        for (int i = 0; i < xc; i++)
        {
            lis[i].Sort();
            var nlis = new List<pair<P, int>>();
            for (int j = 0; j < lis[i].Count; j++)
            {
                if (j + 1 < lis[i].Count && lis[i][j].v1.v1 == lis[i][j + 1].v1.v1) {
                    ans.Add(new P(lis[i][j].v2 + 1, lis[i][j + 1].v2 + 1));
                    ++j;
                }
                else {
                    nlis.Add(lis[i][j]);
            ...
            for (int j = 0; j + 1 < nlis.Count; j += 2)
            {
                ans.Add(new P(nlis[j].v2 + 1, nlis[j + 1].v2 + 1));
            }
            if (nlis.Count % 2 == 1) {
                if (prev != -1) {
                    ans.Add(new P(prev, nlis[nlis.Count - 1].v2 + 1));
                    prev = -1;
                }
                else {
                    prev = nlis[nlis.Count - 1].v2 + 1;
                }
            }
        }
        // Assert(prev == -1);
        // Assert(ans.Count == n / 2);
