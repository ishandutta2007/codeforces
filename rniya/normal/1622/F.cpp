...
int main() {
    ...
    auto base = ZobristHash(n + 1);
    auto min_factor = elementary_math::osa_k(n);
    vector<uint64_t> fact(n + 1, 0);
    uint64_t sum = 0;
    for (int i = 1; i <= n; i++) {
        auto factor = elementary_math::prime_factor(min_factor, i);
        fact[i] = fact[i - 1];
        for (int& x : factor) fact[i] ^= base[x];
        sum ^= fact[i];
    }
    ...
    if (sum == 0) {  // n
        ...
        if (exc > 0) {
            ...
            map<uint64_t, int> mp;
            for (int i = 1; i <= n; i++) mp[fact[i]] = i;
            int exc1 = -1, exc2 = -1;
            for (int i = 1; i <= n; i++) {
                if (mp.count(sum ^ fact[i])) {
                    exc1 = i;
                    exc2 = mp[sum ^ fact[i]];
                }
            }
