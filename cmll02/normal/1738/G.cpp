...
signed main()
{
	...
	int T=read();rg(T)
	int n=read(),m=read()-1;
	rg(n)rg_(j,n)b[i][j]=0;gr gr
	rg(n)rg_(j,n)a[i][j]=re1d();
	if(a[i][j]==0)a[i][j]=a[i-1][j-1]+1,b[i][j]=0;
	else a[i][j]=0;
	...
	for(int i=k;i>=0;i--)
	{
		...
		while(1)
		{
			if(a[x][y]==i)
			{
				if(y==n||a[x-1][y]==i||b[x][y+1]==1)x--;
				else y++;
			}
			else if(y==n||b[x][y+1])x--;
			else y++;
			if(x==0||y>n)break;
			b[x][y]=1;
// printf("  ");			odp(x,y);
	...
	rg(n)rg_(j,n)od(!!b[i][j]);gr puts("");gr
	gr
	return 0;
}