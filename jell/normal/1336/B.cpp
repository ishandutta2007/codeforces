...
struct solver
{
    ...
    solver()
    {
        ...
        // g is mid
        {
            for(auto ig=begin(g),ir=begin(r),ib=begin(b); ig!=end(g); ++ig)
            {
                while(next(ir)!=end(r) && *next(ir)<=*ig) ir++;
                if(*ir>*ig) continue;
                while(ib!=end(b) && *ib<*ig) ++ib;
                if(ib==end(b)) break;
                chmin(ans, squ(*ib-*ig)+squ(*ib-*ir)+squ(*ir-*ig));
            }
            swap(r,b);
            for(auto ig=begin(g),ir=begin(r),ib=begin(b); ig!=end(g); ++ig)
            {
