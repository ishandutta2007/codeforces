...
mint perm(int n, int k) {
  ...
  while(tt--) {
    ...
    rrep(i, n) sb[i] = sb[i+1] + a[i];
    int half_cnt = 0;
    if (sf[n] % 2 == 0) {
      for (int i = 1; i < n; i++) {
        half_cnt += sf[i] == sf[n] / 2;
      }
    }
    mint ans = 1;
    int p = n - 1;
    for (int i = 1; i < n && sf[i] < (sf[n] + 1) / 2;) {
      // cout << "a" << i << endl;
      int j = i + 1;
      while(sf[i] == sf[j]) j++;
      // cout << "a" << i << endl;
      ...
      if (sb[p] == sf[i]) {
        ...
        int cnt2 = p - q;
        mint v;
        for(int c = 0; c <= min(cnt1, cnt2); c++) {
          v += comb(cnt1, c) * comb(cnt2, c);
        }
        ans *= v;
        q = p;
      }
