...
void solve() {
    ...
    V<int> a(n);
    {
        ...
        for (int i = 0; i < n; i++) {
            a[i] = int(
                lower_bound(a2.begin(), a2.end(), a3[i]) - a2.begin()
            );
        }
    }

    atcoder::segtree<S, op, e> rmq(n);
    for (int i = 0; i < n; i++) {
        rmq.set(a[i], i);
    }
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        dp[i] = 1;
        while (true) {
            int j = rmq.prod(ma + 1, a[i]);
            if (j == -1) break;
            rmq.set(a[j], -1);
//        }
//        for (int j = i - 1; j >= 0; j--) {
//            if (!(ma < a[j] && a[j] < a[i])) continue;
            dp[j] = fw.sum(a[j], a[i]) + 1;
            fw.add(a[j], dp[j]);
        }
        ma = a[i];
    }
