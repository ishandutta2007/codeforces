#line 1 "codeforces-workspace/contest/1495/c/c.cpp"
...
#line 11 "codeforces-workspace/contest/1495/c/c.cpp"
...
void main() {
  ...
  bool flag = 0;
  for (auto i : range((grid.size() - 1) / 3 * 3 + 1)) {
    auto &row = grid[i];
    if (i % 3) {
      if (flag) {
        if (auto c = row.find('X'); ~c) {
          flag = false;
          grid[i][c] = 'X';
          auto j = i % 3 == 1 ? i + 1 : i - 1;
          if (j < n) grid[j][c] = 'X';
        }
      }
    } else {
      if (flag) {
        grid[i - 1][0] = grid[i - 2][0] = 'X';
      }
      fill(begin(row), end(row), 'X');
      flag = 1;
    }
  }

  if (grid.size() % 3 == 0) {
    for (auto &&[j, c] : enumerate(grid.back())) {
      if (c == 'X') {
        grid[grid.size() - 2][j] = 'X';
      }
    }
  }
