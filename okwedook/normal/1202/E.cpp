...
signed main() {
    ...
    int n;
    cin >> n;
    map<int, map<zet, int>> len;
    for (int i = 0; i < n; ++i) {
        string t;
        cin >> t;
        ++len[sz(t)][myhash(t).h.back()];
    }
    vector<ll> start(sz(s)), end(sz(s));
    for (auto &i : len) {
        for (int j = 0; j + i.f <= sz(s); ++j)
            if (i.s.count(h.get(j, j + i.f - 1))) {
                ll val = i.s[h.get(j, j + i.f - 1)];
                start[j] += val;
                end[j + i.f - 1] += val;
            }
    }
