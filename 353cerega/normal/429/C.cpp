...
void F(int msk, int p, int lft, int k)
{
    if (lft<a[p]) return;
    F(msk,p+1,lft,k);
    if ((1<<p)&msk) return;
    if (p>0 and a[p]==a[p-1] and (msk&(1<<(p-1)))==0) return;
    if (lft==a[p] and k>0)
    {
        nxt.push_back(msk+(1<<p));
        return;
    }
    F(msk+(1<<p),p+1,lft-a[p],k+1);
}

void rec(int msk, int p)
{
    if (ok==1) return;
    if (was.count(msk)!=0) return;
    was[msk] = 1;
    if (p==n)
    {
        if (msk==(1<<(n-1))-1) ok = 1;
