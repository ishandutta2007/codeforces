...
fun Graph.bellmenford(root:Int): IntArray? {
    val inf = plarge
    ...
    for(phase in 0 until n){
        ...
        for(j in 0 until edgecount){
            if(dist[from[j]] < inf){
                if(dist[to[j]] > dist[from[j]] + weights[j]){
                    dist[to[j]] = maxOf(-inf, dist[from[j]] + weights[j])
                    any = true
                    x = to[j]
                }
            }
...
fun main(){
    solve.cases{9
        ...
        var best = -1
        var construction:IntArray? = null
        for(v in 0 until n){
            val d = H.bellmenford(v) ?: break
            for(i in 0 until G.edgecount step 2){
                val a = G.from[i]
                val b = G.to[i]
                val t = G.weights[i shr 1]
                if(t == 1){
                    if(d[b] != d[a] + 1) continue
                }else{
                    if((d[a] - d[b]).absoluteValue != 1) continue
                }
            }
            val dif = d.maxOrNull()!! - d.minOrNull()!!
            if(dif > best){
                best = dif
                construction = d
            }
        ...
        put("YES")
        put(best)
        val min = construction.minOrNull()!!
        for(i in 0 until n) {
            construction[i] += -min
        }
        put(construction.conca)
...
directly BFS or contract

directly BFS is enough
either solve contraction solve edges, or find good bfs methoh
