...
void inc(int a, int b, int i) {
	if (a > b) swap(a, b);
	ord[pii(a, b)].push_back(i);
}
const int nax = 1e5 + 44;
vector <int> tree[nax], cycle[nax];
vector <int> dfs_ord;
...
int main() {
	...
	REP(_, t) {
		...
		for (auto &[edge, nums] : ord) {
			if (nums.size() == 1u) {
				cycle[edge.first].push_back(edge.second);
				cycle[edge.second].push_back(edge.first);
			}
			else if (nums.size() == 2u) {
				debug(imie(nums), "is tree edge");
				tree[nums[0]].push_back(nums[1]);
				tree[nums[1]].push_back(nums[0]);
			}
			else
				assert(false);
		}
		FOR(i, 1, n) assert(cycle[i].size() == 2u);
		{
			int p = 1, x = -1;
			do {
				printf("%d ", p);
		...
		dfs(0);
		for (int x : dfs_ord) printf("%d ", x + 1);
		printf("\n");
		REP(i, n + 1) {
			tree[i].clear();
			cycle[i].clear();
