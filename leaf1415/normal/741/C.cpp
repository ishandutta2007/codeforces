...
int main(void)
{
	...
	rep(i, 1, 2*n) if(i % 2) G[i].push_back(i+1), G[i+1].push_back(i);
	rep(i, 1, n) G[a[i]].push_back(b[i]), G[b[i]].push_back(a[i]);
	...
	rep(i, 1, 2*n){
		...
		do{
			...
			cnt++;
			ans[v] = cnt%2+1;
			if(used[G[v][0]]) v = G[v][1];
			else v = G[v][0];
		}while(!used[v]);
	}
	rep(i, 1, n) outl(ans[a[i]], ans[b[i]]);
