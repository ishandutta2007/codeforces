...
public:
    void solve(istream &inp, ostream &outp) {
        ...
        struct payment {
            ...
            payment(int u, int v, ll d) : u(u + 1), v(v + 1), d(d) {}
        };
        vec<payment> answer;
        for (int i : range(n)) {
            if (balance[i] > 0) {
                while (balance[i] > 0 && !neg.empty()) {
                    ll qty = min(balance[i], -balance[neg.back()]);
                    answer.emplace_back(neg.back(), i, qty);
                    balance[i] -= qty;
                    balance[neg.back()] += qty;
                    if (balance[neg.back()] == 0) {
                        neg.pop_back();
                    }
                }
                if (balance[i] > 0) {
                ...
                while (balance[i] < 0 && !pos.empty()) {
                    ll qty = min(-balance[i], balance[pos.back()]);
                    answer.emplace_back(i, pos.back(), qty);
                    balance[i] += qty;
                    balance[pos.back()] -= qty;
                    if (balance[pos.back()] == 0) {
                        pos.pop_back();
                    }
                }
                if (balance[i] < 0) {
