...
const int mod0 = (int)1e9 + 403, seed0 = (int)1e5 + 19; // seed > maxn && seed is a primitive root and prime
...
int pw0[maxn], ivs0[maxn];
struct Hash {
	int len, h0;
	bool operator == (Hash const &t) const {
		return len == t.len && h0 == t.h0;
	}
		...
		if(typ == 1) {
			...
			while(tot--) {
				...
				while(okU.len < seg[u].hR.len && rem > 0) {
					...
					Hash nxtU = seg_right_hash(uL, uR, com + okU.len);
					Hash nxtV = seg_left_hash(vL, vR, com + okV.len);
