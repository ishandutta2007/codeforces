// Author:  HolyK
// Created: Fri Mar  4 21:18:07 2022
#include <bits/stdc++.h>
...
void solve() {
  ...
  if (n & 1) {
    ...
    b[0] = m + 1;
    for (int i = 1; i < t; i++) {
      a[i] = 1LL * (i + m) * (m - i + 1 + P) % P * a[i - 1] % P;
      b[i] = 1LL * b[i - 1] * (i + 1 + m) % P * (m - i + 1 + P) % P;
    }
    for (int i = 1; i < t; i++) {
      a[i] = 1LL * a[i] * ifac[i * 2] % P;
      b[i] = 1LL * b[i] * ifac[i * 2 + 1] % P;      
    }
    ...
    b[0] = c;
    // for (int i = 0; i < t; i++) {
    //   std::cerr << i * 2 + 1 << " " << (i & 1 ? (P - b[i]) % P : b[i]) << "\n";
    // }
    // std::cerr << b[0] << " " << m << "\n";
    Poly p(n + 1);
    for (int i = 0; i < t; i++) {
      p[i * 2 + 1] = i & 1 ? P - b[i] : b[i];
    }
    Poly r = (Poly{1} - p).inv(n + 1);
    for (int i = 0; i <= n; i++) {
      p[i] = 1LL * p[i] * i % P;
    }
    p = p * r;
