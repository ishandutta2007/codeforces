...
int main() {
    ...
    rep(_, f) {
        ...
        auto E = flow.edges();
        rep(i, si(E)) {
            if((E[i].from == s or E[i].to == t) and E[i].flow) {
                auto F = tflow;
                F.change_edge(i, 0, 0);
                if(F.flow(s, t) == target) {
                    tflow.change_edge(i, 0, 0);
                    if(E[i].from == s) {
                        v.eb(E[i].to + 1);
                    } else
                        v.eb(-(E[i].from - n + 1));
    ...
    rep(i, k) {
        ...
        rep(j, n + 1) {
            rep2(a, max(j, i + 2), n) {
                if(chmax(dp[i + 1][a], dp[i][j] + max(0LL, x - y * (a - j)))) pre[i + 1][a] = j;
            }
        }
    }
    int i = k, now = k + 1;
