...
signed main(){
	...
	auto add=[&](int i){
		auto itr=s[a[i]].lower_bound(i);
		int x=-inf,y=inf;
		if(itr!=s[a[i]].ed)y=*itr;
		if(itr!=s[a[i]].bg)x=*prev(itr);
		if(0<=x&&y<n){
			qs.pb({x,y,x-y});
		}
		if(0<=x)qs.pb({x,i,i-x});
		if(y<n)qs.pb({i,y,y-i});
		s[a[i]].insert(itr,i);
	};
	
	auto del=[&](int i){
		auto itr=s[a[i]].find(i);
		assert(itr!=s[a[i]].ed);
		itr=s[a[i]].erase(itr);
		int x=-inf,y=inf;
		if(itr!=s[a[i]].ed)y=*itr;
		...
		if(0<=x)qs.pb({x,i,x-i});
		if(y<n)qs.pb({i,y,i-y});
	};
