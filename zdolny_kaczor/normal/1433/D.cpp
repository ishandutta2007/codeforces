...
void solve() {
	...
	bool same = true;
	REP(i, n) if (in[i].first != in[0].first) same = false;
	if (same) {
		puts("NO");
		return;
	}
	...
	vpii reord;
	int wh_diff = -1;
	REP(i, n) if (in[i].first != in[0].first) wh_diff = i;
	assert(wh_diff != -1);
	reord.push_back(in[0]);
	reord.push_back(in[wh_diff]);
	REP(i, n) if (i != 0 && i != wh_diff) reord.push_back(in[i]);
	vpii ans = {{0, 1}};
	for (int i = 2; i < n; ++i) if (reord[i].first != reord[0].first) ans.emplace_back(i, 0);
	else ans.emplace_back(i, 1);
	for (auto [a, b] : ans) {
		printf("%d %d\n", 1 + reord[a].second, 1 + reord[b].second);
	}
}
int main() {
