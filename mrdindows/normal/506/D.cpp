...
int main()
{
	...
	for (int c = 0; c < m; ++c){
		if (ribs[c].size() > 300){
			...
			curc = 1;
			for (auto rib : ribs[c]){
				v[rib.first].push_back(rib.second);
				v[rib.second].push_back(rib.first);
			}
			FI(n){
				if (!used[i]){
					go(i);
	...
	while (q--){
		...
		res += upper_bound(all(e[a]), b) - lower_bound(all(e[a]), b);
		cout << res << '\n';
	}
	return 0;
}