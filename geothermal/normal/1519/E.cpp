...
inline namespace Output {
    ...
    template<class ...T> void prn(const T&... t) { pr_sep(cout,"",t...); pr_sep(cout,"",'\n'); }
	// print w/ spaces
	template<class ...T> void ps(const T&... t) { pr_sep(cout," ",t...); }
    template<class ...T> void psn(const T&... t) { pr_sep(cout," ",t...); pr_sep(cout, "", '\n'); }
	// debug to cerr
...
int get(pl A, pl B) {
    ll X = B.f * A.s, Y = B.s * A.f;
    ll G = __gcd(X, Y);
    X /= G; Y /= G;
    if (!comp.count({X, Y})) {
        comp.ins({{X, Y}, sz(comp)});
    }
    return comp[{X, Y}];
}
...
bool dfs(int v, int p = -1) {
    ...
    trav(a, graph[v]) {
        if (dep[a.f] == -1) {
            dep[a.f] = dep[v] + 1;
            if (dfs(a.f, a.s)) ops.pb(a.s);
        } else if (dep[a.f] > dep[v]) {
            ops.pb(a.s);
        }
    }
    bool res = true;
    ...
    F0R(i, sz(ops)/2) {
        if (i*2+1 >= sz(ops)) break;
        ans.pb({ops[i*2], ops[i*2+1]});
    }
    return res;
}
