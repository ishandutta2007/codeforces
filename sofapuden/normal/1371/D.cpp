...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		for(int j = 0; j < n; ++j){
			grid.validate(j,(i+j)%n);
			cn++;
			if(cn == k)goto print;
		}
	}
	print:
	for(int i = 0; i < sz(grid.g); ++i){
		for(int j = 0; j < sz(grid.g[i]); ++j){
			col[i]+=grid.g[i][j].cost;
			row[j]+=grid.g[i][j].cost;
		}
	}
	int min1 = 500, min2 = 500, max1 = 0, max2 = 0;
	for(int i : col){
		min1 = min(min1, i);
		max1 = max(max1, i);
	}
	...
	cout << (max1-min1)*(max1-min1) + (max2-min2)*(max2-min2) << "\n";		
	for(int i =0 ; i< sz(grid.g); ++i){
		  for(int j = 0; j < sz(grid.g[i]); ++j){
			  cout << grid.g[i][j].cost;
		  }
		  cout << "\n";
	 }		  
