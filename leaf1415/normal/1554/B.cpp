...
{
	P ret;
	if(a.first > b.first) ret.first = a.first, ret.second = max(a.second, b.first);
	else ret.first = b.first, ret.second = max(b.second, a.first);
	return ret;
}
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 0, n){
			reverse(all(vec[i]));
			if(sz(vec[i]) >= 2) chmax(ans, vec[i][0]*vec[i][1] - k * i);
		}
		
		ll logn = digitnum(n, 2), N = 1<<logn;
		rep(i, 0, N-1){
			p[i] = P(0, 0);
			if(sz(vec[i]) >= 1) p[i].first = vec[i][0];
			if(sz(vec[i]) >= 2) p[i].second = vec[i][1];
		}
		zeta_transform(p, logn);
