...
void solve() {
    ...
    vi digits;
    for (int v = s; sz(digits) < Z; v /= 10)
        digits.push_back(v % 10);
    pd(digits);

    int digit_sum = accumulate(all(digits), 0);
    while (digit_sum < n) {
        for (int i = 1; i < Z; i++) {
            if (digits[i] > 0) {
                digits[i - 1] += 10;
    ...
    vvi ans_split(n, vi(Z));
    for (int i = 0; !items.empty(); i = (i + 1) % n) {
        ans_split[i][items.back()]++;
        items.pop_back();
    }
