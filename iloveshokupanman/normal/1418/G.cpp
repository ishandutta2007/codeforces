...
P op(P u, P v) {
    ...
    if (u.first > v.first)return v;
    return { u.first, u.second + v.second };
}
P e() { return { INF,0 }; }
P mp(ll x, P p) {
...
int main() {
    ...
    rep(i, N) {
        ...
        rep(j, L) {
            rep(k, 5) {
                if (j + k < L)nx[lis[i][j]][k] = lis[i][j + k];
            }
        }
    }
    set<ll> st;
    lazy_segtree<P, op, e, ll, mp, comp, id> sg(N);
    rep(i, N)sg.set(i, { 0,1 });
    rep(i, N) {
        if (lis[i].size() > 0) {
            ll x = lis[i][0];
            //cout << nx[x][0] << " " << nx[x][2] << endl;
            sg.apply(nx[x][0], nx[x][2], 1);
           st.insert(nx[x][3]);
        }
    }
    ll ans = 0;
    rep(i, N) {
        P p = sg.prod(i,*st.begin());
        if (p.first == 0)ans += p.second;

        sg.apply( nx[i][0], nx[i][2], -1);
        sg.apply(nx[i][1], nx[i][3], 1);
        st.erase(nx[i][3]);
        st.insert(nx[i][4]);
    }
    cout << ans << endl;
