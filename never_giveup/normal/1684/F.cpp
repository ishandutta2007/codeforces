...
void solve() {
    ...
    sort(all(segs));
    vector<int> rmax(m + 1, -1);
    for (int i = 0; i < m; i++)
        rmax[i + 1] = max(rmax[i], segs[i].sc);
    auto check = [&](int l, int r) {
        int it = low_bo(segs, make_pair(l + 1, 0));
        return rmax[it] >= r;
    };
    int slow = n;
    if (0) {
        ...
        if (slow != 0) {
            ...
            for (int L = 0; L < n; L++) {
                for (int R = L; R < n; R++) {
                    ...
                    for (int i = 0; i < n; i++)
                        for (int j = i + 1; j < n; j++)
                            if ((i < L || i > R) && (j < L || j > R) && a[i] == a[j] && check(i, j))
                                can = 0;
                    if (can) {
                        setmin(slow, R - L + 1);
                        setmax(ps[R], L);
    ...
    auto can = [&](int i) -> bool {
        int w = a[i];
        auto it = q[w].lower_bound(i);
        if (!q[w].empty() && it != q[w].begin() && check(*prev(it), i))
            return 0;
        if (!q[w].empty() && it != q[w].end() && check(i, *it))
            return 0;
        q[w].insert(i);
        return 1;
    };
