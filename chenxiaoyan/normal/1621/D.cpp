...
void mian() {
	n = read();
	REP(i, 1, 2 * n) REP(j, 1, 2 * n) a[i][j] = read();
	int ans = 0;
	REP(i, n + 1, 2 * n) REP(j, n + 1, 2 * n) ans += a[i][j];
	int arr[] = {a[1][n + 1], a[1][2 * n], a[n][n + 1], a[n][2 * n],
	             a[n + 1][1], a[2 * n][1], a[n + 1][n], a[2 * n][n]};
	ans += *min_element(arr, arr + 8);
	prt(ans), pc('\n');
}
