...
int main() {
    TEST {
        ...
        VEC(string, s, 3);
        int z[3] = {};
        rep(i, 3) z[i] = count(all(s[i]), '0');
        int Z = 0;
        rep(i, 3) Z += (z[i] * 2 >= n * 2);
        bool rev = false;
        if(Z <= 1) {
            rev = true;
            rep(i, 3) for(auto &e : s[i]) e = e ^ '1' ^ '0';
            rep(i, 3) z[i] = count(all(s[i]), '0');
        }
        vector<string> t;
        rep(i, 3) {
            if(z[i] * 2 >= n * 2 and t.size() < 2) t.eb(s[i]);
        }
        string ans;
        while(!empty(t[0]) or !empty(t[1])) {
            int c = 0, d = 0;
            while(!empty(t[0]) and t[0].back() == '1') c++, t[0].pop_back();
            while(!empty(t[1]) and t[1].back() == '1') d++, t[1].pop_back();
            rep(_, max(c, d)) ans.pb('1');
            bool f = false;
            rep(i, 2) if(!empty(t[i])) t[i].pop_back(), f = true;
            if(f) ans.pb('0');
        }
        reverse(all(ans));
        if(rev)
            for(auto &e : ans) e = e ^ '1' ^ '0';
        while(si(ans) < n * 3) ans.pb('0');
        cout << ans << endl;
    }
}