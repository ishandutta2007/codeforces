...
int n, k, ans;
int a[N];
int neg[N], pos[N]; int cntneg, cntpos;
...
signed main(){
    ...
    sort(neg + 1, neg + 1 + cntneg);
    sort(pos + 1, pos + 1 + cntpos);
    ...
    FordE(i, cntneg, 1){
        int val1 = sumpos, valOther = pq.top().fi;
        // cout << neg[i] << ' ' << val1 << ' ' << valOther << endl;
        if (val1 > valOther){
            sumpos += neg[i];
            ans += val1;
        }
            ...
            sumOther[cac.se] += neg[i];
            pq.push(make_pair(sumOther[cac.se], cac.se));
            ans += valOther;
        }
    }
    cout << ans;
