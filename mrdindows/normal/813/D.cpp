...
int dp[5050][5050];
int simple[5050];
array<int, 3> nextP1[5050];
array<int, 3> nextM1[5050];
array<int, 3> next7[5050];
...
int go(int i, int j) {
    ...
    int res = simple[j] - 1;
    pii p;
    for (auto& p : {nextP1[i], nextM1[i], next7[i]}) {
        for (auto to : p) {
            if (to != -1 && to != j) {
                res = max(res, 1 + go(to, j));
            }
        }
    }
    dp[i][j] = res;
...
int main() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        int x = a[i];
        nextP1[i] = getVec(pos[x + 1]);
        nextM1[i] = getVec(pos[x - 1]);
        next7[i] = getVec(bymod[x % 7]);
        pos[x].push_back(i);
        bymod[x % 7].push_back(i);
        simple[i] = 1;
        for (auto& p : {nextP1[i], nextM1[i], next7[i]}) {
            for (int to : p) {
                if (to != -1) {
                    simple[i] = max(simple[i], 1 + simple[to]);
                }
            }
        }
    }
