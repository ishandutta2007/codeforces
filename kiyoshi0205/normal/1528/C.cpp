...
set<ll> se,leaf;
void solve(ll now=0){
  auto [itr,f]=se.emplace(g2.in[now]);
  assert(f);
  leaf.emplace(now);
  vec eraselist;
  if(now!=0){
    auto fitr=prev(itr);
    auto lca=g2.lca(g2.ETlist[*fitr],now);
    auto x=leaf.find(lca);
    if(x!=leaf.end()){
      eraselist.emplace_back(lca);
      leaf.erase(x);
    }
    ++fitr;++fitr;
    if(fitr!=se.end()){
      lca=g2.lca(g2.ETlist[*fitr],now);
      x=leaf.find(lca);
      if(x!=leaf.end()){
        eraselist.emplace_back(lca);
        leaf.erase(x);
  ...
  chmax(ans,ll(leaf.size()));
  for(auto x:g[now])solve(x);
  se.erase(itr);
  for(auto x:eraselist)leaf.emplace(x);
  leaf.erase(now);
}
int main(){
  startupcpp();
