#include<bits/stdc++.h>
using namespace std;
const int MAXN=20000,INF=1e9,MAXM=100000;
...
int main(){
   ...
   for(int i=0;i<n;i++)
      for(int j=0;j<m;j++){
         if((i+j)&1){
            addedge(i*m+j,m*n+1,1,0);
            continue;
         }
         else{
            addedge(m*n,i*m+j,1,0);
         }
         if(i!=0){
            if(graph[i][j]==graph[i-1][j])
               addedge(i*m+j,(i-1)*m+j,1,0);
            else
               addedge(i*m+j,(i-1)*m+j,1,1);
         }
         if(i!=n-1){
            if(graph[i][j]==graph[i+1][j])
               addedge(i*m+j,(i+1)*m+j,1,0);
         ...
         if(j!=0){
            if(graph[i][j]==graph[i][j-1])
               addedge(i*m+j,i*m+j-1,1,0);
            else
               addedge(i*m+j,i*m+j-1,1,1);
         }
         if(j!=m-1){
            if(graph[i][j]==graph[i][j+1])
               addedge(i*m+j,i*m+j+1,1,0);
