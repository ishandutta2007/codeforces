#line 1 "codeforces-workspace\\contest\\1406\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 14 "codeforces-workspace\\contest\\1406\\c\\c.cpp"
namespace workspace {
constexpr char eol = '\n';
using namespace std;
...
struct workspace::solver {
  solver() {
    ...
    auto dfs = fixed_point([&](auto dfs, int now, int pre) -> int {
      sz[now] = 1;
      par[now] = pre;
      for (int to : tr[now])
        if (pre != to) {
          int cz = dfs(to, now);
          sz[now] += cz;
          if (cz * 2 == n) a = to, b = now, uni = false;
        }
      return sz[now];
    });
    dfs(0, -1);
    if (!uni) {
      sz.assign(n, 0);
      dfs(b, a);
      auto fi = find(begin(sz), end(sz), 1);
      assert(fi != end(sz));
      int c = fi - begin(sz);
      cout << c + 1 << " " << par[c] + 1 << eol;
      cout << c + 1 << " " << a + 1 << eol;
    } else {
      cout << 1 + a << " " << b + 1 << "\n";
      cout << a + 1 << " " << 1 + b << "\n";
