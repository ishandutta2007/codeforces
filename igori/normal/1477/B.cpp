...
struct tree{
    ...
    void Push(int L, int R, int V)
    {
        if (push[V] != -1)
        {
            ...
            summ[2 * V + 1] = push[V] * (M - L);
            summ[2 * V + 2] = push[V] * (R - M);
            push[V] = -1;
        }
    }
    void Set(int l, int r, int x, int L, int R, int V)
    {
        if (r <= L || R <= l) return;
        if (l <= L && R <= r)
        {
    ...
    int Get(int l, int r)
    {
        return Get(l, r + 1, 0, summ.size() / 4, 0);
    }
};
