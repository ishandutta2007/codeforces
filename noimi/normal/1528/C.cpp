    ...
    TEST {
        ...
        vi in(n), out(n), in2(n), out2(n);
        {
            ...
            REC([&](auto &&f, int x, int p) -> void {
                in[x] = t++;
                in2[x] = t2++;
                for(auto e : G[x]) { f(e, x); }
                out[x] = t++;
                out2[x] = t2;
            })
            (0, -1);
        }
        RST<int> seg(n * 2), seg2(n);
        ...
        REC([&](auto &&f, int x, int p) -> void {
            int erase = -1, add = -1;
            if(!seg2.query(in2[x], out2[x])) {
                now++;
                int k = seg.query(0, in[x]);
                add = x;
                if(k) {
                    now--;
                    seg.update(in[k - 1], 0);
                    seg.update(out[k - 1], 0);
                    seg2.update(in2[k - 1], 0);
                    erase = k;
                }
                seg.update(in[x], x + 1);
                seg.update(out[x], -x - 1);
                seg2.update(in2[x], x + 1);
                chmax(ans, now);
            }
            for(auto e : g[x]) f(e, x);
            if(erase != -1) {
                int k = erase;
                seg.update(in[k - 1], k);
                seg.update(out[k - 1], -k);
                seg2.update(in2[k - 1], k);
                now++;
            }
            if(add != -1) {
                seg.update(in[add], 0);
                seg.update(out[add], 0);
                seg2.update(in2[add], 0);
                now--;
            }
        })
        (0, -1);
