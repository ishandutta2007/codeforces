...
void solve() {
    ...
    umap<int, int> ct;
    F0R (i, N) { int v; IO::read_int(v); ++ct[v]; }

    int least = INT_MAX;
    trav (e, ct) ckmin(least, e.s);
    ...
    FOR (s, 2, least + 2) {
        ...
        trav (e, ct) {
            ...
            int num_small = need % s;
            if (num_small) num_small = s - num_small;

            int pages = (need + num_small) / s;
            if (pages < num_small) { screens = INT_MAX; break; }
