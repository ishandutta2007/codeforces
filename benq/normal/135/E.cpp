...
mi p(int a, int b) {
	if (a < b) return 0;
	return C.fac[a]*C.ifac[a-b];
}

mi solve(int w, bool bad = 0) {
	// first z must be distinct, last z must be distinct
	mi ret = 0;
	FOR(n,w+1,w+k+1) {
		if (bad && n == w+1) continue;
		int z = n-w;
		mi cur = p(k,z); // 1 <= n-w <= k
		if (2*z <= n) cur *= p(k,z)*pow(mi(k),n-2*z);
		else cur *= p(k-(2*z-n),n-z);
		ret += cur;
		// ps("HUH",n,w,cur);
	}
	return ret;
