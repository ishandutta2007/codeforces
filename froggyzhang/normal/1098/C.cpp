...
int n,lim,cnt[N],dep[N],qwq[N],f[N],son[N];
vector<int> tmp[N];
ll m;
...
int main(){
	n=read(),m=read();
	if(m>1LL*n*(n+1)/2||m<2*n-1){
		return !printf("No\n");
	}
	lim=1;
	while(m<calc(lim))++lim;
	printf("Yes\n");
	ll now=m-calc(lim);
	ll tot=0;
	for(int i=1,j=1,g=1;i<=n;++g){
		for(int k=1;k<=j;++k){
			dep[i]=g;
			++i;	
		}
		qwq[i-1]=1;
		j=min(1LL*j*lim,1LL*n-i+1);
	}
	for(int i=1;i<=n;++i)tot+=dep[i];
	int las=n;
	for(int i=n-1;i>=1&&now;--i){
		if(qwq[i])continue;
		if(now>=dep[las]+1-dep[i]){
			now-=dep[las]+1-dep[i];
			dep[i]=dep[las]+1;
		}
		else{
	...
	for(int i=1;i<=n;++i)tmp[dep[i]].push_back(i);
	for(int i=2;i<=n;++i){
		f[i]=tmp[dep[i]-1].back();
		++son[f[i]];
		if(son[f[i]]==lim)tmp[dep[i]-1].pop_back();
	}
	for(int i=2;i<=n;++i){
		printf("%d ",f[i]);
	}
