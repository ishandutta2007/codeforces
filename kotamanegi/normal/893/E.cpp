...
vector<modint<MAX_MOD>> mae, gyaku;
...
void solve(){
    mae.push_back(modint<MAX_MOD>(1));
    gyaku = mae;
    for (int q = 1; q <= 2e6; ++q) {
        mae.push_back(mae.back() * modint<MAX_MOD>(q));
        gyaku.push_back(modint<MAX_MOD>(1) / mae.back());
    }
    int n;
    cin >> n;
    REP(tea, n) {
        ...
        for (int q = 2; q * q <= x; ++q) {
            ...
            if (cnt == 0) continue;
            ans *= comb(y + cnt-1, cnt);
        }
        if (x != 1) {
            ans *= modint<MAX_MOD>(y);
        }
        if (y != 1) {
            ans *= pows<modint<MAX_MOD>>(modint<MAX_MOD>(2), y-1);
        }
        cout << ans << endl;
    }
}
