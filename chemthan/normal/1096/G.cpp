...
namespace FFT {
    ...
    void multiply(int a[], int b[], int na, int nb, int c[], int dup, int mod) {
        int n = na + nb - 1;
        while (n != (n & -n)) n += n & -n;
        for (int i = 0; i < n; i++) fa[i] = fb[i] = cp();
    ...
    vector<int> multiply(vector<int> a, vector<int> b, int dup, int mod) {
        static int fa[maxf], fb[maxf], fc[maxf];
        int na = a.size(), nb = b.size();
        for (int i = 0; i < na; i++) fa[i] = a[i];
        for (int i = 0; i < nb; i++) fb[i] = b[i];
        multiply(fa, fb, na, nb, fc, dup, mod);
        int k = na + nb - 1;
        vector<int> res(k);
    ...
    vector<int> invert(vector<int> a, int n, int mod){
        ...
        while (x.size() < n) {
            ...
            vector<int> nx = multiply(multiply(x, x, 1, mod), tmp, 0, mod);
            x.resize(2 * x.size());
    ...
    pair<vector<int>, vector<int> > divmod(vector<int> a, vector<int> b, int mod) {
        ...
        vector<int> d = multiply(a, rb, 0, mod);
        reverse(a.begin(), a.end());
        reverse(b.begin(), b.end());
        while (d.size() > n - m + 1) d.pop_back();
        reverse(d.begin(), d.end());
        vector<int> r = multiply(d, b, 0, mod);
...
void chemthan() {
    ...
    vi vals(10);
    FOR(i, 0, k) {
        int x; cin >> x; vals[x] = 1;
    }
    vi res = vals;
    n >>= 1, n--;
    while (n) {
        if (n & 1) res = FFT::multiply(res, vals, 0, MOD);
        n >>= 1;
        if (!n) break;
        vals = FFT::multiply(vals, vals, 1, MOD);
    }
    int ans = 0;
    FOR(i, 0, sz(res)) {
        addmod(ans, mult(res[i], res[i]));
    }
    cout << ans << "\n";
}
