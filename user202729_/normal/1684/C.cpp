...
// should practice formally prove algorithm correctness (for Codeforces there's always test generator, although sometimes that fails as well.)
...
void up(){
	...
	std::vector<std::vector<int>> table(numRow, std::vector<int>(numCol));

	std::set<int> differentIndices;

	for(auto& row: table){
		for(auto& cell: row) std::cin>>cell;
		if(not std::is_sorted(begin(row), end(row))){
			auto sortedRow=row;
			std::sort(begin(sortedRow), end(sortedRow));
			for(int const col: range(0, numCol)){
				if(differentIndices.size()>=3) break;
				if(row[col]!=sortedRow[col]){
					differentIndices.insert(col);
				}
			}
		}
	}

	if(differentIndices.size()>=3){
cannot:
		std::cout<<"-1\n";
		return;
	}

	if(differentIndices.empty()){
		std::cout<<"1 1\n";
		return;
	}

	assert(differentIndices.size()==2);
	auto const a=*differentIndices.begin(), b=*differentIndices.rbegin();

	if([&]{
		for(auto& row: table){
			std::swap(row[a], row[b]);
			auto const rowGood=std::is_sorted(begin(row), end(row));
			std::swap(row[a], row[b]);

			if(not rowGood) return false;
		}
		return true;
	}()){
		std::cout<<a+1<<' '<<b+1<<'\n';
