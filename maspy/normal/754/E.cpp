...
void solve() {
  ...
  FOR(c, 26) {
    vc<BS> C(H1);
    FOR(x, H1) {
      FOR(y, W1) if (B[x][y] != 'a' + c && B[x][y] != '?') C[x][y] = 1;
    }
    FOR(x, H + H1) FOR(y, W + W1) {
      if (A[x % H][y % W] != 'a' + c) continue;

      FOR(row, H1) {
        ll add_to = x - row;
        if (add_to < 0 || H <= add_to) continue;
        BS mask = C[row];
        ll p = y - W1 + 1;
        if (p > 0)
          mask <<= p;
