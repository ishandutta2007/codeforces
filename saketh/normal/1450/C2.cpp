...
void solve() {
    ...
    array<int, 3> groups = { 0, 0 };
    F0R (d, sz(on_diag)) {
        groups[d % 3] += on_diag[d];
    }

    int exclude = max_element(all(groups)) - groups.begin();
    ...
    int handle = T - groups[exclude];
    int mode = need_flip > (handle - need_flip);
