...
void solve() {
    string s, t;
    read(s, t);
    vector<vector<bool>> forward(sz(s), vector<bool>(sz(t) + 1));
    for (int i = 0; i < sz(s); ++i)
        forward[i][1] = s[i] == t[0];
    for (int i = 1; i < sz(s); ++i)
        for (int wascnt = 1; wascnt < sz(t); ++wascnt)
            if (s[i] == t[wascnt] && forward[i - 1][wascnt])
                forward[i][wascnt + 1] = true;
    auto &backward = forward;
    for (int i = sz(s) - 2; i >= 0; --i)
        for (int wascnt = 1; wascnt < sz(t); ++wascnt)
            if (s[i] == t[wascnt] && backward[i + 1][wascnt])
                backward[i][wascnt + 1] = true;
    for (int i = 0; i < sz(s); ++i)
        if (backward[i][sz(t)]) {
            println("YES");
            return;
