...
int main()
{
	...
	bool lose = true;
	ll ans1s, ans1e, ans2s, ans2e;
	for (int i = 0; lose && (i < w1); ++i)
	{
		if (m2[i] <= i + 1)
		{
			lose = false;
	...
	{
		cout << "Yes\n";
		cout << ans1e - ans1s << ' ' << ans2e - ans2s << '\n';
		for (int i = ans1s; i < ans1e; ++i)
			cout << v[i].Y << ' ';
		cout << '\n';
		for (int i = ans2s; i < ans2e; ++i)
			cout << v[i].Y << ' ';
		cout << '\n';
	}
