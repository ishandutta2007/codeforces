...
int main() {
  ...
  #endif
  n = read(), x = readl();
  for (ri i = 1; i <= n; ++i) a[n + 1 - i] = read();
  reverse(a + 1, a + n + 1);
  for (ri i = 1; i <= n; ++i) vl[i] = n + 1 - i;
  vl[0] = 1;
  for (ri i = 1; i <= n; ++i) ss[i] = ss[i - 1] + a[i], sss[i] = sss[i - 1] + S(a[i]);
  for (ri i = n + 1; i <= 3 * n; ++i) ss[i] = ss[i - 1] + (a[i] = a[i - n]), sss[i] = sss[i - 1] + S(a[i]);
  ll res = 0;
  for (ri i = 1; i <= n; ++i) {
    ll cur = x + ss[i - 1];
    int ps = lower_bound(ss + 1, ss + n * 3 + 1, cur) - ss;
    ckmax(res, sss[ps] - sss[i - 1] - S(ss[ps] - cur));
  }
  cout << res;
  return 0;
}