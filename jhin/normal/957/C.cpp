...
vector<int> vs;double an;
int main()
{
ios_base::sync_with_stdio(false);

    cin>>n>>m;
    for(int i=0;i<n;i++)cin>>x,vs.push_back(x);
    for(int i=0;i<n-2;i++)
    {
        auto it=lower_bound(vs.begin(),vs.end(),vs[i]+m+1);
        if(vs[i+2]-vs[i]>m)continue;
        it--;int temp=*it;
        an=max(an,(temp-vs[i+1])/double(temp-vs[i]));
    }
    if(an==0)an=-1;
