...
void solve(){
    ...
    REP(i, m) {
        ...
        inputs.push_back(make_tuple(a, b, c, i));
    }
    inputs.push_back(make_tuple(1e9, 0, 0, -1e9));
    sort(ALL(inputs));
    reverse(ALL(inputs));
    for (int i = 1; i <= n; ++i) {
        while (get<0>(inputs.back()) == i) {
            nexts.push(make_tuple(get<1>(inputs.back()), get<2>(inputs.back()), get<3>(inputs.back())));
            inputs.pop_back();
        }
        if (target[i].size() > 1) {
            cout << -1 << endl;
