...
bool judge(const vector<int> &vs){
	const int n=vs.size();
	tuple<int,int,int> cur{-2,-1,n};
	for(cauto &v:vs){
		// out(v,cur,1);
		cauto [a,b,c]=cur;
		if(a==-2){
			if(b<c && b<v && v<c) cur={b,v,c};
			else if(b<v) cur={-2,v,c};
			else if(v<c) cur={-2,b,v};
			else return false;
		}else{
			if(v<a) cur={-2,a,v};
			else if(c<v) cur={-2,v,c};
			else cur = b<v ? tuple{b,v,c} : tuple{a,v,b};
		}
	}
	return true;
}
...
int main(){
	...
	cauto n=read(0);
	cauto vs=readV(n,1);
	cauto init=tuple{-2,-1,n};
	map<tuple<int,int,int>,ll> dp;
	ll re=0;
	for(cauto &v:vs){
		++dp[init];
		map<tuple<int,int,int>,ll> nxt; for(cauto &[tp,val]:dp){
			cauto &[a,b,c]=tp;
			if(a==-2){
				if(b<c && b<v && v<c) nxt[{b,v,c}]+=val;
				else if(b<v) nxt[{-2,v,c}]+=val;
				else if(v<c) nxt[{-2,b,v}]+=val;
			}else{
				if(v<a) nxt[{-2,a,v}]+=val;
				else if(c<v) nxt[{-2,v,c}]+=val;
				else nxt[b<v ? tuple{b,v,c} : tuple{a,v,b}]+=val;
			}
		}
		swap(dp,nxt);
		for(cauto &p:dp) re+=p.Y;
		out(v,dp,1);
	}
	out(dp,1);
	cout<<re NL;
