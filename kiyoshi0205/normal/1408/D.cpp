...
int main(){
  ...
  rep(j,K)cin>>b[j].first>>b[j].second;

  rep(i,N)rep(j,K){
    if(a[i].first>b[j].first||a[i].second>b[j].second)continue;
    v.eb(b[j].first-a[i].first+1,b[j].second-a[i].second+1);
  }
  vsort(v);
  M=v.size();
  j=-1;
  brep(i,M){
    if(chmax(j,v[i].second))sv.eb(v[i]);
  }
  M=sv.size();
  // output(v);
  // output(sv);
  ...
  if(sv.size()==1){
    print(min(sv[0].first,sv[0].second));
    return 0;
  }
  j=inf;
  rep1(i,M){
    chmin(j,sv[i].first+sv[i-1].second);
  }
  chmin(j,sv[0].first);
  chmin(j,sv[M-1].second);
  print(j);
// }
}