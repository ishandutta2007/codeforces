...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  v[N-2][N-1]^=v[0][1];
  v[N-1][N-2]^=v[0][1];
  if(v[1][0]==0&&v[N-2][N-1]==1&&v[N-1][N-2]==1)print(0);
  else if(!v[1][0]){
    print(2-v[N-2][N-1]-v[N-1][N-2]);
    if(!v[N-2][N-1])print(N-1<<" "<<N);
    if(!v[N-1][N-2])print(N<<" "<<N-1);
  }else if(v[N-2][N-1]==v[N-1][N-2]){
    print(1);
    if(v[N-2][N-1])print("2 1");
    else print("1 2");
  }else{
    print(2);
    print("2 1");
