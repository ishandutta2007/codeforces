...
void z_algorithm(string &s, vector<ll> &z)
{
	z.resize(s.size());
	z[0] = s.size();
...
int main(void)
{
	...
	ll p = 0; vector<ll> z;
	while(p < sz(t)){
		string S = t.substr(p) + "#" + s + "$" + revs;
		z_algorithm(S, z);
		
		ll len = 0, pos;
		rep(i, sz(t)-p+1, sz(z)-1){
			if(len < z[i]){
				len = z[i];
				pos = i;
			}
		...
		if(pos <= sz(t)+sz(s)) ans.push_back(P(pos-sz(t), pos-sz(t)+len-1));
		else{
			ans.push_back(P(pos-(sz(s)+sz(t)+1), pos-(sz(s)+sz(t)+1)+len-1));
			ans.back().first = sz(s)+1 - ans.back().first;
			ans.back().second = sz(s)+1 - ans.back().second;
		}
		p += len;
	}
