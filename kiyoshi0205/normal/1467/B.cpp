...
ll solve(vec& v){
  ...
  rep(i,N){
    ...
    if(i-2>=0)res+=(v[i-1]<v[i]&&v[i-1]<v[i-2])||(v[i-1]>v[i]&&v[i-1]>v[i-2]);
    if(i+2<N)res+=(v[i+1]<v[i+2]&&v[i+1]<v[i])||(v[i+1]>v[i+2]&&v[i+1]>v[i]);
    chmax(m,bf-res);
    swap(v[i],tmp);
...
int main(){
  ...
  v.resize(N);
  rep(i,N)cin>>v[i];
  // N=9;v={88,84,3,51,54,99,32,60,76};
  // debug(judge(v));
  // debug(solve(v));
  print(solve(v));
  }
