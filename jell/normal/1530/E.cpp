#line 1 "codeforces-workspace\\contest\\1530\\e\\e.cpp"
...
#line 16 "codeforces-workspace\\contest\\1530\\e\\e.cpp"
//*/
...
void main() {
  ...
  if (count(begin(s), end(s), s.front()) == s.size()) eject(string(s));
  ...
  if (auto f = find(begin(cnt), end(cnt), 1); f != end(cnt)) {
    ans = s;
    swap(ans[ans.find(f - begin(cnt))], ans.front());
    sort(ans.begin() + 1, ans.end());
  } else {
    vector<pair<int, int>> ap;
    for (auto &&[i, c] : enumerate(cnt)) {
      if (c) {
        ap.emplace_back(i, c);
      }
    }
    ...
    if (ap.size() > 2) {
      if (ap[0].second > s.size() - ap[0].second + 2) {
        ans += ap.front().first;
        ap.front().second -= 1;
        ans += ap[1].first;
        ap[1].second -= 1;
        while (ap.front().second) {
          ans += ap.front().first;
          ap.front().second--;
        }
        ans += ap[2].first;
        ...
        t.erase(t.find(t.back()));
        while (ap[0].second) {
          if (ans.back() != ap[0].first) {
            ans += ap[0].first;
            ap[0].second--;
          } else {
            ans += t.back();
      ...
      if (ap[0].second > ap[1].second + 2) {
        ans += ap[0].first;
        ap[0].second--;
        ans += string(ap[1].second, ap[1].first);
        ans += string(ap[0].second, ap[0].first);
      } else {
        ans += ap[0].first;
        ans += ap[0].first;
        ap[0].second -= 2;
