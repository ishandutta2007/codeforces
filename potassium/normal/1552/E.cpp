...
int main() {
	...
	const int thres = k - 1;
	const int itv = (n + thres - 1) / thres;

	vector<pii> ans(n, { -1, -1 });
	for (int i = 0; i < itv; i++) {
		int l = thres * i, r = min(l + thres - 1, n - 1);

		vector<int> val;
		for (int j = l; j <= r; j++)
			val.insert(val.end(), all(occ[j]));
		sort(all(val));

		int taken = 0;
		vector<int> state(thres, -1);
		for (auto& j : val) {
			int v = a[j];
			if (ans[v].first != -1)
				continue;
