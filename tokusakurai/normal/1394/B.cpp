...
int main(){
    ...
    rep(i, N){
        sort(all(es[i]));
        rep(j, sz(es[i])) es[i][j].first = j;
    }
    vector<pii> rs[N];
    rep(i, N){
        each(e, es[i]){
            rs[e.second].eb(sz(es[i])-1, e.first);
        }
    }
    vec4(bool, ng, K, K, K, K, false);
    rep(i, N){
        map<pii, int> mp;
        each(e, rs[i]) mp[e]++;
        each(e, mp){
        ...
        rep(j, n){
            rep(k, j){
                auto a = rs[i][j], b = rs[i][k];
                ng[a.first][a.second][b.first][b.second] = true;
                ng[b.first][b.second][a.first][a.second] = true;
            }
        }
    }
    vector<int> v(K, 0);
