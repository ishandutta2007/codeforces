...
// #pragma GCC target("avx2,fma,popcnt")
...
const int mod = 1e9 + 7, mod1 = 998244353, mod2 = 1e9 + 9, inf = 1e9 + 7;
ll infll = 1e18 + 7;
...
int cal(int x){
    if (x < 10) return x;
    int ans = 9, t = cntdig(x);
    For(i, 2, t){
        ans += 9 * binpow(10, i - 2);
    }
    int firstdig = x / binpow(10, t - 1), lastdig = x % 10;
    For(i, 1, firstdig){
        ans += binpow(10, t - 2);
    }
    ans += (x - firstdig * binpow(10, t - 1)) / 10 + 1;
    if (lastdig < firstdig) ans--;
    return ans;
}
