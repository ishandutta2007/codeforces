...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) {
        sl = min(sl, a[i]);
        bg = max(bg, b[i]);
        if(ms.count(a[i])) ms[a[i]] = min(ms[a[i]], c[i]);
        else ms[a[i]] = c[i];
        if(mb.count(b[i])) mb[b[i]] = min(mb[b[i]], c[i]);
        else mb[b[i]] = c[i];
        if(mpr.count(mp(a[i], b[i]))) mpr[mp(a[i], b[i])] = min(mpr[mp(a[i], b[i])], c[i]);
        else mpr[mp(a[i], b[i])] = c[i];
        _ s = 1E17;
        if(mpr.count(mp(sl, bg))) s = min(s, mpr[mp(sl, bg)]);
        s = min(s, ms[sl]+mb[bg]);
        writeln(s);
    }
