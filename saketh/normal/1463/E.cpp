...
int main() {
    ...
    // already guaranteed that x are unique (out-degree 1) and y are unique (in-degree 1)
    // so we have only chains and cycles
    F0R (k, K) {
        int x, y; re(x, y); --x, --y;
    ...
    // form everyone into chains
    vi chain(N, -1);
    vi rank_within_chain(N);

    const int C = sz(heads);
    ...
    F0R (ci, C) {
        ...
        while (loc != -1) {
            chain[loc] = ci;
            rank_within_chain[loc] = rank++;
            loc = succ[loc];
        }
    }

    if (*min_element(all(chain)) == -1) { // cycle
        fail();
    }

    // ok, now can we order everything?
    vvi g(C);
    ...
    F0R (i, N) {
        ...
        if (a == b) {
            if (rank_within_chain[par] > rank_within_chain[i])
                fail();
        } else {
            g[a].pb(b);
        }
    ...
    scc res = strongly_connected_components(g);

    if (res.components < C)
        fail();

    sort_by(heads, res.label[chain[a]] > res.label[chain[b]]);
