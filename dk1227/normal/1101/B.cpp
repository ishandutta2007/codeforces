...
int main() {
	...
	for(; lft < s.size(); lft++) if(s[lft] == '[') break;
	if(lft == s.size()) return puts("-1");
	for(; rht >= 0; rht--) if(s[rht] == ']') break;
	if(rht < 0) return puts("-1");
	for(; lft < rht; lft++) if(s[lft] == ':') break;
	for(; rht > lft; rht--) if(s[rht] == ':') break;
	if(lft >= rht) puts("-1");
	else {
		int ans = 4;
