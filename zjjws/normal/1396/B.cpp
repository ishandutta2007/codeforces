...
inline void work()
{
    rin>>n;for(int i=1,x;i<=n;i++)rin>>x,a[i]=make_pair(1,x);sort(a+1,a+n+1);
    for(bool ans=false;true;ans^=1)
    {
        if(!n||!a[n].first){puts(ans?"T":"HL");return;}
        a[n].second--;a[n].first=0;if(n!=1)a[1].first=1;if(!a[n].second)n--;sort(a+1,a+n+1);
    }
    return;
}
int main()
