...
struct qus{
	ll l,r,id;
	int operator<(const qus&b)const{
		return l/len==b.l/len?(l/len&1)?r>b.r:r<b.r:l/len<b.l/len;
	}
}qs[1111111];
ll s=0;
ll t[1111111],ans[1111111],a[1111111];
void add(ll u){
	s+=(2*t[u]+++1)*u;
}
void del(ll u){
...
//	IO(M);
	...
	rep(i,1,q)qs[i].l=readll(),qs[i].r=readll(),qs[i].id=i;
