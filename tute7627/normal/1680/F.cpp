...
void solve(){
  ...
  if(nibu_graph(g).empty()){
    ...
    int cnt=0;
    vector<int>imos(2*sz+1);
    for(auto ver:guf){
      int pre=-1;
      for(auto z:ver){
        if(cidx[z]!=-1){
          if(pre!=-1){
            int l=min(cidx[pre],cidx[z]);
            int r=max(cidx[pre],cidx[z]);
            if((r-l)%2!=(t[z]^t[pre])){
              imos[l]++;
              imos[r]--;
            }
    ...
    //debug(imos);
    rep(i,0,2*sz)imos[i+1]+=imos[i];
    int ex=-1;
    rep(i,0,sz){
      if(cnt==imos[i]+imos[i+sz]){
        ex=ce[i];
      }
    }
    if(ex==-1){
