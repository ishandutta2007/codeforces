...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            io.cache.append("d ").append(1).append('\n');
            io.flush();
            where = io.readInt();
        ...
        public Node findCenter(Node root, Node father, int totalSize) {
            int max = totalSize - root.size;
            for (Node node : root.next) {
                if (node == father) {
                    continue;
                }
                max = Math.max(max, node.size);
                Node result = findCenter(node, root, totalSize);
                if (result != null) {
                    return result;
                }
            }
        ...
        public void findSize(Node root, Node father) {
            root.size = 1;
            for (Node node : root.next) {
                if (node == father) {
                    continue;
        ...
        public void find(Node root) {
            findSize(root, null);
            root = findCenter(root, null, root.size);
            ...
            io.cache.append("d ").append(root.id).append('\n');
            io.flush();
            int dist = io.readInt();
            if (dist == 0) {
                io.cache.append("! ").append(root.id).append('\n');
                return;
            }

            if (where - root.depth != dist) {
                findTop(root);
                return;
            }

            io.cache.append("s ").append(root.id).append('\n');
            io.flush();
            int id = io.readInt();
            for (Node node : root.next) {
                if (node.id == id) {
