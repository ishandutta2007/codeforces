...
int main() {
    ...
    int last = -1, llast = -1; vi pv(MAXV), ppv(MAXV);
    F0R (i, MAXV) {
        if (ct[i] >= 2) last = llast = i;
        else if (ct[i] >= 1) swap(last, llast), last = i;
        pv[i] = last;
        ppv[i] = llast;
    }
    ...
    for (int y = 2; y <= MAXV; y++) {
        ...
        while (xlo <= xhi) {
            ...
            // take 2X from one stick
            if (a.back() >= 2 * x) {
                // can we do it without losing an extra Y?
                bool good = false;
                for (int z = 0; z < MAXV; z += y) {
                    // find the last value in this section
            ...
            // take X from two different sticks?
            if (sz(a) >= 2 && a[sz(a) - 2] >= x) {
                int safely = 0;
                for (int z = 0; z < MAXV; z += y) {
                    // find the last value in this section
                    int zz = pv[min(MAXV, z + y) - 1];
                    ...
                    // find the second value in this section
