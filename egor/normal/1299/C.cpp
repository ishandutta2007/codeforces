...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        vec<pair<ll, int>> parts;
        reverse(all(a));
        for (int i : a) {
            ll sum = i;
            int q = 1;
            while (!parts.empty() && sum * parts.back().second >= parts.back().first * q) {
                sum += parts.back().first;
                q += parts.back().second;
                parts.pop_back();
            }
            parts.emplace_back(sum, q);
        }
        reverse(all(parts));
        for (const auto& p : parts) {
            ld current = ld(p.first) / p.second;
            for (int x : range(p.second)) {
