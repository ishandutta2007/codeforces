...
ll fin(ll a, ll b, ll c){
	if(dp[a][b] != -1)return dp[a][b];
	ll x = c-b;
	if(!a)return dp[a][b] = pw(2,x);
	return dp[a][b] = ((x?x*fin(a-1,b+1,c):0ll)+(b?b*fin(a-1,b,c):0ll))%MOD;
}

void solve(){
	ll n, k; cin >> n >> k;
	for(int i = 0 ; i < MX; ++i)for(int j = 0; j < MX; ++j)dp[i][j] = -1;
	cout << fin(k,0,n) << '\n';
