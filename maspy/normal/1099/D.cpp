...
void solve() {
  ...
  ANS[0] = A[0];
  const ll INF = 1LL << 60;
  FOR(v, N) if (dep[v] % 2 == 1) ANS[v] = INF;
  FOR(v, N) if (dep[v] >= 2 && dep[v] % 2 == 0) {
    ll p = par[v];
    ll pp = par[p];
    chmin(ANS[p], A[v] - A[pp]);
  }
  FOR(v, N) if (ANS[v] == INF) ANS[v] = 0;
  FOR(v, N) if (dep[v] >= 2 && dep[v] % 2 == 0) {
    ll p = par[v];
    ll pp = par[p];
    ANS[v] = A[v] - A[pp] - ANS[p];
  }
  vi DP = ANS;
  for (auto&& v: hld.V) {
    if (v == 0) continue;
  ...
  FOR(v, N) if (A[v] >= 0 && A[v] != DP[v]) return print(-1);
  if (MIN(ANS) < 0) return print(-1);
  print(SUM(ANS));
}
