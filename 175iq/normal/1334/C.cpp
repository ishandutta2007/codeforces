...
//
//Input
//The first line contains one integer T (1T150000)  the number of test cases.
//
//Then the test cases follow, each test case begins with a line containing one integer n 
//(2n300000)  the number of monsters. Then n lines follow, each containing two integers
// ai and bi (1ai,bi1012)  the parameters of the i-th monster in the circle.
//
//It is guaranteed that the total number of monsters in all test cases does not exceed 300000.
//
//Output
//For each test case, print one integer  the minimum number of bullets you have to fire to 
...
int main()
{
	...
	while(T--)
	{
		...
		temp = a[0] + prefix_sum(1,n-1);
		ans=temp;

		F(i,1,n-1)
		{
			temp = a[i];
			temp += prefix_sum(i+1,n-1) + prefix_sum(0,i-1) ; 
			ans = min(temp,ans);
		}
		temp = a[n-1] + prefix_sum(0,n-2);
		ans = min(temp,ans);
		cout<<ans<<endl;
	}
