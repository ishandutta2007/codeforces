...
int main() {
    ...
    loop(t) {
        ...
        while (i < n) {
            int level = 0;
            bool neg_level = false;

            int j = i;
            auto ok = [&]() -> bool {
                if (j - i < 2) return false;
                if (not neg_level and level == 0) return true;
                if ((j - i) & 1) {
                    if (manacher.odd_length((i + j) >> 1) >= j - i) {
                        return true;
                    }
                } else {
                    if (manacher.even_length((i + j) >> 1) >= j - i) {
                        return true;
                    }
                }
