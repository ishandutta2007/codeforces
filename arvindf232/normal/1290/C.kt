...
class DisjointUnionSets_Parity(val nsuggest: Int) {
    val n = nsuggest + 1
    ...
    val same = IntArray(n){1}
    val different = IntArray(n){0}
    var ops = 0
...
//        just dei "$x $y $dif"
        ...
        if(size[xx] < size[yy]){
            parent[xx] = yy
            if(overdif) {
                same[yy] += different[xx]
                different[yy] += same[xx]
            }else{
                same[yy] += same[xx]
                different[yy] += different[xx]
            }
            ...
            if(overdif){
                ...
                different[xx] += same[yy]
            }else{
                same[xx] += same[yy]
                different[xx] += different[yy]
            }
        }
        ops += getValue(xx)
        return true
    ...
    fun getValue(i:Int):Int {
        ...
        if(find(i).first == fixed.first){
            ...
            return minOf(same[r],different[r])
        }
    }
...
fun main(){
    solve.cases{
        ...
        val k = getint
        val str = getstr
        val initial = BooleanArray(n){str[it] == '1'}
        val has = List(n){
            mint
        }

        val U = DisjointUnionSets_Parity(k)
        repeat(k){
            val s = getint
            val a = getline(s)
            for(c in a){
        ...
        for(i in 0 until n){
            ...
            if(here.isEmpty()){
                ...
                U.union(here[0],k,!initial[i])
            }else{
                U.union(here[0],here[1],!initial[i])
            }
            put(U.ops)
        }
