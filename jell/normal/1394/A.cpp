...
#line 4 "codeforces-workspace/contest/1394/a/a.cpp"
#include <ext/rope>
...
#line 3 "Library/utils/binary_search.hpp"
...
#line 11 "codeforces-workspace/contest/1394/a/a.cpp"
...
solver()
{
    ...
    vector<int> b,c;
    {
        auto pos=upper_bound(begin(a), end(a), m);
        c={begin(a),pos};
        b={pos,end(a)};
    }
    int bs=b.size();
    ...
    for(i64 i=bs,j=0,use=bs,sum=accumulate(begin(b), end(b), i64(0));
        i>=0;
        i? sum -= b[--i],use-- : --i)
    {
        if(use>d*(bs-i)) continue;
        bool ng=false;
        while(use<d*(bs-i-1))
        {
            if(j==cs)
    ...
    ans=accumulate(begin(a), end(a), i64(0))-ans;
    cout << ans << "\n";
}};