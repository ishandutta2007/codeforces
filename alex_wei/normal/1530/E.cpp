...
void solve(){
	...
	bool issame=1;
	for(int i=2;i<=n;i++)issame&=s[i]==s[i-1];
	if(issame)return cout<<(s+1)<<endl,void();
	...
	if(c!=-1){
		cout<<(char)(a+'a'),buc[a]--;
		cout<<(char)(b+'a'),buc[b]--;
		for(int i=1;i<=buc[a];i++)cout<<(char)(a+'a');
		cout<<(char)(c+'a'),buc[c]--; 
		for(int i=b;i<26;i++)
			for(int j=1;j<=buc[i];j++)
				cout<<(char)(i+'a');
	...
	cout<<(char)(a+'a'),buc[a]--;
	for(int i=1;i<=buc[b];i++)cout<<(char)(b+'a');
	for(int i=1;i<=buc[a];i++)cout<<(char)(a+'a');
	cout<<endl;
	return;
