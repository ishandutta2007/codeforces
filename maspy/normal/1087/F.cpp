...
void solve() {
  ...
  VEC(P, query, Q);
  for (auto&& [i, c]: query) --i;
  vi ANS(Q + 1);
  string S0 = S;

  auto f = [&](char G, char C, char P) -> void {
    S = S0;
    // 
    //  G 
    // C   fastset
    ...
    FOR(i, N) {
      if (S[i] == G) bit.add(i, 1);
      if (S[i] == C) ssC.insert(i);
      if (S[i] == P) ssP.insert(i);
    }
    ...
    auto calc = [&]() -> ll {
      ...
      res += bit.sum(CL, CR);
      //  P 
      res += bit.sum(min(CL, PL));
      //  P 
      res += bit.sum(max(CR, PR), N);
      return res;
    };
    ...
    FOR(q, Q) {
      auto [i, c] = query[q];
      if (S[i] == G) bit.add(i, -1);
      if (S[i] == C) ssC.erase(i);
      if (S[i] == P) ssP.erase(i);
      S[i] = c;
      if (S[i] == G) bit.add(i, 1);
      if (S[i] == C) ssC.insert(i);
      if (S[i] == P) ssP.insert(i);
