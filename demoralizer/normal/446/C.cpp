...
int get(int a,int b,int x){
	if(x==1)return a;
	return (a*F[x-2]%M + b*F[x-1]%M)%M;
}
int getsum(int a,int b,int x){
	if(x==0)return 0;
	if(x==1)return a;
	return (a*(1+Fs[x-2])%M + b*Fs[x-1]%M)%M;
}
...
struct update{
	int a=0,b=0,l;
	update(){}
	update(int a,int b,int l):a(a),b(b),l(l){}
	void combine(update &other,int32_t tl,int32_t tr){
		l=tl;
		a+=get(other.a,other.b,tl-other.l+1);a%=M;
		b+=get(other.a,other.b,tl-other.l+2);b%=M;
	}
	void apply(node &x,int32_t tl,int32_t tr){
		// cout<<a<<" "<<b<<" "<<l<<" "<<tl<<" "<<tr<<"\n";
		x.v+=(getsum(a,b,tr-l+1) - getsum(a,b,tl-l) + M)%M;
	}
}noop;
struct segtree{
	bool built=false;
	...
	void apply(int32_t v,update &val)
	{
		if(tl!=tr){
			lazy[v]=1;
