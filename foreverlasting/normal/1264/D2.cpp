...
namespace MAIN{
    ...
    inline int C(const res &x,const res &y){
        return (x<y||x<0||y<0)?0:mul(fac[x],mul(inv[y],inv[x-y]));
    }
    inline void MAIN(){
        scanf("%s",str+1),n=int(strlen(str+1)),fac[0]=fac[1]=inv[0]=inv[1]=1;
        for(res i=1;i<=n;i++)a[i]=a[i-1]+(str[i]=='?'),b[i]=b[i-1]+(str[i]=='(');
        for(res i=2;i<=n;i++)fac[i]=mul(fac[i-1],i),inv[i]=mul(inv[kcz%i],kcz-kcz/i);
        for(res i=2;i<=n;i++)inv[i]=mul(inv[i-1],inv[i]);
        for(res i=1;i<=n;i++){
            res A=a[n-i],p=b[n-i];
            add(ans,Add(mul(p,C(a[n],i-b[n])),mul(A,C(a[n]-1,i-b[n]-1))));
        }
//        for(res j=0;j<=n;j++){
//            res ret=0;
//            for(res i=j;i<=n;i++){
//                res A=a[n-i],B=a[n]-a[n-i],p=b[n-i],q=b[n]-b[n-i];
//                add(ret,mul(C(A,j-p),C(B,i-j-q)));
//            }
//            add(ans,mul(ret,j));
//        }
        printf("%d\n",ans);
