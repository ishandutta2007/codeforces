...
vector<pii> v[101010];
const int T = 2056;
vector<tri> qq[102010 / T];
char data[101010][T];
int rest[T][12];
int ans[101010]; int qs = 0;
int solve() {
    int n = readInt();
    int k, q;
    k = readInt();
    ...
    FI(q) {
        ...
        if (t == 3) {
            if (b < k) {
                ...
                qq[c / T].push_back({b, c % T, qs++});
            }
        } else {
            par[kk++] = {b, c, t};
        }
    }
    for (int from = 0; from < n; from += T) {
        FI(k) {
            FJ(T) {
                rest[j][i] = a[i][from + j];
            }
        ...
        FJ(T) {
            sort(rest[j], rest[j] + k);
        }
        FI(k) {
            FJ(T) {
                data[i][j] = char(lower_bound(rest[j], rest[j] + k, a[i][from + j]) - rest[j]);
            }
        }
        for (auto q : qq[from / T]) {
            v[q.a].push_back({q.b, q.c});
        }
        for (int t = k; t < kk; ++t) {
            int x = par[t].a;
            int y = par[t].b;
            ...
            for (auto q : v[t]) {
                ans[q.second] = rest[q.first][pt[q.first]];
            }
        }
        for (auto q : qq[from / T]) {
            v[q.a].clear();
