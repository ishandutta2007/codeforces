...
struct bit{
	...
	int get(int l,int r){
		if(l>r)return 0;
		int s=0;
		while(r)s+=sum[r],r-=lowbit(r);
		for(--l;l;l-=lowbit(l))s-=sum[l];
		return s;
	}
}fw[3];
set<int>g[3];
int calc(){
	...
	for(int i=0;i<3;i++){
		if(!g[(i+1)%3].empty()){
			if(!g[(i+2)%3].empty()){
				int a=*g[(i+1)%3].begin(),b=*g[(i+1)%3].rbegin();
				int c=*g[(i+2)%3].begin(),d=*g[(i+2)%3].rbegin();
				ret+=fw[i].get(c,d)+fw[i].get(1,min(a,c))+fw[i].get(max(b,d),n);
			}
		}else ret+=fw[i].get(1,n);
	}
	return ret;
}
int main(){
	...
	scanf("%d%d",&n,&m);
	scanf("%s",s+1);
	for(i=1;i<=n;i++) v[i]=to[s[i]];
	for(i=1;i<=n;i++) fw[v[i]].add(i,1),g[v[i]].insert(i);
	printf("%d\n",calc());
	for(i=1;i<=m;i++){
		scanf("%d",&p);scanf("%s",o);
		q=to[*o];
		fw[v[p]].add(p,-1),g[v[p]].erase(p);
		v[p]=q;
		fw[v[p]].add(p,1),g[v[p]].insert(p);
		printf("%d\n",calc());
	}
}