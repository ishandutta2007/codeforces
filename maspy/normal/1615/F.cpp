...
void solve() {
  ...
  auto calc_dp = [&]() -> vc<vc<mint>> {
    //  0  N [0,N+N] 
    vv(mint, DP, N + 1, N + N + 1);
    DP[0][N] = 1;
    FOR(i, N) {
      auto& dp = DP[i];
      auto& newdp = DP[i + 1];
      FOR(diff, N + N + 1) {
        ...
        FOR(s, 2) {
          ...
          if (s == 1 && S[i] == '0') continue;
          FOR(t, 2) {
            if (t == 0 && T[i] == '1') continue;
            if (t == 1 && T[i] == '0') continue;
            ll add = 0;
            add += (s == 0 ? 1 : -1);
            add -= (t == 0 ? 1 : -1);
            add /= 2;
            newdp[diff + add] += dp[diff];
          }
        }
      }
    }
  ...
  FOR(x, N + 1) {
    auto& dp1 = DP1[x];
    auto& dp2 = DP2[x];
    FOR(v, N + N + 1) { ANS += dp1[v] * dp2[N + N - v] * mint(abs(N - v)); }
  }
  print(ANS);
}
