...
void solve_case(int tc = 0) {
	...
	s = "?" + s;
	t = "?" + t;
	VVV<int> go(n + 1, vector(A, vi(2, n + 1)));
	for (int i = n - 1; i >= 1; --i) {
		go[i][s[i + 1] - 'a'][(i + 1) % 2] = i + 1;
		f0r(k, 2) {
			f0r(j, A) {
				ckmin(go[i][j][k], go[i + 1][j][k]);
			}
		}
	}
	bool ok = false;
	vector<vi> dp(n + 1, vi(2, -1)); // max position of parity x i can go off
	f0r(i, n + 1) {
		dp[i][0] = 0;
	}
	auto adv = [&](int loc, int p) {
		if (dp[loc][p] == -1) {
			return;
		}
		int nxt = dp[loc][p] + 1;
		...
		if (t[nxt] == s[loc + 1]) {
			ckmax(dp[loc + 1][p ^ 1], nxt);
		}
		int to = go[loc][t[nxt] - 'a'][(loc % 2) ^ 1];
		if (to != n + 1) {
			ckmax(dp[to][p ^ 1], nxt);
		}
	};
