...
main() {
    ...
    rep2(i, 1, n) rep2(j, 1, n) {
        rep(t, 4) {
            if(s[i][j] and s[i + d[t]][j + d[t ^ 1]]) uf.unite(f(i, j), f(i + d[t], j + d[t ^ 1]));
        }
    }
    int ans = 0;
    rep2(i, 1, n - k + 1) {
        ...
        auto ins = [&](int i, int j) {
            if(!s[i][j]) return;
            mp[uf.find(f(i, j))]++;
            if(mp[uf.find(f(i, j))] == 1) now += uf.size(f(i, j));
        };
        auto del = [&](int i, int j) {
            if(!s[i][j]) return;
            mp[uf.find(f(i, j))]--;
            if(!mp[uf.find(f(i, j))]) now -= uf.size(f(i, j));
        };
        rep2(ii, i - 1, i + k) {
            rep2(jj, 0, k + 1) {
                if((ii == i - 1 or ii == i + k) and (!jj or jj == k + 1)) continue;
                ins(ii, jj);
            }
        }
        rep(ii, k) rep(jj, k) now += !s[i + ii][jj + 1];
        chmax(ans, now);
        rep2(j, 2, n - k + 1) {
            del(i - 1, j - 1), del(i + k, j - 1), ins(i - 1, j + k - 1), ins(i + k, j + k - 1);
            rep2(ii, i, i + k - 1) del(ii, j - 2), ins(ii, j + k);
            rep2(ii, i, i + k - 1) now -= !s[ii][j - 1], now += !s[ii][j + k - 1];
            chmax(ans, now);
        }
    }
    cout << ans << endl;
