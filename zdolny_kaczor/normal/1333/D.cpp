...
int main() {
	...
	int siz = 0;
	auto put = [&](int x) {
		splits.resize(max<int>(splits.size(), x + 1));
		if (!splits[x]) {
			splits[x] = 1;
			siz++;
		}
	...
	while (any) {
		any = false;
		int last = -100;
		REP(i, n) if (in[i] == 'R' && in[i + 1] == 'L' && i - 1 != last) {
			last = i;
			swap(in[i], in[i + 1]);
			ans.push_back(i);
			any = true;
	...
	debug(imie(ans), imie(splits));
	for (int i = 0; i <= (int) ans.size() && siz <= k; ++i)
		put(i);
	debug(imie(ans), imie(splits));
	if (siz == k + 1) {
		int last = 0;
		for (int i = 1; i < (int) splits.size(); ++i) {
			if (splits[i]) {
				printf("%d", i - last);
				for (int k = last; k < i; ++k) printf(" %d", ans[k] + 1);
				printf("\n");
				last = i;
			}
		}
