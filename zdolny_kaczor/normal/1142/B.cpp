...
int jumps[nax];
int mul_jump[nax];
void mul(int n, int a[nax], int b[nax], int c[nax]) {
	REP(i, n) a[i] = b[c[i]];
}
int ans[nax];
int main() {
	...
	REP(i, m) scanf("%d", a + i);
	p[n] = p[0];
	REP(i, n) next_p[p[i]] = p[i + 1];
	REP(i, (int)nax) next[i] = jumps[i] = nax - 1;
	FORD(i, m - 1, 0) {
		jumps[i] = next[next_p[a[i]]];
		next[a[i]] = i;
	}
	REP(i, (int)nax) mul_jump[i] = i;
	debug(range_v(jumps, 0, m));
	REP(i, 18) {
		if ((n - 1) >> i & 1)
			mul(m, mul_jump, jumps, mul_jump);
		mul(m, jumps, jumps, jumps);
		debug(range_v(jumps, 0, m), range_v(mul_jump, 0, m));
	}
	debug(range_v(mul_jump, 0, m));
	REP(i, m) put(i, mul_jump[i]);
	REP(i, q) {
		int l, r;
		scanf("%d%d", &l, &r);
		l--;
