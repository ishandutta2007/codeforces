...
int main() {
	ll ax,ay,bx,by,cx,cy; R(ax,ay,bx,by,cx,cy);
	
	ll abdist = sqr(by-ay) + sqr(bx-ax);
	ll bcdist = sqr(cy-by) + sqr(cx-bx);
	
	pll abvec = {bx-ax,by-ay};
	pll bcvec = {cx-bx,cy-by};
	
	if (abdist != bcdist || abvec == bcvec)
		W("No");
	else
		W("Yes");
