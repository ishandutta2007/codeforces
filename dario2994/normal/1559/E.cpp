...
mint solve(int n, int m, vector<int>& l, vector<int>& r) {
    for (int i = 0; i < n; i++) m -= l[i], r[i] -= l[i];
    if (m < 0) return 0;
    ...
    for (int i = 0; i < n; i++) {
        new_ans = ans;
        for (int j = 1; j <= m; j++) {
            new_ans[j] += new_ans[j-1];
            if (j >= r[i]+1) new_ans[j] -= ans[j-r[i]-1];
        }
        swap(ans, new_ans);
    }
    mint res = 0;
...
int main() {
    ...
    for (LL p = 2; p < MAXM; p++) {
        ...
        for (LL i = 2*p ; i < MAXM; i += p) is_comp[i] = true;
        for (LL i = p; i < MAXM; i += p) mu[i] *= -1;
        for (LL i = p*p; i < MAXM; i += p*p) mu[i] = 0;
    }
    // cout << mu[6] << " " << mu[18] << " " << mu[7] << endl;
