...
pll dfs(vector<vector<pii>> &G, int now, int par) {
  ...
  rep (i, G[now].size()) {
    ...
    if (ch == par) continue;
    pll temp = dfs(G, ch, now);
    childs.push_back(pll(temp.first + w, temp.second));
  }
  ...
  pll res(0, 0);
  rep (i, m) {
    res.first += childs[i].second;
    res.second += childs[i].second;
  }
  rep (i, k) {
    if (i == m) break;
    if (childs[i].first < childs[i].second) break;
    if (i < k - 1 and childs[i].first > childs[i].second) {
      res.first += childs[i].first - childs[i].second;
      res.second += childs[i].first - childs[i].second;
    }
    else {
      res.second += childs[i].first - childs[i].second;
    }
