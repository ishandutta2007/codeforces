...
int main(){
    ...
    for(int i=0;i<N;i++){
        int b=A[i];
        if(b+k<m/2){
            seg.update(MA[b]+1,MA[b+k],1);
        }else{
            seg.update(MA[b]+1,M,1);
            seg.update(0,MA[(b+k)%(m/2)],1);
        }
    }
    
    pair<ll,ll> res={INF,INF};
    for(int i=0;i<M;i++){
        chmin(res,mp(seg.query(i,i+1),ll(i)));
    }
    
    cout<<res.fi<<" "<<SE[res.se]<<"\n";
    ...
    for(int i=0;i<N;i++){
        int b=A[i];
        if(b<=t){
            if(t-b>0&&t-b<k) cout<<i+1<<" ";
        }
        else{
            if(t+m/2-b>0&&t+m/2-b<k) cout<<i+1<<" ";
        }
    }
    cout<<endl;
}