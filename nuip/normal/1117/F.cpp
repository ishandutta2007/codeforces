...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> nxt(p,n);
	vector<int> ind(p); iota(all(ind),0);
	rrep(i,n){
		int x=s[i]-'a';
		sort(all(ind),[&](int i,int j){ return nxt[i]<nxt[j];});
		int st=0;
		out(i,ind,1);
		for(int j:ind){
			if(nxt[j]==n) break;
	...
	vector<int> vst(1<<p);
	queue<int> que;
	que.emplace((1<<p)-1); vst[(1<<p)-1]=1;
	int re=n;
	while(que.size()){
		int st=que.front(); que.pop();
		int sum=0;
		rep(i,p)if(st>>i&1) sum+=cnt[i];
		MN(re,sum);
		rep(i,p)if(st>>i&1){
			int nxt=st^(1<<i);
			if(ok[nxt] && MX(vst[nxt],1)) que.emplace(nxt);
		}
	}
	out(ok,vst,1);
	cout<<re<<endl;
