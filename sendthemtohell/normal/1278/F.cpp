...
int add(int a, int b) {
	a += b;
	if (a >= FFT_MOD) a -= FFT_MOD;
	return a;
}

int mul(int a, int b) {
	return int(ll(a) * b % FFT_MOD);
}
...
inline void solve() {
	...
	cin >> n >> m >> k;
	coef[0][0] = 1;
	int im = inv(m, FFT_MOD);
	for (int i = 0; i < k; ++i) {
		for (int j = 0; j <= i; ++j) {
			coef[i + 1][j] = add(coef[i + 1][j], mul(j, coef[i][j]));
			coef[i + 1][j + 1] = add(coef[i + 1][j + 1], mul(coef[i][j], mul(im, n - j)));
		}
	}
	int ans = 0;
	for (int i = 0; i <= k; ++i) {
		ans = add(ans, coef[k][i]);
	}
	cout << ans << endl;
}
