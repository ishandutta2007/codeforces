...
struct SegmentTree
{
    ...
    void update(int v, int tl, int tr, int l, int r, ll minn) {
        if (l > r)
            return;
        if (l == tl && r == tr) {
            t[v] = min(t[v], minn);
            ...
            int tm = (tl + tr) / 2;
            update(v*2, tl, tm, l, min(r, tm), minn);
            update(v*2+1, tm+1, tr, max(l, tm+1), r, minn);
        }
    }
    ...
    void upd(int l, int r, ll minn)
    {
        update(1, 0, n-1, l, r, minn);
    }
...
int main()
{
    ...
    set<pair<int, int>> who;

    stack<int> st;
    for (int i = 0; i<n; i++)
    {
        while (!st.empty() && w[st.top()]>w[i]) st.pop();
        if (!st.empty()) who.insert(mp(st.top(), i));
        st.push(i);
    }
    ...
    {
        while (!st.empty() && w[st.top()]>w[i]) st.pop();
        if (!st.empty()) who.insert(mp(i, st.top()));
        st.push(i);
    }

    vector<vector<int>> segs(n);
    for (auto it: who) segs[it.first].push_back(it.second);
    ...
    for (int i = 0; i<q; i++)
    {
        int l, r; cin>>l>>r; l--; r--; queries[l].push_back(mp(r, i));
    }
    ...
    for (int i = n-1; i>=0; i--)
    {
        for (auto it: segs[i])
        {
            ll val = (x[it]-x[i])*(w[i] + w[it]);
            tree.upd(it, n-1, val);
        }
        for (auto it: queries[i])
        {
            ans[it.second] = tree.get(it.first);
        }
    }
