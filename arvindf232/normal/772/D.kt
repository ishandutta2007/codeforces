...
fun main(){
    solve.cases{
        val M = 1_000_000
        val n = getint()
        val L = IntArray(1_000_000)

        repeat(n){
            val a = getint()
            L[a] ++
        }
        val pows = intArrayOf(1,10,100,1000,10000,100000)
        val lineorder = Array(6){IntArray(100_000)}
        for(c in 0 until 6){
            val type = pows[c]
            var pointer = 0
            for(a in 0 until pows[c]){
                for(b in 0 until pows[5-c]){
                    val v = 9 * type + a + b * type * 10
                    lineorder[c][pointer++] = v
                }
            }
        }
        val upperLeftCount = L.copyOf()
        val upperLeftSum = L.copyOf()
        val upperLeftQuadratic = L.copyOf()
        for(i in 0 until M){
            upperLeftSum[i] = upperLeftSum[i] modM i
            upperLeftQuadratic[i] = upperLeftQuadratic[i] modM i modM i
        }
        for(c in 0 until 6){
            val type = pows[c]
            for(swe in lineorder[c]){
                var now = swe
                for(d in 1..9){
                    upperLeftCount[now - type] += upperLeftCount[now]
                    upperLeftSum[now-type] = (upperLeftSum[now-type]+upperLeftSum[now]).snap()
                    upperLeftQuadratic[now-type] = (upperLeftQuadratic[now-type]+upperLeftQuadratic[now]).snap()
                    now -= type
                }
            }
        }
        val tv = IntArray(M)
        for(i in 0 until M){
            if(upperLeftCount[i] <= 1){
                tv[i] = upperLeftQuadratic[i]
            }else{
                val f = intPow(2,upperLeftCount[i] - 2,pI)
                tv[i] = f modM (upperLeftQuadratic[i] modPlus (upperLeftSum[i] modM upperLeftSum[i]))
            }
        }
//        val tosubtract = tv.copyOf()
//        for(i in 0 until M){
//            tv[i] = tv[i] modM 2
//        }
        for(c in 0 until 6){
            ...
            for(swe in lineorder[c]){
                var now = swe - 9 * type
                for(d in 1..9){
                    tv[now] = tv[now] modMinus  tv[now + type]
                    now += type
                }
            }
        }
//        for(i in 0 until M){
//            tv[i] = tv[i] modMinus tosubtract[i]
//        }
        var ret = 0L
        for(i in 0 until M){
            ret = (ret) xor ( i.toLong() * tv[i])
        }
        put(ret)
    }
    done()
...
/*
 ...
 bad dimensioal convolution techniquse
 */