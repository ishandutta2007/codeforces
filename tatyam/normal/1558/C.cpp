...
void solve(){
    ...
    rep(n){
        a[i]--;
        if((a[i]^i)&1)return(void)out(-1);
    }
    vector<ll>ans;
    auto query=[&](ll length){
        assert(length&1&&1<=length&&length<=n);
        ans.push_back(length);
        reverse(a.begin(),a.begin()+length);
    };
    for(ll i=n-1;i>0;i-=2){
        ll o=find(all(a),i)-a.begin(),e=find(all(a),i-1)-a.begin();
        if(o<e){
            query(o+1);
        }
        else{
            query(o+1);
            e=find(all(a),i-1)-a.begin();
