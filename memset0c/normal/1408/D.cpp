...
std::vector<int> max;
std::vector<std::pair<int,int>> ned;
...
int main(){
  ...
  for(int i=1;i<=m;i++)read(c[i],d[i]);
  for(int i=1;i<=n;i++)
    for(int j=1;j<=m;j++)
      if(a[i]<=c[j]&&b[i]<=d[j]){
        // printf("%d %d : %d %d\n",i,j,c[j]-a[i],d[j]-b[i]);
        ned.push_back({c[j]-a[i]+1,d[j]-b[i]+1});
      }
  ...
  // for(const auto &it:ned){
  //   printf("%d %d\n",it.first,it.second);
  // }
  max.resize(ned.size());
  max.back()=ned.back().second;
  for(int i=(int)max.size()-2;i>=0;i--){
    max[i]=std::max(max[i+1],ned[i].second);
  }
  ans=std::min(max[0],ned.back().first);
  for(int i=(int)max.size()-2;i>=0;i--){
    ans=std::min(ans,ned[i].first+max[i+1]);
    // printf("%d : %d %d\n",i,ned[i].first,max[i+1]);
  }
  print(ans,'\n');
}