...
int main(){
    ...
    rep(zz,T){
        ...
        vector<int> inv(n+1);
        rep(i,n)inv[a[i]] = i;
        segtree<int> seg_mi(a);
        rep(i,n)a[i] = -a[i];
        segtree<int> seg_mx(a);
        rep(i,n)a[i] = -a[i];
        while(id!=n-1){
            res++;
            if(a[id]<a[id+1]){
                ...
                while(ng-ok>1){
                    int mid = (ng+ok)/2;
                    if(seg_mi.query(id,mid+1)==a[id]){
                        ok = mid;
                    }else{
                        ng = mid;
                    }
                }
                int mx = seg_mx.query(id,ok+1);
                int nxt = inv[-mx];
                id = nxt;
            }else{
                int ok = id+1;
                ...
                while(ng-ok>1){
                    int mid = (ng+ok)/2;
                    if(seg_mx.query(id,mid+1)==-a[id]){
                        ok = mid;
                    }else{
                        ng = mid;
                    }
                }
                int mi = seg_mi.query(id,ok+1);
                int nxt = inv[mi];
                id = nxt;
            }
        }
