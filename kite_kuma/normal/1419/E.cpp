	...
	for(int i = 0; i < t; i++) {
		...
		rep(j, vs) {
			...
			if(j <= vs - 3) {
				...
				tie(now, num) = v[j];
				vll arr;
				rep(m, num) { arr.push_back(POW(now, m + 1)); }
				int k = j + 2;
				while(k < vs) {
					int sz = arr.size();
					rep(l, sz) { rep(m, v[k].second) arr.push_back(arr[l] * POW(v[k].first, m + 1)); }
					k++;
				}
				// foa(a, arr) if(a != x) res.push_back(a);
				// arr.clear();
				// rep(m, num) { arr.push_back(POW(now, m + 1)); }
				k = j + 1;
				//dump(arr);
				...
				foa(a, arr) {
					...
					if(a != x)
						// if(a != v[k].first)
						if(a != now * v[k].first) res.push_back(a);
					
				}
				res.push_back(now * v[k].first);
			} else if(j == vs - 2) {
				ll now;
				ll num;
				tie(now, num) = v[j];
				...
				int sz = arr.size();
				//dump(arr);
				rep(l, sz) { rep(m, v[vs - 1].second) arr.push_back(arr[l] * POW(v[vs - 1].first, m + 1)); }
				//dump(arr);
				foa(a, arr) if(a != x) res.push_back(a);
			} else {
				ll now;
