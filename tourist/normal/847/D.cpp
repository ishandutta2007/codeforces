...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    cin >> t;
    if (i + 1 < T) {
      r = add(r, new node(t + n - i), [&](node* v) { return t + n - i < v->id; });
/*      pair<node*,node*> p = split(r, [&](node* v) { return T + n - i <= v->id; });
      ans = max(ans, get_size(p.first));
      r = merge(p.first, p.second);*/
      ans = max(ans, count_left(r, [&](node* v) { return T + n - i <= v->id; }));
    }
  }
  cout << ans << '\n';
  return 0;
