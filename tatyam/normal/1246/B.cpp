...
signed main(){
    LL(n,k);
    VEC(ll,a,n);
    auto hash=[](const map<ll,ll>& a)->ull{
        ull ans=88172645463325252;
        each(i,a){
            ans^=i.first;
            ans^=(ans<<13);
            ans^=(ans>>7);
    ...
    unordered_map<map<ll,ll>,ll,function<ull(const map<ll,ll>&)>>cnt(200000,hash);
    each(b,a){
        map<ll,ll>factor;
