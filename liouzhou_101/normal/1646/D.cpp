...
void solve()
{
	...
	dfs(0, -1);
	pll res = max({ dp[0][0], dp[0][1] });
	writeln(res.first, ' ', -res.second);
	go(0, res == dp[0][0] ? 0 : 1, -1);
	print(w, w + n);
}
