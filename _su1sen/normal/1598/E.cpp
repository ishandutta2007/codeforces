...
int main() {
    ...
    auto f = [&](int j) {
        if (j < 0 or j + 1 >= w) return 0LL;
        long long res = 0;
        int i = 0;
        while (i <= h) {
            while (i < h and (g[i][j] == 1 or g[i][j + 1] == 1)) ++i;
            if (i == h) break;
            int k = i;
            while (k < h and (g[k][j] == 0 and g[k][j + 1] == 0)) ++k;
            bool b1 = i > 0 and g[i - 1][j + 1] == 0;
            bool b2 = k < h and g[k][j] == 0;
            ...
            res += (l - 1) * (b2 ? 4 : 2);
            if (b1 and b2) res += 4;
            else if (b1 or b2) res += 2;
            else res += 1;
            i = k;
        }
        rep(i, 1, h) {
            res += g[i][j] == 0 and g[i - 1][j + 1] == 0;
        }
        return res;
    };
