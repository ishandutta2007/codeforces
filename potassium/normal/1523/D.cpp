...
int main() {
	...
	for (int iter = 0; iter < 25; iter++) {
		...
		const int BITS = (int)bits.size();
		const int FULL = 1 << BITS;

		vector<int> count(FULL, 0);
		for (int i = 0; i < n; i++) {
			int mask = 0;
		...
		for (int i = 0; i < BITS; i++) {
			const int curBit = 1 << i;
			for (int mask = 0; mask < FULL; mask++)
				if (!(mask & curBit))
					count[mask] += count[mask ^ curBit];
		}

		for (int mask = 0; mask < FULL; mask++) {
			if (count[mask] >= (n + 1) / 2 && __builtin_popcount(mask) > ansSize) {
				ansSize = __builtin_popcount(mask);
				ansMask.reset();
				for (int j = 0; j < BITS; j++)
					if (mask & (1 << j))
						ansMask[bits[j]] = 1;
