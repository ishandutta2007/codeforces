...
int main(){
    ...
    mint ans = comb(cq, n-cb);
    { // WBBW
        bool all_either = true;
        int cnt = 0;
        for(int i = 0; i < n; i++){
            if(ok(t[i], "BW") && ok(t[i], "WB")) cnt++;
            if(!ok(t[i], "BW") && !ok(t[i], "WB")) {
                all_either = false;
            }
        }
        if(all_either) ans -= pow(mint(2), cnt);
    }
    { // BWOK
        bool all_bw = true;
        for(int i = 0; i < n; i++){
            if(!ok(t[i], "BW")) all_bw = false;
        }
        if(all_bw) ans += 1;
    }
    { // WWBOK
