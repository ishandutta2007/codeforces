...
void Push(int V)
{
    push[2 * V + 1] += push[V], push[2 * V + 2] += push[V];
    tree[2 * V + 1].fi += push[V], tree[2 * V + 2].fi += push[V];
    push[V] = 0;
}
...
signed main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        Add(0, pos[x][z - 1] + 1, -1);
        Add(0, pos[x][z - 3] + 1, 3);
        Add(0, pos[x][z - 4] + 1, -INF);
        pair<ll, int> G = Get(0, i);
...
Add something here.
	...
	* WRITE STUFF DOWN
	* DON'T GET STUCK ON ONE APPROACH (Benq)
*/