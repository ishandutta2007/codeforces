...
int ans[201010];
vector<array<int, 3>> queries[202002];
vector<int> g[202020];
set<int> dfs(int cur) {
    set<int> s;
    for (int x : g[cur]) {
...
int solve() {
    ...
    FI(m) {
        ...
        --x; --y;
        int l = lca(x, y);
        where[x][0] = min(where[x][0], l);
        where[y][0] = min(where[y][0], l);
        g[x].push_back(y);
        g[y].push_back(x);
    }
    for (int i = n - 1; i > 0; --i) {
        ...
        int r = where[i][0];
        if (r < p) {
            where[p][0] = min(where[p][0], r);
        }
    }
    for (int h = 1; h < Z; ++h) {
        for (int i = 0; i < n; ++i) {
            where[i][h] = where[where[i][h - 1]][h - 1];
        }
    }
    int q = readInt();
    FI(q) {
        ...
        ans[i]++;
        queries[x].push_back({i, tin[y], tout[y]});
    }
    dfs(0);
    FI(q) {
        writeInt(ans[i]);
