...
int main() {
    ...
    rep(i, 10) {
        ...
        sum[i].eb(0);
        for(auto e : a[i]) { sum[i].eb(sum[i].back() + log(e)); }
    }
    double ma = 0;
    vi ans;
    vi w(10, 5);
    w[0] = 2, w[1] = 1, w[5] = 2;
    rep(i, 10) chmin(w[i], si(a[i]) + 1);
    for(auto v : ndFOR(w)) {
        int t = 1;
        rep(j, 10) { t = (t * POW(j, v[j])) % 10; }
        if(t == d) {
            vi s(10);
            double tmp = 0;
            rep(i, 10) {
                ...
                if(i == 0) {
                    ...
                    c = (v[i] ? v[i] + (si(a[i]) - v[i]) / 4 * 4 : 0);
                s[i] = c;
                tmp += sum[i][c];
            }
            if(chmax(ma, tmp)) ans = s;
        }
    }
    if(empty(ans)) drop(-1);
    cout << SUM(ans) << endl;
    rep(i, 10) { rep(j, ans[i]) cout << a[i][j] << " "; }
    cout << endl;
}