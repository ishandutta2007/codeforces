...
inline void add(vector<pii> *g,int x,int y,int z){
...
inline void findcir(int x){
	...
	for(auto i:g[x]){
		...
		add(f,x,y,i.SS);
		dep[y]=dep[x]+1;fa[y]=x;from[y]=i.SS;
		findcir(y);
	}
}
inline ll dfs(int x){
