...
const val shift = 1_000_005
fun main(){
    FACT.preCal(shift)
    solve.cases{
        val n = getint
        val L = getline(n)
        val types = Array(2){tempIntervalSum(shift * 2 + 1)}
        for(a in L){
            val where = (a + 1 ) %2
            if(where == 0){
                val e = (a-1)/2
                types[where].add(-e + shift, e + shift, 1)
            }else{
                val e = (a/2) -1
                types[where].add(-e-1+shift,e + shift, 1)
            }
        }
        val As = types.map { it.compile() }
        val actual = IntArray(shift * 2 + 1)
        for(p in 0 until 2){
            for(i in As[p].indices) {
                val real = ( i - shift) * 2 + p + shift
                if(real in actual.indices) {
                    actual[real] += As[p][i]
                }
            }
        }
        ...
        for(i in actual.indices) {
            ...
            val new = (vnow modPlus vnow modPlus v modMinus 1) modM v modM inv2 modM p.adjust()
            max_sum = new modPlus max_sum

//            i dei new

            ways = ways modM FACT.store[v]
