...
vector<string> ns={"red","orange","yellow","green","blue","indigo","violet"};
map<string,int> nm;

struct val{
  array<array<pii,8>,2> m{};

  val(){}
  val(string s){
    if(s=="lock") forn(q,2) forn(i,8) m[q][i]=mp(1,i);
    else if(s=="unlock") forn(q,2) forn(i,8) m[q][i]=mp(0,i);
    else{
      int v=nm.at(s);
      forn(q,2) forn(i,8) m[q][i]=mp(q,q?i:v);
    }
  }

  void merge(const val&a, const val&b){
    forn(q,2){
      forn(i,8){
        pii p=a.m[q][i];
        m[q][i]=b.m[p.X][p.Y];
...
struct segtree{
  ...
  void init(int a,int b,int k,const vector<string>&A){
    if(b==a+1){
      T[k]=val(A[a]);
      return;
    }
    ...
    init(a,c,k*2+1,A);
    init(c,b,k*2+2,A);
    T[k].merge(T[k*2+1],T[k*2+2]);
  }
  void setv(int p,val v,int a,int b,int k){
    if(b==a+1){
      T[k]=v;
      return;
    }
...
int main(int argc, char **argv){
  ...
  cin>>tc;
  forn(qq,tc+1){
    cout<<ns[T.T[0].m[0][4].Y]<<'\n';
    if(qq==tc) break;
    int p;
    string s;
    cin>>p>>s;
    --p;
    T.setv(p,val(s),0,n,0);
  }
