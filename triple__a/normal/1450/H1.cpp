...
int inv(int u){
    return basics::power(u,mod-2,mod);
}
signed main(){
    ...
    int res=0;
    auto C=[&](int u,int v){
        return f[u]*inv(f[v]*f[u-v]%mod)%mod;
    };
    for (int diff=-eq;diff<=oq;++diff){
        if ((diff+w)&1) continue;
        if (diff<0){
            res=(res+abs(diff+w)*C(eq+oq,eq+diff))%mod;
        }
        else{
            res=(res+abs(diff+w)*C(eq+oq,oq-diff))%mod;
        }
    }
    cout<<res*inv(basics::power(2,oq+eq,mod))%mod<<endl;
    return 0;
}