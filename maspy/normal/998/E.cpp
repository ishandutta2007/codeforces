...
void solve() {
  ...
  auto f = [&](ll a, ll b) -> mint {
    if (a == 0 || b == 0) return mint(3).pow((N - a) * (N - b) + a + b);
    return mint(3).pow((N - a) * (N - b) + 1);
  };
  ...
  FOR(a, N + 1) {
    mint coef = MC.C(N, a);
    if (a % 2 == 1) coef = -coef;
    mint r = mint(3).pow(N - a);
    mint x = (r - mint(1)).pow(N);
    ANS -= coef * x * mint(3);
  }

  FOR(a, N + 1) FOR(b, N + 1) {
    if (a > 0 && b > 0) break;
    mint coef = MC.C(N, a) * MC.C(N, b);
    if ((a + b) % 2 == 0) coef *= -1;
    ANS -= coef * mint(3).pow((N - a) * (N - b) + 1);
    if (a + b > 0) ANS += coef * mint(3).pow((N - a) * (N - b) + a + b);
  }
  print(ANS);
}
