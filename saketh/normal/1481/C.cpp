...
void solve() {
    ...
    int last = 0;
    while (last < N && (a[last] == b[last] || b[last] != painters.back())) {
        last++;
    }

    if (last == N) {
        last = 0;
        while (last < N && b[last] != painters.back()) {
            last++;
        }
    }
    ...
    F0Rd (i, M - 1) {
        ...
        if (!change_to[color].empty()) {
            res[i] = change_to[color].back();
            change_to[color].pop_back();
        } else {
            res[i] = last;
        }
    }
