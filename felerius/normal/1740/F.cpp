// Conditional Mix (https://codeforces.com/contest/1740/problem/F)
...
int main() {
    ...
    // mx_sum[k]: maximum sum for the k largest elements in the multiset
    vector mx_sum(n + 1, 0);
    for (auto& [_, c] : cnt)
        rep(i, 1, n + 1)
            mx_sum[i] += min(i, c);

    // dp[k][x][y]: number of valid multisets of size k with sum x and minimum y
    using MI = StaticMontgomeryInt<998'244'353, true>;
    MI ans = 0;
    // dp_prev contains suffix sums over y
    vector dp(n + 1, vector<MI>(n + 1)), dp_prev = dp;
    fill(all(dp_prev[0]), 1);
