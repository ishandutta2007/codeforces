...
ll calc(){ll n=readll(),s=readll(),mx=0,cm=0;rep(i,1,n)qmax(mx,cm,readll());return(s/(mx+cm))*2+!!(s%(mx+cm))+(s%(mx+cm)>mx);}
int main(){IO();ll T=readll();while(T --> 0)writell(calc(),10);return 0;}