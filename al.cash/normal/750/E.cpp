...
struct S {
  ...
  S(int diag = 0) { REP(i, M) REP(j, M) m[i][j] = i == j ? diag : INF; }

  friend S operator + (const S& lhs, const S& rhs) {
    S res(INF);
    REP(i, M) REP(j, M) REP(k, M) smin(res.m[i][j], lhs.m[i][k] + rhs.m[k][j]);
    return res;
  }
};

template <> struct is_commutative<S> { constexpr static bool value = false; };
...
int main() {
  ...
  REP(i, q) {
    int a, b;
    read(a, b);
    int ans = tree.query(a - 1, b).m[0][4];
