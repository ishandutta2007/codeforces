...
void solve()
{
    ...
    for (int i = 0; i<n; i++)
    {
        int val; cin>>val; cnt[val-1]++;
    }
    vector<int> guys;
    for (auto it: cnt) if (it) guys.push_back(it);


    int ans = 1e9;
    for (int deg = 1; deg<=2*n; deg*=2)
    {
        int cur = 0;
        int taken = 0;
        while (cur<guys.size() && guys[cur]+taken<=deg)
        {
            taken+=guys[cur];
            cur++;
        }
        ...
        int sum = 0;
        for (int i = cur; i<=guys.size(); i++)
        {
            if (i<guys.size()) sum+=guys[i];
            int sum1 = rem - sum;
            ans = min(ans, deg + ab(sum) + ab(sum1));
        }
    }
