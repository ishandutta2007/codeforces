...
class bitset32 {
  ...
  int n;
  vector<DataType> bits;

public:
  bitset32(int n = 1) : n(n), bits(n / 32 + 1) {}

  int size() const { return n; }
  void resize(int size) { n = size; bits.resize(n / 32 + 1); }
  void reserve(int size) { bits.reserve(size / 32 + 1); }
  ...
  void set(int i, bool value = true) {
    if (value) bits[i >> 5] |= DataType(1) << (i & 31);
    else bits[i >> 5] &= ~(DataType(1) << (i & 31));
  }
  void flip(int i) { bits[i >> 5] ^= DataType(1) << (i & 31); }

  void doOr(const bitset32& other, int shift) {
    int size = other.size() + shift;
    if (size > n) resize(size);
    DataType* data = bits.data() + shift / 32;
    shift &= 31;
    if (shift == 0) {
      ...
      for (int i = 0; i < other.bits.size(); ++i) {
        data[i] |= other.bits[i] << shift;
        data[i + 1] |= other.bits[i] >> -shift;
      }
    }
  }
};

int write(const bitset32& bits) {
  int res = 0;
  for (int i = 0; i < bits.size(); ++i) res += write(bits[i] ? '1' : '0');
  return res;
}
...
int solve(int s) {
  for (int l : c) if (l <= s) ++cnt[l];
  bitset32 bits(1);
  bits.set(0);
  bits.reserve(n);
  FOR(l, 1, s+1) if (cnt[l] > 0) {
    if (2 * l > s) cnt[l] = 1;
    int k2 = (cnt[l] - 1) / 2;
    cnt[l] -= 2 * k2;
    cnt[2 * l] += k2;
...
int main() {
  ...
  writeln(k0 + solve(min(k0, n - k0)), ans);
