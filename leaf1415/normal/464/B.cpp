...
int main(void)
{
	...
	rep(i, 0, 7) rep(j, 0, 7) rep(k, 0, 7) rep(l, 0, 7){
		vector<ll> tmp;
		tmp.pb(i), tmp.pb(j), tmp.pb(k), tmp.pb(l);
		uniq(tmp);
		...
		do{
			...
			do{
				...
				do{
					T d[3];
					rep(m, 0, 2) d[0][m] = a[j][p[0][m]] - a[i][m];
					rep(m, 0, 2) d[1][m] = a[k][p[1][m]] - a[i][m];
					rep(m, 0, 2) d[2][m] = a[l][p[2][m]] - a[i][m];
					
					if(dot(d[0], d[1]) || dot(d[1], d[2]) || dot(d[2], d[0])) continue;
					if(norm(d[0]) == 0 || norm(d[0]) != norm(d[1]) || norm(d[1]) != norm(d[2]) || norm(d[2]) != norm(d[0])) continue;
					
					vector<T> vec;
					rep(m, 0, 7){
						T sum = a[i];
						if(m & 1) sum = add(sum, d[0]);
						if(m & 2) sum = add(sum, d[1]);
						if(m & 4) sum = add(sum, d[2]);
						vec.pb(sum);
					}
					
					multiset<T> S2 = S; bool flag = true;
					for(auto t : vec){
						sort(all(t));
						if(S2.count(t) == 0){
							flag = false;
						...
						S2.erase(S2.lower_bound(t));
					}
					
					if(flag){
						bool used[8]; T ans[8];
						rep(m, 0, 7) used[m] = false;
