...
fun main(){
    solve.cases{
        ...
        val n = aa + 1
        val dif = ask(List(n){'a'}.conca())
        if(dif == n){
            ask(List(aa){'b'}.conca())
            return@cases
        }
        val ret = BooleanArray(n)
        var remainingdif = dif
        for(i in 0 until n-1){
            val ans = ask(List(n){if(it == i)'b' else 'a'}.conca())
            if(ans < dif){
                ret[i] = true
                remainingdif --
            }
        ...
        put(List(n){if(ret[it]) 'b' else 'a'}.conca())
...
apparoch doesn't work

always check for constraints 
but who woudl have thought seriosuly 
