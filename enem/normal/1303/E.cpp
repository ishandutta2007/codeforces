        ...
        REP(i,0,t.size())
        {
            ...
            REP(i,0,a.size()+1)
            {
                REP(j,0,b.size()+1)
                {
                    ...
                    {
                        dp[i][j]=pd[dp[i][j-1]][b[j-1]-'a'];
                    }
                    else if(j==0)
                    {
                        dp[i][j]=pd[dp[i-1][j]][a[i-1]-'a'];
                    }
                    else
                    {
                        dp[i][j]=min(pd[dp[i][j-1]][b[j-1]-'a'],pd[dp[i-1][j]][a[i-1]-'a']);
                    }
                    if(dp[i][j]==(1LL<<60))
                    {
                        tmp=0;
