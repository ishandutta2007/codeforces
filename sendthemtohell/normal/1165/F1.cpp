...
bool ok(vector<int>& v, int mx_day) {
    int n = sz(v);
    unordered_map<int, int> cnt_buy;
    int rst = 0;
    for(int i = 1; i <= n; ++i) {
        int j = upper_bound(all(kek[i]), mx_day) - kek[i].begin() - 1;
        if (j >= 0) {
            cnt_buy[kek[i][j]] += v[i - 1];
        }
        else {
            rst += v[i - 1];
        }
    ...
    int cur = 0;
    //cout << "! " << mx_day << endl;
    for(int d = 1; d <= mx_day; ++d) {
        int buy = min(++cur, cnt_buy[d]);
        cur -= buy;
        //cout << d << " -> " << buy << endl;
        cnt_buy[d] -= buy;
        rst += cnt_buy[d];
...
inline void solve() {
    ...
    //for(int i = 1; i <= 10; ++i) cout << kek[i] << endl;
