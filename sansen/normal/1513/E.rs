...
fn run() {
    ...
    small.sort();
    let small = run_length_encoding(&small);
    large.sort();
    let large = run_length_encoding(&large);
    if small.len() == 1 && small[0].1 == 1 {
        let mut ans = M::one();
        let mut rem = n;
        ans *= M::from(rem);
        rem -= 1;
    ...
    let mut ans = M::new(2) * pc.comb(n, mid);
    let mut sum = 0;
    for (_, k) in small {
        sum += k;
        ans *= pc.ifact(k);
