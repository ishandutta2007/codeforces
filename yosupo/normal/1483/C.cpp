...
int main() {
    ...
    using P = tuple<int, ll, ll>;
    V<ll> dp(n);
    dp[0] = b[0];
    V<P> st;
    st.push_back({h[0], dp[0], max(0LL, dp[0])});
    multiset<ll, greater<ll>> dp0;
    dp0.insert(-TEN(18));
    dp0.insert(dp[0]);
    for (int i = 1; i < n; i++) {
        ll dp1 = -TEN(18);
        while (st.size()) {
            ...
            if (h[i] < get<0>(p)) {
                st.pop_back();
                dp1 = max(dp1, get<2>(p));
                dp0.erase(dp0.find(get<1>(p)));
            } else {
                break;
            }
        }
        dp[i] = max(*dp0.begin(), dp1) + b[i];
        st.push_back({h[i], dp[i], max(dp[i], dp1)});
        dp0.insert(dp[i]);
    }
    dbg(dp);
    for (auto x : dp0) dbg(x);

    pr.writeln(*dp0.begin());
    return 0;
}