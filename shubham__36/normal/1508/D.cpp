...
inline bool up(ll i) {
  return y[i] > 0 or (y[i] == 0 and x[i] >= 0);
}
...
bool cmp(ll i, ll j){
	return up(i) == up(j) ? cross(i,j) > 0 : up(i) < up(j);
}
...
void solve(){
	...
	if(sz(vec)){
		ll r = vec.back();
		vec.pop_back();
		forn(i,n) x[i]-=x[r],y[i]-=y[r];
		sort(all(vec),cmp);
		ll m = sz(vec);
		vp64 ans;
		forn(i,m){
			if(U.findSet(vec[i])!=U.findSet(vec[(i+1)%m]) && cross(vec[i],vec[(i+1)%m])>0){
				U.unionSet(vec[i],vec[(i+1)%m]);
				swap(p[vec[i]],p[vec[(i+1)%m]]);
				ans.pb(mp(vec[i],vec[(i+1)%m]));
			}
		}
		while(p[r]!=r){
			ans.pb(mp(r,p[r]));
