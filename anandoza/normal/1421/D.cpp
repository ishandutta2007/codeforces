...
template<typename T> struct point {
    ...
    template <typename I> friend void sort_by_angle(I first, I last, const point<T> &origin = point<T>{}) {
        first = partition(first, last, [&](auto p) { return p == origin; });
        auto pivot = partition(first, last, [&](auto p) { return p > origin; });
        auto compare = [&](auto l, auto r) { return compare_by_angle(l, r, origin) < 0; };
...
const int Z = 6;
const vector<P> basis{{1, 1}, {0, 1}, {-1, 0}, {-1, -1}, {0, -1}, {1, 0}};
...
void solve() {
    ...
    re(c);
    for (int i = 0; i < Z; i++) {
        ckmin(c[(i + 1) % Z], c[i] + c[(i + 2) % Z]);
    }

    ll ans = INF;
    for (int i = 0; i < Z; i++) {
        P a = basis[i], b = basis[(i + 1) % Z];

        ll d = cross(a, b);
        ll u = dot({b.y, -b.x}, p) / d;
        ll v = dot({-a.y, a.x}, p) / d;

        if (u < 0 || v < 0)
            continue;

        ckmin(ans, u * c[i] + v * c[(i + 1) % Z]);
    }
