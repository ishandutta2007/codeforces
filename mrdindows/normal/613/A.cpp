...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        cin >> x[i] >> y[i];
        double d = sqr(x[i] - x0) + sqr(y[i] - y0);
        if (d > maxd) {
            maxd = d;
        }
        if (d < mind) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        double l = 0;
        double r = 1;
        auto getd = [&](double x, double y){
            return sqr(x - x0) + sqr(y - y0);
        };
        for (int iter = 0; iter < 70; ++iter) {
            double m1 = l + (r - l) * 0.49;
            double m2 = l + (r - l) * 0.51;
            if (getd(x[i] + dx * m1, y[i] + dy * m1) < getd(x[i] + dx * m2, y[i] + dy * m2)) {
                r = m2;
            } else {
                l = m1;
            }
        }
        double dist = getd(x[i] + l * dx, y[i] + r * dy);
        if (dist < mind) {
            mind = dist;
        }
    }
//    DBN(maxd, mind);
    long double ans = PI * maxd - PI * mind;
    cout << fixed;
    cout.precision(10);
    cout << ans << endl;
    return 0;
