...
int zap(int i, int j, bool _or)
{
	cout << (_or ? "or" : "and") << ' ' << i + 1 << ' ' << j + 1 << endl;
	int ans;
	cin >> ans;
	return ans;
}
int f(int a, int _and, int _or)
{
	return _and ^ _or ^ a;
}
int another_main()
{
	int n, k;
	cin >> n >> k;
	int ab0 = zap(0, 1, 0), ab1 = zap(0, 1, 1), ac0 = zap(0, 2, 0), ac1 = zap(0, 2, 1), bc0 = zap(1, 2, 0), bc1 = zap(1, 2, 1);
	vi ans(1);
	for (int i = 0; i < 30; ++i)
	{
		int q = 0;
	...
	ans.pb(f(ans[0], ab0, ab1));
	ans.pb(f(ans[0], ac0, ac1));
	for (int i = 3; i < n; ++i)
		ans.pb(f(ans[0], zap(0, i, 0), zap(0, i, 1)));
	sort(whole(ans));
