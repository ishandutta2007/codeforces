...
#ifdef ANAND_LOCAL
    auto max_it = max_element(secret.begin() + l, secret.begin() + r);
    int max_value = *max_it;
    *max_it = -1;
    result = max_element(secret.begin() + l, secret.begin() + r) - secret.begin();
    *max_it = max_value;
#else
    re(result);
    result--;
...
#ifdef ANAND_LOCAL
    assert(i == max_element(all(secret)) - secret.begin());
#endif
}
...
int main() {
    ...
    bool go_right = second == 0 || query(0, second + 1) != second;
    ...
    auto test = [&](int count) {
        ...
        return second == query(l, r);
    };

    int c = -1 + binarysearch(test, 2, n);

    finish(go_right ? second + c : second - c);
