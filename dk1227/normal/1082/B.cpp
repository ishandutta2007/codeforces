...
int main() {
	...
	if(cnt == 2) {
		if(pos[1].se == pos[2].fi - 2) printf("%d\n", pos[2].se - pos[2].fi + 1 + pos[1].se - pos[1].fi + 1);
		else printf("%d\n", max(pos[2].se - pos[2].fi + 1, pos[1].se - pos[1].fi + 1) + 1);
		return 0;
	}
	int ans = 0;
	rep(i, 1, cnt - 1) {
		To_max(ans, pos[i].se - pos[i].fi + 2);
		if(pos[i].se == pos[i + 1].fi - 2) To_max(ans, pos[i + 1].se - pos[i + 1].fi + pos[i].se - pos[i].fi + 3);
	}
	To_max(ans, pos[cnt].se - pos[cnt].fi + 2);
	cout << ans << endl;
 	return 0;
}