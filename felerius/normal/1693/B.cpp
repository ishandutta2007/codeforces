// Fake Plastic Trees (https://codeforces.com/contest/1693/problem/B)
...
int main() {
    ...
    while (t--) {
        ...
        vector range(n, pair(0, 0));
        read(range);

        auto dfs = [&](int v, auto&& self) -> pair<int, ll> {
            ll potential = 0;
            int ops = 0;
            trav(v2, adj[v]) {
                auto [child_ops, child_potential] = self(v2, self);
                ops += child_ops;
                potential += child_potential;
            }

            potential = min(potential, ll(range[v].second));
            if (potential >= range[v].first)
                return {ops, potential};
            return {ops + 1, range[v].second};
        };
        println(dfs(0, dfs).first);
    }
}