...
int main()
{
    ...
    while(T--)
    {
        ...
        string s,t;
        cin>>s>>t;
        vc<vi> ps(n+1,vi(26)),pt(n+1,vi(26));
        vc<vi> dp(n+1,vi(n+1,1e9));
        for(int i=1;i<=n;i++)
        {
            for(int j=0;j<26;j++)
            {
        ...
        if(!suc) print(-1);
            ...
            for(int i=0;i<=n;i++)
            {
                for(int j=i;j<=n;j++)
                {
                    ...
                    if(i&&s[i-1]==t[j-1]) dp[i][j]=min(dp[i][j],dp[i-1][j-1]);
                    if(i&&ps[i][s[i-1]-'a']<=pt[j][s[i-1]-'a']) dp[i][j]=min(dp[i][j],dp[i-1][j]);
                    //dbg(i,j,dp[i][j]);
                }
            }
            print(dp[n][n]);
