...
#endif
pii bru(int n, int x, int y) {
	vector <int> rest;
	FOR(i, 1, n) if (i != x) rest.push_back(i);
	pii ans = {inf, -inf};
	PER(ALL(rest)) {
		// debug(imie(rest));
		int curr = 1;
		REP(i, n - 1) {
			int c1 = rest[i];
			int c2 = i + 1 + (i + 1 >= y);
			// debug(imie(c1), imie(c2));
			if (c1 + c2 <= x + y)
				curr++;
		}
		mini(ans.first, curr);
		maxi(ans.second, curr);
	}
	rans;
}
pii solve(int n, int x, int y) {
	int s = x + y;
	if (s <= n) return {1, s - 1};
	else return {min(n, s - n + 1), n};
}
int main() {
	// FOR(n, 1, 8) {
		// vector <map <pii, pii> > by_sum(2 * n + 1);
		// FOR(x, 1, n) FOR(y, x, n) {
			// pii b = bru(n, x, y);
			// by_sum[x + y][pii(x, y)] = b;
		// }
		// REP(sum, 2 * n + 1) if (!by_sum[sum].empty()) {
			// pii v = by_sum[sum].begin()->second;
			// bool bad = false;
			// for (auto [_, v2] : by_sum[sum]) if (v != v2) bad = true;
			// if (bad) debug(arr(by_sum, sum));
			// else debug(imie(n), imie(sum), imie(v));
		// }
		// debug(imie(n));
	// }
	// FOR(n, 1, 10) FOR(x, 1, n) FOR(y, 1, n) {
		// auto s = solve(n, x, y), b = bru(n, x, y);
		// if (s != b) {
			// debug(imie(s), imie(b), imie(n), imie(x), imie(y));
			// abort();
		// }
	// }
	int t;
	...
	REP(_, t) {
		int n, x, y;
		scanf("%d%d%d", &n, &x, &y);
		auto [a, b] = solve(n, x, y);
		// assert(bru(n, x, y) == bru(n, y, x));
		printf("%d %d\n", a, b);
	}
}