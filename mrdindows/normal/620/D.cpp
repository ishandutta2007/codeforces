...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
            pa.push_back(2*(a[i] + a[j]));
        }
    }
    for (int i = 0; i < m; ++i) {
        for (int j = i + 1; j < m; ++j) {
            pb.push_back(2*(b[i] + b[j]));
        }
    }
//    sort(all(pa));
    sort(all(pb));
    ...
    ll by = LLINF;
    for (ll x : pa) {
        auto it = lower_bound(pb.begin(), pb.end(), -sa + sb + x);
        if (it == pb.end()) --it;
        ll y = *it;
        ll res = abs(sa - sb - x + y);
        if (res < ans) {
            ans = res;
            bx = x;
            by = y;
        }
        if (it == pb.begin()) continue;
        --it;
        y = *it;
        res = abs(sa - sb - x + y);
        if (res < ans) {
