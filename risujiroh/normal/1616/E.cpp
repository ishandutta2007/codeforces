...
void rj::main(int) {
  ...
  auto ans = inf_v<i64>;
  auto cur = 0ll;
  auto count = vector(26, 0);
  auto p = vector(n, -1);
  auto f = atcoder::fenwick_tree<int>(n);
  for (auto i : rep(n)) f.add(i, 1);
  for (auto i : rep(n)) {
    for (char c : rep('a', t[i])) {
      auto x = count[c - 'a'];
      if (x == size(pos[c - 'a'])) {
        continue;
      }
      p[i] = pos[c - 'a'][x];
      chmin(ans, cur + f.sum(0, p[i]));
    }
    auto x = count[t[i] - 'a'];
    if (x == size(pos[t[i] - 'a'])) {
      break;
