...
impl TE for R {
    ...
    fn eval(x: &Self::T, f: &Self::E) -> Self::T {
        f.map_or(*x, |p| if p == 0 {(x.2, 0, x.2)} else {(0, x.2, x.2)})
    }
    fn merge(g: &Self::E, h: &Self::E) -> Self::E {
        if h.is_some() {
            *h
...
fn run<W: Write>(sc: &mut scanner::Scanner, out: &mut std::io::BufWriter<W>) {
    ...
    for _ in 0..t {
        let n: usize = sc.next();
        let q: usize = sc.next();
        let s: Vec<u8> = sc.next_bytes();
        let f: Vec<u8> = sc.next_bytes();
        let mut op = vec![(0, 0); q];
        for p in op.iter_mut() {
            p.0 = sc.next::<usize>() - 1;
            p.1 = sc.next::<usize>();
        }
        let mut ini = vec![(0, 0, 1); n];
        for (ini, f) in ini.iter_mut().zip(f.iter()) {
            if *f == b'0' {
                ini.0 = 1;
            } else {
                ini.1 = 1;
        ...
        let mut can = true;
        for &(l, r) in op.iter().rev() {
            let (a, b, c) = seg.find(l, r);
            if a.min(b) > (c - 1) / 2 {
                can = false;
                break;
            }
            if a > b {
                ...
                seg.update(l, r, Some(1));
            }
        }
        for i in 0..n {
            can &= (s[i] == b'0') ^ (seg.find(i, i + 1).0 == 0);
        }
        if can {
            writeln!(out, "YES").ok();
        } else {
