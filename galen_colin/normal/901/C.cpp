...
vector<ll> iv[300005];
ll res[300005];
vector<pair<ll, ll>> qv[300005];

vector<ll> st;
bool vis[300005];
bool ist[300005];
vector<ll> at[300005];
segtree<sum_t> good, bad;
...
void solve(int tc = 0) {
	...
	good.init(n, c);
	bad.init(n, c);
	for (ll i = 0; i < n; i++) good.update(i, i, 1);
	...
	for (ll i = 0; i < n; i++) {
		...
		// for (ll j = 0; j < n; j++) {
			// cout << make_pair(good.query(j, j).val, bad.query(j, j).val) << " ";
		// }
		// cout << endl;
		
		for (pair<ll, ll> x: qv[i]) {
			ll ans = -(sumv(i) - sumv(x.f - 1));
			ll cnt = good.query(x.f, i).val;
			ans += cnt * (i + 1);
			ans += bad.query(x.f, i).val;
			res[x.s] = ans;
		}
	}
