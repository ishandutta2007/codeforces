...
fun modcountbrute(x:Int, mod:Int, l:Int,r:Int):Int{
    var ret = 0
    for(a in l..r){
        if(a % mod == x || (a % mod) + mod == x){
            ret++
        }
    }
    return ret
...
//    (-1).div
    solve.cases{
        ...
        val bs = (s+1)/k
        val left = (s+1) % k
        var base = (bs divceil 2L) * k + if(bs %2L == 0L) left else 0L
//        put(base)
        if(n <= base-1) {
            put("NO")
        }else{
...
//        val base = (s+1)/k
//        var plusone = (s+1) % k
//        var normal = k - plusone
//
//        val plusoneReward = ((base +1) divceil 2L)
//        val normalReward = (base divceil 2L )
//
//
//        assert(plusone * (base + 1) + normal * base == s+1)
//        var total = 2
//        if(n % k == 0L) {
//            plusone--
//
