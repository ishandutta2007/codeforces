...
int64 mypower(int64 p,int64 e,int64 m)
{
	if (e==0) return 1%m;
	if (e&1) return mypower(p,e-1,m)*p%m;
	else return mypower(p*p%m,e/2,m);
}
int main()
{
//	freopen("input.txt","r",stdin);
	int _m;
	scanf("%s%s%d",&sb,&sn,&_m);
	int64 m=_m;
	int64 b=getmod(sb,m);
	if ((int)strlen(sn)<=15)
	{
	...
	else
	{
		...
		else
		{
			...
			int64 p=getmod(sn,e);
			int64 h=e;
			while (h<10000000LL*10000000LL) h*=2;
			h+=p;
			int64 R=(b+m-1)%m;
			int64 T=mypower(b,h-1,m);
			R=(R*T)%m;
			if (R==0) R=m;
			printf("%d\n",(int)R);
		}
