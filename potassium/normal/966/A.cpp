...
int main() {
	...
	int q; R(q);
	while (q--) {
		int x1,y1,x2,y2; R(x1,y1,x2,y2);
		int ans = INT_MAX;

		auto calc = [&](int p, int v) {
			return abs(y1 - p) + (abs(x2 - x1) + v - 1) / v + abs(y2 - p);
		};

		if (x1 == x2)
			chkmin(ans, abs(y2 - y1));
		
		auto itl = lower_bound(l.begin(), l.end(), y1);
		chkmin(ans, calc(*itl, 1));
		chkmin(ans, calc(*(itl - 1), 1));
		
		auto ite = lower_bound(e.begin(), e.end(), y1);
		chkmin(ans, calc(*ite, v));
		chkmin(ans, calc(*(ite - 1), v));
