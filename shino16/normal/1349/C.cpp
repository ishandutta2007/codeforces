...
int main() {
  int h = IN, w = IN, t = IN;
  auto table = IN.v<string>(h);

  auto start = make_v(h, w, INF);
  ...
  rep(r, h) rep(c, w) rep(i, 4) {
    int r2 = r + dr[i], c2 = c + dc[i];
    if (inside(r2, c2) and table[r2][c2] == table[r][c]) start[r][c] = 0;
  }

  deque<PII> togo;
  rep(r, h) rep(c, w) if (start[r][c] == 0) togo.emplace_back(r, c);
  ...
  while (not togo.empty()) {
    ...
    rep(i, 4) {
      int r2 = r + dr[i], c2 = c + dc[i];
      if (not inside(r2, c2)) continue;
      if (table[r2][c2] == table[r][c]) {
        if (chmin(start[r2][c2], start[r][c])) togo.emplace_front(r2, c2);
      } else {
        if (chmin(start[r2][c2], start[r][c]+1)) togo.emplace_back(r2, c2);
      }
    }
  }

  rep(i, t) {
    int r = IN1, c = IN1; ll p = IN;
    if (start[r][c] == INF or p < start[r][c]) { OUT(table[r][c]); continue; }
    else OUT((p - start[r][c]) % 2 ? char(table[r][c]^'0'^'1') : table[r][c]);
  }
}