    ...
    TEST {
        ...
        rep(i, 2) per(j, n) {
            if(j == n - 1) {
                ...
                mi1[i][j] = max(mi1[i][j + 1], a[i][j] + j);
                mi2[i][j] = max(mi2[i][j + 1], a[i][j] - j);
            }
        }
        OUT(bin_search(inf<int>, -1,
                       [&](int x) {
                           int i = 0, j = 0;
                           int t = x;
                           while(j < n) {
                               if(j < n - 1 and mi2[i][j + 1] + j <= t - 1 and mi1[i ^ 1][j] - (n - 1) <= t - 1 + (n - j)) { return true; }
                               i ^= 1;
                               if(a[i][j] > t) return false;
                               t++;
                               if(j < n - 1 and mi2[i][j + 1] + j <= t - 1 and mi1[i ^ 1][j + 1] - (n - 1) <= t - 1 + (n - j)) { return true; }
                               j += 1;
                               if(j == n) break;
                               if(a[i][j] > t) return false;
                               t++;
