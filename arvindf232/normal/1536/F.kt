...
fun main(){
    ...
    solve.cases{
        ...
        for(space in 0..n){
            val chars = n - space
            if(chars <= 0) continue
            if(chars %2 != 0 ) continue
            if(chars < space) continue
            val have = FACT.choose(chars,space) modM FACT.store[chars - 1]
            ret = ret modPlus have
        }
        put(ret modM 2 modM n)

//        var DP = IntArray(4)
//        DP[0] = 1
//        repeat(n){
//            val (a,b,a_,b_) = DP
//            val new = intArrayOf(a + a_, b + b_, a, b)
//            DP = new
//        }
//        val ans = DP[0]
