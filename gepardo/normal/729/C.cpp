...
int n = in, k = in, s = in, t = in;
vector < pair <int, int> > car;
vector <int> g;

inline int time_dist(int v, int d, bool& ok)
{
    if (v < d)
    {
        ok = false;
...
inline bool check(int v)
{
    bool ok = true;
    int time = 0;
    time += time_dist(v, g[0] - 0, ok);
    for (int i = 1; i < k; i++)
        time += time_dist(v, g[i] - g[i-1], ok);
    time += time_dist(v, s - g[k-1], ok);

    if (!ok) return false;
    return time <= t;
}

inline int binsearch()
{
    int l = 0, r = n-1;
    while (l < r)
    {
        int m = (l+r) / 2;
        if (check(car[m].S)) r = m; else l = m+1;
    }
    if (check(car[l].S))
        return car[l].F;
    else
