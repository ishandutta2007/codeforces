...
public:
    ...
    vector<ui> W;
    map<ui,ui> E;
    map<ui,vector<pair<ui,ui>>> F;

    inline ui mod(ull x, ui m) {
        if (x >= m) return m+x%m;
        else return x%m;
    }

    inline ui pow(ui a, ui p, ui m){ if(p==0) return a;
        ui r=1,e=a;while(p){if(p&1){r=mod((ull)r*e,m);}e=mod((ull)e*e,m);p>>=1;}return r;}
    ...
    ui ans(ui l, ui r, ui m) {
        if (l > r || m == 1) return 0;
        factorize(m);
        return mod(1ULL*pow(W[l], ans(l+1, r, E[m]), m), m);
    }
