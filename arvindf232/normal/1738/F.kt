...
fun main(){
    solve.cases{
		...
		while(deleted.any { !it }){
			val best = (0 until n).maxByOrNull { if(deleted[it]) -1  else degree[it] }!!
			deleted[best] = true
			for(i in 0 until degree[best]){
				val v = ask(best)
				dsu.union(best,v)
...
//			val have = List(degree[best]){ask(best)}
//
//			for(v in have){
//				dsu.union(best,v)
//				deleted[v] = true
...
//			}
		...
		put("! ${(0 until n).map { dsu.find(it) + 1 }.conca()}")
