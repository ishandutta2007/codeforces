...
void solve() {
    ...
    read(a, n);
    ranges.rsz(n);
    F0R(i, n) ranges[i] = mp(a[i], 1);
    F0R(i, n) s.insert(i);
    F0R(i, n-1) x.insert(i);
    while (! x.empty()) {
        int i = *x.begin(); x.erase(x.begin());
        if (s.find(i)==s.end() || s.ub(i)==s.end()) continue;
        int j = *s.ub(i);
        if (ranges[i].F*ranges[j].S >= ranges[j].F*ranges[i].S) {
            ranges[i].F += ranges[j].F;
            ranges[i].S += ranges[j].S;
            s.erase(j);
            x.insert(i);
            if (s.find(i) != s.begin()) x.insert(*next(s.find(i), -1));
        }
    }
    for (int i : s) {
        db val = (db)ranges[i].F / ranges[i].S;
        F0R(rep, ranges[i].S) cout << val << nl;
    }
}
