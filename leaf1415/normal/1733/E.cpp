...
int main(void)
{
	...
	while(Q--){
		...
		rep(i, 0, 119) rep(j, 0, 119) a[i][j] = b[i][j] = 0;
		
		a[0][0] = t, b[0][0] = t-1;
		rep(i, 0, 119) rep(j, 0, 119){
			a[i+1][j] += a[i][j]/2, a[i][j+1] += ceil(a[i][j], 2);
			b[i+1][j] += b[i][j]/2, b[i][j+1] += ceil(b[i][j], 2);
		}
		if(a[x][y] == b[x][y]) no();
		else yes();
	}
