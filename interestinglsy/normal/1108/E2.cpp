...
int main(){
	...
	int allmin = *min_element(a+1,a+1+n);

	int poi;

	Build(ROOT);
	sort(All(modi),[]( const Modi &a , const Modi &b ){ return a.r < b.r; });
	poi = 0;
	For(i,n){
		while( poi < modi.size() && modi[poi].r <= i ){
			Sub(modi[poi].l,modi[poi].r,ROOT);
			++poi;
		}
		lmin[i] = Min(1,i,ROOT);
	}
	...
	Build(ROOT);
	sort(All(modi),[]( const Modi &a , const Modi &b ){ return a.l < b.l; });
	poi = modi.size()-1;
	fOR(i,n){
		while( poi >= 0 && modi[poi].l >= i ){
			Sub(modi[poi].l,modi[poi].r,ROOT);
			--poi;
		}
		rmin[i] = Min(i,n,ROOT);
	...
	For(i,n){
		int tmin = allmin;
		if( i != 1 ) Mymin(tmin,lmin[i-1]);
		if( i != n ) Mymin(tmin,rmin[i+1]);
		int tans = a[i] - tmin;
		if( tans > ans ){
			ans = tans;
			ansmaxpos = i;
	...
	vc<int> ansl;
	for( auto useg : modi ){
		if( useg.l <= ansmaxpos && ansmaxpos <= useg.r ) continue;
		ansl.Pb(useg.id);
	}

	printf("%d\n",ans);
	printf("%d\n",ansl.size());
	for( auto u : ansl ) printf("%d ",u);
