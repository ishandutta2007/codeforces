...
sort(delta.begin(),delta.end(),[&](int a,int b){return a>b;});
for(int x=1;x<=n;x++)
rb[x]=rb[x-1]+delta[x-1];
int m;
cin>>m;
unordered_map<int,unordered_map<int,int,custom_hash>,custom_hash>um;
while(m--)
{
	int a,b;
	cin>>a>>b;
	int qwq=gcd(a,b);
	if(um[a][b]){cout<<um[a][b]<<endl;continue;}
	int ans=-1;
	if(n%qwq==0){
	int x=0,y=0;
	exGcd(a,b,x,y);
	x=x*n/qwq,y=y*n/qwq; 
	x=(x%(b/qwq)+(b/qwq))%(b/qwq);
	y=(n-a*x)/b;
	for(int k=0;y-a*k/qwq>=0;k++)
	if(b*(y-a*k/qwq)<=n)ans=max(ans,rb[b*(y-a/qwq*k)]);}
	um[a][b]=ans;
	cout<<ans<<endl;
}
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
