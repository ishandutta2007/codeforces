...
int main(int argc, char** argv) {
    ...
    struct ST {
        ...
        ST(int n) {
            a.resize(4 * n);
            fill(begin(a), end(a), pair{1e9, 1e9});
        }

        pair<int, int> pull(pair<int, int> x, pair<int, int> y) {
            // vector<int> t = {x.first, x.second, y.first, y.second};
            // sort(begin(t), end(t));
            // return {t[0], t[1]};
            if (x.first > y.first) {
                swap(x, y);
            }
            return {x.first, min(min(x.second, y.first), y.second)};
        }

        void build(int i, int l, int r, const vector<int>& b) {
            if (l + 1 == r) {
                a[i] = {b[l], 1e9};
            } else {
                int m = (l + r) >> 1;
        ...
        void set(int i, int l, int r, int qi, pair<int, int> qx) {
            if (l + 1 == r) {
                a[i] = qx;
            } else {
                int m = (l + r) >> 1;
