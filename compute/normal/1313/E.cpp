...
int main()
{
    ...
    for(int i=0;i<n;i++)
    {
        while(cur-i<m-1&&cur<n)
        {
            if(fb(cur)>0)
            {
                cnt.add(max(0,m-1-fb(cur)),1);
                sum.add(max(0,m-1-fb(cur)),fb(cur));
            }
            cur++;
        }
        if(fa(i)>0)
        {
            int cc=cnt.query(fa(i)-1);
            ans+=sum.query(fa(i)-1)+(ll)cc*(fa(i)-m+1);
            //dbg(i,fa(i),fb(i),cc,ans);
        }
        if(fb(i)>0)
        {
            cnt.add(max(0,m-1-fb(i)),-1);
            sum.add(max(0,m-1-fb(i)),-fb(i));
        }
    }
    print(ans);
