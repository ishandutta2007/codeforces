...
int main() {
    ...
    rep(c, n) {
        ...
        rep(i, 1 << n) {
            if(i) {
                int k = topbit(i);
                AA[c][i] = AA[c][i - (1 << k)] * (a[k] + a[c]);
            }
        }
        rep(i, 1 << n) AA[c][i] = inv(AA[c][i]);
    }
    ...
    rep(i, n) {
        pp[i][0] = 1;
        rep(j, n) pp[i][j + 1] = pp[i][j] * a[i];
    }

    vmint dp(1 << n), cnt(1 << n);

    auto id = iota(1 << n);
    sort(all(id), [](int i, int j) { return popcount(i) < popcount(j); });
    ...
    for(auto i : id) {
        ...
        int rem = allbit(n) - i;
        {
            mint now = (1 - dp[i]) * popcount(i);
            rep(j, n) {
                if(i & 1 << j) now *= AA[j][rem], now *= pp[j][popcount(rem)];
            }
            ans += now;
        }
        dp[i] = 1 - dp[i];

        for(int b = rem; b; b = (b - 1) & rem) {
            mint now = dp[i];
            rep(j, n) {
                if(i & 1 << j) {
                    now *= AA[j][b];
