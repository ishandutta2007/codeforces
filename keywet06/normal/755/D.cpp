...
inline int64 sum(int x) {
    tmp = 0;
    while (x) tmp += s[x], x &= x - 1;
    return tmp;
}
inline int64 sum(int l, int r) { return sum(r) - sum(l - 1); }
inline void write(int64 x) {
    while (x) c[--cnt] = x % 10 + '0', x /= 10;
    printf("%s ", c + cnt), cnt = S;
}
...
int main() {
    ...
    for (int i = 1; i <= n; ++i) {
        to = (now + k - 1) % n + 1;
        ans += to < now ? sum(now + 1, n) + sum(1, to - 1) + 1
                        : sum(now + 1, to - 1) + 1;
        write(ans);
        add(now), add(now = to);
    }
