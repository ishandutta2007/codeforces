...
int main() {
    ...
    vll r(K + 1, 0), l(K + 1, 2 * N);
    rep(i, K)cin >> l[i] >> r[i + 1];

    lazy_segtree<ll, min, e, ll, min, min, e> lsg(2 * N + 1);

    lsg.set(l[0], 0);

    rep(i,K){
        lazy_segtree<ll, min, e, ll, min, min, e> nlsg(2 * N + 1);
        ...
        rep(j, 2 * N + 1) {           
            ...
            ll lef = j+l[i + 1] - r[i + 1];
            ll ri = j+l[i + 1] - l[i];
            nlsg.apply(lef, ri, x + 2);
            nlsg.apply(ri, x);
            //cout << j << " " << lef << ri << endl;
            lef = (l[i] - j) + l[i + 1] - r[i + 1];
            ri = (l[i] - j) + l[i + 1] - l[i];
            //cout <<j<<" "<< lef << ri << endl;
            nlsg.apply(lef, ri + 1, x + 1);
        }
    ...
    ll ans = lsg.get(N);
    if (ans == INF) { cout << "Hungry" << endl; }
    else { cout << "Full" << endl << ans << endl; }
