...
int main() {
    ...
    for (int i = 0; i<n; i++)
    {
        int cur = (i+1)%n;
        int best = (cur+1)%n;
        while (cur!=i && (cur+1)%n != i)
        {
            if (best==cur) best = (best+1)%n;
            while ((best+1)%n!=i && area(a[i], a[cur], a[best])<area(a[i], a[cur], a[(best+1)%n])) best = (best+1)%n;
            ll x = area(a[i], a[cur], a[best]);
            if (x>maxx)
            {
                maxx = x;
                ans = {a[i], a[cur], a[best]};
            }
            cur = (cur+1)%n;
        }
    ...
    for (int i = 0; i<3; i++)
    {
        cout<<ans[(i+1)%3].x + ans[(i+2)%3].x - ans[i].x<<' '<<ans[(i+1)%3].y + ans[(i+2)%3].y - ans[i].y<<endl;
    }
