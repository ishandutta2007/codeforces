...
void solve() {
    ...
    cin >> s;
    REP(q, 26) {
        cnts[q][s.length() + 1] = s.length();
        cnts[q][s.length()] = s.length();
    }
    ...
    REP(i, query) {
        ...
        if (d == "-") {
            ...
            if (e == 0) {
                ...
                for (int i = val[0] - 1; i <= val[0]; ++i) {
                    REP(q, val[1] + 1) {
                        REP(j, val[2] + 1) {
                            if (i < val[0]) {
                                dp[i + 1][q][j] = min(dp[i + 1][q][j], cnts[Go[0][i]][dp[i][q][j]] + 1);
                            }
                            if (q < val[1]) {
                                dp[i][q + 1][j] = min(dp[i][q + 1][j], cnts[Go[1][q]][dp[i][q][j]] + 1);
                            }
                            if (j < val[2]) {
                                dp[i][q][j + 1] = min(dp[i][q][j + 1], cnts[Go[2][j]][dp[i][q][j]] + 1);
                            }
                        }
                    }
                }
        ...
        if (dp[val[0]][val[1]][val[2]] <= s.length()) {
            cout << "Yes" << endl;
        }
        else {
            cout << "No" << endl;
