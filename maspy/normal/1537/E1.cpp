...
void solve() {
  ...
  auto compare = [&](int i, int j) -> bool {
    assert(i <= j);
    auto xi = RH.query(SH, 0, i);
    auto xj = RH.query(SH, 0, j);
    auto check = [&](int n) -> bool {
      if (n <= i) { return 1; }
      if (n <= j) {
        auto a = RH.combine(xi, RH.query(SH, 0, n - i), n - i);
        auto b = RH.query(SH, 0, n);
        return a == b;
      }
      auto a = RH.combine(xi, RH.query(SH, 0, n - i), n - i);
      auto b = RH.combine(xj, RH.query(SH, 0, n - j), n - j);
      return a == b;
    };
    ll lcp = binary_search(check, 0, i + j + 1);
    if (lcp == i + j) return false;
    char a = (lcp < i ? S[lcp] : S[lcp - i]);
    char b = (lcp < j ? S[lcp] : S[lcp - j]);
    return a < b;
  };
  ll best = 1;
  FOR(n, 2, len(S) + 1) {
    if (!compare(best, n)) best = n;
  }
  S.resize(best);
  while (len(S) < K) S += S;
  S.resize(K);
