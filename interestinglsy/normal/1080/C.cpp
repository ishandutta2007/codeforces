...
ll x1,y11,x2,y2;
ll x3,y3,x4,y4;
pll Count( ll n , ll m , bool iswhite ){
	ll s = n*m;
	ll w = s/2, b = s/2;
	if( iswhite && s%2 == 1 ) w++;
	if( !iswhite && s%2 == 1 ) b++;
...
int main(){
	...
	For(TwT,Testcase){
		...
		ans.F += Count((x2-x1+1),(y2-y11+1),(x1+y11)%2 == 0).S;
		ll y5 = max(y11,y3);
		ll y6 = min(y2,y4);
		ll x5 = max(x1,x3);
		ll x6 = min(x2,x4);
		if( y5 <= y6 && x5 <= x6 ){
			ans.F -= Count(x6-x5+1,y6-y5+1,(x5+y5)%2==0).S;
			//cout << ans.F << endl;
		}
		ans.F -= Count(x4-x3+1,y4-y3+1,(x3+y3)%2==0).F;
		printf("%lld %lld\n",ans.F,n*m-ans.F);
	}
