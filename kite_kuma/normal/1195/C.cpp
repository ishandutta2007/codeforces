...
int main() {
	...
	vector<vll> dp(3, vll(n + 1, -dekai));
	// 01
	dp[0][0] = 0;
	rep(i, n) {
		chmax(dp[0][i + 1], max({dp[0][i], dp[1][i], dp[2][i]}));
		chmax(dp[1][i + 1], dp[2][i] + h[0][i]);
		chmax(dp[1][i + 1], dp[0][i] + h[0][i]);
		chmax(dp[2][i + 1], dp[1][i] + h[1][i]);
		chmax(dp[2][i + 1], dp[0][i] + h[1][i]);
	}
	ll ans = 0;
	rep(i, 3) {
		rep(j, n + 1) { chmax(ans, dp[i][j]); }
	}
	drop(ans);
}