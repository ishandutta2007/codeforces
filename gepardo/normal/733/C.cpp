...
void eatage(int l, int r, int beg)
{
    if (l == r) return;
    int mv = -1;
    for (int i = l; i <= r; i++)
        mv = max(mv, a[i]);
    int pivot = -1, side = 0;
    if (a[l] == mv && a[l+1] < mv) pivot = l, side = +1;
    if (a[r] == mv && a[r-1] < mv) pivot = r, side = -1;
    for (int i = l+1; i < r; i++)
    {
        if (a[i] == mv && a[i+1] < mv) pivot = i, side = +1;
        if (a[i] == mv && a[i-1] < mv) pivot = i, side = -1;
    }
    int curid = pivot - l + beg;
    if (side > 0)
    {
        for (int i = pivot+1; i <= r; i++)
            out.putLn(curid, ' ', 'R');
        for (int i = pivot-1; i >= l; i--)
            out.putLn(curid, ' ', 'L'), curid--;
    }
    else
    {
        for (int i = pivot-1; i >= l; i--)
...
main()
{
    ...
    for (int j = 1; j <= m; j++)
        for (int i = 1; i <= n; i++)
        {
            ...
            for (int k = i-1; k >= 0; k--)
            {
                if (a[k] != last) equals = false;
                sum += a[k];
                last = a[k];
                if (equals && k != i-1) continue;
                if (sum == b[j-1] && dp[k][j-1])
                {
                    dp[i][j] = true;
                    p[i][j] = k;
                }
