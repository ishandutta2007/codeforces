...
tuple<vi, int, int> check(const int c, const vpib& v, const vi& d, const vi& e, unordered_map<int, int> D, unordered_map<int, int> E) {
	tuple<vi, int, int> ans;
	get<1>(ans) = int(1e9) - (c + 1) / 2;
	get<2>(ans) = get<1>(ans) + c;
	for (const pib& p : v) {
		int y = abs(p.X - c);
		if (p.Y) {
			if (E[p.X] == 0)
...
int another_main() {
	...
	for (int I = 0; I < t; ++I) {
		...
		tuple<vi, int, int> ans;
		for (int i = 0; i < n; ++i) {
			ans = check(abs(d[0] - e[i]), v, d, e, D, E);
			if (!get<0>(ans).empty())
				break;
			ans = check(abs(d[0] + e[i]), v, d, e, D, E);
			if (!get<0>(ans).empty())
