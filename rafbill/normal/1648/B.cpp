...
void solve(){
  ...
  { vi A(n); for(li i = 0; i < (li)(n); ++i) {cin>>A[i];E[A[i]]=1; } }

  BIT bit(2*c+1);
  for(li i = 0; i < (li)(c+1); ++i) if(E[i]) bit.add(i,1);

  for(li y = (1); y <= (li)(c); ++y) if(E[y]) {
    for(li k = 1; k*y <= c; k++) if(!E[k]) {
      if(bit.getRange(k*y, (k+1)*y-1)) {
        cout << "No\n";
        return;
      }
    }
