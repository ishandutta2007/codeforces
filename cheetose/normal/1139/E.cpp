...
const int S = 10003, T = 10004;
int level[10005],work[10005];
bool bfs() {
	MEM_1(level);
	level[S] = 0;
	queue<int> q;
...
int main() {
	...
	fup(i,0,n-1,1)scanf("%d",b+i),b[i]--;
	fup(i,0,m-1,1)addedge(S,i,1,0);
	int q;
	scanf("%d",&q);
	while(q--)
	{
	...
	fup(i,0,n-1,1)
	{
		if(!chk[i])addedge(b[i],m+a[i],1,0);
	}
	int now=0,fl=0;
	while(1)
	{
