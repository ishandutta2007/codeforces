...
fun solve(things:IntArray):Pair<Int,Int>{
    things.shuffle()
    val n  = things.size
    val all = things.toMutableSet()
    if( n == 0){
        return Pair(0,1)
    }
    val root = things[0]
    ...
    for(i in 1 until n) {
        val test = things[i]
        val d = test modMinus root
        var now = root
        var total = 1
        while(true) {
            now = now modPlus d
            if(now != root && all.contains(now)) {
                total++
                continue
            } else {
                break
...
fun main(){
    solve.cases{
        ...
        if(n >= pI /2){
            val all = IntArray(pI){1}
            for(a in things){
                all[a]--
            }
            val newall = (0 until pI).filter{all[it] == 1}.toIntArray()
            val ans = solve(newall)
            if(ans.first == -1){
                put(-1)
            }else{
                val (a,b) = ans
                put(a modMinus b )
                put(if(b == 0) 0 else pI - b)
            }
        }else{
            val ans = solve(things)
            if(ans.first == -1){
                put(-1)
            }else{
                put(ans.first)
