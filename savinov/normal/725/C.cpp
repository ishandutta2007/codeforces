...
void solve(int test) {
    ...
    forn(i, s.length()) cnt[s[i] - 'A']++;

    int pos = max_element(cnt, cnt + 26) - cnt;

    int p = s.find_last_of(string(1, pos + 'A'));

    string v = s.substr(0, p) + s.substr(p + 1);
