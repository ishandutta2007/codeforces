...
int min_end_depth(tree<edge> &tr, int loc, int K) {
    vector<int> cres;

    trav (e, tr.children[loc])
        cres.push_back(min_end_depth(tr, e(loc), K));
    ...
    trav (e, cres) {
        ...
        if (e + 2 > K)
            must_be_last.push_back(e);
        else
            remaining.push_back(e);
    }

    if (sz(must_be_last) > 1)
        return -1;

    if (sz(must_be_last) > 0)
        return must_be_last[0] + 1;

    int use_last = INT_MAX;
    trav (e, remaining) ckmin(use_last, e);
    return use_last + 1;
}
