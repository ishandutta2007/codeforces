...
vector<int> diff;
void solve() {
    static_assert(sizeof(decay_t<decltype(diff.size())>) == sizeof(uint64_t));
    cin >> n >> x >> y >> a >> b;
    for (auto&& c : a) c -= '0';
    for (auto&& c : b) c -= '0';
    diff.clear();
    ...
    x = min(x, y * 2);
    if (diff.size() == 2) {
        if (diff[0] + 1 == diff[1]) cout << x << '\n';
        else cout << min(1LL * y, 1LL * (diff[1] - diff[0]) * x) << '\n';
        return;
    }
    if (y <= x) {
        cout << (diff.size() / 2) * y << '\n';
        ...
        // do everything with y
        int64_t f[2];  // whether to pair this one with the next one
        int64_t g[2];
        f[0] = (diff.size() / 2) * y;
        f[1] = 1e18;
        for (int i = 0; i < diff.size(); i++) {
            ...
            f[1] = 1e18;
            f[0] = min(f[0], g[0]);
            if (i) f[0] = min(f[0], g[1] + 1LL * x * (diff[i] - diff[i - 1]) - y);
            f[1] = min(f[1], g[0]);
        }
        cout << f[0] << '\n';
    }
