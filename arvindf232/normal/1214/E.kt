...
fun main(){
    solve.cases{
        val n = getint()
        val L = getline(n)
        val at = L.withIndex().map{Pair(it.index, it.value)}.sortedByDescending { it.second }
        val guy = IntArray(2 * n)

        val ret = mutableListOf<Pair<Int,Int>>()
        var lastP = n-1
        for(i in 0 until n){
            guy[i] = (2 * at[i].first + 1)
        }
        for(i in 0 until n){
            val here = (2 * at[i].first + 2 )
            val attach = at[i].second + i - 1
            if(attach == lastP){
                lastP ++
                guy[lastP] = here
            }else{
                ret.add(Pair(guy[attach],here))
            }
        }
        for(i in 0..lastP-1){
            ret.add(Pair(guy[i], guy[i+1]))
        }
        for(p in ret){
