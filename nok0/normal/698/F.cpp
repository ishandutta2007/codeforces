...
void main_() {
	...
	map<int, V<>> mp;
	REPS(i, n) {
		if(pf.isPrime(i) or i == 1) {
			mp[f(i)].pb(i);
		}
	}

	auto prime_vec = [&](int x) {
		auto vec = pf.PF(x);
		V<> primes;
		foa(p, vec) primes.pb(p.first);
		if(x == 1) primes.pb(1);
		return primes;
	};
	...
	REP(i, n) {
		if(p[i]) {
			auto p_primes = prime_vec(p[i]);
			auto i_primes = prime_vec(i + 1);
			if(SZ(p_primes) != SZ(i_primes)) {
				print(0);
				return;
			}

			REP(j, SZ(p_primes)) {
				int u = p_primes[j], v = i_primes[j];
				if(f(u) != f(v) or (out[u] and out[u] != v) or (in[v] and in[v] != u)) {
					print(0);
					return;
				}
				out[u] = v;
