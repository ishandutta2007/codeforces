...
int main() {
    ...
    while (cur!=n)
    {
       ...
       set<int> in;
       set<int> left;
       if (a[cur]<0) {cout<<-1; return 0;}
       in.insert(a[cur]);
       left.insert(a[cur]);
       cur++;
       while (cur<n&&!left.empty())
       {
           if (a[cur]>0)
           {
               if (in.find(a[cur])!=in.end()) {cout<<-1; return 0;}
               in.insert(a[cur]);
               left.insert(a[cur]);
           }
           if (a[cur]<0)
           {
               if (left.find(-a[cur])==left.end()) {cout<<-1; return 0;}
               left.erase(-a[cur]);
           }
           cur++;
       }
       if (!left.empty()) {cout<<-1; return 0;}
       answer.push_back(cur - cur1);
    }
