...
namespace Graphs{
	...
	public:
		inline uint32_t latest_minimum_depth(const uint32_t& x, const uint32_t& y) const { // returns latest occurence of minimum depth node.
			...
			return (b == -1 ? y : (a == -1 ? x : (depth[a] < depth[b] ? x : y)));
		}
...
//     Graphs::forest t(10);
//     t.build();
//     // trace(type_name<decltype(t.rmq)>());

//     // trace(type_name<decltype(forest::latest_depth(t))>());
//     // trace(type_name<decltype(&t.latest_minimum_depth)>());

//     // auto g = &forest::latest_minimum_depth;
//     // trace(type_name<decltype(g)>());
//     auto ptr = &t;
//     trace(type_name<decltype(ptr)>());

//     // trace(type_name<decltype(ptr->*g)>());
//     // auto g2 = (t.*g)(1, 2);

//     // auto hello = &forest::latest_depth;


//     // trace(type_name<decltype(f)>());
//     // auto f2 = bind(forest::latest_depth, t, placeholders::_1, placeholders::_2);



//     auto func = [&] (const Graphs::forest& f, uint32_t x, uint32_t y) {
//     	return f.latest_minimum_depth(x, y);
//     };

//     trace(type_name<decltype(func)>());


//     // std::_Bind<unsigned int (*(forest, std::_Placeholder<1>, std::_Placeholder<2>))(forest const&, unsigned int const&, unsigned int const&)> g2 = bind(forest_helper, t, placeholders::_1, placeholders::_2);
//     // trace(type_name<decltype(g2)>());


//     // indexed_rmq<decltype(g2)> Q(g2);


//     // trace(type_name<decltype(forest_helper)>());


//     // trace(type_name<decltype(f2)>());
...
//     // trace((g.*t)(1, 2));
//     // trace(decltype(t.rmq));
//     // decltype(&t.latest_minimum_depth);
// }
int main(){
