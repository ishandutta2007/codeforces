...
_ n, a[maxn], sm, c, fc[maxn], iv[maxn];
v_ b, d;
...
inline void AC() {
    ...
    n = read();
    fc[0] = iv[0] = 1;
    for(_ i = 1; i <= n; ++i) fc[i] = fc[i-1]*i%mod;
    for(_ i = 1; i <= n; ++i) iv[i] = pw(fc[i], mod-2);
    for(_ i = 1; i <= n; ++i) sm += a[i] = read();
    if(sm % n) {
        writeln("0");
        leave
    }
    ...
    if(b.size() == 1 || d.size() == 1) {
        if(d.size() == 1) swap(b, d);
        _ sp = n-c;
        s = sp*fc[d.size()]%mod;
        sort(d.begin(), d.end());
        d.push_back(1E11);
        _ t = 1;
        for(_ i = 1; i < d.size(); ++i) if(d[i] == d[i-1]) ++t;
        else {
            s = s*iv[t]%mod;
            t = 1;
        }
    } else {
        s = 2*fc[d.size()]%mod*fc[b.size()]%mod;
        sort(b.begin(), b.end());
        sort(d.begin(), d.end());
        d.push_back(1E11);
        b.push_back(1E11);
        ...
        t = 1;
        for(_ i = 1; i < b.size(); ++i) if(b[i] == b[i-1]) ++t;
        else {
            s = s*iv[t]%mod;
            t = 1;
        }
    }
    writeln(s*fc[n]%mod*iv[c]%mod*iv[n-c]%mod);
