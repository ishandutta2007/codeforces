...
signed main(){fastio
    ...
    auto mst=[](vvl v,ll n,vl_l &e){
        sort(all(v),[](vl a,vl b){return a[2]<b[2];});
        dsu d(n);
        ll sm=0;
        for(auto x:v){
            if(!d.same(x[0],x[1])){
    ...
    if(n>=700){
        set<l_l>edges;
        rep(i,0,m-1){
            edges.insert({A[i][0],A[i][1]});
            edges.insert({A[i][1],A[i][0]});
        }
        set<ll>unused;
        rep(i,0,n-1)unused.insert(i);
        vvl G;
        while(!unused.empty()){
            vl v;
            queue<ll>q;
        ...
        ll gs=G.size();
        rep(i,0,gs-1){
            for(auto x:G[i])num[x]=i;
        }
        vvl es;
        for(auto x:A){
            es.pb({num[x[0]],num[x[1]],x[2]});
        }
        vl_l aaa;
        cout<<mst(es,gs,aaa)<<endl;
        return 0;
    }
    vvl cost(n,vl(n));
    for(auto x:A){
        cost[x[0]][x[1]]=cost[x[1]][x[0]] = x[2];
    }
    vvl aaa;
    rep(i,0,n-1){
        rep(j,i+1,n-1){
    ...
    vl_l reds;
    {
        ...
        rep(i,0,n-1)rep(j,i+1,n-1){
            if(s.find({i,j})==s.end())reds.pb({i,j});
        }
    }
    sort(all(reds),[&](l_l a,l_l b){
        return cost[a.fi][a.se]<cost[b.fi][b.se];
    });
    ll XOR=0;
    for(auto x:A)XOR^=x[2];
    vvl v(n);
    for(auto x:tr){
        v[x.fi].pb(x.se);
        v[x.se].pb(x.fi);
  ...
  //  cerr<<"MST "<<MST<<endl;
    for(auto E:reds){
        if(cost[E.fi][E.se]>=XOR)break;
        vl used(n);
        ll cnt=0;
        auto dfs=[&](ll pos,auto dfs)->void{
            if(pos==E.se){
                if(cnt>=1){
                    cout<<MST+cost[E.fi][E.se]<<endl;
                    exit(0);
                }
                return;
            }
