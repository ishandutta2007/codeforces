...
int main(){
  ...
  while(t--){
    ...
    vector<int>a(n);
    rep(i,0,n)cin>>a[i];
    auto [mxl,mxr]=val_range(a,false);
    vector<vector<int>>idx(sz);
    rep(i,0,n){
      idx[a[i]].PB(i);
    }
    auto [mil,mir]=val_range(a,true);
    rep(i,0,n)if(mil[i]>=0&&a[mil[i]]==a[i])mil[i]=mil[mil[i]];
    rrep(i,0,n)if(mir[i]<n&&a[mir[i]]==a[i])mir[i]=mir[mir[i]];
    ll ret=0;
    rep(i,0,n){
      auto dt=osak.divisor_enum(a[i]);
      for(auto d:dt){
        if(idx[d].empty())continue;
        int r=lower_bound(ALL(idx[d]),i)-idx[d].begin();
        int ri=n;
        if(r<idx[d].size())ri=idx[d][r];
        int l=r-1;
        if(ri!=n&&ri==i)l++;
        int li=-1;
        if(l>=0)li=idx[d][l];
        int ml,mr;
        if(ri!=n&&mil[ri]<=i)ml=mil[ri],mr=mir[ri];
        else if(li!=-1&&mir[li]>=i)ml=mil[li],mr=mir[li];
        else continue;
        //debug(idx[d]);
        //cout<<i spa d spa ml spa mr spa li spa ri<<endl;
        chmax(ml,mxl[i]);
        chmin(mr,mxr[i]);
        chmin(ri,mr);
        chmax(li,ml);
        //cout<<i spa d spa ml spa mr spa li spa ri<<endl;
        ret+=max(0LL,
        max<ll>(0,mr-i)*max<ll>(0,i-ml)-max<ll>(0,ri-i)*max<ll>(0,i-li));
        //cout<<ret<<endl;
      }
    }
    cout<<ret<<endl;
