...
vi query(vpii q) {
	...
	loc({
		vi ans;
		for (auto [x, y] : q) ans.push_back(a[x] == a[y]);
		rans;
	})
	onl({
		printf("Q %d", (int) q.size());
		for (auto [x, y] : q) printf(" %d %d", x, y);
		printf("\n");
		fflush(stdout);
		static char scan[nax];
		scanf("%s", scan);
		vi ans(q.size());
		REP(i, q.size()) ans[i] = scan[i] == '1';
		rans;
	})
}
void answer(vi ans[3]) {
	printf("A");
	REP(i, 3) printf(" %d", (int) ans[i].size());
	printf("\n");
	REP(i, 3) {
	...
	loc({
		REP(i, 3) REP(j, i) if (!ans[i].empty() && !ans[j].empty()) assert(a[ans[i][0]] != a[ans[j][0]]);
		REP(i, 3) for (int x : ans[i]) assert(a[x] == a[ans[i][0]]);
	})
	fflush(stdout);
}
void solve() {
	int n;
	scanf("%d", &n);
	loc(for (int i = 1; i <= n; ++i) scanf("%d", a + i));
	vpii q1, q2;
	for (int i = 1; i < n; ++i) (i % 2 ? q1 : q2).emplace_back(i, i + 1);
	vi a1 = query(q1), a2 = query(q2);
	vi points;
	points.push_back(1);
	for (int i = 1; i < n; ++i) if (!(i % 2 ? a1 : a2)[(i - 1) / 2]) points.push_back(i + 1);
	points.push_back(n + 1);
	debug(imie(points));
	int cou = points.size() - 1;
	vpii r1, r2;
	for (int i = 0; i + 2 < cou; ++i) (i & 2 ? r1 : r2).emplace_back(points[i], points[i + 2]);
	vi b1 = query(r1), b2 = query(r2);
	vi block_ans = {0, 1};
	auto it1 = b1.begin(), it2 = b2.begin();
	for (int i = 0; i + 2 < cou; ++i) {
		int eq = *((i & 2 ? it1 : it2)++);
		int a = block_ans.back(), b = *(block_ans.end() - 2);
		if (eq) block_ans.push_back(b);
		else block_ans.push_back(3 ^ a ^ b);
	}
	vi ans[3];
	for (int i = 0; i < cou; ++i) {
		int l = points[i], r = points[i + 1];
		for (int x = l; x < r; ++x) ans[block_ans[i]].push_back(x);
	}
	answer(ans);
}
int main() {
