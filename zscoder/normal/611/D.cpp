...
int main()
{
	...
	for(int b = 2; b <= n; b++)
	{
		for(int c = b; c <= n; c++)
		{
			...
			{
				a = max(1, 2*b-c);
				pref[b][c] = (pref[b-1][c]%MOD + pref[b-1][b-1]%MOD - pref[a-1][b-1]%MOD)%MOD;
				//cout << b << " " << c << " " << 2*b-c-1 << endl;
				if(2*b-c-1 > 0)
				{
					a = 2*b-c-1;
					next = nxt[a][b];
					...
					if(next < INF)
					{
						if(next+a < b && next+b <= c && digits[next+a] < digits[next+b])
						{
							pref[b][c] += dp(a, b-1);
							pref[b][c] %= MOD;
						}
