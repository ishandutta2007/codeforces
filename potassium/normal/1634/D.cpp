...
int main() {
	CASE(t) {
		...
		const int id = max_element(v.begin(), v.end()) - v.begin();
		...
		const int id2 = max_element(v2.begin(), v2.end()) - v2.begin();
		...
		vector<int> cand;
		if (x < r) cand.emplace_back(id2);
		if (y < r) cand.emplace_back(id);
		if (z < r) cand.emplace_back(0);
		assert(1 <= (int)cand.size() && (int)cand.size() <= 2);
