...
struct stree {
	void draw(int &cur, int l, int r, int x, int y, int what) {
		if (cur == 0) cur = last++;
		if (x < l) x = l;
		if (y > r) y = r;
if (full[cur] && val[cur] > what) return;
		if (l == x && y == r) {
			full[cur] = true;
			val[cur] = what;
		}
...
if (full[cur] && val[cur] > what) return;
			...
			if (y <= m) {
				draw(lp[cur], l, m, x, y, what);
			}
			else if (x > m) {
				draw(rp[cur], m + 1, r, x, y, what);
			}
			else {
				draw(lp[cur], l, m, x, m, what);
				draw(rp[cur], m + 1, r, m + 1, y, what);
			}
		}
	}
	int get_max(int cur, int l, int r, int x) {
		if (cur == 0) return 0;
...
if (full[cur] && val[cur] > what) return;
		...
		int m = (l + r) >> 1;
		if (x <= m) return get_max(lp[cur], l, m, x);
		else return get_max(rp[cur], m + 1, r, x);
	}
};
...
int main()
{
	...
	while (q--) {
		...
		if (c == 'U') {
			...
			s[0].insert(x);
			auto val = trees[0].get_max(roots[0], 1, INF, x);
			cout << y - val << '\n';
			if (y == val) continue;
			trees[1].draw(roots[1], 1, INF, val + 1, y, x);
		}
		else {
			if (s[1].count(y)) {
				cout << "0\n";
			...
			s[1].insert(y);
			auto val = trees[1].get_max(roots[1], 1, INF, y);
			cout << x - val << '\n';
			if (x == val) continue;
			trees[0].draw(roots[0], 1, INF, val + 1, x, y);
		}
	}
