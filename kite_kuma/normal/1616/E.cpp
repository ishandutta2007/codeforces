...
ll ans(int n, string s, string t) {
	const int chsize = 26;
	vector<vector<int>> splace(chsize);
	rep(i, n) { splace[s[i] - 'a'].push_back(i); }
	vector<int> nxt(chsize);
	ll ret = INF;
	binary_indexed_tree<ll> bit(n);
	rep(i, n) bit.add(i, 1LL);
	ll ans = 0LL;
	debug(splace);

	rep(change, n) {
		const int t_nxt = t[change] - 'a';

		rep(smallch, t_nxt) {
			const auto &arr = splace[smallch];
			auto it = lower_bound(arr.begin() + nxt[smallch], arr.end(), 0);
			if(change == 5 && smallch == 5){
				debug(nxt);
				debug(arr);
			}
			if(it == arr.end()) continue;
			// chmin(ret, it - arr.begin() - change);
			int place = *it;
			chmin(ret, ans + bit.sum0(place));
		}

		const auto &arr = splace[t_nxt];
		debug(arr);
		if(nxt[t_nxt] == int(arr.size())) break;
		int place = arr[nxt[t_nxt]++];
		bit.add(place, -1LL);
		ans += bit.sum0(place);
