...
int main() {
    STR(s);
    if(s == string(si(s), '0')) OUT(si(s)), exit(0);
    int m = 0;
    int c = 0;
    vi idx{0};
    vi v;
    ...
    mint ans;
    v.eb(c);
    atcoder::segtree<int, f, e> seg(v);
    rep(i, si(s)) {
        int k = ub(idx, i) - 1;
        if(i >= idx[1] and i - idx[k] <= si(s) - idx.back()) ans += dp[i];
        dp[i + 1] += dp[i];
        if(s[i] == '1') {
            if(i >= idx[1]) {
                int r = seg.max_right(k, [&](int x) { return x <= v[k]; });
                // dump(i, v[k], v, r, idx);
                if(r < si(v)) { dp[idx[r] + v[k] + 1] += dp[i]; }
            }
        } else {
            if(idx.back() > i) dp[*upper_bound(all(idx), i)] += dp[i];
        }
        cout.flush();
    }
    // OUT(dp);
