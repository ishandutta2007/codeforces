...
void tc() {
	...
	int nOdds = odds.size(), nEvens = evens.size();
	if (((nOdds - reqOdds) & 1) || nOdds < reqOdds || nEvens + (nOdds - reqOdds) / 2 < reqEvens) printf("NO\n");
	else {
		printf("YES\n");
		if (reqEvens) {
			repeat (reqOdds) {
		...
		printf("%u", odds.size() + evens.size());
		for (int x : evens) printf(" %d", x);
		for (int x : odds) printf(" %d", x);
		printf("\n");
	}
}
