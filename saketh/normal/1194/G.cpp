...
int main() {
    ...
    FOR (x, 1, 10) {
        FOR (y, 1, 10) {
            ...
            vb ttop(10), tbot(10);
            FOR (t, 1, 10) FOR (b, 1, 10)
                if (t * y == b * x)
                    ttop[t] = tbot[b] = true;

            enum cmp { LEQ, GT };
            using DP = map<tuple<int, int, int, int, bool>, mn>;
            DP dp; dp[mt(0, 0, 0, 0, LEQ)] = 1;

            F0R (i, sz(N)) {
                DP ndp;

                trav (entry, dp) {
                    const auto &[tuse, buse, tcarry, bcarry, rel] = entry.f;
                    F0R (nd, 10) {
                        int tval = tcarry + nd * x; int tlead = tval % 10, ntcarry = tval / 10;
                        int bval = bcarry + nd * y; int blead = bval % 10, nbcarry = bval / 10;

                        int ntuse = tuse | (ttop[tlead] ? 1 << tlead : 0);
                        int nbuse = buse | (tbot[blead] ? 1 << blead : 0);

                        int nlead = x > y ? tlead : blead;
                        int nrel = (nlead == N[i] - '0') ? rel : (nlead < N[i] - '0') ? LEQ : GT;

                        ndp[mt(ntuse, nbuse, ntcarry, nbcarry, nrel)] += entry.s;
                    }
                }
            ...
            trav (entry, dp) {
                const auto &[tuse, buse, tcarry, bcarry, rel] = entry.f;
                if (tcarry != 0 || bcarry != 0 || rel != LEQ) continue;
                bool ok = false;
                FOR (t, 1, 10) FOR (b, 1, 10) if (t * y == b * x)
                    if ((tuse & (1 << t)) && (buse & (1 << b))) ok = true;
                if (ok) ans += entry.s;
            }
        }
    }
