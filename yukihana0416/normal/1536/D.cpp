...
void ins(node *&o, int p, int l, int r, int id)
{
    if(o == &nil)
    {
        o = new node;
    ...
    if(l == r) return ;
    int mid = MID(l, r);
    if(p <= mid) ins(o->lc, p, lson);
    else ins(o->rc, p, rson);
    o->cnt = o->lc->cnt + o->rc->cnt;
}

int query(node *o, int L, int R, int l, int r, int id)
{
    if(L > R) return 0;
    if(o == &nil) return 0;

    if(L <= l && r <= R) return o->cnt;

    int mid = MID(l, r);
    int lv = 0, rv = 0;
    if(L <= mid) lv = query(o->lc, L, R, lson);
    if(mid < R) rv = query(o->rc, L, R, rson);
    return lv + rv;
}
...
void work()
{
    ...
    for(int i = 1; i < n; i++)
    {
        ...
        if(query(rt, l + 1, r - 1, -inf, inf, 1) > 0)
        {
            cout <<"NO" <<endl;
            return ;
        }
        ins(rt, a[i], -inf, inf, 1);
    }
