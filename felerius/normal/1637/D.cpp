// Yet Another Minimization Problem (https://codeforces.com/contest/1637/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        rep(i, n) {
            set<int> next_diff;
            trav(d, diff)
                next_diff.emplace(d + a[i] - b[i]), next_diff.emplace(d + b[i] - a[i]);
            diff = next_diff;
        }
        ...
        int x = accumulate(all(a), 0) + accumulate(all(b), 0);
        int as = (x + d) / 2, bs = x - as;
        ll cost = ll(as) * as + ll(bs) * bs;
        rep(i, n)
            cost += (n - 2) * (ll(a[i]) * a[i] + ll(b[i]) * b[i]);
        println(cost);
    }
}