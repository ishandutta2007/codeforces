...
bool solve(string a, string b, int n) {
    for (char c = '0', ic = '1'; c <= '1'; ++c, --ic) {
        vector<int> ca, cb;
        for (int i = 0; i < sz(a); ++i)
            if (a[i] == c) ca.pb(i);
        for (int i = 0; i < sz(b); ++i)
            if (b[i] == c) cb.pb(i);
        if (min(sz(ca), sz(cb)) >= n) {
            ca.insert(ca.begin(), -1);
            cb.insert(cb.begin(), -1);
            if (sz(ca) > sz(cb)) ca.resize(sz(cb));
            if (sz(cb) > sz(ca)) cb.resize(sz(ca));
            dbg(ca, cb);
            string ans;
            dbg(a, b);
            dbg(c);
            for (int i = 1; i < sz(ca); ++i) {
                int mx = max(ca[i] - ca[i - 1], cb[i] - cb[i - 1]) - 1;
                dbg(mx);
                ans += string(mx, ic);
                ans += c;
            }
            for (int i = ca.back() + 1; i < sz(a); ++i) ans += a[i];
            for (int i = cb.back() + 1; i < sz(b); ++i) ans += b[i];
            if (sz(ans) <= 3 * n) {
                println(ans);
                return true;
            }
...
void solve() {
    ...
    string a, b, c;
    read(a, b, c);
    if (solve(a, b, n)) return;
    if (solve(c, b, n)) return;
    if (solve(a, c, n)) return;
    assert(false);
}
