...
int main() {
    TEST {
        INT(n, k);
        auto f = [&](int x, int y) { return x * k + y; };
        map<pair<pii, pii>, int> mp;
        rep(i, n) {
            INT(a, b, c, d);
            a--, b--, c--, d--;
            rep(2) {
                a = k - 1 - a, b = k - 1 - b, c = k - 1 - c, d = k - 1 - d;
                pii p1(a, b), p2(c, d);
                if(p1 > p2) swap(p1, p2);
                if(a != c) {
                    mp[pair(pii(p1.first + 1, p1.second), pii(p1.first + 1, p1.second + 1))]++;
                } else {
                    mp[pair(pii(p1.first, p1.second + 1), pii(p1.first + 1, p1.second + 1))]++;
                }
            }
        }

        Wgraph<int> g((k + 1) * (k + 1));
        auto ff = [&](int x, int y) { return x * (k + 1) + y; };

        rep(i, k + 1) rep(j, k) add(g, ff(i, j), ff(i, j + 1), mp[pair(pii(i, j), pii(i, j + 1))]);
        rep(i, k) rep(j, k + 1) add(g, ff(i, j), ff(i + 1, j), mp[pair(pii(i, j), pii(i + 1, j))]);

        OUT(n - dijkstra(g, 0)[ff(k / 2, k / 2)]);
    }
}