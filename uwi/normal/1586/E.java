...
public class E {
	...
	void solve()
	{
		...
		int[] par = pars[0];
		LCAFast2 lcf = LCAFast2.build(g, 0);
		...
		for(int Q = ni();Q > 0;Q--){
			int x = ni()-1, y = ni()-1;
			int lca = lcf.lca(x, y);
			Deque<Integer> routef = new ArrayDeque<>();
			Deque<Integer> routeb = new ArrayDeque<>();
			for(int u = x;u != lca;u = par[u]){
				routef.addLast(u);
			}
			routef.addLast(lca);

			for(int u = y;u != lca;u = par[u]){
				routeb.addFirst(u);
			}
			sb.append(routef.size() + routeb.size() + "\n");
			for(int u : routef){
