...
int calc(int i, int last) {
    if (mp[i].second == last) return mp[i].first;

    ll num = ceil(a[i], last);
    if (i == 0) {
        mp[i] = pii(num - 1, last);
        return mp[i].first;
    }

    ll add = calc(i - 1, a[i] / num);
    mp[i] = pii((add + (num - 1) * (i + 1)) % MOD, last);
    return mp[i].first;
}
