...
int main(void)
{
	...
	rep(i, 1, n){
		...
		ll m = sz(vec[i]);
		rep(j, 0, m-1){
			ll l = inf, r = -inf, l2 = inf, r2 = -inf;
			rep(k, 0, m-1){
				ll v = vec[i][(j-k+m)%m];
				if(v <= n) chmin(l, v), chmax(r, v);
				else chmin(l2, v), chmax(r2, v);
				
				if(k % 2){
					ll nv = vec[i][(j-k-1+m)%m];
					ans += get(l, r, l2, r2);
					
					ll L = l, R = r, L2 = l2, R2 = r2;
					if(nv <= n) chmin(L, nv), chmax(R, nv);
					else chmin(L2, nv), chmax(R2, nv);
					
					ans -= get(L, R, L2, R2);
					//outl(l, r, l2, r2, L, R, L2, R2);
				}
				if(j % 2 == 0 && k == m-1) ans += get(l, r, l2, r2);//, outl(l, r, l2, r2);
			}
		}
	}
	outl(ans);
