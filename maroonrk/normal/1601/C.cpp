...
void slv(){
	int n,m;sc.read(n,m);
	vi tmp(n+1);iota(all(tmp),0);
	seglazy<N> seg(tmp);
	auto below=[&](int i){
		seg.ch(0,i+1,&N::add,1);
	};
	auto above=[&](int i){
		seg.ch(i+1,n+1,&N::add,-1);
	};
	vc<pi> qs;qs.reserve(n+m);
	vi arw(n);
	rep(i,n){
		int a;sc.read(a);
		qs.eb(a,i*2);
