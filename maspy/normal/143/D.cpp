...
void solve() {
  auto f = [&](ll H, ll W) -> ll {
    ...
    FOR(x, H) FOR(y, W) {
      ll v = W * x + y;
      if ((x + y) % 2 == 0) G.add_edge(source, v, 1);
      if ((x + y) % 2 == 1) G.add_edge(v, sink, 1);
      if ((x + y) % 2 == 1) continue;
      FOR(d, 8) {
  ...
  auto g = [&](ll H, ll W) -> ll {
    ...
    if (H == 2) {
      ll k = W / 4;
      if (W % 4 == 0) return 4 * k;
      if (W % 4 == 1) return 4 * k + 2;
      if (W % 4 == 2) return 4 * k + 4;
      if (W % 4 == 3) return 4 * k + 4;
    }
    if (H * W % 2 == 0) return H * W / 2;
    return (H * W + 1) / 2;
  };

  /*
  FOR3(x, 1, 10) FOR3(y, 1, 10) {
    if (f(x, y) != g(x, y)) print(x, y, f(x, y), g(x, y));
  }
  */
