...
void solve() {
    autoint n, k;
    vector<int> ans;
    for (int i = 0; i + 1 < k; ++i) ans.push_back(1);
    ans.push_back(n - accumulate(ans.begin(), ans.end(), 0ll));
    if (ans.back() <= 0) answer("NO");
    if (ans.back() % 2 == 0) {
        for (int i = 0; i + 1 < k; ++i) ans[i] = 2;
        ans.back() = (n - accumulate(ans.begin(), ans.end(), 0ll) + ans.back());
    } else answer("YES", ans);
    if (ans.back() <= 0) answer("NO");
    if (ans.back() % 2 == 1) answer("NO");
    answer("YES", ans);
}