...
int main() {
    ...
    vector<vector<int>> ls(MAX), rs(MAX);
    for (int i = 1; i < n; i++) {
    ...
    m2[1][1]++;
    m2[1][0]++;
    vector<SM> init(n - 1, m1);
    atcoder::segtree<SM, op, e> seg(init);
    mint ans = 0;

    for (int i = 0; i < MAX; i++) {
        for (int& idx : ls[i]) seg.set(idx - 1, m2);
        auto res = seg.all_prod();
        ans += res[l[0] <= i and i <= r[0]][1];
        for (int& idx : rs[i]) seg.set(idx - 1, m1);
    }
