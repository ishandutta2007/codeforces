...
void solve() {
    ...
    for (int i = 1, t, p; i <= n; i++) {
        cin >> t >> p;
        g[t].emplace_back(p, i);
    }
    sort(g[2].begin(), g[2].end());
    g[2].emplace_back(0, 0);
    reverse(g[2].begin(), g[2].end());
    sort(g[1].begin(), g[1].end());
    g[1].emplace_back(0, 0);
    reverse(g[1].begin(), g[1].end());
    pair<int, int> best = {-1, 0};
    for (int j = 1; j < g[1].size(); j++) g[1][j].first += g[1][j - 1].first;
    for (int j = 1; j < g[2].size(); j++) g[2][j].first += g[2][j - 1].first;
    for (int i = 0; i < g[2].size(); i++) {
        if (i * 2 > v) break;
        int score = g[2][i].first + g[1][min(int(g[1].size()) - 1, v - i * 2)].first;
        best = max(best, {score, i});
    }
    cout << best.first << '\n';
    for (int i = 1; i <= best.second; i++) cout << g[2][i].second << ' ';
    for (int i = 1; i <= min(int(g[1].size()) - 1, v - best.second * 2); i++) cout << g[1][i].second << ' ';
}
