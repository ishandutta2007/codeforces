...
void solve(){
	mint prob_invers = 1 / mint(10000);

	int n;
	cin >> n;
	vector<vector<mint>> a(n, vector<mint>(n)), inv_a(n, vector<mint>(n));

	cin >> a;

	vector<mint> col_product(n, -1), row_product(n, -1);
	...
	for(int mask = 0; mask < (1 << n); ++mask){
		...
		mint tot_row = 1;
		vector<mint> col_values = col_product;
		
		for(int i = 0; i < n; ++i){
				
			if (mask & (1 << i)){
				tot_row *= row_product[i];
				for(int j = 0; j < n; ++j){
					col_values[j] *= inv_a[i][j];
				}
			}
		}
