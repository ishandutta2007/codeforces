...
vvi solve(vi C){
  ...
  while(1) {
    int a = -1, b = -1;
    for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(i < j && C[i] == C[j]+1) {
      a = i;
      b = j;
    }
    if(a == -1) break;
    ...
    int a2 = a;
    while(C[a2+1] == C[a2]+1) a2++;
    int b1 = b;
    while(C[b1-1] == C[b1]-1) b1--;
    vi Q;
    auto add = [&](int x) { if(x) Q.push_back(x); };
    add(a);
    add(a2-a+1);
    add(b-(a2+1)+1);
    add((n-1)-(b+1)+1);
    ...
    for(auto x : Q) {
      curr -= x;
      for(li i = 0; i < (li)(x); ++i) D[curr+i] = C[curl+i];
      curl += x;
    }
    C = D;
  }
...
int main(){
  ...
  int n; cin>>n;
  vi C(n); for(li i = 0; i < (li)(n); ++i) { cin>>C[i]; --C[i]; }
