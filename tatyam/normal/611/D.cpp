...
int main(){
    LL(n);
    STR(a);
    vector<vector<int>> c(n, vector<int>(n + 1, INF)); // first index of k (k >= j && a[k - i] != a[k])
    for(int i = 1; i < n; i++) for(int j = n - 1; j >= i; j--){
        if(a[j - i] == a[j]) c[i][j] = c[i][j + 1];
        else c[i][j] = j;
    }
    auto comp = [&a, &c](int at, int length){ // return a[at - length : at] < a[at : at + length]
        if(c[length][at] >= at + length) return false;
        else return a[c[length][at] - length] < a[c[length][at]];
    };
    vv(Modint,dp,n+1,n+1);
    rep(n)dp[0][1]=1;
    rep(l,n)rep(r,l+1,n){
        dp[l][r+1]+=dp[l][r];
        const ll d=r-l;
        if(a[r]=='0'||r+d>n)continue;
        if(comp(r,d))dp[r][r+d]+=dp[l][r];
        elif(r+d+1<=n)dp[r][r+d+1]+=dp[l][r];
    }
    Modint ans=0;
    rep(n)ans+=dp[i][n];
    out(ans);
