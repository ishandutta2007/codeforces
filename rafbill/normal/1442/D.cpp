...
int main(){
  ...
  li n,k; cin>>n>>k;
  for(li i = 0; i < (li)(n); ++i) {
    cin>>sz[i]; A[i].resize(sz[i]);
    for(li j = 0; j < (li)(sz[i]); ++j) cin>>A[i][j];
    S[i] = accumulate(begin(A[i]), end(A[i]), 0ll);
  }

  li ans = 0;
  auto solve = letrec([&](auto solve, int l, int r, vi ar) {
    if(l == r) {
      li s = 0;
      smax(ans, ar[k]);
      for(li i = 0; i < (li)(sz[l]); ++i) if(k-1-i >= 0) {
        s += A[l][i];
        smax(ans, ar[k-1-i] + s);
      }
      return;
    ...
    li m = (l+r)/2;
    vi L = ar, R = ar;
    for(li i = (l); i <= (li)(m); ++i) for(li j = (k-sz[i]); j >= (li)(0); --j) smax(L[j+sz[i]], L[j]+S[i]);
    for(li i = (m+1); i <= (li)(r); ++i) for(li j = (k-sz[i]); j >= (li)(0); --j) smax(R[j+sz[i]], R[j]+S[i]);
    solve(l, m, R); solve(m+1, r, L);
  });
  solve(0, n-1, vi(k+1, 0));
