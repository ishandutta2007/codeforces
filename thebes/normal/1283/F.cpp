...
const int MN = 2e5+5;
int n, i, x, y, arr[MN], cnt[MN], w[MN];
struct pq{bool operator()(const int&i,const int&j){return w[i]>w[j];}};
priority_queue<int,vi,pq> q;
vector<pii> ans;

int main(){
    scanf("%d",&n);
    fox(i,1,n-1) scanf("%d",&arr[i]),cnt[arr[i]]++;
    fox(i,1,n){
        w[i]=i;
        if(!cnt[i]) q.push(i);
    }
