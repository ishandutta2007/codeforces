...
int main() {
    ...
    int n, k, l;
    cin >> n >> k >> l;
    factorials::init(2 * n + k);

    vector<mint> match_dp(2 * n + 1);
    match_dp[0] = 1;
    for (int i = 2; i < len(match_dp); i += 2)
        match_dp[i] = mint(i - 1) * match_dp[i - 2];

    mint answer = 0;
    for (int before = 0; before <= 2 * n; before++) {
        int after = 2 * n - before;

        mint total = 0;
        mint good = 0;

        for (int between = 0; between <= min(before, after); between++) {
            mint current = choose(before, between) * choose(after, between)
                * match_dp[before - between] * match_dp[after - between] * fact[between];
