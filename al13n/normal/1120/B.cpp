...
int main(int argc, char **argv){
  ...
  auto can=[&](int i){
             ...
             if(!C[i]) return false;
             if(C[i]<0 && (a[i]=='0' || (a[i]=='1' && !i))) return false;
             if(C[i]<0 && (a[i+1]=='0' || (a[i+1]=='1' && !i))) return false;
             if(C[i]>0 && a[i]=='9') return false;
             if(C[i]>0 && a[i+1]=='9') return false;
             return true;
           };
  vector<int> Q;
  LL sum=0;
  forn(i,n){
    C[i]=b[i]-a[i]-(i?C[i-1]:0);
    if(can(i)) Q.pb(i);
    sum+=abs(C[i]);
  }
  if(C[n-1]){
  ...
  forn(qq,min(sum,100000ll)){
    ...
    if(!c1 && can(p-1)) Q.pb(p-1);
    if(!c2 && can(p+1)) Q.pb(p+1);
    if(can(p)) Q.pb(p);
  }
