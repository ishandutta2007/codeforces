...
signed main()
{
	...
	inv[0]=1; for(int i=1;i<=n;i++) inv[i]=mul(inv[i-1],__inv[i]);
	...
	for(int i=1;i<=k;i++)
	{
		...
		vector<int> f(2*c2+1),g(c1+1);
		for(int i=0;i<=2*c2;i++) f[i]=C(2*c2,i);
		for(int i=0,t=1;i<=c1;i++,t=add(t,t)) g[i]=mul(C(c1,i),t);
		f=f*g;
		for(int i=0;i<(int)f.size();i++) ans[i+x+1]=add(ans[i+x+1],f[i]);
	}
	int Q=read();
	while(Q--) printf("%d\n",ans[read()/2]);
#ifdef __LOCAL__
