        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            for (int i = 0; i < h; i++)
            {
                ...
                for (int j = 0; j < w; j++)
                {
                    if (s[i][j] != '.') {
                        int k = s[i][j] - 'a';
                        c = Math.Max(c, k + 1);
                        mai[k] = Math.Max(mai[k], i);
                        maj[k] = Math.Max(maj[k], j);
                        mii[k] = Math.Min(mii[k], i);
                        mij[k] = Math.Min(mij[k], j);
                    }
                }
            }
            for (int k = 0; k < 26; k++)
            {
                if (mai[k] > mii[k] && maj[k] > mij[k]) {
                    Prt("NO");
                    goto A;
                }
                for (int i = mii[k]; i <= mai[k]; i++)
                {
                    for (int j = mij[k]; j <= maj[k]; j++)
                    {
                        if (s[i][j] == '.' || s[i][j] < 'a' + k) {
                            Prt("NO");
                            goto A;
            ...
            for (int i = c - 1; i >= 0 ; i--)
            {
                if (mii[i] == M) {
                    ...
                    ans[i] = new int[] { mii[i] + 1, mij[i] + 1, mai[i] + 1, maj[i] + 1 };
                }
            }
