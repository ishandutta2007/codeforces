...
public class G2 {
	...
	void solve()
	{
		...
		int[] stack = new int[2*n+3];
		int[] has = new int[2*n+3];
		for(int z = 0;z < Q;z++) {
			int K = ni(), L = ni();
			int[] qa = na(K);
			int[] qb = na(L);
			...
			int[][] qs = new int[K+L][];
			for(int i = 0;i < K;i++) {
				qs[i] = new int[] {isa[qa[i]-1], 0};
			}
			for(int i = 0;i < L;i++) {
				qs[i+K] = new int[] {isa[qb[i]-1], 1};
			}
			...
			long ans = 0;
			{
				...
				for(int[] q : qs) {
					if(prev < q[0]) {
						int val = rmq(st, prev+1, q[0]+1);
						int h = 0;
						while(sp > 0 && stack[sp-1] >= val) {
							h += has[sp-1];
							sum -= (long)stack[sp-1] * has[sp-1];
							sp--;
						}
