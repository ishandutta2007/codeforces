...
void plus(int i, int x, int y);

int n;
int dp[N][C];
std::string s;

inline int &plus(int &x, int y) { return x = (x + y) % mod; }
inline void plus(int i, int x, int y) { plus(dp[i][x], dp[i - 1][y]); }
...
int main() {
    ...
    if (s[0] == '?') {
        ...
        dp[0][(s[0] == '0') + 2 * (s[0] == '1')] = 1;
    }
    for (int i = 1; i < n; ++i) {
        if (s[i] == '*' || s[i] == '?') plus(i, 0, 2), plus(i, 0, 0);
        if (s[i] == '0' || s[i] == '?') plus(i, 1, 1);
        if (s[i] == '1' || s[i] == '?') plus(i, 1, 0), plus(i, 2, 1);
        if (s[i] == '2' || s[i] == '?') plus(i, 2, 0);
    }
