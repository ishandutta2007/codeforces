...
int main(){
  ...
  rep(i,N){
    cin>>j;
    if(j&1)o.emplace_back(i,j);
    else e.emplace_back(i,j);
  }
  sort(all(o),[](auto l,auto r){return l.second<r.second;});
  sort(all(e),[](auto l,auto r){return l.second<r.second;});
  vec lr(N);
  rep(i,N){
    cin>>s;
    lr[i]=s[0]=='R';
  ...
  pvec st;
  for(auto x:o){
    if(st.empty()||lr[x.first])st.emplace_back(x);
    else{
      if(lr[st.back().first]){
        //RL
        ans[st.back().first]=ans[x.first]=(x.second-st.back().second)/2;
      }else{
        //LL
        ans[st.back().first]=ans[x.first]=(x.second+st.back().second)/2;
      }
  ...
  while(st.size()>1){
    ...
    if(lr[a.first]){
      //RR
      ans[a.first]=ans[b.first]=(K*2-a.second-b.second)/2;
    }else{
      //LR
      ans[a.first]=ans[b.first]=(K*2-b.second+a.second)/2;
    }
  }
  st.clear();
  for(auto x:e){
