...
void dfs(int v, vvi &g, vi &a, vi &b, vi &tour, vi &vis, vi &pnt)
{
...
void solve()
{
    ...
    vector<int> kek;
    forn(i, n) kek.push_back(a[i]);
    forn(i, n) kek.push_back(b[i]);
    uniq(kek);
    map<int, int> mm;
    forn(i, kek.size())
        mm[kek[i]] = i;
    ...
    forn(i, n) g[a[i]].push_back(i), g[b[i]].push_back(i);
    vector<int> bad;
    forn(i, m)
    {
        if (g[i].size() % 2 == 1)
    ...
    vector<int> tour, vis(a.size()), pnt(m);
    dfs(0, g, a, b, tour, vis, pnt);
    if (bad.size())
    {
        vector<int> t2;
        int id = 0;
        ...
        {
            if (tour[i] == bad[0] && tour[(i + 1) % tour.size()] == bad[1])
                id = i;
            if (tour[i] == bad[1] && tour[(i + 1) % tour.size()] == bad[0])
                id = i;
        }
        for (int i = id + 1; i < tour.size(); i++)
        {
            t2.push_back(tour[i]);
        }
        for (int i = 0; i <= id; i++)
