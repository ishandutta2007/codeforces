...
struct FT {
  ...
  void update(int i, int key, int val) {
    for (; i < N; i |= i + 1) {
//      if (!ft[i].empty() && ft[i].size() == ft[i].capacity()) {
//        reduce(i);
//        ft[i].reserve(max(ft[i].capacity(), ft[i].size() * 4));
//      }
      ft[i].push_back({key, val});
    }
  }
  ...
  void acc(int i) {
    int64_t p = 0;
    sort(ft[i].begin(), ft[i].end());
    for (int j = ft[i].size(); j--; ) {
      p = ft[i][j][1] += p;
    }
    ft[i].erase(unique(ft[i].begin(), ft[i].end(), CompareField<2, 1>{}), ft[i].end());
    ft[i].shrink_to_fit();
  }

  int64_t query(int i, int l) {
    int64_t res{};
    for (; i; i &= i - 1) {
      if (auto it = lower_bound(ft[i - 1].begin(), ft[i - 1].end(), array{l, -1}); it != ft[i - 1].end()) {
        res += it[0][1];
      }
    }
    return res;
...
int main() {
  ...
  vector<int> divi, rdivi;
  for (int k = 1; k < N; ++k) {
    divi.clear();
    rdivi.clear();
    for (int d = 1; d * d <= k && d < k; ++d) if (k % d == 0) {
      divi.push_back(d);
      if (d * d != k && d != 1) {
        rdivi.push_back(k / d);
      }
    }
    divi.insert(divi.end(), rdivi.rbegin(), rdivi.rend());
    for (int i = 0; i < divi.size(); ++i) {
      ft.update(k, divi[i], divi.size() - i - 1);
    }
    if (k % 3 == 0) {
      if (k % 2 == 0) {
        ft.update(k, k / 2, 1);
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    int l = $, r = $ + 1;
    int64_t d = r - l;
    print(d * (d - 1) * (d - 2) / 6 - ft.query(r, l));
  }
}