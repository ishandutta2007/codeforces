...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 2 "/home/maspy/compro/library/mod/primitive_root.hpp"
...
#line 5 "/home/maspy/compro/library/mod/primitive_root.hpp"
// int
...
#line 2 "/home/maspy/compro/library/other/discrete_log.hpp"
// log_a b 
//  H : X -> long long 
// [lb, ub) 
//  -1
...
#line 2 "/home/maspy/compro/library/seq/coef_of_rational_fps.hpp"
...
using mint1 = modint107;
using mint = modint<1'000'000'006>;

void solve() {
  LL(N, f1, f2, f3, c);
  const int mod = mint1::get_mod();
  ll g = primitive_root(mod);
  auto ord = [&](ll x) -> mint {
    auto H = [&](mint1 x) -> ll { return x.val; };
    return discrete_log<Group_Mul<mint1>>(mint1(g), mint1(x), H, 0, mod);
  };
  mint a1 = ord(f1);
  mint a2 = ord(f2);
  mint a3 = ord(f3);
  mint cf = ord(c);
  mint a4 = mint(2) * cf + a3 + a2 + a1;
  mint a5 = mint(4) * cf + a4 + a3 + a2;
  vc<mint> A = {a1, a2, a3, a4, a5};
  vc<mint> den = {1, -1, -1, -1};
  FOR(2) {
    vc<mint> f = {1, -1};
    den = convolution(den, f);
  }
  vc<mint> num = convolution(den, A);
  num.resize(5);
  mint a = coef_of_rational_fps<mint>(num, den, N - 1);
  mint1 ANS = mint1(g).pow(a.val);
  print(ANS);
}
