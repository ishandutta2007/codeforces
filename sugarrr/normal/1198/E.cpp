...
signed main(){fastio
    ...
    ERASE(cx);ERASE(cy);
    for(auto &a:x)for(auto &b:a)b=POSL(cx,b);
    for(auto &a:y)for(auto &b:a)b=POSL(cy,b);
    ...
    mf_graph<ll>g(sx+sy+2);
    ll S=sx+sy;
    ll T=S+1;
    rep(i,0,sx-2){
        g.add_edge(S,i,cx[i+1]-cx[i]);
    }
    rep(i,0,sy-2){
        g.add_edge(i+sx,T,cy[i+1]-cy[i]);
    }
    rep(i,0,sx-2){
        rep(j,0,sy-2){
            if(a[i][j]){
                g.add_edge(i,j+sx,(1e9)+5);
            }
        }
    }
