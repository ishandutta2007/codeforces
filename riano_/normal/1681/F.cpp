...
vector<ll> d_anc(5e5+1,0);
vector<ll> ss(5e5+1,0);
vector<ll> root_ss(5e5+1,0);
vector<ll> dir(5e5+1,1);
vector<ll> col(5e5+1,0);
ll ans = 0;
//tree
    ...
    void dfs_(long long s,long long i,bool dp_){
        ...
        if(i==0){
            if(par[s]!=-1){
                ...
                ans += subtree_size[s]*(N-1-subtree_size[s]);
            }
            if(d_anc[s]!=1){
                ss[d_anc[s]] += subtree_size[s];
            }
            else{
                root_ss[col[s]] += subtree_size[s];
            }
        }
        if(i==1){
            if(d_anc[s]!=1){
                ans -= (ss[d_anc[s]]-subtree_size[s]+N-1-subtree_size[d_anc[s]])*subtree_size[s];
                ans -= (N-1-subtree_size[s])*ss[s];
                ans += (ss[d_anc[s]]-subtree_size[s]+N-1-subtree_size[d_anc[s]])*ss[s];
            }
            else{
                ans -= (root_ss[col[s]]-subtree_size[s])*subtree_size[s];
                ans -= (N-1-subtree_size[s])*ss[s];
                ans += (root_ss[col[s]]-subtree_size[s])*ss[s];
            }
        }
        len--;
        dir[col[s]] = d_anc[s];
...
int main(){
    ...
    tr.dfs(1,false,2e9,0);
    //cout << ans << endl;
    tr.dfs(1,false,2e9,1);
    cout << ans << endl;
}