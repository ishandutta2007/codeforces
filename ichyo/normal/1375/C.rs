...
use text_scanner::{fscan, fscan_iter, scan_iter, scanln, scanln_iter};
use utils::adj4_iter;
...
fn run() {
    ...
    for _ in 0..t {
        let n: usize = fscan(&mut stdin).unwrap();
        let a: Vec<usize> = fscan_iter(&mut stdin).take(n).map(|x| x.unwrap()).collect();
        if a[0] < a[n - 1] {
            println!("YES");
        } else {
            println!("NO");
