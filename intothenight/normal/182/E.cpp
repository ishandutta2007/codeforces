...
int main(){
	...
	vector<vector<vector<Zp>>> dp(l + 1, vector<vector<Zp>>(n, vector<Zp>(2)));
	// width
	for(auto i = 1; i <= l; ++ i){
		for(auto p = 0; p < n; ++ p){
			for(auto pp = 0; pp < 2; ++ pp){
				if(i >= a[p][!pp] && (!pp || a[p][pp] != a[p][!pp])){
					if(i == a[p][!pp]){
						++ dp[i][p][pp];
					}
					else{
						for(auto q = 0; q < n; ++ q){
							if(p != q){
								for(auto qq = 0; qq < 2; ++ qq){
									if(a[p][!pp] == a[q][qq]){
										dp[i][p][pp] += dp[i - a[p][!pp]][q][qq];
									}
								}
							}
						}
