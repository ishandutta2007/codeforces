...
ll size(ll a){
    ...
    while(l<=a){
        if(r<=a){
            ans+=to_string(l).size()*(a-l+1+a-(r-1)+1)*(r-l)/2;
        }
        else{
            ans+=to_string(l).size()*(a-l+1+1)*(a+1-l)/2;
        }
        l*=10;
        r*=10;
    }
...
char solve(ll a){
    ll l=1,r=10;
    while(1){
        a-=to_string(l).size()*(r-l);
        if(a<0){
            a+=to_string(l).size()*(r-l);
            l+=a/to_string(l).size();
            a%=to_string(l).size();
            return to_string(l)[a];
        }
        l*=10;
        r*=10;
    }
...
signed main(){
    ...
    rep(_,q){
        LL(n);
        ll ok=0,ng=2000000000,cen;
        while(abs(ok-ng)>1){
            cen=(ok+ng)/2;
            if(size(cen)<n)ok=cen;
            else ng=cen;
        }
        n-=size(ok);
        out(solve(n-1));
