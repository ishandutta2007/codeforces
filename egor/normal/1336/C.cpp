...
public:
    void solve() {
        ...
        int n = s.size();
        int m = t.size();
        arr<modint> curSubstr(m, 1);
        arr<modint> suf(m + 1, 0);
        suf[m] = 1;
        arr<modint> nextSubstr(m);
        modint answer = 0;
        for (int i : range(n)) {
            fill(all(nextSubstr), 0);
            for (int j : range(m - i)) {
                if (j > 0 && s[i] == t[j - 1]) {
                    nextSubstr[j - 1] += curSubstr[j];
                }
                if (s[i] == t[j + i]) {
                    nextSubstr[j] += curSubstr[j];
                }
            }
            ...
            suf[m] *= 2;
            if (i < m) {
                suf[m - i - 1] += nextSubstr[m - i - 1];
                nextSubstr[m - i - 1] = 0;
            }
            swap(curSubstr, nextSubstr);
            answer += suf[0];
        }
