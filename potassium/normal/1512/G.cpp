...
int main() {
	vector<int> ans(MAXN + 1, INT_MAX);
	vector<int> acc(MAXN + 1, 1);
	vector<ll> d(MAXN + 1, 1);

	ans[1] = 1;
	for (int i = 2; i <= MAXN; i++) {
		acc[i] = sieve.mpf[i];
		int last = i / sieve.mpf[i];
		if (sieve.mpf[last] == sieve.mpf[i])
			acc[i] *= acc[last];

		d[i] = d[last] * (sieve.mpf[i] - 1) / (acc[i] - 1) * (1LL * acc[i] * sieve.mpf[i] - 1) / (sieve.mpf[i] - 1);
		if (d[i] <= MAXN)
			chkmin(ans[d[i]], i);
	}
