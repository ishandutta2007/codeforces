...
{
    vector<int> ans(k);
    for (int i = 0; i<k; i++) ans[i] = (6 - a[i] - b[i])%3;
    return ans;
}
...
int main()
{
    ...
    cin>>n>>k;
    vector<vector<int>> guys(n, vector<int>(k));
    for (int i = 0; i<n; i++)
        for (int j = 0; j<k; j++) cin>>guys[i][j];

    set<vector<int>> setik(guys.begin(), guys.end());

    int ans = 0;
    for (auto it: guys)
    {
        int cnt = 0;
        for (int i = 0; i<n; i++) if (guys[i]!=it)
        {
            if (setik.count(combine(guys[i], it))) cnt++;
        }
        cnt/=2;
        ans+=(cnt*(cnt-1))/2;
    }
