...
//author: I_Love_Minions _Love_Minins Love_Minions ove_Minions ve_Minions e_Minions _Minions Minions inions nions ions ons ns s
//orz tjw
//orz yjz
...
void dfs(int x){
	st[x]=++cnt;
	nw[cnt]=a[x];
	for (int i=0;i<G[x].size();i++) if (!st[G[x][i]])dfs(G[x][i]);
	ed[x]=cnt;
}
...
int main(){
	...
	for (int i=0;i<q;i++){
		int x;scanf("%d",&x);
		l[i]=st[x];r[i]=ed[x];scanf("%d",&k[i]);
		qs[l[i]/sqr][r[i]/sqr].push_back(i);
	}
	add(0,n);
	int nl=1,nr=0;
	for (int i=0;i<455;i++){
		for (int j=0;j<455;j++){
			for (int p=0;p<qs[i][j].size();p++){
				int id=qs[i][j][p];
				int lo=l[id],hi=r[id];
				while (nl>lo){
					nl--;
					add(cur[nw[nl]],-1);
					add(++cur[nw[nl]],1);//cout<<nl<<endl;
				}
				while (nl<lo){
					add(cur[nw[nl]],-1);
					add(--cur[nw[nl]],1);//cout<<nl<<endl;
				...
				ans[id]=query(100000)-query(k[id]-1);
			}
		}
	}
	for (int i=0;i<q;i++){
