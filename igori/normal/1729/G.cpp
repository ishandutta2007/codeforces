...
void solve()
{
    ...
    vi g(pos.size(), s.size());
    for (int i = 0; i < pos.size(); i++)
    {
        for (int j = i; j < pos.size(); j++)
        {
            if (pos[i] + t.size() <= pos[j])
            {
                g[i] = pos[j];
                break;
            }
    ...
    vvi dp(pos.size() + 1, vi(pos.size() + 1));
    vvi ye(pos.size() + 1, vi(pos.size() + 1));
    for (int i = 0; i < pos.size(); i++)
    {
        if (pos[0] + t.size() > pos[i])
            dp[i][1] = ye[i][1] = 1;
        for (int j = 2; j <= pos.size(); j++)
        {
            for (int k = 0; k < i; k++)
            {
                if (ye[k][j - 1] && g[k] + t.size() > pos[i] && g[k] <= pos[i])
                {
                    ye[i][j] = 1;
    ...
    dbg(g);
    dbg(dp);
    int ans1 = pos.size(), ans2 = 0;
    for (int i = 0; i < pos.size(); i++)
    {
        for (int j = 0; j <= pos.size(); j++)
        {
            if (ye[i][j] && g[i] == s.size())
            {
                if (j < ans1)
                {
                    ans1 = j;
