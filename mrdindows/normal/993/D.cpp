...
string readString() {
    auto c = [&]() { if (__ir == __er) std::fill(__bur, __bur + __BS, 0), cin.read(__bur, __BS), __ir = __bur; };
    c(); while (isspace(*__ir)) ++__ir; c(); while (isspace(*__ir)) ++__ir;
    string res; while (!isspace(*__ir)) { res += *__ir++; c(); }
    return res;
}
...
//pii a[2444];
//pii b[2444];
//int eval(pii p1, pii p2, int& what) {
//    int res = 0;
//    if (p1.first == p2.first) ++res, what = p1.first;
//    if (p1.first == p2.second) ++res, what = p1.first;
//    if (p1.second == p2.first) ++res, what = p1.second;
//    if (p1.second == p2.second) ++res, what = p1.second;
//    return res;
//}
//int main() {
//    int n, m;
...
//        DBN(i, v[i]);
        ...
        ll sum = 0;
        for (int j = 0; j < sz; ++j) {
            difs[j] = v[i][j].first - v[i][j].second;
        }
        sort(difs, difs + sz);
        for (int cnt = 0; cnt <= sz; ++cnt) {
            for (int j = sz - cnt; j < n; ++j) {
                dp[i + 1][j - sz + cnt + cnt] = min(dp[i + 1][j - sz + cnt + cnt], dp[i][j] + sum);
            }
            sum += difs[cnt];
        }
    }
