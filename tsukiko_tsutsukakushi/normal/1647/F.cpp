...
void solve() {
    ...
    rep(_, 2) {
        ...
        vc<int> dp1(n, inf), dp2(n, inf);
        dp1[0] = -1;
        for(int i = 1; i <= id; ++ i) {
            if(dp1[i - 1] < a[i]) chmin(dp1[i], a[i - 1]);
            if(a[i - 1] < a[i]) chmin(dp1[i], dp1[i - 1]);
        }
        dp2[n - 1] = -1;
        for(int i = n - 2; i >= id; -- i) {
            if(a[i] > dp2[i + 1]) chmin(dp2[i], a[i + 1]);
            if(a[i] > a[i + 1]) chmin(dp2[i], dp2[i + 1]);
        }
        vc<int> dp3(n, inf), dp4(n, -inf);
        dp3[id] = dp1[id];
        for(int i = id + 1; i < n; ++ i) {
            if(a[i - 1] > a[i]) chmin(dp3[i], dp3[i - 1]);
            if(dp4[i - 1] > a[i]) chmin(dp3[i], a[i - 1]);

            if(a[i - 1] < a[i]) chmax(dp4[i], dp4[i - 1]);
            if(dp3[i - 1] < a[i]) chmax(dp4[i], a[i - 1]);
