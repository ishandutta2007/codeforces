...
int main()
{
	...
	vector<vector<Mod>>gyou(50,vector<Mod>(50,Mod(0)));
	for (int i = 0; i < 50; ++i) {
		for (int j = 0; j < 50; ++j) {
			if(j<=i)gyou[i][j]=Mod(1)/Mod(i+1);
			else gyou[i][j]=Mod(0);
		}
	}
	vector<vector<Mod>>k_gyou=powgyou(gyou,K);

	map<long long int,int>mp(soinnsuu(N));
	Mod ans(1);
	for (auto m : mp) {
		long long int num=m.first;
		long long int cnt=m.second;
		vector<vector<Mod>>ini_v(1,vector<Mod>(50));
		ini_v[0][cnt]=Mod(1);
		vector<vector<Mod>>end_v=keisann(ini_v,k_gyou);
		Mod nans=0;
		for (int i = 0; i < 50; ++i) {
			nans+=end_v[0][i]*mod_pow(num,i);
		}
		ans*=nans;
	}
	cout<<ans.num<<endl;
