...
namespace MAIN{
    ...
    inline bool cmp(const RG Pair &x,const RG Pair &y){
        return x.se-x.fi!=y.se-y.fi?x.se-x.fi>y.se-y.fi:x.se>y.se;
    }
    int vis[N];
    inline void MAIN(){
        n=read();
        ...
        a[ax=1]=b[1];
        for(res T=2;T<=n;T++){
            res l=b[T].fi,r=b[T].se;
            for(res i=1;i<=ax;i++){
                if(a[i].fi<=l&&r<=a[i].se){
                    if(l==a[i].fi&&a[i].se==r){
                        ...
                        vis[r+1]=1;
                        printf("%d %d %d\n",a[i].fi,a[i].se,r+1);
                        if(r+2<=a[i].se)a[++ax]=mp(r+2,a[i].se);
                        a[i].se=r;
                    }
                    else {
                        vis[l-1]=1;
                        printf("%d %d %d\n",a[i].fi,a[i].se,l-1);
                        if(l-2>=a[i].fi)a[++ax]=mp(a[i].fi,l-2);
                        a[i].fi=l;
                    }
                    break;
                }
        ...
        for(res i=1;i<=n;i++)if(!vis[i])printf("%d %d %d\n",i,i,i);
        puts("");
    }
}
int main(){
