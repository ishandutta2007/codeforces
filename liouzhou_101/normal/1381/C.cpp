...
void solve()
{
	...
	for (int i = 1; i <= n + 1; ++i)
	{
		if (v[i].size() == 0) not_exist = i;
		H.insert({ v[i].size(), i });
	}
	// greedily find x positions of the majority colors
	for (int i = 0; i < x; ++i)
	{
		auto [cnt, col] = *H.rbegin();
		assert(cnt >= 1);
		H.erase({ cnt, col });
		res[v[col].back()] = col;
		v[col].pop_back();
	...
	// for the remaining not assigned n-x positions, apply the shift trick.
	vector<int> pos;
	for (int i = 1; i <= n + 1; ++i)
		pos.insert(pos.end(), v[i].begin(), v[i].end());
	assert(pos.size() == n - x);
	// apply the trick
	for (int i = 0; i < n - x; ++i)
		res[pos[i]] = b[pos[(i + (n - x) / 2) % (n - x)]];
	int d = n - y; // the occurrences of color not_exist
	for (int i = 0; i < n - x; ++i)
	{
		// when the shift causes color conflicts, it means that the occurrences of this color is > (n-x)/2,
		// just make that position of color not_exist.
		if (res[pos[i]] == b[pos[i]])
		{
