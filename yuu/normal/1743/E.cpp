...
// what is the last shot gun, how many time have we shoot singularly since then.
int64_t f[5001];
// time point, total damage
...
void solve() {
    ...
    cin >> h >> s;
    {
        int64_t cooldown[2] = {t[0], t[1]};
        v.emplace_back(0, 0);
        int64_t total_time = 0;
        int64_t total_damage = 0;
        while (total_damage < h) {
            int use = cooldown[1] < cooldown[0];
            total_time += cooldown[use];
            total_damage += p[use] - s;
            cooldown[1 - use] -= cooldown[use];
            cooldown[use] = t[use];
            v.emplace_back(total_time, total_damage);
        }
    }
    int k = v.size() - 1;
    fill(f + 0, f + h + 1, 1e18);
    ...
    for (int i = 0; i <= h; i++) {
        for (int j = 0; j <= k; j++) {
            auto&& [extra_time, extra_damage] = v[j];
            if (i + extra_damage >= h) {
                ans = min(ans, f[i] + extra_time);
                break;
            }
            // shoot both
            int64_t cooldown[2];
            for (int id = 0; id < 2; id++) cooldown[id] = t[id] - extra_time % t[id];

            int total_damage = i + extra_damage + p[0] + p[1] - s;
            int64_t next_time = f[i] + extra_time + max(cooldown[0], cooldown[1]);
            if (total_damage > h) {
                ans = min(ans, next_time);
            } else {
                f[total_damage] = min(f[total_damage], next_time);
            }
        }
    }
    cout << ans << '\n';
