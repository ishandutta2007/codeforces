...
void solve() {
    ...
    sums.push_back(0);
    REP(q, inputs.size()) {
        sums.push_back(sums.back() + inputs[inputs.size() - q - 1]);
    }
    sums.erase(sums.begin());
    LazySegmentTree<int> seg(sums.size(), [](int a, int b) {return min(a, b); }, [](int a, int b) {return a + b; }, [](int a, int b) {return a + b; }, (int)1e18, 0);
    for (int q = 0; q < sums.size(); ++q) {
        int base = (int)1e18;
        int remaining = n / 2;
        int hoge = remaining - q;
        hoge = max(hoge, 0LL);
        int nexts = sums[q] + x * hoge;
        seg.update(q, q + 1, nexts - base);
    }
    REP(q, sums.size()) {
        if (seg.query(q, sums.size()) > 0) {
            cout << q + 1LL + (n) / 2LL << endl;
            return;
        }
        if (q + 1 < sums.size()) {
            seg.update(q + 1, sums.size(), x);
        }
    }
    cout << -1 << endl;
    return;
