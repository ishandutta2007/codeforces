...
int main() {
	CASE(t) {
		...
		auto isBad = [&](int u, int v) {
			if (u > v) swap(u, v);
			auto it = lower_bound(bad.begin(), bad.end(), make_pair(u, v));
			return it != bad.end() && *it == make_pair(u, v);
		};
		...
		for (auto& i : cnt)
			c[i.second].emplace_back(i.first);
		for (auto& i : c)
		...
		for (auto& i : cnt) {
			for (auto& j : c) {
				for (auto& k : j.second) {
					if (i.first != k && !isBad(i.first, k)) {
						chkmax(ans, 1LL * (i.first + k) * (i.second + j.first));
						break;
					}
				}
			}
