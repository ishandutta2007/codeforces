...
il bool operator<( const auto &a , const auto &b ){
...
int main(){
	...
	For(i,n){
		for( auto u : env[i] ){
			if( u.add == 1 ) st.insert(u);
			else st.erase(st.find(u));
		}
		if(st.empty()) maxget[i] = {0,i,0};
		else maxget[i] = *st.begin();
	}
	...
	Forx(j,0,m){
		...
		Msn(dp[v],INF);
		For(i,n){
			Mymin(dp[v][i+1],dp[u][i]);
			Mymin(dp[u][maxget[i].d+1],dp[u][i]+maxget[i].coin);
		}
		Mymin(ans,dp[u][n+1]);
	}
	cout << ans << endl;
