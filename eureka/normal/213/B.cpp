...
template<class T> inline T minfacFunction(T n)//NOTES:minfacFunction(
...
i64 gao(){
    ...
    for(int i=9; i>=0; --i){
        for(int j=a[i]; j<=n; ++j){
            for(int k=a[i]; k<=j; ++k){
                if(i != 0){
                    dp[i][j] += dp[i+1][j-k] * cmn[j][k];
                }else if(j-k > 0){
                    dp[i][j] += dp[i+1][j-k] * cmn[j-1][k];
                }
                dp[i][j] %= P;
            }
        }
...
int main(){
    ...
    for(int i=1; i<N; ++i){
        cmn[i][0] = 1;
        for(int j=1; j<=i; ++j){
            cmn[i][j] = (cmn[i-1][j-1] + cmn[i-1][j]) % P;
        }
    }
    while(scanf("%d", &n) != EOF){
        for(int i=0; i<10; ++i){
