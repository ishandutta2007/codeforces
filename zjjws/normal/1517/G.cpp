...
void insert(int C,int D,int A,int B){add(A+nd,B,INFV);add(B+nd,C,INFV);add(C+nd,D,INFV);return;}
namespace Flew
{
    int S,T,n;
    int dis[N<<1];
...
void Init()
{
    ...
    for(int i=1;i<=nd;i++)if((nod[i].x&1)&&(nod[i].y&1))add(S,i,INFV);
    for(int i=1;i<=nd;i++)if((nod[i].x&1)&&(nod[i].y%2==0))add(i+nd,T,INFV);
    for(int i=1;i<=nd;i++)if(nod[i].x%2==0&&nod[i].y%2==0)
    {
        int x=nod[i].x,y=nod[i].y,A,B,C,D;A=i;
        if((B=ck(x+1,y))&&(C=ck(x-1,y-1))&&(D=ck(x,y-1)))insert(A,B,C,D);
        if((B=ck(x-1,y))&&(C=ck(x+1,y+1))&&(D=ck(x,y+1)))insert(A,B,C,D);
        if((B=ck(x-1,y))&&(C=ck(x+1,y-1))&&(D=ck(x,y-1)))insert(A,B,C,D);
        if((B=ck(x+1,y))&&(C=ck(x-1,y+1))&&(D=ck(x,y+1)))insert(A,B,C,D);
