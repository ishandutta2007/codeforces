...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	vector<int> par(n,-1),hei(n);
	function<void(int,int,int)> dfs=[&](int v,int p,int h){
		par[v]=p;
		hei[v]=h;
		for(int w:g[v])if(w!=p) dfs(w,v,h+1);
	};
	rep(i,n)if(par[i]<0) dfs(i,-1,0);
	vv<int> G(n+1);
	rep(i,n)for(int j:g[i]) G[i+1].pb(j+1);
	rep(i,n)if(par[i]<0){
		G[0].pb(i+1);
		G[i+1].pb(0);
	}
	...
	out(G,1);
	for(pii p:es){
		int r=h.LCA(p.X+1,p.Y+1)-1;
		out(p,r,1);
		int mn=r,mx=r;
		for(int v=p.X; v!=r; v=par[v]){ MN(mn,v); MX(mx,v);}
		for(int v=p.Y; v!=r; v=par[v]){ MN(mn,v); MX(mx,v); out(v,1);}
		ng.eb(mn,mx);
	}
	
	vector<int> hs(n,-1);
	for(pii p:ng) MX(hs[p.Y],p.X);
	reps(i,1,n) MX(hs[i],hs[i-1]);
	vector<ll> sum(n+1);
	rep(i,n) sum[i+1]=sum[i]+hs[i]+1;
	out(ng,hs,sum,1);
	int q;
	cin>>q;
	while(q--){
		int a,b;
		cin>>a>>b; --a;
		int id=lower_bound(all(hs),a)-hs.begin();
		MX(id,a);
		MN(id,b);
		//sum [id,b]
		ll ng=sum[b]-sum[id] - 1ll*a*(b-id);
		out(a,b,id,ng,1);
		cout<<(b-a)*(b-a+1ll)/2-ng<<"\n";
	}
  return 0;
}