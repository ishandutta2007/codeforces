...
int find(int x){
	...
	if(tp[u]==1)swap(u,v);
	else if(tp[u]==2&&tp[v]!=1)swap(u,v);
	f[u]=v,tp[u]=tp[v];
} bool insane(int x,int y){return find(x)==find(y);}
...
void solve(){
	...
	for(int i=1;i<=n;i++){
		...
		if(k==1){
			int u=read(); u=find(u);
			if(tp[u]!=1)tp[u]=1,ans.pb(i);
		}
		else{
			int u=read(),v=read(); u=find(u),v=find(v);
			if(!(tp[u]==1&&tp[v]==1)&&!insane(u,v))ans.pb(i),con(u,v);
		}
	}
	cout<<ksm(2,ans.size())<<" "<<ans.size()<<endl;
