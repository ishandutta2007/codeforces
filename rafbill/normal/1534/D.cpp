...
int main(){
  ...
  auto qu = [&](int i) -> vi {
    vi D = ask(i);
    for(li j = 0; j < (li)(n); ++j) if(D[j] == 1) if(uf.unite(i,j)) es.emplace_back(i,j);
    return D;
  };
  vi D = qu(0);

  li sz0 = 1, sz1 = 1;
  for(li i = 0; i < (li)(n); ++i) if(D[i] != 0) {
    if(D[i]&1) sz1 += 1;
    else sz0 += 1;
  }

  if(sz1<sz0) {
    for(li i = 0; i < (li)(n); ++i) if(D[i]!=0 && (D[i]&1)) qu(i);
  }else{
    for(li i = 0; i < (li)(n); ++i) if(D[i]!=0 && !(D[i]&1)) qu(i);
  }
  cout << "!" << endl;
