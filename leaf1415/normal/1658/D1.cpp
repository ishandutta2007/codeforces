...
void calc2(vector<ll> vec, ll x[], ll r)
{
	rep(i, 0, 16) x[i] = -1;
	per(i, 16, 0){
		vector<ll> vec0, vec1;
		...
		if(sz(vec0) == sz(vec1)) break;
		
		if(r == 1){
			if(sz(vec0) > sz(vec1)) vec = vec1, x[i] = 0;
			else vec = vec0, x[i] = 1;
		}
		else{
			if(sz(vec0) < sz(vec1)) vec = vec1, x[i] = 1;
			else vec = vec0, x[i] = 0;
		}
	}
}
...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> vec;
		rep(i, 0, n-1) vec.pb(a[i]);
		calc2(vec, x, 1);
		
		ll ret = 0;
		rep(i, 0, 16){
			if(x[i] == 1) ret |= 1<<i;
			if((l & (1<<i)) && (r & (1<<i))) ret |= 1<<i;
		}
		outl(ret);
	}
