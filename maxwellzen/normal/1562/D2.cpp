...
void solve() {
    cin >> n >> q >> s;
    pref.rsz(n+1); pref[0]=0;
    odd.clear();
    F0R(i, n) {
        int add = (s[i]=='+' ? 1 : -1);
        if (i%2!=0) add = -add;
        pref[i+1] = pref[i]+add;
        odd[pref[i+1]+pref[i]].insert(i+1);
    }
    F0R(i, q) {
        cin >> l >> r;
        if (r%2==l%2) {
            cout << 1 << nl;
            auto it = odd[pref[r] + pref[l-1]].lower_bound(l);
            cout << *it << nl;
        } else if (pref[r]==pref[l-1]) cout << 0 << nl;
        else {
            cout << 2 << nl;
            cout << l << ' ';
            auto it = odd[pref[r] + pref[l]].lower_bound(l+1);
            cout << *it << nl;
        }
    }
}
