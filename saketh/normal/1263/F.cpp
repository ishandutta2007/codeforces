...
int main() {
    ...
    F0R (t, 2) {
        int V; re(V); tree<E> tr(V); re<PARENT_LIST, 1>(tr);
        vi pow(N); re(pow); trav (e, pow) --e;
        vpii cov(V, {N, -1});
        F0R (i, N) cov[pow[i]] = {i, i+1};
        trav (u, tr.reverse_preorder) {
            trav (c, tr.children[u]) {
                ckmin(cov[u].f, cov[c].f);
                ckmax(cov[u].s, cov[c].s);
            }
            avoid[cov[u].s].eb(cov[u].f, tr.subt_sz[u] - !u);
        }
    }

    vi dp(N + 1);
    FOR (i, 1, N + 1) trav (p, avoid[i]) ckmax(dp[i], dp[p.f] + p.s);
    ps(dp.back());
