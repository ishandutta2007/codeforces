...
void dfs(int v, int p, vvi &g, vi &dist)
{
    dist[v] = dist[p] + 1;
    for (auto u : g[v]) if (u != p) dfs(u, v, g, dist);
}
...
void solve()
{
    ...
    dfs(0, 0, g, dist);
    vi deg(n);
    forn(i, n) deg[i] = g[i].size();
    set<pair<int, int>, greater<pii> > even, odd;
    for (int i = 0; i < n; i++)
    {
        if (deg[i] % 2 == 1)
        {
    ...
    while (even.size() || odd.size())
    {
        ...
        auto v = (*even.begin()).second;
        even.erase({dist[v], v});
        for (auto u : g[v])
        {
            if (!removed[u])
            {
                if (deg[u] % 2) odd.erase({dist[u], u});
                else even.erase({dist[u], u});
                deg[u]--;
                if (deg[u] % 2) odd.insert({dist[u], u});
                else even.insert({dist[u], u});
            }
        }
        removed[v] = 1;
        ans.pb(v + 1);
