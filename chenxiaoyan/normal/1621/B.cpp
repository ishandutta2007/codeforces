...
void mian() {
	n = read();
	int L = inf, R = -inf, lc = inf, rc = inf;
	tii mxR(-inf, inf);
	while(n--) {
		int l = read(), r = read(), c = read();
		if(l < L) L = l, mxR = mt(r, -c), lc = c;
		else if(l == L) chkmx(mxR, mt(r, -c)), chkmn(lc, c);
		if(r > R) R = r, rc = c;
		else if(r == R) chkmn(rc, c);
		int ans = min(lc + rc, X(mxR) == R ? -Y(mxR) : inf);
		prt(ans), pc('\n');
	}
}
