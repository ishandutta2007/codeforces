...
signed main() {
    ...
    vector<vector<vector<pair<int, pii>>>> G(n, vector<vector<pair<int, pii>>>(m));
    auto addedge = [&](pii from, pii to, int w) {
        G[from.f][from.s].pb({w, to});
        G[to.f][to.s].pb({w, from});
    };
    for (int i = 0; i < n; ++i)
        for (int j = 1; j < m; ++j) {
            int x;
    ...
    auto at = [&](vector<vector<int>> &dp, int di, int dj) -> int& {
        return dp[di + 10][dj + 10];
    };
    vector<vector<vector<int>>> dp(n, vector<vector<int>>(m, vector<int>(11, mod)));
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < m; ++j)
            dp[i][j][0] = 0;
    for (int len = 1; len <= k / 2; ++len) {
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < m; ++j)
                for (auto e : G[i][j]) {
                    chmin(dp[i][j][len], dp[e.s.f][e.s.s][len - 1] + e.f);
                }
    }
