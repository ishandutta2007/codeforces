...
int main() {
    ...
    for (int i = sz(use)-1; i >= 0; i--) {
        ...
        int l = use[i].f;
        int r = use[i].s;
        f1r(j, l, r+1) ckmax(best, {a[j], j});
        bool ok = false;
        f1r(j, l, r) {
            if (a[j] == best.f && a[j+1] != best.f) {
                best.s = j;
                ok = true;
                break;
            }
        ...
        if (id > l && a[id] != a[id-1]) {
            ...
            id--;
            r--;
        } else if (id < r && a[id] != a[id+1]) {
            moves.eb(id+1, 'R');
            r--;
        }
        f0r(j, r-id) moves.eb(id+1, 'R');
        f0r(j, id-l) { 
            moves.eb(id+1-j, 'L');
        }
    }
    ps("YES");
    for (auto x : moves) ps(x.f, x.s);
    return 0;
}