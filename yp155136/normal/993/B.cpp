...
int main () {
    ...
    {
        pii p = v1[i];
        //if (setq.find(p) != setq.end()) continue;
        multiset<int> can_num;
        for (int j=0;m>j;j++)
        {
            pii q = v2[j];
            if (p == q) continue;
            ...
            if(st.size() == 3)
            {
                ...
                stt.insert(q.F);
                stt.insert(q.S);
                if (stt.count(p.F) == 2) can_num.insert(p.F);
                else if(stt.count(p.S) == 2) can_num.insert(p.S);
            }
        }
        if (can_num.size() >= 2 &&can_num.count( (*can_num.begin()) ) != can_num.size())
        {
            cout << -1 << endl;
            return 0;
        }
        if (can_num.empty()) continue;
        ans.insert((*can_num.begin()));
    }
    swap(n,m);
    swap(v1,v2);
    for (int i=0;n>i;i++)
