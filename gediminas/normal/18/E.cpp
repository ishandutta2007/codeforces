		...
		for (int j = 0; j < 26; j++) {
			for (int k = 0; k < 26; k++) {
				...
				for (int r = 0; r < 26; r++) {
					...
					if (mi[i + 1][j][r] > kie[i + 1][j][k]) {
						mi[i + 1][j][r] = kie[i + 1][j][k];
						kada[i + 1][j][r] = makep(j, k);
					}
				}
			}
		}
