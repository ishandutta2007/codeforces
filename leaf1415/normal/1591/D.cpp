...
//vecvec2swap-1
ll inversion(vector<ll> vec, vector<ll> vec2) 
{
	if(sz(vec) != sz(vec2)) return -1;
	int n = sz(vec);
	
	map<ll, vector<int> > mp;
	reps(i, vec2) mp[vec2[i]].push_back(i);
...
int main(void)
{
	...
	while(T--){
		...
		sort(a+1, a+n+1);
		rep(i, 1, n) vec2.push_back(a[i]);
		
		bool flag = false;
		rep(i, 1, n-1){
			if(a[i] == a[i+1]) flag = true;
		}
		if(flag || inversion(vec, vec2) % 2 == 0) cout << "YES" << "\n";
		else cout << "NO" << "\n";
	}
