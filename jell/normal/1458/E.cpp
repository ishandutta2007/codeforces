#line 1 "codeforces-workspace\\contest\\1458\\e\\e2.cpp"
...
#line 11 "codeforces-workspace\\contest\\1458\\e\\e2.cpp"
...
#line 2 "Library\\src\\data_structure\\map.hpp"
...
#line 9 "Library\\src\\data_structure\\map.hpp"

#line 2 "Library\\src\\data_structure\\set.hpp"
...
#line 9 "Library\\src\\data_structure\\set.hpp"

#line 12 "Library\\src\\data_structure\\set.hpp"
...
#line 11 "Library\\src\\data_structure\\map.hpp"
...
#line 29 "codeforces-workspace\\contest\\1458\\e\\e2.cpp"
...
void main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<pair<i32, i32>> pos(n + m);
  cin >> pos;

  map<int, vector<int>> X, Y;

  for (auto [i, p] : enumerate(pos)) {
    X[p.first].emplace_back(i);
    Y[p.second].emplace_back(i);
  }

  constexpr auto inf = 1 << 30;
  ...
  while (true) {
    ...
    cx += d, cy += d;
    auto xs = 0, ys = 0;
    for (auto &[S, c, shift] : {tie(X, cx, xs), tie(Y, cy, ys)}) {
      if (!S.empty() && S.begin()->first == c) {
        for (auto i : S.begin()->second) {
          auto [x, y] = pos[i];
          if (x > cx || y > cy) continue;
          if (i < n) {
            if (cx == x && cy == y) xs = ys = 1;
            shift = 1;
          } else
            qs.emplace_back(i);
        }
    ...
    for (auto i : qs) {
      auto [x, y] = pos[i];
      if (x < cx || y < cy) continue;
      if (x - cx == y - cy) ans[i - n] = 1;
    }
  }

  set<pair<int, int>> check(pos.begin(), pos.begin() + n);
  for (auto i : range(m)) {
    if (check.count(pos[i + n])) {
      ans[i] = 1;
    }
  }
  for (auto &&x : ans) {
