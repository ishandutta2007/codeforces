...
int e = 0;
segtree<decltype(f), decltype(e)>pre((1<<18), f, e);
segtree<decltype(f), decltype(e)>suf((1<<18), f, e);
void solve(){
	...
	rep(i, q){
		...
		if(id == 1){
			int d, num; cin >> d >> num;
			int x = pre.query(d, d);
			pre.update(d, min(b, x+num) - x, true);
			x = suf.query(d, d);
			suf.update(d, min(a, x+num) - x, true);
		}
		else{
			int p; cin >> p;
			o(pre.query(1, p-1) + suf.query(p+k, n));
		}
	}
}
signed main(){
