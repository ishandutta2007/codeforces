...
ans_type solve() {
    auto time_to = [&](ld pos, vector<llong>& pivots) {
        ld ans = 0;
        ld speed = 1;
        ld prev = 0;
        for (auto i: pivots) {
        ...
        return ans + (pos - prev) / speed;
    };
    
    vector<llong> b(a, a + n), c;
    for (int i = n; i--;) {
        c.push_back(l - a[i]);
    }
    ...
    rep(cnt, 100) {
        ld mid = (left + right) / 2;
        if (time_to(mid, b) < time_to(l - mid, c)) left = mid;
        else right = mid;
    }
    
    return time_to(left, b);
}
