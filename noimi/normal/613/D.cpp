...
main() {
    ...
    auto merge = [&](int i, int j) {
        if(size(dp[i]) < size(dp[j])) swap(dp[i], dp[j]);
        for(auto P : dp[j]) {
            auto a = P.fi;
            auto b = P.se;
            if(dp[i].count(a)) {
    ...
    auto dfs = [&](auto &&f, int x, int p) -> void {
        ...
        for(auto e : query[x]) {
            if(dp[x].count(e)) {
                if(d[dp[x][e].fi] == d[x] + 1) no[e] = 1;
                ans[e] += dp[x][e].se, dp[x].erase(e);
            }
        }
        for(auto e : take[x])
            if(dp[x].count(e)) { ans[e]++, dp[x].erase(e); }
        for(auto e : query[x]) dp[x][e] = {x, 1};
    };
    dfs(dfs, 0, -1);
    rep(i, Q) {
        if(no[i])
