...
int main() {
  ...
  ModInt ans = 0;
  FOR(c, 1, n) {
    const vector<int> d = divisor.query(n - c);
    const int m = d.size();
    vector<ModInt> ways(m);
    REP(i, m) {
      const int g = d[i];  // gcd(a, b)
      ways[i] = (n - c) / g - 1;
    }
    for (int i = m - 1; i >= 0; --i) FOR(j, i + 1, m) {
      if (d[j] % d[i] == 0) ways[i] -= ways[j];
    }
    REP(i, m) ans += ways[i] * d[i] * c / gcd(d[i], c);
  }
  cout << ans << '\n';
  return 0;
}