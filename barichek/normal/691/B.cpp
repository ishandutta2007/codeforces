...
bool check(char a,char b)
{
    ...
    if (b=='b'&&a=='d') return true;
    if (a=='p'&&b=='q') return true;
    if (b=='p'&&a=='q') return true;
    if (a=='o'&&b=='o') return true;
    if (a=='v'&&b==a) return true;
    if (a=='w'&&b==a) return true;
    if (a=='x'&&b==a) return true;
...
int main()
{
    ...
    if (s.length()%2==1)
    {
        if (!check(s[s.length()/2])) return cout<<"NIE", 0;
        for (int l=s.length()/2-1,r=s.length()/2+1;l>=0&&r<s.length();l--,r++)
            if (!check(s[l],s[r])) return cout<<"NIE", 0;
        cout<<"TAK";
    }
    else
    {
        for (int l=s.length()/2-1,r=s.length()/2;l>=0&&r<s.length();l--,r++)
            if (!check(s[l],s[r])) return cout<<"NIE", 0;
        cout<<"TAK";
    }
}