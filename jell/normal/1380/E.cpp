...
struct solver
{
    ...
    solver()
    {
        int n=read<int>();
        int m=read<int>();
        auto t=read<vector<int>>(n);
        for(int &x: t)
        {
            x--;
        }
        i64 ans=-1;
        vector<map<int,int>> ivl(m);
        ...
        for(int q=1; q<m; q++)
        {
            ...
            if(ivl[a].size()<ivl[b].size()) swap(a,b);
            for(auto [l,r]: ivl[b])
            {
                auto pos=ivl[a].emplace(l,r).first;
                if(next(pos)!=end(ivl[a]) and next(pos)->first==r)
                {
                    r=pos->second=next(pos)->second;
                    pos=prev(ivl[a].erase(next(pos)));
                    ans--;
                }
                if(pos!=begin(ivl[a]) and prev(pos)->second==l)
                {
                    prev(pos)->second=r;
                    ivl[a].erase(pos);
                    ans--;
