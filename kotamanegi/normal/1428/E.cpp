...
void solve() {
	...
	cin >> n >> k;
	int ans = 0;
	priority_queue<tuple<int,int,int, int>> nexts;
	REP(i, n) {
		int a;
		cin >> a;
		ans += a * a;
		int hoge = a * a;
		nexts.push(make_tuple(eval(a, 1), eval(a, 2), a, 1));
	}
	k -= n;
	while (!nexts.empty() and k != 0) {
		...
		int b = get<3>(now);
		b++;
		nexts.push(make_tuple(eval(a, b), eval(a, b + 1), a, b));
	}
	cout << ans << endl;
}
#undef int
