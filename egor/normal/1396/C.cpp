...
public:
    void solve() {
        ...
        auto withMove = [&](int id) -> ll {
            return min(ll(r1) * (a[id] + 2), ll(r1 + r2));
        };
        auto clean = [&](int id) -> ll {
            return ll(r1) * a[id] + r3;
        };
        arr<ll> finish(n);
        finish[n - 1] = clean(n - 1);
        for (int i : RevRange(n - 1)) {
            finish[i] = finish[i + 1] + withMove(i) + 2 * d;
        }
        for (int i : RevRange(n - 1)) {
            minim(finish[i], finish[i + 1] + clean(i) + d);
            if (i + 2 < n) {
                minim(finish[i], finish[i + 2] + withMove(i) + withMove(i + 1) + 4ll * d);
            }
            if (i + 2 == n) {
                minim(finish[i], withMove(i) + withMove(i + 1) + 3ll * d);
            }
            if (i + 3 < n) {
                minim(finish[i], finish[i + 3] + withMove(i) + withMove(i + 1) + withMove(i + 2) + 7ll * d);
            }
            if (i + 3 == n) {
                minim(finish[i], withMove(i) + withMove(i + 1) + withMove(i + 2) + 6ll * d);
            }
        }
        out.printLine(finish[0]);
    }
};
