...
#line 1 "/home/maspy/compro/library/seq/find_linear_rec.hpp"
template <typename mint>
...
void solve() {
  ...
  FOR(i, LIM) {
    F[i] = SUM(DP[K]);
    vv(mint, newDP, K + 1, 1 << M);
    FOR(s, 1 << M) {
      ll t = (s << 1) & mask;
      FOR(k, K + 1) newDP[k][t] += DP[k][s];
      FOR(k, K) newDP[k + 1][t + 1] += mint(popcnt(s) + 1) * DP[k][s];
    }
    swap(DP, newDP);
  }
  print(interpolate_linear_rec(F, N));
}
