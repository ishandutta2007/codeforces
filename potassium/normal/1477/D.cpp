...
int main() {
	CASE(t) {
		...
		auto getNext = [&](int cur) {
			auto it = untrav.upper_bound(cur);
			if (it == untrav.end())
				return -1;
			else
				return *it;
		...
		function<void(int)> dfs = [&](int u) {
			untrav.erase(u);
			for (int to = getNext(0); to != -1; to = getNext(to)) {
				if (!g[u].count(to)) {
					g2[u].emplace_back(to);
					g2[to].emplace_back(u);
					dfs(to);
				}
			}
		};
		...
		while (!leaf.empty()) {
			...
			for (auto& v : g2[center]) {
				if (!rm[v] && deg[v] <= 1) {
					removeNode(v);
					stars.back().second.emplace_back(v);
				}
			}
		...
		int num = 0;
		for (auto& star : stars) {
			const int leafsz = star.second.size();

			++num;
			p[star.first - 1] = num;
			q[star.first - 1] = num + leafsz;

			for (auto& v : star.second) {
				++num;
				p[v - 1] = num;
				q[v - 1] = num - 1;
			}
