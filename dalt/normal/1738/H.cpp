...
template<class T> struct SegTree {  // cmb(ID,b) = b
  ...
  /// int first_at_least(int lo, int val, int ind, int l, int r) { // if seg
  /// stores max across range 	if (r < lo || val > seg[ind]) return -1; 	if (l ==
  /// r) return l; 	int m = (l+r)/2; 	int res = first_at_least(lo,val,2*ind,l,m);
  /// if (res != -1) return res; 	return first_at_least(lo,val,2*ind+1,m+1,r);
  /// }
};
void SolveOne(int test_id, IStream &in, OStream &out) {
  //using SBT = SelfBalanceTreeRegistry<int, int>;
  //using ST = SegTree<SBT>;
  int Q;
  in >> Q;
  seq::Eertree tree(Q);
  ...
  for(int i = 0; i < Q; i++) {
    ...
    if(op == "push") {
      ...
      in >> c;
      tree.push_back(c);
      nodes.push_back(tree.back_build_last);
      total_size++;
    } else {
      if(tree.back_build_last->len == total_size) {
        tree.back_build_last = tree.back_build_last->fail; 
      }
      tree.front_size++;
  ...
  //ST::Register(-1, -1, adder, adder, adder);
  //ST st(N);
