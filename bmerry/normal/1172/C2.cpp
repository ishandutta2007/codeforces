...
struct picture
{
    ...
    {
        return tie(d, w) < tie(other.d, other.w);
    }
};

static void solve(vector<picture> &pics, const vi &wsum, vector<mr> &ans,
                  int A, int B, const vector<mr> &pdist, int d)
{
    int M = SZ(pdist) - 1;
    int W = wsum[B] - wsum[A];
    mr scale0 = mr(W).inverse();
    mr prod = 0;
    for (int i = 0; i <= M; i++)
    {
        mr scale = scale0 * (W + i * d);
        prod += scale * pdist[i];
    }
    for (int i = A; i < B; i++)
        ans[pics[i].idx] = prod * pics[i].w;
}
...
int main(int argc, const char **argv)
{
    ...
    int WL = wsum[cut];
    vector<mr> pleft{1};
    unordered_map<int, mr> inverse_cache;
    auto inversew = [&](int w)
    {
        auto pos = inverse_cache.find(w);
        if (pos == inverse_cache.end())
            return inverse_cache[w] = mr(w).inverse();
        else
            return pos->second;
    };
    ...
    for (int i = 0; i < M; i++)
    {
        ...
        for (int j = SZ(pleft) - 1; j >= 0; j--)
        {
            ...
            if (j > 0 && pleft[j - 1] != 0)
            {
                int w = W - (j - 1) + (i - j + 1);
                int wl = WL - (j - 1);
                p += pleft[j - 1] * wl * inversew(w);
            }
            if (pleft[j] != 0)
            {
                int w = W - j + (i - j);
                int wl = WL - j;
                p += pleft[j] * (w - wl) * inversew(w);
            }
            pleft[j] = p;
        }
    }
    ...
    vector<mr> ans(N);
    solve(pics, wsum, ans, 0, cut, pleft, -1);
    reverse(RA(pleft));
    solve(pics, wsum, ans, cut, N, pleft, 1);
