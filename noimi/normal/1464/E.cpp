...
template <typename T> vector<T> xor_conv(vector<T> f, vector<T> g) {
    int n = 1;
...
int main() {
    ...
    auto mex = [&](int i) -> int {
        vi w;
        for(auto e : g[i]) w.eb(v[e]);
        UNIQUE(w);
        rep(i, si(w)) if(w[i] != i) return i;
        return si(w);
    };
    SCC scc(g);
    vi id = iota(n);
    sort(all(id), [&](int i, int j) { return scc[i] < scc[j]; });
    rep3(i, n - 1, 0) { v[id[i]] = mex(id[i]); }
    ...
    rep(i, n) f[v[i]] += 1;
    fwt(f);
    rep(i, N) f[i] = f[i] * inv((mint)1 - f[i] * inv(n + 1));
    ifwt(f);
    cout << (mint)1 - f[0] * inv(n + 1) * inv(n + 1) - inv(n + 1) << endl;
}