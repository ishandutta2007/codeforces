...
fun main(){
    solve.cases{
        ...
        val need = TreeMap<Long,Int>()
        if(n == 1){
            put("YES")
            return@cases
        }
        for(a in L){
            need[a.toLong()] = need.getOrDefault(a.toLong(),0) + 1
        }
        val has = TreeMap<Long,Int>()
        has[total] = 1
        while(has.isNotEmpty()){
//            just dei has
            val max = has.pollLastEntry()
            if(max.value != 1){
                has[max.key] = max.value - 1
            }
            if(max.key < maxOf(2,if(need.isNotEmpty()) need.lastKey() else 0)) break
            val v = max.key
            val new1 = v / 2
            val new2 = (v+1)/ 2

            for(new in listOf(new1,new2)){
                if(need.getOrDefault(new,0) >= 1){
                    val nv = need[new]!! - 1
                    if(nv == 0){
                        need.remove(new)
                    }else{
...
//                    if
                    ...
                    has[new] = has.getOrDefault(new,0) + 1
                }
            }
        }
        if(has.isEmpty()){
