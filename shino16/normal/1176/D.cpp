...
int main() {
  ...
  bitset<2750132> notprime;
  notprime[0] = notprime[1] = true;
  for(int i = 2; i < sqrt(2750132); i++){
    if(not notprime[i]){
      for(int j = 2; i * j < 2750132; j++){
        notprime[i * j] = true;
      }
    }
  }
  ...
  map<int, int> pord;
  int cnt = 0;
  rep(i, 2750132) if (not notprime[i]) pord[i] = ++cnt;

  VI P, A, B, D;
  for (auto it = b.rbegin(); it != b.rend(); it++) {
    auto [e, k] = *it;
    if (k == 0 or not notprime[e]) continue;
    rep(k) B.push_back(e);
    int d;
    rep(i, 2, e+1) if (e % i == 0) { d = e / i; break; }
    b[e] = 0;
    b[d] -= k;
    rep(k) D.push_back(d);
    debug(e);
