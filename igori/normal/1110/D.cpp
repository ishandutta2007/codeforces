...
signed main()
{
    ...
    for (int i = 1; i <= k; i++)
    {
        for (int use0 = 0; use0 < 3; use0++)
        {
            for (int used1 = 0; used1 < 3; used1++)
            {
                for (int used2 = 0; used2 < 3; used2++)
                {
                    //dp[i][used2][used1][use0]
                    for (int sunset = 0; sunset < 3; sunset++)
                    {
                        if (cnt[i] < use0)
                            break;
                        int e = dp[i - 1][sunset][used2][used1];
                        int cnt3 = cnt[i - 3] - sunset - used2 - used1;
                        int cnt2 = cnt[i - 2] - used2 - used1;
                        int cnt1 = cnt[i - 1] - used1;
                        if (cnt2 >= use0 && cnt1 >= use0)
                        {
                            if (cnt2 % 3 < use0) e--;
                            if (cnt1 % 3 < use0) e--;
                            e += use0 + (cnt[i] - use0) / 3;
                            dp[i][used2][used1][use0] = max(e, dp[i][used2][used1][use0]);
                        }
                    }
                        a = max(a, dp[i][used2][used1][use0]);
                }
            }
        }
    }
