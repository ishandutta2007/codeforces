...
inline fun simpleSegTree.segDivisonOrdered(l:Int, r:Int, act:(index:Int)->Unit){
    var left = l + n
    var right = r + n + 1
    var level = 0
    while(left < right){
...
class simpleSegTree (nSuggest  :Int){
    ...
    val arr = Array<item>(n * 2 + 1 ){item(0,0,0,0)}
    val arrSize = n * 2 + 1

    private fun updateNode(i:Int){
        arr[i] = arr[i shl 1 ] merge arr[(i shl 1) + 1]
    }
...
class item(val tt:Long, val rr:Long, val tr:Long, val rt:Long){
    infix fun merge(other:item):item{
        return item(
            tt = minOf(this.tt + other.tt, this.tr + other.rt),
            tr = minOf(this.tt + other.tr, this.tr + other.rr),
            rt = minOf(this.rt + other.tt , this.rr + other.rt),
            rr = minOf(this.rr + other.rr, this.rt + other.tr))
    }
}
...
fun main(){
    solve.cases{
        ...
        val door1 = mutableListOf<Pair<Int,Int>>()
        val door2 = mutableListOf<Pair<Int,Int>>()
        repeat(n-1){
            val (a,b,c,d) = getline(4)
            door1.add(Pair(a,b))
            door2.add(Pair(c,d))
        }
        ...
        for(i in 1 until n-1){
            val key = Pair(i+1,i+1)
            val tt = dist(door1[i-1],door1[i]).toLong()
            val rr = dist(door2[i-1], door2[i]).toLong()
            val tr = dist(door1[i-1],key) + dist(key, door2[i]) + 0L
            val rt = dist(door2[i-1], key) + dist(key, door1[i]) + 0L
            val new = item(tt,rr,tr,rt)
            st.arr[st.n + i] = new
        }
        val z = st.arr[st.n + 1] merge st.arr[st.n  +2]
        st.updateAll()
        val q = getint
        fun query(a:IntArray):Long{
            var (x1,y1,x2,y2) = a
            var r1 = maxOf(x1,y1)
            var r2 = maxOf(x2,y2)
            if(r1 == r2){
                return (dist(Pair(x1,y1), Pair(x2,y2)).toLong())
            }
            if(r1 > r2){
                r1 = r2.also { r2 = r1  }
                x1 = x2.also { x2 = x1 }
            ...
            val p1 = Pair(x1,y1)
            val p2 = Pair(x2,y2)
            val AT = dist(p1,door1[r1-1])
            val AR = dist(p1,door2[r1-1])

            val M = if(r1 + 1 == r2) item(0,0,phuge,phuge) else st.queryFunctional(r1,r2-2)

            val (K1,K2) = door1[r2-2]
            val (M1,M2) = door2[r2-2]

            val rd1 = Pair(K1+1,K2)
            val rd2 = Pair(M1,M2+1)


            val BT = dist(p2,rd1)
            val BR = dist(p2,rd2)
            val ret = minOf(AT + BT + M.tt, AT + BR + M.tr, AR + BT + M.rt, AR + BR + M.rr)
            assert( ret >= 0 )
            return (ret+1)
        }
        ...
        fun verify(){
            val G = Graph(n*n, 4 *n*n, false)
            for(i in 1..n){
                for(j in 1..n){
                    if(i != maxOf(i,j)){
                        G.add(code(i,j), code(i+1,j))
                    }
                    if(j != maxOf(i,j)){
                        G.add(code(i,j), code(i,j+1))
                    }
                }
            }
            for((a,b) in door1){
                G.add(code(a,b),code(a+1,b))
            }
            for((a,b) in door2){
                G.add(code(a,b),code(a,b+1))
            }
            fun uncode(i:Int):Pair<Int,Int>{
                return Pair((i/n) +1 , (i %n) + 1)
            }
            val MM = n*n
            for(i in 0 until MM){
                val dist = G.BFS(i)
                for(j in 0 until MM){
                    ...
                    val (a,b) = uncode(i)
                    val (c,d) = uncode(j)
                    val ans2 = query(intArrayOf(a,b,c,d))
                    if(a1.toLong() != ans2){
                        just dei "WRONG $a $b $c $d, brute $a1 got $ans2"
                    }
                }
            }
        }
