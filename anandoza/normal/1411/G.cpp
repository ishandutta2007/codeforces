...
template <typename I> int find_mex(I first, I last) {
    vector<int> seen;
...
int main() {
    ...
    vector<int> mex(n);
    for (int cur : reversed(topo)) {
        vector<int> choices;
        for (int next : adj[cur])
            choices.push_back(mex[next]);
        mex[cur] = find_mex(all(choices));
    }

    pd(mex);

    int S = 1 + accumulate(all(mex), 0, bit_or<int>());
