// Problem: E. Mr. Kitayuta vs. Bamboos
...
#endif
int h[1000005],a[1000005];
int n=read(),m=read(),x=read(),p=read();
int b[1000005];
struct zky{
	int pos,val;
	bool  operator<(const zky&b)const{
		return val>b.val;
	}
};
int f(int x,int y)
...
int check(int qwq)
{
	std::priority_queue<zky>q;
	rg(n)b[i]=qwq;
	if(b[i]-m*a[i]<h[i])q.push((zky){i,b[i]/a[i]});
	// q.push((zky){i,m-b[i]/a[i]+1});
	gr
	rg(m)rg_(j,x)
	if(q.empty())return 1;
	auto qwq=q.top();q.pop();
	if(qwq.val<i)return 0;
	#define i qwq.pos
	b[i]+=p;
	if(b[i]-m*a[i]<h[i])q.push((zky){i,b[i]/a[i]});
	#undef i
	...
	// {
		// rg_(j,x)
		// auto qwq=q.top();q.pop();
		// int pos=qwq.pos,v=qwq.val;
		// // printf("round %d add %d(%d)\n",i,pos,v);
		// if(v<=0)goto QWQ;
		// if(v>i)return 0;
		// b[pos]+=p;
		// if(m-b[pos]/a[pos]+1>i)return 0;
		// q.push((zky){pos,m-b[pos]/a[pos]+1});
		// ttt--;
		// gr
		// // if(0)
		// // {
	...
	// QWQ:;
	// rg(n)b[i]-=m*a[i];gr
	// rg(n)if(b[i]<0)return 0;gr
	// // rg(n)odb(h[i]-b[i]);gr puts("");
	// // odl(ttt);
	// int tot=0;
	// rg(n)tot+=f(h[i]-b[i],p);gr
	// // odl(tot);
	// // odl(p);
	// // odl(tot<=ttt);
	// return tot<=ttt;
...
{
	initprog();
	rg(n)h[i]=read(),a[i]=read();gr
	int l=*std::min_element(a+1,a+1+n),r=1e18,ans;
	// check(17);
	while(l<=r)
	{
		int m=l+r>>1;
