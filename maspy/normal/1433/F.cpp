...
void solve() {
  ...
  const ll INF = 1LL << 60;
  Matrix<ll> DP(W + 1, K);
  Matrix<ll> newDP(W + 1, K);
  DP.fill(-INF);
  DP[0][0] = 0;
  FOR(h, H) {
    FOR(cnt, W / 2 + 1) {
      FOR(k, K) { chmax(DP[0][k], DP[cnt][k]); }
    }
    FOR(w, W) {
      newDP.fill(-INF);
      FOR(cnt, W + 1) {
        FOR(k, K) {
          // 
          chmax(newDP[cnt][k], DP[cnt][k]);
          // 
          if (cnt + 1 <= W) {
            chmax(newDP[cnt + 1][(k + A[h][w]) % K], DP[cnt][k] + A[h][w]);
          }
        }
      }
      swap(DP, newDP);
  ...
  ll ANS = 0;
  FOR(cnt, W / 2 + 1) { chmax(ANS, DP[cnt][0]); }
  print(ANS);
}
