...
int main(){
  ...
  rep(i,0,n)rep(j,i+1,n){
    mp[abs(x[i]-x[j])+abs(y[i]-y[j])].EB(i,j);
  }
  Comb comb(1000);
  for(auto z:mp){
    for(auto [u,v]:z.se){
    ...
    //cout<<z.fi<<":"<<z.se<<endl;
    rep(i,0,n){
      if(i!=uf.root(i)||uf.find(i,n))continue;
      ll m=uf.size(i);
      if(m==1)continue;
      ll ve=uf.val(i);
      uf.unite(n,i);
      if(ve!=m*(m-1)/2)continue;
      vector<modint>ndp(n+1);
      rrep(i,0,n+1){
        if(i+1<=n)ndp[i+1]+=dp[i];
        if(i+m<=n)ndp[i+m]+=dp[i];
      }
      //cout<<m spa uf.val(i)<<endl;
      rem-=m;
      dp.swap(ndp);
    }
  }
