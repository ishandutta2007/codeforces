...
const int L=110;
using val=array<uint64_t,L>;

template<>
ostream& operator<< <uint64_t,L>(ostream&o,const val&v){
  for(int i=1;i<=6;++i) o<<((v[i>>6]>>(i&63))&1);
  return o;
}
...
int main(int argc, char **argv){
  ...
  vector<val> sqfm(7001);
  for(int v=1;v<sz(sqfm);++v){
    for(int x:sqf){
      x*=v;
      if(x>=L*64) break;
      sqfm[v][x>>6]|=1ull<<(x&63);
    }
  }
  vector<val> divs(7001);
  for(int x=1;x<sz(divs);++x){
    for(int v=x;v<sz(divs);v+=x) divs[v][x>>6]|=1ull<<(x&63);
  }
  ...
  forn(qq,tc){
    ...
    if(ty==1||ty==4){
      ...
      if(ty==1){
        ...
        val &v1=A[x],&v2=sqfm[v];
        int r=0;
        forn(i,L) r+=__builtin_popcountll(v1[i]&v2[i]);
        cout<<r%2;
      }
    }else{
      int y,z;
      cin>>y>>z;
      --y;--z;
      val &v1=A[x],&v2=A[y],&v3=A[z];
      if(ty==2){
        forn(i,L) v1[i]=v2[i]^v3[i];
      }else{
        forn(i,L) v1[i]=v2[i]&v3[i];
      }
    }
