...
void solve()
{
    ...
    for (int i = 0; i <= n; i++)
    {
        for (int j = 0; j <= m; j++)
        {
            ...
            {
                if (i + 1 <= n && j + 1 <= m) dp[i + 1][j + 1][ii] = max(dp[i + 1][j + 1][ii], dp[i][j][ii] + a[i] * b[j]);
                if (i + 1 <= n && ii + 1 <= k) dp[i + 1][j][ii + 1] = max(dp[i + 1][j][ii + 1], dp[i][j][ii] + a[i] * c[ii]);
                if (j + 1 <= m && ii + 1 <= k) dp[i][j + 1][ii + 1] = max(dp[i][j + 1][ii + 1], dp[i][j][ii] + b[j] * c[ii]);
            }
        }
    }
    ll ans = 0;
