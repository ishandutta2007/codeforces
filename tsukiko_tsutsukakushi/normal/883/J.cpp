...
int solve(int n, int m, vector<long long> s, vector<int> r, vector<long long> p) {
...
int main() {
    ...
    rep(i, n) {
        ...
        if (a[i] > rmax[i + 1]) {
            s.push_back(sum);
            r.push_back(upper_bound(all(b), a[i]) - b.begin());
        }
    }
    r.push_back(0);
    assert(is_sorted(r.rbegin(), r.rend()));
    // debug(b);
    // debug(p);
    // debug(s);
    // debug(r);
    n = s.size();
    print(solve(n, m, s, r, p));
    return 0;
}