...
inline int ge(int a, int b, int cnt)
{
	if(b == 1)
	{
		return mul(a, cnt);
	}
    return dv(mul(a, sub(1, bp(b, cnt))), sub(1, b));
}
...
inline void _main_function()
{
    ...
    for(int i = 0; i < k; ++i)
    {
        if(s[i] == '+')
        {
            ans = add(ans, ge(mul(bp(a, n - i), bp(b, i)), step, (n + 1) / k));
        }
        else
        {
            ans = sub(ans, ge(mul(bp(a, n - i), bp(b, i)), step, (n + 1) / k));
        }
    }
