...
val tran = Array<Array<IntArray>>(2*SZ){Array<IntArray>(4){IntArray(4)} }

fun comb(l: Array<IntArray>, r: Array<IntArray>): Array<IntArray> {
    val res = Array<IntArray>(4){IntArray(4)}
    for (i in 0..3) for (j in 0..3) for (k in 0..3) {
        res[i][k] = add(res[i][k],mul(l[i][j],r[j][k]))
    }
    return res
}
...
fun getAns(): Int {
    ...
    for (i in 0..3) for (j in 0..3)
        ans = add(ans,tran[1][i][j])
    return ans
}

fun upd(_ind: Int, newVal: Array<IntArray>) {
    var ind = _ind+SZ
    tran[ind] = newVal
    while (ind > 1) {
        ind /= 2
        tran[ind] = comb(tran[2*ind],tran[2*ind+1])
    }
}

fun strict(a: Int): Array<IntArray> {
    val res = Array<IntArray>(4){IntArray(4)}
    if (a == 0) {
        for (i in 0..3) for (j in 0..i-1) res[i][j] = 1
    } else {
        for (i in 0..3) for (j in i+1..3) res[i][j] = 1
    }
    return res
}

fun nonStrict(a: Int): Array<IntArray> {
    val res = Array<IntArray>(4){IntArray(4)}
    if (a == 0) {
        for (i in 0..3) for (j in 0..i) res[i][j] = 1
    } else {
        for (i in 0..3) for (j in i..3) res[i][j] = 1
    }
    return res
}
...
fun solve() {
    for (i in 1..2*SZ-1) for (j in 0..3) tran[i][j][j] = 1
    val (n,q) = io.ints(2)
    val a = ArrayList(io.ints(n-1))
    a.add(0) // lex greater
    fun recalc(i: Int) {
        if (i < 0) return
        if (a[i] == a[i+1]) {
