...
struct p{
	...
	int operator*(const p &B)const{return ((__int128)x*B.y-(__int128)y*B.x>=0)-1;}
};
p st[1000005];
int cc,top,id;
int k=0,b=0;
...
signed main()
{
	...
	if(op==1)
	{
		...
		p c=(p){cc,-cc*k-b};cc+=x;
		while(top>1&&(c-st[top-1])*(st[top]-st[top-1])>=0)top--;
		st[++top]=c;
	}else{
		b+=x;
		k+=read();
	}
	while(top>1&&f(st[top])>=f(st[top-1]))top--;
	// rg(top)printf("(%d %d) ",st[i].x,st[i].y);gr puts("");
	// printf("[%d %d]\n",k,b);
	oldp(st[top].x+1,f(st[top]));
	gr
	return 0;
}