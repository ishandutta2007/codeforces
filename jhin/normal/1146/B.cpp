...
int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000],c2[500000],dd[100];string s2;vector<int>v,vr,v2[100003];int sum,ans,num;char ch;
...
cin>>s;
for(int i=0;i<s.size();i++){if(s[i]!='a')x++;}
char tmp[x];
for(int i=0,j=0;i<s.size();i++)if(s[i]!='a')tmp[j]=s[i],j++;
if(x%2)return cout<<":(",0;
for(int i=0;i<x/2;i++)
{
   if(tmp[i]!=tmp[i+x/2])y=-1;
}
if(y!=-1)
{
    int tm=s.size();
tm-=x/2;
for(int i=tm;i<s.size();i++)if(s[i]=='a')return cout<<":(",0;
    for(int i=0;i<tm;i++)cout<<s[i];
}
else cout<<":(";
return 0;
}