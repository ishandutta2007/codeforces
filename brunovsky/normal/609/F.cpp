...
struct Segnode {
    ...
    static constexpr int MAX = INT_MAX / 3;
    int minimum = MAX;
    int lazy = MAX;
...
int main() {
    ...
    cin >> N >> M;
    vector<int64_t> xs;
    vector<pair<int64_t, int64_t>> frog(N), mosq(M);
    for (auto& [x, l] : frog) {
        cin >> x >> l;
        xs.push_back(x);
    }
    ...
    sort(begin(order), end(order),
         [&](int i, int j) { return frog[i].first < frog[j].first; });
    for (int i = 0; i < N; i++) {
        rank[order[i]] = i;
    }
    for (auto& [x, s] : mosq) {
    ...
    segtree<Segnode> seg(X, {});
    for (int i = 0; i < N; i++) {
        auto [x, l] = frog[i];
        int y = upper_bound(begin(xs), end(xs), xs[x] + l) - begin(xs);
        seg.update_range(x, y, rank[i]);
    }
    set<pair<int64_t, int>> onhold;
    vector<int> count(N);
    for (int j = 0; j < M; j++) {
        auto [p, s] = mosq[j];
        int i = seg.query_point(p).minimum;
        if (i == Segnode::MAX) {
            onhold.emplace(xs[p], j);
            continue;
        }
        i = order[i];
        auto& [x, l] = frog[i];
        l += s, count[i]++;
        auto pos = onhold.lower_bound({xs[x], 0});
        while (pos != end(onhold) && pos->first <= xs[x] + l) {
            auto [_, add] = mosq[pos->second];
            pos = onhold.erase(pos);
            l += add;
            count[i]++;
        }
        int y = upper_bound(begin(xs), end(xs), xs[x] + l) - begin(xs);
