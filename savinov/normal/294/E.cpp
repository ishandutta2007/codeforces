...
int dfs2(int v, int p, ll add = 0, int c = 0)
{
    dp[v] += add;
    cnt[v] += c;
    int ret = v;
    forn(i, g[v].size())
    {
        ...
        if (to != p)
        {
            int ind = dfs2(to, v, dp[v] - (ll)cnt[to] * len - dp[to] + (ll)(cnt[v] - cnt[to]) * len, cnt[v] - cnt[to]);
            if (dp[ind] < dp[ret])
                ret = ind;
        }
    }
...
int main()
{
    ...
    forn(i, n - 1)
    {
        dfs1(e[i].x.x, e[i].x.y);
        int q = dfs2(e[i].x.x, e[i].x.y);
        dfs1(e[i].x.y, e[i].x.x);
        int w = dfs2(e[i].x.y, e[i].x.x);
        ll res = 0;
        //cout << e[i].x.x << " " << e[i].x.y << endl;
        //cout << q << " " << w << endl;
        
        forn(j, n)
            res += dp[j];
        //cout << endl;
        res += 2 * dp[q] * cnt[w] + 2 * dp[w] * cnt[q] + 2ll * e[i].y * cnt[w] * cnt[q];
        remin(best, res / 2);
    }
    cout << best << endl;
    return 0;
