#line 1 "codeforces-workspace\\contest\\1572\\c\\c.cpp"
...
#line 13 "codeforces-workspace\\contest\\1572\\c\\c.cpp"
...
#line 30 "codeforces-workspace\\contest\\1572\\c\\c.cpp"

namespace workspace {

using etd = extended<int>;
constexpr auto inf = etd::infinity;
...
void main() {
  ...
  for (auto i : range(N)) {
    input a;
    --a;
    group[a].emplace_back(i);
    color.emplace_back(a);
  }

  ndvec dp({N + 1, N + 1}, inf);
  for (auto i : range(N + 1)) {
    dp[i][i] = 0;
  }

  for (auto R : range(N)) {
    for (auto L : range(R + 1)) {
      for (auto M : group[color[R]]) {
        if (M > R or M < L) continue;
        chle(dp[L][R + 1], dp[L][M] + dp[M][R] + (L != M));
      }
    }
  }
