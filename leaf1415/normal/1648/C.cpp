...
int main(void)
{
	...
	rep(i, 1, n) cnt[s[i]]++;
	rep(i, 1, min(n, m)) cnt2[t[i]]++;
	
	mint ans = 0, mul = 1;
	rep(i, 1, M) if(cnt[i] > 0) bit.add(i, fact[cnt[i]] * fact_inv[cnt[i]-1]);
	rep(i, 1, M) mul *= fact_inv[cnt[i]];
	
	bool flag = true;
	rep(i, 1, M) if(cnt[i] != cnt2[i]) flag = false;
	if(flag && n < m) ans += 1;
	
	rep(i, 1, min(n, m)){
		ans += bit.query(t[i]-1) * mul * fact[n-i];
		
		if(cnt[t[i]] == 0) break;
		mul *= fact[cnt[t[i]]];
		bit.add(t[i], fact[cnt[t[i]]] * fact_inv[cnt[t[i]]-1] * mint(-1));
		cnt[t[i]]--;
		mul *= fact_inv[cnt[t[i]]];
		if(cnt[t[i]] > 0) bit.add(t[i], fact[cnt[t[i]]] * fact_inv[cnt[t[i]]-1]);
	}
