...
void _solve() {
    ...
    using zal = pair<pair<int, int>, int>;
    vector<zal> a(n);
    cin >> a;

    zal mn = {{INT_MAX, 0}, INT_MAX};
    zal mx = {{0, INT_MIN}, INT_MAX};
    zal al = {{INT_MAX, INT_MIN}, INT_MAX};

    for(int i = 0; i < n; ++i) {
        if(a[i].x.x < mn.x.x || (a[i].x.x == mn.x.x && a[i].y < mn.y)) {
            mn = a[i];
        }

        if(a[i].x.y > mx.x.y || (a[i].x.y == mx.x.y && a[i].y < mx.y)) {
            mx = a[i];
        }

        if(mn.x.x < al.x.x || mx.x.y > al.x.y) {
            al = {{INT_MAX, INT_MIN}, INT_MAX};
        }

        if(a[i].x.x == mn.x.x && a[i].x.y == mx.x.y && a[i].y < al.y) {
            al = a[i];
        }

        if(al.y != INT_MAX && al.y < mn.y + mx.y) {
            cout << al.y << endl;
        } else {
            cout << mn.y + mx.y << endl;
        }
