...
int main() {
    ...
    while (M--) {
        ...
        if (c) {
            ...
            distincts.emplace_back(a, b);
        }
    }

    vector<int> roots;
    REP(i, N) roots.emplace_back(uf0.find(i));
    roots = sort_unique(roots);

    const int V = roots.size();

    UnionFind uf(V * 2);

    for (auto [a, b] : distincts) {
        if (uf0.same(a, b)) {
            puts("0");
            return 0;
        }
        a = arglb(roots, uf0.find(a)), b = arglb(roots, uf0.find(b));
        uf.unite(a, b + V);
        uf.unite(b, a + V);
    }
    ...
    REP(i, V) {
        ...
        cons.push_back(min(uf.find(i), uf.find(i + V)));
    }
    cons = sort_unique(cons);
    cout << mint(2).pow(cons.size() - 1) << '\n';
}