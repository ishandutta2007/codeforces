...
int main(){
	...
	cauto n=read();
	cauto c=readT(n,n,0ll);
	auto sums=table(n+1,n+1,0ll);
	rep(i,n)rep(j,n) sums[i+1][j+1]=c[i][j];
	rep(i,n+1)rep(j,n) sums[i][j+1]+=sums[i][j];
	rep(i,n)rep(j,n+1) sums[i+1][j]+=sums[i][j];
	cauto sum=
		[&](int li, int ri, int lj, int rj){
			if(li>=ri || lj>=rj) return 0ll;
#ifdef NUIP
			ll re=0;
			reps(i,li,ri)reps(j,lj,rj) re+=c[i][j];
			assert(re==sums[ri][rj]+sums[li][lj]-sums[ri][lj]-sums[li][rj]);
#endif
			return sums[ri][rj]+sums[li][lj]-sums[ri][lj]-sums[li][rj];
		};
	cauto get=
		[&](int l, int r){
			return sum(l,r,0,l)+sum(l,r,r,n);
		};
	cauto INF=(ll)1e18;
	auto dp=table(n+1,n+1,INF);
	auto prv=table(n+1,n+1,0);
	rep(i,n+1)rep(j,n+1)if(j-i<=1) dp[i][j]=0;
	out(dp,1);
	reps(d,2,n+1)rep(l,n+1-d){
		cauto r=l+d;
		reps(m,l,r)if(MN(dp[l][r], dp[l][m]+dp[m+1][r]+get(l,m)+get(m+1,r))) prv[l][r]=m;
	}
	out(dp,prv,1);
	vector<int> re(n);
	cauto rec=
		[&](auto &&rec, int l,int r, int p)->void{
			if(l+1==r) re[l]=p;
			if(r-l<=1) return;
			cauto &m=prv[l][r];
			re[m]=p;
