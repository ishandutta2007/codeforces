...
struct qwq{
	std::vector<int>v;
	int w;
	bool operator==(const qwq&b)const{return v==b.v;}
}a[200005];
std::map<int,int>mp;
int cc;
// bitset< kong jian fu za du bu dui,
signed main()
{
	initprog();
	int n=read(),m=read();
	rg(n)rg_(j,m)a[i].v.push_back(read());gr a[i].w=read();gr
	// std::sort(a+1,a+1+n,[&](qwq x,qwq y){return x.v==y.v?x.w<y.w:x.v<y.v;});
	// // n=std::unique(a+1,a+1+n)-a-1;
	// int ccc=0;
	// rg(n)if(i==1||!(a[i]==a[i-1]))a[++ccc]=a[i];gr
	// n=ccc;
	std::sort(a+1,a+1+n,[&](qwq x,qwq y){return x.w<y.w;});
	// rg(n)
	// for(auto &x:a[i].v)
	// {
	...
	int ans=2147483647;
	rg(T)
	int r=i*B,l=r-B+1;if(r>n)r=n;
	if(a[l].w+a[l+1].w<ans&&l<r)
	{
		unordered_map<int,std::bitset<B> >mp;
		// unordered_map<int,int>mp2;
		for(int i=l;i<=r;i++)for(int j=0;j<m;j++)mp[a[i].v[j]].set(i-l);
		// for(int i=l;i<=r;i++)for(int j=0;j<m;j++)if(mp2[a[i].v[j]]==0)mp2[a[i].v[j]]=1,mp[a[i].v[j]]=~mp[a[i].v[j]];
		for(int i=l;i<=n;i++){
		if(a[i].w+a[l].w>=ans)break;
		ok.reset();
		if(r==n)
		{
			for(int i=l;i<=r;i++)ok.set(i-l);
		}
		else ok.set();
		int f=0;
		for(int j=0;j<m;j++)
		...
		if(f==0)ans=min(ans,a[i].w+a[l+(ok._Find_first())].w);//odp(i,l+(ok._Find_first()));
		gr
	}
	// for(auto )
	gr
	...
	// if(comp(a[i],a[j]))
	// gr gr
	if(ans==2147483647)puts("-1");else odl(ans);
