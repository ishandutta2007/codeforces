...
fn dfs(u: usize, par: usize, e: &[Vec<usize>], a: &[(i32,usize)], a0: (i32,usize), b: i32) -> Z {
	assert!(a[u] <= a0 && a[u].0 >= b);
	let mut res = Z1;
	for &v in e[u].iter() {
		if v != par && a[v] <= a0 && a[v].0 >= b {
			res *= Z1 + dfs(v, u, e, a, a0, b);
		}
	}
	res
}
