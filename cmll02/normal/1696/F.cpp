...
int check()
{
	...
	rg(n)rg_(j,n)D[i][j]=inf;gr gr
	rg(n)D[i][i]=0;gr
	for(auto x:ans)
	{
		D[x.first][x.second]=1;
		D[x.second][x.first]=1;
	}
	rg_(k,n)rg(n)rg_(j,n)mne(D[i][j],D[i][k]+D[k][j]);gr gr gr
	rg(n)rg_(j,i-1)rg_(k,n)
	if((D[i][k]==D[j][k])!=s[i][j][k])return 0;
	gr gr gr
	return 1;
}
int fa[1<<22],sz[1<<22];
std::set<int>ss[155555];
...
inline int uf(int x,int y)
{
    ...
    for(auto p:ss[fx])ss[fy].insert(p);
    ss[fx].clear();
    return 1;
}
int f2(int x,int y)
...
signed main()
{
	...
	int T=read();rg(T)
	n=read();
	rg(n*n)fa[i]=i,sz[i]=1;ss[i].clear();gr
	
	rg(n)rg_(j,i-1)int o=f2(i,j);ss[o].insert(i);ss[o].insert(j);gr gr
	
	rg(n)
	rg_(j,n-i)rg_(k,n)s[i][i+j][k]=s[i+j][i][k]=re1d();
	if(s[i][i+j][k])uf(f2(i+j,k),f2(i,k));
	gr gr
	gr
	ans.clear();
	rg(n)rg_(j,i-1)
	int o=f2(i,j);
	if(fa[o]==o&&sz[o]==n-1&&ss[o].size()==n)
	{
		ans.clear();
		rg_(j,n)rg_(k,j-1)
		if(f(f2(j,k))==o)ans.push_back(mp(j,k));
		gr gr
		
		if(!check())goto end2;
		else {
			puts("Yes");
			for(auto x:ans)odp(x.first,x.second);
			goto end1;
		}
	}
	end2:;
	...
	// if(s[i][j][k])b.set(k);
	// gr
	// if(b.count()>=1)v.push_back(mp(b.count(),b));
	// gr gr
// 	
