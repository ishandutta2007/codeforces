...
void make_op(const vl& op, unordered_set<ll>& u, vl& v, vvl& ans)
{
	if (u.count(op.back()) == 0)
	{
		ans.pb(op);
		u.insert(op.back());
		v.pb(op.back());
...
void divide(const ll d, ll e, ll& n, ll& m, unordered_set<ll>& u, vl& v, vvl& ans)
{
	ll ddd = d;
	while (2 * ddd < e)
	{
		make_op({ ddd, ddd, 0, ddd * 2 }, u, v, ans);
		ddd *= 2;
	}
	while ((ddd >= d) && e)
	{
		if (__builtin_clzll(ddd) == __builtin_clzll(e))
		{
			ll nn = e ^ ddd;
			make_op({ ddd, e, 1, nn }, u, v, ans);
			e = nn;
			if (e != 0)
				m = min(m, e);
			if (e & 1)
...
vvl lol(ll n)
{
	...
	while (nn > n)
	{
		...
		while (2 * ddd < n)
		{
			make_op({ ddd * n, ddd * n, 0, ddd * 2 * n }, u, v, ans);
			ddd *= 2;
		}
		make_op({ ddd * n, n, 0, (ddd + 1) * n }, u, v, ans);
		++ddd;
		ll m = n;
		ll mm = 1e18;
		while (mm > m)
	...
	if (u.count(1) == 0)
	{
		while (true)
		{
			...
			for (int i = 0; i < sz(v); ++i)
				...
				{
					vl aa = { v[i], v[j], 0, v[i] + v[j] };
					if (aa.back() < a.back() && u.count(aa.back()) == 0)
						a = aa;
