...
ll get(const vector<ll> &p, int l, int r) {
    if (l > r) return 0;
...
void solve() {
    ...
    rsort(a);
    ll fst = 0;
    while (fst < n && a[fst] > m) ++fst;
    reverse(a.begin() + fst, a.end());
    auto pref = a;
    for (ll i = 1; i < n; ++i) pref[i] += pref[i - 1];
    ll ans = 0;
    for (ll cnt = 0; cnt <= n && (n - 1) / (d + 1) >= cnt - 1; ++cnt) {
        ll lst = max(0ll, (cnt - 1) * (d + 1) + 1);
        chmax(ans, get(pref, 0, cnt - 1) + get(pref, max(fst, lst), n - 1));
    }
    println(ans);
}
