...
void check(int L,int R){
  ...
  for(int i=sz(G)-1;i>=0;--i){
    ...
    for(pii p:G[i]){
      ...
      assert(p.X>i);
      W.pb(p.X);
      for(int j:C[p.X]) C[i].pb(j+p.Y);
    }
    sort(all(W));
    assert(unique(all(W))==W.end());
    sort(all(C[i]));
    assert(unique(all(C[i]))==C[i].end());
  }
  assert(sz(C[0])==R-L+1);
  assert(C[0][0]==L);
  assert(C[0].back()==R);
...
int main(int argc, char **argv){
  ...
  auto doit=[&](int v0,int R,int add){
              G[v0].pb(mp(31,1+add));
              ++m;
              --R;
              forn(i,32){
                if(!(R&(1<<i))) continue;
                assert(30-i>v0);
                dbg(v0,R,add,i,((R>>(i+1))<<(i+1)));
                G[v0].pb(mp(30-i,((R>>(i+1))<<(i+1))+1+add));
                ++m;
              }
            };
  forn(i,20) doit(30-i,1<<i,0);
