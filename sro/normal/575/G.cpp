...
int n,m;
vector<int> nei[10][100007];
int zdist[100007],zpre[100007];
int pre[100007],pred[100007];
queue<int> q;
...
int main()
{
	...
	while(1)
	{
		...
		{
			bool had=0;
			for(pair<int,bool> j:vq)
			{
				if(!j.second)had=0;
				for(int k:nei[i][j.first])if(pre[k]==-1)
				{
					pre[k]=j.first;
					pred[k]=i;
					nq.push_back(mp(k,had));
					had=1;
				}
			}
		}
		...
		swap(nq,vq);
		int best=-1;
		for(pair<int,bool> i:vq)
		{
			if(best!=-1&&!i.second)break;
			if(zdist[i.first]!=Inf&&(best==-1||zdist[best]>zdist[i.first]))
				best=i.first;
		}
		if(best!=-1)
		{
			...
			puts("");
			reverse(ALL(ans));
			for(int i=best;i!=n-1;i=zpre[i])
				ans.push_back(zpre[i]);
			printf("%d\n",SZ(ans));
