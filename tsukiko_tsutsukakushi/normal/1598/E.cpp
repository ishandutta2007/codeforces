...
void solve() {
    ...
    rep(i, q) {
        ...
        for(int k = -1; k <= 0; ++ k) {
            unless(ok(x - y + k) and ok(x - y + k + 1)) {
                tmp ++;
                continue;
            }
            ...
            if(left < 0) left = 0;
            if(right < 0) right = 0;
            if(k == -1 and se[f(x - y + k)].find(x) != se[f(x - y + k)].end()) right = 0;
            if(k == 0 and se[f(x - y + k + 1)].find(x) != se[f(x - y + k + 1)].end()) left = 0;
            debug(k, left, right);
            // assert(left >= 0);
            // assert(right >= 0);
            tmp += (left + 1) * (right + 1);
