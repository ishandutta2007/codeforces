...
{
    int t,l,r;
    inline void Read(){rin>>t>>l>>r;return;}
    inline bool operator <(gyq y){return t<y.t;}
}a[N];
inline void work()
{
    rin>>n>>m;for(int i=1;i<=n;i++)a[i].Read();sort(a+1,a+n+1);
    int l,r,t;l=r=m;t=0;
    for(int i=1,j;i<=n;i=j)
    {
        int nl=a[i].l,nr=a[i].r;
        for(j=i+1;j<=n&&a[j].t==a[i].t;j++){nl=max(nl,a[j].l);nr=min(nr,a[j].r);}
        if(nl>nr){puts("NO");return;}
        l-=(a[i].t-t);r+=(a[i].t-t);l=max(l,nl);r=min(r,nr);
        if(l>r){puts("NO");return;}t=a[i].t;
        // printf("[%d,%d] j:%d nl:%d nr:%d\n",l,r,j,nl,nr);
    }
    puts("YES");
    return;
}
