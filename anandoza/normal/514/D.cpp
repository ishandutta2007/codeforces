...
int main() {
    ...
    auto combine = [&](const T &u, const T &v) {
        T ans{};
        for (int i = 0; i < m; i++)
            ans[i] = max(u[i], v[i]);
        return ans;
    ...
    auto init = L1(i, a[i]);
    sparse_table<T, decltype(combine)> st(n, combine, init);
    ...
    auto can = [&](int len) -> T {
        ...
        for (int l = 0; l + len <= n; l++) {
            auto cost = st.accumulate(l, l + len);
            opt = min(opt, cost, L2(u, v, accumulate(all(u), 0LL) < accumulate(all(v), 0LL)));
        }
        return opt;
    };

    int len = -1 + binarysearch(
                       [&](int len) {
                           auto ans = can(len);
                           return accumulate(all(ans), 0LL) > k;
                       },
                       0, n + 1);
