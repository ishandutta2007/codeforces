...
int main() {
   ...
   for (int cur: ord) {
      ...
      for (int nxt: ch[cur]) {
         if (ans[nxt]) continue;
         if (ptr[cur] == -1 || sz(buf[ptr[nxt]]) > sz(buf[ptr[cur]])) {
            ptr[cur] = ptr[nxt];
         }
      }
      pp(ptr[cur]);
      if (ptr[cur] == -1) {
         ptr[cur] = sz(buf);
         buf.push_back(set<int>());
         offset.push_back(0);
      }
      buf[ptr[cur]].insert(offset[ptr[cur]]);
      for (int nxt: ch[cur]) {
         if (ans[nxt]) continue;
         if (ptr[nxt] == ptr[cur]) continue;
         for (int i: buf[ptr[nxt]]) {
            if (buf[ptr[cur]].count(i ^ offset[ptr[nxt]] ^ offset[ptr[cur]] ^ v[cur])) {
               ans[cur] = 1;
            }
         }
         for (int i: buf[ptr[nxt]]) {
            buf[ptr[cur]].insert(i ^ offset[ptr[nxt]] ^ offset[ptr[cur]]);
         }
      }
      if (buf[ptr[cur]].count(offset[ptr[cur]] ^ v[cur])) {
         ans[cur] = 1;
      }
      pp(offset[ptr[cur]], v[cur]);
      offset[ptr[cur]] ^= v[cur];
   }
   int res = 0;
   for (int i: ans) res += i;
   cout << res << "\n";
