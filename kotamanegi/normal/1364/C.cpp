...
void solve(){
    ...
    int verifyitr = 0;
    REP(i, n) {
        if (checkitr != inputs[i]) {
            check.insert(checkitr);
            verify.insert(checkitr);
            ans.push_back(checkitr);
            while (check.count(checkitr) == 1) {
                checkitr++;
            }
            if (checkitr != inputs[i]) {
                cout << -1 << endl;
            ...
            while (verify.count(verifyitr) == 1) {
                verifyitr++;
            }
            ans.push_back(verifyitr);
            check.insert(verifyitr);
            verify.insert(verifyitr);
        }
    }
    REP(i, n) {
        cout << ans[i] << " ";
