#line 1 "codeforces-workspace\\contest\\1539\\e\\e.cpp"
...
#line 11 "codeforces-workspace\\contest\\1539\\e\\e.cpp"
...
void main() {
  ...
  input<int> n, m;
  input<vector<pair<int, array<P, 2>>>> queries(n);

  auto in = [&](auto x, auto p) -> auto {
    return x >= p.first && x <= p.second;
  };
  auto cap = [&](auto &p, auto q) -> auto {
    chgr(p.first, q.first);
    chle(p.second, q.second);
  };

  array<P, 2> allowed = {P{0, m}, {0, m}};
  vector<array<int, 2>> post(n);
  P banned{m + 1, 0};

  for (auto &&[nxt, query] : reversed(zip(post, queries))) {
    auto [k, cond] = query;
    auto tmp = cond;

    for (auto i : {0, 1}) {  // which to choose
      if (in(k, cond[i])) {
        if (in(k, allowed[i])) {
          nxt[i] = !i;
        } else {
  ...
  auto hand = in(0, allowed[0]);
  if (!in(0, allowed[!hand])) eject("No");
  cout << "Yes"
       << "\n";
  for (auto &&a : post) {
    cout << hand << " ";
