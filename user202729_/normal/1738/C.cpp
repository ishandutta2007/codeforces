// didn't think through very carefully but not much point
...
bool const ALICE=false, BOB=true;
auto result=[]{
	std::array<std::array<std::array<std::array<bool, 2>, 2>, 101>, 101> result;
	// [count 0 left][count 1 left][next player][parity of all items picked by Alice]  winner
	for(int const count0: range(0, int(result.size()))){
		for(int const count1: range(0, int(result[count0].size()))){
			for(int const next: range(0, 2)){
				for(int const parity: range(0, 2)){
					bool& r=result[count0][count1][next][parity];
					if(count0==0 and count1==0){
						r=parity==0 ? ALICE: BOB;
					}else{
						r=not next;
						if(count0>0
								and
								result[count0-1][count1][not next][parity]==next){
							r=next;
						}
						if(count1>0
								and
								result[count0][count1-1][not next][parity xor (next==ALICE)]==next){
							r=next;
						}
					}
				}
...
void up(){
	...
	std::cout<<(
		result[count[0]][count[1]][ALICE][0]==ALICE
		? "Alice\n": "Bob\n");
