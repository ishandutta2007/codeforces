...
int main(){
	...
	while(std::all_of(begin(aa),end(aa),[&](auto xy){
				return ((xy.first+xy.second)&1)==
					((aa[0].first+aa[0].second)&1);
				}))
		for(auto& xy:aa){
			auto [x,y]=xy;
			xy={(x+y)>>1,(x-y)>>1};
	...
	std::vector<int> a(n);
	std::transform(begin(aa),end(aa),begin(a),[](auto xy){return (xy.first+xy.second)&1;});

	io::writeInt(std::count_if(begin(a),end(a),[=](int x){return x;}));
	io::writeChar('\n');
