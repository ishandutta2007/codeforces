...
fun main(){
    solve.cases{
        val n = getint
        val level = ceil(sqrt(n.toDouble())).toInt()
        val want = BooleanArray(n){true}
        for(i in 0 until n step level ){
            want[i] = false
        }
        ...
        fun act(a:List<Int>){
            ...
            if((0 until n).all{!want[it] || active[it]}){
                ok = true
            }
            if(a.isEmpty()){
                return
            }
            println("${a.size} ${a.map{it+1}.conca()}")
            System.out.flush()
