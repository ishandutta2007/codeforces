...
void solve() {
  ...
  vc<mint> G = {1, 1};
  FOR(_, 1LL << 19) {
    F.eb(F[F.size() - 2] + F[F.size() - 1]);
    G.eb(G[G.size() - 2] + G[G.size() - 1]);
  }
  vc<mint> Fc = {0}, Gc = {0};
  FOR(i, F.size()) Fc.eb(Fc.back() + F[i]);
  FOR(i, G.size()) Gc.eb(Gc.back() + G[i]);

  // operator (a,b) := aF[i] + bG[i] 
  using E = tuple<ll, ll, mint>;
  ...
  SegmentTree<E, OP> seg(
    [&](E x, E y) -> E {
      ...
      if (lx == -1) return y;
      if (ly == -1) return x;
      return mt(lx, ry, sx + sy);
    },
    [&](E x, OP y) -> E {
      auto [lx, rx, sx] = x;
      auto [a, b] = y;
      return mt(lx, rx, sx + a * (Fc[rx] - Fc[lx]) + b * (Gc[rx] - Gc[lx]));
    },
    [&](OP x, OP y) -> OP { return mp(x.fi + y.fi, x.se + y.se); },
    E({-1, -1, 0}), OP({0, 0}));
  seg.init(N);
  FOR(i, N) seg.set(i, mt(i, i + 1, A[i]));

  FOR(q, Q) {
    LL(t, L, R);
    --L;
    if (t == 2) {
      auto [l, r, x] = seg.fold(L, R);
      print(x);
    }
    elif (t == 1) {
      mint det = F[L] * G[L + 1] - F[L + 1] * G[L];
      mint a = (G[L + 1] - G[L]) / det;
      mint b = (F[L] - F[L + 1]) / det;
      seg.operate_range(L, R, OP({a, b}));
    }
  }
}
