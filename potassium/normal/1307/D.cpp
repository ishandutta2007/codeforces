...
int main() {
	...
	vector<int> distu(n + 1, INF), distv(n + 1, INF);
	bfs(1, distu);
	bfs(n, distv);

	vector<pii> spec;
	for (int i = 0; i < k; i++)
		spec.pb({distu[a[i]], distv[a[i]]});

	sort(all(spec), [](pii a, pii b){return a.F - a.S < b.F - b.S;});
	
	int suff = spec[k - 1].S;
	int tmp = 0;
	for (int i = k - 2; i >= 0; i--) {
		chkmax(tmp, spec[i].F + suff + 1);
		chkmax(suff, spec[i].S);
	}
	int ans = distu[n];
