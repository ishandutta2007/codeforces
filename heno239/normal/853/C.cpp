...
void solve() {
	...
	rep(aa, q) {
		...
		ans -= (ll)x1*(x1 - 1) / 2;
		ans -= (ll)y1*(y1 - 1) / 2;
		ans -= (ll)(n - 1 - x2)*(n - 1 - x2 - 1) / 2;
		ans -= (ll)(n - 1 - y2)*(n - 1 - y2 - 1) / 2;
		//cout << ans << "\n";
		ll c = b.query(0, 0, x1, y1);
		//cout << "!? " << c << "\n";
		ans += c * (c - 1) / 2;
		c = b.query(0, y2 + 1, x1, n);
		//cout << "!? " << c << "\n";
		ans += c * (c - 1) / 2;
		c = b.query(x2 + 1, 0, n, y1);
		//cout << "!? " << c << "\n";
		ans += c * (c - 1) / 2;
		c = b.query(x2 + 1, y2 + 1, n, n);
		//cout << "!? " << c << "\n";
		ans += c * (c - 1) / 2;
		cout << ans << "\n";
	}
