...
void solve() {
  ...
  vvc<T> query(LIM + 1);
  FOR(i, N) {
    for (auto&& [p, e]: factor(A[i])) { query[p].eb(0, i, e); }
  }
  FOR(q, Q) {
    LL(idx, x);
    --idx;
    for (auto&& [p, e]: factor(x)) { query[p].eb(q, idx, e); }
  }

  vc<int> C(N);
  SegTree<Monoid_Min<int>> seg(C);
  vc<mint> ANS(Q, mint(1));

  FOR(p, LIM + 1) {
    if (len(query[p]) < N) continue;
    ll now = 0;
    for (auto&& [qid, i, e]: query[p]) {
      C[i] += e;
      seg.set(i, C[i]);
      ll nxt = seg.prod_all();
      ANS[qid] *= mint(p).pow(nxt - now);
      now = nxt;
    }
    for (auto&& [qid, i, e]: query[p]) {
      C[i] = 0;
