...
void work(){
	...
	sort(seg.begin(),seg.end(),[&](pii a,pii b){
		if(a.fi==1) return false;
		if(b.fi==1) return true;
		if(a.se==n) return false;
		if(b.se==n) return true;
		return a.se-a.fi<b.se-b.fi;
	});
	for(auto g:seg){
		if(!k) break;
		if(g.fi==1){
			for(int i=g.se;i>=g.fi;--i){
				a[i]=1,--k;
				if(!k) break;
			}
		}else{
			for(int i=g.fi;i<=g.se;++i){
				a[i]=1,--k;
				if(!k) break;
			}
		}
	...
	int su=0;
	for(int i=1;i<=n;++i) if(a[i]) su+=a[i]+a[i-1];
	printf("%d\n",su);
}
int main(){
#ifndef ONLINE_JUDGE
