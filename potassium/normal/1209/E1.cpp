...
int main() {
	...
	while (t--) {
		...
		vector<vector<int>> dp(m+7,vector<int>(16+7,0));
		dp[0][0] = 0;
		for (int i = 1; i <= m; i++) {
			for (int mask = 0; mask < (1<<n); mask++) {
				for (int fmask = mask; fmask < (1<<n); fmask++) {
					if ((fmask & mask) != mask) continue;
					int diff = fmask - mask;
					for (int rot = 0; rot < n; rot++) {
						...
						dp[i][fmask] = max(dp[i][fmask],sum+dp[i-1][mask]);
						//W(i,fmask,dp[i][fmask]);
					}
				}
			}
		}
