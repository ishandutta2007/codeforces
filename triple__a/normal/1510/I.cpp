...
int n,m,mn=0,myinc=0;

int c[maxn];
string s;

double solve(int u,int w,int mn,double rem){
    double rate=5*exp(-rem)*(w+1)/(mn+1);
    return exp(-rate*u);
}
signed main(){
    ...
    rep(i,m){
        ...
        int d[2]={0,0};
        rep(j,n){
            if (1.3*c[j]<=myinc) d[s[j]-'0']+=(int)(100*(myinc-1.3*c[j]));
        }
        if (!d[0]&&!d[1]) d[0]++,d[1]++;
        ...
        int w;
        cin>>w;
        rep(j,n) if (s[j]-'0'!=w) c[j]++;
        if ((ret>=d[0])!=w) myinc++;
        mn=myinc;
        rep(j,n) mn=min(mn,c[j]); 
    }
    return 0;
}