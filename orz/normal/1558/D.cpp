...
int C(const int n, const int k)
{
	...
	if (fact.empty())
	{
		fact.resize(C, 1);
		for (int i = 2; i < C; ++i)
			fact[i] = prod(i, fact[i - 1], M);

		ifact.resize(C, 1);
		ifact[C - 1] = inverse(fact[C - 1], M);
		for (int i = C - 1; i >= 1; --i)
			ifact[i - 1] = prod(ifact[i], i, M);
	}
	return prod(fact[n], prod(ifact[k], ifact[n - k], M), M);
}
struct node;
typedef node* nd;
struct node
{
	int x, p, s;
	nd l, r;
	node(const int x) : x(x), p(0), s(1), l(nullptr), r(nullptr)
	{
	}
	pair<nd, nd> splt(const int g);
	void psh()
	{
		if (p)
		{
...
pair<nd, nd> split(nd v, const int g)
{
	return v ? v->splt(g) : make_pair(nullptr, nullptr);
}
pair<nd, nd> node::splt(const int g)
{
	psh();
	if (x >= g)
	{
		pair<nd, nd> pr = split(l, g);
		l = pr.Y;
		return make_pair(pr.X, fix_size());
	}
	else
	{
		pair<nd, nd> pr = split(r, g);
		r = pr.X;
		return make_pair(fix_size(), pr.Y);
	}
}
nd merge(nd l, nd r)
{
...
void add(nd x, int value)
{
	if (x)
		x->p += value;
}
nd insert(nd abc, int value)
{
	pair<nd, nd> ABC = split(abc, value - 1);
	nd a = ABC.X;
	nd bc = ABC.Y;
	add(bc, 1);
	pair<nd, nd> BC = split(bc, value + 1);
	nd b = BC.X;
	nd c = BC.Y;
	if (!b)
		b = new node(value);
...
int find_ans(const int n, const vi& v)
{
	int b = find_b(v);
	return C(2 * n - 1 - b, n);
}
