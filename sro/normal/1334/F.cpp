...
&&@@&&&&&&&$$$$&@@&&&@&@#####@@&&&@@&$%|!!;;;::::::::::::::;;;;;;;:;::::::;$###########@$%$&$%$$$$$%|;````:::::::::::::''''':
&&@@&&&&&&$$$$$&@@&&&&&&######@&&&&&&$%|!!;;;:::::::::::::::::::::::::::::|@#########@@&%;!&$%$$$$$$|;'`````'':::''::::''''':
Coded by Natsu Kinmoe, a 21-yr-old beautiful girl.
*/
#include <bits/stdc++.h>
using namespace std;
...
int n,m;
ll a[500005],b[500005],p[500005];

struct segtree{
	ll dt[2000005],wp[2000005];
	void pushup(int now){
	...
	void add(int l,int r,ll x,int now=1,int nl=0,int nr=m){
		if(nl>=l&&nr<=r){
			dt[now]+=x;
			wp[now]+=x;
			return;
	...
	ll get(int to,int now=1,int nl=0,int nr=m){
		if(nl==nr){
			return dt[now];
		}
		pushdown(now);
		int m=(nl+nr)>>1;
		if(m>=to){
			return get(to,now<<1,nl,m);
		}else{
			return get(to,now<<1|1,m+1,nr);
		}
	}
	void change(int to,ll x,int now=1,int nl=0,int nr=m){
		if(nl==nr){
			dt[now]=x;
			wp[now]=0;
			return;
...
int main(){
	...
	cont(i,n){
		...
		if(b[pos]==a[i]){
			seg.add(pos,m,p[i]<0?p[i]:0);
			ll lst=seg.get(pos-1),now=seg.get(pos);
			if(lst<now){
				seg.add(pos,pos,lst-now);
			}
			seg.add(0,pos-1,p[i]);
		}else{
			seg.add(pos,m,p[i]<0?p[i]:0);
