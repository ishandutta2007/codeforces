...
int main() {
	...
	const ll sum = accumulate(all(a), 0);
	vector<vector<double>> ways(n + 1, vector<double>(sum + 1, 0));
	ways[0][0] = 1;

	for (auto& i : a)
		for (int sz = n - 1; sz >= 0; sz--)
			for (int val = 0; val + i <= sum; val++)
				ways[sz + 1][val + i] += ways[sz][val];

	vector<double> plog(n + 1, 0);
	for (int i = 2; i <= n; i++)
		plog[i] = plog[i - 1] + log2(i);

	double ans = 0;
	for (int sz = 0; sz < n; sz++) {
		for (int val = 0; val <= sum; val++) {
			double buy = 1.0 * (sum - val) / (n - sz);
			double random = (1.0 * n / (n - sz) + 1) * x / 2;

			ans += 1.0 * ways[sz][val] * pow(2, plog[sz] + plog[n - sz] - plog[n]) * min(buy, random);
		}
	}
