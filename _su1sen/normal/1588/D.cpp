...
string solve(int n, vector<vector<int>> s) {
    vector<int> sz(n);
    rep(i, n) sz[i] = s[i].size();
    vector nxt(n, vector(52, vector<int>{}));
    rep(i, n) rep(c, 52) {
        nxt[i][c].resize(sz[i] + 1, sz[i]);
    }
    rep(i, n) {
        int si = sz[i];
        rrep(j, si) {
            rep(c, 52) nxt[i][c][j] = nxt[i][c][j + 1];
            nxt[i][s[i][j]][j] = j;
        }
    }
    map<vector<int>, pair<vector<int>, char>> trans;
    map<vector<int>, int> dp;
    auto dfs = [&](auto dfs, vector<int> idx) {
        if (auto it = dp.find(idx); it != dp.end()) return it->second;
        int len = 0;
        vector<int> nidx = idx;
        int l0 = idx[0], s0 = sz[0];
        rep(i0, l0, s0) {
            bool valid = true;
            nidx[0] = i0 + 1;
            int c = s[0][i0];
            rep(i, 1, n) {
                nidx[i] = nxt[i][c][idx[i]];
                if (nidx[i] == sz[i]) {
                    valid = false;
                    break;
                }
            ...
            if (valid and chmax(len, 1 + dfs(dfs, nidx))) {
                trans[idx] = { nidx, c };
            }
        }
        return dp[idx] = len;
