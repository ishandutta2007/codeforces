...
ll solve()
{
	...
	rep(i, 0, n){
		rep(j, 0, n){
			if(i > j) continue;
			ll lb = sum[i].fi, ub = sum[i].se;
			ll l = (sum[j]-sum[i]).fi, r = (sum[j]-sum[i]).se;
			ll L = -(sum[n]-sum[j]).se, R = -(sum[n]-sum[j]).fi;
			chmax(lb, L-r), chmin(ub, R-l);
			if(lb > ub) continue;
			chmax(ret, min(R, lb+r)-lb+1);
		}
	}
	return ret;
}
