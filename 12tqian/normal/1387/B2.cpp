...
int main() {
    ...
    vi dep(n);
    ll res = 0;
    function<void(int, int, int)> make_bag = [&](int src, int par, int id) {
        res += 2 * dep[src];
        if (id != -1)
            bag[id].eb(src);
        for (int nxt : adj[src]) {
    ...
    set<pi> use;
    f0r(i, n) {
        if (sz(bag[i]) == 0) continue;
        use.emplace(sz(bag[i]), i);
    }
    vpi pairs;
    vi ans(n);
    while (sz(use) >= 2) {
        ...
        pairs.eb(xx, yy);
        if (sz(bag[xid])) 
            use.emplace(sz(bag[xid]), xid);
        if (sz(bag[yid]))
            use.emplace(sz(bag[yid]), yid);
    }
    if (sz(use) == 1) {
        int id = (*use.begin()).s;
        pairs.eb(c, bag[id].back());
        bag[id].pop_back();
    } else {
