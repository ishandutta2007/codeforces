...
struct dsu {
	...
	bool uni(int u, int v) {
		...
		vector<int> mer = { maxv[u], maxv[v], smaxv[u], smaxv[v] };
		sort(rall(mer));
		maxv[u] = mer[0];
		smaxv[u] = mer[1];
...
int main() {
	...
	vector<int> num;
	for (auto& i : a) {
		num.emplace_back(i.fi);
		num.emplace_back(i.se);
	}
	distinct(num);

	for (auto& i : a) {
		i.fi = lower_bound(all(num), i.fi) - num.begin();
		i.se = lower_bound(all(num), i.se) - num.begin();
	}
	...
	vector<int> cir;
	for (auto& i : a) {
		if (!dsu.uni(i.fi, i.se))
			cir.emplace_back(i.fi);
	}
	...
	for (int u = 0; u < m; u++) {
		...
		if (mark[u] >= 2) can = 0;
		if (mark[u] == 1) chkmax(ans, dsu.maxv[u]);
		if (mark[u] == 0) chkmax(ans, dsu.smaxv[u]);
	}
