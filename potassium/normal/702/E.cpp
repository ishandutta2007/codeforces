...
int main() {
	ll n,k; R(n,k);
	const int MAXH = 35;
	vector<vector<ll>> to(MAXH, vector<ll>(n)); R(to[0]);
	vector<vector<ll>> w(MAXH, vector<ll>(n)); R(w[0]);
	vector<vector<ll>> minw = w;

	for (int i = 1; i < MAXH; i++) {
		for (int u = 0; u < n; u++) {
			int mid = to[i - 1][u];
			to[i][u] = to[i - 1][mid];
			w[i][u] = w[i - 1][mid] + w[i - 1][u];
			minw[i][u] = min(minw[i - 1][mid], minw[i - 1][u]);
		}
	}

	for (int i = 0; i < n; i++) {
		ll sum = 0, minv = LLONG_MAX;
		int now = i;
		for (int j = 0; j < MAXH; j++) {
			if (k & (1LL << j)) {
				chkmin(minv, minw[j][now]);
				sum += w[j][now];
				now = to[j][now];
			}
		}
