...
int a[N];
void calc(char i){while(a[i])cout<<i,--a[i];}
void solve()
{
    for(char x:read<string>())++a[x];
    if(read<string>()=="abc"&&a['a'])calc('a'),calc('c'),calc('b');
    else calc('a'),calc('b'),calc('c');
    for(char i='d';i<='z';++i)calc(i);
    cout<<'\n';
}
int main()
{
