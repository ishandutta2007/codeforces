...
struct Binomial {
    ...
    static T zero_layout(int n, int k) {
        return k == 0 ? n == 0 : choose(n + k - 1, k - 1);
    }

    // Layout n identical items over k distinct bins, >=1 per bin
    static T one_layout(int n, int k) { return k == 0 ? n == 0 : choose(n - 1, k - 1); }
...
// dp[n][p][b]: count number of partitions of n into p parts, each at most b
// O(5000*100) is too much

// Count partitions: we have n, a number of parts p, and an exclusive upper bound b
num get(int n, int p, int b) {
    num ans = 0;
    for (int k = 0; k <= p; k++) {
        ans += (k % 2 ? -1 : +1) * B::choose(p, k) * B::zero_layout(n - k * b, p);
    }
    return ans;
}
...
int main() {
    ...
    B::ensure_factorial(5000);
    int n, s, r;
    cin >> n >> s >> r;
    num win = 0, ways = B::zero_layout(s - r, n);
    for (int v = r; v <= s; v++) {
        for (int c = 1; c <= n && c * v <= s; c++) {
            num a = get(s - v * c, n - c, v) * B::choose(n - 1, c - 1);
            win += a / c;
        }
    }
    cout << win / ways << '\n';
