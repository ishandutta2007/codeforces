...
const int N = 200 * 1001 + 57;
ll t[4 * N];
ll sv[4 * N];

void build(vector<int> &a, int curl, int curr, int curv) {
	if (curr - curl == 1) {
		t[curv] = a[curl];
		return;
	}
	int m = (curl + curr) >> 1;
	build(a, curl, m, curv << 1);
	build(a, m, curr, (curv << 1) | 1);

	t[curv] = min(t[curv << 1], t[(curv << 1) | 1]);
}


void upd(int l, int r, int vl, int curl, int curr, int curv) {
	if (l >= curr || r <= curl) {
		return;
	}
	if (l <= curl && r >= curr) {
	...
	upd(l, r, vl, curl, m, curv << 1);
	upd(l, r, vl, m, curr, (curv << 1) | 1);

	t[curv] = min(t[curv << 1] + sv[curv << 1], t[(curv << 1) | 1] + sv[(curv << 1) | 1]);
}

ll get(int l, int r, int curl, int curr, int curv) {
	if (l >= curr || r <= curl) {
		return INF;
	}
	if (l <= curl && r >= curr) {
		return t[curv] + sv[curv];
	}
	int m = (curr + curl) >> 1;
	//push(curv);

	return min(get(l, r, curl, m, curv << 1), get(l, r, m, curr, (curv << 1) | 1)) + sv[curv];
...
inline void solve() {
	...
	for (int i = 0; i < q; ++i) {
		getline(cin, s);
		int l, r, vl;
		if (sscanf(s.c_str(), "%d%d%d", &l, &r, &vl) == 2) {
			if (l > r) {
				cout << min(get(0, r + 1, 0, n, 1), get(l, n, 0, n, 1)) << endl;
			}
			else {
				cout << get(l, r + 1, 0, n, 1) << endl;
			}
		}
		else {
			if (l > r) {
