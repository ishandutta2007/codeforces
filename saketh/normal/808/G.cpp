...
template<typename T> struct kmp {
    ...
    size_t adv(size_t len, const T& nxt) const {
        while (len > 0 && (len == sz(word) || word[len] != nxt))
            len = fail[len];
...
int main() {
    ...
    string s, t; re(s, t); kmp<char> k(all(t));

    vector<array<int, 26>> adv(sz(t) + 1);
    F0R (len, sz(adv)) {
        F0R (i, 26) {
            if (len < sz(t) && t[len] == char('a' + i)) adv[len][i] = len + 1;
            else adv[len][i] = adv[k.fail[len]][i];
        }
    }

    vi dp(sz(t) + 1, -1), DP = dp;
    dp[0] = 0;

    trav (c, s) {
        fill(all(DP), -1);
        F0R (tm, sz(dp)) if (dp[tm] != -1) {
            if (c != '?') {
                int nm = adv[tm][c - 'a'];
                ckmax(DP[nm], dp[tm] + (nm == sz(t)));
            } else F0R (i, 26) {
                int nm = adv[tm][i];
                ckmax(DP[nm], dp[tm] + (nm == sz(t)));
            }
