...
int n,m;
char S[105];
int dpmin[105][105][2],dpmax[105][105][2];
...
int main(){
	...
	for(int i=0;i<105;i++){
		for(int j=0;j<105;j++){
			dpmin[i][j][0]=dpmin[i][j][1]=Inf;
			dpmax[i][j][0]=dpmax[i][j][1]=-Inf;
		}
	}
			...
			for(int s=0;s<2;s++){
				{
					...
					int ss=s;
					int dx=s?-1:1;
					dpmin[i+1][jj][ss]=min(dpmin[i+1][jj][ss],dpmin[i][j][s]+dx);
					dpmax[i+1][jj][ss]=max(dpmax[i+1][jj][ss],dpmax[i][j][s]+dx);
				}
				{
					int jj=(S[i]!='T')?j+1:j;
					int ss=s^1;
	...
	int ansmn=Inf,ansmx=-Inf;
	for(int j=n%2;j<=n;j+=2){
		for(int s=0;s<2;s++){
			ansmn=min(ansmn,dpmin[m][j][s]);
			ansmx=max(ansmx,dpmax[m][j][s]);
		}
	}
	fio<<max(abs(ansmn),abs(ansmx))<<endl;
	return 0;
}