...
public:
    void solve() {
        ...
        ll answer = 0;
        for (int x : range(a, b + 1)) {
            if (b + x > d) {
                answer += ll(c - b + 1) * (d - c + 1);
            } else if (b + x > c) {
                if (c + x > d) {
                    ll len = d - (b + x) + 1;
                    answer += ll(c - b + 1) * (d - c + 1) - len * (len + 1) / 2;
                } else {
                    answer += ll(b + x - c + x) * (c - b + 1) / 2;
                }
            } else {
                if (c + x > d) {
                    ll left = x;
                    ll right = c + x - d;
                    answer += (left + right) * (d - c + 1) / 2;
                } else {
                    answer += ll(x) * (x + 1) / 2;
                }
            }
...
int main() {
    ...
    auto time = clock();
#endif
    CSoschitaiteTreugolniki solver;
