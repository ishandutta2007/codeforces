...
//   
using T = tuple<int, mint, int>;
struct Node {
    // 
    vector<P> v[SIZE];
    set<T> ALL;
    int l, r;
    Node(int l, int r, int h) : l(l), r(r) {
        rep2(i, l, r - 1) {
            v[i - l].eb(1, h + 1);
            ALL.emplace(h + 1, 1, i - l);
        }
    }
    void insert(int pos, P x) {
        ALL.emplace(x.se, x.fi, pos - l);
        v[pos - l].eb(x);
    }
    mint update(int L, int R, ll x) {
        mint res;
        if(R <= l or r <= L) return 0;
        if(L <= l and r <= R) {
            // 
            while(!ALL.empty()) {
                auto [h, val, i] = *ALL.begin();
                if(h > x) break;
                ALL.erase(ALL.begin());
                res += val;
                v[i].pop_back();
...
int main() {
    ...
    reverse(all(a));
    vector<Node> Nodes;
    for(int i = 0; i < w; i += SIZE) { Nodes.eb(Node(i, min(w, i + SIZE), h)); }
    for(auto [u, l, r, s] : a) {
        mint res;
        for(auto &node : Nodes) { res += node.update(l, r, u + s); }
        if(!l or r == w) res *= 2;
        vi nxt;
        if(l) nxt.eb(l - 1);
        if(r != w) nxt.eb(r);
        for(auto e : nxt) { Nodes[e / SIZE].insert(e, P(res, u)); }
    }
    mint ans;
    for(auto &node : Nodes) ans += node.sum();
    cout << ans << endl;
}