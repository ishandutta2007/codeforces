...
int main() {
    ...
    // let's do it in probabilities first
    vmn ans(N);

    mn tail = 0;

    F0Rd (i, N) {
        // what's the chance this guy has at least (N - 1) / 2 descendants?
        int behind = i;
        int ahead = N - i - 1;

        if (ahead >= CUT) {
            mn numer = mn::ncr(behind + ahead - CUT, behind);
            mn denom = mn::ncr(behind + ahead, behind);

            mn big = numer / denom;

            ans[i] = big - tail * mn::small_inv(behind + 1);
