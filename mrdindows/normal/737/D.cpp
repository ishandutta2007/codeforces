...
TT1 inline ostream& operator << (ostream& os, const set<T>&v);
TT1T2 inline ostream& operator << (ostream& os, const map<T1, T2>& v);
...
TT1 inline ostream& operator << (ostream& os, const set<T>&v){ bool fi = 1; os << "["; for (auto ii = v.begin(); ii != v.end(); ++ii){ if (!fi)os << ", "; os << *ii; fi = 0; }return os << "]"; }
TT1 inline ostream& operator << (ostream& os, const multiset<T>&v){ bool fi = 1; os << "["; for (auto ii = v.begin(); ii != v.end(); ++ii){ if (!fi)os << ", "; os << *ii; fi = 0; }return os << "]"; }
TT1T2 inline ostream& operator << (ostream& os, const map<T1, T2>& v){ bool fi = 1; os << "["; for (auto ii = v.begin(); ii != v.end(); ++ii){ if (!fi)os << ", "; os << "(" << ii->first << " -> " << ii->second << ") "; fi = 0; }return os << "]"; }
...
int a[202020];
int pref[40404];
int dp[4001][70][144][2];
bool used[4001][70][144][2];
const int K = 66;
const int SHIFT = K + 5;
int n;
int go(int who, int pos, int move, int dif) {
    if (used[pos][move][dif + SHIFT][who]) {
        return dp[pos][move][dif + SHIFT][who];
    }
    int left = n - pos - (pos + dif);
    if (left < move) {
        return 0;
    }
    int res;
    if (who == 0) {
        if (left == move) {
            res = pref[pos + move] - pref[pos];
        } else {
            res = max(pref[pos + move]  - go(1, pos + move, move, dif - move),
                      pref[pos + move + 1] - go(1, pos + move + 1, move + 1, dif - move - 1)) - pref[pos];
        }
    } else {
        int rpos = n - (pos + dif);
        if (left == move) {
            res = pref[rpos] - pref[rpos - move];
        } else {
            res = pref[rpos] + max(-pref[rpos - move] - go(0, pos, move, dif + move),
                                   -pref[rpos - move - 1] - go(0, pos, move + 1, dif + move + 1));
        }
    }
    used[pos][move][dif + SHIFT][who] = true;
    dp[pos][move][dif + SHIFT][who] = res;
    return res;
}
int main() {
