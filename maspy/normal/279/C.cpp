...
void solve() {
  ...
  SegTree<E> seg(
    [](E x, E y) {
      ...
      z.lx = x.lx;
      z.rx = y.rx;
      z.inc = x.inc & y.inc & x.rx <= y.lx;
      z.dec = x.dec & y.dec & x.rx >= y.lx;
      z.lad = (x.inc & y.lad & x.rx <= y.lx) | (x.lad & y.dec & x.rx >= y.lx);
      return z;
    },
    E({-1, -1, 1, 1, 1}));
  seg.init(N);

  FOR(i, N) { seg.set(i, E({A[i], A[i], 1, 1, 1})); }
