...
int main() {
    ...
    if (k % 2 == 0) {
        VV<ll> dp(n, V<ll>(m, 0));
        for (int ph = 0; ph < k / 2; ph++) {
            VV<ll> ndp(n, V<ll>(m, TEN(18)));
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                    if (j) ndp[i][j] = min(ndp[i][j], dp[i][j - 1] + a[i][j - 1]);
                    if (j < m - 1) ndp[i][j] = min(ndp[i][j], dp[i][j + 1] + a[i][j]);

                    if (i) ndp[i][j] = min(ndp[i][j], dp[i - 1][j] + b[i - 1][j]);
                    if (i < n - 1) ndp[i][j] = min(ndp[i][j], dp[i + 1][j] + b[i][j]);
                }
            }
            dp = ndp;
        }
