...
struct node {
	...
	void upd(T x) { val = min(val, x), p = min(p, x); }
...
void add_seg(int l, int r) {
	auto prev = zero_segs.lower_bound({l, -1});

	while (prev != zero_segs.end() && prev->F <= r + 1) {
		l = min(l, prev->F);
		r = max(r, prev->S);
		zero_segs.erase(prev);
		prev = zero_segs.lower_bound({l, -1});
	}

	prev = zero_segs.lower_bound({l, -1});
	while (prev != zero_segs.begin()) {
		prev--;
		if (prev->S < l - 1) {
			zero_segs.insert({l, r});
			return;
		}
		l = min(l, prev->F);
		r = max(r, prev->S);
...
bool check_zero(int pl) {
	auto z = zero_segs.lower_bound({pl, mod});
	if (z == zero_segs.begin())
		return false;
	z--;
	// cout << (z->F) << ' ' << pl << ' ' << (z->S) << endl;
	return (*z).F <= pl && pl <= (*z).S;
}
...
bool check_one(int pl) {
	int l = pl, r = pl;
	auto z = zero_segs.lower_bound({pl, mod});
	if (z != zero_segs.end() && z->F == pl + 1)
		r = z->S;
	if (z != zero_segs.begin()) {
		z--;
		if (z->S == pl - 1)
	...
	int lowest = t_min.get(l, pl + 1).val;
	if (lowest <= r)
		return true;
