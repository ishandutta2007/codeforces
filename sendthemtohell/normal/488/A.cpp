...
void inc(vector<char> &kk) {
    int i = 0;
    for (; i < sz(kk) && kk[i] == '9'; kk[i] = '0', ++i);
    if(i < sz(kk))++kk[i];
    else kk.pb(1);
}

void decr(vector<char> &kk, bool &ok) {
    int i = 0;
    for (; i < sz(kk) && kk[i] == '0'; kk[i] = '9', ++i);
    if (i < sz(kk))--kk[i];
    else kk.clear(),kk.pb('1'), ok = true;
}

bool find(vector<char> &kk) {
    for (auto &i : kk) {
        if (i == '8') return true;
    }
    return false;
