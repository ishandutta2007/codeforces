...
void pre() {
  p2s[0] = 1; mp2s[0] = 1;
  for(li i = 0; i < (li)(N-1); ++i) { p2s[i+1] = ZN(2)*p2s[i]; mp2s[i+1] = mp2s[i] * (ZN(1) - p2s[i+1]); }
  for(li i = 0; i < (li)(N); ++i) imp2s[i] = ZN(1)/mp2s[i];
}
...
ZN solve(){
  ...
  if(x == 0) {
    if(n > k) return 0;
    ZN p = 1;
    for(li i = 0; i < (li)(n); ++i) { p *= (p2k - p2s[i]); }
    return p;
  }
