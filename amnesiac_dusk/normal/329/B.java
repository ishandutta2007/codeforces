...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		ho=new ArrayDeque<pair>();
		ho.add(new pair(hox,hoy));
		process();
		int te=dp[asx][asy];
		int ans=0;
		for(int i=0; i<r; i++)
	...
	static void process()
	{
		...
		while(!ho.isEmpty())
		{
			Queue<pair> next=new ArrayDeque<pair>();
			while(!ho.isEmpty())
			{
				temp=ho.poll();
				if(dp[temp.a][temp.b]==1000000000)
				{
					dp[temp.a][temp.b]=te;
					if(temp.a!=dp.length-1&&s[temp.a+1].charAt(temp.b)!='T')
						next.add(new pair(temp.a+1,temp.b));
					if(temp.a!=0&&s[temp.a-1].charAt(temp.b)!='T')
						next.add(new pair(temp.a-1,temp.b));
					if(temp.b!=dp[0].length-1&&s[temp.a].charAt(temp.b+1)!='T')
						next.add(new pair(temp.a,temp.b+1));
					if(temp.b!=0&&s[temp.a].charAt(temp.b-1)!='T')
						next.add(new pair(temp.a,temp.b-1));
				}
			}
			te++;
