...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  cin>>s;
  rep(i,N)v[i].first=s[i]=='R';
  vsort(v);
  bool f=true;
  debug(v);
  rep(i,N){
    if(v[i].first)f&=v[i].second<=i+1;
    else f&=i+1<=v[i].second;
  }
  if(f)printyes;
  else printno;
  // cin>>N;
  ...
  //     rep(i,b.size()){
  //       auto itr=se.begin();
  //       if(itr==se.end()||*itr>b[i]){
  //         ch=false;
  //         break;
  //       }
  //       v.emplace_back(b[i]-*itr);
  //     }
  //     uniq(v);
  //     ch=se.empty()&&v.size()<=1;
  //   }
  //   f|=ch;
  // }
  // if(b.size()&&b[0]>=1){
  ...
  //     rep(i,r.size()){
  //       auto itr=se.begin();
  //       if(itr==se.end()||*itr<r[i]){
  //         ch=false;
  //         break;
  //       }
  //       v.emplace_back(*itr-r[i]);
  //     }
  //     uniq(v);
  //     ch=se.empty()&&v.size()<=1;
  //   }
  ...
  // // uniq(r);
  // // uniq(b);
  // // if(int(r.size()+b.size())!=N){
  // //   printno;
  // //   continue;
  // // }
  // // if((r.size()&&r.back()-r[0]>=N)||(b.size()&&b.back()-b[0]>=N)){
  // //   printno;
  // //   continue;
  // // }
  // // if(b[0]<1||r.back()>N){
  ...
  // // }
  // // A=1-r[0];
  // // if(A>0)for(auto& x:r)x+=A;
  // // set<ll> se;
  // // rep(i,N)se.emplace(i+1);
  // // for(auto x:r)se.erase(x);
  // // vec ch;
  // // rep(i,b.size()){

  // // }
  // // B=b.back()-N;
  // // if(B>0)for(auto& x:b)x-=B;

  // // // vec ch(N,1);
  // // // rep(i,a.size())ch[a[i]-a[0]]=0;
  // // // vec chv;
  // // // rep(i,N)if(ch[i])chv.emplace_back(i);
