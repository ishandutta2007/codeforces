...
void solve(){
	...
	// cout << my << " ";
	int mulx  = mod_int(1).mod_inv(x,my);
	int mulmy = (mulx * x - 1) / my;
	// cout << mulx*x << " " << mulmy*my << " ";
	...
	vector<int> reqx,reqmy;
	for(int i = 32; i >= 0; i--){
		if(mulx>>i&1) reqx.pb(i);
		if(mulmy>>i&1) reqmy.pb(i);
	}
	
	int zz = xpw[reqx.back()];
	reqx.ppb();
	for(int u:reqx){
		v.pb({'+',{xpw[u],zz}});
		zz += xpw[u];
