...
int main(){
	...
	// cout<<"OK"<<endl;
	// forn(i,n){
	// 	p.addEdge(n+1+i,m+n+1,a[i]);
	// }
	// cout<<"OK"<<endl;
	forn(i,m){
		int u,v,w; cin>>u>>v>>w;
		u--; v--;
		p.addEdge(i+n+1,n+m+1,w);
		ans+=w;
		// cout<<u<<" "<<v<<" "<<w<<ln;
		p.addEdge(u+1,n+1+i,MOD);
		p.addEdge(v+1,n+1+i,MOD);
		// cout<<"OK"<<endl;
	}
	// cout<<p.max_flow(0,n+m+1)<<ln;
	cout<<ans-p.max_flow(0,n+m+1)<<ln;
