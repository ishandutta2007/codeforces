...
int main () {
	...
	for (int i = 0; i < (int)b.size(); i++) {
		bi[i] = upper_bound(a.begin(), a.end(), makep(b[i].x, 0)) - a.begin();
	}

	if (a.empty() || b.empty() || a.front().x > b.back().x)
	{
		cout << "-1\n";
		return 0;
	}
	ld fr = 0, to = ((long double)(b.back().x - a.front().x)) /
	                ((long double)(-b.back().y + a.front().y));
	vector<ld> aa(a.size());
	while (true)
	{
		if (double(clock() - pradz) / CLOCKS_PER_SEC >= 1.45)
			break;
		ld mid = (fr + to) / 2;

		for (int i = 0; i < (int)a.size(); i++) {
			aa[i] = (a[i].x + a[i].y * mid);
		}
		...
		for (int i = 0; i < (int)b.size(); i++) {
			int t = upper_bound(aa.begin(), aa.end(), (ld)b[i].x + b[i].y * mid) - aa.begin();
	...
	cout << fixed << setprecision(35) << (fr + to) / 2;
}