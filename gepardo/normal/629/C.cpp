...
int getDp(int n, int k, int used)
{
    if (k < 0 || k > maxb) return 0;
        else return dp[n][k][used];
}
...
main()
{
    ...
    {
        if (j - ck >= mk) dp[0][j][1] = (getDp(0, j, 1) + getDp(0, j - ck, 0)) % mod;
    }

    for (int i = 1; i <= q; i++)
    {
        for (int j = 0; j <= maxb; j++)
            dp[i][j][0] = (getDp(i - 1, j - 1, 0) + getDp(i - 1, j + 1, 0)) % mod;

        for (int j = 0; j <= maxb; j++)
        {
            dp[i][j][1] = (getDp(i - 1, j - 1, 1) + getDp(i - 1, j + 1, 1)) % mod;
            if (j - ck >= mk)
            {
                //out.putLn("! ", i, ' ', j, ' ', j - ck);
                dp[i][j][1] = (getDp(i, j, 1) + getDp(i, j - ck, 0)) % mod;
            }
        }
    }
/*
    for (int i = 0; i <= q; i++)
    {
        for (int j = 0; j <= maxb; j++) out = dp[i][j][0], out = ' ';
        out = endl;
    }
    out = endl;
    for (int i = 0; i <= q; i++)
    {
        for (int j = 0; j <= maxb; j++) out = dp[i][j][1], out = ' ';
        out = endl;
    }
    out = endl;
*/
