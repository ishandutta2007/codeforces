...
void dfs2(int x)
{
    int curr_chain = chain;
    auto itr = s.lower_bound(make_pair(tin[x],-1));
    if(itr!=s.begin())
    {
        auto temp = isancestor((*itr).S,x);
        if(temp==0)
            chain++;
        auto temp_itr = itr--;
        if(temp_itr!=s.end())
        {
            auto temp = isancestor((*itr).S,(*temp_itr).S);
            if(temp==0)
                chain--;
        }
    }
