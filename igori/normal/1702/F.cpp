...
// based on https://atcoder.jp/contests/abc254/submissions/32271181
...
void solve()
{
    ...
    auto node = [&](int val)
    {
        if (renum.find(val) == renum.end())
        {
            renum[val] = orig.size();
            orig.push_back(val);
            subtreeA.push_back(0);
            subtreeB.push_back(0);
        }
        return renum[val];
    };
    ...
    forn(i, n)
    {
        ...
        for (auto v : path) node(v);
        for (auto v : path) subtreeA[node(v)]++;
    }
    ...
    forn(i, n)
    {
        ...
        for (auto v : path) node(v);
        for (auto v : path) subtreeB[node(v)]++;
    }
    ...
    {
        int v = orig[i];
        if (v % 2 == 1 && subtreeA[i] < subtreeB[i])
        {
            cout << "NO\n";
            return;
        }
        ans += abs(subtreeA[i] - subtreeB[i]);
    }
