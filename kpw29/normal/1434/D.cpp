...
S op(S l, S r) {
    return S{
        max(l.zero, r.zero), max(l.one, r.one)
    };
}
...
void toggle(int edge, lazy_segtree<S, op, e, F, mapping, composition, id> &seg) {
	int node = B[edge];
...
vector<int> solve(int root) {
	debug(root);
	DL = 0;
	FOR(i, 1, n) odw[i] = 0, d[i] = 0, maxpre[i] = 0, pre[i] = 0;
	dfs(root);
