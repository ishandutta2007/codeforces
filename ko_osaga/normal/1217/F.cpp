...
void interface1(){
	...
	while(m--){
		...
		printf("%d %d %d\n", t, x, y);
		fflush(stdout);*/
		if(t == 0) HDLT::add_edge(x, y);
		if(t == 1) HDLT::del_edge(x, y);
		if(t == 2){
			bool dap = HDLT::is_conn(x, y);
			if(dap) lastans = x, puts("Y");
			else lastans = y, puts("N");
		}
	}
}
	...
	while(m--){
		...
		if(x < y){
			if(s.find(pi(x, y)) != s.end()){
				comp += HDLT::del_edge(x, y);
				s.erase(pi(x, y));
			}
			else{
				comp -= HDLT::add_edge(x, y);
				s.insert(pi(x, y));
			}
		}
		else printf("%d\n", HDLT::is_conn(x, y));
...
void interface3(){
	...
	while(m--){
		...
		else printf("%d", last = HDLT::is_conn(x, y));
	}
}
