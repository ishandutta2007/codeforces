...
void calc(int a,int b,int &s1,int &s2)
{
	if (b==1)
	{
		s1+=a;
...
void construct(int a,int b,string &s)
{
	if (b==1)
	{
		for (int i=0;i<a-1;i++) s+='B';
		return;
	}
	construct(b,a%b,s);
	char current='B'+'T'-s[LENGTH(s)-1];
	for (int i=0;i<a/b;i++) s+=current;
}
void solve(int n,int m)
{
	int R=n+1,D=-1;
	for (int x=1;x<m;x++) if (gcd(x,m)==1)
	{
		int s1=0,s2=0;
		calc(m,x,s1,s2);
		if (s1==n && s2<R) R=s2,D=x;
	}
	if (R>n) 
		printf("IMPOSSIBLE\n");
	else
