...
i64 solve(i64 a,i64 b,i64 c,i64 n){
	if (!a) return (b/c)*n;
	if (a>=c||b>=c) return ((n&1)?(n+1)/2*n:n/2*(n+1))*(a/c)+(b/c)*(n+1)+solve(a%c,b%c,c,n);
	i64 m=(long double)a*n/c;
	i64 r=a*n+b-m*c;
	while(r>=c) r-=c,++m;
	while(r<0) r+=c,--m;
//	i64 m=(a*n+b)/c;
...
i64 calc(int a,int b,i64 u){
	if(u<0) return 0;
	return solve(a,u%a,b,u/a-1);
}
i64 cal(int a,int b,i64 u){
	return calc(a,b,u)-calc(a,b,u-cnt[a]*a)-calc(a,b,u-cnt[b]*b)+calc(a,b,u-cnt[a]*a-cnt[b]*b);
}
bool chk(i64 mid,i64 lim){
	int i,j;
	i64 sum=0,ans=0;
	for(i=j=1;i<=m;++i)if(cnt[i]){
		while(sum<=mid&&j<=m)sum+=cnt[j]*j,++j;
		if(sum>mid){
			int p=j-1,q=p;
			if(p>i){ 
				ans+=cnt[i]*(cnt[i]+1)/2;
				ans+=cnt[i]*(scnt[p-1]-scnt[i]);
				i64 ws=sum-cnt[i]*i;
				ans+=cal(i,p,mid-ws+cnt[p]*p);
				while(ws<=mid&&q<=m)ans+=cal(i,++q,mid-ws),ws+=cnt[q]*q;
			}else{
				i64 t=cnt[i]-mid/i;
				ans+=cnt[i]*(cnt[i]+1)/2;
				ans-=t*(t+1)/2;
		...
		}else ans+=cnt[i]*(scnt[m]-scnt[i])+cnt[i]*(cnt[i]+1)/2;
		sum-=cnt[i]*i;
		if(ans>=lim)return true;
		if(ans+(scnt[m]-scnt[i])*(scnt[m]-scnt[i]+1)/2<lim)return false; 
	}
	throw "GG"; 
}
int main(){
//	freopen("in.txt","r",stdin);
	...
	scan(n);
	read(a+1,a+n+1);
	for(i=1;i<=n;++i)pos[a[i]].pb(i),chkmax(m,a[i]);
	i64 l=0,r=0;
	for(d=m;d>=1;--d){
		vector<int> o;
		for(i=d;i<=m;i+=d){
			cnt[d]-=cnt[i];
			for(int k:pos[i])o.pb(k);
		}
		sort(o.begin(),o.end());
		for(c=i=0;i<o.size();++i){
			if(!i||o[i]==o[i-1]+1)++c;
			else cnt[d]+=i64(c)*(c+1)/2,c=1; 
		}
		cnt[d]+=i64(c)*(c+1)/2;
		r+=cnt[d]*d;
	}
	for(d=1;d<=m;++d)scnt[d]=scnt[d-1]+cnt[d];
	i64 mid=(l+r)>>1;
	i64 lim=i64(n)*(n+1)>>1;
	lim=lim*(lim+1)>>1;
	lim=(lim+1)>>1;
	while(l<r){
		if(chk(mid,lim))r=mid;
		else l=mid+1;
		mid=(l+r)>>1;
	}
	cout<<mid<<endl;
