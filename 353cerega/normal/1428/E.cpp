...
ll rec(ll a, ll cnt)
{
    return (a/cnt)*(a/cnt)*cnt + (a%cnt)*(2*(a/cnt)+1);
}
...
int main() {
    ...
    for (int i=0;i<n;i++)
    {
        ...
        ans += a[i].X*a[i].X;
        a[i].Y = 1;
        ll delta = rec(a[i].X,a[i].Y)-rec(a[i].X,a[i].Y+1);
        kek.insert({-delta,i});
    }
    while (k>0)
    {
        k--;
        pair<ll,ll> cur = (*kek.begin());
        ans += cur.X;
        kek.erase(cur);
        ll num = cur.Y;
        a[num].Y++;
        ll delta = rec(a[num].X,a[num].Y)-rec(a[num].X,a[num].Y+1);
        kek.insert({-delta,num});
    }
    cout << ans << endl;
}