...
vector<int> min_factor = elementary_math::osa_k(MAX_N);
...
void solve() {
    ...
    vector<mint> val(n);
    mint sum = 0;
    vector<int> cur(n + 1, 0), Max(n + 1, 0);
    auto dfs = [&](auto self, int v, int p) -> void {
        ...
        for (auto& e : G[v]) {
            ...
            for (int& p : X) chmax(Max[p], cur[p]);
            for (int& p : Y) chmax(Max[p], cur[p]);
            self(self, u, v);
            for (int& p : X) cur[p]++;
            for (int& p : Y) cur[p]--;
        }
