...
int main(){
    ...
    each(a,b,A)in(b);
    sor(A);
    if(all_of(all(A),[&](pll x){ return x.first==A[0].first;})){
        out(fact(n));
        return 0;
    }
    if(A[0].second==0){
    ...
    const ll x=A[0].first+n-1;
    if(A.back().first>x){
        out(0);
        return 0;
    }
    Modint ans=perm(n,count(A,pll{x,1}));
    A.erase(remove(all(A),pll{x,1}),A.end());
    rep(i,1,A.size())if(A[i]==A[i-1]&&A[i].second==1){
        out(0);
        return 0;
    }
    map<ll,ll>cnt;
    each(a,b,A)ans*=++cnt[a*2+b];
    ll lo=0;
    each(key,value,cnt){
        const ll a=key/2;
        if(key%2){
            lo+=value;
            const ll lb=a+n-lo,ub=a+n-1;
            if(x<lb||ub<x){
                out(0);
                return 0;
            }
