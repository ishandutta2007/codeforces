...
public:
    ...
    T& operator[](const pii& at) {
        return operator()(at.first, at.second);
    }

    const T& operator[](const pii& at) const {
        return operator()(at.first, at.second);
    }
...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        vec<pair<int, string>> answer;
        for (int i : range(n - 1)) {
            int steps = min(m - 1, k / 3);
            if (steps != 0) {
                answer.emplace_back(steps, i % 2 == 0 ? "DUR" : "DUL");
                k -= steps * 3;
            }
            if (steps < m - 1) {
                if (k == 1) {
                    answer.emplace_back(1, "D");
                } else if (k == 2) {
                    answer.emplace_back(1, "DU");
                }
                k = 0;
                break;
            }
        ...
        if (k) {
            int steps = min(m - 1, k);
            if (steps != 0) {
                answer.emplace_back(steps, (n - 1) % 2 == 0 ? "R" : "L");
                k -= steps;
            }
        }
        if (k) {
            for (int i : RevRange(n)) {
                int steps = min(m - 1, k);
                if (steps != 0) {
                    answer.emplace_back(steps, i % 2 == 0 ? "L" : "R");
                    k -= steps;
                }
                if (k) {
                    answer.emplace_back(1, "U");
                    k--;
                } else {
                    break;
                }
