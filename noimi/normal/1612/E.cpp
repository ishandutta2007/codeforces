...
int main() {
    ...
    rep2(t, 1, tar) {
        if(t > n) break;
        if(t > m) break;
        vector<pair<frac, int>> d(m);
        rep(i, m) d[i].se = i;
        rep(i, n) { d[a[i]].fi += (b[i] >= t ? frac(1) : frac(b[i], t)); }
        if(t == tar) {
            int mem = -1;
            sort(all(d), greater{});
            frac s = 0;
            rep(i, m) {
                s += d[i].fi * t;
                if(i + 1 >= tar and chmax(ma, s / (i + 1))) { mem = i + 1; }
            }
            if(mem != -1) {
                ans.clear();
                rep(i, mem) ans.eb(d[i].se);
            }
            break;
        }
        nth_element(begin(d), begin(d) + t, end(d), greater{});
        frac sum;
        rep(i, t) sum += d[i].fi;
        if(chmax(ma, sum)) {
            ans.clear();
            rep(i, t) ans.eb(d[i].se);
        }
    }
    OUT(si(ans));
    OUT(ans);
