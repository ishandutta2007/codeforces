...
vector<vector<vector<vector<int>>>> dp(57, vector<vector<vector<int>>>(57, vector<vector<int>>(57, vector<int>(57, -1))));
vector<vector<int>> b;
bool isAreaEmpty(int r1, int r2, int c1, int c2) {
	return b[r2][c2] - b[r1-1][c2] - b[r2][c1-1] + b[r1-1][c1-1] == 0;
}
bool isRowEmpty(int r, int c1, int c2) {
	return isAreaEmpty(r, r, c1, c2);
}
bool isColEmpty(int c, int r1, int r2) {
	return isAreaEmpty(r1, r2, c, c);
}

int solve(int r1, int r2, int c1, int c2) {
	if (dp[r1][r2][c1][c2] > -1) return dp[r1][r2][c1][c2];
	if (r2 < r1 || c2 < c1) return 0;
	if (isAreaEmpty(r1,r2,c1,c2))
		return dp[r1][r2][c1][c2] = 0;
		
	int ans = max(r2-r1+1, c2-c1+1);
	for (int r = r1; r <= r2; r++)
		if (isRowEmpty(r, c1, c2))
			chkmin(ans, solve(r1, r-1, c1, c2) + solve(r+1, r2, c1, c2));
			
	for (int c = c1; c <= c2; c++)
		if (isColEmpty(c, r1, r2))
			chkmin(ans, solve(r1, r2, c1, c-1) + solve(r1, r2, c+1, c2));
