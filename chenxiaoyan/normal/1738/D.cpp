...
void mian() {
  ...
  rep(i, 1, n) b[i] = rd(), buc[b[i]].pb(i);
  int num = sz(buc[0]) ? 0 : n + 1, now = 0, which = num;
  while(true) {
    for(int &x : buc[num]) if(sz(buc[x])) {
      swap(x, buc[num].back()); break;
    }
    rep(i, 0, sz(buc[num]) - 1) a[now + 1 + i] = buc[num][i], wch[now + 1 + i] = which;
    now += sz(buc[num]);
    if(now == n) break;
    num = a[now];
    which = which ? 0 : n + 1;
  ...
  int k = 0;
  rep(i, 1, n) if(wch[i] == n + 1) cmx(k, a[i]);
  prt(k), pc('\n');
  rep(i, 1, n) prt(a[i]), pc(" \n"[i == n]);
}
