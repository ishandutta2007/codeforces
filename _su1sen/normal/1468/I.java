...
public class I {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int x2 = sc.nextInt();
        int y2 = sc.nextInt();
        if (Math.abs((long) x1 * y2 - (long) x2 * y1) != N) {
            cp.println("NO");
        } else {
            cp.println("YES");
            int[][] points = new int[4][2];
            points[1] = new int[] { x1, y1 };
            points[2] = new int[] { x1 + x2, y1 + y2 };
            points[3] = new int[] { x2, y2 };
            long[][] lines = new long[4][3];
            boolean para = false;
            for (int n = 0; n < 4; n++) {
                int[] p1 = points[n];
                int[] p2 = points[(n + 1) % 4];
                int dx = p1[0] - p2[0];
                int dy = p1[1] - p2[1];
                lines[n] = new long[] { dy, -dx, (long) p1[1] * dx - (long) p1[0] * dy };
                if (dy == 0) {
                    para = true;
                }
            }
            ...
            for (int n = 0; n < 4; n++) {
                ymin = Math.min(ymin, points[n][1]);
                ymax = Math.max(ymax, points[n][1]);
            }
            ArrayList<int[]> cands = new ArrayList<>();
            for (int n = ymin; n <= ymax; n++) {
                if (para) {
                    long[] ceil = new long[2];
                    long[] floor = new long[2];
                    int p = 0;
                    for (int m = 0; m < 2; m++) {
                        if (lines[p][0] == 0) {
                            m--;
                        } else {
                            long a = -(lines[p][1] * n + lines[p][2]);
                            long b = lines[p][0];
                            if (b < 0) {
                                a = -a;
                                b = -b;
                    ...
                    Arrays.sort(ceil);
                    Arrays.sort(floor);
                    for (long m = ceil[0]; m <= floor[1]; m++) {
                        cands.add(new int[] { (int) m, n });
                    }
                } else {
                    long[] ceil = new long[4];
                    long[] floor = new long[4];
                    for (int m = 0; m < 4; m++) {
                        long a = -(lines[m][1] * n + lines[m][2]);
                        long b = lines[m][0];
                        if (b < 0) {
                            a = -a;
                            b = -b;
            ...
            for (int[] cand : cands) {
                int x = cand[0];
                int y = cand[1];
                if (lines[1][0] * x + lines[1][1] * y + lines[1][2] != 0
                        && lines[2][0] * x + lines[2][1] * y + lines[2][2] != 0) {
                    cp.println(x + " " + y);
                    count++;
                }
            }
