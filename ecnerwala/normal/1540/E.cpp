...
int main() {
	...
	int Q; cin >> Q;
	int R = 0;
	vector<tuple<int, int, int>> ops; ops.reserve(Q + N * N);
	{
		vector<int> cur_wakeup(N, N+1);
		auto update_wakeup = std::y_combinator([&](auto self, int a, int d) -> void {
			if (d >= cur_wakeup[a]) return;
			assert(d < N);
			cur_wakeup[a] = d;
			ops.push_back({a, d, -2});
			for (int p : par[a]) self(p, d+1);
		});

		vector<int> cur_A = A;
		for (int i = 0; i < N; i++) {
			if (cur_A[i] > 0) update_wakeup(i, 0);
		}
		for (int q = 0; q < Q; q++) {
			int op; cin >> op;
			if (op == 1) {
	...
	const int MAXK = 1010;
	tensor<num, 2> dp({MAXK, N+1});
	for (int st = 0; st < N; st++) {
		int cur_R = 0;
		num cur_A = A[st];
		int cur_wakeup = MAXK+1;
		for (int i = 0; i < st; i++) {
			dp[{0, i}] = 0;
		}
		dp[{0, st}] = num(1);
		for (int t = 1; t < MAXK; t++) {
			for (int i = 0; i <= st; i++) {
				dp[{t, i}] = dp[{t-1, i}] * num(i+1);
			}
			for (int i = 0; i <= st; i++) {
				for (int p : par[i]) dp[{t, p}] += dp[{t, i}];
			}
		}
		for (int t = 0; t < MAXK; t++) {
			num cur_pref = 0;
			for (int i = 0; i <= N; i++) {
				dp[{t, i}] = std::exchange(cur_pref, cur_pref + (i <= st ? dp[{t, i}] : 0));
			}
		}
		for (int q = 0; q < int(ops.size()); q++) {
			if (get<2>(ops[q]) == -1) {
				auto [x, p, _] = ops[q];
				if (p == st) cur_A += x;
			} else if (get<2>(ops[q]) == -2) {
				auto [a, d, _] = ops[q];
				if (a == st) cur_wakeup = d;
			} else {
				auto [k, l, r] = ops[q];

				int t = max(k - cur_wakeup, 0);
				ans[cur_R] += num(cur_A) * (dp[{t, r}] - dp[{t, l}]);
