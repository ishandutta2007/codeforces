...
vector< int > dijkstra(vector< vec > &g, int s) {
...
signed main(){
    ...
    int t=in();
    while(t--){
        int n=in(),m=in(),a=in()-1,b=in()-1,c=in()-1;
        vecll cost;rep(i,m)cost.pb(in());
        vector<vec> g(n);
        rep(i,m){
            int x=in()-1,y=in()-1;
            g[x].pb(y);
        ...
        vecll rui;
        rui.pb(0);
        for(auto e:cost)rui.pb(rui.back()+e);
        vec used(n);
        queue<int> q;q.push(b);
        ll ans = LONG_LONG_MAX;
        rep(now,n){
            if(A[now]+B[now]+C[now]<rui.size())
            chmin(ans,rui[A[now]+ B[now] + C[now]] + rui[B[now]]);
        }
        cout << ans << endl;
    }
