...
int main(){
	...
	cauto g=readG(n);
	vector<int> degs(n);
	rep(v,n) degs[v]=g[v].size();
	priority_queue<pii,vector<pii>,greater<pii>> pq;
	rep(v,n)if(degs[v]==1) pq.emplace(hs[v],v);
	ll h=0;
	ll re=0;
	while(pq.size()>2){
		cauto nh=pq.top().X;
		re+=(nh-h)*pq.size();
		out(h,nh,pq.size(),1);
		while(pq.size()>2 && pq.top().X<=nh){
			cauto v=pq.top().Y; pq.pop();
			for(cauto &u:g[v])if(--degs[u]==1) pq.emplace(hs[u],u);
		}
		h=nh;
	}
	re+=(*max_element(all(hs))-h)*pq.size();
	cout<<re NL;
	return 0;
}