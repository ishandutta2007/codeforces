...
void solve() {
  ...
  FOR(i, N) {
    ...
    FOR(j, N - K + 1) {
      // [j, j+K)  W  white 
      bool ok = (Xc[j + K] - Xc[j] == now);
      if (!ok) continue;
      // [i-K+1,i] x {j}
      int l = max<int>(0, i - K + 1);
      add(l, j, i + 1, j + 1);
    }
  }
  FOR(i, N) {
    vi X(N);
    FOR(j, N) X[j] = (G[j][i] == 'B' ? 1 : 0);
    int now = SUM(X);
    if (now == 0) {
      ++base;
      continue;
  ...
  FOR(i, N) FOR(j, N + 1) dp[i + 1][j] += dp[i][j];
  FOR(i, N) FOR(j, N + 1) dp[j][i + 1] += dp[j][i];

  ll ANS = 0;
  FOR(i, N - K + 1) FOR(j, N - K + 1) chmax(ANS, dp[i][j]);
  ANS += base;
  print(ANS);
}
