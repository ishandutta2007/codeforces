...
struct BinaryTrie{
  ...
  vector<vector<ll>>insert(T x, ll add = 1){
    auto ret=vec(size,2,0LL);
    Node* pos = root;
    bit = rev^x;
    for(int i=size-1;i>=0;i--){
      ...
      if(pos->next[!bit[i]]){
        ret[i][bit[i]]+=pos->next[!bit[i]]->num;
      }
      pos->num += add;
      pos = pos->next[bit[i]];
    }
...
int main(){
  ...
  vector<ll>a(n);
  rep(i,0,n)cin>>a[i];
  auto cnt=vec(30,2,0LL);
  BinaryTrie<>tri;
  rep(i,0,n){
    auto tmp=tri.insert(a[i]);
    rep(j,0,30)rep(o,0,2)cnt[j][o]+=tmp[j][o];
    //debug(tmp,5,2);cout<<endl;
  }
  ll num=0,val=0;
  rep(i,0,30){
    ll t=0;
    if(cnt[i][0]>cnt[i][1])t=1;
    num+=cnt[i][t];
    if(t)val^=1<<i;
  }
  cout<<num spa val<<endl;
  return 0;
}