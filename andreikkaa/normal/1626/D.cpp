...
void _solve() {
    ...
    for(int i = 0; i < sz(a); ++i) {
        sum += a[i].y;
        ans = min(ans, f(sum) - sum + f(n - sum) - (n - sum) + 1);
    }
    ...
    for(int i = 0; i < sz(a); ++i) {
        ...
        for(int s2 = 1; s2 <= f(n - sum1); s2 *= 2) {
            ...
            int sum3 = psum(l, sz(a));
            assert(sum3 > 0);
            assert(sum1 + sum2 + sum3 == n);
            ans = min(ans, f(sum1) + f(sum2) + f(sum3) - sum1 - sum2 - sum3);
        }

        for(int s3 = 1; s3 <= f(n - sum1); s3 *= 2) {
            int l = i + 1;
            int r = sz(a);
            while(l != r) {
                int m = (l + r) / 2;
