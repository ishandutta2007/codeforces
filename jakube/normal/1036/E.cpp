...
public:
    ...
    friend istream& operator>>(std::istream& is, Point<T>& pt) { return is >> pt.x >> pt.y; }
    friend ostream& operator<<(std::ostream& os, Point<T> const& pt) { return os << pt.x << " " << pt.y; }
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        auto s = segments[i];
        auto vec = s.p - s.q;
        cnt += abs(gcd(vec.x, vec.y)) + 1;
        set<Pt> intersections;
        for (int j = i + 1; j < n; j++) {
            auto t = segments[j];
            Pt p = s.intersect(t);
