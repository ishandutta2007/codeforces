...
const int maxn = 3e5 + 5000, maxw = 1e6 + 10, inf = 1e9 + 100, LG = 19, SQ = 500;
...
void dfs(int v) {
    ...
    for (int id : zis[v]) {
        if (has)
            break;
        auto [P, u] = que[id];
        if (f.get(seg[v].fr, seg[v].sc) == f.get(seg[u].fr, seg[u].sc))
            has = 1;
    }
    if (has) {
        ans++;
...
void solve() {
    ...
    pfs(0);
    for (int i = 0; i < m; i++) {
        cin >> que[i].fr >> que[i].sc;
        que[i].fr--;
        que[i].sc--;
        if (seg[que[i].fr].fr > seg[que[i].sc].fr)
            swap(que[i].fr, que[i].sc);
    }
    vector<int> gis;
    for (int i = 0; i < m; i++) {
        if (up[0][que[i].sc] == que[i].fr) {
            cout << "-1\n";
            return;
        }
        if (seg[que[i].fr].sc >= seg[que[i].sc].sc)
            zis[cli(que[i].sc, h[que[i].sc] - h[que[i].fr] - 1)].push_back(i);
        else
            gis.push_back(i);
    }
    f.init(n);
    ...
    for (int id : gis) {
        auto [v, u] = que[id];
        if (f.all == f.get(seg[v].fr, seg[v].sc) + f.get(seg[u].fr, seg[u].sc)) {
            ans++;
            break;
        }
    }
