...
dp calc(int u, int p) {
    ...
    dp my; my.col = a[u]; my.cc = a[u] > -1;
    trav (cr, cres) if (cr.col != -1) {
        if (my.col != -1 && my.col != cr.col) { ps(0); exit(0); }
        my.col = cr.col;
        my.cc += cr.cc;
    }

    if (my.col == -1) {
        vmn pref = prefixes(cres, mn(1), [](mn m, dp r){ return m * (r.full + r.empty); });
        vmn suff = suffixes(cres, mn(1), [](dp r, mn m){ return m * (r.full + r.empty); });
        my.empty = pref.back();
        F0R (i, sz(cres)) my.full += pref[i] * cres[i].full * suff[i+1];
    } else {
        mn ways = 1;
        trav (cr, cres) ways *= (cr.col == my.col) ? cr.part : (cr.full + cr.empty);
        if (my.cc == cc[my.col]) my.full = ways, my.col = -1, my.cc = 0;
        else my.part = ways;
    }
    return my;
}
