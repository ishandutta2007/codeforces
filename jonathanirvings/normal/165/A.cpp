...
int main()
{
    ...
    REPN(i,n)
    {
    		satu = dua = tiga = empat = 0;
    		REPN(j,n)
    		{
    			if (data[i].fi < data[j].fi && data[i].se == data[j].se) satu = 1;
    			if (data[i].fi > data[j].fi && data[i].se == data[j].se) dua = 1;
    			if (data[i].se < data[j].se && data[i].fi == data[j].fi) tiga = 1;
    			if (data[i].se > data[j].se && data[i].fi == data[j].fi) empat = 1;
    		}
    		if (satu && dua && tiga && empat) risan++;
    }
    printf("%d\n",risan);
	return 0;
}