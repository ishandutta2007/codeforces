...
fn rec(l: usize, r: usize, memo: &mut [Vec<Option<(i32,i32)>>]) -> (i32,i32) {
	assert!(l <= r);
	if let Some( res ) = memo[l][r] {
		return res;
	}
	let mut res0 = (r - l + 1) as i32;
	let mut res1 = 0;
	for i in l..r {
		let ll = rec(l, i, memo);
		let rr = rec(i+1, r, memo);
		if ll.0 == 1 && ll == rr {
			res0 = 1;
			res1 = ll.1 + 1;
			break;
...
fn main() {
	...
	let n = rin.u();
	let a = rin.vi(n);
	let mut memo = v!([n+1][n+1] = None);
	for i in 1..=n {
		memo[i][i].replace( (1,a[i-1]) );
	}
	let sol = rec(1, n, &mut memo).0;
	writeln!(rout, "{}", sol).ok();
}