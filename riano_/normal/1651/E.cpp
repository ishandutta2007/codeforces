...
ll calc(ll lmn,ll lmx,ll rmn,ll rmx,ll pv,ll nx,ll N){
    ...
    ll lmn2 = lmn,lmx2 = lmx,rmn2 = rmn,rmx2 = rmx;
    if(pv<=N){
        chmax(lmx2,pv); chmin(lmn2,pv);
    }
    else{
        chmax(rmx2,pv); chmin(rmn2,pv);
    }
    res -= (N+1-lmx2)*(lmn2)*(rmn2-N)*(2*N+1-rmx2);
    if(nx<=N){
    ...
    res -= (N+1-lmx)*(lmn)*(rmn-N)*(2*N+1-rmx);
    if(nx<=N){
        chmax(lmx2,nx); chmin(lmn2,nx);
    }
    else{
        chmax(rmx2,nx); chmin(rmn2,nx);
    }
    res += (N+1-lmx2)*(lmn2)*(rmn2-N)*(2*N+1-rmx2);
    return res;
}
    ...
    for(int i=1;i<=N;i++){
        ...
        rep(j,L){
            ...
            rep(k,L-2){
                ...
                // cout << j << " " << L << " " << lmn << " " << lmx << " " << rmn << " " << rmx << endl;
                // cout << calc(lmn,lmx,rmn,rmx,pv,nx,N) << endl;
                // cout << ans << endl;
            }
        }
        ll lmn = 2e9,lmx = -1,rmn = 2e9,rmx = -1;
