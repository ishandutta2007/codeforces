...
void solve()
{
	...
	ll sum = 0;
	for (auto x : a) sum += x;
	if (sum == 0) return writeln(0);

	vector<int> u(n);
	vector dp(n, vector<modint>(n, 0));
	vector small(n, vector<int>(n, 1));
	...
	function<void(int)> go = [&](int x)
	{
		...
		for (auto y : v[x])
		{
			...
			{
				dp[x][j] += dp[y][j - 1];
				if (!(small[x][j] && small[y][j - 1] && dp[x][j].GetValue() <= bound))
					small[x][j] = 0;
			}
		}
	};
	...
	for (int i = 0; i < n; ++i)
	{
		...
		for (int j = 0; j < n; ++j)
		{
			if (cnt_small[j] && a[i] > 0)
			{
				if (!(cnt_small[j] && small[i][j] && cnt[j].GetValue() + dp[i][j].GetValue() * a[i] <= bound))
					cnt_small[j] = 0;
			}
			cnt[j] += dp[i][j] * a[i];
		}
	...
	modint res = 0;
	int cur = 0;
	int max_step = *max_element(r.begin(), r.end());
	int bias = 0;
	for (int step = 1; step <= n; ++step)
	{
		...
		if (cur == 0)
			...
			cur -= 1;
		if (step >= max_step && cur == 0) return writeln(res.GetValue());
	}
