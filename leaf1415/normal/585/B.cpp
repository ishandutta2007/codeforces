...
bool get(ll t, ll x, ll y)
{
	return (x+t > n || c[x+t][y] == '.');
}
...
int main(void)
{
	...
	while(T--){
		...
		rep(t, 0, n) rep(x, 1, n) rep(y, 0, 2) dp[t][x][y] = false;
		dp[0][sx][sy] = true;
		
		rep(t, 0, n){
			rep(x, 1, n){
				rep(y, 0, 2){
					if(!get(2*t, x+1, y)) continue;
					rep(k, -1, 1){
						if(y+k >= 0 && y+k <= 2 && get(2*t, x+1, y+k) && get(2*t+1, x+1, y+k) && get(2*t+2, x+1, y+k)){
							dp[t+1][x+1][y+k] |= dp[t][x][y];
						}
					}
				}
			}
		...
		bool ans = false;
		rep(t, 0, n) rep(y, 0, 2) ans |= dp[t][n][y];
		if(ans) yes();
		else no();
	}
