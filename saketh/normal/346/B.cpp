...
void rep_max(string& a, const string& b) {
    if (sz(b) > sz(a)) a = b;
}
...
int main() {
    ...
    string s1, s2, virus; re(s1, s2, virus);

    kmp<char> k(all(virus));

    vector<map<pii, string>> dp(sz(s1) + sz(s2) + 1);
    dp[0][{0,0}] = "";

    string ans = "";
    F0R (d, sz(dp)) {
        trav (e, dp[d]) if (e.f.s < sz(virus)) {
            rep_max(ans, e.s);
            int i1 = e.f.f, i2 = d - i1;
            if (i1 < sz(s1) && i2 < sz(s2) && s1[i1] == s2[i2]) {
                rep_max(dp[d+2][{i1+1, k.adv(e.f.s, s1[i1])}], e.s + s1[i1]);
            }
            if (i1 < sz(s1)) rep_max(dp[d+1][{i1+1, e.f.s}], e.s);
            if (i2 < sz(s2)) rep_max(dp[d+1][{i1, e.f.s}], e.s);
        }
    }
