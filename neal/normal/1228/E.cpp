...
int main() {
    ...
    for (int rows = 0; rows <= N; rows++)
        for (int cols = 0; cols <= N; cols++) {
            mod_int ways = choose(N, rows) * choose(N, cols);
            mod_int sign = (rows + cols) % 2 == 0 ? +1 : -1;
            int covered_count = (rows + cols) * N - rows * cols;
            total += sign * ways * mod_int(K - 1).pow(covered_count) * mod_int(K).pow(N * N - covered_count);
        }
