...
int main(){
    ...
    set<Pr> s;
    vector<vector<mint>> cf(N+1);
    rep(i,N){
        ll k = G.G[i+1].size();
        if(i!=0) k--;
        if(G.G[i+1].size()==0) continue;
        vector<mint> vv = {mint(1),k};
        cf[i+1] = vv;
        s.emplace(2LL,i+1);
    }
    ...
    while(s.size()>1){
        ...
        auto[l1,v1] = *itr; itr++;
        auto[l2,v2] = *itr;
        cf[v1] = convolution(cf[v1],cf[v2]);
        s.erase(make_pair(l1,v1)); s.erase(make_pair(l2,v2)); s.emplace(l2+l1-1,v1);
    }

    auto itr2 = s.begin(); auto[l,v] = *itr2;
    rep(i,l){
        ans += cf[v][i]*fact[N-i]*mint(-1).pow(i);
        //cout << cf[v][i] << endl;
    }
