...
struct nodeval {
    vector<ll> val;
    int get_count(ll x) const {
        return upper_bound(all(val), x) - lower_bound(all(val), x);
    }
};

nodeval mergeval(const nodeval &a, const nodeval &b) {
    nodeval ans;
    ans.val.resize(sz(a.val) + sz(b.val));
    merge(all(a.val), all(b.val), ans.val.begin());
    return ans;
}
...
nodemin mergemin(const nodemin &a, const nodemin &b) {
    nodemin ans;
...
struct Tree {
    ...
    ll get_count(int l, int r, ll x) const { // get at [l, r), doesn't support push
        l += n;
        r += n;
        int cr = r;
        ll ans = 0;
        for (; l < r; l >>= 1, r >>= 1) {
            if (l & 1) ans += t[l++].get_count(x);
            if (r & 1) ans += t[--r].get_count(x);
        }
        return ans;
    }
};
...
void solve() {
    ...
    makepref(p[0]);
    makepref(p[1]);
    auto getv = [&](int i, int i2) -> ll {
        return i == 0 ? 0 : p[i2][i - 1] - p[!i2][i - 1];
    };
    vector<Tree<nodemin, mergemin>> mntree(2, Tree<nodemin, mergemin>(n));
    vector<Tree<nodeval, mergeval>> valtree(2, Tree<nodeval, mergeval>(n));
    for (int i = 0; i < n; ++i) {
        int i2 = i % 2;
        ll v = a[i] + getv(i, i2);
        mntree[i2].updup(i, v);
        valtree[i2][i].val.pb(v);
    }
    for (auto &valtree : valtree) {
        valtree.build();
    }
    ll ans = 0;
    for (int l = 0; l < n; ++l) {
        ...
        while (qr - ql > 1) {
            ...
            for (int i2 = 0; i2 < 2; ++i2) {
                auto vl = getv(l, i2);
                if (vl > mntree[i2].getup(l, mid + 1).mn) {
                    flag = false;
                    break;
                }
            }
        ...
        for (int i2 = 0; i2 < 2; ++i2) {
            ans += valtree[i2].get_count(l, qr, getv(l, i2));
        }
    }
    println(ans);
}
