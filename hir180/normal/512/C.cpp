...
void solve(){
	...
	int s = 201, t = 202;
	rep(i, n){
		if(a[i] & 1) g.add_edge(s, i, 2);
		else g.add_edge(i, t, 2);
	}
	rep(i, n) if(a[i]&1){
		rep(j, n) if(a[j]%2==0){
			int v = a[i] + a[j];
			for(int x=2;x*x<=v;x++) if(v%x == 0) goto fail;
			g.add_edge(i, j, 1); fail:;
		}
	}
	auto ret = g.flow(s, t);
	if(ret != n){
		...
		for(auto a:all){
			int x = a.from, y = a.to;
			if(x < n and y < n and a.flow){
				adj[x].pb(y);
				adj[y].pb(x);
			}
		}
		rep(i,n)assert(adj[i].size() == 2);
		vc<int>vis(n);
		vc<vc<int>>cyc;
		rep(i,n){
			if(vis[i]) continue;
