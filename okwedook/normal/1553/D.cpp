...
void solve() {
    ...
    s = '#' + s;
    t = '#' + t + '$';
    vector<set<int>> dp(sz(s)), dptrash(sz(s));
    dp[0].insert(0);
    auto makesimple = [](set<int> &st) {
        st.insert(0);
        vector<int> v(all(st));
        reverse(v);
        st.clear();
    ...
    for (int i = 1; i < sz(s); ++i) {
        for (auto wasdp : dp[i - 1]) {
            if (s[i] == t[wasdp + 1])
                dp[i].insert(wasdp + 1);
            dp[i].insert(wasdp - 1);
            dptrash[i].insert(wasdp);
        }
        for (auto val : dptrash[i - 1])
            dp[i].insert(val);
        makesimple(dp[i]);
        makesimple(dptrash[i]);
    }
    println(*(--dp.back().end()) == sz(t) - 2 ? "YES" : "NO");
}
