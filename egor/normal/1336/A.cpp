...
public:
    void solve() {
        ...
        arri level(n);
        arri children(n);
        function<int(int, int, int)> dfs = [&](int vert, int last, int lev) -> int {
            level[vert] = lev;
            children[vert] = 0;
            for (auto* e : graph[vert]) {
                ...
                children[vert] += dfs(next, vert, lev + 1);
            }
            return children[vert] + 1;
        };
        dfs(0, -1, 0);
        auto order = createOrder(n);
        sort(all(order), [&](int a, int b) -> bool {
            return level[a] - children[a] < level[b] - children[b];
        });
        ll answer = 0;
        for (int i : range(n - k)) {
            answer += children[order[i]] - level[order[i]];
        }
