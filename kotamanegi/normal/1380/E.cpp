...
ld calc(Point a) {
    ...
    for (int i = 0; i < positions.size(); ++i) {
        ans += abs(a - Point(positions[i][0], positions[i][1]));
    }
    return ans;
}
...
public:
    double getMinDistSum(vector<vector<int>>& pos) {
        positions = pos;
        ld ans = calc(Point(0,0));
        ld now_x = 0;
        ld now_y = 0;
        while(true){
            ld min_x = 0, min_y = 0;
            ld max_x = 100, max_y = 100;
            REP(q, 100) {
                ld bot_x = (min_x + min_x + max_x) / 3.0L;
                ld good_x = (max_x + min_x + max_x) / 3.0L;
                if (calc(Point(bot_x, now_y)) > calc(Point(good_x, now_y))) {
                    min_x = bot_x;
                }
                else {
                    max_x = good_x;
            ...
            now_x = min_x;
            REP(q, 100) {
                ld bot_y = (min_y + min_y + max_y) / 3.0L;
                ld good_y = (max_y + min_y + max_y) / 3.0L;
                if (calc(Point(now_x, bot_y)) > calc(Point(now_x, good_y))) {
                    min_y = bot_y;
                }
                else {
                    max_y = good_y;
            ...
            now_y = min_y;
            ld hoge = calc(Point(now_x, now_y));
            if (ans < hoge + 1e-6) {
                break;
            }
            ans = hoge;
        }
        return calc(Point(now_x, now_y));
    }
};
...
void solve(){
    ...
    REP(i, m-1) {
        int a, b;
        cin >> a >> b;
        merges.push_back(mp(a, b));
    }
    priority_queue<tuple<int, pair<int, int>, int, int>> nexts[2];
    REP(i, n-1) {
        if(inputs[i] != inputs[i+1])
        nexts[0].push(make_tuple(-(m-1)/2, mp(inputs[i], inputs[i + 1]), 0, m-1));
    }
    while (nexts[0].empty() == false) {
        REP(i, m + 1) {
            ut[i] = i;
        }
        for (int q = 0; q < m-1; ++q) {
            while (nexts[0].empty() == false and get<0>(nexts[0].top()) == -q) {
                tuple<int, pair<int, int>, int, int> now = nexts[0].top();
                nexts[0].pop();
                if (get<2>(now) == get<3>(now) - 1) {
                    ans[get<2>(now)]++;
                    continue;
                }
                if (uf(get<1>(now).first) != uf(get<1>(now).second)) {
                    get<2>(now) = -get<0>(now);
                }
                else {
                    get<3>(now) = -get<0>(now);
                }
                get<0>(now) = get<2>(now) + get<3>(now);
                get<0>(now) /= 2;
                get<0>(now) *= -1;
                nexts[1].push(now);
            }
            um(merges[q].first, merges[q].second);
        }
        swap(nexts[0], nexts[1]);
    }
    for (int q = m; q >= 1; --q) {
        ans[q - 1] += ans[q];
    }
