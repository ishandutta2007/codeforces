...
int main(){
    ...
    n=a.size();
    rep(60)if(count_if(all(a),[&](ll x){return x>>i&1;})>=3)return out(3);
    ll ans=LINF;
    rep(s,n){
        vv(ll,cost,n,n,LINF);
        cost[s][s]=0;
        ...
        q.emplace(s,s);
        while(q.size()){
            auto[prev,at]=q.front();
            q.pop();
            if(at==s&&prev!=s){
                chmin(ans,cost[prev][at]);
                break;
            }
            rep(n)if(a[at]&a[i]&&i!=prev&&i!=at&&chmin(cost[at][i],cost[prev][at]+1))q.emplace(at,i);
        }
    }
    if(ans==LINF)ans=-1;
    out(ans);
