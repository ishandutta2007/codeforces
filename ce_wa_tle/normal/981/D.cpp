...
const ll low=2e7,mod=1e9+7;
ll a[114514],dp[55][55],sum[23333];
int main(){
	//IO(CF981D Bookshelves);
	ll rill(n,k);
	rep(i,1,n){
		rill(a[i]);
		sum[i]=sum[i-1]+a[i];
	...
	per(t,57,0){
		...
		rep(i,1,n){
			rep(j,1,k){
				dp[i][j]=0;
				rep(_i,0,i-1)
				if(((sum[i]-sum[_i])&ans)==ans){
					dp[i][j]|=dp[_i][j-1]&!!((sum[i]-sum[_i])&(1ll<<t));
//					writeln(sys(dp,i,j),sys(dp,_i,j-1));
					if(dp[i][j])break;
				}
			}
		}
