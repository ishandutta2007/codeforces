...
class Graph(val n:Int, val m:Int, val directed:Boolean) {
    ...
    inline fun BFS2(distRoot:Int, distRoot2:Int, reached:(Int, Int)->Unit = {_,_ ->}): IntArray {
        val toDo = fastDeque(0,n)
        val explored = IntArray(n+1){-1} // also store parents
        toDo.addLast(distRoot)
        toDo.addLast(distRoot2)
        explored[distRoot] = -2
        explored[distRoot2] = -2
        val dist = IntArray(n){-1}
        dist[distRoot] = 0
        dist[distRoot2] = 0
    ...
    inline fun trueOrderDFS(root:Int?,newroot:(r:Int) ->Unit, preexplore:(v:Int) ->Unit, postExplore:(v:Int)->Unit , crossStack:(v:Int,w:Int)->Unit){
        val Q = fastDeque(0,n)
...
fun Graph.getAllCyclesEnds():MutableList<Pair<Int,Int>>{
    val visited = BooleanArray(n)
    val onstack = BooleanArray(n)
    var ret:MutableList<Pair<Int,Int>> = mutableListOf()
    trueOrderDFS(0,{},{},{},{v,w -> ret.add(Pair(v,w))})
    return ret
}
fun Graph.isBipartite():Boolean{
    val types = IntArray(n)
    anyOrderExplore(null,{r->types[r] =1}){
            from, to,root,e, spanning ->
        if(spanning){
            types[to] = -types[from]
        }else{
            if(types[to] == types[from]){
                return false
            }
        }
    }
...
fun main(){
    solve.cases{
        ...
        if(G.isBipartite()){
            put("YES")
            val dist = G.BFS(0)
            val ret = BooleanArray(n){dist[it] %2 == 0}
            put(ret.map { if(it) '1' else '0' }.joinToString(""))
            return@cases
        }
        val cycles = G.getAllCyclesEnds()
        val T = Graph(n,n-1,false)
        for(i in 1 until n) {
            T.add(G.parent[i],i)
        }
        var odds = 0
        ...
        T.treeOrderDFS(false)
        T.prepareLifting()
        val allodds = mutableListOf<Pair<Int,Int>>()
        for((v, w) in cycles) {
            val isodd = T.dist(v, w) % 2 == 0
            if(isodd) odds ++
            if(isodd){
                allodds.add(Pair(v,w))
            }
        ...
        fun trythis(a:Int, b:Int):Boolean {
            val d = G.BFS2(a,b)
            val ret = BooleanArray(n) {d[it] and 1 == 0
            }
...
//                    continue
//                }
//                val d = minOf(T.dist(a,v), T.dist(b,v))
//                ret[v] = d %2 == 0
//            }
            var bad = 0
            G.everyEdge { x, y ->
            ...
            put("YES")
            put(ret.map{if(it) '1' else '0'}.conca())
            return true
        }
        if(allodds.size == 1){
            val (a,b) = allodds[0]
