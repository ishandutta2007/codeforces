...
void solve(){
    ...
    int now = d + 2;
    for (int i = 1; i <= d+1; ++i) {
        int cnt = min(i - 1, d+1 - i);
        if (cnt == 0) continue;
        queue<pair<int, int>> next;
        REP(q, k - 2) {
            if (n == 0) break;
            n--;
            edges.push_back(mp(i, now));
            next.push(mp(now, cnt - 1));
            now++;
        }
        while (next.empty() == false) {
            pair<int, int> go = next.front();
            next.pop();
            if (go.second == 0) continue;
            REP(q, k - 1) {
                if (n == 0) break;
                n--;
                edges.push_back(mp(go.first, now));
                next.push(mp(now, go.second - 1));
                now++;
            }
        }
    }
