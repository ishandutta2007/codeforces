...
class EMaksimalnayaPodposledovatelnost {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        ...
        auto f = process(arri(a.begin(), a.begin() + n / 2));
        auto s = process(arri(a.begin() + n / 2, a.end()));
        int answer = (f.back() + s.back()) % m;
        int at = s.size() - 1;
        for (int i : f) {
            while (at >= 0 && i + s[at] >= m) {
                at--;
...
int main() {
    ...
    auto time = clock();
#endif
    EMaksimalnayaPodposledovatelnost solver;
