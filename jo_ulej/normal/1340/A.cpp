...
void solve() {
	int n = fetch<int>();
	auto p = fetch_vec<int>(n);
	for(auto& x: p) --x;
	vector<int> q(n, NIL);
	for(int i = 0; i < n; ++i) q[p[i]] = i;
	set<int> setik;
	setik.insert(NIL);
	vector<int> dp(n, -INT_INF);
	multiset<int> dpVals;
	for(auto& x: dp) dpVals.insert(x);
	for(int i = n - 1; i >= 0; --i) {
		int j = q[i];
		setik.insert(j);
		auto iter = setik.find(j);
		auto nxt = iter;
		++nxt;
		if(nxt != setik.end()) {
			dpVals.erase(dpVals.find(dp[*nxt]));
			dp[*nxt] = (*nxt) - j;
			dpVals.insert(dp[*nxt]);
		}
		assert(iter != setik.begin());
		auto prv = iter;
		--prv;
		dpVals.erase(dpVals.find(dp[j]));
		dp[j] = j - (*prv);
		dpVals.insert(dp[j]);
		auto last = dpVals.end();
		--last;
		if(dp[j] != *last) {
			cout << "No\n";
			return;
