...
void solve(ll) {
  auto n = input<ll>();
  auto k = input<ll>();
  auto x = input<std::vector<ll>>(n);
  ALL(std::sort, x);
  DUMP(x);
  auto l = ll(0);
  auto r = ll(n);
  auto ans = ll(0);
  while (l + k - 1 < n && x[l + k - 1] < 0) {
    ans += -x[l];
    l += k;
  }
  while (0 <= r - k && 0 < x[r - k]) {
  ...
  if (auto b = x | sliced(l, r)) {
    ans += std::max(b.back(), ll(0)) - std::min(b.front(), ll(0));
  }
  ans *= 2;
  ans -= std::max(std::abs(x[0]), std::abs(x.back()));
  println(ans);
}