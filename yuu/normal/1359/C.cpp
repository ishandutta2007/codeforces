...
void solve() {
    ...
    cin >> h >> c;
    cin >> t;
    // t = (n_hot * h + n_cold * c) / (n_hot + n_cold)
    // let n_hot be x, then n_cold must be x or x - 1
    // n_cold = x
    // t = x * (h + c) / (2x) = (h + c) / 2
    ...
    // 2xt - t = xh + xc - c
    // x (2t - h - c) = t - c
    long double nice_x = static_cast<long double>(t - c) / (2 * t - h - c);
    for (int64_t x = nice_x - 2; x <= nice_x + 2; x++) {
        if (x < 1) continue;
        long double res = abs(static_cast<long double>(x * h + (x - 1) * c) / (x + x - 1) - t);
        if (res < ans) {
            ans = res;
            ans_cnt = x + x - 1;
        }
