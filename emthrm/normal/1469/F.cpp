...
int main() {
  ...
  if (accumulate(ALL(l), 0LL) - n * 2 + 1 < k) {
    cout << "-1\n";
    return 0;
  }
  sort(ALL(l), greater<int>());
  using M = monoid::RangeSumAndAddQuery<ll>;
  LazySegmentTree<M> seg(M::init(400000));
  seg.set(0, M::Node{1, 1});
  int ans = INF;
  ll white = 1;
  REP(i, n) {
    int p = seg.find_right(0, [](const M::Node &x) { return x.sum == 0; });
    seg.apply(p, p + 1, -1);
    seg.apply(p + 2, p + 2 + (l[i] - 1) / 2, 1);
    seg.apply(p + 2, p + 2 + l[i] / 2, 1);
    white += l[i] - 2;
    if (white >= k) chmin(ans, seg.find_right(0, [&](const M::Node &x) { return x.sum < k; }));
  }
  cout << ans << '\n';
  return 0;
}