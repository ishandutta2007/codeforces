...
int main(void)
{
	...
	rep(i, 1, n) cin >> a[i];
	rep(i, 1, m) cin >> b[i];
	rep(i, 1, m) vec[b[i]].pb(i);
	
	rep(i, 0, 300) rep(j, 0, n) dp[i][j] = inf;
	dp[0][0] = 0;
	rep(i, 1, 300){
		mn[0] = dp[i-1][0];
		rep(j, 1, n) mn[j] = min(mn[j-1], dp[i-1][j]);
		rep(j, 1, n){
			ll p = upper_bound(all(vec[a[j]]), mn[j-1]) - vec[a[j]].begin();
			if(p >= sz(vec[a[j]])) continue;
			dp[i][j] = vec[a[j]][p];
		}
	}
