...
class obsta(val l:Int, val r:Int, val d:Int){
...
fun main(){
    solve.cases{
        ...
        A.radixsort()
        A.reverse()
        val all = MutableList(k){
            obsta(getint(), getint() , getint() )
        }
        all.sortBy { it.l }

        val ans = BinarySearchLastTrue(0,m){
            val power = if(it == 0) Int.MAX_VALUE else A[it-1]
            var time = n+1
            var L:Int? = null
            var R:Int? = null
            for(c in all){
                if(c.d <= power ) continue
                if(L != null){
                    if(c.l - 1> R!!){
                        val cost = (R -L) * 2
                        time += cost
