...
main() {
    ...
    rep(i, n) {
        ll mi = *mul.begin();
        pq.emplace(a[i] + 1440, mi + 120);
        pq.emplace(a[i] + 90, mi + 50);
        pq.emplace(a[i] + 1, mi + 20);
        for(auto e : {20, 50, 120}) mul.emplace(mi + e);
        while(pq.top().fi <= a[i]) {
            mul.erase(mul.lower_bound(pq.top().se));
            pq.pop();
        }
        cout << *mul.begin() - pre << endl;
        pre = *mul.begin();
    }
}