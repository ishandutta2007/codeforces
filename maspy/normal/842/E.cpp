...
struct EulerTour {
  ...
  int LA(int v, int n){
    FOR(k, 20) if(n & 1<<k) v = ancestors[k][v];
    return v;
  }  
...
void solve() {
  ...
  auto remove_range = [&](ll l, ll r) -> void {
    while (1) {
  ...
  FOR3(v, 1, N) {
    ...
    if (d < radius) {
      ...
      int new_center = ET.move(center, v);
      radius++;
      if (ET.in_subtree(new_center, center)) {
        // new_center 
        ll l = LID[new_center], r = RID[new_center];
        remove_range(l, r);
      } else {
        // center 
        ll l = LID[center], r = RID[center];
        remove_range(0, l);
        remove_range(r, N + N);
      }
      center = new_center;
