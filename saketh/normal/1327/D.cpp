...
void solve() {
    ...
    vi perm(N); re(perm); trav (e, perm) --e;
    vi color(N); re(color);
    ...
    F0R (i, N) {
        ...
        for (int t = perm[i]; !visit[t]; t = perm[t]) {
            cyc.pb(t);
            visit[t] = true;
        }

        sv.for_each_divisor_unordered(sz(cyc), [&](int d) {
                int mod = d;
                fill(group.begin(), group.begin() + mod, -1);
                int r = 0;
                for (int i = 0; i < sz(cyc); i++) {
                    if (++r == mod) r = 0;
                    if (group[r] == -1) group[r] = color[cyc[i]];
                    else if (group[r] != color[cyc[i]]) group[r] = -2;
                }
                if (*max_element(group.begin(), group.begin() + mod) > -1)
                    ckmin(ans, d);
        });
    }
...
int main() {
    ...
    /*double worst = 0.;
    for (int v = 1; v < int(2e5 + 1); v++) {
        ll tot  = 0;
        sv.for_each_divisor_unordered(v, [&](int d){ tot += d; });
        ckmax(worst, tot / double(v));
    }
    cerr << worst << endl;*/
