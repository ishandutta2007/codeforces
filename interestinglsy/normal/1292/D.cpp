...
int primeE[MAXK][MAXK];
void GetPrimeE(){
	Forx(i,2,MAXK-1){
		memcpy(primeE[i],primeE[i-1],sizeof primeE[i]);
		int t = i;
		for( int j = 2 ; j*j <= t ; ++j ){
			while( t%j == 0 ){
				++primeE[i][j];
...
ll Solve(){
	static int largest[MAXK];
	ll ans = 0;
	ll curAns = 0;

	Forx(i,0,MAXK-1){
		if(!cnt[i]){ largest[i] = 1; continue; }
		Forx(j,2,i){
			if(primeE[i][j]){
				curAns += 1LL*primeE[i][j]*cnt[i];
				largest[i] = j;
			}
		}
	}
	...
	while( *max_element(largest+1,largest+MAXK) > 1 ){
		static int counter[MAXK]; Ms(counter);
		Forx(i,0,MAXK-1) counter[largest[i]] += cnt[i];
		int p = max_element(counter+1,counter+MAXK) - counter;
		if( p == 1 ) break;
		if( counter[p]*2 <= n ) break;
		curAns += n - 2*counter[p];
		MyMin(ans,curAns);
		Forx(i,0,MAXK-1){
			if( largest[i] != p ) largest[i] = 1;
			if( largest[i] == 1 ) continue;
			--primeE[i][largest[i]];
			while( largest[i] > 1 and primeE[i][largest[i]] == 0 )
				--largest[i];
		}
	}
