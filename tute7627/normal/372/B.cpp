...
int main(){
  ...
  Sum2D<int>sum(a);
  auto b=make_v(h,h,w,w,0);
  rep(x1,0,h)rep(y1,0,w)rep(x2,x1,h)rep(y2,y1,w){
    if(sum.query(x1,y1,x2,y2))continue;
    b[x1][x2][y1][y2]++;
  }
  rep(i,0,h)rep(j,0,h)rep(o,0,w)rep(k,0,w-1)b[i][j][o][k+1]+=b[i][j][o][k];
  rep(i,0,h)rep(j,0,h)rrep(o,0,w-1)rep(k,0,w)b[i][j][o][k]+=b[i][j][o+1][k];
  rep(i,0,h)rep(j,0,h-1)rep(o,0,w)rep(k,0,w)b[i][j+1][o][k]+=b[i][j][o][k];
  rrep(i,0,h-1)rep(j,0,h)rep(o,0,w)rep(k,0,w)b[i][j][o][k]+=b[i+1][j][o][k];
  while(q--){
    int x,y,z,w;cin>>x>>y>>z>>w;x--;y--;z--;w--;
    cout<<b[x][z][y][w]<<endl;
  }
  return 0;
}