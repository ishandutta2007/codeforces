...
//xvector
#define ALL(a)  (a).begin(),(a).end() //sort
...
void solve(){
 ...
 // cout<<y<<" "<<x<<" "<<p<<endl;
  ...
  ans.at(0)=a.at(y);
  for(int i=0;i<n;i++){
    ans.at(i).at(0)=b.at(x).at(i);
  }
  if(n<=m){
    vector<vector<ll>> ans2(n, vector<ll>(m));
    sor(a);
    vector<ll> q(n);
    for(int i=0;i<n;i++){
      q.at(i)=a.at(i).at(0);
    ...
    for(int i=0;i<n;i++){
      ll tmp=ans.at(i).at(0);
      auto Iter1 = lower_bound(ALL(q),tmp);
      ll xx=Iter1-q.begin();
      ans2.at(i)=a.at(xx);
    }
    for(int i=0;i<n;i++){
    ...
    vector<vector<ll>> ans2(m, vector<ll>(n));
    sor(b);
    vector<ll> q(m);
    for(int i=0;i<m;i++){
      q.at(i)=b.at(i).at(0);
