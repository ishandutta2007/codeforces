...
vector<set<pair<int, int>>> inoutset;

set<pair<int, int>> allsegs, leaves;
...
void add(int v)
{
    auto pr = mp(in[v], out[v]);
    allsegs.insert(pr);
    min_in = min(min_in, in[v]);
    max_out = max(max_out, out[v]);
    ...
    auto it = leaves.lower_bound(mp(out[v]+1, -1));

    if (it==leaves.begin())
    {
        leaves.insert(pr); return;
    }
    ...
    if (pr1.first<pr.first && pr1.second>pr.second)
    {
        leaves.erase(pr1);
        leaves.insert(pr);
    }
    else if (pr1.second<pr.first)
    {
        leaves.insert(pr);
    }
}
...
int cnt()
{
    if (allsegs.size()<=1) return allsegs.size();
    int ans = leaves.size();

    int v = mapin[min_in];
    if (out[v]!=max_out) return ans;

    //cout<<"CHECKING"<<endl;

    int L = leaves.begin()->first;
    int R = prev(leaves.end())->second;

    auto it = inoutset[v].lower_bound(mp(L, 1e9));
    it = prev(it);
    if ((it->second)>=R) ans++;
    return ans;
}
...
int main()
{
    ...
    for (int i = 0; i<n; i++)
    {
        for (auto it: children[i]) inoutset[i].insert(mp(in[it], out[it]));
    }

    vector<pair<int, int>> order;

    for (int i = 0; i<n; i++) order.push_back(mp(h[i], i));
    ...
    for (auto it: order)
    {
        //cout<<"Processing: "<<it.second<<" with "<<in[it.second]<<' '<<out[it.second]<<endl;
        add(it.second);
        int newsz = cnt();
        if (newsz>cursz)
        {
    ...
    ans+=(order[0].first-order[1].first);
