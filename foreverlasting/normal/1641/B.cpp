...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        for(res i=1;i<=n;){
            res j=i;
            while(b[i]==b[j]&&j<=n)j++;
            if((j-i)&1){puts("-1");return;}
            i=j;
        }
        res now=1,tag=0;
        while(233){
            if(now>n)break;
            for(res i=now+1;i<=n;i++){
                if(a[i]==a[now]){
                    for(res j=now;j<i;j++)b[j]=a[j];
                    tag+=i-now;
                    res L=now,R=i;
                    while(233){
                        if(b[L]!=a[R]){
...
//                        printf("%d %d %d %d\n",L,R,a[L],a[R]);
                    ...
                    ans.pb((i-now)*2),now=R;
                    break;
                }
            }
        }
        printf("%d\n",int(con.size()));
        for(auto x:con)printf("%d %d\n",x.fi,x.se);
        printf("%d\n",int(ans.size()));
        for(auto x:ans)printf("%d ",x);puts("");
        for(res i=1;i<=n;i++)a[i]=0;
    }
}
int main(){
//    srand(time(0));
