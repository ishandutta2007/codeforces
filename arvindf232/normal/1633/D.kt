...
fun main(){
    val steps = IntArray(1001){plarge}
    steps[0] = 0
    steps[1] = 0
    for(i in 1..1000){
        for(x in i downTo 1){
            val new = i + (i / x)
            if(new > 1000) continue
            steps[new] = minOf(steps[new],steps[i] + 1 )
        }
    }
//    val max = 13
//    just dei DP.maxOrNull()!!
    solve.cases{
        val n = getint()
        var k = getint()
        k = minOf(k,13 * n)
        val target = getline(n)
        val profit = getline(n)
        ...
        for(i in 0 until n){
            for(pre in 0..k){
                DP[i+1][pre] = maxOf(DP[i+1][pre], DP[i][pre])
                val here = DP[i][pre]
                val need = steps[target[i]]
                if(pre + need > k) continue
                DP[i+1][pre+need] = maxOf(DP[i+1][pre+need] ,here + profit[i])
            }
        }
        put(DP.last().maxOrNull()!!)
    }
    done()
}
