...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let mut v: Vec<(i64, i64)> = cin.read_vec(n);
    v.sort_by_key(|(a, b)| a - b);
    let b = v.partition_point(|&(a, b)| a < b) as i64;
    let a = n as i64 - b;

    let mut suma = vec![0i64; n + 1];
    let mut sumb = vec![0i64; n + 1];
    for i in 0..n {
        suma[i + 1] = v[n - 1 - i].0;
        sumb[i + 1] = v[i].1;
    }
    ...
    for _ in 0..m {
        let (u, v): (i64, i64) = cin.read();
        let g = gcd(u, v);
        if n as i64 % g != 0 {
            outln!(-1);
            continue;
        }
        let (x0, y0) = egcd(u, v, n as i64);
        let t0 = (a - u * x0) / (u * (v / g));
        let mut ans: i64 = -1;
        for t in t0 - 5..t0 + 5 {
            let x = x0 + t * v / g;
            let y = y0 - t * u / g;
            assert_eq!(x * u + y * v, n as i64);
            if x < 0 || y < 0 {
                continue;
            }
            ans = ans.max(suma[(x * u) as usize] + sumb[(y * v) as usize]);
        }
        outln!(ans);
    }
}
...
///  # Bundled libraries
/// 
///  - `rlib_gcd 0.1.0 (path+)`         published in https://github.com/maksim1744/rlib licensed under `MIT` as `crate::__cargo_equip::crates::rlib_gcd`
...
mod __cargo_equip {
    pub(crate) mod crates {
        pub mod rlib_gcd {use crate::__cargo_equip::preludes::rlib_gcd::*;use rlib_integer::*;pub fn gcd<T:Integer>(a:T,b:T)->T{let mut a=a.abs();let mut b=b.abs();while b!=T::ZERO{a%=b;std::mem::swap(&mut a,&mut b);}a}pub fn lcm<T:Integer>(a:T,b:T)->T{a.abs()/gcd(a,b)*b.abs()}pub fn egcd<T:Integer>(a:T,b:T,c:T)->(T,T){if a==T::ZERO{assert!(c%b==T::ZERO);return(T::ZERO,c/b);}let(y0,x0)=egcd(b%a,a,c);(x0-(b/a)*y0,y0)}}
    ...
    mod preludes {
        pub mod rlib_gcd {pub(in crate::__cargo_equip)use crate::__cargo_equip::crates::rlib_integer;}
        pub mod rlib_integer {}
