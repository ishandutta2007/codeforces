...
void calc(int l1,int r1)
{
	...
	for (int i=0;i<q;++i)
	{
		if (po>=l[i] && po<r[i])
		{
			int le=po-max(l[i],l1)+1,ri=min(r[i],r1)-po;
			//cout<<i<<' '<<l[i]<<' '<<r[i]<<' '<<po<<' '<<le<<' '<<ri<<' '<<seg[po+M].x<<en;
			ans[i]+=le*1ll*ri*seg[po+M].x;
		}
	}
	calc(l1,po);
	calc(po+1,r1);
...
int main()
{
	...
	for (int i=0;i<n-1;++i) seg[i+M]={abs(h[i+1]-h[i]),i};
