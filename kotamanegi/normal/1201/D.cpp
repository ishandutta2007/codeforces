...
void solve(){
    ...
    pair<int,int> l,r;
    sort(ALL(inputs[0]));
    l = mp(inputs[0].back() * 2 - inputs[0][0], inputs[0][0]);
    r = mp(inputs[0].back(), inputs[0].back());
    for (int i = 1; i < n; ++i) {
        if (inputs[i].size() == 0) continue;
        sort(ALL(inputs[i]));
        pair<int, int> next_l = mp(1e18, inputs[i][0]);
        pair<int, int> next_r = mp(1e18, inputs[i].back());
        REP(q, 2) {
            auto x = lower_bound(ALL(place), l.second);
            if (x != place.end()) {
                //check right
                int StartPoint = *x;
                next_l.first = min(abs(inputs[i].back() - StartPoint) + abs(l.second - StartPoint) + l.first, next_l.first);
                next_r.first = min(abs(inputs[i][0] - StartPoint) + abs(l.second - StartPoint) + l.first, next_r.first);
            }
            if (x != place.begin()) {
                //check left
                x--;
        ...
        next_l.first += inputs[i].back() - inputs[i][0];
        next_r.first += inputs[i].back() - inputs[i][0];
        l = next_l;
        r = next_r;
    }
    cout << min(l.first, r.first) + addition << endl;
}
#undef int
int main() {
    init();
