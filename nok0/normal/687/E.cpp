...
void main_() {
	INT(n, m);
	debug(n, m);
	graph g(n, m, 0, 1), rg(n);
	REP(i, n) {
		for(auto [to, cost] : g[i]) {
			rg.add_edge(to, i, cost, 1);
		}
	}
	V<> ok_ver(n);
	REP(i, n) {
	...
	REP(i, n) {
		for(auto [to, cost] : g[i]) {
			if(!ok_ver[i] and !ok_ver[to]) {
				nex_g.add_edge(i, to, 1, 1);
			}
		}
	}
	strongly_connected_components scc(nex_g);
	auto sg = scc.graph_compressed;
	auto sn = sg.size();
	VV<> s_sz(sn);
	REP(i, n) {
		s_sz[scc[i]].pb(i);
	...
	debug(ok_ver);
	auto minimal_cycle_size = [&](graph g) -> int {
		int res = inf;
		int k = SZ(g);
		VV<> candi(k);
		REP(i, k) {
	...
	REP(i, sn) {
		...
		if(SZ(sg[i])) {
			...
			V<> cur;
			set<int> st;
			foa(v, vec) st.insert(v), cur.pb(v);
			graph tmp(SZ(st));
			REP(i, n) {
				if(!st.count(i)) continue;
				for(auto [to, cost] : nex_g[i]) {
					if(!st.count(to)) continue;
					tmp.add_edge(lb(cur, i), lb(cur, to), 1, 1);
				}
			}
			int min_sz = minimal_cycle_size(tmp);
			res += min_sz * 998 + 1;
		}
	}
	print(res);
