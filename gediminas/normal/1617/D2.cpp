...
int ask(int a, int b, int c) {
	if (a < 1 or b < 1 or c < 1 or a > __n or b > __n or c > __n or a == b or b == c or a == c) {
...
void solve(int n) {
	...
	for (int i = 1; i <= n; i += 3) {
		if (i == skimp || i == skger || i + 1 == skimp || i + 1 == skger || i + 2 == skimp || i + 2 == skger) {
			continue;
		}

		if (k[i] == 0) {
			if (ask(i, i + 1, skger) == 0) {
				imp.push_back(i);
				imp.push_back(i + 1);

				if (ask(i + 2, i + 1, skger) == 0) {
					imp.push_back(i + 2);
				}
				else {
				}
			...
			if (ask(i, i + 1, skimp) == 1) {
				if (ask(i + 2, i + 1, skimp) == 1) {
				}
				else {
					imp.push_back(i + 2);
				}
	...
	for (int ii = 1; ii <= n; ii += 3) {
		if (ii == skimp || ii == skger || ii + 1 == skimp || ii + 1 == skger || ii + 2 == skimp || ii + 2 == skger) {
