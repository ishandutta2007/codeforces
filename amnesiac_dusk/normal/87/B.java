...
public class Main
{
	...
	{	
		int n=ni();
		Map<String,Integer> voi=new HashMap<String,Integer>();
		voi.put("void", 0);
		for(int i=0; i<n; i++)
		{
			if(ns().equals("typedef"))
			{
				String hola=ns(),vola=ns();
				int st=0,j=0;
				for(j=0; j<hola.length(); j++)
				{
					if(hola.charAt(j)=='&')
						st++;
					else if(hola.charAt(j)=='*')
						break;
				}
				if(!voi.containsKey(hola.substring(st, j)))
					voi.put(vola,-1);	
				else if(voi.get(hola.substring(st, j))>=0)
					voi.put(vola,voi.get(hola.substring(st, j))-st+hola.length()-j);
				else
					voi.put(vola,-1);	
			}
			else
			{
				...
				if(!voi.containsKey(hola.substring(st, j)))
					pr("errtype");
				else if(voi.get(hola.substring(st, j))>=0&&voi.get(hola.substring(st, j))-st+hola.length()-j>=0)
					pr("void"+pr("*",voi.get(hola.substring(st,j))-st+hola.length()-j));
				else
					pr("errtype");
			}
		}
