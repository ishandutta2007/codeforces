...
ll N;
ll prmax(ll i,ll j,bool f=true){
  cout<<"? 1 "<<i+1<<" "<<j+1<<" "<<N-1<<"\n";
  ll x;
  cin>>x;
  if(x==N-1&&f)return prmax(j,i,false);
  return x;
}
ll prmin(ll i,ll j,bool f=true){
  cout<<"? 2 "<<i+1<<" "<<j+1<<" "<<1<<"\n";
  ll x;
  cin>>x;
  if(x==2&&f)return prmin(j,i,false);
  return x;
}
void prans(vec& p){
  cout<<"! ";
...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  if(i==N){
    rep1(i,N)if(p[i]==1)break;
    assert(i<N);
    p[0]=prmax(0,i);
    if(p[0]<N){
      assert(p[0]==N-1);
      rep1(i,N)if(p[i]==N-1)++p[i];
    }
    prans(p);
    continue;
  }
  ll j;
  rep1(j,i+1)if(p[j]==p[0])p[j]=prmax(0,j);
  if(p[0]<N/2){
    For(j,i+1,N){
      p[j]=prmax(0,j);
      if(p[j]==p[0])p[j]=prmin(0,j);
    }
  }else{
    For(j,i+1,N){
      p[j]=prmin(0,j);
      if(p[j]==p[0])p[j]=prmax(0,j);
    }
  }
  prans(p);
}
