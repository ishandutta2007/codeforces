...
const int maxn = 5e5 + 100, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 600, X = 43, LG = 17;

int slow(multiset<int> G) {
    priority_queue<ll> g;
    for (int i : G)
        g.push(-i);
    int ans = 0;
...
int main()
{
    ...
    cout.tie(0);
        ...
        for (int i = 0; i < n; i++) {
            ...
            k--;
            if (c == '+')
                q[k] += w, p[k]++, st[k].insert(w);
            else
                q[k] -= w, p[k]--, st[k].erase(st[k].find(w));
            int ans = 0;
            ll s = 0;
            for (int i = 0; i < 30; i++)
            if (p[i] > 0) {
