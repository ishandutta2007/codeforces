...
int nie[maxn], ki[maxn], S;
int A[maxn], B[maxn];

int main()
{
    scanf("%d%d", &n, &m);
    FOR(i, 1, m) scanf("%d", &nie[i]);
    FOR(i, 1, m) scanf("%d", &ki[i]);
    prep(m);
    for (int i=1; i<=m; ++i)
    {
        scanf("%d%d", &x.e1.e2, &x.e2.e1);
        A[i] = x.e1.e2; B[i] = x.e2.e1;
        x.e1.e1 = nie[i];
        kraw[i] = x;
        kraw[i].e2.e2 = i;
    }
    scanf("%d", &S);
    FOR(i, 1, m) ki[i] = (S / ki[i]);
    
    sort(kraw+1, kraw+m+1);
    ll cost = 0, nad = 0, usun = 0;
    for (int i=1; i<=m; ++i)
    {
        a = kraw[i].e1.e2;
        b = kraw[i].e2.e1;
    ...
    for (int i=1; i<=m; ++i)
    {
        ...
        ll wyn2 = cost - co + kraw[i].e1.e1 - ki[kraw[i].e2.e2];
        if (wyn2 < wyn) {
			wyn = wyn2;
			usun = kraw[i].e2.e2;
		}
    ...
    //skoro juz wiemy jaka to byla krawedz, to teraz trzeba ja wyrzucic
    FOR(i, 1, n) f[i] = i, ile[i] = 1;
    _union(A[usun], B[usun]);
    FOR(i, 1, m) {
		w_drzewie[i] = 0;
		if (kraw[i].e2.e2 == usun) w_drzewie[i] = 1;
		else 
		{
			a = kraw[i].e1.e2;
			b = kraw[i].e2.e1;
			if (_union(a,b) == false) w_drzewie[i] = 1;
		}
	}
	printf("%lld\n", wyn);
	FOR(i, 1, m)
