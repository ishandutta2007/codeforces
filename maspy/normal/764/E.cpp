...
void solve() {
  ...
  VEC(ll, A, N);
  if (MIN(A) == MAX(A)) return print(MIN(A), 0);

  if (N % P == 0) {
    auto CNT = bincount(A, P);
    if (MIN(CNT) != MAX(CNT)) return print(-1);
    return print(0, 1);
  }
  if (N % P == 1) {
    auto CNT = bincount(A, P);
    if (MIN(CNT) + 1 != MAX(CNT)) return print(-1);
    ll x = max_element(all(CNT)) - CNT.begin();
    return print(x, 1);
  }
  if (N % P == P - 1) {
    auto CNT = bincount(A, P);
    if (MIN(CNT) + 1 != MAX(CNT)) return print(-1);
    ll x = min_element(all(CNT)) - CNT.begin();
    x = (x + 1) % P;
    return print(x, 1);
  }
  ...
  mint avg = S1 / mint(N);
  mint var = S2 / mint(N) - avg * avg;
  mint dd = var * mint(12) / mint(N * N - 1);
  mint d = mod_sqrt(dd);
  if (dd != d * d) return print(-1);
  mint a = avg - mint(N - 1) * d / mint(2);
  vi B;
  FOR(i, N) B.eb((a + d * i).val);

  sort(all(A));
  sort(all(B));
  if (A == B) return print(a, d);
  print(-1);
}
