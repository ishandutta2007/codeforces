...
int main()
{
    ...
    for(int i=1;i<=n;i++)
    {
        add_edge(s,2*i-1,1,c[a[i]]);
        add_edge(2*i,t,1,0);
        add_edge(2*i-1,2*i,1,-100000001);
    }
    for(int i=1;i<=n;i++)
        for(int j=i+1;j<=n;j++)
        {
            if(a[i]==a[j]) add_edge(2*i,2*j-1,1,0);
            else add_edge(2*i,2*j-1,1,c[a[j]]);
        }
    printf("%lld\n",1LL*n*(100000001)+min_cost_flow(s,t,k));
    return 0;
}