...
ll calc(int l, int r, int x) {
    if (l > r) return 0;
    if (ans[l][r][x]) return ans[l][r][x];
    ckmax(ans[l][r][x],a[x+1]+calc(l+1,r,0));
    // collapse l+1 to t-1
    FOR(t,l+1,r+1) if (s[l] == s[t]) ckmax(ans[l][r][x],calc(l+1,t-1,0)+calc(t,r,x+1));
    return ans[l][r][x];
}

int main() {
    // you should actually read the stuff at the bottom
    setIO(); re(n,s); FOR(i,1,n+1) re(a[i]);
    pr(calc(0,n-1,0));
