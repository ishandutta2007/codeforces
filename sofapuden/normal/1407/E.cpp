...
void solve(){
	...
	for(int i = 0; i < sz(from[n-1]); ++i){
		Q.push({{from[n-1][i].f,n-1},{from[n-1][i].s,1}});
	}
	int dis = -1;
	while(!Q.empty()){
		auto x = Q.front();
		Q.pop();
		if(v[x.f.f].s != -1)continue;
		if(v[x.f.f].f == -1){
			v[x.f.f].f = x.s.f^1;
			continue;
		}
		if(v[x.f.f].f == (x.s.f^1))continue;
		if(x.f.f == 0){dis = x.s.s; break;}
		v[x.f.f].s = 0;
		for(int i = 0; i < sz(from[x.f.f]); ++i){
			if(from[x.f.f][i].f == x.f.s)continue;
			Q.push({{from[x.f.f][i].f,x.f.f},{from[x.f.f][i].s,x.s.s+1}});
		}
	}
	cout << dis << "\n";
	for(auto i : v){
