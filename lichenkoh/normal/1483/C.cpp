...
vi h,b;
MinSegTree<mn> seg;
ll f(int l, int r, int bl, int br) {
	if (l>r) return 0;
	int p=seg.query(l,r).snd;
	ll ansL=f(l,p-1,bl,1);
	if (bl) chkmax(ansL,0LL);
	ll ansR=f(p+1,r,1,br);
	if (br) chkmax(ansR,0LL);
	ll ans=ansL+b[p]+ansR;
	if (bl) chkmax(ans, ansR);
	if (br) chkmax(ans, ansL);
	//printf("l:%d r:%d bl:%d br:%d p:%d ans:%lld\n",l,r,bl,br,p,ans);
	return ans;
} 
...
int main() 
{
  ...
  seg.init(n);
	for (int i=0;i<n;i++) seg.t[n+i]=MP(h[i],i);
	seg.build();
	ll final=f(0,n-1,0,0);
	printf("%lld\n",final);
}
