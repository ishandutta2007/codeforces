...
int main() {
    ...
    while(TT--) {
        ...
        vi fir,fro;
        rep(i,n) {
            if(a[i]) fir.emplace_back(b[i]);
            else fro.emplace_back(b[i]);
        }
        sort(all(fir));
        sort(all(fro));
        ll ans = 0;
        if(fir.size() == fro.size()) {
            ans = (sum(fir) + sum(fro)) * 2 - min(fir[0],fro[0]);
        }
        else {
            if(fir.size() > fro.size()) swap(fir,fro);
            ans += fro[0];
            if(!fir.empty())ans += sum(fir) * 2;
            rep(i,fro.size() - fir.size(),fro.size()) ans += 2 * fro[i];
            rep(i,1,fro.size() - fir.size()) ans += fro[i];
        }
        cout << ans << '\n';
    }
}