...
public:
    void solve() {
        ...
        arr3d<arr<ll>> answer(n + 1, n + 1, 1 << n, arr<ll>());
        function<arr<ll>(int, int, int, int)> go = [&](int left, int right, int mask, int size) -> arr<ll> {
            if (answer(left + 1, right + 1, mask).size()) {
                return answer(left + 1, right + 1, mask);
            }
            arr<ll> ans(1 << ((left != -1) + (right != -1) + size - 1), 0);
            if (size == 0) {
                if (left != -1 && right != -1) {
                    ans[r(left, right)] = 1;
                } else {
                ...
                return answer(left + 1, right + 1, mask) = ans;
            }
            if (size == 1) {
                int a = -1;
                for (int i : range(n)) {
            ...
            for (int i = mask; i; i = (i - 1) & mask) {
                if (bitCount(i) == mid) {
                    for (int j : range(n)) {
                        if ((mask >> j & 1) && !(i >> j & 1)) {
                            auto lft = go(left, j, i, mid);
                            auto rht = go(j, right, mask - i - (1 << j), size - mid - 1);
                            for (int k : range(lft.size())) {
                                for (int l : range(rht.size())) {
                                    ans[k * rht.size() + l] += lft[k] * rht[l];
                                }
                            }
                        }
                    }
