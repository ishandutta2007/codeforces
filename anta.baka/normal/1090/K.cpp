...
struct num {
    ...
    bool operator < (const num &he) const { return a<he.a || a==he.a&&b<he.b; }
};
...
main() {
    ...
    for(int i = 0; i < n; i++) {
        string s, t; cin >> s >> t;
        int m = 0;
        for(char ch : t) m |= (1 << (ch - 'a'));
        while(!s.empty() && (m & (1 << (s.back() - 'a'))) != 0) s.pop_back();
        num h = 0;
        for(char ch : s) {
            h = h * P + ch;
        }
    ...
    cout << sz(mapik) << '\n';
    for(auto it = mapik.begin(); it != mapik.end(); it++, cout << '\n') {
        vi& v = it->se;
        cout << sz(v) << ' ';
