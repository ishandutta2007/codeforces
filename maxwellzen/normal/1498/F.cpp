...
void dfs(int i, int p) {
    cnt[i].rsz(2*k, 0); cnt[i][0] = a[i];
    for (int nb : adj[i]) if (nb != p) {
        dfs(nb,i);
        F0R(j, 2*k) cnt[i][(j+1)%(2*k)] ^= cnt[nb][j];
    }
}
void dfs2(int i, int p, vll above) {
    ll sum = 0;
    FOR(j, k, 2*k) sum ^= cnt[i][j] ^ above[j];
    debug(i, cnt[i], above);
    if (sum==0) ans[i]=false;
    else ans[i]=true;
    for (int nb : adj[i]) if (nb != p) {
        vll cur(2*k, 0);
        F0R(j, 2*k) cur[(j+1)%(2*k)] = above[j] ^ cnt[i][j] ^ cnt[nb][(j+2*k-1)%(2*k)];
        dfs2(nb, i, cur);
    }
}
