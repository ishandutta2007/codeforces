...
fun main(){
    solve.cases{
        ...
        repeat(q){
            ...
            val tree = L.sortedBy { intime[it] }
            val realtree = mutableSetOf<Int>()
            for(a in L){
                realtree.add(a)
            }
            for(a in tree.windowed(2)){
                val (x,y) = a
                realtree.add(G.LCA(x,y))
            }
            val tree2 = realtree.sortedBy { intime[it] }
            val degree = multiset()
            val stack = ArrayDeque<Int>()
            for(a in tree2){
                while(stack.isNotEmpty()){
                    if(!isancestor(stack.last(),a)){
                        stack.removeLast()
                    }else{
                        break
                    }
                ...
                if(stack.isNotEmpty()){
                    degree.addone(stack.last())
                    degree.addone(a)
                }
                stack.addLast(a)
            }
            put(if(degree.st.values.all { it <= 2 }){ "YES"} else "NO")
        }
