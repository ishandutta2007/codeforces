...
void build(int cur, int l, int r) {
    if (l == r) {
        ...
        build(dcur, l, m);
        build(dcur + 1, m + 1, r);
        tr[cur].first = max(tr[dcur].first, tr[dcur + 1].first);
        tr[cur].second = min(tr[dcur].second, tr[dcur + 1].second);
    }
}
pll get(int cur, int l, int r, int x, int y) {
    if (x > r || y < l) {
        return {LLINF, 0};
    }
    if (x <= l && r <= y) {
    ...
    int m = (l + r) / 2;
    int dcur = cur + cur;
    pll a = get(dcur, l, m, x, y);
    pll b = get(dcur + 1, m + 1, r, x, y);
