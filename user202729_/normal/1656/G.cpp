// miraculously I got (virtual) rating +
// although obviously not enough for 3000

// no, I figured out the algorithm in contest, just don't have enough time to implement it
//  this algorithm...
//     did not prove carefully though. Might be wrong
...
void up(){
	...
	std::vector<char> countMod2(number);
	for(auto& it: a) {
		std::cin>>it;
		--it;
		countMod2[it]^=1;
	...
	if(std::accumulate(begin(countMod2), end(countMod2), int(0))>=2){
		std::cout<<"NO\n";
		return;
	}
	...
	for(int const index: range(0, number)){
		items[index]={.value=a[index], .originalIndex=index};
	}
	std::sort(begin(items), end(items));

	Item centerItem; // only used if number%2!=0
	if(number%2!=0){
		auto const centerValue=int(std::find(begin(countMod2), end(countMod2), 1)-countMod2.begin());
		auto iter=std::find_if(begin(items), end(items), [&](auto const& it){
			return it.value==centerValue;
		});
		centerItem=*iter; items.erase(iter);
	}

	std::vector<Item> anyOrder; anyOrder.reserve(number);
	for(int index=0; index<int(items.size()); index+=2){
		assert(items[index].value==items[index+1].value);
		anyOrder.push_back(items[index]);
	}
	if(number%2!=0) anyOrder.push_back(centerItem);
	for(int index=int(items.size())-1; index>=0; index-=2){
		anyOrder.push_back(items[index]);
	}
	items.clear();

	// now anyOrder is a palindrome... but it's not necessary cyclic...
	auto dsu=Dsu(); dsu.assign(number);
	// two items in the same cycle are joined in dsu...
	assert(int(anyOrder.size())==number);
	for(int const index: range(0, number)){
		dsu.join(anyOrder[index].originalIndex, index);
	}

	for(int const index: range(0, number>>1)){
		auto const j=number-1-index;
		assert(index<j);
		assert(anyOrder[index].value==anyOrder[j].value);
		if(dsu.root(index)!=dsu.root(j)){
			dsu.join(index, j);
			std::swap(anyOrder[index], anyOrder[j]);
		}
	}

	// ... then...
	if(number%2!=0 and dsu.root(number>>1)==(number>>1)
		and range(0, number).count([&](auto const& it){
			return dsu.root(it)==(number>>1);
		})==1){
		// that the odd center is in a single component now...
		if(not[&]{
			for(int const index: range(0, number>>1)){
				if(anyOrder[index].value==anyOrder[number>>1].value){
					dsu.join(index, number>>1);
					std::swap(anyOrder[index], anyOrder[number>>1]);
					return true;
				}
			}
			std::cout<<"NO\n"; return false;
	...
	// now just join all the pairs together
	for(int const index: range(1, number>>1)){
		if(dsu.root(0)!=dsu.root(index)){
			Item& a=anyOrder[0];
			Item& b=anyOrder[index];
			Item& c=anyOrder[number-1-index];
			Item& d=anyOrder.back();
			std::tie(a, b, c, d)=std::make_tuple(c, a, d, b); // hopefully... this is the part I didn't prove carefully
			dsu.join(0, index);
		}
	}
	...
	auto const tmp=[&]{
		...
		// first it's palindrome
		for(int const index: range(0, number)){
			assert(anyOrder[index].value==anyOrder.rbegin()[index].value);
		}
		...
		for(int const index: range(0, number)){
			assert(dsu.root(index)==dsu.root(0)); // inefficient but this is assert anyway
		}
	...
	std::cout<<"YES\n";
	for(auto [value, originalIndex]: anyOrder) std::cout<<originalIndex+1<<' ';
	std::cout<<'\n';
}

// time-limit: 1000
// problem-url: https://codeforces.com/contest/1656/problem/G