...
int is(int a, int b, int c, int d) {
	return d & max(a & b, a & c);
}
int should(int a, int b, int c, int d) {
	return max(a & b & d, a & c & d);
}
const int aim = 100001, vax = 300001;
bool seen[aim];
int main() {
	int k;
	scanf("%d", &k);
	int D = (1 << 17) - 1;
	int B = k, C = (1 << 17), A = B | C;
	debug(fun(is, A, B, C, D), fun(should, A, B, C, D));
	assert(should(A, B, C, D) - is(A, B, C, D) == k);
	printf("3 2\n%d %d\n%d %d\n%d %d\n", A, B, C, A, 0, D);
	return 0;
	srand(TIME);
	int missed = aim;
	while (missed) {
		int a = rand() % vax, b = rand() % vax, c = rand() % vax, d = rand() % vax;
		int v1 = is(a, b, c, d), v2 = should(a, b, c, d);
		assert(v2 >= v1);
		if (v2 - v1 < aim) {
			if (!seen[v2 - v1]) {
				seen[v2 - v1] = true;
				missed--;
				if (missed % 1000 == 0) debug(imie(v2), imie(v1), imie(missed));
			}
		}
	}
}