...
int globalK;
ll go(int curA, int k) {
    ll& mem = dp[curA][k];
    if (mem) return mem - 1;
    if (curA >= k) {
        mem = (globalK + curA - k + pa * revb) % mod;
    } else {
        mem = (pa * go(curA + 1, k) + pb * go(curA, k - curA)) % mod;
    }
    ++mem;
    return mem - 1;
}
int solve() {
    ...
    cin >> k >> pA >> pB;
    globalK = k;
    pa = pA * powmod(pA + pB, mod - 2, mod) % mod;
    pb = pB * powmod(pA + pB, mod - 2, mod) % mod;
    assert(pa + pb == mod + 1);
    reva = powmod(pa, mod - 2, mod);
    revb = powmod(pb, mod - 2, mod);
    fk = (k + pa * revb) % mod;
    cout << go(1, k) << endl;
    return 0;
    return 0;
