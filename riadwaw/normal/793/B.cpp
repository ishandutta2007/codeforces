...
public:
	void solve(std::istream& in, std::ostream& out) {
		int n, m;
		in >> n >> m;
		auto dp = make_vector<int>(n, n, 10);
	...
	bool solve(vector<string> s) {
		...
		//cerr << si << ' ' << ti << sf << ' ' << st << ' ' << tf << ' ' << tt << endl;
		for (int j : inclusiveRange(sf, st)) {
			if (j >= tf && j <= tt) {
				bool ok = true;
				for (int i: inclusiveRange(si, ti)) {
					if (s[i][j] == '*') {
						ok = false;
					}
				}
