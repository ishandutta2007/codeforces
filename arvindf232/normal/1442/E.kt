...
fun main(){
    solve.cases{
        ...
        val h0 = IntArray(n){nlarge} //head 0
        val h1 = IntArray(n){nlarge} // head 1
        var ret = 0
        G.leafFirst {v->
            var best0 = 0
            var best1 = 0
            ...
            G.subs(v){
                w->
                best0 = maxOf(best0,h0[w])
                best1 = maxOf(best1,h1[w])
                t0.add(h0[w])
                t1.add(h1[w])
            }
            val here = C[v]
            if(here == 1){
                best0 = maxOf(best0, best1 + 1)
                best1 = nlarge
            }else if(here == 2){
                best1 = maxOf(best1, best0+1)
                best0 = nlarge
            }
            h0[v] = best0
            h1[v] = best1
            ...
            val twowhite = t0.take(2).sum() + if(here == 2) 1 else -1
            val twoblack = t1.take(2).sum() + if(here == 1) 1 else -1
            val oneeach = t0.take(1).sum() + t1.take(1).sum()

            ret = maxOf(ret, twowhite, twoblack, oneeach)
        }
        put((ret+2)/2)
...
/*
observation: top down DP of some kind
