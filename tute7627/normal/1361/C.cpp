...
vector<edge<T>> euler_path(int s,Graph<T>&g, int edge_num){
  ...
  if(ret.size()==edge_num)return ret;
  else return vector<edge<T>>();
}

bool solve(ll d,vector<ll>a){
  ll n=a.size();
  Graph<int> g(1<<d);
  rep(i,0,n/2){
    ll u=a[2*i]%(1<<d),v=a[2*i+1]%(1<<d);
    g[u].EB(v,0,i);
    g[v].EB(u,0,i);
  }
  rep(i,0,g.size())if((ll)g[i].size()&1)return false;
  auto v=euler_path(a[0]%(1<<d),g,n/2);
  if(v.empty())return false;
  vector<ll>ret(n);
  rep(i,0,n/2){
    //cout<<v[i].id spa v[i].src spa v[i].to<<endl;;
    if(v[i].to==a[2*v[i].id]%(1<<d)){
      ret[2*i]=2*v[i].id+1;
      ret[2*i+1]=2*v[i].id;
    }
    else{
      ret[2*i+1]=2*v[i].id+1;
      ret[2*i]=2*v[i].id;
    }
  }
  rep(i,0,n)ret[i]++;
