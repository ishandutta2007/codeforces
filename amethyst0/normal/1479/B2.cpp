...
int main() {
	...
	for (int i = n - 1; i >= 0; i--) {
		...
		if (i < 2 || best.first.second != v[i - 2]) {
			cur = min(cur, best.first.first + 1);
		}
		else {
			cur = min(cur, best.first.first);
		}

		if (i < 2 || best.second.second != v[i - 2]) {
			cur = min(cur, best.second.first + 1);
		}
		else {
			cur = min(cur, best.second.first);
		}

		if (i >= 2) {
			cur = min(cur, res[v[i - 2]] + cnt);
		}
