...
void solve() {
  ...
  ll root = 0;
  FOR(i, N) if (deg[root] < deg[i]) root = i;

  EulerTour<Graph<int>> ET(G, root);
  //  1  bit
  FenwickTree<ll> bit(N + 10);
  ...
  FOR(_, Q) {
    ...
    if (t == 1 || t == 2) {
      ...
      if (lca == a) {
        dist = dep[b] - dep[a];
        white = bit.sum(ID[b] + 1) - bit.sum(ID[b] + 1 - dist);
      } else {
        assert(lca == root);
        white += bit.sum(ID[a] + 1) - bit.sum(ID[a] + 1 - dep[a]);
        white += bit.sum(ID[b] + 1) - bit.sum(ID[b] + 1 - dep[b]);
        dist = dep[a] + dep[b];
      }
      print(white == 0 ? dist : -1);
    }
  }
}
