    ...
    int[] naive(int n) {
        ...
        for (int i = 1; i < (1 << n); i++)
        {
            ...
            for (int j = 0; j < l.Count; j++)
            {
                ...
                {
                    max = Math.Max(max, (int)MyMath.gcd(l[j], l[k]));
                }
            }
            min[l.Count] = Math.Min(min[l.Count], max);
        }
        return min.Skip(2).ToArray();
    }
}
