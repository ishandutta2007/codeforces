...
void solve(){
    int n, m, l, r;
    cin >> n >> m >> l >> r;
    modint<MOD> even(r / 2 - (l-1) / 2), odd((r + 1) / 2 - (l) / 2);
    if ((n * m) % 2 == 1) {
        modint<MOD> ans = pows<modint<MOD>>(modint<MOD>(r - l + 1), n * m);
        cout << ans << endl;
        return;
    }
    vector<vector<modint<MOD>>> tmp;
    tmp.push_back(vector<modint<MOD>>{even, odd});
    tmp.push_back(vector<modint<MOD>>{odd, even});
    Matrix<modint<MOD>> ans(tmp);
    ans = pows<Matrix<modint<MOD>>>(ans, n * m);
