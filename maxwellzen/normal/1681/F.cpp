...
void dfs(int i, int parent) {
    ...
    ord.pb(i);
    sz[i] = 1;
    for (pi nb : adj[i]) if (nb.F!=parent) {
        val[nb.F] = nb.S;
        dfs(nb.F, i);
        sz[i] += sz[nb.F];
    }
...
void init(int root) {
    tim=0;
    st.rsz(n), ed.rsz(n), sz.rsz(n), val.rsz(n);
    dfs(root, root);
}
...
ll dfs2(int i, int c) {
    ...
    for (int nb : vadj[i]) res += (ll)dp[i] * dp[nb];
    return res;
}
...
void solve() {
    ...
    init(0);
    byval.rsz(n);
    for (int i : ord) if (i != 0) byval[val[i]].pb(i);

    ll ans = 0;
    vadj.rsz(n);
    dp.rsz(n);
    F0R(c, n) if (!byval[c].empty()) {
        vi stack; stack.pb(0);
        for (int i : byval[c]) {
            while (!stack.empty() && !anc(stack.back(), i)) stack.pop_back();
            vadj[stack.back()].pb(i);
            stack.pb(i);
        }
        ans += dfs2(0, c);
        vadj[0].clear(); for (int i : byval[c]) vadj[i].clear();
    }
    cout << ans << nl;
}
