...
fun main(){
	fun solve(arr:BooleanArray,k:Int,l:Int):Long {
		val n = arr.size
		if(k * l.toLong() >= n){
			return 0
		}
		val chill = IntArray(n){arr[it].chi}
		val Q = chill.torsq()
		val total = chill.sum().toLong()
		fun once(v:Int):Pair<Long, Int> {
			val DP = LongArray(n+1){phuge}
			val taken = IntArray(n+1)
			DP[0] = total
			for(i in 1..n){
				if(DP[i-1] < DP[i]){
					DP[i] = DP[i-1]
					...
					taken[i] = maxOf(taken[i], taken[i-1])
				}
				if(i >= l){
					val new = DP[i-l] + v - Q.sumQuery(i-l,i-1)
					if(new < DP[i]) {
						DP[i] = new
						taken[i] = taken[i-l] + 1
					}else if(new == DP[i]) {
						taken[i] = maxOf(taken[i], taken[i-l] + 1)
					}
				}
			}
			return Pair(DP.last(), taken.last())
		}
		val real = BinarySearchLastTrue(0,n){
			once(it).second >= k
		}!!
		val cost = once(real).first - real * k.toLong()
		return cost
	}
	solve.cases{
		val n = getint
		...
		val str = getstr
		val arr = BooleanArray(n){str[it].isUpperCase()}
		val neg = BooleanArray(n){!arr[it]}
		put(minOf(solve(arr,k,l), solve(neg,k,l)))
	}
	done()
}

/*
successive shortest path: method 1
sqrt : 2
