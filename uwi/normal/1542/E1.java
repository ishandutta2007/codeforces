...
//			tr(wpre);
			...
			long s = 0;
			for(int j = 0;j < pre.length;j++){
//				s0 += pre[j] * (cpre[pre.length-1-j-2] - (pre.length-1-(i-1)-1 >= 0 ? cpre[pre.length-1-(i-1)-1] : 0));
//				s0 %= mod;
				// pre[j] * (i * suf[j+1+1] + (i-1) * suf[j+2+1] + ...)
				// pre[j] * (i * pre[P-1-(j+1+1)] + (i-1) * pre[P-1-(j+2+1)] + ...)
				// pre[j] * (wpre[P-1-(j+1+1)+1] * (ind-1)
				{
...
//		int n = 5;
//		int[] f = new int[n*(n-1)/2+1];
//		for(int[] p : new IterablePermutation(n)){
//			int inv = 0;
//			for(int i = 0;i < n;i++){
//				for(int j = i+1;j < n;j++){
//					if(p[i] > p[j])inv++;
