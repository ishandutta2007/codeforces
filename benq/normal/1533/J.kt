...
//    println(ans)
    val n = io.int()
    val todo = Array<ArrayList<Int>>(500001){ArrayList<Int>()}
    for (i in 0 until n) {
        val (x,y) = io.ints(2)
        todo[y].add(x)
    }
    for (i in 1..500000) todo[i].sort()
    var ans = n
    fun go(_parity: Boolean) {
        var parity = _parity
        var lst = ArrayList<Int>()
        for (i in 1..500000) {
            var ind = 0
            val nex = ArrayList<Int>()
            if (parity) {
                todo[i].reverse()
                lst.reverse()
                var stk = 0
                for (x in todo[i]) { // decreasing order
                    while ((ind < lst.size) && (lst[ind] > x)) {
                        ++ind
                        ++stk
                    }
                    if (stk > 0) {
                ...
                nex.reverse()
                todo[i].reverse()
                // go through in decreasing order, match above
            } else {
                // go through in increasing order, match below
                var stk = 0
                for (x in todo[i]) {
                    while ((ind < lst.size) && (lst[ind] < x)) {
                        ++ind
