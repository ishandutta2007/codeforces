...
void solve(){
	int n; cin >> n;
	vi v(n);read(v);
	vector<vector<pair<int,int>>> dp(n, vector<pair<int,int>> (4,{10000000,0}));
	dp[0][0] = {v[0],1};
	for(int i = 1; i < n; ++i){
		dp[i][0].f = v[i]+min(dp[i-1][2].f,dp[i-1][3].f);
		dp[i][1].f = v[i]+dp[i-1][0].f;
		dp[i][2].f = min(dp[i-1][0].f,dp[i-1][1].f);
		dp[i][3].f = dp[i-1][2].f;

	}
	cout << min(min(dp[n-1][0].f,dp[n-1][1].f),min(dp[n-1][2].f, dp[n-1][3].f)) << "\n";			
}
