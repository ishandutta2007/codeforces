...
const int N = 6e3, M = 21e3, INF = 0x3f3f3f3f;
...
signed main() {
    ...
    fr(n), m = 4, s = 0, S = n * 2 + 1, t = S + 1, adde(s, S, m, 0);
    for (int i = 1; i <= n; i++) fr(a[i]);
    for (int i = 1; i <= n; i++) {
        adde(S, i, INF, 0), adde(i, i + n, 1, 1), adde(i, i + n, INF, 0), adde(i + n, t, INF, 0);
        for (int j = i + 1; j <= n; j++) if (a[i] - 1 == a[j]) { adde(i + n, j, INF, 0); break; }
        for (int j = i + 1; j <= n; j++) if (a[i] + 1 == a[j]) { adde(i + n, j, INF, 0); break; }
        for (int j = i + 1; j <= n; j++) if (a[i] % 7 == a[j] % 7) { adde(i + n, j, INF, 0); break; }
    }
