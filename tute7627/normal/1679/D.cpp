...
int main(){
  ...
  ll ok=max(a)+1,ng=min(a)-1;
  while(ok-ng>=2){
    ll mid=(ok+ng)/2;
    Graph<ll>gg(n);
    rep(i,0,m){
      if(a[u[i]]<=mid&&a[v[i]]<=mid){
        gg[u[i]].EB(v[i]);
      }
    }
    SCC scc(gg);
    ...
    rep(i,0,n){
      for(auto to:gg[i]){
        if(scc.index[i]==scc.index[to])mx=INF+5;
      }
    }
    scc.build_edges();
    vector<ll>dp(scc.scc_graph.size(),0);
    rep(i,0,dp.size()){
    ...
    chmax(mx,max(dp));
    //cout<<mid spa mx<<endl;
    //debug(gg);
    if(mx>=k-1)ok=mid;
    else ng=mid;
  }
  //cout<<ok<<endl;
  if(ok>max(a))cout<<-1<<endl;
  else cout<<ok<<endl;
  return 0;
}