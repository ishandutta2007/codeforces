...
struct Node {
	int sum = 0; // # of distinct integers
	int branch = 0; // whether exists branch at level i for 0 <= i <= 19
	int lazy_or = 0, lazy_xor = 0; // take or, then take xor
	int depth;
	AR<int,2> c{0,0};
};
...
void apply_xor(int ind, int val) { // OK
	if (ind == 0) return;
	Node& n = trie[ind];
	n.lazy_xor ^= val;
}
...
int merge(int x, int y, int lev = 20) {
	...
	AR<int,2> child;
	F0R(i,2) child[i] = merge(trie[x].c[i],trie[y].c[i],lev-1);
	free_up(x), free_up(y);
...
int make_or(int ind, int val, int lev = 20) {
	...
	AR<int,2> child;
	F0R(i,2) child[i] = make_or(n.c[i],val,lev-1);
	if (val&(1<<(lev-1))) {
