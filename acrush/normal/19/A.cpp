...
int main()
{
	...
	{
		scanf("%s",str);
		for (int k=0;str[k];k++) if (str[k]=='-') str[k]=' ';
	...
	for (int i=0;i<n;i++)
		Q[i]=MP(MP(-S[i],-(D[i]-F[i])),MP(-D[i],i));
	sort(Q,Q+n);
	set<string> MR;
	for (int i=0;i<n/2;i++) MR.insert(A[Q[i].second.second]);
	vector<string> R(MR.begin(),MR.end());
