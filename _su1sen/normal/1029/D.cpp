...
int main() {
    ...
    read(a);
    ll ans = 0;
    vec<ll> pow_mod(M + 1, 1);
    rep(i, M) {
        pow_mod[i + 1] = pow_mod[i] * 10 % k;
    }
    vec<map<ll, int>> cnts(M + 1);
    rep(i, M + 1) {
        for (ll v : a) ++cnts[i][v * pow_mod[i] % k];
    }
    for (ll v : a) {
        int digit = 0;
        for (ll x = v; x; x /= 10, ++digit);
        ll r = (k - 1) - (v - 1) % k;
        auto iter = cnts[digit].find(r);
        if (iter != cnts[digit].end()) {
            ans += iter->second;
        }
        ans -= v * (pow_mod[digit] + 1) % k == 0;
    }
    print(ans);
    return 0;
}