...
void BFS(){
    ...
    while(!Q.empty()){
        ...
        {
            int toh=u.h+dh[u.dir];
            int tow=u.w+dw[u.dir];
            
            if(toh>=0&&toh<H&&tow>=0&&tow<W&&S[toh][tow]!='*'){
                if(chmin(dis[toh][tow][u.dir],d)){
                    Q.push_front({toh,tow,u.dir});
                }
            }
        }
        
        for(int k=0;k<4;k++){
            if(chmin(dis[u.h][u.w][k],d+1)){
                Q.push_back({u.h,u.w,k});
            }
        }
    }
...
int main(){
    ...
    for(int k=0;k<4;k++) chmin(ans,dis[gh][gw][k]);
