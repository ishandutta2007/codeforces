...
int solve(int b, int n) {
    ...
    for (auto p : p) {
        p.f -= b;
        p.s -= n;
        if (p.f < 0) p.f = -p.f, p.s = -p.s;
        if (p.s < 0) chmax(mx, p.f - p.s);
        else chmax(mx, max(p.f, p.s));
    }
    return mx;
}
...
int best(int b) {
    ...
    while (r - l > 2) {
        ...
        if (solve(b, m1) > solve(b, m2)) l = m1;
        else r = m2;
    }
    int ans = l;
    for (int i = l + 1; i <= r; ++i)
        if (solve(b, ans) > solve(b, i))
            ans = i;
    return ans;
}

int solve(int b) {
    return solve(b, best(b));
}
