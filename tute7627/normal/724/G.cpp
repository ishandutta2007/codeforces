...
int main(){
  ...
  bool judge = true;
	ll n,m;cin>>n>>m;
	auto g=readGraph<ll>(n,m,1,false,true);
	vector<ll>h(n,-1);
	modint ret=0;
	rep(st,0,n){
		if(h[st]!=-1)continue;
		nazo_basis b;
		auto cnt1=vec(60,2,0LL);
		auto dfs=[&](auto &&f,ll k,ll x)->void{
			h[k]=x;
			rep(i,0,60)cnt1[i][x>>i&1]++;
			for(auto e:g[k]){
				if(h[e.to]==-1){
					f(f,e.to,x^e.cost);
				}
				else{
					b.insert(x^e.cost^h[e.to]);
				}
			}
		};
		dfs(dfs,st,0);
		vector<ll>cnt2(60);
		for(auto z:b.basis){
			rep(i,0,60)if(z>>i&1)cnt2[i]++;
		}
		modint cf=1;
		ll r=b.rank();
		rep(i,0,60){
			modint c0,c1;
			if(cnt2[i]==0)c0=1LL<<r,c1=0;
			else c0=c1=1LL<<r-1;
			ret+=cf*c0*cnt1[i][0]*cnt1[i][1];
			ret+=cf*c1*(cnt1[i][0]*(cnt1[i][0]-1)/2+cnt1[i][1]*(cnt1[i][1]-1)/2);
			cf*=2;
		}
	}
	cout<<ret<<endl;
