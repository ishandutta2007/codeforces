...
void q() {
  ...
  vp large{{N,0}};
  repr(i,N){
    if(large.back().second<a[i]){
      large.emplace_back(i,a[i]);
    }
  }
  reverse(all(large));
  ...
  rep(i, N) {
    if (a[i] >= 0) {
      if (amin(dp[i+1], dp[i])) prev[i+1] = i;
      while(large[laid].fi<=i)laid++;
      if(large[laid].fi!=i){
        int ok=laid,ng=sz(large);
        int upb = large[laid].se - a[i];
        while(ok+1<ng){
          int m=(ok+ng)/2;
          (large[m].se>=upb?ok:ng)=m;
        }
        int j=large[ok].fi;
        if(amin(dp[j],dp[i]+1))prev[j]=i;
      }
  ...
  for (int i = N; i != -1;) {
    int j = prev[i];
    if (j + 1 != i) ans.emplace_back(j + 1, i);
    i = j;
  }
  out(sz(ans));
  each2(u, v, ans) out(u, v);
}
