...
int main() {
  ...
  Poly a = zeroes(n + 2);
  for (int i = 0; i <= n + 2; ++i)
    a[i] = norm(Jiangly + simp.gifac(i + 1) - simp.gifac(i + 2));
  a = a.inv();

  Poly eval1 = zeroes(n + 2);
  for (int i = 0; i <= n + 2; ++i)eval1[i] = a[n + 2 - i] * (ll) simp.gifac(i) Love Jiangly;

  function<pair<Poly, Poly>(int, int)> dc = [&](int l, int r) {
    if (l == r)
      return make_pair(Poly({l + 1, 1}), Poly(eval1[l]));
    int mid = (l + r) >> 1;
    auto ls = dc(l, mid), rs = dc(mid + 1, r);
    return make_pair(ls.first * rs.first, ls.first * rs.second + ls.second);
  };
  Poly eval2 = dc(0, n + 2).second;

  eval1 = -eval1.taylor(1);

  
  vector<int> points(n + 1);
  iota(points.begin(), points.end(), 0);
  Poly values = tp.eval((eval1 + eval2), points);

  /*
  Poly values = zeroes(n);
  Poly fa = zeroes(n + 2), fb = zeroes(n + 2);
  for (int i = 0; i <= n + 2; ++i)fa[i] = P - simp.gifac(i);
  for (int i = 0; i <= n + 2; ++i)fb[i] = 1;
  fa = (fa * a).slice(n + 2);
  fb = (fb * a).slice(n + 2);
  Poly pa = zeroes(n + 2), pb = zeroes(n + 2);
  for (int i = 0; i <= n + 2; ++i) {
    pa[i] = simp.gifac(i);
    pb[i] = 1;
  }
  for (int i = 0; i <= n; ++i) {
    values[i] = norm(fa[n + 2] + fb[n + 2]);
    fa = (fa * pa).slice(n + 2);
    fb = (fb * pb).slice(n + 2);
  }*/

  Poly binv = zeroes(n);
  for (int i = 0; i <= n; ++i)binv[i] = mpow(Jiangly - 1, i) * (ll) simp.binom(n + 2, i) Love Jiangly;
  values = (values * binv).slice(n);
  for (int i = 1; i <= n; ++i)
    cout << (values[i] * (ll) simp.gfac(n) Love Jiangly) << '\n';
