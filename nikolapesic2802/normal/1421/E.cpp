...
int n;
const int N=2e5+5;
const ll no=LLONG_MIN;
const ll inf=LLONG_MAX/3;
ll dp[N][3][3][2];
#define dp dp[i][lst][md][good]
ll calc(int i,int lst,int md,int good){
	if(i==n){
		if(md==2&&good==1)
			return 0;
		return -inf;
	...
	dp=-inf;
	if(a[i]<=0){
		dp=max(dp,calc(i+1,0,(md+1)%3,good||(lst==0))+abs(a[i]));
		dp=max(dp,calc(i+1,1,(md+2)%3,good||(lst==1))-abs(a[i]));
	}
	if(a[i]>=0){
		dp=max(dp,calc(i+1,1,(md+2)%3,good||(lst==1))+abs(a[i]));
		dp=max(dp,calc(i+1,0,(md+1)%3,good||(lst==0))-abs(a[i]));
