...
void solve()
{
	...
	for (int i = 0; i < n; ++i) piece[i].insert(i);
	vector<vector<pair<int, int>>> mark(n);
	vector<int> res(q, -1);
	for (int i = 0; i < q; ++i)
	{
		int x, y;
		...
		{
			mark[x].push_back({ x, i });
			mark[y].push_back({ y, i });
		}
	}

	DisjointSet g(n), h(n);

	auto go = [&](int x, int y, int i)
	{
		x = h.Find(x);
		y = h.Find(y);
		if (x == y) return;
		if (mark[x].size() > mark[y].size()) swap(x, y);
		h.UnionOblivious(x, y);
		for (auto [z, id] : mark[x])
		{
			if (res[id] == -1 && h.Find(get<0>(queries[id])) == h.Find(get<1>(queries[id])))
			{
				res[id] = i + 1;
			}
			if (res[id] == -1)
			{
				mark[y].push_back({ z, id });
			}
		}
		mark[x].clear();
	};
	...
	for (int i = 0; i < m; ++i)
	{
		...
		{
			x = g.Find(x), y = g.Find(y);
			if (piece[x].size() > piece[y].size()) swap(x, y);
			g.UnionOblivious(x, y);
			for (auto z : piece[x])
			{
				auto it = piece[y].insert(z).first;
				if (it != piece[y].begin() && *prev(it) == z - 1)
				{
					go(z - 1, z, i);
				}
				if (next(it) != piece[y].end() && *next(it) == z + 1)
				{
					go(z, z + 1, i);
				}
			}
