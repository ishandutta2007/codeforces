...
ll dp1[500000];
ll dp2[500000];
vector<pair<int, long long>> graph[500000];
ll h[500000];
...
long long dfs(int v)
{
    ...
    for (int j = 0; j < graph[v].size(); j++)
    {
        ...
        if (h[u] > h[v])
        {
            results.push_back(dfs(u) - w);
        }
    }
    sort(all(results));
    reverce(all(results));
    if (results.size() > 1)
        ans = max(ans, results[0] + results[1] + a[v]);
    if (results.size() > 0)
        ans = max(ans, results[0] + a[v]);
    //results.push_back(a[v]);
    sort(all(results));
    reverce(all(results));
    ans = max(ans, a[v]);
    //cout << v << " " << results[0] << " " << (results.size() > 1 ? results[1] : -1) << endl;
    return (results.size() > 0) ? max(results[0] + a[v], a[v]) : a[v];
}
