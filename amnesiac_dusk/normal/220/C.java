...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
			posa[a[i]]=i;
		NavigableMap<Integer,Integer> hola=new TreeMap<Integer,Integer>();
		for(int i=0; i<n; i++)
			hola.put(i-posa[b[i]],hola.getOrDefault(i-posa[b[i]], 0)+1);
		for(int i=0; i<n; i++)
		{
			int ans=n;
			try {
				int tol=hola.ceilingKey(i);
				ans=min(ans,tol-i);
			} catch(Exception e) {}
			try {
				int lol=hola.floorKey(i);
				ans=min(ans,i-lol);
			} catch(Exception e) {}
			pr(ans);
			try {
			if(hola.get(i-posa[b[i]])!=1)
			hola.put(i-posa[b[i]],hola.get(i-posa[b[i]])-1);
			else
				hola.remove(i-posa[b[i]]);
			hola.put(i-posa[b[i]]+n,hola.getOrDefault(i-posa[b[i]]+n, 0)+1);
			}
			catch(Exception e)
			{
