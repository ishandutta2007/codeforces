...
istream& operator >> (istream& in, vi& r) {
    ...
    std::fread(s + first, 3 << 19, 1, stdin);
...
void fft0(cd* p, int n2) {
    int n = n2 / 2;
    if (n == 4) {
#define F(i, j, k) { cd x = p[i], y = p[j]; p[i] = (x + y); p[j] = (x - y) * conj(W[k]);}
        F(0, 4, 4);
        F(2, 6, 6);
        F(0, 2, 2);
...
void fft1(cd* p, int n2) {
    ...
    int n = n2 / 2;
    if (n == 4) {
#define F(i, j, k) { cd x = p[i], y = p[j] * W[k]; p[i] = x + y; p[j] = x - y;}
        F(0, 1, 1);
...
int main() {
    ...
    for (int i = 0; i <= 2; ++i) {
        ...
        for (int i = 21; i >= 0; --i) {
            ...
            switch (n) {
#define CASE(z) case (z): xs = Sqr<z>(x); break;
                    CASE(1 << 8);
                    CASE(1 << 9);
                    CASE(1 << 10);
                    CASE(1 << 11);
