...
void solve_case(int tc = 0) {
    ...
    f0r(i, n) f0r(j, n) ans[i][j] = INF;
    
    vl loc(n);
    
    f0r(st, n) { // starting vertex
        f0r(i, n) vis[i] = false;
        f0r(i, n) arrive[i].clear();
        ...
        auto process = [&](int u) {
            ...
            each(nxt, g[u]) {
                int v = nxt.f;
                int w = nxt.s;
                if (u != st) loc[(v + ti + mn) % n] = w;
                else loc[(v + ti) % n] = w;
            }
            f0r(i, 2 * n) { // go around twice
                ckmin(loc[go[i % n]], loc[i % n] + 1);
            }
            f0r(i, n) {
                if (vis[i]) continue; 
                if (ckmin(ans[st][i], ti + loc[i])) {
                    arrive[ans[st][i]].pb(i);
                }
            }
        };
...
/**
 * if I want to get from u to v
 * other cannons must be <= n - 1
 * sum of the other cannons you use <= n - 1
 * time you wait should be too long
 * each cannon can be used at most once
 * can i just do a dijkstra like thing
 * create an outer ring 
 * i always shoot towards the outer ring
 * distance 1 counter clockwise
 */

int main() {
    setIO("");
    int tt = 1;
    // re(tt); // uncomment if multitest
