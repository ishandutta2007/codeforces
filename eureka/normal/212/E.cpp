...
int dfs(int r, int f){
    int cnt = 0;
    son[r] = 1;
    for(int i=node[r].next; ~i; i=edge[i].next){
        if(edge[i].v == f) continue;
        cnt ++;
        son[r] += dfs(edge[i].v, r);
    }
    mem(dp, 0);
    if(cnt != 0){
        dp[0] = true, dp[v-son[r]] = true;
        for(int i=node[r].next; ~i; i=edge[i].next){
            if(edge[i].v == f) continue;
            int u = edge[i].v;
            for(int j=v; j>=son[u]; --j){
                dp[j] |= dp[j-son[u]];
            }
        }
        for(int i=1; i<v-1; ++i){
            mark[i] |= dp[i] | dp[v-1-i];
        }
    }
    return son[r];
}
int main(){
    while(cin >> v){
        ...
        dfs(1, 0);
        int cnt = 0;
        for(int i=1; i<v-1; ++i) if(mark[i]) cnt ++;
        printf("%d\n", cnt);
        for(int i=1; i<v-1; ++i){
