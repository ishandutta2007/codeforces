...
int main() {
    ...
    loop(t) {
        ...
        long long base = accumulate(all(a), 0LL) + accumulate(all(b), 0LL);
        ...
        if (p == 0 or q == 0) {
            ...
            loop(2) {
                int c0 = min(p - 1, q);
                int c1 = min(p, q);
                long long a0 = accumulate(a.begin(), a.begin() + c0, 0LL);
                long long a1 = accumulate(b.begin(), b.begin() + c1, 0LL);
                chmax(ans, base + a0 + a1);
                swap(a, b);
                swap(p, q);
            }
            print(ans);
