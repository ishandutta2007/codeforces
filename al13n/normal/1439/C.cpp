...
struct segtree{
  ...
  void init(int a,int b,int k,vector<LL>&A){
    if(b==a+1){
      T[k].a=T[k].s=T[k].m=A[a];
      return;
    }
    int c=(a+b)/2;
    init(a,c,k*2+1,A);
    init(c,b,k*2+2,A);
    T[k].s=T[k*2+1].s+T[k*2+2].s;
    T[k].m=T[k*2+2].m;
  }

  int fle(int l,LL v,int a,int b,int k){
    if(T[k].m>v||l>=b) return b;
    smax(l,a);
    if(T[k].a!=-1) return l;
    int c=(a+b)/2;
    int t=fle(l,v,a,c,k*2+1);
    if(t==c) return fle(l,v,c,b,k*2+2);
    return t;
  }

  void set(int l,int r,LL v,int a,int b,int k){
    if(l>=b||r<=a) return;
    if(l<=a&&r>=b){
      T[k].a=v;
      T[k].s=(b-a)*v;
    ...
    int c=(a+b)/2;
    if(T[k].a!=-1){
      T[k*2+1].a=T[k*2+2].a=T[k*2+1].m=T[k*2+2].m=T[k].a;
      T[k*2+1].s=T[k].a*(c-a);
      T[k*2+2].s=T[k].a*(b-c);
      T[k].a=-1;
    }
  ...
  pll find(int l,LL s,int a,int b,int k){
    if(l>=b) return mp(b,s);
    smax(l,a);
    if(T[k].a!=-1){
      LL q=min((LL)b-l,s/T[k].a);
      return mp(l+q,s-q*T[k].a);
    }
    if(l<=a&&T[k].s<=s) return mp(b,s-T[k].s);
    int c=(a+b)/2;
    pll t=find(l,s,a,c,k*2+1);
    if(t.X<c) return t;
    return find(l,t.Y,c,b,k*2+2);
  }
};
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    cin>>ty>>p>>v;
    if(ty==1){
      int a=T.fle(0,v,0,n,0);
      if(a<p) T.set(a,p,v,0,n,0);
    }else{
      --p;
      LL r=0;
      while(true){
