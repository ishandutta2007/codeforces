...
int main() {
    ...
    loop(t) {
        [] {
            ...
            int lh = 0;
            rep(i, 0, l) chmax(lh, h[i]);

            int rh = 0;
            rep(i, r, n + 1) chmax(rh, h[i]);
            
            vector<int> ls, rs;
            rep(i, 0, l) {
                if (lh == h[i]) ls.push_back(i);
            }
            rep(i, r, n + 1) {
                if (rh == h[i]) rs.push_back(i);
            }
            ...
            for (int i : { ls.front(), ls.back() }) {
                int h1 = h[i];
                rep(j, r + 1, n + 1) {
                    int h2 = h[j];
                    debug(i, j, seg.prod(i, j));
                    if (seg.prod(i, j) <= h1 + h2) {
                        print(1);
                        print(i + 1, j);
                        return;
                    }
            ...
            int k = max_element(all(h)) - h.begin();
            print(2);
            print(1, k);
            print(k + 1, n);
        }();
