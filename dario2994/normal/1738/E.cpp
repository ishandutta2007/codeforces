...
void solve() {
    auto f = [&](int x, int y) {
        ...
        if (x > y) swap(x, y);
        mint ris = 0;
        for (int k = 0; k <= x; k++) ris += binom(x, k) * binom(y, k);
        return ris;
    };
    ...
    suff[0] = a[n-1];
    for (int i = 1; i < n; i++) suff[i] = suff[i-1] + a[n-1-i];
    LL sum = pref[n-1];
    int center = 0;
    for (int i = 0; i < n-1; i++) center += 2*pref[i] == sum;
    LL up = (sum+1)/2; // we must stay strictly below this value.
    map<LL, pair<int,int>> cnt;
    for (int i = 0; i < n; i++) {
        if (pref[i] < up) cnt[pref[i]].first++;
        if (suff[i] < up) cnt[suff[i]].second++;
    }
    mint res = power(2, center);
    for (auto pp: cnt) {
        auto z = pp.second;
        if (z.first > 0 and z.second > 0) res *= f(z.first, z.second);
    }
    cout << res << "\n";
}
