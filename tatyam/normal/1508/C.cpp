...
int main(){
    ...
    vec(pll,add,0);
    if([&]{
        unordered_set s(initializer_list<pll>{},0,[](pll x){return x.first<<32^x.second;});
        each(a,b,c,edge)s.emplace(a,b);
        rep(n)rep(j,i+1,n)if(!s.count({i,j})){
        ...
        rep(add.size()){
            UnionFind uf(n);
            rep(j,add.size())if(i!=j)uf.unite(add[j].first,add[j].second);
            ll cnt=0;
            each(a,b,c,edge)if(uf.unite(a,b))cnt+=c;
            chmin(ans,cnt);
        }
        ll cnt=XOR;
        each(a,b,c,edge)if(uf.unite(a,b))cnt+=c;
    ...
    g.resize(n);
    each(a,b,c,edge)g.add_edge(a,b,c);
    queue<ll>q;
    unordered_set<ll>s;
    rep(n)s.insert(i);
    while(s.size()){
    ...
    ll ans=0;
    each(a,b,c,edge)if(uf.unite(a,b))ans+=c;
    out(ans);
}