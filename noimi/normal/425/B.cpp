...
int main() {
    ...
    if(n < m) {
        vv(int, A, m, n);
        rep(i, n) rep(j, m) A[m - 1 - j][i] = a[i][j];
        swap(A, a);
        swap(n, m);
    }
    ...
    if(n > k) {
        ...
        vv(int, dp, n, 1 << m, inf<int>);
        int x = 0;
        rep(j, m) if(a[0][j]) x += 1 << j;
        rep(j, 1 << m) dp[0][j] = popcount(x ^ j);
        rep2(i, 1, n - 1) {
            int x = 0;
            rep(j, m) if(a[i][j]) x += 1 << j;
            rep(j, 1 << m) { dp[i][j] = min(dp[i - 1][j], dp[i - 1][allbit(m) ^ j]) + popcount(x ^ j); }
        }
