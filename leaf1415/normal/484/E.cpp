...
ll n, Q;
ll h[100005];
ll l[100005], r[100005], w[100005];
vector<ll> comp;
ll lb[100005], ub[100005], mid[100005];

bool used[100005];
vector<ll> vec[100005], qvec[100005];
SegTree seg(17);
UnionFind uf(100005);
...
int main(void)
{
	...
	rep(i, 1, n) cin >> h[i];
	cin >> Q;
	rep(i, 1, Q) cin >> l[i] >> r[i] >> w[i];
	
	rep(i, 1, n) comp.pb(h[i]);
	uniq(comp);
	rep(i, 1, n) h[i] = lower_bound(all(comp), h[i]) - comp.begin();
	ll H = sz(comp);
	rep(i, 1, n) vec[h[i]].pb(i);
	
	rep(i, 1, Q) lb[i] = 0, ub[i] = H;
	rep(i, 1, 20){
		rep(j, 0, H-1) qvec[j].clear();
		rep(j, 1, Q){
		...
		per(j, H-1, 0){
			for(auto x : vec[j]){
				used[x] = true;
				if(used[x-1]) uf.unite(x, x-1);
				if(used[x+1]) uf.unite(x, x+1);
				seg.update(x, uf.v[uf.root(x)]);
			}
			for(auto q : qvec[j]){
				ll L = l[q], R = r[q], len = 0;
				if(used[l[q]]) L = uf.r[uf.root(l[q])], chmax(len, min(w[q], L-l[q]+1));
				if(used[r[q]]) R = uf.l[uf.root(r[q])], chmax(len, min(w[q], r[q]-R+1));
				chmax(len, seg.query(L+1, R-1));
				if(len >= w[q]) lb[q] = mid[q];
				else ub[q] = mid[q];
			}
		}
	}
	rep(i, 1, Q) cout << comp[lb[i]] << "\n";
