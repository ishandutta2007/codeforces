...
void build(int cur, int l, int r) {
    if (l == r) {
        ...
        build(dcur + 1, m + 1, r);
        tr[cur].resize(r - l + 1);
        merge(tr[dcur].begin(), tr[dcur].end(), tr[dcur + 1].begin(), tr[dcur + 1].end(), tr[cur].begin());
    }
}
int getCnt(int cur, int l, int r, int x, int y, int val) {
//    DBN(cur, l, r, x, y);
    if (l == x && r == y) {
        return int(upper_bound(tr[cur].begin(), tr[cur].end(), val) - tr[cur].begin());
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        if (y <= m) {
            return getCnt(dcur, l, m, x, y, val);
        } else if (x > m) {
            return getCnt(dcur + 1, m + 1, r, x, y, val);
        } else {
            return getCnt(dcur, l, m, x, m, val) + getCnt(dcur + 1, m + 1, r, m + 1, y, val);
        }
    }
}
int main() {
    ...
    while (q--) {
        ...
        prevAns = r - l + 1 - getCnt(1, 1, n, l, r, r);
        cout << prevAns << '\n';
    }
    return 0;
}