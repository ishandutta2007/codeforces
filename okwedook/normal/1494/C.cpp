...
int solve(vector<int> a, vector<int> b) {
    if (sz(b) == 0 || sz(a) == 0) return 0;
    sort(a);
    sort(b);
    int cntgood = 0;
    auto goodpt = [](const vector<int> &b, int x) {
        return binary_search(all(b), x);
    };
    for (auto i : a)
        cntgood += goodpt(b, i);
    int ans = cntgood;
    int ptrl = 0, ptrr = 0;
    auto getcnt = [](const vector<int> &b, int l, int r) {
    ...
    for (int i = 0; i < sz(a); ++i) {
        ...
        if (i + 1 < sz(a))
            next = a[i + 1];
        while (ptrl < sz(b) && b[ptrl] < a[i]) {
            ++ptrl;
        }
        while (ptrr < sz(b) && b[ptrr] < next) {
            ++ptrr;
        }
        cntgood -= goodpt(b, a[i]);
        for (int j = ptrl; j < ptrr; ++j) {
            chmax(ans, cntgood + getcnt(b, b[j] - i, b[j]));
        }
    }
    return ans;
}
...
void solve() {
    ...
    msg("ANSWER");
    println(solve(nega, negb) + solve(posa, posb));
}
