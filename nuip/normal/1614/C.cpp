...
int main(){
	...
	TT{
		cauto n=read();
		cauto m=read();
		cauto ints=readV(m,tuple(1,0,0));
		mint re=0;
		rrep(d,30){
			vector<int> zeros(n+1);
			for(cauto &[l,r,v]:ints)if(~v>>d&1) ++zeros[l], --zeros[r];
			rep(i,n) zeros[i+1]+=zeros[i];
			cauto c1=count(all(zeros),0)-1;
			(re+=re)+=(c1?modpow(2,n-1):0);
		}
		cout<<re NL;
	}
	return 0;
