...
int main() {
  ...
  for (int k = m - 1; k >= 0; --k) {
    for (int i = 0; i <= k; ++i) {
      int j = k - i;
      if (x + y + i - j > 0)
        f[i][j] = (f[i + 1][j] * (ll)(x + i) + f[i][j + 1] * (ll)(y - 1 - j)) % P * nt.inv(x + y + i - j) % P;
    }
  }
  int e1 = 0;
  for (int i = 0; i <= m; ++i)
    for (int j = 0; i + j <= m; ++j) {
      ...
      if (j > y)
        continue;
      if (i)
        g[i][j] = (g[i][j] + g[i - 1][j] * (ll) (x + i - 1) % P * nt.inv(x + i - 1 + y - j)) % P;
      if (j)
        g[i][j] = (g[i][j] + g[i][j - 1] * (ll) (y - (j - 1)) % P * nt.inv(x + i + y - (j - 1))) % P;
      if (i + j == m) {
        e1 = (e1 + g[i][j] * (ll) i) % P;
      }
    }
  ...
  for (int i = 1; i <= n; ++i) {
    if (a[i] == 0)
      printf("%lld ", w[i] * (ll)f[0][0] % P);
    else
      printf("%lld ", w[i] * (ll)e1 % P);
  }
