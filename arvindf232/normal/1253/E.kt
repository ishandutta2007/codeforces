...
fun main(){
    solve.cases{
        ...
        val firstX = X.minOrNull()!!
        for(i in 0 until m){
            val old = if(i > 0) DP[i-1] else 0
            for(k in 0 until n){
//                if(X[k] < i) continue
                val need = maxOf(0,X[k] - i)
                val cost = maxOf(0,need- S[k])
                val newcost = old + cost
                val newpos = minOf(maxOf(X[k] + need,X[k] +S[k]), m-1)
                DP[newpos] = minOf(DP[newpos],newcost)
            }
            if(i > firstX)
                DP[i] = minOf(old +1 , DP[i])
        }
        put(DP.last())
    }
    done()
...
1. exact range
2. before 0 , not a sound assumption

Codng;
    forgot to actually make the correction
 */