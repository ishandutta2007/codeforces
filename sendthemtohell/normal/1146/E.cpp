...
void set_t(int v, int l, int r, int V, int L, int R) {
    push(v, l, r);
    if (R <= l || r <= L) {
        return;
    }
    ...
    set_t(v << 1, l, (l + r) >> 1, V, L, R);
    set_t(v << 1 | 1, (l + r) >> 1, r, V, L, R);
}


void flip_t(int v, int l, int r, int L, int R) {
    push(v, l, r);
    if (R <= l || r <= L) {
        return;
    }
    ...
    flip_t(v << 1, l, (l + r) >> 1, L, R);
    flip_t(v << 1 | 1, (l + r) >> 1, r, L, R);
}

int get_t(int v, int l, int r, int id) {
    push(v, l, r);
    if (r <= id || l > id) {
        return 0;
    }
    ...
    return get_t(v << 1, l, (l + r) >> 1, id) + get_t(v << 1 | 1, (l + r) >> 1, r, id);
}
...
inline void solve() {
    ...
    for (int _ = 0; _ < q; ++_) {
        ...
        if (s == '>' && x >= 0) {
            ...
            set_t(1, 0, N, 3, abs(x), N);
            flip_t(1, 0, N, 0, abs(x));
        }
        else if (s == '<' && x >= 0){
            set_t(1, 0, N, 2, x, N);
            flip_t(1, 0, N, 0, x);
            ...
            set_t(1, 0, N, 2, abs(x) + 1, N);
        }
    }

    for (int i = 0; i < n; ++i) {
        int to_do = get_t(1, 0, N, abs(v[i]));
        if (to_do == 0) {
            cout << v[i] << " ";
        }
        else if (to_do == 1) {
