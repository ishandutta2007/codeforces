...
//    just dei pI
    solve.cases{
        ...
        val allret = Array(n){IntArray(n)}
        val dist = Array(n){G.BFS(it)}
        for(a in 0 until n){
            for(b in a until n){
                val d = dist[a][b]
                val combinedist = IntArray(n){dist[a][it] + dist[b][it]}
                val found = IntArray(n)
//                val putinQ = BooleanArray(n)
                found[a] = 1
                var baoling = false
                val absoluteDist = IntArray(n)
                for(i in 0 until n){
                    if(combinedist[i] == d){
                        absoluteDist[dist[a][i]] ++
                        if(absoluteDist[dist[a][i]] >= 2) {
                            baoling = true
                        }
                    }
                }
...
//                putinQ[a] = true
                if(!baoling){
                    G.everyEdge { v, w ->
                        if(dist[a][w] <= dist[a][v]) return@everyEdge
                        val ok1 = combinedist[v] == combinedist[w] && combinedist[v] == d
                        val ok2 = dist[a][w] - dist[a][v] == 1 && dist[b][w] - dist[b][v] == 1
                        if(ok1 || ok2) {
                            found[w] ++
                        }
                    }
