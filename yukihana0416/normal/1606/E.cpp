...
void work()
{
    ...
    for(int i = 1; i <= x; i++)
        for(int j = 1; j <= n; j++)
        {
            if(i == 1) f[i][j][1] += ifac[j];
            if(i + 1 <= x) f[i + 1][j][0] += f[i][j][0] + f[i][j][1];
            for(int k = 1; k <= n - j; k++)
                f[i][j + k][1] += f[i][j][0] * ifac[k];
        }

    for(int i = 1; i <= x; i++)
        for(int j = 1; j <= n; j++)
        {
//            cout <<i <<" " <<j <<" " <<(f[i][j][0] + f[i][j][1]) * fac[j] <<endl;
            f[i][j][0] = (f[i][j][0] + f[i][j][1]) * fac[j] + f[i - 1][j][0];
        }
    ...
    for(int i = 1; i <= x; i++)
        for(int j = 1; j <= n; j++)
        {
            if(j - 1 >= i) g[i][j] += f[i][j][0] - f[i - 1][j][0];
            for(int l = 1; l <= n - j; l++)
            {
                int atk = l + j - 1;
                if(i + atk <= x)
                    g[i + atk][j + l] += g[i][j] * f[min(l + j - 1, n)][l][0] * C(j + l, j);
            }
            if(i + j - 1 <= x) g[i + j - 1][j] += g[i][j];
        }

    mint ans = 0;
    for(int i = 1; i <= x; i++) ans += g[i][n];
    cout <<ans <<endl;
}
