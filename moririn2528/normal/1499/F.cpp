...
namespace sol{
    ...
    vector<mint> dfs(int pos,int bef){
        vector<mint> vs={1},v2;
        int i,j,k;
        for(auto to:path[pos]){
            if(to==bef)continue;
            vector<mint> v1=dfs(to,pos);
            v2.assign(min(m+1,(int)max(v1.size()+1,vs.size())),0);
            mint a=0;
            for(auto num:v1)a+=num;
            for(i=0;i<vs.size();i++){
                for(j=0;j<v1.size() && i+j<m;j++){
                    v2[max(i,j+1)]+=vs[i]*v1[j];
                }
                v2[i]+=vs[i]*a;
            }
            swap(vs,v2);
    ...
    void solve(){
        ...
        vector<mint> vs=dfs(0,-1);
        mint s=0;
        for(auto num:vs)s+=num;
        cout<<s<<endl;
    }
}
