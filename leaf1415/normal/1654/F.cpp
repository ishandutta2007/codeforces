...
int main(void)
{
	...
	cin >> n >> s;
	N = 1<<n;
	rep(i, 0, N-1) vec.pb(s[i]-'a');
	
	rep(i, 0, n-1){
		ll l = 1<<i;
		vector<P> tmp, comp;
		for(int j = 0; j < N; j += 2*l){
			rep(k, 0, l-1) tmp.pb(P(vec[j+k], vec[j+l+k]));
			rep(k, 0, l-1) tmp.pb(P(vec[j+l+k], vec[j+k]));
		}
		comp = tmp;
		uniq(comp);
		rep(i, 0, N-1) vec[i] = lower_bound(all(comp), tmp[i]) - comp.begin();
	}
	
	ll min_val = inf, min_i;
	rep(i, 0, N-1) if(min_val > vec[i]){
		min_val = vec[i], min_i = i;
	}
	string ans;
	rep(i, 0, N-1) ans += s[i^min_i];
	outl(ans);
