...
void main_() {
	...
	sparse_table<S, op> sp(cum);
	int len = 0;
	int l = -1, r = -1;
	REP(i, n) {
		ll base = cum[i] - s;
		debug(base);
		if(cum[i + 1] < base) continue;
		if(sp.prod(i + 1, n + 1) >= base) {
			if(chmax(len, n - i)) {
				l = i + 1, r = n;
			}
		} else {
			auto f = [&](ll x) { return sp.prod(i + 1, x) >= base; };
			int rig = bin_search(i + 1, n + 1, f);
			if(chmax(len, rig - (i + 1))) {
				l = i + 1, r = rig - 1;
			}
		}
	}
