...
void solve() {
    ...
    auto solve_slow = [&]() {
        ...
        for (int m = 1; m < S; m++) {
            ...
            bool good = false;
            if (any_of(all(b), L1(u, u % 2 == 1)))
                good = true;
            else {
                ll sum = accumulate(all(b), 0LL) / 2;
                ll g = 0;
                for (auto x : b)
                    g = gcd(g, x);
                // pd(b, sum, g);
                if (sum % g == 0) {
                    pd(b, sum, g, __builtin_ctz(sum), __builtin_ctz(g));
                    good = true;
                }
            }
    ...
    mn count_bad = 0;
    a.erase(remove_if(all(a), L1(u, u % 2 == 1)), a.end());
    n = sz(a);
    ...
    int so_far = 0;
    for (auto [k, v] : reversed(powers)) {
        count_bad += mn(2).pow(v - 1) * mn(2).pow(so_far);
        so_far += v;
    }

    auto ans = total - count_bad;
    ps(ans);
}
