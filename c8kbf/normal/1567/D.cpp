...
inline void AC() {
    ...
    m = read();
    pw[0] = 1;
    for(_ i = 1; i <= 13; ++i) pw[i] = pw[i-1]*10;
    memset(a, 0, sizeof a);
    memset(b, 0, sizeof b);
    for(_ i = 1, j = n; j; ++i, j /= 10) b[i] = j%10;
    p = 1;
    for(_ i = 13; i >= 1; --i) if(b[i]) {
        if(p == m) {
            a[p] += pw[i-1]*b[i];
            continue;
        }
        for(_ j = b[i]; j >= 1; --j) if(n-pw[i-1]*j >= m-p+1-j) {
            for(_ k = j; k--; ) {
                a[p] += pw[i-1];
                if(p != m) ++p;
            }
