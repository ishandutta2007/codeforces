...
bool find_path(int src,int dest)
{
	if (src==dest) return true;
	visited[src]=true;
	for (int i=0;i<n;i++) if (g[src][i] && !visited[i] && find_path(i,dest)) return true;
	return false;
}
int get_next(int current,int opt)
{
	int result=-1;
	for (int k=0;k<n;k++)
	{
		int p=(opt)?(n-1-k):k;
		if (!g[current][p]) continue;
		g[current][p]=g[p][current]=false;
		result=p;
		memset(visited,false,sizeof(visited));
		bool is_bridge=!find_path(current,p);
		g[current][p]=g[p][current]=true;
		if (!is_bridge) return p;
	}
	return result;
}
vector<int> get_next_all(int current)
{
	int result=-1;
	vector<int> ret;
	for (int p=0;p<n;p++)
	{
		...
		bool is_bridge=!find_path(current,p);
		g[current][p]=g[p][current]=true;
		if (!is_bridge) ret.push_back(p);
	}
	if (SIZE(ret)==0 && result>=0) ret.push_back(result);
	return ret;
}
void get_opt(int *arr)
{
	int *a=arr;
	while (1)
	{
		int current=*a;
		int next=get_next(current,0);
		if (next<0) break;
		*(++a)=next;
		g[current][next]=g[next][current]=false;
	}
}
int main()
{
	...
	for (int i=0;i<m+1;i++) p[i]--;
	for (int i=0;i<m+1;i++) a[i]=p[i];
	bool is_good=false;
	for (int i=m-1;i>=0;i--)
	{
		g[a[i]][a[i+1]]=g[a[i+1]][a[i]]=true;
		int next=get_next(a[i],1);
		if (next!=a[i+1])
		{
			is_good=true;
			vector<int> h=get_next_all(a[i]);
			int next=-1;
			for (int j=0;j<SIZE(h);j++) if (h[j]>a[i+1]) { next=h[j]; break; }
			a[i+1]=next;
			g[a[i]][a[i+1]]=g[a[i+1]][a[i]]=false;
			get_opt(a+i+1);
			break;
		}
	}
