...
vector<string> solve(int k0){
  ...
  k-=v*v;
  if(k<=0){
    R.resize(v,string(v,'o'));
    if(k==-1) R[0].back()='.';
  }else if(k!=2*v-1){
    dbg("B");
    R.resize(v+1,string(v+1,'o'));
    for(int i=(k+1)/2;i<=v;++i) R[0][i]=R[sz(R)-1-i].back()='.';
    if(k%2) R[1][v-1]='.';
  }else{
    R.resize(v+2,string(v+2,'o'));
    for(int i=1;i<=v+1;++i) R[0][i]=R[sz(R)-1-i].back()='.';
    R[1][v]=R[1][v-1]=R[2][v]=R[2][v-1]='.';
  }
  ...
  forv(i,R){
    forv(j,R){
      assert(R[i][j]==R[sz(R)-1-j][sz(R)-1-i]);
      if(j<sz(R)-1 && R[i][j]=='.') assert(R[i][j+1]=='.');
      if(R[i][j]=='o')++c;
    }
  }
  assert(c==k0);//*/
...
int main(int argc, char **argv){
  ...
  //for(int k=1;k<=100;++k) solve(k);

  int k;
  cin>>k;
  auto r=solve(k);
  if(!sz(r)){cout<<-1;return 0;}
  cout<<sz(r);
  forv(i,r) cout<<'\n'<<r[i];
