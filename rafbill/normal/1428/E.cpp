...
int main(){
  ...
  vi R(n, 1);
  max_queue<tuple<li, int>> Q;
  for(li i = 0; i < (li)(n); ++i) if(R[i] < A[i]) {
    Q.push(make_tuple(cost(A[i], R[i]) - cost(A[i], R[i]+1), i));
  }

  for(li l = 0; l < (li)(k-n); ++l) {
    li v; int i; tie(v, i) = Q.top(); Q.pop();

    R[i] += 1;
    if(R[i] < A[i]) Q.push(make_tuple(cost(A[i], R[i]) - cost(A[i], R[i]+1), i));
  }

  li ans = 0;
  for(li i = 0; i < (li)(n); ++i) ans += cost(A[i], R[i]);
  cout << ans << endl;
