...
int main() {
    TEST {
        ...
        ll l = -1e7, r = 1e7;
        SORT(a);
        auto c = [&](int i, int j, ll x) { return a[i] * a[j] + x * (a[i] + a[j]); };
        auto f = [&](ll x) {
            int k = lb(a, -x);
            ll res = 0;
            if(k == 0) {
                rep(i, 1, n) res += c(0, i, x);
            } else if(k == n) {
                rep(i, n - 1) res += c(i, n - 1, x);
            } else {
                rep(i, k) res += c(i, n - 1, x);
                rep(i, k, n) res += c(0, i, x);
                res -= c(0, n - 1, x);
            }
            chmax(ma, res);
            // dump(x, res);
