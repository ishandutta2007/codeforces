...
int main(){
  ...
  cin>>H>>W;
  mat sum(H,vec(W,0));
  // vector<segtree<int>> v(H,segtree<int>(W,1,[](int a,int b){return a*b;}));
  rep(i,H){
    cin>>s;
    rep(j,W){
      if(s[j]=='*')sum[i][j]=1;
      // if(s[j]=='.')v[i].input(j,0);
    }
  }
  rep(i,H)rep(j,W-1)sum[i][j+1]+=sum[i][j];
  ans=0;
  rep(i,H)rep(j,W){
    rep(k,min({H-i,W-j,j+1})){
      int t=sum[i+k][j+k];
      if(j-k-1>=0)t-=sum[i+k][j-k-1];
      if(t==k*2+1)continue;
      break;
    }
    ans+=k;
