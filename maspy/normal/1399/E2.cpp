...
void solve() {
  ...
  FORIN(v, dfs.VR) {
    auto p = dfs.par[v];
    if (dfs.ch[v].size() == 0) dp[v] = 1;
    if (p >= 0) dp[p] += dp[v];
  }
  VEC<tuple<ll, ll, ll>> data;
  FORIN(e, G.edges) {
    ll c, d;
    ll v = (dfs.par[e.frm] == e.to ? e.frm : e.to);
    tie(c, d) = e.cost;
    data.eb(mt(dp[v], c, d));
  }

  auto value = [&](ll wt, ll mult) -> ll { return (wt - wt / 2) * mult; };

  // 
  using T = tuple<ll, ll, ll>;
  priority_queue<T> que_1;
  priority_queue<T> que_2;
  FORIN(t, data) {
    ll a, b, c;
    tie(a, b, c) = t;
    T x = mt(value(b, a), b, a);
    if (c == 1) que_1.push(x);
    if (c == 2) que_2.push(x);
  }

  auto calc = [&](priority_queue<T>& que) -> vi {
    //  -> 
    vi A;
    while (!que.empty()) {
      auto [upd, wt, mult] = que.top();
      que.pop();
      A.eb(upd);
      wt /= 2;
      if (wt == 0) continue;
      auto x = mt(value(wt, mult), wt, mult);
      que.push(x);
    }
    return cumsum(A);
  };
  ...
  ll now = A.back() + B.back();
  const ll INF = 1LL << 60;
  ll ANS = INF;
  FOR(i, A.size()) {
    if (A[i] + B.back() < now - S) continue;
    auto j = lower_bound(all(B), now - S - A[i]) - B.begin();
    chmin(ANS, i + j + j);
  }
  print(ANS);
}
