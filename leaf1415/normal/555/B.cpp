...
int main(void)
{
	...
	rep(i, 1, m) cin >> p[i].first, p[i].second = i;
	sort(p+1, p+m+1);
	
	rep(i, 1, n-1){
		ll lb = l[i+1]-r[i], ub = r[i+1]-l[i];
		ll lpos = lower_bound(p+1, p+m+1, P(lb, -inf)) - p;
		ll rpos = upper_bound(p+1, p+m+1, P(ub, inf)) - p - 1;
		if(lpos > rpos){
			outl("No");
			return 0;
		}
		vec[lpos].push_back(P(rpos, i));
	}
	
	set<P> S;
	rep(i, 1, m){
		for(auto p : vec[i]) S.insert(p);
		if(sz(S)){
			ans[S.begin()->second] = p[i].second;
			S.erase(S.begin());
		}
		if(sz(S) && S.begin()->first <= i){
			outl("No");
			return 0;
		}
	}
