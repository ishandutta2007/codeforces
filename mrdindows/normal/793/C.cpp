...
pii pos[101010];
pii v[101010];
vector<pair<double, double>> allt;
using pdd = pair<double, double>;
pdd calc(int x, int v, int from, int to) {
    if (v == 0) {
        if (x <= from || x >= to) {
            cout << -1 << endl;
            exit(0);
    ...
    if ((x >= to && v > 0) || (x <= from && v < 0)) {
        cout << -1 << endl;
        exit(0);
    }
    pdd res;
    res.first = (from - x) * 1.0 / v;
    res.second = (to - x) * 1.0 / v;
...
int main() {
    ...
    FI(n) {
        cin >> pos[i] >> v[i];
        allt.push_back(calc(pos[i].first, v[i].first, p1.first, p2.first));
        allt.push_back(calc(pos[i].second, v[i].second, p1.second, p2.second));
    }
    for (auto p : allt) {
        if (p.first > maxf) {
            maxf = p.first;
