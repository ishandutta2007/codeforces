// Glory Addicts (https://codeforces.com/contest/1738/problem/A)
...
int main() {
    ...
    while (t--) {
        int n; read(n);
        vector<int> a_(n), b_(n), a, b; read(a_, b_);
        rep(i, n)
            (a_[i] ? b : a).push_back(b_[i]);
        ...
        auto check = [&](auto x, auto y) {
            ...
            while (sz(x) && sz(y)) {
                z.emplace_back(x.back(), 1);
                z.emplace_back(y.back(), 0);
                x.pop_back();
                y.pop_back();
            }
            while (sz(x))
                z.emplace_back(x.back(), 1), x.pop_back();
            while (sz(y))
                z.emplace_back(y.back(), 0), y.pop_back();

            ll ans = 0;
            rep(i, sz(z))
                ans += z[i].first * (i + 1 < sz(z) && z[i].second != z[i + 1].second ? 2 : 1);

            return ans;
        };

        println(max(check(a, b), check(b, a)));
    }
}