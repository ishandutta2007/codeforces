...
signed main(){
    ...
    rep1(_,100){
        ...
        vi oc;
        oc.clear();
        rep(i,m) if (s[pos][i]=='1') oc.pb(i);
        vi cnt;
        cnt.clear();
        cnt.resize(1<<sz(oc),0);
        rep(i,n){
            int res=0,idx=0;
            for (auto c:oc){
                if (s[i][c]=='1') res+=1<<idx;
                idx++;
            }
            cnt[res]++;
        }
        FWT<int> fwt;
        auto res=fwt.AND(cnt);
        rep(c,sz(res)){
            if (res[c]<(n+1)/2) continue;
            if (__builtin_popcount(c)>ress){
                ress=__builtin_popcount(c);
                vi hii;
                hii.clear();
                rep(i,15){
                    if (c>>i&1)  hii.pb(oc[i]);
                }
                ans=hii;
            }
        }
