...
int main(){
  ...
  vector<vector<int>> check=dsu.groups();
  {
    vector<vector<int>> g;
    for(auto x:check)if(x.size()>1)g.emplace_back(x);
    swap(g,check);
  }
  if(check.empty()){
    print(0);
  ...
  while(check.size()>1){
    auto lis1=check.back();check.pop_back();
    auto lis2=check.back();check.pop_back();
    rep1(i,lis1.size())lis1[i]=v[lis1[i-1]];
    rep1(i,lis2.size())lis2[i]=v[lis2[i-1]];
    op(lis1[0],lis2[0]);
    rep1(i,lis1.size())op(lis1[i],lis2[0]);
    rep1(i,lis2.size())op(lis1[0],lis2[i]);
    op(lis1[0],lis2[0]);
  }
  if(check.size()){
    auto lis=check[0];
    ll K=lis.size();
    if(K==2){
      rep1(i,N)if(i!=lis[0]&&i!=lis[1])break;
      op(i,lis[0]);
      op(i,lis[1]);
      op(i,lis[0]);
    }else{
      rep1(i,K)lis[i]=v[lis[i-1]];
      op(lis[0],lis[K-1]);
      op(lis[0],lis[K-2]);
      rep1(i,K-1)op(lis[i],lis[K-1]);
      op(lis[0],lis[K-1]);
    }
  }
  assert(is_sorted(all(v))&&*min_element(all(v))==0);
  print(ans.size());
  matoutput(ans);
// }
}