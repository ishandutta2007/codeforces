...
int main() {
	...
	if (pn == 1) {
		int idx = lower_bound(all(star), pac[1]) - star.begin();
		int L = idx - 1;
		int R = star.size() - idx;

		int ans = min((L == max(L, R) ? abs(star[1] - pac[1]) : INT_MAX), (R == max(L, R) ? abs(star.back() - pac[1]) : INT_MAX));
		W(max(L, R), ans);
		return 0;
	}
	...
	auto gnxt = [&](int pos) {
		int ret = (pos >= n ? INF : nxt[pos]);
		return ret;
	};
	
	auto go = [&](int k) {
		vector<int> L(pn + 1), R(pn + 1);
		for (int i = 1; i <= pn; i++) {
			L[i] = pac[i] - k;
			R[i] = pac[i] + k;
		}
		...
		for (int i = 1; i <= pn; i++) {
			chkmax(dp[i], dp[i - 1] >= L[i] ? gnxt(pac[i] + 1) : dp[i - 1]);
			chkmax(dp[i], dp[i - 1] >= pac[i] ? gnxt(R[i] + 1) : dp[i - 1]);
			if (i - 2 >= 0)
				chkmax(dp[i], dp[i - 2] >= L[i] ? gnxt(R[i - 1] + 1) : dp[i - 2]);
		}
