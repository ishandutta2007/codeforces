// Integers Shop (https://codeforces.com/contest/1621/problem/B)
...
int main() {
    ...
    while (t--) {
        int n; read(n);
        vector seg(n, array{0, 0, 0});
        read(seg);

        println(seg[0][2]);
        auto left = array{seg[0][0], seg[0][2]};
        auto right = array{seg[0][1], -seg[0][2]};
        auto left_right = array{seg[0][0], -seg[0][1], seg[0][2]};
        rep(i, 1, n) {
            auto [l, r, c] = seg[i];
            left = min(left, array{l, c});
            right = max(right, array{r, -c});
            left_right = min(left_right, array{l, -r, c});

            int cost = left[1] - right[1];
            if (left[0] == left_right[0] && right[0] == -left_right[1])
                cost = min(cost, left_right[2]);
            println(cost);
        }
    }
}