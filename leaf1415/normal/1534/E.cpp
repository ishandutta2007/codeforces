...
int main(void)
{
	...
	for(auto x : vec){
		...
		qvec.clear();
		rep(i, 1, n){
			if(state[i] && a) a--, qvec.push_back(i), state[i] = 1-state[i];
			else if(!state[i] && b) b--, qvec.push_back(i), state[i] = 1-state[i];
		}
		ans ^= query();
	}
	outl("!", ans);
