...
int main(){
  ...
  string s; cin >> s;
  string t;
  for(char c : s) if(t.empty() || c!=t.back()) t.pb(c);
  cnt[0]=1;
  for(char c : t) {
    int ci=c-'a';
    FORD(i,5000,0) {
      lli dt = cnt[i]-cnt0[ci][i];
      dt%=MOD; if(dt<0) dt+=MOD;
      (cnt[i+1]+=dt)%=MOD;
      (cnt0[ci][i]+=dt)%=MOD;
      (cnt0[ci][i+1]+=dt)%=MOD;
    }
  ...
  lli ans=0;
  FOR(i,t.size()+1) {
    (ans += (cnt[i]*part[i][n]))%=MOD;
    //cout << i << " " << cnt[i] << " " << part[i][n] << " " << cnt[i]*part[i][n] << endl;;
  }
  cout << ans << endl;
  return 0;
}