...
void main_() {
	...
	REP(i, m) {
		int v = a[i];
		if(last_idx[v] == -1) {
			chmax(Mi[v], v + BT.sum(v + 1, n));
		} else {
			querys.pb(T(last_idx[v] + 1, i, v));
		}
		if(BT.sum(v, v + 1) == 0) BT.add(v, 1);
		last_idx[v] = i;
	}

	REP(i, n) {
		if(used[i]) {
			querys.pb(T(last_idx[i] + 1, m, i));
		}
	}

	{
		int now = -1;
		V<> last(n, -1);
		sort(all(querys), [](T a, T b) {
			return get<1>(a) < get<1>(b);
		});
		BIT<int> B(m);
		for(auto [l, r, v] : querys) {
			for(int i = now + 1; i < r; i++) {
				if(last[a[i]] != -1) B.add(last[a[i]], -1);
				last[a[i]] = i;
				B.add(last[a[i]], 1);
				now = r - 1;
			}
			chmax(Mi[v], B.sum(l, r));
		}
	}
