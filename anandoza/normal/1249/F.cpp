...
int main() {
    ...
    vvi dp(n, vi(n + 1)); // dp[i][j] is for the subtree at i using nodes at depth >= j
    for (int root : reversed(tr.preorder)) {
        auto &cur = dp[root];

        vi children;
        tr.for_each_child(root, [&](auto e) { children.push_back(e.get_nbr(root)); });

        for (int d_small = 0; 1 + d_small < n; d_small++) {
            for (int d_large = d_small; d_large < n; d_large++) {
                int tot = 0;
                for (int c : children) {
                    tot += dp[c][d_large];
                }
                ...
                for (int c : children) {
                    int cand = dp[c][d_small];
                    if (1 + d_small + 1 + d_large > k && 1 + d_large + 1 + d_large > k) {
                        ckmax(cand, tot + dp[c][d_small] - dp[c][d_large]);
                    }
                    ckmax(cur[1 + d_small], cand);
                }
            }
        }

        for (int d = n - 1; d - 1 >= 0; d--)
            ckmax(cur[d - 1], cur[d]);
        ckmax(cur[0], a[root] + cur[k + 1]);
    }
