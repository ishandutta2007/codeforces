...
int main(void)
{
	...
	while(T--){
		cin >> h >> w;
		rep(y, 1, h) rep(x, 1, w) cin >> c[x][y], c[x][y] -= '0';
		rep(y, 1, h) rep(x, 1, w) sum[x][y] = sum[x-1][y] + sum[x][y-1] - sum[x-1][y-1] + c[x][y];

		//rep(x, 1, w) outa(sum[x], 1, h);
		...
		rep(i, 1, h){
			rep(j, 1, h){
				...
				m[0] = inf;
				rep(k, 1, w){
					a[k] = (j-i-1) - get(k, i+1, k, j-1);
					s[k] = get(1, i+1, k, j-1) + 2*k - get(1, i, k, i) - get(1, j, k, j);
					m[k] = min(m[k-1], a[k]-s[k]);
				}
				/*outa(a, 1, w);
				outa(s, 1, w);
				outa(m, 1, w);*/
				rep(k, 4, w) chmin(ans, a[k]+s[k-1]+m[k-3]);
			}
		}
		outl(ans);
	}
