...
int main(){
	...
	TT{
		...
		rep(i,n){
			++re;
			if(i+1<n) ++re;
			if(i+2<n && !(vs[i]<=vs[i+1] && vs[i+1]<=vs[i+2])
				 && !(vs[i]>=vs[i+1] && vs[i+1]>=vs[i+2])) ++re;
			if(i+3<n &&
				 (vs[i+1]>max({vs[i],vs[i+2],vs[i+3]}) || vs[i+1]<min({vs[i],vs[i+2],vs[i+3]})) &&
				 (vs[i+2]>max({vs[i],vs[i+1],vs[i+3]}) || vs[i+2]<min({vs[i],vs[i+1],vs[i+3]}))) ++re;
		}
		cout<<re NL;
	}
	return 0;
