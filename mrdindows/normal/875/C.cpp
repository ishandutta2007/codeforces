...
//        v[i].push_back(0);
    ...
    //        if (v[i] > v[i + 1]) {
            int pos = 0;
            while (pos < v[i].size() && pos < v[i + 1].size() && v[i][pos] == v[i + 1][pos]) {
                ++pos;
            }
            if (pos == v[i].size() && pos == v[i + 1].size()) {
                continue;
            }
            if (v[i].size() > pos && v[i + 1].size() == pos) {
                cout << "No" << endl;
                return 0;
            }
            if (v[i].size() == pos && v[i + 1].size() > pos) {
    ...
    //        int y = v[i + 1][pos];
            ...
            for (auto p : positions[x]) {
                *p -= m;
            }
            ans.push_back(x);
            if (v[i][pos] > v[i + 1][pos]) NO();
    //        }
        }
        for (int i = n - 2; i >= 0; --i) {
            //        if (v[i] > v[i + 1]) {
