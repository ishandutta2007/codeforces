...
const int LITTLE_BORDER = 200001; // TODO: TURN_BACK
struct factorizator
{
	int N;
	vi pr;
...
struct segment_tree {
	...
	bool b;
	stp l, r;
	segment_tree(ll a, bool b) : a(a), b(b), l(0), r(0) {
	}
	ll fe(int pos, pii p) {
		int m = (p.X + p.Y) / 2;
		if (pos < m) {
			if (l)
				return l->fe(pos, pii(p.X, m));
			else if (b)
				return a + pos;
			else
				return a;
	...
	void as(int L, int R, ll val, pii p) {
		if (R <= p.X || L >= p.Y)
			return;
		if (L <= p.X && R >= p.Y) {
			destr();
			...
			cre(p);
			int m = (p.X + p.Y) / 2;
			l->as(L, R, val, pii(p.X, m));
			r->as(L, R, val, pii(m, p.Y));
		}
	}
	void cre(pii p) {
		if (!l)
			l = new segment_tree(a, b);
		if (!r) {
			if (b) {
				int m = (p.X + p.Y) / 2;
				r = new segment_tree(a, b);
			} else
				r = new segment_tree(a, b);
		}
	}
	...
	ll mn(pii p) {
		if (l && r) {
			int m = (p.X + p.Y) / 2;
			return min(l->mn(pii(p.X, m)), r->mn(pii(m, p.Y)));
		} else if (b)
			return a;
		else
			return a - (p.Y - 1);
...
int another_another_main();
void solve_wrong(istream& cin = std::cin, ostream& cout = std::cout);
int another_main()
{
	solve_wrong();
...
void solve_wrong(istream& cin, ostream& cout) {
	int n;
	cin >> n >> segment_tree::t;
	vi g(n), c(n);
	for (int i = 0; i < n; ++i)
		cin >> g[i] >> c[i];
	int D = 0;
	...
	for (int i = 1; i < n; ++i) {
		...
		ans += d;
		d %= segment_tree::t;
		D = sum(d, D, segment_tree::t);
		c[i] = sum(c[i], D, segment_tree::t);
	}
	pii p(0, segment_tree::t);
	segment_tree st(0, true);

	/*for (ll r : st.full_arr(p))
		cout << r << ' ';
	cout << '\n';*/

	for (int i = n - 1; i >= 0; --i) {
		int L = raz(g[i], c[i], segment_tree::t);
		int R = raz(0, c[i], segment_tree::t);
		ll val = st.fe(R, p);
		if (L < R) {
			st.as(L, R, val, p);
		} else {
			st.as(0, R, val, p);
			st.as(L, segment_tree::t, val + segment_tree::t, p);
		}
		/*for (ll r : st.full_arr(p))
			cout << r << ' ';
		cout << '\n';*/
...
void solve_test(istream& cin = std::cin, ostream& cout = std::cout)
{
	...
	for (auto it = segs.begin(); it != segs.end(); ++it)
	{
		...
		auto st = nx == segs.end() ? t : nx->first - 1;
		auto q = it->second;
