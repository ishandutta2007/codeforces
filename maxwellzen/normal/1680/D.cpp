...
void solve() {
    ...
    ll upval, downval;
    F0R(i, m+1) {
        ll val = p[n] + i*k - (m-i)*k;
        if (abs(val) <= k) {
            upnum = i;
            downnum = m-i;
            upval = i*k - max(0ll, val);
            downval = (i-m)*k - min(0ll, val);
            break;
        }
    }
    ll ans = 0;
    FOR(j, 1, n) {
        F0R(i, j+1) {
            ll val = p[j+1] - p[i];
            int num = cnt[j+1] - cnt[i];
            if (num >= upnum) ckmax(ans, abs(val + upval + min(0ll, downval + (m-num)*k)));
            else ckmax(ans, abs(val + k*num));
            if (num >= downnum) ckmax(ans, abs(val + downval + max(0ll, upval - (m-num)*k)));
            else ckmax(ans, abs(val - k*num));
        }
    }
    cout << ans+1 << nl;
