...
class maintain(val n:Int){
    val bad = Array(2){IntArray(n)}
    var power0 = 0
    var power = n
    fun level(i:Int):Int {
        return (bad[0][i] > 0).chi + (bad[1][i] >0).chi
    }
    fun remove(i:Int){
        val level = level(i)
        power0 -= (level == 2).chi
    ...
    fun addbad(t:Int, i:Int){
        remove(i)
        bad[t][i] ++
        add(i)
    }
    fun removebad(t:Int, i:Int){
        remove(i)
        bad[t][i] --
        add(i)
    }
...
fun main(){
    solve.cases{
        ...
        var t = IntArray(2)
        val rowbad = maintain(n)
        val colbad = maintain(m)

        val previous = mutableMapOf<Long,Int>()

        fun deletefrom(i:Int, j:Int, v:Int){
            rowbad.removebad((j +v ) %2 , i)
            colbad.removebad((i+v) %2 , j)
            t[(i + j + v) % 2] --
        }
        fun addto(i:Int, j:Int,v:Int){
            rowbad.addbad((j +v ) %2 , i)
            colbad.addbad((i+v) %2 , j)
            t[(i + j + v) %2 ] ++
        }
        ...
        repeat(k){
            ...
            val last = previous[makepair(i,j)] ?: -1
            ...
            previous[makepair(i,j)] = new

            val o1 = rowbad.getcount()
            val o2 = colbad.getcount()
            var ret  =o1 modPlus o2
            if(t[0] == 0){
                ret = ret modMinus 1
            }
            if(t[1] == 0) {
