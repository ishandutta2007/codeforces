...
pair<pll,pll> dfs(ll n,ll a[])
{
    if(n==1)
    {
        if(a[0]==1&&a[1]==1)
        {
            return MP(MP(1,-(1<<30)),MP(-(1<<30),-(1<<30)));
        }
        else if(a[0]==1||a[1]==1)
        {
            return MP(MP(-(1<<30),1),MP(1,-(1<<30)));
        }
        else
        {
            return MP(MP(-(1<<30),-(1<<30)),MP(-(1<<30),0));
        }
    }
    pair<pll,pll> t1=dfs(n-1,a);
    pair<pll,pll> t0=dfs(n-1,a+(1<<(n-1)));
    pair<pll,pll> t=MP(MP(-(1<<30),-(1<<30)),MP(-(1<<30),-(1<<30)));
    REP(w1,0,2)
    {
        REP(w0,0,2)
        {
                ...
                REP(l0,0,2)
                {
                    REP(rw,0,2)
                    {
                        REP(rl,0,2)
                        {
                            ...
                            {
                                ll x=0,ww,wl,lw,wlw;
                                if((w1==1&&rw==1)||(w0==1&&rw==0)) ww=1;
                                else ww=0;
                                if((w1==1&&rw==0)||(w0==1&&rw==1)) wl=1;
                                else wl=0;
                                if((l1==1&&rl==1)||(l0==1&&rl==0)) lw=1;
                                else lw=0;
                                if((wl==1&&rwl==1)||(lw==1&&rwl==0)) wlw=1;
                                else wlw=0;
                                if(w1==1||w0==1) x++;
                                if(l1==1||l0==1) x++;
                                if(wl==1||lw==1) x++;
                                ...
                                if(ww==1&&wlw==1) t.F.F=max(t.F.F,x);
                                else if(ww==1&&wlw==0) t.F.S=max(t.F.S,x);
                                else if(ww==0&&wlw==1) t.S.F=max(t.S.F,x);
                                else t.S.S=max(t.S.S,x);
                            }
                        }
                    }
                }
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        pair<pll,pll> t=dfs(n,a);
        cout<<max(max(t.F.F+1,t.F.S+1),max(t.S.F+1,t.S.S));
