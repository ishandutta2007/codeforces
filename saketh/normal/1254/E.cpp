...
int main() {
    setIO();
    auto fail = []() { ps(0); exit(0); };

    int N; re(N); using E = edge; tree<E> tr(N); re(tr);

    vi pred(2 * (N - 1), -1), succ(2 * (N - 1), -1);

    auto put = [&](int& t, int v) {
        if (t != -1) fail();
        t = v;
    };
    auto set = [&](int f, int s) {
        if (f != INT_MIN) put(succ[f], s);
        if (s != INT_MAX) put(pred[s], f);
    };
    auto rev = [&](int e) {
        if (e < N - 1) return e + N - 1;
        return e - N + 1;
    };

    F0R (v, N) if (int u; re(u), u--) {
        if (u == v) fail();
        vi up, down; // u --> v
        for (int s = u, t = v; s != t; ) {
            if (tr.depth[s] >= tr.depth[t]) {
                E sp = tr.up_edge(s);
                up.pb(s < sp(s) ? sp.i : (sp.i + N - 1));
                s = sp(s);
            } else {
                E tp = tr.up_edge(t);
                down.pb(t < tp(t) ? (tp.i + N - 1) : tp.i);
                t = tp(t);
            }
        }
        reverse(all(down)), up.insert(up.end(), all(down)), down.clear();

        set(INT_MIN, up.front());
        F0R (i, sz(up) - 1) set(rev(up[i]), up[i+1]);
        set(rev(up.back()), INT_MAX);
    }

    mn ans = 1;

    F0R (u, N) {
        int adj = sz(tr.nbrs[u]);
        int fi = -1, vis = 0;

        trav (e, tr.nbrs[u]) {
            int ei = u < e(u) ? e.i : (e.i + N - 1);
            if (pred[ei] != -1) adj--;
            if (succ[ei] == INT_MAX) adj--;
            if (pred[ei] == INT_MIN) { if (fi != -1) fail(); fi = ei; }

            // make sure there are no cycles... means we can visit everything starting from either
            // -inf or from something without a predecessor
            if (pred[ei] == -1) {
                vis++;
                while (succ[ei] != -1 && succ[ei] != INT_MAX) ei = succ[ei], vis++;
            }
        }

        if (fi != -1) { // visit starting from -inf
            int clen = 1;
            while (succ[fi] != -1 && succ[fi] != INT_MAX) fi = succ[fi], clen++;

            // if there is a chain from -inf to +inf it has to contain everything
            if (succ[fi] == INT_MAX && clen != sz(tr.nbrs[u])) fail();

            vis += clen;
        }

        if (vis != sz(tr.nbrs[u])) fail();
        ans *= mn::factorial(max(adj, 0));
    }
    ps(ans);
