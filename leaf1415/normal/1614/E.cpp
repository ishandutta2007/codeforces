...
struct DynamicLazySegTree{
	...
	struct SegNode{
		...
		SegNode(int p){
			left = right = -1;
			parent = p;
			val = Ident(), delay = NoDelay();
		}
	};
	
	static SEG Ident(){ //identity element
		return 0;
	}
	static SEG ope(SEG a, SEG b){ //operator
		return a+b;
	}
	static DELAY NoDelay(){ //
		return 0;
	}
	static DELAY merge(DELAY a, DELAY b){ //
		return a+b;
	}
	static SEG act(SEG a, DELAY b, ll l, ll r){ //
		return a + b * (r-l+1);
	}
	...
	void check(int l, int r, int p){
		...
		if(seg[p].right == -1){
			seg.push_back(SegNode(p));
			seg[p].right = sz(seg)-1;
		}
	}
	
	void eval(int l, int r, int p)
	{
		if(seg[p].delay != NoDelay()){
			seg[p].val = act(seg[p].val, seg[p].delay, l, r);
			if(l < r){
				check(l, r, p);
				seg[seg[p].left].delay = merge(seg[seg[p].left].delay, seg[p].delay);
				seg[seg[p].right].delay = merge(seg[seg[p].right].delay, seg[p].delay);
			}
			seg[p].delay = NoDelay();
		}
	}
	
	void add(int a, int b, int p, int l, int r, DELAY val)
	{
		eval(l, r, p);
		
		if(b < l || r < a) return;
		if(a <= l && r <= b){
			seg[p].delay = merge(seg[p].delay, val);
			eval(l, r, p);
			return;
		}
		...
		seg[p].val = ope(seg[seg[p].left].val, seg[seg[p].right].val);
	}
	...
	SEG query(int a, int b, int p, int l, int r)
	{
		eval(l, r, p);
		...
		check(l, r, p);
		SEG lval = query(a, b, seg[p].left, l, (l+r)/2);
		SEG rval = query(a, b, seg[p].right, (l+r)/2+1, r);
	...
	ll lower_bound(ll x){
		x++;
		ll p = 0, l = 0, r = (1<<size)-1;
		eval(l, r, p);
		
		while(l < r){
			ll m = (l+r)/2, L = seg[p].left, R = seg[p].right;
			eval(l, m, L), eval(m+1, r, R);
			if(seg[L].val >= x) r = m, p = L;
			else l = m+1, p = R, x -= seg[L].val;
		}
		return l;
	}
};
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> t >> k;
		ll l = seg.lower_bound(t), r = seg.lower_bound(t+1);
		seg.add(0, 0, 1), seg.add(l, l, -1), seg.add(r, r, -1);
		
		ll x;
		rep(j, 1, k){
			cin >> x;
			x = (x+last) % (L+1);
			last = seg.query(0, x) - 1;
			cout << last << "\n";
		}
	}
