...
int n,m,k,a[N],pr[M>>3],pcnt,f[M],g[M],mu[M],fv[M];
vector<int> res;
bool fl[M],vis[N];
int main(){
	scan(n,k);
	for(int i=1;i<=n;++i) scan(a[i]),f[a[i]]++,fv[a[i]]++,chkmax(m,a[i]);
	mu[1]=1;
	for(int i=2,j;i<=m;++i) if(!fl[i]) {
		for(pr[++pcnt]=i,mu[i]=-1,j=1;i*j<=m;j++) if(!fl[i*j]) fl[i*j]=true,mu[i*j]=-mu[j];
		for(j=i;i64(i)*j<=m;j+=i) mu[i*j]=0;
	}
	for(int i=1;i<=pcnt;++i) for(int p=pr[i],d=m/p;d>=1;--d) f[d]+=f[d*p];
	for(int i=2;i<=m;++i) if(f[i]>=k) {
		for(int j=1;j<=n;++j){
			if(a[j]%i==0) res.pb(j);
			if(res.size()==k) break;
		}
	...
	for(int i=1;i<=m;++i) g[i]=mu[i]*f[i];
	for(int i=1;i<=pcnt;++i) for(int p=pr[i],d=1;d*p<=m;++d) g[d*p]+=g[d];
	for(int i=1;i<=m;++i) if(g[i]+1>=k&&fv[i]>0) {
		for(int j=1;j<=n;++j) if(a[j]==i) {res.pb(j); break; }
		for(int j=1;j<=n;++j) {
			if(__gcd(a[j],i)==1) res.pb(j);
			if(res.size()==k) break;
		}
		for(int i=0;i<k;++i) cout<<res[i]<<" \n"[i+1==k];
		exit(0);
