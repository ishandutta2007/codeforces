...
void solve()
{
    ...
    while (S.size())
    {
        auto it = *S.begin();
        S.erase(S.begin());
        if (alive.find(it[1]) == alive.end()) continue;
        if (alive.find(it[2]) == alive.end()) continue;

        ans.push_back({it[1], it[2]});
        alive.erase(it[1]);
        alive.erase(it[2]);

        auto nxt = alive.upper_bound(it[2]);
        auto prv = nxt;
        if (prv == alive.begin()) continue;
        if (nxt == alive.end()) continue;
        prv--;
        if (s[*prv] == s[*nxt]) continue;
        S.insert({abs(a[*prv] - a[*nxt]), *prv, *nxt});
    }
