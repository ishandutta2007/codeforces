...
int n,X[24],Y[24],G[24][24];
int f[two(24)];

void construct(int set)
{
	if (set==0) return;
	int exp=f[set];
	for (int i=0;i<n;i++) if (contain(set,i))
	{
		...
		for (int j=i+1;j<n;j++)
			if (contain(set,j))
			{
				int t=f[left-two(j)]+G[i][j];
				if (t==exp)
				{
					printf(" %d %d 0",i+1,j+1);
					construct(left-two(j));
					return;
				}
			}
	}
...
int main()
{
	...
	for (int i=0;i<n;i++)
		for (int j=0;j<n;j++)
			G[i][j]=sqr(X[i])+sqr(Y[i])+sqr(X[j])+sqr(Y[j])+sqr(X[i]-X[j])+sqr(Y[i]-Y[j]);
	memset(f,255,sizeof(f));
	f[0]=0;
	for (int set=1;set<two(n);set++) f[set]=100000000;
	for (int set=0;set<two(n);set++) if (f[set]<100000000)
	{
		int cost=f[set];
		for (int i=0;i<n;i++) if (!contain(set,i))
		{
			int t=cost+G[i][i];
			if (t<f[set|two(i)]) f[set|two(i)]=t;
			for (int j=i+1;j<n;j++)
				if (!contain(set,j))
				{
					int t=cost+G[i][j];
					if (t<f[set|two(i)|two(j)]) f[set|two(i)|two(j)]=t;
				}
			break;
		}
	}
