...
bool _chk( int x1, int y1, int x2, int y2 ) {
    return (x1+x2<=h && max(y1,y2)<=w) || (x1+x2<=w && max(y1,y2)<=h);
}

bool chk( int x1, int y1, int x2, int y2 ) {
    return _chk(x1,y1,x2,y2) || _chk(y1,x1,x2,y2) || _chk(x1,y1,y2,x2) || _chk(y1,x1,y2,x2);
}
...
int main() {
    ...
    REP(i,n) R(a[i],b[i]);
    int ans=0;
    REP(i,n) REP(j,i) if ( chk(a[i],b[i],a[j],b[j]) ) chkmax(ans,a[i]*b[i]+a[j]*b[j]);
    W(ans);
    return 0;
}