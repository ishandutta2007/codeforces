...
public:
    void solve() {
        ...
        vi ans(2);
        ans[1] = 1;
        auto dsq = [&](int a, int b) -> ll {
            return ll(x[a] - x[b]) * (x[a] - x[b]) + ll(y[a] - y[b]) * (y[a] - y[b]);
        };
        auto acute = [&](int a, int b, int c) -> bool {
            return dsq(a, b) + dsq(b, c) > dsq(a, c);
        };
        for (int i : range(2, n)) {
            if (acute(i, ans[0], ans[1])) {
                ans.insert(ans.begin(), i);
                continue;
            }
            if (acute(ans[0], i, ans[1]) && (i == 2 || acute(i, ans[1], ans[2]))) {
                ans.insert(ans.begin() + 1, i);
                continue;
            }
            for (int j : range(2, i - 1)) {
                if (acute(ans[j - 2], ans[j - 1], i) && acute(ans[j - 1], i, ans[j]) && acute(i, ans[j], ans[j + 1])) {
                    ans.insert(ans.begin() + j, i);
                    break;
                }
            }
            if (ans.size() == i) {
                if (acute(ans[i - 2], ans[i - 1], i)) {
                    ans.push_back(i);
                    continue;
                }
                ans.insert(ans.end() - 1, i);
            }
        }
        for (int& i : ans) {
            i++;
