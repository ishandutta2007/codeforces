...
#line 1 "/home/maspy/compro/library/alg/group_affine.hpp"
template <typename K>
struct Group_Affine {
  using F = pair<K, K>;
  using value_type = F;
...
#line 3 "/home/maspy/compro/library/alg/lazy_cntsum_affine.hpp"
...
void solve() {
  ...
  VEC(ll, A, N);
  LazySegTree<Lazy_CntSum_Affine<Re>> seg(N);
  FOR(i, N) { seg.set(i, mp(1.0, A[i])); }
  ...
  FOR_(Q) {
    ...
    if (t == 1) {
      LL(l1, r1, l2, r2);
      --l1, --l2;
      auto [n1, sum1] = seg.prod(l1, r1);
      auto [n2, sum2] = seg.prod(l2, r2);
      seg.apply(l1, r1, mp((n1 - 1) / n1, sum2 / (n1 * n2)));
      seg.apply(l2, r2, mp((n2 - 1) / n2, sum1 / (n1 * n2)));
    } else {
      LL(l, r);
      --l;
      auto [cnt, sum] = seg.prod(l, r);
      print(sum);
    }
  }
}
