...
void bld(int l, int r, int o) {
    if(l == r) {
        cin >> p[l];
        mx[o] = l + p[l], mn[o] = l - p[l];
    } else bld(lch), bld(rch), pu(o);
}
void filtmx(int L, int R, int l, int r, int o) {
    if(mx[o] < R) return;
    if(l == r) return d[q[++::r] = l] = nw + 1, mx[o] = 0, mn[o] = 1e9, void();
    if(L <= mid) filtmx(L, R, lch);
    if(R > mid) filtmx(L, R, rch);
    pu(o);
}
void filtmn(int L, int R, int l, int r, int o) {
    if(mn[o] > L) return;
    if(l == r) return d[q[++::r] = l] = nw + 1, mx[o] = 0, mn[o] = 1e9, void();
    if(L <= mid) filtmn(L, R, lch);
    if(R > mid) filtmn(L, R, rch);
    pu(o);
}
...
void solve() {
    ...
    while(1) {
        int u = q[l++];
        nw = d[u];
        if(u == b) return cout << nw << '\n', void();
        filtmx(max(1, u - p[u]), u, 1, n, 1);
        filtmn(u, min(n, u + p[u]), 1, n, 1);
    }
}
int main() {
