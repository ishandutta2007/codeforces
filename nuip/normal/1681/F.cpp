...
template<class T> vv<pair<int,T>> readG(const int &n,const int &m){ vv<pair<int,T>> g(n); rep(_,m){ cauto a=read<int>(1),b=read<int>(1); cauto c=read<T>(); g[a].eb(b,c-1); g[b].eb(a,c-1);} return g;}
...
int main(){
	...
	vector<int> szs(n,1);
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p)->int{
				for(cauto &[u,ignore]:g[v])if(u!=p) szs[v]+=dfs(dfs,u,v);
				return szs[v];
			};
		dfs(dfs,0,-1);
	}
	...
	vector<pii> ps;
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p)->int{
				for(cauto &[u,c]:g[v])if(u!=p){
						cszs[cs[c].back()]-=szs[u];
						ps.eb(u,cs[c].back());
						cs[c].eb(u);
						dfs(dfs,u,v);
						cs[c].pop_back();
	...
	out(cszs,ps,1);
	ll re=0;
	for(cauto &[a,b]:ps) re+=1ll*cszs[a]*cszs[b];
	cout<<re NL;
	return 0;
}