...
int main() {
    ...
    vector<int> between(m+1);
    int left = n;
    for (auto it: a) left-=it;
    for (int i = 0; i<m+1; i++) between[i] = left/(m+1);
    for (int i = 0; i<left%(m+1); i++) between[i]++;
    for (int i = 0; i<m+1; i++) if (between[i]>=d) {cout<<"NO"; return 0;}
    cout<<"YES"<<endl;
    int cur = 0;
    for (int i = 0; i<between[0]; i++) cout<<0<<' ';
    cur = between[0];
    for (int i = 0; i<m; i++)
    {
        for (int j = 0; j<a[i]; j++) cout<<i+1<<' ';
        for (int j = 0; j<between[i+1]; j++) cout<<0<<' ';
    }
