...
void solve() {
  LL(N);
  VEC(ll, A, N + 1);
  if (A[0] == 0) return print(0);
  A.eb(0);
  vc<pi> XY;
  FOR(x, N + 1) {
    XY.eb(x, A[x]);
    FOR(i, A[x + 1], A[x]) { XY.eb(x + 1, i); }
  }
  ...
  auto isin = [&](ll x, ll y) -> bool { return A[x] > y; };

  for (auto&& [x, y]: XY) {
    if (0 < y && isin(x, y - 1)) { ANS += C<mint>(x + y - 1, x); }
    if (0 < x && isin(x - 1, y)) ANS += C<mint>(x + y - 1, y);
    // print(x, y, ANS);
  }
  ANS -= mint(1);
