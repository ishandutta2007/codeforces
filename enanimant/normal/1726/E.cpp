/*
 * author:  ADMathNoob
 * created: 09/06/22 10:35:15
 * problem: https://codeforces.com/contest/1726/problem/E
 */
...
int main() {
  ...
  while (tt--) {
    ...
    Mint ans = 0;
    for (int c4 = 0; 4 * c4 <= n; c4++) {
      // c4 cycles of length 4
      Mint res = 1;
      {
        // 2*c4 pairs of adjacent
        res *= C(n - 2 * c4, 2 * c4);
        // make c4 pairs from the groups
        res *= fact[2 * c4] / fact[c4] / pow2[c4];
        // each cycle of 4 can be arranged in 2 ways
        res *= pow2[c4];
      }
      int s = n - 4 * c4;
      // remaining s: groups of 1 or 2
      
      // Mint ways = 0;
      // for (int p = 0; 2 * p <= s; p++) {
      //   ways += C(s, 2 * p) * fact[2 * p] / fact[p] / pow2[p];
      // }
      // debug(c4, ways);
      // res *= ways;
      res *= g[s];
