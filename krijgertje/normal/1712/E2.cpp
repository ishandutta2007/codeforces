...
struct BIT {
	...
	ll get(int idx) { ++idx; ll ret = 0; while (idx >= 1) { ret += val[idx]; idx -= idx & -idx; } return ret; }
};
...
void solve() {
	...
	for (int v = MAXVAL; v >= 1; --v) {
		for (int w = v; w <= MAXVAL; w += v) {
			++dvcnt[w];
			if (dvcnt[w] >= 2) bit.mod(w, choose2(dvcnt[w] - 1) - choose2(dvcnt[w] - 2));
		}
		for (int qid : who[v]) {
