...
bool naive(const vector<ll> &vs, vector<pii> ps){
	if(count(all(vs),0)==vs.size()) return true;
	sort(all(ps));
	do{
		auto cur=vs;
		for(cauto &[l,r]:ps){
			if(cur[l]!=cur[r]) break;
			reps(i,l,r) cur[i]=cur[r];
			if(count(all(cur),0)==cur.size()) return true;
		}
	}while(next_permutation(all(ps)));
	return false;
}
bool naive0(const vector<ll> &vs, vector<pii> ps){
	if(count(all(vs),0)==vs.size()) return true;
	sort(all(ps));
	do{
		auto cur=vs;
		for(cauto &[l,r]:ps){
			if(cur[l] || cur[l]!=cur[r]) break;
			reps(i,l,r) cur[i]=cur[r];
			if(count(all(cur),0)==cur.size()) return true;
		}
	}while(next_permutation(all(ps)));
...
bool solve(const vector<ll> &vs, const vector<pii> &ps){
	if(vs.back()) return false;
	cauto n=(int)vs.size();
	cauto m=(int)ps.size();
	set<int> non0;
	rep(i,n)if(vs[i]) non0.emplace(i);
	non0.emplace(n);
	vector<int> cnts(m);
	vv<int> es(n);
	rep(i,m){
		if(non0.count(ps[i].X)) ++cnts[i], es[ps[i].X].eb(i);
		if(non0.count(ps[i].Y)) ++cnts[i], es[ps[i].Y].eb(i);
	}
	queue<int> que;
	rep(i,m)if(cnts[i]==0) que.emplace(i);
	// out(cnts,es,non0,1);
	while(que.size()){
		cauto i=que.front(); que.pop();
		// out(i,ps[i],1);
		cauto &[l,r]=ps[i];
		for(auto it=non0.lower_bound(l);*it<r;it=non0.erase(it)){
			for(cauto &j:es[*it])if(--cnts[j]==0) que.emplace(j);
		}
	}
	return n==*non0.begin();
}
...
int main(){
	...
	// out(solve({0,3,0},{{0,2}}),1); return 0;
	if(0)reps(n,2,7){
		vector<pii> ps0;
		rep(r,n)rep(l,r) ps0.eb(l,r);
		cauto N=ps0.size();
		int cnt=0;
		out(n,N,1);
		rep(st,1<<N){
			vector<pii> ps;
			rep(i,N)if(st>>i&1) ps.eb(ps0[i]);
			vector<ll> vs(n);
			rep(_,100){
				do{
					for(auto &v:vs) v=rand()%(n+n+1)-n;
				}while(vs.back());
				cauto exp=naive(vs,ps);
				cauto act=solve(vs,ps);
				if(exp) ++cnt;
				if(exp!=act){
					out(exp,act,vs,ps,1);
					return 0;
	...
	TT{
		...
		cauto as=readV(n);
		cauto bs=readV(n);
		cauto ps=readV(m,pii(1,0));
		vector<ll> vs(n+1);
		rep(i,n) vs[i+1]=vs[i]+as[i]-bs[i];
		cout<<(solve(vs,ps)?"YES":"NO") NL;
	}
	return 0;
}