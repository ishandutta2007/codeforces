...
void solve() {
	...
	sort(all(vx));
	vx.push_back(vx.back() + 1);
	vx.erase(unique(all(vx)), vx.end());
	auto f = [&](LP a, LP b) {
		return LP{ a.first + b.first,a.second + b.second };
	};
	auto g = [&](LP a, int b, int l, int r) {
		if (b < 0)return a;
		if (b == 0) {
			a.first = 0;
		}
	...
	vector<LP> ori(vx.size() - 1);
	rep(i, vx.size() - 1) {
		ori[i] = { 0,vx[i + 1] - vx[i] };
	}
	SegT<LP, int> st(ori, LP{ 0,0 }, -1, f, g, h);
	auto comp = [&](LP a, LP b) {
	...
	rep(i, q) {
		...
		int loc = st.searchloc(0, { 0,0 },comp);
		loc = min(loc, (int)vx.size() - 1);
		cout << vx[loc] << "\n";
	}
}
