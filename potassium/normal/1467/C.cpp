...
int main() {
	...
	for (int i = 0; i < 3; i++) {
		a[i].resize(n[i]); R(a[i]);
	}

	vector<ll> sum(3), minv(3);
	for (int i = 0; i < 3; i++) {
		sum[i] = accumulate(all(a[i]), 0LL);
		minv[i] = *min_element(all(a[i]));
	}

	ll totalSum = accumulate(all(sum), 0LL);
	ll totalMin = accumulate(all(minv), 0LL);
	ll ans = 0;
	for (int i = 0; i < 3; i++) {
		chkmax(ans, totalSum - sum[i] * 2);
		chkmax(ans, totalSum - totalMin * 2 + minv[i] * 2);
	}
