...
const int SIZE = 1<<20;
int r[3],c[3],n,x,y;
bool check(VPII p,VPII q){
    sort(ALL(p));
    if(p[0]==MP(1,2)&&p[1]==MP(2,1)&&q[0]==MP(1,1))return 1;
    if(p[0]==MP(1,n-1)&&p[1]==MP(2,n)&&q[0]==MP(1,n))return 1;
    if(p[0]==MP(n-1,1)&&p[1]==MP(n,2)&&q[0]==MP(n,1))return 1;
    if(p[0]==MP(n-1,n)&&p[1]==MP(n,n-1)&&q[0]==MP(n,n))return 1;
    return 0;
}
void solve() {
    int cnt[2]={};
    ...
    if(check(p[AA],p[AA^1])){
        W((x==p[AA^1][0].F||y==p[AA^1][0].S)?"YES":"NO");
        return;
    }
    if(cnt[(x+y)&1]==2){
        W("YES");
    ...
    if(abs(p[(x+y)&1][0].F-x)%2==0&&abs(p[(x+y)&1][0].S-y)%2==0){
        W("YES");
    }else{
        W("NO");
    }
