...
vi solve(ll n,ll m,vl A){
    vvl a(n);
    rep(i,0,m-1){
        bool f=false;
        for(auto x:A){
            if((x>>i)&1){f=true;break;}
        }
        if(f){
            rep(j,0,n-1){
                a[j].pb((A[j] >> i)&1);
    ...
    m=a[0].size();
    {
        ...
        rep(j,0,m-1){
            ...
            rep(i,0,n-1){
                if(i==nxi)continue;
                if(a[i][j])rep(pj,0,m-1)a[i][pj] ^= a[nxi][pj];
            }
            nxi++;
            if(nxi == n)break;
        }
