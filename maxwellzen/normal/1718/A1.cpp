...
void solve() {
    cin >> n;
    read(a, n);
    vi p(n); p[0] = a[0]; FOR(i,1,n) p[i] = p[i-1]^a[i];
    int dp[n][8192];
    int mn[n];
    F0R(i,n) F0R(j,8192) dp[i][j]=1e9;
    F0R(i, n) mn[i] = 1e9;
    F0R(j, 8192) dp[0][j] = 2;
    dp[0][p[0]]--;
    dp[0][0]--;
    mn[0] = min(dp[0][p[0]], dp[0][0]);
    FOR(i,1,n) F0R(j, 8192) {
        dp[i][j] = mn[i-1]+1+(p[i]!=j);
        ckmin(dp[i][j], dp[i-1][j]+(p[i]!=j));
        ckmin(mn[i], dp[i][j]);
    }
    // F0R(i, n) debug(i, dp[i][0], dp[i][5]);
    cout << dp[n-1][p[n-1]] << nl;
}
