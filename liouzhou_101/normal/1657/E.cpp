...
void solve()
{
	int n, k;
	read(n, k);
	vector<vector<modint>> dp(n * n + 1, vector<modint>(k + 1));
	vector<vector<modint>> pref(n * n + 1, vector<modint>(k + 1));
	dp[0][0] = 1;
	for (int i = 1; i < n * n; ++i)
	{
		modint pref = dp[i - 1][0];
		for (int j = 1; j <= k; ++j)
		{
			dp[i][j] = pref + dp[i - 1][j] * j;
			pref += dp[i - 1][j];
		}
	}
	for (int i = 0; i <= n * n; ++i)
	...
	vector<vector<modint>> f(n + 2, vector<modint>(k + 2));
	f[0][0] = 1;
	for (int j = 0; j < k; ++j)
		for (int i = 0; i <= n - 1; ++i)
			for (int c = 0; i + c < n; ++c)
			{
					...
					f[i + c][j + 1] += f[i][j] * modint::Binomial(n - 1 - i, c) * pref[c * (c - 1) / 2 + c * i][k - j];
			}
