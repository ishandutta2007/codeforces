...
fun LongArray.shiftRight(){
    for(i in this.lastIndex downTo 1){
        if(i != lastIndex){
            this[i] = this[i-1]
        }else{
            this[i] = maxOf(this[i],this[i-1])
        }
...
fun main(){
    fun conv(a:LongArray,b:LongArray, limit:Int):LongArray{
        val ret = LongArray(a.size)
        val b = b.copyOf()
        b.shiftRight()
        for(i in 0 until a.size){
            for(j in 0 until b.size){
                if(i + j <= limit) continue
                ret[minOf(i,j)] = maxOf(ret[minOf(i,j)],a[i] + b[j])
            }
        }
        return ret
    }
    ...
    solve.cases{
        ...
        val DP = MutableList(n){LongArray(k+2)}
        G.setup()
        G.DFS { v ->
            var start = DP[v]
            for(s in G.subs[v]){
                start = conv(start,DP[s],k)
            }
            start[0] = maxOf(start[0], L[v] + start[k+1])
//            for(i in start.indices) start[i] += L[v].toLong()
            DP[v] = start
        }
        put(DP[0].maxOrNull()!!)
    }
