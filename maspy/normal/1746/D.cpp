...
void solve() {
  ...
  auto dfs = [&](auto& dfs, int v, ll k) -> ll {
    if (k == 0) return 0;
    ll key = k * N + v;
    ...
    ll n2 = k - n * k1;
    ll n1 = n - n2;
    assert(n == n1 + n2 && k == k1 * n1 + k2 * n2);

    vi dp1, dp2;
    for (auto&& to: ch) {
      ll x1 = (n1 > 0 ? dfs(dfs, to, k1) : 0);
      ll x2 = (n2 > 0 ? dfs(dfs, to, k2) : 0);
      dp1.eb(x1);
      dp2.eb(x2);
    }
