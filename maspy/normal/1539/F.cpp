...
void solve() {
  ...
  LazySegTree<Lazy_MinMax_Add<ll, INF>> seg(N + 1);
  vc<pi> seg_raw(N + 1);
  FOR(i, N + 1) seg_raw[i] = {i, i};
  seg.reset(seg_raw);

  FOR(t, N) {
    // t  -1, t  +1 
    for (auto&& i: pos[t]) {
      auto [a, b] = seg.prod(0, i + 1);
      auto [c, d] = seg.prod(i + 1, N + 1);
      ll x = d - a;
      if (x >= 1) { chmax(ANS[i], x / 2); }
    }
    // t  -1 
    for (auto&& i: pos[t]) { seg.apply(i + 1, N + 1, -2); }
    // t  -1, t  +1 
    for (auto&& i: pos[t]) {
      auto [a, b] = seg.prod(0, i + 1);
      auto [c, d] = seg.prod(i + 1, N + 1);
      ll x = b - c;
      if (x >= 1) { chmax(ANS[i], (x - 1) - x / 2); }
    }
  }
  print(ANS);
}
