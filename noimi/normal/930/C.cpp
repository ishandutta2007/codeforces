...
template< typename T >
vec longest_increasing_subsequence(const vector< T > &a, bool strict) {
  vector< T > lis;
  vec res;
  for(auto &p : a) {
...
signed main(){
    ...
    rep(i,m)r[i+1]+=r[i];
    r.pop_back();
    vec pre = longest_increasing_subsequence(r,0);
    reverse(all(r));
    vec suf = longest_increasing_subsequence(r,0);
    reverse(all(suf));
    int ma=0;
    rep(i,m){
        chmax(ma,pre[i]+suf[i]-1);
    }
    cout<<ma<<endl;
}