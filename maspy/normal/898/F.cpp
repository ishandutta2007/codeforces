...
struct Mono {
  ...
  static constexpr X unit = mp(1, 0);
...
void solve() {
  ...
  using X1 = modint998;
  using X2 = modint107;
  using X3 = modint<1000000009>;
  vc<pair<X1, X1>> segraw1(N);
  vc<pair<X2, X2>> segraw2(N);
  vc<pair<X3, X3>> segraw3(N);
  FOR(i, N) segraw1[i] = mp(10, A[i]);
  FOR(i, N) segraw2[i] = mp(10, A[i]);
  FOR(i, N) segraw3[i] = mp(10, A[i]);
  SegTree<Mono<X1>> seg1(segraw1);
  SegTree<Mono<X2>> seg2(segraw2);
  SegTree<Mono<X3>> seg3(segraw3);

  auto check_leading = [&](ll L, ll R) -> bool {
  ...
  auto check = [&](ll L, ll R) -> bool {
    ...
    if (!check_leading(0, L)) return false;
    if (!check_leading(L, R)) return false;
    if (!check_leading(R, N)) return false;
    if (seg1.prod(0, L).se + seg1.prod(L, R).se != seg1.prod(R, N).se)
      return false;
    if (seg2.prod(0, L).se + seg2.prod(L, R).se != seg2.prod(R, N).se)
      return false;
    if (seg3.prod(0, L).se + seg3.prod(L, R).se != seg3.prod(R, N).se)
      return false;
    return true;
  };
  FOR(R, N) {
    ll n = N - R;
    vi cand = {n - 1, n, R - (n - 1), R - n};
    for (auto&& L: cand) {
      if (check(L, R)) {
