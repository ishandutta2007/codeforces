...
// int word[nax];
// bool bt(int c, int i, int l, int a, int b) {
	// if (i == c) {
		// int p[l + 1];
		// p[0] = p[1] = 0;
		// FOR(j, 2, l) {
			// int v = p[j - 1];
			// while (v && word[v] != word[j - 1]) v = p[v];
			// if (word[v] == word[j - 1]) v++;
			// p[j] = v;
		// }
		// return p[l] == l - c;
	// }
	// int t = (l + c - i - 1) / c;
	// if (t <= a) {
		// for (int j = i; j < l; j += c) word[j] = 0;
		// if (bt(c, i + 1, l, a - t, b)) return true;
	// }
	// if (t <= b) {
		// for (int j = i; j < l; j += c) word[j] = 1;
		// if (bt(c, i + 1, l, a, b - t)) return true;
	// }
	// return false;
// }
// bool can(int a, int b, int c) {
	// bool r = bt(c, 0, a + b, a, b);
	// return r;
// }
bool can(int a, int b, int c) {
	int small_s = (a + b) / c, big_s = (a + b) / c + 1;
	int big_cou = (a + b) % c, small_cou = c - big_cou;
	assert(small_s * small_cou + big_s * big_cou == a + b);
	bool ans = false;
	FOR(i, 0, small_cou)
		if ((a - i * small_s) % big_s == 0 && (a - i * small_s) / big_s <= big_cou && (a - i * small_s) / big_s >= 0)
			ans = true;
	// debug(imie(a), imie(b), imie(c), imie(small_s), imie(small_cou), imie(big_s), imie(big_cou), imie(ans));
	// if (small_s != (a + b) / (c + 1)) debug("change");
	rans;
}
bool can2(int a, int b, int c) {
	int small_s = (a + b) / c, big_s = (a + b) / c + 1;
	int big_cou = (a + b) % c, small_cou = c - big_cou;
	if (big_cou == 0) return a % small_s == 0;
	assert(small_s * small_cou + big_s * big_cou == a + b);
	int big_mul = a, small_mul = -a;
	//big_mul += x * (small_s); small_mul += x * (-big_s)
	assert(big_mul * 1ll * big_s + small_mul * 1ll * small_s == a);
	auto test_around = [&](int x){
		FOR(d, x - 1, x + 1) {
			int sm = small_mul - d * big_s;
			int bi = big_mul + d * small_s;
			if (sm >= 0 && bi >= 0 && sm <= small_cou && bi <= big_cou) return true;
		}
		return false;
	};
	return test_around(small_mul / big_s)
		|| test_around(-big_mul / small_s)
		|| test_around((small_mul - small_cou) / big_s)
		|| test_around((big_cou - big_mul) / small_s);
}
	...
	// PER(ALL(in)) {
		// FOR(c, 1, a + b) {
			...
			// FOR(x, c, a + b - 1)
				...
				// }
			// if (good) {
				// if (c == 1) debug(imie(in));
				// ans.insert(c);
				// break;
			// }
		// }
	// }
	// FOR(c, 1, a + b) assert(can(a, b, c) == ans.count(c));
	// debug(imie(ans));
// }
int solve(int a, int b) {
	...
	while (x <= a + b) {
		...
		assert((a + b) / (to + 1) != (a + b) / x);
		assert((a + b) / to == (a + b) / x);
		if (x == to)
			ans += can2(a, b, x);
		else {
			int be = can2(a, b, x), en = can2(a, b, to);
			if (be || en)
				ans += be + en + (to - x - 1);
			else if (to - x >= 2 && can2(a, b, x + 1))
				ans += to - x - 1;
		}
		debug(imie(v), imie(x), imie(to));
		x = to + 1;
	}
	loc({
		int brans = 0;
		FOR(c, 1, a + b) if (can2(a, b, c)) brans++;
		assert(ans == brans);
	})
	rans;
}
