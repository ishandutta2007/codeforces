...
int main(){
  ...
  auto cmp = [&](int mask1, int mask2, int k, int i, int j) {
    for(li x = 0; x < (li)(1<<k); ++x) {
      if(s[i+(x^mask1)] < s[j+(x^mask2)]) return -1;
      if(s[i+(x^mask1)] > s[j+(x^mask2)]) return 1;
    }
    return 0;
  };
  ...
  vector<vii> V(n+1);
  vector<vvi> W(n);
  for(li k = 0; k < (li)(n+1); ++k) {
    for(li i = 0; i < (li)(1<<(n-k)); ++i)
      for(li j = 0; j < (li)(1<<k); ++j) {
      V[k].emplace_back(i,j);
    }
  }
  sort(begin(V[0]), end(V[0]), [&](pii i, pii j){ return s[i%xx]<s[j%xx]; });
  auto dec = [&](int k, pii i) {
    int i1 = i%xx, i2 = i%yy;
    int ia,ib;

    ia = W[k][2*i1][i2&~(1<<k)];
    ib = W[k][2*i1+1][i2&~(1<<k)];
    if(i2&(1<<k)) swap(ia,ib);

    return make_tuple(ia,ib);
  };

  auto calcW = [&](int k){

    W[k].resize(1<<(n-k));
    for(li i = 0; i < (li)(1<<(n-k)); ++i) W[k][i].resize(1<<k);
    if(k == 0) {
      for(li i = 0; i < (li)(1<<n); ++i) W[0][i][0] = s[i];
      return;
    }

    int cnt = 0;
    pii last = make_tuple(-1,-1);
    for(auto i : V[k]) {
      if(dec(k-1,i) != last) {
        last = dec(k-1,i);
        cnt++;
  ...
  for(li k = 0; k < (li)(n); ++k) {
    calcW(k);
    sort(begin(V[k+1]), end(V[k+1]), [&](pii i, pii j){
      return dec(k,i) < dec(k,j);
    });
  }

  int j = V[n][0]%yy;

  for(li i = 0; i < (li)(1<<n); ++i) cout << s[i^j];
  cout << endl;
  return 0;
}