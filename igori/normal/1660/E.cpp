...
void solve()
{
    ...
    int ans = INF;
    int total = 0, k = 0;
    for (int i = 0; i < n; i++) forn(j, n) if (s[i][j] == '1') total++;
    vector<vector<int> > table(2 * n, vector<int>(2 * n));
    for (int i = 0; i < 2 * n; i++)
        for (int j = 0; j < 2 * n; j++)
            table[i][j] = s[i % n][j % n] - '0';
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            k = table[i + n][j + n] - table[i][j], ans = min(ans, n - k + (total - k));
