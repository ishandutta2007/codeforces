...
int pmd2[256];
void solve() {
    for (int i = 0; i < 256; i++) pmd2[i] = __builtin_popcount(i) % 2;
    cin >> n >> m >> k;
    start.push_back(0);
    start.push_back(1);
    start.push_back(m + 1);
    for (int i = 1; i <= n; i++) {
        cin >> s[i].l >> s[i].r;
        start.push_back(s[i].l);
        start.push_back(s[i].r + 1);
    }
    sort(start.begin(), start.end());
    start.erase(unique(start.begin(), start.end()), start.end());
    sort(s + 1, s + n + 1, [](const auto& A, const auto& B) { return A.l < B.l; });
    for (int i = 0; i < 256; i++) f[i] = -1e9;
    f[0] = 0;
    for (int i = 1, j = 1; i < start.size(); i++) {
        int kill_list = 255;
        while ((!q.empty()) && (q.top().first < start[i])) {
            assert(q.top().first + 1 == start[i]);
            current_spell[q.top().second] = 0;
            kill_list ^= (1 << q.top().second);
            q.pop();
        }
        // update dp from previous stage to this stage
        for (int mask = 0; mask < 256; mask++) {
            f[mask & kill_list] = max(f[mask & kill_list], f[mask] + pmd2[mask] * (start[i] - start[i - 1]));
            if ((mask & kill_list) < mask) f[mask] = -1e9;
        }

        // add new segment if possible
        int new_list = 0;
        while ((j <= n) && (s[j].l == start[i])) {
            int idx = 0;
            while (current_spell[idx]) idx++;
            current_spell[idx] = j;
            q.emplace(s[j].r, idx);
            new_list ^= (1 << idx);
            j++;
        ...
        for (int mask = 0; mask < 256; mask++) {
            int avail_update = (mask ^ 255) & new_list;
            while (avail_update > 0) {
                int change = 1 << __lg(avail_update);
                f[mask ^ change] = max(f[mask ^ change], f[mask]);
                avail_update ^= change;
            }
        }
    }
