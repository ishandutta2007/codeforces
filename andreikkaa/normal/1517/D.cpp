...
inline void _main_function() {
    ...
    for(int tt = 1; tt <= k / 2; ++tt) {
        for(int i = 0; i < n; ++i) {
            for(int j = 0; j < m; ++j) {
                if(i) {
                    dp[i][j][tt].maxeq(dp[i - 1][j][tt - 1].x + 2 * ver[i - 1][j]);
                }
                if(j) {
                    dp[i][j][tt].maxeq(dp[i][j - 1][tt - 1].x + 2 * hor[i][j - 1]);
                }
                if(i + 1 < n) {
                    dp[i][j][tt].maxeq(dp[i + 1][j][tt - 1].x + 2 * ver[i][j]);
                }
                if(j + 1 < m) {
                    dp[i][j][tt].maxeq(dp[i][j + 1][tt - 1].x + 2 * hor[i][j]);
                }
            }
        }
    }
