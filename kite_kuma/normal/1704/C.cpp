...
ll solve() {
	...
	vll intervals;
	rep(i, m) { intervals.push_back((infected[(i + 1) % m] - infected[i] - 1 + n + n) % n); }
	...
	for(ll day2 = 0; day2 < m; day2++) {
		ll nxtinterval = intervals[day2];
		if(nxtinterval <= 2 * day2 * 2) {
			ans += nxtinterval;
			continue;
		}
		nxtinterval -= 2 * day2 * 2;
		ans += day2 * 4;
		ans += min(nxtinterval - 1, 1LL);
		debug(ans);
	}
