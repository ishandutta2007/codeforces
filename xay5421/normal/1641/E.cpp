...
void solve(int l,int r){
	...
	Poly f1;
	f1.resize(mid-l+1);
	rep(i,l,mid)f1[i-l]=g[i];
	Poly f2;
	f2.resize(r-mid);
	rep(i,mid+1,r)f2[i-mid-1]=f[i];
	Poly f3(f1*f2);
	rep(i,0,r-l-1){
		h[i+l+mid+1]+=f3[i];
	}
...
int main(){
	...
	f.resize(n*3+1),g.resize(n*3+1);
	rep(i,0,n*3)f[i]=g[i]=1;
	rep(i,1,m){
		int x;
		rd(x);
		f[x+n]=(P+1)>>1;
	...
	rep(i,1,n*3)f[i]*=f[i-1],g[i]*=g[i-1];
	if(0){
		Z ans=0;
		rep(i,1,n){
			Z cur=0;
			...
			DD(cur);
			ans+=cur*a[i];
			ans-=Z(a[i])*min(i+n,n*3-(i+n)+1)*Z((P+1)>>1).pow(m);
		}
		ans=ans*Z(2).pow(m);
		ans/=(Z(2).pow(m)-1);
		pt(ans.val(),'\n');
