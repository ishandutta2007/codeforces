...
int main() {
  ...
  f0r(i, n) {
    ...
    ll val = dp.query(l, i - 1).val + cov;
    if (l == i) val = 1;
    dp.update(i, i, val - i - 1);
    // cout << i << " " << l << " " << tpos << " " << dp.query(i, i).val << endl;
  }

  f0r(i, n) vans[xx[i].s] = dp.query(i, i).val + i + 1;
  ao(vans, n);
}