...
void solve(){
  ...
  SegmentTree<int> L(f,LINF), R(f,LINF);

  vec b = a, c = a;
  REP(i,N+2) b[i] -= i, c[i] -= (N+1)-i;
  L.build(b); R.build(c);

  auto check = [&](int mid){
    FOR(i,1,N+1){
      int l = max(0LL,i-mid), r = min(N+1,i+mid);
      if(L.query(l,i+1)+i-mid <= 0) continue;
      if(R.query(i,r+1)+(N+1-i)-mid <= 0) continue;
      return false;
    }
    return true;
  };
