...
int main(){
	...
	REP(teas, 2) {
		int diff = ans[1].first - ans[0].first;
		int itring = -1;
		for (int q = 2; q < ans.size(); ++q) {
			if (ans[q].first - ans[q - 1].first != diff) {
				if (itring != -1) {
					itring = -2;
					break;
				}
				itring = q;
				if (q != ans.size() - 1) {
					if (ans[q + 1].first - ans[q - 1].first != diff) {
						itring = -2;
						break;
					}
				}
		...
		if (itring != -2) {
			if (itring == -1) itring = 0;
			cout << ans[itring].second << endl;
			return 0;
		}
