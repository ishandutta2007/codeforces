...
struct BIT {
	...
	BIT(int sz) : N(sz + 1) { a.resize(N + 1, 0); }

	void add(int idx, int val, bool rec = true) {
		if (rec) stk.emplace_back(make_pair(idx, val));
	...
	void reset() {
		while (!stk.empty()) {
			add(stk.back().fi, -stk.back().se, false);
			stk.pop_back();
		}
	}
};
...
int main() {
	...
	{
		int time = 0;
		function<void(int, int)> dfs_order = [&](int u, int p) {
			in[u] = ++time;
			for (auto& v : g[u])
				if (v != p)
					dfs_order(v, u);
	...
	for (auto& c : col) {
		...
		const int total = c.se.size();
		for (auto& u : c.se) {
			int sub = bit.qry(in[u], out[u]);
			if (sub < total) {
				mark[u]++;
			}
	...
	function<void(int, int)> dfs_push = [&](int u, int p) {
		if (p != -1)
			mark[u] += mark[p];
