...
vector<int> primes(int N){
    ...
    vector<int> ans;
    rep(N) if(isprime[i]) ans.push_back(i);
    return ans;
}
vector<int>p;
ll query(ll n){
...
void solve(){
    ...
    ll from=0,to=0,n=1,q=0;
    while(q<22){
        q++;
        n=1;
        while(1){
            ld n2=ld(n)*ld(p[to]);
            if(n2>1000000000000000000.5L)break;
            n=n2;
            to++;
        }
        ll ans=query(n);
        rep(i,from,to)if(ans%p[i]==0){
            factor[i]=1;
            if(q<22){
                q++;
                ll n=1;
                while(1){
                    ld n2=ld(n)*ld(p[i]);
                    if(n2>1e18+0.5)break;
                    n=n2;
                }
                ll ans=query(n);
                while((ans/=p[i])>1)factor[i]++;
            }
        }
        from=to;
    }
    ll ans=1;
    each(i,factor)ans*=i.second+1;
    if(ans<7)answer(ans+7);
    else answer(ans*2);
}
signed main(){
    p=primes(1000000);
