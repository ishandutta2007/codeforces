...
int main() {
	...
	FOR(v, 1, n)
	{
		LL cost = 0;
		VI left;
		int my_votes = voters[1].size();
		FOR(i, 2, m) {
			int take = max(0, (int)(voters[i].size() - v + 1));
			my_votes += take;
			REP(j, take)
				cost += voters[i][j];
			FOR(j, take, voters[i].size() - 1)
				left.PB(voters[i][j]);
		}
		int missing = v - my_votes;
		debug(imie(v), imie(missing), imie(my_votes), imie(left));
		if (missing > 0) {
			nth_element(left.begin(), left.begin() + missing, left.end());
		}
		REP(i, missing)
			cost += left[i];
		debug(imie(v), imie(cost));
		mini(ans, cost);
	}
	printf("%lld\n", ans);
}