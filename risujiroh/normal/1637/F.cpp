...
#define let auto const
  let n = input();
  let h = input<vla<int>>(n);
  let root = all(h).max_element();
  ...
  auto pv = vla<int>(n);
  fix([&](auto self, int v, int p) -> void {
    pv[v] = p;
    for (let u : all(g[v]).filter(_1 != p)) { self(u, v); }
  })(root, -1);
  auto ans = h[root] * 2ll;
  auto alive = vla(n, false);
  auto order = rep(n).filter(_1 != root).to();
  all(order).sort(LAMBDA(h[_2] < h[_1]));
  auto deg = vla(n, 0);
  auto enable = [&](int v) -> void {
    assert(!alive[v]);
    alive[v] = true;
    for (let u : all(g[v]).filter(LAMBDA(alive[_1]))) {
      ++deg[v];
      ++deg[u];
    }
  };
  enable(root);
  for (let v : all(order).filter(LAMBDA(!alive[_1]))) {
    auto x = v;
    while (true) {
      if (alive[x]) {
        if (2 < deg[x]) { ans += h[v]; }
