...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = 0; j < c; j++) cin >> p[i].s[j];
    sort(p + 1, p + n + 1, [](const auto& A, const auto& B) { return A.a > B.a; });

    const int mx = (1 << c);
    for (int i = 0; i < mx; i++) f[i] = -1e18;
    f[0] = 0;
    for (int i = 1; i <= n; i++) {
        for (int j = 0; j < mx; j++) g[j] = -1e18;
        for (int j = 0; j < mx; j++) {
            int done = i - 1 - __builtin_popcount(j);
            if (done < k) g[j] = max(g[j], f[j] + p[i].a);
            else g[j] = max(g[j], f[j]);
            for (int l = 0; l < c; l++)
                if (((j >> l) & 1) == 0) {
                    g[j ^ (1 << l)] = max(g[j ^ (1 << l)], f[j] + p[i].s[l]);
                }
        }
