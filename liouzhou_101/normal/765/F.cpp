...
void solve()
{
	...
	auto work = [&]()
	{
		...
		{
			return ValueType{ max(a.val, b.val) };
		};
		struct TagType
		{
		};
		...
		SegmentTree<ValueType, decltype(value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
			tree(len, value_init, value_merge, tag_init, tag_merge, tag_spread);

		ValueType res_value_init = { INF };
		auto res_value_merge = [](const ValueType& a, const ValueType& b) -> ValueType
		{
			return ValueType{ min(a.val, b.val) };
		};
		SegmentTree<ValueType, decltype(res_value_merge), TagType, decltype(tag_merge), decltype(tag_spread)>
			res_tree(n, res_value_init, res_value_merge, tag_init, tag_merge, tag_spread);

		sort(task.begin(), task.end(), [](const query& lhs, const query& rhs)
			{
				return lhs.y < rhs.y;
			});
		...
		{
			int j = tree.Query(a[i], len).val;
			while (j != -1)
			{
				int diff = p[a[j]] - p[a[i]];
				if (res_tree.Query(j).val > diff)
					res_tree.ModifyValue(j, { diff });
				int half = (p[a[i]] + p[a[j]] + 1) / 2;
				int rank = lower_bound(p.begin(), p.end(), half) - p.begin();
				j = tree.Query(a[i], rank).val;
			}
			tree.ModifyValue(a[i], { i });
			while (k < m && task[k].y == i)
			{
				freshmin(res[task[k].id], res_tree.Query(task[k].x, task[k].y + 1).val);
				k += 1;
			}
		}
	};
