...
int main() {
    ...
    rep(t, 20) {
        ll x = rnd() % N;
        vll keta;
        rep(i, M) { if ((a[x] >> i) & 1)keta.push_back(i); }
        ...
        rep(i, 1ull << P)rc[i] = count[i];

        rep(i, P)rep(j, 1ull << P) {
            if (!((j >> i) & 1)) { rc[j] += rc[j ^ (1ull << i)]; }
        }
