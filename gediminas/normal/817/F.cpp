...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/248912/problem/H */
/*input
3
1 3 4
3 1 6
...
// #define DEBUG
	...
	for (int i = 0; i < n; i++) {
		...
		if (t == 1) {
			int kur = s.order_of_key(x) - 1;
			unsigned lmao = s.order_of_key(y + 1);
			if (kur < 0)
				kur = 0;

			while (kur < (int)s.size() and s[kur] < x) {
				kur++;
			}

			while (kur < (int)s.size() and s[kur] <= y) {
				s.erase(s[kur]);
			}

			if (s.order_of_key(x + 1) % 2 == 0) {
				change(x);
			}

			if (s.order_of_key(y + 1) % 2 != lmao % 2) {
				change(y + 1);
			}
