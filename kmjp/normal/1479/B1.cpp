...
void solve() {
	...
	int ret=0;
	FOR(i,N) cin>>A[i];
	st.update(0,1,1+(A[0]!=A[1])-def);
	st.update(A[0],A[0]+1,2-def);
	for(i=2;i<N;i++) {
		// other
		int ma=max({st.getval(0,A[i])+1,st.getval(A[i],A[i]+1),st.getval(A[i]+1,N+1)+1});
		// same
		if(A[i]!=A[i-1]) {
			st.update(0,N+1,1);
		}
		y=st.getval(A[i-1],A[i-1]+1);
		if(ma>y) st.update(A[i-1],A[i-1]+1,ma-y);
	}
