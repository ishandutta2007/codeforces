...
ll getSumTwo(int from, int to) {
    return prefTwo[to + 2] - prefTwo[from];
}
void buildPrefs(int *a, int n) {
    FI(n) {
        pref[i + 1] = pref[i] + a[i];
    }
    FI(n) {
        prefTwo[i + 2] = prefTwo[i] + a[i];
    }
}
ll solve(int *a, int n, ll dif) {
    if (n < 2) return 0;
    buildPrefs(a, n);
    ll ans = 0;
    for (int x = n - 1; x >= 1; --x) {
        ll sumX = getSum(0, x - 1);
        int lz = 2 - (n - x) % 2;
        int rz = n - x + 2;
        if (rz < lz) continue;
        while (lz != rz) {
            ...
            int y = n - x - mz;
            if (y < 0 || y % 2) continue;
            ll res = getSum(n - mz, n - 1) - sumX;
            res += getSumTwo(x, x + y - 2) - getSumTwo(x + 1, x + y - 1);
            res += dif;
            if (res > 0) {
                rz = mz;
            } else {
...
int solve() {
    ...
    while (tc--) {
        ...
        ll ans = 0;
        ans += solve(a, n, 0);
        ans += solve(a + 1, n - 1, a[0]);
        ans += solve(a, n - 1, -a[n - 1]);
        ans += solve(a + 1, n - 2, a[0] - a[n - 1]);
        buildPrefs(a, n);
        FI(n) {
            if (getSum(0, i) > getSum(i + 1, n - 1)) {
                ++ans;
            }
        }
        writeInt(ans % fftmod, '\n');
