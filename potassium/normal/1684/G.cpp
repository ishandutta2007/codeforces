...
int main() {
	...
	const ll thres = (m + 2) / 3;
	vector<ll> l,r;
	for (int i = 0; i < n; i++)
		(a[i] >= thres ? l : r).emplace_back(a[i]);
	...
	for (int i = 0; i < N; i++) {
		...
		int rr = km.lm[i];
		if (l[i] % r[rr] != 0 || 2 * l[i] + r[rr] > m) {
			is = false;
			break;
		}

		assert(2 * l[i] + r[rr] <= m);
		ans.push_back({ 2 * l[i] + r[rr], l[i] + r[rr] });
	}	
