...
void solve() {
  ...
  FOR(k, K - 1) {
    auto& dp = DP[k];
    auto& newdp = DP[k + 1];
    FOR(v, N) newdp[v] = dp[dp[v]];
  }

  vi indeg(N);
  FOR(v, N) indeg[TO[v]]++;
  ll add = 0;
  FOR(v, N) add += indeg[v] == 0;
  ll step = (MAX(A) - (N - 1)) / add;

  // print("step", step);
  FOR(v, N) {
    ll p = v;
    FOR(k, K) if (step & 1LL << k) p = DP[k][p];
    TO[v] = p;
  }
  vc<vi> FRM(N);
  FOR(v, N) FRM[TO[v]].eb(v);
  // FOR(v, N) print(v, FRM[v]);
  vi ANS(N, -1);
  vi LO(N);
  FastSet ss(N);
  FOR(i, N) ss.insert(i);
  FOR(v, N) if (len(FRM[v])) {
    ll a = A[v];
    ll frm = FRM[v][0];
    ss.erase(a);
    ANS[frm] = a;
    for (auto&& i: FRM[v]) LO[i] = a;
  }
  // print("ANS", ANS);
  // print("LO", LO);
  FOR(i, N) if (ANS[i] == -1) {
