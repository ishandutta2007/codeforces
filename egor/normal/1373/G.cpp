...
public:
    void solve() {
        ...
        auto change = [&](int root, int val, int left, int right) {
            qty[root] += val;
            res[root] -= val;
            value[root] =
                    qty[root] != 0 ? res[root] : left + 1 == right ? 0 : min(value[2 * root + 1], value[2 * root + 2]);
        };
        RecursiveFunction update = [&](const auto& self, int root, int left, int right, int from, int to,
            ...
            if (left >= from && right <= to) {
                change(root, val, left, right);
                return;
            }
            int mid = (left + right) / 2;
            self(2 * root + 1, left, mid, from, to, val);
            self(2 * root + 2, mid, right, from, to, val);
            res[root] = min(res[2 * root + 1], res[2 * root + 2]) - qty[root];
            value[root] = qty[root] == 0 ? min(value[2 * root + 1], value[2 * root + 2]) : res[root];
        };
        set<pii> pawns;
        for (int z : range(m)) {
            int x = in.readInt();
