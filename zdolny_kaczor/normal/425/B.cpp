...
int main() {
	...
	if (n > m) {
		REP(i, nax) REP(j, i) swap(a[i][j], a[j][i]);
		swap(n, m);
	}
	int ans = 1e9;
	if (n > k) {
		...
		REP(tried, n) {
			...
			REP(i, n) if (i != tried) {
				int diff = 0;
				REP(j, m) if (a[i][j] != a[tried][j]) diff++;
				debug(imie(i), imie(diff));
				curr += min(diff, m - diff);
			}
			debug(imie(tried), imie(curr));
			mini(ans, curr);
		}
	}
	else {
		debug("check all masks");
		REP(mask, 1 << (n - 1)) {
			int curr = 0;
			REP(j, m) {
				int diff = 0;
