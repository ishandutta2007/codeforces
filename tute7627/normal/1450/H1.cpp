...
int main(){
  ...
  Comb comb(2*n+5);
  ll sz=n+3;
  auto p=vec(2,sz,modint(0));
  rep(i,0,2){
    modint div=modint(1)/pow(2LL,add[i]);
    //cout<<add[i] spa cnt[i]<<endl;
    rep(j,0,sz){
      ll d=j-cnt[i];
      if(d>=0&&d<=add[i]){
        p[i][j]=comb.C(add[i],d)*div;
      }
    }
  }
  auto q=vec(2,sz+2,modint(0));
  auto all=vec(2,2,modint(0));
  rep(i,0,2){
    rep(j,0,sz){
      q[i][j+2]=q[i][j]+p[i][j];
      all[i][j&1]+=p[i][j];
  ...
  //debug(p,p.size(),p[0].size());
  //debug(q,q.size(),q[0].size());
  //debug(all,2,2);
  modint ret=0;
  rep(i,0,2){
    rep(j,1,sz){
      /*if(p[i][j]!=0){
        cout<<i spa j spa q[i^1][j] spa all[i^1][j&1]<<endl;
      }*/
      if(i==0){
        ret+=j*p[i][j]*q[i^1][j];
        ret-=j*p[i][j]*(all[i^1][j&1]-q[i^1][j]);
      }
      else{
        ret+=j*p[i][j]*q[i^1][j+2];
        ret-=j*p[i][j]*(all[i^1][j&1]-q[i^1][j+2]);
      }
    }
  }
  cout<<ret<<endl;
