...
int Dp(){
	...
	Msn(dp[cur],-INF);
	Forx(i,0,m-k) dp[cur][i] = sum[1][i+k] - sum[1][i];

	Forx(layer,2,n){
		int last = cur; cur ^= 1;
		Msn(dp[cur],-INF);
		auto udp = dp[cur], bdp = dp[last];
		auto usum = sum[layer];
		...
		auto InRange = [=]( int pos )->bool{
			return pos >= 0 and pos <= m-k;
		};
		{
			// Part2
			...
			auto AddContri = [&]( int j ){
				int x = Calc(j);
				st.insert(x);
			};
			auto DelContri = [&]( int j ){
				int x = Calc(j);
				auto p = st.find(x);
				// assert( p != st.end() );
				st.erase(p);
			...
			Forx(i,0,m-k){
				if(InRange(i-k)) DelContri(i-k);
				AddContri(i);
				int mx = *st.rbegin();
				Mymax( udp[i] , mx+usum[i] );
			}
		}

		{
			...
			Forx(i,m-k+1,m) AddContri(i);
			fORx(i,0,m-k){
				if(InRange(i+k)) DelContri(i+k);
				AddContri(i);
				int mx = *st.rbegin();
				Mymax( udp[i] , mx-usum[i+k] );
			}
		}

		Forx(i,0,m-k)
			udp[i] += usum[i+k] - usum[i];
	}
