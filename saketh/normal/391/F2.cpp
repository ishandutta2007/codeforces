...
int main() {
    ...
    int first = 0;
    while (first + 1 < sz(p) && p[first] > p[first + 1])
        first++;
    ...
    auto merge_smallest = [&]() {
        while (legs[pq.top().s] != pq.top().f)
            pq.pop();

        int inx = pq.top().s;
        pq.pop();

        ll combined = legs[prev[inx]] - legs[inx] + legs[succ[inx]];
        profit -= legs[inx];
        ...
        int new_prev = prev[prev[inx]];
        int new_succ = succ[succ[inx]];
        ...
        pq.push({ combined, inx });
    };

    insert(LLONG_MAX / 2);
    F0R (i, sz(extrema) - 1) {
        if (i % 2 == 0) profit += extrema[i + 1] - extrema[i];
        insert(abs(extrema[i + 1] - extrema[i]));
    }
    insert(LLONG_MAX / 2);

    for (int trades = sz(extrema) / 2; trades > K; trades--)
        merge_smallest();
