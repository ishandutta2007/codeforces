...
int main() {
    TEST {
        ...
        vi ans(m, -1), vis(n), par(n), d(n);
        REC([&](auto &&f, int x) -> void {
            vis[x] = true;
            fore(e, g[x]) {
                if(!vis[e]) d[e] = d[x] + 1, f(e), ans[e.cost] = 0, par[e] = e.cost;
            }
        })
        (0);
        ...
        Edges<int> E;
        rep(i, n) fore(e, g[i]) {
            if(i < e and ans[e.cost] == -1) E.eb(i, e.to, e.cost);
        }
        ...
        if(si(E) < 3) {
            ...
            set<int> s;
            fore(e, E) s.emplace(e.from), s.emplace(e.to);
            dump(s);
            ...
            if(si(s) == 4) {
                ...
                int x, D = -1;
                fore(e, s) if(chmax(D, d[e])) x = e;

                ans[par[x]] = 1;
                bool flag = true;
                fore(e, E) {
                    if(flag and (e.from == x or e.to == x)) {
                        flag = false;
                        ans[e.cost] = 0;
                    } else
                        ans[e.cost] = 1;
