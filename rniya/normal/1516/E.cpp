...
int main() {
    ...
    vector<mint> comb(MAX_K, 1);
    for (int i = 1; i < MAX_K; i++) comb[i] = comb[i - 1] * (n + 1 - i) / i;

    vector<vector<mint>> dp(MAX_K, vector<mint>(MAX_K, 0));
    for (int i = 0; i < min(n + 1, MAX_K); i++) dp[i][0] = comb[i];
    for (int i = MAX_K - 1; i >= 0; i--) {
        for (int j = 0; j < MAX_K; j++) {
            for (int k = 2; i - k >= 0 && j + (k - 1) < MAX_K; k++) {
                dp[i - k][j + (k - 1)] += dp[i][j] * COM.C(i - 1, k - 1) * COM.fac(k - 1);
            }
        }
    }

    vector<mint> ans(k + 1, 0);
    for (int i = 0; i <= k; i++) ans[i] += dp[0][i];
    for (int i = 0; i + 1 < k; i++) ans[i + 2] += ans[i];
    for (int i = 1; i <= k; i++) cout << ans[i] << (i == k ? '\n' : ' ');
    return 0;
}