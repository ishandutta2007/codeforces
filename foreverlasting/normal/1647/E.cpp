...
namespace MAIN{
    ...
    int p[N];
    int a[N];
    int F[N][31],pos[N],ans[N],vis[N];
    set<int> S;
    inline void MAIN(const res &Case){
        n=read();
        for(res i=1;i<=n;i++)p[i]=F[i][0]=read();
        for(res i=1;i<=n;i++)a[i]=read(),pos[i]=i,S.insert(i);
        for(res i=1;i<=30;i++)for(res j=1;j<=n;j++)F[j][i]=F[F[j][i-1]][i-1];
        res t=(*max_element(a+1,a+n+1)-n)/(n-(int)set<int>(p+1,p+n+1).size());
        for(res i=30;~i;i--)if(t&(1<<i))for(res j=1;j<=n;j++)pos[j]=F[pos[j]][i];
        for(res i=1;i<=n;i++){
            if(vis[pos[i]])continue;
            vis[pos[i]]=1,ans[i]=a[pos[i]],S.erase(ans[i]);
        }
        for(res i=1;i<=n;i++)if(!ans[i]){
            auto it=S.lower_bound(a[pos[i]]);
            ans[i]=*it,S.erase(it);
        }
