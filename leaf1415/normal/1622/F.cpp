...
const ll L = 6; ll hmod[L];
struct Hash{
	ll h[L];
	Hash(){ rep(i, 0, L-1) h[i] = 0;}
	Hash(ll x){ rep(i, 0, L-1) h[i] = x % hmod[i];}
	...
	Hash& operator*=(const Hash &ope){
		rep(i, 0, L-1) h[i] *= ope.h[i], h[i] %= hmod[i];
		return *this;
	}
	Hash& operator^=(const Hash &ope){
		rep(i, 0, L-1) h[i] ^= ope.h[i];
		return *this;
	}
	bool operator<(const Hash &ope) const{
		rep(i, 0, L-1) if(h[i] != ope.h[i]) return h[i] < ope.h[i];
		return false;
	}
	bool operator ==(const Hash &ope){
		rep(i, 0, L-1) if(h[i] != ope.h[i]) return false;
		return true;
	}
	bool operator !=(const Hash &ope){return !(*this == ope);}
	Hash inverse(){
		Hash ret;
		rep(i, 0, L-1) ret.h[i] = modpow(h[i], hmod[i]-2, hmod[i]);
		return ret;
	}
};
ostream& operator <<(ostream &os, Hash &ope){
	cout << "("; rep(i, 0, L-1) os << ope.h[i] << (i + 1 == L ? "" : ", "); cout << ")";
	return os;
}
...
int main(void)
{
	...
	beki[0] = Hash(1), beki[1] = Hash(1145143);
	rep(i, 2, n) beki[i] = beki[i-1] * beki[1];
	...
	rep(i, 1, n){
		...
		for(auto p : mp) if(p.snd % 2) h[i] ^= beki[p.fst];
	}
	rep(i, 1, n) h[i] ^= h[i-1];
	...
	vector<ll> tmp; Hash sum;
	if(n % 2) tmp.pb(n), sum ^= h[n];
	tmp.pb(n/2), sum ^= h[n/2];
	if(sum == all) ans = tmp;
	else tmp.pb(2), ans = tmp;
	...
	if(all == Hash(0)) ans.clear();
	
	
	vector<ll> avec;
	for(auto x : ans) used[x] = true;
	rep(i, 1, n) if(!used[i]) avec.pb(i);
	outl(sz(avec));
	outl(avec);
