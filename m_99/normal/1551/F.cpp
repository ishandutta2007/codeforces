...
int k;
void dfs(int f,int cur,int p,vector<vector<int>> &E){
	dp[cur].assign(105,0);
	rep(i,E[cur].size()){
		int to = E[cur][i];
		if(to==p)continue;
	...
	if(f==cur){
		...
		rep(i,105){
			vector<mint> ddp(k+1,0);
			ddp[0] = 1;
			rep(j,E[cur].size()){
				int to = E[cur][j];
				if(dp[to].size()<=i)continue;
				for(int l=k;l>=1;l--){
					ddp[l] += ddp[l-1] * dp[to][i];
				}
			}
			ans += ddp.back();
		}
	...
	dp[cur].insert(dp[cur].begin(),1);
	
	while(dp[cur].back()==0)dp[cur].pop_back();
