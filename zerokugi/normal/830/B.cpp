...
struct SegTreeLazy{
	...
	val_t update(int a, int b, opr_t x){
		if(b <= a){
			return hdl.merge(update(a, n, x), update(0, b, x));
		}
		return update(a, b, x, 0, 0, n);
	}
	val_t query(int a, int b){
		if(b <= a){
			return hdl.merge(query(a, n), query(0, b));
		}
...
main(){
	...
	SegTreeLazy<handler> seg(n);
	int p = 0;
	ll ans = 0;
	for(auto mp : a){
		while(!mp.second.empty()){
			auto it = mp.second.lower_bound(p);
			if(it == mp.second.end()) it = mp.second.begin();
			seg.update(p, *it + 1, 1);
			p = *it;
			ans += seg.query(p, p + 1);
			mp.second.erase(it);
