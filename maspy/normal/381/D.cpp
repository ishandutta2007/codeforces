...
void solve() {
  ...
  FOR3(i, 1, K) {
    if (PAR[i]) continue;
    PAR[i] = PAR[i - 1] + 1;
    if (!(PAR[i] & (PAR[i] - 1))) PAR[i + 1] = PAR[i];
  }
  ...
  FOR3_R(lv, 1, N + 1) {
    // lv 
    FOR(q, Q) if (get<0>(query[q]) == lv && get<3>(query[q]) == 0) {
      ll v = get<1>(query[q]);
      vi nums;
      FOR(i, q) {
        auto [t, l, r, x] = query[i];
        if (t != lv || x == 0) continue;
        if (l <= v && v <= r) nums.eb(x);
      }
      FORIN(x, nums) {
        if (CNT[x] == 0) ++ANS[q];
        ++CNT[x];
      }
      FORIN(x, nums) { CNT[x] = 0; }
    }
    // 
    FOR(q, Q) {
      auto [t, l, r, x] = query[q];
      if (t != lv || x == 0) continue;
      query[q] = mt(t - 1, PAR[l], PAR[r], x);
    }
  }
  FOR(q, Q) if (get<3>(query[q]) == 0) print(ANS[q]);
}
