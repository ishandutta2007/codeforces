...
char s[100007],t[100007],o[100007];
int w[26][26];
...
int main()
{
	...
	{
		scanf(" %c %c %d",&a,&b,&c);
		if(w[a-'a'][b-'a']==-1||c<w[a-'a'][b-'a'])w[a-'a'][b-'a']=c;
	}
	for(int i=0;i<26;i++)
		for(int j=0;j<26;j++)
			for(int k=0;k<26;k++)
				if(w[i][k]!=-1&&w[j][i]!=-1&&(w[j][k]==-1||w[i][k]+w[j][i]<w[j][k]))
					w[j][k]=w[i][k]+w[j][i];
	int ans=0;
	if(strlen(s)!=strlen(t))OVER("-1");
	for(int i=0;s[i];i++)
	{
		int sml=Inf;
		for(int j=0;j<26;j++)
			if(w[s[i]-'a'][j]!=-1&&w[t[i]-'a'][j]!=-1&&w[s[i]-'a'][j]+w[t[i]-'a'][j]<sml)
				sml=w[s[i]-'a'][j]+w[t[i]-'a'][j],o[i]=j+'a';
		if(sml==Inf)OVER("-1");
		ans+=sml;
	}
	printf("%d\n",ans);
