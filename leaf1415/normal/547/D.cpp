...
void calc(ll v, vector<ll> &vec)
{
	while(sz(G[v])){
		ll nv = G[v].begin()->fst, id = G[v].begin()->snd;
		vec.pb(id), G[v].erase(P(nv, id)), G[nv].erase(P(v, id));
		v = nv;
	}
}
...
int main(void)
{
	...
	rep(i, 1, 2*b){
		while(sz(G[i])){
			vector<ll> vec;
			ll u = i, v = G[i].begin()->fst, id = G[i].begin()->snd;
			vec.pb(id), G[u].erase(P(v, id)), G[v].erase(P(u, id));
			calc(v, vec);
			reverse(all(vec));
			calc(u, vec);
			reps(i, vec){
