...
inline void AC() {
    ...
    for(_ i = n; i >= 1; --i) b[i] = b[i+1]+a[i];
    _ s = 0;
    for(_ i = 1; i <= n; ++i) st.insert(mp(p[i], -i));
    for(_ i = n; i >= 1; --i) {
        st.erase(mp(p[i], -i));
        set<_p>::iterator it = st.lower_bound(mp(b[i], -(_)1E17));
        if(it == st.end()) continue;
        _p t = *it;
        if(t.first != b[i]) continue;
        s = max(s, n-i+1-t.second);
    }
    writeln(s);
