...
void solve() {
    ...
    int side_length = 0;
    while ((side_length + 1) * 2 <= n && s[side_length] == s[n - 1 - side_length]) {
        side_length++;
    }

    palindromes p(all(s));

    pair<int, pii> best = {0, {-1, -1}}; // length, [#left, #right]
    for (int i = 0; i < 2 * n - 1; i++) {
        ...
        // can't attach palindrome to either side
        if (l > side_length && r < n - 1 - side_length)
            continue;
        ...
        ckmax(best, {left + right, {left, right}});
    }

    auto [left, right] = best.second;
    ps(s.substr(0, left) + s.substr(n - right, right));
}
