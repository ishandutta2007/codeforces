...
/*Main-head---------------------------------------------------------------------------------------*/
    int n,m;
    cin>>n>>m;
    auto f=[](int x,int y)->int{if(x<y-1||y==1)return 1;else if(y==2)if(x==1)return 3;else return 4;else if(y%2)if(x==y-1)return 2;else return y/2+1;else if(x==y-1)return 1;else return y/2-1;};
    for(int x=1;x<=n;x++)
    {
        for(int y=1;y<=m;y++)
        cout<<f(x,n)*f(y,m)<<' ';
