...
int dia(Graph &g) {
    int n = si(g);
    vv(int, d, n, n, inf<int>);
    rep(i, n) d[i][i] = 0;
    rep(i, n) fore(e, g[i]) d[i][e] = 1;
    rep(k, n) rep(i, n) rep(j, n) chmin(d[i][j], d[i][k] + d[k][j]);
    int ma = 0;
    rep(i, n) rep(j, i) if(d[i][j] != inf<int>) chmax(ma, d[i][j]);
    return ma;
}
struct UnionFind {
    vector<int> data;
...
int main() {
    // rep(n, 1, 10) {
    //     vpi v;
    //     rep(i, n) rep(j, i) v.eb(j, i);
    //     vi ans(6);
    //     rep(b, 1 << si(v)) {
    //         vector<Graph> g(2, Graph(n));
    //         vector<UnionFind> uf(2, UnionFind(n));
    //         rep(i, si(v)) {
    //             add(g[(b >> i & 1)], v[i].fi, v[i].se);
    //             uf[b >> i & 1].unite(v[i].fi, v[i].se);
    //         }
    //         if(uf[0].num > 1 or uf[1].num > 1) continue;
    //         ans[min(dia(g[0]), dia(g[1]))]++;
    //     }
    //     dump(ans);
    // }
    ...
    if(k <= 1) {
        ...
        OUT(n - 1);
        set<pii> s;
        rep(i, n - 1) OUT(i + 1, i + 2), s.emplace(i, i + 1);
        vector<Graph> g(2, Graph(n));
        vector uf(2, UnionFind(n));
        rep(i, n) rep(j, i) {
            if(s.count(pii(j, i)))
                add(g[0], i, j), uf[0].unite(i, j);
            else
                add(g[1], i, j), uf[1].unite(i, j);
        }
        dump(uf[0].num, uf[1].num);

        dump(dia(g[0]), dia(g[1]));
    } else if(k == 3) {
        if(n <= 3) drop(-1);
        set<pii> s;
        OUT(3 + (n - 4) * 2);
        rep(i, 3) OUT(i + 1, i + 2), s.emplace(i, i + 1);
        rep(i, 5, n + 1) OUT(1, i), OUT(3, i), s.emplace(0, i - 1), s.emplace(2, i - 1);
