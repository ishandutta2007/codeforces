...
constexpr int mod2 = (119 << 23) + 1; // 998244353
// constexpr int mod = 1e9 + 9; // 1000000009
using Zp = Z_p<integral_constant<decay<decltype(mod)>::type, mod>>;
using Zp2 = Z_p<integral_constant<decay<decltype(mod)>::type, mod2>>;
...
int main(){
	...
	for(auto r = 0; r < p; ++ r){
		vector<int> cur;
		for(auto i = r; i < n; i += p){
			cur.push_back(a[i]);
		}
		Zp h1 = 1;
		Zp2 h2 = 1;
		for(auto i = 0; i < min(m - 1, (int)cur.size()); ++ i){
			h1 *= x + cur[i];
			h2 *= x + cur[i];
		}
		for(auto i = m - 1; i < (int)cur.size(); ++ i){
			h1 *= x + cur[i];
			h2 *= x + cur[i];
			if(h1 == obj1 && h2 == obj2){
				res.push_back(r + p * (i - m + 1));
			}
			h1 /= x + cur[i - m + 1];
			h2 /= x + cur[i - m + 1];
		}
