...
vector<int> p, pB, invp;

int nxtB(int x)
{
    for (int i = 0; i<B; i++) x = p[x];
    return x;
}
...
void op1()
{
    ...
    for (int i = 1; i<=B; i++)
    {
        if (curx==x||curx==y) break;
        else
        {
            guysx.push_back(curx);
            curx = invp[curx];
        }
    }
    reverse(guysx.begin(), guysx.end());

    vector<int> guysy;
    int cury = invp[y];
    for (int i = 1; i<=B; i++)
    {
        if (cury==x||cury==y) break;
        else
        {
            guysy.push_back(cury);
            cury = invp[cury];
        }
    }
    reverse(guysy.begin(), guysy.end());
    ...
    if (!guysx.empty())
    {
        pB[guysx[0]] = nxtB(guysx[0]);
        for (int i = 1; i<guysx.size(); i++)
        {
            pB[guysx[i]] = p[pB[guysx[i-1]]];
        }
    }

    if (!guysy.empty())
    {
        pB[guysy[0]] = nxtB(guysy[0]);
        for (int i = 1; i<guysy.size(); i++)
        {
            pB[guysy[i]] = p[pB[guysy[i-1]]];
        }
    }
}
