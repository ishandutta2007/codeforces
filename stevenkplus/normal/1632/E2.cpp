...
// ============= Solution ============= //
// f_y(x) = max dist from 0 to any node if centering at y-th ancestor of farthest leaf
//  = max(max[ A[i] if B[i] > 2 * y ], x + y) = max(dp[2 * y + 1], x + y)
// all f_y are non-decreasing over x (___/ shape)
// the ___ part is non-increasing over y
// therefore, the optimal y(x) that minimizes f(x) is non-increasing as well
...
int main() {
   ...
   while (T--) {
      ...
      vector<int> A(n), B(n);
      yc([&](auto rec, int cur, int prv = -1) -> void {
         for (int nxt: ed[cur]) {
            if (nxt == prv) continue;
            A[nxt] = A[cur] + 1;
            rec(nxt, cur);
      ...
      auto f = [&](int x, int y) -> int {
         if (y < 0) return dp[0];
         if (2 * y + 1 >= n) return x + y;
         return max(dp[2 * y + 1], x + y);
      };
      int y = n - 1;
      for (int x = 1; x <= n; ++x) {
