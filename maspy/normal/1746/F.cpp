...
void solve() {
  ...
  auto trial = [&]() -> void {
    using ARR = typename Grp::value_type;
    vc<ARR> H(len(X));
    FOR(i, len(X)) { FOR(j, len(H[i])) H[i][j] = RNG(0, 1 << 30); }

    vc<ARR> dat(N);
    FOR(i, N) dat[i] = H[A[i]];

    FenwickTree<Grp> bit(dat);

    FOR(q, Q) {
      auto [t, a, b, c] = query[q];
      if (t == 1) {
        ll i = a, x = b;
        ARR diff = Grp::op(H[x], Grp::inverse(dat[i]));
        dat[i] = H[x];
        bit.add(i, diff);
      }
      if (t == 2) {
