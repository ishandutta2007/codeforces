...
struct chash {
    ...
    int operator()(unsigned long long mask) const { return hash_f(mask); }
};
...
signed main() {
    ...
    const int CNT = 100;
    gp_hash_table<Mask, int, chash> st;
    for (int t = 0; t < CNT; ++t) {
        int ind = gen() % n;
        Mask mask = msk[ind];
        for (Mask submask = mask; submask > 0; submask = (submask - 1) & mask) {
            ++st[submask];
        }
    }
    vector<pair<int, Mask>> check;
    for (auto i : st) {
        if (i.s > 0.30 * CNT)
            check.pb({i.s, i.f});
    }
    ...
    Mask ans(0);
    for (auto i : check) {
        if (clock() > 2 * CLOCKS_PER_SEC) break;
        int pc = __builtin_popcountll(i.s);
        int was = __builtin_popcountll(ans);
        if (pc > was) {
            int cnt = 0;
            for (auto mask : msk) {
                if ((mask & i.s) == i.s)
