...
int main(){
	...
	for(int i=1; i<=m ;i++){
		...
		sx[2*i-1]=x1[i];sx[2*i]=x2[i]+1;
		sy[2*i-1]=y1[i];sy[2*i]=y2[i]+1;
	}
	sort(sx+1,sx+2*m+1);
	sort(sy+1,sy+2*m+1);
	flow_graph<ll>g(4*m,0,4*m-1);
	for(int i=1; i<=2*m-1 ;i++) g.add(0,i,sx[i+1]-sx[i],0);
	for(int i=1; i<=2*m-1 ;i++) g.add(i+2*m-1,4*m-1,sy[i+1]-sy[i],0);
	for(int i=1; i<=m ;i++){
		for(int cx=1; cx<2*m ;cx++){
			for(int cy=1; cy<2*m ;cy++){
				if(hv[cx][cy]) continue;
				if(sx[cx]>x2[i] || sx[cx+1]<=x1[i] || sy[cy]>y2[i] || sy[cy+1]<=y1[i]) continue;
				hv[cx][cy]=true;
				g.add(cx,cy+2*m-1,1e18,0);
			}
		}
	}
