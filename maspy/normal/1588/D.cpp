...
void solve() {
  ...
  auto c_to_i = [&](char c) -> ll {
    if ('a' <= c && c <= 'z') return c - 'a';
    return c - 'A' + 26;
  };

  auto i_to_c = [&](ll i) -> char {
  ...
  */
  vc<bool> is_valid(K << N);
  Graph<int> G(K << N, 1);
  FOR(k, K) FOR(s, 1 << N) {
    bool valid = [&]() {
      FOR(i, N) {
        ll n = (s >> i) & 1;
        if (n >= IDX[k][i].size()) return false;
      }
      return true;
    }();
    is_valid[(k << N) | s] = valid;
    if (!valid) continue;
    FOR(nxt, K) {
      ll t = [&]() -> ll {
        ll t = 0;
        FOR(i, N) {
          bool find = false;
          ll now = IDX[k][i][(s >> i) & 1];
          FOR(j, IDX[nxt][i].size()) {
            ll p = IDX[nxt][i][j];
            if (now < p) {
              find = true;
              t |= j << i;
  ...
  ll V = K << N;
  vi in_deg(V);
  FORIN(e, G.edges) in_deg[e.to]++;
  vi que;
  FOR(v, V) if (in_deg[v] == 0) que.eb(v);
  vi TOPO;
  TOPO.reserve(V);
  while (!que.empty()) {
    ll v = que.back();
  ...
  FORIN(v, TOPO) {
    if (is_valid[v])
      ...
      continue;
    // print(v, i_to_c(v >> N), v & ((1 << N) - 1), DP[v]);
    FORIN(e, G[v]) {
      if (chmax(DP[e.to], DP[v] + 1)) PAR[e.to] = v;
    }
  }

  ll state = max_element(all(DP)) - DP.begin();
  ll ANS = DP[state];
  print(ANS);
  if (ANS == 0) return print("");
