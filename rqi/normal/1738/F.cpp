...
void solve(){
    ...
    priority_queue<pi> max_deg;
    for(int i = 1; i <= n; i++){
        max_deg.push(mp(d[i], i));
    }
    ...
    vb taken(n+1, 0);
    while(sz(max_deg)){
        int node = max_deg.top().s; max_deg.pop();
        ...
        for(int i = 0; i < d[node]; i++){
            ...
            taken[node] = taken[response] = true;
            
            if(dsu.size(response) == 1){
                dsu.unite(node, response);
            }
            else{
                assert(taken[response]);
                dsu.unite(node, response);
                break;
            }
        }
