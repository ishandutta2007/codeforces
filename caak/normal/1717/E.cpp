...
inline void AC() {
    ...
    n = read();
    for(_ i = 1; i <= n; ++i) g[i] = ss((n+i-1)/i);
    fc[0] = iv[0] = 1;
    for(_ i = 1; i <= n; ++i) {
        fc[i] = fc[i-1]*i%mod;
        iv[i] = pw(fc[i], mod-2);
...
//    for(_ i = n; i >= 1; --i) {
//        a[i] = i-1;
//        for(_ k = 2; k <= n; k += i) a[i] -= a[k];
//    }
    for(_ i = n; i >= 1; --i) if(i+i <= n) {
        ++g[i][2];
        for(_ j = 3; i*j <= n; ++j) {
            if(~a[j]) {
                g[i][j] = a[j];
