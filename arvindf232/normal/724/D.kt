...
fun main() {
    ...
    val things = IntArray(str.length)

    for((i,a) in str.withIndex()){
        things[i] = a.toInt() - 'a'.toInt()
    }

    val ok = BinarySearchFirstTrue(0,26)BS@{
        var lastseen = -1
    ...
    for((i,v) in things.withIndex()){
        if(v < ok){
            ...
            if(i - lastseen >= m){
//                println("bad at $i")
                for(del in 0 until m){
                    if(things[i- del] == ok){
                        ret[ok] += 1
                        lastseen = i - del

//                        println("readding ${lastseen}")
                        break
                    }
                }
            }
...
//    println(letters.length)
    val at = mutableListOf<Char>()
    for((i,c) in ret.withIndex()){
        repeat(c){
            at.add(letters[i])
        }
    }
...
//fun CRT(a1:Int,a2:Int,m1:Int,m2:Int):Int?{
//    var (k, m, v) = Triple(m1, m2, a2-a1)
//    val d = gcd(k, m)
//    if (v % d != 0)
//        return null
//    v /= d
