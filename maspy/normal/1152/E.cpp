...
#line 2 "/home/maspy/compro/library/graph/eulerwalk.hpp"
...
void solve() {
  ...
  VEC(ll, A, N - 1);
  VEC(ll, B, N - 1);
  FOR(i, N - 1) if (A[i] > B[i]) return print(-1);
  vi X = A;
  X.insert(X.end(), all(B));
  UNIQUE(X);
  for (auto&& a: A) a = LB(X, a);
  for (auto&& a: B) a = LB(X, a);

  Graph G(len(X));
  FOR(i, N - 1) { G.add(A[i], B[i]); }
  G.build();
  auto P = euler_walk(G);
  if (len(P) == 0) return print(-1);
  for (auto&& p: P) p = X[p];
  print(P);
}
