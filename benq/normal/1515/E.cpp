...
mi brute(vb cur) {
	...
	F0R(i,sz(cur)) if (!cur[i]) {
		auto CUR = cur;
		CUR[i] = 1;
		if (i > 1 && CUR[i-2]) CUR[i-1] = 1;
		if (i+2 < sz(cur) && CUR[i+2]) CUR[i+1] = 1;
		num += brute(CUR);
	}
	if (num == 0) num = 1;
	return num;
...
vmi convolve(const vmi& a, const vmi& b) {
	vmi res(sz(a)+sz(b)-1);
	F0R(i,sz(a)) F0R(j,sz(b)) res[i+j] += scmb[i+j][j]*a[i]*b[j];
	return res;
}
...
int main() {
	...
	dp[0][0] = 1; // total, # of comps
	F0R(i,N) {
		F0R(j,i+1) {
			dp[i+1][j+1] += (j+1)*dp[i][j];
			dp[i+1][j] += 2*j*dp[i][j]; // one side
			dp[i+2][j] += 2*j*dp[i][j]; // one side
			if (j > 1) {
				// dp[i+1][j-1] += (j-1)*dp[i][j];
				dp[i+2][j-1] += 2*(j-1)*dp[i][j];
				dp[i+3][j-1] += (j-1)*dp[i][j];
			}
		}
	}
	// FOR(i,1,N+1) dbg(i,dp[i][1]);

	ps(dp[N][1]);


	// FOR(i,1,7) dbg(i,brute(vb(i)));
	// dp[0] = {1};
	// FOR(i,1,N+1) {
	// 	FOR(j,1,i+1) {
	// 		int l = j-1, r = i-j;
	// 		FOR(x,max(l-1,0),l+1) FOR(y,max(r-1,0),r+1) {
	// 			if (x == l && y == r && l && r) continue;
	// 			if (x == l-1 && x == 0) continue;
	// 			if (y == r-1 && y == 0) continue;
	// 			// vmi res = conv(dp[x],dp[y]);
	// 			// each(t,res) t *= comb[x][y];
	// 			dp[i] += convolve(dp[x],dp[y]);
	// 		}
	// 	}
	// 	dp[i] = shift(dp[i],1);
	// 	mi sum = 0; each(t,dp[i]) sum += t;
	// 	dbg("HA",i,sum);
	// }
	// mi ans = 0;
	// each(t,dp[N]) ans += t;
