    ...
    auto add_edge = [&](int u, int v, int edge_id) {
        ...
        if (is_odd(u) and is_odd(v)) {
            if (p[u] == v) {
                ...
                int edge_color = col[u] ^ RED ^ BLUE;
                if (edge_color == RED) ans += edge_hash;
                mint nxt_sum = sum[u] + sum[v] + edge_hash;
                mint nxt_red_sum = red_sum[u] + red_sum[v] + (edge_color == RED ? edge_hash : 0);

                p[p[u]] = p[v], p[p[v]] = p[u];
                sum[p[u]] = sum[p[v]] = nxt_sum;
                red_sum[p[u]] = red_sum[p[v]] = nxt_red_sum;
        ...
        } else if (is_odd(u) or is_odd(v)) {
            if (is_odd(u)) std::swap(u, v);

            int edge_color = col[v] ^ RED ^ BLUE;
            if (edge_color == RED) ans += edge_hash;

            p[p[v]] = u, p[u] = p[v];
            mint nxt_sum = sum[v] + edge_hash;
            mint nxt_red_sum = red_sum[v] + (edge_color == RED ? edge_hash : 0);

            sum[p[v]] = sum[u] = nxt_sum;
            red_sum[p[v]] = red_sum[u] = nxt_red_sum;
            col[u] = edge_color;
