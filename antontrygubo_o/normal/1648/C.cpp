...
int main()
{
    ...
    int ways = facs[n];
    for (auto it: S) ways = mul(ways, invfacs[it.second]);

    FenwickTree tree(2e5 + 100);

    for (auto it: s) tree.add(it, 1);
    ...
    //cout<<X.order_of_key(mp(2, -1))<<endl;

    //cout<<endl;

    for (int i = 0; i<min(n, m); i++)
    {
        int smaller = tree.sum(0, t[i]-1);

        ans = add(ans, mul(ways, mul(smaller, get_inv(n-i))));

        //cout<<i<<": "<<smaller<<' '<<ways<<endl;
