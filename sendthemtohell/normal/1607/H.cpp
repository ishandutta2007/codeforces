...
map<int, vector<tr>> mp;
pair<int, int> ans[200 * 1000 + 57];
...
inline void solve() {
    ...
    while (t--) {
        ...
        for (auto& v : mp) {
            ...
            for (auto& tt : v.y) {
                int mn = max(0, tt.a - tt.m);
                int mx = v.x - max(0, tt.b - tt.m);
                sg.push_back({ { mn, mx }, tt.id });
            }
            sort(all(sg), [](pair<pair<int, int>, int>& a, pair<pair<int, int>, int>& b) {return a.x.y < b.x.y; });
            int lft = -1;
