...
namespace quick_IO {
	...
	cl_pp(Checker1,int,short,long,long long,unsigned int,unsigned long,unsigned long long,double,long double,float)
	...
	{
    	register char ch=lastc==-2?gc():lastc;for (;blank(ch);ch=gc())if(ch==EOF)return 0;
		for (;ch!=-1&&!blank(ch);ch=gc()) *s++=ch;*s=0;lastc=ch;
  	}
	...
	cl_pp(Checkers1,int,short,long,long long,unsigned int,unsigned long,unsigned long long)
	template <typename T>inline void ycl(typename Checkers1<T>::Type c){x=0;}
	template <typename T>void ycl(T c){}
	template<typename T>void writem(typename Checker1<T>::Type c) {
		if(c<0)putchar('-'),c=-c;
		ycl<T>(c);
		if(!x)return;long double y=0.5;
		for(ll i=1;i<=x;i++)y/=10;c+=y;writellm(ll(c),y-(x?x+1:0));c-=ll(c);
		pc('.');for(ll i=1;i<=x;i++)c*=10,pc(char(c)^48),c-=char(c);
	}
...
ld eps=1e-12;
ld p[2222],u[2222],f[2222];
ll cnt1,cnt2,n,a,b;
void check(ld mid1,ld mid2){
//	writeln(mid1,' ',mid2);
//	printf("%lf %lf\n",double(mid1),double(mid2));
	cnt2=0,cnt1=0;
	ll lcnt1=cnt1,lcnt2=cnt2;
	f[0]=0;
	rep(i,1,n){
		f[i]=f[i-1];
		if(f[i-1]+p[i]-mid1>f[i]+eps){
			f[i]=f[i-1]+p[i]-mid1;
			cnt1=lcnt1+1;
			cnt2=lcnt2;
		}
		if(f[i-1]+u[i]-mid2>f[i]+eps){
			f[i]=f[i-1]+u[i]-mid2;
			cnt1=lcnt1;
			cnt2=lcnt2+1;
		}
		if(f[i-1]+p[i]-mid1+u[i]-p[i]*u[i]-mid2>f[i]+eps){
//		debugln(f[i-1]+p[i]-mid1+u[i]-p[i]*u[i]-mid2+cnt1*mid1+cnt2*mid2,' ',f[i]+cnt1*mid1+cnt2*mid2);
			f[i]=f[i-1]+p[i]-mid1+u[i]-p[i]*u[i]-mid2;
			cnt1=lcnt1+1;
			cnt2=lcnt2+1;
		}
//		debugln(i,' ',cnt1,' ',cnt2,' ',f[i-1]+p[i]-mid1+u[i]-p[i]*u[i]-mid2+cnt1*mid1+cnt2*mid2,' ',f[i]+cnt1*mid1+cnt2*mid2);
		lcnt1=cnt1,lcnt2=cnt2;
	}
}
int main(){
	//IO(D);
	...
	rep(i,1,50){
		ld mid1=(l1+r1)/2.0;
//		fprintf(stderr,"%lf %lf %lf\n",double(l1),double(mid1),double(r1));
		l2=0,r2=1;
		rep(j,1,50){
			ld mid2=(l2+r2)/2.0;
			check(mid1,mid2);
	...
	writeln(gs(f[n]+a*r1+b*r2,0,6));
//	printf("%lf",(double)(f[n]+a*r1+b*r2));
//	writeln()
	return 0;
}