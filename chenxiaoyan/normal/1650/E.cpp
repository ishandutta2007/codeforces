...
void mian() {
	n = read(), d = read();
	REP(i, 1, n) a[i] = read();
	Mn[0] = mN[n] = mN[n + 1] = inf;
	Mx[0] = mX[n] = mX[n + 1] = -inf;
	REP(i, 1, n) Mn[i] = min(Mn[i - 1], a[i] - a[i - 1]), Mx[i] = max(Mx[i - 1], a[i] - a[i - 1]);
	PER(i, n - 1, 1) mN[i] = min(mN[i + 1], a[i + 1] - a[i]), mX[i] = max(mX[i + 1], a[i + 1] - a[i]);
	tii mn(inf, inf);
	REP(i, 1, n) chkmn(mn, mt(a[i] - a[i - 1], i));
	int ans = 0;
	for(int x : {Y(mn) - 1, Y(mn)}) {
		if(!x) continue;
		int nw = min(Mn[x - 1], mN[x + 1]);
		if(x < n) chkmn(nw, a[x + 1] - a[x - 1]);
		int mx = max(Mx[x - 1], mX[x + 1]);
		if(x < n) chkmx(mx, a[x + 1] - a[x - 1]);
		mx /= 2;
		chkmx(mx, d - (x == n ? a[n - 1] : a[n]));
		chkmn(nw, mx);
		chkmx(ans, nw);
	}
	prt(ans - 1), pc('\n');
