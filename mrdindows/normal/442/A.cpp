...
int main()
{
	...
	int best = 10;
	for (int maska = 0; maska < (1 << 5); ++maska){
		for (int maskb = 0; maskb < (1 << 5); ++maskb){
			bool ua[10];
			bool ub[10];
			FI(5) FJ(5) t[i][j] = c[i][j] = aa[i][j] = bb[i][j] = 0;
			FI(5) 
			{
				ua[i] = checkbit(maska, i); 
				ub[i] = checkbit(maskb, i);
			}
			int cur = bits_count(maska) + bits_count(maskb);
			if (cur >= best) continue;
			for (int i = 0; i < n; ++i){
				if (ua[a[i]] && ub[b[i]]) {
				}
				else if (!ua[a[i]] && !ub[b[i]]){
					c[a[i]][b[i]] = 1;
				}
				else if (ua[a[i]]){
					aa[a[i]][b[i]] = 1;
