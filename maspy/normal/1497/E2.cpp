...
void solve() {
  ...
  vv(int, DP, K + 1, N + 1, INF);
  DP[0][0] = 0;
  FOR(a, K + 1) FOR(b, K - a + 1) FOR(L, N) {
    ll R = TO[b][L];
    chmin(DP[a + b][R], DP[a][L] + 1);
  }
  ll ANS = INF;
  FOR(k, K + 1) chmin(ANS, DP[k][N]);
  print(ANS);
}
