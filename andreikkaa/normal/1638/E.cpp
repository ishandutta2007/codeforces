...
ll get_t(int v, int l, int r, int P) {
    ll ans = 0;
    while(1) {
        ans += t[v];
        if(r - l == 1) {
...
void _solve() {
    ...
    set<pair<pair<int, int>, int>> seg = {{{0, n}, 1}};
    vector<ll> pls(n + 1);
    ...
    for(int i = 0; i < q; ++i) {
        ...
        if(s == "Color") {
            ...
            {
                //split left
                auto it = seg.upper_bound({{l, INT_MAX}, INT_MAX});
                assert(it != seg.begin());
                --it;
                assert(it->x.x <= l);
                if(it->x.x < l) {
                    auto p = *it;
                    seg.erase(it);
                    seg.insert({{p.x.x, l}, p.y});
                    seg.insert({{l, p.x.y}, p.y});
                }
            }
            {
                //split right
                auto it = seg.lower_bound({{r, INT_MIN}, INT_MIN});
                assert(it != seg.begin());
                --it;
                assert(r <= it->x.y);
                if(r < it->x.y) {
                    auto p = *it;
                    seg.erase(it);
                    seg.insert({{p.x.x, r}, p.y});
                    seg.insert({{r, p.x.y}, p.y});
                }
            }
            for(;;) {
                auto it = seg.lower_bound({{l, INT_MIN}, INT_MIN});
                if(it == seg.end() || it->x.x == r) {
                    break;
                }
                upd_t(1, 0, n, it->x.x, it->x.y, pls[it->y]);
                seg.erase(it);
            }

            seg.insert({{l, r}, c});
            upd_t(1, 0, n, l, r, -pls[c]);
        } else if(s == "Add") {
            int c, x;
            cin >> c >> x;
            pls[c] += x;
            ...
            auto it = seg.upper_bound({{id, INT_MAX}, INT_MAX});
            assert(it != seg.begin());
            --it;
            int c = it->y;

            cout << pls[c] + get_t(1, 0, n, id) << endl;
        }
