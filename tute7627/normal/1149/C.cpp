...
int main(){
  ...
  bool judge = true;
  struct A{
    ll rpos,lpos,posmin,mxd,mx;
  };
  auto ff=[&](A x,A y){
    A z;
    z.rpos=max({x.rpos,y.rpos,y.mxd-x.posmin*2});
    z.lpos=max({x.lpos,y.lpos,x.mxd-y.posmin*2});
    z.mx=max({x.mx,y.mx,x.lpos+y.mxd,x.mxd+y.rpos});
    z.mxd=max({x.mxd,y.mxd});
    z.posmin=min({x.posmin,y.posmin});
    return z;
  };
  auto gg=[&](A x,ll y){
    x.rpos-=y;
  ...
  A iden={-INF,-INF,INF,-INF,-INF};
  ll n,q;cin>>n>>q;
  n=2*(n-1);
  LazySegmentTree seg(n,ff,gg,hh,iden,0);
  string s;cin>>s;
  ll now=0;
  rep(i,0,n){
    if(s[i]=='(')now++;
    else now--;
    seg.set(i,{-now,-now,now,now,0});
  }
  seg.build();
  cout<<seg.query(0,n).mx<<endl;
  //rep(i,0,n)cout<<seg.query(i,i+1).mxd<<" ";cout<<endl;
  while(q--){
    ll a,b;cin>>a>>b;a--;b--;
    if(s[a]!=s[b]){
      for(auto z:{a,b}){
