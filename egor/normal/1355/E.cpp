...
public:
    void solve() {
        ...
        sort(all(h));
        vi poi(all(h));
        poi.push_back(accumulate(all(h), 0ll) / n);
        poi.push_back(poi.back() + 1);
        sort(all(poi));
        unique(poi);
        ll sumLess = 0;
        ll less = 0;
        ll sumMore = accumulate(all(h), 0ll);
        ll more = n;
        ll answer = numeric_limits<ll>::max();
        for (int i : poi) {
            while (less < n && h[less] < i) {
                sumLess += h[less];
                sumMore -= h[less];
                less++;
                more--;
            }
            ll totalLess = i * less - sumLess;
            ll totalMore = sumMore - i * more;
            ll mn = min(totalLess, totalMore);
            ll current = mn * min(a + r, m) + (totalLess - mn) * a + (totalMore - mn) * r;
            minim(answer, current);
        }
...
int main() {
    ...
    auto time = clock();
#endif
    ERestavratsionnoeRasstoyanie solver;
