...
int mn[N][N], pre_mn[N][N];

void smin(int& a, int b, int cost = 0) {
    if (b == -1) return;
    if (a == -1) a = b + cost;
    else a = min(a, b + cost);
}
void solve() {
    ...
    auto less = [&](int i1, int i2, int len) {
        bool ans = sa.rank[i1] < sa.rank[i2] && sa.get_lcp(i1, i2) < len;
        return ans;
    };
    auto less_equal = [&](int i1, int i2, int len) {
        return !less(i2, i1, len);
    };

    memset(mn, -1, sizeof(mn));
    memset(pre_mn, -1, sizeof(pre_mn));
    for (int i = 0; i < n; i++) {
        for (int len = 1; len <= n; len++) {
            int j = i-len+1;
            if (j >= 0 && s[j] != '0') {
                if (j == 0) dp[i][len]++, smin(mn[i][len], 1);
                else {
                    if (j-len >= 0 && less_equal(j-len, j, len)) {
                        dp[i][len] += dp[j-1][len];
                        smin(mn[i][len], mn[j-1][len], 1);
                    }
                    dp[i][len] += pre[j-1][len-1];
                    smin(mn[i][len], pre_mn[j-1][len-1], 1);
                }
            }
            ...
            smin(pre_mn[i][len], mn[i][len]);
            smin(pre_mn[i][len], pre_mn[i][len-1]);
        }
    }
    cout << pre[n-1][n] << '\n';
    ll ans = LLONG_MAX;
    for (int len = 1; len <= min(n, 20); len++) if (mn[n-1][len] != -1) {
        ll cur = 0;
        for (int i = n - len; i < n; i++) {
            cur *= 2;
            cur += s[i] - '0';
        }
        ans = min(ans, mn[n-1][len] + cur);
    }
    if (ans < LLONG_MAX) {
        cout << ans << '\n';
        return;
    }
    for (int len = 1; len <= n; len++) if (mn[n-1][len] != -1) {
        num cur = 0;
        for (int i = n - len; i < n; i++) {
            cur *= 2;
            cur += s[i] - '0';
