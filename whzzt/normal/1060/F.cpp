...
const int N=55;
vector<int>adj[N];
ld c[N][N],fac[N],dp[N][N],t1[N],t2[N];
int n,sz[N];
void dfs(int u,int fa){
	...
	for(int v:adj[u])if(v!=fa){
		...
		for(i=1;i<=sz[v];i++)t1[i]=t1[i-1]+dp[v][i-1];
		for(i=0;i<=sz[v];i++)t1[i]=t1[i]*0.5+dp[v][i]*(sz[v]-i);
		for(i=0;i<=sz[v];i++)for(j=0;j<=sz[u];j++)t2[i+j]+=t1[i]*dp[u][j]*c[i+j][i]*c[sz[u]+sz[v]-i-j][sz[v]-i];
		sz[u]+=sz[v];
		for(i=0;i<=sz[u];i++)dp[u][i]=t2[i];
	}
	++sz[u];
}
int main(){
	int i,j,u,v;
	scan(n);
	for(i=0;i<=n;i++)for(j=*c[i]=1;j<=i;j++)c[i][j]=c[i-1][j]+c[i-1][j-1];
	fac[0]=1;
	for(i=1;i<n;i++)scan(u,v),adj[u].pb(v),adj[v].pb(u),fac[i]=fac[i-1]*i;
	for(u=1;u<=n;u++)dfs(u,0),printf("%.12lf\n",(double)(dp[u][n-1]/fac[n-1]));
}