...
signed main(){
    ...
    ui n=in(),a=in(),b=in(),c=in(),d=in();

    constexpr int u[8] = { 1, 7, 11, 13, 17, 19, 23, 29 };
    constexpr int v[30] = {
        0, 1, 0, 0, 0, 0, 0, 2, 0, 0,
        0, 4, 0, 8, 0, 0, 0, 16, 0, 32,
    ...
    bb[0] = 1;
    for (int h = 0; h < 8; h++) {
        int i = (bb.size() - 1) * 30 + u[h];
        if (i > n) bb[i / 30] |= 1 << h;
    }

    int m = (int)sqrt(n) / 30 + 1;

    for (int k = 0; k < m; k++) {
        for (int h = 0; h < 8; h++) {
            if (bb[k] >> h & 1) continue;
            int i = k * 30 + u[h];
            for (int j = i * i; j <= n; j += i) {
                if (int t = v[j % 30]; t) bb[j / 30] |= t;
            }
        }
    }
    ui ans = 0;
