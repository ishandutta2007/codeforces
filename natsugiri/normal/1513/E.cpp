...
// copy a;
template<class T> vector<pair<T, int> > sort_count(vector<T> a) {
    return sort_count(a.data(), a.size());
}
...
void MAIN() {
	...
	if (small.empty() && large.empty()) {
	    ans = 1;
	} else if (small.size() == 1u || large.size() == 1u) {
	    ans = way(VI(A, A+N));
	} else {
	    assert(small.size());
	    assert(large.size());
	    ans = way(small) * way(large) * 2 * nCk(N, middle);
