...
fun main(){
    solve.tc {
        val d = 17
        share(d)
        val parent = IntArray(1 shl d){ it / 2 }
        val removed = 8
        var rp = parent[removed]
        if(rp > removed) {
            rp --
        }
        for(i in 1 until (1 shl d)){
            if(i == removed || i == 1) continue
            val ii = if(i > removed) i-1  else i
            if(parent[i] == removed){
                share("${rp} ${ii}")
            }else{
                val pp = if(parent[i] > removed) parent[i] - 1 else parent[i]
                share("${pp} ${ii}")
            }
        }
...
//    solve.usetc()
    solve.cases{
        ...
        val degree = G.degree()
        val edgeTypes = IntArray(2 * (n-1)){ nlarge }
        val useless = 0
        ...
        fun NewGraph.edgeDFS(e:Int):Int{
            if(edgeTypes[e] != nlarge){
                return edgeTypes[e]
            }
            val v = to[e]
            if(degree[v] > 3){
...
//            e dei new
            ...
            if(new.size == 0){
                ...
                if(new[0] == new[1]){
                    ret = new[0] + 1
                }else if(new[0] + new[1] == -1){
                    ret = -(maxOf(new[0],new[1]))
                }
            }
            edgeTypes[e] = ret
            return ret
        }
        for(e in 0 until G.maxedge){
            G.edgeDFS(e)
        }
        val ret = mint
        first@for(v in 0 until n){
            if(degree[v] > 4) continue
            val here = mint
            var bad = false
            G.NS_E(v){e,v ->
                val new = G.edgeDFS(e)
                ...
                here.add(G.edgeDFS(e))
            }
//            (v+1) dei here
            if(bad){ continue@first}

            here.sort()
            if(here.size == 2 && here[0] == -2 && here[1] == 1){
                ret.add(v)
                continue
            }
            if(here.size == 3 && here[0] == d-2 && here[1] == d-2 && here[2] == d-1){
                ret.add(v)
                continue@first
            }
            if(here.size == 4){
                val min = here[0]
                val need = (-min)
                if(need !in 3..(d-1)) continue@first
                if(here[1] == need - 2 && here[2] == need - 2 && here[3] == need - 1){
                    ret.add(v)
                    continue@first
                }
            }
...
/*
degree count and edge degree count
