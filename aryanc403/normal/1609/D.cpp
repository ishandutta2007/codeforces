...
ii op(ii a, ii b) { return {a.X+b.X,a.Y+b.Y}; }
ii se() { return {0,0}; }
...
// cin>>T;while(T--)
{
    ...
    atcoder::dsu d(n);
    cnt=1;
    atcoder::segtree<ii, op, se> seg(n+2);
    // map<lli,lli> mm;
    auto update=[&](lli c,lli v){
        // mm[c]+=v;
        auto g=seg.get(c);
        g.X+=v;
        g.Y+=v*c;
    ...
    fo(i,m)
    {
        ...
        const lli r=n+1;
        const lli l=seg.min_left<f>(r);
        // dbg(l,r,mm);
        auto g=seg.prod(l,r);
        lli ans=g.Y+(cnt-g.X)*(l-1);
        cout<<ans-1<<endl;
    }
