...
fn run() {
    ...
    for n in 2..=8 {
        ...
        while {
            ...
            let v = p.iter().zip(ip.iter()).map(|(&a, &b)| a.max(b) - a.min(b)).max().unwrap();
            cnt[v] += 1;
            if v == 1 {
                let mut len = [0; 5];
                for i in 0..n {
    ...
    */
    ...
    let m = *ask.iter().max().unwrap();
    let mut zero = vec![M::zero(); m + 1];
    zero[0] = M::one();
    zero[1] = M::one();
    for i in 2..=m {
        zero[i] = zero[i - 1] + M::from(i - 1) * zero[i - 2];
    }
    let mut f = vec![M::one(); m + 1];
    for i in 2..=m {
        f[i] = M::from(i - 1) * f[i - 2];
    }
    ...
    for n in ask {
        ...
        for i in 0..=(n / 4) {
            ...
            let a = 2 * i;
            let mut way = M::one();
            way *= pc.binom(a + rem, rem);
            way *= f[a] * M::new(2).pow((a / 2) as u64);
            way *= zero[rem];
            ans += way;
        }
        writeln!(out, "{}", ans).ok();
