...
class medianThings(val a:LongArray){
    val counts = longSumSimple(a)
    val sums = longSumSimple(LongArray(a.size){1L * a[it] * it})
    fun setvalue(i:Int, v:Long){
        counts.setValue(i,v)
        sums.setValue(i, 1L * i * v)
    }
    fun getmedianDist():Long {
        val median = counts.indexOf(counts.arr[1] / 2)
        val leftc = counts.query(0,median -1)
        val rightc = counts.query(median +  1, a.size - 1)
        val leftTotal = sums.query(0,median - 1 )
        val rightTotal = sums.query(median +  1, a.size - 1)
        return (1L * median * leftc - leftTotal) + (rightTotal - 1L * rightc * median)
    }
}
...
fun main(){
//    val st = longSumSimple(longArrayOf(1,2,3,4))
//    for(i in 0..9){
//        i dei st.indexOf(i.toLong())
//    }
    solve.cases{
        val n = getint()
         val m = getint()
         val q = getint()
         val all = Array(n){IntArray(m)}
        ...
        val xx = medianThings(X)
        val yy = medianThings(Y)
        fun output(){
            put(xx.getmedianDist() + yy.getmedianDist())
        }
        output()
        repeat(q){
            val i = getint() - 1
...
/*
one of the most basic mistakes
 */