...
signed main(){
    ...
    int n=in();
    vec a;
    SegmentTree<int> seg(N,[](ll x,ll y){return x|y;},0);
    rep(i,n){
        a.pb(in());
        seg.set(i,a.back());
    }
    ...
    vec l(n),r(n);
    rep(i,n){
        int next=seg.find_first(i,[&](int x){return x>a[i];});
        r[i] = (next == -1?n-1:next-1);
        next = seg.find_last(i+1,[&](int x){return x>a[i];});
        l[i] = (next == -1 ? 0:next+1);
    }
    set<int> se;
    vec id=iota(n);
