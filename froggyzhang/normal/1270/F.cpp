...
int n,s[N],a[N],buc[N*B+N],pos[N],tot;
ll ans;
char c[N];
int main(){
	scanf("%s",c+1);
	...
	for(int t=1;t<=B;++t){
		for(int i=0;i<=n;++i)++buc[a[i]=(t*s[i]-i+n)];
		for(int i=0;i<=n;++i)ans+=1LL*buc[a[i]]*(buc[a[i]]-1)/2,buc[a[i]]=0;
	}
	for(int i=1;i<=n;++i){
		if(c[i]=='1')pos[++tot]=i;
	}
	...
	for(int i=1;i<=n;++i){	
		if(pos[k]<i)++k;
		for(int j=k;j<=min(tot,k+n/B+1);++j){
			int cnt=j-k+1;
			int l=max((B+1)*cnt,pos[j]-i+1),r=pos[j+1]-1-i+1;
			if(l<=r){
				ans+=r/cnt-(l-1)/cnt;
			}
		}
