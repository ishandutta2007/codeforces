...
int main() {
    ...
    vector<int> a = pref_function(t);
    int n = s.size();
    vector<int> end_here(n, -1);
    ...
    /*for (auto it: can) cout<<it<<' ';
    cout<<endl;*/

    for (int i = m-1; i<n; i++)
    {
        bool check = true;
        for (int k = 0; k<m; k++) if (s[i-k]!=t[m-1-k] && s[i-k]!='?') check = false;
        if (check)
        {
            if (i==m-1) {end_here[i] = 1; overall[i] = 1;
                continue;}

            for (int k = 1; k<m; k++)
            {
                if (can[m-k]&&end_here[i-k]!=-1) end_here[i] = max(end_here[i], end_here[i-k]+1);
            }
            end_here[i] = max(end_here[i], overall[i-m]+1);
        }
        if (i>0) overall[i] = max(end_here[i], overall[i-1]);
    }

    cout<<overall[n-1]<<endl;

    /*for (auto it: end_here) cout<<it<<' ';
    cout<<endl;
    for (auto it: overall) cout<<it<<' ';
    cout<<endl;*/
