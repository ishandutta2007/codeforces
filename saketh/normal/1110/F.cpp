...
void dfs(int u, int p = -1) {
    trav (e, tr.adj[u]) if (int v = e[u]; v != p) {
        leaf[u] = false;
        rdist[v] = rdist[u] + e.w;
        dfs(v, u);
        ckmin(lrange[u].f, lrange[v].f);
        ckmax(lrange[u].s, lrange[v].s);
    }
    if (leaf[u]) lrange[u] = {u, u+1};
}

struct minll { ll operator()(const ll& a, const ll& b) const { return min(a, b); } } mll;
using segtree = segment_tree_lazy<ll, ll, minll, decltype(plus<ll>()), decltype(plus<ll>())>;
segtree st;

vll ans;
struct query { int l, r, i; };
vector<vector<query>> qq;

void adjust(pii lr, int wt) {
    if (lr.f > lr.s) { st(0, st.SZ, -wt); }
    else {
        if (lr.f > 0) st(0, lr.f, -wt);
        st(lr.f, lr.s, wt);
        if (lr.s < st.SZ) st(lr.s, st.SZ, -wt);
    }
}
...
int main() {
    ...
    int N, Q; re(N, Q);
    vector<E> edges(N-1);
    F0R (i, N-1) { E& e = edges[i]; re(e.u, e.w); --e.u; e.v = i+1; }
    tr = tree<E>(edges);

    leaf.resz(N, true), lrange.resz(N, {INT_MAX, -1}), rdist.resz(N);
    dfs(0);

    st = segtree(N, LLONG_MAX/2, 0ll, mll, plus<ll>(), plus<ll>());
    st.set_leaves([&](int i) { return leaf[i] ? rdist[i] : LLONG_MAX/2; });

    qq.resz(N);
    F0R (q, Q) { int v, l, r; re(v, l, r); qq[v-1].pb({ l-1, r, q }); }
