...
void solve() {
  ...
  auto get = [&]() -> vc<int> {
    vc<int> A;
    STR(S);
    for (auto&& x: S) A.eb(x - 'a');
    return A;
  };
  auto S = get(), A = get(), B = get();
  if (A > B) return NO();
  ...
  auto check = [&](vc<int> C) -> bool {
    vc<int> cnt(K);
    for (auto&& x: C)
      if (x != -1) cnt[x]++;
    if (MAX(cnt) >= 2) return false;
    vc<int> T;
    for (auto&& x: S) T.eb(C[x]);
    if (MIN(T) == -1) return false;
    return (A <= T && T <= B);
  };
