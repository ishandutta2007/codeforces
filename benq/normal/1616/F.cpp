...
using mi = mint<3,5>; // 5 is primitive root for both common mods
using vmi = V<mi>;
...
void make_equation(vi v, mi sum) {
	dbg("make_equation",v,sum);
	equations.pb(vmi(M+1));
	each(t,v) equations.bk[t] = 1;
	equations.bk.bk = sum;
}

void solve(int tc) {
	re(N,M);
	V<vi> col(N+1,vi(N+1,-1));
	vpi edge;
	equations.clear();
	F0R(i,M) {
		ints(A,B,C);
		edge.pb({A,B});
		col[A][B] = col[B][A] = i;
		if (C != -1) make_equation({i},C);
	}	
	FOR(A,1,N+1) FOR(B,A+1,N+1) FOR(C,B+1,N+1) if (col[A][B] != -1 && col[B][C] != -1 && col[C][A] != -1) {
		make_equation({col[A][B],col[B][C],col[C][A]},0);
	}
	dbg("BEF");
	each(t,equations) dbg(t);
	gauss(equations);
	dbg("AFT");
	each(t,equations) dbg(t);
	bool bad = 0;
	...
	F0R(i,sz(equations)) {
		int j = 0;
		while (j < sz(equations[i]) && equations[i][j] == 0) ++j;
		if (j == sz(equations[i])) continue; 
		if (j == sz(equations[i])-1) {
			bad = 1;
		} else {
			ans[j] = equations[i].bk.v;
		}
	}
	if (bad) {
		ps(-1);
	...
	ps();
	// while (1) {
	// 	for (auto [A,B,C]: triangle) {
	// 		int det = (col[A][B] != -1)+(col[B][C] != -1)+(col[C][A] != -1);
	// 	}
	// }
}
