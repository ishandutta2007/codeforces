...
public:
    void solve() {
        ...
        int k = in.readInt();
        arri u, v, w;
        in.readArrays(m, u, v, w);
        decreaseByOne(u, v);

        arr<vector<pii>> edges(n, vector<pii>());
        for (int i : range(m)) {
            edges[u[i]].emplace_back(w[i], v[i]);
        }
        arr2d<int> id(k + 1, k);
        int cid = 0;
        ...
        arr<bool> bad(cid, false);
        arr<ull> incoming(n, 0);
        for (int i : range(n)) {
            sort(all(edges[i]));
            for (int j : range(edges[i].size())) {
                if (isSet(incoming[edges[i][j].second], id(edges[i].size(), j))) {
                    bad[id(edges[i].size(), j)] = true;
                } else {
                    incoming[edges[i][j].second] = setBit(incoming[edges[i][j].second], id(edges[i].size(), j));
                }
            }
        }
        arr<ull> forbidden(cid, 0);
        for (ull mask : incoming) {
            for (int i : range(cid)) {
                if (isSet(mask, i)) {
                    forbidden[i] |= mask;
                }
        ...
        int answer = 0;
        RecursiveFunction rec = [&](const auto& self, int at, ull used) -> void {
            if (at == k + 1) {
                answer++;
                return;
            }
            for (int i : range(at)) {
                if (!bad[id(at, i)] && (used & forbidden[id(at, i)]) == 0) {
                    self(at + 1, setBit(used, id(at, i)));
                }
            }
        };
        rec(1, 0);
