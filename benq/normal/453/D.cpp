...
template<class T> struct modular {
    ...
    friend void pr(modular a) { pr(a.val); }
...
mi mul(mi a, mi b) { // matthew99 HELP
	ll x = a.val, y = b.val;
...
int main() {
    setIO(); re(m,t,MOD); 
    e.rsz(1<<m); F0R(i,1<<m) e[i] = rand() % MOD; re(e);
    b.rsz(m+1); F0R(i,m+1) b[i] = rand() % MOD; re(b);
    MOD *= 1<<m;
    B.rsz(1<<m), E.rsz(1<<m);
    F0R(i,1<<m) {
    ...
    // ps("HUH",B,E);
    fft(E); fft(B); 
    F0R(i,1<<m) E[i] = mul(E[i],powe(B[i],t));
    // exit(0);
    fft(E);
    trav(t,E) ps((ll)t/(1<<m));
}
