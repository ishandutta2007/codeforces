...
#endif
//#STAY AT HOME
const int nax = 1010, bits = 63, qax = 13;
vector <int> subs[nax];
ll res[qax];
ll ans[nax];
int main() {
	...
	scanf("%d", &n);
	vi all(qax);
	fill(all.begin() + (qax / 2), all.end(), 1);
	REP(i, n) {
		subs[i] = all;
		next_permutation(ALL(all));
	}
	REP(q, 13) {
		vi which;
		REP(i, n) if (subs[i][q]) which.push_back(i);
		if (!which.empty()) {
			printf("? %d", (int) which.size());
	...
	REP(b, bits) {
		vi which(qax);
		REP(q, qax) which[q] = (res[q] >> b & 1);
		int at = -1;
		REP(i, n) if (subs[i] == which) at = i;
		debug(imie(b), imie(which));
		if (which == vi(qax, 0));
		else if (at == -1) {
			REP(i, n) ans[i] |= 1ll << b;
		}
		else {
			REP(i, n) if (i != at) ans[i] |= 1ll << b;
		}
	}
	printf("! ");
