...
int main() {
    ...
    // pr(T.res);
    for (int i: T.res) for (int j: T.adj[i]) ckmin(range[j].s,range[i].s-1);
    reverse(all(T.res));
    for (int i: T.res) for (int j: T.radj[i]) ckmax(range[j].f,range[i].f+1);
    FOR(i,1,n+1) 
    ...
    FOR(i,1,n+1) v.pb({range[i],i});
    sort(all(v));
    set<pi> todo;
    int ind = 0;
    FOR(i,1,k+1) {
        while (v[ind].f.f == i) {
            todo.insert({v[ind].f.s,v[ind].s});
            ind ++;
        }
        if (!sz(todo)) {
            cout << -1;
