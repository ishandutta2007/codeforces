...
int main()
{
    ...
    for (int i = 1; i <= k; ++i)
    { 
        res += (i - 1) * 1ll * a[b[i]] - pref[i - 1];
    }
    minr = res;
    //DBN(res);
    for (int i = k + 1; i <= n; ++i)
    {
        res += k * 1ll * a[b[i]] - (pref[i] - pref[i - k])
            - ((pref[i - 1] - pref[i - k - 1]) - a[b[i - k]] * 1ll * k);
        //res += k * 1ll * (a[b[i]] + a[b[i - k]]) - pref[i] + pref[i - k] - pref[i - 1] + pref[i - k - 1];
        //DBN(res);
        if (res < minr)
        {
            minr = res;
