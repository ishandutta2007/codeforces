...
void solve() {
  ...
  sort(all(X));
  FORIN(x, X) {
    ll a = upper_bound(all(A), x) - A.begin();
    ll b = upper_bound(all(B), x) - B.begin();
    ll cnt_a = upper_bound(all(A), x) - lower_bound(all(A), x);
    ll cnt_b = upper_bound(all(B), x) - lower_bound(all(B), x);
    if (abs(a - b) > 1) {
      NO();
      return;
    }
    if (cnt_a + cnt_b == 2 && (a + b) % 2 == 1) {
      NO();
      return;
    }
  }
