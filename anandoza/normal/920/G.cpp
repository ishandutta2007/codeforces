...
const sieve<int(1e6) + 1> sv;

constexpr int MAX_ANS = 6539381; // (1e6, 510510, 1e6)  -- 510510 is a primorial
...
void solve() {
    ...
    auto count = [&](ll r) {
        int ans = 0;
        auto process = [&](int d) { ans += sv.nums[d].mu * (r / d); };
        sv.for_each_divisor_unordered(n, process, SQUARE_FREE);
        return ans;
    };

    k += count(x);

    auto ans = binarysearch(L1(u, count(u) >= k), 1, MAX_ANS);
