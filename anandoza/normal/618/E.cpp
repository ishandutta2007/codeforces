...
struct A {
    ...
    friend A operator+(const A &l, const A &r) { return {l.translate + r.translate * l.angle, l.angle * r.angle}; }

    A lengthen(double z) const { return {translate * (1 + z / translate.len()), angle}; }

    A rotate(const P &z) const { return {translate * z, angle * z}; }
};
...
int main() {
    ...
    segment_tree st(n, A{P{}, P{1, 0}}, plus<A>());
    st.assign(L1(unused, A{P{1, 0}, P{1, 0}}));

    vector<P> angles;
    for (int i = 0; i < 360; i++) {
        angles.push_back(P{1, 0}.rotate(-(PI / 180) * i));
    }
    ...
    while (m--) {
        ...
        if (type == 1) {
            st.assign(index, st[index].lengthen(z));
        } else {
            st.assign(index, st[index].rotate(angles[z]));
        }

        auto ans = st.accumulate(0, n);
        ps(ans.translate.x, ans.translate.y);
    }
