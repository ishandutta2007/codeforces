...
vector<pii> GetCount(const vector<int>& v) {
    map<int, int> m;
    for (auto x : v) {
        ++m[x];
    }
...
void run() {
    ...
    for (auto [idx, val] : c | Enumerate) {
        for (int i = 1; i <= n; ++i) {
            cnt[val.first * i].pb((int)idx);
        }
    }

    const int s = 0, t = 1;
    g.resize(2 + len(c));
    used.resize(2 + len(c), 0);
    auto dfs = YPredicate([&](auto& dfs, int v, int cur_flow = INF) -> int {
        if (v == t) return cur_flow;
        used[v] = timer;
        for (auto x : g[v]) {
            auto& [to, f, c] = edges[x];
            if (f == c || used[to] == timer) continue;
            if (auto ret = dfs(to, min(cur_flow, c - f)); ret != 0) {
                f += ret;
                edges[x ^ 1].f -= ret;
                return ret;
            }
    ...
    for (auto [idx, val] : c | Enumerate) {
        add_edge(s, (int)idx + 2, val.second);
    }

    int ct = 0;
    ll sm = 0;
    for (auto [crd, vals] : cnt) {
        g.emplace_back();
        used.pb(0);
        add_edge(len(g) - 1, t, 1);
        for (auto x : vals) {
            add_edge(x + 2, len(g) - 1, 1);
        }
        if (auto f = dfs(s); f != 0) {
            ct += f;
            sm += crd;
