...
int main() {
    ...
    cin >> A;
    dbg(A);
    vector P(N, vector<mint>(N)); // i->j prob.
    REP(i, N) REP(j, N) if (i != j) P[i][j] = A[i] / (A[i] + A[j]);
    // REP(i, N) REP(j, N) P[i][j] = A[j] / (A[i] + A[j]);

    vector ps(N, vector<mint>(1 << N));
    REP(i, N) {
        REP(s, 1 << N) {
            mint p = 1;
            REP(j, N) if ((s >> j) & 1) p *= P[j][i];
            ps[i][s] = p;
        }
    }
    dbg(ps);
