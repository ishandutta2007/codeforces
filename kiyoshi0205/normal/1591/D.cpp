...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  vec p(N),rp(N);
  {
    ...
    rep(i,N){
      p[i]=lower_bound(all(ch),v[i])-ch.begin();
      rp[p[i]]=i;
    }
  }
  rep(i,N-2){
    if(p[i]==i)continue;
    if(p.back()==i){
      swap(rp[p[i]],rp[p[N-1]]);
      swap(p[i],p[N-1]);
      swap(rp[p[N-2]],rp[p[N-1]]);
      swap(p[N-2],p[N-1]);
    }else{
      j=rp[i];
      //rotate(i,j=rp[i],N-1)
      swap(rp[p[i]],rp[p[j]]);
      swap(p[i],p[j]);
      swap(rp[p[j]],rp[p[N-1]]);
      swap(p[j],p[N-1]);
    }
  }
  if(is_sorted(all(p)))printYES;
  else printNO;
}
}