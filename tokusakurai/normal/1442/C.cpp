...
int main(){
    INT(N, M);
    vi u(M), v(M);
    Weighted_Graph<ll> G1(2*N);
    rep(i, N) G1.add_edge(i, N+i, inf);
    rep(i, M){
        in(u[i], v[i]);
        u[i]--, v[i]--;
        G1.add_edge(u[i], v[i], 1, true);
        G1.add_edge(N+v[i], N+u[i], 1, true);
    }
    G1.dijkstra(0);
    ll D = min(G1.d[N-1], G1.d[2*N-1]);
    int x = D/inf, y = D%inf;
    if(x >= 18){
        mint ans = mint(2).pow(x)-1+y;
        out(ans), exit(0);
    }
    Weighted_Graph<int> G2(36*N);
    rep(i, N){
        rep(j, 17){
            G2.add_edge(j*N+i, (19+j)*N+i, 1<<j, true);
            G2.add_edge((18+j)*N+i, (1+j)*N+i, 1<<j, true);
        }
    }
    rep(i, M){
        rep(j, 18){
            G2.add_edge(j*N+u[i], j*N+v[i], 1, true);
            G2.add_edge((18+j)*N+v[i], (18+j)*N+u[i], 1, true);
        }
    }
    G2.dijkstra(0);
    int ans = inf;
    rep(i, 36) chmin(ans, G2.d[(i+1)*N-1]);
    out(ans);
}