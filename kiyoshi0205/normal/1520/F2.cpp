...
int main(){
  ...
  cin>>x;
  vector<int> ficalc;
  for(int i=0;i*bk<N;i++)ficalc.emplace_back(queli(i*bk));
  while(T--){
    int fid=lower_bound(all(ficalc),x)-ficalc.begin();
    if(!fid){
      print("! 1");
      for(auto& a:ficalc)--a;
      if(T)cin>>x;
    ...
    assert(0<=fid-1&&fid-1<int(ficalc.size()));
    int ng=(fid-1)*bk,ok=min(ng+bk,N-1),mid;
    while(ok-ng>1){
      mid=(ok+ng)/2;
      if(queli(mid)<x)ng=mid;
      else ok=mid;
    }
    print("! "<<ok+1);
    while(fid<int(ficalc.size()))ficalc[fid++]--;
    if(T)cin>>x;
  }
}