...
void solve(){
	...
	int pile = n;
	if(pile <= 2)cout << 0 << "\n";
	else cout << *prev(pos.end())-*pos.begin()-dis.rbegin()->first << endl;
	...
	for(int i = 0; i < q; ++i){
		...
		if(a){	
			...
			if(pile == 1){}
			else if(pile == 2){dis[abs(*pos.begin()-b)]++;}
			else if(x == pos.end()){
				dis[b-*prev(x)]++;
			}
			else if(x == pos.begin()){
			...
			if(x != prev(pos.end())){
				auto y = next(x);
				dis[*y-*x]--;
				if(!dis[*y-*x])dis.erase(*y-*x);
			}
			if(x != pos.begin() && x != prev(pos.end())){
				dis[*next(x)-*prev(x)]++;
			}
			pos.erase(x);
		}
