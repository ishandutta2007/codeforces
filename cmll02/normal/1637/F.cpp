...
arr d,H,t,a,dfn,islf,L,R,w;
int cc,ans,root,n=read();
pair dfs(int u,int fa=0)
{
	if(u==root)
	{
		int c=0;
		pair y=(pair){0,u},z=(pair){0,u};
		fe(u)
		if(v==fa)continue;
		pair x=dfs(v,u);
		if(y<x)z=y,y=x;
...
{
	initprog();
	rg(n)H[i]=a[i]=t[i]=read();gr
	// std::sort(t+1,t+1+n);
	// int l=std::unique(t+1,t+1+n)-t-1;
	// rg(n)a[i]=std::lower_bound(t+1,t+1+n,a[i])-t;gr
	if(n==2)
	{
		oldl(max(H[1],H[2])*2);
		return 0;
	}
	rg(n-1)int u=read(),v=read();
	addedge(u,v,1),addedge(v,u,1);
	d[v]++,d[u]++;
	gr
	root=1;
	rg(n)if(a[root]<a[i])root=i;gr
	dfs(root);
	// rg(n)pre[i]=max(pre[i-1],w[dfn[i]]);gr
	// for(int i=n;i>=1;i--)suf[i]=max(suf[i+1],w[dfn[i]]);
	// check(root);
	// for(int =1;i)
	// std::sort(w+1,w+1+n);
	// ans+=w[n]-w[n-1];
