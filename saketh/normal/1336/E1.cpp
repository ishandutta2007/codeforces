...
int main() {
    ...
    vll compressed_basis;
    F0R (i, M) {
        if (basis[i]) {
            ll compressed = 0;
            F0R (j, sz(dependent_bits))
                if (basis[i] & (1ll << dependent_bits[j]))
                    compressed ^= 1ll << j;
            compressed_basis.pb(compressed);
        }
    }

    vmn ans(M + 1);

    int B = sz(compressed_basis);
    if (B <= 20) {
        vll res(1 << B);

        F0R (bit, B)
            F0R (inx, 1 << bit)
                res[inx + (1 << bit)] = res[inx] ^ compressed_basis[bit];

        F0R (inx, 1 << B)
            ans[__builtin_popcount(inx) + __builtin_popcountll(res[inx])] += 1;
    } else {
        int D = sz(dependent_bits);
        vvmn dp(B + 1, vmn(1 << D, 0));
        ...
        F0R (i, B) {
            F0Rd (bc, B)
                F0R (cv, 1 << D)
                    dp[bc + 1][cv ^ compressed_basis[i]] += dp[bc][cv];
        }

        F0R (bc, B + 1)
            F0R (cv, 1 << D)
                ans[bc + __builtin_popcount(cv)] += dp[bc][cv];
    }
