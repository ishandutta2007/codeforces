...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		in >> n;
		int SZ = 1 << n;
		auto pr = make_vector<double>(SZ, SZ, 0.0);
		for (int i: range(SZ)) {
			for (int j: range(SZ)) {
				int x;
				in >> x;
		...
		auto p = make_vector<double>(n + 1, SZ, 0.0);
		auto dp = make_vector<double>(n + 1, SZ, 0.0);

		p[0] = vector<double>(SZ, 1.0);

		for (int i: inclusiveRange(1, n)) {
			for (int first = 0; first < SZ; first += (1 << i)) {
				int last = first + (1 << i);
				int mid = first + (1 << (i - 1));

				double mxLeft = 0.0;
				for (int j: range(first, mid)) {
					updateMax(mxLeft, dp[i - 1][j]);
				}

				double mxRight = 0.0;
				for (int j: range(mid, last)) {
					updateMax(mxRight, dp[i - 1][j]);
				}

				for (int j: range(first, last)) {
					dp[i][j] = dp[i - 1][j] + (j < mid ? mxRight : mxLeft);
				}
				for (int j: range(first, last)) {
					int x, y;
					if (j < mid) {
