...
void solve() {
  ...
  */
  ...
  auto seg_f = [&](T x, T y) -> T {
    T z;
    z.size = x.size + y.size;
    // , 
    z.best = max(x.best, y.best);
    chmax(z.best, x.rp + y.lp);
    chmax(z.best, x.rp + y.lm);
    chmax(z.best, x.rm + y.lm);

    z.lp = x.lp;
    if (x.lp == x.size) chmax(z.lp, x.size + y.lm);
    if (x.rp == x.size) chmax(z.lp, x.size + y.lp);

    z.lm = x.lm;
    if (x.lm == x.size) chmax(z.lm, x.size + y.lm);

    z.rp = y.rp;
    if (y.rp == y.size) chmax(z.rp, x.rp + y.size);

    z.rm = y.rm;
    if (y.rm == y.size) chmax(z.rm, x.rp + y.size);
    if (y.lm == y.size) chmax(z.rm, x.rm + y.size);

    return z;
  };

  SegTree<T> seg(seg_f, unit);

  auto set = [&](ll i, ll x) -> void {
    T t;
    t.size = 1, t.best = (x == 0 ? 0 : 1);
    t.lp = (x > 0 ? 1 : 0);
    t.lm = (x < 0 ? 1 : 0);
    t.rp = t.lp;
    t.rm = t.lm;
