...
void solve() {
  ...
  FOR3(n, 1, N) {
    vc<bool> OK(1 << n);
    FOR(i, n) FOR(j, n) if (A[i] + A[j] == A[n]) { OK[1 << i | 1 << j] = 1; }
    FOR(s, 1 << n) if (OK[s]) { FOR(i, n) OK[s | 1 << i] = 1; }
    if (n == 1) OK[1] = 1;

    FOR3(s, 1 << (n - 1), 1 << n) if (DP[s] && OK[s]) {
      ll t = s | 1 << n;
      DP[t] = 1;
      FOR(j, n) DP[t & (~(1 << j))] = 1;
    }
  ...
  FOR3(s, 1 << (N - 1), 1 << N) if (DP[s]) chmin(ANS, popcnt(int(s)));
  if (ANS == INF) ANS = -1;
  print(ANS);
}
