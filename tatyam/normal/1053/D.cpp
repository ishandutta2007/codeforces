...
int main(){
    ...
    /*
     ...
     p_j  p_j - 1  p_i  p_i - 1 
     zero 
     p_j - 1  p_i p_j  zero  (p_i  zero )
      p_i 
     */
    each(p,P){
        auto[_,f]=lcm.try_emplace(p,pll{1,1});
        if(f)continue;
        rest.push_back(--p);
        each(key,val,factor(p)){
            auto&[mx,cnt]=lcm[key];
            if(chmax(mx,val))cnt=0;
            if(mx==val)cnt++;
        }
    }
    // 1  LCM 
    each(x,rest)if([&]{
        each(p,c,factor(x)){
            if(lcm[p].first==c&&lcm[p].second==1)return 0;
        }
        return 1;
    }())zero=1;
    Modint ans=1;
    each(p,val,lcm)ans*=Modint(p).pow(val.first);
    out(ans+zero);
}