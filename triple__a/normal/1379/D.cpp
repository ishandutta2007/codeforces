...
int n,h,m,k;
int hi[maxn],mi[maxn];
priority_queue<pii> evnt;

void add(int u){
    if ((u+1)%m<(u+k)%m) {evnt.push({-(u+1)%m,-1ll}), evnt.push({-(u+k)%m,1ll});}
    else{ evnt.push({-(u+1)%m,-1ll}), evnt.push({-m,1ll}), evnt.push({0ll,-1ll}), evnt.push({-(u+k)%m,1ll});}
}
...
signed main(){
    ...
    cin>>n>>h>>m>>k;
    m/=2;
    rep(i,n) cin>>hi[i]>>mi[i];
    evnt.push({-m,0ll});
    rep(i,n) add(mi[i]);
    int ot=-1,ans=1e9,now=0;
    int res=0;
    while (evnt.size()){
        pii tmp=evnt.top();
        if (tmp.first!=now){
    ...
    cout<<sz(res0)<<" "<<ot<<endl;
    for (auto c:res0) cout<<c<<" ";
    return 0;    
}