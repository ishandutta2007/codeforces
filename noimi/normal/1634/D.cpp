...
int main() {
    TEST {
        ...
        LOCAL {
            n = rnd(4, 7);
            a.resize(n);
            rep(i, 1, n) a[i] = rnd(1, 10);
            random_shuffle(a);
            // n = 6, a = {1, 99, 99, 34, 0, 29};
            dump(n, a);
        }
        int cntt = 0;
        ...
        vi mem;
        rep(i, 2, n) {
            mem.emplace_back(ask(x, y, i));
            if(chmax(ma, mem.back())) t = i;
        }
        y = t;
        int s = x;
        ma = -1;
        vi v;
        rep(i, n) {
            if(i == x or i == t) continue;
            v.emplace_back(ask(x, y, i));
            if(chmax(ma, v.back())) s = i;
        }
        set<int> se;
        rep(i, n) se.emplace(i);
        se.erase(x), se.erase(s), se.erase(y);
        int k = *se.begin();

        if(mem == vi(si(mem), ma)) {
            ans(0, 1);
        } else if(v == vi(si(v), ma)) {
            ans(x, y);
        } else if(ask(s, y, k) < ask(s, x, k)) {
            ans(s, x);
        } else
            ans(s, y);
    }
