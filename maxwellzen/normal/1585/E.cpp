...
int n, q, cur;
vi a, ans, cnt, p, pinv, lb;
vvi adj;
vector<vector<array<int, 3>>> queries;

void dfs(int i) {
    int val = a[i];
    int pos1 = pinv[val], pos2 = lb[cnt[val]+1]-1;
    swap(p[pos1], p[pos2]);
    swap(pinv[p[pos1]], pinv[p[pos2]]);
    lb[cnt[val]+1]--;
    cnt[val]++;
    for (ai<3> tmp : queries[i]) {
        if (lb[tmp[0]]+tmp[1] <= n) ans[tmp[2]] = p[lb[tmp[0]]+tmp[1]-1] + 1;
        else ans[tmp[2]] = -1;
    }
    for (int nb : adj[i]) dfs(nb);
    pos1 = pinv[val], pos2 = lb[cnt[val]];
    swap(p[pos1], p[pos2]);
    swap(pinv[p[pos1]], pinv[p[pos2]]);
    lb[cnt[val]]++;
    cnt[val]--;
...
void solve() {
    ...
    ans.rsz(q);
    queries.clear(); queries.rsz(n);
    F0R(i, q) {
        int v, l, k; cin >> v >> l >> k;
        queries[v-1].pb({l, k, i});
    }
    cnt.assign(n, 0);
    p.rsz(n); F0R(i, n) p[i] = i;
    pinv.rsz(n); F0R(i, n) pinv[i] = i;
    lb.assign(n+1, n); lb[0] = 0;
    dfs(0);
    F0R(i, q) cout << ans[i] << ' ';
    cout << nl;
}
