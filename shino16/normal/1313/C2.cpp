...
int main() {
  int n = IN;
  VLL m(n+2); m[0] = m[n+1] = -1;
  rep(i, n) m[i+1] = IN;
  VI ixl(n+2), ixr(n+2);
  stack<int> st({0}); // non-strictly increasing
  rep(i, 1, n+1) {
    while (m[st.top()] > m[i]) st.pop();
    ixl[i] = st.top();
    st.push(i);
  }
  st = stack<int>({n+1});
  ...
  l[0] = m[0];
  rep(i, 1, n+1)
    l[i] = l[ixl[i]] + m[i] * (i - ixl[i]);
  r[n+1] = m[n+1];
  repr(i, 1, n+1)
    r[i] = r[ixr[i]] + m[i] * (ixr[i] - i);
  ll res = -10; int ix;
  rep(i, 1, n+1) {
    if (chmax(res, l[i] + r[i] - m[i])) ix = i;
  }
  repr(i, ix) chmin(m[i], m[i+1]);
  rep(i, ix, n+1) chmin(m[i+1], m[i]);
  OUT.iter(m.begin()+1, m.end()-1);
}