...
struct RationalNumber{
  ...
  RationalNumber &operator/=(const RationalNumber &x){
    long long q1 = gcd(a, x.a), q2 = gcd(b, x.b); 
  ...
  bool operator<(const RationalNumber &x)const{
    return __int128_t(a) * x.b < __int128_t(b) * x.a;
  }
  bool operator>(const RationalNumber &x)const{
    return a * x.b > b * x.a;
  }
...
int main(){
  ...
  bool judge = true;
  ll n;cin>>n;
  vector<rat>px(n),py(n);
  rep(i,0,n){
    ll a,b,c,d;cin>>a>>b>>c>>d;
    rat x(a,b),y(c,d);
    px[i]=(x+1)/y,py[i]=x/(y+1);
  }
  //cout<<endl;
  //for(auto [a,b]:co)cout<<a spa b<<endl;
  auto mp=Compress(px,py);
  ll sz=mp.size();
  Graph<int>g(sz);
  rep(i,0,n){
    g[mp[px[i]]].EB(mp[py[i]],0,i);
    g[mp[py[i]]].EB(mp[px[i]],0,i);
    //cout<<i spa mp[px[i]] spa mp[py[i]]<<endl;
  }
  vector<P>ret;
  vector<bool>used(sz);
  vector<bool>check(n);
  rep(i,0,sz){
    if(used[i])continue;
    auto dfs=[&](auto &&f,ll k,ll ep)->bool{
      used[k]=true;
      stack<ll>koutai;
      for(auto e:g[k]){
        ...
        check[e.id]=true;
        if(!used[e.to]){
          auto tmp=f(f,e.to,e.id);
          if(tmp)koutai.push(e.id);
        }
        else koutai.push(e.id);
      }
      while(koutai.size()>=2){
        ll x=poll(koutai);
        ll y=poll(koutai);
      ...
      if(koutai.size()==1&&ep!=-1){
        ret.EB(ep+1,poll(koutai)+1);
        return false;
      }
      else return true;
    };
