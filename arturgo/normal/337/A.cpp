...
int main() {
	...
	int nbEleves = read<int>(), nbPuzzles = read<int>();

	auto pieces = sorted(read<vector<int>>(nbPuzzles));

	int mini = 1000;
	for(int i : range(0, nbPuzzles - nbEleves + 1)) {
		mini = min(mini, pieces[i + nbEleves - 1] - pieces[i]);
	}
