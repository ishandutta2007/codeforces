...
int main() {
	int n; R(n);
	vector<int> minv(n, INF), maxv(n, -INF), is(n, 0);
	...
	for (int i = 0; i < n; i++) if (is[i]) iscnt++; else {tmp.pb(minv[i]); tmpmax.pb(maxv[i]);}
	sort(all(tmp));
	
	ll ans = iscnt * n * 2 - iscnt * iscnt;
	
	for (int i = 0; i < tmp.size(); i++) {
		ans += lower_bound(all(tmp), tmpmax[i]) - tmp.begin();
	}
