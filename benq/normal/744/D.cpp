...
void testInf() {
	...
	trav(t,r) {
		ld A = 0;
		F0R(i,sz(z)) A += abs(cross(t,z[i],z[(i+1)%sz(z)]));
		if (A > a) {
			ps(-1);
			exit(0);
		}
	...
	trav(t,r) {
		F0R(i,sz(z)) if (cross(z[i],z[(i+1)%sz(z)],t) == 0) {
			ps(-1);
			exit(0);
		}
	}
...
void check(P x, P y) {
	ld d = dist(P(0,0),x,y); if (d == 0) return;
	d = 1/d/2;
	// ps("WUT",x,y,R,B,d);
	ckmax(ans,d);
}
...
void testB(P x, P y) {
	auto d = cross(x,y,P(0,0)); if (d == 0) return;
	trav(t,B) if (t != y) if (op(cross(x,y,t),d)) return;
	check(x,y);
}
...
void test(P p) {
	R.clear(), B.clear();
	trav(t,r) R.pb(image(p,t));
	trav(t,b) if (t != p) B.pb(image(p,t));
	B = convex_hull(B);
	trav(x,R) trav(y,B) testB(x,y);
	F0R(i,sz(B)) if (sz(B) == 2 || !op(cross(B[i],B[(i+1)%sz(B)],B[(i+2)%sz(B)]),cross(B[i],B[(i+1)%sz(B)],mp(0,0)))) 
		testR(B[i],B[(i+1)%sz(B)]);
	trav(y,B) test(y,y*P(1,1));
}

int main() {
    setIO(); re(n,m);
    r.rsz(n), b.rsz(m); re(r,b);
    testInf();
    trav(t,b) test(t);
