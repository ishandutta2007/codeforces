...
void solve()
{
	...
	for (auto& x : a[0]) if (x) x += 1;
	for (auto& x : a[1]) if (x)x += 1;
	...
	for (int i = n - 1; i >= 0; --i)
	{
		freshmax(b[0][i], b[0][i + 1]);
		freshmax(b[1][i], b[1][i + 1]);
		freshmax(c[0][i], c[0][i + 1]);
		freshmax(c[1][i], c[1][i + 1]);
	}
	...
	auto check = [&](int m)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			if (i % 2 == 0)
			{
				if (a[0][i] > cur) return false;
				cur += 1;
				{
					int k = n - i;
					if (b[0][i] <= m - k - n + 1 && b[1][i] <= m - k + n)
					{
						return true;
					}
				}

				if (a[1][i] > cur) return false;
				cur += 1;
				{
					int k = n - i - 1;
					if (c[1][i + 1] <= m - k - n + 1 && c[0][i + 1] <= m - k + n)
					{
						return true;
					}
				}
