...
struct min_node{
    ...
    void apply(int l, int r, const min_node& b) {
      // mn += b.add;
      // add += b.add;
    }
    void reset_lazy(){
    ...
    void unite(const min_node &a, const min_node &b) {
      // assert (add == 0);
      mn = min(a.mn, b.mn);  
    }
};
...
struct max_node{
    ...
    void apply(int l, int r, const max_node& b) {
      // mn += b.add;
      // add += b.add;
    }
    void reset_lazy(){
    ...
    void unite(const max_node &a, const max_node &b) {
      // assert (add == 0);
      mx = max(a.mx, b.mx);  
    }
};
...
void solve(){
	...
	vector<int> lf(n, -1), lb(n, n + 1), gf(n, -1), gb(n, n + 1);
	...
	segtree<min_node> LB(lb), GB(gb);
	segtree<max_node> LF(lf), GF(gf);

	vector<int> bfs;

	auto add_to_bfs = [&] (int x, int q) {
		assert (d[x] > q);
		d[x] = q;
		LB.modify(x, x, INF);
		GB.modify(x, x, INF);
	...
	auto expand = [&] (int x) {
		if (lf[x] > x){
			...
			auto fn = [&] (const min_node& A){
				return A.mn <= x;
			};
			while (true){
				int z = GB.find_first(x + 1, y, fn);
				if (z == -1)
					break;
				assert (x < z && z <= y);
		...
		if (lb[x] < x) {
			...
			auto fn = [&] (const max_node& A) {
				return A.mx >= x;
			};
			while (true){
				int z = GF.find_first(y, x - 1, fn);
				if (z == -1)
					break;
				assert (x > z && z >= y);
