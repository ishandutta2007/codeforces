...
const int N=1e5+5,M=7005,L=7000;
bitset<M> B[N],U[M],G[M],MU;
int n,q;
int main(){
	int i,j,o,x,y,z;
	scan(n,q);
	for(i=1;i<=L;++i)MU[i]=1;
	for(i=2;i<=L;++i)for(j=i*i;j<=L;j+=i*i)MU[j]=0;
	for(i=1;i<=L;++i)for(j=i;j<=L;j+=i)U[j][i]=1,G[i][j]=MU[j/i];
	for(i=1;i<=q;++i){
		scan(o,x,y);
		if(o==1){
			B[x]=U[y];
