...
int main() {
  ...
  for (int i = 0; i < 4 * n; ++i) {
    int a = $ - 1, b = $ - 1, c = $ - 1;
    rules[a][meta[a][1]++] = {b, c, i + 1};
    ++meta[b][0], ++meta[c][0];
  }
  int v = --n;
  while (ans.size() != n0 * 4) {
    if (meta[v][0] == 9 || meta[v][1] == 0) {
      if (todo.size()) {
        v = todo.back(); todo.pop_back();
        continue;
      } else if (n--) {
        v = n;
        continue;
    ...
    auto t = --meta[v][1];
    ++meta[v][0];
    auto [b, c, i] = rules[v][t];
    --meta[b][0];
    --meta[c][0];
    ans.push_back(rules[v][t][2]);
    todo.push_back(rules[v][t][0]);
    v = rules[v][t][1];
  }
  print("YES");
  print(range{ans.rbegin(), ans.rend()});
}