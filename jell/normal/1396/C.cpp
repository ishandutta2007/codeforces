...
#line 3 "codeforces-workspace\\contest\\1396\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 11 "codeforces-workspace\\contest\\1396\\c\\c.cpp"
namespace workspace {
    constexpr char eol = '\n';
...
solver()
{
    ...
    for(int k=0; k<n; k++)
    {
        if(k<n-1) chle(dp[k+2],dp[k]+2*d+min(r1+r2,r1*(a[k]+2))+min(r1+r2,r1*(a[k+1]+2)));
        if(k==n-2)
        {
            chle(dp[n],dp[k]+d+min(r1+r2,r1*(a[k]+2))+(r1*a[n-1]+r3));
        }
        if(k==n-1)
        {
            chle(dp[n],dp[k]+d*2+min(r1*(a[k]+2),r1+r2));
        }
        chle(dp[k+1],dp[k]+r1*a[k]+r3);
    }
    cout<<dp[n]+d*(n-1)<<eol;
}};