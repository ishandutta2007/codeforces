...
int main() {
    TEST {
        ...
        vvv(int, dp, n, m, 2);
        rep(i, n) rep(j, m) dp[i][j][0] = -inf<int>, dp[i][j][1] = inf<int>;
        rep(i, 2) dp[0][0][i] = a[0][0];
        rep(i, n) rep(j, m) {
            if(i < n - 1) {
                chmax(dp[i + 1][j][0], dp[i][j][0] + a[i + 1][j]);
                chmin(dp[i + 1][j][1], dp[i][j][1] + a[i + 1][j]);
            }
            if(j < m - 1) {
                chmax(dp[i][j + 1][0], dp[i][j][0] + a[i][j + 1]);
                chmin(dp[i][j + 1][1], dp[i][j][1] + a[i][j + 1]);
            }
        }
        // dump(dp[n - 1][m - 1]);
        YES(inc(0, dp[n - 1][m - 1][1], dp[n - 1][m - 1][0] + 1));
    }
}