...
int main() {
    ...
    rep(i, N) {
        ll x = div[a[i]][0];
        for (ll y : div[a[i]])d.merge(x, y);
    }
   
    auto dgr = d.groups();
    vector<int> gr(L);
    rep(i, dgr.size()) {
        for (ll x : dgr[i]) {gr[x] = i; }
    }

    //rep(i, N)cout << gr[a[i]] << endl;
    
    vector<set<int>> lis(N);
    rep(i, N) {
        lis[i].insert(gr[div[a[i]][0]]);
        for (ll x : div[a[i] + 1]) {
            lis[i].insert(gr[x]);
        }
    }

    vector<set<int>> ivlis(L);
    rep(i, N)for (ll x : lis[i]) { ivlis[x].insert(i); }

    ll S = 500;
    vector<int> often;
    rep(i, L) {
        if (ivlis[i].size() > S) { often.push_back(i); }
    }
   
    map<pair<int,int>,int> mp;
    for (ll x : often)for (ll y : often) {
        rep(i, N) {
            if (lis[i].count(x) && lis[i].count(y)) { mp[{x,y}] = 1; break; }
        }
    }
    ...
    rep(q, Q) {
        ...
        if (ivlis[x].size() > S && ivlis[y].size() > S) {
            if (mp[{x, y}] == 1) { cout << 1 << endl; }
            else { cout << 2 << endl; }
        }
        else {
            if (ivlis[x].size() > ivlis[y].size()) { swap(x, y); }

            int ans = 2;
            for (ll i : ivlis[x]) {
                if (lis[i].count(y)) { ans = 1; }
            }
            cout << ans << endl;
        }
    }
