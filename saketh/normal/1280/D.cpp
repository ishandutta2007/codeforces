...
using res = pair<int, ll>;
#define win first
#define bal second

// sz(root) * sz(child) is ok
// all states in root have the top node "open"
void attach(vector<res>& root, vector<res>& child) {
    vector<res> comb(sz(root) + sz(child) - 1, {0, LLONG_MIN/4});
    F0R (rc, sz(root)) F0R (cc, sz(child)) {
        auto [rw, rb] = root[rc];
        auto [cw, cb] = child[cc];
        ckmax(comb[rc + cc], {rw + cw, rb + cb});
    }
    root = comb;
    child.clear();
}

// "close" all the states... needs to be linear
void extend(vector<res>& root) {
    root.eb(0, LLONG_MIN/4);
    F0Rd (i, sz(root) - 1) {
        ckmax(root[i+1], mp(root[i].win + (root[i].bal > 0), 0ll));
    }
}

void solve() {
    int N, M; re(N, M);
    vi bee(N), wasp(N); re(bee, wasp);
    using E = edge; tree<E> tr(N); re(tr);

    vector<vector<res>> dp(N);
    trav (u, tr.reverse_preorder) {
        dp[u] = {mp(0, wasp[u] - bee[u])};
        trav (e, tr.children[u]) attach(dp[u], dp[e(u)]);
        if (u) extend(dp[u]);
    }

    ps(dp[0][M - 1].win + (dp[0][M-1].bal > 0));
}
