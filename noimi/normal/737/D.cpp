...
int b[5000];
__gnu_pbds::cc_hash_table<int, int, custom_hash> mp;
ll dfs(int l, int r, int k) {
    int S = l + (r << 12) + ((ll)k << 24);
    if(mp.find(S) != end(mp)) return mp[S];
    if(r < l) return 5e8;
    if(r - l < k) return 0;
    if(r - l == k) { return b[r] - b[l]; }
    ll A = b[l + k] - b[l];
    ll B = b[l + k + 1] - b[l];
    if(r - l < 2 * k) return mp[S] = max(A, B);
    A = A + min(dfs(l + k, r - k, k) - b[r] + b[r - k], dfs(l + k, r - k - 1, k + 1) - b[r] + b[r - k - 1]);
    if(r - l < 2 * (k + 1)) return mp[S] = max(A, B);
    B = B + min(dfs(l + k + 1, r - k - 1, k + 1) - b[r] + b[r - k - 1], dfs(l + k + 1, r - k - 2, k + 2) - b[r] + b[r - k - 2]);
    return mp[S] = max(A, B);
}
main() {
    LL(n);
