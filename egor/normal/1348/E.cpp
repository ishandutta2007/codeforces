...
public:
    void solve() {
        ...
        arr<ll> answer(k, numeric_limits<ll>::min());
        answer[0] = 0;
        arr<ll> next(k);
        ll add = 0;
        int sb = 0;
        for (int i : range(n)) {
            int fA = max(0, a[i] / k - 1);
            add += fA;
            a[i] -= k * fA;
            int fB = max(0, b[i] / k - 1);
            add += fB;
            b[i] -= k * fB;
            fill(all(next), numeric_limits<ll>::min());
            for (int j : range(a[i] + 1)) {
                int rem = a[i] - j;
                int need = rem % k == 0 ? 0 : k - rem % k;
                if (b[i] >= need) {
                    int delta = (rem + need) / k + (b[i] - need) / k;
                    int db = (b[i] - need) % k;
                    int cb = sb;
                    for (int l : range(k)) {
                        maxim(next[(j + l) % k], answer[l] + (j + l) / k + delta + (cb + db) / k);
                        cb--;
                        if (cb < 0) {
                            cb = k - 1;
                        }
        ...
        out.printLine(add + *max_element(all(answer)));
    }
};
