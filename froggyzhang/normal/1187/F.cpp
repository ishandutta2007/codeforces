...
int main(){
	...
	for(int i=1;i<=n;++i){
		p[i]=1LL*qpow(1LL*(r[i]-l[i]+1)*(r[i+1]-l[i+1]+1)%mod,mod-2)*max(0,min(r[i],r[i+1])-max(l[i],l[i+1])+1)%mod;
		s[i]=((s[i-1]+1-p[i])%mod+mod)%mod;
		ans=(ans+1-p[i]+2LL*(i>=2?s[i-2]:0)*(mod+1-p[i]))%mod;
		ans=(ans+mod)%mod;
	}
	for(int i=1;i<n;++i){
		int z=(mod+1-p[i]-p[i+1]+1LL*qpow(1LL*(r[i]-l[i]+1)*(r[i+1]-l[i+1]+1)%mod*(r[i+2]-l[i+2]+1)%mod,mod-2)*max(0,min(min(r[i],r[i+1]),r[i+2])-max(max(l[i],l[i+1]),l[i+2])+1)%mod)%mod;
		z=(z+mod)%mod;
		ans=(ans+2LL*z)%mod;
	}
	printf("%d\n",ans);
