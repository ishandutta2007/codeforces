...
int main(){
  ...
  rrep(i,0,n){
    if(s[i]==suf[0]){
      if(s[i]<suf[1])t[ord[i]]=true;
    }
    else{
      if(s[i]<suf[0])t[ord[i]]=true;
      suf=string(1,s[i])+string(1,suf[0]);
    }
    //cout<<i spa suf<<endl;
  }
  vector<bool>multi(n);
  //debug(t);
  {
    auto dfs=[&](auto &&f,ll v,ll d,ll lastd,bool exr)->void{
      if(t[v]&&d-lastd<=k&&!exr){
        //cout<<v<<endl;
        ll nv=v;
        rep(_,0,d-lastd){
          //cout<<nv<<endl;
      ...
      if(multi[v])lastd=d,exr=false;
      if(l[v]!=-1)f(f,l[v],d+1,lastd,exr);
      if(multi[v])lastd=d,exr=false;
      if(r[v]!=-1)f(f,r[v],d+1,lastd,exr||!multi[v]);
    };
    dfs(dfs,0,0,-1,false);
  }
  string ret;
