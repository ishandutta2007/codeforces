...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        vec<pair<ll, ll>> dev;
        dev.emplace_back(x0, y0);
        while (dev.back().first - xs + dev.back().second - ys <= t) {
            dev.emplace_back(dev.back().first * ax + bx, dev.back().second * ay + by);
        }
        int answer = 0;
        for (int i : range(dev.size())) {
            ll cx = xs;
            ll cy = ys;
            int cur = 0;
            ll rem = t;
            for (int j : range(i, dev.size())) {
                ll dist = abs(cx - dev[j].first) + abs(cy - dev[j].second);
                if (dist > rem) {
                    break;
                }
                cur++;
