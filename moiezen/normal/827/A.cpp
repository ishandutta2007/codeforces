...
int main()
{
	...
	for(int i=1;i<=n;i++)
		for(int j=0;j<vec[id[i]].size();j++)if(!num[vec[id[i]][j]])num[vec[id[i]][j]]=id[i];
	int lst=0,pos=0;
	for(int i=1;i<=len;i++)
	{
		if(num[i]&&s[num[i]].size()>s[lst].size()-pos)lst=num[i],pos=0;
		if(lst&&pos<s[lst].size())putchar(s[lst][pos++]);
		else putchar('a');
	}
	return 0;
}