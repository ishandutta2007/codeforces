...
public class Round678E {
	...
	boolean[] go(int[] a)
	{
		...
		SegmentTreeRMQ st = new SegmentTreeRMQ(n+5);
		boolean[] ex = new boolean[n+3];
		boolean[] isin = new boolean[n+3];
		int mx = 0;
		for(int i = 0;i < n;i++){
			int v = a[i];
			if(v != 1)ex[1] = true;
			mx = Math.max(mx, v);
			if(v > 1 && st.min(1, v) >= prev[i]){
				ex[v] = true;
			}
			st.update(v, i);
			isin[v] = true;
	...
	public static void main(String[] args) throws Exception { new Round678E().run(); }
