...
inline void solve() {
	int n;
	cin >> n;
	int step = 1000 * 1000 * 1000 / n, cur = step;
	char at_0;
	at_0 = point(0, 0);
	--n;
	while (n && at_0 == point(0, cur)) {
		cur += step;
		--n;
	}
	if (!n) {
	...
	//////////////////////
	int kekx = 1000 * 1000 * 1000;
	int stepx = kekx / n / 3, curx = kekx / 2;
	char at_kek;
	at_kek = point(kekx, curx);
	--n;
	if (at_kek != at_0) {
		stepx = -stepx;
	}
	curx += stepx;
	while (n && at_kek == point(kekx, curx)) {
		curx += stepx;
		--n;
	}
	if (!n) {
	...
	while (n && r - l > 4) {
		--n;
		int m = (l + r) >> 1;
		bool go_up = (stepx > 0) ^ (point(kekx, m) == at_kek);
		if (!go_up) {
			l = m;
		}
		else {
