// time-limit: 1000
// problem-url: https://codeforces.com/contest/1740/problem/D
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	...
	bool result=[&]{
		int numRemoved=0;
		std::vector<char> removed(k);
		for(int target=k; target--;){
			while(not removed[target]){
				removed[a.back()]=true;
				numRemoved++;
				a.pop_back();
			}
			if(numRemoved>=n*m-2){
				// if this happens then the loop above has just run at least one iteration
				// otherwise from the last iteration numRemoved must be <= n*m-1, then -- again, cannot reach this value
				// which means the last removed is adjacent to the initial cell i.e. not adjacent to the final cell
				// so we're stuck
				return false;
			}
			// otherwise there's at least one free cell to move things around, can proceed
