...
int main() {
  ...
  while (not pque.empty()) {
    ...
    for (auto v2 : graph_rev[v])
      if (chmin(dist[v2], d+1))
        pque.emplace(d+1, v2),
        next[v2] = v;
  }
  int mi = 0, ma = 0;
  rep(i, k-1) {
    bool optimal = dist[p[i+1]] == dist[p[i]] - 1;
    int optcnt = 0;
    for (auto v2 : graph[p[i]]) {
      optcnt += dist[v2] == dist[p[i]] - 1;
      if (optcnt > 1) break;
    }
    if (not optimal) mi++, ma++;
    else if (optcnt > 1) ma++;
  }
  OUT(mi, ma);
}