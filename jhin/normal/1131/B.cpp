...
long long m,x,n,y,z,x1,k,mx,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500000];string s2;vector<int>v,v2[100003];long long sum,ans,num;char ch;
...
for(int i=0;i<n;i++)
{
    ...
    cin>>x>>y;
    if(min(x,y)-max(x2,y2)>=0){
    ans+=min(x,y)-max(y2,x2)+1;
    if(z==0)ans--;}
    if(x==y)z=0;
    else z=1;
    x2=x;y2=y;
