...
int main() {
    R(n);
    REP1(i,1,n) R(a[i]);
    REP1(i,1,n) l[i]=max(l[i-1]+1,a[i]);
    for ( int i=n; i>=1; i-- ) r[i]=max(r[i+1]+1,a[i]);
    REP1(i,1,n) ls[i]=ls[i-1]+(l[i]-a[i]);
    for ( int i=n; i>=1; i-- ) rs[i]=rs[i+1]+(r[i]-a[i]);
    LL ans=LLONG_MAX;
    REP1(i,1,n) {
        LL now=0;
        now+=ls[i];
        now+=rs[i+1];
        now+=max(0,r[i+1]+1-l[i]);
        chkmin(ans,now);
    }
    W(ans);
    return 0;
