...
void solve(){
  ...
  for(li i = (1); i <= (li)(n-1); ++i) {
    if(ask(L,{i})) {
      for(li j = (i+1); j <= (li)(n-1); ++j) if(ask(L,{j}) == 0) ans.push_back(j);
      li lo = 0, hi = i-1;
      while(lo != hi) {
        li mi = (lo+hi)/2;
        vi H; for(li j = (lo); j <= (li)(mi); ++j) H.push_back(j);
        if(ask(H,{i})) hi = mi; else lo = mi+1;
      }
      for(li j = (0); j <= (li)(i-1); ++j) if(j != lo) ans.push_back(j);
      break;
    }else{
      L.push_back(i);
    }
