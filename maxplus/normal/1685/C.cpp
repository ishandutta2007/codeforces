...
Mountain scan(int bal, int l, int r, int flip, bool brk = 1) {
  int flap = sgn(r - l), mx = bal, ot = 0, oz = -1;
  for (int i = l; i != r; i += flap) {
    bal += s[i] == '('? flip: -flip;
    if (maxb(mx, bal)) {
      ot = (i - l) * flap + 1;
    } else if (bal < 0) {
...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    int sl = 0, sr = s.size(), cl = sl, cr = sr, bal = 0;
    while (sl != sr) {
      auto ml = scan(bal, sl, sr, 1);
      if (ml.oz == -1) {
        break;
      }
      auto sd = sgn(sr - sl);
      auto mr = scan(bal, sr - sd, sl - sd, -1);
      ans.emplace_back(cl + ml.ot + 1, cr - mr.ot);
      bal = ml.mt + mr.mt;
      cl += ml.ot + mr.oz - mr.ot, cr -= mr.ot + ml.oz - ml.ot;
      tie(sl, sr) = pair{sr - (mr.oz + 1) * sd, sl + (ml.oz - 1) * sd};
    }
    if (ans.size() > 1) {
      auto ml = scan(0, 0, s.size(), 1, 0);
      ans = {{1, ml.ot}, {ml.ot + 1, s.size()}};
    }
    prints(ans);
  }
}