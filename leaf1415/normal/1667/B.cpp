...
ll dp[500005];
SegTree pseg, nseg;
ll pos[500005], neg[500005], z[500005];
...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> comp;
		rep(i, 0, n) comp.pb(s[i]);
		uniq(comp);
		rep(i, 0, n) s[i] = lower_bound(all(comp), s[i]) - comp.begin();
		ll c = sz(comp);
		
		rep(i, 0, c-1) pos[i] = neg[i] = z[i] = -inf;
		pseg = SegTree(digitnum(c, 2));
		nseg = SegTree(digitnum(c, 2));
		pseg.init(), nseg.init();
		
		pos[s[0]] = neg[s[0]] = z[s[0]] = 0;
		pseg.update(s[0], pos[s[0]]), nseg.update(s[0], neg[s[0]]);
		dp[0] = 0;
		
		rep(i, 1, n){
			dp[i] = -inf;
			chmax(dp[i], pseg.query(0, s[i]-1)+i);
			chmax(dp[i], nseg.query(s[i]+1, c-1)-i);
			chmax(dp[i], z[s[i]]);
			chmax(pos[s[i]], dp[i]-i);
			chmax(neg[s[i]], dp[i]+i);
			chmax(z[s[i]], dp[i]);
			pseg.update(s[i], pos[s[i]]);
			nseg.update(s[i], neg[s[i]]);
		}
		cout << dp[n] << "\n";
