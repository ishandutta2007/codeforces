...
void solve(){
    ...
    REP(i, n) {
        if (inputs[i][0] != inputs[i].back()) {
            Itr[inputs[i][0] - '0'].push_back(i);
        }
        else {
            hoge |= (1 << (inputs[i][0] - '0'));
        }
    }
    if (hoge == 3 and Itr[0].size() + Itr[1].size() == 0) {
        cout << "-1" << endl;
        return;
    }
    if (Itr[0].size() > Itr[1].size()) swap(Itr[0], Itr[1]);
    vector<int> ans;
    int backs = (int)Itr[1].size() - 1;
    set<string> already;
    REP(i, Itr[0].size()) {
        already.insert(inputs[Itr[0][i]]);
    }
    while (Itr[0].size()+1 < Itr[1].size()) {
        if (backs == -1) {
            cout << -1 << endl;
            return;
        }
        reverse(ALL(inputs[Itr[1][backs]]));
        if (already.find(inputs[Itr[1][backs]]) == already.end()) {
            ans.push_back(Itr[1][backs]);
            Itr[1].pop_back();
            Itr[0].push_back(0);
        }
        backs--;
