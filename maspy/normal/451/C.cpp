...
void solve() {
  LL(N, K, d1, d2);
  auto f = [&](ll d1, ll d2) -> bool {
    ll a = (K + d1 + d1 + d2) / 3;
    ll b = a - d1;
    ll c = b - d2;
    if (min({a, b, c}) < 0 || a + b + c != K) return false;
    ll rest = N - K;
    ll a1 = (rest - d1 - d1 - d2) / 3;
    ll b1 = a1 + d1;
    ll c1 = b1 + d2;
    if (min({a1, b1, c1}) < 0 || a1 + b1 + c1 != rest) return false;
    return true;
  };

  yes(f(d1, d2) || f(d1, -d2) || f(-d1, d2) || f(-d1, -d2));
}
