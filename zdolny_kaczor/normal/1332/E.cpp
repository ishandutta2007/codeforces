...
using matr = array <array<ll, 2>, 2>;
matr operator*(matr a, matr b) {
	matr r = {};
	REP(i, 2) REP(j, 2) REP(k, 2) (r[i][k] += a[i][j] * b[j][k]) %= mod;
	return r;
}
const matr id = {array<ll, 2>{1, 0}, array<ll, 2>{0, 1}};
matr powe(matr a, ll e) {
	matr ans = id;
	while (e) {
		if (e & 1) ans = ans * a;
		a = a * a;
...
int main() {
	...
	r++;
	ll siz = n * 1ll * m;
	int odd = floor_div(r, 2) - floor_div(l, 2), even = ceil_div(r, 2) - ceil_div(l, 2);
	debug(imie(l), imie(r), imie(odd), imie(even));
	if (siz % 2) {
		printf("%lld\n", fix(powe(r - l, siz), mod));
	}
	else {
		matr m = {array<ll, 2>{even, odd}, array<ll, 2>{odd, even}};
		matr p = powe(m, siz);
		ll ans = p[0][0];
		printf("%lld\n", fix(ans, mod));
	}
