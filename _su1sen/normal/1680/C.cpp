...
int main() {
    ...
    loop(t) {
        ...
        vector<int> m2(n + 1, inf);
        rep(i, n) {
            m2[i + 1] = min(m2[i], sum[i] - i);
        }

        int ans = inf;

        sliding_window_minimum<int> smin(n + 1, [&](int i) { return sum[i]; });

        rep(r, n + 1) {
            if (r >= sum[n]) {
                chmin(ans, r - sum[r] + m2[r - sum[n]]);
            }
            smin.proc_right();
            if (smin.get_window_size() > sum[n] + 1) smin.proc_left();
            chmin(ans, sum[n] - sum[r] + smin.query());
        }
        print(ans);
    }
    return 0;
