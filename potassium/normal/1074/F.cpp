...
S op(S l, S r) {
	...
	ans.maxcnt = (l.val == ans.val ? l.maxcnt : 0) + (r.val == ans.val ? r.maxcnt : 0);
	return ans;
}
...
int main() {
	...
	for (int i = 0; i < q; i++) {
		...
		if (post[v] <= post[u]) {
			int child = *lower_bound(g[u].begin(), g[u].end(), v, [&](int x, int y) {
		...
		S ans = st.all_prod();
		W(ans.val == (int)edges.size() ? ans.maxcnt : 0);
	}
