...
int main(){
	...
	TT{
		cauto n=read();
		vv<int> g(n);
		rep(i,n) g[i]=readV(read(),1);
		vv<int> h(n);
		rep(v,n)for(cauto &u:g[v]) h[u].eb(v);
		vector<int> degs(n);
		rep(i,n) degs[i]=g[i].size();
		set<int> st;
		rep(v,n)if(degs[v]==0) st.emplace(v);
		int re=0;
		int last=MOD;
		while(st.size()){
			if(*st.rbegin()<last){
			...
			cauto it=st.lower_bound(last);
			cauto v=*it;
			st.erase(it);
			last=v;
			for(cauto &u:h[v])if(--degs[u]==0) st.emplace(u);
		}
		if(*max_element(all(degs))){
			cout<<-1 NL;
			continue;
		}
		cout<<re NL;
