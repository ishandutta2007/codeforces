...
int main() {
	CASE(t) {
		int n; R(n);
		vector<string> s(2); R(s);
		s[0] = '@' + s[0];
		s[1] = '@' + s[1];

		vector<vector<int>> last(2, vector<int>(n + 1, -1));
		vector<int> pos(2, -1);
		for (int i = 1; i <= n; i++) {
			pos[s[0][i] - '0'] = i;
			pos[s[1][i] - '0'] = i;
		...
		for (int i = 1; i <= n; i++) {
			...
			if (last[0][i] != -1)
				chkmax(dp[i], dp[last[0][i] - 1] + 1);

			if (min(last[0][i], last[1][i]) != -1)
				chkmax(dp[i], dp[min(last[0][i], last[1][i]) - 1] + 2);
		}
