...
fun solve(inp: InputReader, out: OutputWriter, testNumber: Int) {
    var n = inp.readInt()
    var k = inp.readLong()
    var a = Array<Int>(n, { inp.readInt() - 1 })
    var b = Array<Int>(n, {0})
    for (i in (0..a.size - 1)) {
        b[a[i]] = i
    }
    var answer = power(b, k).map { it + 1 }
    out.printLine(answer.toIntArray())
}

fun power(a: Array<Int>, k: Long):Array<Int> {
    if (k == 0.toLong()) {
        return Array(a.size, { it })
    }
    if (k % 2.toLong() == 1.toLong()) {
        return multiply(power(a, k - 1.toLong()), a)
    } else {
        var res = power(a, k / 2.toLong())
        return multiply(res, res)
    }
}

fun multiply(a: Array<Int>, b: Array<Int>): Array<Int> {
    var c = Array<Int>(a.size, {a[b[it]]})
    return c
}
