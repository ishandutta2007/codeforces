...
LL n,c[MAXN+10],s[MAXN+10],s2[MAXN+10],pow2[MAXN+10],f[MAXN+10],ans,g[MAXN+10];
void read(){
	Read(n);
	int i,a;
	*pow2=1;
	for(i=1;i<=n;i++){
		Read(a);
		c[a]++,s[a]+=a,s2[a]+=1ll*a*a;
		pow2[i]=pow2[i-1]*2%MOD;
	}
	for(a=1;a<1e6;a*=10)
		for(i=MAXN-1;i>=0;i--)
			if(i/a%10)
				c[i-a]+=c[i],(s[i-a]+=s[i])%=MOD,(s2[i-a]+=s2[i])%=MOD;
	for(i=0;i<MAXN;i++){
		s[i]%=MOD,s2[i]%=MOD;
		if(!c[i])
			continue;
			...
			f[i]=(pow2[c[i]-2]*(s[i]%MOD*s[i]%MOD+s2[i])%MOD)%MOD;
	}
}
void solve(){
	int i,j,p,s,now;
	for(i=0;i<MAXN;i++){
		for(s=0;s<(1<<6);s++){
			...
			if(j<6)
				continue;
			g[i]=(g[i]+((__builtin_popcount(s)&1)?-1:1)*f[now])%MOD;
		}
		ans^=(g[i]+MOD)%MOD*i;
	}
	cout<<ans<<endl;
