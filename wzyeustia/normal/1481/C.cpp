...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ll n, m; cin >> n >> m;
        vector<int> start(n), end(n), need(n+5), can(n+5);
        vector<queue<int>> position(n+5);
        REP(i, n) {cin >> start[i];}
        REP(i, n) {
            cin >> end[i];
            if (end[i] != start[i]) {
                need[end[i]]++;
                position[end[i]].emplace(i);
            }
        }
        ...
        ll lastColor = edit[m-1], wildcard;
        bool flag1 = false;
        if (need[lastColor]) {
            flag1 = true;
            wildcard = position[lastColor].back();
        }
        else {
            REP(i, n) {
                if (end[i] == lastColor) {
        ...
        if (flag0 && flag1) {
            ...
            REP(i, m) {
                if (need[edit[i]]) {
                    ...
                    cout << position[edit[i]].front()+1 << " ";
                    position[edit[i]].pop(); need[edit[i]]--;
                }
                else {
                    cout << wildcard+1 << " ";
                }
