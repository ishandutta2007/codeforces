...
inline void AC() {
    ...
    m = read();
    read(a+2);
    for(_ i = 2; i <= n; ++i) if(a[i] == 'a' && a[i+1] == 'b' && a[i+2] == 'c') st.insert(i);
    for(; m--; ) {
        x = read()+1;
        t = getLw();
        st.erase(x-2);
        ...
        st.erase(x);
        a[x] = t;
        if(a[x-2] == 'a' && a[x-1] == 'b' && a[x] == 'c') st.insert(x-2);
        if(a[x-1] == 'a' && a[x] == 'b' && a[x+1] == 'c') st.insert(x-1);
        if(a[x] == 'a' && a[x+1] == 'b' && a[x+2] == 'c') st.insert(x);
        writeln((_)st.size());
    }
