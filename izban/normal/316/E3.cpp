...
void push(int v, int tl, int tr) {
	if (d[v]) {
		t[v].first = (t[v].first + 1LL * d[v] * fs[tr - tl]) % mod;
		t[v].second = (t[v].second + 1LL * d[v] * (fs[tr - tl + 1] - fs[0])) % mod;
		if (tl != tr) {
			d[v * 2] += d[v]; if (d[v * 2] >= mod) d[v * 2] -= mod;
			d[v * 2 + 1] += d[v]; if (d[v * 2 + 1] >= mod) d[v * 2 + 1] -= mod;
		}
		d[v] = 0;
	}
}
...
vector<int> fastsolve() {
	...
	for (int i = 0; i < m; i++) {
		...
		if (q[i] == 3) {
			add(1, 0, n - 1, x1[i] - 1, x2[i] - 1, x3[i]);
		}
	}
	return res;
}
...
void precalc() {
	...
	fs[0] = 1;
	for (int i = 1; i < maxn; i++) fs[i] = (f[i] + fs[i - 1]) % mod;
}
