...
//        FREOUT("out.txt");
    ...
    FI(n) {
        FJ(i) {
            int res = p[i].first * p[i].second + p[j].first * p[j].second;
            bool ok = false;
            ok |= p[i].first <= a && p[j].first <= a && (p[i].second + p[j].second <= b);
            ok |= p[i].first <= a && p[j].second <= a && (p[i].second + p[j].first <= b);
            ok |= p[i].second <= a && p[j].second <= a && (p[i].first + p[j].first <= b);
            ok |= p[i].second <= a && p[j].first <= a && (p[i].first + p[j].second <= b);
            swap(a, b);
            ok |= p[i].first <= a && p[j].first <= a && (p[i].second + p[j].second <= b);
            ok |= p[i].first <= a && p[j].second <= a && (p[i].second + p[j].first <= b);
            ok |= p[i].second <= a && p[j].second <= a && (p[i].first + p[j].first <= b);
