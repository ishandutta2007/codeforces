...
#line 2 "/home/maspy/compro/library/poly/poly_gcd.hpp"
...
#line 3 "/home/maspy/compro/library/mod/mod_sqrt.hpp"
...
template <typename T>
vc<T> conv(int N, vc<T> f, vc<T> g) {
  f = convolution(f, g);
  FOR(i, N, len(f)) f[i % N] += f[i];
  f.resize(N);
  return f;
}

template <typename mint>
bool trial(ll N, vi B, vi C) {
  using poly = vc<mint>;
  poly f(N);
  FOR(i, N) f[i] = B[(N - i) % N];
  poly f_inv = [&]() -> poly {
    ...
    g[0] = 1;
    g[N] = -1;
    auto [h, f_inv, g_inv] = poly_extgcd(f, g);
    if (len(h) > 1 || h[0] == mint(0)) return {};
    f_inv.resize(N);
    mint cf = h[0].inverse();
    FOR(j, N) f_inv[j] *= cf;
    return f_inv;
  }();
  if (len(f_inv) == 0) return false;
  ...
  auto check = [&](vi A, vi B, vi C) -> bool {
    if (abs(MIN(A)) > 100000) return false;
    if (abs(MAX(A)) > 100000) return false;
...
void solve() {
  ...
  //  det  0 
  if (trial<modint<998244353>>(N, B, C)) return;
  if (trial<modint<924844053>>(N, B, C)) return;
  if (trial<modint<880803841>>(N, B, C)) return;
  if (trial<modint<754974721>>(N, B, C)) return;
  if (trial<modint<469762049>>(N, B, C)) return;
  if (trial<modint<167772161>>(N, B, C)) return;
}
