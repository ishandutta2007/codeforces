...
ld ans;
ll d;
void dfs(ll t,ll a,ll b,ll c,ld nowprob=1){
  if(c*nowprob*t<eps)return;
  //c
  ans+=c*t*nowprob/40000;

  ++t;
  //a
  if(a!=0){
    if(b!=0)dfs(t,max(a-d,0LL),b+min(a,d)/2,c+min(a,d)/2,nowprob*a/40000);
    else dfs(t,max(a-d,0LL),0,c+min(a,d),nowprob*a/40000);
  }
  //b
  if(b!=0){
    if(a!=0)dfs(t,a+min(b,d)/2,max(b-d,0LL),c+min(b,d)/2,nowprob*b/40000);
    else dfs(t,0,max(b-d,0LL),c+min(b,d),nowprob*b/40000);
  }
}
int main(){
  startupcpp();
  ...
  ld A,B,C,D;
  cin>>A>>B>>C>>D;
  ll a=round(A*40000),b=round(B*40000),c=round(C*40000);
  d=round(D*40000);
  ans=0;
  dfs(1,a,b,c);
  print(ans);
