...
int c[maxn],d[maxn],m,n,st[maxn],tp;
ll ans,f[maxn];
int main(){
	n=qr(),m=qr();
	for(ri i=1;i<=n;++i)d[i]=qr(),c[i]=qr();
	build(1,1,n);
	for(ri i=1;i<=n;++i){
		modify(1,1,i,m-c[i]);
		if(i>1){
			f[i-1]=1ll*(d[i]-d[i-1])*(d[i]-d[i-1]);
			while(tp&&f[st[tp]]<=f[i-1]){
				modify(1,st[tp-1]+1,st[tp],f[st[tp]]);
				--tp;
			}
			modify(1,st[tp]+1,i-1,-f[i-1]);
			st[++tp]=i-1;
		}
		ckmax(ans,t[1].mx);
	}
