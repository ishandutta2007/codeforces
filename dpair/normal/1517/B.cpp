...
struct DPair{
    int x, y, val;
    DPair(int x = 0, int y = 0, int val = 0) : x(x), y(y), val(val) {}
    inline bool operator < (const DPair &tmp) const{return val > tmp.val;}
};
struct DSair{
    int y, val;
    DSair(int y = 0, int val = 0) : y(y), val(val) {}
    inline bool operator < (const DSair &tmp) const{return val < tmp.val;}
};
#include <queue>
#define hp std :: priority_queue
bool vis[105][105];
hp <DSair> b[105];
hp <DPair> a;
inline void work(){
    read(n, m); while(!a.empty()) a.pop();
    rep(i, 1, n) {
        while(!b[i].empty()) b[i].pop();
        rep(j, 1, m){
            vis[i][j] = 0; int x = read();
            b[i].push(DSair(j, x));
            a.push(DPair(i, j, x));
        }
    }
    rep(i, 1, m) {
        while(!a.empty() && vis[a.top().x][a.top().y]) a.pop();
        const DPair x = a.top(); a.pop();
        vis[x.x][x.y] = 1; ans[x.x][i] = x.val;
        rep(j, 1, n) {
            if(j == x.x) continue;
            while(!b[j].empty() && vis[j][b[j].top().y]) b[j].pop();
            const DSair y = b[j].top(); b[j].pop();
            vis[j][y.y] = 1; ans[j][i] = y.val;
        }
    }
    rep(i, 1, n) print(ans[i] + 1, ans[i] + m + 1);
}
