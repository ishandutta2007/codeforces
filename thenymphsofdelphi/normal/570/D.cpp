...
int n, q;
vi adj[N]; int h[N], maxh[N];
string s;
vpii que[N]; int ans[N];
vi col[N];
...
void dfs2(int u){
    ...
    if (idmxh){
        col[u].swap(col[idmxh]);
    }
    col[u].pb(1 << (s[u] - 'a'));
    Fora(v, adj[u]){
        if (v == idmxh) continue;
        For(i, 0, col[v].size()){
            col[u][i + (col[u].size() - col[v].size() - 1)] ^= col[v][i];
        }
    }
    Fora(v, que[u]){
        ans[v.se] = (0 > v.fi - h[u] || v.fi - h[u] >= col[u].size()
                    ? 0 : col[u][col[u].size() - 1 - (v.fi - h[u])]);
    }
}
