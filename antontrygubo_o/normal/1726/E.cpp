...
{
    int n; cin>>n;
    vector<int> twocycle(n+1);
    twocycle[0] = 1;
    twocycle[1] = 1;
    for (int i = 2; i<=n; i++)
    {
        twocycle[i] = add(twocycle[i-1], mul(i-1, twocycle[i-2]));
    }
    ...
    for (int fourcycles = 0; fourcycles<=n/4; fourcycles++)
    {
        int pairs = 2*fourcycles;

        int tmp = C(n-pairs, pairs);
        tmp = mul(tmp, twocycle[n - 4*fourcycles]);

        tmp = mul(tmp, mul(curfac, curdeg));

        curfac = mul(curfac, pairs + 1);
        curdeg = mul(curdeg, 2);
