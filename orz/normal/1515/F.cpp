...
void dfs(const int v, const vvi& g, const vvi& gg, vvi& h, vvi& hh, vi& p, vi& pp, vb& nv)
{
	nv[v] = false;
	for (int i = 0; i < sz(g[v]); ++i)
	{
		...
		if (nv[to])
		{
			...
			p[to] = v;
			pp[to] = gg[v][i];
			dfs(to, g, gg, h, hh, p, pp, nv);
		}
	}
}
void dfs1(const int v, const vvi& g, const vvi& gg, const vi& p, const vi& pp, vl& a, int w, vi& ans, vb& wp)
{
	for (int i = 0; i < sz(g[v]); ++i)
	{
		int to = g[v][i];
...
int another_main()
{
	...
	vi p(n);
	vi pp(n);
	dfs(0, g, gg, h, hh, p, pp, nv);
	vi ans;
	vb wp(n);
	wp[0] = true;
	dfs1(0, h, hh, p, pp, a, x, ans, wp);
	dfs1(0, h, hh, p, pp, a, x, ans, wp);
	cout << "YES\n";
