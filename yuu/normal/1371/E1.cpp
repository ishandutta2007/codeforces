...
void solve() {
    ...
    // number of way = s[1] * (s[2] - 1) * (s[3] - 2) * ... *(s[n] - n + 1)

    // as x increase, c[i] = c[i + 1] for all 1 < i <= n, c[1] += c[2]
    ...
    int zero = 1;
    int prefix = 0;
    for (int i = 1; i < n; i++) current.insert(s[i]);

    for (int i = 1; i <= n; i++) {
        (zero += p - 1) %= p;
        current.erase(current.find(s[i]));
        (prefix += c[i]) %= p;
        current.insert(s[i + n - 1]);
        if ((current.find(zero) == current.end()) && ((prefix - (i - 1) - zero) % p)) {
