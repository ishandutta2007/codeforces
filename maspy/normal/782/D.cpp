...
#line 3 "/home/maspy/compro/library/graph/scc.hpp"
...
#line 2 "/home/maspy/compro/library/graph/twosat.hpp"
struct TwoSat {
  vc<int> values;
...
void solve() {
  ...
  FOR(i, N) {
    STR(A, B);
    string X = A.substr(0, 3);
    string Y = A.substr(0, 2) + B.substr(0, 1);
    S0.eb(X), S1.eb(Y);
  }
  FOR(i, N) FOR(j, N) if (i != j) {
    if (S0[i] == S0[j]) TS.implies(~i, j);
    if (S0[i] == S1[j]) TS.implies(~i, ~j);
    if (S1[i] == S0[j]) TS.implies(i, j);
    if (S1[i] == S1[j]) TS.implies(i, ~j);
    if (S0[i] == S0[j]) TS.set(i);
  }
  bool ok = TS.calc();
  auto vals = TS.values;
  if (!ok) return NO();
  YES();
  FOR(i, N) { print(vals[i] ? S1[i] : S0[i]); }
}
