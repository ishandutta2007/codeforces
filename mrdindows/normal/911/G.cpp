...
//    std::cerr << "::::" << string(__bur, __bur + 16) << "::::" << endl;
    __ir = __bur; };};
...
uchar a[202020];
const int F = 64;
void foo(uchar *a, int n, uchar x, uchar y) {
    int n32 = n / F;
    while (n32--) {
        for (int i = 0; i < F; ++i) {
            a[i] = a[i] == x ? y : a[i];
...
int solve() {
    ...
    for (int bstart = 0; bstart < n; bstart += BLOCK_SIZE) {
        int bend = min(n, bstart + BLOCK_SIZE);
        for (int i = 0; i < q; ++i) {
            int l = max(queries[i][0], bstart);
            int r = min(queries[i][1], bend);
            if (l < r) {
                uchar x = queries[i][2];
                uchar y = queries[i][3];
                auto ap = a + l;
