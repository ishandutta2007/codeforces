...
inline matrix Plus(const matrix& a,const matrix& b,int mode)//a+b
{
...
inline void Plusto(matrix& a,const matrix& b,int mode)//a+=b
{
	if(mode==2)
	{
...
inline matrix times(const matrix& a,const matrix& b,int mode)//a*b
{
...
inline void SunSandy(int now,int nl,int nr,int l,int r,int what)//Delta version
{
	int m=(nl+nr)>>1;
	if(ic[now].m[0][0])
	{
