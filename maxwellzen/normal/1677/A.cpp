...
void solve() {
    ...
    read(p, n);
    dp.assign(n, vi(n, 0));
    F0R(i, n) FOR(j, i+2, n) if (p[i]>p[j]) dp[i][j]++;
    R0F(i, n) R0F(j, n-1) dp[i][j] += dp[i][j+1];
    R0F(i, n-1) R0F(j, n) dp[i][j] += dp[i+1][j];
    ll ans = 0;
    F0R(i, n-1) FOR(j, i+2, n-1) if (p[i]<p[j]) ans += dp[i+1][j+1] - dp[j][j+1];
    cout << ans << nl;
}
