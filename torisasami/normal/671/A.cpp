...
int main() {
    ...
    rep(i, n) {
        double x, y;
        cin >> x >> y;
        double dt = d(tx, ty, x, y), da = dt - d(ax, ay, x, y), db = dt - d(bx, by, x, y);
        va.pb(da, i), vb.pb(db, i);
        ans += dt * 2;
    }
    sort(rall(va)), sort(rall(vb));
    double val;
    if (n == 1)
        val = max(va[0].first, vb[0].first);
    else if (va[0].second != vb[0].second)
        val = max(va[0].first, vb[0].first) + max((double)0, min(va[0].first, vb[0].first));
    else
        val = max(max(va[0].first, vb[1].first) + max((double)0, min(va[0].first, vb[1].first)), max(va[1].first, vb[0].first) + max((double)0, min(va[1].first, vb[0].first)));
    cout << ans - val << endl;
}