...
void solve() {
    ...
    REP(i, m) {
        int a, b;
        scanf("%lld%lld", &a, &b);
        if (vertexs[a].size() == 1 and vertexs[a][0] == b) continue;
        vertexs[a].push_back(b);
    }
    vector<int> close_spot;
    while (true) {
        ...
        for (int i = 1; i <= n; ++i) {
            ...
            closed[i] = 0;
            if (must_close[i] == 1) {
                close_spot.push_back(i);
            }
        }
        assert(close_spot.size() * 7 <= n * 4);
        vector<int> kouho;
        for (int i = 1; i <= n; ++i) {
            if (cnt[i] == 2) {
                close_spot.push_back(i);
            ...
            for (auto& x : vertexs[i]) {
                cnt[x] = max(cnt[x], cnt[i] + 1LL);
            }
        }
        for (auto& x : kouho) {
            int swappings = 0;
            for (auto y : vertexs[x]) {
                if (closed[y] == 1) swappings++;
            }
            if (swappings == 2) continue;
            if (swappings == 0) {
                close_spot.push_back(x);
            }
            else {
                for (auto y : vertexs[x]) {
                    if (closed[y] == 0) {
                        close_spot.push_back(y);
                        closed[y] = 1;
                        break;
                    }
                }
        ...
        if ((int)close_spot.size() * 7 <= 4 * n) break;
    }
    printf("%lld\n", (int)close_spot.size());
    REP(i, close_spot.size()) {
        printf("%lld ", close_spot[i]);
    }
    printf("\n");
}
