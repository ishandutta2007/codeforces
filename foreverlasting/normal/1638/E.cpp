...
namespace MAIN {
	...
	set<P> S;
	char opt[10];
	auto split(const int &p){
		auto it=S.lower_bound({p,-1,0});
		if(it!=S.end()&&it->l==p)return it;
		it--;
		auto [l,r,c]=(*it);
		S.erase(it),S.insert({l,p-1,c});
		return S.insert({p,r,c}).fi;
	}
	LL tr[N];
	...
	inline void MAIN(const int &Case) {
		n=read(),q=read(),S.insert({1,n,1});
		while(q--){
			scanf("%s",opt+1);
			if(opt[1]=='C'){
				int l=read(),r=read(),c=read();
				auto R=split(r+1),L=split(l);
				for(auto it=L;it!=R;it++)modify(it->l,tag[it->c]),modify(it->r+1,-tag[it->c]);
				S.erase(L,R),S.insert({l,r,c});
				modify(l,-tag[c]),modify(r+1,tag[c]);
			}
			else if(opt[1]=='A'){
				int c=read(),v=read();
				tag[c]+=v;
