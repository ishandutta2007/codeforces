...
int main() {
    ...
    cin >> n >> m;
    vi u(m), v(m), w(m);
    forn(i, m) cin >> u[i] >> v[i] >> w[i], --u[i], --v[i];
    int L = 0, R = 1e9 + 1;
    while (R - L > 1) {
        int M = (L + R) / 2;
        Dinic dinic(2 * n + 2);
        int S = 2 * n, T = 2 * n + 1;
        forn(i, n) dinic.add_edge(S, i, 1), dinic.add_edge(i + n, T, 1);
        forn(i, m) if (w[i] <= M) dinic.add_edge(u[i], v[i] + n, 1);
        (dinic.max_flow(S, T) == n ? R : L) = M;
    }
    if (L == (int)1e9) R = -1;
    cout << R << '\n';
