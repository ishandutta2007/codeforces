...
void solve() {
    ...
    h -= t;
    c -= t;
    auto Judge = [h, c](int a) {
        int hoge = (h + c) * a + h;
        return Fraction(hoge, a * 2 + 1);
    };
    Fraction ans = Fraction(abs(h+c), 2);
    int itr = 2;
    int bot = 0;
    int top = 1e9;
    while (top - bot > 10) {
    ...
    for (int q = bot; q <= top; ++q) {
        int hoge = q * (h + c);
        hoge += h;
        if (Fraction(abs(hoge), q * 2 + 1) < ans) {
            ans = Fraction(abs(hoge), q * 2 + 1);
            itr = 2 * q + 1;
        }
    }
    cout << itr << endl;
