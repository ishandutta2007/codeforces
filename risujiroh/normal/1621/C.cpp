...
template <class T> enable_if_t<is_floating_point_v<T>, bool> scan(T& x) {
  if constexpr (is_same_v<T, float>) return scanf("%f", &x) != EOF;
  if constexpr (is_same_v<T, double>) return scanf("%lf", &x) != EOF;
  if constexpr (is_same_v<T, long double>) return scanf("%Lf", &x) != EOF;
}
...
template <char = 0, class T> enable_if_t<is_integral_v<T>> print(T x) {
  char buf[64];
  for_each(buf, to_chars(buf, end(buf), common_type_t<T, int>(x)).ptr, [](char c) { print(c); });
}
template <char = 0, class T> enable_if_t<is_floating_point_v<T>> print(T x) {
  if constexpr (is_same_v<T, float>) printf("%.6f", x);
  if constexpr (is_same_v<T, double>) printf("%.15f", x);
  if constexpr (is_same_v<T, long double>) printf("%.18Lf", x);
}
...
void rj::main(int) {
  const auto n = input();
  const auto query = [](int i) -> int {
    println('?', i + 1);
    return input() - 1;
  };
  auto used = vector(n, false);
  auto ans = vector(n, -1);
  for (const auto s : rep(n) | IF(!used[$1])) {
    auto v = vector<int>();
    while (true) {
      const auto i = query(s);
      v.push_back(i);
      if (used[i]) {
        break;
      }
    ...
    for (const auto i : rep(size(v) - 1)) {
      ans[v[i]] = v[i + 1];
    }
  }
  println('!', ans | mapped(lambda($1 + 1)));
}