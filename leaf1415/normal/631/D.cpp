...
void merge(vector<P> &vec)
{
	vector<P> tmp;
	for(auto p : vec){
		if(sz(tmp) == 0 || p.first != tmp.back().first) tmp.push_back(p);
		else tmp.back().second += p.second;
	}
	vec = tmp;
}

void z_algorithm(vector<P> &s, int z[])
{
	z[0] = s.size();
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> s;
		tvec.push_back(get(s));
	}
	rep(i, 1, m){
		cin >> s;
		svec.push_back(get(s));
	}
	merge(svec), merge(tvec);
	
	if(sz(svec) == 1){
		ll ans = 0;
		for(auto p : tvec){
			if(p.first == svec[0].first) ans += max(0LL, p.second - svec[0].second + 1);
		}
		outl(ans);
		return 0;
	}
	...
	vec.push_back(P(-1, -1));
	vec.insert(vec.end(), all(tvec));
	
	z_algorithm(vec, z);
	
	//outl(svec), outl(tvec);
	
	ll ans = 0, S = sz(svec), T = sz(tvec);
	reps(i, tvec){
		if(tvec[i].first != svec[0].first || tvec[i].second < svec[0].second) continue;
		if(i+S-1 >= T || tvec[i+S-1].first != svec.back().first || tvec[i+S-1].second < svec.back().second) continue;
		if(z[S-2+1+(i+1)] < sz(svec)-2) continue;
		ans++;
	}
	outl(ans);
