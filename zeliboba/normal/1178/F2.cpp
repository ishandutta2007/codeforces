...
/*  if (v[i][0] >= r || v[i].back() < l) {
  ...
  //cerr << l << ' ' << r << ' ' << i << ' ' << v[i][0] << ' ' << v[i].back() << endl;
...
//  assert(it == posl[i].size() - 1);
  assert(it < posl[i].size() && posl[i][it] + 1 == l);
//  assert(posl[i].size() - it == v[i][0] - l + 1 && posl[i].back() + 1 == v[i][0]);
  ...
  assert(it < posr[i].size() && posr[i][it] + 1 == r);
//  assert(it + 1 == r - v[i].back() && posr[i][0] == v[i].back());
  while (it >= 0) {
    //cerr << "r: " << i << ' ' << v[i].back() + 1 << " " << posr[i][it] + 1 << " " << r << endl;
...
//      cerr << i << ' ' << j << ' ' << v[j][0] << ' ' << v[j].back() << ' ' << v[i][it] << endl;
      cout << 0 << endl;
      return 0;
    }
  }
  ...
  cout << solve(0, m, c, v, posl, posr) << endl;
