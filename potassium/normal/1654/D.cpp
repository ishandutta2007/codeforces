...
int main() {
	CASE(t) {
		...
		for (int i = 0; i + 1 < n; i++) {
			int u,v,x,y; R(u,v,x,y);
			g[u].push_back({ v, { x, y }});
			g[v].push_back({ u, { y, x }});
		}

		Mint ans(0, MOD998);
		map<int, int> lcm_factor;
		{
			map<int, int> factor;
			Mint prod(1, MOD998);
			function<void(int, int, int, int)> dfs = [&](int u, int p, int x, int y) {
				prod *= x;
				prod /= y;
				ans += prod;

				for (auto& i : sieve.factorize(x)) {
					factor[i.first] -= i.second;
					chkmax(lcm_factor[i.first], factor[i.first]);
				}
				for (auto& i : sieve.factorize(y)) {
					factor[i.first] += i.second;
					chkmax(lcm_factor[i.first], factor[i.first]);
				}
				...
				for (auto& e : g[u]) {
					...
					dfs(e.first, u, e.second.second, e.second.first);
				}
		...
		for (auto& i : lcm_factor)
			ans *= Mint(i.first, MOD998).pow(max(0, i.second));
