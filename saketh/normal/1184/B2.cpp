...
int main() {
    ...
    int S, B, K, H; re(S, B, K, H);

    vi sloc(S), atk(S), fuel(S);
    F0R (s, S) re(sloc[s], atk[s], fuel[s]), --sloc[s];

    vi bloc(B), def(B);
    F0R (b, B) re(bloc[b], def[b]), --bloc[b];

    const int SRC = 0, SHIPS = 1, BASES = SHIPS + S, SNK = BASES + B;
    dinic<int> g(SNK + 1);

    F0R (s, S) {
        g.arc(SRC, SHIPS + s, 1);
        F0R (b, B)
            if (atk[s] >= def[b] && dist[sloc[s]][bloc[b]] <= fuel[s])
                g.arc(SHIPS + s, BASES + b, 1);
    }
    F0R (b, B) g.arc(BASES + b, SNK, 1);

    int mf = get<0>(g.max_flow(SRC, SNK));

    ll ans = ll(mf) * K;
    F0R (dummy, S + 1) {
        ckmin(ans, ll(dummy) * H + ll(mf - max(0, dummy - (S - mf))) * K);
    }
