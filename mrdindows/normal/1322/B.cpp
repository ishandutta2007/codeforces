...
//    }
//    FK(30) {
//        FI(n) FJ(i) rest[k] += checkbit(a[i] + a[j], k);
//    }
    sort(a, a + n);
    int ans = 0;
    for (int bit = 24; bit >= 0; --bit) {
        ll cnt = 0;
//        DBN(bit); printarray(a, n);
        for (int i = 0; i < n; ++i) {
            int x = a[i];
            int r = int(lower_bound(a + i + 1, a + n, (1 << (bit + 1)) - x) - a);
            int l = int(lower_bound(a + i + 1, a + r, (1 << bit) - x) - a);
            cnt += r - l;
            r = n;
            l = int(lower_bound(a + i + 1, a + r, (3 << bit) - x) - a);
            cnt += r - l;
        }
//        int naive = 0;
//        FI(n) FJ(i) naive += checkbit(a[i] + a[j], bit);
//        DBN(bit, checkbit(res, bit), cnt & 1, cnt, rest[bit], naive);
        if (cnt & 1) {
//            DBN(1 << bit);
            ans |= 1 << bit;
        }
        auto it = lower_bound(a, a + n, 1 << bit);
        for (int i = int(it - a); i < n; ++i) {
            a[i] -= (1 << bit);
        }
//        for (int i = 0; i < n; ++i) {
//            if (a[i] >= (1 << bit)) {
