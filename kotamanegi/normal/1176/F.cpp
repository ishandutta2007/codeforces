...
void solve() {
    ...
    REP(i, n) {
        ...
        for (int q = 3; q >= 2; --q) {
            if (go[q].size() == 0) continue;
            sort(ALL(go[q]));
            tmps.push_back(mp(q, go[q].back()));
        }
        sort(ALL(go[1]));
        reverse(ALL(go[1]));
        REP(t, min(3LL, (ll)go[1].size())) {
            tmps.push_back(mp(1, go[1][t]));
        }
        REP(q, 10) {
            dp[i + 1][q] = max(dp[i + 1][q], dp[i][q]);
            int hoge = 0;
            REP(j, tmps.size()) {
                hoge = max(hoge, tmps[j].second);
            }
            if (q + 1 == 10) hoge *= 2;
            dp[i + 1][(q + 1) % 10] = max(dp[i + 1][(q + 1) % 10], dp[i][q] + hoge);
            REP(j, tmps.size()) {
                for (int t = j + 1; t < tmps.size(); ++t) {
                    if (tmps[j].first + tmps[t].first > 3) continue;
                    int hoge = (tmps[j].second + tmps[t].second);
                    if (q + 2 >= 10) hoge += max(tmps[j].second, tmps[t].second);
                    dp[i + 1][(q + 2) % 10] = max(dp[i + 1][(q + 2) % 10], dp[i][q] + hoge);
                    for (int p = t + 1; p < tmps.size(); ++p) {
                        if (tmps[j].first + tmps[t].first + tmps[p].first > 3) continue;
                        int hoge = (tmps[j].second + tmps[t].second + tmps[p].second);
                        if (q + 3 >= 10) hoge += max({ tmps[j].second, tmps[t].second ,tmps[p].second });
                        dp[i + 1][(q + 3) % 10]  = max(dp[i + 1][(q + 3) % 10], dp[i][q] + hoge);
                    }
                }
            }
        }
