...
int main() {
    ...
    ll n, k;
    cin>>n>>k;
    if (k<n*(n+1)/2) {cout<<-1; return 0;}

    ll maxx = 0;
    for (int i = 0; i<n/2; i++) maxx+=2*(n-i);
    if (n%2==1) maxx+=(n+1)/2;
    k = min(k, maxx);
    ll k1 = k;
    vector<int> m(n);
    for (int i = 0; i<n; i++) {m[i] = n-i; k-=m[i];}
    for (int i = 0; i<n; i++)
    {
        if (k>0)
        {
    ...
    vector<int> usedA(n+1);
    vector<int> usedB(n+1);
    ...
    for (int i = 0; i<n; i++)
    {
        if (i%2==0)
        {
            while (usedB[curB]) curB--;
            b[i] = curB;
            usedB[curB] = true;
        }
        else
        {
            while (usedA[curA]) curA--;
            a[i] = curA;
            usedA[curA] = true;
        }
    }
