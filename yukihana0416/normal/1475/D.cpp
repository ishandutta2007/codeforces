...
//    for(int i = 1; i <= cc; i++) cout <<c[i] <<" "; cout <<endl;
//    for(int i = 1; i <= dd; i++) cout <<d[i] <<" "; cout <<endl;
    ...
    for(int i = 0; i <= cc; i++)
    {
        ...
        if(res != -1) ans = min(ans, i + res * 2);
//        cout <<i <<" " <<res <<" " <<c[i] <<" " <<d[res] <<endl;
    }
    for(int i = 0; i <= dd; i++)
    {
        int l = 0, r = cc, res = -1, mid;
