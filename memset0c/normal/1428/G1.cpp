...
inline int weight(int k){
  return k%3==0?k/3:0;
}
inline long long calc(int s){
  if((s+8)/9>k){
    return -inf;
  }
  int t=s/9;
...
inline vector<long long> transform(vector<long long> dp,int val){
  vector<vector<long long>> g(3,vector<long long>(dp.size(),-inf));
  for(int r=0;r<9;r++)
    for(int cas=0;cas<3;cas++){
      int u=1,v=0;
      for(int j=r,l=0;j<dp.size();j+=9,l++){
        while(u<=v&&dp[j]-3ll*l*val>dp[stk[v]*9+r]-3ll*stk[v]*val)--v;
        stk[++v]=l;
        while(u<=v&&k-cas<l-stk[u])++u;
        if(u<=v){
          g[cas][j]=3ll*l*val+dp[stk[u]*9+r]-3ll*val*stk[u];
        }
      }
    }
  for(int i=0;i<dp.size();i++){
    dp[i]=max(dp[i],g[0][i]);
    for(int j=i;j>=max<int>(0,i-9);j--){
      dp[i]=max(dp[i],g[1][j]+(long long)weight(i-j)*val);
    }
    for(int j=i;j>=max<int>(0,i-9);j--){
      dp[i]=max(dp[i],g[2][j]+(long long)(i-j)/3*val);
    }
  }
  return dp;
}
...
int32_t main(){
  ...
  for(int i=0;i<1000000;i++){
    ...
    if(t<0){
      ...
      dp[0][i]=(long long)t*f[0];
    }
  }
  int w=10;
  for(int k=1;k<6;k++,w*=10){
    for(int r=0;r<w;r++){
      vector<long long> src;
      for(int i=r;i<1000000;i+=w){
        src.push_back(dp[k-1][i]);
      }
      vector<long long> rsp=transform(src,f[k]);
      for(int i=r,j=0;i<1000000;i+=w,j++){
        dp[k][i]=rsp[j];
      }
    }
  }
