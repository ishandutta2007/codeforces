...
class longMaxLazy (nSuggest  :Int){
    ...
    fun rangeQueryMin(l:Int,r:Int):Int {
        pushPath(l+n)
        pushPath(r+n)
...
fun main(){
    solve.cases{
        ...
        val max = str.count { it == 'R' }

        val ST = longMaxLazy(max+1)
        var balance = 0
        for(i in 0 until n){
            val s = str[i]
            if(s == 'R'){
                ...
                val erase = current[cursor]
                val new = if(s == '(') 1 else if(s == ')') -1 else 0
                if(erase != new){
                    balance -= erase
                    current[cursor] = new
                    ST.rangeAdd(cursor, ST.lastIndex,new - erase)
                    balance += new
                }
...
didn't clean out the conditiosn
variable swap on indices

isn't actually fast enouhg to squeeze
observing it is slightly easier beacuse of L, R restrictions
 */