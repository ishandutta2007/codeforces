...
int main() {
    ...
    for(int i = 0, j = 0; i < n; i++) {
        while(j < n) {
            ...
            bool flag = false;
            for(auto [e, _] : f) {
                if(mp.count(e)) flag = true;
            }
            if(flag)
                break;
            else {
                for(auto [e, _] : f) mp[e] += 1;
                j++;
            }
        }
        nxt[i] = j;
