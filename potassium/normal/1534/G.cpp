...
int main() {
	...
	maxheap<ll> left;
	minheap<ll> right;
	left.emplace(0); right.emplace(0);
	ll ans = 0;
	for (auto& i : pts) {
		ll lb = left.top() - i.first;
		ll rb = right.top() + i.first;
		ans += max({ lb - i.second, i.second - rb, 0LL });
		if (i.second <= lb) {
			left.emplace(i.second + i.first);
			left.emplace(i.second + i.first);
			right.emplace(left.top() - i.first * 2);
			left.pop();
		}
		else {
			right.emplace(i.second - i.first);
			right.emplace(i.second - i.first);
			left.emplace(right.top() + i.first * 2);
			right.pop();
		}
	}
