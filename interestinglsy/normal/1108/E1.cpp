...
int Min( int l , int r ){
	int ans = INF;
	Forx(i,l,r) Mymin(ans,a[i]);
	return ans;
}

int segl[MAXM], segr[MAXM];
...
int main(){
	...
	// int maxpos = 5;
		...
		vec.clear();
		For(i,m){
			if( segl[i] <= maxpos && maxpos <= segr[i] ) continue;
			vec.Pb({segl[i],1});
			if( segr[i] == n ) continue;
			vec.Pb({segr[i]+1,-1});
		}
		sort(All(vec));
		int sum = 0, tmin = Min(1,n);
		int l = 0, r;
		while( l < vec.size() ){
			r = l;
			while( r < vec.size() && vec[r].F == vec[l].F ){
				sum += vec[r].S;
				++r;
			}

			int ul = vec[l].F;
			int ur = r==vec.size() ? n : vec[r].F-1;
			// cout << ul << " " << ur << " " << sum << endl;
			int ttmin = Min(ul,ur) - sum;
			Mymin(tmin,ttmin);

			l = r;
		}
		// cout << maxpos << " " << tmin << endl;
		int tans = a[maxpos] - tmin;
		if( ans < tans ){
			ans = tans;
			ansl.clear();
			For(i,m){
