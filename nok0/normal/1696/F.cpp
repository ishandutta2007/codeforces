...
void main_() {
	INT(n);
	atcoder::dsu uf_tmp(n);
	VV<> mat(n, V<>(n));
	VV<string> s(n, V<string>(n));
	rep(i, n - 1) {
		rep(j, i + 1, n) {
			cin >> s[i][j];
			s[j][i] = s[i][j];
			if(count(all(s[i][j]), '1')) {
				uf_tmp.merge(i, j);
				mat[i][j] = mat[j][i] = count(all(s[i][j]), '1');
			}
		}
	}
	auto gr = uf_tmp.groups();
	if(SZ(gr) != 2) {
		No();
		return;
	}
	V<> col(n);
	rep(k, 2) foa(v, gr[k]) col[v] = k;
	rep(i, n) {
		rep(j, i + 1, n) {
			if((col[i] == col[j]) ^ (mat[i][j] > 0)) {
				No();
				return;
			}
		}
	...
	rep(i, n) {
		rep(j, n) {
			if(i == j or col[i] == col[j]) continue;
			...
			while(SZ(que)) {
				...
				for(auto e : g[c]) {
					rep(l, n) {
						if(l == c or l == e.to) continue;
						if(s[c][l][e.to] == '1') {
							if(es[l][e.to]) continue;
							if(uf.same(l, e.to)) {
								ok = 0;
								goto fin;
							}
		...
		fin:;
			if(ok and edge_cnt == n - 1) {
				auto chk = [&]() -> bool {
					auto dist = g.warshall_floyd();
					rep(i, n) {
						rep(j, n - 1 - i) {
							rep(k, n) {
								if((dist[i][k] == dist[i + j + 1][k]) ^ (s[i][i + j + 1][k] == '1')) {
									return false;
								}
							}
						}
