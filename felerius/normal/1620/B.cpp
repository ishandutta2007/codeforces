// Triangles on a Rectangle (https://codeforces.com/contest/1620/problem/B)
...
int main() {
    ...
    while (t--) {
        ...
        ans = max(ans, (left.back() - left[0]) * w);
        ans = max(ans, (right.back() - right[0]) * w);
        ans = max(ans, (top.back() - top[0]) * h);
        ans = max(ans, (bot.back() - bot[0]) * h);
