...
cin>>s[x];
for(int x=1;x<=n;x++)
if(s[x]%2)q.add_edge(S,x,2);
else q.add_edge(x,T,2);
for(int x=1;x<=n;x++)
for(int y=x+1;y<=n;y++)
if(!ck[s[x]+s[y]])
if(s[x]%2)q.add_edge(x,y,1);
else q.add_edge(y,x,1);
int ans=q.Dinic(S,T);
if(ans!=n)ex("Impossible");
bitset<100001>bs;
...
if(s[x]%2&&!bs[x])
{
	...
	while(1)
	{
		bs[qqqq]=1;
		now.push_back(qqqq);
		for(auto qwq:q.Edge[qqqq])
        {
            if(q.Stream[qwq.second|1]!=0&&qwq.first!=T&&qwq.first!=S&&!bs[qwq.first])
            {
                qqqq=qwq.first;
                goto nxt;
            }
	...
	anssss.push_back(now);
}
cout<<anssss.size()<<endl;
for(auto w:anssss)
{
    cout<<w.size()<<' ';
	for(auto e:w)
