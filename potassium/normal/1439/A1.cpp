...
int main() {
	CASE(t) {
		...
		auto go = [&](int r, int c, int dx1, int dy1, int dx2, int dy2) {
			a[r][c] ^= 1;
			a[r + dx1][c + dy1] ^= 1;
			a[r + dx2][c + dy2] ^= 1;
			++r; ++c;
			ans.pb({{r, c}, {r + dx1, c + dy1}, {r + dx2, c + dy2}});
		};
		...
		auto go3 = [&](pii x, pii y, pii z) {
			a[x.F][x.S] ^= 1;
			a[y.F][y.S] ^= 1;
			a[z.F][z.S] ^= 1;
			ans.pb({{x.F + 1, x.S + 1}, {y.F + 1, y.S + 1}, {z.F + 1, z.S + 1}});
		};
