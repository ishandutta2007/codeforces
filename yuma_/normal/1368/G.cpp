...
int main() {
    ...
    Graph g(H*W);
    vector<Graph>gs(2,Graph(H*W/2));
    vector<vector<int>>comes(2,vector<int>(H*W/2));

    vector<vector<int>>ids(H,vector<int>(W,-1));
    {
        int nid=0;
        for(int y=0;y<H;++y){
            for(int x=0;x<W;++x){
                if(ids[y][x]==-1){
                    int way=mp[field[y][x]];
                    ids[y][x]=nid;
                    ids[y+dy[way]][x+dx[way]]=nid;
                    nid++;
                }
            }
        }
