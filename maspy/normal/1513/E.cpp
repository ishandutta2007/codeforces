...
void solve() {
  ...
  for (auto&& x: A) CNT[x]++;
  mint cf = 1;
  for (auto&& [k, v]: CNT) cf *= fact_inv<mint>(v);
  cf *= fact<mint>(sm) * fact<mint>(eq) * fact<mint>(gr);

  if (eq == N) return print(1);
  if (sm > 1 && gr > 1) {
    // 
    mint ans = cf * mint(2) * C<mint>(N, eq);
    print(ans);
  } else {
    mint ans = cf * fact<mint>(N) * fact_inv<mint>(sm) * fact_inv<mint>(eq)
               * fact_inv<mint>(gr);
    print(ans);
  }
}
