...
void solve(){
  li n; cin>>n;
  vi L(n), R(n);
  for(li i = 0; i < (li)(n); ++i) cin>>L[i]>>R[i];
  vvi G(n);
  ...
  auto dfs = letrec([&](auto dfs, li u, li p) -> pii {
    li x=0,y=0;
    for(li v : G[u]) if(v != p) {
        li a,b; tie(a,b) = dfs(v, u);
        x += max(a + abs(L[u]-L[v]), b + abs(L[u]-R[v]));
        y += max(a + abs(R[u]-L[v]), b + abs(R[u]-R[v]));
      }
    return pii(x,y);
  });
  li a,b; tie(a,b) = dfs(0,-1);
  cout << max(a,b) << '\n';
}
