...
int main() {
	int n,m,MOD; R(n,m,MOD);
	vector<string> a(m); R(a);
	...
	enum CC_TYPE { CHAIN, CYCLE };
	function<CC_TYPE(int, int)> dfs = [&](int u, int p) {
		if (trav[u]) {
			return CYCLE;
		}
	...
	vector<vector<Mint>> dp(n + 7, vector<Mint>(n + 7, Mint(0, MOD)));
	vector<vector<bool>> is(n + 7, vector<bool>(n + 7, false));
	dp[0][0] = Mint(1, MOD);
	is[0][0] = 1;
	
	function<Mint(int, int)> solve = [&](int chain, int lone) {
		if (is[chain][lone])
			return dp[chain][lone];
			
		is[chain][lone] = 1;
		if (chain >= 2)
			dp[chain][lone] += (chain * (chain - 1) / 2 * 4) * solve(chain - 1, lone);
			
		if (chain >= 1 && lone >= 1)
			dp[chain][lone] += chain * lone * 2 * solve(chain, lone - 1);
			
		if (chain >= 1)
			dp[chain][lone] += chain * solve(chain - 1, lone);
			
		if (lone >= 2)
			dp[chain][lone] += (lone * (lone - 1) / 2) * solve(chain + 1, lone - 2);
