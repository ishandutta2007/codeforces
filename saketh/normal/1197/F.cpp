...
int mex(const vi& vals) {
    int res = 0;
    while (find(all(vals), res) != vals.end())
        res++;
    return res;
}
...
int main() {
    ...
    F0R (i, M) {
        int x, y, c;
        re(x, y, c);
        fixed[x-1].eb(y-1, c-1);
    }

    array<array<int, 3>, 3> allow;
    re(allow);
            ...
            F0R (sg2, 4) {
                F0R (col, 3) {
                    int sg3 = mex({
                            ...
                            allow[col][1] ? sg1 : -1,
                            allow[col][2] ? sg0 : -1 });
                    uncolored[sg1 + (sg2 << 2) + (sg3 << 4)][sg0 + (sg1 << 2) + (sg2 << 4)] += 1;
                }
            }
        }
    }

    const int MAXB = 32 - __builtin_clz(1e9);
    vector<mat> ppow(1, uncolored);
    while (ppow.size() < MAXB) ppow.pb(ppow.back() * ppow.back());
    auto jump = [&](vmn& w, int j) {
        F0R (k, 32 - __builtin_clz(j)) if (j & (1 << k)) w = ppow[k] * w;
    };

    array<mn, 4> dp{ 1, 0, 0, 0 };
    F0R (i, N) {
        auto& mf = fixed[i]; sort(all(mf));

        int csz = 0;
        vmn ways(64); ways[3 + (3 << 2) + (3 << 4)] = 1;

        for (int j = 0; j < sz(mf); j++) {
            if (mf[j].f > csz) {
                jump(ways, mf[j].f - csz);
                csz = mf[j].f;
            }
            ...
            F0R (sg0, 4) {
                F0R (sg1, 4) {
                    F0R (sg2, 4) {
                        const int nc = mf[j].s;
                        int sg3 = mex({
                                csz >= 1 && allow[nc][0] ? sg2 : -1,
                                csz >= 2 && allow[nc][1] ? sg1 : -1,
                                csz >= 3 && allow[nc][2] ? sg0 : -1 });
                        WAYS[sg1 + (sg2 << 2) + (sg3 << 4)] += ways[sg0 + (sg1 << 2) + (sg2 << 4)];
                    }
                }
            }
            swap(ways, WAYS);
        ...
        if (len[i] > csz) jump(ways, len[i] - csz);

        array<mn, 4> res{0,0,0,0};
        F0R (sg0, 4)
            F0R (sg1, 4)
                F0R (sg2, 4)
                    res[sg2] += ways[sg0 + (sg1 << 2) + (sg2 << 4)];

        decltype(dp) DP{0,0,0,0};
        F0R (p, 4)
            F0R (k, 4)
                DP[p^k] += dp[p] * res[k];
        dp = move(DP);
