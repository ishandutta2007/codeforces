...
public:
    void solve() {
        ...
        arr<ll> profit(n);
        auto val = [&](ll a, ll s) -> ll {
            ll sm = a / s;
            ll bi = sm + 1;
            ll qb = a % s;
            ll qs = s - qb;
            return qb * bi * bi + qs * sm * sm;
        };
        arr<ll> q(n, 1);
        for (int i : range(n)) {
            profit[i] = val(a[i], 1) - val(a[i], 2);
        }
        auto cmp = [&](int x, int y) -> bool {
            return profit[x] < profit[y];
        };
        priority_queue<int, vi, decltype(cmp)> qu(cmp);
        for (int i : range(n)) {
            qu.push(i);
        }
        for (int i : range(k - n)) {
            ...
            qu.pop();
            q[cur]++;
            profit[cur] = val(a[cur], q[cur]) - val(a[cur], q[cur] + 1);
            qu.push(cur);
        }
        ll ans = 0;
        for (int i : range(n)) {
