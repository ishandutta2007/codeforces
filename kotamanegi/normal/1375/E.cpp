...
void solve(){
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        for (int q = i - 1; q >= 0; --q) {
            if (inputs[q] > inputs[i]) {
                nexts.push_back(mp(inputs[q], q));
            }
        }
        sort(ALL(nexts));
        REP(q, nexts.size()) {
            ans.push_back(mp(nexts[q].second, i));
            int hoge = inputs[nexts[q].second];
            swap(inputs[i], inputs[nexts[q].second]);
                for (int j = nexts[q].second + 1; j < i; ++j) {
                    if (hoge > inputs[j] and inputs[nexts[q].second] == inputs[j]) {
                        ans.push_back(mp(nexts[q].second, j));
                        swap(inputs[j], inputs[nexts[q].second]);
                    }
                }
