...
{
	//[0,2*pi]
	static void fip(int d,int a, int b, int r)
	{
		double a1=a*a+b*b;
		double a2=d*d;
		double a3=r*r;
		double angle=Math.acos((a1+a2-a3)/(2*Math.sqrt(a1)*d));
		double st=angle(a,b)-angle;
		double en=st+2*angle;
		if(st<0)
		{
			star++;
			st=2*Math.PI+st;
	...
	//[0,2*pi]
	static int star;
	static PriorityQueue<Double> start=new PriorityQueue<Double>();
	static PriorityQueue<Double> end=new PriorityQueue<Double>();
	public static void main(String[]args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
		{
			...
			int dmin=(int)Math.ceil((Math.sqrt(x1*x1+y1*y1)-r)/d);
			int dmax=(int)((Math.sqrt(x1*x1+y1*y1)+r)/d);
