...
struct triple{
    ...
    friend istream &operator>>(istream &is, triple &tri){
        is >> tri.a >> tri.b >> tri.c;
        return is;
    }
    friend ostream &operator<<(ostream &os, const triple &tri){
        os << tri.a << " " << tri.b << " " << tri.c;
        return os;
    }
};

int count(triple q, triple t){
    return abs(q.a - t.a) + abs(q.b - t.b) + abs(q.c - t.c);
}

void solve() {
    autoread<triple> query;
    triple best(1e9, 0, 0);
    for (int i = 1; i <= 20000; i += 1){
        for (int j = i; j <= 20000; j += i){
            for (int k = j; k <= 20000; k += j){
                if (count(query, {i, j, k}) < count(query, best)) best = {i, j, k};
            }
        }
    }
    answer(count(query, best), best);
}