#include<bits/stdc++.h>
using namespace std;
namespace IO
{
	const int buffer_size=1e5+5;
	char buf[buffer_size],*S,*T;
	inline char read_char()
	{
		if(S==T)
			T=(S=buf)+fread(buf,1,buffer_size,stdin);
		return S!=T?*(S++):EOF;
	}
	bool flag_EOF;
	inline int read_int()
	{
		int flag=1;
		char c=read_char();
		while(c<'0'||c>'9')
		{
			if(c==EOF)
			{
				flag_EOF=true;
				return 0;
			}
			flag=(c=='-'?-1:1);
			c=read_char();
		}
		int x=0;
		while(c>='0'&&c<='9')
		{
			x=x*10+(c^48);
			c=read_char();
		}
		return flag*x;
	}
	char st[16];
	int _top;
	inline void Write(int x)
	{
		if(x==0)
		{
			putchar('0');
			return;
		}
		if(x<0)
		{
			putchar('-');
			x=-x;
		}
		while(x)
		{
			st[++_top]=x%10+'0';
			x/=10;
		}
		while(_top>0)
			putchar(st[_top--]);
	}
}
int n;
const int max_n=1e6+5;
int End[max_n<<1],Last[max_n],Next[max_n<<1],e;
inline void add_edge(int x,int y)
{
	End[++e]=y;
	Next[e]=Last[x];
	Last[x]=e;
	End[++e]=x;
	Next[e]=Last[y];
	Last[y]=e;
}
namespace segment_tree
{
	const int max_size=21e6+5;
	int ls[max_size],rs[max_size],val[max_size],cnt_node;
	void modify(int &p,int l,int r,int k,int v)
	{
		if(!p)
			p=++cnt_node;
		if(l==r)
		{
			val[p]+=v;
			return;
		}
		int mid=(l+r)>>1;
		if(k<=mid)
			modify(ls[p],l,mid,k,v);
		else
			modify(rs[p],mid+1,r,k,v);
		val[p]=max(val[ls[p]],val[rs[p]]);
	}
	void merge(int &p,int &q,int l,int r)
	{
		if(!p||!q)
		{
			p|=q;
			return;
		}
		if(l==r)
		{
			val[p]+=val[q];
			return;
		}
		int mid=(l+r)>>1;
		merge(ls[p],ls[q],l,mid);
		merge(rs[p],rs[q],mid+1,r);
		val[p]=max(val[ls[p]],val[rs[p]]);
	}
	int res;
	void query(int p,int l,int r)
	{
		if(l==r)
		{
			res=l;
			return;
		}
		int mid=(l+r)>>1;
		if(val[ls[p]]>=val[rs[p]])
			query(ls[p],l,mid);
		else
			query(rs[p],mid+1,r);
	}
}
int root[max_n],dep[max_n],ans[max_n];
void dfs(int x,int fa)
{
	dep[x]=dep[fa]+1;
//	fprintf(stderr,"x=%d dep[x]=%d\n",x,dep[x]);
	segment_tree::modify(root[x],1,n,dep[x],1);
	for(int i=Last[x];i;i=Next[i])
	{
		int y=End[i];
		if(y!=fa)
		{
			dfs(y,x);
			segment_tree::merge(root[x],root[y],1,n);
		}
	}
	segment_tree::query(root[x],1,n);
	ans[x]=segment_tree::res-dep[x];
}
int main()
{
//	freopen("genealogy.in","r",stdin);
//	freopen("genealogy.out","w",stdout);
	n=IO::read_int();
	for(int i=1;i<n;++i)
	{
		int x=IO::read_int(),y=IO::read_int();
		add_edge(x,y);
	}
	dfs(1,0);
	for(int i=1;i<=n;++i)
	{
		IO::Write(ans[i]);
		putchar('\n');
	}
	return 0;
}