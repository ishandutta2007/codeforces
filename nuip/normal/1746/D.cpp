...
int main(){
	...
	TT{
		...
		cauto t=read(0);
		vv<int> g(n);
		reps(v,1,n) g[read(1)].eb(v);
		cauto vs=readV(n);
		map<pii,ll> memo;
		cauto dfs=
			[&](auto &&dfs, int v, int t)->ll{
				if(cauto it=memo.find(pii(v,t)); it!=memo.end()) return it->Y;
				ll re=vs[v]*t;
				cauto m=(int)g[v].size();
				vector<ll> ads; ads.reserve(m);
				for(cauto &u:g[v]){
					out(v,u,m,1);
					cauto low=dfs(dfs,u,t/m);
					cauto high=dfs(dfs,u,(t+m-1)/m);
					re+=low;
					ads.eb(high-low);
				}
				if(m && t%m){
