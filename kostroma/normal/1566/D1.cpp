...
void solve(__attribute__((unused)) bool read) {
  ...
  for (int row = 0; row < n; ++row) {
    ...
    for (int i = row * m; i < row * m + m; ++i) {
      cur_row.push_back(a[i]);
    }
    for (int i = 0; i < cur_row.size(); ) {
      int j = i;
      while (j < cur_row.size() && cur_row[j].first == cur_row[i].first) {
        ++j;
      }
      reverse(cur_row.begin() + i, cur_row.begin() + j);
      i = j;
    }
    vector<pair<int, int>> events;
    for (int i = 0; i < cur_row.size(); ++i) {
      events.emplace_back(cur_row[i].second, i);
    }
    sort(all(events));
