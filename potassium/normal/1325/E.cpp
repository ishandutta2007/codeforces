...
primeSieve<int> sieve(1000007);

vector<vector<int>> g;

int order_of_prime(int x) {
	if (x == 1) return 0;
	return lower_bound(all(sieve.primes), x) - sieve.primes.begin() + 1;
}
...
void upd(int a) {
	...
	auto fact = sieve.factorize(a);
	vector<int> v;
	for (auto& i : fact)
		if (i.S & 1)
			v.pb(order_of_prime(i.F));
...
void bfs(int node) {
	...
	while (!q.empty()) {
		for (auto& i : g[q.front()]) {
			if (dist[i] != INF) {
				if (dist[q.front()] <= dist[i])
					chkmin(ans, dist[q.front()] + dist[i] + 1);
			}
			else {
...
int main() {
	...
	for (int i = 0; clock() <= 2900 && i < sieve.primes.size(); i++)
		bfs(i);
