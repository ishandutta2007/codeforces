...
int n, m, nxt[MAXN], lz[MAXN], cnt[MAXN], block;
...
void process(int i){
    if (nxt[i] > n || !same(i, nxt[i])) 
        lz[i] = i, cnt[i] = 0;
    else
        lz[i] = lz[nxt[i]], cnt[i] = cnt[nxt[i]] + 1;
}
...
void Solve(){
    ...
    for (int i = 1, t, p, b; i <= m; i++){
        ...
        if (t){
            ...
            cin >> b;
            nxt[p] = p + b;
            for (int j = p; j && same(j, p); j--)
                process(j);
            
        }
        // cout << "Upd" << endl;
        // for (int i = 1; i <= n; i++) cout << nxt[i] << ' '; cout << endl;
        // for (int i = 1; i <= n; i++) cout <<  lz[i] << ' '; cout << endl;
