...
int main() {
    ...
    CommutativeDualSegmentTree<S, S, mapping, composition, id> seg(vector<S>(n, { 0, 0 }));
    ...
    rrep(i, n) {
        ...
        int l = i >= k - 1 ? i - (k - 1) : 0;
        long long add_val = i >= k - 1 ? k : i + 1;

        if (v >= b[i]) continue;
        long long t = cld(b[i] - v, add_val);
        seg.apply(l, l + k, { t, -t * (l - 1) });
