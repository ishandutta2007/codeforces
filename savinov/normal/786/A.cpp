...
struct st {
    ...
    st move(int x) {
        return st{player ^ 1, (pos + x) % n};
    }
};

void dfs(st v) {
    used[v.player][v.pos] = 1;
    for (int delta : move[v.player ^ 1]) {
        st to = v.move(delta);
        if (used[to.player][to.pos])
            continue;

        if (lose[v.player][v.pos]) {
            win[to.player][to.pos] = 1;
        } else if (--deg[to.player][to.pos] == 0) {
            lose[to.player][to.pos] = 1;
        } else {
            continue;
        }
        dfs(to);
