...
void solve(){
	...
	using _T = P2;
	_T e = mp(mp(0, 0), mp(0, 0));
	//need initialize
	vc<_T>dat(n, e);
	rep(i, n) dat[i].a.a = num[i];
	//f & f2 ... maybe need capture
	...
	auto f = [](_T a, _T b) -> _T{
		int mx = 0;
		int all = a.a.b + b.a.b;
		int full = a.b.a + b.b.a;
		int pmax = max(a.b.b, b.b.b);
		return mp(mp(mx, all), mp(full, pmax));
	};
	//merge top node & subtree
	auto f2 = [&](_T a, _T b) -> _T{
		//assume top node is ok
		int mx = 1 + b.b.a + b.b.b;
		int all = 1 + b.a.b;
		int full = (mx==all?all:0);
		int pmax = (mx!=all?mx:0);
		if(a.a.a < TARGET){
			mx = full = pmax = 0;
		}
		return mp(mp(mx, all), mp(full, pmax));
	...
	while(ub-lb > 1){
		...
		tree_dp<decltype(f), decltype(f2), decltype(e)> fuyuko(adj, dat, f, f2, e);
		rep(i, n){
			if(fuyuko.boss[i].a.a >= cnt){
				lb = mid;
				goto nxt;
			}
			//cerr<<mid<<" "<<TARGET<<" "<<fuyuko.boss[i]<<endl;
		}
		ub = mid;
		nxt:;
	}
