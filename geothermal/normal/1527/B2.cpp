...
void solve() {
    int N; cin >> N;
    string S; cin >> S;
    int mid = 1; if (N % 2) mid = S[N/2] - '0';
    int c1 = 0, c2 = 0;
    F0R(i, N/2) {
        int cnt = 0; if (S[i] == '0') cnt++;
        if (S[N-i-1] == '0') cnt++;
        if (cnt == 2) c2++;
        if (cnt == 1) c1++;
    }
    int res = dp[c2][c1][mid][0];
    if (res > 0) {
        cout << "ALICE" << nl;
    } else if (res < 0) {
        cout << "BOB" << nl;
...
int main() {
    setIO();
    F0R(i, MX) F0R(j, MX) F0R(k, 2) F0R(x, 2) dp[i][j][k][x] = -1e9;
    dp[0][0][1][1] = 1;
    F0R(i, MX) {
        F0R(j, MX) {
            F0Rd(k, 2) {
                F0Rd(x, 2) {
                    ...
                    if (i > 0 && j+1 < MX) {
                        found = true;
                        ckmax(dp[i][j][k][x], -1-dp[i-1][j+1][k][0]);
                    }
                    if (j > 0) {
                        found = true;
                        ckmax(dp[i][j][k][x], -1-dp[i][j-1][k][0]);
                    }
                    if (k == 0) {
                        found = true;
                        ckmax(dp[i][j][k][x], -1-dp[i][j][1][0]);
                    }
                    if (x == 0 && j != 0) {
                        found = true;
                        ckmax(dp[i][j][k][x], -dp[i][j][k][1]);
                    }
                    if (!found) dp[i][j][k][x] = 0;
                }
            }
        }
    }
