...
int solve() {
    ...
    sort(ids, ids + n, [](int x, int y){
        return p[x].first - p[x].second < p[y].first - p[y].second;
    });
    FI(n) {
        where[ids[i]] = i;
    }
    ...
    FI(n) {
        int id = ids[i];
        ans[id] += sumX + p[id].second * (ll) i;
        sumX += p[id].first;
    }
    ll sumY = 0;
    for (int i = n - 1; i >= 0; --i) {
        int id = ids[i];
        ans[id] += sumY + p[id].first * (ll) (n - 1 - i);
        sumY += p[id].second;
    }
    FI(m) {
        int x, y;
        cin >> x >> y;
        --x; --y;
        ll score = min(p[x].first + p[y].second, p[y].first + p[x].second);
        ans[x] -= score;
        ans[y] -= score;
    }
    FI(n) {
