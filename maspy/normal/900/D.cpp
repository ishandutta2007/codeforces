...
template <typename T>
pair<vc<ll>, vc<T>> mulfunc_on_divisors(ll N, function<T(ll, ll)> f) {
  auto pf = factor(N);
  vi divs = {1};
  vc<T> vals = {1};
  for (auto&& [p, e]: pf) {
    ...
    FOR3(k, 1, e + 1) {
      pp *= p;
      T x = f(p, k);
      FOR(i, n) divs.eb(divs[i] * pp);
      FOR(i, n) vals.eb(vals[i] * x);
    }
  }
  return {divs, vals};
}

pair<vc<ll>, vc<ll>> mobius_on_divisors(ll N) {
  auto f = [&](ll p, ll e) -> ll {
    if (e == 0) return 1;
    if (e == 1) return -1;
    return 0;
  };
  return mulfunc_on_divisors<ll>(N, f);
}

void solve() {
  LL(N, S);
  if (S % N) return print(0);
  S /= N;
  auto [div, mu] = mobius_on_divisors(S);
  mint ANS = 0;
  FOR(i, len(div)) {
    ll d = div[i];
    if (S % d != 0) continue;
    ANS += mint(mu[i]) * mint(2).pow(S / d - 1);
  }
