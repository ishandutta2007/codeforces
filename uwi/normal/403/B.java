...
public class B {
	...
	void solve()
	{
		...
		long sum = 0;
		for(int i = 0;i < n;i++){
			int[] fs = factorToSeq(a[i], primes);
			int f = fs.length;
			for(int v : fs){
				if(Arrays.binarySearch(bads, v) >= 0)f -= 2;
			}
			sum += f;
		}
		
		int[] gs = new int[n];
		int[] lffs = new int[n+1];
		for(int i = 0;i < n;i++){
			if(i == 0){
				gs[i] = a[i];
			}else{
			...
			int[] lf = factorToSeq(gs[i], primes);
			int lff = lf.length;
			for(int v : lf){
				if(Arrays.binarySearch(bads, v) >= 0)lff -= 2;
			}
			lffs[i] = lff;
		}
		int cur = 0;
