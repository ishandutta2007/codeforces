...
// 	FI(400)
// 		FJ(100){
// 		dmin[i][j] = dmax[i][j] = { -INF, -INF };
// 	}
// 	for (int i = 0; i < n; ++i){
// 		cin >> a[i];
// 		s[i + 1] = s[i] + a[i];
// 		dmin[i][1] = dmax[i][1] = { 0, s[i + 1] };
// 	}
// 	for (int st = 2; st < k; ++st){
// 		for (int i = 1; i < n; ++i){
// 			pii pmin = dmin[i - 1][st - 1];
// 			pii pmax = dmax[i - 1][st - 1];
// 			int sum = 0;
// 			//DBN(i, st, pmin, pmax);
// 			for (int j = i; j < n; ++j){
// 				sum += a[j];
// 				pii& rmin = dmin[j][st];
// 				pii& rmax = dmax[j][st];
// 				int res = pmin.first + max(sum - pmin.second, pmax.second - sum);
// 				if (rmin.first - rmin.second < res - sum){
// 					rmin = { res, sum };
// 				}
// 				if (rmax.first + rmax.second < res + sum){
// 					rmax = { res, sum };
// 				}
// // 				if (rmin.first < res || rmin.first == res && rmin.second > sum){
// // 					rmin = { res, sum };
// // 				}
// // 				if (rmax.first < res || rmax.first == res && rmax.second < sum){
// // 					rmax = { res, sum };
// // 				}
// 			}
// 		}
...
// 		for (int j = i; j < n; ++j){
// 			sum = s[j + 1] - s[i];
// 			ans = max(ans, dmin[i - 1][k - 1].first + sum - dmin[i - 1][k - 1].second, dmax[i - 1][k - 1].first - sum + dmax[i - 1][k - 1].second);
// 		}
// 	}
// 	cout << ans << endl;
// 	return 0;
