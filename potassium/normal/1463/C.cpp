...
int main() {
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (time < stopT) {
				ll bpos = pos + dir * (a[i].F - stT);

				ll fpos = (i + 1 == n ? dest : pos + dir * (min(a[i + 1].F, stopT) - stT));

				if (abs(tar - bpos) + abs(tar - fpos) == abs(bpos - fpos))
					ans++;
			} else {
				pos = dest;
				dest = tar;
				stT = time;
				stopT = time + abs(pos - dest);
				dir = (pos == dest ? 0 : (dest - pos) / abs(pos - dest));

				if (i + 1 == n || a[i + 1].F >= stopT)
					ans++;
			}
		}
