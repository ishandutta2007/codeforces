...
cin>>n>>k;
for(int i=0;i<n;i++){cin>>x;myset.insert(x);}
while(k--)
{
    if(!myset.empty())if(*myset.begin()==0)myset.erase(myset.begin());
    if(myset.empty())cout<<0<<endl;
    else
    {
        auto it=myset.begin();


        cout<<(*myset.begin())-ans<<endl;
        ans+=(*myset.begin())-ans;
        while(ans>=*it){it++;if(it==myset.end())break;}
