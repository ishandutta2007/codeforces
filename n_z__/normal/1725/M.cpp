...
#define pos second
int n;
vector<int>e[1000001],f[1000001];
vector<int>c[1000001],fc[1000001];
bool vis[1000001][2];
int d[1000001][2];
int qq[1000001];
void dij(int S)
{
    ...
    while(!q.empty())
    {
        ...
        tm=q.top().pos.second;
        q.pop();
        if(vis[now][tm])continue;
        vis[now][tm]=true;
        for(int i=0;i<e[now].size();i++)
        {
            int nxt=e[now][i];
            if(d[now][0]+c[now][i]<d[nxt][0])
            {
                d[nxt][0]=d[now][0]+c[now][i];
                if(!vis[nxt][0])q.push({d[nxt][0],{nxt,0}});
            }
        }
        for(int i=0;i<f[now].size();i++)
        {
            int nxt=f[now][i];
            if(min(d[now][1],d[now][0])+fc[now][i]<d[nxt][1])
            {
                d[nxt][1]=min(d[now][1],d[now][0])+fc[now][i];
                if(!vis[nxt][1])q.push({d[nxt][1],{nxt,1}});
            }
        }
    }
}
...
//also,if want to hack them,https://codeforces.com/blog/entry/62393 is a good choise.
	...
	dij(s);
	for(int t=2;t<=n;t++)
	cout<<(min(d[t][0],d[t][1])>1e15?-1:min(d[t][0],d[t][1]))<<' ';
//444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444444//
/*Main-end----------------------------------------------------------------------------------------*/
}
/*Main-end----------------------------------------------------------------------------------------*/
