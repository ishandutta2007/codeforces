...
int main()
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < m; j++)
		{
			if (a[i][j] && a[i + 1][j] && a[i][j + 1] && a[i + 1][j + 1]) pat.PB(MP(min(n - 1, i + 1), min(m - 1, j + 1))), nerk(min(n - 2, i), min(m - 2, j));// cout << i << " " << j << endl;;
