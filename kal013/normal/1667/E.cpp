...
void solve(){
	...
	for(int i = limit + 1; i <= n; ++i){
		f[i] = pref * fact[i - 1] * inv_fact[i];
		pref += f[i] - f[i - limit];
	}


	vector<mint> G(limit + 1);

	for(int i = 0; i < limit; ++i){
		G[i] = f[n - 2 - i] * fact[i];
	}
	const int offset = limit;
	vector<mint> decr(offset + 1);
	for(int i = 0; i <= offset; ++i){
		decr[i] = inv_fact[offset - i];
	}

	// trace(f[n - 1 - limit] * fact[n - 1 - limit], G.back());

	vector<mint> g2 = G * decr;
