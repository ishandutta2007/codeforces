...
struct DSU{
    ...
    DSU(int n){
        ...
        sz.resize(n+7,1);
        mn.resize(n+7);
        for (int i=1;i<=n;++i) p[i]=i,mn[i]=i;
    }
    int find(int u){
        if (p[u]==u) return u;
...
int a[maxn],n;
vector<pii> d[5000007];
vi c[2500007];
signed main(){
    ios::sync_with_stdio(false);
    cin.tie(0);
    cin>>n;
    rep1(i,n) cin>>a[i],c[a[i]].pb(i);
    vi ans;
    ans.clear();
    rep1(i,3e6){
        if (sz(c[i])>1) ans.pb(i);
    }
    if (sz(ans)>=2){
        cout<<"YES\n";
        cout<<c[ans[0]][0]<<" "<<c[ans[1]][0]<<" "<<c[ans[0]][1]<<" "<<c[ans[1]][1];
        return 0;
    }
    if (sz(ans)&&sz(c[ans[0]])>=4){
        cout<<"YES\n";
        rep(j,4) cout<<c[ans[0]][j]<<" ";
        return 0;
    }
    vi tp;
    tp.clear();
    rep1(i,3e6){
        if (sz(c[i])) tp.pb(c[i][0]);
    }
    for (int i=0;i<sz(tp);++i){
        for (int j=0;j<i;++j){
            int key=a[tp[i]]+a[tp[j]];
            if (sz(d[key])){
                cout<<"YES\n";
                cout<<d[key][0].F<<" "<<d[key][0].S<<" "<<tp[i]<<" "<<tp[j]<<endl;
                return 0;
            }
            d[key].pb({tp[i],tp[j]});
        }
    }
    if (sz(ans)&&sz(d[2*ans[0]])){
        cout<<"YES\n";
        cout<<c[ans[0]][0]<<" "<<c[ans[0]][1]<<" "<<d[2*ans[0]][0].F<<" "<<d[2*ans[0]][0].S<<endl;
        return 0;
    }
    cout<<"NO";
}

//Joseph Needman's Grand Problem.
//
//1. ();
//2. ().
//