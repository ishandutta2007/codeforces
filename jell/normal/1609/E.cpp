#line 1 "codeforces-workspace\\contest\\1609\\e\\e2.cpp"
...
#line 10 "codeforces-workspace\\contest\\1609\\e\\e2.cpp"
// #include "lib/cxx20"
...
#line 13 "codeforces-workspace\\contest\\1609\\e\\e2.cpp"
...
#line 12 "Library\\src\\algebra\\extended.hpp"
...
template <class _Tp> class extended {
 ...
 public:
  ...
  template <class _U> constexpr operator _U const &() const noexcept {
    static_assert(std::is_same<_U, bool>::value or
                  std::is_convertible<_Tp, _U>::value);
    if _CXX17_CONSTEXPR (std::is_same<_U, bool>::value) return sign;
    return value;
  }
...
#line 30 "codeforces-workspace\\contest\\1609\\e\\e2.cpp"

namespace workspace {

constexpr auto sigma = 3;
using ext = extended<int>;

void main() {
  // start here!

  struct mono : array<array<ext, sigma>, sigma> {
    mono() {
      for (auto i : range(sigma)) {
        for (auto j : range(i)) {
          (*this)[i][j] = ext::infinity;
        }
      }
    }
    ...
    mono operator+(mono x) const {
      mono ret;
      for (auto i : range(sigma)) {
        for (auto j : range(i, sigma)) {
          ret[i][j] = ext::infinity;
        }
      }

      for (auto i : range(sigma)) {
        for (auto j : range(i, sigma)) {
          for (auto k : range(j, sigma)) {
            chle(ret[i][k], (*this)[i][j] + x[j][k]);
          }
        }
      }
