...
int main() {
    TEST {
        INT(n);
        VEC(int, a, n);
        atcoder::segtree<int, MAX, INF> dp(n + 1), seg(a);
        vv(int, query, n);
        rep(i, n) {
            int l = seg.min_left(i, [&](int x) { return x < a[i]; });
            if(l) { query[l - 1].eb(i); }
        }
        dp.set(0, 0);
        vi mem(n);
        rep(i, n) {
            for(auto e : query[i]) chmax(mem[e], dp.prod(0, a[e] + 1) + 2);
            dp.set(a[i], max({dp.get(a[i]), dp.prod(0, a[i] + 1) + 1, mem[i]}));
            // cout << dp.get(a[i]) << endl;
        }
