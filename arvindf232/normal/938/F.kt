...
fun main(){
    ...
    solve.cases{
        ...
        val n = str.length
        val SC = IntArray(n){str[it].code}
        var previous = IntArray(n+1)
        previous[0] = 1
        val ret = mutableListOf<Char>()
        val k = log2(n.toDouble()).toInt()
        val willtake = (n - ((1 shl k) - 1))
//        val wholemask = (1 shl k) - 1

        repeat(willtake){phrase ->
...
//            val next = previous.copyOf()
            for(c in 0 until k){
                for(a in 0 until (1 shl k)){
                    val ct = 1 shl c
                    if(a and ct == 0){
                        next[1 + phrase + (a xor ct)] += next[1 + phrase + a]
                    }
                }
            }
