...
void slv(){
	...
	int mx=ab[n-1].a;
	{
		...
		for(int i=0;i<n;){
			int j=i;
			while(j<n&&ab[i]==ab[j])j++;
			
			if(ab[i].b==0){
				int need=mx-ab[i].a;
				if(!inc(0,need,i))tot=0;
			}else{
				if(ab[i].a<mx){
					int need=(n-1)-(mx-ab[i].a);
					if(j-i>=2||!inc(0,need,i))tot=0;
				}
			}
		...
		int tcnt=count(all(ab),pi(mx,1));
		
		ans+=tot*choose(n,tcnt);
	}
