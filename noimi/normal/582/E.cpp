...
template <typename T> vector<T> and_conv(vector<T> f, vector<T> g) {
    int n = 1;
...
vmint expr() {
    ...
    if(s[i] == '|') {
        i++;
        vmint nxt = expr();
        return ORconv::or_conv(now, nxt);
    } else if(s[i] == '&') {
        i++;
        vmint nxt = expr();
        return ANDconv::and_conv(now, nxt);
    } else if(s[i] == '?') {
        i++;
        vmint nxt = expr();
        return ORconv::or_conv(now, nxt) + ANDconv::and_conv(now, nxt);
    } else {
        return now;
    }
}
int main() {
    ...
    rep(i, 4) {
        int state = 0;
        rep(mask, 16) if(~mask & 1 << i) state += 1 << mask;
        v['a' + i].resize(1 << 16);
        v['a' + i][state] = 1;
        w[state]++;
    }
