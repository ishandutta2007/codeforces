...
signed main()
{
	initprog();
	int T=read();rg(T)
	int n=read(),f=1,ge=1,le=1;
	rg(n)a[i]=read();if(a[i])f=0;if(a[i]>0)le=0;if(a[i]<0)ge=0;gr
	if(f==1)
	{
		puts("0");continue;
	}
	int maxn=-inf,minn=inf;
	rg(n)mxe(maxn,a[i]);gr
	rg(n)mne(minn,a[i]);gr
	int sum=0;rg(n)sum+=a[i];gr
	if(minn*(n-1)+sum-minn>0)
	{
		puts("INF");
		continue;
	}
	if(maxn*(n-1)+sum-maxn<0)
	{
		puts("INF");
		continue;
	}
	rg(n+1)pre[i]=suf[i]=0;gr
	std::sort(a+1,a+1+n);
	a[0]=a[n+1]=1234567891234567894456;
	a[0]=-1234567891234567894456;
	rg(n+1)pre[i]=a[i]+pre[i-1];gr
	for(int i=n;i>=1;i--)suf[i]=a[i]+suf[i+1];
	int ans=-1234567891234567894456;
	for(int i=0;i<=n;i++){
		int L=-a[i+1],R=-a[i];
	int G=pre[i]*a[n];
	int S=suf[i+1]*a[1];
	G+=S;
	int B=(i)*a[n]+pre[i];
	int C=a[1]*(n-i)+suf[i+1];
	B+=C;
	if(i==0)G-=a[1]*a[1],B-=a[1]*2;
	else if(i==n)G-=a[n]*a[n],B-=a[n]*2;
	else G-=a[1]*a[n],B-=a[1]+a[n];
	// mxe(ans,(B*(-a[i])+G));
	mxe(ans,L*B+G);
	mxe(ans,R*B+G);
	gr
