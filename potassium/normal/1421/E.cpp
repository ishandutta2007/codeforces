...
int main() {
	...
	const ll IMP = -1e15;
	vector<vector<vector<vector<ll>>>> dp(n + 1, vector<vector<vector<ll>>>(2, vector<vector<ll>>(2, vector<ll>(3, IMP))));
	dp[1][0][0][2] = -a[1];
	dp[1][1][0][1] = a[1];
		...
		for (int now = 0; now <= 1; now++) {
			for (int last = 0; last <= 1; last++) {
				for (int m3 = 0; m3 <= 2; m3++) {
					const int mul = (now == 1 ? 1 : -1);
					const int lm3 = (m3 + 3 - mul) % 3;
					
					if (dp[i - 1][last][0][lm3] != IMP)
						chkmax(dp[i][now][0][m3], dp[i - 1][last][0][lm3] + a[i] * mul);
					
					if (dp[i - 1][last][1][lm3] != IMP)
						chkmax(dp[i][now][1][m3], dp[i - 1][last][1][lm3] + a[i] * mul);
						
					if (dp[i - 1][last][0][lm3] != IMP && now == last)
						chkmax(dp[i][now][1][m3], dp[i - 1][last][0][lm3] + a[i] * mul);
				}
			}
		}
	}
