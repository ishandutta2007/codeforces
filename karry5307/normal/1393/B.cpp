...
int main()
{
	...
	for(register int i=0;i<qcnt;i++)
	{
		...
		if(op=='+')
		{
			...
			if(barrel[u]==3||barrel[u]==5||barrel[u]==7)
			{
				s[barrel[u]>>1].erase(u),s[(barrel[u]+1)>>1].insert(u);
			}
			barrel[u]++;
		}
		else
		{
			...
			if(barrel[u]==4||barrel[u]==6||barrel[u]==8)
			{
				s[barrel[u]>>1].erase(u),s[(barrel[u]-1)>>1].insert(u);
			}
			barrel[u]--;
		...
		if((s[3].size()>1)||(s[3].size()&&(s[2].size()||s[1].size())))
		{
			puts("YES");
			continue;
		}
		if((s[2].size()>1)||(s[2].size()&&s[1].size()>1))
		{
			puts("YES");
			continue;
		}
