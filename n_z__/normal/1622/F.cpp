...
auto issqr=[&](auto a){return a==sqr((int)(0.5+sqrt(a)));};
bitset<1000001>nans;
if(n%4==0)nans[n/2]=1;
else if(n==1);
else if(n%4==1)nans[n/2]=nans[n]=1;
else if(n%4==2&&issqr(n+2))nans[n/2+1]=1;
else if(n%4==2&&issqr(n*(n/2-1)))nans[n/2-2]=1;
else if(n%4==2)nans[n/2]=nans[2]=1;
else if(issqr(n+1))nans[n/2+1]=nans[n]=1;
else if(issqr(2*(n/2)*(n/2-1)))nans[n]=nans[n/2-2]=1;
else if(issqr(n*(n/2-1)))nans[n/2-2]=nans[n-2]=1;
else nans[2]=nans[n/2]=nans[n]=1;
cout<<n-nans.count()<<endl;
for(int x=1;x<=n;x++)
if(!nans[x])cout<<x<<' ';
#endif
}
