...
fun main(){
    solve.cases{
		...
		val pair = MutableList(n){Pair(L[it], T[it])}
		pair.sortByDescending { it.first }
		...
		val remain = simpleSegTree(IntArray(n){1})
		for((a,b) in pair){
			if(b != -1){
				remain.setValue(b,0)
			}
		...
		for((a,b) in pair){
			if(b != -1) continue
			val already = remain.query(maxOf(a-s,0),n-1)
			if(already <= deleted){
				put(0)
				return@cases
			}
			now = now modM (already - deleted)
//			just dei (already - deleted)
			deleted ++
		}
		put(now)
