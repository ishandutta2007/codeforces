...
signed main() {
    ...
    int m = readUInt();
    vector<int> b(m + 2);
    for (int i = 1; i <= m; ++i) b[i] = readUInt();
    b[m + 1] = n + 1;
    n = sz(a);
    m = sz(b);
    for (int i = 0; i < n; ++i) {
        int ind = upper_bound(all(b), a[i]) - b.begin() - 1;
        if (b[ind] == a[i]) a[i] = 2 * ind;
        else a[i] = 2 * ind + 1;
    }
    vector<ll> sum(2 * m, 0), nsum(2 * m, 0), dp(2 * m, inf);
    for (int i = 0; i < n; ++i)
        if (p[i] < 0) nsum[a[i] + a[i] % 2] += p[i];
    dp[0] = 0;
    for (int i = 1; i < n; ++i) {
        int v = a[i];
        if (p[i] < 0) nsum[v + v % 2] -= p[i];
        if (v % 2 == 0)
            checkmin(dp[v], sum[v] + dp[v - 2] + nsum[v]);
        sum[v + v % 2] += p[i];
    }
    ll ans = dp[2 * m - 2];
    if (ans > n * (ll)mod) {
