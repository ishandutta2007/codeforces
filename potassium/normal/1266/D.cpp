...
int main() {
	...
	while (!posNet.empty()) {
		ll r = min(net[posNet.back()], -net[negNet.back()]);
		net[posNet.back()] -= r;
		net[negNet.back()] += r;
		ans.pb({{posNet.back(),negNet.back()},r});
		
		if (net[posNet.back()] == 0) posNet.pop_back();
		if (net[negNet.back()] == 0) negNet.pop_back();
	}
