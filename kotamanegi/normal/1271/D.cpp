...
void solve() {
    ...
    vector<int> require_now;
    REP(i, n) {
        if (k < get<0>(inputs[i])) {
            cout << -1 << endl;
            return;
        }
        require_now.push_back(k - get<0>(inputs[i]));
        k += get<1>(inputs[i]);
    }
    require_now.push_back(k);
    for (int q = n; q >= 1; --q) {
        require_now[q - 1] = min(require_now[q - 1], require_now[q]);
    }
    vector<int> can_use;
    priority_queue<int> nexter;
    can_use.push_back(require_now[1]);
    for (int i = 2; i < require_now.size(); ++i) {
        can_use.push_back(require_now[i] - require_now[i - 1]);
    }
    int ans = 0;
    for (int i = n - 1; i >= 0; --i) {
        if (Dont[i] == 0) {
            Dont[i] = 1;
            nexter.push(get<2>(inputs[i]));
        }
        for (auto& x : vertexs[i]) {
            if (Dont[x] == 1) continue;
            Dont[x] = 1;
            nexter.push(get<2>(inputs[x]));
        }
        REP(q, can_use[i]) {
            if (nexter.empty() == true) break;
            ans += nexter.top();
            nexter.pop();
        }
    }
