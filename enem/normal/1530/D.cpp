...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            else if(((ll)rad[i].size())>=2)
            {
                REP(j,1,((ll)rad[i].size()))
                {
                    fno.insert(rad[i][j]);
                }
            }
        ...
        if(((ll)fno.size()) == 1 && ((ll)bno.size()) == 1 && ((ll)spc.size()) == 1)
        {
            ll t = spc[0];
            fno.clear();
            spc.clear();
            fno.insert(rad[ad[t]][0]);
        }
        if(((ll)spc.size()) == 1)
        {
            ll t = -1;
            ...
            ad[spc[0]] = t;
            fno.erase(fno.find(spc[0]));
            bno.erase(bno.find(t));
            spc.clear();
        }
        if(!spc.empty())
        {
            REP(i,0,((ll)spc.size())-1)
            {
                ad[spc[i]] = spc[i+1];
                fno.erase(fno.find(spc[i]));
                bno.erase(bno.find(spc[i+1]));
            }
            spc.clear();
        }
        for(auto it:fno)
