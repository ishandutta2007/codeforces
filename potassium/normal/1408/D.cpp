...
SegTree<int> SEG(1000007);

int main() {
	int n,m; R(n,m);
	vector<pii> r(n), s(m); R(r,s);
	
	sort(all(r));
	sort(all(s));
	deque<pii> st;
	for (auto& i : s) {
		while (!st.empty() && st.back().F <= i.F && st.back().S <= i.S) {
			st.pop_back();
		}
	...
	st.push_front({-1, st[0].S});
	st.push_back({st.back().F, -1});
	...
	for (auto& i : r) {
		for (int j = 1; j < m; j++) {
			if (s[j].F < i.F) continue;
			int addx = max(0, s[j - 1].F - i.F + 1);
			int addy = max(0, s[j].S - i.S + 1);
			
			int l = addx, r = (j + 1 < m ? (s[j].F - i.F + 1) - 1 : 1000000);
			SEG.update(l, r, addy);
		}
	}
	...
	for (int i = 0; i <= 1000000; i++) {
		chkmin(ans, SEG.query(i, i).val + i);
	}
