...
int main() {
	...
	ull answer = 0;
	fori (bi, 0, 62) {
		// choose bit bi to make expected value away from original value
		// if a tie, prefer 0, since we eventually win if we find all 0s have
		// expected value 0
		ll ev[2];
		ev[0] = 0;
		ev[1] = 0;
		fori (i, 0, n) {
			if (mask[i] < (1ull<<(bi+1))) {
				ev[0] += __builtin_parityll(mask[i] & answer) ? -value[i] : value[i];
				ev[1] += __builtin_parityll(mask[i] & (answer | (1ull << bi))) ? -value[i] : value[i];
			}
		}
		if (origSum < 0) {
			if (ev[1] > ev[0]) {
