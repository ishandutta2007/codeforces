...
public class TaskD {
	...
	private void solve() {
		...
		Graph graph = new BidirectionalGraph(cityCount);
		...
		out.println(graph.dfs(0, new DFSCallback<Result, Integer>() {
			public Result enterUnvisitedVertex(Graph graph, int vertex, Edge edge, Integer parameters) {
				return new Result();
			}

			public Result enterVisitedVertex(Graph graph, int vertex, Edge edge, Integer parameters) {
				return null;
			}
			...
			public Integer beforeUsingEdge(Graph graph, int vertex, Edge edge, Integer parameters, Result currentResult, Edge nextEdge) {
				if (nextEdge.getDestination() == parameters)
					return null;
				return vertex;
			}

			public boolean afterVisitingEdge(Graph graph, int vertex, Edge edge, Integer parameters, Result currentResult, Edge nextEdge, Result visitResult, Integer passedParameters) {
				if (visitResult != null) {
