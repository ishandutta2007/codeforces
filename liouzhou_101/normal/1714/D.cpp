...
void solve()
{
	...
	vector<vector<pair<int, int>>> res(m + 1);
	dp[0] = 0;
	for (int i = 0; i < m; ++i)
	{
		for (int j = 0; j < n; ++j)
		{
			...
			{
				if (ii + s[j].size() > i && t.substr(ii, s[j].size()) == s[j])
					if (freshmin(dp[ii + s[j].size()], dp[i] + 1))
					{
						res[ii + s[j].size()] = res[i];
						res[ii + s[j].size()].push_back({ j, ii });
					}
			}
		}
	}
