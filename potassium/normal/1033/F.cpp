...
int main() {
	...
	const int MAXW = 12;
	const int FULL = 1 << MAXW;
	vector<int> cnt(FULL, 0);
	for (int i = 0; i < n; i++)
		cnt[a[i]]++;
 
	vector<int> pow3(MAXW + 1, 1);
	for (int i = 1; i <= MAXW; i++)
		pow3[i] = pow3[i - 1] * 3;
 
	vector<vector<int>> bin_bits(FULL, vector<int>(MAXW));
	for (int mask = 0; mask < FULL; mask++)
		for (int i = 0; i < MAXW; i++)
			bin_bits[mask][i] = (mask >> i) & 1;
 
	const int MFULL = pow3[MAXW];
	assert(MFULL == 531441);
	vector<int> mcnt(MFULL, 0);
	for (int mask = 0; mask < FULL; mask++) {
		int mmask = 0;
		for (int i = 0; i < MAXW; i++)
			mmask += bin_bits[mask][i] * pow3[i];
		mcnt[mmask] = cnt[mask];
	}
	
	for (int mmask = 0; mmask < MFULL; mmask++) {
		vector<int> bits(MAXW);
		int tmp = mmask;
		for (int i = 0; i < MAXW; i++) {
			bits[i] = tmp % 3;
			tmp /= 3;
		...
		int high2 = MAXW - 1;
		while (high2 >= 0 && bits[high2] != 2) high2--;
		if (high2 >= 0) {
			assert(mcnt[mmask] == 0);
			mcnt[mmask] = mcnt[mmask - pow3[high2]] + mcnt[mmask - pow3[high2] * 2];
		}
	}
 
	vector<vector<int>> allowed_mask(128, vector<int>(2, 0));
	vector<char> gates = { 'A', 'O', 'X', 'a', 'o', 'x' };
	for (auto& c : gates) {
		for (int a = 0; a <= 1; a++) {
			for (int b = 0; b <= 1; b++) {
				if (c == 'A' && (a & b) == 0) allowed_mask[c][a] |= (1 << b);
				if (c == 'O' && (a | b) == 0) allowed_mask[c][a] |= (1 << b);
				if (c == 'X' && (a ^ b) == 0) allowed_mask[c][a] |= (1 << b);
				if (c == 'a' && (a & b) == 1) allowed_mask[c][a] |= (1 << b);
				if (c == 'o' && (a | b) == 1) allowed_mask[c][a] |= (1 << b);
				if (c == 'x' && (a ^ b) == 1) allowed_mask[c][a] |= (1 << b);
			}
		}
	}
	...
	while (m--) {
		...
		vector<int> reqMask(FULL, 0);
		vector<bool> can(FULL, 1);
		for (int i = 0; i < MAXW; i++) {
			int bit = (1 << i);
			for (int mask = 0; mask < bit; mask++) {
				if (!can[mask]) {
					...
					reqMask[mask | bit] = reqMask[mask] + (allowed_mask[s[i]][1] == 0 ? can[mask | bit] = 0 : (allowed_mask[s[i]][1] - 1) * pow3[i]);
					reqMask[mask] += (allowed_mask[s[i]][0] == 0 ? can[mask] = 0 : (allowed_mask[s[i]][0] - 1) * pow3[i]);
				}
			}
		}
		...
		for (int mask = 0; mask < FULL; mask++)
			if (can[mask])
				ans += cnt[mask] * mcnt[reqMask[mask]];
