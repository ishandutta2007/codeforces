...
void solve() {
    ...
    segment_tree_t* tree = new segment_tree_t(1, m);
    cin >> q;
    for (int i = 1, x, y, u, v, k; i <= q; i++) {
        cin >> x >> y >> u >> v >> k;
        if (((x - u) % k) || ((y - v) % k)) {
            cout << "NO\n";
            continue;
        ...
        if (y > v) swap(y, v);
        int required_height = tree->get(y, v) + 1;
        int diff = x % k - required_height % k;
        if (diff < 0) diff += k;
        required_height += diff;
        if (required_height > n) {
            cout << "NO\n";
        } else {
            cout << "YES\n";
        }
