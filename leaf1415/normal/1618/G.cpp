...
int main(void)
{
	...
	ll ans = 0;
	rep(i, 1, n) vec.push_back(P(a[i], 1)), ans += a[i];
	rep(i, 1, m) vec.push_back(P(b[i], 0));
	sort(all(vec));
	
	reps(i, vec) dm[i].add(-vec[i].first), num[i] = vec[i].second, dm[i].adjust(num[i]);
	rep(i, 0, sz(vec)-1) mp[vec[i+1].first-vec[i].first].push_back(i);
	
	amp[-1] = ans;
	for(auto p : mp){
		vector<ll> &tmp = p.second;
		for(auto u : tmp){
			ll v = uf.root(u+1); u = uf.root(u);
			if(dm[u].size() > dm[v].size()) swap(u, v);
			num[v] += num[u];
			uf.unite(u, v);
			ans -= -dm[u].get() + -dm[v].get();
			for(auto x : dm[u].lowS) dm[v].add(x);
			for(auto x : dm[u].highS) dm[v].add(x);
			dm[v].adjust(num[v]);
			ans += -dm[v].get();
		}
		amp[p.first] = ans;
