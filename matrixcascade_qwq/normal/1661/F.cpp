...
int f(int x,int y)
{
	...
	int q=p+1;
	int val1,val2;
	int gw=x-p*y;val2=gw;val1=y-gw;
//	cout<<x<<" "<<y<<" "<<val1<<" "<<val2;edl;
	int vl=val1*(p*p)+val2*(q*q);
	return vl;
}
pii check(int x)
{
	...
	{
		int l=1,r=a[i]-1;
		pii ans=mkp(f(a[i],0),0);
		while(l<=r)
		{
			int mid=(l+r)>>1;
			if(f(a[i],mid-1)-f(a[i],mid)>=x)
			{
				ans=mkp(f(a[i],mid),mid);
				l=mid+1;
			}
			else r=mid-1;
		}
	//	cout<<ans.se<<" "<<f(a[i],0)-f(a[i],1);edl;
		if(ans.fi<0)
		{
		//	cout<<ans.se;edl;
		}
		as.fi+=ans.fi;as.se+=ans.se;
	}
	return as;
}
signed main()
...
//	cout<<f(100,0);
	while(T--)
	{
		...
		rdm;//cout<<ww.fi<<" "<<ww.se;edl;return 0;
		int l=1,r=1e18;
		int ans=-1;
		while(l<=r)
		{
			int mid=(l+r)>>1;
			if(check(mid).fi<=m)ans=mid,l=mid+1;
			else r=mid-1;
		}
	//	cout<<ans;edl;
	//	cout<<check(111).fi;edl;
		pii w=check(ans+1);
		int anss;//cout<<w.fi<<" "<<w.se;edl;
		anss=w.se+(w.fi-m+ans-1)/ans;
		opl(anss);
	}
}
/*
