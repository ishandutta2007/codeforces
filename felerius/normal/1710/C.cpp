// XOR Triangle (https://codeforces.com/contest/1710/problem/C)
...
int main() {
    ...
    vector dp(m + 1, array<array<MI, 8>, 8>{});
    dp[0][0][7] = 1;
    rep(i, m) {
        rep(j, 8) rep(k, 8) rep(bits, 8) {
            int k2 = k;
            if (bits == 0b011 || bits == 0b100)
                k2 |= 1 << 2;
            if (bits == 0b101 || bits == 0b010)
                k2 |= 1 << 1;
            if (bits == 0b110 || bits == 0b001)
                k2 |= 1 << 0;

            int a = (bits >> 2) & 1, b = (bits >> 1) & 1, c = bits & 1;
            int a_match = (j >> 2) & 1, b_match = (j >> 1) & 1, c_match = j & 1;
            int n_bit = n[i] - '0';
            ...
            if (n_bit) {
                ...
                if ((a_match && a) || (b_match && b) || (c_match && c))
                    continue;
            }

            dp[i + 1][k2][(a_match << 2) | (b_match << 1) | c_match] += dp[i][k][j];
        }
    }

    println(accumulate(all(dp[m][7]), MI(0)));
}