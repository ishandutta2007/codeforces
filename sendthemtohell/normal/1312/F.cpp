...
bool comp(int st1, int st2) {
	for (int pl = 0; pl < 6; ++pl) {
		for (int j = 0; j < 3; ++j) {
			if (dp[j][st1 + pl] != dp[j][st2 + pl]) return false;
		}
	}
	return true;
}
...
int stp() {
	for (int i = 1; i < N; ++i) {
		{
			vector<int> v = {dp[0][max(0, i - x)], dp[1][max(0, i - y)], dp[2][max(0, i - z)]};
			dp[0][i] = mex(v);
		}
		{
			vector<int> v = { dp[0][max(0, i - x)], dp[2][max(0, i - z)] };
			dp[1][i] = mex(v);
		}
		{
			vector<int> v = { dp[0][max(0, i - x)], dp[1][max(0, i - y)] };
			dp[2][i] = mex(v);
		}
	}

	for (int i = 106; i < N - 6; ++i) {
		if (comp(100, i)) {
			return i - 100;
		}
	}
