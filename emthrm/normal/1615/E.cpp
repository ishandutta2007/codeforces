...
int main() {
  ...
  int leaf = 0;
  FOR(i, 1, n) leaf += graph[i].size() == 1;
  if (leaf <= k) {
    const int r = min(n / 2, k);
    cout << 1LL * r * (n - r) << '\n';
    return 0;
  }
  vector<int> dep(n, 1);
  ...
  LazySegmentTree<monoid::RangeMaximumAndAddQuery<int, INF>> seg(n);
  REP(i, n) seg.set(hld.id[i], dep[i]);
  // REP(i, n) cerr << hld.id[i] << " \n"[i + 1 == n];
  // REP(i, n) cerr << seg[i] << " \n"[i + 1 == n];
  vector<int> can_paint_blue(n, true);
  REP(_, k) {
    const int max_dep = seg.get(0, n);
    for (int l = hld.inv[seg.find_right(0, [&](int x) -> bool { return x < max_dep; })];
         l != -1 && can_paint_blue[l]; l = hld.parent[l]) {
      can_paint_blue[l] = false;
      hld.subtree_v_update(l, [&](int a, int b) -> void { seg.apply(a, b, -1); });
    }
  }
  const int b = min(count(ALL(can_paint_blue), true), n / 2LL);
  cout << 1LL * (n - b - k) * (k - b) << '\n';
  return 0;
}