...
int main(){
  ...
  while(t--){
    ...
    sort(ALL(a));
    auto ch=[&](ll x,bool print){
      multiset<int>st(a.begin(),a.begin()+2*n);
      rep(i,0,n){
        ll mx=*st.rbegin();
        st.erase(st.find(mx));
        if(!st.count(x-mx))return false;
        if(print)cout<<mx spa x-mx<<endl;
    ...
    ll x=-1;
    rep(i,0,2*n-1){
      if(ch(a[2*n-1]+a[i],false))x=a[2*n-1]+a[i];
    }
    ans2(x!=-1);
    if(x!=-1){
      cout<<x<<endl;
