...
void solve() {
    ...
    FOR(i, 1, n+1) {
        if (a[i-1].S==0) constants += abs(a[i-1].F);
        else if (a[i-1].S>0) ord.pb(a[i-1]);
        else ord.pb(mp(-a[i-1].F, -a[i-1].S));
    ...
    sort(all(ord), cmp);
    m = ord.size();
    pre.rsz(m+1); pre[0] = mp(0, 0);
    F0R(i, m) pre[i+1] = mp(pre[i].F + ord[i].F, pre[i].S + ord[i].S);
    debug(ord, constants);
    // debug(pre);
    cin >> q;
    while (q--) {
        ...
        while (l<=r) {
            int m = (l+r)/2;
            if (cur*ord[m].S + ord[m].F > 0) {
                ans = m;
                r = m-1;
            } else l = m+1;
        }
        debug(ans);
        ll total = constants + cur*(pre[m].S - 2*pre[ans].S) + pre[m].F - 2*pre[ans].F;
        cout << total << nl;
    }
}
