...
const int N=3e5+10;
int n,a[N],x2[N],x3[N],dp[N];
int main() {
    R(n);
    REP1(i,1,n) R(a[i]);
    REP1(i,1,n-1) x2[i]=max(a[i],a[i+1]);
    REP1(i,1,n-2) x3[i]=max(min(a[i],a[i+1]),min(a[i+1],a[i+2]));
    REP1(i,1,n-1) {
        int lc=i-1;
        int rc=n-i-1;
        int d=abs(lc-rc);
    ...
    REP1(i,2,n-1) chkmax(dp[i],dp[i-2]);
    dp[n-1]=*max_element(a+1,a+n+1);
    W(VI(dp,dp+n));
    return 0;
}