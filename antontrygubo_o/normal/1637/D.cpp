...
void solve()
{
    ...
    int best = 0;
    for (int i = 0; i<=sum; i++) if (can[i] && abs(sum-2*i)<abs(sum-2*best)) best = i;

    ll sum1 = best; ll sum2 = sum-best;

    ll ans = sum1*sum1 + sum2*sum2;

    ll sumsq = 0;
    for (auto it: a) sumsq+=it*it;
    for (auto it: b) sumsq+=it*it;

    cout<<ans+sumsq*(n-2)<<endl;
