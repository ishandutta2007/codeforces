...
int main(){
	...
	cauto p=read();
	cauto q=read();
	cauto mp=readV<string>(h);
	// cauto h=300000;
	// cauto w=1;
	// cauto p=1;
	// cauto q=1;
	// vector<string> mp(h,".");
	// reps(i,1,h-1) mp[i][0]="DU"[i%2];

	const ll INF=1e17;
	auto cs=table(h,w,INF);
	using TP=tuple<ll,int,int>;
	priority_queue<TP,vector<TP>,greater<TP>> pq;
	cauto upd=
		[&](const int r, const int c, const ll d){
			if(MN(cs[r][c],d)) pq.emplace(d,r,c);
		};
	rep(r,h)rep(c,w)if(mp[r][c]=='.') upd(r,c,0);
	...
	ds['D']=2;
	while(pq.size()){
		cauto [d,r,c]=pq.top(); pq.pop();
		if(cs[r][c]!=d) continue;
		rep(d,4){
			cauto nr=r+dr[d];
			cauto nc=c+dc[d];
			if(nr<0 || nc<0 || nr>=h || nc>=w || mp[nr][nc]=='.' || mp[nr][nc]=='#') continue;
			cauto e=ds[mp[nr][nc]];
			//out(r,c,d,nr,nc,e,1);
			upd(nr+dr[e], nc+dc[e], cs[r][c]+(d==e?q:p));
		}
	}
	ll re=INF;
	rep(r,h)rep(c,w){
		if(r) MN(re, cs[r-1][c]+cs[r][c]);
		if(c) MN(re, cs[r][c-1]+cs[r][c]);
	}
	cout<<(re==INF?-1:re) NL;
	//out(cs,1);
	return 0;
