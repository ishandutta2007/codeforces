...
void chemthan() {
    ...
    FORd(v, n, 0) FOR(k, 1, n + 2) {
        ...
        FORd(u, v, 0) {
            ...
            while (u <= ptr - 1 && max(a[ptr - 1] - a[u], dp[ptr - 1][v][k - 1]) < max(a[ptr] - a[u], dp[ptr][v][k - 1])) {
                ptr--;
            }
            dp[u][v][k] = max(a[ptr] - a[u], dp[ptr][v][k - 1]);
        }
    }
    long long res = 0;
    FOR(i, 0, m) {
        int s, f, c, r; cin >> s >> f >> c >> r; s--, f--;
        chkmax(res, (long long) dp[s][f][r + 1] * c);
    }
    cout << res << "\n";
}
