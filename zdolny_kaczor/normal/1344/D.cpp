...
ll bou(ll a, ll b) {
	ll appr = llround((sqrt(3 * (4 * (ld)a - 4 * (ld)b - 1)) - 3) / 6);
	mini(appr, a);
	maxi(appr, 0);
	int steps = 0;
	while (appr > 0 && df(a, appr) < b) {
		appr--;
		steps++;
	}
	while (appr < a && df(a, appr + 1) >= b) {
		appr++;
		steps++;
	}
	debug(imie(a), imie(b), imie(steps), imie(appr));
	// assert(steps <= 1);
	return appr;
}
...
ll fin[nax];
int main() {
	debug(fun(df, 1e9, 1e9));
	for (int i = 0; i < 10; ++i) debug(fun(df, 5, i));
	int n;
	scanf("%d", &n);
	ll k;
	scanf("%lld", &k);
	REP(i, n) scanf("%d", a + i);
	ll low = -4e18, high = 4e18, ans = 0;
	while (low <= high) {
		ll med = (low + high) / 2;
		ll taken = 0;
		REP(i, n) taken += bou(a[i], med);
		debug(imie(med), imie(taken));
		if (taken <= k) {
			ans = med;
			high = med - 1;
		}
	...
	debug(imie(ans));
	REP(i, n) mi[i] = bou(a[i], ans);
	REP(i, n) MA[i] = bou(a[i], ans - 1);
	ll misum = 0;
	REP(i, n) misum += mi[i];
	ll MAsum = 0;
	REP(i, n) MAsum += MA[i];
	debug(imie(misum), imie(MAsum));
	assert(misum <= k && MAsum >= k);
	ll left_to_add = k - misum;
	debug(range(mi, 0, n), range(MA, 0, n));
	for (int i = 0; i < n; ++i) {
		ll now = min(left_to_add, MA[i] - mi[i]);
		left_to_add -= now;
		fin[i] = mi[i] + now;
	}
	assert(left_to_add == 0);
	REP(i, n) printf("%lld ", fin[i]);
	printf("\n");
	exit(0);
}
//#STAY AT HOME