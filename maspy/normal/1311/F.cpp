...
signed main() {
  ...
  auto V = vin(N);
  VEC<pi> XV(N);
  FOR(i, N) XV[i] = mp(X[i], V[i]);

  sort(all(XV), [](auto& x, auto& y) { return x.fi < y.fi; });

  sort(all(V));
  auto rank = [&](ll v) -> ll { return lower_bound(all(V), v) - V.begin(); };

  // v  xx 
  FenwickTree<ll> bit_c(N);
  FenwickTree<ll> bit_s(N);
  ...
  FORIN(p, XV) {
    ...
    // 
    v = rank(v);
    ll c = bit_c.get_sum(v + 1);
    ll s = bit_s.get_sum(v + 1);
    ANS += x * c - s;
    bit_c.add(v, 1);
    bit_s.add(v, x);
  }
