...
int main(void)
{
	...
	while(ub-lb>1){
		mid = (ub+lb)/2;
		if(check(mid) != -1 || check(mid-1) != -1) ub = mid;
		else lb = mid;
	}
	ll p = check(ub);
	
	set<ll> lset, rset;
	rep(i, 1, p) rset.insert(i);
	rep(i, p+1, ub) lset.insert(i);
	...
	vector<ll> llvec, lrvec, rlvec, rrvec;
	rep(i, 1, ub){
		if(i <= p){
			if(lset.count(i)) rlvec.push_back(i);
			else rrvec.push_back(i);
		}
		else{
			if(lset.count(i)) llvec.push_back(i);
			else lrvec.push_back(i);
		}
	}
	assert(abs(sz(lrvec)-sz(rlvec)) <= 1);
	
	if(sz(lrvec) + sz(rlvec) == 0 && sz(llvec) && sz(rrvec)){
		ll lid = llvec.back(); llvec.pop_back();
		ll rid = rrvec.back(); rrvec.pop_back();
		if(vec[lid].back() > vec[rid].back()) vec[rid].push_back(vec[lid].back()), vec[lid].pop_back();
		else vec[lid].push_back(vec[rid].back()), vec[rid].pop_back();
		lrvec.push_back(lid);
		rlvec.push_back(rid);
	}
	
	vector<ll> ans;
	bool left = (sz(lrvec) > sz(rlvec));
	if(sz(lrvec) + sz(rlvec) == 0) left = (sz(llvec) > 0);
	
	while(1){
		ll id;
		if(left){
			if(sz(llvec)) id = llvec.back(), llvec.pop_back();
			else if(sz(lrvec)) id = lrvec.back(), lrvec.pop_back(), left = false;
			else break;
		}
		else{
			if(sz(rrvec)) id = rrvec.back(), rrvec.pop_back();
			else if(sz(rlvec)) id = rlvec.back(), rlvec.pop_back(), left = true;
			else break;
		}
		ans.insert(ans.end(), all(vec[id]));
	}
