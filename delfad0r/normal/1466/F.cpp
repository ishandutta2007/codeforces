...
int ufind(int i) {
	return (f[i] == i) ? (i) : (f[i] = ufind(f[i]));
}
bool& ub(int i) {
	return b[ufind(i)];
}
void ujoin(int i, int j) {
	i = ufind(i);
	j = ufind(j);
	assert(i != j);
	if(rnk[i] < rnk[j]) {
...
for(int n = 0; n < N; ++n) {
	...
	if(k == 1) {
		...
		int x, y;
		cin >> x >> y;
		if(ufind(x) == ufind(y) or (ub(ufind(x)) and ub(ufind(y)))) {
			continue;
		} else {
			ujoin(x, y);
			ans.push_back(n + 1);
...
[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(pow(imod(2), ans.size())), " ", make_string_formatter(ans.size()), "\n", make_string_formatter(ans), "\n");  }();
