...
struct Mono {
  using value_type = pi;
  using X = value_type;
  static X op(X x, X y) { return {max(x.fi, y.fi), min(x.se, y.se)}; }
  static constexpr X unit = mp(-INF, INF);
...
void solve() {
  ...
  VEC(ll, B, N);
  SegTree<Mono> seg(N);
  FOR(i, N) seg.set(i, mp(A[i], B[i]));

  ll ANS = 0;
  FOR(L, N) {
    auto check_1 = [&](pi x) -> bool { return x.fi < x.se; };

    auto check_2 = [&](pi x) -> bool { return x.fi <= x.se; };

    ll R1 = seg.max_right(check_1, L);
    ll R2 = seg.max_right(check_2, L);
    ANS += R2 - R1;
  }
  print(ANS);
}
