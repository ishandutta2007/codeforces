...
void go(int v, int l, int r, int step){
    if (l == r){
        p[v][l] = 1;
        return;
    }
    int m = (l + r) / 2;
    go(2 * v, l, m, step / 2);
    go(2 * v + 1, m + 1, r, step / 2);
    for (int i = l; i <= m; i++)
        for (int j = m + 1; j <= r; j++)
            p[v][i] += p[2 * v + 1][j] * g[i][j],
            p[v][j] += p[2 * v][i] * g[j][i];
    for (int i = l; i <= m; i++)
        p[v][i] *= p[2 * v][i];
    for (int i = m + 1; i <= r; i++)
    ...
    for (int i = l; i <= m; i++){
        for (int j = m + 1; j <= r; j++)
            q[v][i] = max(q[v][i], q[2 * v + 1][j] + k * p[v][i]);
        q[v][i] += q[2 * v][i];
    }
    for (int i = m + 1; i <= r; i++){
        for (int j = l; j <= m; j++)
            q[v][i] = max(q[v][i], q[2 * v][j] + k * p[v][i]);
        q[v][i] += q[2 * v + 1][i];
    }
}
...
int main() {
    ...
    ld answer = 0;
    for (int i = 0; i < (1 << n); i++)
        answer = max(answer, q[1][i]);
    cout << fixed << setprecision(20) << (double)answer;
}