...
inline void AC() {
    ...
    n = read();
    rd(a, n);
    if(count(a+1, a+1+n, 0) && count(a+1, a+1+n, 1)) no(7);
    ef(!count(a+1, a+1+n, 0) && !count(a+1, a+1+n, 1)) yes(7);
    ef(count(a+1, a+1+n, 0) && !count(a+1, a+1+n, 1)) yes(7);
    else {
        sort(a+1, a+1+n);
        bool ok = true;
        for(_ i = 2; i <= n; ++i) if(a[i] == a[i-1]+1) ok = false;
        if(ok) yes(7);
        else no(7);
    }
