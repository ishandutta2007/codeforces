#line 1 "codeforces-workspace\\contest\\1558\\a\\a.cpp"
...
#line 16 "codeforces-workspace\\contest\\1558\\a\\a.cpp"
//*/
...
void main() {
  ...
  input a, b;
  set<i32> ans;
  for (auto m : {(a + b) / 2, (a + b + 1) / 2}) {
    for (auto x : range(min<i64>(a, m) + 1)) {
      if (a - x > a + b - m) continue;
      ans.emplace(x + (a + b - m - (a - x)));
    }
  }
  cout << ans.size() << "\n";
  cout << ans << "\n";
