...
void solve() {
    int n, m;
    cin >> n >> m;
    rep(i, 1, n) rep(j, 1, n) d[i][j] = ww[i][j] = inf;
    rep(i, 1, n) d[i][i] = 0;
    int u, v, w;
    rep(i, 1, m) {
        cin >> u >> v >> w;
        d[u][v] = d[v][u] = 1, cmin(ww[u][v], w), cmin(ww[v][u], w);
    }
    rep(k, 1, n) rep(i, 1, n) rep(j, 1, n) cmin(d[i][j], d[i][k] + d[k][j]);
    ll ans = INT64_MAX;
    rep(i, 1, n) rep(j, 1, n) if(ww[i][j] != inf) {
        rep(k, 1, n) ans = min(ans, (ll)ww[i][j] * (d[1][k] + d[n][k] + d[i][k] + 2));
        ans = min(ans, (ll)ww[i][j] * (d[1][i] + d[n][j] + 1));
    }
    cout << ans << '\n';
}
int main() {
