...
int main(void)
{
	...
	ll ans = 0, pos = 0, epos = 0;
	while(1){
		//outl(pos, deq);
		if(deq.front().first > inf/2){
			outl(-1);
			return 0;
		}
		if(pos + deq.front().second < vec[epos].first){
			ll cost = deq.front().first, len = deq.front().second;
			ans += cost * len, pos += len;
			deq.push_back(P(inf, len));
			deq.pop_front();
		}
		else{
			ll cost = deq.front().first, len = vec[epos].first - pos;
			ans += cost * len, pos += len;
			if(pos == d) break;
			
			deq.front().second -= len;
			deq.push_back(P(inf, len));
			if(deq.front().second == 0) deq.pop_front();
			
			ll sum = 0, price = vec[epos].second;
			while(sz(deq) && deq.back().first >= price){
				sum += deq.back().second;
				deq.pop_back();
			}
			deq.push_back(P(price, sum));
			epos++;
		}
	}
	outl(ans);
