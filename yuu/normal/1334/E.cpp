...
void solve_case() {
    ...
    for (auto&& [p, c] : factors) {
        ...
        if (diff > 0) pos.emplace_back(diff);
        else if (diff < 0) neg.emplace_back(-diff);
    }
    cout << (solve(pos) * solve(neg)) % base << '\n';
}

void solve() {
    cin >> d;
    for (int64_t p = 2; p * p <= d; p += 1 + (p > 2)) {
        if (d % p == 0) {
    ...
    if (d > 1) factors.emplace_back(d, 1);
    int q;
    cin >> q;
    while (q--) solve_case();
}
