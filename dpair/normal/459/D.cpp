...
int n, a[MAXN], b[MAXN], m, c[MAXN], cnt[MAXN];
int s[MAXN], t[MAXN];

inline int lowbit(int x) {return x & -x;}
inline void add(int x) {for (;x <= n;x += lowbit(x)) ++ c[x];}
inline int ask(int x) {int ret = 0; for (;x;x -= lowbit(x)) ret += c[x]; return ret;}

signed main(){
    read(n); read(a + 1, a + n + 1); memcpy(b + 1, a + 1, n * sizeof(int));
    std :: sort(b + 1, b + n + 1); m = std :: unique(b + 1, b + n + 1) - b - 1;
    rep(i, 1, n) a[i] = std :: lower_bound(b + 1, b + m + 1, a[i]) - b;
    
    rep(i, 1, n) s[i] = (++ cnt[a[i]]);
    memset(cnt, 0, sizeof(cnt)); per(i, n, 1) t[i] = (++ cnt[a[i]]);
    LL ans = 0; per(i, n, 1) {ans += ask(s[i] - 1); add(t[i]); }
    print(ans);
}