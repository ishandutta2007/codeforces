...
int p[MAXN], tot; bool b[MAXN];
int fa[MAXN], n, q, a[MAXN];
inline int find(int x) { return fa[x] == x? x : fa[x] = find(fa[x]); }
inline void sieve(const int N = 1e6 + 1) {
    rep(i, 2, N) {
        if(!b[i]) p[++ tot] = i;
        rep(j, 1, tot) {
            if(i * p[j] > N) break;
...
inline void init() { sieve();
    read(n, q); read(a + 1, a + n + 1);
    rep(i, 1, n) { int x = a[i];
        if(!b[a[i]]) continue;
        rep(j, 1, tot) {
            if(p[j] * p[j] > x) break;
            if(x % p[j] == 0) { unin(a[i], p[j]); while(x % p[j] == 0) x /= p[j]; }
        } if(x > 1) unin(a[i], x);
    }
    rep(i, 1, n) { int x = a[i] + 1;
        if(!b[a[i] + 1]) continue;
        std::vector < int > tmp; tmp.push_back(find(a[i]));
        rep(j, 1, tot) {
            if(p[j] * p[j] > x) break;
            if(x % p[j] == 0) { 
                const int id = find(p[j]);
                for (auto it : tmp) vis[std::make_pair(it, id)] = vis[std::make_pair(id, it)] = 1;
                tmp.push_back(id); while(x % p[j] == 0) x /= p[j];
            }
        } if(x > 1) { const int id = find(x); for (auto it : tmp) vis[std::make_pair(it, id)] = vis[std::make_pair(id, it)] = 1; }
    }
}
signed main(){
    init(); while(q --) {
        int u, v; read(u, v); if(find(a[u]) == find(a[v])) print("0");
        else {
            if(vis[std::make_pair(find(a[u]), find(a[v]))]) print("1");
            else print("2");
        }
    }
}
