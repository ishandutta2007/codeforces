...
int main(){
  ...
  int p[2] = {n/2, (n+1)/2};
  for(li i = 0; i < (li)(n); ++i) if(A[i]) p[A[i]&1] -= 1;
  for(li i = 0; i < (li)(n+1); ++i) for(li j = 0; j < (li)(n+1); ++j) for(li k = 0; k < (li)(2); ++k) dp[i][j][k] = 1e9;
  li r = p[0]+p[1];
  dp[0][p[0]][0] = dp[0][p[0]][1] = 0;
  for(li i = 0; i < (li)(n); ++i) {
    if(A[i] == 0) {
      for(li j = 0; j < (li)(n+1); ++j) for(li k = 0; k < (li)(2); ++k) for(li l = 0; l < (li)(2); ++l) if((l==0&&j)||(l==1&&r-j)) {
        dp[i+1][j-(l==0)][l] = min(dp[i+1][j-(l==0)][l], dp[i][j][k] + (k != l));
      }
      r -= 1;
    }else{
      for(li j = 0; j < (li)(n+1); ++j) for(li k = 0; k < (li)(2); ++k) {
        dp[i+1][j][A[i]&1] = min(dp[i+1][j][A[i]&1], dp[i][j][k] + (k != (A[i]&1)));
      }
    }
  }
