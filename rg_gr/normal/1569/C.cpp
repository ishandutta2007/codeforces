...
int fac[1000005],inv[1000005],a[200005];
...
signed main()
{
	...
	rg(n)a[i]=read();gr
	std::sort(a+1,a+1+n);
	if(a[n]==a[n-1])oldl(fac[n]);
	else if(a[n]-a[n-1]>1)odl(0);
	else
	{
		int c=0;
		rg(n)if(a[i]==a[n-1])c++;gr
		int ans=0;
		rg(n)ans=(ans+C(n-i,c)*fac[n-1-c]%mod*fac[c]%mod)%mod;gr
		oldl((fac[n]-ans+mod)%mod);///
	}
	gr
	return 0;
}