...
void solve() {
  ...
  ll THRESH = 1;
  while (THRESH * THRESH <= LIM) ++THRESH;
  // THRESH 
  // small  p  p^k 
  vvc<int> small_dat(LIM + 1);
  for (auto&& p: primes) {
    if (p >= THRESH) break;
    ll pp = 1;
    while (1) {
      ...
      FOR(i, N) small_dat[pp][i + 1] += small_dat[pp][i];
    }
  }

  FOR(x, LIM + 1) IDS[x].clear();
  FOR(i, N) IDS[A[i]].eb(i);

  // large prime  nxt, prev 
  vc<int> nxt(N, N), prev(N, -1);
  FOR(x, LIM + 1) {
    auto& I = IDS[x];
    FOR(i, len(I) - 1) {
      nxt[I[i]] = I[i + 1];
  ...
  // large prime bucket to bucket 
  ll b_sz = 1;
  while (b_sz * b_sz < N) ++b_sz;
  ll b_num = ceil(N, b_sz);
  while (len(A) < b_sz * b_num) A.eb(1);
  vv(mint, large_dat, b_num, b_num + 1);
  {
    vi CNT(LIM + 1);
    Mo mo;
    vc<pi> LR;
    FOR(l, b_num) FOR3(r, l, b_num + 1) {
      LR.eb(l, r);
      mo.add(b_sz * l, b_sz * r);
    }
    mint ANS = 1;
    auto ADD = [&](int i) -> void {
      ll x = A[i];
      if (CNT[x] == 0) ANS *= mint(x);
      CNT[x]++;
    };
    auto RM = [&](int i) -> void {
      ll x = A[i];
      CNT[x]--;
      if (CNT[x] == 0) ANS *= inv<mint>(x);
    };
  ...
  // FOR(i, b_num) print(large_dat[i]);

  auto solve_small = [&](ll L, ll R) -> mint {
    mint ANS = 1;
    for (auto&& p: primes) {
      if (p >= THRESH) break;
      ll pp = 1;
      while (1) {
        pp *= p;
        if (pp > LIM) break;
        auto& dat = small_dat[pp];
        if (dat[R] - dat[L] > 0) ANS *= p;
      }
    }
    return ANS;
  };

  auto solve_large = [&](ll L, ll R) -> mint {
    // 
    ll l = ceil(L, b_sz);
    ll r = floor(R, b_sz);
    if (r <= l) {
    ...
    mint ANS = large_dat[l][r];
    FOR3_R(i, L, l * b_sz) if (A[i] > 1) {
      if (nxt[i] < r * b_sz) continue;
      ANS *= A[i];
    }
    FOR3(i, r * b_sz, R) if (A[i] > 1) {
      if (prev[i] >= L) continue;
      ANS *= A[i];
    }
    return ANS;
