...
for(int i = 0; i < 3; ++i) {
	for(int j = i + 1; j < 3; ++j) {
		if(ones[i] >= N and ones[j] >= N) {
			a = i;
			b = j;
			c = '1';
			d = '0';
		} else if(ones[i] <= N and ones[j] <= N) {
			a = i;
			b = j;
			c = '0';
			d = '1';
...
int i = 0, j = 0;
for(int k = 0; k < 3 * N; ++k) {
	if(i < 2 * N and S[a][i] == d) {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(d), "");  }(); ++i;
	} else if(j < 2 * N and S[b][j] == d) {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(d), "");  }(); ++j;
	} else {
		[&]() { auto& out = cout; put_stream(out, "", make_string_formatter(c), "");  }(); ++i; ++j;
	}
}
