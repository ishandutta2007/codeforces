...
int main() {
    ...
    F0R (i, M) {
        ...
        if (!u) is_root_adj[v] = 1, root_adj_wt[v] = w;
        if (!v) is_root_adj[u] = 1, root_adj_wt[u] = w;
    }
    ...
    vi pair_wt(N);
    {
        ...
        for (auto [u, v, w, i] : edges) {
            assert(u < v);
            if (is_root_adj[u] && is_root_adj[v]) {
                partner[u] = v;
                partner[v] = u;
                pair_wt[u] = w;
            }
    ...
    vb used_edge(M);
    F0R (ei, M) if (edges[ei].u == 0) used_edge[ei] = 1;

    auto dfs = [&](auto& self, int loc, int &res) -> void {
        visited[loc] = 1;
        for (int ei : adj[loc]) {
            if (used_edge[ei]) continue;
            used_edge[ei] = 1;
            ...
            if (visited[nbr]) {
                int cyc_wt = path_wt[loc] ^ w ^ path_wt[nbr];
                res = add[res][index(elts, ss(1ull << cyc_wt))];
            } else {
                path_wt[nbr] = path_wt[loc] ^ w;
                self(self, nbr, res);
            }
        }
    };
    ...
    vb done(N);
    F0R (u, N) if (is_root_adj[u] && partner[u] >= u) {
        int space = 0;
        dfs(dfs, u, space);
        /*cerr << u << ": ";
        F0R (b, 32) cerr << ((elts[space] >> b)&1);
        cerr << endl;*/

        vmn WAYS = ways;
        if (partner[u] == u) {
            F0R (e, sz(ways))
                WAYS[add[e][space]] += ways[e];
        } else if (partner[u] > u) {
            int cyc_wt = root_adj_wt[u] ^ pair_wt[u] ^ root_adj_wt[partner[u]];
            int if_both = add[space][index(elts, ss(1ull << cyc_wt))];
            F0R (e, sz(ways)) {
                WAYS[add[e][space]] += 2 * ways[e];
                WAYS[add[e][if_both]] += ways[e];
            }
        }
