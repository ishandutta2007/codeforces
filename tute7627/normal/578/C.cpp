...
int main(){
  ...
  ll res=0,buf=0;
  bool judge = true;
  ConvexHullTrickAddMonotone<ld,false>mxc;
  ConvexHullTrickAddMonotone<ld,true>mic;
  ll n;cin>>n;
  vector<ll>a(n);
  ll cum=0;
  mxc.add(0,0);
  ...
  vector<ld>c;
  rep(i,0,mic.H.size()-1){
    ld ax=mic.H[i].fi,ay=mic.H[i].se;
    ld bx=mic.H[i+1].fi,by=mic.H[i+1].se;
    c.PB((by-ay)/(ax-bx));
  }
  rep(i,0,mxc.H.size()-1){
    ld ax=mxc.H[i].fi,ay=mxc.H[i].se;
    ld bx=mxc.H[i+1].fi,by=mxc.H[i+1].se;
    c.PB((by-ay)/(ax-bx));
  }
  ld ret=INF;
  for(auto z:c){
    chmin(ret,mxc.query(z)-mic.query(z));
  }
  lfs<<ret<<endl;
  return 0;
}