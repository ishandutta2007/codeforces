...
inline void AC() {
    ...
    read(b);
    _ s = 0, c = 0;
    for(_ i = 0; i < n; ++i) if(a[i] != b[i]) ++s;
    else ++c;
    bool fa = count(a.begin(), a.end(), '1') == count(b.begin(), b.end(), '1'), fb = count(a.begin(), a.end(), '0')+1 == count(b.begin(), b.end(), '1');
    if(!fa && !fb) writeln(-1);
    ef(fa && fb) writeln(min(c, s));
    ef(fa) writeln(s);
    else writeln(c);
