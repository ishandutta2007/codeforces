...
signed main(){
    ...
    ll odd=0;
    each(i,s)odd+=i&1;
    if(odd>1)return out(0);
    rep(n){
        if(a[i]!=a[n-1-i])break;
        if(i+1==n)return out(n*(n+1)/2);
    }
    vec(ll,cnt,n);
    ll min=LINF,max=0;
    rep(n/2){
    ...
    rep(n/2){
        cnt[a[n-1-i]]++;
        if(s[a[n-1-i]]<cnt[a[n-1-i]]*2){
            max=n-1-i;
            break;
        }
    }
    if(min==LINF&&max==0){
        rep(n/2)if(a[i]!=a[n-1-i]){
            chmin(min,i);
            chmax(max,i);
        }
        out((min+1)*(n-max)*2-(min+1)*(min+1));
    }
    else{
        ll same=0;
        chmax(max,(n-1)/2);
        ...
        ll l1=same,r1=max,l2=min,r2=n-1-same;
        out((l1+1)*(n-r1)+(l2+1)*(n-r2)-(l1+1)*(n-r2));
    }
}