...
int main(void)
{
	...
	rep(i, 1, n){
		...
		if(t > 1) mp[i][t]++;
		for(auto p : mp[i]) vec.pb(P(i, p.fst));
	}
	uniq(vec);
	reps(i, vec) rmp[vec[i]] = i+1;
	...
	ll S = N+1, T = N+2;
	reps(i, vec){
		if(vec[i].fst % 2) dc.add_edge(S, i+1, mp[vec[i].fst][vec[i].snd]);
		else dc.add_edge(i+1, T, mp[vec[i].fst][vec[i].snd]);
	}
	...
	rep(i, 1, m){
		...
		for(auto p : mp[u]){
			ll q = p.fst;
			if(mp[v].count(q) == 0) continue;
			dc.add_edge(rmp[P(u, q)], rmp[P(v, q)], inf);
		}
	}
	outl(dc.calc(S, T));
