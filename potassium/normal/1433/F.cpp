...
int main() {
	...
	const int IMP = -1e7;
	auto knap = [&](vector<int>& val) {
		vector<vector<vector<int>>> dp(m + 1, vector<vector<int>>(m + 1, vector<int>(k, IMP)));
		dp[0][0][0] = 0;
		for (int i = 0; i < m; i++) {
			for (int used = 0; used <= m / 2; used++) {
				for (int rem = 0; rem < k; rem++) {
					chkmax(dp[i + 1][used + 1][(rem + val[i]) % k], dp[i][used][rem] + val[i]);
					chkmax(dp[i + 1][used][rem], dp[i][used][rem]);
				}
			}
		}

		vector<int> result(k, IMP);
		for (int rem = 0; rem < k; rem++) {
			for (int used = 0; used <= m / 2; used++) {
				chkmax(result[rem], dp[m][used][rem]);
			}
		}
	...
	auto convo = [&](vector<int>& v1, vector<int>& v2) {
		vector<int> ans(k, IMP);
	...
	for (int i = 0; i < n; i++) {
		vector<int> v(m); R(v);
		vector<int> res = knap(v);
		ans = move(convo(ans, res));
	}
