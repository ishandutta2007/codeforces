...
fun main(){
    solve.cases{
        ...
        val hash = LongArray(n){0}
        val G = Graph(n,false)

        for(i in 0 until n){
            hash[i] = hash[i] xor rand[i]
        }
        repeat(m){
            val a = getint() - 1
            val b = getint() -1
            G.add(a,b)
            hash[a] = hash[a] xor rand[b]
            hash[b] = hash[b] xor rand[a]
        }
        val component = IntArray(n)
        val groups = (0 until n).groupBy { hash[it] }
        var count = -1
        ...
        val comCount = count + 1
        val contracted = Graph(comCount,false)
        val done = mutableSetOf<Long>()
        G.everyEdge { a, b ->
            val c1 = component[a]
            val c2 = component[b]
            if(c1 == c2) return@everyEdge
            val c11 = minOf(c1,c2)
            val c22 = maxOf(c1,c2)
            val dothis = ((c11.toLong()) shl 32) xor c22.toLong()
            if(done.contains(dothis)) return@everyEdge
            done.add(dothis)
            contracted.add(c11,c22)
        }
        if(comCount == 1){
            put("YES")
            put(List(n){1}.joinToString(" "))
            return@cases
        }
        if(done.size == comCount -1 ){
            val root = (0 until n).firstOrNull { contracted.E[it].size == 1 }
            if(root != null){
                var bad = false
                val ret = IntArray(comCount)
                var final = 4
                fun Graph.dfs(v:Int, p:Int){
                    var found = 0
                    final++
                    ret[v] = final
                    NS(v) { w ->
                ...
                contracted.dfs(root,-1)
                if(!bad){
                    val finalRet = IntArray(n)
                    for(i in 0 until n){
                        finalRet[i] = ret[component[i]]
                    }
                    put("YES")
