...
fun solve() {
    ...
    fun recalc(ind: Int) {
        val x = makeOnes(w[ind],num[ind])
//        val ori = ArrayList(x)
//        fft(x)
//        println("START ${w[ind]} ${num[ind]}")
//        for (i in 0..10) println(x[i])
        val y = fft(x)
        val z = fft(y,true)
//        for (i in 0..10) println("${i} ${x[i]} ${z[i]}")
        upd(ind,fft(x))
    }
