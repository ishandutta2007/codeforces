...
void solve(){
    ...
    REP(i, k) {
        ...
        set<int> already;
        REP(q, n) {
            if (already.count(hoge[q])) continue;
            already.insert(hoge[q]);
            if (cnts.count(sums - hoge[q])) {
                cout << "YES" << endl;
                cout << cnts[sums - hoge[q]].first << " " << cnts[sums - hoge[q]].second << endl;
                cout << i + 1 << " " << q + 1 << endl;
                return;
            }
            cnts[sums - hoge[q]] = mp(i + 1, q + 1);
        }
    }
    cout << "NO" << endl;
    return;
