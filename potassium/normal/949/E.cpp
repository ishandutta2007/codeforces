...
int main() {
	...
	auto doubling = [&](vector<int> v) {
		for (auto& i : v)
			i *= 2;
		return v;
	};

	function<vector<int>(set<int>&)> brute = [&](set<int>& s) {
		s.erase(0);
		if (s.empty()) return vector<int>();
		if (s.size() == 1 && abs(*s.begin()) == 1) return vector<int>{ *s.begin() };
		...
		if (hasOdd) {
			set<int> s_n1 = e, s_p1 = e;
			for (auto& i : o) {
				s_n1.insert(i + 1);
				s_p1.insert(i);
			}
			
			vector<int> ans_n1 = doubling(brute(s_n1));
			ans_n1.emplace_back(-1);
			vector<int> ans_p1 = doubling(brute(s_p1));
			ans_p1.emplace_back(+1);

			return (ans_n1.size() < ans_p1.size() ? ans_n1 : ans_p1);
		}
		else
			return doubling(brute(e));
	};
	
	vector<int> ans = brute(s);
	W(ans.size());
	W(ans);
