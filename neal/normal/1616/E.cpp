...
void run_case() {
    ...
    vector<deque<int>> locations(ALPHABET);

    for (int i = 0; i < N; i++)
        locations[S[i] - MIN_CHAR].push_back(i);

    fenwick_tree<int> taken(N);
    int64_t best = INF64;
    int64_t current = 0;

    for (int index = 0; index < N; index++) {
        int t = T[index] - MIN_CHAR;
        int min_loc = N;

        for (int c = 0; c < t; c++)
            if (!locations[c].empty())
                min_loc = min(min_loc, locations[c].front());

        if (min_loc < N) {
            min_loc += taken.query_suffix(min_loc + 1);
            assert(min_loc >= index);
            best = min(best, current + min_loc - index);
        }

        if (locations[t].empty())
            break;

        int loc = locations[t].front();
        taken.update(loc, +1);
        loc += taken.query_suffix(loc + 1);
        assert(loc >= index);
        current += loc - index;
        locations[t].pop_front();
    }
