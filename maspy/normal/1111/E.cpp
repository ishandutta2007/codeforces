...
void solve() {
  ...
  auto collect_child = [&](int root, int v) -> vc<int> {
    vc<int> I;
    if (v == root) {
      // 
      I = ss.collect(0, N);
    } else {
      int to = hld.move(v, root);
      if (to == hld.parent[v]) {
        I = ss.collect(hld.LID[v], hld.RID[v]);
      } else {
        I = ss.collect(0, hld.LID[to]);
        auto J = ss.collect(hld.RID[to], N);
        I.insert(I.end(), all(J));
      }
    }
    vc<int> res;
    for (auto&& i: I) res.eb(hld.V[i]);
    return res;
  };
  ...
  FOR_(Q) {
    ...
    for (auto&& a: A) --a;
    vi dists(K);
    FOR(i, K) dists[i] = hld.dist(root, A[i]);
    auto I = argsort(dists);
    reverse(all(I));
    A = rearrange(A, I);
    bool has_root = false;
    for (auto&& a: A)
      if (a == root) has_root = true;
    if (!has_root) { A.eb(root); }
    for (auto&& v: A) {
      auto ch = collect_child(root, v);
      for (auto&& c: ch) ss.erase(hld.LID[c]);
      ss.insert(hld.LID[v]);
      // labeled  n 
      FOR3(n, 1, LIM + 1) {
        if (has_root || v != root) {
          mint dp = n;
          for (auto&& c: ch) { dp *= DP[c][n - 1]; }
          DP[v][n] = dp;
        } else {
          mint dp = 1;
          for (auto&& c: ch) { dp *= DP[c][n]; }
          DP[v][n] = dp;
        }
      }
    }
    vc<mint> f(m + 1);
    FOR(i, m + 1)
    f[i] = DP[root][i] * mint(i % 2 == 0 ? 1 : -1) * fact_inv<mint>(i);
    vc<mint> g(m + 1);
    FOR(i, m + 1) g[i] = fact_inv<mint>(i);
    f = convolution(f, g);
    f.resize(m + 1);
    FOR(i, len(f)) if (i % 2 == 1) f[i] = -f[i];
    mint ANS = SUM(f);
    print(ANS);
    for (auto&& v: A) FOR(n, LIM + 1) DP[v][n] = 0;
    for (auto&& v: A) ss.erase(hld.LID[v]);
  }
}
