...
void solve(){
	...
	S.ins(-1);
	for(int i = 0; i < n; ++i){
		int x = *prev(S.upper_bound(v2[i].s));
		int y = *S.upper_bound(v2[i].s);
		v3[v2[i].s] = {y-x-1,v[v2[i].s]};
		S.ins(v2[i].s);
	}
	sort(all(v3));
	map<int,int> M;
	...
	int l = 0;
	v3.push_back({1000000,0});
	for(int i = 1; i <= n; ++i){
		if(sz(M) == n-i+1 && prev(M.end())->first == n-i+1){
			cout << 1;
		}
		else cout << 0;
		while(v3[l].f <= i){
			M[v3[l].s]--;
			if(!M[v3[l].s])M.erase(v3[l].s);
			l++;
		}
	}
	cout << "\n";
