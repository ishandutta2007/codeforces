...
main()
{
	...
	in.getv(v, n);
	string s = in;
	vector <int> p(n), t(n), sp(n), st(n);
	p[0] = v[0];
	sp[n - 1] = v[n - 1];
	for (int i = 1; i < n; i++) p[i] = p[i - 1] + v[i];
	for (int i = n - 2; i >= 0; i--) sp[i] = sp[i + 1] + v[i];
	t[0] = (s[0] == 'B') ? v[0] : 0; 
	st[n - 1] = (s[n - 1] == 'B') ? v[n - 1] : 0;
	for (int i = 1; i < n; i++) t[i] = t[i - 1] + ((s[i] == 'B') ? v[i] : 0);
	for (int i = n - 2; i >= 0; i--) st[i] = st[i + 1] + ((s[i] == 'B') ? v[i] : 0);	
	/*
	out.putLn(p);
	out.putLn(sp);
	out.putLn(t);
	...
	*/
	int maxv = t[n - 1];
	for (int i = 0; i < n; i++)
		maxv = max(maxv, t[n - 1] - t[i] + (p[i] - t[i]));
	for (int i = 0; i < n; i++)
		maxv = max(maxv, t[n - 1] - st[i] + (sp[i] - st[i]));
