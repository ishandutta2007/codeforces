...
int main() {
  ...
  for (int start = 0; start < n; ++start) {
    ...
    for (int _ = n + 1; _--;) {
      ...
      for (int i = 0; i <= n; ++i) {
        if (fixed[i]) continue;
        if (nearest == -1 or dist[i] < dist[nearest]) nearest = i;
      }
      fixed[nearest] = true;

      if (nearest != super) {
        auto&& e = dist[nearest + 1 == n ? 0 : nearest + 1];
        e = min(e, dist[nearest] + 1);
      }

      for (auto&& [to, cost] : as_const(g[nearest == super ? start : nearest])) {
        auto&& e = dist[atcoder::modint(to + dist[nearest]).val()];
        e = min(e, dist[nearest] + cost);
      }
    }
