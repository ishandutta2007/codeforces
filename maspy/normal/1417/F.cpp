...
#line 1 "/home/maspy/compro/library/graph/tree_of_unionfind.hpp"
/*
...
void solve() {
  ...
  VEC(pi, query, Q);
  FOR(q, Q) query[q].se--;
  FOR(q, Q) if (query[q].fi == 2) TIME[query[q].se] = q;
  auto I = argsort(TIME);
  reverse(all(I));
  E = rearrange(E, I);
  TIME = rearrange(TIME, I);
  auto G = tree_of_unionfind(N, E);
  // G.debug();
  int root = N + M;
  HLD hld(G, root);
  ...
  vc<int> dat(N + Q + 1);
  iota(all(dat), 0);
  TreeMonoid<decltype(hld), Mono, 0> TM(hld, dat);

  P.resize(N + M + 1, 0);
  vi pos(N + 1, -1);
  FOR(i, N) pos[P[i]] = i;
  TreeMonoid<decltype(hld), Mono, 0> TM2(hld, P);
  ...
  FOR(q, Q) {
    ...
    int v = query[q].se;
    auto check = [&](auto e) -> bool { return e < N + n; };
    int r = TM.max_path(check, v, root);
    int x = TM2.prod_subtree(r);
    print(x);
    if (x) {
      TM2.set(pos[x], 0);
      P[pos[x]] = 0;
