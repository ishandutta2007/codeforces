...
int main() {
	...
	auto go = [&](int color, int r, int c) {
		W(color, r + 1, c + 1);
		col[r][c] = color;
	};

	const int evenCol = 1, oddCol = 2, auxCol = 3;
	int evenR = 0, evenC = 0;
	int oddR = 0, oddC = 1;

	auto goEven = [&](int color) {
		assert(color != oddCol);
		go(color, evenR, evenC);
		evenC += 2;
		if (evenC >= n) {
			evenR++;
			evenC &= 1;
	...
	auto goOdd = [&](int color) {
		assert(color != evenCol);
		go(color, oddR, oddC);
		oddC += 2;
		if (oddC >= n) {
			oddR++;
