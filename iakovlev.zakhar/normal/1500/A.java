...
public class C {
	...
	public void solve() {
		...
		if (eq.size() > 1 && eq.get(eq.size() - 1) - eq.get(0) > 1) {
			int fi = eq.get(0);
			int sc = eq.get(eq.size() - 1);
			out.println("YES");
			out.print(p[fi - 1].y + " " + p[sc - 1].y + " " + p[fi].y + " " + p[sc].y);
			return;
		}
		Pair[] b = new Pair[n - eq.size()];
		int f = 0;
		int max = 5000000;
		...
		for (int i = 0; i < p.length; i++) {
			if (i == 0 || p[i].x != p[i - 1].x) {
				...
				less[2 * p[i].x] = p[i - 1].y;
				more[2 * p[i].x] = p[i].y;
			}
		}
		for (int i = 0; i < b.length; i++) {
			for (int j = i + 1; j < b.length; j++) {
				if (less[b[i].x + b[j].x] > 0) {
					out.println("YES");
					out.println(b[i].y + " " + b[j].y + " " + less[b[i].x + b[j].x] + " " + more[b[i].x + b[j].x]);
					return;
				}
				less[b[i].x + b[j].x] = b[i].y;
				more[b[i].x + b[j].x] = b[j].y;
			}
		}
		out.println("NO");
	}
