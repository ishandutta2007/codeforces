...
int& get(int idx, bool a, bool i) {
  return nodes[idx].ans[a][i];
}

int walk(int idx, bool x) {
  if (!nodes[idx].transition[x]) {
    nodes[idx].transition[x] = ++nnodes;
    nodes[nnodes] = {};
  }
  return nodes[idx].transition[x];
}

int cwalk(int& idx, bool x) {
  if (!nodes[idx].transition[x]) {
    return -1;
  }
  return nodes[idx].transition[x];
...
int main() {
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    for (int i = 0; i < n; ++i) {
      int cur = 0;
      for (int pl = 0, node = 0; pl <= L && node != -1; ++pl) {
        maxi(cur, get(node, i >> L - pl & 1, a[i] >> L - pl & 1 ^ 1));
        node = cwalk(node, i >> L - pl & 1 ^ a[i] >> L - pl & 1);
      }
      ++cur;
      for (int pl = 0, node = 0; pl <= L; ++pl) {
        maxi(get(node, a[i] >> L - pl & 1, i >> L - pl & 1), cur);
        node = walk(node, a[i] >> L - pl & 1 ^ i >> L - pl & 1);
      }
      maxi(ans, cur);
    }
    print(ans);
