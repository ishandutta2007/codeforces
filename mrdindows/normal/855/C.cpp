        ...
        if (!used[to]) {
            ...
            for (int a = 10; a >= 0; --a) {
                for (int b = 10 - a; b >= 0; --b) {
                    dpnext[a + b][2] += dp[cur][a][2] * (dp[to][b][0] + dp[to][b][1] + dp[to][b][2]) % mod;
                    dpnext[a + b][0] += dp[cur][a][0] * (dp[to][b][0] + dp[to][b][2]) % mod;
                    dpnext[a + b][1] += dp[cur][a][1] * dp[to][b][2] % mod;
                }
            }
            FI(11) FJ(3) dp[cur][i][j] = dpnext[i][j] % mod;
        }
    }
}
int32_t solve() {
