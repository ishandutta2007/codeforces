...
int main() {
    ...
    sort(idx.begin(), idx.end(), [&](int l, int r) { return b[l] > b[r]; });

    struct node {
        double sum, mul;
        node(double sum, double mul) : sum(sum), mul(mul) {}
        bool operator==(const node& rhs) const { return sum == rhs.sum && mul == rhs.mul; }
    };
    auto f = [](node a, node b) { return node(a.sum * b.mul + b.sum, a.mul * b.mul); };
    auto g = [](node a, node b) { return node(a.sum + b.sum * a.mul, a.mul * b.mul); };
    LazySegmentTree<node, node> segl(n + 1, f, f, f, node(0, 1), node(0, 1)),
        segr(n + 1, g, g, g, node(0, 1), node(0, 1));
    vector<node> init(n + 1, node(1, 1));
    segl.build(init);
    segr.build(init);
    double ans = 0;

    for (int& x : idx) {
        double coef = segl.query(0, x + 1).sum * segr.query(x + 1, n + 1).sum * 0.5;
        ans += b[x] * coef;
        segl.set_val(x, node(0.5, 0.5));
        segr.set_val(x + 1, node(0.5, 0.5));
    }
