...
using namespace std;
int n,m,Mx,Testnum;
int b[14],lg[4102],t[2002],id[2002];
LL res[4102];
int a[14][2002];
LL dp[14][4102],mx[14][4102];
...
int main()
{
	...
	while(Testnum--)
	{
		...
		for(int i=1;i<=m;++i)t[i]=0,id[i]=i;
		for(int i=1;i<=n;++i)
			for(int j=1;j<=m;++j)
				read(a[i][j]),t[j]=max(t[j],a[i][j]);
		sort(id+1,id+m+1,cmp),m=min(n,m);
		for(int i=1;i<=m;++i)
		{
			for(int j=0;j<Mx;++j)mx[i][j]=0;
			for(int j=1;j<=n;++j)
			{
				for(int k=1;k<=n;++k)b[k]=a[(k+j-1)%n+1][id[i]];
