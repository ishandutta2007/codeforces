...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ll n; cin >> n;
        vector<ll> cnt(60), arr(n);
        REP(i, n) {
            ll temp; cin >> temp;
            arr[i] = temp;
            ll div = ((ll)1 << 59), idx = 59;
            while (temp > 0) {
                if (temp / div) {
                    temp -= div;
                    cnt[idx]++;
        ...
        REP(i, n) {
            ...
            while (div > 0) {
                if (temp / div) {
                    ...
                    right = (right + (div % MOD) * n) % MOD;
                    left = (left + (div % MOD) * cnt[idx]) % MOD; 
                    // cout << "right: " << div << " " << cnt[idx] << endl; 
                }
                else {
                    right = (right + (div % MOD) * cnt[idx]) % MOD;
                }
                div >>= 1;
                idx--;
            }
            // cout << "get " << left << " * " << right << endl;
