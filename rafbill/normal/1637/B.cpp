...
void solve() {
  ...
  vvi dp(n,vi(n));
  for(li sz = (1); sz <= (li)(n); ++sz) {
    for(li l = 0; l < (li)(n); ++l) if(li r = l+sz-1;1) if(r < n) {
      set<li> S; for(li i = (l); i <= (li)(r); ++i) S.insert(A[i]);
      while(S.count(dp[l][r])) dp[l][r]++;
      dp[l][r] += 1;
      for(li k = (l); k <= (li)(r-1); ++k) {
        smax(dp[l][r], dp[l][k]+dp[k+1][r]);
      }
    }
  }
  li ans = 0;
  for(li l = 0; l < (li)(n); ++l) for(li r = (l); r <= (li)(n-1); ++r) ans += dp[l][r];
  cout << ans << endl;
}
