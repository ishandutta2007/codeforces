#line 1 "codeforces-workspace\\contest\\1552\\e\\e.cpp"
...
#line 16 "codeforces-workspace\\contest\\1552\\e\\e.cpp"
//*/
...
void main() {
  ...
  input n, k;
  vector pos(n, vector<i32>());
  for (auto i : range(n * k)) {
    input a;
    --a;
    pos[a].emplace_back(i + 1);
  }
  // for (auto &&v : pos) {
  ...
  for (auto i = 0; i < n; i += k - 1) {
    auto j = min((int)n, i + k - 1);  // [i, j)
    auto t = 0;
    while (count(begin(used), end(used), 1) < j) {
      ++t;
      auto next = -1;
      for (auto x = i; x < j; x++) {
        if (used[x]) continue;
        if (next < 0 || pos[next][t] > pos[x][t]) {
          next = x;
        }
      }
      used[next] = 1;
      ans[next] = {pos[next][t - 1], pos[next][t]};
    }
  }
