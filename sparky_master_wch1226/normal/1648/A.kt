...
fun main(){
    solve.cases{
        val n = getint
        val m = getint
        val board = Array(n){getline(m)}

        fun solve(b:Array<IntArray>):Long {
            val found = IntArray(100004)
            val nn = b.size
            val mm = b[0].size
            var total = 0L
            for(i in 0 until nn){
...
//                    ret[i][j] = found[b[i][j]]
        ...
        val transpose = Array(m){IntArray(n)}
        for(i in 0 until n){
            for(j in 0 until m){
        ...
        put(solve(board) + solve(transpose))
