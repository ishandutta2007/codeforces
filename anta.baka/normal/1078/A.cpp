...
main() {
    ...
    if(x1 == x2 || y1 == y2) return cout << ans, 0;
    vector<pnt> p;
    ld Y1 = (ld)(-c - a*x1) / b; p.pb({x1, Y1});
    ld X1 = (ld)(-c - b*y1) / a; p.pb({X1, y1});
    ld Y2 = (ld)(-c - a*x2) / b; p.pb({x2, Y2});
    ld X2 = (ld)(-c - b*y2) / a; p.pb({X2, y2});
    for(int i = 0; i < 2; i++)
        for(int j = 2; j < 4; j++) {
            ans = min(ans, d({x1,y1}, p[i]) + d(p[i], p[j]) + d(p[j], {x2,y2}));
        }
    cout << ans;
}