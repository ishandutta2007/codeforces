...
int n, bel[MAXN];
struct NODE {int x, y, id; inline bool operator< (const NODE &b) const {return bel[x] == bel[b.x]? (bel[x] & 1? y < b.y : y > b.y) : x < b.x; } }a[MAXN];

signed main(){
    read(n); rep(i, 1, 1000000) bel[i] = (i - 1) / block + 1;
    rep(i, 1, n) read(a[i].x, a[i].y), a[i].id = i;
    std::sort(a + 1, a + n + 1); rep(i, 1, n) sprint(a[i].id);
    print();
}