...
constexpr int N = 3e3;
int b[N], m;
using DP = vector<pair<int, int64_t>>;
vector<int> nei[N];
...
DP solve(int v, int p) {
  ...
  for (auto u: nei[v]) if (u != p) {
    ...
    int sz = res.size();
    res.resize(min(m, int(res.size() + cur.size()) - !cur.empty()));
    reverse(cur.begin(), cur.end());
    for (int i = res.size(); i--; ) {
      auto t = pair{-1, 0ll};
      for (int j = unz(i - (int)cur.size() + 1); j <= min(i, sz - 1); ++j) {
        maxi(t, pair{res[j].first + cur[cur.size() - 1 - i + j].first, res[j].second + cur[cur.size() - 1 - i + j].second});
      }
      res[i] = t;
    }
  }
  ...
  for (int j = 1; j < res.size(); ++j) {
    auto nt = res[j];
    maxi(res[j], pair{t.first + (t.second > 0), 0ll});
    t = nt;
  }
  nei[v].clear();
  return res;
