...
int main(){
    ...
    vp ans(k);
    if(n%3==0){
        rep(i,n/3)ans[i]=P(i*3+2,n+1-i*3-2);
        rep(i,n/3-1)ans[i+n/3]=P(i*3+4,n+2-i*3-4);
        ans[k-1]=P(1,1);
    }
    else{
        rep(i,(k+1)/2)ans[i*2].fi=ans[k-i*2-1].se=i*3+1;
        rep(i,k/2)ans[i*2+1].fi=ans[k-i*2-1-1].se=i*3+3;
    }
