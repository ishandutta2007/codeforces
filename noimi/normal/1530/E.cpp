...
int main() {
    TEST {
        ...
        rep(i, 26) if(cnt[i] == 1) {
            string ans;
            ans.pb('a' + i);
            rep(j, 26) if(i != j) rep(_, cnt[j]) ans.pb('a' + j);
            OUT(ans);
            flag = true;
            break;
        }
        ...
        if(n - cnt[m] >= cnt[m] - 2) {
            ...
            if(s.back() == 'a' + k) {
                int x = cnt[m], y = cnt[k];
                string ans;
                ans.pb('a' + m), ans.pb('a' + k);
                rep(_, y - 1) ans.pb('a' + k);
                rep(_, x - 1) ans.pb('a' + m);
                OUT(ans);
            } else {
                int x = cnt[m], y = cnt[k];
                string ans;
                ...
                string p;
                for(auto e : s)
                    if(e != 'a' + m and e != 'a' + k) p.pb(e);
                sort(all(p));
                rep(_, x - 1) ans.pb('a' + m);
                ans.pb(p[0]);
                rep(_, y - 1) ans.pb('a' + k);
                rep2(i, 1, si(p) - 1) ans.pb(p[i]);
                OUT(ans);
            }
        }
    }
