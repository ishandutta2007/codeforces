...
int main() {
  ...
  for(int i = 0; i < N; i++) {
    ...
    int res = 0;
    pair<int, int> pos = sa.find(S, M);
    set<pair<int, int> > st;
    for(int i = M - 1; i >= 0; i--) {
      pair<int, int> r = sa.backstep(pos.first, S[i]);
      pos.first = r.first;
      pos.second = min(M, min(pos.second + 1, r.second));

      if(pos.second == M) {
        r = sa.range(pos.first, M);
        if(st.insert(r).second) {
