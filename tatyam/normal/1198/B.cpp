...
struct RchmQ : DualSegmentTree<T>{
    ...
    void c(T& a, const T& b){ chmax(a, b); }
    RchmQ(ll n, const T& def_value, const T& def_lazy = numeric_limits<T>::min()) : Base(n, def_value, def_lazy){}
    RchmQ(const vector<T>& v, const T& def_lazy = numeric_limits<T>::min()) : Base(v, def_lazy){}
};
int main(){
    LL(n);
    VEC(ll,a,n);
