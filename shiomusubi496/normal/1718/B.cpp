...
void solve() {
    ...
    vector<ll> f{1, 1};
    {
        ll a_sm = accumulate(all(a), 0ll);
        ll sm = 2;
        while (sm < a_sm) {
            ll t = f[f.size() - 2] + f[f.size() - 1];
            f.push_back(t);
            sm += t;
        }
        if (sm != a_sm) {
    ...
    while (!st.empty()) {
        ...
        if (y % 2 == 0) {
            ...
            s[p[0].second].push_back(y);
            a[p[0].second] -= x;
            if (a[p[0].second] != 0) que.push({a[p[0].second], p[0].second});
        }
        else {
            if (p.size() > 2 || (p.size() == 2 && p[0].first != x && p[1].first != x)) {
                puts("NO");
                return;
            }
            if (p.size() == 2) {
                if (p[0].first == x) swap(p[0], p[1]);
                if (p[0].first == x) {
                    if (s[p[0].second].size() > 0 && s[p[0].second].back() == y + 1) swap(p[0], p[1]);
                }
            }
            s[p[0].second].push_back(y);
            a[p[0].second] -= x;
            if (a[p[0].second] != 0) que.push({a[p[0].second], p[0].second});
            if (p.size() == 2) {
                for (ll i = 0; i < f.size() && f[i] <= x; i += 2) {
                    if (!st.count({f[i], i})) {
                        puts("NO");
                        return;
                    }
                    st.erase({f[i], i});
                    s[p[1].second].push_back(i);
                    a[p[1].second] -= f[i];
                }
            }
        }
