...
int main(){
	...
	while(T--)
	{
		...
		for(int i=1;i<=n;i++)if(a[i]==0)L.push_back(b[i]);
		for(int i=1;i<=n;i++)if(a[i]==1)R.push_back(b[i]);
		...
		if(l.size()==r.size())
		{
			for(int i=1;i<=n;i++)ans+=b[i],ans+=b[i];
			ans-=min(l[0],r[0]);
			cout<<ans<<"\n";
			continue;
		}
		if(l.size()<r.size())swap(l,r);
		for(auto it:r)ans+=it,ans+=it;
		reverse(l.begin(),l.end());
		int ct=r.size();
//		cerr<<ct<<"%\n";
		for(auto it:l)if((ct--)>0)ans+=it,ans+=it;else ans+=it;
		cout<<ans<<"\n";
	}
	pc('1',1);
	return 0;
