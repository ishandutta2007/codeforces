...
bool d[1 << K][N+1];
vector<int> good[N+1];
char isbranches[1 << K][N+1];
int bits[1 << K];
int rootc[1 << K];
bool is_correct(int mask, int ones)
{
	if (ones < 2) return false;
	if (mask == 0) return true;
	if (isbranches[mask][ones] != 0)
	{
		return isbranches[mask][ones] == 1;
	}
	int root = rootc[mask];
	for (int branch : good[root])
	{
		if ((branch & mask) != branch) continue;
		int to = mask ^ branch;
		int left = ones - (c[root] - bits[branch]);
		if (left < 0) continue;
		if (d[to][left] || is_correct(to, left))
		{
			isbranches[mask][ones] = 1;
			return true;
		}
	}
	isbranches[mask][ones] = -1;
...
int main()
{
	...
	for (int mask = 1; mask < N; ++mask)
	{
		...
		int one = c[root] - bits[mask];
		if (one > ones || one < 0) continue;
		if (is_correct(mask ^ (1 << root), one))
		{
			d[mask][one] = true;
			good[root].push_back(mask);
		}
		rootc[mask] = root;
	}
	if (d[N - 1][ones])
