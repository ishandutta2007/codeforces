...
signed main(){
    ...
    while(q--){
        ...
        ll ev1=0, od1=0;
        for(ll i = 0;i<n;i++){
            if(s[i]==t[i]) {even++;if(s[i]=='1') ev1++;}
            else {odd++;if(s[i]=='1') od1++;}
        }
        ll res = inf;
        if(even&1){
            //TOTAL OPS ODD, each even at least one time
            ll x = ev1, y = even-ev1, z = min(x, y);
            x-=z, y-=z;

            if(x==1) res=min(res, z*2+1);
        }
        if((odd%2==0)){
            //TOTAL OPS EVEN, each odd at least one time

            ll x = od1, y = odd-od1, z = min(x, y);
            x-=z, y-=z;

            if(!x && !y) res=min(res, z*2);
        }
        if(res<inf) cout<<res<<'\n';
        else cout<<"-1\n";
    }
