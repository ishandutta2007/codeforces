...
void solve() {
    cin >> n;
    vi pass(n);
    FOR(i, 2, n) pass[i] = query(0, 1, i);
    // all the same
    bool same = true;
    FOR(i, 3, n) if (pass[i] != pass[2]) same=false;
    if (same) {
        FOR(i, 3, n) pass[i] = query(0, 2, i);
        FOR(i, 3, n) if (pass[i] > pass[2]) {
            answer(2, i);
            return;
        }
        answer(0, 1);
    ...
    int mx = 2;
    FOR(i, 3, n) if (pass[i]>pass[mx]) mx = i;

    FOR(i, 1, n) if (i != mx) pass[i] = query(0, mx, i);
    same = true;
    FOR(i, 2, n) if (i != mx) if (pass[i] != pass[1]) same=false;
    if (same) {
        answer(0, mx);
        return;
    }
    int m2 = 1;
    FOR(i, 2, n) if (i != mx && pass[i]>pass[m2]) m2=i;
    answer(mx, m2);
}
