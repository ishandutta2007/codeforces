...
int dist[nax], ans[nax];
void dfs(int x) {
	static multiset<int> jumps;
	static int num;
	if (!jumps.empty() && fin[x])
		mini(dist[x], num + *jumps.begin());
	debug(arr(dist, x), get_word(x));
	int c = dist[x] - num + 1;
	jumps.insert(c);
	if (fin[x]) num++;
	for (int v : down[x]) {
		mini(dist[v], dist[x] + 1);
		dfs(v);
	}
	jumps.erase(jumps.find(c));
}
int main() {
	int n;
	scanf("%d", &n);
	...
	REP(i, n + 1) sort(ALL(down[i]), [](int x, int y){return c[x] < c[y];});
	debug(draw_tree(0, down, [](int x){string o(x ? 1 : 0, c[x]); if (fin[x]) o = "[" + o + "]_" + to_string(fin[x]); ros;}));
	memset(dist, 60, sizeof(dist));
	dist[0] = 0;
	dfs(0);
	for (int i = 0; i <= n; ++i) if (fin[i]) ans[fin[i]] = dist[i];
