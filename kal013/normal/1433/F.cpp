...
vector<long long> get_vector(const vector<int>& A, int k){
	const static int N = 72;
	static long long dp[N][N][N];
	int m = A.size();
	int allowed = (m / 2);
	...
	for(int i = 1; i <= m; ++i){
		...
		for(int j = 0; j <= allowed; ++j){
			for(int l = 0; l < k; ++l){
				...
				if (j > 0){
					dp[i][j][l] = max(dp[i][j][l], dp[i - 1][j - 1][(z + l) % k] + A[i - 1]);
				}
				ans[l] = max(ans[l], dp[i][j][l]);
			}
		}
	}
	return ans;
