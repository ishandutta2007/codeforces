...
void solve() {
    partial_sum(b,b+n,s+1);
    dump(vector<int>(b,b+n));
    dump(vector<int>(s,s+n+1));
    vector<pii> midx(1,make_pair(1000,-1));
    for(int r=0; r<n; ++r) {
        for(int i=0; i<2; ++i) cnt[i][r+1]=cnt[i][r];
        cnt[s[r]&1][r+1]++;
    }
    for(int x=0; x<n; ++x) {
        while(!midx.empty() && midx.back().fir<=b[x]) {
            midx.pop_back();
        }
        midx.emplace_back(make_pair(b[x],x));
        dump(x,midx.size());
        for(int i=(int)midx.size()-1; i>0; --i) {
            int crit=s[x+1]-midx[i].fir*2;
            int k=upper_bound(s,s+n+1,crit)-s;
            int l=midx[i-1].sec;
            int r=midx[i].sec;
            r=min(r+1,k);
            l=max(l+1,0);
            int h=s[x+1]&1;
            int add=max(0,cnt[h][r]-cnt[h][l]);
            ans+=add; 
            dump(midx[i],add);
        }
    }
