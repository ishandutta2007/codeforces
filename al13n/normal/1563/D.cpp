...
node nodealloc[1000000];
int nodecnt=0;

node*alloc(int v){
  assert(nodecnt<sizeof(nodealloc)/sizeof(nodealloc[0]));
...
void push(node*n){
  if(!n||!n->add) return;
  if(n->l){ n->l->v+=n->add; n->l->add+=n->add; }
  if(n->r){ n->r->v+=n->add; n->r->add+=n->add; }
  n->add=0;
}
...
void split(node*n,int v,node*&l,node*&r){
  if(!n){ l=r=0; return; }
  push(n);
  if(n->v<v){
    l=n;
...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    LL t=F[n*2-1-c]*iF[n]%mod*iF[n-1-c]%mod;
    cout<<t<<'\n';
  }
