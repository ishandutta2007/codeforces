...
struct Hash {
    static uint64_t splitmix64(uint64_t x) {
        ...
        return (((((x += A) ^= x >> 30) *= B) ^= x >> 27) *= C) ^= x >> 31;
    }
    static void combine(uint64_t& a, uint64_t b) { a ^= (b + 0x9e3779b9 + (a << 6) + (a >> 2)); }
    ...
    size_t operator()(const T& x) const noexcept {
        if constexpr (is_same_v<T, uint64_t>) {
            ...
            uint64_t h = 0;
            apply([&](auto&&... items) { (combine(h, (*this)(items)) , ...); }, x);
            return h;
        } else
            return hash<T>(x);
    }
...
template <class K, class V, bool FastGcc = true>
using HashMap = conditional_t<FastGcc,
                              __gnu_pbds::gp_hash_table<K, V, Hash>,
                              unordered_map<K, V, Hash>>;
template <class K, bool FastGcc = true>
using HashSet = conditional_t<FastGcc,
                              __gnu_pbds::gp_hash_table<K, __gnu_pbds::null_type, Hash>,
                              unordered_set<K, Hash>>;
#else
template <class K, class V, bool FastGcc = true>
using HashMap = unordered_map<K, V, Hash>;
template <class K, bool FastGcc = true>
using HashSet = unordered_set<K, Hash>;
#endif

HashMap<uint32_t, uint32_t> p;
uint32_t find(uint32_t x) { return p[x] = (x >= p[x] ? x : find(p[x])); }

int main() {
    int m; read(m);
    uint32_t ans = 0;
    while (m--) {
        uint32_t x, y; read(x, y);
        x = find(x), y = find(y);
        if (x > y) swap(x, y);
        ans = max(ans, x);
        if (ans == -1)
