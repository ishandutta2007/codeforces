...
void solve(){
  ...
  seg.build(vec(N,0)); seg2.build(vec(N,0));

  int cnt = 0;
  vector<int> b(N,-1);
  REP(i,N){
    if(a[i]==i) cnt++, seg.add(i,1), b[i] = i;
    else if(a[i]<i){
      int x = cnt-(i-a[i]);
      auto check = [&](int a){return a > x;};
      int id = seg.lower_find(0,N,check);
      if(id) seg.add(id-1,1), cnt++, b[i] = id-1;
    }
    // REP(j,N) cout << seg.query(j,j+1) << " "; cout << endl;
  }
  ...
  REP(i,Q){
    int l,r;
    cin >> l >> r;
    q[N-1-r].emplace_back(l,i);
  }

  vec ans(Q);
  REP(i,N){
    if(~b[i]) seg2.add(b[i],1);
    for(P p:q[i]) ans[p.sc] = seg2.query(p.fs,N);
  }
