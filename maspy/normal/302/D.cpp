...
void solve() {
  ...
  auto check = [&](ll init) -> bool {
    ...
    while (!que.empty()) {
      auto [have, v] = que.top();
      que.pop();
      if (DP[v] != have) continue;
      FOR(to, N) {
        if (v == to) continue;
        ll cost = abs(XY[v].fi - XY[to].fi) + abs(XY[v].se - XY[to].se);
        cost *= D;
        if (have < cost) continue;
        if (chmax(DP[to], have - cost + A[to])) que.push(mp(DP[to], to));
      }
    }
    return DP[N - 1] > 0;
  };

  ll ANS = binary_search(check, 1LL << 60, 0) - 1;
  print(ANS);
}
