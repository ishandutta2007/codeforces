...
void qqwq(int n)
{
	static int vis[1000005];
	memset(vis,0,sizeof(vis));
	std::vector<std::vector<int> >v[2][2];
	std::set<int>s[2];
	rg(n)s[a[i]].insert(i);gr
	std::set<int>g[2][2];
	static std::vector<int>all[1000005];
	for(int i=1;i<=n;i++)
	{
		int c=a[i];
		if(g[c^1][c^1].size())
		{
			auto p=*g[c^1][c^1].begin();
			g[c^1][c^1].erase(p);
			all[p].push_back(i);
			g[c^1][c].insert(p);
		}
		else if(g[c][c^1].size())
		{
			auto p=*g[c][c^1].begin();
			g[c][c^1].erase(p);
			all[p].push_back(i);
			g[c][c].insert(p);
		}
	...
	for(auto x:g[0][0])v[0][0].push_back(all[x]);
	for(auto x:g[0][1])v[0][1].push_back(all[x]);
	for(auto x:g[1][0])v[1][0].push_back(all[x]);
	for(auto x:g[1][1])v[1][1].push_back(all[x]);
	std::vector<int>ans;
	if(v[0][0].size()||v[1][1].size())
	{
		assert(v[0][0].size()+v[1][1].size()==1);
		if(v[0][0].size())
		{
			for(auto &x:v[1][1])for(auto y:x)ans.push_back(y);
			for(auto &x:v[0][1])for(auto y:x)ans.push_back(y);
			for(auto &x:v[0][0])for(auto y:x)ans.push_back(y);
			for(auto &x:v[1][0])for(auto y:x)ans.push_back(y);
		}
		else
		{
			for(auto &x:v[0][0])for(auto y:x)ans.push_back(y);
	...
	{
		if(v[1][0].size()&&v[0][1].size())
		{
			v[0][0].push_back(*v[0][1].rbegin());v[0][1].pop_back();
			v[1][1].push_back(*v[1][0].rbegin());v[1][0].pop_back();
			if(*v[0][0][0].rbegin()>*v[1][1][0].rbegin())v[1][1][0].push_back(*v[0][0][0].rbegin()),v[0][0][0].pop_back();
			else v[0][0][0].push_back(*v[1][1][0].rbegin()),v[1][1][0].pop_back();
		}
		for(auto &x:v[0][0])for(auto y:x)ans.push_back(y);
		for(auto &x:v[1][0])for(auto y:x)ans.push_back(y);
		for(auto &x:v[1][1])for(auto y:x)ans.push_back(y);
	...
	int res=0;
	for(int i=0;i+1<ans.size();i++)res+=ans[i]>ans[i+1];
	odl(res);
	for(auto x:ans)odb(x);
}
signed main()
{
	initprog();
...
// 		
	...
	// {
		...
		// int s=S<T;
		// odl(min(S,T));
		// // rg(n)if(p[i]!=q[i]){s=p[i]<q[i];break;}gr
		// int *t;if(s)t=q;else t=p;
		// rg(n)odb(t[i]);gr
	// }
	qqwq(n);
	return 0;
