...
ll dfs(int v, int p) {
	...
	vector<pair<ll, int>> zal;
	mx[v] = ht[v];
	for (auto u : g[v]) {
		if (u != p) {
			s.push_back({ dfs(u, v), u });
		}
	}
	sort(all(s));
	reverse(all(s));
	...
	to_return = max(to_return, s[0].x);
	if (sz(s) == 1) {
		dp[v] = dp[s[0].y] + max(0LL, ht[v] - s[0].x);
		mx[v] = to_return;
...
inline void _solve() {
	...
	ll mx = *max_element(ht + 1, ht + n + 1);
	for (int i = 1; i <= n; ++i) {
		if (ht[i] == mx) {
			dfs(i, -1);
			vector<pair<ll, int>> ans;
			for (auto u : g[i]) {
				ans.push_back({ ::mx[u], u });
			}
			sort(all(ans));
			reverse(all(ans));
			if (sz(g[i]) == 1) {
				...
				cout << dp[i] + max(0LL, ht[i] - ans[1].x) << endl;
			}
			//cout << dp[i] << endl;
			return;
		}
