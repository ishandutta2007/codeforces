...
void solve(){
  li n; cin>>n;
  li curl = 1e15, curr=-1e15;
  li costboth = 1e18;
  li costl = 1e18, costr = 1e18;
  for(li i = 0; i < (li)(n); ++i) {
    li l,r,c; cin>>l>>r>>c;
    if(l < curl){
      curl=l; costl=1e18; costboth=1e18;
    }
    if(r > curr) {
      curr=r; costr=1e18; costboth=1e18;
    }
    if(l==curl) { smin(costl,c); }
    if(r==curr) { smin(costr,c); }
    if(l==curl&&r==curr) { smin(costboth, c); }
    cout << min(costboth,costl+costr) << endl;
  }
}
