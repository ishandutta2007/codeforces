...
signed main(){
    read(n, m); int t = -1; read(a + 1, a + n + 1);
    rep(i, 1, n) chmax(t, a[i]), r[a[i]] = i;
    per(i, n, 1) l[a[i]] = i;
    rep(i, 1, n){
        if(!a[i]) {
            if(t < m) a[i] = (t = m);
            ...
            if(!s.empty() && *(-- s.end()) > a[i]) {print("NO"); return 0; }
            if(l[a[i]] == i) s.emplace(a[i]);
            if(r[a[i]] == i) s.erase(a[i]);
        }
    } if(t < m) {print("NO"); return 0; }
    print("YES"); print(a + 1, a + n + 1);
}