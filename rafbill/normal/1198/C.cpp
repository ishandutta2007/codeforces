...
void solve(){
  ...
  for(li i = 0; i < (li)(n); ++i) if(!E[i]) {
    for(auto e : G[i]) if(!E[e^xx]) {
        E[i]=1;
        E[e^xx]=1;
        mc.emplace_back(e^yy);
        break;
      }
  }

  vi is;
  for(li i = 0; i < (li)(n); ++i) if(!E[i]) is.push_back(i);

  if((int)mc.size() >= n/3) {
    cout << "Matching" << '\n';
    for(li i = 0; i < (li)(n/3); ++i) cout << mc[i]+1 << ' ';
    cout << '\n';
  }else{
    cout << "IndSet" << '\n';
    for(li i = 0; i < (li)(n/3); ++i) cout << is[i]+1 << ' ';
    cout << '\n';
  }
}
