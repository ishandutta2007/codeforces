...
void solve() {
	...
	//cout << st.query(0, 1) << "??\n";
	rep(i, n) {
		ll memo = st.query(a[i],a[i]+1)+min((ll)0,p[i]);
		if (exi[a[i]]) {
			memo = min(memo,st.query(pre[a[i]], pre[a[i]] + 1));
		}
		//cout << i << " " << a[i] << "\n";
		st.add(p[i], 0, a[i]);
		st.add(min(ll(0),p[i]),a[i] + 1, n + 1);
		//cout << i<<" "<<st.query(a[i], a[i]+1) << "\n";
		//cout << i << " " << memo << "\n";
		st.update(a[i], memo);
		//cout << i << " " << st.query(a[i], a[i] + 1) << "\n";
	}
	ll ans = st.query(b.back(), b.back() + 1);
	if (ans >= INF / 100) {
		cout << "NO\n";
	}
	else {
