...
int main()
{
    ...
    for (int i = 0; i<n; i++)
    {
        if (s[i]=='1')
        {
            cnt1++; pos.push_back(i);
        }
    }

    pos.push_back(n);

    if (k == 0 || cnt1<k) {cout<<1<<endl; return 0;}

    ll ans = 1;

    for (int i = 1; i<=cnt1; i++) ans = add(ans, pos[i+1] - pos[i-1] - 2);

    for (int i = 1; i<=cnt1; i++)
        for (int j = i+1; j<=min(cnt1, i+k-1); j++)
        {
            int ways = C(pos[j+1] - pos[i-1] - 1, j-i+1);
            ways = sub(ways, C(pos[j+1] - pos[(i+1)-1] - 1, j-(i+1)+1));
            ways = sub(ways, C(pos[(j-1)+1] - pos[i-1] - 1, (j-1)-i+1));
            ways = add(ways, C(pos[(j-1)+1] - pos[(i+1)-1] - 1, (j-1)-(i+1)+1));
            ans = add(ans, ways);
