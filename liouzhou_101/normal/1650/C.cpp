...
void solve()
{
	...
	for (int i = 0; i < m - 1; ++i)
	{
		if (res == L[i] + R[i + 1])
		{
			priority_queue<tuple<int, int, int>> ql, qr;
			for (int j = 0; j <= i; ++j)
			{
				auto [x, w] = v[j];
				ql.push({ w, x, j });
			...
			while (ql.size() > n) ql.pop();
			while (qr.size() > n) qr.pop();
			...
			for (int i = 0; i < n; ++i)
			{
				writeln(to[p[i].first] + 1, ' ', to[p[2 * n - i - 1].first] + 1);
			}
			break;
		}
	}
