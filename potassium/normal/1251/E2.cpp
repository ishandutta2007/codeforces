...
int main() {
	CASE(t) {
		int n; R(n);
		vector<pll> vote(n); R(vote);
		
		//sort(all(vote), [&](pll a, pll b) {if (a.F != b.F) return a.F < b.F; else return a.S > b.S;});
		sort(all(vote));
		...
		for (int i = n-1; i >= 0; i--) {
			if (vote[i].F <= i + buy) take.insert(vote[i].S);
			else {
				int top = *take.begin();
				if (top < vote[i].S) {
					take.erase(take.begin());
					take.insert(vote[i].S);
					ans += top;
				} else ans += vote[i].S;
				buy++;
			}
