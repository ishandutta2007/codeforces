...
int main() {
    ...
    ll sum = 0;
    rep(i, N)sum += a[i];
    if (sum < V) { cout << "NO" << endl; return 0; }
    ...
    if (dp[N][V%K]==0) { cout << "NO" << endl; return 0; }
    ...
    ll now = V % K;
    for (ll i = N - 1; i >= 0; i--) {
        if (dp[i][now] ==1) { nst.insert(i); continue; }
        now = ((now - a[i]) % K + K) % K;
        st.insert(i);
    }
    ...
    if (st.empty()) {
        rep(x, N) { if(a[x]>0)cout << (a[x]+K-1) / K << " " << x + 1 << " " << 1 << endl; }
        if(V>0)cout << V / K << " " << 1 << " " << 2 << endl;
        return 0;
    }
    else if (nst.empty()) {
        rep(x, N) { if(a[x]>0)cout << (a[x] + K - 1) / K << " " <<x + 1 << " " << 1 << endl; }
        if (sum > V) {
            cout << (sum - V) / K << " " << 1 << " " << 2 << endl;
        }
        return 0;
    }
    ...
    rep(x, N) {
        if (st.count(x)&&a[x]>0) {
            cout << (a[x] + K - 1) / K << " " << x + 1 << " " << c + 1 << endl;
            csum += a[x];
        }
        else if(nst.count(x)&&a[x]>0) {
            cout << (a[x]+K-1) / K << " " << x + 1 << " " << d + 1 << endl;
        }
    }
   
    if (csum > V) {
        cout << (csum - V) / K << " " << c + 1 << " " << d + 1 << endl;
    }
    if (csum < V) {
        cout << (V - csum) / K << " " << d + 1 << " " << c + 1 << endl;
    }
