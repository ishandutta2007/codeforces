...
//            println()
//            for(a in board){
//                println(a.map{if(it) '1' else '0'}.joinToString(""))
//            }
        }
        ...
        for(i in 0 until n-2){
            for(j in 0 until m){
                if(board[i][j]){
                    if(j != m-1){
                        add(intArrayOf(i,j,i+1,j,i+1,j+1))
...
//                    board[i+1][j] = !board[i+1][j]
        ...
        for(j in 0 until m-2){
            for(i in n-2 until n){
                if(board[i][j]){
                    add(intArrayOf(i,j,n-2,j+1,n-1,j+1))
//                    board[i][j] = false
//                    board[n-2][j+1] = !board[n-2][j+1]
//                    board[n-1][j+1] = !board[n-2][j+1]
                    output()
                }
            }
        }
        while(true){
            val yes = mutableListOf<Pair<Int,Int>>()
...
/*
dumbed, invert by operation first
 */