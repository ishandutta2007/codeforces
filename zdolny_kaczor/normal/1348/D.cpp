...
int main() {
	...
	REP(_, t) {
		...
		while (sum < n) {
			int left = n - sum;
			assert(left >= ans.back());
			if (left <= 2 * ans.back()) {
				ans.push_back(left);
			}
			else if (left >= 4 * ans.back()) {
				ans.push_back(2 * ans.back());
			}
			else {
				ans.push_back(max(ans.back(), ceil_div(left, 3)));
			}
			sum += ans.back();
		}
		debug(imie(ans), imie(n));
		printf("%d\n", (int) ans.size() - 1);
		for (int i = 0; i < (int) ans.size() - 1; ++i) printf("%d ", ans[i + 1] - ans[i]);
		printf("\n");
	}
	exit(0);
}
