...
vector<bool> done(N),visited(N);
vector<vector<pair<int,int>>> graf(N);
vector<pair<int,int>> ed;
int askE(vector<int> o){
	vector<int> c;
	for(auto p:o)
		c.pb(ed[p].f),c.pb(ed[p].s);
	sort(all(c));
	c.erase(unique(all(c)),c.end());
	return ask(c);
}
vector<int> takenEd;
void dfs(int tr,int target){
	if(takenEd.size()==target)
		return;
	visited[tr]=1;
	for(auto p:graf[tr]){
		if(visited[p.f]||done[p.s])
			continue;
		takenEd.pb(p.s);
		dfs(p.f,target);
		if(takenEd.size()==target)
...
int main()
{
	...
	int val=askE(edges);
	while(edges.size()!=1){
		vector<int> my=take(edges.size()/2);
		set<int> ok;
		for(auto p:my)
			ok.insert(p);
		vector<int> novi;
