...
void solve(){
	...
	while(!q.empty()){
		...
		if(u.second==1){
			if(ok[u.first][0]==1)
				for(auto j:s2){
					if(!vis[(u.first-j+n)%n][1]&&!--d[(u.first-j+n)%n][1]){
						q.push(pii((u.first-j+n)%n,2));
						ok[(u.first-j+n)%n][1]=0;
						vis[(u.first-j+n)%n][1]=1;
					}
				}
			else{
				for(auto j:s2){
					d[(u.first-j+n)%n][1]--;
					if(!vis[(u.first-j+n)%n][1]){
						q.push(pii((u.first-j+n)%n,2));
						ok[(u.first-j+n)%n][1]=1;
						vis[(u.first-j+n)%n][1]=1;
					}
				}
			}
			...
			if(ok[u.first][1]==1)
				for(auto j:s1){
					if(!vis[(u.first-j+n)%n][0]&&!--d[(u.first-j+n)%n][0]){
						q.push(pii((u.first-j+n)%n,1));
						ok[(u.first-j+n)%n][0]=0;
						vis[(u.first-j+n)%n][0]=1;
					}
				}
			else{
				for(auto j:s1){
					d[(u.first-j+n)%n][0]--;
					if(!vis[(u.first-j+n)%n][0]){
						q.push(pii((u.first-j+n)%n,1));
						ok[(u.first-j+n)%n][0]=1;
						vis[(u.first-j+n)%n][0]=1;
					}
				}
			}
