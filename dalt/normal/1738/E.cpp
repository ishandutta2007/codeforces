...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  Vec<i64> A = a;
  Vec<i64> B = b;
  HashMap<i64, int> a_map(N);
  HashMap<i64, int> b_map(N);
  for(int i = 1; i < N; i++) {
    A[i] += A[i - 1];
  }
  for(int i = N - 2; i >= 0; i--) {
  ...
  Vec<int> prev_pos(N);
  for(int i = 0; i < N; i++) {
    prev_pos[i] = i == 0 ? -1 : (a[i - 1] == 0 ? prev_pos[i - 1] : i - 1);
  }
  for(int i = N - 1; i >= 0; i--) {
    next_pos[i] = i == N - 1 ? N : (b[i + 1] == 0 ? next_pos[i + 1] : i + 1);
  }
  Debug(prev_pos);
  Debug(next_pos);
  math::Combination<Mi> comb(N + 10);
  Mi total = 1;
  Mi last = 0;
  for(int i = N - 1; i >= -1; i--) {
    if(i >= 0 && a[i] == 0) {
    ...
    i64 sum = i < 0 ? 0 : A[i];
    var right_iter = b_map.find(sum);
    if(right_iter == b_map.end()) {
      continue;
    }
    int j = right_iter->second;
    if(j - i <= 0) {
      continue;
    }
    int left_zero = i < 0 ? (a[0] != 0 ? 0 : next_pos[0]) : next_pos[i] - 1 - i;
    int right_zero = j >= N ? (a[N - 1] != 0 ? 0 : N - 1 - prev_pos[N - 1]) : j - 1 - prev_pos[j];
    Debug(left_zero);
    Debug(right_zero);
    //all in one block
    Mi local_way = 0;
    if(left_zero == j - 1 - i) {
      ...
      Mi way;
      if(i == -1) {
        way = comb.combination(left_zero + right_zero, left_zero);
      } else {
        way = comb.combination(left_zero + right_zero + 2, left_zero + 1) - 1;
      }
      local_way = total * way;
    }
    if (j - i == 1) {
