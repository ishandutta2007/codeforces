...
struct S {
    ...
    S(int total_, int lo_, int hi_) : total(total_), lo(lo_), hi(hi_) {}
    S(int v) : total(v) {
        lo = min(0, v);
        hi = max(0, v);
    }

    int ans() { return hi - lo + 1; };

    friend S operator+(const S &l, const S &r) {
        return {l.total + r.total, min(l.lo, l.total + r.lo), max(l.hi, l.total + r.hi)};
    };
};
...
void solve() {
    ...
    segment_tree st(n, S{0}, std::plus<S>());
    st.assign([&](int i) { return input[i] == '+' ? 1 : -1; });
    ...
    for (int query = 0; query < m; query++) {
        ...
        auto ans = st.accumulate(0, l) + st.accumulate(r, n);
        ps(ans.ans());
    }
}
