...
int main() {
	...
	vector<pii> itv(n + 1, { 1, k });
	for (int u = 1; u <= n; u++)
		if (r[u] != 0) itv[u].fi = itv[u].se = r[u];
		
	for (auto& u : order) {
		for (auto& v : g[u])
			chkmax(itv[u].fi, itv[v].fi + 1);
	}
	reverse(all(order));
	for (auto& u : order) {
		for (auto& v : rg[u])
			chkmin(itv[u].se, itv[v].se - 1);
	}

	for (int u = 1; u <= n; u++) {
		if (itv[u].fi > itv[u].se) no();
	}

	vector<bool> hasRank(k + 1, 0);
	for (int u = 1; u <= n; u++)
		hasRank[r[u]] = 1;
	
	vector<pair<pii, int>> sorted_itv;
	for (int u = 1; u <= n; u++)
		sorted_itv.push_back({ itv[u], u });
	sort(rall(sorted_itv));

	minheap<pii> pq;
	for (int now = 1; now <= k; now++) {
		while (!sorted_itv.empty() && sorted_itv.back().fi.fi <= now) {
			assert(sorted_itv.back().fi.fi == now);
			pq.push({ sorted_itv.back().fi.se, sorted_itv.back().se });
			sorted_itv.pop_back();
		}
