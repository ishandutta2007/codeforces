...
int main() {
	...
	const int MAXV = 1 << k;
	vector<Node> tree(MAXV << 1);

	for (auto& i : a) {
		string s = bitset<20>(i).to_string().substr(20 - k, k);
		int now = 1;
		for (auto& j : s)
			now = now * 2 + (j - '0');
			
		tree[now].minv = tree[now].maxv = 0;
	}

	vector<bool> swapped(MAXV, false);
	auto build = [&](int u) {
		int cur = 1 << (k - __lg(u) - 1);
		Node lc = tree[u * 2];
		Node rc = tree[u * 2 + 1];
		if (swapped[u]) swap(lc, rc);
		tree[u].minv = min(lc.minv, cur + rc.minv);
		tree[u].maxv = max(lc.maxv, cur + rc.maxv);
		int cross = cur + rc.minv - lc.maxv;
		tree[u].ans = min({ lc.ans, rc.ans, cross });
	};
	...
	ans[0] = tree[1].ans;
	for (int i = 1, j = 0; i < MAXV; i++) {
		int lowbit = i & -i;
		int b = __lg(lowbit);
		j ^= 1 << (k - 1 - b);

		for (int u = 1 << b; u < 1 << (b + 1); u++)
			swapped[u] = !swapped[u];

		for (int u = (1 << (b + 1)) - 1; u >= 1; u--)
			build(u);
