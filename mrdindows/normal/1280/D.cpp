...
vector<int> v[3030];
int a[3030];
pair<int, ll> dp[3030][3030];
int sz[3030];
bool used[3030];
using P = pair<int, ll>;
P merge(P a, P b, int t) {
    if (t == 0) {
        a.first += b.first;
        a.second += b.second;
    } else {
...
void go(int cur) {
    ...
    for (int to : v[cur]) {
        if (!used[to]) {
            go(to);
//            int newSz = sz[cur] + sz[to];
            for (int x = sz[cur]; x > 0; --x) {
                auto dpx = dp[cur][x];
                dp[cur][x] = {0, -LLINF};
                for (int y = sz[to]; y > 0; --y) {
                    auto pa = merge(dpx, dp[to][y], 0);
                    auto pb = merge(dpx, dp[to][y], 1);
//                    DBN(pa, pb, dpx, dp[to][y]);
                    dp[cur][x + y - 1] = max(dp[cur][x + y - 1], pa);
                    dp[cur][x + y] = max(dp[cur][x + y], pb);
                }
            }
            sz[cur] += sz[to];
        }
