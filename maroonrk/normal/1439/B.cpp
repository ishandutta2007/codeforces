...
void slv(){
	...
	while(si(pq)){
		...
		if(deg[v]!=d)continue;
		if(d>=k){
			rep(i,n)if(alive[i])ans.pb(i);
			output(1);
			return;
		}
		for(auto to:g[v])if(alive[to]){
			ans.pb(to);
		}
		if(d==k-1){
			assert(si(ans)==k-1);
			...
			rep(i,si(ans)){
				rng(j,i+1,si(ans)){
					if(es.find(pi(ans[i],ans[j]))==es.ed){
						ok=false;
						break;
					}
				}
		...
		alive[v]=false;
		for(auto to:ans)pq.emplace(--deg[to],to);
		ans.clear();
	}
	pr.writeln(-1);
}
