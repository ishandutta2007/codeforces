...
struct Node{
	int sum, mi_presum, mx_presum;
};
Node t[TREE];
il Node operator+( const Node &x , const Node &y ){
	return {
		x.sum + y.sum,
		min( x.mi_presum , x.sum + y.mi_presum ),
		max( x.mx_presum , x.sum + y.mx_presum )
	};
}
#define LSON l,l+r>>1,pos<<1
#define RSON (l+r>>1)+1,r,pos<<1|1
#define ROOT 1,1048575,1
...
void Modify( int gpos , int x , ARG ){
	...
	int mid = l+r >> 1;
	gpos <= mid ? Modify(gpos,x,LSON) : Modify(gpos,x,RSON);
	Update(pos);
}

void Query(){
	int mi = t[1].mi_presum, mx = t[1].mx_presum;
	if( mi < 0 or t[1].sum != 0 ) printf("-1 ");
	else{
		printf("%d ",mx);
	}
}
