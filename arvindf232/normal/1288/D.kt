...
fun main(){
    solve.cases{
        val n = getint()
        val m = getint()
        val L = Array(n){getline(m)}

        fun solve(need:Int):Pair<Int,Int>? {
            ...
            val counts = BooleanArray(1 shl m)
            for(i in masks){
                counts[i]= true
            }
            for(x in 0 until (1 shl m)){
                for(y in 0 until ( 1 shl m)){
                    if(x or y != ((1 shl m) - 1 )) continue
                    if(counts[x] && counts[y]){
                        val first = masks.indices.first{masks[it] == x}
                        val second = masks.indices.first{masks[it] == y}
        ...
        val ans = BinarySearchLastTrue(-1,1_000_000_003){need ->
            solve(need) != null
        }!!
        val final = solve(ans)!!
        put(final.first + 1 )
