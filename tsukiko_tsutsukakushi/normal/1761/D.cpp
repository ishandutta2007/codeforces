...
mint H(int sum, int cnt) {
    return bc.com(sum + cnt - 1, sum);
}   

void solve() {
    otera::pow_table pw(mint(3));
    INT(n, k);
    mint ans = 0;
    if(k == 0) {
        ans = pw[n];
    ...
    for(int x = 1; x <= k and n - 2 * x >= 0; ++ x) {
        ans += pw[n - 2 * x] * H(k - x, x) * H(n - k - x, x + 1);
    }
    for(int x = 1; x <= k and n - 2 * x + 1 >= 0; ++ x) {
        ans += pw[n - 2 * x + 1] * H(k - x, x) * H(n - k - x + 1, x);
    }
    out(ans);
}
