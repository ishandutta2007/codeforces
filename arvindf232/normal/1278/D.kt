...
data class interval(val id:Int, val right:Int):Comparable<interval>{
...
fun main(){
    solve.cases{
       ...
       var bad = false
        ...
        val s = TreeSet<interval>()
        outer@for(i in scan.indices){
            val here = scan[i]
            if(here > 0){

                for(other in s){
                    if(other.right > right[here]){
                        break
                    }else{
                        val doit = dsu.union(other.id,here)
                        if(!doit){
                            bad = true
                            break@outer
                        }
                ...
                s.add(interval(here,right[here]))
            }else{
                val v = - here
                s.remove(interval(v,i))
            }
        }
        if(dsu.successfulUnions != n-1){
            bad = true
        }
        if(bad){
            put("NO" )
