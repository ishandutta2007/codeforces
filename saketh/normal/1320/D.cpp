...
int main() {
    ...
    F0R (i, N) {
        if (t[i] == '0') {
            hashes.pb(f(c0, hashes.back()));
        } else {
            hashes.pb(f(c1, hashes.back()));
        }
    }

    auto get = [&](int i, int len) {
        return f(hashes[i + len], inv(hashes[i]));
    };
