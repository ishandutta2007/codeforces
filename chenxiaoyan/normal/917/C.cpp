...
int f[N];
int tot, msk[N], id[1 << 8];

struct matrix {
    int a[N][N];
    int *operator[](int x) { return a[x]; }
    matrix(int v = inf) {
        REP(i, 1, N - 1) REP(j, 1, N - 1) a[i][j] = i == j ? v : inf;
    }
    friend matrix operator*(matrix f, matrix g) {
        matrix h;
        REP(i, 1, tot) REP(j, 1, tot) REP(k, 1, tot)
            chkmn(h[i][j], f[i][k] + g[k][j]);
        return h;
    }
    void deal(int *f) {
        static int g[N];
        memset(g, 0x3f, sizeof(g));
        REP(i, 1, tot) REP(k, 1, tot) chkmn(g[i], a[i][k] + f[k]);
        REP(i, 1, tot) f[i] = g[i];
    }
} pw[30];
...
int now;
void deal(int x) {
    REP(i, 0, 29) if(x >> i & 1) pw[i].deal(f);
    now += x;
}
...
void mian() {
    ...
    REP(i, 1, q) {
        int p = read(), wp = read();
        w[i] = mt(p, wp);
        REP(j, max(1ll, p - K), p - 1) spc.pb(j);
    }
    sort(ALL(spc)), spc.resize(unique(ALL(spc)) - spc.begin());
    REP(i, 0, (1 << K) - 1) if(__builtin_popcount(i) == X) {
        id[i] = ++tot, msk[tot] = i;
    }
    REP(i, 1, tot) {
        int msk = ::msk[i], x = __builtin_ctz(msk);
        if(x != 0) pw[0][id[msk >> 1]][i] = 0;
        else {
            REP(j, 1, K) if(~msk >> j & 1)
                pw[0][id[(msk >> 1) | (1 << (j - 1))]][i] = c[j];
        }
    }
    ...
    for(int S : spc) {
        ...
        matrix d;
        auto fd = [&](int x) {
            REP(i, 1, q) if(X(w[i]) == x) return Y(w[i]);
            return 0ll;
        };
        REP(i, 1, tot) {
            int msk = ::msk[i], x = __builtin_ctz(msk);
            if(x != 0) d[id[msk >> 1]][i] = 0;
            else {
                REP(j, 1, K) if(~msk >> j & 1)
                    d[id[((msk ^ 1) >> 1) | (1 << (j - 1))]][i] = c[j] + fd(S + j);
            }
        }
        d.deal(f);
        ++now;
