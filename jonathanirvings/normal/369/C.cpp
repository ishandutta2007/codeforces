...
void dfs(int now)
{
	REP(i,SIZE(adj[now])) if(adj[now][i].fi != parent[now])
	{
		parent[adj[now][i].fi] = now;
		height[adj[now][i].fi] = height[now] + 1;
		if (adj[now][i].se == 2) repair.pb(adj[now][i].fi);
		dfs(adj[now][i].fi);
	}
}
...
int main()
{
	...
	sudah[0] = 1;
	sort(ALL(repair),f);
	REP(i,SIZE(repair)) if(!sudah[repair[i]])
	{
		risan.pb(repair[i]);
		int x = repair[i];
		while (!sudah[x])
