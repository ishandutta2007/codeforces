...
const int MAX_N = 1005, MAX_K = 60;
vector<vector<mint>> dp(MAX_N, vector<mint>(MAX_K, 0)), ndp(MAX_N, vector<mint>(MAX_K, 0));
...
void solve() {
    ...
    mint ans = 0;
    for (int i = 0; i <= n - k; i++) ans += dp[i][k] * COM.C(n - i, k);
    ans *= COM.fac(k);
    ...
    for (int k = 0; k < MAX_N; k++) {
        for (int i = 0; i < MAX_N; i++) {
            for (int j = 0; j < MAX_K; j++) {
                ndp[i][j] += dp[i][j];
                if (i + k < MAX_N && j + 1 < MAX_K) ndp[i + k][j + 1] += dp[i][j];
                dp[i][j] = 0;
            }
        }
        swap(dp, ndp);
