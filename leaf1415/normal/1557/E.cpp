...
int main(void)
{
	...
	while(T--){
		...
		ll x = 2, cnt; bool left = false;
		rep(i, 2, 7){
			cin >> res;
			if(res == "Done") goto done;
			/*if(res == "Down" || res == "Down-Right" || res == "Down-Left"){
				outl(i+1, x);
				continue;
			}*/
			...
			while(1){
				...
				cin >> res;
				if(res == "Done") goto done;
				if(res == "Up" || res == "Up-Left" || res == "Up-Right") cnt = 0;
				else cnt++;
				if(cnt >= 10 || res == "Down" || res == "Down-Right" || res == "Down-Left"){
					outl(i+1, x);
					break;
				}
			}
