...
int main() {
	...
	wne(stk) {
		...
		sum += 2 * depth[cur];
		forin(i, cons[cur]) {
			depth[cons[cur][i]]=1+ depth[cur];
			stk.push_back(cons[cur][i]);
		}
	}
	veci color(n);
	forin(i, cons[mid]) {
		color[cons[mid][i]] = cons[mid][i];
	}
	forn(i, n) {
		int cur = ord2[i];
		forin(i, cons[cur]) {
			color[cons[cur][i]] = color[cur];
		}
	}
	color[mid] = mid;
	vector<veci> allcolor(n);
	fori(i, n) {
		allcolor[color[i]].push_back(i);
	}
	set<point> s;
	fori(i, n) {
		s.insert({ (lol)0-allcolor[i].size(),i });
	}
	if (n & 1) {
		s.erase({ -1,mid });
	}
	...
	fori(i, n / 2) {
		...
		int b = it->second;
		s.erase(it);
		goal[allcolor[a].back()] = allcolor[b].back()+1;
		goal[allcolor[b].back()] = allcolor[a].back()+1;
		allcolor[a].pop_back();
		allcolor[b].pop_back();
		s.insert({ (lol)0-allcolor[a].size(),a });
		s.insert({ (lol)0-allcolor[b].size(),b });
	}
	if (n & 1) {
		int mid2 = (mid + 1) % n;
		goal[mid] = goal[mid2];
