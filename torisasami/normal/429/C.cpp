...
int main() {
    ...
    queue<pair<vector<int>, pii>> que, nque;
    vector<int> li0(n, 0);
    li0[0] = c1;
    que.push({li0, {1, 0}});
    rep(t, sz) {
        ...
        map<pair<vector<int>, pii>, bool> m;
        while (!que.empty()) {
            auto q = que.front();
            que.pop();
            if (m[q])
                continue;
            m[q] = true;
            bool f = q.second.second;
            if (!f) {
                q.second.first = d[t];
                q.second.second = n;
            }
            auto li = q.first;
            auto [tar, prev] = q.second;
            rep2(i, min(tar - 1, prev)) {
                if (i + 1 == tar - 1 && !f)
                    continue;
                if (!li[i])
                    continue;
