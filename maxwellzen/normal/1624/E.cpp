...
void solve() {
    cin >> n >> m;
    l.clear(); r.clear(); x.clear(); dict.clear();
    F0R(i, n) {
        cin >> s;
        FOR(d, 2, 4) F0R(j, m-d+1) {
            str cur = s.substr(j, d);
            if (dict.find(cur) == dict.end()) {
                dict[cur] = l.size();
                l.pb(j+1); r.pb(j+d); x.pb(i+1);
            }
        }
    }
    cin >> s;
    dp.assign(m+1, -1); dp[m] = 0;
    cnt.assign(m+1, 0);
    R0F(i, m-1) {
        if (i+2<=m && dp[i+2]!=-1 && dict.find(s.substr(i, 2)) != dict.end()) {
            dp[i] = dict[s.substr(i, 2)];
            cnt[i] = cnt[i+2]+1;
        } else if (i+3<=m && dp[i+3]!=-1 && dict.find(s.substr(i, 3)) != dict.end()) {
            dp[i] = dict[s.substr(i, 3)];
            cnt[i] = cnt[i+3]+1;
        }
    }
    if (dp[0]==-1) cout << -1 << nl;
