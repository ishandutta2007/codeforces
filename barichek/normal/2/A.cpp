...
int main()
{
    vector< pair<string,int> > a(1000,mp("",0));
    map<string,int> cnt;
    map<string,bool> winer;
    map<string,bool> use;
    int n;
    read(n);
    for (int i=0;i<n;i++)
    ...
    int ans=-maxint;
    for (int i=0;i<n;i++)
        ans=max(ans,cnt[a[i].fir]);
    for (int i=0;i<n;i++)
        if (!use[a[i].fir]&&cnt[a[i].fir]==ans) use[a[i].fir]=true, winer[a[i].fir]=true;
    cnt.clear();
    for (int i=0;i<n;i++)
    {
        cnt[a[i].fir]+=a[i].sec;
        if (cnt[a[i].fir]>=ans&&winer[a[i].fir]) return writeln(a[i].fir), 0;
    }
}