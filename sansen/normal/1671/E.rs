fn run() {
    ...
    let mut s = s;
    s.insert(0, 0);
    let mut dp = vec![M::one(); 1 << n];
    let mut key = vec![vec![]; 1 << n];
    for i in (1..(1 << n)).rev() {
        if 2 * i >= s.len() {
            key[i] = vec![s[i]];
            continue;
        }
        let mut a = key[2 * i].clone();
        let mut b = key[2 * i + 1].clone();
        key[i].push(s[i]);
        if a == b {
            dp[i] = dp[2 * i] * dp[2 * i];
        } else {
            dp[i] = M::new(2) * dp[2 * i] * dp[2 * i + 1];
        }
        if a > b {
            std::mem::swap(&mut a, &mut b);
        }
        key[i].extend(a.iter().cloned());
        key[i].extend(b.iter().cloned());
    }
    println!("{}", dp[1]);
}
