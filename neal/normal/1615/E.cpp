...
int main() {
    ...
    int n_leaves = int(leaves.size());

    if (K >= n_leaves) {
        int64_t red = min(K, max(N / 2, n_leaves));
        int64_t white = N - red;
        cout << white * red << '\n';
        return 0;
    }

    sort(leaves.begin(), leaves.end(), [&](int a, int b) -> bool {
    ...
    for (int leaf : leaves) {
        ...
        for (int node = leaf; node >= 0 && !covered[node]; node = lca.parent[node]) {
            covered[node] = true;
            count++;
        }

        cover_cnt.push_back(count);
    }

    nth_element(cover_cnt.begin(), cover_cnt.begin() + K, cover_cnt.end(), greater<int>());
    int max_blue = N;

    for (int k = 0; k < K; k++)
        max_blue -= cover_cnt[k];

    int64_t red = K;
    int64_t worst = red * (N - red);

    for (int64_t blue = 0; blue <= max_blue; blue++)
