...
unsigned int a[101010];
unsigned int b[101010];
int32_t solve() {
    int q;
    cin >> q;
    ll checkAns = 0;
    ...
    FI(q) {
        ...
        if (t == 1) {
            ...
            l = rand() % 1000;
            r = 100000 - rand() % 1000;
            k = rand() % 1000000000 - 500000000;
#else
            cin >> l >> r >> k;
#endif
            unsigned int t = k > 0 ? k : -k;
            auto* arr = k > 0 ? a : b;
            arr += l;
            int n = r - l;
            while (n % 16) {
            ...
            int n4 = n / 4;
            while (n4--) {
#define F(i) arr[i] = arr[i] > t ? t : arr[i];
                F(0); F(1); F(2); F(3);
                arr += 4;
            }
        } else {
            ...
            int c4 = cnt / 4;
            while (c4--) {
                #define F(i) ans += (ap[i] + bp[i]) & ~(((int32_t)(ap[i] | bp[i]) >> 31));
                F(0); F(1); F(2); F(3);
                ap += 4;
                bp += 4;
            }
