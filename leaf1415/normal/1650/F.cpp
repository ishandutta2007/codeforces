...
int main(void)
{
	...
	while(T--){
		...
		rep(e, 1, n){
			ll M = sz(vec[e]);
			rep(i, 0, M) rep(j, 0, 100) dp[i][j] = inf;
			dp[0][0] = 0;
			...
			rep(i, 0, M-1){
				...
				rep(j, 0, 100){
					...
					ll nj = min(100LL, j+p[id]);
					if(dp[i+1][nj] > dp[i][j] + t[id]) dp[i+1][nj] = dp[i][j] + t[id], pre[i+1][nj] = j;
				}
			}
			ft[e] = dp[M][100];
