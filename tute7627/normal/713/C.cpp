...
//https://github.com/beet-aizu/library/blob/master/datastructure/slope.cpp
template<typename T>
struct Slope{
  using P = pair<T, T>;
...
int main(){
  ...
  rep(i,0,n){
    cin>>a[i];
    a[i]-=i;
    slope.add_a_minus_x(a[i]);
    slope.add_x_minus_a(a[i]);
    slope.apply_cumulative_min();
  }
  cout<<slope.entire<<endl;
  return 0;
}