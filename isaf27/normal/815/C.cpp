...
vector<int> dfs(int p)
{
    ...
    for (int i : v[p])
    {
        vector<int> u = dfs(i);
        vector<int> t = in;
        in.resize(t.size() + u.size());
        merge(t.begin(), t.end(), u.begin(), u.end(), in.begin());
        kol[p] += kol[i];
    }
    vector<ll> sb(v[p].size() + 1);
    sb.resize(1);
    sb[0] = 0;
    for (int i = 0; i < v[p].size(); i++)
    {
        int g = v[p][i];
        vector<ll> nw(sb.size() + kol[g]);
        for (int x = 0; x < nw.size(); x++)
        {
            nw[x] = INF;
            for (int pp = max(0, x - (int)sb.size() + 1); pp <= min(x, kol[g]); pp++)
                nw[x] = min(nw[x], sb[x - pp] + dp[g][pp]);
        }
        sb = nw;
    }
    dp[p][0] = 0;
