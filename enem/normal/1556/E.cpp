...
int main()
{
    ...
    {
        bl[i][0][0] = a[i];
        bl[i][0][1] = max(0LL, a[i]);
        bl[i][0][2] = min(0LL, a[i]);
    }
    REP(x,1,18)
    {
        REP(i,0,n-(1LL<<(x-1)))
        {
            bl[i][x][0] = bl[i][x-1][0] + bl[i+(1LL<<(x-1))][x-1][0];
            bl[i][x][1] = max(bl[i][x-1][1], bl[i][x-1][0] + bl[i+(1LL<<(x-1))][x-1][1]);
            bl[i][x][2] = min(bl[i][x-1][2], bl[i][x-1][0] + bl[i+(1LL<<(x-1))][x-1][2]);
        }
    }
    while(q--)
    {
        ...
        REP(j,0,18)
        {
            if(r&(1LL<<j))
            {
                t[1] = max(t[1], t[0] + bl[l][j][1]);
                t[2] = min(t[2], t[0] + bl[l][j][2]);
                t[0] += bl[l][j][0];
                l+=(1<<j);
            }
        }
