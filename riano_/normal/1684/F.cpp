...
int main(){
    ...
    ll T; cin >> T;
    rep(ii,T){
        ll N,M; cin >> N >> M; ans = 2e9;
        vector<ll> a(N),pv(N,-1); map<ll,ll> app;
        rep(i,N){
            cin >> a[i];
            if(app.count(a[i])) pv[i] = app[a[i]];
            app[a[i]] = i;
        }
        vector<vector<ll>> span(N+1);
        rep(i,M){
            ll l,r; cin >> l >> r;
            span[l-1].emplace_back(r);
        }
        queue<Pr> from;
        vector<bool> ban(N,false);
        vector<ll> back(N,2e9);
        rep(i,N){
            for(ll x:span[i]){
                from.emplace(x,i);
            }
        ...
        //     cout << ban[i] << endl;
        // }
        ll l_tmp = 2e9,r_tmp = 0;
        multiset<ll> lt,rt; lt.insert(2e9); rt.insert(0);
        vector<tuple<ll,ll,ll,ll>> ran; 
        for(auto[x,p]:v){
            // cout << x << endl;
            // out(p);
            // cout << endl;
            ll K = p.size();
            if(back[p[K-1]]>p[0]){
                chmax(r_tmp,p[K-2]+1); chmin(l_tmp,p[1]);
            }
            else{
                ran.emplace_back(p[0],p[K-2]+1,p[1],p[K-1]+1);
                lt.insert(p[0]); rt.insert(p[K-2]+1);
            }
        }
        sort(all(ran));
        auto itr3 = rt.end(); itr3--; auto itr4 = lt.begin();
        chmin(ans,max(r_tmp,*itr3)-min(l_tmp,*itr4));
        //cout << *((rt.end())--) << " " << *(lt.begin()) << endl;
        for(auto[x,y,z,w]:ran){
            //cout << x << " " << y << " " << z << " " << w << endl;
            lt.erase(lt.find(x)); lt.insert(z);
            rt.erase(rt.find(y)); rt.insert(w);
            auto itr = rt.end(); itr--; auto itr2 = lt.begin();
            chmin(ans,max(r_tmp,*itr)-min(l_tmp,*itr2));
            //cout << *itr << " " << *(lt.begin()) << endl;
        }
