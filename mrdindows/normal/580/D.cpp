...
int main() {
    ...
    FI(k) {
        ...
        --rules[i].a; --rules[i].b;
        s[rules[i].a][rules[i].b] = rules[i].c;
    }
    ll ans = 0;
    for (int mask = 1; mask < (1 << n); ++mask) {
        ...
        if (bits_count(mask) == 1) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                int t = mask ^ (1 << i);
                for (int p = 0; p < n; ++p) {
                    if (!checkbit(t, p)) continue;
                    d[i][mask] = max(d[i][mask], a[i] + s[p][i] + d[p][t]);
                }
            }
        }
        if (bits_count(mask) == m) {
            for (int i = 0; i < n; ++i) {
                if (checkbit(mask, i)) {
                    ans = max(ans, d[i][mask]);
                }
            }
        }
    }
