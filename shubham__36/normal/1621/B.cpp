...
void solve()
{
	...
	forn(i,n){
		...
		if(!i){
			...
			if(l1 < vec[0].fi || (l1==vec[0].fi && c<ans[0])){
				vec[0]=mp(l1,r1);
				ans[0]=c;
			}
			if(r1 > vec[1].se || (r1==vec[1].se && c<ans[1])){
				vec[1]=mp(l1,r1);
				ans[1]=c;
			}
			if(r1-l1 > rn.se-rn.fi || (r1-l1 == rn.se-rn.fi && c<ans1)){
				rn=mp(l1,r1);
				ans1=c;
			}
		}
		ll cans = ans[0]+ans[1];
		if(rn.fi==vec[0].fi && rn.se==vec[1].se) cans = min(cans,ans1);
		cout << cans << ln;
	}
}
