...
ll n, m, k, l, r, x, y;
ll summ, ans;
ll partial_summ[600002];
ll nexts[600002];
vector<ll> tret, tret2;
...
int main()
{
    ...
    {
        cin >> k;
        partial_summ[i + 1] = partial_summ[i] + k;
        summ += k;
    }
    if (summ % 3 != 0)
    {
    ...
    for (int i = 1; i < n; i++)
    {
        if (partial_summ[i] == summ / 3)
        {
            nexts[i] = 1;
            tret.pb(i);
        }
        if (partial_summ[i] == 2 * summ / 3)
        {
            nexts[i] += 2;
            tret2.pb(i);
        }
    ...
    int ch = 0;
    for (int i = 0; i < (int)tret.size(); i++)
    {
        int u = tret[i];
        while (ch < (int)tret2.size() && tret2[ch] <= u)
            ch++;
        ans += (int)tret2.size() - ch;
    }
    cout << ans;
    //for (int i = 0; i < n + 1; i++)
    //{
