...
int main(void)
{
	...
	pos[r] = d;
	vec[d].pb(inf), vec[d].pb(r);
	
	rep(i, 0, d){
		sort(all(vec[i])), reverse(all(vec[i]));
		seg.set(i, P(n-2*(sz(vec[i])-1), i));
		if(i == d) seg.set(i, P(n-1, i));
		seg2.set(i, P(vec[i].back(), i));
	}
	seg.setup(), seg2.setup();
	
	/*outl(r);
	rep(i, 0, d) cout << seg.query(i, i) << " "; cout << endl;
	rep(i, 0, d) cout << seg2.query(i, i) << " "; cout << endl;*/
	...
	rep(i, 1, n){
		...
		if(res.fst == 0) jd = res.snd;
		else{
			P res = min(seg2.query(0, id-1), seg2.query(id+1, d));
			if(id == d) chmin(res, seg2.query(id, id));
			jd = res.snd;
		}
		seg.add(0, d, -1), seg.add(jd, jd, 1);
		p[i] = vec[jd].back();
		vec[jd].pop_back();
		seg2.update(jd, P(vec[jd].back(), jd));
	}
	
	ll ans = 0;
	rep(i, 1, n) ans += lca.getDist(i, p[i]);
	outl(ans);
	outa(p, 1, n);
