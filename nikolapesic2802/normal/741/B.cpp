...
vector<int> sumW(N),sumB(N);
int w[N],b[N];
int dp[N][N];
int W,n;
int calc(int a,int j){
	if(j>W)
		return INT_MIN;
	if(a==n)
		return 0;
	if(dp[a][j]!=-1)
		return dp[a][j];
	dp[a][j]=calc(a+1,j);
	dp[a][j]=max(dp[a][j],calc(a+1,j+sumW[a])+sumB[a]);
	for(auto p:group[a])
		dp[a][j]=max(dp[a][j],calc(a+1,j+w[p])+b[p]);
	return dp[a][j];
}
int main()
{
	...
	for(int i=0;i<n;i++)
		group[find(i)].pb(i),sumW[find(i)]+=w[i],sumB[find(i)]+=b[i];
	printf("%i\n",calc(0,0));
	return 0;
}