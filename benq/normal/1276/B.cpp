...
int DFS(int x, int y) {
	FOR(i,1,2*n+1) pre[i] = 0;
	pre[x] = pre[y] = -1;
	dfs(x);
	int ret = 0;
	FOR(i,1,n+1) if (pre[i] > 0) ret ++;
	return ret;
}
...
void solve() {
	...
	trav(t,B.bccs) {
		set<int> S;
		trav(u,t) S.insert(B.ed[u].f), S.insert(B.ed[u].s);
		co ++;
		trav(z,S) ADJ[z].pb(co), ADJ[co].pb(z);
	}
	pre[a] = -1; dfs(a);
	vi path;
	while (b != a) {
	...
	path.pb(b); // ps("HUH",path);
	int x = DFS(path[0],path[1]), y = DFS(path.back(),path[sz(path)-2]);
	ps((ll)x*y);
}

int main() {
	cin.sync_with_stdio(0); cin.tie(0); re(t);
	F0R(i,t) solve();
