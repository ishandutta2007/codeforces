...
bool dfs(int x, int d, int tar) {
	...
	for (int v : graf[x]) if (dfs(v, d + 1, tar)) {
		path.push_back(x);
		return true;
	}
	return false;
...
bool try_path() {
	for (int i = 1; i <= n; ++i) random_shuffle(ALL(graf[i]));
	memset(seen, 0, sizeof(seen[0]) * (n + 10));
	int x = rand() % n + 1;
	bool r = dfs(x, 0, ceil_div(n, 2));
	if (r) {
		printf("PATH\n%d\n", (int) path.size());
...
bool con(int a, int b) {
	for (int x : graf[a]) if (x == b) return true;
	return false;
}
bool try_pairs() {
	vi ord(n);
	...
	vpii got;
	memset(where, -1, sizeof(where[0]) * (n + 10));
	REP(i, n / 2) {
		int a = ord[i * 2], b = ord[i * 2 + 1];
		if (con(a, b)) continue;
		vi is;
		for (int c : {a, b}) for (int x : graf[c]) if (where[x] != -1) is.push_back(where[x]);
		sort(ALL(is));
		int si = is.size();
		bool good = true;
		for (int i = 0; i + 2 < si; ++i) if (is[i] == is[i + 2]) {
			good = false;
			break;
		}
		if (good) {
	...
	if (2 * (int)got.size() >= ceil_div(n, 2)) {
		printf("PAIRING\n%d\n", (int) got.size());
		for (auto [a, b] : got) printf("%d %d\n", a, b);
		return true;
	}
	return false;
}
