...
int main(){
    ...
    rep(zz,t){
        ...
        cout << res + min({a[n-1][n],a[n][n-1],a[0][n],a[n][0],a[2*n-1][0],a[0][2*n-1],a[n-1][2*n-1],a[2*n-1][n-1]}) << "\n";
        // vv(ll,d,2*n,2*n,(1LL<<60));
        ...
        //         int ni = (i+di + 2*n)%(2*n);
        //         int nj = j; 
        //         if(chmin(d[ni][nj],d[i][j] + a[ni][nj])){
        //             pq.push(MT(d[ni][nj],ni,nj));
        //         }
        //     }
        //     srep(dj,-1,2){
        //         int nj = (j+dj + 2*n)%(2*n);
