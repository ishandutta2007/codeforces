...
int main() {
    ...
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        tk[i] = max((ll)0, a * z[i] + c - (min(a * (z[i] + 1), b) + a));
        if (tk[i] > 12345678901) {
            tk[i] = 12345678901;
            f[i] = true;
        }
    }
    dp[2] = max((ll)0, dp[0] - d + tk[1]);
    for (int i = 2; i < n+1; i++) {
        dp[i] = max({ dp[i], dp[i - 1],dp[i - 2] + tk[i - 2] + tk[i-1] - 2 * d ,dp[i-1]+tk[i-1]-2*d});
    }
    for (int i = 0; i < n; i++) {
        ll u = a * z[i] + c;
        if (f[i])u = 12345678901 + (min(a * (z[i] + 1), b) + a);
        ans += u;
    }
    ans += (n - 1) * d;
    ans -= dp[n];
