...
bs b[26], ans; char s[MAXN], t[MAXN];
signed main(){
    read(s + 1); n = strlen(s + 1); read(m);
    rep(i, 1, n) b[s[i] - 97].set(i);
    while(m --) {
        int opt; read(opt);
        if(opt == 1) {
            int x; char c; read(x); read(c); b[s[x] - 97].set(x, 0); b[(s[x] = c) - 97].set(x);
        } else {
            ans.set(); int l, r; read(l, r); read(t + 1); const int len = strlen(t + 1);
            if(len > r - l + 1) {print("0"); continue;}
            rep(i, 1, len) ans &= (b[t[i] - 97] >> (l + i - 1));
            print(ans.count() - (ans >> (r - l + 2 - len)).count());
        }
    }
}