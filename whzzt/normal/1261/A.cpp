...
int main(){
	...
	for(int tt=1;tt<=T;++tt){
		...
		scanf("%s",str+1);
		vector<pii> z;
		for(int i=1;i<=2*(k-1);++i) tar[i]=(i&1)?'(':')';
		for(int i=2*k-1;i<=2*k-1+(n/2-k+1)-1;++i) tar[i]='(';
		for(int i=2*k-1+(n/2-k+1);i<=n;++i) tar[i]=')';
		for(int i=1,j;i<=n;++i){
			for(j=i;j<=n;++j) if(str[j]==tar[i]) break;
			if(j!=i){
				z.pb(i,j);
				for(int k=i;k<=j-(k-i);++k) swap(str[k],str[j-(k-i)]);
			}
		}
		printf("%d\n",z.size());
		for(auto x:z) printf("%d %d\n",x.fi,x.se);
	}
}