...
public class E {
	...
	void solve()
	{
		...
		int[] ww = new int[2*m];
		for(int i = 0;i < m;i++){
			ff[i] = from[i]; tt[i] = to[i]; ww[i] = 1;
			ff[i+m] = to[i]; tt[i+m] = from[i]; ww[i+m] = ws[i] == 1 ? -1 : 1;
		}

		int max = -5;
		int[] arg = null;
		for(int i = 0;i < n;i++){
			int[] bf = bf(ff, tt, ww, n, i);
			if(bf != null){
				boolean valid = true;
		...
		for(int i = 0;i < m;i++){
			if(ws[i] == 1){
				assert arg[to[i]] - arg[from[i]] == 1;
			}else{
				assert Math.abs(arg[to[i]] - arg[from[i]]) == 1;
			}
		}
		for(int i = 0;i < n;i++){
			assert arg[i] >= 0;
