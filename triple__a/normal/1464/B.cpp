...
string s;
int x,y;
int c1[maxn],c2[maxn],c1r[maxn],c2r[maxn],sumc0[maxn],sumc1[maxn],fk[maxn];
signed main(){
    ...
    for (auto c:s){
        if (c=='0') k1++;
        if (c=='1') k2++,sum+=y*k1;
        if (c=='?') c1r[i]=k1, c2r[i]=k2;
        i--; 
    }
    for (int i=1;i<=n;++i){
        sumc0[i]=sumc0[i-1]+y*c2[i]+x*c2r[i];
        sumc1[i]=sumc1[i-1]+x*c1[i]+y*c1r[i];
        fk[i]+=fk[i-1];
    }
    int ans=1e18;
 //   cerr<<sum<<endl;
    for (int i=0;i<=n;++i){
 //       cerr<<sumc1[i]<<" "<< sumc0[n]-sumc0[i]<<" "<<fk[n]-fk[i]<<endl;
        
        ans=min(ans,sum+min(sumc0[i]+sumc1[n]-sumc1[i]+x*fk[i]*(fk[n]-fk[i-1]), sumc1[i]+sumc0[n]-sumc0[i]+y*fk[i]*(fk[n]-fk[i-1])));
    }
    cout<<ans<<"\n";
}