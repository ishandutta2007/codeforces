...
void solve(int test) {
    ...
    multiset<char> u(all(a));
    multiset<char> v(all(b));

    while (u.size() > (a.size() + 1) / 2) {
        u.erase(u.find(*u.rbegin()));
    }

    while (v.size() > a.size() / 2) {
        v.erase(v.begin());
    }
