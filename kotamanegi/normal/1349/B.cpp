...
void solve() {
    ...
    REP(tea, 2) {
        ...
        for (int i = n - 1; i >= 0; --i) {
            backs[i] = backs[i + 1] + inputs[i];
            calcs[i] = backs[i];
        }
        for (int q = 0; q < n; ++q) {
            calcs[q + 1] = max(calcs[q + 1], calcs[q]);
        }
        pos.push_back(-1);
        pos.push_back(n);
        sort(ALL(pos));
        for (int q = 1; q < pos.size(); ++q) {
            int now_max = -1e9;
            for (int j = pos[q] - 1; j > pos[q - 1]; --j) {
                if (j + 1 < pos[q]) {
                    if (inputs[j] + inputs[j + 1] == 2) {
                        cout << "yes" << endl;
                        return;
                    }
                }
                if (j + 2 < pos[q]) {
                    if (inputs[j] + inputs[j + 1] + inputs[j + 2] == 1) {
                        cout << "yes" << endl;
                        return;
                    }
                }
