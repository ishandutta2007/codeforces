...
struct PolynomialInterpolation
{
	int deg;
	vector<mint> vec;
	vector<mint> lvec, rvec;
	
	PolynomialInterpolation(){};
	void init(vector<mint> val){
		deg = (int)val.size()-1;
		vec.resize(deg+1);
		for(int i = 0; i <= deg; i++){
			vec[i] = val[i] * fact_inv[i] * fact_inv[deg-i];
			if((deg-i) % 2) vec[i] = -vec[i];
		}
		lvec.resize(deg+1), rvec.resize(deg+1);
	}
	mint get(ll x){
		mint mul;
		mul = 1; for(int i = 0; i <= deg; i++) mul *= mint(x-i), lvec[i] = mul;
		mul = 1; for(int i = deg; i >= 0; i--) mul *= mint(x-i), rvec[i] = mul;
		
		mint ret = 0;
		for(int i = 0; i <= deg; i++){
			mul = 1;
			if(i > 0) mul *= lvec[i-1];
			if(i < deg) mul *= rvec[i+1];
			ret += vec[i] * mul;
		}
		return ret;
	}
...
int main(void)
{
	...
	vector<mint> vec;
	vec.push_back(0);
	rep(i, 1, k+1) vec.push_back(vec.back() + modpow(i, k));
	pi.init(vec);
