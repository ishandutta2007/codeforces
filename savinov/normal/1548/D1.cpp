...
void solve(int) {
    ...
    forn(a00, 4) forn(a01, 4) forn(a10, 4) forn(a11, 4) {
        if (a00 + a01 + a10 + a11 != 3) continue;
        vector <pii> c;
        forn(u, a00) c.push_back({0, 0});
        forn(u, a01) c.push_back({0, 1});
        forn(u, a10) c.push_back({1, 0});
        forn(u, a11) c.push_back({1, 1});

        int g = gcd(abs(c[0].x - c[1].x), abs(c[0].y - c[1].y)) +
                gcd(abs(c[0].x - c[2].x), abs(c[0].y - c[2].y)) +
                gcd(abs(c[1].x - c[2].x), abs(c[1].y - c[2].y));

        if (g % 2 == 0) {
            ans += bin(cnt[0][0], a00) * bin(cnt[0][1], a01) * bin(cnt[1][0], a10) * bin(cnt[1][1], a11);
        }
    }
