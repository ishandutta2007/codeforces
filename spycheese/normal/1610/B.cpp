...
vector<int> rev(vector<int> a) { reverse(all(a)); return a; }
vector<int> rem(vector<int> const& a, int x) { vector<int> b; for (int y : a) if (y != x) b.push_back(y); return b; }
...
void solve() {
  ...
  for (int i = 0; i < n; ++i) {
    if (a[i] != a[n - 1 - i]) {
      if (rem(a, a[i]) == rev(rem(a, a[i]))) {
        cout << "YES\n";
        return;
      }
      if (rem(a, a[n-1-i]) == rev(rem(a, a[n-1-i]))) {
        cout << "YES\n";
        return;
      }
      break;
