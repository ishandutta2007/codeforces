...
struct Monoid {
    ...
    Monoid() { rep(i, 10) a[i] = i; }
    Monoid(const string &s, int k) {
        int m = si(s);
        rep(i, m) {
            int now = i;
            rep(_, m) {
...
int main() {
    INT(n, m, Q);
    VEC(string, s, n);
    SegmentTree<Monoid> seg(n, f, Monoid());
    rep(i, n) { seg.set(i, Monoid(s[i], i)); }
    seg.build();
    while(Q--) {
        CHR(c);
        if(c == 'A') {
            INT(x, y);
            y--;
            auto a = seg.query(0, x);
            int res = seg.query(0, x)[y];
            if(res == -inf<int>) {
                cout << -1 << " " << -1 << endl;
            } else if(res < 0) {
                res = -res;
                cout << res / 2 << " " << (res & 1 ? m + 1 : 0) << endl;
            } else {
                cout << 0 << " " << res + 1 << endl;
            }
        } else {
            ...
            CHR(c);
            s[x - 1][y - 1] = c;
            seg.update(x - 1, Monoid(s[x - 1], x - 1));
        }
    }
}