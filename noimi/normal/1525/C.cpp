...
int main() {
    TEST {
        ...
        rep(i, n) {
            v[a[i] & 1].eb(a[i], b[i] == 'R', i);
            x.eb(a[i], b[i] == 'R', i);
        }
        vi ans(n, -1);
        for(auto w : v) {
            set<tuple<int, int, int>> s;
            for(auto e : w) s.emplace(e);
            pqg<tuple<int, int, int>> pq;
            sort(all(w));
            // for(auto e : w) { cerr << "(" << get<0>(e) << ", " << get<1>(e) << ", " << get<2>(e) << ")"; }
            // cerr << endl;
            rep(i, si(w) - 1) {
                auto [x1, v1, i1] = w[i];
                auto [x2, v2, i2] = w[i + 1];
                if(v1) {
                    if(!v2) {
                        pq.emplace(x2 - x1, i1, i2);
                    } else {
                        pq.emplace(m * 2 - x1 - x2, i1, i2);
                    }
                } else {
                    if(v2)
                        pq.emplace(x1 + (m - x2) + m, i1, i2);
                    else
                        pq.emplace(x1 + x2, i1, i2);
                }
            }
            vi used(n);
            while(!empty(pq)) {
                auto [t, i1, i2] = pq.top();
                pq.pop();
                if(used[i1] or used[i2]) continue;
                used[i1] = used[i2] = true;
                ans[i1] = ans[i2] = t / 2;
                s.erase(x[i1]);
                s.erase(x[i2]);
                auto it = s.lower_bound(x[i2]);
