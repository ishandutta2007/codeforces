...
void solve() {
	...
	vi a(n);
	REP(i, n) scanf("%d", &a[i]);
	debug("new test", imie(a), imie(x), imie(y));
	vi b(n, -1);
	map <int, vi> wh;
	REP(i, n) wh[a[i]].push_back(i);
	int miss = -1;
	for (int i = 1; i <= n + 1; ++i) if (!wh.count(i)) miss = i;
	assert(miss != -1);
	priority_queue<pii> by_occ;
	for (auto &[k, v] : wh) by_occ.push({v.size(), k});
	REP(_, x) {
		auto [c, v] = by_occ.top();
		by_occ.pop();
		assert(c);
		...
		assert(b[t] == -1);
		b[t] = v;
		debug(imie(a), imie(b), imie(wh), imie(by_occ));
	}
	vpii rest;
	REP(i, n) if (b[i] == -1)
		rest.emplace_back(a[i], i);
	sort(ALL(rest));
	int off = rest.size() / 2;
	vi match, mismatch;
	debug(imie(rest));
	REP(i, rest.size()) {
		int p = rest[i].second;
		int v = rest[(i + off) % rest.size()].first;
		b[p] = v;
		if (a[p] == b[p]) match.push_back(p);
		else mismatch.push_back(p);
	}
	debug(imie(a), imie(b));
	debug(imie(match), imie(mismatch));
	int mismatch_cou = y - x;
	if (mismatch_cou > (int) mismatch.size()) {
		puts("No");
		return;
	}
	for (int x : match) b[x] = miss;
	for (int i = mismatch_cou; i < (int) mismatch.size(); ++i) b[mismatch[i]] = miss;
	puts("Yes");
