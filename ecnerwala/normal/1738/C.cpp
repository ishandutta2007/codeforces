...
int main() {
	...
	const int V = 200;
	tensor<bool, 3> wins({V, V, 2});
	for (int c0 = 0; c0 < V; c0++) {
		for (int c1 = 0; c1 < V; c1++) {
			for (int want = 0; want < 2; want++) {
				if (c0 == 0 && c1 == 0) {
					wins[{c0, c1, want}] = want == 0;
				} else {
					int owant = (c1 & 1) ^ want ^ 1;
					wins[{c0, c1, want}] = (c0 > 0 && !wins[{c0-1, c1, owant}]) || (c1 > 0 && !wins[{c0, c1-1, owant}]);
				}
			}
		}
	}
	...
	while (T--) {
		...
		cout << (wins[{cnt0, cnt1, 0}] ? "Alice" : "Bob") << '\n';
	}
