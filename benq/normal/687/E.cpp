...
template<int SZ> struct scc {
    ...
    void genSCC(int _N) { // fills allComp
        ...
        FOR(i,1,N+1) trav(t,adj[i]) if (comp[i] != comp[t]) out[comp[i]] = 1;
        trav(t,allComp) if (!out[t]) bad.pb(t);
    }
};

scc<5001> S;
int n,m,ans,small[5001];
// 998*(size of cycle)+n+(# of cycles)
...
int main() {
    ...
    S.genSCC(n);
    FOR(i,1,n+1) small[i] = MOD;
    // exit(0);
    FOR(i,1,n+1) {
    	if (!sz(S.adj[i])) {
    		ckmin(small[S.comp[i]],-1);
    		continue;
    	}
    	if (find(all(bad),S.comp[i]) != bad.end()) ckmin(small[S.comp[i]],solve(i));
    	// exit(0);
    }
    ans += n;
    trav(t,bad) {
    	if (small[t] == -1) continue;
    	ans += 998*small[t]+1;
    }
    ps(ans);
}
