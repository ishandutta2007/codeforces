...
int main() {
  ...
  for (int testcases = $, testcase = 0; testcase++ < testcases; ) {
    ...
    ans.clear();
    for (int i = 0; i < n; ++i)
    for (int j = i + 1; j < n; dsuc(i * n + j, j * n + i), ++j)
    for (int k = 0; k < n; ++k) {
      if (same[k][i][j] = !$ == '1') {
        dsuc(k * n + i, k * n + j);
      }
    ...
    for (int i = 0; i < n; ++i)
    for (int j = i + 1; j < n; ++j) if (ans.empty() && (dsuc(i * n + j) == i * n + j || dsuc(i * n + j) == j * n + i) && dsuc[i * n + j] == 2 * (n - 1)) {
      dsuv.reset();
      ...
      for (int j_ = i_ + 1; j_ < n; ++j_) if (dsuc(i_ * n + j_) == dsuc(i * n + j)) {
        dsuv(i_, j_);
        dst[i_][j_] = dst[j_][i_] = 1;
      }
      if (dsuv[0] != n) {
        continue;
      }
      ...
      for (int i = 0; ok && i < n; ++i)
      for (int j = i + 1; ok && j < n; ++j) {
        ok &= (dst[k][i] == dst[k][j]) == same[k][i][j];
      }
      if (ok) {
        for (int i_ = 0; i_ < n; ++i_)
        for (int j_ = i_ + 1; j_ < n; ++j_) if (dsuc(i_ * n + j_) == dsuc(i * n + j)) {
          ans.emplace_back(i_ + 1, j_ + 1);
        }
      }
    }
    print(ans.empty()? "No": "Yes", ans);
  }
}