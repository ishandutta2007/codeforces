#line 1 "/Users/nok0/Documents/Programming/nok0/graph/graph.hpp"
#include <algorithm>
#include <cassert>
#include <deque>
#include <iostream>
...
void main_() {
	...
	REP(i, n - 1) {
		bool fin = 0;
		REP(j, min(SZ(s[i]), SZ(s[i + 1]))) {
			if(s[i][j] != s[i + 1][j]) {
				G.add_edge(s[i][j] - 'a', s[i + 1][j] - 'a', 1, 1);
				fin = 1;
				break;
			}
		}
		if(!fin and SZ(s[i]) > SZ(s[i + 1])) {
			print("Impossible");
			return;
		}
	}
