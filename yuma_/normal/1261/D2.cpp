...
int main(){
	...
	vector<ll>x{1,M-2,1};
	vector<vector<ll>>muls(18);
	muls[0]=x;
	//WHATS(muls[0])
	for(int i=1;i<18;++i){
		muls[i]=ntt.conv(muls[i-1],muls[i-1]);
		while(muls[i].back()==0)muls[i].pop_back();
	}
	//WHATS(muls[3])
	vector<ll>nums{1};
	for(int i=0;i<18;++i){
		if(cnt&(1<<i)){
			nums=ntt.conv(nums,muls[i]);
		}
	}
	ll answer=accumulate(nums.begin()+cnt+1,nums.end(),0ll);
	answer%=998244353;
	answer*=NTT::modpow(M,N-cnt,998244353);
	answer%=998244353;
	cout<<answer<<endl;
	return 0;
}