...
fun main() {
	cases{
		...
		val ret:List<Int>
		if(L[0] == 1){
			ret = listOf(n+1) + (1..n).toList()
		}else if(L[n-1] == 0){
			ret = (1..n).toList() + listOf(n+1)
		}else{
			val firstOk = L.windowed(2).indexOfFirst { it[0] == 0 && it[1] == 1 } + 1
			ret = (1..firstOk).toList() + listOf(n+1) + (firstOk+1 .. n).toList()
		}
