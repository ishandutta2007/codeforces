...
void _solve() {
    ...
    for(int i = 0; i < m; ++i) {
        for(int j = i; j < m; ++j) {
            int sum = e[i].y + e[j].y;
            vals.push_back(sum / 2 + 1);
            vals.push_back(sum / 2);
            vals.push_back(sum / 2 - 1);
        }
    }
    ...
    for(int i = 0, j = 0; i < k; ++i) {
        bool need = (i == 0);
        while(j < sz(vals) && vals[j] <= q[i]) {
            ++j;
            need = true;
        }

        if(need) {
            sort(all(e), [&q, &i](const pair<pair<int, int>, int> &a, const pair<pair<int, int>, int> &b) {
                return abs(a.y - q[i]) < abs(b.y - q[i]);
            });
