...
bigint operator*(const bigint &a,const bigint &b)
{
	bigint R;
	R.L=a.L+b.L;
	for (int i=0;i<a.L;i++) for (int j=0;j<b.L;j++)
	{
		int &t=R.A[i+j];
		t+=a.A[i]*b.A[j];
		if (t>=CUT)
...
int n,deg[maxn],children[maxn][maxn];
bigint *f[maxn],g[maxn],t[maxn];
int s[maxn],c[maxn];

void DFS(int v,int father)
{
	for (int i=0;i<deg[v];i++)
		if (children[v][i]==father)
		{
			children[v][i]=children[v][--deg[v]];
			break;
		}
	for (int i=0;i<deg[v];i++)
		DFS(children[v][i],v);
...
void solve(int v)
{
	...
	{
		int key=children[v][i];
		for (int c=1;c<=s[v];c++) t[c]=f[v][c];
		for (int c=1;c<=s[v];c++) f[v][c]=f[v][c]*g[key];
		for (int c=1;c<=s[v];c++) if (t[c].L>0) for (int dc=1;dc<=s[key] && c+dc<=s[v];dc++)
		{
			bigint tmp=t[c]*f[key][dc];
			if (tmp>f[v][c+dc]) f[v][c+dc]=tmp;
		}
		delete[] f[key];
	}
	g[v]=0;
