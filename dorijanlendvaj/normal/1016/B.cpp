...
tuple<int,int,int> has(string x)
{
	ll h1=0,h2=0,h3=0;
	x=to_lower(x);
	for (char a: x)
...
int main()
{
	...
	cin>>n>>m>>q>>h>>t;
	for (int i=0;i<=n-m;++i) if (h.substr(i,t.size())==t) ++oc[i];
	for (int i=0;i<n;++i) pre[i+1]=pre[i]+oc[i];
	while (q--)
	{
		int l,r;
		cin>>l>>r;
