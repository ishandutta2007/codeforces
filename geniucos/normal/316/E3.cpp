...
int ans_x, ans_y, mod, N, M, a[200009], lazy[800009], x[800009], y[800009], coef_x[200009], coef_y[200009], sum_x[200009], sum_y[200009], fibo[200009];
...
void split (int nod, int st, int dr, int mij)
{
    ...
    int d = lazy[nod], f1 = nod << 1, f2 = f1 + 1;

    lazy[f1] += d;
    if (lazy[f1] >= mod) lazy[f1] -= mod;

    lazy[f2] += d;
    if (lazy[f2] >= mod) lazy[f2] -= mod;

    x[f1] = ((long long) x[f1] + 1LL * d * sum_x[mij - st]) % mod;
    y[f1] = ((long long) y[f1] + 1LL * d * sum_y[mij - st]) % mod;

    x[f2] = ((long long) x[f2] + 1LL * d * sum_x[dr - mij - 1]) % mod;
    y[f2] = ((long long) y[f2] + 1LL * d * sum_y[dr - mij - 1]) % mod;
    lazy[nod] = 0;
}
...
void Update2 (int nod, int st, int dr, int x_q, int y_q, int val)
{
    if (x_q <= st && dr <= y_q)
    {
        lazy[nod] += val;
        if (lazy[nod] >= mod) lazy[nod] -= mod;
        x[nod] = ((long long) x[nod] + 1LL * sum_x[dr - st] * val) % mod;
        y[nod] = ((long long) y[nod] + 1LL * sum_y[dr - st] * val) % mod;
        return ;
    }
    int mij = (st + dr) >> 1;
    split (nod, st, dr, mij);
    if (x_q <= mij) Update2 (nod << 1, st, mij, x_q, y_q, val);
    if (y_q > mij) Update2 ((nod << 1) + 1, mij + 1, dr, x_q, y_q, val);
    Refresh (nod, st, dr, mij);
}
