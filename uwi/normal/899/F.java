...
public class F {
	...
	void solve()
	{
		...
		Node[] sroots = new Node[128];
		for(int i = n-1;i >= 0;i--){
			nodes[i] = new Node(s[i]);
			sroots[s[i]] = merge(new Node(i), sroots[s[i]]);
			root = merge(nodes[i], root);
		}
		for(int z = 0;z < Q;z++){
			int l = ni()-1, r = ni()-1;
			int c = nc();
			...
			int lcount = 0;
			while(sr != null){
				if(index(nodes[sr.v]) >= l){
					sr = sr.left;
				}else{
					lcount += count(sr.left) + 1;
					sr = sr.right;
				}
			}
			while(lcount < count(sroots[c])){
				Node sl = get(sroots[c], lcount);
				int index = index(nodes[sl.v]);
				if(index > r)break;
				root = erase(root, index(nodes[sl.v]));
				sroots[c] = erase(sroots[c], lcount);
				r--;
			}
		}
		for(Node node : nodes(root)){
			out.print((char)node.v);
		}
		out.println();
	}
