...
public class D3 {
	...
	void solve()
	{
		...
		int[][] h = Arrays.copyOf(g, n+1);
		h[n] = new int[n];
		for(int i = 0;i < n;i++)h[n][i] = i;
		int[] cycle = dfs(n, h, 0, new int[n+2], new boolean[n+1], new boolean[n+1]);
		if(cycle == null){
			out.println("YES");
			return;
		}
		
		for(int i = 0, j = 1;i < cycle.length;i++,j++){
			if(j == cycle.length)j = 0;
			if(sortTopologically(g, cycle[i], cycle[j]) != null){
				out.println("YES");
				return;
			}
		}
	...
	public static int[] sortTopologically(int[][] g, int ngs, int ngt)
	{
		int n = g.length;
		int[] ec = new int[n];
		for(int i = 0;i < n;i++){
			for(int to : g[i]){
				if(i == ngs && to == ngt)continue;
				ec[to]++;
			}
		}
		int[] ret = new int[n];
