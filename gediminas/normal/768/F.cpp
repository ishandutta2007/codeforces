...
int main () {
	...
	auto C = [&](ll n, ll m) -> ll {
		...
		return ((fak[n] * fakinv[m]) % mod *fakinv[n - m]) % mod;
	};

	ll iki = min(a, b);

	for (int i = 1; i <= iki + 1 and i <= a; i++) {
		for (int j = max(i - 1, 1); j <= i + 1 and j <= b; j++) {
			ll daug = 1;

			if (i == j)
				daug = 2;

			debug(a, i, b, j, a - h, b - h);
			viso += (C(a - 1, i - 1) * C(b - 1, j - 1)) % mod * daug;

			tinka += (C(a - 1, i - 1) * C(b - (h) * j - 1, j - 1)) % mod * daug;
	...
	cout << (tinka * modInverse(viso)) % mod;
}