...
int main(int argc, char** argv) {
    ...
    for (auto& p : m2) {
        ...
        while (isz(pa) >= 2) {
            ...
            pp[x] = y;
            pp[y] = x;
            paired[pr[x].first].insert(x);
            paired[pr[y].first].insert(y);
        }
        if (isz(pa)) {
            int x = pa.back();
            pa.pop_back();
            simple[pr[x].first].insert(x);
        }
    }
