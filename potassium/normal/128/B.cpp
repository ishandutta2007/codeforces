...
struct SuffixAutomata {
	...
	void insert(char c) {
		int cur = (int)dfa.size();
		dfa.emplace_back(Node());
		dfa[cur].len = dfa[last].len + 1;
		
		int p = last;
		while (p != -1 && !dfa[p].nxt.count(c)) {
			dfa[p].nxt[c] = cur;
			p = dfa[p].link;
		}
		...
		if (p == -1) {
			...
			int q = dfa[p].nxt[c];
			if (dfa[q].len == dfa[p].len + 1) {
				dfa[cur].link = q;
			}
			else {
				int clone = (int)dfa.size();
				dfa.emplace_back(dfa[q]);
				dfa[clone].len = dfa[p].len + 1;

				dfa[q].link = dfa[cur].link = clone;

				while (p != -1 && dfa[p].nxt[c] == q) {
					dfa[p].nxt[c] = clone;
					p = dfa[p].link;
				}
			}
...
int main() {
	string s; R(s);
	ll k; R(k);
	const ll n = (int)s.length();

	SuffixAutomata sam;
	for (auto& i : s)
		sam.insert(i);

	const int S = (int)sam.dfa.size();
	vector<ll> cnt(S, 0);
	for (int now = 0, i = 0; i < n; i++) {
		now = sam.dfa[now].nxt[s[i]];
		cnt[now] = 1;
	}

	vector<vector<int>> l(n + 1);
	for (int i = 1; i < S; i++)
		l[sam.dfa[i].len].emplace_back(i);
		
	for (int i = n; i >= 0; i--)
		for (auto& j : l[i])
			if (sam.dfa[j].link > 0)
				cnt[sam.dfa[j].link] += cnt[j];

	vector<bool> done(S, false);
	vector<ll> dp = cnt;
	function<void(int)> dfs_dp = [&](int u) {
		done[u] = true;
		for (auto& e : sam.dfa[u].nxt) {
			if (!done[e.second])
				dfs_dp(e.second);
			dp[u] += dp[e.second];
		}
