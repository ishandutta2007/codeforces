...
//using namespace std;
//int n,m;
//ModInt <mod> dp[2005][2005][2];
//ModInt <mod> f1[2005];
//ModInt <mod> f2[2005];
//
//inline void solve(ModInt <mod> dp[][2005][2])
//{
//    for(int j=2;j<=m;j++)
//    {
//        dp[1][j][0]=1;
...
//    for(int i=2;i<=n;i++)
//    {
//        f1[0]=dp[i-1][0][0]+dp[i-1][0][1];
//        f2[0]=0;
//        for(int j=1;j<=m;j++)
//        {
//            f1[j]=f1[j-1]+dp[i-1][j][0]+dp[i-1][j][1];
//            f2[j]=f2[j-1]+(dp[i-1][j][0]+dp[i-1][j][1])*j;
//            dp[i][j][0]=f1[j-1]*(j+1)-f2[j-1];
...
//        for(int j=2;j<=m;j++)
//        {
//            // cout<<"i="<<i<<' '<<"j="<<j<<' '<<dp1[i+1][j]<<' '<<dp2[n-i][j]<<' '<<(m-j+1)<<endl;
//            res+=dp[i+1][j][0]*(dp[n-i][j][0]+dp[n-i][j][1])*(m-j+1);
//        }
//    }
//    cout<<res<<endl;
//    return 0;
