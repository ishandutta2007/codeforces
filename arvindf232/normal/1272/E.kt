...
fun main(){
    solve.cases{
        ...
        fun solve(parity:Int){
            val G = Graph(n+1, true)
            val root = n
            for(i in 0 until n){
                if(L[i] % 2 != parity){
                    continue
            ...
            val dist = G.BFS (root){ i, i2 ->  }
            for((i,v)in dist.withIndex()){
                if(i == root) continue
                if(L[i] %2 != parity) continue
                ret[i] = v
            }
        }
        solve(0)
...
/*
construction of graph need to bec areful
forgot about a single -1 initilaitaoits
