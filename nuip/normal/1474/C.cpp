...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		cauto n=read()*2;
		auto vs=readV(n); sort(all(vs));
		vector<pii> re;
		rep(i,n-1){
			multiset<int> st(vs.begin(),vs.begin()+n-1);
			st.erase(st.find(vs[i]));
			vector<pii> ps{{vs.back(),vs[i]}};
			while(st.size()){
				cauto a=*st.rbegin(); st.erase(st.find(a));
				cauto it=st.find(ps.back().X-a);
				out(a,ps.back().X-a,1);
				if(it==st.end()) break;
				cauto b=*it; st.erase(it);
				ps.eb(a,b);
			}
			out(ps,1);
			if(ps.size()*2==n){
		...
		if(re.size()){
			cout<<"YES" NL;
			cout<<re[0].X+re[0].Y NL;
			for(cauto [a,b]:re) cout<<a<<" "<<b NL;
		}else{
			cout<<"NO" NL;
		}
	}
