...
void solve() {
  ...
  VEC(ll, A, N);
  for (auto&& a: A) --a;
  auto CNT = bincount(A, N);
  sort(all(CNT));
  reverse(all(CNT));
  while (CNT.back() == 0) CNT.pop_back();

  if (len(CNT) == 1) { return print(1); }

  ll G = 0;
  for (auto&& c: CNT) G = gcd(G, c);

  vc<mint> C(N + 1), S(N + 1);
  FOR3(p, 1, N + 1) if (N % p == 0) {
    ll g = N / p;
    if (G % g != 0) continue;
    mint x = fact<mint>(p);
    for (auto&& c: CNT) x *= fact_inv<mint>(c / g);
    C[p] = x;
    mint same = 0;
    for (auto&& c: CNT) same += (c / g) * (c / g - 1);
    mint tot = p * (p - 1);
    mint prob = mint(1) - same / tot;
    S[p] = C[p] * prob * N;
  }
  ...
  FOR3(n, 1, N + 1) if (N % n == 0) {
    C[n] *= inv<mint>(n);
    S[n] *= inv<mint>(n);
  }

  mint num = SUM(S), den = SUM(C);
  mint ANS = num / den;
  print(ANS);
}
