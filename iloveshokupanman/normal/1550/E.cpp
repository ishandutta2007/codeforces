...
int main() {
    ...
    vvll ren(N, vll(K, 0));
    rep(i, N)rep(j, K) {
        if (S[i] != '?' && S[i] != 'a' + j)ren[i][j] = 0;
        else if (i == 0)ren[i][j] = 1;
        else ren[i][j] = ren[i - 1][j] + 1;

        //cout << i << j << " " << ren[i][j] << endl;
    }

    auto ch = [&](ll x) {
        vvll slis(K);
        rep(i, N)rep(j, K)if (ren[i][j] >= x) { slis[j].push_back(i); }
       // cout << x << endl;
        //rep(j, K) {
          //  for (ll x : slis[j])cout << x << " ";
            //cout << endl;
        //}
        //cout << endl;
        ...
        rep(i, 1ull << K) {
            rep(j, K) {
                ...
                ll k = i ^ (1ull << j);
                //cout << k << endl;
                auto itr = lower_bound(slis[j].begin(), slis[j].end(), dp[k] + x);
                if (itr != slis[j].end()) { dp[i] = min(dp[i], *itr); }
            }
           // cout << i << " " << dp[i] << endl;
        }

        return dp[(1ull << K) - 1] < N;
    };
