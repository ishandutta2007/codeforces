...
long long ans=0;
long long tot_sum=0;
void dfs(int x,int p,vvi&adj,vl&dp,vl&prefix,vi&val)
{
    prefix[x]=val[x];
    for(auto node:adj[x])
    {
        if(node==p) continue;
        dfs(node,x,adj,dp,prefix,val);
        dp[x]+=prefix[node]+dp[node];
        prefix[x]+=prefix[node];
    }
}
void dfs2(int x,int p,vvi&adj,vl&dp,vl&prefix)
{
    ans=max(ans,dp[x]);
    for(auto node:adj[x])
    {
        if(node==p) continue;
        dp[x]-=(dp[node]+prefix[node]);
        prefix[x]-=prefix[node];
        dp[node]+=dp[x]+prefix[x];
        prefix[node]+=prefix[x];
        dfs2(node,x,adj,dp,prefix);
        prefix[node]-=prefix[x];
        dp[node]-=(dp[x]+prefix[x]);
        prefix[x]+=prefix[node];
        dp[x]+=dp[node]+prefix[node];
...
int main()
{
    ...
    vl dp(n,0),prefix(n,0);
    dfs(0,-1,adj,dp,prefix,val);
    dfs2(0,-1,adj,dp,prefix);
    cout<<ans<<'\n';
}