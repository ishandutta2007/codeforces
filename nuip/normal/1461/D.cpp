...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		sort(all(vs));
		vector<ll> sum(n+1);
		rep(i,n) sum[i+1]=sum[i]+vs[i];
		queue<pii> que; que.emplace(vs[0],vs.back());
		set<ll> ex;
		while(que.size()){
			cauto [mn,mx]=que.front(); que.pop();
			ex.emplace(sum[upper_bound(all(vs),mx)-vs.begin()]-
								 sum[lower_bound(all(vs),mn)-vs.begin()]);
			if(mn==mx) continue;
			cauto mid=(mn+mx)/2;
			auto it=upper_bound(all(vs),mid);
			que.emplace(*it,mx);
			--it;
			que.emplace(mn,*it);
		}
		out(ex,1);
		for(cauto q:qs) cout<<(ex.count(q)?"YES":"NO") NL;
	}
  return 0;
}