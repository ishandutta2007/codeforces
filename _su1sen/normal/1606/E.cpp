...
int main() {
    ...
    auto dfs = [&](auto dfs, int num, int hp) -> mint {
        if (num == 1) return 0;
        if (num == 0) return 1;
        if (hp <= 0) return 0;
        if (seen[num][hp]) return dp[num][hp];
        mint res = 0;
        rep(k, num + 1) {
            mint v = dfs(dfs, num - k, hp - (num - 1));
            res += fac.binom(num, k) * mint(min(hp, num - 1)).pow(k) * v;
        }
        seen[num][hp] = true;
        return dp[num][hp] = res;
    };
    print(dfs(dfs, n, m));
    return 0;
}