...
int dfs(int node, int parent, vector<vector<int>> &edges, int& diameter) {
    int len1 = 0, len2 = 0;
    for (auto adj : edges[node]) {
        if (adj == parent) continue;
        
        int len = dfs(adj, node, edges, diameter);
        if (len > len1) {
            swap(len, len1);
        }
        if (len > len2) {
    ...
    chkmax(diameter, len1 + len2);
    return len1 + 1;
}

void getDis(int node, int parent, vector<vector<int>> &edges, vector<int> &dis) {
    if (parent != -1) 
        dis[node] = dis[parent] + 1;
    for (auto adj : edges[node]) {
        if (adj == parent) continue;
        getDis(adj, node, edges, dis);
    }
}
...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        int diameter = 0;
        dfs(0, -1, edges, diameter);

        // cout << "diameter:" << diameter << endl;

        vector<int> disA(n, 0), disB(n, 0);
        getDis(a, -1, edges, disA); getDis(b, -1, edges, disB);
        
        int distance = 1e9;
        REP(i, n) {
            chkmin(distance, disA[i]+disB[i]);
        }

        // cout << "distance:" << distance << endl;

        if (2 * da >= min(db, diameter) || distance <= da) {
            cout << "Alice" << endl;
        }
        else {
            cout << "Bob" << endl;
