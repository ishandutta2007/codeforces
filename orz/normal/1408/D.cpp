...
bool ip2(const int x)
{
	return (x & (x - 1)) == 0;
}
void equify(const int x, const int y, vpii& a)
{
	if (ip2(y - x))
	{
		if (y - x > 1)
	...
	{
		int m = 1 << (31 - __builtin_clz(y - x));
		equify(x, x + m, a);
		int mm = 1 << ((32 - __builtin_clz(y - x - m - 1)));
		equify(y - mm, y, a);
	}
}
int another_main()
{
	...
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < m; ++j)
			if (a[i].X <= c[j].X)
				v[c[j].X - a[i].X] = max(v[c[j].X - a[i].X], 1 + c[j].Y - a[i].Y);
	int ans = 2 * N;
	for (int i = N - 2; i >= 0; --i)
	{
		v[i] = max(v[i], v[i + 1]);
