...
struct sum_of_top_k {
    ...
    void move_up() {
        top_sum += *rest.rbegin();
        top.insert(*rest.rbegin());
        rest.erase(rest.find(*rest.rbegin()));
    }

    void move_down() {
        top_sum -= *top.begin();
        rest.insert(*top.begin());
        top.erase(top.begin());
    }
};
...
int main() {
    ...
    ll ans = 0;
    sum_of_top_k pos(k), neg(k);

    auto insert = [&](int x) { (x >= 0 ? pos : neg).insert(abs(x)); };
    auto erase = [&](int x) { (x >= 0 ? pos : neg).erase(abs(x)); };
    auto process = [&]() {
        auto big = abs(pos.full_sum - neg.full_sum + 2 * neg.top_sum);
        auto small = abs(pos.full_sum - neg.full_sum - 2 * pos.top_sum);
        ckmax(ans, max(big, small));
    };
