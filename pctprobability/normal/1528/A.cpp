...
void dfs(ll a,ll b){
  ...
  for(auto e:g[a]){
    if(e==b) continue;
    dfs(e,a);
    sum+=max(abs(l[a]-l[e])+lm[e],abs(l[a]-r[e])+rm[e]);
    sum2+=max(abs(r[a]-l[e])+lm[e],abs(r[a]-r[e])+rm[e]);
  }
  lm[a]=sum;
  rm[a]=sum2;
}
void solve(){
  ll a;
  cin>>a;
  for(int i=0;i<a;i++) cin>>l[i]>>r[i];
  for(int i=0;i<a;i++) g[i]={};
  for(int i=0;i<a;i++) lm[i]=rm[i]=0;
  for(int i=0;i<a-1;i++){
    ll x,y;
    cin>>x>>y;
    x--;
