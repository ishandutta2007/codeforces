...
ll n, Q;
ll tp[200005], l[200005], r[200005], x[200005];
ll t[200005];
LazySegTree seg(18);
...
int main(void)
{
	...
	rep(i, 1, Q){
		cin >> tp[i];
		if(tp[i] == 0) cin >> l[i] >> r[i] >> x[i];
		else cin >> x[i];
	}
	
	seg.init();
	rep(i, 1, n) seg.set(i, P(inf, i));
	seg.setup();
	
	per(i, Q, 1) if(tp[i] == 0 && x[i] == 0) seg.add(l[i], r[i], i);
	
	rep(i, 1, n) t[i] = inf;
	per(i, Q, 1){
		if(tp[i] == 1 || x[i] == 0) continue;
		if(l[i] == r[i]){
			t[l[i]] = i;
			continue;
		}
		P res = seg.query(l[i], r[i]);
		seg.update(res.se, P(-inf, res.se));
		P res2 = seg.query(l[i], r[i]);
		if(res2.fi < inf/2) chmin(t[res.se], max(res2.fi, i));
		seg.update(res.se, P(inf, res.se));
	}
	...
	rep(i, 1, Q){
		...
		if(t[x[i]] < inf/2){
			...
			P res = seg.query(x[i], x[i]);
			if(res.fi < inf/2 && i >= res.fi) cout << "NO" << "\n";
			else cout << "N/A" << "\n";
		}
	}
