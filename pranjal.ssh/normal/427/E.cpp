...
int main()
{
    ...
    int n, m; cin >> n >> m;
    vll a(n); FOR (i, 0, n - 1) cin >> a[i],a[i] += 1e9, a[i] *= 2;
    sort(all(a));
    ...
    while (st < en) {
        ll mid = (st + en) / 2;
        ll d1 = 0, d2 = 0;
        int ye = upper_bound(all(a), mid) - a.begin();
        for (int it = 0; it < ye and a[it] < mid; it += m) {
            d1++;
        }
        for (int it = n - 1; it >= ye; it -= m) {
            d2++;
        }
        if (d1 < d2) st = mid + 1;
        else en = mid;
    ...
    ll d1 = 0, d2 = 0;
    int ye = upper_bound(all(a), st) - a.begin();
    for (int it = 0; it < ye; it += m) {
        d1 += 2 * (st - a[it]);
    }
    for (int it = n - 1; it >= ye; it -= m) {
        d2 += 2 * (a[it] - st);
