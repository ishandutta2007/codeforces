...
int dp[1123][1123][2][2]; //00,01,0,pass

int main(){
	ios_base::sync_with_stdio(false); cin.tie(0);
	cout<<fixed<<setprecision(0);
	fill(dp[0][0][0],dp[1122][0][0],MOD);
	dp[0][0][0][0]=0;
	dp[0][0][0][1]=0;
	rep(i,1023)rep(j,1023)rep(k,2)rrep(f,2){
		if(j && !f) MN(dp[i][j][k][f], -dp[i][j][k][1]);
		if(i) MN(dp[i][j][k][f], 1-dp[i-1][j+1][k][0]);
		if(j) MN(dp[i][j][k][f], 1-dp[i][j-1][k][0]);
		if(k) MN(dp[i][j][k][f], 1-dp[i][j][k-1][0]);
		// if(i==0 && j==0 && k==1 && f==0) out(i,j,k,f,dp[i][j][k][f],1);
	}
	// rep(i,3)rep(j,3){
	// 	out(i,j,1);
	// 	rep(k,2)rep(f,2) ouT(dp[i][j][k][f]);
	// 	cout NL;
	// }
	TT{
		cauto n=read();
		cauto s=read<string>();
		int c01=0,c00=0,c0=0;
		rep(i,n){
			cauto j=n-1-i;
			if(i>j) break;
			cauto ci=s[i]-'0';
			...
			if(ci==0 && cj==0) ++c00;
			else if(ci==0 || cj==0) ++c01;
		}
		cauto re=dp[c00][c01][c0][0];
		out(c01,c00,c0,re,1);
		cout<<(re>0?"BOB":re?"ALICE":"DRAW") NL;
	}
	return 0;
}