...
struct AbsoluteSum{
	...
	void calc(){
		sort(all(vec));
		svec.push_back(0);
		for(auto x : vec) svec.push_back(svec.back() + x);
	}
	ll query(ll x){
		ll ret = 0, p = upper_bound(all(vec), x) - vec.begin();
		ret += x*p - svec[p];
		ret += svec.back() - svec[p] - x*(sz(vec)-p);
		return ret;
	}
};
...
int main(void)
{
	...
	rep(i, 2, n) p[i] = P(0, b[i]-a[i]);
	p[1] = P(1, -a[1]);

	for(auto q : pvec){
		for(int i = n/q; i >= 1; i--) p[i*q] -= p[i];
	}
	ll b = 0;
	rep(i, 1, n){
		if(p[i].first < 0) p[i].first *= -1, p[i].second *= -1;
		if(p[i].first) as.add(-p[i].second);
		else b += abs(p[i].second);
	}
	as.calc();
