...
void solve() {
    ...
    int bk = 0;
    V<int> dp(n + 2, -TEN(9)); // not selected (i - 1)-th
    atcoder::segtree<int, op, e> seg(n + 2); // seg[i] = dp[i] - i
    ...
    for (int i = 0; i <= n; i++) {
        ...
        dp[i + 1] = max(dp[i + 1], seg.prod(bk, i) + i);
        seg.set(i + 1, dp[i + 1] - (i + 1));
    }
    pr.writeln(dp.back());
}
int main() {
