...
fun solveopt(x: Int, l: Int, r: Int) : Int {
    return abs(l - r) + min(abs(x - l), abs(x - r));
}
...
fun solve() : Int {
    ...
    var ans = min(solveopt(x, 1, n), solveopt(y, 1, n));
    for (mid in 1..n - 1) {
        ans = min(ans, max(solveopt(x, 1, mid), solveopt(y, mid + 1, n)));
    }
    pr.println(ans);
    return 0;
}
