...
void solve() {
  ...
  VEC(string, A, N);
  STR(S);
  ll large = 0, eq = 0, small = 0;
  FOR(i, N) if (len(A[i]) > len(S))++ large;
  FOR(i, N) if (len(A[i]) == len(S))++ eq;
  FOR(i, N) if (len(A[i]) < len(S))++ small;

  auto f = [&](ll n) -> ll { return n + (n - 1) / K * 5; };

  print(f(small + 1), f(small + eq));
}
