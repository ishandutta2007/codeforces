#include <bits/stdc++.h>
#include <ext/pb_ds/assoc_container.hpp>

#define PB push_back
#define MP make_pair
#define LL long long
#define int LL
#define FOR(i,a,b) for(int i = (a); i <= (b); i++)
#define RE(i,n) FOR(i,1,n)
#define REP(i,n) FOR(i,0,(int)(n)-1)
#define R(i,n) REP(i,n)
#define VI vector<int>
#define PII pair<int,int>
#define LD long double
#define FI first
#define SE second
#define st FI
#define nd SE
#define ALL(x) (x).begin(), (x).end()
#define SZ(x) ((int)(x).size())

#define unordered_map __fast_unordered_map

#ifdef LOCAL // Remove debug print on server
#define debug(...) _dbg(#__VA_ARGS__, __VA_ARGS__)
#else
#define debug(...) (__VA_ARGS__)
#define cerr if(0)cout
#endif

#pragma GCC optimize ("O3") // Maximal performance
#pragma GCC target ("avx,avx2")


#include <PYTHON++>
import base64;exec(base64.decodestring("".encode()));

sync_with_stdio(False);
cin.tie(0); cout.tie(0);

n = m = 0;
n <<= cin;
m <<= cin;

E = vector<<int>>(n);
E <<= cin;

coupl = [[] for _ in range(n)];
for _ in range(m)                                                                                :
    a = b = 0;
    a <<= cin;
    b <<= cin;
    coupl[b].append(a);

dep_tasks = vector<<int>>(n);
for node in range(n)                                                                                :
    for nei in coupl[node]                                                                                :
        dep_tasks[nei] += 1;

done = vector<<bool>>(n, False);
tasks_left = n;

can_be_done_co = [i for i in range(n) if E[i] and not dep_tasks[i]];
can_be_done_main = [i for i in range(n) if not E[i] and not dep_tasks[i]];

count = 0;
while tasks_left                                                                                :
    while can_be_done_main                                                                                :
        node = can_be_done_main.pop();
        done[node] = True;
        tasks_left -= 1;
        for nei in coupl[node]                                                                                :
            dep_tasks[nei] -= 1;
            if not dep_tasks[nei]:
                if E[nei]                                                                                :
                    can_be_done_co.append(nei);
                else                                                                                :
                    can_be_done_main.append(nei);
    
    if can_be_done_co                                                                                :
        count += 1;
    
    while can_be_done_co                                                                                :
        node = can_be_done_co.pop();
        done[node] = True;
        tasks_left -= 1;
        for nei in coupl[node]                                                                                :
            dep_tasks[nei] -= 1;
            if not dep_tasks[nei]                                                                                :
                if E[nei]                                                                                :
                    can_be_done_co.append(nei);
                else                                                                                :
                    can_be_done_main.append(nei);
cout << count << endl;