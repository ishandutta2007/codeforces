...
{
	mint ret = 1;
	rep(i, 0, n-1) if(S & (1<<i)) ret *= sum[i][T];
	//outl(S, T, ret);
	return ret;
}
...
mint calc(ll s)
{
	...
	rep(i, 1, N-1){
		if((i & (1<<s)) == 0) continue;
		mint p = get(i, (N-1)-i).inverse();
		dp[i] = p;
		for(int j = i; ; j = (j-1)&i){
			if((j & (1<<s)) && j != i) dp[i] -= dp[j] * p * get(j, (N-1)-i);
			if(j == 0) break;
		}
	}
	//outa(dp, 0, N-1);
...
int main(void)
{
	...
	rep(i, 0, n-1){
		rep(j, 0, N-1){
			sum[i][j] = 1;
			rep(k, 0, n-1) if(j & (1<<k)) sum[i][j] *= (a[i]+a[k]) / a[k];
		}
	}
	
	mint ans = 0;
	rep(i, 0, n-1) ans += calc(i);
	outl(ans);
