...
public:
    void solve() {
        ...
        for (int i : range(n)) {
            ...
            clusters[{x[i] / g, y[i] / g}].push_back(i);
        }
        arr<ld> dist(n);
        for (int i : range(n)) {
            dist[i] = hypot(ld(x[i]), ld(y[i]));
        }
        vector<ld> deltas(1, 0);
        for (auto& p : clusters) {
            sort(all(p.second), [&](int a, int b) -> bool {
                return dist[a] > dist[b];
            });
            for (int i : range(min((k - 1) / 2 + 1, int(p.second.size())))) {
                deltas.push_back((k - 1 - 2 * i) * dist[p.second[i]]);
            }
        }
        sort(all(deltas), greater<>());
        if (deltas.size() < k) {
            int need = k - deltas.size();
            for (auto& p : clusters) {
                if (p.second.size() > (k - 1) / 2) {
                    for (int i : range(need)) {
                        deltas.push_back(
                                (k - 1 - 2 * ((k - 1) / 2 + 1 + i)) * dist[p.second[p.second.size() - need + i]]);
                    }
                    break;
                }
            }
        }
        out.printLine(accumulate(deltas.begin(), deltas.begin() + k, ld(0)));
    }
};
