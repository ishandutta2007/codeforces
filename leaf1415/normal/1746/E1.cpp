...
int main(void)
{
	...
	while(sz(vec) >= 4){
		vector<ll> qvec, qvec2;
		reps(i, vec){
			if(i&1) qvec.pb(vec[i]);
			if(i&2) qvec2.pb(vec[i]);
		}
		ll res = query(qvec), res2 = query(qvec2);
		
		vector<ll> nvec;
		reps(i, vec){
			if((i&1) == res || ((i>>1)&1) == res2) nvec.pb(vec[i]);
		}
		vec = nvec;
	}
	...
	if(sz(vec) == 3){
		ll res = query({vec[0]});
		if(res == 0) res = query({vec[0]});
		if(res == 0) vec = {vec[1], vec[2]};
		else{
			res = query({vec[0], vec[1]});
			if(res == 1) vec = {vec[0], vec[1]};
			else vec = {vec[0], vec[2]};
		}
	}
