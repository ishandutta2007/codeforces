...
int main() {
    ...
    F0R (_, 2) F0R (i, N) a.pb(a[i]);

    const int NN = N + N + N;

    vi ans(NN);

    deque<pii> pf{{INT_MIN, NN}};
    F0Rd (i, NN) {
        int stop = ub(all(pf), mp(a[i], INT_MIN), greater<pii>())->s;
        ans[i] = stop;
        if (i+1 < sz(ans)) ckmin(ans[i], ans[i+1]);

        while (pf.front().f > 2 * a[i]) pf.pop_front();
        pf.push_front({ 2 * a[i], i });
    }
