...
fun main(){
    solve.cases{
        val n = getint()
         val L = IntArray(n+1){if(it == 0) 0 else getint()}
        val s = L.sumOf { it.toLong() }
        if(s %n != 0L){
            put(-1)
            return@cases
        }
        ...
        val ret = mutableListOf<Triple<Int,Int,Long>>()
        for(i in 2..n){
            val need = ((L[i] divCeil i) * i ) - L[i]
            ret.add(Triple(1,i,need.toLong()))
            val total = L[i] divCeil i
            ret.add(Triple(i,1,total.toLong()))
        }
        ret.add(Triple(1,n,s))
        for(i in n downTo 2){
            ret.add(Triple(i,i-1,target))
        }
        ret.add(Triple(1,n,target))
