...
void solve(int) {
  ...
  for ([[maybe_unused]] auto e : rep(n - 1)) {
    auto u = input<int>() - 1;
    auto v = input<int>() - 1;
    g[u].push_back(v);
    g[v].push_back(u);
  ...
  auto q = std::vector<std::vector<Fp>>();
  std::function<void(int, int)> dfs = [&](int v, int p) -> void {
    auto c = 0;
    for (int u : g[v] | IF(_ != p)) {
      dfs(u, v);
      ++c;
    }
    if (c) {
  ...
  dfs(0, -1);
  for (auto pos = 0; pos + 2 <= int(q.size()); pos += 2) {
    q.push_back(atcoder::convolution(q[pos], q[pos + 1]));
  }
  auto fact = std::vector<Fp>(n + 1);
  fact[0] = 1;
  for (int i : rep(n)) {
    fact[i + 1] = fact[i] * (i + 1);
  }
  Fp ans;
  for (auto i : rep(int(q.back().size()))) {
    if (i & 1) {
      ans -= q.back()[i] * fact[n - i];
    } else {
      ans += q.back()[i] * fact[n - i];
    }
  }
