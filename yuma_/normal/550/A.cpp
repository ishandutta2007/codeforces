...
int main() {
    ...
    for(int i=0;i<st.size()-1;++i){
        if(st.substr(i,2)=="AB")nums[0].push_back(i);
        if(st.substr(i,2)=="BA")nums[1].push_back(i);
    }
    for(int l=0;l<min(3,int(nums[0].size()));++l){
        for(int r=0;r<min(3,int(nums[1].size()));++r){
            if(abs(nums[0][l]-nums[1][r])>=2)ok=true;
        }
    }
    if(ok)cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
