...
signed main()
{
	initprog();
	int T=read();rg(T)
	int n=read(),m=n-read(),k=read();
	for(int i=0;i<=n;i++)
		for(int j=0;j<=m;j++)a[i][j]=dp[i][j]=0;
	for(int i=0;i<=n;i++)dp[i][m]=(n-i),a[i][m]=n-i;
	for(int i=0;i<=m;i++)dp[n][i]=0,a[n][i]=0;
	for(int i=n-2;i>=0;i--)
		for(int j=m-1;j>=0;j--)
		{
			long double x=dp[i+1][j]+1,y=(dp[i+1][j]+dp[i+1][j+1])/2;
			if(x<y)dp[i][j]=x,a[i][j]=ae(a[i+1][j],1);
			else dp[i][j]=y,a[i][j]=me(mod+1>>1,ae(a[i+1][j],a[i+1][j+1]));
		}
	oldl(me(a[0][0],k));
	gr
	return 0;
