...
fun main() {
    ...
    val (n, m) = readInts(2)
    val a = List(n) { readInts(m) }
    val b = MutableList(n) { MutableList(m) {0} }
    val pos = listOf(mp(0, 0), mp(0, 1), mp(1, 0), mp(1, 1))
    val ans = mutableListOf<pii>()
    for (i in 0 until n - 1) {
        for (j in 0 until m - 1) {
            var ok = true
            for (p in pos) {
                if(a[i + p.first][j + p.second] == 0) ok = false
            }
            if(ok){
                ans.add(mp(i + 1, j + 1))
                for(p in pos){
                    b[i + p.first][j + p.second] = 1
                }
            }
        }
    }
