...
int main() {
    ...
    REC([&](auto &&f, int l, int r, int t) -> void {
        ...
        chmax(ma, t);
        if(r - l < k) {
            rep2(i, l, r - 1) rep2(j, l + 1, r - 1) g[i][j] = t;
        } else {
            int d = r - l;
            int L = l;
            rep(i, k) {
                int R = L + (d + i) / k;
                rep2(x, l, L - 1) rep2(y, L, R - 1) { g[x][y] = t; }
                f(L, R, t + 1);
                L = R;
            }
        }
    ...
    vector<Graph> G(ma + 1, Graph(n));

    rep(i, n) rep2(j, i + 1, n - 1) G[g[i][j]][i].eb(j);

    bool flag = true;
    rep(i, ma) {
        vi dp(n);
        rep(j, n) {
            for(auto e : G[i][j]) chmax(dp[e], dp[j] + 1);
        }
        if(MAX(dp) >= k) flag = false;
    }

    OUT(ma);
    rep(i, n) rep2(j, i + 1, n - 1) cout << g[i][j] << " ";
    OUT();
}
