...
int main(){
  ...
  auto f=[&](ll i){
    unsigned long long p=abs(x[i]-x[(i+1)%n]);
    unsigned long long q=abs(y[i]-y[(i+1)%n]);
    return p*p+q*q;
  };
  rep(i,0,n/2){
    if(f(i)!=f(i+n/2))judge=false;
    Point p(x[i],y[i]),q(x[(i+1)%n],y[(i+1)%n]);
    Point r(x[i+n/2],y[i+n/2]),s(x[(i+1+n/2)%n],y[(i+1+n/2)%n]);
    if(!parallel(Line(p,q),Line(r,s)))judge=false;
  }
  ans2(judge);
  return 0;
}