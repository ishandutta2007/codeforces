...
void solve()
{
	...
	for (int k = 0; k < n; ++k)
	{
		vector<vector<int>> dp(v[k].size() + 1, vector<int>(101, INF));
		vector<vector<int>> pre(v[k].size() + 1, vector<int>(101));
		dp[0][0] = 0;
		for (int i = 0; i < v[k].size(); ++i)
		{
			...
			for (int j = 0; j <= 100; ++j)
			{
				if (freshmin(dp[i + 1][j], dp[i][j]))
				{
					pre[i + 1][j] = j;
				}
				if (freshmin(dp[i + 1][min(j + p, 100)], dp[i][j] + t))
				{
					pre[i + 1][min(j + p, 100)] = j;
				}
			}
		}
		int cost = dp.back()[100];
		cur += cost;
		if (cur > a[k]) return writeln(-1);
		for (int i = v[k].size() - 1, j = 100; i >= 0; --i)
		{
			if (pre[i + 1][j] < j)
			{
				sol.push_back(v[k][i][2] + 1);
			}
			else
			{
