...
void solve() {
  ...
  FOR(_, Q) {
    ...
    FOR(i, N) {
      ll c = C[i];
      // 
      chmax(DP[c], DP[c] + A[i] * a);
      // 
      auto upd = [&](ll frm) -> void {
        if (frm == c) return;
        chmax(DP[c], DP[frm] + A[i] * b);
      };
      upd(COL.fi);
      upd(COL.se);
      // 
      if (DP[COL.fi] < DP[COL.se]) swap(COL.fi, COL.se);
      if (c == COL.fi || c == COL.se) continue;
      if (DP[COL.fi] < DP[c]) COL = mp(c, COL.fi);
      elif (DP[COL.se] < DP[c]) COL.se = c;
    }
    ll ANS = 0;
    chmax(ANS, MAX(DP));
    print(ANS);
