...
bool is_lucky(int n)
{
	if (n==0)
		return true;
	return (n%10==4 || n%10==7) && is_lucky(n/10);
}
...
int main()
{
	...
	int key=-1;
	for (int i=0;i<n;i++) if (is_lucky(a[i])) key=i;
	if (key<0)
	{
		printf("-1\n");
		return 0;
	...
	ret.clear();
	for (int i=0;i<n;i++) qd[i]=MP(a[i],i);
	sort(qd,qd+n);
	for (int i=0;i<n;i++) d[qd[i].second]=i;
	for (int i=0;i<n;i++) visited[i]=false;
	for (int step=-1;step<n;step++)
	{
		int k=(step<0)?key:step;
		if (visited[k]) continue;
		if (d[k]==k) continue;
		int size=0;
		for (int i=k;!visited[i];i=d[i]) 
		{
			visited[i]=true;
			q[size++]=i;
		}
		ret.push_back(MP(key,q[size-1]));
		for (int i=size-2;i>=1;i--) ret.push_back(MP(q[i],q[i+1]));
		if (k!=key)
		{
			ret.push_back(MP(q[0],q[1]));
			ret.push_back(MP(key,q[0]));
		}
		else
			key=d[key];
	}
	for (int i=0;i<SIZE(ret);i++)
		swap(a[ret[i].first],a[ret[i].second]);
	for (int i=0;i<n-1;i++)
		if (a[i]>a[i+1])
		{
			printf("ERROR\n");
