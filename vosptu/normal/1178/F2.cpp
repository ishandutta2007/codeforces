...
int calc(int i, int x, int y) {
	...
	for (int j = 1; j <= n; j++) {
		...
		for (int k = i; k <= n; k++) {
			if (r[c[l[k] - 1]] == l[k] - 1) {
				g[pp] = (g[pp] + 1LL * g[c[l[k] - 1]] * dp[k][pp]) % P;
			}
		}
	}
	return g[c[y]];
	...
	for (int i = 1; i <= n; i++) {
 	...
 	for (int i = n; i; i--) {
 		...
 		for (int j = 1; j < (int) colors[i].size(); j++) {
 			if (colors[i][j - 1] + 1 != colors[i][j]) {
 				base = 1LL * base * calc(i, colors[i][j - 1] + 1, colors[i][j] - 1) % P;
 			}
 		}
		...
		memset(tmpr, 0, sizeof tmpr);
 		...
 		for (int j = i; j <= n; j++)
 			for (int k = i; k <= n; k++)
 				dp[j][k] = (dp[j][k] + 1LL * tmpl[j] * tmpr[k] % P * base) % P;
 	}
