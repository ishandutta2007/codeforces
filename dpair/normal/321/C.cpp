...
int ans[MAXN];
void dfz(const int x, int dep) {
    ans[x] = dep; vis[x] = 1; for (int i = head[x];i;i = nxt[i]) {
        if(vis[e[i]]) continue;
        root = 0; getsz(e[i], x); getroot(e[i], x, sz[e[i]]);
        dfz(root, dep + 1);
    }
} 
...
signed main(){ SZ[0] = INF;
    ...
    root = 0; getsz(1, 0); getroot(1, 0, n); dfz(root, 1);
    rep(i, 1, n) putcha(ans[i] + 64), putcha(32);
    putcha(10);
}