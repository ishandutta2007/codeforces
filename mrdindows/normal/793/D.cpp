...
int dp[81][81][81][81];
vector<pii> v[81];
int go(int cur, int l, int r, int k) {
    if (k == 0) {
        return 0;
    }
    int& res = dp[cur][l][r][k];
    if (res > 0) {
        return res - 1;
    }
    res = INF;
    for (auto p : v[cur]) {
        int to = p.first;
        if (to >= l && to < cur) {
            res = min(res, go(to, l, cur - 1, k - 1) + p.second);
        } else if (to > cur && to <= r) {
            res = min(res, go(to, cur + 1, r, k - 1) + p.second);
        }
    }
    ++res;
    return res - 1;
