...
int main() {
	...
	CASE(t) {
		...
		int sumC = (x + y) / 2 - (n - 1);
		int sumR = x - sumC;
		assert(0 <= sumR && sumR <= 2 * n - 2);
		assert(0 <= sumC && sumC <= 2 * m - 2);
		assert(sumR + sumC == x);
		assert(2 * (n - 1) - sumR + sumC == y);
		...
		int found = dig(r, c);
		if (found) {
			int res = dig(sumR - r, sumC - c);
			assert(res);
		}
		else {
			int res1 = dig(sumR - r, c);
			int res2 = dig(r, sumC - c);
			assert(res1 && res2);
		}
	}
