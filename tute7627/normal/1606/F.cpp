...
int main(){
  ...
  vector<int>ordsz(n);
  iota(ALL(ordsz),0);
  sort(ALL(ordsz),[&](int i,int j){
    return g[i].size()>g[j].size();
  });
  ll idx=0;
  vector<ll>ch(n),chc(n+1);
  rep(i,0,n){
    ch[hld.in[i]]=max(0LL,(ll)g[i].size()-(i!=0)-1);
  }
  rep(i,0,n){
    chc[i+1]=chc[i]+ch[i];
  }
  auto path=[&](ll u,ll v)->ll{
    ll sum=0;
    auto tmp=hld.query_path(u,v,false);
    for(auto z:tmp)sum+=chc[z.se]-chc[z.fi];
    sum-=ch[hld.in[u]];
    sum-=ch[hld.in[v]];
    return sum;
  };
  ...
  vector<ll>rev(n);
  rrep(k,0,sz){
    if(gq[k].empty())continue;
    while(idx<n-1&&g[ordsz[idx]].size()>=k){
      v.PB(ordsz[idx]);
      idx++;
    }
    auto nv=v;
    ...
    auto gt=hld.lca_tree(v);
    rep(i,0,v.size())rev[v[i]]=i;
    vector<vector<ll>>tq(v.size());
    for(auto z:gq[k]){
      tq[rev[z.fi]].PB(z.se);
    }
    //cout<<"k" spa k<<endl;
    ...
    auto dfs=[&](auto &&f,ll x)->ll{
      ...
      for(auto to:gt[x]){
        auto tmp=f(f,to);
        //if(k==3)cout<<"inctmp" spa inc spa tmp spa path(v[x],v[to])<<endl;
        inc+=max(0LL,tmp+path(v[x],v[to])-k*(hld.dep[v[to]]-hld.dep[v[x]]));
      }
      ll chnow=g[v[x]].size()-(v[x]!=0);
      //if(k==3)cout<<v[x] spa chnow spa inc<<endl;
      for(auto z:tq[x]){
        //if(k==3)cout<<z spa v[x] spa inc spa chnow<<endl;
        ret[z]=inc+chnow;
      }
      return inc+chnow-1;
    };
