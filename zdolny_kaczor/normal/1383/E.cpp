...
void solve() {
	...
	seq.push_back(curr);
	debug(imie(s), imie(seq));
	if (seq.size() == 1u) done("%d\n", seq[0]);
	for (int &x : seq) x++;
	ll mul = seq[0] * 1ll * seq.back() % mod;
	debug(imie(mul));
	seq.pop_back();
	seq.erase(seq.begin());
	debug(imie(seq), imie(mul));
	int n = seq.size();
	dp[n] = 1;
	vpii stack = {{nax, n + 1}};
	ll su = 0;
	for (int i = n - 1; i >= 0; --i) {
		int v = seq[i];
		while (stack.back().first <= v) {
			auto [vs, po] = stack.back();
			assert(stack.size() >= 2u);
			auto [_, rp] = stack[stack.size() - 2];
			(su -= (dp[po] - dp[rp]) * vs) %= mod;
			stack.pop_back();
		}
		auto [_, po] = stack.back();
		(su += (dp[i + 1] - dp[po]) * v) %= mod;
		stack.emplace_back(v, i + 1);
		dp[i] = su + 1;
		debug(arr(dp, i), range(seq, i, n), imie(stack));
	}
	printf("%lld\n", fix(dp[0] * mul, mod));
}
int main() {
