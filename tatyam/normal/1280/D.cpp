...
void solve(){
    ...
    rep(n)a[i]=b[i]-a[i];
    vec(vector<pll>,dp,n);
    auto merge=[](vector<pll>a,vector<pll>b){
        vector c(a.size()+b.size()-1,pll{-1,0});
        rep(a.size())rep(j,b.size())chmax(c[i+j],a[i]+b[j]);
        return c;
    };
    auto up=[](vector<pll>a){
        const ll n=a.size();
        a.push_back({-1,0});
        rrep(n)chmax(a[i+1],{a[i].first+(a[i].second>0),0});
        return a;
    };
    rep(n)dp[i]={{0,a[i]}};
    auto dfs = [&](ll from, ll at, auto dfs) -> void {
        each(i, g[at]) if(i != from){
            dfs(at, i, dfs);
            dp[at]=merge(move(dp[at]),up(move(dp[i])));
        }
    };
    dfs(-1, 0, dfs);
    out(dp[0][m-1].first+(dp[0][m-1].second>0));
}
int main(){
    LL(t);
    rep(t)solve();
