...
void solve(int l, int r) {
	if(l >= r) return;
	int len = r - l;
	int mL = rmq.query(&a[0], l, r - 1);
	int val = a[mL];
	events.push_back(mp(len, val));	//lenval
...
int main() {
	while(~scanf("%d", &n)) {
		...
		vi ans(n, -1);
		int maxi = 0;
		for(int i = n, ei = (int)events.size() - 1; i >= 1; -- i) {
			for(; ei >= 0 && i <= events[ei].first; -- ei)
				amax(maxi, events[ei].second);
			ans[i - 1] = maxi;
		}
		rep(i, n) {
			if(i != 0) putchar(' ');
