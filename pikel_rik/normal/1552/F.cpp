...
int main() {
	...
	vector<mint> sum(n), pref(n);
	sum[0] = x[0] - y[0];
	pref[0] = sum[0];

	auto range_sum = [&](int l, int r) -> mint {
		if (l <= r) {
	...
	for (int i = 1; i < n; i++) {
		int idx = lower_bound(x.begin(), x.end(), y[i]) - x.begin();
		sum[i] = range_sum(idx, i - 1) + x[i] - y[i];
		pref[i] = sum[i] + pref[i - 1];
	}
