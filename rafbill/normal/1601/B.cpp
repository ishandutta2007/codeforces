...
struct st {
  ...
  void rem_range_(int i, int a, int b, int l, int r, li const &v) {
    if(l <= a && b <= r) {
      A[i].S.erase(v);
      return;
    }
    int c = (a + b) / 2;
    if(r <= c) {
      rem_range_(2 * i, a, c, l, r, v);
    } else if(l >= c + 1) {
      rem_range_(2 * i + 1, c + 1, b, l, r, v);
    } else {
      rem_range_(2 * i, a, c, l, r, v);
      rem_range_(2 * i + 1, c + 1, b, l, r, v);
    }
  ...
  void rem_range(int l, int r, li const &v) {
    rem_range_(1, 0, n - 1, l, r, v);
  }

  template<class F>
  void query_(int i, int a, int b, int x, F&& f) {
    for(li x : A[i].S) f(x);
    if(a == b) return;
    int c = (a + b) / 2;
    if(x <= c) query_(2*i,a,c,x,f);
    else query_(2*i+1,c+1,b,x,f);
  }

  template<class F>
  void query(int x, F&& f) {
    query_(1,0,n-1,x,f);
  }
};
...
int main(){
  ...
  vi B(n); for(li i = 0; i < (li)(n); ++i) cin>>B[i];

  vvi from(n);
  for(li i = 0; i < (li)(n); ++i) from[i+B[i]].push_back(i);
  ...
  st S(n+10);
  for(li i = 0; i < (li)(n); ++i) if(A[i] != 0) {
    if(i-A[i] == -1) {
      D[i] = 1;
      Q.push(i);
      fr1[i] = -1;
    }else{
      S.add_range(i-A[i],i-1, i);
    }
  }
