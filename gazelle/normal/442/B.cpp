...
int main() {
    ...
    sort(ALL(p), [](long double & l, long double & r) { return l > r; });

    long double ans = 0;

    long double tmp_sum = 0, tmp_prod = 1;
    
    if(p[0] >= 0.5 + eps) {
         cout << fixed << setprecision(39) << p[0] << endl;
         return 0;
    }
    ...
    REP(i, n) {
        ...
        ans = max(ans, tmp_sum * tmp_prod);
    }

    cout << fixed << setprecision(39) << ans << endl;
