 ...
 * and idempotent.
 */
template<typename T, T empty_sum, typename F = function<T(const T&, const T&)>>
struct sparse_table {
    int SZ;
    F combine;
    vector<T> table;
...
int main() {
    ...
    vll diff(W + 1);
    F0R (r, N) {
        int L; re(L); vi a(L); re(a);
        sparse_table<int, INT_MIN> st(a, [](int a, int b){ return max(a,b); });

        vi imp;
        F0R (i, min(L+1, W)) imp.pb(i);
        FOR (i, max(L+1, W-L-1), W) imp.pb(i);
        dedup(imp);
        imp.pb(W);
        ...
        F0R (j, sz(imp) - 1) {
            ...
            int big = st(max(0, L-W+i), min(L, i+1));
            if (big < 0 && (i >= L || W-i > L)) big = 0;
