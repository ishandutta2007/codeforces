...
void solve() {
  ...
  vc<FastSet> X, Y;
  FOR(x, H) X.eb(FastSet(W));
  FOR(y, W) Y.eb(FastSet(H));
  FOR(x, H) FOR(y, W) X[x].insert(y), Y[y].insert(x);
  ...
  auto can = [&](ll x0, ll y0, ll x, ll y) -> bool {
    if (x < 0 || H <= x) return false;
    if (y < 0 || W <= y) return false;
    if (abs(x0 - x) > K) return false;
    if (abs(y0 - y) > K) return false;
    return G[x][y] == '.';
  };
  ...
  while (!que.empty()) {
    ...
    while (1) {
      ...
      ll lx = Y[y].prev(x - 1);
      ll rx = Y[y].next(x + 1);
      if (can(x, y, x, ly)) add(x, ly, dist[x][y] + 1);
      if (can(x, y, x, ry)) add(x, ry, dist[x][y] + 1);
      if (can(x, y, lx, y)) add(lx, y, dist[x][y] + 1);
      if (can(x, y, rx, y)) add(rx, y, dist[x][y] + 1);
      if (now == prog) break;
    }
  }
  // FOR(x, H) print(dist[x]);
  ll ANS = dist[gx][gy];
  if (ANS == INF) ANS = -1;
  print(ANS);
}
