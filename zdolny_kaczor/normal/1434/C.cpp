...
#endif
//#STAY AT HOME
ll bru(int a, int b, int c, int d) {
	if ((ll)b * c < a) return -1;
	assert(c * 1ll * d <= 10000000);
	vector <ll> delta((c + 1) * (d + 1) * 2);
	debug(imie(a), imie(b), imie(c), imie(d));
	for (int i = 0; i < 2 * c; ++i) {
		delta[i * d] += a;
		for (int j = 1; j <= c; ++j) delta[i * d + j] -= b;
		// debug(imie(delta));
	}
	ll ans = 0;
	ll curr = 0;
...
ll ans(int a, int b, int c, int d) {
	if ((ll)b * c < a) return -1;
	if (d >= c) return a;
	int can_pos = a / b;
	int top_i = can_pos / d;
	assert(top_i * d <= c);
	ll ans = a * (top_i + 1ll) - b * 1ll * d * c2(top_i + 1);
	return ans;
	// return onl(to ma si nie kompilowa) bru(a, b, c, d);
}
void solve() {
	int t;
	scanf("%d", &t);
...
void run_tests(int M) {
	REP(_, 100000) {
		int a = rand() % M + 1, b = rand() % M + 1, c = rand() % M + 1, d = rand() % M + 1;
		ll a1 = ans(a, b, c, d), a2 = bru(a, b, c, d);
		if (a1 != a2) {
			debug(imie(a), imie(b), imie(c), imie(d), "is ", imie(a1), "should ", imie(a2));
			abort();
		}
	}
}
