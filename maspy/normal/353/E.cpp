...
void solve() {
  ...
  ll N = S.size();
  vector<int> A(N + N);
  FOR(i, N) A[i + N] = A[i] = S[i] - '0';
  vector<int> B;
  FOR(i, N) if (A[i] != A[i + N - 1]) {
    B.insert(B.end(), A.begin() + i, A.begin() + i + N);
    break;
  }
  A = B;
  /*
  ...
  */
  ...
  FOR3(i, 1, N) {
    ...
    FOR(a, 2) {
      newDP[a][0] = DP[a][1];
      newDP[a][1] = min(DP[a][0] + 1, DP[a][1] + (same ? 0 : 1));
    }
    swap(DP, newDP);
  }
  ll ANS = min({DP[0][1], DP[1][0], DP[1][1]});
  print(ANS);
}
