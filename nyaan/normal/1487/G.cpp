...
void Nyaan::solve() {
  ...
  // N=2
  rep(i, 3) rep(j, 3) {
    mint x = (i == 2 ? 24 : 1) * (j == 2 ? 24 : 1);
    dp[2][(i == 0) + (j == 0)][(i == 1) + (j == 1)][i][j] = x;
  }

  reg(i, 2, N) {
    int up = (i / 2) + 10;
    rep(a, up) rep(b, up) rep(c, 3) rep(d, 3) {
      if (dp[i][a][b][c][d] == mint(0)) continue;
      mint cur = dp[i][a][b][c][d];
      rep(e, 3) {
        if (c == e and e != 2) continue;
        mint nx = cur;
        if (e == 2) {
          if (c == 2)
            nx *= 23;
  ...
  int Up = N / 2 + 10;
  // 
  rep(a, Up + 1) rep(b, Up + 1) rep(c, 3) rep(d, 3) {
    /*
    if(dp[N][a][b][c][d] != dp[N][a][b][c==2?c:c^1][d==2?d:d^1]){
    trc(N,a,b,c,d);
    assert(dp[N][a][b][c][d] == dp[N][a][b][c==2?c:c^1][d==2?d:d^1]);
    }*/
    ans += dp[N][a][b][c][d];
  }
  ...
  // 
  each(x, A) {
    reg(a, x + 1, Up + 1) rep(b, Up + 1) rep(c, 3) rep(d, 3) ans -=
        dp[N][a][b][c][d];
  }
  ...
  rep(i, 26) rep(j, i) {
    int x = A[i];
    int y = A[j];
    reg(a, x + 1, Up + 1) reg(b, y + 1, Up + 1) rep(c, 3) rep(d, 3) ans +=
        dp[N][a][b][c][d];
  }
