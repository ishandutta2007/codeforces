...
int32_t solve() {
    long long k, d, t;
    cin >> k >> d >> t;
    long long kd = (k + d - 1) / d * d;
    long double s = 1.0 / t;
    long double sv = 1.0 / (2 * t);
    
    long double full = (s * k + sv * (kd - k));
//    DBN(full, kd);
    long long cnt = 1.0 / full + 1e-12;
    long double left = 1.0 - cnt * full;
    long double ans = cnt * kd;
    if (s * k >= left) {
        ans += left / s;
    } else {
        ans += k + (left - s * k) / sv;
