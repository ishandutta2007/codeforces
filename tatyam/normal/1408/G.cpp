...
vector<Modint> operator*(const vector<Modint>&a,const vector<Modint>&b){
    vector<Modint>ans(a.size()+b.size()-1);
    rep(a.size())rep(j,b.size())ans[i+j]+=a[i]*b[j];
    return ans;
}
struct UnionFind{
    vector<ll>data;
    ...
    vector<ll>sz;
    UnionFind(ll n):data(n,-1),f(n,{0,1}),sz(n){}
...
int main(){
    ...
    VV(ll,a,n,n);
    vector<tuplis>edge;
    rep(n)rep(j,i+1,n)edge.push_back({i,j,a[i][j]});
    sort(all(edge),[](auto a,auto b){return a[2]>b[2];});
    UnionFind uf(n);
    rep(n*(n-1)/2+1){
        vector<ll>p;
        while(edge.size()&&edge.back()[2]==i){
            auto [x,y,z]=edge.back();
            edge.pop_back();
            uf.unite(x,y);
            p.push_back(x);
            p.push_back(y);
        ...
        each(i,p){
            const ll x=uf[i];
            if(uf.sz[x]==uf.size(x)*(uf.size(x)-1)/2)uf.f[x][1]=1;
        }
    }
    auto ans=move(uf.f[uf[0]]);
    ans.erase(ans.begin());
    out(ans);
}