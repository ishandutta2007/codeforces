#line 1 "codeforces-workspace\\contest\\1580\\a\\a.cpp"
...
#line 13 "codeforces-workspace\\contest\\1580\\a\\a.cpp"
...
void main() {
  ...
  for (auto u : range(4, H)) {
    vector<int> col(W);

    for (auto l : rrange(u + 1)) {  // rows in [l, u]
      for (auto y : range(W)) {     // update col
        col[y] += 1 - grid[l][y];
      }
      ...
      if (width >= 5) {
        ...
        for (auto y : range(W)) {  // check (x, y)
          if (wait.size() > 2) {
            chle(best, wait.front());
            wait.pop();
          }

          auto cost =
              width - 2 - (col[y] - (1 - grid[l][y]) - (1 - grid[u][y]));

          chle(ans, best + sum + cost);
          sum += col[y];
          wait.emplace(-sum + cost);
        }
      }
