...
void solve(){
    ...
    Sort(s);
    const ll n=s.size();
    if(count(all(s),s[0])==n)return(void)out(s);
    rep(i,'a','z'+1)if(count(all(s),i)==1){
        auto p=find(all(s),i);
        rotate(s.begin(),p,p+1);
        return(void)out(s);
    }
    const char c1=s[0];
    ll cnt1=count(all(s),c1);
    if(cnt1<=n/2+1){
        rep(i,cnt1-2)swap(s[cnt1+i],s[i*2+2]);
        out(s);
        return;
    }
    const char c2=s[cnt1];
    ll cnt2=count(all(s),c2);
    if(cnt1+cnt2==n){
