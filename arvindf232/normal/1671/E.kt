...
fun main(){
    solve.cases{
        val n = getint
        val typecache = mutableMapOf<Triple<Int,Int,Int>,Int>()

        fun register(a:Triple<Int,Int,Int>):Int{
            if(typecache[a] == null){
                typecache[a] = typecache.size
            }
            return typecache[a]!!
        }
        val atype = register(Triple(0,-1,-1))
        val btype = register(Triple(0,-2,-2))

        val str = getstr
        val C = CharArray(1 shl n){if(it == 0) 'X' else str[it-1]}
        val type = IntArray(1 shl n)
        for(i in (1 shl n) - 1 downTo 1) {
            if(i >= (1 shl (n-1))){
                type[i] = if(C[i] == 'A') atype else btype
            }else{
                var left = type[i shl 1]
                var right = type[(i shl 1) + 1]
                if(left > right){
                    left = right.also { right = left }
                }
                type[i] = register(Triple(if(C[i] == 'A' )1 else 0, left,right))
            }
        }
        ...
        for(i in (1 shl n) - 1 downTo 1){
            if(i >= (1 shl (n - 1))) {
                ...
                if(type[i shl 1] == type[(i shl 1) + 1]) {
                    val v = DP[i shl 1]
                    DP[i] = (v modM v)
                }else{
                    DP[i] = DP[i shl 1 ] modM DP[(i shl 1) + 1 ] modM 2
                }
            }
        }
        put(DP[1])
