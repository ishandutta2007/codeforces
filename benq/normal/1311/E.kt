...
var cur = 0
val par = IntArray(5001)
val mn = IntArray(5001)
val mx = IntArray(5001)

fun con(n: Int, _d: Int, p: Int) {
    if (n == 0) return
    var d = _d
    d -= n
    val label = ++cur
    par[label] = p
    for (i in 0..n-1) {
        if (mn[i]+mn[n-1-i] <= d && d <= mx[i]+mx[n-1-i]) {
            val dif = minOf(d-mn[i]-mn[n-1-i],mx[i]-mn[i])
            con(i,mn[i]+dif,label)
            con(n-1-i,d-mn[i]-dif,label)
            return
        }
    }
    assert(false)
...
fun PrintWriter.solve() {
    ...
    for (i in 1..5000) {
        ...
        mx[i] = -MOD
        for (j in 0..i-1) {
            mn[i] = minOf(mn[i],mn[j]+mn[i-1-j])
            mx[i] = maxOf(mx[i],mx[j]+mx[i-1-j])
        }
        mn[i] += i
        mx[i] += i
    }
