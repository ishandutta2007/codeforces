...
signed main(){
	...
	vc<pi> ps(n);
	rep(i,n)ps[i]=readpi();
	rep(i,n)rep(j,n)rep(k,n)val[i][j][k]=tri2(ps[i],ps[j],ps[k]);
	rep(i,n)dp[i][(i+1)%n]=pi(0,0);
		...
		rng(w,2,n+1){
			rep(i,n){
				...
				while(y!=j){
					...
					pi r(p.a+q.a,p.b+q.b+val[i][x][j]);
					if(r.b>=mid){
						r.a++;
						r.b=0;
					}
