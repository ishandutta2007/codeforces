...
void dfs(const int& v, const vvi& g, vb& nv, vi& kva)
{
	nv[v] = false;
...
pii good_gcd(const int& x, const int& y)
{
	if (y == 0)
		return pii(1, 0);
	else
	{
		int a = x / y;
		pii old = good_gcd(y, x - a * y);
		return pii(old.Y, old.X - a * old.Y);
	}
}
int obr(const int& x, const int& M)
{
	pii p = good_gcd(x, M);
	int a = p.X;
	while (a < 0)
...
const int p1 = 5211, p2 = 1486;
int o1, o2;
vi pw1, pw2, ow1, ow2, hsh1, hsh2;
int find_hash(const int& x, const int& y, const vi& ow, const vi& hsh, const int& M)
{
	int r = raz(hsh[y], hsh[x], M);
	r = prod(r, ow[x], M);
	return r;
}
pii find_hash(const int& x, const int& y)
{
	int h1 = find_hash(x, y, ow1, hsh1, M1);
	int h2 = find_hash(x, y, ow2, hsh2, M2);
	return pii(h1, h2);
}
...
int main()
{
	...
	for (int i = 0; i < m; ++i)
	{
		pw1[i + 1] = prod(pw1[i], p1, M1);
		pw2[i + 1] = prod(pw2[i], p2, M2);
		ow1[i + 1] = prod(ow1[i], o1, M1);
		ow2[i + 1] = prod(ow2[i], o2, M2);
	}
	hsh1.resize(m + 1);
	hsh2.resize(m + 1);
	for (int i = 0; i < m; ++i)
	{
		hsh1[i + 1] = sum(hsh1[i], prod(pw1[i], t[i], M1), M1);

		hsh2[i + 1] = sum(hsh2[i], prod(pw2[i], t[i], M2), M2);
	}
	vi zeros, ones;
	vb b(n);
	for (int i = 0; i < n; ++i)
	...
	{
		ll sz0 = ((ll)s0) * sz(zeros);
		ll sz1 = m - sz0;
		if ((sz1 > 0) && (sz1 < m) && (sz1 % sz(ones) == 0))
		{
			int s1 = sz1 / sz(ones);
			bool ok = true;
			pii hesh0(-1, -1), hesh1(-1, -1);
			int cur = 0;
			for (int i = 0; ok && (i < n); ++i)
			{
				...
				{
					new_cur = cur + s1;
					pii real_hash = find_hash(cur, new_cur);
					if (hesh1.X == -1)
						hesh1 = real_hash;
					else if (hesh1 != real_hash)
						ok = false;
				}
				else
				{
