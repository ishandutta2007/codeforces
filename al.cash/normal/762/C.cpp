...
int main() {
  ...
  REP(x, A) r[n][x] = n;
  FORD(i, n-1, 0) {
    REP(x, A) r[i][x] = r[i+1][x];
    r[i][a[i]] = i;
  }
  lc[0] = 0;
  REP(j, m) {
    if (lc[j] == n+1) lc[j+1] = n+1;
    else lc[j+1] = r[lc[j]][b[j]] + 1;
  }
//  writeln(lc, m+1);

  REP(x, A) l[0][x] = 0;
  FOR(i, 1, n+1) {
    REP(x, A) l[i][x] = l[i-1][x];
    l[i][a[i-1]] = i;
  }
  rc[m] = 0;
  FORD(j, m-1, 0) {
    if (rc[j+1] == n+1) rc[j] = n+1;
    else rc[j] = n - l[n-rc[j+1]][b[j]] + 1;
  }
//  writeln(rc, m+1);
...
//  writeln(ans, p);
  ...
  if (ans == m) writeln('-');
    ...
    REP(j, m) b[j] += 'a';
    output->writeString(b, p.X);
    output->writeString(b + p.Y, m - p.Y);
    writeln();
  }
