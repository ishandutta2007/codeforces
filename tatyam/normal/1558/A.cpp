...
void solve(){
    ...
    set<ll>ans;
    {
        const ll sa=(a+b+1)/2,sb=(a+b)/2;
        rep(ka,0,sa+1)if(ka<=a){
            const ll ba=a-ka,kb=sb-ba,bb=b-kb;
            if(sa==ka+bb&&0<=min(kb,bb))ans.insert(ba+bb);
        }
    }
    {
        const ll sa=(a+b)/2,sb=(a+b+1)/2;
        rep(ka,0,sa+1)if(ka<=a){
            const ll ba=a-ka,kb=sb-ba,bb=b-kb;
            if(sa==ka+bb&&0<=min(kb,bb))ans.insert(ba+bb);
        }
