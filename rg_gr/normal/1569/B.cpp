...
{
	int T=read();
	rg(T)int n=read(),cc=0;
	rg(n)s[i]=re1d();if(!s[i])f[++cc]=i;gr
	memset(a,'=',sizeof(a));
	rg(n)a[i][i]='X';gr
	if(cc==1||cc==2){puts("NO");continue;}
	puts("YES");
	rg(cc-1)a[f[i]][f[i+1]]='+',a[f[i+1]][f[i]]='-';gr
	a[f[cc]][f[1]]='+',a[f[1]][f[cc]]='-';
	rg(n)rg_(j,n)putchar(a[i][j]);gr putchar(10);gr
	gr
	return 0;
}