...
void pdfs(int v,int p) {
    ...
    for(int i=0; i<deg; ++i) {
        l[v][i+1]=l[v][i];
        if(tre[v][i]!=p) l[v][i+1]+=dp[tre[v][i]];
    }
    for(int i=deg-1; i>=0; --i) {
        r[v][i]=r[v][1+i];
        if(tre[v][i]!=p) r[v][i]+=dp[tre[v][i]];
    }
    sz[v]++;
}

void dfs(int v,int p,i64 tmp) {
    tmp+=n-sz[v];
    dp2[v]=tmp+dp[v];
    int deg=tre[v].size();
    for(int i=0; i<deg; ++i) {
        if(tre[v][i]==p) continue;
        dfs(tre[v][i],v,tmp+dp[v]-dp[tre[v][i]]-sz[tre[v][i]]);
    }
}
