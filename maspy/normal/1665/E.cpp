...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/ds/waveletmatrix.hpp"
#include <immintrin.h>
...
vector<T> 
 O(log N) 
freq(l, r, lower, upper)[lower, upper) 
kth(l, r, lower, upper)[lower, upper)  k 
*/
template <typename T>
...
void solve() {
  ...
  FOR_(Q) {
    ...
    vc<int> vals;
    ll n = min(32LL, R - L);
    FOR(i, n) vals.eb(WM.kth(L, R, i));
    ll ANS = 1LL << 60;
    FOR(i, n) FOR(j, i) chmin(ANS, vals[i] | vals[j]);
    print(ANS);
  }
}
