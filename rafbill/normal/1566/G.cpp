...
map<int,li> G[N];
set<tuple<li,int>> W[N];
array<int, 3> C3[N];
set<tuple<li,int,int>> S;
set<tuple<li,int>> SW;
li LW[N];

bool test(li u, li v) {
  return find(begin(C3[v]), end(C3[v]),u) != end(C3[v]);
}

void add(int u){
  int cnt = 0;
  li sw = 0;
  for(auto const& [w,v] : W[u]) {
    if(test(u,v)) {
      S.insert(make_tuple(w,min(u,v),max(u,v)));
    }
    LW[u] = w;
    sw += w;
    C3[u][cnt] = v;
...
void del(int u){
  ...
  for(auto const& [w,v] : W[u]) {
    if(test(u,v)) {
      S.erase(make_tuple(w,min(u,v),max(u,v)));
    }
    sw += w;
    C3[u][cnt] = -1;
    cnt++;
    if(cnt == 3) {
      SW.erase(make_tuple(sw,u));
      break;
    }
  }
};
...
void readDel(){
  ...
  if(find(begin(C3[u]), end(C3[u]),v) == end(C3[u]) && find(begin(C3[v]), end(C3[v]),u) == end(C3[v])) {
    W[u].erase(pii(w, v));
    W[v].erase(pii(w, u));
  }else{
    del(u); del(v);
...
vector<tuple<li,int,int>> T;
void print (){
  li ans = 1ll<<50;
  if(!SW.empty()) ans = get<0>(*begin(SW));

  T.clear();
  for(auto p : S) {
    T.emplace_back(p);
    if((li)T.size() == 7) break;
  }

  for(li i = 0; i < (li)(T.size()); ++i) for(li j = (i+1); j <= (li)((li)T.size()-1); ++j) {
    li w1; int i1,j1; tie(w1,i1,j1) = T[i];
    li w2; int i2,j2; tie(w2,i2,j2) = T[j];
    if(w1+w2>ans) break;
...
int main() {
  ...
  cin>>n>>m;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(3); ++j) C3[i][j] = -1;
  for(li i = 0; i < (li)(n); ++i) LW[i] = -1;
