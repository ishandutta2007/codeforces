...
void work()
{
    rin>>n;
    int ans=0;
    for(int i=1;i<=n;i++)rin>>a[i],b[i]=c[i]=0;
    for(int i=1;i<=n;i++)if(!b[a[i]])b[a[i]]=1,c[i]=a[i],ans++;
    
    int kakaa=0;dt=qt=0;
    for(int i=1,j=1;i<=n;i++){if(!c[i])q[++qt]=i;if(!b[i])d[++dt]=i;}
    if(dt==1&&qt==1&&q[qt]==d[dt])
    {
        for(int i=1;i<=n;i++)if(c[i]==a[d[1]]){c[i]=d[1];c[d[1]]=a[d[1]];break;}
        printf("%d\n",ans);
    ...
    int cutt=0,lst;
    for(int i=1;i<=qt;i++)if(q[i]==d[i])cutt++,lst=i;
    if(cutt==1)
    {
        if(lst==1)jh(d[lst],d[2]);else jh(d[lst],d[1]);
        for(int i=1;i<=qt;i++)c[q[i]]=d[i];
        for(int i=1;i<=n;i++)printf("%d ",c[i]);puts("");
        return;
    }
    dt=0;
    for(int i=1;i<=qt;i++)if(q[i]!=d[i])c[q[i]]=d[i];else d[++dt]=d[i];
    for(int i=1;i<dt;i++)c[d[i]]=d[i+1];c[d[dt]]=d[1];
    for(int i=1;i<=n;i++)printf("%d ",c[i]);puts("");
    return;
}
int main()
{
    int T;rin>>T;for(;T;T--)work();
    return 0;
}