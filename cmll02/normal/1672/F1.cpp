...
signed main()
{
	...
	rg(T)
	int n=read();
	rg(n)a[i]=read(),t[i]=0;gr
	rg(n)t[a[i]]++;gr 
	rg(n)s[i].clear();gr
	rg(n)s[a[i]].insert(i);gr
	int ss=0;rg(n)mxe(ss,t[i]);gr
	if(0)
	{
		int p=0;
		rg(n)if(t[i]==ss)p=i;gr
		int l=1,r=1;
		while(1)
		{
			while(a[l]==p&&l<=n)l++;
	...
	else
	{
		...
		rg(n)if(t[i])q.push(std::make_pair(t[i],i));gr
		while(q.size()>1)
		{
			std::vector<int>v;
			...
			{
				auto u=q.top();q.pop();
				v.push_back(*s[u.second].begin());s[u.second].erase(s[u.second].begin());
				if(--u.first)q2.push(u);
			}
			q.swap(q2);
			int p=a[v[0]];
