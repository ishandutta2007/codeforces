...
void solve() {
	string s; cin >> s;
    int n = (int)s.size();
    static int dp[1010][2];
    static int dp2[1010][2];
    rep(i, 1010) {
        rep(j, 2) {
            dp[i][j] = INF;
            dp2[i][j] = INF;
    ...
    rep(i, n) {
        if(s[i] == '0') {
            ...
            //
            chmin(dp2[i + 1][0], dp2[i][0] + 1);
            chmin(dp2[i + 1][0], dp2[i][1] + 1);
            chmin(dp2[i + 1][1], dp2[i][1]);
        }
    }
    cout << min({dp[n][0], dp[n][1], dp2[n][0], dp2[n][1]}) << endl;
}
