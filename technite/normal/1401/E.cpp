...
ll qry(ll l1,ll r1,ll i=1,ll l2=0,ll r2=n-1)
{
    if(l1<=l2&&r2<=r1)
    ...
    ll m2=(l2+r2)/2;
    psh(i,l2,m2,r2);
    /*for min*///return min(l1<=m2?qry(l1,r1,2*i,l2,m2):(ll)1e9,m2<r1?qry(l1,r1,2*i+1,m2+1,r2):(ll)1e9);
...
int main()
{
    ...
    int h,z;
    cin>>h>>z;
    vector<pair<int,pi>> v1(h+1);
    vector<pair<pi,int>> v2(z);
    v1[0]={0,{0,1e6}};
    long long ans=1;
    for(int i=1;i<=h;i++)
    {
    ...
    for(int i=0;i<z;i++)
    {
        int x,l,r;
        cin>>x>>l>>r;
        v2[i]={{min(l,r),max(l,r)},x};
    }
    sort(all(v2));
    int i=0,j=0;
    for(auto vv:v2)
    {
        int x=vv.S,l=vv.F.F,r=vv.F.S;
        while(v1[i].F<=r&&i<v1.size())
        {
            upd2(v1[i].S.F,v1[i].S.S,1);
            i++;
        }
        while(v1[j].F<l&&j<v1.size())
        {
            upd2(v1[j].S.F,v1[j].S.S,-1);
            j++;
        }
        ans+=max(qry(x,x)-1,0ll);
    }
    cout<<ans<<'\n';
}