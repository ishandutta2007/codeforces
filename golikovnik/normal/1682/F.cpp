...
void solveTest() {
  ...
  vector<tuple<ll, int, int, int>> qs[2];

  auto addQuery = [&](int tp, int l, int r, ll x, int i) {
    qs[tp].emplace_back(x, l, r, i);
  };
  ...
  for (int i = 0; i < q; ++i) {
    ...
    --l;
    ll X = pb[l];
    addQuery(0, l + 1, r, X, i);
    addQuery(1, l, r - 1, X, i);
    // ll ans = 0;
    // for (int i = l + 1; i < r; ++i) {
    //   ans += a[i] * llabs(pb[i] - X);
    // }
    // for (int i = l; i < r - 1; ++i) {
    //   ans -= a[i] * llabs(pb[i + 1] - X);
    // }
    // cout << ans << '\n';
  }
  ...
  auto solve = [&](int ti) {
    ...
    for (int i = 0; i < n; ++i) {
      ff[i] = pb[i + ti];
      f[i] = make_pair(ff[i], i);
    }
    sort(f.begin(), f.end());
    int ptr = 0;
    fenwick<mint> fc(n), fk(n);
    for (int i = 0; i < n; ++i) {
      fc.upd(i, a[i] * ff[i]);
      fk.upd(i, -a[i]);
    }
    for (auto[X, l, r, id] : qq) {
      while (ptr < n && f[ptr].first <= X) {
        int i = f[ptr].second;
        fc.upd(i, -2 * a[i] * ff[i]);
        fk.upd(i, 2 * a[i]);
        ++ptr;
      }
      ans[id] += (1 - 2 * ti) * (fc.get(l, r) + fk.get(l, r) * X);
    }
  };
