...
int main(){
	scan(n,m,ta,tb,k);
	for(int i=1;i<=n;++i) gi(a[i]); sort(a+1,a+n+1);
	for(int i=1;i<=m;++i) gi(b[i]); sort(b+1,b+m+1);
	if(n<=k||m<=k) puts("-1"),exit(0);
	for(int i=1;i<=k+1;++i){
		int x=lower_bound(b+1,b+m+1,a[i]+ta)-b+(k+1-i);
		if(x>m) puts("-1"),exit(0);
		chkmax(ans,b[x]);
	}
	cout<<i64(ans)+tb<<endl;
}