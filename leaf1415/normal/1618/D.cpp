...
int main(void)
{
	...
	while(T--){
		...
		vector<ll> bunsi, bunbo;
		rep(i, 1, n){
			if(i <= n-k) bunsi.push_back(a[i]);
			else bunbo.push_back(a[i]);
		}
		while(sz(bunbo) < sz(bunsi)) bunbo.push_back(1);
		sort(all(bunsi)), sort(all(bunbo));
		
		ll ans = 0;
		reps(i, bunsi) ans += bunsi[i] / bunbo[i];
		outl(ans);
	}
