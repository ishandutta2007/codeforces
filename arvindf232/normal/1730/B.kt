...
fun main(){
    solve.cases{
        ...
        fun solve(t:Int):Int?{
            ...
            for(i in 0 until n){
                val extratime = t - dress[i]
                rmin = maxOf(rmin,Pos[i] - extratime)
                rmax = minOf(rmax, Pos[i] + extratime )
            }
            if(rmin > rmax){
                return null
            }else{
...
//        just dei solve(2)
        val possible = BinarySearchFirstTrue(0,500000000){
            solve(it) != null
        } !!

        put(solve(possible)!!.toDouble() / 2.0)
