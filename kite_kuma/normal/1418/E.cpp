...
int main() {
	...
	INT(n, m);
	VEC(int, d, n);
	vector<int> dur(m), def(m);
	rep(i, m) { cin >> dur[i] >> def[i]; }
	sort(all(d));
	vll acc(n + 1);
	rep(i, n) { acc[i + 1] = acc[i] + d[i]; }

	auto sum = [&](int from, int to) { return acc[to] - acc[from]; };
	...
	rep(i, m) {
		...
		int it = lower_bound(all(d), b) - d.begin();
		...
		mint sumlow = acc[it] - acc[0];
		mint sumup = acc[n] - acc[it];
		debug(sumlow, sumup);
		ll nokori = n - it;
		dump(nokori);
		if(nokori >= a) res += sumlow / (nokori + 1) * (nokori - a + 1);
		if(nokori - 1 >= a) res += sumup / (nokori) * (nokori - a);
		// res /= fac[n];
		ans[i] = res;
	}
	vout(ans, 1);
