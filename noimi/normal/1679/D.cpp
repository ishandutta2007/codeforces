...
int main() {
    ...
    LL(k);
    VEC(int, a, n);
    auto g = getG(n, m, true);
    Graph G(n);
    int res = bin_search(inf<int>, 0, [&](int x) {
        rep(i, n) G[i].clear();
        vi deg(n);
        rep(i, n) fore(e, g[i]) {
            if(a[i] <= x and a[e] <= x) {
        ...
        vl dp(n, -1);
        vi used(n);
        rep(i, n) if(!deg[i] and a[i] <= x) q.emplace(i);
        while(!empty(q)) {
            int x = q.front();
            q.pop();
            used[x] = 1;
            if(dp[x] == -1) dp[x] = 1;
            fore(e, G[x]) {
                if(!--deg[e]) q.emplace(e), chmax(dp[e], dp[x] + 1);
            }
        }
        // if(x == 2) { dump(dp, used, G, deg); }
        rep(i, n) {
            if(a[i] <= x and !used[i]) return true;
        }
        return MAX(dp) >= k;
    });
    if(res == inf<int>)
