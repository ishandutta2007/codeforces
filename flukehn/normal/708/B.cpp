...
char s[N];
int p[N];
bool work(int a,int b,int c,int d,int n,int m){
	if(C(n)!=a||C(m)!=d)return 0;
	if(b+c!=1ll*n*m)return 0;
	if(n){
		p[n]=b/n;
		//if(c)++p[c];
		if(b%n)p[b%n]++;
	}
	if(n)p[0]=m-(b/n)-(b%n!=0);
	else p[0]=m;
	int cnt=0;
	rep(i,1,p[0])s[++cnt]='1';
	rep(i,1,n){
...
int main(){
	...
	if(work(a,b,c,d,n,m)||
	work(a,b,c,d,n+1,m+1)||
	work(a,b,c,d,n+1,m)||
	work(a,b,c,d,n,m+1))return 0;
	NOI();
	debug("ok\n");
