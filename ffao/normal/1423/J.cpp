...
int solve(long long m, int carry, int idx) {
    if (idx == -1) return carry == 0;
    int &ret = pd[carry][idx];

    if (ret == -1) {
        ret = 0;

        for (int coef = 0; coef <= min(carry, 7); coef++) {
            int nxt = (carry-coef)<<1;
            if (idx > 0) nxt += ((m>>(idx-1))&1);
            if (nxt < 14) ret = (ret + solve(m, nxt, idx-1)) % mod;
        }
    }
