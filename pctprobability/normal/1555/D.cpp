...
void solve(){
  ...
  string s;
  cin>>s;
  vector<ll> a(n+100),b(n+100),c(n+100);
  for(int i=0;i<n;i++){
    if(s[i]=='a'){
      a[i+4]++;
    }
  ...
  while(m--){
    ...
    ll ans=0;
    vector<ll> A={0,1,2};
  // cout<<p[0]<<" "<<p[1]<<" "<<p[2]<<" "<<q[0]<<" "<<q[1]<<" "<<q[2]<<endl;
    do{
      ll res=0;
      res+=a[q[A[0]]+4]-a[p[A[0]]+1];
      res+=b[q[A[1]]+4]-b[p[A[1]]+1];
      res+=c[q[A[2]]+4]-c[p[A[2]]+1];
      chmax(ans,res);
     // cout<<ans<<" "<<res<<" "<<a[q[A[0]]+3]-a[p[A[0]]]<<" "<<b[q[A[1]]+3]-b[p[A[1]]]<<" "<<c[q[A[2]]+3]-c[p[A[2]]]<<endl;
    }while(NP(all(A)));
    cout<<r-l+1-ans<<endl;
  }
}
