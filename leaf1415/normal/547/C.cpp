...
ll get(ll x)
{
	...
	ll n = sz(vec), N = 1<<n, ret = 0;
	rep(i, 0, N-1){
		ll mul = 1;
		rep(j, 0, n-1) if(i & (1<<j)) mul *= vec[j];
		if(popcount(i) % 2) ret -= cnt[mul];
		else ret += cnt[mul];
	}
	return ret;
}
...
int main(void)
{
	...
	make_prime(500005), make_qrime(500005);
	rep(i, 1, 500000) for(int j = i; j <= 500000; j+=i) vec[j].pb(i);
