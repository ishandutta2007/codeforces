...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/alg/monoid_xor_basis.hpp"
...
ll calc_dim(int H, int W) {
  using Mono = Monoid_XorBasis<ll, 60>;
  auto ID = [&](ll x, ll y) -> ll { return W * x + y; };
  auto X = Mono::unit();
  FOR(x, H) FOR(y, W) {
    ll s = 0;
    FOR(i, W) s |= 1LL << ID(x, i);
    FOR(i, H) s |= 1LL << ID(i, y);
    Mono::add_element(X, s);
  }
  ll dim = 0;
  for (auto&& x: X)
    if (x) dim++;
...
void aaa() {
  vv(ll, dp, 10, 10);
  FOR(x, 10) FOR(y, 10) if (x * y <= 60) { dp[x][y] = x * y - calc_dim(x, y); }
  FOR(x, 10) FOR(y, 10) if (x * y % 2 == 1) print(x, y, dp[x][y]);
}
...
void solve() {
  ...
  */
  UnionFind uf(H + W);
  FOR(x, H) FOR(y, W) if (S[x][y] == '?') uf.merge(x, H + y);
  vi E(H + W), deg(H + W), V(H + W);
  FOR(x, H) FOR(y, W) {
    if (S[x][y] == '0') continue;
    if (S[x][y] == '1') {
      deg[uf[x]] += 1;
  ...
  FOR(i, H + W) if (uf[i] == i) {
    mint a = [&]() -> mint {
      if ((deg[i] + V[i] * 0) % 2 == 1) return 0;
      int n = V[i];
      int m = E[i];
      assert(m >= n - 1);
      ll dim = m - n + 1;
      return mint(2).pow(dim);
    }();
    mint b = [&]() -> mint {
      if ((deg[i] + V[i] * 1) % 2 == 1) return 0;
      int n = V[i];
      int m = E[i];
      assert(m >= n - 1);
      ll dim = m - n + 1;
