...
int solve(const int *vs, const int n){
	cauto N=accumulate(vs,vs+n,0);
	int re=ceil(N)+1;
	int sum=0;
	int li=0;
	for(int pw=1;pw<N;pw+=pw){
		while(sum+vs[li]<=pw){
			sum+=vs[li];
			++li;
		}
...
int main(){
	...
	TT{
		cauto N=read(0);
		map<int,int> cnts;
		for(cauto &v:readV(N)) ++cnts[v];
		cauto n=(int)cnts.size();
		vector<int> vs;
		for(cauto &p:cnts) vs.eb(p.Y);
		out(vs,1);
		int re=ceil(N)+2;
		int sum=0;
		int li=0;
