...
int main() {
    INT(n);
    Wgraph<int> g = getWg<int>(n);

    ll ans = 0;
    REC([&](auto &&f, int x, int p) -> tuple<ll, ll, map<int, pii>> {
        ll S = 1, s = 0;
        map<int, pii> mp;
        fore(e, g[x]) {
            if(e == p) continue;
            ...
            t -= m[e.cost].fi;
            t += T - m[e.cost].se;
            m[e.cost] = pii(T - m[e.cost].se, T);
            ans += 1LL * s * T + 1LL * S * t;
            dump(S, s, mp, T, t, m);
            if(si(mp) > si(m)) {
                fore(x, e, m) {
                    if(mp.count(x)) { ans -= 1LL * e.fi * mp[x].se + 1LL * e.se * mp[x].fi; }
                }
                fore(x, y, m) { mp[x] += y; }
            } else {
                fore(x, e, mp) {
                    if(m.count(x)) ans -= 1LL * e.fi * m[x].se + 1LL * e.se * m[x].fi;
                }
                fore(x, y, mp) m[x] += y;
                swap(m, mp);
            }
