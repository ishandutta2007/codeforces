...
class Solver {
    ...
    public void solve() {
        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            Array.Reverse(a);
            Array.Reverse(b);
            var aimos = new int[n + 1];
            var bimos = new int[n + 1];
            for (int i = 0; i < n; i++)
            {
                aimos[i + 1] = aimos[i] + a[i];
                bimos[i + 1] = bimos[i] + b[i];
            }
            int ng = -1, ok = n + 9;
            while (ok - ng > 1) {
                int m = (ok + ng) / 2;
                int N = n + m;
                int k = N - N / 4;
                int asum = 100 * Math.Min(m, k) + aimos[Math.Max(k - m, 0)];
                int bsum = bimos[Math.Min(k, n)];
                if (asum >= bsum) {
                    ok = m;
                }
                else {
