...
void solve(){
	...
	int ans = 1e18;
	rep(on,10)rep(tw,10){
		vc<int>mn(3, INF), mx(3, -INF);
		rep(a,on+1)rep(b,tw+1) {
			chmax(mx[(a+b*2)%3], a+b*2);
			chmin(mn[(a+b*2)%3], a+b*2);
		}
		int tmp = on+tw, mxv = 0;
		rep(i, n){
			int rem = a[i] % 3;
			if(mx[rem] < 0 or mn[rem] > a[i]) goto nxt;
			int zan = a[i] - mx[rem];
			if(zan > 0) chmax(mxv, zan/3);
		}
		//if(tmp <= 2) cout<<on<<" "<<tw<<" "<<mxv<<endl;
		chmin(ans, tmp + mxv);
		nxt:;
	}
