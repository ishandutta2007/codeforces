...
int n;
int a[30][30],p[30][30],skip[30][30],np[30][30];
int dp[2][2097152],sm[22][2097152];

int solve(){
	memset(dp,0,sizeof(dp));
	dp[0][msk2(n)] = 1;int cur = 0;
	int tmul = 1;
	...
	F0R(i,1<<n){
		...
		int c2 = 1;
		F0R(j,n){
			c2 = mul(c2,sub(1,sm[j][(msk2(n)-i)&z[j]]));
		}
		if(pct(i)%2){
			ans = sub(ans,mul(cc,c2));
		}else{
			ans = add(ans,mul(cc,c2));
		}
	}
	return mul(tmul,ans);
}
...
int main() {
	re(n);
	int cinv = mpow(10000,mod-2);
	F0R(i,n){
		F0R(j,n){
			re(a[i][j]);
			p[i][j] = a[i][j]*cinv%mod;
		}
	}
	F0R(i,n){
		F0R(j,1<<n){
	...
	ans = add(ans,sub(1,solve()));
	memset(skip,0,sizeof(skip));
	F0R(i,n)skip[i][i] = 1;
	ans = add(ans,solve());
	memset(skip,0,sizeof(skip));
	F0R(i,n)skip[i][n-1-i] = 1;
	ans = add(ans,solve());
	F0R(i,n)skip[i][i] = 1;
	ans = sub(ans,solve());
	ps(ans);
