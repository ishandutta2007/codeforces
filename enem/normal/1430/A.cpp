...
int main()
{
    ...
    */
    ...
    REP(i,0,1001)
    {
        if(dp[i]!=MT(-1,-1,-1))
        {
            dp[i+3]=MT(G(0,dp[i])+1,G(1,dp[i]),G(2,dp[i]));
            dp[i+5]=MT(G(0,dp[i]),G(1,dp[i])+1,G(2,dp[i]));
            dp[i+7]=MT(G(0,dp[i]),G(1,dp[i]),G(2,dp[i])+1);
        }
    }
    REP(tc,1,ntc+1)
    {
        ...
        ll n;
        cin>>n;
        if(dp[n]==MT(-1,-1,-1)) cout<<-1;
        else cout<<G(0,dp[n])<<' '<<G(1,dp[n])<<' '<<G(2,dp[n]);
