...
void solve() {
	...
	for (int i = 2; i <= n; ++i) {
		for (int j = 0; j <= n; ++j) {
			if (a[i] != 0) {
				dp[i][j][a[i] % 2] = min(dp[i - 1][j][a[i] % 2], dp[i - 1][j][(a[i] % 2) ^ 1] + 1);
				continue;
			}
			for (int k = 0; k < 2; ++k) {
				if (k == 0 && j == 0)
					continue;
				dp[i][j][k] = min(dp[i - 1][j - (!k)][k], dp[i - 1][j - (!k)][k ^ 1] + 1);
			}
		}
	}
	vb used(n + 1);
	...
	cout << min(dp[n][cnt][0], dp[n][cnt][1]);
}
