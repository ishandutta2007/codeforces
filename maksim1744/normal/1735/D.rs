...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    let k: usize = cin.read();

    let mut v = Vec::new();
    for _ in 0..n {
        v.push(cin.read_vec::<i32>(k));
    }

    let mut ind = HashMap::new();

    for (i, card) in v.iter().enumerate() {
        ind.insert(card.clone(), i);
    }
    ...
    for i in 0..n {
        for j in i + 1..n {
            let third: Vec<i32> = v[i]
                .iter()
                .zip(v[j].iter())
                .map(|(x, y)| (30 - x - y) % 3)
                .collect();
            if let Some(&ind) = ind.get(&third) {
                assert_ne!(i, ind);
                assert_ne!(j, ind);
                if ind <= j {
                    continue;
    ...
    outln!(cnt.into_iter().map(|x| x * (x - 1) / 2).sum::<i32>());
}
