...
int main(void)
{
	...
	cin >> n >> m;
	rep(i, 0, 29) uf[i] = UnionFind(n);
	ll u, v, w;
	rep(i, 1, m){
		cin >> u >> v >> w;
		rep(j, 0, 29){
			if(w & (1<<j)) uf[j].unite(u, v);
		}
		if((w & (1<<0)) == 0){
			rep(j, 1, 29) ok[j][u] = ok[j][v] = true;
		}
	}
	rep(i, 1, n){
		rep(j, 0, 29){
			if(ok[j][i]) ok[j][uf[j].root(i)] = true;
		}
	}
	...
	rep(i, 1, Q){
		cin >> u >> v;
		ll ans = 2;
		rep(j, 1, 29) if(ok[j][uf[j].root(u)]) ans = 1;
		rep(j, 0, 29) if(uf[j].same(u, v)) ans = 0;
		cout << ans << "\n";
	}
