...
vector<int> solve(int x){
    ...
    vector<int> res;
    rep(n) st1.insert(a[i]),st2.insert(b[i]);
    rep1(_,n){
        int d;
        if(*st1.rbegin()>=*st2.rbegin()){
            d=*st1.rbegin();
            auto it=st2.find(abs(d-x));
            if(it==st2.end()) return {};
            st2.erase(it);
            st1.erase(st1.find(*st1.rbegin()));
        }
        else{
            d=x-*st2.rbegin();
            auto it=st1.find(abs(d));
...
void orzck(){
    ...
    if(sz(vec)){
        int minn=*min_element(all(vec));
        chkmin(minn,0ll);
        print("YES");
        for(auto j: vec) print0(j-minn);
        cout << "\n";
        print(-minn,x-minn);
    }
    else print("NO");
