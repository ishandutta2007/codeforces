...
//    DBN(mapa);
    for (int i = 0; i < m; ++i) {
        ...
        for (int val = 0; val < 2; ++val) {
            auto getVal = [&](int code, int bit) -> int {
                if (code == 0) {
                    return val;
                } else {
                    return bs[code][bit];
            ...
            auto doOp = [](int x, int y, int code) {
                if (code == 'X') {
                    return x ^ y;
                } else if (code == 'O') {
                    return x | y;
            ...
            for (int j = 0; j < cnt; ++j) {
                ...
                int bc = lines[j].op1;
                int cc = lines[j].op2;
                int code = lines[j].code;
                bs[ac][i] = doOp(getVal(bc, i), getVal(cc, i), code);
            }
            for (int j = 1; j < (int) mapa.size(); ++j) {
                vals[val] += bs[j][i];
            }
        }
        if (vals[1] > vals[0]) {
