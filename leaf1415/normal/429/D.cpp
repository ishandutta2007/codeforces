...
ll calc(ll l, ll r){
	...
	for(auto p : vec){
		ll x = p.se, y = p.fi;
		ll L = lower_bound(all(vec), P(y-D, -inf)) - vec.begin();
		ll R = upper_bound(all(vec), P(y+D, inf)) - vec.begin();
		rep(i, L, R-1){
			ll nx = vec[i].se, ny = vec[i].fi;
			if(P(x, y) == P(nx, ny)) continue;
			chmin(d, (x-nx)*(x-nx) + (y-ny)*(y-ny));
		}
	}
	return d;
}
