...
void solve() {
    ...
    vll l, r;
    rep(i, M) {
        while (!r.empty() && r.back() >= p[i].second) {
            l.pop_back();
            r.pop_back();
        }
    ...
    //cout << endl;
    vvll dp(N + 2, vll(2, INF));
    dp[0][0] = dp[0][1] = 0;

    ll lef = 0;
    ll ri = 0;
    repn(i, N + 1) {
        while (lef < M && l[lef] <= a[i-1])lef++;
        while (ri < M && r[ri] < a[i])ri++;
       //cout << lef << " " << ri << endl;
        for (ll j = lef; j <= ri; j++) {
            ll lku = 0;
            if (j > lef) { lku = l[j - 1] - a[i - 1]; }
            ll rku = 0;
            if (j < ri) { rku = a[i] - r[j]; }

            dp[i][0] = min(dp[i][0], dp[i - 1][0] + lku + rku * 2);
            dp[i][0] = min(dp[i][0], dp[i - 1][1] + lku * 2 + rku * 2);
            dp[i][1] = min(dp[i][1], dp[i - 1][0] + lku + rku);
            dp[i][1] = min(dp[i][1], dp[i - 1][1] + lku * 2 + rku);
