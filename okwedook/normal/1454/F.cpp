...
void solve() {
    ...
    for (int i = 0; i < n; ++i)
        pos[a[i]].pb(i);
    set<int> small, big;
    for (int i = 0; i < n; ++i) big.insert(i);
    small.insert(-1);
    small.insert(n);
    for (auto i : pos) {
        for (auto j : i.s)
        ...
        if (sz(i.s) >= 3) {
            for (auto j : i.s) {
                if (j == i.s[0]) continue;
                if (j == i.s.back()) continue;
                int l = *(--small.lower_bound(j)) + 1;
                int r = *(small.lower_bound(j)) - 1;
                chmax(l, i.s[0] + 1);
                chmin(r, i.s.back() - 1);
                if (l > r) continue;
                if (sz(big) && *big.begin() < l) continue;
                if (sz(big) && *big.rbegin() > r) continue;
                int x = l;
                int z = n - r - 1;
                int y = n - x - z;
                println("YES");
