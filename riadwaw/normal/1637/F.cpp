...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dfs = [&](auto& dfs, int v, int p) -> int64_t {
			mx[v] = 0;
			int64_t ans = 0;
			for (int to: g[v]) {
				if (to == p) {
		...
		int64_t ans = dfs(dfs, root, -1);
		sort(g[root].begin(), g[root].end(), [&](int q, int w) {
			return mx[q] > mx[w];
		});

		for (int i = 0; i < 2; ++i) {
			if (i < g[root].size()) {
				//cerr << "adding " << h[root] - mx[g[root][i]] << endl;
				ans += h[root] - mx[g[root][i]];
			} else {
				ans += h[root];
			}
		}
