...
#endif
int dp[2][1000005],a[1000005],b[1000005];
void print(int x)
{
...
#ifndef QWQ
#endif
	int n=read(),cd=read(),tmp=0;
	rg(n)b[i]=read();gr
	rg(n)a[i]=read();gr
	rg(n)tmp+=b[i],a[i]-=b[i],b[i]=0;gr
	std::priority_queue<int,std::vector<int>,std::greater<int> >q;
	int ans=0,sum=0;
	rg(n)q.push(a[i]+cd*(n-i+1));gr
	for(int k=1;k<=n;k++)
	{
		// print(q.top());puts("");
		sum+=q.top();q.pop();
		mne(ans,sum-cd*k*(k+1)/2);
	}
	// memset(dp[0],0x3f,sizeof(int)*(n+3));
	// dp[0][0]=0;
	// rg(n)
	// int c=i&1,d=c^1;
	// memset(dp[c],0x3f,sizeof(int)*(n+3));
	// for(int k=0;k<i;k++)
	// {
		// //
		// mne(dp[c][k],dp[d][k]+k*cd+a[i]);
		// mne(dp[c][k+1],dp[d][k]);
// 		
	// }
	// gr
	// int ans=inf;
	// for(int k=0;k<=n;k++)mne(ans,dp[n&1][k]);
	// oldl(ans+tmp);
	print(ans+tmp);
	return 0;
}