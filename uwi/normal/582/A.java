...
public class A2 {
	...
	void solve()
	{
		...
		int[] ft = new int[n*n+1];
		int p = 0;
		for(int i = 0;i < n*n;){
			int j = i;
			while(j < n*n && a[j] == a[i])j++;
			nums[p] = a[i];
			addFenwick(ft, p, j-i);
			p++;
			i = j;
		...
		for(int i = 0;i < n;i++){
			int rem = n*n-i*i;
			int last = findGFenwick(ft, rem-1);
			if(last < 0){
				last = -last-1;
			}else{
				last++;
			...
			rets[i] = nums[last];
			addFenwick(ft, last, -1);
			for(int j = 0;j < i;j++){
				addFenwick(ft, Arrays.binarySearch(nums, 0, p, gcd(rets[j], rets[i])), -2);
			}
		}
		for(int i = 0;i < n;i++){
			out.print(rets[i] + " ");
