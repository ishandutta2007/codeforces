...
int main() {
  int n = IN;
  auto a = IN.vll(n);
  stack<pair<ld, int>> deq;
  deq.emplace(1e8, n);
  repr(i, n) {
    if (deq.top().first <= (ld)a[i] + eps) {
      int at = i;
      ld val = a[i];
      do {
        ld avg = deq.top().first * (deq.top().second - at) + val * (at - i + 1);
        avg /= (deq.top().second - i + 1);
        at = deq.top().second;
        val = avg;
        deq.pop();
      } while (deq.top().first <= val + eps);
      deq.emplace(val, at);
    } else deq.emplace((ld)a[i], i);
  }
  vector<ld> res(n+1);
  int prev = -1;
  while (not deq.empty()) {
    auto [x, ix] = deq.top(); deq.pop();
    rep(i, prev+1, ix+1) res[i] = x;
    prev = ix;
  }
  rep(i, n) printf("%.9f\n", res[i]);
}