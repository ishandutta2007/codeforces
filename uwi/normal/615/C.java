...
public class C {
	...
	void solve()
	{
		...
		char[] z = sb.toString().toCharArray();
		int[] sa = sa(z);
		int[] isa = new int[z.length];
		for(int i = 0;i < z.length;i++)isa[sa[i]] = i;
		int[] lcp = buildLCP(z, sa);
		int[] dp = new int[m+1];
...
//		tr(z);
		...
		for(int i = 0;i < m;i++){
			int lc = 99999999;
			for(int j = isa[i]-1;j >= 0;j--){
				lc = Math.min(lc, lcp[j+1]);
				if(sa[j] > m && sa[j] != n+m+1){
					for(int k = 1;k <= lc && i+k <= m;k++){
						if(dp[i]+1 < dp[i+k]){
							dp[i+k] = dp[i] + 1;
							if(sa[j] > m+1+n){
								fs[i+k] = m+1+n+1+n-sa[j]-1;
								ts[i+k] = m+1+n+1+n-sa[j]-k;
							}else{
								fs[i+k] = sa[j]-m-1;
								ts[i+k] = sa[j]+k-1-m-1;
							}
			...
			lc = 99999999;
			for(int j = isa[i]+1;j < z.length;j++){
				lc = Math.min(lc, lcp[j]);
				if(sa[j] > m && sa[j] != n+m+1){
					for(int k = 1;k <= lc && i+k <= m;k++){
						if(dp[i]+1 < dp[i+k]){
							dp[i+k] = dp[i] + 1;
		...
		if(dp[m] > 5000){
			...
			int pos = m;
			while(pos > 0){
				hs[dp[pos]-1] = new int[]{fs[pos], ts[pos]};
				pos -= Math.abs(fs[pos]-ts[pos])+1;
			}
			for(int i = 0;i < dp[m];i++){
				out.println((hs[i][0]+1) + " " + (hs[i][1]+1));
			}
		}
	}
