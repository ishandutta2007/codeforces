...
int32_t main() {
    ...
    if(~f&&~s) {
        ...
        rep(i, N) if(i==f||i==s) {
            rep1(j, n) {
                if(a[j]==f) {
                    if(!ans[0]) ans[0]=j;
                    else if(!ans[2]) ans[2]=j;
                }
                if(a[j]==s) {
                    if(!ans[1]) ans[1]=j;
                    else if(!ans[3]) ans[3]=j;
                }
            }
        }
    ...
    vi poly(N); rep(i, N) poly[i]=!!occ[i];
    vi add=FFT::multiply<int>(poly, poly);
    rep(i, N) if(occ[i]==1) add[i<<1]--;
    int tg=0;
    rep(i, add.size()) if(add[i]>2) {tg=i; break;}
    if(!tg) puts("NO");
    else {
        int ans[4]={0};
        if(tg%2==0&&occ[tg/2]>1) {
            int k=tg/2;
            rep1(j, n) if(a[j]==k) {
                if(!ans[0]) ans[0]=j;
                else if(!ans[1]) ans[1]=j;
            }
        }
        rep(i, N) if(i!=tg-i&&tg-i<N&&occ[i]&&occ[tg-i]) {
            bool t=0;
            rep(j, 4) if(a[ans[j]]==i) t=1;
            if(t) continue;
            if(!ans[0]) ans[0]=idx[i], ans[1]=idx[tg-i];
            else if(!ans[2]) ans[2]=idx[i], ans[3]=idx[tg-i];
        }
        cout<<"YES\n"<<ans[0]<<' '<<ans[1]<<' '<<ans[2]<<' '<<ans[3]<<endl;
    }
}