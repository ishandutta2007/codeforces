...
vector<ll> G[100005];
mint dp[100005][2];
mint dp2[100005][2], dp3[100005][2][2];
	...
	rep(t, 0, 1){
		...
		rep(i, 0, d-1){
			...
			rep(j, 0, 1){
				dp2[i+1][j] += dp2[i][j];
				rep(l, 0, 1){
					dp2[i+1][j^l] += dp2[i][j] * dp[u][l];
				}
			}
		}
		rep(j, 0, 1) dp[v][j^1] += dp2[d][j];
		
		reverse(all(G[v]));
	}
	
	rep(t, 0, 1){
		rep(i, 0, d) rep(j, 0, 1) rep(k, 0, 1) dp3[i][j][k] = 0;
		dp3[0][0][0] = 1;
		...
		rep(i, 0, d-1){
			...
			rep(j, 0, 1){
				rep(k, 0, 1){
					dp3[i+1][j][k] += dp3[i][j][k];
					dp3[i+1][j^t][k^1] += dp3[i][j][k] * dp[u][t];
				}
			}
		}
		if(t == 0) rep(j, 0, 1) dp[v][j^1] -= dp3[d][j][0] + dp3[d][j][1];
		else rep(j, 0, 1) dp[v][j^1] -= dp3[d][j][1];
	}
}
