...
int main()
{
    ...
    while(!cards.empty())
    {
        ...
        cards.pop();
        int cur_team = pos % n;
        int cur_player = pos / n;
        int dt = max(1, c[cur_card] - a[cur_team][cur_player] - b[cur_team][cur_player ^ 1] - d[cur_team][cur_card]);
        if (dt <= time)
        {
            ans[cur_team].pb(s[cur_card]);
            time -= dt;
        }
        else
        {
            d[cur_team][cur_card] += time;
            cards.push(cur_card);
            time = 0;
        }
        if (time == 0)
