...
{
    int n,m; cin>>n>>m;
    vector<string> mp(n); cin>>mp;

    vector<vector<int>> fi(n+1,vector<int>(m+1));
    for(int i=0; i<n; ++i)
    {
        for(int j=0; j<m; ++j)
        {
    ...
    auto g=[&](int i, int j, int s)->bool
    {
        if(i+s>n || j+s>m) return false;
        int who=s*s;
        int sum=fi[i+s][j+s]+fi[i][j]-fi[i][j+s]-fi[i+s][j];
        return who==sum;
    };

    auto f=[&](const int T)->bool
    {
        const int s=T*2+1;
        // if(s>min(n,m)) return false;
    ...
    const int T=bin(f,0,(min(n,m)+1)/2);
