// https://codeforces.com/contest/1202/submission/114246035
// 
...
int main() {
    ...
    vector<vector<string>> lis(L);
    rep(i, N) {
        ll x = S[i].size();
        if (x < L) { lis[x].push_back(S[i]); }
    }
    ...
    rep(i, L) {
        ...
        vll v = r_hash(T, i, 20003);

        map<ll, ll> hashS;
        for (string U : lis[i]) {
            //cout << U << endl;
            vll t = r_hash(U, i, 20003);
            //cout << t[0] << endl;
            hashS[t[0]]++;
        }
