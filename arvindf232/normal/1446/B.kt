...
fun main(){
    solve.cases{
        ...
        //implitic i = 0
        for(p in 0 until n){
            val new = IntArray(m+1)
            for(last in 0 until m){
                if(A[p] == B[last]){
                    new[last+1] = maxOf(new[last+1],DP[last] + 2)
                }
            }
            for(last in 0 until m){
                new[last] = maxOf(DP[last] -1  , new[last])
            }
            for(last in 0 until m){
                new[last+1] = maxOf(new[last+1] , new[last] -1 )
                max = maxOf(max, new[last+1])
            }
            DP = new
        }
        put(max)
