...
#define int ll
vector<int> cnter;
pair<modint<MAX_MOD>,modint<MOD>> hoge[300000] = {};
pair<modint<MAX_MOD>,modint<MOD>> rev[300000] = {};
void solve() {
    int n;
    cin >> n;
    string t;
    ...
    pair<modint<MAX_MOD>, modint<MOD>> nexts;
    nexts.first = 1;
    nexts.second = 1;
    REP(i, cnter.size()) {
        hoge[i + 1] = hoge[i];
    ...
    REP(i, query) {
        ...
        cin >> a >> b >> c;
        a--; b--;
        pair<int, int> Left = mp(lower_bound(ALL(cnter), a * 2) - cnter.begin(), lower_bound(ALL(cnter), (a + c)*2) - cnter.begin());
        pair<int, int> Right = mp(lower_bound(ALL(cnter), b * 2) - cnter.begin(), lower_bound(ALL(cnter), (b + c) * 2) - cnter.begin());
        if (Left.second - Left.first != Right.second - Right.first) {
            cout << "No" << endl;
            continue;
        }
        int ok = 1;
        pair<modint<MAX_MOD>, modint<MOD>> aa,bb;
        aa = hoge[Left.second];
        aa.first -= hoge[Left.first].first;
        aa.second -= hoge[Left.first].second;
        if (a % 2 == b % 2) {
            bb = hoge[Right.second];
            bb.first -= hoge[Right.first].first;
            bb.second -= hoge[Right.first].second;
        }
        else {
            bb = rev[Right.second];
            bb.first -= rev[Right.first].first;
            bb.second -= rev[Right.first].second;
        }
        if (Left.first!= 0) {
            aa.first /= pows<modint<MAX_MOD>>(2, Left.first);
            aa.second /= pows<modint<MOD>>(2, Left.first);
        }
        if (Right.first!= 0) {
            bb.first /= pows<modint<MAX_MOD>>(2, Right.first);
            bb.second /= pows<modint<MOD>>(2, Right.first);
        }
