#line 1 "codeforces-workspace\\contest\\1444\\c\\c.cpp"
#include <bits/extc++.h>
...
#line 8 "codeforces-workspace\\contest\\1444\\c\\c.cpp"
...
#line 21 "codeforces-workspace\\contest\\1444\\c\\c.cpp"
...
void main() {
  ...
  union_find uf(n);
  map<pair<int, int>, vector<pair<int, int>>> outer;
  for (auto i = 0; i != m; ++i) {
    int a, b;
    cin >> a >> b;
    --a, --b;
    if (belong[a] == belong[b]) {
      ...
      outer[{belong[a], belong[b]}].emplace_back(a, b);
    }
  }
  vector<int> ok(k, true);
  int emp = 0;
  ...
  auto okc = count(begin(ok), end(ok), true);
  auto okp = i64(okc) * (okc - 1) / 2;
  for (auto &&[cp, cv] : outer) {
    auto [ca, cb] = cp;
    if (ok[ca] && ok[cb]) {
      bool fail = 0;
      map<int, vector<int>> tmp;
      for (auto &&[a, b] : cv) {
        auto ia = uf.find(a) * 2 + color[a];
        auto ib = uf.find(b) * 2 + color[b];
        tmp[ia].emplace_back(ib);
        tmp[ib].emplace_back(ia);
        tmp[ia].emplace_back(ia ^ 1);
        tmp[ia ^ 1].emplace_back(ia);
        tmp[ib].emplace_back(ib ^ 1);
        tmp[ib ^ 1].emplace_back(ib);
      }

      map<int, bool> col;
      for (auto &&[v, adj] : tmp) {
        if (col.count(v)) continue;
        queue<int> q;
        q.push(v);
        col[v] = 0;
        while (!q.empty()) {
