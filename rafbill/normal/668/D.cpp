...
int main(int, char**){
  ...
  int n; cin >> n;
  viii QS;
  unordered_map<int, set<int>> TS;
  FOR(i, n) {
    int a, t, x; cin >> a >> t >> x;
    QS.pb(mt(a, t, x));
    TS[x].insert(t);
  }
  unordered_map<int, unordered_map<int, int> > FROMT;
  unordered_map<int, BIT > bits;
  for(auto const& p : TS) {
    int x = p.first;
    set<int> const& ts = p.second;
    int i = 0;
  ...
  for(auto const& q : QS) {
    DESTRUCT3(q, a, t, x);
    if(a == 1) {
      bits[x].add(FROMT[x][t], 1);
    }else if(a == 2) {
      bits[x].add(FROMT[x][t], -1);
    }else{
      cout << bits[x].get(FROMT[x][t]) << "\n";
    }
  }
  cout << flush;
