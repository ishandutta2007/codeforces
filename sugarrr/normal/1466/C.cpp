...
bool anspro(){
    ...
    bool f1[n],f2[n];
    rep(i,0,n-1){
        f1[i]=false;f2[i]=false;
    }
    rep(i,1,n-1){
        if(s[i]==s[i-1])f1[i]=true;
    }
    rep(i,2,n-1){
        if(s[i]==s[i-2])f2[i]=true;
    }
    ll dp[n][4];rep(i,0,n-1)rep(j,0,3)dp[i][j]=inf;
    if(f1[1]==false)dp[1][0]=0;
    dp[1][1]=dp[1][2]=1;
    dp[1][3]=2;
    rep(i,2,n-1){
        if(!f1[i]&&!f2[i])chmin(dp[i][0],dp[i-1][0]);
        if(!f1[i])chmin(dp[i][0],dp[i-1][2]);
        chmin(dp[i][1],dp[i-1][0]+1);
        chmin(dp[i][1],dp[i-1][2]+1);
        
        if(!f2[i])chmin(dp[i][2],dp[i-1][1]);
