	...
	while(T--){
		...
		rep(i, 0, n){
			rep(j, 0, 80){
				rep(k, 0, a-1){
					rep(l, 0, b-1){
						dp[i][j][k][l] = Q(0, P(0, 0));
					}
				}
			}
		}
		dp[0][40][0][0] = Q(1, P(0, 0));
		...
		rep(i, 0, n-1){
			...
			rep(j, 0, 80){
				rep(k, 0, a-1){
					rep(l, 0, b-1){
						if(dp[i][j][k][l].first == 0) continue;
						//outl(i, j, k, l);
						dp[i+1][j+1][(k*10+c)%a][l] = Q(1, P(k, l));
						dp[i+1][j-1][k][(l*10+c)%b] = Q(-1, P(k, l));
					}
				}
			}
		}
		...
		string ans;
		per(i, n, 1){
			if(pre.first == 1) ans += "R";
			else ans += "B";
			J -= pre.first, K = pre.second.first, L = pre.second.second;
			pre = dp[i-1][J][K][L];
		}
		reverse(all(ans));
		outl(ans);
