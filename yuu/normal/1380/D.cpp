...
int64_t cost(int l, int r) {
    if (l > r) return 0;
    int keep_max = 0;
    if (l > 1) keep_max = max(keep_max, a[l - 1]);
    if (r < n) keep_max = max(keep_max, a[r + 1]);
    int inner_max = 0;
    for (int i = l; i <= r; i++) inner_max = max(inner_max, a[i]);
    if ((r - l + 1 < k) && (inner_max > keep_max)) return 1e17;
    if (k * y <= x) {  // better to delete element by element
        if (inner_max < keep_max) return y * (r - l + 1);
        else return y * (r - l + 1 - k) + x;
    } else {  // better to delete k at once
        return y * ((r - l + 1) % k) + x * ((r - l + 1) / k);
    }
}
