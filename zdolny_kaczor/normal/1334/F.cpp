...
sim> REV<c> reversed(c&x) {using value_type=typename c::value_type;return REV<c>{x};}sim> CREV<c> reversed(const c&x) {return CREV<c>{x};}
...
int main() {
	...
	n++;
	m++;
	REP(i, n) occs[a[i]].push_back(i);
	REP(i, m) in_b[b[i]] = true;
	REP(i, n) put(i, p[i]);
	int last = 0;
	for (int v = 1; v <= n; ++v) {
		if (in_b[v]) {
			debug("run", imie(v), imie(last));
			if (last == 0) {
				for (int x : occs[v]) {
					dp[x] = read(x);
					debug(range(a, 0, x + 1), range(p, 0, x + 1), arr(dp, x));
				}
			}
			else {
				ll best = llinf;
				...
				auto go_j = [&]() {
					...
					ll r = read(at);
					dp[at] = best + r;
					debug(range(a, 0, at + 1), range(p, 0, at + 1), arr(dp, at), imie(best) + imie(r));
				};
				while (pi < (int) occs[last].size() && pj < (int) occs[v].size())
					if (occs[last][pi] < occs[v][pj]) go_i();
					else go_j();
				while (pi < (int) occs[last].size()) go_i();
				while (pj < (int) occs[v].size()) go_j();
			}
			for (int w = last + 1; w <= v; ++w) for (int x : occs[w]) if (p[x] > 0) put(x, -p[x]);
			last = v;
		}
	}
	ll ans = llinf;
	REP(i, n) if (a[i] == b[m - 1]) mini(ans, dp[i]);
	if (ans < 1e16) printf("YES\n%lld\n", ans);
	else printf("NO\n");
	exit(0);
}
//#STAY AT HOME