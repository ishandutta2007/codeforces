...
inline Node merge(const Node& a, const Node& b){
    Node res;
    res.sum = a.sum + b.sum;
    res.minSuf = min(b.minSuf, a.minSuf + b.sum);
    res.maxPref = max(a.maxPref, a.sum + b.maxPref);
    res.maxPrefAns = max({a.maxPrefAns, a.maxFullAns + b.maxPref, -a.sum + b.maxPrefAns});
    res.maxSufAns = max({b.maxSufAns, b.maxFullAns - a.minSuf, b.sum + a.maxSufAns});
    res.maxFullAns = max({a.maxFullAns + b.sum, -a.sum + b.maxFullAns, -a.sum + b.sum});
    res.maxAns = max({a.maxAns, b.maxAns, a.maxSufAns + b.maxPref, -a.minSuf + b.maxPrefAns});
    return res;
}
Node tr[604040];
void build(int cur, int l, int r) {
    if (l == r) {
        if (s[l] == '(') {
            tr[cur] = {1, 0, 1, 1, 1, 1, 1};
        } else {
            tr[cur] = {-1, -1, 0, 1, 1, 1, 1};
        }
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
