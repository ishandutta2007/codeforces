...
inline void init() {
    rep(i, 1, n) f[0][i] = 1;
    rep(i, 1, n) rep(j, 1, n) f[i][j] = 1ll * (f[i - 1][j] + f[i][j - 1]) * inv2 % MOD;
}
...
int dep[MAXN], lca[MAXN][MAXN], dfn[MAXN], tot, id[MAXN];
void dfs(int x, int pre) {
    dep[x] = dep[pre] + 1; dfn[x] = ++ tot; id[tot] = x;
    for (int i = head[x];i;i = nxt[i]) {
        if(e[i] == pre) continue;
        dfs(e[i], x);
        rep(j, dfn[e[i]], tot)
        rep(k, dfn[x], dfn[e[i]] - 1)
        lca[id[j]][id[k]] = lca[id[k]][id[j]] = x;
    }
}

int calc(int x) { int ret = 0;
    rep(i, 1, n) rep(j, 1, i - 1) Add(ret, f[dep[i] - dep[lca[i][j]]][dep[j] - dep[lca[i][j]]]);
    return ret;
}

signed main(){ int ans = 0;
    read(n); rep(i, 2, n) link(read(), read());
    init(); rep(root, 1, n) {tot = 0; dfs(root, 0); Add(ans, calc(root));}
    print(1ll * ans * ksm(n, MOD - 2) % MOD);
}
/*
