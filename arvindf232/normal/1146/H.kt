...
class vector(val a:Int, val b:Int, val v:Point)
...
fun main(){
    solve.cases{
        ...
        repeat(n){
            all.add(Point(getint(), getint()))
        }
        Point.mode = 0
        all.sort()
        Point.mode = 2
        var V = mutableListOf<vector>()
        for(i in 0 until n){
            for(j in 0 until n){
                if(i == j) continue
                V.add(vector(i,j,all[j] - all[i]))
            }
        }
        V.sortBy { it.v }
        var total = 0L
