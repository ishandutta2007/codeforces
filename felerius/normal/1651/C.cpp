// Fault-tolerant Network (https://codeforces.com/contest/1651/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        auto find_best = [&](int val, vector<int>& x) {
            auto beg = begin(x) + 1, en = end(x) - 1;
            auto it = lower_bound(beg, en, val);
            if (it == en)
                return int(it - 1 - begin(x));
            if (it == beg)
                return int(it - begin(x));
            int above = *it, below = *prev(it);
            return (above - val < val - below ? int(it - begin(x)) : int(it - 1 - begin(x)));
        };

        array best_a0 = {0, n - 1, find_best(a[0], b)};
        array best_an = {0, n - 1, find_best(a[n - 1], b)};
        array best_b0 = {0, n - 1, find_best(b[0], a)};
        array best_bn = {0, n - 1, find_best(b[n - 1], a)};

        ll ans = INFL;
        trav(ma0, best_a0) trav(man, best_an) trav(mb0, best_b0) trav(mbn, best_bn) {
            set<pair<int, int>> edges = { pair(0, ma0), pair(n - 1, man), pair(mb0, 0), pair(mbn, n - 1) };
            ll cost = 0;
            for (auto [i, j] : edges)
                cost += abs(a[i] - b[j]);
            ans = min(ans, cost);
