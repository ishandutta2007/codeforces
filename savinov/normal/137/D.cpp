...
int main()
{
    ...
    for(int i = 0; i < n; ++i)
        for(int j = 1; j <= k; ++j)
            for(int l = 0; l <= i; ++l)
                if (dp[i + 1][j] > dp[l][j - 1] + ans[l][i])
                {
                    dp[i + 1][j] = dp[l][j - 1] + ans[l][i];
                    p[i + 1][j] = l;
                }
    int best = 1;
    for(int i = 1; i <= k; ++i)
    ...
    while(best)
    {
        t.pb(s.substr(p[cur][best], cur - p[cur][best]));
        cur = p[cur][best];
        --best;
    }
    reverse(all(t));
    for(int i = 0; i < t.size(); ++i)
    {
        ...
        for(int j = 0; j < t[i].length(); ++j)
            if (t[i][j] != t[i][t[i].length() - 1 - j])
                t[i][j] = t[i][t[i].length() - 1 - j];
        cout << t[i];
    }
    cout << endl;
}