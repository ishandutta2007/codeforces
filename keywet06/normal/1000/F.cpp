...
void add(int i, int l, int r, int ind, int val) {
    ...
    int mid = (l + r) / 2;
    if (ind < mid)
        add(2 * i + 1, l, mid, ind, val);
    else
        add(2 * i + 2, mid, r, ind, val);
...
pair<int, int> get(int i, int l, int r, int l1, int r1) {
    ...
    int mid = (l + r) / 2;
    return max(get(2 * i + 1, l, mid, l1, min(r1, mid)),
...
int main() {
    ...
    for (int i = 0; i < elem.size(); i++)
        if (where[i].size() > k) add(0, 0, n, where[i][k - 1], where[i][k] - 1);
    int q;
    cin >> q;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (where[who].size() > K + 1)
            add(0, 0, n, where[who][K], where[who][K + 1] - 1);
    }
    for (int i = 0; i < q; i++) {
        if (ans[i] == -1)
            cout << 0 << "\n";
