...
void main_() {
	...
	// 	if(min(i, j) == 0) {
	// 		int k = max(i, j);
	// 		return mint(3).pow(k + n * (n - k));
	// 	}
	// 	return 3 * mint(3).pow((n - i) * (n - j));
	// };
	// REP(i, n + 1) {
	// 	REP(j, n + 1) {
	// 		if(i + j == 0) continue;
	// 		res += f(i, j) * COM(n, i) * COM(n, j) * mint(-1).pow(i + j + 1);
	// 	}
	// }
	// print(res.val());
	...
	//part1
	REPS(i, n) {
		res += COM(n, i) * (i % 2 ? 1 : -1) * mint(3).pow((ll)n * (n - i) + i);
	}
	res *= 2;

	//part2
	REP(i, n) {
		mint v = mint(3).pow(i) * -1;
		mint tmp = (1 + v).pow(n);
		tmp -= v.pow(n);
		tmp *= COM(n, i);
		if(i % 2 == 0) tmp *= -1;
