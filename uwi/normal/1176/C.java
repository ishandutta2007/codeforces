...
public class C {
	...
	void solve()
	{
		int n = ni();
		int[] a = na(n);
		int[] q = {4,8,15,16,23,42};
		for(int i = 0;i < n;i++){
			a[i] = Arrays.binarySearch(q, a[i]);
		}
		int[][] g = makeBuckets(a, 6);
		LST[] ls = new LST[6];
		for(int i = 0;i < 6;i++){
			ls[i] = new LST(g[i].length+1);
			ls[i].setRange(g[i].length);
		}
		...
		for(int i = 0;i < n;i++){
			int pos = 0;
			for(int j = 0;j < 6;j++){
				int ind = lowerBound(g[j], pos);
				int nex = ls[j].next(ind);
				if(nex == -1)break outer;
				pos = g[j][nex];
				ls[j].unset(nex);
			}
