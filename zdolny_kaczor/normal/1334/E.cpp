...
#endif
//#STAY AT HOME
const int mod = 998244353, nax = 100;
ll fac[nax], fin[nax];
ll inv(ll b) {
	int e = mod - 2;
	ll r = 1;
...
int main() {
	...
	REP(_, q) {
		...
		vi pos, neg;
		for (ll p : primes) {
			int e1 = vp(p, u), e2 = vp(p, v);
			if (e1 > e2) pos.push_back(e1 - e2);
			else if (e1 < e2) neg.push_back(e2 - e1);
		}
		debug(imie(u), imie(v), imie(pos), imie(neg));
		printf("%lld\n", fix(run(pos) * run(neg), mod));
	}
	exit(0);
}
//#STAY AT HOME