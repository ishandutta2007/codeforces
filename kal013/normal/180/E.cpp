...
inline int cc(int l,int c){
	auto it = upper_bound(all(cnt[c]),l) - cnt[c].begin();
	--it;
	return it;
}

inline int cntc(int l,int r,int c){
	return cc(r, c) - cc(l - 1, c);
}


bool pos(int x,const vector<int>& V, int k){
	int n = V.size();
	for(int i = 0; i < n; ++i){
		int r = i + x - 1 ;
		if (r >= n){
