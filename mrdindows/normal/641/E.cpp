...
int sum(int *arr, int sz, int pos) {
    int res = 0;
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        int t = q[i].c;
        int id = lower_bound(all(v), t) - v.begin();
        if (v[id] != t) {
            id = -1;
        }
        q[i].c = id;
    ...
    for (int i = 0; i < n; ++i) {
        if (q[i].c != -1 && q[i].a != 3) {
            g[q[i].c].push_back(q[i].b);
        }
    }
    for (int i = 0; i < n; ++i) {
        if (g[i].empty()) continue;
        sort(all(g[i]));
        int sz = int(g[i].size() + 1);
        siz[i] = sz;
        ptr[i] = bp;
        bp += sz;
    }
    for (int i = 0; i < n; ++i) {
        ...
        int id = q[i].c;
        int time = q[i].b;
        time = int(upper_bound(all(g[id]), time) - g[id].begin());
        if (q[i].a == 1) {
            --time;
            add(ptr[id], siz[id], time, 1);
        } else if (q[i].a == 2) {
            --time;
            add(ptr[id], siz[id], time, -1);
        } else {
            --time;
            int res = sum(ptr[id], siz[id], time);
            printf("%d\n", res);
        }
    }
