...
bool eval(vector<int> A,vector<int> B,int x,int y) {
    map<int, int> AA, BB;
    REP(i, A.size()) {
        AA[A[i]]++;
        BB[B[i]]++;
    ...
    for (auto& x : AA) {
        y -= min(x.second, BB[x.first]);
    }
    if (x == 0 and y == 0) return 1;
    return 0;
}

vector<int> cnts[200000];
...
void solve() {
    ...
    REP(tea, 2) {
        ...
        REP(i, n + 3) {
            if (cnts[i].size() != 0) {
                nexts.push(mp(cnts[i].size(), i));
            }
        }
        if (tea == 1) {
            if (nexts.size() > 2) {
                pair<int, int> a = nexts.top();
                nexts.pop();
                pair<int, int> b = nexts.top();
                nexts.pop();
                pair<int, int> c = nexts.top();
                nexts.pop();
                a.first--;
                b.first--;
                c.first--;

                ans[cnts[a.second].back()] = b.second;
                y--;
                cnts[a.second].pop_back();

                ans[cnts[b.second].back()] = c.second;
                y--;
                cnts[b.second].pop_back();

                ans[cnts[c.second].back()] = a.second;
                y--;
                cnts[c.second].pop_back();
        ...
        while (nexts.size() > 0) {
            ...
            nexts.pop();
            if (x != 0) {
                ans[cnts[a.second].back()] = a.second;
                cnts[a.second].pop_back();
                a.first--;
                x--;
                y--;
            ...
            ans[cnts[b.second].back()] = a.second;
            y--;
            cnts[b.second].pop_back();
            if (a.first != 0) nexts.push(a);
            if (b.first != 0) nexts.push(b);
        }
        REP(i, n) {
            if (ans[i] == 0) ans[i] = safe;
        }
        if (eval(inputs,ans,memo_x,memo_y)) {
            cout << "YES" << endl;
            REP(i, ans.size()) {
                cout << ans[i] << " ";
            }
