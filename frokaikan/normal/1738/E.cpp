...
int main() {
	...
	rep(i, 0, maxn) ifac[i] = Mint{1} / fac[i];
	...
	int cas; cin >> cas; while (cas--) {
		...
		vector<ll> ps(n), ss(n);
		ps[0] = as[0];
		rep(i, 1, n - 1) ps[i] = ps[i - 1] + as[i];
		ss[n - 1] = as[n - 1];
		revrep(i, 0, n - 2) ss[i] = ss[i + 1] + as[i];
		reverse(all(ss));
		Mint ans = 1;
		int l2 = 0;
		for (int l = 0; l < n;) {
			int r = l;
			while (r < n && ps[r] == ps[l]) r++;
			r--;
			ll x = ps[l];
			while (l2 < n && ss[l2] < x) l2++;
			int r2 = l2;
			while (r2 < n && ss[r2] <= x) r2++;
			r2--;
			if (l2 <= r2) {
				int L = n - 1 - r2; 
				int R = n - 1 - l2;
				if (r < L) {
					int mn = min(r - l + 1, R - L + 1);
					Mint s = 0;
					rep(i, 1, mn) s += binom(r - l + 1, i) * binom(R - L + 1, i);
					ans *= s + 1;
				} else if (l < R) {
					int len = R - l + 1;
					Mint s = 0;
					rep(i, 1, len / 2) s += binom(len, i * 2);
					ans *= s + 1;
				} 
			}
			l = r + 1;
