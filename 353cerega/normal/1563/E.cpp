...
ll n;
vector<ll> a, b, was, used, val, prv;
vector<vector<ll>> g;
ll cur;
int V = -1;

int dfs(int u, int p, int F, ll W)
{
    was[u] = 1;
    val[u] = W;
    prv[u] = p;
    for (int v: g[u])
    {
        ...
        {
            if (used[v]==1 and W!=cur)
            {
                return u;
            }
            if (used[v]==1 and val[v]!=cur)
            {
                return v;
            }
            if (used[v]==0 and W>=a[v])
            {
                return v;
            }
        }
