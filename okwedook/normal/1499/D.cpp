...
vector<pii> dec(int x) {
    ...
    if (x > 1) st[x]++;
    return vector<pii>(all(st));
}

vector<int> divs;

void rec(const vector<pii> &d, int i, int curr) {
    if (i == sz(d)) {
        divs.pb(curr);
        return;
    }
...
vector<int> alldivs(const vector<pii> &d) {
    divs.clear();
    rec(d, 0, 1);
    return divs;
}
