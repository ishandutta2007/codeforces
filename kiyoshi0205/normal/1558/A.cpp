...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  c=(a+b)/2;
  d=(a+b+1)/2;
  int mi=min(max(a-c,0)+max(b-d,0),max(a-d,0)+max(b-c,0));
  int ma=max(min(a,c)+min(b,d),min(a,d)+min(b,c));
  vector<int> ans;
  if(c==d){
    //even
    for(int i=mi;i<=ma;i+=2)ans.emplace_back(i);
  }else{
    //odd
    for(int i=mi;i<=ma;i++)ans.emplace_back(i);
  }
  print(ans.size());
  output(ans);
}
