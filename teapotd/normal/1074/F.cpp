#define _USE_MATH_DEFINES/////////////////////////////////////////////////////
#include <bits/stdc++.h>//////////////////////////////////////////////////////
#ifdef LOC////////////////////////////////////////////////////////////////////
#include "debuglib.h"/////////////////////////////////////////////////////////
#else/////////////////////////////////////////////////////////////////////////
#define deb(...)//////////////////////////////////////////////////////////////
#define DBP(...)//////////////////////////////////////////////////////////////
#endif////////////////////////////////////////////////////////////////////////
#define x first///////////////////////////////////////////////////////////////
#define y second//////////////////////////////////////////////////////////////
#define mp make_pair//////////////////////////////////////////////////////////
#define pb push_back//////////////////////////////////////////////////////////
#define sz(x)int((x).size())//////////////////////////////////////////////////
#define each(a,x)for(auto&a:(x))///////////////// by teapotd /////////////////
#define all(x)(x).begin(),(x).end()///////////////////////////////////////////
#define rep(i,b,e)for(int i=(b);i<(e);i++)////////////////////////////////////
using namespace std;using namespace rel_ops;using ll=int64_t;using Pii=pair///
<int,int>;using ull=uint64_t;using Vi=vector<int>;void run();int main(){cin.//
sync_with_stdio(0);cin.tie(0);cout<<fixed<<setprecision(10);run();return 0;}//
//--------------------------------------------------------------------------//

int uplg(int n) { return 32-__builtin_clz(n); }
int uplg(ll n)  { return 64-__builtin_clzll(n); }
int cmp(double a, double b, double eps=1e-9) { return (a>b+eps) - (a+eps<b); }

struct SegmentTree {
    using T = int;
    static constexpr T ID = 0;

    struct Node {
        T extra{ID};
        T great{INT_MAX}, nGreat{0};

        void init(T x, int size) {
            great = x; nGreat = size;
        }

        void merge(const Node& R) {
            if (great > R.great)    nGreat =R.nGreat;
            else if(great==R.great) nGreat+=R.nGreat;
            great = min(great, R.great);
        }

        T apply(T x, int) {
            extra += x;
            great += x;
            return x;
        }
    };

    vector<Node> V;
    int len;

    SegmentTree(int n=0, T def=ID) {init(n,def);}

    void init(int n, T def) {
        for (len = 1; len < n; len *= 2);
        V.assign(len*2, {});
        rep(i, len, len+n) V[i].init(def, 1);
        for (int i = len-1; i > 0; i--) update(i);
    }

    int getChild(int i, int j) { return i*2+j; }
    int L(int i) { return getChild(i, 0); }
    int R(int i) { return getChild(i, 1); }

    void update(int i) {
        int a = L(i), b = R(i);
        V[i] = {};
        V[i].merge(V[a]);
        V[i].merge(V[b]);
    }

    void push(int i, int size) {
        int e = V[i].extra;
        if (e != ID) {
            e = V[L(i)].apply(e, size/2);
            V[R(i)].apply(e, size/2);
            V[i].extra = ID;
        }
    }

    T modify(int vBegin, int vEnd, T x,
             int i = 1,
             int begin = 0, int end = -1) {
        if (end < 0) end = len;
        if (vEnd <= begin || end <= vBegin)
            return x;

        if (vBegin <= begin && end <= vEnd) {
            return V[i].apply(x, end-begin);
        }

        int mid = (begin + end) / 2;
        push(i, end-begin);
        x = modify(vBegin,vEnd,x,L(i),begin,mid);
        x = modify(vBegin,vEnd,x,R(i),mid,end);
        update(i);
        return x;
    }

    Node query(int vBegin, int vEnd, int i = 1,
               int begin = 0, int end = -1) {
        if (end < 0) end = len;
        if (vEnd <= begin || end <= vBegin)
            return {};
        if (vBegin <= begin && end <= vEnd)
            return V[i];

        int mid = (begin + end) / 2;
        push(i, end-begin);
        Node x = query(vBegin,vEnd,L(i),begin,mid);
        x.merge(query(vBegin,vEnd,R(i),mid,end));
        return x;
    }
};

struct Vert {
    Vi E, jumps;
    int pre, post;
};

vector<Vert> G;
SegmentTree tree;
set<Pii> special;
int cnt;

void dfs(int i, int p) {
    G[i].jumps.pb(p != -1 ? p : i);
    G[i].pre = cnt++;
    each(e, G[i].E) if (e != p) {
        dfs(e, i);
    }
    G[i].post = cnt;
}

int before(int a, int b) {
    for (int i = sz(G[b].jumps)-1; i >= 0; i--) {
        int k = G[b].jumps[i];
        if (G[k].pre > G[a].pre) b = k;
    }
    return b;
}

void handle() {
    int a, b, sign;
    cin >> a >> b;
    a--; b--;
    Pii edge = { min(a,b), max(a,b) };

    if (special.count(edge)) {
        special.erase(edge);
        sign = -1;
    } else {
        special.insert(edge);
        sign = 1;
    }

    if (G[a].pre > G[b].pre) swap(a, b);

    if (G[a].pre <= G[b].pre && G[b].post <= G[a].post) {
        int c = before(a, b);
        tree.modify(G[c].pre, G[c].post, sign);
        tree.modify(G[b].pre, G[b].post, -sign);
    } else {
        tree.modify(0, sz(G), sign);
        tree.modify(G[a].pre, G[a].post, -sign);
        tree.modify(G[b].pre, G[b].post, -sign);
    }

    auto q = tree.query(0, sz(G));
    assert(q.great >= 0);
    cout << (q.great == 0 ? q.nGreat : 0) << '\n';
}

void run() {
    int n, q; cin >> n >> q;
    G.resize(n);

    rep(i, 1, n) {
        int a, b; cin >> a >> b;
        G[a-1].E.pb(b-1);
        G[b-1].E.pb(a-1);
    }

    dfs(0, -1);
    tree.init(n, 0);

    rep(j, 0, uplg(n)) {
        each(v, G) v.jumps.pb(G[v.jumps[j]].jumps[j]);
    }

    while (q--) handle();
}