...
void solve() {
	ll n; cin>>n;
	ll ans = 0;
	for (int i = 0; i < lehmer.np && (ll) lehmer.pr[i] * lehmer.pr[i] < n; i++) {
		ans += lehmer.calc(n / lehmer.pr[i]) - i - 1;
	}
	for (int i = 0; i < lehmer.np && (ll) lehmer.pr[i] * lehmer.pr[i] * lehmer.pr[i] <= n; i++) {
		ans++;
	}
	cout<<ans<<"\n";
}
