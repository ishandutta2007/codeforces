...
int main() {
    ...
    vector<pt> cr(4);
    F0R (i, 4) cr[i] = {(i == 1 || i == 2) ? -W/2. : W/2., i&2 ? -H/2. : H/2.};

    vector<pt> poly;
    F0R (i, 4) F0R (j, 4) {
        auto isect = seg_intersect(cr[i], cr[(i+1)&3], cr[j].rotate(A/180. * PI), cr[(j+1)&3].rotate(A/180. * PI));
        poly.insert(poly.end(), all(isect));
    }
    ps(abs(area(convex_hull(poly)).d) / 2.);
