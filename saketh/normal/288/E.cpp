...
int main() {
    ...
    vmn four(MAXL), seven(MAXL), p2(MAXL), p10(MAXL);

    p2[0] = p10[0] = 1;
    FOR (i, 1, MAXL) p2[i] = 2 * p2[i - 1];
    FOR (i, 1, MAXL) p10[i] = 10 * p10[i - 1];
    FOR (i, 1, MAXL) four[i] = 10 * four[i - 1] + 4;
    FOR (i, 1, MAXL) seven[i] = 10 * seven[i - 1] + 7;

    vmn sum(MAXL), adj(MAXL);
    FOR (i, 1, MAXL) {
        adj[i] =
            p10[i-1] * p10[i-1] * (4 * 4 + 7 * 7) * (p2[i-1] - 1)
          + p10[i-1] * (4 + 7) * (2 * sum[i-1] - four[i-1] - seven[i-1])
          + 1 * (1 + 1) * (adj[i-1])
          + (4 * p10[i-1] + seven[i-1]) * (7 * p10[i-1] + four[i-1]);
        sum[i] = 2 * sum[i - 1] + (4 + 7) * p10[i-1] * p2[i-1];
    }

    auto get = [&](string X) {
        mn last = 0, pref = 0, ans = 0;
        F0R (i, sz(X)) {
            int t = sz(X) - i;
            if (X[i] == '7') {
                mn front = pref * p10[t] + 4 * p10[t-1];
                ans += last * (front + four[t-1]);
                ans += front * front * (p2[t-1] - 1)
                     + front * (2 * sum[t-1] - four[t-1] - seven[t-1])
                     + adj[t-1];
                last = front + seven[t-1];
            }
            pref = 10 * pref + (X[i] - '0');
        }
        ans += last * pref;
