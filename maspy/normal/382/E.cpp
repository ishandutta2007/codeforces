...
void solve() {
  ...
  DP0[1][0] = 1;
  FOR3(n, 2, N + 1) {
    FOR(a, n) FOR(b, n) if (a + b + 1 == n) {
      mint coef = MM.fact(n) * MM.fact_inv(a) * MM.fact_inv(b) / 2;
      FOR(x, n + 1) FOR(y, n + 1) {
        if (x + y > N) break;
        DP0[n][x + y] += DP1[a][x] * DP1[b][y] * coef;
        if (x + y + 1 > N) break;
        DP1[n][x + y + 1] += DP0[a][x] * DP1[b][y] * coef;
        DP1[n][x + y + 1] += DP1[a][x] * DP0[b][y] * coef;
        DP1[n][x + y + 1] += DP0[a][x] * DP0[b][y] * coef;
      }
    }
  }
