...
int main() {
  ...
  for(int i = 0; i < N; i++) {
    ...
    for(int j = 0; j < N; j++) {
      if(i != j) {
        chmin(min_d, abs(X[i] - X[j]) + abs(Y[i] - Y[j]));
      }
    }
    vector< int > prod{i};
    for(int j = 0; j < N; j++) {
      if(i != j) {
        if(min_d == abs(X[i] - X[j]) + abs(Y[i] - Y[j])) {
          prod.emplace_back(j);
        }
      }
    }
  ...
  for(auto &g: group) {
    if(g.second == g.first.size()) {
      for(auto &p: g.first) ok[p] = 1;
      vector< modint > dp2(N + 1);
      for(int j = 1; j <= N; j++) {
        dp2[j - 1] = dp1[j] * j;
      }
      vector< modint > dp3(N + 1);
      for(int k = 0; k < g.second; k++) {
        for(int l = 1; l <= N; l++) {
          dp3[l - 1] = dp1[l] * l;
        }
        dp1 = dp3;
  ...
  cout << accumulate(begin(dp1), end(dp1), modint(0)) << "\n";
}