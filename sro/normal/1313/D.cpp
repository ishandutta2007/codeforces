...
vector<int> lss;
vector<int> fs[200005],nxt[200005];
int dp[256],tmp[256];
...
int main(){
	...
	lss.pub(m+1);
	sort(all(lss));
	lss.resize(unique(all(lss))-lss.begin());
	cont(i,n){
		l[i]=lower_bound(all(lss),l[i])-lss.begin();
		r[i]=lower_bound(all(lss),r[i])-lss.begin();
		circ(j,l[i],r[i]-1){
			fs[j].pub(i);
			nxt[j].pub(SZ(fs[j+1]));
		}
	...
	loop(i,SZ(lss)-1){
		...
		loop(j,Mask(SZ(fs[i]))){
			...
			dp[j]=$*(lss[i+1]-lss[i])+tmp[$$];
		}
	}
	int $=0;
	loop(i,Mask(SZ(fs[SZ(lss)-2]))){
		$=max($,dp[i]);
	}
	fio<<$<<ln;
	return 0;
