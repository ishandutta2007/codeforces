...
void solve() {
  ...
  // 
  vc<T> dp0(K + 10);
  fill(all(dp0[1]), mint(1));
  FOR(k, 1, K - 1) dp0[k + 1] = mul(dp0[k], dp0[k]);

  LL(N);
  vc<string> color = {"white", "green", "red", "yellow", "blue", "orange"};
  map<ll, ll> dat;
  FOR(N) {
    LL(x);
    STR(S);
    FOR(i, 6) if (color[i] == S) dat[x] = i;
  }
  vi X;
  for (auto&& [x, v]: dat) {
    ll a = x;
  ...
  for (auto&& x: X) {
    ...
    if (ht == 1) {
      ...
      T left = (dp.count(2 * x + 0) ? dp[2 * x + 0] : dp0[ht - 1]);
      T right = (dp.count(2 * x + 1) ? dp[2 * x + 1] : dp0[ht - 1]);
      dp[x] = mul(left, right);
    }
    if (dat.count(x)) {
      ll c = dat[x];
      FOR(i, 6) if (i != c) dp[x][i] = mint(0);
    }
  }
  mint ANS = 0;
  for (auto&& x: dp[1]) ANS += x;
  print(ANS);
}
