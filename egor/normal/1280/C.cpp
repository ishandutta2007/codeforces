...
public:
    void solve(istream &inp, ostream &outp) {
        ...
        ll bad = 0;
        ll good = 0;
        using edge = IntBiWeightedEdge;
        Graph<edge> graph(n);
        for (int i : Range(2 * k - 1)) {
            graph.addEdge(new edge(a[i], b[i], t[i]));
        }
        function<void(int, int)> dfs = [&](int vert, int last) {
            ...
            for (edge *e : graph[vert]) {
                ...
                bad += ll(e->weight) * min(qq, n - qq);
                q[vert] += qq;
            }
        };
        dfs(0, -1);

        out.printLine(good, bad);
    }
};
