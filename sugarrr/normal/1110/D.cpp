...
int main(){fastio
    ...
    rep(i,0,N-1)rep(j,0,4)rep(k,0,2)dp[i][j][k]=-inf;
    dp[0][0][0]=0;
    rep(i,0,N-1){
        rep(j,0,4){
            rep(k,0,2){
                ...
                rep(ad,0,2){
                    if(j+ad>cnt[i+1] || k+ad>cnt[i+2] ||ad>cnt[i+3])break;
                    chmax(dp[i+1][k+ad][ad],dp[i][j][k]+ad+(cnt[i+1]-(j+ad))/3);
                }
            }
        }
    }
    ...
    ll ans=0;
    rep(i,0,N-1)rep(j,0,4)rep(k,0,2)chmax(ans,dp[i][j][k]);
    cout<<ans<<endl;
