...
template<class num_t> struct SegmentTree {
    ...
    void build(int p, int L, int R, vi& dc) {
        if (L == R) {
            st[p] = dc[L];
            return;
        }
        build(p << 1, L, L + R >> 1, dc);
        build(p << 1 | 1, (L + R >> 1) + 1, R, dc);
...
void chemthan() {
    ...
    int d = sz(dc);
    seg.init(d);
    seg.build(1, 0, d - 1, dc);
    map<pi, int> hs;
    int sum = 0;
    multiset<int> st;
    st.insert(lower_bound(all(dc), n - 1) - dc.begin());
    FOR(i, 0, m) {
        int x, y; tie(x, y) = vals[i];
        int t = lower_bound(all(dc), y + abs(x - k)) - dc.begin();
        if (!hs[mp(x, y)]) {
            seg.upd(t + 1, d - 1, -1);
            sum++;
            st.insert(t);
        ...
        hs[mp(x, y)] ^= 1;
        int z = *st.rbegin();
        cout << max(0, sum + seg.query(0, z) - n) << "\n";
    }
}
