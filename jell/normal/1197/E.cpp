...
void updat(int x, pair<i64,mint> y) {
    if(mins[x].fir>y.fir) {
        mins[x]=y;
    } else if(mins[x].fir==y.fir) {
        mins[x].sec+=y.sec;
    }
}
...
void solve() {
    ...
    rep(i,n) {
        ...
        updat(out,x);
        if(mxin[i]<out) {
            if(chmin(tmp,x.fir+vals[out])) {
                ans=x.sec;
            } else if(tmp==x.fir+vals[out]) {
                ans+=x.sec;
            }
        }
    }
