...
Node merge(Node a, const Node& b) {
    ...
    res.cnt = a.cnt + b.cnt;
    for (int i = 0; i < n; ++i) {
        res.front[i] = a.front[i];
    }
    for (int i = 0; i < n; ++i) {
        res.back[i] = b.back[i];
    }
    for (int i = 0; i < n; ++i) {
        if (a.back[i].first == b.front[i].first && a.back[i].second != b.front[i].second) {
            pii p = {a.back[i].second, b.front[i].second};
...
void build(int cur, int l, int r) {
    if (l == r) {
        for (int i = 0; i < n; ++i) {
            if (i && a[i][l] == a[i - 1][l]) {
                tr[cur].front[i] = tr[cur].back[i] = tr[cur].back[i - 1];
            } else {
                tr[cur].front[i] = tr[cur].back[i] = {a[i][l], l * 16 + i};
                tr[cur].cnt++;
            }
        }
    } else {
