...
void dfs(int x, int y) {
    ...
    for (int q = 0; q < 4; ++q) {
        int next_x = x + xe[q];
        int next_y = y + ye[q];
        if (next_x <= inputs.size() and next_y <= inputs[0].size() and next_x >= 1 and next_y >= 1) {
            if (inputs[next_x - 1][next_y - 1] == '*' or visited[next_x-1][next_y-1] == 1) {
                continue;
            }
        oks:;
            cout << s[q] << endl;
            ...
            cin >> a >> b;
            if (a == x and b == y) {
                swap(s[q], s[(q / 2 * 2) + ((q % 2 + 1) % 2)]);
                goto oks;
            }
            dfs(a, b);
            cout << s[(q / 2 * 2) + ((q%2 + 1) % 2)] << endl;
            cin >> a >> b;
        }
    }
}
