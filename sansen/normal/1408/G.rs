...
fn run() {
    ...
    let mut poly = vec![vec![M::one()]; n];
    for &(a, b) in e.iter().skip(1) {
        if let Some((x, y)) = dsu.unite(a, b) {
            edge[x] += edge[y];
            poly[x] = mul(&poly[x], &poly[y]);
            poly[x].push(M::zero());
            poly[y] = vec![];
        }
        let r = dsu.root(a);
        ...
        let s = dsu.size(r);
        if edge[r] == s * (s - 1) / 2 {
            *poly[r].last_mut().unwrap() = M::one();
        }
    }
    let ans = poly[dsu.root(0)].clone();
    let mut s = String::new();
