...
main() {
    ...
    rep(i, n) {
        int p = s[i] - '0';
        rep(j, 3) rep(k, 2) chmax(dp[i + 1][j][k], dp[i][j][k]);
        for(int j : {0, 2}) chmax(dp[i + 1][j][p ^ 1], dp[i][j][p] + 1);
        chmax(dp[i + 1][1][p], dp[i][1][p ^ 1] + 1);
        chmax(dp[i + 1][1][p], max(dp[i][0][p], dp[i][0][p ^ 1] + 1));
        chmax(dp[i + 1][2][p ^ 1], max(dp[i][1][p ^ 1], dp[i][1][p] + 1));
    }
    int ma = 0;
    rep(j, 3) rep(k, 2) chmax(ma, dp[n][j][k]);
    cout << ma << endl;
}