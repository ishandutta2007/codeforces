...
void solve()
{
    ...
    int ans = 0;
    int a = m; int b = n-m;
    if (b==0) {cout<<mul(a, k)<<endl; return;}
    if (a==0) {cout<<0<<endl; return;}

    for (int i = 1; i<=a; i++)
    {
        int ways = C(a+b-i-1, a-i);
        ways = mul(ways, invdegs[a+b-i]);
        ans = add(ans, mul(ways, i));
    }
...
int main()
{
    ...
    invdegs[0] = 1;
    for (int i = 1; i<LIM; i++) invdegs[i] = mul(invdegs[i-1], inv2);
