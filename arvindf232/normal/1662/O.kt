...
fun main(){
    solve.cases{
        val n = getint
        val walls = mutableSetOf<Pair<Int,Int>>() // r, theta
        val circu = mutableSetOf<Pair<Int,Int>>() //r, theta
        repeat(n){
            val type = getstr
            if(type == "C"){
                val r = getint
                ...
                val angle = getint
                while(r1 != r2){
                    walls.add(Pair(r1,angle))
                    r1 ++
                }
            }
        }
        val dsu = DisjointUnionSets(21 * 360)
        fun code(r:Int, angle:Int):Int {
            return r * 360 + angle
        }
        for(r in 0 until 20){
            for(angle in 0 until 360){
                if(Pair(r+1,angle) !in circu){
                    dsu.union(code(r,angle), code(r+1,angle))
                }
        ...
        for(r in 0..20){
            for(angle in 0 until 360){
                val nexta = angle.next(360)
                if(Pair(r,nexta) !in walls){
                    dsu.union(code(r,angle), code(r,nexta))
                }
            }
        }
        val ans = dsu.find(code(0,0)) == dsu.find(code(20,0))
        if(ans){
            put("YES")
        }else{
            put("NO")
