...
signed main(){
    ...
    int n=in(),k=in();
    ll res = 0;
    vector<ll> dp(k,-1e15); // amod k
    dp[0]=0;
    rep(i,n){
        ll a=in(),b=in();
        int ret = (a+b)%k;
        vector<ll> nxt(k,-LONG_LONG_MAX);
        int aa = min<int>(a+1,k);
        rep(j,k){
            if(dp[j]<0)continue;
            rep(ii,aa){
                int jj = (ret + k - ii); if(jj>=k)jj-=k;
                if(jj>b)continue;
                int pls=0;
                int A = j;
                int B = (res + k - A) ; if(B>=k)B-=k;
                A+=ii,B+=jj;
                if(A>=k)pls++,A-=k;
                if(B>=k)pls++,B-=k;
                chmax(nxt[A],pls + (a+b-ii-jj)/k + dp[j]);
            }
        }
        dp = nxt;
        res = (res + ret);
