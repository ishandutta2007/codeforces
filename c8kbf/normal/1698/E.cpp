...
_ n, m, pa[maxn], pb[maxn], a[maxn], b[maxn], p[maxn];
bitset<maxn> v;
...
//    rd(b, n);
    rd(pb, n);
    for(_ i = 1; i <= n; ++i) if(~pb[i]) b[pb[i]] = i;
    reverse(a+1, a+1+n);
    reverse(b+1, b+1+n);
    for(_ i = 1; i <= n; ++i) {
        p[a[i]] = min(n, i+m);
    ...
    for(_ i = 1; i <= n; ++i) if(!v.test(i)) g.push_back(p[i]);
    _ s = 1;
    sort(g.begin(), g.end());
    _ p = 0, c = 0;
    for(_ i : g) {
        for(; p+1 <= i; ++p) if(!~b[p+1]) ++c;
        if(c <= 0) {
            writeln(0);
            return;
        }
