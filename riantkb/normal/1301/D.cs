...
class Solver {
    ...
    public void solve() {
        ...
        var mv = new List<pair<int, char>>();
        for (int i = 0; i < w - 1; i++)
        {
            mv.Add(new pair<int, char>(h - 1, 'D'));
            mv.Add(new pair<int, char>(h - 1, 'U'));
            mv.Add(new pair<int, char>(1, 'R'));
        }
        {
            for (int i = 0; i < h - 1; i++)
            {
                mv.Add(new pair<int, char>(1, 'D'));
                mv.Add(new pair<int, char>(w - 1, 'L'));
                mv.Add(new pair<int, char>(w - 1, 'R'));
            }
            mv.Add(new pair<int, char>(h - 1, 'U'));
            mv.Add(new pair<int, char>(w - 1, 'L'));
        }
        var ans = new List<pair<int, char>>();
        foreach (var item in mv)
        {
            if (k == 0) break;
            int c = Math.Min(k, item.v1);
            if (c > 0)
                ans.Add(new pair<int, char>(c, item.v2));
            k -= c;
        }
        if (k > 0) {
            Prt("NO");
