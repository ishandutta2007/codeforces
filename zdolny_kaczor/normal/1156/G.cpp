...
struct how_to_get {
	...
	how_to_get(ull _l, char _o, ull _r) : input(false), which(), left(_l), right(_r), op(_o) {}
	how_to_get() {assert(false);}
};
#ifdef LOCAL
ostream & operator<<(ostream &o, how_to_get x) {
	if (x.input) ros << "[input(" << x.which << ")]";
	else ros << "[" << x.left << x.op << x.right << "]";
}
#endif
...
int main() {
	...
	REP(_, n) {
		...
		if (op != -1) {
			string assigned(line, line + equal), left(line + equal + 1, line + op), right(line + op + 1, line + l);
