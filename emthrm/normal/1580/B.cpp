...
int main() {
  ...
  ModInt::set_mod(p);
  ModInt::fact(n);
  auto c = pascal<ModInt>(n);
  vector dp(m + 1, vector(n + 1, vector(k + 1, ModInt(0))));
  vector is_visited(m + 1, vector(n + 1, vector(k + 1, 0)));
  auto f = [&](auto &&f, int m, int n, int k) -> ModInt {
    ModInt &d = dp[m][n][k];
    if (is_visited[m][n][k]) return d;
    is_visited[m][n][k] = true;
    if (m > n) return d = (k == 0 ? ModInt::fact(n) : 0);
    if (m == 1) return d = (k == 1 ? ModInt::fact(n) : 0);
    if (k > n) return d = 0;
    if (k > 0) {
      REP(p, n) {
        ModInt pat = 0;
        pat += f(f, m - 1, p, 0) * f(f, m - 1, n - 1 - p, k);
        const int r = min(p + 1, k);
        FOR(i, max(k - (n - 1 - p), 1), r) {
          pat += f(f, m - 1, p, i) * f(f, m - 1, n - 1 - p, k - i);
        }
        pat += f(f, m - 1, p, k) * f(f, m - 1, n - 1 - p, 0);
        d += pat * c[n - 1][p];
      }
    } else {
      REP(p, n) d += f(f, m - 1, p, 0) * f(f, m - 1, n - 1 - p, 0) * c[n - 1][p];
    }
    return d;
  };
  cout << f(f, m, n, k) << '\n';
