...
int main() {
    TEST {
        ...
        rep(i, n) mp[a[i]]++;
        vpi v;
        fore(x, y, mp) v.eb(x, y);
        vvc<int> ma(n + 1);
        fore(x, y, v) ma[y].eb(x);
        fore(e, ma) {
            SORT(e);
            REV(e);
        }
        auto f = [](int x, int y) {
            if(x > y) swap(x, y);
            return 1LL * ((ll)x << 30) + y;
        };
        set<ll> s;
        vvc<pii> c(n + 1);
        rep(i, 1, n + 1) { rep(j, si(ma[i])) c[j].eb(ma[i][j], i); }
        while(m--) {
            INT(x, y);
            s.emplace(f(x, y));
        }
        ...
        vi id;
        rep(i, 1, n + 1) if(si(ma[i])) id.eb(i);

        fore(x, y, v) {
            fore(i, id) {
                fore(e, ma[i]) {
                    if(s.count(f(x, e))) continue;
                    if(x == e) continue;
                    chmax(res, 1LL * (x + e) * (y + i));
                    break;
                }
            }
        }
