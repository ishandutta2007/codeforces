...
int64_t get(int target) {
    if (target < 0) return 1e18;
    int high = lower_bound(h + 1, h + n + 1, target + 1) - h;
    int low = upper_bound(h + 1, h + n + 1, target - 1) - h - 1;
    assert((high == n + 1) || (h[high] > target));
    assert((low == 0) || (h[low] < target));
    int64_t a_low = static_cast<int64_t>(low) * target - s[low];
    int64_t r_high = (s[n] - s[high - 1]) - static_cast<int64_t>(n - high + 1) * target;
    return min(a_low, r_high) * m + max(0LL, a_low - r_high) * a + max(0LL, r_high - a_low) * r;
}
void solve() {
    cin >> n >> a >> r >> m;
    m = min(m, a + r);
    h | ARRAY | 1 | n | ARRAY_INPUT | SORT;
    for (int i = 1; i <= n; i++) s[i] = s[i - 1] + h[i];
    int64_t ans = 1e18;
    for (int i = 1; i <= n; i++) ans = min(ans, get(h[i]));
    ans = min(ans, get(s[n] / n));
    ans = min(ans, get(s[n] / n - 1));
    ans = min(ans, get(s[n] / n + 1));
    cout << ans << '\n';
}
