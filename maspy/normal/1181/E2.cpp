...
#line 1 "/home/maspy/compro/library/alg/monoid_min_idx.hpp"
...
#line 3 "/home/maspy/compro/library/alg/lazy_minidx_add.hpp"
...
void solve() {
  ...
  auto dfs = [&](auto& dfs, vc<T> dat) -> bool {
    const int N = len(dat);
    if (len(dat) <= 1) return 1;
    pair<int, int> xrange, yrange;
    {
      // 
      vc<int> X, Y;
      for (auto&& [a, b, c, d]: dat) X.eb(a), X.eb(c), Y.eb(b), Y.eb(d);
      UNIQUE(X), UNIQUE(Y);
      for (auto&& [a, b, c, d]: dat) {
        a = LB(X, a), b = LB(Y, b), c = LB(X, c), d = LB(Y, d);
      }
      xrange = {0, len(X) - 1}, yrange = {0, len(Y) - 1};
    }
    //  -> index
    vvc<int> XI(xrange.se + 1), YI(yrange.se + 1);
    FOR(i, N) {
      auto [a, b, c, d] = dat[i];
      XI[a].eb(i), YI[b].eb(i);
    }
    ...
    using Mono = Lazy::X_structure;
    auto make_raw = [&](int n) -> vc<P> {
      vc<P> a(n);
      FOR(i, n) a[i] = {0, i};
      return a;
    };
    vc<bool> done(N);
    auto seg_raw_X = make_raw(xrange.se);
    auto seg_raw_Y = make_raw(yrange.se);
    LazySegTree<Lazy> seg_X(seg_raw_X), seg_Y(seg_raw_Y);
    auto add = [&](int i) -> void {
      auto [a, b, c, d] = dat[i];
      seg_X.apply(a + 1, c, 1);
      seg_Y.apply(b + 1, d, 1);
    ...
    auto cut_x = [&]() -> bool {
      auto [v, x] = seg_X.prod(xrange.fi + 1, xrange.se);
      if (v != 0) return 0;
      int mid = (xrange.fi + xrange.se) / 2;
      if (x <= mid) {
        while (xrange.fi < x) {
          for (auto&& i: XI[xrange.fi]) { rm(i); }
          xrange.fi++;
        }
      } else {
        while (xrange.se > x) {
          xrange.se--;
          for (auto&& i: XI[xrange.se]) { rm(i); }
        }
      }
      dfs(dfs, tmp);
      tmp.clear();
    ...
    auto cut_y = [&]() -> bool {
      auto [v, y] = seg_Y.prod(yrange.fi + 1, yrange.se);
      if (v != 0) return 0;
      int mid = (yrange.fi + yrange.se) / 2;
      if (y <= mid) {
        while (yrange.fi < y) {
          for (auto&& i: YI[yrange.fi]) { rm(i); }
          yrange.fi++;
        }
      } else {
        while (yrange.se > y) {
          yrange.se--;
          for (auto&& i: YI[yrange.se]) { rm(i); }
        }
      }
      dfs(dfs, tmp);
      tmp.clear();
