...
void dijkstra() {
	...
	priority_queue<tii, vii, greater<tii>> q; q.push(mt(dis[s] = 0, s));
	while(SZ(q)) {
		int x = Y(q.top()); q.pop();
...
void mian() {
	...
	REP(i, 0, n) to[i].clear(), nei[i].clear(), cls[i].clear(), way[i] = wy[i] = 0;
	while(m--) {
		int x = read(), y = read();
		nei[x].pb(y, 1), nei[y].pb(x, 1);
		to[x].pb(y), to[y].pb(x);
	}
	dijkstra();
	REP(i, 1, n) cls[dis[i]].pb(i);
	way[s] = 1;
	REP(i, 1, n) {
		for(int x : cls[i]) for(int y : to[x]) if(dis[y] < dis[x]) addto(way[x], way[y]);
	}
	wy[t] = 1;
	PER(i, n, 0) {
		for(int x : cls[i]) for(int y : to[x]) if(dis[y] > dis[x]) addto(wy[x], wy[y]);
	}
	int ans = way[t];
	REP(x, 1, n) for(int y : to[x]) if(dis[x] == dis[y]) addto(ans, (ll)way[x] * wy[y] % P);
	prt(ans), pc('\n');
}
