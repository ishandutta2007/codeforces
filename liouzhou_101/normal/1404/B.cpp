...
void solve()
{
	...
	vector<int> u(n);
	function<void(int, int)> go = [&](int x, int d)
	{
		u[x] = 1;
		if (d >= da) return;
		for (auto y : v[x])
	...
	go(a, 0);
	if (u[b]) return writeln("Alice");

	vector<int> f(n), d1(n), d2(n);
	function<void(int, int)> dfs = [&](int x, int pre)
	{
		for (auto y : v[x])
		{
			...
			if (d1[y] + 1 > d1[x])
			{
				...
				freshmax(d2[x], d1[y] + 1);
		}
		freshmax(f[x], d1[x] + d2[x]);
	};
	dfs(0, -1);
	if (f[0] <= 2 * da) return writeln("Alice");

	if (2 * da >= db) return writeln("Alice");
