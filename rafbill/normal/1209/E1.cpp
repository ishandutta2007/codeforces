...
void solve() {
  int n,m; cin>>n>>m;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(m); ++j) cin >> A[j][i];
  for(li i = 0; i < (li)(1<<n); ++i) dp[i] = 0;
  for(li i = 0; i < (li)(m); ++i) {
    for(li ma = 0; ma < (li)(1<<n); ++ma) if(ma) {
      int k = ilog2(ma);
      pre[ma] = pre[ma^(1<<k)] + A[i][k];
    }
    for(li ma = 0; ma < (li)(1<<n); ++ma) {
      for(li j = 0; j < (li)(n); ++j) {
        int ma2 = ((ma<<j) | (ma>>(n-j)))&((1<<n)-1);
        pre[ma2] = max(pre[ma],pre[ma2]);
      }
    }
    for(li ma = ((1<<n)-1); ma >= (li)(0); --ma) {
      for(int s=ma; ; s=(s-1)&ma) {
        dp[ma] = max(dp[ma], dp[s]+pre[ma^s]);
        if(s==0) break;
      }
    }
  }
