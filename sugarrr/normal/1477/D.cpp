...
ll solve(){
    ...
    rep(i,1,n)SORT(qq[i]);
    ll C=1;
    ll ansp[n+1],ansq[n+1];
    rep(i,1,n)ansp[i]=ansq[i]=-1;
    rep(i,1,n){
        if(qq[i].size()==n-1){
            ansp[i]=C;
            ansq[i]=C;
 ...
 /*   if(C==n+1){
        rep(i,1,n)cout<<ansp[i]<<" ";cout<<endl;
        rep(i,1,n)cout<<ansq[i]<<" ";cout<<endl;
        return 0;
    }*/
    vector<ll>v[n+1];
    dsu d(n+1);
    rep(i,1,n){
        if(ansp[i]==-1){
            rep(j,1,n){
                if(j==i)continue;
                ll pl=POSL(qq[i],j);
                if(pl<=(int)qq[i].size()-1&&qq[i][POSL(qq[i],j)]==j)continue;
                if(!d.same(i,j)){
                    v[i].pb(j);
                    v[j].pb(i);
                    d.merge(i,j);
  ...
  /*  rep(i,1,n){
    ...
    for(auto x:G){
        ...
        que.push(root);
        while(!que.empty()){
            ll t=que.front();que.pop();jun.pb(t);
            for(auto x:v[t]){
                if(used[x])continue;
                used[x]=true;
                que.push(x);
    ...
    ll CNT=0;
    rep(i,1,n)if(ansp[i]!=ansq[i])CNT++;
    return CNT;
}
