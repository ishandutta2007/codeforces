...
int main() {
	CASE(t) {
		int n; R(n);
		vector<vector<ll>> a(2 * n, vector<ll>(2 * n)); R(a);
		...
		ll c = LLONG_MAX;
		for (int rx = n, cx = 0, t = 0; t < 2; t++, swap(rx, cx))
			for (int i = 0, t = 0; t < 2; t++, i += n - 1)
				for (int j = 0, t = 0; t < 2; t++, j += n - 1)
					chkmin(c, a[rx + i][cx + j]);
