...
void dfs(int x,int pre){
	...
	swap(mp[x],mp[son[x]]);
	offset[x]=offset[son[x]]^a[x];
	mp[x][offset[son[x]]]=1;
	int flag=mp[x].count(offset[x]);
	for (auto y : e[x]){
		if (y==pre||y==son[x])
			continue;
		for (auto p : mp[y]){
			if (mp[x].count(p.fi^offset[y]^offset[x]))
				flag=1;
		}
		if (flag)
			break;
		for (auto p : mp[y])
			mp[x][p.fi^a[x]^offset[y]^offset[x]]=1;
	}
	if (flag){
		ans++;
		mp[x].clear();
