...
const ll N=(1ll<<22)|111;
ll g[N],fa[N],vis[N],has[N],a[N],w,n,m,ans;
ll find(ll x){return x==fa[x]?x:fa[x]=find(fa[x]);}
void link(ll x,ll y){fa[find(x)]=find(y);}
ll dfs(ll x,ll id){
	if (vis[x]){
		if (has[x])link(has[x],id);
		return has[x];
	}vis[x]=1;ll sum=0;
	rep(i,0,w)if (x>>i&1)sum+=dfs(x^(1<<i),id);
	if (g[x]){link(g[x],id);++sum;}
	if (sum)has[x]=id;
	return sum;
}
int main(){
	w=read();n=(1<<w);m=read();
	For(i,1,m)a[i]=read(),fa[i]=i,g[a[i]]=i;
	For(i,1,m)dfs(a[i]^(n-1),i);
	For(i,1,m)ans+=find(i)==i;
	writeln(ans);
}