...
string solve(){
	...
	for(int i=0; i<sx; i++){
		s1 += nx[i].first;
		maxflow.add_edge(src1, i, nx[i].first);
		maxflow.add_edge(src2, i, nx[i].second - nx[i].first);
	}
	for(int i=0; i<sy; i++){
		s2 += ny[i].first;
		maxflow.add_edge(sx + i, snk1, ny[i].first);
		maxflow.add_edge(sx + i, snk2, ny[i].second - ny[i].first);
	}
	for(int i=0; i<n; i++){
		maxflow.add_edge(x[i], y[i] + sx, 1, i);
	}
	if(s1 > n || s2 > n) return "-1";
	maxflow.add_edge(src, src1, s1);
	maxflow.add_edge(src, src2, max(s1, s2) - s1);
	maxflow.add_edge(snk1, snk, s2);
	maxflow.add_edge(snk2, snk, max(s1, s2) - s2);
	if(maxflow.match(src, snk) != max(s1, s2)) return "-1";
	maxflow.add_edge(src, src2, n - max(s1, s2));
	maxflow.add_edge(snk2, snk, n - max(s1, s2));
	maxflow.match(src, snk);
	//int dap = maxflow.match(src, snk) + max(s1, s2);
	//printf("%d\n", dap);
	string ret(n, 'a');
	for(int i=0; i<sx; i++){
