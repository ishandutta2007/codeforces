...
void solve() {
	...
	// n = rand() % 100 + 2;
	q = 100;
	// rep(n) a.push_back(rand() % 10);
	a = {49, 42, 30, 45, 3, 23, 31, 91, 99, 18, 86, 0};
	n = int(a.size());
#else
	cin >> n;
	cin >> q;
	...
	const vi& divs = divisors[n];
	const int divlen = (int)divs.size();
	...
	rep(i, divlen) {
		...
		rep(start, d) {
			...
			res[i].push_back(sum * d);
			results.insert(sum * d);
		}
	}

	debug(results);

	print(*prev(results.end()));
...
#ifdef LOCAL
	if(calc_answer() != *prev(results.end())) {
		assert(false);
	}
	debug("ok");
#endif
	...
	rep(q) {
		...
		rep(i, divlen) {
			...
			ll& pre = res[i][start];
			debug(results);
			results.erase(results.find(pre));
			pre += diff * d;
			debug(results);
			results.insert(pre);
			debug(results);
