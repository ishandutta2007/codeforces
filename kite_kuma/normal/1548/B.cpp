...
int solve() {
	...
	VEC(ll, a, n);
	ST<ll> st(
		n - 1, [](ll a, ll b) { return std::gcd(a, b); }, 0LL);

	rep(i, n - 1) { st.update(i, abs(a[i] - a[i + 1])); }
	...
	int ans = 1;
	while(l < n - 1) {
		while(st.get(l, r) > 1LL && r < n - 1) r++;
		chmax(ans, r - l);
		if(st.get(l, r) > 1LL) chmax(ans, r - l + 1);
		l++;
		while(r <= l && r < n - 1) r++;
		// if(r == n - 1) break;
	}
