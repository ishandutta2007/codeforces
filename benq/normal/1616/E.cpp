...
void solve(int tc) {
	...
	Tree<int> rem;
	F0R(i,N) rem.ins(i);
	auto get_swaps = [&](int x) -> ll {
		return rem.ook(x);
	};
	ll cur_ans = 0, min_ans = BIG;
	each(_c,T) {
		int c = _c-'a';
		int min_lef = MOD;
		F0R(i,c) if (sz(pos[i])) {
			ckmin(min_lef,pos[i].ft);
		}
		if (min_lef != MOD) {
			ckmin(min_ans,cur_ans+get_swaps(min_lef));
		}
		if (!sz(pos[c])) break;
		cur_ans += get_swaps(pos[c].front());
		rem.erase(pos[c].front());
		pos[c].pop_front();
	}
	if (min_ans == BIG) min_ans = -1;
	ps(min_ans);
}
