...
template<int MAXV> struct sieve {
    ...
    struct num {
        ...
        int  distinct_primes;   // number of distinct prime divisors

        static num ONE() { return { INT_MAX, 1, 0, 0 }; }

        num prod(int my_value, int p) const {
            if (p < least_prime)
                return { p, my_value, 1, distinct_primes + 1 };
            assert(p == least_prime);
            return { p, my_value, char(lp_multiplicity + 1), distinct_primes };
        }
    };
    vector<num> nums;
...
void solve() {
    ...
    for (int x_prime : sv.unordered_divisors(x)) {
        // is dividing twice faster than using std::div?
        if ((x_prime + d) % c != 0)
            continue;
        int q = (x_prime + d) / c;
        ans += 1 << sv[q].distinct_primes;
    }
