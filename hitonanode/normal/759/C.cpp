...
int main() {
    ...
    cin >> M;
    vector<int> init(M + 1);
    atcoder::lazy_segtree<int, op, e, int, mp, composition, id> tree(init);
    vector<int> sequence(M + 1, -1);
    REP(_, M) {
        int p, t;
        cin >> p >> t;
        if (t == 1) {
            ...
            cin >> x;
            sequence[p] = x;
            tree.apply(p, init.size(), 1);
        } else {
            tree.apply(p, init.size(), -1);
        }
        int rightmost = tree.get(init.size() - 1);
        dbg(rightmost);
        int current = rightmost - tree.all_prod();
        dbg(current);
        int ret = -1;
        if (current) {
            int l = tree.min_left(init.size(), [&](int x) { return x >= rightmost; });
            ret = sequence[l];
        }
        cout << ret << '\n';
    }
