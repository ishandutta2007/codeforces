...
int _runtimeTerror_()
{
    int n,k;
    cin>>n>>k;
    dp[1][0] = polyn(vector<Mint>{1});
    dp[1][1] = polyn(vector<Mint>{1,1});
    dp[1][2] = polyn(vector<Mint>{1,3,1});
    for(int i=2;i<30;++i)
    {
        polyn tmp = dp[i-1][1] * dp[i-1][1];
        dp[i][2] = dp[i-1][2] * dp[i-1][2] + tmp.mul_xk(1);
        tmp = dp[i-1][0] * dp[i-1][1]; 
        dp[i][1] = dp[i-1][2] * dp[i-1][1] + tmp.mul_xk(1);
        tmp = dp[i-1][0] * dp[i-1][0];
        dp[i][0] = dp[i-1][1] * dp[i-1][1] + tmp.mul_xk(1);
        for(int j=0;j<3;++j)
            dp[i][j] = dp[i][j].mod_xk(k+1);
    }   
    auto merge = [&](polyn &x,polyn &y,polyn &z,polyn &a,polyn &b,polyn &c)
    {
        polyn tmp = a * x;
        x = b * y + tmp.mul_xk(1);
        tmp = a * y; 
    ...
    for(int j=29;j>=1;--j)
    {
        if(n&(1<<j))
        {
            a = dp[j][0],b = dp[j][1],c = dp[j][2];
            for(--j;j>=1;--j)
            {
                if(n&(1<<j))
                    merge(a,b,c,dp[j][0],dp[j][1],dp[j][2]);
            }
            break;
        }
    }
    if(n >= 2 && n % 2 == 1)
        c = c + c.mul_xk(1) + b.mul_xk(1);
    else if(n == 1)
        c = dp[1][1];
    for(int i=1;i<=k;++i)
        cout<<c[i]<<" ";
