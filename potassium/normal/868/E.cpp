...
int main() {
	...
	for (int u = 1; u <= n; u++)
		if ((int)g[u].size() == 1) dp[0][u] = 0;

	auto go = [&](vector<int>& V, int u, int c, int totalC) {
		int ans = INT_MAX;
		...
		for (int v : V)
			if (c - put[v] - 1 >= 0)
				pq.push({ dist[u][v] + dp[totalC - put[v] - 1][v], v });
	...
	for (int c = 1; c <= m; c++) {
		for (int u = 1; u <= n; u++) {
			if ((int)g[u].size() != 1) continue;

			vector<int> V;
			for (int v = 1; v <= n; v++)
				if ((int)g[v].size() == 1 && v != u)
					V.emplace_back(v);
	...
	for (int u = 1; u <= n; u++) {
		...
		while (!q.empty()) {
			...
			if ((int)g[u].size() == 1) V.emplace_back(u);
			c += cnt[u];
			for (auto& v : g[u])
				if (!trav[v]) {
					trav[v] = true;
