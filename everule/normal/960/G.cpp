...
void solve(){
    ...
    ply = ply.inv(n+5);
    ply = ply.ln(n+5);
    auto pa = ply.pow(a-1, n+5);
    auto pb = ply.pow(b-1, n+5);
    /*vector<vector<mint>> s(n, vector<mint>(max(a, b), 0));
    s[0][0] = 1;
    for(int i=1;i<n;i++){
    ...
    mint ans = 0;
    for(int i=0;i<n;i++){
        ans += ((pa[i] * pb[n-i-1] * fact[n-1]) / fact[a-1]) / fact[b-1];
        //ans += s[i][a-1] * s[n-i-1][b-1] * nck(n-1, i);
    }
    cout<<ans<<"\n";
}
int main() {
