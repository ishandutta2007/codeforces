...
set<i64> cur;
vector<string> seq;
void send(i64 a,i64 b,int o){
	if(o==1&&cur.count(a^b)) return;
	if(o==0&&cur.count(a+b)) return;
	if(o==1) cur.insert(a^b); else cur.insert(a+b);
	string res="";
	res+=to_string(a);
	if(o==1) res+=" ^ ";
	else res+=" + ";
...
void work(){
	cin>>x;
	//for(x=999999;x>=3;x-=2){
	//cur.clear(); seq.clear(); v.clear();
		ins(x);
		send(x,x,0);
		ins(x+x);
		i64 C=2,pC=x;
		while(v.back()!=1){
			int r=v.size();
			int k1=rand()%v.size(),k2=rand()%v.size();
			send(v[k1],v[k2],0);
			ins(v[k1]+v[k2]);
			if(pC!=v.back()) pC=v.back(),C=1;
			send(v.back(),C*v.back(),0);
			++C;
			ins(C*v.back());
		}
	//if(seq.size()>10000) cerr<<"WARNING "<<x<<' '<<seq.size()<<'\n';
		cout<<seq.size()<<'\n';
		for(auto g:seq) cout<<g<<'\n';
	//}
}
int main(){
#ifndef ONLINE_JUDGE
