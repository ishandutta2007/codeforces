...
public class F2 {
	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			long x = ni(), t = ni();
			frog = insertb(frog, new Node(x, t, i));
			len[i] = t;
		}
		...
		while(cur != null){
			Node next = next(cur);
			if(next != null && cur.v + cur.t + 1 > next.v){
				long plus = cur.v + cur.t + 1 - next.v;
				next.t -= plus;
				next.v += plus;
				if(next.t < 0){
					frog = erase(frog, index(next));
				}else{
					cur = next;
				}
			}else{
		...
		for(int i = 0;i < m;i++){
			...
			if(ind >= 0){
				...
				if(x.id != -1 && p <= x.v + x.t){
					...
					frog = insertb(frog, new Node(p, b, -1));
				}
			}
		}
		for(int i = 0;i < n;i++){
			out.println(num[i] + " " + len[i]);
		}
	}
	...
	static public class Node
	{
		...
		public Node(long v, long t, int id)
		{
			this.v = v;
			this.t = t;
			this.id = id;
