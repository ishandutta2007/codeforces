...
int main() {
  ...
  cin >> n >> k;
  f0r(i, n) cin >> seg[i].f.f >> seg[i].f.s;
  f0r(i, n) seg[i].s = i;
  sort(seg, seg+n);
  ms(a, 0);
  st.init(bd, a);
  f0r(i, n) {
    st.update(--seg[i].f.f, --seg[i].f.s, 1);
  }
  int pt = 0;
  f0r(i, bd) {
    while (pt < n && seg[pt].f.f <= i) {
      segs.insert(seg[pt]);
      pt++;
    }
    while (segs.size() && segs.rbegin()->f.s < i) {
      segs.erase(--segs.end());
    }
    while (st.query(i, i).val > k) {
      nums.pb(segs.begin()->s);
      st.update(segs.begin()->f.f, segs.begin()->f.s, -1);
      segs.erase(segs.begin());
    }
  }
  cout << nums.size() << endl;
  for (int x: nums) cout << x + 1 << " ";
}