...
static uint16_t bit(int x)
{
    return 1 << x;
}

static mr solve1(const vector<uint16_t> &grid, int C, int badr, int badc)
{
    int R = SZ(grid);
    int Cb = 1 << C;

    vector<uint16_t> nog = grid;
    if (badr != -1)
    {
        for (int i = badc; i >= 0 && !(grid[badr] & bit(i)); i--)
            nog[badr] |= bit(i);
        for (int i = badr; i >= 0 && !(grid[i] & bit(badc)); i--)
            nog[i] |= bit(badc);
    }
    ...
    {
        vector<mr> dp2(Cb);
        for (uint16_t g = 0; g < Cb; g++)
        {
            if (g & nog[r])
                continue;
            uint16_t cover = g;
            for (int i = 0; i < C; i++)
                cover |= (cover << 1) & (Cb - 1) & ~grid[r];
            int miss = (Cb - 1) & ~grid[r] & ~cover;
            if (r == badr)
                miss &= ~bit(badc);
            for (int n = 0; n < Cb; n++)
                if (!(n & grid[r]))
...
static void solve()
{
    ...
    mr ans = solve1(grid, C, -1, -1);
    for (int i = 0; i < R; i++)
        for (int j = 0; j < C; j++)
            if (!(grid[i] & bit(j)))
                ans += solve1(grid, C, i, j);
    cout << ans << '\n';
}
