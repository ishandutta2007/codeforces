...
int main()
{
	...
	sort(a+1,a+n+1,[&](const node &a,const node &b){return a.l>b.l;});
	for(i=1;i<=n;){
		for(j=i;j<=n&&a[i].l==a[j].l;j++){
			int p=lower_bound(b+1,b+cntb+1,a[j].r)-b;
			ans+=getsum(p);
		}
		ans+=1ll*(j-i)*(j-i-1)/2; 
		for(j=i;j<=n&&a[i].l==a[j].l;j++){
