...
void solve() {
	...
	scanf("%d%d", &n, &m);
	vpii type(m);
	REP(i, m) scanf("%d%d", FS(&type[i]));
	seg_set<pii, Sum, less<pii>, true> s;
	ll ans = 0;
	REP(i, m) s.insert({type[i].first, i});
	REP(i, m) {
		auto [a, b] = type[i];
		s.erase({a, i});
		auto it = s.upper_bound(make_pair(b, inf));
		int cou = (it == s.end() ? 0 : s.size() - s.order_of_key(*it));
		if (cou > n - 1) {
			it = s.find_by_order(s.size() - (n - 1));
			cou = n - 1;
		}
		ll here = s.read(it, s.end()) + a + b * (n - 1ll - cou);
		debug(imie(a), imie(b), imie(here));
		maxi(ans, here);
		s.insert({a, i});
	}
	printf("%lld\n", ans);
