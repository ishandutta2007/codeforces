...
int calc(vector <int> a) {
	...
	int ans = 0;
	REP(i, n) {
		for (int j = i + 1, head = i + 2; head < n; ++j) {
			while (head < n && a[i] + a[j] > a[head])
				head++;
			if (head < n && a[i] + a[j] == a[head])
				ans++;
		}
	}
	rans;
...
int main() {
	...
	if (m == 0) {
		REP(i, n) printf("%d ", 2 * i + 1);
		printf("\n");
		exit(0);
	}
	int skip = 0;
	while (n > 1 && best(n - 1) >= m) {
		skip++;
		n--;
	}
	debug(imie(n), imie(skip));
	vector <int> gain(n * 5);
	for (int i = 1; i < n; ++i) for (int j = i + 1; j < n; ++j) gain[i + j]++;
	debug(imie(gain));
	vector <int> ans;
	...
	assert(found);
	int largest = ans.back();
	REP(i, skip) ans.push_back(largest * 10 * (i + 10) + 1);
	loc(assert(calc(ans) == m));
