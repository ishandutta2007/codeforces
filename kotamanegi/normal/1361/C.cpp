...
// Thanks https://www.geeksforgeeks.org/eulerian-path-undirected-graph/
...
void solve() {
    ...
    for (int i = 20; i >= 0; --i) {
        vector<vector<edge>> Graph((1 << i),vector<edge>{});
        int blind = (1 << i) - 1;
        REP(q, inputs.size()) {
            int Target_b = Graph[blind & inputs[q].second].size();
            int Target_a = Graph[blind & inputs[q].first].size();
            if ((blind & inputs[q].second) == (blind & inputs[q].first)) {
                Target_b++;
            }
            Graph[blind & inputs[q].first].push_back(edge{ blind & inputs[q].second ,q * 2,q*2+1,Target_b,false});
            Graph[blind & inputs[q].second].push_back(edge{ blind & inputs[q].first ,q * 2 + 1,q * 2,Target_a,false});
        }
        int starts = -1;
        REP(q, (1 << i)) {
            visited[q] = 0;
            ...
            if (Graph[q].size()) starts = q;
        }
        if (starts == -1) continue;
        stack<tuple<int,int,int>> now_looking;
        now_looking.push(make_tuple(starts,-1,-1));
        vector<int> ans;
        while (now_looking.empty() == false) {
            tuple<int, int, int> now = now_looking.top();
            now_looking.pop();
            int trying = 0;
            while (visited[get<0>(now)] != Graph[get<0>(now)].size()) {
                auto& x = Graph[get<0>(now)][visited[get<0>(now)]];
                visited[get<0>(now)]++;
                if (x.used == true) continue;
                trying = 1;
                x.used = true;
                Graph[x.to][x.rev].used = true;
                now_looking.push(now);
                now_looking.push(make_tuple(x.to, x.StartEdge, x.EndEdge));
                break;
            }
            if (trying == 0) {
                ans.push_back(get<2>(now));
                ans.push_back(get<1>(now));
            }
        }
        ans.pop_back();
        ans.pop_back();
