...
main() {
    ...
    rep(i, n) cin >> a[i];
    rep(i, n) cin >> b[i];
    auto aid = iota(n), bid(iota(n));
    sort(all(aid), [&](int x, int y) { return a[x] < a[y]; });
    sort(all(bid), [&](int x, int y) { return b[x] < b[y]; });
    vi reva(n), revb(n);
    rep(i, n) reva[aid[i]] = i, revb[bid[i]] = i;
    // i 
    rep(i, n) id[i] = revb[aid[i]];
    ...
    auto calc = [&](int i) -> void {
        ...
        if(id[i] != i)
            ...
            d[i][0] = -inf;
        if(i < n - 1 and id[i] != i + 1 and id[i + 1] != i)
            d[i][1] = a[i] * b[i + 1] + a[i + 1] * b[i];
        else
            d[i][1] = -inf;
        if(i < n - 2 and id[i] != i + 2 and id[i + 1] != i and id[i + 2] != i + 1)
            d[i][2] = a[i] * b[i + 2] + a[i + 1] * b[i] + a[i + 2] * b[i + 1];
        else
            d[i][2] = -inf;
        if(i < n - 2 and id[i] != i + 1 and id[i + 1] != i + 2 and id[i + 2] != i)
            d[i][3] = a[i] * b[i + 1] + a[i + 1] * b[i + 2] + a[i + 2] * b[i];
        else
            d[i][3] = -inf;
    };
    rep(i, n) { calc(i); }
    while(Q--) {
        INT(x, y);
        x--, y--;
        swap(id[reva[x]], id[reva[y]]);
        rep2(i, -3, 3) calc(reva[x] + i), calc(reva[y] + i);
        rep2(i, 1, n) dp[i] = -inf;
        rep(i, n) {
            chmax(dp[i + 1], dp[i] + d[i][0]);
            chmax(dp[i + 2], dp[i] + d[i][1]);
            chmax(dp[i + 3], dp[i] + d[i][2]);
            chmax(dp[i + 3], dp[i] + d[i][3]);
        }
        cout << dp[n] << endl;
    }
}