...
struct Value
{
    int abc=0,ab=0,bc=0,a=0,b=0,c=0;
    Value(){}
    Value(char ch)
    {
        if(ch=='a')
    ...
    Value(Value l,Value r):
        abc(min({l.abc+r.c,l.a+r.abc,l.ab+r.bc})),
        ab(min(l.ab+r.b,l.a+r.ab)),
        bc(min(l.b+r.bc,l.bc+r.c)),
        a(l.a+r.a),
        b(l.b+r.b),
        c(l.c+r.c)
    {}
...
signed main()
{
    ...
    string s;
    cin>>s;
    st.build(1,n,[s](int i){return s[i-1];});
    while(q--)
    {
        int pos;
        char ch;
