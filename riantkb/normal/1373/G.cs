...
class Solver {
    ...
    class node {
        ...
        public static node merge(node left, node right) {
            ...
            if (left.r <= right.l) {
                res.r = right.r;
                res.l = left.l + right.l - left.r;
            }
            else {
                res.l = left.l;
                res.r = right.r + left.r - right.l;
            }
            return res;
        }
    }
    ...
    public void solve() {
        ...
        --k;
        var sg = new Segtree<node>(n * 2, node.merge, new node());
        for (int i = 0; i < n * 2; i++)
        {
            sg.assign(i, new node());
        }
        var se = new HashSet<int>[n];
        for (int i = 0; i < n; i++)
        {
            se[i] = new HashSet<int>();
        }
        for (int _ = 0; _ < q; _++)
        {
            int i, j;
            sc.Multi(out i, out j);
            --i;
            --j;
            int l = j + Math.Abs(i - k);
            if (!se[i].Contains(j)) {
                sg.at(l).add(l);
                sg.update(l);
                se[i].Add(j);
            ...
            Prt(Math.Max(sg.run(0, n * 2).r - n, 0));
        }
