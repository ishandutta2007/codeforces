		...
		while (!Q.empty()) {
			...
			for (int to = 0; to < SIGMA; to++) {
				if (dfa[u].trie[to] != -1) {
					...
					dfa[v].succCount += dfa[dfa[v].link].succCount;
...
int main() {
	...
	for (int i = 0; i < 2; i++) {
		...
		for (int i = 0, node = 0; i < tl; i++) {
			int to = ACA::toNum(t[i]);
			node = aca.dfa[node].next[to];
			a[i] = aca.dfa[node].succCount;
		}
