...
signed main()
{
	...
	rg(n)if(g[i].empty());else tmp[++cc]=i;gr
	
	rg(cc)
	std::sort(g[tmp[i]].begin(),g[tmp[i]].end());
	std::reverse(g[tmp[i]].begin(),g[tmp[i]].end());
	gr
	int ans=0;
	rg(m)
	int x=read(),y=read();
	...
	int u=tmp[i],v=tmp[j];
	std::set<qwq> q;
	q.insert((qwq){(pair){0,0},(g[u][0]+g[v][0])});
	while(!q.empty())
	{
		auto p=*q.begin();
		q.erase(p);
		if(s.count((pair){g[u][p.x.x],g[v][p.x.y]})||(p.x.x==p.x.y&&u==v))
		{
			auto y=p.x,z=p.x;
			y.x++;
			if(y.x<g[u].size())q.insert((qwq){y,(g[u][y.x]+g[v][y.y])});
			y.y++;y.x--;
			if(y.y<g[v].size())q.insert((qwq){y,(g[u][y.x]+g[v][y.y])});
		}
		else
		{
			// odp(i,j);
