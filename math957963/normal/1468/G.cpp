...
using Point = pll;
ll cross(Point x, Point y) { return x.fi * y.se - x.se * y.fi; }
using Line = pair<Point, Point>;
double crosspoint(const Line &l, const Line &m) {
    ll a = cross(l.se - l.fi, m.se - m.fi);
    ll b = cross(l.se - l.fi, l.se - m.fi);
    double len = hypot(m.fi.fi - m.se.fi, m.fi.se - m.se.se);
    if(!a and !b) return len;
    return len * b / a;
}
...
int main() {
    ...
    double ans = 0;
    Point p(P.back());
    Point A(P.back().fi, P.back().se + H);
    rep3(i, n - 2, 0) {
        if(cross(A - p, A - P[i]) <= 0) {
            ans += crosspoint(Line(A, p), Line(P[i], P[i + 1]));
            p = P[i];
        }
    }
    cout << ans << endl;
