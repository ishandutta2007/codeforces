...
int main(){
  ...
  while(t--){
    ...
    HLD hld(g,0);
    auto ord=descend(hld.dep);
    vector<ll>d(n+1,n+1);
    //d[i]:=ix
    ll x=ord[0],y=ord[0],mx=0;
    d[0]=0;
    for(auto i:ord){
      if(i==0)break;
      if(chmax(mx,hld.dist(x,i)))y=i;
      if(chmax(mx,hld.dist(y,i)))x=i;
      //cout<<i spa x spa y spa mx<<endl;
      d[hld.dep[i]-1]=max(0LL,hld.dep[i]-1-(mx+1)/2);
    }
    rrep(i,0,n)chmin(d[i],d[i+1]);
    //debug(d);
    vector<ll>ret(n);
    rep(i,0,n){
      rep(j,d[i]+1,d[i+1]+1){
        if(j>=1&&j<=n)ret[j-1]=i+1;
      }
    }
    debug(ret);
  }
