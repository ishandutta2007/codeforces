...
int32_t solve() {
    ...
    int cnts = 0;
    FI(n) {
        cin >> name[i] >> isSample[i];
        cnts += isSample[i];
    }
    for (int i = 1; i <= cnts; ++i) {
        freeSamples.insert(encode(toS(i)));
    }
    for (int i = cnts + 1; i <= n; ++i) {
        freeTests.insert(encode(toS(i)));
    }
    set<ll> samples = freeSamples;
    set<ll> tests = freeTests;
    vector<int> testsOnSamples;
    vector<int> samplesOnTests;
    FI(n) {
        ll code = encode(name[i]);
        oldName[i] = code;
        if (freeSamples.count(code)) {
            freeSamples.erase(code);
            if (isSample[i]) {
                used[i] = true;
            } else {
                testsOnSamples.push_back(i);
            }
        }
        if (freeTests.count(code)) {
            freeTests.erase(code);
            if (!isSample[i]) {
                used[i] = true;
            } else {
                samplesOnTests.push_back(i);
            }
        }
    }
    auto isSample = [&](ll x) {
        return samples.count(x);
    };
    auto isTest = [&](ll x) {
        return tests.count(x);
    };
    while (true) {
        if (!testsOnSamples.empty() && !freeTests.empty()) {
            int id = testsOnSamples.back();
            used[id] = true;
            auto test = freeTests.begin();
            ans.push_back({name[id], decode(*test)});
            freeTests.erase(test);
            testsOnSamples.pop_back();
            ll old = oldName[id];
            if (isSample(old)) {
                freeSamples.insert(old);
            }
            continue;
        }
        if (!samplesOnTests.empty() && !freeSamples.empty()) {
            int id = samplesOnTests.back();
            used[id] = true;
            auto test = freeSamples.begin();
            ans.push_back({name[id], decode(*test)});
            freeSamples.erase(test);
            samplesOnTests.pop_back();
            ll old = oldName[id];
            if (isTest(old)) {
                freeTests.insert(old);
            }
            continue;
        }
        break;
    ...
    for (int i = 0; i < n; ++i) {
        if (used[i]) continue;
        if (::isSample[i] && !freeSamples.empty()) {
            ans.push_back({name[i], decode(*freeSamples.begin())});
            used[i] = true;
            freeSamples.erase(freeSamples.begin());
            continue;
        }
        if (!::isSample[i] && !freeTests.empty()) {
            ans.push_back({name[i], decode(*freeTests.begin())});
            used[i] = true;
            freeTests.erase(freeTests.begin());
            continue;
        }
    }
    string f = "111111";
    bool e = true;
    string where = f;
//    DBN(samplesOnTests, testsOnSamples);
    while (samplesOnTests.size() > 0 || testsOnSamples.size() > 0) {
        e = false;
        int id = samplesOnTests.back();
        samplesOnTests.pop_back();
        ans.push_back({name[id], where});
        int tid = testsOnSamples.back();
        testsOnSamples.pop_back();
        ans.push_back({name[tid], name[id]});
        where = name[tid];
    }
    if (!e) {
        ans.push_back({f, where});
    }
    cout << ans.size() << endl;
    for (auto p : ans) {
