...
int vit[N];
vector<int>nxt[N];
inline void work(int x,int fa,int dep)
{
    if(nxt[x].size()==1)vit[x]=dep;else vit[x]=-1;
    bool nows=false;
    for(vector<int>::iterator i=nxt[x].begin();i!=nxt[x].end();i++)
    {
        if((*i)==fa)continue;work(*i,x,dep^1);
        if(vit[x]==-1)vit[x]=vit[*i];else if(vit[x]!=vit[*i])tag=true;
        if(nxt[*i].size()==1&&!nows)nows=true,ans++;if(nxt[*i].size()>1)ans++;
    }
    return;
}
int main()
{
    rin>>n;for(int i=1,x,y;i<n;i++){rin>>x>>y;nxt[x].push_back(y);nxt[y].push_back(x);}
    for(int i=1;i<=n;i++)if(nxt[i].size()!=1){work(i,0,0);break;}printf("%d %d\n",tag?3:1,ans);
    return 0;
}