...
int main(){
    ...
    rep(zz,q){
        ...
        if(mp.find(MP(a,b))!=mp.end()){// 
            ...
            int ym = (b+c)%2;
            cnt_yoko[a][ym]--;
            if(cnt_yoko[a][ym]==0&&cnt_yoko[a][1-ym]>=1){
                ng_yoko--;
            }
            if(cnt_yoko[a][ym]==0&&cnt_yoko[a][1-ym]==0){
                fix_yoko--;
            }
            int tm = (a+c)%2;
            cnt_tate[b][tm]--;
            if(cnt_tate[b][tm]==0&&cnt_tate[b][1-tm]>=1){
                ng_tate--;
            }
            if(cnt_tate[b][tm]==0&&cnt_tate[b][1-tm]==0){
                fix_tate--;
            }
            mp.erase(MP(a,b));
        }
        ...
        if(c!=-1){
            ...
            int ym = (b+c)%2;
            cnt_yoko[a][ym]++;
            if(cnt_yoko[a][ym]==1&&cnt_yoko[a][1-ym]>=1){
                ng_yoko++;
            }
            if(cnt_yoko[a][ym]==1&&cnt_yoko[a][1-ym]==0){
                fix_yoko++;
            }
            int tm = (a+c)%2;
            cnt_tate[b][tm]++;
            if(cnt_tate[b][tm]==1&&cnt_tate[b][1-tm]>=1){
                ng_tate++;
            }
            if(cnt_tate[b][tm]==1&&cnt_tate[b][1-tm]==0){
                fix_tate++;
            }
        }
        mod res = 0;
        dbg(ng_tate,ng_yoko,fix_tate,fix_yoko);
        if(ng_tate==0){
            res += mod_pow((mod)2,m-fix_tate);
        }
        if(ng_yoko==0){
            res += mod_pow((mod)2,n-fix_yoko);
        }
        if(cnt[0]==0){
            res -= 1;
        }
