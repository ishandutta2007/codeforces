...
mat g;
vec dep,par;
void dfs(ll now=0,ll p=-1,ll d=1){
  dep[now]=d;
  par[now]=p;
  for(auto x:g[now]){
    if(dep[x])continue;
...
// int codeforces;cin>>codeforces;while(codeforces--){
  ...
  dfs();
  HLD tree(N);
  rep1(i,N)tree.add_edge(par[i],i);
  tree.init();
  if(M==N-1){
    K=(K+1)/2;
    vec cnt(2,0);
    ...
    rep(i,N){
      if(cnt[0]>=cnt[1]){
        if(~dep[i]&1)ans.emplace_back(i+1);
      }else{
        if(dep[i]&1)ans.emplace_back(i+1);
      }
    }
  ...
  for(auto [a,b]:edge){
    ...
    if(chmin(anssize,dep[a]+dep[b]-dep[tree.lca(a,b)]*2+1))loop=datas(a,b);
  }
  vec ca,cb;
  ll p=tree.lca(loop.first,loop.second);
  while(loop.first!=p){
    ca.emplace_back(loop.first+1);
    loop.first=par[loop.first];
  }
  while(loop.second!=p){
    cb.emplace_back(loop.second+1);
    loop.second=par[loop.second];
  }
  ca.emplace_back(p+1);
  reverse(all(cb));
  for(auto x:cb)ca.emplace_back(x);
  if(ll(ca.size())<=K){
    print(2);
    print(ca.size());
    output(ca);
