#line 1 "codeforces-workspace/contest/1506/g/g.cpp"
...
#line 11 "codeforces-workspace/contest/1506/g/g.cpp"
...
void main() {
  ...
  string ans;
  int now = 0;
  while (count(begin(stat), end(stat), 0)) {
    for (char c = 'z'; c >= 'a'; --c) {
      if (stat[c]) continue;
      int nxt = s.find(c, now);
      if (~nxt) {
        bool fail = 0;
        for (char c = 'a'; c <= 'z'; ++c) {
          if (stat[c]) continue;
          if (s.find(c, nxt) == string::npos) fail = 1;
        }
        if (!fail) {
          stat[c] = 1;
          ans += c;
