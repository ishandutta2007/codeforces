...
void main_() {
	...
	ll ans = INF;
	REP(bit, 1 << 10) {
		if(popcnt(bit) != k) continue;
		set<int> st;
		REP(i, 10) {
			if(bit >> i & 1) st.insert(i);
		}
		auto dfs = [&](auto dfs, int i, ll res, bool f) {
			if(i == n) {
				if(res >= N) chmin(ans, res);
				return;
			}
			if(f) {
				while(i < n) res = res * 10 + *st.begin(), i++;
				if(res >= N) chmin(ans, res);
				return;
			}
			int d = s[i] - '0';
			...
			dfs(dfs, i + 1, res * 10 + *itr, (*itr > d));
			if(*itr > d) return;
			itr++;
			if(itr == st.end()) return;
			dfs(dfs, i + 1, res * 10 + *itr, 1);
		};
		dfs(dfs, 0, 0, 0);
	}
	print(ans);
