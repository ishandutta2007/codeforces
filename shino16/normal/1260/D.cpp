...
int main() {
  int m = IN, n = IN, k = IN, t = IN;
  auto a = IN.vi(m);
  VI l(k), r(k), d(k);
  rep(i, k) IN >> l[i] >> r[i] >> d[i];
  vector<tuple<int, int, int>> lrd(k);
  rep(i, k) lrd[i] = make_tuple(l[i], r[i], d[i]);
  sort(all(lrd));
  rep(i, k) tie(l[i], r[i], d[i]) = lrd[i];

  int ok = 2e5+1, ng = -1;  // a[i] == ok is saved
  while (ok - ng > 1) {
    debug(ok); debug(ng);
    int m = (ok+ng)/2;
    VI ll, rr;
    rep(i, k) if (d[i] > m) {
      if (not rr.empty() and rr.back() >= l[i]) {
        chmin(ll.back(), l[i]);
        chmax(rr.back(), r[i]);
      } else {
        ll.push_back(l[i]);
