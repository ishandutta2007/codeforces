...
void solve(){
    ...
    LazySegmentTree<int> seg(n + 2, [](int a, int b) {return min(a, b); },
        [](int a, int b, int c) {return a + b; },
        [](int a, int b) {return a + b; }, 1e18, 0);
    REP(i, n) {
        seg.set(i, inputs[i]);
    }
    seg.build();
    vector<tuple<int, int, int, int>> nexts;
    REP(i, queries.size()) {
        seg.update(queries[i].first, queries[i].second, -1);
        nexts.push_back(make_tuple(queries[i].first, queries[i].first, queries[i].second, 1));
        nexts.push_back(make_tuple(queries[i].second, queries[i].first, queries[i].second, -1));
    }
    sort(ALL(nexts));
    reverse(ALL(nexts));
    int ans = -1e18;
    ...
    vector<int> teas;
    REP(i, n) {
        while (nexts.empty() == false && get<0>(nexts.back()) == i) {
            seg.update(get<1>(nexts.back()), get<2>(nexts.back()), get<3>(nexts.back()));
            nexts.pop_back();
        }
        int now_ans = inputs[i] - seg.query(0, n);
        if (ans < now_ans) {
            ans = now_ans;
            itr = i;
        }
    ...
    REP(q, n) {
        int i = itr;
        int geko = inputs[i] - inputs[q];
        vector<int> tmps;
        REP(j, m) {
            if (queries[j].first <= i && i < queries[j].second) {
                continue;
            }
            if (queries[j].first <= q && q < queries[j].second) {
                geko++;
                tmps.push_back(j + 1);
            }
        }
