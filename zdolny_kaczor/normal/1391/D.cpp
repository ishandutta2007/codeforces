...
void solve() {
	...
	REP(start, (1 << n)) {
		int oth = 0;
		REP(i, n - 1) {
			if ((start >> i & 1) ^ (start >> (i + 1) & 1) ^ (oth >> i & 1) ^ 1)
				oth |= 1 << (i + 1);
		}
		debug(imie(start), imie(oth));
		int curr = 0;
		REP(j, m) {
			int diff = 0;
			int take = j % 2 ? start : oth;
			REP(i, n) {
				debug(arr(in, i, j), imie(take) >> i & 1);
				if (in[i][j] - '0' != (take >> i & 1))
					diff++;
			}
