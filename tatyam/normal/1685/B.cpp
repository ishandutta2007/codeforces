...
bool solve(){
    LL(a,b,c,d);
    STR(s);
    const ll A=a+c+d,B=b+c+d;
    if(count(s,'A')!=A||count(s,'B')!=B)return 0;
    vector<ll>div={0};
    const ll n=s.size();
    rep(i,1,n)if(s[i-1]==s[i])div.push_back(i);
    div.push_back(n);
    vector<ll>odd,evenC,evenD;
    rep(i,1,div.size()){
        const ll l=div[i-1],r=div[i];
        if((r-l)%2)odd.push_back(r-l);
        elif(s[l]=='A')evenC.push_back(r-l);
        else evenD.push_back(r-l);
    }
    Sort(evenC);
    Sort(evenD);
    each(x,evenC){
        if(c>=x/2)c-=x/2;
        else odd.push_back(x-1);
    }
    each(x,evenD){
        if(d>=x/2)d-=x/2;
        else odd.push_back(x-1);
