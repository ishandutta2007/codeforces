...
int k[1007], m[1007], e[1007][17], isTraversed[1007][2527], ans[1007][2527];
bool cycle[1007];
int main() {
	int n; R(n);
	...
	while (q--) {
		...
		int node = x;
		int cval = ((y + k[x]) % 2520 + 2520) % 2520;
		isTraversed[node][cval] = 0;

		bool shouldContinue = 1;
		if (ans[node][cval] != -1) shouldContinue = 0;
		while (shouldContinue) {
			//W(node,cval);
			path.pb(mp(node, cval));

			node = e[node][cval % m[node]];
			cval = ((cval + k[node]) % 2520 + 2520) % 2520;

			if (isTraversed[node][cval]) {
				if (ans[node][cval] == -1) {
					for (int i = 1; i <= n; i++) cycle[i] = 0;
					int cyclelen = 0;
					for (int i = isTraversed[node][cval]; i < path.size(); i++)
						if (cycle[path[i].F] == 0) {
							cycle[path[i].F] = 1;
							cyclelen++;
						}
					for (auto& i : path)
						ans[i.F][i.S] = cyclelen;
				} else {
					for (auto& i : path)
						ans[i.F][i.S] = ans[node][cval];
				}
				shouldContinue = 0;
				continue;
			}

			isTraversed[node][cval] = path.size();
		}
