...
struct sol{
  vector<LL> xs;
  vector<pll> P,S;
  sol(vector<pll> A):xs(sz(A)){
    forv(i,A){
      xs[i]=A[i].X;
    }
    sort(all(xs));
    xs.erase(unique(all(xs)),xs.end());
    vector<pll> V(sz(xs)+1,mp(LINF,-LINF));
    P=S=V;
    forv(i,A){
      int x=lower_bound(all(xs),A[i].X)-xs.begin();
      V[x]=mer(V[x],mp(A[i].Y,A[i].Y));
    } 
    forv(i,xs){
      P[i+1]=mer(P[i],V[i]);
    }
  ...
  bool check(LL D){
    ...
    forv(a,xs){
      if(xs[a]>0)
	break;
      int b=upper_bound(all(xs),min(-xs[a],xs[a]+d))-xs.begin();
      pll t=mer(P[a],S[b]);
      if(t.X==LINF || (t.Y-t.X<=d && sqr(max(t.Y,-t.X))+sqr(-xs[a])<=D))
	return true;
    }
    return false;
  }
