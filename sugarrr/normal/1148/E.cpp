...
int main(){fastio
    ...
    rep(i,0,n-1){cin>>a[i].fi;a[i].se=i+1;}
    rep(i,0,n-1)cin>>b[i];
    {
        ll sum=0;
        for(auto x:a)sum+=x.fi;
        for(auto x:b)sum-=x;
        if(sum!=0)no();
    }
    SORT(a);SORT(b);
    ll posr=0;
    while(posr<=n-1&&a[posr].fi<=b[posr])posr++;
    rep(posl,0,n-1){
        if(a[posl].fi>b[posl])no();
        while(a[posl].fi<b[posl]){
            assert(posr<=n-1);
            ll dl=b[posl]-a[posl].fi;
            ll dr=a[posr].fi-b[posr];
            ll dis=min(dl,dr);
            ans.pb({a[posl].se,a[posr].se,dis});
            a[posl].fi+=dis;
            a[posr].fi-=dis;
            assert(a[posl].fi<=a[posr].fi);
            while(posr<=n-1&&a[posr].fi<=b[posr])posr++;
        }
    }
    rep(i,0,n-1){
