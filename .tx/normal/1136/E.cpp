...
struct ST {
    ...
    llong op(int i, int l, int r, int ql, int qr, int qx) {
        push(i, l, r);
        if (r <= ql || qr <= l) {
            return 0;
        }
        ...
        int m = (l + r) >> 1;
        llong res = op(i + i + 1, l, m, ql, qr, qx) + op(i + i + 2, m, r, ql, qr, qx);
        pull(i);
        return res;
    }
};
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < n; i++) {
        st.op(0, 0, n, i, i + 1, a[i]);
        if (i == 0 || a[i] - a[i - 1] > k[i - 1]) {
            s.insert(i);
        }
    }
    ...
    for (int iq = 0; iq < q; iq++) {
        ...
        if (t == "s") {
            ...
            while (x) {
                ...
                llong d = st.op(0, 0, n, *it, (*it) + 1, 0) - st.op(0, 0, n, *(it) - 1, *it, 0);
                int t = min<llong>(d - k[*(it) - 1], x);
                st.op(0, 0, n, i, *it, t);
                x -= t;
                if (t == d - k[*(it) - 1]) {
                    s.erase(*it);
