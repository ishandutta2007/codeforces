...
void add(vector<ll> &dp, int i){ //O(K)
    rep3(j, K, T[i]){
        chmax(dp[j], dp[j-T[i]]+A[i][T[i]]);
    }
}

void partially_add(const vector<ll> &dp, int i){ //O(K)
    rep2(j, 0, K){
        if(K-j > T[i]) continue;
        chmax(ans, dp[j]+A[i][K-j]);
    }
}

void rec(const vector<ll> &dp, int l , int r){
    if(r-l == 1){
        partially_add(dp, l);
        return;
    }
    int m = (l+r)/2;
    auto dpl = dp;
    rep2(i, m, r-1) add(dpl, i);
    rec(dpl, l, m);
    auto dpr = dp;
    rep2(i, l, m-1) add(dpr, i);
    rec(dpr, m, r);
}
...
int main(){
    ...
    rep(i, N){
        in(T[i]);
        A[i].resize(T[i]+1);
        rep2(j, 1, T[i]) in(A[i][j]);
        A[i][0] = 0;
        rep2(j, 1, T[i]) A[i][j] += A[i][j-1];
    }
