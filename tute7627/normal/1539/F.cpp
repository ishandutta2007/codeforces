...
int main(){
  ...
  auto segmi=add_min::make(n+1);
  auto segmx=add_max::make(n+1);
  rep(i,0,n+1){
    segmi.set(i,i);
    segmx.set(i,i);
  }
  ...
  rep(i,0,n){
    for(auto j:v[i]){
      ll q=segmx.query(j+1,n+1)-segmi.query(0,j+1);
      q--;
      if(q&1)q++;
      chmax(ret[j],abs(q/2));
    }
    for(auto j:v[i]){
      segmi.update(j+1,n+1,-2);
      segmx.update(j+1,n+1,-2);
    }
    for(auto j:v[i]){
      ll q=segmi.query(j+1,n+1)-segmx.query(0,j+1);
      q++;
      if(q&1)q++;
      chmax(ret[j],abs(q/2));
    }
