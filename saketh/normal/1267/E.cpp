...
int main() {
    ...
    int N, M; re(N, M); vvi vote(M, vi(N)); re(vote);

    vi ans(M); F0R (i, M) ans[i] = i + 1;
    ...
    F0R (geq, N - 1) {
        ...
        F0R (i, M) {
            if (balance >= 0) break;
            can.pb(stations[i].s + 1);
            balance -= stations[i].f;
        }

        if (sz(can) < sz(ans)) ans = can;
    }
