...
void run() {
    ...
    array<array<vector<int>, 2>, 2> pos;

    for (int i = 0; i < n; ++i) {
        pos[A[i] - '0'][B[i] - '0'].pb(i);
    }

    const int a = len(pos[0][0]), b = len(pos[0][1]), c = len(pos[1][0]), d = len(pos[1][1]);
    ...
    for (int b1 = 0; b1 <= b; ++b1) {
        for (int c1 = 0; c1 <= c; ++c1) {
            ...
            vector<int> ans;
            ans.insert(ans.end(), pos[0][0].begin(), pos[0][0].begin() + a1);
            ans.insert(ans.end(), pos[0][1].begin(), pos[0][1].begin() + b1);
            ans.insert(ans.end(), pos[1][0].begin(), pos[1][0].begin() + c1);
            ans.insert(ans.end(), pos[1][1].begin(), pos[1][1].begin() + d1);
            for (auto x : ans) {
                cout << x + 1 << " ";
            }
            cout << endl;
