...
signed main() {
  ...
  FOR3(n, 1, N + 1) {
    if (n >= 3) {
      auto x = max({B[n - 3], B[n - 2], B[n - 1]});
      auto y = min({B[n - 3], B[n - 2], B[n - 1]});
      if (chmin(DP[n], DP[n - 3] + x - y)) PAR[n] = n - 3;
    }
    if (n >= 4) {
      auto x = max({B[n - 4], B[n - 3], B[n - 2], B[n - 1]});
      auto y = min({B[n - 4], B[n - 3], B[n - 2], B[n - 1]});
      if (chmin(DP[n], DP[n - 4] + x - y)) PAR[n] = n - 4;
    }
    if (n >= 5) {
      auto x = max({B[n - 5], B[n - 4], B[n - 3], B[n - 2], B[n - 1]});
      auto y = min({B[n - 5], B[n - 4], B[n - 3], B[n - 2], B[n - 1]});
      if (chmin(DP[n], DP[n - 5] + x - y)) PAR[n] = n - 5;
    }
  }
  vi PATH({N});
  while (1) {
  ...
  vi ANS(N);
  FOR(i, K) {
    ll L = PATH[i], R = PATH[i + 1];
    FOR3(j, L, R) { ANS[IDS[j]] = i + 1; }
  }
  print(ANS);
