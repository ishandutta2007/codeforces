...
fun solve() {
    val (n1,n2,m) = io.ints(3)
    val k = io.ints(n1)
    val D = DSU(n1+n2)
    val mark = ArrayList<Int>()
    repeat(m) {
        val (x,y) = io.ints(2).map{it-1}
        if (D.unite(x,n1+y)) {
    ...
    val marked = BooleanArray(n1+n2)
    for (i in mark) marked[D.get(i)] = true
    val mn = IntArray(n1+n2){Int.MAX_VALUE}
    for (i in 0 until n1) if (!marked[D.get(i)]) {
        mn[D.get(i)] = minOf(mn[D.get(i)],k[i])
    }
    var ans = 0
    for (i in mn) if (i != Int.MAX_VALUE) ans += i
    println(ans)
}
