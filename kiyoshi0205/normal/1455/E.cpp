...
ll solve(vec x,vec y){
  if(x[0]>x[1])swap(x[0],x[1]);
  if(x[2]>x[3])swap(x[2],x[3]);
  if(y[0]>y[1])swap(y[0],y[1]);
  if(y[2]>y[3])swap(y[2],y[3]);
  debug(x,y);
  B=max(x[3]-x[0],x[1]-x[2]);
  D=max(y[3]-y[0],y[1]-y[2]);
  if(x[0]>x[2])swap(x[0],x[2]),swap(x[1],x[3]);
  if(y[0]>y[2])swap(y[0],y[2]),swap(y[1],y[3]);
  A=max(x[2]-x[1],0LL);
  C=max(y[2]-y[1],0LL);
  debug(A,B,C,D);
  if(A>C)swap(A,C),swap(B,D);
  ll res=x[1]+x[3]-x[0]-x[2]+y[1]+y[3]-y[0]-y[2]+max(0LL,C-B)*2;
  debug(res);
  return res;
}
ll ans;
