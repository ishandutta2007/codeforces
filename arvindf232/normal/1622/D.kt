...
fun main(){
    ...
    solve.cases{
        ...
        val DP = Array(pos.size){IntArray(pos.size){1} }

        for(d in 1..n){
            for(l in pos.indices){
                val r = l+d+1
                if(r !in pos.indices) continue
                val start  = pos[l] + 1
                val end = pos[r] -1
                 val base = FACT.choose(end-start+1, d)
                DP[l][r] = base
            }
        }
        if(k > str.count { it == '1' }){
...
//        just dei DP
        ...
        for(d in 1..k) {
            for(l in pos.indices) {
                val r = l + d + 1
                if(r !in pos.indices) continue
                val real = DP[l][r] modPlus DP[l + 1][r - 1] modMinus (DP[l + 1][r] modPlus DP[l][r - 1])

//                "$l $r" dei real
                ret = ret modPlus real
            }
        }
        put(ret modPlus 1)
