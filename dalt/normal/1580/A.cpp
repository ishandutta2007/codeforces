...
void SolveOne(int test_id, IStream &in, OStream &out) {
  ...
  Debug("Read input");
  Vec<Vec<int>> colPS = mat;
  for(int i = 1; i <= N; i++) {
    for(int j = 1; j <= M; j++) {
      colPS[i][j] += colPS[i - 1][j];
    }
  }
  auto query = [&](int c, int l, int r) {
    return colPS[r][c] - colPS[l - 1][c]; 
  };
  Vec<int> A(M + 1);
  Vec<int> B(M + 1);
  int global_best = 1e9;
  for(int b = 1; b <= N; b++) {
    for(int h = b + 4; h <= N; h++) {
      int H = h - b + 1;
      for(int j = 1; j <= M; j++) {
        A[j] = H - 2 - query(j, b + 1, h - 1);
        B[j] = query(j, b + 1, h - 1) + 1 - mat[b][j] + 1 - mat[h][j];
        B[j] += B[j - 1];
      }
      //B as prefix sum
      int best = 1e9;
      ...
      for(int j = 2; j <= M; j++) {
        while(scan + 3 <= j) {
          best = Min(best, A[scan] - B[scan]);
          scan++;
        }
        local_best = Min(local_best, best + A[j] + B[j - 1]);
      }
      Debug(b);
      Debug(h);
      Debug(local_best);
