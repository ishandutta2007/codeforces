  ...
  for(ll x:{0LL,t}){
    ...
    rep(i,idx,m){
      ...
      rep(j,0,n){
        ...
        else if(tv[j]==1)cv.PB(j);
      }
      if(!all)continue;
      //debug(tv,n);
      if(ci!=-1){
        ll v1=s[ci][idx],v2=s[ci][i];
        bool sw=true;
        rep(j,0,n)if(tv[j]+(v1!=s[j][idx])+(v2!=s[j][i])>2)sw=false;
        if(sw){
          ret=s[x];
          ret[idx]=v1,ret[i]=v2;
        }
        ...
        ll k=cv[0];
        ll v1=s[k][idx],v2=s[k][i];
        set<ll>st1,st2;
        for(auto z:cv){
          if(s[z][idx]!=v1)st1.insert(s[z][i]);
          if(s[z][i]!=v2)st2.insert(s[z][idx]);
        }
        if(st1.empty())st1.insert(v2);
        if(st2.empty())st2.insert(v1);
        if(st1.size()<=1){
          ret=s[x];
          ret[idx]=v1,ret[i]=*st1.begin();
        }
        if(st2.size()<=1){
          ret=s[x];
          ret[idx]=*st2.begin(),ret[i]=v2;
        }
      }
    }
  }
  ans1(!ret.empty());
  if(!ret.empty())debug(ret,m);
  return 0;
}