...
int n, m, k, s;
ll a[MAX_N], b[MAX_N], type[MAX_N];
vector<vector<pair<ll, int> > > v(two);

bool can(int t, bool wr)
{
    int d1 = 0;
    for (int i = 1; i < t; i++)
        if (a[i] < a[d1])
    ...
    for (int i = 0; i < v[0].size(); i++)
        al.push_back(make_pair(a[d1] * v[0][i].first, v[0][i].second));
    for (int i = 0; i < v[1].size(); i++)
        al.push_back(make_pair(b[d2] * v[1][i].first, v[1][i].second));
