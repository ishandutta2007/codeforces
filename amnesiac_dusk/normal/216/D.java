...
public class Main
{
	public static void main(String[] args)
	{
		...
		for(int i=0; i<n; i++)
		{
			for(int j=0,k=0,l=0; j<a[i].length-1; j++)
			{
				int te1=a[i][j],te2=a[i][j+1];
				while(k<a[(i-1+n)%n].length&&a[(i-1+n)%n][k]<te1)
					k++;
				while(l<a[(i+1)%n].length&&a[(i+1)%n][l]<te1)
					l++;
				int an1=0,an2=0;
				while(k<a[(i-1+n)%n].length&&a[(i-1+n)%n][k]<te2)
				{
					k++;
					an1++;
				}
				while(l<a[(i+1)%n].length&&a[(i+1)%n][l]<te2)
				{
					l++;
					an2++;
				}
