...
#line 1 "library/alg/monoid_minmax.hpp"
...
void solve() {
  ...
  ll ANS = 0;
  FOR(root, N) if (!done[root]) {
    vi C = {root};
    while (1) {
      ll v = C.back();
      for (auto&& e: G[v]) {
        if (len(C) >= 2 && C[len(C) - 2] == e.to) continue;
        C.eb(e.to);
        break;
      }
      if (C.back() == root) break;
    }
    C.pop_back();
    for (auto&& c: C) done[c] = 1;
    ll n = len(C);
    FOR(i, n) C.eb(C[i]);
    /*
    ...
    */
    ll nxt = 0;
    FOR3(k, 2, n) {
      SWAG<Monoid_MinMax<ll, INF>> swag1, swag2;
      auto add = [&](ll i) -> void {
        auto v = C[i];
        if (v < N) {
          swag1.push({v, v});
      ...
      FOR(L, n) {
        while (R < L + k) { add(R++); }
        auto [xl, xr] = swag1.prod();
        auto [yl, yr] = swag2.prod();
        swag1.pop();
        swag2.pop();
        // 
        ll ng1 = C[n + L - 1];
        ll ng2 = C[R];
        ll cnt1 = 0, cnt2 = 0;
        if (xl <= ng1 && ng1 <= xr) continue;
        if (xl <= ng2 && ng2 <= xr) continue;
        if (yl <= ng1 && ng1 <= yr) continue;
        if (yl <= ng2 && ng2 <= yr) continue;
        ll lo = 0, hi = N;
        if (0 <= ng1 && ng1 < xl) chmax(lo, ng1 + 1);
        if (0 <= ng2 && ng2 < xl) chmax(lo, ng2 + 1);
        if (xr <= ng1 && ng1 < N) chmin(hi, ng1);
        if (xr <= ng2 && ng2 < N) chmin(hi, ng2);
        cnt1 = (xl - lo + 1) * (hi - xr);
        lo = N, hi = N + N;
        if (N <= ng1 && ng1 < yl) chmax(lo, ng1 + 1);
        if (N <= ng2 && ng2 < yl) chmax(lo, ng2 + 1);
        if (yr <= ng1 && ng1 < N + N) chmin(hi, ng1);
        if (yr <= ng2 && ng2 < N + N) chmin(hi, ng2);
        cnt2 = (yl - lo + 1) * (hi - yr);
        ll cnt = cnt1 * cnt2;
        // print(k, cnt);
        ANS += (k / 2) * cnt;
