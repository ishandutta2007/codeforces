...
//int primes[2];
//int step[2] = {31, 37};
//int stepPower[2][202002];
//struct Hash {
//    int val[2];
//    int position;
//    void addChar(int c) {
//        FI(2) val[i] = (val[i] * 1LL * step[i] + c) % primes[i];
//    }
//    bool operator<(const Hash& other) const {
//        return val[0] < other.val[0] || (val[0] == other.val[0] && val[1] < other.val[1]);
//    }
//    void sub(Hash& h, int dif) {
//        FI(2) {
//            val[i] = (val[i] - stepPower[i][dif] * h.val[i]) % primes[i]; if (val[i] < 0) val[i] += primes[i];
//        }
//    }
//};
//vector<Hash> hashes[101010];
//string qm[101010], qk[101010];
//Hash hp[101010];
struct state {
    bool is_clon;
    int first_pos;
...
void output_all_occurences (int v) {
    if (! st[v].is_clon) {
        res.push_back(st[v].first_pos);
    }
    for (size_t i = 0; i < st[v].inv_link.size(); ++i) {
        output_all_occurences(st[v].inv_link[i]);
    }
}
int solve() {
//    FI(2) {
...
//    DBN(s);
    ...
    FI(n) {
        ...
        for (char c : m) {
            c -= 'a';
            if (!st[cur].next.count(c)) { cur = -1; break; }
            cur = st[cur].next[c];
        }
        if (cur == -1) {
            cout << "-1\n";
