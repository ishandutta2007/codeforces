...
//		for(int m = 1; m <= n; m++)
//			printf("%d %d: %I64d\n", i, m, det(b[i].first, a[m].first - nowp));
		if(l > 1 && det(b[i].first, a[l - 1].first - nowp) == 0)
			l--;
		if(l > 2 && det(b[i].first, a[l - 2].first - nowp) == 0)
			l -= 2;
		if(l + 2 <= n && det(b[i].first, a[l + 2].first - nowp) == 0 && det(b[i].first, a[l + 1].first - nowp) <= 0)
			l++;
//		printf("%d: %d %I64d %I64d\n", i, l, det(b[i].first, a[l].first - nowp), det(b[i].first, a[l + 1].first - nowp));
		ans += (long long)((l - 1) * (l - 2) / 2) * ((n - l - 1) * (n - l - 2) / 2);
	}
	ans /= 2;
	printf("%I64d\n", ans);
	return 0;
