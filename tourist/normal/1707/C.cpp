...
int main() {
  ...
  for (auto& ee : e) {
    ...
    if (g.pos[x] <= g.pos[y] && g.end[y] <= g.end[x]) {
      int z = g.go_up(y, g.depth[y] - g.depth[x] - 1);
      fenw.modify(g.pos[z], -1);
      fenw.modify(g.end[z] + 1, +1);
      fenw.modify(g.pos[y], +1);
      fenw.modify(g.end[y] + 1, -1);
    } else {
      fenw.modify(g.pos[x], +1);
      fenw.modify(g.end[x] + 1, -1);
      fenw.modify(g.pos[y], +1);
      fenw.modify(g.end[y] + 1, -1);
      goal += 1;
    }
  ...
  for (int i = 0; i < n; i++) {
    cout << (fenw.get(g.pos[i]) == goal ? 1 : 0);
  }
  cout << '\n';
  return 0;
}