...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n) cin>>mp[i];
	rrep(i,n)rrep(j,m){
		if(i+1<n) dr[i][j]=dr[i+1][j]+(mp[i+1][j]=='R');
		if(j+1<m) rr[i][j]=rr[i][j+1]+(mp[i][j+1]=='R');
	}
	...
	goR[0][0]=goD[0][0]=1;
	rep(i,n)rep(j,m){
		if(!(mp[i][j]=='R' && rr[i][j]>=(m-1-j))){
			int l=-1,r=j;
			while(r-l>1){
				int med=(l+r)/2;
				(rr[i][med]<=(m-1-j)?r:l)=med;
			}
			// reps(k,r,j) goD[i][j]+=goR[i][k];
			goD[i][j]+=sumR[i][j]-sumR[i][r];
		}
		if(!(mp[i][j]=='R' && dr[i][j]>=(n-1-i))){
			int l=-1,r=i;
			while(r-l>1){
				int med=(l+r)/2;
				(dr[med][j]<=(n-1-i)?r:l)=med;
			}
			// reps(k,r,i) goR[i][j]+=goD[k][j];
			goR[i][j]+=sumD[i][j]-sumD[r][j];
		}

		sumR[i][j+1]=sumR[i][j]+goR[i][j];
		sumD[i+1][j]=sumD[i][j]+goD[i][j];
	}
	// rep(i,n){rep(j,m) cout<<goR[i][j]<<",";cout<<endl;} cout<<endl;
	// rep(i,n){rep(j,m) cout<<goD[i][j]<<",";cout<<endl;} cout<<endl;
	cout<<goR[n-1][m-1]+goD[n-1][m-1]<<endl;
  return 0;
}