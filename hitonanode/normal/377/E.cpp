...
int main() {
    ...
    lint S;
    cin >> N >> S;
    vector<pair<int, int>> C2V(N);
    for (auto &p : C2V) {
        cin >> p.second >> p.first;
    }
    sort(C2V.begin(), C2V.end());

    AddMonotoneConvexHullTrick<false, lint> cht;
    cht.add_line(0, 0);

    int best_slope = 0;
    for (auto [c, v] : C2V) {
        if (!chmax(best_slope, v)) continue;
        lint x = 0;
        if (c > 0) {
            lint ok = 1;
            while (cht.get(ok) < c) ok *= 2;
            lint ng = ok / 2;
            while (ok - ng > 1) {
                const auto y = (ok + ng) / 2;
                (cht.get(y) < c ? ng : ok) = y;
            }
            x = ok;
        }
        lint amari = cht.get(x) - c;
        cht.add_line(v, amari - x * v);
    }

    lint ok = 1;
    while (cht.get(ok) < S) ok *= 2;
    lint ng = ok / 2;
    while (ok - ng > 1) {
