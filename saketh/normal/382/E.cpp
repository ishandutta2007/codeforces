...
int main() {
    ...
    FOR (N, 2, MAXN) {
        // one child only
        F0R (K, 2 * MAXN - 1) dp[N][K+1] += N * dp[N-1][K];

        // two children
        FOR (left, 1, (N - 1)/2 + 1) {
            int right = N - 1 - left;
            mn label = left != right ? N * mn::ncr(N-1, left) : N * mn::ncr(N-2, left-1);
            F0R (KL, 2 * MAXN) F0R (KR, 2 * MAXN) {
                int K = (KL/2 + KR/2) * 2;
                K += 1 + ((KL | KR)&1);
                if (K < 2 * MAXN) dp[N][K] += label * dp[left][KL] * dp[right][KR];
            }
        }
    }

    int N, K; re(N, K);
    ps((dp[N][2*K] + dp[N][2*K+1]) * mn::small_inv(N));
