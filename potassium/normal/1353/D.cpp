...
int main() {
	CASE(t) {
		...
		for (int i = n; i >= 1; i--) {
			while (!seg[i].empty()) {
				pii curSeg = *seg[i].begin();
				seg[i].erase(seg[i].begin());
				
				int mid = (curSeg.F + curSeg.S) / 2;
				ans[mid] = ++last;
				
				pii leftSeg = {curSeg.F, mid - 1};
				pii rightSeg = {mid + 1, curSeg.S};
				
				int leftLen = leftSeg.S - leftSeg.F + 1;
				if (leftLen >= 1)
					seg[leftLen].insert(leftSeg);
					
				int rightLen = rightSeg.S - rightSeg.F + 1;
				if (rightLen >= 1)
					seg[rightLen].insert(rightSeg);
			}
		}
