...
#define int ll
modint<MOD> mae[500000];
modint<MOD> gyaku[500000];
...
void solve(){
    mae[0] = 1;
    gyaku[0] = 1;
    for (int q = 1; q < 400000; ++q) {
        mae[q] = mae[q - 1] * modint<MOD>(q);
    ...
    modint<MOD> ans = 0;
    for (int q = 0; q <= cnt; ++q) {
        for (int j = q + 1; j + q <= cnt; ++j) {
            modint<MOD> hoge = 1;
            if(cnt - q - j != 0) hoge = pows<modint<MOD>>(k - 2LL, cnt - q - j);
            ans += comb(cnt, q) * comb(cnt - q, j) * hoge * base;
        }
    }
    cout << ans << endl;
}
