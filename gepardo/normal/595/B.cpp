/*-------------------( alex256 )--------------------+
|    Contest: CodeForces Round 330 (Div. 2)         |
|       Task: D                                     |
|       Date: 31. 10. 2015                          |
+--------------------------------------------------*/
#include <algorithm>
...
long cnt(int a, int b)
{
    long long q = 1;
    for (int i = 0; i < k; i++) q *= 10;
    long long v = (q - 1) / a + 1, wa = ((q / 10 * b) - 1) / a + 1, wb = (q / 10 * (b + 1) - 1) / a + 1;
    //cout << a << " " << b << " " << v << " " << wa << " " << wb << endl;
    if (b == 0) return (v - wb); else return (v + wa - wb);
}
...
main()
{
    ...
    long long res = 1;
    for (int i = 0; i < (n / k); i++)
        res = (res * cnt(a[i], b[i])) % 1000000007;
    cout << res;
    return 0;
}