...
ll getRest(int elevens_left, int doubs_left, int extra_sings){
    int used = min(elevens_left, doubs_left);
    ll res = 0;
    res+=b*used+a*used;
    elevens_left-=used;
    doubs_left-=used;
    ...
    if(doubs_left > 0){
        assert(elevens_left == 0);
        res+=a;
        return res;
    }

    if(elevens_left > 0){
        int use_sings = min(elevens_left, extra_sings);
        res+=max(ll(0), use_sings*(b-c));
    }

    return res;
}

ll solveRuns(int elevens, vi mid_doubs, int extra_doubs, int extra_sings){ //choose 0 first
    sort(all(mid_doubs));
    int tot_doubs = extra_doubs;
    for(auto u: mid_doubs){
        tot_doubs+=u;
    }
    // int tot_sings = sz(mid_doubs)+extra_sings;
    dbg(elevens, mid_doubs, extra_doubs, extra_sings);
    ll res = 0;

    ll cur_money = 0;
    // dbg(mid_doubs);
    for(int i = 0; i <= sz(mid_doubs); i++){ //how many singles do we use?
        //are we even able to use this many singles?
        if(i >= 1){
            int elevens_used = mid_doubs[i-1];
                
            if(elevens-elevens_used < 0){
                break;
            }
            cur_money+=b*(elevens_used+1);
            cur_money+=a*(mid_doubs[i-1]);
            tot_doubs-=mid_doubs[i-1];
            cur_money-=c;

            elevens-=elevens_used;
        }

        ckmax(res, cur_money+getRest(elevens, tot_doubs, extra_sings));
        // dbg(cur_money+getRest(elevens, tot_doubs, extra_sings));
    }
...
void solveTestCase(){
    ...
    ll ans = 0;
    ckmax(ans, solveRuns(elevens, mid_doubs, extra_doubs, extra_sings));
    // dbg(ans);
    if(elevens > 0){
        elevens--;
        ckmax(ans, b+solveRuns(elevens, mid_doubs, extra_doubs, extra_sings));
    }
