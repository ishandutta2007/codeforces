...
const int MX = 2e5+5;
const ll INF = (ll)(1e18)+5; // not too close to LLONG_MAX
...
template<class T> struct Seg { // comb(ID,b) = b
	...
	int walk(ll& c, ll& ans, int& next_ind, int ind, int L, int R) {
		...
		if (next_ind <= L) {
			if (seg[ind][1] <= c) {
				if (L == R) return L;
			} else if (seg[ind][0] <= c) { // just subtract all
				c -= seg[ind][0];
				ans += seg[ind][2];
				return MOD;
			} else {
...
void upd(int ind, int dif) {
	...
	cur[ind] += dif;
	F0R(p,31) recalc(p,ind);
}

int main() {
	setIO(); re(N,Q); cur.rsz(N);
	F0R(i,31) segs[i].init(SZ);
	V<AR<int,4>> cands;
	vi init(N);
	F0R(i,N) {
		ints(a,w,v);
	...
	F0R(i,N) upd(i,init[i]);
	F0R(i,N) dbg(i,val[i],wei[i],cur[i]);
	dbg("INITED");
	rep(Q) {
		ints(t);
		if (t == 1) {
