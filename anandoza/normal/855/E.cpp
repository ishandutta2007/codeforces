...
DP count_ways(int n, int b) {
    DP diff(n + 1);
    diff[0][0] = 1;

    for (int i = 0; i < n; i++) {
        diff[i + 1][1] += b * diff[i][0];
        diff[i + 1][b - 1] += b * diff[i][b];
        for (int m = 1; m < b; m++) {
            diff[i + 1][m - 1] += m * diff[i][m];
            diff[i + 1][m + 1] += (b - m) * diff[i][m];
        }
    }
...
void precalc() {
    for (int b = 2; b <= 10; b++) {
        ...
        cache[b] = count_ways(len, b);
    }
}

ll solve(int b, ll lim) {
    vector<int> digits;
    while (lim > 0) {
        digits.push_back(lim % b);
        lim /= b;
    }
    reverse(all(digits));
    ...
    // same_len = 0, num_zero = 0
    ans += cache[b][n - 1][1] * (digits[0] - 1);

    // same_len = 0, num_zero > 0
    for (int num_zero = 1; num_zero < n; num_zero++) {
        ans += cache[b][n - num_zero - 1][1] * (b - 1);
    }

    for (int same_len = 1; same_len < n; same_len++) {
        int mask = 0;
        for (int i = 0; i < same_len; i++) {
            mask ^= (1 << digits[i]);
        }

        for (int d = 0; d < digits[same_len]; d++) {
            int target = mask ^ (1 << d);
            ll ways = cache[b][n - same_len - 1][__builtin_popcount(target)];
            ans += ways;
        }
    }
