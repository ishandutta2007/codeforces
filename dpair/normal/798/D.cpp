...
inline void solve() { LL SA = 0, SB = 0;
    rep(i, 1, (n >> 1) + 1) {
        SA += a[i].a;
        SB += a[i].b;
    }
...
inline void shuffle(T *bg, T *ed){
    int len = ed - bg;
    rep(i, 1, len - 1) std :: swap(bg[i], bg[rand_() % i]);
}
}

signed main(){
    Orzlxl :: srand_(114514);
    read(n); rep(i, 1, n) a[i].id = i;
    rep(i, 1, n) read(a[i].a), sa += a[i].a; 
    rep(i, 1, n) read(a[i].b), sb += a[i].b;
    solve();
    while(1) {
        Orzlxl :: shuffle(a + 1, a + n + 1);
        solve();
    }
