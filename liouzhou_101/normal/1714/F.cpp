...
void solve()
{
	...
	auto go = [&](int x, vector<int> v, vector<int> d)
	{
		int m = v.size();
		int sum = 1;
		for (int i = 0; i < m; ++i)
			sum += d[i];
		if (sum > n) return writeln("NO");
		...
		int cur = max({ x, *max_element(v.begin(), v.end()) });
		for (int i = 0; i < m; ++i)
		{
			int last = x;
			for (int j = 1; j < d[i]; ++j)
	...
	auto ok = [&](int a, int b, int c, int dab, int dbc)
	{
		go(b, { a, c }, { dab, dbc });
	};
	...
	int dx2 = sd - d13;
	int dx3 = sd - d12;
	if (!(dx1 >= 1 && dx2 >= 1 && dx3 >= 1))
	{
		return writeln("NO");
	}
	go(4, { 1, 2, 3 }, { dx1, dx2, dx3 });
}
