// 2022-04-11, Mon, 18:48// 7:08
//23 minutes if passed
...
class DisjointUnionSets(val n: Int) {
    ...
    fun union(x: Int, y: Int):Boolean {
        ...
        if(size[xRoot] < size[yRoot]){
            ...
            root[yRoot] = maxOf(root[yRoot], root[xRoot])
        }else{
            parent[yRoot] = xRoot
            size[xRoot] += size[yRoot]
            root[xRoot] = maxOf(root[yRoot], root[xRoot])
        }
        return true
    }
...
class linkedset(ind:Int, col:Int){
    val head = IntArray(ind){-1}
    val next = IntArray(col){-1}
    val value = IntArray(col)
    var pt = -1
    fun add(i:Int, v :Int){
        pt++
        value[pt] = v
    ...
    inline fun forall(i:Int, act:(Int)->Unit){
        var h = head[i]
        while(h != -1){
            act(value[h])
            h = next[h]
...
fun main(){
    solve.cases {
        ...
        for(i in n - 1 downTo 0) {
            if(str[i] == '0') max1[i] = 0 else max1[i] = 1 + (max1.getOrNull(i + 1)  ?: 0)
            if(str[i] == '1') max0[i] = 0 else max0[i] = 1 + (max0.getOrNull(i + 1) ?: 0)
        }
        val dp = IntArray(n) { maxOf(max1[it], max0[it]) }

        val L = linkedset(n+1,n)
//        val values = List(n+1){mint}

        for((i, v) in dp.withIndex()) {
            L.add(v,i)
//            values[v].add(i)
        }
        val dsu = DisjointUnionSets(n+1)
        val retall = IntArray(n+1)
        for(k in 1..n){
            L.forall(k-1){
                dsu.delete(it)
            }
