...
public:
    void solve(istream& cin, ostream& cout) {
		...
		if (N == 4) {
			...
			for (int i = 0; i < k; ++i) {
				for (int j = i + 1; j < k; ++j) {
					A.push_back({2 * i + 1, 2 * j + 1, 2 * i + 2, 2 * j + 2});
					if (N % 2 == 0 && (j == (i + 1) || (i == 0 && j == k - 1))) {
						if (j == i+1) {
							A.push_back({2 * i + 1, 2 * j + 2, 2 * i + 2});
							A.push_back({2 * i + 1, 2 * j + 1, 2 * i + 2});
						} else {
							A.push_back({2 * j + 1, 2 * i + 2, 2 * j + 2});
							A.push_back({2 * j + 1, 2 * i + 1, 2 * j + 2});
						}
					} else {
						A.push_back({2 * i + 1, 2 * j + 1, 2 * i + 2, 2 * j + 2});
					}
		...
		cout << A.size() << endl;
		for (auto &a: A) cout << (int)a.size() << ' ' << a;
    }
};
