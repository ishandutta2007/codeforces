...
int n, m;
LL a[MN][MN];
unordered_map<LL, LL> c1[MN][MN], c2[MN][MN];
void xor_to(const unordered_map<LL, LL> &a, unordered_map<LL, LL> &b, LL val) {
	for (auto x : a)
		b[x.f ^ val] += x.s;
}
void search(int x, int y, unordered_map<LL, LL> cou[MN][MN], int del, int len) {
	cou[x][y][a[x][y]] = 1;
	FOR(d, 1, len) {
		FOR(i, 0, d) {
			int j = d - i;
			if (i < n && j < m) {
				debug(imie(x) + imie(i) * imie(del), imie(y) + imie(j) * imie(del));
				if (i)
					xor_to(cou[x + (i - 1) * del][y + j * del], cou[x + i * del][y + j * del], a[x + i * del][y + j * del]);
				if (j)
					xor_to(cou[x + i * del][y + (j - 1) * del], cou[x + i * del][y + j * del], a[x + i * del][y + j * del]);
			}
		}
	}
}
int main() {
	...
	REP(i, n)
		REP(j, m)
			scanf("%lld", &a[i][j]);
	int len = m + n - 2;
	search(0, 0, c1, 1, len / 2);
	search(n - 1, m - 1, c2, -1, (len + 1) / 2);
	LL ans = 0;
	REP(i, n)
		REP(j, m)
			if (c1[i][j].size() && c2[i][j].size()) {
				debug(arr2(c1, i, j), arr2(c2, i, j));
				for (auto x : c1[i][j])
					ans += x.s * c2[i][j][x.f ^ k ^ a[i][j]];
			}
	printf("%lld\n", ans);
}