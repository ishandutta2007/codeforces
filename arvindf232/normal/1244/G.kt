...
fun main(){
    solve.cases{
        ...
        val atleast = (1L * n * (n+1))/2
        if(k < atleast){
            put("-1")
            return@cases
        }
        var extras = k - atleast
        val ret = mint
        for(i in (n-1) downTo 1 step 2){
            if(extras >= i ) {
                ...
                val taking = minOf(extras, i.toLong()).toInt()
                if(taking > 0) {
                    ret.add(taking)
                }
                break
        ...
        val truereture = atleast + ret.sumOf { it.toLong() }
        val trueret = mint
        val all = BooleanArray(n+1){true}
//        val has = TreeSet<Int>()
//        for(i in 1..n){
//            has.add(i)
//        }
        for((i,a) in ret.withIndex()){
            trueret.add(i+1+a)
            all[i+1+a] = false
//            has.remove(i+1+a)
        }
