...
// { ==== Begin library Graph2/haveAnyCycle ====
...
// } ==== End library YComb ====
template<class T> bool haveAnyCycleDirectedGraph(Graph<T>& graph){
	char const WHITE=0, GRAY=1, BLACK=2;
	std::vector<char> color(graph.size());
	for(auto item: color) assert(item==WHITE);
	struct haveCycle {};

	auto const work=makeYComb([&](auto work, int node)->void{
		assert(color[node]==WHITE);
		color[node]=GRAY;
	...
	try{
		for(int const node: range(0, int(graph.size()))) if(color[node]==WHITE){
			work(node);
		}
	}catch(haveCycle){
		return true;
...
// } ==== End library Graph2/haveAnyCycle ====
void up();
int main(){
...
void up(){
	...
	auto const mostFrequent=int(std::max_element(begin(count), end(count))-count.begin());
	Graph<> graph(number+1);
	for(int const index: range(0, number)){
		if(a[index]==mostFrequent or b[index]==mostFrequent) continue;
		graph.addEdgeDirected(a[index], b[index]);
	}

	std::cout<<(haveAnyCycleDirectedGraph(graph) ? "WA\n": "AC\n");
...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1672/problem/F2