...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		if (odds.size() < evens.size()) {
			for (int x: range(odds.size())) {
				total.push_back(evens[x]);
				total.push_back(odds[x]);
			}
			total.push_back(evens.back());
		...
		for (int i: range<int>(1, total.size())) {
			if (abs(total[i] - total[i - 1]) != 1) {
				out << "NO\n";// << i << ' ' << a[i - 1] << ' ' << a[i];
				return;
			}
		}
