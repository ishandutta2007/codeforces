...
int main() {
    ...
    vvll basis(N);
    rep(i, N) {
        if (i > 0)basis[i] = basis[i - 1];

        ll now = a[i];
        for (ll x : basis[i]) { now = min(now, now ^ x); }
        if (now > 0) { basis[i].push_back(now); }
    }
    ...
    rep(snuke, Q) {
        ll l, x;
        cin >> l >> x;
        for (ll t : basis[l - 1]) { x = min(x, x ^ t); }
        if (x > 0) { cout << 0 << endl; }
        else {
            ll dim = l - basis[l - 1].size();
            cout << (((mint)2).pow(dim)).val() << endl;
        }
    }
