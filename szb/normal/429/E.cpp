...
const ll N=200010;
vector<ll>g[N],w[N];
ll visted[N],dfn[N],a[N],b[N],q[N],vis[N],n,top;
void dfs(ll x){
	visted[x]=1;
	rep(i,0,g[x].size()){
		if (vis[abs(w[x][i])])continue;
		vis[abs(w[x][i])]=w[x][i]<0?1:2;
		dfs(g[x][i]);
	}
}
int main(){
	...
	For(i,1,n){
		a[i]=lower_bound(q+1,q+top+1,a[i])-q,
		b[i]=lower_bound(q+1,q+top+1,b[i])-q;
		g[a[i]].push_back(b[i]);
		g[b[i]].push_back(a[i]);
		w[a[i]].push_back(i);
		w[b[i]].push_back(-i);
	...
	For(i,1,top)if (!visted[i])dfs(i);
	For(i,1,n)write(vis[i]-1),putchar(' ');
}