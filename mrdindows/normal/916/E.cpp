...
void push(int cur, int l, int r) {
    ...
    pu[dcur] += pu[cur];
    pu[dcur + 1] += pu[cur];
    tr[dcur] += (m - l + 1LL) * pu[cur];
    tr[dcur + 1] += (r - m) * pu[cur];
    pu[cur] = 0;
}
...
int h[101010];
int curT = 0;
void go(int cur, int curh = 0, int p = 0) {
    h[cur] = curh;
    par[cur][0] = p;
    tin[cur] = ++curT;
    //    DBN(cur, tin[cur]);
...
int dist(int x, int y) {
    int l = lca(x, y);
    return h[x] + h[y] - h[l] * 2;
}
int getPar(int x, int w) {
    for (int i = 0; i < 18; ++i) {
        if (checkbit(w, i)) {
            x = par[x][i];
        }
        ...
        if (t == 1) {
            ...
            if (root == v) {
                ...
                if (root != v) {
                    d = h[root] - h[v] - 1;
                    int t = getPar(root, d);
                    add(1, 1, n, tin[t], tout[t], -val);
                }
            } else {
                add(1, 1, n, tin[v], tout[v], val);
            }
        } else {
            int v = readInt();
            --v;
            ...
            if (root == v) {
                ...
                int d = h[root] - h[v] - 1;
                int t = getPar(root, d);
                ans = sum(1, 1, n, 1, n) - sum(1, 1, n, tin[t], tout[t]);
            } else {
                ans = sum(1, 1, n, tin[v], tout[v]);
            }
            writeInt(ans, '\n');
        }
    }
