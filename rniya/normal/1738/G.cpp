...
bool check(int n, int k, const vector<string>& ans) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            if (i + 1 < n and j + 1 < n)
                dp[i + 1][j + 1] = max(dp[i + 1][j + 1], dp[i][j] + (ans[i + 1][j + 1] == '1'));
        }
    }
    return dp[n - 1][n - 1] < k;
}
...
void solve() {
    ...
    for (int k = 0; k <= 2 * (m - 1); k++) {
        for (int i = 0; i < m; i++) {
            ...
            int z = 0;
            if (i - 1 >= 0) z = max(z, dp[i - 1][j] + (d(i, j) < d(i - 1, j)));
            if (j - 1 >= 0) z = max(z, dp[i][j - 1] + (d(i, j) < d(i, j - 1)));
            int x = i - min(i, j) + z, y = j - min(i, j) + z;
            for (; x < n and y < n; x++, y++, z++) {
                if (S[x][y] != '1') continue;
                ans[x][y] = '0';
                break;
            }
