...
dp calc(int u, int p) {
    ...
    if (!sz(cres)) { return {1, 1}; }

    mn dont = 1;
    trav (cr, cres) dont *= cr.doesnt;

    mn use = 1;
    trav (cr, cres) use *= cr.extends + cr.doesnt;
    use -= dont;

    mn only_one = 0;
    vmn pref = prefixes(cres, mn(1), [](mn a, dp b){ return a * b.doesnt; });
    vmn suff = suffixes(cres, mn(1), [](dp b, mn a){ return a * b.doesnt; });
    F0R (i, sz(cres)) {
        only_one += pref[i] * cres[i].extends * suff[i+1];
    }

    dp res = { use, dont + use - only_one };
