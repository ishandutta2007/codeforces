...
int main() {
	...
	FOR(i, 2, n) {
		int p = wh[i];
		invs += curr.size() - curr.order_of_key(p);
		debug(arr(wh, i), imie(invs));
		int m1 = *curr.find_by_order((curr.size() - 1) / 2);
		int m2 = *curr.find_by_order(curr.size() / 2);
		debug(imie(curr), imie(m1), imie(m2));
		if (p < m1) {
			diffs += abs(p - m1);
		}
		else if (p > m2) {
		...
		else debug(imie(p), "in range", imie(m1), imie(m2));
		curr.insert(p);
		ans[i] = diffs + invs - upto(i / 2) - upto((i - 1) / 2);
		debug(imie(diffs), imie(curr));
	}
