...
ll test,n,res,kk;
ll perm[MAXN],col[MAXN],vis[MAXN];
vector<ll>v,factor;
// Fast IO
inline ll read()
{
...
// Functions
inline vector<ll> getFact(ll num)
{
	vector<ll>factor;
	for(register int i=1;i<=sqrt(num);i++)
	{
		if(num%i==0)
		{
			factor.push_back(i);
	...
	return sort(all(factor)),factor;
}
inline ll calc(vector<ll>v)
{
	ll sz=v.size(),flg;
	factor=getFact(sz);
	for(auto fact:factor)
	{
		for(register int i=0;i<fact;i++)
		{
			flg=1;
			for(register int j=i;j<sz;j+=fact)
			{
				if(v[i]!=v[j])
				{
					flg=0;	
...
inline void solve()
{
	...
	for(register int i=1;i<=n;i++)
	{
		...
		v.clear(),v.push_back(col[i]),vis[i]=1,kk=perm[i];
		while(kk!=i)
		{
			v.push_back(col[kk]),vis[kk]=1,kk=perm[kk];
		}
		chkmin(res,calc(v));
	}
	printf("%d\n",res);
