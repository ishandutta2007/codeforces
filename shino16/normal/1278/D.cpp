...
bool sndcmp(PII a, PII b) {
  return make_pair(a._2, a._1) < make_pair(b._2, b._1);
}
...
int main() {
  ...
  union_find uf(n);
  set<PII> ri;
  ri.emplace(lr[0]._2, 0);
  int cnt = 0;
  rep(i, 1, n) {
    auto it1 = ri.lower_bound({lr[i]._1, 0});
    auto it2 = ri.lower_bound({lr[i]._2, 0});
    for (; it1 != it2; it1++) {
      if (cnt + 1 > n-1) EXIT("NO");
      int j = it1->second;
      if (uf.same(i, j)) EXIT("NO");
      uf.merge(i, j);
      debugos << "merge(" << i << ", " << j << ");\n";
      cnt++;
    }
    ri.emplace(lr[i]._2, i);
  }
  debug(cnt);
  OUT(cnt == n-1 ? "YES" : "NO");
}