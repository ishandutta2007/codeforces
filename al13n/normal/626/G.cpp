...
int main(int argc, char **argv){
  ...
  priority_queue<val> inq,deq;
  int cnt=0;
  ld sum=0;
  auto qu=[&](priority_queue<val>&q){
    while(!q.empty()){
      val v=q.top();
      if(C[v.i]==v.c && L[v.i]==v.l)
	return v;
      q.pop();
    }
    return val{-1,-1,-1,-1};
  ...
  auto upi=[&](int i){
    if(C[i])
      deq.push(val{get(i,C[i]-1)-get(i,C[i]),i,L[i],C[i]});
    if(C[i]<L[i])
      inq.push(val{get(i,C[i]+1)-get(i,C[i]),i,L[i],C[i]});
  };
  forn(i,n)
    upi(i);
  auto upc=[&](int i,int d){
    sum-=get(i,C[i]);
    C[i]+=d;
    sum+=get(i,C[i]);
    cnt+=d;
