...
void draw(int cur, int l, int r) {
    ...
    if (qx <= l && r <= qy) {
        ...
        draw(dcur, l, m);
        draw(dcur + 1, m + 1, r);
        tr[cur] = tr[dcur + 1] | tr[dcur];
    }
}
ll get(int cur, int l, int r, int x, int y) {
    if (x > r || y < l) return 0;
    if (full[cur]) return tr[cur];
    if (x <= l && r <= y) {
        return tr[cur];
    } else {
        int m = (l + r) / 2;
        int dcur = cur + cur;
        return get(dcur, l, m, x, y) | get(dcur + 1, m + 1, r, x, y);
    }
}
vector<int> v[401010];
int tout[401001];
...
int main() {
    ...
    FI(m) {
        ...
        if (t == 1) {
            ...
            v = read_int();
            --v;
            ll res = get(1, 0, N - 1, tin[v], tout[v]);
            write_int(bits_count(res));
        }
    }
    cout.write(outbuf, outp - outbuf);
