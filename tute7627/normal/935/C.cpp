...
int main(){
  ...
  Circle c(p1,r);
  lfs;
  if(distance(p1,p2)>r-EPS){
    cout<<p1 spa r<<endl;
  }
  else if(distance(p1,p2)<EPS){
    cout<<p1.real()+r/2 spa p1.imag() spa r/2<<endl;
  }
  else{
    auto p=crosspoint(c,Line(p1,p2));
    if(distance(p.fi,p2)<distance(p.se,p2))swap(p.fi,p.se);
    auto o=(p.fi+p2)*0.5;
    cout<<o spa distance(o,p2)<<endl;
  }
  return 0;
}