...
int main(){
	...
	fl.init(26+n+(n/2)*26+2);
	for(i=0;i<26;i++)
	{
		fl.addedge(fl.s,i,cnt[i],0);
		for(j=0;j<n/2;j++)
		{
			fl.addedge(i,n+26+j*26+i,1,0);
		}
	}
	for(i=0;i<n;i++)
	{
		fl.addedge(26+i,fl.t,1,0);
		for(j=0;j<26;j++)
		{
			fl.addedge(n+26+(i<n/2?i:n-i-1)*26+j,i+26,1,((s[i]-'a')==j)?-b[i]:0);
		}
	}
	cout<<-fl.min_cost_max_flow()<<endl;
	return 0;
}