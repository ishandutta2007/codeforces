// Same as the previous submission, but with fast IO.
...
#pragma GCC diagnostic ignored "-Wmisleading-indentation" // does not work?...
...
decltype(auto) get_(U&& x){
	return *std::get_if<T>(&std::forward<U>(x));
};

struct DSU{
	struct link{ int node;bool diff; };
	struct root{ int samecompsize,diffcompsize; int rank; };
	using item=std::variant<link,root>;
	std::vector<item> d;

	int minvalue;

	DSU(int n): d(n+1,root{1,0,0}),minvalue(0) {
		int const zeronode=(int)d.size()-1;
		d[zeronode]=root{0,0,0};
	}

	link findroot(int x){
		if(std::holds_alternative<root>(d[x])){ return {x,false}; }
		auto [parx,x_diff_parx]=get_<link>(d[x]);
		auto [rootx,parx_diff_rootx]=findroot(parx);
		return get_<link>(d[x])={rootx,x_diff_parx xor parx_diff_rootx};
	}

	int value(int rooti){
		int const zeronode=(int)d.size()-1;
		assert(std::holds_alternative<root>(d[rooti]));

		auto [root_zn,zn_diff_rootzn]=findroot(zeronode);
		auto const [samecompsize,diffcompsize,_]=get_<root>(d[rooti]);
		if(root_zn==rooti){ // this set is forced on some side
			return zn_diff_rootzn ? samecompsize : diffcompsize;
		}else{
			return std::min(samecompsize,diffcompsize);
		}
	}

	void merge(int x,int y,bool diff=false){
		auto [rootx,x_diff_rootx]=findroot(x);
		auto [rooty,y_diff_rooty]=findroot(y);
		if(rootx==rooty){
			assert((x_diff_rootx xor y_diff_rooty)==diff);
			return;
		}

		minvalue-=value(rootx);
		minvalue-=value(rooty);
		auto const rootx_diff_rooty=diff xor x_diff_rootx xor y_diff_rooty;

		if(get_<root>(d[rootx]).rank < get_<root>(d[rooty]).rank)
			std::swap(rootx,rooty);
		else if(get_<root>(d[rootx]).rank == get_<root>(d[rooty]).rank)
			++get_<root>(d[rootx]).rank;


		if(rootx_diff_rooty){
			get_<root>(d[rootx]).samecompsize+=get_<root>(d[rooty]).diffcompsize;
			get_<root>(d[rootx]).diffcompsize+=get_<root>(d[rooty]).samecompsize;
		}else{
			get_<root>(d[rootx]).samecompsize+=get_<root>(d[rooty]).samecompsize;
			get_<root>(d[rootx]).diffcompsize+=get_<root>(d[rooty]).diffcompsize;
		}
		d[rooty]=link{rootx,rootx_diff_rooty};
		minvalue+=value(rootx);

		assert([&]{
			int total=0;
			for(int i=0;i<(int)d.size();++i)
				if(std::holds_alternative<root>(d[i]))
					total+=value(i);
			assert(total==minvalue);
			return 1;
		}());
	}
};
...
int main(){
	...
	int n,nset;io::read(n, nset);
	std::string s;s.reserve(n);io::read(s);
	for(char& c:s)c=char(c-'0');

	std::vector<std::array<int,2>> sets(n,{-1,-1}); // [i] = sets that contain lamp i
	for(int setidx=0;setidx<nset;++setidx){
		int ssize;io::read(ssize);
		while(ssize--){
			int i;io::read(i);--i;
			*std::find(begin(sets[i]),end(sets[i]),-1)=setidx;
		}
	}

	// NOTE [nset] is force off
	DSU dd(nset);

	for(int lampi=0;lampi<n;++lampi){
		if(sets[lampi][0]<0){
			assert(s[lampi]);
		}
		else if(sets[lampi][1]<0){
			auto const setidx=sets[lampi][0];
			if(s[lampi]) dd.merge(setidx,nset);
			else dd.merge(setidx,nset,true);
		}
		else{
			auto [i,j]=sets[lampi];
			if(s[lampi]) dd.merge(i,j);
			else dd.merge(i,j,true);
		}

		io::write(dd.minvalue,'\n');
	}
}