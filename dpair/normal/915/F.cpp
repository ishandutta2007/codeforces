...
signed main(){
    read(n); read(a + 1, a + n + 1); rep(i, 1, n) p[i] = i;
    std::sort(p + 1, p + n + 1, [](const int x, const int y) {return a[x] < a[y];});
    rep(i, 2, n) link(read(), read());


    rep(k, 1, n) { const int x = p[k]; fa[x] = x; sz[x] = 1;
        for (int i = head[x];i;i = nxt[i]) {
            if(sz[e[i]]) { 
                ans += 1ll * sz[x] * sz[find(e[i])] * a[x];
                sz[x] += sz[find(e[i])]; fa[find(e[i])] = x; 
            }
        }
    } memset(fa, 0, sizeof(fa)); memset(sz, 0, sizeof(sz));
    per(k, n, 1) { const int x = p[k]; fa[x] = x; sz[x] = 1;
        for (int i = head[x];i;i = nxt[i]) {
            if(sz[e[i]]) {
                ans -= 1ll * sz[x] * sz[find(e[i])] * a[x];
                sz[x] += sz[find(e[i])]; fa[find(e[i])] = x; 
