...
signed main(){
    ...
    if(n < m){
        rep(i,n){
            int mi = accumulate(all(a[i]),INT_MAX,[](int x,int y){return min(x,y);});
            rep(j,m)a[i][j] -= mi;
            ans.eb(i+1,mi);
        }
        rep(j,m){
            int mi = INT_MAX;
            rep(i,n) chmin(mi,a[i][j]);
            rep(i,n) a[i][j] -= mi;
            ans.eb(-(j+1),mi);
        }
    }
    ...
    bool flag = false;
    rep(i,n)rep(j,m) if(a[i][j]) flag = true;
    if(flag) cout<<-1<<endl;
    else{
        cout << accumulate(all(ans),0,[](int x,pii y){return x + y.se;}) << endl;
        for(auto [x,y]:ans){
            if(x > 0){
                rep(i,y)
                cout << "row " << x << endl;
