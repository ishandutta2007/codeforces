...
fun main(){
    solve.cases{
        ...
        var ret = plarge
        for(last in -1..n-1){
            val already = Q.sumQuery(0,last)
            val needops = (last+1) - already
            if(needops > k) break
            while(options.isNotEmpty()){
                if(options.st.firstKey() + ops <= k){
                    val opt = options.st.firstKey()
                    options.removeone(opt)
                    remainsize --
                    ops += opt
                    taken.addone(opt)
