...
// RMQ
SegmentTree<int> seg(N,[](int x,int y){return min(x,y);},INT_MAX);
signed main(){
    ...
    rep(i,m){
        ...
        if(!a[i]) ans[i] = tmp;
            ...
            int ok = id[a[i]-1].size(),ng = -1;
            while(abs(ok-ng) > 1){
                int mid = (ok + ng) >> 1;
                if(pre[id[a[i]-1][mid]] > tmp) ok = mid;
                else ng = mid;
            }
            if(ok == id[a[i]-1].size()) tmp = m;
            else tmp = id[a[i]-1][ok];
            ans[i] = tmp;
        }
    }
    
    rep(i,m)seg.set(i,ans[i]);
    seg.build();

    while(q--){
        int l = in()-1 , r = in();
        cout << (seg.query(l,r) < r) ;
    }
    cout << endl;
}