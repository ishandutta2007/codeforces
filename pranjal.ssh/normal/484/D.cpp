...
int main() 
{
	...
	FOR (i, 2, n) {
		inc[i] = _dec[i] = i;
		if (a[i] > a[i - 1]) _dec[i] = _dec[i - 1];
		else if(a[i] < a[i - 1]) inc[i] = inc[i - 1];
	}

	FOR (i, 1, n) {
		dp[i] = dp[i - 1];
		if (_dec[i] < i)
			dp[i] = max({dp[i], dp[_dec[i]] + abs(a[i] - a[_dec[i] + 1]), dp[_dec[i] - 1] + abs(a[i] - a[_dec[i]])});
		if (inc[i] < i)
			dp[i] = max({dp[i], dp[inc[i]] + abs(a[i] - a[inc[i] + 1]), dp[inc[i] - 1] + abs(a[i] - a[inc[i]])});
	}
	cout << dp[n];
