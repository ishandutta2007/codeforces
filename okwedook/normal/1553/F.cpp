...
signed main() {
    ...
    int n;
    read(n);
    Fenwick<ll> used(N), usedcnt(N), preadd(N), preaddx(N);
    vector<int> a(n);
    read(a);
    ll ans = 0;
    for (auto i : a) {
        ans += i * usedcnt.get(i + 1, N - 2);
        ans += used.get(0, i - 1);
        ans += preaddx.get(i) * i - preadd.get(i);
        dbg(preaddx.get(i));
        // dbg(sum.get(i, i + 1).sum);
        for (int j = i; j < N; j += i) {
            int l = j, r = min(N - 2, j + i - 1);
            ans += used.get(l, r) - l * usedcnt.get(l, r);
            // dbg(l, r);
            preaddx.add(l, 1);
            preaddx.add(r + 1, -1);
            preadd.add(l, l);
