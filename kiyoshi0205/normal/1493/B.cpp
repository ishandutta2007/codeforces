...
constexpr int mir[10]={0,1,5,-1,-1,2,-1,-1,8,-1};
int N,M;
bool check(int a,int b){
  if(mir[a%10]==-1||mir[a/10]==-1||mir[b%10]==-1||mir[b/10]==-1)return false;
  int x=mir[b%10]*10+mir[b/10],y=mir[a%10]*10+mir[a/10];
  return x<N&&y<M;
}
int main(){
  // startupcpp();
  int codeforces;scanf("%d",&codeforces);while(codeforces--){
  int i,j;
  scanf("%d %d\n%d:%d",&N,&M,&i,&j);
  while(!check(i,j)){
    ++j;
