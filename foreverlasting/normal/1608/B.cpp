...
namespace MAIN{
    ...
    inline void MAIN(){
        ...
        if(abs(a-b)>1||a+b>n-2)puts("-1");
            ...
            res now=3;
            for(res i=1;i<=a;i++)x[now]=i,now+=2;
            now=2;
            for(res i=n;i>=n-b+1;i--)x[now]=i,now+=2;
            x[1]=a+1;
            if(a==b){
                for(res i=a+b+2;i<=n;i++)x[i]=i-b;
            }
            else if(a>b){
                now=2;
                for(res i=1;i<=a;i++)x[now]=i,now+=2;
                ...
                for(res i=a+b+2;i<=n;i++)x[i]=n-i+a+2;
            }
            for(res i=1;i<=n;i++)printf("%d ",x[i]);
            puts("");
        }
    }
}
