...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            // cout<<i<<'\n';
            // for(auto it:mp[i]) cout<<it.F<<' '<<it.S<<'\n';
            auto it=mp[i].begin();
            ll k=0;
            for(ll j=str_x%p<=i?str_x+i-str_x%p:str_x+i-str_x%p+p;j<str_x+100005;j+=p)
            {
                while(it!=mp[i].end()&&(*it).F<=j)
                {
                    k+=(*it).S;
                    it++;
                }
                if(k>0) f[j-str_x]=false;
            }
        }
        // cout<<"lmao"<<'\n';
        // for(auto it:mpp) cout<<it.F<<' '<<it.S<<'\n';
        auto it=mpp.begin();
        ll k=0;
        for(ll j=str_x;j<str_x+100005;j++)
        {
            while(it!=mpp.end()&&(*it).F<=j)
            {
                k+=(*it).S;
                it++;
            }
