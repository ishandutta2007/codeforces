...
vi g[maxn];
vi res;
int n,t,m,par[maxn],dep[maxn],tot[maxn],top[maxn];
bool vis[maxn],track[maxn],ok[maxn];

void dfs(int u,bool& ck){
    vis[u]=1;
    track[u]=1;
    for (auto c:g[u]){
        if (vis[c]){
...
void dfs2(int u){
    ...
    for (auto c:g[u]){
        ...
        dfs2(c);
        tot[u]+=tot[c];
        if (top[c]&&dep[top[c]]<dep[u]&&(!top[u]||dep[top[c]]<dep[top[u]])) top[u]=top[c];
    }
}

void dfs3(int u){
    vis[u]=1;
    if (top[u]&&tot[u]==1&&ok[top[u]]) ok[u]=1;
    for (auto c:g[u]){
        if (vis[c]) continue;
        dfs3(c);
    }
...
bool solve(int u){
    rep1(i,n){
        vis[i]=0, track[i]=0, par[i]=0, dep[i]=0;
    }
    bool ck=1;
    dfs(u,ck);
    rep1(i,n) ck&=vis[i];
            ...
            if (solve(x)){
                rep1(j,n){
                    for (auto c:g[j]){
                        if (dep[c]<dep[j]){
                            tot[c]--, tot[j]++;
                            if (!top[j]||dep[top[j]]>dep[c]) top[j]=c;
                        }
                    }
                }
                rep1(j,n) vis[j]=0;
                dfs2(x);
                rep1(j,n) vis[j]=0, ok[j]=0;
                ok[x]=1;
                dfs3(x);
                rep1(j,n){
                    if (ok[j]) res.pb(j);
