...
private:
	...
	inline void calcMatching(){ if(M.size()==N){return;}calcMatchingSize();M=vector<int>(N,-1);for(int i=0;i<A;++i) { for(pii ed:E[i]) { if(G.Residual[ed.y]==0){ M[i]=ed.x;M[ed.x]=i; }}}}
