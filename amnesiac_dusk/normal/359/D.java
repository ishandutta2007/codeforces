...
public class Main
{
	...
	void build(int[]a)
	{
		...
		table=new int[1+lgn][n];
		for(int i=0; i<n; i++)
			table[0][i]=a[i];
		for(int j=1; j<=lgn; j++)
			for(int i=0; i+(1<<(j-1))<n; i++)
				table[j][i]=ho.combine(table[j-1][i], table[j-1][i+(1<<(j-1))]);
	}
	int query(int l, int r)
	{
		...
		k--;
		return ho.combine(table[k][l],table[k][r+1-(1<<k)]);
	}
	}
	...
	public static void main(String[] args)
	{
		int n=ni();
		int[]a=nia(n);
		sparse gcd=new sparse(a, new Combiner()
				{
		...
		for(int i=0; i<n; i++)
		{
			int low=i,high=n-1,mid=(low+high+1)/2;
			while(low<high)
			{
				if(gcd.query(i,mid)!=a[i])
					high=mid-1;
				else
					low=mid;
				mid=(low+high+1)/2;
