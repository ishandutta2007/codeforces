...
struct dr{
    int x, y;
    int id;
    bool operator<(const dr& d) const {
        ll comp = x * 1ll * d.y - y * 1ll * d.x;
        if (comp != 0) {
            return comp < 0;
        }
        else {
...
void del(int x) {
    for (int to : v[x]) {
        if (bad[to]) continue;
        s.erase({fr[to], (int) v[to].size(), to});
        fr[to]--;
        s.insert({fr[to], (int) v[to].size(), to});
    }
}
dr best_ans;
int sx[1000010];
int sy[1000100];
int delted[100010]; int ds = 0;
vector<int> w;
int main()
{
...
//    DBN(w);
    ...
    for (int i = 0; i < p; ++i) {
//        DBN(i);
//        DBN(best_ans.x, best_ans.y, best_ans.id);
//        for (auto el : s) {
//            cout << el.x << ", " << el.y << ", " << el.id << endl;
//        }
        int dx = s.begin()->x;
        int dy = s.begin()->y;
        delted[i] = s.begin()->id;
