...
int main() {
    ...
    auto calc = [&](const vector<int>& v) -> vector<int> {
        BinaryIndexedTree<int> BIT(n);
        for (int i = 0; i < n; i++) BIT.add(i, 1);
        vector<int> res;

        for (int i = 0; i < n - 1; i++) {
            res.emplace_back(BIT.query(0, v[i]));
            BIT.add(v[i], -1);
        }
        return res;
    };
    ...
    auto add = [&](const vector<int>& a, const vector<int>& b) -> vector<int> {
        ...
        for (int i = n - 2, j = 2; i >= 0; i--, j++) {
            if (i - 1 >= 0) res[i - 1] += (res[i] + a[i] + b[i]) / j;
            res[i] = (res[i] + a[i] + b[i]) % j;
        }
        return res;
    };

    auto restore = [&](const vector<int>& v) {
        vector<int> res, used(n, false);
        BinaryIndexedTree<int> BIT(n);
        for (int i = 0; i < n; i++) BIT.add(i, 1);

        for (int i = 0; i < n - 1; i++) {
            int nxt = BIT.lower_bound(v[i] + 1);
    ...
    vector<int> ans = restore(add(calc(p), calc(q)));
    cout << ans << '\n';
    return 0;
}