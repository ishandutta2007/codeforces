...
template<class T> struct Seg { // comb(ID,b) = b
    ...
    void add(int p, T val) { // set val at position p
    ...
    while (q--) {
        ...
        if (type=="Color") {
            ...
            if (it->F > l) {
                ...
                if (it->S!=c) {
                    add.pb(mp(l, c));
                    seg.add(l, min(r, tmp), cval[it->S] - cval[c]);
                    if (tmp>r) add.pb(mp(r, it->S));
                }
                it++;
            }
            if (it->F == l) add.pb(mp(l, c));
            while (it->F < r) {
                rem.pb(*it);
                int curl = it->F, curc = it->S;
                it++;
                seg.add(curl, min(it->F, r), cval[curc] - cval[c]);
                if (it->F > r) add.pb(mp(r, curc));
            }
            for (pi p : rem) s.erase(p);
            for (pi p : add) s.insert(p);
        } else if (type=="Add") {
            int c; ll x; cin >> c >> x; c--;
            cval[c] += x;
        } else if (type=="Query") {
            int i; cin >> i; i--;
            auto it = s.ub(mp(i, n)); it--;
            cout << seg.get(i) + cval[it->S] << nl;
        }
        debug(s);
    }
}
