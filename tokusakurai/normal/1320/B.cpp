...
struct Weighted_Graph{
    ...
    Weighted_Graph(int n) : INF_T(numeric_limits<T>::max()), n(n){
        es.resize(n), rs.resize(n), d.resize(n), pre_v.resize(n);
    }

    void add_edge(int from, int to, T cost, bool directed = false){
        es[from].eb(to, cost), rs[to].eb(from, cost);
...
int main(){
    ...
    G.dijkstra(p.back());
    rep(i, K-1){
        if(G.d[p[i]] < G.d[p[i+1]]+1) m++, M++;
        else{
            bool flag = false;
            each(e, G.rs[p[i]]){
                if(e.to == p[i+1]) continue;
                if(G.d[e.to]+1 == G.d[p[i]]) flag = true;
            }
            if(flag) M++;
        }
    }
