...
fun main(){
    solve.cases{
        val n = getint
        val m = getint
        val bad = mutableListOf<Pair<Int,Int>>()
        repeat(m){
            bad.add(Pair(getint - 1 , getint - 1 ))
        }
        ...
        if(dsu.components != 1){
            val comps = Array(n){mint}
            for(i in 0 until n){
                comps[dsu.find(i)].add(i)
            }
            val ret1 = IntArray(n)
            val ret2 = IntArray(n)
            val cc = comps.filter { it.isNotEmpty() }
            var now1 = 0
            var now2 = 0
            for(c in cc){
                for(v in c){
            ...
            for(i in 1..cc.size){
                val reali = if(i == cc.size) 0 else i
                for(v in cc[reali]){
                    ret2[v] = now2
                    now2 ++
                }
            }
            put(ret1.map { it + 1 }.conca())
            put(ret2.map { it +1}.conca())
            return@cases
        }else{
            if(m.toLong() == (n.toLong() * (n-1))/2){
                val that = (1..n).toList().conca()
                put(that)
                put(that)
                return@cases
            }else{
                val sack = bad.map { if(it.first > it.second) makepair(it.second, it.first) else makepair(it.first,it.second) }.toSet()
                ...
                var now0 = 0
                val ret = IntArray(n){-1}
                val useful = BooleanArray(n){degree[it] != n-1}
                for(i in 0 until n){
                    if(!useful[i]){
                        ret[i] = now0
                        now0 ++
                ...
                fun isedge(a:Int, b:Int):Boolean{
                    val aa = minOf(a,b)
                    val bb = maxOf(a,b)
                    return makepair(aa,bb) !in sack
                }
                val stardecomposition = mutableListOf<List<Int>>()
                val H = Graph(n+1,n,false)
                val root = n
                val st = TreeSet<Int>()
                fun go(v:Int){
                    var now = -1
                    while(true){
                        val next = st.higher(now)
                        if(next == null) break
                        if(isedge(v,next)){
                            H.add(v,next)
                            st.remove(next)
                ...
                H.root = root
                H.treeOrderDFS()
                val depth = H.calculateDepth()
                val stuff = (0 until n).sortedByDescending { depth[it] }
                val taken = BooleanArray(n)
                val subdegree = IntArray(n)
                for(i in 0 until n){
                    if(depth[i] == 1 && useful[i]){
                        H.subs(i){
                            subdegree[i] ++
                        }
                    }
                ...
                for(v in stuff){
                    ...
                    if(depth[p] == 2){
                        val pp = H.parent[p]
                        subdegree[pp]--
                        if(subdegree[pp]== 0 && !taken[pp]){
                            new.add(pp)
                            taken[pp] = true
                        }
                    }
                    stardecomposition.add(new)
                }
                val ret1 = ret.copyOf()
                val ret2 = ret.copyOf()
                var now1 = now0
                var now2 = now0
                for(s in stardecomposition){
                    assert(s.size > 1)
                    val t = s[0]
                    ret1[t] = now1
                    now1 ++
...
//                var antiedge = Pair(-1,-1)
//                for(i in 0 until n){
//                    for(j in (i+1) until n){
//                        if(makepair(i,j) !in sack){
//                            antiedge =Pair(i,j)
//                             break
//                        }
//                    }
//                }
//                assert(antiedge != Pair(-1,-1))
...
evrything that is complete cannot move

decompose into star groups

star decomposition is ok
