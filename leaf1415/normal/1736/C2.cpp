...
ll n, Q;
ll a[200005];
ll r[200005], r2[200005];
ll rsum[200005], rsum2[200005];
...
int main(void)
{
	...
	ll ans = 0;
	rep(i, 1, n){
		r[i] = seg.right_find(i, -i);
		r2[i] = seg.right_find(r[i]+1, -i);
		ans += r[i]-i;
	}
	rep(i, 1, n) rsum[i] = rsum[i-1] + r[i], rsum2[i] = rsum2[i-1] + r2[i];
	...
	rep(i, 1, Q){
		...
		ll tmp = ans;
		if(a[p] > x){
			ll lb = max(1LL, -a[p]), ub = min(n, -x-1);
			chmax(lb, (ll)(lower_bound(r+1, r+n+1, p) - r));
			if(lb <= ub) tmp -= rsum[ub]-rsum[lb-1] - p*(ub-lb+1);
		}
		else if(a[p] < x){
			ll lb = max(1LL, -x), ub = min(n, -a[p]-1);
			chmax(lb, (ll)(lower_bound(r+1, r+n+1, p) - r));
			if(lb <= ub) tmp += rsum2[ub]-rsum2[lb-1] - p*(ub-lb+1);
		}
		cout << tmp << "\n";
	}
