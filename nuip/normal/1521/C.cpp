...
int maxmin(const int i,const int j,const int x){
	cout<<"? 1 "<<i+1<<" "<<j+1<<" "<<x+1<<endl; cout.flush();
	// return max(min(x,p[i]),min(x+1,p[j]));
	return read(1);
}
int minmax(const int i,const int j,const int x){
	cout<<"? 2 "<<i+1<<" "<<j+1<<" "<<x+1<<endl; cout.flush();
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		cauto n=read();
		// cauto n=10;
		// p.resize(n); iota(all(p),0); random_shuffle(all(p));
		int mni=-1;
		rep(i,n){
			if(i%2) continue;
			cauto j=(i+1<n?i+1:i-1);
			if(cauto t=minmax(i,j,0);t<=1){
				if(t==0){
					mni=i;
					break;
				}
		...
		out(mni,p[mni],1);
		vector<int> re(n);
		rep(i,n)if(i!=mni) re[i]=maxmin(mni,i,n-2);
		// assert(re==p);
		cout<<"!";
