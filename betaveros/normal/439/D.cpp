...
struct Tag {
	...
	Tag(int v, bool aa): val(v), a(aa) {}
};
vector<Tag> tags;
void tc() {
	readInput();
	fori (i, 0, n) tags.push_back(Tag(a[i], true));
	fori (i, 0, m) tags.push_back(Tag(b[i], false));
	sort(allof(tags));
	ll cost = 0;
	int val0 = tags[0].val;
	fori (i, 0, m) cost += b[i] - val0;
	ll mincost = cost;
	int la = 0;
	int rb = m;
	for (Tag t : tags) {
		cost += (ll) (la - rb) * (ll) (t.val - val0);
		debugf("%" PRId64 " <- %d\n", cost, t.val);
		val0 = t.val;
		minify(mincost, cost);
		if (t.a) la++; else rb--;
