...
const int maxsize=250+5;

int sizeX,sizeY;
char A[maxsize][maxsize];
bool visited[maxsize][maxsize];
int n,X[maxsize*maxsize],Y[maxsize*maxsize];
...
void DFS(int x,int y)
{
	...
	X[n]=x;
	Y[n++]=y;
	for (int dx=-1;dx<=1;dx++) for (int dy=-1;dy<=1;dy++) DFS(x+dx,y+dy);
}
bool checkS()
{
...
bool checkD()
{
	int mins=X[0]+Y[0],maxs=X[0]+Y[0],mind=X[0]-Y[0],maxd=X[0]-Y[0];
	for (int i=0;i<n;i++)
	{
		int s=X[i]+Y[i];
		int d=X[i]-Y[i];
	...
	for (int i=0;i<n;i++)
	{
		int s=X[i]+Y[i];
		int d=X[i]-Y[i];
		if (s==mins || s==maxs || d==mind || d==maxd)
			C++;
	}
	return C==n && C==(D/2)*4;
}
int main()
{
	...
	for (scanf("%d",&testcase);testcase>0;testcase--)
	{
		...
		for (int i=0;i<sizeX;i++) for (int j=0;j<sizeY;j++) visited[i][j]=false;
		int R=0;
		for (int i=0;i<sizeX;i++) for (int j=0;j<sizeY;j++) if (!visited[i][j] && A[i][j]=='1')
		{
			n=0;
			DFS(i,j);
			if (checkS() || checkD()) R++;
