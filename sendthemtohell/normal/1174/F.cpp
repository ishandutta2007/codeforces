...
int cnt[N];
int pre[N];
pair<int,int> l_son[N];

void dfs_make(int u, int p) {
	pre[u] = p;
	int pos = -1;
...
int dfs_ch(int u, int h, int cur_h) {
	if (h == cur_h) return 1;
	int ret = 0;
	for (auto v : g[u]) {
		ret += dfs_ch(v, h, cur_h + 1);
	}
	return ret;
}

int dfs_calc(int u, int h, int cur_h) {
	cnt[u] = 0;
	ok[u] = false;
	l_son[u] = { 0,0 };
	if (h == cur_h) {
...
inline void solve() {
	...
	int cur_root = 1;
	dfs_calc(cur_root, h, 0);
	assert(cnt[1] > 0);
	int num = 0;
	while (cnt[cur_root] > 1) {
		++num;
		while (l_son[cur_root].x == cnt[cur_root]) {
			--h;
			cur_root = l_son[cur_root].y;
		}
		int hh = h;
		int cur = cur_root;
		while (l_son[cur].x > cnt[cur_root] / 2) {
			--hh;
			cur = l_son[cur].y;
		}
		if (cur != cur_root) {
			...
			if (ans == hh) {
				...
				int cur_val = cnt[cur_root];
				assert(dfs_ch(cur_root, h, 0) == cur_val);
				int pred = pre[cur];
				assert(pred != -1);
				int i;
				for (i = 0; i < sz(g[pred]); ++i) {
					if (g[pred][i] == cur) break;
				}
				assert(i < sz(g[pred]));
				g[pred].erase(g[pred].begin() + i);

				dfs_calc(cur_root, h, 0);
				//assert(cnt[cur_root] == cur_val - cnt[cur]);
			}
		}
		else {
			//cout << "s " << cur_root << endl;
			//cout.flush();
			int ans = ask_s(cur);
			//cin >> ans;
			assert(ans != -1);
