...
void solve(){
    ...
    for(int k=2;k<=K;++k){
        for(int i=1;i<=n;++i){
            ...
            if (i<k) continue;
            int prf=0;
            // sum po2[(i-j)*j]*dp[j][k-1]
            for(int st=1;st<=i;++st){
                if (st!=0){
                    int j=st-1;
                    prf=(prf+po2[(i-j)*j]*dp[j][k-1])%mod;
                }
                dp[i][k]+=(prf*po2[i-st])%mod;
                if (dp[i][k]>=mod) dp[i][k]-=mod;
                // for(int j=1;j<st;++j){
                //     dp[i][k]+=(po2[(i-j)*j+i-st]*dp[j][k-1])%mod;
                //     if (dp[i][k]>=mod) dp[i][k]-=mod;
                // }
            }
        }
