...
int solve() {
    ...
    REP(i, N) {
        auto pd = sieve.factorize(A[i]);
        for (auto [p, deg] : pd) {
            mp[p].emplace_back(i, deg);
        }
    }
    int ret = 0;
    for (auto [p, id] : mp) {
        int dmax = 0, dmin = 1 << 29;
        for (auto [i, d] : id) {
            chmax(dmax, d);
            chmin(dmin, d);
        }
        ...
        if (dmin == 0) {
            vector<int> is;
            for (auto [i, d] : id) if (d > dmin) is.push_back(i);
            for (auto [i, d] : id) if (d > dmin) is.push_back(i + N);
            int last = -2, suc = 0;
            for (auto i : is) {
                if (i > last + 1) {
                    suc = 1;
            ...
            vector<int> dmins(N * 2);
            for (auto [i, d] : id) dmins[i] = dmins[i + N] = d;
            int last_min = N * 2;
            REP(i, dmins.size()) {
                if (dmins[i] == dmin) last_min = i;
                else chmax(ret, i - last_min);
            }
        }
    }
    return ret;
