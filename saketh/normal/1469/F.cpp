...
int main() {
    ...
    int64_t white = 1;
    trav (l, len)
        white += max(0, l - 2); // actually l >= 3 anyway
    if (white < K) {
        ps(-1);
        return 0;
    }
    ...
    while (hi - lo > 1) {
        ...
        trav (l, len) {
            ...
            while (attach < sz(ct) && !ct[attach])
                attach++;
            if (attach == sz(ct) || !ct[attach])
                break;

            ct[attach]--;
            --avail;

            add_range(attach + 2, attach + 2 + (l - 1) / 2);
            add_range(attach + 2, attach + 2 + (l - 1 + 1) / 2);
        }
