...
inline void input(V&v1,V&v2){
	...
	if(!b[n][U/2]){FL=0;return;}
	for(i=n,j=U/2;i;--i)if(0<=j-a[i] && j-a[i]<U && b[i-1][j-a[i]])j-=a[i],v1.push_back(a[i]);
		else j+=a[i],v2.push_back(a[i]);
	if(v1.size()>v2.size())swap(v1,v2);
}
int main(){
	srand(114514);
	for(scanf("%d",&T);T--;){
		FL=1;input(v1,v2);input(v3,v4);
