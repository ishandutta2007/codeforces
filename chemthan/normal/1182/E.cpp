...
void chemthan() {
    long long n; int f1, f2, f3, c; cin >> n >> f1 >> f2 >> f3 >> c;
    auto func = [&] (int x) {
        static int magic = 5e4 + 5;
        map<int, int> hs;
        int t = 1;
        FOR(i, 0, magic) {
            hs[t] = i;
    ...
    f1 = func(f1), f2 = func(f2), f3 = func(f3), c = func(c);
    int f[] = {f1, f2, f3};
    Matrix<3, (int) 1e9 + 6> mat;
    FOR(i, 0, 3) {
        mat[0][i] = 1;
    }
    mat[1][0] = mat[2][1] = 1;
    ...
    int res = 0;
    FOR(i, 0, 3) {
        addmod(res, mult(f[3 - i - 1] + mult(c, 3 - i, MOD - 1), mat[0][i], MOD - 1), MOD - 1);
    }
    submod(res, mult(c, n % (MOD - 1), MOD - 1), MOD - 1);
    cout << fpow(5, res) << "\n";
}
