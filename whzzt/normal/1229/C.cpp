...
const int N=1e5+5;
int n,m,q,cnt[N],deg[N];
i64 sum;
vector<int> adj[N],ic[N];
int main(){
	...
	for(int u=1;u<=n;++u) for(int v:adj[u]) if(u>v) ++cnt[u],ic[v].pb(u);
	for(int u=1;u<=n;++u) sum+=(i64)(deg[u]-cnt[u])*cnt[u];
	print(sum),putc('\n');
	for(scan(q);q;--q){
		int x;
		scan(x);
