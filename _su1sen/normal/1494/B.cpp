...
int main() {
    ...
    loop(t) {
        ...
        rep(s, 1 << 4) {
            ...
            u2 -= kth_bit(s, 0) + kth_bit(s, 1);
            r2 -= kth_bit(s, 1) + kth_bit(s, 2);
            d2 -= kth_bit(s, 2) + kth_bit(s, 3);
            l2 -= kth_bit(s, 3) + kth_bit(s, 0);
            auto ok = [&](int x) { return 0 <= x and x <= n - 2; };
            ans |= ok(u2) and ok(r2) and ok(d2) and ok(l2);
        }
        print(ans ? "YES" : "NO");
    }
    return 0;
