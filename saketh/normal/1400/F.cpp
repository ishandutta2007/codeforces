...
int main() {
    ...
    trie<'0', 10> tr(all(xprimes));

    const int T = sz(tr.data);
    vi dp(T, INT_MAX), DP = dp;
    dp[0] = 0;
    ...
    trav (c, s) {
        ...
        F0R (loc, T) {
            ...
            ckmin(DP[loc], dp[loc] + 1);

            int nloc = tr.child_link(loc, c);
            if (tr.data[nloc].dict_index != -1)
                continue;

            ckmin(DP[nloc], dp[loc]);
        }
        swap(dp, DP);
    }
