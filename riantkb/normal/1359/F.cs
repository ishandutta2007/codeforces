...
class Solver {
    ...
    public void solve() {
        ...
        {
            int ss;
            sc.Multi(out x[i], out y[i], out dx[i], out dy[i], out ss);
            s[i] = Math.Sqrt(dx[i] * dx[i] + dy[i] * dy[i]) / (double)ss;
        }
        double ans = LM * 2;
        for (int i = 0; i < n; i++)
        {
            for (int j = i + 1; j < n; j++)
            {
                ...
                if (det == 0) {
                    int dxx = x[j] - x[i];
                    int dyy = y[j] - y[i];
                    if (dx[i] * dyy == dy[i] * dxx) {
                        double kk1 = dxx / (double)dx[i];
                        double kk2 = -dxx / (double)dx[j];
                        if (kk1 > 0 && kk2 > 0) {
                            ans = Math.Min(ans, dxx / (dx[i] / s[i] - dx[j] / s[j]));
                        }
                        else if (kk1 > 0) {
                            ans = Math.Min(ans, s[i] * kk1);
                        }
                        else if (kk2 > 0) {
                            ans = Math.Min(ans, s[j] * kk2);
                        }
                    }
                    continue;
                }
                int sign = det > 0 ? 1 : -1;
                int k1 = -dy[j] * (x[j] - x[i]) + dx[j] * (y[j] - y[i]);
                int k2 = -dy[i] * (x[j] - x[i]) + dx[i] * (y[j] - y[i]);
                if (k1 * sign >= 0 && k2 * sign >= 0) {
                    ans = Math.Min(ans, Math.Max(k1 * s[i] / det, k2 * s[j] / det));
                }
            }
        }
        if (ans > LM) {
