...
void main()
{
	...
	for(int j=0;j<pnt.size();++j)
	{
		for(int k=0;k<=j;++k)
		{
			for(int l=j;l<pnt.size();++l)
			{
				ll d=min(
				dis(S,pnt[j])+dis(pnt[j],pnt[k])
				+dis(pnt[k],pnt[l]),
				dis(S,pnt[j])+dis(pnt[j],pnt[l])
				+dis(pnt[k],pnt[l]));
				if(d<=t) ans=max(ans,l-k+1);
			}
		}
	}
	cout<<ans<<"\n";
