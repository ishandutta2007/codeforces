...
pll parity(int lo, int hi) {
    int nums = hi - lo + 1;
    int even = nums/2, odd = nums/2;
    if (nums&1) ((lo&1) ? odd : even)++;
    return {even, odd};
}
...
int main() {
    ...
    if ((x1 + y1) % 2) ps(x.f * y.s + x.s * y.f);
    else ps(x.f * y.f + x.s * y.s);

    /*int check = 0;
    FOR (xv, x1, x2 + 1) FOR (yv, y1, y2 + 1) {
        if ((xv + yv) % 2 == 0)
            check++;
    }
    ps(check);*/
