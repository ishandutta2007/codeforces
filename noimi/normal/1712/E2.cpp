...
int main() {
    auto F = [](ll l, ll r) {
        auto f = [&](int x, int y, int z) {
            int L = lcm(lcm(x, y), z);
            int lt = z * l, rt = r * x;
            int lp = ceil(lt, L), rp = (rt / L);
            // dump(lp, rp);
            if(lp <= rp) return rp - lp + 1;
            return 0;
        };
        return f(2, 3, 4) + f(2, 3, 5);
    };
    ...
    fore(i, id) {
        while(n >= a[i].fi) {
            rep(j, n, N, n) {
                d[j]++;
                seg.set(j, (d[j] - 1) * (d[j] - 2) / 2);
            }
            n--;
        }
        auto [l, r] = a[i];
        ll sum = (ll)(r + 1 - l) * (r - l) * (r - l - 1) / 6;
        sum -= seg.prod(l, r + 1);
        sum -= F(l, r);
        ans[i] = sum;
    }
    ...
    TEST {
        LL(l, r);
        vi d(N);
        rep(i, l, N) { rep(j, i, N, i) d[j]++; }
        ll sum = (r + 1 - l) * (r - l) * (r - l - 1) / 6;
        r++;
        rep(i, l, r) { sum -= (d[i] - 1) * (d[i] - 2) / 2; }
