...
// S f(S x, S y) {
//     S res();
//     rep(i, 3) rep(j, 3) res[i][j] = -inf<ll>;
//     rep(i, 3) rep(j, 3) rep(k, 3) { chmax(res[i][k], x[i][j] + y[j][k]); }
//     return res;
// }
// S e() {
//     S s;
//     rep(i, 3) rep(j, 3) s[i][j] = (i == j ? 0 : -inf<ll>);
//     return s;
// }
...
T f(T x, T y) { return T{max(x.ma1, y.ma1), max(x.ma2, y.ma2), max({x.ans, y.ans, x.ma1 + y.ma2})}; }
T e() { return T{-inf<ll>, -inf<ll>, -inf<ll>}; }
...
int main() {
    ...
    vl v(n);
    v[0] = a[0][0];
    rep(i, n - 1) v[i + 1] = v[i] + a[0][i + 1];
    vl w(n);
    w[n - 1] = a[2][n - 1];
    per(i, n - 1) w[i] = a[2][i] + w[i + 1];
    ...
    vector<S> q(m);
    rep(i, m) cin >> q[i].l >> q[i].r >> q[i].k;
    SORT(q);

    vector<T> setter(n);
    rep(i, n) setter[i] = T{v[i], w[i], v[i] + w[i]};
    segtree<T, f, e> seg(setter);
    ...
    fore(l, r, k, q) {
        ...
        if(r - 1 < n)
            if(seg[r - 1].ma1 < t) {
                dump(r - 1, t);
                seg.set(r - 1, T{t, seg[r - 1].ma2, seg[r - 1].ma2 + t});
            }
        dump(l, r, k);
    }
