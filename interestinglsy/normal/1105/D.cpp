...
struct InputReader{
    ...
    il void Specialread( char &x ){
        char c = C();
        while( c != '#'  &&  c != '.'  &&  c != '='  &&  c != 'B' && !isdigit(c) ) c = C();
        x = c;
    }
    #undef bs
}In;
...
int diff = 0;
struct Point{ int x,y,gone; };
struct Player{
    int id;
    deque<Point> adj;
    void Expand(){//O(adj.size());
        for( auto &u : adj ) u.gone = 0;
        while(!adj.empty()){
            auto u = adj.front();
            if( u.gone == s[id] ) break;
            adj.pop_front();
            int ux = u.x, uy = u.y;
            For0(k,4){
                int vx = ux + dx4[k];
