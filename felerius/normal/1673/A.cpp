// Subtle Substring Subtraction (https://codeforces.com/contest/1673/problem/A)
...
int main() {
    ...
    while (t--) {
        ...
        auto get_score = [&](char c) { return c - 'a' + 1; };
        int tot = 0;
        trav(c, s)
            tot += get_score(c);
        if (sz(s) % 2) {
            int diff = tot - 2 * min(get_score(s[0]), get_score(s.back()));
            println(diff > 0 ? "Alice" : "Bob", abs(diff));
        } else
            println("Alice", tot);
    }
}