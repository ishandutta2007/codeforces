...
int main() {
    ...
    string s; re(s);
    vvi locs(26);
    F0R (i, sz(s)) locs[s[i] - 'a'].pb(i);

    int Q; re(Q); F0R (q, Q) {
        int L, R; re(L, R); --L;
        array<int, 26> ct;
        F0R (c, 26) ct[c] = lb(all(locs[c]), R)
                          - lb(all(locs[c]), L);

        int distinct = 0;
        trav (e, ct) distinct += e > 0;

        bool irr;
        if (R - L == 1) irr = 1; // len 1 always irreducible
        else if (s[L] != s[R-1]) irr = 1; // if front and back different just swap them
        else if (distinct >= 3) irr = 1;  // if we have bc, do abca and acba
        else irr = 0;
