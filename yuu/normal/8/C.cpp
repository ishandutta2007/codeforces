...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        y[i] -= oy;
        for (int j = 0; j <= i; j++)
            s[i][j] = s[j][i] = x[i] * x[i] + y[i] * y[i] + x[j] * x[j] + y[j] * y[j] + (x[i] - x[j]) * (x[i] - x[j]) +
                                (y[i] - y[j]) * (y[i] - y[j]);
    }

    fill(f + 1, f + (1 << n), 1e9);
    for (int i = 0; i < (1 << n); i++) {
        static vector<int> one_bit;
        one_bit.clear();
        for (int j = 0; j < n; j++)
            if (((i >> j) & 1) == 0) one_bit.push_back(j);
        for (auto&& x : one_bit) {
            for (auto&& y : one_bit) {
                int nx = i | (1 << x) | (1 << y);
                if (f[nx] > f[i] + s[x][y]) {
    ...
    cout << f[current] << '\n' << 0;
    while (current > 0) {
        int prev = trace[current];
        int used = current ^ prev;
        for (int j = 0; j < n; j++)
            if ((used >> j) & 1) {
                cout << ' ' << j + 1;
            }
