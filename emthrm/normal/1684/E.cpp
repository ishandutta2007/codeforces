...
void solve() {
  ...
  FenwickTree<int> num(n + 1), sum(n + 1);
  ...
  for (int mex = 0; mex <= n; ++mex) {
    if (empty > k) break;
    const int all = sum.sum(n + 1);
    if (all <= empty || all <= k) {
      cout << 0 << '\n';
      return;
    }
    // mex = 0 
    const int idx = sum.lower_bound(k);
    if (sum.sum(idx + 1) == k) {
      chmin(ans, num.sum(n + 1) - num.sum(idx + 1));
    } else {
      chmin(ans, num.sum(n + 1) - num.sum(idx) - (k - sum.sum(idx)) / idx);
    }
    if (auto it = a.find(mex); it != a.end()) {
