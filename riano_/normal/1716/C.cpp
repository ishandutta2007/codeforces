...
int main(){
    ...
    rep(ii,T){
        ...
        rep(i,N){
            if(i%2==0){
                chmax(a[1][i],a[0][i]+1);
                if(i!=N-1) chmax(a[1][i+1],a[1][i]+1);
            }
            else{
                chmax(a[0][i],a[1][i]+1);
                if(i!=N-1) chmax(a[0][i+1],a[0][i]+1);
            }
        }
        ll shita = -2e9,ue = -2e9;
        rrep(i,N){
            chmax(shita,max(b[0][i],b[1][i]));
            chmax(ue,max(c[0][i],c[1][i]));
            if(i%2==0){
                chmin(ans,max(a[1][i],shita+i)+2*(N-1-i));
            }
            else{
                chmin(ans,max(a[0][i],ue+i)+2*(N-1-i));
            }
            //cout << ans << endl;
        }
        chmin(ans,ue+2*N-1);
