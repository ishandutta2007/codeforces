...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		cauto n=read();
		cauto vs=readV(n);
		queue<int> todo; rep(i,n) todo.emplace(i);
		set<int> ex; rep(i,n) ex.emplace(i);
		vector<int> re;
		while(todo.size()){
			cauto i=todo.front(); todo.pop();
			auto it=ex.find(i);
			if(it==ex.end()) continue;
			++it;
			if(it==ex.end()) it=ex.begin();
			if(__gcd(vs[i],vs[*it])==1){
				re.eb(*it);
				if(i!=*it) todo.emplace(i);
				ex.erase(it);
			}
		}
		out(re,1);
