...
int a[K],b[K],aa[K],bb[K],ab[K],ba[K],la[K],lb[K],ra[K],rb[K];
...
signed main()
{
    ...
    for(int i=3;i<=k;i++)
    {
        a[i]=a[i-1]+a[i-2];
        b[i]=b[i-1]+b[i-2];
        aa[i]=aa[i-1]+aa[i-2]+ra[i-2]*la[i-1];
        bb[i]=bb[i-1]+bb[i-2]+rb[i-2]*lb[i-1];
        ab[i]=ab[i-1]+ab[i-2]+ra[i-2]*lb[i-1];
        ba[i]=ba[i-1]+ba[i-2]+rb[i-2]*la[i-1];
        la[i]=la[i-2];
        lb[i]=lb[i-2];
        ra[i]=ra[i-1];
        rb[i]=rb[i-1];
        ...
        for(int r1=0;r1<=1;r1++)
            for(int l2=0;l2<=1;l2++)
                for(int r2=0;r2<=1;r2++)
                    for(int m1=0;m1*2<=(n-l1-r1);m1++)
                        for(int m2=0;m2*2<=(m-l2-r2);m2++)
                            if(a[k]*m1+b[k]*m2+aa[k]*l1*r1+bb[k]*l2*r2+ab[k]*l2*r1+ba[k]*l1*r2==x)
                            {
//                                cout<<a[k]<<" "<<m1<<" "<<b[k]<<" "<<m2<<" "<<aa[k]<<" "<<l1<<" "<<r1<<" "<<bb[k]<<" "<<l2<<" "<<r2<<" "<<ab[k]<<" "<<l2<<" "<<r1<<" "<<ba[k]<<" "<<l1<<" "<<r2<<" "<<x<<"\n";
                                string s1;
                                while(l1--) s1+="C";
                                while(m1--) s1+="AC";
                                while(s1.size()<n-r1) s1+="X";
                                while(r1--) s1+="A";
                                string s2;
                                while(l2--) s2+="C";
                                while(m2--) s2+="AC";
                                while(s2.size()<m-r2) s2+="X";
                                while(r2--) s2+="A";
                                cout<<s1<<"\n"<<s2<<"\n";
                                exit(0);
                            }
