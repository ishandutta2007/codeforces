...
int aget(int l,int u,int r,int d){
	return sums[d][r]-sums[u][r]-sums[d][l]+sums[u][l];
}
char chch[2000000];
int main(){
	...
	for(int i=0;i<H;++i){
		for(int j=0;j<W;++j){
			sums[i+1][j+1]=sums[i][j+1]+sums[i+1][j]-sums[i][j]+(field[i][j]=='X');
		}
	}
	int ok=1;
	int ng=10000;
	while(ok+1!=ng){
		...
		if(asz>min(H,W)){
			...
			vector<vector<int>>cnts(H+2,vector<int>(W+2));

			for(int u=0;u<=H-asz;++u){
				for(int l=0;l<=W-asz;++l){
					int cnt=aget(l,u,l+asz,u+asz);
					if(cnt==asz*asz){
						cnts[u+1][l+1]++;
						cnts[u+1][l+asz+1]--;
						cnts[u+asz+1][l+1]--;
						cnts[u+asz+1][l+asz+1]++;
					}
				}
			}
			//WHATS(cnts)
			for(int u=0;u<H;++u){
				for(int l=0;l<W;++l){
					cnts[u+1][l+1]+=cnts[u][l+1]+cnts[u+1][l]-cnts[u][l];
					if((cnts[u+1][l+1]>=1)^(field[u][l]=='X')){
	...
	for(int u=0;u<=H-asz;++u){
		for(int l=0;l<=W-asz;++l){
			if(aget(l,u,l+asz,u+asz)==asz*asz){
				answer[u+ok-1][l+ok-1]='X';
			}
		}
	}
	cout<<ok-1<<endl;
	//if(H==999999)return 0;
