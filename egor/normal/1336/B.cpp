...
public:
    void solve() {
        auto n = in.readIntArray(3);
        vec<arri> w;
        for (int i : range(3)) {
            w.push_back(in.readIntArray(n[i]));
            sort(all(w[i]));
        }
        ...
        while (true) {
            ...
            for (int i : range(3)) {
                ...
                if (minId == -1 || w[i][at[i]] < w[minId][at[minId]]) {
                    minId = i;
                }
                if (maxId == -1 || w[i][at[i]] >= w[maxId][at[maxId]]) {
                    maxId = i;
                }
            }
            if (!ok) {
            ...
            if (at[minId] + 1 < n[minId] && w[minId][at[minId] + 1] <= w[maxId][at[maxId]]) {
                at[minId]++;
                continue;
            }
            int midId = 3 - minId - maxId;
            while (true) {
                ll cur = 0;
                for (int i : range(3)) {
                    for (int j : range(i)) {
                        cur += ll(w[i][at[i]] - w[j][at[j]]) * (w[i][at[i]] - w[j][at[j]]);
                    }
                }
                minim(answer, cur);
                if (at[midId] + 1 < n[midId] && w[midId][at[midId] + 1] <= w[maxId][at[maxId]]) {
                    at[midId]++;
                } else {
                    break;
                }
