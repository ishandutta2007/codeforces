...
int main(){
  ...
  vector<int>ord(n);
  iota(ALL(ord),0);
  sort(ALL(ord),[&](int i,int j){return argument_compare(p[i],p[j]);});
  vector<Point>q;
  vector<vector<ll>>v;
  ll idx=0;
  vector<ll>ret(n,0);
  while(idx<n){
    ret[ord[idx]]=1;
    Point now=p[ord[idx]];
    v.emplace_back(1,ord[idx]);
    idx++;
    while(idx<n){
      if(norm(now+p[ord[idx]])<=(ll)1e12){
        now+=p[ord[idx]];
        ret[ord[idx]]=1;
      }
      else if(norm(now-p[ord[idx]])<=(ll)1e12){
        now-=p[ord[idx]];
        ret[ord[idx]]=-1;
      }
      else break;
  ...
  rep(i,0,1<<sz){
    ...
    if(sum.x<=(ll)2e9&&sum.y<=(ll)2e9&&norm(sum)<=1500000LL*1500000){
      //cout<<sum<<endl;
      rep(j,0,sz){
        if(~j>>i&1)for(auto z:v[j])ret[z]*=-1;
      }
      debug(ret);
      return 0;
    }
