...
void run() {
    const int mod = 1e9 + 7;
    constexpr int rev10 = 700000005;
    constexpr int rev2 = mod - (mod >> 1);
    ...
    vector<int> pw(n + 1, 1);
    for (int i = 1; i <= n; ++i) {
        pw[i] = int(1ll * pw[i - 1] * 10 % mod);
    }

    ll ans = 0;
    for (int i = 0; i < n; ++i) {
        ans += (((1ll * pw[n - i - 1] * (s[i] - '0') % mod) * i % mod) * (i + 1) % mod) * rev2;
        ans %= mod;
    }
    ...
    for (int i = n - 1; i > -1; --i) {
        ...
        cur_p *= rev10;
        cur_p %= mod;
        ans += ((s[i] - '0') * cur_mul % mod) * pw[n - i - 1];
        ans %= mod;
    }
    cout << ans << endl;
}