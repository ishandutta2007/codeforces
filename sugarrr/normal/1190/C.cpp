...
signed main(){fastio
    ll n,k;cin>>n>>k;
    string s;cin>>s;
    vector<segtree<l_l,op,e>>seg(2,segtree<l_l,op,e>(n));
    rep(i,0,n-1){
        if(s[i]=='0'){
            seg[0].set(i,{i,i});
        }else{
    ...
    rep(i,0,n-k){
        vl_l A(2);
        rep(z,0,1)A[z]=op(seg[z].prod(0,i),seg[z].prod(i+k,n));
        rep(z,0,1){
            if(A[z].fi==inf)first();
        }
       // rep(z,0,1)cout<<A[z].fi<<" "<<A[z].se<<endl;cout<<endl;
       // vl_l B=A;
        //chmin(A[0].fi,i);chmax(A[0].se,i+k-1);
        if(max(A[0].se-A[0].fi+1,A[1].se-A[1].fi+1)>k)ev=true;
    }
    if(ev)even();
    else second();
