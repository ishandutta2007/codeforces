...
mint naive(vector<mint> p){
    int n = p.size();
    vector<mint> sum(n+1), ans(n+1);
    for(int i = 0; i < n; i++){
        sum[i+1] = sum[i]/2 + p[i]/2;
        ans[i+1] = ans[i] + sum[i]*p[i]/2;
    }
    return ans[n];
}

using M = Matrix<mint, 3, 3>;
using V = Matrix<mint, 1, 3>;
...
int main(){
    ...
    for(int i = 0; i < q; i++){
        cin >> idx[i] >> x[i]; idx[i]--;
        cnt[idx[i]]++;
        vt.push_back(T(x[i], idx[i], cnt[idx[i]]));
    }
    auto cp = Compress<T>(vt);
    int m = cp.size();
    Seg seg(m);
    cnt.assign(n, 0);
    for(int i = 0; i < n; i++){
        int idx = cp[T(p[i].val(), i, 0)];
        seg.set(idx, f(p[i]));
    }
    V v(0);
    v[0][2] = 1;
    // cout << naive(p) << endl;
    auto ans = v*seg.all_prod();
    cout << ans[0][1] << endl;
    for(int i = 0; i < q; i++){
        int i_prev = cp[T(p[idx[i]].val(), idx[i], cnt[idx[i]])];
        cnt[idx[i]]++;
        int i_next = cp[T(x[i], idx[i], cnt[idx[i]])];
        seg.set(i_prev, e());
        seg.set(i_next, f(mint(x[i])));
        p[idx[i]] = x[i];
        auto ans = v*seg.all_prod();
        cout << ans[0][1] << endl;
    }
