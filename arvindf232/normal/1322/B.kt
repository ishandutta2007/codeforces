...
fun pairsThatExceed(line:IntArray,cut:Int):Long {
    val n = line.size
    var r = n-1
    var ret = 0L
    first@for(l in 0 until n){
        while(line[l] + line[r] >= cut){
            r --
            if(r == -1){
                break@first
            }
...
fun main(){
    solve.cases{
        ...
        for(b in 1..25){
            ...
            cut.radixsort()
            val noextrapairs = pairsThatExceed(cut,( 1 shl b))
            var carries = (n.toLong() * n) - noextrapairs
            carries -= cut.count { it >= (1 shl (b-1)) }
            carries = carries /2

            val presentBits = cut.count{it and ( 1 shl (b-1)) != 0}
            total[b-1] += presentBits.toLong() * (n-1)
            total[b] += carries
        }
        var ret = 0
        for(i in 0..25){
