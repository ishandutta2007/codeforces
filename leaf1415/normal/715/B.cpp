...
void dijkstra(vector<edge> G[], int n, int S, llint dist[], P pre[])
{
...
int main(void)
{
	...
	rep(i, 1, m) if(w[i] == 0) b[i] = true, w[i] = 1;
	...
	while(1){
		...
		ll eid = 0, v = t;
		while(v != s){
			if(b[pre[v].second]) eid = pre[v].second;
			v = pre[v].first;
		}
		if(eid == 0){
			no();
