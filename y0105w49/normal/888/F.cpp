...
int solve_half(int,int);

int solve_full(int L,int R) {
	static int dp[N][N];
	...
	ans=solve_half(L,R-1);
	for(int M=L+1;M<=R-1;M++) {
		if(!a[M][R]) continue;
		add(ans,int(1LL*solve_full(L,M)*solve_full(M,R)%P));
	}
	return ans;
}

int solve_half(int L,int R) {
	static int dp[N][N];
	static bool seen[N][N];
	int &ans=dp[L][R];
	bool &sn=seen[L][R];
	...
	for(int M=L+1;M<=R;M++) {
		if(!a[L][M]) continue;
		add(ans,int(1LL*solve_full(L,M)*solve_half(M,R)%P));
	}
	return ans;
}
