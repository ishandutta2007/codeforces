...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dp = make_vector<double>(n + 1, n * m + 1, 0.0);
		auto pref = make_vector<double>(n + 1, n * m + 2, 0.0);
		dp[0][0] = 1;
		for (int i: range(n)) {
			for (int j: range(dp[i].size())) {
				pref[i][j + 1] = pref[i][j] + dp[i][j];
			}
			for (int j: range(dp[i].size())) {
				if(j >= v[i]) {
					dp[i + 1][j] = (pref[i][j + 1] - pref[i][max(0, j - m + 1)] - dp[i][j - v[i]]) / (m - 1);
				}
				else {
					dp[i + 1][j] = (pref[i][j + 1] - pref[i][max(0, j - m + 1)] - 0) / (m - 1);
				}
			}
		}
