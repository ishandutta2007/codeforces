...
int mat[N][N], disa[N], disb[N];
double numa[N], numb[N];
int q[N], head, tail;
int n;
void bfs(int *dis, double *num, int s){
    memset(dis, 0x3f, sizeof(disa));
    memset(num, 0, sizeof(numa));
    dis[s] = 0;
    num[s] = 1;
    head = tail = 0;
    q[tail ++] = s;
    while(head < tail){
        int u = q[head++];
        for(int i=1; i<=n; ++i){
            if(mat[u][i] == 0 || dis[i] <= dis[u]) continue;
            if(dis[i] > dis[u] + 1) q[tail ++] = i;
            dis[i] = dis[u] + 1;
            num[i] += num[u];
        }
    }
...
int main(){
    ...
    while(scanf("%d%d", &n, &m) != EOF){
        ...
        double ans = numa[n];
        for(int i=2; i<n; ++i){
            if(disa[i] + disb[i] == len){
                checkmax(ans, numa[i] * numb[i] * 2);
            }
        }
        printf("%.9lf\n", ans / numa[n]);
    }
