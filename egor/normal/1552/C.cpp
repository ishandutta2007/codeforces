...
public:
    void solve() {
        ...
        cFree[0] = 0;
        for (int i : range(2 * n)) {
            cFree[i + 1] = cFree[i] + (free[i] ? 1 : 0);
        }
        int answer = (n - k) * (n - k - 1) / 2;
        for (int i : range(k)) {
            int inside = cFree[max(y[i], x[i])] - cFree[min(y[i], x[i])];
            int outside = 2 * n - 2 * k - inside;
            answer += min(inside, outside);
            int a = min(x[i], y[i]);
            int b = max(x[i], y[i]);
            for (int j : range(i)) {
                if ((a <= x[j] && x[j] <= b) ^ (a <= y[j] && y[j] <= b)) {
                    answer++;
                }
            }
        }
