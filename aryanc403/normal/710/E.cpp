...
lli sol(lli n)
{
    ...
    ans=INF;
    if(n&1)
        return ans=x+min({y+sol(n/2),y+sol(n/2+1),sol(n-1)});
    return ans=min({y+sol(n/2),x+sol(n-1)});
}
...
// cin>>T;while(T--)
{
    ...
    repA(i,2,n)
    {
        dp[i]=min(dp[i],x+dp[i-1]);
        if(i&1)
        {
            dp[i]=min(dp[i],x+y+dp[i/2]);
            dp[i]=min(dp[i],x+y+dp[i/2+1]);
        }
        else
