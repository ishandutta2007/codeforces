...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/249177/problem/I */
/*input
4
40 10 30 20
1 2 1
...
int a[5000];
vector<pii> gra[5000];
priority_queue<pii> eil[5000];
int ats[5000];
int dien = 0;
int apl = 1;

void diena(int x, int p = -1, int c = 0) {
	if (x != 0) {
		mini(c, eil[x].size());

		for (int i = 0; i < c; i++) {
			eil[p].emplace(eil[x].top());
			eil[x].pop();
		}
	}
...
int main () {
	...
	for (int i = 0; i < n - 1; i++) {
		...
		cin >> a >> b >> c;
		a--, b--;
		gra[a].emplace_back(b, c);
		gra[b].emplace_back(a, c);
	}
