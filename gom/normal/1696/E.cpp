...
int n,a[N];
mi fact[N],finv[N],ans;
mi dp[100][100],dp2[100][100];
mi ncr(int n,int r){
...
void solve(){
	...
	fact[0]=1;
	for(int i=1;i<=400005;i++) fact[i]=fact[i-1]*i;
	finv[400005]=1/fact[400005];
	for(int i=400004;i>=0;i--) finv[i]=finv[i+1]*(i+1);
	if(!a[1]){
		cout<<"0\n";
		return;
	}
	while(!a[n]) n--;
	for(int i=1;i<=n;i++){
		ans+=ncr(i+a[i]-1,a[i]-1);
		//for(int j=0;j<a[i];j++) ans+=ncr(i-1+j,j);
		//for(int j=0;j<a[i];j++) cout<<i<<" "<<j<<": "<<ncr(i-1+j,j)<<" "<<dp[i][j+1]<<endl;
		//cout<<i<<" "<<a[i]<<" | "<<ncr(i+a[i]-1,a[i]-1)<<endl;
	}
	cout<<ans;
}
int main(){
