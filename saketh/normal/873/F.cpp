...
void solve() {
    ...
    union_find uf(N);
    F0R (i, N) uf[i].size = t[sa.by_rank[i]] != '1';

    FORd (len, 1, N + 1) {
        ckmax(biggest_component, uf[sa.rank(N - len)].size);

        trav (rank, merges[len]) {
            uf.unio(rank, rank + 1);
            ckmax(biggest_component, uf[rank].size);
        }

        ckmax(ans, ll(len) * biggest_component);
    }
