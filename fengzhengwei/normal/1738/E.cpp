...
{
	ll res=1;
	for(int i=1;i<=min(L,R);i++)(res+=C(L,i)*C(R,i))%=mod;
	return res%mod;
}
int main(){
	pre();
	...
	while(T--)
	{
		...
		for(int i=1;i<=n;i++)
		{
			if(id[i]<i||!a[i])continue;
			if(pf[i]==pf[n])continue;
			if(sf[id[i]]!=pf[i])continue;
			lp=i+1,rp=id[i]-1;
			while(a[lp]==0)++lp;
			while(a[rp]==0)--rp;
			L=lp-i,R=id[i]-rp;
