...
void solve() {
  LL(N, L, R);
  ll x0 = floor(R, 3) - floor(L - 1, 3);
  ll x1 = floor(R + 2, 3) - floor(L + 2 - 1, 3);
  ll x2 = floor(R + 1, 3) - floor(L + 1 - 1, 3);
  assert(x0 + x1 + x2 == R - L + 1);
  ...
  FOR(N) {
    vc<mint> newdp(3);
    FOR(i, 3) {
      newdp[i] += dp[i] * mint(x0);
      newdp[(i + 1) % 3] += dp[i] * mint(x1);
      newdp[(i + 2) % 3] += dp[i] * mint(x2);
    }
    swap(dp, newdp);
  }
  print(dp[0]);
