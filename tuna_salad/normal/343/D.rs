...
fn dfs(u: usize, e: &[Vec<usize>], par: &mut [usize], dfst: &mut [(usize,usize)], t: &mut usize) {
	dfst[u].0 = *t;
	*t += 1;
	for &v in e[u].iter() {
		if v != par[u] {
...
fn main() {
	...
	let mut par = v!([n] = n);
	dfs(0, &e, &mut par, &mut dfst, &mut 0);
	...
	rep!{[rin.u()]
		...
		if t == 1 {
			let mut q = v!([]);
			for &i in empty.range(dfst[v].0..=dfst[v].1) {
				q.push(i);
			}
			if v != 0 && q.len() > 0 {
				empty.insert(dfst[par[v]].0);
			}
			for i in q {
				empty.remove(&i);
			}
		} else if t == 2 {
			empty.insert(dfst[v].0);
		} else {
			let res = if let Some(_) = empty.range(dfst[v].0..=dfst[v].1).next() {
				0
			} else {
				1
			};
