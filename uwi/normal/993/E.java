...
public class E {
	...
	void dfs(int l, int r, boolean[] a, long[] anss)
	{
		if(r-l == 1){
			anss[a[l] ? 1 : 0]++;
			return;
		...
		long[] f = fl.length >= 30000 ? convolute(fl, fr) : fl.length >= 100 ? convolute1(fl, fr) : mulnaive(fl, fr);
		for(int i = 0;i < f.length && i < anss.length;i++){
			anss[i] += f[i];
		}
...
//		for(int i = 0;i < n;i++){
//			sb.append(gen.nextInt(100000)-50000 + " ");
//		}
//		INPUT = sb.toString();
