...
public class E {
	...
	public void solve() {
		for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			...
			HashSet<Long> banned = new HashSet<>();
			for (int i = 0; i < m; i++) {
				banned.add(merge(in.nextInt(), in.nextInt()));
			}
			HashMap<Integer, Integer> szn = new HashMap<>();
			for (int i : map.values()) {
				add(szn, i);
			}
			int cnts = szn.size();
			int[][] values = new int[cnts][];
			int[] cnt = new int[cnts];
			int[] sz = new int[cnts];
			int id = 0;
			HashMap<Integer, Integer> cnt2id = new HashMap<>();
			for (int x : szn.keySet()) {
				cnt[id] = x;
				cnt2id.put(x, id);
				values[id++] = new int[szn.get(x)];
			}
			for (int i : map.keySet()) {
				id = cnt2id.get(map.get(i));
				values[id][sz[id]++] = i;
			}
			...
			for (int x : map.keySet()) {
				...
				for (int i = 0; i < cnts; ++i) {
					for (int j = sz[i] - 1; j >= 0; --j) {
						if (x != values[i][j] && !banned.contains(merge(x, values[i][j]))) {
							if ((cntx + cnt[i]) * 1L * (x + values[i][j]) > ans) {
								ans = (cntx + cnt[i]) * 1L * (x + values[i][j]);
							}
							break;
						}
					}
