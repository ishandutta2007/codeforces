...
segtree<int, op, e> seg(VI(200100, 1));
...
int main() {
    ...
    while(tt--) {
        ...
        int cnt = 0;
        for(int y: y) {
            int idx = seg.max_right(0, [&](int i) { return i < y + 1; });
            seg.set(idx, 0);
            inserted_pos.push_back(idx);
            if (seg.get(idx + 1)) cnt++;
        }
        int t = n - 1 - cnt;
        mint ans = comb(n + t, t);
        cout << ans.val() << '\n';

        for(int i: inserted_pos) seg.set(i, 1);
    }
}