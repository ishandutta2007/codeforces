...
#endif
using std::vector;
int a[1000005],d[1000005],z[1000005];
struct pair{int b,d;};
std::set<int> s;
std::vector<pair>g[1000005];
int v[1000005];
int L[1000005],R[1000005];
int n,m;
void sdsd(int id,int l,int r)
{
	if(v[id])return;
	v[id]=1;;
	auto p=s.lower_bound(l);
	for(auto q=p;(q)!=s.end()&&(*q)<=r;q++)z[*q]=1;
	std::vector<int>w;
	for(auto q=p;(q)!=s.end()&&(*q)<=r;q++)
	{
		int l=*q;
		for(auto P:g[l])
		{
			int r=P.b,d=P.d;
			if(v[d])continue;
			if(z[l]&&z[r])w.push_back(d);
		}
	}
	for(auto q=p;(q)!=s.end()&&(*q)<=r;)q=s.erase(q);
	for(auto x:w)sdsd(x,L[x],R[x]);
	// printf("%d %d %d\n",id,l,r);fflush(stdout);
}
signed main()
{
	initprog();
	...
	int T=read();rg(T)
	n=read(),m=read();
	rg(n)a[i]=read();gr rg(n)a[i]-=read();d[i]=z[i]=0;gr
	z[0]=1;
	rg(n)d[i]=d[i-1]+a[i];z[i]=!d[i];gr
	// rg(n)odb(d[i]);gr puts("");
	s.clear();
	rrg(n+5)g[i].clear();gr
	rrg(m+5)v[i]=0;gr
	rrg(n+1)if(!z[i])s.insert(i);gr
	rg(m)
	int l=read(),r=read();l--;
	L[i]=l,R[i]=r;
	g[l].push_back((pair){r,i});g[r].push_back((pair){l,i});
	gr
	// rg(n)odb(z[i]);gr puts("");continue;
	rrg(n+1)int l=i;
	for(auto P:g[i])
	{
		int r=P.b,d=P.d;
		...
		if(z[l]&&z[r])
		// odl(d),odp(l,r),
		sdsd(d,min(l,r),max(l,r));
	}
	gr
	int f=0;
	rg(n)if(z[i]==0)f=1;gr
	// odl(f);
