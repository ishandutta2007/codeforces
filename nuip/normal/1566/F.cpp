...
struct sorted_impl {
    template<class T>
    friend std::vector<T> operator-(std::vector<T> a, sorted_impl) {
...
int main(){
	...
	TT{
		cauto n=read();
		cauto m=read();
		auto vs=readV(n)-sorted;
		cauto ps=readV<pll>(m);
		vv<pll> ints(n+1);
		for(cauto &[l,r]:ps){
			cauto i=lower_bound(all(vs),l)-vs.begin();
			if(i==n || r<vs[i]){
				ints[i].eb(l,r);
			}
		}
		for(auto &ps:ints) sort(rall(ps));
		const ll INF=1e15;
		vector<ll> ret(n+1,INF),awa(n+1,INF);
		{
			ll t=0;
			for(cauto &p:ints[0]) MX(t,vs[0]-p.Y);
			ret[0]=t+t;
			awa[0]=t;
		}
		vs.eb(INF);
		rep(i,n){
			ll mx=0;
			for(cauto &[l,r]:ints[i+1]){
				cauto d=l-vs[i];
				cauto mn=min(ret[i]+d,awa[i]+d+d);
				MN(ret[i+1],mn+mx+mx);
				MN(awa[i+1],mn+mx);
				MX(mx,vs[i+1]-r);
			}
			MN(ret[i+1],min(ret[i],awa[i])+mx+mx);
			MN(awa[i+1],min(ret[i],awa[i])+mx);
		}
		out(ints,vs,ret,awa,1);
		cout<<min(ret[n],awa[n]) NL;
	}
	return 0;
}