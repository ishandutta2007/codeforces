...
int main() {
    ...
    v<v<v<v<mod>>>> dp(2, v<v<v<mod>>>(n+1, v<v<mod>>(n+1, v<mod>(2, 0))));
    dp[0][0][0][0] = 1;
    for (int i = 1; i <= m; i++) {
        for (int j = 0; j <= n; j++)
            for (int k = 0; k <= n; k++)
                for (int b = 0; b < 2; b++) {
                    int c = i == x ? 0 : b;
                    dp[1][j][k][b] = dp[0][j][k][b]; // do nothing
                    if (j > 0 && k > 0)
                        dp[1][j][k][b] += dp[0][j-1][k-1][c]; // open interval
                    if (j < n)
                        dp[1][j][k][b] += dp[0][j+1][k][b]; // close interval
                    if (k > 0)
                        dp[1][j][k][b] += dp[0][j][k-1][c]; // both open and close
                }
        swap(dp[0], dp[1]);
    }
    mod ans = dp[0][0][n][1];
