...
inline void AC() {
    ...
    for(; !q.empty(); ) {
        node t = q.top();
        q.pop();
        if(d[t.p][t.t] < t.s) continue;
        if(t.t) {
            for(edge i : g[t.p]) if(d[i.to][0] > d[t.p][t.t]+(t.t+i.cost)*(t.t+i.cost)) {
                d[i.to][0] = d[t.p][t.t]+(t.t+i.cost)*(t.t+i.cost);
                q.push((node){i.to, 0, d[i.to][0]});
            }
        } else {
            for(edge i : g[t.p]) if(d[i.to][i.cost] > d[t.p][0]) {
                d[i.to][i.cost] = d[t.p][0];
                q.push((node){i.to, i.cost, d[i.to][i.cost]});
            }
        }
    }
    for(_ i = 1; i <= n; ++i) writesc(d[i][0] > 1E11 ? -1 : d[i][0]);
    clr();
