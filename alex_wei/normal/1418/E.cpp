...
int main(){
	...
	for(int i=1;i<=m;i++){
		ll a,b; cin>>a>>b;
		ll sm=lower_bound(d+1,d+n+1,b)-d-1,la=n-sm,ans=0;
		if(la<a)puts("0");
		else{
			if(sm>0)ans=fc[la]*(la-a+1)%mod*fc[n]%mod*ifc[la+1]%mod*s[sm];
			cout<<(ans+fc[la-1]*(la-a)%mod*fc[n]%mod*ifc[la]%mod*(s[n]-s[sm]+mod))%mod*ifc[n]%mod<<endl;
		}
	}
	return 0;
}