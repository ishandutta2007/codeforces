...
void solve()
{
    ...
    while (n--)
    {
        cin>>tmp; setik.insert(tmp);
    }


    if (!setik.count(1)) {cout<<"No"<<endl; return;}

    //vector<pair<int, int>> bad;

    vector<int> st(c+1), en(c+1);

    for (int i = 2; i<=c; i++) if (!setik.count(i))
    {
        for (auto x: setik)
        {
            if (1ll*i*x>1ll*c) break;
            else
            {
                //bad.push_back(mp(i*x, min(c, i*x+i-1)));
                st[i*x]++;
                en[min(c, i*x+x-1)]++;
            }
        }
    ...
    {
        cnt+=st[i];
        if (cnt && setik.count(i)) {cout<<"No"<<endl; return;}
        cnt-=en[i];
    }
