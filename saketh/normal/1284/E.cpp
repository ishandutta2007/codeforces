...
int main() {
    ...
    F0R (origin, N) {
        ...
        vector<pt> others;
        F0R (j, N) if (j != origin) others.pb(p[j] - zero);

        pt ZERO = { 0, 0 };

        sort(all(others), [&](pt a, pt b) {
            ...
            return cross(a, b - a) > 0;
        });

        /*F0R (i, N - 2) {
            double ang1 = others[i].ang();
            if (ang1 < 0) ang1 += 4 * atan2(1, 0);
            double ang2 = others[i+1].ang();
            if (ang2 < 0) ang2 += 4 * atan2(1, 0);
            pd(ang1, ang2);
            assert(ang1 <= ang2);
        }*/

        F0R (i, N - 1) others.pb(others[i]); // double them

        int j = 0;
        F0R (i, N - 1) {
            while (j <= i || cross(ZERO-others[i], others[j]) < 0) j++;
            ans -= nc3(j - i - 1);
        }
    }
