...
int main() {
    ...
    VEC(int, a, n);
    VEC(int, b, m);
    rep(i, m) idx[b[i]].eb(i);
    vv(int, dp, c + 1, n + 1, inf<int>);
    rep(i, si(idx)) idx[i].eb(inf<int> + 1);
    dp[0][0] = -1;
    rep2(i, 1, c) {
        int mi = inf<int>;
        bool ok = false;
        rep2(j, 1, n) {
            chmin(mi, dp[i - 1][j - 1]);
            chmin(dp[i][j], *upper_bound(all(idx[a[j - 1]]), mi));
        }
    }
    int ans = 0;
    rep2(i, 1, c) rep2(j, 1, n) {
        if((j + dp[i][j] + 1) + i * e <= s) chmax(ans, i);
    }
    cout << ans << endl;
}