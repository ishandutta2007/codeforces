...
void solve() {
  ...
  FOR3(lca, 1, N) {
    auto isin = [&](ll v) -> bool {
      return LID[lca] <= LID[v] && LID[v] < RID[lca];
    };
    ...
    FOR(v, N) {
      if (isin(v))
        dp[lca] += dist[v] - dist[lca];
      else
        dp[0] += dist[v];
    }
    FORIN(v, V) {
      if (v == 0 || v == lca) continue;
      auto p = par[v];

      if (isin(v)) {
        dp[v] = dp[p] + (sz[lca] - sz[v] - sz[v]) * wt[v];
      } else {
        dp[v] = dp[p] + (sz[0] - sz[v] - sz[v]) * wt[v];
      }
    }
    ll sum1 = dp[0], sum2 = dp[lca];
    ll sz1 = N - sz[lca], sz2 = sz[lca];
