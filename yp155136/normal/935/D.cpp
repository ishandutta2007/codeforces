...
int main () {
    ...
    REP1(i,n)
    {
        ...
        {
            ans += (m-b[i])*1LL*get_rev(m)%mod*pre%mod;
            //cout << "val1 = "<<m-b[i]<<" , val2 = "<<get_rev(m)<<" , val3 = "<<pre<<endl;
            //cout << "ans += "<<(m-b[i])*get_rev(m)%mod*pre%mod<<endl;
            pre *= get_rev(m);
            pre %= mod;
            ans %= mod;
        }
        ...
        {
            ans += (a[i]-1)*get_rev(m)%mod*pre%mod;
            //cout << "val1 = "<<a[i]-1<<" , val2 = "<<get_rev(m)<<" , val3 = "<<pre<<endl;
            //cout << "ans += "<<(a[i]-1)*get_rev(m)%mod*pre%mod<<endl;
            pre *= get_rev(m);
            pre %= mod;
            ans %= mod;
        }
        ...
        {
            //ans += ((m*1LL*m*1LL - m)/2 )%mod * get_rev(m)%mod*get_rev(m)%mod*pre%mod;
            ans += (m-1)%mod * get_rev(2) % mod * get_rev(m) % mod * pre % mod;
            pre *= get_rev(m);
            pre %= mod;
            ans %= mod;
        }
