...
void solve(){
    ...
    map<pair<int, int>, vector<int>> sankaku_edge;
    REP(i, n - 2) {
        int a, b, c;
        cin >> a >> b >> c;
    ...
    REP(i, n - 2) {
        sankaku_edge[mp(min(get<0>(inputs[i]), get<1>(inputs[i])), max(get<0>(inputs[i]), get<1>(inputs[i])))].push_back(i);
        sankaku_edge[mp(min(get<2>(inputs[i]), get<1>(inputs[i])), max(get<2>(inputs[i]), get<1>(inputs[i])))].push_back(i);
        sankaku_edge[mp(min(get<2>(inputs[i]), get<0>(inputs[i])), max(get<2>(inputs[i]), get<0>(inputs[i])))].push_back(i);
    }
    for (auto& x : sankaku_edge) {
        REP(q, x.second.size() - 1) {
            vertexs[x.second[q]].push_back(x.second[q + 1]);
            vertexs[x.second[q + 1]].push_back(x.second[q]);
            cnt[x.second[q]]++;
            cnt[x.second[q + 1]]++;
        }
        if (x.second.size() == 1) {
            loops[x.first.first].push_back(x.first.second);
            loops[x.first.second].push_back(x.first.first);
        }
    }
    queue<int> nexts;
    vector<int> ans;
    ...
    vector<int> visited(n + 1, 0LL);
    visited[1] = 1;
    int now = 1;
    REP(i, n) {
        cout << now << " ";
        for (auto& x : loops[now]) {
            if (visited[x] == 0) {
                visited[x] = 1;
                now = x;
                break;
