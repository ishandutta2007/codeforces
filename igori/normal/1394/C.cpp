...
signed main()
{
    ...
    {
        int M = (L + R) / 2;
        int amin = 0, amax = 1e6, bmin = 0, bmax = 1e6, difmin = -1e6, difmax = 1e6;
        for (int i = 0; i < n; i++)
        {
            amin = max(amin, a[i] - M);
            amax = min(amax, a[i] + M);
            bmin = max(bmin, b[i] - M);
            bmax = min(bmax, b[i] + M);
            difmin = max(difmin, a[i] - b[i] - M);
            difmax = min(difmax, a[i] - b[i] + M);
        }
        int t = 0;
        if (amin <= amax && bmin <= bmax && difmin <= difmax)
        {
            int c = amin - bmax;
            int d = amax - bmin;
            if (d < difmin || difmax < c)
                L = M;
            else
                R = M;
        }
    ...
    int t = 0;
    int a1 = amin, b1 = bmax, a2 = amax, b2 = bmin;
    while (!(difmin <= a1 - b1 && a1 - b1 <= difmax))
    {
        if (a1 < a2) a1++;
        else b1--;
    }
