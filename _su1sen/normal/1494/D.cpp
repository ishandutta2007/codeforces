...
int main() {
    ...
    sort(all(ord), [&](auto &p, auto &q) { return a[p.first][p.second] < a[q.first][q.second]; });
    ...
    atcoder::dsu uf(n * n + n);
    for (auto [i, j] : ord) {
        int ti = uf.leader(top[i]), tj = uf.leader(top[j]);
        if (ti == tj) continue;
        if (c[ti] == a[i][j] and c[tj] == a[i][j]) {
            uf.merge(ti, tj);
        } else if (c[ti] == a[i][j]) {
            top[j] = ti;
            par[tj] = ti;
            ...
            par.push_back(-1);
            top[i] = top[j] = node_id;
            par[ti] = par[tj] = node_id;
            ++node_id;
        }
    }

    vector<int> ids(node_id, -1);
    rep(i, n) ids[i] = i;
    int id = n;
    for (auto &group : uf.groups()) {
        if (group[0] >= node_id or group[0] < n) continue;
        for (int v : group) ids[v] = id;
        ++id;
    }
    ...
    rep(i, node_id) {
        int r = uf.leader(i);
        if (par[r] < 0) root = ids[r];
    }

    vector<int> ans_c(id), ans_par(id);
    rep(i, node_id) {
        int id = ids[i], r = uf.leader(i);
        ans_c[id] = c[r];
        if (par[r] >= 0) ans_par[id] = ids[par[r]];
    }
