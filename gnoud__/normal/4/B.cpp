...
using namespace std;
int n,s,f[40][500],l[40],r[40],trc[40][500],a[40];
main()
{
    ...
    f[0][0]=1;
    forinc(i,0,n-1) forinc(j,0,240) if(f[i][j])
    {
        forinc(t,l[i+1],r[i+1])
        {
            f[i+1][j+t]=1;
            trc[i+1][j+t]=t;
        }
    }
    if(!f[n][s]) {cout<<"NO";return 0;}
    cout<<"YES\n";
    fordec(i,n,1) a[i]=trc[i][s],s-=trc[i][s];
