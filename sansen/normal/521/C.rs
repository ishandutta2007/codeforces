...
fn run() {
    ...
    let s = s.into_iter().map(|s| M::new((s - b'0') as u32)).collect::<Vec<_>>();
    let mut sum = s.iter().fold(M::zero(), |s, a| s + *a);
    let mut ans = M::zero();
    let mut pow = M::one();
    for (i, &s) in s.iter().rev().enumerate() {
        sum -= s;
        if k >= 1 && n >= k + i + 1 {
            ans += pow * sum * pc.comb(n - 1 - 1 - i, k - 1);
        }
        if n >= k + 1 + i {
            ans += pow * s * pc.comb(n - 1 - i, k);
        }
        pow *= M::new(10);
    }
    println!("{}", ans);
