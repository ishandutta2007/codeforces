...
fun main(){
    solve.cases{
        val n = getint
        val m = getint
//        val forward = NewGraph(n,m,true)
//        val reversed = NewGraph(n,m,true)
        val G = Graph(n,true)
        repeat(m){
            val a = getint - 1
             val b = getint - 1
        ...
        var now = -1
        var type0 =0
        G.strongSearch({v ->ret[v] = now; if(now == 0) type0 ++},{now++})
        if(now == 0){
            put("NO")
            return@cases
        }
...
//        val d1 = forward.BFS(0)
//        val d2 = reversed.BFS(0)
//        val components = BooleanArray(n)
//        var ok = 0
//        for(i in 0 until n){
//            if(d1[i] != -1 && d2[i] != -1){
//                ok ++
...
class Graph(val n:Int, val directed:Boolean){
    ...
    inline fun everyOrientedEdge(act:(Int,Int)->Unit){
        for((i,v) in E.withIndex()){
            for(to in v){
                act(i,to)
            }
    ...
    //Long dfs is on a slightly differnt order, need the strong orietation
    ...
    inline fun DFS_Long_Exhaust_iter(dfsOrder:(Int)->Unit,newroot:(Int)->Unit = {},long:(p:Int,into:Int,root:Int)->Unit = {_,_,_ ->}){
        //Long: (from,to) i.e. (parent, child)
        var pointer:Int = -1
        val stack = IntArray(n)
        val iter = IntArray(n){-2}
        for(root in 0 until n){
            if(iter[root] == -1) continue
            newroot(root)
            pointer = 0
            stack[0] = root
            iter[root] = E[root].lastIndex
            while(pointer >= 0){
                val here = stack[pointer]
                val i = iter[here]
                if(i == -1){
                    dfsOrder(here)
                    pointer--
                ...
                val next = E[here][iter[here]]
                iter[here] --
                if(iter[next] == -2){
                    iter[next] = E[next].lastIndex
                    pointer ++
                    stack[pointer] = next
                    long(here,next,root)
                }
...
inline fun Graph.strongSearch(newitem:(root:Int)->Unit,newcomp:(v:Int)->Unit){
    ...
    var pointer2 = n-1
    DFS_Long_Exhaust_iter({exitOrder[pointer2] = it; pointer2--},{}){a,b,_ ->}
    val reverse = List(n){ mutableListOf<Int>()}
    everyOrientedEdge { a, b -> reverse[b].add(a) }
...
/*
reduce reliance on fastdeque (just use an easier version)
...
the two BFS cannot subsitute to finding strongly connected components
