...
int main(){
    ...
    int n, a[2];
    while(scanf("%d%d%d", &n, &a[0], &a[1]) != EOF){
        sum[0] = 0;
        h[0] = 0;
        for(int i=1; i<=n; ++i){
            scanf("%d", &h[i]);
        ...
        for(int i=0; i<n; ++i){
            ...
            for(int j=0; j<=sum[i]; ++j){
                checkmin(dp[i+1][j+h[i+1]][0], dp[i][j][0]);
                checkmin(dp[i+1][j][1], dp[i][j][0]+add);
                checkmin(dp[i+1][j][1], dp[i][j][1]);
                checkmin(dp[i+1][j+h[i+1]][0], dp[i][j][1]+add);
            }
        }
        int ans = intmax;
        for(int j=0; j<=sum[n]; ++j){
            if(j <= a[0] && sum[n]-j <= a[1]){
                checkmin(ans, dp[n][j][0]);
                checkmin(ans, dp[n][j][1]);
            }
        }
        if(ans == intmax) ans = -1;
        printf("%d\n", ans);
