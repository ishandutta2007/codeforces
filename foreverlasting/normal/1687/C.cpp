...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read(),m=read();
        for(res i=1;i<=n;i++)sa[i]=sa[i-1]+read();
        for(res i=1;i<=n;i++)sb[i]=sb[i-1]+read();
        for(res i=1;i<=n;i++)A[i]=sa[i]-sb[i];
        for(res i=0;i<=n+1;i++)fa[i]=i;
        for(res i=0;i<=n;i++)vector<Pair>().swap(que[i]);
        for(res i=1;i<=m;i++){
            res l=read(),r=read();
            que[l-1].emplace_back(l,r),que[r].emplace_back(l,r);
        }
        he=1,ta=0;
        for(res i=0;i<=n;i++){
            if(!A[i]){
                fa[i]=find(i+1),Q[++ta]=i;
            }
        }
        while(he<=ta){
            res x=Q[he++];
            for(auto [l,r]:que[x]){
                if(find(l-1)==l-1)continue;
                if(find(r)==r)continue;
                for(res i=find(l);i<=r;i=find(i)){
                    fa[i]=find(i+1),Q[++ta]=i;
                }
            }
        }
        res ret=0;
        for(res i=0;i<=n;i++)ret+=find(i)==i;
        puts(ret==0?"YES":"NO");
    }
}
int main(){
