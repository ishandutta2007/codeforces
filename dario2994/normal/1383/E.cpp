...
struct ModularInteger {
    ...
    LL get() { return n; }
    void fast_mod(ULL x) { // Barrett reduction.
        ULL q = (inv_mod * x) >> 64;
        n = x - q * mod;
        n -= mod * (n >= mod);
    }
...
ModularInteger operator *(const ModularInteger& A, const ModularInteger& B) {
    ModularInteger C;
    C.fast_mod(((ULL)A.n) * B.n);
    return C;
}

void operator *=(ModularInteger& A, const ModularInteger& B) {
    A.fast_mod(((ULL)A.n) * B.n);
}
...
void dfs(string S) {
    ...
    for (int i = 0; i < SZ(S); i++) {
        if (S[i] == '0' or (i > 0 and S[i-1] == '1')) {
            string T = "";
            if (i > 0 and i < SZ(S)-1) T = S.substr(0, i) + S.substr(i+1);
            else if (i == 0) T = S.substr(1);
            else if (i == SZ(S)-1) T = S.substr(0, SZ(S)-1);
            // cout << T << endl;
            // if (i == 2) cout << S << " " << T << endl;
            if (!visited.count(T)) dfs(T);
        }
    }
}

int very_slow(string S) {
   dfs(S);
   return visited.size();
}   
...
int main() {
    ...
    while (it < N) {
        int co = 0;
        while (it < N and S[it] == '1') co++, it++;
        int cz = 0;
        while (it < N and S[it] == '0') cz++, it++;
        z.push_back(cz);
        o.push_back(co);
    }
    int B = z.back();
    ...
    // cout << very_slow(S) << endl;
    
    cout << (A+1) * f[0] * (B+1) << endl;
}