...
int main() {
    ...
    vector<vector<mint>> w(n, vector<mint>(n));
    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) {
        w[i][j] = a[i]/(a[i]+a[j]);
    }
    mint res = 0;
    vector<mint> prob(1<<n, 1);
    vector<mint> cost(n);
    for (int bb = 0; bb < (1<<n); bb++) {
        for (int i = 0; i < n; i++) {
            cost[i] = 1;
            for (int j = 0; j < n; j++) {
                if ((bb&(1<<i)) and !(bb&(1<<j))) prob[bb] *= w[i][j];
                if (!(bb&(1<<j))) cost[i] *= w[i][j];
            }
        }
        ...
        for (int cc = bb; cc > 0; cc = (cc-1)&bb) {
            ...
            mint ris = prob[cc];
            int dd = bb^cc;
            for (int i = 0; i < n; i++) if (dd&(1<<i)) ris *= cost[i];
            prob[bb] -= ris;
        }
        res += __builtin_popcount(bb) * prob[bb];
    }
    cout << res << "\n";
}