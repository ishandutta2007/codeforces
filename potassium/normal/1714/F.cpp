...
int main() {
	CASE(t) {
		int n,d12,d23,d31; R(n,d12,d23,d31);
		
		bool is = true;
		int euler = d12 + d23 + d31;
		if (euler % 2 != 0)
			is = false;
		...
		if (!is) {
			...
			vector<pii> edges;
			int now = (center == 4 ? 4 : 3);
			function<void(int, int)> add = [&](int u, int len) {
				if (len >= 2) {
					++now;
					edges.emplace_back(u, now);
					add(now, len - 1);
				}
				else if (len == 1) {
					edges.emplace_back(u, center);
				}
			};
			add(1, d1);
			add(2, d2);
