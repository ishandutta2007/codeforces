// Computer Game (https://codeforces.com/contest/1598/problem/A)
...
int main() {
    ...
    while (t--) {
        ...
        vector reach(n, array{false, false});
        reach[0][0] = true;
        reach[0][1] = (g[1][0] == '0');
        rep(i, 1, n)
            rep(j, 2)
                reach[i][j] = (g[j][i] == '0') && (reach[i - 1][j] || reach[i - 1][1 - j]);

        println(reach[n - 1][1] ? "YES" : "NO");
    }
}