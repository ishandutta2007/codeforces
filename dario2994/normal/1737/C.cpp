...
void solve() {
    ...
    while (!(p[0].first == p[1].first and p[0].second == p[2].second))
        random_shuffle(all(p));

    int x0 = p[0].first, y0 = p[0].second;

    if ((x0 == 1 or x0 == n) and (y0 == 1 or y0 == n)) {
        if (x == x0 or y == y0) cout << "YES\n";
        else cout << "NO\n";
        return;
    }

    if ((x-x0) % 2 == 0) cout << "YES\n";
    else if ((y-y0) % 2 == 0) cout << "YES\n";
    else cout << "NO\n";
}
