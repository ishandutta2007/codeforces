...
int main() {
	...
	if (n % 2) {
		for (int i = 0; i < m / 2; ++i)
			num[n - 1][i * 2] = num[n - 1][i * 2 + 1] = first_free++;
		a -= m / 2;
	}
	if (m % 2) {
		for (int j = 0; j < n / 2; ++j)
			num[j * 2][m - 1] = num[j * 2 + 1][m - 1] = first_free++;
		b -= n / 2;
	}
	if (a < 0 || b < 0) err();
	for (int i = 0; i < n / 2; ++i)
	...
	debug(dump(num, 0, n - 1, 0, m - 1));
	for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) {
		if (mapping[num[i][j]]) continue;
		set <char> seen;
		for (int d = -3; d <= 3; ++d)
			for (int c = -3; c <= 3; ++c)
				if (i + d >= 0 && j + c >= 0)
					seen.insert(mapping[num[i + d][j + c]]);
		for (char c = 'a'; c <= 'z'; ++c)
			if (!seen.count(c)) {
				mapping[num[i][j]] = c;
				break;
