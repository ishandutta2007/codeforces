...
public class Main
{
	public static void main(String[]args) throws Exception
	{
		...
		int r=ni(),c=ni();
		int x=ni(),y=ni();
		a[0]=pr("*",m+2).toCharArray();
		a[n+1]=pr("*",m+2).toCharArray();
		for(int i=1; i<=n; i++)
			a[i]=("*"+ns()+"*").toCharArray();
		PriorityQueue<int[]>hola=new PriorityQueue<int[]>(new Comparator<int[]>()
				{
		...
		hola.add(new int[] {0,r,c});
		int[]te;
		int[][]val=new int[n+2][m+2];
		for(int i=0; i<=n+1; i++)
			Arrays.fill(val[i],2000_000_000);
		val[r][c]=0;
		...
		{
			te=hola.poll();
			val[te[1]+1][te[2]]=min(val[te[1]+1][te[2]],val[te[1]][te[2]]);
			val[te[1]-1][te[2]]=min(val[te[1]-1][te[2]],val[te[1]][te[2]]);
			val[te[1]][te[2]+1]=min(val[te[1]][te[2]+1],val[te[1]][te[2]]+1);
			val[te[1]][te[2]-1]=min(val[te[1]][te[2]-1],val[te[1]][te[2]]+1);
			if((!vis[te[1]+1][te[2]])&&(a[te[1]+1][te[2]]!='*')) {
				vis[te[1]+1][te[2]]=true;
				hola.add(new int[] {te[0],te[1]+1,te[2]});
			}
			if((!vis[te[1]-1][te[2]])&&(a[te[1]-1][te[2]]!='*')) {
				vis[te[1]-1][te[2]]=true;
				hola.add(new int[] {te[0],te[1]-1,te[2]});
			}
			if((!vis[te[1]][te[2]+1])&&(a[te[1]][te[2]+1]!='*')) {
				vis[te[1]][te[2]+1]=true;
				hola.add(new int[] {te[0]+1,te[1],te[2]+1});
			}
			if((!vis[te[1]][te[2]-1])&&(a[te[1]][te[2]-1]!='*')) {
				vis[te[1]][te[2]-1]=true;
				hola.add(new int[] {te[0]+1,te[1],te[2]-1});
		...
		for(int i=1; i<=n; i++)
			for(int j=1; j<=m; j++)
				if(a[i][j]!='*')
				{
					yy=j-c+(val[i][j]-j+c)/2;
					xx=c-j+(val[i][j]+j-c)/2;
					if(xx<=x&&yy<=y)
						ans++;
				}
		p(ans);
