...
static ll n,m,a[N],rn[N],ansa,ansb,cur;
static char c;
static V(ll) ad[N],rad[N];
static map<ll,ll> mp;

void dfs(ll x,ll d,ll cyc)
{
    rn[x]=cur;
    if(a[x]==1)mp[d%cyc]++;
    for(auto it:rad[x])
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            {
                cin>>c;
                if(c=='U') ad[i*m+j].PB(i*m+j-m);
                if(c=='D') ad[i*m+j].PB(i*m+j+m);
                if(c=='L') ad[i*m+j].PB(i*m+j-1);
                if(c=='R') ad[i*m+j].PB(i*m+j+1);
                rad[ad[i*m+j][0]].PB(i*m+j);
            }
        }
        REP(i,0,n*m)
        {
