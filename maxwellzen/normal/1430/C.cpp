...
void solve() {
    cin >> n;
    vi e, o;
    FOR(i, 1, n+1) if (i%2==0) e.pb(i); else o.pb(i);
    cout << 2 << nl;
    F0R(step, n-1) {
        if (e.size()==1 && o.size()==1) {
            cout << e[0] << ' ' <<  o[0] << nl;
        } else if (o.size()==0 || e.size()>0 && e.back() > o.back()) {
            cout << e.back() << ' ' << e.end()[-2] << nl;
            int res = (e.back() + e.end()[-2])/2;
            e.pop_back(); e.pop_back();
            if (res%2==0) e.pb(res); else o.pb(res);
        } else {
            cout << o.back() << ' ' << o.end()[-2] << nl;
            int res = (o.back() + o.end()[-2])/2;
