...
int main() {
    ...
    string s;
    cin >> s;
    vector<Mint> abefore(n, 0), cafter(n, 0), qbefore(n, 0), qafter(n, 0);

    for (int i = 1; i < n; ++i) {
        abefore[i] = abefore[i - 1] + (s[i - 1] == 'a');
        qbefore[i] = qbefore[i - 1] + (s[i - 1] == '?');
    }
    for (int i = n - 2; i >= 0; --i) {
        cafter[i] = cafter[i + 1] + (s[i + 1] == 'c');
        qafter[i] = qafter[i + 1] + (s[i + 1] == '?');
    }
    ...
    for (int i = 0; i < n; ++i) {
        if (s[i] == 'b') {
            ans += abefore[i] * cafter[i] * total;
            ans += qbefore[i] * cafter[i] * total / 3;
            ans += abefore[i] * qafter[i] * total / 3;
            ans += qbefore[i] * qafter[i] * total / 9;
        } else if (s[i] == '?') {
            ans += abefore[i] * cafter[i] * total / 3;
            ans += qbefore[i] * cafter[i] * total / 9;
            ans += abefore[i] * qafter[i] * total / 9;
            ans += qbefore[i] * qafter[i] * total / 27;
        }
    }
    cout << ans << '\n';
