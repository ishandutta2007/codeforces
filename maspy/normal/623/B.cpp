...
void solve() {
  ...
  VEC(ll, A, N);
  vi cand;
  for (auto&& [p, e]: factor(A[0])) cand.eb(p);
  for (auto&& [p, e]: factor(A[0] - 1)) cand.eb(p);
  for (auto&& [p, e]: factor(A[0] + 1)) cand.eb(p);
  for (auto&& [p, e]: factor(A.back())) cand.eb(p);
  for (auto&& [p, e]: factor(A.back() - 1)) cand.eb(p);
  for (auto&& [p, e]: factor(A.back() + 1)) cand.eb(p);
  UNIQUE(cand);
  const ll INF = 1LL << 60;
  ...
  ll ANS = a * N;
  auto f = [&](ll p) -> void {
    ll dp0 = 0, dp1 = INF, dp2 = INF;
    // print(p);
    for (auto&& x: A) {
      ll r = x % p;
      if (r == 0) {
        tie(dp0, dp1, dp2) = mt(dp0, min(dp0, dp1) + a, min(dp1, dp2));
      }
      elif (r == 1 || r == p - 1) {
        tie(dp0, dp1, dp2) = mt(dp0 + b, min(dp0, dp1) + a, min(dp1, dp2) + b);
      }
      else {
        tie(dp0, dp1, dp2) = mt(INF, min(dp0, dp1) + a, INF);
      }
      chmin(dp0, INF);
      chmin(dp1, INF);
      chmin(dp2, INF);
