...
int main() {
  ...
  n = strlen(s);
  dr[n][0] = dr[n][1] = true;
  FORD(i, n-1, 0) REP(l, 2) if (i+l+2 <= n) {
    int t = 1 - l;
    dr[i][l] = dr[i+l+2][t];
    if (!dr[i][l] && dr[i+l+2][l] && string(s + i, s + i+l+2) != string(s + i+l+2, s + i+2*(l+2)))
      dr[i][l] = true;
  }
  FOR(i, 5, n) REP(l, 2) if (i+l+2 <= n && dr[i][l]) {
    a.insert(string(s + i, s + i+l+2));
  }
  printf("%d\n", sz(a));
  for (auto& t : a) printf("%s\n", t.c_str());
