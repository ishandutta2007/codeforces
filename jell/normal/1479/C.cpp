#line 1 "codeforces-workspace/contest/1479/c/c.cpp"
...
#line 12 "codeforces-workspace/contest/1479/c/c.cpp"
...
void main() {
  ...
  for (auto i : range(22)) {
    for (auto j : range(i)) {
      edges.emplace_back(n - i, n - j, 1 << max(0, j - 1));
    }
  }
  ...
  for (auto p = 1 << 19, k = n - 20; R - L > 0; p >>= 1, ++k) {
    assert(p);
    assert(k < n);
    if (L + p > R) continue;
    // [L, L + p)
    edges.emplace_back(1, k, L - 1);
    L += p;
  }
