...
const int NN = 64;
using T = array<size_t, NN>;

T def;

T combine(T const& a, T const& b) {
  T r = a;
  FOR(i, NN) hash_combine(r[i], b[i]);
  return r;
}

int main(int, char**){
  ios::sync_with_stdio(0); cin.tie(0);
  srand(time(0)); FOR(i, NN) def[i] = rand()*RAND_MAX+rand();
  int n; cin >> n;
  tree t(n); t.read_graph();
  t.calceorder();
  auto v = t.makeEVec<T>();
  for(auto p : t.eorder) {
    DESTRUCT2(p, i, j_); int j = t.G[i][j_];
    v[i][j_] = def;
    vector<T> cs;
    FOR(k_, t.G[j].size()) {
      int k = t.G[j][k_];
    ...
    sort(all(cs));
    for(T hh : cs) v[i][j_] = combine(v[i][j_], hh);
  }
  set<T> S;
  FOR(i, n) if(t.G[i].size() < 4) {
    T h = def;
    vector<T> cs;
    FOR(j_, t.G[i].size()) cs.pb(v[i][j_]);
    sort(all(cs));
    for(T hh : cs) h = combine(h, hh);
    S.insert(h);
  }
  cout << S.size() << endl;
  return 0;
