...
int main(void)
{
	...
	while(T--){
		...
		ll sum = 0, slp = 0;
		ll umin = -inf, vmax = inf;
		reps(i, vec){
			if(i >= 1) sum += slp*(vec[i].fi-vec[i-1].fi);
			slp += vec[i].se;
			ll x = vec[i].fi, p = sum-m;
			if(p > 0) chmax(umin, x+p), chmin(vmax, x-p);
		}
		
		rep(i, 1, n){
			if(x[i]+p[i] >= umin && x[i]-p[i] <= vmax) ans[i] = 1;
			else ans[i] = 0;
		}
