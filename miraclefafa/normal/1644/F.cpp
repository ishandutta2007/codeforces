...
const ll N=201000;
int pr[N/5],p[N+100],tot,mu[N+100],smu[N+100];
void init() {
	mu[1]=1;p[1]=1;
	rep(i,2,N+1) {
...
int n,k,cnt[N];
ll fac[N],fnv[N],pinv[N],ret[N],p1[N],p2[N];
ll comb(int a,int b) {
	if (b<0||b>a) return 0;
...
int main() {
	scanf("%d%d",&n,&k);
	fac[0]=fac[1]=fnv[0]=fnv[1]=pinv[1]=1;
	if (n==1||k==1) {
		puts("1");
		return 0;
	}
	rep(i,2,200101) {
		...
		fac[i]=fac[i-1]*i%mod;
		fnv[i]=fnv[i-1]*pinv[i]%mod;
	}
	init();
	Poly ps(n),qs(n);
	rep(s,1,n) ps[s]=smu[(n-1)/s]*fac[s-1]%mod;
	rep(s,0,n) qs[s]=fnv[n-1-s];
	auto pr=ps*qs;
	rep(r,1,n) {
		cnt[r+1]=fnv[r-1]*pr[n-1+r]%mod;
	}
	Poly ff(n);
	rep(i,0,n) ff[i]=cnt[i+1];
	VI xx(k);
	rep(i,0,k) xx[i]=i;
	auto rr=tp.eval(ff,xx);
	rep(z,1,k+1) p1[z]=fnv[z-1]*rr[z-1]%mod;
	rep(z,0,k+1) p2[z]=powmod(mod-1,z)*fnv[z]%mod;
	rep(z,1,k+1) p2[z]=(p2[z]+p2[z-1])%mod;
	ll ans=0;
	rep(z,1,k+1) {
		ans=(ans+p1[z]*p2[k-z])%mod;
	}
	printf("%lld\n",ans);
}