...
int main() {
	CASE(t) {
		...
		int ans = b - a;
		function<void(int, int, int, bool, bool)> go = [&](int bit, int ca, int cb, bool af, bool bf) {
			if (bit == -1) {
				chkmin(ans, ca + cb - a - b + 1);
				return;
			}
			int x = a >> bit & 1;
			int y = b >> bit & 1;
			if (x == y) go(bit - 1, ca - (x == 1 && af ? 1 << bit : 0), cb - (y == 1 && af && bf ? 1 << bit : 0), af, bf);
			if (x == 0 && y == 1) {
				if (!af) go(bit - 1, ca + (1 << bit), cb, true, bf);
				go(bit - 1, ca, cb - (bf ? 1 << bit : 0), af, bf);
			}
			if (x == 1 && y == 0) {
				if (af)
					go(bit - 1, ca - (1 << bit), cb, af, bf);
				else
					go(bit - 1, ca, cb + (1 << bit), af, true);
			}
		};
		go(20, a, b, false, false);
