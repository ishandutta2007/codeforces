...
#line 2 "/home/maspy/compro/library/ds/cumsum2d.hpp"
...
void solve() {
  ...
  FOR(i, N) FOR(j, N) {
    if (i + K <= N) {
      bool ok = 1;
      FOR(k, K) if (dat[i + k][j] == '#') ok = 0;
      if (ok) { FOR(k, K) A[i + k][j]++; }
    }
    if (j + K <= N) {
      bool ok = 1;
      FOR(k, K) if (dat[i][j + k] == '#') ok = 0;
      if (ok) { FOR(k, K) A[i][j + k]++; }
    }
  }
  ll best = -1;
  pi ANS = {0, 0};
  FOR(x, N) FOR(y, N) {
    if (chmax(best, A[x][y])) ANS = {x + 1, y + 1};
  }
  print(ANS);
}
