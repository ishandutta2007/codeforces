...
int up[1001][1001];
int lefts[1001][1001];
int rights[1001][1001];
int down[1001][1001];

int topper[1001][1001];
int righter[1001][1001];
...
void solve() {
    ...
    vector<tuple<int, int, int>> inputts;
    REP(i, n) {
        REP(q, m) {
            int nexts = min({ down[i][q], lefts[i][q],up[i][q],rights[i][q] });
            if (nexts <= 0) continue;
            inputts.push_back(make_tuple(i + 1, q + 1, nexts));
            topper[i - nexts][q]++;
            topper[i + nexts + 1][q]--;
            righter[i][q - nexts]++;
            righter[i][q + nexts + 1]--;
    ...
    REP(i, n) {
        REP(q, m) {
            if (topper[i][q] == 0 && righter[i][q] == 0 && inputs[i][q] == 1) {
                cout << "-1" << endl;
                return;
            }
            topper[i + 1][q] += topper[i][q];
            righter[i][q + 1] += righter[i][q];
        }
    }
    cout << inputts.size() << endl;
    REP(i, inputts.size()){
        cout << get<0>(inputts[i]) << " " << get<1>(inputts[i]) << " " << get<2>(inputts[i]) << endl;
    }
}
#undef int
int main() {
