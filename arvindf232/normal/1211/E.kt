...
fun main(){
    solve.cases{
        ...
        fun make(max:Int):IntArray?{
            val ret = IntArray(n)
            val got = IntArray(max+1)
                ...
                if(L[i] > max){
                    ...
                    if(got[L[i]] in 0..1){
                        ...
                        if(ret[i] == 1) {
                            ...
                            if(stack.first() != L[i]) return null
                            stack.removeFirst()
                        }
                    }else{
                        return null
            ...
            if((1..max).any { got[it] != 2  }) return null
            return ret
        }
        val ans = BinarySearchLastTrue(0,n){
            make(it) != null
        }!!
        put(make(ans)!!.map{if(it == 1)'R' else if(it == 2) 'G' else 'B'}.joinToString(""))
    }
    done()
}
