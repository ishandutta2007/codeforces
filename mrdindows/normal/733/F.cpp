...
int getTo(int from, int e) {
    return a[e] == from ? b[e] : a[e];
}
vector<int> v[202020];
int tin[202020];
int tout[202020];
...
bool used[202020];
int gl[202020];
pair<int, pii> par[202020][19];
void go(int cur, int prev = 0, int curg = 0) {
    tin[cur] = ++T;
    used[cur] = true;
    gl[cur] = curg;
    for (int i = 1; i <= 18; ++i) {
        par[cur][i].first = par[par[cur][i - 1].first][i - 1].first;
        par[cur][i].second = max(par[cur][i - 1].second, par[par[cur][i - 1].first][i - 1].second);
    }
    for (int e : v[cur]) {
        int to = getTo(cur, e);
        if (!used[to]) {
...
int lca(int x, int y) {
    ...
    for (int i = 18; i >= 0; --i) {
        if (!isPar(par[x][i].first, y)) {
            x = par[x][i].first;
        }
    }
    return par[x][0].first;
}
pii getq(int to, int from) {
    int dif = gl[from] - gl[to];
    int cur = from;
    pii res = {-1, -1};
    for (int i = 0; i <= 18; ++i) {
        if (dif & 1) {
            res = max(res, par[cur][i].second);
            cur = par[cur][i].first;
        }
        dif >>= 1;
    }
...
//    FREIN("input.txt");
    ...
    FI(m) {
        ...
        int y = b[id];
        auto f = query(x, y);
        ll cur = res + w[id] - s / c[id] - f.first;
        if (cur <= ans) {
            ans = cur;
            best = id;
            toRemove = f.second;
