...
void solve() {
    ...
    vector<int> inputs;
    REP(i, n) {
        inputs.push_back(hoge.fetch());
        cnt[inputs.back()]++;
    }
    REP(i, n) {
        int tmp = inputs[i];
        if (itring[tmp] != -1) {
            dp[0][i] = dp[0][itring[tmp]] + 1;
        }
        else {
            dp[0][i] = 1;
        }
        dp[1][i] = 0;
        if (itring[tmp] == -1) {
            if (tmp > 0 and itring[tmp - 1] != -1) {
                dp[1][i] = dp[0][itring[tmp - 1]] + 1;
                if (cnt[tmp - 1] == 0) {
                    dp[1][i] = max(dp[1][i], dp[1][itring[tmp - 1]] + 1);
                }
            }
        }
        else {
            dp[1][i] = dp[1][itring[tmp]] + 1;
        }
        if (itring[tmp] != -1) {
            dp[2][i] = max(dp[2][i], dp[2][itring[tmp]] + 1);
        }
        if (tmp > 0 and itring[tmp - 1] != -1) {
            dp[2][i] = max(dp[2][i], dp[0][itring[tmp-1]] + 1);
            if (cnt[tmp - 1] == 0) {
                dp[2][i] = max(dp[2][i], dp[1][itring[tmp - 1]] + 1);
            }
        }
        cnt[tmp]--;
        itring[tmp] = i;
