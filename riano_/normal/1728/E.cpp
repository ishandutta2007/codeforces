...
//a = x*s (mod m)  s 
long long solve_modinv(long long a,long long x,long long m){
    long long g = gcd(x,m);
    x /= g; m /= g; a /= g;
    return modinv(x%m,m)*(a%m)%m;
}

int main(){
    riano_; ll ans = 0;
    ll N,M; cin >> N;
    vector<ll> d; ll sum = 0;
    rep(i,N){
    ...
    rep(i,M){
        ...
        ll m = solve_modinv(N,a,b);
        if(m*a>N){
            cout << -1 << "\n"; continue;
        }
        ll l = a*b/g;
        ll r = max(0LL,mi-m*a)/l;
        ans = res[m*a+r*l];
        if(m*a+(r+1)*l<=N){
            chmax(ans,res[m*a+(r+1)*l]);
        }
        cout << ans << "\n";
    }
    //cout << ans << endl;
