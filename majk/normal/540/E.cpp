...
void solve(istream& cin, ostream& cout) {
    ...
    set<int> R;
    for (pii s:S) { R.insert(s.x); R.insert(s.y); }
    map<int, int> P;
    vector<int> W;
    int M = 0;
    for (auto it = R.begin(); it != R.end(); ++M, ++it) { W.push_back(*it); P[*it] = M; }
    vector<int> V(M), D(M);
    for (int i = 1; i < M; ++i) D[i] = D[i-1] + W[i] - W[i-1] - 1;
    iota(V.begin(),V.end(), 0);
    for (pii &s: S) swap(V[P[s.x]], V[P[s.y]]);
    Fenwick<int> I(M, 0);
    ll ans = 0;
    for (int i = 0; i < M; ++i) {
        ans += abs(D[V[i]] - D[i]);
