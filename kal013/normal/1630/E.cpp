...
void solve(){
    ...
    int g = 0;
    {
 	...
 	vector<mint> precalc_exp(n + 1), precalc_ways(n + 1);
 	...
 	for(int i = 0; i < n; ++i){
 		...
 		if (done[cycle_len]){
 			tot += precalc_exp[cycle_len];
 			all_ways += precalc_ways[cycle_len];
 			continue;
 		}
 		...
 		precalc_ways[cycle_len] = ways;
 		precalc_exp[cycle_len] = ways * (n - expected);
