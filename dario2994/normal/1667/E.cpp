...
int main() {
    ...
    // for (int i = 0; i <= n+1; i++) CB[i] *= fact[i+2];
    ...
    for (int i = 1; i <= n; i++) {
        ...
        mint ris1 = 0;
        // for (int k = m; k <= n-2; k++) ris1 += fact[k] * ifact[k+2-i];
        // for (int t = 0; t <= n + i - 2; t++) ris1 += A[t] * B[n+i-2-t];
        ris1 = AB[n+i-2];
        ris1 *= (i-1) * fact[n-i];
        ...
        if (i == 1) {
            for (int j = i+1; j <= n-m; j++) {
                mint ris3 = 0;
                // for (int k = 0; k <= n-j; k++) ris3 += C[n-(k+2)] * B[j+k];
                // for (int t = 0; t <= n-2+j; t++) ris3 += C[t] * B[n-2+j - t];
