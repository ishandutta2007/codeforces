...
void solve() {
  int n = in;
  auto [a, b] = in.vecs<int, int>(n);
  int cnt0 = count(all(a), 0), cnt1 = count(all(a), 1);
  if (cnt0 == cnt1) {
    out(accumulate(all(b), 0LL) * 2 - *min_element(all(b)));
  } else {
    vector<int> bs[2];
    rep(i, n) bs[a[i]].push_back(b[i]);
    rep(j, 2) sort(all(bs[j]), greater{});
    ll ans = accumulate(all(b), 0LL);
    int k = min(cnt0, cnt1);
    rep(j, 2) {
      ans += accumulate(bs[j].begin(), bs[j].begin() + k, 0LL);
    }
    out(ans);
  }
}
