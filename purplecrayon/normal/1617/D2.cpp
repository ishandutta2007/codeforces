...
void solve() {
    ...
    auto query = [&](int i, int j, int k) {
        ...
        int x;
#ifdef ANAND_LOCAL
        x = (secret[i] + secret[j] + secret[k]) >= 2;
#else
        re(x);
#endif
    ...
    int u = -1, v = -1; // u is 0, v is 1

    for (int i = 0; i < n; i += 3) {
        auto q = query(i, i + 1, i + 2);
    ...
    int zero = -1, one = -1;
    {
        vector<int> t{u, u + 1, u + 2, v, v + 1, v + 2};
        auto q0 = query(u, u + 1, u + 2);
        for (int i = 1; i < sz(t); i++) {
            auto q = query(t[i], t[i + 1], t[i + 2]);
            if (q != q0) {
                if (q == 0)
                    zero = t[i + 2], one = t[i - 1];
                else
                    one = t[i + 2], zero = t[i - 1];
                break;
            }
        }
    }
    ...
    for (int i = 0; i < n; i += 3) {
        ...
        auto x = query(i, i + 1, i + 2);

        auto diff = x == 0 ? one : zero;

        auto y = query(i, i + 1, diff);
        if (y == x) {
            values[i] = x;
            values[i + 1] = x;
            values[i + 2] = query(one, zero, i + 2);
        } else {
            values[i + 2] = x;
            values[i] = query(one, zero, i);
