...
int main() {
    ...
    int r, n;
    cin >> r >> n;
    vector<pair<ll,pair<ll,ll>>> kek(n+1);
    for (int i=1;i<=n;i++)
    {
        cin >> kek[i].X >> kek[i].Y.X >> kek[i].Y.Y;
    }
    ...
    {
        ans = max(dp[i],ans);
        for (int j=i+1;j<=i+4*r and j<=n;j++)
        {
            if (abs(kek[j].Y.X-kek[i].Y.X)+abs(kek[j].Y.Y-kek[i].Y.Y)<=kek[j].X-kek[i].X)
            {
