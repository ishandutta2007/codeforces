...
template <typename T = long long, const int sz = 9 * 10000000> struct IO {
...
int main() {
    ...
    rep(_, n) {
        ...
        ll t = min(k, m);
        sz[_] = t;
        rep2(i, 1, t) a[_][i] = a[_][i - 1] + read();
        m -= t;
        while(m--) read();
    }
    // pq<pii> p;
    // rep(i, n) p.emplace(sz[i], i);
    queue<int> q;
    // int nxt = n;
    // while(p.size() > 1) {
    //     auto [_, i] = p.top();
    ...
    //     auto [__, j] = p.top();
    //     p.pop();
    //     int ma = min(k, sz[i] + sz[j]);
    //     rep(t, ma + 1) {
    //         if(t < sz[i])
    //             chmax(a[nxt][t], a[i][t]);
    //         else
    //             chmax(a[nxt][t], a[i][sz[i]] + a[j][t - sz[i]]);
    //         if(t < sz[j])
    //             chmax(a[nxt][t], a[j][t]);
    //         else
    //             chmax(a[nxt][t], a[j][sz[j]] + a[i][t - sz[j]]);
    //     }
    //     sz[nxt] = ma;
    //     q.emplace(nxt++);
    // }
    ...
    while(!q.empty()) {
        ...
        rep3(j, k, 1) {
            int t = min(j, sz[i]);
            rep2(s, 1, t) chmax(v[j], v[j - s] + a[i][s]);
        }
    }
    cout << v[k] << endl;
}