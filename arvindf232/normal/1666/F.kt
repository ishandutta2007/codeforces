...
//        val halfn = n /2
        ...
        for(i in counts.indices){
            if(counts[i] != 0){
                prod = prod modM FACT.store[counts[i]]
            }
        }

        val lastdifferent = IntArray(n){-1}
        for(i in 1 until n){
            if(L[i] != L[i-1]){
                lastdifferent[i] = i-1

            }else{
                lastdifferent[i] = lastdifferent[i-1]
            }
        }

        val have = Array(n + 1 ){IntArray(n+1)}
        have[0][0] = 1
        for(p in 0 until n){
            val key = (p == 0 || (p >= 3 && p %2 == 1 ))
            if(!key){
                for(lead in 1..n){
                    have[p+1][lead] = have[p][lead] modM (below[lead-1] - (p -1))
                }
            }else{
                val prefixsum = IntArray(n+1)
                for(lead in 0 until n) {
                    prefixsum[lead] = have[p][lead] modPlus (if(lead > 0) prefixsum[lead-1] else 0)
                }
                for(lead in 1..n){
                    val upto = lastdifferent[lead-1]
                    have[p+1][lead] = prefixsum[upto + 1]
                }
            }
        }
        put(have.last().last() modDivide prod)
