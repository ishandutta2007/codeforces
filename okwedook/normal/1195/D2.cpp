...
signed main() {
    ...
    sort(a);
    vector<int> len(n);
    for (int i = 0; i < n; ++i) len[i] = sz(to_string(a[i]));
    zet suma = 0, sumb = 0;
    vector<zet> lena(11), lenb(11);
    vector<zet> c(11);
    zet ans = 0;
    for (int i = 0; i < n; ) {
        int j = i;
        while (j < n && len[j] == len[i]) ++j;
        for (int t = i; t < j; ++t) {
            zet val = conv(a[t]);
            lena[len[i]] += val;
            lenb[len[i]] += val * 10;
            ...
            for (int l = 1; l < len[i]; ++l) {
                zet add = a[t] / p10[l];
                zet left = conv(a[t] % p10[l]);
                ans += add * mypow(zet(10), 2 * l) * c[l] + left * c[l] + lenb[l];
                add = a[t] / p10[l - 1];
                left = conv(a[t] % p10[l - 1]);
                ans += add * mypow(zet(10), 2 * l - 1) * c[l] + (left * 10) * c[l] + lena[l];
            }
        }
        ans += (lena[len[i]] + lenb[len[i]]) * c[len[i]];
        i = j;
    }
    cout << ans;
    return 0;
