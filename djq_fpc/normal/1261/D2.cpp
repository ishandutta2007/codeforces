...
namespace polynomial
{
    namespace basic
    {
        ...
        inline vector<int> mul(const vector<int> &aa,const vector<int> &bb) //aa*bb
        {
            ...
            while (ans.size()>c1) ans.pop_back();
...
poly solve(int k)
{
	poly a;a.push_back(1);
	poly b;b.push_back(1);b.push_back((m-2)%mod);b.push_back(1);
	int i;
	fz0g(i,18){
		if((1<<i)>k) break;
		if(i!=0) b=b*b;
...
//	ans.print();
	...
	for(i=0;i<len&&i<c1;i++) inc(sum,ans[i]);
	cout<<1ll*sum*qp(m,c0)%mod<<endl;
	return 0;
}