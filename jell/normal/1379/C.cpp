...
public:
    ...
    size_t size() const { return uniquely.size(); }
...
struct solver
{
    ...
    solver()
    {
        ...
        auto aso=a;
        sort(begin(aso), end(aso));
        vector<i64> racc(m+1);
        for(int i=m-1; i>=0; i--) racc[i]=racc[i+1]+aso[i];

        i64 ans=0;
        for(int i=0; i<m; i++)
        {
            const int lb=lower_bound(begin(aso), end(aso), b[i])-begin(aso);
            if(n<m-lb)
            {
                chmax(ans,racc[m-n]);
            }
