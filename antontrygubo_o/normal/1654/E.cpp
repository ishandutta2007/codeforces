...
int main()
{
    ...
    {
        vector<int> b(n);
        for (int i = 0; i<n; i++) b[i] = a[i] - d*i;
        best = max(best, maxsame(b));
    }

    int B = min(n, 200);

    vector<map<int, int>> who(n);
    for (int i = n-1; i>=0; i--)
    {
        vector<int> gopa;
        for (int d = 1; d<=B; d++)
        {
            if (i+d>=n) continue;
            if (a[i+d]%d != a[i]%d) continue;
            int step = (a[i+d]-a[i])/d;
            gopa.push_back(step);
        }
        /*cout<<i<<": "; for (auto it: gopa) cout<<it<<' ';
        cout<<endl;
        cout<<maxsame(gopa)<<endl;*/

        best = max(best, maxsame(gopa)+1);
    }
