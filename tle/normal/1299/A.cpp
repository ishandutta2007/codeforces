...
void main()
{
	...
	for(int i=1;i<=n;++i)
		ans=max(ans,pair<int,int>(a[i]&(~(t[i-1]|u[i+1])),i));
	swap(a[ans.second],a[1]);
	for(int i=1;i<=n;++i)
		cout<<a[i]<<" ";
	cout<<"\n";
}
