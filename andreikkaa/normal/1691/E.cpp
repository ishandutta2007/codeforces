...
vector<pair<int, int>> g(vector<pair<int, int>> &a) {
    vector<pair<int, int>> sm, bg;
...
bool has(const pair<int, int> &p, const vector<pair<int, int>> &a) {
    int l = 0;
    int r = sz(a);
    ...
    return l != sz(a) && a[l].x < p.y;
}

int f(const vector<pair<int, int>> &sm, const vector<pair<int, int>> &bg) {
    int ans = 0;
    for(const auto &i : sm) {
        ans += !has(i, bg);
    }
