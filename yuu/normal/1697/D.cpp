...
void solve() {
    cin >> n;
    for (int i = 1; i <= n; i++) {
        vector<pair<int, char>> current_diff;
        current_diff.clear();
        for (int c = 'a'; c <= 'z'; c++)
            if (last[c]) current_diff.emplace_back(last[c], c);
        int total_diff = diff(1, i);
        if (total_diff > current_diff.size()) {  // new char
            ans[i] = ask(i);
        } else if (current_diff.size() == 1) {
            ans[i] = current_diff.back().second;
        } else {
            current_diff | SORT;
            int low = 1, high = current_diff.size() - 1, mid, res = 0;
            while (low <= high) {
                mid = (low + high) / 2;
                int diff_size = diff(current_diff[mid].first, i);
                if (diff_size == current_diff.size() - mid) {  // not a new char
                    res = mid;
                    low = mid + 1;
                } else {
                    high = mid - 1;
