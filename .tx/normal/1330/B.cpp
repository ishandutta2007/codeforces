...
void solve() {
    ...
    vector<int> a(n);
    cin >> a;
    set<int> l, r(begin(a), end(a));
    multiset<int> ml, mr(begin(a), end(a));
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (mr.find(a[i]) == end(mr)) {
            r.erase(a[i]);
        }
        if (
            isz(l) == isz(ml) && isz(r) == isz(mr) && *begin(l) == 1 && *begin(r) == 1 &&
            *rbegin(l) == isz(l) && *rbegin(r) == isz(r) && isz(l) == i + 1 && isz(r) == n - i - 1
        ) {
