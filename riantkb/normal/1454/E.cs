        ...
        for (int _ = 0; _ < T; _++)
        {
            ...
            var incycle = new bool[n];
            {
                ...
                while (q.Any()) {
                    ...
                    {
                        if (p == u && item == v || p == v && item == u) continue;
                        if (dist[item] < dist[p] + 1) continue;
                        dist[item] = dist[p] + 1;
                        q.Enqueue(item);
                    }
                }
                ...
                while (u != now) {
                    ...
                    {
                        if (now == u && item == v || now == v && item == u) continue;
            ...
            long ans = n * (long)(n - 1);
            {
                ...
                foreach (var item in edges)
                {
                    if (incycle[item.v1] && incycle[item.v2]) continue;
                    uf.unite(item.v1, item.v2);
                }
                for (int i = 0; i < n; i++)
                {
                    if (uf.ispar(i)) {
                        ans -= uf.unionsize(i) * (long)(uf.unionsize(i) - 1) / 2;
                    }
                }
            }
            Prt(ans);
