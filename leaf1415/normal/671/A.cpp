...
int main(void)
{
	...
	rep(i, 1, n){
		...
		ans += d * 2;
		avec.push_back(Q(d-dist(ax, ay, x, y), i));
		bvec.push_back(Q(d-dist(bx, by, x, y), i));
	}
	sort(all(avec)), sort(all(bvec));
	reverse(all(avec)), reverse(all(bvec));
	
	if(n == 1) ans -= max(avec[0].first, bvec[0].first);
	else if(avec[0].second != bvec[0].second){
		ans -= max(avec[0].first + bvec[0].first, max(avec[0].first, bvec[0].first));
	}
	else{
		ans -= max(max(avec[1].first+bvec[0].first, avec[0].first+bvec[1].first),
			max(avec[0].first, bvec[0].first));
	}
	printf("%.11Lf\n", ans);
