...
#line 2 "/home/maspy/library/graph/dfstree.hpp"
...
void solve() {
  ...
  FORIN(e, G1.edges) {
    ...
    FOR(i, C.size() - 1) uf.merge(C[i], C[i + 1]);
  }

  vi V;
  vi is_cyc;
  FOR(v, N) if (uf[v] == v) V.eb(v);
  FORIN(v, V) is_cyc.eb(uf.size[v] >= 2);
  vi convert(N);
  FOR(v, N) convert[v] = LB(V, uf[v]);
