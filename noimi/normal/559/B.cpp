...
int main() {
    ...
    RollingHash S(s), T(t);
    int n = si(s);
    auto check = [&](auto &&f, int l1, int r1, int l2, int r2) -> bool {
        if(S.get(l1, r1) == T.get(l2, r2)) return true;
        if((r1 - l1) & 1) return false;
        int m1 = l1 + r1 >> 1, m2 = l2 + r2 >> 1;
        if(S.get(l1, m1) == S.get(m1, r1)) return f(f, l1, m1, l2, m2) and f(f, l1, m1, m2, r2);
        if(T.get(l2, m2) == T.get(m2, r2)) return f(f, l1, m1, l2, m2) and f(f, m1, r1, l2, m2);
        if(rnd(2))
            return (f(f, l1, m1, l2, m2) and f(f, m1, r1, m2, r2)) or (f(f, l1, m1, m2, r2) and f(f, m1, r1, l2, m2));
        else
            return (f(f, m1, r1, l2, m2) and f(f, l1, m1, m2, r2)) or (f(f, m1, r1, m2, r2) and f(f, l1, m1, l2, m2));
    };
    cout << YES[check(check, 0, n, 0, n)] << endl;
}