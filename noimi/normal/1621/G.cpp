...
int main() {
    TEST {
        ...
        zip(a);
        vi id = iota(n);
        sort(all(id), [&](int i, int j) { return (a[i] != a[j] ? a[i] < a[j] : i > j); });
        int l = 0;
        auto solve = [](vi v, bool inc = true, bool f = true) {
            int n = si(v);
            RST<mint> seg(n);
            vmint ans;
            bool start = true;
            fore(e, v) {
                mint k;
                if(inc)
                    k = seg.prod(0, e) + (f or start);
                else
                    k = seg.prod(e + 1, n) + (f or start);
                ans.eb(k);
                seg.set(e, seg[e] + k);
                start = false;
            }
        ...
        per(i, n) {
            ...
            if(ma == a[i]) {
                ...
                REV(w);
                zip(w);
                auto res = solve(w, false, false);
                REV(res);
                rep(i, si(v)) sum -= res[i] * L[v[i]];
                // sum -= f(w);
            }
        }
        OUT(sum);
