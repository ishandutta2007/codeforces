...
main() {
    INT(n, m);
    vi a(n + 1);
    rep3(i, n, n - m + 1) cin >> a[i];
    vll b(n + 1);
    b[1] = 0;
    rep2(i, 2, n) b[i] = b[i - 1] + (i - 1) * 2;
    // print(b);
    ll now = b[n];
    bool flag = true;
    rep3(i, n, 1) {
        if(b[i] > now) flag = false;
        if(i >= n - m + 1)
            now -= a[i];
        else {
            int d = now - b[i - 1];
    ...
    rep3(i, n - 1, 0) {
        ...
        sort(id.begin(), id.begin() + i, [&](int x, int y) { return a[x] < a[y]; });
    }
    rep2(i, 1, n) ans[i][i] = 'X';
    rep2(i, 1, n) {
        rep2(j, 1, n) cout << ans[i][j];
        cout << endl;
    }
}