...
const int modular_int<mod>::inverse_limit = 100300;
...
void solve(){
	...
	// trace(cnt, lo, hi);
	mint ans = fact[n] * ifact[cnt] * ifact[n - cnt];
	...
	gp_hash_table<int,int, custom_hash> res;
	...
	ans *= fact[lo.size()] * fact[hi.size()];
	for(auto e: res){
		ans *= ifact[e.second];
	}
	int l1 = lo.size(), r1 = hi.size();
