...
{   //*
    ...
    for (int i = 0; i < n; i++)
    {
        int l, r;
        scanf("%d%d", &l, &r);
        ev.push_back(make_pair(l, 0));
        ev.push_back(make_pair(r, 1));
    }
    sort(ev.begin(), ev.end());
    int bal = 0;
    for (pair<int, int> t : ev)
    {
        if (t.second == 0)
            bal++;
        else
            bal--;
        inf.push_back(make_pair(t.first, bal));
    }
    vector<int> degs;
    int it = 0;
    for (pair<int, int> t : inf)
    {
        if (t.second >= k)
        {
            if (it == 0 || inf[it - 1].second < k)
                degs.push_back(t.first);
        }
        else
        {
            if (it != 0 && inf[it - 1].second >= k)
    ...
    cout << degs.size() / 2 << "\n";
    for (int i = 0; i < degs.size(); i += 2)
        cout << degs[i] << " " << degs[i + 1] << "\n";
    return 0;
}