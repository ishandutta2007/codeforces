			...
			rep(t2,3){
				if(f){
					...
					if(cnt > k/3)
						continue;
					int flag = 1;
					rep(i, n) rep(j, n - 2) if ((t[i][j] == 'O' && t[i][j + 1] == 'O' && t[i][j + 2] == 'O') || (t[i][j] == 'X' && t[i][j + 1] == 'X' && t[i][j + 2] == 'X')) flag = 0;
					rep(i, n - 2) rep(j, n) if ((t[i][j] == 'O' && t[i + 1][j] == 'O' && t[i + 2][j] == 'O') || (t[i][j] == 'X' && t[i + 1][j] == 'X' && t[i + 2][j] == 'X')) flag = 0;
					if(flag){
