...
int main() {
    ...
    rep(i, n) {
        x += d[i];
        v.eb(x), v.eb(x + 1), v.eb(x - 1);
        a.eb(x);
    }
    UNIQUE(v);
    ll ma = -1;
    rep(i, n + 1) rep2(j, i + 1, n) { chmax(ma, a[j] - a[i] + 1); }
    if(ma == -1) {
        cout << 1 << " ";
        mint ans;
        rep(i, n) ans += abs(d[i]);
        ...
        rep(i, n) {
            ...
            rep2(x, i, n - 1) {
                rep(j, si(v) - 1) {
                    int cnt = 0, p = 0;
                    if(dp[x][j].a == 0) continue;
                    rep2(k, x, n - 1) {
                        if(k == x and a[x] > a[x + 1]) continue;
                        if(a[k] < v[j] + 1 and v[j + 1] <= a[k + 1]) {
                            cnt++;
                            rep2(t, 0, p) { dp[k][j + 1] += dp[x][j] * C(p, t) * binom(v[j + 1] - v[j] - 1 + p - t, p); }
                        } else if(a[k + 1] <= v[j + 1] and v[j + 1] < a[k]) {
                            rep2(t, 0, p) dp[k][j + 1] += dp[x][j] * C(p, t) * binom(v[j + 1] - v[j] - 1 + p - t, p);
                            p++;
                        }
                    }
            ...
            // if(i == 0) { rep(i, n + 1) cout << dp[i] << endl; }
            rep2(x, i, n) if(lb(v, a[i] + ma - 1) < si(v)) ans += dp[x][lb(v, a[i] + ma - 1)];
        }
        cout << ma << " " << ans << endl;
    }
}