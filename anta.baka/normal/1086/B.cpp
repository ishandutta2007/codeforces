...
main() {
    ...
    for(int i = 0; i < n; i++) if(deg[i] == 1) q.push_back(i), was[i] = true;
    ld ans = s;
    int tot = 0;
    for(int step = 0; tot < n - 1; step++) {
        int lst = q.back();
        int v;
        int cur = 0;
        do {
            ...
            cur++;
            tot++;
            ans = min(ans, (2 * step + 1 + (cur >= 2)) * (s / (ld)tot));
            for(int to : g[v]) if(!was[to] && --deg[to] == 1) {
                was[to] = true; q.push_back(to);
            }
        } while(v != lst);
        //
    }
