...
int main() {
	...
	vector<vector<vector<ll>>> dp(25, vector<vector<ll>>(2));
	vector<vector<vector<bool>>> calc(25, vector<vector<bool>>(2, vector<bool>(n + 7, 0)));

	vector<ll> fact(n + 7, 1);
	for (int i = 2; i <= n; i++)
		fact[i] = (fact[i - 1] * i) % MOD107;

	vector<vector<ll>> pwcnt(25, vector<ll>(2, 0));
	pwcnt[0][0] = n;
	pwcnt[0][1] = n / 3;
	for (int i = 1; i < 25; i++) {
		for (int j = 0; j < 2; j++) {
			pwcnt[i][j] = pwcnt[i - 1][j] / 2;
		}
	}

	for (int i = 0; i < 25; i++)
		for (int j = 0; j < 2; j++)
			dp[i][j].resize(pwcnt[i][j] + 7, 0);

	function<ll(int, int, int)> solve = [&](int p2, int p3, int f) {
		if (p2 == 0 && p3 == 0) return fact[n - f];
		if (calc[p2][p3][f]) return dp[p2][p3][f];
		calc[p2][p3][f] = true;

		ll& ans = dp[p2][p3][f];
		ans = 0;

		if (p2) ans += solve(p2 - 1, p3, f + 1) * (pwcnt[p2 - 1][p3] - pwcnt[p2][p3]);
		if (p3) ans += solve(p2, p3 - 1, f + 1) * (pwcnt[p2][p3 - 1] - pwcnt[p2][p3]);
		if (pwcnt[p2][p3] - f) ans += solve(p2, p3, f + 1) * (pwcnt[p2][p3] - f);
