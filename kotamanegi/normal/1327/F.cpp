...
public:
    ...
    u64 value = 0;
    modint(ll a, ll b) : value(((a% b) + 2 * b) % b) {
...
vector<pair<int, int>> fails[600000];

void solve() {
    int n, k, m;
    scanf("%lld%lld%lld", &n, &k, &m);
    auto segUse = [](int a, int b) {
        return (a & b);
    };
    auto segMerge = [](int a, int b) {
        return (a | b);
    };
    LazySegmentTree<int> seg(n, segUse, segMerge, segMerge, 0, 0);
    vector<tuple<int, int, int>> inputs;
    REP(i, m) {
        int a, b, c;
        scanf("%lld%lld%lld", &a, &b, &c);
        a--;
        fails[b].push_back(mp(a, c));
    ...
    vector<int> data;
    REP(i, n) {
        data.push_back(seg.query(i, i + 1));
    }
    const int mod = 998244353LL;
    modint<mod> ans(1);
    REP(i, k) {
        vector<modint<mod>> gogo(n + 1, modint<mod>(0));
        gogo[0] = modint<mod>(1);
        int left = 0;
        modint<mod> val = modint<mod>(1);
        for (int q = 1; q <= n; ++q) {
            if ((data[q - 1] & (1 << i)) == 0) {
                gogo[q] = val;
                val += gogo[q];
            ...
            REP(j, fails[q].size()) {
                if (!((1 << i) & fails[q][j].second)) {
                    while (left < fails[q][j].first + 1) {
                        val -= gogo[left];
                        left++;
                    }
                }
