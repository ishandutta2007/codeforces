...
fun main(){
//    just dei (0).previous(5)
    solve.cases{
        val n = getint
        val pts = mutableListOf<Point>()
//        val ID = mutableMapOf<Point,Int>()
        for(i in 0 until n){
            pts.add(Point(getint, getint))
        }
//        val answers = mutableMapOf<Point,Long>()
        val base = (n-1).choose4()
        var overall =0L
        for(a in pts.toList()){
            var total = base
            Point.origin = a
            ...
            for(left in 0 until n-1){
                while(right != left.previous(n-1) && a.makeClockwiseTurn(pts[left],pts[right.next(n-1)])){
                    right = right.next(n-1)
                }
                val interiorPoints = left.distanceTo(right,n-1)
                val deduct = interiorPoints.choose3()
//                left dei interiorPoints
                total -= deduct
            }
            pts.add(a)
//            a.ans = total
...
/*
iff ahs trinagle enclosure ( almost failed because triangle cannot be faster, does not mean 4 sided cannot be faster by reducing to triangle )


transformed by negations
did totally missed that
don't have good sortings
read my own documentations and sortings
 */