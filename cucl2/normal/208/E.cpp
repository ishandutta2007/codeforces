...
int ceng[100007];
vector<int> sons[100007];
vector<int> cg[100007];
int bg[100007],en[100007],bgbg=0;
int fa[27][100007];
int n;

void dfs(int now,int cegn){
	ceng[now]=cegn;
	bg[now]=bgbg;
	bgbg++;
	cg[cegn].pb(bg[now]);
...
int main(){
	...
	while(m--){
		...
		int c=ceng[a]+b;
		fio<<lower_bound(ALL(cg[c]),en[a])-lower_bound(ALL(cg[c]),bg[a])-1<<(m==0?'\n':' ');
	}
	return 0;
}