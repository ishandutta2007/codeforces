...
struct Fraction {
	ll n,d;
	Fraction(ll x, ll y) : n(x), d(y) {
		ll gcd = __gcd(abs(n), abs(d));
		n /= gcd;
		d /= gcd;
	}
...
long double solve(Fraction c, Fraction m, Fraction p, Fraction v) {
	long double ans = 1;
	for (int times = 0; times < 2; times++) {
		if (c.n > 0) {
			Fraction toc = c - v;
			Fraction d = v;
			if (toc.n <= 0) { toc = ZERO; d = c; }
			Fraction tom = (m.n == 0 ? ZERO : m + d.half());
			Fraction top = ONE - toc - tom;
			ans += (long double)c * solve(toc, tom, top, v);
		}
		swap(c, m);
	}
	return ans;
...
int main() {
	const ll SCALING = 10000;
	CASE(t) {
		long double tc,tm,tp,tv; R(tc,tm,tp,tv);
		Fraction c(round(tc * SCALING), SCALING);
		Fraction m(round(tm * SCALING), SCALING);
		Fraction p(round(tp * SCALING), SCALING);
		Fraction v(round(tv * SCALING), SCALING);

		W((long double)solve(c, m, p, v));
	}
