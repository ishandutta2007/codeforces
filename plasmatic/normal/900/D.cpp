...
int main() {
    ...
    vector<int> pr;
    int cy = Y;
    for (int i = 2; i * i <= cy; i++) {
        while (cy % i == 0) {
            if (pr.empty() || pr.back() != i) pr.pb(i);
            cy /= i;
        }
    }
    if (cy > 1) pr.pb(cy);
    ...
    MI ans = 0;
    function<void(ll, int, int)> rec = [&] (ll curv, int mul, int idx) {
        if (idx == SZ(pr)) {
            assert(curv <= Y);
            ans += mul * MI::pow(2, Y/curv-1);
            return;
        }
        if (curv * pr[idx] <= Y) rec(curv*pr[idx], mul*-1, idx+1);
        rec(curv, mul, idx+1);
    };
    rec(1, 1, 0);
