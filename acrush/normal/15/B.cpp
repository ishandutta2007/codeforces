...
int main()
{
	...
	for (scanf("%d",&testcase);testcase>0;testcase--)
	{
		...
		int64 x2=nextInt()-1;
		int64 y2=nextInt()-1;
		if (x1>x2) x1=n-1-x1,x2=n-1-x2;
		if (y1>y2) y1=m-1-y1,y2=m-1-y2;
		int64 sx1=0,sy1=0,tx1=n-1-(x2-x1),ty1=m-1-(y2-y1);
		int64 sx2=sx1+(x2-x1),sy2=sy1+(y2-y1),tx2=tx1+(x2-x1),ty2=ty1+(y2-y1);
		int64 s1=(tx1-sx1+1)*(ty1-sy1+1);
		int64 s2=(tx2-sx2+1)*(ty2-sy2+1);
		int64 s12=0;
		if (sx1<=tx2 && sx2<=tx1 && sy1<=ty2 && sy2<=ty1)
			s12=(min(tx1,tx2)-max(sx1,sx2)+1)*(min(ty1,ty2)-max(sy1,sy2)+1);
		int64 s=s1+s2-s12;
		int64 R=n*m-s;
//		printf("%lld\n",R);
		printf("%I64d\n",R);
