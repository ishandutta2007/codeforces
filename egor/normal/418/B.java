...
class TaskB {
    public void solve(int testNumber, InputReader in, OutputWriter out) {
		...
		long monitorCost = in.readInt();
		int[] cost = new int[count];
		int[] required = new int[count];
		int[] mask = new int[count];
		for (int i = 0; i < count; i++) {
			cost[i] = in.readInt();
			required[i] = in.readInt();
			int canSolve = in.readInt();
			for (int j = 0; j < canSolve; j++)
				mask[i] += 1 << (in.readInt() - 1);
		}
		int[] order = ArrayUtils.order(required);
		ArrayUtils.reverse(order);
		long[] answer = new long[1 << taskCount];
		Arrays.fill(answer, Long.MAX_VALUE / 2);
		for (int i : order) {
			answer[mask[i]] = Math.min(answer[mask[i]], cost[i] + required[i] * monitorCost);
			for (int j = 1; j < answer.length; j++)
				answer[j | mask[i]] = Math.min(answer[j | mask[i]], answer[j] + cost[i]);
		}
		if (answer[answer.length - 1] == Long.MAX_VALUE / 2)
			out.printLine(-1);
		else
