...
void solve(int tc_num = -1) {
	...
	int sisa = n - k;
	suniq = s;
	suniq.erase(unique(ALL(suniq)), suniq.end());
	suniq.erase(suniq.begin(), suniq.begin() + 1);
	suniq += 'z' + 1;

	LOG("asu");
	LOG(cnt[suniq[0]], sisa, suniq[0]);
	if (cnt[suniq[0]] == sisa) {
		FOR (i, 0, (sisa + k - 1) / k) {
			cout << suniq[0];
		}
	...
	auto iter = suniq.begin();
	while (iter < suniq.end()) {
		if (cnt[*iter] == k) {
			sisa -= k;
			cout << *iter;
		} else {
