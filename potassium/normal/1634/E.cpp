...
int main() {
	...
	vector<map<int, vector<int>>> pos(m);
	map<int, int> last;
	vector<vector<pii>> g(m + 1);
	auto get = [&](int v) {
		auto it = last.find(v);
		if (it == last.end()) return -1;
		return it->second;
	};

	const int sumN = accumulate(n.begin(), n.end(), 0);
	vector<int> val_list;
	val_list.reserve(sumN / 2);

	for (int i = 0; i < m; i++) {
		for (int j = 0; j < n[i]; j++) {
			pos[i][a[i][j]].emplace_back(j);

			int u = get(a[i][j]);
			if (u == -1)
				last[a[i][j]] = i;
			else {
				const int id = (int)val_list.size();
				val_list.emplace_back(a[i][j]);
	...
	assert((int)val_list.size() == sumN / 2);
	vector<bool> dead(sumN / 2, false);

	vector<string> ans(m);
	for (int i = 0; i < m; i++)
		ans[i] = string(n[i], '_');
	
	auto put = [&](int u, int val, char side) {
		assert(!pos[u][val].empty());
		int p = pos[u][val].back();
		pos[u][val].pop_back();
	...
	function<void(int)> dfs = [&](int u) {
		while (!g[u].empty() && dead[g[u].back().second])
			g[u].pop_back();
