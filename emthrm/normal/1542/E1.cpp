...
int main() {
  ...
  int n, mod; cin >> n >> mod;
  ModInt::set_mod(mod);
  vector inv(n, vector((n - 1) * (n - 2) / 2 + 1, ModInt(0)));
  inv[0][0] = 1;
  if (n > 1) inv[1][0] = 1;
  FOR(i, 2, n) REP(j, i * (i - 1) / 2 + 1) {
    inv[i][j] += inv[i - 1][j];
    if (j > 0) inv[i][j] += inv[i][j - 1];
    if (j >= i) inv[i][j] -= inv[i - 1][j - i];
  }
  // REP(i, n) REP(j, (n - 1) * (n - 2) / 2 + 1) cout << inv[i][j] << " \n"[j == (n - 1) * (n - 2) / 2];
  auto sum = inv;
  REP(i, n) FOR(j, 1, (n - 1) * (n - 2) / 2 + 1) sum[i][j] += sum[i][j - 1];
  // REP(i, n) REP(j, (n - 1) * (n - 2) / 2 + 1) cout << sum[i][j] << " \n"[j == (n - 1) * (n - 2) / 2];
  ModInt ans = 0;
  REP(f, n) {
    int m = n - f;
    ModInt pat = 0;
    REP(i, m) FOR(j, i + 1, m) {
      int diff = j - i;
      FOR(k, diff + 1, (m - 1) * (m - 2) / 2 + 1) pat += inv[m - 1][k] * sum[m - 1][k - (diff + 1)];
    }
    REP(i, f) pat *= n - i;
    ans += pat;
  }
  ...
  // vector<int> ord(n);
  // iota(ALL(ord), 0);
  // vector<int> pat(n * (n - 1) / 2 + 1, 0);
  // do {
  //   int inv = 0;
  //   REP(i, n) FOR(j, i + 1, n) inv += ord[i] > ord[j];
  //   ++pat[inv];
  // } while (next_permutation(ALL(ord)));
  // REP(i, n * (n - 1) / 2 + 1) cout << pat[i] << " \n"[i == n * (n - 1) / 2];
}