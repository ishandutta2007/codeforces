...
int main() {
	...
	while (t--) {
		string s; R(s);
		bool isWS = 1, isAD = 1;
		int minv = 0, maxv = 0, cur = 0;
		vector<int> maxpos, minpos; maxpos.pb(0); minpos.pb(0);
		for (int i = 0; i < s.length(); i++) {
			...
			if (s[i] == 'S') {
				...
				if (cur < minv) {
					...
					minpos.pb(i);
					minv = cur;
				} else if (cur == minv) minpos.pb(i);
			}
		}
		auto it = upper_bound(minpos.begin(),minpos.end(),maxpos.front());
		if (it != minpos.end() && *it < maxpos.back()) isWS = 0;
		it = upper_bound(maxpos.begin(),maxpos.end(),minpos.front());
		if (it != maxpos.end() && *it < minpos.back()) isWS = 0;
		if (maxv-minv < 2) isWS = 0;
		
		ll WS = maxv-minv+1;
		
		
		minv = 0, maxv = 0, cur = 0;
		maxpos.clear(), minpos.clear(); maxpos.pb(0); minpos.pb(0);
		for (int i = 0; i < s.length(); i++) {
			if (s[i] == 'A') {
				cur++;
				if (cur > maxv) {
		...
		it = upper_bound(minpos.begin(),minpos.end(),maxpos.front());
		if (it != minpos.end() && *it < maxpos.back()) isAD = 0;
		it = upper_bound(maxpos.begin(),maxpos.end(),minpos.front());
		if (it != maxpos.end() && *it < minpos.back()) isAD = 0;
		if (maxv-minv < 2) isAD = 0;
		
		ll AD = maxv-minv+1;
		
		ll ans = WS*AD;
		//W(WS,AD,ans,isWS,isAD);
		if (isWS) ans = min(ans,(WS-1)*AD);
		if (isAD) ans = min(ans,WS*(AD-1));
