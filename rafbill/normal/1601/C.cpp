...
struct node {
  ...
  void combine(node const& a, node const& b) {
    ...
    mi = min(a.value + b.mi, a.mi - b.value);
  }
};
...
void solve(){
  ...
  vi XS(n+m);
  for(li i = 0; i < (li)(n); ++i) XS[i]=A[i];
  for(li i = 0; i < (li)(m); ++i) XS[n+i]=B[i];

  pdqsort(begin(XS), end(XS));

  vi YS; YS.reserve(n+m);

  for(li i = 0; i < (li)(n+m); ++i) {
    if(i==0 || XS[i]!=XS[i-1]) {
  ...
  hash_map<li,li> unY;
  for(li i = 0; i < (li)(YS.size()); ++i) unY[YS[i]]=i;

  for(li i = 0; i < (li)(n); ++i) A[i]=unY[A[i]];
  for(li i = 0; i < (li)(m); ++i) B[i]=unY[B[i]];

  pdqsort(begin(B), end(B));

  int ny = YS.size();

  st S(n);
  for(li i = 0; i < (li)(n); ++i) S.A[S.n+i].set(1);
  S.build();

  vvi atA(ny), atB(ny);
  for(li i = 0; i < (li)(n); ++i) atA[A[i]].push_back(i);
  for(li i = 0; i < (li)(m); ++i) atB[B[i]].push_back(i);
  ...
  for(li i = 0; i < (li)(ny); ++i) {
    li cnt = atB[i].size();
    if(cnt) {
      for(int j : atA[i]) { S.set_point(j,0); }
      node v = S.A[1];
      ans += cnt * ((v.mi+(n-atA[i].size()))/2);
    }
    for(int j : atA[i]) { S.set_point(j,-1); }
  }
  cout << ans << endl;
}
