...
#endif
using Pair = array<array<ll, 2>, 2>;
const Pair neu = {array<ll, 2>{-inf, -inf}, array<ll, 2>{-inf, -inf}};
Pair init(bool p1, bool p2) {
	Pair ans = neu;
	ans[p1][p2] = ans[p2][p1] = p1 || p2;
	rans;
}
Pair merge(Pair a, Pair b) {
	Pair ans = neu;
	REP(p0, 2) REP(p1, 2) REP(p2, 2) REP(p3, 2) REP(r1, 2) REP(r2, 2) REP(r3, 2) {
		int so_far = a[p0][p2] + b[p1][p3];
		int q1 = r1 ? p0 : p1;
		int q2 = r1 ? p1 : p0;
		int q3 = r2 ? p2 : p3;
		int q4 = r3 ? q2 : q3;
		int now = (p0 || p1) + (p2 || p3) + (q2 || q3);
		maxi(ans[q1][q4], so_far + now);
	}
...
int main() {
	...
	vector <Pair> mer;
	REP(i, (1 << n) / 2) mer.push_back(init(fav[2 * i], fav[2 * i + 1]));
	while (mer.size() > 1u) {
		debug(imie(mer));
		vector <Pair> mer2;
		REP(i, mer.size() / 2)
			mer2.push_back(merge(mer[2 * i], mer[2 * i + 1]));
		swap(mer, mer2);
	}
	debug(imie(mer));
	Pair last = mer[0];
	int ans = 0;
	REP(i, 2) REP(j, 2) maxi(ans, last[i][j] + (i || j));
	printf("%d\n", ans);
}