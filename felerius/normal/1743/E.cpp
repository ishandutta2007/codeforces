// FTL (https://codeforces.com/contest/1743/problem/E)
...
int main() {
    int p1, p2, h, s; ll t1, t2; read(p1, t1, p2, t2, h, s);
    vector dp(h + 1, INFL);
    dp[0] = 0;
    rep(_, 2) {
        rep(c1, 1, h + 1) {
            ...
            int d = int(min(ll(h), c1 * p1 + c2 * p2 - (c1 + c2 - 1) * s));
            rep(i, h)
                dp[min(h, i + d)] = min(dp[min(h, i + d)], dp[i] + c1 * t1);
        }
    ...
    rep(i, h) {
        int rem = h - i;
        ll t = dp[i];
        array next{t + t1, t + t2};
        while (rem > 0) {
            if (next[0] <= next[1])
                t = next[0], rem -= p1 - s, next[0] += t1;
            else
                t = next[1], rem -= p2 - s, next[1] += t2;
        }
