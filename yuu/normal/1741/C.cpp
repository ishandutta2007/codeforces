...
void solve() {
    ...
    int ans = n;
    for (int into = 1; into <= n; into++) {
        int seg_sum = sum / into;
        if (sum != seg_sum * into) continue;
        int thick = 0;
        for (int i = 1, current = 0, length = 0; i <= n; i++) {
            current += a[i];
            length++;
            if (current == seg_sum) {
                thick = max(thick, length);
                length = current = 0;
            } else if (current > seg_sum) {
                thick = n + 1;
                break;
            }
        }
