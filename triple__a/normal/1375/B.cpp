...
const int maxn=507;
int n,m,t;
int a[maxn][maxn],target[maxn][maxn];
...
signed main(){
    ...
    while (t--){
        ...
        rep(i,n) rep(j,m) target[i][j]=4;
        rep(i,n) target[i][0]--, target[i][m-1]--;
        rep(j,m) target[0][j]--, target[n-1][j]--;
        rep(i,n) rep(j,m) if(a[i][j]>target[i][j]){
            cout<<"NO\n";
            goto cont;
        }
        cout<<"YES\n";
        rep(i,n){
            rep(j,m) cout<<target[i][j]<<" ";
            cout<<"\n";
        }
        cont:;
    }
