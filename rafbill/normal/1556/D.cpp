...
struct union_find {
  ...
  union_find(li n = 0) : A(n), P(n,0), PR(n,2) {
  ...
  void unite(li a, li b, li pab) {
    li ra = find(a); li rb = find(b);
    if(a != b) {
      A[ra] = rb;
      P[ra] = P[a]^P[b]^pab;
      if(PR[ra] != 2) PR[rb] = P[ra] ^ PR[ra];
    }
  }

  void setPR(li a, li p) {
    li ra = find(a);
    PR[ra] = p^P[a];
  }
    ...
    auto test = [&](li i, li j){
      ...
      for(li k = 0; k < (li)(50); ++k) {
        ...
        if(A&m) {
          ...
          uf.unite(i*50+k,j*50+k,1);
        }
      }
    };

    for(li i = 0; i < (li)(n-1); ++i) test(i,i+1);
    test(0,2);
