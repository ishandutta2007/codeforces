...
int main(){
  ...
  segtree<ll> ma(sum,-inf,[](ll a,ll b){return max(a,b);}),mi(sum,inf,[](ll a,ll b){return min(a,b);});
  // debug(sum);
  while(K--){
    cin>>A>>B;
    ll ans1=ma.queli(0,A),ans2=mi.queli(0,A);
    // debug(ans1,ans2);
    D=-sum[B]+sum[--A];
    // debug(D);
    chmax(ans1,ma.queli(B+1,N+1)+D);
    chmin(ans2,mi.queli(B+1,N+1)+D);
    // debug(ans1,ans2);
    assert(ans1>=ans2);
    print(ans1-ans2+1);
  }
