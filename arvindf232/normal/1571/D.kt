...
class answers(val A:List<IntArray>,val myA:Int,val myB:Int){
	val n = A.size
	val rowSum = IntArray(n)
	val columnSum = IntArray(n)
	...
	fun resultfor(a:Int,b:Int):Int{
		val rank = if(a == myA) 0 else 1  + if(myB == b) 0 else 1
		val correct = A[a][b]
		val partial = rowSum[a] + columnSum[b] - correct
		if(rank == 0) return 1
		else if(rank == 1) return correct + 1
		else return partial + 1
	}
	fun worstCase():Int{
		var bad = 0
		for(i in 0 until n){
			for(j in 0 until n){
				if( i == j) continue
				val result = resultfor(i,j)
				bad = maxOf(result,bad)
			}
		}
		return bad
	}
