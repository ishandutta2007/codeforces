...
int main() {
    ...
    int total_up = 0;
    vi up_on_path(n, 0);
    for (int i : range(tr.preorder.begin() + 1, tr.preorder.end())) {
        int p = tr.parent[i];

        bool up = dir[{i, p}];
        total_up += up;
        up_on_path[i] = up_on_path[p] + up;
    }

    vi score(n);
    for (int i = 0; i < n; i++) {
        int down_on_path = tr.depth[i] - up_on_path[i];
        score[i] = (total_up - up_on_path[i]) + down_on_path;
    }

    int best = *min_element(all(score));
    ps(best);
    for (int i = 0; i < n; i++) {
        if (score[i] == best)
            ps(i + 1);
