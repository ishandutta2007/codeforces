...
inline void solve(ld tt) {
	...
	while (r - l > 1) {
		...
		for (i = 50; i >= 0; --i) {
			int kek = suf_sm[i] - u - 2 * min(pr_mn[i], m);//sum unused - ends = sum - sum used - ends
			u += min(pr_mn[i], m);
			if (kek > 0) {
				l = m;
				break;
