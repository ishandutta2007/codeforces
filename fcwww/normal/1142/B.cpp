...
#undef N
#define N 200050
int a[N],ans[N],n,m,q,p[N],nw[N],f[20][N];
int c[N],nxt[N];
inline void fix(int x) {for(;x<=m;x+=x&(-x))c[x]++;}
...
int main() {
	...
	sort(b+1,b+q+1);
	p[n+1]=p[1];for(i=1;i<=n;i++) nxt[p[i]]=p[i+1];
	for(i=m;i;i--) {
		f[0][i]=nw[nxt[a[i]]];
		for(j=1;j<20;j++) {
			f[j][i]=f[j-1][f[j-1][i]];
		...
		nw[a[i]]=i;
		int t=n-1,x=i;
		for(j=0;j<20;j++) if(t&(1<<j)) x=f[j][x];
		if(x) fix(x);
		for(;k<=q&&b[k].l==i;k++) {
			ans[b[k].id]=inq(b[k].r)>0;
		}
	}
