...
public class M {
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean[][] conn = new boolean[s.length][s.length];
        for (int n = 0; n < N; n++) {
            for (int m = 0; m < N; m++) {
                if (con[n][m] == 1) {
                    conn[idx[n]][idx[m]] = true;
                }
            }
        }
        char[][][] map = new char[s.length * 2][40][40];
        for (int n = 0; n < s.length; n++) {
            for (int m = 0; m < 40; m++) {
                Arrays.fill(map[n * 2][m], '.');
                Arrays.fill(map[n * 2 + 1][m], '#');
            }
            for (int m = 0; m < s[n].length; m++) {
                map[n * 2][39][m * 4] = (char) (s[n][m] + '1');
            }
        }
        for (int n = 0; n < s.length; n++) {
