...
vector<vector<int>> g;
vector<Mint> dp0, dp1, dpn;

void dfs(int now, int par) {
	dp0[now] = dp1[now] = dpn[now] = 1;
	for (auto& i : g[now]) {
		if (i != par) {
			dfs(i, now);
			dp0[now] *= ((dp0[i] + dp1[i] - dpn[i]) + (dp0[i] + dp1[i]));
			dp1[now] *= ((dp0[i] + dp1[i] - dpn[i]) + (dp0[i]));
			dpn[now] *= ((dp0[i] + dp1[i] - dpn[i]));
		}
	}
}
