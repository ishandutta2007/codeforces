...
//    println(ans)
    val n = io.int()
    val a = ArrayList<String>(io.strs(2*n-2))
    fun standardize(cands: ArrayList<String>): ArrayList<ArrayList<String>> {
        val res = ArrayList<ArrayList<String>>()
        for (i in 0 until n) {
            res.add(ArrayList<String>())
        }
        for (cand in cands) res[cand.length].add(cand)
        for (i in 0 until n) res[i].sort()
        return res
    }
    val desired = standardize(a)
    val ans = ArrayList<String>()
    fun check(s: String) {
        val cands = ArrayList<String>()
        for (len in 1..n-1) {
            cands.add(s.substring(0,len))
            cands.add(s.substring(n-len,n))
        }
        val used = BooleanArray(n)
        if (standardize(cands) == desired) {
            for (s in a) {
                if (s.equals(cands[2*s.length-2]) && !used[s.length]) {
                    io.print('P')
                    used[s.length] = true
                } else io.print('S')
            }
    ...
    for (i in 0..1) for (j in 0..1) {
        check(desired[1][i]+desired[n-1][j])
    }
    println(ans[0])
}
