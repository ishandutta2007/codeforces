...
int constexpr MAXN=100000;
struct info{
	unsigned deg,sumi;
};
std::array<info,MAXN> d;
std::array<std::array<unsigned,3>,MAXN-2> tria; // (ngle
std::array<unsigned,MAXN> pending /* vertices currently adjacent to 1 tria */,nxt;
std::array<std::pair<unsigned,std::array<unsigned,3>> /* {tria index, tria where [0] = added node}*/,MAXN-2> q;

int main(){
	io::input.reset(new io::InputFile(stdin, false));
	io::output.reset(new io::OutputFile());

	int ntest;io::read(ntest);
	while(ntest--){
		int n;io::read(n);
		for(int i=0;i<n;++i)d[i]={0,0};
		for(int i=0;i<n-2;++i){
			for(unsigned& x:tria[i]){
				io::read(x);
				--x;
				d[x].deg+=1;
				d[x].sumi+=i;
		...
		for(int z=0;z<n-3;++z){
			assert(pendtop!=begin(pending));
			int vertex=*--pendtop;
			assert(d[vertex].deg==1);
			unsigned triaind=d[vertex].sumi;

			q[z]={triaind,tria[triaind]};
			auto iter=std::find(begin(q[z].second),end(q[z].second),vertex);
			assert(iter!=end(q[z].second));
			std::swap(*iter,q[z].second[0]);

			std::for_each(1+begin(q[z].second),end(q[z].second),[&](int x){
				d[x].sumi-=triaind;
				--d[x].deg;
				if(1==d[x].deg){
					*pendtop++=x;
		...
		assert(pendtop-begin(pending)==3);
		--pendtop;
		assert(d[*pendtop].deg==1);
		unsigned triaind=q[n-3].first=d[*pendtop].sumi;

		nxt[tria[triaind][0]]=tria[triaind][1];
		nxt[tria[triaind][1]]=tria[triaind][2];
		nxt[tria[triaind][2]]=tria[triaind][0];

		std::for_each(q.rend()-(n-3),q.rend(),[&](auto zz){
			auto& [triaind,triax]=zz;
			auto& [a,b,c]=triax;
			if(nxt[b]!=c)std::swap(b,c);
			assert(nxt[b]==c);
			nxt[b]=a;nxt[a]=c;
		});

		int x=0;
		for(int z=0;z<n;++z,x=nxt[x])
			io::write(x+1,' ');
		assert(x==0);
		io::write('\n');

		std::for_each(begin(q),n-2+begin(q),[](auto x){ io::write(x.first+1,' '); });
		io::write('\n');
	}
}