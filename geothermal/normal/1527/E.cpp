...
template<class T, int SZ> struct LazySeg {
	...
	T query(int lo, int hi, int ind=1, int L=0, int R=SZ-1) {
		push(ind,L,R); if (lo > R || L > hi) return 1e18;
...
void solve() {
    ...
    FOR(i, 1, N+1) {
        ...
        X--;
        FOR(j, 1, K) {
            dp[j].upd(i, i, dp[j-1].query(0, i-1));
            //cout << dp[j].query(i, i) << " ";
        }
        //cout << nl;
        if (lst[X] >= 0) {
            F0R(j, K) {
                dp[j].upd(0, lst[X], i-lst[X]);
            }
        }
        lst[X] = i;
        /*FOR(j, 1, K+1) {
            dp[j].update(i, i, dp[j-1].query(0, i-1));
            cout << dp[j].query(i, i) << " ";
        }FOR(j, 1, K+1) {
            dp[j].update(i, i, dp[j-1].query(0, i-1));
            cout << dp[j].query(i, i) << " ";
