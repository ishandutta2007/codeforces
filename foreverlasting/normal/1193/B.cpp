...
namespace MAIN{
    ...
    void dfs(res x){
        for(auto tox:G[x]){
            dfs(tox);
            if(dp[x].size()<dp[tox].size())swap(dp[x],dp[tox]);
            for(auto y:dp[tox])dp[x][y.fi]+=y.se;
            dp[tox].clear();
        }
        if(!w[x])return;
        dp[x][d[x]]+=w[x];
        for(auto p=dp[x].upper_bound(d[x]);p!=dp[x].end();){
            if(p->se>w[x]){p->se-=w[x];break;}
            w[x]-=p->se,dp[x].erase(p++);
        }
    }
    inline void MAIN(){
