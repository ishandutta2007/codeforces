...
Sparse sparse;
vector<int>psa(500005);
vector<vector<int>>ind(1000005); int extra = 500001;
string s;
int n;
vector<int>arr(500005);
int ans = 0;
int f(int l, int r){//all one indexed so change from zero indexed values in suffix
	//array into one indexed values     and also [l,r] btw
	int low = l; int high = r+1; int mid = (low+high)/2;
	if(s[low-1]==')')return 0;
	int sum = psa[l-1];
	while(low+1<high){
		if(sparse.query(l,mid)<sum)high = mid;
		else low = mid;
		mid = (low+high)/2;
	}
	//cerr << "nae\n";
	int left = lower_bound(ind[sum+extra].begin(),ind[sum+extra].end(),l)-ind[sum+extra].begin();
	int right = upper_bound(ind[sum+extra].begin(),ind[sum+extra].end(),low)-ind[sum+extra].begin()-1;
	return right-left+1;
}
signed main(){
