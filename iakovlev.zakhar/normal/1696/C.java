...
//	boolean[] b = new boolean[MAX];
	...
	public int dfs(int v, int p) {
		...
		return 2 + Math.min(min[0], min[1]);
	}
	
	public ArrayList<Pair> toList(int[] a, int m) {
		...
		for (int i = 0; i < a.length; i++) {
			...
			if (ans.size() > 0 && ans.get(ans.size() - 1).x == x) {
				ans.get(ans.size() - 1).y += number;
			} else {
				ans.add(new Pair(x, number));
			}
		}
		return ans;
	}
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; --qwerty) {
			...
			ArrayList<Pair> lista = toList(a, m);
			ArrayList<Pair> listb = toList(b, m);
