...
vector<int> adj2[N << 1];
bool vis[N << 1];
void dfs(int n, int d, vector<int>& taken){
	vis[n] = true;
	if (d & 1)
		taken.push_back(n);
...
void solve(){
	...
	vector<bool> ignore(2 * n, false);

	auto remove_row = [&] (int x){
		if (ignore[x])
			return;
		assert (!chosen[x]);
		ignore[x] = true;
		...
		for(int i = 0; i < n; ++i){
			avail[i][a[x][i]].erase(x);
			if (avail[i][a[x][i]].size() == 1){
				auto bring_in = *avail[i][a[x][i]].begin();
				if (!chosen[bring_in] && !ignore[bring_in]){
					queue.push_back(bring_in);
				}
			}
		}
	...
	for(int i = 0; i < n; ++i){
		for(int j = 0; j < n; ++j){
			if (cnt[i][j] == 1){
				queue.push_back(*avail[i][j].begin());
			}
		}
	}
	...
	for(int i = 0; i < n; ++i){
		for(int j = 0; j < n; ++j){
			int sz = avail[i][j].size();
			assert (sz == 0 || sz == 2);

			if (sz == 2){
				vector<int> tmp;
				for(auto e: avail[i][j]){
					tmp.push_back(e);
				}
				adj[tmp[0]][tmp[1]] = true;
				adj[tmp[1]][tmp[0]] = true;
			}
		}
	}
	...
	assert (cnt_chosen == cnt_ignore);
	mint tmp = 1;

	for(int k = 0; k < (device.comp - 2 * cnt_chosen); ++k){
		tmp *= 2;
	}
	cout << tmp << endl;
	for(auto e: ans){
