...
void run() {
    ...
    vector<Zp> fac(int(2e5+5), 1);
    rep(i, 2, sz(fac)) fac[i] = fac[i-1]*i;
    auto ch = [&](int n, int m) { return fac[n] / (fac[m] * fac[n-m]); };
    ...
    while (t--) {
        ...
        if (x == y) {
            ...
            int cnt = 0;
            while (cnt < n-1 && a[n-cnt-2] == y) cnt++;
            Zp ans = fac[n];
            ans -= fac[n-cnt-1] * fac[cnt] * ch(n, cnt+1);
            cout << ans.x << '\n';
        }
    }
}