...
signed main()
{
    ...
    /*for (int i = 0; i < cnt; i++)
    {
        ...
        {
            dp2[j] = (dp2[j] + dp[j] * (k - 2) + dp[j - 1] + dp[j + 1]) % MOD;
        }
        dp = dp2;
    ...
    long long val = 0;
    for (int look = 0; 2 * look <= cnt; look++)
    {
        long long x = inq(k - 2, cnt - 2 * look);
        x = x * cnk(cnt, look) % MOD;
        x = x * cnk(cnt - look, look) % MOD;
        val = (val + x) % MOD;
    }
    //cout << "val find : " << val << endl;
    long long all_combo = inq(k, cnt);
    all_combo = (all_combo + MOD - val) % MOD;
    long long ans = all_combo * inq(2, MOD - 2) % MOD;
    cout << ans * mp % MOD << "\n";
}
