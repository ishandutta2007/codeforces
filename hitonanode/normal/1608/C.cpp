...
string solve() {
    ...
    REP(t, 2) {
        ...
        REP(j, N - 1) {
            int s = idx[j], t = idx[j + 1];
            graph.add_edge(t, s);
        }
        swap(A, B);
    }
    const int ngrp = graph.FindStronglyConnectedComponents();
    auto newgraph = graph.GenerateTopologicalGraph();
    vector<int> is_root(ngrp);
    REP(j, ngrp) is_root[j] = newgraph.from[j].empty();
    string ret(N, '0');
    if (accumulate(is_root.begin(), is_root.end(), 0) == 1) {
        REP(i, N) ret[i] += is_root[graph.cmp[i]];
    }
    return ret;
}
