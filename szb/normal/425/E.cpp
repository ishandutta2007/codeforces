...
const ll mod=1e9+7;
ll ans,n,k,f[550][550],bin[250050];	
int main(){
	n=read(),k=read();
	bin[0]=1;For(i,1,250000)bin[i]=bin[i-1]*2%mod;
	f[0][0]=1;
	For(i,1,k)For(j,1,n)rep(k,0,j)	Add(f[j][i],f[k][i-1]*(bin[j-k]-1)%mod*bin[(n-j)*(j-k)]);
	For(i,0,n)Add(ans,f[i][k]);
	writeln(ans);
}