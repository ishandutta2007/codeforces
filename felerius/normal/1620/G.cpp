// Subsequences Galore (https://codeforces.com/contest/1620/problem/G)
...
int main() {
    int n; read(n);
    vector cnts(n, array<uint16_t, 26>{});
    string s;
    rep(i, n) {
        read(s);
        trav(c, s)
    ...
    vector dp(1 << n, MI(0));
    vector set_cnts(1 << n, array<uint16_t, 26>{});
    set_cnts[0].fill(20'000);
    rep(m, 1, 1 << n) {
        int i = __builtin_ctz(m);
        int m2 = m ^ (1 << i);
        dp[m] = 1;
        rep(j, 26) {
            set_cnts[m][j] = min(set_cnts[m2][j], cnts[i][j]);
            dp[m] *= set_cnts[m][j] + 1;
        }
