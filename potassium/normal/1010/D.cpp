...
struct Gate {
	...
	const static int XOR = 2;
	const static int NOT = 3;
	const static int IN = 4;
	const static int getType(const string& name) {
		switch (name[0]) {
...
int main() {
	...
	auto compute = [&](int tp, int p, int q) {
		assert(tp != Gate::IN);
		switch (tp) {
			case Gate::AND : return p & q; break;
			case Gate::OR  : return p | q; break;
			case Gate::XOR : return p ^ q; break;
			case Gate::NOT : return 1 - p; break;
			default: assert(false);
		}
	};
	...
	function<void(int)> dfs2 = [&](int u) {
		if (g[u].tp == Gate::IN) return;

		int p = out[g[u].x];
		int q = out[g[u].y];

		dp[p][g[u].x] = dp[q][g[u].y] = dp[out[u]][u];
		dp[!p][g[u].x] = dp[compute(g[u].tp, !p, q)][u];
		if (g[u].tp != Gate::NOT)
			dp[!q][g[u].y] = dp[compute(g[u].tp, p, !q)][u];
