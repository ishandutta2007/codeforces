...
mint solve_pair(int c0, int c1) {
    mint ans = 0;
    for (int i = 0; i <= min(c0, c1); i++) {
        ans += comb<mint>(c0, i) * comb<mint>(c1, i);
    }
    return ans;
}

mint solve_mid(int c) {
    return mint(2).pow(c);
}
...
void solve() {
    ...
    for (auto p : mp) {
        ...
        if (x * 2 == sum) {
            dbg("MID", p.second);
            ans *= solve_mid(p.second);
        } else if (x < sum - x) {
            int c0 = p.second, c1 = 0;
            if (mp.count(sum - x)) c1 = mp[sum - x];
            dbg("PAIR", c0, c1);
            ans *= solve_pair(c0, c1);
        }
    }
    dbg(ans);
    pr.writeln(ans.val());
