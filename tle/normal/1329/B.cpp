...
void main()
{
	...
	for(int i=0;i<t;++i) {
		...
		for(ll i=1;i<=d;i*=2)
		{
			ll l=i,r=min(i+i-1,d);
			if(l>r) continue;
			s.pb(r-l+1);
		}
		ll f=1;
		for(auto w:s) f=f*(w+1)%m;
		--f;
		f=(f%m+m)%m;
		cout<<f<<"\n";
	}
