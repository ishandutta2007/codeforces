...
vector <int> window(vector <int> values, int w) {
    vector <int> result;
    int cur = 0;
    for (int i = (int)values.size() - 1; i > -w; --i) {
        if (i >= 0) {
            cur += values[i];
        } else {
            cur += values[0];
...
int solveProblem(int n) {
    ...
    for (int prefix = n - 1; prefix >= 0; --prefix) {
        int r = n - prefix - 1;
        for (int a = 0; a < n - prefix; ++a) {
            for (int b = a + 1; b < n - prefix; ++b) {
                int pos = r * (r - 1) / 2 + b - a + 1;
                if (pos < z.size()) {
                    res += binomial(n, prefix) * 1ll * factorial(prefix) % mod * z[pos] % mod;
                    res %= mod;
                }
            }
        }

        z = window(window(z, n - prefix), n - prefix);
    }
...
void solve(int) {
    memset(bin, 255, sizeof(bin));
    memset(fact, 255, sizeof(fact));
