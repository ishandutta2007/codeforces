...
void solve() {
  ...
  using T = ll;
  const ll INF = 1LL << 60;
  SegmentTree<T, T> segX([](T x, T y) { return max(x, y); },
                         [](T x, T y) { return max(x, y); },
                         [](T x, T y) { return max(x, y); }, 0, T(0));
  SegmentTree<T, T> segY([](T x, T y) { return max(x, y); },
                         [](T x, T y) { return max(x, y); },
                         [](T x, T y) { return max(x, y); }, 0, T(0));
  ll K = len(key);
  segX.init(K);
  ...
  FOR(q, Q) {
    ...
    if (d == 0) {
      // L
      ll v = segY.fold(y, y + 1);
      auto i = LB(key, v + 1);
      segX.operate_range(i, x, key[y]);
      print(key[x] - v);
    }
    elif (d == 1) {
      ll v = segX.fold(x, x + 1);
      auto i = LB(key, v + 1);
      segY.operate_range(i, y, key[x]);
      print(key[y] - v);
    }
  }
}
