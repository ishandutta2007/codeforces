...
int main(){
  ...
  ll res=0,buf=0;
  bool judge = true;
  using S=SquareMatrix<ll,3>;
  auto fmul=[&](S x,S y){
    return x*y;
  };
  vector<S>mat(3);
  rep(i,0,3)rep(x,0,3)rep(y,0,3){
    if(x>y)continue;
    if(i==y)mat[i][x][y]=1;
    else mat[i][x][y]=0;
  }
  ll n,q;cin>>n>>q;
  SegmentTree seg(n,fmul,S::I(3));
  string s;cin>>s;
  rep(i,0,n)seg.set(i,mat[s[i]-'a']); 
  seg.build();
  /*{
    auto mat=S::I(3);
    rep(i,0,n){
  ...
  while(q--){
    ...
    auto m=seg.seg[1];
    ll ret=INF;
    rep(i,0,3)rep(j,0,3)chmin(ret,m[i][j]);
    cout<<ret<<endl;
    //seg.print();
  }
  return 0;
