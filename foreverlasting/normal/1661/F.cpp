...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        for(res i=n;i>=1;i--)a[i]-=a[i-1];
        m=Read();
        auto f=[&](const res &x,const res &k){
            res t=x%(k+1),p=x/(k+1),q=(x+k)/(k+1);
            return (LL)t*q*q+((LL)k+1-t)*p*p;
        };
        auto ck=[&](const LL &lim){
            res v=0;
            LL w=0;
            for(res i=1;i<=n;i++){
                res l=1,r=a[i],ret=0;
                while(l<=r){
                    res mid=(l+r)>>1;
//                    printf("%lld %lld %lld %d %d\n",lim,f(a[i],mid-1),f(a[i],mid),mid,i);
                    if(f(a[i],mid-1)-f(a[i],mid)>=lim)l=mid+1,ret=mid;
                    else r=mid-1;
                }
                v+=ret,w+=f(a[i],ret);
            }
