...
pair<int,int>tl(pair<int,int>x){return make_pair(x.first/Gcd(x.first,x.second),x.second/Gcd(x.first,x.second));}
...
void insert(int x,int y,int z)
{
    ...
    int now=q[r];if(!now)now=(q[r]=++numcutt);
    for(auto i:nod[now])if(z!=-1||make_pair(x,y)!=make_pair(i.first,i.second))ans[tl(make_pair(i.first+x,i.second+y))]+=(z<<1);
    ans[tl(make_pair(x,y))]+=z;all+=z;
    if(z==1)nod[now].push_back(make_pair(x,y));
    else
    {
        for(vector<pair<int,int> >::iterator i=nod[now].begin();i!=nod[now].end();i++)
        if((*i)==make_pair(x,y)){nod[now].erase(i);break;}
    }
    return;
}
void check(int x,int y){printf("%d\n",all-ans[tl(make_pair(x,y))]);return;}

void Read(){rin>>n;return;}
void Init(){return;}
void Work()
{
    for(;n;n--)
    {
        ...
        if(typ==1)insert(x,y,1);
        if(typ==2)insert(x,y,-1);
        if(typ==3)check(x,y);
    }
    return;
}
