...
static ll n,dp[N][2],dpp[N],ch[N];
static multiset<ll> st[N][2];
static V(ll) ad[N];
...
void dfs1(ll x,ll y)
{
    ...
    for(auto it:ad[x])
    {
        ...
        {
            dfs1(it,x);
            dp[x][0]=max(dp[x][0],max(dp[it][1]+ch[x]-1,dp[it][0]+ch[x]-1));
            st[x][0].insert(max(dp[it][1],dp[it][0]));
            dp[x][1]=max(dp[x][1],dp[it][0]+1);
            st[x][1].insert(dp[it][0]);
        }
    }
    return;
}
void dfs2(ll x,ll y)
{
    dpp[x]=max(dp[x][0],dp[x][1]);
    if(y!=-1)
    {
        st[y][0].erase(st[y][0].find(max(dp[x][1],dp[x][0])));
        st[y][1].erase(st[y][1].find(dp[x][0]));
        ll dpp0,dpp1;
        if(st[y][0].size()==0)
        {
            dpp0=ad[y].size()-1;
        ...
        {
            dpp0=(*st[y][0].rbegin())+ad[y].size()-2;
        }
        if(st[y][1].size()==0)
        {
            dpp1=1;
        ...
        {
            dpp1=(*st[y][1].rbegin())+1;
        }
        dpp[x]=max(dpp[x],max(max(dpp0,dpp1)+ad[x].size()-1,dpp0+1));
        st[x][0].insert(max(dpp0,dpp1));
        st[x][1].insert(dpp0);
        st[y][0].insert(max(dp[x][1],dp[x][0]));
        st[y][1].insert(dp[x][0]);
    }
    for(auto it:ad[x])
    {
        if(it!=y)
