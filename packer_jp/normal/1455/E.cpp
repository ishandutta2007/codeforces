    ...
    while (t--) {
        ...
        rep(i, 4) {
            rep(j, 4) {
                int c = x[i], r = y[j];
                vint xc(x), yc(y);
                rep(k, 4) { xc[k] = x[k] - c, yc[k] = y[k] - r; }
                rep(k, 2) {
                    rep(p, 2) {
                        vint idx(4);
                        iota(all(idx), 0);
                        do {
                            vint v = {xc[idx[1]], xc[idx[3]], yc[idx[2]], yc[idx[3]]};
                            sort(all(v));
                            int a = max(1LL, v[1]);
                            int cur = 0;
                            cur += abs(xc[idx[0]]) + abs(yc[idx[0]]);
                            cur += abs(xc[idx[1]] - a) + abs(yc[idx[1]]);
                            cur += abs(xc[idx[2]]) + abs(yc[idx[2]] - a);
                            cur += abs(xc[idx[3]] - a) + abs(yc[idx[3]] - a);
                            chmin(ans, cur);
