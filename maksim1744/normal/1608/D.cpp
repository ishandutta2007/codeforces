...
int main() {
    ...
    auto calc = [&](int ind) -> tuple<int, int, int> {
        int a = 0, b = 0, q = 0;
        for (int i = 0; i < n; ++i) {
            if (v[i][ind] == 'A') ++a;
            if (v[i][ind] == 'B') ++b;
    ...
    auto [al, bl, ql] = calc(0);
    auto [ar, br, qr] = calc(1);

    init_C(n * 2 + 20);

    Mint ans = 0;
    for (int ca = 0; ca <= n; ++ca)
        ans += C(ql, ca - al) * C(qr, ca - ar);
    ...
    Mint alla = 1, allb = 1;
    for (const string& s : v) {
        if (count(s.begin(), s.end(), 'A')) allb = 0;
        if (count(s.begin(), s.end(), 'B')) alla = 0;
    }
