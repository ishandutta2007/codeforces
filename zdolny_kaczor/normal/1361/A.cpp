...
int main() {
	...
	for (int i = 1; i <= n; ++i) by_type[type[i]].push_back(i);
	vi ans;
	for (int i = 1; i <= n; ++i) for (int v : by_type[i]) {
		static int timer = 0;
		timer++;
		ans.push_back(v);
		for (int w : graf[v]) last_seen[type[w]] = timer;
		for (int w : graf[v]) if (type[w] == type[v]) done("-1\n");
		for (int j = 1; j < type[v]; ++j)
			if (last_seen[j] != timer) done("-1\n");
	}
