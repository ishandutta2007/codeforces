...
void eval(int x) {
    ...
    l = val = 0;
    vector<ai<4>> tmp;
    for (ai<3> e : edges) tmp.pb({abs(x-e[0]), e[0], e[1], e[2]});
    sort(all(tmp));
    DSU d(n);
    for (ai<4> e : tmp) if (d.unite(e[2], e[3])) val += e[0], l += (e[1]<=x);
}
...
void solve() {
    ...
    F0R(i, m) {
        pts.pb(edges[i][0]+1);
        FOR(j, i+1, m) pts.pb((edges[i][0]+edges[j][0])/2 + 1);
    }
    sort(all(pts));
    pts.pb(200000000);
    cin >> p >> k >> a >> b >> c;
    ...
    int pos = 0;
    F0R(i, k) {
        if (i==0 || queries[i] >= pts[pos]) {
            while (queries[i] >= pts[pos]) pos++;
            debug(pos); cout.flush();
            eval(queries[i]);
        }
        ans ^= val + (2*l - n + 1) * (queries[i] - pre);
    }
    cout << ans << nl;
}
