...
void main_() {
	...
	dp[1][0] = dp[1][1] = 0;
	REP(i, 1, n) {
		chmin(dp[i + 1][0], dp[i][0] + abs(a[i] - a[i - 1]) + abs(b[i] - b[i - 1]));
		chmin(dp[i + 1][0], dp[i][1] + abs(a[i] - b[i - 1]) + abs(b[i] - a[i - 1]));
		chmin(dp[i + 1][1], dp[i][0] + abs(a[i] - b[i - 1]) + abs(b[i] - a[i - 1]));
		chmin(dp[i + 1][1], dp[i][1] + abs(a[i] - a[i - 1]) + abs(b[i] - b[i - 1]));
	}
	print(min(dp.back()[0], dp.back()[1]));
}