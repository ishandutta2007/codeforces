...
struct solver
{
    ...
    solver()
    {
        ...
        vector<vector<int>> ids(n+1);
        {
            ...
            {
                i--;
                ids[i].emplace_back(idx);
                ap[i]=1;
                idx++;
            }
            while(ap[nap]) nap++;
        }
        vector<int> ans(n); ans=b;

        heap<p32> qu;
        for(int i=0; i<=n; i++)
        {
            qu.emplace(ids[i].size(),i);
        }
        vector<bool> fix(n);
        // int r=n-y+x;
        ...
        assert(!qu.empty());
        const int rot=qu.top().first;
        // if(rot*2>n-x)
        // {
        //     cout << "NO" << "\n";
        //     return;
        // }
        vector<vector<int>> nids(n+1);
        for(int i=0; i<n; i++)
        {
            if(!fix[i])
            {
                nids[ans[i]].emplace_back(i);
            }
        }
        vector<int> mov;
        for(int i=0; i<=n; i++)
        ...
        auto movcp=mov;
        auto anscp=ans;
        rotate(begin(mov),begin(mov)+rot,end(mov));
        for(int i=0; i<(int)mov.size(); i++)
        {
            ans[movcp[i]]=anscp[mov[i]];
        }
        int e=n-y;
        for(int i=0; i<n; i++)
        {
        ...
        for(int i=0; i<n and e>0; i++)
        {
            if(!fix[i] and ans[i]!=nap)
            {
                e--;
                ans[i]=nap;
            }
