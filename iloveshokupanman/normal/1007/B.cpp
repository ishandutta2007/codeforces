...
int main() {
    ...
    ll M = 100000;
    vll div(M + 1, 0);
    repn(i, M)for (ll j = i; j <= M; j += i)div[j]++;
    ...
    rep(x, 64) {
        rep(i, 3)rep(j, 6) {
            if ((x >> j) & 1)val[x][i] |= 1ull << lis[j][i];
        }
    }

    vll pm(64,-1);
    rep(x, 64) {
        rep(i, 6)if ((x >> i) & 1)pm[x] = -pm[x];
    }

    rep(snuke, T) {
        ll A, B, C;
        cin >> A >> B >> C;
        vll c = { 0,A,B,gcd(A,B),C,gcd(A,C),gcd(B,C),gcd(A,gcd(B,C)) };

        ll abc = 0;
        ll aab = 0;
        repn(x, 63) {
            
            //cout << val[x][0] << " " << val[x][1] << " " << val[x][2] << endl;
            ll now = div[c[val[x][0]]] * div[c[val[x][1]]] * div[c[val[x][2]]];
            abc += now * pm[x];

            
            now = div[c[val[x][0] | val[x][1]]] * div[c[val[x][2]]];
        ...
        ll aaa = div[c[7]];
        //cout << abc << " " << aab << " " << aaa << endl;
        ll ans = abc + aab * 3 + aaa * 2;
        ans /= 6;
        cout << ans << endl;
    }
}