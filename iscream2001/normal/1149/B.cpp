...
void MAIN(){
	...
	for(int j=0;j<26;++j) to[n+1][j]=to[n+2][j]=n+1;
	for(int i=n;i>=1;--i){
...
//			cout<<op<<" "<<ch<<endl;
			b[op][++len[op]]=ch;
			for(int i=(op==1?len[1]-1:0);i<=len[1];++i){
				for(int j=(op==2?len[2]-1:0);j<=len[2];++j){
					for(int k=(op==3?len[3]-1:0);k<=len[3];++k){
						if(i<len[1]){
							dp[i+1][j][k]=min(dp[i+1][j][k],to[dp[i][j][k]+1][b[1][i+1]]);
						}
						if(j<len[2]){
							dp[i][j+1][k]=min(dp[i][j+1][k],to[dp[i][j][k]+1][b[2][j+1]]);
						}
						if(k<len[3]){
							dp[i][j][k+1]=min(dp[i][j][k+1],to[dp[i][j][k]+1][b[3][k+1]]);
						}
					}
				}
			}
			...
			for(int i=(op==1?len[1]:0);i<=len[1];++i){
				for(int j=(op==2?len[2]:0);j<=len[2];++j){
					for(int k=(op==3?len[3]:0);k<=len[3];++k){
						dp[i][j][k]=inf;
					}
				}
			}
