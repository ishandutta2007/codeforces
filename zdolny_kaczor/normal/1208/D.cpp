...
pair <ll, int> tree[nax * 2];
pair <ll, int> mer(pli a, pli b) {
...
pli read(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
	push_down(wh);
	if (l >= rb || r <= lb) return {llinf, -inf};
	if (l <= lb && r >= rb) return tree[wh];
	int s = (lb + rb) / 2;
	return mer(read(l, r, wh * 2, lb, s), read(l, r, wh * 2 + 1, s, rb));
}
void add(int l, int r, ll val, int wh = 1, int lb = 0, int rb = nax) {
	if (wh == 1) debug(imie(l), imie(r), imie(val));
	push_down(wh);
	if (l >= rb || r <= lb) return;
	if (l <= lb && r >= rb) {
		push[wh] += val;
	...
	int s = (lb + rb) / 2;
	add(l, r, val, wh * 2, lb, s);
	add(l, r, val, wh * 2 + 1, s, rb);
	tree[wh] = mer(tree[wh * 2], tree[wh * 2 + 1]);
}
int ans[nax];
int main() {
	int n;
