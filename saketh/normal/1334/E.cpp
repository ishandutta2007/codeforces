...
void solve() {
    ...
    F0R (p, P) {
        while (a % primes[p] == 0) { afac[p]++; a /= primes[p]; }
        while (b % primes[p] == 0) { bfac[p]++; b /= primes[p]; }
        ...
        int diff = abs(afac[p] - bfac[p]);
        ans *= mn::inverse_factorial(diff);
    }

    ps(ans * mn::factorial(atot) * mn::factorial(btot));
}
