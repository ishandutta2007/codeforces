...
int main(){
	...
	TT{
		cauto n=read();
		cauto es=readV(n-1,pii{1,1});
		vector<int> deg(n);
		for(cauto &[a,b]:es) ++deg[a], ++deg[b];
		...
		vector<int> path;
		for(int v=min_element(all(deg))-deg.begin(),p=0;path.size()<n;){
			path.eb(v);
			cauto nxt=xs[v]^p;
			p=v;
			v=nxt;
		...
		vector<int> re(n-1);
		reps(i,1,n) re[inv[{path[i-1],path[i]}]]=i%2;
		for(cauto &x:re) cout<<2+x<<" ";
		cout NL;
	}
	return 0;
}