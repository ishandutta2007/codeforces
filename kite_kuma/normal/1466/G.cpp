...
int word_search(string &s, string &word) {
	auto t = word + s;
	auto res = z_algorithm(t);
	int len = word.size();
	int ret = 0;
	Rep(i, len, t.size()) {
		if(res[i] >= len) {
...
int main() {
	...
	VV<mint> ruiseki(26, V<mint>(n + 1));

	rep(i, 26) {
		auto &now = ruiseki[i];
		rep(j, n) {
			now[j + 1] = now[j] * 2;
			if(t[j] == 'a' + i) now[j + 1] += 1;
		}
	}
	...
	V<mint> mp = {1};
	rep(_, n) mp.push_back(mp.back() * 2);
	V<mint> rev = {1};
	mint div = ((mint)2).inverse();
	rep(_, n) rev.push_back(rev.back() * div);
	debug(mp);
	auto kukan = [&mp, &ruiseki](int ch, int lef, int rig) { return ruiseki[ch][rig] - ruiseki[ch][lef] * mp[rig - lef]; };
	...
	rep(i, q) {
		...
		if(k < s.size() && s[k].size() < len) {
			continue;
		}

		// que.emplace_back(level, k, i);

		mint res = 0;

		int slen = s[level].size();
		string t = "";
		Rep(i, slen - len + 1, slen) t += s[level][i];
		t += 'a';
		rep(i, len - 1) t += s[level][i];

		// vi cnt(26);
		rep(nxt, 26) {
			res += kukan(nxt, level, k) * word_search(t, word);
			t[len - 1] += 1;
		}

		int sa = k - level;
		res += (mint)modpow(2, sa, mod) * word_search(s[level], word);
