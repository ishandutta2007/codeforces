...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        LL ans=0;
        for(res i=0;i<=n+1;i++)for(res j=0;j<=n+1;j++)pre[i][j]=suf[i][j]=0;
        for(res i=1;i<=n;i++)pre[i][p[i]]++;
        for(res i=n;i>=1;i--)suf[i][p[i]]++;
        for(res i=1;i<=n;i++)for(res j=1;j<=n;j++)pre[i][j]+=pre[i-1][j];
        for(res i=1;i<=n;i++)for(res j=1;j<=n;j++)pre[i][j]+=pre[i][j-1];
        for(res i=n;i>=1;i--)for(res j=1;j<=n;j++)suf[i][j]+=suf[i+1][j];
        for(res i=1;i<=n;i++)for(res j=1;j<=n;j++)suf[i][j]+=suf[i][j-1];
        for(res b=2;b<=n;b++){
            for(res c=b+1;c<n;c++){
                ans+=(LL)pre[b-1][p[c]-1]*suf[c+1][p[b]-1];
            }
        }
        printf("%lld\n",ans);
    }
