...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		cin >> X[i].x >> X[i].l >> X[i].r;
	}

	for(int i = 0; i < m; ++i){
		cin >> Y[i].x >> Y[i].l >> Y[i].r;
	}
	...
	for(auto e: Y){
		events.push_back({e.l << 1, e.x});
		events.push_back({(e.r << 1) | 1, e.x});

	}

	sort(all(events));
	for(auto e: X){
		while(iter < events.size()){
			if (events[iter].first <= 2 * e.x){
				ll x = events[iter].first;
				if (x & 1){
					upd(events[iter].second, -1);
				}
				else{
					upd(events[iter].second, 1);
				}
