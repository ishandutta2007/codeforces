...
const val max = 1000005
fun main(){
    solve.cases{
        val bestpossible = LongArray(max)
        val n = getint
        val C = getint
        for(i in 0 until n){
            val c = getint
            val d = getint
            val h = getint
            bestpossible[c] = maxOf(bestpossible[c], d.toLong() * h )
        }
        for(i in 1 until max){
            var mul = 1
            for(j in i * 2 until max step i){
                mul ++
                bestpossible[j] = maxOf(bestpossible[j], bestpossible[i] * mul )
            }
        }
        for(i in 1 until max){
            bestpossible[i] = maxOf(bestpossible[i], bestpossible[i-1])
        }
        val m = getint
        repeat(m){
            val h = getint
            val d = getlong
            val need = h.toLong() * d
            val ans = BinarySearchFirstTrue(0, max-1) {
                bestpossible[it] > need
            } ?: (max + 1)
            put(if(ans > C) -1 else ans )
        }
