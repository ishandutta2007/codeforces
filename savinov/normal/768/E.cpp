...
struct state {
    ...
    bool operator<(const state &r) const {
        return cur < r.cur || (cur == r.cur && used < r.used);
    }
...
int solve(state s) {
    ...
    for (int v = 1; v <= s.cur; ++v) {
        if (s.used & (1ll << v)) continue;
        z.push_back(solve(s.move(v)));
    }
    res = mex(z);
    return res;
}
