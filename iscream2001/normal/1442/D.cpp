...
LL sol(int id,int l,int r){
	...
	if(l==r){
		...
		for(int i=0;i<=lim;++i){
			re=max(re,dp[id][i]+s);
			if(i<V[l].size())s+=V[l][i];
		}
		return re;
	}
	int mid=(l+r)>>1;
	for(int j=0;j<=K;++j) dp[id+1][j]=dp[id][j];
	for(int i=mid+1;i<=r;++i){
		for(int j=V[i].size();j<=K;++j){
			dp[id+1][j-V[i].size()]=max(dp[id+1][j-V[i].size()],dp[id+1][j]+su[i]);
		}
	}
	re=max(re,sol(id+1,l,mid));
	for(int j=0;j<=K;++j) dp[id+1][j]=dp[id][j];
	for(int i=l;i<=mid;++i){
		for(int j=V[i].size();j<=K;++j){
			dp[id+1][j-V[i].size()]=max(dp[id+1][j-V[i].size()],dp[id+1][j]+su[i]);
...
/*
3 25
10 0 0 1 1 2 2 3 3 3 4 
10 0 0 0 0 2 2 3 3 3 4 
10 0 0 1 1 1 1 3 3 3 4 
3 0 0 5
3 1 2 3
2 1 2
*/
