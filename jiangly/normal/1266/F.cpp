...
vector<pair<int, int>> merge(const vector<pair<int, int>> &lhs, const vector<pair<int, int>> &rhs) {
    ...
    while (i < lhs.size() || j < rhs.size()) {
        ...
        if (i < lhs.size() && lhs[i].first == x)
            ++i;
        if (j < rhs.size() && rhs[j].first == x)
            ++j;
    }
    return res;
}
...
void dfs2(int u, int p) {
    ...
    vector<int> a;
    for (int v : e[u]) {
        up[v] = 1 + max(up[u], (dp[v][0] + 1 == dp[u][0] ? dp[u][1] : dp[u][0]));
    ...
    for (int i = 0; i < a.size(); ) {
        ...
        if (lst != 0)
            modify(1, 1, n, cnt[u].back().first, cnt[u].back().first + 1, 1, a.size() - i + 1);
...
int main() {
    ...
    fill(ans, ans + n, 1);
    for (int i = 0; i < n; ++i)
        ans[0] = max(ans[0], (int)e[i].size() + 1);
    dfs1(0, -1);
    dfs2(0, -1);
    push(1, 1, n);
    for (int i = 0; i < n; ++i)
