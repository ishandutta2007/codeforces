...
int main(int argc, char **argv){
  ...
  forn(qq,tc){
    ...
    forn(i,n){
      if(!sz(F[i])) continue;
      ++K;
      if(sz(F[i])==2&&sz(F[F[i][0]])) F[i][0]=F[i][1];
      add(F[i][0],i);
    }
    vector<int> V,A,B;
    forn(i,n){
      if(P[i]==-1&&iP[i]==-1) V.pb(i);
      else if(P[i]==-1) A.pb(i);
      else if(iP[i]==-1) B.pb(i);
    }
    assert(sz(A)==sz(B));
    assert(sz(V)!=1||sz(A));
    if(sz(V)){
      forn(i,sz(V)-1) add(V[i],V[i+1]);
      A.pb(V.back());
      B.insert(B.begin(),V[0]);
    }
    forv(i,A) add(A[i],B[i]);
    cout<<K<<'\n';
    forn(i,n){
      assert(P[i]!=-1);
      cout<<P[i]+1<<' ';
