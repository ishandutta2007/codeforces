...
int main(){
	...
	TT{
		...
		cauto getNgc=
			[&](){
				int ngc=0;
				rep(i,n)reps(j,1,m)if(vs[i][j-1]>vs[i][j]) ++ngc;
				return ngc;
			};
		cauto total=getNgc();
		cauto get=
			[&](const int k){
				rep(j,m)if(k!=j){
					vector<int> cols{j,j+1,k,k+1};
					sort(all(cols)); UNIQUE(cols);
					int cnt=0;
					for(cauto &v:cols)if(0<v&&v<m)rep(i,n)if(vs[i][v-1]>vs[i][v]) --cnt;
					// out(cols,vs,cnt,1);
					// for(cauto &v:cols)if(0<v&&v<m)rep(i,n)if(vs[i][v-1]>vs[i][v]) out(i,v,1);
					rep(i,n) swap(vs[i][j],vs[i][k]);
					for(cauto &v:cols)if(0<v&&v<m)rep(i,n)if(vs[i][v-1]>vs[i][v]) ++cnt;
					rep(i,n) swap(vs[i][j],vs[i][k]);
					out(k,j,cols,total,cnt,1);
					if(total+cnt==0) return j;
				}
				return -1;
			};
		int ngj=-1;
		reps(j,1,m){
			int ok=1;
			rep(i,n)if(vs[i][j-1]>vs[i][j]) ok=0;
			if(!ok){
				ngj=j;
				break;
			}
