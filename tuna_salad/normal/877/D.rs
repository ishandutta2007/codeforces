...
fn main() {
	let mut rin = reader();
	let mut rout = writer();
	let adj = |i:i16, j:i16| -> Vec<(i16,i16)> {
	...
	l!(r0,c0,r1,c1 = rin.p::<i16>());
	let mut dist = v!([r+2][c+2][4] = -1_i32);
	let mut q = BinaryHeap::new();
	for k in 1..4 {
		dist[r0 as usize][c0 as usize][k] = 0;
	}
	q.push( (0,0,r0,c0,0) );
	while let Some( (d,e,r,c,dir) ) = q.pop() {
		if dist[r as usize][c as usize][dir] >= 0 { continue; }
		let d = -d;
		dist[r as usize][c as usize][dir] = d;
		if (r,c) == (r1,c1) {
			writeln!(rout, "{}", d).ok();
			return;
		}
		for (i,(rr,cc)) in adj(r,c).into_iter().enumerate() {
			if b[rr as usize][cc as usize] == b'#' || dist[rr as usize][cc as usize][i] >= 0 { continue; }
			let (dd,ee) = if i == dir && e > 0 {
				(d, e-1)
			} else {
				(d+1, k-1)
			};
			q.push( (-dd,ee,rr,cc,i) );
		}
	}
	writeln!(rout, "-1").ok();
}