...
ll h, w, k;
ll c[100005];
ll sx[100005], sy[100005], tx[100005], ty[100005], wg[100005];
vector<Q> vec[100005];
vector<ll> dp[100005];
...
int main(void)
{
	...
	while(T--){
		...
		cin >> h >> w >> k;
		rep(i, 1, h) cin >> c[i];
		rep(i, 1, k) cin >> sy[i] >> sx[i] >> ty[i] >> tx[i] >> wg[i];
		sx[0] = 1, sy[0] = 0, tx[0] = 1, ty[0] = 1, wg[0] = 0;
		sx[k+1] = w, sy[k+1] = h, tx[k+1] = w, ty[k+1] = h+1, wg[k+1] = 0;
		
		rep(i, 0, h+1) vec[i].clear();
		rep(i, 0, k+1){
			vec[sy[i]].pb(Q(sx[i], P(i, 1)));
			vec[ty[i]].pb(Q(tx[i], P(i, 2)));
		}
		rep(i, 0, h+1){
			sort(all(vec[i])), dp[i].resize(sz(vec[i]));
			reps(j, dp[i]) dp[i][j] = inf;
		}
		dp[0][0] = 0;
		
		//rep(i, 0, h+1) outl(vec[i]);
		
		rep(i, 0, h+1){
			if(sz(dp[i]) == 0) continue;
			ll mn = dp[i][0], l = sz(dp[i]);
			rep(j, 0, l-1){
				chmin(dp[i][j], mn);
				mn = dp[i][j];
				if(j < l-1) mn += c[i]*(vec[i][j+1].fst-vec[i][j].fst);
			}
			mn = dp[i][l-1];
			per(j, l-1, 0){
				chmin(dp[i][j], mn);
				mn = dp[i][j];
				if(j > 0) mn += c[i]*(vec[i][j].fst-vec[i][j-1].fst);
			}
			reps(j, vec[i]){
				if(vec[i][j].snd.snd == 2) continue;
				ll id = vec[i][j].snd.fst;
				ll p = lower_bound(all(vec[ty[id]]), Q(tx[id], P(id, 2))) - vec[ty[id]].begin();
				chmin(dp[ty[id]][p], dp[i][j]-wg[id]);
			}
		}
		if(dp[h+1][0] > inf/2) outl("NO ESCAPE");
		else outl(dp[h+1][0]);
		
		//rep(i, 0, h+1) outl(dp[i]);
	}
