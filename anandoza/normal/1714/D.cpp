...
void solve() {
    ...
    int k = sz(t);
    vector<pair<int, pair<int, int>>> dp(k + 1, {INF, {-1, -1}});
    dp[0] = {0, {-1, -1}};
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            if (i + sz(v) > k)
                continue;
            if (t.substr(i, sz(v)) == v) {
                for (int next = i + 1; next <= i + sz(v); next++) {
                    ckmin(dp[next], {dp[i].first + 1, {i, j}});
                }
            }
        }
    }
