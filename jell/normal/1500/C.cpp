#line 1 "codeforces-workspace/contest/1500/c/c.cpp"
...
#line 6 "codeforces-workspace/contest/1500/c/c.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1500/c/c.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1500/c/c.cpp"
...
void main() {
  ...
  auto rgt = make_vector<int>({m, n}, n);
  for (auto j : range(m)) {
  ...
  vector<int> oks(m);
  for (auto i : range(m)) {
    if (rgt[i][0] == n) oks[i] = 1;
  }

  vector<pair<int, int>> intv{{0, n}};

  vector<int> ans;

  auto find_col = [&]() -> int {
    for (auto c : range(m)) {
      if (oks[c] == intv.size() && !used[c]) return c;
    }
    return -1;
  };
  ...
  for (auto _t : range(m)) {
    ...
    for (auto [l, r] : intv) {
      ...
      for (int y = l, x = y; x != r; x = y) {
        while (y < r && b[x][c] == b[y][c]) ++y;
  ...
  for (auto [l, r] : intv) {
    int pre = n;
    for (auto i : rrange(l, r)) {
      auto &v = mp[b[i]];
      if (v.empty()) eject("-1");
      if (pre < v.back()) eject("-1");
      pre = v.back();
      v.pop_back();
    }
  }
  ...
  // verify
  // for (auto &&j : ans) {
  //   stable_sort(begin(a), end(a), [&](const auto &r1, const auto &r2) {
  //     return r1[j - 1] < r2[j - 1];
  //   });
  // }
  // assert(a == b);
}
