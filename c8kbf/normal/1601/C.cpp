...
_ n, m, a[maxn], b[maxn], d[maxn], s, c[maxn];
v_ g;

void merge(_ l, _ md, _ r);
void msort(_ l, _ r);
void retrv(_ l, _ r, _ x, _ y);
inline void AC() {
    ...
    _ p = 1;
    for(_ i = 1; i <= n; ++i) {
        for(; p <= m && d[p] <= i; ++p) g.push_back(b[p]);
        g.push_back(a[i]);
    }
    for(; p <= m; ++p) g.push_back(b[p]);
    msort(0, g.size()-1);
    writeln(s);
...
void retrv(_ l, _ r, _ x, _ y) {
    if(x > y) return;
    _ md = (x+y)>>1, s = 1E11, c = -1, t = 0;
    for(_ i = l; i <= r; ++i) if(a[i] < b[md]) ++t;
    s = t;
    c = l;
    for(_ i = l; i <= r-1; ++i) {
        if(a[i] < b[md]) --t;
