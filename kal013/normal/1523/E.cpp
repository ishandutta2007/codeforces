...
const int modular_int<mod>::inverse_limit = 300000;
...
mint sumDecreaseNcr(long long a, long long b, long long r){
	// ncr(a, r) + ... ncr(b, r);
	a = max(a, r);
	if (b < a)
		return 0;
	return ncr(b + 1, r + 1) - ncr(a, r + 1);
}


mint dfs(vector<int>& A, int k, int ones, mint prob, vector<int>& cnts){
	int n = A.size();
	mint ans = 0;
...
void solve(){
    ...
    for(int p = 1; p < n; ++p){
    	...
    	long long spaces = n - (p - 1) * 1LL * (k - 1);
    	cur += ncr(spaces, p) * (k - 1) * mint(p - 1);

    	cur += sumDecreaseNcr(spaces - k + 1, spaces - 1, p) * (p + 1);


    	cur *= fact[p];

    	cur *= inv_fact[n] * fact[(n - p - 1)]; // ??
