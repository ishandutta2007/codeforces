...
namespace Poly
{
	...
	{
		int n=A.size()-1,m=B.size()-1,t,len,inv;
	...
	{
		int n=A.size()-1,t,len,inv;
		for(t=-1,len=1;len<=n+n;len<<=1,++t);
		inv=_inv[len];
		for(int i=1;i<len;++i)_rev[i]=(_rev[i>>1]>>1)|((i&1)<<t);
		A.resize(len),NTT(A,1);
		for(int i=0;i<len;++i)A[i]=((LL)A[i]*A[i])%mod;
		NTT(A,0),A.resize(n+n+1);
		for(int i=0;i<A.size();++i)A[i]=((LL)A[i]*inv)%mod;
		return A;
	}
	...
	inline void Solve()
	{
		Init(),scanf("%s",s),n=strlen(s),t=max((int)ceil((n-1)*logl(10)/logl(3))-2,0),A.push_back(1),B.push_back(3),ans=(t+5)*3;
		if(n==1 && s[0]=='1')return (void)(puts("1"));
		for(int i=n-1;~i;--i)S.push_back(s[i]^48);
		for(int i=t;i;B=Mul2(B),Maintain(B),i>>=1)if(i&1)A=Mul(A,B),Maintain(A);
		for(int i=0;i<5;Maintain(A=Mul(A,3)),++i)
		{
			for(res=0,B=A;Cmp(B,S);Maintain(B=Mul(B,2)))res+=2;
			ans=min(ans,(t+i)*3+res);
			if(!res)break;
		}
		printf("%d",ans);
	}
