...
void build(int u){
    ...
    for (int i=29;i>-1;--i){
        if (u>>i&1){
            if (!c[nd].r) cnt++, c[nd].r=cnt;
            nd=c[nd].r, c[nd].sum++;
        }
        else{
            if (!c[nd].l) cnt++, c[nd].l=cnt;
            nd=c[nd].l, c[nd].sum++;
        }
    }
}
signed main(){
    ...
    cin>>n;
    rep(i,n) cin>>a[i];
    rep(i,n) cin>>b[i], build(b[i]);
    rep(i,n){
        int nd=0;
        for (int j=29;j>-1;--j){
            if (a[i]>>j&1){
                if (c[nd].r&&c[c[nd].r].sum) nd=c[nd].r, c[nd].sum--, a[i]^=(1<<j);
                else nd=c[nd].l, c[nd].sum--;
            }
            else{
                if (c[nd].l&&c[c[nd].l].sum) nd=c[nd].l, c[nd].sum--;
                else nd=c[nd].r, c[nd].sum--, a[i]^=(1<<j);
            }
        }
        cout<<a[i]<<" ";
    }
