...
int solve() {
    ...
    multiset<int> ms;
    REP(m, exist.size()) if (exist[m]) ms.insert(dp[m]);

    int ret = INF;

    auto dp_set = [&](int i, int val) -> void {
        if (dp[i] == val) return;

        if (exist[i]) {
            ms.erase(ms.lower_bound(dp[i]));
            ms.insert(val);
        }
    ...
    IFOR(lo, 1, M + 1) {
        ...
        for (lint i = lint(lo) * lo; i <= M; i += lo) {
            // i -> (lo, i / lo) ->
            int newhi = max(lo, dp[i / lo]);
            if (newhi < dp[i]) dp_set(i, newhi);
        }

        chmin(ret, *prev(ms.end()) - lo);
    }
    return ret;
}
