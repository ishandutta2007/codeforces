...
int main(){
    ...
    rep(ii,T){
        ...
        rep(j,M-1){
            rep(i,N){
                if(a[i][j]>a[i][j+1]) ng.emplace(j);
            }
        }
        if(ng.empty()){
            cout << 1 << " " << 1 << "\n"; continue;
        }
        auto itr = ng.begin(); ll k = *itr; ll l=-1,r=-1;
        rep(j,M){
            set<ll> tmp = ng;
            rep(i,N){
                swap(a[i][j],a[i][k]);
            ...
            bool ok = true;
            if(j!=0){
                if(tmp.count(j-1)) tmp.erase(j-1);
                rep(i,N){
                    if(a[i][j-1]>a[i][j]) ok = false;
                }
            }
            if(j!=M-1){
                if(tmp.count(j)) tmp.erase(j);
                rep(i,N){
                    if(a[i][j]>a[i][j+1]) ok = false;
                }
            }
            if(k!=0){
                if(tmp.count(k-1)) tmp.erase(k-1);
                rep(i,N){
                    if(a[i][k-1]>a[i][k]) ok = false;
                }
            }
            if(k!=M-1){
                if(tmp.count(k)) tmp.erase(k);
                rep(i,N){
                    if(a[i][k]>a[i][k+1]) ok = false;
                }
            }
            if(tmp.size()!=0) ok = false;
            if(ok){
                l = min((ll)j,k); r = max((ll)j,k);
            }
            rep(i,N){
                swap(a[i][j],a[i][k]);
            }
