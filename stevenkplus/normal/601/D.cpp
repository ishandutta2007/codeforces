...
int main() {
  ...
  vector<int> label(n);
  // trie data -- tree-node i corresponds to trie rooted at trie-node i
  vector<map<int, int>> trie(n);
  vector<int> sizes(n, 1);

  for (int &i: val) cin >> i;
  string labelinp; cin >> labelinp;
  for (int i = 0; i < n; ++i) label[i] = labelinp[i] - 'a';
  for (int i = 0; i < n - 1; ++i) {
    int a, b;
    cin >> a >> b; --a, --b;
    ed[a].push_back(b);
  ...
  auto add = y_combinator([&](auto add, int cur, int ch, int node) -> void {
    int &nxt = trie[cur][ch];
    if (!nxt) {
      nxt = node;
...
// - 4/16/20 implementation error - confused var names when there's 2 trees (1 input tree, 1 parallel trie). also logical error; could've avoided this by reusing the node labels