#include<bits/stdc++.h>
using namespace std;
#define ll long long
#define db double
#define in inline
namespace fast_io
{
	char buf[1<<12],*p1=buf,*p2=buf,sr[1<<23],z[23],nc;int C=-1,Z=0;
	in char gc() {return p1==p2&&(p2=(p1=buf)+fread(buf,1,1<<12,stdin),p1==p2)?EOF:*p1++;}
	in ll read()
	{
		ll x=0,y=1;while(nc=gc(),(nc<48||nc>57)&&nc!=-1) if(nc==45) y=-1;
		x=nc-48;while(nc=gc(),47<nc&&nc<58) x=(x<<3)+(x<<1)+(nc^48);return x*y;
	}
	in db gf() {int a=read(),b=(nc!='.')?0:read(),c=ceil(log10(b));return (b?a+(db)b/pow(10,c):a);}
	in int gs(char *s) {char c,*t=s;while(c=gc(),c<32);*s++=c;while(c=gc(),c>32)*s++=c;return s-t;}
	template <typename T>
	in void write(T x,char t)
	{
		int y=0;if(x<0) y=1,x=-x;while(z[++Z]=x%10+48,x/=10);
		if(y) z[++Z]='-';while(sr[++C]=z[Z],--Z);sr[++C]=t;
	}
	in void write(char *s) {int l=strlen(s);for(int i=0;i<l;i++,*s++)sr[++C]=*s;sr[++C]='\n';}
	in void ot() {fwrite(sr,1,C+1,stdout);C=-1;}
};
using namespace fast_io;
const int N=1e6+5;
int n,k,b[N];
struct pii{
	int a,b;
	in bool operator<(pii x)const{return a^x.a?a>x.a:b<x.b;}
}a[N];
int main()
{
	int T=read();
	while(T--)
	{
		n=read(),k=read();
		for(int i=1;i<=n;i++) a[i].a=read(),a[i].b=read();
		sort(a+2,a+n+1);
		if(k>a[1].a+a[1].b) {write("No");continue;}
		if(k<=a[1].a) {write("Yes");continue;}
		b[n+1]=0,k-=a[1].b;int op=0;
		for(int i=n;i;i--) b[i]=max(b[i+1],a[i].a+a[i].b);
		for(int i=2;i<=n&&!op;i++)
		{
			if(b[i+1]>=k||a[i].a>=k) op=1;
			if(a[i].a+a[i].b>=k) k-=a[i].b;
		}
		op?write("Yes"):write("No");
	}
	return ot(),0;
}