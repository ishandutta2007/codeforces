...
void solve()
{
    ...
    forn(i, n) cin >> a[i + 1];
    n += 2;
    vector<double> tmL(n), tmR(n);
    a[n - 1] = L;
    tmL[0] = a[0];
    for (int i = 1; i < n; i++)
    {
        tmL[i] = tmL[i - 1] + 1.0 * (a[i] - a[i - 1]) / i;
    }
    tmR[n - 1] = 0;
    for (int i = n - 2; i >= 0; i--)
    {
        tmR[i] = tmR[i + 1] + 1.0 * (a[i + 1] - a[i]) / (n - i - 1);
    }
    //for (int i = 0; i < n; i++) printf("%.9f\n", tmL[i]); printf("\n");
    //for (int i = 0; i < n; i++) printf("%.9f\n", tmR[i]); printf("\n");
    for (int i = 0; i < n; i++)
    {
        if (abs(tmL[i] - tmR[i]) < eps)
        {
            printf("%.9f\n", tmL[i]);
            return;
        }
    ...
    for (int i = 0; i + 1 < n; i++)
    {
        ...
        double spL = i + 1;
        double spR = n - i - 1;
        if (tmL[i] - eps < tmR[j] && tmR[j] < tmL[j] + eps)
        {
            double si = a[j] - a[i];
            double T = tmR[j];
            si -= (T - tmL[i]) * spL;
        ...
        if (tmR[j] - eps < tmL[i] && tmL[i] < tmR[i] + eps)
        {
            double si = a[j] - a[i];
            double T = tmL[i];
            si -= (T - tmR[j]) * spR;
