...
int main() {
    ...
    int SCC;
    vi label;
    tie(SCC, label) = strongly_connected_components(adj);
    ...
    vi topo(N);
    F0R (i, N) topo[i] = i;
    sort(all(topo), [&](int a, int b) { return label[a] < label[b]; });

    vi min_reach(N, -1);
    trav (u, topo) {
        min_reach[u] = u;
        trav (v, adj[u]) {
            assert(min_reach[v] != -1);
            ckmin(min_reach[u], min_reach[v]);
        }
    }
    ...
    trav (u, topo) {
        min_hcaer[u] = u;
        trav (v, jda[u]) {
            assert(min_hcaer[v] != -1);
            ckmin(min_hcaer[u], min_hcaer[v]);
        }
    }
    ...
    stringstream ss;
    F0R (i, N) {
        int lo = min(min_reach[i], min_hcaer[i]);
        if (lo < i) ss << 'E';
        else {
            ans++;
            ss << 'A';
