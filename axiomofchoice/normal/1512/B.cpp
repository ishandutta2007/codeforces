...
void Solve(){
	...
	repeat(i,0,n){
		scanf("%s",s[i]);
		repeat(j,0,n)if(s[i][j]=='*')v.push_back({i,j});
	}
	int x1,y1,x2,y2;
	tie(x1,y1)=v[0];
	tie(x2,y2)=v[1];
	if(x1==x2){
		s[(x1+1)%n][y1]=s[(x2+1)%n][y2]='*';
	}
	else if(y1==y2){
		s[x1][(y1+1)%n]=s[x2][(y2+1)%n]='*';
	}
	else{
		s[x1][y2]=s[x2][y1]='*';
	}
