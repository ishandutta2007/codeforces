...
int main() {
	...
	const string MATCH = "abc";
	vector<vector<Mint>> dp = vector<vector<Mint>>(4, vector<Mint>(4, ZERO)), ndp;
	dp[0][0] = Mint(1, MOD107);
	...
	Mint ans = ZERO;
	for (int k = 0; k < min(Qcnt + 1, 4); k++) {
		ans += dp[3][k] * (Mint(3, MOD107)).pow(Qcnt - k);
	}
