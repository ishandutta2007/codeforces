...
fun factorSingleNumber(a:Long): List<Long> {
    val ret = mong
    var now = a
...
fun main(){
    ...
    solve.cases{
        ...
        val off = listOf(-1,0,1)
        var attempts = 0
        var max = Int.MAX_VALUE
        first@while(attempts <= 720){
            val r1= L.random() + off.random()
            val r2 = L.random() + off.random()
            if(r1 == 0L || r2 == 0L){
                continue
            }
            val test = gcd(r1,r2)
            val a = factorSingleNumber(test)
            for(p in a){
