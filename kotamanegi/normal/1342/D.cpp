...
vector<int> inputs;
int n, k;
vector<vector<int>> calc(int now) {
    vector<vector<int>> ans(now, vector<int>{});
    int limits = 0;
    for (int i = k; i >= 1; --i) {
        limits += inputs[i - 1] * now;
        if (limits - cnt[i] < 0) {
            ans.clear();
            return ans;
        }
        REP(q, cnt[i]) {
            ans[limits % now].push_back(i);
            limits--;
        }
    }
    return ans;
...
void solve() {
    ...
    for (int q = 0; q < inputs.size() - 1; ++q) {
        inputs[q] = inputs[q] - inputs[q + 1];
    }
    int bot = n;
    int top = 0;
    while (bot - top > 1) {
    ...
    cout << bot << endl;
    vector<vector<int>> ans = calc(bot);
    REP(i, ans.size()) {
        cout << ans[i].size();
        sort(ALL(ans[i]));
        REP(q, ans[i].size()) {
