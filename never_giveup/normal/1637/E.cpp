...
const int maxn = 3e5 + 10, maxm = 1e6 + 100, maxw = 1e7 + 10, inf = 1e9 + 100, LG = 18, SQ = 300;
...
void solve() {
    ...
    vector<int> has(n + 1);
    map<int, pair<vector<int>, set<int>>> go;
    for (int i = 0; i < m; i++) {
        int x, y;
        cin >> x >> y;
        if (mp[x] > mp[y])
            swap(x, y);
        go[x].fr.push_back(mp[y]);
        go[y].fr.push_back(mp[x]);
        go[y].sc.insert(x);
        go[x].sc.insert(y);
    }
    ll best = 0;
        ...
        if (q[c2].size() != 0) {
            ...
            for (int y : q[c2]) {
                ...
                for (int c1 : cands)
                    if (!has[c1]) {
//                        cerr << c1 << ' ' << c2 << ' ' << y << ' ' << q[c1][0] << '\n';
                        setmax(best, (c1 + c2) * (ll)(y + q[c1][0]));
                    } else {
                        for (int x : q[c1])
                            if (x != y && go[y].sc.find(x) == go[y].sc.end()) {
//                                cerr << c1 << ' ' << c2 << ' ' << y << ' ' << x << '\n';
                                setmax(best, (c1 + c2) * (ll)(y + x));
                                break;
                            }
                    }
