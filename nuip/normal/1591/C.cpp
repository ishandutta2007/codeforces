...
ll sol(vector<ll> vs, const int t){
	ll re=vs.back();
	reverse(all(vs));
	while(vs.size() && vs.back()==0) vs.pop_back();
	reverse(all(vs));
	for(int i=vs.size()%t;i<vs.size();i+=t)if(i) re+=2*vs[i-1];
	out(vs,t,re,1);
	return re;
}
...
int main(){
	...
	TT{
		cauto n=read();
		cauto t=read(0);
		cauto vs=readV(n)-sorted;
		vector<ll> pos{0},neg{0};
		for(cauto &v:vs)if(v>0) pos.eb(v); else neg.eb(-v);
		sort(all(pos));
		sort(all(neg));
		cout<<sol(pos,t)+sol(neg,t)+min(pos.back(),neg.back()) NL;
	}
	return 0;
}