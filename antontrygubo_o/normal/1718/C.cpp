...
void solve()
{
    ...
    vector<int> div;
    for (int i = 2; i<=n; i++) if (n%i == 0 && check_prime(i)) div.push_back(n/i);
    ...
    {
        vector<ll> tmp(it);
        for (int i = 0; i<n; i++) tmp[i%it]+=(1ll*it*a[i]);
        sums.push_back(tmp);

        multiset<ll> setik;
        for (auto it: tmp) setik.insert(it);
        sets.push_back(setik);
    }
    ...
    for (int iter = 0; iter<=q; iter++)
    {
        if (iter!=0)
        {
            ...
            for (int i = 0; i<k; i++)
            {
                sets[i].erase(sets[i].find(sums[i][p%div[i]]));
                sums[i][p%div[i]]+=1ll*div[i]*diff;
                sets[i].insert(sums[i][p%div[i]]);
            }
        }
