...
fun main(){
    solve.cases{
        val options = mutableListOf<String>("abc","acb","bac","bca","cab","bca")
        val n = getint()
        val a = getstr()
        val b = getstr()
        val bad = mutableListOf<String>("aa","bb","cc")
        if(!bad.contains(a) && !bad.contains(b)){
            val first = options.first{!it.contains(a) && !it.contains(b)}
            ...
            for(s in options){
                val newstr = List(n){s}.joinToString("")
                if(!newstr.contains(a) && !newstr.contains(b)){
                    put("YES")
                    put(newstr)
                    return@cases
                }
