...
void solve() {
    ...
    auto sum = [&](auto l, auto r) { return accumulate(l, r, 0, bit_xor<int>()); };

    vi ans;

    auto flip = [&](int i) {
        int res = sum(a.begin() + i, a.begin() + i + 3);
        fill(a.begin() + i, a.begin() + i + 3, res);
        ans.push_back(i + 1);
    };

    auto process = [&](int l, int r) {
        assert(sum(a.begin() + l, a.begin() + r) == 0);
        assert((r - l) % 2 == 1);

        for (int i = l; i + 3 <= r; i += 2)
            flip(i);
        for (int i = r - 3 - 2; i >= l; i -= 2)
            flip(i);

        assert(all_of(a.begin() + l, a.begin() + r, L1(u, u == 0)));
    };
