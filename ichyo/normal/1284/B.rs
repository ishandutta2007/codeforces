...
fn run() {
    ...
    for _ in 0..n {
        let m: usize = scan();
        let a: Vec<i32> = scan_iter().take(m).collect();
        ...
        if !ok {
            rest.push((*a.iter().min().unwrap(), *a.iter().max().unwrap()));
        }
    }
    let m = rest.len();
    ...
    max_nums.sort();
    let mut ans = n as u64 * n as u64 - m as u64 * m as u64;
    for &(x, _) in &rest {
        let idx = max_nums.upper_bound(&x);
        let cnt = max_nums.len() - idx;
        ans += cnt as u64;
    }
    println!("{}", ans);
}
