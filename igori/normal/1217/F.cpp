...
set<pair<int, int> > solve(int l, int r, set<pair<int, int> > q)
{
    set<pair<int, int> > dont_know;
    for (int i = l; i < r; i++)
    {
        if (t[i] == 1)
        {
            if (v[i] > u[i]) swap(v[i], u[i]);
            if (q.find({v[i], u[i]}) != q.end()) q.erase({v[i], u[i]}), dont_know.insert({v[i], u[i]});
            v[i] = (v[i] + 1) % n, u[i] = (u[i] + 1) % n;
            if (v[i] > u[i]) swap(v[i], u[i]);
            if (q.find({v[i], u[i]}) != q.end()) q.erase({v[i], u[i]}), dont_know.insert({v[i], u[i]});
            v[i] = (v[i] + n - 1) % n, u[i] = (u[i] + n - 1) % n;
        }
    }
    dsu_cut D(n);
    for (auto e : q)
        D.Merge(e.fi, e.se);
    int fix_sz = D.cuts.size();
    for (int i = l; i < r; i++)
    {
        if (t[i] == 1)
        {
            int V = v[i], U = u[i];
            if (lastans) V = (V + 1) % n, U = (U + 1) % n;
            if (V > U) swap(V, U);
            int t = 0;
            if (dont_know.find({V, U}) != dont_know.end()) t = 1;
            if (t) dont_know.erase({V, U});
            else dont_know.insert({V, U});
        }
        if (t[i] == 2)
        {
            for (auto e : dont_know) D.Merge(e.fi, e.se);
            int V = v[i], U = u[i];
            if (lastans) V = (V + 1) % n, U = (U + 1) % n;
            lastans = D.Root(V) == D.Root(U);
            cout << lastans;
            while (D.cuts.size() > fix_sz) D.Cut();
        }
    }
    for (auto e : dont_know) q.insert(e);
    return q;
}
