...
int main(){
    ...
    each(x,a){
        vector<tuplis>dp2;
        each(a,b,c,dp){
            dp2.push_back({x,b,c+(a!=x)});
            if(dp2.back()[0]>dp2.back()[1])swap(dp2.back()[0],dp2.back()[1]);
            dp2.push_back({a,x,c+(b!=x)});
            if(dp2.back()[0]>dp2.back()[1])swap(dp2.back()[0],dp2.back()[1]);
        }
        sort(all(dp2),[](const auto& a,const auto& b){if(a[2]!=b[2])return a[2]>b[2];return a<b;});
        dp2.erase(unique(all(dp2)),dp2.end());
        dp2.resize(32);
        dp=move(dp2);
    }
    out(dp[0][2]);
