...
mint DP(int l, int x, int y)
{
    if(vis[l][x][y]) return dp[l][x][y];
    vis[l][x][y] = 1;
    if(x == 0) return dp[l][x][y] = 1;
    if(y == 0) return dp[l][x][y] = 0;
    dp[l][x][y] = inv2 * (DP(l, x - 1, y) + DP(l, x, y - 1));
    return dp[l][x][y];
}

int dep[222], fa[222], sz[222];
