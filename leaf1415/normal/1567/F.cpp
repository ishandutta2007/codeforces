...
int main(void)
{
	...
	rep(y, 0, h-1) rep(x, 0, w-1){
		...
		rep(i, 0, 3){
			...
			if(c[nx][ny] != 'X') vec.push_back(ny*w+nx);
		}
		if(sz(vec) % 2){
			no();
			return 0;
		}
		reps(i, vec) G[vec[i]].push_back(vec[(i+1)%sz(vec)]);
		reverse(all(vec));
		reps(i, vec) G[vec[i]].push_back(vec[(i+1)%sz(vec)]);
	}
	...
	rep(y, 0, h-1) rep(x, 0, w-1){
		ll v = y*w+x;
		if(c[x][y] != 'X' && color[v] == 0) dfs(v, 1);
	}
	rep(y, 0, h-1) rep(x, 0, w-1){
		ll v = y*w+x;
		if(color[v] == 1) ans[x][y] = 1;
		if(color[v] == -1) ans[x][y] = 4;
	}
	rep(y, 0, h-1) rep(x, 0, w-1){
		ll v = y*w+x;
		if(color[v]) continue;
		rep(i, 0, 3){
			ll nx = x + dx[i], ny = y + dy[i];
			if(c[nx][ny] != 'X') ans[x][y] += ans[nx][ny];
		}
	}
