#line 1 "codeforces-workspace\\contest\\1610\\b\\b.cpp"
...
#line 13 "codeforces-workspace\\contest\\1610\\b\\b.cpp"
...
void main() {
  ...
  vector<i32> cand{0};
  for (auto i = 0; i < n - 1 - i; ++i) {
    if (a[i] != a[n - 1 - i]) {
      cand.emplace_back(a[i]);
      cand.emplace_back(a[n - 1 - i]);
      break;
    }
  }
  ...
  for (auto e : cand) {
    auto b = a;
    b.erase(remove(begin(b), end(b), e), end(b));
    auto c = b;
