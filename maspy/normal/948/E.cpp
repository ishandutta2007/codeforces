...
struct Mono {
  ...
  static X op(X x, X y) {
    if (x.fi == -1) return y;
    if (y.fi == -1) return x;
    if (y.fi == 0) return {x.fi, x.se + y.se};
    return {x.fi + y.fi, y.se};
  }
  static constexpr X unit = X(-1, -1);
  static constexpr bool commute = false;
};

void solve() {
  STR(S, T);
  SegTree<Mono> seg1(len(S)), seg2(len(T));
  FOR(i, len(S)) {
    if (S[i] == 'A')
      seg1.set(i, {0, 1});
    else
  ...
  auto check = [&](ll b1, ll a1, ll b2, ll a2) -> bool {
    if (b1 > b2) return false;
    if ((b2 - b1) % 2 != 0) return false;
    if (a1 < a2) return false;
    // A -> BB 
    if ((a1 - a2) % 3 == 0 && (b1 > 0 || b1 == b2)) return true;
    // A -> BB 
    return a1 > a2 && b1 < b2;
  };
  LL(Q);
  ...
  FOR_(Q) {
    LL(a, b, c, d);
    auto [x1, y1] = seg1.prod(--a, b);
    auto [x2, y2] = seg2.prod(--c, d);
    ANS += (check(x1, y1, x2, y2) ? '1' : '0');
  }
  print(ANS);
}
