...
void solve(){
    ...
    REP(i, n) {
        ...
        int now_dp_min = 1e9;
        for (int j = 0; j <= m; ++j) {
            if (inputs[i].first - j - 1 >= 0) {
                now_dp_min = min(now_dp_min, dp[i][inputs[i].first - j - 1]);
            }
            int cost = max(0LL, j - inputs[i].second);
            dp[i + 1][inputs[i].first + j] = min(dp[i + 1][inputs[i].first + j], now_dp_min + cost);
        }
        for (int q = 299999; q >= 1; --q) {
            dp[i + 1][q - 1] = min(dp[i + 1][q - 1], dp[i + 1][q]);
        }
    }
    cout << dp[n][m] << endl;
}
