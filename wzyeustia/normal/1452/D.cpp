...
int main(int argn, char **argv) {
    ...
    get(n);
    vector<ll> memo(n+1, (ll)0);
    vector<ll> prefix(n+1, (ll)0);
    // REP(i, n+1) {
    //     memo[i] = 0;
    // }
    vector<bool> calculated(n+1, false);

    ll y = fastPow(2, n, modNum);
    ...
    FOR(i, 2, n) {
        ...
        //     memo[i] += memo[i-j];
        // }
        prefix[i] = (prefix[i - 2] + memo[i]) % modNum;
    }
    // show("y = ", y);
    // showArr("memo:", memo, n+1);

    ll divider = gcd(memo[n], y);
    memo[n] /= divider;
    y /= divider;

    ll yReverse = getReverse(y, modNum);

    // show("divider = ", divider);
    // show("yReverse = ", yReverse);

    cout << (memo[n] * yReverse) % modNum << endl;
}