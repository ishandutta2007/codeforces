...
void solve() {
  ...
  vv(mint, POWc, 6, N + 2);
  FOR(x, N + 1) POW[0][x] = 1;
  FOR(k, 5) FOR(x, N + 1) POW[k + 1][x] = POW[k][x] * mint(x);
  FOR(k, 6) FOR(x, N + 1) POWc[k][x + 1] = POWc[k][x] + POW[k][x];
  ...
  SegmentTree<E, OP> seg(
    [&](E x, E y) {
      ...
      z.L = x.L;
      z.R = y.R;
      FOR(k, 6) z.data[k] = x.data[k] + y.data[k];
      return z;
    },
    [&](E x, OP y) {
      E z;
      z.L = x.L, z.R = x.R;
      FOR(k, 6) z.data[k] = (POWc[k][z.R] - POWc[k][z.L]) * mint(y);
      return z;
    },
    [&](OP x, OP y) {
      if (y == -1) return x;
