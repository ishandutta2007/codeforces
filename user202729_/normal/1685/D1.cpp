...
// ...
// actually, proving correctness is a bit difficult, but alright this looks correct.
...
void up(){
	...
	std::vector<int> color(number, -1);
	{
		int lastColor=-1;
		for(int const index: range(0, number)) if(color[index]==-1){
			++lastColor;
			int x=index;
			do{
				color[x]=lastColor;
...
continue_outer:
		for(int const index: range(1, number)){
			if(color[index-1]!=color[index]){
				// swap two arbitrary item, join the cycle
				for(auto& a: p){
				...
				// merge the color
				auto a=color[index-1], b=color[index];
				for(auto& x: color) if(x==a) x=b;
...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1685/problem/D1