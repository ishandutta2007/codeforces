...
struct max_t {
  int val;
  static const int null_v = -100000000;
...
int main() {
  ...
  f0r(i, 1000000) t.pb('a');

  int pos = 0, cnt = 0, batch = 0;
  ...
  f0r(i, n) {
    if (s[i] == 'L') {
      ...
      st.update(pos, 999999, ne - old);
      mt.update(pos, 999999, ne - old);
      ...
      if (cnt != 0) {
        ...
        ll v = mt.query(0, 999999).val;

        if (v < -1) v = -1;
        if (v > -1) {
          v = st.query(0, 999999).val;
        }
        batch = v;
      }
    }
