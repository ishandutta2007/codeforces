...
ll Pow( ll a , ll b , const ll p = MOD ){
	ll ret = 1;
	for( ; b ; a = a*a%p, b >>= 1LL )
...
int main(){
	...
	Read(n);
	For(i,n) Read(val[i],msk[i]);
	ll sum = accumulate(val+1,val+1+n,0LL);
	if( sum < 0 ){ for_each(val+1,val+1+n,[]( ll &x ){ x = -x; }); sum *= -1; }
	// O(sum);
	// Print(val,1,n);
	...
	for( ll k = 0 ; k < 63 ; ++k ){
		ll tsum = 0;
		For(i,n) if( (msk[i]>>k&1LL) and (msk[i]<(1LL<<k+1)) ) tsum += val[i];
		if( tsum > 0 ){
			// O(k);
			ans |= 1LL<<k;
			For(i,n) if( (msk[i]>>k&1LL) ){
				sum -= 2*val[i];
				val[i] = -val[i];
			}
		}
