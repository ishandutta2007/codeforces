...
int eval(array<int,3> x, int y) {
	return BIG-(x[0]*y*y+x[1]*y+x[2]);
}

int get(int a, int b) {
	return st[a]+b-(bound[a].f-1);
}

int main() {
    setIO(); re(n,m); f.rsz(n);
    F0R(i,n) re(f[i]);
    int nex = 2;
    F0R(i,n) {
    	re(bound[i]);
    	st[i] = nex++; 
    	D.addEdge(0,st[i],INF);
    	FOR(j,bound[i].f,bound[i].s+1) {
    		D.addEdge(nex-1,nex,eval(f[i],j));
    		nex ++;
    	}
    	D.addEdge(nex-1,1,INF);
    }
    F0R(i,m) {
    	int u,v,d; re(u,v,d); u--,v--;
    	FOR(j,bound[u].f-1,bound[u].s) {
    		if (j-d >= bound[v].f-1) {
    			if (j-d >= bound[v].s) D.addEdge(get(u,j),1,INF);
    			else D.addEdge(get(u,j),get(v,j-d),INF);
    		}
    	}
    }
    ps(BIG*n-D.maxFlow(0,1));
}
