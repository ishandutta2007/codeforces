...
bool used[101010][2];
int n;
double go(int cur, int x, int first) {
    if (cur == n - 1) {
        int t = (1 - x) + (1 - first);
        return t * pr[cur];
    }
    ...
    res = 0;
    if (x) {
        res = (go(cur + 1, 1, first) + 1) * pr[cur] + go(cur + 1, 0, first) * (1 - pr[cur]);
    } else {
        res = (go(cur + 1, 1, first) + 2) * pr[cur] + go(cur + 1, 0, first) * (1 - pr[cur]);
    }
    used[cur][x] = true;
    return res;
}
...
//        DBN(i, pr[i]);
    ...
    long double ans = (go(1, 1, 1) + 2) * pr[0];
    for (int i = 0; i < n; ++i) {
