...
int n,a[N],b[N],q;
i64 sum;
void dn(int x){ sum-=max(a[x]-b[x],0),--b[x],sum+=max(a[x]-b[x],0); }
void up(int x){ sum-=max(a[x]-b[x],0),++b[x],sum+=max(a[x]-b[x],0); }
map<pii,int> O;
int main(){
	scan(n);
	read(a+1,a+n+1);
	...
	while(q--){
		int s,t,u;
		scan(s,t,u);
		if(O.count(mp(s,t))) dn(O[mp(s,t)]),O.erase(mp(s,t));
		if(u) up(O[mp(s,t)]=u);
		print(sum),putc('\n');
	}
}