#line 1 "codeforces-workspace\\contest\\1550\\a\\a.cpp"
...
#line 11 "codeforces-workspace\\contest\\1550\\a\\a.cpp"
...
#line 35 "codeforces-workspace\\contest\\1550\\a\\a.cpp"
...
void main() {
  ...
  const auto inf = u32(-1) / 2;

  static auto dp = [&] {
    const auto S = 5000;
    ndvec dp({S + 1, S + 1}, inf);
    iota(begin(dp[1]) + 1, end(dp[1]), 1);
    for (auto i : range(2, S + 1)) {
      for (auto j : range(i, S + 1)) {
        dp[i][j] = min({dp[i][j - i], dp[i - 1][j - i], dp[i - 2][j - i]}) + 1;
      }
    }
    return dp;
  }();
