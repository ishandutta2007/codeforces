...
const lli maxN = 5000L;
    ...
    string s;
    vi a;
    lli dp[maxN+5][maxN+5],nxt[maxN+5];
    //priority_queue < ii , vector < ii > , CMP > pq;// min priority_queue .

lli solve(lli i,lli k)
{
    if(i==n||k==0)
        return 0;
    lli &ans=dp[i][k],j;
    ...
    // while(j<n&&a[j]-a[i]<=5)
    // {
    //     ans=max(ans,j-i+1+solve(j+1,k-1));
    //     j++;
    // }
    j=nxt[i];//lower_bound(a.begin(),a.end(),a[i]+6)-a.begin();
    dbg(i,j);
    ans=max(ans,j-i+solve(j,k-1));
...
// cin>>T;while(T--)
{
    ...
    cin>>n>>k;
    a.clear();a.reserve(n);
    memset(dp,-1,(maxN+5)*(maxN+5)*sizeof(lli));
    fo(i,n)
    {
        cin>>in;
        a.pb(in);
