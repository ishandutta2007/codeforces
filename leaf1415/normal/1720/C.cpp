	...
	while(T--){
		...
		rep(y, 1, h) rep(x, 1, w){
			if(c[x][y] == '1') cnt++;
				...
				if(x+1 <= w && c[x+1][y] == '0') flag = true;
				if(y+1 <= h && c[x][y+1] == '0') flag = true;
			}
		}
		if(flag){
			outl(cnt);
		...
		bool dflag = false, zflag = false;
		rep(y, 1, h) rep(x, 1, w){
			if(c[x][y] == '1') continue;
			zflag = true;
			if(x+1 <= w && y+1 <= h && c[x+1][y+1] == '0') dflag = true;
			if(x-1 >= 0 && y+1 <= h && c[x-1][y+1] == '0') dflag = true;
		}
		if(dflag) cnt -= 0;
		else if(zflag) cnt -= 1;
		else cnt -= 2;
