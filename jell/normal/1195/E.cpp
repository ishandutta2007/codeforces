...
void solve() {
    for(int i=0; i<n*m; ++i,g=(g*x+y)%z) {
        gs[i/m][i%m]=g;
    }
    for(int i=0; i<m; ++i) {
        for(int j=0; j<a; ++j) {
            while(!mp[i].empty() && mp[i].back().fir>gs[j][i]) {
                mp[i].pob();
            }
            if(!mp[i].empty() && mp[i].back().fir==gs[j][i]) {
                mp[i].back().sec++;
            } else {
                mp[i].emb(gs[j][i],1);
            }
    ...
    for(int i=0; i+a<=n; ++i) {
        ...
        for(int j=0; j<b; ++j) {
            ...
            mi=fro.fir;
            cnt=fro.sec;
            while(!stk.empty() && stk.back().fir>mi) {
                stk.pop_back();
            }
            if(!stk.empty() && stk.back().fir==mi) {
                stk.back().sec+=cnt;
            } else {
                stk.emb(mi,cnt);
            }
        ...
        for(int j=0; j<m; ++j) {
            if(gs[i][j]==mp[j].front().fir) {
                if(!--mp[j].front().sec) mp[j].pof();
            } 
            if(i+a<n) {
                while(!mp[j].empty() && mp[j].back().fir>gs[i+a][j]) {
                    mp[j].pob();
                }
                if(!mp[j].empty() && mp[j].back().fir==gs[i+a][j]) {
                    mp[j].back().sec++;
                } else {
                    mp[j].emb(gs[i+a][j],1);
                }
