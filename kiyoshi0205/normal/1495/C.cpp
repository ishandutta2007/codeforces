...
int main(){
  ...
  rep(i,N){
    cin>>v[i];
    if(i%3==0)rep(j,M)v[i][j]='X';
  }
  unionfind dsu(N*M);
  rep(i,N)rep(j,M)if(v[i][j]=='X'){
    if(i&&v[i-1][j]=='X')dsu.unite(i*M+j,(i-1)*M+j);
    if(j&&v[i][j-1]=='X')dsu.unite(i*M+j,i*M+j-1);
  }
  rep1(i,N){
    if(i%3||dsu.parcheck(i*M,(i-3)*M))continue;
    rep(j,M)if(v[i-1][j]=='X'||v[i-2][j]=='X'){
      v[i-1][j]=v[i-2][j]='X';
      break;
    }
    if(j==M)v[i-1][0]=v[i-2][0]='X';
  }
  if(N%3==0){
    rep(j,M)if(v[N-1][j]=='X')v[N-2][j]='X';
  }
