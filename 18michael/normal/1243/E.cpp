...
int main()
{
	...
	{
		read(n[i]);
		for(int j=1;j<=n[i];++j)read(x),vec[i].push_back(x),sum[i]+=x,num[i].push_back(++num_t),arr[num_t]=x,id[num_t]=num_t,to[num_t]=i;
		res+=sum[i];
	}
	...
	{
		ok=0;
		for(int j=0;j<n[i];++j)ok|=(deg[num[i][j]]>0);
		if(!ok)return 0&puts("No");
	}
	for(int i=1,x;i<=k;++i)
		for(int j=0;j<n[i];++j)
			if(deg[num[i][j]]>0 && !col[num[i][j]])
			{
