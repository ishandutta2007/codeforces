...
int hopcroftKarp(vector<vi>& g, vi& btoa) {
 ...
 for (;;) {
  ...
  for(li a = 0; a < (li)(g.size()); ++a) if(A[a] == 0) cur.push_back(a);
...
int main(){
  ...
  vi L,R;
  for(li i = 0; i < (li)(n); ++i) {
    if(A[i] > m/3) L.emplace_back(A[i]);
    else R.emplace_back(A[i]);
  }

  li nl = L.size(), nr = R.size();
  vvi G(nl);
  vi btoa(nr, -1);
  for(li i = 0; i < (li)(nl); ++i) for(li j = 0; j < (li)(nr); ++j) if(L[i]%R[j] == 0 && 2*L[i]+R[j] <= m) G[i].emplace_back(j);
  int cnt = hopcroftKarp(G,btoa);
  if(cnt != nl) {
    cout << -1 << endl;
    return 0;
  }
  do { cerr << "g.cpp" ":" "74" "  {" << "L,R" << "} = {"; debug_impl_seq(L,R); cerr << endl; } while(0);
  vi atob(nl);
  for(li i = 0; i < (li)(nr); ++i) if(btoa[i] != -1) atob[btoa[i]] = i;
  vii ans;
  for(li i = 0; i < (li)(nl); ++i) ans.emplace_back(2*L[i] + R[atob[i]], L[i] + R[atob[i]]);
  for(li i = 0; i < (li)(nr); ++i) if(btoa[i] == -1) ans.emplace_back(3*R[i],2*R[i]);
  cout << ans.size() << endl;
