...
void solve() {
  LL(a, b, c, d, e, f);
  ll N = -1;
  FOR(x, 1000) if (x * x == a * b + c * d + e * f) N = x;
  if (N == -1) return print(-1);

  string S = "ABC";
  FOR(_, 3) {
    tie(a, b, c, d, e, f) = mt(c, d, e, f, a, b);
    ...
    if (c == H && e == H) {
      print(N);
      vc<string> ANS(N);
      FOR(i, N) ANS[i].assign(N, '.');
      FOR(i, a) FOR(j, N) ANS[i][j] = S[0];
      FOR3(i, a, N) FOR(j, N) ANS[i][j] = (j < d ? S[1] : S[2]);
      FOR(i, N) print(ANS[i]);
      return;
    }
  }
