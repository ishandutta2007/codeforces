...
void solve() {
  ...
  REP(i, n) {
    ...
    for (int j = i; !is_visited[j]; j = p[j]) {
      is_visited[j] = true;
      union_find.unite(j, p[b[j]]);
    }
  }
  fill(ALL(is_visited), false);
  REP(i, n) {
    if (is_visited[i]) continue;
    for (int j = i; !is_visited[j]; j = q[j]) {
      is_visited[j] = true;
      union_find.unite(j, q[a[j]]);
    }
  }
  vector<int> d(n); REP(i, n) cin >> d[i], --d[i];
  map<int, int> m;
  REP(i, n) m[union_find.root(i)] = -1;
  REP(i, n) {
    if (d[i] != -1) {
      if (m[union_find.root(i)] != -1) {
        if (m[union_find.root(i)] != (d[i] == a[i])) {
          cout << 0 << '\n';
          return;
        }
      } else {
        m[union_find.root(i)] = (d[i] == a[i]);
      }
    }
  }
  map<int, bool> is_fixed;
  REP(i, n) is_fixed[union_find.root(i)] = true;
  REP(i, n) is_fixed[union_find.root(i)] &= (a[i] == b[i]);
  int fr = 0;
  for (const auto [root, flag] : m) fr += (flag == -1 && !is_fixed[root]);
  cout << ModInt(2).pow(fr) << '\n';
}
