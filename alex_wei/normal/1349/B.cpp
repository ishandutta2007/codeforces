...
void solve(){
	cin>>n>>k; bool hv=0;
	for(int i=1;i<=n;i++)cin>>a[i],hv|=(a[i]==k);
	if(!hv){puts("no"); return;}
	if(n==1){puts("yes"); return;}
	for(int i=2;i<n;i++)if(a[i+1]>=k&&a[i-1]>=k){puts("yes"); return;}
	for(int i=1;i<n;i++)if(a[i]>=k&&a[i+1]>=k){puts("yes"); return;}
	puts("no");
}
