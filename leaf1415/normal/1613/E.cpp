...
int main(void)
{
	...
	while(T--){
		...
		rep(y, 0, h-1) rep(x, 0, w-1){
			cin >> c[y*w+x];
			if(c[y*w+x] == 'L') sx = x, sy = y;
		}
		
		queue<P> Q;
		Q.push(P(sx, sy));
		rep(y, 0, h-1) rep(x, 0, w-1) used[y*w+x] = false;
		used[sy*w+sx] = true;
		...
		while(sz(Q)){
			...
			rep(i, 0, 3){
				...
				if(c[ny*w+nx] == '#' || used[ny*w+nx]) continue;
				ll cnt = 0;
				rep(j, 0, 3){
					ll nnx = nx + dx[j], nny = ny + dy[j];
					if(nnx < 0 || nnx >= w || nny < 0 || nny >= h) continue;
					if(c[nny*w+nnx] == '#' || used[nny*w+nnx]) continue;
					cnt++;
				}
				if(cnt <= 1){
					used[ny*w+nx] = true;
