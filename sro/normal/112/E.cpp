...
inline void dfs(int now,int i,int j,int k,int cnt,int jd){
	if(now==n){
		dp[i][k][jd]=min(dp[i][k][jd],cnt);
		return;
	}
	dfs(now+1,i,j,k|l2r(now),cnt+1,jd|MASK(now));
	if(BIT(j,now)){
		dfs(now+1,i,j,k,cnt,jd);
	}
}

inline int solvelst(int I,int J){
	int ans=0;
	for(int i=0;i<n;i++){
		if(!BIT(I,i)){
			I|=MASK(i);
...
int main(){
	...
	for(int i=0;i<MASK(n);i++){
		for(int j=0;j<MASK(n);j++){
			ans=min(ans,dp[m][i][j]+solvelst(i,j));
		}
	}
	fio<<n*m-ans<<endl;
	return 0;
