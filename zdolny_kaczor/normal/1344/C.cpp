...
vi graf1[nax], graf2[nax];
char vis1[nax], vis2[nax];
void dfs(char vis[], vi graf[], int x) {
	if (vis[x] == 2) return;
	if (vis[x] == 1) cycle();
	vis[x] = 1;
	for (int v : graf[x]) dfs(vis, graf, v);
	vis[x] = 2;
}
char ans[nax];
int main() {
	...
	REP(_, m) {
		int a, b;
		scanf("%d%d", &a, &b);
		graf1[a].emplace_back(b);
		graf2[b].emplace_back(a);
	}
	int as = 0;
	for (int i = 1; i <= n; ++i) {
		if (!vis1[i] && !vis2[i]) {
