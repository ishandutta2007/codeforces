...
int main() {
    ...
    auto find = [&](int x, int y) {
        auto it = s.lower_bound(tuple<int, int, int>(x, y, 0));
        if(it == s.end() or get<0>(*it) != x or get<1>(*it) != y) return 0;
        return get<2>(*it) + 1;
    };
    auto cnt = [&](int x, int y) {
        int res = 0;
        rep2(i, -1, 1) res += !!find(x + i, y - 1);
        return res;
    };
    auto check = [&](int x, int y) {
        rep2(i, -1, 1) {
            if(find(x + i, y + 1)) {
                if(cnt(x + i, y + 1) == 1) return false;
            }
        }
        return true;
    };
    for(auto [x, y, i] : s)
        if(check(x, y)) p.emplace(i);
    int t = 1;
    while(!p.empty()) {
        int now;
        if(t)
            now = *p.rbegin(), p.erase(--p.end());
        else
            now = *p.begin(), p.erase(p.begin());
        t ^= 1;
        ans = ans * m + now;
        auto [x, y] = v[now];
        s.erase(T(x, y, now));
