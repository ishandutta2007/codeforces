...
fun main(){
    solve.cases{
        ...
        val alive = TreeSet<Int>()
        for(i in 0 until n){
            alive.add(i)
        }
        val keeper = interval_incomparable(true)
        ...
        repeat(q){
            ...
            if(t == 0){
                ...
                if(!alive.contains(i)){
                    ...
                    val leftupto = (alive.floor(i-1) ?: nlarge ) + 1
                    val rightupto = (alive.ceiling(i+1) ?: plarge) - 1
                    val bestchoice = keeper.t.ceilingEntry(leftupto)
                    if(bestchoice != null && bestchoice.value <= rightupto){
                        put("YES")
                        return@repeat
                    }
                    put("N/A")
