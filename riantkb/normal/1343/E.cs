...
class Solver {
    ...
    public void solve() {
        ...
        {
            int n, m, a, b, c;
            sc.Multi(out n, out m, out a, out b, out c);
            --a;
            --b;
            --c;
            ...
            var da = calcdist(edge, a);
            var db = calcdist(edge, b);
            var dc = calcdist(edge, c);
            long ans = LM;
            for (int i = 0; i < n; i++)
            {
                if (da[i] + db[i] + dc[i] <= m) {
                    ans = Math.Min(ans, ims[db[i]] + ims[da[i] + db[i] + dc[i]]);
                }
            }
            Prt(ans);
        }
    }
    int[] calcdist(List<int>[] edge, int s) {
        int n = edge.Length;
        var dist = new int[n];
        for (int i = 0; i < n; i++)
        {
