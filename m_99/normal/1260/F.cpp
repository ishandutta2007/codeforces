...
Data mapping(pair<mint,mint> a,Data b){
	b.ab += a.first * a.second * b.sz; 
	b.ab += a.first * b.bs;
	b.ab += a.second * b.as;
	b.as += a.first * b.sz;
	b.bs += a.second * b.sz;
...
int main(){
	...
	cin>>n;
	vector<int> l(n),r(n);
	vector<vector<int>> Add(100006),Del(100006);
	...
	vector<Data> v(n);
	rep(i,n){
		if(i==H.pos[0])continue;
		Data d;
		d.sz = 1;
		v[i] = d;
	}

	lazy_segtree<Data,op,e,pair<mint,mint>,mapping,composition,id> seg(v);		
	...
	rep(i,100006){
		...
		rep(j,Add[i].size()){
			int ind  = Add[i][j];
			mint p = mint(r[ind]-l[ind]).inv();
			
			auto ret = H.query(0,ind);
			seg.apply(0,n,make_pair(0,p));
			rep(k,ret.size()){
				int ll = ret[k].first,rr = ret[k].second;
				if(ll>rr)swap(ll,rr);
				rr++;
				seg.apply(ll,rr,make_pair(p,-p));
			}
