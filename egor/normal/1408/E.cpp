...
class EIzbegaiteRaznotsvetnikhTsiklov {
public:
    void solve() {
        int m = in.readInt();
        int n = in.readInt();
        ...
        arr<arri> s(m);
        for (int i : range(m)) {
            s[i] = in.readIntArray(in.readInt());
            decreaseByOne(s[i]);
        }
        ...
        for (const auto& p : order) {
            ...
            } else if (!dsu.join(p.second, spec[p.first])) {
                answer += a[p.first] + b[p.second];
            }
        }
...
int main() {
    ...
    auto time = clock();
#endif
    EIzbegaiteRaznotsvetnikhTsiklov solver;
