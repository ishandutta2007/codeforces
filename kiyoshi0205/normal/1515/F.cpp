...
int main(){
  ...
  vec v(N);
  rep(i,N)cin>>v[i];
  if(accumulate(all(v),0LL)<(N-1)*K){
    printNO;
    return 0;
  }
  printYES;
  vector<set<datas>> g(N);
  map<datas,ll> edgenum;
  set<ll> pr;
  unionfind dsu(N);
  rep(A,M){
    cin>>i>>j;
  ...
  rep(i,N){
    if(v[i]<K)continue;
    ll now=dsu.root(i);
    while(v[now]>=K&&g[now].size()){
      auto t=g[now].begin();
      //ab
      auto [a,b]=*t;
      g[now].erase(t);
      print(edgenum[minmax(a,b)]);
      pr.erase(edgenum[minmax(a,b)]);
      ll difroot=dsu.root(a);
      g[difroot].erase(datas(b,a));
      if(g[difroot].size()>g[now].size())swap(now,difroot);
      for(auto x:g[difroot])g[now].emplace(x);
      v[now]+=v[difroot]-K;
      v[difroot]=0;
      dsu.unite(difroot,now);
      difroot=dsu.root(now);
      if(now!=difroot){
        swap(v[now],v[difroot]);
        swap(g[now],g[difroot]);
        now=difroot;
      }
    }
  }
