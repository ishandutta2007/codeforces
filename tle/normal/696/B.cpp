...
#define SZ 666666
int n,nxt[SZ],fst[SZ],fa[SZ],sz[SZ];
void addc(int x,int f) {nxt[x]=fst[f]; fst[f]=x;}
ld vs[SZ];
void dfs1(int x)
{
	sz[x]=1;
	for(int c=fst[x];c;c=nxt[c])
	{
		dfs1(c); sz[x]+=sz[c];
	}
}
...
{
	scanf("%d",&n);
	for(int i=2;i<=n;i++) scanf("%d",fa+i), addc(i,fa[i]);
	dfs1(1); vs[1]=1;dfs2(1);
	for(int i=1;i<=n;i++) printf("%.6lf ",vs[i]);
}