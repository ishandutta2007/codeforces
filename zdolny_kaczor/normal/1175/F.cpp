...
struct tree {
	...
	int read(int l, int r, int wh = 1, int lb = 0, int rb = nax) {
		...
		if (l <= lb && r >= rb) return val[wh];
		return max(read(l, r, wh * 2, lb, (lb + rb) / 2), read(l, r, wh * 2 + 1, (lb + rb) / 2, rb));
	}
...
tree prevs, maxes;
int where[nax], a[nax];
bool repeats(int l, int r) {
	return prevs.read(l, r) >= l;
}
int main() {
	...
	REP(i, n)
		scanf("%d", a + i);
	REP(i, n) maxes.put(i, a[i]);
	REP(i, n) {
		prevs.put(i, where[a[i]]);
		debug << imie(i) arr(where, a[i]);
		where[a[i]] = i;
	}
	int ans = 0;
	REP(start, n) {
		...
		while (to <= n) {
			if (repeats(start, to)) {
				debug << "repeats " << range(a + start, a + to);
				break;
			}
			int MA = maxes.read(start, to);
			if (MA == to - start) {
				debug << "good " << range(a + start, a + to);
				ans++;
				to++;
			}
			else
