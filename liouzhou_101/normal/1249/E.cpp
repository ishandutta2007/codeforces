...
void solve()
{
	...
	segment_tree<node> Ha(n, initialize, merge, pushdown);
	segment_tree<node> Hb(n, initialize, merge, pushdown);
	Ha.modify(1, 1, [&](node &it, int L, int R) -> void
	{
		it = F[1];
	});
	Hb.modify(1, 1, [&](node &it, int L, int R) -> void
	{
		it = F[1];
	});
	for (int i = 2; i <= n; ++ i)
	{
		int fa = Ha.query(1, i-1)+sa[i-1];
		int fb = Hb.query(1, i-1)+sb[i-1]+c;
		F[i] = min(fa, fb);
		Ha.modify(i, i, [&](node &it, int L, int R) -> void
		{
			it = F[i]-sa[i-1];
		});
		Hb.modify(i, i, [&](node &it, int L, int R) -> void
		{
			it = F[i]-sb[i-1];
		});
	}
