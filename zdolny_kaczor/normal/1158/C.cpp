...
int turn = 0;
int pos = 1;
#define CURR (turn * 2 - 1)
#define DONE (turn * 2)
bool dfs(int x) {
	if (col[x] == CURR) return true;
	if (col[x] == DONE) return false;
	col[x] = CURR;
	for (int y : graf[x]) {
		debug << "go " imie(x) imie(y);
...
void split(int x, int l, int r, int wh = 1, int lb = 0, int rb = nax - 1) {
	if (l > rb || r < lb) return;
	if (l <= lb && r >= rb) {
		debug << x << "->" << wh;
		graf[x].push_back(wh);
	...
	int s = (lb + rb) / 2;
	split(x, l, r, wh * 2, lb, s);
	split(x, l, r, wh * 2 + 1, s + 1, rb);
}
void solve() {
	turn++;
	debug << imie(turn);
