...
int main()
{
    ...
    a1.a = -2 * (a[0].x - a[1].x);
    a1.b = -2 * (a[0].y - a[1].y);
    a1.c = sqr(a[0].x) + sqr(a[0].y) - sqr(a[1].x) - sqr(a[1].y);
    a2.a = -2 * (a[0].x - a[2].x);
    a2.b = -2 * (a[0].y - a[2].y);
    a2.c = sqr(a[0].x) + sqr(a[0].y) - sqr(a[2].x) - sqr(a[2].y);
    pt center;
    intersect(a1, a2, center);
    double radi = sqrt(sqr(center.x - a[0].x) + sqr(center.y - a[0].y));
    double phi[3];
    forn(i, 3)
    {
        a[i].x = (a[i].x - center.x) / radi;
        a[i].y = (a[i].y - center.y) / radi;
        phi[i] = atan2(a[i].x, a[i].y);
        if (phi[i] < 0)
            phi[i] += 2 * PI;
        phi[i] /= 2 * PI;
    }
    sort(phi, phi + 3);
    double delta1 = phi[1] - phi[0], delta2 = phi[2] - phi[1], delta3 = 1.0 - delta1 - delta2;
    int cur = 3;
    while (1)
    {
        int a = (delta1 * cur + 0.5), b = (delta2 * cur + 0.5), c = (delta3 * cur + 0.5);
        if (fabs(a - delta1 * cur) < 1e-3 && fabs(b - delta2 * cur) < 1e-3 && fabs(c - delta3 * cur) < 1e-3 && a + b + c == cur)
        {
            double ans = 0.5 * sqr(radi) * cur * sin(2 * PI / cur);
            printf("%.9f\n", ans);
            return 0;
        }
        ++cur;
