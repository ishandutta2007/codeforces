...
void bfs(int v, int n, pair<int, int> d[N]) {
    fill(d, d + n + 1, make_pair(N, 0));
    d[v] = {0, 1};
...
inline void _solve() {
    ...
    for(int i = 1; i <= n; ++i) {
        for(int j : g[i]) {
            if(d[1][i].x == d[1][j].x && d[0][i].x + 1 + d[1][j].x == d[0][t].x + 1) {
                addeq(ans, mul(d[0][i].y, d[1][j].y));
            }
        }
    }
    cout << ans << endl;
