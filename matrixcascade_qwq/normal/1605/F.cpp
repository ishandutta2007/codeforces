...
int pw[10101];
int f1[88][88],f2[88][88],dp[88][88];
int pwdn(int n,int m)
{
	int ans=1;
...
signed main()
{
	...
	while(T--)
	{
		...
		pre();
		pw[0]=1;
		up(i,1,10000)pw[i]=pw[i-1]*2%mod;
		up(i,0,80)
		{
			up(j,0,80)
			{
				//f1[i][j],i ge,2^j-1, or = 2^j-1
				up(k,0,j)//k wei wei 0
				{
					if((k)%2==0)f1[i][j]+=pw[i*(j-k)]*C(j,k)%mod,f1[i][j]%=mod;
					else f1[i][j]+=(mod-pw[i*(j-k)]*C(j,k)%mod),f1[i][j]%=mod;
				}
				up(k,0,j)
				{
					if(k%2==0)f2[i][j]+=C(j,k)*pwdn((pw[j-k]-1),i)%mod,f2[i][j]%=mod;
					else f2[i][j]+=(mod-C(j,k)*pwdn((pw[j-k]-1),i)%mod),f2[i][j]%=mod;
				}
//				cout<<i<<" "<<j<<" "<<f1[i][j]<<" "<<f2[i][j];edl;
			}
		}
		up(i,1,n)
		{
			up(j,1,m)
			{
				up(p,0,i-1)
				{
					up(q,0,j-1)
					{
						...
						val=((f1[p][q]-dp[p][q]+mod)%mod)*C(i,p)%mod*C(j,q)%mod*pw[(i-p)*q]%mod*f2[i-p][j-q]%mod;
						dp[i][j]+=val;dp[i][j]%=mod;
					}
				}
			}
		}
...
//		cout<<dp[2][2];edl;
		up(i,0,m)
		{
			ans+=(f1[n][i]-dp[n][i]+mod)%mod*C(m,i)%mod;ans%=mod;
		}
		cout<<ans;edl;
	}
}
