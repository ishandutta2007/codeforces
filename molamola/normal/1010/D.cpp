...
int eval(char c, int x, int y = -1) {
	if(c == 'N') return !x;
	else if(c == 'X') return x ^ y;
	else if(c == 'A') return x & y;
	else return x | y;
}
...
void Do(int x) {
	...
	int r[2], z = 0;
	for(int e : E[x]) r[z++] = val[e];
	int now = 0;
	for(int e : E[x]) {
		int save = r[now];
		rep(u, 2) {
			r[now] = u;
			ans[e][u] = ans[x][eval(C[x], r[0], r[1])];
		}
		r[now] = save;
		now++;
		Do(e);
...
int main() {
	...
	ans[1][0] = 0, ans[1][1] = 1;
	Do(1);
	for(int i=1;i<=N;i++) if(R[i]) printf("%c", R[i]);
	puts("");
	return 0;
}