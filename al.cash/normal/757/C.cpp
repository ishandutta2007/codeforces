...
int main() {
  ...
  REP(i, n) {
    ...
    vi pok(k);
    read(all(pok));
    auto f = group(all(pok));
    sort(all(f), [](pii x, pii y) { return x.Y < y.Y; });
    REP(i, sz(f)) {
      vi c;
      int j = i;
      while (j < sz(f) && f[i].Y == f[j].Y) c.pb(f[j].X-1), ++j;
      sort(all(c), [](int x, int y) { return g[x] < g[y]; });
      REP(u, sz(c)) {
        int v = u;
        int gu = g[c[u]];
        while (v < sz(c) && g[c[v]] == gu) g[c[v]] = kg, ++v;
        ++kg;
        u = v - 1;
      }
      i = j - 1;
  ...
  REP(i, m) ++c[g[i]];
  int ans = 1;
  for (auto p : c) ans = mul(ans, C.factorial(p.Y));
  writeln(ans);
