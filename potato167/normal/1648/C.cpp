...
void solve(){
	...
	ll tmp=1;
	rep(i,L){
		tmp=(table.fact_rev[seg.get(i)]*tmp)%mod;
	}
	vector<int> p(M);
	rep(i,M) cin>>p[i];
	ll ans=0;
	rep(i,M){
		ll v=(tmp*seg.query(0,p[i]))%mod;
		v=(v*table.fact[N-i-1])%mod;
		ans=(ans+v)%mod;
		if(seg.get(p[i])==0) break;
		tmp=(tmp*seg.get(p[i]))%mod;
		if(i==N-1&&M>N) ans=(ans+1)%mod;
		seg.addl(p[i],-1);
	}
	cout<<ans<<"\n";
}