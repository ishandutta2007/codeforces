...
// URL : http://codeforces.com/contest/1385/problem/F
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int main()
{
	for(int t=ri();t--;){
		...
		while(moze.size()){
			...
			for(int i=0;i<k;i++)
				done[cntLeaf[tr].back()]=2,cntLeaf[tr].pop_back();
			deg[tr]-=k;
			if(cntLeaf[tr].size()<k)
				moze.erase(moze.begin());
			if(deg[tr]==1){
				for(auto p:graf[tr]){
					if(!done[p]){
						cntLeaf[p].pb(tr);
