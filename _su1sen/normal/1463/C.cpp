...
int main() {
    ...
    loop(t) {
        input(int, n);
        vector<pair<long long, long long>> ops(n);
        read(ops);
        ops.emplace_back(numeric_limits<long long>::max() / 2, 0);
        ...
        while (i < n) {
            ...
            while (j < n) {
                auto [tj, xj] = ops[j];
                long long uj = ops[j + 1].first;

                if (ti + dt <= tj) break;

                if (x <= xj and xj <= xi) {
                    long long t = ti + xj - x;
                    if (tj <= t and t <= uj) ++ans;
                } else if (xi <= xj and xj <= x) {
                    long long t = ti + x - xj;
                    if (tj <= t and t <= uj) ++ans;
                }
                ++j;
