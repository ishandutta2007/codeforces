...
namespace sg2
{
    ...
    int query(int lk,int rk,int l,int r,int L)
    {
        // cout<<"??"<<lk<<" "<<rk<<" "<<l<<" "<<r<<" "<<L<<" "<<tr[lk]<<" "<<tr[rk]<<endl;
        if (r<L) return 0;
        if (lk==rk) return 0;
        if (!rk) return 0;
        if (L<=l)
...
void BellaKira()
{
    ...
    for (int i=1;i<=q;i++)
    {
        ...
        while (tp)
        {
            ...
            if (!res[sta[tp]])
            {
                ...
                } else
                {
                    ...
                    nowfl[sta[tp]]=t;
                    // cout<<"???"<<now<<" "<<sg1::query(rt[lstl],rt[now],0,B,nowt)+sg2::query(rt1[lstl],rt1[now],0,B,nowt+1)*nowt<<" "<<h<<endl;
                    if (now>1)
                    {
                        sta[++tp]=now-1;
                        nowfl[sta[tp]]=t;
        ...
        // for (int j=tp;j>=1;j--) cout<<sta[j]<<",";
        // cout<<endl;
        // cout<<"?"<<h<<" "<<nowfl[sta[tp]]<<endl;
        // cout<<endl;
        // cout<<"?"<<h<<endl;
        ans+=h;
    }
