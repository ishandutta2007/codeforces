...
fn run() {
    ...
    let f = f;
    let factorize = |mut a: usize| -> Vec<(usize, usize)> {
        let mut res = vec![];
        while a > 1 {
            let p = f[a];
            let mut c = 0;
    ...
    let mut map = vec![std::collections::BTreeMap::<usize, usize>::new(); n];
    let mut set = vec![std::collections::BTreeSet::new(); m + 1];
    ...
    for (i, set) in set.iter().enumerate() {
        if set.len() == n {
            ans *= M::from(i).pow(set.iter().next().unwrap().0 as u64);
        }
    }
    ...
    for (i, x) in ask {
        ...
        for (p, c) in factorize(x) {
            let set = &mut set[p];
            if set.len() == n {
                ans *= M::from(p).pow(set.iter().next().unwrap().0 as u64).inv();
            }
            let po = map.entry(p).or_insert(0);
            set.remove(&(*po, i));
            *po += c;
            set.insert((*po, i));
            if set.len() == n {
                ans *= M::from(p).pow(set.iter().next().unwrap().0 as u64);
            }
        }
        writeln!(out, "{}", ans).ok();
    }
