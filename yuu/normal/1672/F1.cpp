...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        if (!pos[i].empty()) q.emplace(pos[i].size(), i);
    order.clear();
    while (!q.empty()) {
        int now = order.empty() ? 0 : a[order.back()];
        auto [c, u] = q.top();
        q.pop();
        if (now && (!pos[now].empty())) q.emplace(pos[now].size(), now);
        order.push_back(pos[u].back());
        pos[u].pop_back();
    }
    for (int i = 1; i < order.size(); i++) swap(a[order[i]], a[order[i - 1]]);
