...
//	FastIO::OutputWriter out(stdout);
	cin >> n;
	F(i, 0, n){
		F(j, 0, n){cin >> likes[i][j]; --likes[i][j]; ps[i][likes[i][j]] = j;}
	}
	F(i, 0, n){int x; cin >> x; --x; rv[i] = x; p[x] = i;}
	int m1 = n, m2 = n;
	F(i, 0, n){
		int curr = p[i];
	...
	F(j, 0, n){
		...
		int pos = -1;
		F(i, 0, n){
			//cout << i << ' ' << j << ' ' << send[j][i] << endl;
			if(send[j][i] == -1)cont;
			if(ps[j][ans] >= ps[j][send[j][i]]){ans = send[j][i]; pos = i;}
		}
		assert(pos != -1);
		cout << pos + 1 << ' ';
	}
