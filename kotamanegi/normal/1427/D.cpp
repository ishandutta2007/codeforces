...
#define int ll
vector<int> apply(vector<int> now, vector<int> D) {
	vector<vector<int>> geko;
	int itr = 0;
	REP(i, D.size()) {
		vector<int> tmp;
		REP(q, D[i]) {
			tmp.push_back(now[itr]);
			itr++;
		}
		geko.push_back(tmp);
	}
...
void solve() {
	...
	while (true) {
		...
		for (int i = 0; i < n; ++i) {
			int targets = 1;
			for (int q = i + 1; q < n; ++q) {
				if (inputs[q] != inputs[q - 1] + 1LL) break;
				targets++;
			}
			for (int q = i + 1; q < n; ++q) {
				if (inputs[q] == inputs[i] - 1) {
					//spinning!
					vector<int> hoge;
					if (i != 0) {
						hoge.push_back(i);
					}
					hoge.push_back(targets);
					int geko = n - (i + targets + (n - 1 - q));
					hoge.push_back(geko);
					if (q != n - 1) {
						hoge.push_back(n - 1 - q);
					}
					ans.push_back(hoge);
					inputs = apply(inputs, hoge);
					ok = 1;
					break;
				}
			}
