...
#line 4 "/home/maspy/compro/library/graph/statictreemonoid.hpp"
...
struct Mono {
  ...
  static constexpr X unit() { return X({0, 0}); };
...
void solve() {
  ...
  vc<pi> dat(N);
  FOR(i, N) dat[i] = {1, A[i]};
  StaticTreeMonoid<decltype(hld), Mono, 0> TM(hld, dat);
  ...
  FOR_R(i, N) {
    ...
    ll& dp = DP[v];
    for (auto&& e: G[v]) {
      if (e.to != hld.parent[v]) chmax(dp, DP[e.to] - 1);
    }
    if (dp) continue;
    auto check = [&](pi e) -> bool { return e.fi <= L && e.se <= S; };
    ll to = TM.max_path(check, v, 0);
    if (to == -1) return print(-1);
    dp = hld.dist(v, to) + 1;
    ++ANS;
  }
  print(ANS);
