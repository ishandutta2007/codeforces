...
use crate::iters::shifts::shift_by_uldr;
#[allow(unused)]
...
fn solve(input: &mut Input, _test_case: usize) {
    ...
    let mut bbox = BoundingBox::new(&cur, &cur);
    for &dir in s.iter() {
        let shift = shift_by_uldr(dir);
        cur = cur.apply_shift(&shift);
        let prev_box = bbox.clone();
        bbox.add(&cur);
        if bbox.dx() + 1 > n || bbox.dy() + 1 > m {
            bbox = prev_box;
            break;
        }
    }
    let start_row = -bbox.min.x + 1;
    let start_col = -bbox.min.y + 1;
