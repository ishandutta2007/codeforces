	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			if(a[i] == 1){
				...
				es.add(new Edge(i, sink, 1, 0));
			}
			if(i+1 < n)es.add(new Edge(i, i+1, 9999, 1));
			if(i+1 < n)es.add(new Edge(i+1, i, 9999, 1));
		}
		long C = solveMinCostFlow(compileWD(sink+1, es), src, sink, n);
		out.println(C);
	}
