...
int main()
{
	...
	mint t = 1;
	rep(i, n){
		rep(j, 3) rep(k, 4) dp[i + 1][j][k] = dp[i][j][k];
		if (s[i] == 'a')
			dp[i + 1][0][0] += 1;
		else if(s[i]=='b')
			rep(k,2) dp[i + 1][1][k] += dp[i][0][k];
		else if(s[i]=='c')
			rep(k,3) dp[i + 1][2][k] += dp[i][1][k];
		else{
			dp[i + 1][0][1] += 1;
			rep(k,2) dp[i + 1][1][k + 1] += dp[i][0][k];
			rep(k,3) dp[i + 1][2][k + 1] += dp[i][1][k];
			t *= 3;
		}
	}
	cout << t*(dp[n][2][0]+dp[n][2][1]/3+dp[n][2][2]/9+dp[n][2][3]/27) << endl;
}