...
fun main(){
    solve.cases{
        ...
        var mb1 = 0
        val ret = LongArray(n)
        val size = G.calculateSizes()

        var start = n.toLong() * n
        G.subs(0){v ->
            val s = size[v]
            start -= s.toLong() * s
        }
        ...
        for(v in 1 until n){
            if(ep1 == ep2){
                assert(ep1 == 0)
                ep1 = v
                mb1 = G.calParent(v,G.depth[v] - 1 )
            }else{
                if(G.isancestor(ep1,v)){
                    ep1 = v
                }else if(G.isancestor(v,ep1) ){
                    //nothing
                }else if(G.isancestor(ep2,v) && (ep2 != 0 || G.LCA(v,ep1) == 0)){
                    ep2 = v
                }else if( G.isancestor(v,ep2)){
                    //nothing
                }else{
                    break
                }
            ...
            assert(ep1 != 0)
            val s1 = size[ep1]
            val s2 = if(ep2 == 0) (n-size[mb1]) else size[ep2]
            ret[v] = s1.toLong() * s2 * 2
        }
        val realret = LongArray(n+1)
        var remain = n.toLong() * n
        for(mex in 1..n){
            val now = ret[mex-1]
            val next = if(mex == n) 0 else ret[mex]
            realret[mex] = now - next
            remain -= realret[mex]
        }
        realret[0] = remain
        realret[0] -= (n-1).toLong()
        realret[1] --

        for(i in realret.indices){
            realret[i] /= 2L
        }
