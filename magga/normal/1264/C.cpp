...
void solve(){
    ...
    ll ans = SegTree[0].query(0,n-1,SegTree).F;
    set<long long int>check_points;
    check_points.insert(0);
    check_points.insert(n);
    for(int i=0;i<q;i++){
        cin>>d;d--;
        if(check_points.find(d)==check_points.end()){
            auto p = check_points.lower_bound(d);
            int v = *p; p--;
            int u = *p;
            ans = sub(ans,SegTree[0].query(u,v-1,SegTree).F);
            ans = add(ans,SegTree[0].query(u,d-1,SegTree).F);
            ans = add(ans,SegTree[0].query(d,v-1,SegTree).F);
            check_points.insert(d);
        }else{
            check_points.erase(d);
            auto p = check_points.lower_bound(d);
            int v = *p; p--;
            int u = *p;
            ans = add(ans,SegTree[0].query(u,v-1,SegTree).F);
            ans = sub(ans,SegTree[0].query(u,d-1,SegTree).F);
            ans = sub(ans,SegTree[0].query(d,v-1,SegTree).F);
        }
        cout<<ans<<"\n";
    }
}
