...
void solve(int test) {
    ...
    for (int i = 0; i < e.size(); ++i) {
        ...
        int j = i;
        while (j < e.size() && e[j].pos == e[i].pos) {
            int x = lower_bound(all(L), e[j].x) - L.begin();
            add(x, e[j].add);
            ++j;
        }
        i = j - 1;
    ...
    for (int i = 0; i < e.size(); ++i) {
        ...
        while (j < e.size() && e[j].pos == e[i].pos) {
            if (e[j].add == 1) {
                q.insert(mp(e[j].x, e[j].id));
            } else {
                q.erase(mp(e[j].x, e[j].id));
            }
            ++j;
        }
        i = j - 1;
