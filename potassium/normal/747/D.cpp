...
int main() {
	...
	vector<int> negpos; negpos.reserve(n);
	for (int i = 0; i < n; i++) {
		int x; R(x);
		if (x < 0) negpos.pb(i);
	}
	...
	int ans = 2*negpos.size();
	
	int lastcost = n - negpos.back() - 1;
	
	k -= negpos.size();
	
	vector<int> cost(negpos.size()-1);
	for (int i = 0; i < negpos.size()-1; i++)
		cost[i] = max(0,negpos[i+1]-negpos[i]-1);
