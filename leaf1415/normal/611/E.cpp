...
int main(void)
{
	...
	while(sz(S)){
		...
		if(x > b+c) ans++, S.erase(it); //a+b+c
		else if(x > a+c) ans++, S.erase(it), del(S, a); //b+c
		else if(x > max(a+b, c)) ans++, S.erase(it), del(S, b); //a+c
		//else if(c < a+b && x > c) ans++, S.erase(it), del(S, c); //a+b
		else break;
	}
