...
#line 2 "/home/maspy/compro/library/linalg/mat_pow.hpp"
template<typename T>
...
void solve() {
  ...
  mint total = N * (N - 1) / 2;
  FOR(k, n + 1) {
    ll a = k, b = n - k, c = n - k, d = N - n - n + k;
    if (min({a, b, c, d}) < 0) continue;
    if (k < n) mat[k][k + 1] += mint(b * c) / total;
    if (k > 0) mat[k][k - 1] += mint(a * d) / total;
    mat[k][k] += mint(total - a * d - b * c) / total;
  }
