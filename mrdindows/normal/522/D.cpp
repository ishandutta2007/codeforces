...
int get_first(int cur, int l, int r, int pos){
	if (l == r){
		if (tr[cur] == -INF) return -1;
		return l;
	}
	else {
		int m = (l + r) >> 1;
		int dcur = cur + cur;
		if (tr[dcur] >= pos){
			return get_first(dcur, l, m, pos);
		}
		else {
			return get_first(dcur + 1, m + 1, r, pos);
		}
	}
}
vector<pii> v[500101];
...
int main()
{
	...
	FI(n){
		...
		lastp[x] = i;
		for (auto q : v[i]){
			ans[q.second] = get_first(1, 1, n, q.first);
		}
	}
	FI(m){
		cout << ans[i] << '\n';
