...
void _main() { /* CURSOR START */
	...
	int ans=0;
	for(;sz(pq);) {
		auto [d,u,k]=pq.top(); pq.pop(); d=-d;
		if(d>dp[u][k]) continue;
		if(u==n) ans=max(ans,k);
		for(auto [v,t]:adj[u]) {
			if(d+t>T) continue;
			if(d+t>=dp[v][k+1]) continue;
			dp[v][k+1]=d+t;
			pr[v][k+1]=u;
			pq.push({-(d+t),v,k+1});
		}
	...
	cout<<ans<<endl;
	vector<int> zz={n};
	for(;sz(zz)<ans;) zz.pb(pr[zz.back()][ans-sz(zz)+1]);
	reverse(all(zz));
	for(int i=0;i<ans;i++) cout<<zz[i]<<" \n"[i==ans-1];
}