...
int last = 2;
int root = 1;
ll go(int cur, int bit, ll x, ll res, bool ok = true) {
//    DBN(cur, bit, x, res);
    if (cnt[cur] == 0) return -1;
    if (bit == -1) return (res ^ x) > x ? res : -1;
    if ((res ^ x) + (2LL << bit) - 1 <= x) return -1;
    for (int dir = 0; dir < 2; ++dir) {
        int where = dir ^ checkbit(x, bit);
        ll num = go(ptr[cur][where], bit - 1, x, res | (ll(where) << bit));
        if (num >= 0) return num;
    }
    return -1;
}
ll get(ll x) {
    return go(root, 60, x, 0, false);
}
void del(ll x) {
    int cur = root;
    for (int i = 60; i >= 0; --i) {
        cur = ptr[cur][checkbit(x, i)];
        cnt[cur]--;
    }
}
int solve() {
