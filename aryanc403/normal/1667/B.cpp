...
ii op(ii a, ii b) { return {max(a.X, b.X),max(a.Y, b.Y)}; }
...
cin>>T;while(T--)
{
    ...
    dbg(a);
    atcoder::segtree<ii, op, e> seg(n+3);
    vi same(n+3,-INF);
    seg.set(a[0],{0,0});
    same[a[0]]=max(same[a[0]],0LL);
    for(lli i=1;i<=n;i++){
        const lli x=a[i];
        const lli cur=max({seg.prod(0,x).X+i,seg.prod(x+1,n+3).Y-i,same[x]});
        same[x]=max(same[x],cur);
        auto g=seg.get(x);
        g.X=max(g.X,cur-i);
        g.Y=max(g.Y,cur+i);
        seg.set(x,g);
