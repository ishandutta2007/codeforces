...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	cauto ns=readV(4);
	vv<ll> cs(4);
	rep(i,4) cs[i]=readV(ns[i]);
	vector<ll> dp=cs[0];
	const ll INF=1e18;
	rep(i,3){
		cauto m=read();
		cauto ps=readV(m,pii(1,1));
		cauto n0=ns[i];
		cauto n1=ns[i+1];
		vv<int> ng(n1);
		for(cauto &[a,b]:ps) ng[b].eb(a);
		vector<ll> nxt(n1,INF);
		multiset<ll> st(all(dp));
		rep(j,n1){
			for(cauto &a:ng[j]) st.erase(st.find(dp[a]));
			if(st.size()) nxt[j]=cs[i+1][j]+*st.begin();
			for(cauto &a:ng[j]) st.emplace(dp[a]);
		}
		dp=nxt;
		out(dp,1);
	}
	ll re=*min_element(all(dp));
	cout<<(re>=INF?-1:re) NL;
  return 0;
}