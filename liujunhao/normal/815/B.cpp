...
int n,a[MAXN+10],fac[MAXN+10],inv[MAXN+10];
void prepare(){
...
void solve(){
	if(n<=3){
		int la=0,i,j;
		for(i=1;i<n;i++){
			for(j=1;j<=n-i;j++,la^=1)
				if(!la)
					a[j]=(a[j]+a[j+1])%MOD;
				else
					a[j]=(a[j]+MOD-a[j+1])%MOD;
		}
		printf("%d\n",a[1]);
	}
	else if(n%4==1){
		...
		int v1=0,v2=0,v3=0;
		for(int i=0;i<=t;i++){
			v1=(v1+1ll*C(t,i)*a[i*2+1])%MOD;
			v2=(v2+1ll*C(t,i)*a[i*2+2])%MOD;
			v3=(v3+1ll*C(t,i)*a[i*2+3])%MOD;
		}
		cout<<((1ll*v1+v2+v2-v3)%MOD+MOD)%MOD<<endl;
	}
	else{
		int t=(n-2)/2,i,flag,ans=0;
			if(!(n&3))
				flag=MOD-1;
			else
				flag=1;
		for(i=0;i<=t;i++){
			ans=(ans+1ll*C(t,i)*(a[i*2+1]+1ll*flag*a[i*2+2]%MOD)%MOD)%MOD;
		}
		cout<<ans<<endl;
	}
}
