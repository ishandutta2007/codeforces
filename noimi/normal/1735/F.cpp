...
struct frac {
    ...
    long double val() { return ld(1) * q / p; }
...
int main() {
    ...
    TEST {
        ...
        INT(aa, bb);
        ld a = aa, b = bb;
        multiset<pair<frac, long double>> s;
        VEC(int, p, n);
        VEC(int, q, n);
        ...
        rep(i, n) {
            s.emplace(frac(q[i], p[i]), p[i] * 2);
            a -= p[i], b -= q[i];
            ans += p[i];

            while(a < -EPS) {
                // if(empty(s)) rep(inf<int>) OUT(1);
                auto [x, p] = *begin(s);
                s.erase(begin(s));
                if(a + p >= -EPS) {
                    if(a + p >= EPS) s.emplace(x, a + p);
                    a = 0;
                } else {
                    a += p;
                }
            ...
            while(b < -EPS) {
                // if(empty(s)) rep(inf<int>) OUT(1);
                auto [x, p] = *rbegin(s);
                long double q = (p * x.val());
                ans -= p;
                s.erase(--end(s));
                if(b + q >= -EPS) {
                    s.emplace(x, ((b + q) / x.val()));
                    ans += ((b + q) / x.val());
                    b = 0;
                } else {
                    b += q;
