...
void solve(){
	int n, m; cin >> n >> m;
	vi a(n), b(n), c(m);read(a,b,c);
	int ind = -1;
	for(int i = 0; i < n; ++i){
		if(b[i] == c[m-1]){
			if(a[i] != b[i])ind = i+1;
			else if(ind == -1)ind = i+1;
		}
	}
	if(ind == -1){
		cout << "NO\n";
	...
	for(int i = 0; i < n; ++i){
		if(ab[i][0] != ab[i][2]){
			while(cn < m && d[cn].first < ab[i][0]){
				cn++;
			}
			if(cn == m || d[cn].first != ab[i][0]){
				cout << "NO\n";
				return;
			}
			e[d[cn++].second]=ab[i][1]+1;
		}
	}
	cout << "YES\n";
	for(int i : e){
