...
inline int GetSubstNumber(vector<int> &numbers)
{
    int fact = 1, result = 0;
    t.assign(numbers.size(), 0);
    upd(numbers.back());
    for (int n = numbers.size() - 2; n >= 0; n--)
    {
        fact *= numbers.size() - n - 1;
        int pos = get(numbers[n] - 1);
        upd(numbers[n]);
        result += pos * fact;
    }
  return result;
