...
void d(){
    ...
    ll sumaIzq = 0;
    ll sumaDer = 0;
    F0(i,n/2) if(t[i] != '?') sumaIzq += (ll) t[i] - (ll) '0';
    REP(i,n/2,n-1) if(t[i] != '?') sumaDer += (ll) t[i] - (ll) '0';

    ll vaciosIzq = 0;
    ll vaciosDer = 0;
    F0(i,n/2) if(t[i] == '?') vaciosIzq++;
    REP(i,n/2,n-1) if(t[i] == '?') vaciosDer++;

    if((vaciosDer - vaciosIzq) != 0 and (sumaIzq - sumaDer)*2 == 9 * (vaciosDer - vaciosIzq)) cout<<"Bicarp\n";
    else if(vaciosDer - vaciosIzq == 0 and sumaIzq - sumaDer == 0) cout<<"Bicarp\n";
    else cout<<"Monocarp\n";
}
