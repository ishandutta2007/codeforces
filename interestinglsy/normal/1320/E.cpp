...
static vc<int> imp;
for( auto [id,pos,speed] : viruses ) imp.Eb(pos);
for( auto [pos,id] : focusOn ) imp.Eb(pos);
sort(All(imp),CmpByIn);

int oSz = imp.size();
For0(i,oSz-1)
	imp.Eb(LCA(imp[i],imp[i+1]));
imp.Eb(1);
sort(All(imp),CmpByIn);
...
static vc<Edge> ne[MAXN];
function<void(int,vc<int>::iterator&)> BuildNE = [&]( int pos , auto &curP ){
	assert( *curP == pos );
	int curIn = in[pos];
	int curOut = out[pos];
	++curP;
	while(true){
		if( curP == imp.end() or out[*curP] > curOut ) break;
		int v = *curP;
		ne[pos].Pb({v,dep[v]-dep[pos]});
		ne[v].Pb({pos,dep[v]-dep[pos]});
		// P(pos,v);
		BuildNE(v,curP);
	}
};
...
struct Fuck{
	...
	int pos;
	int speed;
	il bool operator<( const Fuck &b )const{
		// @inversed
		if( int(ceil(time-(1e-8))) != int(ceil(b.time-(1e-8))) ) return time > b.time;
		return type > b.type;
	}
};
static priority_queue<Fuck> pq;	// {time,type,pos,speed}
for( auto [id,pos,speed] : viruses )
	pq.push({0,id,pos,speed});
for( auto p : imp ) ansl[p] = 0;

while(!pq.empty()){
	auto [uTime,uType,pos,uSpeed] = pq.top();
	pq.pop();
	if(ansl[pos]) continue;
	ansl[pos] = uType;
	for( auto [v,cost] : ne[pos] ){
		if(ansl[v]) continue;
		pq.push({
			uTime + (ld)cost/uSpeed,
			uType,
			v,
			uSpeed
		});
