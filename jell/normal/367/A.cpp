#line 1 "codeforces-workspace/contest/367/a/a.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/367/a/a.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/367/a/a.cpp"
...
void main() {
  ...
  for (auto &&c : s) {
    for (auto i : range(3)) {
      cnt[i].emplace_back(cnt[i].back() + ('x' + i == c));
    }
  }
  int q;
  cin >> q;
  while (q--) {
    ...
    for (auto i : range(3)) {
      for (auto j : range(3)) {
        if (cnt[i][b] - cnt[i][a] + cnt[j][a] - cnt[j][b] > 1) {
          fail = 1;
        }
      }
    }
