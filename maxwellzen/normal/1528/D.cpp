...
void solve() {
    cin >> n >> m;
    vvll go(n, vll(n, 1e18));
    F0R(i, m) {
        int a, b, c; cin >> a >> b >> c;
        go[a][b] = c;
    }
    F0R(i, n) F0R(rep, 2) F0R(j, n) ckmin(go[i][(j+1)%n], go[i][j]+1);
    vi dist(n, INT_MAX);
    vb vis(n, false);
    F0R(i, n) {
        ...
        F0R(rep, n) {
            int i = -1;
            F0R(j, n) if (!vis[j] && (i==-1 || dist[j]<dist[i])) i=j;
            vis[i] = true;
            F0R(nb, n) if (!vis[nb]) {
                int nb2 = (nb - dist[i])%n;
                if (nb2<0) nb2 += n;
                if (dist[nb] > dist[i] + go[i][nb2]) {
                    dist[nb] = dist[i] + go[i][nb2];
                }
            }
        }
        F0R(j, n) cout << dist[j] << ' ';
