#line 1 "codeforces-workspace/contest/1503/e/e.cpp"
...
#line 6 "codeforces-workspace/contest/1503/e/e.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace/contest/1503/e/e.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1503/e/e.cpp"
 ...
 */
template <class _Tp>
constexpr std::__enable_if_t<(is_integral_ext<_Tp>::value), _Tp> pow_mod(
    _Tp __x, _Tp __n, _Tp __mod) noexcept {
  assert(__mod > 0);

  using int_type = typename multiplicable_uint<_Tp>::type;

  int_type __y{1};

  while (__n) {
    if (__n & 1) (__y *= __x) %= __mod;
    __x = (int_type)__x * __x % __mod;
    __n >>= 1;
  }
 ...
 */
template <class _Tp>
constexpr std::__enable_if_t<(is_integral_ext<_Tp>::value), _Tp> sqrt_mod(
  ...
  int_type __a = pow_mod(__x, (__q + 1) >> 1, __mod), __b = 2;
...
#line 27 "codeforces-workspace/contest/1503/e/e.cpp"

namespace workspace {

using mint = modint<998244353>;
auto bin = binomial<mint>;

void main() {
  // start here!

  auto calc = [&](int h, int w) -> mint {
    mint sum;

    for (auto l : range(1, h)) {
      for (auto k : range(1, w)) {
        sum += bin(w - k + l - 1, l) * bin(k + l - 1, k) *
               bin(h - l + k - 1, k - 1) * bin(h - l + w - k - 1, w - k);
      }
    }
  ...
  auto calc2 = [&](int h, int w) -> mint {
    ...
    for (auto i : range(1, h)) {
      ...
      for (auto l : range(w)) {
        sum += acc * bin(w - l - 1 + i, i) * bin(l + i - 1, l);
        acc += bin(w - l + h - i - 1, h - i - 1) * bin(l - 1 + h - i, h - i);
      }
    }
  ...
  int n, m;
  cin >> n >> m;
  cout << (calc2(n, m) + calc2(m, n) + calc(m, n)) * 2 << "\n";
}
