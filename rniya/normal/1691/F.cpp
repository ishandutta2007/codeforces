        ...
        auto dfs = [&](auto self, int v, int p, int D) -> void {
            ...
            for (int i = 0; i < m; i++) {
                ...
                if (u == p)
                    ...
                    sz.emplace_back(sub[u]);
            }
            vector<mint> dis;
            for (int& x : sz) dis.emplace_back(BINOM.C(x, k));
            mint sum = accumulate(ALL(dis), mint(0));
            ans += (BINOM.C(n, k) - sum) * n;
            for (int i = 0; i < m; i++) {
                int u = G[v][i];
                ans += (BINOM.C(n - sz[i], k) - (sum - dis[i])) * sz[i] * (n - sz[i]);
                if (u == p) continue;
                self(self, u, v, n - sub[u]);
            }
        };
        dfs(dfs, 0, -1, 0);
    }
