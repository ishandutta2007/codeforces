...
vector<int> reflect = {0, 1, 5, -1, -1, 2, -1, -1, 8, -1};

int rev(int x) {
    int i = x / 10, j = x % 10;
    if (reflect[i] == -1 || reflect[j] == -1)
        return -1;
    return reflect[j] * 10 + reflect[i];
}
...
void solve() {
    ...
    re(start);
    int a, b;
    a = stoi(start.substr(0, 2)), b = stoi(start.substr(3, 2));

    while (true) {
        if (rev(a) != -1 && rev(b) != -1 && rev(a) < m && rev(b) < h) {
            pr(a < 10 ? "0" : "", a, ":", b < 10 ? "0" : "", b);
            ps();
            return;
        }
