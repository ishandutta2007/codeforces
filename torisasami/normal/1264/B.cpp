...
int main()
{
	...
	rep(i, a[0]) ans.pb(1), ans.pb(0);
	a[1] -= a[0];
	if(a[1] && a[2] <= a[1] + a[3]){
		ans.pb(1);
		a[1]--;
	}
	rep(i, a[1]) ans.pb(2), ans.pb(1);
	a[2] -= a[1];
	if (a[2]){
		ans.pb(2);
		a[2]--;
	}
	rep(i, a[2]) ans.pb(3), ans.pb(2);
	a[3] -= a[2];
	if (a[3])
		ans.pb(3);
	rep(i,siz(ans)-1){
	...
	cout << "YES" << endl;
	rep(i, ans.size()) cout << (f ? 3 - ans[i] : ans[i]) << (i + 1 == ans.size() ? '\n' : ' ');
}