...
public class CFCR549 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            for (int i = 1; i <= n; i++) {
                ...
                if (p == -1) {
                    ...
                    nodes[i].father = nodes[p];
                }
                nodes[i].c = c == 1;
            }

            List<Node> deletion = new ArrayList<>();
            dfs(root, deletion);
            if (deletion.isEmpty()) {
                io.cache.append(-1);
            }

            deletion.sort((a, b) -> a.id - b.id);
        ...
        public void dfs(Node root, List<Node> waitForDeletion) {
            boolean deleted = root.c && root.father != null;
            root.relyOn = root.children.size();
            for (Node child : root.children) {
                dfs(child, waitForDeletion);
                deleted = deleted && child.c;
            }
            if (deleted) {
                root.deleted = deleted;
                waitForDeletion.add(root);
            }
        }
    }
