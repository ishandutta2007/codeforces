...
int mn;
void combine_ranges(int l){
	if(l >= mn) return;
	int val = t.query(l,l).v;
	int r = t.descent_right(l,val);
	t.rupd(l,r-1,val);
	combine_ranges(r);
}
...
void solve(){
	...
	rep(i,0,q){
		...
		if(type == "s"){
			int l,r;
			cin >> l >> r;
			int ans = t.query(l,r).v + ps[r] - ps[l - 1];
			cout << ans << "\n";
		}
		else{
			int pos, val;
			cin >> pos >> val;
			int ov = t.query(pos,pos).v;
			val += ov;
			int r = t.descent_right(pos,val);
			t.rupd(pos,r-1,val);
			combine_ranges(r);
			amin(mn,pos);
		}
