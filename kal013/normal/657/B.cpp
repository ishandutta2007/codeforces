...
// 	d2(n,k);
    ...
    for(int i=n+40;i>=0;--i){
        if (P[i][0]==P[i][1]) continue;
        if (P[i][0]>P[i][1]) {z=1;break;}
        z=-1;
        break;
    }
    int msb=-1;
    if (z==0){
        ...
        int carry=0;
        For(i,n+41){
            // trace(carry,i,P[i][0],P[i][1]);
            if (P[i][0]>=carry+P[i][1]){
                P[i][0]-=carry+P[i][1];
                ans[i]=P[i][0];
                carry=0;
            }
