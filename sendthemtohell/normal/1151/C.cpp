...
ll calc_e(ll n) {
	return n % MOD * (n % MOD + 1) % MOD;
}

ll calc_o(ll n) {
	return (calc_e(n) - n % MOD + MOD) % MOD;
}
...
ll get_ans(ll c) {
	...
	return (calc_e(num_e) + calc_o(num_o) + MOD) % MOD;
}
...
inline void solve() {
	...
	cout << (get_ans(r) - get_ans(l - 1) + MOD) % MOD << endl;
}
