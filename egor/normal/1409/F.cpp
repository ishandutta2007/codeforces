...
class FPodposledovatelnostiDliniDva {
public:
    void solve() {
        int n = in.readInt();
        int k = in.readInt();
        ...
        if (t[0] == t[1]) {
            int total = min(int(count(all(s), t[0])) + k, n);
            out.printLine(total * (total - 1) / 2);
            return;
        }
        arr3d<int> best(n + 1, n + 1, k + 1, numeric_limits<int>::min());
        best(0, 0, 0) = 0;
        for (int i : range(n)) {
            for (int j : range(i + 1)) {
                for (int l : range(k + 1)) {
                    maxim(best(i + 1, j, l), best(i, j, l));
                    if (s[i] == t[0] || l != k) {
                        maxim(best(i + 1, j + 1, l + (s[i] == t[0] ? 0 : 1)), best(i, j, l));
                    }
                    if (s[i] == t[1] || l != k) {
                        maxim(best(i + 1, j, l + (s[i] == t[1] ? 0 : 1)), best(i, j, l) + j);
                    }
                }
            }
        }
...
int main() {
    ...
    auto time = clock();
#endif
    FPodposledovatelnostiDliniDva solver;
