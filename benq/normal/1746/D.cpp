...
pl dfs(int x, int k) {
	if (!sz(child.at(x))) return {S.at(x) * k, S.at(x) * (k+1)};
	ll sum_ans = 0;
	vl difs;
	for (int y: child.at(x)) {
		auto [ans, extra_ans] = dfs(y, k / sz(child.at(x)));
		sum_ans += ans;
		difs.pb(extra_ans - ans);
	}
	sort(rall(difs));
	F0R(i,k%sz(child.at(x))) {
		sum_ans += difs.at(i);
	}
	ll extra_ans = sum_ans + difs.at(k%sz(child.at(x)));
	sum_ans += S.at(x) * k;
	extra_ans += S.at(x) * (k+1);
	return {sum_ans, extra_ans};
}
