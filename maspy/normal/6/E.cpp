...
void solve() {
  ...
  VEC(ll, A, N);
  using T = pi;
  SegTree<T> seg([](T x, T y) { return mp(min(x.fi, y.fi), max(x.se, y.se)); }, mp(INF, -INF));
  seg.init(N);
  FOR(i, N) { seg.set_val(i, mp(A[i], A[i])); }

  vc<T> ANS;
  FOR(L, N) {
    auto check = [&](T x) -> bool { return x.se - x.fi <= K; };
  ...
  int cnt = 0;
  FORIN(p, ANS) {
    if (best > p.se - p.fi) continue;
    if (chmax(best, p.se - p.fi)) cnt = 0;
    ++cnt;
  }
  print(best + 1, cnt);
  FORIN(p, ANS) {
    if (p.se - p.fi == best) print(p);
  }
}
