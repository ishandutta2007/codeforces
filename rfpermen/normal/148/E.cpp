...
int n,m,k[105],y[105],x[105][105],dp[105][10005],mx;
...
int f(int nw,int rem){
	...
	int ret = 0;
	rep(i,0,k[nw])ret = max(ret,f(nw+1,rem-i) + x[nw][i]);
...
// freopen("output.txt","w",stdout);
	...
	rep(i,1,n){
		cin>>k[i];
		rep(j,1,k[i])cin>>y[j], y[j]+=y[j-1];
		For(j,1,k[i]){
			mx = 0;
			rep(o,0,j)mx = max(mx,y[o]+y[k[i]]-y[k[i]-j+o]);
			x[i][j] = mx;
		}
		x[i][k[i]] = y[k[i]];
	}
