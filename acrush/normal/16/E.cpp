...
int n;
double A[maxn][maxn];
bool visited[two(maxn)];
double f[two(maxn)][maxn];

void solve(int set)
{
	if (visited[set]) return;
	visited[set]=true;
	memset(f[set],0,sizeof(f[set]));
	if ((set&(set-1))==0)
	{
		for (int i=0;i<n;i++) if (contain(set,i)) f[set][i]=1.0;
		return;
	}
	int c=0;
...
int main()
{
	...
	solve(two(n)-1);
	for (int i=0;i<n;i++) printf("%.18lf ",f[two(n)-1][i]);
	printf("\n");
	return 0;
}