...
Node replace(int u, int v, int rt, Node* x, Node* y, int sig) {
	int ix = ct.lev[rt];
	x->sum += sig * ct.info[ix][u].d;
	x->cnt += sig;
	if (ix) {
		x->sum -= sig * ct.info[ix - 1][u].d;
	}
	FOR(i, 0, sz(ct.tree[rt])) {
		int w = ct.tree[rt][i];
		if (ct.isparent(w, u)) {
			if (rt == v) {
				x->child[i] = y->child[i];
				...
				replace(u, v, w, x->child[i], y->child[i], sig);
			}
		}
	}
}
...
void solve() {
	...
	while (q--) {
		...
		if (op == 1) {
			...
			int a; cin >> a;
			int x = (ans % (1 << 30)) ^ a;
			int v = ct.lca(p[x - 1], p[x]);
			replace(p[x], v, ct.rt, rt[x], rt[x + 1], 1);
			replace(p[x - 1], v, ct.rt, rt[x], rt[x - 1], -1);
			swap(p[x - 1], p[x]);
		}
	}
}
