...
int main(){
	...
	rep(_,40){
		cauto b=ss[rand()%n];
		vector<int> bits;
		rep(d,m)if(b[d]=='1') bits.eb(d);
		const int D=bits.size();
		vector<int> cnt(1<<D);
		for(cauto &s:ss){
			int t=0;
			rep(i,D)if(s[bits[i]]=='1') t|=1<<i;
			++cnt[t];
		}
		rep(d,D)rep(i,1<<D)if(i>>d&1) cnt[i^(1<<d)]+=cnt[i];
		rrep(i,1<<D)if(cnt[i]>=(n+1)/2 && MX(mx,__builtin_popcount(i))){
			fill(all(re),'0');
			rep(d,D)if(i>>d&1) re[bits[d]]='1';
		}
		out(mx,re,1);
	}
	{
