...
fun randomtree(n:Int){
    for(i in 2..n){
        val v = Random.nextInt(1 until i)
        share("$i $v")
    }
}
...
const val large = (1e12).toLong()
fun main(){
    solve.rand {
        val n = 5
        share(n)
        val costs = IntArray(n){Random.nextInt(100..300)}
        val hei = IntArray(n){Random.nextInt(1..(n-1))}
        share(costs)
        share(hei)
        randomtree(n)
...
//    solve.userand()
    solve.cases{
        ...
        val d = G.degree()
        val DP = Array(2){LongArray(n){phuge} }
        G.leafFirst {v ->
            for(new in 0..1){
                val options = mong
                var base = d[v] * cost[v]
                G.subs(v){
                        w ->
                    val t0 = if(hei[w] <= hei[v]) DP[0][w] else large
                    val t1 = if(hei[w] >= hei[v]) DP[1][w] else large
                    base += t0
                    options.add(t1 - t0 )
                }
                val t0 = if(new == 1) 0L else large
                val t1 = if(new == 0) 0L else large
                if(v != G.root){
                    base += t0
                    options.add(t1 - t0)
                }
                options.sort()
                val kind = LongArray(options.size + 1 )
                for(i in 0 until options.size){
                    kind[i+1] = kind[i] + options[i]
                }
                var ret = phuge
                val x = options.size
                val q = rmq(kind)
                for(pair in 0..x){
                    val new = q.query(pair, x-pair) - pair * cost[v] + base
                    ret = minOf(ret, new )
                }
                DP[new][v] = ret
            }
...
//        val DP = Array(2){LongArray(n){phuge} } // have node, doesn't have node

//        val DP = Array(2){LongArray(n){phuge} }
//        var best = phuge
////         1: can extend upwards, 2: can extend downwards
//
//        val ret = mong
//        for(root in 0 until n){
//            G.root = root
//            G.treeOrderDFS(false)
//            G.leafFirst {v ->
//
////            just dei "new $v"
//                val here = cost[v]
//                var old = Array(6){LongArray(3){phuge}}
//                old[0][0] = 0
//                //taken, final type
//
//                G.subs(v){w ->
//                    val new = Array(6){LongArray(3){phuge} }
//                    for(left in 0 until 6){
//                        for(result in 0 until 3){
//
//                            if(hei[w] >= hei[v] ) {
//                                if(left == 0 || left == 2) {
//                                    for(newleft in if(left == 0) listOf(1, 4) else listOf(3)) {
//                                        val newresult = result
//                                        new[newleft][newresult] =
//                                            minOf(new[newleft][newresult], old[left][result] + DP[0][w])
//                                    }
//                                }
//                                new[left][result] = minOf(new[left][result], old[left][result] + DP[0][w] + here )
//                                new[left][1] = minOf(new[left][1], old[left][result] + DP[0][w] + here )
//                            }
//                            if(hei[w] <= hei[v]) {
//                                if(left == 0 || left == 1){
//                                    for(newleft in if(left == 0) listOf(2,5) else listOf(3)){
//                                        val newresult = result
//                                        new[newleft][newresult] = minOf(new[newleft][newresult], old[left][result] + DP[1][w])
//                                    }
//                                }
//
//                                new[left][result] = minOf(new[left][result], old[left][result] + DP[1][w] + here )
//                                new[left][2] = minOf(new[left][2], old[left][result] + DP[1][w] + here )
//                            }
//                        }
//                    }
//                just dei new
//                    old = new
//                }
//                val receives = LongArray(2){phuge}
//                for(left in listOf(0,3,4,5)){
//                    for(result in 0 until 3){
//                        val realcost = old[left][result] + (if(left >= 1) 1 else 0) * here
//                        receives[0] = minOf(receives[0], realcost + here)
//                        receives[1] = minOf(receives[1], realcost + here )
//                        for(r in 0..1){
//                            if(result == r + 1 || left - 4 == r){
//                                receives[r] = minOf(receives[r], realcost)
//                            }
//                        }
//                        if(v == G.root){
//                            best = minOf(best, realcost)
//                        }
//                    }
//                }
//                DP[0][v] = receives[0]
