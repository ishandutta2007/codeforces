...
int main() {
  ...
  REP(i, l) {
    us[x][y] = true;
    bool move = false;
    if (s[i] == 'U' && x > 1) --x, move = true;
    if (s[i] == 'D' && x < n) ++x, move = true;
    if (s[i] == 'L' && y > 1) --y, move = true;
    if (s[i] == 'R' && y < m) ++y, move = true;
    if (move && !us[x][y]) ans[i+1] = 1;
  }
  ans[l] += n * m - accumulate(ans, ans + l + 1, 0);
  print("%d", ans, ans + l + 1);
