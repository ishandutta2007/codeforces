...
ll N;
vector<pair<int,int>> check(multiset<int,greater<int>> se,int X){
  ...
  rep(i,N){
    ...
    A=*itr;
    se.erase(itr);
    auto sitr=se.lower_bound(X-A);
    if(sitr==se.end()||*sitr!=X-A)return res;
    X=A;
    res.emplace_back(A,*sitr);
    se.erase(sitr);
  }
  return res;
}
