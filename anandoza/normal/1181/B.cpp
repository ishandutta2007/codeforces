...
int main() {
    ...
    for (int i = 1; i < n; i++) {
        if (s[i] != '0')
            cand.push_back({abs(i - (n - i)), i});
    }
    sort(all(cand));
    while (cand.size() > 3)
        cand.pop_back();

    bignum ans = bignum(s.substr(0, cand[0].second)) + bignum(s.substr(cand[0].second, n - cand[0].second));
    for (int i = 0; i < cand.size(); i++)
        ckmin(ans, bignum(s.substr(0, cand[i].second)) + bignum(s.substr(cand[i].second, n - cand[i].second)));
