...
void solve(){
    ...
    REP(i, t) {
        ...
        cin >> a >> b;
        a--;
        queries.push_back(mp(a, mp(b, i)));
    }
    sort(ALL(queries), [](auto& l, auto& r) {
        if (l.first / DIV != r.first / DIV) return l.first / DIV < r.first / DIV;
    ...
    int itr_right = 0;
    REP(i, queries.size()) {
        while (itr_right < queries[i].second.first) {
            ans += inputs[itr_right] * (cnts[inputs[itr_right]] * 2 + 1);
            cnts[inputs[itr_right]]++;
            itr_right++;
        }
        while (itr_right > queries[i].second.first) {
            itr_right--;
        ...
        while (itr_left < queries[i].first) {
            cnts[inputs[itr_left]]--;
            ans -= inputs[itr_left] * (cnts[inputs[itr_left]] * 2 + 1);
            itr_left++;
        }
        while (itr_left > queries[i].first) {
            itr_left--;
        ...
        calc[queries[i].second.second] = ans;
    }
    REP(i, queries.size()) {
        cout << calc[i] << "\n";
    }
