...
public class D4 {
	...
	int[] lpf = enumLowestPrimeFactors(20000000);
	...
	byte[] make(int n)
	{
		...
		a[1] = 0;
		for(int i = 2;i <= n;i++){
			a[i] = (byte)(lpf[i] == lpf[i/lpf[i]] ? a[i/lpf[i]] : a[i/lpf[i]] + 1);
		}
		return a;
	}
	...
	void go()
	{
		long C = nl(), D = nl(), X = nl();
		long ans = 0;
		for(int d : enumDivisorsFast((int)X, lpf)){
			ans += analyze(d, C, D, X);
		}
		out.println(ans);
	}
	...
	long analyze(long g, long C, long D, long X)
	{
		// Ck-D = X/G
		if((g+D) % C != 0)return 0;
		int k = (int)((g+D) / C);
