...
void solve() {
  ...
  --K;
  ll ANS = 0;
  FOR3(i, x1, x2 + 1) FOR3(j, y1, y2 + 1) {
    if ((i ^ j) <= K) ANS += (i ^ j) + 1;
  }
  using T = tuple<ll, ll, ll, ll, ll>;
  using P = pair<mint, mint>;
  map<T, P> MEMO;
  auto dfs = [&](auto self, ll x1, ll x2, ll y1, ll y2, ll k) -> P {
    if (x1 > x2 || y1 > y2 || k < 0) return {0, 0};
    if (k == 0) {
      ll a = max(x1, y1);
      ll b = min(x2, y2);
      if (a > b) return {0, 0};
    ...
    FOR(x, 2) FOR(y, 2) {
      ...
      ll b2 = floor(y2 - y, 2);
      ll v = floor(k - (x ^ y), 2);
      auto [cnt, sum] = self(self, a1, a2, b1, b2, v);
      ANS.fi += cnt;
      ANS.se += mint(2) * sum + mint(x ^ y) * cnt;
    }
    return ANS;
  };
  auto [cnt, sum] = dfs(dfs, x1, x2, y1, y2, K);
  print(cnt + sum);
}
