...
int main() {
    ...
    const int s = accumulate(all(c), 0);
    vector<int> sub = c;
...

    {
        auto dfs = [&](auto dfs, int u, int p) -> void {
            ...
            for (int v : g[u]) {
                ...
                if (c2 - sub[v] >= 2 and c1 - c[v]) { // c
                    ++imos[vis[v]];
                    --imos[lev[v]];
                }

                if (s - sub[u] and c1 - c[v]) { // d
                    ++imos[vis[v]];
                    --imos[lev[v]];
                }

                if (p >= 0 and c[p] and sub[u] - sub[v]) { // e
                    ++imos[vis[v]];
                    --imos[lev[v]];
                }

                if (c[u] and c2 - sub[v]) { // e
                    ++imos[vis[v]];
                    --imos[lev[v]];
                }
            }
