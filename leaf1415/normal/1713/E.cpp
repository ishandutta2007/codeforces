...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) rep(j, 1, n){
			...
			if(a[i][j] > a[j][i]){
				if(!uf.same(i, j) && !uf.same(n+i, n+j)){
					uf.unite(i, n+j), uf.unite(n+i, j);
					G[i].pb(n+j), G[n+j].pb(i);
					G[n+i].pb(j), G[j].pb(n+i);
				}
			}
			else{
				if(!uf.same(i, n+j) && !uf.same(n+i, j)){
					uf.unite(i, j), uf.unite(n+i, n+j);
					G[i].pb(j), G[j].pb(i);
					G[n+i].pb(n+j), G[n+j].pb(n+i);
				}
			}
		}
		rep(i, 1, 2*n) used[i] = false;
		rep(i, 1, n) if(!used[i]) dfs(i);
		
		rep(i, 1, n) rep(j, 1, n){
			if(i < j && b[i]^b[j]) swap(a[i][j], a[j][i]);
		}
		rep(i, 1, n){
