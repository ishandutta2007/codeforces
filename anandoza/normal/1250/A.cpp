...
template <typename I> struct _range_struct {
    const I first_, last_;
    explicit _range_struct(const I first, const I last) : first_{first}, last_{last} {}
...
int main() {
    ...
    bool flip = true;
    for (char c : range(s.begin() + 1, s.end())) {
        flip &= 'A' <= c && c <= 'Z';
    }

    if (flip) {
        for (char &c : range(s.rbegin(), s.rend()))
            c ^= 'A' ^ 'a';
    }
