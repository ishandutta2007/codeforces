...
int main() {
	int n, m, k;
	cin >> n >> m >> k;
	vector<mint> f(k + 1), binom(k + 1), g(k + 1, 1);
	binom[0] = 1;
	for(int i = 1; i <= min(n, k); i++)
		binom[i] = binom[i - 1] * (n + 1 - i) / i;

	vector<mint> nfac(k + 1);
	nfac[0] = fast_mod_factorial(n);
	for(int i = 1; i <= k; i++)
		nfac[i] = (n + 1 - i > 0 ? nfac[i - 1] / (n + 1 - i) : 1);

	for(int i = 0; i <= k; i++)
		f[i] = binom[i] * mint(i).pow(k) * nfac[i];
