...
int cn1=0,cn2=0,s1[107],s2[107];
int n;
int minn=Inf;

inline bool bigtosmall(int x,int y){return x>y;}
int main()
{
	cin>>n;
	for(int i=0;i<n;i++)
	...
	sort(s1,s1+cn1,bigtosmall);
	sort(s2,s2+cn2,bigtosmall);
	for(int i=0;i<=cn1;i++)
		for(int j=0;j<=cn2;j++)
		{
			if(i+2*j>=minn)continue;
			int sum=0;
			for(int k=i;k<cn1;k++)
				sum+=s1[k];
			for(int k=j;k<cn2;k++)
				sum+=s2[k];
			if(sum<=i+2*j)minn=i+2*j;
		}
	cout<<minn<<endl;
	return 0;
}