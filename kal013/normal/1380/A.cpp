...
void solve(){
	...
	vector<int> p(n + 1);
	vector<pair<int,int> > pref(n + 1), suf(n+2);
	for(int i = 1; i <= n; ++i){
		cin >> p[i];
	}
	...
	for(int i = 1; i <=n ;++i){
		pref[i] = min(pref[i-1],{p[i],i});
	}

	suf[n] = {p[n],n};

	for(int i = n - 1; i >=1; --i){
		suf[i] = min(suf[i+1],{p[i],i});
	}

	for(int j = 2; j < n; ++j){
		pii X = pref[j - 1], Y = suf[j + 1];
		if (X.first < p[j] && Y.first < p[j]){
			cout << "YES\n";
			cout << X.second << " " << j << " " << Y.second << "\n";
			return;
		}
	}
