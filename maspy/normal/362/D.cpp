...
void solve() {
  ...
  vc<pi> ANS;
  pqg<pi> que;
  FOR(v, N) if (uf[v] == v) que.push(mp(A[v], v));
  while (C > q) {
    assert(len(que) >= 2);
    auto [av, v] = que.top();
    que.pop();
  ...
  if (q == N && len(ANS) < p) return NO();
  ll v = [&]() -> ll {
    FOR(v, N) if (uf[v] != v) return v;
    return -1;
  }();

  while (len(ANS) < p) { ANS.eb(v, uf[v]); }

  if (len(ANS) != p) return NO();
  YES();
