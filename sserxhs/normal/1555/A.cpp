...
int main()
{
	...
	read(T);
	n=1e4;
	for (i=1;i<=6;i++) f[i]=15;
	f[7]=f[8]=20;f[9]=f[10]=25;
	for (i=11;i<=n;i++) f[i]=min(f[i-8]+20,min(f[i-10]+25,f[i-6]+15));
	while (T--)
	{
		ll n;
		read(n);m=1e4;
		ll x=n;if (x%2==0) x/=2; else x=x+1>>1;x*=5;
		if (n>=m) enter(x); else enter(f[n]);
	}
}