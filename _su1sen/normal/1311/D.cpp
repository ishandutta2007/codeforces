...
int main() {
    ...
    loop(t) {
        ...
        rep(B, 1, M) {
            ...
            auto it = lower_bound(all(divisors[B]), a);
            int x = numeric_limits<int>::max();
            if (it != divisors[B].end()) {
                if (chmin(x, *it - a)) {
                    A = *it;
                }
            }
            if (it != divisors[B].begin()) {
                if (chmin(x, a - *--it)) {
                    A = *it;
                }
            }
            if (chmin(ans, x + z + abs(b - B))) {
                ansp = {A, B, C};
            }
        }
        print(ans);
        print(get<0>(ansp), get<1>(ansp), get<2>(ansp));
    }
    return 0;
}