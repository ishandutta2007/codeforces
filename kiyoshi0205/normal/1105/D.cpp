...
int main(){
  ...
  rep(i,H)rep(j,W)if(isdigit(g[i][j]))que.emplace(g[i][j]-'1',v[g[i][j]-'1'],i*W+j);
  while(!que.empty()){
    auto [d,nokori,x]=que.top();
    que.pop();
    auto a=x/W;auto b=x%W;
    if(!chmin(dist[a][b],d))continue;
    debug(a,b,d);
    if(nokori!=v[d%K]){
      if(a&&g[a-1][b]=='.')que.emplace(d,nokori+1,x-W);
      if(a<H-1&&g[a+1][b]=='.')que.emplace(d,nokori+1,x+W);
      if(b&&g[a][b-1]=='.')que.emplace(d,nokori+1,x-1);
      if(b<W-1&&g[a][b+1]=='.')que.emplace(d,nokori+1,x+1);
    }else{
      if(a&&g[a-1][b]=='.')que.emplace(d+K,1,x-W);
      if(a<H-1&&g[a+1][b]=='.')que.emplace(d+K,1,x+W);
      if(b&&g[a][b-1]=='.')que.emplace(d+K,1,x-1);
      if(b<W-1&&g[a][b+1]=='.')que.emplace(d+K,1,x+1);
    }
  }
  vec ans(K,0);
  rep(i,H)rep(j,W){
