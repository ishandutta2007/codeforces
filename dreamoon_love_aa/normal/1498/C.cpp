...
int u[SIZE][SIZE][2];
int n;
LL f(int pos, int k, int dir){
    if(u[pos][k][dir==1]) return dp[pos][k][dir==1];
    u[pos][k][dir==1]=1;
    dp[pos][k][dir==1]=0;
    if(pos+dir<=n&&pos+dir>=1){
        ADD(dp[pos][k][dir==1],f(pos+dir,k,dir));
    }
    if(k>1){
        ADD(dp[pos][k][dir==1],1);
        if(pos-dir>=1&&pos-dir<=n)ADD(dp[pos][k][dir==1],f(pos-dir,k-1,-dir));
    }
    return dp[pos][k][dir==1];
}
int k;
void solve() {
    FOR(i,1,n)FOR(j,1,k)u[i][j][0]=u[i][j][1]=0;
    LL an=f(1,k,1);
    ADD(an,1);
    W(an);
}
