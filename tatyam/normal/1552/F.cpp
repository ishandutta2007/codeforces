...
void solve(){
    ...
    VEC(tuplis,a,n);
    vec(Modint,cost,n*2);
    vec(Modint,cost_sum,n*2);
    rep(n){
        auto[x,y,f]=a[i];
        const ll at=lower_bound(all(a),tuplis{y,0,0})-a.begin();
        cost[i*2]=a[at][0]-y+cost_sum[i*2]-cost_sum[at*2];
        cost_sum[i*2+1]=cost_sum[i*2]+cost[i*2];
        if(i+1==n)continue;
        cost[i*2+1]=a[i+1][0]-x;
        cost_sum[i*2+2]=cost_sum[i*2+1]+cost[i*2+1];
    }
    Modint ans=cost_sum.back()+1+a[0][0];
    rep(n){
        auto[x,y,f]=a[i];
        if(f==0)ans-=cost[i*2];
    }
