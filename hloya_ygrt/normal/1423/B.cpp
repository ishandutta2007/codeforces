...
//#pragma GCC target("sse,sse2,sse3,ssse3,sse4,abm,mmx,avx2")
...
bool dfs(int u) {
  for (int &i = pt[u]; i < g[u].size(); ++i) {
    int v = g[u][i].first;
    if (us[v]==tim&&dist[v] == dist[u] + 1) {
      if (!matched[v] || (us[matched[v]]==tim&&dist[matched[v]] == dist[v] + 1 && dfs(matched[v]))) {
        matched[v] = u;
        matched[u] = v;
        return true;
      }
