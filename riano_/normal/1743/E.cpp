...
auto dp = v2ll(5001,5001,2e18);

int main(){
    riano_; ll ans = 2e18; 
    ll p1,p2,t1,t2,h,s; cin >> p1 >> t1 >> p2 >> t2 >> h >> s;
    vector<ll> mt(5001,2e18);
    rep(i,5001){
        rep(j,5001){
            if(i==0&&j==0) continue;
            ll d = p1*i+p2*j-s*(i+j);
            if(i>0&&j>0) d += s;
            chmin(d,5000LL);
            chmin(mt[d],max(t1*i,t2*j));
            if(d==5000) break;
        }
    }
    ...
    dp[0][0] = 0;
    rep(i,5000){
        ll k = i+1; ll c = mt[k]; ll mn = 2e18;
        rep(j,k){
            for(int l=j;l<10001;l+=k){
                if(l>5000){
                    l = 5000;
                }
                chmin(mn,dp[i][l]);
