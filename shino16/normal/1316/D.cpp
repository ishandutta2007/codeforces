...
int main() {
  int n = IN;
  VVI x, y; x = y = make_v(n, n, 0);
  rep(r, n) rep(c, n) IN >> x[r][c] >> y[r][c];
  rep(r, n) rep(c, n) if (x[r][c] != -1) x[r][c]--, y[r][c]--;


  int X = 4;
  string str = "DURLX";
  auto res = make_v(n, n, -1); auto done = make_v(n, n, false);
  rep(r, n) rep(c, n) if (x[r][c] != -1) res[x[r][c]][y[r][c]] = X;
  rep(r, n) rep(c, n) if (res[r][c] == X and (x[r][c] != r or y[r][c] != c)) EXIT("INVALID");
  rep(r, n) rep(c, n) if (x[r][c] == -1) {
    bool found = false;
    rep(i, 4) if (inside(r+dr[i], c+dc[i], n, n) and x[r+dr[i]][c+dc[i]] == -1) {
      found = true;
      res[r][c] = i;
    }
    if (not found) EXIT("INVALID");
  }

  auto dfs = fix([&](auto f, auto r, auto c)->void{
    rep(i, 4) if (inside(r+dr[i], c+dc[i], n, n) and not done[r+dr[i]][c+dc[i]]) {
      if (x[r][c] == x[r+dr[i]][c+dc[i]] and y[r][c] == y[r+dr[i]][c+dc[i]]) {
        done[r+dr[i]][c+dc[i]] = true;
        if (r == 0) {
          debug(r); debug(c); debug(i); debug(r+dr[i]); debug(c+dc[i]);
        }
        res[r+dr[i]][c+dc[i]] = i ^ 1;
        f(r+dr[i], c+dc[i]);
      }
    }
  });

  rep(r, n) rep(c, n) if (res[r][c] == X) {
    done[r][c] = true;
    dfs(r, c);
  }

  rep(r, n) rep(c, n) if (res[r][c] == -1) EXIT("INVALID");
  OUT("VALID");
  rep(r, n) {
    rep(c, n) OUT << str[res[r][c]];
    OUT << newl;
  }
}