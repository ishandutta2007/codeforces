...
int fin_dep[nax];
vector <int> path;
void dfs1(int x, int d = 0) {
	dep[x] = d;
	deepest[x] = {d, x};
	for (int v : down[x]) {
		dfs1(v, d + 1);
		if (maxi(deepest[x], deepest[v]))
			way[x] = v;
	}
	debug(arr(dep, x), arr(deepest, x));
}
void calc(int x) {
	if (fin_dep[x] || x == 0) return;
	calc(up[x]);
	fin_dep[x] = fin_dep[up[x]] + 1;
}
void find_path() {
	for (int i = 0; i < n; ++i) calc(i);
	for (int i = 0; i < n; ++i) if (fin_dep[i] == n - 1) {
		int x = i;
		for (int j = 0; j < n; ++j) {
			path.push_back(x);
			x = up[x];
...
void solve(int x) {
	if (down[x].empty()) return;
	solve(way[x]);
	int moved = way[x];
	for (int v : down[x]) if (v != way[x]) {
		debug(arr(deepest, v), arr(dep, x));
		for (int i = 0; i < deepest[v].first - dep[x]; ++i)
			ans.push_back(moved);
		debug(imie(ans));
		up[moved] = deepest[v].second;
		moved = v;
		solve(v);
	}
}
