...
int sz[MAXN];
signed main(){
    read(n, m); rep(i, 1, m) read(u[i], v[i]);
    if(!m) {
        print(3, 1ll * n * (n - 1) * (n - 2) / 6);
        return 0;
    }
    rep(i, 1, n) fa[i] = i, sz[i] = 1;
    rep(i, 1, m) {
        const int U = find(u[i]), V = find(v[i]);
        if(U ^ V) {link(u[i], v[i]); fa[U] = V; sz[V] += sz[U]; b[i] = 1; }
    }
    rep(i, 1, n) if(!vis[find(i)]) {c[find(i)] = dfs(find(i), 0); vis[find(i)] = 1;}
    rep(i, 1, m) if(!b[i] && (dep[u[i]] & 1) == (dep[v[i]] & 1)) return print("0 1"), 0;
    int md = 0; rep(i, 1, n) chmax(md, dep[i]);
    if(md == 1) {
        int cc = 0; rep(i, 1, n) cc += dep[i];
        print(2, 1ll * cc * (n - 2));
        return 0;
    }
    LL ans = 0;
    rep(i, 1, n) {
        if(dep[i] & 1) ans += c[find(i)];
        else ans += sz[find(i)] - c[find(i)];
    }
    ans -= n; ans >>= 1; print(1, ans);
}
/*
 3
 0 1
