...
void solve() {
  ...
  const int m = c.size();
  LazySegmentTree<monoid::RangeMaximumAndAddQuery<int, INF>> seg(m);
  seg.set(lower_bound(ALL(c), 0) - c.begin(), 0);
  vector<int> dp(n + 1, -INF);
  dp[0] = 0;
  map<ll, int> mp{{0, 0}};
  FOR(i, 1, n + 1) {
    seg.apply(0, m, 1);
    const int idx = lower_bound(ALL(c), b[i]) - c.begin();
    dp[i] = dp[i - 1] + sgn(a[i - 1]);
    if (mp.count(b[i])) chmax(dp[i], mp[b[i]]);
    chmax(dp[i], seg.get(0, idx));
    if (!mp.count(b[i])) mp[b[i]] = -INF;
    chmax(mp[b[i]], dp[i]);
    if (seg[idx] < dp[i]) seg.set(idx, dp[i]);
  }
  // REP(i, n + 1) cout << dp[i] << " \n"[i == n];
  cout << dp[n] << '\n';
