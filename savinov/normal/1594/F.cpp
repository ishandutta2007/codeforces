...
bool solve(int n, int s, int k) {
    s -= n;
    assert(s >= 0);

    result.resize(n);

    function<bool(int, int, int, vector <int>&)> rec = [&](int pos, int cur, int rem, vector <int> &psum) {
        if (pos == n) {
            return rem == 0;
        }

        for (int delta = 0; delta <= rem; ++delta) {
            if (cur + delta + 1 >= k && psum[cur + delta + 1 - k]) continue;
            result[pos] = delta + 1;
//            cerr << cur << " " << rem << " " <<  cur + delta + 1 << " " << psum.size() << "\n";
            assert(cur + delta + 1 < psum.size());
            psum[cur + delta + 1] = 1;
            auto res = rec(pos + 1, cur + delta + 1, rem - delta, psum);
            psum[cur + delta + 1] = 0;
    ...
    vector <int> psum(s + n + 1);
    psum[0] = 1;

    return rec(0, 0, s, psum);
}

bool solve2(ll n, ll s, ll k) {
    if (s == k) return false;
    if (k > n) return true;
//    if (2 * k <= n) {
        ll steps = n / k;
...
//        for (int k = 1; k <= n + 1; ++k) {
//            cerr << n << " " << k << "\n";
//            for (int s = n; s <= 2 * n + 10; ++s) {
//                if (solve(n, s, k) != solve2(n, s, k)) {
//                    cerr << "\n! " << n << " " << s << " " << k << " " << solve(n, s, k) << " " << solve2(n, s, k) << "\n";
//                }
//                assert(solve(n, s, k) == solve2(n, s, k));
////                if (!solve(n, s, k)) {
////                    cerr << s << " ";
////                } else {
////                    cerr << endl;
