...
struct CombSum {
	int n, klo, khi;
	int sum; // sum(k=klo..khi,choose(n,k))

	CombSum() { n = 0, klo = 0, khi = 0, sum = 1; }

	// if klo<=khi sum(k=klo..khi,choose(n+1,k))=sum(k=klo..khi,choose(n,k-1)+choose(n,k))=2*sum+choose(n,klo-1)-choose(n,khi)
	int upd(int nn, int nklo, int nkhi) {
		while (n < nn) {
			if (klo <= khi) sum = ((ll)2 * sum + choose(n, klo - 1) + MOD - choose(n, khi)) % MOD;
			++n;
		}
		while (n > nn) {
			if (klo <= khi) sum = ((ll)sum + MOD - choose(n - 1, klo - 1) + choose(n - 1, khi))*INV2%MOD;
			--n;
		}
		while (klo > nklo) {
			--klo;
			if (klo <= khi) inc(sum, choose(n, klo));
		}
		while (klo < nklo) {
			if (klo <= khi) dec(sum, choose(n, klo));
			++klo;
		}
		while (khi > nkhi) {
			if (klo <= khi) dec(sum, choose(n, khi));
			--khi;
		}
		while (khi < nkhi) {
			++khi;
			if (klo <= khi) inc(sum, choose(n, khi));
		}
		return sum;
	}
};

CombSum asum, bsum, csum, dsum;
...
int calc(int evenq, int oddq, int evenw, int oddw) {
	...
	if (evenq + oddq == 1) {
		...
		num = (num + (ll)amlt*asum.upd(an, alo, ahi)) % MOD;
		num = (num + (ll)bmlt*bsum.upd(bn, blo, bhi)) % MOD;
		num = (num + (ll)cmlt*csum.upd(cn, clo, chi)) % MOD;
		num = (num + (ll)dmlt*dsum.upd(dn, dlo, dhi)) % MOD;
