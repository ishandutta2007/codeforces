...
/*}}}*/
const int SIZE = 1<<20;
#define UnionFindSIZE 123456
struct Union_Find {
    int d[UnionFindSIZE], num[UnionFindSIZE];
    int good[UnionFindSIZE];
    void init(int n){
        for(int i = 0; i < n; i++) {
            d[i] = i;
            num[i] = 1;
...
void solve() {
    REP(i,30){
        U[i].init(n);
        REP(j,m){
            if(get_bit(w[j],i))U[i].uu(a[j],b[j]);
        }
    }
    REPP(i,1,30){
        V[i].init(n);
        REP(j,m){
            if(get_bit(w[j],i)&&get_bit(w[j],0))V[i].uu(a[j],b[j]);
        }
        REP(j,n){
            for(auto &[y,v]:e[j]){
                if(!(v&1))V[i].good[V[i].find(j)]=1;
            }
        }
    }
    int q;
