...
#define GETVALUE(n) __m128i ivalue##n = _mm_loadu_si128(&xarray[n])
#define XORVALUE(n) __m128i value##n = _mm_xor_si128(ivalue##n, pxor)
...
int countThem(int *a, int n, int p, int k) {
    __m128i *xarray = (__m128i*)a;
    __m128i xboundary1 = _mm_set1_epi32(k - 1);
    __m128i pxor = _mm_set1_epi32(p);
    __m128i count = _mm_setzero_si128();
    int n16 = n / 16;
    while (n16--) {
        GETVALUE(0); GETVALUE(1); GETVALUE(2); GETVALUE(3);
        XORVALUE(0); XORVALUE(1); XORVALUE(2); XORVALUE(3);
    ...
    return n + _mm_cvtsi128_si32(count);
}
