...
inline void work() {
    read(n); read(a + 1, a + n + 1); std::sort(a + 1, a + n + 1);
    bool x = 0, y = 0, z = 0; int ans = a[n];
    rep(i, 1, n) {
        if(a[i] * 2 == a[n]) x = 1;
        if(a[i] * 3 == a[n]) y = 1;
        if(a[i] * 5 == a[n]) z = 1;
    } if(x && y && z) chmax(ans, a[n] / 2 + a[n] / 3 + a[n] / 5);
    int cnt = 0, stk[3], num = 0;
    per(i, n, 1) {
        if(cnt == 3) break;
        bool ck = 1; rep(j, 0, cnt - 1) if(stk[j] % a[i] == 0) ck = 0;
        if(ck) stk[cnt ++] = a[i], num += a[i];
    }
    print(mx(ans, num));
