...
struct Node {
	...
	bool operator < (const Node &oth) const {
		return v == oth.v ? id < oth.id : v < oth.v;
	}
} w[N];
...
int main() {
	...
	W(T) {
		...
		int n, m, x;
		qread(n, m, x);
		rep(i, 1, n) qread(w[i].v), w[i].id = i;
		sort(w + 1, w + n + 1);
		rep(i, 1, m) st.insert((Node){0, i});
		per(i, n, 1) {
			Node now = *st.begin(); st.erase(now);
			now.v += w[i].v;
			pos[w[i].id] = now.id;
			st.insert(now);
		}
		Node ans = *st.begin(), res = *st.rbegin();
		if(res.v <= ans.v + x) {
			puts("YES");
