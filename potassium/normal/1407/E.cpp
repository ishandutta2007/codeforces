...
int main() {
	...
	while (!Q.empty()) {
		...
		if (d < max(dist[u][c], dist[u][!c])) continue;
		
		for (auto& e : g[u]) {
			if (chkmin(dist[e.F][e.S], d + 1)) {
				Q.push({dist[e.F][e.S], {e.F, e.S}});
			}
		}
	}
	
	int ansv = max(dist[1][0], dist[1][1]);
	W(ansv >= 1e9 ? -1 : ansv);
	
	string ans;
	for (int i = 1; i <= n; i++)
		ans += (dist[i][0] > dist[i][1] ? '0' : '1');
