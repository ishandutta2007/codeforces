...
int go(int cur, int hold, bool rec) {
    if (cur == n-1) {
        if (rec) {
            ans.push_back({cur+1, hold+1});
        }
        return max(a[cur], a[hold]);
    }
    if (cur == n) {
        if (rec) {
            ans.push_back({hold+1, -1});
        }
        return a[hold];
    }
    ...
    if (ret == INF || rec) {
        ret = INF;
        vi can = {hold, cur, cur+1};

        int best_h = -1;
        for (int nh = 0; nh < 3; nh++) {
            int ott = 0;
            for (int o = 0; o < 3; o++) if (o != nh) ott = max(ott, a[can[o]]);
            int nv = go(cur+2, can[nh], false) + ott;
            if (nv < ret) {
                ret = nv;
                best_h = nh;
            }
        ...
        if (rec) {
            vi al;
            for (int o = 0; o < 3; o++) if (o != best_h) al.push_back(can[o]);
            ans.push_back({al[0] + 1, al[1] + 1});
            go(cur+2, can[best_h], true);
        }
    }
