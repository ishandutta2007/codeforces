...
int main(){
  ...
  for(li i = 0; i < (li)(N-1); ++i) {
    dp0[i+1] = 1 + dp0[i] + (dp0[i]+1) * dp0[i] / ZN(2);
  }
  for(li i = 0; i < (li)(N-1); ++i) {
    dp2[i+1] = (dp0[i]+1) * dp0[i] / ZN(2);
    if(i>0) dp2[i+1] -= (dp0[i-1]+1) * dp0[i-1] / ZN(2);
    dp3[i+1] = (dp0[i]+2) * (dp0[i]+1) * dp0[i] / ZN(6);
    if(i>0) dp3[i+1] -= (dp0[i-1]+2) * (dp0[i-1]+1) * dp0[i-1] / ZN(6);
  }
  ...
  for(li i = (1); i <= (li)(n-1); ++i) ans += dp2[i] * (dp0[n-i-1] - (n-i-2>=0?dp0[n-i-2]:0));
  ans += dp2[n];
