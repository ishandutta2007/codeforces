...
struct mincostflow {
    ...
    const T inf_T = numeric_limits<T>::max(), eps = 1e-6; // set epsilon for non-integral types
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        f.edge(N-2, i, 1, -q[i]);
        f.edge(i, N-1, 1, 0);
        f.edge(i, N-1, 1, p[i]*q[i]);
    }
    auto [flow, cost] = f.solve(N-4, N-1);
    cout << setprecision(25) << fixed << -cost << "\n";
}