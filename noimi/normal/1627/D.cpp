...
struct divisor_transform {
    template <typename T> static void zeta_transform(vector<T> &a) {
        int N = a.size() - 1;
        auto sieve = prime_enumerate(N);
        for(auto &p : sieve)
            for(int k = 1; k * p <= N; ++k) a[k * p] += a[k];
    }
    template <typename T> static void mobius_transform(T &a) {
        int N = a.size() - 1;
        auto sieve = prime_enumerate(N);
        for(auto &p : sieve)
    ...
    template <typename T> static void zeta_transform(map<long long, T> &a) {
    ...
    template <typename T> static void mobius_transform(map<long long, T> &a) {
        for(auto &x : a)
            for(auto p = rbegin(a); p != rend(a); p++) {
...
struct multiple_transform {
    ...
    template <typename T> static void mobius_transform(vector<T> &a) {
        int N = a.size() - 1;
        auto sieve = prime_enumerate(N);
        for(auto &p : sieve)
...
template <typename mint> vector<mint> gcd_convolution(const vector<mint> &a, const vector<mint> &b) {
    assert(a.size() == b.size());
    auto s = a, t = b;
    multiple_transform::zeta_transform(s);
    // multiple_transform::zeta_transform(t);
    for(int i = 0; i < (int)a.size(); i++) s[i] = s[i].pow(1000001);
...
int main() {
    ...
    vmint v(ten(6) + 1);
    fore(e, a) v[e] = 1;
    auto res = gcd_convolution(v, v);
    int sum = 0;
    rep(i, si(res)) if(res[i].a) dump(i);
    OUT(si(res) - count(all(res), mint(0)) - n);
}