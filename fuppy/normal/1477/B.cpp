...
int h(int a, int b, int l, int r) {return (r - l) * b;}
...
void solve() {
    ...
    vi temp;
    rep (i, t.size()) temp.push_back(t[i] - '0');
    LazySegmentTree<int, int> seg(temp, f, g, h, 0, -1);

    rrep (i, q) {
        int l = lr[i].first, r = lr[i].second;
        int len = r - l;
        int su = seg.query(l, r, 0, 0, seg.n);
        if (2 * su == len) {
            cout << "NO" << '\n';
            return;
        }
    ...
    rep (i, n) {
        if (seg.query(i, i + 1, 0, 0, seg.n) != (s[i] - '0')) {
            cout << "NO" << '\n';
            return;
        }
    }
