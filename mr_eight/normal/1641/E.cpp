...
int n,m,p[1000002],f[1000002],g[1000020],pww[100002],ipw[100002],a[1000002],v[1000002];
inline void solve(int l,int r){
	if(l==r)return;
	int mid=(l+r)>>1;
	solve(l,mid);solve(mid+1,r);
	auto x=A.subpoly(l,mid+1),y=B.subpoly(mid+1,r+1);
	x=x*y;//for(auto i:x.a)cerr<<i<<" ";cerr<<"  ";for(auto i:y.a)cerr<<i<<" ";cerr<<endl;
	F(i,0,x.size())(res[i+l+mid+1]+=x[i])%=mod;
}
int main() {
	init_inv_with_fac(1050000);
	NTT_set(1050000);res.resize(1000000);
	pww[0]=ipw[0]=1;
	F(i,1,100001)pww[i]=pww[i-1]*2%mod,ipw[i]=1ll*ipw[i-1]*inv2%mod;
	cin>>n>>m;n*=3;
	A.resize(n);B.resize(n);C.resize(n);D.resize(n);
	F(i,1,n/3)read(v[i]);
	F(i,1,m)read(p[i]),p[i]+=n/3;
	F(i,1,m){
		f[p[i]]++;
	}
	f[0]=1;
	...
	//F(i,1,n)cerr<<A[i]<<" "<<B[i]<<" "<<C[i]<<" "<<D[i]<<endl;
	solve(0,n);
	A=C,B=D;
	solve(0,n);
	F(i,1,n)a[i]=1ll*pww[m]*res[(i+n/3)*2-1]%mod;
	//F(i,1,n)cerr<<a[i]<<" ";
	ll ans=0;
	F(i,1,n){
		a[i]-=min(i+n/3,n-i-n/3+1);//cerr<<a[i]<<" ";
		ans+=1ll*a[i]*v[i]%mod;
	}
	cout<<getm(ans%mod,pww[m]-1)%mod;
	return 0;
}