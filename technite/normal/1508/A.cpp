...
int main()
{
    ...
    TT
    {
        ...
        string a,b,c;
        cin>>a>>b>>c;
        vector<pair<int,string>> cnt(3);
        int cnt1=0,cnt2=0,cnt3=0;
        cnt[0]=make_pair(solve(a),a);
        cnt[1]=make_pair(solve(b),b);
        cnt[2]=make_pair(solve(c),c);
        sort(cnt.begin(),cnt.end());
        string ans="";
        char common;
        if(cnt[1].F>=n and cnt[2].F>n)
        {
            a=cnt[1].S;
            b=cnt[2].S;
            common='0';
        ...
        while(i<2*n or j<2*n)
        {
            ...
            while(a[i]!=common and i<2*n)
            {
                ans+=a[i];
                i++;
            }
            while(b[j]!=common and j<2*n)
            {
                ans+=b[j];
                j++;
            }
