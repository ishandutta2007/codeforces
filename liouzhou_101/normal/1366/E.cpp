...
int n, m;
int a[MAXN], b[MAXN], pos[MAXN], mina[MAXN];
int L[MAXN], R[MAXN];
vector<int> v[MAXN];
ll f[MAXN];
...
void solve()
{
	...
	for (int j = 1; j <= m; ++j)
	{
		if (!H.count(b[j])) return writeln(0);
	}

	if (H.begin()->first != b[1]) return writeln(0);
	...
	for (auto [val, v] : H)
	{
		int j = lower_bound(b + 1, b + m + 1, val) - b;
		if (b[j] == val) continue;
		R[j-1] = max(R[j-1], v.back());
	}
	for (int j = 2; j <= m; ++j)
	{
		if (R[j - 1] >= L[j]) return writeln(0);
	}

	ll res = 1;
	for (int j = 1; j < m; ++j)
	{
		res = res * (L[j + 1] - R[j]) % MOD;
	}
	writeln(res);
}
