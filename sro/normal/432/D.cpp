...
//
/*
int rnk[100005],tmp[100005],lcp[100005],sa[100005],vcl[100005];
int n;
int K;
char s[100005];
const int Ah=19260817;
INT mii[100005];
...
bool sort_comp(int a,int b){
	...
	int Fa=(a+K<n?rnk[a+K]:-1);
	int Fb=(b+K<n?rnk[b+K]:-1);
	return Fa<Fb;
}
...
int main(){
	...
	K=1;
	while(K<n){
		sort(sa,sa+n+1,sort_comp);
		tmp[sa[0]]=0;
		for(int i=1;i<=n;i++){
			tmp[sa[i]]=tmp[sa[i-1]]+sort_comp(sa[i-1],sa[i]);
		}
		for(int i=1;i<=n;i++){
			rnk[i]=tmp[i];
		}
	...
	for(int i=0;i<n;i++){
		ha1=ha1*Ah+s[i];
		ha2=ha2+mii[i]*s[n-i-1];
		//cerr<<ha1<<' '<<ha2<<endl;
		if(ha1==ha2){
			ans.pb(mp(i,vcl[n-i-1]));
		}
	}
	fio<<SZ(ans)<<endl;
	for(int i=0;i<SZ(ans);i++){
		fio<<ans[i].first+1<<' '<<ans[i].second<<endl;
	}
	return 0;
}
