...
public class E {
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			...
			SegmentTreeSet tree = new SegmentTreeSet(f);
			for (int i = q - 1; i >= 0; i--) {
				int bal = tree.get(1, 0, tree.pow - 1, l[i], r[i]);
				int len = (r[i] - l[i] + 1);
				if (bal < (len + 1) / 2) {
					tree.set(1, 0, tree.pow - 1, l[i], r[i], 0);
				} else if (bal > len / 2) {
					tree.set(1, 0, tree.pow - 1, l[i], r[i], 1);
				} else {
					out.println("NO");
					continue f;
				}
			...
			for (int i = 0; i < n; i++) {
				if (s[i] != tree.get(1, 0, tree.pow - 1, i, i)) {
					out.println("NO");
					continue f;
				}
			}
