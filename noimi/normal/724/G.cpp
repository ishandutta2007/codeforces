...
int main() {
    ...
    for(auto e : E) {
        int r = uf[e.from];
        ll x = v[e.from] ^ v[e.to] ^ e.cost;
        for(auto e : basis[r]) chmin(x, x ^ e);
        if(x) basis[r].eb(x);
    }
    mint ans = 0;
    vmint p(61);
    p[0] = 1;
    rep(i, 60) p[i + 1] = p[i] * 2;
    auto bel = uf.belong();
    // dump(basis);
    rep(i, n) {
        if(uf[i] == i) {
            int N = uf.size(i);
            ll k = 0;
            for(auto e : basis[i]) k |= e;
            rep(b, 61) {
                if(k & 1LL << b) {
                    ans += p[si(basis[i])] * N * (N - 1) / 4 * p[b];
                } else {
                    int cnt = 0;
                    for(auto e : bel[i])
                        if(v[e] & 1LL << b) cnt++;
                    ans += p[si(basis[i])] * cnt * (N - cnt) * p[b];
                }
            }
        }
    }
