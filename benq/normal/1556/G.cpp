...
int main() {
	...
	// dbg(_sig);
	dbg(sig);
	F0R(i,sz(sig)-1) assert(sig[i] < sig[i+1]);
	...
	each(t,query) {
		...
		if (c == 'B')
		FOR(i,comp(x),comp(y)) {
			// dbg("DEACTIVATE",i,sig[i],sig[i+1])
			active[i] = 0;
		}
	}
	auto activate = [&](int x) {
		// dbg("START ACTIVE");
		active[x] = 1;
		// power of 2 size interval
		ll a = sig.at(x), b = sig.at(x+1);
		// dbg("ACTIVATE",x,a,b);
		ll dif = b-a;
		assert(pct(dif) == 1);
		// sig.at(x), sig.at(x+1)
		F0R(i,N) if (dif <= (1LL<<i)) {
			// [0,3]
	...
	// dbg(sig);
	F0R(i,sz(active)) if (active[i]) activate(i);
	// dbg("---");
	// F0R(i,1<<N) dbg(i,comp(i));
