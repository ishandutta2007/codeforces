...
void solve() {
    ...
    int ans = 0;
        for (int q = 0; q < m; ++q) {
            up[0][q] = 0;
            for (int j = 0; j < n; ++j) {
                if (j != 0 and inputs[j][q] != inputs[j-1][q]) {
                    up[j][q] = 0;
                }
                up[j + 1][q] = up[j][q] + 1;
            }
            down[n - 1][q] = 0;
            for (int j = n - 1; j >= 0; --j) {
                if (j != n-1 and inputs[j][q] != inputs[j+1][q]) {
                    down[j][q] = 0;
                }
                if (j - 1 >= 0) {
                    down[j - 1][q] = down[j][q] + 1;
                }
            }
        }
        REP(q, n) {
            int now_l = -1;
            REP(j, m) {
                now_l = min(now_l + 1, min(up[q][j], down[q][j]));
                if (j != 0 and inputs[q][j] != inputs[q][j - 1]) {
                    now_l = min(now_l, 0);
                }
                lefts[j] = now_l;
            }
            int now_r = -1;
            REP(j, m) {
                now_r = min(now_r + 1, min(up[q][m - j - 1], down[q][m - j - 1]));
                if (j != 0 and inputs[q][m - j - 1] != inputs[q][m - j]) {
                    now_r = min(now_r, 0);
                }
                rights[m - j - 1] = now_r;
            }
            REP(j, m) {
                ans += min(lefts[j], rights[j]) + 1LL;
            }
        }
    cout << ans << "\n";
}
