...
int main() {
  ...
  alive[0] = false;
  while (true) {
    int c = count(ALL(alive), true);
    if (c == 1) {
      int v = g.seq[sz(begin(alive), find(ALL(alive), true))];
      printf("! %d %d\n", v + 1, g.p(v) + 1);
      fflush(stdout);
      break;
    }
    ...
    for (int i : rep(n))
      if (alive[i]) {
        if (c-- == 0) break;
        v.push_back(g.seq[i]);
        v.push_back(g.p(g.seq[i]));
      }
    sort(ALL(v));
    v.erase(unique(ALL(v)), end(v));
    c = count(ALL(alive), true) / 2;
    if (query(v) == mx) {
      int t = 0;
      for (int i : rep(n))
        if (alive[i]) {
          if (t >= c) alive[i] = false;
          ++t;
        }
    } else {
      int t = 0;
