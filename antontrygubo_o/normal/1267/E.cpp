...
int main() {
    ...
    vector<int> answer;
    for (int i = 0; i<m; i++) answer.push_back(i);
    ...
    for (int i = 0; i<n-1; i++)
    {
        ...
        for (int j = 0; j<m; j++) diff.push_back(mp(a[j][i]-a[j][n-1], j));
        ...
        {
            answer.clear();
            for (int j = 0; j<last; j++) answer.push_back(diff[j].second);
        }
    }
    cout<<answer.size()<<endl;
