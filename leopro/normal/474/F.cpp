...
struct segment_tree {
    struct segment {
        ...
        segment left() const { return {l, (l + r) / 2, 2 * i + 1}; }

        segment right() const { return {(l + r) / 2, r, 2 * i + 2}; }
    };
    ...
    struct data {
        ...
        data(int g = 0, int c = 0) : gcd(g), cnt(c) {}

        friend data merge(const data &g_c_1, const data &g_c_2) {
            int g = ::gcd(g_c_1.gcd, g_c_2.gcd);
            int cnt = 0;
            if (g == g_c_1.gcd) cnt += g_c_1.cnt;
            if (g == g_c_2.gcd) cnt += g_c_2.cnt;
            return {g, cnt};
        }
    };
    ...
    explicit segment_tree(const vector<int> &s) : n(1) {
        while (n < s.size()) n <<= 1;
        ans.assign(2 * n - 1, {});
        for (int i = 0; i < s.size(); ++i) ans[i + n - 1] = {s[i], 1};
        for (int i = n - 2; i >= 0; --i) ans[i] = merge(ans[2 * i + 1], ans[2 * i + 2]);
    }

    int count(query q) { return count(q, {0, n, 0}).cnt; }

    data count(const query &q, const segment &s) {
        if (s.r <= q.l || q.r <= s.l) return 0;
        if (q.l <= s.l && s.r <= q.r) return ans[s.i];
        return merge(count(q, s.left()), count(q, s.right()));
    }
};
