...
int main()
{
	...
	{
		sort(b[i]+1,b[i]+m+1);
		int tot=unique(b[i]+1,b[i]+m+1)-b[i]-1;
		h[i]=tot;
		for(j=1;j<=m;j++)d[i][j]=lower_bound(b[i]+1,b[i]+tot+1,a[i][j])-b[i];
	}
	for(i=1;i<=m;i++)
	{
		sort(c[i]+1,c[i]+n+1);
		int tot=unique(c[i]+1,c[i]+n+1)-c[i]-1;
		l[i]=tot;
		for(j=1;j<=n;j++)a[j][i]=lower_bound(c[i]+1,c[i]+tot+1,a[j][i])-c[i];
	}
	for(i=1;i<=n;i++)
	{
		for(j=1;j<=m;j++)
		{
			printf("%d ",max(d[i][j]-1,a[i][j]-1)+1+max(h[i]-d[i][j],l[j]-a[i][j]));
		}
		puts("");
	}
}