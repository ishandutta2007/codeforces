        ...
        for(int j=0;j<=i;j++)
            if(i%2==j%2)
            {
                ...
                if(!ok(i-1,j+1))
                    ...
                    dp[i][j][1]=(dp[i-1][j-1][0]+1)%MOD;
                else if(!ok(i-1,j-1))
                    dp[i][j][0]=dp[i-1][j+1][2],
                    dp[i][j][1]=(dp[i-1][j+1][0]+1)%MOD;
                else
                    dp[i][j][0]=(dp[i-1][j-1][2]+dp[i-1][j+1][2])%MOD,
                    dp[i][j][1]=max(dp[i-1][j-1][0]+1+dp[i-1][j+1][2],
                                    dp[i-1][j-1][2]+dp[i-1][j+1][0]+1)%MOD;
