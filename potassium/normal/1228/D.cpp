...
int n,m;
vector<set<int>> g(100007);
vector<ll> hashv;
vector<vector<int>> cc;
int curcc = 0;
bitset<100007> isTrav = 0;
...
inline ll hashx(int node) {
	const ll h = pow(13,7,MOD);
	ll hashed = 0;
	for (auto& i : g[node]) {
		hashed *= h;
		hashed += i;
...
int main() {
	...
	for (int i = 0; i < curcc; i++) {
		for (int j = 1; j <= 3; j++) {
			...
			for (int k = 0; k < cc[i].size(); k++)
				if (color[cc[i][k]] == 0 && g[cc[i][k]].count(n1) == 0) {
					if (g[cc[i][k]].size() != g[n1].size() || hashv[cc[i][k]] != hashv[n1]) no();
					color[cc[i][k]] = j;
				}
		}
	}
	...
	int cnt[3] = {0,0,0};
	for (int i = 1; i <= n; i++) {
		if (color[i] != 1 && color[i] != 2 && color[i] != 3) no();
		cnt[color[i]-1]++;
	}

	if (cnt[0] == 0 || cnt[1] == 0 || cnt[2] == 0) no();
