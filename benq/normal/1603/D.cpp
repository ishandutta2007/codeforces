...
void solve(int tc) {
	ints(N,K); ckmin(K,IT-1);
	ps(dp[K][N]);
}

int main() {
	setIO();
	FOR(i,1,MX) for (int j = i; j < MX; j += i) dist[j].pb({i,j/i});
	FOR(i,1,MX) {
		R0F(j,sz(dist[i])) FOR(k,j+1,sz(dist[i])) if (dist[i][k].f%dist[i][j].f == 0) {
	...
	FOR(k,2,IT) {
		...
		// 	V<pair<int,ll>> ncands;
		// 	each(t,cands) {
		// 		ll v = t.s;
		// 		if (t.f < hi) v += ad;
		// 		if (sz(ncands) && ncands.bk.s <= v) continue;
		// 		ncands.pb({t.f,v});
		// 	}
		// 	swap(cands,ncands);
		// };
		LazySeg<ll,1<<18> LS;
		FOR(i,1,MX) {
			LS.set_at(i-1,dp[k-1][i-1]);
			// cands.insert(begin(cands),{i-1,dp[k-1][i-1]});
			// m[k-1] = dp[k-1][i-1];
			each(t,dist[i]) {
				LS.upd(t.f-1,t.s);
				// all candidates < t.f += t.s
			}
			dp[k][i] = LS.seg[1];
			// dbg(k,i,dp[k][i]);
