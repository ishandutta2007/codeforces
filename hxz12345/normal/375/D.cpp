...
vector<pair<int,int>> edge[1001010];
long long suf[1001010],col[1001010],up[1001010],Answer[1001010],opt,H[1010100],dfn[1010100],e[1010100],a[1010100],b[1010100],d[1010100],ans;
...
void delet(int x)
{
	...
	for (int i=dfn[x];i<=dfn[x]+E[x].siz-1;i++)
	       {
	       if (col[E[H[i]].col]!=0)modify(col[E[H[i]].col],-1);
	       col[E[H[i]].col]--;
	       if (col[E[H[i]].col]!=0)modify(col[E[H[i]].col],1);
	       }
}
...
void sc(int x,int fa)
{
	...
	if (col[E[x].col]!=0)modify(col[E[x].col],-1);
	col[E[x].col]++;
	modify(col[E[x].col],1);
	
	for (i=0;i<edge[x].size();i++)
	    {  
	    int y=edge[x][i].first;
	    Answer[edge[x][i].second]=query(y);
		}
}
int main()
{
