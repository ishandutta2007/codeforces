...
signed main(){fastio
    ...
    ll n;cin>>n;
    vl x(n),y(n),c(n);
    rep(i,0,n-1)cin>>x[i]>>y[i]>>c[i];
    vl cmp;
    rep(i,0,n-1){
        cmp.pb(x[i]);
        cmp.pb(y[i]);
    }
    cmp.pb(0);cmp.pb(x[n-1]+1);
    ERASE(cmp);
    assert((int)cmp.size()==2*n+2);
    rep(i,0,n-1){
        x[i]=POSL(cmp,x[i]);
        y[i]=POSL(cmp,y[i]);
    }
   ...
   // rep(i,0,2*n+1)cerr<<flag[i]<<" ";cerr<<endl;
    vi kei(2*n+1);
    kei[2*n]=1;
    rrep(i,2*n,1){
        if(x[flag[i]]==i){
            ...
            kei[i-1]= kei[i] - dis[flag[i]];
        }
    }
   // rep(i,0,2*n)cerr<<kei[i].val()<<" ";cerr<<endl;
    mint ans=0;
    rep(i,0,2*n){
        ans+=(cmp[i+1]-cmp[i])*kei[i];
    }
    cout<<ans.val()<<endl;
