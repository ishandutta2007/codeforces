...
struct Box {
    ...
    static Box from_letter(string s) {
        Box res;
    ...
    vector<string> wrap(bool wrap_hi, bool wrap_lo) const {
        vector<string> res;
        rep(i, box.size()) {
            string l;
            if (i == base) {
                ...
                l += box[base];
                l += "->+";
            } else if ((wrap_hi and i < base) or (wrap_lo and i > base)) {
                l += '|';
                l += box[i];
                l += "  |";
            } else {
    ...
    void append(const vector<string>& s) {
        for (auto& e : s) append(e);
    }

    void append_prefix() {
        int h = height();
        rep(i, h) {
            box[i].insert(0, i == base ? "->" : "  ");
        }
    }
    ...
    Box que() const {
        Box res;
        res.base = 1;
        res.append(upper_empty());
        res.append(upper());
        res.append(middle());
        res.append(wrap(true, false));
        res.append_prefix();
        return res;
    }
    ...
    static Box concat_v(vector<Box> boxes) {
        const int base = boxes[0].base;
        const int num = boxes.size();

        int w = 0;
        for (auto &box : boxes) chmax(w, box.width());
        ...
        rep(i, num) {
            int fr = res.box.size();
            res.append(boxes[i].wrap(i != 0, i != num - 1));
            int bi = fr + boxes[i].base;

            int h = boxes[i].height();
            rep(j, fr, fr + h) {
                res.box[j].insert(res.box[j].end() - 3, w - boxes[i].width(), j == bi ? '-' : ' ');
            }

            if (i + 1 != num) {
                string x(res.box.front().size(), ' ');
                x.front() = x.back() = '|';
                res.append(x);
            }
        }
    ...
    static Box concat_h(Box l, Box r) {
        const int base = max(l.base, r.base);
        if (l.base < base) l.box.insert(l.box.begin(), base - l.base, string(l.width(), ' '));
        if (r.base < base) r.box.insert(r.box.begin(), base - r.base, string(r.width(), ' '));
        int lw = l.width(), rw = r.width();
        int h = max(l.height(), r.height());
        l.box.resize(h, string(lw, ' ')), r.box.resize(h, string(rw, ' '));
        rep(i, h) {
            l.box[i] += r.box[i];
        }
        l.base = base;
...
Box expr(State& it) {
    vector<Box> bs;
    bs.push_back(term(it));
    while (is(it, '|')) {
        consume(it, '|');
        bs.push_back(term(it));
    }
    reverse(all(bs));
    if (bs.size() == 1) return bs.front();
    return Box::concat_v(bs);
}
Box term(State& it) {
    Box r = atom(it, false);
    if (is(it, ')') or is(it, '*') or is(it, '?') or is(it, '+') or in(it, 'A', 'Z')) {
        return Box::concat_h(term(it), r);
    } else {
        return r;
    }
}
Box atom(State& it, bool b = false) {
    if (is(it, ')')) {
        Box res = expr(++it);
        consume(it, '(');
        return res;
    ...
    if (is(it, '*')) {
        return atom(++it, true).ast();
    } else if (is(it, '?')) {
        return atom(++it, true).que();
    } else if (is(it, '+')) {
        return atom(++it, true).pls();
    } else {
        assert_range(it, 'A', 'Z');
        string letter;
        while (in(it, 'A', 'Z')) {
...
int main() {
    ...
    reverse(all(s));
    auto it = s.cbegin();
    auto ans = expr(it).answer().box;
    print(ans.size(), ans[0].size());
    print_all(ans, "\n");
}