...
int main() {
    ...
    while(T--) {
        ...
        for(auto& s : S) cin >> s;
        RollingHash hash;
        vector< vector< uint64_t > > hashed;
        set< uint64_t > st;
        for(auto& s : S) {
            hashed.emplace_back(hash.build(s));
            st.emplace(hashed.back().back());
        }
        for(int i = 0; i < N; i++) {
            bool f = false;
            for(int j = 1; j < (int) S[i].size(); j++) {
                f |= st.count(hash.query(hashed[i], 0, j)) and
                        st.count(hash.query(hashed[i], j, (int) S[i].size()));
            }
            cout << f;
        }
        cout << endl;
