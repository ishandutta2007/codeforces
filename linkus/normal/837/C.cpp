...
int main(){
    ...
    FOR(i,0,n){
        auto &f = arr[i];
        int patarea = f.e1*f.e2;
        FOR(x,0,i){
            auto &s = arr[x];
            int area = patarea+s.e1*s.e2;
            if(area <= best)
                continue;
            /* tmpa -= f.e1+s.e1; tmpb -= f.e2+s.e2; */
            tmpa = a, tmpb = b;
            if(!(f.e2 > tmpb || s.e2 > tmpb)){
                tmpa -= s.e1+f.e1;
                if(tmpa >= 0 && tmpb >= 0){
                    best = max(best,area);
                    continue;
