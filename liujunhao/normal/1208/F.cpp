...
struct node{
	int mx,smx,num;
}tree[5000000+100],*tcnt=tree,*rt;
void insert(int x){
	int i=1;
	int t=1<<20;
	tree[i].num++;
	if(x>tree[i].mx)
		tree[i].smx=tree[i].mx,tree[i].mx=x;
	else if(x>tree[i].smx)
		tree[i].smx=x;
	while(t){
		i<<=1;
		if(x&t)
...
void merge(int x,int y,int len){
	...
	if(tree[x].mx>tree[y].mx){
		tree[x].mx=tree[x].mx;
		tree[x].smx=max(tree[y].mx,tree[x].smx);
	}
	else{
		tree[x].smx=max(tree[x].mx,tree[y].smx);
		tree[x].mx=tree[y].mx;
	}
	if(len<0)
		return;
	merge(x<<1,y<<1,len-1);
	merge((x<<1)|1,(y<<1)|1,len-1);
}
void update(int i,int len){
	...
	update(i<<1,len-1);
	update((i<<1)|1,len-1);
	merge(i<<1,(i<<1)|1,len-1);
}
int get_ans(int i,int len,int pos){
	if(len<0)
		return 0;
	if(a[pos]&(1<<len)){
		return (1<<len)|get_ans(i<<1,len-1,pos);
	}
	else{
		if(tree[(i<<1)|1].num>1&&tree[(i<<1)|1].smx>pos)
			return (1<<len)|get_ans((i<<1)|1,len-1,pos);
		else
			return get_ans(i<<1,len-1,pos);
	}
}
int main()
{
