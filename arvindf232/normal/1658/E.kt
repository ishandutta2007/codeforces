...
fun main(){
    solve.cases{
        ...
        val max = mutableListOf<Triple<Int,Int,Int>>()
        val ox = IntArray(n * n)
        val oy = IntArray(n * n)
        repeat(n){ x->
            repeat(n){y->
                ...
                oy[v] = y
//                table[x][y] = getint
//                max.add(Triple(table[x][y],x,y))
            }
        }
        val ret = Array(n){BooleanArray(n)}
//        max.sortByDescending { it.first }

        var isfirst = true

        var maxsum:Int = plarge
        var minsum:Int  = nlarge
        var maxsubtract:Int = plarge
        var minsubtract:Int = nlarge
        fun update(x:Int, y:Int){
            maxsum = minOf(maxsum, x + y + k)
            minsum = maxOf(minsum, x + y - k)
            maxsubtract = minOf(maxsubtract, x - y + k)
            minsubtract = maxOf(minsubtract, x - y - k )
        }
        fun ok(x:Int, y:Int):Boolean{
            return ((x + y) in (minsum .. maxsum ))  && ((x -y ) in (minsubtract ..maxsubtract))
        }
        ...
        for(a in ret){
            put(a.map{if(it) 'M' else 'G'}.conca())
        }
