...
int main()
{
	...
	for (int i=0;i<n;++i) if (h[i]==man) gn=i;
	if (nn%2)
	{
		bad.push_back(gn);
	}
	for (auto i: bad) bd[i]=1;
	for (int i=0;i<n;++i) if (!bd[i]) good.push_back(i+1);
	for (int i=0;i<bad.size();++i) ++bad[i];
	if (bad.size()==0)
	{
		int cur=good[0];
		for (int i=1;i<good.size();++i)
		{
			cout<<1<<' '<<cur<<' '<<good[i]<<en;
			cur=good[i];
		}
	}
	else if (bad.size()==1)
