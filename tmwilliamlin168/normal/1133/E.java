...
public class Main {
    public static void main(String[] args) {
        ...
        EKSbalansirovannihKomand solver = new EKSbalansirovannihKomand();
        solver.solve(1, in, out);
        out.close();
    }

    static class EKSbalansirovannihKomand {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            for (int p = 0; p < k; p++) {
                ...
                Arrays.fill(next, 0);
                for (int i = 0; i < n; i++) {
                    next[i + 1] = Math.max(cur[i + 1], cur[leftmost[i]] + (i - leftmost[i] + 1));
                }
                for (int i = 1; i <= n; i++) {
