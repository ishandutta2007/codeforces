...
void main_() {
	...
	//dp
	vector dp_data(n, vector(k + 2, vector(k + 2, mint(0))));
	const auto non = k + 1;
	auto CHMAX = [&](int &x, int y) {
		if(x == non and y == non) return;
		if(x != non) {
			if(y != non) return;
			x = max(x, y);
			return;
	...
	auto MIN = [&](int x, int y) {
		if(x == non or y == non) return non;
		return min(x, y);
	};
	auto dp = [&](auto dp, int now) -> void {
		vector ndp(k + 2, vector(k + 2, mint(0)));
		ndp[non][0] = ndp[k][non] = 1;
		for(auto [to, cost] : g[now]) {
			dp(dp, to);
			vector nndp(k + 2, vector(k + 2, mint(0)));
			REP(i, 1, k + 2) {
				REP(j, k + 2) {
					REP(l, 1, k + 2) {
						REP(m, k + 2) {
							...
							int dep = j;
							chmin(dep, m + 1);
							nndp[ni][dep] += ndp[i][j] * dp_data[to][l][m];
						}
					}
				}
			}
	...
	dp(dp, 0);
	print(accumulate(all(dp_data[0][non]), mint(0)));
	debug(dp_data);
}