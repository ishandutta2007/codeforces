...
struct dsu
{
	...
	vi p, r, a;
	vb v, f;
	dsu(const int n) : p(n), r(n, 1), a(n, 0), v(n), ans(0), f(n, true)
	{
		for (int i = 0; i < n; ++i)
			p[i] = i;
	}
	...
	void us(int x, int y, bool q)
	{
		if (x != y)
		{
			bool need_invert = (q != (on(x) != on(y)));
			x = fs(x);
			y = fs(y);
			if (x != y)
			{
