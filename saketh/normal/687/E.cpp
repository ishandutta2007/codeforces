...
int main() {
    ...
    int SCC; vi scc; tie(SCC, scc) = strongly_connected_components(adj);
    vvi elts(SCC);
    F0R (i, N) elts[scc[i]].pb(i);

    trav (cc, elts) {
        bool sink = true;
        trav (v, cc) trav (u, adj[v]) if (scc[u] != scc[v]) sink = false;
        if (!sink) continue;
        ...
        trav (v, cc) {
            vi dist(N, -1);
            queue<int> bfs;
            trav (u, adj[v]) dist[u] = 1, bfs.push(u);
            for (; !bfs.empty(); bfs.pop()) {
                int cloc = bfs.front();
                trav (nbr, adj[cloc]) if (dist[nbr] == -1) {
                    dist[nbr] = dist[cloc] + 1;
                    bfs.push(nbr);
                }
            }
            ckmin(cyc, dist[v]);
