...
void solve() {
  ...
  vc<mint> pow(N + 1);
  pow[0] = 1;
  FOR(n, N) pow[n + 1] = pow[n] * base;

  PersistentArray<mint> PA_H;
  PersistentArray<char> PA_A;
  vc<PersistentArray<mint>::np> H(N + 1);
  vc<PersistentArray<char>::np> A(N + 1);
  vc<int> SZ(N + 1);
  {
    H[N] = PA_H.get_root();
    PA_H.set(H[N], 0, mint(0));
    A[N] = PA_A.get_root();
    SZ[N] = 0;
  }

  auto get = [&](auto root, int l, int r) -> mint {
    return PA_H.get(root, r) - PA_H.get(root, l) * pow[r - l];
  };
  ...
  FOR_R(i, N) {
    ...
    auto A0 = A[i + 1];
    int n0 = SZ[i + 1] + 1;
    A0 = PA_A.set(A0, n0 - 1, S[i]);
    mint now = PA_H.get(H0, n0 - 1);
    H0 = PA_H.set(H0, n0, now * base + mint(S[i]));
    if (i + 1 >= N || S[i] != S[i + 1]) {
      H[i] = H0;
      A[i] = A0;
      SZ[i] = n0;
    ...
    auto check = [&](ll k) -> bool {
      if (k > max(n0, n1)) return false;
      return get(H0, n0 - k, n0) == get(H1, n1 - k, n1);
    };
    int lcp = binary_search(check, 0, n0 + 1);
    if (n0 == lcp || n1 == lcp) {
      if (n1 == lcp) {
        swap(H0, H1);
        swap(A0, A1);
    ...
    char a0 = PA_A.get(A0, n0 - 1 - lcp);
    char a1 = PA_A.get(A1, n1 - 1 - lcp);
    if (a0 > a1) {
      swap(H0, H1);
      swap(A0, A1);
      swap(n0, n1);
  ...
  FOR(i, N) {
    ...
    if (n <= 10) {
      string ANS;
      FOR(k, n) ANS += PA_A.get(A[i], n - 1 - k);
      print(n, ANS);
    } else {
      string ANS(10, '.');
      FOR(k, 5) ANS[k] = PA_A.get(A[i], n - 1 - k);
      ANS[9] = PA_A.get(A[i], 0);
      ANS[8] = PA_A.get(A[i], 1);
      print(n, ANS);
    }
  }
}
