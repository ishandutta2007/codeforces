...
int main() {
    ...
    while(si(v) > 3) {
        ...
        rep(i, si(v)) {
            if(i % 4 < 2) a[0].eb(v[i]);
            if(i % 4 and i % 4 < 3) a[1].eb(v[i]);
        }

        int b[2] = {ask(a[0]), ask(a[1])};

        vi nxt;
        rep(i, si(v)) {
            int t = i % 4;
            if(((t < 2) ^ b[0]) and (inc(t, 1, 3) ^ b[1])) {
            } else
                nxt.eb(v[i]);
        }
    ...
    rep(i, 3) {
        vi g(4);
        rep(j, 4) {
            if(count(all(w[j]), i) ^ res[j]) g[j] = true;
        }
        rep(j, 3) if(g[j] and g[j + 1]) f[i] = false;
        if(f[i]) {
            if(ans(v[i])) exit(0);
        }
    }
