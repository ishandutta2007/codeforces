...
string test(string s) {
	...
	do {
		...
		REP(i, n) {
			REPS(j, i) {
				if(t.substr(0, j) == t.substr(i + 1 - j, j)) chmax(f, j);
			}
		}
		if(now > f) {
			res = t;
...
int main() {
	...
	// 	REP(j, i) {
	// 		REP(k, k) {
	// 			if(i + j + k > 10 or i + j + k == 0) continue;
	// 			string ret = "";
	// 			REP(i)
	// 			ret += 'a';
	// 			REP(j)
