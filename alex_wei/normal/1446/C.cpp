...
int dfs(vector <int> c){
	...
	vector <int> tmp[31];
	int ans=0,num=0,val[31],pd[31];
	for(int it:c){
		int bt; for(int i=0;i<=30;i++)if((1<<i)>it){bt=i; break;}
		tmp[bt].pb(it-(bt==0?0:(1<<bt-1)));
	} for(int i=30;~i;i--){
		val[i]=pd[i]=0;
		if(!tmp[i].size())continue;
		val[i]=dfs(tmp[i]),pd[i]=1;
	} for(int i=1;i<=30;i++)pd[i]|=pd[i-1];
	for(int i=30;i;i--)if(val[i])ans=max(ans,val[i]+(num++)+pd[i-1]);
	return ans;
}
