...
class CTheThreeLittlePigs {
 public:
  void solveOne() {
    int n, q;
    cin >> n >> q;
    vector<mint> invs(3 * n + 5);
    invs[1] = 1;
    for (int i = 2; i < invs.size(); ++i) {
      invs[i] = invs[mint::mod() % i] * (-(mint::mod() / i));
    }
    vector<mint> facts(invs.size());
    vector<mint> invfacts(invs.size());
    ...
    for (int i = 1; i <= 3 * n + 3; ++i) {
      a[i - 1] = facts[3 * n + 3] * invfacts[i] * invfacts[3 * n + 3 - i];
    }
    vector<mint> b(3);
    for (int i = 1; i <= 3; ++i) {
      b[i - 1] = facts[3] * invfacts[i] * invfacts[3 - i];
    }
    mint invb = b[b.size() - 1].inv();
    vector<mint> c(3 * n + 1);
    for (int i = c.size() - 1; i >= 0; --i) {
      mint by = a[i + b.size() - 1] * invb;
      c[i] = by;
      for (int j = 0; j < b.size(); ++j) {
        a[i + j] -= b[j] * by;
      }
      assert(a[i + b.size() - 1] == 0);
    }
