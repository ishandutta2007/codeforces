...
struct Dsu{
	static constexpr int DSUSZ = 524288;
	int dsu[DSUSZ];
...
int ClimbTo( int a , int b , int lca , int step ){
	using LCA_nmsp::dep;
	if( step > dep[a]-dep[lca] ){
		return LCA_nmsp::ClimbUp(b,(dep[b]-dep[lca])-(step-(dep[a]-dep[lca])));
	}else{
		return LCA_nmsp::ClimbUp(a,step);
	}
}

bool Solve( int a , int b ){
	using LCA_nmsp::dep;
	int lca = LCA_nmsp::LCA(a,b);
	if( dep[a]+dep[b]-2*dep[lca] <= 2*k ) return true;
	int aTo = ClimbTo(a,b,lca,k);
	int bTo = ClimbTo(b,a,lca,k);
	// P(aTo,bTo);
	return dsu.Same(aTo,bTo);
}
...
int main(){
	...
	Read(n,k,r);
	ReadGraph();
	For(i,r){ iRead(x); as.Eb(x); }

	LCA_nmsp::Init();
	GetAdjacentRestStops();
