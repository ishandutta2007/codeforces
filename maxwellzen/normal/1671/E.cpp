...
bool equal(int i, int j) {
    if (i>=twon || j>=twon) return true;
    return s[i]==s[j] && equal(2*i, 2*j) && equal(2*i+1, 2*j+1);
}
bool cmp(int i, int j) {
    if (i>=twon || j>=twon) return false;
    if (s[i] != s[j]) return s[i]<s[j];
    if (equal(2*i, 2*j)) return cmp(2*i+1, 2*j+1);
    else return cmp(2*i, 2*j);
}
void swaptree(int i, int j) {
    if (i>=twon || j>=twon) return;
    swap(s[i], s[j]);
    swaptree(2*i, 2*j);
    swaptree(2*i+1, 2*j+1);
}
...
void solve() {
    ...
    twon = 1 << n;
    h = twon / 2;
    ROF(i, 1, h) if (cmp(2*i+1, 2*i)) swaptree(2*i, 2*i+1);
    int cnt = 0;
    ROF(i, 1, h) if (!equal(2*i, 2*i+1)) cnt++;
    ll ans = 1;
    F0R(i, cnt) ans = (2*ans)%MOD;
    cout << ans << nl;
}
