...
void fft(vector<complex<double>>& a, bool inv = false) {
	...
	complex<double> w_n(cos(ang), sin(ang)), w = 1;

	for (int len = 2; len <= n; len <<= 1) {
		double ang = 2 * PI / len * (inv ? 1 : -1);
		complex<double> w_n(cos(ang), sin(ang));
...
int main() {
	...
	vector<ll> cnt(n + 37, 0);
	for (auto& i : ps)
		cnt[i]++;

	const int SHIFT = n + 10;
	vector<ll> mcnt(n + 37, 0);
	for (auto& i : ps)
		mcnt[SHIFT - i]++;

	vector<ll> mult = multiply(cnt, mcnt);
	vector<ll> ans(n + 1, 0);
	for (int i = 1; i <= n; i++)
		ans[i] = (i + SHIFT < (int)mult.size() ? mult[i + SHIFT] : 0);
