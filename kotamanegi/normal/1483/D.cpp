...
void solve(){
	...
	REP(trial, 3) {
		...
		REP(i, n) {
			REP(q, n) {
				REP(j, n) {
					dist2[i][q] = min(dist2[i][q], dist2[i][j] + dist2[j][q]);
				}
			}
		}
	}
	int query;
	cin >> query;
	priority_queue<tuple<int, int, int>,vector<tuple<int,int,int>>> nexts;
	REP(i, query) {
		int a, b, c;
		cin >> a >> b >> c;
		a--; b--;
		dist[a][b] = c;
		nexts.push(make_tuple(c, a, b));
	}
	while (nexts.empty() == false) {
		tuple<int, int, int> now = nexts.top();
		nexts.pop();
		if (dist[get<1>(now)][get<2>(now)] != get<0>(now)) continue;
		for (int q = 0; q < n;++q) {
			{
				int smaller = min(q, get<1>(now));
				int larger = max(q, get<1>(now));
				if (dist[smaller][larger] < get<0>(now) - dist2[get<2>(now)][q]) {
					dist[smaller][larger] = get<0>(now) - dist2[get<2>(now)][q];
					nexts.push(make_tuple(dist[smaller][larger], smaller, larger));
				}
			}
			{
				int smaller = min(q, get<2>(now));
				int larger = max(q, get<2>(now));
				if (dist[smaller][larger] < get<0>(now) - dist2[get<1>(now)][q]) {
					dist[smaller][larger] = get<0>(now) - dist2[get<1>(now)][q];
					nexts.push(make_tuple(dist[smaller][larger], smaller, larger));
				}
			}
		}
	...
	REP(i, edges.size()) {
		...
		int smaller = min(get<0>(edges[i]), get<1>(edges[i]));
		int larger = max(get<0>(edges[i]), get<1>(edges[i]));
		if (dist[smaller][larger] >= get<2>(edges[i])) {
			ans++;
		}
	}
	cout << ans << endl;
