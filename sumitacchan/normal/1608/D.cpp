...
signed main(){
    ...
    REP(i, n){
        ...
        else if(s[i] == "B?" || s[i] == "?W") y01++;
        else if(s[i] == "W?" || s[i] == "?B") y10++;
    }
    ...
    REP(k, y + 1){
        mint c1 = C.nCr(y, k);
        int n1 = y01 + k, n2 = y10 + (y - k);
        int d = x10 - x01;

        mint c2;
        if(d >= 0) c2 = C.nCr(n1 + n2, d + n2);
        else c2 = C.nCr(n1 + n2, n1 + llabs(d));
        if(x01 == 0 && x10 == 0) c2 -= 1;
    ...
    for(char t: {'B', 'W'}){
        bool f = true;
        REP(i, n) if(s[i][0] == t || s[i][1] == t) f = false;
        if(f) ans += 1;
    }
    Out(ans);
