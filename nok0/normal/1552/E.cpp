...
void main_() {
	...
	V<> used(n);
	int l = 0;
	SegmentTreeBeats<int> seg(n * k);
	REP(i, n * k) {
		if(pl[c[i]] != -1) {
			res[c[i]] = pair(pl[c[i]] + 1, i + 1);
			used[c[i]] = 1;
			seg.update_add(pl[c[i]], i + 1, 1);
			pl[c[i]] = -1;
			if(l < n * k and seg.query_max(l, i + 1) == M) {
				while(seg.query_max(l, i + 1) == M) {
					if(pl[c[l]] == l) pl[c[l]] = -1;
					l++;
				}
			}
			continue;
