...
fn is_connected(g: &Graph) -> bool {
    ...
    let all_bits : Vec<_> = g.bits.iter()
        .enumerate()
        .filter(|&(i, v)| *v > 0)
        .map(|x| x.0)
        .map(|x| dsu.find(x)).collect();
    all_bits.iter().all(|x| *x == all_bits[0])
}
...
fn add_edge(v: u32, graph: &mut Graph, add: i32) {
    for b1 in 0..30 {
        if ((v >> b1) & 1) == 0 {
            continue;
        }
...
fn solve(input: &mut Input, _test_case: usize) {
    ...
    a.iter_mut().for_each(|x| *x = input.read());
    let mut ans = 0;
    for x in &mut a {
        if *x == 0 {
            *x += 1;
    ...
    let mut graph = Graph {
        edges: vec![vec![0; 30]; 30],
        bits: vec![0; 30],
    };
    for x in &a {
        add_edge(*x, &mut graph, 1);
    }
    ...
    if !is_connected(&graph) {
        let pos = find_max_trailing_pos(&a);
        ans += 1;
        add_edge(a[pos], &mut graph, -1);
        a[pos] -= 1;
        add_edge(a[pos], &mut graph, 1);
        let pos2 = find_max_trailing_pos(&a);
        add_edge(a[pos2], &mut graph, -1);
        a[pos2] += 1;
        add_edge(a[pos2], &mut graph, 1);
        ans += 1;
    }
...
fn find_max_trailing_pos(a: &Vec<u32>) -> usize {
    a.iter()
