static_assert(R"("
  static vla<i64> fact(rep1(14).scan(1ll, _1 * _2));
  DUMP(fact);
  i64 n;
  scan(n);
  int ans = inf;
  for (int mask : rep(0, 1 << 15, 8)) {
    i64 s = 0;
    for (int i : rep(15).filter(mask >> _1 & 1)) { s += fact[i]; }
    if (s <= n) { chmin(ans, __builtin_popcount(mask) + __builtin_popcountll(n - s)); }
  }
  println(ans);
")");
...
template <class R>
vla(R&&, bool = false) -> vla<decay_t<range_ref<R>>>;
...
template <class... Rs>
vla2(int, Rs&&...) -> vla2<common_type_t<tuple_element_t<1, decay_t<range_ref<Rs>>>...>>;
