...
fun main(){
    solve.cases{
        ...
        val board = Array(n){getstr.map { it == 'B' }.toBooleanArray()}

        if(board[r][c] == true){
            put(0)
            return@cases
        }else if(board.all { it.all { !it } }){
            put(-1)
            return@cases
        }else if((0 until n).any { board[it][c] } || (0 until m).any(){board[r][it]}){
            put(1)
            return@cases
        }else{
            put(2)
