...
double calc(double x) {
    ...
    for (int i = 1; i <= n; ++i) {
        double p = pr[i];
        double res = max(abs(p - minpr), abs(p - maxpr));
        maxs = max(maxs, p - minpr);
        mins = min(mins, p - maxpr);
        if (res > ans) {
            ans = res;
        }
        if (p > maxpr) {
...
//    double maxans = INF;
//    double maxx = -INF;
//    for (double x = -2000; x <= 2000; x += 1e-3) {
//        double t = calc(x);
//        if (t < maxans) {
//            maxans = t;
//            maxx = x;
