...
int main() {
    ...
    vi uniq = a;
    trav (e, mod) --e.f, uniq.pb(e.s);
    dedup(uniq);

    int U = sz(uniq);
    vb live(U);
    trav (e, a) live[e = index(uniq, e)] = 1;
    trav (e, mod) e.s = index(uniq, e.s);

    struct node { int lo = INT_MAX/2, ct = 0; };
    auto nn = [](node a, node b) {
        if (a.lo != b.lo) return a.lo < b.lo ? a : b;
        return node{a.lo, a.ct + b.ct};
    };
    using upd = int;
    auto uu = plus<int>();
    auto un = [](upd u, node n) { return node{n.lo+u, n.ct}; };

    segment_tree_lazy st(U, node{}, 0, nn, uu, un);
    st.set_leaves([&](int i) { return node{0, live[i]}; });

    auto set_ct = [&](int i, int ct) {
        node cur = st[i];
        cur.ct = ct;
        st.replace(i, cur);
    };
    auto mark = [&](int i, bool neg) {
        if (i < 0 || i + 1 == sz(a)) return;
        if (a[i] < a[i+1]) {
            st(a[i]+1, a[i+1] + 1, neg ? -1 : 1);
        }
    };
    F0R (i, sz(a) - 1) mark(i, 0);

    for (auto [inx, val] : mod) {
        mark(inx - 1, 1);
        mark(inx, 1);
        set_ct(a[inx], 0);
        a[inx] = val;
