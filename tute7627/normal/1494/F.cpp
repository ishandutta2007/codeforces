...
//()
template<typename T>
vector<edge<T>> euler_path(int s,Graph<T>&g, int edge_num){
  vector<bool>used(edge_num);
  vector<int>idx(g.size());
  vector<edge<T>>ret;
  stack<edge<T>>st;
  int now=s;
  while(1){
    while(idx[now]<g[now].size()&&used[g[now][idx[now]].id])idx[now]++;
    if(idx[now]==g[now].size()){
      ...
      st.push(g[now][idx[now]]);
      used[g[now][idx[now]].id]=true;
      now=g[now][idx[now]].to;
    }
  }
...
int main(){
  ...
  rep(x,0,m+n){
    ...
    for(auto i:{a[x],b[x]}){
      ...
      ll cnt=0;
      rep(o,0,m){
        if(o!=x&&i==a[o]&&d[b[o]]%2==1){
          tmp.PB(b[o]);
          continue;
        }
        if(o!=x&&i==b[o]&&d[a[o]]%2==1){
          tmp.PB(a[o]);
          continue;
        }
        gn[a[o]].EB(b[o],1,o);
      ...
      vector<int>odd;
      rep(u,0,n)if(gn[u].size()&1)odd.PB(u);
      //debug(odd,odd.size());
      if(odd.size()>2||odd.size()==1)continue;
      if(odd.size()==2&&odd[0]!=i&&odd[1]!=i)continue;
      auto eu=euler_path(i,gn,m);
      //cout<<gn<<endl;
      //cout<<x spa i spa cnt spa eu.size()<<endl;
      if(eu.size()!=cnt)continue;
      reverse(ALL(eu));
      vector<int>ret;
      for(auto e:eu)ret.PB(e.to+1);
      ret.PB(i+1);
      ret.PB(-1);
      for(auto z:tmp){
        ret.PB(z+1);
