...
bool check(int v){
	int cnt=0,tmp=0,kk=k>>1;
	vector <int> buc;
	for(int i=1;i<=n;i++){
		if(a[i]<=v)tmp++;
		else buc.pb(tmp),tmp=0;
	} if(tmp)buc.pb(tmp);
	if((k&1)&&buc.size()){
		if(a[1]<=v&&a[n]<=v&&buc.front()%2&&buc.back()%2)kk++;
		else{
			if(a[1]<=v)buc[0]--;
			if(a[n]<=v)buc[buc.size()-1]--;
		}
	}
	if((k&1)==0&&buc.size()){
		if(a[1]<=v&&a[n]<=v){
			if(buc.front()%2==0)buc[0]--;
			else buc[buc.size()-1]--;
		}
	}
	for(int it:buc)cnt+=it+1>>1;
	return cnt>=kk;
}
