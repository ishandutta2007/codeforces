...
public:
  ...
  int>::type read(I& arg) { return readUnsignedInt(arg, skipCharacters()) > 0 ? 1 : 0; }
  ...
  int>::type read(char* arg, I limit, const char* terminators = "") {
    if (!*terminators) return readString(arg, static_cast<int>(limit), InputDevice::isSpace);
    else return readString(arg, static_cast<int>(limit),
  ...
  template <class... Ts, class I> inline typename enable_if<is_integral<I>::value,
  int>::type read(char* arg, I limit, bool (*isTerminator)(char), Ts&&... args) {
    int argsRead = readString(arg, static_cast<int>(limit), isTerminator);
    return argsRead + read(forward<Ts>(args)...);
  }
  template <class... Ts, class I> inline typename enable_if<is_integral<I>::value,
  int>::type read(char* arg, I limit, const char* terminators, Ts&&... args) {
    int argsRead = read(arg, static_cast<int>(limit), terminators);
    return argsRead + read(forward<Ts>(args)...);
  }
  ...
  inline int read(complex<T>& arg) {
    T real, imag;
    int res = read(real) && read(imag);
    if (res > 0) arg.real(real), arg.imag(imag);
    return res;
  }
  template <class T>
  ...
  // generic forwarding
  template <class T>
  inline auto read(T& arg) -> decltype(arg.read()) { return arg.read(); }
...
public:
  ...
  inline int writeString(const char* arg, size_t length, bool checkWidth = true) {
    ...
    int result = static_cast<int>(count) + (checkWidth && count < width ? writeFill(width - count) : 0);
    if (__builtin_expect(output + count + MAX_LENGTH < end, true)) {
  ...
  // generic forwarding
  template <class T>
  inline auto write(const T& arg) -> decltype(arg.write()) { return arg.write(); }
