/*-------------------( alex256 )--------------------\
|    Contest: Codeforces Round 388 (Div. 2)         |
|       Task: D                                     |
|       Date: 19. 12. 2016                          |
\--------------------------------------------------*/
// Includes
...
vector<int> maxPoses(n+1, -1);
set< pair<int, int>, greater< pair<int, int> > > poses;
vector< vector<int> > numPoses(n+1);
...
main()
{
    ...
    for (int i = 0; i < sz(v); i++) {
        maxPoses[v[i].F] = max(maxPoses[v[i].F], i);
        numPoses[v[i].F].pb(i);
    }
    for (int i = 0; i <= n; i++) {
        poses.insert({maxPoses[i], i});
        sort(numPoses[i].begin(), numPoses[i].end());
    }
    int q = in;
    for (int i = 0; i < q; i++) {
        vector <int> a;
        ...
        for (int i = 0; i < sz(a); i++)
            poses.erase({maxPoses[a[i]], a[i]});
        int posTop = poses.begin()->S;
        poses.erase(poses.begin());
        int curTopPos = (poses.empty()) ? -1 : poses.begin()->F;
        int res = *lower_bound(numPoses[posTop].begin(), numPoses[posTop].end(), curTopPos);
        out.putLn(v[res].F, ' ', v[res].S);
        for (int i = 0; i < sz(a); i++)
            poses.insert({maxPoses[a[i]], a[i]});
        poses.insert({maxPoses[posTop], posTop});
    }
    return 0;
}