...
void solve() {
    ...
    const int A = 20;
    // vector<vector<llong>> inv(A, vector<llong>(A));
    auto inv = make_vector<llong>(A, A);
    vector<int> cnt(A);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < A; j++) {
            inv[a[i]][j] += cnt[j];
    ...
    // vector<llong> d(1 << A, 1e18);
    auto d = make_vector<llong, llong(1e18)>(1 << A);
    d[0] = 0;
    for (int ms = 0; ms < (1 << A) - 1; ms++) {
        for (int i = 0; i < A; i++) {
            if ((ms >> i) & 1) {
                continue;
            }
