...
public class F2 {
	...
	boolean ok(int h, int[] a, long[] b, int L)
	{
		int n = a.length;
		int[] imos = new int[n+1];
		for(int i = 0;i < n;i++){
			for(int k = -1;k <= 1;k++){
				int u = lowerBound(b, (long)a[i] + k * L + h+1 + (h*2 == L ? -1 : 0));
				int d = lowerBound(b, (long)a[i] + k * L - h);
				u -= i;
				d -= i;
	...
	public static int doMaximumMatchingBetweenIntervalAndPoint(long[][] rs, long[] a)
	{
		int n = rs.length;
		...
		int mat = 0;
		for(long v : a){
			while(p < n && rs[p][0] <= v)pq.add(rs[p++]);
			while(!pq.isEmpty()){
				long[] cur = pq.poll();
				if(cur[0] <= v && v <= cur[1]){
					mat++;
