...
// URL : http://codeforces.com/contest/1433/problem/G
// Memory Limit : 256 MB
// Time Limit : 1000 ms
		...
		while(q[j].size()){
			...
			for(auto p:graf[tr.s]){
				if(d[tr.f][p.f]>i+p.s){
					...
					if(k>=N)
						k-=N;
					d[tr.f][p.f]=i+p.s,q[k].pb({tr.f,p.f});
				}
			}
		}
	}
...
int main()
{
	...
	for(auto p:ed){
		int nov=0;
		for(auto t:r)
			nov+=min({d[t.f][t.s],d[t.f][p.f]+d[p.s][t.s],d[t.f][p.s]+d[p.f][t.s]});
		cost=min(cost,nov);
	}
	printf("%i\n",cost);
	return 0;
