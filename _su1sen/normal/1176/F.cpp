...
int main() {
    ...
    vector<long long> pd(10, numeric_limits<long long>::min());
    pd[0] = 0;
    ...
    loop(n) {
        ...
        rep(c, 1, 4) {
            sort(all(cards[c]), greater<long long>());
            if (c == 1) {
                if (cards[c].size() > 3) cards[c].resize(3);
            } else {
                if (cards[c].size() > 1) cards[c].resize(1);
            }
        }
        vector<long long> dp(10, numeric_limits<long long>::min());
        rep(m, 10) {
            chmax(dp[m], pd[m]);
            rep(c, 1, 4) {
                if (cards[c].empty()) continue;
                if (m == 9) {
                    chmax(dp[0], pd[m] + 2 * cards[c][0]);
                } else {
                    chmax(dp[m + 1], pd[m] + cards[c][0]);
                }
            }
            if (cards[1].size() >= 2) {
                long long x = cards[1][0], y = cards[1][1];
                if (m >= 8) {
                    chmax(dp[(m + 2) % 10], pd[m] + x * 2 + y);
                } else {
                    chmax(dp[m + 2], pd[m] + x + y);
                }
            }
            if (cards[1].size() >= 1 and cards[2].size() >= 1) {
                long long x = cards[1][0], y = cards[2][0];
                if (x < y) swap(x, y);
                if (m >= 8) {
                    chmax(dp[(m + 2) % 10], pd[m] + x * 2 + y);
                } else {
            ...
            if (cards[1].size() == 3) {
                long long x = cards[1][0], y = cards[1][1], z = cards[1][2];
                if (m >= 7) {
                    chmax(dp[(m + 3) % 10], pd[m] + x * 2 + y + z);
                } else {
                    chmax(dp[m + 3], pd[m] + x + y + z);
                }
            }
        }
        pd.swap(dp);
    }
    print(*max_element(all(pd)));
    return 0;
}