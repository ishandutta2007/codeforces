...
struct BITree {
	...
	void add(int pos, const long long &x) {
		while (pos <= N) {
			tree[pos] += x;
			pos += lsb(pos);
		}
...
int main () {
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0; j < k + 1; j++) {
			queries.emplace_back(makep(temp[j] + 1, temp[j + 1] - 1), i);
		}
	}

	sort(a.begin(), a.end(), [](const auto & i, const auto & j) -> bool {
		return makep(i.y, i.x) < makep(j.y, j.x);
	});

	sort(queries.begin(), queries.end(), [](const auto & i, const auto & j) -> bool {
		return makep(i.x.y, i.x.x) < makep(j.x.y, j.x.x);
	});
	...
	while (pq < sz) {
		while (pa < n && a[pa].y <= queries[pq].x.y) {
			medis.add(a[pa].x, +1);
			pa++;
		}

		ats[queries[pq].y] -= medis.get(queries[pq].x.x, queries[pq].x.y);
		pq++;
	}
