...
int main() {
    ...
    for(int j=0;j<29;++j){
        ...
        for(int l=0;l<N;++l){
            while(ok_l){
                if(v[l].first+v[ok_l-1].first>=(1<<(j+1))){
                    ok_l--;
                }else{
                    break;
                }
            }
            int plus=max(0,N-ok_l);
            plus=min(plus,N-l-1);
            kuris[j+1]+=plus;
        }
        ...
        for(auto x:v){
            if(x.second&(1<<(j+1))){
                pair<int,int>ax=make_pair(x.first|(1<<(j+1)),x.second);
                rv.push_back(ax);
            }else{
                lv.push_back(x);
            }
    ...
    int plus=0;
    for(int j=0;j<30;++j){
        int asum=cnts[j]*(N-1)+kuris[j];
        answer|=(asum&1)<<j;
        plus=cnts[j]*(cnts[j]-1)/2;
       // WHATS(plus)
       // WHATS(asum)
    }
    //WHATS(check(xxx))
