...
int main() {
  ...
  rep(i, n) a[i] = IN.operator ll();
  bitset<32> res;
  VLL b(n);
  int carry = 0;
  rep(t, 32) {
    res[t] = res[t] ^ (carry & 1);
    rep(i, n) res[t] = res[t] ^ (a[i][t] & (n-1) & 1);
    rep(i, n) b[i] = (a[i].to_ullong() & ((1LL<<t+1)-1));
    sort(all(b));
    carry = 0;
    rep(i, n) carry ^= b.begin()+i - lower_bound(b.begin(), b.begin()+i, (1LL<<t+1) - b[i]);
  }
  OUT(res.to_ullong());
}