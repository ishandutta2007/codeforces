...
int main(){
  ...
  ll sz=17;
  string s;cin>>s;
  auto ret=vec(sz+1,1<<sz,modint(0));
  ll q_init=0;
  for(auto c:s)if(c=='?')q_init++;
  rep(i,0,n){
    rep(j,0,2){
      ll l=i,r=i+j;
      ll qnum=q_init;
  ...
  while(q--){
    string s;cin>>s;
    ll bit=0;
    rep(i,0,s.size())bit|=1<<(s[i]-'a');
    cout<<ret[popcount(bit)][bit]<<endl;
  }
  return 0;
}