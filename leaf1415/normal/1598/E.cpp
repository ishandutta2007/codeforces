...
int main(void)
{
	...
	cin >> h >> w >> Q;
	rep(i, 1, h) S[get(0, i)].insert(i), S[get(0, i)-1].insert(i);
	rep(i, 1, h) S[get(w+1, i)].insert(i+w+1), S[get(w+1, i)-1].insert(i+w+1);

	rep(i, 1, w) S[get(i, 0)].insert(i), S[get(i, 0)-1].insert(i);
	rep(i, 1, w) S[get(i, h+1)].insert(i+h+1), S[get(i, h+1)-1].insert(i+h+1);

	ll ans = 0;
	rep(x, 1, w) rep(y, 1, h){
		auto it = S[get(x, y)].upper_bound(x+y);
		ll t = *it;
		ans += t-(x+y);

		it = S[get(x, y)-1].upper_bound(x+y);
		t = *it;
		ans += t-(x+y);
	}
	...
	rep(q, 1, Q){
		cin >> y >> x;
		if(T.count(P(x, y)) == 0) T.insert(P(x, y));
		else T.erase(P(x, y));

		//outl(x, y);

		ans += calc(get(x, y), x+y);
		ans += calc(get(x, y)-1, x+y);
		outl(ans - (h*w-sz(T)));
	}
