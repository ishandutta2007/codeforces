...
void q() {
  ...
  SegmentTree sego(sz(xs)+2,[](int a,int b){return a+b;},0);
  SegmentTree sege(sz(xs)+2,[](int a,int b){return a+b;},0);

  each(x,a) {
    int i=lb(xs,x);
    int s;
    if(x%2==0){
      s=sege.query(i+1,sz(xs));
    }else{
      s=sego.query(i+1,sz(xs));
    }
    if(s!=0)die("No");
    if(x%2==0){
      sege.add(i,x);
