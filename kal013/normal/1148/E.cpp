...
void solve(){
    ...
    sort(s,s+n);
    sort(t,t+n);
    if (s[0].F>t[0].F || s[n-1].F<t[n-1].F){
        d1("NO");
        return;
    }
    while (res.size()>0 && iter.size()>0){
        auto it=res.begin();
        // trace(res,iter);
        int x=(*it).F;
        auto it1=iter.begin();
        ...
        if (x<X.F){
            d1("NO");
            // trace(res.size(),iter.size(),res,iter);
            return;
        }
        res.erase(it);
        iter.erase(it1);
        if (x==X.F) continue;
        auto it2=iter.lower_bound({x,0});
        if (it2==iter.end()){
            d1("NO");
            // trace(1,res.size(),iter.size(),res,iter);
            return;
        }
        pii Y=*it2;
        iter.erase(it2);
