...
int main() {
  ...
  for(int i=0;i<t;i++){
    ...
    while(abs(ok-ng)>1){
      ll ti=(ok+ng)/2;
     // cout<<ok<<" "<<ng<<" "<<ti<<" "<<s(ti)<<" "<<sum(l,ti)<<endl;
      if(s(ti)<=t&&sum(l,ti)<=t*m){
        ok=ti;
      }
      else{
        ng=ti;
    ...
    for(int p=max(l,ok-3);p<=ok+3;p++){
    //  cout<<p<<" "<<s(p)<<" "<<sum(l,p)<<endl;
      if(s(p)>t||sum(l,p)>t*m){
        cout<<p-1<<endl;
        break;
      }
    }
