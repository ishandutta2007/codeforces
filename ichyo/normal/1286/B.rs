...
fn dfs(r: usize, children: &[Vec<usize>], c: &[i32]) -> Option<Vec<(i32, usize)>> {
    let mut cs = Vec::new();
    for &ch in &children[r] {
        if let Some(s) = dfs(ch, children, c) {
            cs.push(s.into_iter().collect::<VecDeque<_>>());
        } else {
            return None;
        }
    }
    ...
    let mut ok_idx: HashSet<usize> = (0..cs.len()).collect();
    let mut id = 1;
    if id == c[r] + 1 {
        result.push((id, r));
        id += 1;
    }
    while !ok_idx.is_empty() {
        let mut to_remove = Vec::new();
        for &idx in &ok_idx {
            if let Some((_, v)) = cs[idx].pop_front() {
                result.push((id, v));
                id += 1;
                if id == c[r] + 1 {
                    result.push((id, r));
...
fn run() {
    ...
    for i in 0..n {
        if p[i] == -1 {
            ...
            children[p[i] as usize].push(i);
        }
    }

    if let Some(r) = dfs(root, &children, &c) {
        println!("YES");
        let mut ans = vec![0; n];
        for (id, v) in r {
            ans[v] = id;
