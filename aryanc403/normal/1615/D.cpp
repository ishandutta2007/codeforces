...
cin>>T;while(T--)
{
    ...
    for(int i=0;i<n;++i)
        components[i].pb(i);
    bool possible=true;
    const auto addEdge=[&](lli u,lli v,lli w)->void{
        if(d.size(u)>d.size(v))
            swap(u,v);
        const lli lu=d.leader(u),lv=d.leader(v);
        if(par[u]^par[v]^w){
            if(lu==lv){
                possible=false;
                return;
            }
            for(auto x:components[lu])
                par[x]^=1;
        }
        if(d.same(u,v))
            return;
