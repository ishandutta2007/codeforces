...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        decreaseByOne(p);
        arr<bool> has(n, false);
        for (int i : p) {
            if (i != -1) {
                has[i] = true;
            }
        }
        arr3d<int> ans(2, n + 1, n + 1, n);
        ans(0, 0, 0) = ans(1, 0, 0) = 0;
        for (int i : range(n)) {
            for (int j : range(2)) {
                for (int k : range(i + 1)) {
                    for (int l : range(p[i] == -1 ? 0 : p[i] % 2, p[i] == -1 ? 2 : p[i] % 2 + 1)) {
                        minim(ans(l, k + 1 - l, i - k + l), ans(j, k, i - k) + (j == l ? 0 : 1));
                    }
                }
            }
        }
        out.printLine(min(ans(0, (n + 1) / 2, n / 2), ans(1, (n + 1) / 2, n / 2)));
    }
};
