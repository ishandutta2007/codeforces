...
template<typename T>
vector<ll>KMP(const T &v){
  vector<ll>ret(v.size()+1);
  ret[0]=-1;
  int j=-1;
  for(int i=0;i<v.size();i++){
    while(j>=0&&v[i]!=v[j])j=ret[j];
    j++;
    ret[i+1]=j;
  }
  return ret;
...
int main(){
  ...
  string s;cin>>s;
  vector<ll>v(10000);
  rep(i,1,10000)v[i]=v[i/10]+1;
  ll n=s.size();
  vector<ll>dp(n+1,INF);
  dp[0]=0;
  rep(i,0,n){
    auto kv=KMP(s.substr(i));
    rep(j,i+1,n+1){
      ll len=j-i;
      ll m=kv[len];
      if(len%(len-m)==0){
        chmin(dp[j],dp[i]+len-m+v[len/(len-m)]);
      }
      else{
        chmin(dp[j],dp[i]+len+1);
      }
    }
  }
  cout<<dp[n]<<endl;
