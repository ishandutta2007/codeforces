...
int main() {
    INT(n);
    VV(int,a,n,n);
    vvi dp1(n,vi(n,1e9)),dp2(n,vi(n,1e9));
    vvi dir1(n,vi(n)),dir2(n,vi(n));
    ...
    rep(i,n) {
        rep(j,n) {
            ...
            if(i > 0) {
                if(chmin(dp1[i][j],dp1[i-1][j])) dir1[i][j] = 0;
                if(chmin(dp2[i][j],dp2[i-1][j])) dir2[i][j] = 0;
            }
            if(j > 0) {
                if(chmin(dp1[i][j],dp1[i][j-1])) {
                    dir1[i][j] = 1;
                }
                if(chmin(dp2[i][j],dp2[i][j-1])) {
                    dir2[i][j] = 1;
                }
            }
            dp1[i][j] += c1;
