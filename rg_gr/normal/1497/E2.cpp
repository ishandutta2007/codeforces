...
int a[200005],dp[200005][22],f[200005][22],t[10000001];
int ans=0;
void remove(int x)
{
	// debug('-',x,'\n');
...
signed main()
{
	...
	for(int K=0;K<=k;K++)
	{
		ans=0;
		rg(n)
		// rg(6)odb(t[i]);gr puts("");
		for(int d=f[i-1][K]+1;d<=i;d++)add(a[d]);
		int t=max(i,f[i-1][K]);
		f[i][K]=t;
		t++;
		for(;t<=n;t++)
	...
	//dp
	rg(n+1)for(int K=0;K<=k;K++)dp[i][K]=0x3f3f3f3f;gr
	dp[0][0]=0;
	rg(n)for(int K=0;K<=k;K++)
	{
		if(dp[i-1][K]==0x3f3f3f3f)continue;
		for(int j=0;j+K<=k;j++)
			dp[f[i][j]][j+K]=min(dp[f[i][j]][j+K],dp[i-1][K]+1);
	}gr
	int ans=0x3f3f3f3f;
