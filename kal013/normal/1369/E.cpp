...
void solve(){
	...
	while(!comp.empty()){
		...
		for(auto e:adj[i]){
			if (!vis[e.first]){
				comp.erase({W[e.first] - ideg[e.first],e.first});
				--ideg[e.first];

				comp.insert({W[e.first] - ideg[e.first],e.first});

				ordr.push_back(e.second);
			}
		}
		vis[i] = true;
	}
