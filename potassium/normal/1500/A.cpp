...
int main() {
	...
	for (auto& i : m) {
		if (i.se.size() >= 4) {
			vector<int> ans;
			for (int j = 0; j < 4; j++)
				ans.emplace_back(i.se[j]);
 
			YES(ans);
		}
 
		if (i.se.size() >= 2)
			dup.emplace_back(i.fi);
	}
	...
	vector<pii> s(10000000, {-1, -1});
	for (int i = 0; i < n; i++)
		for (int j = i + 1; j < n; j++) {
			int d = a[j].fi - a[i].fi;
			if (s[d].fi != -1) { 
				vector<int> ans = { a[i].se, s[d].se, a[j].se, s[d].fi };
