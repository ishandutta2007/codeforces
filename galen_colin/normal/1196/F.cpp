...
int main() {
  ...
  ll cnt = 0;
  f0r(i, min(m, k)) {
    ll a = edges[i].s.f, b = edges[i].s.s;
    if (!mark[a]) {
      mark[a] = ++cnt;
    }
    edges[i].s.f = mark[a] - 1;
    if (!mark[b]) {
      mark[b] = ++cnt;
    }
    edges[i].s.s = mark[b] - 1;
  }
  n = cnt;
  d.init(n);
  f0r(i, min(m, k)) {
    d.edge(edges[i].s.f, edges[i].s.s, edges[i].f);
  }
  f0r(i, n) {
    d.run(i);
    for (ll i: d.dists) wts.pb(i);
  }
  sort(all(wts));
  cout << wts[n + 2 * k - 1] << endl;
}