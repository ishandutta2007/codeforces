...
struct ss {
    mn ct = 1, sy = 0, syy = 0;
    ss friend operator+(ss a, ss b) {
        return ss{
            a.ct * b.ct,
            a.sy * b.ct + b.sy * a.ct,
            a.syy * b.ct + 2 * a.sy * b.sy + b.syy * a.ct
        };
    }
};
...
int main() {
    ...
    const int MAXV = 1e6;
    const array<int, 6> p10 = {1, 10, 100, 1'000, 10'000, 100'000};
    vector<array<int, 6>> digs(MAXV);
    F0R (v, MAXV) {
        int t = v;
        F0R (i, 6) { digs[v][i] = t % 10; t /= 10; }
    }

    int N; re(N);
    vector<ss> res(MAXV);
    F0R (i, N) { int v; re(v); res[v] = res[v] + ss{2, v, mn(v)*v}; };
    F0R (d, 6) F0Rd (v, MAXV) if (digs[v][d] < 9) res[v] = res[v] + res[v + p10[d]];

    F0R (v, MAXV) {
        FOR (m, 1, 1 << 6) {
            int vv = v;
            F0R (i, 6) if (m & (1 << i)) {
                if (digs[v][i] == 9) { vv = -1; break; }
                vv += p10[i];
            }
            if (vv != -1) {
                if (__builtin_popcount(m)&1) res[v].syy -= res[vv].syy;
                else res[v].syy += res[vv].syy;
            }
        }
    }

    ll ans = 0;
    F0R (v, MAXV) ans ^= v * ll(res[v].syy.v);
    ps(ans);
