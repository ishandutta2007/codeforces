...
#else
        unsigned long long x =
            (unsigned long long)(((unsigned int64_t)(z)*im) >> 64);
#endif
...
using is_unsigned_int128 =
    typename std::conditional<std::is_same<T, uint64_t>::value ||
                                  std::is_same<T, unsigned int64_t>::value,
                              std::true_type,
                              std::false_type>::type;
...
vector<ll> solve(int l, int r){
    if (l + 1 == r) return vector<ll>(a[l] + 1, 1);

    int m = l + r >> 1;
    return atcoder::convolution(solve(l, m), solve(m, r));
}
