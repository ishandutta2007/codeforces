...
void rec(int i, int j) {
	...
	if(k - i <= j - (k+1)) {
		for(int l = k; l >= i; -- l) {
			int s = (asum[l] + ak) % K;
			queries[j+1].push_back(mp(s, +1));
			queries[k+1].push_back(mp(s, -1));
		}
	}else {
		for(int r = k + 1; r <= j; ++ r) {
			int s = (asum[r] + (K - ak)) % K;
			queries[k+1].push_back(mp(s, +1));
			queries[i].push_back(mp(s, -1));
		}
	}
	-- ans;
}
...
int main() {
	...
	while(~scanf("%d%d", &n, &K)) {
		...
		rmq.build(&a[0], n);
		ans = 0;
		queries.assign(n+2, vpii());
		rec(0, n);
		vector<int> cnt(K, 0);
		rer(i, 0, n+1) {
			each(j, queries[i])
				ans += cnt[j->first] * j->second;
			if(i <= n)
				++ cnt[asum[i] % K];
		}
		cout << ans << endl;
