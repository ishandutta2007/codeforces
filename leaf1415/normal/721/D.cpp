...
void calc()
{
	...
	rep(i, 1, n) Q.push(P(abs(a[i]), i));
	rep(i, 1, k){
		ll id = Q.top().second;
		Q.pop();
		a[id] += sgn(a[id])*x;
		Q.push(P(abs(a[id]), id));
	}
}
...
int main(void)
{
	...
	if(mn < k*x){
		P p = P(inf, inf); ll mn_i;
		rep(i, 1, n){
			P q = P(ceil(abs(a[i])+1, x), abs(a[i]) % x);
			if(p > q) p = q, mn_i = i;
		}
		ll t = ceil(abs(a[mn_i])+1, x);
		a[mn_i] -= sgn(a[mn_i]) * t * x;
		k -= t;
		calc();
	}
	else a[mn_i] -= sgn(a[mn_i]) * k*x;
	outa(a, 1, n);
