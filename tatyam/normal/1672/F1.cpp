...
void solve(){
    ...
    each(x,a)cnt[x]++;
    vector<pll>b(all(cnt));
    sort(all(b),[](pll a,pll b){return a.second>b.second;});
    unordered_map<ll,vector<ll>>nxt;
    b.push_back({0,0});
    rep(i,1,b.size()){
        rep(_,b[i].second)nxt[b[i-1].first].push_back(b[i].first);
        rep(_,b[i-1].second-b[i].second)nxt[b[i-1].first].push_back(b[0].first);
    }
    auto pop=[](vector<ll>&a){
        ll ans=a.back();
        a.pop_back();
        return ans;
    };
    each(x,a)x=pop(nxt[x]);
    out(a);
}
int main(){
    LL(t);
