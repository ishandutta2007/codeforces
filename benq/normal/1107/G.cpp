...
int main() {
    ...
    R[1].build(cum);
    vpi dif;
    FOR(i,2,n+1) dif.pb({d[i]-d[i-1],i});
    sort(dif.rbegin(),dif.rend()); cur.insert({1,n});
    trav(a,dif) {
        auto z = *prev(cur.lb({a.s,MOD})); cur.erase(z);
        cur.insert({z.f,a.s-1}), cur.insert({a.s,z.s});
        // a.s to r 
        ckmax(ans,R[0].query(a.s,z.s)+R[1].query(z.f-1,a.s-2)-(ll)a.f*a.f);
        // pr(z,a,R[0].query(a.s,z.s),R[1].query(z.f-1,a.s-2),(ll)z.f*z.f);
        // l-1 to a.s-2 
        // l -- a.s-1 
        // a.s -- r 
    }
