...
void main_() {
	...
	V<ll> xmin(n, INF), ymin(n, INF);
	auto solve = [&](vector<ll> &x, bool f = 0) {
		ll tmp = 0, min_val = INF;
		REP(i, SZ(x)) {
			tmp += x[i];
			chmin(min_val, x[i]);
			(f ? xmin[i] : ymin[i]) = tmp + min_val * (n - 1 - i);
		}
	};
	solve(a);
	solve(b, 1);
	REP(i, n / 2) {
		chmin(res, xmin[i] + ymin[i]);
		chmin(res, xmin[i + 1] + ymin[i]);
	}
	print(res);
}
