...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
			hi[i]=nda(n);
		double[]ifyoulikemycode=new double[1<<n];
		ifyoulikemycode[0]=1;
		for(int i=0; i<(1<<n); i++)
		{
			for(int j=0; j<n; j++)
			{
				if((i&(1<<j))==0)
				{
					...
					ifyoulikemycode[i+(1<<j)]+=ifyoulikemycode[i]*(messagemeat/cfrahuldugar);
				}
			}
		}
		int i=(1<<n)-1;
		double peace=0;
		for(int j=0; j<n; j++)
			peace+=ifyoulikemycode[i^(1<<j)];
		for(int j=0; j<n; j++,p(" "))
			p(ifyoulikemycode[i^(1<<j)]/peace);
