...
int main(){
    ...
    auto dfs = [&](ll from, ll at, auto dfs) -> void {
        each(i, g[at]) if(i != from){
            ...
            dfs(at, i, dfs);
            if(chmax(height[at],height[i]+1))prev[at]=i;
        }
    };
    dfs(-1, 0, dfs);
    {
        ll leaf=0;
        rep(n)if(child[i].empty())leaf++;
        if(leaf<=k){
            chmin(k,max(leaf,n/2));
            return out(k*(n-k));
        }
    }
    ll w=0;
    priority_queue q([&](ll x,ll y){return height[x]<height[y];},vector<ll>{0});
    rep(k){
        ll at=q.top();
        q.pop();
        w+=height[at];
        while(height[at]){
            const ll nx=prev[at];
            each(i,child[at])if(i!=nx)q.push(i);
            at=nx;
        }
    }
    ll r=k,b=n-r-w;
