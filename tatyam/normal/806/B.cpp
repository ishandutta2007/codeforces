...
int main(){
    ...
    VEC(A,a,n);
    vec(ll,cnt,5);
    rep(n)rep(j,5)cnt[j]+=a[i][j]!=-1;
    rep(2)rep(j,5)if(a[i][j]==-1)a[i][j]=250;
    map<ll,ll>plus;
    auto&x=a[0],&y=a[1];
    rep(i,5){
        if(x[i]<y[i]||x[i]==250){
            for(ll m:{1,2,4,8,16,32})plus[cnt[i]*m]+=2*(y[i]-x[i]);
        }
        else{
            for(ll m:{1,2,4,8,16,32})if(cnt[i]*m<=n)plus[cnt[i]*m]+=2*(y[i]-x[i]);
            for(ll m:{2,4,8,16,32})if(cnt[i]*m<=n)plus[(n-cnt[i])*m/(m-1)+1]-=2*(y[i]-x[i]);
        }
    }
    auto p=plus.begin();
    ll sum=0;
    while(p!=plus.end()){
        if(sum>0&&p->first>n)return out(0);
        sum+=p->second;
        if(sum>0&&p->first>=n)return out(p->first-n);
        p++;
    }
    out(-1);
}