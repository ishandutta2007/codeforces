...
int main(){
	...
	cauto n=read();
	cauto pts=readV<tuple<int,int,int>>(n);
	vector<int> zip(n);
	rep(i,n) zip[i]=get<0>(pts[i]);
	mint re=(zip.back()+1)%MOD;
	rep(i,n){
		cauto &[x,y,s]=pts[i];
		cauto li=lower_bound(all(zip),y)-zip.begin();
		cauto c=(x-y)%MOD+Sum(i)-Sum(li);
		Add(i,c);
		if(s) re+=c;
	}
	cout<<re NL;
