...
void solve() {
  ...
  FOR(i, N) {
    ...
    if (A[i] != 0 && B[i] != 0) {
      ...
      elif (A[i] < B[i]) { pa = 0, eq = 0, pb = 1; }
      else {
        pa = 0, pb = 0, eq = 1;
      }
    }
    elif (A[i] == 0 && B[i] > 0) { eq = p, pa = mint(M - B[i]) * p, pb = mint(B[i] - 1) * p; }
    elif (B[i] == 0 && A[i] > 0) { eq = p, pb = mint(M - A[i]) * p, pa = mint(A[i] - 1) * p; }
    else {
      eq = p, pa = q, pb = q;
    }
    PB += EQ * pb;
