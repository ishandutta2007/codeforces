...
void mian() {
	...
	if(m == n - 1) {
		if(n == 4) puts("-1");
			...
			static int a[N], b[N];
			REP(i, 0, n / 2 - 1) a[i] = i, b[i] = n - 1 - a[i];
			b[0] = n - 4, b[1] = 3, a[3] = n - 2, b[3] = n - 1;
			REP(i, 0, n / 2 - 1) prt(a[i]), pc(' '), prt(b[i]), pc('\n');
			int sum = 0;
			REP(i, 0, n / 2 - 1) sum += a[i] & b[i];
			assert(sum == m);
		}
	}
	else {
		...
		vector<int*> v;
		REP(i, 0, n / 2 - 1) {
			if(a[i] == m || b[i] == n - 1) v.pb(b + i);
			if(b[i] == m || a[i] == n - 1) v.pb(a + i);
		}
		if(m) swap(*v[0], *v[1]);
		REP(i, 0, n / 2 - 1) prt(a[i]), pc(' '), prt(b[i]), pc('\n');
		int sum = 0;
