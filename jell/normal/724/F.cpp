#line 1 "codeforces-workspace\\contest\\a.cpp"
...
#endif
#line 8 "codeforces-workspace\\contest\\a.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\a.cpp"
...
#line 11 "Library\\src\\data_structure\\ndvec.hpp"
...
#line 36 "codeforces-workspace\\contest\\a.cpp"
...
void main() {
  ...
  for (auto m : range(1, n / 2 + 1)) {
    for (auto k : rrange(dp.size())) {
      for (auto s : range(dp.size<1>())) {
        mint z = 1, w = dp[m - 1][d - 1] + dp[m - 1][0];
        for (auto t : range(1, dp.size<1>() - s)) {
          if (m * t > k) break;
          z *= w++;
          z /= t;
          dp[k][s + t] += dp[k - m * t][s] * z;
        }
      }
    }
  }

  cout << dp[n - 1][d] + dp[n - 1][1] + dp[n - 1][0] -
              (n & 1 ? mint{0} : dp[n / 2][1] * (dp[n / 2][1] - 1) / 2)
       << "\n";
}
