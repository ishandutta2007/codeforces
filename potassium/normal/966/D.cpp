...
int main() {
	...
	vector<int> ans;
	auto upd = [&](const vector<int>& now) {
		if (ans.empty() || now.size() < ans.size())
			ans = now;
	};
	...
	auto hasEdge = [&](int u, int v) {
		return *lower_bound(g[u].begin(), g[u].end(), v) == v;
	};
	...
	for (auto& u : g[1]) {
		...
		if (all1) {
			bool done = false;
			for (int i = 0; i < (int)cc.size() && !done; i++)
				for (int j = i + 1; j < (int)cc.size() && !done; j++) {
					int x = cc[i];
					int y = cc[j];
					if (!hasEdge(x, y)) {
						for (int k = 0; k < (int)cc.size(); k++) {
							int z = cc[k];
							if (z == x || z == y) continue;
							if (hasEdge(x, z) && hasEdge(y, z)) {
								done = true;
								upd(vector<int>{ 1, x, z, y, x, n });
								break;
							}
						}
						assert(done);
