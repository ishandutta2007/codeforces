...
S op(S l, S r) {
	return S{ l.val + r.val, min(l.l, r.l), max(l.r, r.r) };
}
...
S mapping(F l, S r) {
	if (l.time == -1) {
		...
		r.val = l.tag * (r.r - r.l + 1);
	}
	return r;
}

F composition(F l, F r) {
	return l.time > r.time ? l : r;
}
...
int main() {
	CASE(t) {
		...
		bool can = 1;
		for (int i = q - 1; i >= 0; i--) {
			int len = qry[i].se - qry[i].fi + 1;
			int cnt1 = st.prod(qry[i].fi, qry[i].se + 1).val;
			...
			if (cnt1 * 2 < len) {
				F setAllZero = { q - i, 0 };
				st.apply(qry[i].fi, qry[i].se + 1, setAllZero);
			}
			else {
				F setAllOne = { q - i, 1};
				st.apply(qry[i].fi, qry[i].se + 1, setAllOne);
			}
		}

		string res(n, '_');
		for (int i = 0; i < n; i++) {
			res[i] = st.get(i).val + '0';
		}
