...
void MAIN() {
    ...
    REP (i, N) {
	D[i] = B[i] - A[i];
	sums[i+1] = sums[i] + B[i] - A[i];
    }

    IRMQ<LL> rmq(sums, sums+N+1);
    IRXQ<LL> rxq(sums, sums+N+1);
    IRMQ<LL> rmq2(D, D+N);
	...
	scanf("%d%d", &l, &r);
	l--;
	LL mi = rmq.min_v(l, r+1) - sums[l];

	if (sums[r] - sums[l] != 0 || mi < 0) {
	    puts("-1");
	} else {
	    LL a = -rmq2.min_v(l, r);
	    LL b = rxq.max_v(l, r+1) - sums[l];
	    LL ans = max(a, b);
	    printf("%lld\n", ans);
