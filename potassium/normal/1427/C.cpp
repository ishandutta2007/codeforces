...
int main() {
	...
	for (int i = 1; i <= n; i++) {
		pair<int, pii>& now = event[i];
		R(now.F, now.S.F, now.S.S);
	}
	
	for (int i = 1; i <= n; i++) {
		for (int j = i - 1; j >= 0; j--) {
			if (event[i].F - event[j].F <= 1007) {
				int dist = abs(event[i].S.F - event[j].S.F) + abs(event[i].S.S - event[j].S.S);
				int td = event[i].F - event[j].F;
