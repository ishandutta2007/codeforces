...
signed main(){
    ...
    rep(n){
        STR(s);
        const ll cnt=count(all(s),'B');
        a.emplace_back(cnt,s.size()-cnt);
    }
    Uniq(a);
    ll ok = 1e6, ng = -1;
    ll ymin=-LINF,ymax=LINF,xmin=-LINF,xmax=LINF,x_ymin=-LINF,x_ymax=LINF;
    auto check = [&](ll k) -> bool {
        ymin=-LINF,ymax=LINF,xmin=-LINF,xmax=LINF,x_ymin=-LINF,x_ymax=LINF;
        each(x,y,a){
            chmax(xmin,x-k);
            chmax(ymin,y-k);
            chmax(x_ymin,x-y-k);
        ...
        return ymin<=ymax&&xmin<=xmax&&x_ymin<=x_ymax&&ymin<=xmax-x_ymin&&xmin-x_ymax<=ymax;
    };
    while(abs(ok - ng) > 1){
        ll cen = (ok + ng) / 2;
        (check(cen) ? ok : ng) = cen;
    ...
    out(ok);
    check(ok);
    rrep(x,xmin,xmax+1)for(ll y=min(ymax,x-x_ymin);y>=max(ymin,x-x_ymax);y--)return out(string(x,'B')+string(y,'N'));
}