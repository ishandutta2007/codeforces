...
int main()
{
    ...
    int n,m,k;
    cin>>n>>m>>k;
    vvi vert(n,vi(m-1)),horz(n-1,vi(m)),ans(n,vi(m,-1));
    for(int i = 0 ; i < n ; i++)
    {
        for(int  j = 0; j < m-1; j++)
        {
    ...
    for(int jump = 2;jump<=k+1;jump+=2)
    {
        for(int i = 0 ; i < n; i++)
        {
            for(int j = 0 ; j < m ; j++)
            {
                if(i-1>=0)
                {
                    dp[i][j][jump]=min(dp[i][j][jump],dp[i-1][j][jump-2]+2*horz[i-1][j]);
                }
                if(i+1<n)
                {
                    dp[i][j][jump]=min(dp[i][j][jump],dp[i+1][j][jump-2]+2*horz[i][j]);
                
                }
                if(j-1>=0)
                {
                    dp[i][j][jump]=min(dp[i][j][jump],dp[i][j-1][jump-2]+2*vert[i][j-1]);
                
                }
                if(j+1<m)
                {
                    dp[i][j][jump]=min(dp[i][j][jump],dp[i][j+1][jump-2]+2*vert[i][j]);
                
                }
            //    cout<<dp[i][j][jump]<<" "<<i<<" "<<j<<" "<<jump<<endl;
            }
        }
