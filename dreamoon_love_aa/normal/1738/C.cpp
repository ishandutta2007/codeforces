...
bool  visited[SIZE][SIZE][2][2];
bool f(int x, int y, int v1, int v2){
    if(!x&&!y){
        if(n%2==0){
            return v1==0;
        }else{
    ...
    if(visited[x][y][v1][v2])return dp[x][y][v1][v2];
    visited[x][y][v1][v2]=1;
    int &an=dp[x][y][v1][v2];
    an=0;
    if(x>0&&!f(x-1,y,v2,v1^1))an=1;
    if(y>0&&!f(x,y-1,v2,v1))an=1;
    return an;
}
void solve() {
    MS0(visited);
    ...
    W(f(odd_num,n-odd_num,0,0)?"Alice":"Bob");
}
void input() {
    R(n);
    REP(i,n)R(a[i]);
