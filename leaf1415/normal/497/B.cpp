...
int main(void)
{
	...
	rep(i, 1, n){
		ll p = 0, cnt = 0, cnt2 = 0;
		while(p < n){
			ll pos = lower_bound(sum+p+1, sum+n+1, sum[p]+i) - sum;
			ll pos2 = lower_bound(sum2+p+1, sum2+n+1, sum2[p]+i) - sum2;
			p = min(pos, pos2);
			if(p > n) break;
			if(a[p] == 1) cnt++;
			else cnt2++;
		...
		if(p == n){
			if(a[n] == 1 && cnt > cnt2) ans.pb(P(cnt, i));
			if(a[n] == 2 && cnt < cnt2) ans.pb(P(cnt2, i));
		}
	}
	sort(all(ans));
	
	outl(sz(ans));
	for(auto p : ans) cout << p.fst << " " << p.snd << "\n";
	return 0;
}