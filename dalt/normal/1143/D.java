...
public class CFCR549 {
    ...
    public static class Task implements Runnable {
        ...
        public void solve() {
            ...
            int a = io.readInt();
            int b = io.readInt();
            int[] l = new int[]{a + b, a + k - b, k - a + b, k - a + k - b};
            long min = (long) 1e15;
            long max = 1;
            for (int guess : l) {
                long[] r = tryAll(guess);
            ...
            io.cache.append(min).append(' ').append(max);
        }

        public long[] tryAll(int l) {
            long min = (long) 1e15;
            long max = 1;
            long nk = (long)n * k;
            for (int i = 0; i < n; i++) {
                long guess = l + (long) i * k;
                long gcm = nk / Mathematics.gcd(guess, nk);
                max = Math.max(max, gcm);
                min = Math.min(min, gcm);
            }
