// Red-Blue Matrix (https://codeforces.com/contest/1606/problem/D)
...
void solve() {
    ...
    auto lmx = a, rmx = a, lmn = a, rmn = a;
    rep(i, n) {
        rep(j, 1, m)
            lmx[i][j] = max(lmx[i][j - 1], a[i][j]), lmn[i][j] = min(lmn[i][j - 1], a[i][j]);
        repr(j, m - 2)
            rmx[i][j] = max(rmx[i][j + 1], a[i][j]), rmn[i][j] = min(rmn[i][j + 1], a[i][j]);
    }
    ...
    rep(i, m - 1) {
        string ans(n, 'R');
        sort(all(idx), [&](int i, int j) { return lmx[i] < lmx[j]; });
        int blue_left = 0, blue_right = INF;
        multiset<int> red_left, red_right;
        rep(j, n)
            red_left.emplace(lmn[j][i]), red_right.emplace(rmx[j][i + 1]);
        trav(j, idx) {
            ans[j] = 'B';
            red_left.erase(red_left.find(lmn[j][i]));
            red_right.erase(red_right.find(rmx[j][i + 1]));
            blue_left = max(blue_left, lmx[j][i]);
            blue_right = min(blue_right, rmn[j][i + 1]);
            if (!sz(red_left))
                break;

            if (blue_left < *begin(red_left) && *rbegin(red_right) < blue_right) {
                println("YES");
                println(ans, i + 1);
                return;
            }
