...
int main()
{
	...
	cin>>n;
	mc["A"]=mc["B"]=mc["C"]=mc["AB"]=mc["AC"]=mc["BC"]=mc["ABC"]=1000000;
	while (n--)
	{
		int a;
		string b;
		...
		{
			string c;
			for (int i=0;i<b.size();++i) if (mask&(1<<i)) c.push_back(b[i]);
			sort(c.begin(),c.end());
			mc[c]=min(mc[c],a);
		}
	}
	int x=min(min(mc["A"]+mc["BC"],mc["B"]+mc["AC"]),min(min(mc["C"]+mc["AB"],mc["ABC"]),mc["A"]+mc["B"]+mc["C"]));
	if (x>=1000000) cout<<-1;
	else cout<<x;
}