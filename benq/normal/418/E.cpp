...
void upd(int p, int inc) {
	int v = a[p];
	if (inc == -1) t[v].erase(p);
	else t[v].insert(p);
	FOR(i,(p+BLOCK-1)/BLOCK,N/BLOCK+1) {
		CNT[i][cnt[i][v]] --; cnt[i][v] += inc; CNT[i][cnt[i][v]] ++;
		int c = cnt[i][v];
		if (c > 0 && !in[i][c]) {
			in[i][c] = 1;
			con[i].pb(c);
...
int query3(int y) {
	...
	vi ok;
	trav(i,con[b]) {
		if (!CNT[b][i]) { in[b][i] = 0; continue; }
		ok.pb(i); if (i >= mn) ans += CNT[b][i];
	}
	swap(con[b],ok);
	FOR(i,BLOCK*b+1,y+1) {
		cnt[b][a[i]] ++;
