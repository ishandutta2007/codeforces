...
int main(){
	...
	std::vector<modular> factorial(number+1);
	factorial[0]=1;
	for(int i=1; i<(int)factorial.size(); ++i)
		factorial[i]=factorial[i-1]*i;

	std::vector<modular> result(number);

	std::vector<modular> data(number);
	data[0]=1;

	auto const process=[&](int l){
		// now data[i] should be the number of permutations of l elements, i consecutive pairs are increasing
		auto const tmp=factorial[number]/factorial[l];
		for(int i=0; i<l; ++i)
			result[i]+=data[i]*tmp;
	};
	...
	for(auto it: result) std::cout<<(int)it<<' ';
	std::cout<<'\n';
}