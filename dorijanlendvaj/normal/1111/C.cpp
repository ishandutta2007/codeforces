...
int dx[]={0,1,0,-1};
int dy[]={1,0,-1,0};
gp_hash_table<int,ll,custom_hash> seg;
gp_hash_table<int,int,custom_hash> av;
...
assert(k+n!=100030);
	...
	for (int i=0;i<k;++i)
	{
		//if (i%1500==0 && n*k>=2e6) cout<<i<<endl;
		int z;
		cin>>z;
		--z;
		z+=n;
		...
		while (z)
		{
			av[z]=av[z*2]+av[z*2+1];
			if (av[z*2]==0)
				seg[z]=min(a+seg[z*2+1],b*av[z]*l);
			else if (av[z*2+1]==0)
				seg[z]=min(a+seg[z*2],b*av[z]*l);
			else
				seg[z]=min(seg[z*2+1]+seg[z*2],b*av[z]*l);
			z/=2;
			l*=2;
		}
	}
