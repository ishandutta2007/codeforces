...
int main() {
    ...
    for (int sz = 4; sz <= N; sz *= 2) {
        for (int i = 0; i < N; i += sz) {
            ...
            F0R (a, 4) F0R (b, 4) {
                ...
                { // wi wins winner match
                    ckmax(res[2 * wi + (wj || li || lj)],
                        dp[i][a] + dp[j][b] + (wi || wj) + (li || lj) + (wj || li || lj));
                }
                { // wj wins winner match
                    ckmax(res[2 * wj + (wi || li || lj)],
                        dp[i][a] + dp[j][b] + (wi || wj) + (li || lj) + (wi || li || lj));
                }
            }
            dp[i] = res;
        }
    ...
    int ans = 0;
    F0R (i, 4) ckmax(ans, dp[0][i] + (i != 0)); // final match
    ps(ans);
