#line 1 "codeforces-workspace\\contest\\1556\\d\\d.cpp"
...
#line 16 "codeforces-workspace\\contest\\1556\\d\\d.cpp"
//*/
...
auto fin(int a) { cout << "finish " << a << endl; }
...
void main() {
  ...
  vector<i32> ors{0}, ands{0};
  for (auto i : range(1, n)) {
    ors.emplace_back(ask_or(0, i));
    ands.emplace_back(ask_and(0, i));
  }

  vector<i32> a(n);
  {
    // auto or12 = ask_or(1, 2);
    auto and12 = ask_and(1, 2);
    a[0] = ands[1] | ands[2] | (ors[1] & ors[2] & ~and12);
  }

  for (auto i : range(1, n)) {
    a[i] = a[0] ^ (ors[i] ^ ands[i]);
  }
