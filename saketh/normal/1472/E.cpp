...
void solve() {
    ...
    vi order(N);
    iota(all(order), 0);
    sort_by(order, w[a] != w[b] ? w[a] < w[b] : h[a] < h[b]);

    vector<pair<int, pii>> useful;
    useful.emplace_back(-1, mp(INT_MAX, -1));

    trav (inx, order) {
        if (h[inx] >= useful.back().s.f)
            continue;

        useful.emplace_back(w[inx], mp(h[inx], inx));
    }

    F0R (i, N) {
        if (i) pr(" ");

        auto it = lb(all(useful), mp(w[i], mp(-1, -1)));

        if (prev(it)->s.f < h[i]) {
            pr(prev(it)->s.s + 1);
        } else {
            auto it2 = lb(all(useful), mp(h[i], mp(-1, -1)));
            if (prev(it2)->s.f < w[i]) {
                pr(prev(it2)->s.s + 1);
            } else {
                pr(-1);
