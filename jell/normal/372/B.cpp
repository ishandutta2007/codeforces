#line 1 "codeforces-workspace\\contest\\372\\b\\b.cpp"
...
#line 6 "codeforces-workspace\\contest\\372\\b\\b.cpp"
// #include "lib/cxx20"
...
#endif
#line 8 "codeforces-workspace\\contest\\372\\b\\b.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace\\contest\\372\\b\\b.cpp"
...
void main() {
  ...
  auto pv = make_vector<int>({n, m}, 0);
  for (auto &&[row, v] : zip(grid, pv)) {
    auto last = 0;
    for (auto &&[i, x, y] : enumerate(row, v)) {
      if (x == '1')
        y = i + 1;
      else
        y = last;
  ...
  auto dp = cached([&](auto &&dp, int a, int b, int c, int d) -> int {
    if (a == c || b == d) return 0;
    auto ret = dp(a, b, c - 1, d) + dp(a, b, c, d - 1) - dp(a, b, c - 1, d - 1);
    int w = b;
    for (auto i : rrange(a, c)) {
      chgr(w, pv[i][d - 1]);
      ret += d - w;
    }
    return ret;
