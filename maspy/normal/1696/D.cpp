...
#line 2 "/home/maspy/compro/library/alg/monoid_max_idx.hpp"
template <typename T, bool tie_is_left = true>
...
void solve() {
  ...
  Graph G(N);
  VEC(ll, A, N);
  vc<pair<ll, int>> seg_raw(N);
  FOR(i, N) seg_raw[i] = {A[i], i};
  SegTree<Monoid_Min_Idx<ll>> seg_mi(seg_raw);
  SegTree<Monoid_Max_Idx<ll>> seg_ma(seg_raw);

  FOR(i, N - 1) {
    if (A[i] > A[i + 1]) {
      auto check = [&](auto e) -> bool { return e.fi <= A[i]; };
      int j = seg_ma.max_right(check, i);
      auto [v, k] = seg_mi.prod(i, j);
      // print(i, k);
      G.add(i, k);
    } else {
      auto check = [&](auto e) -> bool { return e.fi >= A[i]; };
      int j = seg_mi.max_right(check, i);
      auto [v, k] = seg_ma.prod(i, j);
      // print(i, k);
      G.add(i, k);
    }
  }
