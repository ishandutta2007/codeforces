...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	while(T--){
		...
		const int a=max(0,p1.X-p0.Y);
		const int b=abs(p1.X-p0.X)+abs(p1.Y-p0.Y)-a;
		const ll inter=max(0,p0.Y-max(p0.X,p1.X));
		auto rem=lb-inter*n;
		out(n,lb,a,b,inter,rem,1);
		ll re=1e18;
		ll cost=0;
		if(rem<=0){
			cout<<0 NL;
		...
		reps(i,1,n+1){
			cost+=a;
			auto tmp=min<ll>(rem,b);
			cost+=tmp;
			rem-=tmp;
			out(rem,cost,1);
			MN(re,cost+max(0ll,rem)*2);
		}
		cout<<re NL;
	}
  return 0;
