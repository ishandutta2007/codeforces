...
int main() {
    ...
    vector<int> l(n);
    vector<vector<int>> cum_min(n);
    vector<map<int, vector<int>>> mps(n);
    rep(i, n) {
        int m = s[i].size();
        cum_min[i].resize(m + 1, 0);
        ...
        rep(j, m) {
            ...
            cum_min[i][j + 1] = max(cum_min[i][j], -level[j + 1]);
            mps[i][level[j + 1]].push_back(j + 1);
        }
        l[i] = level[m];
    }
    ...
    int ans = 0;
    vector sum_l(1 << n, 0);
    vector dp(1 << n, numeric_limits<int>::min());
    dp[0] = 0;
    rep(s, 1, 1 << n) {
        rep(i, n) {
            if (not kth_bit(s, i)) continue;
            int t = s ^ (1 << i);
            sum_l[s] = sum_l[t] + l[i];
            if (dp[t] == numeric_limits<int>::min()) continue;
            
            int m = cum_min[i].size();
            int r = upper_bound(all(cum_min[i]), sum_l[t]) - cum_min[i].begin();
            int v = dp[t];
            if (auto it = mps[i].find(-sum_l[t]); it != mps[i].end()) {
                auto &ids = it->second;
                v += lower_bound(all(ids), r) - ids.begin();
            }
            chmax(ans, v);
            if (r == m) {
                chmax(dp[s], v);
