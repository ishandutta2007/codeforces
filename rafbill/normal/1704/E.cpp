...
void solve() {
  ...
  vector<int> used(n,0);
  for(li step = 0; step < (li)(2*n); ++step) {
    bool anyused = 0;
    for(li i = 0; i < (li)(n); ++i) {used[i] = A[i]>0; if(used[i]) anyused = 1;}
    if(!anyused) {
      cout << step << endl;
      return;
    }
    for(li i = 0; i < (li)(n); ++i) if(used[i]) {for(int j : G[i]) { A[j] += 1; } A[i] -= 1;}
  }
  vi N(n,0);
  for(li i = 0; i < (li)(n); ++i) N[i] = RG[i].size();
  vi Q;
  for(li i = 0; i < (li)(n); ++i) if(!N[i]) Q.push_back(i);
  vector<ZN> B(n); for(li i = 0; i < (li)(n); ++i) B[i] = A[i];
  while(!Q.empty()) {
    int i = Q.back(); Q.pop_back();
    for(int j : G[i]) {
      B[j] += B[i];
      N[j]-=1;
      if(N[j] == 0) Q.push_back(j);
    }
    if(G[i].empty()) {
      cout << B[i]+2*n << endl;
    }
