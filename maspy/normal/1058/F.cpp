...
void solve() {
  ...
  FOR(i, N) X[i] -= i;
  VEC(ll, A, N);
  vc<pair<i128, i128>> seg_raw(N);
  FOR(i, N) seg_raw[i] = {A[i], A[i] * X[i]};
  ...
  FOR_(Q) {
    ...
    if (a < 0) {
      ...
      --a;
      i128 n = seg.prod(a, b).fi;
      auto check = [&](auto e) -> bool {
        auto [c, s] = e;
        return c <= n / 2;
      };
      auto m = seg.max_right(check, a);
      i128 x = X[m];
      i128 cost = 0;
      {
        auto [c, s] = seg.prod(a, m);
        cost += x * c - s;
      }
      {
        auto [c, s] = seg.prod(m, b);
        cost += s - x * c;
      }
      cost %= mod;
      print(int(cost));
