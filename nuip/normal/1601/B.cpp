...
int main(){
	...
	cauto downs=readV(n);
	vv<int> g(n);
	rep(i,n) g[i+downs[i]].eb(i);
	vector<int> goals{-1};
	vector<pii> ps(n);
	rep(i,n) ps[i]={i-ups[i],i};
	sort(rall(ps));
	out(g,ps,1);
	rep(t,MOD){
		out(goals,1);
		if(ps.empty() || ps.back().X>goals.back()) break;
		int mx=goals.back();
		while(ps.size() && ps.back().X<=goals.back()){
			cauto v=ps.back().Y; ps.pop_back();
			for(cauto &u:g[v]) MX(mx,u);
		}
		if(goals.back()==mx) break;
		goals.eb(mx);
	}
	if(goals.back()!=n-1){
		cout<<-1 NL;
	}else{
		cout<<goals.size()-1 NL;
		rrep(i,goals.size()-1) cout<<goals[i]+1<<" ";
		cout NL;
	}
	return 0;
}