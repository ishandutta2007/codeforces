...
//	freopen("testdata.in","r",stdin);
	...
	for (; Testing; Testing--) {
		...
		for (i=1;i<=n-2;i++)
		   ...
		   fuck=S[i];fuck=fuck+S[i+1];fuck=fuck+S[i+2];
		   if ((fuck=="one")||(fuck=="two"))
		      if ((exist[i]|exist[i+1]|exist[i+2])==false)
		          {
		          	if (S[i]=='o') 
					     {
		          	...
		          	if (S[i]=='t') 
		          	   {
		          	   	if ((i+3<=n)&&(S[i+3]=='o')) exist[i+1]=true;
		          	   	else exist[i+2]=true;
						 }
				  }
		}
