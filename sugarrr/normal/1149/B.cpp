...
vvvl dp;
string a,b,c;
void cal(ll i,ll j,ll k){
    if(i-1>=0 && dp[i-1][j][k]<inf){
        chmin(dp[i][j][k],nx[dp[i-1][j][k]][a[i-1]-'a']);
    }
    if(j-1>=0 && dp[i][j-1][k]<inf){
        chmin(dp[i][j][k],nx[dp[i][j-1][k]][b[j-1]-'a']);
    }
    if(k-1>=0 && dp[i][j][k-1]<inf){
        chmin(dp[i][j][k],nx[dp[i][j][k-1]][c[k-1]-'a']);
    }
}
signed main(){fastio
    ll n,q;cin>>n>>q;
    string s;cin>>s;
    nx.assign(n+1,vl(26,inf));
    rep(j,0,25){
        char x='a'+j;
        ll pos=inf;
        rrep(i,n,0){
    ...
    while(q--){
        ...
        as=a.size();bs=b.size();cs=c.size();
        if(dp[as][bs][cs]<inf)cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
