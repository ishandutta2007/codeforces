...
struct cmp {
    bool operator()(const vector<mint>& a, const vector<mint>& b) {
...
int main() {
    ...
    multiset<vector<mint>, cmp> ms;
    ms.emplace(vector<mint>(1, 1));
    for (auto e : mp) {
        ms.emplace(vector<mint>(e.second + 1, 1));
    }
    while (ms.size() != 1) {
        auto j = ms.begin();
        auto i = j++;
        ms.emplace(multiply(*i, *j));
        ms.erase(i);
        ms.erase(j);
    }
    vector<mint> res = *ms.begin();
    int z = res.size();
    cout << res[z / 2] << '\n';
    return 0;
}