...
#endif
int a[1000005],b[1000005],c[1000005],lst[1000005],nxt[1000005];
...
;signed main()
{
	initprog();
	int T=read();rg(T)
	int n=read();rg(n)a[i]=read();gr rg(n)b[i]=read();gr
	rg(n)c[i]=lst[i]=0;gr
	for(int i=n;i>=1;i--)
	{
	...
	while(B<=n)
	{
		...
		else if(nxt[cc])c[nxt[cc]]+=c[cc],cc++;
		else {ok=1;break;}
	}
	if(ok)puts("NO");
	else puts("YES");
	// tr.n=n;rg(n)tr.a[i]=0;gr
	// rg(n)tr.add(i,1);gr
	// rg(n)s[i].clear();gr
	// rg(n)s[b[i]].insert(i);gr
	// int ok=0;
	// rg(n)
	// // if(s[a[i]].)
	// if()
