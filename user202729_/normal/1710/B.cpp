...
void up(){
	int const numDay=read<int>();
	int const maxAllowed=read<int>();
	
	std::map<int, int> critical;
	std::vector<std::pair<int, int>> days(numDay);
	for(auto& [pos, intensity]: days){
		pos=read<int>();
		intensity=read<int>();
		
		critical[pos-intensity]+=1;
		critical[pos]-=2;
		critical[pos+intensity]+=1;
	}

	int pos=critical.begin()->first;
	int64_t value=0;
	int slope=0;

	int64_t minNeedCover=INT64_MAX, maxNeedCover=INT64_MIN;

	for(auto [pos2, slopeChange]: critical){
		value+=int64_t(slope)*(pos2-pos);
		pos=pos2;
		slope+=slopeChange;

		if(value>maxAllowed){
			minNeedCover=std::min(minNeedCover, pos-(value-maxAllowed));
			maxNeedCover=std::max(maxNeedCover, pos+(value-maxAllowed));
		}
	}

	// won't overflow right? +- is 2e9 max

	for(auto [pos, intensity]: days){
		std::cout<<(
				(pos-intensity <= minNeedCover and maxNeedCover <= pos+intensity)
				? '1': '0');
	}
	std::cout<<'\n';
}
...
// time-limit: 4000
// problem-url: https://codeforces.com/contest/1710/problem/B