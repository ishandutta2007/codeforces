...
//    cout <<w <<endl;
    ...
    for(int i = 0; i < w.size(); i++) L[i] = R[i] = 0;

    for(int i = 0; i < w.size(); i++) S[i] = w[i];
    S[w.size()] = 0;
    for(int i = 0; i < s[k].size(); i++) T[i] = s[k][i];
    T[s[k].size()] = 0;
    getNext(T);
    getExtend(S, T);
    for(int i = 0; i < w.size(); i++)
        if(i + Extend[i] == w.size())
            R[i] = 1;

    getNext(S);
    getExtend(T, S);
    for(int i = 0; i < w.size(); i++)
        if(Extend[s[k].size() - 1 - i] == i + 1)
            L[i] = 1;

//    for(int i = 0; i < w.size(); i++) cout <<L[i]; cout <<endl;
//    for(int i = 0; i < w.size(); i++) cout <<R[i]; cout <<endl;
    ...
    {
        int ch = w[i] - 'a';
        if((i == 0 || L[i - 1]) && (i + 1 == w.size() || R[i + 1]))
            cc[ch]++;
    }

    mint res = 0;
    res = mint(cnt) * mint(2).pow(wq - k);
//    cout <<"cnt : " <<cnt <<" " <<res <<endl;

    for(int i = 0; i < 26; i++)
    {
        mint mul = po[wq][i] - po[k][i] * mint(2).pow(wq - k);
        res += mint(cc[i]) * mul;
    }
    return res.x;
}
