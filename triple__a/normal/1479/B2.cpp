...
signed main(){
    ...
    seg.apply(1,n+7,{-100000000});
    rep1(i,n){
        auto ret=seg.all_prod();
        ret.sum=max(ret.sum,seg.get(a[i]).sum+1);
        if (a[i]==a[i-1]) seg.apply(0,n+7,{1});
        if (seg.get(a[i]).sum<ret.sum) seg.set(a[i-1],ret);
//        rep(j,n+1) cout<<seg.get(j).sum<<" ";
//        cout<<endl;
    }
    cout<<n-seg.all_prod().sum;
    return 0;
}