...
void solve(set<int>& s, V& v) {
	sort(all(v));
	int ls = 0, su = 0;
	map<int, int> p;
	for(auto [y, x] : v) {
		if(*s.lb(y) != ls) ls = *s.lb(y), p.clear(), su = 0;
		as += su - p[x], su++, p[x]++;
	}
}
void solve() {
	...
	int x, y;
	scanf("%d%d%d", &n, &m, &k);
	while(n--) scanf("%d", &x), xs.insert(x);
	while(m--) scanf("%d", &y), ys.insert(y);
	V a, b;
	while(k--) {
		scanf("%d%d", &x, &y);
		if(xs.count(x) & ys.count(y)) continue;
		xs.count(x) ? a.eb(y, x) : b.eb(x, y);
	};
	as = 0, solve(ys, a), solve(xs, b), printf("%lld\n", as);
}
int main() {
