...
int main(void)
{
	...
	while(T--){
		...
		ll logn = digitnum(n+5, 2);
		DisjointSparseTable sp(logn);
		rep(i, 1, n) sp.set(i, a[i]);
		sp.calc();
		
		ll ans = 0, ans_i;
		rep(i, 1, n){
			ll ub = n+1, lb = i-1, mid;
			while(ub-lb>1){
				mid = (ub+lb)/2;
				if(sp.query(i, mid) >= a[i-1] - s) lb = mid;
				else ub = mid;
			}
			if(ans < lb-i+1){
				ans = lb-i+1, ans_i = i;
