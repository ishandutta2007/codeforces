...
struct stable {
	...
	void init() {
		...
		REP(i, 1, n) d[0][i] = a[i];
		REP(i, 1, LOG_N - 1) REP(j, 1, n - (1 << i) + 1) d[i][j] = gcd(d[i - 1][j], d[i - 1][j + (1 << i - 1)]);
	}
	int cd(int l, int r) {
		int lg0 = lg[r - l + 1];
		return gcd(d[lg0][l], d[lg0][r - (1 << lg0) + 1]);
	}
} gcd_st;
...
void mian() {
	...
	REP(i, 1, n) {
		int las = i;
		PER(j, 20, 0) if(las > 1 << j && gcd_st.cd(las - (1 << j), i) >= i - (las - (1 << j)) + 1) las -= 1 << j;
		if(gcd_st.cd(las, i) == i - las + 1) can[i] = las + 1;
		else can[i] = 1;
	}
	st.init(n, can);
	REP(i, 1, n) {
		int las = i + 1;
		PER(j, 20, 0) if(las > 1 << j && st._mx(las - (1 << j), i) <= las - (1 << j)) las -= 1 << j;
		if(las == 1) dp[i] = 0;
		else dp[i] = 1 + dp[las - 2];
	}
	REP(i, 1, n) prt(dp[i]), pc(" \n"[i == n]);
}
