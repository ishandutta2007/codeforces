...
void dfs(ll l, ll r, ll d)
{
	...
	ll p = m;
	rep(i, l, m){
		while(p+1 <= r && a[p+1] < a[i]) p++;
		inv[d][0] += p-m;
	}
	ll q = l-1;
	rep(i, m+1, r){
		while(q+1 <= m && a[q+1] < a[i]) q++;
		inv[d][1] += q-(l-1);
	}
	...
	while(p <= m || q <= r){
		if(p > m) vec.pb(a[q++]);
		else if(q > r) vec.pb(a[p++]);
		else if(a[p] < a[q]) vec.pb(a[p++]);
		else vec.pb(a[q++]);
	}
	rep(i, l, r) a[i] = vec[i-l];
...
int main(void)
{
	...
	rep(i, 1, Q){
		cin >> q;
		if(q > 0) rep(j, 1, q) state[j] ^= 1;
		ll ans = 0;
		rep(j, 1, n) ans += inv[j][state[j]];
		cout << ans << "\n";
	}
