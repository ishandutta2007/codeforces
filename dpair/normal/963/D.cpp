...
int p[MAXN], tot;
signed main(){
    read(s + 1); const int n = strlen(s + 1);
    rep(i, 1, n) b[s[i] - 97].set(i);
    int q = read(); while(q --) {
        const int k = read(); read(t + 1); const int m = strlen(t + 1); ans.set(); rep(i, 1, m) ans &= (b[t[i] - 97] >> i);
        tot = 0; int it = ans._Find_first(); while(it != ans.size()) {p[++ tot] = it; it = ans._Find_next(it); }
        if(tot < k) print("-1");
        else { int ret = INF; rep(i, k, tot) chmin(ret, p[i] - p[i - k + 1]); print(ret + m);}
    }
}