...
fn main() {
	...
	for u in 0..n {
		for v in 0..n {
			...
			let (x,y,_) = p[0];
			let (xx,yy,_) = p[1];
			for (xx,yy) in vec![(xx,yy), (yy,xx)] {
				let d1 = d + dist[u][x] + dist[v][y];
				let d2 = d + dist[u][xx] + dist[v][yy];
				if d1 <= p[0].2 && d2 <= p[1].2 {
					min!(res, d1 + d2 - d);
				}
			}
		}
	}
	let sol = m as i32 - res;
	writeln!(rout, "{}", sol).ok();
}