...
public:
	void solve(std::istream& in, std::ostream& out) {
		...
		auto dp = make_vector<Z>(n, 2, 201, Z());

		for (int i: inclusiveRange(1, 201)) {
			if (a[0] == -1 || a[0] == i) {
				dp[0][0][i] = 1;
			}
		}
		...
		for (int i: range(1, n)) {
			for (int t: range(2)) { // current
				for (int u: range(2)) { // prev
					auto ps = prefixSums(dp[i - 1][u]);
					for (int j: inclusiveRange(1, 200)) {
						if (a[i] == -1 || a[i] == j) {
							if (t == 1 && u == 0) {
								dp[i][t][j] = dp[i - 1][u][j];
							}
							else if (t == 1) {
								dp[i][t][j] += ps.back() - ps[j];
							}
							else {
								dp[i][t][j] += ps[j];
							}
		...
		auto res = dp.back()[1];
		out << accumulate(res.begin(), res.end(), Z());
	}
};
