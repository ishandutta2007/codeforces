...
ll dfs(int v, int d)
{
	if(d != 1 && in[v] >= 2) return -1;
	if(sz(G[v]) >= 2) return -1;
	if(sz(G[v]) == 0) return d;
	return dfs(G[v][0], d+1);
}
...
int main(void)
{
	...
	cin >> n >> m >> k;
	ll l, x; vector<ll> vec;
	rep(i, 1, n){
		cin >> l; vec.clear();
		rep(j, 1, l) cin >> x, vec.push_back(x);
		rep(j, 1, l-1) G[vec[j-1]].push_back(vec[j]);
	}
	rep(i, 1, k) uniq(G[i]);
	rep(i, 1, k) for(auto u : G[i]) in[u]++;
	...
	dp[0] = 1;
	rep(i, 0, m-1){
		for(auto p : mp) if(i+p.first <= m) dp[i+p.first] += dp[i] * mint(p.second);
	}
	outl(dp[m]);
