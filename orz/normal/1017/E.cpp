...
int main()
{
	...
	vpll e1(n), e2(m);
	cin >> e1 >> e2;
	auto h1 = convex_hull(e1), h2 = convex_hull(e2);
	vpll hh1(sz(h1)), hh2(sz(h2));
	for (int i = 0; i < sz(h1); ++i)
		hh1[i] = e1[h1[i]];
	for (int i = 0; i < sz(h2); ++i)
		hh2[i] = e2[h2[i]];
	bool answer = false;
	n = sz(hh1);
	m = sz(hh2);
	if (n == m)
	{
		...
		for (int i = 0; i < n; ++i)
		{
			v.eb(false, (hh1[i] - hh1[raz(i, 1, n)]) * (hh1[i] - hh1[sum(i, 1, n)]));
			v.eb(true, sq_dist(hh1[i], hh1[sum(i, 1, n)]));
		}
		for (int i = 0; i < n; ++i)
		{
			v.eb(false, (hh2[i] - hh2[raz(i, 1, n)]) * (hh2[i] - hh2[sum(i, 1, n)]));
			v.eb(true, sq_dist(hh2[i], hh2[sum(i, 1, n)]));
		}
		for (int i = 0; i < n; ++i)
		{
			v.eb(false, (hh2[i] - hh2[raz(i, 1, n)]) * (hh2[i] - hh2[sum(i, 1, n)]));
