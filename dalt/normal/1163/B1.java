...
public class CFContest1163 {
    ...
    public static class Task implements Runnable {
        ...
        public boolean test(TreeSet<Point> set) {
            if (set.size() <= 1) {
                return true;
            }
            boolean ret = false;
            Point first = set.pollFirst();
            Point last = set.pollLast();
            if (set.isEmpty()) {
                ret = first.cnt == 1 || first.cnt == last.cnt - 1;
            } else {
                if (first.cnt == 1 && last.cnt == set.first().cnt) {
                    ret = true;
                } else if (first.cnt == set.last().cnt && first.cnt == last.cnt - 1) {
                    ret = true;
                }
            }
            set.add(first);
    ...
    public static class Point implements Comparable<Point> {
        ...
        public int compareTo(Point o) {
            return cnt == o.cnt ? color - o.color : cnt - o.cnt;
        }
    }
