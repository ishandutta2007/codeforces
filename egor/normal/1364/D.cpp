...
public:
    void solve() {
        ...
        RecursiveFunction dfs = [&](const auto& self, int vert, int last, bool color) -> void {
            if (visited[vert]) {
                while (true) {
            ...
            if (white.size() + black.size() == k) {
                return;
            }
            stack.push_back(vert);
            if (color) {
                white.push_back(vert + 1);
            } else {
                black.push_back(vert + 1);
            }
            visited[vert] = true;
        ...
        dfs(0, -1, false);
        if (cycle.empty()) {
            vi answer = white.size() > black.size() ? white : black;
            answer.resize((k + 1) / 2);
            out.printLine(1);
...
int main() {
    ...
    auto time = clock();
#endif
    DPosledneeSledstvieYekhaba solver;
