...
void main()
{
	...
	for(int i=0;i<n;++i) {
		...
		while(sn)
		{
			if(st[sn].fi*g.se<=g.fi*st[sn].se)
				break;
			g.fi+=st[sn].fi;
			g.se+=st[sn].se;
			--sn;
	...
	for(int i=1;i<=sn;++i)
	{
		long double w=st[i].first/(long double)(st[i].second);
		cout.setf(ios::fixed);
		cout.precision(9);
