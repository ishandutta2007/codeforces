...
template <typename T>
inline T minElement(const vector<T>& v) {
    return *min_element(v.begin(), v.end());
}

template <typename T>
inline T maxElement(const vector<T>& v) {
...
template <typename T>
inline void fill(vector<T>& v, const T& val) {
...
public:
	void solve(std::istream& inp, std::ostream& outp) {
        ...
        vi max(n);
        vector<bool> newTeam(n);
        for (int i = 1; i < n; i++) {
            max[i] = max[i - 1];
            if (i >= 3 && max[i - 3] + delta[i - 1] > max[i]) {
                max[i] = max[i - 3] + delta[i - 1];
                newTeam[i] = true;
            }
        }
        int current = n - 3;
        int teamId = 1;
        vi answer(n);
        answer[id[n - 1].second] = 1;
        answer[id[n - 2].second] = 1;
        answer[id[n - 3].second] = 1;
        while (current > 0) {
            int next = current - 1;
            if (newTeam[current]) {
                next = current - 3;
            ...
            for (int i = next; i < current; i++) {
                answer[id[i].second] = teamId;
            }
            current = next;
        }
        out.printLine(a[n - 1] - a[0] - max[n - 3], teamId);
