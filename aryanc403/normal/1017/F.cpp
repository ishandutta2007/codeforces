...
const lli mod = 1000000007L;
    ...
    lli T,n,i,m,j,k,in,cnt,l,a,b,c,d;
    vi :: iterator it;
    //bool vis[100000002];
    bitset<100000002> vis;
...
lli solve(lli n)
{
    ...
    for(i=5;i<=n;i++)
    {
        ...
        if((i+1)%6&&(i-1)%6)
            continue;
        //cout<<"prime "<<i<<endl;
        if((i%6==5&&!vis.test((i+1)/3-1))||(i%6==1&&!vis.test((i-1)/3)))
        {
            //cout<<"prime "<<i<<endl;
            cnt+=val(i);
            for(j=i;j<=n;j+=i)
...
int main(void) {
    ...
    cout<<solve(n);
    //vis.set(11);
    //cout<<endl<<endl<<vis.test(11)<<endl;
    aryanc403();
    return 0;
}