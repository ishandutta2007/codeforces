...
main()
{
    ...
    in.getv(v, n);   
    int minIn2 = inf, maxIn2 = -inf, minIn1 = inf, maxIn1 = -inf;
    int cur = 0;
    for (int i = 0; i < n; i++) {
		if (v[i].second == 1) {
			minIn1 = min(minIn1, cur);
			maxIn1 = max(maxIn1, cur);
		} else {
			minIn2 = min(minIn2, cur);
			maxIn2 = max(maxIn2, cur);
		}
		cur += v[i].first;
	}
	if (minIn2 == inf) {
