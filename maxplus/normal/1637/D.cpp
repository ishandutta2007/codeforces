...
int main() {
  ...
  for (int testcase = $; testcase; --testcase) {
    ...
    int ts = accumulate(a, a + n, accumulate(b, b + n, 0));
    auto ssqr = [](auto a, auto b) { return a + b * b; };
    int tsq = accumulate(a, a + n, accumulate(b, b + n, 0, ssqr), ssqr) * (n - 2);
    dp[0][0] = 1;
    for (int i = 1; i <= n; ++i)
    for (int s = 0; s <= i * N; ++s) {
      dp[i][s] = (s >= a[i - 1] && dp[i - 1][s - a[i - 1]]) || (s >= b[i - 1] && dp[i - 1][s - b[i - 1]]);
    }
    for (int s = 0; s <= n * N; ++s) if (dp[n][s]) {
      mini(ans, s * s + (ts - s) * (ts - s));
    }
    print(ans + tsq);
  }
}