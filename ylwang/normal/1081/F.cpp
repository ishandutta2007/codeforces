...
int get(int l,int r) {
	...
	assert((t+res+(r-l+1)-n)%2==0); int ans=(t+res+(r-l+1)-n)/2; assert(ans<=2);
	do { res=ask(l,r),res=ask(l,r); } while(res!=t);
	return ans;
}
signed main() {
	n=read(),t=read();
	if(abs(t*2-n)==1) {
		For(i,1,n-1) a[i]=get(i,i+1);
		ans[1]=t; 
		for(int i=n-1;i>1;i-=2) ans[1]-=a[i];
		assert(ans[1]==0||ans[1]==1);
		For(i,2,n) ans[i]=a[i-1]-ans[i-1];
	} else {
		For(i,1,n) ans[i]=get(i,i);
	}
	cout<<"! ";
