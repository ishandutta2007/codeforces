...
void solve(){
    int n, m, d;
    cin >> n >> m >> d;
    vector<pair<int, int>> edge_back, edges, ans;
    REP(i, m) {
        int a, b;
        cin >> a >> b;
        a--; b--;
        edge_back.push_back(mp(a, b));
    }
    REP(i, n) {
        ut[i] = i;
    }
    REP(i, edge_back.size()) {
        if (edge_back[i].first == 0 || edge_back[i].second == 0) continue;
        um(edge_back[i].first, edge_back[i].second);
        edges.push_back(edge_back[i]);
    }
    REP(i, edge_back.size()) {
        if (edge_back[i].first == 0 || edge_back[i].second == 0) {
            if (um(edge_back[i].first, edge_back[i].second) == 1) {
                ans.push_back(edge_back[i]);
                d--;
            }
            else {
                edges.push_back(edge_back[i]);
    ...
    REP(i, ans.size()) {
        um(ans[i].first, ans[i].second);
    }
    REP(i, edges.size()) {
        if (edges[i].first == 0 || edges[i].second == 0) {
            if (d > 0) {
                if (um(edges[i].first, edges[i].second) == 1) {
                    d--;
    ...
    REP(i, edges.size()) {
        if (edges[i].first == 0 || edges[i].second == 0) continue;
        if (um(edges[i].first, edges[i].second) == 1) {
            ans.push_back(edges[i]);
        }
    }
