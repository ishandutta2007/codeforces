...
const int m=100002;
struct node{int x,v;}p[N];
int n,w,a[N],c[N],sum[N];
ll ans;
...
int main(){
	...
	scanf("%d%d",&n,&w);
	for(int i=1;i<=n;i++)
		scanf("%d%d",&p[i].x,&p[i].v),sum[p[i].x+m]++;
	for(int i=0;i<=m*2;i++)
		ans+=(ll)sum[i]*(sum[i]-1)/2;
	
	sort(p+1,p+n+1,[&](node a,node b){
		double aa=(ld)a.x/(a.v+w),bb=(ld)b.x/(b.v+w);
		if(fabs(aa-bb)>eps) return aa<bb;
		return a.x<b.x;
	});
	clr(c,0);
	for(int i=n;i;i--)
		if(p[i].x>0) ans+=get(p[i].x-1),add(p[i].x,1);
	
	clr(c,0);
	for(int i=n;i;i--)
		if(p[i].x<0) ans+=get(m+p[i].x-1),add(m+p[i].x,1);
	sort(p+1,p+n+1,[&](node a,node b){
		double aa=(ld)a.x/(a.v+w),bb=(ld)b.x/(b.v+w);
		if(fabs(aa-bb)>eps) return aa<bb;
		return a.x>b.x;
