...
void solve() {
    ...
    vvc<int> ldp(n + 1, vc<int>(m + 1, inf)), rdp(n + 1, vc<int>(m + 1, inf));
    {
        ldp[0][0] = 0;
        rep(i, n + 1) {
            rep(j, m + 1) {
                if(i < n and j < m and s[i] == t[j]) {
                    chmin(ldp[i + 1][j + 1], ldp[i][j] + 1);
                }
                if(i < n) {
                    chmin(ldp[i + 1][j], ldp[i][j] + 2);
                }
            }
        }
    }
    {
        ...
        rep(i, n + 1) {
            rep(j, m + 1) {
                if(i < n and j < m and s[n - 1 - i] == t[m - 1 - j]) {
                    chmin(rdp[i + 1][j + 1], rdp[i][j] + 1);
                }
                if(i < n) {
                    chmin(rdp[i + 1][j], rdp[i][j] + 1);
                }
            }
        }
    }
    int ans = inf;
    rep(i, n + 1) {
        string u = (i == n ? "" : s.substr(i, n - i));
        u += '$';
        u += t;
        auto zalgo = atcoder::z_algorithm(u);
        rep(j, m + 1) {
            int res = (i == 0 ? 0 : 1);
            int len = (j == m ? 0 : zalgo[n - i + 1 + j]);
            res += ldp[i][j] + rdp[n - i - len][m - j - len];
            chmin(ans, res);
        }
    }
    out(ans);
