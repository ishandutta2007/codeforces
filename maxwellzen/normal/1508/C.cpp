...
void solve() {
    ...
    remain.erase(0); add.push(0);
    DSU d(n);
    while (! (remain.empty() && add.empty() && q.empty())) {
        debug(remain); 
        int cur;
        if (! add.empty()) {
            cur = add.front();
            ...
            do {
                ...
                cost = q.top()[0];
                q.pop();
            } while (remain.find(cur)==remain.end() && !q.empty());
            if (remain.find(cur)==remain.end()) break;
            remain.erase(cur);
            d.unite(cur, nxt);
            ans += cost;
        }
        vi reach;
        for (int i : remain) if (! binary_search(all(adj2[cur]), i)) reach.pb(i);
        for (int i : reach) remain.erase(i), add.push(i);
        for (pi p : adj[cur]) if (remain.count(p.F)) q.push({p.S, p.F, cur});
    }
    if (use==zero) {
        vector<ai<3>> edges;
        F0R(i, n) for (pi p : adj[i]) if (p.F>i) edges.pb({p.S, i, p.F});
        sort(all(edges));
        edges.pb({1<<30,0,0});
        for (ai<3> e : edges) {
            if (e[0]>sum) {
