...
constexpr int NM = 300000;
ModInt dp[NM + 1]{}, p2[NM + 1];

int main() {
  // https://oeis.org/A045883
  dp[2] = 1; dp[3] = 3;
  FOR(i, 4, NM + 1) dp[i] = dp[i - 1] * 3 - dp[i - 3] * 4;
  p2[0] = 1;
  REP(i, NM) p2[i + 1] = p2[i] * 2;

  int n, m; cin >> n >> m;
  vector<string> board(n); REP(i, n) cin >> board[i];
  int white = 0;
  REP(i, n) white += count(ALL(board[i]), 'o');
  ModInt ans = 0;
  REP(i, n) for (int j = 0; j < m;) {
    if (board[i][j] == '*') {
      ++j;
      continue;
    }
    int k = j + 1;
    while (k < m && board[i][k] == 'o') ++k;
    ans += dp[k - j] * p2[white - (k - j)];
    j = k;
  }
  REP(j, m) for (int i = 0; i < n;) {
    if (board[i][j] == '*') {
      ++i;
      continue;
    }
    int k = i + 1;
    while (k < n && board[k][j] == 'o') ++k;
    ans += dp[k - i] * p2[white - (k - i)];
    i = k;
  }
  cout << ans << '\n';
  return 0;
