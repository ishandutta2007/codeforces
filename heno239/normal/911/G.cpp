...
void solve() {
	...
	int q; cin >> q;
	vector<int> le(q), ri(q), x(q), y(q);
	rep(i, q) {
		cin >> le[i] >> ri[i] >> x[i] >> y[i];
		le[i]--; ri[i]--; x[i]--; y[i]--;
	}
	using ar = array<int, 100>;
	ar e; rep(i, 100)e[i] = i;
	auto f = [&](ar a, ar b) {
		ar res;
		rep(i, 100)res[i] = b[a[i]];
		return res;
	};
	SegT<ar> st(q, e, f);
	vector<vector<int>> ad(n),del(n);
	rep(i, q) {
		ad[le[i]].push_back(i);
		if (ri[i] + 1 < n)del[ri[i] + 1].push_back(i);
	}
	vector<int> ans(n);
	rep(i, n) {
		for (int id : ad[i]) {
