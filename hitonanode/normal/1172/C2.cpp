...
int main()
{
    ...
    lint modinv_wtot_1 = mod_inverse(w_tot_1);
    lint modinv_wtot_0 = mod_inverse(w_tot_0);
    REP(i, M + 1)
    {
    ...
    lint expm1 = exp_1 * modinv_wtot_1 % MOD;
    lint expm0 = exp_0 * modinv_wtot_0 % MOD;
    REP(i, N)
    {
        lint ret = 0;
        if (like[i])
        {
            ret = (w[i] + w[i] * expm1) % MOD;
        }
        else
        {
            ret = (w[i] - w[i] * expm0 % MOD + MOD) % MOD;
        }
        printf("%lld\n", ret);
    }
}