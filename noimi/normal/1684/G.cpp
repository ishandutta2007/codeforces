...
int main() {
    {
        LL(n, m);
        VEC(ll, a, n);
        if(MAX(a) * 2 + 1 > m) { drop(-1); }
        int c = 0;
        SORT(a);
        auto r = runLength(a);
        atcoder::mf_graph<int> flow(si(r) * 2 + 2);
        int s = si(r) * 2, t = si(r) * 2 + 1;
        rep(i, si(r)) {
            auto &[x, y] = r[i];
            if(x * 3 <= m) {
                flow.add_edge(si(r) + i, t, y);
                continue;
            }
            flow.add_edge(s, i, y);
            rep(j, si(r)) {
                if(x > r[j].fi and x % r[j].fi == 0 and x * 2 + r[j].fi <= m) { flow.add_edge(i, si(r) + j, inf<int>); }
            }
            c += y;
        }
        if(flow.flow(s, t) == c) {
            vpi ans;
            fore(e, flow.edges()) {
                if(e.flow and e.from < si(r)) {
                    rep(e.flow) {
                        int x = r[e.from].fi;
                        int y = r[e.to - si(r)].fi;
                        ans.eb(x * 2 + y, x + y);
                    }
                }
                if(e.to == t) {
                    rep(e.cap - e.flow) {
                        int x = r[e.from - si(r)].fi;
                        ans.eb(x * 3, x * 2);
                    }
                }
            }
