...
int main(){
  ...
  for(li i = 0; i < (li)(n); ++i) x[i] += 1e7;
  for(li i = 0; i < (li)(n); ++i) y[i] += 1e7;

  li p2 = 1;
  while(1) {
    bitset<2> xs=0, ys=0;
    for(li i = 0; i < (li)(n); ++i) xs[(x[i]&p2)?1:0]=1;
    for(li i = 0; i < (li)(n); ++i) ys[(y[i]&p2)?1:0]=1;
    if(xs.count() == 2 || ys.count() == 2) break;
    p2 *= 2;
  }

  { vi A[2];
    for(li i = 0; i < (li)(n); ++i) {
      A[((x[i]^y[i])&p2)?1:0].push_back(i);
    }
    if(!A[0].empty() && !A[1].empty()) {
      cout << A[0].size() << endl;
      for(int a : A[0]) cout << a+1 << ' ';
      cout << endl;
      return 0;
    }
  }
  ...
  { vi A[2];
    for(li i = 0; i < (li)(n); ++i) {
      li a = (x[i]+y[i])&(p2*2);
      li b = (x[i]-y[i]+(1ll<<50))&(p2*2);
      A[(a^b)?1:0].push_back(i);
    }
    if(!A[0].empty() && !A[1].empty()) {
      cout << A[0].size() << endl;
      for(int a : A[0]) cout << a+1 << ' ';
