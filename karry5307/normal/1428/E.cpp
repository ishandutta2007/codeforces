...
// Functions
inline ll YLWangAKIOI(ll x,ll y)
{
...
int main()
{
	...
	{
		x[i]=read(),res+=x[i]*x[i],t[i]=2,cur=x[i]>>1;
		if(x[i]>1)
		{
			q.push(mp(x[i]*x[i]-cur*cur-(x[i]-cur)*(x[i]-cur),i));
		}
	}
	for(register int i=n+1;i<=kk;i++)
	{
		p=q.top(),u=p.F,v=p.S,q.pop(),res-=u,t[v]++;
		if(x[v]>=t[v])
		{
			q.push(mp(YLWangAKIOI(x[v],t[v]-1)-YLWangAKIOI(x[v],t[v]),v));
		}
	}
	printf("%lld\n",res);
}