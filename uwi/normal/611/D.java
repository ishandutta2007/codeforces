	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			...
			for(int j = i;j < n;j++){
				...
				// [j+1, j+(j-i+1)]
				if(j+(j-i+1) < n && s[j+1] != '0'){
					// same len
					int sm = isa[i], la = isa[j+1];
					if(sm < la){
						int lc = rmq(st, lcp, sm+1, la+1);
						if(lc < j-i+1){
							dp[j+1][j+(j-i+1)] += dp[i][j];
							if(dp[j+1][j+(j-i+1)] >= mod)dp[j+1][j+(j-i+1)] -= mod;
							continue outer;
						}
					}
					if(j+(j-i+2) < n){
						dp[j+1][j+(j-i+2)] += dp[i][j];
						if(dp[j+1][j+(j-i+2)] >= mod)dp[j+1][j+(j-i+2)] -= mod;
					}
				}
			}
			ret += dp[i][n-1];
