...
ll get(ll sum[505][505], ll sx, ll sy, ll tx, ll ty)
{
...
int main(void)
{
	...
	rep(y, 1, h) rep(x, 1, w){
		...
		if(c[x][y] == '.' && c[x+1][y] == '.') sum[x][y]++;
	}
	rep(y, 1, h) rep(x, 1, w){
		sum2[x][y] = sum2[x-1][y] + sum2[x][y-1] - sum2[x-1][y-1];
		if(c[x][y] == '.' && c[x][y+1] == '.') sum2[x][y]++;
	}
	...
	rep(i, 1, Q){
		cin >> sy >> sx >> ty >> tx;
		ll ans = get(sum, sx, sy, tx-1, ty) + get(sum2, sx, sy, tx, ty-1);
		cout << ans << "\n";
	}
