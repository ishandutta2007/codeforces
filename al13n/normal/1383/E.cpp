...
int main(int argc, char **argv){
  ...
  forv(q,A){
    D[q+1]=D[q];
    int p=0;
    while(sz(st) && st.back().X<=A[q]){
      (D[q+1]+=(D[q]-D[st.back().Y]+mod)*(st.back().X-p+1))%=mod;
      p=st.back().X+1;
      st.pop_back();
    }
    if(sz(st)>1) (D[q+1]+=(D[q]-D[st.back().Y]+mod)*(A[q]-p+1))%=mod;
    else if(q==0) (D[q+1]+=(D[q]+mod)*(A[q]-p+1))%=mod;
    else (D[q+1]+=(D[q]-D[0]+mod)*(A[q]-p+1))%=mod;
    st.pb(mp(A[q],q));
    //(ans+=(D[q+1]-D[q]+mod)*(cz+1))%=mod;
  }
  LL ans=(D.back()-1+mod)*(cz+1)%mod;
  dbg(A,cz,D);
  cout<<ans;
