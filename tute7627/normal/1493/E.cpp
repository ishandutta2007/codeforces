...
int main(){
  ...
  auto xo=[&](string x,string y){
    string z=x;
    rep(i,0,n)if(y[i]=='1')z[i]=(z[i]=='0'?'1':'0');
    return z;
  };
  //[0..x]xor
  auto score=[&](string x){
    if(x[n-1]=='0'&&(n==1||x[n-2]=='0'))return x; 
    if(x[n-1]=='1'&&(n==1||x[n-2]=='0'))return string(n-1,'0')+'1';
    if(x[n-1]=='0')return x.substr(0,n-1)+'1';
    if(x[n-1]=='1')return string(n,'0');
  };
  {
    string nl=l;
    vector<string>sv;
    ...
    if(nl==r){
      ...
      rep(i,0,sz)rep(j,i,sz){
        chmax(ret,xo(score(dec(sv[i])),score(sv[j])));
      }
      cout<<ret<<endl;
      return 0;
    }
