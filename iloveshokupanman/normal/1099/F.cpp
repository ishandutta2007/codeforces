...
segtree<ll, ad, e> cost(L), num(L);
ll len = 0;

void dfs(ll v) {
    ll s = cost.get(t[v]);
    cost.set(t[v], s + x[v]*t[v]);
    ...
    ll now = cost.max_right<ch>(0);
    ll rem = G - cost.prod(0, now);

    eat[v] = num.prod(0, now) + rem / now;
    eat[v] = min(eat[v], num.all_prod());
...
ll dfs2(ll v) {
    vll lis(2, -1);
    for (ll w : g[v]) {
        lis.push_back(dfs2(w));
    }

    sort(lis.begin(), lis.end(), greater<ll>());
...
int main() {
    ...
    x.resize(N), t.resize(N), l.resize(N);
    rep(i, N)cin >> x[i];
    rep(i, N)cin >> t[i];
    rep(i, N - 1) {
        ll k; cin >> k;
