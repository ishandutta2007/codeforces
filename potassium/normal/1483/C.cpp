...
int main() {
	...
	vector<Block> stk;
	for (int i = 0; i < n; i++) {
		ll best = (i == 0 ? 0 : dp[i - 1]);
		while (!stk.empty() && h[i] < stk.back().h) {
			best = max(best, stk.back().val);
			stk.pop_back();
		}

		dp[i] = best + b[i];
		if (!stk.empty())
			dp[i] = max(dp[i], stk.back().pre);

		stk.push_back({ h[i], best, stk.empty() ? dp[i] : max(dp[i], stk.back().pre) });
	}
