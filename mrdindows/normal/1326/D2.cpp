...
int solve() {
    ...
    while (tc--) {
        ...
        string suf;
        int ptr = 0;
        while (ptr * 2 + 2 <= s.size() && s.size() > 1 && s[ptr] == s[s.size() - 1 - ptr]) ++ptr;
        pref = s.substr(0, ptr);
        suf = s.substr(s.size() - ptr);
        s = s.substr(ptr, s.size() - ptr * 2);
        int n = (int)s.size();
        vector<int> d1 (n);
        int l=0, r=-1;
        for (int i=0; i<n; ++i) {
...
//        DBN(d1, d2);
        ...
        FI(n) {
            int k = d1[i];
            dp[i + k - 1] = max(dp[i + k - 1], k * 2 - 1);
        }
        FI(n) {
            int k = d2[i];
            dp[i + k - 1] = max(dp[i + k - 1], k * 2);
        }
//        printarray(dp, n);
        string ans;
        int cur = dp[s.size() - 1];
        if (cur > ans.size()) {
            ans = s.substr(s.size() - cur);
        }
        FI(n) dp[i] = 0;
        FI(n) {
            int k = d1[i];
