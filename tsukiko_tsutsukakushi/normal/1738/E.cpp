...
void solve() {
    INT(n);
    VEC(ll, a, n);
    vc<ll> lsum(n + 1, 0), rsum(n + 1, 0);
    ll all = 0LL;
    // map<ll, int> lmp, rmp;
    rep(i, n) {
        all += a[i];
        lsum[i + 1] = lsum[i] + a[i];
        rsum[i + 1] = rsum[i] + a[n - 1 - i];
    }
    vc<int> lz(n, 0), rz(n, 0);
    {
        int cur = 0;
        for(int i = n - 1; i >= 0; -- i) {
            if(a[i] == 0) {
    ...
    mint ans = 1;
    while(l <= r) {
        if(lsum[l] < all - lsum[r]) {
            ++ l;
        } else if(lsum[l] > all - lsum[r]) {
            -- r;
        } else {
            if(r - l <= lz[l]) {
                int len = r - l;
                ...
                int lcnt = lz[l] + 1, rcnt = rz[r - 1] + 1;
                debug(lcnt, rcnt);
                mint res = 0;
                for(int i = 0; i <= min(lcnt, rcnt); ++ i) {
                    res += bc.com(lcnt, i) * bc.com(rcnt, i);
                }
                ans *= res;
                l += lz[l] + 1;
                r -= (rz[r - 1] + 1);
