...
int gauss(vector < vector<int> > a, vector<int>& ans) {
	...
	for (int col = 0, row = 0; col < m && row < n; ++col) {
		...
		for (int i = 0; i < n; ++i)
			if (i != row) {
				int c = prod(a[i][col], inverse(a[row][col], M), M);
				for (int j = col; j <= m; ++j)
					a[i][j] = raz(a[i][j], prod(a[row][j], c, M), M);
			}
		++row;
	}
	...
	for (int i = 0; i < m; ++i)
		if (where[i] != -1)
			ans[i] = prod(a[where[i]][m], inverse(a[where[i]][i], M), M);
...
void dfs(const int v, vi& ans, const vvi& g)
{
	unordered_set<int> u;
	for (int to : g[v])
	{
...
int another_main()
{
	...
	for (int i = 0; i < u; ++i)
	{
		matrix[i][i] = raz(0, n + 1, M);
		for (int j = 0; j < u; ++j)
			matrix[i][i ^ j] = sum(matrix[i][i ^ j], p[j], M);
		if (i)
			matrix[i].back() = M - 1;
	}
	vi answer;
