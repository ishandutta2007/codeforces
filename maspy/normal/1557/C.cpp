...
void solve() {
  LL(N, K);
  vc<mint> POW2(N + K + 10, 1);
  FOR(i, N + K + 5) POW2[i + 1] = POW2[i] * 2;

  // 
  mint EQ = 1, LARGE = 0, SMALL = 0;
  FOR(_, K) {
    mint eq = 0, large = 0, small = 0;
    // (1, 1)
    if (N % 2 == 1) eq += 1;
    // (1, 0)
    if (N % 2 == 0) large += 1;
    ...
    // (0, 0)
    eq += (N % 2 == 0 ? c - 1 : c);
    mint total = eq + large + small;
    LARGE = LARGE * total + EQ * large;
    SMALL = SMALL * total + EQ * small;
    EQ = EQ * eq;
  }
  print(LARGE + EQ);
}
