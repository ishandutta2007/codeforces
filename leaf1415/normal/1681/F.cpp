...
struct UndoUnionFind{
	...
	void init(){
		...
		for(int i = 0; i <= size; i++) parent[i] = i, rank[i] = 0, v[i] = 1;
	}
	int root(int i){
...
struct DynamicConnectivity{
	...
	void dfs(int k, int l, int r){
		...
		for(int i = 0; i < seg[k].size(); i++){
			if(uf.unite(seg[k][i].first, seg[k][i].second)) cnt++;
		}
		if(l == r){
			int t = k - (1<<b);
			if(t <= T){
				for(int i = 0; i < qvec[t].size(); i++){
					...
					avec[qid] = uf.v[uf.root(u)] * uf.v[uf.root(v)];
				}
			}
		}
		else{
...
int main(void)
{
	...
	cin >> n;
	DynamicConnectivity dc(n, 4*(n-1));
	ll u, v, w;
	rep(i, 1, n-1){
		cin >> u >> v >> w;
		evec[w].pb(P(u, v));
	...
	ll id = n-1;
	rep(i, 1, n){
		for(auto p : evec[i]) dc.erase(++id, p.fi, p.se);
		for(auto p : evec[i]) dc.check(++id, p.fi, p.se);
		for(auto p : evec[i]) dc.add(++id, p.fi, p.se);
	}
	dc.calc();
	
	ll ans = 0;
	for(auto x : dc.avec) ans += x;
	outl(ans);
