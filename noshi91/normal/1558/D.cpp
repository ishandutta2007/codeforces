...
public:
  ...
  T perm(const usize n, const usize r) const {
...
usize op(usize x, usize y) { return x + y; }
usize id() { return 0; }

using mint = atcoder::modint998244353;

void main_() {
  const usize tnum = scan<usize>();

  const usize N_MAX = OJ_LOCAL(200000, 100);

  atcoder::segtree<usize, op, id> seg(N_MAX);
  for (const usize i : rep(0, N_MAX)) {
    seg.set(i, 1);
  }

  const mint_utils<mint> util(N_MAX * 2 + 100);

  for (const usize testcases : rep(0, tnum)) {
    const usize n = scan<usize>();
    const usize m = scan<usize>();

    std::vector<std::pair<usize, usize>> xy(m);
    for (auto &[x, y] : xy) {
      std::cin >> x >> y;
      x -= 1;
      y -= 1;
    ...
    for (auto &[x, y] : xy) {
      y = seg.max_right(0, [y = y](usize t) { return t <= y; });
      seg.set(y, 0);
      if (seg.get(y + 1) == 1) {
        leq_cnt += 1;
      }
    ...
    std::cout << util.homo(n - leq_cnt, n).val() << eoln;
