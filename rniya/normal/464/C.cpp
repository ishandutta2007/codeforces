...
int main() {
    ...
    vector<vector<mint2>> sz(n + 1, vector<mint2>(10, 0));
    for (int j = 0; j < 10; j++) dp[n][j] = j, sz[n][j] = 1;
    for (int i = n - 1; i >= 0; i--) {
        int start = T[i][0] - '0';
        string to = T[i].substr(3);
        for (int j = 0; j < 10; j++) {
            if (j == start) continue;
            dp[i][j] = dp[i + 1][j];
            sz[i][j] = sz[i + 1][j];
        ...
        for (int j = to.size() - 1, k = 0; j >= 0; j--, k++) {
            int x = to[j] - '0';
            dp[i][start] += dp[i + 1][x] * mint(10).pow(sz[i][start].value());
            sz[i][start] += sz[i + 1][x];
        }
    }
    ...
    for (char& c : S) {
        int x = c - '0';
        ans += dp[0][x] * mint(10).pow(cur.value());
        cur += sz[0][x];
    }
    cout << ans << '\n';
    return 0;
