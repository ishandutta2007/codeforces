...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n)rep(j,m-1){
		cauto v=read();
		g[fun(i,j)].eb(fun(i,j+1),v);
		g[fun(i,j+1)].eb(fun(i,j),v);
	}
	rep(i,n-1)rep(j,m){
		cauto v=read();
		g[fun(i,j)].eb(fun(i+1,j),v);
		g[fun(i+1,j)].eb(fun(i,j),v);
	}
	if(t%2){
	...
	fill(dp[1],dp[12],INF);
	rep(i,t/2){
		rep(v,n*m)for(cauto &[u,w]:g[v]) MN(dp[i+1][v],dp[i][u]+w);
	}
	rep(i,n)rep(j,m) cout<<dp[t/2][fun(i,j)]*2<<" \n"[j+1==m];
  return 0;
}