...
void Enumerate(){
	...
	For(i,100){
		...
		int b = 0;
		Forx(bi,a+1,131072)
			if( !st.count(bi) and !st.count(a^bi) ){
				b = bi;
				break;
			}
		st.insert(a);
		...
		// P(a,b,a^b);
		// constexpr int bit = 8;
		// if( a >= (1<<bit) or b >= (1<<bit) or (a^b) >= (1<<bit) ) break;
		// P(bitset<bit>(a),bitset<bit>(b),bitset<bit>(a^b));
		res[++p] = a;
		res[++p] = b;
		res[++p] = a^b;
	}
...
ll Core( vc<ll> ts , ll n ){
	ll start_10 = 1;
	ll ans = 0;
	Forstep(bitW,0,233,2){
		ll start_r = start_10 + (1LL<<bitW);
		// P(bitW,start_10,start_r);
		...
		ll len = (1LL<<bitW)*4;
		ll dId = ((n-start_r+1)-1) % len / (1LL<<bitW);
		ans |= ts[dId] << bitW;
...
ll GetAAns( ll n ){
	return Core({0,1,2,3},n);
}
ll GetBAns( ll n ){
	return Core({0,2,3,1},n);
}
...
int main(){
	...
	For(TwT,T){
		...
		// n = TwT;
		int t = n%3;
		ll aAns = GetAAns(t?n/3+1:n/3);
		ll bAns = GetBAns(t?n/3+1:n/3);
		ll ans = vc<ll>{aAns^bAns,aAns,bAns}[t];
		printf("%lld\n",ans);
		// P(n,ans,res[n]);
		// assert( ans == res[n] );
	}
