...
public:
    ...
    vec find_shortest_path(int s, vec dist){
        vec prev(N, -1);
        REP(v, N){
            for(auto e: G[v]) if(dist[e.to] == dist[v] - 1){
                if(prev[v] >= 0) prev[v] = -2;
                else if(prev[e.to] == -2) prev[v] = -2;
                else prev[v] = e.to;
            }
        }
        return prev;
...
signed main(){
    ...
    mmat ans(n, mvec(n, 0));
    REP(i, n){
        vec prev = G.find_shortest_path(i, dist[i]);
        //debug(i); vdebug(prev);
        REP(j, i + 1) if(prev[j] != -2){
            v_bool used(n, false);
            int now = j;
            while(now >= 0){
                used[now] = true;
            ...
            REP(k, n) if(!used[k]){
                int cnt = 0;
                for(auto e: G.G[k]){
                    if(dist[i][e.to] == dist[i][k] - 1 && dist[j][e.to] == dist[j][k] - 1) cnt++;
                }
                x *= cnt;
            }
