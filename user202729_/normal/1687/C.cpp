...
void up(){
	...
	int const numSegment=read<int>();
	
	std::set<int> remain(int_iter<int>(0), int_iter<int>(len+1)); // ugly ><
	std::vector<int64_t> a(len);
	for(auto& it: a) {
		std::cin>>it;
	}
	std::vector<int64_t> b(len);
	for(auto& it: b) {
		std::cin>>it;
	}

	struct Segment{int left, right, remainEnd=2;};
	std::vector<Segment> segments(numSegment);
	std::vector<std::vector<std::vector<Segment>::iterator>> segments_contain(len+1);
	for(int const index: range(0, int(segments.size()))){
		auto& [l, r, _]=segments[index];
		std::cin>>l>>r;
		--l;
		segments_contain[l].push_back(segments.begin()+index);
		segments_contain[r].push_back(segments.begin()+index);
	}

	
	std::partial_sum(begin(a), end(a), begin(a));
	std::partial_sum(begin(b), end(b), begin(b));

	std::vector<Segment> done_segments;
	auto const addPoint=[&](std::set<int>::iterator pointIter){
		auto const point=*pointIter;
		auto nextIter=remain.erase(pointIter);
		for(auto iter: segments_contain[point]){
			--iter->remainEnd;
			assert(iter->remainEnd>=0);
			if(iter->remainEnd==0){
				done_segments.push_back(*iter);
			}
		}
		return nextIter;
	};

	addPoint(remain.find(0));
	for(int const i: range(0, len)){
		if(a[i]==b[i]) addPoint(remain.find(i+1));
	}
	a.clear(); b.clear();

	while(not done_segments.empty()){
		auto const [l, r, _]=done_segments.back(); done_segments.pop_back();
		auto iter=remain.lower_bound(l);
		assert(iter==remain.end() or *iter>l);
		while(true){
			if(iter==remain.end() or *iter>r) break;
			assert(*iter<r);
			iter=addPoint(iter);
		}
	}

	std::cout<<(remain.empty() ? "YES\n": "NO\n");
