...
void solve(){
    ...
    VEC(ll,a,n);
    Sort(a);
    if(sum(a)+a[0]*(n-2)>0||sum(a)+a.back()*(n-2)<0)return(void)out("INF");
    ll l = -1e15, r = 1e15;
    auto f = [&](i128 t) -> i128 {
        i128 ans=a[0]*a.back()+t*(a[0]+a.back());
        rep(i,1,n-1)ans+=min(a[0]*a[i]+t*(a[0]+a[i]),a.back()*a[i]+t*(a.back()+a[i]));
        return ans;
    };
    while(abs(l-r)>=6){
        const ll l2=(l+l+r)/3,r2=(r+r+l)/3;
        if(f(l2)<f(r2))l=l2;
        else r=r2;
    }
    ll ans=-LINF;
    rep(i,l,r+1)chmax(ans,f(i));
    out(ans);
}
int main(){
    LL(t);
