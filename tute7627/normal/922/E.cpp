...
int main(){
  ...
  rep(i,0,n)cin>>c[i];
  rep(i,0,n)cin>>v[i];
  ll lim=10005;//cout<<"lim"<<endl;
  auto dp=make_v(n+1,lim,-1LL);
  dp[0][0]=w;
  rep(i,0,n){
    SWAG<ll>swag;
    vector<ll>tmp(lim,-1);
    rep(j,0,lim){
      swag.push(dp[i][j]+j*v[i]);
      chmax(tmp[j],swag.query()-j*v[i]);
      if(j>=c[i])swag.pop();
    }
    //debug(tmp,lim);
    rep(j,0,lim)if(tmp[j]!=-1)dp[i+1][j]=min(w+j*b,tmp[j]+x);
  }
  //debug(dp,n+1,lim);
  rep(i,0,lim)if(dp[n][i]!=-1)res=i;
  cout<<res<<endl;
  return 0;
}