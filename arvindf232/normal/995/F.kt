...
fun interpolate(X:IntArray, Y:IntArray, newpoint:Int):Int{
    // N^2 version
    kotlin.assert(X.size == Y.size)
//    val find = X.indexOf(newpoint)
//    if(find != -1){
//        return Y[find]
//    }
    var ans = 0
    ...
    for (i in 0 until n) {
        ...
        for (j in 0 until n) {
            if (i != j) {
                num = num modM (newpoint modMinus X[j])
                den = den modM (X[i] modMinus X[j])
            }
        }
        ans = (ans modPlus (num modM den.inverse()))
    }
    return ans
}
...
fun main(){
    solve.cases{
        ...
        G.setup()
        val ret = Array(n){IntArray(max){1} }
        G.DFS { v->
            for(w in G.subs[v]){
                for(a in 0 until max){
                    ret[v][a] = ret[v][a] modM ret[w][a]
                }
            }
            for(a in 1 until max){
                ret[v].put(a,ret[v][a-1])
            }
        }
        put(interpolate(IntArray(max){it},ret[0],D-1))
//        put(ret[0][D-1])
    }
    done()
}
