...
void solve() {
    ...
    REP(i, n) {
        ...
        next.push_back(i);
        while (true) {
            int next_go = p[next.back()];
            if (dp[next_go] != 0) {
                int stock = dp[next.back()] - dp[next_go] + 1;
                reverse(ALL(next));
                while (stock != next.size()) {
                    next.pop_back();
                }
                break;
            }
            dp[next_go] = dp[next.back()] + 1;
            next.push_back(next_go);
        }
        auto judge = [c,next](int a) {
            REP(i, a) {
                int ok = 1;
                for (int q = i; q < next.size(); q += a) {
                    if (c[next[q]] != c[next[i]]) {
                        ok = 0;
                        break;
                    }
                }
        ...
        for (int q = 1; q * q <= next.size(); ++q) {
            if (next.size() % q == 0) {
                if (judge(q) == true) {
                    ans = min(ans, q);
                }
                if (judge(next.size() / q) == true) {
                    ans = min(ans, next.size() / q);
                }
            }
        }
    }
