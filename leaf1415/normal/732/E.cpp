...
int main(void)
{
	...
	set<P> S;
	rep(i, 1, n) S.insert(P(p[i], i));

	typedef pair<P, ll> T;
	priority_queue<T> Q;
	rep(i, 1, m) Q.push(T(P(s[i], 0), i));

	ll ans = 0;
	while(Q.size()){
		ll x = Q.top().first.first, id = Q.top().second;
		Q.pop();
		auto it = S.lower_bound(P(x, -inf));
		...
		if(x == 1) continue;
		a[id]++;
		Q.push(T(P(ceil(x, 2), -a[id]), id));
	}

	ll sum = 0;
	rep(i, 1, m) if(!used[i]) a[i] = 0;
	rep(i, 1, m) sum += a[i];
