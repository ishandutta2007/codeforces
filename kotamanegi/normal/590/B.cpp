...
void solve(){
	...
	ld v, t;
	cin >> v >> t;
	pair<ld, ld> first_wind, second_wind;
	cin >> first_wind.first >> first_wind.second;
	cin >> second_wind.first >> second_wind.second;
	ld bot = 1e18;
	ld top = 0;
	REP(q, 200) {
		ld mid = (bot + top) / 2.0L;
		ld expected_x = x;
		ld expected_y = y;
		ld first_think = min(mid, t);
		ld second_think = max(0.0L, mid - t);
		expected_x -= first_think * first_wind.first;
		expected_y -= first_think * first_wind.second;
		expected_x -= second_think * second_wind.first;
		expected_y -= second_think * second_wind.second;
		expected_x /= mid;
		expected_y /= mid;
		if (pow(expected_x, 2.0L) + pow(expected_y, 2.0L) <= v * v) {
			bot = mid;
		}
		else {
			top = mid;
