...
#line 1 "/home/maspy/compro/library/geo/base.hpp"
struct Point {
  ll x, y;
...
void solve() {
  ...
  FOR(v, N) {
    UnionFind uf(N);
    FOR(a, N) FOR(b, N) if (a != v && b != v) {
      Point va = Point(XY[a]) - Point(XY[v]);
      Point vb = Point(XY[b]) - Point(XY[v]);
      if (va.det(vb) == 0) uf.merge(a, b);
    }
    GRP[v] = uf.find_all();
  }
  vv(Re, A, N, N);
  FOR(v, N) {
    vc<vi> grp(N);
    FOR(i, N) if (v != i) grp[GRP[v][i]].eb(i);
    ll n_grp = 0;
    for (auto&& S: grp) n_grp += len(S) > 0;
    for (auto&& S: grp)
      if (len(S)) {
        Re p = 1.0 / n_grp;
        p /= (len(S) + 1);
        for (auto&& to: S) A[to][v] += p;
        A[v][v] += p;
      }
  }
  ll LIM = 32;
  vvv(Re, P0, LIM + 1, N, N);
  vvv(Re, P1, LIM + 1, N, N);
  FOR(v, N) P0[0][v][v] = P1[0][v][v] = 1;
  P0[1] = A;
  FOR3(i, 1, LIM) P0[i + 1] = mat_mul(P0[i], P0[1]);
  P1[1] = P0[LIM];
  FOR3(i, 1, LIM) P1[i + 1] = mat_mul(P1[i], P1[1]);

  auto f = [&](vc<Re> v, auto A) -> vc<Re> {
    vc<Re> w(N);
    FOR(i, N) FOR(j, N) w[j] += v[i] * A[i][j];
    return w;
  };
  auto g = [&](vc<Re> vals) -> Re {
    Re ANS = 0.0;
    FOR(v, N) {
      vc<vi> grp(N);
      FOR(i, N) if (v != i) grp[GRP[v][i]].eb(i);
