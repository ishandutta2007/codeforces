...
int main()
{
    rin>>x>>y;if(x>y||((x&1)!=(y&1))){puts("-1");return 0;}if(!x&&!y){puts("0");return 0;}if(x==y){printf("1\n%lld\n",x);return 0;}
    a=x;y-=a;for(int i=62;i>0;i--)if(y&(1LL<<i)){b+=(1LL<<i-1);if(a&(1LL<<i-1))c+=(1LL<<i-1);else a+=(1LL<<i-1);}
    if(c)printf("3\n%lld %lld %lld\n",a,b,c);else printf("2\n%lld %lld\n",a,b);
    return 0;
}