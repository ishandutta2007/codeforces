...
int32_t main() {
    ...
    for (int i = 0; i < n; ++i) {
        scanf("%d", &a[i]);
        rmq_min[0][i] = rmq_max[0][i] = a[i]; next[i] = i + 1;
        jump[i] = min(i + jump_small, n); times[i] = jump[i] - i;
        //cout << times[i] << " !\n";
    }
    for (int i = 0; i < n - 1; ++i) {
        now_min[i] = min(a[i], a[i + 1]); now_max[i] = max(a[i], a[i + 1]);
        heap.push({now_min[i] - now_max[i], i});
    }
    for (int i = 1; i < log_n; ++i) {
        for (int j = 0; j + (1 << i) <= n; ++j) {
            rmq_min[i][j] = min(rmq_min[i - 1][j], rmq_min[i - 1][j + (1 << i - 1)]);
            rmq_max[i][j] = max(rmq_max[i - 1][j], rmq_max[i - 1][j + (1 << i - 1)]);
            //cout << rmq_min[i][j] << " " << rmq_max[i][j] << " !!\n";
        }
    }

    vector<int> last(n); vector<bool> visit(n);
    auto update = [&](int x, int limit) {
        ...
        if (next[x] < n && next[x] - x <= jump_small)
            heap.push({now_min[x] - now_max[x], x});
        vector<int> stack; jump[x] = x; times[x] = 0;
        stack.push_back(jump[x]);
        while (jump[x] < n && next[jump[x]] - x <= jump_small) {
            jump[x] = next[jump[x]]; ++times[x];
            stack.push_back(jump[x]);
        }
        last[x] = stack.size();
        visit[x] = true;
        for (int i = x - 1; i >= x - jump_small && ~i; --i) {
            if (next[i] < n && visit[next[i]]) {
                while (stack.back() - i > jump_small)stack.pop_back();
                jump[i] = stack.back();
    ...
    vector<pair<int, int>> queries(q); vector<int> answer(q);
    for (int i = 0; i < q; ++i) {
    ...
    for (auto p : queries) answer[p.second] = query(p.first);
    for (int i = 0; i < q; ++i)
        printf("%d\n", answer[i]);
    return 0;
}