...
int x, k, y;
ll cost(const vi &in, int l, int r) {
	if (in.empty()) return 0;
	int top = *max_element(ALL(in));
	ll ans = numeric_limits<ll>::max();
	int len = in.size();
	if (top < max(l, r)) mini(ans, len * 1ll * y);
	if (len >= k) {
		mini(ans, (len - k) * 1ll * y + x);
		int rem = len % k;
		mini(ans, rem * 1ll * y + (len / k) * 1ll * x);
	}
	debug(imie(in), imie(l), imie(r), imie(ans));
	if (ans == numeric_limits<ll>::max()) err();
	rans;
}
int main() {
	int n, m;
	...
	ll ans = 0;
	int p = -1, q = 0;
	auto at = [&](int x) {if (x < 0 || x >= n) return -1; return a[x];};
	REP(i, n) {
		if (a[i] == b[q]) {
			debug(arr(a, i), arr(b, q));
			ans += cost(vi(a.begin() + (p + 1), a.begin() + i), at(p), at(i));
			p = i;
			q++;
			if (q == m) break;
		}
	...
	if (q != m) err();
	ans += cost(vi(a.begin() + p + 1, a.end()), at(p), -1);
	printf("%lld\n", ans);
	exit(0);
}
//#STAY AT HOME