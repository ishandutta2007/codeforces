...
int main(int argc, char **argv){
  ...
  if(argc>1){ cout<<"100000 49000\n";forn(i,49000) cout<<"49000\n";return 0; }
  
  int n,m;
  cin>>n>>m;
  vector<int> A(m),P(m),B(m+1);
  LL s=0;
  forn(i,m){
    cin>>A[i];
    P[i]=i;
  ...
  B[m]=0;
  for(int i=m-1;i>=0;--i) B[i]=max(B[i+1],i+A[i]);
  if(s<n || B[0]>n){
    cout<<-1;
    return 0;
  }
  ...
  for(int i=0;i<m;++i){
    if(!f){
      int t=min(n-add-B[i],i?(A[i-1]-1):0);
      if(t<0) while(true);
      add+=t;
      if(add+B[i]==n) f=true;
    }
    P[i]+=add;
    assert(P[i]+A[i]<=n);
    if(P[i]+A[i]==n) f2=true;
    if(i) assert(P[i]>P[i-1]); else assert(P[i]==0);
  }

  assert(f2);
  if(!f) while(true){volatile void*volatile p=malloc(1<<25);}
  forn(i,m) cout<<P[i]+1<<' ';
