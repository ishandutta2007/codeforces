...
int main() {
    ...
    for (int i = 0, j = 0; i < n; i++) {
        int pos = 100 * i;
        while (j < m and x[j] <= pos) j++;
        int d = INF;
        if (j - 1 >= 0) chmin(d, pos - x[j - 1]);
        if (j < m) chmin(d, x[j] - pos);
        if (d == 0) continue;
        ll l = pos - d, r = pos + d;
        l = 10LL * l + 1;
        r = 10LL * r;
        ranges.emplace_back(l, r, p[i]);
        v.emplace_back(l);
        v.emplace_back(r);
    }
    ...
    mkuni(v);
    int sz = v.size();
    atcoder::lazy_segtree<ll, op, e, ll, mapping, composition, id> seg(sz);
    for (auto& t : ranges) {
        ll l, r;
        int val;
        tie(l, r, val) = t;
