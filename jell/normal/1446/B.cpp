#line 1 "codeforces-workspace\\contest\\1446\\b\\b.cpp"
/*
 * @file template.cpp
 * @brief Template
 */
...
#line 13 "codeforces-workspace\\contest\\1446\\b\\b.cpp"
...
void main() {
  ...
  string a, b;
  cin >> a >> b;
  auto dp = make_vector<int>({1 + n, 1 + m});
  int ans = 0;
  for (auto i = 0; i <= n; ++i) {
    for (auto j = 0; j <= m; ++j) {
      if (i < n) chge(dp[i + 1][j], dp[i][j] - 1);
      if (j < m) chge(dp[i][j + 1], dp[i][j] - 1);
      if (i < n && j < m && a[i] == b[j]) chge(dp[i + 1][j + 1], dp[i][j] + 2);
      chge(ans, dp[i][j]);
    }
  }
  cout << ans << eol;
