...
int main() {
    ...
    {
        auto dfs = [&](auto self, int v, mint D) -> void {
            ans[v] = dp[v] * D;
            int sz = G[v].size();
            vector<mint> ch(sz);
            for (int i = 0; i < sz; i++) ch[i] = dp[G[v][i]] + 1;
            vector<mint> left(sz + 1, 1), right(sz + 1, 1);
            for (int i = 0; i < sz; i++) left[i + 1] = left[i] * ch[i];
            for (int i = sz - 1; i >= 0; i--) right[i] = right[i + 1] * ch[i];
            for (int i = 0; i < sz; i++) {
                int u = G[v][i];
                mint nD = D * left[i] * right[i + 1] + 1;
                self(self, u, nD);
            }
        };
        dfs(dfs, 0, 1);
