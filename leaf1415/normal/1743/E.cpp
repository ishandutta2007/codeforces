...
int main(void)
{
	...
	rep(i, 1, h){
		ll ub = ceil(h, p1-s)*t1, lb = 0, mid;
		while(ub-lb>1){
			mid = (ub+lb)/2;
			if(mid/t1*(p1-s) + mid/t2*(p2-s) >= i) ub = mid;
			else lb = mid;
		}
		need[i] = ub;
	}
	...
	rep(i, 1, h){
		dm1[i] = i*(p1-s) + t1*i/t2*(p2-s);
		if(t1*i/t2 >= 1) dm1[i] += s;
		dm2[i] = i*(p2-s) + t2*i/t1*(p1-s);
		if(t2*i/t1 >= 1) dm2[i] += s;
	}

	ll ans = inf;
	rep(i, 1, h) dp[i] = inf;
	rep(i, 0, h){
		chmin(ans, dp[i]+need[h-i]);
		rep(j, 1, h){
			chmin(dp[min(h, i+dm1[j])], dp[i] + j*t1);
			chmin(dp[min(h, i+dm2[j])], dp[i] + j*t2);
		}
	}
	outl(ans);
