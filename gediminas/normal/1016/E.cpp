...
struct Atkarpa {
	...
	bool j = true;
	ll sum;
	Atkarpa(ll fr, ll to, bool j, ll sum): fr(fr), to(to), j(j), sum(sum) {
...
int main () {
	...
	{
		ll la = -10000000100000ll;
		ll suma = 0;

		for (int i = 0; i < n; i++) {
			ll fr, to;
			cin >> fr >> to;
			atk.emplace_back(la, fr, false, suma);
			suma += to - fr;
			atk.emplace_back(fr, to, true, suma);
			la = to;
		}

		atk.emplace_back(la, 10000000100000ll, false, suma);
	}

	// for (auto && x : atk) {
	// 	debug(x.fr, x.to, x.j, x.sum);
	// }

	auto kuris = [&](ld p)->int {
		// debug(p);
		int mi = 0, ma = atk.size();
		ma--;

		while (mi <= ma) {
			int mid = (mi + ma) / 2;

			if (atk[mid].fr <= p + 0.0000001 and p <= atk[mid].to + 0.0000001) {
				return mid;
			}
			else if (p < atk[mid].fr) {
				ma = mid - 1;
			}
			else {
				mi = mid + 1;
	...
	auto suma = [&](ld fr, ld to)->ld {
		...
		if (fri == toi) {
			...
			ld sum = atk[toi - 1].sum - atk[fri].sum;

			if (atk[fri].j) {
				if (atk[toi].j) {
					return sum + to - atk[toi].fr + atk[fri].to - fr;
				}
				else {
					return sum + atk[fri].to - fr;
				}
			}
			else {
				if (atk[toi].j) {
					return sum + to - atk[toi].fr;
				}
				else {
					return sum;
				}
	...
	for (int i = 0; i < q; i++) {
		...
		ld pa = (a - x) / (sy + y) * y + x;
		ld pb = (b - x) / (sy + y) * y + x;
		cout << fixed << setprecision(8) << suma(pa, pb) / y * (sy + y) << "\n";
	}
