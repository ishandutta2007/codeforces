...
fn solve<R: Read, W: Write>(mut sc: Scanner<R>, mut w: BufWriter<W>) {
    ...
    for _ in 0..t {
        let n: usize = sc.read();
        let a: Vec<u32> = sc.read_vec(n);
        let m: usize = sc.read();
        let ps: Vec<(u32, u32)> = sc.read_vec(m);
        let mut max_power = vec![0; n + 1];
        for (p, s) in ps {
            max_power[s as usize].set_max(p);
        }
        for i in (0..n).rev() {
            let next = max_power[i + 1];
            max_power[i].set_max(next);
        }
        if *a.iter().max().unwrap() > max_power[0] {
            writeln!(w, "-1");
            continue;
        }
        let mut answer = 0;
        ...
        while start < n {
            answer += 1;
            let mut max_mon = a[start];
            for end in start..n {
                if end == n - 1 {
                    start = end + 1;
                    break;
                }
                max_mon.set_max(a[end + 1]);
                let len = end - start + 2;
                if max_power[len] < max_mon {
                    start = end + 1;
                    break;
                }
            }
