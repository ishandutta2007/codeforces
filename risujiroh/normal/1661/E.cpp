...
void main(int) {
  int n;
  scan(n);
  vector s(3, string(n, 0));
  scan(s);
  auto get_mask = [&](int i) {
    int ret = 0;
    for (int z : rep(3)) { ret |= (s[z][i] & 1) << z; }
    return ret;
  };
  vector<int> suff(n + 1);
  for (int i : rep(n)) {
    if (get_mask(i) != 0b111) { continue; }
    int j = i + 1;
    while (j < n && get_mask(j) == 0b101) { ++j; }
    if (j < n && get_mask(j) == 0b111) {
      for (int k : rep(i + 1, j)) {
        suff[k] = 1;
        s[1][k] = '1';
      }
  ...
  for (int i : rev(rep(n))) { suff[i] += suff[i + 1]; }
  vector<int> suff_v(n + 1);
  for (int i : rev(rep(n))) { suff_v[i] = __builtin_popcount(get_mask(i)) + suff_v[i + 1]; }
  vector<int> suff_e(2 * n);
  for (int i : rev(rep(2 * n - 1))) {
    suff_e[i] = suff_e[i + 1];
    if (i & 1) {
      suff_e[i] += __builtin_popcount(get_mask(i / 2) & get_mask((i + 1) / 2));
    } else {
      int t = get_mask(i / 2);
      suff_e[i] += (t & 0b011) == 0b011;
      suff_e[i] += (t & 0b110) == 0b110;
    }
  }
  vector<int> suff_f(n);
  for (int i : rev(rep(n - 1))) {
    suff_f[i] = suff_f[i + 1];
    int t = get_mask(i) & get_mask(i + 1);
    suff_f[i] += (t & 0b011) == 0b011;
    suff_f[i] += (t & 0b110) == 0b110;
  }
  int q;
  scan(q);
  while (q--) {
    ...
    scan(l, r);
    --l;
    if (suff[l] - suff[r] == r - l) {
      println(2);
      continue;
    }
    int ans = suff_v[l] - suff_v[r];
    ans -= suff_e[2 * l] - suff_e[2 * r - 1];
    ans += suff_f[l] - suff_f[r - 1];
    println(ans);
  }
}
