...
int main(){
  ...
  while(t--){
    ll n;cin>>n;
    auto g=readGraph<int>(n,n-1,0);
    HLD hld(g,0);
    vector<ll>ret(n+1);
    ret[0]=n*(n-1)/2;
    ret[1]=n*(n-1)/2;
    for(auto to:g[0]){
      ret[1]-=(ll)hld.sz[to]*(hld.sz[to]-1)/2;
    }
    ll u=0,v=0;
    BIT<ll>bit(n);
    auto path_sum=[&](ll x,ll y){
      ll sum=0;
      auto tmp=hld.query_path(x,y,false);
      for(auto z:tmp)sum+=bit.sum(z.fi,z.se);
      return sum;
    };
    bit.add(hld.in[0],1);
    rep(i,1,n){
      ...
      if(path_sum(u,v)==i+1){
        ...
        if(u==0){
          ll mv=hld.child(u,v);
          ret[i+1]=hld.sz[v]*(hld.sz[u]-hld.sz[mv]);
        }
        else ret[i+1]=hld.sz[u]*hld.sz[v];
      }
      else break;
    }
    //debug(ret,n+1);
