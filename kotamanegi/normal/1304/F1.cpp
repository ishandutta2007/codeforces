    ...
    REP(i, n) {
        ...
        if (i == 0) {
        ...
        else {
            {
                int hoge = 0;
                for (int q = m - k; q >= k; --q) {
                    hoge = max(hoge, dp[i][q] + calc(q, q + k));
                    dp[i+1][q - k] = max(dp[i+1][q - k], hoge + calc(q - k, q));
                }
                hoge = 0;
                for (int q = 0; q + k < m - k + 1; ++q) {
                    hoge = max(hoge, dp[i][q] + calc(q, q + k));
                    dp[i+1][q + k] = max(dp[i+1][q + k], hoge + calc(q + k, q + 2 * k));
                }
            }//without div
            ...
            REP(q, m - k + 1) {
                nexts.push(mp(dp[i][q] - summings[q], q));
                while (nexts.top().second <= q - k) {
                    nexts.pop();
                }
                dp[i + 1][q] = max(dp[i + 1][q], nexts.top().first + summings[q + k]);
            }
            while (!nexts.empty()) {
                nexts.pop();
            }
            for (int q = m - k; q >= 0; --q) {
                nexts.push(mp(dp[i][q] + summings[q + k], q));
                while (nexts.top().second >= q + k) {
                    nexts.pop();
                }
                dp[i + 1][q] = max(dp[i + 1][q], nexts.top().first - summings[q]);
            }
        }
    }
    int ans = 0;
