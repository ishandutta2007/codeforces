...
int main()
{
    ...
    int n,k;
    cin>>n>>k;
    for(int i=1;i<=n;i++) cin>>red[i]>>blue[i];
    ll tot=0;
    dp[0][0]=0;
    for(int i=1;i<=n;i++)
    {
        for(int j=0;j<k;j++)
        {
            ...
            int pb=(tot-j-dp[i-1][j]*k);
            for(int c=0;c<min(k,red[i]+1);c++)
            {
                int cursum=red[i]-c;
                int var=cursum%k;
                if(var!=0&&var+blue[i]<k) continue;
                ll d=cursum/k+(var+blue[i])/k;
                //ll bluerem=cursum+blue[i]-d*k;
                //bluerem+=pb;
                //d+=bluerem/k;
                d+=(c+j)/k;
                int nxt=(c+j)%k;
                //dbg(i,j,c,d);
                dp[i][nxt]=max(dp[i][nxt],dp[i-1][j]+d);
            }
        }
        //dbg(i,dp);
        tot+=red[i]+blue[i];
    ...
    {
        if(dp[n][i]>0)
            mx=max(mx,dp[n][i]+(tot-i-dp[n][i]*k)/k);
    }
    print(mx);
}