...
int main(){
  ...
  rep(i,N){
    ...
    ans+=c[i];
    if(i){
      if(a[i]-a[i-1]>c[i-1])g[i-1].emplace_back(a[i]-a[i-1]-c[i-1],i);
      g[i].emplace_back(0,i-1);
    }
  }
  debug(a,c);
  rep(i,N-1){
    j=upper_bound(all(a),a[i]+c[i])-a.begin()-1;
    g[i].emplace_back(0,j);
    if(j+1<N)g[i].emplace_back(a[j+1]-a[i]-c[i],j+1);
  }
  ans+=dijkstra(g,0).back();
  print(ans);
}