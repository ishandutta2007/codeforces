...
int calc(vi &h){
    int res=0;
    rep(i,1<<n){
        res+=h[i]*sgn(n-__builtin_popcount(i));
    }
    return res;
}

void dfs(vi &ret,int beg,int mx,int id){
    if (beg==n) mp[now]=cnt, ans[cnt++]=calc(ret);
    for (int i=1;beg+i<=n&&i<=mx;++i){
        vi tmp(ret);
        for (int j=0;j<(1<<n);++j){
            ret[j]=ret[j]*dpu[i][j];
        }
...
//    cin.tie(0), cout.tie(0);
    ...
    rep(i,1<<n) rep(j,n) if (i>>j&1){
        rep(k,n) if (g[j][k]) dp[i][j]+=dp[i^(1<<j)][k];
    }
    rep(j,n+1) dpu[j].resize(1<<n,0);
    rep(j,n+1) rep(i,1<<n){
        if (__builtin_popcount(i)==j){
            rep(k,n) dpu[j][i]+=dp[i][k];
        }
    }
    rep(j,n+1) dpu[j]=FWT::OR(dpu[j]);
    vi tmp;
    tmp.resize(1<<n,1);
    dfs(tmp,0,n,0);
    rep(i,1<<n-1){
