...
int main() {
    ...
    Combination<mint> COM(3000010);
    int n, q;
    cin >> n >> q;
    vector<mint> coef(3 * (n + 1) + 2, 0);
    for (int i = 0; i <= 3 * (n + 1); i++) coef[i] = COM.C(3 * (n + 1), i);
    coef[0]--;
    vector<mint> ans(3 * n + 1, 0);
    mint inv3 = mint(1) / 3;
    for (int i = 0; i <= 3 * n; i++)
        ans[i] = (coef[i + 1] - (i - 1 >= 0 ? ans[i - 1] * 3 : 0) - (i - 2 >= 0 ? ans[i - 2] : 0)) * inv3;
