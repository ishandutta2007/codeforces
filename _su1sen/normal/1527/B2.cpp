...
constexpr int N_MAX = 1010;

constexpr int to_state(bool center, int diff, int zero, bool rev) {
    return (center << 21) | (diff << 11) | (zero << 1) | rev;
}

constexpr int MAX_STATE = to_state(true, N_MAX, N_MAX, true);

vec<int> memo(MAX_STATE + 1, IINF);

int dp(bool center, int diff, int zero, bool rev) {
    int state = to_state(center, diff, zero, rev);
    int& res = memo[state];
    if (res != IINF) {
        return res;
    }
    if (center) {
        chmin(res, -dp(false, diff, zero, false) + 1);
    }
    if (not rev and diff > 0) {
        chmin(res, -dp(center, diff, zero, true));
    }
    if (diff > 0) {
        chmin(res, -dp(center, diff - 1, zero, false) + 1);
    }
    if (zero > 0) {
        chmin(res, -dp(center, diff + 1, zero - 1, false) + 1);
    }
    if (res == IINF) {
        res = 0;
    }
...
int main() {
    ...
    while (t --> 0) {
        ...
        rep(i, n / 2) {
            diff += s[i] != s[n - i - 1];
            zero += s[i] == '0' and s[n - i - 1] == '0';
        }
        int ans = dp((n & 1) == 1 and s[n / 2] == '0', diff, zero, false);
        if (ans > 0) {
            print("BOB");
        } else if (ans < 0) {
            print("ALICE");
