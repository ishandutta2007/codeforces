...
void main(int) {
  ...
  vla<option> options(m);
  for (auto&& e : options) {
    scan(e.e, e.t, e.p);
    --e.e;
  }
  vla2<int> g(n, rep(m).map(LAMBDA(pair(options[_1].e, _1))));
  int cur = 0;
  basic_string<int> ans;
  for (int i : rep(n)) {
    vla f(sz(g[i]) + 1, vla(101, inf));
    vla nxt(sz(g[i]) + 1, vla(101, pair(-1, -1)));
    f[sz(g[i])][0] = 0;
    for (int z : rep(sz(g[i])).rev()) {
      auto e = options[g[i][z]];
      f[z] = f[z + 1];
      for (int x : rep1(0, 100)) {
        if (chmin(f[z][min(x + e.p, 100)], f[z + 1][x] + e.t)) { nxt[z][min(x + e.p, 100)] = {x, g[i][z]}; }
      }
    }
    if (a[i] < cur + f[0][100]) { return println(-1); }
    {
      int x = 100;
      for (int z : rep(sz(g[i]))) {
        if (~nxt[z][x].first) {
          ans += nxt[z][x].second;
          x = nxt[z][x].first;
        }
