...
int main() {
	...
	REP(i, n) REP(j, m) if ((i + j) & 1) in[i][j] ^= 1;
	memset(dist, -1, sizeof(dist));
	REP(c, 2) {
		queue<pii> bfs_queue;
		auto touch = [&](pii x, int d) {
			if (x.first < 0 || x.first >= n || x.second < 0 || x.second >= m) return;
			if (dist[c][x.first][x.second] == -1) {
				dist[c][x.first][x.second] = d;
				bfs_queue.push(x);
			}
		};
		REP(i, n) REP(j, m) if (in[i][j] == '0' + c) touch(pii(i, j), 0);
		while (!bfs_queue.empty()) {
			auto cu = bfs_queue.front();
			bfs_queue.pop();
			for (pii d : vpii{{0, 1}, {1, 0}, {0, -1}, {-1, 0}}) {
				pii to = cu + d;
				touch(to, dist[c][cu.first][cu.second] + 1);
			}
		}
	}
	REP(c, 2) debug(imie(c), dump(dist[c], 0, n, 0, m));
	REP(_, q) {
		int x, y;
		ll t;
		scanf("%d%d%lld", &x, &y, &t);
		...
		debug(imie(x), imie(y));
		int c = in[x][y] - '0';
		int dop = dist[c ^ 1][x][y];
		debug(imie(c), imie(dop), imie(t));
		printf("%lld\n", (t < dop || dop == -1 ? c : (c ^ dop ^ t ^ 1) & 1) ^ (x + y & 1));
	}
	exit(0);
}
//#STAY AT HOME