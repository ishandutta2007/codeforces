...
void solve() {
  ...
  COST[0] = 0;
  i128 sum = 0;
  FOR(i, N + 1) sum += COST[i];
  FOR(d, LIM) {
    for (auto&& [f, t, c]: query[d]) {
      if (t != 0) continue;
      sum -= COST[f];
      chmin(COST[f], c);
      sum += COST[f];
    }
    DP1[d] = min<i128>(INF, sum);
  }
  vi DP2(LIM, INF);
  FOR(i, N + 1) if (i) COST[i] = INF;
  sum = 0;
  FOR(i, N + 1) sum += COST[i];
  FOR_R(d, LIM) {
    for (auto&& [f, t, c]: query[d]) {
    ...
    DP2[d] = min<i128>(INF, sum);
  }

  // FOR(d, LIM - K) { print(DP1[d], DP2[d + K]); }

  ll ANS = INF;
  FOR(d, LIM - K) { chmin(ANS, DP1[d] + DP2[d + K]); }
  if (ANS == INF) ANS = -1;
  print(ANS);
}
