...
#line 2 "/home/maspy/compro/library/nt/divisors.hpp"
vc<ll> divisors(ll N) {
  auto pf = factor(N);
  vi div = {1};
  for (auto&& [p, e]: pf) {
...
void solve() {
  ...
  auto contain = [&](ll b, ll q, ll y, ll x) -> bool {
    if ((x - b) % q != 0) return false;
    ll k = (x - b) / q;
    return 0 <= k && k < y;
  };
  ...
  //  b  c 
  //  ok first, last 
  if (r % q != 0) return print(0);
  ll c_first = c, c_last = c + (z - 1) * r;
  if (!contain(b, q, y, c_first)) return print(0);
  if (!contain(b, q, y, c_last)) return print(0);

  if (!contain(b, q, y, c_first - r)) return print(-1);
  if (!contain(b, q, y, c_last + r)) return print(-1);

  mint ANS = 0;
  // a,p,x
  for (auto&& p: div) {
    if (p * q / gcd(p, q) != r) continue;
    ANS += mint(r / p) * mint(r / p);
  }
  print(ANS);
}
