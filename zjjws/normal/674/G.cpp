...
namespace gyqtree
{
    ...
    struct node
    {
        int tag;
        pair<int,int>a[5];
        void operator +=(node &ath)
        {
            for(int j=0;j<5;j++)if(ath.a[j].first)
            {
                bool kakaa=false;
                for(int i=0;i<5;i++)if(a[i].first==ath.a[j].first){a[i].second+=ath.a[j].second;kakaa=true;break;}
                if(kakaa)continue;

                int minl=0x3f3f3f3f,fd=-1;for(int i=0;i<5;i++)if(a[i].second<minl)minl=a[i].second,fd=i;
                if(minl>=ath.a[j].second)for(int i=0;i<5;i++)a[i].second-=ath.a[j].second;
                else {if(minl)for(int i=0;i<5;i++)a[i].second-=minl;a[fd]=make_pair(ath.a[j].first,ath.a[j].second-minl);}
            }
            return;
        }
        void addcover(int l,int r,int x){tag=x;a[0]=make_pair(x,r-l+1);for(int i=1;i<5;i++)a[i]=make_pair(0,0);return;}
    }nod[N<<2];
    node ans;
    void up(int i){nod[i]=nod[ls(i)];nod[i].tag=0;nod[i]+=nod[rs(i)];return;}
    void down(int l,int r,int i)
    {
        int mid=l+r>>1;
        if(nod[i].tag)nod[ls(i)].addcover(l,mid,nod[i].tag),nod[rs(i)].addcover(mid+1,r,nod[i].tag),nod[i].tag=0;return;
    }
    void maketree(int l,int r,int i)
    {
        if(l==r){nod[i].addcover(l,r,a[l]);return;}
        int mid=l+r>>1;maketree(l,mid,ls(i));maketree(mid+1,r,rs(i));up(i);return;
    }
    void cover(int l,int r,int i,int ql,int qr,int qk)
    {
        if(ql<=l&&r<=qr){nod[i].addcover(l,r,qk);return;}
        int mid=l+r>>1;down(l,r,i);if(ql<=mid)cover(l,mid,ls(i),ql,qr,qk);if(mid<qr)cover(mid+1,r,rs(i),ql,qr,qk);up(i);return;
    }
    void check(int l,int r,int i,int ql,int qr)
    {
        if(ql<=l&&r<=qr){ans+=nod[i];return;}
        int mid=l+r>>1;down(l,r,i);if(ql<=mid)check(l,mid,ls(i),ql,qr);if(mid<qr)check(mid+1,r,rs(i),ql,qr);up(i);return;
    }
    #undef ls(i)
    #undef rs(i)
}
...
bool myru_a(pair<int,int> x,pair<int,int> y){return x.second>y.second;}
void Read(){rin>>n>>m>>p;for(int i=1;i<=n;i++)rin>>a[i];return;}
void Init(){maketree(1,n,1);return;}
void Work()
{
    for(int i=1;i<=m;i++)
    {
        int typ,l,r,x;rin>>typ>>l>>r;
        if(typ==1)rin>>x,cover(1,n,1,l,r,x);
        else 
        {
            ans.addcover(1,0,0);x=0;check(1,n,1,l,r);sort(ans.a+0,ans.a+5,myru_a);printf("%d",(100/p));
            for(int i=0,ed=(100/p);i<ed;i++)printf(" %d",ans.a[i].first?ans.a[i].first:114514);puts("");
        }
        // printf("i:%d over\n",i);
    }
    return;
