...
class ERazbivaemKvadrat {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        arri y, lx, rx;
        in.readArrays(n, y, lx, rx);
        arri x, ly, ry;
        in.readArrays(m, x, ly, ry);

        FenwickTree<int> tree(1000000);
        ll answer = 1;
        multiset<pair<int, pii>> events;
        for (int i : range(m)) {
            if (ly[i] == 0) {
                tree.add(x[i], 1);
                if (ry[i] == 1000000) {
                    ...
                    events.emplace(ry[i] + 1, pii(x[i], -1));
                }
            } else {
                events.emplace(ly[i], pii(x[i], 1));
            }
        }
        vector<pair<int, pii>> horizontal;
        for (int i : range(n)) {
            horizontal.emplace_back(y[i], pii(lx[i], rx[i]));
        }
        sort(all(horizontal));
        for (const auto& p : horizontal) {
            while (!events.empty() && events.begin()->first <= p.first) {
                tree.add(events.begin()->second.first, events.begin()->second.second);
                events.erase(events.begin());
            }
            if (p.second.first == 0 && p.second.second == 1000000) {
                answer++;
            }
            answer += tree.get(p.second.first, p.second.second + 1);
        }
