...
int main(){
  ...
  string s; cin>>s;
  li n = s.size();
  li xy[2]; cin>>xy[0]>>xy[1];



  for(li j = 0; j < (li)(2); ++j) for(li i = 0; i < (li)(n); ++i) {
    S[i+1][j] = S[i][j] + (s[i] == ('0'+j));
  }
  for(li i = 0; i < (li)(n); ++i) {
    S[i+1][2] = S[i][2] + (s[i] == ('?'));
  }
  for(li j = 0; j < (li)(2); ++j) for(li i = 0; i < (li)(n); ++i) {
    li z = 0;
    if(s[i] == '?') z = (S[i][j^1] - S[0][j^1]) * xy[j^1] + (S[n][j^1] - S[i+1][j^1]) * xy[j];
    if(s[i] == '0') z = (S[i][1] - S[0][1]) * xy[1] + (S[n][1] - S[i+1][1]) * xy[0];

    C[i+1][j] = C[i][j] + z;
  }
  li ans = 1ll<<60;
  for(li j = 0; j < (li)(2); ++j) for(li k = 0; k < (li)(2); ++k) for(li i = 0; i < (li)(n); ++i) {
    ans = min(ans,
              (C[i][j] - C[0][j]) +
              (C[n][k] - C[i][k]) +
              ((j==0&&k==1) ? (S[i][2] - S[0][2]) * (S[n][2] - S[i][2]) * xy[0] : 0) +
              ((j==1&&k==0) ? (S[i][2] - S[0][2]) * (S[n][2] - S[i][2]) * xy[1] : 0)
    );
  }
  cout << ans << endl;
