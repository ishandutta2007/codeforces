...
//    solve.usetc()
    solve.cases{
        ...
        val store = Array(405){IntArray(205)}
        for(c1 in 0 until 405){
            for(r in 0 until 205){
                val nn = n - c1
                store[c1][r] = FACT.bigChoose(nn,r)
            }
        }
        val DP = Array(k+1){IntArray(2 * k + 1)}
        var to = 0
        DP[0][0] = 1
        for(newlen in 2..k+1){
            for(ops in k downTo 0){
                for(wid in 0..2 * ops){
                    val each = FACT.store[newlen -1]
//                    val factor = FACT.choose(n - wid, newlen) modM FACT.store[newlen-1]
                    var ms = DP[ops][wid]
                    var newops = ops
                    var newwid = wid
                    for(mul in 1..k){
                        newops += newlen - 1
                        newwid += newlen
                        if(newops !in 0..k || newwid !in 0..(k shl 1)) {
                            break
                        }
                        to ++
                        ms = ms modM store[ newwid - newlen][newlen] modM each
                        DP[newops][newwid] = DP[newops][newwid] modPlus (ms modM FACT.invStore[mul])
                    }
                }
            }
        }
//        just dei to
        val allops = IntArray(k+1){
            var ret = 0
            for(c in DP[it]){
                ret = ret modPlus c
            }
        ...
        for(finalops in 1..k){
            var ans = 0
            for(actualops in finalops downTo 0 step 2){
                ans = ans modPlus allops[actualops]
            }
            put(ans)
        }
