...
void solve() {
    ...
    if (sz(st) == 1) {
        if (st[0].f == 'L') println(max(0, 2 * k - 1));
        else println(2 * n - 1);
        return;
    }
    vector<pii> x;
    for (int i = 1; i < sz(st) - 1; ++i)
        if (st[i].f == 'L') x.pb(st[i].s);
    vector<pii> y;
    if (st.back().f == 'L') y.pb(st.back().s);
    if (st[0].f == 'L') y.pb(st[0].s);
    sort(all(x), [&](pii p, pii d) { return p.s - p.f < d.s - d.f; });
    for (auto i : x) {
        for (int j = i.f; j < i.s && k > 0; ++j) {
            s[j] = 'W';
            --k;
        }
    }
