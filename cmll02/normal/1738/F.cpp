...
int d[1000005];
mgs
int p[1000005],ip[1000005],vis[1000005];
signed main()
{
	initprog();
	int T=read();
	while(T--)
	{
		int n=read();
		rg(n)d[i]=read();gr
		rg(n)fa[i]=i,sz[i]=d[i];gr
		rg(n)p[i]=i;gr
		std::sort(p+1,p+1+n,[&](int x,int y){return d[x]>d[y];});
		rg(n)ip[p[i]]=i;gr
		rg(n)vis[i]=0;gr
		...
		gr
		printf("! ");rg(n)odb(p[f(ip[i])]);gr fflush(stdout);
	}
	return 0;
}