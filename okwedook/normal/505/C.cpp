...
const int diff = 250;
vector<int> poss;
vector<vector<int>> dp(N, vector<int>(2 * diff + 1, -mod));
...
signed main() {
    ...
    for (int l = 1; l < N; ++l)
        if (abs(l - d) <= diff) poss.pb(l);
    dp[d][d - poss[0]] = cnt[d];
    for (int i = 0; i < N; ++i) {
        for (auto l : poss)
            if (i - l >= 0) {
                for (int was = l - 1; was <= l + 1; ++was)
                    if (was >= poss[0] && was <= poss.back())
                        chmax(dp[i][l - poss[0]], dp[i - l][was - poss[0]] + cnt[i]);
            }
    }
    int ans = 0;
    for (auto i : dp)
