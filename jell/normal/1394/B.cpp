...
#line 4 "codeforces-workspace/contest/1394/b/b.cpp"
#include <ext/rope>
...
#line 11 "codeforces-workspace/contest/1394/b/b.cpp"
...
solver()
{
    ...
    for(auto [u,v]: egs)
    {
        rg[v].emplace_back(od[u]-1,oc[u]);
        oc[u]++;
    }
    assert(oc==od);

    vector ban(k*k,vector<bool>(k*k));
    for(int i=0; i<n; ++i)
    {
        auto& ngb=rg[i];
        vector<bool> chkd(k*k);
        vector<int> lst;
        for(auto [u,v]:ngb)
        {
            int z=u*k+v;
            if(chkd[z]) ban[z][z]=1;
            else chkd[z]=1,lst.emplace_back(z);
        }
        for(int z: lst)
        {
            for(int zz: lst)
