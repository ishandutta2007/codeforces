...
int dp[N][N][N];
char prv[N][N][N];
pair<pair<int, int>, int> lol[N][N][N];
...
inline void solve() {
	...
	for (int i = 0; i <= sz(s); ++i) {
		for (int j = 0; j <= sz(t); ++j) {
			for (int bal = 0; bal < N - 1; ++bal) {
				...
				int adi = (i == sz(s) ? 0 : (s[i] == '('));
				int adj = (j == sz(t) ? 0 : (t[j] == '('));
				if (dp[i + adi][j + adj][bal + 1] > dp[i][j][bal] + 1) {
					dp[i + adi][j + adj][bal + 1] = dp[i][j][bal] + 1;
					prv[i + adi][j + adj][bal + 1] = '(';
					lol[i + adi][j + adj][bal + 1] = { {i,j},bal };
				}
				adi = (i == sz(s) ? 0 : (s[i] == ')'));
				adj = (j == sz(t) ? 0 : (t[j] == ')'));
				if ((adi || adj) && bal > 0) {
	...
	for (int bal = 0; bal < N; ++bal) {
		if (dp[sz(s)][sz(t)][bal] + bal < dp[sz(s)][sz(t)][mn] + mn) {
			mn = bal;
		}
		//cerr << dp[sz(s)][sz(t)][bal] + bal << endl;
	}

	string ans(mn, ')');
	int ci = sz(s), cj = sz(t), cb = mn;
	while (prv[ci][cj][cb] != '#') {
		assert(prv[ci][cj][cb] != 0);
		ans.push_back(prv[ci][cj][cb]);
		auto np = lol[ci][cj][cb];
		ci = np.x.x;
		cj = np.x.y;
		cb = np.y;
		//cerr << ci << " " << cj << " " << cb << endl;
