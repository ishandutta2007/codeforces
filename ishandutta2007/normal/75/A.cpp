...
//freopen (file".out", "w", stdout);
 ...
 char ac[100], bc[100],cc[100],ccd[100];
  ss(a);ss(b);
  int ci=atoi(a)+atoi(b);
  itoa(ci,c,10);
  int j=0;  for(int i=0;i<strlen(a);i++){    if(a[i]=='0')continue;    ac[j++]=a[i];  }  ac[j]='\0';
  j=0;  for(int i=0;i<strlen(b);i++){    if(b[i]=='0')continue;    bc[j++]=b[i];  }  bc[j]='\0';
  j=0;  for(int i=0;i<strlen(c);i++){    if(c[i]=='0')continue;    cc[j++]=c[i];  }  cc[j]='\0';
  if(atoi(cc)==(atoi(ac)+atoi(bc)))psn("YES");
  else psn("NO");
//sp;
  return 0;
}