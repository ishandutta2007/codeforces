...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/flow/incremental_maxflow.hpp"
...
void solve() {
  ...
  Incremental_MaxFlow<ll> G(2 * F + 10, N, N + 1, 1LL << 30);

  FOR(v, N) G.add(source, v, CNT[v]);
  ll nxt_v = N + 2;
  ll nxt = 1;
  ...
  while (f < F) {
    ...
    G.add(nxt_v, sink, 1);
    nxt = cand + 1;
    FOR(v, N) if (cand % A[v] == 0) { G.add(v, nxt_v, 1); }
    ++nxt_v;
    ll new_f = G.flow();
    assert(f < new_f);
    if (f < new_f) { ANS += cand; }
