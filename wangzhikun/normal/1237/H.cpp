...
void solve1(str a,str b){
	...
	for(int i = n-2;i>=0;i-=2){
		...
		int cp = -1;
		for(int j = i-2;j>=0;j-=2) if(a[j] == b[i] && a[j+1] == b[i+1]){cp = j;break;}
		if(cp == -1){
			rev(i+2);
			if(a[i] == b[i] && a[i+1] == b[i+1])continue;
			for(int j = i-2;j>=0;j-=2) if(a[j] == b[i] && a[j+1] == b[i+1]){cp = j;break;}
...
int main() {
	...
	while(t--){
		...
		for(int i = n-2;i>=0;i-=2){
			...
			vi cv,cv2;
			for(int j = i;j>=0;j-=2){
				if(a[j] == b[i] && a[j+1] == b[i+1])cv.pb(j);
				if(a[j] == b[i+1] && a[j+1] == b[i])cv2.pb(j);
			}
			if(!cv.size()){
				rev(cv2[rng()%cv2.size()]+2);
				cv.pb(0);
			}
			rev(cv[rng()%cv.size()]+2);
			rev(i+2);
		}
		//dbg(a,b);
		ps(sz(op));
