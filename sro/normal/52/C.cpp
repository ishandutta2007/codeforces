...
struct segtree
{
	INT dt[1600000];
	INT wp[1600000];
	void jia(INT now,INT l,INT r,INT nl,INT nr,INT num)
	{
		dt[now]+=wp[now];
		wp[now*2]+=wp[now];
		wp[now*2+1]+=wp[now];
		...
		INT m=(nl+nr)/2;
		jia(now*2,l,r,nl,m,num);
		jia(now*2+1,l,r,m+1,nr,num);
		dt[now]=min(dt[now*2],dt[now*2+1]);
	}
	INT jisuan(INT now,INT l,INT r,INT nl,INT nr)
	{
		dt[now]+=wp[now];
		wp[now*2]+=wp[now];
		wp[now*2+1]+=wp[now];
		...
		if(nl>=l&&nr<=r)
			return dt[now];
		INT m=(nl+nr)/2;
		return min(jisuan(now*2,l,r,nl,m),jisuan(now*2+1,l,r,m+1,nr));
	}
}sg;
...
int main()
{
	...
	{
		getline(cin,s);
		if(sscanf(s.c_str(),"%I64d%I64d%I64d",&a,&b,&c)==3)
		{
			if(a<=b)
				sg.jia(1,a,b,0,n-1,c);
			else
			{
				sg.jia(1,a,n-1,0,n-1,c);
				sg.jia(1,0,b,0,n-1,c);
			}
		}
		else
		{
			if(a<=b)
				printf("%d\n",sg.jisuan(1,a,b,0,n-1));
			else
				printf("%d\n",min(sg.jisuan(1,a,n-1,0,n-1),sg.jisuan(1,0,b,0,n-1)));
		}
	}
	return 0;
}