...
int h[N]; // height of node u
int sz[N]; // size of subtree u
int mxnum[N]; // maximum amount of #heights in node i
map <int, int> mpp[N]; // {#heights, amount}

void dfs(int u, int p){
    int mxsz = 0, idmxsz = 0; // maximum subtree size, and the id of that subtree
    sz[u] = 1;
    Fora(v, adj[u]){
        if (v == p) continue;
        h[v] = h[u] + 1;
    ...
    mxnum[u] = mxnum[idmxsz];
    swap(mpp[u], mpp[idmxsz]);
    ans[u] = ans[idmxsz];
    Fora(v, adj[u]){
        if (v == p) continue;
        if (v == idmxsz) continue;
        Fora(&x, mpp[v]){
            mpp[u][x.fi] += x.se;
            if (mpp[u][x.fi] > mxnum[u]){
                mxnum[u] = mpp[u][x.fi];
                ans[u] = x.fi;
            }
            else if (mpp[u][x.fi] == mxnum[u]){
                ans[u] = min(ans[u], x.fi);
            }
        }
    }
    mpp[u][h[u]] += 1;
    if (mpp[u][h[u]] > mxnum[u]){
        mxnum[u] = mpp[u][h[u]];
        ans[u] = h[u];
    }
    else if (mpp[u][h[u]] == mxnum[u]){
        ans[u] = min(ans[u], h[u]);
    }
}
