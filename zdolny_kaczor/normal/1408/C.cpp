...
void solve() {
	...
	vector <ld> t1(n + 2), t2(n + 2);
	for (int i = 1; i < n + 2; ++i) {
		int dist = a[i] - a[i - 1];
		t1[i] = t1[i - 1] + (ld) dist / (ld) i;
	}
	for (int i = n; i >= 0; --i) {
		int dist = a[i + 1] - a[i];
		t2[i] = t2[i + 1] + (ld) dist / (ld) (n + 1 - i);
	}
	debug(imie(t1), imie(t2), imie(a));
	REP(i, n + 1) {
		if (t1[i + 1] > t2[i + 1]) {
			ld start1 = t1[i], start2 = t2[i + 1];
			ld left = a[i + 1] - a[i];
			int v1 = i + 1, v2 = n - i + 1;
			if (start1 > start2)
				left -= (start1 - start2) * v2;
			else {
				left -= (start2 - start1) * v1;
			}
			ld ans = max(start1, start2) + left / (v1 + v2);
			debug(imie(start1), imie(start2), arr(a, i), arr(a, i + 1), imie(left) / (imie(v1) + imie(v2)));
			printf("%.10Lf\n", ans);
			return;
		}
	}
