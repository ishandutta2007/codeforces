...
ll divconq(int l, int r) {
    if(r - l <= 1) return max(0LL, a[l]);
    int mid = (l + r) >> 1;
    ...
    for(auto [a, b]: line) cht.add(a, b);
    ll ret = 0; ret = max(ret, cht.query(0));
    sum = 0; pyrm = 0;
    for(int i = mid - 1; i >= l; i--) {
        pyrm += sum + a[i];
        sum += a[i];
        ret = max(ret, pyrm + cht.query(mid - i));
    }
    return max({ret, divconq(l, mid), divconq(mid, r)});
}
