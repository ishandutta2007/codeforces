...
public:
	void solve(istream& inp, ostream& outp) {
        ...
        int c = in.readInt();
        int k = in.readInt();
        auto map = in.readTable<char>(r, c);

        int totalRice = 0;
        for (const auto& row : map) {
            totalRice += count(all(row), 'R');
        }
        auto get = [](int id) -> char {
            if (id < 10) {
                return '0' + id;
            }
            id -= 10;
        ...
        char current = get(k - 1);
        auto answer = makeArray(r, c, ' ');
        int needRice = totalRice / k;
        int direction = 1;
        for (int i = 0; i < r; i++) {
            int start = direction == 1 ? 0 : c - 1;
            for (int j = start; j >= 0 && j < c; j += direction) {
                if (map[i][j] == 'R') {
                    totalRice--;
                    needRice--;
                    if (needRice < 0) {
