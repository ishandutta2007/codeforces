...
void solve() {
  ...
  vector<int> inv(n);
  REP(i, n) inv[a[i]] = i;
  SegmentTree<monoid::RangeMaximumQuery<int>> mx(a);
  SegmentTree<monoid::RangeMinimumQuery<int>> mn(a);
  int ans = 0;
  for (int pos = 0; pos < n - 1; ++ans) {
    if (a[pos] < a[pos + 1]) {
      const int r = mn.find_right(pos, [&](const int x) -> bool { return x >= a[pos]; });
      pos = inv[mx.get(pos, r)];
    } else {
      const int r = mx.find_right(pos, [&](const int x) -> bool { return x <= a[pos]; });
      pos = inv[mn.get(pos, r)];
    }
  }
  cout << ans << '\n';
}
