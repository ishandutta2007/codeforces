...
int main() {
    ...
    for (int c = 1; c <= N; c++) {
        if (ops[c].second) {
            int x = ops[c].first;
            vector<num> dp(N + 2);
            // dp[s]: how many subsequences up til now with s numbers before x
            // we can always choose not to pick the current i (either + or -)
            dp[0] = 1; // there is initially the empty subsequence with s=0
            ...
            num ways = accumulate(begin(dp), end(dp), num(0));
            ans += ways * x;
        }
    }
    cout << ans << '\n';
