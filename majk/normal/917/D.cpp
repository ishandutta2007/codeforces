...
public:
    int N;
    vector<vector<int>> E;
    vector<vector2<FieldMod>> D;

    int dfs(int u, int p) {
        vector2<FieldMod> DP(1,1,0);
        int s = 1;
        DP[0][0] = 1;
        for (int v: E[u]) {
            if (v != p) {
                ...
                int t = D[v].size();
                int ns = t + r;
                vector2<FieldMod> A(ns,ns,0);
                for (int i = 0; i < r; ++i) {
                    for (int j = i; j < r; ++j) {
                        for (int k = 0; k < t; ++k) {
                            for (int l = k; l < t; ++l) {
                                // copy verbatim and merge the residual component
                                A[i+k][j+l] += DP[i][j] * D[v][k][l];

                                // remove the edge to child
                                A[i+k+1][j+cur] += DP[i][j] * D[v][k][l] * (cur-l);
                            }
                        }
                    }
                }
    ...
    void solve(istream& cin, ostream& cout) {
        auto F = FieldMod::fact(101);
        auto I = FieldMod::invfact(101);

        cin >> N;
        E.resize(N);
        D = vector<vector2<FieldMod>>(N);
        for (int i = 0; i < N - 1; ++i) {
            int u, v;
            cin >> u >> v;
            --u;
        ...
        dfs(0, -1);
        vector<FieldMod> Z(N, 0);
        for (int i = 0; i < N; ++i) { // component count
            for (int j = 0; j < N; ++j) { // component size
                Z[i] += D[0][i][j] * (N - j);
            }
        }
        ...
        vector<FieldMod> Ans(N, 0);
        for (int i = 0; i < N; ++i) {
            Ans[i] = Z[i];
            for (int j = 0; j < i; ++j) {
                FieldMod c =  Z[j] * F[N-j-1] * I[N-1-i] * I[i-j];
                if ((i-j)%2==1) Ans[i] -= c;
                else Ans[i] += c;
            }
        }
