...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		...
		segment_tree dp_plus((int)cmpr.size(), [&](auto x, auto y){ return max(x, y); }, minf);
		vector<int> dp_zero((int)cmpr.size(), minf);
		segment_tree dp_minus((int)cmpr.size(), [&](auto x, auto y){ return max(x, y); }, minf);
		vector<int> dp(n + 1);
		{
			int p = ranges::lower_bound(cmpr, pref[0]) - cmpr.begin();
			dp_plus.set(p, 0);
			dp_zero[p] = 0;
			dp_minus.set(p, 0);
		}
		for(auto i = 1; i <= n; ++ i){
			int p = ranges::lower_bound(cmpr, pref[i]) - cmpr.begin();
			dp[i] = max({
				dp_plus.query(p + 1, (int)cmpr.size()) - i,
				dp_zero[p],
				dp_minus.query(0, p) + i
			});
			debug(i);
			debug(dp_plus);
			debug(dp_zero);
			debug(dp_minus);
			debug(dp[i]);
			dp_plus.set(p, max(dp_plus.query(p), dp[i] + i));
			ctmax(dp_zero[p], dp[i]);
			dp_minus.set(p, max(dp_minus.query(p), dp[i] - i));
		}
		debug(dp);
		cout << dp[n] << "\n";
		return 0;
