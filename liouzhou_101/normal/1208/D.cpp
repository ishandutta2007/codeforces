...
void update(int k)
{
	tree[k].minv = min(tree[k<<1].minv, tree[k<<1|1].minv);
}
...
void clear(int k)
{
	if (tree[k].tag)
	{
		...
		tree[k<<1].minv.X += tree[k].tag;
		tree[k<<1|1].tag += tree[k].tag;
		tree[k<<1|1].minv.X += tree[k].tag;
		tree[k].tag = 0;
	}
}
...
void modify(int k, int L, int R, int x, ll p)
{
	if (L == R)
	{
		tree[k].minv.X = p;
