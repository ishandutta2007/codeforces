...
signed main() {
    ...
    FOR (i, n) {
        ...
        for (int j = 0; j <= i && i + j < n; ++j) {
            ...
            else if (s[i - j] == '?') want |= 1 << (s[i + j] - 'a'), know++;
            else if (s[i + j] == '?') want |= 1 << (s[i - j] - 'a'), know++;
            else if (s[i - j] != s[i + j]) break;
            FORN (alp, 17) {
                //if (j == 0) continue;
                a[alp][want] += kn[alp][know];
                if (alp == 2 && want <= 3) {
                    //debug(mp(i, j));
                    //debug(know);
                }
    ...
    FOR (i, n) {
        int ev = 0, know = 0;
        int want = 0;
        for (int j = 0; j <= i && i + 1 + j < n; ++j) {
            if (s[i - j] == '?' && s[i + 1 + j] == '?') ev++, know++;
            else if (s[i - j] == '?') want |= 1 << (s[i + 1 + j] - 'a'), know++;
            else if (s[i + 1 + j] == '?') want |= 1 << (s[i - j] - 'a'), know++;
    ...
    FORN (alp, 17) {
        FOR (bit, 17) {
            FOR (mask, 1 << 17) {
                if (Bit(mask, bit)) a[alp][mask] += a[alp][mask ^ (1 << bit)];
            }            
        }
    }
