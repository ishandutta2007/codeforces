...
const int N=1e6+5;
int n,fac[N],ans,ifac[N];
int main(){
	int i;
	scan(n);
	fac[0]=1;
	for(i=1;i<=n;i++) fac[i]=(i64)fac[i-1]*i%P;
	ifac[n]=Inv(fac[n],P);
	for(i=n;i>=1;i--) ifac[i-1]=(i64)ifac[i]*i%P;
	ans=fac[n];
	for(i=1;i<n;i++) inc(ans,(i64)(fac[n-i]-1)%P*fac[n]%P*ifac[n-i]%P);
	print(ans);
}