...
{
	ll ret = 0;
	rep(i, 1, n) ret += max(0LL, ceil(h[i]+a[i]*m-x, p));
	if(ret > k*m) return false;
	...
	rep(i, 1, n){
		...
		while(1){
			...
			if(t+d >= m){
				rem -= a[i] * (m-t);
				while(rem < h[i]) rem += p, vec.pb(m);
				break;
			}
			rem -= a[i]*d, t += d;
			while(rem < a[i]) vec.pb(t), rem += p;
		}
	}
	sort(all(vec));
	
	reps(i, vec) if(vec[i] < i/k+1) return false;
	return true;
}
