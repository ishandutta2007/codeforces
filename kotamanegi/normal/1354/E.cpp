...
int Visited[20000];
int dist[200000];
vector<pair<vector<int>, vector<int>>> going;
void dfs(int now, int back) {
    Visited[now] = 1;
    if (dist[now] == 0) {
        going.back().first.push_back(now);
    }
    else {
        going.back().second.push_back(now);
    }
    for (auto& x : vertexs[now]) {
        if (x == back) continue;
        if (Visited[x] == 0) {
...
void solve() {
    ...
    REP(i, edges.size()) {
        if (dist[edges[i].first] + dist[edges[i].second] != 1) {
            cout << "NO" << endl;
            return;
        }
    }
    ...
    REP(i, going.size()) {
        REP(q, n+1) {
            ...
            if (q + going[i].first.size() <= n) {
                dp[i + 1][q + going[i].first.size()] = 1;
            }
            if (q + going[i].second.size() <= n) {
                dp[i + 1][q + going[i].second.size()] = 1;
            }
        }
    }
    if (dp[going.size()][b] == 0) {
        cout << "NO" << endl;
        return;
    }
    string ans(n, '0');
    for (int i = going.size() - 1; i >= 0; --i) {
        int itr = b - going[i].first.size();
        if (itr >= 0 and dp[i][itr] == 1) {
        }
        else {
            swap(going[i].first, going[i].second);
        }
        for (auto& x : going[i].first) {
            ans[x] = '2';
            b -= 1;
        }
        for (auto& x : going[i].second) {
            if (a > 0) {
                ans[x] = '1';
                a -= 1;
            }
