...
int main()
{
    ...
    TT
    {
        ...
        {
            pl prev=path[path.size()-1];
            if(prev.F%2)
            {
                if(sum%2==0) path.pb({prev.F,prev.S+1});
                else path.pb({prev.F+1,prev.S});
                
            }
            else
            {
                if(sum%2) path.pb({prev.F,prev.S+1});
                else path.pb({prev.F+1,prev.S});
            }
            sum/=2;
        }
        for(auto x:path) cout<<x.F<<" "<<x.S<<endl;
