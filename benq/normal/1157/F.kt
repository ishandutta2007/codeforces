...
fun solve() {
    val n = io.int()
    val a = io.ints(n)
    val cnt = IntArray(200005){0}
    for (i in a) {
        cnt[i] ++
    }
    var best = 0L
    var bestInterval = arrayOf(-1,-1)
    var lef = 0
    while (lef < 200005) {
        if (cnt[lef] == 0) {
            lef ++
        ...
        if (cnt[rig] == 0) rig --
        var sum = 0L
        for (i in lef..rig) sum += cnt[i]
        if (sum > best) {
            best = sum
            bestInterval = arrayOf(lef,rig)
        }
        if (lef == rig) {
            lef = rig+1
        } else {
    ...
    println(best)
    for (i in bestInterval[0]..bestInterval[1]) {
        io.print("${i} ")
        cnt[i] --
    }
    for (i in bestInterval[1] downTo bestInterval[0]) {
        while (cnt[i] > 0) {
            cnt[i] --
            io.print("${i} ")
        }
    }
//    println(bestInterval[0])
//    println(bestInterval[1])
}
