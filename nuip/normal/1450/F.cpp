...
int bf(const vector<int> vs){
	const int n=vs.size();
	int re=MOD;
	vector<int> ind(n); iota(all(ind),0);
	do{
		int ok=1;
		reps(i,1,n)if(vs[ind[i-1]]==vs[ind[i]]){ok=0; break;}
		if(ok) continue;
		int s=0;
		reps(i,1,n)if(abs(ind[i-1]-ind[i])>1) ++s;
		MN(re,s);
...
int solve(const vector<int> vs){
	const int n=vs.size();
	{
		...
		if(*max_element(all(cnt))>(n+1)/2) return -1;
	}
	vector<int> cnt(n);
	++cnt[vs[0]];
	++cnt[vs.back()];
	int re=0;
	reps(i,1,n)if(vs[i-1]==vs[i]) cnt[vs[i]]+=2, ++re;
	cauto mx=*max_element(all(cnt));
	cauto sum=accumulate(all(cnt),0);
	return re+=max(0,mx-(sum-mx)-2+1)/2;
}
