...
void push (int idx, int lf, int rg)
{
    ...
    forl(i, 2)
    {
        int len = (i == 1 ? mid - lf + 1 : rg - mid);
        
        t[2 * idx + i].ft = int((t[2 * idx + i].ft + ((len < 2 ? 0 : s[len - 2]) + 1) * 1LL * add[idx]) % mod);
        t[2 * idx + i].sc = int((t[2 * idx + i].sc + s[len - 1] * 1LL * add[idx]) % mod);
...
void upd (int idx, int lf, int rg, int pos, int val)
{
    ...
    t[idx].ft = int((t[2 * idx + 1].ft + f[len - 1] * 1LL * t[2 * idx + 2].ft + f[len] * 1LL * t[2 * idx + 2].sc) % mod);
    t[idx].sc = int((t[2 * idx + 1].sc + f[len] * 1LL * t[2 * idx + 2].ft + f[len + 1] * 1LL * t[2 * idx + 2].sc) % mod);
}

void inc (int idx, int lf, int rg, int l, int r, int val)
{
    if (lf == l && rg == r)
    {
        int len = rg - lf + 1;
        
        t[idx].ft = int((t[idx].ft + ((len < 2 ? 0 : s[len - 2]) + 1) * 1LL * val) % mod);
        t[idx].sc = int((t[idx].sc + s[len - 1] * 1LL * val) % mod);
...
int sum (int idx, int lf, int rg, int l, int r, int k)
{
    if (lf == l && rg == r)
        return int((t[idx].ft * 1LL * f[k] + t[idx].sc * 1LL * f[k + 1]) % mod);
    ...
    int res = sum(2 * idx + 1, lf, mid, l, mid, k);
