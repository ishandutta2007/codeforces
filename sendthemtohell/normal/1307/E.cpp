...
vector<int> tr[5057];
vector<pair<int, int>> typ[5057];

int gett(int t, int cnt, int lr) {
	if (cnt > sz(tr[t])) {
		if (lr == 0) {
			return INF;
		} else {
...
void solve() {
	...
	vector<int> v(n);
	cin >> v;
	vector<pair<int, int>> cw(m);
	cin >> cw;
	for (int i = 0; i < n; ++i) {
		tr[v[i]].push_back(i);
	}
	for (int i = 0; i < m; ++i) {
		typ[cw[i].x].push_back({ gett(cw[i].x, cw[i].y, 0), gett(cw[i].x, cw[i].y, 1) });
	}
	vector<pair<int, int>> pp;
	for (int i = 0; i < m; ++i) {
		pair<int, int> cur = cw[i];
		int pos = gett(cur.x, cur.y, 0);
		int ppos = gett(cur.x, cur.y, 1);
		if (pos == INF) continue;
		int ans = 1;
		int cnt = 1;
		for (int t = 0; t <= n; ++t) {
			if (t == cur.x) {
				...
				if (ad1 + both > 0 && ad2 + both > 0 && ad1 + ad2 + both > 1) {
					ans += 2;
					int var = (ad1 + both) * (ad2 + both) - both;
					cnt = ll(cnt) * var % MOD;
				} else if (ad1 + both > 0 || ad2 + both > 0) {
					++ans;
					int var = ad1 + ad2 + 2 * both;
					cnt = ll(cnt) * var % MOD;
				}
			}
		}
		pp.push_back({ ans, cnt });
	}
	for (int i = 0; i < m; ++i) {
		pair<int, int> cur = cw[i];
		int pos = gett(cur.x, cur.y, 0);
		int ppos = gett(cur.x, cur.y, 1);
		if (ppos == -INF) continue;
		int ans = 1;
		int cnt = 1;
		for (int t = 0; t <= n; ++t) {
			if (t == cur.x) {
