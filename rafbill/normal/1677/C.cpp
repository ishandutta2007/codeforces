...
void solve() {
  ...
  { vi P(n), Q(n);
    ...
    for(li i = 0; i < (li)(n); ++i) { cin>>Q[i]; --Q[i]; }
    vi IQ(n); for(li i = 0; i < (li)(n); ++i) IQ[Q[i]] = i;
    for(li i = 0; i < (li)(n); ++i) A[i] = IQ[P[i]];
  }
  vi E(n);
  ...
  for(li i = 0; i < (li)(n); ++i) if(!E[i]) {
    li sz = 0;
    for(li j = i; !E[j]; j = A[j]) {
      E[j] = 1;
      sz++;
    }
    neven += sz/2;
    if(sz%2 == 1 && sz != 1) nodd += 1;
  }
  li a = 0, b = n-1;
  li ans = 0;
  for(li i = 0; i < (li)(neven); ++i) {
    ans -= 2*a;
    ans += 2*b;
    a++; b--;
  }
