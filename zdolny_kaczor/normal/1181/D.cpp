...
int main() {
	...
	FOR(i, 1, m) by_cou[cou[i]].push_back(i);
	REP(i, q) {
		ll x;
		scanf("%lld", &x);
	...
	FOR(i, 0, n) {
		for (int v : by_cou[i])
			active.insert(v);
		ll till = time + active.size();
		debug(arr(by_cou, i), imie(active), imie(time), imie(till));
		while (it != queries.end() && it->first <= till) {
			debug(imie(*it), imie(it->first) - imie(time) - 1);
			ans[it->second] = *active.find_by_order(it->first - time - 1);
			it++;
		}
		time = till;
	}
	while (it != queries.end()) {
		ll left = it->first - time - 1;
		debug(imie(*it), imie(left));
		ans[it->second] = left % m + 1;
		it++;
	}
	REP(i, q) printf("%d\n", ans[i]);
}