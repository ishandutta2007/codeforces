...
int main() {
    ...
    rep(i, N) {
        rep(j, N)g[i][(j + 1) % N] = min(g[i][(j + 1) % N], g[i][j] + 1);
        rep(j, N)g[i][(j + 1) % N] = min(g[i][(j + 1) % N], g[i][j] + 1);
    }
    ...
    rep(x, N) {
        ...
        ll mindis = INF;
        rep(i, N)mindis = min(mindis, g[x][i]);

        rep(d, mindis+N) {
            if (d > 0 && d < mindis) { d = mindis - 1; continue; }
            rep(i, N) {
                if (dis[i] != d) { continue; }
                ...
                rep(j, N) {
                    ll k = (j + d) % N;
                    //cout << i << " " << k << endl;
                    //cout << d << " " << dis[k] << " " << d + g[i][j] << endl;
                    dis[k] = min(dis[k], d + g[i][j]);                 
                 }
            }
        }
