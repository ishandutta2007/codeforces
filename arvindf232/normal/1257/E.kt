...
fun main(){
    solve.cases{
        ...
        val DP = Array(n+1){IntArray(3){plarge} }
        DP[0][0] = 0
        DP[0][1] = 0
        DP[0][2] = 0
        for(i in 0 until n){
            for(left in 0 until 3){
                val here = DP[i][left]
                for(plus in 0 until 3){
                    val new = left + plus
                    if(new > 2) continue
                    val newcost = here + if(new != who[i]) 1 else 0
                    DP[i+1][new] = minOf(DP[i+1][new],newcost)
                }
            }
        }
