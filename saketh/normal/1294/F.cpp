...
array<int, 2> join(int a, int b) { return {a,b}; }
array<int, 3> join(array<int, 2> a, int b) { return {a[0], a[1], b}; }

int main() {
    setIO();

    using E = edge;
    int N; re(N); tree<E> tr(N, 0); re(tr);

    struct res {
        pair<int, int>           p1 = {NINF,  -1};
        pair<int, array<int, 2>> p2 = {NINF, {-1,-1}};
        pair<int, array<int, 3>> p3 = {NINF, {-1,-1,-1}};
    };
    vector<res> dp(N);

    trav (u, tr.reverse_preorder) {
        ckmax(dp[u].p1, mp(0, u+1));
        trav (e, tr.children[u]) {
            int v = e(u);
            ckmax(dp[u].p3, dp[v].p3);
            ckmax(dp[u].p3, mp(dp[u].p2.f + dp[v].p1.f + 1, join(dp[u].p2.s, dp[v].p1.s)));
            ckmax(dp[u].p3, mp(dp[u].p1.f + dp[v].p2.f + 1, join(dp[v].p2.s, dp[u].p1.s)));
            ckmax(dp[u].p2, mp(dp[v].p2.f + 1, dp[v].p2.s));
            ckmax(dp[u].p2, mp(dp[u].p1.f + dp[v].p1.f + 1, join(dp[u].p1.s, dp[v].p1.s)));
            ckmax(dp[u].p1, mp(dp[v].p1.f + 1, dp[v].p1.s));
        }
    }

    ps(dp[0].p3.f), pc(dp[0].p3.s);
