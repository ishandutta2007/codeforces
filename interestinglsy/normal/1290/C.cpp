...
struct Dsu{
	static constexpr int DSUSZ = MAXN;
	int dsu[DSUSZ];
	il void Init( int sz ){
		For(i,sz) dsu[i] = i;
	}
...
struct ToFatherEdge{ int fa, rev; };
ToFatherEdge dsu[MAXN];
struct HeaderInfo{ int szB, szW; bool forceB, forceW; };
HeaderInfo info[MAXN];
void InitInfos(){
	For(i,k+1){
		dsu[i] = {i,0};
		info[i] = {1,0,0,0};
...
void CompressPath( int pos ){
	int fa = dsu[pos].fa;
	if( fa == pos ) return;
	CompressPath(fa);
	dsu[pos].fa = dsu[fa].fa;
	dsu[pos].rev = dsu[pos].rev ^ dsu[fa].rev;
}

int ans = 0;
il int GetAns( int pos ){
	if(info[pos].forceB) return info[pos].szB;
	else if(info[pos].forceW) return info[pos].szW;
	else return min( info[pos].szB , info[pos].szW );
}

void AddEdge( int u , int v , bool isInv ){
	if(overallDsu.Same(u,v)) return;
	overallDsu.Unite(u,v);
	CompressPath(u);
	CompressPath(v);

	if(dsu[u].rev) isInv ^= 1;
	if(dsu[v].rev) isInv ^= 1;
	u = dsu[u].fa;
	v = dsu[v].fa;

	if( info[u].forceB or info[u].forceW ){
		assert( !info[v].forceB and !info[v].forceW );
		swap(u,v);
	}
	...
	// if( info[v].forceB or info[v].forceW ){
	//
	// }else{
		info[v].szB += isInv ? info[u].szW : info[u].szB;
		info[v].szW += isInv ? info[u].szB : info[u].szW;
	// }
...
int main(){
	...
	For(i,n){
		if( isOn[i] == 0 ){
			assert(!inSets[i].empty());
			if( inSets[i].size() == 1 )
				AddEdge(inSets[i][0],k+1,1);
			else
				AddEdge(inSets[i][0],inSets[i][1],1);
		}else{
			if( inSets[i].size() == 0 ){}
			else if( inSets[i].size() == 1 )
				AddEdge(inSets[i][0],k+1,0);
			else
				AddEdge(inSets[i][0],inSets[i][1],0);
		}
		printf("%d\n",::ans);
	}
