...
void solve() {
  int n = in;
  (void)n;
  auto pts = in.vec<pair<int, int>>(3);
  sort(all(pts));
  pair<int, int> goal = in;
  if (count(all(pts), goal)) return out.YES();
  vector<vector<pair<int, int>>> stuck = {
      {{1, 1}, {1, 2}, {2, 1}},
      {{1, n - 1}, {1, n}, {2, n}},
      {{n - 1, 1}, {n, 1}, {n, 2}},
      {{n - 1, n}, {n, n - 1}, {n, n}}};
  if (count(all(stuck), pts)) {
    int i = find(all(stuck), pts) - stuck.begin();
    auto [r, c] = goal;
    bool yes = i == 0 ? r == 1 || c == 1
                      : i == 1 ? r == 1 || c == n
                               : i == 2 ? r == n || c == 1 : r == n || c == n;
    return out.YES(yes);
  }
  bool found[2][2] = {};
  for (auto [x, y] : pts) found[x % 2][y % 2] = true;
  out.YES(found[goal.first % 2][goal.second % 2]);
}
