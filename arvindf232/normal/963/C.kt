...
class rect( val x:Long, val y:Long, val counts:Long){
...
fun Map<Long,Long>.valuegcd():Long{
	var now:Long? = null
	for((i,v) in this.entries){
		if(now == null){
			now = v
		}else{
			now = gcd(now, v )
...
fun main(){
    ...
    solve.cases{
		...
		val has = mutableMapOf<Pair<Long,Long>,Long>()
		repeat(n){
			all.add(rect(getlong, getlong, getlong))
		}
		for(a in all){
			has[Pair(a.x,a.y)] = a.counts
		}
		val xx = mutableMapOf<Long, Long>()
		val yy = mutableMapOf<Long, Long>()
		for(a in all){
			xx[a.x] = xx.getOrDefault(a.x, 0L ) + a.counts
			yy[a.y] = yy.getOrDefault(a.y, 0L) + a.counts
		}
		var xg = xx.valuegcd()
		var yg = yy.valuegcd()

		var now:Long? = null
		for((x,xc) in xx.entries){
			for((y,yc) in yy.entries){
				val need1 = (xc /xg )
				val need2 = (yc /yg)
				if(need1.toDouble()  * need2.toDouble() > Long.MAX_VALUE){
					put(0)
					return@cases
				}
				val need = need1 * need2
				val have = has.getOrDefault(Pair(x,y), 0L)
				if(have % need != 0L){
					put(0)
					return@cases
				}
				val ratio = have / need
				if(now != null && now != ratio){
					put(0)
					return@cases
				}
				if(now == null){
		...
		val factors = factorSingleNumber(now!!)
		var m = 1L
		for((c,p) in factors){
			m *= (p+1)
		}
