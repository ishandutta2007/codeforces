...
int main() {
    int K;
    cin >> K;
    vector<map<lint, vector<mint>>> tree(K);
    int N;
    cin >> N;
    ...
    while (N--) {
        ...
        string s;
        cin >> v >> s;
        vector<mint> tmp{0, 0, 0};
        if (s == "white" or s == "yellow") tmp[0] += 1;
        if (s == "green" or s == "blue") tmp[1] += 1;
        if (s == "red" or s == "orange") tmp[2] += 1;
        tree[floor_lg(v)][v] = tmp;
    }

    vector<mint> free_dp(3, 2), one(3, 2);

    auto merge = [&](vector<mint> dpl, vector<mint> dpr, vector<mint> dp0) -> vector<mint> {
        vector<mint> dpnxt(3);
        REP(d, 3) REP(e, 3) REP(f, 3) {
            if (d == e or d == f) continue;
            dpnxt[d] += dp0[d] * dpl[e] * dpr[f];
        }
        return dpnxt;
    };
    IREP(d, tree.size() - 1) {
        vector<lint> xs;
        for (auto [x, d] : tree[d]) xs.push_back(x);
        for (auto [x, d] : tree[d + 1]) xs.push_back(x / 2);
        xs = sort_unique(xs);

        for (auto x : xs) {
            vector<mint> dpl, dpr, dp0;
            dpl = tree[d + 1].count(x * 2) ? tree[d + 1][x * 2] : free_dp;
            dpr = tree[d + 1].count(x * 2 + 1) ? tree[d + 1][x * 2 + 1] : free_dp;
            dp0 = tree[d].count(x) ? tree[d][x] : one;
            tree[d][x] = merge(dpl, dpr, dp0);
        }
        dbg(free_dp);
        free_dp = merge(free_dp, free_dp, one);
        dbg(free_dp);
    }
    cout << accumulate(ALL(tree[0].begin()->second), mint(0)) << '\n';
}