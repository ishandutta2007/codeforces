...
void upd(int id, int l, int r, int ql, int qr, char x, char y) {
	if (ql <= l && r <= qr) {
		for (int i = 0; i < 100; i++)
			mp[id][i] = (mp[id][i] == x ? y : mp[id][i]);
		return;
	}
