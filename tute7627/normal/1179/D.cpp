...
int main(){
  ...
  }
  {
    ...
    auto dfs=[&](auto &&f,ll k,ll par)->void{
      ...
      if(v.empty())dp[k]=0;
      for(auto z:v){
        chmin(dp[k],z.se+(sz[k]-z.fi)*(sz[k]-z.fi-1)/2);
        if(!cht.empty()){
          ll p=cht.query_monotone_inc(2*z.fi);
          //cout<<z spa p spa p+2*z.se+z.fi*z.fi-(2*n-1)*z.fi+n*n<<endl;
          chmin(res,p+2*z.se+z.fi*z.fi-(2*n-1)*z.fi+n*n-n);
        }
        cht.add(z.fi,2*z.se+z.fi*z.fi-(2*n-1)*z.fi);
      }
      //cout<<res<<endl;
      //cout<<k spa dp[k]<<endl;
      if(k==0)chmin(res,dp[k]*2);
    };
    dfs(dfs,0,-1);
    cout<<n*(n-1)-res/2<<endl;
  }
  return 0;
}