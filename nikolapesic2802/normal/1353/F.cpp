...
const int N=100;
const ll oo=LLONG_MAX/3;
ll dp[N][N][2];
int x,y,n,m;
ll a[N][N];
ll calc(int i,int j,int d){
	if(dp[i][j][d]!=-1)
		return dp[i][j][d];
	if(d==0){
		int dis=abs(x-i)+abs(y-j);
		ll treba=a[i][j]-a[x][y]+dis;
		dp[i][j][d]=oo;
		if(treba<0)
			return dp[i][j][d];
		if(i==0&&j==0)
			dp[i][j][d]=treba;
		if(i!=0)
			dp[i][j][d]=min(dp[i][j][d],calc(i-1,j,d)+treba);
		if(j!=0)
			dp[i][j][d]=min(dp[i][j][d],calc(i,j-1,d)+treba);
		return dp[i][j][d];
	}
	else{
		int dis=abs(x-i)+abs(y-j);
...
void test(){
	...
	for(x=0;x<n;x++)
		for(y=0;y<m;y++)
			memset(dp,-1,sizeof(dp)),sol=min(sol,calc(x,y,0)+calc(x,y,1));
	printf("%lld\n",sol);
}
int main()
{
