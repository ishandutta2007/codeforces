...
void solve()
{
	...
	ValueType value_init = ValueType();
	DisjointSet g(12);
	auto value_merge = [&](const ValueType& a, const ValueType& b) -> ValueType
	{
		...
		for (int k = 0; k < 3; ++k)
		{
			if (a.a[k + 3][k + 3] && b.a[k][k])
			{
				if (g.Find(k + 3) != g.Find(k + 6))
				{
					c.cnt -= 1;
					g.Union(k + 3, k + 6);
				}
		...
		for (int i = 0; i < 6; ++i)
		{
			if (i < 3 && !a.a[i][i]) continue;
			if (i >= 3 && !b.a[i][i]) continue;
			for (int j = i; j < 6; ++j)
			{
				if (j < 3 && !a.a[j][j]) continue;
				if (j >= 3 && !b.a[j][j]) continue;
				c.a[i][j] = c.a[j][i] = g.Find(i < 3 ? i : i + 6) == g.Find(j < 3 ? j : j + 6);
			}
		}
		return c;
	};
	...
	vector<ValueType> a(n, value_init);
	for (int k = 0; k < n; ++k)
	{
		DisjointSet g(3);
		for (int i = 0; i < 3; ++i)
			for (int j = 0; j < 3; ++j)
			{
				if (abs(i - j) == 1 && v[i][k] == '1' && v[j][k] == '1')
					g.Union(i, j);
			}
		for (int i = 0; i < 6; ++i)
			for (int j = 0; j < 6; ++j)
			{
				a[k].a[i][j] = v[i % 3][k] == '1' && v[j % 3][k] == '1' && g.Find(i % 3) == g.Find(j % 3);
			}
		a[k].cnt = 0;
		for (int i = 0; i < 3; ++i)
		{
