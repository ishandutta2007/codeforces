...
int main() {
  ...
  while (t--) {
    ...
    for (int i = m - 1; i >= 0; --i) {
      const int pos = bit.lower_bound(y[i] + 1);
      bit.add(pos, -1);
      a.emplace_back(pos, x[i]);
    }
    sort(ALL(a));
    int z = n - 1 - int(a.size());
    if (!a.empty() && a.back().first + 1 == n) ++z;
    REP(i, int(a.size()) - 1) {
      if (a[i].first + 1 == a[i + 1].first && a[i].second < a[i + 1].second) ++z;
    }
    cout << ModInt::nCk(n + z, n) << '\n';
    for (auto [pos, _] : a) bit.add(pos, 1);
  }
  return 0;
}