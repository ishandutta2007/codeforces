...
int LIS(vector<int> v){
    int n = sz(v);
    vector<int> dp(n, inf);
    each(e, v) *upper_bound(all(dp), e) = e;
...
int main(){
    ...
    vi b(K+2);
    b[0] = -1, b[K+1] = N;
    rep2(i, 1, K) in(b[i]), b[i]--;
    int ans = N-K;
    rep(i, K+1){
        int l = (b[i] == -1? -inf : a[b[i]]), r = (b[i+1] == N? inf : a[b[i+1]]);
        if(l > r) {out(-1); return 0;}
        vector<int> v;
        rep2(j, b[i]+1, b[i+1]-1){
            if(l <= a[j] && a[j] <= r) v.eb(a[j]);
        }
        ans -= LIS(v);
    }
    out(ans);
