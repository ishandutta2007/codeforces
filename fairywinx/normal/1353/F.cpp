...
void solve() {
	...
	for(int i = 0; i < n; ++i) {
		for(int j = 0; j < m; ++j) {
			if (get_time() + o / 10 > 2.3)
				break;
			long long c = a[i][j] + (n - 1 - i) + (m - 1 - j);
			if (c > a[n - 1][m - 1])
				continue;
			...
			d[n - 1][m - 1] = a[n - 1][m - 1] - c;
			q.push({a[n - 1][m - 1] - c, make_pair(n - 1, m - 1)});
			for(int u = 0; u < n; ++u) {
				for(int y = 0; y < m; ++y) {
					if (u == n - 1 && y == m - 1)
						continue;
			...
			while(q.size()) {
				...
				if (now.first - 1 >= 0) {
					int x = now.first - 1;
					int y = now.second;
					long long z = c - abs(n - 1 - x) - abs(m - 1 - y);
					if (a[x][y] >= z && d[x][y] > pr + a[x][y] - z) {
						d[x][y] = pr + a[x][y] - z;
						q.push({d[x][y], {x, y}});
					}
				}
