    ...
    int a[500001];
        ...
        int main()
        {
         ...
         while(t--)
         {
             cin>>s>>ss;for(int i=0;i<27;i++)a[i]=0;yes=false;
             for(int i=0;i<s.size();i++){int ind=int(s[i]-'a');a[ind]=1;}
             for(int i=0;i<ss.size();i++){int ind=int(ss[i]-'a');if(a[ind])yes=true;}
             if(yes)cout<<"YES\n";
             else cout<<"NO\n";
         }
