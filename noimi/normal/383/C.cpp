...
main() {
    ...
    auto g = getTree(n);
    auto f = plus<int>();
    DuelSegmentTree<int> seg1(n, plus<int>(), 0), seg2(n, plus<int>(), 0);
    vi INa(n), OUT(n), d(n);
    {
        auto dfs = [&](auto &&f, int x, int p, int &t, int de = 0) -> void {
            INa[x] = t++;
            d[x] = de;
            for(auto e : g[x]) {
                if(e != p) f(f, e, x, t, de + 1);
            }
            OUT[x] = t;
        };
        int times = 0;
    ...
    rep(i, n) {
        int a = A[i];
        seg1.update(INa[i], INa[i] + 1, a);
        seg2.update(INa[i], INa[i] + 1, a);
    }
    while(m--) {
        INT(id);
        if(id - 1) {
            ...
            x--;
            if(d[x] & 1)
                cout << seg1[INa[x]] << endl;
            else
                cout << seg2[INa[x]] << endl;
        } else {
            INT(x, y);
            x--;
            if(d[x] & 1) {
                seg1.update(INa[x], OUT[x], y);
                seg2.update(INa[x], OUT[x], -y);
            } else {
                seg1.update(INa[x], OUT[x], -y);
                seg2.update(INa[x], OUT[x], y);
            }
