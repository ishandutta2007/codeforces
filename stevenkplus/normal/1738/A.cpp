...
int main() {
   ...
   while (T--) {
      ...
      for (int &i: w) cin >> i, tot += i;
      vector<vector<int>> vals(2);
      for (int i = 0; i < n; ++i) {
         vals[v[i]].push_back(w[i]);
      }
      for (int i = 0; i < 2; ++i) {
      ...
      ll ans = 0;
      for (int i = 0; i < 2; ++i) {
         // start with i. get the max(len(vals[j]), len(vals[i]) - 1) biggest of vals[i]
         // get the max(len(vals[i]), len(vals[j]) biggest of vals[j]
         int j = 1 - i;
         ll sum = tot;
         for (int x = 0; x < sz(vals[j]) && x < sz(vals[i]) - 1; ++x) {
            sum += vals[i][x];
         }
         for (int x = 0; x < sz(vals[j]) && x < sz(vals[i]); ++x) {
            sum += vals[j][x];
         }
         ans = max(ans, sum);
      }
