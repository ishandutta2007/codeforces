...
int initw[2];
modular dp0[N][W][N][N]; // EV of weight of a 0-type after i visits, initially having weight j, total weight of 0-type is initw[0] - k, total weight of 1-type is initw[1] + l
modular dp1[N][W][N][N]; // EV of weight of a 1-type after i visits, initially having weight j, total weight of 0-type is initw[0] - k, total weight of 1-type is initw[1] + l
...
signed main(){
    ...
    For(j, 1, W){
        ForE(k, 0, (m - 0)){
            int l = (m - 0) - k;
            int curw[2] = {initw[0] - k, initw[1] + l};
            if (curw[0] < 0 or j > curw[0]){
                continue;
            }
            dp0[0][j][k][l] = j;
        }
    ...
    ForE(i, 1, m){
        For(j, 1, W){
            ForE(k, 0, (m - i)){
                ...
                dp0[i][j][k][l] = ((modular)j / (modular)(curw[0] + curw[1])) * dp0[i - 1][j - 1][k + 1][l];
                dp0[i][j][k][l] += ((modular)(curw[0] - j) / (modular)(curw[0] + curw[1])) * dp0[i - 1][j][k + 1][l];
                dp0[i][j][k][l] += ((modular)curw[1] / (modular)(curw[0] + curw[1])) * dp0[i - 1][j][k][l + 1];
            }
        }
    }
    ...
    For(j, 1, W){
        ForE(k, 0, (m - 0)){
            int l = (m - 0) - k;
            int curw[2] = {initw[0] - k, initw[1] + l};
            if (curw[0] < 0 or j > curw[1] or j > 50 + (m - 0)){
                continue;
            }
            dp1[0][j][k][l] = j;
        }
    ...
    ForE(i, 1, m){
        For(j, 1, W){
            ForE(k, 0, (m - i)){
                int l = (m - i) - k;
                int curw[2] = {initw[0] - k, initw[1] + l};
                if (curw[0] < 0 or j > curw[1] or j > 50 + (m - i)){
                    continue;
                }
                dp1[i][j][k][l] = ((modular)j / (modular)(curw[0] + curw[1])) * dp1[i - 1][j + 1][k][l + 1];
                dp1[i][j][k][l] += ((modular)(curw[1] - j) / (modular)(curw[0] + curw[1])) * dp1[i - 1][j][k][l + 1];
                dp1[i][j][k][l] += ((modular)curw[0] / (modular)(curw[0] + curw[1])) * dp1[i - 1][j][k + 1][l];
            }
        }
    }
    ...
    ForE(i, 1, n){
        if (a[i] == 0){
            cout << dp0[m][w[i]][0][0] << endl;
        }
        else{
            cout << dp1[m][w[i]][0][0] << endl;
        }
    }
}
