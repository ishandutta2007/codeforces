...
static ll n, a[14], ad[14][14], dp[1<<14];
static map<pll, ll> mp;

ll xd(ll t, ll x)
{
    if(mp.count(MP(t, x))) return mp[MP(t, x)];
    if(x == 0) return mp[MP(t, x)] = 0;
    ...
    for(ll tt = ((x-1)&x); tt != x; tt = ((tt-1)&x))
    {
        pr2 = pr2 + M1 - xd(x, tt);
    }

    return mp[MP(t, x)] = (pr*pr2)%M1;
}
...
int main()
{
    ...
    REP(t,1,(1<<n))
    {
        for(ll x = (t-1)&t; x != 0; x = ((x-1)&t))
        {
            ...
            if(t == (1<<n)-1)
            {
                ans += (pr*__builtin_popcountll(x))%M1;
            }
        }
        dp[t] = 1 + M1 - dp[t]%M1;
        if(t == (1<<n)-1)
        {
            ans += (dp[t]*__builtin_popcountll(t))%M1;
        }
    }
