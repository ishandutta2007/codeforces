...
int main() {
	...
	V<vmi> dp(N,vmi(N+1));
	dp.at(0).at(0) = 1;
	mi ans = 1; // (N+1 ... N+1) and (N ... N)
	ROF(mn,1,N+1) {
		// condition: at least N-mn numbers >= mn+2
		// difference: 2+3+...+N+1-mn
		if ((N+3-mn)*(N-mn)/2 > mn) break;
		{ // add any amount with mn+1
			V<vmi> ndp(N,vmi(N+1));
			FOR(num,N-mn,N) F0R(sum,N) FOR(nnum,num,N) {
				ndp.at(nnum).at(sum) += ifac.at(nnum-num)*dp.at(num).at(sum);
			}
			swap(dp,ndp);
		}
		{
			V<vmi> ndp(N,vmi(N+1));
			F0R(num,N) F0R(sum,N) if (num+sum <= mn) {
				// dbg("YY");
				ndp.at(num).at(num+sum) += dp.at(num).at(sum);
			}
			swap(dp,ndp);
		}
		// condition: sum <= mn
		F0R(num,N) F0R(sum,N+1) {
			// dbg("ZZ",N-num,num,sum);
			ans += fac.at(N)*ifac.at(N-num)*dp.at(num).at(sum);
		}
		// dbg("GOT",mn,ans);
	}
	// for (int mn = N; mn >= 1; --mn) {
	// 	// in all solutions, must have at least N-mn numbers with value > mn
	// 	// dbg("AA");
	// 	// (N+1)*a_1 >= a_1+a_2+...+a_N
	// 	// sum(differences) <= a_1
	// 	// if (mn == 2) dbg("??",dp.at(1).at(2));

	// 	// >= N+1
	// 	// >= N-
	// 	ROF(num,N-mn,N) F0R(sum,mn+1) { // transition
	// 		FOR(nnum,num+1,N+1) {
	// 			mi mul = dp.at(num).at(sum)*ifac.at(nnum-num);
	// 			// if (mul == 0) continue;
	// 			if (nnum == N) {
	// 				// if (mn == 2 && num == 1 &&)
	// 				ans += mul*fac.at(N);
	// 			} else dp.at(nnum).at(sum) += mul;
	// 		}
	// 	}
	// 	// dbg("GOT",mn,ans);
	// 	V<vmi> ndp(N,vmi(N+1));
	// 	F0R(i,N) F0R(j,N+1) if (j+i <= N) {
	// 		ndp.at(i).at(j+i) = dp.at(i).at(j);
	// 	}
	// 	swap(dp,ndp);
	// }
	ps(ans);
