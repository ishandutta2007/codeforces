...
ans_type solve() {
    auto ok = [&](llong ans) {
        vector<llong> cur_accepted(n + 1);
        for (int u = n; u >= 1; u--) {
            if (len(children[u]) == 0) {
                if (citizen[u] > ans) return false;
                cur_accepted[u] = ans - citizen[u];
                continue;
            }
            for (auto v: children[u]) cur_accepted[u] += cur_accepted[v];
            cur_accepted[u] -= citizen[u];
            if (cur_accepted[u] < 0) return false;
        }
        return true;
    };
    llong l = 0, r = 1;
