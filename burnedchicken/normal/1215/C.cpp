...
int main(){
	...
	cin >> n >> str1 >> str2;
	rep(n){
		if(str1[i]=='a'&&str2[i]=='b') vec1.pb(i+1);
		else if(str1[i]=='b'&&str2[i]=='a') vec2.pb(i+1);
	}
	x=vec1.size(),y=vec2.size();
	if(x+y&1){
		print1(-1);
		return 0;
	}
	print1((x>>1)+(y>>1)+2*(x&1));
	rep(x>>1) print2(vec1[i<<1],vec1[(i<<1)+1]);
	rep(y>>1) print2(vec2[i<<1],vec2[(i<<1)+1]);
	if(x&1){
		print2(vec1.back(),vec1.back());
		print2(vec1.back(),vec2.back());
	}
	return 0;
}