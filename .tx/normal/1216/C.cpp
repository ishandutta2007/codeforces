...
template <typename T, T val = T()>
auto make_vector(size_t d, auto... ds) {
...
void solve() {
    ...
    auto ins = [&](int p, int q, int i) {
        return x[i * 2] <= p && p <= x[i * 2 + 1] && y[i * 2] <= q && q <= y[i * 2 + 1];
    };

    auto check = [](bool f, string ans) {
        if (f) {
            cout << ans << "\n";
            cout.flush();
            exit(0);
    ...
    check(ins(x[0], y[0], 1) && ins(x[1], y[1], 1), "NO");
    check(ins(x[0], y[0], 2) && ins(x[1], y[1], 2), "NO");

    check(!ins(x[0], y[0], 1) && !ins(x[0], y[0], 2), "YES");
    check(!ins(x[0], y[1], 1) && !ins(x[0], y[1], 2), "YES");
    check(!ins(x[1], y[0], 1) && !ins(x[1], y[0], 2), "YES");
    check(!ins(x[1], y[1], 1) && !ins(x[1], y[1], 2), "YES");

    check(min(y[2], y[3]) > max(y[4], y[5]) || min(y[4], y[5]) > max(y[2], y[3]), "YES");
    check(min(x[2], x[3]) > max(x[4], x[5]) || min(x[4], x[5]) > max(x[2], x[3]), "YES");
