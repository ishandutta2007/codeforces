...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	rep(i,n){
		gcd0[0][i]=__gcd<ll>(vs[i],(par[0][i]>=0?vs[par[0][i]]:0ll));
	}
	rep(i,19)rep(v,n){
		if(par[i][v]>=0){
			gcd0[i+1][v]=__gcd<ll>(gcd0[i][v],gcd0[i][par[i][v]]);
	...
	// rep(i,n){rep(j,5) cout<<par[j][i]<<",";cout<<endl;}
	mint re=0;
	rep(v,n){
		int cur=v;
		ll gc=vs[v];
		while(cur>=0){
			gc=__gcd<ll>(vs[cur],gc);
			int nxt=cur;
			int sz=0;
			rrep(i,20)if(par[i][nxt]>=0 && (gc==0 && gcd0[i][nxt]==0 || gc!=0 && gcd0[i][nxt]%gc==0)){
				nxt=par[i][nxt];
				sz+=1<<i;
			}
			re+=mint(gc%MOD)*(sz+1);
			out(v,cur,nxt,sz,gc,1);
			cur=par[0][nxt];
		}
	}
	cout<<re<<endl;
