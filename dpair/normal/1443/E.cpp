...
inline void solve(LL x, int N) {
    memset(vis, 0, sizeof(vis));
    rep(i, 1, N) { int it = 1;
        while(x >= fac[N - i]) ++ it, x -= fac[N - i];
        a[i] = find(it, N); vis[a[i]] = 1;
    }
}

signed main(){
    read(n, q); const int lim = mx(n - 21, 0);
    fac[0] = 1; rep(i, 1, n - lim) fac[i] = fac[i - 1] * i;
    solve(0, n - lim);
    while(q --) {
        int opt = read();
        if(opt == 1) {
            int l, r; read(l, r); LL ans = 0;
            if(r > lim) rep(i, mx(lim + 1, l), r) ans += a[i - lim] + lim;
            if(l <= lim) ans += (1ll * (l + mn(lim, r)) * (mn(lim, r) - l + 1) >> 1);
            print(ans);
        } else {
            int x = read(); m += x;
            solve(m, n - lim);
