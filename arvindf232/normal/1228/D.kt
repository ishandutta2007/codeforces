...
fun main(){
    solve.cases{
        ...
        val stuff = Array(3){ mutableListOf<Int>() }
        var bad = false
        val used = BooleanArray(n)
        var comp = -1
        done@for(i in 0 until n){
            if(used[i] ) continue
            comp ++
            if(comp >= 3){
                bad= true
            ...
            val negate = BooleanArray(n){true}
            G.NS(i){
                v ->
                negate[v] = false
            }
            for(i in negate.indices){
                if(negate[i] == false) continue
                if(used[i]){
                    bad = true
                    break@done
                }
        ...
        val comps = stuff.map { it.toSet() }
        val without = IntArray(4)
        G.everyEdge { a, b ->
            val c = (0..2).first{comps[it].contains(a)}
            val c2 = (0..2).first{comps[it].contains(b)}
            if(c == c2){
                bad = true
                return@everyEdge
            }
            val other = (0..2).first{it != c && it != c2 }
            without[other] ++
        }
        for(i in 0..2){
            val except = (0..2).filter{it != i }
            val expects = 1L * comps[except[0]].size * comps[except[1]].size * 2
            if(expects != without[i].toLong()){
                bad= true
            }
        }
        if(bad){
