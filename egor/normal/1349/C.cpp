...
public:
    void solve() {
        ...
        int m = in.readInt();
        int t = in.readInt();
        auto mp = in.readCharTable(n, m);
        for (char& d : mp) {
            d -= '0';
        }

        arr2d<int> join(n, m, -1);
        for (int i : range(n)) {
            for (int j : range(m)) {
        ...
        while (!q.empty()) {
            pii cur = q.pop();
            for (int i : range(4)) {
                int r = cur.first + DX4[i];
                int c = cur.second + DY4[i];
                if (isValidCell(r, c, n, m) && join(r, c) == -1) {
                    join(r, c) = join[cur] + 1;
                    q.emplace(r, c);
                }
            }
        ...
        for (int x : range(t)) {
            ...
            int c = in.readInt() - 1;
            ll p = in.readLong();
            if (join(r, c) == -1 || p < join(r, c) || (p - join(r, c)) % 2 == 0) {
                out.printLine(int(mp(r, c)));
            } else {
                out.printLine(int(1 - mp(r, c)));
            }
        }
    }
};
