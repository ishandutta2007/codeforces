...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < n; i++) a[i] = t[i] - '0';
	st.init(n, a);
	
	bool pos = 1;
	for (ll i = q - 1; i >= 0; i--) {
		ll v = st.query(lv[i], rv[i]).val;
		
		ll p = (rv[i] - lv[i]) / 2;
				
		if (v <= p) {
			st.update(lv[i], rv[i], 0);
		} else if ((rv[i] - lv[i] + 1) - v <= p) {
			st.update(lv[i], rv[i], 1);
		} else pos = 0;
	}
	
	for (ll i = 0; i < n; i++) pos &= (st.query(i, i).val == s[i] - '0');
