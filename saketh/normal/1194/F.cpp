...
int main() {
    ...
    F0R (i, N) {
        ...
        ll loa = i + 1 < N ? max(0ll, T - total - time[i+1] + 1) : 0;
        ll hib = min(T - total, ll(i+1));

        for (ll b = loa - 1; b <= hib; b++) {
            if (b >= loa) ans += mn::ncr(i + 1, b) / mn(2).pow(i + 1) * (i + 1);
            else ans += mn::ncr(i + 1, b) / mn(2).pow(i + 1) * (i + 1) / 2;
        }
    }
