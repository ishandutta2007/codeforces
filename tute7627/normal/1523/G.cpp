...
namespace RMQ2D{
  ...
  using I = int;
  auto merge=[](I x,I y){return min(x,y);};
...
int main(){
  ...
  bool judge = true;
  int n,m;cin>>n>>m;
  auto seg=RMQ2D::make(n);
  vector<int>l(m),r(m);
  vector<vector<int>>gq(n+1);
  rep(i,0,m){
    cin>>l[i]>>r[i];l[i]--;r[i]--;
    seg.preupdate(l[i],r[i]);
  }
  rrep(i,0,m){
    gq[r[i]-l[i]+1].PB(i);
  }
  ...
  rrep(i,1,n+1){
    ...
    for(auto z:gq[i]){
      seg.update(l[z],r[z],z);
    }
    auto dfs=[&](auto &&f,int nl,int nr)->void{
      if(nl+i>nr)return;
      int nxt=seg.query(nl,nr,nl,nr);
      if(nxt==RMQ2D::identity)return;
      ret[i-1]+=r[nxt]-l[nxt]+1;
      f(f,nl,l[nxt]);
      f(f,r[nxt]+1,nr);
    };
    dfs(dfs,0,n);
