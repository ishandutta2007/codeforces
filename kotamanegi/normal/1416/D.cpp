...
#define int ll
int used[500000];
int erased_edge[500000];
int val[500000];
vector<int> vertexs[500000];
set<int> datas[1000000];
int itr = 0;
int Going[500000];
void constructor(int now) {
	queue<int> next;
	next.push(now);
	Going[now] = itr;
	if (used[val[now]] == 0) {
		datas[itr].insert(-val[now]);
	}
	while (!next.empty()) {
		int now = next.front();
		next.pop();
...
void solve() {
	...
	vector<pair<int, int>> pre_query;
	REP(i, query) {
		int a, b;
		cin >> a >> b;
		b--;
		pre_query.push_back(mp(a, b));
		if (a == 2) {
			erased_edge[b] = i;
		}
	}
	REP(i, m) {
		if (erased_edge[i] == -1) {
			erased_edge[i] = pre_query.size();
			pre_query.push_back(mp(2, i));
		}
	}
	vector<pair<int, int>> Query;
	dsu hoge(n);
	//reconstruct First!
	for (int i = pre_query.size() - 1; i >= 0; --i) {
		if (pre_query[i].first == 1) {
			Query.push_back(pre_query[i]);
			continue;
		}
		int target = pre_query[i].second;
		if (hoge.size(edges[target].first) > hoge.size(edges[target].second)) {
			swap(edges[target].first, edges[target].second);
		}
		if (hoge.same(edges[target].first,edges[target].second) == 0) {
			hoge.merge(edges[target].first, edges[target].second);
			vertexs[edges[target].first].push_back(edges[target].second);
			vertexs[edges[target].second].push_back(edges[target].first);
			Query.push_back(pre_query[i]);
		}
	}
	reverse(ALL(Query));
	set<int> Already;
	REP(i, n) {
		if (Already.count(hoge.leader(i))) continue;
		Already.insert(hoge.leader(i));
		constructor(i);
	}
	REP(i, Query.size()) {
		if (Query[i].first == 1) {
			while (true) {
				if (datas[Going[Query[i].second]].empty() == true) {
					cout << 0 << endl;
					break;
				}
				int go = -(*datas[Going[Query[i].second]].begin());
				datas[Going[Query[i].second]].erase(datas[Going[Query[i].second]].begin());
				if (used[go] == 0) {
					used[go] = 1;
					cout << go << "\n";
					break;
			...
			int lefts = edges[Query[i].second].first;
			int rights = edges[Query[i].second].second;
			vertexs[lefts].pop_back();
			vertexs[rights].pop_back();
			constructor(lefts);
			for (auto x : datas[itr - 1]) {
				int targets = Going[rights];
				int geko = x;
				datas[targets].erase(geko);
			}
		}
	}
}
