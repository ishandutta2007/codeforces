...
mint solve(const string s){
	...
	vector<int> nxt(n,n);
	{
		vector<pii> stk{{MOD,n}};
		rrep(i,n){
			while(stk.back().X<=vs[i]) stk.pop_back();
			nxt[i]=stk.back().Y;
			stk.eb(vs[i],i);
		}
	}
	out(vs,nxt,1);
	vector<mint> tanomu(n);
	vector<mint> dp(n+1); dp[1]=vs[0]+1;
	reps(i,1,n-1){
		for(int j=i,prv=-1;j<n-1;j=nxt[j]){
			dp[j+1]+=dp[i]*(vs[j]-prv);
			out(i,j,vs[j],prv,dp[i]*(vs[j]-prv),1);
			prv=vs[j];
		}
	}
	// rep(i,n){
	...
	// 	tanomu[i]+=dp[i];
	// 	if(nxt[i]<n){
	// 		dp[nxt[i]+1]+=tanomu[i]*(vs[nxt[i]]-vs[i]);
	// 		tanomu[nxt[i]]+=tanomu[i];
	// 	}
	// }
	auto re=accumulate(all(dp),mint(0));
	out(dp,re,1);
	re*=vs.back()+1;
	return re;
}
