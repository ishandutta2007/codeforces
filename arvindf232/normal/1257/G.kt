            ...
            while(len <= n){
                ...
                while(i < n){
                    ...
                    for(j in 0 until (len shr 1)){
                        val u = a[i+j]
                        val v = (1L * a[i+j+(len shr 1)] * w % fftmod).toInt()
                        a[i+j] = if(u+v < fftmod) u + v else u + v - fftmod
                        a[i+j+(len shr 1)] = if(u-v >= 0) u-v else u-v+fftmod
...
fun main(){
    solve.cases{
        ...
        val all = occurs.toIntArray()

        fun solve(l:Int, r:Int):IntArray{
            if(l == r) return IntArray(all[l]+1){1}
            val mid = (l+r)/ 2
