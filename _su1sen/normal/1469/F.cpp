...
pair<long long, int> op(pair<long long, int> x, pair<long long, int> y) {
    return { x.first + y.first, x.second + y.second };
}
pair<long long, int> e() {
    return { 0, 0 };
}
pair<long long, int> mapping(long long f, pair<long long, int> x) {
...
int main() {
    ...
    long long sum = accumulate(all(l), 0LL);
    if (sum - 2 * n + 1 < k) {
        print(-1);
        return 0;
    }
    ...
    vector<pair<long long, int>> init(L, { 0, 1 });
    init[0].first = 1;
    atcoder::lazy_segtree<pair<long long, int>, op, e, long long, mapping, composition, id> seg(init);

    int res = L;
    int d = 0;
    rep(i, n) {
        int r = seg.max_right(0, [k](auto v) { return v.first < k; });
        if (r != L) {
            chmin(res, r);
        }

        while (seg.get(d).first == 0) ++d;
        seg.apply(d, -1);

        const int lc = (l[i] - 1) / 2;
        const int rc = l[i] - 1 - lc;
        seg.apply(d + 2, d + 2 + lc, 1);
        seg.apply(d + 2, d + 2 + rc, 1);
    }
    int r = seg.max_right(0, [k](auto v) { return v.first < k; });
    if (r != L) {
        chmin(res, r);
