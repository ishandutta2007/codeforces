...
bitset<1 << 21> used[202];
int len = 20;
string s[201];
string last[201];
string first[201];
int encode(int len, int mask) {
    int cnt = (1 << len);
    int from = (1 << len) - 1;
//    DBN(len, from);
    return from + mask;
...
int solve() {
    int n;
    cin >> n;
    auto process = [](int idx, const string& s) {
        for (int j = 0; j < s.size(); ++j) {
            int mask = 0;
            for (int k = 1; j + k <= s.size() && k <= 19; ++k) {
                mask = mask * 2 + s[j + k - 1] - '0';
                used[idx][encode(k, mask)] = true;
            }
        }
    };
    FI(n) {
        ...
        used[i][0] = true;
        last[i] = first[i] = s[i];
        while (first[i].size() > 20) {
            first[i].pop_back();
        }
        if (last[i].size() > 20) {
            last[i] = last[i].substr(last[i].size() - 20);
        }
        process(i, s[i]);
//        for (int j = 0; j < 8; ++j) {
//            DBN(i, j, used[i][j]);
...
//        cout << endl;
    ...
    FI(m) {
        ...
        --a; --b;
        int cur = n + i;
        used[cur] = used[a] | used[b];
        first[cur] = first[a] + first[b];
        while (first[cur].size() > 20) {
            first[cur].pop_back();
        }
        last[cur] = last[a] + last[b];
        if (last[cur].size() > 20) {
            last[cur] = last[cur].substr(last[cur].size() - 20);
        }
        string concat = last[a] + first[b];
//        DBN(concat);
        process(cur, concat);
        int k = 0;
        int firstZero = 0;
        while (used[cur][firstZero]) ++firstZero;
//        DBN(firstZero);
        while (k < 19) {
            int cnt = 1 << (k + 1);
            if (firstZero > cnt) {
