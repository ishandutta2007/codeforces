...
void solve(){
  ...
  for(li i = 0; i < (li)(n-1); ++i) {
    ...
    G[y].emplace_back(i);
    if(v != -1) {
      if(!uf.unite(x,y,__builtin_popcountll(v)&1)) {
        ok = 0;
      }
    }
  }
  ...
  auto dfs = letrec([&](auto dfs, int i, int p) -> void {
    for(int e : G[i]) if(int j = es[e]^i;1) if(j != p) {
        if(uf.find(i) != uf.find(j)) {
          uf.unite(i,j,0);
        }
        uf.find(i); uf.find(j);
        if(label[e] == -1) {
          label[e] = uf.P[i]^uf.P[j];
        }
        dfs(j,i);
      }
  });
