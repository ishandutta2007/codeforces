...
public class Main {
    ...
    static class TaskD {
        ...
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            Arrays.fill(hash2, -1);
            size = new int[2 * n - 2];
            Set<LongLongPair> distinct = new HashSet<>();
            long[] pow1 =
                    IntegerUtils.generatePowers(AbstractStringHash.MULTIPLIER, 2 * n + 1, AbstractStringHash.FIRST_MOD);
            long[] pow2 =
                    IntegerUtils.generatePowers(AbstractStringHash.MULTIPLIER, 2 * n + 1, AbstractStringHash.FIRST_MOD);
            degree = new int[n];
            for (int i = 0; i < n; i++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                long res1 = calculateHash(i, -1, hash1, AbstractStringHash.FIRST_MOD, pow1).first;
                long res2 = calculateHash(i, -1, hash2, AbstractStringHash.SECOND_MOD, pow2).first;
                distinct.add(new LongLongPair(res1, res2));
            }
            out.printLine(distinct.size());
        }

        private LongIntPair calculateHash(int vertex, int last, long[] hash, long mod, long[] pow) {
            int length = last == -1 ? degree[vertex] : degree[vertex] - 1;
            long[] hDown = new long[length];
            int[] sDown = new int[length];
            int at = 0;
            for (int i = graph.firstOutbound(vertex); i != -1; i = graph.nextOutbound(i)) {
                ...
                if (hash[i] == -1) {
                    LongIntPair pair = calculateHash(next, vertex, hash, mod, pow);
                    hash[i] = pair.first;
                    size[i] = pair.second;
                }
                hDown[at] = hash[i];
            ...
            int[] order = ArrayUtils.order(hDown);
            long result = 0;
            int size = 1;
            for (int i : order) {
                result += pow[size] * hDown[i] % mod;
                size += sDown[i];
            }
            result += pow[size++];
            result %= mod;
            return new LongIntPair(result, size);
        }
