...
mint solve() {
	...
	VEC(ll, x, n);
	mint res = 0;
	// foa(t, x) foa(s, x) foa(u, x) { res += ((mint)(t & s)) * (mint)(s | u); }
	int bitmax = 60;
	...
	vi cnt(bitmax);
	rep(i, bitmax) foa(t, x) { cnt[i] += (t >> i) & 1; }
	debug(cnt);
	V<mint> nibeki(bitmax);
	nibeki[0] = 1;
	rep(i, bitmax - 1) { nibeki[i + 1] = nibeki[i] * 2; }
	mint pre = 0;
	foa(t, x) {
		mint sum = 0;
		rep(i, bitmax) {
			if((t >> i) & 1) {
				sum += nibeki[i] * cnt[i];
				if(n == 1) debug(i, sum, nibeki[i], cnt[i]);
			}
		}
		debug(sum);
		pre += sum;
