...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	...
	cauto g=readG(n);
	cauto vs=readV(n);
	using AR=array<int,40>;
	AR zero; fill(all(zero),0);
	vector<AR> sums(n,zero);
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p)->void{
				for(cauto u:g[v])if(u!=p){
						dfs(dfs,u,v);
						rep(i,t+t-1) sums[v][i+1]^=sums[u][i];
						sums[v][0]^=sums[u][t+t-1];
					}
				sums[v][0]^=vs[v];
			};
		dfs(dfs,0,-1);
	...
	{
		cauto dfs=
			[&](auto &&dfs,int v,int p,AR &ar)->void{
				rep(i,t+t) ar[i]^=sums[v][i];
				if(accumulate(ar.begin()+t,ar.begin()+t+t,0,bit_xor<int>())) re[v]=1;
				out(v,ar,1);
				for(cauto u:g[v])if(u!=p){
						AR nxt=zero;
						rep(i,t+t) nxt[(i+2)%(t+t)]=ar[(i+1)%(t+t)]^sums[u][i];
						dfs(dfs,u,v,nxt);
					}
			};
		dfs(dfs,0,-1,zero);
