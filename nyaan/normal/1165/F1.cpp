...
bool ok(vpii &w, int day){
    ...
    while(i > 0){
        if(money == 0) break;
        int loopflg = 1;
        while(it!=w.rend() && loopflg){
            if(money == 0) break;
            while(it!=w.rend() && it->fi > i) it++;
            if(it==w.rend())break;
            if(it->fi < i) break;
            while(it!=w.rend() && it->fi == i){
                if(money == 0) break;
                if(v2[it->se - 1] < v[it->se - 1]){
                    v2[it->se - 1]++;
                    money--; cnt--;
                    vdbg(v2,N); trc(i,money,cnt);
                }
                else if( (it+1)!= w.rend() && (it+1)->fi == i )it++;
                else{
                    loopflg = 0;
                    break;
                }
    ...
    trc(money,cnt,day);
    if(cnt * 2 <= day - vsum(v,N) + cnt) return true;
    else return false;
}

inline int bs(vpii &w){
    int low = 0, high = 2 * vsum(v,N) + 1; // high = 
    //  (1)low < med < hight (2) !ok(low) && ok(high)
    while(low + 1 < high){
        int med = (low + high) / 2;
        if(ok(w,med))high = med; else low = med;
    }
    return high; // (()) or ()
    // return low; // ()) or (-1)
}
