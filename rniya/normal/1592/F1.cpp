...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            int cur = 0;
            if (i + 1 < n) cur ^= a[i + 1][j];
            if (j + 1 < m) cur ^= a[i][j + 1];
            if (i + 1 < n && j + 1 < m) cur ^= a[i + 1][j + 1];
            need[i][j] = cur != a[i][j];
            sum += need[i][j];
        }
    }

    CumulativeSum2D<int> CS(need);
    int diff = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            int area = 1, x = need[n - 1][m - 1];
            if (i - 1 >= 0) area++, x += need[i - 1][m - 1];
            if (j - 1 >= 0) area++, x += need[n - 1][j - 1];
            if (i - 1 >= 0 && j - 1 >= 0) area++, x += need[i - 1][j - 1];
            chmin(diff, -x + (area - x) + 3);
        }
    }
    int ans = sum + diff;
