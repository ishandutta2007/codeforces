...
struct Cow{ int l,r; };
vc<Cow> cows[MAXN];
int l_d[MAXN][MAXN], r_d[MAXN][MAXN];
...
void Readin(){
	...
	static vc<int> minHunger[MAXN];
	For(i,m){
		iRead2(col,hun);
		minHunger[col].Eb(hun);
	}
	For(i,n){
		sort(All(minHunger[i]));
		// while( minHunger[i].size() > 2 ) minHunger[i].pop_back();
		for( auto hun : minHunger[i] ){
			if( hun > poses[i].size() ) continue;
			int l = poses[i][hun-1];
			int r = poses[i][poses[i].size()-hun];
			cows[i].Pb({l,r});
		}
	}
	For(col,n){
		auto l_d = ::l_d[col];
		auto r_d = ::r_d[col];
		for( auto [l,r] : cows[col] ){
			++l_d[l]; ++r_d[r];
		}
		For(i,n) l_d[i] += l_d[i-1];
		fOR(i,n) r_d[i] += r_d[i+1];
	}
}
...
int main(){
	...
	Forx(maxL,0,n){
		int tans = 0;
		For(col,n){
			if(cows[col].empty()) continue;
			if( cows[col].size() == 1 ){
				if( cows[col][0].l <= maxL or cows[col][0].r > maxL ) ++tans;
			}else{
				Cow c1 = cows[col][0], c2 = cows[col][1];
				if( (c1.l <= maxL and c2.r > maxL) or (c2.l <= maxL and c1.r > maxL) ) tans += 2;
				else if( c1.l <= maxL or c1.r > maxL or c2.l <= maxL or c2.r > maxL ) ++tans;
			}
		}
		MyMax(mx,tans);
	}
	...
	For(maxL,n){
		...
		if(cows[maxLCol].empty()) continue;
		if( cows[maxLCol].size() == 1 ){
			Cow c = cows[maxLCol][0];
			if( c.l != maxL ) continue;
			++canGet;
		}else{
			bool found = 0;
			for( auto [l,r] : cows[maxLCol] ) found |= l == maxL;
			if(!found) continue;
			int ano = 0;
			for( auto [l,r] : cows[maxLCol] ){
				if( l == maxL ) continue;
				ano += r > maxL;
			}
			if(!ano){
		...
		For(col,n){
			...
			if( cows[col].size() == 1 ){
				...
				Cow c1 = cows[col][0], c2 = cows[col][1];
				if( (c1.l <= maxL and c2.r > maxL) or (c2.l <= maxL and c1.r > maxL) ){
					canGet += 2;
					int tWays = 0;
					for( auto [l,r] : cows[col] ){
						if( l > maxL ) continue;
						int ttWays = (r_d[col][maxL+1]-(r>maxL));
						Addi( tWays , ttWays );
					}
					Muli(tans,tWays);
				}else{
					int tWays = l_d[col][maxL] + r_d[col][maxL+1];
					if(tWays){
						canGet += 1;
						Muli(tans,tWays);
					}
	...
	// r only
	{
		...
		For(col,n){
			if(cows[col].empty()) continue;
			canGet += 1;
			Muli(tans,(int)cows[col].size());
		}

		if( canGet == mx ) Addi(ways,tans);
	}
	cout << ways << endl;
