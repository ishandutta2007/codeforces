...
//
//Input
//The first line contains 4 integers n, m, c0 and d0 (1n1000, 1m10, 1c0,d0100). 
//Each of the following m lines contains 4 integers. The i-th line contains numbers ai, bi, ci and di
//(1ai,bi,ci,di100).
//
//Output
...
int calc(int n, int j)
{
    ...
    {
        if(n-i*c[j]>=0)
            ans = max( ans, calc(n-i*c[j],j-1) + i*d[j] ) ;
    }
    return ans; 
    //if(a[j]-b[j]>=0)
    //    ans = max( calc(n,j-1,a) , calc(n-c[j],j,a) + d[j] ) ;
    //else
    //    ans = calc(n,j-1,a) ;  
...
int main()
{
    ...
    FE(i,0,n)
    {
        ans = max(ans, calc(n-i,m-1) + (i/c0)*(d0));
    }
    cout<<ans;  
    return 0; 
}