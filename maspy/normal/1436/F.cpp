...
#line 1 "/home/maspy/compro/library/nt/primetable.hpp"
...
void solve() {
  ...
  struct E {
    // sum |A|^iS(A)^j
    mint sum00; // sum S(A)^0
    mint sum01; // sum S(A)^1
    mint sum02; // sum S(A)^2
    mint sum10; // sum |A|
    mint sum11; // sum |A|S(A)
    mint sum12; // sum |A|S(A)^2
  };
  E unit = {1, 0, 0, 0, 0, 0};

  auto mul = [&](E& A, E& B) -> E {
    return {A.sum00 * B.sum00,
            A.sum01 * B.sum00 + A.sum00 * B.sum01,
            A.sum02 * B.sum00 + mint(2) * A.sum01 * B.sum01 + A.sum00 * B.sum02,
            A.sum10 * B.sum00 + A.sum00 * B.sum10,
            A.sum11 * B.sum00 + A.sum10 * B.sum01 + A.sum01 * B.sum10
                + A.sum00 * B.sum11,
            A.sum00 * B.sum12 + mint(2) * A.sum01 * B.sum11 + A.sum02 * B.sum10
                + A.sum10 * B.sum02 + mint(2) * A.sum11 * B.sum01
                + A.sum12 * B.sum00};
  };
  auto pow = [&](E A, ll n) -> E {
    if (n == 0) return unit;
  ...
  vc<E> DP(lim + 1, unit);
  FOR(a, lim + 1) if (CNT[a]) {
    E e = {2, a, a * a, 1, a, a * a};
    DP[a] = pow(e, CNT[a]);
  }

  for (auto&& p: primes) {
    FOR3_R(i, 1, lim / p + 1) { DP[i] = mul(DP[i], DP[p * i]); }
  }

  vc<mint> ANS(lim + 1);
  FOR3(n, 1, lim + 1) { ANS[n] = DP[n].sum12 - DP[n].sum02; }

  for (auto&& p: primes) {
    FOR3(i, 1, lim / p + 1) { ANS[i] -= ANS[p * i]; }
  }
  print(ANS[1]);
}
