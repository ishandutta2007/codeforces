...
void solve() {
  ...
  FOR3(i, 1, N) {
    ...
    vi Q = gen_quotient(a);
    vc<mint> newDP_c(Q.size());
    vc<mint> newDP_s(Q.size());
    ll j = 0;
    FOR(k, key.size()) {
      ll val = key[k];
      while (1) {
      ...
      if (left == val) {
        ll cost = a / left - 1;
        newDP_c[j] += DP_c[k];
        newDP_s[j] += DP_c[k] * mint(cost) + DP_s[k];
        continue;
      }
      ll right = left + 1;
      ll cost = (a + right - 1) / right - 1;
      newDP_c[j] += DP_c[k];
      newDP_s[j] += DP_c[k] * mint(cost) + DP_s[k];
    }
    swap(DP_c, newDP_c);
