...
public class E {
	...
	public void solve() {
		...
		int j = 0;
		int step = 0;
		for (int i = 1; deque.peekFirst() != max; i++) {
			...
			deque.addFirst(Math.max(x, y));
			deque.addLast(Math.min(x, y));
			step = i;
		}
		deque.pollFirst();
		n--;
		...
		for (; j < q.length; j++) {
			first[q[j].y] = max;
			last[q[j].y] = b[(int) ((q[j].x - step - 1) % n)];
		}
		for (int i = 0; i < first.length; i++) {
			out.println(first[i] + " " + last[i]);
		}
	}
