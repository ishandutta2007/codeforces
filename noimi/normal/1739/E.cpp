...
int main() {
    ...
    VEC(string, s, 2);
    vv(int, a, 2);
    rep(i, 2) a[i] = toint(s[i], '0');

    vvv(int, dp, 2, n, 2, -inf<int>);
    dp[0][0][0] = 0;
    if(a[1][0]) dp[1][0][0] = 1;

    rep(j, 1, n) {
        rep(i, 2) { dp[i][j][i] = max(dp[i][j - 1][i], dp[i][j - 1][i ^ 1]) + a[i][j]; }
        if(a[0][j] and a[1][j]) {
            rep(i, 2) dp[i][j][i ^ 1] = dp[i ^ 1][j - 1][i ^ 1] + 2;
        } else {
            rep(i, 2) {
                if(a[i][j]) { dp[i][j][i ^ 1] = dp[i ^ 1][j - 1][i ^ 1] + 1; }
            }
        }
    }
    OUT(max(MAX(dp[0][n - 1]), MAX(dp[1][n - 1])));
}