...
#line 1 "/home/maspy/compro/library/ds/powerquery.hpp"
//  B 
...
struct Mono {
  using value_type = array<array<mint, 10>, 10>;
  using X = value_type;
  static X op(X x, X y) {
    X z{};
    FOR(i, 10) FOR(j, 10) {
      if (x[i][j] == mint(0)) continue;
      FOR(k, 10) z[i][k] += x[i][j] * y[j][k];
    }
    return z;
  }
  static constexpr X unit() {
    X a{};
    FOR(i, 10) a[i][i] = mint(1);
    return a;
  }
...
void solve() {
  Mono::value_type A;
  FOR(i, 9) A[i][i + 1] = mint(1);
  A[9][0] = A[9][1] = mint(1);
  PowerQuery<Mono, 1024> PQ(A);
