...
int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003];string s2;vector<int>v,vr,v2[100003];int sum,ans,num;char ch;
...
ios_base::sync_with_stdio(false);
     ...
     for(int i=0;i<n;i++)
     {
         cin>>ch;
         if(ch=='L')
         for(int j=0;j<10;j++){if(b[j]==0){b[j]=1;break;}}
         else if(ch=='R')
         {for(int j=9;j>=0;j--)if(b[j]==0){b[j]=1;break;}}
         else {int tmp=int(ch)-int('0');b[tmp]=0;}
     }
