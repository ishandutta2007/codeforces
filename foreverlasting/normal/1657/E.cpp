...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        ...
        for(res k=1;k<=m;k++)for(res j=1;j<=n;j++)add(f[j][k],mul(qpow(m-k+1,Mul(j-1,j,2)),inv[j]));
        for(res i=1;i<n;i++){
            for(res l=2;l<=m;l++){
                add(f[i][l],f[i][l-1]);
                for(res j=1;j<=n-i;j++){
                    add(f[i+j][l],mul(f[i][l-1],mul(qpow(m-l+1,Mul(2*i+j-1,j,2)),inv[j])));
                }
            }
        }
        res ans=0;
        for(res k=1;k<=m;k++)add(ans,f[n][k]);
        printf("%d\n",mul(ans,fac[n]));
    }
}
int main(){
