...
int main() {
    ...
    vll rui(N + 1);
    repn(i, N)rui[i] = rui[i - 1] + t[i-1];

    vll zero(N + 1);//+0
    rep(i, N+1) {
        zero[i] = (rui[i] + i) - T;
        zero[i] = max(0, zero[i]);
        zero[i] = min(zero[i], i + 1);
        //cout << zero[i] << endl;
    }
    ...
    repn(i, N) {
        now *= 2;
        for (ll j = zero[i-1]; j < zero[i] - 1; j++) { now += com(i-1, j) * 2; }
        now += com(i-1, zero[i] - 1);

        pos[i] = now / ((mint)2).pow(i);
        //cout << pos[i].val() << endl;
    }

    mint ans = 0;
    repn(i, N) {
        ans += (i-1) * (pos[i] - pos[i - 1]);
    }
    ans += N * (1 - pos[N]);
