...
modint<MAX_MOD> dp[200000];
int m;
modint<MAX_MOD> check(int now) {
    if (dp[now].value) return dp[now];
    dp[now] = 1;
    vector<int> hoge;
    int geko = now;
    int val = 1;
    ...
    if (geko != val) {
        return dp[geko] = check(val);
    }
    int cnts[(1 << 6)] = {};
    for (int i = (1 << hoge.size()) - 1; i >= 0; --i) {
        int eval = 1;
        REP(q, hoge.size()) {
            if ((1 << q) & i) {
                eval *= hoge[q];
    ...
    int stocks = cnts[(1 << hoge.size()) - 1];
    dp[val] = modint<MAX_MOD>(m) / modint<MAX_MOD>(m - stocks);
    REP(i, (1 << hoge.size()) - 1) {
        int tmp = 1;
        REP(q, hoge.size()) {
            if ((1 << q) & i) {
        ...
        dp[val] += check(tmp) * cnts[i] / (m - stocks);
    }
    return dp[val];
}
void solve(){
