...
int main(){
	...
	TT{
		cauto n=read();
		cauto es=readV(n-1,tuple(1,1,0ll,0ll));
		vv<pair<int,pll>> g(n);
		...
		int et=0;
		vector<map<int,int>> imoss(n+1);
		cauto dfs=
			[&](auto &&dfs,int v,int p)->void{
				++et;
				for(cauto &[u,w]:g[v])if(u!=p){
						cauto l=et;
						dfs(dfs,u,v);
						cauto r=et;
						out(v,u,w,l,r,facts[w.X],facts[w.Y],1);
						for(cauto &[a,b]:facts[w.Y]) imoss[a][l]-=b, imoss[a][r]+=b;
						for(cauto &[a,b]:facts[w.X]) imoss[a][l]+=b, imoss[a][r]-=b;
					}
			};
		dfs(dfs,0,-1);
		out(imoss,1);
		out(inv[2],1);
		vector<mint> imos(n+1,1);
		reps(i,1,n+1)if(cauto &imo=imoss[i]; imo.size()){
			int mn=0;
			{
				int sum=0;
				for(cauto &[a,b]:imo) MN(mn,sum+=b);
			}
			rep(_,-mn) imos[0]*=i;
			for(cauto &[a,b]:imo){
				if(b>0)rep(_,b) imos[a]*=i;
				if(b<0)rep(_,-b) imos[a]*=inv[i];
			}
			out(i,mn,imo,imos,1);
		}
		out(imos,1);
		...
		out(imos,1);
		cout<<accumulate(all(imos),0-imos[n]) NL;
	}
	return 0;
}