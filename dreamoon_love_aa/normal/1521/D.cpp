...
void solve() {
    ...
    FOR(x,1,n){
        if(v[x]){
            flow.add_edge(0,x,max(0,SZ(e[x])-2));
            for(int y:e[x]){
                flow.add_edge(x,y,1);
            }
        }else{
            flow.add_edge(x,n+1,max(0,SZ(e[x])-2));
        }
    }
    flow.maxflow();
    VPII rm;
    for(int i=0;i<flow.m;i+=2){
        if(!es[i].c&&es[i].k!=n+1&&es[i^1].k){
            rm.PB(es[i].k,es[i^1].k);
            e[es[i].k].erase(es[i^1].k);
            e[es[i^1].k].erase(es[i].k);
        }
    }
    DEBUG("rmSize",SZ(rm));
    FOR(i,1,n){
    ...
    W(SZ(rm));
    REP(i,SZ(rm)){
        W(rm[i].F, rm[i].S,added[i].S,added[i+1].F);
    }
