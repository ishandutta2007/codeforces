...
inline bool is_not_sort(const vector<int>& A){
	int n = A.size();
...
void do_query(const vector<int>& D, vector<int>& A){
	int n = A.size();
...
void print_query(const vector<int>& D, int n){
	assert(D.size() > 1);
	cout << D.size() << " ";
	int sum = 0;
	for(auto e: D){
...
void solve(){
	...
	while(is_not_sort(a)){
		gp_hash_table<int,int> idx;
		int x = -1, y = -1;
		for(int k = 0; k < n; ++k){
			idx[a[k]] = k;
			auto it = idx.find(a[k] + 1);
			if (it != idx.end()){
				auto X = *it;
				x = X.second;
				y = k;
