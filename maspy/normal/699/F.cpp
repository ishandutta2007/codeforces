...
void solve() {
  ...
  FOR(k, K) {
    ...
    FOR(n, N) {
      ll dx = B[n].fi - A[k].fi;
      ll dy = B[n].se - A[k].se;
      sort_key[n] = dx * dx + dy * dy;
      ll g = gcd(abs(dx), abs(dy));
      dir[k][n] = mp(dx / g, dy / g);
    }
    auto I = argsort(sort_key);
    for (auto&& i: I) { to[dir[k][i]].eb(i); }
  }

  auto check = [&](auto self, ll s, set<ll> TARGET) -> bool {
    if (len(TARGET) == 0) return true;
    if (popcnt(s) < len(TARGET)) return false;

    for (auto&& target: TARGET) {
      FOR(k, K) if (s & 1 << k) {
        set<ll> new_se = TARGET;
        new_se.erase(target);
        ll t = s - (1 << k);
        ll lim = popcnt(t);
        auto D = dir[k][target];
        ...
        for (auto&& i: I) {
          if (i == target) break;
          if (len(new_se) > lim) break;
          new_se.insert(i);
        }
        if (self(self, t, new_se)) return true;
      };
    }
    return false;
  };
