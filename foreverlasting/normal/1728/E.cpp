...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		f[0]=accumulate(b+1,b+n+1,0ll),sort(c+1,c+n+1,greater());
		for(int i=1;i<=n;i++)f[i]=f[i-1]+c[i];
		int mx=(int)(max_element(f,f+n+1)-f);
		int q=read();
		while (q--) {
			int x=read(),y=read();
			...
			A=(int)((LL)A*(n/g)%(y/g));
			if(A<0)A+=y/g;
			if((LL)x*A>n){puts("-1");continue;}
			LL d=(LL)x*y/g;
			int Mn=x*A;
			int Mx=(int)(n-(n-x*A)%d);
			LL ans=max(f[Mn],f[Mx]);
			if(mx>=Mn&&mx<=Mx){
				int l=(int)(mx-(mx-Mn)%d);
				int r=(int)(mx+(Mx-mx)%d);
				ans=max({ans,f[l],f[r]});
			}
			printf("%lld\n",ans);
		}
