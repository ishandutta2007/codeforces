...
int main(void)
{
	...
	ll M = 1<<9;
	rep(i, 0, M-1) succ[n][i] = n+1;
	per(i, n-1, 0){
		rep(j, 0, M-1) succ[i][j] = succ[i+1][j];
		succ[i][a[i+1]] = i+1;
	}
	
	rep(i, 0, M-1) rep(j, 0, M-1) dp[i][j] = inf;
	dp[0][0] = 0;
	...
	rep(i, 0, M-1){
		rep(j, 0, M-1){
			...
			rep(k, i, M-1){
				ll s = succ[dp[i][j]][k];
				if(s <= n) chmin(dp[k][j^k], s);
			}
		}
	}
	
	vector<ll> ans;
	rep(i, 0, M-1) rep(j, 0, M-1) if(dp[i][j] <= n) ans.push_back(j);
	uniq(ans);
