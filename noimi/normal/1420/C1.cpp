...
struct A {
    ...
    A() { rep(i, 2) a[i][i] = 0, a[i][i ^ 1] = -inf<ll>; }
};

A f(A x, A y) {
    A res;
    rep(i, 2) rep(j, 2) rep(k, 2) { chmax(res[i][k], x[i][j] + y[j][k]); }
    return res;
}
...
int main() {
    TEST {
        INT(n, q);
        VEC(int, a, n);
        SegmentTree<A> seg(n, f, A());
        rep(i, n) seg.set(i, A(a[i]));
        seg.build();
        cout << seg.query(0, n)[0][1] << endl;
        while(q--) {
            INT(l, r);
            l--, r--;
            auto L = seg[l], R = seg[r];
            seg.update(l, R), seg.update(r, R);
            cout << seg.query(0, n)[0][1] << endl;
        }
    }
}