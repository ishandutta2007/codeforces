...
int main() {
	...
	vector<vector<int>> g2;
	{
		...
		int curtime = 0;
		function<void(int, int)> dfs_low = [&](int u, int p) {
			if (low[u] != INT_MAX)
				return;

			time[u] = low[u] = ++curtime;
			for (auto& v : g[u]) {
				if (v != p) {
					dfs_low(v, u);
					chkmin(low[u], low[v]);
		...
		function<void(int)> dfs_bcc = [&](int u) {
			vis[u] = 1;
			bcc.back().emplace_back(u);
			for (auto& v : g[u]) {
				int par = (time[u] < time[v] ? u : v);
				int child = u ^ v ^ par;
				if (!vis[v] && low[child] <= time[par])
					dfs_bcc(v);
			}
		};
		for (int u = 1; u <= n; u++) {
		...
		for (int u = 1; u <= n; u++) {
			for (auto& v : g[u]) {
				if (gp[u] != gp[v]) {
					g2[gp[u]].emplace_back(gp[v]);
					g2[gp[v]].emplace_back(gp[u]);
				}
			}
		}
