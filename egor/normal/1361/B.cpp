...
public:
    void solve() {
        ...
        for (int i = 0; i < n; i += 2) {
            if (i + 1 == n || k[i] != k[i + 1]) {
                modint answer = power(modint(p), k[i]);
                ll need = 1;
                int at = k[i];
                for (int j : range(i + 1, n)) {
                    while (need < numeric_limits<int>::max() && at > k[j]) {
                        need = min(need * p, ll(numeric_limits<int>::max()));
                        at--;
                    }
                    need--;
                    answer -= power(modint(p), k[j]);
                    if (need == 0) {
                        i = j - 1;
                        break;
                    }
