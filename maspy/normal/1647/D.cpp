...
void solve() {
  ...
  if (primetest(x / d / d)) return NO();

  if (len(pfd) >= 2) return YES();

  ll other = 0;
  for (auto&& [p, e]: pfx) {
    if (p != pfd[0].fi) other += e;
  }
  if (other >= 1) return YES();

  assert(len(pfd) == 1 && len(pfx) == 1);
  ll a = pfd[0].se;
  ll b = pfx[0].se;
  assert(a > 1);
  if (b == a + a + 1) return NO();
  if (a >= 3) return YES();
  assert(a == 2);
  YES(b != 7);
}
