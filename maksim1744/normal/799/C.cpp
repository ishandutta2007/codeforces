...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        int beauty, cost;
        char what;
        cin >> beauty >> cost >> what;
        if (what == 'C') {
            if (cost <= c) {
                int mx = max(money.ask(0, c - cost), diam.ask(0, d));
                if (mx != 0)
                    ans = max(ans, mx + beauty);
            }
            money.set(cost, max(money.ask(cost, cost), beauty));
        } else {
            if (cost <= d) {
                int mx = max(money.ask(0, c), diam.ask(0, d - cost));
                if (mx != 0)
                    ans = max(ans, mx + beauty);
            }
            diam.set(cost, max(diam.ask(cost, cost), beauty));
        }
    }
    cout << ans << '\n';
