...
struct MinCostFlow{
	typedef llint CAP;
	typedef llint COST; //doubledijkstra
	...
	COST calc(int s, int t, CAP f)
	{
		...
		while(f > 0){
			...
			while(prevv[p] != -1){
				...
				if(G[prevv[p]][preve[p]].rev != -1) G[p][G[prevv[p]][preve[p]].rev].cap += flow;
				p = prevv[p];
			}
			f -= flow;
...
int main(void)
{
	...
	rep(i, 1, m){
		...
		ans += f;
		if(f < c){
			mcf.add_edge(u, v, f, -1);
			mcf.add_edge(u, v, c-f, 1);
			mcf.add_edge(u, v, inf, 2);
		}
		if(f > c){
			mcf.add_edge(u, v, c, -1);
			mcf.add_edge(u, v, f-c, 0);
			mcf.add_edge(u, v, inf, 2);
		}
		if(f == c){
			mcf.add_edge(u, v, f, -1);
	...
	mcf.add_edge(1, n, inf, 0);
	
	ans += mcf.calc(1, n, 1e9);
	outl(ans);
