...
void update(int q){
    t[q] = max(t[2*q+1], t[2*q+2]);
}
void _change(int from, int to, int x, int l=0, int r=N, int q=0){
    if(from <= l && r <= to){
        apply(q, x);
        return;
    }
    ...
    int m = (l+r)/2;
    if(from < m)
        _change(from, to, x, l, m, 2*q+1);
    if(to > m)
        _change(from, to, x, m, r, 2*q+2);
    update(q);
}
int get(){
//    cout<<"get()="<<t[0]<<"\n";
