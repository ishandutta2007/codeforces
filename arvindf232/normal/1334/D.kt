...
fun phrase(n:Int, p:Int):Long{
    return (1L * n * (n-1)) - (1L * (n-p) * (n-1-p))
}
fun getnum(n:Int, num :Long):Int{
    val p = BinarySearchLastTrue(0,n){
        num >= phrase(n,it)
    }!!
    if(p == n){
        return 1
    ...
    val root = p + 1
    val comp = num - phrase(n,p)
    if(comp % 2 == 0L){
        return root
    }else{
        val extra = (comp+1) / 2
        return extra.toInt() + root
    }
}
/*
1 2 1 3 1 4
