...
int ct(int exc, int lim) {
    auto pr = sv.factor(exc);
...
int main() {
    ...
    FOR (sel, 2, M + 1) {
        ...
        sv.for_each_divisor_unordered(sel, [&](int div) {
            int ways = ct(sel/div, M/div);

            if (div == sel) self = ways;
            else ans[sel] += ways * ans[div];
        });

        ans[sel] = (ans[sel] * MINV + 1) / (1 - self * MINV);
    }
