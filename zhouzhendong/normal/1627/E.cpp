...
void solve(){
	...
	For(i,1,n){
		...
		for (auto it=id[i].begin();it!=id[i].end();it++){
			int a=i,b=it->fi;
			int idd=it->se;
			ckmin(tmp,dp[idd]-(LL)cost[i]*b);
			outvals(a,b,idd,dp[idd]);
			for (auto E : e[idd]){
				int cd=E.fi,h=E.se;
				ckmin(dp[cd],tmp+(LL)cost[i]*b-h);
			}
		}
		outval(i);
		if (i==n){
			if (tmp>9e17)
				return puts("NO ESCAPE"),void();
			printf("%lld\n",tmp+(LL)cost[i]*m);
			return;
		}
		tmp=INF;
		for (auto it=id[i].end();it!=id[i].begin();){
			it--;
			int a=i,b=it->fi;
			int idd=it->se;
			ckmin(tmp,dp[idd]+(LL)cost[i]*b);
