...
PII find(int x) {
	...
	if (f[x] == x) return {f[x], toparent[x]};
	PII par = find(f[x]);
	f[x] = par.e1;
	toparent[x] ^= par.e2;
	return {f[x], toparent[x]};
}
...
int main() {
	...
	while (tests--) {
		...
		if (T == 1) {
			...
			PII f1 = find(L - 1), f2 = find(R);
			if (f1.e1 != f2.e1) { //we get some new information
				f[f2.e1] = f1.e1;
				assert(toparent[f2.e1] == 0);
				toparent[f2.e1] ^= (toparent[L - 1] ^ toparent[R] ^ X);
			}
		}
		else {
			L ^= last; R ^= last;
