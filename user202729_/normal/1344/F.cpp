...
auto constexpr values   ="WRYB";
auto constexpr valuesDot=".RYB";
int constexpr valueOf(char ch){
	switch(ch){
		case values[0]: return 0;
		case values[1]: return 1;
		case values[2]: return 2;
		case values[3]: return 3;
		default: assert(false);__builtin_unreachable();
	}
}

template<int M, int N> struct Matrix :
	std::array<std::bitset<N>, M> 
{ // mod 2
	using std::array<std::bitset<N>, M>::array;
	template<int P> Matrix<M, P> operator*(Matrix<N, P> const& other) const{
		Matrix<M, P> result{};
		for(int i=0;i<M;++i) 
			for(int j=0;j<N;++j) 
				for(int k=0;k<P;++k) 
					if((*this)[i][j]&other[j][k])
						result[i].flip(k);
		return result;
	}
};
...
int main(){
	int numCell, numOperation; cin>>numCell>>numOperation;
	std::vector<Matrix<2, 2>> state(numCell, identity);
	// current color = get(state, initial color)

	auto const RESULT_INDEX=2000;
	assert(numCell*2<=RESULT_INDEX);
	std::vector<std::bitset<RESULT_INDEX+1>> conditions;

	for(int _=0;_<numOperation;++_) {
		char data[4]; int number;
		cin>>data>>number;

		if(data[0]=='m'){
			conditions.emplace_back();
			conditions.emplace_back();
			auto& condition0=conditions.end()[-2];
			auto& condition1=conditions.back();

			for(int _=0;_<number;++_) {
				int pos; cin>>pos;
				--pos;
				condition0[2*pos  ]=state[pos][0][0];
				condition0[2*pos+1]=state[pos][0][1];
				condition1[2*pos  ]=state[pos][1][0];
				condition1[2*pos+1]=state[pos][1][1];
			}
			char ch;
			//cin>>ch;
			do cin>>ch; while(ch==' ');

			condition0[RESULT_INDEX]=valueOf(ch)&1;
			condition1[RESULT_INDEX]=valueOf(ch)>>1;
		}else{
			Matrix<2, 2> transform{};
			...
			switch(data[0]-data[1]){
				case 'R'-'Y':
					...
					break;
				case 'R'-'B':
					transform[0][0]=transform[1][0]=transform[1][1]=1;
					break;
				case 'Y'-'B':
					transform[0][0]=transform[0][1]=transform[1][1]=1;
					break;
				default:
	...
	int numFixedRow{};
	for(int col=0;col<RESULT_INDEX;++col) {
		for(int row=numFixedRow;row<(int)conditions.size();++row) {
			if(conditions[row][col]){
				std::swap(conditions[row], conditions[numFixedRow]);

				for(row=0;row<(int)conditions.size();++row)
					if(row!=numFixedRow and conditions[row][col])
						conditions[row]^=conditions[numFixedRow];
				++numFixedRow;
				break;
			}
		}
	...
	std::bitset<RESULT_INDEX> result{};
	for(auto const& condition: conditions){
		auto pos=(int)condition._Find_first();
		if(pos==RESULT_INDEX+1) continue;
		if(pos==RESULT_INDEX){
			cout<<"NO\n";
			return 0;
		}
		result[pos]=condition[RESULT_INDEX];
	}

	cout<<"YES\n";
	for(int pos=0;pos<numCell;++pos) 
		cout<<valuesDot[result[pos*2+1]<<1|result[pos*2]];
	cout<<'\n';
}