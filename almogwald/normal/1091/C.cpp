...
vecl anser;
vector<point> divisors;
lol m;
void sorter(int pos, lol ans) {
	if (pos == 0) {
		anser.push_back((m / ans *(ans-1)*ans)/2+ ans);
		return;
	}
	pos--;
	fori(i, divisors[pos].second + 1) {
		sorter(pos, ans);
		ans *= divisors[pos].first;
	}
}
int main() {
	...
	fort(i,2, 100000) {
		...
		if (cur != 0) {
			divisors.push_back(make_pair(i, cur));
		}
	}
	if (n > 1) {
		divisors.push_back(make_pair(n, 1));
	}
	sorter(divisors.size(), 1);
	sort(all(anser));
	forin(i, anser) {
		out(anser[i])
	}
