...
int main(){
  ...
  UnionFindUndo uf(n);
  ll q;cin>>q;
  vector<bool>ret(q,true);
  ll sz=500005;  
  vector<vector<P>>g(sz);
  vector<vector<ll>>gw(sz);
  rep(i,0,m)gw[w[i]].PB(i);
  rep(x,0,q){
    bool sw=true;
    ll k;cin>>k;
    rep(i,0,k){
  ...
  rep(i,0,sz){
    ...
    for(auto z:g[i]){
      ...
      if(!uf.unite(u[z.se],v[z.se]))ret[z.fi]=false;
    }
    uf.rollback();
    for(auto z:gw[i])uf.unite(u[z],v[z]);
  }
  rep(i,0,q)ans2(ret[i]);
  return 0;
}