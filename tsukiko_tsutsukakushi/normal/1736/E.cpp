...
void solve() {
    ...
    VEC(int, a, n);
    int ans = 0;
    vector dp(n + 1, vector(n, vc<int>(n + 1, -inf)));
    dp[1][0][0] = a[0];
    dp[1][1][1] = a[1];
    vector pre(n + 1, vc<int>(n + 2, -inf));
    {
        pre.assign(n + 1, vc<int>(n + 2, -inf));
        for(int k = 0; k <= 1; ++ k) {
            rep(j, n) {
                chmax(pre[j + 1][k], pre[j][k]);
                chmax(pre[j + 1][k], dp[1][j][k]);
            }
        }
    }
    rep(i, 1, n) {
        rep(j, n) {
            for(int k = 0; k <= i + 1; ++ k) {
                if(k >= 1) {
                    chmax(dp[i + 1][j][k], dp[i][j][k - 1] + a[j]);
                }
                if(j >= i) {
                    int need = j - i;
                    if(k >= need) {
                        // rep(x, j) {
                        //     chmax(dp[i + 1][j][k], dp[i][x][k - need] + a[j]);
                        // }
                        chmax(dp[i + 1][j][k], pre[j][k - need] + a[j]);
                    }
                }
            }
        }
