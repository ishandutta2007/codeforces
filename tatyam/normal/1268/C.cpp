...
signed main(){
    ...
    rep(n){
        ...
        s1.set(a,0);
        if(s2.get(0,a)<(i+1)/2){
            ll cen=s2.find_first([a=i/2+1](ll b){return b<a;},n);
            ans+=s1.get(a,cen);
            ans+=s2.get(a,n)-s2.get(0,a);
        }
        elif(s2.get(a+1,n)<(i+1)/2){
            ll cen=s2.find_first([a=(i+1)/2](ll b){return b<a;},n);
            ans+=s1.get(cen,a);
        }
        s2.set(a,1);
        v.push_back(ans);
