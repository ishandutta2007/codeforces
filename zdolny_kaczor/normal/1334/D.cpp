...
struct BT {
	...
	vi ans;
	BT(int n_) : n(n_), used(n, vector<bool>(n)), ans() {bool r = go(0); assert(r);}
	bool go(int x) {
		ans.push_back(x + 1);
		if ((int)ans.size() == n * (n - 1) + 1) return true;
		REP(y, n) if (x != y && !used[x][y]) {
			used[x][y] = true;
			if (go(y)) return true;
			used[x][y] = false;
		}
...
vi gen(int n, ll l, ll r) {
	vi ans;
#define RUN(i, a, b) for (ll i = max<ll>(a, l); i < min<ll>(b, r); ++i)
	ll last = 0;
	for (int start = 1; start < n; ++start) {
		int now_len = 2 * (n - start);
		ll end = last + now_len;
		debug(imie(last), imie(end));
		RUN(i, last, end) {
			int here = i - last;
			debug(imie(start), imie(i), imie(here));
			ans.push_back(here % 2 ? here / 2 + start + 1 : start);
		}
		last = end;
	}
	debug(imie(last));
	...
	debug(imie(n), imie(l), imie(r), imie(ans));
	rans;
}
int main() {
	loc(for (int n = 2; n <= 7; ++n) debug(fun(go, n)));
	int t;
	scanf("%d", &t);
	REP(_, t) {
		int n;
		...
		loc(if (n <= 30) {
			vi full = go(n);
			debug(imie(full), imie(out));
			assert(out == vi(full.begin() + l, full.begin() + r));
		})
