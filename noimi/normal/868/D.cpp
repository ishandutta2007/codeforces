...
int main() {
    ...
    struct S {
        ...
        string suf, pre;
        bool all;
        S() { rep(i, M) v.eb(vi(1 << i + 1)); }
        vi &operator[](int k) { return v[k]; }
    };
    vector<S> v(n);
    rep(i, n) {
        rep(j, si(s[i])) {
        ...
        if(si(s[i]) <= M) {
            v[i].all = true;
            v[i].suf = s[i], v[i].pre = s[i];
        } else {
            v[i].all = false;
            v[i].pre = s[i].substr(0, M);
            v[i].suf = s[i].substr(si(s[i]) - M, M);
        }
    }
    ...
    while(Q--) {
        ...
        x--, y--;
        S nxt;
        rep(i, M) rep(j, 1 << i + 1) nxt[i][j] |= v[x][i][j] | v[y][i][j];
        string p = v[x].suf + v[y].pre;
        rep(i, si(p)) {
            int now = 0;
            rep(k, si(p) - i) {
                if(k >= M) break;
        ...
        rep(i, M) {
            ...
            if(i == M - 1) rep(_, inf<int>) OUT(353);
        }
        if(v[x].all) {
            if(si(p) <= M) {
                nxt.all = true;
                ...
                nxt.all = false;
                nxt.pre = p.substr(0, M);
                if(v[y].all) {
                    nxt.suf = p.substr(si(p) - M, M);
                } else
                    nxt.suf = v[y].suf;
            }
        } else if(v[y].all) {
