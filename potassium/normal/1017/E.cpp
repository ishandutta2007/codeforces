...
pt operator - (pt x, pt y) {
	return pt(x.first - y.first, x.second - y.second);
}
...
vector<pt> hull(vector<pt>& pts) {
	...
	for (int i = 0; i < (int)pts.size(); i++) {
		...
		while (sz >= 2 && cross(hull[sz - 1] - hull[sz - 2], pts[i] - hull[sz - 1]) <= 0) {
			hull.pop_back();
			sz--;
		}
	...
	const int p = (int)hull.size();
	for (int i = (int)pts.size() - 2; i >= 0; i--) {
		int sz = (int)hull.size();
		while (sz >= p + 1 && cross(hull[sz - 1] - hull[sz - 2], pts[i] - hull[sz - 1]) <= 0) {
			hull.pop_back();
			sz--;
		}
		hull.emplace_back(pts[i]);
...
vector<ll> go(const vector<pt>& p) {
	...
	for (int i = 0; i < n; i++) {
		pt x = p[i] - p[(i + n - 1) % n];
		pt y = p[(i + 1) % n] - p[i];
		ans.emplace_back(norm(x));
		ans.emplace_back(dot(x, y));
	}
...
int main() {
	int n,m; R(n,m);
	vector<pt> p1(n), p2(m); R(p1, p2);
	vector<ll> s1 = go(hull(p1)), s2 = go(hull(p2));

	if ((int)s1.size() != (int)s2.size()) {
		W("No");
	}
	else {
		const int sz = (int)s1.size();
		vector<int> kmp(sz, -1);
		for (int i = 1; i < sz; i++) {
			int l = kmp[i - 1];
			while (l >= 0 && s1[l + 1] != s1[i]) l = kmp[l];
			if (s1[l + 1] == s1[i]) l++;
			kmp[i] = l;
		}
		...
		bool is = false;
		for (int i = 0, l = -1; i < 2 * sz; i++) {
			while (l >= 0 && s1[l + 1] != s2[i]) l = kmp[l];
			if (s1[l + 1] == s2[i]) l++;
			if (l == sz - 1) {
				// if (l % 2 == 1)
				is = true;
