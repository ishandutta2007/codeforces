...
int main()
{
	...
	for(int i=1;i<=m;i++)
	{
		for(int j=a;j<=i&&j<=b;j++)
		{
			for(int k=a;k<=b;k++)
			{
				dp[i][j][1]=min(dp[i][j][1],dp[i-j][k][0]+cnw[i]-cnw[i-j]);
				dp[i][j][0]=min(dp[i][j][0],dp[i-j][k][1]+cnb[i]-cnb[i-j]);
			}
		}
	}
	int minnum=Inf;
	for(int i=a;i<=b;i++)
	{
		minnum=min(minnum,dp[m][i][0]);
		minnum=min(minnum,dp[m][i][1]);
	}
	printf("%d\n",minnum);
	return 0;
}