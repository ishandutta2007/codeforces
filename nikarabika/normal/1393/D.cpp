...
int solve() {
    ...
    for (int d1 = 0; d1 < 4; d1++) {
        int d2 = (d1 + 1) & 3;
        int curdx = dx[d1] + dx[d2];
        int curdy = dy[d1] + dy[d2];
        int ist = (curdx == +1) ? n - 1 : 0;
        int ien = (curdx == -1) ? n : -1;
        int jst = (curdy == +1) ? m - 1 : 0;
        int jen = (curdy == -1) ? m : -1;
        for (int i = ist; i != ien; i -= curdx)
            for (int j = jst; j != jen; j -= curdy) {
                int ii = i + curdx;
                int jj = j + curdy;
                if (is_valid(i, jj) and is_valid(ii, j) and
                        s[i][jj] == s[i][j] and s[ii][j] == s[i][j])
                    dp[d1][i][j] = min(dp[d1][i][jj], dp[d1][ii][j]) + 1;
                else
                    dp[d1][i][j] = 1;
            }
    }
