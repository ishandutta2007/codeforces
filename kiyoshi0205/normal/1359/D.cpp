...
int main(){
  ...
  rep(i,N){
    cin>>v[i].first;
    sum[i+1]+=v[i].first+sum[i];
    v[i].second=i+1;
  }
  vrsort(v);
  segtree<ll> mi(sum,inf,[](ll a,ll b){return min(a,b);});
  segtree<ll> ma(sum,-inf,[](ll a,ll b){return max(a,b);});
  set<ll> se;
  se.insert(0);
  se.insert(N+1);
  rep(i,N){
    while(v[j].first>v[i].first){
      se.insert(v[j++].second);
    }
    auto itr=se.upper_bound(v[i].second);
    ll a,b;
    b=*itr;--itr;a=*itr;
    chmax(ans,ma.queli(v[i].second,b)-mi.queli(a,v[i].second)-v[i].first);
  }
  print(ans);
  // }return 0;
}