...
int n, ch[N], c[N], fa[N], q[N]; V G[N];
void dfs(int u) {
    for(int v : G[u]) if(v ^ fa[u]) fa[v] = u, ch[u]++, dfs(v), c[u] += ch[v] > 0;
}

void solve() {
    scanf("%d", &n);
    rep(i, 1, n) G[i].clear(), ch[i] = c[i] = 0;
    int u, v;
    rep(i, 2, n) scanf("%d%d", &u, &v), G[u].pb(v), G[v].pb(u);
    int l = 1, r = 0;
    dfs(1);
    rep(i, 2, n) if(!c[i] && ch[i]) q[++r] = i;
    while(l <= r) {
        int u = q[l++];
        if(fa[u] == 1) continue;
        if(!--ch[fa[u]] && !--c[fa[fa[u]]] && ch[fa[fa[u]]] && fa[fa[u]] > 1) q[++r] = fa[fa[u]];
        if(!--c[fa[u]] && ch[fa[u]]) q[++r] = fa[u];
        fa[u] = 1;
    }
    int as = 0, leaf = 1;
    rep(i, 2, n) {
