...
int main()
{
    ...
    vector<vector<pint>> toi(N);
    while (M--) {
        int u, v, c;
        cin >> u >> v >> c;
        toi[v - 1].emplace_back(u - 1, c);
    }
    dbg(toi);
    vector<vector<vector<int>>> dpl, dpr;
    constexpr int INF = 1e9;
    ndarray(dpl, INF, N + 1, N + 1, K);
    ndarray(dpr, INF, N + 1, N + 1, K);
    REP(i, N)
    {
        dpl[i][i + 1][0] = dpr[i][i + 1][0] = 0;
    }
    ...
    FOR(w, 1, N + 1)
    {
        REP(l, N + 1 - w)
        {
            ...
            for (auto [j, w] : toi[l]) {
                if (j < l)
                    REP(k, K - 1)
                chmin(dpl[j][r][k + 1], dpl[l][r][k] + w);
                if (j >= r)
                    REP(k, K - 1)
                chmin(dpr[l][j + 1][k + 1], dpl[l][r][k] + w);
            }
            for (auto [j, w] : toi[r - 1]) {
                if (j < l)
                    REP(k, K - 1)
                chmin(dpl[j][r][k + 1], dpr[l][r][k] + w);
                if (j >= r)
                    REP(k, K - 1)
                chmin(dpr[l][j + 1][k + 1], dpr[l][r][k] + w);
            }
        }
    }
    int ret = INF;
    REP(i, N + 1)
    REP(j, N + 1)
    chmin(ret, min(dpl[i][j].back(), dpr[i][j].back()));
    cout << (ret == INF ? -1 : ret) << '\n';
}