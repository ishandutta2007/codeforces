...
bool solve(int sa[],int ea[],int sb[],int eb[]){
    map<int,int> bg,ed;
    vector<pair<int,pii>> evnt;
    bg.clear(), ed.clear(), evnt.clear();
    rep(i,n) evnt.pb({2*sa[i],{sb[i],eb[i]}}), evnt.pb({2*ea[i]+1,{sb[i],eb[i]}});
    sort(range(evnt));
    for (auto c:evnt){
        if (c.F&1){
            bg[c.S.F]--, ed[c.S.S]--;
            if (!bg[c.S.F]) bg.erase(c.S.F);
            if (!ed[c.S.S]) ed.erase(c.S.S);
        }
        else{
            bg[c.S.F]++, ed[c.S.S]++;
            if (c.S.S<(*(prev(bg.end()))).F) return 0;
            if (c.S.F>(*ed.begin()).F) return 0;
        }
    }
    return 1;
}   
...
signed main(){
    ...
    if (solve(sa,ea,sb,eb)&&solve(sb,eb,sa,ea)) cout<<"YES";
    else cout<<"NO";
    return 0;
}