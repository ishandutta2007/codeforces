...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	TT{
		...
		cauto m=read();
		cauto bs=readV(m);
		vector<ll> suma(n+1); rep(i,n) suma[i+1]=suma[i]+as[i];
		vector<ll> sumb(m+1); rep(i,m) sumb[i+1]=sumb[i]+bs[i];
		auto dp=table(n+1,m+1,0ll);
		rep(i,n+1)rep(j,m+1){
			if(i) MX(dp[i][j],dp[i-1][j]);
			if(j) MX(dp[i][j],dp[i][j-1]);
			MX(dp[i][j],suma[i]+sumb[j]);
		}
		cout<<dp[n][m] NL;
	}
  return 0;
