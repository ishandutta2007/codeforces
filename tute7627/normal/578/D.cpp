...
int main(){
  ...
  SuffixArray sa(s);
  auto lcp=sa.lcp();
  auto seg=range_min::make(lcp.size());
  rep(i,0,lcp.size())seg.set(i,lcp[i]);
  seg.build();
  auto f=[&](ll i,ll j){
    return seg.query(min(sa.rank[i],sa.rank[j]),max(sa.rank[i],sa.rank[j]));
  };
  ll ret=0;
  {
    ll cnt=0;
    rrep(i,0,n){
      if(i==n-1||s[i]!=s[i+1])ret+=(n-i)*(m-1)-(i!=n-1);
      ret+=(m-1)*cnt;
      if(i<n-2&&s[i]!=s[i+1])ret-=f(i,i+2);
      //cout<<i spa cnt spa ret<<endl;
