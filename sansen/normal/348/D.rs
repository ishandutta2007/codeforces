...
fn run() {
    ...
    let calc = |src: (usize, usize), dst: (usize, usize)| -> M {
        let mut dp = vec![M::zero(); w];
        for (i, s) in s.iter().enumerate() {
            let mut next = vec![M::zero(); w];
    ...
    let a = calc((1, 0), (h - 1, w - 2));
    let b = calc((1, 0), (h - 2, w - 1));
    let c = calc((0, 1), (h - 1, w - 2));
    let d = calc((0, 1), (h - 2, w - 1));
    let ans = a * d - b * c;
    println!("{}", ans);
}
