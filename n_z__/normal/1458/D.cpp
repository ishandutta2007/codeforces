...
int n=s.size();
s=' '+s;
unordered_map<int,array<int,2>,custom_hash>qwq;
int now=0;
for(int x=1;x<=n;x++)
qwq[now][s[x]-'0']++,now=now+(s[x]-'0')-('1'-s[x]);
now=0;
for(int x=1;x<=n;x++)
if(qwq[now][0]&&qwq[now-1][1])qwq[now][0]--,now--,cout<<0;
else if(qwq[now][1])qwq[now][1]--,now++,cout<<1;
else qwq[now][0]--,now--,cout<<0;
cout<<endl; 
#endif
}
}
