...
int GetSign(ll value) {
	return (value > 0 ? 0 : 1);
}
void solve() {
	int n = fetch<int>();
	auto a = fetch_vec<ll>(n);
	vector<vector<pll>> dp(n + 1, vector<pll>(2, {-LL_INF, -LL_INF}));
	dp[0][0] = {0, 0};
	dp[0][1] = {0, 0};
	for(int i = 0; i < n; ++i) for(int j = 0; j < 2; ++j) {
		...
		int s = GetSign(a[i]);
		if(s != j) {
			dp[i + 1][s] = max(dp[i + 1][s], {dp[i][j].first + 1, dp[i][j].second + a[i]});
		}
	}
	auto answ = max(dp[n][0], dp[n][1]);
	cout << answ.second << '\n';
}
