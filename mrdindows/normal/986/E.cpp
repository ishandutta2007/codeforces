...
void dfs(int cur, int p = -1) {
    ...
    for (auto& q: qq[cur]) {
        ...
        int cnt = 0;
        for (int j = 0; j < m; ++j) {
            cnt += min(j + 1, k) * dp[p][j];
        }
        if (s) cnt = -cnt;
        res[id].push_back({p, cnt * q[3]});
    }
    for (int to : v[cur]) {
        if (to != p) {
            dfs(to, cur);
...
int solve() {
    ...
    FI(q) {
        ...
        while (val != 1) {
            ...
            p = pid[p];
            qq[x].push_back({p, k, i, 1});
            qq[y].push_back({p, k, i, 1});
            qq[l].push_back({p, -k, i, 2});
        }
    }
    dfs(0);
    for (int i = 0; i < q; ++i) {
