...
int main() {
	...
	forib(i, n - 1) {
		b[i] = maxi;
		while (tops.size() > 0 && a[tops.back()] < a[i]) {
			int num = a[tops.back()];
			min2(b[i], b[tops.back()]+1)
			tops.pop_back();
			while (tops.size() > 0 && a[tops.back()] == num) {
				tops.pop_back();
			}
		}
		while (bots.size() > 0 && a[bots.back()] > a[i]) {
			int num = a[bots.back()];
			min2(b[i], b[bots.back()]+1)
				bots.pop_back();
			while (bots.size() > 0 && a[bots.back()] == num) {
				bots.pop_back();
			}
		}
		if (tops.size()) {
