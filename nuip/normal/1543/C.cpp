...
Double dfs(const int a,
	...
	if(a){
		cauto d=min(a,v);
		assert(b+d/2!=1);
		if(b+d/2==1) out(a,b,c,d,v,1);
		if(b){
			re+=a*dfs(a-d, b+d/2, c+d/2);
		}else{
			re+=a*dfs(a-d, 0, c+d);
		}
	}
	if(b){
		cauto d=min(b,v);
		if(b-d==1) out(a,b,c,d,v,1);
		if(a){
			re+=b*dfs(a+d/2, b-d, c+d/2);
		}else{
			re+=b*dfs(0, b-d, c+d);
		}
	}
	return memo[{a,b,c}]=re/(a+b+c)+1;
}
int main(){
