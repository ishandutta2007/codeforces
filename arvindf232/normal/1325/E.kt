...
//            reached(x,explored[x])
            NS(x){ a->
                if(explored[a] == -1){
                    ...
                    cycle = minOf(cycle,dist[x] + dist[a]+ 1 )
                }
            }
        }
        return cycle
...
//    just dei edges
    ...
    solve.cases{
        ...
        val primecompressor = IntArray(1000005)
        var pt = 1
        primecompressor[1] = 0
        for(pi in 0 until sieve.primeCounter) {
            val p = sieve.primes[pi]
            primecompressor[p] = pt
            pt ++
        }
        val G = Graph(pt,n,false)
        for(e in edges){
            G.add(primecompressor[e.first],primecompressor[e.second])
        }

        var shortest = Int.MAX_VALUE
        for(v in 0..170){
            val dist = G.BFS(v)
            shortest = minOf(shortest, dist)
        }
        if(shortest >= 10000){
            put(-1)
            return@cases
...
//        val d = Array(pt){IntArray(pt){10000} }
//        fun add(a:Int,b:Int){
//            if(adj[a] == null) adj[a] = mint
//            if(adj[b] == null) adj[b] = mint
//            adj[a]!!.add(b)
//            adj[b]!!.add(a)
//        }
//        for(e in edges){
...
//                for(o in clist){
//                    if(o <= maxcompression){
//                        val vv = primecompressor[v]
//                        val oo = primecompressor[o]
//                        d[vv][oo] = 1
//                        d[oo][vv] = 1
//                    }
//                }
//            }else{
//                for(i1 in 0 until clist.size){
//                    for(i2 in i1+1 until clist.size){
//                        val v1 = primecompressor[clist[i1]]
//                        val v2 = primecompressor[clist[i2]]
//                        val old = minOf(d[v1][v2],d[v2][v1])
//                        shortest = minOf(old +2 , shortest)
//                        d[v1][v2] = minOf(d[v1][v2],2)
//                        d[v2][v1] = minOf(d[v2][v1],2)
//                    }
//                }
//            }
//        }
...
//        }
//        val s = pt
//        val originalWeights = Array(s){d[it].copyOf() }
//
//
//        for(k in 0 until s){
//            for(i in 0 until k){
//                for(j in 0 until i){
//                    shortest = minOf(shortest,d[i][j] + originalWeights[i][k] + originalWeights[k][j])
//                }
//            }
//            for(i in 0 until s){
//                for(j in 0 until s){
...
extremely unsound contractions of graphs
WHY

prepare prime powers

failde shortest path algorithim

3
1451 2902 2

very very unsafe to do contractions of graphs
another way ?

another way of shortest path ?

shortest cycle, also cannot use vertex, must use edges based
because BFS does nto work

problem: Does it create new cycles

very bad at shortest cycles
