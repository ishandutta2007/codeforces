...
void Solve() {
    string ss;
    cin >> ss;
    int r = 0, p = 0, s = 0, n = ss.size();
    for (auto c:ss){
        if (c == 'S') r ++;
        if (c == 'R') p ++;
        if (c == 'P') s ++;
    }
    if (r == max({r, p, s})) {string res(n, 'R'); cout << res << endl; return;}
    if (p == max({r, p, s})) {string res(n, 'P'); cout << res << endl; return;}
    if (s == max({r, p, s})) {string res(n, 'S'); cout << res << endl; return;}
}
