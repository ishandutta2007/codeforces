...
fun main(){
    solve.cases{
        ...
        repeat(n-1){
            all.add(edge(getint() - 1 , getint() - 1 , getint() ))
        }
        all.sortBy { it.weight }
        val U = DisjointUnionSets(n)
        val ans = TreeMap<Int,Long>()
        val weights = all.map{it.weight}.toSortedSet()
        var pointer = -1
        for(c in weights){
            while(pointer != all.lastIndex && all[pointer +1 ].weight <= c){
                pointer ++
                val e = all[pointer]
                U.union(e.a,e.b)
            }
