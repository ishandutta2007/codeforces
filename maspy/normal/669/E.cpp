...
void solve() {
  ...
  for (auto&& [x, query]: MP) {
    vi T;
    for (auto&& [qid, time, a]: query) T.eb(time);
    UNIQUE(T);

    ll N = len(query);
    FenwickTree<Group_Add<ll>> bit(N);
    for (auto&& [qid, time, a]: query) {
      ll t = LB(T, time);
      if (a == 1) bit.add(t, 1);
      elif (a == 2) bit.add(t, -1);
      elif (a == 3) ANS[qid] = bit.sum(0, t);
    }
  }
  FOR(q, Q) if (ANS[q] != -1) print(ANS[q]);
}
