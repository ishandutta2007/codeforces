...
int main(void)
{
	...
	rep(y, 0, h-1) rep(x, 0, w-1) cin >> a[x][y];
	
	rep(y, 0, h-1){
		b[y] = inf;
		rep(x, 0, w-1) chmin(b[y], a[x][y]);
	}
	
	rep(x, 0, w-1){
		rep(i, 0, 25) cvec[i].clear();
		rep(y, 0, h-1) cvec[c[x][y]-'a'].pb(P(a[x][y], 1<<y));
		rep(i, 0, 25){
			sort(all(cvec[i])), reverse(all(cvec[i]));
			ll sum = 0, mask = 0;
			if(sz(cvec[i])){
				rep(j, 0, sz(cvec[i])-1) mask |= cvec[i][j].snd, sum += cvec[i][j].fst;
				vec.pb(P(sum-cvec[i][0].fst, mask));
			}
		}
	}
	
	ll n = sz(vec), H = 1<<h;
	rep(i, 1, H-1) dp[0][i] = inf;
	rep(i, 0, n-1){
		rep(j, 0, H-1) dp[(i+1)&1][j] = dp[i&1][j];
		rep(j, 0, H-1) chmin(dp[(i+1)&1][j|vec[i].snd], dp[i&1][j]+vec[i].fst);
	}
	
	ll ans = inf;
	rep(i, 0, H-1){
		ll tmp = dp[n%2][i];
		rep(j, 0, h-1) if((i & (1<<j)) == 0) tmp += b[j];
		chmin(ans, tmp);
	}
	outl(ans);
