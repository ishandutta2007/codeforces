...
_ n, m, x, y, h[maxn], f[maxn][21], lg2[maxn], a[maxn], b[maxn], fd[maxn];
queue<node> q;
vector<_> g[maxn];
v(_p) bd;
...
inline void AC() {
    ...
    bfs();
    for(_p i : bd) {
        _ l = lca(i.first, i.second);
        if(l == i.first || l == i.second) {
            if(l == i.first) swap(i.first, i.second);
            ++a[1];
            --a[chld(i.first, i.second)];
            ++a[i.first];
        } else {
            ++a[i.first];
            ++a[i.second];
    ...
    dfs(1, -1);
    for(_ i = 1; i <= n; ++i) write(a[i] == bd.size());
    clr();
