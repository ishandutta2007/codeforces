...
const int BIT = 262144;
vector<pii> g[MAXN * 2];
vector<pii> g2[MAXN * 20];
vector<pii> cost = vector<pii>(MAXN * 2, {INT_MAX, INT_MAX});
vector<int> cost2 = vector<int>(MAXN * 20, INT_MAX);
bitset<MAXN * 2> is;
bitset<MAXN * 20> is2;
...
int main() {
	...
	for (int i = 0; i < m; i++) {
		...
		for (int j = 0, uu = u, vv = v; j < 20; j++, uu += BIT, vv += BIT, swap(uu, vv)) {
			g2[uu].pb({vv, 1});
		}
	}
	...
	for (int lev = 0, co = 1; lev + 1 < 20; lev++, co <<= 1) {
		for (int u = 1 + BIT * lev; u <= n + BIT * lev; u++) {
			g2[u].pb({u + BIT, co});
		}
	}
	...
	pii ansCost = min(cost[n], cost[n ^ BIT]);
	
	const Mint TWO = Mint(2, MOD998);
	Mint ans = TWO.pow(ansCost.F) - 1;
	ans += ansCost.S;
