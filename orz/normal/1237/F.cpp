...
void init_f()
{
	...
	{
		FACT.push_back(prod(FACT.back(), i, M));
		INV_FACT.push_back(inverse(FACT[i], M));
	}
}
int C(int n, int k)
{
	if ((k < 0) || (k > n))
		return 0;
	int ans = FACT[n];
	ans = prod(ans, INV_FACT[k], M);
	ans = prod(ans, INV_FACT[n - k], M);
	return ans;
}
int another_main()
{
	...
	int h, w, n;
	cin >> h >> w >> n;
	vector<bool> free_row(h, true), free_col(w, true);
	int fr = 0, fc = 0;
	vvi dyn_col(w / 2 + 1, vi(w + 1, 0));
	vvi dyn_row(h / 2 + 1, vi(h + 1, 0));
	...
	dyn_col[0].assign(w + 1, 1);

	for (int i = 1; i < w / 2 + 1; ++i)
		for (int j = 2; j < w + 1; ++j)
			if (free_col[j - 1] == false)
				dyn_col[i][j] = dyn_col[i][j - 1];
			else
			{
				dyn_col[i][j] = dyn_col[i][j - 1];
				if (free_col[j - 2])
					dyn_col[i][j] = sum(dyn_col[i][j], dyn_col[i - 1][j - 2], M);
			}

	dyn_row[0].assign(h + 1, 1);

	for (int i = 1; i < h / 2 + 1; ++i)
		for (int j = 2; j < h + 1; ++j)
			if (free_row[j - 1] == false)
				dyn_row[i][j] = dyn_row[i][j - 1];
			else
			{
				dyn_row[i][j] = dyn_row[i][j - 1];
				if (free_row[j - 2])
					dyn_row[i][j] = sum(dyn_row[i][j], dyn_row[i - 1][j - 2], M);
			}


	int ans = 0;
	for (int hor_do = 0; hor_do <= fc / 2; ++hor_do)
		for (int ver_do = 0; ver_do <= fr / 2; ++ver_do)
		{
			int cur_ans = 1;

			cur_ans = prod(cur_ans, C(fr - 2 * ver_do, hor_do), M);
			cur_ans = prod(cur_ans, C(fc - 2 * hor_do, ver_do), M);
			cur_ans = prod(cur_ans, dyn_col[hor_do][w], M);
			cur_ans = prod(cur_ans, dyn_row[ver_do][h], M);
			cur_ans = prod(cur_ans, FACT[hor_do], M);
			cur_ans = prod(cur_ans, FACT[ver_do], M);

			ans = sum(ans, cur_ans, M);
		}
	cout << ans << '\n';
	return 0;
}
