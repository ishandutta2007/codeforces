...
int cnt(int r, int total, const vector<int> &pref, int a, int b) {
    if (r == 0) return 0;
    int c = (r + 1) % (a * b);
    return pref[c] + (r - c + 1) / (a * b) * total;
}

void solve() {
    autoint a, b, q;
    vector<bool> good(a * b);
    for (int x = 0; x < a * b; ++x) good[x] = (x % a) % b != (x % b) % a;
    int total = count(good.begin(), good.end(), true);
    vector<int> pref(a * b);
    for (int x = 0; x < a * b - 1; ++x) pref[x + 1] = pref[x] + good[x];
    do {
        autoint l, r;
        int ans = cnt(r, total, pref, a, b) - cnt(l - 1, total, pref, a, b);
        cout << ans << ' ';
    } while (--q);
    cout << '\n';
}