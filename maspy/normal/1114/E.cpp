...
void solve() {
  ...
  vi nums;
  FOR_(30) { nums.eb(ask_kth(RNG(0, N))); }

  auto check = [&](ll x) -> bool { return !ask_large(x); };
  ll mx = binary_search(check, LIM + 1, 0);
  ll d = 0;
  for (auto&& v: nums) d = gcd(d, mx - v);
  print("!", mx - (N - 1) * d, d);
}
