...
int main(void)
{
	...
	rep(i, 1, n){
		deque<char> deq; ll p = 1; string t;
		deq.pb(s[i][0]), deq.pb(s[i][1]);
		rep(j, 2, sz(s[i])-1){
			if(p-1 >= 0 && deq[p-1] == s[i][j]) p--;
			else if(p+1 < sz(deq) && deq[p+1] == s[i][j]) p++;
			else{
				if(p == 0) deq.push_front(s[i][j]);
				else if(p == sz(deq)-1) deq.pb(s[i][j]), p++;
				else goto end;
			}
		}
		
		s[i] = "";
		for(auto c : deq) s[i] += c;
		t = s[i]; uniq(t);
		if(sz(s[i]) != sz(t)) goto end;
		
		aho.addString(s[i], c[i]);
		reverse(all(s[i]));
		aho.addString(s[i], c[i]);
		end:;
	}
	...
	rep(i, 0, N-1){
		rep(j, 0, m-1){
			rep(k, 0, 11){
				if(i & (1<<k)) continue;
				ll nj = aho.trans(j, k);
				if(dp[i|(1<<k)][nj] < dp[i][j] + aho.trie[nj].sum){
					dp[i|(1<<k)][nj] = dp[i][j] + aho.trie[nj].sum;
					rec[i|(1<<k)][nj] = P(k, j);
				}
			}
		}
	}
	string str; ll I = N-1, J, ans = -inf;
	rep(j, 0, m-1){
		if(ans < dp[N-1][j]) ans = dp[N-1][j], J = j;
	}
	while(I){
		str += rec[I][J].fi+'a';
		ll nI = I & ~(1<<rec[I][J].fi);
		ll nJ = rec[I][J].se;
		I = nI, J = nJ;
	}
	outl(str);
