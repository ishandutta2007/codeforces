...
int main(){
    ...
    sort(ALL(a));
    vector<int> half(n),nxt(n);
    for (int i=0,cur=0,now=0;i<n;++i){
        while(cur<n&&a[cur]<=a[i]/2) ++cur;
        while(now<n&&a[now]<a[i]*2) ++now;
        half[i]=cur; nxt[i]=now;
    }

    vector<vector<mint>> dp(n+1,vector<mint>(n+1,0));
    dp[1][0]=1;
    for (int i=1;i<n;++i){
        for (int j=0;j<n;++j) dp[i][j+1]+=dp[i][j];
        for (int j=0;j<n;++j){
            dp[i+1][j]+=dp[i][j]*(half[j]-i+1);
            dp[i+1][j+1]-=dp[i][j]*(half[j]-i+1);
            dp[i+1][nxt[j]]+=dp[i][j];
            dp[i+1][n]-=dp[i][j];
        }
    }
