...
///////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
int tp[] = {20031111,1,2,3,1,3,2,1};
int inpa[10];

int nowday;
int a[10];
int ans;
inline bool Fullweek(){
	if( a[1] < 3 or a[2] < 2 or a[3] < 2 ) return 0;
	int minx = min({ a[1]/3 , a[2]/2 , a[3]/2 });
	a[1] -= minx*3;
	a[2] -= minx*2;
	a[3] -= minx*2;
	ans += minx*7;
...
int main(){
	...
	Read(inpa[1],inpa[2],inpa[3]);

	int aans = 0;
	For(i,7){
		nowday = i;
		ans = 0;
		tie(a[1],a[2],a[3]) = {inpa[1],inpa[2],inpa[3]};
		while( nowday != 1 and Aday() );
		while(Fullweek());
		while(Aday());
		Mymax(aans,ans);
	}
