...
ll n, M;
mint dp[350005], ndp[350005], dpsum[350005];
ll zero = 175002;
...
mint calc(ll n)
{
	...
	rep(i, 0, n-1) rep(j, 0, n-1){
		if(i >= j) continue;
		//outl(i, j, dpsum[M+zero] - dpsum[j-i+zero]);
		ret += dpsum[M+zero] - dpsum[j-i+zero];
	}
	//outa(dp, -M+zero, M+zero);
	
	rep(j, -M, M){
		ll l = max(j-(n-1), -M), r = j;
		ndp[j+zero] = dpsum[r+zero] - dpsum[l-1+zero];
	}
	rep(j, -M, M) dp[j+zero] = ndp[j+zero], dpsum[j+zero] = dpsum[j-1+zero] + dp[j+zero];
	
	rep(j, -M, M){
		ll l = j, r = min(j+(n-1), M);
		ndp[j+zero] = dpsum[r+zero] - dpsum[l-1+zero];
	}
	rep(j, -M, M) dp[j+zero] = ndp[j+zero], dpsum[j+zero] = dpsum[j-1+zero] + dp[j+zero];
...
mint perm2(ll n, ll k)
{
	mint ret = mint(1);
	rep(i, 1, k) ret *= mint(n+1-i);
	return ret;
}
...
int main(void)
{
	...
	dp[zero] = mint(1);
	rep(j, -M, M) dpsum[j+zero] = dpsum[j-1+zero] + dp[j+zero];
	
	mint ans = mint(0);
	rep(i, 1, n) ans += calc(i) * perm2(n, n-i);
	outl(ans);
