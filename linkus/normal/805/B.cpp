...
int main(){
    ...
    sc(n);
    int orn = n;
    /* const string seq = "aabbacbbaabc"; */
    const string seq = "aabbaabbaabb";
    string want;
    while(n >= 12){
        want.insert(want.end(),all(seq));
        n -= 12;
    }
    want.insert(want.end(),seq.begin(),seq.begin()+n);
    /* if(want.back() == 'c'){ */
    /*     want.back() = want[want.size()-2]; */
    /* } */
    cout << want;
    //cout << string(n/2,'a') << string((n/2)+(n&1),'b');
}