...
void solve() {
    ...
    REP(i, inputs.size()) {
        for (int q = 0; q < 101; ++q) {
            for (int j = 0; j <= k; ++j) {
                if (dp[0][q][j].value == 0) continue;
                int next_j = j;
                if(i != 0) next_j += q * (inputs[i] - inputs[i - 1]);
                if (next_j > k) {
                    dp[0][q][j] = 0;
                    continue;
                }
                dp[1][q][next_j] += dp[0][q][j] * (q+1); //take on solo
                if (q != 0) {
                    dp[1][q - 1][next_j] += dp[0][q][j] * q;//finisher
                }
                if (q + 1 < 101) {
                    dp[1][q + 1][next_j] += dp[0][q][j];//starter
                }
                dp[0][q][j] = 0;
            }
        }
