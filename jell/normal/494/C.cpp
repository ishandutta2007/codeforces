#line 1 "codeforces-workspace\\contest\\494\\c\\c.cpp"
/* @file template.cpp
 * @brief Code template
 */
...
#line 12 "codeforces-workspace\\contest\\494\\c\\c.cpp"
...
void main() {
  ...
  vector<int> a(n);
  cin >> a;
  const auto m = *max_element(begin(a), end(a));

  // vector<tuple<int, i64, double>> cur;
  // for (auto i = 0; i != n; ++i) {
  //   int a;
  //   cin >> a;
  //   // cur.emplace_back(i, a, 1);
  //   cur[i].emplace_back(a, 1);
  // }

  vector<tuple<int, int, double>> itrv(q);
  for (auto &&[l, r, p] : itrv) {
    cin >> l >> r >> p;
    l--;
  }
  sort(begin(itrv), end(itrv), [](auto i, auto j) {
    int li, ri, lj, rj;
    tie(li, ri, ignore) = i;
    tie(lj, rj, ignore) = j;
    return ri - li < rj - lj;
  });
  itrv.emplace_back(0, n, 0);  // whole

  map<int, pair<int, vector<double>>> cur;

  for (auto &&[l, r, p] : itrv) {
    auto be = cur.lower_bound(l);
    auto en = cur.lower_bound(r);

    // assert(be->first == l);
    // assert((r == n && en == end(cur)) or (r < n && en->first == r));

    vector<double> nv(q * 2 + 1, 1.0);
    auto pv = l;
    for (auto itr = be; itr != en; pv = itr->second.first, ++itr) {
      auto zer = 0;
      while (pv < itr->first) {
        auto dif = a[pv] - m + q;
        chge(zer, dif);
      ...
      for (auto i = 0; i < zer; i++) {
        nv[i] = 0;
      }
      auto tmp = itr->second.second;
  ...
  double ans = m, sum = 0;
  assert(cur.begin()->first == 0);
  auto [ren, top] = cur.begin()->second;
  assert(ren == n);
  for (auto i = 0; i <= q * 2; ++i) {
    if (i < q) assert(top[i] < 1e-6);
    ans += (i - q) * (top[i] - sum);
    sum = top[i];
  }
  assert(top.back() > 1 - 1e-6);
  cout << ans << endl;
}
}