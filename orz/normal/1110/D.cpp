		...
		for (int p = 0; p < 3; ++p)
			for (int q = 0; q < 3; ++q)
				for (int r = 0; r < 3; ++r)
				{
					if (
						((r == 0) || ((i >= 0) && (c[i] >= r))) &&
						((q + r == 0) || ((i - 1 >= 0) && (c[i - 1] >= q + r))) &&
						((p + q + r == 0) || ((i - 2 >= 0) && (c[i - 2] >= p + q + r))))
					{
						int new_ans = r + pd[i - 1][p][q];
						if (i - 2 >= 0)
							new_ans += (c[i - 2] - p - q - r) / 3;
						pd[i][q][r] = max(pd[i][q][r], new_ans);
					}
				}
	}
	cout << pd.back().front().front();
	return 0;
}