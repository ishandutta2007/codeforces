...
int main() {
  ...
  vector<ll> p(m);
  vcin(p);
  auto k=z_algorithm(s);
  for(int i=0;i<m-1;i++){
    if(p[i+1]-p[i]>=s.size()) continue;
    //cout<<p[i+1]-p[i]<<" "<<k[p[i+1]-p[i]]<<" "<<(s.size()-(p[i+1]-p[i]))<<endl;
    if(k[p[i+1]-p[i]]!=(s.size()-(p[i+1]-p[i]))){
      cout<<0<<endl;
      return 0;
    }
  }
 ...
 // for(auto e:sum) cout<<e<<endl;
  for(int i=1;i<n;i++) sum[i]+=sum[i-1];
  ll ans=1;
//  for(auto e:sum) cout<<e<<endl;
  for(int i=0;i<n;i++) if(sum[i]>=0) (ans*=26)%=mod;
  cout<<ans<<endl;
}