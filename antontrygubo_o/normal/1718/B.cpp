...
void solve()
{
    ...
    if (k==1)
    {
        if (a[0].first == 1) cout<<"Yes"<<endl; else cout<<"No"<<endl; return;
    }

    ll tot = 0; for (auto it: a) tot+=it.first;
    ...
    if (tot!=f[b+2]-1) {cout<<"No"<<endl; return;}
    ...
    for (int n = b; n>=0; n--)
    {
        ...
        int cur = k-1;
        if (a[cur].second == last) cur = k-2;

        last = a[cur].second;
        if (a[cur].first<f[n]) {cout<<"No"<<endl; return;}
        a[cur].first-=f[n];
    }
