...
void solve() {
  ...
  // 
  vi key(H * W);
  FOR(x, H) FOR(y, W) key[ID(x, y)] = A[x][y];
  UNIQUE(key);
  FOR(x, H) FOR(y, W) A[x][y] = LB(key, A[x][y]);
  // 
  UnionFind uf(H * W);
  vi pos(H * W, -1);
  FOR(x, H) {
    FOR(y, W) {
      if (pos[A[x][y]] != -1) uf.merge(ID(x, y), ID(x, pos[A[x][y]]));
      pos[A[x][y]] = y;
    }
    FOR(y, W) { pos[A[x][y]] = -1; }
  }
  FOR(y, W) {
    FOR(x, H) {
      if (pos[A[x][y]] != -1) uf.merge(ID(x, y), ID(pos[A[x][y]], y));
      pos[A[x][y]] = x;
    }
    FOR(x, H) { pos[A[x][y]] = -1; }
  }
  ...
  //  -> 
  vc<vi> IDS(len(key));
  FOR(x, H) FOR(y, W) IDS[A[x][y]].eb(ID(x, y));
