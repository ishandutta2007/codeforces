...
void solve() {
    ...
    if (pos.size() == 2) {
        ...
        if (pos[0] + 1 == pos[1]) {
            ...
            ans = min(y, 1ll * x * (pos[1] - pos[0]));
        }
        cout << ans << '\n';
        return;
    }

    vector<vector<ll>> dp(len(pos) + 1, vector<ll>(2, (ll)INF));
    dp[0][0] = 0;

    for (int i = 0; i < len(pos); ++i) {
        if (i > 0) {
            for (int j = 0; j < 2; ++j) {
                cmin(dp[i + 1][j], dp[i - 1][j] + x * (pos[i] - pos[i - 1]));
            }
        }
        cmin(dp[i + 1][0], dp[i][1] + y);
        cmin(dp[i + 1][1], dp[i][0]);
    }
    cout << dp[len(pos)][0] << '\n';
}