...
int main() {
    ...
    for (int ph = 0; ph <= D; ph++) {
        ...
        for (auto a: av[ph]) {
            for (int i = k; i >= 0; i--) {
                for (int j = 1; i + j <= k && j <= a; j++) {
                    int s = a ^ (a - j);
                    //dbg(ph, a, j, s);
                    assert(0 <= s && s < md);
                    for (int l = 0; l < md; l++) {
                        dp[i + j][l ^ s] += ifac[j] * dp[i][l];
                    }
                }
            }
        }
