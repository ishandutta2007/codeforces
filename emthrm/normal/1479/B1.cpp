...
int main() {
  int n; cin >> n;
  vector<int> a(n, n); REP(i, n) cin >> a[i], --a[i];
  // {
  //   int ans = 0;
  //   REP(i, 1 << n) {
  //     int b[2] = {-1, -1}, score = 0;
  ...
  //   cout << ans << '\n';
  // }
  LazySegmentTree<monoid::RangeMaximumAndAddQuery<int, INF>> seg(n + 1);
  seg.set(n, 1);
  FOR(i, 1, n) {
    int mx = max(max(seg.get(0, a[i]), seg.get(a[i] + 1, n + 1)) + 1, seg[a[i]]);
    if (a[i - 1] != a[i]) {
      seg.apply(0, a[i - 1], 1);
      seg.apply(a[i - 1] + 1, n + 1, 1);
    }
    if (seg[a[i - 1]] < mx) seg.set(a[i - 1], mx);
  }
  cout << seg.get(0, n + 1) << '\n';
  return 0;
}