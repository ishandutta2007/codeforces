...
const ll thePow = 617673396283947LL;

int n = get<int>();
vpii edg = getVector<pii>(n-1);
vector<vi> g(n);
...
void process(ll x, ll y, ll step, int v, int p, int angle) {
	ans[v] = {x, y};
	angle += 2; angle &= 3;
	for (int i = 0; i < sz(g[v]); i++) {
		...
		angle++; angle &= 3;
		process(x + step*dX[angle], y + step*dY[angle], step/3, to, v, angle);
	}
}
...
signed main() {
	for (int i = 0; i < sz(edg); i++) {
		edg[i].F--; edg[i].S--;
		g[edg[i].F].pb(edg[i].S);
		g[edg[i].S].pb(edg[i].F);
	}
	for (int i = 0; i < n; i++) {
		if (sz(g[i]) > 4) {
			putLn("NO");
