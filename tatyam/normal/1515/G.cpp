...
int main(){
    ...
    WeightedGraph g(n);
    {
        ...
        each(x,y,z,edge){
            x--; y--;
            g.add_directed_edge(x,y,z);
            g2.add_edge(x,y);
        }
        vector<ll>id(n);
        auto scc=g2.scc();
        rep(scc.size())each(x,scc[i])id[x]=i;
        rep(n){
            auto&v=g[i];
            v.erase(remove_if(all(v),[&](const auto&e){ return id[e.to]!=id[i]; }),v.end());
        }
    }
    vector<pll>cost(n,{-1,-1});
    queue<ll>q;
    vec(bool,in_q,n);
    rep(n){
        ...
        cost[i]={0,0};
        q.push(i);
        auto update=[&](ll at,ll mod1,ll rem1){
            if(mod1)rem1%=mod1;
            if(cost[at]==pll{mod1,rem1})return;
            if(!in_q[at]){
                q.push(at);
                in_q[at]=1;
            }
            auto&[mod2,rem2]=cost[at];
            if(mod2==-1){
                cost[at]={mod1,rem1};
                return;
            }
        ...
        while(q.size()){
            ...
            q.pop();
            in_q[at]=0;
            each(i,g[at])update(i,cost[at].first,cost[at].second+i.cost);
        }
    }
    LL(Q);
    while(Q--){
        LL(v,s,t);
        auto[mod,rem]=cost[v-1];
        YES(s%gcd(mod,t)==0);
    }
}