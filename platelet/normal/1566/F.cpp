...
void solve() {
    scanf("%d%d", &n, &m), a[0] = INT_MIN, a[n + 1] = INT_MAX;
    ...
    rep(i, 1, m) {
        scanf("%d%d", &l, &r);
        auto pl = lb(a + 1, a + n + 1, l), pr = ub(a + 1, a + n + 1, r);
        if(pl == pr) b[pl - a].eb(r, l);
    }
    rep(i, 1, n + 1) {
        sort(all(b[i])), f[i] = g[i] = 1e18;
        auto trs = [&](int l, int r) {
            if((i == 1 && l > a[0]) || (i > n && r < a[n + 1])) return;
            l -= a[i - 1], r = a[i] - r;
            ll t = min(f[i - 1] + l, g[i - 1] + l * 2ll);
            cmi(f[i], t + r * 2ll), cmi(g[i], t + r);
        };
        int L = a[i - 1];
        for(auto [r, l] : b[i]) trs(L, r), cma(L, l);
        trs(L, a[i]);
    }
    printf("%lld\n", min(f[n + 1], g[n + 1]));
}
