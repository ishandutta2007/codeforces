...
int main () {
	...
	for (int i = -M - 10; i <= M + 10; ++i) inv[i] = inverse (tot[0] + tot[1] + i);
	...
	for (int k = M - 1; k >= 0; --k) {
		for (int d1 = 0; d1 <= k; ++d1) {
			int d2 = k - d1;
			int i = tot[0] - d1, j = tot[1] + d2;
			dp[d1][d2] = (1ll * i * inv[i + j - tot[0] - tot[1]] % MOD * dp[d1 + 1][d2] + 1ll * j * inv[i + j - tot[0] - tot[1]] % MOD * dp[d1][d2 + 1]) % MOD;
		}
	}
	int prob = dp[0][0];
