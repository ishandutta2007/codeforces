...
const int maxn = 1e5 + 100, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 100, X = 43, LG = 18, B = 3000;
...
vector<string> ask(int l, int r) {
    #ifndef ONPC
    cout << "? " << l + 1 << ' ' << r + 1 << endl;
    int cs = (r - l + 1) * (r - l + 2) / 2;
    vector<string> q(cs);
    for (auto &i : q)
        cin >> i, sort(i.begin(), i.end());
    return q;
    #else
    vector<string> q;
    for (int i = l; i <= r; i++)
        for (int j = i; j <= r; j++)
            q.push_back(hid.substr(i, j - i + 1));
    for (auto &i : q)
        sort(i.begin(), i.end());
    return q;
    #endif
...
int main() {
    ...
    for (auto i : q)
        o.push_back(i);
    sort(o.begin(), o.end(), [](string a, string b) {return a.size() < b.size();});
    vector<int> e(256);
    string s;
    for (int i = 0; i < n; i++) {
        vector<int> h(256);
