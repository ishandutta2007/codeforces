...
inline void AC() {
    ...
    n = read();
    m = read();
    for(_ i = 1; i <= n<<1; ++i) v[i] = false;
    g.clear();
    a.clear();
    for(_ i = 1; i <= m; ++i) {
        x = read();
        y = read();
        g.push_back(mp(min(x, y), max(x, y)));
        v[x] = true;
        v[y] = true;
    }
    for(_ i = 1; i <= (n<<1); ++i) if(!v[i]) a.push_back(i);
    _ ad = (_)a.size()>>1;
    for(_ i = 0; i < ad; ++i) g.push_back(mp(a[i], a[i+ad]));
    _ s = 0;
    for(_ i = 0; i < g.size(); ++i) for(_ j = i+1; j < g.size(); ++j)
        if((g[i].first < g[j].second && g[i].first > g[j].first) != (g[i].second < g[j].second && g[i].second > g[j].first)) {
            ++s;
        }
    writeln(s);
