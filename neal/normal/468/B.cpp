// Solution to https://codeforces.com/contest/468/problem/B
#include <algorithm>
#include <cassert>
#include <iostream>
#include <map>
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (location.count(A - x))
            TS.implies(vars[i], vars[location[A - x]]);
        else
            TS.set_value(TS.inv(vars[i]));

        if (location.count(B - x))
            TS.implies(TS.inv(vars[i]), TS.inv(vars[location[B - x]]));
        else
            TS.set_value(vars[i]);
    }
    ...
    for (int i = 0; i < N; i++)
        cout << (TS.assignment[vars[i]] ? '0' : '1') << (i < N - 1 ? ' ' : '\n');
}