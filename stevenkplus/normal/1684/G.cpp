...
// https://ekzlib.netlify.app/external/spaghetti-source%252Falgorithm,graph%252Fbipartite_matching_HK.cc
struct graph {
   int L, R;
...
int main() {
   ...
   graph g(sz(left), sz(right));
   for (int i = 0; i < sz(left); ++i) {
      for (int j = 0; j < sz(right); ++j) {
         if (right[j] % left[i] == 0 && left[i] + 2 * right[j] <= m) {
            g.add_edge(i, j);
         }
      }
   }
   vector<int> mate = g.maximum_matching();
   pp(left, right);
   pp(g.adj);
   pp(mate);
   int mated = 0;
   ...
   if (mated == sz(right)) {
      ...
      for (int i = 0; i < sz(left); ++i) {
         if (mate[i] == -1) {
            ...
            int j = mate[i] - sz(left);
            cout << left[i] + 2 * right[j] << " " << left[i] + right[j] << "\n";
         }
      }
   } else {
      cout << "-1\n";
