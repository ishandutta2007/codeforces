...
//                    int id = p.second;
//                    int what = p.first;
//                    ans[id] = s[to][1].count(what) || s[to][2].count(what);
//                }
//            }
            s[to][0].insert(to);
            for (int v : s[to][0]) {
                ...
                for (auto p : vq[v][2]) {
                    int id = p.second;
                    int what = p.first;
                    ans[id] |= s[to][1].count(what);
                }
            }
            merge(s[cur][1], s[to][1]);
            s[cur][1].insert(to);
        } else {
            s[to][0].insert(to);
//            DBN(to, s[to][1]);
...
//                DBN(v, s[to][0]);
                for (auto p : vq[v][1]) {
                    int id = p.second;
                    int what = p.first;
                    ans[id] |= s[to][0].count(what);
                }
            }
            merge(s[cur][0], s[to][0]);
//            merge(s[cur][1], s[to][1]);
//            s[cur][0].insert(to);
        }
    }
//    DBN(cur, s[cur][0], s[cur][1]);
    for (auto p : vq[cur][0]) {
        int id = p.second;
        int what = p.first;
        ans[id] |= s[cur][0].count(what);
    }
//    for (auto p : vq[cur][1]) {
//        int id = p.second;
//        int what = p.first;
//        ans[id] = s[cur][1].count(what);
//    }
}
int32_t solve() {
    int n;
...
//        DBN(u, t, v);
        if (t == 0) {
            vq[u][t].push_back({v, i});
        } else {
            vq[v][t].push_back({u, i});
            vq[u][2].push_back({v, i});
        }
    }
    go(0);
    FI(q) {
