...
int main() {
  ...
  ji(n) rint(a[i]);
  VPII ia(n);
  ji(n) ia[i] = make_pair(a[i], i);
  sort(ia.begin(), ia.end());
  reverse(ia.begin(), ia.end());
  VI nim(n);
  for (auto& pr : ia) {
    int threshold = pr.first;
    int index = pr.second % pr.first;
    VI nimset;
    for (int i = index; i < n; i += threshold) {
      if (a[i] > threshold) {
        nimset.push_back(nim[i]);
      }
    }
    nim[pr.second] = mex(nimset);
    // printf("nim[%d] = %d\n", pr.second, nim[pr.second]);
  }
  ji(n) {
    if (nim[i] != 0) {
      putchar('A');
