...
int main() {
	...
	int n1 = vx.size() - 1, n2 = vy.size() - 1;
	flow_graph<i64> G(n1 + n2 + m + 2, n1 + n2 + m, n1 + n2 + m + 1);

	int s = n1 + n2 + m;
	int t = n1 + n2 + m + 1;
	for (int i = 0; i < n1; i++) {
		G.add(s, i, vx[i + 1] - vx[i]);
	}
	for (int i = 0; i < n2; i++) {
		G.add(i + n1, t, vy[i + 1] - vy[i]);
	}
	for (int i = 0; i < m; i++) {
		int p1 = lower_bound(vx.begin(), vx.end(), x1[i]) - vx.begin();
		int p2 = lower_bound(vx.begin(), vx.end(), x2[i]) - vx.begin();
		int q1 = lower_bound(vy.begin(), vy.end(), y1[i]) - vy.begin();
		int q2 = lower_bound(vy.begin(), vy.end(), y2[i]) - vy.begin();
		for (int j = p1; j < p2; j++)
			G.add(j, i + n1 + n2, inf);
		for (int j = q1; j < q2; j++)
			G.add(i + n1 + n2, j + n1, inf);
	}
	printf("%lld\n", G.max_flow());
}