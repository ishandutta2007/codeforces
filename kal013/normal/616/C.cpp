...
char arr[1050][1050];
int dp[1050][1050],vis[2][1050][1050],val[1050][1050],used[maxn];
int tot=0,cnt=2;
void dfs(int i,int j,int n,int m,int idx){
    if (i<0 || j<0 || i>=n || j>=m) return;
    if (vis[idx][i][j] || arr[i][j]=='*') return;
    vis[idx][i][j]=1;
    if (idx) {dp[i][j]=tot; val[i][j]=cnt;}
    else tot++;
    dfs(i,j+1,n,m,idx);
    dfs(i,j-1,n,m,idx);
    dfs(i-1,j,n,m,idx);
...
signed main(){
    ...
    cin>>n>>m;
    For(i,maxn) used[i]=0;
    For(i,n) For(j,m) {cin>>arr[i][j];dp[i][j]=0;For(k,2) { vis[k][i][j]=0;}}
    For(i,n){
        For(j,m){
    ...
    For(i,n){
        For(j,m){
            if (arr[i][j]=='.') cout<<arr[i][j];
                ...
                int s=1;
                if (i!=0 ) {s+=dp[i-1][j];used[val[i-1][j]]=1;}
                if (j!=0 && used[val[i][j-1]]==0) {s+=dp[i][j-1]; used[val[i][j-1]]=1;}
                if (i!=n-1 && used[val[i+1][j]]==0) {s+=dp[i+1][j];used[val[i+1][j]]=1;}
                if (j!=m-1 && used[val[i][j+1]]==0) {s+=dp[i][j+1]; used[val[i][j+1]]=1;}
