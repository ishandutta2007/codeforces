...
int itr[300000];
int oneB[300000];
int dfs(int now, int back,int &cnter) {
    color[now] = cnter;
    itr[now] = cnter;
    cnter++;
    for (auto x : vertexs[now]) {
        ...
        depth[x] = depth[now] + 1;
        oneB[x] = now;
        itr[now] = max(itr[now], dfs(x, now, cnter));
    }
    return itr[now];
}
void solve() {
    ...
    REP(i, m) {
        ...
        REP(q, a) {
            ...
            b--;
            if (oneB[b] != 0) {
                hoge.push_back(mp(depth[oneB[b]], oneB[b]));
            }
        }
        if (hoge.empty() == true) {
            cout << "YES" << endl;
        ...
        sort(ALL(hoge));
        int ok = 1;
        for (int q = 1; q < hoge.size(); ++q) {
            pair<int, int> l = mp(color[hoge[q - 1].second], itr[hoge[q - 1].second]);
            pair<int, int> r = mp(color[hoge[q].second], itr[hoge[q].second]);
            if (l.first <= r.first && r.second <= l.second) {
