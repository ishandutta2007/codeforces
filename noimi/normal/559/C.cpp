...
int main() {
    ...
    sort(all(v), [](auto x, auto y) { return x.fi + x.se < y.fi + y.se; });
    ...
    rep2(i, 1, n - 1) {
        rep(j, i) {
            if(v[j].fi <= v[i].fi and v[j].se <= v[i].se) { rep(k, 2) dp[k ^ 1][i] += dp[k][j] * C(v[i].fi - v[j].fi + v[i].se - v[j].se, v[i].fi - v[j].fi); }
        }
    }
