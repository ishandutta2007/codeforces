...
struct union_find {
  ...
  union_find(int _n): n(_n), count(n), par(_n), rank(_n), sz(_n) {
    iota(all(par), 0);
    fill(all(sz), 1);
  }
  int root(int x) {
...
int main() {
  ...
  rep(v, 1, n) {
    map<int, int> cnt;
    for (auto u : graph[v]) cnt[uf.root(u)]++;
    bool merged = false;
    VI erase;
    for (auto r : roots) if (cnt[uf.root(r)] < uf.size(r)) {
      if (merged) erase.push_back(r);
      uf.merge(r, v), merged = true;
    }
    for (auto e : erase) roots.erase(e);
    if (not merged) roots.insert(v);
  }
  OUT(roots.size() - 1);
}