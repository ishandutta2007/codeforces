...
void update(ll u,ll l,ll r,ll ql,ll qr,ll f,ll c){
	if(f==3){
		if(ql<=l&&r<=qr){
			val[u].sum=val[u].max=c;
			return;
		}
	}
	else{
	...
	ll mid=(l+r)>>1;
	if(mid>=ql)update(u<<1,l,mid,ql,qr,f,c);
	if(mid+1<=qr)update(u<<1|1,mid+1,r,ql,qr,f,c);
	pushup(u);
}
...
int main(){
	//IO(CF438D The Child and Sequence);
	ll rill(n,m);
	rep(i,1,n)rill(a[i]);
	build(1,1,n);
	rep(i,1,m){
		ll rill(f),l,r,x;
		switch(f){
			case 1:rill(l,r),writeln(query(1,1,n,l,r));
				break;
			case 2:rill(l,r,x),update(1,1,n,l,r,f,x);
				break;
			case 3:rill(l,x),update(1,1,n,l,l,f,x);
		}
	}
	return 0;
}