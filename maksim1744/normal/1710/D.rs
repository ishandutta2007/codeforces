...
fn solve(cin: &mut Reader, cout: &mut Writer, _test: usize) {
    ...
    for l in 0..n {
        for (r, _c) in (l..n).zip(cin.read::<String>().chars()).skip(1).filter(|&(_r, c)| c == '1') {
            good.push((l, r));
        }
    }
    good.sort_by_key(|&(l, r)| r - l);
    ...
    let genp = |n: usize| -> Vec<usize> {
        if n % 2 == 0 {
            (1..n + 1).step_by(2).rev().chain((2..n + 1).step_by(2).rev()).map(|i| i - 1).collect()
        } else {
            (1..n)
                .step_by(2)
                .rev()
    ...
    for (l, r) in good.into_iter() {
        if dsu.check(l, r) {
            continue;
        }
        let mut cur = Vec::new();
        for i in l..=r {
            if i == l || !dsu.check(i, i - 1) {
                cur.push(i);
            }
        }
        *cur.last_mut().unwrap() = r;
        assert!(cur.len() != 3);
        // eprintln!("l={} r={} {:?} {:?}", l + 1, r + 1, cur, genp(cur.len()));
        for ar in genp(cur.len()).windows(2) {
            assert!(dsu.un(cur[ar[0]], cur[ar[1]]));
            ans.push((cur[ar[0]], cur[ar[1]]));
        }
    }
    assert_eq!(ans.len(), n - 1);
    for (l, r) in ans.into_iter() {
        outln!(l + 1, r + 1);
    }
    outln!();
}
...
///  # Bundled libraries
/// 
///  - `rlib_dsu 0.1.0 (path+)`         published in https://github.com/maksim1744/rlib licensed under `MIT` as `crate::__cargo_equip::crates::rlib_dsu`
...
mod __cargo_equip {
    pub(crate) mod crates {
        pub mod rlib_dsu {#[derive(Clone,Debug)]pub struct DSU{p:Vec<usize>,sz:Vec<usize>,}impl DSU{pub fn new(n:usize)->Self{Self{p:(0..n).collect(),sz:vec![1;n],}}pub fn reset(&mut self,n:usize){self.p.resize(n,0);for i in 0..n{self.p[i]=i;}self.sz.resize(n,0);for i in 0..n{self.sz[i]=1;}}pub fn par(&mut self,v:usize)->usize{if self.p[v]!=v{self.p[v]=self.par(self.p[v]);}self.p[v]}pub fn un(&mut self,mut u:usize,mut v:usize)->bool{u=self.par(u);v=self.par(v);if u==v{return false;}if self.sz[u]>self.sz[v]{std::mem::swap(&mut u,&mut v);}self.sz[v]+=self.sz[u];self.p[u]=v;true}pub fn check(&mut self,u:usize,v:usize)->bool{self.par(u)==self.par(v)}pub fn size(&mut self,v:usize)->usize{let v=self.par(v);self.sz[v]}}}
