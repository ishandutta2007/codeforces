...
inline ll inv(ll r, ll _mod = MOD) { return pwm(r, _mod - 2, _mod); }
/*--------------------------------------------------------------------------------------------------------*/
...
inline void solve() {
	...
	for (int i = 0; i < n; ++i) {
		...
		//
		dp[i + 1][dg % 3] = max(dp[i + 1][dg % 3], dp[i][0]);
		dp[i + 1][dg % 3] = max(dp[i + 1][dg % 3], dp[i][0] + 1);
		//
		dp[i + 1][(dg + 1) % 3] = max(dp[i + 1][(dg + 1) % 3], dp[i][1]);
		dp[i + 1][dg % 3] = max(dp[i + 1][dg % 3], dp[i][1]);
		//
		dp[i + 1][(dg + 2) % 3] = max(dp[i + 1][(dg + 2) % 3], dp[i][2]);
		dp[i + 1][dg % 3] = max(dp[i + 1][dg % 3], dp[i][2]);
	}
	cout << max({ dp[n][0] + 1, dp[n][1], dp[n][2] }) << endl;
}
...
#ifdef LOCAL
	cout << "Time: " << (ld)clock() / CLOCKS_PER_SEC << endl;
	while (true);
#endif // LOCAL
	return 0;
}