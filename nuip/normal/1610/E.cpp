...
int main(){
	...
	TT{
		cauto orig=readV(read(),0);
		map<int,int> cnts;
		for(cauto &v:orig) ++cnts[v];
		cauto vs=orig-distinct;
		const int n=vs.size();
		int re=cnts.rbegin()->Y;
		rep(o,n-1){
			for(int i=o+1,cnt=cnts[vs[o]]; i<n; i=lower_bound(all(vs), 2*vs[i]-vs[o])-vs.begin()){
				++cnt;
				out(o,i,cnt,1);
				MX(re,cnt);
			}
