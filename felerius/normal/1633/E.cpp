// Spanning Tree Queries (https://codeforces.com/contest/1633/problem/E)
...
pair<ll, int> eval(const vector<array<int, 3>>& e, int x, int n) {
    int m = int(partition_point(all(e), [&](auto& ei) { return ei[2] <= x; }) - begin(e));
    int l = m - 1, r = m;
    Dsu dsu(n);
    ll weight = 0;
    int small = 0;
    while (l >= 0 && r < sz(e)) {
        if (x - e[l][2] < e[r][2] - x) {
            if (dsu.join(e[l][0], e[l][1]))
                weight += x - e[l][2], ++small;
            --l;
        } else {
            if (dsu.join(e[r][0], e[r][1]))
                weight += e[r][2] - x;
            ++r;
        }
    }
...
int main() {
    ...
    rep(i, m) {
        p.push_back(e[i][2]);
        rep(j, i + 1, m)
            p.push_back((e[i][2] + e[j][2] + 1) / 2);
    }
    sort(all(p));
    p.erase(unique(all(p)), end(p));

    vector evaled(sz(p), pair(0ll, 0));
    rep(i, sz(p))
        evaled[i] = eval(e, p[i], n);

    int P, k, a, b, c; read(P, k, a, b, c);
    vector q(P, 0);
    read(q);
    q.resize(k);
    rep(i, P, k)
        q[i] = int((ll(q[i - 1]) * a + b) % c);

    ll val = 0;
    trav(qi, q) {
        auto it = upper_bound(all(p), qi) - 1;
        auto [base, small] = evaled[it - begin(p)];
        ll above = qi - *it;
        ll mst = base + small * above - (n - 1 - small) * above;
        val ^= mst;
    }
