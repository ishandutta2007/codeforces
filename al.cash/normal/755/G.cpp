...
struct ConstIntModulus {
  constexpr static int modulus = static_cast<int>(998244353);
};
...
class FFTBase {
protected:
  using F = typename conditional<is_integral<T>::value, complex<double>, T>::type;
  vector<F> w;

  template <class C>
  void fft(const C* coeffs, F* values, int size, int offset = 0) const {
    if (size == 1) return *values = *coeffs, void();
    fft(coeffs, values, size >>= 1, offset + 1);
    fft(coeffs + (1 << offset), values + size, size, offset + 1);
    for (int i = 0; i < size; ++i) {
      F product = values[i + size] * w[i << offset];
      values[i + size] = values[i] - product;
      values[i] += product;
    }
  }
...
public:
  ...
  void init(int size) {
    ...
    if (n != this->size()) {
      w.resize(n);
      static_cast<Derived*>(this)->unityRoots();
    }
  }

  template <class C>
  void fft(const C* coeffs, F* values) const { fft(coeffs, values, size()); }

  void multiply(const T* lhs, const T* rhs, T* product) {
    vector<F> lhsValues(size()), rhsValues(size());
    fft(lhs, lhsValues.data());
    fft(rhs, rhsValues.data());
    for (int i = 0; i < size(); ++i) lhsValues[i] *= rhsValues[i];
    auto derived = static_cast<Derived*>(this);
    derived->invert();
    derived->fftInverse(lhsValues.data(), rhsValues.data(), product);
    derived->invert();
  }

  vector<T> multiply(vector<T> lhs, vector<T>& rhs){
    int n = static_cast<int>(lhs.size() + rhs.size() - 1);
    init(n);
    lhs.resize(size());
    rhs.resize(size());
    multiply(lhs.data(), rhs.data(), lhs.data());
    lhs.resize(n);
    return lhs;
  }
};
...
template <class M>
class NTT : public FFTBase<M, NTT<M>> {
  static_assert(M::modulus() >= 0, "NTT is defined only for modular types");

public:
  void unityRoots() {
    int n = this->size();
    M g = M(3) ^ ((M::modulus() - 1) / n);
    this->w[0] = 1;
    for (int i = 1; i < n; ++i) this->w[i] = this->w[i - 1] * g;
  }

  void invert() { reverse(this->w.begin() + 1, this->w.end()); }

  void fftInverse(const M* values, M* coeffs) const {
    this->fft(values, coeffs);
    M sizeInverse = M(1) / this->size();
    for (int i = 0, n = this->size(); i < n; ++i) coeffs[i] *= sizeInverse;
  }
  void fftInverse(const M* values, const M* medium, M* coeffs) const {
    fftInverse(values, coeffs);
  }
};
