...
int main() {
    INT(n);
    VEC(int, a, n);
    vi last(n + 1), front(n + 1), cnt(n + 1), dp(n + 1);
    rep(i, n) last[a[i]] = i;
    rep3(i, n - 1, 0) front[a[i]] = i;
    vi rui(n + 1);
    rep(i, n) rui[i] = cnt[a[i]]++;
    int ma = 0;
    rep(i, n) {
        if(front[a[i]] == i) {
            chmax(dp[last[a[i]] + 1], dp[i] + cnt[a[i]]);
            chmax(ma, dp[last[a[i]] + 1]);
        } else {
            // dump(dp[i], cnt[a[i]], rui[i]);
            chmax(ma, dp[i] + cnt[a[i]] - rui[i]);
        }
        // cout << ma << endl;
        chmax(dp[i + 1], dp[i]);
    }
