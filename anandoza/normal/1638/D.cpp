...
int main() {
    ...
    vector<vector<vector<pii>>> blocks(n, vector<vector<pii>>(m));

    for (int i = 0; i + 1 < n; i++) {
        for (int j = 0; j + 1 < m; j++) {
            pii b{i, j};
            blocks[i][j].push_back(b);
            blocks[i + 1][j].push_back(b);
            blocks[i][j + 1].push_back(b);
            blocks[i + 1][j + 1].push_back(b);
        }
    }

    vector<vector<vector<int>>> colors(n - 1, vector<vector<int>>(m - 1));
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            for (auto [u, v] : blocks[i][j])
                colors[u][v].push_back(grid[i][j]);
        }
    }
    ...
    auto test = [&](int i, int j) {
        ...
        if (!colors[i][j].empty() && colors[i][j].front() == colors[i][j].back()) {
            ans.push_back({i, j, colors[i][j].front()});
            used[i][j] = true;
        }
    };
    ...
    for (int index = 0; index < sz(ans); index++) {
        auto [i, j, c] = ans[index];
        for (auto [u, v] : array<pii, 4>{{{i, j}, {i + 1, j}, {i, j + 1}, {i + 1, j + 1}}}) {
            for (auto [x, y] : blocks[u][v]) {
                auto it = find(all(colors[x][y]), grid[u][v]);
                if (it != colors[x][y].end()) {
                    colors[x][y].erase(it);
                    test(x, y);
                }
            }
