    ...
    map<string,string>smp;
        int main()
        {
            ...
            while(t--)
            {
                cin>>n>>k;map<long long,bool>checker;
                for(long long i=1,x=0;i<1e17;i*=k,x++)checker[x]=false;
                yes=true;

                for(int i=0;i<n;i++)
                {
                    cin>>a[i];long long old=0;cnt=0;
                    while(a[i]!=old)
                    {
                        old=a[i];
                        if(a[i]==0)break;
                        if(a[i]%k==0)a[i]/=k,cnt++;
                        else if(a[i]==1&&checker[cnt]==false){checker[cnt]=true;break;}
                        else if(a[i]==1&&checker[cnt]){yes=false;break;}
                        else
                        {
                            //cout<<"1 "<<a[i]<<endl;
                        for(long long j=1;j<=1e16;j*=k)
                        {
                            //if(k==9)cout<<a[i]<<" "<<j<<endl;
                            if(j>a[i])break;
                            if((a[i]-j)%k==0&&checker[cnt]==false){checker[cnt]=true,a[i]-=j;break;}
                            else if((a[i]-j)%k==0&&checker[cnt]) yes=false;
                            if(a[i]==0)break;
                        }
                            //cout<<"2 "<<a[i]<<endl;
                        }
                    ...
                    if(a[i]!=0&&a[i]!=1)yes=false;
                    b[i]=a[i];
                }
                if(yes)cout<<"YES\n";
                else cout<<"NO\n";
                //for(int i=0;i<5;i++)cout<<checker[i]<<" ";cout<<endl;
