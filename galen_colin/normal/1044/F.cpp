...
struct max_t {
  ...
  max_t(): val(0), cnt(0) {}
  max_t(long long v): val(v), cnt(1) {}
  max_t(long long v, long long c): val(v), cnt(c) {}

  max_t op(max_t& other) {
	if (val == other.val) return max_t(val, cnt + other.cnt);
    else if (val > other.val) return *this;
	else return other;
  }
  
  max_t lazy_op(max_t& v, int size) {
    return max_t(val + v.val, cnt);
  }
};
...
ll st[200005], en[200005], dep[200005], tt = -1;
segtree<max_t> mt;
vector<ll> edges[200005];
set<pair<ll, ll>> elist;
lca_lift lca;
...
void solve(int tc = 0) {
	...
	for (ll i = 0; i < q; i++) {
		cin >> x >> y;
		--x; --y;
		if (pair{dep[x], x} > pair{dep[y], y}) swap(x, y);
				
		ll f = 1;
		if (elist.find(pair{x, y}) != elist.end()) {
			f = -1;
			elist.erase(pair{x, y});
		} else {
			elist.insert(pair{x, y});
		}
		
		mt.update(st[y], en[y], f);
		
		if (st[x] <= st[y] && en[y] <= en[x]) {
			mt.update(0, n - 1, f);
			
			ll v = lca.get(y, dep[y] - dep[x] - 1);
			mt.update(st[v], en[v], -f);
		} else {
			mt.update(st[x], en[x], f);
		}
		
		max_t v = mt.tree[0];
		ll ans = 0;
		if (v.val == elist.size()) ans = v.cnt;
		
		// for (ll i = 0; i < n; i++) cout << mt.query(i, i).val << " ";
