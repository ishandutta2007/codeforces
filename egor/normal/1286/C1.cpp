...
public:
    void solve(istream& inp, ostream& outp) {
        ...
        out.flush();
        if (n == 1) {
            out.printLine("!", in.readString());
            out.flush();
            return;
        }
        auto read = [&](arr<multiset<vi>>& s) {
            vi q(26);
        ...
        arr<multiset<vi>> a(n + 1, multiset<vi>());
        for (int i : range(n * (n + 1) / 2)) {
            read(a);
        }
        out.printLine("? 2", n);
        out.flush();
        arr<multiset<vi>> b(n + 1, multiset<vi>());
        for (int i : range(n * (n - 1) / 2)) {
            read(b);
        }
        for (int i : range(n)) {
        ...
        for (int i : range(1, n + 1)) {
            ...
            answer += char(find(all(cur), 1) - cur.begin() + 'a');
        }
//        reverse
