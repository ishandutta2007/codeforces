...
int main()
{
    ...
    vector<int> A(N);
    cin >> A;
    vector<vector<int>> x2i(N + 1);
    REP(i, N) x2i[A[i]].emplace_back(i);
    vector<pint> ranges;
    for (auto v : x2i) if (v.size()) {
        v.emplace_back(N);
        if (v[0] > 0) ranges.emplace_back(0, v[0]);
        REP(i, v.size() - 1) {
            if (v[i] + 1 < v[i + 1]) {
                ranges.emplace_back(v[i] + 1, v[i + 1]);
            }
        }
    }
    ...
    sort(ALL(ranges));
    dbg(ranges);
    RangeMinimumQuery<int> rmq(vector<int>(N + 5, -1), 1e9);

    vector<vector<int>> r2ls(N + 1);
    for (auto [l, r] : ranges) {
        r2ls[r].emplace_back(l);
    }
    ...
    REP(i, N)
    {
        rmq.update(A[i], i);
        for (auto l : r2ls[i + 1]) {
            int i = rmq.binary_search(1, [&](int x) { return x < l; });
            mexs.emplace_back(i);
        }
    }
    mexs = srtunq(mexs);
