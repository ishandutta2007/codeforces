...
void print(vi R){
  ...
  FOR(i,n) FORU(j,i+1,n-1) {
    G.addEdge(S,2+n+cur,1);
    VV[i][j] = G.addEdge(2+n+cur,2+i,1);
    G.addEdge(2+n+cur,2+j,1);
    cur+=1;
  }
  int f = G.flow(S,T);
  assert(f == accumulate(all(R),0));

  FOR(i,n) FORU(j,i+1,n-1) {
    ans[i][j] = (VV[i][j]->f)?1:0;
    ans[j][i] = 1-ans[i][j];
  }
  cout << n << endl;
  FOR(i,n) {
...
int main(){
  ...
  dp[0][0][0] = 1;
  last[0][0][0] = -1;
  FOR(i,n) FOR(j,100) FOR(k,5000) if(dp[i][j][k]) {
    if(k+A[i]>=(j*(j+1))/2 && k+A[i]<5000) {dp[i][j+1][k+A[i]] = 1;last[i][j+1][k+A[i]]=i;}
    if(i+1<n&&j!=0) {
      if(k+A[i+1]>=(j*(j+1))/2 && k+A[i+1]<5000) {dp[i+1][j+1][k+A[i+1]] = 1;last[i+1][j+1][k+A[i+1]]=i;}
    }
  }
  FORU(j,1,100) if(dp[n-1][j][(j*(j-1))/2]) {
    vi R;
    int l=n-1;
    int jj=j;
    int kk=(j*(j-1))/2;
