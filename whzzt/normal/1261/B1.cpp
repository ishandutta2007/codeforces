...
int n,a[N],m,ls[M],rs[M],sz[M],cnt,rt[N];
pii res[N];
...
int find(int x,int l,int r,int k){
	if(l==r) return l;
	int mid=(l+r)>>1;
	if(sz[ls[x]]>=k) return find(ls[x],l,mid,k);
	return find(rs[x],mid+1,r,k-sz[ls[x]]);
}
int main(){
	scan(n);
	read(a+1,a+n+1);
	for(int i=1;i<=n;++i) res[i]=pii(-a[i],i);
	sort(res+1,res+n+1);
	for(int i=1;i<=n;++i) insert(rt[i],rt[i-1],1,n,res[i].se);;
	scan(m);
	for(int tt=1;tt<=m;++tt){
		int k,pos;
		scan(k,pos);
		print(a[find(rt[k],1,n,pos)]),putc('\n');
	}
}