...
int main() {
    ...
    auto g = getG(n);
    vi f{1, 1};
    rep(i, 27) f.eb(f[i] + f[i + 1]);
    if(find(all(f), n) == end(f)) drop(NO);
    if(n == 1 or n == 2 or n == 3) drop(YES);
    int m = lb(f, n);
    vi sub(n);
    vi ban(n);
    auto solve = [&](auto &&F, int x, int p, int m) -> bool {
        // dump(x, p, f[m]);
        if(m <= 3) return true;
        int t = -1, s;
        auto dfs = [&](auto &&ff, int x, int p) -> int {
            sub[x] = 1;
            for(auto e : g[x]) {
                if(e != p and !ban[e]) sub[x] += ff(ff, e, x);
            }
            if(sub[x] == f[m - 1] or sub[x] == f[m - 2]) {
                t = x;
                s = p;
            }
            return sub[x];
