...
int main()
{
	...
	RESET(kanan,0);
	REP(i,r) FORD(j,c-1,0) kanan[i][j] = (data[i][j] == '1' ? 1 + kanan[i][j+1] : 0);
	/*REP(i,r)
	{
		REP(j,c) printf("%d ",kanan[i][j]);
		puts("");
	}*/
	int risan = 0;
	REP(j,c)
	{
		...
		REP(k,SIZE(angka))
			MAX(risan,(k+1)*angka[k]);
		//REP(k,SIZE(angka)) printf("%d ",angka[k]);
		//puts("");
	}
	printf("%d\n",risan);
	return 0;
