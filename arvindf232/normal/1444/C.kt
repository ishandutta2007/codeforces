...
fun main(){
    solve.cases{
        ...
        val groups = getline(n)
        for(i in groups.indices) {
            groups[i] --
        }
        val dsu = DisjointUnionSets_Parity(n)
        val groupedges = mutableMapOf<Long,MutableList<Long>>()

        val groupdied = BooleanArray(k)
        repeat(m){
            val a = getint - 1
            val b = getint -1
            if(groups[a] == groups[b]){
                val ans = dsu.union(a,b,true)
                if(!ans){
                    groupdied[groups[a]] = true
                }
            }else{
                val ga = groups[a]
                val gb = groups[b]
                val gp = makepair(minOf(ga,gb), maxOf(ga,gb))
                if(groupedges[gp] == null){
                    groupedges[gp] = mong
                }
                groupedges[gp]!!.add(makepair(a,b))
            }
        }
        val notdead = groupdied.count { !it }
        var base = (notdead).toLong() * (notdead - 1) / 2
        for((g,elist) in groupedges){
            val ga = g.first
            val gb = g.second
            if(groupdied[ga] || groupdied[gb]) {
                continue
            }
            var vnow = dsu.getversion()
            var bad = false
            for(e in elist){
                val ans = dsu.union(e.first, e.second,true)
                if(! ans){
                    bad = true
                    break
                }
