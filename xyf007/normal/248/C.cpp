...
Circle SmallestCircle(const std::vector<Point> &b) {
  ...
  std::srand(std::chrono::_V2::steady_clock::now().time_since_epoch().count());
      ...
      for (size_t j = 0; j < i; j++) {
        if (!InCircle(O, a.at(j))) {
          ...
          for (size_t k = 0; k < j; k++) {
            if (!InCircle(O, a.at(k))) {
              O = Circle(a.at(i), a.at(j), a.at(k));
            }
          }
        }
      }
...
int main(int argc, char const *argv[]) {
  ...
  std::cin >> d >> u >> w >> x >> y >> r;
  Line l(Point(x, y),
         Vector(Point(x, y), Point(0.0, 2 * w - 3 * r - d)));
  double dis = Dis(Point(0.0, 2 * w - 2 * r - u), l);
  if (Cmp(dis - r) < 0) {
    std::cout << -1;
  } else {
    Line yw(Point(0.0, w - r), Vector(Point(0.0, 0.0), Point(1.0, 0.0)));
    std::cout << std::setprecision(kPrecision) << std::fixed
              << Intersection(l, yw).x_;
  }
  return 0;
}