...
bool go(int cpos) {
  ...
  for (int cv = 0; cv < sz(rems); cv++) if (rems[cv].second) {
    cvals[cpos] = cv;
    if (cpos == 2 && cvals[1] > cvals[2]) continue;

    if (cpos == 2) {
      int nval = rems[cvals[0]].first + rems[cvals[1]].first + rems[cvals[2]].first;
      nval = s - nval;
      int id = lower_bound(asvals.begin(), asvals.end(), nval) - asvals.begin();
      if (id >= sz(asvals) || asvals[id] != nval || !rems[id].second) continue;
    }
    if (cpos == 5) {
      int nval = rems[cvals[3]].first + rems[cvals[4]].first + rems[cvals[5]].first;
      nval = s - nval;
      int id = lower_bound(asvals.begin(), asvals.end(), nval) - asvals.begin();
      if (id >= sz(asvals) || asvals[id] != nval || !rems[id].second) continue;
    }
    if (cpos == 6) {
      int nval = rems[cvals[0]].first + rems[cvals[3]].first + rems[cvals[6]].first;
      nval = s - nval;
      int id = lower_bound(asvals.begin(), asvals.end(), nval) - asvals.begin();
      if (id >= sz(asvals) || asvals[id] != nval || !rems[id].second) continue;
    }
