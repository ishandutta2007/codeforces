...
int main() {
	CASE(t) {
		int n; R(n);
		const int V = binpow(2, n, MOD107);
		const int E = n * binpow(2, n - 1, MOD107);
		...
		for (int u = 0; u < V; u++) {
			if (__builtin_popcount(u) < 2) continue;
			int lowbit = u & -u;
			int slowbit = (u - lowbit) & -(u - lowbit);

			int x = 0, y = 0;
			while (g[ans[u ^ lowbit]][x] != g[ans[u ^ slowbit]][y] || used[g[ans[u ^ lowbit]][x]])
				(g[ans[u ^ lowbit]][x] < g[ans[u ^ slowbit]][y] ? x : y)++;

			ans[u] = g[ans[u ^ lowbit]][x];
			used[ans[u]] = true;
		}
