...
//  vector 
template <typename G> vector<bool> color_two(G &g) {
    int n = g.size();
    vector<bool> used(n), res(n);
    queue<int> q;
    for(int i = 0; i < n; ++i) {
        if(!used[i]) used[i] = true, q.emplace(i);
        while(!q.empty()) {
            int now = q.front();
            q.pop();
            for(auto &e : g[now]) {
                if(!used[e])
                    used[e] = true, res[e] = res[now] ^ 1, q.emplace(e);
                else if(res[e] == res[now])
                    return vector<bool>();
            }
        }
    }
...
main() {
    ...
    rep(i, n) {
        if(g[i].size() == n - 1)
            ...
            for(auto e : g[i])
                if(g[e].size() != n - 1) G[e][i] = true, N -= uf.unite(e, i);
    }
    vi v;
    rep(i, n) if(uf.find(i) == i and g[i].size() != n - 1) v.eb(i);
    int cnt[2] = {};
    rep(i, n) rep(j, n) if(G[i][j] and g[i].size() != n - 1 and g[j].size() != n - 1) {
        if(v[0] == uf.find(i))
            cnt[0]++;
        else
            cnt[1]++;
    ...
    if(v.empty() or
       (v.size() < 3 and cnt[0] == (ll)uf.size(v[0]) * (uf.size(v[0]) - 1) and (v.size() == 1 or cnt[1] == (ll)uf.size(v[1]) * (uf.size(v[1]) - 1)))) {
        cout << "Yes\n";
        rep(i, n) {
            if(g[i].size() != n - 1 and uf.find(i) == v[0]) ans[i] = 'c';
        }
        cout << ans << endl;
    } else
        cout << "No\n";
