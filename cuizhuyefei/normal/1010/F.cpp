...
int n,sz[N],son[N],Fa[N];ll x;Vi e[N];
...
inline void fft(int *a, int n, int tp){
	...
	for(int j=2;j<=n;j<<=1){
		...
		rep(i,2,(j>>1)-1)w[i]=1LL*w[i-1]*w[1]%mo;//WA
...
inline Vi mul(Vi a, Vi b){
	...
	int ni=power(n,mo-2);Vi c;rep(i,0,SZ(a)+SZ(b)-1)c.pb(1LL*ni*f[i]%mo);return c;
}
inline void solve(int l, int r, Vi &f, Vi &g){
	if(l==r){f=g=h[l];return;}int mid=(l+r)>>1;
	Vi fl,fr,gl,gr;solve(l,mid,fl,gl);solve(mid+1,r,fr,gr);
	f=mul(fl,fr);g=add(mul(gr,fl),gl);
}
inline Vi dfs2(int u, int fa){
	for(int x=u;x;x=son[x]){
		t[x].clear();
		rep(i,0,SZ(e[x])-1)if(e[x][i]!=son[x]&&e[x][i]!=Fa[x])t[x]=dfs2(e[x][i],x);
		if(SZ(t[x])<1)t[x].resize(1);t[x][0]++;
		t[x].insert(t[x].begin(),0);//*x
	}
	len=0;for(int x=u;x;x=son[x])h[++len]=t[x];
	Vi f,g;solve(1,len,f,g);return g;
}
int main() {
	read(n);scanf("%lld",&x);x%=mo;rep(i,1,n-1){
		int u,v;read(u);read(v);e[u].pb(v);e[v].pb(u);
	}
	dfs1(1,0);Vi res=dfs2(1,0);ll ans=0,now=1;
	rep(i,1,SZ(res)-1){
