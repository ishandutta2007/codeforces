...
void solve() {
  ...
  ll LIM = 100000;
  vc<vc<int>> div(LIM + 1);
  FOR3(d, 1, LIM + 1) FOR3(i, 1, LIM / d + 1) div[d * i].eb(d);

  vi isin(LIM + 1);
  vc<set<int>> sss(LIM + 1);

  auto f = [&](ll x, ll k, ll s) -> ll {
    if (x % k != 0) return -1;
    ll lim = s - x + 1;
    if (lim <= 1) return -1;
    auto& ss = sss[k];
    if (ss.lower_bound(lim) == ss.begin()) return -1;
    ll x0 = 0;
    FOR_R(k, 17) {
      ll l = x0;
      ll m = x0 | 1 << k;
      ...
      if (m >= r) continue;
      if (x & 1 << k) {
        if (ss.lower_bound(l) == ss.lower_bound(m)) x0 |= 1 << k;
      } else {
        if (ss.lower_bound(m) != ss.lower_bound(r)) x0 |= 1 << k;
      }
    }
    return x0;
  };
  ...
  FOR_(Q) {
    ...
    if (t == 1) {
      LL(x);
      if (isin[x]) continue;
      for (auto&& d: div[x]) sss[d].insert(x);
      continue;
    }
    LL(x, k, s);
    print(f(x, k, s));
