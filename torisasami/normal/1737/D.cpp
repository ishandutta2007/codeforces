...
int main() {
    ...
    while (T--) {
        ...
        rep(i, m) cin >> u[i] >> v[i] >> w[i], u[i]--, v[i]--, d[u[i]][v[i]] = 1, d[v[i]][u[i]] = 1;
        rep(k, n) rep(i, n) rep(j, n) chmin(d[i][j], d[i][k] + d[k][j]);
        ll ans = 1e18;
        // rep(i, n) print(d[i]);
        rep(i, m) {
            ll r = 2e9;
            chmin(r, d[u[i]][0] + d[v[i]][n - 1] + 1);
            chmin(r, d[u[i]][n - 1] + d[v[i]][0] + 1);
            rep(k, n) chmin(r, min(d[u[i]][k], d[v[i]][k]) + d[k][0] + d[k][n - 1] + 2);
            chmin(ans, r * w[i]);
        }
        cout << ans << '\n';
    }
