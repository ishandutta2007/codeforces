...
struct hsh {
    const int operator() (const pair<int,int> &_p) const 
    {
        return _p.x * 5757767 + _p.y;
    }
};

unordered_set<pair<int, int>, hsh> used;
set<pair<ll,pair<int, int>>, greater<pair<ll, pair<int, int>>>> nxt;

pair<int, int> d[] = { {1,0}, {0,1}, {0,-1}, {-1,0} };
pair<int, int> operator+(pair<int, int> &a, pair<int, int> &b) {
    return { a.x + b.x, a.y + b.y };
}

ll val_of(pair<int,int> &p, int n, int m, int r) {
    if (p.x < 0 || p.x >= n || p.y < 0 || p.y >= m) {
        return -1;
    }
    int lx = max(0, p.x - r + 1);
    int rx = min(p.x, n - r);

    int ly = max(0, p.y - r + 1);
    int ry = min(p.y, m - r);
    return (ll)(ry - ly + 1) * (rx - lx + 1);
}

inline void solve(ld tt) {
    int n, m, r, k;
    cin >> n >> m >> r >> k;
    pair<int, int> t = { n / 2,m / 2 };
    nxt.insert({ val_of(t, n ,m, r), t });
    ll ans = 0;
    for (int i = 0; i < k; ++i) {
        auto cur = nxt.begin()->y;
        ans += nxt.begin()->x;
        used.insert(cur);
        nxt.erase(nxt.begin());
        for (auto &p : d) {
