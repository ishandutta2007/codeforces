...
int main() {
    TEST {
        ...
        VV(ll, a, n * 2, n * 2);
        ll ans = 0;
        rep(i, n, 2 * n) rep(j, n, 2 * n) ans += a[i][j];
        vv(ll, d, n * 2, n * 2, inf<ll>);
        pqg<pair<ll, pii>> pq;
        rep(i, n) rep(j, n) {
            d[i][j] = 0;
            pq.emplace(pair(0, pii(i, j)));
        }
        ...
        for(auto i : {n, 2 * n - 1})
            for(auto j : {n, 2 * n - 1}) {
                fore(dx, dy, dx4) {
                    int ni = (i + dx + 2 * n) % (2 * n);
                    int nj = (j + dy + 2 * n) % (2 * n);
                    if(inc(ni, n, 2 * n) and inc(nj, n, 2 * n)) continue;
                    chmin(m, a[ni][nj]);
                }
            }
        ...
        while(!empty(pq)) {
            ...
            fore(dx, dy, dx4) {
                int ni = (i + dx + 2 * n) % (2 * n);
                int nj = (j + dy + 2 * n) % (2 * n);
                if(inc(ni, n, 2 * n) and inc(nj, n, 2 * n)) {
                    if((ni == n or ni == n * 2 - 1) and (nj == n or nj == n * 2 - 1)) {
                        chmin(mi, c);
                    } else {
                        chmin(mi, c + m);
                    }
                } else {
                    if(chmin(d[ni][nj], c + a[ni][nj])) pq.emplace(c + a[ni][nj], pii(ni, nj));
                }
            }
        }
        OUT(ans + mi);
