...
ans_type solve() {
    ...
    rep(i, (n + 1) / 2) {
        rep(f, (m + 1) / 2) {
            ...
            if (2 * i + 1 != n) vals.push_back(a[n - i - 1][f]);
            if (2 * f + 1 != m) vals.push_back(a[i][m - f - 1]);
            if (2 * i + 1 != n and 2 * f + 1 != m) vals.push_back(a[n - i - 1][m - f - 1]);
            sort(vals.begin(), vals.end());
            for (auto x: vals) ans += abs(vals[len(vals) / 2] - x);
        }
    }
    return ans;
}
