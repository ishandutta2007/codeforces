...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  while(que.size()>1){
    auto v1=que.front();que.pop();
    auto v2=que.front();que.pop();
    i=j=0;A=v1.size();B=v2.size();
    vec nextv;
    while(i<A&&j<B){
      if(query(v1[i],v2[j]))nextv.emplace_back(v1[i++]);
      else nextv.emplace_back(v2[j++]);
    }
    while(i<A)nextv.emplace_back(v1[i++]);
    while(j<B)nextv.emplace_back(v2[j++]);
    que.emplace(nextv);
  }
  vec v=que.front();
  rep1(i,N)ans[v[i-1]][v[i]]=1;
  auto ch=v;
  j=0;
  brep(i,N){
    while(ch.size()&&(ch.back()==v[i]||query(v[i],ch)))ch.pop_back();
    ans[v[i]][v[ch.size()]]=1;
    if(ch.empty())break;
  }
  rep(k,N)rep(i,N)rep(j,N)ans[i][j]|=ans[i][k]&ans[k][j];
  print(3);
  rep(i,N){
