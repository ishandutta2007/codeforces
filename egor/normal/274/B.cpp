...
public:
    void solve() {
        ...
        RecursiveFunction dfs = [&](const auto& self, int vert, int last) -> pair<ll, ll> {
            ll pl = 0;
            ll mi = 0;
            for (auto& e : graph[vert]) {
                ...
                auto call = self(next, vert);
                maxim(pl, call.first);
                maxim(mi, call.second);
            }
            if (last != -1) {
                if (d[vert] > d[last]) {
                    pl += d[vert] - d[last];
        ...
        auto call = dfs(0, -1);
        ll answer = call.first + call.second + abs(d[0] + call.first - call.second);
