...
#define int ll
int check(vector<pair<int, int>> inputs) {
	int ans = 0;
	int now = inputs[0].second;
	for (int i = 1; i < inputs.size(); ++i) {
		if (inputs[i].second > now or i == inputs.size()-1) {
			ans += max(0LL, inputs[i].first - now);
			now = inputs[i].second;
		}
	}
	return ans;
...
void solve() {
	...
	REP(i, n) {
		...
		pre_ans += b;
		if (i == 0) tmp = mp(a, a+b);
		inputs.push_back(make_pair(a, a + b));
	}
	sort(ALL(inputs));
	int itr = 0;
	REP(i, n) {
	...
	REP(i, n) {
			go[0].push_back(inputs[i]);
	}
	cout << pre_ans + check(go[0]) << endl;
}
#undef int 
int main() {
	init();
