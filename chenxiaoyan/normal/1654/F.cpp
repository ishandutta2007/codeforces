...
void mian() {
	n = 1 << read(), reads(a);
	REP(i, 0, n - 1) rl[i] = i;
	sort(rl, rl + n, [&](int x, int y) { return a[x] < a[y]; });
	for(int l = 0, r; l < n; l = r + 1) {
		r = l; while(r + 1 < n && a[rl[r + 1]] == a[rl[l]]) ++r;
		REP(i, l, r) ord[rl[i]] = r;
	}
	for(int d = 2; d <= n; d *= 2) {
		static int rk[N];
		for(int i = 0; i < n; i += d) {
			REP(j, i, i + d / 2 - 1) rk[j] = 1000000000 * ord[j] + ord[j + d / 2];
			REP(j, i + d / 2, i + d - 1) rk[j] = 1000000000 * ord[j] + ord[j - d / 2];
		}
		sort(rl, rl + n, [&](int x, int y) { return rk[x] < rk[y]; });
		for(int l = 0, r; l < n; l = r + 1) {
			r = l; while(r + 1 < n && rk[rl[r + 1]] == rk[rl[l]]) ++r;
			REP(i, l, r) ord[rl[i]] = r;
		}
	}
	int key = rl[0];
	REP(i, 0, n - 1) pc(a[i ^ key]);
	pc('\n');
}
