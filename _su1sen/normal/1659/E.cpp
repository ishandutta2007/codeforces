...
int main() {
    ...
    vector<int8_t> evn(n, false);

    vector<atcoder::dsu> ufs(L, atcoder::dsu{n});
    rep(i, m) {
        input(int, u, v, w);
        --u, --v;
        if (w % 2 == 0) {
    ...
    vector<int8_t> is_ok(n, false);
    rep(i, 1, L) {
        for (auto group : ufs[i].groups()) {
            if (none_of(all(group), [&](int v) { return evn[v]; })) continue;
            for (int v : group) is_ok[v] = true;
        }
    }

    auto query = [&](int u, int v) -> int {
        rep(i, L) {
            if (ufs[i].same(u, v)) return 0;
        }
        if (is_ok[u]) return 1;
        return 2;
    };
