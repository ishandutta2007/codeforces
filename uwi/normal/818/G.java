...
public class G {
	...
	void solve()
	{
		...
		int ssrc = sink + 1, ssink = ssrc + 1;
		es.add(new Edge(src, ssrc, 4, 0));
		es.add(new Edge(sink, ssink, 4, 0));
		for(int i = 0;i < n;i++){
			addNegativeEdge(es, new Edge(i, i+n, 1, -1), ssrc, ssink);
//			es.add(new Edge(i, i+n, 1, -1));
			es.add(new Edge(src, i, 1, 0));
			es.add(new Edge(i+n, sink, 1, 0));
			es.add(new Edge(i+2*n, i, 1, 0));
			es.add(new Edge(i+3*n, i, 1, 0));
			for(int j = i+1;j < n;j++){
				if(a[i] == a[j]){
					es.add(new Edge(i+2*n, j+2*n, 9, 0));
					break;
				}
			}
			for(int j = i+1;j < n;j++){
				if(a[i]+1 == a[j]){
					es.add(new Edge(i+n, j+2*n, 1, 0));
					break;
				}
			}
			for(int j = i+1;j < n;j++){
			...
			for(int j = i+1;j < n;j++){
				if(a[i] % 7 == a[j] % 7){
					es.add(new Edge(i+3*n, j+3*n, 9, 0));
					es.add(new Edge(i+n, j+3*n, 1, 0));
					break;
				}
			}
		}
//		for(Edge e : es){
//			tr(e.from, e.to, e.cost);
//		}
		long F = solveMinCostFlow(compileWD(ssink+1, es), ssrc, ssink, 4+n);
		out.println(-F + n);
	}
