...
fun main(){
    val max = 2000001
    // 0 not taken, 1 taken
    val DP = Array(2){IntArray(max){nlarge} }
    for(i in 0 until 2){
        DP[i][1] = 0
        DP[i][2]= 0
    }
    ...
    for(i in 3 until max){
        DP[0][i] = (2 modM DP[1][i-2]) modPlus DP[1][i-1]
//        DP[1][i] = (2 modM DP[0][i-2]) modPlus DP[0][i-1] modPlus 1
        //maxOf(DP[1][i],DP[0][i])

        DP[1][i] = if(i % 3 == 0) DP[0][i] modPlus 1 else DP[0][i]
    }
    solve.cases{
        val n = getint()
         put(DP[1][n] modM 4)
