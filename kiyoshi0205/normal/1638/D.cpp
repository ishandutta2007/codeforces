...
int main(){
  ...
  rep(i,H-1)rep(j,W-1){
    if(g[i][j]==g[i+1][j]&&g[i][j]==g[i][j+1]&&g[i][j]==g[i+1][j+1]){
      que.emplace(i,j,g[i][j]);
    }
  }
  while(!que.empty()){
    auto [x,y,c]=que.front();que.pop();
    if((g[x][y]|g[x+1][y]|g[x][y+1]|g[x+1][y+1])==0)continue;
    ans.emplace_back(x+1,y+1,c);
    g[x][y]=g[x+1][y]=g[x][y+1]=g[x+1][y+1]=0;
    for(int i=x-1;i<=x+1;++i){
      for(int j=y-1;j<=y+1;++j){
        if(i>=0&&j>=0&&i+1<H&&j+1<W){
          auto c=g[i][j]|g[i+1][j]|g[i][j+1]|g[i+1][j+1];
          if(c==0)continue;
          if(g[i][j]!=c&&g[i][j])continue;
          if(g[i+1][j]!=c&&g[i+1][j])continue;
          if(g[i][j+1]!=c&&g[i][j+1])continue;
          if(g[i+1][j+1]!=c&&g[i+1][j+1])continue;
          que.emplace(i,j,c);
        }
      }
    }
