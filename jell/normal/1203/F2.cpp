...
void input() {
    ...
    for(int i=0; i<n; ++i) {
        ...
        if(b>=0) {
            ...
            nts.emplace_back(a,b);
        }
    }
    sort(all(pts));
    sort(all(nts),[](auto x,auto y){return x.fir+x.sec>y.fir+y.sec;});
    tsk=pts;
    for(auto &e:nts) {
        tsk.emplace_back(e);
    }
...
void solve() {
    ...
    int i=0;
    for(auto &e:tsk) {
        for(int w=0; w<=60000; ++w) dp[i^1][w]=dp[i][w]; 
        for(int w=e.fir; w<=60000; ++w) {
            chmax(dp[i^1][w+e.sec],1+dp[i][w]);
        }
        i^=1;
    }
    write(*max_element(dp[i],dp[i]+60001));
}