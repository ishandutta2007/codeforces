...
int main() {
    ...
    double a, b, c; re(a, b, c);

    double ans = 0.;

    {
        double tri_ht = sqrt(.75 * a * a);
        double height = sqrt(a * a - pow(2 * tri_ht/3, 2));
        ans += a * tri_ht / 2 * height / 3;
    }

    {
        double base = b * b;
        double leg = sqrt(2) * b / 2;
        double height = sqrt(b * b - leg * leg);
        ans += base * height / 3;
    }

    {
        ...
        cerr << PI << endl;
        double inradius = c / (2 * tan(PI / 5));
        double leg = sqrt(inradius * inradius + (c/2) * (c/2));
        double height = sqrt(c * c - leg * leg);

        double area = c * c / 4 * sqrt(25 + 10 * sqrt(5));
        ans += area * height / 3;
    }
