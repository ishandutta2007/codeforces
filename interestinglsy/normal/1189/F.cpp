...
int dpsum[MAXN][MAXN];
int Solve( int delta ){	//  delta
	if( delta > mxa/(k-1) ) return 0;
	For(i,n) memset(dpsum[i],0,k+1<<2); //For(j,k) dpsum[i][j] = 0;
	// Ms(dp); Ms(dpsum);
	int pre = 0;
	int ans = 0;
	For(i,n){
		...
		Forx(j,2,min(i,k)){
			int u = j == 2 ? pre : dpsum[pre][j-1];
			dpsum[i][j] = Mymo(dpsum[i-1][j]+u);
			if( j == k ){
				ans = Mymo(ans+u);
				Mymod(ans);
			}
