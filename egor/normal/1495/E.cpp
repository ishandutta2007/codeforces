...
class ETsinshanIDaniel {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        arri p, k, b, w;
        in.readArrays(m, p, k, b, w);
        ...
        for (int i : range(n)) {
            ...
            int rem = a[i];
            while (rem > 0) {
                while (t[at] == 0 || ans[at] == a[at]) {
                    at++;
                    if (at == n) {
                        at = 0;
                    }
                }
                int cur = min(rem, a[at] - ans[at]);
                ans[at] += cur;
                rem -= cur;
            }
        }
