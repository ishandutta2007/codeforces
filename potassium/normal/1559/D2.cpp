...
struct dsu {
	...
	bool uni(int u, int v) {
		...
		sz_order.erase({ sz[u], u });
		sz_order.erase({ sz[v], v });

		p[v] = u;
		sz[u] += sz[v];
		gp[u].insert(gp[u].end(), all(gp[v]));

		sz_order.insert({ sz[u], u });
...
int main() {
	...
	vector<pii> ans;
	while (m1 + (int)ans.size() + 1 < n) {
		assert((int)dsu1.sz_order.size() >= 2);
		assert((int)dsu2.sz_order.size() >= 2);

		int x = -1;
		const int v = dsu2.gp[dsu2.sz_order.begin()->second][0];
		for (auto& u : dsu1.gp[dsu1.sz_order.begin()->second])
			if (dsu2.find(u) == dsu2.find(v)) {
				x = u;
				break;
			}

		const int u = dsu1.gp[dsu1.sz_order.begin()->second][0];
		if (x == -1) {
			ans.push_back({ u, v });
			assert(dsu1.uni(u,v));
			assert(dsu2.uni(u,v));
		...
		int w = 1;
		while (w <= n && (dsu1.find(u) == dsu1.find(w) || dsu2.find(v) == dsu2.find(w)))
			w++;
		assert(w != n + 1);
