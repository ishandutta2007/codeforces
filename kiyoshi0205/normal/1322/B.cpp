...
int main(){
  ...
  for(int t=24;t>=0;--t){
    //t bit
    for(auto& x:a)x&=(1<<(t+1))-1;
    vsort(a);
    int cnt=0;
    const int T=1<<t;
    for(int i=0;i<N;++i){
      const auto& x=a[i];
      //[i+1,N)T-x<=y<2*T-x,3*T-x<=y<4*T-x
      cnt^=lower_bound(a.begin()+i+1,a.end(),T*2-x)-lower_bound(a.begin()+i+1,a.end(),T-x);
      cnt^=lower_bound(a.begin()+i+1,a.end(),T*4-x)-lower_bound(a.begin()+i+1,a.end(),T*3-x);
    }
    if(cnt&1)ans|=1<<t;
  }
  print(ans);
