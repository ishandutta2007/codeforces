...
void solve() {
    ...
    string ans;
    REP(tea, 2) {
        vector<int> going = manacher(s);
        vector<tuple<int, int, int>> checks;//ans, left,right
        REP(i, s.length() / 2 + 1) {
            int left = i - going[i];
            int right = i + going[i];
            int now_ans = (left + 2LL) / 2LL;
            now_ans *= 2LL;
            if (i % 2 == 1) {
                now_ans += (going[i] / 2LL) * 2LL;
            }
            else {
                now_ans += 1 + ((going[i] - 1LL) / 2LL) * 2LL;
            }
            checks.push_back(make_tuple(now_ans, left, right));
        }
        sort(ALL(checks));
        reverse(ALL(checks));
        int left_border = 1e9;
        ...
        REP(q, s.length()) {
            ...
            if (s[q] != ' ')
                aa.push_back(s[q]);
            if (s[s.length() - 1 - q] != ' ')
                bb.push_back(s[s.length() - 1 - q]);
        }
        reverse(ALL(bb));
        aa += bb;
        if (ans.length() < aa.length()) {
            ans = aa;
        }
        REP(q, checks.size()) {
            if (left_border <= get<1>(checks[q])) continue;
            //construct
            string now_ans;
            REP(j, get<2>(checks[q])) {
                if (s[j] == ' ') continue;
                now_ans.push_back(s[j]);
            }
            string d;
            for (int j = 0; j < get<1>(checks[q]) + 1; ++j) {
                if (s[j] == ' ') continue;
                d.push_back(s[s.length() - 1 - j]);
            }
            reverse(ALL(d));
            now_ans += d;
            assert(now_ans.length() <= hoget);
            if (now_ans.length() > ans.length()) {
                ans = now_ans;
            }
            break;
        }
