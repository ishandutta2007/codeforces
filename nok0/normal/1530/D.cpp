...
void main_() {
	...
	if(SZ(none) >= 2) {
		...
		//
		reverse(all(notused));
		//assert(SZ(none) == SZ(notused));
		reverse(all(none));
		REP(i, SZ(notused)) {
			if(notused[i] == none[i]) {
				if(i == 0) {
					swap(notused[i], notused[SZ(notused) - 1]);
				} else
					swap(notused[i], notused[0]);
			}
		}
		REP(i, SZ(notused)) {
			b[notused[i]] = none[i];
		}
		print(accumulate(all(used), 0));
		ADD(b, 1);
		print(b);
		return;
	}
