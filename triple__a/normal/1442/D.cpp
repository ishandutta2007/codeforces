...
int n,k;
vi a[maxn];
int dp[maxn],sz[maxn],sum[maxn];
int res=0;

void solve(int l,int r,int t[]){
    int b[maxn],c[maxn];
    for (int i=0;i<=k;++i) b[i]=c[i]=t[i];
    if (l==r){
        int u=0;
    ...
    int md=(l+r)>>1;
    for (int i=l;i<=md;++i){
        for (int j=k;j>=sz[i];--j) b[j]=max(b[j],b[j-sz[i]]+sum[i]);
    }
    for (int i=md+1;i<=r;++i){
        for (int j=k;j>=sz[i];--j) c[j]=max(c[j],c[j-sz[i]]+sum[i]);
    }
    solve(l,md,c);
    solve(md+1,r,b);
}
