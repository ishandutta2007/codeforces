...
int main(){
    ...
    MCMF mcmf(n*(n+3)/2+1,50000,0,n*(n+3)/2+1);
    int cnt=0;
    for (int i=1;i<=n;++i){
        for (int j=1;j<i;++j){
            mcmf.insert(0,++cnt,1,0);
            if ((a[i][j]+1)&1) mcmf.insert(cnt,n*(n-1)/2+j,1,0);
            if ((a[i][j]+1)&2) mcmf.insert(cnt,n*(n-1)/2+i,1,0);
        }
    }
    for (int i=1;i<=n;++i){
        for (int j=1;j<=n;++j){
            mcmf.insert(n*(n-1)/2+i,n*(n+1)/2+j,1,2*j-1);
        }
        mcmf.insert(n*(n+1)/2+i,n*(n+3)/2+1,n,0);
    }
    ...
    for (int i=1;i<=n;++i){
        for (int j=1;j<i;++j){
            ++cnt;
            for (int k=mcmf.p[cnt];k>-1;k=mcmf.g[k].next){
                if (!mcmf.g[k].rem){
                    a[i][j]=(mcmf.g[k].to==n*(n-1)/2+i)?1:0;
                }    
            }
        }
    }
