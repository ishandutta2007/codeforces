...
int main(){
    ...
    FORR(i,in)
        sc(i.e1,i.e2);
    sort(ph,ph+n,[](const pi &f, const pi &s){return f.e1 == s.e1 ? f.e2 < s.e2 : f.e1 > s.e1;});
    sort(in,in+m,[](const pi &f, const pi &s){return f.e1 == s.e1 ? f.e2 < s.e2 : f.e1 > s.e1;});
    if(in[0].e2+ph[0].e2 > d) REE(0)
    ll ans;
    ll cur = ans = in[0].e1+ph[0].e1;
    ll rem = d-in[0].e2-ph[0].e2;
    int ind = 1;
    for(; ind < m; ++ind){
        if(rem < in[ind].e2)
            break;
