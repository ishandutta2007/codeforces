...
int main() {
    ...
    vector<vector<pair<int, int>>> ps(M);
    loop(n) {
        input(int, l, r, w);
        --l, --r, --w;
        ps[w].emplace_back(l, r);
    }
    
    int ans = M;
    atcoder::lazy_segtree<int, op, e, int, mapping, composition, id> seg(vector<int>(m - 1, M));
    rrep(i, M) {
        for (const auto &[l, r] : ps[i]) seg.apply(l, r, i);
        int r = seg.all_prod();
        if (r == M) continue;
        chmin(ans, r - i);
    }
