...
int main(){
    ...
    f(mask,1,(1<<n)){
    	...
    	//cout<<Pow[mask][n-Cnt(mask)]<<" "<<n-Cnt(mask)<<" "<<Pow[mask][0]<<endl;
    	//cout<<mask<<" "<<1ll*Cnt(mask)*dp[mask]%mod*Pow[mask][n-Cnt(mask)]%mod*mv[(1<<n)-mask-1]%mod<<endl;
    	//Add(ans,1ll*Cnt(mask)*dp[mask]%mod*Pow[mask][n-Cnt(mask)]%mod%mv[(1<<n)-mask-1]%mod);
	}
	//eror(ans);
