...
vi f(vi x, vi y) {
    int i = 0, j = 0;
    x.eb(inf<int>), y.eb(inf<int>);
    vi res;
    while(i < si(x) and j < si(y)) {
        if(x[i] < y[j])
            res.eb(x[i++]);
        else
            res.eb(y[j++]);
...
main() {
    ...
    auto g = getG(n);
    vv(int, a, n);
    rep(i, m) a[in() - 1].eb(i + 1);
    rep(i, n) {
        sort(all(a[i]));
        while(a[i].size() > 10) a[i].pop_back();
    }
    HLDecomposition hld(g);
    auto v = hld.build(a);
    SegmentTree<vi> seg(v, f, vi());
    while(Q--) {
        INT(v, u, a);
        v--, u--;
        auto res = hld.query(
            u, v, vi(), [&](int i, int j) { return seg.query(i, j); }, f);
        while(si(res) > a) res.pop_back();
        cout << si(res);
        rep(i, si(res)) cout << " " << res[i];
        cout << endl;
    }
}