...
int main() {
    ...
    sort(all(v));
    if(empty(v)) cout << modpow(2, n - 1) << endl, exit(0);

    vector<tuple<int, int, mint>> now;
    int pre;
    {
        auto [a, i] = *begin(v);
        pre = a;
        if(a == 1) {
            ...
            if(a <= i) now.eb(i - a + 1, i, modpow(2, a - 2));
            if(i + a - 1 <= n) now.eb(i, i + a - 1, modpow(2, a - 2));
        }
        v.erase(begin(v));
    }

    for(auto [a, i] : v) {
        vector<tuple<int, int, mint>> nxt;
        int mov = a - pre;
        pre = a;
        for(auto [l, r, tmp] : now) {
            if(l <= i and i <= r) continue;
            if(i < l) {
                int nl = i, nr = r + (mov - (l - i));
                nxt.eb(nl, nr, tmp * C(mov - 1, l - i - 1));
            } else {
                int nr = i, nl = l - (mov - (i - r));
                nxt.eb(nl, nr, tmp * C(mov - 1, i - r - 1));
            }
        }
        swap(nxt, now);
    }
    mint ans;
    for(auto [l, r, tmp] : now) { ans += tmp * C(n - pre, l - 1); }
    cout << ans << endl;
}