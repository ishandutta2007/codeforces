...
void draw(int cur, int l, int r, int x, int y, int val) {
    ...
    if (x <= l && r <= y) {
        nodes[cur].pu = val;
        nodes[cur].tr = val * (r - l + 1LL);
        return;
    }
    int dcur = cur + cur;
    int m = (l + r) / 2;
    if (nodes[cur].pu != -1) {
        int pucur = nodes[cur].pu;
        nodes[dcur].pu = pucur;
        nodes[dcur + 1].pu = pucur;
        nodes[dcur].tr = pucur * (m - l + 1LL);
        nodes[dcur + 1].tr = pucur * 1LL * (r - m);
        nodes[cur].pu = -1;
    }
    draw(dcur, l, m, x, y, val);
    draw(dcur + 1, m + 1, r, x, y, val);
    nodes[cur].tr = nodes[dcur].tr + nodes[dcur + 1].tr;
}
ll sum(int cur, int l, int r, int x, int y) {
    if (x > r || y < l) return 0;
    if (x <= l && r <= y) return nodes[cur].tr;
    int dcur = cur + cur;
    int m = (l + r) / 2;
    if (nodes[cur].pu != -1) {
        int pucur = nodes[cur].pu;
    ...
    for (int h = 0; h <= 500000; ++h) {
        ...
        for (int i = cnt - 2; i >= 0; --i) {
            ...
            if (v != prev) {
                ...
                auto from = 0;
                draw(1, 0, cnt - 1, from, to, ::h[v]);
                curSum = sum(1, 0, cnt - 1, i + 1, cnt - 1);
                prev = v;
            } else {
                curSum += ::h[v];
            }
