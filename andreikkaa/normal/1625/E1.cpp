...
void dfs(int v, int p) {
    ...
    for(int i : g[v]) {
        ...
        ans[v] += ans[i];
        ps[v].push_back(ans[i]);
    }

    partial_sum(all(ps[v]), ps[v].begin());
    ans[v] += ll(sz(g[v])) * (sz(g[v]) - 1) / 2;
    ++ans[v];

    //cout << v << " " << ans[v] << endl;
}

int get_id(int v, int x) {
    auto it = lower_bound(all(g[v]), x);
    assert(it != g[v].end() && *it == x);
    return it - g[v].begin();
}
...
void _solve() {
    ...
    for(int _ = 0; _ < q; ++_) {
        ...
        ll ans = ll(ri - li + 1) * (ri - li) / 2;
        ans += ps[pl][ri] - (li == 0 ? ll(0) : ps[pl][li - 1]);
        cout << ans << endl;
    }
}
