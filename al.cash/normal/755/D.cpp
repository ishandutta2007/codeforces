...
class SegmentTreeSimple {
  ...
  template <class U = T> decltype(U() += U(), T())
  query(int l, int r, int) const {
    T res{};
...
public:
  ...
  template <class U = T> decltype(U() += U(), void())
  modify(int i, const T& value) {
    check(i);
...
#define WRAPPER(Name, defaultValue, maxValue, sum, add) \
template <class T> struct Name { \
T v; \
constexpr Name(const T& v = defaultValue) : v(v) {} \
explicit operator T() const { return v; } \
explicit operator bool() const { return v != defaultValue; } \
void operator += (const Name& other) { add; } \
friend Name operator + (const Name& lhs, const Name& rhs) { return sum; } \
}; \
namespace std { \
template <class T> struct numeric_limits<Name<T>> { \
constexpr static T max() { return numeric_limits<T>::max(); } \
}; \
}
WRAPPER(Sum, 0, numeric_limits<T>::max(), lhs.v + rhs.v, v += other.v)
WRAPPER(Min, numeric_limits<T>::max(), -numeric_limits<T>::max(),
        min(lhs.v, rhs.v), smin(v, other.v))
WRAPPER(Max, -numeric_limits<T>::max(), numeric_limits<T>::max(),
        max(lhs.v, rhs.v), smax(v, other.v))
#undef WRAPPER
...
int main() {
  ...
  read(n, k);
  if (2 * k > n) k = n - k;
  SegmentTreeSimple<Sum<int>> tree(n);
  int x = 0;
  ll sum = 1;
  REP(i, n) {
    int y = x + k;
    if (y >= n) {
      y -= n;
      if (x + 1 != n) sum += (int)tree.query(x + 1, n);
      if (y != 0) sum += (int)tree.query(0, y);
    } else {
      sum += (int)tree.query(x + 1, y);
    }
    tree.modify(x, 1);
    tree.modify(y, 1);
    x = y;
