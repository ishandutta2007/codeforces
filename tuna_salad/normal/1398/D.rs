...
fn main() {
	...
	l!(rn,gn,bn = rin.u());
	let mut r = rin.vi(rn); r.sort();
	let mut g = rin.vi(gn); g.sort();
	let mut b = rin.vi(bn); b.sort();
	let mut dp = v!([rn+1][gn+1][bn+1] = 0);
	let mut sol = 0;
	for i in (0..=rn).rev() {
		for j in (0..=gn).rev() {
			for k in (0..=bn).rev() {
				...
				max!(sol, z);
				if i * j > 0 {
					max!(dp[i-1][j-1][k], z + r[i-1] * g[j-1]);
				}
				if i * k > 0 {
					max!(dp[i-1][j][k-1], z + r[i-1] * b[k-1]);
				}
				if j * k > 0 {
					max!(dp[i][j-1][k-1], z + g[j-1] * b[k-1]);
				}
			}
		}
	}
