...
void solve()
{
	...
	int flag = 0;
	function<tuple<int, int, int>(int, int)> go = [&](int L, int R) -> tuple<int, int, int>
	{
		if (L == R)
		{
			return { a[L], a[L], 0 };
		...
		int m = (L + R) / 2;
		auto [la, lb, lc] = go(L, m);
		auto [ra, rb, rc] = go(m + 1, R);
		if (lc == -1 || rc == -1) return { -1, -1, -1 };
		if (la <= lb && lb <= ra && ra <= rb)
			return { la, rb, lc + rc };
		if (ra <= rb && rb <= la && la <= lb)
			return { ra, lb, lc + rc + 1 };
		return { -1, -1, -1 };
	};
	auto [x, y, z] = go(0, m - 1);
	writeln(z);
