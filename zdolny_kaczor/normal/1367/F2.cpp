...
int main() {
	...
	REP(_, t) {
		...
		for (int i = 0; i < (int) v.size(); ) {
			int start = i, end = i;
			while (end + 1 < (int) v.size() && v[end].back() < v[end + 1][0]) end++;
			i = end + 1;
			int in = 0;
			debug(range(v, start, end + 1));
			for (int j = start; j <= end; ++j) in += v[j].size();
			debug(imie(in));
			if (start) {
				for (int x : v[start - 1]) if (x < v[start][0]) in++;
			}
			if (end + 1 != (int)v.size()) {
				for (int x : v[end + 1]) if (x > v[end].back()) in++;
			}
			maxi(longest, in);
		}
		for (int i = 0; i + 1 < (int) v.size(); ++i) {
			vi a = v[i], b = v[i + 1];
			vpii in;
			for (int x : a) in.emplace_back(x, 0);
			for (int y : b) in.emplace_back(y, 1);
			sort(ALL(in));
			int as = 0, bs = b.size();
			maxi(longest, bs);
			for (auto [_, c] : in) {
