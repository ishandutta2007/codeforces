...
int main() {
    ...
    auto cmp = [&](int u, int v) { return lca.first_visit[u] < lca.first_visit[v]; };

    int Q; re(Q); F0R (q, Q) {
        int K, M; re(K, M);

        vpii virus(K);
        F0R (k, K) { int v, s; re(v, s); virus[k] = { v-1, s }; }

        vi query(M); re(query); trav (e, query) --e;

        vi poi;
        {
            poi = query;
            for (auto [v, s] : virus) poi.pb(v);
            sort(all(poi), cmp);
            FORd (i, 1, sz(poi)) poi.pb(lca.lca(poi[i-1], poi[i]));
            sort(all(poi), cmp);
            poi.erase(unique(all(poi)), poi.end());
        }
        auto index = [&](int u) -> int {
            return lb(all(poi), u, cmp) - poi.begin();
        };
        ...
        vvi adj(P);
        {
            ...
            above.push(0);
            FOR (i, 1, sz(poi)) {
                while (!lca.is_ancestor(poi[above.top()], poi[i])) above.pop();
                adj[above.top()].pb(i);
                adj[i].pb(above.top());
                above.push(i);
            }
        ...
        vi infect(P, -1);
        {
            ...
            F0R (k, K) {
                pq.push({ 0, k, virus[k].f = index(virus[k].f) });
            }
            while (!pq.empty()) {
                auto [turn, vid, loc] = pq.top();
                pq.pop();
                if (infect[loc] != -1) continue;
                infect[loc] = vid;

                for (auto nbr : adj[loc]) {
                    int plen = lca.dist(poi[virus[vid].f], poi[nbr]);
                    int turns = ceil(plen, virus[vid].s);
                    pq.push({ turns, vid, nbr });
                }
            }
        }
