...
void run_case() {
    ...
    vector<int> recent(K + 1, N);
    vector<vector<int>> dp(N + 1, vector<int>(K + 1, INF));
    dp[N] = vector<int>(K + 1, 0);

    for (int i = N - 1; i >= 0; i--) {
        if (closest[A[i]] >= 0) {
            auto it = lower_bound(recent.begin(), recent.end(), closest[A[i]]);
            recent.insert(it, closest[A[i]]);
            recent.pop_back();
        }
        ...
        for (int a = 0; a <= K; a++) {
            ...
            for (int b = 0; a + b <= K; b++)
                dp[i][a + b] = min(dp[i][a + b], dp[end][b] + 1);
        }
