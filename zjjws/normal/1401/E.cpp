...
inline void add(int x,int y){for(;x<=S;x+=lowbit(x))f[x]+=y;return;}
...
int main()
{
    ...
    {
        rin>>y>>l>>r;
        if(l==0&&r==S){Y.push_back(y);continue;}
    ...
    {
        rin>>x>>l>>r;
        if(l==0&&r==S){X.push_back(x);continue;}
        if(l==0)U.push_back(gyq(x,r));
        if(r==S)D.push_back(gyq(x,l));
    }
    ans=1LL*(X.size()+1)*(Y.size()+1);
    sort(X.begin(),X.end());sort(Y.begin(),Y.end());
    sort(L.begin(),L.end(),myru_x);sort(R.begin(),R.end(),myru_x);
    sort(U.begin(),U.end(),myru_x);sort(D.begin(),D.end(),myru_x);
    ...
    for(;i!=R.end();i++)
    {
        ...
        ans+=check(i->y)+(X.size()-(X.end()-upper_bound(X.begin(),X.end(),i->x)));
    }
    for(;u!=U.end();u++)add(1,1),add(u->y+1,-1);
    for(;d!=D.end();d++)add(d->y,1);
    i=L.begin();u=U.begin();d=D.begin();
    for(;i!=L.end();i++)
    {
        for(;u!=U.end()&&u->x<i->x;u++)add(1,-1),add(u->y+1,1);
        for(;d!=D.end()&&d->x<i->x;d++)add(d->y,-1);
    ...
    for(i=U.begin();i!=U.end();i++)ans+=(Y.size()-(Y.end()-upper_bound(Y.begin(),Y.end(),i->y)));
    for(i=D.begin();i!=D.end();i++)ans+=(Y.end()-lower_bound(Y.begin(),Y.end(),i->y));
    printf("%lld\n",ans);
    return 0;
}