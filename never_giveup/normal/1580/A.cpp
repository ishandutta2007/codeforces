...
const int maxn = 310 + 100, maxw = 5e6 + 10, inf = 1e9 + 100, LG = 20, SQ = 450;

int ri[maxn][maxn], dw[maxn][maxn];

int n, m, gr[maxn][maxn];

int dok[maxn], zuz[maxn];
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        ri[i][m] = 0;
        for (int j = m - 1; j >= 0; j--)
            ri[i][j] = ri[i][j + 1] + gr[i][j];
    }
    for (int j = 0; j < m; j++) {
        dw[j][n] = 0;
        for (int i = n - 1; i >= 0; i--)
            dw[j][i] = dw[j][i + 1] + gr[i][j];
    }
    auto hor = [&](int i, int l, int r) {
        return l <= r ? ri[i][l] - ri[i][r + 1] : 0;
    };
    auto ver = [&](int j, int l, int r) {
        return l <= r ? dw[j][l] - dw[j][r + 1] : 0;
    };
    int best = inf;
    for (int il = 0; il < n; il++)
        for (int ir = il + 4; ir < n; ir++) {
            int bsum = inf;
            dok[m] = 0;
            for (int j = m - 1; j >= 0; j--) {
                if (j + 3 < m)
                    setmin(bsum, dok[j + 1] - dok[j + 3] + zuz[j + 3]);
                int s = ver(j, il + 1, ir - 1);
                int sos = (ir - il - 1 - s);
                zuz[j] = sos;
                if (bsum != inf)
                    setmin(best, bsum + sos);
                int val = s + !gr[il][j] + !gr[ir][j];
                dok[j] = dok[j + 1] + val;
                if (bsum != inf)
                    bsum += val;
            }
