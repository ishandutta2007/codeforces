...
public class TaskB {
	...
	private void solve() {
		...
		while (right - left > 1e-8) {
			...
			for (int energy : a) {
				if (energy > current)
					deficit -= k * (energy - current) / 100.;
				else
					deficit += current - energy;
			}
			if (deficit > 0)
				right = current;
			else
	...
	private static List<Test> createTests() {
		...
		tests.add(new Test("3 50\n" +
				"4 2 1", "2"));
		tests.add(new Test("2 90\n" +
				"1 11", "1.909090909"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
...
//		return tokenCheck(result, output);
		return isDoubleEquals(Double.parseDouble(output), Double.parseDouble(result.trim().replace(',', '.')), 1e-6) ? null : result.trim();
	}
