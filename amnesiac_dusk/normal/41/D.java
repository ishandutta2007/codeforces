...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		for(int i=0; i<n; i++)
			for(int j=0; j<m; j++)
				for(int l=0; l<k; l++)
					dp[i][j][l]=-10000000;
		for(int j=0; j<m; j++)
			dp[0][j][bo[0][j]%k]=bo[0][j];
		for(int i=0; i<n-1; i++)
			for(int j=0; j<m; j++)
				for(int l=0; l<k; l++)
				{
					if(dp[i][j][l]!=-10000000)
					{
						if(j!=m-1)
							dp[i+1][j+1][(dp[i][j][l]+bo[i+1][j+1])%k]=
							Math.max(dp[i+1][j+1][(dp[i][j][l]+bo[i+1][j+1])%k], dp[i][j][l]+bo[i+1][j+1]);
						if(j!=0)
							dp[i+1][j-1][(dp[i][j][l]+bo[i+1][j-1])%k]=
							Math.max(dp[i+1][j-1][(dp[i][j][l]+bo[i+1][j-1])%k], dp[i][j][l]+bo[i+1][j-1]);
					}
		...
		while(i>0)
		{
			if(j!=0&&dp[i-1][j-1][(dp[i][j][val%k]-bo[i][j])%k]==val-bo[i][j])
			{
				p("R");
				val-=bo[i][j];
				i--;
