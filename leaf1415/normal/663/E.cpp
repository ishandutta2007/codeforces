...
int main(void)
{
	...
	rep(i, 0, n-1) rep(j, 1, m) cin >> c[i][j];
	rep(j, 1, m){
		ll b = 0;
		rep(i, 0, n-1) if(c[i][j] == '1') b |= 1LL<<i;
		a[b]++;
	}
	
	ll N = 1<<n;
	rep(i, 1, N-1) pop[i] = pop[i&(i-1)] + 1;
	rep(i, 0, N-1) chmin(pop[i], n-pop[i]);
	
	FWT(a, n), FWT(pop, n);
	rep(i, 0, N-1) a[i] *= pop[i];
	IFWT(a, n);
	
	ll ans = inf;
	rep(i, 0, N-1) chmin(ans, a[i]);
	outl(ans);
