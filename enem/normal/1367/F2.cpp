    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            {
                ans=max(ans,t+upper_bound(a,a+n,MP(a[i].F,n+1))-lower_bound(a,a+n,MP(a[i].F,a[i-1].S)));
                t=1+upper_bound(a,a+n,MP(a[i-1].F,a[i].S))-lower_bound(a,a+n,MP(a[i-1].F,-1LL));
            }
            ans=max(ans,t);
        }
        for(auto it=mp.begin();it!=mp.end();it++)
        {
            ...
            if(it2!=mp.end())
            {
                REP(i,0,(*it).S.size())
                {
                    ans=max(ans,i+1+(ll)(*it2).S.size()-(ll)(upper_bound((*it2).S.begin(),(*it2).S.end(),(*it).S[i])-(*it2).S.begin()));
                }
            }
        }
        cout<<n-ans;
