...
int n;
ll query(ll x) {
	int it = lower_bound(point, point + n + 1, x) - point;
	debug(imie(x), arr(point, it));
	if (point[it] == x) return hugs[it];
	return hugs[it - 1] + c2(x - point[it - 1]);
}
int main() {
	ll x;
	scanf("%d%lld", &n, &x);
	...
	ll ans = 0;
	for (int i = 0; i < n; ++i) {
		point[i + 1] = point[i] + d[i];
		hugs[i + 1] = hugs[i] + c2(d[i]);
	}
	debug(range(point, 0, n + 1), range(hugs, 0, n + 1));
#ifdef LOCAL
	for (ll i = 0; i <= point[n]; ++i)
		debug(fun(query, i));
#endif
	for (int i = 0; i <= n; ++i) {
		ll e = point[i];
		ll b = point[i] - x;
		if (b >= 0) maxi(ans, query(e) - query(b));
	}
	printf("%lld\n", ans + x);
	exit(0);
}
