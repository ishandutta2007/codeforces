...
int opcross(ipair p0,ipair p1,ipair p2)
{
	int64 t=(int64)(p1.first-p0.first)*(int64)(p2.second-p0.second)
		   -(int64)(p2.first-p0.first)*(int64)(p1.second-p0.second);
	if (t==0) return 0;
	return (t<0)?-1:1;
}
...
int main()
{
	...
	for (int i=0;i<n;i++)
		if (a[i].second>0)
		{
			for (;m>=2 && opcross(a[i],p[m-1],p[m-2])<=0;m--);
			p[m++]=a[i];
		}
		else if (m>0)
		{
			...
			for (;T-H>5;)
			{
				int M1=(H*2+T)/3;
				int M2=(H+T*2)/3;
				double d1=(double)(a[i].first-p[M1].first)/(double)(p[M1].second-a[i].second);
				double d2=(double)(a[i].first-p[M2].first)/(double)(p[M2].second-a[i].second);
				if (d1<d2)
					T=M2;
				else
					H=M1;
			...
			for (int M=H;M<=T;M++)
			{
				double d=(double)(a[i].first-p[M].first)/(double)(p[M].second-a[i].second);
				if (d<result)
					result=d;
			}
		}
