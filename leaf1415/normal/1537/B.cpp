...
ll calc(ll x1, ll y1, ll x2, ll y2)
{
	...
	ll ret = inf;
	do{
		ll x = sx, y = sy, nx, ny, sum = 0;
		rep(i, 0, 1){
			if(p[i] == 0) nx = x1, ny = y1;
			if(p[i] == 1) nx = x2, ny = y2;
...
int main(void)
{
	...
	while(T--){
		cin >> h >> w >> sy >> sx;
		ll X[4] = {1, 1, w, w}, Y[4] = {1, h, h, 1};
		
		ll ans = -1, x1, y1, x2, y2;
		rep(i, 0, 3) rep(j, 0, 3){
			ll res = calc(X[i], Y[i], X[j], Y[j]);
			if(res > ans) ans = res, x1 = X[i], y1 = Y[i], x2 = X[j], y2 = Y[j];
		}
		outl(y1, x1, y2, x2);
	}
