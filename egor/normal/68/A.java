...
public class TaskA {
	...
	private void solve() {
		int[] p = in.readIntArray(4);
		int a = in.readInt();
		int b = in.readInt();
		int answer = b - a + 1;
		for (int pp : p)
			answer = Math.max(Math.min(answer, pp - a), 0);
		out.println(answer);
	}

	private static List<Test> createTests() {
		List<Test> tests = new ArrayList<Test>();
		tests.add(new Test("2 7 1 8 2 8", "0"));
		tests.add(new Test("20 30 40 50 0 100", "20"));
		tests.add(new Test("31 41 59 26 17 43", "9"));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
//		tests.add(new Test("", ""));
