...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            // cout<<i<<'\n';
            // for(auto it:upp) cout<<it<<' ';
            // cout<<'\n';
            // for(auto it:dow) cout<<it<<' ';
            // cout<<'\n';
            dp[i]=dp[i+1]+1;
            if(a[i]<a[i+1])
            {
                while(!upp.empty()&&a[upp.back()]>a[i])
                {
                    dp[i]=min(dp[i],dp[upp.back()]+1);
                    upp.DB();
                }
                if(!upp.empty())
                {
                ...
                upp.PB(i);
                while(!dow.empty()&&a[dow.back()]<=a[i])
                {
                    dow.DB();
                }
                dow.PB(i);
