...
int main() {
	...
	for (int parts = 1; parts <= n; parts++) {
		...
		int midPos = (len % 2) * parts;
		if ((int)odds.size() <= midPos) {
			for (int i = 0; i < 128; i++) {
				while ((int)odds.size() < midPos && cnt[i] >= 2) {
					odds.emplace_back(i);
					odds.emplace_back(i);
					cnt[i] -= 2;
				}
			...
			vector<string> ans(parts, string(len, '_'));
			int ptr = 0;
			for (int i = 0; i < parts; i++) {
				for (int j = 0; j < len / 2; j++) {
					while (cnt[ptr] < 2) ++ptr;
					ans[i][j] = ans[i][len - 1 - j] = ptr;
					cnt[ptr] -= 2;
				}

				if (len % 2 == 1) {
					ans[i][len / 2] = odds.back();
					odds.pop_back();
				}
			}
