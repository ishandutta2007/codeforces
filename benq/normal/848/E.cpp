...
void solve(int l, int r) {
	...
	solve(l,m);
	vmi F[3];
	F0R(i,3) FOR(j,l,m+1) F[i].pb(f[i][j]);
	
	///////// f[0]
	// g[j]*j*j is coefficient for j+1, combine with f[0]
	{
		vmi v(r-l+1);
		F0R(j,r-l+1-1) v[j+1] = g[j]*j*j;
		v = conv(v,F[0]);
		FOR(j,m+1,r+1) f[0][j] += v[j-l];
	}
    // g[j]*(j+1)*(j+1) is coefficient for j+3, combine with f[1]
	{
		vmi v(r-l+1);
		F0R(j,r-l+1-3) v[j+3] = g[j]*(j+1)*(j+1);
		v = conv(v,F[1]);
		FOR(j,m+1,r+1) f[0][j] += v[j-l];
	}
	...
	{
		vmi v(r-l+1);
		F0R(j,r-l+1-1) v[j+1] = g[j]*(j+1)*(j+1);
		v = conv(v,F[0]);
		FOR(j,m+1,r+1) f[1][j] += v[j-l];
	}
	{
		vmi v(r-l+1);
		F0R(j,r-l+1-3) v[j+3] = g[j]*(j+2)*(j+2);
		v = conv(v,F[1]);
		FOR(j,m+1,r+1) f[1][j] += v[j-l];
	}
	///////// f[2]
...
int main() {
    ...
    FOR(i,1,n+1) {
    	mi tot = 0;
    	if (1 <= i && i <= n-1) tot += g[i-1]*f[0][n-1-i];
    	if (2 <= i && i <= n-2) tot += 2*g[i-2]*f[1][n-2-i];
    	if (3 <= i && i <= n-3) tot += g[i-3]*f[2][n-3-i];
    	if (i == n) tot += (g[i-1]+g[i-3]);
    	ans += tot*i*(i-1)*(i-1);
    }
    ps(ans);
}
