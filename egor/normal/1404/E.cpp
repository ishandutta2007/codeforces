...
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        auto c = in.readCharTable(n, m);

        arr2d<int> topId(n, m, -1);
        arr2d<int> leftId(n, m, -1);
        arr2d<int> cellId(n, m, -1);
        int cId = 0;
        int tId = 0;
        int lId = 0;
        for (int i : range(n)) {
        ...
        int source = cId + tId + lId;
        int sink = source + 1;
        Graph<FlowEdge<int>> graph(sink + 1);
        int infty = numeric_limits<int>::max();
        for (int i : range(n)) {
            for (int j : range(m)) {
                if (topId(i, j) != -1) {
                    graph.addEdge(source, topId(i, j), 1);
                    graph.addEdge(topId(i, j), tId + cellId(i, j), infty);
                    graph.addEdge(topId(i, j), tId + cellId(i + 1, j), infty);
                }
                if (leftId(i, j) != -1) {
                    graph.addEdge(tId + cId + leftId(i, j), sink, 1);
                    graph.addEdge(tId + cellId(i, j), tId + cId + leftId(i, j), infty);
                    graph.addEdge(tId + cellId(i, j + 1), tId + cId + leftId(i, j), infty);
                }
            }
        }
        out.printLine(cId - (tId + lId - dinic<FlowEdge<int>, int>(graph, source, sink)));
    }
};
