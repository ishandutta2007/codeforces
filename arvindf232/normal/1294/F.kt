...
//            invE[a].add(b)
    ...
    fun furthestFrom(v:Int):Int {
        val all = IntArray(n)
        all[v] = 0
        DFS_long(v,{}){
            parent,here ->
            all[here] = all[parent] + 1
        }
        return (0 until n).maxByOrNull { all[it] }!!
    }
    ...
    fun distanceFromColoured(colours:BooleanArray):IntArray{
...
fun <T> List<T>.rotateFirst(index: Int) = drop(index) + take(index)
fun <T> List<T>.rotateLast(index: Int) = rotateFirst((index + 1)% size)
inline fun <reified T,reified S:Comparable<S>> List<T>.minIndexBy(f:(T)->S):Int{
    if(isEmpty()){return -1}
    var minIndex = 0;var min = f(this[0])
    for(i in 1..lastIndex){val next = f(this[i]);if(next < min){min = next;minIndex = i}}
    return minIndex
}
inline fun <reified T,reified S:Comparable<S>> List<T>.maxIndexBy(f:(T)->S):Int{
    if(isEmpty()){return -1}
    var maxIndex = 0;var max = f(this[0])
    for(i in 1..lastIndex){val next = f(this[i]);if(next > max){max = next;maxIndex = i}}
    return maxIndex
}
...
fun main(){
    solve.cases{
        ...
        var root = 0
        val a = G.furthestFrom(root)
        val b = G.furthestFrom(a)
        val coloured = BooleanArray(n)
        val path = G.pathbetween(a,b)
        for(c in path){
            coloured[c] = true
        }
        val dist = G.distanceFromColoured(coloured)
        var c = dist.indexOf(dist.maxOrNull()!!)
        for(c in G.pathbetween(b,c)){
            coloured[c] = true
        }
        if(c == a || c == b){
            c = (0 until n).first{it != a && it != b}
        }
        put(coloured.count{it} - 1 )
        put("${a+1} ${b+1} ${c+1}")
    }
    done()
}
