...
int bisearch(vector<int> &a, int l, int r, int e)
{
    if (r - l <= 1)
    {
        if (a[r] > e) return l; else return r;
    }
    int m = (l + r) / 2;
    if (e < a[m]) return bisearch(a, l, m, e);
    else return bisearch(a, m, r, e);
}
...
int main()
{
    ...
    for (int i = 0; i < m; i++)
        if (a[0] > b[i]) cout << "0 ";
        else cout << bisearch(a, 0, n - 1, b[i]) + 1 << " ";
    return 0;
}