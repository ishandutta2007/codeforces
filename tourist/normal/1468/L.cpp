...
int main() {
  ...
  vector<vector<long long>> ps(n);
  map<long long, vector<int>> single;
  for (int i = 0; i < n; i++) {
    auto f = Factor::factorize(x[i]);
    set<long long> z;
    for (auto& p : f) {
      z.insert(p);
    }
    ps[i] = vector<long long>(z.begin(), z.end());
    if (ps[i].size() == 1) {
      single[ps[i][0]].push_back(i);
    }
  }
  vector<pair<int, long long>> singles;
  set<long long> good;
  int total = 0;
  for (auto& s : single) {
    int sz = (int) s.second.size();
    if (sz >= 2) {
      singles.emplace_back(sz, s.first);
      good.emplace(s.first);
      total += sz;
    }
  }
  if (singles.empty()) {
  ...
  for (int i = (int) singles.size() - 1; i >= 0; i--) {
    int take = min(k - (int) res.size(), singles[i].first);
    if (take == 1) {
      if (singles.back().first == 2) {
        ok = false;
        break;
      }
      res.erase(res.begin());
    ...
    long long p = singles[i].second;
    for (int j = 0; j < take; j++) {
      res.push_back(single[p][j]);
    }
  }
  if (ok) {
    while ((int) res.size() < k && !extra.empty()) {
      res.push_back(extra.back());
      extra.pop_back();
    }
    if ((int) res.size() == k) {
      for (int i = 0; i < k; i++) {
  ...
  if (k % 2 == 1 && singles.back().first == 2 && !ok) {
    res.clear();
    set<long long> done;
    for (int i : extra) {
      if (ps[i].size() > 1 && 2 * (int) ps[i].size() + 1 <= k) {
        res.push_back(i);
        for (auto& p : ps[i]) {
          for (int j : single[p]) {
            res.push_back(j);
