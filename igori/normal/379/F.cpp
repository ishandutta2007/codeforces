...
int dist(int v, int u, vvi &go, vi &d)
{
    int L = 0;
    for (int j = 19; j >= 0; j--)
    {
        int v0 = go[j][v];
        if (in[v0] <= in[u] && out[u] <= out[v0])
            continue;
        v = v0;
        L += (1 << j);
    }
...
signed main()
{
    ...
    for (int i = 0; i < q; i++)
    {
        ...
        p[2 * i + 4] = x;
        p[2 * i + 5] = x;
        graph[x].push_back(2 * i + 4);
        graph[x].push_back(2 * i + 5);
    }
    dfs(0, graph);
    vector<vector<int> > go(20, vector<int>(n));
    int v = 0, u = 0, di = 0;
    for (int i = 1; i < n; i++)
    {
        d[i] = d[p[i]] + 1;
        ...
        int d0 = dist(i, v, go, d);
        int d1 = dist(i, u, go, d);
        if (d0 > d1)
        {
            if (d0 > di)
            {
