...
void degnome(int& a, const int& p, int& k)
{
	while (a % p == 0)
	{
		a /= p;
...
struct tree
{
	...
	vpii fact;
	int pn;
	void mk(const vi& a, const int& mood)
	{
		mod = mood;
		n = sz(a);
		fact = fct(mod);
		pn = sz(fact);
		pow_push.resize(4 * n, vi(pn));
		pure_push.resize(4 * n, 1);
		real_push.resize(4 * n, 1);
		t.resize(n * 4);
		pure_num.resize(n, 1);
		powers.resize(n, vi(pn));
		init(0, 0, n, a);
	}
	void init(const int& v, const int& al, const int& ar, const vi& a)
	{
		if (al + 1 != ar)
		{
			int am = (al + ar) / 2;
		...
		else
		{
			...
			pure_num[al] = a[al];
			for (int i = 0; i < pn; ++i)
				degnome(pure_num[al], fact[i].X, powers[al][i]);
			pure_num[al] %= mod;
		}
	}
	void div(const int& l, const int& x)
	{
		push(0, 0, n);
		tree_div(0, 0, n, l, x);
	}
	void tree_div(const int& v, const int& al, const int& ar, const int& ol, const int& x)
	{
		if ((ol < al) || (ol >= ar))
			return;
		if (al + 1 != ar)
		{
			int am = (al + ar) / 2;
			push(2 * v + 1, al, am);
			tree_div(2 * v + 1, al, am, ol, x);
			push(2 * v + 2, am, ar);
			tree_div(2 * v + 2, am, ar, ol, x);
			t[v] = sum(t[2 * v + 1], t[2 * v + 2], mod);
		}
		else
		{
			...
			int xx = x;
			for (int i = 0; i < pn; ++i)
				degnome(xx, fact[i].X, x_powers[i]);
			xx %= mod;
			pure_num[ol] = prod(pure_num[ol], inverse(xx, mod), mod);
			for (int i = 0; i < pn; ++i)
				powers[ol][i] -= x_powers[i];
			t[v] = pure_num[ol];
			for (int i = 0; i < pn; ++i)
				if (t[v] != 0)
					t[v] = prod(t[v], pow_mod(fact[i].X, powers[ol][i], mod), mod);
		}
	}
	void mul(const int& l, const int& r, const int& x)
	{
		push(0, 0, n);
		int xx = x;
		vi xp(pn);
		for (int i = 0; i < pn; ++i)
			degnome(xx, fact[i].X, xp[i]);
		xx %= mod;
		tree_mul(0, 0, n, l, r, xp, xx, x % mod);
	}
	void push(const int& v, const int& al, const int& ar)
	{
		if (al + 1 != ar)
		{
			if (real_push[v] != 1)
			{
				t[v] = prod(t[v], real_push[v], mod);
				real_push[2 * v + 1] = prod(real_push[2 * v + 1], real_push[v], mod);
				real_push[2 * v + 2] = prod(real_push[2 * v + 2], real_push[v], mod);
				real_push[v] = 1;
			}
			if (pure_push[v] != 1)
			{
				pure_push[2 * v + 1] = prod(pure_push[2 * v + 1], pure_push[v], mod);
				pure_push[2 * v + 2] = prod(pure_push[2 * v + 2], pure_push[v], mod);
				pure_push[v] = 1;
			}
			for (int i = 0; i < pn; ++i)
			{
				pow_push[2 * v + 1][i] += pow_push[v][i];
				pow_push[2 * v + 2][i] += pow_push[v][i];
				pow_push[v][i] = 0;
			}
		}
		else
		{
			...
			if (pure_push[v] != 1)
			{
				pure_num[al] = prod(pure_num[al], pure_push[v], mod);
				pure_push[v] = 1;
			}
			for (int i = 0; i < pn; ++i)
			{
				powers[al][i] += pow_push[v][i];
				pow_push[v][i] = 0;
			}
		}
	}
	void tree_mul(const int& v, const int& al, const int& ar, const int& el, const int& er, const vi& xp, const int& px, const int& x)
	{
		if ((al >= er) || (el >= ar))
			return;
		if ((al >= el) && (ar <= er))
		{
			real_push[v] = prod(real_push[v], x, mod);
			pure_push[v] = prod(pure_push[v], px, mod);
			for (int i = 0; i < pn; ++i)
				pow_push[v][i] += xp[i];
			push(v, al, ar);
		}
		else
		{
			int am = (al + ar) / 2;
			push(2 * v + 1, al, am);
			tree_mul(2 * v + 1, al, am, el, er, xp, px, x);
			push(2 * v + 2, am, ar);
			tree_mul(2 * v + 2, am, ar, el, er, xp, px, x);
			t[v] = sum(t[2 * v + 1], t[2 * v + 2], mod);
		}
	}
	int summ(const int& l, const int& r)
	{
		push(0, 0, n);
		return tree_sum(0, 0, n, l, r);
	}
	int tree_sum(const int& v, const int& al, const int& ar, const int& el, const int& er)
	{
		if ((al >= er) || (el >= ar))
			return 0;
		if ((al >= el) && (ar <= er))
		...
		int am = (al + ar) / 2;
		push(v * 2 + 1, al, am);
		int ans1 = tree_sum(2 * v + 1, al, am, el, er);
		push(v * 2 + 2, am, ar);
		int ans2 = tree_sum(2 * v + 2, am, ar, el, er);
		return sum(ans1, ans2, mod);
	}
