...
void solve() {
  ...
  FOR(v, N) G.add(v, 4 * N + v, 0);
  FOR(v, N) G.add(2 * N + v, v, 0);
  FOR(v, 2 * N) G.add(N + v / 2, N + v, 0);
  FOR(v, 2 * N) G.add(3 * N + v, 3 * N + v / 2, 0);
  ...
  FOR(q, Q) {
    ...
    if (t == 1) {
      ...
      l += N, r += N;
      auto add = [&](ll i) -> void {
        if (t == 2) G.add(v, N + i, cost);
        if (t == 3) G.add(3 * N + i, v, cost);
      };
      while (l < r) {
        if (l & 1) add(l++);
        if (r & 1) add(--r);
  ...
  vi ANS = {dist.begin(), dist.begin() + N};
  print(ANS);
}
