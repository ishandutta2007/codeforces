...
void solve(){
	...
	for(int i = 1; i <= n; ++i){
		...
		for(int j = 0; j < V.size(); j += 2){
			int k = ext + V[j].first + V[j + 1].first;
			ans += k / i;

			ext = min(V[j + 1].first, k % i);
		}

		for(int j = 0; j < V.size(); j += 2){
			if (j > 0 && V[j].first + V[j + 1].first + T.back().first <= i){
				int k = T.size();
				T[k - 2].first += T[k - 1].first;
				T[k - 1].first = 0;
				if (j + 2 < V.size()){
					V[j + 2].first += V[j + 1].first;
				}
			}
			else{
				T.push_back({V[j]});
