...
public:
    void solve() {
        ...
        sort(all(big), greater<>());
        arr<ll> sumSmall(less + 1);
        sumSmall[0] = 0;
        for (int i : range(less)) {
            sumSmall[i + 1] = sumSmall[i] + small[i];
        }
        ll answer = (big.empty() ? 0 : big[0]) + sumSmall[less];
        ll current = (big.empty() ? 0 : big[0]);
        int rem = n - 1;
        for (int i : range(1, big.size())) {
            if (rem <= d) {
                break;
            }
            rem -= d + 1;
            current += big[i];
            maxim(answer, current + sumSmall[min(rem, less)]);
        }
...
int main() {
    ...
    auto time = clock();
#endif
    ABoboniuObshchaetsyaSDu solver;
