...
int cnt[N], num[N];
int a[500][500], b[555][555];

int main() {
    INT(n, k);
    rep(i, n) rep(j, n) cin >> a[i][j];
    // rep(i, n) rep(j, n) a[i][j] = i * n + j;

    rep(i, n) rep(j, n) num[a[i][j]]++;
    int now = 0;
    rep(i, n * n + 1) now += (!!num[i]);
    ...
    int N = n * n + 1;
    vi l(N, n), r(N, 0), u(N, n), d(N, 0);

    rep(i, n) rep(j, n) {
        int x = a[i][j];
        chmin(l[x], j), chmax(r[x], j);
        chmin(u[x], i), chmax(d[x], i);
    }

    rep(k, 1, n + 1) {
        rep(i, n - k + 1) rep(j, n - k + 1) b[i][j] = 0;
        rep(i, 1, N) {
            if(!num[i]) continue;
            int L = r[i] - k + 1, R = l[i];
            int U = d[i] - k + 1, D = u[i];
            chmax(L, 0), chmax(U, 0);
            if(L > R or U > D) continue;
            b[U][L]++, b[U][R + 1]--, b[D + 1][L]--, b[D + 1][R + 1]++;
        }
        rep(i, n) rep(j, n) b[i + 1][j] += b[i][j];
        rep(i, n) rep(j, n) b[i][j + 1] += b[i][j];
        rep(i, n - k + 1) rep(j, n - k + 1) {
            if(b[i][j] == need or b[i][j] - 1 == need) {
                dump(k, i, j);
                drop(1);
            }
        }
