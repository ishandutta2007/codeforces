...
int main(){
  ...
  modint::set_mod(m);
  Comb comb(15000);
  auto dp=vec(k+1,k+1,modint(0));
  dp[0][0]=1;
  rep(i,0,k){
    rep(j,0,k-1){
      dp[i+1][j+2]+=dp[i][j];
    }
    rep(j,0,k)dp[i+1][j+1]+=dp[i+1][j];
  }
  auto powers=vec(k+1,k+1,modint(1));
  rep(i,0,k+1){
    rep(j,0,k)powers[i][j+1]=powers[i][j]*i;
  }
  modint ret=0;
  //debug(dp);
  rep(i,2,k+1){
    rep(j,1,i+1){
      modint groups=comb.P(k,i)*dp[j][i]*comb.rfact(j);
      //cout<<i spa j spa groups spa powers[k-1][j]*powers[i-j][k-i]<<endl;
      ret+=groups*powers[k-1][j]*powers[i-j][k-i];
    }
  }
  cout<<ret<<endl;
  return 0;
