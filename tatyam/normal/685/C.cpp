...
void solve(){
    LL(n);
    vec(i128,mn,4,LLONG_MAX);
    vec(i128,mx,4,LLONG_MIN);
    rep(n){
        LL(x,y,z);
        chmin(mn[0],x+y+z);
        chmax(mx[0],x+y+z);
    ...
    i128 ok = LLONG_MAX, ng = -1;
    auto check = [&](i128 d) -> bool {
        return [mn,mx,d]() mutable -> bool {
            rep(4){
                tie(mn[i],mx[i])=pair{mx[i]-d,mn[i]+d};
                if(mn[i]%2)mn[i]++;
                if(mx[i]%2)mx[i]--;
                if(mn[i]>mx[i])return 0;
            }
            vector<i128>ans={mn[1]+mn[2]+mn[3],mn[1],mn[2],mn[3]};
            rep(i,1,4)if(mn[0]>ans[0]){
                const i128 x=min(mn[0]-ans[0],mx[i]-ans[i]);
                ans[0]+=x;
                ans[i]+=x;
            }
            return mn[0]<=ans[0]&&ans[0]<=mx[0];
        }() || [mn,mx,d]() mutable -> bool {
            rep(4){
                tie(mn[i],mx[i])=pair{mx[i]-d,mn[i]+d};
                if(mn[i]%2==0)mn[i]++;
                if(mx[i]%2==0)mx[i]--;
    ...
    auto output=[&](vector<i128> a){
        rep(i,1,4)a[i]=(a[0]-a[i])/2;
        out(ll(a[1]),ll(a[2]),ll(a[3]));
    };
    const ll d=ok;
    [mn,mx,d,&output]() mutable -> bool {
        rep(4){
            tie(mn[i],mx[i])=pair{mx[i]-d,mn[i]+d};
            if(mn[i]%2)mn[i]++;
            if(mx[i]%2)mx[i]--;
        ...
        if(mn[0]<=ans[0]&&ans[0]<=mx[0])output(ans);
        return mn[0]<=ans[0]&&ans[0]<=mx[0];
    }() || [mn,mx,d,&output]() mutable -> bool {
        rep(4){
            tie(mn[i],mx[i])=pair{mx[i]-d,mn[i]+d};
