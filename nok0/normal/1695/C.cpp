...
void main_() {
	INT(n, m);
	VVEC(int, a, n, m);
	vector mn(n, vector(m, inf));
	vector mx(n, vector(m, -inf));
	mn[0][0] = mx[0][0] = a[0][0];
	rep(i, n) {
		rep(j, m) {
			if(i + 1 < n) {
				chmax(mx[i + 1][j], mx[i][j] + a[i + 1][j]);
				chmin(mn[i + 1][j], mn[i][j] + a[i + 1][j]);
			}
			if(j + 1 < m) {
				chmax(mx[i][j + 1], mx[i][j] + a[i][j + 1]);
				chmin(mn[i][j + 1], mn[i][j] + a[i][j + 1]);
			}
		}
	}
	YES(mx.back().back() >= 0 and mn.back().back() <= 0 and mx.back().back() % 2 == 0);
}
