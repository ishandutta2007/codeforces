...
bool dfs(ll px,ll py,vvb & used,vvl & ans,ll prex,ll prey){
    rep(z,0,3){
        ll nx=px+dx[z];
        ll ny=py+dy[z];
        ...
        used[nx][ny]=true;
        if(!dfs(nx,ny,used,ans,px,py))return false;
    }
    return true;
}

bool check(vvl &ans,bool henkou=true){
    ll n=ans.size();
    ll m=ans[0].size();
    N=n;M=m;
    assert(ans[0][0]==1);
    ...
    used[0][0]=true;
    if(!dfs(0,0,used,ans,-10,-10))return false;
    rep(i,0,n-1)rep(j,0,m-1){
        if(ans[i][j]==1 &&used[i][j]==0){
            assert(j==2);
            if(henkou)ans[i][1]=1;
        }
    }
...
vvl solve(vector<string>s){
    ll n=s.size();ll m=s[0].size();
    vvl ans(n,vl(m,0));
    rep(i,0,n-1){
        rep(j,0,m-1){
            if(s[i][j]=='X')ans[i][j]=1;
        }
    }
    ll p=1;
    if(n%3==1)p=0;
    for(ll i=p;i<=n-1;i+=3){
        if(i-1>=0&& s[i-1][1]=='X'){
            if(m>=3){
                ans[i-1][2]=1;
                if(!(m>=4 && s[i-1][3]=='X'))ans[i][2]=1;
                rep(j,3,m-1)ans[i][j]=1;
            }
        }else if(i+1<=n-1 && s[i+1][1]=='X'){
            if(m>=3){
                ans[i+1][2]=1;
                if(!(m>=4 && s[i+1][3]=='X'))ans[i][2]=1;
                rep(j,3,m-1)ans[i][j]=1;
            ...
            rep(j,0,m-1)ans[i][j]=1;
        }
    }
    rep(i,0,n-1)ans[i][0]=1;
    if(!check(ans)){
...
int main(){fastio
    /*rep(i,1,10){
        ...
        rep(i,0,n-1)rep(j,0,m-1)if(s[i][j]=='X')ans[i][j]=1;
        cout<<check(ans)<<endl;
    }*/
    /*
    rep(i,1,500){
        ...
        rep(i,0,n-1){
            rep(j,0,m-1){
                bool f=true;
                if(j-1>=0&&s[i][j-1]=='X')f=false;
                rep(k,j-1,j+1){
                    if(0<=k&&k<=m-1&&i-1>=0&&s[i-1][k]=='X')f=false;
                }
                if(f && rand()%3)s[i]+='X';
                else s[i]+='.';
            }
