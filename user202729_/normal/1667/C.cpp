// somehow I can't figure out B fully
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
int main(){
	...
	switch((n-1)%3){
		case 0: case 1:
			std::cout<<a*2+1<<'\n';
			for(int const i: range(0, a+1)){
				std::cout<<a-i+1<<' '<<i+1<<'\n';
			}
			for(int const i: range(0, a)){
				std::cout<<n-i<<' '<<n-(a-1-i)<<'\n';
			}
			break;
		case 2:
			std::cout<<a*2+2<<'\n';
...
// time-limit: 1000
// problem-url: https://codeforces.com/contest/1667/problem/C