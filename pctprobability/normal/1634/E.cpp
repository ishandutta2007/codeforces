...
ll seen[200000];
ll use2[200000];
void dfs(int u, vector<int> &trail,vector<int> &ind) {
  seen[u]=1;
        while (!g[u].empty()) {
                vector<ll> v = g[u].back();
                g[u].pop_back();
          if(use2[v[1]]) continue;
          use[v[1]]=v[2];
          use2[v[1]]=1;
          ind.pb(v[1]);
                dfs(v[0], trail,ind);
...
void solve(){
  ...
  for(int i=0;i<n;i++){
    ...
    for(auto e:b[i]) m[e]++;
    for(auto e:m){
      if(e.se%2) c[e.fi].pb(i);
    }
  }
  ll now=0;
  vector<vector<ll>> h;
  for(auto e:c){
    for(int i=0;i+1<e.se.size();i+=2){
      ll p=i,q=(i+1)%(e.se.size());
      p=e.se[p];
      q=e.se[q];
      g[p].pb({q,now,0});
      g[q].pb({p,now,1});
