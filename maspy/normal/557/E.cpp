...
#line 1 "/home/maspy/library/string/trie.hpp"
template <int sigma>
struct Trie {
  int n_node;
...
void solve() {
  ...
  ll N = len(S);
  Trie<2> trie;
  FOR(L, N) { trie.add(S.substr(L)); }
  auto& TO = trie.TO;
  ll V = len(trie.TO);

  // half palind or not?
  vv(int, DP, N, N + 1);
  FOR_R(L, N) FOR3(R, L, N + 1) {
    if (L == R) DP[L][R] = 1;
    elif (L + 1 == R) DP[L][R] = 1;
    elif (L + 2 == R) DP[L][R] = S[L] == S[R - 1];
    elif (L + 3 == R) DP[L][R] = S[L] == S[R - 1];
    else {
      DP[L][R] = DP[L + 2][R - 2] && S[L] == S[R - 1];
    }
  }
