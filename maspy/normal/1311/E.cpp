...
signed main() {
  ...
  FOR3(n, 2, MAX) {
    HIGH[n] = HIGH[n - 1] + n - 1;
    LOW[n] = (n - 1) + LOW[(n - 1) / 2] + LOW[n - 1 - (n - 1) / 2];
  }

  auto f = [&](auto self, ll n, ll d) -> pair<bool, vi> {
    vi par;
    if (n == 0) {
      assert(d == 0);
      return mp(true, par);
    }
    if (LOW[n] > d || HIGH[n] < d) return mp(false, par);
    FOR(L, n) {
      ll R = n - 1 - L;
      if (LOW[L] + LOW[R] <= d - (n - 1) && d - (n - 1) <= HIGH[L] + HIGH[R]) {
        ll L_sz = max(LOW[L], d - (n - 1) - HIGH[R]);
        ll R_sz = d - (n - 1) - L_sz;
        auto [bl_L, par_L] = self(self, L, L_sz);
        auto [bl_R, par_R] = self(self, R, R_sz);
        par.reserve(L + R);
        ll vl = 1;
        ll vr = L + 1;
        if (L > 0) par.eb(0);
        FORIN(x, par_L) par.eb(x + vl);
        if (R > 0) par.eb(0);
        FORIN(x, par_R) par.eb(x + vr);
        return mp(true, par);
      }
    }
    return mp(false, par);
