//segtree beats : https://judge.yosupo.jp/submission/48417

/**
 *  date : 2021-01-30 23:41:09
 */
...
#undef unlikely
  ...
  void push(int k) {
    _mm_prefetch(v.data() + k * 2 + 0, _MM_HINT_T0);
    _mm_prefetch(v.data() + k * 2 + 1, _MM_HINT_T0);
  ...
  inline void _apply(int k, i64 x) {
    _mm_prefetch(v.data() + k, _MM_HINT_T0);
  ...
  template <int cmd>
  inline void op(i64& a, i64 b) {
    if constexpr (cmd == 1) a = min(a, b);
    if constexpr (cmd == 2) a = max(a, b);
    if constexpr (cmd == 3) a += b;
  }
  ...
  i64 inner_fold(int l, int r) {
    ...
    while (l < r) {
      if (l & 1) op<cmd>(lx, v[l++].sum);
      if (r & 1) op<cmd>(rx, v[--r].sum);
      l >>= 1;
      r >>= 1;
    }
    op<cmd>(lx, rx);
...
void Nyaan::solve() {
  ...
  vector<i64> init(N,0),a(N,0),lessL(N,0),lessR(N,0),moreR(N,0),moreL(N,0);
 ...
 for(int i=0;i<N;i++)
 {
     lessL[i] = max(lessL[i],seg.range_sum(a[i],a[i]+1));
     seg.range_chmax(0,n,0);
     seg.range_add(0,a[i]+1,1);
     seg.range_add(a[i]+1,n,-1);
 }
 
 //moreL
 AngelBeats seg2(init);
    
 for(int i=0;i<N;i++)
 {
     moreL[i] = max(moreL[i],seg2.range_sum(a[i],a[i]+1));
     seg2.range_chmax(0,n,0);
     seg2.range_add(a[i],n,1);
     seg2.range_add(0,a[i],-1);
 }
    
 //lessR   
 AngelBeats seg3(init);
    
 for(int i=N-1;i>=0;i--)
 {
     lessR[i] = max(lessR[i],seg3.range_sum(a[i],a[i]+1));
     seg3.range_chmax(0,n,0);
     seg3.range_add(0,a[i]+1,1);
     seg3.range_add(a[i]+1,n,-1);
 }
    
 //moreR   
 AngelBeats seg4(init);
    
 for(int i=N-1;i>=0;i--)
 {
     moreR[i] = max(moreR[i],seg4.range_sum(a[i],a[i]+1));
     seg4.range_chmax(0,n,0);
     seg4.range_add(a[i],n,1);
     seg4.range_add(0,a[i],-1);
 } 
    
  for(int i=0;i<N;i++)
  {
      i64 res = 0;
      res = max(res ,(lessR[i] + lessL[i] + 1)/2);
      res = max(res ,(moreR[i] + moreL[i])/2);
      cout << res << ' '; 
  }      
}