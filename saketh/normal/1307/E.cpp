...
int main() {
    ...
    F0R (sw, N) sort(all(cows[sw]));

    auto have_cow = [&](int sw, int eat) {
        return find(cows[sw], eat) != cows[sw].end();
    };

    int SLEEP = 0;
    mn WAYS = 0;

    auto offer = [&](int sleep, mn ways) {
        if (sleep > SLEEP) {
            SLEEP = sleep;
            WAYS = 0;
        }
        if (sleep == SLEEP) WAYS += ways;
    };

    { // case with no cows on left side
        int sleep = 0;
        mn ways = 1;
        F0R (sw, N) {
            int pos = 0;
            trav (e, cows[sw]) if (e <= with_sweet[sw]) pos++;
            if (pos) { // send one cow from the right if we can
                sleep++;
                ways *= pos;
            }
        }
    ...
    vi seen_with_sweet(N);
    F0R (first_left, N) { // split on where the first cow from the left will stop
        int STOP = grass[first_left];
        seen_with_sweet[STOP]++;
        if (!have_cow(STOP, seen_with_sweet[STOP])) continue; // need the cow

        int sleep = 1;
        mn ways = 1;

        F0R (sw, N) {
            const vi& cow = cows[sw];

            const int ON_LEFT = seen_with_sweet[sw];
            const int ON_RIGHT = with_sweet[sw] - ON_LEFT;

            if (sw == STOP) { // see if we can send one more from the right
                int pos = 0;
                trav (e, cow) if (e != seen_with_sweet[STOP] && e <= ON_RIGHT) {
                    pos++;
                }
                if (pos > 0) {
                    sleep++;
            ...
            // ways to send 2
            if (sz(cow) >= 2 && cow[0] <= min(ON_LEFT, ON_RIGHT) && cow[1] <= max(ON_LEFT, ON_RIGHT)) {
                mn pairs = 0;
                int can_right = 0;
                trav (e, cow) if (e <= ON_RIGHT) can_right++;
                trav (e, cow) {
                    if (e <= ON_LEFT) {
                        pairs += can_right - (e <= ON_RIGHT);
                    }
                }
                sleep += 2;
                ways *= pairs;
