...
#define Accepted 0
using namespace std;
const int mod1=1e9+7,mod2=1e9+9,p=17;
...
int main()
{
	...
	for (int i=1;i<=n;i++)
	{
		...
		else
		{
			if (i&1)
			{
				...
				has12[i]=(1ll*has12[i-1]*p+1)%mod2;
			}
			else
			{
				has11[i]=(1ll*has11[i-1]*p+2)%mod1;
				has12[i]=(1ll*has12[i-1]*p+2)%mod2;
			}
		}
	}
	for (int i=1;i<=n;i++)
	{
		...
		else
		{
			if (i%2==0)
			{
				...
				has22[i]=(1ll*has22[i-1]*p+1)%mod2;
			}
			else
			{
				has21[i]=(1ll*has21[i-1]*p+2)%mod1;
				has22[i]=(1ll*has22[i-1]*p+2)%mod2;
			}
		}
	}
	for (int i=1;i<=n;i++)
	...
	while(q--)
	{
		...
		if (x&1)
		{
			...
			h2=(1ll*has12[x+z-1]-1ll*pw2[nx]*has12[x-1])%mod2;
			h1=(h1+mod1)%mod1;h2=(h2+mod2)%mod2;
		}
		else
		{
			h1=(1ll*has21[x+z-1]-1ll*pw1[nx]*has21[x-1])%mod1;
			h2=(1ll*has22[x+z-1]-1ll*pw2[nx]*has22[x-1])%mod2;
			h1=(h1+mod1)%mod1;h2=(h2+mod2)%mod2;
		}
		if (y&1)
		{
			h3=(1ll*has11[y+z-1]-1ll*pw1[ny]*has11[y-1])%mod1;
			h4=(1ll*has12[y+z-1]-1ll*pw2[ny]*has12[y-1])%mod2;
			h3=(h3+mod1)%mod1;h4=(h4+mod2)%mod2;
		}
		else
		{
			h3=(1ll*has21[y+z-1]-1ll*has21[y-1]*pw1[ny])%mod1;
			h4=(1ll*has22[y+z-1]-1ll*pw2[ny]*has22[y-1])%mod2;
			h3=(h3+mod1)%mod1;h4=(h4+mod2)%mod2;
		}
		if (h1==h3 && h2==h4) printf("Yes\n");
		else printf("No\n");
	}
	return Accepted;
}