...
int main(){
    ...
    rep(ii,T){
        ll N,M; cin >> N >> M;
        ll R = M-1;
        auto dp = v4ll(62,63,2,2,0); dp[61][0][0][0] = 1;
        for(int i=60;i>=0;i--){
            rep(j,61){
                rep(k,2){
                    rep(l,2){
                        rep(m,2){
                            if(k==0&&m==1&&(R&(1LL<<i))==0) continue;
                            int fl = k;
                            if(m==0&&(R&(1LL<<i))!=0) fl = 1;
                            if(m==1&&(N&(1LL<<i))!=0){
                                dp[i][0][fl][(l+j+1)%2] += dp[i+1][j][k][l];
                            }
                            else if(m==0&&(N&(1LL<<i))==0){
                                dp[i][0][fl][l] += dp[i+1][j][k][l];
                            }
                            else{
                                dp[i][j+1][fl][l] += dp[i+1][j][k][l];
                            }
                        }
                    }
                }
