...
struct cp {
	...
	cp(ld xx = 0, ld yy = 0) : x(xx), y(yy) {}

	cp operator*(const cp& oth) const {
		return cp(x * oth.x - y * oth.y, x * oth.y + y * oth.x);
	}

	cp operator+(const cp& oth) const {
		return cp(x + oth.x, y + oth.y);
	}
...
void prec() {
	...
	for (int lg = 0; lg < L; ++lg) {
		int bl = (1 << lg);
		for (int j = 0; j < bl; ++j) {
			ld cs = cos(PI / bl * j), sn = sin(PI / bl * j);
			*(ptr++) = cp(cs, sn);
			*(iptr++) = cp(cs, -sn);
		}
	}
...
void fft(cp* a, bool inv) {
	...
	for (int lg = 0; lg < L; ++lg) {
		int bl = (1 << lg);
		for (int pos = 0; pos < N; pos += 2 * bl) {
			for (int ptr = 0; ptr < bl; ++ptr) {
				cp tmp = a[pos + ptr] + w[ptr] * a[pos + ptr + bl];
				a[pos + ptr + bl] = a[pos + ptr] - w[ptr] * a[pos + ptr + bl];
				a[pos + ptr] = tmp;
			}
		}
		w += (1 << lg);
...
inline void solve() {
	...
	int n = readInt<int>(), m = readInt<int>();
	for (int i = 0; i < n; ++i) {
		a[v[i] = readInt<int>()] = 1;
	}
