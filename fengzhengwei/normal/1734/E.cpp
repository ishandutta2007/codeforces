...
int main(){
	...
	if(a[i]==0)
	{
		for(int j=i;j<=i+n;j++)p[j>n?j-n:j]=j-i;
		break;
	}
	for(int i=1;i<=n;i++)
	{
		...
		{
			for(int j=i;j<=i+n;j++)
				ans[i][j>n?j-n:j]=p[i]*(j-i)%n;
		}
		else 
		{
			int D=a[i]*ksm(p[i],n-2,n)%n;
			for(int j=i;j<=i+n;j++)
				ans[i][j>n?j-n:j]=p[i]*((j-i+D))%n;
		}
	}
//	for(int j=1;j<=n;j++)
//	{
...
//				for(int w=1;w<i;w++)
//				{
////					if((ans[i][j]-ans[i][k]+n+n)%n==(ans[w][j]-ans[w][k]+n+n)%n)
////					{
////						cerr<<i<<" "<<w<<"\n";
////						cerr<<j<<" "<<k<<"\n";
////					}
//					assert((ans[i][j]-ans[i][k]+n+n)%n!=(ans[w][j]-ans[w][k]+n+n)%n);
//				}
//			}
//		}
//	}
	for(int i=1;i<=n;i++)for(int j=1;j<=n;j++)cout<<(ans[i][j]+mn)%n<<" \n"[j==n];
	pc('1',1);
	return 0;
}