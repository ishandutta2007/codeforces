...
template <class D> struct Mat : vector<vector<D>> {
    using vector<vector<D>>::size;
    int h() const { return int(size()); }
    int w() const { return int((*this)[0].size()); }
    Mat operator*(const Mat& r) const {
        assert(w() == r.h());
        Mat res(h(), vector<D>(r.w()));
        for (int i = 0; i < h(); i++) {
            for (int j = 0; j < r.w(); j++) {
                for (int k = 0; k < w(); k++) {
...
template <class Mint> vector<Mint> solve_linear(Mat<Mint> a, vector<Mint> b) {
    int h = a.h(), w = a.w();
    int r = 0;
    vector<int> idxs;
    for (int x = 0; x < w; x++) {
        ...
        for (int y = r + 1; y < h; y++) {
            ...
            if (a[r][x] == 0)
                assert(0);
                //return {-1};
            auto freq = mul[a[y][x]][a[r][x]];
            for (int k = x; k < w; k++) a[y][k] = dif[a[y][k]][mul[freq][a[r][k]]];
            b[y] = dif[b[y]][mul[freq][b[r]]];
        }
        r++;
        idxs.push_back(x);
        if (r == h) break;
        ...
        for (int j = i + 1; j < n; j++)
            for (int k = j + 1; k < n; k++)
                if (e[i][j] != -1 && e[i][k] != -1 && e[j][k] != -1) {
                    int su = 0;
                    vector<int> has;
                    for (int id : {e[i][j], e[i][k], e[j][k]})
                        if (ind[id] != -1) {
                            has.push_back(ind[id]);
                        } else {
                            su = dif[su][edg[id]];
                        }
                    if (has.empty() && su != 0) {
