...
int choose(int xw, int yw, int xb, int yb)
{
  ...
  if (ww <= bb && (ww < bw))
    return 0; // white, go straight to target
  if (bb < ww && (bb < wb - 1))
    return 1; // black, go straight to target
  if ((xw + yw) % 2 != (xb + yb) % 2)
    return 2; // white, go to black target first
  return 3; // black, go to white target first
}
...
void getNextStep(int &c, int &x, int &y, int xx, int yy)
{
  ...
  // optimize base on (distToOtherTarget, distToMyTarget)
  ...
  y = bestY;
  assert(isValid(x, y));
  // switch to going straight to my target if it's close enough
