...
ll n,k,c[N],ans=-1e18,tmp,s[N],ss[N],buc[N];

int main(){
	cin>>n>>k;
	for(int i=1;i<=n;i++)scanf("%lld",&c[i]);
	sort(c+1,c+n+1);
	for(int i=n;i;i--)s[i]=s[i+1]+c[i],ss[i]=ss[i+1]+c[i]*i;
	if(k==0)cout<<ss[1]-s[1]<<endl,exit(0);
	int pos=1;
	for(int i=1;i<=n;i++){
		ll p=c[i]*(buc[pos]+1);
		tmp+=p,buc[pos]++;
		ans=max(ans,tmp+ss[i+1]-(i-buc[pos])*s[i+1]);
		pos++;
		if(pos>k+1)pos=1;
	} cout<<ans-s[1]<<endl;
	return 0;
