...
void solve() {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        for (auto& x : s) {
            ++cnt;
            if (len(x) > i) continue;
            if (x != t.substr(i - len(x), len(x))) continue;
            if (cmin(dp[i], *min_element(dp.begin() + i - len(x), dp.begin() + i) + 1)) {
                prev[i] = cnt - 1;
            }
        }
    }
    ...
    while (cur > 0) {
        assert(prev[cur] != -1);
        auto& x = s[prev[cur]];
        ans.emplace_back(prev[cur], cur - len(x));
        cur = int(min_element(dp.begin() + cur - len(x), dp.begin() + cur) - dp.begin());
    }

    cout << len(ans) << '\n';
    for (auto [idx, pos] : ans) {
        cout << idx + 1 << " " << pos + 1 << '\n';
    }
}