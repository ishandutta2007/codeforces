...
int main() {
    ...
    vector<vector<pair<int, long long>>> es(n);
    ...
    vector<long long> min_cut(n);

    rep(i, n) {
        for (auto [j, w] : es[i]) {
            seg.apply(0, j, w);
        }
        min_cut[i] = a[i] + seg.all_prod();
    }

    atcoder::segtree<long long, op, e> seg2(min_cut);
