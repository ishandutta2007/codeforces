...
int n,t;
int c[maxn];
set<int> s[maxn],st[maxn];
set<pii> deg[3],degt[3];

void remove_leaf(set<int> s[],set<pii> deg[],int v){
    if (!sz(s[v])){
        deg[c[v]].erase({0,v});
        return;
    }
    auto u=*s[v].begin();
//    cerr<<"remove (u,v):"<<u<<" "<<v<<endl;
    assert(v);
    deg[c[v]].erase({sz(s[v]),v});
    deg[c[u]].erase({sz(s[u]),u});
    s[u].erase(v), s[v].erase(u);
    deg[c[u]].insert({sz(s[u]),u});
}

void erase_grey(set<int> s[],set<pii> deg[]){
    while (1){
        if (!sz(deg[0])) break;
        auto ret=*deg[0].begin();
        if (ret.F>1) break;
        remove_leaf(s,deg,ret.S);
    }
}

int solve(set<int> s[],set<pii> deg[],int beg){
    int res=0;
    erase_grey(s,deg);
    while (1){
        if (!sz(deg[0])&&!sz(deg[1])&&!sz(deg[2])) break;
        if (!sz(deg[beg])) {beg=3-beg; continue;}
        res++;
        while (1){
            if (!sz(deg[beg])) break;
            auto ret=*deg[beg].begin();
//            cerr<<ret.F<<" "<<ret.S<<endl;
            if (ret.F>1) break;
            remove_leaf(s,deg,ret.S);
            erase_grey(s,deg);
        }
...
signed main(){
    ...
    while (t--){
        ...
        rep1(i,n){
            deg[c[i]].insert({sz(s[i]),i});
            degt[c[i]].insert({sz(s[i]),i});
        }
        cout<<max(min(solve(s,deg,1),solve(st,degt,2)),1ll)<<" ";
    }
