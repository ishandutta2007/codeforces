...
void solve() {
    cin >> n >> k;
    read(a, n); read(b, k);
    req.rsz(n); fill(all(req), false);
    F0R(i, k) b[i]--, req[b[i]]=true;
    F0R(i, n) a[i] -= i;
    F0R(i, k-1) if (a[b[i]]>a[b[i+1]]) {NO; return;}
    multiset<int> top;
    int prev = -1;
    F0R(i, n) {
        if (prev != -1 && a[i]<a[prev]) continue;
        if (req[i]) {
            while (top.size()>0 && *--top.end() > a[i]) top.erase(--top.end());
            top.insert(a[i]);
            prev=i;
        } else {
            auto it = top.ub(a[i]);
