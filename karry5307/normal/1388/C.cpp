...
ll test,n,m,sum,x,y; 
vector<ll>gr[MAXN];
ll p[MAXN],h[MAXN],t[MAXN],b[MAXN],fa[MAXN],tt[MAXN];
// Fast IO
inline ll read()
{
...
{
	t[node]=p[node],fa[node]=f;
	for(register int i:gr[node])
	{
		if(i!=f)
		{
			dfs(i,node),t[node]+=t[i];
		}
	}
}
inline void clr()
...
inline void solve()
{
	...
	{
		x=read(),y=read(),gr[x].pb(y),gr[y].pb(x);
	}
	dfs(1,0);
	for(register int i=1;i<=n;i++)
	{
		if(h[i]>t[i]||h[i]<-t[i]||((t[i]-h[i])&1))
		{
			return puts("NO"),clr();
		}
	}
