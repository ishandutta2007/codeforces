...
void solve()
{
    ...
    if (n==1) {cout<<"YES"<<endl; for (int i = 0; i<26; i++) cout<<char('a' + i); cout<<endl; return;}
    ...
    for (int i = 0; i<26; i++) if (G[i].size()>2) {cout<<"NO"<<endl; return;}
    int cnt1 = 0;
    for (int i = 0; i<26; i++) if (G[i].size()==1) cnt1++;
    int cnt2 = 0;
    for (int i = 0; i<26; i++) if (G[i].size()==1) cnt2++;

    if (cnt1!=2) {cout<<"NO"<<endl; return;}
    int cur = 0;
    while (G[cur].size()!=1) cur++;
    vector<int> res = {cur, *G[cur].begin()};
    cur = *G[cur].begin();

    set<int> guys;
    for (int i = 0; i<26; i++) guys.insert(i);

    while (G[cur].size()==2)
    {
        for (auto it: G[cur]) if (it!=res[res.size()-2])
        {
            res.push_back(it);
            cur = it;
            break;
    ...
    for (auto it: res) guys.erase(it);
    for (auto it: guys) res.push_back(it);
    cout<<"YES"<<endl;
    for (auto it: res) cout<<char('a' + it);
    cout<<endl; return;
