...
fun main(){
    solve.cases{
        val n = getint()
         val m = getint()
        val people = getline(n)
        val happy = getline(n)
        val G = Tree(n)

        val totalpeople = IntArray(n)
        val root = 0
        val totalhappy = IntArray(n)
        G.setup()
        G.DFS{
            totalpeople[it] += people[it]
            if(it != root){
                totalpeople[G.parent[it]] += totalpeople[it]
            }
            val dif = totalpeople[it] - happy[it]
            if(dif %2 != 0){
                put("NO")
            ...
            val new = dif / 2
            if(new > totalpeople[it] || new < 0){
                put("NO")
                return@cases
            }
            totalhappy[it] = (totalpeople[it] - new)
            if(!(totalhappy[it] >= G.subs[it].sumOf{totalhappy[it]})){
                put("NO")
                return@cases
            }
        }
