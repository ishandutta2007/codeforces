...
int n, k;
vector<string> mask = { "1110111", "0010010", "1011101", "1011011", "0111010", "1101011", "1101111", "1010010", "1111111", "1111011" };
string s[MAXN];
int f[MAXN][MAXN];
...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		for (int d = 9; d >= 0; --d)
		{
			int tmp = diff(mask[d], s[i]);
			if (tmp <= k && f[i + 1][k - tmp])
			{
				printf("%d", d);
				k -= tmp;
				break;
