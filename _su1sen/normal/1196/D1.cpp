...
int main() {
    ...
    loop(q) {
        ...
        rep(i, n) {
            t[i % 3].push_back(mp[s[i]]);
        }
        vector cum(3, vector(3, vector<int>{}));
        rep(m, 3) {
            int l = t[m].size();
            rep(c, 3) {
                cum[m][c].resize(l + 1, 0);
                rep(i, l) {
                    cum[m][c][i + 1] = cum[m][c][i] + (t[m][i] == c);
                }
            }
        }
        ...
        rep(i, n - k + 1) {
            ...
            rep(c, 3) {
                ...
                rep(m, 3) {
                    int cm = (c + m) % 3;
                    int cnt = cum[m][cm][r[m]] - cum[m][cm][l[m]];
                    cnt_sum += (r[m] - l[m]) - cnt;
                }
                chmin(min_sum, cnt_sum);
            }
            chmin(ans, min_sum);
        }
        print(ans);
