...
l_l op(l_l x,l_l y){
    return (l_l){min(x.fi,y.fi),max(x.se,y.se)};
}
l_l e(){
    return (l_l){inf,-inf};
}
...
void solve(){
    ...
    vl s(n+1);
    rep(i,1,n)s[i] = a[i-1] + s[i-1];
    segtree<l_l,op,e>t0(n+1),t1(n+1);
    rep(i,0,n){
        if(i%2==0){
            t0.set(i,(l_l){s[i],s[i]});
        }else{
            t1.set(i,(l_l){s[i],s[i]});
        }
    }
    ll ans=0;
    map<ll,vl>mp;
    rep(i,0,n)mp[s[i]].pb(i);
    for(auto XX:mp){
        target = XX.fi;
        vl v=XX.se;
        ll vs=v.size();
        rep(i,0,vs-2){
            ll p1 = min(t0.max_right<up>(v[i]), t1.max_right<down>(v[i]));
            ll p2 = min(t1.max_right<up>(v[i]), t0.max_right<down>(v[i]));
            ll pos=max(p1,p2)-1;
            assert(v[i]<=pos);
            ll j=POSU(v,pos)-1;
            assert(i<=j);
