...
int main(void)
{
	...
	rep(i, 0, 3) rep(y, 0, h+1) rep(x, 0, w+1) dp[i][x][y] = -inf;
	dp[0][1][h] = a[1][h];
	per(y, h, 1) rep(x, 1, w) if(P(x, y) != P(1, h)) dp[0][x][y] = a[x][y] + max(dp[0][x-1][y], dp[0][x][y+1]);
	dp[1][w][h] = a[w][h];
	per(y, h, 1) per(x, w, 1) if(P(x, y) != P(w, h))  dp[1][x][y] = a[x][y] + max(dp[1][x+1][y], dp[1][x][y+1]);
	dp[2][w][1] = a[w][1];
	rep(y, 1, h) per(x, w, 1) if(P(x, y) != P(w, 1))  dp[2][x][y] = a[x][y] + max(dp[2][x+1][y], dp[2][x][y-1]);
	dp[3][1][1] = a[1][1];
	rep(y, 1, h) rep(x, 1, w) if(P(x, y) != P(1, 1))  dp[3][x][y] = a[x][y] + max(dp[3][x-1][y], dp[3][x][y-1]);
	
	ll ans = -inf;
	rep(y, 2, h-1) rep(x, 2, w-1){
		chmax(ans, dp[0][x-1][y]+dp[1][x][y+1]+dp[2][x+1][y]+dp[3][x][y-1]);
		chmax(ans, dp[3][x-1][y]+dp[0][x][y+1]+dp[1][x+1][y]+dp[2][x][y-1]);
	}
	outl(ans);
