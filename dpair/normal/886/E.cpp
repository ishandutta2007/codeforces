...
int f[MAXN], fac[MAXN], inv[MAXN], n, k;
...
inline void init(){
    ...
    inv[n] = ksm(fac[n], MOD - 2); per(i, n, 1) inv[i - 1] = 1ll * inv[i] * i % MOD;
}
inline int C(int x, int y) {return 1ll * fac[x] * inv[y] % MOD * inv[x - y] % MOD;}
signed main(){
    read(n, k); init(); int s = f[0] = 1; rep(i, 1, n) {
        f[i] = 1ll * fac[i - 1] * s % MOD; s = (1ll * f[i] * inv[i] + s) % MOD;
        if(i - k >= 0) s = (MOD + s - (1ll * f[i - k] * inv[i - k] % MOD)) % MOD;
    } int ans = 0; rep(i, 1, n) ans = (1ll * C(n - 1, i - 1) * f[i - 1] % MOD * fac[n - i] + ans) % MOD;
    print((fac[n] - ans + MOD) % MOD);
}