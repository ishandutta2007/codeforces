...
type el = 0; //DP 
vector<int> vis,depth,et_in,et_out; int t,et;
vector<type> dp; vector<vector<type>> dp_cand; vector<long long> subtree_size;
void dfs_dp_(graph &g, int s,int i){
    //open
    t++; et++; et_in[s] = et;
    for(auto[nx,cost,label]:g.G[s]){
        if(vis[nx]==i) continue;
        depth[nx] = depth[s] + cost;
        vis[nx] = i; g.par_v[nx] = s; g.par_e[nx] = label;
        dfs_dp_(g,nx,i);
    }
    ...
    if(g.par_v[s]!=-1){
        dp_cand[g.par_v[s]].push_back(dp[s]); //
        subtree_size[g.par_v[s]] += subtree_size[s];
    }
    //DP part-----------------------------------------------------------//
    if(g.par_v[s]!=-1){
        ll t = g.par_v[s];
        if(dp[s]>=2){
            rep(j,2){
                rep(k,2){
                    V.unite(s+k*3e5,t+j*3e5,1,true);
                }
            }
            //cout << "2" << " " << t << " " << s << endl;
        }
        else if(dp[s]==1){
            V.unite(s,t+3e5,1,true);
            //cout << "1" << " " << t << " " << s << endl;
        }
        if(bc-dp[s]>=2){
            rep(j,2){
                rep(k,2){
                    V.unite(t+k*3e5,s+j*3e5,1,true);
                }
            }
            //cout << "2" << " " << s << " " << t << endl;
        }
        else if(bc-dp[s]==1){
            V.unite(t,s+3e5,1,true);
            //cout << "1" << " " << s << " " << t << endl;
        }
    }
    //close
...
void dfs_dp(graph &g, int s=1,int i=0){
    int N = g.G.size();
    if(i==0){
        et_in.assign(N,-1); et_out.assign(N,-1); vis.assign(N,-1); depth.assign(N,-1);
        dp.assign(N+1,el); dp_cand.assign(N+1,vector<type>()); subtree_size.assign(N,1);
    }
    vis[s] = i; depth[s] = 0; t = -1; et = -1;
    dfs_dp_(g,s,i);
}


//multistart BFS
vector<ll> dist(6e5+3,2e18);
vector<ll> bfs_multistart(graph &g,vector<ll> starts){
    ll N = g.G.size();
    queue<int> go;
...
int main(){
    ...
    vector<ll> bs;
    rep(i,N){
        cin >> bl[i+1]; bc += bl[i+1];
        if(bl[i+1]) bs.emplace_back(i+1);
    }
    graph G(N+1,N-1);
    dfs_dp(G,1);
    auto d = bfs_multistart(V,bs);

    rep(i,N){
        if(d[i+1]<2e18||d[i+1+3e5]<2e18) cout << 1 << " ";
        else cout << 0 << " ";
    }
    //cout << endl;
    //cout << ans << endl;
