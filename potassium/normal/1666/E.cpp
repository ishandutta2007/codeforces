...
int main() {
	...
	for (int i = n - 1; i >= 0; i--) {
		itv[i] = { 
			max(itv[i + 1].first - maxv, (i == 0 ? 0 : a[i - 1])),
			min(itv[i + 1].second - minv, (i == 0 ? 0 : a[i]))
		};
	}
	assert(itv[0] == make_pair(0, 0));
	for (int i = 1; i <= n; i++) {
		chkmax(itv[i].first, itv[i - 1].first + minv);
		chkmin(itv[i].second, itv[i - 1].second + maxv);
	}
	assert(itv[n] == make_pair(L, L));
	for (int i = 1; i < n; i++)
		assert(itv[i].first <= itv[i].second);

	int minw = INT_MAX, maxw = 0;
	for (int i = 0; i < n; i++) {
		W(itv[i].first, itv[i + 1].first);
		int len = itv[i + 1].first - itv[i].first;
		chkmin(minw, len);
		chkmax(maxw, len);
	}
	assert(minw == minv && maxw == maxv);
