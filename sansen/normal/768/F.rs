...
fn run() {
    ...
    let pc = precalc::Precalc::new(f + w + h + 10);
    if w == 0 {
        println!("1");
        return;
    }
    if f == 0 {
        println!("{}", (w > h) as usize);
        return;
    }
    let mut nu = M::zero();
    let mut de = M::zero();
    for i in 1..=f.min(w) {
        for &(a, b) in [(i, i + 1), (i + 1, i), (i, i), (i, i)].iter() {
            if a <= f && b <= w {
                let l = pc.comb(f - a + a - 1, a - 1);
                let r = pc.comb(w - b + b - 1, b - 1);
                de += l * r;
            }
            if a <= f && b <= w / (h + 1) {
                let l = pc.comb(f - a + a - 1, a - 1);
                let r = pc.comb(w - (h + 1) * b + b - 1, b - 1);
                nu += l * r;
            }
        }
    }
