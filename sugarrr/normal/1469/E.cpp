...
ll sum0(vl &sm,ll l,ll r){//[l,r]
    if(l>r)return 0;
    ll ret=sm[r];
    if(l>=0)ret-=sm[l];
    return ret;
}
string print(ll ans,ll k){
    string s;
    rep(i,0,k-1)s+='0';
    rep(i,0,min(25,k-1)){
        if((ans>>i)&1){
...
bool solve(){
    ll n,k;cin>>n>>k;
    string s;cin>>s;
    vl sm(n+1);rep(i,1,n)sm[i]=sm[i-1]+('1'-s[i-1]);
    vl v;
    rep(l,0,n-k){
        ll r=l+k;//[l,r)
        if(sum0(sm,l,r-25)>=1)continue;
        ll sum=0;
        rrep(i,r-1,max(l,r-30)){
            if(s[i]=='0')sum+=1ll<<(r-1-i);
        }
        v.pb(sum);
    }
    v.pb(inf);
