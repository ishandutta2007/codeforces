...
#endif
#line 2 "codeforces-workspace\\contest\\1392\\e\\e.cpp"
#include <bits/stdc++.h>
...
#line 12 "codeforces-workspace\\contest\\1392\\e\\e.cpp"
...
solver()
{
    ...
    for(i64 i=1,p=1; i<n*2-2; i++)
    {
        for(int y=0; y<n and y<=i; y++)
        {
            int x=i-y;
            if(x>=n) continue;
            if(y&1)
    ...
    while(q--)
    {
        ...
        i64 k=read();
        for(i64 x=0,y=0,z=0,p=1; z<n*2-2; z++,p*=2,k/=2)
        {
            i64 q=0;
            if(k%2)
            {
