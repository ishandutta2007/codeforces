...
int n;
int x1[140007],x2[140007],y1[140007],y2[140007];
int qzx1[140007],qzx2[140007],qzy1[140007],qzy2[140007];
int hzx1[140007],hzx2[140007],hzy1[140007],hzy2[140007];
...
int main()
{
	...
	//
	fio>>n;
	qzx1[0]=qzy1[0]=hzx1[n+1]=hzy1[n+1]=-Inf;
	qzx2[0]=qzy2[0]=hzx2[n+1]=hzy2[n+1]=Inf;
	for(int i=1;i<=n;i++)
	{
		fio>>x1[i]>>y1[i]>>x2[i]>>y2[i];
		qzx1[i]=max(qzx1[i-1],x1[i]);
		qzy1[i]=max(qzy1[i-1],y1[i]);
		qzx2[i]=min(qzx2[i-1],x2[i]);
		qzy2[i]=min(qzy2[i-1],y2[i]);
	}
	for(int i=n;i>=1;i--)
	{
		hzx1[i]=max(hzx1[i+1],x1[i]);
		hzy1[i]=max(hzy1[i+1],y1[i]);
		hzx2[i]=min(hzx2[i+1],x2[i]);
		hzy2[i]=min(hzy2[i+1],y2[i]);
	}
	for(int i=1;i<=n;i++)
	{
		int nx1,nx2,ny1,ny2;
		nx1=max(qzx1[i-1],hzx1[i+1]);
		ny1=max(qzy1[i-1],hzy1[i+1]);
		nx2=min(qzx2[i-1],hzx2[i+1]);
		ny2=min(qzy2[i-1],hzy2[i+1]);
		if(nx1<=nx2&&ny1<=ny2)
		{
			fio<<nx1<<' '<<ny1<<endl;
			return 0;
