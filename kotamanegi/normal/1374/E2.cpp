...
SegmentTree<int> cnt(2e4, [](int a, int b) {return a + b; }, 0);
SegmentTree<int> val = cnt;

vector<pair<int, int>> ko[4];
void solve() {
    int n, m, k;
    cin >> n >> m >> k;
    vector<int> inputs[3];
    auto P = [](int a) {
        cnt.update(a, cnt[a] + 1);
        val.update(a, val[a] + a);
    };
    auto L = [](int a) {
        cnt.update(a, cnt[a] - 1);
        val.update(a, val[a] - a);
    ...
    REP(i, n) {
        ...
        cin >> a >> b >> c;
        b += c * 2;
        ko[b].push_back(mp(a, i + 1));
        if (b == 0) {
            P(a);
        }
        else {
            inputs[b - 1].push_back(a);
        }
    }
    REP(i, 3) {
        sort(ALL(inputs[i]));
    ...
    int X = 0;
    REP(q, inputs[2].size() + 1) {
        if (inputs[0].size() >= k - q and inputs[1].size() >= k - q) {
            int req_m = m - q - max((k - q), 0LL) * 2;
            int bako = now;
            if (req_m > 0) {
                if (cnt.query(0, 1e4 + 1) >= req_m) {
                    int bot = 0;
                    int top = 1e4 + 1;
                    while (top - bot > 1) {
                        int mid = (bot + top) / 2LL;
                        if (cnt.query(0, mid + 1) < req_m) {
                            bot = mid;
                        }
                        else {
                            top = mid;
                    ...
                    bako += val.query(0, bot + 1) + (req_m - cnt.query(0, bot + 1)) * (bot + 1);
                }
                else {
                    bako = 1e18;
                }
        ...
        REP(tea, 2) {
            if (inputs[tea].size() != 0 and inputs[tea].size() == k - q) {
                now -= inputs[tea].back();
                P(inputs[tea].back());
                inputs[tea].pop_back();
            }
    ...
    vector<pair<int, int>> hoget;
    REP(i, ko[3].size()) {
        if (i < X) {
            cout << ko[3][i].second << " ";
            continue;
        }
        hoget.push_back(ko[3][i]);
    }
    REP(i, ko[0].size()) {
        hoget.push_back(ko[0][i]);
    }
    REP(te, 2) {
        REP(i, ko[te+1].size()) {
            if (i < k - X) {
                cout << ko[te+1][i].second << " ";
                continue;
            }
            hoget.push_back(ko[te+1][i]);
        }
    }
    sort(ALL(hoget));
    REP(i, m - X - max(k - X, 0LL) * 2) {
        cout << hoget[i].second << " ";
    }
    cout << endl;
}
