...
class Segment_tree
{
  ...
  public:
    ...
    void build(A<Monoid, std::allocator<Monoid>> &v)
    {
...
void __solve()
{
    ...
    cin>>a;
    const int inf=1e9;
    Segment_tree<int,int> seg(n+1,-inf,[](int a,int b){return max(a,b);},sbmax<int>);
    auto segg=seg;
    seg.build(a);
    int m; cin>>m;
    for(i32 i=0; i<m; ++i)
    ...
    for(int i=0; i<n; ++ans)
    {
        auto f=[&](int x)->bool
        {
            return segg.query(x,n+1)>=seg.query(i,i+x);
        };
        int j=bin(f,0,n+1-i);
        if(j)
        {
