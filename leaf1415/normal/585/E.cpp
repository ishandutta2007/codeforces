...
int main(void)
{
	...
	for(auto p : pvec){
		for(int i = 10000000/p; i >= 1; i--) cnt[i] += cnt[i*p];
	}
	rep(i, 1, 10000000) num[i] = modpow(2, cnt[i]) - 1;
	...
	rep(i, 1, n){
		...
		rep(j, 1, M-1){
			ll pop = 0, mul = 1;
			rep(k, 0, m-1) if(j & (1<<k)) pop++, mul *= vec[k];
			if(pop % 2) ans -= num[mul];
			else ans += num[mul];
		}
	}
	
	for(auto p : pvec){
		for(int i = 1; i <= 10000000/p; i++) num[i] -= num[i*p];
	}
	ans += mint(n) * (modpow(2, n)-1 - num[1]);
	outl(ans);
