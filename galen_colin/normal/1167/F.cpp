...
void solve(int tc = 0) {
	...
	ll ans = 0;
	for (ll i = 0; i < n; i++) {
		ans = (ans + p[i].f * (((p[i].s + 1) * (n - p[i].s) + (n - p[i].s) * lt.query(0, p[i].s).val + (p[i].s + 1) * rt.query(p[i].s, n - 1).val) % mod)) % mod;
		// cout << p[i].f << " " << ans << '\n';
		lt.update(p[i].s, p[i].s, p[i].s + 1);
		rt.update(p[i].s, p[i].s, n - p[i].s);
	}
	cout << ans << '\n';
}
