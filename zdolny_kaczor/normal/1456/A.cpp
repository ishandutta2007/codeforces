...
void solve() {
	char s[nax];
	int n, p, k, x, y;
	scanf("%d%d%d%s%d%d", &n, &p, &k, s, &x, &y);
	p--;
	ll ans = numeric_limits<ll>::max();
	for (int i = p; i < min(n, p + k); ++i) {
		vi sub;
		for (int j = i; j < n; j += k) sub.push_back(s[j] - '0');
		int zeros = 0;
		for (int c : sub) if (!c) zeros++;
		if (i == p) mini(ans, zeros * 1ll * x);
		debug(imie(sub));
		debug(imie(zeros) * imie(x));
		for (int j = 0; j < (int) sub.size(); ++j) {
			mini(ans, zeros * 1ll * x + ((i - p) + j * 1ll * k) * y);
			if (!sub[j]) zeros--;
			debug(imie(zeros) * imie(x) + (imie(i) + imie(j + 1) * imie(k)) * imie(y));
		}
	}
	printf("%lld\n", ans);
}
