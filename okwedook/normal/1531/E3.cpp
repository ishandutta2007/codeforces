...
vector<int> x = {13, 6, 1, 7, 12, 5, 4, 15, 14, 16, 10, 11, 3, 8, 9, 2};
...
void mymerge(int l, int r) {
    ...
    while (i < m && j < r) {
        int ans = !get();
        if (ans) cp[ptr++] = perm[i], G[perm[i]].pb(perm[j]), ++i;
        else cp[ptr++] = perm[j], G[perm[j]].pb(perm[i]), ++j;
    }
    // if (sz(G) == 16) {
    //     for (int i = l; i < r; ++i)
    //         print(perm[i], ' ');
...
signed main() {
    ...
    read(s);
    int l = 0, r = 1e5 + 1;
    auto trysolve = [&](int len) -> int {
        G = graph(len);
        ptr = 0;
        cp = perm = vector<int>(len);
        iota(all(perm), 0);
        mymerge(0, len);
        if (ptr != sz(s)) {
            return ptr - sz(s);
