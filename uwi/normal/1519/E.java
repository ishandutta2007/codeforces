...
public class E2 {
	...
	void solve()
	{
		...
		long B = BigInteger.probablePrime(30, gen).intValue();
		...
		for(int i = 0;i < n;i++){
			long[] x = new long[]{ni(), ni()};
			long[] y = new long[]{ni(), ni()};
			long[] R = div(y, add(x, ONE));
			long[] U = div(add(y, ONE), x);
			int f = -1, t = -1;
			{
				long code = R[0] * B + R[1];
				if (!map.containsKey(code)) map.put(code, ge++);
				f = map.get(code);
			}
			{
				long code = U[0] * B + U[1];
			...
			from[p] = f; to[p] = t; ws[p] = i+1;
			p++;
		}
		int[][][] g = packWU(ge, from, to, ws);

		boolean[] ved = new boolean[ge];
	...
	boolean dfs(int cur, int come, int[][][] g, boolean[] ved, boolean[] done)
	{
		ved[cur] = true;
		int stack = -1;
		for(int[] e : g[cur]){
			if(e[1] == come)continue;
			if(done[e[1]])continue;
			done[e[1]] = true;
			if(!ved[e[0]]){
				if(dfs(e[0], e[1], g, ved, done)){
					if(stack == -1){
						stack = e[1];
					}else{
						anss.add(new int[]{e[1], stack});
						stack = -1;
					}
				}
			}else{
