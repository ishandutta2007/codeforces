...
struct suffixTree {
    ...
    int dfs(int u, int depth, LL &ans, string &p) // u:nowNode  depth:prefixLen
    {
        int sz = 0;
        for (int i = 0; i <= 26; i++) {
            if (ch[u][i]) {
                sz += dfs(ch[u][i], depth + len[ch[u][i]], ans, p);
            }
        }

        if (sz == 0 && depth > 0) {
            if (p[p.length() - depth] == '0') sz = 1;
        }

        ans = max(ans, 1LL * sz * depth);
        return sz;
    }
