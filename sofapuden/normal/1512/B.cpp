...
void solve(){
	...
	for(int i = 0; i < n; ++i){
		for(int j = 0; j < n; ++j){
			if(v[i][j] == '*')s.push_back({i,j});
		}
	}
	v[s[0].first][s[1].second] = '*';
	v[s[1].first][s[0].second] = '*';
	if(s[0].first == s[1].first){
		v[(s[0].first == 0 ? 1 : 0)][s[1].second] = '*';
		v[(s[0].first == 0 ? 1 : 0)][s[0].second] = '*';
	}
	if(s[0].second == s[1].second){
		v[s[0].first][(s[1].second == 0 ? 1 : 0)] = '*';
		v[s[1].first][(s[1].second == 0 ? 1 : 0)] = '*';
	}
	for(int i = 0; i < n; ++i){
		print(v[i]);
	}
