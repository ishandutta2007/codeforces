...
int main() {
	...
	int h, w; cin >> h >> w;
	auto g = make_v(h, w, 0);
	rep(r, h) rep(c, w) cin >> g[r][c];

	VI cntR(h), cntC(w);
	int mi = *min_element(all(g[0]));
	rep(c, w) {
		cntC[c] = g[0][c] - mi;
	}
	rep(r, h) rep(c, w) g[r][c] -= cntC[c];
	mi = INF;
	rep(r, h) chmin(mi, g[r][0]);
	rep(r, h) {
		cntR[r] = g[r][0] - mi;
	}
	rep(r, h) rep(c, w) g[r][c] -= cntR[r];

	int x = g[0][0];
	if (x < 0) { cout << -1 << newl; exit(0); }
	rep(r, h) rep(c, w) if (g[r][c] != x) { cout << -1 << newl; exit(0); }
	if (h < w) {
		rep(r, h) cntR[r] += x;
	} else {
		rep(c, w) cntC[c] += x;
	...
	int cnt = accumulate(all(cntR), 0) + accumulate(all(cntC), 0);
	cout << cnt << newl;
	rep(r, h) rep(cntR[r]) cout << "row " << r+1 << newl;
	rep(c, w) rep(cntC[c]) cout << "col " << c+1 << newl;
}