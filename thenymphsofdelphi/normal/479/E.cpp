...
const int N = 5e3 + 5;
int mod = 1e9 + 7, mod1 = 998244353, mod2 = 1e9 + 9, inf = 1e9 + N;
ll infll = 1e18 + 7;
...
signed main(){
    ...
    ForE(j, 1, k){
        ...
        ForE(i, 1, n){
            ...
            // cout << i << ' ' << l << ' ' << r << endl;
            dp[jj][i] = (sum[jj ^ 1][r] - sum[jj ^ 1][l - 1] + mod) % mod;
            if (dp[jj][i] >= mod) dp[jj][i] -= mod;
            dp[jj][i] -= dp[jj ^ 1][i];
            if (dp[jj][i] < 0) dp[jj][i] += mod;
        }
        ForE(i, 1, n){
            sum[jj][i] = sum[jj][i - 1] + dp[jj][i];
            if (sum[jj][i] >= mod) sum[jj][i] -= mod;
        }
        // ForE(i, 1, n){
        //     cout << dp[jj][i] << ' ';
        // }
