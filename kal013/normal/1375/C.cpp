...
void solve(){
    ...
    if (stk.size() == 0){
		...
		while(stk.size() > 0 && curr.back() <= stk.back()){
			stk.pop_back();
		}
		if (stk.size() == 0){
			cout <<"NO"<<endl;
 ...
 //    int j = 0;
	// for( ;j < curr.size(); ++j){
	// 	if (stk.size() == 0 || curr[j] > stk.back()) break;
	// }
	// if (j == curr.size()){
