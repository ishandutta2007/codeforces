...
void solve(){
	...
	cin >> n >> k;
	vector<int> inputs;
	inputs.push_back(-1e9);
	REP(i, n) {
		int a;
		cin >> a;
		inputs.push_back(a);
	}
	inputs.push_back(2e9);
	vector<int> b;
	REP(i, k) {
		int a;
		cin >> a;
	...
	for (int i = 1; i < b.size(); ++i) {
		int cnt = inputs[b[i]] - inputs[b[i - 1]] - (b[i] - b[i - 1]);
		if (cnt < 0) {
			cout << -1 << endl;
			return;
		}
		map<int, int> hogea;
		for (int q = b[i - 1] + 1; q < b[i]; ++q) {
			int req = inputs[b[i - 1]] + (q - b[i - 1]);
			if (req > inputs[q]) continue;
			int must_big = inputs[b[i]] - (b[i] - q);
			if (inputs[q] > must_big) continue;
			int geko = inputs[q] - req;
			hogea[geko] = 1;
		}
		{
			int cnter = 0;
		...
		segtree<int, op, e> fw(hogea.size());
		for (int q = b[i - 1] + 1; q < b[i]; ++q) {
			int req = inputs[b[i - 1]] + (q - b[i - 1]);
			if (req > inputs[q]) continue;
			int must_big = inputs[b[i]] - (b[i] - q);
			...
			int geko = inputs[q] - req;
			geko = hogea[geko];
			fw.set(geko, fw.prod(0, geko + 1) + 1);
		}
		ans += b[i] - b[i - 1] - 1 - fw.all_prod();
	}
	cout << ans << endl;
}
