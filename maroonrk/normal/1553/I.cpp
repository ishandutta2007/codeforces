...
void slv(){
	...
	using A=array<array<Poly<mint>,2>,2>;
	auto dfs=[&](auto self,int l,int r)->A{
		dmp2(l,r);
		if(l==r){
			A res;
			rep(i,2)rep(j,2){
				if(i==j)res[i][j]=Poly<mint>{i==0,i==1};
				else res[i][j]=Poly<mint>{0};
			}
			return res;
		}else{
			const int m=(l+r)/2;
			auto x=self(self,l,m);
			auto y=self(self,m+1,r);
			A res;
			rep(i,2)rep(j,2)rep(k,2)rep(z,2){
				auto tmp=x[i][j]*y[k][z];
				if(j==0&&k==0&&ls[m]==1)tmp*=inv2;
				res[i][z]+=tmp;
			}
			return res;
		}
	};
	auto ans=dfs(dfs,0,si(ls));
	dmp(ans[0][0]);
	mint sum;
	rep(k,min(si(ans[0][0]),si(ls)+1)){
		int rem=si(ls)-k;
		sum+=fact[rem]*mint(2).pow(rem)*ans[0][0][k]*(k%2==0?1:-1);
	}
	print(sum);
}
