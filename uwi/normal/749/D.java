...
public class D {
	...
	void solve()
	{
		...
		boolean[] dead = new boolean[n+3];
		int[] lasts = new int[n+3];
		int p = 0;
		int q = n-1;
		for(int i = n-1;i >= 0;i--){
			if(!dead[who[i]]){
				dead[who[i]] = true;
				lasts[p] = who[i];
				while(q >= 0 && dead[who[q]])q--;
				p++;
			}
		}
		int[][] buc = makeBuckets(who, n+1);
				...
				if(killed[lasts[i]]){
					...
					for(int j = i+1;j < n+2;j++){
						if(killed[lasts[j]]){
							...
							if(lasts[j] == 0)break;
							pos = buc[lasts[j]][buc[lasts[j]].length-1];
							break;
						}
					}
					int ind = lowerBound(buc[lasts[i]], pos);
					out.println(lasts[i] + " " + bid[buc[lasts[i]][ind]]);
					break;
				}
			}
