...
void solve(){
  ...
  if(u[0]==u[2]){
    ...
    for(int i=2;i<u.size();i++){
     // cout<<i<<" "<<u[i]<<" "<<q.size()<<endl;
      if(u[i]!=u[0]) break;
      if(q.size()){
        ans.pb(q[int(q.size())-1]);
        q.pop_back();
      }
      ans.pb(u[0]);
    }
    rever(q);
    for(auto e:q) ans.pb(e);
    bool solve=true;
    for(int i=1;i<ans.size()-1;i++){
      if(ans[0]==ans[i]&&ans[1]==ans[i+1]) solve=false;
    }
    if(solve){
      cout<<ans<<endl;
      return;
    ...
    if(c==2){
      ...
      ll now=1;
      vector<char> f;
      for(auto e:m) f.pb(e.fi);
      string ans;
      sor(f);
      ans.pb(f[0]);
      ans.pb(f[1]);
      vector<bool> use(int(s.size()),true);
      for(int i=1;i<u.size();i++){
        if(ans[0]==u[i]){
          ans.pb(u[i]);
          use[i]=false;
