...
int main() {
	...
	const Mint HALF = Mint(1, MOD998) / 2;

	vector<Mint> dp(n + 1, Mint(0, MOD998));
	function<void(int, int)> dfs = [&](int u, int p) {
		Mint P_unmatched(1, MOD998);
		for (auto& v : g[u]) {
			if (v == p) continue;
			dfs(v, u);
			P_unmatched *= 1 - (HALF * (1 - dp[v]));
		}
		dp[u] = 1 - P_unmatched;
	};
	dfs(1, -1);

	W(accumulate(dp.begin() + 1, dp.end(), Mint(0, MOD998)) * Mint(2, MOD998).pow(n));
