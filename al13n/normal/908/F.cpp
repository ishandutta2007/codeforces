...
int main(int argc, char **argv){
  ...
  forn(i,n){
    ...
    if(c=='G'){
      ...
      (c=='B'?B.back().bs:B.back().rs).pb(x);
  }
  LL ans=0;
  for(const bin& b:B){
    auto doit=[&ans](LL a,LL b,const vector<LL>&v)->LL{
      if(!sz(v)) return b-a;
      if(a==-INF*10ll&&b==INF*10ll){
        ans+=v.back()-v[0];
        return 0;
      }
      LL mxg=max(v[0]-a,b-v.back());
      forn(i,sz(v)-1) mxg=max(mxg,v[i+1]-v[i]);
      ans+=b-a-mxg;
      return mxg;
    };
    LL g=doit(b.a,b.b,b.rs)+doit(b.a,b.b,b.bs);
    if(b.a==-INF*10ll || b.b==INF*10ll) continue;
    ans+=min(g,b.b-b.a);
  }
  cout<<ans;
