...
void calc(int n,int k){
    assert(prec.size()==k+2);
    
    int ans=0;
    int res=1;
    loop(i,1,k+2){
        res=(res*(n-i))%mod;
        res=(res*(fast_expo(-i,mod-2))%mod)%mod;
        if (res<0) res+=mod;
    }
    ans=(prec[0]*res)%mod;
    loop(i,1,k+2){
        res=(res*(n-i+1))%mod;
        res=(res*fast_expo(n-i,mod-2))%mod;
        res=(res*(-k+i-2+mod))%mod;
        res=(res*fast_expo(i,mod-2))%mod;
        ans=(ans+(prec[i]*res)%mod)%mod;
    }
