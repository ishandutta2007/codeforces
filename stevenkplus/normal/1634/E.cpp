...
int main() {
   ...
   for (int i = 0; i < m; ++i) {
      cin >> sizes[i];
      for (int a = 0, j = sizes[i] - 1; a < j; ++a, --j) {
         edges.push_back(pii(sz(v) + a, sz(v) + j));
      }
      for (int j = 0; j < sizes[i]; ++j) {
         ...
         v.push_back(x);
         if (last[x]) {
            edges.push_back(pii(sz(v) - 1, last[x] - 1));
            last[x] = 0;
         } else {
            last[x] = sz(v);
         }
   ...
   for (auto p: edges) {
      ed[p.fi].push_back(p.se);
      ed[p.se].push_back(p.fi);
   }
   vector<int> color(n);
   auto visit = yc([&](auto rec, int cur, int col) -> void {
      if (color[cur]) return;
      color[cur] = col;
      for (int nxt: ed[cur]) {
         rec(nxt, 3 - col);
