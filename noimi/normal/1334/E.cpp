...
vector<int> prime_factorization(ll n,vecll &primes){
    vec res;
    for(auto e:primes){
        int cnt=0;
        while(n%e==0)n/=e,cnt++;
...
signed main(){
    ...
    vecll primes;
    for(auto e:P)primes.eb(e.fi);
    int q=in();
    const int n=primes.size();
    calc();
    while(q--){
        ll S=lin(),G=lin();
        vec s=prime_factorization(S,primes),g=prime_factorization(G,primes);
        vec u;
        rep(i,primes.size())u.pb(min(s[i],g[i]));
        mint ans=1;
        int cnt=0;
        rep(i,n)cnt+=s[i]-u[i];
        rep(i,n)ans*=cmb(cnt,s[i]-u[i]),cnt-=s[i]-u[i];
        rep(i,n)cnt+=g[i]-u[i];
        rep(i,n)ans*=cmb(cnt,g[i]-u[i]),cnt-=g[i]-u[i];
        cout<<ans<<endl;
    }
