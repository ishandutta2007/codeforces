...
int main() {
	...
	V<tuple<int, int, ll>> eds;
	rep(m) {
		int u, v;
		ll c;
		cin >> u >> v >> c;
		eds.emplace_back(u, v, c);
	}
	...
	// int u, v;
	// tie(u, v, ignore) = eds[memo_i];

	// ll left = g.dijkstra(s)[u];
	// ll right = g.dijkstra(t)[v];
