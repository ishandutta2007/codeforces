...
void solve() {
    ...
    vvc<int> dp(n, vc<int>(2, inf));
    dp[0][0] = 0;
    pqg<tuple<ll, int, int>> pque;
    pque.push(make_tuple(0, 0, 0));
    while(pque.size()) {
        auto [d, v, f] = pque.top(); pque.pop();
        if(d > dp[v][f]) continue;
        if(!f) {
            for(auto [nv, w]: g[v]) {
                if(chmin(dp[nv][0], d + w)) {
                    pque.push(make_tuple(dp[nv][0], nv, 0));
                }
            }
            if(chmin(dp[v][1], d)) {
                pque.push(make_tuple(dp[v][1], v, 1));
            } 
        } else {
            for(auto [nv, w]: rg[v]) {
                if(chmin(dp[nv][1], d + w)) {
                    pque.push(make_tuple(dp[nv][1], nv, 1));
                }
            }
        }
    }
