...
void solve() {
    ...
    long double ans = -1.0;
    REP(i, (int)inputs.size() - 2) {
        auto x = lower_bound(ALL(inputs), inputs[i] + u + 1);
        int hoge = x - inputs.begin();
        hoge--;
        if (i + 1 >= hoge) continue;
        long double bunsi = inputs[hoge] - inputs[i + 1];
        long double bunbo = inputs[hoge] - inputs[i];
        ans = max(ans, bunsi / bunbo);
    }
    if (ans == -1.0) {
        cout << -1 << endl;
        return;
