...
int a[N],b[N];
void Solve(){
	int n=read(),cost=read();
	repeat(i,0,n)a[i]=read();
	repeat(i,0,n-1)b[i]=read();
	int maxspeed=a[0],nowmoney=0,nowday=0,ans=INF;
	repeat(i,0,n){
		if(i!=0){
			int t=(b[i-1]-nowmoney+maxspeed-1)/maxspeed;
			nowday+=t+1;
			nowmoney+=t*maxspeed-b[i-1];
			maxspeed=max(maxspeed,a[i]);
		}
		ans=min(ans,nowday+(cost-nowmoney+maxspeed-1)/maxspeed);
	}
	print(ans,1);
}
signed main(){
