...
Fenw f1;
long long solve1(vector<pii> v) {
    vector<F> allF;
    for (pii p : v) {
        auto from = F{p.first, p.second + w};
        auto to = F{p.first, p.second - w};
        allF.push_back(from);
        allF.push_back(to);
    }
    allF.push_back({0, 1});
    ...
    for (pii& p : v) {
        auto from = F{p.first, p.second + w};
        auto to = F{p.first, p.second - w};
        p = {int(lower_bound(all(allF), from) - allF.begin()), int(lower_bound(all(allF), to) - allF.begin())};
    }
    sort(all(v), [](pii p1, pii p2){
        return p1.first < p2.first || (p1.first == p2.first && p1.second > p2.second);
    });
    ll res = 0;
    memset(f1.tr, 0, sizeof(f1));
    for (int i = 0; i < (int) v.size(); ++i) {
        res += i - f1.sum(v[i].second - 1);
        f1.add(v[i].second);
    }
    return res;
}
Fenw f[2];
long long solve2(vector<pii> v[2]) {
    vector<F> allF;
    FI(2) {
        for (pii p : v[i]) {
            auto from = F{p.first, p.second + w};
...
int solve() {
    ...
    FI(n) {
        cin >> x[i] >> a[i];
        v[x[i] < 0].push_back({abs(x[i]), abs(a[i])});
    }
    ll ans = 0;
    ans += solve1(v[0]);
    ans += solve1(v[1]);
...
//    int ans = 0;
//    for (int i = l; i <= r; ++i) {
//        res[i] = int(upper_bound(sb, sb + vb.size() + 1, r - i) - sb);
//    }
//    for (int i = l; i <= r; ++i) {
//        if (bs[i]) {
//            ans = res[i];
