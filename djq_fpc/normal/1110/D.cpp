...
=[]=========================== noname01.pas ==========================1=[?]=
var a:array[1..100] of longint; i,j,n,m,t:longint;                            
begin                                                                         
readln(n);                                                                    
for i:=1 to n do                                                              
   begin                                                                      
...
int n,m,i,j,k,l,p,t,cnt[1000005],x,ans,sum,dp[2][10][10];
int main()
{
	scanf("%d%d",&n,&m);
	fz1(i,n)
	...
	fz0g(i,m+2)
	{
		memset(dp[(i+1)&1],-15,sizeof(dp[(i+1)&1]));
		fz0g(j,8)
		{
			fz0g(k,8) if (dp[i&1][j][k]>dp[0][9][9])
			{
				l=cnt[i+1];
				t=min(min(2,j),min(k,l));
				fz0g(p,t)
				{
					dp[(i+1)&1][k-p][l-p]=max(dp[(i+1)&1][k-p][l-p],dp[i&1][j][k]+p+(j-p)/3);
				}
			}
		}
	}
	fz0g(i,8) fz0g(j,8) ans=max(ans,dp[(m+3)&1][i][j]);
	cout<<ans+sum<<endl;
	return 0;
}