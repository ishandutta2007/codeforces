...
int main() {
	...
	cerr << sizeof(suffix_automaton<'a', 26>::state) << endl;

    string s; re(s);
    suffix_automaton<'a', 26> sa(all(s));

    vector<int> last_visited(sa.data.size(), -1);
	...
	int Q; re(Q); F0R (q, Q) {
        ...
        F0R (_, 2) {
            F0R (i, L) {
                if (cur_len == L) { // remove first character
                    cur_len--;
                    while (loc != 0 && sa[sa[loc].suffix_link].len >= cur_len)
                        loc = sa[loc].suffix_link;
                }

                // append queries[q][i]
                while (loc != -1 && sa.transition(loc, qry[i]) == -1) {
                    loc = sa[loc].suffix_link;
                    cur_len = sa[loc].len;
                }
                if (loc == -1) break;

                loc = sa.transition(loc, qry[i]);
                cur_len++;

                if (cur_len == L && last_visited[loc] != q) {
                    ans += sa.ct_end_pos[loc];
                    last_visited[loc] = q;
                }
            }
        }
