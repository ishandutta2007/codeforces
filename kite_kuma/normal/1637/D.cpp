...
ll solve(int n, vi a, vi b) {
	...
	const int maxdiffrence = n * maxvalue + 10;
	const int sum_a = accumulate(all(a), 0);
	const int sum_b = accumulate(all(b), 0);
	vi dp(maxdiffrence);
	auto tmp = dp;
	dp[0] = 1;
	rep(i, n) {
		tmp.assign(maxdiffrence, 0);
		const int abdiff = abs(a[i] - b[i]);
		rep(val, maxdiffrence) {
			if(!dp[val]) continue;
			for(int mult = -1; mult < 2; mult += 2) {
				const int nxtval = abs(mult * abdiff + val);
				if(nxtval < maxdiffrence) {
					tmp[nxtval] = 1;
				}
			}
		}
	...
	int minimum_diffrence = 0;
	for(; minimum_diffrence < maxdiffrence; minimum_diffrence++) {
		if(dp[minimum_diffrence]) {
			break;
		}
	}
	ll sa = (sum_a + sum_b + minimum_diffrence) / 2;
	ll sb = sum_a + sum_b - sa;

	ll ans = 0;
	rep(i, n) { ans += (n - 2) * (POW(a[i], 2) + POW(b[i], 2)); }

	ans += POW(sa, 2) + POW(sb, 2);
	return ans;
}
int main() {
