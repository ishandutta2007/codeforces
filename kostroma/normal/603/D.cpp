...
void solve() {
	...
	for (int i = 0; i < n; ++i) {
		...
		ld t = -c / (ld)1.0 / (a * a + b * b);

		Point cur = Point(a, b) * t;

		all_points.push_back(cur);
	}

	li res = onZero * 1LL * (onZero - 1) / 2 * (n - onZero);
	...
	for (int i = 0; i < n; ++i) {
		...
		for (int j = 0; j < n; ++j) {
			if (j != i) {
				points.push_back(all_points[j] - all_points[i]);
			}
		}
		for (int i = 0; i < points.size(); ++i) {
			if (points[i].y <= -eps || fabs(points[i].y) <= eps && points[i].x < 0) {
		...
		for (int j = 0; j < points.size();) {
			int r = j;
			while (r < points.size() && fabs(points[r] * points[j]) < eps) {
				++r;
			}
			int cur_cnt = r - j;
			cur_res += cur_cnt * 1LL * (cur_cnt - 1) / 2;
			j = r;
		}
	}
