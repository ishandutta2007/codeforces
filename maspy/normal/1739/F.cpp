...
#line 1 "/home/maspy/compro/library/graph/path_cycle.hpp"
//  2 
// , 
// 
...
void solve() {
  ...
  FOR(s, 1 << K) {
    FOR(k, K) {
      ...
      FOR(v, N) {
        ...
        int to = X.TO[v][k];
        if (chmax(DP[t][to], DP[s][v] + X.node_value[to])) {
          PAR[t][to] = {s, v};
        }
      }
    }
  ...
  int s = (1 << K) - 1;
  int v = max_element(all(DP[s])) - DP[s].begin();
  string ANS;
  while (s) {
    int t = s;
    int to = v;
