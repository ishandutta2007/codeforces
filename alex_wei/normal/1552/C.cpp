...
int n,k,x[N],y[N],res[N],buc[N],cnt;
void solve(){
	cin>>n>>k,mem(buc,0),cnt=0;
	for(int i=1;i<=k;i++)cin>>x[i]>>y[i],buc[x[i]]=buc[y[i]]=1;
	for(int i=1;i<=2*n;i++)if(!buc[i])res[++cnt]=i;
	sort(res+1,res+cnt+1);
	for(int i=1;i<=cnt/2;i++)k++,x[k]=res[i],y[k]=res[i+cnt/2];
	for(int i=1;i<=n;i++)if(x[i]>y[i])swap(x[i],y[i]);
	cnt=0;
	for(int i=1;i<=n;i++)
		for(int j=i+1;j<=n;j++)
			cnt+=(x[i]<x[j]&&x[j]<y[i]&&y[i]<y[j]||x[j]<x[i]&&x[i]<y[j]&&y[j]<y[i]);
	cout<<cnt<<endl;
}
