...
void solve() {
  ...
  }
  {
    ...
    UNIQUE(X);
    FOR(i, N) FOR(j, M) A[i][j] = LB(X, A[i][j]);
  }
  {
    auto I = argsort(wt);
    vi wt2(N);
  ...
  vi hash_base(N * M);
  FOR(i, N * M) hash_base[i] = RNG(0, 1LL << 60) * 2 + 1;

  vv(ll, sets, N, 1 << M);
  FOR(i, N) FOR(s, 1 << M) FOR(j, M) if (s & 1 << j) {
    sets[i][s] ^= hash_base[A[i][j]];
  }
  vi H(N << M);
  FOR(i, N) FOR(s, 1 << M) { H[i << M | s] = sets[i][s]; }
  UNIQUE(H);
  FOR(i, N) FOR(s, 1 << M) sets[i][s] = LB(H, sets[i][s]);

  // wt  n i 
  vc<int> NG(N), OK(N, N + 1);
  vc<vc<int>> QID(N + 1);
  vc<int> CNT(len(H));

  int coef[32];
  FOR(s, 32) coef[s] = (popcnt(s) % 2 == 0 ? 1 : -1);

  while (1) {
    FOR(i, N + 1) QID[i].clear();
    FOR(x, len(H)) CNT[x] = 0;
    ...
    FOR(i, N) {
      // i 
      FOR(j, 1 << M) CNT[sets[i][j]]++;

      for (auto&& q: QID[i + 1]) {
        ll x = 0;
        FOR(s, 1 << M) { x += CNT[sets[q][s]] * coef[s]; }
        if (x > 0) {
          OK[q] = i + 1;
        } else {
          NG[q] = i + 1;
