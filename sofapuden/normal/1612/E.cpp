...
void solve(){
	...
	ll bes = 1;
	ll besan = 0;
	for(ll d1 = 1; d1 < min(23,n); ++d1){
		ll ans1 = 0;
		{
			map<ll,ll> M;
			for(int i = 0; i < n; ++i){
				M[v[i].first]+=min(d1,v[i].second);
			}
			vector<ll> x;
			for(auto y : M)x.push_back(y.second);
			sort(rall(x));
			for(int i = 0; i < d1; ++i){
		...
		if(besan*d1 < ans1*bes){bes = d1;besan = ans1;}
	}
	map<ll,ll> M;
	for(int i = 0; i < n; ++i){
		M[v[i].first]+=min(v[i].second,bes);
	}
	vector<pair<ll,ll>> x;
	for(auto y : M)x.push_back({y.second,y.first});
	sort(rall(x));
	cout << bes << endl;
	for(int i = 0; i < bes; ++i){
		cout << x[i].second << ' ';
