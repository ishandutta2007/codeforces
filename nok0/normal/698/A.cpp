...
void main_() {
	...
	dp[0][0] = 0;
	REP(i, n) {
		chmin(dp[i + 1][0], min({dp[i][0] + 1, dp[i][1] + 1, dp[i][2] + 1}));
		if(a[i] == 2 or a[i] == 3) {
			chmin(dp[i + 1][1], min(dp[i][0], dp[i][2]));
		}
		if(a[i] == 1 or a[i] == 3) {
			chmin(dp[i + 1][2], min(dp[i][0], dp[i][1]));
		}
	}
	print(*min_element(all(dp.back())));
}