...
void dfs(const int& v, const int& par, const vvi& g, vb& nv, vvi& gd, vl& opt, vl& sopt, const vl& w)
{
	nv[v] = false;
	for (int to : g[v])
		if (to != par)
		{
			if (nv[to])
			{
				dfs(to, v, g, nv, gd, opt, sopt, w);
				if (sz(gd[to]))
					gd[v].pb(to);
				else
				{
					ll new_var = w[to] + opt[to];
					if (new_var > opt[v])
					{
						sopt[v] = opt[v];
						opt[v] = new_var;
...
void dfs1(const int& v, int par, const vvi& g, vb& nv, bool upper, vvi& gd)
{
	if (upper)
		gd[v].pb(par);
	nv[v] = false;
	for (int to : g[v])
		if (to != par)
		{
			if (nv[to])
			{
				dfs1(to, v, g, nv, sz(gd[v]) != (sz(gd[to]) >= 1), gd);
			}
		}
}
...
int another_main()
{
	...
	--e;
	vvi g = adjlist_from_edgelist(e, n);
	vb nv(n, true);
	vvi gd(n);
	vl opt(n);
	vl sopt(n);
	...
	for (int i = 0; i < n; ++i)
	{
		ans = max(ans, opt[i] + sopt[i] + w[i]);
		if (gd[i] >= 2)
		{
			op = max(op, max(pll(op.X, opt[i]), pll(opt[i], op.X)));
			sum += w[i];
		}
	}
	cout << max(ans, sum + op.X + op.Y);
	*/
	return 0;
}
