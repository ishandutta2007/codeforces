...
int get_dist(int pos, int x, int y) {
    if (x <= pos && pos <= y) return 0;
...
//    DBN(INFLL);
    ...
    for (int i = 1; i <= n; ++i) {
        cin >> px[i] >> py[i];
        positions.push_back(px[i]);
        positions.push_back(py[i]);
    }
    sort(all(positions));
    positions.resize(std::unique(all(positions)) - positions.begin());
//    cout << positions << endl;
    
    int s = (int)(lower_bound(positions.begin(), positions.end(), x) - positions.begin());
...
//    DBN(s);
    for (int i = 1; i <= n; ++i) {
        ll minres = curd[0] - positions[0];
        for (int j = 0; j < k; ++j) {
            minres = min(minres, curd[j] - positions[j]);
            d[j] = positions[j] + minres + get_dist(positions[j], px[i], py[i]);
        }
        minres = curd[k - 1] + positions[k - 1];
        for (int j = k - 1; j >= 0; --j) {
            minres = min(minres, curd[j] + positions[j]);
            d[j] = min(d[j], -positions[j] + minres + get_dist(positions[j], px[i], py[i]));
        }
//        printarray(d, k);
        swap(d, curd);
    }
