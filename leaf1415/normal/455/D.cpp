...
int B = 330;
int a[120005];
int cnt[350][120005], rot[350];
int tmp[350];

int get(int p)
{
	int id = p/B;
	return (p-id*B+B-rot[id])%B+id*B;
}

void adjust(int id)
{
	rep(i, 0, B-1) tmp[i] = a[get(id*B+i)];
	rot[id] = 0;
	rep(i, 0, B-1) a[id*B+i] = tmp[i];
}
...
int main(void)
{
	...
	rep(q, 1, Q){
		...
		if(t == 1){
			adjust(l/B), adjust(r/B);
			int x = a[r];
			
			if(L > R){
				per(i, r, l+1) a[i] = a[i-1];
				a[l] = x;
			}
			else{
				cnt[l/B][x]++;
				rep(i, l, L*B-1) swap(x, a[i]);
				cnt[l/B][x]--;
		...
		if(t == 2){
			...
			ans = 0;
			if(L > R){
				rep(i, l, r) if(a[get(i)] == k) ans++;
			}
			else{
				rep(i, l, L*B-1) if(a[get(i)] == k) ans++;
				rep(i, R*B, r) if(a[get(i)] == k) ans++;
				rep(i, L, R-1) ans += cnt[i][k];
			}
			cout << ans << "\n";
		}
	}
