...
int n,m;
ll a[MAXN];
vc<int> l[MAXN], r[MAXN];

typedef tuple<ll,ll,ll,ll> Token;
il void operator^=( Token &a , const Token &b ){
	a = {
		get<0>(a) ^ get<0>(b),
		get<1>(a) ^ get<1>(b),
		get<2>(a) ^ get<2>(b),
...
void GetTokens( pair<int,Token> res[] ){
	// static ll lVal[MAXN];
	For(i,n){
		res[i].F = i;
		res[i].S = {0,0,0,0};
	...
	For(i,n){
		// Print(l[i]);
		const Token t = {Rand(),Rand(),Rand(),i};
		for( auto v : l[i] )
			res[v].S ^= t;
	}
}
...
ll Solve(){
	static pair<int,Token> tokens[MAXN];
	GetTokens(tokens);
	sort(tokens+1,tokens+1+n,[]( const auto &a , const auto &b ){
		return a.S < b.S;
	});
	ll ans = 0;
	ll tSum = 0;
	For(i,n){
		if(r[tokens[i].F].empty()) continue;
		tSum += a[tokens[i].F];
		if( i == n or tokens[i+1].S != tokens[i].S ){
			ans = gcd(ans,tSum);
			tSum = 0;
		}
	}
...
int main(){
	...
	// For(t,100000){
	// 	int a = rand()%10000000-5000000;
	// 	int b = rand()%10000000-5000000;
