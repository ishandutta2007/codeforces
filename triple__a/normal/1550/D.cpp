...
signed main(){
    ...
    cin>>_;
    f[0]=1;
    rep1(i,200000) f[i]=f[i-1]*i%mod;
    while (_--){
        cin>>n>>l>>r;
        auto C=[&](int u,int v){
            if (u<v||v<0||u<0) return 0ll;
            return f[u]*basics::power(f[v]*f[u-v]%mod,mod-2,mod)%mod;
        };
        int ans=0;
        if (n&1){
            int x=min(1-l,r-n);
            ans=x*(C(n,n/2)+C(n,n/2+1))%mod;
            int le=1,re=n;
            rep1(i,n){
                if (re+x+i>r) re--;
                if (le-x-i<l) le++;
                ans+=C(re-le+1,re-n/2)+C(re-le+1,re-n/2-1);
                // cout<<ans<<endl;
                ans%=mod;
            }
        }
