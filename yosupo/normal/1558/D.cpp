...
/*
AA TreeStarry Sky Tree
merge/split
*/

template <class D, class C = less<D>> struct AASSTree {
    struct Node;
    typedef Node* NP;
    static Node last_d;
    static NP last;
    struct Node {
        ...
        static NP make(D vv) { return new Node(vv); }
    ...
    void add(int l, int r, D x) { add(n, l, r, x); }
    ...
    D get(int l, int r) { return get(n, l, r); }
    ...
    int lb(D v) { return lb(n, v); }
    void insert(D x) { n = insert(n, lb(x), x); }
    void erase(D x) { n = remove(n, lb(x)); }
    ...
    void insert(int k, D x) { n = insert(n, k, x); }
...
/*        for (int j = 0; j < tr.sz(); j++) {
    ...
    mint ans = comb<mint>(n - 1 - tr.sz() + n, n);
