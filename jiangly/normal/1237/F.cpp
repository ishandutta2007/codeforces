#include <bits/stdc++.h>
using namespace std;
constexpr int MAX_N = 3600;
constexpr int MOD = 998244353;
int A[MAX_N + 1][MAX_N + 1], F[MAX_N + 1][MAX_N / 2 + 1], dpv[2][MAX_N / 2 + 1], dph[2][MAX_N / 2 + 1];
int main() {
    ...
    for (auto i = ++x.begin(), j = x.begin(); i != x.end(); j = i++)
        if (i->first != j->second)
            r.push_back(i->first - j->second);
    for (auto i = ++y.begin(), j = y.begin(); i != y.end(); j = i++)
        if (i->first != j->second)
            c.push_back(i->first - j->second);
    int cv = 0, ch = 0;
    dpv[0][0] = 1;
    int sv = 0, sh = 0;
    for (int i = 0; i < r.size(); ++i) {
        ...
        for (int j = 0; j <= sv / 2; ++j)
            for (int k = 0; k <= r[i] / 2; ++k)
                dpv[cv][j + k] = (dpv[cv][j + k] + 1LL * dpv[cv ^ 1][j] * F[r[i]][k]) % MOD;
        sv += r[i];
    }
    dph[0][0] = 1;
    for (int i = 0; i < c.size(); ++i) {
        ...
        for (int j = 0; j <= sh / 2; ++j)
            for (int k = 0; k <= c[i] / 2; ++k)
                dph[ch][j + k] = (dph[ch][j + k] + 1LL * dph[ch ^ 1][j] * F[c[i]][k]) % MOD;
        sh += c[i];
    }
    int ans = 0;
    for (int v = 0; 2 * v <= sv; ++v)
        for (int h = 0; 2 * h + v <= sh && 2 * v + h <= sv; ++h)
            ans = (ans + 1LL * dpv[cv][v] * dph[ch][h] % MOD * A[sv - 2 * v][h] % MOD * A[sh - 2 * h][v]) % MOD;
    cout << ans << "\n";
    return 0;
}