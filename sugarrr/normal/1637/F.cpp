...
signed main(){fastio
    ...
    b[root] =a[root];
    for(auto x:jun){
        if((int)chi[x].size()==0)continue;
        if(root==x){
            sort(all(chi[x]),[&](ll y,ll z){return a[y]>a[z];});
            if((int)chi[x].size()==1){
                for(auto c:chi[x]){
                    b[c] = b[x];
                }
            }else{
                assert((int)chi[x].size()>=2);
                for(auto c:chi[x]){
                    b[c] = a[c];
                }
                rep(i,0,1){
