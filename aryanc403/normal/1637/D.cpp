...
using bt = bitset<10000+1>;
int main(void) {
    ios_base::sync_with_stdio(false);cin.tie(NULL);
    // freopen("txt.in", "r", stdin);
    // freopen("txt.out", "w", stdout);
// cout<<std::fixed<<std::setprecision(35);
auto getNear=[&](vi a,const lli c)->lli{
    bt b;
    b[0]=1;
    for(auto x:a)
        b|=b<<x;
...
cin>>T;while(T--)
{
    ...
    lli sum=0,bsum=0,total=0;
    vi b;
    lli ans=0;
    for(auto x:a){
        const lli u=min(x.X,x.Y),v=x.X^x.Y^u;
        sum+=u;
        b.pb(v-u);
        bsum+=v-u;
        ans+=u*u+v*v;
    ...
    ans*=n-2;
    lli c=getNear(b,(bsum+1)/2);

    // run 1000 times so that we can benchmark them.
    for(lli itr=0;itr<=1000;++itr)
        c=min(c,getNear(b,(bsum+1)/2));

    dbg(b,c);
    const lli sa=sum+c;
    ans+=sa*sa+(total-sa)*(total-sa);
    cout<<ans<<endl;
}   aryanc403();
    return 0;
}