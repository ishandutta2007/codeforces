namespace Solution {
	...
	operation Diff (x : Qubit[], y : Int) : Unit {
		body (...) {
			let n = Length(x);
			for (i in 0..n-y-1) {
				Controlled X([x[i + y]], (x[i]));
			}
		}
		adjoint auto;
	}
	operation CheckPeriod (x : Qubit[], y : Qubit, p : Int) : Unit {
		body (...) {
			let n = Length(x);
			Diff(x, p);
			Apply(x[0..n-p-1], X);
	...
	operation FillPeriod (x : Qubit[], y : Qubit[]) : Unit {
		body (...) {
			let n = Length(x);
			for (p in 1..n-1) {
				CheckPeriod(x, y[p - 1], p);
			}
		}
		adjoint auto;
	}
    operation Solve (x : Qubit[], y : Qubit) : Unit {
        body (...) {
			let n = Length(x);
			using (extra = Qubit[n - 1]) {
				FillPeriod(x, extra);
				Apply(extra, X);
				X(y);
				Controlled X(extra, (y));
				Apply(extra, X);
				Adjoint FillPeriod(x, extra);
			}
        }
		adjoint auto;
    }
