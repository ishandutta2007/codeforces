...
bool ban[SZ];
vector<int> op;
//misread statement for 1h, very cool
void dfs(int ro)
{
	if(ro>=(1<<g)) return;
	int x;
	...
	{
		b[x]=a[x];
		if(a[x+x]==0&&a[x+x+1]==0)
		{
			if(x>=(1<<g)) a[x]=0;
			else ok=0;
			break;
		}
		if(a[x+x]>a[x+x+1]) a[x]=a[x+x],x=x+x;
		else a[x]=a[x+x+1],x=x+x+1;
	}
	if(!ok) break; op.pb(ro);
	}
	int y=x;
	while(y!=ro) a[y]=b[y],y>>=1;
	a[ro]=b[ro];
	dfs(ro+ro+1); dfs(ro+ro);
}
void main()
{
	scanf("%d",&T);
	while(T--)
	{
		scanf("%d%d",&h,&g);
		int n=(1<<h)-1;
		for(int i=1;i<=n+n+2;++i) a[i]=0;
		for(int i=1;i<=n;++i)
			scanf("%d",a+i),ban[i]=0;
		op.clear(); dfs(1);
		ll su=0;
		for(int i=1;i<(1<<g);++i) su+=a[i];
		printf("%lld\n",su);
		for(auto t:op)
			printf("%d ",t);
		puts("");
