...
fun BooleanArray.compare(other:BooleanArray){
...
fun main(){
    solve.cases{
        ...
        val fone = str.indexOfFirst { it == '1' }
        if(fone == -1){
            put(0)
            return@cases
        }
        str = str.slice(fone until n)
        n = str.size

        val firstzero = str.indexOfFirst { it == '0' }
        if(firstzero == -1){
            put(str)
            return@cases
        }
        val got = BooleanArray(n)
        check@for(p in 0..firstzero){
            val new = BooleanArray(n){str[it] == '1'}
            for(i in 0 until n){
                if(i + p !in 0 until n) continue
                if(str[i] == '1'){
                    new[i+p] = true
                }
            }
        ...
        put(got.map { if(it)'1' else '0' }.conca())
