...
array<ll, 2> count_inversions(vi a) {
    // a = compress(a);
    // int n = sz(a);

    // ll invs = 0;
    // for (int i = 0; i < n; i++) {
    //     for (int x = a[i]+1; x > 0; x -= x&-x) invs += tree[x];
    //     for (int x = a[i]+1; x <= n; x += x&-x) tree[x]++;
    // }
    ...
    for (int b = 29; b >= 0; b--) {
        ...
        for (vi& a : arrs) {
            ...
            for (int i = 0; i < sz(a); i++) {
                ...
                if (a[i] & (1<<b)) ones.push_back(a[i]);
                else zeros.push_back(a[i]);
            }

            auto invs = count_inversions(th);
            inv0 += invs[0]; inv1 += invs[1];
            if (!ones.empty()) nxt_arrs.push_back(ones);
            if (!zeros.empty()) nxt_arrs.push_back(zeros);
        }
