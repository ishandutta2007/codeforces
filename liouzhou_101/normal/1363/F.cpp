...
void solve()
{
	...
	sort(s.begin(), s.end());
	sort(t.begin(), t.end());
	if (s != t) return writeln(-1);

	for (int i = 1; i <= n; ++i)
	{
		for (int j = 0; j < 26; ++j) sa[i][j] = sa[i - 1][j], sb[i][j] = sb[i - 1][j];
		sa[i][a[i - 1] - 'a'] ++;
		sb[i][b[i - 1] - 'a'] ++;
	}
	...
	for (int i = 1; i <= n; ++ i)
		...
		{
			f[i][j] = f[i - 1][j] + 1;
			if (a[i - 1] == b[j - 1]) freshmin(f[i][j], f[i - 1][j - 1]);
			int x = b[j - 1] - 'a';
			if (sa[n][x] - sa[i][x] > sb[n][x] - sb[j][x]) freshmin(f[i][j], f[i][j - 1]);
		}
	writeln(f[n][n]);
}
