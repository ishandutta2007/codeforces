...
void solve() {
	...
	sort(ALL(hoge));
	REP(i, n) {
		sums[i + 1] = sums[i] + hoge[i];
	}
	REP(i, m) {
		int a, b;
		cin >> a >> b;
		auto x = lower_bound(ALL(hoge), b);
		int cnter = hoge.end() - x;
		if (cnter < a) {
			cout << 0 << endl;
			continue;
		}
		mint ans = sums[n] - sums[n - cnter];
		ans *= (mint)(cnter - a) / (mint)(cnter);
		ans += sums[n - cnter] * (mint)(cnter - a + 1) / (mint)(cnter + 1);
		cout << ans.val() << endl;
	}
}
#undef int
