...
{
	if(g.empty())return 'N';
	printf("? ");odb(g.size());for(auto x:g)odb(x);puts("");fflush(stdout);
	return re1d();
}
int q(int x)
{
	printf("! ");odl(x);fflush(stdout);
	return re1d();
}
signed main()
{
	...
	// if(x==')')return 0;
	std::vector<int>g;
	rg(n)g.push_back(i);gr
	while(g.size()>3)
	{
		int sz=g.size()/4;std::vector<int>a,b,c,d,e,f;
		for(int i=0;i<sz;i++)a.push_back(g[i]),f.push_back(g[i]),e.push_back(g[i]);
		for(int i=sz;i<sz+sz;i++)b.push_back(g[i]),f.push_back(g[i]);
		for(int i=sz+sz;i<sz+sz+sz;i++)c.push_back(g[i]),e.push_back(g[i]);
		for(int i=sz+sz+sz;i<g.size();i++)d.push_back(g[i]);
		int x=query(f),y=query(e);
		g.clear();
		if(x=='Y'&&y=='Y')
		{
