...
int main() 
{
  ...
  string t; cin>>t;
  dc3.Construct(t);
  string b[2]; cin>>b[0]>>b[1];
	int n=t.length();
  for (int k=0;k<2;k++) {
  	int blen=b[k].length();
  	for (int x=0;x<=n-blen;x++) {
  		//printf("x:%d bklen:%d n:%d\n",x,b[k].length(),n);
  		//assert(x+b[k].length()-1<=n-1);
  		if (t.substr(x,blen) == b[k]) {
  			v[k].PB(x);
  		}
  	}
  }
  vi revsa=dc3.GetREVSA();
  vector<uint64_t> res;
  for (auto &_r:v[1]) {
  	int r=_r+b[1].length()-1;
  	for (auto &l:v[0]) {
  		if (l>r) break;
  		//printf("l:%d r:%d\n",l,r);
  		int len=r-l+1;
  		if (len>=(int)max(b[0].length(),b[1].length()))
  		c2sa[r-l+1].PB(revsa[l]);
  	}
  }
  int ans=0;
  for (int len=1;len<=n;len++) {
  	sort(c2sa[len].begin(),c2sa[len].end());
  	int prev=-1;
  	for (auto &now:c2sa[len]) {
  		if (prev!=-1 && dc3.queryLCPsaidx(now,prev)>=len) {
  			continue;
  		}
  		else {
  			prev=now;
