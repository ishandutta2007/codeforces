...
void check(string s)
{
    for (int i = 0; i < m; i++)
    {
        if (s.substr(0, 2) == v[i].first)
        {
            w[s] = w[s] ||
                   w[v[i].second + s.substr(2, s.length() - 2)];
            //out.putLn(s, ' ', v[i].second, ' ', s.substr(2, v[i].first.substr().length() - 2));
        }
    }
    if (w[s]) cnt++;
}

void gen(string s, int rng)
{
    if (s.length() == rng) { check(s); return; }
    for (char c = 'a'; c <= 'f'; c++)
        gen(s + c, rng);
}
