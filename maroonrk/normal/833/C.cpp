...
signed main(){
	...
	
	{
		...
		eq[0][0]=1;
		rep(_,len){
			rep(i,9)rep(j,2)eq[i+1][j]+=eq[i][j];
			rng(i,lw[0]+1,up[0]){
				eq[i][1]+=eq[i][0];
				eq[i][0]=0;
			}
	...
	dp[0][0][0][0][1][1]=1;
	rep(i,len){
		rep(last,10)rep(ru,2)rep(lu,2)rep(rcmp,3)rep(lcmp,2){
			int w=dp[i][last][ru][lu][rcmp][lcmp];
			if(w){
				rng(nx,last,10){
