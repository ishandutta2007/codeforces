...
{
    dp[u][0]=1;
    dp[u][1]=(int)G[u].size()-(fa!=-1);
    for(auto v:G[u])if(v!=fa)
    {
        dfs(v,u);
        ans=max(ans,dp[u][0]+dp[v][1]);
        ans=max(ans,dp[u][1]-1+(fa!=-1)+max(dp[v][1],dp[v][0]));
        dp[u][0]=max(dp[u][0],dp[v][1]+1);
        dp[u][1]=max(dp[u][1],max(dp[v][1],dp[v][0])-1+(int)G[u].size()-(fa!=-1));
    }
    ans=max({ans,dp[u][0],dp[u][1]+(fa!=-1)});
    // dbg(u,dp[u][0],dp[u][1]);
}
int main()
{
