...
int main() {
	...
	int n; cin >> n;
	vector<string> t(n); VI k(n); VVI x(n);
	rep(i, n) {
		cin >> t[i] >> k[i];
		x[i].resize(k[i]);
		rep(j, k[i]) cin >> x[i][j];
	...
	rep(i, 2000050) {
		if (id[i] != -1) {
			rep(j, max(i, nxt), i+t[id[i]].size()) res[j] = t[id[i]][j-i];
			chmax(nxt, i+(int)t[id[i]].size());
		}
	}
	while (res.back() == '$') res.pop_back();
	for (auto& c : res) if (c == '$') c = 'a';
	cout << res << newl;
}