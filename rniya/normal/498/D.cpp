...
int main() {
    ...
    vector<int> id(MAX, 0);
    SegmentTree<vector<int>> seg(n, f, id);
    vector<vector<int>> init(7, vector<int>(MAX));
    for (int i = 2; i <= 6; i++) {
        for (int j = 0; j < MAX; j++) {
            init[i][j] = 1 + (j % i == 0);
        }
    }
    for (int i = 0; i < n; i++) seg.update(i, init[a[i]]);

    auto update = [&](int x, int y) { seg.update(x, init[y]); };
    auto query = [&](int x, int y) { return seg.query(x, y)[0]; };
