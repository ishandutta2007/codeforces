...
int op(int x, int y) { return min(x, y); }
  ...
  while(tt--) {
    ...
    string s, t;
    cin >> s >> t;
    VI match_front(n + 1), match_back(n + 1);
    {
      int j = 0;
      rep(i, n) {
        if (j < m && s[i] == t[j]) j++;
    ...
    VI sa_inv(n + m + 2);
    rep(i, n + m + 2) sa_inv[sa[i]] = i;
    DisjointSparseTable<int, op> dst(move(lcp_array(st, sa)));
    auto get_lcp = [&](int i, int j) {
      int pi = sa_inv[i], pj = sa_inv[n + 1 + j];
      if (pi > pj) swap(pi, pj);
    ...
    int ans = INF;
    rep(i, n + 1) rep(j, m + 1) {
      if (match_front[i] < j || j < match_back[i]) continue;
      int len = get_lcp(i, j);
      int cnt = n - m + m - (j + len) + i + (i != 0);
      chmin(ans, cnt);
    }
    cout << ans << '\n';
  }
