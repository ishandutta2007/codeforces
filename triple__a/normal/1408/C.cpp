...
signed main(){
    ...
    while (t--){
        ...
        tt1[0]=tt1[n+1]=0;
        tt2[0]=tt2[n+1]=0;
        rep1(i,n+1) tt1[i]=tt1[i-1]+1.0*(a[i]-a[i-1])/i;
        for (int i=n;i>-1;--i) tt2[i]=tt2[i+1]+1.0*(a[i+1]-a[i])/(n+1-i);
        int idx=0;
//        rep(i,n+2) cout<<tt1[i]<<" "<<tt2[i]<<endl;
        rep(i,n+2) if (tt1[i]>tt2[i]) {idx=i; break;}
 //       cout<<idx<<endl;
        if (tt2[idx]>tt1[idx-1]) cout<<tt2[idx]+1.0*(a[idx]-a[idx-1]-(tt2[idx]-tt1[idx-1])*idx)/(n+2)<<"\n";
        else cout<<tt1[idx-1]+1.0*(a[idx]-a[idx-1]-(tt1[idx-1]-tt2[idx])*(n+2-idx))/(n+2)<<"\n";
    }   
}