...
int main() {
  int n;
  scanf("%d", &n);
  std::vector<ModT> ten(n + 1);
  ten[0] = ModT{1};
  for (int i = 1; i <= n; ++i) {
    ten[i] = ModT{10} * ten[i - 1];
  }
  std::vector<ModT> exact(n + 1);
  ModT sum0{0}, sum1{0};
  for (int i = n; i >= 1; --i) {
    exact[i] = ModT(n - i + 1) * ten[n - i + 1] - sum1 + ModT(i - 1) * sum0;
    // for (int j = n; j > i; --j) {
    //   exact[i] -= exact[j] * ModT(j - i + 1);
    // }
    sum0 += exact[i];
    sum1 += exact[i] * ModT(i);
  }
  for (int i = 1; i <= n; ++i) {
    printf("%d%c", exact[i].get(), " \n"[i == n]);
  }
}