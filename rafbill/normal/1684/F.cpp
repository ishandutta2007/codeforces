...
void solve(){
  ...
  { vi op(n);
    ...
    multiset<li> opens;
    for(li i = 0; i < (li)(n); ++i) {
      for(li k = 0; k < (li)(op[i]); ++k) opens.insert(i);

      if(opens.empty()) seen[A[i]].clear();
      while(!seen[A[i]].empty() && !opens.empty() && *seen[A[i]].begin() < *opens.begin()) {
        seen[A[i]].erase(seen[A[i]].begin());
      }

      seen[A[i]].insert(i);
      if(seen[A[i]].size() >= 2) {
        auto it = seen[A[i]].begin();
        li j = *it;
        it++;
        li k = *it;
        auto rit = seen[A[i]].rbegin();
        rit++;
        li l = *rit;
        smax(minst,l);
        pairs.emplace_back(j,k,i);
      }

      for(li l : cl[i]) {
        opens.erase(opens.find(l));
      }
    }
  }
  ...
  for(auto [x,y,z] : pairs) {
    op[x]++;
    at[z].emplace_back(x,y);
  }
  multiset<li> S;
  li ans = n;
  for(li i = 0; i < (li)(n); ++i) {
    for(li k = 0; k < (li)(op[i]); ++k) S.insert(i);
    for(auto [l1,l2] : at[i]) { S.erase(S.find(l1)); S.insert(l2); }
    if(i >= minst && (li)S.size() == (li)pairs.size()) {
      smin(ans, i - *S.begin() + 1);
    }
  }
  cout << ans << endl;
}
