...
int main() {
    STR(s, t);
    RollingHash rh(s), T(t);
    int n = si(s), m = si(t);
    vi v(n + 1);
    rep(i, n - m + 1) if(rh.get(i, i + m) == T.get(0, m)) v[i + 1] = 1;
    rep(i, n) v[i + 1] += v[i];
    RAT<mint> seg(n + 1);
    seg.update(0, 1, 1);
    rep(i, n) {
        seg.update(i + 1, i + 2, seg[i]);
        if(i + m <= n) {
            if(v[n] - v[i] == 0) continue;
            int ok = n, ng = i;
            while(ok - ng > 1) {
                int mid = ok + ng >> 1;
                (v[mid] - v[i] ? ok : ng) = mid;
