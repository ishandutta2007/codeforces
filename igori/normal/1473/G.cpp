...
signed main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        vector<int> dp2(dp.size() + a[i] - b[i]);
        int Go = a[i] + b[i];
        vector<int> ways(Go + 1, 0);
        int x = dp.size();
        for (int j = 0; j <= Go; j++)
        {
            if (j >= Go / 2 - 5 - x && j <= Go / 2 + 5 + x)
            {
                ways[j] = cnk(Go, j);
            }
        }
        ...
        for (int j = Go; j >= 0; j--)
            if (ways[j] == 0)
                ...
                break;
        vector<int> ways_mini(ways.size() - fr - bk);
        for (int j = fr, k = 0; j < ways.size() - bk; j++, k++)
            ways_mini[k] = ways[j];
        vector<int> s0 = F.fft(dp, ways_mini);
        vector<int> S(s0.size() + fr + bk);
        for (int j = 0, k = fr; j < s0.size(); j++, k++)
            S[k] = s0[j];
        for (int j = b[i], k = 0; j < S.size() - b[i]; j++, k++)
            dp2[k] = S[j];
        dp = dp2;
    }
