...
void unite(int a, int b) {
    ...
    for (int v : components[a]) {
        assert(parents[v] == a);
        parents[v] = b;
        components[b].push_back(v);
    }
...
int init() {
    parents[sz] = sz;
    components[sz].push_back(sz);
    return sz++;
}

int getId(int a, int c) {
    int &x = p2id[{a, c}];
    if (x) return x;
    x = init();
...
void addEdge(int a, int b, int c) {
    int ac = getId(a, c);
    int bc = getId(b, c);
    cerr << "addEdge " << a << " " << b << " " << c << " " << ac << " " << bc << endl;

    unite(a, bc);
    unite(b, ac);

    aux[parents[b]].insert(a);
    aux[parents[a]].insert(b);
