...
int main(void)
{
	...
	ll sum = 0;
	rep(i, 1, n) sum += a[i], b[i] -= a[i];
	sort(b+1, b+n+1), reverse(b+1, b+n+1);
	rep(i, 1, n) s[i] = s[i-1] + b[i];
	
	ll p = 0;
	rep(i, 1, n) if(b[i] > 0) p = i;
	...
	rep(i, 1, Q){
		cin >> x >> y;
		P res = congruence(y, n, x);
		if(res.se == -1){
			cout << -1 << "\n";
			continue;
		}
		ll b = res.fi, m = res.se;
		ll l = floor(p-b*y, m*y), r = l+1;
		l = y*(l*m+b), r = y*(r*m+b);
		...
		ll ans = -1;
		if(l >= 0 && l <= n) chmax(ans, sum + s[l]);
		if(r >= 0 && r <= n) chmax(ans, sum + s[r]);
		cout << ans << "\n";
	}
