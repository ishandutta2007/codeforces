...
int main() {
	int n,m; R(n,m);
	vector<vector<ll>> cannon(n, vector<ll>(n, 1e14));
	for (int i = 0; i < m; i++) {
		int u,v,c; R(u,v,c);
		cannon[u][v] = c;
	}
	...
	for (int u = 0; u < n; u++) {
		for (int v = 0; v < 3 * n; v++) {
			chkmin(cannon[u][v % n], cannon[u][(v + n - 1) % n] + 1);
		}
	}
	
	vector<vector<ll>> ans(n, vector<ll>(n, 1e16));
	for (int u = 0; u < n; u++) {
		...
		for (int t = 0; t < n; t++) {
			int now = -1;
			for (int v = 0; v < n; v++) {
				if (!done[v] && (now == -1 || ans[u][v] < ans[u][now]))
					now = v;
			}
			...
			for (int v = 0; v < n; v++) {
				ll shift = (v - ans[u][now]) % n;
				if (shift < 0) shift += n;
				chkmin(ans[u][v], ans[u][now] + cannon[now][shift]);
			}
		}
