...
fun ok(str:String, ispatternOf:String):Boolean {
    for(i in 0 until 2) {
        if(ispatternOf[i] == 'B' && str[i] != 'B') {
            return false
        }else if(ispatternOf[i] == 'W' && str[i] != 'W'){
            return false
        }
    }
    return true
...
fun main(){
    ...
    solve.cases{
        ...
        var ok = 0
        ok += all.all { ok("BW",it) }.chi
        ok += all.all { ok("WB",it) }.chi
        put(ok modPlus base modMinus cn)
