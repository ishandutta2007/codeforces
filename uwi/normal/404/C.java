...
public class C {
	...
	void solve()
	{
		...
		for(int i = 0;i < n;i++){
			if(i > 0){
				if(b[i+1].length > (long)b[i].length * (K-1)){
					out.println(-1);
					return;
				}
			}else{
				if(b[i+1].length > (long)b[i].length * K){
					out.println(-1);
					return;
				}
			}
		...
		out.println(n-1);
		for(int i = 0;i < n;i++){
			for(int p = 0, pm = 0, q = 0;p < b[i+1].length;p++, pm++){
				if(pm == (i == 0 ? K : K-1)){
					pm = 0;
					q++;
				}
				out.println((b[i][q]+1) + " " + (b[i+1][p]+1));
			}
		}
	}
