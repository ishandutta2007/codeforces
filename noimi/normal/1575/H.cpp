...
int main(){
  ...
  int sz=tri.data.size();
  const int inf=1e9;
  rep(i,0,n+1)rep(j,0,n+1)rep(o,0,sz)dp[i][j][o]=inf;
  dp[0][0][0]=0;
  rep(i,0,n){
    rep(j,0,n+1){
      rep(o,0,sz){
        rep(k,0,2){
          int to=tri.data[o].nxt[k];
          chmin(dp[i+1][j+(to==state)][to],dp[i][j][o]+(a[i]-'0'!=k));
        }
      }
    }
  }
  vector<int>ret(n-m+2,inf);
  rep(i,0,n-m+2){
    rep(j,0,sz)chmin(ret[i],dp[n][i][j]);
