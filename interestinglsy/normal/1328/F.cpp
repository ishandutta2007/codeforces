...
int main(){
	...
	ll leCnt = 0; ll leSum = 0;
	ll geCnt = n; ll geSum = 0;
	for( auto [num,cnt] : counter ) geSum += 1LL*num*cnt;
	...
	for( auto [num,cnt] : counter ){
		...
		int k = ::k - cnt;
		auto GetCost = [&]( ll cnt , ll sum , int need ){
			return abs( cnt*num - sum ) - (cnt-need);
		};

		ll tans = LINF;
		if( leCnt >= k ){
			Mymin(tans,GetCost(leCnt,leSum,k));
		}
		if( geCnt >= k ){
			Mymin(tans,GetCost(geCnt,geSum,k));
		}
		if( leCnt < k ){
			Mymin(tans,GetCost(leCnt,leSum,leCnt)+GetCost(geCnt,geSum,k-leCnt));
		}
		if( geCnt < k ){
			Mymin(tans,GetCost(geCnt,geSum,geCnt)+GetCost(leCnt,leSum,k-geCnt));
		}
		MyMin(ans,tans);
		// P(num,tans);
