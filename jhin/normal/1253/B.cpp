                   ...
                   int m,x,n,y,z,x1,k,one,gd,x2,y2,two,mn=1e9,mid,l,r,pos,a[500003],b[500003],c[500003],d[500003];vector<int>v,vr,v2[100003];long long sum,ans,num;char ch;
                    ...
                    int main()
                    {
                        ...
                        ios_base::sync_with_stdio(false);
                         ...
                         set<int>tmpset,usedset;
                        for(int i=0;i<n;i++)
                        {
                           if(a[i]>0)
                           {
                               auto it2=usedset.lower_bound(a[i]);
                               if(*it2==a[i])return cout<<-1,0;
                           auto it= tmpset.lower_bound(a[i]);
                           if(*it==a[i])return cout<<-1,0;
                           else tmpset.insert(a[i]),cnt++,usedset.insert(a[i]);
                           }
                           if(a[i]<0)
                           {
                               auto it= tmpset.lower_bound(abs(a[i]));
                           if(*it==abs(a[i]))
                           {
                               cnt++;
                               tmpset.erase(it);
                           ...
                           if(tmpset.empty()){v.push_back(cnt),cnt=0;x+=cnt;usedset.clear();}
                         }
                         if(x!=cnt)return cout<<-1,0;
                         cout<<v.size()<<endl;
