...
int main(){
    ...
    auto g1=[](int a,int b){return (b!=-1?b:a);};
    auto h=[](int a,int b){return b;};

    struct node{
        ll a; int b;
        node(ll a ,int b):a(a),b(b){}
    };
    auto f2=[](node a,node b){return node(a.a+b.a,a.b+b.b);};
    auto g2=[](node a,int b){return (b!=-1?node((ll)a.b*b,a.b):a);};

    LazySegmentTree<int,int> seg_val(n,f1,g1,h,INF,-1);
    LazySegmentTree<node,int> seg_sum(n,f2,g2,h,node(0,0),-1);
    vector<int> init1;
    vector<node> init2;
    for (int i=0;i<n;++i){
        init1.emplace_back(a[i]);
        init2.emplace_back(a[i],1);
    }
    seg_val.build(init1);
    seg_sum.build(init2);

    auto update=[&](int x,int y){
        int pos=seg_val.find_first(0,[&](int a){return a<y;});
        if (pos<0) return;
        seg_val.update(pos,x+1,y);
        seg_sum.update(pos,x+1,y);
    };
    auto query=[&](int x,int y){
        int res=0;
        for (int cur=x;cur<n;){
            int pos=seg_val.find_first(cur,[&](int a){return a<=y;});
            if (pos<0) break;
            int nxt=seg_sum.find_first(pos,[&](node a){return a.a>y;});
            if (nxt<0) nxt=n;
            res+=nxt-pos;
            y-=seg_sum.query(pos,nxt).a;
            cur=nxt;
        }
        return res;
    };
