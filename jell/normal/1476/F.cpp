#line 1 "codeforces-workspace/contest/1476/f/f.cpp"
...
#line 12 "codeforces-workspace/contest/1476/f/f.cpp"
...
void main() {
  ...
  for (auto i : range(1, n)) {
    pair<int, int> ch1{-1, -1}, ch2{-1, -1};

    while (!s.empty()) {
      bool pop = false;
      auto [x, y, z] = s.top();
      if (y <= i + p[i]) {
        if (0 > get<0>(ch1)) ch1 = {x, z};
        pop = true;
      }
      if (x >= i - p[i]) {
        pop = true;
    ...
    tuple tp1{-1, -1, -1}, tp2{tp1};
    if (~ch1.first) {  // R
      auto [x, z] = ch1;
      if (x > i) chge(x, i + p[i] + 1);
      tp1 = {x, i + p[i], z};
    }
    if (~ch2.first) {  // L
      auto [y, z] = ch2;
      tp2 = {max(i, y + 1), max(y, i - 1), i};
      bef[i] = z;
    }

    if (auto v1 = get<0>(tp1); ~v1) {
      if (auto v2 = get<0>(tp2); ~v2) {
        if (v1 > v2) {
          tp1.swap(tp2);
        }
        v1 = get<1>(tp1);
        v2 = get<1>(tp2);
        if (v1 > v2) s.emplace(tp1);
        s.emplace(tp2);
      } else
        s.emplace(tp1);
    } else {
