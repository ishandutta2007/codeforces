#line 1 "/Users/nok0/Documents/Programming/nok0/atcoder/string.hpp"
...
void main_() {
	INT(n);
	STR(s);
	vector ok_len(n, vector(n, inf));
	//ok_len[i][j] : s[i, i + len) < s[j, j + len) 
	VV<> zs(n);
	REP(i, n) {
		string t = s.substr(i);
	...
	REP(i, n) {
		REP(j, i + 1, n) {
			int len = zs[i][j - i];
			if(j + len == n) continue;
			if(s[i + len] < s[j + len]) ok_len[i][j] = len + 1;
		}
	}

	vector dp(n, vector(n + 1, mint(0)));
	vector imos(n, vector(n + 2, mint(0)));
	REP(i, n) {
		dp[0][i + 1] = 1;
	}
