...
fn rec(n: usize, x: usize, mut b: Vec<usize>, a: &mut Vec<Vec<usize>>) {
	if n == 1 {
		b.reverse();
		a.push(b);
	} else if n < x {
...
fn main() {
	...
	let n = rin.u();
	let p = &[2,3,5,7,11,13,17,19,23];
	let mut pp = v!([9][n]);
	for i in 0..9 {
		let mut z = 1_i64;
		pp[i].push(z);
		rep!{[n-1]
			z = z.saturating_mul(p[i]);
			pp[i].push(z);
		}
	}
	let mut a = v!([]);
	rec(n, 2, v!([]), &mut a);
	let mut sol = std::i64::MAX;
	a.sort();
	a.dedup();
	for b in a {
		let mut res = 1_i64;
		for i in 0..b.len() {
			res = res.saturating_mul(pp[i][b[i]]);
		}
		min!(sol, res);
	}
	writeln!(rout, "{}", sol).ok();
