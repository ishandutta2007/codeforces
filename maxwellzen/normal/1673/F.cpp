...
void setvals(int x1, int x2, int y1, int y2, int l, int r) {
    if (r-l==1) {
        vals[x1][y1] = l;
        return;
    }
    ...
    if (x2-x1 > y2-y1) {
        int xm = (x1+x2)/2;
        setvals(x1, xm, y1, y2, l, m);
        FOR(x, xm, x2) FOR(y, y1, y2) vals[x][y] = vals[2*xm-1-x][y] + bit;
    } else {
        int ym = (y1+y2)/2;
        setvals(x1, x2, y1, ym, l, m);
        FOR(x, x1, x2) FOR(y, ym, y2) vals[x][y] = vals[x][2*ym-1-y] + bit;
    }
}

void solve() {
    cin >> n >> k;
    setvals(0, 32, 0, 32, 0, 1024);
    // F0R(i, n) {F0R(j, n) cout << vals[i][j] << ' '; cout << nl;}
    F0R(i, n) {
        F0R(j, n-1) cout << (vals[i][j]^vals[i][j+1]) << ' ';
        cout << endl;
    }
    F0R(i, n-1) {
        F0R(j, n) cout << (vals[i][j]^vals[i+1][j]) << ' ';
        cout << endl;
    }
    F0R(i, n) F0R(j, n) loc[vals[i][j]] = mp(i, j);
    int pre = vals[0][0];
    while (k--) {
        int cur; cin >> cur;
        pre = pre^cur;
        cout << loc[pre].F+1 << ' ' << loc[pre].S+1 << endl;
    }
}
