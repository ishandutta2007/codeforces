...
inline void AC() {
    ...
    for(_ j = 1; j <= m; ++j) {
        for(_ i = 1; i <= n; ++i) {
            if(a[i][j] == 'X') c[i][j] = false;
            else c[i][j] = c[i-1][j] || c[i][j-1] || !(i-1) || !(j-1);
        }
        for(_ i = 1; i <= n; ++i) if(a[i][j] == '.' && !c[i][j]) p = j;
        if(p != j) for(_ i = 1; i <= n; ++i) if(!c[i][j] && !c[i-1][j] && !c[i][j-1] && i != 1 && j != 1) p = j;
        if(p == j) for(_ i = 1; i <= n; ++i) c[i][j] = a[i][j] == '.';
        b[j] = p;
    }
    k = read();
    for(; k--; ) {
