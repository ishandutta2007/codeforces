...
void solve() {
  ...
  FOR(i, 4) A.eb(A[i]);
  FOR(i, 4) {
    if (A[i][0] && A[i][3]) return YES();
    if (A[i][1] && A[i][3]) return YES();
    if (A[i][2] && A[i][3]) return YES();
    if (A[i + 1][0] && A[i][3]) return YES();
    if (A[i + 2][1] && A[i][3]) return YES();
    if (A[i + 3][2] && A[i][3]) return YES();
  }
  NO();
}
