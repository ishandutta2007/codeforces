...
void solve() {
    int n;
    cin >> n;
    map<int, map<int, vector<pair<int, int>>>> a;
    for (int i = 0; i < n; i++) {
        int x, y, z;
        cin >> x >> y >> z;
        a[x][y].emplace_back(z, i);
    }
    ...
    for (auto& xit : a) {
        ...
        for (auto& yit : xit.second) {
            ...
            for (int i = 0; i + 1 < isz(zs); i += 2) {
                ans.emplace_back(zs[i].second, zs[i + 1].second);
            }
            if (isz(zs) % 2) {
                l.emplace_back(zs.back().second);
            }
        }
        for (int i = 0; i + 1 < isz(l); i += 2) {
            ans.emplace_back(l[i], l[i + 1]);
        }
        if (isz(l) % 2) {
            g.push_back(l.back());
        }
    ...
    for (int i = 0; i + 1 < isz(g); i += 2) {
        ans.emplace_back(g[i], g[i + 1]);
    }
