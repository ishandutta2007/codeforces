...
int32_t main() {
	scanf("%d%d",&n,&T);
	for(int i=1;i<=n;i++) scanf("%d%d",a+i,t+i), ts[a[i]].pb(i);


	int Z=-1;
	priority_queue<int> bel;
	priority_queue<int,vector<int>,greater<int> > abo;
	int s=0,tt=0;
	for(int k=n;k>=1;k--) {
		for(int j:ts[k]) abo.push(t[j]);
		for(;!abo.empty() && !bel.empty();) {
			if(bel.top() <= abo.top()) break;
			int x=bel.top();
			int y=abo.top();
			tt -= x;
			tt += y;
	...
	for(int i=n;i>=Z;i--) for(int j:ts[i]) ans.pb({t[j],j});
	sort(ans.begin(),ans.end());
	for(int i=0;i<Z;i++) printf("%d%c",ans[i].se," \n"[i+1==Z]);
	if(!Z) printf("\n");
}