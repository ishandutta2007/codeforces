...
public class CFContest {
    ...
    public static class Task implements Runnable {
        ...
        public void publish(int i, int j) {
            if (i == j) {
                return;
            }
            ops.add(new OP(i, j));
        ...
        public void solve() {
            int n = io.readInt();
            List<AB> list1 = new ArrayList<>(n);
            List<AB> list2 = new ArrayList<>(n);
            for (int i = 1; i <= n; i++) {
                int a = io.readInt();
                int b = io.readInt();
                if (a == b) {
                ...
                if (a < b) {
                    list1.add(new AB(a, b, i));
                } else {
                    list2.add(new AB(a, b, i));
                }
            }

            list1.sort((a, b) -> -(a.a - b.a));
            list2.sort(((a, b) -> a.b - b.b));
            List<AB> longest = list1.size() <= list2.size() ? list2 : list1;
            io.cache.append(longest.size()).append('\n');
            for (AB ab : longest) {
                io.cache.append(ab.index).append(' ');
            }
        }
    }
    ...
    public static class AB {
        ...
        public AB(int a, int b, int index) {
            this.a = a;
            this.b = b;
            this.index = index;
        }
