...
int main(){
    ...
    for (int i=0;i<n;++i) cin >> p[i],r[--p[i]]=i+1;
    for (int i=0;i<n;++i) cin >> q[i];
    auto f=[](int a,int b){return min(a,b);};
    auto g=[](int a,int b){return a+b;};
    LazySegmentTree<int,int> seg(n+1,f,g,g,INF,0);
    for (int i=0;i<n+1;++i) seg.set_val(i,0);
    vector<int> ans(n,0);
    int now=0;
    for (int i=n-1;i>=0;--i){
        seg.update(r[i],n+1,1);
        while(now<n&&seg[n]>seg.query(0,n+1)){
            seg.update(q[now++],n+1,-1);
        }
        ans[now-1]=max(ans[now-1],i+1);
    }
