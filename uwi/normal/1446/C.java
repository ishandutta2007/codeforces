...
public class C {
	...
	void solve()
	{
		...
		out.println(n - trie.solve(1, trie.W-1));
	}
	...
	public static class TrieArrayBinary {
		...
		public int solve(int cur, int d)
		{
			if(d == 0)return 1;
			int L = next.a[cur*2+0];
			int R = next.a[cur*2+1];
//			tr(cur, d, L, R);
			if(L == -1 && R == -1)return 0;
			if(L == -1)return solve(R, d-1);
			if(R == -1)return solve(L, d-1);
			int resl = solve(L, d-1);
			int resr = solve(R, d-1);
//			tr(resl, resr);
			if(resl == 0 || resr == 0)return resl + resr;
//			tr(cur, d, Math.max(resl, resr) + 1);
