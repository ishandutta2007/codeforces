...
{
	if(u.x < v.x - eps) return true;
	if(u.x > v.x + eps) return false;
	return u.y < v.y - eps;
}
...
bool check(double t)
{
	...
	rep(i, 1, sz(cvec)-1){
		if(isInTriangle(vec2d(0, 0), cvec[i-1]*t, cvec[i]*t, vec2d(X, Y))) return true;
	}
	return false;
}
...
int main(void)
{
	...
	cin >> n >> X >> Y;
	ll x, y;
	rep(i, 1, n) cin >> x >> y, vec.push_back(vec2d(x, y));
	...
	if(sz(cvec) == 1){
		outf(max(X/cvec[0].x, Y/cvec[0].y));
		return 0;
	}
	
	double ub = 2e6, lb = 0, mid;
	rep(i, 1, 100){
		mid = (ub+lb)/2;
		if(check(mid)) ub = mid;
		else lb = mid;
