...
vector<int> a[1505];
vector<int> b[1505];
bool goodPair[1505][1505];
int cntBad[1505];
auto getCmp(int j) {
    return [j](const vector<int>& a, const vector<int>& b) {
        return a[j] < b[j];
    };
}
vector<int> whereBad[1505];
int solve() {
    int n, m;
    n = readInt();
    m = readInt();
    ...
    while (true) {
        ...
        ans.push_back(res);
        for (int i = 0; i < n - 1; ++i) {
            if (b[i][res] < b[i + 1][res] && whereBad[i].size() > 0) {
                for (int c : whereBad[i]) {
                    cntBad[c]--;
                }
                whereBad[i].clear();
            }
    ...
    reverse(all(ans));
    for (int x : ans) {
        std::stable_sort(a, a + n, getCmp(x));
    }
    FI(n) {
        if (a[i] != b[i]) {
            cout << -1 << endl;
