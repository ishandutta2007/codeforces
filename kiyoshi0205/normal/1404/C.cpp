...
int main(){
  ...
  ll i,j=0;
  cin>>N>>K;
  segtree<int> tree(N+1,0,[](int a,int b){return a+b;});
  vector<int> a(N),ans(K);
  vector<pair<pair<int,int>,int>> lis(K);
  rep(i,N){cin>>a[i];--a[i];}
  rep(i,K){
    cin>>lis[i].first.second>>lis[i].first.first;
    lis[i].first.first=N-lis[i].first.first;
    lis[i].second=i;
  }
  // print("");
  vsort(lis);
  lis.eb(pair<int,int>(-1,0),0);
  rep(i,N){
    while(i==lis[j].first.first){
      ans[lis[j].second]=tree.queli(lis[j].first.second,N+1);
      ++j;
    }
    if(a[i]>i)continue;
    if(a[i]==i){
      ...
      if(tree.queli(0,N+1)+a[i]<i)continue;
      int ok=0,ng=i,mid;
      while(ng-ok>1){
        mid=(ok+ng)>>1;
        if(tree.queli(mid,N+1)+a[i]<i)ng=mid;
        else ok=mid;
      }
      // print(i<<" "<<ok);
      tree.action(ok,1);
