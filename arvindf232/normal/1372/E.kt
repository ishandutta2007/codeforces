...
fun main(){
    solve.cases{
        ...
        //l , r, which point
        val have = Array(m){Array(m){IntArray(m)} }
        val squares = IntArray(n+1){it * it }
        repeat(n){
            val k = getint
            repeat(k){
                val a = getint - 1
                val b = getint -1
                for(left in 0..a){
                    for(right in b until m){
                        for(actual in a..b){
                            have[left][right][actual] ++
                        }
                    }
                }
            }
        ...
        for(len in 0 until m){
            for(l in 0 until m){
                val r = l + len
                if(r !in 0 until m) continue
                for(m in l..r){
                    val left = if(l <= m-1) DP[l][m-1] else 0
                    val right = if(m+1 <= r) DP[m+1][r] else 0
                    val actual = squares[have[l][r][m]]
                    val total = actual + left + right
                    DP[l][r] = maxOf(DP[l][r], total )
                }
            }
        }
        put(DP[0][m-1])
...
flow, must be greedily improve,
otherwise cannot flow
