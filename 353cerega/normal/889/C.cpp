...
int main()
{
    ...
    for (ll s=k+1;s<=n;s++)
    {
        cnt[s] = (cnt[s-1]*s+(f[s-k-1]-cnt[s-k-1])*f[s-1]%mod*obrf[s-1-k])%mod;
        if (cnt[s]<0) cnt[s] += mod;
    }
    for (ll s=k+1;s<n;s++)
    {
        ans = (ans+f[n-1]*obrf[s]%mod*obrf[n-1-s]%mod*f[n-s]%mod*f[s-1]%mod*(f[s-k-1]-cnt[s-k-1])%mod*obrf[s-k-1]%mod)%mod;
    }
    if (ans<0) ans += mod;
    cout << ans << endl;
}