...
//	cout<<n;edl;
	...
	if(x==fib[n])return mkp((n+1)/2,0);
	if(x<=fib[n-1])
	{
		pii w=f(n-1,x);
		int f1=min(w.se+2,w.fi);int f2=min(w.se+1,w.fi+1);
		f2+=f(n-2,1).se;
		return mkp(f1,f2);
	}
	else
	{
		pii w=f(n-2,x-fib[n-1]);
		return mkp(w.fi+1,w.se);
	}
}
int solve(int n,int a,int b,int l)
{
	if(a==b)return 0;
	if(n==1)return 1;
	//a<b
	...
	{
		pii A=f(n-1,a),B=f(n-2,b-fib[n-1]),C=f(n,a),D=f(n,b);
		return Min(B.fi+1+min(A.fi,A.se),C.fi+D.se+l,D.fi+C.se+l);
	}
	else return solve(n-2,a-fib[n-1],b-fib[n-1],l+1);
}
signed main()
{
	int T=1;
	fib[0]=1;fib[1]=2;
	up(i,2,81)fib[i]=fib[i-1]+fib[i-2];
//	cout<<f(33,89).fi;edl;
	while(T--)
	{
		int q;rd(q);rdn;
		while(q--)
		{
			...
			int nw=0;
			while(fib[nw]<b)++nw;
			int gw=2;//cout<<fib[nw];edl;
			if(nw==n)gw=Min();
			int ans=solve(nw,a,b,gw);
			opl(ans);edl;
		}
	}
}
