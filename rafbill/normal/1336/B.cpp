...
void solve(){
  int a,b,c; cin>>a>>b>>c;
  vi A(a),B(b),C(c);
  for(li i = 0; i < (li)(a); ++i) cin>>A[i];
  for(li i = 0; i < (li)(b); ++i) cin>>B[i];
  for(li i = 0; i < (li)(c); ++i) cin>>C[i];
  sort(begin(A), end(A));
  sort(begin(B), end(B));
  sort(begin(C), end(C));
  li ans = 1ll<<62;
  auto test0 = [&](li x, li y, li z) {
    li w = (x-y)*(x-y) + (x-z)*(x-z) + (y-z)*(y-z);
    ans = min(ans, w);
  };
  auto test = [&](vi const& A, vi const& B, vi const& C) {
    int ia = 0, ib = 0;
    for(li ic = 0; ic < (li)((int)C.size()); ++ic) {
      while(ia+1 < (int)A.size() && A[ia+1] <= C[ic]) ia += 1;
      if(A[ia] > C[ic]) continue;
      li gb = A[ia]+C[ic];
      while(ib+1 < (int)B.size() && 2*B[ib+1] <= gb) ib++;
      test0(A[ia],B[ib],C[ic]);
      if(ib+1 < (int)B.size()) test0(A[ia],B[ib+1],C[ic]);
    }
  };
  test(A,B,C);
  test(A,C,B);
