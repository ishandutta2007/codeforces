...
const ll N=100100;
ll head[N],nxt[N],vet[N],dfn[N],n,m,tot,x[N],y[N],q[N];
...
int main(){
	...
	For(x,1,n){
		...
		For(i,1,m)	++dfn[y[i]];
		--dfn[x];	ll he=0,ta=0;
		For(i,1,n)if (!dfn[i])	q[++ta]=i;
		while(he^ta)	for(ll i=head[q[++he]];i;i=nxt[i])	if (!--dfn[vet[i]])	q[++ta]=vet[i];
		if (ta==n)	return puts("YES"),0;
	}puts("NO");
}