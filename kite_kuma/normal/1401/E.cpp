...
int main() {
	...
	// pqdn<t> yoko;
	// pqdn<t> yoko2;
	// V<t> tateshita, tateue;

	rep(i, n) {
		ll y, lx, rx;
		cin >> y >> lx >> rx;
		swp[lx].push_back({y, 1});
		swp[rx + 1].push_back({y, -1});
		// t tup = {y, {lx, rx}};
		// yoko.push(tup);
		// tup.first = h - y;
		// yoko2.push(tup);
	...
	rep(i, h) {
		for(auto tup : swp[i]) {
			//debug(tup);
			bit.add(tup.first, tup.second);
		}

		if(it < sz && tatesen[it].first == i) {
			//debug(i);
			//debug(it);
			ans += bit.sum(tatesen[it].second.first, tatesen[it].second.second + 1);
			it++;
		}
	}
