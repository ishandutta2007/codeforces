...
int main()
{
    ...
    vector<pair<int, int>> meet, notmeet;

    while (m--)
    {
        int tp, u, v; cin>>tp>>u>>v; u--; v--;
        G[u].push_back(v); G[v].push_back(u);
        if (tp==1)
        {
            notmeet.push_back(mp(u, v));
        }
        else
        {
            meet.push_back(mp(u, v));
        }
    }
    ...
    for (int i = 0; i<n; i++)
        for (auto it: G[i])
        {
            if (pos[i]>pos[it]) {cout<<"NO"<<endl; return 0;}
        }

    cout<<"YES"<<endl;
    for (int i = 0; i<n; i++)
    {
        if (color[i]==0) cout<<"R "; else cout<<"L ";
        cout<<pos[i]<<endl;
    }
}