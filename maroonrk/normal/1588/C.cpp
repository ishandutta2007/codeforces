...
void slv(){
	...
	vi even(n+1,-inf),odd(n+1,inf);
	rep(i,n+1)if(i%2==0)even[i]=b[i];
	else odd[i]=b[i];
	segtree<MinNode> minseg(odd);
	segtree<MaxNode> maxseg(even);
	vi vs=b;mkuni(vs);
	vvc<int> ls(si(vs));
	int ans=0;
	rep(i,n+1){
		int j=max(
			minseg.min_left(i+1,&MinNode::ok,b[i]).a,
			maxseg.min_left(i+1,&MaxNode::ok,b[i]).a
		);
		int tar=lwb(vs,b[i]);
		ans+=si(ls[tar])-lwb(ls[tar],j);
		ls[tar].pb(i);
	}
	print(ans);
}
