...
void solve(int a, int b) {
    ...
    for (int ph = 0; ph < 2; ph++) {
        ...
        int lx = max(0, a - min(a, c));
        int ux = max(a, min(a, d));
        int ly = max(0, b - min(b, d));
        int uy = max(b, min(b, c));

        int lxy = a - c;
        int uxy = d - b;

        dbg(a, b, c, d, lx, ux, ly, uy, lxy, uxy);

        for (int x = lx; x <= ux; x++) {
            // x - y <= uxy
            // x - y >= lxy
            int low = x + max(ly, x - uxy);
            int high = x + min(uy, x - lxy);
