...
int main() {
    ...
    std::map<Point, int, std::function<bool(Point, Point)>> cnt([&](Point lhs, Point rhs) {
        if (sgn(lhs) != sgn(rhs))
            return sgn(lhs) == 0;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (op == 'A') {
            ...
            int64_t x, y;
            if (need[p[0]] > 0) {
                x = 1ll * g[p[0]] * need[p[1]] * need[p[2]] - 1ll * g[p[2]] * need[p[0]] * need[p[1]];
                y = 1ll * g[p[1]] * need[p[0]] * need[p[2]] - 1ll * g[p[2]] * need[p[0]] * need[p[1]];
            } else if (need[p[1]] > 0) {
                x = g[p[0]];
