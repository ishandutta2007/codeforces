...
int main() {
    ...
    while(T--) {
        ...
        X[x3]; X[++x4]; Y[y3]; Y[++y4];
        map<int,int> rx,ry;
        for(auto&t:X) t.se=++ix,rx[ix]=t.fi;
        for(auto&t:Y) t.se=++iy,ry[iy]=t.se;
        for(int i=X[1];i<X[n+1];++i)
            for(int j=Y[1];j<Y[m+1];++j) g[{i,j}]=3;
        for(int i=X[x1];i<X[x2];++i)
            for(int j=Y[y1];j<Y[y2];++j)
                g[{i,j}]=1;
        for(int i=X[x3];i<X[x4];++i)
            for(int j=Y[y3];j<Y[y4];++j)
                g[{i,j}]=2;
        ll black=0;
        for(auto t:g) {
            int xl=rx[t.fi.fi],xr=rx[t.fi.fi+1]-1;
            int yl=ry[t.fi.se],yr=ry[t.fi.se+1]-1;

        }*/
        auto num_white=[&](int x1,int x2,int y1,int y2) {
            ll tc=(ll)(x2-x1+1)*(ll)(y2-y1+1);
            if((x2-x1+1)%2==0||(y2-y1+1)%2==0)
                return tc/2;
            return (tc+((x1+y1)%2==0))/2;
        };
        ll ans=
        num_white(1,m,1,n)+(x2-x1+1)*(ll)(y2-y1+1)-num_white(x1,x2,y1,y2);
        ans-=num_white(x3,x4,y3,y4);
        x3=max(x3,x1);
        y3=max(y3,y1);
        x4=min(x4,x2);
        y4=min(y4,y2);
//        cerr<<x3<<"~"<<x4<<" "<<y3<<"~"<<y4<<" "<<num_white(x3,x4,y3,y4)<<"\n";
        if(x3<=x4&&y3<=y4)
            ans-=(x4-x3+1)*(ll)(y4-y3+1)-num_white(x3,x4,y3,y4);
        printf("%lld %lld\n",ans,n*(ll)m-ans);
    }
	return 0;
}