...
int main() {
    ...
    VEC(ll, a, m);
    ll ans = 0;
    rep(i, m - 1) ans += abs(a[i + 1] - a[i]);
    vv(ll, v, n + 1);
    rep(i, m) {
        if(i and a[i - 1] != a[i]) v[a[i]].eb(a[i - 1]);
        if(i < m - 1 and a[i] != a[i + 1]) v[a[i]].eb(a[i + 1]);
    }
    ll ma = 0;
    rep2(i, 1, n) {
        sort(all(v[i]));
        ll now = 0;
        for(auto e : v[i]) { now -= abs(e - i); }
        int k = si(v[i]) / 2;
        for(auto e : v[i]) now += abs(e - v[i][k]);
        chmax(ma, -now);
    }
    cout << ans - ma << endl;
}