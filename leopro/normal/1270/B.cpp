...
namespace {
    ...
    template<typename S, typename T>ostream &operator<<(ostream &os, const pair<S, T> &p){ os << p.first << (whitespace(p.second) ? " " : "") << p.second; return os;}
...
void solve() {
    ...
    cin >> a;
    for (int i = 1; i < n; ++i) {
        if (abs(a[i - 1] - a[i])>= 2) {cout<<"YES\n";answer(pair<int, int>{i, i+1});}
    }
    answer("NO");
}