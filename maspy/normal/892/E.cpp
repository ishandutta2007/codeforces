...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/pds/rollbackunionfind.hpp"
...
void solve() {
  ...
  FOR(i, M) {
    LL(a, b, c);
    edges.eb(--a, --b, c);
    if (c >= len(EID)) EID.resize(c + 1);
    EID[c].eb(i);
  }
  LL(Q);
  vc<int> ANS(Q, 1);
  // weight -> (qid, edges)
  vc<vc<pair<int, int>>> query(len(EID));
  FOR(q, Q) {
    LL(K);
    FOR_(K) {
      LL(eid);
      --eid;
      auto cost = get<2>(edges[eid]);
      query[cost].eb(q, eid);
    }
  }
  ...
  FOR(sz, len(EID)) {
    int time = uf.time();
    ll prev = -1;
    for (auto&& [qid, eid]: query[sz]) {
      if (prev != qid) { uf.rollback(time); }
      prev = qid;
      auto [a, b, c] = edges[eid];
      if (!uf.merge(a, b)) ANS[qid] = 0;
    }
    uf.rollback(time);
    for (auto&& eid: EID[sz]) {
      auto [a, b, c] = edges[eid];
