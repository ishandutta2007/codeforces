...
ll Solve( Gems &a , Gems &b , Gems &c ){
	ll ans = LINF;
	int pB = 1, pC = 1;
	For(i,a.n){
		while( pB != b.n and b.g[pB+1] <= a.g[i] ) ++pB;
		while( pC != c.n and c.g[pC]   <  a.g[i] ) ++pC;
		ll tans = Sqr(a.g[i]-b.g[pB]) + Sqr(a.g[i]-c.g[pC]) + Sqr(b.g[pB]-c.g[pC]);
		MyMin(ans,tans);
	}
	return ans;
}
...
int main(){
	...
	For(TwT,T){
		...
		Read(b.g,1,b.n);
		Read(c.g,1,c.n);
		sort(a.g+1,a.g+a.n+1);
		sort(b.g+1,b.g+b.n+1);
		sort(c.g+1,c.g+c.n+1);
		// Print(a.g,1,a.n);
		// Print(b.g,1,b.n);
		// Print(c.g,1,c.n);
		ll ans = LINF;
		Mymin( ans , Solve(a,b,c) );
		Mymin( ans , Solve(a,c,b) );
		Mymin( ans , Solve(b,a,c) );
		Mymin( ans , Solve(b,c,a) );
		Mymin( ans , Solve(c,a,b) );
		Mymin( ans , Solve(c,b,a) );
		printf("%lld\n",ans);
	}
