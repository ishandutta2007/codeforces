...
inline ANDREIKKAA_RETURN_TYPE mainFunction(ANDREIKKAA_PARAMETERS)
{
	...
	vec<char> ok = { 'A', 'H', 'I', 'M', 'O', 'o', 'T', 'U', 'V', 'v', 'W', 'w', 'X', 'x', 'Y' };
	vec<pair<char, char>> okk = { {'b', 'd'}, {'p', 'q'} };
	
	string t, u;
	t = s.substr(0, s.size() / 2);
	u = s.substr(s.size() - t.size());
	...
	for (int i = 0; i < t.size(); ++i)
	{
		if (t[i] == u[i])
		{
			if (find(ok.begin(), ok.end(), t[i]) == ok.end())
			{
				st = false;
			}
		}
		...
		{
			if (find(okk.begin(), okk.end(), make_pair(min(t[i], u[i]), max(t[i], u[i]))) == okk.end())
			{
				st = false;
			}
		}
	...
	if (s.size() % 2)
	{
		if (find(ok.begin(), ok.end(), s[s.size() / 2]) == ok.end())
		{
			st = false;
		}
	}
