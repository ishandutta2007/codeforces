...
ll val(string x) {
	ll ans = 0;
	for (char c : reversed(x)) ans = (c - 'A') + ans * 26;
	rans;
}
string in = "H	He Li	Be B	C	N	O	F	Ne	Na	Mg Al	Si	P	S	Cl	Ar	K	Ca Sc	Ti	V	Cr	Mn	Fe	Co	Ni	Cu	Zn	Ga	Ge	As	Se	Br	Kr	Rb	Sr Y	Zr	Nb	Mo	Tc	Ru	Rh	Pd	Ag	Cd	In	Sn	Sb	Te	I	Xe	Cs	Ba La	Ce	Pr	Nd	Pm	Sm	Eu	Gd	Tb	Dy	Ho	Er	Tm	Yb	Lu	Hf	Ta	W	Re	Os	Ir	Pt	Au	Hg	Tl	Pb	Bi	Po	At	Rn	Fr	Ra Ac	Th	Pa	U	Np	Pu	Am	Cm	Bk	Cf	Es	Fm	Md	No	Lr	Rf	Db	Sg	Bh	Hs	Mt	Ds	Rg	Cn	Nh	Fl	Mc	Lv	Ts	Og D E Q Z";
char str[20];
bool dp[20];
int main() {
	vector <string> poss;
	string curr = "";
	for (char c : in) if (!isalpha(c)) {
		if (!curr.empty()) {
			poss.push_back(curr);
			debug(imie(curr));
		}
	...
	else curr.push_back(toupper(c));
	debug(imie(poss));
	scanf("%s", str);
	dp[0] = 1;
	int n = strlen(str);
	set <string> s(ALL(poss));
	for (int i = 1; i <= n; ++i) {
		for (int j = 0; j < i; ++j) if (dp[j] && s.count(string(str + j, str + i))) dp[i] = true;
	}
	if (dp[n]) printf("YES\n");
	else printf("NO\n");
}