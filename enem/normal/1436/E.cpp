...
void build(ll v,ll tl,ll tr,ll st[])
{
    if(tl==tr)
    {
        st[v]=-1;
    ...
    build((v<<1),tl,((tl+tr)>>1),st);
    build((v<<1)|1,((tl+tr)>>1)+1,tr,st);
    st[v]=min(st[(v<<1)],st[(v<<1)|1]);
    return;
}

void update(ll v,ll tl,ll tr,ll pos,ll val,ll st[])
{
    if(pos>tr||pos<tl)
    {
        return;
    ...
    update((v<<1),tl,((tl+tr)>>1),pos,val,st);
    update((v<<1)|1,((tl+tr)>>1)+1,tr,pos,val,st);
    st[v]=min(st[(v<<1)],st[(v<<1)|1]);
    return;
}

ll query(ll v,ll tl,ll tr,ll val,ll st[])
{
    if(tl==tr)
    {
        return tl;
    }
    if(st[(v<<1)]<val) return query((v<<1),tl,((tl+tr)>>1),val,st);
    else return query((v<<1)|1,((tl+tr)>>1)+1,tr,val,st);
}
...
int main()
{
    ...
    */
    ...
    REP(tc,1,ntc+1)
    {
        ...
        {
            q2[it.S[0]].PB(0);
            REP(i,0,((ll)it.S.size())-1)
            {
                q2[it.S[i+1]].PB(it.S[i]+1);
            }
            q2[n].PB(it.S[((ll)it.S.size())-1]+1);
        }
        ll st[(n+1)<<2];
        build(1,0,n,st);
        q2[n].PB(0);
        ...
        REP(i,0,n+1)
        {
            for(auto it:q2[i])
            {
                ...
                {
                    q1[MP(it,i)]=query(1,0,n,it,st);
                }
            }
            update(1,0,n,a[i],i,st);
        }
        ...
        while(true)
        {
            ...
            if(mp[ans].empty())
            {
                if(q1[MP(0,n)]==ans) bad=false;
            }
            else
            {
                if(q1[MP(0,mp[ans][0])]==ans) bad=false;
                REP(i,0,((ll)mp[ans].size())-1)
                {
                    if(q1[MP(mp[ans][i]+1,mp[ans][i+1])]==ans) bad=false;
                }
                if(q1[MP(mp[ans][((ll)mp[ans].size())-1]+1,n)]==ans) bad=false;
            }
            if(bad) break;
            ans++;
        }
