...
#define int ll
int cnt(pair<int,int> X, int p){
	return (X.second / p) - ((X.first - 1) / p);
}
void solve(){
	...
	for(int i = 0; i + 1 < a.size(); ++i){
		...
		ll c1 = cnt(x, p), c2 = cnt(y, p);
		ll len1 = x.second - x.first + 1, len2 = y.second - y.first + 1;

		// trace(c1, c2, len1, len2);

		ll tot = c1 * len2 + (len1 - c1) * c2;
		__float128 pr = tot;
		pr *= 2000;
