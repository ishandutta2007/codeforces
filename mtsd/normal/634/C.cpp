...
int main(){
    ...
    rep(zzz,q){
        ...
        if(c==1){
            ...
            cin >> d >> s;
            d--;
            sg.range(d,d+1,min(sg.query(d,d+1)+s,b));
            sg2.range(d,d+1,min(sg2.query(d,d+1)+s,a));
        }else{
            int d;
            cin >> d;
            d--;
            // cerr << sg.query(0,d) << " " << sg2.query(d+k,n) << endl;
            cout << sg.query(0,d) + sg2.query(d+k,n) << "\n";
        }
    }
    return 0;
}