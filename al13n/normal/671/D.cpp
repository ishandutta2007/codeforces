...
stat*merge(stat*a,stat*b){
  assert(a&&b);
  if(a->s.size()<b->s.size())
    swap(a,b);
  for(const pll&it:b->s){
    a->s.insert(mp(it.X+b->add-a->add,it.Y));
  }
  return a;
}
...
int dfs2(int v,int pr,stat **out){
  stat*s=&Q[v];
  int up=D[v];
  for(int p:gr[v]){
    if(p==pr)
  ...
  *out=s;
  if(up<D[v])
    return up;
  while(sz(s->s) && s->s.begin()->Y>=D[v])
    s->s.erase(s->s.begin());
  if(!sz(s->s))
    nope();
  LL c=s->s.begin()->X+s->add;
  s->add-=c;
  up=s->s.begin()->Y;
  ans+=c;
  return up;
