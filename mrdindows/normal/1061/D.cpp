...
int solve() {
    ...
    FI(n) {
        cin >> p[i];
        events.push_back({p[i].first, 0});
        events.push_back({p[i].second, 1});
        ans += (p[i].second - p[i].first) * y % mod;
    }
    sort(all(events));
    vector<int> st;
//    DBN(ans);
    for (auto ev : events) {
        swap(ev.first, ev.second);
//        DBN(ev);
        if (ev.first == 0) {
            if (st.empty() || (ev.second - st.back()) * y >= x) {
                ans += x;
//                DBN(x);
            } else {
//                DBN(st.back(), ev.second);
                ans += (ev.second - st.back()) * y % mod;
                st.pop_back();
            }
        } else {
            st.push_back(ev.second);
        }
    }
    cout << ans % mod << endl;
