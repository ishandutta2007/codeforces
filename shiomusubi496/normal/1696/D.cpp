...
void solve() {
    ...
    vector<int> a(n);
    rep (i, n) scanf("%d", &a[i]);
    RangeMaximumQuery<int> rmaq(a);
    RangeMinimumQuery<int> rmiq(a);
    int ans = 0;
    int id = 0;
    while (id < n - 1) {
        ans++;
        if (a[id] < a[id + 1]) {
            int r = rmiq.max_right(id, [&](int x) { return x >= a[id]; });
            int t = rmaq.prod(id, r);
            id = rmaq.max_right(id, [&](int x) { return x < t; });
        }
        else {
            int r = rmaq.max_right(id, [&](int x) { return x <= a[id]; });
            int t = rmiq.prod(id, r);
            id = rmiq.max_right(id, [&](int x) { return x > t; });
        }
    }
    printf("%d\n", ans);
}