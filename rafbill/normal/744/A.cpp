...
int main(){
  ...
  vi E(n);
  FOR(i,k) E[uf.find(K[i])]=1;
  int s0=0;
  vi S(n);
  FOR(i,n) S[uf.find(i)]++;
  FOR(i,n) if(!E[uf.find(i)]) s0++;
  lli ans=-m;
  int mxi=-1; int mxval=0;
  FOR(i,n) if(E[i]&&uf.find(i)==i&&S[i]>mxval){mxi=i;mxval=S[i];}
  S[mxi]+=s0;
  FOR(i,n) if(E[i]&&uf.find(i)==i) ans += (S[i]*(S[i]-1))/2;
  cout << ans << endl;
  return 0;
}