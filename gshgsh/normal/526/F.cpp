...
int main()
{
	N=get();For(i,1,N)p[get()]=get();build(1,1,N);
	...
	For(i,1,N){upd(1,1,N,i,N,1);for(auto j:u[i])upd(1,1,N,j.second.first,j.second.second,j.first);pt2(1,1);}
	cout<<ans[1]-1ll*N*(N-1)/2<<'\n';return 0;
}