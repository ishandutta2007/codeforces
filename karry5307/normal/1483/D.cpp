...
#define ll long long
const ll MAXN=851,MOD=998244353,inf=0x3f3f3f3f;
// Structures and variables
ll n,m,kk,u,v,w,res;
ll dist[MAXN][MAXN],vg[MAXN][MAXN],rr[MAXN];
vector<pii>v1[MAXN];
// Fast IO
inline ll read()
{
...
int main()
{
	...
	Forr(i,1,m)
	{
		u=read(),v=read(),w=read(),vg[u][v]=vg[v][u]=w;
		chkmin(dist[u][v],w),chkmin(dist[v][u],w);
	}
	Forr(k,1,n)
	{
		Forr(i,1,n)
	...
	Forr(i,1,kk)
	{
		u=read(),v=read(),w=read(),v1[u].pb({v,w}),v1[v].pb({u,w});
	}
	Forr(i,1,n)
	{
		Forr(j,1,n)
		{
			...
			for(auto k:v1[j])
			{
				tie(v,w)=k,chkmax(rr[j],w-dist[i][v]);
			}
		}
		Forr(j,i+1,n)
		{
