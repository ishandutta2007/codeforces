// Qpwoeirut And The City (https://codeforces.com/contest/1706/problem/C)
...
int main() {
    ...
    while (t--) {
        ...
        if (n % 2) {
            ll cnt = 0;
            for (int i = 1; i < n; i += 2)
                cnt += max(0, max(h[i - 1], h[i + 1]) + 1 - h[i]);
            println(cnt);
            continue;
        }

        array<vector<ll>, 2> dp{};
        dp.fill(vector(n, 0ll));
        rep(i, 1, n - 1) {
            dp[1 - i % 2][i] = dp[1 - i % 2][i - 1];
            dp[i % 2][i] = dp[i % 2][i - 1] + max(0, max(h[i - 1], h[i + 1]) + 1 - h[i]);
        }

        ll mn = min(dp[0][n - 2], dp[1][n - 2]);
        for (int i = 1; i < n - 2; i += 2)
            mn = min(mn, dp[1][i] + dp[0][n - 2] - dp[0][i + 1]);
