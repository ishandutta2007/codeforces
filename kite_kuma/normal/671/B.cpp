...
ll minvalue(vll c, int n, ll k) {
	sort(all(c));
	vll acc(n + 1);
	rep(i, n) acc[i + 1] = acc[i] + c[i];
	long long ok = c.front();
	long long ng = c.back() + 1;
	while(abs(ok - ng) > 1) {
...
int main() {
	...
	ll lower = sum / n;
	ll upper = dup(sum, ll(n));
	int amari = sum % ll(n);
	vll ave(n);
	rep(i, n) { ave[i] = (i < n - amari ? lower : upper); }
	ll lift = 0;
	rep(i, n) {
		if(ave[i] > c[i]) {
			lift += ave[i] - c[i];
	...
	ll mn = minvalue(c, n, k);
	foa(t, c) t = ll(inf) - t;
	ll mx = ll(inf) - minvalue(c, n, k);
