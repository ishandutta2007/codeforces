...
int main() {
	...
	CASE(t) {
		...
		ll c1 = min(calc({ 1, a[0] }), calc({ a[n - 1], 1 }));
		ll cx = min(calc({ x, a[0] }), calc({ a[n - 1], x }));
		ll c1x = min({ 
			calc({ 1, x, a[0] }),
			calc({ x, 1, a[0] }),
			calc({ a[n - 1], 1, x }),
		...
		for (int i = 1; i < n; i++) {
			chkmin(c1, calc({ a[i - 1], 1, a[i] }) - calc({ a[i - 1], a[i] }));
			chkmin(cx, calc({ a[i - 1], x, a[i] }) - calc({ a[i - 1], a[i] }));
			chkmin(c1x, calc({ a[i - 1], 1, x, a[i] }) - calc({ a[i - 1], a[i] }));
			chkmin(c1x, calc({ a[i - 1], x, 1, a[i] }) - calc({ a[i - 1], a[i] }));
		}
		chkmin(c1x, c1 + cx);
