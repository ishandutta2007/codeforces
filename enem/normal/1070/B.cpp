...
int main()
{
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            ...
            ll x = s.find('/');
            ll b = stoi(s.substr(x+1, ((ll)s.size())-x-1));
            s = s.substr(0, x);
            ll t = 0;
            REP(j,0,4)
            {
                x = s.find('.');
                t += (stoll(s.substr(0, x))<<(24-8*j));
                s = s.substr(x+1, ((ll)s.size())-x-1);
            }
            t &= (-(1LL<<(32-b)));
            mp[typ][t]=max(mp[typ][t], 32-b);
        }
        bool bad = false;
        for(auto it:mp[0])
        {
        ...
        REP(i,0,32)
        {
            ...
            for(auto it:mp[0])
            {
                ...
                {
                    ll nxt = (it.F&(-(1LL<<(i+1))));
                    if(!cunt.count(nxt) || cunt[nxt]>i+1)
                    {
                        ad.PB({nxt, i+1});
                        del.PB(it);
                    }
            ...
            for(auto it:del) mp[0].erase(mp[0].find(it.F));
            for(auto it:ad) mp[0][it.F] = max(mp[0][it.F], it.S);
        }
        cout<<mp[0].size()<<'\n';
        for(auto it:mp[0])
        {
            cout<<(it.F>>24)<<'.'<<((it.F>>16)&((1LL<<8)-1))<<'.'<<((it.F>>8)&((1LL<<8)-1))<<'.'<<(it.F&((1LL<<8)-1))<<'/'<<32-it.S<<'\n';
        }
