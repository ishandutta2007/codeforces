...
void solve() {
	...
	REP(_, m) {
		...
		scanf("%d", &x);
		debug(imie(x));
		if (x > best && sum <= 0) printf("-1 ");
		else if (x <= best) {
			int it = lower_bound(ALL(go), x) - go.begin();
			printf("%d ", it);
		}
		else {
			ll needed = ceil_div(x - best, sum);
			printf("%lld ", needed * n + (int)(lower_bound(ALL(go), x - needed * sum) - go.begin()));
		}
	}
	puts("");
}
