...
int n,ans,hii;
int a[maxn],dep[maxn],par[maxn],rev[maxn],res[maxn],mn[maxn],mx[maxn];
vi g[maxn];
bool ok=true;
bool vis[maxn];
...
void dfs2(int u,int p){
    ...
    sort(range(ls),[&](int u,int v){return a[u]<a[v];});
    for (auto c:ls) dfs2(c,u);
}
...
void dfs3(int u,int p){
    ...
    for (auto c:ls){
        dfs3(c,u);
//        cout<<c<<" "<<mn[c]<<" "<<mx[c]<<endl; 
        mn[u]=min(mn[u],mn[c]), mx[u]=max(mx[u],mx[c]);
        if (pv>mn[c]) ok=false;
        pv=mx[c];
    }
}
...
signed main(){
    ...
    int qaq=-1;
    rep1(i,n){
        if (!solve(rev[i])) {qaq=rev[i]; break;}
        vis[rev[i]]=1, ans+=dep[rev[i]], res[rev[i]]=i, hii++;
    }
    dfs2(1,-1);
//    rep1(i,n) cout<<res[i]<<" ";
//    cout<<endl;
