...
int main(void)
{
	...
	while(T--){
		cin >> h >> w >> k;
		rep(y, 1, h) rep(x, 1, w) cin >> c[x][y], a[x][y] = 0;
		...
		rep(y, 1, h) rep(x, 1, w){
			...
			rep(i, 1, inf){
				...
				if(c[x-i][y-i] == '.' || c[x+i][y-i] == '.') break;
				a[x][y]++;
			}
			if(a[x][y] < k) a[x][y] = 0;
		}
		
		rep(y, 1, h) rep(x, 1, w) d[x][y] = '.';
		rep(y, 1, h) rep(x, 1, w){
			if(a[x][y] == 0) continue;
			d[x][y] = '*';
			rep(i, 1, a[x][y]) d[x-i][y-i] = d[x+i][y-i] = '*';
		}
		
		bool ans = true;
		rep(y, 1, h) rep(x, 1, w) ans &= (c[x][y] == d[x][y]);
		if(ans) yes();
		else no();
	}
