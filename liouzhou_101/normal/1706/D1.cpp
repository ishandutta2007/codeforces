...
void solve()
{
	...
	while (1)
	{
		auto [e, id] = *q.rbegin();
		int val = e;
		if (val == 1) return writeln(res);
		while (q.rbegin()->first >= val)
		{
			auto [e, id] = *q.rbegin();
			q.erase(prev(q.end()));
			p[id] = a[id] / (val - 1);
			while (a[id] / p[id] >= val) p[id] += 1;
			while (p[id] >= 2 && a[id] / (p[id] - 1) <= val - 1) p[id] -= 1;
			if (p[id] > k) return writeln(res);
			q.insert({ a[id] / p[id], id });
		}
		freshmin(res, q.rbegin()->first - q.begin()->first);
	}
}
