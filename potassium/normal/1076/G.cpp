...
struct Block {
	...
	Block(int l, int r) : l(l), r(r), tag(0), val(r - l + 1), root(r - l + 1) {
		t[0].resize(r - l + 1); t[1].resize(r - l + 1);
		build();
	}
	...
	int parent(int v) {
		...
		v -= l;
		if (v - m - 1 < 0) return -1;
		if (t[val[v]][v - m - 1] == -1) return -1;
		return t[val[v]][v - m - 1] + l;
	}
	...
	void build() {
		...
		for (int i = 0; i < r - l + 1; i++) {
			...
			t[val[i]][i] = i;
			int par = parent(i + l);
			root[i] = (par != -1 ? root[par - l] : i);
		}
	}

	int get(int v, int idx) {
		if (idx < l) return -1;
		if (idx > r) idx = r;
		if (t[v ^ tag][idx - l] == -1) return -1;
		return t[v ^ tag][idx - l] + l;
	}
...
int main() {
	...
	for (int l = 0; l < n; l += BLOCK)
		blk.emplace_back(Block(l, min(l + BLOCK - 1, n - 1)));
	...
	while (q--) {
		...
		if (cmd == 1) {
			...
			int cur = r + m + 1;
			for (int i = bid[r]; i > bid[l]; i--) {
				int nxt = blk[i].get(0, cur - m - 1);
				if (nxt != -1)
					cur = blk[i].getRoot(nxt);
			}

			int nxt = blk[bid[l]].get(0, cur - m - 1);
			while (nxt >= l && nxt != -1) {
				cur = nxt;
				nxt = blk[bid[l]].parent(cur);
			}
