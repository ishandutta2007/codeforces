...
int main() {
    ...
    function<void(int, int)> dfs = [&](int v, int p) {
        ...
        Mint res = 1;
        for (auto k : ch) {
            res *= (dp[k][0] + dp[k][1] + dp[k][2]);
        }
        dp[v][2] = res;
        Mint res0 = 1, res1 = 0;
        for (auto k : ch) {
            res1 = res1 * (dp[k][0] + dp[k][1] + dp[k][2]) + (res0 + res1) * (dp[k][0] + dp[k][1] + dp[k][2] * 2);
            res0 *= dp[k][0] + dp[k][1] + dp[k][2];
        }
        dp[v][1] = res1;
        res0 = 1;
        res1 = 0;
        for (auto k : ch) {
            res1 = res1 * (dp[k][0] + dp[k][1] + dp[k][2]) + (res0 + res1) * (dp[k][1] + dp[k][2]);
            res0 *= dp[k][0] + dp[k][1] + dp[k][2];
        }
        dp[v][0] = res1;
    };
    dfs(0, -1);
    cout << dp[0][0] + dp[0][1] + dp[0][2] - 1 << '\n';
