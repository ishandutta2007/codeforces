...
int main() {
    ...
    set<ll> gr, gc, gl;
    for (ll v: g[r]) { gr.insert(v); }
    for (ll v : g[c]) { gc.insert(v); }
    for (ll v : g[l]) { gl.insert(v); }
    ...
    while (!q.empty()) {
        ...
        for (ll w : g[v]) {
            ...
            //cout << v << " " << w << endl;
            if (gc.count(w)) {
                if (!gl.count(w)&&w!=l) { ans[w] = 1; }
                else if (!gr.count(w)) { ans[w] = -1; }
                else { ans[w] = 0; }
            }
            else {
                if (ans[v] > 0) { ans[w] = ans[v] + 1; }
                else { ans[w] = ans[v] - 1; }
            }
            q.push(w);
            count[ans[w]]++;
    ...
    rep(v, N) {
        for (ll w : g[v]) {
            if (abs(ans[w] - ans[v]) > 1) { cout << "NO" << endl; return 0; }
        }

        if (count[ans[v] - 1] + count[ans[v]] + count[ans[v] + 1] != g[v].size() + 1) {
