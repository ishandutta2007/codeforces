...
// unordered_map<LL,vector<int>> memo;
...
void add(int x) {
    pair<int,int> id = {c[0][x], c[1][x]};
    // LL id2 = ((LL)id.first)*MAXN + id.second;
    // memo[id].push_back(x);
    val1.insert(id);
    swap(id.first, id.second);
    val2.insert(id); 
    ...
    while (m1 < n-1 and m2 < n-1) {
        ...
        for (int i = 0; i < 4 and !works; i++) for (int j = i+1; j < 4; j++) {
            ...
            if (pp1.first != pp2.first and pp1.second != pp2.second) {
                // dbg(pp1, pp2);
                if (comp[0][pp1.first].size() < comp[0][pp2.first].size()) swap(pp1, pp2);
                int b = -1;
                for (int x: comp[0][pp2.first]) {
                    if (c[1][x] == pp2.second) {
                        b = x;
                        break;
                    }
