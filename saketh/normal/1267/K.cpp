...
// number of permutations such that fing[i] < i+2
ll count(vi fing) {
    ll ans = 1;

    int t = 0;
    F0R (i, sz(fing)) {
        while (t < sz(fing) && fing[t] < i+2) t++;
        ans *= t - i;
    }

    for (int i = 0, j = 0; i < sz(fing); i = j) {
        while (j < sz(fing) && fing[j] == fing[i]) j++;
        F0R (k, j - i) ans /= k + 1;
    }
