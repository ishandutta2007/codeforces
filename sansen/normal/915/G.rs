...
fn run() {
    ...
    let mut mobius = vec![M::one(); k + 1];
    let mut factor = (0..=k).collect::<Vec<_>>();
    for i in 2..=k {
        if i == factor[i] {
            for mo in mobius[i..].iter_mut().step_by(i) {
                *mo = -*mo;
            }
            let pp = i.saturating_mul(i).min(k + 1);
            for mo in mobius[pp..].iter_mut().step_by(pp) {
                *mo = M::zero();
            }
            for f in factor[pp..].iter_mut().step_by(i) {
                *f = i;
            }
        }
    }
    let mut pow = vec![M::one(); k + 1];
    for (i, f) in factor.iter().enumerate().skip(2) {
        if *f == i {
            pow[i] = M::from(i).pow(n);
        } else {
            pow[i] = pow[*f] * pow[i / *f];
        }
    }
    let mut imos = vec![M::zero(); k + 2];
    for (w, &sign) in mobius.iter().enumerate().filter(|p| !p.1.is_zero()).skip(2) {
        for i in (1..).take_while(|i| i * w <= k) {
            let l = i * w;
            imos[l] += sign * pow[i];
            let r = (k + 1).min((i + 1) * w);
            imos[r] -= sign * pow[i];
