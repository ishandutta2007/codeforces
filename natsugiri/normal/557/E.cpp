...
char B[5011][5011]; // 1:0.2.4, 2:1.3.5
char S[5011];
int N, K;
...
struct CMP {
	...
	return -1 == SS.compare(x.first, x.second, y.first, y.second);
    }
};
...
int main() {
    ...
    for (int l=N-1; l>=0; l--) {
	for (int len=0, r=l+len; r<=N; len++, r++) {
	    if (B[l+1][r-1] & 1) B[l][r] |= 2;
	    if (S[l] == S[r-1] && (B[l+1][r-1] & 2)) B[l][r] |= 1;
	}
    }
    
    vector<pair<int, int> > X;
    REP (r, N+1) REP (l, r) {
	if (B[l][r] & 1) X.push_back(make_pair(l, r-l));
    }
    SAIS SS((string)S);
    SS.buildRMQ();
    //sort(X.begin(), X.end(), CMP(SS));
    nth_element(X.begin(), X.begin()+K, X.end(), CMP(SS));
    // EACH (e, X) {
    // 	eprintf("%s\n", string(S+e->first, S+e->second+e->first).c_str());
    // }

    puts(string(S+X[K].first, S+X[K].first+X[K].second).c_str());
    // REP (i, X[K-1].second) putchar(S[X[K-1].first+i]);
    // putchar('\n');
    return 0;
}