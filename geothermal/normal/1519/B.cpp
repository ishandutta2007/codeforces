...
void solve() {
    ...
    int N, M; cin >> N >> M;
    ll K; cin >> K;
    int lo[N][M], hi[N][M]; F0R(i, N) F0R(j, M) lo[i][j] = K+1;
    F0R(i, N) F0R(j, M) hi[i][j] = -1;
    lo[0][0] = 0; hi[0][0] = 0;
    F0R(i, N) {
        F0R(j, M) {
            if (i < N-1) {
                ckmin(lo[i+1][j], lo[i][j] + j+1);
                ckmax(hi[i+1][j], hi[i][j] + j+1);
            }
            if (j < M-1) {
                ckmin(lo[i][j+1], lo[i][j] + i+1);
                ckmax(hi[i][j+1], hi[i][j] + i+1);
            }
        }
    }
    pr(lo[N-1][M-1]<=K&&K<=hi[N-1][M-1]?"YES":"NO", nl);
