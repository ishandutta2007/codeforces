...
namespace tietietree
{
    ...
    struct node
    {
        bool ans;
        int zjj,gyq;
        inline void init(){zjj=m+1;gyq=0;ans=false;return;}
    }nod[N<<2];
    inline void up(int i)
    {
        nod[i].zjj=min(nod[ls].zjj,nod[rs].zjj);nod[i].gyq=max(nod[ls].gyq,nod[rs].gyq);
        nod[i].ans=nod[ls].ans||nod[rs].ans||(nod[ls].zjj<=nod[rs].gyq);return;
    }
    inline void maketree(int l,int r,int i){nod[i].init();if(l==r)return;int mid=l+r>>1;maketree(l,mid,ls);maketree(mid+1,r,rs);return;}
    inline void updata(int l,int r,int i,int qx)
    {
        ...
        if(l==r)
        {
            ...
            // printf("l:%d zjj:%d gyq:%d\n",l,nod[i].zjj,nod[i].gyq);
            return;
        }
        int mid=l+r>>1;if(qx<=mid)updata(l,mid,ls,qx);else updata(mid+1,r,rs,qx);up(i);return;
    }
    #undef ls
    #undef rs
}
...
{
    int x,y,z;rin>>x>>y;
    if(qy.find(make_pair(x,y))==qy.end()){if(x&1)zjj[x+1>>1].insert(y+1>>1);else gyq[x>>1].insert(y>>1);qy[make_pair(x,y)]=true;}
    else {if(x&1)zjj[x+1>>1].erase(y+1>>1);else gyq[x>>1].erase(y>>1);qy.erase(make_pair(x,y));}
    updata(1,n,1,x+1>>1);puts(nod[1].ans?"NO":"YES");return;
}
int main()
{
    rin>>n>>m>>q;maketree(1,n,1);for(;q;q--)work();
    return 0;
}