...
int main() {
    ...
    int trips_allowed = min(sz(inp[0]), min(sz(inp[1]), sz(inp[2])));

    vector<pair<char, int>> trips;
    for(char c = 'A'; c <= 'Z'; c++){
        while(trips_allowed > 0){
            vi high_two_inds = getHighTwo(cnt[c]);
            assert(sz(high_two_inds) == 2);
            if(cnt[c][high_two_inds[0]] > 0 && cnt[c][high_two_inds[1]] > 0){
                cnt[c][high_two_inds[0]]--;
                cnt[c][high_two_inds[1]]--;
                trips.pb(mp(c, 3-high_two_inds[0]-high_two_inds[1]));
                trips_allowed--;
            }
            else{
                break;
    ...
    while(sz(chars_left[0]) + sz(chars_left[1]) + sz(chars_left[2]) > 0){
        ...
        if(num_nonzero == 1){
            ...
            vi inds = getHighTwo(vi{sz(chars_left[0]), sz(chars_left[1]), sz(chars_left[2])});
