...
int main(void)
{
	...
	rep(i, 1, n) need[i] = i * a[i].first - sum[i];
	
	ll ans = -1;
	per(i, n+1, 1){
		ll rem = m;
		rem -= (n+1-i) * A - (sum[n] - sum[i-1]);
		if(rem < 0) continue;
		...
		ll p = upper_bound(need+1, need+i, rem) - need - 1;
		rem -= need[p];
		if(ceil(rem, p) + a[p].first >= A) continue;
		
		ll tmp = (n+1-i) * x + (rem/p + a[p].first) * y;
		if(ans < tmp){
			ans = tmp;
			rep(j, i, n) b[j] = A;
			rep(j, 1, p){
				if(j <= rem%p) b[j] = ceil(rem, p) + a[p].first;
				else b[j] = rem/p + a[p].first;
			}
			rep(j, p+1, i-1) b[j] = a[j].first;
		}
	}
	rep(i, 1, n) c[a[i].second] = b[i];
