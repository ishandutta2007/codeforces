...
void fft2d(vector<fps>& a) {
  rep(i, sz(a)) {
    bool ok = 0;
    each(x, a[i]) if (x != mint()) ok = 1;
    if (ok) a[i].ntt();
  }
  fps buf(sz(a));
  rep(i, sz(a[0])) {
    rep(j, sz(a)) buf[j] = a[j][i];
    buf.ntt();
    rep(j, sz(a)) a[j][i] = buf[j];
  }
}
void ifft2d(vector<fps>& a) {
  fps buf(sz(a));
  rep(i, sz(a[0])) {
    rep(j, sz(a)) buf[j] = a[j][i];
    buf.intt();
...
void Nyaan::solve() {
  ...
  vi l(N);
  in(l);
  int L = accumulate(all(l), 0ll);
  mint iL = mint(L).inverse();
  ...
  rep(i, N) {
    ...
    rep(k, l[i] / K + 1) {
      ...
      int ds = l[i] - k * K;
      if (ds == 0) break;
      mint coe1 = (iL * ds).pow(k) * C.finv(k);
      mint coe2 = (iL * ds).pow(k - 1) * C.finv(k - 1);
      if (k & 1) coe1 = -coe1, coe2 = -coe2;
      nx[ds][0] += coe1;
      if (k) nx[ds][1] += coe2;
    }
    fft2d(nx);
    rep(s, H) rep(t, W) P[s][t] *= nx[s][t];
  }
  ifft2d(P);
  //trc(P);
  //out("pre");
  ...
  rep(t, L) {
    ...
    rep(_s, sz(P[t])) {
      ...
      trc(t,_s,s,P[t][_s]);
      fps f(s + 1);
      rep(i, s + 1) f[i] = C.fac(i + s) * C.finv(i);
      mint soe = (mint(1) - iL * t).inverse().pow(s + 1) * C.fac(s);
      //mint soe = sum_of_exp_limit<mint>(f, iL * t, C);
      ans += P[t][_s] * soe;
    }
  }
  out(ans);
