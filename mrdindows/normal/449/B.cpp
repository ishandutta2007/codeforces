...
int main()
{
    ...
    for (int i = 2; i <= n; ++i)
    {
        ...
        if (z[i])
        {
            q.insert(make_pair(z[i], i));
            d[i] = z[i];
            e[i] = true;
        }
    }
    q.insert(make_pair(0LL, 1));
    while (!q.empty())
    {
        pair<ll, int> curp = *q.begin();
        q.erase(q.begin());
        int cur = curp.second;
        ll dist = curp.first;
        used[cur] = true;
...
//  {
//      long long res = 0;
//      for (int x = min(n - 1, k); x >= 0 && x >= n - 10000000; --x)
//      {
//          int y = k - x;
//          if (y < 0) continue;
//          if (y >= m) break;
//          ll s = (n / (x + 1)) * 1ll * (m / (y + 1));
//          if (s >= res) res = s;
//      }
//      for (int y = min(m - 1, k); y >= 0 && y >= m - 10000000; --y)
//      {
//          int x = k - y;
//          if (x < 0) continue;
//          if (x >= n) break;
...
//  {
//      ll res = 0;
//      double x = k / (1.0 + m * 1. / n) - 1;
//      double y = k / (1.0 + n * 1. / m) - 1;
//      double sizex = n / (x + 1);
//      double sizey = m / (y + 1);
// 
