...
void solve()
{
	...
	for (int i = 1; i <= n; ++i)
	{
		for (int j = 0; j <= n + 1; ++j) L[j] = R[j] = tag[j] = INF;

		L[0] = f[i - 1][0];
		for (int j = 1; j <= n; ++j) L[j] = min(L[j - 1], f[i - 1][j]);

		for (int j = n; j >= 1; --j) R[j] = min(R[j + 1], f[i - 1][j]);
		...
		for (int j = n; j >= 0; --j)
		{
			if (a[i] > j)
			{
				freshmin(tag[j], f[i - 1][j] + 1);
				if (a[i] <= n) freshmin(f[i][a[i]], f[i - 1][j] + a[i] - j);
			}
			else if (a[i] == j)
			{
				freshmin(f[i][j], f[i - 1][j]);
				//freshmin(tag[j + 1], f[i - 1][j] + 2);
			}
			else
			{
				freshmin(tag[a[i]], f[i - 1][j]);
			}
		}
		for (int j = 1; j <= n; ++j) tag[j] = min(tag[j], tag[j - 1] + 1);
		for (int j = 0; j <= a[i] && j <= n; ++j) freshmin(f[i][j], tag[j]);
	}

	int ans = INF;
	for (int j = 0; j <= n; ++j) freshmin(ans, f[n][j]);
	writeln(ans);
}
