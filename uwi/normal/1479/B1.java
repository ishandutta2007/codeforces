...
public class B1 {
	...
	void solve()
	{
		...
		dp[0] = 0;
		SegmentTreeRAddRUpdateRMQ st = new SegmentTreeRAddRUpdateRMQ(dp);
		int pre = 0;
		for(int v : a){
			if(v == pre){
				long imax = Math.min(st.min(pre, pre+1), Math.min(st.min(0, pre), st.min(pre+1, n+1)) - 1);
				st.update(pre, pre+1, imax);
			}else{
				// pre x
				// v
				long imax = Math.min(st.min(pre, pre+1) - 1, Math.min(st.min(0, v), st.min(v+1, n+1)) - 1);
				st.add(0, n+1, -1);
				st.update(pre, pre+1, imax);
			}
			pre = v;
		}
		out.println(-st.min(0, n+1));
	}

	public static class SegmentTreeRAddRUpdateRMQ {
		public int M, H, N;
		public long[] st;
		public long[] plus;
		public long[] cover;
		...
		public SegmentTreeRAddRUpdateRMQ(long[] a)
		{
			N = a.length;
	...
	public static void main(String[] args) throws Exception { new B1().run(); }
