...
vector<ll> vec, nvec;
bool flag[100005], nflag[100005];
ll cnt[100005], ncnt[100005];
...
int main(void)
{
	...
	while(T--){
		...
		per(i, n, 2){
			...
			cnt[a[i]]++;
			if(!flag[a[i]]) flag[a[i]] = true, vec.push_back(a[i]);
			
			for(auto p : vec){
				ll x = max(1LL, ceil(a[i-1], p));
				ans += mint(x-1) * mint(cnt[p]) * mint(i-1);
				
				ll y = a[i-1]/x;
				ncnt[y] += cnt[p];
				if(!nflag[y]) nflag[y] = true, nvec.push_back(y);
			}
			
			for(auto p : vec) flag[p] = false, cnt[p] = 0; vec.clear();
			for(auto p : nvec) flag[p] = true, cnt[p] = ncnt[p]; vec = nvec;
			for(auto p : nvec) nflag[p] = false, ncnt[p] = 0; nvec.clear();
		}
		outl(ans);
		
		for(auto p : vec) flag[p] = false, cnt[p] = 0;
		vec.clear();
	}
