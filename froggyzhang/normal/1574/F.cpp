...
int main(){
	...
	for(int i=1;i<=n;++i){
		...
		for(int j=0;j<k;++j){
			if(vis[A[j]])S.ban[S.getf(A[j])]=1;
			vis[A[j]]=true;
		}
		for(int j=0;j<k-1;++j){
			if(suf[A[j]]&&suf[A[j]]^A[j+1]||pre[A[j+1]]&&pre[A[j+1]]^A[j]){
				S.ban[S.getf(A[j])]=S.ban[S.getf(A[j+1])]=1;
			}
			S.Merge(A[j],A[j+1]);
			suf[A[j]]=A[j+1],pre[A[j+1]]=A[j];
		}
		for(auto x:A)vis[x]=false;
	}
	for(int i=1;i<=K;++i){
		static int vis[N],tmp[N];
		if(vis[i])continue;
		vector<int> jb;
		for(int u=i;u;u=suf[u]){
			if(u&&vis[u]){if(tmp[u])S.ban[S.getf(u)]=1;break;}
			vis[u]=tmp[u]=1;
			jb.push_back(u);
		}
		for(auto u:jb)tmp[u]=0;
	}
	poly A(m+1);
	for(int i=1;i<=K;++i){
		if(S.getf(i)==i&&!S.ban[i]){
			if(S.siz[i]<=m)++A[S.siz[i]];
		}
	}
	for(int i=1;i<=m;++i){
		A[i]=mod-A[i];
