...
void solve() {
    ...
    for (int i = 1; i <= max_length + 2; i++) cout << '*';
    cout << '\n';
    for (int i = 1; i <= n; i++) {
        cout << '*';
        int padding_left = (max_length - s[i].size()) / 2;
        int padding_right = max_length - s[i].size() - padding_left;
        if (padding_left != padding_right) {
            if (!align_left) swap(padding_left, padding_right);
            align_left = !align_left;
        }
        for (int j = 1; j <= padding_left; j++) cout << ' ';
        cout << s[i];
        for (int j = 1; j <= padding_right; j++) cout << ' ';
        cout << "*\n";
    }
    for (int i = 1; i <= max_length + 2; i++) cout << '*';
}
