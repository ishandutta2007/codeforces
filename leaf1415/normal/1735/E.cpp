...
int main(void)
{
	...
	while(T--){
		...
		bool flag = true;
		rep(i, 1, n) if(d[0][i] != d[1][i]) flag = false;
		if(flag){
			yes();
			outa(d[0], 1, n);
			outl(0, 0);
		...
		for(auto x : dvec){
			if(x == 0) continue;
			multiset<ll> S[2];
			rep(i, 1, n) S[0].insert(d[0][i]);
			rep(i, 1, n) S[1].insert(d[1][i]);
			
			bool flag = true; vector<ll> ans;
			rep(i, 1, n){
				ll id = 0;
				if(*S[0].rbegin() < *S[1].rbegin()) id = 1;
				ll p = *S[id].rbegin(), np = p - x;
				if(p < x) np = x - p;
				if(S[1-id].count(np) == 0){
					flag = false;
					break;
				}
				S[id].erase(S[id].lower_bound(p));
				S[1-id].erase(S[1-id].lower_bound(np));
				if(id == 0) ans.pb(p);
				else ans.pb(x-p);
			}
			if(!flag) continue;
			...
			ll m = min(0LL, ans[0]);
			for(auto &y : ans) y -= m;
			for(auto y : ans) assert(y <= 2000000000);
