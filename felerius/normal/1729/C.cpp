// Jumping on Tiles (https://codeforces.com/contest/1729/problem/C)
...
int main() {
    ...
    while (t--) {
        string s; read(s);
        int c0 = s[0] - 'a', cn = s.back() - 'a', n = sz(s);
        ...
        vector<int> p;
        int step = cn > c0 ? 1 : -1;
        for (int i = c0; i != cn; i += step)
            p.insert(end(p), all(idx[i]));
        p.insert(end(p), all(idx[cn]));

        println(abs(cn - c0), sz(p));
        println(p);
    }
}