...
public class TaskB {
	...
	private void solve() {
		...
		int[] start = new int[participantCount];
		int[] end = new int[participantCount];
		int[] time = new int[participantCount];
		int[] prize = new int[participantCount];
		in.readIntArrays(start, end, time, prize);
		int totalPrize = 0;
		for (int i = 1; i <= segmentCount; i++) {
			int bestTime = Integer.MAX_VALUE;
			int bestIndex = -1;
			for (int j = 0; j < participantCount; j++) {
				if (i >= start[j] && i <= end[j] && time[j] < bestTime) {
					bestTime = time[j];
					bestIndex = j;
				}
			}
			if (bestIndex != -1)
				totalPrize += prize[bestIndex];
		}
