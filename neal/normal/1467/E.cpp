...
int main() {
    ...
    for (int node : lca.tour_list)
        tour_locations[A[node]].push_back(lca.tour_start[node]);

    auto get_subtree_count = [&](int node, int color) -> int {
        auto begin = lower_bound(tour_locations[color].begin(), tour_locations[color].end(), lca.tour_start[node]);
        auto end = lower_bound(tour_locations[color].begin(), tour_locations[color].end(), lca.tour_end[node]);
    ...
    for (int node = 0; node < N; node++) {
        int color = A[node];
        int overall = int(tour_locations[color].size());

        for (int child : lca.adj[node]) {
            int below = get_subtree_count(child, color);
        ...
        int me = get_subtree_count(node, color);

        if (me < overall) {
            delta[lca.tour_start[node]]++;
            delta[lca.tour_end[node]]--;
        }
    }
