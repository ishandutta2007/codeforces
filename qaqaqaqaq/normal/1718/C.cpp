...
signed main(){
  ...
  while(_--){
    ...
    rep(i,n) cin>>a[i];
    map<int,int> mp;
    rep(i,n) rep(j,10) sum[i][j]=0;
    rep(j,sz(lst)){
      int mo=n/lst[j];
      for (int i=0;i<n;++i) sum[i%mo][j]+=a[i];
      for (int i=0;i<mo;++i) mp[-sum[i][j]*mo]++;
    }
    auto ret=mp.begin()->F;
    cout<<-ret<<"\n";
    while (q--){
      ...
      rep(j,sz(lst)){
        int mo=n/lst[j];
        mp[-sum[pos%mo][j]*mo]--;
        if (!mp[-sum[pos%mo][j]*mo]) mp.erase(-sum[pos%mo][j]*mo);
        sum[pos%mo][j]-=a[pos], sum[pos%mo][j]+=x;
        mp[-sum[pos%mo][j]*mo]++;
        // a[pos]=x;
      }
      a[pos]=x;
