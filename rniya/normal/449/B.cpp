...
int main() {
    ...
    for (int i = 0; i < k; i++) {
        ...
        check.emplace(s, y);
        train.emplace_back(--s, y);
        G[0].emplace_back(s, y);
        G[s].emplace_back(0, y);
    }

    vector<ll> dp(n, IINF);
    vector<pair<mint1, mint2>> path(n, {0, 0});
    ...
    while (!pq.empty()) {
        ...
        for (auto& e : G[v]) {
            int u = e.first;
            ll cost = val + e.second;
            if (cost < dp[u]) {
                dp[u] = cost;
                path[u] = path[v];
                pq.emplace(-dp[u], u);
            } else if (cost == dp[u]) {
                path[u].first += path[v].first;
                path[u].second += path[v].second;
            }
        }
    }

    for (auto& p : train) {
        int s = p.first, y = p.second;
        if (dp[s] < y || (dp[s] == y && path[s] != make_pair(mint1(1), mint2(1)))) ans++;
    }
