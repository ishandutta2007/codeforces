...
int main() {
    ...
    int N, M; re(N, M); vpii g(N); re(g);
    vvi rat(M), lat(M); trav (p, g) {
        --p.f, --p.s;
        lat[p.f].pb(p.s);
        rat[p.s].pb(p.f);
    }
    ...
    auto nn = [](pil a, pil b){ return pil{a.f+b.f, a.s+b.s}; };
    auto nd = [](pil a, pil b){ return pil{a.f-b.f, a.s-b.s}; };
    auto uu = [](bool a, bool b){ return a || b; };
    auto un = [](bool u, pil n){ return u ? pil{0,0} : n; };

    segment_tree_lazy even(M, pil{0,0}, false, nn, uu, un),
                       odd(M, pil{0,0}, false, nn, uu, un);

    even.set_leaves([](int i){ return i&1 ? pil{0,0} : pil{1,i}; });
    odd.set_leaves([](int i){ return i&1 ? pil{1,i} : pil{0,0}; });

    vll emp(M + 1);
    FOR (i, 1, sz(emp)) emp[i] = emp[i - 1] + i * ll(i + 1) / 2;
    ...
    F0Rd (a, M) {
        ...
        if (sz(ev) + sz(od) == 0) empty++;
        else { ans -= emp[empty]; empty = 0; }

        auto& bnd = a&1 ? ev : od;
        int UB = sz(bnd) ? *bnd.begin() : M;
        pil av = nn(even(a, UB), odd(a, UB));

        int LA = max(sz(ev) ? *ev.rbegin() : (a - 1),
                     sz(od) ? *od.rbegin() : (a - 1));
        av = nd(av, (a&1 ? even : odd)(a, min(LA + 1, UB)));

        ans += av.s - (a - 1) * ll(av.f);

        trav (r, lat[a]) {
            (a&1 ? even : odd)(a, r+1, true);
            if ((r - a)&1) {
                even(r, M, true);
                odd(r, M, true);
            }
            if (r&1) od.erase(od.find(r));
            else ev.erase(ev.find(r));
        }
    }
