...
// URL : http://codeforces.com/contest/1398/problem/D
// Memory Limit : 256 MB
// Time Limit : 2000 ms
...
int calc(int i,int j,int k){
	...
	dp[i][j][k]=0;
	if(i&&j)
		dp[i][j][k]=max(dp[i][j][k],calc(i-1,j-1,k)+r[i]*g[j]);
	if(i&&k)
		dp[i][j][k]=max(dp[i][j][k],calc(i-1,j,k-1)+r[i]*b[k]);
	if(k&&j)
		dp[i][j][k]=max(dp[i][j][k],calc(i,j-1,k-1)+b[k]*g[j]);
	return dp[i][j][k];
}
int main()
{
