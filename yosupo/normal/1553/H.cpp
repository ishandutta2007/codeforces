...
D mrg(int sz, D a, D b) {
    D d = {INF, -INF, INF};

    if (a.min != INF) d.min = a.min;
    else if (b.min != INF) d.min = b.min + sz;

    if (b.max != -INF) d.max = b.max + sz;
    else if (a.max != -INF) d.max = a.max;
    
    d.ans = min(a.ans, b.ans);
    if (a.min != INF && b.min != INF) {
        d.ans = min(d.ans, b.min + sz - a.max);
    }
    return d;
}
...
int main() {
    ...
    auto solve = [&](auto self, int h, int s) -> V<D> {
        if (h == 0) {
            if (has[s]) {
                return {D{0, 0, INF}};
            } else {
                return {D{INF, -INF, INF}};
            }
        }
        int sz = 1 << (h - 1);
        auto ans0 = self(self, h - 1, s);
        auto ans1 = self(self, h - 1, s + sz);

        V<D> ans(1 << h);

        for (int i = 0; i < sz; i++) {
            ans[i] = mrg(sz, ans0[i], ans1[i]);
            ans[i + sz] = mrg(sz, ans1[i], ans0[i]);
        }
        return ans;
    };

    auto ans = solve(solve, k, 0);
