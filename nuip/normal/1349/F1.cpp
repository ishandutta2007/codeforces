...
mint A(int n,int m){
	mint re=0;
	rep(k,m+1){
		mint tmp=comb(n+1,k)*modpow(m+1-k,n);
		if(k%2) re-=tmp;
		else re+=tmp;
	}
	return re;
...
int main(){
	// rep(i,10){rep(j,10) cout<<test(j,i,10)<<","; cout NL;} cout NL;
	// rep(i,6) test(i+1);
  ios_base::sync_with_stdio(false); cin.tie(0);
  cout<<fixed<<setprecision(0);
	dp[0][0]=1;
	rep(n,5005)rep(m,5005){
		if(n&&m) dp[n][m]+=dp[n-1][m-1]*(n-m);
		if(n) dp[n][m]+=dp[n-1][m]*(m+1);
	}
	int n;
	cin>>n;
	vector<mint> re(n);
	rep(i,n)rep(j,n){
		re[j]+=dp[i+1][j]*finv[i+1];
	}
	rep(i,n) cout<<re[i]*fact[n]<<" "; cout NL;
  return 0;
}