...
inline void solve() {
    fct[0] = 1;
    for (int i = 1; i < N; ++i) {
        fct[i] = fct[i - 1] * ll(i) % MOD;
    }
    ifct[N - 1] = inv(fct[N - 1], MOD);
    for (int i = N - 1; i > 0; --i) {
        ifct[i - 1] = ifct[i] * ll(i) % MOD;
    }
    ...
    while (t--) {
        ...
        cin >> n >> l >> r;
        int ans = 0;
        for (int go_dw = n / 2; go_dw <= (n + 1) / 2; ++go_dw) {
            // can_add (0, r - n]
            // can sub (0, 1 - l]

            int mn = min(r - n, 1 - l);
            ans += cnk(n, go_dw) * ll(mn) % MOD;
            ans %= MOD;

            for (int pl = 1; pl <= n; ++pl) {
                // change by (mn + pl)
                // [1 ... c) must go up, c = l + (mn + pl)
                // (c ... n] must go down, c = r - mn - pl
                int c_up = max(l + mn + pl, 1);
                int c_dw = min(r - mn - pl, n);
                if (n - c_dw > go_dw || n - c_up + 1 - (n - c_dw) < go_dw - (n - c_dw)) {
                    break;
                }
                ans += cnk(n - c_up + 1 - (n - c_dw), go_dw - (n - c_dw));
                ans %= MOD;
            }
        }
        cout << ans << endl;
