...
int calc(int d,int n){
	...
	if(n==0)return dp[d][n]=1;
	if(d==9)return dp[d][n]=(calc(0,n-1)+calc(1,n-1))%mod;
	return dp[d][n]=calc(d+1,n-1);
}
char s[20];
void Solve(){
	scanf("%s",s); int n=read();
	ll ans=0;
	repeat(i,0,strlen(s))ans+=calc(s[i]-'0',n);
	ans%=mod;
	print(ans,1);
}
signed main(){
