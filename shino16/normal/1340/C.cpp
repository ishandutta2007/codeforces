...
int main() {
  ...
  while (not deq.empty()) {
    auto [i, k] = deq.front(); deq.pop_front();
    for (auto i2 : VI{i-1, i+1}) if (i2 >= 0 and i2 < m) {
      int dd = abs(d[i] - d[i2]);
      if (k + dd == g and chmin(dist[i2][0], dist[i][k]+1))
        deq.emplace_back(i2, 0);
      else if (k + dd < g and chmin(dist[i2][k+dd], dist[i][k]))
        deq.emplace_front(i2, k+dd);
    }
  }

  ll res = INF_LL;
  if (dist[m-1][0] != INF) chmin(res, dist[m-1][0]*(g+r) - r);
  rep(k, 1, g) if (dist[m-1][k] != INF) chmin(res, dist[m-1][k]*(g+r) + k);

  OUT(res == INF_LL ? -1 : res);
}