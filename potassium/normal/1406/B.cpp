...
int main() {
	CASE(t) {
		...
		auto take = [&](vector<ll>& v, int count, bool large) {
			ll prod = 1;
			if (large) {
				for (int i = 0; i < count; i++)
					prod *= v[i];
			} else {
				for (int i = v.size() - 1; i >= v.size() - count; i--)
					prod *= v[i];
			}
			return prod;
		};
		...
		bool canPos = pos.size();
		if (canPos) {
			for (int negz = 0; negz < 5; negz += 2) {
				if (neg.size() < negz) break;
				ll negv = take(neg, negz, 1);
				ll posv = (pos.size() >= 5 - negz ? take(pos, 5 - negz, 1) : 0);
				
				chkmax(ans, negv * posv);
			}
		} else {
			if (zero.size()) {
				ans = 0;
