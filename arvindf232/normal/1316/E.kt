...
class person(val id:Int, val aud:Int, val pos:IntArray)
...
fun main(){
    solve.cases{
		...
		val all = MutableList(n){
			person(it,L[it], getline(p))
		}
		all.sortByDescending {it.aud  }
		var DP = LongArray(1 shl p){nhuge}
        DP[0] = 0L
		for(i in 0 until n){
			val new = LongArray(1 shl p){nhuge}
			for(m in DP.indices){
				for(j in 0 until p){
					if(m.hasElement(j)) continue
					val newm = m xor ( 1 shl j)
					new[newm] = maxOf(new[newm],DP[m] + all[i].pos[j])
				}
				val bonus = if(i - m.countOneBits() < k ) all[i].aud else 0
				new[m] = maxOf(new[m], bonus.toLong() + DP[m])
			}
			DP = new
		}
