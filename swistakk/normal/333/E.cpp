  ...
  vector<vector<bool>> InsideDiagonalsMatrix() { // seems ok, but test it on CF #250 C !!
    ...
    for (int i = 0; i < n; i++) {
      for (int j = 0; j < n; j++) {
        ...
        while (act != i) {
          passed_j |= (act == j);
          areas[passed_j] += pts[i].CrossProd(pts[act], pts[(act + 1) % n]);
          act = (act + 1) % n;
        }
...
#define int long long
  ...
  sort(pkt + 1, pkt + 1 + n, cmp);
  for (int i = 1; i <= n; i++)
  {
    for (int j = 1; j <= n; j++)
    {
      int dx = pkt[i].x - pkt[j].x;
      int dy = pkt[i].y - pkt[j].y;
      distan[i][j] = dx * dx + dy * dy;
    }
  }
//   Polygon poly2;
//   for (int j = 1; j <= n; j++) {
//     poly2.Add(Point(pkt[j].x, pkt[j].y));
//   }
//   poly2.MakeConvexHull();
  
  int kl = 0, kp = 8e8, aktc, faj = -1;
  while (kl <= kp)
  {
    aktc = (kl + kp) / 2;
    for (int i = 1; i <= n; i++)
    {
      ...
      for (int j = 0; j < pkts.size(); j++) {
        poly.Add(Point(pkts[j].x, pkts[j].y));
      }
      poly.MakeConvexHull();
      auto furthest_pair = poly.FurthestPair();
      
      //cout<<"zonk"<<endl;
      double best = poly.pts[furthest_pair.first].Dist(poly.pts[furthest_pair.second]);
      if (best * best + kEps >= aktc)
      {
        kl = aktc + 1;
        faj = aktc;
        goto A;
  ...
  cout<<pow(double(faj), 0.5) / 2<<endl;
  //printf("%.7lf\n", pow(double(faj), 0.5) / 2);
  // nie zapomnij o ll
  return 0;
}