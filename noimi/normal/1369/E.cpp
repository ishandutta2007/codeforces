...
main() {
    ...
    auto add = [&](int i) {
        for(auto e : g[i]) {
            cnt[e.cost]++;
            if(cnt[e.cost] == 1) q.emplace(e.cost);
        }
    };
    rep(i, n) if(a[i] >= 0) add(i);
    while(!q.empty()) {
        int now = q.front();
        q.pop();
        ans.eb(now + 1);
        a[E[now].from]++, a[E[now].to]++;
        if(a[E[now].from] == 0) add(E[now].from);
        if(a[E[now].to] == 0) add(E[now].to);
    }
    if(ans.size() == k) {
        cout << "ALIVE\n";
        reverse(all(ans));
