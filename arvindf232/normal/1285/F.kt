...
fun mobiusFactors(v:Int):IntArray {
    val ret = mint
    ret.add(1)
    v.eachPrimePower { p, e ->
        ret.addAll(ret.map { it * p })
    }
...
fun main(){
    ...
    val mu = Array(max){if(it == 0) intArrayOf() else mobiusFactors(it) }

    solve.cases{
        val n = getint
        val L = getline(n)
        val has = BooleanArray(max)
        var ret = 0L
        for(a in L){
            has[a] = true
        }
        val facts = IntArray(max)
        val dot = IntArray(max)
        var coprime = 0
        fun removeLeft(s:Int){
            for(v in mu[s]) {
                facts[v] --
                if(dot[v] == 1){
                    coprime --
        ...
        fun addRight(s:Int){
            for(v in mu[s]){
                dot[v] = sieve.mobius[v]
                if(dot[v] == 1) coprime += facts[v]
                else coprime -= facts[v]
            }
        }
        for(g in 1 until max){
            var left = 1
            var right = ((max-1) / g)

            var s = 0
            for(gs in g until max step g){
                s ++
                if(has[gs]){
                    for(v in mu[s]){
                        facts[v] ++
            ...
            while(left <= right){
                ...
                left = bestleft + 1
                ret = maxOf(right.toLong() * bestleft * g, ret)
                removeRight(right)
                removeLeft(right)
                right --
            }
...
/*
largest coprime products, in O(x)
hard to be independent
unless O(1)

or jsut sieve it out

two points coprime in O(max) is doable ?

maintain gcd array hard
maintain copirme array hard
