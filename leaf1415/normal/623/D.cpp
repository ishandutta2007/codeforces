...
ll n;
ll p[105];
double q[105], qbeki[105];
double lmul[105], rmul[105];
...
int main(void)
{
	...
	cin >> n;
	rep(i, 1, n) cin >> p[i];
	rep(i, 1, n) q[i] = (100-p[i])/100.0, qbeki[i] = 1;
	
	ll M = 300000;
	double ans = 0;
	rep(i, 1, M){
		lmul[0] = 1, rmul[n+1] = 1;
		rep(j, 1, n) lmul[j] = lmul[j-1] * (1-qbeki[j]);
		per(j, n, 1) rmul[j] = rmul[j+1] * (1-qbeki[j]);
		
		double mx = -1; ll mx_j;
		rep(j, 1, n){
			double p = lmul[j-1] * rmul[j+1] * (1-qbeki[j]*q[j]);
			if(mx < p) mx = p, mx_j = j;
		}
		if(i <= n) mx_j = i;
		qbeki[mx_j] *= q[mx_j];
