...
int main(int argn, char **argv) {
    ...
    const int num = 26;
    vector<vector<int>> positions(num, vector<int>());
    REP(i, n) {
        positions[str[i] - 'a'].eb(i);
    }
    ...
    REP(i, n) {
        ...
        int endP = n-1-i;
        // endP -= check(BIT, endP);
        int startP = positions[ch - 'a'].back();
        startP -= check(BIT, startP);

        // cout << endP << " " << startP << endl;
        
        edit(BIT, positions[ch - 'a'].back()+1, n);
        positions[ch - 'a'].pop_back();
