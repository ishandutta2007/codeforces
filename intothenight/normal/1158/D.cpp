...
int main(){
	...
	int p = *min_element(rem.begin(), rem.end(), [&](int i, int j){ return a[i] < a[j]; });
	rem.erase(find(rem.begin(), rem.end(), p));
	cout << p + 1 << " ";
	for(auto rep = 0; rep < n - 2; ++ rep){
		p = s.back() == 'L' ? *min_element(rem.begin(), rem.end(), [&](int i, int j){ return doubled_signed_area(a[p], a[i], a[j]) > 0; }) : *max_element(rem.begin(), rem.end(), [&](int i, int j){ return doubled_signed_area(a[p], a[i], a[j]) > 0; });
		rem.erase(find(rem.begin(), rem.end(), p));
		s.pop_back();
		cout << p + 1 << " ";
	}
