...
LL Add(int i, int l, int r, int a, int b, LL x) {
	...
	LL ret = Infll; Pushdown(i), ret = std::min(ret, Add(ls, a, b, x)), ret = std::min(ret, Add(rs, a, b, x));
	return mn[i] = std::min(mn[li], mn[ri]), ret;
}
LL Get(int i, int l, int r, int p) {
	if (l == r) return Z[t2[i]] - t1[i]; Pushdown(i);
...
int main() {
	Init(), scanf("%d%d", &N, &Q);
	for (int i = 1, x; i <= N; ++i)
		scanf("%d", &x), Mdf(1, 1, N, i, i, Z[gv(x)] - x, gv(x));
	while (Q--) {
		int o, l, r, x;
		scanf("%d%d", &o, &l);
		if (o == 1) printf("%lld\n", Get(1, 1, N, l));
		if (o == 2) scanf("%d%d", &r, &x), Mdf(1, 1, N, l, r, Z[gv(x)] - x, gv(x));
		if (o == 3) {scanf("%d%d", &r, &x); while (!Add(1, 1, N, l, r, x));}
	}
	return 0;
}