...
class DRearrange {
public:
    bool isBitonic(const vector<int> &A) {
//        cerr << "isBitonic" << A << endl;
        int N = A.size();
        for (int i = 0; i < N-2; ++i) {
...
//        cout << X << Y;
        ...
        vector2<int> Ans(N, M, -1);
        vector<vector<int>> E(N*M);
        vector<int> IDX(N*M, -1);

        auto addRow = [&](int r, int c) {
            for (int i = 0; i < c; ++i) {
                E[r*M + i + 1].push_back(r*M + i);
            }
        ...
        auto addColumn = [&](int r, int c) {
            for (int i = 0; i < r; ++i) {
                E[i*M + c + M].push_back(i*M + c);
            }
...
//            cerr << r << ' ' << N << ' ' << c << ' ' << M << endl;
            ...
            if (isR) addRow(r-1, c-1);
            if (isC) addColumn(r-1, c-1);
            if (isR || isC) {
                Ans[r-1][c-1] = h;
                IDX[h-1] = (r-1)*M + (c-1);
                C[IDX[h-1]] += 100;
        ...
        for (int h = N*M; h >= 1; --h) {
            if (IDX[h-1] != -1) {
                for (int j: E[IDX[h-1]]) if (!--C[j]) Q.push_back(j);
            } else {
                int q = Q[idx++];
                int r = q/M, c = q%M;
                Ans[r][c] = h;
