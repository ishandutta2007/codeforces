...
int main()
{
    ...
    static ll n,m,dis[2][22][N],mn[2][N],mnsup[2][N];
    static V(ll) ad[2][N];
    ...
    priority_queue<pair<pll,pll>> pr;
    REP(t,0,2)
    {
        REP(i,0,n)
        {
    ...
    pr.push(MP(MP(0,0),MP(0,0)));
    while(!pr.empty())
    {
        pair<pll,pll> t=pr.top();
        pr.pop();

        if(mn[t.S.F][t.S.S]!=-1) continue;

        mn[t.S.F][t.S.S]=-t.F.F;
        mnsup[t.S.F][t.S.S]=-t.F.S;

        for(auto it:ad[t.S.F][t.S.S])
        {
            if(mn[t.S.F][it]==-1) pr.push(MP(MP(t.F.F,t.F.S-1),MP(t.S.F,it)));
        }

        if(mn[t.S.F^1LL][t.S.S]==-1) pr.push(MP(MP(t.F.F-1,t.F.S),MP(t.S.F^1LL,t.S.S)));
    }

    if(min(mn[0][n-1],mn[1][n-1])>=20)
    {
        if(mn[0][n-1]<mn[1][n-1]||(mn[0][n-1]==mn[1][n-1]&&mnsup[0][n-1]<mnsup[1][n-1]))
        {
            cout<<((powM(2,mn[0][n-1],M2)+M2-1+mnsup[0][n-1])%M2);
        }
        else
        {
            cout<<((powM(2,mn[1][n-1],M2)+M2-1+mnsup[1][n-1])%M2);
        }
    }
    else
    {
        ...
        priority_queue<pair<ll,tuple<ll,ll,ll>>> pr2;

        pr2.push(MP(0,MT(0,0,0)));
        while(!pr2.empty())
        {
            pair<ll,tuple<ll,ll,ll>> t=pr2.top();
            pr2.pop();

            if(dis[G(0,t.S)][G(1,t.S)][G(2,t.S)]!=-1) continue;

            dis[G(0,t.S)][G(1,t.S)][G(2,t.S)] = -t.F;

            for(auto it:ad[G(0,t.S)][G(2,t.S)])
            {
                if(dis[G(0,t.S)][G(1,t.S)][it]==-1) pr2.push(MP(t.F-1,MT(G(0,t.S),G(1,t.S),it)));
            }

            if(G(1,t.S)<21) if(dis[G(0,t.S)^1LL][G(1,t.S)+1][G(2,t.S)]==-1) pr2.push(MP(t.F-powM(2,G(1,t.S),M2),MT(G(0,t.S)^1LL,G(1,t.S)+1,G(2,t.S))));
        }
        ...
        REP(t,0,2)
        {
            REP(j,0,22)
            {
                if(dis[t][j][n-1]!=-1&&(ans==-1||ans>dis[t][j][n-1]))
                {
                    ans=dis[t][j][n-1];
                }
            }
