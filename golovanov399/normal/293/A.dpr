...
begin
 ...
 for i := 1 to n do
 begin
  if ((s[i] = '1') and (t[i] = '1')) then inc(a) else
  if ((s[i] = '1') and (t[i] = '0')) then inc(b) else
  if ((s[i] = '0') and (t[i] = '1')) then inc(c);
 end;
 a := a mod 2;
 if a + b > c then writeln('First') else
 if a + b < c - 1 then writeln('Second') else
 writeln('Draw');
end.