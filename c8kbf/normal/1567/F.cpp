...
_ n, m, dx[] = {1, 0, -1, 0}, dy[] = {0, 1, 0, -1}, o[maxn][maxn];
bool a[maxn][maxn];
v(_p) g[maxn][maxn];

void dfs(_ x, _ y, _ c);
inline void AC() {
    
    n = read();
    m = read();
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) a[i][j] = gc() == 'X';
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) if(a[i][j]) {
        v(_p) c;
        for(_ k = 0; k < 4; ++k) {
            _ nx = i+dx[k], ny = j+dy[k];
            if(nx <= 0 || ny <= 0 || nx > n || ny > m) continue;
            if(!a[nx][ny]) c.push_back(mp(nx, ny));
        }
        ...
        if(c.size() == 2) {
            o[i][j] = 5;
            g[c[0].first][c[0].second].push_back(c[1]);
            g[c[1].first][c[1].second].push_back(c[0]);
        } ef(c.size() == 4) {
            o[i][j] = 10;
            c.push_back(c[0]);
            for(_ k = 0; k < 4; ++k) {
                g[c[k].first][c[k].second].push_back(c[k+1]);
                g[c[k+1].first][c[k+1].second].push_back(c[k]);
            }
        }
    }
    for(_ i = 1; i <= n; ++i) for(_ j = 1; j <= m; ++j) if(!a[i][j] && !o[i][j]) dfs(i, j, 1);
    yes(7);
    for(_ i = 1; i <= n; ++i, clr()) for(_ j = 1; j <= m; ++j) writesc(o[i][j]);
...
void dfs(_ x, _ y, _ c) {
    o[x][y] = c;
    for(_p i : g[x][y]) if(!o[i.first][i.second]) dfs(i.first, i.second, c == 1 ? 4 : 1);
    return;
}