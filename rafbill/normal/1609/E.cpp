...
struct node {
  ...
  void set(char c) {
    empty = 0;
    for(li i = 0; i < (li)(3); ++i)for(li j = 0; j < (li)(3); ++j)for(li k = 0; k < (li)(2); ++k)cost[i][j][k] = INFTY;
    if(c=='a') {
      cost[1][0][0] = 0;
      cost[0][1][0] = 1;
      cost[0][0][1] = 1;
  ...
  void combine(node const& a, node const& b) {
    if(b.empty) { *this = a; return; }
    if(a.empty) { *this = b; return; }
    empty = false;
    for(li i = 0; i < (li)(3); ++i)for(li j = 0; j < (li)(3); ++j)for(li k = 0; k < (li)(2); ++k)cost[i][j][k] = INFTY;
    for(li ia = 0; ia < (li)(3); ++ia) for(li ja = 0; ja < (li)(3); ++ja) for(li ka = 0; ka < (li)(2); ++ka) {
      for(li ib = 0; ib < (li)(3); ++ib) for(li jb = 0; jb < (li)(3); ++jb) for(li kb = 0; kb < (li)(2); ++kb) {
        if(ia+jb >= 3) continue;
        int ic = max({(ia==1 && kb==1) ? 2 : ia, ib});
        int jc = max({ja, (jb == 1 && ka==1) ? 2 : jb});
        int kc = max(ka,kb);
        smin(cost[ic][jc][kc], a.cost[ia][ja][ka]+b.cost[ib][jb][kb]);
      }
    }
  }
  ...
  li answer() {
    li r=INFTY;
    for(li i = 0; i < (li)(3); ++i)for(li j = 0; j < (li)(3); ++j)for(li k = 0; k < (li)(2); ++k)smin(r,cost[i][j][k]);
    return r;
  }
};
...
struct st {
  ...
  void set_(int i, int a, int b, int x, char d) {
    if(a == b) {
      A[i].set(d);
      return;
    }
...
int main(){
  ...
  string s; cin>>s;
  st S(n);
  for(li i = 0; i < (li)(n); ++i) S.A[S.n+i].set(s[i]);
  S.build();
