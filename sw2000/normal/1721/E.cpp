...
void solve() {
  ...
  while (q--) {
    ...
    cin >> a;
    s += a;
    for (int i = n, j = nxt[n - 1]; i < s.size(); i++) {
      int tot = 12;
      while (--tot && j != -1 && s[i] != s[j + 1]) j = nxt[j];
      if (!tot) {
        if (h.count(s.substr(n, i - n + 1))) {
          j = h[s.substr(n, i - n + 1)];
        } else {
          j = -1;
          for (int k = n; k <= i; k++) {
            if (s.substr(k, i - k + 1) == s.substr(0, i - k + 1)) {
              j = i - k;
              break;
            }
          }
