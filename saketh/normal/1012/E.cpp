...
int main() {
    ...
    int N, S; re(N, S); vi a(N); re(a);

    vi ti(N);
    vi st = a, did(N); sort(all(st));
    vvi occ(N);
    F0R (i, N) if (a[i] != st[i]) {
        int w = index(st, a[i]);
        while (a[w + did[w]] == st[w + did[w]])
            did[w]++;
        ti[i] = w + did[w]++;
        occ[w].pb(i);
    } else ti[i] = i;
    ...
    F0R (i, N) F0R (j, sz(occ[i]) - 1) {
        if (uf.unio(occ[i][j], occ[i][j+1])) {
            swap(ti[occ[i][j]], ti[occ[i][j+1]]);
        }
    }

    int wr = 0; F0R (i, N) if (a[i] != st[i]) wr++;
    if (wr > S) { ps(-1); return 0; }
    if (a == st){ ps(0); return 0; }

    vvi cyc;
    F0R (i, N) if (i == uf.rep(i) && i != ti[i]) {
        cyc.pb({i+1});
        for (int t = ti[i]; t != i; t = ti[t])
            cyc.back().pb(t+1);
    }

    int merge = min(S - wr, sz(cyc));
    if (merge > 2) {
        vi loop, fix;
        FOR (c, sz(cyc) - merge, sz(cyc)) {
            loop.insert(loop.end(), all(cyc[c]));
