...
int main(){
	...
	cauto n=read(0);
	cauto m=read(0);
	cauto mp=readT(n,m,0);
	auto dp=table(n,m,pii(MOD,-MOD));
	dp[0][0]=pii();
	rep(i,n)rep(j,m){
		if(i){
			MN(dp[i][j].X, dp[i-1][j].X);
			MX(dp[i][j].Y, dp[i-1][j].Y);
		}
		if(j){
			MN(dp[i][j].X, dp[i][j-1].X);
			MX(dp[i][j].Y, dp[i][j-1].Y);
		}
		dp[i][j].X+=mp[i][j];
		dp[i][j].Y+=mp[i][j];
	}
	cauto &[l,r]=dp[n-1][m-1];
	cout<<(n%2!=m%2&&l<=0&&0<=r ? "YES" : "NO") NL;
}
	return 0;
}