...
int main() {
    ...
    while(n--) { int l; cin >> l; int r; cin >> r; v.emplace_back(l, r); }
    for(int i = 0; i < (30); ++i) {
        amogus_map<int, int> ev; int m = 0, x = (1 << i);
        for(pair<int, int> p : v)
            if(p.second - p.first + 1 >= x) m++;
            else {
                int a = p.first % x, b = p.second % x;
                if(a <= b) ev[a]++, ev[b + 1]--;
                else ev[0]++, ev[b+1]--, ev[a]++, ev[x]--;
            }
        vector<pair<int, int> > seen; for (auto o:ev) seen.push_back(o);
    ...
    int q; cin >> q; while(q--) {
        int w; cin >> w; int i = 0; while(!((w >> i) & 1)) i++;
        cout << ans[i] << '\n';
    }
}