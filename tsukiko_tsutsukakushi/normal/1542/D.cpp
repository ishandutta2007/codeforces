...
void solve() {
    ...
    rep(i, n) {
        if(c[i] == '+') {
            rep(a, n + 1) rep(b, n + 1) rep(f, 2) dp[a][b][f] = 0;
            dp[0][0][0] = 1;
            rep(a, n) {
                if(a != i) {
                    if(c[a] == '-') {
                        rep1(j, 0, a) {
                            rep(f, 2) {
                                // do -
                                if(j - 1 >= 0) dp[a + 1][j - 1][f] += dp[a][j][f];
                                else dp[a + 1][j][0] += dp[a][j][f];
                                // do nothing
                                dp[a + 1][j][f] += dp[a][j][f];
                            }
                        }
                        ...
                        bool ok = (x[a] < x[i]);
                        if(x[a] == x[i] and a < i) ok = 1;
                        rep1(j, 0, a) {
                            rep(f, 2) {
                                // do +
                                if(ok) dp[a + 1][j + 1][f] += dp[a][j][f];
                                else dp[a + 1][j][f] += dp[a][j][f];
                                // do nothing
                                dp[a + 1][j][f] += dp[a][j][f];
                            }
                        }
