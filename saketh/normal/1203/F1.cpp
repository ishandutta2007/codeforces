...
int main() {
    ...
    const int MAXV = 30000 + 100 * 300 + 1;
    vi dp(MAXV), DP(MAXV);

    sort_by(lose, a.f - a.s < b.f - b.s);

    trav (pr, lose) {
        DP = dp;
        FOR (sk, max(pr.f, pr.s), sz(DP)) ckmax(DP[sk], 1 + dp[sk-pr.s]);
        swap(dp, DP);
    }

    ps(ans + dp[R] == N ? "YES" : "NO");
