...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		for (int i: range(n)) {
			...
			in >> a;
			if (a >= n - m + 1) {
				big_positions.push_back(i);
			}
		}

		assert(big_positions.size() == m);

		using Z = ZnConst<998244353>;

		Z ans = Z::rawValueOf(1);

		for (int i: range(m - 1)) {
			ans *= big_positions[i + 1] - big_positions[i];
		}

		out << (n + n - m + 1) * 1LL * m / 2 << " " << ans << "\n";
	}
};
...
int main() {
	...
	CRazbieniyaPerestanovki solver;
