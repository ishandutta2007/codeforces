...
ll n, m, m2;
UnionFind uf(100005), uf2(100005);
map<ll, ll> mp[100005];
...
int main(void)
{
	...
	rep(i, 1, n) mp[uf.root(i)][uf2.root(i)] = i;
	
	//rep(j, 1, n) cout << j << ": ", outl(mp[j]);
	
	vector<P> ans;
	map<ll, ll> cmp = mp[uf.root(1)];
	rep(i, 2, n){
		...
		map<ll, ll> tmp;
		for(auto p : mp[r]) tmp[uf2.root(p.first)] = p.second;
		if(sz(cmp) == 1 && sz(tmp) == 1 && cmp.begin()->first == tmp.begin()->first) continue;
		
		if(sz(tmp) >= 2){
			u = cmp.begin()->second;
			for(auto p : tmp){
				if(uf2.root(u) != p.first){
					v = p.second;
					break;
				}
			}
		...
		assert(uf2.root(u) != uf2.root(v));
		
		ll uroot = uf2.root(u), vroot = uf2.root(v);
		
		uf2.unite(u, v);
		ans.push_back(P(u, v));
		for(auto p : tmp) cmp[p.first] = p.second;
		
		if(uf2.root(uroot) == uroot) cmp.erase(vroot);
		else cmp.erase(uroot);
		
		/*set<ll> S;
		rep(i, 1, n) S.insert(uf2.root(i));
		outl(S);*/
	}
