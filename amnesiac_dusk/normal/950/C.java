...
public class Main
{
	public static void main(String[] args) throws IOException
	{
		String s=ns();
		boolean pos=true;
		vecti[]kehai=new vecti[200000];
		vecti[]shikyunknihuryo=new vecti[100000];
		int hola=0,kola=0;
		for(int i=0; i<s.length(); i++)
		{
			if(s.charAt(i)=='0')
			{
				if(kola!=0)
				{
					vecti te=shikyunknihuryo[--kola];
					te.add(i+1);
					kehai[hola++]=te;
				}
				else
				{
					vecti te=new vecti(1);
					te.add(i+1);
					kehai[hola++]=te;
				}
			}
			else
			{
				...
				{
					vecti hol=kehai[--hola];
					hol.add(i+1);
					shikyunknihuryo[kola++]=hol;
				}
			}
		}
		if(pos&&(kola==0))
