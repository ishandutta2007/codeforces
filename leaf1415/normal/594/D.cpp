...
ll cnt[1000005];
mint val[1000005], val2[1000005];
vector<ll> fvec[200005];
mint ret = 1;
...
struct Mo{
	...
	void setArray(vector<ll> &src){
		vec = src;
		B = 1400;//sqrt((int)src.size());
	}
	...
	void add(ll x, bool left)
	{
		for(auto p : fvec[x]){
			cnt[p]++;
			if(cnt[p] == 1) ret *= val[p];
		}
	}
	void erase(ll x, bool left)
	{
		for(auto p : fvec[x]){
			cnt[p]--;
			if(cnt[p] == 0) ret *= val2[p];
		}
	}
	ll get()
	{
...
ll n, Q;
ll a[200005];
ll l[200005], r[200005], qid[200005];
mint mul[200005];
Mo mo;
...
int main(void)
{
	...
	make_prime(1000005);
	rep(i, 1, 1000000){
		val[i] = mint(1) - mint(i).inverse();
		val2[i] = val[i].inverse();
	}
	...
	mul[0] = 1;
	rep(i, 1, n) mul[i] = mint(a[i]);
	rep(i, 1, n) mul[i] *= mul[i-1];
	
	map<ll, ll> mp;
	rep(i, 1, n){
		ll t = a[i];
		while(prime[t]){
			fvec[i].push_back(prime[t]);
			t /= prime[t];
		}
		uniq(fvec[i]);
	}
	...
	rep(i, 1, Q){
		mint ret = mul[r[i]] / mul[l[i]-1];
		ret *= mo.getAnswer(qid[i]);
		cout << ret << "\n";
	}
