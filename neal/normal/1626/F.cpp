...
const int BASE = 720720;

int main() {
    int N, A0, X, Y, K, M;
    cin >> N >> A0 >> X >> Y >> K >> M;
    barrett_reduction barrett(M);
    vector<int64_t> A(N);
    A[0] = A0;

    for (int i = 1; i < N; i++)
        A[i] = barrett(A[i - 1] * X + Y);

    mod_int inv_N = mod_int(N).inv();
    vector<mod_int> prob(BASE, 0);
    mod_int div_sum = 0;
    ...
    mod_int avg = div_sum * inv_N;
    mod_int total = avg * K;

    for (int k = 1; k <= K; k++)
        for (int x = 0; x < BASE; x++) {
            total += x * prob[x];
            mod_int change = prob[x] * inv_N;
            prob[x] -= change;
            prob[x - x % k] += change;
        }

    total *= mod_int(N).pow(K);
    cout << total << '\n';
}