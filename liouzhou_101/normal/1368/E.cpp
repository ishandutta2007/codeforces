...
void solve()
{
	...
	vector<int> u(n + 1, 0), go(n + 1, 0), die(n+1, 0);
	vector<set<int>> diex(n+1), diey(n+1);
	for (int x = 1; x <= n; ++x)
	{
		for (auto y : v[x])
			for (auto z : v[y])
	...
	{
		int flag = 0;
		for (auto x : diex[z]) if (!die[x])
		{
			for (auto y : v[x]) if (!die[y])
				if (diey[z].count(y)) flag = 1;
		}
		if (flag == 1)
		{
			die[z] = 1;
