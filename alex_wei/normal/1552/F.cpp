...
ll n,m,x[N],y[N],s[N],ans;

ll c[N];
void add(ll x,ll v){while(x<=n)c[x]=(c[x]+v)%mod,x+=x&-x;}
ll query(ll x){ll s=0; while(x)s+=c[x],x-=x&-x; return s%mod;}
...
int main(){
	...
	for(int i=n;i;i--){
		...
		int tm=query(i);
		if(i<n)ans=(ans+(x[i+1]-x[i])*tm)%mod;
		ans=(ans+(x[p]-y[i])*(tm+s[i]))%mod;
		add(p,tm+s[i]);
	}
