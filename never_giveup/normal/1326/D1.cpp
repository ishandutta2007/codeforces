...
const int maxn = 2e5 + 50, maxw = 1e6 + 100, inf = 1e9 + 100, sq = 100, LG = 18;

vector<int> man(int n, string &s) {
    vector<int> d1 (n);
    int l=0, r=-1;
    for (int i=0; i<n; ++i) {
...
vector<int> man1(int n, string &s) {
    vector<int> d2 (n);
    int l=0, r=-1;
    for (int i=0; i<n; ++i) {
...
int main() {
    ...
    while (tst--) {
        ...
        cin >> s;
        int n = s.size();
        vector<int> a = man(n, s), b = man1(n, s);
        int len = 0;
        for (int i = 0; i < n; i++)
            if (s[i] != s[n - 1 - i]) {
                break;
