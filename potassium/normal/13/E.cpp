...
const int SQRT = 500;
int n,m;
int to[MAXN], nxt[MAXN], cnt[MAXN], last[MAXN];

void build(int l, int r) {
	for (int i = r; i >= l; i--) {
		if (to[i] == n || to[i] / SQRT != i / SQRT) {
			nxt[i] = to[i];
			cnt[i] = 1;
		}
		else {
		...
		last[i] = (to[i] == n ? i : last[to[i]]);
	}
}

int main() {
	R(n, m);
	for (int i = 0; i < n; i++) { R(to[i]); to[i] = min(to[i] + i, n); }
