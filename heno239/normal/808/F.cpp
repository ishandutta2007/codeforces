...
void solve() {
	...
	auto can = [&](int x) {
		...
		rep(i, n) {
			if (l[i] <= x) {
				if (c[i] == 1)ones.push_back(p[i]);
				else {
					rest.push_back({ c[i],p[i] });
				}
			}
		}
		sort(all(ones));
		...
		if (ones.size()) {
			int val = ones.back();
			Dinic dc(rest.size() + 2);
			int s = rest.size(); int t = s + 1;
			rep(i, rest.size()) {
				if (isp[rest[i].first + 1])continue;
				val += rest[i].second;
				if (rest[i].first % 2) {
					dc.addedge(s, i, rest[i].second);
				}
				else {
					dc.addedge(i, t, rest[i].second);
				}
			}
			rep(i, rest.size())if (rest[i].first % 2) {
				rep(j, rest.size())if (rest[j].first % 2 == 0) {
					if (isp[rest[i].first + rest[j].first]) {
						dc.addedge(i, j, mod);
					}
				}
			}
