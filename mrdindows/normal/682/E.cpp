...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            if (j == rp) ++rp;
            if (rp == n) rp = 0;
            ll curs = calcS(v[i], v[j], v[rp]);
            while (true) {
                int t = rp + 1;
                if (t == n) t = 0;
                ll nexts = calcS(v[i], v[j], v[t]);
                if (nexts >= curs) {
                    rp = t;
                    curs = nexts;
                } else {
    ...
    pt f[3] = {v[res.a], v[res.b], v[res.c]};
    pt ans[3] = {
        {f[0].x + f[2].x - f[1].x, f[0].y + f[2].y - f[1].y},
        {f[1].x + f[0].x - f[2].x, f[1].y + f[0].y - f[2].y},
        {f[2].x + f[1].x - f[0].x, f[2].y + f[1].y - f[0].y}};
    FI(3) {
