...
int main() {
    int q; read(q);
    while (q--) {
        ll n, m, c, c0; read(n, m, c, c0);
        vector f(n, array{0, 0, 0});
        read(f);
        sort(all(f));
        f.push_back({int(m), 0, 0});
        ...
        for (auto& [ti, a, b] : f) {
            ...
            while (sz(avail) && c0 < dt) {
                auto& [bi, ai] = *begin(avail);
                ll x = min(ll(dt - c0), ai);
                cost += x * bi;
                c0 += x;
                sum_avail -= x;
                ai -= x;
            ...
            avail[b] += a;
            sum_avail += a;
            while (c0 + sum_avail > c) {
                auto& [bi, ai] = *rbegin(avail);
                ll x = min(ai, c0 + sum_avail - c);
                sum_avail -= x;
                ai -= x;
                if (!ai)
