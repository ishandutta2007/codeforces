...
vector<string> g(1007, string(1007, 'N'));
int now = 3;
vector<vector<int>> node(50, vector<int>(50, -1));
int getNewNode() {
	return now++;
}
int getNode(int dep, int num) { //up to num = dep, val = 2^num
	if (node[dep][num] > -1)
		return node[dep][num];
	else
		return node[dep][num] = getNewNode();
}
...
int maxDep = 32;
void build(int dep) {
	if (dep == maxDep+1) return;
	
	addedge(getNode(dep, 0), getNode(dep-1, 0));
	
	for (int i = 1; i <= dep; i++)
		for (int j = 0; j < i; j++)
			addedge(getNode(dep, i), getNode(dep-1, j));
...
int main() {
	...
	for (int bit = 0; bit < 33; bit++)
		if ((k >> bit) & 1)
			addedge(getNode(maxDep, bit+1), 2);
