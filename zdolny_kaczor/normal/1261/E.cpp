...
int main() {
	...
	loc(memcpy(acopy, a, n * sizeof(a[0])));
	vector <bitset<nax> > ans(n + 1);
	for (int i = 0; i < n; ++i) {
		int start = 0;
		for (int j = 0; j < n; ++j) {
			if (H[j] == H[j + 1]) {
		...
		int siz = 0;
		while (start + siz < n && H[start + siz] == H[start + siz + 1])
			siz++;
		start = max(start + 1, start + siz - a[i] + 1);
		ull h = mix(i + SALT);
		for (int j = 0; j < a[i]; ++j) {
			int v = (start + j) % (n + 1);
			H[v] ^= h;
...
#ifdef LOCAL
	for (int i = 0; i < (int) ans.size(); ++i)
		for (int j = 0; j < i; ++j) assert(ans[i] != ans[j]);
	for (auto b : ans)
		for (int i = 0; i < n; ++i)
			if (b[i])
				acopy[i]--;
	for (int i = 0; i < n; ++i) assert(acopy[i] == 0);
#endif
}