...
int main(){
	...
	REP(i, 1, n){
		if(A[i] == A[i-1]){
			...
			// std::cout << "mialem " << co << " tera " << mx+1 << std::endl;
			int delt = std::max(co, mx+1) - co;
			// std::cout << "dodaje " << delt << std::endl;
			tree.add(A[i], A[i], delt);
			// std::cout << "jest " << tree.query(A[i], A[i]) << std::endl;
		}else{
			tree.add(A[i], A[i], -1);
			int mx = tree.query(0, n);
			tree.add(A[i], A[i], 1);
