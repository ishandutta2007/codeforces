...
vector<vector<pii>> G;
vector<int> a;
vector<pnode> dist(200010, nullptr);
vector<int> link;
 
vector<int> ans;
 
void dfs(int v = 0, ll d = 0) {
    for (auto i : G[v])
        dfs(i.f, d + i.s);
    sort(all(G[v]), [](const pii &a, const pii &b) {
        return getcnt(dist[link[a.f]]) > getcnt(dist[link[b.f]]);
    });
    for (int i = 1; i < sz(G[v]); ++i)
        insert(dist[link[G[v][i].f]], dist[link[G[v][0].f]]);
    if (sz(G[v])) link[v] = link[G[v][0].f];
    else link[v] = v;
    ans[v] = upper_bound(dist[link[v]], d);
    insert(dist[link[v]], d - a[v]);
}
