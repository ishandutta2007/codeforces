...
int main(){
	...
	auto __solve_tc = [&](auto __tc_num)->int{
		...
		auto fillable = [&](int obj)->bool{
			int to_fill = obj - ranges::count_if(cnt, [&](auto p){ return p.first < obj; });
			int rem = 0;
			for(auto [x, c]: cnt){
				rem += c - (x < obj);
			}
			debug(obj, to_fill, rem);
			return to_fill <= k && to_fill <= rem;
		};
		int obj_mex = *ranges::partition_point(ranges::iota_view(0, n + 2), fillable) - 1;
		debug(obj_mex);
		vector<int> b;
		for(auto [x, c]: cnt){
			if(x >= obj_mex){
