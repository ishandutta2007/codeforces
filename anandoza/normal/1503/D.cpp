...
int main() {
    ...
    {
        int min_right = n, max_left = -1;
        for (auto &[l, r] : a) {
            ckmax(max_left, min(l, r));
            ckmin(min_right, max(l, r));
        }
    ...
    vi flipped(n);
    for (int i = 0; i < n; i++) {
        index[a[i].f] = index[a[i].s] = i;
        flipped[i] = a[i].f > a[i].s;
    }
    ...
    for (int i = n; i < 2 * n; i++)
        bigs.insert(i);
    int last_r = 1e9;
    for (int small = n - 1; small >= 0; small--) {
        int i = index[small];
        int r = max(a[i].f, a[i].s);
        bigs.erase(r);

        auto start = bigs.begin();
        if (last_r < r && !bigs.empty() && *bigs.begin() < last_r) {
            start = prev(bigs.lower_bound(last_r));
        }
        last_r = r;

        for (auto it = start; it != bigs.end() && *it < r; it++) {
            int j = index[*it];
            auto res = uf.unite(i, j, flipped[i] ^ flipped[j] ^ 1);
            if (!res.component_is_bipartite) {
                ps(-1);
                return 0;
            }
        }
    ...
    int ans = uf.min_nodes_on_side_1;
    ps(ans);
