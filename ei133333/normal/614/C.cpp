...
int main()
{
  ...
  for(int i = 0; i < N; i++) {
    MaxDist = max(MaxDist, (p - data[i]).abs());
    MinDist = min(MinDist, Geometory::Distance(Segment(data[i], data[(i + 1) % N]), p));
  }
  cout << fixed << setprecision(10) << (MaxDist * MaxDist - MinDist * MinDist) * PI << endl;
}