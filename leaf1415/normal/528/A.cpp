...
int main(void)
{
	...
	hset.insert(0), hset.insert(h), hmset.insert(h);
	wset.insert(0), wset.insert(w), wmset.insert(w);
	
	char c; ll x;
	rep(i, 1, n){
		cin >> c >> x;
		if(c == 'H'){
			auto r = hset.lower_bound(x), l = r; l--;
			hmset.erase(hmset.lower_bound(*r-*l));
			hmset.insert(*r-x), hmset.insert(x-*l);
			hset.insert(x);
		}
		else{
			auto r = wset.lower_bound(x), l = r; l--;
			wmset.erase(wmset.lower_bound(*r-*l));
			wmset.insert(*r-x), wmset.insert(x-*l);
			wset.insert(x);
		}
		cout << *hmset.rbegin() * *wmset.rbegin() << "\n";
	}
