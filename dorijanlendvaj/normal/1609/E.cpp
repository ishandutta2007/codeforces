...
int q,n;
string s;
array<int,6> seg[M*2+10],id,sa={1,0,0,0,0,0},sb={0,1,0,0,0,0},sc={0,0,1,0,0,0}; //a,b,c,ab,bc,abc

array<int,6> mer(array<int,6> a,array<int,6> b)
{
	array<int,6> c;
	c[0]=a[0]+b[0];
	c[1]=a[1]+b[1];
	c[2]=a[2]+b[2];
	c[3]=min(a[0]+b[3],a[3]+b[1]);
	c[4]=min(a[1]+b[4],a[4]+b[2]);
	c[5]=min(a[3]+b[4],min(a[0]+b[5],a[5]+b[2]));
	return c;
}
...
array<int,6> ge(int l,int r,int lo=0,int hi=M,int i=1)
{
