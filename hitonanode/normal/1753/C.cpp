...
mint solve() {
    ...
    vector<int> A(N);
    cin >> A;
    const int n1 = count(ALL(A), 1);
    const int n0 = N - n1;
    int ndiff = count(A.begin(), A.end() - n1, 1);

    const mint den = (mint(N) * (N - 1) / 2);

    mint ret = 0;
    while (ndiff) { ret += den / (mint(ndiff) * ndiff), --ndiff; }

    // dbg(make_tuple(N, A, n1, ndiff));
    // if (!ndiff) return 0;

    // if (N == 2) return 1;

    // using dual = DualNumber<mint>;
    // const int M = min(n1, n0);
    // vector<dual> dp(M + 1);
    // dp.at(1) = dual(0, 1);
    ...
    // FOR(i, 1, M) {
    //     const mint a = mint(i) * i * deninv;
    //     const mint c = mint(n0 - i) * (n1 - i) * deninv;
    //     const mint b = 1 - a - c;
    //     dbg(make_tuple(a, b, c));
    //     dp.at(i + 1) = (dp.at(i) - mint(1) - dp.at(i - 1) * a - dp.at(i) * b) / c;
    // }

    // {
    //     const mint a = mint(M) * M * deninv;
    //     const mint b = 1 - a;
    //     auto tmp = dp.at(M - 1) * a + dp.at(M) * (b - 1) + mint(1);
    //     mint x = tmp.root();
    //     return dp.at(ndiff).eval(x);
    // }
    // dbg(dp);
