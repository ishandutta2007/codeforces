...
int main() {
	...
	defn defm defk
		vector<vecp> cons(n);
	vec < vec < vec<veci>>> bad(k, vec < vec<veci> > (k, vec<veci>(k, veci(k))));
	fori(i, m) {
		defa defb defc
			a--;
		b--;
	...
	vec < vec<veci>> arr(n, vec<veci>(k, veci(k)));
	fori(i, n) {
		sort(all(cons[i]));
		forin(j, cons[i]) {
			arr[cons[i][j].ss][j][cons[i].size() - 1]++;
		}
	}
	fori(i, n) {
		vecp in;
		...
		forin(a, in) {
			forin(b, in) {
				if(a!=b)
				bad[in[a].ff][in[a].ss][in[b].ff][in[b].ss]=1;
			}
		}
	}
	lol pow = 1;
