...
signed main(){
    read(n); rep(i, 1, n) read(c[i].val), c[i].id = i;
    std :: sort(c + 1, c + n + 1, [](NODE x, NODE y){return x.val < y.val;});
    const int lim = (n / 3) + (n % 3? 1 : 0);
    rep(i, 1, lim) {a[i].id = b[i].id = c[i].id; b[i].val = c[i].val - (a[i].val = i - 1);}
    rep(i, lim + 1, n - lim) {a[i].id = b[i].id = c[i].id; a[i].val = c[i].val - (b[i].val = i - 1);}
    rep(i, n - lim + 1, n) {a[i].id = b[i].id = c[i].id; a[i].val = c[i].val - (b[i].val = n - i);}
    std :: sort(a + 1, a + n + 1, [](NODE x, NODE y){return x.id < y.id;});
    std :: sort(b + 1, b + n + 1, [](NODE x, NODE y){return x.id < y.id;});
    print("YES"); rep(i, 1, n) sprint(a[i].val);
    putcha(10); rep(i, 1, n) sprint(b[i].val);
    putcha(10);
}