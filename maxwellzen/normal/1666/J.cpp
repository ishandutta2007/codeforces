...
void setparent(int i, int j, int par) {
    int k = root[i][j];
    ans[k] = par+1;
    if (k>i) setparent(i, k-1, k);
    if (k<j) setparent(k+1, j, k);
}

void solve() {
    cin >> n;
    p.rsz(n+1, vll(n+1, 0));
    F0R(i, n) F0R(j, n) cin >> p[i+1][j+1];
    FOR(i, 1, n+1) FOR(j, 1, n+1) p[i][j] += p[i-1][j];
    FOR(i, 1, n+1) FOR(j, 1, n+1) p[i][j] += p[i][j-1];
    root.rsz(n, vi(n));
    dp.rsz(n, vll(n, 1e18));
    F0R(i, n) dp[i][i] = 0, root[i][i] = i;
    FOR(d, 1, n) F0R(i, n-d) {
        int j = i+d;
        FOR(k, i, j+1) {
            ll left = 0, right = 0;
            if (k>i) left = dp[i][k-1] + p[k][i] - p[i][i] + p[k][n] - p[i][n] - p[k][k] + p[i][k];
            if (k<j) right = dp[k+1][j] + p[j+1][k+1] - p[k+1][k+1] + p[j+1][n] - p[j+1][j+1] - p[k+1][n] + p[k+1][j+1];
