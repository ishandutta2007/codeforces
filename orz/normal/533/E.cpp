...
const pii P(1000000007, 1000000009);
const pii O(239, 241);
const int N = 100100;
vpii po;
...
void init()
{
	...
	for (int i = 1; i < N; ++i)
	{
		po[i].X = prod(po[i - 1].X, O.X, P.X);
		po[i].Y = prod(po[i - 1].Y, O.Y, P.Y);
	}
}

vpii all_hashes(const string& s)
{
	int n = sz(s);
	vpii right(n + 1);
	for (int i = n - 1; i >= 0; --i)
	{
		right[i].X = prod(right[i + 1].X, O.X, P.X);
		right[i].X = sum(right[i].X, s[i], P.X);

		right[i].Y = prod(right[i + 1].Y, O.Y, P.Y);
		right[i].Y = sum(right[i].Y, s[i], P.Y);
	}

	vpii left(n + 1);
	for (int i = 0; i < n; ++i)
	{
		pii add(prod(s[i], po[i].X, P.X), prod(s[i], po[i].Y, P.Y));

		left[i + 1].X = sum(left[i].X, add.X, P.X);
		left[i + 1].Y = sum(left[i].Y, add.Y, P.Y);
	}
	...
	for (int i = 0; i <= n; ++i)
	{
		...
		for (char c = 'a'; c <= 'z'; ++c)
		{
			...
			add.X = sum(add.X, prod(c, po[i].X, P.X), P.X);
			add.Y = sum(add.Y, prod(c, po[i].Y, P.Y), P.Y);

			add.X = sum(add.X, prod(right[i].X, po[i + 1].X, P.X), P.X);
			add.Y = sum(add.Y, prod(right[i].Y, po[i + 1].Y, P.Y), P.Y);
...
int main()
{
	...
	cout << NN << '\n';
	for (int i = 0; i < NN; ++i)
		cout << static_cast<char>(rng() % 26 + 'a');
	cout << '\n';
	for (int i = 0; i < NN; ++i)
		cout << static_cast<char>(rng() % 26 + 'a');
	cout << '\n';
	...
	*/
	...
	cin >> s >> t;
	init();
	auto as = all_hashes(s), at = all_hashes(t);
	...
	for (int i = 0; i < sz(as); ++i)
	{
		if ((i == 0) || (as[i] != as[i - 1]))
		{
			while ((x < sz(at)) && (at[x] < as[i]))
				++x;
			
			if ((x < sz(at)) && (at[x] == as[i]))
				++a;
		}
	}
