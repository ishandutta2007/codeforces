...
int main()
{
    ...
    vector<bool> leaves(n+1,0);
    vector<bool> visited(n+1,0);
    vector<int> depth(n+1,0);
    vector<double> probability(n+1);
    probability[1]=1.0000000000000000;
    int cnt;
    for(int i=1;i<=n;i++)
    {
        if(adj[i].size()==1) {leaves[i]=1;cnt++;}
    }   
    dfs(adj,visited,1,depth,-1,probability);
    double sum=0;
    //printvector(leaves);
    ...
    for(int i=1;i<=n;i++)
    {
        if(leaves[i]) sum+=probability[i]*depth[i];
    }
