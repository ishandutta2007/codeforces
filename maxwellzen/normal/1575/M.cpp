...
pl intersect(pl a, pl b) {
    if (a.F < b.F) return mp(a.S - b.S, b.F - a.F);
    return mp(b.S - a.S, a.F - b.F);
}
ll eval(pl a, ll x) {return a.F*x + a.S;}
...
void solve() {
    ...
    F0R(i, n+1) {
        cin >> s;
        F0R(j, m+1) grid[i][j] = (s[j]=='1');
    }
    ydiff.rsz(n+1, vi(m+1, 1e9));
    F0R(i, n+1) {
        int cur = -1;
        F0R(j, m+1) {
            if (grid[i][j]) cur = j;
            if (cur>-1) ckmin(ydiff[i][j], j-cur);
        }
        cur = -1;
        R0F(j, m+1) {
            if (grid[i][j]) cur = j;
            if (cur>-1) ckmin(ydiff[i][j], cur-j);
        }
    }
    debug(ydiff);
    ll ans = 0;
    F0R(j, m+1) {
        debug(j);
        lines.clear(); dq.clear();
        F0R(i, n+1) if (ydiff[i][j] < 1e9) {
            pl cur = mp(-2*i, i*i + ydiff[i][j]*ydiff[i][j]);
            debug(cur);
            while (dq.size()>1 && cmp(intersect(cur, *(dq.end()-1)), intersect(*(dq.end()-1), *(dq.end()-2))))
                dq.pop_back();
            dq.push_back(cur);
        }
        F0R(i, n+1) {
            while (dq.size()>1 && eval(dq[0], i) >= eval(dq[1], i)) dq.pop_front();
            debug(eval(dq[0], i) + i*i);
            ans += eval(dq[0], i) + i*i;
        }
    }
    cout << ans << nl;
