...
namespace MAIN {
	...
	inline void MAIN(const int &Case) {
		...
		if(y<=x){
			if(m==0)puts("0");
			else if(m==2&&A[1]+1==A[2])printf("%d\n",min(x,2*y));
			else printf("%lld\n",(LL)y*m/2);
		}
		else {
			for(int i=0;i<=m;i++){
				for(int j=0;j<=m;j++)for(int k=0;k<=1;k++)f[i][j][k]=INF;
			}
			f[0][0][0]=0;
			for(int i=1;i<=m;i++){
				for(int j=0;j<=i/2;j++){
					if(!j)f[i][j][0]=f[i-1][j][0];
					else {
						f[i][j][0]=min(f[i-1][j][0],f[i-1][j][1]);
						f[i][j][1]=f[i-1][j-1][0]+(LL)x*(A[i]-A[i-1]);
					}
				}
			}
			LL ans=INF;
			for(int i=0;i<=m/2;i++){
				ans=min(ans,min(f[m][i][0],f[m][i][1])+(LL)y*(m/2-i));
			}
			printf("%lld\n",ans);
		}
	}
