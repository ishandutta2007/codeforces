...
// rm ${BUILT_PRODUCTS_DIR}/Data && ln -s ${PROJECT_DIR}/JustForCoding/Data/ ${BUILT_PRODUCTS_DIR}/Data
...
int solveSlow(int n) {
    ...
    do {
        ...
        for (int i = 0; i <= a.size(); ++i) {
            ...
            used[x] = true;
            --cnt;
            if (x + 2 < n && used[x + 2] && !used[x + 1]) {
                used[x + 1] = true;
                --cnt;
            }
            if (x - 2 >= 0 && used[x - 2] && !used[x - 1]) {
                used[x - 1] = true;
                --cnt;
            }
        }
...
int dp[2][404][404];
int M;
int go(int n, int len, int lr) {
    if (n <= 0) return len == 0;
    if (len < 0) return 0;
    if (len * 2 + 1 < n) return 0;
    if (len > n) return 0;
    if (n == 1) return (len == 1 && lr < 2) || (len == 0 && lr == 2);
    int& res = dp[n][len][lr];
    if (res != -1) return res;
    res = 0;
    for (int i = 0; i < n; ++i) {
        ...
        int minLenL = (left - 1) / 2;
        int maxLenL = left;
        int minLenR = (right - 1) / 2;
        int maxLenR = right;
        minLenL = max(minLenL, len - 1 - maxLenR);
        minLenL = max(minLenL, 0);
        maxLenL = min(maxLenL, len - 1 - minLenR);
        for (int len_l = minLenL; len_l <= maxLenL; ++len_l) {
            int len_r = len - len_l - 1;
//            if (n == 2 && len == 1 && l == 1 && r == 0) {
//                DBN(i, left, right, len_l, len_r);
//                DBN(go(left, len_l, l, 1), go(right, len_r, 1, r));
//            }
            res = (res + 1LL * go(left, len_l, (lr > 0) + 1) * go(right, len_r, (lr > 1) + 1) % M * cnk[len - 1][len_l]) % M;
        }
    }
    if (n > 0 && len > 0 && res > 0) {
        DBN(n, len, res);
...
int solve() {
        ...
        for (int len = (n - 1) / 2; len <= n; ++len) {
            for (int lr = 0; lr < 2; ++lr) {
                ...
                for (int left = 0; left < n; ++left) {
                    ...
                    for (int len_l = minLenL; len_l <= maxLenL; ++len_l) {
                        int len_r = len - len_l - 1;
                        res += 1LL * dp[1][left][len_l] * dp[lr][right][len_r];
                        if (res >= MM) res -= MM;
                    }
                }
                dp[lr][n][len] = res % M * (len ? powmod(len, M - 2, M) : 1) % M;
            }
        }
    }
    int ans = 0;
    ...
    for (int len = 1; len <= n; ++len) {
        ...
        for (int left = 0; left < n; ++left) {
            ...
            for (int len_l = minLenL; len_l <= maxLenL; ++len_l) {
                int len_r = len - len_l - 1;
                res += 1LL * dp[0][left][len_l] * dp[0][right][len_r];
                if (res >= MM) res -= MM;
            }
        }
        res = res % M * (len ? powmod(len, M - 2, M) : 1) % M;
        ans = (ans + res * fact) % M;
//        ans = (ans + go(n, i, 0)) % M;
    }
    cout << ans << endl;
