...
#endif
char a[1234567];
int t[1000005][27],qwq[27][27];
signed main()
{
	initprog();
	scanf("%s",a+1);int n=strlen(a+1);
	rg(n)a[i]-=96;gr
	rg(n)rg_(j,26)t[i][j]=t[i-1][j];gr t[i][a[i]]++;gr
	rg(n)rg_(k,26)if(k!=a[i])qwq[a[i]][k]+=t[i][k];else qwq[a[i]][a[i]]+=t[i-1][k];gr gr
	int ans=0;
	rg(26)rg_(j,26)mxe(ans,qwq[i][j]);gr gr
	rg(26)mxe(ans,t[n][i]);gr
	oldl(ans);
	return 0;
}