...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        cin >> a[i]; a[i]--;
        if (i > 0) a[i] -= a[i-1];
        if (i&1) odd.set(i/2, a[i]);
        else even.set(i/2, a[i]);
    }
    int q; cin >> q;
    while (q--) {
        int t, l, r; cin >> t >> l >> r;
        if (t == 1) {
            ...
            if (l&r&1) {
                odd.add(l/2, N-1, k);
                even.add((r+1)/2, N-1, -k);
            } else if (l&1)
                odd.add(l/2, (r-1)/2, k);
            else if (r&1)
                even.add(l/2, (r-1)/2, k);
            else {
                even.add(l/2, N-1, k);
                odd.add((r+1)/2, N-1, -k);
            }
        } else {
            int offset = l == 0 ? 0 : (l&1 ? even.get((l-1)/2) : -odd.get((l-1)/2));
            // for (int i = 0; i < n; i++)
            //     debug(i, i&1 ? odd.get(i/2) : even.get(i/2));
            // debug(l, r, offset);
            bool ok = ((r&1) ? odd.get(r/2)+offset : even.get(r/2)-offset) == 0;
            ok = ok && odd.min((l+((l&1)^1))/2, (r-((r&1)^1))/2)+offset >= 0;
            ok = ok && even.min((l+(l&1))/2, (r-(r&1))/2)-offset >= 0;
            cout << (int)ok << "\n";
        }
    }
}