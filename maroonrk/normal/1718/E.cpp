...
pair<vc<A>,vc<A>> gethash(int n,int m,vvc<int> rw){
	vc<A> a(n),b(n),c(m),d(m);
	rep(_,50){
		swap(a,b);
		swap(c,d);
		rep(i,n)rep(k,2)a[i][k]=0;
		rep(j,m)rep(k,2)c[j][k]=0;
		rep(i,n)rep(j,m){
			rep(k,2){
				a[i][k]+=(d[j][k]+1)*z[rw[i][j]][k];
				c[j][k]+=(b[i][k]+1)*z[rw[i][j]][k];
			}
		}
	}
	dmp2(a,c);
...
void slv(){
	...
	dmp(rw1);
	dmp(rw2);
	auto [a,b]=gethash(n,m,rw1);
	auto [c,d]=gethash(n,m,rw2);
