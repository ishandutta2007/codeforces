...
int main()
{
	...
	while (!w.empty())
	{
		...
		if (elem.b.size() > 1)
		{
			auto last_it = prev(elem.b.end());
			elem.sum -= last_it->first;
			elem.b.erase(last_it);

			ans += elem.sum;

			auto &next_elem = w[x + 1];
			next_elem.sum += elem.sum;
			merge(next_elem.b, elem.b);
		}
		w.erase(it);
	}
	cout << ans << endl;
