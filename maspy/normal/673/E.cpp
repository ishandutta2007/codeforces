...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
#line 1 "/home/maspy/compro/library/ds/cht_monotone.hpp"
...
void solve() {
  ...
  auto S = cumsum(A);
  vc<Re> B(N), C(N);
  FOR(i, N) B[i] = S[i + 1] / A[i], C[i] = 1 / A[i];
  B = cumsum(B), C = cumsum(C);

  auto calc = [&](ll L, ll R) -> Re {
    Re a = S[L];
    Re b = S[L] * C[L] - B[L];
    Re x = -C[R];
    return a * x + b + B[R];
  ...
  FOR_(K) {
    vc<Re> newDP(N + 1, INF);
    ConvexHullTrick<Re, true> CHT;
    FOR(i, N + 1) {
      if (i > 0) { newDP[i] = CHT.query_monotone_dec(-C[i]) + B[i]; }
      if (i < N) { CHT.add(S[i], S[i] * C[i] - B[i] + DP[i]); }
    }
    swap(DP, newDP);
  }
  print(DP.back());
