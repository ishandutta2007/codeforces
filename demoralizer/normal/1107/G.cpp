...
void get(int l, int r){
	...
	int split = g.query(l,r-1).maxID();
	int tmp = -gap[split] * gap[split];
	
	tmp += t.query(split+1,r).maxVal() - t.query(l-1,split-1).minVal();
	amax(ans, tmp);
	get(l,split);
	get(split+1,r);
}
