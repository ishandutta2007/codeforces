...
fun main() {
	val n = getint()
	val k = getint()
	val allow = n divCeil (k-1)
	val arr = getline(n * k).map{it - 1 }.toIntArray()

	val retL = IntArray(n)
	val retR = IntArray(n)
	val overlaps = IntArray(n * k)
	val found = BooleanArray(n)
	val endPoint = arr.lastIndex
	repeat(n){ v ->
//		println("y $v")
		val startPoint = overlaps.indexOfFirst { it < allow }
		val lastOccurence = IntArray(n){-1}
		for(i in startPoint..endPoint){
			val a = arr[i]
			if(found[a]){
				continue
			}
			if(lastOccurence[a] != -1){
				retL[a] = lastOccurence[a]
				retR[a] = i
				found[a] = true
//				println("found $a")
				for(c in 0..i){
