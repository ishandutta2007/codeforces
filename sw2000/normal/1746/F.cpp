...
struct TBIT {
  vector<T> a;
  int n;
  TBIT(int n_) : n(n_), a(n_, 0) {}
  void modify(int p, T x) {
    for (p++; p <= n; p += p & -p) a[p - 1] += x;
  }
  T get(int p) {  //
    T ret = 0;
    for (p++; p > 0; p &= p - 1) ret += a[p - 1];
    return ret;
  }
};
using BIT = TBIT<ll>;
void solve() {
  ...
  for (auto& i : ar) {
    i = lower_bound(br.begin(), br.end(), i) - br.begin();
  }
  for (auto& iq : qr) {
    if (iq[0] == 1) {
      iq[2] = lower_bound(br.begin(), br.end(), iq[2]) - br.begin();
    }
  }
  vector<vl> mp(M, vl(br.size()));
  for (auto& i : mp) {
    for (auto& j : i) {
      j = rnd(0, Q);
    }
  }
  vector<BIT> bit(M, BIT(n));
  for (int i = 0; i < M; i++) {
    for (int j = 0; j < n; j++) {
      bit[i].modify(j, mp[i][ar[j]]);
    }
  }
  for (auto& iq : qr) {
    if (iq[0] == 1) {
      for (int i = 0; i < M; i++) {
        bit[i].modify(iq[1], mp[i][iq[2]] - mp[i][ar[iq[1]]]);
      }
      ar[iq[1]] = iq[2];
    } else {
      bool ok = true;
      for (int i = 0; i < M; i++) {
        ll d = bit[i].get(iq[2]) - bit[i].get(iq[1] - 1);
        ok &= (d % iq[3] == 0);
      }
      cout << (ok ? "YES" : "NO") << endl;
    }
