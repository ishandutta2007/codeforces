...
main()
{
    LD px = in, py = in, vx = in, vy = in, a = in, b = in, c = in, d = in;
    LD k = sqrt(vy * vy + vx * vx);
    out.putLn(ofl(px + vx * b / k, 10), ' ', ofl(py + vy * b / k, 10));
    out.putLn(ofl(px - vy * a / 2 / k, 10), ' ', ofl(py + vx * a / 2 / k, 10));
    out.putLn(ofl(px - vy * c / 2 / k, 10), ' ', ofl(py + vx * c / 2 / k, 10));
    out.putLn(ofl(px - vy * c / 2 / k - vx * d / k, 10), ' ', ofl(py + vx * c / 2 / k  - vy * d / k, 10));
    out.putLn(ofl(px + vy * c / 2 / k - vx * d / k, 10), ' ', ofl(py - vx * c / 2 / k  - vy * d / k, 10));
    out.putLn(ofl(px + vy * c / 2 / k, 10), ' ', ofl(py - vx * c / 2 / k, 10));
    out.putLn(ofl(px + vy * a / 2 / k, 10), ' ', ofl(py - vx * a / 2 / k, 10));
    return 0;
