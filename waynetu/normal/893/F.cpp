...
#define M ((L + R) >> 1 )
    ...
    void build(int now, int L, int R, const vector<int> &arr) {
        if (L == R) return v[now] = arr[L], void();
        build(now << 1, L, M, arr); build(now << 1 | 1, M + 1, R, arr);
        v[now] = min(v[now << 1], v[now << 1 | 1]);
    }
    int query(int now, int L, int R, int l, int r) {
        if (L > r || l > R) return inf;
        if (L >= l && R <= r) return v[now];
        return min(query(now << 1, L, M, l, r), query(now << 1 | 1, M + 1, R, l, r));
    }
#undef M
};
...
struct decomp {
    ...
    inline void init(int _n) {
        n = _n;
        int blk = 0;
        // fill(&minv[0][0], &minv[maxn / sq + 9][maxn - 1] + 1, inf);
        for (int i = 1; i <= n; ) {
            ++blk;
            int j = 0;
            ...
            // sp[blk].build(1, 0, n - 1, vec);
        }
        // for (int i = 1; i <= blk; ++i) {
            // for (int j = 0; j < n; ++j) cout << minv[i][j] << ' '; cout << endl;
        // }
        /* for (int i = 1; i <= blk; ++i) {
            vector<int> vec;
            for (int j = 0; j < n; ++j) vec.push_back(minv[i][j]);
            sp[i].init(n, vec);
        } */
    }
