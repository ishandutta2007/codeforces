...
void solve() {
    ...
    for(int i = 1; i * i <= n; ++ i) {
        if(n % i == 0) {
            ds.eb(i);
            if(i * i != n && i != 1) ds.eb(n / i);
        }
    }
    int sz = si(ds);
    vvc<int> g(sz);
    vc<atcoder::segtree<int, op, e>> h(sz);
    vc<int> cur(sz);
    rep(i, sz) {
        int d = ds[i];
        debug(d);
        ...
        h[i] = atcoder::segtree<int, op, e>(d);
        rep(j, d) {
            h[i].set(j, g[i][j] * d);
        }
        cur[i] = h[i].all_prod();
    }
    atcoder::segtree<int, op, e> seg(cur);
    ...
    rep(_, q) {
        ...
        rep(i, sz) {
            ...
            g[i][pos] -= pre;
            g[i][pos] += x;
            h[i].set(pos, g[i][pos] * d);
            int val = h[i].all_prod();
            seg.set(i, val);
            cur[i] = val;
        }
        a[p] = x;
