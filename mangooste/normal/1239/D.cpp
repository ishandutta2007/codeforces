...
void solve() {
    ...
    auto scc = g.scc();
    auto dug = g.build_dug(scc, false);
    if (len(dug) == 1) {
        cout << "No\n";
        return;
    }
    ...
    cout << "Yes\n";
    vector<int> in(len(dug));
    for (const auto &[from, to, cost] : dug.edges)
        in[to]++;

    int who = -1;
    for (int i = 0; i < len(dug); i++)
        if (in[i] == 0) {
            who = i;
            break;
        }
    ...
    for (int i = 0; i < n; i++)
        (scc[i] == who ? second : first).push_back(i);
