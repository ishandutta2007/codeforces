...
class DZookeeperAndTheInfiniteZoo {
 public:
  string tobin(int x) {
    string s = "";
    while (x) {
  ...
  void solveOne() {
    ...
    string s = tobin(u);
    string t = tobin(v);
    vector<vector<bool>> can(s.size() + 1, vector<bool>(t.size() + 1));
    vector<int> first(t.size() + 1, s.size() + 1);
    can[0][0] = true;
    first[0] = 0;
    for (int pt = 1; pt <= t.size(); ++pt) {
      if (t[pt - 1] == '0' && can[0][pt - 1]) {
        can[0][pt] = true;
        first[pt] = 0;
      }
    }
    for (int ps = 1; ps <= s.size(); ++ps) {
      for (int pt = 1; pt <= t.size(); ++pt) {
        if (s[ps - 1] == '0') {
          can[ps][pt] = can[ps - 1][pt];
        } else if (t[pt - 1] == '0') {
          can[ps][pt] = can[ps][pt - 1];
        } else if (ps == pt) {
          can[ps][pt] = can[ps - 1][pt - 1];
        } else if (ps < pt) {
          if (first[pt - 1] < ps) {
            can[ps][pt] = true;
          }
        ...
        if (can[ps][pt]) {
          first[pt] = min(first[pt], ps);
        }
      }
    }
    if (can[s.size()][t.size()]) {
      cout << "YES\n";
    } else {
      cout << "NO\n";
    }
...
int main() {
    ...
    std::cin.tie(0);
    DZookeeperAndTheInfiniteZoo solver;
