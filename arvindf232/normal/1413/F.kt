...
class lazyBuildableSegTree (nSuggest  :Int){
    ...
    val best0L = IntArray(n * 2)
    val best1L = IntArray(n * 2)
    val best0R = IntArray(n * 2)
    val best1R = IntArray(n * 2)
    val lazy = IntArray(n) // lazy : already applied to this node

    constructor(withArray:IntArray, d1:IntArray,d2:IntArray):this(withArray.size){
    ...
    private fun updateNode(i:Int,level:Int) {
        ...
        best0L[i] = maxOf(best0L[left], best0L[right])
        best1L[i] = maxOf(best1L[left], best1L[right])
        best0R[i] = maxOf(best0R[left], best0R[right])
        best1R[i] = maxOf(best1R[left], best1R[right])

        if(lazy[i] == 1) {
            best0L[i] = best1L[i].also { best1L[i] = best0L[i] }
            best0R[i] = best1R[i].also { best1R[i] = best0R[i] }
        }
//        arr[i] = arr[i shl 1 ] + arr[(i shl 1) + 1] + lazy[i] * ( 1 shl level)
    }
    fun nodeAdd(i:Int, level:Int){
...
fun main(){
    solve.cases{
        ...
        val d0 = G.BFS(0)
        val v2 = d0.indexOf(d0.maxOrNull()!!)
        val d2 = G.BFS(v2)
        val v1 = d2.indexOf(d2.maxOrNull()!!)
        val d1 = G.BFS(v1)
//        val furthest = IntArray(n){maxOf(d1[it], d2[it])}
        val xored = IntArray(n)
        G.root = v1
        G.rootFirstEdge { from, to, e ->
            xored[to ] = xored[from] xor G.weights[e]
        }
        val depth = G.calculateDepth()
        val ain = G.intime()
        val aout = G.outtime()
        val m = getint
        val stxor = IntArray(n)
        val std1 = IntArray(n)
        val std2 = IntArray(n)
        for(i in 0 until n){
            stxor[ain[i]] = xored[i]
        ...
        val st = lazyBuildableSegTree(stxor,std1,std2)
        val t1 = ain[v1]
        val t2 = ain[v2]
        val diameter = d2[v1]
        repeat(m){
            val e = getint -1
            var from = G.from[e shl 1]
            var to = G.to[( e shl 1) ]
            if(depth[from] > depth[to]){
                from = to.also { to = from  }
            }
            st.rangeAdd(ain[to], aout[to])


            st.preparePoint(t2)
//            st.pushall()
            if(st.best0L[t2 + st.n] == diameter){
                put(diameter)
                return@repeat
            }else{
                val one = st.best0L[1]
