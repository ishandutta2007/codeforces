...
/*link https://codeforces.com/group/8rU0UX1QW3/contest/249177/problem/E */
/*input
142
76
*/
...
int main () {
	...
	for (ll j = 0; j < 64 and j >= 0;) {
		kiee++;

		if (kiee > 1000000)
			break;

		unsigned long long te = (1ull << j);

		if ((a & te) == 0 and (b & te) == 0) {
			j++;
			continue;
		}

		debug(j, a, b, x, y);
		debug(bitset<10>(a), bitset<10>(b));
