...
int main(void)
{
	...
	rep(i, 1, n) rep(j, 1, n) rep(k, 0, 1) dp[i][j][k] = inf;
	rep(i, 1, n) dp[i][i][0] = 0, dp[i][i][1] = 1;
	...
	rep(len, 2, n){
		rep(l, 1, n){
			...
			if(r > n) break;
			if(a[l] == a[r]){
				if(len > 2) chmin(dp[l][r][0], dp[l+1][r-1][0]);
				else dp[l][r][0] = 0;
			}
			rep(i, l, r-1){
				chmin(dp[l][r][0], dp[l][i][1] + dp[i+1][r][0]);
				chmin(dp[l][r][0], dp[l][i][0] + dp[i+1][r][1]);
			}
			dp[l][r][1] = dp[l][r][0] + 1;
		}
	}
