...
int main() {
    ...
    for (int i = 1; i<n; i++)
    {
        ...
        for (int j = 0; j<=m-k; j++)
        {
            ...
            //cout<<dp1[j]<<endl;
            for (int kek = max(0, j-k+1); kek<=min(m-k, j+k-1); kek++)
            {
                dp1[j] = max(dp1[j], dp[kek] + adder - get_sum(i, max(j, kek), min(j+k-1, kek+k-1)));
                //if (i==1 && j==2) cout<<kek<<": "<<dp[kek]<<' '<<dp1[j]<<' '<<get_sum(i, max(j, kek), min(j+k-1, kek+k-1))<<endl;
            }
        }
        dp = dp1;
    }
