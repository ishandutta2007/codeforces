...
ll solve(){
    ...
    auto p=press(a);
    each(x,a)x=p[x];
    RMQ<ll>up(p.size(),-LINF),down(p.size(),-LINF),same(p.size(),-LINF);
    up.set(a[0],0);
    down.set(a[0],0);
    same.set(a[0],0);
    rep(i,1,n+1){
        const ll x=a[i];
        const ll val=max({up.get(0,x)+i,down.get(x+1,down.size)-i,same[x]});
        if(i==n)return val;
        up.set(x,max(up[x],val-i));
        down.set(x,max(down[x],val+i));
        same.set(x,max(same[x],val));
    }
    abort();
}
int main(){
