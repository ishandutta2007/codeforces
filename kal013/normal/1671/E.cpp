...
void solve(){
	...
	auto rec = [&] (const auto& self, int x, int l, int r) -> pair<string, mint> {

		string ans(1, s[x - 1]);
		if (l == r){
			return make_pair(ans, mint(1));
		}
		int m = (l + r) >> 1;


		auto [s1, a1] = self(self, 2 * x, l, m);
		auto [s2, a2] = self(self, 2 * x + 1, m + 1, r);

		if (s1 < s2){

			return make_pair(ans + s1 + s2, a1 * a2 * 2);
		}
		else if (s1 > s2){
			return make_pair(ans + s2 + s1, a1 * a2 * 2);
		}
		else{
			assert (a1 == a2);
			return make_pair(ans + s1 + s2, a1 * a2);
		}
	};

	auto [x, ans] = rec(rec, 1, 0, (1LL << (n - 1)) - 1);
