...
void solve() {
  ...
  vi DP(1 << N);
  FOR(i, N) DP[1 << i] = nbd[i];
  FOR(s, 1 << N) if (DP[s]) {
    FOR(v, N) {
      if (!(DP[s] & 1 << v)) continue;
      ll t = s | 1 << v;
      if (s < t) DP[t] = DP[s] | nbd[v];
    }
  }
  ll full = (1 << N) - 1;
  ll ANS = N;
  ll s0 = (1 << N) - 1;
  FOR(s, 1 << N) {
    if (DP[s] == full && chmin(ANS, popcnt(s))) s0 = s;
  }
  vi A;
  FOR(i, N) if (s0 & 1 << i) A.eb(i + 1);
  print(len(A));
  print(A);
}
