...
ll n, Q;
BIT xbit(200005), ybit(200005);
ll xcnt[200005], ycnt[200005];
...
int main(void)
{
	...
	rep(i, 1, Q){
		...
		if(t == 1){
			cin >> y >> x;
			ycnt[y]++; xcnt[x]++;
			if(ycnt[y] == 1) ybit.add(y, 1);
			if(xcnt[x] == 1) xbit.add(x, 1);
		}
		if(t == 2){
			cin >> y >> x;
			ycnt[y]--; xcnt[x]--;
			if(ycnt[y] == 0) ybit.add(y, -1);
			if(xcnt[x] == 0) xbit.add(x, -1);
		}
		if(t == 3){
			ll lx, ly, rx, ry;
			cin >> ly >> lx >> ry >> rx;
			if(xbit.query(lx, rx) == rx-lx+1 || ybit.query(ly, ry) == ry-ly+1) cout << "Yes" << "\n";
			else cout << "No" << "\n";
		}
	}
