...
const int modular_int<mod>::inverse_limit = 2000005;
...
void solve(){
    ...
    cin >> a;
    for(int i = 0; i < n; ++i){
    	// a[i] = random_long(0, 1000000);
    }
    vector<mint> dp(1 << n);
    mint ans = 0;

    vector<mint> sum_prod(1 << n, 1), inv_sum_prod(1 << n);
    vector<vector<mint>> val_prod(n + 2, vector<mint> (1 << n, 1));
    ...
    for(int mask = 0; mask < (1 << n); ++mask){
    	for(int j = 0; j < n; ++j){
    		if (mask & (1 << j)){
    			...
    			for(int k = j + 1; k < n; ++k){
    				if (mask & (1 << k)){
    					sum_prod[mask] *= (a[j] + a[k]);
    				}
    			}
    		}
    	}
    	inv_sum_prod[mask] = 1 / sum_prod[mask];
    	val_prod[0][mask] = 1;

    	for(int j = 2; j <= n; ++j){
    		val_prod[j][mask] = val_prod[j - 1][mask] * val_prod[1][mask];
    	}
    }


    auto calculate = [&] (int mask1, int mask2){
    	int bits = __builtin_popcount(mask1);
    	return (val_prod[bits][mask2]) * (inv_sum_prod[mask1 | mask2]) * (sum_prod[mask1] * sum_prod[mask2]);
    };
    ...
    for(int i = 0; i < n; ++i){
    	...
    	for(int mask = 0; mask < (1 << n); ++mask){
    		...
    		for(int sub_mask = mask; sub_mask > 0; ){
    			sub_mask = (sub_mask - 1) & mask;
    			dp[mask] -= dp[sub_mask] * calculate(sub_mask | (1 << i), mask ^ sub_mask);
    		}
    	}
    	ans += dp[(1 << n) - 1 - (1 << i)];
    }
