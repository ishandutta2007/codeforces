...
void solve() {
  ...
  auto f = [&](ll d, ll p, ll x) -> pi {
    if (d == 1) { return (p <= x ? mp(1, x - p) : mp(-1, 2 * s - x - p)); }
    return (x <= p ? mp(-1, p - x) : mp(1, p + x));
  };

  ll ANS = t2 * abs(x1 - x2);
  auto [d1, cost1] = f(d, p, x1);
  auto [d2, cost2] = f(d1, x1, x2);
  chmin(ANS, (cost1 + cost2) * t1);
  print(ANS);
}
