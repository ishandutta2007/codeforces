...
signed main(){
    ...
    while (t--){
        ...
        cin>>s[0]>>s[1]>>s[2];
        int a[3]={0,0,0};
        rep(i,3) rep(j,2*n) a[i]+=(s[i][j]=='1');
        int hii=0;
        rep(i,3) hii+=(a[i]>=n);
        if (hii<2){
            rep(i,3) rep(j,2*n) s[i][j]=s[i][j]=='0'?'1':'0';
            rep(i,3) a[i]=2*n-a[i];
        }
        if (a[0]<n) swap(s[0],s[2]);
        if (a[1]<n) swap(s[1],s[2]);
        int i=0,j=0;
//        cout<<s[0]<<" "<<s[1]<<" "<<s[2]<<endl;
        while (i<2*n||j<2*n){
            int l=i, r=j;
            while (l<2*n&&s[0][l]!='1') l++;
            while (r<2*n&&s[1][r]!='1') r++;
            rep(k,max(l-i,r-j)) cout<<((hii<2)?1:0);
            if (l<2*n||r<2*n) cout<<((hii<2)?0:1);
            i=l+1, j=r+1; 
        }
        cout<<"\n";
