...
void solve() {
    ...
    for (int i = 1, w; i <= n; i++) {
        cin >> w;
        tuple<int, int, int, int> min_cost = {1e9, 0, 0, 0};
        for (int x = 1; x <= k; x++) {
            for (int y_l = 1; y_l <= k; y_l++) {
                int64_t y_r = y_l + w - 1;
                if (y_r > k) break;
                if (*max_element(g[x] + y_l, g[x] + y_r + 1) == 1) continue;
                int64_t cost = 0;
                for (int y = y_l; y <= y_r; y++) {
                    cost += abs(x - c) + abs(y - c);
                }
                min_cost = min(min_cost, {cost, x, y_l, y_r});
            }
        }
        auto&& [cost, x, y_l, y_r] = min_cost;
        if (cost == 1e9) {
            cout << "-1\n";
        } else {
            fill(g[x] + y_l, g[x] + y_r + 1, 1);
            cout << x << ' ' << y_l << ' ' << y_r << '\n';
        }
    }
}
