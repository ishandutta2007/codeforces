...
public:
    ...
    map<pair<pii, pair<char, bool>>, bool> M;

    bool wins(int firstMarble, int secondMarble, char minLabel, bool maxTurn) {
        auto it = M.find({{firstMarble, secondMarble}, {minLabel, maxTurn}});
        if (it != M.end()) return it->y;
        bool ans = false;
        for (auto &e: E[firstMarble]) {
            if (e.y >= minLabel && !wins(secondMarble, e.x, e.y, !maxTurn)) {
                ans = true;
                break;
            }
        }

        M[{{firstMarble, secondMarble}, {minLabel, maxTurn}}] = ans;
        return ans;
    }
