...
void solve(){
    ...
    vector<char> oth(n + 1, 'a' - 1);
	vector<int> len(n + 1, 0);
	vector<string> lst(n + 1);
	for(int i = n - 1; i >= 0; --i){
		string res;
		if (i + 1 == n || s[i] != s[i + 1]){
            ...
            if (i + 1 == n){
                ...
                oth[i] = (s[i] != ans[i + 1][0]) ? ans[i + 1][0] : oth[i + 1];
            }
			res = s[i];
			res += ans[i + 1];
			len[i] = len[i + 1] + 1;
            ...
            if (ans[i + 2].empty() || ans[i + 2][0] < s[i] || (ans[i + 2][0] == s[i] && oth[i + 2] < s[i])) {
                res = ans[i + 2];
                len[i] = len[i + 2];
                lst[i] = lst[i + 2];
                oth[i] = oth[i + 2];   
	...
	for(int i = 0; i < n; ++i){
		cout << len[i] << " ";
		if (len[i] > 10){
			cout << ans[i].substr(0, 5) << "..." << lst[i] << endl;
		}
		else{
			cout << ans[i] << endl;
		}
