...
int main() {
    vvvv(int, dp, 1001, 1001, 2, 2);
    rep(j, 1001) rep(i, 1001) rep(k, 2) rep3(s, 1, 0) {
        bool p = (!i);
        auto &res = dp[i][j][k][s];
        res = -inf<int>;
        if(i) chmax(res, -1 - dp[i - 1][j][k][0]);
        if(j and i + 1 <= 1000) chmax(res, -1 - dp[i + 1][j - 1][k][0]);
        if(k) chmax(res, -1 - dp[i][j][0][0]);
        if(!s and i) chmax(res, -dp[i][j][k][1]);
        if(i == 0 and j == 0 and k == 0) dp[i][j][k][s] = 0;
    }
    // cout << dp[0][1][1][0] << endl;
    // cout << dp[0][1][0][0] << endl;
    // cout << dp[1][0][0][0] << endl;
    // cout << dp[1][0][0][1] << endl;
    // cout << dp[0][1][0][0] << endl;
    TEST {
        INT(n);
        int a = 0, b = 0, c = 0;
        STR(s);
        rep(i, n / 2) {
            int k = (s[i] == '1') + (s[n - 1 - i] == '1');
            if(k == 1) a++;
            if(k == 0) b++;
        }
        if(n & 1 and s[n / 2] == '0') c++;
        int t = dp[a][b][c][0];
        dump(a, b, c, t);
        if(t > 0) {
            cout << "ALICE" << endl;
