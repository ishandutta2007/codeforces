...
void solve(){
  ...
  vi S(n+1); for(li i = 0; i < (li)(n); ++i) S[i+1] = S[i]+A[i];

  set<li> setS(begin(S), end(S));
  vi vecS(begin(setS), end(setS));
  li m = vecS.size();
  hash_map<li,li> ms;
  for(li i = 0; i < (li)(m); ++i) ms[vecS[i]] = i;
  ...
  for(li i = 0; i < (li)(n); ++i) {
    cur = cur + sgn(A[i]);
    smax(cur, bit.get(ms[S[i+1]])+(i+1));
    bit.add(ms[S[i+1]]+1, cur-(i+1));
  }
  cout << cur << endl;
}
