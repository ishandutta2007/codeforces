...
public class Main {
    ...
    static class TaskD {
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            Set<TaskD.Cell> current = new EHashSet<>();
            boolean[][] used = new boolean[401][401];
            current.add(new TaskD.Cell(200, 200, 1));
            for (int i : t) {
                Set<TaskD.Cell> next = new EHashSet<>();
                for (TaskD.Cell cell : current) {
                    int x = cell.x;
                    int y = cell.y;
                    for (int j = 0; j < i; j++) {
                        x += MiscUtils.DX8[cell.dir];
                        y += MiscUtils.DY8[cell.dir];
                        used[x][y] = true;
                    }
                    next.add(new TaskD.Cell(x, y, (cell.dir + 1) & 7));
                    next.add(new TaskD.Cell(x, y, (cell.dir + 7) & 7));
                }
                current = next;
            }
            int answer = 0;
            for (boolean[] row : used) {
                answer += ArrayUtils.count(row, true);
            }
