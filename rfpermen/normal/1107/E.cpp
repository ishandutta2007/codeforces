...
ll n,dp[MAX][MAX][MAX][2],x[MAX];
string s;
bool z[MAX];

ll f(int le,int ri,int len,bool ls){
	if(le>ri)return x[len];
	ll &ret = dp[le][ri][len][ls];
	if(ret!=-1)return ret;
	ret = 0;
	if(len)ret = f(le,ri,0,0) + x[len];
	if(!len || z[le]==ls)ret = max(ret,f(le+1,ri,len+1,z[le]));
	For(i,le,ri)ret = max(ret,f(le,i,0,0) + f(i+1,ri,len,ls));
	return ret;
}
