...
#ifdef COMB
	fac[0]=inv[0]=inv[1]=1;
	rg(4000000)fac[i]=fac[i-1]*i%mod;gr
	rg(4000000)if(i>1)inv[i]=inv[mod%i]*(mod-mod/i)%mod;gr
	// rg(4000000)rnv[i]=inv[i];gr
	rg(4000000)inv[i]=inv[i]*inv[i-1]%mod;gr
#endif
}
#ifdef COMB
int C(int n,int m)
...
#endif
int f[4000005],g[4000005],a[4000005];
signed main()
{
	initprog();
	int n=read(),p=0;
	rg(n)a[i]=read();p=std::__gcd(p,a[i]);gr
	rg(n)rnv[a[i]]++;gr
	const int N=300000;
	rg(N)for(int j=i;j<=N;j+=i)f[i]+=rnv[j];gr
	
	rg_(k,8)
	for(int i=N;i>=1;i--)
	{
		g[i]=C(f[i],k);
		for(int j=i*2;j<=N;j+=i)mde(g[i],g[j]);
	}
	if(g[1])return odl(k),0;
	gr
	puts("-1");
	return 0;
}