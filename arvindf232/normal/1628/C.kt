...
fun getoddboard(size:Int):Array<BooleanArray>{
    val ret = Array(size){BooleanArray(size)}
    for(i in 0 until size){
        for(j in 0 until size){
            if((i + j) % 2 == 1) continue
            val rank = minOf(i,j,size-1-i, size-1-j)
            if(rank % 4 == 1 || rank % 4 == 2){
                ret[i][j] = true
            }
        }
    }
...
fun getevenboard(size:Int):Array<BooleanArray>{
    val ret = Array(size){BooleanArray(size)}
    val preboard = getoddboard(size-1)
...
fun main(){
//    val a = getoddboard(5)
//    val b = getevenboard(6)
    solve.cases{
        val n = getint()
        val arr = Array(n){getline(n)}
        val board = getevenboard(n)
        var ret = 0
        for(i in 0 until n){
            for(j in 0 until n){
                if(board[i][j]){
                    ret = ret xor arr[i][j]
                    ret = ret xor arr[n-1-i][j]
                }
            }
        }
        put(ret)
