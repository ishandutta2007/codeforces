...
void solve() {
    ...
    rep(i, 1, n) rep(j, 1, m) su[i][j] = su[i - 1][j] + s[i][j] - 48;
    ...
    rep(i, 1, n) rep(j, i + 4, n) {
        ...
        rep(k, 1, m) {
            ...
            int t = j - i - 1 - su[j - 1][k] + su[i][k];

            if(k >= 4) cmin(as, sum + t + pre[k - 3]);

            sum += (s[i][k] == 48) + (s[j][k] == 48) + su[j - 1][k] - su[i][k];

            pre[k] = min(t - sum, pre[k - 1]);
