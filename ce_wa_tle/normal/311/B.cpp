...
const ll inf=0x3f3f3f3ff33ff3;
ll f[311111][2],s[311111],q[311111],a[311111],d[311111];
int main(){
	//IO(CF311B Cats Transport);
//	f[i]=min(f[j]+a[i]*(i-j)-(s[i]-s[j]))
//	=min(f[j]+s[j]-a[i]*j)-s[i]+a[i]*i
	ll rill(n,m,p);
	rep(i,2,n)rill(d[i])+d[~-i];
	ll sum=0;
	rep(i,1,m){
		ll rill(h,t);
		a[i]=t-d[h];
//		if(a[i]<0)sum-=a[i],a[i]=0;
	}
	std::sort(a+1,a+m+1);
	rep(i,1,m)s[i]=s[i-1]+a[i];
//	return 0;
	#define y(d) (f[d][j&1^1]+s[d])
	#define x(d) d
	#define xl(a,b,c) ((y(c)-y(b))*(x(b)-x(a))>=(y(b)-y(a))*(x(c)-x(b)))
	rep(i,1,m)f[i][0]=inf;
	rep(j,1,p){
		ll h=1,t=1;q[h]=0;
		rep(i,1,m){
			while(t>h&&(y(q[-~h])-y(q[h]))<=a[i]*(x(q[-~h])-x(q[h])))h++;
			f[i][j&1]=y(q[h])-x(q[h])*a[i]-s[i]+a[i]*i;
//			writeln("f[",i,"][",j,"]=",f[i][j&1]);
//			writeln("q[",h,"]=",q[h],' ',y(q[h]));
			while(t>h&&!xl(q[~-t],q[t],i))t--;
			q[++t]=i;
		}
	}
	writeln(f[m][p&1]+sum);
