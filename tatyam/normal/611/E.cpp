...
int main(){
    LL(n,a,b,c);
    VEC(ll,t,n);
    if(any_of(all(t),[&](ll x){return x>a+b+c;}))return out(-1);
    vector<ll>d={a,b,c,a+b,b+c,c+a,a+b+c};
    Sort(d);
    vec(ll,cnt,7);
    each(x,t)rep(7)if(x<=d[i]){
        cnt[i]++;
        break;
    }
    ll ans=LINF;
    rep(n){
        chmin(ans,[=]()mutable->ll{
            ll ans=cnt.back();
            cnt.pop_back();
            cnt[1]+=max(0LL,cnt.front()-cnt.back());
            ans+=cnt.back();
            cnt.pop_back();
            cnt.erase(cnt.begin());
            cnt[1]+=max(0LL,cnt.front()-cnt.back());
            ...
            cnt.pop_back();
            cnt.erase(cnt.begin());
            ans+=max(cnt.back(),(cnt[0]+cnt[1]+1)/2);
            return ans;
        }()+i);
        for(ll x:{a,b,c}){
            bool f=1;
            rrep(7)if(cnt[i]&&d[i]<=x){
                cnt[i]--;
                f=0;
                break;
            }
