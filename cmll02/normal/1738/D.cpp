...
{
	if(u>=1&&u<=n)rs[u]=++cc;
	fe(u)if(!vis[v]&&v>=1&&v<=n)rs[v]=(++cc);gr
	fe(u)if(vis[v])dfs(v,n);gr
}
arr shaber;
signed main()
{
	...
	int T=read();while(T--)
	{
		...
		odl(k);
		int r=0;
		rg(cc)if(!shaber[i])r=i;gr
		// odl(r);
		int u=r;
		while(u)
		{
			fe(u)if(is[u]!=v)odb(v);gr
			fe(u)if(is[u]==v)odb(v);gr
			u=vis[u];
		}
		puts("");
