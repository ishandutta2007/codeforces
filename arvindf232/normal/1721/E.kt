...
//fun keepgoing(s:String,t:String,now:IntArray){
//
//}
...
fun main(){
    solve.cases{
        ...
        cache[""] = ret[s.length - 1]
        fun solve(ns:String):Int{
            if(cache[ns] != null){
                return cache[ns]!!
            }
            t = ns
            val i = s.length + ns.lastIndex
            var j = solve(ns.dropLast(1))
            while(j > 0 && read(i) != read(j)){
                j = if(j-1 >= s.length) cache[ns.take(j-1-s.length)]!! else ret[j-1]
            }
            if(read(i) == read(j)){
                j++
            }
