...
bool valid(int a,int b,int c,int d){
	int e=a*8+b*4+c*2+d;
	return e!=3&&e!=5&&e!=14&&e!=15;
}

int n;
int a[3005];
int cnt;
int go[9000005][2],cntgo;
	...
	for(int i=1;i<=n;i++){
		...
		for(int j=i;j>=1;j--){
			...
			for(int k=1;k<=4;k++){
				...
				if(k==4&&!valid(a[j+k-4],a[j+k-3],a[j+k-2],a[j+k-1])){
					//cerr<<123123123<<' '<<i<<endl;
					continue;
				}
				dp[cntgo]=(dp[cntgo]+dp[now])%Mod;
				now=fa[now];
			}
			fa[cntgo]=y;
			y=go[y][a[j]]=cntgo;
			ans=(ans+dp[cntgo])%Mod;
		}
		fio<<ans<<endl;
	}
	return 0;
