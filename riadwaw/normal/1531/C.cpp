...
public:
	...
	void solve(std::istream& in, std::ostream& out) {
		...
		int size = diagSize;
		while (size * size - (size - diagSize) * (size - diagSize) < n) {
			++size;
		}
		vector<string> result(size, string(size, '.'));

		for (int i: range(diagSize)) {
			for (int j: range(diagSize)) {
				result[i][j] = 'o';
				--n;
			}
		}
