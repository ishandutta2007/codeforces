...
void Add(pll &x, pll y, int v, int vv) {
	x.se = (x.se + y.se) % p;
	x.fi = (x.fi + 1ll * v * Pow[vv] % p * y.se % p + y.fi) % p;
}

pll dfs(int st, int sta, bool z, bool b) {
	if(!st) return mp(0, 1);
	if(~dp[st][sta][z][b].fi && ~dp[st][sta][z][b].se) return dp[st][sta][z][b];
	pll ret = mp(0, 0);
	rep(i, 0, b ? num[st] : 9) {
		if(__builtin_popcount((z && !i) ? 0 : (sta | (1 << i))) <= k) Add(ret, dfs(st - 1, (z && !i) ? 0 : (sta | (1 << i)), z && !i, b && (i == num[st])), i, st - 1);
	}
	return dp[st][sta][z][b] = ret;
}
...
ll solve(ll x) {
	...
	return dfs(n, 0, 1, 1).fi;
}

int main() {
	Pow[0] = 1;
	rep(i, 1, 18) Pow[i] = Pow[i - 1] * 10;
	qread(l, r, k);
