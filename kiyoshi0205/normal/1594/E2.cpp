...
tuple<ll,ll,ll> getval(ll id,ll frag){
  ...
  auto[al,bl,cl]=(memo.find(id*2)!=memo.end()?memo[id*2]:dp[cnt-1]);
  auto[ar,br,cr]=(memo.find(id*2+1)!=memo.end()?memo[id*2+1]:dp[cnt-1]);
...
int main(){
  ...
  while(N--){
    ...
    string s;
    cin>>id>>s;
    ll f=(s[0]=='w'||s[0]=='y')|(s[0]=='g'||s[0]=='b')<<1|(s[0]=='r'||s[0]=='o')<<2;
    v.emplace_back(id,f);
    while(id/=2)v.emplace_back(id,7);
  }
  uniq(v);
  rep1(i,v.size())if(v[i-1].first==v[i].first){
    if(v[i].second==7)v[i].second=v[i-1].second;
    else{
      print(0);
      return 0;
    }
