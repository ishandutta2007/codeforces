...
//*************************************************************************************
int a[101010];
bool used[2][27 * 27 * 27];
bool vis[10001][4];
string s;
int n;
//void go(int pos, int last) {
//    if (vis[pos][last]) return;
//    if (last != 0) {
//        int x = 0;
...
int main() {
    ...
    set<string> r;
    int ans = 0;
    canbe[n][2] = canbe[n][3] = true;
    for (int i = n - 2; i >= 5; --i) {
        for (int len = 2; len <= 3; ++len) {
            if (canbe[i + len][len] && s.substr(i, len) != s.substr(i + len, len) || canbe[i + len][5 - len]) {
                canbe[i][len] = true;
                r.insert(s.substr(i, len));
            }
        }
    }
    cout << r.size() << endl;
