...
void solve() {
    ...
    vector<int> x, y;
    REP(i, n) {
        int hoge = query(1, i + 1, n, i + 1);
        if (hoge % 2 == 1) {
            y.push_back(i + 1);
        }
        hoge = query(i + 1, 1, i + 1, n);
        if (hoge % 2 == 1) {
            x.push_back(i + 1);
        }
    }
    ...
    REP(i, x.size()) {
        REP(q, y.size()) {
            if (query(x[i], y[q], x[i], y[q]) == 1) {
                ans.push_back(mp(x[i], y[q]));
            }
        }
    }
    cout << "!";
