...
struct UpdateSum_LazySegmentTree {
  ...
  T g(T a, E b) { return b == UNUSED_VALUE ? a : T(b * a.second, a.second); };
  E h(E, E b) { return b; };
  T ti = T(0, 0);
  E ei = UNUSED_VALUE;
  vector<T> dat;
...
void q() {
  ...
  vi init(N);
  rep(i, N) init[i] = T[i] - '0';
  UpdateSum_LazySegmentTree<int, -1> seg(init);

  repr(i, Q) {
    int l = L[i], r = R[i];
    trc(l, r);
    int sm = seg.query(l, r);
    if (sm * 2 == r - l) die("NO");
    if (sm * 2 < r - l)
      seg.update(l, r, 0);
    else
  ...
  rep(i, N) if (seg.query(i, i + 1) != S[i] - '0') die("NO");
  out("YES");
}
