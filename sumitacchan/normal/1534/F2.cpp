...
signed main(){
    ...
    Sort(Q);
    v_Pii ss;
    for(Pii q: Q) if(!G.visited[q.se]){
        ss.pb(Pii(q.se % m, q.se));
        G.dfs0(q.se);
    }
    Sort(ss);
    int N = SZ(ss);
    ...
    mat vs(sz);
    REP(i, n * m) vs[cmp[i]].pb(i);
    vec dp(sz, INF), dq(sz, -INF);
    REP(i, N){
        dp[cmp[ss[i].se]] = i;
        dq[cmp[ss[i].se]] = i;
    }
    IREP(k, sz){
        for(int v : vs[k]){
            for(auto e: G.G[v]) if(cmp[e.to] != k){
                chmin(dp[k], dp[cmp[e.to]]);
                chmax(dq[k], dq[cmp[e.to]]);
            }
        }
    }

    Graph H(N + 1);
    REP(k, sz) if(dp[k] <= dq[k]){
        H.add_Directed_edge(dp[k], dq[k] + 1, 1);
    }
    REP(i, N) H.add_Directed_edge(i + 1, i, 0);
    int ans = H.dijkstra(0, N)[N];
    Out(ans);
