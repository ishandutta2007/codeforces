...
int main() {
    ...
    vi row_ct(N);
    vi row_first(N, INT_MAX);
    vi row_last(N, INT_MIN);

    vi col_ct(M);
    vi col_first(M, INT_MAX);
    vi col_last(M, INT_MIN);
    ...
    vi empty_rows;
    F0R (i, N) if (!row_ct[i]) empty_rows.pb(i);

    vi empty_cols;
    F0R (j, M) if (!col_ct[j]) empty_cols.pb(j);
    ...
    F0R (r, N) {
        F0R (c, M) {
            if (grid[r][c] == '.') {
                if (row_first[r] <= c && c <= row_last[r]) {
                    ps(-1);
                    return 0;
                }
                if (col_first[c] <= r && r <= col_last[c]) {
                    ps(-1);
                    return 0;
                }
            }
    ...
    int black = accumulate(all(row_ct), 0);

    union_find uf(N * M);
    F0R (r, N) {
        F0R (c, M) {
            if (r+1<N && grid[r][c] == '#' && grid[r+1][c] == '#')
                black -= uf.unio(r * M + c, (r + 1) * M + c);
            if (c+1<M && grid[r][c] == '#' && grid[r][c+1] == '#')
                black -= uf.unio(r * M + c, r * M + c + 1);
        }
    }
