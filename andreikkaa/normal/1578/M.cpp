...
void _solve() {
    ...
    for(int i = 0; i < (1 << n); ++i) {
        ...
        for(int j = 0; j < n; ++j) {
            ...
            if(sum[i] + a[j].cc.begin()->x < 0) {
                assert(a[j].cc.count(-sum[i] - 1));
                int to = a[j].cc[-sum[i] - 1][0];
                if(a[j].cc.count(-sum[i])) {
                    ans = max(ans, dp[i] + int(lower_bound(all(a[j].cc[-sum[i]]), to) - a[j].cc[-sum[i]].begin()));
                }
                //until first -sum-1 balance positions with -sum balance
                continue;
            }
            int ad = 0;
            if(sum[i] + a[j].cc.begin()->x == 0) {
                ad = sz(a[j].cc.begin()->y);
            }
            //cout << i << " " << sum[i] << " " << j << " " << dp[i] << " " << ad << endl;
            dp[i | (1 << j)] = max(dp[i | (1 << j)], dp[i] + ad);
        }
    }

    cout << max(ans, *max_element(dp, dp + (1 << N))) << endl;
}
