...
void solve() {
  ...
  const int MAX = 100;  // prime factor 

  vc<T> cand;
  cand.eb(T({1, vi()}));
  FOR(k, MAX - 1) {
    ll x = 1;
    FOR(i, k + 1) x = mul(x, primes[i]);
    if (x == INF) break;
    vc<T> newcand;
    FORIN(t, cand) {
      ll xpow = x;
      ll e = 1;
      while (mul(t.N, xpow) < INF) {
        T to;
        to.N = t.N * xpow;
        to.E = t.E;
        FORIN(cnt, to.E) cnt += e;
        while (to.E.size() < k + 1) to.E.eb(e);
        newcand.eb(to);
    ...
    cand.insert(cand.end(), all(newcand));
  }
  LL(N);
  ll ANS = INF;
  FORIN(t, cand) {
