...
int main(){
    ...
    SegmentTree<int> seg(sz,[](int a,int b){return max(a,b);},0);
    for (int i=0;i<n;++i){
        int x=xw[i].first,w=xw[i].second;
        int a=seg.query(0,mp[x-w]+1)+1;
        seg.update(mp[x+w],max(seg[mp[x+w]],a));
        // debug(i,a);
    }
