...
int main() {
	...
	wne(stk) {
		...
		forin(i, cons[cur]) {
			...
			if (color[oth] != color[cur]) {
				con
			}
			if (dist[oth] == -1) {
				dist[oth]=dist[cur]+ cons[cur][i].ss;
				stk.push_back(oth);
			}
			else {
				lol dif = abs(dist[oth] - (dist[cur] + cons[cur][i].ss));
				gcdd[color[cur]] = gcd(gcdd[color[cur]], dif);
			}
		}
	}
	def(t)
