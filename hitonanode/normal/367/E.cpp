...
mint Cat(int n) { return mint(2 * n).nCr(n) / (n + 1); }
...
int main() {
    ...
    vector dp(M + 3, vector(N + 3, vector<mint>(N + 3)));
    dp[0][0][0] = 1;
    REP(i, M + 1) REP(j, N + 1) REP(k, j + 1) if (dp[i][j][k]) {
        if (j + 1 <= N) dp[i + 1][j + 1][k] += dp[i][j][k];
        if (j > k) dp[i + 1][j][k + 1] += dp[i][j][k];
        if (j + 1 <= N and k + 1 <= N) dp[i + 1][j + 1][k + 1] += dp[i][j][k];
        dp[i + 1][j][k] += dp[i][j][k];
    }
    mint ret = 0;
    REP(nu, N) REP(nd, nu + 1) {
        ret += dp[X - 1][nu][nd] * dp[M - X][N - nd][N - (nu + 1)];
        ret += dp[X - 1][nu][nd] * dp[M - X][N - nd - 1][N - nu - 1];
    }
    dbg(ret);
    cout << ret * mint(N).fac() << '\n';
}