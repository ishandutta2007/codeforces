...
void build_ans(int now,bool is_root,int state,int& end_of_the_path,vector<pii>& ans)
{
    if (state==0){
        if (go[now][0]==inf){
            ...
            if (len(guys)%2==1){
                ...
                build_ans(guys.back(),false,1,ans.back().sec,ans);
                guys.pop_back();
            }
            for (int i=0;i<len(guys);i+=2){
                ...
                int buf_sz = ans.size() - 1;
                build_ans(guys[i],false,1,ans[buf_sz].fir,ans);
                build_ans(guys[i+1],false,1,ans[buf_sz].sec,ans);
            }
        }
        else{
            vector<int> guys;
            ...
            build_ans(go[now][0],false,0,fake_variable,ans);
        ...
        if (go[now][1]==inf){
            ...
            if (len(guys)%2==1){
                mark_cur(now,guys.back());
                build_ans(guys.back(),false,1,end_of_the_path,ans);
                guys.pop_back();
            }
            else{
                end_of_the_path=now;
            ...
            build_ans(go[now][1],false,0,fake_variable,ans);
...
void solve(int root,vector<pii>& ans)
{
    dfs2(root,1);
    build_ans(root,1,0,fake_variable,ans);
}
...
//            cout<<"ans :: "<<i.fir<<" "<<i.sec<<"\n";
            if (reb_set[i.fir].count(i.sec)){
//                cout<<"bad :: "<<i.fir<<" "<<i.sec<<"\n";
                bool ok=0;
                if (!ok){
                    for (auto wh:reb_set[i.fir]){
                        if (!marked_cur.count(mp(i.fir,wh))){
                            i.fir=wh;
                            marked_cur.insert(mp(i.fir,wh));
                            ok=1;
                            break;
                        }
                    }
                ...
                if (!ok){
                    for (auto wh:reb_set[i.sec]){
                        if (!marked_cur.count(mp(i.sec,wh))){
                            i.sec=wh;
                            marked_cur.insert(mp(i.sec,wh));
                            ok=1;
                            break;
                        }
                    }
