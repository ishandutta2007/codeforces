...
vpi cool[MX];
ll cyc_len[MX], dist[MX];
bool vis[MX];

void gen_cyc_len(int x) {
	assert(!vis[x]);
	vis[x] = 1;
	for (pi p: cool[x]) {
		if (vis[p.f]) {
			ll dif = abs(dist[x]+p.s-dist[p.f]);
			cyc_len[S.comp[x]] = __gcd(cyc_len[S.comp[x]],dif);
			// dist[x]+p.s, dist[p.f]
		} else {
			dist[p.f] = dist[x]+p.s;
			gen_cyc_len(p.f);
