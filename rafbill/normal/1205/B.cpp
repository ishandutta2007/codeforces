...
int main(){
  ...
  vi nb(64);
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(62); ++j) if(A[i]&(1ll<<j)) nb[j]++;
  for(li j = 0; j < (li)(62); ++j) if(nb[j]>=3) {
    cout << 3 << endl;
    return 0;
  }

  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) if(A[i]&A[j]) G[i][j] = 1;
  li ans = 201;
  for(li u = 0; u < (li)(n); ++u) for(li v = 0; v < (li)(n); ++v) if(u<v && G[u][v]) {
    vi D(n, 1e7);
    queue<int> Q;
    Q.push(u); D[u] = 0;
    while(!Q.empty()) {
      int i = Q.front(); Q.pop();
      for(li j = 0; j < (li)(n); ++j) if(G[i][j]&&i!=j&&make_tuple(i,j)!=make_tuple(u,v) &&make_tuple(j,i) != make_tuple(u,v)) {
        if(D[j] > D[i]+1) {
          D[j] = D[i]+1;
          Q.push(j);
        }
