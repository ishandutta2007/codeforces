...
void solve() {
    ...
    auto get_topk = [](const array<int, 101> &cts, int qty) {
        int sum = 0;
        F0Rd (s, 101) {
            int take = min(qty, cts[s]);
            qty -= take;
            sum += s * take;
        }
        return sum;
    ...
    auto good = [&]() {
        ...
        int my_sum = get_topk(mine, take);
        int his_sum = get_topk(his, take);

        return my_sum >= his_sum;
    };
