...
class DNezzarITainiePerestanovki {
public:
    void solve() {
        int n = in.readInt();
        int m = in.readInt();
        ...
        set<pii> edges;
        for (int i : range(m)) {
            edges.emplace(l[i], r[i]);
            edges.emplace(r[i], l[i]);
        }
        arri p(n);
        arri q(n);
        int last = n;
        arr<bool> special(n, false);
        arr<set<int>> connected(n);
        arri connectedTo(n, -1);
        set<int> rem;
        for (int i : range(n)) {
            if (deg[i] == n - 1) {
                p[i] = last;
        ...
        while (!rem.empty()) {
            ...
            rem.erase(it);
            for (int i : range(n)) {
                if (i == v || edges.count({i, v})) {
                    continue;
                }
                if (special[i]) {
                    connected[i].insert(v);
                    connectedTo[v] = i;
                    break;
                }
                if (connectedTo[i] == -1) {
                    special[v] = true;
                    connected[v].insert(i);
                    connectedTo[i] = v;
                    rem.erase(i);
                    break;
                }
                int ct = connectedTo[i];
                if (connected[ct].size() == 1) {
                    special[ct] = false;
                    connected[ct].clear();
                    special[i] = true;
                    connected[i].insert(ct);
                    connected[i].insert(v);
                    connectedTo[ct] = i;
                    connectedTo[v] = i;
                    break;
                }
                connected[ct].erase(i);
                special[v] = true;
                connected[v].insert(i);
                connectedTo[i] = v;
                break;
...
int main() {
    ...
    auto time = clock();
#endif
    DNezzarITainiePerestanovki solver;
