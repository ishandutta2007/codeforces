...
struct LinearEquations{
	...
	vector<vector<T> > mat;
	vector<T> solvec;
	vector<vector<T> > solbase;
	
	LinearEquations(){}
	void init(vector<vector<T> > &mat, vector<T> &vec){
		h = mat.size(), w = mat[0].size();
		this->mat = mat;
		for(int i = 0; i < h; i++) this->mat[i].push_back(vec[i]);
	}
	bool solve() //solvecsolbase
	{
		int r = 0;
		for(int i = 0; i < w && r < h; i++){
			if(mat[r][i] == 0){
				T max_val = 0; int max_j;
				for(int j = r+1; j < h; j++){
					if(max_val < mat[j][i]) max_val = mat[j][i], max_j = j;
				}
				if(max_val == 0) continue;
				swap(mat[r], mat[max_j]);
			}
			
			T div = T(1) / mat[r][i];
			for(int j = 0; j < w+1; j++) mat[r][j] *= div;
			
			for(int j = 0; j < h; j++){
				T coe = mat[j][i];
				if(j == r || mat[j][i] == 0) continue;
				for(int k = 0; k < w+1; k++) mat[j][k] -= coe * mat[r][k];
			}
			r++;
		}
		
		vector<int> left(h), used(w);
		for(int i = 0; i < w; i++) used[i] = 0;
		...
		for(int i = 0; i < h; i++){
			...
			if(left[i] == w) return false;
			if(left[i] != -1) used[left[i]] = 1;
		}
		
		solvec.resize(w);
		for(int i = 0; i < w; i++) solvec[i] = 0;
		for(int i = 0; i < h; i++) if(left[i] != -1) solvec[left[i]] = mat[i][w];
		
		vector<T> tmp(w);
		solbase.clear();
		for(int i = 0; i < w; i++){
			if(used[i]) continue;
			for(int j = 0; j < w; j++) tmp[j] = 0;
			for(int j = 0; j < h; j++) if(left[j] != -1) tmp[left[j]] = -mat[j][i];
			tmp[i] = 1;
			solbase.push_back(tmp);
		}
		return true;
	}
};
...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n) rep(j, 1, n) rep(k, 1, n){
			if(i >= j || j >= k) continue;
			if(G[i][j] < 0 || G[j][k] < 0 || G[k][i] < 0) continue;
			vector<mint> tmp(m);
			rep(l, 0, m-1) tmp[l] = 0;
			tmp[G[i][j]] = tmp[G[j][k]] = tmp[G[k][i]] = 1;
			mat.pb(tmp), vec.pb(mint(0));
		}
		rep(i, 0, m-1){
			if(c[i] != -1){
				vector<mint> tmp(m);
				rep(l, 0, m-1) tmp[l] = 0;
				tmp[i] = 1;
				mat.pb(tmp), vec.pb(mint(c[i]%3));
			}
		}
		if(sz(mat) == 0){
			rep(i, 0, m-1) cout << 1 << " "; cout << endl;
			continue;
		}
		...
		for(auto &x : le.solvec) if(x.x == 0) x.x = 3;
		outl(le.solvec);
	}
