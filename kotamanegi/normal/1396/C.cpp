...
void solve(){
    ...
    REP(i, n) {
        ...
        int hoge = d;
        if (i == n - 1) hoge = 0;
        next_dp[0] = min({ dp[0] + pistol * enemy[i] + awp, dp[1] + min(laser,pistol * (enemy[i] + 1)) + pistol + d,dp[1] + pistol * enemy[i] + awp + hoge});
        next_dp[1] = dp[0] + min(laser, pistol * (enemy[i] + 1)) + pistol + d;
        swap(dp, next_dp);
    }
    cout << min(dp[0], dp[1] + d) << endl;
}
#undef int
int main() {
    init();
