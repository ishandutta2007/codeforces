...
public class I {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int T = sc.nextInt();
        long M = sc.nextLong();
        long[] fact = new long[402];
        fact[0] = 1;
        for (int n = 1; n <= 401; n++) {
            fact[n] = (fact[n - 1] * n) % M;
        }
        ModIntFactory mf = new ModIntFactory((int) M);
        ...
        for (int n = 0; n <= 400; n++) {
            ...
            for (int m = 1; m <= n; m++) {
                long sum = dp[n - 1][m];
                for (int k = 0; k < m; k++) {
                    sum -= mf.create(fact[m - k + 1]).mul(dp[n][k]).value();
                }
                dp[n][m] = MathLib.safe_mod(sum, M);
            }
        }
        long[] ans = new long[401];
        for (int n = 0; n <= 400; n++) {
            ans[n] = -dp[n][n] + (n % 2 == 0 ? 2 : -2);
        }
        for (int n = 0; n < T; n++) {
            int x = sc.nextInt();
            if (x == 2) {
                ...
                cp.println(MathLib.safe_mod(ans[x - 1], M));
            }
        }
        cp.close();
    }
