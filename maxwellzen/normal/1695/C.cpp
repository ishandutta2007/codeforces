...
void solve() {
    ...
    mn.assign(n, vi(m, 1e9));
    mx.assign(n, vi(m, -1e9));
    mn[n-1][m-1] = mx[n-1][m-1] = a[n-1][m-1];
    R0F(i, n) R0F(j, m) {
        if (i<n-1) ckmin(mn[i][j], a[i][j]+mn[i+1][j]), ckmax(mx[i][j], a[i][j]+mx[i+1][j]);
        if (j<m-1) ckmin(mn[i][j], a[i][j]+mn[i][j+1]), ckmax(mx[i][j], a[i][j]+mx[i][j+1]);
    }
    if (mn[0][0]<=0 && mx[0][0]>=0) YES;
    else NO;
}
