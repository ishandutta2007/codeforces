...
signed main()
{
	...
	int z=0,f=0;
	rg(n)
	int x=read();if(x>0)z++;if(x<0)f++;
	if(x==0)((++c)<=3)&&(a[++s]=0);
	else a[++s]=x;
	gr
	// odp(z,f);
	// odp(s,c);
	if(z>2||f>2){puts("NO");continue;}
	int ans=0;
	std::set<int>ss;rg(s)ss.insert(a[i]);gr
	rg(s)rg_(j,s)rg_(k,s)
	if(i<j&&j<k)
	if(!ss.count(a[i]+a[j]+a[k])){puts("NO");goto no;}
	gr gr gr
	puts("YES");
	no:;
	gr
