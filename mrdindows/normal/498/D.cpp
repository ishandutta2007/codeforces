...
int get_sum(int cur, int l, int r, int x, int y, int time){
	if (l == x && r == y){
		//DBN(l, r, time);
		return tr[time][cur];
	}
		...
		int dcur = cur + cur;
		if (y <= m)
			return get_sum(dcur, l, m, x, y, time);
		else if (x > m)
			return get_sum(dcur + 1, m + 1, r, x, y, time);
		else {
			int res = get_sum(dcur, l, m, x, m, time);
			res += get_sum(dcur + 1, m + 1, r, m + 1, y, (time + res) % 60);
			return res;
		}
	}
}
int get_sum(int x, int y){
	int ans =  get_sum(1, 0, n - 1, x, y, 0);
	//DBN(x, y, ans);
	return ans;
}
int main()
