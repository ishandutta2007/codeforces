...
#include <bits/extc++.h>
#else
#define __gnu_pbds std
#define gp_hash_table map
#endif
#define nln puts("")
...
#ifdef COMB
	fac[0]=inv[0]=inv[1]=1;
	rg(200)fac[i]=fac[i-1]*i%mod;gr
	rg(200)if(i>1)inv[i]=inv[mod%i]*(mod-mod/i)%mod;gr
	rg(200)rnv[i]=inv[i];gr
	rg(200)inv[i]=inv[i]*inv[i-1]%mod;gr
#endif
}
#ifdef COMB
int C(int n,int m)
...
#endif
int dp[4056][80],ans1[80],ans2[80],tmp[80];
...
void solve(int n,int *a)
{
	...
	// rg(n)dp[i][0]=1;gr
	__gnu_pbds::gp_hash_table<int,std::vector<int> >dp[4005];
	rg(n-1)
	int u=read(),v=read();
	// int u=n,v=i;
	g[u].push_back(v),g[v].push_back(u);gr
	rg(n)g[i].push_back(0);gr
	rg_(u,n)for(auto v:g[u])
	{
		// odp(u,v);
		dp[u][v].resize(k+1);
		// odl(dp[u][v].size());fflush(stdout);
		dp[u][v][0]=1;
	}gr
	// return;
	rg(k)rg_(u,n)
	// memcpy(tmp,dp[u],sizeof(tmp));
	std::vector<int>tmp;tmp.resize(k+1);
	for(auto p:g[u])
	{
		if(p)
		{
			for(int k=0;k<=i;k++)mae(tmp[k],dp[p][u][k]);
		}
	}
	// for(auto G:tmp)odb(G);puts("");
	for(auto p:g[u])
	{
		int &ans=dp[u][p][i];
		for(int k=2;k<=i;k++)mae(ans,me(dp[u][p][i-k],tmp[k-2]));
	}
	for(auto p:g[u])
	{
		int &ans=dp[u][p][i];
		...
		{
			if(v==p&&v)
			for(int k=2;k<=i;k++)mde(ans,me(dp[u][p][i-k],dp[v][u][k-2]));
			;
		}
	}
	...
	// return;
	rg(n)
	// odl(dp[i][0].size());fflush(stdout);
	rg_(j,k)mae(a[j],dp[i][0][j]);gr gr
	
	// rg(n)rg_(j,k)odb(dp[i][j]);gr puts("");gr
	// puts("");
	// rg(k)odb(a[i]);gr
	// puts("");
}
signed main()
{
	...
	for(int i=0;i<=k;i++)
		for(int j=0;j<=k;j++)
		{
			if(i+j==k)
			{
				mae(ans,me(me(ans1[i],ans2[j]),C(k,i)));
			}
		}
	oldl(ans);
	return 0;
