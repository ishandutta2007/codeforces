	...
	solve.cases{
		...
		repeat(q){
			...
			for(i in 0 until minOf(s1.length,s2.length)){
				if(s1[s1.length - 1 - i] == s2[s2.length - 1 - i]) maxRhyme ++
				else break
			}
			if(mark == 1){
				atmost = minOf(atmost, maxRhyme)
			}else{
				atleast = maxOf(maxRhyme + 1, atleast)
			}
...
//		2 dei atleast
		if(atmost < atleast){
			...
			put(atmost - atleast + 1 )
			for(i in atleast.. atmost){
				put(i)
			}
		}
