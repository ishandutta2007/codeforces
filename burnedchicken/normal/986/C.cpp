...
const int maxn=pow2(22),maxm=pow2(17),maxk=7777714;

//i_am_noob
//#define wiwihorz  
int n,m,res;
bool a[maxn],vis1[maxn],vis2[maxn];

void dfs(int u, int de){
    if(de==1){
        if(vis1[u]) return;
        vis1[u]=1;
        if(!vis2[u^(pow2(n)-1)]) dfs(u^(pow2(n)-1),2);
    }
    else{
        if(vis2[u]) return;
        vis2[u]=1;
        if(a[u]&&!vis1[u]) dfs(u,1);
        rep(n) if(u>>i&1) if(!vis2[u^pow2(i)]) dfs(u^pow2(i),2);
    }
}
