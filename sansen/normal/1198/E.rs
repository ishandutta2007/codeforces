...
fn run() {
    ...
    for a in a.iter() {
        for i in 1..len {
            for j in 1..len {
                if a.0 <= z[i - 1] && z[i] <= a.2 && a.1 <= z[j - 1] && z[j] <= a.3 {
                    dp[i][j] = true;
                }
            }
        }
    }
    let mut graph = max_flow::Graph::new(2 * len + 2);
    let src = 2 * len;
    let dst = src + 1;
    for i in 1..len {
        graph.add_edge(src, i, z[i] - z[i - 1]);
        graph.add_edge(i + len, dst, z[i] - z[i - 1]);
    }
    for i in 1..len {
        for j in 1..len {
            if i == j {
            ...
            if dp[i][j] {
                graph.add_edge(i, j + len, max_flow::INF);
            }
        }
    }
    let ans = graph.flow(src, dst);
    println!("{}", ans);
}
