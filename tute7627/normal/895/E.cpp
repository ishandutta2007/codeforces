...
struct LazySegmentTree {
  ...
  Monoid g(Monoid x,OperatorMonoid y){
    return MP(x.fi*y.fi+y.se*x.se,x.se);
  };
  OperatorMonoid h(OperatorMonoid x,OperatorMonoid y){
    return MP(y.fi*x.fi,y.fi*x.se+y.se);
  };
  const Monoid M1 = MP(0,0);//
  const OperatorMonoid OM0 =MP(1,0);//
  ...
  void print(){
    for(ll i=0;i<sz;i++)if((*this)[i]==M1)cout<<"x";else cout<<(*this)[i]<<"|";
    cout<<endl;
  }
};
...
int main(){
  ...
  while(q--){
    ...
    if(t==1){
      ...
      ld sum1=seg.query(l1,r1).fi;
      ld sum2=seg.query(l2,r2).fi;
      ll k1=r1-l1,k2=r2-l2;
      seg.update(l1,r1,MP(ld(k1-1)/k1,ld(sum2)/k1/k2));
      seg.update(l2,r2,MP(ld(k2-1)/k2,ld(sum1)/k1/k2));
    }
    else{
      ll l,r;cin>>l>>r;l--;
      cout<<seg.query(l,r).fi<<endl;
    }
  }
  return 0;
}