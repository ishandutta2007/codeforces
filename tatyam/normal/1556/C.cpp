...
int main(){
    ...
    rep(n)a[i+1]+=a[i];
    ll ans=0;
    auto solve=[&](ll l,ll r,ll h,auto solve)->void{
        if(r-l<=1)return;
        if(r-l==2){
            ans+=a[l+1]-max({a[l],a[r],h});
            return;
        }
        vector<ll>idx;
        rep(i,l,r+1)if(a[i]<=h)idx.push_back(i);
        ans+=idx.size()*(idx.size()-1)/2;
        rep(i,1,idx.size()){
            const ll x=idx[i-1],y=idx[i];
            const ll mn=*min_element(all(a,l+1,r));
            ans+=mn-h-1;
            solve(x,y,mn,solve);
        }
        if(idx.front()!=l)solve(l,idx.front(),*min_element(all(a,l,idx.front())),solve);
        if(idx.back()!=r)solve(idx.back(),r,*min_element(all(a,idx.back()+1,r+1)),solve);
    };
    solve(0,n,min(a),solve);
    out(ans);
}