...
void solve()
{
    ...
    for (int i = 1; i<K; i++)
    {
        if (posr[i-1]+rdir>n || posr[i-1]+rdir<1) rdir = -rdir;
        posr[i] = posr[i-1] + rdir;
    }
    ...
    for (int i = 1; i<K; i++)
    {
        if (posc[i-1]+cdir>m || posc[i-1]+cdir<1) cdir = -cdir;
        posc[i] = posc[i-1] + cdir;
    }


    vector<int> good;
    for (int i = 0; i<K; i++) if (posr[i]==rd || posc[i]==cd) good.push_back(i);

    int ans = 0;

    int sz = good.size();

    int heh = po(sub(1, p), sz);

    int ser1 = series1(heh);
    int ser2 = series2(heh);

    for (int i = 0; i<sz; i++)
    {
        //at j-th loop: distance K*(j-1) + good[i], probability p * (1-p)^(sz*(j-1) + i)
        //So we need to sum up (K*j + good[i]) * p * (1-p)^(sz*j + i) over all j

        //Or good[i] * p * (1-p)^i * heh^j

        int tmp1 = mul(good[i], p);
        tmp1 = mul(tmp1, po(sub(1, p), i));
        tmp1 = mul(tmp1, ser1);

        int tmp2 = mul(K, p);
        tmp2 = mul(tmp2, po(sub(1, p), i));
        tmp2 = mul(tmp2, ser2);
