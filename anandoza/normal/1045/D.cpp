...
int main() {
    ...
    // add dummy node that never survives
    p.push_back(0);
    tr.parent[0] = n;
    ...
    auto nodes = [&](int i) { return p[i]; };
    auto edges = [&](int i) { return -p[i] * sum[i]; };
    auto score = [&](int i) { return nodes(i) + edges(i) + edges(tr.parent[i]); };
