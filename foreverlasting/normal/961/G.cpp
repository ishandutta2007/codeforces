...
namespace MAIN{
    ...
    inline void MAIN(const res &Case){
        n=read(),k=read(),inv[0]=inv[1]=1;
        if(n==1){printf("%d\n",read());return;}
        for(res i=2;i<=k;i++)inv[i]=mul(inv[kcz%i],kcz-kcz/i);
        for(res i=2;i<=k;i++)inv[i]=mul(inv[i-1],inv[i]);
        res ans=0;
        for(res j=0;j<k;j++){
            if(j&1)add(ans,kcz-mul(mul(qpow(k-j,n-2),n-1+k-j),mul(inv[j],inv[k-1-j])));
            else add(ans,mul(mul(qpow(k-j,n-2),n-1+k-j),mul(inv[j],inv[k-1-j])));
        }
        res ret=0;
        for(res i=1;i<=n;i++)add(ret,read());
        printf("%d\n",mul(ret,ans));
    }
}
int main(){
