...
struct union_find {
    struct node {
        int par, rnk, size; T state; int ect;
        node(int id = 0) : par(id), rnk(0), size(1), state(id), ect(0) {}
...
int main() {
    ...
    int uh = 0;
    F0R (i, N) if (uf.rep(i) == i) {
        if (uf.uf[i].ect >= uf.uf[i].size)
            uh += uf.uf[i].ect - uf.uf[i].size + 1;
    }
    ps(uh);
