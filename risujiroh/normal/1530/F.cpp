...
int main() {
  ...
  using Fp = atcoder::static_modint<31607>;
  int n;
  cin >> n;
  vector p(n, vector<Fp>(n));
  for (int i = 0; i < n; ++i)
    for (int j = 0; j < n; ++j) {
      int a = 5000;
      cin >> a;
  ...
  vector<Fp> inv(31607);
  for (int i = 1; i < 31607; ++i) inv[i] = Fp(i).inv();
  auto go = [&](const vector<bitset<21>>& bs) -> Fp {
    Fp res;
    int mask = 0;
    Fp coeff = 1;
    vector<Fp> a(n, 1);
    ...
    auto dfs = [&](auto self, int i) -> void {
      if (i == n) {
        ...
        res += power(-1, __builtin_popcount(mask)) * coeff * (f[0] + f[1]);
        return;
      }
      self(self, i + 1);
      mask |= 1 << i;
  ...
  {
    vector<bitset<21>> bs(n);
    for (int i = 0; i < n; ++i) bs[i][n - i - 1] = 1;
    ans += go(bs);
  }
  {
    vector<bitset<21>> bs(n);
    for (int i = 0; i < n; ++i) bs[i][i] = bs[i][n - i - 1] = 1;
    ans -= go(bs);
  }
  cout << ans.val() << '\n';
}