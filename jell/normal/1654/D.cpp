#line 1 "codeforces-workspace\\contest\\1654\\d\\d.cpp"
...
#line 13 "codeforces-workspace\\contest\\1654\\d\\d.cpp"
...
#line 30 "codeforces-workspace\\contest\\1654\\d\\d.cpp"
namespace workspace {
...
least_factor<1 << 18> lpf;

void main() {
  // start here!

  input n;
  vector<vector<tuple<i32, i32, i32>>> G(n);
  ...
  vector<i32> dif(n + 1), max(n + 1);
  mint ans;

  fixed_point dfs = [&](auto dfs, i32 now, i32 pre, mint rat) -> void {
  ...
  for (auto i : range(1, max.size())) {
    ans *= mint(i).pow(max[i]);
  }
