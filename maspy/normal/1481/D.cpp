...
#line 1 "/home/maspy/compro/library/graph/tournament.hpp"
// https://official.contest.yandex.ru/opencupXXII/contest/30766/problems/K
// a  b  bool check(int, int) 
template <typename F>
...
void solve() {
  ...
  VEC(string, A, N);
  FOR(a, N) FOR(b, a) if (A[a][b] == A[b][a]) {
    FOR(i, M + 1) ANS[i] = (i % 2 == 0 ? a : b);
    return out();
  }
  if (M % 2 == 1) {
    FOR(i, M + 1) ANS[i] = (i % 2 == 0 ? 0 : 1);
    return out();
  }

  if (N == 2) return NO();

  auto check = [&](int a, int b) -> bool { return A[a][b] == 'a'; };
  auto P = hamiltonian_path_in_tournament(N, check);
  P.resize(3);
  auto a = P[0], b = P[1], c = P[2];
  if (M % 4 == 0) {
    FOR(i, M + 1) {
      if (i % 4 == 0) ANS[i] = b;
      if (i % 4 == 1) ANS[i] = c;
