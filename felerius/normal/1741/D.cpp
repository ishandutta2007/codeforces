// Masha and a Beautiful Tree (https://codeforces.com/contest/1741/problem/D)
...
int main() {
    ...
    while (t--) {
        ...
        int op = 0;
        auto check = [&](auto&& self, int l, int r, int vl, int vr) -> bool {
            if (r - l == 1)
                return p[l] == vl;
            int m = (l + r) / 2;
            auto [mn_it_l, mx_it_l] = minmax_element(begin(p) + l, begin(p) + m);
            auto [mn_it_r, mx_it_r] = minmax_element(begin(p) + m, begin(p) + r);
            int mn_l = *mn_it_l, mx_l = *mx_it_l, mn_r = *mn_it_r, mx_r = *mx_it_r;
            if (mx_l > mx_r) {
                ++op;
                rep(i, m - l)
                    swap(p[l + i], p[m + i]);
            ...
            int vm = (vl + vr) / 2;
            if (mn_l != vl || mx_l != vm - 1 || mn_r != vm || mx_r != vr - 1)
                return false;
            return self(self, l, m, vl, vm) && self(self, m, r, vm, vr);
        };

        auto pos = check(check, 0, n, 0, n);
        println(pos ? op : -1);
    }
}