...
signed main(){
    read(n, m); read(a + 1, a + m + 1); int t = 1; rep(i, 1, m) { stk[++ tp] = a[i]; while(stk[tp] == t) -- tp, ++ t; }
    REP(i, 1, tp) if(stk[i + 1] > stk[i]) return print("-1"), 0;
    sprint(a + 1, a + m + 1); stk[0] = n + 1; stk[++ tp] = t - 1; while(tp) { int x = stk[tp --]; PER(i, stk[tp] - 1, x) sprint(i); }
}