...
public:
    void solve() {
        ...
        RecursiveFunction rec = [&](const auto& self, int from, int sz) -> void {
            if (sz == 1) {
                return;
            }
            self(from, sz / 2);
            self((from + sz / 2) % n, sz / 2);
            for (int i : range(sz / 2)) {
                ans.emplace_back((from + i) % n + 1, (from + i + sz / 2) % n + 1);
            }
        };
        rec(sz, sz);
        rec(0, sz);
