...
public class D {
	...
	void solve()
	{
		...
		int p = 0;
		for(int i = 0;i < n;i++){
			es[p++] = new int[]{co[i][0]-K+1, co[i][1]-K+1, co[i][1]+1, 1};
			es[p++] = new int[]{co[i][0]+1, co[i][1]-K+1, co[i][1]+1, -1};
		}
		int o = 1000000000+305;
		int[] ys = new int[p];
		for(int i = 0;i < p;i+=2){
			ys[i] = es[i][1]+o;
			ys[i+1] = es[i][2]+o;
		...
		int[] val = new int[m+1];
		for(int[] e : es){
			for(int u = e[1];u < e[2];u++){
				ss[val[u]+e[3]] -= e[0]*(long)(imap[u+1]-imap[u]);
				ss[val[u]] += e[0]*(long)(imap[u+1]-imap[u]);
				val[u] += e[3];
			}
		}
		for(int i = 1;i <= n;i++){
