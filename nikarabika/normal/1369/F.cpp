...
bool who_win(LL s, LL e) {
    if (s * 2 > e)
        return (s ^ e) & 1;
    LL k = e / 4;
    if (e & 1) {
        if (e % 4 == 1)
            return s == 2 * k ? 1 : who_win(s, 2 * k - 1);
        return who_win(s, 2 * k + 1);
    }
    return who_lose(s, e / 2);
}

bool who_lose(LL s, LL e) {
    if (s * 2 > e)
        return 1;
    return who_win(s, e / 2);
}
...
int solve() {
    ...
    for (int i = 0; i < n; i++) {
        LL fi, se;
        cin >> fi >> se;
        bool curW = who_win(fi, se);
        bool curL = who_lose(fi, se);

        bool newW = (canL and curW) or (canW and !curW);
        bool newL = (canL and curL) or (canW and !curL);
