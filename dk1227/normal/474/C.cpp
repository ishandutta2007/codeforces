...
signed main() {
	...
	W(n) {
		rep(i, 0, 3) {
			...
			x -= a, y -= b;
			X[i * 4] = x, Y[i * 4] = y;
			rep(j, 1, 3) X[i * 4 + j] = -Y[i * 4 + j - 1], Y[i * 4 + j] = X[i * 4 + j - 1];
			rep(j, 0, 3) X[i * 4 + j] += a, Y[i * 4 + j] += b;
		}
		int Ans = 1e9;
		rep(i, 0, 3) rep(j, 4, 7) rep(k, 8, 11) rep(l, 12, 15) {
			ans[1] = dis(i, j);
			ans[2] = dis(i, k);
			ans[3] = dis(i, l);
			ans[4] = dis(k, j);
			...
			ans[6] = dis(k, l);
			sort(ans + 1, ans + 7);
			if(ans[1] == ans[2] && ans[2] == ans[3] && ans[3] == ans[4] && ans[5] == ans[6] && ans[1] * 2 == ans[5] && ans[1]) To_min(Ans, i + j + k + l - 24);
		}
		printf("%d\n", Ans == (int)(1e9) ? -1 : Ans);
	}
 	return 0;
