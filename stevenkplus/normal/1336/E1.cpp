...
int main() {
  ...
  int od = sz(bbits);
  for (int i = 0; i < d; ++i) {
    for (int j = 0; j < sz(abits); ++j) {
      a[i] += ((basis[i] >> abits[j]) & 1) << j;
    }
    for (int j = 0; j < sz(bbits); ++j) {
      b[i] += ((basis[i] >> bbits[j]) & 1) << j;
    }
  }
  pp(a, b);
  pp(abits, bbits);
  ...
  if (d <= 20) {
    for (int i = 0; i < 1 << d; ++i) {
      ...
      ans[__builtin_popcountll(k)] += 1;
    }
  } else {
    vector<vector<mint>> dp(d + 1, vector<mint>(1 << od));
    dp[0][0] = 1;
    for (int i = 0; i < d; ++i) {
      for (int j = d - 1; j >= 0; --j) {
        for (int k = 0; k < 1 << od; ++k) {
          dp[j + 1][k ^ b[i]] += dp[j][k];
        }
      }
    }
    ...
    for (int i = 0; i <= d; ++i) {
      for (int k = 0; k < 1 << od; ++k) {
        ans[i + __builtin_popcount(k)] += dp[i][k];
      }
    }
  }
  mint pow = 1;
  for (int i = 0; i < n - d; ++i) pow *= 2;
  for (mint i: ans) {
    i *= pow;
    cout << i << " ";
  }
