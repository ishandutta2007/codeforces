...
void main_() {
	INT(q);
	RangeAddRangeSumBIT<int> BT(200000);
	int cnt = 0;
	while(q--) {
		CHAR(c);
		INT(x);
		...
		if(c == 'L') {
			BT.add(0, 200000, 1);
			BT.add(x, x + 1, -BT.sum(x, x + 1));
			cnt++;
		} else if(c == 'R') {
			BT.add(x, x + 1, cnt - BT.sum(x, x + 1));
			cnt++;
		} else {
			int pos = BT.sum(x, x + 1);
			print(min(pos, cnt - pos - 1));
		}
	}
}
