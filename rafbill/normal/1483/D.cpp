...
int main(){
  ...
  li n,m;cin>>n>>m;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) D[i][j] = 1ll<<60;
  for(li i = 0; i < (li)(n); ++i) D[i][i] = 0;
  for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) Q[i][j] = 0;
  ...
  for(li k = 0; k < (li)(n); ++k) for(li i = 0; i < (li)(n); ++i) for(li j = 0; j < (li)(n); ++j) smin(D[i][j], D[i][k]+D[k][j]);
  ...
  for(li y = 0; y < (li)(n); ++y) for(li u = 0; u < (li)(n); ++u) if(y != u) {
    li p = 0;
    for(li v = 0; v < (li)(n); ++v) smax(p, Q[u][v] - D[y][v]);
    for(li x = 0; x < (li)(n); ++x) if(E[y][x] && W[y][x] + D[x][u] <= p) {
      ans++; E[y][x] = 0;
    }
  }
