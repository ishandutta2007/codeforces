...
int main(){
    ...
    dij.dis(0);
    int an=dij.mi[n-1];
    REP(i,k)AA[i].F=dij.mi[a[i]];
    dij.dis(n-1);
    REP(i,k)AA[i].S=dij.mi[a[i]];
    REP(i,k)BB[i]=MP(AA[i].S,AA[i].F);
    sort(ALL(AA));
    ...
    while(ll<rr){
        ...
        int it=0;
        PII ma[2];
        REP(i,2)ma[i]=MP(-1000000,-1000000);
        bool good=0;
        REP(i,k){
            while(it<k&&BB[it].F+AA[i].F>=mm){
                update(ma,BB[it]);
                it++;
            }
            if(ma[0]==AA[i]){
                if(AA[i].S+ma[1].F>=mm)good=1;
            }
            else{
                if(AA[i].S+ma[0].F>=mm)good=1;
            }
            if(good)break;
        }
        if(good)ll=mm;
