...
ll cnt(vector<pair<ll, int>> &a, ll p)
{
    /*cout<<"counting"<<endl;
    ...
    int cur = 0;
    ll tot = 0;
    while (2*(tot+a[cur].first)<p) {tot+=a[cur].first; cur++;}

    ll total = 0;
    for (int i = 0; i<n; i++) total+=a[i].first * (abs(a[i].second - a[cur].second));

    return total;
}

ll solve(vector<int> a, ll p)
{
    int n = a.size();
