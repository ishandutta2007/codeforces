...
signed main() {
    ...
    for (int i = 0; i < m; ++i) {
        ...
        if (big[tp]) {
            if (mode == 1) {
                ...
                apply.pb({{alive[tp], i}, tp});
                alive.erase(tp);
            }
        } else {
            int add = mode == 1 ? 1 : -1;
            int sxy = x[tp] + y[tp];
            int t = (mode == 1 ? i : alive[tp]) % sxy;
            if (mode == 1) {
                alive[tp] = i;
            } else {
                alive.erase(tp);
            ...
            for (int d = 0; d < sxy; ++d) {
                dbg((d - t + sxy) % sxy);
                cnt[sxy][d] += add * (((d - t + sxy) % sxy >= x[tp]));
            }
        }
        for (int sxy = 1; sxy < B; ++sxy) {
            ans[i] += cnt[sxy][i % sxy];
        }
    }
    for (auto i : alive) {
    ...
    for (auto i : apply) {
        dbg(i.f.f, i.f.s, i.s);
        for (int l = i.f.f; l + x[i.s] < i.f.s; l += x[i.s] + y[i.s]) {
            p[l + x[i.s]] += 1;
            p[min(i.f.s, l + x[i.s] + y[i.s])] += -1;
        }
    }
    for (int i = 1; i < m; ++i) {
        p[i] += p[i - 1];
