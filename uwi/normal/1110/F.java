...
//		}
		...
		int[][] qs = new int[Q][];
		for(int i = 0;i < Q;i++){
			qs[i] = new int[]{ni()-1, ni()-1, ni()-1, i};
		}
		long[] dw = new long[n];
		for(int i = 1;i < n;i++){
			dw[i] = dw[par[i]] + pw[i];
		}
		long[] ins = Arrays.copyOf(dw, n);
		for(int i = 0;i < n;i++){
			if(g[i].length != 1){
				ins[i] = Long.MAX_VALUE / 10;
			}
		}
		...
		StarrySkyTreeL sst = new StarrySkyTreeL(ins);
		int p = 0;
		long[] anss = new long[Q];
		for(int i = 0;i < n;i++){
			while(p < Q && qs[p][0] == i){
				anss[qs[p][3]] = sst.min(qs[p][1], qs[p][2]+1) + dw[qs[p][0]];
				p++;
			}
			// i -> i+1
			if(i < n-1){
				int x = i;
				while(x != par[i+1]){
					sst.add(x, right[x], 2L*pw[x]);
					x = par[x];
				}
				sst.add(i+1, right[i+1], -2L*pw[i+1]);
			}
		}
		for(long v : anss){
			out.println(v);
