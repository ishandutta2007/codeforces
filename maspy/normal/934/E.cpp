...
void solve() {
  ...
  FOR(j, N) FOR(k, N) {
    ...
    P dxdy = B - A;
    Re dd = dxdy.dot(dxdy);
    if (dd < (rj - rk) * (rj - rk)) continue;
    if (dd > (rj + rk) * (rj + rk)) continue;
    Re d = sqrt(dd);
    Re cos_v = (dd + rj * rj - rk * rk) / (2 * rj * d);
    Re theta = acos(cos_v);
    Re phi = atan2(dxdy.y, dxdy.x);
    if (cos_v == 1.0 || cos_v == -1.0)
      cross[j][k].eb(P(aj + rj * cos(phi + theta), bj + rj * sin(phi + theta)));
    else {
      cross[j][k].eb(P(aj + rj * cos(phi + theta), bj + rj * sin(phi + theta)));
      cross[j][k].eb(P(aj + rj * cos(phi - theta), bj + rj * sin(phi - theta)));
    }
  ...
  UnionFind uf(N);
  ll comp = N;
  FOR(i, N) FOR(j, N) if (len(cross[i][j])) comp -= uf.merge(i, j);

  Re eps = 1e-12;

  vc<P> points;
  FOR(i, N) FOR(j, N) for (auto&& p: cross[i][j]) {
    bool ok = [&]() {
      for (auto&& q: points) {
        if ((p - q).dot(p - q) < eps) return false;
      }
      return true;
    }();
    if (ok) points.eb(p);
  }
  // for (auto&& p: points) print(p.x, p.y);
  ll v = len(points);
  ll e = 0;
  FOR(i, N) for (auto&& p: points) {
    auto [a, b, r] = C[i];
    P O(a, b);
    if (abs(r * r - (p - O).dot(p - O)) < eps) ++e;
  }
  // print(comp, v, e);
  print(comp + e - v + 1);
}
