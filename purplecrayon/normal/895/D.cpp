...
num get(string a, string b) { //# perms of a that are < b
    int n = sz(a);
    int cnt[26]={};
    ...
    for (int i = 0; i < n; i++) {
        num tot = fact[n-i-1];
        for (int j = 0; j < 26; j++) tot *= ifact[cnt[j]];

        for (int j = 0; j < b[i]-'a'; j++) if (cnt[j]) { //choose this as the next letter
            num cur = tot*fact[cnt[j]]*ifact[cnt[j]-1];
            ans += cur;
        }
        if (!cnt[b[i]-'a']) break;
        cnt[b[i]-'a']--;
