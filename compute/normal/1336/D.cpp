...
int main()
{
    ...
    cin>>n;
    vc<pi> res(n+1);
    cin>>res[n].first>>res[n].second;
    vi ans(n+1);
    for(int i=n-1;i>2;i--) res[i]=ask(i);
    res[1]=ask(1);
    res[2]=ask(2);
    res[0]=ask(1);
    res[0].first-=res[2].first;
    res[0].second-=res[2].second;
    res[2].first-=res[1].first;
    res[2].second-=res[1].second;
    res[1].first-=res[3].first;
    res[1].second-=res[3].second;
    for(int i=3;i<n;i++)
    {
        res[i].first=res[i].first-res[i+1].first;
        res[i].second=res[i].second-res[i+1].second;
    }
    auto gett=[&](int d){
        int ret=0;
        while(ret*(ret+1)/2<d) ret++;
        return ret;
    };
    //for(int i=0;i<n-1;i++) dbg(i,res[i].first,res[i].second);
    ans[1]=gett(res[0].first);
    ans[3]=res[0].second-res[1].second-1;
    ans[2]=res[1].second/(ans[3]+1);
    ans[4]=(res[2].second-(ans[1]+1)*(ans[3]+1))/(ans[3]+1)-1;
    for(int i=5;i<=n;i++)
    {
        int cur=i-2;
        int rhs=ans[cur-2]*ans[cur-1]+ans[cur-1]*(ans[cur+1]+1);
        ans[i]=(res[cur].second-rhs)/(ans[cur+1]+1)-1;
    }
    ans[n]++;
    cout<<"! ";
