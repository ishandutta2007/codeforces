...
void solve() {
    ...
    vll cp = p; sort(all(cp));
    F0R(i, n+1) p[i] = lower_bound(all(cp), p[i]) - cp.begin() + 1;
    dp.assign(n+1, -1e18); dp[0] = 0;
    st1.init(n+2); st1.upd(p[0], 0);
    st2.init(n+2); st2.upd(p[0], 0);
    FOR(i, 1, n+1) {
        if (a[i-1]<0) dp[i] = dp[i-1]-1;
        dp[i] = max(dp[i], st1.query(p[i], p[i]));
        dp[i] = max(dp[i], st2.query(0, p[i]-1) + i);
        st1.upd(p[i], dp[i]);
        if (dp[i]-i > st2.query(p[i], p[i])) st2.upd(p[i], dp[i]-i);
    }
    cout << dp[n] << nl;
}
