...
int main() {
	...
	const int LOGM = 19;
	vector<vector<int>> table(LOGM + 1, vector<int>(m + 1, 0));

	auto get_table = [&](int p, int v) {
		if (v > m) return 0;
		return table[p][v];
	};

	for (int p = 0; p <= LOGM; p++) {
		const int jump = (1 << p);

		for (int i = m; i >= 1; i--) {
			table[p][i] = get_table(p, i + 2 * jump) ^ get(i + jump, i + 2 * jump - 1);
		}
	}
	...
	CASE(t) {
		...
		for (int p = 0; p <= LOGM; p++) {
			...
			int bit = table[p][l] ^ get_table(p, fr) ^ get(fr + jump, r);
			if (bit)
				isBob = false;
		}
