...
void work()
{
    ...
    for(int i = k; i; i--) b[i] += b[i + 1];

    int mx = 0;
    for(int i = 1; i <= k; i++) mx = max(mx, (b[i] + c[i] - 1) / c[i]);

    cout <<mx <<endl;

    for(int i = 1; i <= k; i++) b[i] -= b[i + 1];

    for(int i = 1; i <= mx; i++) ansvec[i].clear();
    for(int i = k; i >= 1; i--)
    {
        while(b[i])
        {
