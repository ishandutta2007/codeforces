...
void dfs(int cur) {
    ...
    if (vk.size() == 0) {
        ...
        int to = vk[0];
        dp[cur][0] = max(1 + dp[to][1], dp[to][0]);
        dp[cur][1] = max(1 + dp[to][1], dp[to][0]);
    } else {
        int sum = 0;
        int to1 = -1;
        int to2 = -1;
...
//            DBN(sum, max1, max2);
//        }
        dp[cur][1] = max(sum + max1 + 1, sum);
        dp[cur][0] = max(sum + max1 + 1 + max2 + 1, sum);
    }
