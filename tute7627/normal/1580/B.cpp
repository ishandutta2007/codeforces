...
int main(){
  ...
  modint::set_mod(p);
  auto dp=make_v(n+1,n+1,n+1,modint(0));
  dp[0][0][0]=1;
  vector<modint>fact(n+1,1);
  rep(i,0,n)fact[i+1]=fact[i]*(i+1);
  auto c=binomial_table<modint>(1000);
  rep(i,0,n+1){
    rep(j,0,n+1){
      if(j==0||j>i){
        dp[i][j][0]=fact[i];
      ...
      rep(o,0,i-j+2){
        rep(l,0,i){
          ...
          rep(x,0,o+1){
            int y=o-x-(j==1);
            if(y<0)continue;
            //if(i==3&&j==1&&o==1)cout<<l spa r spa x spa y spa dp[l][j-1][x] spa dp[r][j-1][y]<<endl;
            dp[i][j][o]+=dp[l][j-1][x]*dp[r][j-1][y]*c[i-1][l];
          }
        }
