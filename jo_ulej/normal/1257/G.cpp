...
void solve() {
	...
	priority_queue<int, vector<int>, CompareBySize> setik;
	FFT fft(998244353);

	for(auto& pr: cnt) {
		v[head] = vector<int> (pr.second + 1, 1);
		setik.push(head++);
	}

	while(len(setik) > 1) {
		int i = setik.top(); setik.pop();
		int j = setik.top(); setik.pop();
