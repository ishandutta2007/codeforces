...
int main() {
  ...
  vector dp(sum + 1, ModInt(0));
  dp[0] = n - 1;  // dp[0] = 1/(n-1) + (n-2)/(n-1) * (dp[0]+1)
  // dp[i] = (sum-i)/sum * 1/(n-1) + (sum-i)/sum * (n-2)/(n-1) * (dp[i]+1) + i/sum * (dp[i-1]+dp[i]+1)
  FOR(i, 1, sum + 1) dp[i] = (dp[i - 1] * i / sum + 1) * sum * (n - 1) / (sum - i);
  for (int i = sum - 1; i >= 0; --i) dp[i] += dp[i + 1];
  ModInt ans = -dp[0] * (n - 1);
  REP(i, n) ans += dp[a[i]];
  cout << ans / n << '\n';
  return 0;
