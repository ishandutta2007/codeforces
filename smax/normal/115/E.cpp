...
struct SegmentTree {
    ...
    void point(int p, int i, long long val) {
        if (st[p].l == st[p].r) {
            st[p].leaf(val);
            return;
        }
    ...
    void point(int i, long long val) {
        point(1, i, val);
    }
};
...
int main() {
    ...
    st.point(0, 0);
    for (int r=0; r<n; r++) {
        for (auto [l, p] : intervals[r])
            st.update(0, l, p);
        pref -= a[r];
        dp[r+1] = max(dp[r], st.query(0, r) + pref);
        st.point(r + 1, dp[r+1] - pref);
    }
