...
ll n,k,a[MAXN],last[MAXN],same[MAXN];
map<int,int> mp;
struct segtree
{
...
int main()
{
    ...
    for(ll i=1;i<=n;i++)
    {
        if(mp[a[i]]) same[i]=mp[a[i]]; else same[i]=0;
        last[i]=(a[i]==1?i:last[i-1]);
        mp[a[i]]=i;
    }
    for(ll i=1;i<=n;i++) same[i]=max(same[i],same[i-1]);
    P st1[MAXN],st2[MAXN];
    ll t1=0,t2=0,ans=0;
    seg.build(1,1,n);
    for(ll i=1;i<=n;i++)
    {
        ...
        st2[t2++]=P(a[i],now);
        P p=seg.query(1,1,n,same[i]+1,last[i]); 
        if(p.F==i) ans+=p.S;
    }
    printf("%lld\n",ans);
    return 0;
