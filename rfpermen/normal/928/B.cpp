...
int n,k,x[MAX],dp[MAX];

int f(int nw){
	if(dp[nw]!=-1)return dp[nw];
	if(!x[nw])return dp[nw] = 1 + min(k,nw-1) + min(k,n-nw);
	int a = min(n,x[nw]+k), b = max(1,nw-k), c = min(n,nw+k);
	if(a<b)return dp[nw] = (c-b+1) + f(x[nw]);
	return dp[nw] = (c-a) + f(x[nw]);
}
