...
int cnts[2000][2000];
int calcs[2000][2000];
pair<int, int> ut[1001][1001];
pair<int, int> uf(pair<int, int> a) {
    if (ut[a.first][a.second] == a) return a;
    return ut[a.first][a.second] = uf(ut[a.first][a.second]);
}
int um(pair<int, int> a, pair<int, int> b) {
    a = uf(a);
    b = uf(b);
    ut[a.first][a.second] = b;
    return 1;
}
void solve() {
    int n, m;
    ...
    REP(i, n) {
        REP(q, m) {
            if (cnts[i][q] == 0) continue;
            pair<int, int> now = uf(mp(i, q));
            if (calcs[now.first][now.second] == 0) {
                calcs[now.first][now.second] = 1;
                ans++;
            }
        }
    }
