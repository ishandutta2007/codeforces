...
void solve() {
  ...
  {
    UnionFind uf(N);
    FOR(i, M) if (C[i] == 0) uf.merge(FRM[i], TO[i]);
    FOR(i, M) if (C[i] == 1 && uf.merge(FRM[i], TO[i])) I.eb(i);
  }
  vi ANS;
  UnionFind uf(N);
  FORIN(i, I) {
  ...
  FOR(i, M) if (C[i] == 1) {
    if (ANS.size() >= K) break;
    if (uf.merge(FRM[i], TO[i])) ANS.eb(i);
  }
  if (ANS.size() != K) return print(-1);
  FOR(i, M) if (C[i] == 0 && uf.merge(FRM[i], TO[i])) ANS.eb(i);
  if (ANS.size() != K + K) return print(-1);
