...
fun main(){
    solve.cases {
        val n = getint()
        val p = getint()
        val things = mutableMapOf<Pair<Int,Int>,Int>()
        val used = IntArray(n)
        repeat(n){
            val a = getint() - 1
            val b = getint() - 1
            val p = Pair(minOf(a,b),maxOf(a,b))
            things[p] = things.getOrDefault(p,0) + 1
            used[a] ++
            used[b] ++
        }
        var start = 0L
        val all = used.toList().sorted()
        var rightpointer = n-1
        for(l in 0 until n){
            while(rightpointer >= 0 && all[l] + all[rightpointer] >= p){
                rightpointer --
            }
            start += (n-1-rightpointer)
        }
        for(i in 0 until n){
            if(all[i] + all[i] >= p){
                start --
        ...
        for((c,v) in things.entries){
            val base = used[c.first] + used[c.second]
            val new = base - v
            if(base >= p){
                start --
            }
