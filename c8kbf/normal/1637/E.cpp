...
_ n, m, a[maxn], cnt, b[maxn], d[maxn], nn, c[maxn];
v_ g[maxn], h, bd[maxn], p;
bitset<maxn> v;
...
inline void AC() {
    ...
    for(_ i = 1; i <= n; ++i) b[i] = a[i] = read();
    sort(b+1, b+1+n);
    nn = unique(b+1, b+1+n)-b-1;
    for(_ i = 1; i <= n; ++i) {
        d[i] = lower_bound(b+1, b+1+nn, a[i])-b;
        ++c[d[i]];
    }
    for(_ i = 1; i <= nn; ++i) {
        h.push_back(c[i]);
    ...
    for(_ i = 1; i <= m; ++i) {
        _ x = read();
        _ y = read();
        x = lower_bound(b+1, b+1+nn, x)-b;
        y = lower_bound(b+1, b+1+nn, y)-b;
        bd[x].push_back(y);
        bd[y].push_back(x);
    }
    for(_ i = 1; i <= nn; ++i) bd[i].push_back(i);
    for(_ i : h) sort(g[i].begin(), g[i].end());
    _ s = -1E11;
    for(_ i = 1; i <= nn; ++i) {
        for(_ j : p) v.reset(j);
        p = bd[i];
        for(_ j : bd[i]) v.set(j);
        for(_ j : h) {
            _ dx = (_)g[j].size()-1;
            for(; dx >= 0 && v[g[j][dx]]; ) --dx;
            if(dx < 0) continue;
            s = max(s, (c[i]+j)*(b[i]+b[g[j][dx]]));
        }
    }
    writeln(s);
