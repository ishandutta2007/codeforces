...
int main(){
  ...
  rep(i,0,n){
    co.PB(c[i]);
    co.PB((c[i]+g[i])%t);
    co.PB(modulo(c[i]+g[i]-1,t));
  }
  co.PB(0);
  co.PB(t);
  Compress<ll>mp(co);
  ll sz=mp.size();
  auto seg=upd_min::make(sz);
  rep(i,0,sz){
    if(mp.val(i)!=t)seg.set(i,-mp.val(i));
  }
  seg.build();
  //debug(mp.v);
  rep(i,0,n){
    ll mi1=c[i]+t+seg.query(0,mp[t]);
    ll mi2=c[i]+seg.query(0,mp[c[i]]+1);
    ll mi=min(mi1,mi2);
    //cout<<c[i] spa g[i] spa mi<<endl;
    seg.update(mp[c[i]],mi-c[i]);
    seg.update(mp[max(0LL,c[i]+g[i]-t)],mp[c[i]],INF/2);
    if(c[i]+g[i]<t)seg.update(mp[c[i]+g[i]],mp[t],INF/2);
    //seg.print();
  }
  ll ret=INF;
  rep(i,0,sz){
    if(mp.val(i)>=t)continue;
    chmin(ret,seg[i]+mp.val(i));
  }
  cout<<ret+ds<<endl;
  return 0;
}