...
uint64_t mul_mod64_float(uint64_t a, uint64_t b, uint64_t m) {
    static_assert(sizeof(long double) >= 10, "long double too short");
    if (a >= m) a %= m;
    if (b >= m) b %= m;
    uint64_t q = (long double)a * b / m;
    int64_t r = (int64_t)(a * b - q * m);
    return r >= (int64_t)m ? r - m : r < 0 ? r + m : r;
}
...
void main() {
    ...
    while ( t-- ) {
        LL p,q,b;
        R(p,q,b);
        REP(i,60) p=mul_mod64_float(p,b,q);
        W(p==0 ? "Finite" : "Infinite");
    }
}
