...
int main(){
  ...
  // { lli ans0 = 0;
  //   FOR(m,1<<n) if(m) {
  //     ans0 += fexp(__builtin_popcount(m),k);
  //   }
  //   cout << ans0 << endl;
  // }
  ...
  vvi ps(k+1);
  { ps[0].assign(1,1); }
  FOR(i,k) { // ps[i+1] = ps[i] * (X-i);
    ps[i+1].resize(i+2);
    FOR(j,i+1) (ps[i+1][j+1] += ps[i][j]) %= MOD;
    FOR(j,i+1) ps[i+1][j] = nmod(ps[i+1][j] - i * ps[i][j]);
  }
  FORD(i,k,0) {
    r[i] = p[i];
    // p -= r[i] * ps[i];
    FOR(j,i+1) p[j] = nmod(p[j]-r[i]*ps[i][j]%MOD);
  }
  Zn ans = Zn(0);
  FORU(i,1,k) if(i<=n) {
    Zn cur = Zn(fexp(2,n-i))*r[i];
    FOR(j,i) cur = cur * (n-j);
    ans += cur;
  }
  cout << ans.x << endl;
