...
struct UnionFind {
    ...
    UnionFind(int n) : num(n), l(n) { data.assign(n, -1); }
...
struct Diameter {
    ...
    int build(int x) {
        if(empty(to)) to.assign(g.size(), -1);
        auto p = dfs(x, -1);
...
int main() {
    ...
    Graph g = getG(n, m);
    UnionFind uf(n);
    rep(i, n) for(auto e : g[i]) uf.unite(i, e);
    Diameter diameter(g);
    rep(i, n) if(uf[i] == i) { uf.l[i] = diameter.build(i); }
    while(q--) {
        INT(t);
        if(t == 1) {
            INT(a);
            cout << uf.l[uf[a - 1]] << endl;
        } else {
            INT(a, b);
            a--, b--;
            uf.unite(a, b);
