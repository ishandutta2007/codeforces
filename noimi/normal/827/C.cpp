...
#define BIT BinaryIndexedTree<int>
signed main(){
    ...
    mp['C'] = 3;
    vector<int> v;
    for(auto e:s)v.pb(mp[e]);
    int n = s.size();
    auto bit = vector<vector<vector<BIT>>>(11,vector<vector<BIT>>(10,vector<BIT>(4,BIT(n+1))));
    rep2(i,1,10){
        rep(j,n){
            bit[i][j%i][v[j]].add(j+1,1);
        }
    }
    INT(Q);
    while(Q--){
        ...
        if(id==1){
            ...
            char c;cin>>c;
            int y = mp[c];
            rep2(i,1,10)bit[i][x%i][v[x]].add(x+1,-1);
            v[x] = y;
            rep2(i,1,10)bit[i][x%i][v[x]].add(x+1,1);
        }
        else{
            ...
            int ans = 0;
            rep(i,m){
                ans+=bit[m][(l+i)%m][mp[t[i]]].sum(r);
                ans-=bit[m][(l+i)%m][mp[t[i]]].sum(l);
            }
            cout<<ans<<"\n";
        }
    }
