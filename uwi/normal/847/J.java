...
public class J {
	...
	void solve()
	{
		...
		while(high - low > 1){
			...
			for(int i = 0;i < m;i++){
				edges.add(new Edge(src, i, 1));
				edges.add(new Edge(i, es[i][0]+m, 1));
				edges.add(new Edge(i, es[i][1]+m, 1));
			}
			for(int i= 0;i < n;i++){
				edges.add(new Edge(m+i, sink, h));
			}
			if(maximumFlowDinicNoRec(compileWD(sink+1, edges), src, sink) == m){
				high = h;
			}else{
				low = h;
			}
		...
		out.println(high);
		{
			...
			Edge[][] g = compileWD(sink+1, edges);
			maximumFlowDinicNoRec(g, src, sink);
			for(Edge e : edges){
				if(e.from < m && e.to >= m && e.to < m+n && e.flow == 1){
					out.println((e.to-m+1) + " " + ((e.to-m^es[e.from][0]^es[e.from][1])+1));
				}
			}
		}
	}
