...
struct solver
{
    ...
    solver()
    {
        ...
        sort(__all(nio),[](auto x,auto y){return x.b<y.b;});
        int dp[80][80];
        init(dp,-1000000000);
        dp[0][0]=0;
        int bt[80][80]={};
        for(int x=0; x<n; ++x)
        {
            ...
            for(int i=0; i<=k; ++i)
            {
                if(chmax(dp[x+1][i],dp[x][i]+(k-1)*b))
                {
                    bt[x][i]=0;
                }
                if(chmax(dp[x+1][i+1],dp[x][i]+i*b+a))
                {
                    bt[x][i+1]=1;
                }
            }
        ...
        for(int x=n; x--; )
        {
            ...
            {
                ans2.emplace_back(id);
            }
        }
        reverse(__all(ans));
        int la=ans.back(); ans.pop_back();
        for(int x: ans2)
        {
            ans.emplace_back(x);
            ans.emplace_back(-x);
