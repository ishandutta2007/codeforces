...
fun gen(a: Int, b: Int) {
    x = ArrayList<Pair<Int,Int>>()
    y = ArrayList<Pair<Int,Int>>()
    for (i in 0..a-1) {
        x.add(Pair(3+2*i,3)) // white
        y.add(Pair(4+2*i,3)) // black
    }
    y.add(Pair(2,3))
    for (i in 0..a-1) {
        y.add(Pair(3+2*i,4))
...
fun solve() {
    val (b,w) = nextInts()
    val mx = 3*minOf(b,w)+1
    if (mx < maxOf(b,w)) {
        out.append("NO\n")
        return
    }
    gen(minOf(b,w),maxOf(b,w))
    if (b < w) {
        val t = x
        x = y
        y = t
        ...
        for(i in 0..w-1) {
            x[i] = Pair(x[i].first+1,x[i].second)
        }
        for(i in 0..b-1) {
            y[i] = Pair(y[i].first+1,y[i].second)
        }
    }

    out.append("YES\n")
    for (t in x) {
        out.append("${t.first} ${t.second}\n")
    }
    for (t in y) {
        out.append("${t.first} ${t.second}\n")
    }
