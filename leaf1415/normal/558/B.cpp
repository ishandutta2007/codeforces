...
ll n;
ll a[100005];
ll cnt[1000005], l[1000005], r[1000005];
...
int main(void)
{
	...
	rep(i, 1, 1000000) l[i] = inf, r[i] = -inf;
	rep(i, 1, n) cnt[a[i]]++, chmin(l[a[i]], i), chmax(r[a[i]], i);
	
	ll mx = 0;
	rep(i, 1, 1000000) chmax(mx, cnt[i]);
	
	ll ans = -1;
	rep(i, 1, 1000000){
		if(cnt[i] == mx && (ans == -1 ||  r[ans]-l[ans] > r[i]-l[i])) ans = i;
	}
	outl(l[ans], r[ans]);
