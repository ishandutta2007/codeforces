...
int main() {
    ...
    repn(i, N) {
        repn(j, i - 1) {
            if (S[i - 1] == S[j - 1]) { mae[i] = j; }
        }
    }

    vvmint dp(N + 1, vmint(N + 1, 0)), rdp(N + 1, vmint(N + 2, 0));

    dp[0][0] = 1;

    rep(i, N + 1)rep(j, N + 2) {
        if (j > 0)rdp[i][j] = rdp[i][j - 1] + dp[i][j - 1];
        if (i > 0 && j > 0 && j <= N)dp[i][j] += rdp[i - 1][j + 1] - rdp[i - 1][mae[j] + 1];

        //if(j<=N)cout << i << " " << j << " " << dp[i][j].val() << endl;
    }
