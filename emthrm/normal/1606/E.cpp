...
int main() {
  int n, x; cin >> n >> x;
  ModInt::init(n);
  vector dp(n + 1, vector(x + 1, ModInt(0)));
  FOR(rd, 2, n + 1) for (int m = 1; m <= rd - 1 && m <= x; ++m) {
    dp[rd][m] += ModInt::nCk(n, rd) * (ModInt(m).pow(rd) - ModInt(m - 1).pow(rd));
  }
  FOR(rd, 2, n + 1) FOR(m, 1, x + 1) {
    for (int i = 0; i <= n - rd && m + rd + i - 1 <= x; ++i) {
      dp[rd + i][m + rd + i - 1] += dp[rd][m] * ModInt::nCk(n - rd, i) * (ModInt(rd + i - 1).pow(i));
    }
  }
