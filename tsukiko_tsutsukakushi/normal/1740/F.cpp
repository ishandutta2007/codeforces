...
void solve() {
    ...
    vector dp(n + 1, vc<mint>(n + 1, 0));
    vector sdp(n + 1, vc<mint>(n + 2, 0));
    auto calc = [&](int i) -> void {
        rep1(y, 0, n) {
            sdp[y].assign((i == 0 ? n : y / i) + 5, 0);
            rep1(x, 0, ((i == 0 ? n : y / i) + 2)) {
                sdp[y][x + 1] = sdp[y][x] + dp[y][x];
            }
        }
    };
    dp[0][n] = 1;
    ...
    mint ans = 0;
    rep1(i, 1, n) {
        for(int y = 0; y <= sum[i] ; ++ y) {
            dp[y].resize((i == 0 ? n : y / i) + 3);
            for(int x = 0; x * i <= y; ++ x) {
                dp[y][x] = sdp[y - x][min(n + 1, (i > 1 ? (y - x) / (i - 1) + 1 : n + 1))] - (x * (i - 1) <= (y - x) ? sdp[y - x][x] : 0);
            }
        }
        rep1(x, 1, n) {
            if(x * i <= n) {
