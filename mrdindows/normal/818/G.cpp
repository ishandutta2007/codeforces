...
//    FREOUT("out.txt");
    ...
    for (int i = n; i >= 1; --i) {
        add_rib(0, 2 * i - 1, 1, 0);
        add_rib(2 * i - 1, 2 * i, 1, -1);
        add_rib(2 * i, 2 * n + 1, 1, 0);
        
        int cur = 2 * i;
        for (auto& v : {vost[a[i] % 7], vval[a[i] - 1], vval[a[i] + 1]}) {
//            DBN(v);
            for (int i = (int)v.size() - 1; i >= 0 && i >= (int)v.size() - 10; --i) {
//                DBN(cur / 2, v[i]);
                add_rib(cur, 2 * v[i] - 1, 1, 0);
            }
        }
        
        vost[a[i] % 7].push_back(i);
        vval[a[i]].push_back(i);
//        DBN(vost[a[i] % 7], vval[a[i]]);
    }
