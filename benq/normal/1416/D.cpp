...
int main() {
	...
	for(int i = 1; i <= q; i++){
		if(typ[i] == 1){
			...
			nodes1.pb(A); q1.push(A);
			nodes2.pb(B); q2.push(B);
 
			adj[A].erase(B); adj[B].erase(A);
			dsu.rollback();
			assert(dsu.get(a[qval[i]]) != dsu.get(b[qval[i]]));
			...
			B = dsu.get(B);
 			...
 			set<pi> tmp;
			if(sz(q2) == 0){
				for(auto u: nodes2) if (p[u]) {
					assert(vals[V].count(mp(p[u],u)));
					tmp.ins(mp(p[u], u));
