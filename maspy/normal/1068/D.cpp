...
void solve() {
  ...
  dp1[0] = 1;
  for (auto&& a: A) {
    vc<mint> newdp0(LIM + 1), newdp1(LIM + 1);
    auto dp0_c = cumsum(dp0);
    auto dp1_c = cumsum(dp1);

    FOR(y, 1, LIM + 1) {
      if (a != -1 && a != y) continue;
      /*
      FOR(x, LIM + 1) {
        if (x < y) newdp0[y] += dp0[x] + dp1[x];
        if (x == y) newdp1[y] += dp0[x] + dp1[x];
        if (x > y) newdp1[y] += dp1[x];
      }
      */
      newdp0[y] += dp0_c[y] + dp1_c[y];
      newdp1[y] += dp0[y] + dp1[y];
      newdp1[y] += dp1_c.back() - dp1_c[y + 1];
    }
    swap(dp0, newdp0);
    swap(dp1, newdp1);
  }
