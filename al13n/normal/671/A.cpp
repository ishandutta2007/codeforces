...
int main(int argc, char **argv){
  ...
  forn(i,n){
    ld x,y;
    cin>>x>>y;
    ld t=sqrt(sqr(x-T.X)+sqr(y-T.Y));
    res+=t*2;
    va.pb(mp(sqrt(sqr(x-A.X)+sqr(y-A.Y))-t,i));
    vb.pb(mp(sqrt(sqr(x-B.X)+sqr(y-B.Y))-t,i));
  }
  ...
  ld ad=min(va[0].X,vb[0].X);
  if(n>=2){
    if(va[0].Y==vb[0].Y)
      ad=min(ad,min(va[0].X+vb[1].X,va[1].X+vb[0].X));
    else
      ad=min(ad,va[0].X+vb[0].X);
  }
  cout<<res+ad;
