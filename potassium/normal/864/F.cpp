...
int main() {
	...
	vector<vector<vector<pii>>> qry(n + 1, vector<vector<pii>>(n + 1));
	for (int i = 0; i < q; i++) {
		int s,t,k; R(s,t,k);
		qry[t][s].push_back({ k, i });
	}

	vector<vector<bool>> reachable(n + 1, vector<bool>(n + 1, false));
	for (int u = 1; u <= n; u++) {
		function<void(int)> dfs = [&](int cur) {
			if (reachable[u][cur]) return;
			reachable[u][cur] = true;
	...
	vector<int> ans(q, -1);
	for (int t = 1; t <= n; t++) {
		vector<vector<int>> curG(n + 1);
		for (int u = 1; u <= n; u++) {
			if (u == t) continue;
			for (auto& v : g[u])
				if (reachable[v][t]) {
					curG[v].emplace_back(u);
					break;
				}
		}
		...
		function<void(int)> dfs = [&](int cur) {
			...
			for (auto& query : qry[t][cur])
				if ((int)stk.size() >= query.first)
					ans[query.second] = stk[(int)stk.size() - query.first];

			for (auto& v : curG[cur])
				dfs(v);

			assert(!stk.empty() && stk.back() == cur);
			stk.pop_back();
		};
		dfs(t);
	}
