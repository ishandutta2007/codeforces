...
int n, a[MAXN], sec[MAXN], sz;

vector<int> go(int b, int e){
	if (e - b == 1){
		vector<int> ret(sec[b]+1, 1);
		return ret;
	}

	int mid = b + e >> 1;
	vector<int> vl = go(b, mid), vr = go(mid, e);
	return NTT::mod_multiply(vl, vr);
}
