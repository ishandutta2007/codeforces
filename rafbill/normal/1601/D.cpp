...
struct st {
  ...
  void push(int i) {
    if(i < n && A[i].delta) {
      A[2*i].delta += A[i].delta;
      A[2*i].value += A[i].delta;
      A[2*i+1].delta += A[i].delta;
      A[2*i+1].value += A[i].delta;
      A[i].delta = 0;
    }
  }

  void add_range_(int i, int a, int b, int l, int r, li const &v) {
    if(l <= a && b <= r) {
      A[i].value += v;
      A[i].delta += v;
      return;
  ...
  void add_range(int l, int r, li const &v) {
    add_range_(1, 0, n - 1, l, r, v);
  }

  void set_point_(int i, int a, int b, int x, int y) {
    if(a == b) {
      A[i].set(y);
      return;
    }
    push(i);
    int c = (a + b) / 2;
    if(x <= c) set_point_(2*i,a,c,x,y);
    else set_point_(2*i+1,c+1,b,x,y);
    update(i);
  }

  void set_point(int x, int y) {
    set_point_(1,0,n-1,x,y);
  }

  li query_range_(int i, int a, int b, int l, int r) {
    if(l <= a && b <= r) {
      return A[i].value;
    }
    push(i);
    ...
    if(r <= c) {
      ...
      return max(query_range_(2 * i, a, c, l, r), query_range_(2 * i + 1, c + 1, b, l, r));
    }
  }

  li query_range(int x, int y) {
...
int main(){
  ...
  vi XS(n+n+2);
  for(li i = 0; i < (li)(n); ++i) XS[i]=A[i].a;
  for(li i = 0; i < (li)(n); ++i) XS[n+i]=A[i].s;
  XS[n+n] = d;
  XS[n+n+1] = -1;
  pdqsort(begin(XS), end(XS));

  vi YS; YS.reserve(n+n);
  for(li i = 0; i < (li)(n+n+2); ++i) {
    if(i==0 || XS[i]!=XS[i-1]) {
      YS.emplace_back(XS[i]);
    }
  }

  li ny = YS.size();
  hash_map<li,li> unY;
  for(li i = 0; i < (li)(ny); ++i) unY[YS[i]]=i;
  ...
  pdqsort(begin(A), end(A), [&](alp const& a, alp const& b) { return make_tuple(a.a+a.s,a.s) < make_tuple(b.a+b.s,b.s); });

  st S(ny+4);
  S.set_point(d, 0);

  for(auto const& alp : A) {
    li s = alp.s;
    li a = alp.a;
    li y = S.query_range(0,min(s,a-1));
    if(a <= s) S.add_range(a,s,1);
    S.set_point(a,y+1);
  }
  li ans =
    S.A[1].value;
