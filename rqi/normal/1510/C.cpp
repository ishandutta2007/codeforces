...
pair<ll, vi> dp[2];
pair<ll, vi> ndp[2];
pair<ll, vi> DP[mx][2][2]; //second to last thing: forced to choose a special. last thing: whether leftover is used
...
void genDP(int node){
	...
	for(int i1 = 0; i1 < 2; i1++){
		for(int i2 = 0; i2 < 2; i2++){
			//dp[i1] --> DP[node][i2][i1, i1+1]
			if(i1+i2 > sz(children[node])){ //transition impossible
				continue;
			}
...
int main() {
	...
	while(cin >> n >> m){
		...
		// for(int i = 1; i <= n; i++){
		// 	dbg(i, DP[i][0][0], DP[i][1][0], DP[i][0][1], DP[i][1][1]);
		// }
