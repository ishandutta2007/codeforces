    ...
    REP(tc,1,ntc+1)
    {
        ...
        while(!pr.empty())
        {
            ...
            if(dp[j][t]==0)
            {
                ...
                REP(i,0,nv[j^1LL])
                {
                    if(dp[j^1LL][(t+n-v[j^1LL][i])%n]==0)
                    {
                        dp[j^1LL][(t+n-v[j^1LL][i])%n]=1;
                        cur[j^1LL][(t+n-v[j^1LL][i])%n]=nv[j^1LL];
                        REP(i2,0,nv[j])
                        {
                            if(dp[j][(t+n-v[j^1LL][i]+n-v[j][i2])%n]==0)
                            {
                                if(++cur[j][(t+n-v[j^1LL][i]+n-v[j][i2])%n]==nv[j])
                                    pr.push(MP(j,(t+n-v[j^1LL][i]+n-v[j][i2])%n));
                            }
                        }
                    }
                }
