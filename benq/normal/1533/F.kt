...
fun solve() {
    val s = io.read().map{(it-'0').toInt()}.toIntArray()
    val n = s.size
    val pos = Array(2) { c -> (0..n-1).filter{s[it] == c}.toIntArray()}
    val sum = Array(2) { c -> s.runningFold(0) { cur,x -> cur + if (x == c) 1 else 0}.toIntArray() }
    println((1..n).map{i ->
        generateSequence(0) { cur ->
            IntArray(2) { c -> pos[c].getOrElse(sum[c][cur]+i){n} }.maxOrNull()!!
        }.indexOf(n)
    }.joinToString(" "))
//    val anses = ArrayList<Int>()
//
//    for (i in 1..n) {
//        var cur = 0
//        var ans = 0
//        while (cur < n) {
//            val one_ind = cum_ones[cur]+i
//            val zero_ind = cum_zeros[cur]+i
//            ans += 1
//            if (one_ind >= ones.size || zero_ind >= zeros.size) {
//                break
//            }
//            cur = maxOf(ones[one_ind],zeros[zero_ind])
//        }
//        anses.add(ans)
//    }
//    println(anses.joinToString(" "))
