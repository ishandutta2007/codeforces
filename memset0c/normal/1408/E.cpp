...
int find(int x){return anc[x]==x?anc[x]:anc[x]=find(anc[x]);}
...
int main(){
  ...
  for(int len,i=1;i<=n;i++){
    read(len);
    for(int val,j=1;j<=len;j++){
      read(val);
      e.push_back({i,n+val,a[i]+b[val]});
    }
  }
  sort(e.begin(),e.end());
  reverse(e.begin(),e.end());
  for(int i=1;i<=n+m;i++)anc[i]=i;
  long long ans=0;
  for(const auto &it:e){
