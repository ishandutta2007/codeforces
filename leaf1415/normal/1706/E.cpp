...
int main(void)
{
	...
	while(T--){
		...
		rep(i, 1, n-1) ub[i] = m, lb[i] = 0;
		rep(i, 1, 19){
			rep(j, 1, m) qvec[j].clear();
			rep(j, 1, n-1){
				if(ub[j]-lb[j] > 1){
					mid[j] = (lb[j]+ub[j])/2;
					qvec[mid[j]].pb(j);
			...
			rep(j, 1, m){
				uf.unite(u[j], v[j]);
				for(auto q : qvec[j]){
					if(uf.same(q, q+1)) ub[q] = mid[q];
					else lb[q] = mid[q];
				}
			}
		}
		DisjointSparseTable dst(digitnum(n+5, 2));
		rep(i, 1, n-1) dst.set(i, ub[i]);
		dst.calc();
