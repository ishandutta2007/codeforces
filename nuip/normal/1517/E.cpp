...
mint naive(const vector<ll> &vs){
	const int n=vs.size();
	int re=0;
	rep(st,1<<n){
		vector<int> cs,ps;
		rep(i,n)((st>>i&1)?cs:ps).eb(i);
		int ok=1;
		reps(i,1,cs.size()-1)if(cs[i]-cs[i-1]>cs[i+1]-cs[i]) ok=0;
		reps(i,1,ps.size()-1)if(ps[i]-ps[i-1]<ps[i+1]-ps[i]) ok=0;
		ll C=0,P=0;
		for(cauto i:cs) C+=vs[i];
		for(cauto i:ps) P+=vs[i];
		if(C>=P) ok=0;
		if(ok) ++re;
	}
	return re;
}
mint solve(const vector<ll> &vs){
	const int n=vs.size();
		vector<ll> evens(n+1);
		rep(i,n) evens[i+1]=evens[i]+(i%2==0?vs[i]:0);
		vector<ll> odds(n+1);
		rep(i,n) odds[i+1]=odds[i]+(i%2==1?vs[i]:0);
		cauto ub=accumulate(all(vs),-1ll)/2;
		// out(ub,1);
		mint re=0;
		{
			auto sum=accumulate(all(vs),0ll);
			if(sum<=ub) ++re;
			for(cauto &v:vs){
				sum-=v;
				if(sum<=ub) ++re;
		...
		rep(lp,2)rep(rc,2){
			...
			reps(ic,lp,n-1-rc){
				...
				int R=n-1-rc;
				while(L%2!=R%2) --R;
				cauto &sums=(L%2==0?odds:evens);
				int l=L-2,r=R+2;
				while(r-l>2){
					cauto m=(l+r)/2;
					(sum+sums[m]-sums[L]>ub?r:l)=m;
				}
				re+=(r-L)/2;
				// out(lp,rc,L,l,R,(r-L)/2,sum,1);
				// exit(0);
			}
		}
		// out(re,1);
...
int main(){
  ...
  cout<<fixed<<setprecision(0);
	// out(solve({5,5}),1); return 0;
	if(0){
		reps(n,1,10)rep(_,1000){
			vector<ll> vs(n);
