        ...
        for (int s = 0; s <= x; s++)
        {
                ...
                for (int i = 0; i < go.size(); i++)
                {
                    ...
                    for (int a = 0; a <= s - 1; a++)
                        for (int b = 0; b <= s - 1 - a; b++)
                            nw[a + b] = (nw[a + b] + kol[a] * dp[go[i]][0][b]) % MOD;
                    kol = nw;
                }
                dp[p][t][s] = kol[s - 1];
            }
            else if (t == 2)
            {
                ...
                for (int i = 0; i < go.size(); i++)
                {
                    ...
                    for (int a = 0; a <= s; a++)
                        for (int b = 0; b <= s - a; b++)
                            nw[a + b] = (nw[a + b] + kol[a] * (dp[go[i]][0][b] + dp[go[i]][2][b])) % MOD;
                    kol = nw;
                }
                dp[p][t][s] = (kol[s] * (m - k)) % MOD;
            }
            else
            {
                vector<ll> kol(s + 1, 0);
                ...
                for (int i = 0; i < go.size(); i++)
                {
                    ...
                    for (int a = 0; a <= s; a++)
                        for (int b = 0; b <= s - a; b++)
                            nw[a + b] = (nw[a + b] + kol[a] * (dp[go[i]][0][b] + dp[go[i]][2][b] + dp[go[i]][1][b])) % MOD;
                    kol = nw;
                }
                dp[p][t][s] = (kol[s] * (k - 1)) % MOD;
            }
        }
}
