...
bool works(int l, int r, set<int> &s, int x) {
    FOR(i, l, r+1) if (s.find(i^x) == s.end()) return false;
    return true;
}
int calc(int l, int r, set<int>& s) {
    if (l%2 == r%2) {
        int ans = 0;
        FOR(i, l, r+1) ans ^= i;
        for (int i : s) ans ^= i;
    ...
    if (l%2==1 && r%2==0) {
        vi check;
        for (int i : s) if (s.find(i^1) == s.end()) check.pb(i^l);
        for (int x : check) if (works(l, r, s, x)) return x;
    }
    set<int> s2; for (int i : s) s2.insert(i/2);
    return 2*calc(l/2, r/2, s2);
}
