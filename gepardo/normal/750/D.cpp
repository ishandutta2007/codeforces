...
bool _have[500][500] = {};
bool _plvl[500][500][8] = {};
bool _lvl[500][500][8] = {};

inline bool& have(int x, int y) {
	return _have[x + 250][y + 250];
}

inline bool& plvl(int x, int y, int z) {
	return _plvl[x + 250][y + 250][z];
}

inline bool& lvl(int x, int y, int z) {
	return _lvl[x + 250][y + 250][z];
}

int dx[8] = {+1, +1,  0, -1, -1, -1,  0, +1};
int dy[8] = { 0, +1, +1, +1,  0, -1, -1, -1};

inline void move(int x, int y, int t, int dir) {	
	for (int i = 0; i <= t; i++) {
		have(x, y) = true;
		x += dx[dir]; y += dy[dir];
	}
	x -= dx[dir], y -= dy[dir];
	lvl(x, y, (dir+1) % 8) = true;
	lvl(x, y, (dir+7) % 8) = true;
}
