...
pair<int, bool> calc(vb& moves) {
    ...
    F0R (i, sz(moves)) {
        ...
        if (cv > hi) { hi = cv; hloc.clear(); }
        if (cv == hi) hloc.pb(i);

        if (cv < lo) { lo = cv; lloc.clear(); }
        if (cv == lo) lloc.pb(i);
    }

    return { hi - lo + 1, hloc.back() < lloc.front() || lloc.back() < hloc.front() };
}
...
void solve() {
    ...
    trav (c, s) {
        if (c == 'W') vert.pb(1);
        else if (c == 'S') vert.pb(0);
        else if (c == 'A') horiz.pb(0);
        else horiz.pb(1);
    }
    ...
    ll ans = ll(h.f) * v.f;
    if (h.f > 2 && h.s) ckmin(ans, ll(h.f - 1) * v.f);
    if (v.f > 2 && v.s) ckmin(ans, ll(h.f) * (v.f - 1));
    ps(ans);
}
