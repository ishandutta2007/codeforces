...
int main() {
    ...
    cin >> A;
    const lint INF = 1 << 30;
    vector<lint> dpi(N + 1, -INF);
    dpi[0] = 0;
    RangeAddRangeMin<lint> tree(dpi, -INF);

    auto upd = [&](int i, lint val) {
        lint add = val - tree.get(i, i + 1);
        if (add >= 0) tree.add(i, i + 1, add);
    };
    vector<int> last(N + 1, -1);
    REP(i, N) {
        int val = 0;
        int j = last[A[i]];
        if (j >= 0) tree.add(j + 1, j + 2, 1);
        val = tree.get(0, i);
        if (j >= 0) tree.add(j + 1, j + 2, -1);
