...
int main(int argc, char **argv){
  ...
  auto turn=[&]{
              forn(i,n) A[i]=mp(A[i].Y,-A[i].X);
            };
  LL r3=0,r4;
  forn(qq,4){
    pll x1(LINF,0),x2(-LINF,0);
    pll y1=x1,y2=x2;
    forv(i,A){
      x1=min(x1,A[i]);
      x2=max(x2,A[i]);
      y1=min(y1,mp(A[i].Y,A[i].X));
      y2=max(y2,mp(A[i].Y,A[i].X));
    }
    r4=x2.X-x1.X+y2.X-y1.X;
    if(x1==mp(y1.Y,y1.X)) r3=r4;
    forv(i,A) r3=max(r3,x2.X-A[i].X+y2.X-A[i].Y);
    turn();
  }
  cout<<r3*2;
  forn(i,n-3) cout<<' '<<r4*2;
