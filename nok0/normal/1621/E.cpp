#line 1 "/Users/nok0/Documents/Programming/nok0/atcoder/lazysegtree.hpp"
...
#line 10 "/Users/nok0/Documents/Programming/nok0/atcoder/lazysegtree.hpp"
...
void main_() {
	...
	REP(i, m) {
		...
		foa(v, groups[i]) {
			ll sum = sums[i] - v;
			int new_ave = divup(sum, (ll)SZ(groups[i]) - 1);
			int new_ok = lb(a, new_ave);
			if(new_ok == n) {
				idx++;
				continue;
			}
			seg.apply(0, new_ok + 1, -1);
			if(seg.all_prod() >= 0) {
				res_s[idx] = '1';
			}
			seg.apply(0, new_ok + 1, 1);
