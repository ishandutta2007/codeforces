...
public class Main {
    ...
    static class TaskB {
        ...
        public void solve(int testNumber, InputReader in, OutputWriter out) {
            ...
            factorial = IntegerUtils.generateFactorial(n + 1, MiscUtils.MODF);
            ...
            for (int i = graph.firstOutbound(0); i != -1; i = graph.nextOutbound(i)) {
                answer *= go(graph.destination(i), 0);
        ...
        private int calcSize(int vertex, int last) {
            size[vertex] = 1;
            for (int i = graph.firstOutbound(vertex); i != -1; i = graph.nextOutbound(i)) {
                int next = graph.destination(i);
                if (next != last) {
                    size[vertex] += calcSize(next, vertex);
                }
            }
            return size[vertex];
        }
