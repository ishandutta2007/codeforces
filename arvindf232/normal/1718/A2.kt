...
fun main(){
    solve.cases{
        ...
        val last = TreeMap<Int,Int>()
        val DP = IntArray(n+1){plarge}
        DP[0] = 0
        last[0] = 0
        var res = 0
        for(i in 1..n){
            ...
            val earliest = last[res]
            if(earliest != null){
                val extra = i - earliest
                DP[i] = minOf(DP[i], DP[earliest] + (extra - 1))
            }
            last[res] = i
        }
        put(DP.last())
