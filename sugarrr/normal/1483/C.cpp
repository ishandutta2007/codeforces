...
signed main(){fastio
 ...
 //   rep(i,1,n)cout<<P[i]<<" ";cout<<endl;
   ...
   // vl dp(n+1,-inf);dp[0]=0;
    segtree<ll,op,e>dp(n+1);dp.set(0,0);
    
    vl lis;lis.pb(0);
    //vl lisdp;lisdp.pb(-inf);
    segtree<ll,op,e>lisdp(n+1);
    ll lispos=1;//
    rep(i,1,n){
        ll VAL=-inf;
        ll pos=P[i];
        ...
        {
           // rep(j,pos,i-1)chmax(dp[i],dp[j]+b[i]);
            chmax(VAL,b[i]+dp.prod(pos,i));
        }
        while((*lis.rbegin())>h[i]){
            lis.pop_back();
           // lisdp.pop_back();
            lispos--;
        }
        {
           // for(auto x:lisdp)chmax(dp[i],x);
            chmax(VAL,lisdp.prod(0,lispos));
        }
        lis.pb(h[i]);
        //lisdp.pb(dp[i]);
        lisdp.set(lispos,VAL);lispos++;
        dp.set(i,VAL);
    }
