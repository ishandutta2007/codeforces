// time-limit: 1000
// problem-url: https://codeforces.com/contest/1740/problem/C
...
void up(){
	int const numBrick=read<int>();
	std::vector<int> weight(numBrick);
	for(auto& it: weight) {
		std::cin>>it;
	}

	std::sort(begin(weight), end(weight));
	for(int const i: range(1, int(weight.size()))){
		weight[i-1]=weight[i]-weight[i-1];
	} // confusing name but whatever
	weight.pop_back();
	...
	auto const compute=[&]{
		...
		int64_t past {};
		for(auto x: weight){
			result=std::max(result, past+x*2);
			past+=x;
		}
		return result;
	};
	...
	auto result=compute();
	std::reverse(begin(weight), end(weight));

	result=std::max(result, compute());
	std::cout<<result<<'\n';
}