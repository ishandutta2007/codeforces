...
P<int,int> p[55];
int dp[100005],l[55],r[55],m,n,ans[100005],mod=998244353,to[100005];
...
int solve(int t){
	...
	FOR(i,1,n){
		...
		FOR(j,0,tm){
			add(now,dp[j]);
			if(j>r[i]){
				add(now,mod-dp[j-r[i]-1]);
			}
			to[j]=now;
		}
		FOR(j,0,tm)dp[j]=to[j];
	}
	int re=0;
	FOR(i,0,tm)add(re,dp[i]);
	RE re;
}
signed main(){
//	freopen("in.txt","r",stdin);
	...
	for(int i=m;i>=1;i--){
		ans[i]=solve(i);
		for(int j=i+i;j<=m;j+=i)add(ans[i],mod-ans[j]);
	}
	cout<<ans[1];
	io.out();
	RE 0;
