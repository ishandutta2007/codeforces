...
void solve() {
    ...
    modint now = modint(10, mod);
    REP(i, n-1) {
        modint ans(0, mod);
        modint geko = modint(1, mod);
        if (n - (i+2) != 0) {
            geko *= pows<modint>(modint(10, mod), n - (i+2));
        }
        ans += now * modint(18LL, mod) * geko;
        geko /= modint(10, mod);
        ans += now * modint(n - 2 - i, mod) * geko * modint(9, mod) * modint(9, mod);
        cout << ans << " ";
    }
    cout << 10 << endl;
}
