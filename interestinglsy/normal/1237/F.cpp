...
///////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
constexpr int MAXN = 3610;

int n,m,k;
struct RowOrColumnContainer{
	int n;
	bool marked[MAXN];
	int unmarked_cnt;
	int dp[MAXN][MAXN];
	void Dp(){
		unmarked_cnt = 0;
		For(i,n) unmarked_cnt += !marked[i];
		// Print(marked,1,n);
		dp[0][0] = 1;
		For(i,n)
			Forx(j,0,i/2){
				if( !j or i == 1 or marked[i] or marked[i-1] )
					dp[i][j] = dp[i-1][j];
				else
					dp[i][j] = Mymo( dp[i-1][j] + dp[i-2][j-1] );
			}
	}
}row,col;
...
int main(){
	...
	For(i,k){
		iRead4(x1,y1,x2,y2);
		row.marked[x1] = row.marked[x2] = 1;
		col.marked[y1] = col.marked[y2] = 1;
	}
	row.Dp();
	col.Dp();
	...
	Forx(hori,0,m/2)
		Forx(vert,0,n/2){
			ll r = row.dp[n][vert] * C(row.unmarked_cnt-2*vert,hori) % MOD;
			ll c = col.dp[m][hori] * C(col.unmarked_cnt-2*hori,vert) % MOD;
			ll tans = r*c%MOD * (fact[vert]*fact[hori]%MOD) % MOD;
			// P(hori,vert,row.dp[n][vert],C(n-2*vert,hori),col.dp[m][hori],C(m-2*hori,vert),tans);
			Addi( ans , tans );
		}
	cout << ans << endl;
