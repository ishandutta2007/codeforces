...
bool comp1(Point pt1,Point pt2){
	if(quad(pt1)!=quad(pt2)){
		return quad(pt1)<quad(pt2);
	}
	if(cross(pt1,pt2)>0)
		return 1;
	return 0;
...
int main(){
    ...
    rep(j,n){
    	...
    	rep(i,n-1){
    		int sz=vec.size();
    		if(flag && cross(vec[i],vec[sz-1])>0){
    			//cout<<i<<endl;
    			rep(k,i){
    				vec.pb(vec[k]);
    			}
    	...
    	rep(i,2*(n-1)){
    		if(i==0)
    			...
    			sumi[i]=sumi[i-1]+ans[i];
    	}
    	rep(i,n-1){
    		gg+=sumi[ans[i]+i]-sumi[i];
    		gg-=ans[i]*(ans[i]-1)/2;
    	}
    	//cout<<gg<<endl;
    	//return 0;
