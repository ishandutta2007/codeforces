...
int main() {
	...
	vector<int> posN(MAXV, -1);
	vector<int> posM(MAXV, -1);
	...
	const ll g = __gcd(n, m);
	const ll lcm = 1LL * n * m / g;
	...
	for (int i = 0; i < MAXV; i++) {
		if (posN[i] == -1 || posM[i] == -1) continue;
		if (abs(posN[i] - posM[i]) % g != 0) {
			ll v = (posN[i] - posM[i]) % n;
			if (v < 0) v += n;
			ll p = rem[v];
			assert(p == INT_MAX);
			continue;
	...
	while (r > l) {
		...
		ll rem = mid - cyc * lcm;
		assert(rem >= 0);
		ll add = upper_bound(all(pos), rem) - pos.begin();
		if (mid - pre - add < k)
			l = mid + 1;
		else
			r = mid;
