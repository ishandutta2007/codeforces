...
signed main() {
    ...
    string s, t;
    cin >> s >> t;
    vector dp(n + 1, vvint(k + 1, vint(n + 1, LLONG_MIN)));
    dp[0][0][0] = 0;
    rep(i, n) {
        rep(j, k + 1) {
            rep(p, n) {
                chmax(dp[i + 1][j][p], dp[i][j][p]);
                if (s[i] == t[0]) { chmax(dp[i + 1][j][p + 1], dp[i][j][p]); }
                if (j < k) { chmax(dp[i + 1][j + 1][p + 1], dp[i][j][p]); }
                if (s[i] == t[1]) { chmax(dp[i + 1][j][p], dp[i][j][p] + p); }
                if (j < k) { chmax(dp[i + 1][j + 1][p], dp[i][j][p] + p); }
                if (t[0] == t[1]) {
                    if (s[i] == t[0]) { chmax(dp[i + 1][j][p + 1], dp[i][j][p] + p); }
                    if (j < k) { chmax(dp[i + 1][j + 1][p + 1], dp[i][j][p] + p); }
                }
            }
        }
    }
