...
int main() {
    ...
    vpii obs(N); re(obs); trav (e, obs) --e.f, --e.s;
    sort(all(obs));
    obs.pb({H-1, W-1});

    vmn dp(sz(obs));
    F0R (i, sz(dp)) {
        dp[i] = mn::ncr(obs[i].f + obs[i].s, obs[i].f);
        F0R (j, i) {
            int dx = obs[i].f - obs[j].f;
            int dy = obs[i].s - obs[j].s;
            if (dx >= 0 && dy >= 0) dp[i] -= dp[j] * mn::ncr(dx+dy, dx);
        }
    }
    ps(dp.back());
