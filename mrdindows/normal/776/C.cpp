...
int main() {
    ...
    srand(time(0));
    const int K = 50;
    ll bitsPermutation[K];
    for (int i = 0; i < K; ++i) {
        bitsPermutation[i] = 1LL << i;
    }
    random_shuffle(bitsPermutation, bitsPermutation + K);
    auto encode = [bitsPermutation](ll x) {
        ll res = 0;
        for (int i = 0; i < K; ++i) {
            if (checkbit(x, i)) {
                res += bitsPermutation[i];
            }
        }
        return res;
    };
