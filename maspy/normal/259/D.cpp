...
void solve() {
  ...
  FOR(add, MAX) {
    vv(mint, newDP, 7, MAX);
    FOR(cnt, 7) FOR(sum, MAX) {
      FOR(k, 7) {
        if (cnt + k >= 7 || sum + add * k >= MAX) continue;
        mint& to = newDP[cnt + k][sum + add * k];
        to += DP[cnt][sum] * MM.C(C[add], k, true);
      }
    }
    swap(DP, newDP);
  }
  auto dp = DP[6];
  FORIN(x, dp) x *= MM.fact(6);
  mint ANS = 0;
  FOR(a, MAX) FOR(b, a) ANS += dp[b] * C[a];
  print(ANS);
}
