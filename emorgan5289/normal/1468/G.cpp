...
int main() {
    ...
    for (int i = 0; i < n; i++)
        cin >> p[i];
    ld ans = 0;
    point<ll> e = {p[n-1].x, p[n-1].y+h}, t = {0, -inf};
    for (int i = n-2; i >= 0; i--) {
        auto a = p[i], b = p[i+1];
        if (cross(e, b, a) > 0) {}
        else if (cross(e, b, a) == 0) {
            if (cross(e, a, e+t) >= 0)
                ans += dist(a, b);
        } else {
            if (intersects(e, e+t, a, b))
                ans += dist(a, intersection(e, e+t, a, b));
            else if (cross(e, a, e+t) >= 0)
                ans += dist(a, b);
        }
        if (atan2(e-a) < atan2(-t)) t = (a-e)*inf;
    }
