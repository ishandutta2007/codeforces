...
const int MX = 1000 * 1000 + 228;
int t[1000228];

inline int get_t(int l)
{
	int res = 0;
	for (; l > 0; l = ((l + 1) & l) - 1)
	{
...
inline int get_t(int l, int r)
{
	return get_t(r - 1) ^ get_t(l - 1);
}

inline void upd_t(int l, int v)
{
	for (; l < MX; l |= (l + 1))
	{
		t[l] ^= v;
...
inline ANDREIKKAA_RETURN_TYPE mainFunction(ANDREIKKAA_PARAMETERS)
{
	...
	int m;
	rd(m);
	static vector<pair<int, int>> q[MX];
	fori(i, m)
	{
		int l, r;
		rd(l, r);
	...
	forr(i, 1, n + 1)
	{
		...
		for (auto j : q[i])
		{
			ans[j.second] = ps[i] ^ ps[j.first - 1] ^ get_t(j.first, i + 1);
		}
	}
	fori(i, m)
		pr(ans[i]);
