...
//		tr(lcp);
		...
		int[][] rs = new int[2*Q][];
		for(int i = 0;i < Q;i++){
			int L = ni()-1, R = ni()-1, K = ni()-1;
			rs[i] = new int[]{L-1, K, i, -1};
			rs[i+Q] = new int[]{R, K, i, 1};
		}
...
//		print(sa, s);
		...
		long[] ret = new long[Q];
		int[] ft = new int[s.length+2];
		int p = 0;
		for(int[] e : rs){
			while(p < n && p <= e[0]){
				for(int i = offset[p]; i < offset[p+1];i++){
					addFenwick(ft, isa[i], 1);
				}
				p++;
			}
			int cen = isa[offset[e[1]]];
			int len = offset[e[1]+1]-offset[e[1]]-1;
			int sup = st.firstle(cen+1, len-1);
			if(sup == -1)sup = lcp.length;
			int inf = st.lastle(cen, len-1);
//			tr(inf, sup, sumFenwick(ft, sup-1) - sumFenwick(ft, inf-2));
//			tr(restoreFenwick(ft));
			ret[e[2]] += (sumFenwick(ft, sup-1) - sumFenwick(ft, inf-1))*e[3];
		}
		for(int i = 0;i < Q;i++){
			out.println(ret[i]);
		}
