#line 1 "codeforces-workspace/contest/1516/d/d.cpp"
...
#endif
#line 8 "codeforces-workspace/contest/1516/d/d.cpp"
// #include "lib/opt"
...
#line 11 "codeforces-workspace/contest/1516/d/d.cpp"
...
#line 27 "codeforces-workspace/contest/1516/d/d.cpp"

namespace workspace {

least_factor<101010> lpf;
...
void main() {
  ...
  auto dp = make_vector<int>({__bit_width(n) + 1, n + 1});
  {
    auto &f = dp[0];
    vector<vector<int>> divs(1 << 17, {0});
    for (auto [i, v] : enumerate(a)) {
      f[i + 1] = f[i];
      while (v > 1) {
        auto p = lpf(v);
        chgr(f[i + 1], divs[p].back());
        while (v % p == 0) v /= lpf(v);
        divs[p].emplace_back(1 + i);
      }
    }
  }
  for (auto t : range(dp.size() - 1)) {
    auto &nx = dp[t + 1];
    const auto &cu = dp[t];
    for (auto &&[x, y] : zip(cu, nx)) {
      y = cu[x];
    }
  }
