...
struct ysp{
	...
	pi get(int i){
		return pi(cnt,a.sum(0,i)+b.sum(i,s)*i);
	}
};
...
struct HLD{
	...
	void init(){
		...
		vi ss(n);
		rep(i,n)ss[head[i]]++;
		rep(i,n)if(ss[i])mrt[i].init(ss[i]);
	}
	void slv(const int c,const int cost,const vc<query>&qs,vi&ans){
		set<int> cur;
		int sum=0,depsum=0;
		...
		auto calc=[&](int v){
			...
			while(v!=-1){
				int h=head[v];
				int a,b;tie(a,b)=mrt[h].get(dep[v]-dep[h]);
				a-=last;
				b-=last*(dep[v]-dep[h]);
				b+=a*(dep[h]+1);
				res+=b;
		...
		for(auto q:qs){
			if(q.v==-1){
				ans[q.t]=(sum*2+depsum)*sq(cost)-2*cost*depsum*c+sq(c)*n;
			}else{
				auto itr=cur.find(q.v);
				if(itr==cur.ed){
					sum+=calc(q.v);
					add(q.v,1);
					cur.insert(q.v);
