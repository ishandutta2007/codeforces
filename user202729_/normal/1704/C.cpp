// time-limit: 1000
// problem-url: https://codeforces.com/contest/1704/problem/C

// hopefully correct
// completely unproven but surely seems correct
#if not LOCAL
#define NDEBUG 1
#endif
#include<bits/stdc++.h>
...
void up(){
	...
	std::vector<int> gaps; gaps.reserve(m);
	for(int const i: range(1, int(a.size()))){
		gaps.push_back(a[i]-a[i-1]-1);
	}
	gaps.push_back(a[0]+n-a.back()-1);

	std::sort(begin(gaps), end(gaps), std::greater<int>());

	int magic=1;
	int good {}; // sum does not exceed n anyway
	for(auto x: gaps){
		if(x-magic==0){
			good++;
			break;
