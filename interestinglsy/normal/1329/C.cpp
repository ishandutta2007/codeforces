...
int n,m,k;
int a[MAXN];
#define BigSon(x) (a[(x)<<1]>a[(x)<<1|1]?(x)<<1:(x)<<1|1)

il int GetHeight( int pos ){
	return a[pos] ? GetHeight(BigSon(pos))+1 : 0;
}

il void Delete( int pos ){
	if(!a[pos]) return;
	int v = BigSon(pos);
	a[pos] = a[v];
	Delete(v);
...
vc<int> ansl;
ll maxDec;
il void Solve( int pos , int dep ){
	if(!a[pos]) return;
	while( GetHeight(pos) + dep > m ){
		maxDec += a[pos];
		ansl.Eb(pos);
		Delete(pos);
	}
...
int main(){
	...
	For(TwT,T){
		...
		ll ans = sum - maxDec;
		printf("%lld\n",ans);
		for( auto x : ansl ) printf("%d ",x);
		putchar(10);

		ansl.clear();
		maxDec = 0;
		memset(a,0,sizeof a[0]*(k+k+1));
	}
