...
int main(void)
{
	...
	ll S = n+1, T = n+2;
	rep(i, 1, n){
		if(a[i] % 2) dc.add_edge(S, i, 2);
		else dc.add_edge(i, T, 2);
	}
	rep(i, 1, n) rep(j, 1, n){
		if(a[i] % 2 && a[j] % 2 == 0 && prime[a[i]+a[j]] == a[i]+a[j]) dc.add_edge(i, j, 1);
	}
	if(dc.calc(S, T) < n){
		outl("Impossible");
		return 0;
	...
	outl(sz(ans));
	reps(i, ans) outl(sz(ans[i]), ans[i]);
