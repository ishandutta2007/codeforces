...
int n,m,i,j,k,x,y,z,nx,ny,lx,ly,it,fi,fr,r,res,inf,a[82][82],bx[82][82],by[82][82],u[82][82],p[82][82],frx[82][82],fry[82][82];
...
int main() {
  ...
  for (i=0; i<n; i++) for (j=0; j<m; j++) if (((i+j)&1)==0 && bx[i][j]==-1) {
    ...
    for (int kk=0; kk<(n*m)/2; kk++) {
	  nq.clear(); ++it;
	  for (fi=0; fi<q.size(); fi++) {
	    int i=q[fi].first,j=q[fi].second;
        for (k=0; k<4; k++) {
          x=i+dx[k];
	      y=j+dy[k];
	      if (x>=0 && y>=0 && x<n && y<m) {
