...
mint calc(string s)
{
	...
	rep(i, 0, 2000) rep(j, 0, m-1) rep(k, 0, 1) rep(l, 0, 1) dp[i][j][k][l] = 0;
			...
			rep(k, 0, 1){
				rep(l, 0, 1){
					rep(x, 0, 9){
						...
						if(k == 0 && x > c) continue;
						if(x < c) nk = 1;
						dp[i-1][(j*10+x)%m][nk][l^1] += dp[i][j][k][l];
					}
				}
			}
		}
	...
	mint ret = 0;
	rep(k, 0, 1) rep(l, 0, 1) ret += dp[0][0][k][l];
	//outl(ret);
	return ret;
}
...
int main(void)
{
	...
	mint ans = calc(t) - calc(s);
	
	bool flag = true; ll x = 0;
	reps(i, s){
		ll c = s[i] - '0';
		if(i % 2 == 0 && c == d) flag = false;
		if(i % 2 && c != d) flag = false;
		x *= 10, x += c, x %= m;
	}
	if(flag && x == 0) ans += 1;
