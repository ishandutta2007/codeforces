#include <iostream>
#include <vector>
#include <set>
#include <map>
#include <string>
#include <algorithm>
#include <cmath>
//@formatter:off
namespace {
    using namespace std;
    typedef long long ll;
    template<typename T>istream &operator>>(istream &is, vector<T> &v) { for (T &t : v) is >> t; return is; }
    template<typename T>bool space(vector<T>) { return false; }
    template<typename T>bool space(T) { return true; }
    bool space(char) { return false; }
    template<typename T>ostream &operator<<(ostream &os, const vector<T> &v) { for (const T &t : v) { os << t << (space(t) ? " " : ""); } os << "\n"; return os; }
    ll gcd(ll a, ll b) { while (b) { ll mod = a % b; a = b; b = mod; } return a; }
    template<typename T>void incr_sort(vector<T> &v) { stable_sort(v.begin(), v.end()); }
    template<typename T>void decr_sort(vector<T> &v) { stable_sort(v.rbegin(), v.rend()); }
    ll sum(const vector<int> &v) { ll s = 0; for (ll value : v) s += value; return s; }
}
//@formatter:on

void solve();

int main() {
    cin.tie(nullptr);
    cout.tie(nullptr);
    ios::sync_with_stdio(false);
    int t = 1;
//    cin >> t;
    for (int i = 0; i < t; ++i) solve();
    return 0;
}

void solve() {
    int n;
    cin >> n;
    vector<int> a(n);
    cin >> a;
    vector<bool> positive(n);
    for (int i = 0; i < n; ++i) positive[i] = a[i] > 0;
    bool plus = positive[0];
    vector<int> border(0);
    border.reserve(n);
    for (int i = 1; i < n; ++i) if (positive[i] ^ positive[i - 1]) border.push_back(i);
    vector<bool> product(n);
    product[0] = positive[0];
    for (int i = 1; i < n; ++i) product[i] = product[i - 1] == positive[i];
    vector<bool> antiproduct(n);
    for (int i = 0; i < n; ++i)antiproduct[i] = true ^ product[i];
    vector<int> p_prefix(n);
    p_prefix[0] = product[0];
    for (int i = 1; i < n; ++i) p_prefix[i] = p_prefix[i - 1] + product[i];
    vector<int> ap_prefix(n);
    ap_prefix[0] = antiproduct[0];
    for (int i = 1; i < n; ++i) ap_prefix[i] = ap_prefix[i - 1] + antiproduct[i];
    ll sum = 0;
    plus = true;
    for (int i = 0; i < n; ++i) {

        sum += (plus ? p_prefix[n - 1] - (i > 0 ? p_prefix[i - 1] : 0) :
                      ap_prefix[n - 1] - (i > 0 ? ap_prefix[i - 1] : 0));

        if (!positive[i])plus = !plus;
    }
    cout << (long long) (n) * (n + 1) / 2 - sum << " " << sum << "\n";
}