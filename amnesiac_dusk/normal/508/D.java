...
public class Main
{
	public static void main(String[] args) throws Exception
	{
		...
		{
			te=ns();
			gra[te.charAt(0)][te.charAt(1)].adj.add(te.charAt(2));
			gra[te.charAt(0)][te.charAt(1)].oud++;
			gra[te.charAt(1)][te.charAt(2)].ind++;
		}
		int out=0,in=0;
		for(int i=0; i<256; i++)
			for(int j=0; j<256; j++)
			{
				if(gra[i][j].ind==gra[i][j].oud)
					;
				else if(gra[i][j].ind==1+gra[i][j].oud)
					in++;
				else if(gra[i][j].ind+1==gra[i][j].oud)
					out++;
				else {sop("NO");	exit(); }
