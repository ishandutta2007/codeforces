...
struct st {
	int n;
	vi sm;
	st(const int n) : n(n), sm(4 * n) {
	}
	void set(const int l, const int r, vi& u) {
		set(0, 0, n, l, r, u);
	}
	void set(const int v, const int v0, const int v1, const int s0, const int s1, vi& u) {
		if ((v1 <= s0) || (s1 <= v0))
			return;
		if (sm[v] == v1 - v0)
			return;
		if (v1 == v0 + 1) {
			...
			set(v * 2 + 1, v0, (v0 + v1) / 2, s0, s1, u);
			set(v * 2 + 2, (v0 + v1) / 2, v1, s0, s1, u);
			sm[v] = sm[2 * v + 1] + sm[2 * v + 2];
		}
	}
};

int fa(const vi& a, const int p, const bool up, int withess = -1) {
	if (p == sz(a) - 1)
		return 0;
	int xm = p;
	for (int i = max(withess, p); i <= a.size(); ++i) {
		if (i == a.size())
			return 1 + fa(a, xm, !up, a.size());
	}
}
