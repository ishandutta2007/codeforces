...
public class TaskB {
	...
	void solve() {
		...
		int[] permutation = in.readIntArray(coinCount);
		int left;
		for (left = 0; left < coinCount; left++) {
			if (permutation[left] != left + 1)
				break;
		}
		int right;
		for (right = coinCount - 1; right >= 0; right--) {
			if (permutation[right] != right + 1)
				break;
		}
		if (left > right) {
			out.println("0 0");
