...
int main(){
  ...
  vector<modint>f(n+1);
  {
    f[0]=1;
    rep(i,1,n+1){
      f[i]=f[i-1]*(f[i-1]+1)/2+1;
    }
  }
  vector<modint>dp(n+1),df(n+1);
  dp[0]=0,df[0]=1;
  //dp[i]:=i2way
  //f[i]:i
  //df[i]:=i,3way
  rep(i,1,n+1){
    if(i!=n){
      ...
      //cout<<f[n] spa f[n-1] spa g(f[n]) spa g(f[n-1])<<endl;
      df[i]=g(f[n])-g(f[n-1]);
      //dp[i]=g(f[n])-g(f[n-1])-(f[n]-f[n-1]);
    }
  }
  //debug(f,n+1);
  //debug(dp,n+1);
  ...
  rep(i,1,n-1){
    ret+=dp[i]*(df[n-i-1]-1);
    //cout<<i spa dp[i] spa f[n-i-1]<<endl;
  }
  cout<<ret<<endl;
  return 0;
}