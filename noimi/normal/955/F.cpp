...
signed main(){
    ...
    auto dp = vv2(n,20,-1);
    {
        auto dfs = [&](auto &&f,int x,int p)->void{
            ...
            rep2(i,1,18){
                vi tmp;
                for(auto e:g[x]) if(dp[e][i] != -1) tmp.eb(dp[e][i]);
                sort(all(tmp),greater<int>());
                rep3(j,(int)tmp.size()-1,0){
                    if(tmp[j] >= j + 1) {
                        dp[x][i+1] = j + 1;
                        break;
                    }
    ...
    }
    {
        auto dfs = [&](auto &&f,int x)->void{
            for(auto e:g[x]) f(f,e);
            rep3(i,18,1) chmax(dp[x][i],dp[x][i+1]);
            rep2(i,1,19) for(auto e:g[x]) chmax(dp[x][i],dp[e][i]);
        };
        dfs(dfs,0);
    }
    ll ans = 0;
