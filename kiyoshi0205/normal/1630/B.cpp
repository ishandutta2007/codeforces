...
int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep(i,N){
    //[i+1,?]
    if((cnt[N]-cnt[i])*2<N+K)break;
    while((cnt[j]-cnt[i])*2<N+K)++j;
    debug(i,j);
    if(chmin(ans,j-i-1))ansx=i+1;
  }
  print(ansx<<" "<<ans+ansx);
  vec ch(N);
  rep(i,N)ch[i]=(ansx<=a[i]&&a[i]<=ans+ansx);
  j=0;
  pvec anslis;
  ll bf=1;
  rep(i,N){
    ...
    if(j>0){
      anslis.emplace_back(bf,i+1);
      bf=i+2;
      j=0;
    }
  }
  assert(ll(anslis.size())>=K);
  anslis.resize(K);
  anslis.back().second=N;
  for(auto [L,R]:anslis)print(L<<" "<<R);
}
}