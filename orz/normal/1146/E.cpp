...
struct tr
{
	...
	vb s1;
	int N;
	void build(const int& n, const bool& b)
	{
		v.resize(n * 4 + 4);
		s0.resize(n * 4 + 4);
		s1.resize(n * 4 + 4);
	...
	void ch(int u, int ul, int ur, int sl, int sr)
	{
		if ((ul > sr) || (ur < sl))
			return;
		if ((ul >= sl) && (ur <= sr))
		...
		push(u);
		int um = (ul + ur) / 2;
		ch(2 * u + 1, ul, um, sl, sr);
		ch(2 * u + 2, um + 1, ur, sl, sr);
	}
	void set_val(const int& a, const int& val)
	{
		sv(0, 1, N, a, N, val);
	}
	void sv(int u, int ul, int ur, int sl, int sr, int val)
	{
		if ((ul > sr) || (ur < sl))
			return;
		if ((ul >= sl) && (ur <= sr))
		...
		push(u);
		int um = (ul + ur) / 2;
		sv(2 * u + 1, ul, um, sl, sr, val);
		sv(2 * u + 2, um + 1, ur, sl, sr, val);
	}
	int get_val(const int& a)
	{
		return gv(0, 1, N, a);
	}
	int gv(int u, int ul, int ur, int s)
	{
		if (ul == ur)
			return (s1[u] || v[u]);
		push(u);
		int um = (ul + ur) / 2;
		if (s <= um)
			return gv(2 * u + 1, ul, um, s);
		else
			return gv(2 * u + 2, um + 1, ur, s);
	}
};
...
int main()
{
	...
	for (int i = 0; i < q; ++i)
	{
		...
		if (x < 0)
		{
			...
			{
				tr1.set_val(x + 1, false);
				tr2.set_val(x + 1, false);
			}
		}
		else
		{
			if (s == '>')
			{
				tr1.set_val(x + 1, true);
				tr2.set_val(x + 1, true);
			}
			else
			{
				if (x >= 1)
	...
	{
		if (v[i] < 0)
			cout << (tr1.get_val(-v[i]) ? v[i] : -v[i]) << ' ';
		else if (v[i] > 0)
			cout << (tr2.get_val(v[i]) ? -v[i] : v[i]) << ' ';
		else
			cout << "0 ";
	}
	cout << '\n';
