...
//    return s[x][y] == 'X' && s[x][y + 1] == 'X' && s[x][y + 2] == 'X';
//}
//bool checkV(int x, int y) {
//    if (x < 0 || x + 2 >= n) return false;
//    if (y < 0 || y >= n) return false;
//    return s[x][y] == 'X' && s[x + 1][y] == 'X' && s[x + 2][y] == 'X';
//}
...
//            vector<pair<int, pii>> toErase;
//            for (int dx = -2; dx <= 2; ++dx) {
//                for (int dy = -2; dy <= 2; ++dy) {
//                    toErase.push_back({compute(x + dx, y + dy), {x + dx, y + dy}});
//                }
//            }
//            s[x][y] = 'O';
//            for (auto& p : toErase) {
//                q.erase(p);
//                p.first = compute(p.second.first, p.second.second);
//                if (p.first != 0) {
//                    q.insert(p);
//                }
//            }
...
int solve() {
    ...
    while (tc--) {
        ...
        FI(n) {
            if (!v[i].empty()) {
                b[i].first = v[i].back() - v[i].front() + 1;
            }
        }
        set<int> s;
        s.insert(-1);
        ...
        FI(n) {
            int res = 0;
            for (int x : v[i]) {
                auto nex = s.upper_bound(x);
                auto pre = std::prev(nex);
