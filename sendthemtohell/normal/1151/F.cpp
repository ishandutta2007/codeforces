...
void mul_mat(ll a[101][101], ll b[101][101], int n) {
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			tmp[i][j] = 0;
			for (int k = 0; k < n; ++k) {
...
inline void solve() {
	...
	for (int i = 0; i <= cnt; ++i) {
		...
		ll kek = 0;
		if (i > 0) {
			mat[i][i - 1] = i * (n - cnt - (cnt - i)) % MOD * den % MOD;
			kek += mat[i][i - 1];
		}
		if (i < cnt) {
			mat[i][i + 1] = (cnt - i) * (cnt - i) % MOD * den % MOD;
			kek += mat[i][i + 1];
		}
