...
void solve() {
    ...
    REP(i, 3) {
        ...
        sort(ALL(inputs[i]));
        inputs[i].erase(unique(ALL(inputs[i])), inputs[i].end());
    }
    auto calc = [](ll a, ll b, ll c) {
        return (a - b) * (a - b) + (b - c) * (b - c) + (c - a) * (c - a);
    };
    int ans = calc(inputs[0][0], inputs[1][0], inputs[2][0]);
    vector<int> gekos;
    REP(i,3) {
        gekos.push_back(i);
    }
    do {
        int itr[3] = {};
        REP(i, inputs[gekos[0]].size()) {
            while (itr[1] + 1 < inputs[gekos[1]].size() && inputs[gekos[0]][i] > inputs[gekos[1]][itr[1]]) {
                itr[1]++;
            }
            while (itr[2] + 1< inputs[gekos[2]].size() && inputs[gekos[1]][itr[1]] > inputs[gekos[2]][itr[2]]) {
                itr[2]++;
            }
            while (itr[1] + 1 < inputs[gekos[1]].size()) {
                int now_val = calc(inputs[gekos[0]][i], inputs[gekos[1]][itr[1]], inputs[gekos[2]][itr[2]]);
               now_val -= calc(inputs[gekos[0]][i], inputs[gekos[1]][itr[1] + 1], inputs[gekos[2]][itr[2]]);
               if (now_val < 0) break;
               itr[1]++;
            }
            ans = min(ans, calc(inputs[gekos[0]][i], inputs[gekos[1]][itr[1]], inputs[gekos[2]][itr[2]]));
        }
    } while (next_permutation(ALL(gekos)));
    cout << ans << endl;
}
#undef int
int main() {
