// time-limit: 2000
// problem-url: https://codeforces.com/contest/1753/problem/C
...
#define TEST_MODULAR 0

#if TEST_MODULAR
using modular=double;
#else
using modular=Modular<998244353>; // somehow I accidentally typed 1000000007 here although will need to figure out why TEST_MODULAR doesn't work later...
#endif

auto const inverseSquaredPrefixSumInclusive=[]{
#if TEST_MODULAR
	std::vector<double> result(int(2e5)+1);
	for(int const i: range(1, int(result.size()))){
		result[i]=double(1)/i;
	}
#else
	auto result=inverseF<modular>(int(2e5)+1);
#endif
	for(auto& x: result) x*=x;
	assert(int(result[0])==0);
	std::partial_sum(result.begin()+1, result.end(), result.begin()+1);
	return result;
}();
...
void up(){
	...
	auto const num1=std::accumulate(begin(a), end(a), 0);
	auto const num0=len-num1;
	auto const delta=std::accumulate(begin(a), begin(a)+num0, 0);
	std::cout<<
		int(
				modular((n*int64_t(n-1))>>1) * inverseSquaredPrefixSumInclusive[delta]
		   )<<'\n';
}