...
#line 8 "SPJ-Library/graph/graph.hpp"
...
int value_max = 212345;
vector<int> prime_divisor;

mint solve(int n) {
	vector<int> pcount(n + 3, 0), count_min(n + 3, 0), count_max(n + 3, 0);
	auto update = [&](int prime, int add_number) {
		pcount[prime] += add_number;
		chmin(count_min[prime], pcount[prime]);
		chmax(count_max[prime], pcount[prime]);
		return;
	};
	V<pair<int, int>> ratio;
	graph<int> g(n);
	rep(n - 1) {
		int a, b;
		cin >> a >> b;
		...
		ratio.emplace_back(x, y);
		g.add_edge(a, b, 1, false, 0);
	}
	mint ans;
	auto dfs = [&](auto self, int now, int par, mint value = 1) -> void {
		foa(e, g[now]) {
			...
			if(now < e.to)
				...
				tie(y, x) = ratio[e.id];
			mint nxtvalue = value * y / x;
			const int pre_x = x;
			const int pre_y = y;
			while(x > 1) {
				int d = prime_divisor[x];
				x /= d;
				update(d, -1);
			}
			while(y > 1) {
				int d = prime_divisor[y];
				y /= d;
				update(d, 1);
			}
			self(self, e.to, now, nxtvalue);
			x = pre_x;
			y = pre_y;
			while(x > 1) {
				int d = prime_divisor[x];
		...
		ans += value;
		debug(now, value, pcount);
		return;
	};
	dfs(dfs, 0, -1, mint(1));
	rep(i, 2, n + 1) {
		if(count_min[i]) debug(i, count_min[i]);
		ans *= mint(i).pow(-count_min[i]);
	}
	return ans;
}
...
int main() {
	...
	prime_divisor.resize(value_max);
	iota(all(prime_divisor), 0);
	rep(i, 2, value_max) {
		if(prime_divisor[i] == i) {
			rep(j, i + i, value_max, i) { chmin(prime_divisor[j], i); }
		}
	}
