...
*/
    ...
    fun solve(tc: Int) {
        ...
        for (i in 0..n) b[i] = `in`!!.nint()
        rsort(a)
        rsort(b)
        val pmax = IntArray(n + 1)
        val smax = IntArray(n + 1)
        var rr = -2050000000
        for (i in 0 until n) {
            rr = Math.max(rr, b[i] - a[i])
            pmax[i] = rr
        }
        rr = -2050000000
        for (i in n downTo 1) {
            rr = Math.max(rr, b[i] - a[i - 1])
            smax[i] = rr
        }
        val q = `in`!!.nint()
        for (i in 0 until q) {
            ...
            // out.println(x + " " + l + " " + ans);
            if (l > 0) ans = Math.max(ans, pmax[l - 1])
            if (l < n) ans = Math.max(ans, smax[l + 1])
            out!!.print("$ans ")
        }
        out!!.println()
    }
