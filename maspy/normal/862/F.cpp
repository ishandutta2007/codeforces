...
#line 1 "/home/maspy/compro/library/ds/cartesian_tree.hpp"
template <typename T>
...
struct Mono {
  // size, l, r, full
  using value_type = tuple<int, int, int, int>;
  using X = value_type;
  static X op(X x, X y) {
    auto [xs, xl, xr, xf] = x;
    auto [ys, yl, yr, yf] = y;
...
void solve() {
  LL(N, Q);
  VEC(string, S, N);
  auto calc_lcp = [&](ll i) -> ll {
    auto& A = S[i];
    auto& B = S[i + 1];
    ll n = min(len(A), len(B));
    ll res = 0;
    FOR(i, n) {
      if (A[i] == B[i])
        ++res;
  ...
  vi LEN(N);
  vi LCP(N - 1);
  FOR(i, N) LEN[i] = len(S[i]);
  FOR(i, N - 1) LCP[i] = calc_lcp(i);
  SegTree<Monoid_Max<ll, INF>> seg_len(LEN);
  vc<SegTree<Mono>> seg;
  using E = Mono::X;
  FOR(h, THRESH) {
    vc<E> seg_raw(N - 1);
    FOR(i, N - 1) {
      seg_raw[i] = (LCP[i] >= h ? mt(1, 1, 1, 1) : mt(1, 0, 0, 0));
    }
    seg.eb(SegTree<Mono>(seg_raw));
  }

  FOR(i, N - 1) if (LCP[i] >= THRESH) ss.insert(i);

  auto upd_at = [&](ll i) -> void {
    if (LCP[i] >= THRESH)
      ss.insert(i);
    else
      ss.erase(i);
    FOR(h, THRESH) {
      E e = (LCP[i] >= h ? mt(1, 1, 1, 1) : mt(1, 0, 0, 0));
      seg[h].set(i, e);
    }
  };

  auto calc_high = [&](ll L, ll R) -> ll {
    --R;
    vi I;
    ll n = L;
    while (n < R) {
  ...
  FOR_(Q) {
    ...
    if (t == 1) {
      ...
      --L;
      ll ANS = 0;
      chmax(ANS, seg_len.prod(L, R));
      FOR(h, THRESH) {
        auto [s, l, r, f] = seg[h].prod(L, R - 1);
        if (f == 0) break;
        chmax(ANS, h * (f + 1));
      }
      chmax(ANS, calc_high(L, R));
      print(ANS);
    }
    elif (t == 2) {
      LL(i);
