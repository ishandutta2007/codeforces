...
void lazy(int o, int l, int r)
{
    int mid = MID(l, r);
    for(int i = 0; i < 2; i++)
    {
        if(ch[o][i] == 0) ch[o][i] = newnode();
        int c = ch[o][i];
        flag[c] += flag[o];
        sum[c] += LL(flag[o]) * (i == 0 ? mid - l + 1 : r - mid);
    }
    flag[o] = 0;
}

void update(int &o, int L, int R, int v, int l, int r, int id)
{
    if(o == 0) o = newnode();
    ...
    if(flag[o]) lazy(o, l, r);
    int mid = MID(l, r);
    if(L <= mid) update(ch[o][0], L, R, v, lson);
    if(mid < R) update(ch[o][1], L, R, v, rson);
    mer(o);
}

int find_k(int &o, int k, int l, int r, int id)
{
    if(l == r) return l;
    if(flag[o]) lazy(o, l, r);
    int mid = MID(l, r);
    if(sum[ch[o][0]] >= k) return find_k(ch[o][0], k, lson);
    return find_k(ch[o][1], k - sum[ch[o][0]], rson);
}

int find_lp(int &o, int l, int r, int id)
{
    if(l == r) return l;
    if(flag[o]) lazy(o, l, r);
    int mid = MID(l, r);
    if(sum[ch[o][0]]) return find_lp(ch[o][0], lson);
    return find_lp(ch[o][1], rson);
}
...
void work()
{
    ...
    int rt = 0;
    tot = 0;
    update(rt, 0, 0, 1, 0, 1e9, 1);

    int mn = INF;
    for(int i = 0; i < n; i++)
    {
        int p = find_lp(rt, 0, 1e9, 1);
        update(rt, p, p, -1, 0, 1e9, 1);

//        cout <<p <<endl;

        int len = (a[i] - 1) / 2;
        update(rt, p + 2, p + 2 + len - 1, 2, 0, 1e9, 1);

//        cout <<p + 2 <<" " <<p + 2 + len - 1 <<endl;

        if((a[i] - 1) % 2 == 1)
            update(rt, p + 2 + len, p + 2 + len, 1, 0, 1e9, 1);

        if(sum[rt] >= k)
            mn = min(mn, find_k(rt, k, 0, 1e9, 1));
    }
    if(mn >= INF) mn = -1;
    cout <<mn <<endl;
}
