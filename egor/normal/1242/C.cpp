...
class TaskC {
    ...
    map<longint, int> number;
    vector<vi> a;

public:
    vector<pii> go(int mask) {
        if (ans.count(mask) != 0) {
            return ans[mask];
        }
        if (mask == 0) {
            return ans[mask] = vector<pii>(a.size());
        }
        int first = -1;
        for (int i = 0; ; i++) {
            if ((mask >> i & 1) == 1) {
        ...
        if (sums[first] == med) {
            auto res = go(mask - (1 << first));
            if (res.empty()) {
                return ans[mask] = res;
            }
            res[first] = make_pair(a[first][0], first + 1);
            return ans[mask] = res;
        }
        for (int i : a[first]) {
            vector<pair<int, longint> > cycle;
            int used = 0;
            longint current = sums[first] - i;
            bool good = true;
            while (true) {
                longint need = med - current;
                if (number.count(need) == 0 || (mask >> number[need] & 1) == 0 || (used >> number[need] & 1) == 1) {
                    good = false;
                    break;
                }
                int at = number[need];
                cycle.emplace_back(at, need);
                used += 1 << at;
                current = sums[at] - need;
                if (at == first) {
                    if (need != i) {
                        good = false;
                    }
            ...
            if (good) {
                auto res = go(mask - used);
                if (res.empty()) {
                    continue;
                }
                int last = first;
                for (const auto& p : cycle) {
                    res[p.first] = make_pair(int(p.second), last + 1);
                    last = p.first;
                }
                return ans[mask] = res;
            }
        }
        return ans[mask] = vector<pii>();
    }
    ...
    void solve(std::istream& inp, std::ostream& outp) {
        ...
        int k = in.readInt();
        for (int i = 0; i < k; ++i) {
            a.push_back(in.readIntArray(in.readInt()));
        }

        longint sum = 0;
        sums = vector<longint>(k);
        for (int i = 0; i < k; i++) {
            sum += sums[i] = accumulate(all(a[i]), longint(0));
            for (int j : a[i]) {
                number[j] = i;
            }
        }
        ...
        med = sum / k;
        vector<pii> res = go((1 << k) - 1);
        if (res.empty()) {
            out.printLine("No");
        } else {
            out.printLine("Yes");
            for (int i = 0; i < k; ++i) {
                out.printLine(res[i].first, res[i].second);
            }
        }
	}
};
