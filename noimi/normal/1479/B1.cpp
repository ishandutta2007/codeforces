...
template <class T = int> auto _max = [](T x, T y) { return max(x, y); };
template <class T = int> auto _min = [](T x, T y) { return max(x, y); };
template <class T = int> auto _set = [](T x, T y) { return y == inf<int> ? x : y; };

int main() {
    INT(n);
    VEC(int, a, n);
    int ans = 0;
    auto seg = get_lazy_segment_tree(n + 1, _max<int>, plus<int>(), plus<int>(), -inf<int>, 0);
    int pre = a[0];
    seg.set(0, 1);
    rep2(i, 1, n) seg.set(i, -inf<int>);
    seg.build();
    rep2(i, 1, n - 1) {
        if(a[i] == pre) {
            int ma = max({seg.query(0, a[i]) + 1, seg.query(a[i] + 1, n + 1) + 1, seg[a[i]]});
            int now = seg[pre];
            if(now < ma) { seg.update(pre, pre + 1, ma - now); }
        } else {
            int ma = max({seg.query(0, a[i]) + 1, seg.query(a[i] + 1, n + 1) + 1, seg[a[i]]});
            // cout << " " << a[i] << " " << ma << endl;
            seg.update(0, n + 1, 1);
            if(seg[pre] < ma) seg.update(pre, pre + 1, ma - seg[pre]);
            pre = a[i];
        }
        // rep2(i, 0, 3) cout << seg[i] << " ";
        // cout << endl;
    }
