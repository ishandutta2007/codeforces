...
int main(){
	...
	int const start=read<int>()-1;
	
	Graph<> g; g.readDirected(numNode, numEdge);

	std::vector<int> par(numNode, -1); // -1: not visited
	std::vector<int> branchOf(numNode, -1); // -1: not visited
	branchOf[start]=INT_MAX; // for simplicity

	auto const parPath=[&](int node)->std::vector<int>{
		std::vector<int> result{node};
		while(par[result.back()]>=0) result.push_back(par[result.back()]);
		std::reverse(begin(result), end(result));
		return result;
	};


	for(int const branch: range(0, int(g[start].size()))){
		auto const foundSolution=[&](int node, int other){
			auto a=parPath(other), b=parPath(node);
			b.push_back(other);
			std::cout<<"Possible\n";

			for(auto& v: {a, b}){
				std::cout<<v.size()<<'\n';
				for(auto item: v) std::cout<<item+1<<' ';
				std::cout<<'\n';
			}
		...
		auto const dfs=makeYComb([&](auto dfs, int node)->void{
			assert(branchOf[node]==-1);
			branchOf[node]=branch;
			for(auto [other, _]: g[node]){
				if(branchOf[other]==-1){
					// not visited
					par[other]=node;
					dfs(other);
				}else if(branchOf[other]<branch){
					foundSolution(node, other);
					
				}
			}
		});
		auto const other=g[start][branch].node;
		if(branchOf[other]!=-1){
			foundSolution(start, other);
		}
		par[other]=start;
		dfs(other);
	}
