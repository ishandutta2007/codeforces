...
int main() {
    ...
    while (t--) {
        ...
        reverse(all(v));
        ans[0] = a, ans[n - 1] = b;
        for (ll i : rep(n / 2 - 1)) ans[i + 1] = v[i];
        for (ll i : rep(n / 2, n - 1)) ans[i] = v[i - 1];
        ll l = LLONG_MAX, r = LLONG_MIN;
        for (ll i : rep(n / 2)) chmin(l, ans[i]);
        for (ll i : rep(n / 2)) chmax(r, ans[i + n / 2]);
        if (l != a || r != b) {
            cout << -1 << endl;
        } else {
