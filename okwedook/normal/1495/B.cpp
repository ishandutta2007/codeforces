...
void solve() {
    ...
    vector<int> a(n);
    read(a);
    vector<int> downleft = left(a);
    reverse(a);
    vector<int> downright = left(a);
    reverse(downright);
    reverse(a);
    for (auto &i : a) i = -i;
    vector<int> upleft = left(a);
    reverse(a);
    vector<int> upright = left(a);
    reverse(upright);
    reverse(a);
    dbg(downleft);
    dbg(downright);
    dbg(upleft);
    dbg(upright);
    vector<int> mxsuff(n), mxpref(n);
    for (int i = 0; i < n; ++i) {
        chmax(mxpref[i + upright[i]], upright[i]);
        chmax(mxpref[i], upleft[i]);
        chmax(mxsuff[i - upleft[i]], upleft[i]);
        chmax(mxsuff[i], upright[i]);
    }
    for (int i = 1; i < n; ++i)
        chmax(mxpref[i], mxpref[i - 1]);
    for (int i = n - 2; i >= 0; --i)
        chmax(mxsuff[i], mxsuff[i + 1]);
    dbg(mxpref);
    dbg(mxsuff);
    int ans = 0;
    for (int i = 0; i < n; ++i) {
        if (downleft[i] == 0 || downright[i] == 0) continue;
        int mx = max(downleft[i], downright[i]);
        int l = i - downleft[i], r = i + downright[i];
        dbg(i);
        if (mxpref[l] >= mx || mxsuff[r] >= mx) continue;
        ans += win(downleft[i], downright[i]);
    }
    // assert(ans <= 1);
    println(ans);
}
