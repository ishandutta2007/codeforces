...
int solve() {
    ...
    for (int i = 0; i <= n; ++i) {
        if (i + t <= n) {
            int w = i + t;
            int a0 = cntA0[w] - cntA0[i];
            int a1 = cntA1[w] - cntA1[i];
            int b0 = cntB0[w] - cntB0[i];
            int b1 = cntB1[w] - cntB1[i];
            if ((i % 2 && a0 == 0 && b1 == 0) || (i % 2 == 0 && a1 == 0 && b0 == 0)) {
                pii p = {dp[i].first + 1, dp[i].second - (t - a0 - a1 - b0 - b1)};
                dp[w] = max(dp[w], p);
//                DBN(i, p, dp[w]);
            }
        }
