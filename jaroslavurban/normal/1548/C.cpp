...
void ProGamerMove(){
	...
	for(int x=1;x<=3*n;++x){
		...
		// h = d0 + c0 + d0 + c1 + c0 + d0
		// d0 = (h - c0 - 2*c1 ) / 3
		dp[x][0]=(x+3<=3*n)*(choose(3*n,x+1)-2*dp[x-1][0]-dp[x-1][1])*i3;
		dp[x][1]=dp[x-1][0]+dp[x][0];
		dp[x][2]=dp[x-1][1]+dp[x][1];
//		cerr<<x<<": "<<dp[x][0]<<" "<<dp[x][1]<<" "<<dp[x][2]<<endl;
	}
	for(int x:qs)cout<<dp[x][0]+choose(3*n,x)<<" ";cout<<endl;
}
