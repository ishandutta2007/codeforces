...
int main() {
	...
	REP(_, m) {
		...
		map <int, pii> tasks;
		tasks[T + 1] = {0, 0};
		REP(i, n) scanf("%d", type + i);
		REP(i, n) scanf("%d", mand + i);
		REP(i, n) {
			if (type[i])
				tasks[mand[i]].second++;
			else
		...
		int ans = 0;
		int easy_man = 0, hard_man = 0;
		int easy_all = 0, hard_all = 0;
		REP(i, n) if (type[i]) hard_all++; else easy_all++;
		for (auto &x : tasks) {
			if (a * 1ll * easy_man + b * 1ll * hard_man < x.first) {
				int solved = easy_man + hard_man;
				int time_left = x.first - (a * 1ll * easy_man + b * 1ll * hard_man) - 1;
				int extra_easy = min(easy_all - easy_man, time_left / a);
				solved += extra_easy;
				time_left -= extra_easy * a;
				int extra_hard = min(hard_all - hard_man, time_left / b);
				solved += extra_hard;
				time_left -= extra_hard * b;
				assert(time_left >= 0);
				maxi(ans, solved);
			}
			easy_man += x.second.first;
			hard_man += x.second.second;
		}
		assert(easy_man == easy_all);
		assert(hard_man == hard_all);
		printf("%d\n", ans);
	}
}