...
void solve(){
    ...
    REP(i, k) {
        ...
        for (int q = 0; q * k + i < n; ++q) {
            kouho.push_back(q * k + i);
        }
        int now_cost = 0;
        int now_left = 1e9;
        for (int q = 0; q < kouho.size(); ++q) {
            if (q == 0) {
                now_left += cnts[kouho[q]];
            }
            else {
                now_left += cnts[kouho[q]] - cnts[kouho[q - 1] + 1];
            }
            now_left = min(now_left, cnts[kouho[q]]);
            if (s[kouho[q]] == '0')now_left++;
            ans = min(ans, cnts[n] - cnts[kouho[q] + 1] + now_left);
        }
    }
    cout << ans << endl;
}
