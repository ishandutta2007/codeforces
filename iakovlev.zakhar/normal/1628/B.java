...
public class B {
	...
	public void solve() {
		f : for (int qwerty = in.nextInt(); qwerty > 0; qwerty--) {
			int n = in.nextInt();
			HashSet<String> was2 = new HashSet<>();
			HashSet<String> was23 = new HashSet<>();
			HashSet<String> was3 = new HashSet<>();
			boolean flag = false;
			for (int i = 0; i < n; i++) {
				String s = in.next();
				boolean rev = true;
				...
				String s2 = "" + s.charAt(s.length() - 1) + s.charAt(s.length() - 2);
				if (was2.contains(s2)) {
					flag = true;
				}
				if (s.length() == 3) {
					String s3 = "" + s.charAt(2) + s.charAt(1) + s.charAt(0);
					if (was3.contains(s3)) {
						flag = true;
					}
					was3.add(s);
					was23.add(s.substring(0, 2));
				} else {
					if (was2.contains(s2) || was23.contains(s2)) {
						flag = true;
					}
					was2.add(s);
				}
