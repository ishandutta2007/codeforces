...
int main()
{
	...
	for(i=2;i<n;i++){
		LL d=llabs(cross(a[i-1]-a[i],a[i+1]-a[i]));
		ans=min(ans,d/sqrt(dot(a[i-1]-a[i+1],a[i-1]-a[i+1])));
	}
		LL d=llabs(cross(a[n]-a[1],a[2]-a[1]));
		ans=min(ans,d/sqrt(dot(a[n]-a[2],a[n]-a[2])));
		d=llabs(cross(a[n-1]-a[n],a[1]-a[n]));
		ans=min(ans,d/sqrt(dot(a[n-1]-a[1],a[n-1]-a[1])));
	printf("%.6f\n",ans/2);
}