...
struct Variable {
	...
	Variable inline operator + (const Variable& other) {
		Variable ans;
		ans.len = len + other.len;
		if (ans.len < 3) {
			ans.pre = ans.suf = pre + other.pre;
			ans.haha_cnt = 0;
		}
		else {
			ans.pre = (pre + other.pre).substr(0, 3);
			ans.suf = (suf + other.suf).substr((int)(suf.length() + other.suf.length()) - 3, 3);
			ans.haha_cnt = haha_cnt + other.haha_cnt;
			
			string mid = suf + other.pre;
			for (int i = 0; i + 3 < (int)mid.length(); i++)
				if (mid.substr(i, 4) == "haha")
					ans.haha_cnt++;
		}
		return ans;
	}
	...
	CASE(t) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (op == ":=") {
				...
				string rop1, plus_sign, rop2; R(rop1, plus_sign, rop2);
				m[lop] = m[rop1] + m[rop2];
			}
			else assert(false);
			ans = m[lop].haha_cnt;
