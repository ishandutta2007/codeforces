...
void solve(){
  ...
  rep(n,0,1000000){
    ll num0=n*((n+1)/2);
    ll num1=(n+1)/2*(n/2);
    if(num0+num1<m||num0<mx)continue;
    auto ret=vec(n,n,0LL);
    vector<P>p;
    rep(i,0,n)rep(j,0,n)if(i%2==0&&j%2==1)p.EB(i,j);
    rep(i,0,n)rep(j,0,n)if(i%2==0&&j%2==0)p.EB(i,j);
    rep(i,0,n)rep(j,0,n)if(i%2==1&&j%2==0)p.EB(i,j);
    auto ord=descend(a);
    ll idx=0;
    for(auto z:ord){
      rep(_,0,a[z]){
        ret[p[idx].fi][p[idx].se]=z;
        idx++;
      }
    }
    cout<<n<<endl;
