...
namespace NT{
    ...
    template<typename T>
    typename enable_if<is_integral<T>::value, vector<T>>::type get_divisors(T x){
        auto v = factorize(x);
        vector<T> ans = {1}, buf;
        rep(i, v.size()) {
            if(i && v[i] == v[i - 1]) {
                for(auto& x: buf) x *= v[i];
                ans.insert(ans.end(), buf.begin(), buf.end());
            }
            else {
                buf = ans;
                for(auto& x: buf) x *= v[i];
...
int32_t main() {
    ...
    while(T--) {
        ll p, q; cin >> p >> q;
        auto v = NT::get_divisors(p);
        reverse(all(v));
        for(auto& x: v) if(x % q != 0) {
            cout << x << endl;
            break;
        }
    }
