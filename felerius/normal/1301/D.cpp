...
// Problem: D: Time to Run (https://codeforces.com/contest/1301/problem/D)
...
int main() {
    ...
    auto run = [&](char dir, int far) {
        far = min(far, rem);
        if (far > 0)
            inst.emplace_back(far, dir);
        rem -= far;
    };

    run('R', m - 1);
    rep(i, m - 1)
        run('D', n - 1), run('U', n - 1), run('L', 1);
    rep(i, n - 1)
        run('D', 1), run('R', m - 1), run('L', m - 1);
    run('U', n - 1);
