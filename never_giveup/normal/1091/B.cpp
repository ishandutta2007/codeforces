...
const int maxn = 2e5 + 10, maxw = 1e6 + 100, inf = 2e9 + 100, sq = 600, X = 43, mod = 1e9 + 7, LG = 17;
...
int main()
{
    ...
    for (int its = 0; its < n; its++) {
        int x = a[0].fr + b[its].fr, y = a[0].sc + b[its].sc;
        multiset<pair<int, int> > g;
        for (int i = 0; i < n; i++)
            if (i != its)
                g.insert(b[i]);
        for (int i = 1; i < n; i++) {
            pair<int, int> o = {x - a[i].fr, y - a[i].sc};
            auto it = g.lower_bound(o);
            if (it == g.end() || *it != o)
                break;
            g.erase(it);
        }
