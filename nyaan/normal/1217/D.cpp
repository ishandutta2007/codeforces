...
// DAGtruefalse
template<typename T>
bool TopologicalSort(vector<int> &v , T &g){
  int N = g.size();
  ...
  for(int i = 0 ; i < N ; i++){
    if(marked[i] == 0) {
      if(visit(visit , i) == false) return false;
    }
  }
...
void solve(){
  ...
  vvi g(N);
  vi d;
  rep(i,M) g[v[i].fi - 1].pb(v[i].se - 1);
  if(TopologicalSort(d , g) == true){
    out(1);
    rep(i,M) cout << (i?" ":"") << 1;
    cout << endl;
  }
    ...
    out(2);
    rep(i,M) cout << (i?" ":"") << (v[i].fi>v[i].se ? 2 : 1);
    cout << endl;
  }
