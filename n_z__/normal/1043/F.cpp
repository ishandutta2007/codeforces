...
main(){
    ...
    if(b[x])for(int y=2;x*y<=300000;y++)b[x*y]=0;
    n=0;
    for(int x=1;x<=300000;x++)
    if(b[x])a[++n]=x;
    int ans=10;
    while((double)(clock()-st)/CLOCKS_PER_SEC<=2.8)
    {
        int cnt=0,ng=0;
        while(cnt<=7){
            int u=mt()%n+1;cnt++;
            if(cnt==1)ng=a[u];
            else ng=__gcd(ng,a[u]);
            if(ng==1){ans=min(ans,cnt);break;}
        }
    }
    if(ans==10)cout<<-1;else cout<<ans<<endl;
}