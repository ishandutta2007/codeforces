...
int main() {
	CASE(t) {
		...
		const vector<pii> pos = {{0, 1}, {1, 0}, {n - 1, n - 2}, {n - 2, n - 1}};
		const vector<pii> posc = {{1, 2}, {2, 1}, {n, n - 1}, {n - 1, n}};
		auto get = [&](int i) {
			return s[pos[i].F][pos[i].S];
		};
		...
		int dist = abs(cnt[0][0] - cnt[1][0]) + abs(cnt[0][1] - cnt[1][1]);
		...
		if (dist == 2) {
			...
			for (int i = 2 - pos2 * 2; i <= 2 - pos2 * 2 + 1; i++) {
				if (get(i) - '0' == val2) {
					W(posc[i]);
				}
			}
