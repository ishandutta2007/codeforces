...
map<uint, int> process(int l, int r) {
    int hm = r-l;
    ...
    for (int bm = 0; bm < (1<<hm); bm++) {
        if (bm) {
            popcnt[bm] = popcnt[bm ^ (bm&-bm)] + 1;
            vv[bm] = vv[bm ^ (bm&-bm)] ^ sw[l + __builtin_ctz(bm&-bm)];
        }

        if (!ret.count(vv[bm])) ret[vv[bm]] = popcnt[bm];
        else ret[vv[bm]] = min(ret[vv[bm]], popcnt[bm]);
    }
...
int main() {
    ...
    int m = s / 2;
    map<uint, int> procl = process(0, m);
    map<uint, int> procr = process(m, s);
    ...
    for (int i = 0; i < d; i++) {
        ...
        for (auto [x,y] : procl) {
            dbg(x, x ^ al);
            auto it = procr.find(x ^ al);
            if (it != procr.end()) {
                dbg(x, y, al, it->second);
                ans = min(ans, y + it->second);
            }
        }
        cout << (ans == 100 ? -1 : ans) << '\n';
    }
