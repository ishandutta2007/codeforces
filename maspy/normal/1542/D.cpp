...
void solve() {
  ...
  FOR(i0, N) {
    ...
    FOR(i, N) {
      if (i == i0) continue;
      bool small = [&]() -> bool {
        if (A[i] < A[i0]) return true;
        if (A[i] > A[i0]) return false;
        return i < i0;
      }();
      ...
      if (A[i] == 0) {
        FOR(k, N) {
          if (i > i0 && k == 0) continue;
          newDP[max(k - 1, 0LL)] += DP[k];
        }
      }
      elif (small) {
        FOR(k, N - 1) { newDP[k + 1] += DP[k]; }
      }
      elif (!small) { FOR(k, N) newDP[k] += DP[k]; }
      swap(DP, newDP);
    }
    ANS += SUM(DP) * mint(A[i0]);
  }
  print(ANS);
}
