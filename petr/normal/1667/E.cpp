...
std::vector<mint> convolution_threemods(const std::vector<mint>& a,
    ...
    for (int i = 0; i < a.size(); ++i) la[i] = a[i].val();
    for (int i = 0; i < b.size(); ++i) lb[i] = b[i].val();
...
class ETsentroidnieVeroyatnosti {
 public:
  void solveOne() {
    int n;
    cin >> n;
    ...
    vector<mint> prod = atcoder::convolution(p1, p2);
    vector<mint> w(n);
    mint sw = 0;
    for (int below = 0; below < n - 1; ++below) {
      w[n - 1 - below] = facts[below] * prod[below] * (n - 1 - below) - sw / (n - below);
      sw += w[n - 1 - below];
    }
    w[0] = facts[n - 1] - sw;
    for (int i = 0; i < n; ++i) {
...
int main() {
    ...
    std::cin.tie(0);
    ETsentroidnieVeroyatnosti solver;
