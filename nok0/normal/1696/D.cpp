...
void main_() {
	...
	V<> pl(n);
	rep(i, n) pl[a[i]] = i;
	range_edge_graph<int> re(n * 3);
	sparse_table<int, op> mx(a);
	sparse_table<int, op2> mn(a);
	int now = 0;
	int res = 0;
	if(n == 1) {
		print(0);
	...
	while(now < n) {
		res++;
		if(mx.prod(now, n) == a[now] and mn.prod(now, n) == a[n - 1]) {
			break;
		}
		if(mn.prod(now, n) == a[now] and mx.prod(now, n) == a[n - 1]) {
			break;
		}
		int ok = now + 1, ng = n;
		auto f = [&](int x) {
			return mx.prod(now, x) == a[now] or mn.prod(now, x) == a[now];
		};
		auto r = bin_search(ok, ng, f);
		if(r == now + 1) {
			now++;
			continue;
		}
		if(a[now] == mx.prod(now, r)) {
			int minv = mn.prod(now, r);
			now = pl[minv];
		} else {
			int maxv = mx.prod(now, r);
			now = pl[maxv];
		}
	}
	print(res);
