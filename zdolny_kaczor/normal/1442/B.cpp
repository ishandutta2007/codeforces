...
void solve() {
	...
	for (int &x : b) x--;
	vi renum(n);
	for (int i = 0; i < n; ++i) renum[a[i]] = i;
	for (int &x : b) x = renum[x];
	debug(imie(b));
	set <int> rem, waiting;
	for (int &x : b) waiting.insert(x);
	...
	for (int x : b) {
		...
		r++;
		int here = 0;
		bool can_l = (*l != -inf && !waiting.count(*l)), can_r = (*r != inf && !waiting.count(*r));
		if (can_l) here++;
		if (can_r) here++;
		(ans *= here) %= mod;
		if (can_l) rem.erase(l);
		else if (can_r) rem.erase(r);
		else {
			puts("0");
			return;
		}
