...
void dfs(int v, int d)
{
	...
	ll u0 = trie.vec[v].next[0], u1 = trie.vec[v].next[1];
	if(trie.vec[v].num){
		ans += 1;
		if(d+1 <= p){
			if(u1 == -1) ans += dp[p-(d+1)];
			else trie.vec[u1].num = 1;
		}
		if(d+2 <= p){
			if(u0 == -1 || trie.vec[u0].next[0] == -1) ans += dp[p-(d+2)];
			else trie.vec[trie.vec[u0].next[0]].num = 1;
		}
	}
	if(u0 != -1) dfs(u0, d+1);
	if(u1 != -1) dfs(u1, d+1);
...
int main(void)
{
	...
	rep(i, 1, n){
		cin >> a[i];
		string s = lltos(a[i], 2);
		trie.insert(s);
	}
	
	dp[0] = 1, dp[1] = 1;
	rep(i, 2, p) dp[i] = dp[i-1] + dp[i-2];
	rep(i, 1, p) dp[i] += dp[i-1];
