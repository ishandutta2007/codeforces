...
int main() {
    ...
    rep(i, n) {
        if(uf.size(i) == 2) flag2 = true;
        if(uf.size(i) & 1) flag = false;
    }
    if(flag & flag2) {
        cout << "YES\n";
        vi v;
        int r;
        rep(i, n) if(uf.size(i) == 2) r = uf.find(i);
        rep(i, n) if(uf.find(i) == r) v.eb(i);
        uf = UnionFind(n);
        auto p = [&](int i, int j) {
            uf.unite(i, j);
            cout << ++i << " " << ++j << endl;
        ...
        rep(i, n) {
            if(!uf.same(v[0], i)) {
                pii now(v[0], i);
                while(!uf.same(now.fi, now.se)) {
                    p(now.fi, now.se);
                    now.fi = a[now.fi], now.se = a[now.se];
                }
            }
        }
    } else {
