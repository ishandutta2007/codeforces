...
int main(){
  ...
  while(t--){
    ...
    ll o=(m+1)/2;
    ll e=m-o;
    modint inv=modint(o)/e;
    //cout<<o spa e spa inv<<endl;
    modint ret=0;
    modint pp=1;
    modint cf=1;
    modint tp=1+inv;
    rep(i,0,k+1){
      ret+=pre[i][k]*cf*pp*tp.pow(n-i);
      //cout<<i spa k spa pre[i][k] spa cf spa pp<<endl;
      pp*=inv;
      if(n<=i)break;
      cf*=n-i;
      cf*=comb.inv(i+1);
    }
    cout<<ret*comb.fact(k)*pow(e,n)<<endl;
  }
  return 0;
}