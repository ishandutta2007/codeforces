...
int main(){
  ...
  rep(i,1,m+1){
    ...
    bool ok=true;
    for(auto z:mp)if(z.se>=2)ok=false;
    if(!ok)continue;
    int sz=m/i;
    vector<modint>dp(sz+2);
    dp[0]=(mp.size()%2==1?-1:1);
    rep(j,0,n){
      vector<modint>ndp(sz+2);
      int lv=(l[j]+i-1)/i,rv=r[j]/i;
      rep(o,0,sz+1){
        ndp[min<int>(o+lv,sz+1)]+=dp[o];
        ndp[min<int>(o+rv+1,sz+1)]-=dp[o];
      }
      rep(o,0,sz)ndp[o+1]+=ndp[o];
      dp.swap(ndp);
    }
    rep(i,0,sz+1)ret+=dp[i];
    //debug(dp);
  }
  cout<<ret<<endl;
  return 0;
