...
void calc(int n) {
    divs.clear(); divs.pb(1);
    while (n>1) {
        int p = factor[n], exp = 0;
        while (n%p==0) n /= p, exp++;
        int sz = divs.size(), f = 1;
        F0R(rep, exp) {
            f *= p;
            F0R(i, sz) divs.pb(divs[i] * f);
        }
    }
}
...
void solve() {
    ...
    F0R(j, k) {
        seggs[j].init(divs[j]);
        F0R(i, n) seggs[j].upd(i%divs[j], (ll)a[i]*divs[j]);
        ckmax(mx, seggs[j].seg[1]);
    }
    cout << mx << nl;
    while (q--) {
        int p,x; cin >> p >> x; p--;
        mx = 0;
        F0R(j, k) {
            seggs[j].upd(p%divs[j], ll(x-a[p]) * divs[j]);
            ckmax(mx, seggs[j].seg[1]);
        }
        a[p] = x;
        cout << mx << nl;
