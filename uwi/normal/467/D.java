...
public class D {
	...
	void solve()
	{
		...
		int[] to = new int[n];
		for(int i = 0;i < n;i++){
			String f = ns().toLowerCase();
			String t = ns().toLowerCase();
			if(!map.containsKey(f))map.put(f, id++);
			if(!map.containsKey(t))map.put(t, id++);
			from[i] = map.get(f);
			to[i] = map.get(t);
		}
		
		String[] imap = new String[map.size()];
		for(Map.Entry<String, Integer> e : map.entrySet()){
			imap[e.getValue()] = e.getKey();
		}
		
		int[][] g = packD(id, from, to);
		int[][] ig = packD(id, to, from);
		int[] clus = decomposeToSCCNoRec(g, ig);
		
		int[][] cg = collect(g, clus);
		int[] ord = sortTopologically(cg);
		int u = cg.length;
		int[] r = new int[u];
		int[] len = new int[u];
		Arrays.fill(r, 99999999);
		Arrays.fill(len, 99999999);
		for(int i = 0;i < id;i++){
			int lr = 0, llen = imap[i].length();
			for(char c : imap[i].toCharArray()){
				if(c == 'r')lr++;
			}
			if(lr < r[clus[i]] || lr == r[clus[i]] && llen < len[clus[i]]){
				r[clus[i]] = lr;
				len[clus[i]] = llen;
			}
		}
		...
		for(int i = u-1;i >= 0;i--){
			int cur = ord[i];
			for(int e : cg[cur]){
				if(r[e] < r[cur] || r[e] == r[cur] && len[e] < len[cur]){
					r[cur] = r[e];
					len[cur] = len[e];
				}
			}
