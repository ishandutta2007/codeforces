...
struct Event{
	int x,y,dis;
	Event(){}
	Event( int xi , int yi , int di ){
		x = xi; y = yi; dis = di;
	}
	bool operator<( const Event b )const{ return dis > b.dis; }
};
...
void Dijkstra(){
	Msn(minl,INF);
	pq.push(Event(stx,sty,0));
	minl[stx][sty] = 0;
	while(!pq.empty()){
		Event ue = pq.top(); pq.pop();
		int x = ue.x, y = ue.y, d = minl[x][y];
		if(vis[x][y]) continue;
		vis[x][y] = 1;
		For0(k,4){
			int vx = x + dx4[k];
			int vy = y + dy4[k];
			int vd = d + (k==2);
			if( vx < 1 || vx > n || vy < 1 || vy > m || mp[vx][vy] == '*' ) continue;
			if( minl[vx][vy] > vd ){
				minl[vx][vy] = vd;
				pq.push(Event(vx,vy,vd));
			}
		}
	}
}
int Solve(){
	...
	For(i,n)
		For(j,m){
			if( mp[i][j] == '*' ) continue;
			int r = minl[i][j] + j - sty;
			if( r > canr || minl[i][j] > canl ) continue;
			ans++;
		}
	return ans;
}
