...
int codeforces;cin>>codeforces;while(codeforces--){
  ll x,N;
  cin>>N;
  mat dp(2,vec(3000,inf));
  dp[0][0]=0;
  while(N--){
    cin>>x;
    ll i;
    rep(i,3000)dp[1][i]=inf;
    rep(i,3000){
      if(dp[0][i]==inf)continue;
      if(i-x>=0){
        chmin(dp[1][i-x],dp[0][i]);
        chmin(dp[1][dp[0][i]-i+x],dp[0][i]);
      }else if(dp[0][i]+x-i<3000){
        chmin(dp[1][0],dp[0][i]+x-i);
        chmin(dp[1][dp[0][i]+x-i],dp[0][i]+x-i);
      }
      if(i+x<=dp[0][i]){
        chmin(dp[1][i+x],dp[0][i]);
        chmin(dp[1][dp[0][i]-i-x],dp[0][i]);
