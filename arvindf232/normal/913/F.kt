...
fun main(){
    solve.cases{
        ...
        val a = getint()
        val b = getint()
        val pro = a.modDivide(b)
        val q = 1 modMinus pro
        val DP_mess = Array(n+1){IntArray(n+1)}
        val stayodds = IntArray(n+1)
        val dpexpects = IntArray(n+1)
        val qtt = IntArray(n+3){intPow(q,it,pI)}
        val ptt = IntArray(n+3){intPow(pro,it,pI)}
        DP_mess[1][1] = 1
        DP_mess[1][0] = 1
        stayodds[1] = 1
        for(part in 2..n){
            var downOdds = 0
            DP_mess[part][0] = 1
            for(top in 1 until part){
                val p1 = DP_mess[part-1][top-1] modM (qtt[part-top])
                val p2 = DP_mess[part-1][top] modM (ptt[top])
                DP_mess[part][top] = p1 modPlus p2
                downOdds = downOdds modPlus (DP_mess[part][top] modM stayodds[top])
            }
            var ifreduce = 0
            for(i in 1 until part){
                ifreduce = ifreduce modPlus (((dpexpects[i]) modPlus (dpexpects[part-i] - c2(part-i))) modM stayodds[i] modM  DP_mess[part][i])
            }
            stayodds[part] = (1 modMinus downOdds)
            val overall = (c2(part) modDivide downOdds) modPlus (ifreduce modDivide downOdds)
            DP_mess[part][part] = 1
            dpexpects[part] = overall
        }
        put(dpexpects[n])
    }
    done()
...
/*
single case empty product is 1
product outside
 */