...
int main() {
    ...
    rep(i, N)scanf("%lf%lf", &l[i], &r[i]);
    vector<double> rui(N + 1);
    repn(i, N)rui[i] = rui[i - 1] + r[i - 1] - l[i - 1];

    auto len = [&](double x) {
        auto itr = lower_bound(r.begin(), r.end(), x);
        ll t = distance(r.begin(), itr);
        double ret = rui[t];
        //cout << t << endl;

        if (t < N) {
            ret += max<double>(0, x - l[t]);
            //cout << x - l[t] << endl;
        }
        // cout << ret << endl;
        return ret;
    ...
    rep(snuke, Q) {
        ...
        double aa = x + (a - x) / (y - sy) * y;
        double bb = x + (b - x) / (y - sy) * y;
        //cout << aa << " " << bb << endl;
        //cout << len(aa) << endl << len(bb) << endl;

        double ans = len(bb) - len(aa);
        ans = ans / y * (y - sy);
        printf("%.15f\n", ans);
    }
}