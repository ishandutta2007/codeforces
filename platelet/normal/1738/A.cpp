...
void solve() {
    ...
    rep(i, 1, n) {
        cin >> b[i], ans += b[i];
        a[i] ? v1.pb(b[i]) : v0.pb(b[i]);
    }
    sort(all(v0), greater()), sort(all(v1), greater());
    if(SZ(v0) == SZ(v1)) cout << ans * 2 - min(v0.back(), v1.back()) << '\n';
    else {
        int sz = min(SZ(v0), SZ(v1));
        For(i, 0, sz) ans += v0[i] + v1[i];
        cout << "    " << ans << "   \n";
    }
}
int main() {
