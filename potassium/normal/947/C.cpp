...
struct Trie {
	struct Node {
		vector<int> to = vector<int>(2, 0);
		int cnt = 0;
	};
	
	vector<Node> trie = vector<Node>(2);
	...
	void insert(ll val) {
		...
		for (ll b = maxV; b >= 1; b >>= 1) {
			int bit = (val & b) >= 1;
			
			if (trie[curNode].to[bit] == 0) {
				trie[curNode].to[bit] = trie.size();
				trie.pb(Node());
			}
			
			curNode = trie[curNode].to[bit];
			trie[curNode].cnt++;
		}
	}
	
	ll match(ll val, int curNode = 1, int pw = 30) {
		trie[curNode].cnt--;
		if (pw == -1)
			return 0;
		...
		ll ans = -1;
		for (int go = bit; ; go ^= 1) {
			if (trie[curNode].to[go] != 0) {
				ans = match(val, trie[curNode].to[go], pw - 1);
				ans += b * go;

				if (trie[trie[curNode].to[go]].cnt == 0) {
					trie[curNode].to[go] = 0;
				}
