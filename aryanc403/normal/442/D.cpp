...
void chk(lli u)
{
    if(u==1)
        return;
    lli ans=0,v=p[u];
    ii x={dp[v][0].X,dp[v][1].X};
    bool fl=true;
    for(auto &x:dp[v])
        if(x.Y==u)
        {
            fl=false;
            x.X=max(dp[u][0].X,dp[u][1].X+1);
            sort(dp[v].begin(),dp[v].end(),cmp);
        }

    if(fl)
    {
        dp[v].pb({max(dp[u][0].X,dp[u][1].X+1),u});
        sort(dp[v].begin(),dp[v].end(),cmp);
        dp[v].pop_back();
    }

    if(x==mp(dp[v][0].X,dp[v][1].X))
        return;
    chk(v);
}
...
// cin>>T;while(T--)
{
    ...
    cin>>n;
    dp[1].pb({0,0});dp[1].pb({0,0});
    dp[0].pb({0,0});dp[0].pb({0,0});

    repA(i,2,n+1)
    {
        cin>>in;
        p[i]=in;
        dp[i].pb({0,0});dp[i].pb({0,0});
        chk(i);
        cout<<dp[1][0].X<<" ";
        dbg(i,dp[1]);
    }
