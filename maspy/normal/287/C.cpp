...
void solve() {
  LL(N);
  if (N % 4 == 2 || N % 4 == 3) return print(-1);
  vi ANS(N);
  if (N % 4 == 0) {
    FOR(i, N) {
      if (i % 2 == 0 && i < N / 2) { ANS[i] = i + 2; }
      elif (i % 2 == 1 && i < N / 2) { ANS[i] = N - i + 1; }
      elif (i % 2 == 0 && i >= N / 2) { ANS[i] = N - i - 1; }
      elif (i % 2 == 1 && i >= N / 2) { ANS[i] = i; }
    }
    print(ANS);
  }
  elif (N % 4 == 1) {
