...
int main() {
    ...
    vector<char> chars = {'R', 'G', 'B', 'Y', 'W', '1', '2', '3', '4', '5'};
    int ans = 10;

    FOR(i, 0, 1 << 10) {
        set<char> used;

        REP(j, 10) if(i & (1 << j)) used.insert(chars[j]);
        ...
        for(auto con: cons) {
            bool ok2 = false;
            for(auto c: con) {
                if(used.count(c)) ok2 = true;
            }
            ok = ok & ok2;
        }

        if(ok) ans = min(ans, (int)__builtin_popcount(i));
    }
