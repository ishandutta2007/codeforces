...
void __solve()
{
    ...
    for(i32 i = 0; i+1 < n; ++i)
    {
        i32 x,y,z,a,b,c; tie(x,y,z)=p[id[i]], tie(a,b,c)=p[id[i+1]];
        if(x==a and y==b)
        {
            cout << id[i]+1 << " " << id[i+1]+1 << "\n";
            is[i]=is[i+1]=1;
            i++;
        }
    }
    ...
    for(i32 i=0,j=-1; i<n; i++)
    {
        if(is[i])
        {
            continue;
        }
        if(~j and get<0>(p[id[j]])==get<0>(p[id[i]]))
        {
            cout << id[i]+1 << " " << id[j]+1 << "\n";
            is[i]=is[j]=1;
            j=-1;
