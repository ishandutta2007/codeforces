...
signed main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (vis[i] == -1)
        {
            ...
            b2.clear();
            sz = 0;
            for (int j = 0; j < LG; j++) cnt0[j] = 0, cnt1[j] = 0, broke[j] = 0;
            dfs(i, 0ll);
            ll G = 1, F = 1;
            ...
            for (int j = 0; j < LG; j++)
            {
                ...
                if (broke[j])
                {
                    //cout << "en " << j << endl;
                    //cout << G << " " << H << endl;
                    ans = (ans + sz * (sz - 1) / 2 % MOD * G % MOD * H) % MOD;
                }
                else
                {
                    ans = (ans + cnt0[j] * cnt1[j] % MOD * F % MOD * H) % MOD;
                }
            }
        }
    }
