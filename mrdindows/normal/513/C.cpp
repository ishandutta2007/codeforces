...
// 			if (i != 1) upd(dmax[i - 2][st - 1], dmax[i - 1][st - 1]);
// 			if (i != 1) upd(dmin[i - 2][st - 1], dmin[i - 1][st - 1]);
// 			if (st != 2) upd(dmin[i - 2][st - 2], dmax[i - 1][st - 1]);
// 			if (st != 2) upd(dmin[i - 2][st - 2], dmin[i - 1][st - 1]);
// 			int pmin = dmin[i - 1][st - 1];
// 			int pmax = dmax[i - 1][st - 1];
// 			int sum = 0;
// 			DBN(i, st, pmin, pmax);
// 			for (int j = i; j < n; ++j){
// 				sum += a[j];
...
// 	int ans = -INF;
// 	for (int i = 1; i < n; ++i){
// 		if (i != 1) upd(dmax[i - 2][k - 1], dmax[i - 1][k - 1]);
// 		if (i != 1) upd(dmin[i - 2][k - 1], dmin[i - 1][k - 1]);
// 		for (int j = i; j < n; ++j){
// 			int sum = s[j + 1] - s[i];
// 			ans = max(ans, dmin[i - 1][k - 1] + sum, dmax[i - 1][k - 1] - sum);
// 		}
// 	}
// 	cout << ans << endl;
// 	return 0;
...
int r[100];
bool used[100];
int per(int l, int r, int x, int y){
	if (r < x || l > y) return 0;
	else return 1 + min(r, y) - max(l, x);
}
double tot[100];
int main()
{
	...
	for (int i = 0; i < n; ++i){
		for (int j = 0; j < n; ++j){
			if (j == i) continue;
			double k = 1.0 * tot[i];
			for (int res = l[i]; res <= r[i]; ++res){
				double p = k * res;
				int t = per(l[j], r[j], res+(i>j), 100000);
				p *= t * tot[j];
				for (int q = 0; q < n; ++q){
					if (q == i || q == j) continue;
					int t = per(l[q], r[q], 0, res - (i < q));
					p *= t * tot[q];
				}
				ans += p;
			}
