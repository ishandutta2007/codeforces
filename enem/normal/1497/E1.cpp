...
int main()
{
    ...
    static const ll N = 1e7+5, N2 = 2e5+5;
    static ll ntc, n, k, a[N2], ans[30], d[N];
    static V(ll) prim;
    static set<ll> st[30];
    ...
    REP(i,2,N)
    {
        ...
        for(ll j=0; j<((ll)prim.size()) && prim[j]<=d[i] && i*prim[j]<N; j++)
        {
            d[i*prim[j]] = prim[j];
        }
    }
    ...
    REP(tc,1,ntc+1)
    {
        ...
        REP(i,0,n)
        {
            REPI(j,0,k+1)
            {
                ...
                {
                    if(j+1<=k && ans[j+1]>ans[j] || (ans[j+1]==ans[j] && ((ll)st[j+1].size())>((ll)st[j].size())))
                    {
                        ans[j+1] = ans[j];
                        st[j+1] = st[j];
                    }
