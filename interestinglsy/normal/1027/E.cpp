...
void Init(){
	...
	For(l,n){
		For(i,l){
			dp[l][i][0] = dp[l][i][1] = Qpow(2,i-1);
		}
		Forx(i,l+1,n){
			Forx(j,i-l,i-1){
				(dp[l][i][0] += dp[l][j][1]) %= MOD;
				(dp[l][i][1] += dp[l][j][0]) %= MOD;
			}
		}
	}
}
...
void Solve(){
	For(i,n)
		For(j,n){
			if( i*j >= k ) break;
			ll tans = (dp[i][n][0]-dp[i-1][n][0]) * (dp[j][n][0]-dp[j-1][n][0]);
			tans += (dp[i][n][1]-dp[i-1][n][1]) * (dp[j][n][1]-dp[j-1][n][1]);
			Rmod(tans,MOD);
			ans += tans;
			Mymod(ans,MOD);
		}
