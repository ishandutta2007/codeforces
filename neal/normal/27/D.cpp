// Solution to https://codeforces.com/contest/27/problem/D
#include <algorithm>
#include <cassert>
#include <iostream>
#include <vector>
...
bool intersects(pair<int, int> x, pair<int, int> y) {
    if (x.first > y.first)
        swap(x, y);
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        int a, b;
        cin >> a >> b;
        roads.emplace_back(min(a, b), max(a, b));
    }
    ...
    for (int i = 0; i < m; i++)
        for (int j = i + 1; j < m; j++)
            if (intersects(roads[i], roads[j])) {
                TS.either(vars[i], vars[j]);
                TS.either(TS.inv(vars[i]), TS.inv(vars[j]));
            }
