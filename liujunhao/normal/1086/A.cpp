...
//template
int xa,ya,xb,yb,xc,yc,ans=1e9,posx,posy;
void find_way(int sx,int sy,int tx,int ty){
	int i;
	for(i=min(sx,tx)+1;i<max(sx,tx);i++)
		printf("%d %d\n",i,sy);
	if(sx!=tx)
		printf("%d %d\n",tx,sy);
	for(i=min(sy,ty)+1;i<max(sy,ty);i++)
		printf("%d %d\n",tx,i);
	if(ty!=sy)
		printf("%d %d\n",tx,ty);
}
int main()
{
	...
	for(i=0;i<=1000;i++)
		for(j=0;j<=1000;j++){
			if(ans>abs(i-xa)+abs(j-ya)+abs(i-xb)+abs(j-yb)+abs(i-xc)+abs(j-yc)+1)
			posx=i,posy=j,ans=abs(i-xa)+abs(j-ya)+abs(i-xb)+abs(j-yb)+abs(i-xc)+abs(j-yc)+1;
		}
	printf("%d\n",ans);
	printf("%d %d\n",posx,posy);
	find_way(posx,posy,xa,ya);
	find_way(posx,posy,xb,yb);
	find_way(posx,posy,xc,yc);
}