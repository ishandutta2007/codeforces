...
void solve(){
    ...
    string s;
    cin >> s;
    SegmentTree<int> seg(n+5, [](int a, int b) {return min(a, b); }, 1e18);
    for (int i = 1; i <= n; ++i) {
        if (s[i - 1] == '1') {
            int hoge = i + seg.query(max(0LL, i - 2 * k - 1), i);
            int tmp = max(i - k - 1LL, 0LL);
            hoge = min(hoge, i + tmp * (tmp + 1LL) / 2LL);
            seg.update(i, hoge);
        }
        seg.update(i + 1, seg[i] + (i + k + 1));
    }
    int ans = n * (n + 1LL) / 2LL;
    int costs = 0;
    for (int i = n; i >= 0; --i) {
