...
ll n, m;
string s;
ll r[200005], d[200005];
ll sx[200005], sy[200005], tx[200005], ty[200005];

ll get(ll i, ll j)
{
	ll w = max(0LL, min(tx[i], n-(r[m]-r[j])) - sx[i] + 1);
	ll h = max(0LL, min(ty[i], n-(d[m]-d[j])) - sy[i] + 1);
	return w*h;
}
...
int main(void)
{
	...
	while(T--){
		...
		ll p = inf;
		rep(i, 1, m) if(r[i] && d[i]) chmin(p, i);
		
		rep(i, 1, m){
			ll x = 1+r[i], y = 1+d[i];
			sx[i] = x, sy[i] = y;
			if(r[i]) tx[i] = n; else tx[i] = x;
			if(d[i]) ty[i] = n; else ty[i] = y;
		}
		
		ll ans = 1;
		rep(i, 1, m){
			ans += get(i, i);
			if(i+1 <= m && i+1 != p) ans -= get(i+1, i);
		}
		outl(ans);
	}
