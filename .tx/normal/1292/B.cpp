...
void solve() {
	llong x0, y0, ax, ay, bx, by, xs, ys, t;
	cin >> x0 >> y0 >> ax >> ay >> bx >> by >> xs >> ys >> t;

	vector<llong> x(1, x0), y(1, y0);

	auto d = [&](llong x1, llong y1, llong x2, llong y2) {
		return abs(x1 - x2) + abs(y1 - y2);
	};

	while (max(abs(x.back()), abs(y.back())) < llong(3e16)) {
