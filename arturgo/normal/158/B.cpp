...
auto norme_inf(const T& a) {
	return sum(apply(a, Abs<typename T::value_type>()));
}
...
int main() {
	...
	auto tailles = read<vector<int>>();

	map<int, int> paires;
	for(pair<int, int> paire : cut(sorted(tailles))) {
		paires[paire.first] = paire.second;
	}
	...
	nb += paires[3];
	paires[1] -= paires[3];
	nb += div_up(paires[2], 2);
	paires[1] -= 2 * (paires[2] % 2);
	nb += max(0, div_up(paires[1], 4));
