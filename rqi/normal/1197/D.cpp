...
ll ans;
int main() {
    setIO(); // CHANGE TO LLLLllLLLLLLLSFLDFLDSJFIFJWOIEFJ
    ll n, m, k;
    cin >> n >> m >> k;
    for(ll i = 1; i <= n; i++){
    	cin >> a[i];
    ...
    for(ll i = 1; i <= m; i++){ // size of subarray mod m
    	for(ll j = 1; j <= m; j++){ // ending value mod m
    		ll minpre = 3E15;
    		for(ll b = j; b <= n; b+=m){ // ending value
    			minpre+=k;
    			if(b-i < 0) continue;
    			
    			minpre = min(minpre, pres[b-i]+k);
    			
    			ans = max(ans, pres[b]-minpre);
    			
    			//cout << i << " " << j << " " << b << " " << minpre << " " << pres[j] <<"\n";
    		}
    	}
    }
    ans = max(ans, 0LL);
