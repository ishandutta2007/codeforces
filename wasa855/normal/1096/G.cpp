...
namespace Poly
{
	...
	{
		__inv[0]=__inv[1]=1;
		for(int i=2;i<=4000000;i++) __inv[i]=mul(mod-mod/i,__inv[mod%i]);
	}
...
signed main()
{
	...
	int B=0; while((1<<B)<f.size()) B++;
	f.resize(1<<B); init(B);
	NTT(f,B);
	for(int i=0;i<f.size();i++) f[i]=qpow(f[i],n/2);
	reverse(f.begin()+1,f.end());
	NTT(f,B);
	int In=getInv(1<<B);
	for(int i=0;i<f.size();i++) f[i]=mul(f[i],In);
	int ans=0;
	for(int i=0;i<f.size();i++) ans=add(ans,mul(f[i],f[i]));
	cout<<ans<<endl;
	return 0;
}