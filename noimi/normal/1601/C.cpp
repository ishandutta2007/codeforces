...
int main() {
    ...
    while(testnum--) {
        ...
        rep(i, n) a[i] = read();
        rep(i, m) b[i] = read();
        // rep(i, n) a[i] = rnd(inf<int>);
        // rep(i, m) b[i] = rnd(inf<int>);
        ll ans = inversion(a, false);
        vector<tuple<int, int, int>> v;
        v.reserve(n + m);
        rep(i, n) v.eb(a[i], i, 0);
        rep(i, m) v.eb(b[i], i, 1);
        sort(all(v));

        vi setter(n + 1);
        rep(i, n + 1) setter[i] = i;
        atcoder::lazy_segtree<int, f, e, int, g, g, E> seg(setter);
        ...
        while(idx < si(v)) {
            ...
            for(auto e : A) { seg.apply(e + 1, n + 1, -1); }
            ans += 1LL * B * seg.all_prod();
            for(auto e : A) seg.apply(0, e + 1, 1);
            idx = ni;
        }
        OUT(ans);
    }
