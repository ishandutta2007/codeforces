...
int main() {
  int N, A, B;
  cin >> N >> A >> B;
  vector< int64 > hp(N), dmg(N);
  MaximumSum< int64 > sum(B);
  int64 add = 0;
  for(int i = 0; i < N; i++) {
    cin >> hp[i] >> dmg[i];
    add += dmg[i];
    sum.insert(max(0LL, hp[i] - dmg[i]));
  }
  int64 ret = 0;
  for(int i = 0; i < N; i++) {
    sum.erase(max(0LL, hp[i] - dmg[i]));
    sum.insert(max(0LL, hp[i] * (1ll << A) - dmg[i]));
    chmax(ret, sum.query());
    sum.erase(max(0LL, hp[i] * (1ll << A) - dmg[i]));
    sum.insert(max(0LL, hp[i] - dmg[i]));
  }
  cout << ret + add << endl;
}