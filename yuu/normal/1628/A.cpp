...
void solve() {
    ...
    int max_pos = 0;
    b.clear();
    while (!pos[max_mex].empty()) {
        max_pos = max(max_pos, pos[max_mex].front());
        max_mex++;
    }
    while (max_mex > 0) {
        b.push_back(max_mex);
        while (current_pos <= max_pos) {
            int& val = a[current_pos];
            pos[val].pop_front();
            if (pos[val].empty()) max_mex = min(max_mex, val);
            current_pos++;
        }
        for (int i = 0; i < max_mex; i++) max_pos = max(max_pos, pos[i].front());
    }
