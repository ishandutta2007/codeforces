...
_ n, m, t[maxn], a[maxn], b[maxn], c[maxn], s, fc[maxn], iv[maxn], ts = 1;
...
inline void AC() {
    ...
    for(_ i = 1; i <= 200000; ++i) ts = ts*iv[c[i]]%mod;
    rd(b, m);
    for(_ i = 1; i <= min(n, m); ++i) {
        _ cr = query(n < m && i == n ? b[i] : b[i]-1);
        s = (s+cr*fc[n-i]%mod*ts)%mod;
        if(!c[b[i]]) break;
        ts = ts*fc[c[b[i]]]%mod;
        --c[b[i]];
        ts = ts*iv[c[b[i]]]%mod;
