...
fun main(){
    solve.cases{
        ...
        val atleast = BinarySearchFirstTrue(1,5000000){
            ask(it) == 1
        }!!

        var ret = atleast
        for(wantlines in 2..n){
            val potential = (atleast) - (wantlines -1)
            val moddedpotential = ((potential) divceil wantlines)

            val got = ask(moddedpotential)
            if(got != 0){
                ret = minOf(ret, got * moddedpotential)
            }
        }
        println("! $ret")
        System.out.flush()
