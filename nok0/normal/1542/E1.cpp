...
void main_() {
	...
	//fxpk
	//not f 
	vector memo(n + 1, vector(20000, vector(2, -1)));
	auto solve = [&](auto solve, int x, int k, bool f) -> ll {
		if(x <= n and 0 <= k + geta and k + geta < 20000 and memo[x][k + geta][f] != -1) return memo[x][k + geta][f];
		if(x == 0) {
			return memo[x][k + geta][f] = k <= 0;
		}
		ll res = 0;
		if(f) {
			res += solve(solve, x - 1, k, f) * x;
			res %= mod;
			REP(i, x) {
				REP(j, i + 1, x) {
					res += solve(solve, x - 1, k + j - i, 0);
					res %= mod;
				}
			}
		} else {
		...
		return memo[x][k + geta][f] = res;
	};
	print(solve(solve, n, 1, 1) % mod);
}
