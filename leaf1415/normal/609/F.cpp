...
ll n, Q;
ll x[200005], t[200005], ans[200005];
ll X[200005], invX[200005];
SegTree seg(18);
...
int main(void)
{
	...
	seg.init();
	rep(i, 1, n) seg.update(X[i], x[i]+t[i]);
	
	//rep(j, 0, n-1) cout << seg.query(j, j) << " "; cout << endl;
	
	ll p, b; multiset<P> S;
	rep(i, 1, Q){
		cin >> p >> b;
		ll res = seg.lower_bound(p), id = invX[res];
		//outl(p, b, res, id);
		if(x[id] > p){
			S.insert(P(p, b));
			continue;
		...
		t[id] += b, ans[id]++;
		while(sz(S)){
			auto it = S.lower_bound(P(x[id], -inf));
			if(it != S.end() && it->first <= x[id]+t[id]){
				t[id] += it->second, ans[id]++;
				S.erase(it);
			}
			else break;
		}
		seg.update(X[id], x[id]+t[id]);
		
		//rep(j, 0, n-1) cout << seg.query(j, j) << " "; cout << endl;
	}
	rep(i, 1, n-1) cout << ans[i] << " " << t[i] << "\n";
