...
void solve() {
    ...
    vll one_prefix(1);
    for (int i : one)
        one_prefix.push_back(one_prefix.back() + i);
    vll two_prefix(1);
    for (int i : two)
        two_prefix.push_back(two_prefix.back() + i);

    int ans = 1e9;
    for (int i = 0; i < sz(one_prefix); i++) {
        ll remaining = m - one_prefix[i];
        if (two_prefix.back() < remaining)
            continue;
        int j = lower_bound(all(two_prefix), remaining) - two_prefix.begin();
        ckmin(ans, i + 2 * j);
    }
