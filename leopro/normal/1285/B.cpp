...
void solve() {
    ...
    int m = min(*min_element(p.begin(), p.end()), 0ll);
    if (ma - m == sum){
        if ((find(p.rbegin(), p.rend(),m) == p.rend()) && find(p.begin(), p.end(),ma) == p.end() - 1) answer("YES"); else answer("NO");
    }
    if (!(ma - m > sum)) answer("YES"); else answer("NO");
}