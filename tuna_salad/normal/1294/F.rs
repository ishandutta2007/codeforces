...
fn bfs(start: usize, e: &[Vec<usize>]) -> (usize,Vec<i32>) {
	let n = e.len();
	let mut dist = v!([n] = n as i32);
...
fn main() {
	...
	let a = bfs(0, &e).0;
	let (b,a_dist) = bfs(a, &e);
	let b_dist = bfs(b, &e).1;
	let mut sol = -1;
	let mut sol_v = 0;
	for u in 0..n {
		if u != a && u != b {
			let len = (a_dist[u] + b_dist[u] - a_dist[b]) / 2;
			if max!(sol, len) {
				sol_v = u;
			}
		}
	}
	writeln!(rout, "{}", sol + a_dist[b]).ok();
	writeln!(rout, "{} {} {}", a+1, b+1, sol_v+1).ok();
}