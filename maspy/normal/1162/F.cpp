...
void solve() {
  ...
  FOR3(M, 1, N) {
    ...
    bool ok = [&]() {
      if (A[N + M - 1] == 0) return false;

      FOR(v, N) if (A[v] != A[N - 1 - v]) return false;
      FOR3(v, N, N + M) if (A[v] != A[N + N + M - 1 - v]) return false;
      FOR(i, N) if (S[i] != '?') {
        if (A[i] ^ A[i + N] != S[i] - '0') return false;
      }
      return true;
    }();
    if (ok) ANS += mint(2).pow(free);
  }
  print(ANS);
}
