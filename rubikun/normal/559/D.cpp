...
ll Pick(Polygon &P){
    ...
    for(int i=0;i<si(P);i++){
        int t=i+1;
        if(t>=si(P)) t-=si(P);
        if((P[i].id+1)%N==P[(i+1)%si(P)].id){
            ll g=sv[P[i].id];
            rm+=g;
        }else{
            Vector v=P[(i+1)%si(P)]-P[i];
            v.x=abs(v.x);
            v.y=abs(v.y);
            ll g=gcd(v.x,v.y);
            rm+=g;
...
int main(){
    ...
    vector<ld> po(K+1);
    for(int i=2;i<=K;i++) po[i]=powl(2,-i-1);
    
    for(int i=0;i<N;i++){
        Polygon Q;
        for(int k=i;k<=i+1;k++) Q.push_back(P[k%N]);
        for(int j=i+2;j<=i+K;j++){
            ld p=(po[j-i]-x)/(1-x-(ld)(N)*x-(ld)(N)*(N-1)/2*x);
            Q.push_back(P[j%N]);
            ans-=p*Pick(Q);
            Vector v=P[j%N]-P[i];
            v.x=abs(v.x);
