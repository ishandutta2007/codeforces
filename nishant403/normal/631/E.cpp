...
signed main()
{
    ...
    f(i,n)
    {
        res = max(res,sum + go.getBest(a[i]) - (a[i]*(i+1)) - suf[i]);
        go.addLine(i+2,suf[i+1]);
    }
   
    go2.addLine(n,-pref[n-1]);
    
    for(int i=n-1;i>=0;i--)
    {
        res = max(res,sum + go2.getBest(a[i]) - (a[i]*(i+1)) + pref[i]);
        if(i > 0) go2.addLine(i,-pref[i-1]);
    }
