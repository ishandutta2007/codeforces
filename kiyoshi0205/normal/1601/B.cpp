...
vector<ll> dijkstra(pmat& g,ll first){
  ...
  while(!que.empty()){
    ...
    for(auto next:g[x.second]){
      next.first+=x.first;
      if(!chmin(dist[next.second],next.first+1))continue;
      bf[next.second]=x.second;
      que.emplace(next);
    }
  }
  return dist;
...
// int codeforces;cin>>codeforces;while(codeforces--){
  ...
  rep1(i,N)cin>>b[i];
  rep(i,N){
    g[i].emplace_back(0,N+i+b[i]);
    g[i+N].emplace_back(1,i-a[i]);
    if(i)g[i-1].emplace_back(0,i);
  }
  auto dist=dijkstra(g,N*2-1);
  if(dist[0]==inf){
    print(-1);
    return 0;
  }
  ...
  while(i!=N*2-1){
    debug(i,dist[i]);
    if(i>N)keiro.emplace_back(bf[i]);
    i=bf[i];
  }
  reverse(all(keiro));
  output(keiro);
