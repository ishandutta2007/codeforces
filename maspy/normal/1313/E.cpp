...
void solve() {
  ...
  for (auto&& a: A) chmin(a, M - 1);
  for (auto&& b: B) chmin(b, M - 1);

  FenwickTree<Group_Add<ll>> bitc(M);
  FenwickTree<Group_Add<ll>> bits(M);
  ll ANS = 0;
  /*
  FOR(i, N) FOR(j, N) if (j - M + 1 < i && i <= j) {
    ANS += max(0LL, A[i] + B[j] - M + 1);
  }
  */
  FOR(j, N) {
    bitc.add(A[j], 1);
    ...
    if (j - M + 1 >= 0) {
      bitc.add(A[j - M + 1], -1);
      bits.add(A[j - M + 1], -A[j - M + 1]);
    }
    ll lo = M - 1 - B[j];
    chmax(lo, 0);
    ll cnt = bitc.sum_all() - bitc.sum(lo);
    ll sum = bits.sum_all() - bits.sum(lo);
    ANS += sum + cnt * (B[j] - M + 1);
  }
