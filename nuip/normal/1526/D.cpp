...
ll solve(const string &s){
	...
	do{
		...
		rep(i,4) toi[toc[i]]=i;
		vv<int> vs(4,{-1});
		rrep(i,s.size()) vs[toi[s[i]]].eb(i);
		auto t=s;
		sort(all(t),[&](cauto &a,cauto &b){ return toi[a]<toi[b];});
		MX(mx,make_pair(get(s,t),t));
	}while(next_permutation(all(toc)));
	cauto re=mx.Y;
	cout<<re NL;
	assert(multiset<char>(all(s))==multiset<char>(all(re)));
	return get(re,s);
}
...
int main(){
	...
	if(0){
		rep(a,8)rep(b,8)rep(c,8)rep(d,8){
			// if(a+b+c+d>10) continue;
			string s=string(a,'A')+string(b,'N')+string(c,'T')+string(d,'O');
			rep(_,10){
