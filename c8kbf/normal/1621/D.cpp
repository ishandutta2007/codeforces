...
_ n, a[maxn][maxn], b[maxn][maxn], c[maxn][maxn], d[maxn][maxn], p[maxn][maxn], q[maxn][maxn];

inline void AC() {
    
    n = read();
    for(_ i : rg(n*2)) for(_ j : rg(n*2)) {
        if(i <= n && j <= n) a[i][j] = read();
        ef(i <= n && j > n) b[i][j-n] = read();
        ef(i > n && j <= n) c[i-n][j] = read();
        else d[i-n][j-n] = read();
    }
    _ s = 0, cr = 1E17;
    for(_ i : rg(n)) for(_ j : rg(n)) s += d[i][j];
    cr = min(cr, c[1][1]);
    cr = min(cr, c[1][n]);
    cr = min(cr, c[n][1]);
    cr = min(cr, c[n][n]);
