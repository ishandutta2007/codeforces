...
unsigned long long t[c][2], po[c], valt=347;
...
int main()
{
    ...
    for (int i=1; i<=n; i++) {
        ...
        for (auto s:js) {
            ...
            while (pos<bs.size() && sm2(bs[pos], s)) {
                /*
                if (sm(bs[pos], s)!=sm2(bs[pos], s)) {
                    cout << "baj\n";
                    cout << bal << " " << bs[pos] << "\n";
                    cout << jobb << " " << s << "\n";
                    cout << "igazi " << sm(bs[pos], s) << "\n";
                    return 0;
                }
                */
                sum+=regi[bs[pos]];
...
int main()
{
    ...
    po[0]=1;
    for (int i=0; i<n; i++) {
        t[i+1][0]=t[i][0]*valt+(a[i]-'a');
        t[i+1][1]=t[i][1]*valt+(b[i]-'a');
        po[i+1]=po[i]*valt;
    }
    for (int i=0; i<n; i++) {
        for (int j=0; j<n; j++) {
            for (int k=0; k+max(i, j)<=n; k++) {
                bool ua=(a.substr(i, k)==b.substr(j, k));
                bool ha=(calc(i, i+k, 0)==calc(j, j+k, 1));
            }
        }
    }
    return 0;
