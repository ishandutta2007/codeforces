...
ll ans;
pair<int,bool> v[400000];
pair<priority_queue<int,vector<int>,greater<int>>,priority_queue<int>> ufdata[400000];
void merge(int x,int y){
  assert(x!=y);
  auto&[px,mx]=ufdata[x];
  auto&[py,my]=ufdata[y];
  if(px.size()+mx.size()<py.size()+my.size()){
    merge(y,x);
    return;
  }
  par[y]=x;
  while(!py.empty()){
    px.emplace(py.top());py.pop();
  }
  while(!my.empty()){
    mx.emplace(my.top());my.pop();
  }
  if(px.size()&&mx.size()){
    while(px.top()<mx.top()){
      auto a=px.top();
...
int main(){
  ...
  rep(i,N+M){
    if(v[i].second){
      ufdata[i].first.emplace(v[i].first);
      ans+=v[i].first;
    }else ufdata[i].second.emplace(v[i].first);
  }
  vector<pair<int,int>> query(K),sabunlist(N+M-1);
  vector<ll> pr(K);
  rep(i,K){
    cin>>query[i].first;
    query[i].second=i;
  ...
  vsort(query);
  rep(i,N+M-1)sabunlist[i]=pair<int,int>(v[i+1].first-v[i].first,i);
  vsort(sabunlist);
  j=0;
  for(auto[k,id]:query){
    while(j<N+M-1&&sabunlist[j].first<=k){
      merge(root(sabunlist[j].second),root(sabunlist[j].second+1));
      ++j;
    }
    pr[id]=ans;
  }
  for(auto x:pr)print(x);
}