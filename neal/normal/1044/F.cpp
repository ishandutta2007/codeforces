...
struct segment {
    // TODO: for maximums instead, change all occurrences of "min" to "max"
...
min_count_seg_tree tree;

// Returns the child of a that is an ancestor of b.
int child_ancestor(int a, int b) {
    assert(a != b);
...
void update_back_edge(int u, int v, int direction) {
    int u_child = child_ancestor(u, v);
...
int main() {
    ...
    for (int q = 0; q < Q; q++) {
        ...
        printf("%d\n", all.minimum == 0 ? all.count_of_min : 0);
    }
}