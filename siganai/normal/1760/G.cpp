...
int main() {
    ...
    while(TT--) {
        INT(n,a,b);
        a--,b--;
        Wgraph<int> g = winput<int>(n);
        set<int> s;
        auto dfs1 = REC([&](auto f,int now,int par,int val) -> void {
            s.insert(val);
            for(auto &nex:g[now]) {
                if(nex != par && nex != b) {
                    f(nex,now,val ^ nex.cost);
                }
            }
        });
        dfs1(a,-1,0);
        auto dfs2 = REC([&](auto f,int now,int par,int val) -> bool {
            bool flg = false;
            if(b != now && s.count(val)) flg = true;
            for(auto &nex:g[now]) {
                if(nex != par) {
                    if(f(nex,now,val ^ nex.cost)) flg = true;
                }
            }
            return flg;
        });
