...
void solve()
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        {
            if ((pref[j + w - 1] - (j ? pref[j - 1] : 0)) % 9 == i)
            {
                if (cu[i][0] == -1) cu[i][0] = j;
                else if (cu[i][1] == -1) cu[i][1] = j;
            }
        }
    }

    vector<vector<pii> > ans(9, vector<pii>(9, {n, n}));
    ...
    while (m--)
    {
        ...
        cin >> l >> r >> k;
        l--, r--;
        int md = (pref[r] - (l ? pref[l - 1] : 0)) % 9;

        cout << ans[md][k].fi + 1 << " " << ans[md][k].se + 1 << "\n";
    }
}
