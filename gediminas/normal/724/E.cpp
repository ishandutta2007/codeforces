...
int main () {
	...
	for (int i = 0; i < n; i++) {
		if (a.size() > 1) {
			ll el = *(a.end() - 1);
			a.pop_back();
			a.insert(upper_bound( a.begin(), a.end(), el), el);
		}
		...
		if ((a.empty() and liko + viso <= 0) or (!a.empty() and liko <= max(0ll, a[0] - c) - viso)) {
			debug("LMAO1");
			...
			ll t = min(viso + p[i], s[i]);
			ats += t;
			a.push_back(p[i] + viso - t);
			continue;
		}

		ats += s[i];

		if (a.empty() or liko >= a[i - 1] - 1) {
			debug("LMAO2");
			a.push_back(liko);
			continue;
		}
		...
		while (true) {
			...
			while (fr > 0 and a[fr - 1] == iki)
				fr--;

			while (to >= fr and a[to] == iki + c)
				to--;

			ll kie = to - fr + 2;
			ll ikiki = max(a[to] - c, iki - ((iki - liko) / kie) - (((iki - liko) % kie) != 0));

			if (fr > 0)
				maxi(ikiki, a[fr - 1]);

			liko += (kie - 1) * (iki - ikiki);
			...
			if (liko >= ikiki) {
				...
				for (ll l = to; l >= fr and l >= 0 and liko > ikiki; l--) {
					liko--;
					a[l]++;
				}
