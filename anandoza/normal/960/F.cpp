...
int main() {
    ...
    vector<set<pii>> dp(n); // set of {weight, count} and we need both weight, count to be strictly increasing
    for (int i = 0; i < n; i++)
        dp[i].insert({-1, 0});
    ...
    for (int i = 0; i < m; i++) {
        ...
        auto count = prev(dp[u].lower_bound({w, -1}))->second + 1;
        auto it = dp[v].insert({w, count}).first;

        if (it != dp[v].begin() && prev(it)->second >= count)
            dp[v].erase(it);
        else {
            while (next(it) != dp[v].end()) {
                if (next(it)->second <= count) {
                    dp[v].erase(next(it));
                    it = dp[v].find({w, count});
                } else
                    break;
            }
        }
