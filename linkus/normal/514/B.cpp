...
int main(){
    ...
    bool w = 0;
    FORR(i,arr){
        if(i.e1 == x) {w = 1; continue;}
        double A = (y-i.e2)/double(x-i.e1);
        double B = y-A*x;
        if(!s.count({A,B})){
            ++ans;
            s.insert({A,B});
        ...
        /* whatis(A) */
        /* whatis(B) */
            //ITENSE THOUGHT PROCESS BELOW
        /* x={3,7} y = {0,1} //f(x) = 2x+1
        A*x1+b = y1
        A*x2+b = y2 (x1+y2-x2) = y1
        A*3+b = 7
        A*0+b = 6
        (y1-y2)/(x1-x2)
        */
        /* FORR(j,arr){ */
        /*     if(j.e1 == bad) continue; */
        /*     if(fabs(A*j.e1+B-j.e2) < 0.000001) //fucking (((precision))) */
        /*         j.e1 = bad; */
        /* } */
        /* ++ans; */
    }
