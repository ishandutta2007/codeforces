// Lenient Vertex Cover (https://codeforces.com/contest/1680/problem/F)
...
int main() {
    ...
    while (T--) {
        ...
        int num_vio = 0;
        vector col(n, -1), time(n, 0);
        vector vio(n, 0), norm(n, 0);
        ...
        auto dfs1 = [&](int v, int p, auto&& self) -> void {
            ...
            trav(v2, adj[v]) {
                if (v2 == p)
                    continue;
                if (col[v2] == -1)
                    col[v2] = 1 - col[v], tree[v].push_back(v2), self(v2, v, self);
                else if (time[v] > time[v2]) {
                    if (col[v2] == col[v])
                        num_vio++, vio[v]++, vio[v2]--;
                    else
                        norm[v]++, norm[v2]--;
                }
            }
        ...
        rep(i, n)
            if (col[i] == -1)
                roots.push_back(i), col[i] = 0, dfs1(i, -1, dfs1);
