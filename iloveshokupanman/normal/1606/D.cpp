...
void solve() {
    ...
    rep(i, N)rep(j, M) {
        ...
        if (j > 0)lu[i][j] = max(lu[i][j], lu[i][j - 1]);
    }

    vvll ld(N, vll(M, 0));
    for (ll i = N - 1; i >= 0; i--)rep(j, M) {
        ld[i][j] = a[i][j];
        if (i < N - 1)ld[i][j] = min(ld[i][j], ld[i + 1][j]);
        if (j > 0)ld[i][j] = min(ld[i][j], ld[i][j - 1]);
        //cout << i << j << ld[i][j] << endl;
    }

    vvll ru(N, vll(M, 0));
    rep(i, N)for (ll j = M - 1; j >= 0; j--) {
        ru[i][j] = a[i][j];
        if (i > 0)ru[i][j] = min(ru[i][j], ru[i - 1][j]);
        if (j < M - 1)ru[i][j] = min(ru[i][j], ru[i][j + 1]);
    }
    ...
    for (ll i = N - 1; i >= 0; i--)for (ll j = M - 1; j >= 0; j--) {
        ...
        if (j < M - 1) rd[i][j] = max(rd[i][j], rd[i][j + 1]);
    }

    rep(i, N - 1)rep(j, M - 1) {
       // cout << lu[i][j] << ld[i+1][j + 1] << ru[i + 1][j] << rd[i + 1][j + 1] << endl;
