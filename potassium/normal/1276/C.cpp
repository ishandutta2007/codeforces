...
int main() {
	...
	for (int i = 0; i < n; i++) {
		int x; R(x);
		int& cnt = m[x];
		if (cnt < 650) cnt++; else discard++;
	}
	
	n -= discard;

	vector<int> cnt(650 + 7, 0);
	for (auto& i : m)
		++cnt[i.S];
	...
	int bestw = bestans/besth;

	vector<vector<int>> ans(besth, vector<int>(bestw));
	...
	sort(all(s), [](pii a, pii b) {return a.S < b.S;});

	int cc = 0, hh = 0, phase = 0, lcm = bestw*besth/__gcd(bestw,besth), now = 0;
	for (int i = 0; i < bestans; i++) {
		ans[hh][cc] = s.back().F;
		s.back().S--;
		if (s.back().S == 0) s.pop_back();
