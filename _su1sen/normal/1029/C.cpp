...
int main() {
    ...
    vec<int> lmax_l(n + 1, 0), lmax_r(n + 1, 0);
    vec<int> rmin_l(n + 1, inf), rmin_r(n + 1, inf);
    rep(i, n) {
        int l, r;
        read(l, r);
        p[i] = {l, r};
        lmax_l[i + 1] = max(lmax_l[i], p[i].first);
        rmin_l[i + 1] = min(rmin_l[i], p[i].second);
    }
    rrep(i, n) {
        lmax_r[i] = max(lmax_r[i + 1], p[i].first);
        rmin_r[i] = min(rmin_r[i + 1], p[i].second);
    }
    int ans = 0;
    rep(i, n) {
        int lmax = max(lmax_l[i], lmax_r[i + 1]);
        int rmin = min(rmin_l[i], rmin_r[i + 1]);
