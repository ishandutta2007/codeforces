...
dp_state dp0[N], dp1[N], dp2[N], dpup[N];

void dfs(int v, int p, vvi &g)
{
    dp0[v] = {0, -1, v};
    dp1[v] = {0, -1, v};
    dp2[v] = {0, -1, v};
...
void dfs2(int v, int p, vvi &g)
{
    if (p != v)
    {
        dpup[v] = dpup[p];
        if (dp0[p].len > dpup[v].len && dp0[p].dir != v)
            dpup[v] = dp0[p];
        if (dp1[p].len > dpup[v].len && dp1[p].dir != v)
            dpup[v] = dp1[p];
        dpup[v].len++;
    }
    for (auto u : g[v]) if (u != p)
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (g[i].size() == 2)
        {
            int si = dp0[i].len + dp1[i].len;
            if (si > ans)
            {
                ans = si, a = dp0[i].L, b = dp1[i].L, c = i;
            }
            si = dp0[i].len + dpup[i].len;
            if (si > ans)
            {
                ans = si, a = dp0[i].L, b = dpup[i].L, c = i;
            }
            continue;
        }
        int si = dp0[i].len + dp1[i].len + dp2[i].len;
        //cout << i << " " << si << "\n";
        if (si > ans)
        {
            ans = si, a = dp0[i].L, b = dp1[i].L, c = dp2[i].L;
        }
        si = dp0[i].len + dp1[i].len + dpup[i].len;
        //cout << i << " " << si << "\n";
        if (si > ans)
        {
            ans = si, a = dp0[i].L, b = dp1[i].L, c = dpup[i].L;
        }
    }
    cout << ans << "\n" << a + 1 << " " << b + 1 << " " << c + 1 << "\n";
}
