...
using Mat = kactl::Matrix<Mint, 128>;
...
int main() {
  ...
  for (int h : Rep(1, 8)) {
    ...
    if (w == 0) continue;
    {
      dp.insert(begin(dp), (1 << h) - size(dp), 0);
      assert(int(size(dp)) == 1 << h);
      vector<Mint> ndp(1 << h);
      for (int ve : Rep(1 << h)) {
        if (dp[ve] == 0) continue;
        for (int ho : Rep(1 << (h - 1))) {
          ho <<= 1;
          ho |= 1 << 0;
          ho |= 1 << h;
          int mask = 0;
          for (int i : Rep(h))
            if (ve >> i & 1 and ho >> i & 1 and ho >> (i + 1) & 1)
              mask |= 1 << i;
          ndp[mask] += dp[ve];
        }
      }
    ...
    for (int i : Rep(1 << h))
      for (int j : Rep(1 << h)) {
        ...
        for (int ho : Rep(1 << (h - 1))) {
          ...
          bool ok = true;
          for (int k : Rep(h))
            if (i >> k & 1 and j >> k & 1 and ho >> k & 1 and
                ho >> (k + 1) & 1) {
              ok = false;
              break;
            }
