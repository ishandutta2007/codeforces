...
void solve() {
    ...
    auto dfs = [&](auto self, int p, int sum, bool f) {
        if (p == n) {
            if (sum == 0 && f) return true;
            return false;
        }
        if (self(self, p + 1, sum, f)) return true;
        if (self(self, p + 1, sum + a[p], true)) return true;
        if (self(self, p + 1, sum - a[p], true)) return true;

        return false;
    };

    if (dfs(dfs, 0, 0, false)) {
        pr.writeln("YES");
    } else {
        pr.writeln("NO");
    }
