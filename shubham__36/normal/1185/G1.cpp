			...
			forn(j,sz){
				forn(k,sz){
					if(tot-j-k>=0 && tot-j-k<=sz-1){
						if(j>0) dp[j][k][tot-j-k][0]+=dp[j-1][k][tot-j-k][1]+dp[j-1][k][tot-j-k][2];
						if(k>0) dp[j][k][tot-j-k][1]+=dp[j][k-1][tot-j-k][0]+dp[j][k-1][tot-j-k][2];
						if(tot-j-k>0) dp[j][k][tot-j-k][2]+=dp[j][k][tot-j-k-1][1]+dp[j][k][tot-j-k-1][0];
