...
int main(){
	...
	TT{
		...
		rep(_,m){
			cauto a=read(1);
			cauto b=read(1);
			(read<string>()[0]=='i'?diff:same).eb(a,b);
		}
		UF uf(n);
		for(cauto &[a,b]:same) uf(a,b);
		UF bi(n+n);
		for(cauto &[a0,b0]:diff){
			cauto a=uf[a0];
			cauto b=uf[b0];
			bi(a,b+n);
			bi(a+n,b);
		...
		int ok=1;
		rep(i,n)if(bi[{i,i+n}]) ok=0;
		if(!ok){
			cout<<-1 NL;
			continue;
		}
		vector<int> cnt0(n+n),cnt1(n+n);
		rep(i,n) ++cnt0[bi[uf[i]]];
		rep(i,n) ++cnt1[bi[uf[i]+n]];
		vector<int> usd(n+n);
		int re=0;
		out(cnt0,cnt1,1);
		rep(j,2)rep(i,n){
			cauto v=bi[uf[i]+j*n];
			if(!MX(usd[v],1)) continue;
			out(v,1);
			re+=max(cnt0[v],cnt1[v]);
		}
		cout<<re/2 NL;
	}
	return 0;
