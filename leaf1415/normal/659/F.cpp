...
ll rem;
void dfs(int x, int y, int d, int r)
{
	if(rem > 0){
		ans[x][y] = d;
		rem--;
...
int main(void)
{
	...
	rep(y, 0, h-1) rep(x, 0, w-1){
		cin >> a[x][y];
		if(a[x][y]){
			Q.push(T(a[x][y], P(x, y)));
			mp[a[x][y]].push_back(P(x, y));
		}
	...
	rep(i, 1, h*w){
		...
		while(sz(Q) && Q.top().first >= d){
			ll x = Q.top().second.first, y = Q.top().second.second, v = y*w+x;
			Q.pop();
			rep(j, 0, 3){
				ll nx = x + dx[j], ny = y + dy[j], nv = ny*w+nx;
				if(nx < 0 || nx >= w || ny < 0 || ny >= h) continue;
				if(a[nx][ny] >= d) uf.unite(v, nv);
			}
		}
		vector<P> &vec = mp[d];
		for(auto p : vec){
			ll v = p.second * w + p.first;
			if(uf.v[uf.root(v)] >= i){
				rem = i;
				dfs(p.first, p.second, d, uf.root(v));
				yes();
				rep(y, 0, h-1){
					rep(x, 0, w-1) cout << ans[x][y] << " ";
					cout << endl;
				}
				return 0;
			}
