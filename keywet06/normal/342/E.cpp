...
void precalc() {
    for (int i = 2; i <= euler_sz; ++i) logar[i] = 1 + logar[i >> 1];
    for (int i = 1; i <= euler_sz; ++i) dp[0][i] = euler[i];
...
int lca(int a, int b) {
    ...
    if (b < a) swap(a, b);
    int y = logar[b - a + 1];
    if (h[dp[y][a]] < h[dp[y][b - (1 << y) + 1]]) return dp[y][a];
    return dp[y][b - (1 << y) + 1];
}
bool red[N];
int dist[N];
...
int main() {
    ...
    for (int i = 1; i <= m; ++i) {
        ...
        if (type == 1) {
            ...
            if (buffer.size() > RAD) dischargeBuffer();
        } else {
            int best = dist[node];
            for (auto it : buffer) {
