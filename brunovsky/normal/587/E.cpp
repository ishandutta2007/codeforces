...
struct integer_xor_basis {
    ...
    void merge(const integer_xor_basis<T>& other) {
        for (auto x : other.basis) {
            operate(x, true);
        }
    }
  ...
  private:
    ...
    auto operate(T vec, bool add_basis) {
        for (int i = 0, B = basis_size(); i < B && vec; i++) {
...
using Basis = integer_xor_basis<uint32_t>;
...
struct Segxor {
    ...
    uint32_t value = 0, lazy = 0;

    Segxor(uint32_t value = 0) : value(value) {}

    void pushup(const Segxor&, const Segxor&) {}

    void pushdown(Segxor& lhs, Segxor& rhs) {
        if (lazy) {
            lhs.apply(lazy);
            rhs.apply(lazy);
            lazy = 0;
...
struct Segbasis {
    ...
    Segbasis() = default;
    Segbasis(uint32_t v) { basis.insert_check(v); }

    auto size() const {
        uint32_t s = basis.basis_size();
        return 1 << s;
    }

    void pushup(const Segbasis& lhs, const Segbasis& rhs) {
        if (lhs.basis.basis_size() > rhs.basis.basis_size()) {
            basis = lhs.basis, basis.merge(rhs.basis);
        } else {
            basis = rhs.basis, basis.merge(lhs.basis);
        }
    }

    void pushdown(Segbasis& lhs, Segbasis& rhs) {
        if (lazy) {
            lhs.apply(lazy);
            rhs.apply(lazy);
            lazy = 0;
    ...
    void apply(uint32_t y) {
        basis.clear();
        basis.insert_check(y);
        lazy ^= y;
    }
};
...
int main() {
    ...
    segtree<Segxor> segxor(N + 1, a);
    segtree<Segbasis> segbasis(N + 1, b);
    ...
    while (Q--) {
        if (int t; cin >> t, t == 1) {
            ...
            b[l] ^= k;
            b[r] ^= k;
            segxor.update_range(l, r, k);
            segbasis.update_point(l, b[l]);
            segbasis.update_point(r, b[r]);
        } else if (t == 2) {
            int l, r;
            cin >> l >> r, l--;
            auto x = segxor.query_point(l).value;
            if (l + 1 == r) {
                cout << (x ? 2 : 1) << '\n';
            } else {
                auto node = segbasis.query_range(l + 1, r).basis;
                node.insert_check(x);
                auto s = 1u << node.basis_size();
                cout << s << '\n';
            }
        } else {
            assert(false);
