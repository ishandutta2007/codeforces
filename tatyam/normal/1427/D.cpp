...
int main(){
    ...
    each(i,a)i--;
    vec(vector<ll>,ans,0);
    auto query=[&](vector<ll>q){
        q.erase(remove_if(all(q),logical_not()),q.end());
        if(q.size()==1)return;
        ans.push_back(q);
        auto end=a.end();
        q.pop_back();
        each(i,q){
            rotate(a.begin(),a.begin()+i,end);
            end-=i;
        }
    };
    rep(n-1){
        ll next=find(all(a),(a.back()+1)%n)-a.begin();
        ll sz=1;
        while(next+sz<n&&a[next+sz]==(a[next+sz-1]+1)%n)sz++;
        query({next,sz,n-next-sz});
    }
    ll next=find(all(a),0)-a.begin();
    query({next,n-next});
    assert(is_sorted(all(a)));
    out(ans.size());
    each(i,ans){
        i.insert(i.begin(),i.size());
        out(i);
    }
}