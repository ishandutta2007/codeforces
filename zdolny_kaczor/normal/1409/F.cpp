...
void solve() {
	...
	if (t[0] == t[1]) {
		int match = 0;
		REP(i, n) if (s[i] == t[0]) match++;
		int reach = min(n, match + k);
		printf("%d\n", reach * (reach - 1) / 2);
		return;
	}
	char a = t[0], b = t[1];
	...
	REP(i, n) {
		char here = s[i];
		REP(did, n + 1) REP(aoc, n + 1) {
			maxi(dp[i + 1][did + (here != a)][aoc + 1], dp[i][did][aoc]);
			maxi(dp[i + 1][did + (here != b)][aoc], dp[i][did][aoc] + aoc);
			maxi(dp[i + 1][did][aoc], dp[i][did][aoc]);
		}
		REP(did, k + 1) REP(aoc, n + 1)
			maxi(ans, dp[i + 1][did][aoc]);
	}
	printf("%d\n", ans);
}
int main() {
