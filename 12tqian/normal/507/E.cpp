...
template <class C, bool directed> struct Dijkstra {
	...
	void gen(int st) {
		dist = std::vector<C>(SZ, mp((int)1e9, 0));
		typedef std::pair<C, int> T;
		std::priority_queue<T, std::vector<T>, std::greater<T>> pq;
		auto ad = [&](int a, C b, int from) {
			if (dist[a] <= b) return;
			bef[a] = from;
			pq.push({dist[a] = b, a});
		};
		...
		while (!pq.empty()) {
			...
			for (auto &y: adj[x.second]) {
				ad(y.first, cmb(x.first, y.second), x.s);
			}
		}
	}
};
...
int main() {
	...
	f0r(i, sz(ord) - 1) {
		on.insert(cp(ord[i], ord[i + 1]));
	}
	dbg(ord, dijk.dist[n - 1]);
	vector<array<int, 3>> ans;
	for (auto [u, v, z] : e) {
		if (u > v) swap(u, v);
		if (on.count({u, v}) && z == 0) {
			ans.pb({u, v, z ^ 1});
		} else if (!on.count({u, v}) && z == 1) {
			ans.pb({u, v, z ^ 1});
		}
	}
	ps(sz(ans));
...
/**
 * shortest path length z
 * maximize number of 1s on the path
 */