...
fun main(){
    solve.cases{
        val n = getint
        val L = getline(n)
        val ones = L.count{it == 1}

        val arr = IntArray(ones + 1)
        var pt = 0
        for(a in L){
            if(a == 1) {
                pt++
        ...
        val maxtime = (n * (n-1))/2
        //eaten, time
        var lastDP = Array(1 + zeros){IntArray(maxtime+1){plarge}}
        lastDP[0][0] = 0
        val squares = IntArray(100){it * it }
        for(i in arr.indices){
            val newDP = Array(1 + zeros){IntArray(maxtime+1){plarge}}
            for(z in 0..zeros){
                for(usedt in 0..maxtime){
                    val here = lastDP[z][usedt]
                    for(nz in z..zeros) {
                        val dif = nz - z
                        val nt = usedt + (nz - arrQ.sumQuery(0,i)).absoluteValue
                        val cost = here + squares[dif]
                        if(nt in 0..maxtime) {
                            newDP[nz][nt] = minOf(newDP[nz][nt], cost)
                        }
                    }
                }
            }
        ...
        var best = plarge
        for(t in 0..maxtime) {
            val here = lastDP[zeros][t]
            best = minOf(best, here )
            val actual = (zeros * zeros - best) / 2
            put(actual)
        }
...
/*
[eaten][time] phrase eaten

22 miuntes if passed
