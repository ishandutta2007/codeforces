...
namespace MAIN{
    ...
    inline void MAIN(){
        n=read();
        for(res i=1;i<=n;i++)a[i]=read(),b[a[i]]++,f[a[i]]=du[a[i]]=0,vector<int>().swap(G[a[i]]);
        he=1,ta=0;
        for(res i=1;i<=ex;i++)if(b[edge[i].fi]&&b[edge[i].se])G[edge[i].fi].pb(edge[i].se),du[edge[i].se]++;
        for(res i=1;i<=n;i++){
            if(!du[a[i]])Q[++ta]=a[i],du[a[i]]=-1;
        }
        res ans=0;
        while(he<=ta){
            res u=Q[he++];
            f[u]+=b[u],ans=max(ans,f[u]);
            for(auto tox:G[u]){
                f[tox]=max(f[tox],f[u]);
                if(!--du[tox])Q[++ta]=tox;
            }
...
//    freopen("1.out","w",stdout);
    for(res i=1;i<=N-10;i++)for(res j=i+i;j<=N-10;j+=i)edge[++ex]=mp(i,j);
    res Case=read();
