...
signed main() {
  ...
  FOR(_, T) {
    ...
    while (rest) {
      vi IDS;
      FOR(i, N) if (A[i] == 0) IDS.eb(i);
      FOR(i, N) { FORIN(j, IDS) A[i] -= abs(j - i); }
      FORIN(i, IDS) {
        A[i] = -1;
        RANK[i] = nxt;
      }
    ...
    FOR(rk, 26) {
      vi IDS;
      FOR(i, N) if (RANK[i] == rk) IDS.eb(i);
      while (CNT[nxt] < IDS.size()) --nxt;
      FORIN(i, IDS) { ANS[i] = nxt + 'a'; }
      --nxt;
    }
    print(ANS);
  }
