...
namespace MAIN{
    ...
    inline void MAIN(){
        n=read();
        for(res i=1;i<=n;i++)a[i]=read(),f[i][0]=f[i][1]=f[i][2]=0,g[i]=h[i]=0;
        g[0]=h[0]=0,g[n+1]=h[n+1]=0;
        res ans=0;
        for(res i=1;i<=n;i++){
//            printf("%d %d %d %d\n",g[a[i]],g[a[i]-2],h[a[i]],h[a[i]-2]);
            if(a[i]==0)f[i][0]++;
            if(a[i]==1)f[i][2]++;
            add(f[i][0],g[a[i]]);
            if(a[i]>=1)add(f[i][0],g[a[i]-1]);
            if(a[i]>=2)add(f[i][2],g[a[i]-2]);
            add(f[i][1],h[a[i]+1]);
            if(a[i]>=1)add(f[i][2],h[a[i]-2+1]);
            add(g[a[i]],f[i][0]),add(h[a[i]+1],f[i][1]);
            if(a[i]>=1)add(h[a[i]-2+1],f[i][2]);
