    ...
    if(n <= 8) {
        while(1) {
            ...
            rep2(i, 1, n - 1) {
                int x = rnd(i);
                int c = rnd(1, 10);
                g[x].eb(i, c), g[i].eb(x, c);
            }
            vi id = iota(n);
            rnd.shuffle(id);
            vi t(n);
            rep(i, n) for(auto e : g[i]) {
                if(id[i] < id[e]) { rep2(j, id[i], id[e]) t[j] += e.cost; }
            }
            vv(int, d, n, n, inf<int>);
            rep(i, n) d[i][i] = 0;
            rep(i, n) for(auto e : g[i]) d[id[i]][id[e]] = e.cost;
            rep(k, n) rep(i, n) rep(j, n) chmin(d[i][j], d[i][k] + d[k][j]);
            int cnt = 0;
            vi r(n + 1);
            rep(i, n) r[i + 1] = r[i] + t[i];
            rep(i, n) rep2(j, i + 1, n - 1) {
                if(d[i][j] == r[j + 1] - r[i]) { cnt++; }
            }
            if(cnt == n / 2) {
                rep(i, n) {
                    for(auto e : g[i]) {
                        if(id[i] < id[e]) cout << id[i] + 1 << " " << id[e] + 1 << " " << e.cost << endl;
                    }
                }
                rep(i, n) rep2(j, i + 1, n - 1) {
                    if(d[i][j] == r[j + 1] - r[i]) { cout << i + 1 << " " << j + 1 << endl; }
                }
                exit(0);
            }
        }
        ...
        int l = 3;
        while(l < m) {
            cout << m << " " << m + 1 << " " << n - m + 1 << endl;
            cout << l << " " << m << " " << 1 << endl;
            m--, l++;
        }
        if(l == m) { cout << m << " " << m + 1 << " " << 1 << endl; }
        int need = n / 2;
        rep2(i, 3, n) {
            if(need) cout << 1 << " " << i << endl, need--;
            if(need) cout << 2 << " " << i << endl, need--;
        }
    }
}