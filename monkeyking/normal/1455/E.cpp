...
//	do
//	{
//		if(max(p[0].x,p[3].x)<min(p[1].x,p[2].x)) continue;
//		if(max(p[0].y,p[1].y)<min(p[2].y,p[3].y)) continue;
//		ll s=0;
//		s+=mabs(p[2].x-p[1].x)+mabs(p[0].x-p[3].x)+mabs(p[0].y-p[1].y)+mabs(p[2].y-p[3].y);
//		// debug(p[0],p[1],p[2],p[3],s);
//		ll l1=max(0LL,min(p[0].x,p[3].x)-max(p[1].x,p[2].x));
//		ll r1=max(0LL,max(p[0].x,p[3].x)-min(p[1].x,p[2].x));
//		ll l2=max(0LL,min(p[0].y,p[1].y)-max(p[2].y,p[3].y));
//		ll r2=max(0LL,max(p[0].y,p[1].y)-min(p[2].y,p[3].y));
//		// debug(l1,r1,l2,r2);
