...
void solve()
{
	...
	{
		return ValueType{ a.val + b.val, a.cnt + b.cnt };
	};
	...
	{
		if (a.col != -1) b.col = a.col;
	};
	auto tag_spread = [](const TagType& tag, ValueType& value) -> void
	{
		if (tag.col != -1) value.val = tag.col * value.cnt;
	};
	...
	auto add = [&](int x)
	{
		if (tree.Query(x).val == 0)
		{
			tree.ModifyValue(x, { 1, 1 });
			return;
		}
		int y = tree.FindFirst(x, [](const ValueType& val)
			{
				return val.val != val.cnt;
			}) - 1;
		tree.ModifyTag(x, y, { 0 });
		tree.ModifyValue(y, { 1, 1 });
	};

	auto del = [&](int x)
	{
		if (tree.Query(x).val == 1)
		{
			tree.ModifyValue(x, { 0, 1 });
			return;
		}
		int y = tree.FindFirst(x, [](const ValueType& val)
			{
		...
		tree.ModifyTag(x, y, { 1 });
		tree.ModifyValue(y, { 0, 1 });
	};
	...
	while (q--)
	{
		...
		a[x] = y;
		add(a[x]);
		int res = tree.FindLast(N - 1, [](const ValueType& val)
			{
				return val.val != 0;
			});
		writeln(res);
