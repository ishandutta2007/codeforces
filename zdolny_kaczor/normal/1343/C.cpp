...
int a[nax];
using res_t = pair<int, ll>;
res_t append(res_t x, int i) {
	auto [len, sum] = x;
	return {len + 1, sum + a[i]};
}
res_t dp[nax];
int main() {
	int t;
	...
	REP(_, t) {
		...
		scanf("%d", &n);
		REP(i, n) scanf("%d", a + i);
		res_t pos = {0, 0}, neg = {0, 0};
		for (int i = n - 1; i >= 0; --i) {
			res_t here = append(a[i] > 0 ? neg : pos, i);
			maxi(a[i] > 0 ? pos : neg, here);
			dp[i] = here;
		}
		res_t go = max(pos, neg);
		debug(imie(go));
		printf("%lld\n", go.second);
	}
	exit(0);
