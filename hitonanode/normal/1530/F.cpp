...
constexpr unsigned MOD = 31607;
using mint = ModInt<MOD>;

unsigned prod(unsigned x, unsigned y) { return x * y % MOD; }
...
int main() {
    ...
    vector Ainv(N, vector<mint>(N));
    REP(i, N) REP(j, N) Ainv[i][j] = 1 / A[i][j];

    dbg(A);
    dbg(Ainv);
    vector<mint> row_prod(N, 1);
    REP(i, N) REP(j, N) row_prod[i] *= A[i][j];
    mint ret = 0;
    ...
    REP(S, 1 << N) {
        ...
        int r = 0;
        REP(i, N) {
            if ((S >> i) & 1) pinit *= row_prod[i], r++;
        }

        vector<mint> col_prod(N, 1);
        REP(i, N) {
            if ((S >> i) & 1) continue;
            REP(j, N) col_prod[j] *= A[i][j];
        }
        dbg(col_prod);
        ...
        REP(c1, 2) REP(c2, 2) {
            ...
            REP(i, N) {
                if ((S >> i) & 1) continue;
                if (c1) tmp *= A[i][i], cp[i] *= Ainv[i][i];
                if (c2) {
                    if (c1 and i == N - 1 - i) continue;
                    tmp *= A[i][N - 1 - i], cp[N - 1 - i] *= Ainv[i][N - 1 - i];
                }
            }
            dbg(cp);
            for (auto v : cp) {
