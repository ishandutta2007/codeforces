...
int parent[(4 * 400010)], sz[(4 * 400010)], look[(4 * 400010)];
...
void merge(int u, int v) {
    u = find(u), v=find(v);
    if (u!=v) {
        if (v<=n || (v>n && u>n && sz[u]>sz[v])) swap(u, v);
        sz[v] += sz[u];
        parent[u] = v;
    }
}
...
int make_new(int h) {
    ...
    parent[u] = u;
    sz[u] = 0;
    heights.insert(pair<int, int>{h, u});
    return u;
}
...
int main() {
    ...
    while (Q--) {
        ...
        if (t==1) {
            ...
            auto w = heights.lower_bound(pair<int, int>{l, -1});

            int ua=-1, ub=-1;
            while (w!=heights.end() && w->first<=r) {
                auto __p = *w; auto h = __p.first; auto u = __p.second;
                to_kill.push_back(*w);

                int nh, c;
                if ((r+1)-h < h-(l-1)) {
                    nh=r+1;
                    if (ua==-1) ua = make_new(nh);
                    c = ua;
                }
                else {
                    nh=l-1;
                    if (ub==-1) ub = make_new(nh);
                    c = ub;
                }
            ...
            if (ua!=-1) look[find(ua)] = r+1;
            if (ub!=-1) look[find(ub)] = l-1;
