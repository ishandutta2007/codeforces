...
bool rec(int i, int j, int len, const SuffixArray &sa, const vector<vector<bool> > &dp, int &remK, pii &ans) {
	if(len == sa.length() + 1) return false;
	int l = i;
	for(int k = i; ; ++ k) {
		if(k != i && (k == j || sa.lcpArray[k-1] < len)) {
			if(rec(l, k, len+1, sa, dp, remK, ans)) return true;
			l = k;
		}
		if(k == j) break;
		int a = sa.suffixArray[k], b = a + len;
		if(b < (int)dp.size() && dp[a][b]) {
			if((remK --) == 0) {
				ans = mp(a, b);
				return true;
			}
...
int main() {
	...
	while(cin >> s) {
		...
		rer(i, 0, n) dp[i][i] = true;
		rep(i, n) dp[i][i+1] = true;
		rer(i, 0, n-2) dp[i][i+2] = s[i] == s[i+1];
		rer(i, 0, n-3) dp[i][i+3] = s[i] == s[i+2];
		rer(len, 4, n) {
			rer(i, 0, n - len) {
				int j = i + len;
				dp[i][j] = s[i] == s[j-1] && dp[i+2][j-2];
			}
		}
		SuffixArray sa; sa.buildAll(s.c_str(), n);
		int remK = k; pii ans;
		bool ok = rec(1, n+1, 1, sa, dp, remK, ans);
		assert(ok);
