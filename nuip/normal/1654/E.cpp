...
using MAP=gp_hash_table<int, int>;

cauto SQ=200;
cauto BK=100000/SQ+5;
...
int main(){
	...
	reps(d,-SQ,SQ+1){
		MAP cnt;
		rep(i,n) ++cnt[vs[i]+d*i];
		for(cauto &p:cnt) MX(re,p.Y);
	}
	out(re,1);
	vector<MAP> dp(n);
	rep(i,n)reps(j,1,BK){
		...
		if(d%j) continue;
		d/=j;
		if(-SQ<=d && d<=SQ) continue;
		cauto it=dp[i-j].find(d);
		auto &x=dp[i][d];
		MX(x, (it==dp[i-j].end()?1:it->Y) + 1);
		MX(re, x);
	}
	cout<<n-re NL;
	return 0;
