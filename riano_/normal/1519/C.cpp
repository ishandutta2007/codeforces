...
int main(){
    ...
    rep(ii,T){
        ...
        rep(i,N) cin >> s[i];
        rep(i,N){
            uni[u[i]].emplace_back(s[i]);
        }
        vector<ll> sum(N+1); vector<ll> sz(N+1);
        set<ll> rem;
        rep(i,N+1){
            uni[i].emplace_back(0);
            sort(all(uni[i]));
            sz[i] = uni[i].size()-1;
            rep(j,sz[i]+1) sum[i] += uni[i][j]; 
            rep(j,sz[i]) uni[i][j+1] += uni[i][j];
            if(sz[i]>0) rem.insert(i);
        }
        vector<ll> b;
        for(int k=1;k<=N;k++){
            ans = 0;
            ...
            for(ll x:rem){
                ll r = sz[x]%k;
                ans += sum[x]-uni[x][r];
                if(sz[x]==k) er.emplace_back(x);
            }
            for(ll x:er) rem.erase(x);
            b.emplace_back(ans);
        }
        out(b);
    }    
