...
struct SegmentTree {
  ...
  SegmentTree(int n, const F _f, const Monoid &_M1) : f(_f), M1(_M1) {
    sz = 1;
    while(sz < n) sz <<= 1;
    seg.assign(2 * sz, M1);
  }
...
// RMQ
#define RMQ(n) SegmentTree<int>(n,[](int x,int y){return max(x,y);},0)

signed main(){
    ios_base::sync_with_stdio(0),cin.tie(0),cout.tie(0);cout<<fixed<<setprecision(15);
    int n=in();
    vec a;rep(i,n)a.pb(in()-i);
    map<int,int> mp;
    rep(i,n)mp[a[i]]=0;
    int num=0;
    vec rev(n);
    ...
    vecll ans(n);
    {
        auto seg = RMQ(n+1);
        rep3(i,n-1,0){
            int pre = (i ? mp[a[i-1]] : -1);
            int tmp;
            if( pre <= 0 ){
                tmp = seg.query(0,n+1);
            }
            else {
                tmp = seg.query(pre-(rev[pre-1]==rev[pre]-1),n+1);
            }
            ans[i] += tmp;
            seg.update(mp[a[i]],1+max(0,seg.query(mp[a[i]],n+1)));
        }
    }
    {
        auto seg = RMQ(n+1);
        rep(i,n-1){
            seg.update(mp[a[i]],1+seg.query(0,mp[a[i]]+1));
            ans[i+1] += seg[mp[a[i]]];
        }
    }
    cout << n-1-*max_element(all(ans)) << endl;
