...
ll modpow(ll x,ll n,ll mod){
    if(n==0) return 1;
    ll res=modpow((x*x>mod? (x*x%mod)+mod:x*x),n/2,mod);
    if(n&1) res=(res*x > mod? res*x%mod+mod:res*x);
    return res;
}
signed main(){
    ...
    while(q--){
        ...
        else now = (w[r-1]>mods[r-l-1] ? w[r-1]%mods[r-l-1]+mods[r-l-1] : w[r-1]);
        rep3(i,r-2,l){
            now = modpow(w[i],now,mods[i-l]);
        }
        cout << now%m << endl;
    }
}