...
int main() {
    ...
    while (t --> 0) {
        ...
        vec<int> ids(n, 0);
        rep(i, c) {
            for (int v : groups[i]) ids[v] = i;
        }
        vec<int> dp0(c, n), dp1(c, n);
        reverse(all(groups));
        for (auto &group : groups) {
            ...
            for (int u : group) {
                chmin(dp0[id], d[u]);
                for (int v : g[u]) {
                    if (d[u] < d[v]) chmin(dp0[id], dp0[ids[v]]);
                }
            }
        }
        for (auto &group : groups) {
            ...
            for (int u : group) {
                for (int v : g[u]) {
                    if (d[u] < d[v]) {
                        chmin(dp1[id], dp1[ids[v]]);
                    } else {
                        chmin(dp1[id], dp0[ids[v]]);
                    }
                }
            }
        }
