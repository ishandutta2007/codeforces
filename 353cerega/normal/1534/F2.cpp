...
int main()
{
    ...
    for (int i=0;i<n;i++)
    {
        for (int v: g[i])
        {
            if (col[i]!=col[v])
            {
                ...
                g2r[col[v]].push_back(col[i]);
            }
        }
    }
    for (int i=0;i<S;i++) d[i] = g2r[i].size();
    vector<int> q;
    vector<int> was(S);
    for (int i=0;i<S;i++)
    {
    ...
    for (int i=0;i<S;i++) d[i] = g2[i].size();
    q.clear();
    for (int i=0;i<S;i++)
    {
        if (d[i]==0) q.push_back(i);
    ...
    for (int i=0;i<S;i++)
    {
        if (L[i]!=-1 and kek[i]==1)
        {
            ord.push_back({{L[i],R[i]},i});
        }
    }
    ...
    {
        if (i+1<ord.size() and p>=ord[i+1].X.X) continue;
        ans++;
        auto it = lft.upper_bound(ord[i].X.Y);
        if (it==lft.end()) break;
        p = *it;
    }
    cout << ans << endl;
