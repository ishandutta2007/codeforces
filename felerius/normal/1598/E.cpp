// Staircases (https://codeforces.com/contest/1598/problem/E)
...
int main() {
    ...
    ll ans = 0;
    vector free(n, vector(m, true));
    vector stair_down(n, vector(m, 0)), stair_right = stair_down;
    auto update = [&](int i, int j) {
        ans -= max(0, stair_down[i][j] + stair_right[i][j] - 1);
        stair_down[i][j] = stair_right[i][j] = int(free[i][j]);
        if (free[i][j]) {
            if (i + 1 < n)
                stair_down[i][j] += stair_right[i + 1][j];
            if (j + 1 < m)
                stair_right[i][j] += stair_down[i][j + 1];
            ans += stair_down[i][j] + stair_right[i][j] - 1;
        }
    };

    repr(i, n - 1, 0) repr(j, m - 1, 0)
        update(i, j);

    while (q--) {
        int i, j; read(i, j); --i, --j;
        free[i][j] = !free[i][j];
        while (i >= 0 && j >= 0) {
            update(i, j);
            if (i)
