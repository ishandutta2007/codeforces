...
void main_() {
	...
	STR(s);
	map<pii, bool> memo;
	auto dfs = [&](auto self, int i, int wc) -> bool {
		if(memo.count({i, wc})) return memo[{i, wc}];
		if(i == n) return memo[{i, wc}] = abs(wc) == k;
		if(abs(wc) == k) return memo[{i, wc}] = false;
		if(s[i] != '?') {
			return memo[{i, wc}] = self(self, i + 1, (s[i] == 'W' ? wc + 1 : (s[i] == 'L' ? wc - 1 : wc)));
		}
		s[i] = 'D';
		auto ret = self(self, i + 1, wc);
		if(ret) return memo[{i, wc}] = ret;
		s[i] = 'W';
		ret = self(self, i + 1, wc + 1);
		if(ret) return memo[{i, wc}] = ret;
		s[i] = 'L';
		ret = self(self, i + 1, wc - 1);
		if(ret) return memo[{i, wc}] = ret;
