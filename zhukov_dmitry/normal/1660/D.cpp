...
int calc_s2(int l, int r)
{
	return s2[r + 1] - s2[l];
}

int calc_ss(int l, int r)
{
	return ss[r + 1] - ss[l];
}
...
tuple<int, int, int> solve(int l, int r)
{
	...
	For(i, l, r)
	{
		if (calc_ss(i, r) & 1) continue;
		if (uax(ans, calc_s2(i, r))) al = i, ar = n - 1 - r;
	}
	For(i, l, r)
	{
		if (calc_ss(l, i) & 1) continue;
		if (uax(ans, calc_s2(l, i))) al = l, ar = n - 1 - i;
	}
	return {ans, al, ar};
}
...
int main()
{
	...
	while (qq--)
	{
		...
		forn(i, n + 1)
		{
			if (a[i] == 0)
			{
				auto [cur, ll, rr] = solve(lst + 1, i - 1);
				if (uax(ans, cur)) al = ll, ar = rr;
				lst = i;
			}
		}
