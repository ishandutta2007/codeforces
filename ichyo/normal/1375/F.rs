...
fn run() {
    ...
    let id: usize = scan();
    let (a, b, c, diff, first) = if id == 1 {
        (a + t, b, c, 2 * (a + t) - b - c, a + t)
    } else if id == 2 {
        (a, b + t, c, 2 * (b + t) - a - c, b + t)
    } else if id == 3 {
        (a, b, c + t, 2 * (c + t) - a - b, c + t)
    } else if id == 0 {
        return;
    } else {
        unreachable!();
