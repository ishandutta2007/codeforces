...
void chemthan() {
    ...
    rmq.build();
    map<pi, vector<long long>> dp;
    function<vector<long long>(int, int)> calc = [&] (int l, int r) {
        if (r < l) {
            return vector<long long>(1);
        }
        if (l == r) {
            vector<long long> res(2);
            res[1] = (long long) (m - 1) * a[l];
            return res;
        }
        ...
        FOR(i, 0, sz(x)) {
            FOR(j, 0, sz(y)) {
                chkmax(res[i + j], x[i] + y[j] - (long long) 2 * i * j * a[m2]);
            }
        }
        return res;
    };
