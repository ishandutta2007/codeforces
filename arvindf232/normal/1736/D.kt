...
fun main(){
    solve.cases{
		...
		if(str.count { it == '1' } and 1 == 1 || str.count{it == '0'} and 1 == 1){
			put(-1)
			return@cases
		}
		...
		put(onepos.size)
		for(i in onepos.indices step 2){
			onepos[i] = onepos[i] xor 1
		}
		put(onepos.map { it + 1 }.conca())
		put((0 until n).filter { it %2 == 0 }.map { it +1 }.conca())
//		put((0 until n).filter { it % 2 == 1 }.conca())
