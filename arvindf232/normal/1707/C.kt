...
fun Graph.kruskal(): Long {
    val edgeorder = (0 until (this.edgecount shr 1 )).sortedBy { weights[it] }
    var cost = 0L
...
fun main(){
    solve.cases{
        ...
        repeat(m){
            G.addWeighted(getint - 1 , getint - 1 , it + 1 )
        }
        G.kruskal()

        val H = Graph(n,n-1,false)
        H.root = Random.nextInt(n)
        for(e in 0 until m){
            if(G.taken[e]){
        ...
        H.preparesparse()
        H.prepareLifting()
        val st = tempIntervalSum(n)
        var need = 0
        for(e in 0 until m){
            if(G.taken[e]) continue
            var a = G.from[e shl 1]
            var b = G.to[e shl 1]
            need ++
            if(isancestor(a,b) || isancestor(b,a)){
                ...
                val dd = H.calParent(b,H.depth[b] - H.depth[a] -1)
                st.add(tin[b], tout[b], 1)
                st.add(0,n-1,1)
                st.add(tin[dd],tout[dd] ,-1)
            }else{
                st.add(tin[a],tout[a], 1)
                st.add(tin[b],tout[b],1)
            }
        }
        val ans = st.compile()
