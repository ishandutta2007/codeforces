...
void M(const string& S, vector<int>& d, vector<int>& e) {
	int n = len(S);
	d.resize(n + 2, 0);
	e.resize(n + 2, 0);
	int l = NIL, r = NIL;
    ...
    for(int i = 0; i < n; ++i) {
        ...
        while(i - e[i] - 2 >= 0 && i + e[i] + 1 < n && S[i - e[i] - 2] == S[i + e[i] + 1]) {
            ++e[i];
        }
        if(i + e[i] > r) {
            l = i - e[i] - 1;
...
string slow_find(const string& T) {
	string rez;
...
string find(const string& T) {
	...
	int n = len(T), mx = NIL;
	for(int i = 0; i < n; ++i) {
		if(i - d[i] == 0) mx = max(mx, i + d[i]);
		if(i - e[i] - 1 == 0) mx = max(mx, i + e[i]);
	} 
	for(int i = 0; i <= mx; ++i) rez.push_back(T[i]);
	return rez;
}
void solve() {
	auto S = fetch<string>();
