...
lint solve() {
    ...
    REP(d, E) {
        ...
        vector<pint> runl;
        for (auto i : vs) {
            if (runl.empty()) runl.emplace_back(i, 1);
            else if (runl.back().first == i and i == 1) runl.back().second++;
            else runl.emplace_back(i, 1);
        }
        dbg(runl);
        REP(i, runl.size()) {
            auto [v, cnt] = runl[i];
            if (sieve.min_factor[v] != v or v == 1) continue;
            lint nl = 1, nr = 1;
            if (i and runl[i - 1].first == 1) nl += runl[i - 1].second;
            if (i + 1 < int(runl.size()) and runl[i + 1].first == 1) nr += runl[i + 1].second;
            ret += nl * nr - 1;
        }
        dbg(ret);
        dbg(vs);
