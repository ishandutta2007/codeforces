...
main()
{
    ...
    {
        r=n+1;
        forinc(i,1,n) if(s[i]==s[1]) l=i;else break;
        fordec(i,n,1) if(s[i]==s[1]) r=i;else break;
        forinc(i,1,min(n,l+1))
        {
            int _r=max(i,r);
            kq=(kq+(n-_r+2))%mod;
        }
    ...
    {
        forinc(i,1,n) if(s[i]==s[1]) l=i;else break;
        fordec(i,n,1) if(s[i]==s[n]) r=i;else break;
        cout<<l+(n-r+1)+1<<"\n";
    }
}