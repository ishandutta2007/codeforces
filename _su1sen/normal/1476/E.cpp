...
int main() {
    ...
    vector<int> ids(m, 0);
    rep(i, m) {
        rep(j, k) ids[i] = ids[i] * 26 + (s[i].first[j] - 'a');
    }

    int sz = 1;
    loop(k) sz *= 26;
    vector<vector<int>> matches(sz);

    rep(i, n) {
        auto dfs = [&](auto dfs, int j, string &cur, int idx) -> void {
            if (j == k) {
                matches[idx].push_back(i);
                return;
            }
            if (p[i][j] != '_') {
                cur.push_back(p[i][j]);
                dfs(dfs, j + 1, cur, idx * 26 + (p[i][j] - 'a'));
                cur.pop_back();
            } else {
                for (char c = 'a'; c <= 'z'; ++c) {
                    cur.push_back(c);
                    dfs(dfs, j + 1, cur, idx * 26 + (c - 'a'));
                    cur.pop_back();
                }
            }
        };
    ...
    rep(i, m) {
        int u = s[i].second;
        if (not binary_search(all(matches[ids[i]]), u)) {
            print("NO");
            return 0;
        }
        for (int v : matches[ids[i]]) {
            if (u == v) continue;
            g[u].push_back(v);
        }
    }

    topological_sort top(g);
    if (not top.is_dag()) {
        print("NO");
    } else {
        auto ans = top.sorted();
