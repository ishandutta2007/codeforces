...
for(int x=1;x<=n;x++)
{
    cin>>a[x];
    int now=a[x];
    while(qwq[(now-1)/1000][(now-1)%1000+1]==1)qwq[(now-1)/1000][(now-1)%1000+1]=0,bo[(now-1)/1000]--,now++;
    qwq[(now-1)/1000][(now-1)%1000+1]=1;
    bo[(now-1)/1000]++;
    ans=max(ans,now);
}
while(q--)
{
    ...
    if(a[k]!=l)
    {
        ...
        for(int x=(now-1)%1000+1;x<=1000;x++)
        if(qwq[(now-1)/1000][x]==1)qwq[(now-1)/1000][x]=0,bo[(now-1)/1000]--;
        else {qwq[(now-1)/1000][x]=1,bo[(now-1)/1000]++,ans=max(ans,(now-1)/1000*1000+x),now=-114514;break;}
        if(now!=-114514)
        {
            int fst=(now-1)/1000+1;
            for(;bo[fst]==1000;fst++)
            bo[fst]=0,tg[fst]=1;
            if(tg[(now-1)/1000]==1)
            {
            ...
            for(int x=1;x<=1000;x++)
            if(qwq[fst][x]==1)qwq[fst][x]=0,bo[fst]--;
            else {qwq[fst][x]=1,bo[fst]++,ans=max(ans,fst*1000+x),now=-114514;break;}
        }
        now=a[k];
        if(tg[(now-1)/1000]==1)
        {
        ...
        for(int x=(now-1)%1000+1;x<=1000;x++)
        if(qwq[(now-1)/1000][x]==0)qwq[(now-1)/1000][x]=1,bo[(now-1)/1000]++;
        else 
        {
            qwq[(now-1)/1000][x]=0,bo[(now-1)/1000]--;
            {
                ans-=((now-1)/1000*1000+x==ans);
                if(qwq[(ans-1)/1000][(ans-1)%1000+1]==0)
                {
                    for(int u=(ans-1)/1000;u>=0;u--)
                    if(bo[u]!=0)
                    for(int v=1000;v>=1;v--)
                    if(qwq[u][v]==1)
                    {
                        ans=u*1000+v;goto nxt;
                    }
                }
                nxt:;
            }
        ...
        if(now!=-114514)
        {
            ...
            for(int x=1;x<=1000;x++)
            if(qwq[fst][x]==0)qwq[fst][x]=1,bo[fst]++;
            else {qwq[fst][x]=0,bo[fst]--,ans-=(fst*1000+x==ans),now=-114514;break;}
        }
        a[k]=l;
    }
    cout<<ans<<endl;
