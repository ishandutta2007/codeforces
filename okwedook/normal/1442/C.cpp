...
vector<ll> dijkstra(const vector<vector<pair<int, ll>>> &G, int v) {
...
vector<pll> dijkstra2(const vector<vector<pair<int, pll>>> &G, int v) {
    vector<pll> ans(sz(G), {inf, 0});
    ans[v] = {0, 0};
    auto cmp = [&](int i, int j) { return ans[i] > ans[j]; };
    priority_queue<int, vector<int>, decltype(cmp)> que(cmp);
    vector<bool> flag(sz(G));
    ...
    while (!que.empty()) {
        ...
        flag[f] = true;
        for (auto i : G[f])
            if (chmin(ans[i.f], sum(ans[f], i.s))) {
                que.push(i.f);
                flag[i.f] = false;
            }
    }
...
signed main() {
    ...
    int n, m;
    read(n, m);
    vector<vector<pair<int, ll>>> G(n + 1), rev(n + 1);
    vector<pii> edge;
    for (int i = 0; i < m; ++i) {
        int u, v;
        read(u, v);
    ...
    for (int level = 0; level < LOG; ++level) {
        auto newG = level & 1 ? rev : G;
        for (int i = 0; i < n; ++i) newG[n][i] = {i, sh[i]};
        auto nsh = dijkstra(newG, n);
        chmin(ans, nsh[n - 1]);
        sh = move(nsh);
        sh.popb();
        for (auto &i : sh) i += 1ll << level;
    }
    dbg(ans);
    if (ans == inf) {
        vector<vector<pair<int, pll>>> rev(2 * n);
        for (int i = 0; i < n; ++i) {
            rev[i].pb({i + n, {1, 0}});
            rev[i + n].pb({i, {1, 0}});
        }
        for (auto i : edge) {
            rev[i.f].pb({i.s, {0, 1}});
            rev[i.s + n].pb({i.f + n, {0, 1}});
        }
        auto sh2 = dijkstra2(rev, 0);
        pll ans1 = sh2[n - 1];
        pll ans2 = sh2[2 * n - 1];
        chmin(ans1, ans2);
        zet check = ans1.s;
        dbg(ans);
        dbg(ans1.f, ans1.s);
        for (int i = 0; i < ans1.f; ++i) check += mypow(zet(2), i);
        ans = check.val;
    }
    ans %= mod;
    println(ans);
