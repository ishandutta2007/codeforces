...
inline void solve() {
	...
	dp[0][0] = 1;
	for (int i = 1; i <= n; ++i) {
		dp[i][0] += (dp[i - 1][0] * cnt0) % MOD;
		dp[i][0] += (dp[i - 1][1] * cnt2) % MOD;
		dp[i][0] += (dp[i - 1][2] * cnt1) % MOD;
		dp[i][0] %= MOD;


		dp[i][1] += (dp[i - 1][0] * cnt1) % MOD;
		dp[i][1] += (dp[i - 1][1] * cnt0) % MOD;
		dp[i][1] += (dp[i - 1][2] * cnt2) % MOD;
		dp[i][1] %= MOD;


		dp[i][2] += (dp[i - 1][0] * cnt2) % MOD;
		dp[i][2] += (dp[i - 1][1] * cnt1) % MOD;
		dp[i][2] += (dp[i - 1][2] * cnt0) % MOD;
		dp[i][2] %= MOD;
	}
