...
struct BFS {
    ...
    void bfs(const vector<int> &source) {
        ...
        for (int src : source)
            bfs_check(q, next_q, src, src, 0, 0);
        ...
        while (!q.empty() || !next_q.empty()) {
            while (!q.empty()) {
                ...
                for (edge &e : adj[top])
                    bfs_check(q, next_q, e.node, root[top], dist[top] + e.weight, e.weight);
            }
...
int main() {
    ...
    for (int i = 0; i < N - 1; i++)
        if (bfs.root[edges[i][0]] != bfs.root[edges[i][1]])
            remove.push_back(i);

    // assert(int(remove.size()) == K - 1);
    cout << remove.size() << '\n';
    output_vector(remove, true);
}