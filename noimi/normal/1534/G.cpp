 ...
 */
template <typename T> struct SlopeQueue {
...
template <typename T = long long, const int sz = 3 * 10000000> struct IO {
...
int main() {
    ...
    auto id = iota(n);
    sort(all(id), [&](int i, int j) { return x[i] + y[i] < x[j] + y[j]; });
