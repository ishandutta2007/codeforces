...
public:
    void solve() {
        ...
        ll answer = 0;
        arr<bool> processed(n, false);
        arr<multiset<int>> costUp(n, multiset<int>());
        arr<multiset<int>> costDown(n, multiset<int>());
        RecursiveFunction dfs = [&](const auto& self, int vert, int last) -> void {
            ...
            ll times = max({up[vert], down[vert], (up[vert] + down[vert] + any[vert] + 1) / 2});
            while (up[vert] > down[vert] + any[vert] + 1 && !costDown[vert].empty() &&
                   *costDown[vert].begin() < t[vert]) {
                answer += *costDown[vert].begin();
                costDown[vert].erase(costDown[vert].begin());
                times--;
                up[vert]--;
                down[vert]++;
            }
            while (down[vert] > up[vert] + any[vert] + 1 && !costUp[vert].empty() && *costUp[vert].begin() < t[vert]) {
                answer += *costUp[vert].begin();
                costUp[vert].erase(costUp[vert].begin());
                times--;
                down[vert]--;
                up[vert]++;
            }
            answer += times * t[vert];
            if (last != -1) {
                if (up[vert] >= down[vert] + any[vert]) {
                    up[last]++;
                    costDown[last].insert(costDown[vert].empty() ? t[vert] : min(t[vert], *costDown[vert].begin()));
                } else if (down[vert] >= up[vert] + any[vert]) {
                    down[last]++;
                    costUp[last].insert(costUp[vert].empty() ? t[vert] : min(t[vert], *costUp[vert].begin()));
                } else {
                    any[last]++;
                }
            }
