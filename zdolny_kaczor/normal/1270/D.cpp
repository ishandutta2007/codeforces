...
pii query(vector <int> p) {
	...
	assert(cou <= n);
	vpii s;
	for (int x : p) s.emplace_back(a[x], x);
	sort(ALL(s));
	return {s[m - 1].second, s[m - 1].first};
#else
	printf("?");
...
int main() {
	...
	scanf("%d", &m);
	REP(i, n) scanf("%d", a + i);
	REP(i, n) REP(j, i) assert(a[i] != a[j]);
#endif
	map <int, int> cou;
	REP(i, k + 1) {
		vector <int> q;
		REP(j, k + 1) if (i != j) q.push_back(j);
		ans[i] = query(q);
		cou[ans[i].second]++;
		debug(arr(ans, i));
	}
	debug(imie(cou));
	answer(cou.rbegin()->second);
}