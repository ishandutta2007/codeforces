...
struct solver
{
    ...
    solver()
    {
        ...
        for(i64 x_i=x0,y_i=y0; x_i<=2e16 and y_i<=2e16; x_i=x_i*ax+bx,y_i=y_i*ay+by)
        {
            da.emplace_back(x_i,y_i);
        }
        dump(da);

        i64 x_s,y_s,t; cin>>x_s>>y_s>>t;
        int ans=0;

        for(auto itr=da.begin(); itr!=da.end(); ++itr)
        {
            i64 tt=t-abs(x_s-itr->first)-abs(y_s-itr->second);
            if(tt<0) continue;

            {
                ...
                {
                    --p;
                    ttt-=abs(x-p->first)+abs(y-p->second);
                    if(ttt>=0)
                    {
                        cnt++;
                        tie(x,y)=*p;
