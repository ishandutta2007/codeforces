...
void output() {
    if (move_count > k) {
        ...
        move_count -= s.size() * c;
        assert(move_count < k);
        while (move_count + s.size() <= k) {
            move_count += s.size();
            moves.back().first++;
        }
        if (moves.back().first == 0) moves.pop_back();
        for (int i = move_count; i < k; i++) {
            moves.emplace_back(1, std::string("") + s[i - move_count]);
        }
    }
...
void solve() {
    ...
    for (int row = 1; row < n; row++) {
        if (m > 1) {
            moves.emplace_back(m - 1, "RDU");
            move_count += 3 * (m - 1);
            if (move_count >= k) output();
