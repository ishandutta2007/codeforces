...
int n, m, fa[100005];
signed main(){
    read(n, m); if(n == 1) return print(m? "NO" : "YES\n0"), 0;
    if((n == 9 && m == 2) || !(n & 1) || (m > ((n - 3) >> 1))) return print("NO"), 0;
    if((n & (n + 1)) && m == 0) return print("NO"), 0;
    if(!(n & (n + 1)) && m == 1) return print("NO"), 0;
    print("YES"); m = mx(0, (m - 1) << 1);
    for (int i = 2;i <= m;i += 2) fa[i] = fa[i + 1] = i - 1;
    rep(i, m + 2, n) fa[i] = ((i - m) >> 1) + m;
    n -= m; if(!(n & (n + 1)) && m) fa[n + m] = fa[n + m - 1] = 2;
    print(fa + 1, fa + n + m + 1);
}