...
vector<int> fib(30), invfib(200200, -1);
fib[0] = fib[1] = 1;
invfib[1] = 1;
for(int n = 2; n < 30; ++n) {
...
bool final_ans = y_combinator([&](auto check_fib, int cnt, int n0) -> bool {
	if(cnt <= 3) {
		return true;
	}
	int f = invfib[cnt];
	...
	int u = -1, v = -1;
	y_combinator([&](auto dfs, int n, int p) -> void {
		subtree[n] = 1;
		for(int x : G[n]) {
			if(x == p) {
				continue;
		...
		if(subtree[n] == fib[f - 1] or subtree[n] == fib[f - 2]) {
			u = n;
			v = p;
		}
	})(n0, -1);
	...
	G[v].erase(G[v].begin() + to_del);
	int tmp = subtree[u];
	return check_fib(tmp, u) and check_fib(cnt - tmp, v);
})(N, 0);

[&]() { auto& out = cout; put_stream(out, "", make_string_formatter( (final_ans) ? ("YES") : ("NO") ), "\n");  }();
