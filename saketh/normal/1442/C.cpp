...
int main() {
    ...
    pii INF = {INT_MAX/2, INT_MAX/2};

    auto lt = [](pii a, pii b) {
        if (a.f == b.f) return a.s < b.s; // num tpos same, just compare step count

        if (max(a.f, b.f) >= MPOS - 2) { // number of steps is at most 2N-1, so whoever has more tpos is bigger
            return a.f < b.f;
        }

        int atot = (1 << a.f) - 1 + a.s;
        int btot = (1 << b.f) - 1 + b.s;
    ...
    auto cmp = [&](pair<pii, int> a, pair<pii, int> b) {
        if (a.f != b.f) return lt(b.f, a.f); // want smallest distances first
        return a.s < b.s; // arbitrary tie-break
    };

    vpii dist(MPOS * N, INF);
    dist[0] = {0,0};

    priority_queue<pair<pii, int>, vector<pair<pii, int>>, decltype(cmp)> pq(cmp);
    pq.push(mp(dist[0], 0));
    ...
    while (!pq.empty()) {
        ...
        int node = key / MPOS;
        int tpos = key % MPOS; // not actual cost, but capped to MPOS

        // use the graph without transposing it
        trav (nbr, adj[tpos&1][node]) {
            int nkey = MPOS * nbr + tpos;
            pii ncost = mp(cost.f, cost.s + 1);
            if (lt(ncost, dist[nkey])) {
                dist[nkey] = ncost;
                pq.push(mp(ncost, nkey));
            }
        }

        // transpose and use
        trav (nbr, adj[(tpos&1)^1][node]) {
            int ntpos = tpos + 1;
            if (ntpos == MPOS) ntpos -= 2;
            int nkey = MPOS * nbr + ntpos;
            pii ncost = mp(cost.f + 1, cost.s + 1);
            if (lt(ncost, dist[nkey])) {
                dist[nkey] = ncost;
                pq.push(mp(ncost, nkey));
            }
    ...
    pii ans = INF;
    F0R (tpos, MPOS) {
        pii cand = dist[(N - 1) * MPOS + tpos];
        if (lt(cand, ans))
            ans = cand;
    }

    ps(mn(2).pow(ans.f) - 1 + ans.s);
