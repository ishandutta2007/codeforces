...
void solve(){
    ...
    RangeMinQuery<ll> mn(a);
    RangeMaxQuery<ll> mx(a);
    ...
    Tree dp(n);
    for (int i = n-l; i >= 0; i--) {
        int lo = i+l-1, hi = n, mid = (lo+hi)/2;
        if (cost(i, lo) > s) continue;
        if (cost(i, n-1) <= s) {
            ll c = 1;
            dp.update(i, c);
            continue;
        ...
        while (lo < mid && mid < hi) {
            if (cost(i, mid) <= s) lo = mid;
            else hi = mid;
            mid = (lo+hi)/2;
        }
        ll c = dp.query(i+l, lo+1)+1;
        dp.update(i, c);
    }
    cout << (dp.query(0, 0) >= MOD ? -1 : dp.query(0, 0)) << '\n';
}
int main(){
