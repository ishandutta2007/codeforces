...
#define REPS(...) REPSname(__VA_ARGS__, REPS1, REPS0)(__VA_ARGS)
...
void main_() {
	...
	SegmentTreeBeats<int> beats(cum);
	REP(i, m) {
		auto [l, r] = lr[i];
		int mi = 0, ma = 0;
		chmin(mi, beats.query_min(0, l));
		chmax(ma, beats.query_max(0, l));
		int base = cum[r] - cum[l - 1];
		chmin(mi, beats.query_min(r + 1, n + 1) - base);
		chmax(ma, beats.query_max(r + 1, n + 1) - base);
		print(ma - mi + 1);
	}
}
