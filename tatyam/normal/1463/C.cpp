...
void solve(){
    LL(n);
    ll x1=0,moveto=0,ans=0;
    VEC(pll,a,n);
    a.emplace_back(LINF,0);
    rep(n){
        auto[t1,x]=a[i];
        const ll t2=a[i+1].first;
        if(x1==moveto)moveto=x;
        const ll x2=clamp(moveto,x1-(t2-t1),x1+(t2-t1));
        if(x1<=x&&x<=x2||x2<=x&&x<=x1)ans++;
        x1=x2;
    }
    out(ans);
}
