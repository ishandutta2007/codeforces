...
int main() {
  ...
  sort(ALL(a));
  int l = 0;
  vector dp(n + 1, vector(n + 1, ModInt(0)));
  dp[0][0] = 1;
  vector<ModInt> sum(n + 1, ModInt(0));
  sum[0] = 1;
  REP(i, n) {
    while (l < i && a[l] * 2 <= a[i]) {
      ++l;
      REP(j, n + 1) sum[j] += dp[l][j];
    }
    // REP(k, l + 1) REP(j, k + 1) dp[i + 1][j + 1] += dp[k][j];
    REP(j, l + 1) dp[i + 1][j + 1] += sum[j];
    REP(j, l + 1) dp[i + 1][j + 1] += dp[i + 1][j] * (l + 1 - j);
  }
  cout << dp[n][n] << '\n';
  return 0;
}