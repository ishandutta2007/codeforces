...
struct Matrix{
	...
	Matrix operator*(Matrix& ope){
		...
		rep(i, 1, h) rep(k, 1, w){
			...
			rep(j, 1, ope.w) ret.at(i, j) |= ope.at(k, j);
		}
		return ret;
	}
	Matrix pow(llint n){
...
ll calc()
{
	Matrix mat(n, n), pos(n, 1);
	mat.at(n, n) = 1, pos.at(1, 1) = 1;
	
	ll ans = 0;
	rep(i, 0, sz(vec)-2){
		vector<P> &evec = mp[vec[i]];
		for(auto p : evec) mat.at(p.second, p.first) = 1;
		
		Matrix amat[35];
		ll logn = digitnum(vec[i+1]-vec[i], 2);
		
		amat[0] = mat;
		rep(i, 1, logn) amat[i] = amat[i-1] * amat[i-1];
		
		per(j, logn, 0){
			if(ans + (1<<j) > vec[i+1]) continue;
			Matrix npos = amat[j] * pos;
			if(npos.at(n, 1) == 0) pos = npos, ans += 1<<j;
		}
		
		if(ans < vec[i+1]) return ans + 1;
	}
	return inf;
}
...
int main(void)
{
	...
	for(auto p : mp) vec.push_back(p.first);
	vec.push_back(0), vec.push_back(1e9+500);
	uniq(vec);
	
	ll ans = calc();
	
	if(ans > inf/2) outl("Impossible");
	else outl(ans);
