...
int main() {
    ...
    F0R (i, N - 2) {
        // 1, 1, 1 -> 1, 0, 0 on the leading bits
        if (__builtin_clz(a[i]) == __builtin_clz(a[i + 2])) {
            ps(1);
            return 0;
        }
    }
    ...
    F0R (i, N) {
        int lval = 0;
        for (int left = 1; i + left <= N; left++) {
            lval ^= a[i + left - 1];

            int rval = 0;
            for (int right = 1; i + left + right <= N; right++) {
                rval ^= a[i + left + right - 1];

                if (lval > rval)
                    ckmin(ans, left - 1 + right - 1);
            }
        }
    }
