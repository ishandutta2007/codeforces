...
int main(){
    ...
    unordered_set<node*>leaf[4];
    auto dfs=[&](node*p,auto dfs)->void{
        if(p->child.empty())leaf[p->f<<1|bool(p->p)].insert(p);
        else each(i,p->child)dfs(i,dfs);
    };
    each(p,tree)dfs(&p,dfs);
    auto erase=[&](node*p){
        out(p->a);
        leaf[p->f<<1|bool(p->p)].erase(p);
        const auto q=p->p;
        if(q){
            q->child.erase(find(all(q->child),p));
            if(q->child.empty())leaf[q->f<<1|bool(q->p)].insert(q);
        }
    };
    rep(n*2){
        ll x=i&1^1;
        x<<=1;
        if(leaf[x|1].size())x|=1;
        erase(*leaf[x].begin());
    }
}