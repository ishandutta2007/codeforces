...
vector<int> factor(int n){
	...
	reps(i,2,MOD){
		if(i*i>n) break;
		for(;n%i==0;n/=i) re.eb(i);
	}
	if(n>1) re.eb(n);
	return re;
}

bool solve(const int x, const int d){
	auto rem=x;
	int cnt=0;
	while(rem%d==0) ++cnt, rem/=d;
	out(x,d,rem,cnt,1);
	cauto rems=factor(rem);
	cauto facts=factor(d);
	if(cnt<=1) return false;
	if(rems.size()>1) return true;
	if(cnt==2) return false;
	if(facts.size()==1) return false;
	// d,rem: comp
	if(facts[0]!=facts.back()) return true;
	int q=facts.size();
	for(cauto &p:rems)if(p==facts[0]) ++q;
	return ((int)(facts.size()-1)*(cnt-1)>=q);
}
