...
int n, m, k, a[MAXN], b[MAXN], c[MAXN << 1];
int A[MAXN << 1], B[MAXN << 1];
signed main(){
    read(n, m, k); read(a + 1, a + n + 1); read(b + 1, b + m + 1);
    memcpy(c + 1, a + 1, n * sizeof(int)); memcpy(c + n + 1, b + 1, m * sizeof(int));
    std :: sort(c + 1, c + (m + n) + 1); k = std :: unique(c + 1, c + (n + m) + 1) - c - 1;
    rep(i, 1, n) ++ A[std :: lower_bound(c + 1, c + k + 1, a[i]) - c];
    rep(i, 1, m) ++ B[std :: lower_bound(c + 1, c + k + 1, b[i]) - c];
    per(i, k, 1) {
        A[i] += A[i + 1]; B[i] += B[i + 1];
        if(A[i] > B[i]) return print("YES"), 0;
    } print("NO");
}