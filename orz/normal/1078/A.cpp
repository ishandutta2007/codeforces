...
int main()
{
	...
	cout << fixed << setprecision(30) << min(ans, min(min(dist(z1, h1) + dist(h1, h2) + dist(h2, z2),
		dist(z1, h1) + dist(h1, v2) + dist(v2, z2)),
		min(dist(z1, v1) + dist(v1, h2) + dist(h2, z2),
			dist(z1, v1) + dist(v1, v2) + dist(v2, z2)))) << '\n';
	return 0;
}