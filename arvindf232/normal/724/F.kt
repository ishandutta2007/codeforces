...
//        pI = 177545087
        ...
        val board = Array(d+1){IntArray(n+1)}
        board[0][1] = 1
        // root degree, number of vertices
        var centroidBreak = -1
        val needcentroid = (n-1)/2
        for(vc in 1..minOf(needcentroid+1,(n-1))){
            var numtreehere = 0
            for(ds in 0 until d){
                numtreehere = (numtreehere + board[ds][vc]).snap()
            }
            val picking = IntArray(d+1){FACT.bigChoose(numtreehere + it -1,it)}
            for(x in d downTo 0){
                for(y in n downTo 0){
                    val now = board[x][y]
                    var yy = y
                    ...
                    for(outing in 1..d){
                        ...
                        board[xx][yy] = (board[xx][yy] + (picking[outing] modM now)).snap()
                    }
                }
            }
            if(vc == needcentroid){
                centroidBreak = 0
                for(ds in 0..d){
                    centroidBreak = (centroidBreak + board[ds][n]).snap()
                }
            }
        }
        ...
        for(ds in 0 until d){
            for(i in 0 until n+1){
                total.put(i,board[ds][i])
            }
        }
//        var final = total[n] + (if(n %2 == 0) total[n /2].modDivide(2) else 0)
//        var lastterm = 0
//        for(i in 0..n){
//            val other = n-i
//            lastterm = (lastterm + (total[i] modM total[other])).snap()
//        }
        var final = centroidBreak
        if(n % 2 == 0){
            val tochoose = total[n/2]
            final = (final + (tochoose modM (tochoose modPlus 1)).modDivide(2)).snap()
        }
        put(final)
//        var ret = 0
//        for(left in 1..(n-1)) {
//            val right = n - left
//            ret = (ret + (total[left] modM FACT.store[left-1] modM total[right] modM FACT.store[right-1] modM FACT.choose(n-2,left-1)))
//            just dei ret
////            if(left != right){
////                ret = (ret + (total[left] modM total[right])).snap()
////            }else{
////                val s = total[left]
////                ret = (ret + (s modM (s+1) modDivide 2)).snap()
////            }
//        }
//        put(ret modDivide (FACT.store[n-1]))
//        val x = 0
    }
    done()
}
...
10 3 1000000007
7 2 433416647
 modM always need a bracketrs

genreating function: adding function 1 is not thes ame as adding 1 to answer
Centroid recursion induces many constaints, not all are observed carefully ( d or d+1)
very dead on n =2, n = 1 cases
legacy spahetti on snapping
