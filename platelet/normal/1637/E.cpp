...
struct node {
    int u, v, a, b;
    ll val()const { return ll(u + v) * (::v[u][a] + ::v[v][b]); }
...
ll solve() {
    ...
    set<pair<int, int>> s;
    rep(i, 1, n) cin >> x, cnt[x]++, v[i].clear();
    for(auto [x, y] : cnt) v[y].pb(x);
    while(m--) cin >> x >> y, s.emplace(min(x, y), max(x, y));
    priority_queue<node> q;
    rep(i, 1, n) if(SZ(v[i])) rep(j, 1, i) if(SZ(v[j])) For(k, 0, SZ(v[i])) q.push({i, j, k, 0});
    while(1) {
        auto [u, v, a, b] = q.top(); q.pop();
        int x = ::v[u][a], y = ::v[v][b];
        if(x != y && !s.count(pair(min(x, y), max(x, y)))) return ll(u + v) * (x + y);
        if(b + 1 < SZ(::v[v])) q.push({u, v, a, b + 1});
    }
}
int main() {
#ifdef local
