...
class DAKhoroshayaStroka {
public:
    void solve() {
        int n = in.readInt();
        auto s = in.readString();

        int level = bitCount(unsigned(n - 1));
        arri res(n);
        for (int i : range(n)) {
            res[i] = s[i] == 'a' + level ? 0 : 1;
        }
        for (int i : RevRange(level)) {
            int shift = 1 << (level - i);
            for (int j = 0; j < n; j += shift) {
                int left = res[j + shift / 2];
                for (int k : range(j, j + shift / 2)) {
                    left += s[k] == 'a' + i ? 0 : 1;
                }
                int right = res[j];
                for (int k : range(j + shift / 2, j + shift)) {
                    right += s[k] == 'a' + i ? 0 : 1;
                }
