...
struct fastin{
	...
	//	freopen("tmp.in","r",stdin);
		fread(ch,1,SIZE,stdin);
	}
	template<typename T>inline fastin&operator>>(T&x){
		register bool p=0;
   		while(!isdigit(*++now))if(*now=='-')p=1;
  	  	x=*now-'0';
    	while(isdigit(*++now))x=x*10+*now-'0';
    	if(p)x=-x;
    	return*this;
	}
	inline int read(){register int x;*this>>x;return x;}
}din;
#define read din.read
