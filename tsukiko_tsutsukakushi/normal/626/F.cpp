...
void solve() {
    ...
    VEC(int, a, n);
    Sort(a);
    // vector dp(A, vector(n + 1, vc<mint>(k + 1, 0)));
    vector dp(n + 1, vc<mint>(k + 1, 0));
    auto ndp = dp;
    dp[0][0] = 1;
    rep(i, n) {
        ndp.assign(n + 1, vc<mint>(k + 1, 0));
        int len = (i == 0 ? a[0] : a[i] - a[i - 1]);
        for(int j = 0; j <= i; ++ j) {
            for(int x = 0; x <= k; ++ x) {
                if(j >= 1 and x + len * j <= k) {
                    ndp[j - 1][x + len * j] += dp[j][x] * j;
                }
                if(x + len * j <= k) {
                    ndp[j][x + len * j] += dp[j][x] * (j + 1);
                }
                if(x + len * j <= k) {
                    ndp[j + 1][x + len * j] += dp[j][x];
                }
            }
        }
        swap(dp, ndp);
