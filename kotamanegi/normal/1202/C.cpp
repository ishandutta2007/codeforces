...
void solve(){
    ...
    nexts.push_back(mp(0, 0));
    REP(i, s.length()) {
        nexts.push_back(nexts.back());
        if (s[i] == 'W') {
            nexts.back().first++;
        }
        else if (s[i] == 'S') {
    ...
    go.push_back(mp(mp(nexts.back().first, nexts.back().first), mp(nexts.back().second, nexts.back().second)));
    for (int i = (int)nexts.size() - 2; i >= 0; --i) {
        go.push_back(go.back());
        pair<pair<int, int>, pair<int, int>>& x = go.back();
        x.first.first = min(x.first.first, nexts[i].first);
        x.first.second = max(x.first.second, nexts[i].first);
        x.second.first = min(x.second.first, nexts[i].second);
        x.second.second = max(x.second.second, nexts[i].second);
    }
    reverse(ALL(go));
    int ans = 1e18;
    ...
    REP(i, go.size()) {
        {
            int dx = max(X.second, go[i].first.second) - min(X.first, go[i].first.first) + 1;
            int dy = max(Y.second, go[i].second.second) - min(Y.first, go[i].second.first) + 1;
            ans = min(ans, dx * dy);
        }
        {
            int dx = max(X.second, go[i].first.second + 1) - min(X.first, go[i].first.first + 1) + 1;
            int dy = max(Y.second, go[i].second.second) - min(Y.first, go[i].second.first) + 1;
            ans = min(ans, dx * dy);
        }
        {
        ...
        {
            int dx = max(X.second, go[i].first.second) - min(X.first, go[i].first.first) + 1;
            int dy = max(Y.second, go[i].second.second + 1) - min(Y.first, go[i].second.first + 1) + 1;
            ans = min(ans, dx * dy);
        }
        {
            int dx = max(X.second, go[i].first.second) - min(X.first, go[i].first.first) + 1;
        ...
        if (i + 1 < nexts.size()) {
            X.first = min(X.first, nexts[i + 1].first);
            X.second = max(X.second, nexts[i + 1].first);
            Y.first = min(Y.first, nexts[i + 1].second);
            Y.second = max(Y.second, nexts[i + 1].second);
        }
     }
    cout << ans << endl;
}
