...
int main(){
  ...
  vec timestamp(N,-1);
  set<ll> se;
  rep(i,N)se.emplace(i);
  while(Q--){
    ll ch;
    cin>>ch;
    if(ch){
      cin>>i;
      if(timestamp[--i]!=-1)ans[query.size()]=1;
      query.emplace_back(ev.size(),i);
    }else{
      ll L,R,f;
      cin>>L>>R>>f;
      ev.emplace_back(--L,R,f);
      if(f==0){
        auto itr=se.lower_bound(L);
        while(itr!=se.end()&&*itr<R){
          timestamp[*itr]=ev.size();
          itr=se.erase(itr);
        }
      }
    }
  }
  segtree<ll> seg(timestamp,-1,[](ll a,ll b){return max(a,b);});
  for(auto& x:timestamp)if(x==-1)x=inf;
  rep(i,ev.size()){
    auto [L,R,f]=ev[i];
    if(f==0)continue;
    auto itr=se.lower_bound(L);
    if(itr==se.end()||*itr>=R)continue;
    auto sitr=next(itr);
    if(sitr!=se.end()&&*sitr<R)continue;
    chmin(timestamp[*itr],max(seg.prod(L,R),i+1));
  }
  rep(i,query.size()){
    auto [t,id]=query[i];
    if(ans[i]){
