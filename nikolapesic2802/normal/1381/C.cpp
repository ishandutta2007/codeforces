...
int main()
{
	for(int t=ri();t--;){
		...
		for(int i=0;i<x;i++){
			auto tr=*s.rbegin();
			s.erase(tr);
			sol[pos[tr.s].back()]=tr.s;
			pos[tr.s].pop_back();
			if(pos[tr.s].size())
				s.insert({pos[tr.s].size(),tr.s});
		}
		...
		for(int i=1;i<c.size();i++){
			for(int j=0;y&&j<c[i].f;j++){
				sol[pos[c[tr].s].back()]=c[i].s;
				pos[c[tr].s].pop_back();
				if(pos[c[tr].s].empty())
					tr++;
				y--;
		...
		if(tr==0)
			tr++;
		for(int j=0;y&&j<c[0].f&&tr<c.size();j++){
			sol[pos[c[tr].s].back()]=c[0].s;
			pos[c[tr].s].pop_back();
			if(pos[c[tr].s].empty())
				tr++;
			y--;
