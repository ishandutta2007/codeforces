...
int main() {
    ...
    rep(i,1,n+1) {
        if(i % 2) {
            if(mp1.find(cs[i]) != mp1.end()) pre[i] = mp1[cs[i]];
            mp2[cs[i]] = i;
        }
        else {
            if(mp2.find(cs[i]) != mp2.end()) pre[i] = mp2[cs[i]];
            mp1[cs[i]] = i;
        }
    }
    debug(pre);
    rep(i,q) {
        ...
        l--;
        if(cs[l] != cs[r]) cout << -1 << '\n';
        else if (fw.prod(l,r) == 0) cout << 0 << '\n';
        else if ((r - l) % 2) cout << 1 << '\n';
        else {
            if(a[r-1] == 0 || a[l] == 0) cout << 1 << '\n';
