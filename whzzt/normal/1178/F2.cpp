...
int calc(int x){ return mul(mod/2+1,mul(x,x-1)); }
const int N=1e6+5,M=505,L=1005;
#define fmin loc_fmin
int n,m,c[N],mn[M],mx[M],fmin[L][L],f[L][L],g[L][L];
vector<int> a;
int main(){
	...
	read(c+1,c+m+1);
	a.pb(c[1]);
	for(int i=2;i<=m;++i) if(a.back()!=c[i]) a.pb(c[i]);
	if(a.size()>n*2+1) cout<<"0\n",exit(0);
	for(int i=1;i<=n;++i) mn[i]=N,mx[i]=0;
	int len=a.size();
	for(int i=0;i<len;++i) mn[a[i]]=min(mn[a[i]],i),mx[a[i]]=max(mx[a[i]],i);
	for(int i=0;i<len;++i) fmin[i][i]=a[i];
	for(int i=0;i<len;++i) for(int j=i+1;j<len;++j) fmin[i][j]=min(fmin[i][j-1],fmin[j][j]);
	for(int z=0;z<len;++z){
		for(int l=0;l+z<len;++l){
			int r=l+z;
			int c=fmin[l][r],p=l-1;
			if(mn[c]>=l&&mx[c]<=r){
				f[l][r]=1;
				for(int k=l;k<=r;++k) if(a[k]==c){
					if(p+1<=k-1) f[l][r]=mul(f[l][r],g[p+1][k-1]);
					p=k;
				}
				if(p+1<=r) f[l][r]=mul(f[l][r],g[p+1][r]);
			}
			g[l][r]=f[l][r];
			for(int k=l;k<r;++k) g[l][r]=add(g[l][r],mul(g[l][k],f[k+1][r]));
		}
	}
	cout<<g[0][len-1]<<'\n';
	return 0;
