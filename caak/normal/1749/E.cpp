...
inline void AC() {
    ...
    for(_ i = 0; i <= n+3; ++i) for(_ j = 0; j <= m+3; ++j) d[i][j] = 1E17;
    q.clear();
    for(_ i = 1; i <= n; ++i) if(ok(i, 1)) {
        if(a[i][1]) {
            d[i][1] = 0;
            q.push_front(mp(i, 1));
        } else {
    ...
    for(; !q.empty(); ) {
        ...
        q.pop_front();
        if(t.second == m) {
            if(s > d[t.first][t.second]) {
                s = d[t.first][t.second];
                f = t.first;
            }
        }
        for(_ i = 0; i < 4; ++i) {
            _ nx = t.first+dx[i], ny = t.second+dy[i];
            if(nx <= 0 || ny <= 0 || nx > n || ny > m || !ok(nx, ny)) continue;
            _ ds = a[nx][ny] ? 0 : 1;
            if(d[nx][ny] > d[t.first][t.second]+ds) {
                d[nx][ny] = d[t.first][t.second]+ds;
                px[nx][ny] = t.first;
                py[nx][ny] = t.second;
                if(ds) q.push_back(mp(nx, ny));
    ...
    if(s > 1E16) no(7);
        ...
        for(; t.second != 1; ) {
            ...
            _p nw = mp(px[t.first][t.second], py[t.first][t.second]);
            t = nw;
        }
        a[t.first][t.second] = 1;
        for(_ i = 1; i <= n; ++i, clr()) for(_ j = 1; j <= m; ++j) write(a[i][j] ? '#' : '.');
    }
