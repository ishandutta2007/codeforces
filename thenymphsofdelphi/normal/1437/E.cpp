...
const int N = 5e5 + 5;
int mod = 1e9 + 7, mod1 = 998244353, mod2 = 1e9 + 9, inf = 1e9 + 2 * N + 7;
ll infll = 1e18 + 7;

int lis(int *a, int l, int r, int lval, int rval){
    // ForE(i, l - 1, r + 1){
    //     cout << a[i] - i << ' ';
    // } cout << endl;
    int cac[N], ans = 0;
    ForE(i, l, r){
        a[i] -= i;
        if (a[i] < lval || rval < a[i]){
            continue;
        }
        // cout << l << ' ' << r << ' ' << lval << ' ' << rval << ' '
        //      << i << ' ' << a[i] << ' ' << lval + (i - l + 1) << ' ' << rval - (r - i + 1)
        //      << endl;
        if (cac[ans] <= a[i]){
            cac[++ans] = a[i];
            continue;
        }
        int itr = upb(cac + 1, cac + 1 + ans, a[i]) - cac;
        cac[itr] = a[i];
        a[i] += i;
    }
    return ans;
...
signed main(){
    ...
    a[0] = -inf; a[n + 1] = inf; b[0] = 0; b[k + 1] = n + 1;
    ForE(i, 1, k + 1){
        if (b[i] - b[i - 1] - 1 > a[b[i]] - a[b[i - 1]] - 1) Ptest(-1);
    }
    ForE(i, 1, k + 1){
        ans += lis(a, b[i - 1] + 1, b[i] - 1, a[b[i - 1]] - b[i - 1], a[b[i]] - b[i]);
        // cout << ans << endl;
    }
    cout << n - (ans + k);
}
