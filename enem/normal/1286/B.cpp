...
pair<bool,V(ll)> dfs(ll x,V(ll) chl[],ll c[],ll f[],ll a[],ll n)
{
    bool flag=0;
    V(ll) v;
    ll t=c[x];
    ...
    if(flag==0) return MP(false,v);
    for(auto it:chl[x])
    {
        auto p=dfs(it,chl,c,f,a,n);
        if(!(p.F)) return MP(false,v);
        for(auto itt:p.S) v.PB(itt);
    }
    t=c[x];
    for(auto it:v) if(it<a[x]) t--;
    if(t!=0) return MP(false,v);
    v.PB(a[x]);
    return MP(true,v);
}
