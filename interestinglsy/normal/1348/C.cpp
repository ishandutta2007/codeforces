...
void Solve(){
	int alive = k;
	{
		...
		int need = k;
		fORx(i,0,firstC){
			int t = min(need,cnt[i]);
			cnt[i] -= t;
			need -= t;
			if( i != firstC and t ) isMult = true;
		}
		if( isMult or accumulate(cnt,cnt+firstC,0) ){
			putchar(10);
			return;
		}
	}
	For0(uc,26){
		if(!cnt[uc]) continue;
		int acc = accumulate(cnt+uc+1,cnt+26,0);
		if(acc){
			For(fuckOI,cnt[uc]) putchar(uc+'a');
			alive = 1;
		}else{
			int full = cnt[uc]/alive;
			int rest = cnt[uc]%alive;
			For(fuckOI,full) putchar(uc+'a');
			if(rest){
				putchar(uc+'a');
				alive = rest;
			}
