...
int main(int argn, char **argv) {
    ...
    REP(I, T) {
        ...
        FOR(i, 1, n) REP(j, m) {
            cin >> a[i][j];
            seq.eb(a[i][j], i, j);
        }sort(all(seq), greater<tll>());

        vvl val(n+1); vl crt(n+1);
        vl arr[1ll << m];
        FOR(i, 1, n) arr[0].eb(i), val[i].eb(0);

        ll ans1, ans2;
        for (auto [num, i, j] : seq) {
            crt[i] += (1<<j);
            ll sz = val[i].size();
            REP(kk, sz) {
                ll newVal = val[i][kk]+(1<<j);
                val[i].eb(newVal);
                arr[newVal].eb(i);
            }
            ...
            ll rev = (1<<m)-1 - crt[i];
            if (arr[rev].size()>0) {
                ans1 = i, ans2 = arr[rev][0];
                // if (arr[rev][0] == i) ans2 = arr[rev][1];
                break;
            }
        }
